...
Процедура ПриЗаписи(Отказ)
	...
	Если ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации() Тогда
		...
		ТекстСообщения = НСтр("ru = 'Записана константа РазрешенаРаботаСНовостями
			...
			|en = 'Constant РазрешенаРаботаСНовостями is saved
		...
		ОбработкаНовостейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Изменение данных. Константы. РазрешенаРаботаСНовостями';
				|en = 'News. Data change. Constants. РазрешенаРаботаСНовостями'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
