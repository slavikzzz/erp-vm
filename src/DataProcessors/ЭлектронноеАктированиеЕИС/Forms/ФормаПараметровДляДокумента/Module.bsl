...
Процедура ДеревоДетализацииНоменклатураПриИзменении(Элемент)
	...
	Если ЕстьДубли(ТекущиеДанные.ИдентификаторДляАктированияЕИС, ТекущиеДанные.Номенклатура) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр(
			...
			|en = 'You cannot specify the same items upon item drill-down.'"));
		ТекущиеДанные.Номенклатура = ПустаяСсылкаНаНоменклатуру();
	КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыДеревоРаботыУслуги

&НаКлиенте
Процедура ДеревоРаботыУслугиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
...
Функция ПустаяСсылкаНаНоменклатуру()

	Шаблон = "Справочник.%1.ПустаяСсылка";
	ИмяСправочникаНоменклатура = ОбщегоНазначенияБЭД.ИмяПрикладногоСправочника("Номенклатура");
	ПутьКПустойСсылке = СтрШаблон(Шаблон, ИмяСправочникаНоменклатура);
	Возврат ПредопределенноеЗначение(ПутьКПустойСсылке);
...
Процедура ЗаполнитьДеревьяДанныхПоДокументу(
	...
	ЭлектронноеАктированиеЕИС.ЗаполнитьОграничениеТипаНоменклатуры(
		Элементы.ДеревоДетализацииНоменклатура, ТипНоменклатура, Перечисления.ТипыОбъектовЗакупкиЕИС.Товар);

	ЭлектронноеАктированиеЕИС.ЗаполнитьОграничениеТипаНоменклатуры(
		Элементы.ДеревоРаботыУслугиНоменклатура, ТипНоменклатура, Перечисления.ТипыОбъектовЗакупкиЕИС.Услуга);
...
Функция СохранитьНаСервере()
	...
	Если НаборПодписи.Количество() = 0 Тогда
		ОписаниеОшибки = НСтр("ru = 'Не заполнен список подписантов документа.';
								|en = 'The document signer list is not filled.'");
	...
	Если ОтображатьМестаПоставки Тогда
		...
		Если НаборМеста.Количество() = 0 Тогда
			ОписаниеОшибки = НСтр("ru = 'Не заполнен список место доставки.';
									|en = 'The delivery location list is not filled.'");
			ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки);
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
