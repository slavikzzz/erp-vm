...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ЗаголовокИнформации.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Выполняются дополнительные процедуры обработки данных на версию %1
			...
			|Operations with this data are temporarily restricted.'"), Метаданные.Версия);
...
Процедура ПроблемныеСитуацииНажатие(Элемент)
	...
	ОтборЖурнала.Вставить("СобытиеЖурналаРегистрации", НСтр("ru = 'Обновление информационной базы';
															|en = 'Infobase update'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()));
...
Функция СообщениеОРезультатахОбновления()
	...
	Если Прогресс.ВсегоОбработчиков = Прогресс.ВыполненоОбработчиков Тогда
		...
		Если Прогресс.ВсегоОбработчиков = 0 Тогда
			...
			ТекстСообщения = "";
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Все процедуры обновления выполнены успешно (%1)';
																							|en = 'All update procedures are completed (%1).'"), Прогресс.ВыполненоОбработчиков);
		КонецЕсли;
...
Процедура ОбработатьРезультатУстановкиИсправлений(ИнформацияПоИсправлениям)
	...
	Если ИнформацияПоИсправлениям.НеУстановлено > 0 Тогда
		...
		Ссылка = Новый ФорматированнаяСтрока(НСтр("ru = 'Не удалось установить исправления';
													|en = 'Cannot install the patches'"),,,, "НеудачнаяУстановка");
		НадписьИсправления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '(%1 из %2)';
																							|en = '(%1 out of %2).'"), УспешноУстановлено, ВсегоПатчей);
	...
	Иначе
		Ссылка = Новый ФорматированнаяСтрока(НСтр("ru = 'Исправления (патчи)';
													|en = 'The patches'"),,,, "УстановленныеИсправления");
		НадписьИсправления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'успешно установлены (%1)';
				|en = 'are installed (%1).'"), ВсегоПатчей);
		НадписьИсправления = Новый ФорматированнаяСтрока(Ссылка, " ", НадписьИсправления);
