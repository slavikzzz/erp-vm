	...
	Если Не Готово И Модифицированность Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Работа помощника будет завершена,
			           ...
			           |en = 'Wizard will be closed and
			           |all the entered information will be lost. Close the wizard?'"),
			СписокКнопок);
					...
					Если Не ОтменаРедактирования Тогда
						...
						ОбщегоНазначенияКлиент.СообщитьПользователю(
							НСтр("ru = 'Такая номенклатура уже введена.';
								|en = 'These products already exist.'"),,
							"ДеревоЦен");
					Иначе
	...
	Если Элементы.Шаги.ТекущаяСтраница = Элементы.Шаг1 Тогда
		...
		Если СопоставлятьСправочники Тогда
			...
			Состояние(НСтр("ru = 'Осуществляется сопоставление введенных данных
			                     ...
			                     |en = 'Mapping entered data
			                     |with infobase data. Please wait...'"),,,БиблиотекаКартинок.Информация32);
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<будет создана при регистрации>';
																|en = '<will be created on registration>'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<не будет создана при регистрации>';
																|en = '<will not be created on registration>'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<пропущена при регистрации>';
																|en = '<skipped during registration>'"));
...
Процедура ИнициализироватьТабличныйДокумент()
	...
	Для Каждого СтрокаТЧ Из ВыбранныеЦены Цикл
		...
		ОбластьВидЦены.Параметры.МинПартия = СтрШаблон(НСтр("ru = 'Мин. партия %1';
															|en = 'Min. delivery lot %1'"), СтрокаТЧ.Ссылка);
		
		ТабличныйДокумент.Присоединить(ОбластьВидЦены);
		
		Если ИспользоватьУпаковкиНоменклатуры Тогда
			ОбластьУпаковкаЗаказа = Макет.ПолучитьОбласть("УпаковкаЗаказа");
			ОбластьУпаковкаЗаказа.Параметры.УпаковкаЗаказа = СтрШаблон(НСтр("ru = 'Упаковка заказа %1';
																			|en = 'Order packaging unit %1'"), СтрокаТЧ.Ссылка);
...
Процедура ЗавершитьРаботуПомощника()
	...
	Состояние(НСтр("ru = 'Осуществляется создание документа
	                     ...
	                     |for registration of vendor prices. Please wait...'"),,,БиблиотекаКартинок.Информация32);
	...
	Иначе
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Не удалось создать документ регистрации цен поставщика';
													|en = 'Failed to create vendor price registration document'"));
	КонецЕсли;
...
Функция ОбновитьИнформационнуюСтроку()
	...
	Если РегистрироватьТолькоРаспознаннуюНоменклатуру Тогда
		Если ДанныеДляИнформационнойСтроки.БезОшибокСтрок = 0 Тогда
			ФорматированнаяСтрока1 = Новый ФорматированнаяСтрока(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Цены не будут загружены т.к. не распознаны все (%1) строки прайс листа.';
																																|en = 'The prices will not be imported as not all (%1) price list lines are recognized.'"),ДанныеДляИнформационнойСтроки.ВсегоСтрок), Новый Шрифт(,,Истина));
		Иначе
			ФорматированнаяСтрока1 = Новый ФорматированнаяСтрока(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Будет загружено %1 из %2 строк прайс листа по %3 видам цен.';
																																|en = '%1 of %2 price list lines for %3 price types will be imported.'"),ДанныеДляИнформационнойСтроки.БезОшибокСтрок, ДанныеДляИнформационнойСтроки.ВсегоСтрок, ДанныеДляИнформационнойСтроки.ВыбранныеЦены), Новый Шрифт(,,Истина));
		КонецЕсли;
	Иначе
		ФорматированнаяСтрока1 = Новый ФорматированнаяСтрока(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Будет загружено %1 из %2 строк прайс листа по %3 видам цен.';
																															|en = '%1 of %2 price list lines for %3 price types will be imported.'"),ДанныеДляИнформационнойСтроки.ВсегоСтрок, ДанныеДляИнформационнойСтроки.ВсегоСтрок, ДанныеДляИнформационнойСтроки.ВыбранныеЦены), Новый Шрифт(,,Истина));
	КонецЕсли;
	...
	Если (ДанныеДляИнформационнойСтроки.НеРаспознанныхСтрок > 0 
		...
		Если ДанныеДляИнформационнойСтроки.НеРаспознанныхСтрок > 0 Тогда
			ФорматированнаяСтрока2 = Новый ФорматированнаяСтрока(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = ' Нераспознанные (%1)';
																																|en = ' Unrecognized (%1)'"), ДанныеДляИнформационнойСтроки.НеРаспознанныхСтрок), Новый Шрифт(,,Истина));
		КонецЕсли;
		
		Если ДанныеДляИнформационнойСтроки.ДублирующихСтрок > 0 Тогда
			Если ДанныеДляИнформационнойСтроки.НеРаспознанныхСтрок > 0 Тогда
				ФорматированнаяСтрока3 = Новый ФорматированнаяСтрока(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = ' и дублирующие (%1)';
																																	|en = ' and duplicating (%1)'"), ДанныеДляИнформационнойСтроки.ДублирующихСтрок), Новый Шрифт(,,Истина));
			Иначе
				ФорматированнаяСтрока3 = Новый ФорматированнаяСтрока(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = ' Дублирующие (%1)';
																																	|en = ' Duplicating (%1)'"), ДанныеДляИнформационнойСтроки.ДублирующихСтрок), Новый Шрифт(,,Истина));
			КонецЕсли;
		КонецЕсли;

		Если РегистрироватьТолькоРаспознаннуюНоменклатуру Тогда
			ФорматированнаяСтрока4 = Новый ФорматированнаяСтрока(НСтр("ru = ' строки исключены из загрузки.';
																		|en = ' lines excluded from import.'"), Новый Шрифт(,,Истина));
		Иначе
			ФорматированнаяСтрока4 = Новый ФорматированнаяСтрока(НСтр("ru = ' строки будут также загружены.';
																		|en = ' the lines will also be imported.'"), Новый Шрифт(,,Истина));
		КонецЕсли;

		
	КонецЕсли;
	
	ФорматированнаяСтрока5 = Новый ФорматированнаяСтрока(НСтр("ru = ' При необходимости, добавьте комментарий к изменению цен.';
																|en = ' If necessary, add a comment to the price change.'"), Новый Шрифт(,,Истина));
	...
	Для Каждого ТекСтрока Из ДеревоЦен Цикл
		...
		Если ТекСтрока.ХарактеристикиИспользуются И Не ЗначениеЗаполнено(ТекСтрока.Характеристика) Тогда
			...
			ТекстОшибки = НСтр("ru = 'Не заполнено характеристика номенклатуры в строке %НомерСтроки%';
								|en = 'The item variant in the line %НомерСтроки% is not filled in'");
		...
		Если Не РегистрироватьТолькоРаспознаннуюНоменклатуру
			...
			Иначе
				...
				ТекстОшибки = НСтр("ru = 'Не заполнено наименование номенклатуры в строке %НомерСтроки%';
									|en = 'Item name is required in line %НомерСтроки%'");
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ДеревоЦен.Индекс(ТекСтрока) + 1);
			...
			Для Каждого СтрокаТЧ Из УстановкаЦенКлиентСервер.ВыбранныеСтрокиТаблицыВидовЦен(ЭтотОбъект) Цикл
				...
				Если Не ЗначениеЗаполнено(ТекСтрока["УпаковкаЗаказа" + СтрокаТЧ.ИмяКолонки]) И ТекСтрока["УпаковкаЗаполнена" + СтрокаТЧ.ИмяКолонки] И ЗначениеЗаполнено(ТекСтрока.Номенклатура) Тогда
					...
					ТекстОшибки = НСтр("ru = 'Не заполнена упаковка заказа в строке %НомерСтроки%';
										|en = 'Order packaging unit is required in row %НомерСтроки%'");
					ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ДеревоЦен.Индекс(ТекСтрока) + 1);
