...
Процедура ОтменитьСопоставление(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Отменить сопоставление?';
									|en = 'Do you want to clear the mapping?'"), РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура ШагНазад()
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных
		...
		Элементы.Далее.Заголовок = ?(ТипЗагрузки = "ВставкаИзБуфераОбмена",
				НСтр("ru = 'Вставить в список';
					|en = 'Add to list'"), НСтр("ru = 'Далее >';
														|en = 'Next >'"));
			...
			Если Строки.Количество() > 0 Тогда
				...
				ПоказатьВопрос(Оповещение, НСтр("ru = 'Строки, в которых не заполнены обязательные колонки, будут пропущены.';
					...
					+ Символы.ПС + НСтр("ru = 'Продолжить?';
										|en = 'Do you want to continue?'"), РежимДиалогаВопрос.ДаНет);
				Возврат;
			КонецЕсли;
...
Процедура ВыполнитьСопоставление()
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Выполнено сопоставление';
										|en = 'Mapping completed'"),, НСтр("ru = 'Сопоставлено элементов:';
																				|en = 'Items mapped:'") + " " + Строка(КоличествоСопоставленныхПоКолонкам));
	ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
КонецПроцедуры
...
Процедура ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла()
	...
	Элементы.СоздаватьЕслиНеСопоставлено.Заголовок = НСтр("ru = 'Несопоставленные (';
															|en = 'Unmapped items ('") + Статистика.НеСопоставленных + ")";
	Элементы.ОбновлятьСуществующие.Заголовок       = НСтр("ru = 'Сопоставленные элементы (';
															|en = 'Mapped items ('") + Строка(Статистика.Сопоставленных) + ")";
...
Процедура ИнициализацияВставкиИзБуфераОбмена()
	...
	Если ЗначениеЗаполнено("ПредставлениеПоля") Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Вставка из буфера обмена (%1)';
																				|en = 'Paste (%1)'"), Параметры.ПредставлениеПоля);
	Иначе
...
Процедура УстановитьВидимостьЭлементовФормы()
	...
	Заголовок = ?(ПустаяСтрока(Параметры.Заголовок), НСтр("ru = 'Загрузка данных в справочник';
															|en = 'Import data to catalog'"), Параметры.Заголовок);
...
Процедура ВыполнитьШагЗаполнениеТаблицыДаннымиНаСервере(ОписаниеСтрокиВыбора)
	...
	КонецЕсли;
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загрузка данных в справочник ""%1""';
																			|en = 'Import data to catalog ""%1""'"), ЗаголовокОкна);
...
Процедура СопоставитьЗагружаемыеДанные()
	...
	Если СписокНезаполненныхКолонок.Количество() > 0 Тогда
		Если СписокНезаполненныхКолонок.Количество() = 1 Тогда
			ТекстПроКолонки = НСтр("ru = 'Обязательная колонка""';
									|en = 'Required column ""'") + " " + СписокНезаполненныхКолонок[0]
		...
		Иначе
			ТекстПроКолонки = НСтр("ru = 'Обязательные колонки""';
									|en = 'Required columns ""'") + " " + СтрСоединить(СписокНезаполненныхКолонок,", ")
...
Процедура ПоказатьОтчет(АдресРезультата)
	...
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("ВсеЭлементы", НСтр("ru = 'Все (';
																	|en = 'All ('") + Отчет.Всего + ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Новые", НСтр("ru = 'Новые (';
															|en = 'New items ('") + Отчет.Создано+ ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Обновленные", НСтр("ru = 'Обновленные (';
																	|en = 'Updated items ('") + Отчет.Обновлено+ ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Пропущенные", НСтр("ru = 'Пропущенные (';
																	|en = 'Skipped items ('") + Отчет.Пропущено+ ")");
...
Процедура ПоказатьИнформационнуюСтрокуПроОбязательныеКолонки()
	...
	Если Элементы.СтраницыЗаполненияДанных.ТекущаяСтраница = Элементы.СтраницаВариантЗагрузкаИзФайла Тогда
		...
		| • Spreadsheet document (.mxl)'") + Символы.ПС;
	Иначе
		ТекстПодсказки = НСтр("ru = 'Для заполнения таблицы скопируйте данные в таблицу из внешнего файла через буфер обмена.';
								|en = 'To fill in the table, copy and paste data to the table from an external file.'") + Символы.ПС;
	КонецЕсли;
	...
	Если ОбязательныеКолонки.Количество() > 0 Тогда 
		...
		Если ОбязательныеКолонки.Количество() = 1 Тогда
			ТекстПодсказки = ТекстПодсказки + НСтр("ru = 'Колонка, обязательная для заполнения:';
													|en = 'Required column:'") + " " + СписокКолонок;
		Иначе
			ТекстПодсказки = ТекстПодсказки + НСтр("ru = 'Колонки, обязательные для заполнения:';
													|en = 'Required columns:'") + " " + СписокКолонок;
		КонецЕсли;
...
Функция ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, Расширение)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Подсистема %1: Выполнение серверного метода загрузка данных из файла';
			|en = '%1 subsystem: import data from file using the server method'"), "ЗагрузкаДанныхИзФайла");
