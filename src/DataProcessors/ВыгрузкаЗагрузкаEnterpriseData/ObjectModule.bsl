...
Функция КомпонентыОбмена(НаправлениеОбмена, ВерсияФорматаОбменаПриЗагрузке = "", РасширениеФорматаПриЗагрузке = "") 
	...
	КомпонентыОбмена.КлючСообщенияЖурналаРегистрации = НСтр("ru = 'Перенос данных через буфер обмена';
															|en = 'Clipboard data transfer'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если МенеджерОбменаВнутренний Тогда
		...
		Если КомпонентыОбмена.МенеджерОбмена = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не поддерживается версия формата обмена: <%1>.';
																							|en = 'The exchange format version is not supported: %1.'"), ТекВерсияФормата);
		КонецЕсли;
	КонецЕсли;
...
Функция ВыгрузитьДанныеВXML(ИмяФайлаОбмена = "")
	...
	Если СписокДополнениеКВыгрузке.Количество() > 0 Тогда
		...
		СобытиеНачало = НСтр("ru = 'ВыгрузкаЗагрузкаEnterpriseData.Начало';
							|en = 'ВыгрузкаЗагрузкаEnterpriseData.Начало'", ОбщегоНазначения.КодОсновногоЯзыка());
		СобытиеОкончание = НСтр("ru = 'ВыгрузкаЗагрузкаEnterpriseData.Окончание';
								|en = 'ВыгрузкаЗагрузкаEnterpriseData.Окончание'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Если ЭлементыФормата.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неканоническое имя формата обмена <%1>';
																						|en = 'Noncanonical exchange format name: ""%1"".'"), ФорматОбмена);
	КонецЕсли;
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неканоническое представление версии формата обмена: <%1>.';
																						|en = 'Noncanonical presentation of the exchange format version: %1.'"), Результат.Версия);
	КонецЕсли;
