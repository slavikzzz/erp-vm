...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Пароль <> ПодтверждениеПароля Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Подтверждение пароля не совпадает с паролем';
													|en = 'Password confirmation does not match password'"), , "ПодтверждениеПароля");
		Отказ = Истина;
	КонецЕсли;
...
Процедура ЗагрузитьМетаданные(Команда)
	...
	Если Модифицированность
		...
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Загрузить метаданные заново? Внесенные изменения будут потеряны.';
										|en = 'Import metadata again? The changes made will be lost.'"), РежимДиалогаВопрос.ДаНет);
