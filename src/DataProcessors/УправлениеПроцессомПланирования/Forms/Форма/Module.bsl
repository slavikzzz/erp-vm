...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПравоДоступаЧтениеПланПроизводства = ПравоДоступа("Чтение", Метаданные.Документы.ПланПроизводства);
	//-- НЕ УТ
...
Процедура ПересчитатьДоКолонки(Команда)
	...
	ЗапуститьФоновоеЗадание(
		...
		НСтр("ru = 'Заполнить планы по дефицитам до колонки';
			|en = 'Fill plans by shortages before column'"),
		"Обработки.УправлениеПроцессомПланирования.ЗаполнитьПланыПоДефицитуПоПериодам",
		ПараметрыПроцедуры);
...
Процедура ПересчитатьВсеПериоды(Команда)
	...
	ЗапуститьФоновоеЗадание(
		...
		НСтр("ru = 'Заполнить планы по дефицитам во всех периодах';
			|en = 'Fill plans by shortages in all periods'"),
		"Обработки.УправлениеПроцессомПланирования.ЗаполнитьПланыПоДефицитуПоПериодам",
		ПараметрыПроцедуры);
КонецПроцедуры
...
Процедура ПересчитатьДефициты(Команда)
	...
	ЗапуститьФоновоеЗадание(
		...
		НСтр("ru = 'Запустить пересчет дефицитов по этапам по всем периодам';
			|en = 'Run the recalculation of shortages in stages for all the periods'"),
...
Функция ОбновитьДеревоЭтаповНаСервере()
	...
	|
	|ВЫБРАТЬ
	|	ПланПроизводства.ВидПлана,
	|	МИНИМУМ(ПланПроизводства.Статус.Порядок),
	|	ТаблицаПериоды.ПериодПланирования,
	|	ЛОЖЬ
	|ИЗ
	|	Документ.ПланПроизводства КАК ПланПроизводства
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПериоды КАК ТаблицаПериоды
	|		ПО ПланПроизводства.НачалоПериода <= ТаблицаПериоды.ПериодПланирования
	|		И ПланПроизводства.ОкончаниеПериода >= ТаблицаПериоды.ПериодПланирования
	|ГДЕ
	|	ПланПроизводства.Проведен
	|	И ПланПроизводства.Сценарий = &Сценарий
	|	И
	|	НЕ ПланПроизводства.ПометкаУдаления
	|	И ПланПроизводства.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.Отменен)
	|	И ПланПроизводства.НачалоПериода <= &ОкончаниеПериода
...
Процедура ЗапускФоновогоЗаданияПомощникПланирования()
	...
	Если ВключитьЗаполнениеПоДефицитамПомощникПланирования И НовоеФоновоеЗадание Тогда 
		...
		НСтр("ru = 'Глобальный перерасчет помощник планирования';
			|en = 'Global recalculation planning wizard'"),
...
Процедура СоздатьДанныеКорриктировок()
	...
	СоздатьЗаголовок(КорректировкаГруппа,Ложь,Нстр("ru = 'Проверить корректировки';
													|en = 'Check adjustments'"),"Корректировка");
				...
				Иначе
					Если  Элемент.ТипПланов = "Остатки" Тогда
						Если Не ПодсказкаДобавленаОстатки Тогда
							...
							НовыйЭлемент.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Для измения планов с текущего момента заведите планы остатков:';
																						|en = 'Create goods stock plans to change the plans from current moment:'"),,ЦветаСтиля.ПоясняющийТекст);
					...
					Если  Элемент.ТипПланов = "Исходные" Тогда
						Если  Не ПодсказкаДобавленаИсходныеПланы Тогда
							...
							НовыйЭлемент.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Для планирования на следующие периоды заполните планы:';
																						|en = 'Fill in plans to plan for the following periods:'"),,ЦветаСтиля.ПоясняющийТекст);
			...
			Если  Элемент.Состояние = 1 Тогда
				Если Не ПодсказкаДобавленаДефициты Тогда
					...
					КнопкаПересчета.Заголовок = НСтр("ru = 'Перерасчитать по дефицитам все планы';
													|en = 'Recalculate all plans by shortages'");
...
Функция ПроверкаПравДоступаПомощникПланирования(ТипПлана)
	...
	Если  ТипПлана = Перечисления.ТипыПланов.ПланПроизводства Тогда
		Если Не ПравоДоступаИзменениеПланПроизводства Тогда
			Возврат Ложь;	
		КонецЕсли;
		Если Не ПравоДоступаЧтениеПланПроизводства Тогда
			Возврат Ложь;	
		КонецЕсли;
	КонецЕсли;
	//-- НЕ УТ
...
Процедура СкрытьПомощникПланированияНажатиеНаСервере()
	...
	Элементы.СкрытьПомощникПланирования.Заголовок = ?(Элементы.ПомощникПланирования.Видимость,НСтр("ru = 'Скрыть Помощник Планирования';
																									|en = 'Hide Planning Wizard'"),НСтр("ru = 'Показать Помощник Планирования';
																																				|en = 'Show Planning Wizard'"));		
