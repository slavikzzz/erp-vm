...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстСообщения = НСтр("ru = '%ОбъектПланирования%%ВидПлана% %Товар%за период с %НачалоПериода% по %ОкончаниеПериода%';
							|en = '%ОбъектПланирования%%ВидПлана% %Товар%for the period from %НачалоПериода% to %ОкончаниеПериода%'");
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Товар%", НСтр("ru = 'по %Номенклатура%%Характеристика%%Назначение%';
																	|en = 'by %Номенклатура%%Характеристика%%Назначение%'") + " ");
...
Процедура ЗаполнитьПланы()
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		...
		|
		|ВЫБРАТЬ
		|	СУММА(ПланыВыпускаИзделийОбороты.КоличествоОборот),
		|	0,
		|	Документ.Ссылка,
		|	Документ.МоментВремени,
		|	Документ.Дата,
		...
		|						И Характеристика = &Характеристика
		|						И Назначение = &Назначение
		|					ИЛИ &БезНоменклатуры)) КАК ПланыВыпускаИзделийОбороты
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПланПроизводства КАК Документ
		|		ПО ПланыВыпускаИзделийОбороты.ПланПроизводства = Документ.Ссылка
		|ГДЕ
		|	&Приход
		|	И Документ.Проведен
		|	И Документ.Сценарий = &Сценарий
		...
		|ВЫБРАТЬ
		|	0,
		|	СУММА(ПланыПроизводстваОбороты.ЗаданИнтервалПотребностиОборот),
		|	Документ.Ссылка,
		|	Документ.МоментВремени,
		|	Документ.Дата,
		|	Документ.Номер,
		...
		|						И Характеристика = &Характеристика
		|						И Назначение = &Назначение
		|					ИЛИ &БезНоменклатуры)) КАК ПланыПроизводстваОбороты
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПланПроизводства КАК Документ
		|		ПО ПланыПроизводстваОбороты.ПланПроизводства = Документ.Ссылка
		|ГДЕ
		|	&Приход
		|	И Документ.Проведен
		|	И Документ.Сценарий = &Сценарий
		...
		|
		|ВЫБРАТЬ
		|	СУММА(ПланыПотребленияМатериаловОбороты.КоличествоОборот),
		|	0,
		|	Документ.Ссылка,
		|	Документ.МоментВремени,
		|	Документ.Дата,
		...
		|						И Характеристика = &Характеристика
		|						И Назначение = &Назначение
		|					ИЛИ &БезНоменклатуры)) КАК ПланыПотребленияМатериаловОбороты
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПланПроизводства КАК Документ
		|		ПО ПланыПотребленияМатериаловОбороты.ПланПроизводства = Документ.Ссылка
		|ГДЕ
		|	&Расход
		|	И Документ.Проведен
		|	И Документ.Сценарий = &Сценарий
...
Функция ТипПланаСтрока(ВидПлана)
	...
	ИначеЕсли ТипПлана = Перечисления.ТипыПланов.ПланПроизводства Тогда
		СтрокаТипПлана = "ПланПроизводства";
	//-- НЕ УТ
