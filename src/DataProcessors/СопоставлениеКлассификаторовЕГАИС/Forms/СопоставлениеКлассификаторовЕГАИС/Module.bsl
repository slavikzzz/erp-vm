...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоАдресВременногоХранилища(Параметры.Товары)
		...
		Если ОрганизацииЕГАИС = Неопределено И Товары = Неопределено Тогда
			ВызватьИсключение НСтр("ru = 'Не переданы входящие параметры.';
									|en = 'Не переданы входящие параметры.'");
		КонецЕсли;
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru = 'Обработка предназначена только для контекстного открытия из форм конфигурации.';
								|en = 'Обработка предназначена только для контекстного открытия из форм конфигурации.'");
	...
	ИначеЕсли Объект.Организации.Количество() = 0
		...
		ВызватьИсключение НСтр("ru = 'Нет данных для сопоставления.';
								|en = 'Нет данных для сопоставления.'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ШаблонСообщения = НСтр("ru = 'Не заполнено поле ""Характеристика"" в строке %1 списка ""Товары"".';
							|en = 'Не заполнено поле ""Характеристика"" в строке %1 списка ""Товары"".'");
...
Процедура ОбновитьЗаголовкиСтраниц()
	...
	Элементы.СтраницаКлассификаторОрганизацийЕГАИС.Заголовок = СтрШаблон(НСтр("ru = 'Организации (%1 из %2)';
																				|en = 'Организации (%1 из %2)'"), Заполнено, ВсегоСтрок);
	...
	Элементы.СтраницаКлассификаторАлкогольнойПродукцииЕГАИС.Заголовок = СтрШаблон(НСтр("ru = 'Алкогольная продукция (%1 из %2)';
																						|en = 'Алкогольная продукция (%1 из %2)'"), Заполнено, ВсегоСтрок);
	...
	Если Выборка.Следующий() Тогда
		...
		Для Каждого СтрокаТЧ Из НайденныеСтроки Цикл
			...
			ИначеЕсли Выборка.Количество > 1 Тогда
				...
				Иначе
					СтрокаТЧ.СопоставлениеНоменклатура = СтрШаблон(НСтр("ru = '%1 ( + еще %2...)';
																		|en = '%1 ( + еще %2...)'"), Выборка.НоменклатураНаименование, Выборка.Количество - 1);
				КонецЕсли;
			Иначе
				СтрокаТЧ.Сопоставлено                = Ложь;
				СтрокаТЧ.СопоставлениеНоменклатура   = НСтр("ru = '<Не сопоставлено>';
