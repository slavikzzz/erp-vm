...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Подсистема = НСтр("ru = 'Обмен с банками';
						|en = 'EDI with banks'", ОбщегоНазначения.КодОсновногоЯзыка());
	ЖурналСтрока = НСтр("ru = 'Журнал';
						|en = 'Log'", ОбщегоНазначения.КодОсновногоЯзыка());
	
	ИмяСобытия = НСтр("ru = 'Электронное взаимодействие.%1.%2';
						|en = 'Electronic interaction.%1.%2'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыгрузитьЖурналДляПередачиВТехподдержку(Команда)
	...
	ПараметрыСохранения.Диалог.Фильтр = НСтр("ru = 'Данные журнала обмена';
											|en = 'Exchange log data'") + "(*.txt)|*.txt";
