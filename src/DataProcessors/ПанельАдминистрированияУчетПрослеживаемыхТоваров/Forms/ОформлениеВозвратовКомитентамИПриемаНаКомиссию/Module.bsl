...
Процедура ОформитьВозвратИПрием(Команда)
	...
	Если ТекущаяСтрока = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru = 'Не указана строка таблицы, для которой необходимо создать документы.';
								|en = 'Row of the table for which you want to create documents is not specified.'");
...
Функция ОформитьВозвратТоваровПоставщику(ПараметрыЗаполнения)
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось создать документ возврата по причине: %1.';
										|en = 'Cannot create the return document. Reason: %1.'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ОформитьПриобретениеТоваровУслуг(ДокументВозврата)
	...
	Если НЕ ДокументВозвратаПроведен Тогда
		...
		ТекстСообщения = НСтр("ru = 'Документ приема может быть введен только на основании проведенного документа возврата.';
								|en = 'A receipt document can only be entered based on a posted return document.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось создать документ приема по причине: %1.';
										|en = 'Cannot create the receipt document due to: %1.'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
