...
Процедура ДоговорСоздание(Элемент, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ИсточникПоляДоговорКонтрагент) ИЛИ
		...
		Сообщение.Текст = НСтр("ru = 'Перед созданием договора необходимо заполнить поля ""Контрагент"" и ""Организация""';
								|en = 'Before you create a contract, fill the ""Counterparty"" and ""Company"" fields'");
		Сообщение.Сообщить();
...
Процедура Подключаемый_СозданиеТипового(ПараметрыОперации) Экспорт
	...
	Если СозданныйДокумент <> Неопределено Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Создан документ';
											|en = 'Document is created'"), НавигационнаяСсылка, Строка(СозданныйДокумент));
	...
	Для Каждого Запись Из Объект.РеквизитыТабличныхЧастей Цикл
		...
		Если Запись.ИмяРеквизита = "Номенклатура" Тогда
			...
			Элемент.Оформление.УстановитьЗначениеПараметра("Текст", СтрШаблон(НСтр("ru = 'Не сопоставлена: %1';
																					|en = 'Not mapped: %1'"), Запись.РаспознанныйТекст));
		КонецЕсли;
	КонецЦикла;
	...
	Если Не ЗначениеЗаполнено(РеквизитШапки.Значение) Тогда
		...
		Если РеквизитШапки.ИмяРеквизита <> "ИтогоСумма" И РеквизитШапки.ИмяРеквизита <> "ИтогоСуммаНДС"
			...
			Или РеквизитШапки.ИмяРеквизита = "ПокупательОрганизация" Тогда 
				ИзменяемыйЭлемент.ПодсказкаВвода = СтрШаблон(НСтр("ru = 'Не сопоставлена: %1';
																	|en = 'Not mapped: %1'"), РаспознанныйТекст);
			Иначе
				ИзменяемыйЭлемент.ПодсказкаВвода = СтрШаблон(НСтр("ru = 'Не сопоставлен: %1';
																	|en = 'Not mapped: %1'"), РаспознанныйТекст);
			КонецЕсли;
		КонецЕсли;
...
Процедура ИзменитьИсточникиДляВыбораДоговора(ИмяПоля = "")
	...
	РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Договор, "Контрагент, Организация, Партнер");
...
Функция СоздатьДокумент(ТипДокументаСтрокой, ПараметрыЗаполнения)
	...
	Исключение
		...
		УдалосьПровести = Ложь;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Ошибка проведения основного документа:';
								|en = 'Main document posting error:'") + Символы.ПС
	...
	Если Не УдалосьПровести Тогда
		...
		Исключение
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Ошибка создания документа:';
									|en = 'Document creation error:'") + Символы.ПС
				+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			Сообщение.Сообщить();
...
Функция СоздатьСчетФактуруПоПараметрам(ОснованиеСчетаФактуры, ТипДокументаСтрокой, ПараметрыЗаполнения)
	...
	Попытка
		Если Не ОснованиеСчетаФактуры.Проведен Тогда
			...
			|You can post a tax invoice only on the basis of posted documents.'"),ОснованиеСчетаФактуры);
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Ошибка создания счета-фактуры:';
								|en = 'Tax invoice creation error:'") + Символы.ПС
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Сообщение.Сообщить();
...
КонецЦикла;
	...
	Иначе
		...
		Элементы.ГруппаВсплывающаяСвязанныеДокументы.Заголовок =
			СтрШаблон(НСтр("ru = 'Связанные документы (%1)';
							|en = 'Related documents (%1)'"), КоличествоСвязанных);
	КонецЕсли;
...
Процедура ОпределениеДействияДляСоздаваемойСчетФактуры(ОснованиеСчетаФактуры, ТипДокументаСтрокой, ПараметрыЗаполнения)
	...
	Если СчетФактураСсылка <> Неопределено Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Создан документ';
											|en = 'Document is created'"), ПолучитьНавигационнуюСсылку(СчетФактураСсылка), Строка(СчетФактураСсылка));
		СледующийДокумент();
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	//подразделение для входящих/исходящих
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
