	...
	Если Параметры.ОткрытиеПоКомандеОбработки Тогда
		...
		Если Не ЗначениеЗаполнено(ОбъектРасчетов) Тогда
			ТекстИсключения = НСтр("ru = 'Зачет оплаты не предусмотрен для данного заказа.';
									|en = 'Payment offset for this order is not supported.'");
		...
		Если МассивСтрок.Количество() > 0 Тогда
			...
			ПоказатьВопрос(
				...
				НСтр("ru = 'Данные зачета оплаты были изменены. Сохранить изменения?';
					|en = 'Payment offset data has changed. Save the changes?'"),
	...
	Если ТекущиеДанные.СуммаЗачтено <> СуммаПередРедактированием Тогда
		...
		Если ИзменениеСуммы < 0 Тогда
			...
			ТекстПредупреждения = НСтр("ru = 'Зачтенная сумма может быть только уменьшена';
										|en = 'Offset amount can be only reduced'");
...
Процедура ИсправитьПревышение(Команда)
	...
	Если Не Отказ И Превышение > 0 Тогда
		ТекстПредупреждения = НСтр("ru = 'Не удалось полностью исправить превышение. Возможно документы не доступны для редактирования.';
									|en = 'Cannot fully correct excess. Documents may not be available for editing.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	КонецЕсли;
...
Процедура ПодобратьПодарочныйСертификат(Команда)
	...
	Если Валюта <> ВалютаРегламентированногоУчета Тогда
		ТекстСообщения = НСтр("ru = 'Оплачивать подарочными сертификатами можно только документы в валюте регл. учета.';
								|en = 'With gift cards, you can pay only for documents in local accounting currency.'");
	...
	Если ВалютаВзаиморасчетов <> ВалютаРегламентированногоУчета Тогда
		ТекстСообщения = НСтр("ru = 'Оплачивать подарочными сертификатами можно только документы, валютой взаиморасчетов которых является валюта регл. учета.';
								|en = 'With gift cards, you can pay only for documents whose AR/AP currency is a local accounting currency.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если Недостает <= 0 И СуммаЗачтеннаяПодарочнымиСертификатами = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Зачитываемые суммы полностью подобраны.';
								|en = 'Amounts to offset are selected.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<Будет создан документ ""Взаимозачет задолженности"">';
																|en = '<The ""AR/AP offset"" document will be created>'"));
...
Процедура УправлениеЭлементамиФормы()
	...
	Если ИспользоватьДоговоры Тогда
		Элементы.ПояснениеКТаблицеОплаты.Заголовок = НСтр("ru = 'Список нераспределенных платежей и документов оплаты, отнесенных на выбранный договор. Сумма будет распределена на зачитываемый объект расчетов.';
															|en = 'List of open payments and AR/AP transactions allocated to the selected document. The amount will be allocated to the AR/AP object being offset.'");
	Иначе
		Элементы.ПояснениеКТаблицеОплаты.Заголовок = НСтр("ru = 'Список нераспределенных платежей и документов оплаты. Сумма будет распределена на зачитываемый объект расчетов.';
															|en = 'List of non-allocated payments and AR/AP transactions. Amount will be allocated to the AR/AP object being offset.'");
	КонецЕсли;
	...
	Если ИспользоватьДоговоры Тогда
		Элементы.ПояснениеКТаблицеАвансы.Заголовок = НСтр("ru = 'Остатки авансов по договорам контрагента. Для зачета/переноса аванса будет создан документ ""Взаимозачет задолженности"".';
															|en = 'Remaining advances under the counterparty contracts. The ""AR/AP offset"" document will be created for prepayment applying/transfer.'");
	Иначе
		Элементы.ПояснениеКТаблицеАвансы.Заголовок = НСтр("ru = 'Остатки авансов. Для зачета/переноса аванса будет создан документ ""Взаимозачет задолженности"".';
															|en = 'Remaining advances. The ""AR/AP offset"" document will be created for prepayment applying/transfer.'");
	КонецЕсли;
	...
	Заголовок = НСтр("ru = 'Зачет оплат по:';
					|en = 'Offset payments for:'") + " " + Строка(ОбъектРасчетов);
...
Функция ТекстНедоступныеДляЗаписиДокументы()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроверкуФинансовыхДокументов") Тогда
		ТекстЗапроса = "
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ПроверкаДокументовПереопределяемый.Документ КАК Документ
			|ПОМЕСТИТЬ ВтНедоступныеДляЗаписиДокументы
			|ИЗ
			|	(ВЫБРАТЬ Т.Документ КАК Документ ИЗ ВтНераспределенныеПлатежи КАК Т
			|		ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ &Документ КАК Документ) КАК Данные
			|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПроверкаДокументовПереопределяемый КАК ПроверкаДокументовПереопределяемый
			|		ПО Данные.Документ = ПроверкаДокументовПереопределяемый.Документ
			|		И ПроверкаДокументовПереопределяемый.СтатусПроверки = ЗНАЧЕНИЕ(Перечисление.СтатусыПроверкиФинансовыхДокументов.Проверен)
			|";
		ПроверкаДокументовСервер.ПереопределитьВТекстеЗапросаСтатусыПроверкиСУчетомБлокировок(ТекстЗапроса, "Данные.Документ");
	КонецЕсли;
	...
	Если Недостает <= 0 Тогда
		...
		Если ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоЗаказамНакладным")
			...
			ТекстВопроса = НСтр("ru = 'Сумма аванса полностью зачтена.';
								|en = 'Advance amount is fully offset.'");
		Иначе
		...
		Если СтрокаТаблицы.ВерсияДанных <> ВерсияДанныхДокумента(СтрокаТаблицы.Документ) Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось заблокировать %1. Данные были изменены или удалены другим пользователем.';
					|en = 'Cannot lock %1. Data was changed or removed by another user.'"),
		...
		Если ИзменениеЗапрещено Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось изменить %1. Документ находится в запрещенном для изменения периоде.';
					|en = 'Cannot change %1. Document is in the change prohibition period.'"),
				ДокументСообщения);
			ОбщегоНазначения.СообщитьПользователю(
				ТекстОшибки,
				, // КлючДанных
...
Процедура СохранитьРезультатЗачетаОплатыИЗакрыть()
	...
	Если ЕстьРасчетыМеждуОрганизациями И МассивОбработанныхДокументов.Количество() > 0
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Изменен зачет оплат на стороне %1. 
			...
			|en = 'Payment offset on the %1 side is changed.
			|Offset similar payments on the %2 side?'"), Организация ,Партнер);
...
Процедура СоздатьВзаимозачетЗадолженности(ДанныеКор, СтруктураСумм, ПолеОшибки, ТекстОшибки)
	...
	ДокументОбъект.Комментарий = НСтр("ru = 'Сформирован автоматически обработкой ""Зачет оплат"".';
										|en = 'It is generated automatically by data processor ""Payment offset"".'");
	...
	ТекстОшибки = НСтр("ru = 'Не удалось создать документ взаимозачета.';
						|en = 'Cannot create offsetting document.'");
	ДокументОбъект.Записать(РежимЗаписиДокумента.Проведение);
...
Процедура ОповеститьОРезультатеЗачетаОплаты(МассивОбработанныхДокументов)
	...
	Если МассивОбработанныхДокументов.Количество() = 0 Тогда
		Текст = НСтр("ru = 'Изменений в зачете оплаты не было';
					|en = 'There were no changes in payment offset'");
		Пояснение = НСтр("ru = 'Изменений в документах при зачете оплаты не было';
						|en = 'There were no changes in documents on payment offset'");
...
Процедура ОбновитьИнформационнуюНадписьИтогов()
	...
	МассивСтрок.Добавить(Новый ФорматированнаяСтрока(
		...
		НСтр("ru = 'Зачтено на сумму %1 %2.';
			|en = 'Offset in the amount of %1 %2.'"),
...
Процедура СформироватьИнформационнуюНадписьОтборы()
	...
	НадписьОтборы = НСтр("ru = '%Организация%, %Партнер%, %Контрагент%, %Договор%, %НаправлениеДеятельности%';
						|en = '%Организация%, %Партнер%, %Контрагент%, %Договор%, %НаправлениеДеятельности%'");
	...
	НадписьОтборы = СтрЗаменить(НадписьОтборы,"%Организация%",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Организация: %1';
																	|en = 'Company: %1'"), Организация));
	НадписьОтборы = СтрЗаменить(НадписьОтборы,"%Партнер%",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Партнер: %1';
																	|en = 'Partner: %1'"), Партнер));
	НадписьОтборы = СтрЗаменить(НадписьОтборы,"%Контрагент%",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Контрагент: %1';
																	|en = 'Counterparty: %1'"), Контрагент));
	НадписьОтборы = СтрЗаменить(НадписьОтборы,"%Договор%",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Договор: %1';
																	|en = 'Contract: %1'"), Договор));
	НадписьОтборы = СтрЗаменить(НадписьОтборы,"%НаправлениеДеятельности%",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Направление деятельности: %1';
																	|en = 'Line of business: %1'"), НаправлениеДеятельности));
