	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		Если ЗначениеЗаполнено(ДанныеПоОшибкам)
			...
			Пока ВыборкаДокументовБезДанных.Следующий() Цикл
				СкладскяОперация = ?(ТипЗадания <> "ЗаданиеНаРазмещение", НСтр("ru = 'отгружаемая';
																				|en = 'shipping'"), НСтр("ru = 'размещаемая';
																											|en = 'Allocating'"));
				...
				|en = 'Can not print ""%НазваниеФормы%"" for ""%Документ%"",
				|because there is no %СкладскяОперация% items in the document.'");
					...
					Пока ВыборкаПоТовару.Следующий() Цикл
						...
						ПредставлениеНоменклатуры	= ?(ЗначениеЗаполнено(ВыборкаПоТовару.Номенклатура),
														...
														НСтр("ru = '<пустая ячейка>';
															|en = '<empty bin>'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ПредставлениеКомандыПечати(ТипЗадания)
	...
	ИначеЕсли ТипЗадания = "ЗаданиеНаПересчет" Тогда
		ПредставлениеКоманды = НСтр("ru = 'Бланк задания на пересчет товаров (с ячейками справочно)';
									|en = 'Inventory count task template (with storage bins)'");
	Иначе
		ПредставлениеКоманды = НСтр("ru = 'Задание на добор товаров';
									|en = 'Goods collection job'");
	КонецЕсли;
...
Функция НазваниеПечатнойФормы(ТипЗадания)
	...
	Если ТипЗадания = "ЗаданиеНаРазмещение" Тогда
		ТекстЗаголовка = НСтр("ru = 'Задание на размещение товаров';
								|en = 'Put-away task'", ОбщегоНазначения.КодОсновногоЯзыка());
	ИначеЕсли ТипЗадания = "ЗаданиеНаОтбор" Тогда
		ТекстЗаголовка = НСтр("ru = 'Задание на отбор товаров';
								|en = 'Picking job'", ОбщегоНазначения.КодОсновногоЯзыка());
	ИначеЕсли ТипЗадания = "ЗаданиеНаДобор" Тогда
		ТекстЗаголовка = НСтр("ru = 'Задание на добор товаров';
								|en = 'Goods collection job'", ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		ТекстЗаголовка = НСтр("ru = 'Задание на пересчет товаров';
								|en = 'Inventory count task'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
Функция ИспользоватьДанныеДокумента(ИмяДокумента)
	...
	//++ НЕ УТ
	ДокументыИсключения.Добавить(Метаданные.Документы.РаспределениеВозвратныхОтходов.ПолноеИмя());
	ДокументыИсключения.Добавить(Метаданные.Документы.РаспределениеПроизводственныхЗатрат.ПолноеИмя());
	//-- НЕ УТ
...
Функция ТекстОшибкиПоДаннымДокумента(ДанныеДокумента, ДанныеТЧ, ТекущийДокумет, ТипЗадания, НазваниеФормы)
	...
	Если ДанныеДокумента.ИспользуетсяОрдернаяСхема Тогда
		...
		ТекстОшибки = НСтр("ru = 'Невозможно напечатать %НазваниеФормы% для ""%Документ%"",
							...
							|en = 'Cannot print %НазваниеФормы% for ""%Документ%""
							|as Warehouse Management is enabled at warehouse ""%Склад%"". Print the task from the goods receipt/issue.'");
	...
	ИначеЕсли Не ДанныеДокумента.ЭтоНакладная
		...
		ТекстОшибки = НСтр("ru = 'Невозможно напечатать %НазваниеФормы% для ""%Документ%"",
							...
							|en = '%Документ% cannot generate %НазваниеФормы%.
							|The document doesn''t support this print form.
							|To print the form, open Intra-warehouse transfer and select Print.'");
			...
			Если ТипЗадания <> "ЗаданиеНаРазмещение" Тогда
				Если ЗначениеЗаполнено(ПредставлениеСклада) Тогда
					ТекстОшибки = НСтр("ru = 'Невозможно напечатать ""%НазваниеФормы%"" для ""%Документ%"",
										...
										|en = 'Cannot print ""%НазваниеФормы%"" for ""%Документ%""
										|as the document does not contain an item to ship from the ""%Склад%"" warehouse.'");
				Иначе
					ТекстОшибки = НСтр("ru = 'Невозможно напечатать ""%НазваниеФормы%"" для ""%Документ%"",
										|потому что в документе отсутствует отгружаемая номенклатура.';
										|en = 'Cannot print ""%НазваниеФормы%"" for ""%Документ%""
										|as there are no items to ship in the document.'");
				КонецЕсли;
			Иначе
				ТекстОшибки = НСтр("ru = 'Невозможно напечатать ""%НазваниеФормы%"" для ""%Документ%"",
									|потому что в документе отсутствует размещаемая номенклатура на складе ""%Склад%"".';
									|en = 'Cannot print ""%НазваниеФормы%"" for ""%Документ%""
									|as the document does not contain an item to put away in the ""%Склад%"" warehouse.'");
			КонецЕсли;
...
Функция ПараметрыЗаполненияОбластиОснования(ДокументСсылка, ВыборкаПоРаспоряжениям)
	...
	ЗаголовокОснования		= ?(ТекстыПредставлений.Количество() > 1, НСтр("ru = 'Основания:';
																			|en = 'Base documents:'", ОбщегоНазначения.КодОсновногоЯзыка()), НСтр("ru = 'Основание:';
...
Функция ПараметрыЗаполненияОбластиШапкиТаблицы(ВыводитьКоды, КолонкаКодов, ИспользоватьУпаковки, ТипЗадания)
	...
	Если ИспользоватьУпаковки Тогда
		Если ТипЗадания <> "ЗаданиеНаПересчет" Тогда
			ПараметрыЗаполнения.Вставить("НадписьКоличествоУпаковки", НСтр("ru = '(упаковок)';
																			|en = '(packaging units)'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Иначе
			ПараметрыЗаполнения.Вставить("НадписьКоличество",			НСтр("ru = '(Факт)';
																				|en = '(Actual)'", ОбщегоНазначения.КодОсновногоЯзыка()));
			ПараметрыЗаполнения.Вставить("НадписьКоличествоУпаковки",	НСтр("ru = '(По учету)';
																				|en = '(By accounting)'", ОбщегоНазначения.КодОсновногоЯзыка()));
		КонецЕсли;
	КонецЕсли;
