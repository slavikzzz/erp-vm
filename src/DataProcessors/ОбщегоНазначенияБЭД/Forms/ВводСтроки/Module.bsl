...
Процедура ОК(Команда)
	...
	Если ЗначениеЗаполнено(ФункцияПроверкиВвода) Тогда
		ПроверкаПройдена = Вычислить(ФункцияПроверкиВвода + "(Строка)"); // АПК:488 Исполняемый код безопасен.
		Если Не ПроверкаПройдена Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(КомментарийОшибкиПроверкиВвода,, "Строка");
			Отказ = Истина;
		КонецЕсли;
...
Процедура Инициализировать()
	...
	ФункцияПроверкиВвода = Параметры.ФункцияПроверкиВвода;
	Если ЗначениеЗаполнено(Параметры.КомментарийОшибкиПроверкиВвода) Тогда
		КомментарийОшибкиПроверкиВвода = Параметры.КомментарийОшибкиПроверкиВвода;
	Иначе
		КомментарийОшибкиПроверкиВвода = НСтр("ru = 'Необходимо ввести текст';
												|en = 'Please enter the text'");
	КонецЕсли;
