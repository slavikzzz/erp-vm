...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		ТекстГиперссылки = СтрШаблон(НСтр("ru = 'Список (%1)';
											|en = 'List (%1)'"), ЭлектронныеДокументы.Количество());
...
Процедура Печать(Команда)
	...
	Если Не ПечататьЭлектронныйДокумент
		...
		И Не ПечататьРеестрЭлектронныхДокументов
		И Не ПечататьПротоколОбменаПоДокументу Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не выбран состав печати';
														|en = 'Stamp content is not selected'"));
		Возврат;
	КонецЕсли;
	...
	Если ПечатныеФормы.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не сформировано печатных форм для данного состава';
														|en = 'No generated print forms for the set'"));
		Возврат;
	КонецЕсли;
