...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПараметрыФайла = Неопределено И Не МожноЗагрузитьФормат(ПараметрыФайла) Тогда
		Отказ = Истина;
		СообщитьНевозможноЗагрузитьФормат(ПараметрыФайла.Формат);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция МожноЗагрузитьФормат(Знач СодержаниеФайла)
	
	Возврат Не (ЗначениеЗаполнено(СодержаниеФайла.ТипРегламента)
		И СодержаниеФайла.ТипРегламента <> Перечисления.ТипыРегламентовЭДО.Неформализованный
		И ЗначениеЗаполнено(СодержаниеФайла.Формат)
		И Не ЭлектронныеДокументыЭДО.ЭтоСтандартныйФормат(СодержаниеФайла.Формат));
...
&НаСервереБезКонтекста
Процедура СообщитьНевозможноЗагрузитьФормат(Формат)
	
	ТекстСообщения = НСтр("ru = 'Не поддерживается загрузка электронных документов формата ""%1"".
		|Необходимо обновить конфигурацию до актуальной версии.';
		|en = 'Cannot import electronic documents in the ""%1"" format.
		|Update the configuration to the current version.'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", Формат);
