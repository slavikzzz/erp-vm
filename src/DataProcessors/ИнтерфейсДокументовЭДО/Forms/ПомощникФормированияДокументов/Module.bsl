...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Идет формирование документа...';
																			|en = 'Document is being generated...'"));
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ПоясняющийТекст);
...
Процедура УправлениеФормой(ИндексНовойСтраницы)
	...
	Иначе
		...
		Для Каждого СтрокаТЧ Из ДокументыКОтправке Цикл
			...
			Если СтрокаТЧ.ОтсутствуетФайлДоговорногоДокумента Тогда
				СтрокаТЧ.ЕстьОшибки = Истина;
				ТекущаяСтрока = ОбщиеОшибкиДокументов.Добавить();
				ТекущаяСтрока.Представление =
					НСтр("ru = 'Переформирование договорных документов недоступно. Создайте новый документ.';
						|en = 'Regenerating contractual documents is unavailable. Create a new document.'");
			КонецЕсли;
...
Процедура УстановитьЗаголовокГотовоКФормированию(Форма)
	...
	МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Готово к формированию документов:';
															|en = 'Ready to generate documents:'"), Шрифт));
...
Функция СформироватьЗаголовокОжидания(КоличествоДокументов)
	...
	|en = 'Electronic documents are being generated.
	|Approximate generation time %1.'");
...
Функция ПолучитьПредставлениеКлючаСообщения(КлючСообщения)
	...
	Если ТипЗнч(КлючСообщения) = Тип("РегистрСведенийКлючЗаписи.НастройкиОтправкиЭлектронныхДокументовПоВидам") Тогда
		...
		Если ЗначениеЗаполнено(КлючСообщения.Договор) Тогда
			МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'По договору ""%1""';
												|en = 'Under the ""%1"" contract'"), КлючСообщения.Договор));
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПредставлениеОбъекта) Тогда
		ТекстОтвета = СтрШаблон(НСтр("ru = '%1 (%2)';
									|en = '%1 (%2)'"), Представление, ПредставлениеОбъекта);
	Иначе
		ТекстОтвета = СтрШаблон(НСтр("ru = '%1';
									|en = '%1'"), Представление);
	КонецЕсли;
...
Процедура СформироватьЗаголовокКомандыПодписать(Форма)
	...
	Текст = НСтр("ru = 'Готово';
				|en = 'Finish'");
	ТекстПодсказки = НСтр("ru = 'Завершить работу помощника';
							|en = 'Finish wizard'");
	...
	Если Форма.ПодписатьОтправить Тогда
		Если КоличествоВсего = 0 И КоличествоВыбрано = 0 Тогда
			Текст = НСтр("ru = 'Подписать и отправить документы';
						|en = 'Sign and send documents'");
			ТекстПодсказки = НСтр("ru = 'Подписать и отправить';
									|en = 'Sign and send'");
		Иначе
			Если КоличествоВыбрано > 0 Тогда
				Текст = СтрШаблон(НСтр("ru = 'Подписать и отправить документы (%1 из %2)';
										|en = 'Sign and send documents (%1 out of %2)'"), КоличествоВыбрано, КоличествоВсего);
				ТекстПодсказки = НСтр("ru = 'Подписать и отправить';
										|en = 'Sign and send'");
			КонецЕсли;
		КонецЕсли;
	...
	КонецЕсли;
	Форма.Элементы.ПодписатьИОтправить.РасширеннаяПодсказка.Заголовок = ТекстПодсказки;
...
Процедура ЗаполнитьТаблицуОтправляемыхДокументов(ОшибкиПриФормированииДокументов, СписокДокументовКОтправке)
	...
	Если ЗначениеЗаполнено(ОшибкиПриФормированииДокументов) Тогда
		...
		КоличествоЗаписейОбОшибках = ОшибкиПриФормированииДокументов.Количество();
...
Процедура ЗаполнитьДанныеОтправляемогоДокумента(СтрокаТаблицы, ОбъектУчета, ОписаниеОбъектаУчета, ВидДокумента)
	...
	СтрокаТаблицы.ОтсутствуетФайлДоговорногоДокумента = Ложь;
	...
	Если СтрокаТаблицы.ЭтоВнутреннийЭДО Тогда
		...
		Если Не СтрокаТаблицы.НастройкиСозданы Тогда
			СтрокаТаблицы.НастройкиОтправки = СтрШаблон("%1 (%2)",
				НСтр("ru = 'Не настроен';
					|en = 'Not configured'"), МетаданныеДокумента.Синоним);
		...
		Иначе
			СтрокаТаблицы.НастройкиОтправки = СтрШаблон(НСтр("ru = 'Не указана основная печатная форма (%1)';
															|en = 'The main print form is not specified (%1)'"),
...
&НаСервере
Процедура ЗаполнитьИнформациюОбОшибкахДокумента(СтрокаТаблицы, ОшибкиПриФормированииДокумента, КоличествоЗаписейОбОшибках)
	...
	ИначеЕсли КоличествоЗаписейОбОшибках = 1
		И ОшибкиПриФормированииДокумента.Свойство("ОтсутствуетФайлДоговорногоДокумента")
		И ОшибкиПриФормированииДокумента.ОтсутствуетФайлДоговорногоДокумента <> Неопределено Тогда
		
		СтрокаТаблицы.ОтсутствуетФайлДоговорногоДокумента = Истина;
	...
	КоличествоОшибок = СтрокаТаблицы.ОшибкиЗаполнения.Количество()
		...
		+ СтрокаТаблицы.ОтсутствуетФайлДоговорногоДокумента;
	...
	ИначеЕсли (КоличествоОшибок > 1 Или КоличествоПредупреждений > 1)
		...
		Предупреждений = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
			НСтр("ru = ';%1 предупреждение;;%1 предупреждений;%1 предупреждений;%1 предупреждения';
				|en = ';%1 warning;;%1 of warnings;%1 warnings;%1 of warning'"), КоличествоПредупреждений);
	...
	ИначеЕсли СтрокаТаблицы.ТребуетсяУказатьПодписантов Тогда
		ПредставлениеОшибок = НСтр("ru = 'Не указаны подписанты';
									|en = 'Signers are not specified'");
	ИначеЕсли СтрокаТаблицы.ОсновнойВидНеУстановлен Тогда
		ПредставлениеОшибок = НСтр("ru = 'Основной вид не установлен';
									|en = 'The main kind is not specified'");
	ИначеЕсли СтрокаТаблицы.ПечатнаяФормаНеУказана Тогда
		ПредставлениеОшибок = НСтр("ru = 'Не указана печатная форма';
									|en = 'Print form is not specified'");
	ИначеЕсли СтрокаТаблицы.ОтсутствуетФайлДоговорногоДокумента Тогда
		ПредставлениеОшибок = НСтр("ru = 'Отсутствует файл договорного документа';
									|en = 'Contractual document file is missing'");
	КонецЕсли;
		...
		Иначе
			...
			Предупреждений = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
				НСтр("ru = ';%1 предупреждение;;%1 предупреждения;%1 предупреждений;%1 предупреждения';
					|en = ';%1 warning;;%1 warnings;%1 of warnings;%1 of warning'"),
...
Функция ДанныеОтправляемогоДокумента(Документ, ОписаниеОбъектаУчета)
	...
	Если ЗначениеЗаполнено(ОписаниеОбъектаУчета.Договор)
		И Не ОписаниеОбъектаУчета.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДоговорныйДокумент Тогда
...
Процедура ПодготовитьДополнительныеПоляДляЗаполнения(ДополнительныеПоля, ДанныеДокумента)
	...
	МассивСтрок.Добавить(Новый ФорматированнаяСтрока(
		СтрШаблон(НСтр("ru = '%1 требует заполнения дополнительных данных в электронных документах.';
						|en = '%1 requires filling additional data in electronic documents.'"), ДанныеДокумента.Контрагент)));
...
Процедура ОбработатьЗавершениеНастройкиЗаполненияДополнительныхПолей(Результат, Контекст) Экспорт
	...
	МассивСтрок.Добавить(
		...
		,?(МассовыйРежим, НСтр("ru = 'Подписать и отправить';
								|en = 'Sign and send'"), НСтр("ru = 'Готово';
																	|en = 'Finish'"))));
	...
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаПодписанты Тогда
		...
		Если Не ЗначениеЗаполнено(СписокПодписантов) И Не ЗначениеЗаполнено(Маршрут) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не указаны подписанты электронного документа';
															|en = 'Electronic document signers are not specified'"),,
		...
		Для Каждого СтрокаТаблицы Из СписокПодписантов Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.Подписант) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Не указан подписант в строке %1';
																			|en = 'Signer in line %1 is not specified'"), НомерСтроки),,
			...
			Иначе
				ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Обнаружен дубль подписанта ""%1"" в строке %2';
																			|en = 'Duplicate signer ""%1"" found in line %2'"),
...
Функция ОписаниеОшибкиЗапретаФормированияДокумента(ЗначенияКлюча)
	...
	Ошибка.Вставить("ТекстОшибки", НСтр("ru = 'Документ заблокирован для отправки';
										|en = 'The document is locked for dispatch'"));
	Возврат Ошибка;
...
Функция ОписаниеОшибкиЗапретаФормированияВнутреннегоДокумента(ЗначенияКлюча, ИдентификаторОбъектаУчета)
	...
	Ошибка.Вставить("ТекстОшибки", СтрШаблон(НСтр("ru = 'Формирование документов на основании %1 запрещено';
													|en = 'Generation of documents based on %1 is restricted'"),
...
Функция ОписаниеОшибкиОсновнойВидНеУстановлен(ЗначенияКлюча, ИдентификаторОбъекта)
	...
	Ошибка.Вставить("ТекстОшибки", СтрШаблон(НСтр(
		"ru = 'Не указана печатная форма по умолчанию для вида документа ""%1""';
		|en = 'The default print form is not specified for the ""%1"" document kind'"), ИдентификаторОбъекта));
	Возврат Ошибка;
