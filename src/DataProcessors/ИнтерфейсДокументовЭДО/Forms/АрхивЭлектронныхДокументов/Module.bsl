...
Процедура УстановитьСтатусКРаспаковке(Команда)
	...
	ТекстОповещения = НСтр("ru = 'Изменен статус контейнеров на ""К распаковке""';
							|en = 'Container status changed ""To unpack""'") + ": (%1)";
...
Процедура УстановитьСтатусОтменен(Команда)
	...
	ТекстОповещения = НСтр("ru = 'Изменен статус контейнеров на ""Отменен""';
							|en = 'Container status is changed to ""Canceled""'") + ": (%1)";
	ТекстОповещения = СтрЗаменить(ТекстОповещения, "%1", Количество);
	ПоказатьОповещениеПользователя(НСтр("ru = 'Обмен электронными документами';
										|en = 'Electronic document exchange'"), , ТекстОповещения);
	Элементы[ТаблицаКонтейнеров].Обновить();
...
Процедура СохранитьКонтейнерыЭДНаДиск(Команда)
	...
	Если МассивФайлов.Количество() Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для сохранения контейнеров ЭД необходимо установить расширение работы с файлами.';
								|en = 'To save ED containers, install the file system extension.'");
	...
	Для Каждого СтрокаТаблицы Из Элементы[ТаблицаКонтейнеров].ВыделенныеСтроки Цикл
		...
		Исключение
			...
			ОбработкаНеисправностейБЭД.ОбработатьОшибку(
				НСтр("ru = 'изменение статуса контейнеров ЭД';
					|en = 'ED container status change'"), ТекстОшибки, ТекстСообщения,, СтрокаТаблицы.Ссылка);
...
Процедура ОбновитьКнопкиОтборовДокументов()
	...
	Если ЗначениеЗаполнено(КоличествоПользовательскихОтборов) Тогда
		ТекстКнопкиПО = НСтр("ru = 'и еще';
							|en = 'and'") + " " + Строка(КоличествоПользовательскихОтборов);
	Иначе
