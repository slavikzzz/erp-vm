...
Процедура ВыполнитьДействие(Команда)
	...
	Если СпособВыгрузки = "ЧерезКаталог" Тогда
		...
		Если ТаблицаДанных.Количество() = 1 Тогда
			...
			ПараметрыСохранения.Диалог.Фильтр = НСтр("ru = 'Архивы ZIP (*.zip)|*.zip';
													|en = 'Zip archives (*.zip)|*.zip'");
			ДанныеФайла = ТаблицаДанных[0];
			ФайловаяСистемаКлиент.СохранитьФайл(Оповещение
			,ДанныеФайла.АдресХранилищаПакета
			,ДанныеФайла.ИмяФайла + ".zip"
			,ПараметрыСохранения); 
		Иначе			
			МассивФайлов = Новый Массив;
			Для Каждого ДанныеФайла Из ТаблицаДанных Цикл
				ОписаниеФайла = Новый ОписаниеПередаваемогоФайла(
				ДанныеФайла.ИмяФайла + ".zip", ДанныеФайла.АдресХранилищаПакета);
				МассивФайлов.Добавить(ОписаниеФайла);
			КонецЦикла;	
			ПараметрыСохранения = ФайловаяСистемаКлиент.ПараметрыСохраненияФайлов();
			ПараметрыСохранения.Интерактивно = Истина;
			ФайловаяСистемаКлиент.СохранитьФайлы(Оповещение, МассивФайлов, ПараметрыСохранения);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПослеПодключенияРасширения(МассивСохраненныхФайлов, ДополнительныеПараметры) Экспорт
	Если Не МассивСохраненныхФайлов = Неопределено Тогда
		Закрыть();
	КонецЕсли;
КонецПроцедуры
	...
	Иначе
		...
		Если МассивСтруктурОбмена.Количество() > 1 Тогда
			ТемаПисьма = НСтр("ru = 'Пакеты электронных документов';
								|en = 'Electronic document packages'");
		Иначе
			ТемаПисьма = НСтр("ru = 'Пакет электронного документа:';
								|en = 'Package of the electronic document:'") + " " + МассивСтруктурОбмена[0].НаименованиеФайла;
		КонецЕсли;
