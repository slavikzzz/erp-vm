...
// 
// Параметры:
//  ВидДокумента - ПеречислениеСсылка.ВидыДокументовЕГАИС - Вид документа запроса остатков
...
Процедура ЗаполнитьТаблицуОстатков() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		ДанныеЗаписи = РегистрыСведений.АкцизныеМаркиЕГАИС.ПоляЗаписиСтатусаУпаковки();
...
//  Массив Из ОпределяемыйТип.ДокументыЕГАИС - Созданные документы корректировки остатков
...
Процедура СоздатьАктыПостановкиНаБалансВРегистр2(Излишки, АктСписания)
	...
	Акт.Комментарий = НСтр("ru = 'Излишки торгового зала (к определению справок №2)';
							|en = 'Излишки торгового зала (к определению справок №2)'");
