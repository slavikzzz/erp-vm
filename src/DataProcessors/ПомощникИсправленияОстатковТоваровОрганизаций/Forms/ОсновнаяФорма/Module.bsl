...
Процедура ИнициализироватьПомощник()
	...
	//++ НЕ УТ
	ИспользуетсяПроизводство = ИспользуетсяПроизводство();
	//-- НЕ УТ
	
	//++ НЕ УТКА
	Элементы.ДекорацияДиагностикаОтрицательныхОстатковПрочиеРешения3.Видимость = Истина;
	//-- НЕ УТКА
	...
	Если ЭтоУправлениеТорговлейБазовая Тогда
		ТекстОписанияПомощника = НСтр("ru = '1. Анализ и вывод подробной информации о проблемах
										...
										|en = '1. Analyze and display detailed information on issues.
										...
										|5. Register goods recording as received.'");
		ТекстЗаголовкаШага6 = НСтр("ru = 'Шаг 5. Оформление оприходований товаров';
									|en = 'Step 5. Register goods receipt'");
...
Процедура ДекорацияДиагностикаОтрицательныхОстатковПрочиеРешения2ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОформлениеОтчетовПереработчиков" Тогда
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ИмяТекущейСтраницы", "СтраницаОтчеты");
		ПараметрыФормы.Вставить("ПериодРегистрации",  НачалоМесяца(Объект.МесяцИсправления));
		ПараметрыФормы.Вставить("ПроверяемыйПериод",  НачалоМесяца(Объект.МесяцИсправления));
		ПараметрыФормы.Вставить("Отбор",              Новый Структура("НачалоПериода, КонецПериода", НачалоМесяца(Объект.МесяцИсправления), КонецМесяца(Объект.МесяцИсправления)));
		
		ОткрытьФорму("Обработка.ЖурналДокументовПередачиВПереработку2_5.Форма.КОформлениюНакладных", ПараметрыФормы, ЭтотОбъект);
...
Процедура ДекорацияДиагностикаОтрицательныхОстатковПрочиеРешения3ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	//++ НЕ УТКА
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОформлениеОтчетовДавальцев" Тогда
		...
		ОткрытьФорму("Обработка.ЖурналДокументовПриемаВПереработку2_5.Форма.КОформлениюНакладных", ПараметрыФормы, ЭтотОбъект);
...
Процедура ДекорацияШаг6СмТакжеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОформлениеПроизводстваБезЗаказов" Тогда
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ПериодРегистрации", Объект.МесяцИсправления);
		
		ОткрытьФорму("Обработка.ОформлениеПроизводстваБезЗаказов.Форма.ФормаРабочееМесто", ПараметрыФормы, ЭтотОбъект);
...
Процедура ОформитьПроизводствоБезЗаказа(Команда)
	...
	//++ НЕ УТ
	ТекущиеДанные = Элементы.РаспоряженияНаОформлениеОприходований.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыДокументов = ПроизводствоКлиентСервер.ПараметрыФормированияДокументовПроизводстваБезЗаказов();
	ПараметрыДокументов.ПериодРегистрации = Объект.МесяцИсправления;
	...
	Результат = ОформитьПроизводствоБезЗаказаНаСервере(ТекущиеДанные.ПолучитьИдентификатор(), ПараметрыДокументов);
	Если Не Результат = Неопределено Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеОЗакрытииФормыПроизводстваБезЗаказов", ЭтаФорма);
		ПроизводствоКлиент.ОбработатьСформированныеДокументы(ЭтаФорма, Результат, Истина, ОписаниеОповещения);
	КонецЕсли;
	//-- НЕ УТ
...
Процедура НастроитьЭлементыФормы()
	...
	Элементы.НавигацияПерейтиВНачало.Заголовок = ?(ЕстьПроблема, НСтр("ru = 'Перейти в начало';
																		|en = 'Go to beginning'"), НСтр("ru = 'Следующий месяц';
																										|en = 'Next month'"));
	...
	Если НомерСтраницы = 3 Тогда
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru = 'Открыть список сформированных корректировок';
				|en = 'Open list of generated adjustments'"), 
	...
	Если НомерСтраницы = 4 Тогда
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru = 'Открыть список измененных документов';
				|en = 'Open list of changed documents'"), 
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru = 'Открыть список сформированных исправительных счетов-фактур';
				|en = 'Open list of generated replacement tax invoices'"), 
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru = 'Открыть список сформированных исправлений';
				|en = 'Open list of generated corrections'"), 
	...
	Если НомерСтраницы = 5 Тогда
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru = 'Открыть список сформированных корректировок по новым настройкам';
				|en = 'Open list of generated adjustments by new settings'"), 
	...
	Если НомерСтраницы = 6 Тогда
		...
		//++ НЕ УТ
		Если ИспользуетсяПроизводство Тогда
			Элементы.ДекорацияЗаголовокШаг6.Заголовок =
				НСтр("ru = 'Шаг 6. Оформление оприходования/выпуска товаров.';
					|en = 'Step 6. Register receipt/release of goods.'");
			Элементы.ДекорацияШаг6Пояснение1.Заголовок =
				НСтр("ru = 'Этот шаг заключительный. Если предыдущие шаги не помогли, то вы можете оформить оприходование/выпуск товаров, по которым остались отрицательные остатки.';
					|en = 'This step is final. If the previous steps did not help, you can register the receipt/release of goods for which there is negative balance.'");
			Элементы.ДекорацияШаг6НетПроблемыПояснение.Заголовок =
				НСтр("ru = 'Оформление оприходований/выпуска товаров не требуется.';
					|en = 'Registration of receipt/release of goods is not required.'");
		КонецЕсли;
		//-- НЕ УТ
	...
	Если НомерСтраницы = 7 Тогда
		ЗаголовокОтчета = НСтр("ru = 'Отчет о проделанной работе за %МесяцИсправления%';
								|en = 'Progress report for %МесяцИсправления%'");
		ЗаголовокОтчета = 
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru = 'Открыть список сформированных корректировок резервов';
				|en = 'Open list of generated adjustments of reserves'"), 
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru = 'Открыть список сформированных исправлений развернутого сальдо товаров организаций';
				|en = 'Open list of generated corrections of the company goods detailed balance'"), 
		...
		НастроитьГиперссылкуСпискаДокументов(
			НСтр("ru = 'Открыть список сформированных корректировок резервов по новым настройкам';
				|en = 'Open list of generated adjustments of reserves by new settings'"), 
...
Функция СформироватьЗаголовокШаг3Пояснение2()
	...
	ТекстЗаголовка = НСтр("ru = 'При запуске переформирования резервов будут очищены движения регистра ""Резервы товаров организаций"" по проблемным позициям, начиная с %МесяцРодительныйПадеж% %Год% года, и сформированы новые резервы по отрицательным остаткам. Новые резервы будут записаны в документы корректировок регистров. Для каждой организации будет создан отдельный документ.';
							|en = 'When rebuilding reserves, the ""Company goods reserves"" register records for problematic positions will be cleared starting from %МесяцРодительныйПадеж% %Год% and new negative balance reserves will be created. New reserves will be recorded in register adjustment documents. A separate document will be created for each company.'");
...
//++ НЕ УТ
&НаКлиенте
Процедура ОповещениеОЗакрытииФормыПроизводстваБезЗаказов(Результат, ДополнительныеПараметры) Экспорт
	ВыполнитьМетодыОбъекта("ОбновитьДанныеПоОтрицательнымОстаткамИСальдо, ЗаполнитьРаспоряженияНаОформлениеОприходований");
КонецПроцедуры
//-- НЕ УТ
...
Процедура ПроверитьРольНастройкаНСИДоступна(СтандартнаяОбработка)
	...
	Если Не Пользователи.РолиДоступны(Метаданные.Роли.НастройкаНСИ.Имя) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для настройки передачи требуется роль ""%НастройкаНСИ%"". Обратитесь к администратору.';
								|en = 'You need the ""%НастройкаНСИ%"" role to set up transfer. Please contact the administrator.'");
...
Процедура ПроверитьРазрешениеДоступа(Отказ)
	...
	Если Константы.КонтролироватьОстаткиТоваровОрганизаций.Получить()
		...
		НСтр("ru = 'Для работы с помощником требуется роль ""%РазрешитьОтключениеКонтроляТоваровОрганизацийНаВремяСеанса%"" или отключение функциональной опции ""%КонтролироватьОстаткиТоваровОрганизаций%"". Обратитесь к администратору.';
			|en = 'To use the wizard, you need to have the ""%РазрешитьОтключениеКонтроляТоваровОрганизацийНаВремяСеанса%"" role or disable the ""%КонтролироватьОстаткиТоваровОрганизаций%"" functional option. Please contact the administrator.'");
	...
	Исключение
		...
		НСтр("ru = 'Для работы с помощником требуется доступ на чтение всех организаций в информационной базе. Обратитесь к администратору.';
			|en = 'For work with the assistant, you need access to read all companies in the infobase. Ask the administrator.'");	
		ВызватьИсключение ТекстСообщения;
	КонецПопытки;
...
&НаСервереБезКонтекста
Функция ИспользуетсяПроизводство(Склад = Неопределено)
	
	Подразделение = Неопределено;
	Если ЗначениеЗаполнено(Склад) Тогда
		Подразделение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Склад, "Подразделение", Истина);
	КонецЕсли;
	
	Если Подразделение = Неопределено Тогда
		Возврат ПроизводствоСерверПовтИсп.НастройкиПодсистемыПроизводство().ИспользуетсяПроизводство;
	Иначе
		Возврат ПроизводствоСерверПовтИсп.ПараметрыПроизводственногоПодразделения().ПроизводственноеПодразделение;
	КонецЕсли;
	
КонецФункции

&НаСервере
Функция ОформитьПроизводствоБезЗаказаНаСервере(ТекущийИдентификатор, Знач ПараметрыДокументов)
	
	ТекущиеДанные = Объект.РаспоряженияНаОформлениеОприходований.НайтиПоИдентификатору(ТекущийИдентификатор);
	
	Если Не ИспользуетсяПроизводство(ТекущиеДанные.Склад) Тогда
		
		ТекстСообщения = НСтр("ru = 'Подразделение склада ""%1"" не является производственным. Необходимо оформить оприходование товаров.';
								|en = 'The ""%1"" warehouse business unit is not a production business unit. Register the goods receipt.'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, ТекущиеДанные.Склад);
	...
	Запрос = Обработки.ПомощникИсправленияОстатковТоваровОрганизаций.ЗапросДетальныеОстаткиНаКонецМесяцаИсправления(Объект.МесяцИсправления);
	...
	|	ДетальныеОстатки.АналитикаУчетаНоменклатуры.СкладскаяТерритория.Подразделение КАК Подразделение,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукцииНаСклад) КАК НаправлениеВыпуска,
	|	ДетальныеОстатки.АналитикаУчетаНоменклатуры.МестоХранения      КАК Получатель,
	|	ДАТАВРЕМЯ(1,1,1)                                               КАК НачалоПроизводства,
	|	ДетальныеОстатки.АналитикаУчетаНоменклатуры.Номенклатура       КАК Номенклатура,
	|	ДетальныеОстатки.АналитикаУчетаНоменклатуры.Характеристика     КАК Характеристика,
	|	ДетальныеОстатки.АналитикаУчетаНоменклатуры.Серия              КАК Серия,
	|	ДетальныеОстатки.АналитикаУчетаНоменклатуры.Назначение         КАК Назначение,
	|	ДетальныеОстатки.АналитикаУчетаНоменклатуры.Назначение.НаправлениеДеятельности КАК НаправлениеДеятельности,
	...
	|	-СУММА(ДетальныеОстатки.КоличествоОстаток)                     КАК Количество,
	|	ЛОЖЬ                                                           КАК ОтражатьЗатратыДокументом,
	|	ЛОЖЬ                                                           КАК ОшибкаВНастройкахМодели
	|ИЗ
	|	ДетальныеОстаткиНаКонецМесяцаИсправления КАК ДетальныеОстатки
	|
	|ГДЕ
	|	ДетальныеОстатки.Организация = &Организация
	|	И ДетальныеОстатки.АналитикаУчетаНоменклатуры.МестоХранения = &Склад
	|	И ДетальныеОстатки.ВидЗапасов.ТипЗапасов В
	...
	Для Каждого ТекущиеДанные Из Запрос.Выполнить().Выгрузить() Цикл
