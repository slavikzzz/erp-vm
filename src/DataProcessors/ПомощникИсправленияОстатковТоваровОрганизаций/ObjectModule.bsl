...
Процедура ОбновитьДанныеПоОтрицательнымОстаткамИСальдо() Экспорт
	...
	Если ЗначениеЗаполнено(МесяцИсправления) Тогда
		...
		Если ВыборкаДетальныеЗаписи.Следующий() Тогда
			КоличествоПозицийОтрицательныйОстатокПрочее = ВыборкаДетальныеЗаписи.КоличествоПозиций;
		КонецЕсли;
		//-- НЕ УТ
...
Процедура СформироватьНовыеРезервы(ПоНовымНастройкам = Ложь) Экспорт
	...
	Для Каждого Организация Из Организации Цикл
		...
		Попытка
			...
			Если ДокументОбъект.ДополнительныеСвойства.Свойство(ИмяТаблицыРезервыТоваровОрганизаций)
				...
				|en = 'Document is generated automatically from the wizard of correcting company stock balance %ТекущаяДатаСеанса%'");
	...
	Если БылиОшибки Тогда
		ТекстСообщения = НСтр("ru = 'При переформировании резервов товаров организации возникли ошибки. Подбробности в журнале регистрации.';
								|en = 'Errors occurred when rebuilding company goods reserves. For more information, see the event log.'");
	...
	Пока ВыборкаОрганизацииСОтрицательнымОстатком.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось сформировать исправления развернутого сальдо. Подробности в журнале регистрации.';
									|en = 'Cannot generate corrections of detailed balance. See the event log for details.'");
...
Процедура ПерезаполнитьВидыЗапасов() Экспорт
	...
	Пока Не Завершение Цикл
 		Если Итерация > 50 Тогда
			ТекстИсключения = НСтр("ru = 'Превышен лимит итераций перезаполнения видов запасов';
									|en = 'Limit of inventory owner attribute refilling iteration is exceeded'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
Процедура ЗаполнитьРаспоряженияНаОформлениеОприходований() Экспорт
	...
	|		ПО ДетальныеОстатки.АналитикаУчетаНоменклатуры = КлючиАналитикиУчетаНоменклатуры.Ссылка
	|ГДЕ
	|	ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца)
	|	И КлючиАналитикиУчетаНоменклатуры.ТипМестаХранения = ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.Склад)
	|	И КлючиАналитикиУчетаНоменклатуры.СкладскаяТерритория.ЦеховаяКладовая
	|	И КлючиАналитикиУчетаНоменклатуры.СкладскаяТерритория.Подразделение.ПроизводственноеПодразделение
	|	И ДетальныеОстатки.КоличествоОстаток < 0
	//-- НЕ УТКА
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура СформироватьИсправительныеСчетаФактуры() Экспорт
	...
	Если Не ТаблицаИзменнныхДокументов.Количество() Тогда
		ВызватьИсключение НСтр("ru = 'Все необходимые счета-фактуры уже сформированы';
								|en = 'All necessary tax invoices have already been generated'");
	КонецЕсли;
	...
	Если БылиОшибки Тогда
		...
		|en = 'Cannot generate corrective tax invoices for all changed documents. See the event log for details.'");
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	...
	Для Каждого Стр Из ДокументыКПерезаполнению Цикл 
		...
		Исключение
			...
			ТекстОшибки = НСтр("ru = 'Возникла ошибка при перезаполнении видов запасов за период %ТекущийМесяц%: %ПричинаОшибки%';
								|en = 'An error occurred while refilling inventory owner attributes for the %ТекущийМесяц% period. Reason:%ПричинаОшибки%'");
...
Процедура СообщитьПрогрессПерезаполненияВидовЗапасов()
	...
	ТекстСообщения = НСтр("ru = 'Выполняется перезаполнение видов запасов';
							|en = 'Refilling inventory owner attributes'");
	...
	Если РезультатЗапроса[ВГраница - 2].Пустой() <> РезультатЗапроса[ВГраница - 1].Пустой() Тогда
		ТекстИсключения = 
			НСтр("ru = 'По аналитике учета номенклатуры %АналитикаУчетаНоменклатуры% не найдены документы для перезаполнения видов запасов';
				|en = 'No documents are found for refilling inventory owner attributes under the %АналитикаУчетаНоменклатуры% inventory location attributes'",
...
Функция ИмяСобытия()
	Возврат НСтр("ru = 'Ошибка при работе помощника исправления остатков товаров организаций';
				|en = 'An error occurred in the wizard of correcting company stock balance'",  
...
Функция ЗапросКоличестваПроблемныхПозиций()
	...
	Запрос.Текст = СтрЗаменить(
		...
		|	0 КАК КоличествоПозиций",
		"ВЫБРАТЬ
		|	КОЛИЧЕСТВО(ОтборОтрицательныеОстаткиПрочее.АналитикаУчетаНоменклатуры) КАК КоличествоПозиций
		|ИЗ
		|	ОтборОтрицательныеОстаткиПрочее КАК ОтборОтрицательныеОстаткиПрочее");
	//-- НЕ УТ
...
Функция ЗапросОтборовПроблемныхОстатков()
	...
	Запрос.Текст = 
		...
		|	ДетальныеОстатки.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|	ДетальныеОстатки.Организация КАК Организация
		|ПОМЕСТИТЬ ОтборОтрицательныеОстаткиПрочее
		|ИЗ
		|	ДетальныеОстаткиНаКонецМесяцаИсправления КАК ДетальныеОстатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЗапасов КАК ВидыЗапасов
		|		ПО ДетальныеОстатки.ВидЗапасов = ВидыЗапасов.Ссылка
		|ГДЕ
		|	ВидыЗапасов.ТипЗапасов В (&ТипыЗапасовПрочее)
		|
		|СГРУППИРОВАТЬ ПО
		|	ДетальныеОстатки.АналитикаУчетаНоменклатуры,
		|	ДетальныеОстатки.Организация
	...
	ТипыЗапасовПрочее = Новый Массив;
	ТипыЗапасовПрочее.Добавить(Перечисления.ТипыЗапасов.ПродукцияПереработчика);
	
	//++ НЕ УТКА
	ТипыЗапасовПрочее.Добавить(Перечисления.ТипыЗапасов.ПродукцияДавальца);
	//-- НЕ УТКА
	
	Запрос.УстановитьПараметр("ТипыЗапасовПрочее", ТипыЗапасовПрочее);
	//-- НЕ УТ
