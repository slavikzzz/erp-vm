...
Процедура СформироватьДвиженияПоРегистру(Параметры, АдресХранилища = "") Экспорт
	...
	ВывестиПротокол(НСтр("ru = 'Начало заполнения. Предстоит обработать:';
						|en = 'Population start. To be processed:'"), ТипыРегистраторов);
	...
	ВывестиПротокол(НСтр("ru = 'Конец заполнения. Всего обработано:';
						|en = 'Population end. Total processed:'"), ОбработанныеТипы, Замеры);
...
Процедура ДобавитьТекстЗапросаДенежныеДокументы(Запрос, ТаблицаДвижений)
	...
	|	Т.Подразделение КАК Подразделение,
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) КАК НаправлениеДеятельности,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ДенежныеДокументы) КАК Статья,
	|	НЕОПРЕДЕЛЕНО КАК Аналитика,
	|	СУММА(Т.СуммаУпр) КАК Сумма,
	|	СУММА(ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -Т.СуммаУпр ИНАЧЕ Т.СуммаУпр КОНЕЦ) КАК СуммаБаланса,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыИсточниковДвижений.ДвиженияБаланса) КАК ВидИсточника,
...
Процедура ДобавитьТекстЗапросаРасчетыПоФинансовымИнструментам(Запрос, ТаблицаДвижений)
	...
	|				ИЛИ ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.РасчетКурсовыхРазниц)
	|				ИЛИ ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.ЗаключениеДоговораАренды))
	|		И &Отбор
	|) КАК ВложенныйЗапрос
	|	
	|СГРУППИРОВАТЬ ПО
...
Процедура ДобавитьТекстЗапросаПрочиеРасходы(Запрос, ТаблицаДвижений)
	...
	//-- Локализация
	|							ТОГДА ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.РасходыТекущегоПериода)
	|						КОГДА ТИПЗНАЧЕНИЯ(Т.АналитикаРасходов) = ТИП(Справочник.ОбъектыЭксплуатации)
	|							ТОГДА ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.КапитализацияОС)
	|						КОГДА ТИПЗНАЧЕНИЯ(Т.АналитикаРасходов) = ТИП(Справочник.ОбъектыСтроительства)
	|							ТОГДА ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.КапитализацияОбъектовСтроительства)
	|						КОГДА ТИПЗНАЧЕНИЯ(Т.АналитикаРасходов) = ТИП(Справочник.НематериальныеАктивы)
	|							ТОГДА ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.КапитализацияНМАиНИОКР)
	|						ИНАЧЕ
...
Процедура ДобавитьТекстЗапросаТрудозатратыНезавершенногоПроизводства(Запрос, ТаблицаДвижений)
	...
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НезавершенноеПроизводство) КАК Статья,
	|	НЕОПРЕДЕЛЕНО КАК Аналитика,
	|	СУММА(Т.Стоимость) КАК Сумма,
	|	СУММА(ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -Т.Стоимость ИНАЧЕ Т.Стоимость КОНЕЦ) КАК СуммаБаланса,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыИсточниковДвижений.РасчетСебестоимости) КАК ВидИсточника,
...
Процедура ДобавитьТекстЗапросаАмортизацияОС(Запрос, ТаблицаДвижений)
	...
	ПараметрыЗапроса.Вставить("ИмяСтатьиАП", "ОсновныеСредства");
...
Процедура ДобавитьТекстЗапросаАмортизацияНМА(Запрос, ТаблицаДвижений)
	...
	ПараметрыЗапроса.Вставить("ИмяРесурса", "Амортизация");
	ПараметрыЗапроса.Вставить("ИмяСтатьиАП", "НематериальныеАктивы");
...
Процедура ДобавитьТекстЗапросаОбесценениеВНА(Запрос, ТаблицаДвижений)
	...
	ПараметрыЗапроса.Вставить("ИмяСтатьиАП", "ОбесценениеВНА");
...
Функция ОтборПрибылиУбыткиДооценкиАмортизации()
	...
	|		И Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.УвеличениеНакопленнойАмортизацииОС)
...
Функция ОтборПрибылиУбыткиДооценкиСтоимости()
	...
	|		И (Т.СтоимостьРегл <> 0 ИЛИ Т.Стоимость <> 0))";
...
Функция ОтборПрибылиУбыткиРезерваПереоценкиСтоимостиОС()
	...
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.УменьшениеВеличиныДооценкиСтоимостиОС))";
...
Функция ОтборПрибылиУбыткиРезерваПереоценкиСтоимостиНМА()
	...
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.УменьшениеВеличиныДооценкиСтоимостиНМА))";
...
Процедура ДобавитьТекстЗапросаРезервыПоСомнительнымДолгам(Запрос, ТаблицаДвижений)
	...
	|	Т.ОбъектРасчетов.Подразделение КАК Подразделение,
	|	АналитикаПартнеров.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.РезервыПоСомнительнымДолгам) КАК Статья,
	|	НЕОПРЕДЕЛЕНО КАК Аналитика,
	|	СУММА(Т.СуммаУпр) КАК Сумма,
...
Процедура ДобавитьТекстЗапросаИнвестицииВАренду(Запрос, ТаблицаДвижений)
	...
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ИнвестицииВАренду) КАК Статья,
...
Процедура ДобавитьТекстЗапросаДвиженийДенежныхСредств(Запрос)
	...
	|	КБ.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ВЫБОР КОГДА КБ.Сумма > 0
	|		ТОГДА ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ВложенияСобственныхСредств)
	|		ИНАЧЕ ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ВыводСобственныхСредств)
	|	КОНЕЦ КАК Статья,
	|	ДенежныеСредства.Аналитика КАК Аналитика,
	|	ВЫБОР КОГДА КБ.Сумма < 0 ТОГДА -КБ.Сумма ИНАЧЕ КБ.Сумма КОНЕЦ КАК Сумма,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыИсточниковДвижений.РасчетБаланса) КАК ВидИсточника,
	...
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) КАК НаправлениеДеятельности,
	|	ВЫБОР КОГДА КБ.Сумма > 0
	|		ТОГДА ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ВыводСобственныхСредств)
...
Процедура ДобавитьТекстЗапросаДвиженийАктивовПассивов(Запрос)
	...
	Если ПолучитьФункциональнуюОпцию("ФормироватьФинансовыйРезультат") Тогда
		...
		|		И (ТИПЗНАЧЕНИЯ(ДокументСторно.СторнируемыйДокумент) = ТИП(Документ.ВводОстатков)
		|			ИЛИ ТИПЗНАЧЕНИЯ(ДокументСторно.СторнируемыйДокумент) = ТИП(Документ.ВводОстатковВнеоборотныхАктивов2_4)
		|			ИЛИ ТИПЗНАЧЕНИЯ(ДокументСторно.СторнируемыйДокумент) = ТИП(Документ.ВводОстатковПрочиеРасходы)
		|			ИЛИ ТИПЗНАЧЕНИЯ(ДокументСторно.СторнируемыйДокумент) = ТИП(Документ.ВводОстатковПрочихАктивовПассивов)
		|			ИЛИ ТИПЗНАЧЕНИЯ(ДокументСторно.СторнируемыйДокумент) = ТИП(Документ.ВводОстатковСПодотчетниками)
		|		)
		|	)
		|
		|ОБЪЕДИНИТЬ ВСЕ
	...
	Пока ВыборкаПоДокументу.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось обработать %Документ% по причине: %Причина%';
									|en = 'Cannot process %Документ%. Reason: %Причина%'");
...
Функция ИмяСобытияЖурналаРегистрации_ЗаполнениеРегистра()
	...
	Возврат НСтр("ru = 'Заполнение регистра ""Активы и пассивы""';
				|en = 'Populate the ""Assets and liabilities"" register'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СформироватьТекстПоТаблице(ТипыРегистраторов)
	...
	ЗаголовокТаблицы = НСтр("ru = 'Имя документа - Всего документов';
							|en = 'Document name - Total documents'");
	...
	Если ТипыРегистраторов.Колонки.Количество() > 3 Тогда
		ЗаголовокТаблицы = НСтр("ru = 'Имя документа - Обработано документов / Записано движений - Всего затрачено (ЧЧ:ММ:СС,мс)';
								|en = 'Document name - Processed documents / Records saved - Total time spent (HH:MM:SS,ms)'");
	...
	КонецЦикла;
	Если ТипыРегистраторов.Колонки.Количество() = 3 Тогда
		ИтоговаяСтрока = СтрШаблон(ШаблонСтроки, НСтр("ru = 'Итого';
														|en = 'Total'"), ТипыРегистраторов.Итог("ВсегоДокументов"));
...
Функция СформироватьТекстЗамеров(Замеры)
	...
	Текст = Символы.ПС + НСтр("ru = 'Затраты времени:';
								|en = 'Time expenditures:'")+Символы.ПС;
