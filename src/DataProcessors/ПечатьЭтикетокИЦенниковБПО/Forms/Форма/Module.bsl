	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенникаЭтикетки) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не выбран шаблон печати';
									|en = 'Print template is not selected'");
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(ТекущиеДанные.Количество) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не задано количество ценников';
									|en = 'Number of price tags is not set'");
...
Процедура ЗаполнитьПоОтбору(Команда)
	...
	Иначе
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Сохранить данные о количестве, шаблонах в табличной части при перезаполнении?';
												|en = 'Save information on quantity and templates in the table when refilling?'"), РежимДиалогаВопрос.ДаНетОтмена,,КодВозвратаДиалога.Да);
...
Процедура Печать(Команда)
	...
	Иначе
		...
		ДанныеДляПринтераЭтикеток = ДанныеДляПринтераЭтикетокНаСервере(ПараметрыПечати, ПоляДляЗаполнения, СписокУстройств);
...
Процедура УстановитьКоличествоЦенников(Команда)
	...
	ПоказатьВводЧисла(ОписаниеОповещения,ВыбранноеЗначение, НСтр("ru = 'Введите количество ценников/этикеток';
																|en = 'Enter the number of price tags/labels'"), 10, 0);
...
Процедура ОповещениеУстановитьКоличествоЦенников(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ВыбранноеЗначение = Неопределено Тогда
		...
		Текст = НСтр("ru = 'Установлено количество ценников %Количество%.';
					|en = 'Number of %Количество% price tags is set.'");
	...
	Если Количество < КоличествоВсего Тогда
		...
		Текст = Текст
		        ...
		        + НСтр("ru = 'Отметка выбора установлена для %КоличествоВыбранных% строк из %КоличествоВсего%.';
						|en = 'Selection mark is set for %КоличествоВыбранных% lines out of %КоличествоВсего%.'");
	
		Текст = Текст
		        + Символы.ПС
		        + НСтр("ru = 'Проверьте, заполнены ли все обязательные реквизиты.';
						|en = 'Check if all required attributes are filled.'");
	
		Текст = СтрЗаменить(Текст, "%КоличествоВыбранных%", Количество);
		Текст = СтрЗаменить(Текст, "%КоличествоВсего%", КоличествоВсего);
		
		ПоказатьОповещениеПользователя(НСтр("ru = 'Операция выполнена';
											|en = 'The operation is complete'"), , Текст);
...
&НаСервере
Функция ДанныеДляПринтераЭтикетокНаСервере(ПараметрыПечати, ПоляДляЗаполнения, СписокДоступныхУстройств = Неопределено)
	...
	ПоддерживаемыеТипы.ПринтерЭтикеток = Истина;
	
	СписокОборудованияОтбор = МенеджерОборудованияКлиентСервер.СписокОборудованияОтбор();
	СписокОборудованияОтбор.ТипыПО = ПоддерживаемыеТипы;
...
Процедура ЗагрузитьНастройкиОтбора()
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'Не определен макет компоновки данных.';
								|en = 'Data composition template is not defined.'");
