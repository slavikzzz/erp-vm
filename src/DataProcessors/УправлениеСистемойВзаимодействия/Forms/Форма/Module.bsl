...
Процедура СвязиПриложенийПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Оповещение = Новый ОписаниеОповещения("ФормаСвязиПриложенийЗакрыта", ЭтотОбъект);
	ФормаСвязи = ОткрытьФорму(ИмяФормыСвязи, , , , , , Оповещение);
	ФормаСвязи.Открыть();
...
Процедура СвязиПриложенийПередУдалением(Элемент, Отказ)
	...
	Отказ = Истина;
	Оповещение = Новый ОписаниеОповещения("ПодтверждениеРазрываСвязи", ЭтотОбъект);
	
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Отменить совместное использование?';
									|en = 'Do you want to cancel sharing?'"), РежимДиалогаВопрос.ДаНет, , 
...
Процедура ПерейтиНаСтраницуСопоставленияПользователей(Команда)
	...
	Заголовок = НСтр("ru = 'Сопоставление пользователей сервиса';
...
Процедура Зарегистрировать(Команда)
	...
	Если СпособРегистрации = СпособРегистрацииБезСервиса().Значение Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("АктивацияЗавершена", ЭтотОбъект, , "ОшибкаАктивации", ЭтотОбъект);
	...
	ИначеЕсли СпособРегистрации = СпособРегистрацииЧерезСервис().Значение Тогда
		...
		Если РезультатРегистрации <> Неопределено Тогда
			Если РезультатРегистрации.Успешно Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Регистрация успешно выполнена';
			...
			КонецЕсли;
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Укажите необходимые данные';
...
Процедура ПолучитьКодРегистрации(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьКодРегистрацииЗавершение", ЭтотОбъект, , "ПолучитьКодРегистрацииОшибка", ЭтотОбъект);
...
Процедура АктивацияЗавершена(Результат, ТекстСообщения, ДополнительныеПараметры) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("АктивацияЗавершенаПредупреждение", ЭтотОбъект);
...
Процедура СохранитьСопоставлениеНаСервере()
	...
	Для Каждого Стр Из ТаблицаСопоставления Цикл
		...
		//@skip-check empty-except-statement - такой способ предалагется в статье на ИТС
		Попытка
			ИдентификаторПользователяСВ = СистемаВзаимодействия.ПолучитьИдентификаторПользователя(УникальныйИдентификаторПользователя);
		Исключение
		КонецПопытки;
...
Процедура ПолучитьКодРегистрацииЗавершение(Результат, ТекстСообщения, ДополнительныеПараметры) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьКодРегистрацииЗавершениеПредупреждение", ЭтотОбъект);
