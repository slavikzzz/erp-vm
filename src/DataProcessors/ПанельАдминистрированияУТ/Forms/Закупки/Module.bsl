...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ВнешниеРодительскиеКонстанты.Вставить("ИспользоватьПроизводствоНаСтороне2_5");
	//-- НЕ УТ
	...
	Элементы.ПояснениеИспользованиеЭтаповОплатыВЗакупках.Заголовок = НСтр(
		...
		|en = 'Set up possible payment planning options in documents ""Consignment notification"".'");
	
	//++ НЕ УТ
	Элементы.ПояснениеИспользованиеЭтаповОплатыВЗакупках.Заголовок = НСтр(
		"ru = 'Настройка определяет возможные варианты планирования оплаты в документах ""Заказ поставщику"", ""Отчет комитенту"",
		|""Заказ переработчику"".';
		|en = 'Set up possible payment planning options in documents ""Purchase order"", ""Consignment issue notification"",
		|""Purchase order — Subcontracting"".'");
	
	//++ Устарело_Переработка24
	Если НаборКонстант.ИспользуетсяПередачаВПереработку24и25 Тогда
		Элементы.КомментарийИспользоватьДоговорыСПоставщикамиПередачаВПереработку.Заголовок =
		НСтр("ru = '— включена опция ""Производство на стороне (версия 2.5)""';
			|en = '— the ""Production by third party (version 2.5)"" option is enabled'");
	КонецЕсли;
	//-- Устарело_Переработка24
	...
	Если Результат.Свойство("ВерсионированиеВключено") Тогда
		...
		Если Результат.ВерсионированиеВключено Тогда
			Пояснение = НСтр("ru = 'Для документов заказов поставщику установлен вариант версионирования ""Версионировать при проведении""';
							|en = 'Versioning ""Version on posting"" is set for purchase orders'");
		Иначе
			Пояснение = НСтр("ru = 'Для документов заказов поставщику версионирование уже было включено';
							|en = 'Versioning has already been enabled for purchase order documents'");
		КонецЕсли;
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьДоговорыСПоставщиками" ИЛИ РеквизитПутьКДанным = "" Тогда
		...
		//++ НЕ УТ
		ЗначениеКонстантыПередачаВПереработку       = НаборКонстант.ИспользоватьПроизводствоНаСтороне2_5;
		//-- НЕ УТ
		...
		//++ НЕ УТ
		ДоговорыСПоставщикамиРедактированиеДоступно = ДоговорыСПоставщикамиРедактированиеДоступно
													  И Не ЗначениеКонстантыПередачаВПереработку;
		//-- НЕ УТ
		...
		//++ НЕ УТ
		Элементы.КомментарийИспользоватьДоговорыСПоставщикамиПередачаВПереработку.Видимость         = ЗначениеКонстантыПередачаВПереработку;
		//-- НЕ УТ
