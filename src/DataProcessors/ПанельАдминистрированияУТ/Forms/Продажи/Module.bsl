...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КомментарийИспользованияДоговоровПриКомиссионнойПродаже = СтрШаблон(НСтр("ru = '— включена опция ""Комиссионные продажи %1""';
																			|en = '— the ""Consignment sales %1"" option is enabled'"), 
	...
	КомментарийКомиссии = СтрШаблон(
			НСтр("ru = 'Возможность осуществления комиссионных продаж %1 доступна при использовании соглашений с клиентами.';
				|en = 'Consignment sales %1 are available if terms of sales are used.'"), 
	...
	КомментарийКомиссии = СтрШаблон(
			НСтр("ru = 'Возможность осуществления комиссионных продаж %1 доступна при использовании договоров с клиентами.';
				|en = 'Consignment sales %1 are available if contracts with customers are used.'"), 
			КомиссионнаяТорговляСервер.ПостфиксСхемыКомиссии25());
	...
	Если Результат.Свойство("ВерсионированиеЗаказовКлиентовВключено") Тогда
		...
		Если Результат.ВерсионированиеЗаказовКлиентовВключено Тогда
			Пояснение = НСтр("ru = 'Для заказов клиентов установлен вариант версионирования ""Версионировать при проведении""';
							|en = 'The ""Version on posting"" versioning option is set for sales orders'");
		Иначе
			Пояснение = НСтр("ru = 'Для заказов клиентов версионирование уже было включено';
							|en = 'Versioning was already enabled for sales orders'");
		КонецЕсли;
	...
	Если Результат.Свойство("ВерсионированиеЗаявокНаВозвратВключено") Тогда
		...
		Если Результат.ВерсионированиеЗаявокНаВозвратВключено Тогда
			Пояснение = НСтр("ru = 'Для заявок на возврат установлен вариант версионирования ""Версионировать при проведении""';
							|en = 'The ""Version on posting"" versioning option is set for return requests'");
		Иначе
			Пояснение = НСтр("ru = 'Дляаявок на возврат версионирование уже было включено';
							|en = 'Versioning is already performed for return requests'");
		КонецЕсли;
	...
	Если Результат.Свойство("ВерсионированиеСоглашенийВключено") Тогда
		...
		Если Результат.ВерсионированиеСоглашенийВключено Тогда
			Пояснение = НСтр("ru = 'Для соглашений с клиентами установлен вариант версионирования ""Версионировать при проведении""';
							|en = 'The ""Version on posting"" versioning option is set for terms of sales'");
		Иначе
			Пояснение = НСтр("ru = 'Для соглашений с клиентами версионирование уже было включено';
							|en = 'Versioning was already enabled for terms of sales'");
		КонецЕсли;
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "" Тогда
		...
		//++ НЕ УТКА
		ДоступноПроизводствоИзДавальческогоСырья = Истина;
		//-- НЕ УТКА
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьЗаказыКлиентов" ИЛИ РеквизитПутьКДанным = "" Тогда
		...
		КомментарийИспользоватьЗаказы = НСтр("ru = 'Невозможно отключение заказов клиентов, так как';
											|en = 'Cannot disable sales orders as'")
			...
			+ ?(Использовать1СЗаказы, НСтр("ru = 'включена синхронизация данных с мобильным приложением ""1С:Заказы""';
											|en = 'data synchronization with mobile application 1C:Orders is enabled'"), "")
			...
			+ ?(ВедетсяРаботаЧерезТорговыхПредставителей, НСтр("ru = 'включено ведение работы через торговых представителей.';
																|en = 'work via sales representatives is enabled.'"), "");
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьДоговорыСКлиентами" 
		...
		//++ НЕ УТКА
		Элементы.ПредупреждениеИспользоватьДоговорыСКлиентамиПриемВПереработку.Видимость = ЗначениеКонстантыПриемаВПереработку;
		//-- НЕ УТКА
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьПроизводство"
		...
		Элементы.ГруппаКомментарийИспользоватьПроизводствоИзДавальческогоСырья2_5.Видимость =
			Не НаборКонстант.ИспользоватьУправлениеПроизводством2_2
			Или Не НаборКонстант.НоваяАрхитектураВзаиморасчетов;
		
		Если Не НаборКонстант.ИспользоватьУправлениеПроизводством2_2
			Или Не НаборКонстант.НоваяАрхитектураВзаиморасчетов Тогда
			
			ШаблонЗаголовка = НСтр("ru = 'Для использования производства из давальческого сырья необходимо включить опции:
									|%1';
									|en = 'To use subcontracting, enable the following options:
									|%1'");
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья2_5"
		Или РеквизитПутьКДанным = "НаборКонстант.ИспользоватьУправлениеПроизводством2_2"
		Или РеквизитПутьКДанным = "НаборКонстант.НоваяАрхитектураВзаиморасчетов"
		Или РеквизитПутьКДанным = "" Тогда
		
		ИспользоватьУправлениеПроизводством2_2 = НаборКонстант.ИспользоватьУправлениеПроизводством2_2;
		
		Элементы.ИспользоватьПроизводствоИзДавальческогоСырья2_5.Доступность =
			ИспользоватьУправлениеПроизводством2_2
			И НаборКонстант.НоваяАрхитектураВзаиморасчетов;
		
		Элементы.ГруппаАктыРасхожденияДавальцаГоризонтально.Доступность =
			НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья2_5;
		
		Элементы.ИспользоватьЗаказыДавальцев2_5.Доступность =
			НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья2_5;
		
	КонецЕсли;
	
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья2_5"
		Или РеквизитПутьКДанным = "НаборКонстант.ИспользоватьЗаказыДавальцев2_5"
		Или РеквизитПутьКДанным = "" Тогда
		...
		Элементы.ИспользоватьСтатусыЗаказовДавальцев2_5.Доступность =
			НаборКонстант.ИспользоватьЗаказыДавальцев2_5;
		
		Элементы.ИспользоватьОтчетДавальцуПоНесколькимЗаказам2_5.Доступность =
			НаборКонстант.ИспользоватьЗаказыДавальцев2_5
			И НЕ ЕстьОбменыСФормированиемДоговоровПоЗаказам;
		
		Элементы.ГруппаКомментарийИспользоватьОтчетДавальцуПоНесколькимЗаказам2_5.Видимость =
			ЕстьОбменыСФормированиемДоговоровПоЗаказам;
		
		Элементы.ВариантОбособленияМатериаловПриПриемеВПереработку.Доступность =
			НаборКонстант.ИспользоватьЗаказыДавальцев2_5;
		
		ВариантОбособленияЗаказДавальца =
			ПредопределенноеЗначение("Перечисление.ВариантыОбособленияПриПриемеВПереработку.ЗаказДавальца");
		
		СпискиВыбора = Новый Массив;
		СпискиВыбора.Добавить(Элементы.ВариантОбособленияМатериаловПриПриемеВПереработку.СписокВыбора);
		СпискиВыбора.Добавить(Элементы.ВариантОбособленияПродукцииПриПриемеВПереработку.СписокВыбора);
		
		Для каждого ТекущийСписокВыбора Из СпискиВыбора Цикл
			
			ЗначениеЗаказДавальца = ТекущийСписокВыбора.НайтиПоЗначению(ВариантОбособленияЗаказДавальца);
			
			Если НаборКонстант.ИспользоватьЗаказыДавальцев2_5 И ЗначениеЗаказДавальца = Неопределено Тогда
				ТекущийСписокВыбора.Добавить(ВариантОбособленияЗаказДавальца);
			ИначеЕсли Не НаборКонстант.ИспользоватьЗаказыДавальцев2_5 И ЗначениеЗаказДавальца <> Неопределено Тогда
				ТекущийСписокВыбора.Удалить(ЗначениеЗаказДавальца);
			КонецЕсли;
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья2_5"
		Или РеквизитПутьКДанным = "НаборКонстант.ИспользоватьЗаказыДавальцев2_5"
		Или РеквизитПутьКДанным = "НаборКонстант.ИспользоватьСтатусыЗаказовДавальцев2_5"
		Или РеквизитПутьКДанным = "" Тогда
		
		ИспользоватьПроизводствоИзДавальческогоСырья2_5 =
			НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья2_5;
		
		ИспользоватьСтатусыЗаказовДавальцев2_5 =
			НаборКонстант.ИспользоватьСтатусыЗаказовДавальцев2_5;
		
		Элементы.НеЗакрыватьЗаказыДавальцевБезПолнойОтработки2_5.Доступность =
			ИспользоватьПроизводствоИзДавальческогоСырья2_5
			И ИспользоватьСтатусыЗаказовДавальцев2_5;
		
		Элементы.НеЗакрыватьЗаказыДавальцевБезПолнойОплаты2_5.Доступность =
			ИспользоватьПроизводствоИзДавальческогоСырья2_5
			И ИспользоватьСтатусыЗаказовДавальцев2_5;
		
		Элементы.ГруппаКомментарийКонтролироватьЗакрытиеЗаказовДавальцев2_5.Видимость =
			ИспользоватьПроизводствоИзДавальческогоСырья2_5
			И НЕ ИспользоватьСтатусыЗаказовДавальцев2_5;
		
	КонецЕсли;
	
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья2_5"
	 Или РеквизитПутьКДанным = "НаборКонстант.ИспользоватьЗаказыДавальцев2_5"
	 Или РеквизитПутьКДанным = "НаборКонстант.ВариантОбособленияМатериаловПриПриемеВПереработку"
	 Или РеквизитПутьКДанным = "" Тогда
		
		Если НаборКонстант.ВариантОбособленияМатериаловПриПриемеВПереработку =
				ПредопределенноеЗначение("Перечисление.ВариантыОбособленияПриПриемеВПереработку.ЗаказДавальца") Тогда
			
			Элементы.ВариантОбособленияПродукцииПриПриемеВПереработку.Доступность                = Ложь;
			Элементы.ГруппаВариантОбособленияПродукцииПриПриемеВПереработкуКомментарий.Видимость = Истина;
			
		Иначе
			
			Элементы.ВариантОбособленияПродукцииПриПриемеВПереработку.Доступность =
				НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья2_5
				И НаборКонстант.ИспользоватьЗаказыДавальцев2_5;
			
			Элементы.ГруппаВариантОбособленияПродукцииПриПриемеВПереработкуКомментарий.Видимость = Ложь;
...
Процедура ОтображениеПредупрежденияПриРедактировании(РеквизитПутьКДанным)
	...
	СтруктураКонстант = Новый Структура(
		...
		//++ НЕ УТКА
		|ИспользоватьПроизводствоИзДавальческогоСырья2_5,
		|ИспользоватьЗаказыДавальцев2_5,
		|ИспользоватьСтатусыЗаказовДавальцев2_5,
		//-- НЕ УТКА
