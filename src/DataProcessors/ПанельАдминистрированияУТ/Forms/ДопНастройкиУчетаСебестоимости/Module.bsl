...
Функция ПартионныйУчетНельзяВыключать()
	...
	|	4 КАК КодПричины
	|ИЗ
	|	РегистрСведений.НастройкиУчетаУСН КАК РегУчетнаяПолитика
	|ГДЕ
	|	РегУчетнаяПолитика.ОбъектНалогообложенияУСН = ЗНАЧЕНИЕ(Перечисление.ОбъектыНалогообложенияПоУСН.ДоходыМинусРасходы)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
		...
		Если Выборка.КодПричины = 1 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
				|    - " + НСтр("ru = 'есть документы ""Расчет себестоимости товаров"" с методом оценки стоимости товаров ""ФИФО (скользящая оценка)""';
								|en = 'there are the ""Goods cost calculation"" documents with the ""FIFO (rolling valuation)"" goods cost estimation method'");
		ИначеЕсли Выборка.КодПричины = 2 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
				|    - " + НСтр("ru = 'есть организации с методом оценки стоимости товаров ""ФИФО (скользящая оценка)""';
								|en = 'there are companies with the ""FIFO (rolling valuation)"" goods cost estimation method'");
		ИначеЕсли Выборка.КодПричины = 3 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
				|    - " + НСтр("ru = 'есть организации, ведущие учет НДС по фактическому использованию';
								|en = 'There are companies keeping VAT accounting by actual usage '");
		//++ НЕ УТ
