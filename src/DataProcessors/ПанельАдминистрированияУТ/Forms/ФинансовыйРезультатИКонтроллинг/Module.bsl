...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ВариантПроверкиДокументов = ?(НаборКонстант.РазрешатьИзменятьПроверенныеФинансовыеДокументы, 1, 0);
	//-- НЕ УТ
...
Процедура ВариантПроверкиДокументовПриИзменении(Элемент)
	//++ НЕ УТ
	НаборКонстант.РазрешатьИзменятьПроверенныеФинансовыеДокументы = ВариантПроверкиДокументов;
	//-- НЕ УТ
...
Процедура ВосстановитьПоставляемуюМодельПоказателей(Команда)
	ПоказатьВопрос(Новый ОписаниеОповещения("ВосстановитьПоставляемуюМодельПоказателейЗавершение", ЭтаФорма), 
		...
		|en = 'Settings of built-in target categories, target indicators, analysis options, and target values will be reset.
		|Continue with the loss of built-in indicator model settings?'"), РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура ВосстановитьПоставляемуюМодельПоказателейЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ПоказатьПредупреждение(,НСтр("ru = 'Поставляемая модель показателей восстановлена.';
									|en = '1С-supplied indicator model is restored.'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура НастройкаПроверкиДокументов(Команда)
	//++ НЕ УТ
	ОткрытьФорму("РегистрСведений.НастройкиИсключенийПроверкиДокументов.Форма.ФормаНастройкиПроверкиДокументов");
	//-- НЕ УТ
	Возврат;
КонецПроцедуры
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	//++ НЕ УТКА
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьВнутреннююПереработку"
		Или РеквизитПутьКДанным = "" Тогда
		
		ОбщегоНазначенияУТКлиентСервер.ОтображениеПредупрежденияПриРедактировании(
			Элементы.ИспользоватьВнутреннююПереработку,
			НаборКонстант.ИспользоватьВнутреннююПереработку);
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьУчетПрочихДоходовРасходов" ИЛИ РеквизитПутьКДанным = "" Тогда
		...
		//++ НЕ УТ
		Элементы.ФормироватьРезервыПредстоящихРасходов.Доступность = НаборКонстант.ИспользоватьУчетПрочихДоходовРасходов
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьУчетСебестоимости"
	 ...
	 ИЛИ РеквизитПутьКДанным = "" Тогда
		...
		//++ НЕ УТ
		Элементы.ФормироватьРезервыПодОбесценениеЗапасов.Доступность = НаборКонстант.ИспользоватьРеглУчет
			И ПартионныйУчетВерсии22;
		Элементы.ГруппаФормироватьРезервыПодОбесценениеЗапасовЗависимости.Видимость = Не ПартионныйУчетВерсии22;	
		//-- НЕ УТ
	...
	Если РеквизитПутьКДанным = "" Тогда
		...
		//++ НЕ УТКА
		ВыключениеНаправленияДеятельностиНедоступно = УправлениеДаннымиОбИзделияхПовтИсп.ВидПараметраНазначенияИспользуется(
					Перечисления.ВидыПараметровНазначенияСпецификаций.НаправлениеДеятельности, Ложь)
				И Константы.ФормироватьФинансовыйРезультат.Получить();
		Элементы.ГруппаФинансовыйРезультатЛево.Доступность = НЕ ВыключениеНаправленияДеятельностиНедоступно;
		//-- НЕ УТКА
	...
	//++ НЕ УТКА
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьУправлениеПроизводством2_2"
		Или РеквизитПутьКДанным = "НаборКонстант.ИспользоватьНесколькоОрганизаций"
		Или РеквизитПутьКДанным = "" Тогда
		Элементы.ИспользоватьВнутреннююПереработку.Доступность =
			НаборКонстант.ИспользоватьНесколькоОрганизаций
			И НаборКонстант.ИспользоватьУправлениеПроизводством2_2;
	КонецЕсли;
	//-- НЕ УТКА
	
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьПередачиТоваровМеждуОрганизациями"
		//++ НЕ УТКА
		ИЛИ РеквизитПутьКДанным = "НаборКонстант.ИспользоватьВнутреннююПереработку"
		//-- НЕ УТКА
		ИЛИ РеквизитПутьКДанным = "" Тогда
		...
		Элементы.ИспользоватьДоговорыМеждуОрганизациями.Доступность =
			НесколькоОрганизаций
				И (ПередачиМеждуОрганизациями
					//++ НЕ УТКА
					Или НаборКонстант.ИспользоватьВнутреннююПереработку
					//-- НЕ УТКА
					Или Ложь);
	...
	Если РеквизитПутьКДанным = "" Тогда
		...
		Элементы.ГруппаУчетПоОбъектамВозникновенияЗатрат.Видимость = ЛОЖЬ
			//++ НЕ УТ
			ИЛИ НаборКонстант.КомплекснаяАвтоматизация 
			ИЛИ НаборКонстант.УправлениеПредприятием
			//-- НЕ УТ
		;
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьПроверкуФинансовыхДокументов"
		...
		Элементы.ГруппаВариантПроверкиДокументов.Доступность = НаборКонстант.ИспользоватьПроверкуФинансовыхДокументов;
...
Процедура ВыключитьКонтрольОстатковТоваровСервер()
	...
	Если ПараметрыСеанса.ПроводитьБезКонтроляОстатковТоваровОрганизаций Тогда
		ТекстСообщения = НСтр("ru = 'Пользователем %ИмяПользователя% в рамках своего сеанса выключен контроль остатков товаров организаций.';
								|en = 'Control of company stock balance is disabled by the %ИмяПользователя% user during their session.'", ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		ТекстСообщения = НСтр("ru = 'Пользователем %ИмяПользователя% возобновлен контроль остатков товаров организаций.';
								|en = 'Control of company stock balance is resumed by the %ИмяПользователя% user'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
