...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого ЭлементДляПроверкиПрав Из Параметры.МетаданныеДляПроверкиПравДоступаНаИзменение Цикл
		...
		Если ЗначениеЗаполнено(МетаданныеДляПроверкиПрав) И Не ПравоДоступа("Изменение", МетаданныеДляПроверкиПрав) Тогда
			ВызватьИсключение НСтр("ru = 'Недостаточно прав доступа для изменения свойств регламентного задания';
									|en = 'Недостаточно прав доступа для изменения свойств регламентного задания'");
		КонецЕсли;	
	КонецЦикла;
	...
	Если СтрНайти(", Добавить, Скопировать, Изменить,", ", " + Действие + ",") = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Неверные параметры открытия формы ""Регламентное задание"".';
								|en = 'Неверные параметры открытия формы ""Регламентное задание"".'");
	КонецЕсли;
	
	Если Действие = "Добавить" Тогда
		
		ИмяМетаданных = Параметры.ИмяМетаданныхРегламентногоЗадания;
		Если ПустаяСтрока(ИмяМетаданных) Тогда
			ВызватьИсключение НСтр("ru = 'Не указано имя регламентного задания для добавления.';
									|en = 'Не указано имя регламентного задания для добавления.'");
		КонецЕсли;
		
		Расписание = Новый РасписаниеРегламентногоЗадания;
		Задание = Метаданные.РегламентныеЗадания[ИмяМетаданных];
...
Процедура ЗаписатьРегламентноеЗаданиеНаСервере()
	...
	Если Не Действие = "Изменить" Тогда
		ПараметрыЗадания.Вставить("Метаданные", ИмяМетаданных);
	КонецЕсли;
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если Действие = "Изменить" Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (Регламентное задание)';
																				|en = '%1 (Регламентное задание)'"), Представление);
	Иначе
		Заголовок = НСтр("ru = 'Регламентное задание (создание)';
						|en = 'Регламентное задание (создание)'");
	КонецЕсли;
...
Функция АвтоНаименование(Форма)
	...
	СтрокаНаименования = Форма.СинонимМетаданных;
