...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяМетаданныхРегламентногоЗадания = Параметры.ИмяМетаданныхРегламентногоЗадания;
	Если ПустаяСтрока(ИмяМетаданныхРегламентногоЗадания) Тогда
		ВызватьИсключение НСтр("ru = 'Не указано имя регламентного задания для настройки.';
								|en = 'Не указано имя регламентного задания для настройки.'");
	КонецЕсли;
	МетаданныеДляПроверкиПравДоступаНаИзменение = Параметры.МетаданныеДляПроверкиПравДоступаНаИзменение;
	
	Если НЕ РазрешеноИзменение(МетаданныеДляПроверкиПравДоступаНаИзменение) Тогда
	...
	Заголовок = СтрШаблон(НСтр("ru = 'Настройка задания: %1';
								|en = 'Настройка задания: %1'"), Метаданные.РегламентныеЗадания[ИмяМетаданныхРегламентногоЗадания].Представление());
...
Процедура ТаблицаРегламентныеЗаданияПередНачаломИзменения(Элемент, Отказ)
	Отказ = Истина;
	Если РазрешеноИзменение(МетаданныеДляПроверкиПравДоступаНаИзменение) Тогда
...
Процедура ТаблицаРегламентныеЗаданияПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ВыделенныеСтроки.Количество() > 1 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите одно регламентное задание.';
										|en = 'Выберите одно регламентное задание.'"));
	...
	ИначеЕсли Элемент.ТекущиеДанные.Предопределенное Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Невозможно удалить предопределенное регламентное задание.';
										|en = 'Невозможно удалить предопределенное регламентное задание.'") );
	Иначе
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ТаблицаРегламентныеЗаданияПередУдалениемЗавершение", ЭтотОбъект),
			НСтр("ru = 'Удалить регламентное задание?';
				|en = 'Удалить регламентное задание?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьСкопироватьИзменитьРегламентноеЗадание(Знач Действие)
	...
	ПараметрыФормы.Вставить("ИмяМетаданныхРегламентногоЗадания", ИмяМетаданныхРегламентногоЗадания);
   	ПараметрыФормы.Вставить("МетаданныеДляПроверкиПравДоступаНаИзменение", МетаданныеДляПроверкиПравДоступаНаИзменение);
	
	ОткрытьФорму("Обработка.ПанельАдминистрированияУТ.Форма.НастройкаРегламентногоЗадания", ПараметрыФормы, ЭтотОбъект);
...
Процедура НастроитьРасписание(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите регламентное задание.';
										|en = 'Выберите регламентное задание.'"));
...
Функция ЗагрузитьРегламентныеЗадания(ИдентификаторЗадания = Неопределено)
	...
	Если ИдентификаторЗадания = Неопределено Тогда
		РегламентноеЗадание = Метаданные.РегламентныеЗадания[ИмяМетаданныхРегламентногоЗадания];
		ПредставлениеРегламентногоЗадания = РегламентноеЗадание.Представление();
		Отбор = новый Структура("Метаданные", Метаданные.РегламентныеЗадания[ИмяМетаданныхРегламентногоЗадания]);
	Иначе	
	...
	Если ИдентификаторЗадания = Неопределено Тогда
		...
		Для Каждого Задание Из ТекущиеЗадания Цикл
			...
			Если ПустаяСтрока(Обновляемое.Наименование) Тогда
				Обновляемое.Наименование = Автонаименование(ПредставлениеРегламентногоЗадания, Задание.Параметры);
			КонецЕсли;
	...
	Иначе
		...
		Если Строки.Количество() > 0 
			...
			Если ПустаяСтрока(Строки[0].Наименование) Тогда
				Строки[0].Наименование = Автонаименование(ПредставлениеРегламентногоЗадания, ТекущиеЗадания[0].Параметры);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция РазрешеноИзменение(МетаданныеДляПроверкиПравДоступаНаИзменение)
	Результат = Истина;

	Для Каждого ЭлементДляПроверкиПрав Из МетаданныеДляПроверкиПравДоступаНаИзменение Цикл
        МетаданныеДляПроверкиПрав = Метаданные.НайтиПоПолномуИмени(ЭлементДляПроверкиПрав.Значение);
		Если ЗначениеЗаполнено(МетаданныеДляПроверкиПрав) И Не ПравоДоступа("Изменение", МетаданныеДляПроверкиПрав) Тогда
			Результат = Ложь;
			Прервать;
		КонецЕсли;	
	КонецЦикла;
...
Функция АвтоНаименование(Представление, Параметры)
	
	СтрокаНаименования = Представление;
