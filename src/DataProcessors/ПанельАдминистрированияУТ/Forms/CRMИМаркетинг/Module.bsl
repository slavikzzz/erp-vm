...
Процедура ИспользоватьСогласованиеЦенНоменклатурыПриИзменении(Элемент)
	...
	Если НЕ НаборКонстант.ИспользоватьСогласованиеЦенНоменклатуры
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ИспользоватьСогласованиеЦенНоменклатурыПриИзмененииЗавершение", ЭтотОбъект, Новый Структура("Элемент", Элемент)), НСтр("ru = 'Существуют документы установки цен в статусе ""Не согласован"". Перед отключением согласования рекомендуется перейти в список документов установки цен и провести документы в статусе ""Согласован"", иначе дальнейшая работа с ними будет невозможа.';
																																										|en = 'There are pricing documents with status ""Not approved"". Before approval cancellation, it is recommended that you post these documents with status ""Approved"", otherwise, you will not be able to work with them later.'"), СписокКнопок);
		Возврат;
...
Процедура НазваниеГеографическойСхемыДляОтчетовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СписокВыбора.Добавить("Загрузить из файла...", НСтр("ru = 'Загрузить из файла...';
														|en = 'Import from file...'"),, БиблиотекаКартинок.ОткрытьФайл);
	...
	Если ВыбранноеЗначение <> Неопределено Тогда
		...
		Если ВыбранноеЗначение.Значение = "Загрузить из файла..." Тогда
			...
			ВыборФайла.Заголовок = НСтр("ru = 'Выбор карты';
										|en = 'Select map'");
			ВыборФайла.Фильтр = НСтр("ru = 'Географические схемы';
									|en = 'Geographical schemas'") + " (*.geo)|*.geo";
...
Процедура НазваниеГеографическойСхемыДляОтчетовВыборФайла(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Исключение
		...
		|en = 'The selected file format does not correspond to the supported format of geographic diagrams.
	...
	Если Результат.Свойство("ВерсионированиеВключено") Тогда
		...
		Если Результат.ВерсионированиеВключено Тогда
			Пояснение = НСтр("ru = 'Для документа ""Установки цен номенклатуры"" установлен вариант версионирования ""Версионировать при записи""';
							|en = 'Versioning ""Version on saving"" is set for document ""Product pricing""'");
		Иначе
			Пояснение = НСтр("ru = 'Для документа ""Установки цен номенклатуры"" версионирование уже было включено';
							|en = 'Versioning has already been enabled for document ""Product pricing""'");
		КонецЕсли;
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользуетсяЦенообразование25" ИЛИ РеквизитПутьКДанным = "" Тогда
		...
		Иначе
			...
			Если СостояниеПерехода.Состояние = Перечисления.СостоянияЗаданий.Запланировано 
				...
				Элементы.ОткрытьПомощникПереходаНаНовоеЦенообразование.Заголовок = НСтр("ru = 'Проверить текущее состояние перехода';
																						|en = 'Current migration state check'");
			Иначе	
				Элементы.ДекорацияИспользованиеЦенообразованияВерсии25ВПроцессе.Видимость = Ложь;
		...
		//++ НЕ УТ
		СкорректироватьВерсиюЦенообразования(Элементы.ДекорацияИспользованиеЦенообразованияВерсии25Доступен.Заголовок);
		СкорректироватьВерсиюЦенообразования(Элементы.ДекорацияИспользованиеЦенообразованияВерсии25ВПроцессе.Заголовок);
		СкорректироватьВерсиюЦенообразования(Элементы.ДекорацияИспользованиеЦенообразованияВерсии25Выполнено.Заголовок);
		СкорректироватьВерсиюЦенообразования(Элементы.ОткрытьПомощникПереходаНаНовоеЦенообразование.Заголовок);
		//-- НЕ УТ
...
Процедура СкорректироватьВерсиюЦенообразования(СтрокаДляИзменения)
	Если ТипЗнч(СтрокаДляИзменения) = Тип("ФорматированнаяСтрока") Тогда
		СкорректироватьВерсиюЦенообразованиянаСервере(СтрокаДляИзменения);
	Иначе
...
&НаСервереБезКонтекста
Процедура СкорректироватьВерсиюЦенообразованияНаСервере(СтрокаДляИзменения)
	ВнутрСтрокаДляИзменения = ЗначениеВСтрокуВнутр(СтрокаДляИзменения);
	ВнутрСтрокаДляИзменения = СтрЗаменить(ВнутрСтрокаДляИзменения, "11.", "2.");
	СтрокаДляИзменения = ЗначениеИзСтрокиВнутр(ВнутрСтрокаДляИзменения);
КонецПроцедуры
//-- НЕ УТ
