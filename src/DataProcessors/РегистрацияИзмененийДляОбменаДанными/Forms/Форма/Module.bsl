	...
	Если Не ПроконтролироватьНастройки() И ОткрытиеСУзломПараметром Тогда
		...
		ТекстСообщения = СтрЗаменить(
			НСтр("ru = 'Для ""%1"" редактирование регистрации объектов недоступно.';
				|en = 'Cannot change item stage state for node ""%1"".'"),
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ДлительнаяОперацияЗапущена Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Прервать выполнение регистрации данных?';
										|en = 'Abort stating?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура УдалитьРегистрациюИзСпискаСсылок()
	...
	Иначе
		Текст = НСтр("ru = 'Отменить регистрацию выбранных объектов
		                 |на узле ""%1""?';
		                 |en = 'Do you want to unstage the selected items
		                 |at node ""%1""?'"); 
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "%1", УзелОбменаСсылка);
...
Процедура ФоновоеЗаданиеПолучитьРезультатНаСервере()
	...
	Если ФоновоеЗаданиеРезультатВыполнения <> Неопределено Тогда
		...
		ТиповоеПредставлениеОшибки = НСтр("ru = 'При изменении регистрации произошла ошибка. Подробности см. в журнале регистрации';
											|en = 'Error changing stage state. See the Event log for details.'");
...
Процедура ОбработатьЗапретИзмененияУзла()
	...
	Если ОперацииРазрешены Тогда
		...
		Заголовок = НСтр("ru = 'Регистрация изменений для обмена данными';
						|en = 'Data staging manager'");
	Иначе
		Элементы.УзелОбменаСсылка.Видимость = Ложь;
		Заголовок = СтрЗаменить(НСтр("ru = 'Регистрация изменений для обмена с  ""%1""';
									|en = 'Data registration manager: Exchange with ""%1""'"), "%1", Строка(УзелОбменаСсылка));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(УзелОбменаСсылка) Тогда
		...
		Если МодульОбменДаннымиПовтИсп.ЭтоУзелОбменаДаннымиБСП(УзелОбменаСсылка)
			...
			Если РежимВыборочнойРегистрации = МодульОбменДаннымиРегистрацияСервер.РежимВыборочнойРегистрацииОтключен()Тогда
				...
				ТекстЗаголовка = НСтр("ru = 'отключен.';
										|en = 'disabled.'", КодОсновногоЯзыка());
			...
			ИначеЕсли РежимВыборочнойРегистрации = МодульОбменДаннымиРегистрацияСервер.РежимВыборочнойРегистрацииМодифицированность() Тогда
				...
				ТекстЗаголовка = НСтр("ru = 'модифицированность.';
										|en = 'modified.'", КодОсновногоЯзыка());
				
			Иначе
				
				ТекстЗаголовка = НСтр("ru = 'согласно правилам xml.';
										|en = 'According to XML rules.'", КодОсновногоЯзыка());
				
			КонецЕсли;
			
		Иначе
			
			ТекстЗаголовка = НСтр("ru = 'не поддерживается.';
									|en = 'not supported.'", КодОсновногоЯзыка());
			
		КонецЕсли;
		
	Иначе
		
		ТекстЗаголовка = НСтр("ru = 'укажите настройку синхронизации.';
								|en = 'Specify a synchronization setting.'", КодОсновногоЯзыка());
		
	КонецЕсли;
	
	ШаблонЗаголовка = НСтр("ru = 'Режим выборочной регистрации: %1';
							|en = 'Selective registration mode: %1'", КодОсновногоЯзыка());
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ru = 'Общий модуль ""%1"" не существует.';
															|en = 'Common module ""%1"" does not exist.'"), Имя);
	КонецЕсли;
#Иначе
	Модуль = Вычислить(Имя);
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
