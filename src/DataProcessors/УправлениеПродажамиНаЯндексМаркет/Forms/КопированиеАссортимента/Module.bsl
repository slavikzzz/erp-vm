	...
	Если ТекущиеДанные = Неопределено Тогда
		...
		Если МагазинИсточник.Пустая() Тогда
			ПодсказкаСостояниеТовара = НСтр("ru = 'Не выбран магазин-источник ассортимента.';
											|en = 'A store that is a product range source is not selected.'");
		Иначе
			ШаблонПодсказки = НСтр("ru = 'В магазине ""%1"" нет товаров.';
									|en = 'There are no items in the %1 store.'");
...
Процедура Добавить(Команда)
	...
	Если Не ЭтоУстановкаПометокДляВсехПозиций И ИдентификаторыПубликацииВыбранные.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не найдены отмеченные позиции для добавления ассортимента из магазина-источника.';
														|en = 'The items selected to add a product range from the source store are not found.'"));
		Возврат;
	КонецЕсли;

	ПоказатьОповещениеПользователя(НСтр("ru = 'Добавление ассортимента';
										|en = 'Add a product range'"),,
		НСтр("ru = 'Запущено фоновое добавление выбранного ассортимента из магазина-источника.';
			|en = 'Background adding of the selected product range from the source store is started.'"), БиблиотекаКартинок.ЛоготипЯндексМаркет);
	...
	Иначе
		...
		ПараметрыОжидания.ОповещениеПользователя.Пояснение = НСтр("ru = 'Завершено добавление выбранного ассортимента из магазина-источника.';
																	|en = 'The selected product range is added from the source store.'");
...
Процедура ЗаполнитьИнформациюВПодсказке()
	...
	ШаблонПодсказкиМагазинаИсточника = НСтр("ru = 'Копирование предложений (размещение товара из одного магазина в другой) возможно только в рамках аккаунта %1.';
											|en = 'You can copy offers (publish goods from one store to another) only within account %1.'");
	...
	|en = 'You can copy offers from one store to another only within one Yandex account.
	|To copy the product range, follow the listed steps:
	|		Step 1. Select a store whose items you want to add to the product range of the current store. Update the list of goods publications of the source store.
	|		Step 2. Select check boxes for items you want to add to the product range of the current store. Click ""Add"". 
	|		Step 3. The selected items will be added to the current store publications in background mode. The system will notify you when the items are added.
	|		Step 4. Items of the source store with the ""Moderation completed"" status will be added to the product range of the current store in the ""Recommendation is approved"" status and automatically sent for moderation, so that they can be added to the trading platform completely. Items of the destination store with statuses other than ""Moderation completed"" will be added to the current store in statuses corresponding to the source.'");
