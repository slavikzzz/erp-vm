...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ПараметрыОтбора.Свойство("ТипНоменклатуры") Тогда
		ТекстОтбора = НСтр("ru = 'Для выбора доступна номенклатура с типом: %1.';
							|en = 'Items with type: %1 are available for selection.'");
		Типы = Новый Массив;
	...
	Если ТипПлана = Перечисления.ТипыПланов.ПланПроизводства Тогда
		...
		ПараметрыВыбораСпецификаций.ДоступныеТипы.Добавить(Параметры.ТипПроизводственногоПроцесса);
		ПараметрыВыбораСпецификаций.ДоступныеСтатусы.Добавить(Перечисления.СтатусыСпецификаций.Действует);
		
		МассивЭлементов = Новый Массив;
		МассивЭлементов.Добавить(Элементы.ТоварыСпецификация);
	...
	Если ЭтоПланПродажПоКатегориям Тогда
		...
		Элементы.Шаг1Надпись1.Заголовок = НСтр("ru = 'Необходимо скопировать колонки в таблицу из внешнего файла (Excel. Word и пр.) через буфер обмена. Обязательно должна быть заполнена колонка: ""Товарная категория"".';
												|en = 'Copy columns to table from an external file (Excel. Word, etc.) via clipboard. The ""Product category"" column is required.'");
	...
	Иначе
		...
		Элементы.Шаг1Надпись1.Заголовок = НСтр("ru = 'Необходимо скопировать колонки в таблицу из внешнего файла (Excel. Word и пр.) через буфер обмена. Обязательно должна быть заполнена одна из колонок: ""Штрихкод"", ""Код"", ""Артикул"" или ""Номенклатура"".';
												|en = 'Copy columns to the table from an external file (Excel, Word, etc.) using the clipboard. One of the columns is required: ""Barcode"", ""Code"", ""item ID"" or ""Items"".'");
	...
	Если ЭтоАдресВременногоХранилища(Параметры.АдресТаблицыТоваров) Тогда
		...
		Для каждого СтруктураТовар Из МассивСтруктурТоваров Цикл
			...
			Если СтруктураТовар.Свойство("ИдентификаторУпаковка") 
				...
				СтруктураТовар.Вставить("Упаковка",       ?(Найти(Строка(Упаковка), НСтр("ru = '<Объект не найден>';
																						|en = '<Object is not found>'"))> 0, Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка(), Упаковка));
			КонецЕсли; 
