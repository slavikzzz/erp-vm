...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураДанных = ПолучитьИзВременногоХранилища(Параметры.НастройкиПрайсЛиста);//Структура
	...
	Если УпрощеннаяНастройка Тогда
		...
		Если УпрощеннаяНастройка.НастройкиБудутПотеряны Тогда
			...
			ТекстВопроса = НСтр("ru = 'Некоторые настройки могут быть потеряны. Перейти к упрощенным настройкам?';
								|en = 'Some settings may be lost. Go to simple settings?'");
	...
	Иначе
		...
		Если РасширенныеНастройкиБудутПотеряны() Тогда	
			ТекстСообщения = НСтр("ru = 'Редактирование отбора [Цена должна быть] возможно только в простом виде.';
									|en = 'Editing the [The price should be] filter is only available in a simple form.'");
...
Процедура ПерейтиКУстановкеЦен(Команда)
		...
		Если ЗначениеЗаполнено(ВидЦеныОтбор) Тогда
			Если Не ЗначениеЗаполнено(ЦенаОт) И Не ЗначениеЗаполнено(ЦенаДо) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru = 'Не заполнен диапазон цены';
						|en = 'Price range is not filled in'"),,"ЦенаОт",,Отказ);
			КонецЕсли;
			Если ЗначениеЗаполнено(ЦенаОт) И ЗначениеЗаполнено(ЦенаДо) И ЦенаДо < ЦенаОт Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru = 'Нижний предел диапазона цен не должен превышать верхний предел диапазона цен';
						|en = 'Lower limit of price range shall not exceed the upper limit'"),,"ЦенаОт",,Отказ);
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ЦенаОт) ИЛИ ЗначениеЗаполнено(ЦенаДо) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru = 'Не выбран вид цены';
						|en = 'Price type is not selected'"),,"ВидЦеныОтбор",,Отказ);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если УстановкаЦенКлиентСервер.НеВыбраныЗависимыеЦены(ЭтотОбъект) Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПерейтиКУстановкеЦенЗавершение", ЭтотОбъект), НСтр("ru = 'Найдены зависимые от выбранных виды цен. Выбрать все зависимые цены?';
																									|en = 'Price types that depend on the selected ones are found. Select all the dependent prices?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
...
Процедура ПерейтиКУстановкеЦенФрагмент()
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Для перехода к установке цен, необходимо выбрать один или несколько видов цен';
																|en = 'To go to the pricing, select one price type or more'"),,"ВыбранныеЦены");
	КонецЕсли;
...
Процедура РедактироватьНастройки(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Редактировать настройки невозможно. Не выбран вид цены';
													|en = 'Cannot edit settings. Price type is not selected'"));
		Возврат;
	КонецЕсли;
	...
	Если Не ТекущиеДанные.Выбран Тогда
			ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Редактировать настройки схемы компоновки можно только для выбранных видов цен';
														|en = 'You can edit composition schema settings only for the selected price types'"));
		Возврат;
	КонецЕсли;
	...
	ЗаголовокФормыНастройкиСхемыКомпоновкиДанных = НСтр("ru = 'Настройка параметров для вида цены ""%1""';
														|en = 'Setting of parameters for kind of price ""%1""'");
...
Процедура СообщитьОНевозможностиВыбора()
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Нет прав на установку цен по данному виду цены';
															|en = 'No rights for pricing by this price type'"),, "ВыбранныеЦены[" + ВыбранныеЦены.Индекс(ТекущиеДанные) + "].Ссылка");
КонецПроцедуры
...
Процедура ОбновитьНастройкиВидовЦен(Команда = Неопределено)
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Настройки обновлены';
										|en = 'Settings are updated'"),, НСтр("ru = 'Действие выполнено';
																			|en = 'Operation completed.'"), БиблиотекаКартинок.Успешно32);
...
Функция ИнформацияТекущейСтроки() 
	...
	Если ТекущиеДанные.ДатаПоследнейУстановкиЦены > Дата Тогда
		СтрокаИнформация = СтрШаблон(НСтр("ru = 'По виду цен %1 цена установлена - %2';
											|en = 'By the %1 price type, the price is set - %2'"), ТекущиеДанные.Ссылка, ДатаПоследнейУстановкиЦены);
	ИначеЕсли НЕ ЗначениеЗаполнено(ТекущиеДанные.ДатаПоследнейУстановкиЦены) Тогда 
		СтрокаИнформация = СтрШаблон(НСтр("ru = 'По виду цен %1 не было установок цен.';
											|en = 'There was no pricing by the %1 price type.'"), ТекущиеДанные.Ссылка);
	Иначе
		СтрокаИнформация = СтрШаблон(НСтр("ru = 'По виду цен %1 последняя цена установлена %2.';
											|en = 'By the %1 price type the last price is set %2.'"), ТекущиеДанные.Ссылка, ДатаПоследнейУстановкиЦены);
	КонецЕсли;
...
Процедура СкладНаличияПриИзменении(Элемент)
	...
	Если ВариантНаличия = 3 И ЗначениеЗаполнено(СкладНаличия) Тогда
		...
		Сообщить(НСтр("ru = 'Укажите другой вариант отбора по наличию товаров. Для варианта [Не важно] выбрать склад нельзя.';
						|en = 'Specify another filter option based on the availability of goods items. You cannot select a warehouse for the [Doesn''t matter] option.'"))
	КонецЕсли;
