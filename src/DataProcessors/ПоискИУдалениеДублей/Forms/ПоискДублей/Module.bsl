...
Процедура Подробно(Команда)
	...
	Элементы.Подробнее.Заголовок = ?(Элементы.ГруппаПодробно.Видимость, НСтр("ru = '<< Скрыть';
																			|en = '<< Hide'"), НСтр("ru = 'Подробнее >>';
																										|en = 'Details >>'"));
КонецПроцедуры
...
Процедура ШагВыборОсновногоЭлементаПриНажатииКнопкиДалее(ОписаниеСледующегоШага, ДополнительныеПараметры) Экспорт
	...
	Если ВозвращеноМеньшеЧемНайдено Тогда
		...
		ПоказатьВопрос(
			...
			|Start processing the duplicates?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
	...
	Если ЕстьПодсистемаУдаленияПомеченных Тогда
		...
		Если СпособУдаления = "Пометка" Тогда
			ТекстПодсказки = НСтр("ru = 'Выбранные элементы будут <a href = ""[Действие]"">помечены на удаление</a> и заменены на оригиналы (отмечены стрелкой).';
									|en = 'The selected items will be <a href = ""[Действие]"">marked for deletion</a> and replaced with the original items (the original items are marked with an arrow icon).'");
		...
		Иначе
			ТекстПодсказки = НСтр("ru = 'Выбранные элементы будут <a href = ""[Действие]"">безвозвратно удалены</a> и заменены на оригиналы (отмечены стрелкой).';
									|en = 'The selected items will be <a href = ""[Действие]"">deleted permanently</a> and replaced with the original items (the original items are marked with an arrow icon).'");
			ПараметрыСтроки = Новый Структура("Действие", "СпособУдаления");
			ТекстПодсказки = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(ТекстПодсказки, ПараметрыСтроки);
		КонецЕсли;
...
Функция НайтиИУдалитьДубли()
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ШагВыполнениеУдаления Тогда
		...
		Если ПараметрыУдаления.Количество() = 0 Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Выберите хотя бы одну группу дублей.';
														|en = 'Select at least one duplicate group.'"),, "НайденныеДубли");
			Возврат Неопределено;
		КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректная операция %1.';
				|en = 'Incorrect operation: %1.'"), Строка(ТекущаяСтраница));
	КонецЕсли;
		...
		ИначеЕсли Результат.Прогресс.Процент > 0 Тогда
			Сообщение = Сообщение + " " 
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '(найдено %1)';
						|en = '(%1 locations found)'"), Результат.Прогресс.Процент);
		КонецЕсли;
