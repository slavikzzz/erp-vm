	...
	Если Результат <> Неопределено Тогда
		...
		Если Результат.ДатаНачала > Результат.ДатаОкончания Тогда
			...
			ТекстОшибки = НСтр("ru = 'Период задан неверно.
									...
									|en = 'The period is invalid.
									|The period start date must be less than the end date.'");
...
Процедура ОбновитьДанныеФормы()
	...
	Пока ВыборкаПоРасчетам.Следующий() Цикл
		...
		НоваяСтрока.ДокументПредставление = СтрШаблон(НСтр("ru = '№ %1 (%2)';
															|en = 'No. %1 (%2)'"), НоваяСтрока.Номер, Формат(НоваяСтрока.Дата, "ДФ=dd.MM.yy"));
		НоваяСтрока.СуммаПредставление = Модуль.СуммаПредставление(НоваяСтрока);
...
Процедура СформироватьЗаголовкиКомандМеню()
	...
	Элементы.КомандаРаспоряжения.Заголовок = СтрШаблон(НСтр("ru = 'Заказы (%1)';
															|en = 'Orders (%1)'"), КоличествоРаспоряжений);
