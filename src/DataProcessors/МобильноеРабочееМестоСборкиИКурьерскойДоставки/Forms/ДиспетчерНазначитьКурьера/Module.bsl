...
Процедура ОбновитьДанныеФормыСервер()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НоваяСтрока.ПоследняяДоставкаПодпись = НСтр("ru = 'Последняя доставка';
													|en = 'Последняя доставка'") + " - ";
		Если ВыборкаДетальныеЗаписи.ПоследняяДоставка = Дата(1,1,1) Тогда
			НоваяСтрока.ПоследняяДоставка = НСтр("ru = 'нет';
												|en = 'no'");
		Иначе
			НоваяСтрока.ПоследняяДоставка = НСтр("ru = 'в';
												|en = 'in'") + " " + Формат(ВыборкаДетальныеЗаписи.ПоследняяДоставка,"ДФ=HH:mm");
		КонецЕсли;
	КонецЦикла;
