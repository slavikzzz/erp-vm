...
Процедура ОткрытьКарточкуТовара(Раздел)
	...
	Если МассивТоваров.Количество() > 0 Тогда
		...
		Если Раздел = "КСборке" Тогда
			...
			Элементы.ОтменитьСобрано.Видимость = Ложь;
			Элементы.ОтменитьНетВНаличии.Видимость = Ложь;
			Элементы.СобратьТовар.Заголовок         = НСтр("ru = 'Собрать';
															|en = 'To assemble'") + " " + Строка(СтрокаТовар.Количество) + " " + Строка(СтрокаТовар.Упаковка);
		ИначеЕсли Раздел = "Собрано" Тогда
			Элементы.СобратьТовар.Видимость = Ложь;
			Элементы.Заменить.Видимость = Ложь;
...
Процедура ПересчитатьИтогиГрупп()
	...
	Элементы.РазделКСборке.Заголовок  = НСтр("ru = 'Собрать';
											|en = 'To assemble'") + " " + Строка(ГруппыКСборке.Итог("КоличествоСтрок"));
	Элементы.РазделУточнить.Заголовок = НСтр("ru = 'Уточнить';
											|en = 'Specify'") + " " + Строка(ТовараНет.Количество() + Замены.Количество());
	Элементы.РазделСобрано.Заголовок  = НСтр("ru = 'Собрано';
											|en = 'Assembled'") + " " + Строка(ГруппыСобрано.Итог("КоличествоСтрок"));
	
	Для Каждого Строка Из ГруппыКСборке Цикл
		ИмяГруппы = ?(Строка.Группа = "", НСтр("ru = 'Без группы';
												|en = 'Without group'"), Строка.Группа);
	...
	Элементы.ЗаменыГруппа.Заголовок = НСтр("ru = 'Замены';
											|en = 'Replacements'") + " - " + Строка(Замены.Количество());
	Элементы.ТовараНет.Заголовок = НСтр("ru = 'Товара нет';
										|en = 'No goods item available'") + " - " + Строка(ТовараНет.Количество() + Замены.Количество());
	Элементы.ВернутьНаПолкуГруппа.Заголовок = НСтр("ru = 'Вернуть на полку';
													|en = 'Return to shelf'") + " - " + Строка(ВернутьНаПолку.Количество());
...
Процедура ПоискТовараПоШтрихкоду()
	...
	Если ТекущаяФорма = "ФормаРабочийСтол" Тогда
		...
		Иначе
			...
			ПоказатьВопрос(Оповещение, НСтр("ru = 'Товар не найден в заказе, добавить новый?';
											|en = 'Goods item is not found in order, do you want to add a new one?'"), Режим, 0);
