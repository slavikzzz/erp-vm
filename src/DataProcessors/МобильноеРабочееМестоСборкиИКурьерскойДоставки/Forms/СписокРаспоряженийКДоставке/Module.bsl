...
Процедура НазначитьСнятьСотрудника(ПараметрыРаспоряжения, ТипСотрудника, Сотрудник = Неопределено);
	...
	Иначе
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Для склада %1 курьеру запрещено назначать/снимать с себя заказ.';
									|en = 'The courier cannot assign/release themselves from the order for warehouse %1.'"), ПараметрыРаспоряжения.Склад);
...
Процедура СформироватьЗаголовкиКомандМеню()
	Элементы.КомандаНазначенныеМне.Заголовок = СтрШаблон(НСтр("ru = 'Мои (%1)';
																|en = 'My (%1)'"), КоличествоРаспоряженийНазначенныеМне);
	Элементы.КомандаНераспределенные.Заголовок = СтрШаблон(НСтр("ru = 'Свободные (%1)';
																|en = 'Free (%1)'"), КоличествоРаспоряженийНераспределенные);
КонецПроцедуры
