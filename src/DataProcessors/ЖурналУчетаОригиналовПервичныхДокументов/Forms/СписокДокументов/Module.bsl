...
Процедура УстановитьИнтервалЗавершение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если Не ВыбранноеЗначение = Неопределено Тогда
		ТекстЗаголовка = НСтр("ru = 'Журнал учета оригиналов первичных документов';
								|en = 'Source document tracking log'")+ " ";
		...
		ИначеЕсли Не ЗначениеЗаполнено(ВыбранноеЗначение.ДатаНачала) Тогда
			Заголовок = ТекстЗаголовка +СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = '(по [ДатаОкончания])';
																									|en = '(to [ДатаОкончания])'"), Значения);
		ИначеЕсли Не ЗначениеЗаполнено(ВыбранноеЗначение.ДатаОкончания)Тогда
			Заголовок = ТекстЗаголовка +СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = '(с [ДатаНачала])';
																									|en = '(from [ДатаНачала])'"), Значения);
		ИначеЕсли ВыбранноеЗначение.Вариант = ВариантСтандартногоПериода.Сегодня Или ВыбранноеЗначение.Вариант = ВариантСтандартногоПериода.Вчера 
			...
			Заголовок = ТекстЗаголовка +СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = '([ДатаНачала])';
																									|en = '([ДатаНачала])'"), Значения);
		Иначе
			Заголовок = ТекстЗаголовка +СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = '(с [ДатаНачала] по [ДатаОкончания])';
																									|en = '(from [ДатаНачала] to [ДатаОкончания])'"), Значения);
		КонецЕсли;
...
Процедура ЗаполнитьПервоначальныйСписокВыбораВидовДокумента() 
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья") Тогда
		СтрокиКУдалению = ТаблицаВидыДокументов.НайтиСтроки(Новый Структура("ВидДокумента",																	
			Справочники.ИдентификаторыОбъектовМетаданных.НайтиПоРеквизиту("Имя", "ПередачаДавальцу")));
		...
		СтрокиКУдалению = ТаблицаВидыДокументов.НайтиСтроки(Новый Структура("ВидДокумента",																	
			Справочники.ИдентификаторыОбъектовМетаданных.НайтиПоРеквизиту("Имя", "ОтчетДавальцу")));
		...
		СтрокиКУдалению = ТаблицаВидыДокументов.НайтиСтроки(Новый Структура("ВидДокумента",																	
			Справочники.ИдентификаторыОбъектовМетаданных.НайтиПоРеквизиту("Имя", "ПоступлениеСырьяОтДавальца")));
		...
		СтрокиКУдалению = ТаблицаВидыДокументов.НайтиСтроки(Новый Структура("ВидДокумента",																	
			Справочники.ИдентификаторыОбъектовМетаданных.НайтиПоРеквизиту("Имя", "ВозвратСырьяДавальцу")));
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья2_5") Тогда
		...
		СтрокиКУдалению = ТаблицаВидыДокументов.НайтиСтроки(
			Новый Структура(
					"ВидДокумента",
					Справочники.ИдентификаторыОбъектовМетаданных.НайтиПоРеквизиту("Имя", "ОтчетДавальцу2_5")));
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		Если ОтборВидыДокументов.Количество() = КоличествоДокументов Или ОтборВидыДокументов.Количество()= 0 Тогда
			Шаблон = НСтр("ru = 'Показаны все документы журнала <a href=""%Парам1%"">Настроить</a>';
							|en = 'Showing all log documents <a href=""%Парам1%"">Configure</a>'");
		...
		Иначе	
			...
			|en = ';%1 log document is displayed;;
			|%1 log documents are displayed;%1 log documents are displayed;'"),ОтборВидыДокументов.Количество());
			Шаблон = НСтр("ru = '<a href=""%Парам1%"">Настроить</a>';
							|en = '<a href=""%Парам1%"">Configure</a>'");
			Шаблон = СтрЗаменить(Шаблон, "%Парам1%", "НастроитьОтбор");
