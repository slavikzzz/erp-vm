...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(ВариантНастроек) Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Открытие формы невозможно по причине: не указан вариант настроек.';
				|en = 'Cannot open the form. The settings option is not specified.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ВариантНастроек = "ВыгрузкаЗаявкиНаВозврат" Тогда
		Если Не ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
			Отказ = Истина;
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Не выбран объект для выгрузки данных.';
					|en = 'Data export object not selected.'"),, "СсылкаНаОбъект");
			Возврат;
		КонецЕсли;
...
Процедура ВыбратьФайл(Команда)
	...
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("ru = 'Все поддерживаемые форматы файлов(*.xls;*.xlsx;*.ods;*.mxl)|*.xls;*.xlsx;*.ods;*.mxl|Книга Excel 97 (*.xls)|*.xls|Книга Excel 2007 (*.xlsx)|*.xlsx|Электронная таблица OpenDocument (*.ods)|*.ods|Табличный документ (*.mxl)|*.mxl';
											|en = 'All supported file formats((*.xls;*.xlsx;*.ods;*.mxl)|*.xls;*.xlsx;*.ods;*.mxl|Spreadsheet Excel 97 (*.xls)|*.xls|Spreadsheet Excel 2007 (*.xlsx)|*.xlsx|Electronic table OpenDocument (*.ods)|*.ods|Spreadsheet document (*.mxl)|*.mxl'");
	...
	Если Результат <> Неопределено Тогда
		...
		ПоказатьОповещениеПользователя(
			...
			НСтр("ru = 'Выбранный файл успешно загружен.';
				|en = 'The selected file is imported.'"));
	КонецЕсли;
