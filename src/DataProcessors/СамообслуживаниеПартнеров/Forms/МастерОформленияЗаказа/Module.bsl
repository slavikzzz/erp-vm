...
Процедура УправлениеДоступностьюСервер(Данные)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию
		...
		Элементы.ДекорацияТовары.Заголовок = НСтр("ru = 'Проверьте отобранные для оформления заказа товары';
													|en = 'Check the goods selected for the order'");
	КонецЕсли;
...
Процедура ЗаполнитьИтоговуюИнформацию()
	...
	ИтоговаяИнформация = СтрЗаменить(ИтоговаяИнформация,"#NotShipInInstallments#",
	                                ?(НеОтгружатьЧастями,Нстр("ru = 'Не отгружать частями';
																|en = 'Send in one shipment'"),Нстр("ru = 'Возможна отгрузка частями';
																									|en = 'Ship in installments is possible'")));
	...
	Если ИспользоватьУправлениеДоставкой Тогда
		РеквизитыДоставки = НСтр("ru = 'Способ доставки';
								|en = 'Delivery method'") + ": <EM>"
		...
		Если ЗначениеЗаполнено(ВремяДоставкиС)
			...
			РеквизитыДоставки = РеквизитыДоставки
				+ НСтр("ru = 'Желаемое время доставки';
						|en = 'Required delivery time'") +": <EM>"
				+ ?(ЗначениеЗаполнено(ВремяДоставкиС), НСтр("ru = 'c';
															|en = 'c'") + " " + Формат(ВремяДоставкиС,"ДФ=HH:mm; ДЛФ=T") + " ","")
				+ ?(ЗначениеЗаполнено(ВремяДоставкиПо),НСтр("ru = 'до';
															|en = 'to'") + " " + Формат(ВремяДоставкиПо,"ДФ=HH:mm; ДЛФ=T"),"")
				+ "</EM>" + "<BR>";
		КонецЕсли;
	Иначе
		РеквизитыДоставки = "";
	...
	Иначе
		Для каждого Строка Из ЭтапыГрафикаОплаты Цикл
			...
			СтрокаЭтапыГрафикаОплаты = СтрокаЭтапыГрафикаОплаты +  Формат(Строка.ДатаПлатежа,"ДЛФ=D") 
			                           ...
			                           + " "  +  Нстр("ru = '% от суммы заказа)';
														|en = '% of the order amount)'") + " " + "<br>";
