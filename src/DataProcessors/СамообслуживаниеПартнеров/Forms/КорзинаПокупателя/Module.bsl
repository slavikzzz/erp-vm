...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность И ЗавершениеРаботы Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Данные корзины были изменены. Все изменения будут потеряны.';
									|en = 'Cart data has changed. All changes will be lost.'");
		Возврат;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ЗаказКлиента" 
		...
		ПоказатьПредупреждение( ,Нстр("ru = 'Ваш заказ оформлен, ознакомиться с текущим состоянием ваших заказов можно в разделе Заказы.';
										|en = 'Your order is registered; you can find out the current state of your orders in the Orders section.'"));
		ОбработатьИзмененииДанныхКорзины();
...
Процедура КорзинаПокупателяКоличествоУпаковокПриИзменении(Элемент)
	...
	Если ТекущаяСтрока.Доступность = НСтр("ru = 'В наличии';
											|en = 'In stock'")
		Или ТекущаяСтрока.Доступность = НСтр("ru = 'Требует уточнения';
											|en = 'Requires clarification'") И ТекущаяСтрока.Доступно > 0 Тогда
			...
			Если ТекущаяСтрока.Период = НачалоДня(ТекущаяДатаСеанса) Тогда
				...
				Иначе
					ТекущаяСтрока.Доступность = НСтр("ru = 'Требует уточнения';
													|en = 'Requires clarification'");
				КонецЕсли;

			Иначе
				ТекущаяСтрока.Доступность = НСтр("ru = 'Ожидается';
												|en = 'Expected'") + " " + Формат(ТекущаяСтрока.Период,"ДЛФ=D");
			КонецЕсли;
		...
		Иначе
			...
			Иначе
				ТекущаяСтрока.Доступность = НСтр("ru = 'Ожидается';
												|en = 'Expected'") + " " + Формат(ТекущаяСтрока.ДатаПоставки,"ДЛФ=D");
			КонецЕсли;
...
Процедура ПодобратьТовары(Команда)
	...
	ПараметрыФормы.Вставить("Заголовок",                НСтр("ru = 'Подбор товаров в корзину';
															|en = 'Add goods to the shopping cart'"));
...
Процедура ОбработатьДанныеКорзины()
	...
	Для каждого Строка Из КорзинаПокупателя Цикл
		...
		Если Строка.Доступность = НСтр("ru = 'Согласно графика';
										|en = 'According to the schedule'") Тогда
			...
			Если Строка.Доступно > 0 Тогда
				...
				Иначе
					Строка.Доступность = НСтр("ru = 'Ожидается';
												|en = 'Expected'") + " " + Формат(Строка.Период,"ДЛФ=D");
				КонецЕсли;
			...
			Иначе
				...
				Иначе
					Строка.Доступность = НСтр("ru = 'Ожидается';
												|en = 'Expected'") + " " + Формат(Строка.ДатаПоставки,"ДЛФ=D");
				КонецЕсли;
