					...
					Пока ВыборкаКоличество.Следующий() Цикл
						...
						Пока ВыборкаСклад.Следующий() Цикл
							...
							Пока ВыборкаДетали.Следующий() Цикл
								...
								Если ВыборкаДетали.Доступность = НСтр("ru = 'Согласно графика';
									...
									Если ВыборкаДетали.Доступно > 0 Тогда
										...
										Иначе
											ЗначениеДоступность = НСтр("ru = 'Ожидается';
																		|en = 'Expected'") + " " + Формат(ВыборкаДетали.Период,"ДЛФ=D");
									...
									Иначе
										...
										Иначе
											ЗначениеДоступность = НСтр("ru = 'Ожидается';
																		|en = 'Expected'") + " " + Формат(ВыборкаДетали.ДатаПоставки,"ДЛФ=D");
								...
								Если ЗначениеДоступность = НСтр("ru = 'Нет на складе';
																|en = 'Out of stock'") Тогда
...
Процедура ЗаполнитьДоступностьГруппыСкладов(ТаблицаГруппСкладов, СоответствиеДанныеСклада);
	...
	Для Каждого СтрокаТаблицыГруппыСкладов Из ТаблицаГруппСкладов Цикл
		...
		Для Каждого ЭлементМассива Из СтрокаТаблицыГруппыСкладов.МассивПодчиненныхСкладов Цикл
			...
			Если ДанныеСклада <> Неопределено Тогда
				Если ДанныеСклада.Доступность = НСтр("ru = 'В наличии';
													|en = 'In stock'")
					Или ДанныеСклада.Доступность = НСтр("ru = 'Услуга доступна';
														|en = 'The service is available'") Тогда
				...
				Иначе
					...
					Если ДанныеСклада.Доступность = НСтр("ru = 'Ожидается';
														|en = 'Expected'") + " " Тогда
		...
		Если ТекстДоступность = НСтр("ru = 'Ожидается';
									|en = 'Expected'") + " " Тогда
		...
		Если СтрокаТаблицыГруппыСкладов.Область <> Неопределено Тогда
			...
			Если ТекстДоступность = НСтр("ru = 'Нет в наличии';
										|en = 'Not available'") Тогда
