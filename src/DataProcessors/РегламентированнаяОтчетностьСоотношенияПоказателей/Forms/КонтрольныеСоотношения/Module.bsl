...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.РезультатПроверки = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Обработка не предназначена для непосредственного использования.';
								|en = 'Обработка не предназначена для непосредственного использования.'");
	КонецЕсли;
	...
	Если РезультатПроверки.Свойство("СписокРасшифровкиПоказателей") Тогда 
		АдресСпискаРасшифровкиПоказателей = ПоместитьВоВременноеХранилище(ПолучитьИзВременногоХранилища(РезультатПроверки.СписокРасшифровкиПоказателей), УникальныйИдентификатор);
	КонецЕсли;
	Если РезультатПроверки.Свойство("СписокРасшифровкиформулы") Тогда 
		АдресСпискаРасшифровкиформулы = ПоместитьВоВременноеХранилище(ПолучитьИзВременногоХранилища(РезультатПроверки.СписокРасшифровкиформулы), УникальныйИдентификатор);
	КонецЕсли;
...
Процедура СохранитьПослеОтображенияДиалогаВыбораФайла(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Исключение
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			"Не удалось записать файл на диск. Возможно, диск защищен от записи или недостаточно места на диске.");
	КонецПопытки;
КонецПроцедуры
...
Процедура Обновить(Команда)
	...
	Если РезультатПроверки.Свойство("Статус") Тогда
		...
		ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Выполняется проверка контрольных соотношений';
												|en = 'Выполняется проверка контрольных соотношений'");
...
Процедура ОбновитьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			"Не удалось выполнить проверку. Информация для технической поддержки: " + Результат.ПодробноеПредставлениеОшибки);
...
Процедура Показать(Расшифровка) Экспорт
	...
	Если СписокРасшифровкиПоказателей = Неопределено Тогда 
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось получить список показателей. Веротяно, форма отчета была закрыта';
													|en = 'Не удалось получить список показателей. Веротяно, форма отчета была закрыта'"));
		Возврат;
	КонецЕсли;
	...
	Если Показатели.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Расшифровка для данного соотношения отсутствует.';
													|en = 'Расшифровка для данного соотношения отсутствует.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПолеТабличногоДокументаКСОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ОтключитьОбработчикОжидания("Подключаемый_ОбновитьРасшифровкуПриАктивизации");
	...
	Иначе
		Если Расшифровка.ДополнительныеВозможности <> Неопределено Тогда
			Если СтрНайти(Расшифровка.ДополнительныеВозможности, "ВывестиТекстОтчетНеОткрывать:") > 0 Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(Сред(Расшифровка.ДополнительныеВозможности, 30));
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Процедура АктивизироватьЯчейкуОтчета(Расшифровка)
	...
	Если ВладелецФормы = Неопределено ИЛИ ТекДок <> ВладелецФормы.КлючУникальности Тогда
		...
		Попытка
			...
			ФормаОтчета.ОбработкаКС = ЭтотОбъект;
			ФормаОтчета.Открыть();
...
Процедура Подключаемый_ОбновитьРасшифровкуПриАктивизации()
	Если НЕ ПолеТабличногоДокументаКС.ТекущаяОбласть.Расшифровка = Неопределено Тогда
		Показать(ПолеТабличногоДокументаКС.ТекущаяОбласть.Расшифровка);
	КонецЕсли;
КонецПроцедуры
