	...
	Для к = 0 По 4 Цикл
		...
		Если ЗначениеЗаполнено(ЭтотОбъект["ФоновоеЗаданиеПолучитьТарифы" + Строка(к)]) Тогда
			...
			ПараметрыОперации.Вставить("НаименованиеОперации", НСтр("ru = 'Получение списка доступных тарифов.';
																	|en = 'Get list of available service plans.'"));
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Если Отказ Тогда
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Организация %1 не зарегистрирована в Бизнес-Сети. Зарегистрировать?';
					|en = 'Company %1 is not registered in Business Network. Do you want to register it?'"), Организация);
			...
			НСтр("ru = 'Регистрация в Бизнес-Сети';
				|en = 'Registration in Business Network'"));
		Иначе
...
Процедура УстановитьДоступностьПолейПунктПриема(Форма)
	...
	ПодсказкаВвода = НСтр("ru = 'выбирать при заказе доставки';
							|en = 'select when ordering delivery'");
...
Процедура ЗаполнитьАдресПолучателя(Результат, Постфикс)
	...
	Результат.Вставить("ПолучательАдрес",
		НСтр("ru = '690025, Приморский край, городской округ Владивостокский, Владивосток г, 1-я Встречная ул, дом 1';
			|en = '690025, Primorsky Territory, Vladivostok urban district, Vladivostok, 1 1-ya Vstrechnaya st.'"));
	...
	АдресЗначение = НСтр("ru = '{
		...
		|}';
		|en = '{
		|""value"": ""690025, Primorsky Territory, Vladivostok urban district, Vladivostok, 1 1-ya Vstrechnaya st."",
		|""comment"": """",
		|""type"": ""Address"",
		|""country"": ""RUSSIA"",
		|""addressType"": ""Urban"",
		...
		|""districtType"": """",
		|""districtId"": """",
		|""munDistrict"": ""Vladivostok urban district"",
		|""munDistrictType"": ""urban district"",
		|""munDistrictId"": ""437a37c7-681e-4b62-b03d-055ee761024c"",
		|""cityId"": ""7b6de6a5-86d0-4735-b11a-499081111af8"",
		|""settlement"": """",
		|""settlementType"": """",
