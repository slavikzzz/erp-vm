...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Изменения в файле ""%1"" будут потеряны.';
				|en = 'The changes in file ""%1"" will be lost.'"), ИмяИРасширение);
		Возврат;
	КонецЕсли;
...
Процедура СохранитьКак(Команда)
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Все файлы (*.%1)|*.%1';
			|en = 'All files (*.%1)|*.%1'"), РасширениеФайла());
	...
	Если ВыборФайла.Выбрать() Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Файл успешно сохранен';
											|en = 'File saved'"), , ВыбранноеПолноеИмяФайла);
