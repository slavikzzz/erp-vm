...
Процедура СтандартнаяПеречитать(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Данные изменены. Перечитать данные?';
						|en = 'The data was changed. Do you want to refresh the data?'");
...
Процедура УдалитьЭП(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Удалить выделенные подписи?';
											|en = 'Do you want to delete the selected signatures?'"), РежимДиалогаВопрос.ДаНет);
	...
	Если ЗначениеЗаполнено(СсылкаНаТекущийОбъект) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 (Присоединенный файл)';
				|en = '%1 (Attachment)'"), Строка(СсылкаНаТекущийОбъект));
	Иначе
