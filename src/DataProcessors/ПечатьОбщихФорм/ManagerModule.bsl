...
Процедура ЗаполнитьРеквизитыШапкиТОРГ4(ДанныеПечати, ДанныеПоСкладу, Макет, ТабличныйДокумент)
	...
	РеквизитыСопроводительногоДокумента = НСтр("ru = 'сопроводительный документ № %НомерДокументаСопровождения% от %ДатаДокументаСопровождения%';
												|en = 'supporting document No. %НомерДокументаСопровождения% from %ДатаДокументаСопровождения%'", Метаданные.Языки.Русский.КодЯзыка);
...
Процедура ЗаполнитьРеквизитыШапкиСчетФактура(ДанныеПечати, ДанныеОснований, СведенияОПоставщике, ДанныеКонтрагентов, ТабличныйДокумент, ОпцииПечатиСчетаФактуры, ДанныеПоставщиков)
	...
	Если ДанныеПоставщиков = Неопределено Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеПечати, "ПодразделениеДляКПП") Тогда
			КПППоставщика = ОрганизацииЛокализация.КППНаДату(ДанныеПечати.Организация, ДатаСведений, ДанныеПечати.ПодразделениеДляКПП);
		Иначе
			КПППоставщика = ДанныеПечати.КПППоставщика;
		КонецЕсли;
		...
		ИННПоставщика = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			?(Не ПустаяСтрока(ДанныеПечати.КПППоставщика), "/" + КПППоставщика, ""));
	...
	Если ДействующиеПостановления.Постановление1096 Тогда
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеПечати, "ПредставлениеСтроки5а")
			...
			НаименованиеДокумента = ?(ДанныеПечати.ТолькоУслуги, НСтр("ru = 'Акт';
																		|en = 'Certificate of rendered services'"), НСтр("ru = 'Товарная накладная';
																							|en = 'Invoice'"));
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Иначе
			...
			СтрокаБезРасхождений = ДанныеПечати.КорректировочныйСчетФактура
									И СтрокаТовары.КоличествоПоРНПТ = 0
									И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТовары, "КоличествоПоРНПТДо")
									И СтрокаТовары.КоличествоПоРНПТДо = 0
...
Процедура ЗаполнитьРеквизитыПодвалаУПД2_5(ОбластьПодвала, ОбластьПодвалаНакладной, ДанныеПечати, СведенияОбОрганизации, ДанныеКонтрагентов)
	...
	Если ЗначениеЗаполнено(ДанныеПечати.КПППоставщика) Тогда
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеПечати, "ПодразделениеДляКПП") Тогда
			КПППоставщика = ОрганизацииЛокализация.КППНаДату(ДанныеПечати.Организация, ДанныеПечати.Дата, ДанныеПечати.ПодразделениеДляКПП);
		Иначе
			КПППоставщика = ДанныеПечати.КПППоставщика;
		КонецЕсли;
		ПредставлениеОрганизации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, ИНН/КПП %2/%3';
...
Процедура ЗаполнитьТабличныйДокументУКД(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати) Экспорт
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Иначе
			...
			ТекущаяХарактеристикаНабора = Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка();
			Пока СтрокаТовары.Следующий() Цикл
			СтрокаБезРасхождений = СтрокаТовары.КоличествоПоРНПТ = 0
									И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТовары, "КоличествоПоРНПТДо")
									И СтрокаТовары.КоличествоПоРНПТДо = 0
									И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТовары, "РазницаБезНДСУвеличение")
									И СтрокаТовары.РазницаБезНДСУвеличение = СтрокаТовары.РазницаБезНДСУменьшение
...
Процедура ЗаполнитьРеквизитыШапкиУКД(ДанныеПечати, ДанныеОснований, СведенияОПоставщике, СведенияОПокупателе, ДанныеПоставщиков, Макет, ТабличныйДокумент, ОпцииПечати)
	...
	Иначе
		...
		ПредставлениеПоставщика = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1';
				|en = '%1'"),  Сред(ПредставлениеПоставщика, 3));
		
		ПредставлениеАдресПоставщика = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1';
				|en = '%1'"), Сред(ПредставлениеАдресПоставщика, 3));
		
		ПредставлениеИННПоставщика = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1';
				|en = '%1'"), Сред(ПредставлениеИННПоставщика, 3));
		...
		Если НЕ ВыборкаПоДокументам.НайтиСледующий(СтруктураПоиска) Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В документе %1 отсутствуют товары. Печать акта на передачу прав не требуется';
					|en = 'There are no goods in document %1. Printing the assignment deed is not required'"),
				ДанныеПечати.Ссылка);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				Текст,
				ДанныеПечати.Ссылка);
	...
	Если РезультатПоОшибкам <> Неопределено Тогда
		...
		Пока ВыборкаДокументовСОшибками.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(ВыборкаДокументовСОшибками.Склад)
				...
				|en = 'Cannot print the ""Certificate of inventory handover for storage (MH-1)"" for ""%Документ%"".
				|The ""%Склад%"" does not keep records of VMI.'");
			...
			ИначеЕсли ВыборкаДокументовСОшибками.ОрганизацияПоклажедержатель
				...
				|en = 'Cannot print the ""Certificate of inventory handover for storage (MH-1)"" for ""%Документ%"". 
				|%Организация% company of the document matches the custodian company
				| specified in the ""%Склад%"" VMI conditions.'");
		...
		Если Не ВыборкаПоДокументам.НайтиСледующий(СтруктураПоиска) Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'В документе %1 отсутствуют товары. Печать акта не требуется';
							|en = 'Goods are missing in document %1. Certificate printing is not required'"),
						ДанныеПечати.Ссылка);
			...
			Если ВыборкаДанныеПоСкладу.ИсточникИнформацииОЦенахДляПечати = Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоСебестоимости Тогда
				...
				ИначеЕсли ВыборкаДанныеПоСкладу.ПредварительныйРасчет Тогда
					...
					ТекстСообщения = НСтр("ru = 'При печати цен документа %Документ% использовались данные предварительного расчета себестоимости.';
											|en = 'Data of the preliminary cost calculation was used while printing prices of document %Документ%.'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Документ%", ДанныеПечати.Ссылка);
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	...
	Если РезультатПоОшибкам <> Неопределено Тогда
		...
		Пока ВыборкаДокументовСОшибками.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(ВыборкаДокументовСОшибками.Склад)
				...
				|потому что на складе ""%Склад%"" не используется учет товаров переданных на ответственное хранение.';
				|en = 'Cannot print the ""Certificate of stored inventory return (MH-3)"" for ""%Документ%"". 
				|The ""%Склад%"" does not keep records of VMI.'");
...
Процедура ЗаполнитьРеквизитыШапкиМХ18(ДанныеПечати, ВыборкаПоСкладам, Макет, ТабличныйДокумент)
	...
	// Выводим общие реквизиты шапки
	СведенияОбОрганизации = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ВыборкаПоСкладам.Организация, ДанныеПечати.Дата);
...
Процедура ЗаполнитьТабличныйДокументМХ18(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если Не ВыборкаПоДокументам.НайтиСледующий(СтруктураПоиска) Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В документе %1 отсутствуют товары. Печать накладной не требуется';
				|en = 'Goods are missing in document %1. Printing of the invoice is not required'"), ДанныеПечати.Ссылка);
				...
				Если ВыводитьНДИГК Тогда
					...
					Пока ВыборкаПоНДИГКОтправителя.Следующий() Цикл
						...
						Пока ВыборкаПоНДИГКПолучателя.Следующий() Цикл
							...
							ШапкаТаблицы.Параметры.Заполнить(ДанныеПечати);
							ШапкаТаблицы.Параметры.Заполнить(ВыборкаПоНДИГКПолучателя);
							ШапкаТаблицы.Параметры.ПредставлениеПодразделения = ДанныеПечати.ПредставлениеПодразделения;
							...
							Если Не ТабличныйДокумент.ПроверитьВывод(МассивВыводимыхОбластей) Тогда
								НомерСтраницы = НомерСтраницы + 1;
								ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
								УстановитьПараметр(ШапкаТаблицы, "НомерСтраницыШапка", "Страница " + НомерСтраницы);
							...
							// Инициализация итогов в таблице
							ИтоговыеСуммы = СтруктураИтоговыеСуммы();
	
							НомерСтроки = 0;
	
							ДобавитьИтоговыеДанныеПодвала(ИтоговыеСуммы, КоличествоПорядковыхНомеровЗаписей,
								ЗначениеНастроекПовтИсп.ВалютаРегламентированногоУчетаОрганизации(ВыборкаПоОрганизациям.Организация));
							...
							КоличествоПорядковыхНомеровЗаписей = КоличествоПорядковыхНомеровЗаписей + НомерСтроки; 
							// Выводим итоги по таблице в целом
							ОбластьВсего.Параметры.Заполнить(ИтоговыеСуммы);
							ТабличныйДокумент.Вывести(ОбластьВсего);
