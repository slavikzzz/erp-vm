		...
		Иначе
			...
			Если ЗначениеЗаполнено(Параметры.Характеристика) Тогда
				...
				ШаблонТекста = НСтр("ru = 'Выбор поставщика (Номенклатура: %1, Характеристика: %2, Склад: %3)';
									|en = 'Select supplier (Product: %1, Variant: %2, Warehouse: %3)'");
			...
			Иначе
				...
				ШаблонТекста = НСтр("ru = 'Выбор поставщика (Номенклатура: %1, Склад: %2)';
									|en = 'Select vendor(Items: %1, Warehouse: %2)'");
				Заголовок = СтрШаблон(ШаблонТекста, Параметры.Номенклатура, Параметры.Склад);
				
			КонецЕсли;
			
			ВалютаУпрУчета = Константы.ВалютаУправленческогоУчета.Получить();
			Элементы.ПоставщикиЦенаУпрУчет.Заголовок = СтрШаблон(НСтр("ru = 'Цена (%1)';
																		|en = 'Price (%1)'"), ВалютаУпрУчета);
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Параметры.Характеристика) Тогда
				...
				ШаблонТекста = НСтр("ru = 'Выбор склада (Номенклатура: %1, Характеристика: %2)';
									|en = 'Select warehouse (Product: %1, Variant: %2)'");
				Заголовок = СтрШаблон(ШаблонТекста, Параметры.Номенклатура, Параметры.Характеристика);
				
			Иначе
				
				Заголовок = СтрШаблон(НСтр("ru = 'Выбор склада (Номенклатура: %1)';
											|en = 'Select warehouse (Items: %1)'"), Параметры.Номенклатура);
				
			КонецЕсли;
			
			ЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.Номенклатура, "ЕдиницаИзмерения");
			Элементы.СкладыДоступно.Заголовок = СтрШаблон(НСтр("ru = 'Доступно (%1)';
																|en = 'Available (%1)'"), ЕдиницаИзмерения);
			
		КонецЕсли;
		
	//++ НЕ УТКА
	ИначеЕсли Параметры.ТипОбеспечения = Перечисления.ТипыОбеспечения.Производство Тогда
		...
		Заголовок = НСтр("ru = 'Выбор подразделения';
...
Процедура ВыбратьНаКлиенте()
	...
	Иначе
		...
		Если Элементы.Подразделения.ТекущиеДанные = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		ПараметрыПоставкиОптимальные.Вставить("ИсточникОбеспечения", Элементы.Подразделения.ТекущиеДанные.Подразделение);
	//-- НЕ УТКА
	КонецЕсли;
...
Процедура УстановитьТекстЗапросаСпискаПодразделений()
	...
	СвойстваСписка = ОбщегоНазначения.СтруктураСвойствДинамическогоСписка();
	ЗаполнитьЗначенияСвойств(СвойстваСписка, Подразделения);
	СвойстваСписка.ТекстЗапроса =
		"ВЫБРАТЬ
	...
	ОбщегоНазначения.УстановитьСвойстваДинамическогоСписка(Элементы.Подразделения, СвойстваСписка);
