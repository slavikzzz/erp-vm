...
Процедура СформироватьЗаказы(Параметры, АдресРезультата) Экспорт
	...
	ПоляКлючаЗаказа = "ТипПроизводственногоПроцесса, НаправлениеДеятельности, Организация, Подразделение, Партнер, Договор, ДатаПотребности, ХозяйственнаяОперация";
...
Функция ВременнаяТаблицаДанныеЗаполнения()
	...
	ТекстЗапроса =
		...
		//-- Устарело_Переработка24
		|			КОГДА ЗаказДавальца2_5.Ссылка ЕСТЬ НЕ NULL
		...
		//++ Устарело_Переработка24
		|		КОГДА ЗаказыДавальца.УправлениеПроизводством2_2
		|			ТОГДА ЗаказыДавальца.ТипПроизводственногоПроцесса
		//-- Устарело_Переработка24
		|		КОГДА ЗаказДавальца2_5.Ссылка ЕСТЬ НЕ NULL
		|			ТОГДА ЗаказДавальца2_5.ТипПроизводственногоПроцесса
		|		КОГДА ИСТИНА
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыПроизводственныхПроцессов.Сборка)
		//-- НЕ УТКА
		|			ИНАЧЕ
		|				НЕОПРЕДЕЛЕНО
		...
		|	ВЫБОР
		|		КОГДА ВЫРАЗИТЬ(Т.Назначение КАК Справочник.Назначения).ТипНазначения = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое2_5)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья2_5)
		|		КОГДА ВЫРАЗИТЬ(Т.Назначение КАК Справочник.Назначения).ТипНазначения = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.ДавальческоеМатериалыПодЭтап22)
		|				И ВключеныДвальцы2_5.Значение
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья2_5)
		
		//++ Устарело_Переработка24
		
		|		КОГДА ВЫРАЗИТЬ(Т.Назначение КАК Справочник.Назначения).ТипНазначения В(
		...
		|		ПО Т.Заказ = ЗаказДавальца2_5.Ссылка
		|		
		//-- НЕ УТКА
		|
...
Функция ТаблицаЗаказыНаПроизводствоКОформлению()
	...
	ТекстЗапроса =
		...
		|			И Т.Заказ ССЫЛКА Документ.ЗаказДавальца
		//-- Устарело_Переработка24
		|		ИЛИ ЛОЖЬ)
		|СГРУППИРОВАТЬ ПО
...
Функция ТаблицаЗаказыНаПроизводство2_2КОформлению()
	...
	ТекстЗапроса =
		...
		//++ Устарело_Переработка24
		|		И НЕ Т.Заказ ССЫЛКА Документ.ЗаказДавальца
		//-- Устарело_Переработка24
		|		И ИСТИНА)
		|СГРУППИРОВАТЬ ПО
		|	Т.ТипПроизводственногоПроцесса,
		|	Т.Организация, Т.ИсточникОбеспечения, Т.НаправлениеДеятельности,
		|	Т.Давалец, Т.Договор, Т.ДатаПоставки, Т.ХозяйственнаяОперация,
		|	Т.Заказ,
		|	Т.Назначение, Т.Номенклатура, Т.Характеристика, Т.Склад, Т.ПодразделениеПолучатель
		|	
		|УПОРЯДОЧИТЬ ПО
		// порядок обхода заказов
		|	ТипПроизводственногоПроцесса, НаправлениеДеятельности, Организация, Подразделение, Партнер, Договор, ДатаПотребности, ХозяйственнаяОперация,
...
//++ НЕ УТКА
		...
		|		СпрСпособ.Ссылка           КАК СпособОбеспечения,
		|
		|		СУММА(Т.Требуется - Т.РаспределеноИзЗапасовОбособленно - Т.РаспределеноИзПартийОбособленно) КАК Количество
		|	ИЗ
		...
		|
		|	ИМЕЮЩИЕ
		|		СУММА(Т.Требуется - Т.РаспределеноИзЗапасовОбособленно - Т.РаспределеноИзПартийОбособленно) > 0
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ
		...
		|				ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.Пустаяссылка)
		|			КОНЕЦ                  КАК Подразделение,
		|		Т.НазначениеСтруктуры      КАК Назначение,
		|		СпрСпособ.Ссылка           КАК СпособОбеспечения,
		|	
		|		СУММА(Т.РаспределеноИзЗапасовОбособленно + Т.РаспределеноИзПартийОбособленно) КАК Количество
		|	ИЗ
		|		РегистрСведений.СтруктураЗаказа КАК Т
		|
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СпособыОбеспеченияПотребностей КАК СпрСпособ
		...
		|				ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.Пустаяссылка)
		|			КОНЕЦ,
		|		Т.НазначениеСтруктуры,
		|		СпрСпособ.Ссылка
		|
		|	ИМЕЮЩИЕ
		|		СУММА(Т.РаспределеноИзЗапасовОбособленно + Т.РаспределеноИзПартийОбособленно) > 0
//-- НЕ УТКА
		|) КАК Т
		|
		|СГРУППИРОВАТЬ ПО
...
Функция ВременнаяТаблицаПолуфабрикатыКЗапускуВПроизводство(ИмяВременнойТаблицыОтборПоПотребности)
	
	ВременныеТаблицы = Новый Структура();
	ВременныеТаблицы.Вставить("ОтборПоПотребности", ИмяВременнойТаблицыОтборПоПотребности);
	ВременныеТаблицы.Вставить("Результат",          "ОборотыПолуфабрикатов");
	
	ТекстЗапроса =
		РегистрыНакопления.ОбеспечениеПроизводственныхПроцессов.ТекстЗапросаПолуфабрикатыКЗапускуВПроизводство(ВременныеТаблицы);
		
	Возврат ТекстЗапроса;
		
КонецФункции

Функция ВременнаяТаблицаГрафикПроизводства(ИмяВременнойТаблицыОтборПоПотребности)
	...
	ТекстЗапроса =
		...
		|
		|	ВЫБОР
		|		КОГДА Движения.Номенклатура.ТипНоменклатуры В (
		...
		|	КОНЕЦ КАК ЭтоТовар,
		|
		|	СУММА(Движения.Запланировано) КАК Количество
		|
		|ПОМЕСТИТЬ ГрафикПроизводства
		|ИЗ
		...
		|		 И Движения.Склад          В (Товары.Склад, Товары.Подразделение)
		...
		|
		|ИМЕЮЩИЕ
		|	СУММА(Движения.Запланировано) > 0
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура, Характеристика, Склад, Назначение, ЭтоТовар
		|";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВременнаяТаблицаТовары", ИмяВременнойТаблицыОтборПоПотребности);
...
//++ НЕ УТКА
		...
		// Потребности по заказам на производство
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	
		|	Товары.Назначение                                               КАК Назначение,
		|	Товары.ЗаказНаПроизводство                                      КАК Заказ,
		|	Товары.ЖелаемаяДатаОтгрузки                                     КАК ДатаОтгрузки,
		|	Товары.Количество                                               КАК НеОбеспечено,
		|	0                                                               КАК Распределено,
		...
		|
		|		МИНИМУМ(ВЫБОР
		|				КОГДА Товары.ВидСтроки = ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Потребление)
		|					ТОГДА Товары.КлючПартия
		|			КОНЕЦ) КАК КлючПартия,
		|		МИНИМУМ(ISNULL(ДатыГрафика.Начало, Товары.ЗаказНаПроизводство.НачатьНеРанее)) КАК ЖелаемаяДатаОтгрузки,
		|
		|		СУММА(Товары.Требуется - Товары.РаспределеноИзЗапасовОбособленно - Товары.РаспределеноИзПартийОбособленно) КАК Количество
		|
		|	ИЗ
		|		РегистрСведений.СтруктураЗаказа КАК Товары
		|
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СпособыОбеспеченияПотребностей КАК СпрСпособ
		|			ПО &ПодстановкаОсновногоСпособаОбеспечения
		|
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НормативныйГрафикСтруктурыЗаказа КАК ДатыГрафика
		|			ПО Товары.ЗаказНаПроизводство = ДатыГрафика.ЗаказНаПроизводство
		|			 И Товары.КлючПартия          = ДатыГрафика.КлючПартия
		|			 И Товары.Этап                = ДатыГрафика.Этап
		|
		|	ГДЕ
		|		Товары.ВидСтроки НЕ В (
		|			ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Продукция),
		|			ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.ПроизводствоПродукции))
		|
		|	{ГДЕ
		...
		|		Товары.Назначение.*     КАК Назначение}
		|
		|	{ГДЕ Товары.ЗаказНаПроизводство В (
		|		ВЫБРАТЬ
		|			ТаблицаОтбора.Заказ КАК Заказ
		|		ИЗ
		|			ОтборПоЗаказам КАК ТаблицаОтбора
		...
		|		Товары.КлючНоменклатура,
		|		СпрСпособ.Ссылка
		|
		|	ИМЕЮЩИЕ
		|		СУММА(Товары.Требуется - Товары.РаспределеноИзЗапасовОбособленно - Товары.РаспределеноИзПартийОбособленно) > 0
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ
		...
		|		Товары.Характеристика                        КАК Характеристика,
		|		Товары.Склад                                 КАК Склад,
		|		Товары.НазначениеСтруктуры                   КАК Назначение,
		|		Товары.ЗаказНаПроизводство                   КАК ЗаказНаПроизводство,
		|		Товары.КлючНоменклатура                      КАК КлючНоменклатура,
		|
		|		СпрСпособ.Ссылка КАК СпособОбеспечения,
		...
		|		МИНИМУМ(ISNULL(ДатыГрафика.Начало, Товары.ЗаказНаПроизводство.НачатьНеРанее)) КАК ЖелаемаяДатаОтгрузки,
		|
		|		СУММА(Товары.РаспределеноИзЗапасовОбособленно + Товары.РаспределеноИзПартийОбособленно) КАК Количество
		|
		|	ИЗ
		|		РегистрСведений.СтруктураЗаказа КАК Товары
		|
		...
		|		Товары.Характеристика,
		|		Товары.Склад,
		|		Товары.НазначениеСтруктуры,
		|		Товары.ЗаказНаПроизводство,
		|		Товары.КлючНоменклатура,
		|		СпрСпособ.Ссылка
		|
		|	ИМЕЮЩИЕ
		|		СУММА(Товары.РаспределеноИзЗапасовОбособленно + Товары.РаспределеноИзПартийОбособленно) > 0
		|
		|) КАК Товары
		|
//-- НЕ УТКА
...
Функция ВременнаяТаблицаПотребностиВыбранныхСкладов()
	...
	Текст =
		...
		//++ НЕ УТКА
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтруктураЗаказа КАК ГрафикПеремещений
		|		ПО ВсеТовары.КлючПартия = ГрафикПеремещений.КлючПартия
		|		 И ГрафикПеремещений.ВидСтроки = ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Перемещение)
		|		 И НЕ ГрафикПеремещений.ДополнительнаяЗапись
		//-- НЕ УТКА
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТопологияСкладов КАК Топология
		...
		//++ НЕ УТКА
		|				КОГДА ГрафикПеремещений.Склад ЕСТЬ НЕ NULL ТОГДА ГрафикПеремещений.Склад
		//-- НЕ УТКА
		|			КОНЕЦ)
		|		
