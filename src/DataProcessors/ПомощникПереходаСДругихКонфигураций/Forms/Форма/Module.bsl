	...
	Если АвтоТестОбмена Тогда
		...
		Если ПустаяСтрока(Объект.ИмяФайлаДанных) Тогда
			ОписаниеОшибки = НСтр("ru = 'В переданных параметрах отсутствует путь к файлу с данными для загрузки.';
									|en = 'Path to the file with data for import is missing in the transferred parameters.'");
			Рекомендации = НСтр("ru = 'В передаваемых параметрах должен быть элемент ПутьКФайлуЗагрузки.';
								|en = 'Transferred parameters must have the ПутьКФайлуЗагрузки item.'");
		...
		Если ПустаяСтрока(Объект.КонфигурацияИсточник) Тогда
			Если Параметры.Свойство("КонфигурацияИсточник") И Не ПустаяСтрока(Параметры.КонфигурацияИсточник) Тогда
				ОписаниеОшибки = НСтр("ru = 'Загрузка данных из источника %1 не поддерживается.';
										|en = 'Data import from the %1 source is not available.'");
				...
				Рекомендации = НСтр("ru = 'Проверить правильность переданного параметра.';
									|en = 'Check the correctness of the transferred parameter.'");
			Иначе
				ОписаниеОшибки = НСтр("ru = 'В переданных параметрах отсутствует имя Конфигурации-источника.';
										|en = 'Source configuration name is missing in the transferred parameters. '");
				Рекомендации = НСтр("ru = 'В передаваемых параметрах должен быть элемент КонфигурацияИсточник.';
									|en = 'Transferred parameters must have the КонфигурацияИсточник item.'");
			КонецЕсли;
			ЗафиксироватьОшибку(ЭтаФорма, ОписаниеОшибки, Рекомендации);
			Ошибка = Истина;
		КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПустаяСтрока(Объект.КонфигурацияПриемник) Тогда
		...
		ПоказатьПредупреждение(Неопределено, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Данная обработка не предназначена для запуска в программе %1.';
				|en = 'This data processor is not intended for launching in the %1 application.'"), МетаданныеСиноним));
	...
	Если АвтоТестОбмена Тогда
		#Если ВебКлиент Тогда
			ТекстСообщения = Нстр("ru = 'Автотест переноса данных не может выполняться в режиме Веб-клиента.';
									|en = 'Data transfer autotest cannot be run in the web client mode.'");
...
Процедура ДанныеДляЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		ПараметрыДиалога.Вставить("Заголовок", НСтр("ru = 'Выберите путь к файлу с данными';
													|en = 'Select path to a file with data'"));
		ПараметрыДиалога.Вставить("Фильтр",    НСтр("ru = 'ZIP-файл';
													|en = 'ZIP file'") + " (*.zip)|*.zip");
	КонецЕсли;
...
Функция Подключаемый_СтраницаОжидание_ПриОткрытии(Отказ, ПропуститьСтраницу, Знач ЭтоПереходДалее)
	...
	Элементы.ДекорацияНомерШагаСтраницаОжидание.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Шаг %1.';
				|en = 'Step %1.'"), Строка(ПорядковыйНомерПерехода));
КонецФункции
...
Функция Подключаемый_СтраницаВыборДанных_ПриПереходеДалее(Отказ)
	...
	Если Не (ЗначениеЗаполнено(Объект.ИмяФайлаДанных) или ЗначениеЗаполнено(Объект.КаталогПереноса)) Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Необходимо указать данные для загрузки.';
													|en = 'Data for import is required.'"), 30);	
	КонецЕсли;
...
Функция Подключаемый_СтраницаПредварительно_ПриОткрытии(Отказ, ПропуститьСтраницу, Знач ЭтоПереходДалее)
	...
	РасшифровкаЗаголовка = НСтр("ru = 'Предварительные проверки';
								|en = 'Preliminary checks.'",ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура ПеренумероватьЗаголовкиШагов()
	...
	Для Каждого СтрокаПереходов Из ТаблицаПереходов Цикл
		...
		Элементы[ИмяЭлементаДекорации].Заголовок = НСтр("ru = 'Шаг';
														|en = 'Step'") + " " + Нумератор + ".";
	КонецЦикла;
...
Процедура ЗаписатьОшибкуРаботыПомощника(ТекстОшибки)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Помощник перехода с других конфигураций';
									|en = 'Wizard of migration from other configurations'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗаполнитьОписаниеДоступныхКонфигурацийИсточника()
	...
	Если ПустаяСтрока(Объект.КонфигурацияИсточник) Тогда
		...
		Заголовок = НСтр("ru = 'Помощник переноса данных из других конфигураций ""1С:Предприятие 8""';
						|en = 'Data transfer wizard from other 1C:Enterprise 8 configurations'");
		...
		Элементы.Декорация1_1.Заголовок = НСтр("ru = 'Помощник позволит вам загрузить данные из конфигураций';
												|en = 'Wizard allows you to import data from configurations'");
	...
	Иначе
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Помощник переноса данных из ""%1""';
				|en = 'Data transfer wizard from %1'"), СинонимИсточника);
			
		Элементы.Декорация1_1.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Помощник позволит вам загрузить данные из конфигурации ""%1"".';
				|en = 'Wizard allows you to import data from configuration %1.'"), СинонимИсточника);
...
Процедура ЗаголовокЦенообразования(Редакция)
	...
	Строка0 = НСтр("ru = 'Загрузка данных в конфигурацию не возможна, так как включено ценообразование версии %N.5.';
					|en = 'Cannot import data to the configuration as pricing version %N.5 is enabled.'",
					ОбщегоНазначения.КодОсновногоЯзыка());
	Строка1 = НСтр("ru = 'Для корректного перехода с других конфигураций необходимо использовать ценообразование версии %N.0.';
					|en = 'To ensure the correct migration from other configurations, use pricing version %N.0.'",
					ОбщегоНазначения.КодОсновногоЯзыка());
	Строка2 = НСтр("ru = 'Переключить ценообразование на версию %N.0';
					|en = 'Switch pricing to version %N.0'", ОбщегоНазначения.КодОсновногоЯзыка());
	Строка3 = НСтр("ru = 'После загрузки данных можно будет вернуться на ценообразование версии %N.5.
					|Помощник переключения версии ценообразования расположен в разделе';
					|en = 'After the data is imported, you can switch back to pricing version %N.5.
					|To switch the pricing version, go to'", ОбщегоНазначения.КодОсновногоЯзыка());
	Строка4 = НСтр("ru = ' НСИ и Администрирование';
					|en = ' Master data and settings'", ОбщегоНазначения.КодОсновногоЯзыка());
	Строка5 = НСтр("ru = ', меню';
					|en = '–'", ОбщегоНазначения.КодОсновногоЯзыка());
	Строка6 = НСтр("ru = ' CRM и маркетинг';
					|en = ' CRM and marketing'", ОбщегоНазначения.КодОсновногоЯзыка());
	Строка7 = НСтр("ru = ', группа';
					|en = '–'", ОбщегоНазначения.КодОсновногоЯзыка());
	Строка8 = НСтр("ru = ' Маркетинг.';
					|en = ' Marketing.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПодготовитьОтчетОбОшибках()
	...
	ОтчетОбОшибках.Показать(НСтр("ru = 'Список комментариев и ошибок';
								|en = 'List of comments and errors'"));
...
Процедура ПомещениеФайлаВХранилищеНачать()
	...
	Исключение
		ТекстСообщения = НСтр("ru = 'При помещении файлов во временное хранилище произошла ошибка';
								|en = 'An error occurred when placing files into temporary storage'")
...
Процедура ПомещениеФайлаВХранилищеЗавершить(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Не ПустаяСтрока(Адрес) Тогда
		...
		Иначе
			...
			ТекстПояснения = НСтр("ru = 'Проверьте корректность содержимого файла с данными';
									|en = 'Validate content of the file with data'") + ".";
	...
	Иначе
		...
		ТекстПояснения = НСтр("ru = 'Укажите корректный путь к файлу данных';
								|en = 'Specify a correct path to the data file'") + ".";
	...
	Если Не РезультатыВыполнения = Неопределено Тогда
		...
		Иначе
			...
			Если ТекущийЭтап = "Загрузка" Тогда
				...
				//++ НЕ УТ
				ИначеЕсли Объект.КонфигурацияИсточник = "ЗУП31" Тогда
					Если ЗначениеЗаполнено(Объект.СтатьяДоходов) Тогда
						ЗаполнитьДанныеВДокументахРезервыОтпусков();
					КонецЕсли;
				//-- НЕ УТ
	...
	Если Не СостояниеТекущегоЭтапа() = "Ошибка" Тогда
		...
		Если ТаблицаОшибок.Количество() > 0 Тогда
			...
			Элементы.ГиперссылкаПротокол.Заголовок = НСтр("ru = 'Протокол загрузки данных';
															|en = 'Data import protocol'")
				+ " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '(сообщений: %1)';
						|en = '(messages: %1)'"), ТаблицаОшибок.Количество());
		Иначе
	...
	Для Каждого ЭлементАрхива Из Архив.Элементы Цикл
			...
			Для Каждого КонфигурацияИсточника Из ДоступныеКонфигурацииИсточника Цикл
				...
				Если СтрНайти(ИмяФайла, ПодстрокаПоиска + "_ОД") = 1 
					...
					ПредставлениеЭтапа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Загрузка основных данных из конфигурации %1';
							|en = 'Import main data from the %1 configuration'"), КонфигурацияИсточника.Представление);
					Прервать;
				ИначеЕсли СтрНайти(ИмяФайла, ПодстрокаПоиска + "_ЗУП") = 1
					...
					ПредставлениеЭтапа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Загрузка кадровых и зарплатных данных из конфигурации %1';
							|en = 'Import HR and salary data from the %1 configuration'"), КонфигурацияИсточника.Представление);
					Прервать;
				КонецЕсли;
			КонецЦикла;
		Иначе
			...
			ИначеЕсли СтрНайти(ИмяФайла, ПодстрокаПоиска + "_Н") = 1
				...
				ПредставлениеЭтапа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Загрузка настроек учета из конфигурации %1';
						|en = 'Import accounting settings from the %1 configuration'"), КонфигурацияИсточника.Представление);
			ИначеЕсли СтрНайти(ИмяФайла, ПодстрокаПоиска + "_С") = 1
				...
				ПредставлениеЭтапа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Загрузка справочников из конфигурации %1';
						|en = 'Import catalogs from the %1 configuration'"), КонфигурацияИсточника.Представление);
			ИначеЕсли СтрНайти(ИмяФайла, ПодстрокаПоиска + "_О") = 1 
				...
				ПредставлениеЭтапа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Загрузка остатков из конфигурации %1';
						|en = 'Import balance from the %1 configuration'"), КонфигурацияИсточника.Представление);
			КонецЕсли;
		КонецЕсли;
		...
		Если Отказ Тогда
			ТекстСообщения = НСтр("ru = 'Ошибка формата файла. Выберите корректный файл с данными для загрузки.';
									|en = 'File format error. Select a correct file to import data.'");
			Прервать;
		Иначе
...
Функция РезультатЗагрузкиДанныхВДлительнойОперации()
	...
	Если Не ПустаяСтрока(Объект.КонфигурацияИсточник) Тогда
		...
		НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Перенос данных из конфигурации ""%1""';
				|en = 'Transfer data from the ""%1"" configuration'"), КонфигурацияИсточника.Представление);
	Иначе
		НаименованиеЗадания = НСтр("ru = 'Перенос данных из другой конфигурации ""1С:Предприятие 8""';
									|en = 'Transfer data from another 1C:Enterprise 8 configuration'");
	КонецЕсли;
...
Процедура ОбработатьПроверкуСуществованияФайлаТекущийЭтап(Существует, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ДополнительныеПараметры.ЭтоПереходДалее Тогда
		Если Элементы.ПанельОсновная.ТекущаяСтраница = Элементы.СтраницаВыборДанных Тогда
			ВыполнитьПереходПоТаблицеПереходов(Истина);
		Иначе
			ПоказатьПредупреждение(, НСтр("ru = 'Файл с данными не соответствует этапу переноса! Выгрузите данные из базы-источника.';
											|en = 'File with data does not correspond to the transfer step. Export data from the source base.'"), 30);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьПомещениеФайлаТекущийЭтап(Успешно, Адрес, ИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Сообщение.Текст = НСтр("ru = 'Ошибка помещения файла текущего этапа';
								|en = 'An error occurred when placing the current step file'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ОбработатьПолучениеРазмераФайла(Размер, ДополнительныеПараметры) Экспорт
	...
	Файл = ДополнительныеПараметры.Файл;
	Элементы.ДекорацияРазмерФайла.Заголовок = НСтр("ru = 'Размер файла';
													|en = 'File size'") + ": " + Окр(Размер/1024,0) + " Кб.";
...
&НаСервере
Процедура ЗаполнитьДанныеВДокументахРезервыОтпусков()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Ссылка,
	|	СтатьяДоходов,
	|	АналитикаДоходов
	|ИЗ Документ.РезервыПоОплатеТруда
	|ГДЕ НЕ ПометкаУдаления";
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ЗаполнятьСтатьюДоходов = Ложь; 
		ЗаполнятьАналитикуДоходов = Ложь; 

		Если ЗначениеЗаполнено(Объект.СтатьяДоходов) И Выборка.СтатьяДоходов <> Объект.СтатьяДоходов Тогда 
			ЗаполнятьСтатьюДоходов = Истина;
		КонецЕсли; 
		Если ЗначениеЗаполнено(Объект.АналитикаДоходов) И Выборка.АналитикаДоходов <> Объект.АналитикаДоходов Тогда 
			ЗаполнятьАналитикуДоходов = Истина;
		КонецЕсли; 
		Если НЕ (ЗаполнятьСтатьюДоходов Или ЗаполнятьАналитикуДоходов) Тогда
			Продолжить;
		КонецЕсли;
		ДокОбъект = Выборка.Ссылка.ПолучитьОбъект();
		ДокОбъект.ОбменДанными.Загрузка = Истина;
		Если ЗаполнятьСтатьюДоходов Тогда
			ДокОбъект.СтатьяДоходов = Объект.СтатьяДоходов;
		КонецЕсли;
		Если ЗаполнятьАналитикуДоходов Тогда
			ДокОбъект.АналитикаДоходов = Объект.АналитикаДоходов;
		КонецЕсли;
