...
Функция УдалитьЭлементПакета(ПараметрыУдаления, Элемент, МестаИспользования, НеудаленныеЭлементы)
	...
	Исключение
		...
		ЗаписатьПредупреждение(НСтр("ru = 'Не удалось удалить ""%1"" по причине:';
									|en = 'Couldn''t delete ""%1"" due to:'"), ПредставлениеЭлемента, ИнформацияОбОшибке);
...
Процедура УдалитьПомеченныеОбъектыМонопольно(УдаляемыеОбъекты, ПараметрыВыполнения)
	...
	Пока УдаляемыеОбъекты.Количество() > 0 Цикл
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ЗаписатьПредупреждение(НСтр("ru = 'Не удалось удалить элементы по причине:';
										|en = 'Couldn''t delete items due to:'"), Неопределено, ИнформацияОбОшибке);
...
Процедура ОтправитьСтатистику(РежимУдаления, ВремяВыполнения, КоличествоУдаляемыхЭлементов)
	...
	МодульЦентрМониторинга.ЗаписатьОперациюБизнесСтатистики(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Количество элементов: %1';
																	|en = 'Number of items: %1'"), КоличествоУдаляемыхЭлементов));
