...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.РазделениеВключено() И Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ТекстОшибки = НСтр("ru = 'Для удаления помеченных войдите в область данных.';
							|en = 'To delete marked objects, sign in to the data area.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
	...
	ИначеЕсли ВыбранноеЗначение = "Удалить" И НЕ ВыделеннаяСтрока.СсылочногоТипа Тогда
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Нельзя удалить выбранное значение: %1';
				|en = 'Cannot delete the selected value: %1'"),
...
Процедура УстановитьЗаменитьНа(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		Состояние(НСтр("ru = 'Для выбранной строки нельзя указать действие.';
						|en = 'Cannot specify an action for the selected row.'"),,, БиблиотекаКартинок.ДиалогВосклицание);
		Возврат;
	КонецЕсли;
...
Функция УстановитьСнятьПометкуУдаленияНаСервере(Ссылки)
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Удаление помеченных.Установка / снятие пометки удаления';
				|en = 'Delete marked objects.Toggle deletion mark'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Иначе
			Если ПустаяСтрока(ТекстСообщения) Тогда
				ТекстСообщения = НСтр("ru = 'Для места использования %1 нельзя выбрать действие %2.';
										|en = 'Cannot select the %2 action for the %1 usage instance.'");
...
Процедура ОткрытьФормуЗавершенияРаботыПользователей()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЗавершениеРаботыПользователей") Тогда
		...
		ПараметрыФормы.ТекстСообщенияОбОшибке = НСтр("ru = 'Невозможно выполнить удаление помеченных объектов, т.к. в приложении работают другие пользователи:';
													|en = 'Cannot delete the marked objects as other users are signed in:'");
		ПараметрыФормы.ТекстСообщенияОбОшибкеЗавершенияРаботы = НСтр("ru = 'Невозможно выполнить удаление помеченных объектов, т.к. не удалось завершить работу пользователей:';
																	|en = 'Cannot delete the marked objects because the following users are still signed in:'");
...
Процедура УстановитьСписокДействийМестИспользования(Форма)
	...
	Форма.ДействияМестИспользованияНеудаленных.Добавить("ЗаменитьСсылку", НСтр("ru = 'Заменить %ПредставлениеСсылка% на';
																				|en = 'Replace %ПредставлениеСсылка% with'"));
...
Функция ПредставлениеКомандыЗаменитьНа(ТекущиеДанные, Параметр = Неопределено)
	...
	Если Параметр <> Неопределено Тогда
		Результат = СтрЗаменить(НСтр("ru = 'Заменить %1 на %2';
									|en = 'Replace %1 with %2'"), "%1", ПредставлениеСтрокиВерхнегоУровня);
		Возврат СтрЗаменить(Результат, "%2", Параметр);
	Иначе
		Возврат СтрЗаменить(НСтр("ru = 'Заменить %1 на...';
								|en = 'Replace %1 with…'"), "%1", ПредставлениеСтрокиВерхнегоУровня);
	КонецЕсли;
...
Функция ПредставлениеКомандыУдалить(ТекущиеДанные)
	Представление = НСтр("ru = 'Удалить';
						|en = 'Delete'") + Символы.НПП + ТекущиеДанные.Представление;
...
Функция ПредставлениеОтбораМетаданных(ЗначениеОтбора)
	...
	Возврат ?(ПустаяСтрока(ПредставлениеОтбора), НСтр("ru = 'Все помеченные на удаление';
														|en = 'All objects marked for deletion'"), ПредставлениеОтбора);
КонецФункции
...
Функция УстановитьМонопольныйРежимНаСервере(МонопольныйРежим)
	...
	Если МонопольныйРежим() <> МонопольныйРежим Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление помеченных';
											|en = 'Delete marked objects'", ОбщегоНазначения.КодОсновногоЯзыка()),
