...
Процедура МаксимальныйРазмерФайлаОбластиДанныхПриИзменении(Элемент)
	...
	Если МаксимальныйРазмерФайлаОбластиДанных = 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Поле ""Максимальный размер файла"" не заполнено.';
								|en = 'Please specify File size limit.'");
...
Асинх Процедура ВыполнитьДедупликацию(Команда)
	...
	ШаблонВопроса = НСтр("ru = 'Дедупликация файлов позволяет экономить до 30%% места в информационной базе за счет устранения дублей файлов, хранящихся в приложении (вариант хранения ""В информационной базе""). Дедупликация имеющихся файлов занимает от нескольких минут до нескольких часов в зависимости от объема файлов в приложении. В любой момент ее можно будет прервать и возобновить позднее в более подходящий момент времени. При этом все вновь добавляемые файлы уже автоматически сохраняются в приложении только в одном экземпляре.
	 ...
	 |en = 'File deduplication allows you to save up to 30% of the space in the infobase by eliminating duplicates of files stored in the application (the ""In the infobase"" storage option). Deduplication of existing files takes from several minutes to several hours depending on the amount of files in the application. It can be interrupted at any time and resumed later at a more appropriate time. All newly added files are automatically saved in the application in only one instance.
	 |
	 |During file deduplication, the size of the infobase can increase significantly. Before startup:
	 |• Make sure that there is enough free space on the device where the infobase is located (at least %1 MB is required).
	 |• Back up the infobase.
	 |
	 | After completion, compress the infobase so that file deduplication takes effect.
	 |
