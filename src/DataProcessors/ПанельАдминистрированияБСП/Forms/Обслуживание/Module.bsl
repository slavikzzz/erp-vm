...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТКА
	ЭтоДопустимыйРежимРаботы = (НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая() И НЕ ОбщегоНазначения.РазделениеВключено())
		ИЛИ ОбщегоНазначения.РежимОтладки();
	Если ЭтоДопустимыйРежимРаботы
		И ОбщегоНазначения.ПодсистемаСуществует("ОбновлениеЧерезКопию") Тогда
		
		ОбновлениеПоддерживается = ПолучитьФункциональнуюОпцию("УправлениеПредприятием") И ПолучитьФункциональнуюОпцию("ЛокализацияРФ");
		Если ОбновлениеПоддерживается Тогда
			
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаОбновлениеЧерезКопию", "Видимость", Истина);
			
			ЗначенияИспользуется = Новый Массив(3);
			ЗначенияИспользуется[0] = Перечисления.СостоянияОбновлениеЧерезКопию.ВключеноОбновлениеЧерезКопию;
			ЗначенияИспользуется[1] = Перечисления.СостоянияОбновлениеЧерезКопию.ВыгрузкаОбновлениеЧерезКопию;
			ЗначенияИспользуется[2] = Перечисления.СостоянияОбновлениеЧерезКопию.ЗагрузкаОбновлениеЧерезКопию;
			
			ЗначениеКонстанты = Константы.ОбновлениеЧерезКопиюСостояние.Получить();
			ОбновлениеЧерезКопиюВключено = (ЗначенияИспользуется.Найти(ЗначениеКонстанты) <> Неопределено);
			
			ЭтотУзел = ОбменДаннымиПовтИсп.ПолучитьЭтотУзелПланаОбмена("ОбновлениеЧерезКопию");
			ДоступенПомощникЗагрузки = ОбновлениеЧерезКопиюВключено
				И ЗначениеЗаполнено(ЭтотУзел)
				И НЕ (ЭтотУзел.Код = Метаданные.Версия
							И ЗначениеКонстанты = Перечисления.СостоянияОбновлениеЧерезКопию.ВыгрузкаОбновлениеЧерезКопию);
...
Процедура ВключитьОтключитьОбновлениеЧерезКопию()
	...
	//++ НЕ УТКА
	Если ОбновлениеЧерезКопиюВключено Тогда
		
		НовоеСостояние = Перечисления.СостоянияОбновлениеЧерезКопию.ВключеноОбновлениеЧерезКопию;
		
	Иначе
		
		НовоеСостояние = Перечисления.СостоянияОбновлениеЧерезКопию.ОтключеноОбновлениеЧерезКопию;
	
	КонецЕсли;
	
	Константы.ОбновлениеЧерезКопиюСостояние.Установить(НовоеСостояние);
	//-- НЕ УТКА
