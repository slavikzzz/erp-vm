...
Функция ПараметрыПодсказкиШага(ИмяШага, ВидимостьПодсказок)
	...
	Если ИмяШага = "Шапка" Тогда
		Результат.Высота = 3;
		Результат.Текст = Новый ФорматированнаяСтрока(НСтр("ru = 'Проверьте и заполните основные реквизиты документов: Номер, Дата, Контрагент, Организация и Сумма. В первую очередь обращайте внимание на те реквизиты, которые отмечены красным. Если выделить несколько распознанных документов в таблице, то для них всех можно создать новых контрагентов, а также заполнить совпадающие реквизиты.';
															|en = 'Check and fill the main document attributes: Number, Date, Counterparty, Company, and Amount. First of all, pay attention to the attributes that are highlighted in red. If you select several recognized documents in the table, you can create new counterparties for all of them, and fill the matching attributes.'"), ШрифтПодсказок);
	ИначеЕсли ИмяШага = "Дубли" Тогда
		Результат.Высота = 3;
		Результат.Текст = Новый ФорматированнаяСтрока(НСтр("ru = 'Были обнаружены несколько дублей среди распознанных документов. Просмотрите их и выберите основной экземпляр с наилучшим качеством. Остальные дубли будут удалены. Уберите из дублей экземпляры, относящиеся к другим документам. Удалите всю группу дублей, если не требуется ее обрабатывать.';
															|en = 'Several duplicates were found among the recognized documents. View them and select the main instance with the best quality. The rest of the duplicates will be deleted. Remove instances from the duplicates that refer to other documents. Delete the entire group of duplicates if you do not need to process it.'"), ШрифтПодсказок);
	ИначеЕсли ИмяШага = "Комплекты" Тогда
		...
		Результат.Текст = Новый ФорматированнаяСтрока(НСтр("ru = 'Комплект документов – это группа документов, отражающих одну хозяйственную операцию. Объедините в комплект документы, которые относятся к одной операции, тем самым вы сможете сократить число правок номенклатуры и других реквизитов на следующих шагах группового ввода документов. Учетные документы могут быть созданы для каждого отдельного комплекта, либо для всех сразу на последнем шаге обработки.';
															|en = 'A document kit is a group of documents recording one business transaction. Merge the documents that belong to one transaction into a kit, so you can reduce the number of changes to the item and other attributes at the next steps of bulk document input. You can create accounting documents either for each individual kit, or for all kits at once at the last processing step.'"), ШрифтПодсказок);
	ИначеЕсли ИмяШага = "ДопРеквизиты" Тогда
		Результат.Высота = 3;
		Результат.Текст = Новый ФорматированнаяСтрока(НСтр("ru = 'Проверьте и заполните реквизиты: Договор, Склад, Банковский счет, Вид скидки и прочие.  В первую очередь обращайте внимание на реквизиты, выделенные красным. Используйте заполнение одинаковых реквизитов сразу для нескольких выделенных строк. Серые поля в таблице недоступны для редактирования.';
															|en = 'Check and fill the attributes: Contract, Warehouse, Bank account, Discount kind, and others. First of all, pay attention to the attributes highlighted in red. Fill the same attributes for several selected lines at once. Gray fields in the table are not editable.'"), ШрифтПодсказок);
	ИначеЕсли ИмяШага = "Номенклатура" Тогда
		Результат.Высота = 3;
		Результат.Текст = Новый ФорматированнаяСтрока(НСтр("ru = 'Проверьте и заполните номенклатуру и связанные с ней реквизиты. Заполняйте одинаковые значения реквизитов сразу для нескольких выделенных строк. По кнопке «Создать выделенные» можно быстро добавить новую номенклатуру для строк, где нет сопоставления с номенклатурой из базы.';
															|en = 'Check and complete the item and related attributes. Fill the same attribute values for several selected lines at once. Click the ""Create selected items"" button to quickly add a new item for the lines that are not mapped with the item from the database.'"), ШрифтПодсказок);
	ИначеЕсли ИмяШага = "Суммы" Тогда
		Результат.Текст = Новый ФорматированнаяСтрока(НСтр("ru = 'Проверьте и заполните суммы из таблиц документов. Красным выделены значения, которые не стыкуются для одной строки. Также здесь могут быть заполнены Страна происхождения и ГТД.';
															|en = 'Check and fill amounts from document tables. Values that do not match for one line are highlighted in red. You can also fill the Country of origin and CCD here.'"), ШрифтПодсказок);
	ИначеЕсли ИмяШага = "Итого" Тогда
		Результат.Текст = Новый ФорматированнаяСтрока(НСтр("ru = 'Проверьте изменения, которые будут внесены в базу данных. При необходимости вы можете вернуться на предыдущие шаги обработки.';
															|en = 'Check the changes that will be made to the database. If required, you can return to the previous processing steps.'"), ШрифтПодсказок);
	КонецЕсли;
			...
			Если КоличествоНеЗаполнено > 0 Тогда
				...
				Если КоличествоНеЗаполнено = 1 Тогда
					ТекстДокумент = НСтр("ru = ' распознанном документе ';
										|en = ' recognized document'");
				Иначе
					ТекстДокумент = НСтр("ru = ' распознанных документах ';
										|en = ' recognized documents'");
				КонецЕсли;
				...
				ТекстВопроса = Новый ФорматированнаяСтрока(НСтр("ru = 'В ';
					...
					ТекстДокумент + НСтр("ru = 'есть незаполненные поля, поэтому они не будут обработаны. Продолжить?';
										|en = 'there are blank fields, so they will not be processed. Continue?'")
				);
			...
			Если КоличествоНеЗаполнено > 0 Тогда
				...
				ТекстВопроса = Новый ФорматированнаяСтрока(НСтр("ru = 'Для ';
					...
					ТекстДубль + НСтр("ru = 'не выбран основной документ. Их можно выбрать автоматически. Продолжить?';
										|en = 'no main document is selected. It can be selected automatically. Continue?'")
				);
...
Функция СоздатьКонтрагентаНаСервере(ДанныеКонтрагента, Использовать1СКонтрагент)
	...
	Иначе
		...
		Исключение
			...
			Сообщение.Текст = НСтр("ru = 'Описание ошибки:';
									|en = 'Error details:'") + Символы.ПС
...
Функция ИзменитьНаправлениеНаСервере(ДокументСсылка)
	...
	Для Каждого КлючЗначение Из ИменаРеквизитов Цикл
		...
		Результат.Вставить(КлючЗначение.Ключ + "Распознано", НСтр("ru = 'Не сопоставлен: ';
																	|en = 'Not mapped:'") + ДанныеРеквизита.РаспознанныйТекст);
	КонецЦикла;
...
Функция ЗаполнениеСписковВыбораШапкиНаСервере(ИменаРеквизитов, ИдентификаторФормы)
	...
	Если Не Отказ Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Групповой ввод: обновление списков выбора шапки';
																|en = 'Bulk input: header selection list update'");
...
Функция ЗапуститьФоновоеЗаданиеЗаписиРеквизитовШапкиНаСервере(ПараметрыОперации, ИдентификаторФормы)
	...
	Если Не Отказ Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Групповой ввод реквизитов шапки';
																|en = 'Bulk input of header attributes'");
...
Процедура ОбновитьДанныеДокументаВКомплекте(РаспознанныйДокумент)
	...
	ИначеЕсли СтрокиТаблицы.Количество() > 1 Тогда
		...
		ВызватьИсключение НСтр("ru = 'В основной таблице появились дубли документов.';
								|en = 'Duplicate documents are found in the main table.'");
	КонецЕсли;
...
Функция НазваниеКомплекта(СтрокаДанных)
	...
	Если СтрокаДанных.Направление = Перечисления.НаправленияРаспознанногоДокумента.Входящий Тогда
		...
		Если ЗначениеЗаполнено(СтрокаДанных.Контрагент) Тогда
			ВОтКонтрагента = СтрШаблон(НСтр("ru = ' от ""%1""';
											|en = ' from ""%1""'"), СокрЛП(СтрокаДанных.Контрагент));
		Иначе
			ВОтКонтрагента = "";
		КонецЕсли;
	ИначеЕсли СтрокаДанных.Направление = Перечисления.НаправленияРаспознанногоДокумента.Исходящий Тогда
		СтрокаТипДокумента = НСтр("ru = 'Реализация';
									|en = 'Sales'");
		Если ЗначениеЗаполнено(СтрокаДанных.Контрагент) Тогда
			ВОтКонтрагента = СтрШаблон(НСтр("ru = ' в ""%1""';
											|en = ' to ""%1""'"), СокрЛП(СтрокаДанных.Контрагент));
		Иначе
			ВОтКонтрагента = "";
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаДанных.ДатаДокумента) Тогда
		ДатаВДекорации = НСтр("ru = ' от ';
								|en = ' from'") + Формат(СтрокаДанных.ДатаДокумента, "ДФ=dd.MM.yyyy");
	Иначе
		ДатаВДекорации = "";
	КонецЕсли;
	Результат = СтрШаблон(НСтр("ru = '%1%2%3';
								|en = '%1%2%3'"), СтрокаТипДокумента, ДатаВДекорации, ВОтКонтрагента);
	...
	Если Результат.ДанныеОбработки <> Неопределено Тогда
		...
		Если Результат.ДанныеОбработки.КомплектовНеУдалосьОбработать <> 0 Тогда
			ТекстПредупреждения = НСтр("ru = 'Не удалось обработать комплект. Повторите операцию после исправления ошибок:';
										|en = 'Cannot process the kit. Correct the following errors and try again:'");
...
Функция ПроверитьПравилаКомплекта(ДанныеДокументов, НомерКомплекта)
	...
	РезультатПроверки = Новый Структура("ПроверкиПройдены, ТипКомплекта, ТекстСообщения",
		Ложь, "", НСтр("ru = 'Выбранные типы документов не могут входить в один комплект. Уберите лишние документы из комплекта.';
						|en = 'The selected document types cannot be included in one kit. Remove unnecessary documents from the kit.'"));
	СтрокиКомплекта = ДанныеДокументов.НайтиСтроки(Новый Структура("НомерКомплекта", НомерКомплекта));
	
	Если СтрокиКомплекта.Количество() = 0 Тогда
		РезультатПроверки.ПроверкиПройдены = Ложь;
		РезультатПроверки.ТекстСообщения = НСтр("ru = 'Нельзя создать комплект без документов';
												|en = 'Cannot create a kit without documents'");
	...
	Для Каждого ЭтотДокументКомплекта Из СтрокиКомплекта Цикл
		...
		Если ПервыйДокументКомплекта.Направление <> ЭтотДокументКомплекта.Направление Тогда
			...
			РезультатПроверки.ТекстСообщения = НСтр("ru = 'Комплект не может одновременно содержать документы поступления и списания. Убедитесь, что либо все документы комплекта относятся к списанию, либо все – к поступлению.';
													|en = 'A kit cannot contain both receipt and write-off documents. Make sure all documents in the kit are write-off documents or receipt documents.'");
			Возврат;
		КонецЕсли;
		...
		Если ПервыйДокументКомплекта.Организация <> ЭтотДокументКомплекта.Организация Тогда
			...
			РезультатПроверки.ТекстСообщения = НСтр("ru = 'Данные документы принадлежат разным организациям и не могут быть обработаны в комплекте. Проверьте корректность заполнения данных документов или обработайте их по отдельности.';
													|en = 'The documents belong to different companies and cannot be processed together. Check if the documents are filled correctly or process them separately.'");
			Возврат;
		КонецЕсли;
		...
		Если ПервыйДокументКомплекта.Контрагент <> ЭтотДокументКомплекта.Контрагент Тогда
			...
			РезультатПроверки.ТекстСообщения = НСтр("ru = 'Данные документы принадлежат разным контрагентам и не могут быть обработаны в комплекте. Проверьте корректность заполнения данных документов или обработайте их по отдельности.';
													|en = 'The documents belong to different counterparties and cannot be processed together. Check if the documents are filled correctly or process them separately.'");
			Возврат;
		КонецЕсли;
...
Процедура Подключаемый_ПриАктивизацииЯчейкиСумм() Экспорт
	...
	Если ТекущиеДанные.ОшибкаЦены И (ИмяРеквизита = "Количество"
		...
		Если ТекущиеДанные.ВидСкидки = РаспознаваниеДокументовСлужебныйКлиентСервер.ВидыСкидок().НаОтдельныеПозиции Тогда
			НадписьОшибкаТЧ = НСтр("ru = 'Количество * Цена - Скидка не совпадает с Сумма';
									|en = 'Quantity * Price - Discount does not match Amount'");
		Иначе
			НадписьОшибкаТЧ = НСтр("ru = 'Количество * Цена не совпадает с Сумма';
									|en = 'Quantity * Price does not match Amount'");
		КонецЕсли;
	ИначеЕсли ТекущиеДанные.ОшибкаВсего И (ИмяРеквизита = "Сумма"
		...
		НадписьОшибкаТЧ = НСтр("ru = 'Сумма + Сумма НДС не совпадает с Всего';
								|en = 'Amount + VAT amount does not match Total'")
...
Функция ЗапуститьФоновоеЗаданиеЗаписиРеквизитовТаблицыНаСервере(ПараметрыОперации, ИдентификаторФормы)
	...
	Если Не Отказ Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Групповой ввод реквизитов таблицы';
																|en = 'Bulk input of table attributes'");
...
Процедура ЗавершениеВопросаСозданияДокументов(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Если ДанныеОбработки.КомплектовНеУдалосьОбработать + ДанныеОбработки.ДокументовНеУдалосьОбработать <> 0 Тогда
			...
			|en = 'Some documents contain errors and cannot be processed.
			| Cannot process: %1
			|
			|Correct the following errors and try again:
			| - The kit documents must be filled correctly.
			| - The documents must belong to the same kit.'"),
