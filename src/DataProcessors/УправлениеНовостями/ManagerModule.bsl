				...
				ЛогШаговВыполнения.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Лента новостей [%1] исключена из загрузки из-за ошибок внедрения. Имя файла на сервере: %2';
							|en = 'News feed [%1] is excluded from import due to implementation errors. File name on server: %2'"),
		...
		Если ЛогШаговВыполнения.Количество() = 0 Тогда
			ЗавершитьШагВыполнения(КонтекстВыполнения, "",
				НСтр("ru = 'Подсистема Новости внедрена неправильно, но нет лент новостей с параметром [config]';
					|en = 'The News subsystem is implemented incorrectly, but there are no news feeds with the [config] parameter'"));
		Иначе
			ЛогШаговВыполнения.Вставить(0,
				НСтр("ru = 'Подсистема Новости внедрена неправильно, и найдены ленты новостей с параметром [config], которые будут исключены:';
					|en = 'The News subsystem is implemented incorrectly. News feeds with the [config] parameter are found and will be excluded:'"));
			ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.SUBSYSTEM_PLUGGED_INCORRECT/GET_NEWS",
				ЛогШаговВыполнения);
		КонецЕсли;
	КонецЕсли;
...
#Область ЗагрузкаФайлов_Файл_ЛентаНовостей
							...
							Исключение
								...
								ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Ошибка получения новостей из метаданных %1 для ленты новостей %2 по причине
										|%3';
										|en = 'An error occurred while receiving news from the %1 metadata for the %2 news feed due to
										|%3'"),
									ТекущаяЛентаНовостей.ИмяФайла,
									ТекущаяЛентаНовостей.ЛентаНовостей,
										...
										Иначе
											...
											ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru = 'Ошибка получения новостей из макета %1 для ленты новостей %2 - эти метаданные не обрабатываются';
													|en = 'An error occurred while receiving news from the %1 template for the %2 news feed: this metadata is not processed'"),
												ТекущаяЛентаНовостей.ИмяФайла,
												ТекущаяЛентаНовостей.ЛентаНовостей);
												...
												Исключение
													...
													ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
														НСтр("ru = 'ЗагрузитьФайлыНовостейССервера (получено из макета): Не удалось удалить временный файл %1 для ленты новостей %2 по причине:
															...
															|en = 'ЗагрузитьФайлыНовостейССервера (received from the template): Cannot remove the %1 temporary file for the %2 news feed due to:
															|%3
															|
															|It is recommended that you remove this temporary file manually.'"),
											...
											Иначе
												ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
													НСтр("ru = 'Ошибка получения новостей из макета %1 для ленты новостей %2: возможна загрузка только из текстового макета';
														|en = 'An error occurred while receiving news from the %1 template for the %2 news feed: import can be performed only from the text template'"),
													ТекущаяЛентаНовостей.ИмяФайла,
													ТекущаяЛентаНовостей.ЛентаНовостей);
								...
								Исключение
									ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = 'Ошибка получения новостей из макета %1 для ленты новостей %2 по причине:
											|%3';
											|en = 'An error occurred while receiving news from the %1 template for the %2 news feed due to:
											|%3'"),
										ТекущаяЛентаНовостей.ИмяФайла,
										ТекущаяЛентаНовостей.ЛентаНовостей,
										ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				...
				Исключение
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка получения файла %1 для ленты новостей %2 по причине:
							|%3';
							|en = 'An error occurred while receiving the %1 file for the %2 news feed due to:
							|%3'"),
						ТекущаяЛентаНовостей.ИмяФайла,
						ТекущаяЛентаНовостей.ЛентаНовостей,
						ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Загрузка: загрузка файла новостей из ленты новостей %1 (протокол %2, сайт %3, имя файла %4).
						|Успешно? %5';
						|en = 'Import: import news file from news feed %1 (protocol %2, website %3, file name %4).
						|Successfully? %5'"),
					ТекущаяЛентаНовостей.ЛентаНовостей,
...
#Область Получение_HTTP
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Загрузка: установка http/https соединения (протокол %1).
					|Успешно? %2';
					|en = 'Import: setting http/https connection (protocol %1).
					|Successfully? %2'"),
				ТекущееHTTPСоединение.Протокол,
				(ЕстьОшибки = Ложь));
								...
								Исключение
									...
									ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = 'ЗагрузитьФайлыНовостейССервера (получено из интернет): Не удалось получить текст ошибки из временного файла %1 для ленты новостей %2 по причине:
											|%3';
											|en = 'ЗагрузитьФайлыНовостейССервера (received from the Internet): Cannot receive the error text from temporary file %1 for news feed %2 due to:
											|%3'"),
									...
									ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
										НСтр("ru = 'БИП:Новости.Загрузка новостей';
											|en = 'ISL:News.News import'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
										НСтр("ru = 'Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка получения текста ошибки сервера';
											|en = 'News. Import news. Import news files. An error occurred when receiving the server error text'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
							...
							Если НЕ ПустаяСтрока(ТекстХТМЛРегламентныхРабот) Тогда
								...
								Исключение
									...
									ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = 'ЗагрузитьФайлыНовостейССервера (получено из интернет): Не удалось распарсить текст ошибки для ленты новостей %1 по причине:
											...
											|Текст ошибки:
											|%3';
											|en = 'ЗагрузитьФайлыНовостейССервера (received from the Internet): Cannot parse error text for news feed %1 due to:
											|%2
											|
											|Error text:
											|%3'"),
									...
									ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
										НСтр("ru = 'БИП:Новости.Загрузка новостей';
											|en = 'ISL:News.News import'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
										НСтр("ru = 'Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка расшифровки текста ошибки сервера';
											|en = 'News. Import news. Import news files. An error occurred when decrypting server error text'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
										УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
										, // ОбъектМетаданных
										, // Данные
										ТекстСообщения, // Комментарий
								...
								ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Загрузка: ошибка загрузки файла новостей из ленты новостей %1.
										...
										|Текст возврата:
										|%6';
										|en = 'Import: an error occurred when importing a news file from news feed%1.
										|Return code: %2
										|Reason: %3
										|Period-end operation start date:%4
										|Period-end operation end date:%5
										|Return text:
										|%6'"),
									В.ЛентаНовостей,
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Шаг);
		...
		Если ОбрабатыватьОбластиДанных = Истина Тогда
			ЛогШаговВыполнения.Добавить(НСтр("ru = 'Выполнение пересчета отборов по новостям для области данных.';
											|en = 'Recalculate filters by news for data area.'"));
		...
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru = 'Пропуск пересчета отборов по новостям для области данных.';
											|en = 'Skip recalculation of filters by news for data area.'"));
		КонецЕсли;
			...
			Иначе
				...
				ИначеЕсли ТекущаяСтрока.Список = "Список категорий новостей" Тогда // Идентификатор.
					ТекстСообщения = НСтр("ru = 'Загрузка списка категорий новостей';
											|en = 'Import the news category list'");
					ТекущаяСтрока.ПредставлениеДляЗагрузки = ТекстСообщения;
				ИначеЕсли СтрНайти(ВРег(ТекущаяСтрока.Список), ВРег("Значения категории новостей:")) = 1 Тогда // Идентификатор.
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Загрузка значений категории с кодом %1';
							|en = 'Import category values with code %1'"),
...
#Область Проверки
	...
	Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.CONNECTION.NOT_CONNECTED", НСтр("ru = 'Не удалось подключиться к веб-сервисам.';
																						|en = 'Cannot connect to web services.'"));
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка обновления списка значений категории новостей (Прокси.getUpdatedCategoryValuesByCode):
						...
						|errDescription = %3
						|%4';
						|en = 'An error occurred while updating a value list of news category (Прокси.getUpdatedCategoryValuesByCode):
						|sCategoryCode = %1
						|errCode = %2
						|errDescription = %3
						|%4'"),
			...
			Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Информация об обновлении значений категорий новостей (Прокси.getUpdatedCategoryValuesByCode):
						...
						|Полученные данные:
						|%7';
						|en = 'Information on news category value update (Прокси.getUpdatedCategoryValuesByCode):
						|configName = %1
						|configVersion = %2
						|Category code: %3
						|lastSynchronizationDateTime = %4
								...
								Исключение
									...
									ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = 'Ошибка записи элемента (%1) с уникальным идентификатором %2:
											|%3';
											|en = 'An error occurred when writing the item (%1) with UUID %2:
											|%3'"),
										Режим,
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка обновления списка лент новостей (Прокси.getUpdatedChannels):
					...
					|%3';
					|en = 'An error occurred while updating news feed list (Прокси.getUpdatedChannels):
					|errCode = %1
					|errDescription = %2
		...
		Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Информация об обновлении списка лент новостей (Прокси.getUpdatedChannels):
					...
					|Полученные данные:
					|%6';
					|en = 'Information on news feed list update (Прокси.getUpdatedChannels):
					|configName = %1
					|configVersion = %2
					|lastSynchronizationDateTime = %3
					|errCode = %4
							...
							Исключение
								...
								ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Ошибка записи элемента (%1) с уникальным идентификатором %2 по причине:
										|%3';
										|en = 'An error occurred when writing the item (%1) with unique ID %2 due to:
										|%3'"),
									Режим,
									ТекущаяСтрокаХДТО.guid,
									ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка обновления списка лент новостей (Прокси.getAllChannels):
					...
					|Подробное представление ошибки:
					|%3';
					|en = 'An error occurred while updating news feed list (Прокси.getAllChannels):
					|errCode = %1
					|errDescription = %2
					|Detailed error description:
					|%3'"),
		...
		Если ТипЗнч(СписокВсехДанных) = ТипОбъектXDTO Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Информация об обновлении списка лент новостей (Прокси.getAllChannels):
					...
					|Полученные данные:
					|%5';
					|en = 'Information on news feed list update (Прокси.getAllChannels):
					|configName = %1
					|configVersion = %2
					|errCode = %3
					|errDescription = %4
						...
						Если ТекущийЭлементСписка.Пометка = Ложь Тогда
							...
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Лента новостей [%1] помечена на удаление, т.к. она была отключена на сервере 1С';
									|en = 'News feed [%1] is marked for deletion as it was disabled on 1C server'"),
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка обновления списка категорий новостей (Прокси.getUpdatedCategories):
					...
					|Подробное представление ошибки:
					|%3';
					|en = 'An error occurred while updating news category list (Прокси.getUpdatedCategories):
					|errCode = %1
					|errDescription = %2
					|Detailed error description:
					|%3 '"),
		...
		Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Информация об обновлении списка категорий новостей (Прокси.getUpdatedCategories):
					...
					|Полученные данные:
					|%6';
					|en = 'Information on news category list update (Proxy.getUpdatedCategories):
					|configName = %1
					|configVersion = %2
					|lastSynchronizationDateTime = %3
					|errCode = %4
				...
				Исключение
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка обновления списка значений категории новостей (Прокси.getUpdatedCategoryValuesByGuid):
							...
							|Подробное представление ошибки:
							|%4';
							|en = 'An error occurred while updating a value list of news category (Прокси.getUpdatedCategoryValuesByGuid):
							|sCategoryGuid = %1
							|errCode = %2
							|errDescription = %3
							|Detailed error description:
				...
				Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Информация об обновлении значений категорий новостей (Прокси.getUpdatedCategoryValuesByGuid):
							...
							|Полученные данные:
							|%7';
							|en = 'Information on news category value update (Прокси.getUpdatedCategoryValuesByGuid):
							|configName = %1
							|configVersion = %2
							|Category Payment ID: %3
							|lastSynchronizationDateTime = %4
							|errCode = %5
							|errDescription = %6
							|Received data:
...
#Область Обработка
	...
	Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
		...
		ОписаниеРезультата = НСтр("ru = 'Не удалось подключиться к прокси веб-сервисов обновления классификаторов';
									|en = 'Cannot connect to the proxy of web services of classifier update'");
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			?(ТипЗнч(ДатыОбновленияСписков) = ТипОбъектXDTO,
				...
				НСтр("ru = 'Получено значение с типом значения, отличным от ОбъектXDTO';
					|en = 'Value with a value type which is different from ObjectXDTO is received'"))));
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
...
#Область Обработка_ЗагрузкаДанных
	...
	ЗавершитьШагВыполнения(КонтекстВыполнения, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Получение списка последних обновлений, результат
			|errCode = %1
			|errDescription = %2';
			|en = 'Get the latest update list, result
			|errCode = %1
			|errDescription = %2'"),
		errCode,
		errDescription));
		...
		ЛогШаговВыполнения.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Разбор файла новостей для ленты новостей %1
					...
					|en = 'Parsing news file for the %1 news feed
					|News feed format is determined: %2'"),
				ЛентаНовостей,
				НРег(ФорматНовостей)));
					...
					ЛогШаговВыполнения.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Обновление стандартного списка %1 (версия %2) с сервера. Результат: %3';
								|en = 'Updating standard list %1 (version %2) from the server. Result: %3'"),
							ТекущиеОбновляемыеДанные.Список,
							ТекущиеОбновляемыеДанные.ТекущаяВерсияНаКлиенте,
							Результат));
				КонецЦикла;
			Иначе
				ЛогШаговВыполнения.Добавить(
					НСтр("ru = 'Обработка.УправлениеНовостями.РазборXMLФайлаНовостей(rss): Отключена работа с новостями через интернет.
						...
						|en = 'Обработка.УправлениеНовостями.РазборXMLФайлаНовостей(rss): Work with news via the Internet is disabled.
						|Classifiers were not updated.'"));
			КонецЕсли;
						...
						ЛогШаговВыполнения.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'ДатаЗагрузкиПоследнихНовостейНаКлиенте: %1
									...
									|en = 'ДатаЗагрузкиПоследнихНовостейНаКлиенте: %1
									|ДатаЗагрузкиПоследнихНовостейВФайле: %2
									|News in the application does not require update.'"),
					...
					ЛогШаговВыполнения.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'В файле полученном для ленты новостей %1 отсутствует поле channel.lastBuildDate.
								...
								|en = 'No channel.lastBuildDate field in the file received for the %1 news feed.
								|All news will be read and imported but it will take longer.'"),
							ЛентаНовостей));
				КонецЕсли;
			Иначе
				ЛогШаговВыполнения.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'В файле полученном для ленты новостей %1 отсутствует обязательное поле channel. Файл не был обработан.';
							|en = 'No required field channel in the file received for the %1 news feed. The file was not processed.'"),
						ЛентаНовостей));
				НеобходимоОбработатьФайл = Ложь;
			КонецЕсли;
						...
						ЛогШаговВыполнения.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'ДатаЗагрузкиПоследнихНовостейВФайле: %1
									...
									|en = 'ДатаЗагрузкиПоследнихНовостейВФайле: %1
									|ДатаПоследнейПопыткиЗагрузкиНовостей: %2'"),
								ДатаЗагрузкиПоследнихНовостейВФайле,
		...
		Иначе
			ЛогШаговВыполнения.Добавить(
				НСтр("ru = 'Обработка.УправлениеНовостями.РазборXMLФайлаНовостей(atom): Отключена работа с новостями через интернет.
					|Обновление классификаторов не выполнялось.';
					|en = 'Обработка.УправлениеНовостями.РазборXMLФайлаНовостей(atom): Work with news via the Internet is disabled.
					|Classifiers were not updated.'"));
		КонецЕсли;
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В файле полученном для ленты новостей %1 отсутствует поле feed.updated. Все новости будут прочитаны и загружены, но это займет больше времени.';
						|en = 'No feed.updated field in the file received for the %1 news feed. All news will be read and imported but it will take longer.'"),
					ЛентаНовостей));
		КонецЕсли;
		...
		ЛогШаговВыполнения.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Новости не загружены, т.к. передан необрабатываемый формат ленты новостей: %1';
					|en = 'News is not imported as unprocessable news feed format was transferred: %1'"),
				НРег(ФорматНовостей)));
	...
	Если НРег(ФорматНовостей) = "rss" Тогда
		...
		Если ЗаписыватьНовость = Истина Тогда
			...
			Попытка
				...
				ЛогШаговВыполнения.Добавить(НСтр("ru = 'Начало попытки записи новости';
												|en = 'Start of attempt to write news'"));
				...
				Если (СтруктураНовости.Ссылка.Пустая()) И (НЕ СтруктураНовостиПередЗаписью.Ссылка.Пустая()) Тогда
					...
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Найдена новость с такими же значениями ключей (ЛентаНовостей и УИННовости). Запись отменена.';
													|en = 'A news item with the same key values (ЛентаНовостей and УИННовости) was found. The record is canceled.'"));
				...
				Иначе
					...
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Успешная попытка записи новости';
													|en = 'Successful attempt to write news'"));
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Неудачная попытка записи новости по причине:
						|%1';
						|en = 'Unsuccessful attempt to write news. Reason:
						|%1'"),
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				ЛогШаговВыполнения.Добавить(ТекстСообщения);
		...
		РезультатЗагрузкиНовости = ""
			...
			+ НСтр("ru = 'ДатаПубликации';
				...
				+ ?(СтруктураНовости.Ссылка.Пустая(), " " + НСтр("ru = '(в базе не было)';
																|en = '(it was not in the base)'"), " (" + Формат(ДатаПубликацииИБ, "ДЛФ=DT") + ")") + "; "
			+ НСтр("ru = 'ПометкаУдаления';
					|en = 'ПометкаУдаления'") + "="
				+ ЗначениеПометкаУдаления + "/"
				+ ?(СтруктураНовости.Ссылка.Пустая(), " " + НСтр("ru = '(в базе не было)';
																|en = '(it was not in the base)'"), " (" + ЗначениеПометкаУдаленияИБ + ")") + "; "
			+ ?(ЗаписыватьНовость = Истина, НСтр("ru = 'Будет записана';
												|en = 'It will be written'"), НСтр("ru = 'Будет пропущена';
																				|en = 'Will be skipped'")) + "; "
			+ ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ru = 'В базе еще не создана';
														|en = 'It has not been created in the base yet'"), НСтр("ru = 'Уже существует в базе';
																							|en = 'Already exists in the infobase'")) + "; "
			+ НСтр("ru = 'УИН_ИБ';
					|en = 'УИН_ИБ'") + "="
				+ ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ru = '(еще не создана)';
															|en = '(not created yet)'"), "[" + СтруктураНовости.УИН_ИБ + "]")
			+ НСтр("ru = 'НавигационнаяСсылка';
					|en = 'НавигационнаяСсылка'") + "="
				+ ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ru = '(еще не создана)';
															|en = '(not created yet)'"), "[" + ПолучитьНавигационнуюСсылку(СтруктураНовости.Ссылка) + "]");
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось загрузить бинарные данные для новости с УИН %1 по причине:
						|%2';
						|en = 'Cannot import binary data for the news from payment ID %1 due to:
						|%2'"),
				...
				ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru = 'БИП:Новости.Загрузка новостей';
						|en = 'ISL:News.News import'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
					НСтр("ru = 'Новости. Загрузка новостей. Разбор файлов новостей. Разбор файла новостей. Разбор новости. Чтение новости. Блок бинарных данных';
						|en = 'News. News import. News file parsing. News file parsing. News parsing. Read news. Binary data unit'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
	...
	Если ТипЗнч(Обновление) <> ТипОбъектXDTO Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Тип значения параметра Обновление = [%1]. Ожидается [ОбъектXDTO].';
				|en = 'Update parameter value type = [%1]. [ОбъектXDTO] is expected.'"),
			ТипЗнч(Обновление));
	...
	Исключение
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.CANNOT_PARSE", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка загрузки строки обновления списков по причине:
				|%1';
				|en = 'An error occurred while importing list update line due to:
				|%1'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
	КонецПопытки;
...
Функция ЗаполнитьОбъектКатегорияНовостей(ОбъектХДТО, Режим = "")
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если НайденныеДубликаты.Количество() > 0 Тогда
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При загрузке категории новостей [%1] [%2] (%3), режим [%4] обнаружены дубликаты:';
						|en = 'Duplicates found when importing news category [%1] [%2] (%3), mode [%4]:'", КодОсновногоЯзыка),
					лкОбъект.Код,
			...
			ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru = 'БИП:Новости.Загрузка классификаторов';
					|en = 'ISL:News.Classifier import'", КодОсновногоЯзыка), // ИмяСобытия.
				НСтр("ru = 'Новости. Загрузка классификаторов. Список категорий. Ошибка';
					|en = 'News. Classifiers import. List of categories. Error'", КодОсновногоЯзыка), // ИдентификаторШага.
				УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
						...
						ЛогШаговВыполнения.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Из ленты новостей %1 удалена категория %2.';
									|en = 'The %2 category is deleted from the %1 news feed.'"),
						...
						ЛогШаговВыполнения.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'В ленте новостей %1 добавлена категория %2.';
									|en = 'The %2 category is added to the %1 news feed.'"),
								СокрЛП(лкОбъект.Наименование),
	...
	ЛогШаговВыполнения.Добавить(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Начало загрузки стандартных списков
				...
				|en = 'Start of standard list import
				|WSDLlocation = %1'"),
			МестоположениеWSDL));
	...
	Попытка
		...
		Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
			ЛогШаговВыполнения.Добавить(НСтр("ru = 'Неудачное подключение к веб-сервису обновления стандартных списков';
											|en = 'Unsuccessful connection to web service of standard list update'"));
		...
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru = 'Успешное подключение к веб-сервису обновления стандартных списков';
											|en = 'Successfully connected to the web service of standard lists update'"));
			ЗавершитьШагВыполнения(КонтекстВыполнения, "", ЛогШаговВыполнения);
		КонецЕсли;
		...
		ЛогШаговВыполнения.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неудачное подключение к веб-сервису обновления стандартных списков по причине:
					|%1';
					|en = 'Failed to connect to web service of standard lists update due to:
					|%1'"),
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.CONNECTION.NOT_CONNECTED", ЛогШаговВыполнения);
...
Функция ВсеШаблоныСообщенийЗагрузкиФайловНовостей()
	...
	Результат.Вставить("НачалоПопытки",
		НСтр("ru = 'Начало попытки {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			...
			|(URL: {{Запрос.URL}}).';
			|en = 'The start of attempt {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			|to get the [{{Запрос.ИмяФайла}}] file from the [{{Запрос.Сайт}}] server for the [{{ЛентаНовостей}}] news feed
			|to the [{{ВременныйФайл.Имя}}] temporary file.
			|(URL: {{Запрос.URL}}).'"));
	...
	Результат.Вставить("ПолученОтвет",
		НСтр("ru = 'При попытке {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			...
			|en = 'On attempt {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			|to get the [{{Запрос.ИмяФайла}}] file from the [{{Запрос.Сайт}}] server for the [{{ЛентаНовостей}}] news feed
			| to the temporary file, the server response with code [{{Ответ.КодСостояния}}] was received, headers:
			|{{Ответ.Заголовки}}
			|
			|Query headers:
			|{{Запрос.Заголовки}}
			|
			|(URL: {{Запрос.URL}}).
			|Import time (sec): {{Ответ.Длительность}}'"));
	...
	Результат.Вставить("ПолученОтвет304",
		НСтр("ru = 'Успешное завершение попытки {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			...
			|en = 'Successful completion of attempt {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			|to get the [{{Запрос.ИмяФайла}}] file from the [{{Запрос.Сайт}}] server for the [{{ЛентаНовостей}}] news feed.
			|(URL: {{Запрос.URL}}).
			|ETag: [{{Запрос.ETag}}]
			|The news list was not changed, so the file was not imported. The news feed is up to date.'"));
	...
	Результат.Вставить("ПолученОтветФайл",
		НСтр("ru = 'Успешное завершение попытки {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			...
			|to get the [{{Запрос.ИмяФайла}}] file from the [{{Запрос.Сайт}}] server for the [{{ЛентаНовостей}}] news feed
			|to the [{{ВременныйФайл.Имя}}] temporary file.
			|A {{ВременныйФайл.Размер}} byte file is received.
			|(URL: {{Запрос.URL}}).
			|The first file lines:
			|{{ВременныйФайл.ПервыеСтроки}}'"));
	...
	Результат.Вставить("НеУдалосьУдалитьФайл",
		НСтр("ru = 'ЗагрузитьФайлыНовостейССервера (получено из интернет):
			...
			|
			|Рекомендуется вручную удалить этот временный файл.';
			|en = 'ЗагрузитьФайлыНовостейССервера (received from the Internet):
			|Cannot remove the  [{{ВременныйФайл.Имя}}]
			| temporary file for the [{{ЛентаНовостей}}] news feed due to:
			|{{ПодробноеПредставлениеОшибки}}
			|
			|It is recommended that you remove this temporary file manually.'"));
	...
	Результат.Вставить("ПолученОтветНоФайлаНет",
		НСтр("ru = 'Неудачное завершение попытки {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			...
			|en = 'Failed attempt {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}
			|to get the [{{Запрос.ИмяФайла}}] file from the [{{Запрос.Сайт}}] server for the [{{ЛентаНовостей}}] news feed
			|to the [{{ВременныйФайл.Имя}}] temporary file.
			|(URL: {{Запрос.URL}}).
			|The file still does not exist after an attempt to import it.'"));
	...
	Результат.Вставить("ИсключениеПриПопыткеЗагрузкиФайлаНовостей",
		НСтр("ru = 'Попытка {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}.
			...
			|{{ПодробноеПредставлениеОшибки}}';
			|en = 'Attempt {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}. 
			|An error occurred when getting the [{{Запрос.ИмяФайла}}] file from the [{{Запрос.Сайт}}] server for the [{{ЛентаНовостей}}] news feed
			|to the [{{ВременныйФайл.Имя}}] temporary file. Reason:
			|{{ПодробноеПредставлениеОшибки}}'"));
	...
	Результат.Вставить("РезультатЗагрузкиФайлаНовостей",
		НСтр("ru = 'Попытка {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}.
			...
			|en = 'The {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}attempt.
			|Importing the [{{Запрос.ИмяФайла}}] news file from the [{{Запрос.Сайт}}] server for the [{{ЛентаНовостей}}] news feed
			|to the [{{ВременныйФайл.Имя}}] temporary file.
			|(URL: {{Запрос.URL}}).
			|Success? {{ЗагрузкаУспешна}}'"));
	...
	Результат.Вставить("Переадресация",
		НСтр("ru = 'Попытка {{НомерПопыткиЗагрузкиФайлаНовостей}} / {{МаксимумПопытокЗагрузкиФайлаНовостей}}.
			...
			|to the [{{ВременныйФайл.Имя}}] temporary file.
			|(URL: {{Запрос.URL}}).
			|State code: {{Ответ.КодСостояния}}
			|New address: {{URLПереадресации}}'"));
...
Функция ВсеШаблоныСообщенийРазбораXMLФайловНовостей()
	...
	Результат.Вставить("НачалоОбработкиЛенты",
		НСтр("ru = 'Лента: [{ЛентаНовостей}], обязательная: {ОбязательныйКанал}, пометка: {Пометка}';
			|en = 'Feed: [{ЛентаНовостей}], required: {ОбязательныйКанал}, mark: {Пометка}'"));
	...
	Результат.Вставить("БудетПропущена",
		НСтр("ru = 'Лента: [{ЛентаНовостей}] будет пропущена: обязательная: {ОбязательныйКанал}, пометка: {Пометка}';
			|en = 'Feed: [{ЛентаНовостей}] will be skipped: required: {ОбязательныйКанал}, mark: {Пометка}'"));
	...
	Результат.Вставить("ПустойФайл",
		НСтр("ru = 'Не загружен файл новостей для ленты новостей [{ЛентаНовостей}] (пустой адрес в хранилище после загрузки файла [{ИмяФайла}]
			...
			|en = 'News file for the news feed [{ЛентаНовостей}] was not imported (empty address in the storage after importing file [{ИмяФайла}]
			|from website [{Сайт}] using protocol [{Протокол}]). News feed was not processed.'"));
	...
	Результат.Вставить("ОшибкаСозданияФайла",
		НСтр("ru = 'Отсутствует загруженный файл [{ИмяВременногоФайла}] для ленты новостей [{ЛентаНовостей}].
			...
			|en = 'Imported file [{ИмяВременногоФайла}] for the news feed [{ЛентаНовостей}] is missing.
			|There may be not enough space on the disk, you might not have sufficient rights to save files to this directory, or another error occurred.'"));
	...
	Результат.Вставить("ОшибкаПолученияТипаДанных",
		НСтр("ru = 'В загруженном файле для ленты новостей [{ЛентаНовостей}] находится текст не в формате xml. Лента новостей будет пропущена.
			...
			|Первые строки файла:
			|{ПервыеСтрокиФайла}';
			|en = 'The imported file for the news feed [{ЛентаНовостей}] contains text not in XML format. The news feed will be skipped. 
			|
			|Detailed error description:
			|{ПодробноеПредставлениеОшибки}
			|
	...
	Результат.Вставить("ОшибкаОпределенияВерсииRSS",
		НСтр("ru = 'Невозможно определить версию rss в ленте новостей [{ЛентаНовостей}] - отсутствует свойство version.
			...
			|en = 'Cannot determine RSS version in news feed [{ЛентаНовостей}] - the Version property is missing.
			|Data in format of version 2.0 is required.'"));
	...
	Результат.Вставить("ОшибкаНеподдерживаемаяВерсияRSS",
		НСтр("ru = 'Неподдерживаемая версия новостей RSS [{ВерсияRSS}] в ленте новостей [{ЛентаНовостей}].';
			|en = 'Unsupported RSS news version [{ВерсияRSS}] in news feed [{ЛентаНовостей}].'"));
	...
	Результат.Вставить("ОшибкаНеподдерживаемыйТипНовостей",
		НСтр("ru = 'Неподдерживаемый тип новостей [{ТипНовостей}] в файле [{ИмяВременногоФайла}] для ленты новостей [{ЛентаНовостей}]
			...
			|Первые строки файла:
			|{ПервыеСтрокиФайла}';
			|en = 'Unsupported news type [{ТипНовостей}] in file [{ИмяВременногоФайла}] for the news feed [{ЛентаНовостей}]
			|
			|First lines of file:
			|{ПервыеСтрокиФайла}'"));
	...
	Результат.Вставить("ОшибкаУдаленияВременногоФайла",
		НСтр("ru = 'Не удалось удалить временный файл [{ИмяВременногоФайла}] для ленты новостей [{ЛентаНовостей}] по причине:
			...
			|
			|Рекомендуется вручную удалить этот временный файл.';
			|en = 'Cannot delete temporary file [{ИмяВременногоФайла}] for the [{ЛентаНовостей}] news feed due to:
			|{ПодробноеПредставлениеОшибки}
			|
			|It is recommended that you delete this temporary file manually.'"));
	...
	Результат.Вставить("ОкончаниеОбработкиЛенты",
		НСтр("ru = 'Завершение обработки ленты: [{ЛентаНовостей}]. Успешно: {ЕстьОшибки|БЛ=БезОшибок;БИ=ЕстьОшибки}';
			|en = 'Complete feed processing: [{ЛентаНовостей}]. Success: {ЕстьОшибки|BF=БезОшибок;BT=ЕстьОшибки}'"));
