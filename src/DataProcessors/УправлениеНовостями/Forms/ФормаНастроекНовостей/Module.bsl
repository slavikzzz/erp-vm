...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_СоздаютсяНапрямуюВСправочнике", НСтр("ru = 'Создаются напрямую в справочнике';
																												|en = 'Created directly in catalog'"));
	...
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ЗагружаютсяЧерезИнтернетПоПротоколу", НСтр("ru = 'Загружаются через интернет по протоколу';
																														|en = 'Importing via the Internet by protocol'"));
	...
	Если ЗавершениеРаботы = Истина Тогда
		...
		Если ЭтотОбъект.Модифицированность Тогда
			...
			ТекстПредупреждения = НСтр("ru = 'Данные не сохранены.
				...
				|or close the window manually before exiting the application.'");
		КонецЕсли;
	Иначе
	КонецЕсли;
	...
	Если ВРег(ЛокальнаяНавигационнаяСсылка) = ВРег("DeleteNow") Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Пометить на удаление устаревшие новости?';
				|en = 'Mark obsolete news for removal?'"),
			РежимДиалогаВопрос.ДаНет,
			0,
			КодВозвратаДиалога.Нет,
			НСтр("ru = 'Очистка устаревших новостей вручную';
				|en = 'Delete outdated news manually'"));
	КонецЕсли;
...
Процедура КомандаЗагрузитьСтандартныеСписки(Команда)
	...
	ФормаСостоянияОперации = ОткрытьФорму(
		...
		Новый Структура("Заголовок, СостояниеОперации", 
			НСтр("ru = 'Обновление классификаторов с сервера';
				|en = 'Update classifiers from the server'"),
			НСтр("ru = 'Происходит обновление классификаторов с сервера
				|Это может занять некоторое время...';
				|en = 'Updating classifiers from the server
				|This might take a while...'")),
		ЭтотОбъект,
	...
	Оповестить(
		...
		НСтр("ru = 'Происходит обновление классификаторов с сервера
			...
			|This might take a while...
			|
			|Updating classifier relevance dates.'"),
		ИдентификаторФормыСостоянияОперации);
...
Процедура КомандаПолучитьОтладочнуюИнформациюОНовости(Команда)
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.УИННовости) Тогда
		...
		#Иначе
			...
			ТекстовыйДокумент.УстановитьТекст(ИнформацияОНовости);
			ТекстовыйДокумент.Показать(НСтр("ru = 'Отладочная информация о новости';
											|en = 'Debug information on news'"), ИмяВременногоФайла);
...
Процедура КомандаДиагностикаПолученияНовостей(Команда)
	...
	СписокШагов = Новый СписокЗначений;
		...
		СписокШагов.Добавить(12, "Настройки программы: Регламентное задание (неразделенное)", Истина); // Идентификатор.
		СписокШагов.Добавить(13, "Настройки программы: Регламентное задание (для области данных)", Истина); // Идентификатор.
		СписокШагов.Добавить(14, "Настройки программы: Заголовки системы", Ложь); // Идентификатор.
		СписокШагов.Добавить(15, "Настройки программы: Категории новостей", Истина); // Идентификатор.
		СписокШагов.Добавить(16, "Данные программы: Ленты новостей", Истина); // Идентификатор.
		СписокШагов.Добавить(17, "Данные программы: Последние новости с рассчитанными отборами", Истина); // Идентификатор.
		СписокШагов.Добавить(18, "Журнал регистрации: состояние системы", Истина); // Идентификатор.
		СписокШагов.Добавить(19, "Логи: фоновые задания", Истина); // Идентификатор.
	// АПК:1297 -вкл
	...
	#Если ВебКлиент Тогда
		...
		ТекстовыйДокумент.Показать(НСтр("ru = 'Результат диагностики получения новостей';
										|en = 'Result of news receipt diagnostics'"));
	...
	#Иначе
		...
		ТекстовыйДокумент.УстановитьТекст(СтрСоединить(РезультатДиагностикиПолученияНовостей, Символы.ПС));
		ТекстовыйДокумент.Показать(НСтр("ru = 'Результат диагностики получения новостей';
										|en = 'Result of news receipt diagnostics'"), ИмяВременногоФайла);
...
Процедура УправлениеФормой(Форма)
	...
	Если ЛокальнаяРазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
		Элементы.РазрешенаРаботаСНовостямиЧерезИнтернет.Подсказка = НСтр("ru = 'Будет регулярно проверяться наличие новых новостей.';
																		|en = 'Current news will be checked regularly.'");
	...
	Иначе
		Элементы.РазрешенаРаботаСНовостямиЧерезИнтернет.Подсказка = НСтр("ru = 'Проверка новых новостей через интернет отключена.
			...
			|en = 'News checking via the Internet is disabled.
			|News including the important ones will not be received.'");
...
Функция ВыполнитьШагПроверкиНаКлиенте(Шаг, ПрочиеПараметры)
	...
	ИначеЕсли Шаг = 14 Тогда // "Настройки программы: Заголовки системы"
...
Функция ВыполнитьШагПроверкиНаСервере(Знач Шаг, Знач ПрочиеПараметры)
	...
	ИначеЕсли Шаг = 5 Тогда // "Настройки программы: правильность внедрения подсистемы"
		...
		Если ПустаяСтрока(Результат) Тогда
			Результат = Символы.Таб + НСтр("ru = 'Подсистема Новости внедрена правильно';
											|en = 'The News subsystem is implemented correctly'")
				+ Символы.ПС;
		КонецЕсли;
	...
	ИначеЕсли Шаг = 12 Тогда // "Настройки программы: Регламентное задание (неразделенное)"
		Результат = ВыполнитьШагПроверки_РегламентноеЗаданиеНеразделенное();
	ИначеЕсли Шаг = 13 Тогда // "Настройки программы: Регламентное задание (для области данных)"
		Результат = ВыполнитьШагПроверки_РегламентноеЗаданиеДляОбластиДанных();
	ИначеЕсли Шаг = 15 Тогда // "Настройки программы: Категории новостей"
		Результат = ВыполнитьШагПроверки_КатегорииНовостей();
	ИначеЕсли Шаг = 16 Тогда // "Данные программы: Ленты новостей"
		Результат = ВыполнитьШагПроверки_ЛентыНовостей();
	ИначеЕсли Шаг = 17 Тогда // "Данные программы: Последние новости с рассчитанными отборами"
		Результат = ВыполнитьШагПроверки_ПоследниеНовостиСОтборами();
	ИначеЕсли Шаг = 18 Тогда // "Журнал регистрации: состояние системы"
		Результат = ВыполнитьШагПроверки_СостояниеСистемы();
	ИначеЕсли Шаг = 19 Тогда // "Логи: фоновые задания"
...
Функция ВыполнитьШагПроверки_ЗаполненностьОбъектовМетаданных()
	...
	ЛогШаговВыполнения.Добавить("  " + НСтр("ru = 'Категории новостей (первые 20 значений):';
											|en = 'News categories (first 20 values)'"));
	...
	ЛогШаговВыполнения.Добавить("  " + НСтр("ru = 'Значения категорий новостей (первые 20 значений):';
											|en = 'News category values (first 20 values):'"));
	...
	ЛогШаговВыполнения.Добавить("  " + НСтр("ru = 'Ленты новостей (первые 20 значений):';
											|en = 'News feed (the first 20 values):'"));
...
#Область АдминистративныеОтборы
	...
	ЛогШаговВыполнения.Добавить("### " + НСтр("ru = 'Отборы, настроенные администратором (первые 40 значений):';
												|en = 'Filters set up by administrator (the first 40 values):'"));
...
#Область ОтборыДляОбластиДанных
	...
	ЛогШаговВыполнения.Добавить("### " + НСтр("ru = 'Отборы, настроенные для областей данных (первые 40 значений):';
												|en = 'Filters set up for data areas (first 40 values):'"));
	ЛогШаговВыполнения.Добавить(СписокСтрокой);
...
#Область ПользовательскиеОтборы
	...
	ЛогШаговВыполнения.Добавить("### " + НСтр("ru = 'Отборы, настроенные пользователями (первые 40 значений):';
												|en = 'Filters set up by users (the first 40 values):'"));
	ЛогШаговВыполнения.Добавить(СписокСтрокой);
...
Функция ВыполнитьШагПроверки_ПараметрыСеанса(ПрочиеПараметры)
	...
	Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Параметры окружения подсистемы новостей:
			...
			|%1
			|Data area (transferred as a parameter): %2
			|Selected data area (used for settlements): %3
			|Launch parameter /C: %4'"),
...
Функция СформироватьПолныйАдресЛентыНовостей(ЛокальнаяСсылка, ЛокальныйПротокол, ЛокальныйСайт, ЛокальноеИмяФайла)
	...
	Иначе
		Результат = НСтр("ru = 'Адрес ленты новостей можно получить только для протоколов http и https';
						|en = 'News feed address can be received only for protocols http and https'");
	КонецЕсли;
...
#Область ОченьВажныеНовости
	...
	ЛогШаговВыполнения.Добавить("### " + НСтр("ru = 'Список очень важных новостей:';
												|en = 'List of very important news:'"));
	ЛогШаговВыполнения.Добавить(СписокСтрокой);
...
Функция ВыполнитьШагПроверки_ПроверкаHTTPСоединения()
	...
	ЛогШаговВыполнения.Добавить(НСтр("ru = 'Получение новостей по первым нескольким лентам новостей:';
									|en = 'Receive news from several first news feeds:'"));
					...
					ЛогШаговВыполненияВложенный.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '%1Успешное получение файла новостей.
								...
								|%6
								|```';
								|en = '%1News file is successfully received.
								|%1HTTP response code: %2
								|%1HTTP response headers:
								|%3
								|%1Temporary file name: %4
								|%1File size (bytes): %5
								|%1First file lines:
								|```xml
								|%6
					...
					ЛогШаговВыполненияВложенный.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '%1Неудачная попытка получения файла новостей.
								...
								|%3
								|%1Имя временного файла: %4';
								|en = '%1Unsuccessful attempt to receive a news file:
								|%1HTTP response code: %2
								|%1HTTP response titles:
								|%3
								|%1Temporary file name: %4'")
								+ Символы.ПС,
							"    ",
				...
				ЛогШаговВыполненияВложенный.Добавить(
					...
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'ОШИБКА при получении новостей:
							|%1';
							|en = 'An error occurred while receiving news:
							|%1'")
							+ Символы.ПС,
	...
	Результат = Результат
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Автоматическая проверка НЕ реализована.
					...
					|en = 'Automatic check is NOT implemented.
					|Check manually whether there is enough space to store files by the following path:
					|%1
					|Remember that if you use the server cluster, news can be received
					|on any server, that is why you should check free space on all servers.'"),
			ИмяВременногоФайла);
...
&НаСервереБезКонтекста
Функция ВыполнитьШагПроверки_РегламентноеЗаданиеНеразделенное()
...
&НаСервереБезКонтекста
Функция ВыполнитьШагПроверки_РегламентноеЗаданиеДляОбластиДанных()
	...
	РегламентноеЗадание = РегламентныеЗаданияСервер.ПолучитьРегламентноеЗадание(Метаданные.РегламентныеЗадания.ВсеОбновленияНовостейДляОбластиДанных);
	РасписаниеРегламентногоЗадания = РегламентныеЗаданияСервер.РасписаниеРегламентногоЗадания(РегламентноеЗадание, Истина);
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Имя формы: %1
						...
						|%2';
						|en = 'Form name: %1
						|An error occurred when receiving data of the News attribute:
						|%2'")
						+ Символы.ПС,
			...
			Если ТипЗнч(ТекущаяИнформацияЗначениеЭталон.Новости) = ТипМассив
				...
				Для Каждого ТекущаяНовость Из ТекущаяИнформацияЗначениеЭталон.Новости Цикл
					...
					СписокПервыхНовостей.Добавить(
						...
						+ ?(ТекущаяНовость.Прочтена, НСтр("ru = 'Прочтена';
															|en = 'Read'"), НСтр("ru = 'НЕ прочтена';
																				|en = 'NOT read'")) + "; ");
				...
				СписокПервыхНовостей.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'В реквизите Новости или ключ массива контекстных новостей Новостей неправильного типа (%1), или массив - пустой.';
							|en = 'In the News attribute, the array key of the News context news has an incorrect type (%1) or the array is empty.'"),
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Имя формы: %1
						...
						|	Form: %3
						|	ЗаголовокФормыКонтекстныхНовостей: %4
						|	Total amount of context news: %5
						|	Several random context news:
						|%6
						|	ПолучатьНовостиНаСервере: %7
						|	ХранитьМассивНовостейТолькоНаСервере: %8'")
						+ Символы.ПС
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Имя формы: %1
						...
						|en = 'Form name: %1
						|An error occurred when receiving data of the News attribute. Incorrect data type: %2'")
						+ Символы.ПС,
					ТекущаяИнформация.Представление,
...
Функция ВыполнитьШагПроверки_ПоследниеНовостиСОтборами()
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
		Пока Выборка.СледующийПоЗначениюПоля("ЛентаНовостей") Цикл
			ЛогШаговВыполнения.Добавить("-" + НСтр("ru = 'Лента новостей';
													|en = 'News feed'") + ": " + Выборка.ЛентаНовостей); // Идентификатор.
			Пока Выборка.СледующийПоЗначениюПоля("Новость") Цикл
				...
				Пока Выборка.Следующий() Цикл
					ЛогШаговВыполнения.Добавить("    - " + НСтр("ru = 'Редко-меняющийся';
																|en = 'Rarely changed'") + ": " + Выборка.РезультатОтбора_РедкоМеняющийся);
					ЛогШаговВыполнения.Добавить("      - " + НСтр("ru = 'По продукту';
																	|en = 'By product'") + ": " + Выборка.РезультатОтбора_РедкоМеняющийся_ПоПродукту);
					ЛогШаговВыполнения.Добавить("      - " + НСтр("ru = 'По версии продукта';
																	|en = 'By product version'") + ": " + Выборка.РезультатОтбора_РедкоМеняющийся_ПоВерсииПродукта);
					ЛогШаговВыполнения.Добавить("      - " + НСтр("ru = 'По версии платформы';
																	|en = 'By platform version'") + ": " + Выборка.РезультатОтбора_РедкоМеняющийся_ПоВерсииПлатформы);
					ЛогШаговВыполнения.Добавить("      - " + НСтр("ru = 'По пометке удаления';
																	|en = 'By deletion mark'") + ": " + Выборка.РезультатОтбора_РедкоМеняющийся_ПоПометкеУдаления);
					ЛогШаговВыполнения.Добавить("      - " + НСтр("ru = 'По дате завершения';
																	|en = 'By completion date'") + ": " + Выборка.РезультатОтбора_РедкоМеняющийся_ПоДатеЗавершения);
					ЛогШаговВыполнения.Добавить("    - " + НСтр("ru = 'Общий';
																|en = 'Shared'") + ": " + Выборка.РезультатОтбора_Общий);
					ЛогШаговВыполнения.Добавить("    - " + НСтр("ru = 'Область данных';
																|en = 'Data area'") + ": " + Выборка.РезультатОтбора_ОбластьДанных);
					ЛогШаговВыполнения.Добавить("    - " + НСтр("ru = 'Пользователь';
																|en = 'User'") + ": " + Выборка.РезультатОтбора_Пользователь);
				КонецЦикла;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
Функция ВыполнитьШагПроверки_ЛогиФоновыхЗаданий()
	...
	Результат.Добавить("### " + НСтр("ru = 'Общие фоновые задания (не более 30 штук за последние 5 дней)';
									|en = 'Common background jobs (no more than 30 pcs. in the last 5 days)'") + ":");
	...
	Иначе
		Для Каждого ТекущийЭлементСписка Из Список Цикл
			Результат.Добавить("  - " + НСтр("ru = 'УИН';
											|en = 'Payment ID'")          + ": " + ТекущийЭлементСписка.Значение.УникальныйИдентификатор
			...
			Результат.Добавить("  - " + НСтр("ru = 'Расположение';
											|en = 'Location'") + ": " + ТекущийЭлементСписка.Значение.Расположение);
			Результат.Добавить("  - " + НСтр("ru = 'Состояние';
											|en = 'State'")    + ": " + ТекущийЭлементСписка.Значение.Состояние);
	...
	Результат.Добавить("### " + НСтр("ru = 'Разделенные фоновые задания (не более 30 штук за последние 5 дней)';
									|en = 'Separated background jobs (no more than 30 pcs. in the last 5 days)'") + ":");
	Результат.Добавить(""); // Пустая строка
	ОтборФоновыхЗаданий = Новый Структура;
		ОтборФоновыхЗаданий.Вставить("Начало", ТекущаяДатаСеанса() - 5*24*60*60); // За последние 5 дней.
