...
Процедура ОбновитьИнформационныеНадписи()
	...
	Если СуммаКВыплате >= МинимальнаяСуммаНевыплаченнойЗарплаты() Тогда
		НадписьНевыплаченнаяЗарплата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Остаток невыплаченной зарплаты на конец месяца: %1';
																									|en = 'Remaining unpaid salary at the end of the month: %1'"), СуммаКВыплате);
	...
	Элементы.ГруппаСледующийМесяц.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'После %1';
																											|en = 'After %1'"), Сред(Формат(МесяцНачисления, "ДЛФ=ДД"), 3));
	УстановитьОтображениеСообщенияОНеобходимостиПерерасчета();
