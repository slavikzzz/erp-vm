	...
	Если Не ДанныеПечати.Свойство("ШаблонЭтикетки") Тогда
		...
		Если Не ЗначениеЗаполнено(ШаблонЭтикетки) Тогда
			ТекстИсключения = НСтр("ru = 'Печать этикетки не возможна, т.к. в настройках программы не выбран шаблон этикетки по умолчанию. Обратитесь к администратору.';
									|en = 'Cannot print a label as the default label is not selected in the application settings. Please contact the administrator.'");
...
Функция ДанныеДляПечатиЭтикетокДоставки(МассивОбъектов) Экспорт
	...
	Если ТипЗнч(ТекущийОбъект) = Тип("ДокументСсылка.ЗаданиеНаПеревозку") Тогда
		...
		Для Каждого ТипРаспоряжения из ТипыРаспоряжений Цикл
			...
			//++ НЕ УТКА
			Если МетаданныеРаспоряжения = Метаданные.Документы.ЗаказПереработчику2_5
			 //++ Устарело_Переработка24
			 Или МетаданныеРаспоряжения = Метаданные.Документы.ЗаказПереработчику
			 //-- Устарело_Переработка24
			 Или Ложь Тогда
				...
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ",&ТекстЗапросаИмяТЧ", "");
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстУсловияСоединенияШапкиИТЧ",
					"(ДокументРаспоряжениеТовары.Ссылка.ЗаказПереработчику = РаспоряженияПоЗаданию.Распоряжение И НЕ ДокументРаспоряжениеТовары.Отменено)");
		...
		Для Каждого КлючЗначение из СтруктураТипов Цикл
			...
			Если МетаданныеРаспоряжения = Метаданные.Документы.ЗаказПереработчику2_5
			 ...
			 Или Ложь Тогда
				...
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ",&ТекстЗапросаИмяТЧ", "");
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстУсловияСоединенияШапкиИТЧ",
					"(ДокументРаспоряжениеТовары.Ссылка.ЗаказПереработчику = ДокументРаспоряжение.Ссылка И НЕ ДокументРаспоряжениеТовары.Отменено)");
...
Функция ДанныеДляПечатиЭтикетокДоставкиАдаптироватьКРаспоряжению(МетаданныеРаспоряжения, ТекстЗапросаПоРаспоряжению)
	...
	//-- Устарело_Переработка24
	ИменаТЧТовары.Вставить(Метаданные.Документы.ЗаказДавальца2_5,"Продукция");
	//-- НЕ УТКА
