...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ru = 'Сопоставление данных ""[ПредставлениеДанных]""';
					|en = 'Data mapping ""[ПредставлениеДанных]""'");
	...
	НаименованиеЭтойПрограммы = ?(ПустаяСтрока(НаименованиеЭтойПрограммы), НСтр("ru = 'Эта программа';
																				|en = 'This application'"), НаименованиеЭтойПрограммы);
	
	НаименованиеВторойПрограммы = Строка(Объект.УзелИнформационнойБазы);
	НаименованиеВторойПрограммы = ?(ПустаяСтрока(НаименованиеВторойПрограммы), НСтр("ru = 'Вторая программа';
																					|en = 'Other application'"), НаименованиеВторойПрограммы);
...
Процедура ОтменитьСопоставление(Команда)
	...
	Если ЕстьСопоставлениеПоСсылке Тогда
		...
		Заголовок = НСтр("ru = 'Отменить сопоставление';
						|en = 'Clear mapping'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура УстановитьСвязьИнтерактивно()
	...
	Если Не (ТекущиеДанные.СтатусСопоставления=-1 Или ТекущиеДанные.СтатусСопоставления=+1) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Объекты уже сопоставлены';
...
Процедура ЗафиксироватьОшибку(ПодробноеПредставлениеОшибки)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Помощник сопоставления объектов.Анализ данных';
			|en = 'Object mapping wizard.Data analysis'", ОбщегоНазначения.КодОсновногоЯзыка()),
