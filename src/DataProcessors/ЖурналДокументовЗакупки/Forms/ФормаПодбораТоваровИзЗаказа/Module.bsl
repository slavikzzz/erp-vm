...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТКА
	Если Параметры.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
		ИспользоватьЗаказыПоставщикам = ПолучитьФункциональнуюОпцию("ИспользоватьЗаказыДавальцев2_5");
		Элементы.ТаблицаТоваровЗаказПоставщику.Заголовок = НСтр("ru = 'Давалец';
...
Процедура УстановитьУсловноеОформление()
	...
	//++ НЕ УТКА
	СписокЗначений.Добавить(Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5);
	//-- НЕ УТКА
	ОтборЭлемента.ПравоеЗначение = СписокЗначений;
...
Процедура ЗаполнитьПоЗаказамПоПриемке(ДанныеОтбора, Товары, СкладПоступления, ПоОрдеру,
	...
	Для Каждого Строка Из ПринятыеТовары Цикл
		...
		//++ НЕ УТКА
		РаспоряжениеЗаказ   = РаспоряжениеЗаказ 
							  Или ТипЗнч(ДокументПоступления) = Тип("ДокументСсылка.ЗаказДавальца2_5");
		//-- НЕ УТКА	
