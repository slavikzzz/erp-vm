...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СообщенияВСлужбуТехническойПоддержки") Тогда
		Элементы.ПояснениеКЗаголовку.Заголовок = НСтр("ru = 'Для повышения качества обслуживания, пожалуйста, укажите дополнительную информацию о Вашей организации.';
														|en = 'To improve the service quality, please specify additional information on your company.'");
	КонецЕсли;
...
&НаКлиенте
Процедура ПояснениеКЗаголовкуОбработкаНавигационнойСсылки(
		Элемент,
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("ru = 'Не получается ввести информацию об организации при регистрации программного продукта.
				...
				|Факс: %8';
				|en = 'Cannot enter the company information while registering a software.
				|
				|Company name: %1
				|Activity type: %2
				|TIN: %3
		...
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СообщенияВСлужбуТехническойПоддержки") Тогда
			...
			ДанныеСообщения.Тема       = НСтр("ru = 'Интернет-поддержка. Ввод информации об организации.';
												|en = 'Online support. Enter company information.'");
			ДанныеСообщения.Сообщение  = ТекстСообщения + Символы.ПС + ТекстСообщенияПродолжение;
...
Процедура КомандаОтмена(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ru = 'Вы уверены, что хотите отказаться от регистрации программного продукта?';
			|en = 'Are you sure you want to cancel registration of the software?'"),
		РежимДиалогаВопрос.ДаНет,
		,
		,
		НСтр("ru = 'Регистрация программного продукта';
			|en = 'Software registration'"));
...
Функция ЗаполнениеПолейКорректно()
	...
	Если Организация = "-1" Тогда
		...
		Если ПустаяСтрока(НазваниеОрганизации) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Название организации"" не заполнено.';
												|en = 'The ""Company name"" field is not filled in.'"),
				"НазваниеОрганизации",
				Отказ);
		КонецЕсли;
		...
		Если ПустаяСтрока(ТипДеятельности) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Тип деятельности"" не заполнено.';
												|en = 'The ""Activity type"" field is not filled in.'"),
				"ТипДеятельности",
				Отказ);
		КонецЕсли;
	...
	Если ПустаяСтрока(ГдеПриобретенаПрограмма) Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Место покупки"" не заполнено.';
											|en = 'The ""Purchase place"" field is not filled in.'"),
			"ГдеПриобретенаПрограмма",
			Отказ);
	КонецЕсли;
	...
	Если ДатаПриобретенияПрограммы = '00010101' Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Дата покупки"" не заполнено.';
											|en = 'The ""Purchase date"" field is not filled in.'"),
	...
	Если ЧислоРабочихМест = 0 Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Число рабочих мест"" не заполнено.';
											|en = 'The ""Number of workplaces"" field is not filled in.'"),
			"ЧислоРабочихМест",
			Отказ);
	КонецЕсли;
	...
	Если ПустаяСтрока(Ответственный) Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Ответственный сотрудник"" не заполнено.';
											|en = 'The ""Responsible employee"" field is not filled in.'"),
			"Ответственный",
			Отказ);
	КонецЕсли;
