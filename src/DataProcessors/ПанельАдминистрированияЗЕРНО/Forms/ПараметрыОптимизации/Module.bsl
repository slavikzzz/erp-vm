...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДополнитьСписокВыбора("КоличествоЗапросовВМинуту",                          Ложь,   НСтр("ru = 'в минуту';
																							|en = 'в минуту'"));
	ДополнитьСписокВыбора("ИнтервалМеждуОтправкойЗапросаИПолучениемРезультата", Истина, НСтр("ru = 'секунда';
																							|en = 'секунда'"), Истина);
	ДополнитьСписокВыбора("ИнтервалМеждуПолучениемРезультатов",                 Истина, НСтр("ru = 'секунда';
																							|en = 'секунда'"), Истина);
	ДополнитьСписокВыбора("ТаймаутHTTPЗапросов",                                Истина, НСтр("ru = 'секунда';
																							|en = 'секунда'"), Истина);
	ДополнитьСписокВыбора("КоличествоЭлементовСтраницыОтвета",                  Ложь,   НСтр("ru = 'элемент';
																							|en = 'элемент'"), Истина);
	ДополнитьСписокВыбора("КоличествоЭлементовСтраницыОтветаСправочника",       Ложь,   НСтр("ru = 'элемент';
																							|en = 'элемент'"), Истина);
...
Процедура ВерсияAPIПриИзменении(Элемент)
	...
	Если ДанныеВерсии.Количество() <> 3
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Версия API должна быть в формате X.X.X, например, 1.0.8';
										|en = 'Версия API должна быть в формате X.X.X, например, 1.0.8'"));
		ВерсияAPI = АктуальнаяВерсияAPI;
		Возврат;
	ИначеЕсли ИнтеграцияИСКлиентСервер.СравнитьВерсии(ВерсияAPI, АктуальнаяВерсияAPI) < 0 Тогда
		ПоказатьПредупреждение(, СтрШаблон(НСтр("ru = 'Указание версии ниже %1 не подерживается';
												|en = 'Указание версии ниже %1 не подерживается'"), АктуальнаяВерсияAPI));
		ВерсияAPI = АктуальнаяВерсияAPI;
		Возврат;
	КонецЕсли;
...
Функция ПредставлениеВремени(ОбщееКоличествоСекунд)
	...
	Если Дней > 0 Тогда
		Строки.Добавить(СтрШаблон(НСтр("ru = '%1 дн.';
										|en = '%1 дн.'"), Дней));
	КонецЕсли;
	Если Часов > 0 Тогда
		Строки.Добавить(СтрШаблон(НСтр("ru = '%1 ч.';
										|en = '%1 ч.'"), Часов));
	КонецЕсли;
	Если Минут > 0 Тогда
		Строки.Добавить(СтрШаблон(НСтр("ru = '%1 мин.';
										|en = '%1 мин.'"), Минут));
	КонецЕсли;
	Если Секунд > 0 Или Строки.Количество() = 0 Тогда
		Строки.Добавить(СтрШаблон(НСтр("ru = '%1 сек.';
										|en = '%1 сек.'"), Секунд));
	КонецЕсли;
