...
Функция ПечатьПредварительныйПросмотрСчетаНаОплату(Организация, ОбъектыПечати, ИмяМакета)
	...
	СтруктураДанныхШапки.Вставить("НазначениеПлатежа", НСтр("ru = 'Предварительный просмотр печати счета на оплату.';
															|en = 'Preview of commercial invoice printing.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ДанныеЗаголовка.Вставить("Представление", НСтр("ru = 'Счет на оплату';
													|en = 'Proforma invoice'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	СтруктураДанныхПокупатель.Вставить("ПредставлениеПолучателя", 
		НСтр("ru = 'Поле содержит информацию покупателя: полное наименование, ИНН, КПП, юридический адрес, телефоны.';
			|en = 'Field contains customer information: full name, TIN, KPP, legal address, phones.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ОбластьМакета.Параметры.Заполнить(СтруктураДанныхПокупатель);
	ТабличныйДокумент.Вывести(ОбластьМакета);
	...
	Товары.Вставить("Товар",            НСтр("ru = 'Товар (предварительный просмотр)';
											|en = 'Goods (preview)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ОбластьМакета.Параметры.Заполнить(Новый Структура("НДС, ВсегоНДС", НСтр("ru = 'В т.ч. НДС (20%)';
																			|en = 'Including VAT (20%)'", ОбщегоНазначения.КодОсновногоЯзыка()), Товары.СуммаНДС));
	ТабличныйДокумент.Вывести(ОбластьМакета);
...
Процедура ЗаполнитьТабличныйДокументInvoice(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати, Предварительный = Ложь, КодЯзыка = Неопределено)
	...
	ШаблоныОшибок.Вставить("Товары", НСтр("ru = 'В документе %1 отсутствуют товары. Печать Proforma invoice не требуется';
											|en = 'Goods are missing in document %1. Printing of Proforma invoice is not required'"));
	ШаблоныОшибок.Вставить("ЗаменяющиеТовары", НСтр("ru = 'В документе %1 отсутствуют заменяющие товары. Печать Proforma invoice не требуется';
													|en = 'Exchange goods are missing in the %1 document. Printing of Proforma invoice is not required'"));
	ШаблоныОшибок.Вставить("Этапы", НСтр("ru = 'В документе %1 отсутствуют этапы оплаты. Печать Proforma invoice не требуется';
										|en = 'Payment milestones are missing in document %1. Printing of Proforma invoice is not required'"));
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Иначе
			...
			Иначе
				СтруктураИтоги.Вставить("НДС", НСтр("ru = 'Without tax';
													|en = 'Without tax'", ОбщегоНазначения.КодОсновногоЯзыка()));
