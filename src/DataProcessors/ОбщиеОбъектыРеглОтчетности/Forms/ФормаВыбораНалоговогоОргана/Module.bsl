...
Процедура СформироватьТаблицуНО()
	...
	|	СправочникИФНС.НаименованиеИФНС КАК ПолноеНаименование,
	...
	Для каждого Строка Из ТаблицаНО Цикл
		
		Количество = Строка.Ссылка.Подписанты.Количество();
		
		Если Количество = 1 Тогда
			
			Строка.Представитель = СтрШаблон(
				НСтр("ru = '1 представитель (%1) ';
					|en = 'One representative (%1) '"),
				Строка.Ссылка.Подписанты[0].Представитель);
			
		ИначеЕсли Количество > 1 Тогда
			
			Шаблон = ";%1 представитель;;%1 представителя;%1 представителей;%1 представителей";
			Строка.Представитель = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
				Шаблон, 
				Количество);
...
Процедура ИзменитьРегистрацию(Команда)
	...
	ТекущиеДанные = Элементы.ТаблицаНО.ТекущиеДанные;
