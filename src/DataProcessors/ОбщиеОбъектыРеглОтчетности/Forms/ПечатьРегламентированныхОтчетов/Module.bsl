...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СписокПечатаемыхЛистов.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не выбраны листы для вывода на печать.';
													|en = 'Sheets for printing are not selected.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если Параметры.Свойство("ЕстьВыходЗаГраницы") И Параметры.ЕстьВыходЗаГраницы = Истина Тогда 
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Некоторые поля адреса не уместились, рекомендуется воспользоваться печатной формой PDF417';
													|en = 'Some address fields did not fit, it is recommended to use the PDF417 print form'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура Инициализация()
	Для Каждого Лист Из СписокПечатаемыхЛистов Цикл 
		Если ТипЗнч(Лист.Значение) = Тип("Массив")
			И Лист.Значение.Количество() > 0
			И ЭтоАдресВременногоХранилища(Лист.Значение[0]) Тогда 
			
			ТаблДок = ПолучитьИзВременногоХранилища(Лист.Значение[0]);
			Если ТипЗнч(ТаблДок) = Тип("ТабличныйДокумент") Тогда 
				ТаблДок.Область().ЦветФона = Новый Цвет();
				Лист.Значение[0] = ПоместитьВоВременноеХранилище(ТаблДок, Лист.Значение[0]);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	ЗапрещатьРедактированиеФормРеглОтчетности = Константы.ЗапрещатьРедактированиеФормРеглОтчетности.Получить();
	ДополнительныеПараметрыПечати = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ДополнительныеПараметрыПечати", Новый Структура);
	Если Не ЗапрещатьРедактированиеФормРеглОтчетности И ТипЗнч(ДополнительныеПараметрыПечати) = Тип("Структура") Тогда 
		ТабДокШтампыЭЦП = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеПараметрыПечати, "ТабДокШтампыЭЦП");
		Если ТипЗнч(ТабДокШтампыЭЦП) = Тип("ТабличныйДокумент") Тогда 
			ЗапрещатьРедактированиеФормРеглОтчетности = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	ТабличныйДокумент.Область().Защита   = ЗапрещатьРедактированиеФормРеглОтчетности;
...
#Если ВебКлиент Тогда
	НачатьПолучениеФайлаССервера(АдресаФайловВоВременномХранилище[0].АдресВоВременномХранилище,
...
&НаКлиенте
Процедура СформироватьБланкPDF(АдресБланкаPDFВоВременномХранилище) Экспорт
	
	АдресБланкаPDFВоВременномХранилище = Новый Массив();
	СформироватьPDFФайлыИзВсехЛистовНаСервере(АдресБланкаPDFВоВременномХранилище);
...
Процедура СформироватьТекстПодсказки()
	...
	ТекстСтатистики = НСтр("ru = 'Всего: ';
							|en = 'Total:'") + КоличествоЛистовТребуется + " " + ТекстЛистовТребуется + НСтр("ru = ' бумаги';
																											|en = ' papers'");
...
Процедура СписокЛистовПриАктивизацииСтроки(Элемент)
	...
	ОтключитьОбработчикОжидания("СписокЛистовПриАктивизацииСтрокиЗавершение");
	ПодключитьОбработчикОжидания("СписокЛистовПриАктивизацииСтрокиЗавершение", 0.1, Истина);
КонецПроцедуры

&НаКлиенте
Процедура СписокЛистовПриАктивизацииСтрокиЗавершение() Экспорт 
	Элемент = Элементы.СписокЛистов;
...
Процедура СохранитьВФорматеPDFДокумента(Команда)
	ВыгрузитьНаДискВФормате(ТипФайлаТабличногоДокумента.PDF);
КонецПроцедуры

&НаКлиенте
Процедура СохранитьВФорматеMicrosoftExcel(Команда)
	ВыгрузитьНаДискВФормате(ТипФайлаТабличногоДокумента.XLS);
КонецПроцедуры

&НаКлиенте
Процедура СохранитьВВидеТабличныхДокументов(Команда)
	ВыгрузитьНаДискВФормате(ТипФайлаТабличногоДокумента.MXL);
КонецПроцедуры

&НаКлиенте
Процедура ВыгрузитьНаДискВФормате(ВФормате)
	СохранениеМодификацийВПечатнойФорме();
	...
	Если КоличествоВыбранныхТабличныхДокументов() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите листы в дереве печатаемых листов.';
										|en = 'Select sheets in the printed sheet tree.'"));
		Возврат;
	КонецЕсли;
...
Процедура СформироватьPDFФайлыИзВсехЛистовНаСервере(АдресаФайловВоВременномХранилище)
	...
	Если ПакетТаблДок.Состав.Количество() > 0 Тогда
		...
		АдресаФайловВоВременномХранилище.Добавить(Новый Структура("АдресВоВременномХранилище, Наименование",
			ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ИмяФайла), Новый УникальныйИдентификатор), 
...
&НаКлиенте
Асинх Процедура Подключаемый_ПолучитьФайл()
	ОтключитьОбработчикОжидания("Подключаемый_ПолучитьФайл");
	Если АдресаЛистовВоВременномХранилище.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	КаталогВыгрузки = Ждать РегламентированнаяОтчетностьКлиент.ПолучитьПутьВыгрузкиАсинх();
	Если ЗначениеЗаполнено(КаталогВыгрузки) Тогда 
		Для Каждого Лист Из АдресаЛистовВоВременномХранилище Цикл 
			Ждать ПолучитьФайлССервераАсинх(Лист.АдресВоВременномХранилище, КаталогВыгрузки + Лист.Наименование);
		КонецЦикла;
	КонецЕсли;
	
	АдресаЛистовВоВременномХранилище.Очистить();
КонецПроцедуры
