	...
	ИначеЕсли ИмяСобытия = "Позиционирование в списке отчетов" И ТипЗнч(Параметр) = Тип("Структура") Тогда
		Если ТипЗнч(Параметр.Ссылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если Элементы.ЖурналОтчетов.ТекущаяСтрока <> Параметр.Ссылка Тогда // позиционирование не произошло => мешает отбор
				ВидДокумента = 0;
				Параметр.Свойство("ВидДокумента", ВидДокумента);
				Если ВидДокумента <> -1 Тогда  // при загрузке отчета из файла предупреждение не выводим 
					ПоказатьПредупреждение(,НСтр("ru = 'Введенный отчет не отображен в журнале, так как не соответствует отбору.';
												|en = 'The entered report is not displayed in the log as it does not satisfy the filter.'"));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ОткрытьФормуПодробнееОбОтчете(Команда)
	...
	Если Элементы.ДеревоОтчетов.ТекущиеДанные.ЭтоГруппа = Истина Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Функция недоступна для группы отчетов.';
									|en = 'The function is not available for the report group.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьБланк(Команда)
		...
		Если (ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2023Кв4"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					...
					"ru = '%1 Вывод формы на предпросмотр перед печатью прерван.';
					|en = '%1 Display of the form for a preview before printing is canceled.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
	...
	ИначеЕсли Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "Уведомление об участии в международной группе компаний"
		...
		|en = 'Form for the ""%1"" report cannot be printed without 2D PDF417 barcode.'"),
...
Процедура Выгрузить(Команда)
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
		...
		Если СтрНайти(НаименованиеОтчета, "Прил. 13: Объем собранного винограда для производства винодельческой продукции") > 0
			...
			Если ВыбраннаяФорма = "ФормаОтчета2016Кв2" Тогда
				...
				ТекстПредупреждения = НСтр("ru = 'Проверка выгрузки, выгрузка редакции формы, утвержденной ПП РФ №411 от 13.05.2016, 
												...
												|en = 'Export check, export of the form version confirmed by the Russian Federation Government decree No.411 dated 05/13/2016, 
												|implemented starting from 2016 accounting period.
												|Create a new report for a period from 2016 or later.'");
		...
		Если СтрНайти(НаименованиеОтчета, "Заявление о представлении корректирующих деклараций в ФСРАР.") > 0 Тогда
			...
			ТекстПредупреждения = НСтр("ru = 'Проверка выгрузки, выгрузка и отправка редакции формы, 
												...
												|не предусмотрена.';
												|en = 'You cannot check export, export and send the revision of the form 
												|confirmed by the Order of Ministry of Finance of the Russian Federation No.97n 
												|dated June 20, 2017. '");
			ПоказатьПредупреждение(, ТекстПредупреждения);
			Возврат;
		...
		Если НаименованиеОтчета = "Информация об операциях с денежными средствами НФО" Тогда
			...
			Если ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2022Кв2" Тогда
				...
				|версиях программы';
				|en = 'Online export of the report form
		...
		Если НаименованиеОтчета = "Реестр для налоговых вычетов по акцизам на виноград (КНД 1150116)"
			...
			|en = 'Online export of the report
		...
		Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета
			...
			Если ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2015Кв3" Тогда
				...
				|en = 'Exporting the report to digital format 
				|under the Ordinance of the Government of the Russian Federation
				|#761 dated July 25, 761 is not provided for
		...
		Если (ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2023Кв4"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					...
					|en = '%1  Export is canceled.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета
		...
		Если ОписаниеРегОтчета(Элементы.ЖурналОтчетов.ТекущаяСтрока).ВыбраннаяФорма = "ФормаОтчета2018Кв1" Тогда
			...
			|en = 'Cannot export, check export, print 
			|report ""%1"" form with 2D barcode PDF417,
			|recommended by the letter of FTS of Russia dated 16.01.2019 No. SD-4-3/441@'"),
			Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета);
			Сообщение.Сообщить();
			Возврат;
		КонецЕсли;
	...
	Для каждого Строка Из Элементы.ЖурналОтчетов.ВыделенныеСтроки Цикл
		...
		Если СтрокаДанные.НаименованиеОтчета = "Статистика: Форма П-1" Тогда
			...
			Если НЕ ПроверитьВозможностьВыгрузкиП1ИзЖурналаНаСервере(Строка) Тогда
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для отчета ""Статистика: Форма П-1 за %1"" не выбрано Приложение№3.
					...
					|en = 'Annex No. 3 is not selected for the ""Statistics: Form P-1 for %1"" report.
					|Export from the report form first. Export is canceled.'"), СтрокаДанные.ПредставлениеПериода);
			    Сообщение.Сообщить();
				СтандартнаяОбработка = Ложь;
		...
		Если СтрокаДанные.НаименованиеОтчета = "Статистика: Форма П-1 Приложение 3" Тогда 
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выгрузка отчета ""Статистика: форма П-1 (приложение 3) за %1"" производится из формы П-1 за соответствующий период.';
																							|en = 'The ""Statistics: form P-1 (annex 3) for %1"" report is exported from form P-1 for the corresponding period.'"), СтрокаДанные.ПредставлениеПериода);
			Сообщение.Сообщить();
			СтандартнаяОбработка = Ложь;
...
Процедура ВыгрузитьПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	НСтр("ru = '%1';
		|en = '%1'"), ДополнительныеПараметры.ТекстВопроса), РежимДиалогаВопрос.ДаНет);
...
Процедура ПроверитьВыгрузку(Команда)
	...
	Если НаименованиеОтчета = "Информация об операциях с денежными средствами НФО" Тогда
		...
		Если ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2022Кв2" Тогда
			...
			|отчета будет реализована в будущих
			|версиях программы';
			|en = 'Verification of online export of the report form
			|will be available in future
			|application versions'"));
			Возврат;
		КонецЕсли;
	...
	Если НаименованиеОтчета = "Реестр для налоговых вычетов по акцизам на виноград (КНД 1150116)"
		...
		"ru = 'Проверка выгрузки в электронном виде формы
		|отчета не предусмотрена';
		|en = 'Verification of online export of the report
		|form is not provided'"));
		Возврат;
	КонецЕсли;
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета
		...
		Если ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2015Кв3" Тогда
			...
			|от 25.07.2015 № 761 не предусмотрена
			|законодательством. Форма сдается в бумажном виде';
			|en = 'Reviewing the report exported to digital format
			|under the Ordinance of the Government of the Russian Federation
			|#761 dated July 25, 761 is not provided for
			|by law. The form must be submitted as hard copy'"));
			Возврат;
		...
		Если (ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2023Кв4"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					...
					|en = '%1 Export check is canceled.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
...
Процедура Сохранить(Команда)
	...
	Иначе
		...
		ПоказатьПредупреждение(,НСтр("ru = 'В выбранном документе нет текстов для сохранения в файл.';
									|en = 'There are no texts to save to the file in the selected document.'"));
...
Процедура ОбработатьАктивизациюСтрокиОсновнойТаблицы()
	...
	ИсточникиОтчетовПредставление.Добавить("РегламентированныйОтчетРеестрНДСДекларацииНаТоварыВвозРеэкспорт");
	ИсточникиОтчетовПредставление.Добавить("РегламентированныйОтчетРеестрНДС_КНД1155221");
	ИсточникиОтчетовПредставление.Добавить("РегламентированныйОтчетРеестрНДС_КНД1155223");
	...
	ИсточникиОтчетовПредставление.Добавить("РегламентированныйОтчетРеестрАкцизы_КНД1150151");
	...
	ИсточникиОтчетовПредставление.Добавить("РегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ");
	...
	ИсточникиОтчетовПредставление.Добавить("РегламентированныйОтчетФинОтчетностьМГК");
	...
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма2НаукаИнвКр");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1ТОРГМО");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1КСРОсновныеИндикаторы");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФормаП1Приложение2");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1Разрешение");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1Робототехника");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1Здрав");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма3ИнформМП");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма2МПнаука");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма23Н");
	...
	ИсточникиОтчетовФСНБР.Добавить("РегламентированныйОтчетСтатистикаБанкРоссииФорма1СТР");
...
Функция КалендарьЗаполнен()
	...
	Если График.Количество() = 0 Тогда		
		...
		Сообщение.Текст =НСтр("ru = 'Не заполнен производственный календарь.';
								|en = 'Business calendar is not filled in.'");
		Сообщение.Сообщить();
		Возврат Ложь;	
	КонецЕсли;
...
Процедура ПоказатьСДвухмернымШтрихкодомPDF417(Команда)
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "Платежи за пользование недрами" Тогда
		...
		Если ОписаниеРегОтч.ДатаКонцаПериодаОтчета >= '2022-01-01' Тогда
			...
			|en = 'Printing a report form corresponding to the electronic
			|presentation format version 3.00004 with a 2D PDF417 barcode
			|will be available
			|in future application versions'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета
		= "Движение средств нерезидента за пределами РФ" Тогда
		ПоказатьПредупреждение(, НСтр(
		...
		|en = 'Stamp with a PDF417 barcode of the report form
		|in the edition of the Decree of the Government
		| of the Russian Federation date April 5, 2024 No. 633
		| will be realised in the future versions of the program'"));
		Возврат;
	КонецЕсли;
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "Декларация по УСН" Тогда
		...
		Если ОписаниеУСН.ВыбраннаяФорма = "ФормаОтчета2014Кв1" И ОписаниеУСН.РекомендованнаяФорма Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Печать с двухмерным штрихкодом PDF417 формы, рекомендованной письмом ФНС от 20.05.2015 № ГД-4-3/8533@, не предусмотрена.';
											|en = 'Printing with two-dimensional barcode of 2D PDF417 form recommended in FTS email No. GD-4-3/8533@ dated 05/20/15 is not provided.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
		...
		Если (ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2023Кв4"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
					"ru = '%1 Вывод формы на предпросмотр перед печатью с двухмерным штрихкодом PDF417 прерван.';
					|en = '%1 Form preview before printing with two dimensional barcode 2D PDF417 is canceled.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "Декларация 3-НДФЛ" Тогда
		...
		Если Не Описание3НДФЛ.ДоступнаПечатьPDF417 Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Печать формы со штрихкодом PDF417 будет реализована в будущих версиях программы.';
											|en = 'Printing the form with the barcode PDF417 will be implemented in future versions of the program.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПоказатьРезультатПроверкиОбновленияИнформацииМОС(
	...
	Исключение
		...
		ТекстСообщенияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при проверке результата обновления информации механизма онлайн-сервисов регламентированной отчетности
					 |%1';
					 |en = 'An error occurred while checking the result of updating information on functionality of local reporting online service
		...
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru = 'Регламентированная отчетность. Проверка результата обновления информации механизма онлайн-сервисов регламентированной отчетности';
				|en = 'Local reporting. Check the result of updating the functionality of local reporting online service'"), 
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Обновлены внешние формы отчетов - %1';
							|en = 'External report forms are updated - %1'"),
...
Процедура ОтправкиФТСВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли КолонкаВыбора = "ОтправкиФТСПервичноеСообщение" Тогда
		...
		ПолучитьФайл(Адрес, НСтр("ru = 'Подпись';
								|en = 'Signature'") + ИмяФайлаВыгрузки);
