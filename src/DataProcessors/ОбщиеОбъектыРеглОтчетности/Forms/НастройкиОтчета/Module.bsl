...
Процедура УстановитьПометкуСтрокДерева(Пометка, Знач ТекКолонка)
	...
	Иначе
		...
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для установки или снятия меток по требуемой колонке%1предварительно активизируйте колонку.';
																							|en = 'To select or clear marks of required column %1, activate the column first. '"), Символы.ПС));
