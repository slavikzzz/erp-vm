...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыЗаполнения.Вставить("ДеревоВсехИспользуемыхОтчетовПоКатегориям", ДеревоВсехИспользуемыхОтчетовПоКатегориям);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ТипЗнч(ДанныеНайденногоВидаОтчета) = Тип("Структура") И ДанныеНайденногоВидаОтчета.Свойство("Наименование") Тогда
		Если ЗначениеЗаполнено(ДанныеНайденногоВидаОтчета.Наименование) Тогда
			...
			ПодключитьОбработчикОжидания("Подключаемый_СохранитьНастройки", 0.1, Истина);
...
Процедура ПодробнееОбОтчетеНажатие(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(ТекДеревоОтчетов.ТекущиеДанные.Ссылка) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Для выбранного отчета вывод подробной информации не предусмотрен.';
										|en = 'Detailed information cannot be displayed for the selected report.'"));
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ДеревоОтчетовПриАктивизацииЯчейки(Элемент)
...
Функция ДеревоВсехИспользуемыхОтчетов()
	...
	// см. описание в макете ОписаниеВыпускаНовогоОтчетаВВидеПатча
	РегламентированнаяОтчетность.ДополнитьДеревоОтчетовЭлементамиПатчей(ДеревоВсехИспользуемыхОтчетов);
	...
	Для Инд = 1 По КолСтрок Цикл
		...
		Если СтрокаДерева.ЭтоГруппа <> Истина Тогда
			...
			Если ЗначениеЗаполнено(СтрокаДерева.Ссылка) Тогда
				ЗначениеРеквизита = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаДерева.Ссылка, "ПометкаУдаления");
				СтрокаДерева.ИндексКартинки = ?(ЗначениеРеквизита.ПометкаУдаления, 3, 1);
			КонецЕсли;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(СтрокаДерева.Ссылка) Тогда
				ЗначениеРеквизита = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаДерева.Ссылка, "ПометкаУдаления");
				СтрокаДерева.ИндексКартинки = ?(ЗначениеРеквизита.ПометкаУдаления, 2, 0);
			КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_СохранитьНастройки()
...
Функция ОткрытьФормуОтчетаНаСервере(Знач Отчет, ОрганизацияОтчета
	...
	Если Не ЗначениеЗаполнено(ИсточникОтчета) Тогда 
		ИсточникОтчета = РегламентированнаяОтчетность.ИсточникОтчетаПоСинониму(Отчет);
	КонецЕсли;
...
Процедура ДополнитьДеревоЭлектроннымиПредставлениями(ДеревоВсехИспользуемыхОтчетов, ПараметрыОтбораУОС)
	...
	ДополнитьДеревоШаблонамиXML = Ложь;
	...
	Если ДополнитьДеревоШаблонамиXML Тогда
	...
	Для Каждого СтрокаПервогоУровня Из ДеревоВсехИспользуемыхОтчетов.Строки Цикл
		Если СтрокаПервогоУровня.Строки.Найти(Перечисления.ТипыКонтролирующихОрганов.ФСГС, "ТипПолучателя", Ложь) <> Неопределено Тогда 
			...
			Если Не ДополнитьДеревоШаблонамиXML Тогда
				Продолжить;
			КонецЕсли;
