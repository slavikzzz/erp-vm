...
Процедура ПолучитьВерсииФорматовВыгрузкиФормыОтчета(ДеревоФормИФорматов, ДанныеФормы, СтрВерсииФорматов)
	...
	Если СтрНайти(СтрФорматыВыгрузки, ", ") > 0 Тогда
		СтрВерсииФорматов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'XML-форматы выгрузки электронного представления версий: %1.';
				|en = 'XML formats of exporting electronic version presentation: %1.'"), СтрФорматыВыгрузки);
	Иначе
		СтрВерсииФорматов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'XML-формат выгрузки электронного представления версии: %1.';
				|en = 'XML format of exporting electronic version presentation: %1.'"), СтрФорматыВыгрузки);
	КонецЕсли;
