	...
	Если Модифицированность И Не СохранитьПараметры И НЕ ЗавершениеРаботы Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Параметры были изменены. Закрыть без сохранения параметров?';
				|en = 'Parameters were changed. Close without saving the parameters?'"),
			СписокКнопок);
...
Процедура УстановитьВидимостьДоступность(Форма)
	...
	//++ НЕ УТ
	ЭтоПередачаВПереработку = ЭтоПередачаВПереработку(Форма.КлючОбъекта);
	//-- НЕ УТ
	...
	//++ НЕ УТ
	Если ЭтоПередачаВПереработку Тогда
		
		Форма.Элементы.ПечататьПередачу.Видимость   = Ложь;
		Форма.Элементы.ПечататьРеализацию.Видимость = Ложь;
		Форма.Элементы.ПечататьАкт.Видимость        = Ложь;
...
//++ НЕ УТ
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоПередачаВПереработку(КлючОбъекта)
	
	Возврат Не СтрНайти(КлючОбъекта, "ЖурналДокументовПередачиВПереработку2_5") = 0;
