...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ПереданнаяТара,
		"ПередачаСырьяДоступна",
		ПравоДоступа("Просмотр", Метаданные.Документы.ПередачаСырьяПереработчику));
	//-- Устарело_Переработка24
...
Функция СообщитьОбОшибкахФормированияДанныхЗаполненияВозвратаВыкупаТары(ВыборкаРеквизиты)
	...
	ТекстСообщения = НСтр("ru = 'У выделенных строк в документе поступления отличается поле ""%ПредставлениеПоля%""';
							|en = 'The ""%ПредставлениеПоля%"" field of the selected receipt document lines differs.'");
	...
	Если ВыборкаРеквизиты.ЕстьОтличияЗаказПереработчику Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрЗаменить(ТекстСообщения, "%ПредставлениеПоля%", НСтр("ru = 'Заказ переработчику';
...
Процедура ОформитьДокумент(ТипОперации)
	...
	Если ЕстьПередачи И ЕстьРеализации Тогда
		...
		ТекстСообщения = НСтр("ru = 'Команда не может быть выполнена одновременно для тары, переданной переработчикам, и для тары, переданной клиентам.';
								|en = 'The command cannot be executed simultaneously for the package sent to subcontractors and package transferred to customers.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
		
	ИначеЕсли ЕстьПередачи И ТипОперации = "Заявка" Тогда
		
		ТекстСообщения = НСтр("ru = 'Заявка на возврат тары, переданной переработчикам, не предусмотрена.';
								|en = 'Request for return of package transferred to subcontractors is unavailable.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
...
Процедура УстановитьТекстЗапросаПереданнаяТара()
	...
	|
	|ВЫБРАТЬ
	|	ТИП(Документ.ПередачаСырьяПереработчику) КАК ДоступныйТип
	|ГДЕ
	|	&ПередачаСырьяДоступна
	//-- Устарело_Переработка24
