...
Функция ПериодКОтражениюВРеглУчете(МассивОрганизаций, ОрганичениеПериода) Экспорт
	...
	|		ПО ОтчетыДавальцу.Ссылка = Строки.Регистратор
	//-- НЕ УТКА
	|
	|ГДЕ
	...
	//++ НЕ УТКА
	|		ИЛИ ОтчетыДавальцу.Давалец В(&МассивОрганизаций)
	//-- НЕ УТКА
...
Процедура ТаблицыНастроекСчетов(Параметры, АдресРезультата) Экспорт
	...
	Для каждого ЭлементРазделаУчета Из Параметры.СтруктураТаблиц Цикл
		...
		Если ЭлементРазделаУчета.Значение = Неопределено Тогда
			...
			СписокТекстовЗапросаТекущегоРаздела.Добавить(ТекстЗапроса_ТаблицаНастроекСчетовУчета(ЭлементРазделаУчета.Ключ, НастройкаРаздела), "ТаблицаНастроекСчетовУчета");
		Иначе
