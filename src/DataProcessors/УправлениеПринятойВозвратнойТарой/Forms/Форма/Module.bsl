...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТКА
	ПоступлениеСырьяДоступно = Ложь;
	//++ Устарело_Переработка24
	ПоступлениеСырьяДоступно = ПравоДоступа("Просмотр", Метаданные.Документы.ПоступлениеСырьяОтДавальца);
	//-- Устарело_Переработка24
	
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ПринятаяТара,
		"ПоступлениеСырьяДоступно",
		ПоступлениеСырьяДоступно);
	//-- НЕ УТКА
...
Функция СообщитьОбОшибкахФормированияДанныхЗаполненияВозвратаВыкупаТары(ВыборкаРеквизиты)
	...
	ТекстСообщения = НСтр("ru = 'У выделенных строк в документе приобретения отличается поле ""%ПредставлениеПоля%""';
							|en = 'The ""%ПредставлениеПоля%"" field of the selected purchase document lines differs'");
	...
	Если ВыборкаРеквизиты.ЕстьОтличияЗаказДавальца Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрЗаменить(ТекстСообщения, "%ПредставлениеПоля%", НСтр("ru = 'Заказ давальца';
...
Процедура ОформитьДокумент(ТипОперации)
	...
	Если ЕстьПоступления И ЕстьПоступленияСырья Тогда
		...
		ТекстСообщения = НСтр("ru = 'Команда не может быть выполнена одновременно для тары, полученной от давальцев, и для тары, полученной от поставщиков.';
								|en = 'Command can not be executed simultaneously for package received from processors and vendors.'");
...
Функция ПоместитьТаруВоВременноеХранилищеСервер(МассивСтрок)
	...
	//++ Устарело_Переработка24
	МассивТипов.Добавить(Тип("ДокументСсылка.ПоступлениеСырьяОтДавальца"));
	//-- Устарело_Переработка24
	ТипПоступления = Новый ОписаниеТипов(МассивТипов);
	//-- НЕ УТКА
	...
	Если СообщитьОбОшибкахФормированияДанныхЗаполненияВозвратаВыкупаТары(ВыборкаРеквизиты) Тогда
		...
		ТекстОшибки = НСтр("ru = 'Ввод одного документа по выделенным строкам невозможен';
							|en = 'Cannot enter one document by selected rows'");
	...
	Иначе
		...
		//++ Устарело_Переработка24
		РеквизитыШапки.Вставить("ЗаказДавальца",                 ВыборкаРеквизиты.ЗаказДавальца);
		//-- Устарело_Переработка24
...
Процедура УстановитьТекстЗапросаПринятаяТара()
	...
	|
	|ВЫБРАТЬ
	|	ТИП(Документ.ПоступлениеСырьяОтДавальца) КАК ДоступныйТип
	|ГДЕ
	|	&ПоступлениеСырьяДоступно
	//-- Устарело_Переработка24
