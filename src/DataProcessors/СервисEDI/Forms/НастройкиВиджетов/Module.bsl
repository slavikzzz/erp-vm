...
Процедура УправлениеОтображениемТаблицыВиджетов(Форма)
	...
	Если Форма.ОтображатьТаблицуВиджетов Тогда
		...
		Элементы.ДекорацияУправлениеВидимостьюТаблицыВиджетов.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Скрыть все виджеты';
																											|en = 'Hide all widgets'"),,,,"СкрытьТаблицуВиджетов");
		
	Иначе
		
		Элементы.ГруппаТаблицаВиджетов.Видимость                       = Ложь;
		Элементы.ДекорацияУправлениеВидимостьюТаблицыВиджетов.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Показать все виджеты';
																											|en = 'Show all widgets'"),,,,"ПоказатьТаблицуВиджетов");
...
Процедура УстановитьЗаголовокЧтоВыбираем()
	...
	Если ТекущийВиджет = ПредопределенноеЗначение("Перечисление.ДоступныеВиджетыТекущихДелEDI.Купить") Тогда
		ЗаголовокДекорации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выберите отображаемые команды виджета ""%1""';
																							|en = 'Select the displayed widget ""%1"" commands'"), Строка(ТекущийВиджет));
	Иначе
		ЗаголовокДекорации =СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выберите отображаемые разделы виджета ""%1""';
																						|en = 'Select the displayed widget ""%1"" sections'"), Строка(ТекущийВиджет));;
	КонецЕсли;
