...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДокументыEDIИнтеграция.СформироватьЗаголовокСмотриТакже(ЭтотОбъект, Перечисления.КатегорииДокументовEDI.Закупка);
...
Процедура ДекорацияСмотриТакжеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ДокументыEDIИнтеграцияКлиент.ДекорацияСмотриТакжеОбработкаНавигационнойСсылки(ЭтотОбъект,
	                                                                              ...
	                                                                              ПредопределенноеЗначение("Перечисление.КатегорииДокументовEDI.Закупка"));
...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Для каждого ЭлементКоллекции Из Строки Цикл
		...
		ДанныеДокумента.СостояниеПрикладногоОбъекта                    = ДанныеСтроки.СтатусСоответствия;
...
Процедура СоздатьЗаказПоставщику(Команда)
	...
	ИмяЗаказаПоставщику = ИменаПрикладныхДокументов.Получить(
		ПредопределенноеЗначение("Перечисление.ТипыДокументовEDI.ЗаказПоставщику"));
		
	Если ИмяЗаказаПоставщику = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ОткрытьФорму(СтрШаблон("%1.ФормаОбъекта", ИмяЗаказаПоставщику));
...
Процедура НачатьОбновлениеДанныхИзСервиса()
	...
	ТекстСообщения = НСтр("ru = 'Начато обновление данных документов в сервисе';
							|en = 'Starting document data update in service'");
...
Функция РезультатОбновленияДанныхИзСервисаДлительнаяОперация()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Обновление реестра документов EDI';
															|en = 'Update EDI documents registry'");
...
Процедура ИнициализироватьДанныеФормы()
	...
	ДокументыEDI.ЗаполнитьТаблицуОтбораДокументовЗакупки(ТипыДокументов);
	
	ДокументыEDI.ЗаполнитьСписокВыбораПлановаяДата(ЭтотОбъект);
	
	ОтборПоСтатусамОтображается = Истина;
	
	СоответствиеДокументовEDI = Новый Соответствие;
	Для Каждого ЗаписьПоДокументу Из НастройкиEDI.НастройкиУчета().СоответствиеДокументовEDIПрикладнымОбъектам Цикл
		
		МетаданныеПрикладногоОбъекта = Метаданные.НайтиПоТипу(ЗаписьПоДокументу.Значение);
		Если МетаданныеПрикладногоОбъекта = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		СоответствиеДокументовEDI.Вставить(ЗаписьПоДокументу.Ключ, МетаданныеПрикладногоОбъекта.ПолноеИмя());
	КонецЦикла;
	
	ИменаПрикладныхДокументов = Новый ФиксированноеСоответствие(СоответствиеДокументовEDI);
