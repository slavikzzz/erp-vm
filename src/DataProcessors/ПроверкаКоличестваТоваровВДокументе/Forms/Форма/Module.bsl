...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ордер) Тогда
		ТекстИсключения = НСтр("ru = 'Не предусмотрено непосредственное открытие формы обработки.';
								|en = 'Direct opening of the data processor form is not available.'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.РежимИсправления
		...
		ТекстСообщения = НСтр("ru = 'Для исправления таких строк их нужно сначала разупаковать.';
								|en = 'Unpack such lines to correct them.'");
		ТекстЗаголовка = НСтр("ru = 'Есть строки, включенные в упаковочные листы';
								|en = 'There are lines included in packing lists'");
	...
	Если Не ПропуститьВопросПередЗакрытием
		...
		Если Элементы.СтраницыФормы.ТекущаяСтраница = Элементы.СтраницаОшибки Тогда
			ТекстВопроса = НСтр("ru = 'Отразить расхождения в учете перед закрытием обработки?';
								|en = 'Record discrepancies in accounting before closing the data processor?'");
		Иначе
			ТекстВопроса = ?(Объект.РежимИсправления, НСтр("ru = 'Завершить исправление перед закрытием обработки?';
															|en = 'Finish correction before closing the data processor?'"),
				НСтр("ru = 'Завершить проверку перед закрытием обработки?';
					|en = 'Finish check before closing the data processor?'"));
		КонецЕсли;
...
Процедура ЗаполнитьПоОрдеру(Команда)
	...
	Если НайденныеСтроки.Количество() <> Объект.Товары.Количество() Тогда
		...
		ТекстВопроса = НСтр("ru = 'В табличной части присутствуют строки, в которых заполнено количество.
		                    ...
		                    |en = 'There are rows in the table that contain quantity data.
		                    |This data will be lost during auto population. Continue?'");
		Ответ = Неопределено;
...
Процедура ЗаданиеНаДобор(Команда)
	...
	Если Объект.Недобор.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Невозможно напечатать задание на добор. Количество товаров в ордере соответствует количеству товаров к отгрузке';
								|en = 'Cannot print a collection job. Number of goods in the note corresponds to the quantity of goods pending shipment'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьУпаковочныйЛистВВыделенныхСтроках(Команда)
	...
	Если Элементы.Товары.ВыделенныеСтроки.Количество() = 0 Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Выделите строки, в которых нужно изменить упаковочный лист.';
									|en = 'Select lines in which packing lists should be changed.'");
...
Процедура ИзменитьУпаковочныйЛистВВыделенныхСтрокахЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого СтрМас Из Элементы.Товары.ВыделенныеСтроки Цикл
		...
		Иначе
			ТекстСсобщения = НСтр("ru = 'Строка номер %НомерСтроки% отмечена как неотгружаемая. Неотгружаемые строки не нужно размещать в упаковочные листы.';
									|en = 'Line number %НомерСтроки% is marked as unshipped. Unshipped links should not be placed in packing lists.'");
	...
	ТекстСообщения = НСтр("ru = 'Упаковочный лист измен в %ИзмененоВ% из %Всего%.';
							|en = 'Packing list is changed in %ИзмененоВ% from %Всего%.'");
	...
	Если Объект.РежимИсправления Тогда
		...
		Если НЕ РезультатЗапроса[0].Пустой() Тогда
			ТекстИсключения = НСтр("ru = 'В ордере есть строки с действием ""%Отобрать%"". Исправление такого ордера не возможно. Его нужно проверять.';
									|en = 'Note contains lines with the %Отобрать% action. Cannot correct such note. It must be checked.'");
		...
		Если Выборка.Следующий() Тогда
			Если Не Выборка.ЕстьСтрокиБезУпаковочныхЛистов Тогда
				ТекстИсключения = НСтр("ru = 'Поддерживается исправление только строк, которые не включены в упаковочные листы. В этом ордере таких строк нет. Для исправления нужно исключить строки из упаковочных листов.';
										|en = 'Only those lines can be corrected which are not included in the packing lists. There are no such lines in this note. To correct, exclude the lines from the packing lists.'");
				ТекстИсключения = СтрЗаменить(ТекстИсключения, "%Отобрать%", Перечисления.ДействияСоСтрокамиОрдеровНаОтгрузку.Отобрать);
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
	...
	Иначе
		...
		Если РезультатЗапроса[0].Пустой() Тогда
			ТекстИсключения = НСтр("ru = 'Поддерживается проверка только строк с действием ""%Отобрать%"". В этом ордере таких строк нет.';
									|en = 'Only lines with action ""%Отобрать%"" can be checked. There are no such lines in this note.'");
			ТекстИсключения = СтрЗаменить(ТекстИсключения, "%Отобрать%", Перечисления.ДействияСоСтрокамиОрдеровНаОтгрузку.Отобрать);
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
	КонецЕсли;
		...
		Если ОбработкаОрдераНаПеремещение Тогда
			Если СтатусОрдера <> Перечисления.СтатусыОрдеровНаПеремещение.КОтгрузке Тогда
				ТекстИсключения = НСтр("ru = 'Исправление количества в ордере на перемещение товаров возможно, если он находится в статусе ""К отгрузке"".';
										|en = 'Quantity correction in the internal stock transfer is possible if it is in status ""To ship"".'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
		Иначе
			Если СтатусОрдера <> Перечисления.СтатусыРасходныхОрдеров.Проверен
				...
				ТекстИсключения = НСтр("ru = 'Исправление количества в расходном ордере возможно, если он находится в статусе ""Проверен"", ""К отгрузке"" или ""Отгружен"".';
										|en = 'You can correct the quantity in the goods issue note if its status is ""Checked"", ""To ship"" or ""Shipped"".'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
		КонецЕсли
	Иначе
		Если ?(ОбработкаОрдераНаПеремещение,
			...
			ТекстИсключения = НСтр("ru = 'Проверять количество можно только в ордере, находящемся в статусе ""К проверке"".';
									|en = 'You can check the quantity only in a note with status ""For checking"".'");
...
Процедура ВывестиОписаниеПроблемы(ИзменяемаяСтрока = Неопределено)
	...
	Если ТекущиеДанные.СтатусПроблемы = 0 Тогда
		...
		Если ТекущиеДанные.КоличествоУпаковок = 0
			...
			Если ТекущиеДанные.ЭтоТоварноеМесто
				...
				ТекстСтроки1 = НСтр("ru = 'Количества товарных мест недостаточно для сборки целого количества товара.
					...
					|en = 'Quantity of cargo packagings is insufficient to assemble the full quantity of goods.
					|Quantity of goods to ship is %КоличествоВОрдере% %ЕдиницаИзмерения% less than it is required under the note.'");
			...
			Иначе
				ТекстСтроки1 = НСтр("ru = 'Этот товар еще не сканировали. В ордере %КоличествоВОрдере% %ЕдиницаИзмерения%.';
									|en = 'This goods have not been scanned yet. There is/are %КоличествоВОрдере% %ЕдиницаИзмерения% in note.'");
		...
		Иначе
			...
			ТекстСтроки1 = НСтр("ru = 'Количество товара к отгрузке меньше на %КоличествоНехватка% %ЕдиницаИзмерения%, чем нужно по ордеру.';
								|en = 'Quantity of goods to ship is %КоличествоНехватка% %ЕдиницаИзмерения% less than it is required under the note.'");
			ТекстСтроки2 = "";
	...
	ИначеЕсли ТекущиеДанные.СтатусПроблемы = 1 Тогда
		ТекстСтроки1 = НСтр("ru = 'Отобранное количество товаров совпадает с количеством в ордере.';
							|en = 'Picked quantity of goods matches the quantity in the note.'");
		ТекстСтроки2 = "";
	...
	ИначеЕсли ТекущиеДанные.СтатусПроблемы = 2 Тогда
		ТекстСтроки1 = НСтр("ru = 'Отобранного количества достаточно для отгрузки.';
							|en = 'Picked quantity is enough for shipment.'");
		ТекстСтроки2 = НСтр("ru = '%КоличествоНеотгружать% %ЕдиницаИзмерения% не нужно отгружать.';
							|en = 'It is not required to ship %КоличествоНеотгружать% %ЕдиницаИзмерения%.'");
	...
	ИначеЕсли ТекущиеДанные.СтатусПроблемы = 3 Тогда
		ТекстСтроки1 = НСтр("ru = 'Отложите товар, т.к. его не нужно отгружать.';
							|en = 'Hold the goods over as they should not be shipped.'");
		ТекстСтроки2 = "";
	...
	ИначеЕсли ТекущиеДанные.СтатусПроблемы = 4 Тогда
		...
		Если ТекущиеДанные.КоличествоУпаковокВДокументе > 0
			...
			ТекстСтроки1 = НСтр("ru = 'Отобрано больше, чем требуется по ордеру.';
								|en = 'Picked more than it is required according to the note.'");
			ТекстСтроки2 = НСтр("ru = 'Необходимо для %КоличествоОтобраноБольше% %ЕдиницаИзмерения%';
								|en = 'Required for %КоличествоОтобраноБольше% %ЕдиницаИзмерения%'") + " ";
			ТекстСтроки3 = Новый ФорматированнаяСтрока(НСтр("ru = 'отметить';
															|en = 'mark'"),,,,"НеОтгружать");
			ТекстСтроки4 = НСтр("ru = ', что это количество не нужно отгружать.';
								|en = ', that this quantity does not need to be shipped.'");
		...
		Иначе
			ТекстСтроки1 = НСтр("ru = 'В ордере нет такого товара.';
								|en = 'This product is not found in the note.'");
			...
			Иначе
				ТекстСтроки2 = НСтр("ru = 'Необходимо заполнить количество и';
									|en = 'Fill in quantity and'") + " ";
			КонецЕсли;
			ТекстСтроки3 = Новый ФорматированнаяСтрока(НСтр("ru = 'отметить';
															|en = 'mark'"),,,,"НеОтгружать");
			ТекстСтроки4 = НСтр("ru = ', что эту строку не нужно отгружать.';
								|en = ', that this line does not need to be shipped.'");
		КонецЕсли;
...
Процедура ВывестиПредставлениеТекущегоУпаковочногоЛиста(КодУпаковочногоЛиста)
	...
	Иначе
		ТекстСтроки2 = Новый ФорматированнаяСтрока(НСтр("ru = '<не выбран>';
														|en = '<Not selected>'"),Новый Шрифт(,,Истина));;
	КонецЕсли;
	
	ТекстСтроки3 = Новый ФорматированнаяСтрока(НСтр("ru = 'Изменить (F6)';
													|en = 'Change (F6)'"),,,,"Изменить");
	
	ТекстСтроки4 = Новый ФорматированнаяСтрока(НСтр("ru = 'Напечатать этикетку';
													|en = 'Print label'"),,,,"ПечатьЭтикетки");
	
	ТекстСтроки5 = Новый ФорматированнаяСтрока(НСтр("ru = 'Напечатать упаковочный лист';
													|en = 'Print packing list'"),,,,"ПечатьУпаковочногоЛиста");
...
Процедура ПечатьУпаковочногоЛистаОбработчик()
	...
	Если Не ЗначениеЗаполнено(ТекущийУпаковочныйЛист) Тогда
		ТекстСообщения = НСтр("ru = 'Не выбран текущий упаковочный лист.';
								|en = 'The current packing list is not selected.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = НСтр("ru = 'Закончить наполнение текущего упаковочного листа и напечатать его?';
						|en = 'End filling in the current packing list and print it?'");
...
Процедура ПечатьЭтикеткиУпаковочногоЛистаОбработчик()
	...
	Если Не ЗначениеЗаполнено(ТекущийУпаковочныйЛист) Тогда
		ТекстСообщения = НСтр("ru = 'Для печати этикетки выберите упаковочный лист, перейдя по ссылке ""Изменить"".';
								|en = 'To print the label, select a packing list by clicking ""Change"".'");
		ПоказатьПредупреждение(,ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура НеОтгружатьОбработчик()
	...
	ПараметрыРазбиенияСтроки.Заголовок = НСтр("ru = 'Введите количество, которое не нужно отгружать';
												|en = 'Enter quantity that should not be shipped'");
