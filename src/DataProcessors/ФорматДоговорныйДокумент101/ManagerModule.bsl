...
// 
// Возвращаемое значение:
//  Строка - если имя файла слишком длинное, то сокращается наименование
Функция ОбработанноеИмяФайлаДляОткрытия(ИмяФайла) Экспорт
		
	ДопустимаяДлинаИмениФайла = 100;
	Если СтрДлина(ИмяФайла) > ДопустимаяДлинаИмениФайла Тогда
		Префикс = Лев(ИмяФайла, 9);
		ПостФикс = Строка(Новый УникальныйИдентификатор());
		ИмяФайла = Префикс + "~" + ПостФикс;
	КонецЕсли;
...
Функция ОписаниеДанныхОбъектаУчета(ОбъектУчета, Параметры) Экспорт
	...
	КонструкторЭД.ДанныеФайлаPDF = Параметры.ДанныеФайлаДоговора;
...
Функция СформироватьДокумент(КонструкторЭД) Экспорт
	...
	Если ЗначениеЗаполнено(Документ.Ошибки) Тогда
		Возврат Документ;
	КонецЕсли;
	...
	Если КонструкторЭД.ДанныеФайлаPDF <> Неопределено Тогда
		ПоместитьФайлЭДВФайлПДФ(КонструкторЭД.ДанныеФайлаPDF, Документ.ДанныеОсновногоФайла);
	КонецЕсли;
...
Процедура ЗаполнитьДанныеПодписанта(ДанныеФайлаЭД, ДанныеПодписанта, Ошибки) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДанныеПодписанта.Организация) Или Не ЗначениеЗаполнено(
		...
		ТекстОшибки = НСтр("ru = 'Не удалось заполнить подписанта. Не заполнены организация или сертификат подписи';
							|en = 'Cannot fill the signer. A company or a signature certificate is not specified'");
	...
	Если ЗначениеЗаполнено(ДанныеПодписанта.Доверенность) Тогда
		...
		СведенияДоверенности = МашиночитаемыеДоверенности.ПрочитатьДополнительныеСведенияМЧД(ДанныеПодписанта.Доверенность);
		
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
			ДанныеПодписанта.Доверенность, "ДатаСоздания") Тогда
			РеквизитыДоверенности = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеПодписанта.Доверенность,
				"НомерДоверенности, ДатаВыдачи, ДатаСоздания");
			Подписант.СведенияОбЭлектроннойДоверенности.ДатаВнутренняя = РеквизитыДоверенности.ДатаСоздания;
		Иначе
			РеквизитыДоверенности = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеПодписанта.Доверенность,
				"НомерДоверенности, ДатаВыдачи");
			Подписант.СведенияОбЭлектроннойДоверенности.ДатаВнутренняя = РеквизитыДоверенности.ДатаВыдачи;
		КонецЕсли;
		...
		Подписант.СведенияОбЭлектроннойДоверенности.СведенияОбИСПроверки = 
			СведенияДоверенности.СведенияОбИнформационнойСистеме;
		Подписант.СведенияОбЭлектроннойДоверенности.НомерВнутренний = СведенияДоверенности.ВнутреннийНомер;
...
Функция ДанныеДокументаИзФайлаОбмена(ОписаниеФайла) Экспорт
	...
	#Если Не ВнешнееСоединение Тогда
		...
		// Определяем искомое имя вложения
		ИмяВложенияФайлаФормата = СтрЗаменить(НРег(ОписаниеФайла.ИмяФайла), ".pdf", ".xml");
		...
		Для Каждого Вложение Из СписокВложений Цикл
			...
			Если НРег(Вложение.ИмяФайла) = ИмяВложенияФайлаФормата Тогда
				ЧтениеПДФ.Закрыть();
...
Функция ТекстСправки() Экспорт
	...
	|en = 'A contractual document is composed of two parts:
	|1. A contract file in the PDF format.
	|2. An XML attachment automatically generated from the accounting document data.
	|The PDF file data must match the accounting document data.
	|You can view an attachment file in a PDF document with the standard Adobe Acrobat Reader application. To do it, click Attachments: View attachments.'");
...
//
Процедура ПоместитьФайлЭДВФайлПДФ(ДвоичныеДанныеФайлаПДФ, ДанныеФайла)
...
Функция ЗаписатьКоллекциюВложенийВПДФ(ДвоичныеДанныеФайлаПДФ, КоллекцияВложенийПДФ)
	...
	#Если Не ВнешнееСоединение Тогда
		...
		Исключение
			...
			|en = 'An error occurred when converting the file into the PDF/A3 format.
			|The file might have a password.
			|Select another file or update the platform to a newer version.'");
			
			ТекстСообщения = СтрШаблон(НСтр("ru = '%1
			|Текст ошибки: %2';
			|en = '%1
			|Error text: %2'"), ОбщийТекстОшибки, ПодробныйТекстОшибки);
			...
			|en = '%1
			|(For more information, see the event log).'"), ОбщийТекстОшибки);
		...
		Исключение
			...
			|Выберите другой файл или обновите платформу на более новую версию.';
			|en = 'An error occurred when saving the file attachment.
			|Select another file or update the platform to a newer version.'");
		...
		Если Не ЗаписьВложенийУспешна Тогда
			...
			|Выберите другой файл или обновите платформу на более новую версию.';
			|en = 'An error occurred when adding the file attachment.
			|Select another file or update the platform to a newer version.'");
