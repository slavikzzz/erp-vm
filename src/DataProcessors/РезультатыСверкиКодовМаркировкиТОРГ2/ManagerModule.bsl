...
Процедура ЗагрузитьДанныеДокументаДлительнаяОперация(Параметры, АдресРезультата) Экспорт
	...
	ДанныеДокумента.Вставить("ДокументВнутреннегоПеремещения", ОбщегоНазначенияИСВызовСервера.ЭтоДокументВнутреннегоПеремещения(ДанныеДокумента.ПроверяемыйДокумент));
	...
	Если ДокументОснование = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось определить документ-основание для сравнения кодов маркировки';
								|en = 'Не удалось определить документ-основание для сравнения кодов маркировки'");
	КонецЕсли;
	...
	Иначе
		...
		Если ТаблицаМаркируемойПродукцииДокументаОснования.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ru = 'Документ-основание не содежит маркируемой продукции для сравнения кодов маркировки';
									|en = 'Документ-основание не содежит маркируемой продукции для сравнения кодов маркировки'");
		КонецЕсли;
		...
		Если Не ЭтоПродукцияМОТП Тогда
			...
			Для Каждого ВидПродукции Из ВидыПродукцииДокументаОснования Цикл
				...
				ВидПродукцииПоддерживаетОСУ    = ОбщегоНазначенияИСКлиентСерверПовтИсп.ВидПродукцииПоддерживаетОбъемноСортовойУчет(ВидПродукции, ДанныеДокумента.ДокументВнутреннегоПеремещения);
			...
			Для Каждого КлючИЗначение Из СоответствиеШтрихкодовПоВидуПродукции Цикл
				...
				ИначеЕсли РезультатПолученияДанных = Неопределено Тогда
					ВызватьИсключение НСтр("ru = 'Не удалось получить данные штрихкодов в сервисе ИС МП';
											|en = 'Не удалось получить данные штрихкодов в сервисе ИС МП'");
		...
		Иначе
			...
			Если ДанныеНоменклатурыПоДаннымУПД.ШтрихкодНоменклатураТаблица <> Неопределено Тогда
				...
				Для Каждого СтрокаТаблицыАвтоОСУ Из РезультатПолученияДанныхАвтоОСУ.ДанныеКодовМаркировки Цикл
					ИсходнаяСтрокаДокумента = ДанныеНоменклатурыПоДаннымУПД.ШтрихкодНоменклатураТаблица.Найти(СтрокаТаблицыАвтоОСУ.ДанныеРазбора.КодМаркировки, "ЗначениеШтрихкода");
			...
			ИначеЕсли РезультатПолученияДанных = Неопределено Тогда
				ВызватьИсключение НСтр("ru = 'Не удалось получить данные штрихкодов по документу';
										|en = 'Не удалось получить данные штрихкодов по документу'");
			КонецЕсли;
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Произошла ошибка при сохранении результатов сверки: %1';
				|en = 'Произошла ошибка при сохранении результатов сверки: %1'"),
...
Процедура ВосстановитьДеревоДокументаПоРезультатамПроверки(Параметры, ДанныеДокумента, ОписаниеЗамера, АдресРезультата)
	...
	Пока ВыборкаРесурсовПроверки.Следующий() Цикл
		...
		ВидПродукцииПоддерживаетОСУ    = ОбщегоНазначенияИСКлиентСерверПовтИсп.ВидПродукцииПоддерживаетОбъемноСортовойУчет(ВыборкаРесурсовПроверки.ВидМаркируемойПродукции, ДанныеДокумента.ДокументВнутреннегоПеремещения);
		
		Если ДанныеДокумента.ДокументВнутреннегоПеремещения Тогда
			ДействуетОСУПоДатам = Истина;
		Иначе
			ДействуетОСУПоДатам            = ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПриПродажеОптомПоОСУ(ВыборкаРесурсовПроверки.ВидМаркируемойПродукции)
				И Не ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПриПродажеОптомПоштучно(ВыборкаРесурсовПроверки.ВидМаркируемойПродукции);
		КонецЕсли;
	...
	Если РезультатПолученияДереваИСМП.ЕстьОшибки Тогда
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Ошибка при заполнении формы расхождений для ""Акта о расхождениях при поступлении"".
			           |%1';
			           |en = 'Ошибка при заполнении формы расхождений для ""Акта о расхождениях при поступлении"".
			           |%1'"),
