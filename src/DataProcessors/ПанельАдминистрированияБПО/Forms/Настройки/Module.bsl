...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элемент.Заголовок            = НСтр("ru = 'Использовать распределенную фискализацию';
										|en = 'Use distributed fiscalization'");
	...
	Подсказка = Новый ФорматированнаяСтрока(
		НСтр("ru = 'Распределенная фискализация доступна при использовании подсистемы';
			|en = 'You can use the distributed fiscalization if you use the'")+" ",
		Новый ФорматированнаяСтрока(НСтр("ru = '""Обсуждения""';
										|en = 'Conversations subsystem'"),,,,"Обсуждения"),
		".",
		Символы.ПС,
		НСтр("ru = 'В настройках экземпляра подключаемого оборудования с типом ""ККТ с передачей данных"" должен быть установлен флаг ""Автоматическая фискализация чеков из очереди чеков"".';
			|en = 'In the settings of a peripheral instance with the ""Cash register with data transfer"" type, select the ""Automatic receipt fiscalization from the receipt queue"" check box.'"));
...
Процедура СрокХраненияОперацийПриИзменении(Элемент)
	...
	ИначеЕсли Элемент = Элементы.СрокХраненияОперацийСОборудованием Тогда
		ИзменитьИспользованиеОчисткиОпераций("ОчисткаИсторииОперацийСОборудованием");
	КонецЕсли;
...
Процедура ВыполнитьКоманду(Команда)
	...
	ИначеЕсли Команда = Команды.ОткрытьШаблоныЭтикетокИЦенников Тогда
		...
		ОткрытьФорму(ИмяФормыДляОткрытия, ПараметрыФормы, ПараметрыВыполненияКоманды.Источник, ПараметрыВыполненияКоманды.Уникальность, ПараметрыВыполненияКоманды.Окно);
	ИначеЕсли Команда = Команды.ОткрытьОперацииСОборудованием Тогда
...
Процедура НастроитьРасписаниеОчисткиОпераций(Команда)
	...
	ИначеЕсли Команда = Команды.НастроитьРасписаниеОчисткиОперацийСОборудованием Тогда
		Контекст.ИмяЗадания   = "ОчисткаИсторииОперацийСОборудованием";
		Контекст.ИмяРеквизита = "РасписаниеОчисткиОперацийСОборудованием";
	КонецЕсли;
...
Процедура Очистить(Команда)
	...
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = Элементы.ОчиститьОперацииСОборудованием Тогда
		ИмяФоновогоЗадания = "ОчисткаИсторииОперацийСОборудованием";
	КонецЕсли;
...
Процедура ОбновитьСостояниеСертификатов(РезультатОперации)
	...
	Если РезультатОперации.Результат И ОбщегоНазначенияБПОКлиент.ИспользуетсяСертификатыНУЦМинцифры() Тогда
		...
		Для Каждого Сертификат Из СписокСертификатов Цикл
			Текст = Текст
				+ СтрШаблон(НСтр("ru = '%1 установлен, действителен до %2';
								|en = '%1 is installed. It is valid until %2'") + Символы.ПС,
		...
		ТекстСообщения = Новый ФорматированнаяСтрока(
			НСтр("ru = 'Установка корневых сертификатов Национального удостоверяющего центра Министерства цифрового развития, связи и массовых коммуникаций Российской Федерации.';
				|en = 'Install root certificates of the National Certificate Authority of the Ministry of Digital Development, Communications and Mass Media of the Russian Federation.'"),
			Символы.ПС,
			НСтр("ru = 'Также корневые сертификаты можно скачать с сайта';
				|en = 'You can also download the root certificates from'")+" ",
			Новый ФорматированнаяСтрока(НСтр("ru = '""Госуслуги"".';
											|en = 'Gosuslugi.'"),,,,"https://www.gosuslugi.ru/crt"),
			Символы.ПС,
			Символы.ПС,
			Текст);
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	КонецЕсли;
	#КонецЕсли
	Элементы.ГруппаЛогированиеОперацийПодключаемогоОборудованияОткрытие.Доступность = ЛогированиеОперацийСОборудованием;
...
Процедура УстановитьВидимостьРазделов()
	...
	Элементы.ГруппаУправлениеЖурналамиОпераций.Видимость = Не ОбщегоНазначенияБПО.ЭтоМобильнаяПлатформа();
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяПодсистемаЛогирования() 
		И Не ОбщегоНазначенияБПО.ЭтоМобильнаяПлатформа() Тогда
		Элементы.ГруппаОперацииСПодключаемымОборудованием.Видимость = Истина;
	Иначе
		Элементы.ГруппаОперацииСПодключаемымОборудованием.Видимость = Ложь;
	КонецЕсли;
...
Процедура ИзменитьРасписаниеЗадания(ИмяЗадания, РасписаниеРегламентногоЗадания)
	...
	Если ОбщегоНазначенияБПО.ПодсистемаСуществует("СтандартныеПодсистемы.РегламентныеЗадания") Тогда
		...
		// повторный вызов требуется для обновления информации после изменения информации
		РегЗадание = РегламентноеЗаданиеПоИмени(ИмяЗадания);
		Если РегЗадание = Неопределено Тогда
			Возврат;
		КонецЕсли;
...
Процедура УстановитьТекстНадписиРегламентнойНастройки(Задание, Элемент)
	...
	Если Задание = Неопределено Тогда
		ТекстРасписания = НСтр("ru = '<Расписание очистки не задано>';
								|en = '<Cleanup schedule is not specified>'");
	Иначе
		ТекстЗадания = НСтр("ru = 'Расписание';
							|en = 'Schedule'");
		
		Если Задание.Использование Тогда
			ТекстРасписания = СтрШаблон(НСтр("ru = '%1: %2';
											|en = '%1: %2'"), ТекстЗадания, Строка(Задание.Расписание));
		Иначе
			ТекстРасписания = СтрШаблон(НСтр("ru = '%1 (НЕ АКТИВНО)';
											|en = '%1 (INACTIVE)'"), ТекстЗадания);
		КонецЕсли;
	КонецЕсли;
...
Процедура ИзменитьИспользованиеОчисткиОпераций(ИмяЗадания)
	...
	ИначеЕсли ИмяЗадания = "ОчисткаИсторииОперацийСОборудованием" Тогда
		СрокХранения = СрокХраненияОперацийСОборудованием;
	КонецЕсли;
...
Процедура УстановитьЗначенияРеквизитов()
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяПодсистемаЛогирования() Тогда
		...
		СрокХраненияОперацийСОборудованием = МодульЛогированиеОперацийБПО.ПериодХраненияИсторииОперацийСОборудованием();
		Если Не ЗначениеЗаполнено(СрокХраненияОперацийСОборудованием) Тогда
			СрокХраненияОперацийСОборудованием = Перечисления.ПериодХраненияИсторииПлатежныхОпераций.ВсеВремя;
		...
		ЛогированиеОперацийСОборудованием = ПараметрыЛогирования.ИспользоватьЛогирование;
...
Процедура УстановитьНастройкиЗаданий()
	...
	КонецЕсли;
	Если ОбщегоНазначенияБПО.ИспользуетсяПодсистемаЛогирования() Тогда
		УстановитьНастройкуЗадания("ОчисткаИсторииОперацийСОборудованием", РасписаниеОчисткиОперацийСОборудованием);
	КонецЕсли;
...
&НаКлиенте
Процедура ЛогированиеОперацийСОборудованиемПриИзменении(Элемент)
	
	Подключаемый_ПриИзмененииРеквизита(Элемент);
	Если ОбщегоНазначенияБПОКлиент.ИспользуетсяПодсистемаЛогирования() Тогда
		МодульЛогированиеОперацийБПОКлиент = ОбщегоНазначенияБПОКлиент.ОбщийМодуль("ЛогированиеОперацийБПОКлиент");
		МодульЛогированиеОперацийБПОКлиент.ОбновитьПараметрыЛогированияОперацийСПодключаемымОборудованием();
	КонецЕсли;
...
Функция РегламентноеЗаданиеПоИмени(ИмяЗадания)
	...
	Если ОбщегоНазначенияБПО.ПодсистемаСуществует("СтандартныеПодсистемы.РегламентныеЗадания") Тогда
		...
		Иначе
			ИмяСобытия = НСтр("ru = 'БПО.Панель администрирования';
								|en = 'БПО.Панель администрирования'");
			Комментарий = СтрШаблон(НСтр("ru = 'Ошибка получения регламентного задания %1';
										|en = 'Ошибка получения регламентного задания %1'"), ИмяЗадания);
			ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
				ИмяСобытия, Комментарий, "Ошибка", Метаданные.Обработки.ПанельАдминистрированияБПО);
