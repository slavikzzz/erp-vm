...
Процедура ПроверитьЗавершение(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено
		...
		СервисКриптографииDSSКлиент.ВывестиИнформацию(Неопределено, 
									НСтр("ru = 'Сертификат действителен';
										|en = 'Certificate is valid'", СервисКриптографииDSSСлужебныйКлиент.КодЯзыка()));
	Иначе
...
Процедура СкопироватьСертификатПослеЭкспортаСертификата(РезультатВызова, ДополнительныеПараметры) Экспорт
	...
	Если ЗадатьВопрос И ЭтоСертификат(РежимПросмотра) Тогда
		...
		ПоказатьВопрос(ОповещениеСледующее, 
				НСтр("ru = 'Открыть сертификат на просмотр?';
					|en = 'Do you want to view the certificate?'", СервисКриптографииDSSСлужебныйКлиент.КодЯзыка()),
	...
	Если РезультатВызова.Выполнено Тогда
		СервисКриптографииDSSКлиент.ВывестиИнформацию(Неопределено, 
				НСтр("ru = 'Смена пин-кода выполнена успешно.';
					|en = 'PIN is successfully changed.'", СервисКриптографииDSSСлужебныйКлиент.КодЯзыка()), 30);
	Иначе
		СервисКриптографииDSSКлиент.ВывестиОшибку(Неопределено, РезультатВызова.СтатусОшибки);
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	Иначе
		Заголовок = НСтр("ru = 'Запрос на сертификат';
						|en = 'Request for certificate issuance'");
...
Процедура ЗаполнитьСведенияСертификатаДвоичныеДанные(Сертификат)
	...
	ДобавитьСтроку(НСтр("ru = 'Версия';
						|en = 'Version'", КодЯзыка), Сертификат.Версия, 2);
	ДобавитьСтроку(НСтр("ru = 'Серийный номер';
						|en = 'Serial number'", КодЯзыка), ПолучитьHexСтрокуИзДвоичныхДанных(Сертификат.СерийныйНомер), 2);
	ДобавитьСтроку(НСтр("ru = 'Алгоритм подписи';
						|en = 'Signing algorithm'", КодЯзыка), "неопределено", 2);
	ДобавитьСтроку(НСтр("ru = 'Хеш-алгоритм подписи';
						|en = 'Signature hash algorithm'", КодЯзыка), "неопределено", 2);
	ДобавитьСтроку(НСтр("ru = 'Издатель';
						|en = 'Certificate authority'", КодЯзыка), ПредставлениеИздателя.Поля, 2);
	ДобавитьСтроку(НСтр("ru = 'Субъект';
						|en = 'Subject'", КодЯзыка), ПредставлениеСубъекта.Поля, 2);
	ДобавитьСтроку(НСтр("ru = 'Открытый ключ';
						|en = 'Public key'", КодЯзыка), Сертификат.ОткрытыйКлюч, 2);
	ДобавитьСтроку(НСтр("ru = 'Параметры открытого ключа';
						|en = 'Public key parameters'", КодЯзыка), "неопределено", 2);
	...
	Для каждого СтрокаКлюча Из Сертификат.РасширенныеСвойства Цикл
		...
		ДобавитьСтроку(НСтр("ru = 'Улучшенный ключ';
							|en = 'Enhanced key'", КодЯзыка), ПредставлениеКлюча.Поля, 2);
...
Процедура ЗаполнитьСведенияСертификатаОписание(Сертификат)
	...
	ДобавитьСтроку(НСтр("ru = 'Версия';
						|en = 'Version'", КодЯзыка), Сертификат.Версия, 2);
	ДобавитьСтроку(НСтр("ru = 'Серийный номер';
						|en = 'Serial number'", КодЯзыка), Сертификат.СерийныйНомер, 2);
	ДобавитьСтроку(НСтр("ru = 'Алгоритм подписи';
						|en = 'Signing algorithm'", КодЯзыка), Сертификат.АлгоритмПодписи, 2);
	ДобавитьСтроку(НСтр("ru = 'Хеш-алгоритм подписи';
						|en = 'Signature hash algorithm'", КодЯзыка), Сертификат.АлгоритмОткрытогоКлюча, 2);
	ДобавитьСтроку(НСтр("ru = 'Издатель';
						|en = 'Certificate authority'", КодЯзыка), ПредставлениеИздателя.Поля, 2);
	ДобавитьСтроку(НСтр("ru = 'Субъект';
						|en = 'Subject'", КодЯзыка), ПредставлениеСубъекта.Поля, 2);
	ДобавитьСтроку(НСтр("ru = 'Открытый ключ';
						|en = 'Public key'", КодЯзыка), Сертификат.ОткрытыйКлюч, 2);
	ДобавитьСтроку(НСтр("ru = 'Параметры открытого ключа';
						|en = 'Public key parameters'", КодЯзыка), Сертификат.ПараметрыОткрытогоКлюча, 2);
...
Процедура ЗаполнитьСведенияЗапросаНаСертификат(ЗапросНаСертификат)
	...
	ДобавитьСтроку(НСтр("ru = 'Алгоритм ключа';
						|en = 'Key algorithm'", КодЯзыка), ЗапросНаСертификат.АлгоритмПубличногоКлюча, 2);
	ДобавитьСтроку(НСтр("ru = 'Открытый ключ';
						|en = 'Public key'", КодЯзыка), ЗапросНаСертификат.ОткрытыйКлюч, 2);
