...
Функция ПолучитьПредставлениеТелефона()
	...
	Если НастройкиПользователя <> Неопределено И ЗначениеЗаполнено(НастройкиПользователя.Телефон) Тогда
		Результат = НСтр("ru = 'Временный пароль будет отправлена номер мобильного телефона';
						|en = 'A temporary password will be sent to the cell phone number'", КодЯзыка) + 
						" " + СервисКриптографииDSSКлиентСервер.ПредставлениеТелефона(НастройкиПользователя.Телефон);
	Иначе
		Результат = НСтр("ru = 'Временный пароль будет отправлен на зарегистрированный номер мобильного телефона.';
						|en = 'A temporary password will be sent to the registered cell phone number.'", КодЯзыка);
	КонецЕсли;
...
Функция ПолучитьПредставлениеПочты()
	...
	Если НастройкиПользователя <> Неопределено И ЗначениеЗаполнено(НастройкиПользователя.ЭлектроннаяПочта) Тогда
		Результат = НСтр("ru = 'Временный пароль будет отправлен адрес электронной почты';
						|en = 'A temporary password will be sent to the email address'", КодЯзыка) + 
						" " + СервисКриптографииDSSКлиентСервер.ПредставлениеАдресаПочты(НастройкиПользователя.ЭлектроннаяПочта);
	Иначе
		Результат = НСтр("ru = 'Временный пароль будет отправлен на зарегистрированный адрес электронной почты.';
						|en = 'A temporary password will be sent to the registered email address.'", КодЯзыка);
	КонецЕсли;
