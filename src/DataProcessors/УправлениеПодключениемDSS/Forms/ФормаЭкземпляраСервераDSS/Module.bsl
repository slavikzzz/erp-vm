...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПравоНаИзменения Тогда
		ВызватьИсключение НСтр("ru = 'Пользователь не обладает правами редактирования сведений о сервере DSS.';
								|en = 'The user has insufficient rights to edit DSS details.'");
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если НовыйЭлемент Тогда
		...
		ПоказатьВопрос(ОповещениеСледующее, 
			...
			|en = 'Information on the new DSS server is added.
			|Add a new account for this server?'", 
	...
	ИначеЕсли Изменен Тогда
		...
		ПоказатьВопрос(ОповещениеСледующее, 
			...
			|Обновить?';
			|en = 'DSS server settings changed.
			|You will need to refresh all session parameters.
			|Do you want to refresh the parameters?'", СервисКриптографииDSSСлужебныйКлиент.КодЯзыка()),
			РежимДиалогаВопрос.ДаНет,
			30);
	КонецЕсли;	
...
Асинх Процедура ЗащищенноеСоединениеГОСТПриИзменении(Элемент)
	...
	Если ЗащищенноеСоединениеГОСТ И Не Объект.ЗащищенноеСоединениеГОСТ Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, "8.3.24.0") > 0 Тогда
			...
			|Продолжить?';
			|en = 'For establishing secured connections, we recommend using GOST certificates and certified cryptographic software.
			|Continue?'", 
			СервисКриптографииDSSСлужебныйКлиент.КодЯзыка()),
			РежимДиалогаВопрос.ДаНет,
			30);
...
Процедура РасширенныеНастройки(Команда)
	...
	ПараметрыФормы.Вставить("Заголовок", 
				?(ЗначениеЗаполнено(Объект.Ссылка), 
					...
					НСтр("ru = 'новый элемент';
						|en = 'add item'", СервисКриптографииDSSСлужебныйКлиент.КодЯзыка())));
...
Процедура ПроверитьАдресаСервисов(ОтказСобытия)
	...
	Если НЕ ОтказОтПроверки И Модифицированность Тогда
		...
		Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
			ТекстВопроса = ТекстВопроса 
							+ НСтр("ru = 'Рекомендуемый адрес ЦИ:';
									|en = 'Recommended address for the identification center:'", КодЯзыка)
							+ Символы.ПС 
							+ ПредставлениеОшибки 
							+ Символы.ПС;
		КонецЕсли;	
		...
		Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
			ТекстВопроса = ТекстВопроса 
							+ НСтр("ru = 'Рекомендуемый адрес СЭП:';
									|en = 'Recommended address for the digital signature service:'", КодЯзыка) 
							+ Символы.ПС 
							+ ПредставлениеОшибки 
							+ Символы.ПС;
		КонецЕсли;	
		...
		Если ИдВерсии > 1 Тогда
			...
			Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
				ТекстВопроса = ТекстВопроса 
								+ НСтр("ru = 'Рекомендуемый адрес сервиса обработки документов:';
										|en = 'Recommended address for the document processing service:'", КодЯзыка) 
								+ Символы.ПС 
								+ ПредставлениеОшибки 
								+ Символы.ПС;
			КонецЕсли;	
			...
			Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
				ТекстВопроса = ТекстВопроса 
								+ НСтр("ru = 'Рекомендуемый адрес сервиса аудита:';
										|en = 'Recommended address for the audit service:'", КодЯзыка) 
								+ Символы.ПС 
								+ ПредставлениеОшибки 
								+ Символы.ПС;
			КонецЕсли;	
	...
	Если ЗначениеЗаполнено(ТекстВопроса) Тогда
		ТекстВопроса = НСтр("ru = 'Обнаружено расхождение при заполнении настроек сервера.';
							|en = 'A discrepancy is found on filling in the server settings.'", КодЯзыка) 
						+ Символы.ПС 
						+ ТекстВопроса 
						+ Символы.ПС + НСтр("ru = 'Продолжить?';
											|en = 'Do you want to continue?'", КодЯзыка);
