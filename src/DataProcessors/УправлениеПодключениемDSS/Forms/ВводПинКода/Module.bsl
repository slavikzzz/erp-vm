...
Процедура ПодготовитьФорму(ПараметрыФормы)
	...
	Заголовок = НСтр("ru = 'Доступ к закрытому ключу пользователя';
					|en = 'Access to user private key'", КодЯзыка);
	Если ПараметрыФормы.Свойство("ЗаголовокФормы") Тогда
		ЗаголовокФормы = ПараметрыФормы.ЗаголовокФормы;
	Иначе
		ЗаголовокФормы = НСтр("ru = 'Укажите пин-код к закрытому ключу сертификата';
								|en = 'Specify a PIN to certificate private key'", КодЯзыка);
	КонецЕсли;
