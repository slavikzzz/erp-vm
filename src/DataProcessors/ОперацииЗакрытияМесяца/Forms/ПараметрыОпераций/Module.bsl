...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого КлючИЗначение Из ВсеПараметры Цикл
		...
		ПредставлениеОперации = ?(ЗначениеЗаполнено(КлючИЗначение.Ключ),
			...
			"<" + НСтр("ru = 'Общие параметры всех операций';
						|en = 'General parameters of all operations'", ОбщегоНазначения.КодОсновногоЯзыка()) + ">");
	...
	Если Элементы.Операция.СписокВыбора.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Нет настраиваемых параметров операций закрытия месяца';
																|en = 'No configured parameters of month-end closing operations'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Отказ = Истина;
		Возврат;
	...
	Если НЕ ДанныеСтроки.ЭтоГруппа Тогда
		...
		ПояснениеКПараметру = ПояснениеКПараметру + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Значение по умолчанию: %1';
				|en = 'Default value: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		Если ДанныеСтроки.ДиапазонС <> Неопределено ИЛИ ДанныеСтроки.ДиапазонПо <> Неопределено Тогда
			...
			ПояснениеКПараметру = ПояснениеКПараметру + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Диапазон значений: (%1; %2)';
					|en = 'Value range: (%1; %2)'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
