...
// * ПредшествующиеЭтапы - Массив из ПеречислениеСсылка.ОперацииЗакрытияМесяца -
// * ЗависитОтЭтапов - Массив из ПеречислениеСсылка.ОперацииЗакрытияМесяца -
// * ВлияетНаЭтапы - Массив из ПеречислениеСсылка.ОперацииЗакрытияМесяца -
// * ОбновитьСостояниеЭтапов - Массив из ПеречислениеСсылка.ОперацииЗакрытияМесяца -
...
Функция ИнициализироватьТаблицуОписанияЭтапов() Экспорт
	...
	ТаблицаЭтапов.Колонки.Добавить("НомерЭтапа",	      	  Новый ОписаниеТипов("Число")); // порядковый номер этапа
	...
	ТаблицаЭтапов.Колонки.Добавить("ЗависитОтЭтапов", 		  Новый ОписаниеТипов("Массив")); // расширение свойства ПредшествующиеЭтапы - рекурсивно добавляются все этапы, являются предшествующими
	ТаблицаЭтапов.Колонки.Добавить("ВлияетНаЭтапы", 		  Новый ОписаниеТипов("Массив")); // коды следующих этапов, состояние которых зависит от текущего; по сути это свойство ЗависитОтЭтапов "наоборот"
...
Процедура УпорядочитьЭтапыЗакрытияМесяца(ТаблицаЭтапов)
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		ТекстОшибки = НСтр("ru = 'Некорректно указан порядок операций закрытия месяца';
							|en = 'Incorrect order of month-end close'") + ":" + ТекстОшибки;
	...
	Для Каждого ТекущийЭтап Из ТаблицаЭтапов Цикл
		...
		ТекущийЭтап.НомерЭтапа = НомерЭтапа;
		ТекущийЭтап.ЗависитОтЭтапов.Очистить();
		ТекущийЭтап.ВлияетНаЭтапы.Очистить();
		
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ТекущийЭтап.ЗависитОтЭтапов, ТекущийЭтап.ПредшествующиеЭтапы);
		
		Для Каждого ПредшествующийЭтап Из ТекущийЭтап.ПредшествующиеЭтапы Цикл
			
			СтрокаПредшествующегоЭтапа = ТаблицаЭтапов.Найти(ПредшествующийЭтап, "Код");
			
			Если НЕ ЗначениеЗаполнено(СтрокаПредшествующегоЭтапа)
			 ИЛИ СтрокаПредшествующегоЭтапа.ЗависитОтЭтапов.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли;
			
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ТекущийЭтап.ЗависитОтЭтапов, СтрокаПредшествующегоЭтапа.ЗависитОтЭтапов, Истина);
			
		КонецЦикла;

		Для Каждого ПредшествующийЭтап Из ТекущийЭтап.ЗависитОтЭтапов Цикл
			...
			СтрокаПредшествующегоЭтапа.ВлияетНаЭтапы.Добавить(ТекущийЭтап.Код);
...
// * ОбновляемыеЭтапы - Массив из ПеречислениеСсылка.ОперацииЗакрытияМесяца -
// * ОбновитьТолькоОдинЭтап - ПеречислениеСсылка.ОперацииЗакрытияМесяца -
...
Функция ИнициализироватьПараметрыЗапускаДляЗаполненияСостоянияЭтапов(СтруктураДанных, АдресХранилища = "",
	...
	ПараметрыЗапуска.Вставить("ОбновляемыеЭтапы", 	 		 		 Новый Массив);
...
Процедура ЗаполнитьСостоянияЭтапов(ПараметрыЗапуска) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ПараметрыЗапуска.ОбновляемыеЭтапы) Тогда
		
		СтрокиЭтапов = Новый Массив;
		ЭтапыДляОтбора = Новый Соответствие;
		
		Для Каждого ОбновляемыйЭтап Из ПараметрыЗапуска.ОбновляемыеЭтапы Цикл
			
			ЭтапыДляОтбора.Вставить(ОбновляемыйЭтап, Истина);
			
			Если ПараметрыЗапуска.УчетСостоянияПредшествующихЭтапов Тогда
				
				СтрокаЭтапа = СтруктураДанных.ТаблицаЭтапы.Найти(ОбновляемыйЭтап, "Код");
				Если СтрокаЭтапа = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				Для Каждого ПредшествующийЭтап Из СтрокаЭтапа.ЗависитОтЭтапов Цикл
					ЭтапыДляОтбора.Вставить(ПредшествующийЭтап, Истина);
				КонецЦикла;
		...
		Для Каждого ОбновляемыйЭтап Из СтруктураДанных.ТаблицаЭтапы Цикл
			
			Если ЭтапыДляОтбора.Получить(ОбновляемыйЭтап.Код) <> Неопределено Тогда
				СтрокиЭтапов.Добавить(ОбновляемыйЭтап);
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрыЗапуска.АдресХранилища) Тогда
		...
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Время формирования списка: %1
				|Период: %2
				|Организации: %3';
				|en = 'List generation time: %1
				|Period: %2
				|Companies: %3'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		ЗаписьЖурналаРегистрации(
			ЗакрытиеМесяцаСервер.ИмяСобытияЖурналаРегистрации(НСтр("ru = 'Формировании списка операций';
																	|en = 'Generate operation list'", ОбщегоНазначения.КодОсновногоЯзыка())),
		...
		Если НЕ ЭтапДоступен Тогда
			...
			ЗакрытиеМесяцаСервер.УстановитьСостояниеОтключено(
				...
				НСтр("ru = 'Данный вид учета недоступен.';
					|en = 'This accounting kind is unavailable.'"));
			Возврат;
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При определении состояния операции %1 произошла ошибка:
				|%2';
				|en = 'An error occurred when determining %1 the operation state:
				|%2'"),
	...
	Для Каждого ПредшествующийЭтап Из ТекущаяСтрока.ПредшествующиеЭтапы Цикл
		...
		Если СтруктураДанных.ТаблицаЭтапы.Индекс(СтрокаПредшествующегоЭтапа) >= ИндексТекущейСтроки Тогда
			...
			ТекстОшибки = НСтр("ru = 'Порядок выполнения операций ""%1"" и ""%2"" не совпадает с настройками взаимозависимости операций.';
								|en = 'Procedure of ""%1"" and ""%2"" operations execution does not correspond to the operation interdependence settings.'", ОбщегоНазначения.КодОсновногоЯзыка());
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстОшибки,
				ТекущаяСтрока.Наименование,
			...
			ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не выполнена предшествующая операция ""%1""';
						|en = 'The ""%1"" previous operation is not executed'"),
					СтрокаПредшествующегоЭтапа.Наименование),
					,
					,
			...
			Если НомерПопытки = ПараметрыЗакрытияМесяца.КоличествоПопытокВыполненияПроверки Тогда
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При очистке результатов проверок произошла ошибка:
						|%1';
						|en = 'An error occurred when clearing the check results:
						|%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				...
				Если НомерПопытки = ПараметрыЗакрытияМесяца.КоличествоПопытокВыполненияПроверки Тогда
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'При выполнении проверки ""%1"" произошла ошибка:
							|%2';
							|en = 'An error occurred while checking ""%1"":
							|%2'"),
	...
	Если ТекущаяСтрока.Состояние <> Перечисления.СостоянияОперацийЗакрытияМесяца.НеТребуется Тогда
		...
		Для Каждого СтрокаПояснения Из ТаблицаПроблем Цикл
			...
			НовоеПояснение.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 (%2 шт.)';
					|en = '%1 (%2 pcs.)'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При настройке отображения операции %1 произошла ошибка:
					|%2';
					|en = 'An error occurred when configuring %1 the operation display:
					|%2'"),
				ПараметрыОбработчика.ДанныеЭтапа.Наименование,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПроверитьДатыЗапретаИзменения(СтруктураДанных)
	...
	Если ЕстьОрганизацииБезУчетнойПолитики ИЛИ НЕ ЗначениеЗаполнено(ПериодПолитики) Тогда
		...
		СтруктураДанных.ПоясненияКЗапретуИзменений = НСтр("ru = 'Не заданы учетные политики';
															|en = 'Accounting policies are not specified'");
	...
	ИначеЕсли ЗначениеЗаполнено(ДатаЗапрета) ИЛИ ПериодПолитики > ПериодПроверки Тогда
		...
		СтруктураДанных.ПоясненияКЗапретуИзменений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Расчет возможен с периода %1';
				|en = 'Calculation is possible from the period of %1'"),
...
Функция ОписаниеОрганизацийДляРасчета(Период, БыстраяИнициализация = Ложь) Экспорт
	...
	//++ НЕ УТ
	Если НЕ БыстраяИнициализация Тогда
		МассивОСиНМА = ВнеоборотныеАктивы.ГруппыОрганизацийОСиНМА(Период);
	Иначе
		МассивОСиНМА = Новый Массив;
	КонецЕсли;
	//-- НЕ УТ
...
Функция НазваниеГруппыОрганизацийОСиНМА() Экспорт
	Возврат НСтр("ru = 'Правила учета ОС и НМА';
				|en = 'Fixed assets and intangible assets accounting rules'");
КонецФункции
