			...
			Если Поля.Ключ = "Количество" Тогда
				...
				Если Выборка.ЕдиницаИзмеренияПлан = Выборка.ЕдиницаИзмеренияФакт Тогда
					...
					Если Знак = -1 И Выборка.КоличествоФакт <> Выборка.КоличествоПлан Тогда 
						ВывестиСтрокуРасхождения(НСтр("ru = 'Количество';
														|en = 'Количество'"),ПредставлениеКоличествоПлан,ПредставлениеКоличествоФакт,ПредставлениеРасхождения,ОбластьРасхождение,ТабличныйДокумент);
					ИначеЕсли Знак = 1 И Выборка.КоличествоФакт <> 0 Тогда 
						ВывестиСтрокуРасхождения(НСтр("ru = 'Количество';
														|en = 'Количество'"),ПредставлениеКоличествоПлан,ПредставлениеРасхождения,ПредставлениеКоличествоФакт,ОбластьРасхождение,ТабличныйДокумент);
					КонецЕсли;
				Иначе 
					Массив = Новый Массив;
					Массив.Добавить(Выборка.ЕдиницаИзмеренияФакт);
					Если ЗначениеЗаполнено(Выборка.ЕдиницаИзмеренияПлан) Тогда
