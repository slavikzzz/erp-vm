...
Процедура ОчиститьВсеНастройки(Команда)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Очистить все настройки у пользователя ""%1""?';
			|en = 'Do you want to clear all settings for user ""%1""?'"), Строка(ПользовательСсылка));
...
Процедура ОчиститьУстаревшиеНастройки(Команда)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Очистить устаревшие настройки у пользователя ""%1""?';
			|en = 'Clear obsolete settings for the ""%1"" users?'"), Строка(ПользовательСсылка));
...
Процедура РассчитатьКоличествоНастроек()
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.НастройкиОтчетовСтраница.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Настройки отчетов (%1)';
					|en = 'Report settings (%1)'"), КоличествоНастроек);
	Иначе
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ВнешнийВидСтраница.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Внешний вид (%1)';
					|en = 'Interface settings (%1)'"), КоличествоНастроек);
	Иначе
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ПрочиеНастройкиСтраница.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Прочие настройки (%1)';
					|en = 'Other settings (%1)'"), КоличествоНастроек);
	Иначе
	...
	Если Параметр.КопироватьВсе Тогда
		...
		Если МассивНастроек.Количество() = 7 Тогда
			ПояснениеОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Скопированы все настройки %1';
					|en = 'All settings are copied to %1.'"), ПояснениеКомуСкопированыНастройки);
		Иначе
			ПояснениеОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 скопированы %2';
					|en = '%1 copied to %2'"), ИменаНастроек, ПояснениеКомуСкопированыНастройки);
		КонецЕсли;
...
Процедура ОчиститьНастройкиВсехПользователейЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Очистка настроек';
		...
		НСтр("ru = 'Очищены все настройки всех пользователя';
			|en = 'All settings of all users are cleared.'"), БиблиотекаКартинок.ДиалогИнформация);
...
Процедура ОчиститьУстаревшиеНастройкиВсехПользователейЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Очистка настроек';
										|en = 'Clear settings'"), ,
		НСтр("ru = 'Очищены устаревшие настройки всех пользователя';
			|en = 'Obsolete settings are cleared for all users'"), БиблиотекаКартинок.ДиалогИнформация);
...
Процедура ОчиститьВсеНастройкиЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Очистка настроек';
										|en = 'Clear settings'"), , ТекстПояснения, БиблиотекаКартинок.ДиалогИнформация);
...
Процедура ОповеститьОбУдалении(КоличествоНастроек, НазваниеНастройки = Неопределено, КоличествоПользователей = Неопределено)
	...
	Если КоличествоНастроек = 1
	   И КоличествоПользователей = Неопределено Тогда
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '""%1"" очищена пользователю ""%2""';
				|en = '""%1"" cleared for user ""%2.""'"), НазваниеНастройки, Строка(ПользовательСсылка));
		
	ИначеЕсли КоличествоПользователей = Неопределено Тогда
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Очищено %1 пользователю ""%2""';
				|en = '%1 cleared for user ""%2.""'"), ПрописьПредмета, Строка(ПользовательСсылка));
	КонецЕсли;
	...
	Если КоличествоПользователей <> Неопределено Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '""%1"" очищена %2';
					|en = '""%1"" cleared for %2'"), НазваниеНастройки, ПояснениеКомуОчищатьНастройки);
		Иначе
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Очищено %1 %2';
					|en = '%1 cleared for %2'"), ПрописьПредмета, ПояснениеКомуОчищатьНастройки);
		КонецЕсли;
