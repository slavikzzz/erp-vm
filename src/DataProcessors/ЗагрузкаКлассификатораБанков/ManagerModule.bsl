...
Функция ФайлыКлассификатораПолучены(ИмяКаталога)
	...
	Для Каждого ИмяФайла Из ИменаФайловДляПроверки Цикл
		...
		Если Не Файл.Существует() Тогда
			ЗаписатьОшибкуВЖурналРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найден файл %1';
...
Функция ИмяСобытияВЖурналеРегистрации()
	...
	Возврат НСтр("ru = 'Загрузка классификатора банков.ИПП';
				|en = 'Import bank classifier.OnlineSupport'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Исключение
			...
			ЗаписатьОшибкуВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
					...
					|%3';
					|en = 'Cannot update information records about the ""%1"" settlement participant (BIC %2):
					|%3'"),
					Выборка.Ссылка,
					Выборка.Код,
