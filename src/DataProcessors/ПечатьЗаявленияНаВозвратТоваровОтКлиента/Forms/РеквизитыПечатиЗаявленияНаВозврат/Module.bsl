	...
	Если Модифицированность И Не СохранитьПараметры И НЕ ЗавершениеРаботы Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Реквизиты печати заявления на возврат были изменены. Закрыть без сохранения реквизитов?';
				|en = 'Return request print details were changed. Close without saving the details?'"),
			СписокКнопок);
...
Процедура ЗаполнитьВидДокументаСписокВыбора()
	...
	ТекстПоследнегоЭлементаСписка = ?(Элементы.ВидДокумента.СписокВыбора.Количество() = 1,Нстр("ru = 'Выбрать вид документа';
																								|en = 'Select document kind'"),Нстр("ru = 'Выбрать другой вид документа';
																																	|en = 'Select another document kind'"));
		...
		Если НеобходимВопрос Тогда
			...
			ПоказатьВопрос(
				...
				НСтр("ru = 'Вы хотите записать информацию о данных документа удостоверяющего личность?';
					|en = 'Do you want to save information about ID document?'"),
