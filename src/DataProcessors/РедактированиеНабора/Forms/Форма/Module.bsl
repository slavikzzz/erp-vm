...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(ПараметрыВариантаКомплектацииНоменклатуры) Тогда
		ВызватьИсключение НСтр("ru = 'Состав набора не определен';
								|en = 'The set content is not identified'");
	КонецЕсли;
	...
	Для ТекИндекс = 0 По Объект.Комплектующие.Количество()-1 Цикл
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПричиныОтменыЗаказовКлиентов")
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'Необходимо указать причину отмены в строке %1 списка ""Комплектующие""';
								|en = 'Specify a cancellation reason in line %1 of the Components list'"), НомерСтроки),
				,
		...
		Пока Выборка.Следующий() Цикл
			...
			Иначе
				...
				Если ДоступныеТипыНоменклатуры.Количество() > 0 И ДоступныеТипыНоменклатуры.Найти(Выборка.ТипНоменклатуры) = Неопределено Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Товары с типом %1 не могут быть добавлены в состав данного набора.';
																																	|en = 'Goods with type %1 cannot be added to this set content.'"), Выборка.ТипНоменклатуры));
					Продолжить;
				КонецЕсли;
...
Процедура НазначитьРучнуюСкидку(Команда)
	...
	Если Не СкидкиНаценкиЗаполнениеКлиент.ВозможноНазначениеРучнойСкидкиНаценки(Объект, "Комплектующие", НСтр("ru = 'Комплектующие';
																												|en = 'Components'")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОтменитьРучныеСкидки(Команда)
	...
	Если Не СкидкиНаценкиЗаполнениеКлиент.ВозможнаОтменаРучныхСкидокНаценок(Объект, "Комплектующие", НСтр("ru = 'Комплектующие';
																											|en = 'Components'")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОтменитьВыделенныеСтроки(Команда)
	...
	Если Не ПродажиКлиент.НеобходимоЗаполнениеПричиныОтменыВыделенныхСтрок(Объект.Комплектующие, НСтр("ru = 'Комплектующие';
																										|en = 'Components'"), Элементы.Комплектующие.ВыделенныеСтроки) Тогда
		Возврат;
	КонецЕсли;
	...
	Если Объект.КоличествоНаборов = 0 И Объект.Комплектующие.НайтиСтроки(Отбор).Количество() > 0 Тогда
		...
		Если ВариантРасчетаЦеныНабора = ПредопределенноеЗначение("Перечисление.ВариантыРасчетаЦенНаборов.РассчитываетсяИзЦенКомплектующих") Тогда
			...
			ТекстВопроса = НСтр("ru = 'Основная комплектующая набора не найдена. Отвязать все комплектующие от набора?';
								|en = 'The main component of the set is not found. Unlink all components from the set?'");
