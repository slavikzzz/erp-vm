	...
	Если ЗначениеЗаполнено(СокращенноеНаименованиеКонфигурации) Тогда
		Элементы.ПояснениеСинхронизироватьСвязи.Заголовок = СтрШаблон(
			НСтр("ru = 'Синхронизировать связи объектов 1С:Документооборота и %1';
				|en = 'Synchronize connections of 1C:Document Management and %1 objects'"),
			СокращенноеНаименованиеКонфигурации);
		ПояснениеПравилаИнтеграции = СтрШаблон(
			НСтр("ru = 'Задают соответствия между объектами объектами 1С:Документооборота и %1, а также правила заполнения реквизитов.';
				|en = 'Set correspondence between the 1C:Document Management objects and %1 objects, as well as attribute filling rules.'"),
			СокращенноеНаименованиеКонфигурации);
...
Процедура ДекорацияНастройкиАвторизацииНажатие(Элемент)
	...
	Если Не ЗначениеЗаполнено(НаборКонстант.АдресВебСервиса1СДокументооборот) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не указан адрес веб-сервиса.';
										|en = 'A web service address is not specified.'"));
		Возврат;
	КонецЕсли;
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.СервисДоступен(ВерсияСервиса) Тогда
		...
		Если НомерРедакции = "1" Тогда
			...
			Элементы.ДекорацияОписаниеВерсияНеПоддерживается.Заголовок = НСтр(
				...
				|en = 'Integration with 1C:Document Management (revision 1) is not supported.'");
		...
		ИначеЕсли НомерРедакции = "2" Тогда
			...
			Иначе
				...
				Элементы.ДекорацияОписаниеВерсияНеПоддерживается.Заголовок = НСтр(
					...
					|en = 'Integration with 1C:Document Management (revision 2) is not supported.'");
			КонецЕсли;
		...
		ИначеЕсли НомерРедакции = "3" Тогда
			Если ЕстьПодсистемаДО3 Тогда
				...
				Иначе
					...
					Элементы.ДекорацияОписаниеВерсияНеПоддерживается.Заголовок = НСтр(
						...
						|en = 'Functionality is not supported in this version of 1C:Document Management.
						|Version 3.0.7 or later is required.'");
			...
			Иначе
				...
				Элементы.ДекорацияОписаниеВерсияНеПоддерживается.Заголовок = НСтр(
					...
					|en = 'Integration with 1C:Document Management (revision 3) is not supported.'");
			КонецЕсли;
...
Процедура ОбновитьНастройкиОбновленияСвязанныхРеквизитов(Расписание = Неопределено)
	...
	СтрокаСсылки = Новый ФорматированнаяСтрока(НСтр("ru = 'Изменить расписание';
													|en = 'Change schedule'"),,,, "ИзменитьРасписание");
	...
	Если ПустаяСтрока(ИмяПользователяДляОбмена) Тогда
		ИмяКоманды = НСтр("ru = 'Задать служебного пользователя для обмена';
							|en = 'Set a utility user for exchange'");
	Иначе
		...
		КонецЕсли;
		ИмяКоманды = СтрШаблон(НСтр("ru = 'Изменить пользователя (%1)';
									|en = 'Change a user (%1)'"), ИмяПользователяДляОбмена);
	КонецЕсли;
...
Функция ОбъектыПоддерживающиеСогласование()
	...
	//++ НЕ УТ
	ОбъектыПоддерживающиеСогласование.Добавить(Метаданные.Справочники.ДоговорыАренды);
	//-- НЕ УТ
	...
	//++ НЕ УТКА
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыЗаказовДавальцев2_5") Тогда
		ОбъектыПоддерживающиеСогласование.Добавить(Метаданные.Документы.ЗаказДавальца2_5);
	КонецЕсли;
	//-- НЕ УТКА
	
	//++ НЕ УТ
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыЗаказовПереработчикам2_5") Тогда
		ОбъектыПоддерживающиеСогласование.Добавить(Метаданные.Документы.ЗаказПереработчику2_5);
	КонецЕсли;
	//-- НЕ УТ
...
Функция ОбъектыПоддерживающиеУтверждение()
	...
	//++ НЕ УТ
	ОбъектыПоддерживающиеУтверждение.Добавить(Метаданные.Документы.ПланПроизводства);
	//-- НЕ УТ
	
	ОбъектыПоддерживающиеУтверждение.Добавить(Метаданные.Документы.ПланСборкиРазборки);
	
	//++ НЕ УТКА
	ОбъектыПоддерживающиеУтверждение.Добавить(Метаданные.Документы.РазрешениеНаЗаменуМатериалов);
	//-- НЕ УТКА
	
	//++ НЕ УТ
	ОбъектыПоддерживающиеУтверждение.Добавить(Метаданные.Документы.ЭкземплярБюджета);
	//-- НЕ УТ
...
Процедура ЗаполнитьПояснениеИспользоватьСогласованиеЧерезДО(СокращенноеНаименованиеКонфигурации)
	...
	МассивСтрокПояснениеИспользоватьСогласованиеЧерезДО.Добавить(
		НСтр("ru = 'Использование 1С:Документооборота для изменения статусов согласования и утверждения документов.';
			|en = 'Use 1C:Document Management to change the document approval and confirmation statuses.'"));
	...
	МассивСтрокПояснениеИспользоватьСогласованиеЧерезДО.Добавить(СтрШаблон(
		НСтр("ru = 'Выключает внутренний механизм согласования %1.';
			|en = 'Disables the internal %1 approval procedure.'"),
		СокращенноеНаименованиеКонфигурации));
	МассивСтрокПояснениеИспользоватьСогласованиеЧерезДО.Добавить(Символы.ПС);
	МассивСтрокПояснениеИспользоватьСогласованиеЧерезДО.Добавить(
		Новый ФорматированнаяСтрока(НСтр("ru = 'Список объектов, поддерживающих изменение статуса.';
										|en = 'The list of objects whose status can be changed.'"),,
			Элементы.ПояснениеСрокХраненияСообщений.ЦветТекста,,
...
Функция ТабличныйДокументСписокОбъектовПоддерживающихИзменениеСтатуса(РедакцияДО)
	...
	Ячейча = Секция.Область("R1C1");
	Ячейча.ШиринаКолонки = 87;
	
	ТабличныйДокумент.НачатьАвтогруппировкуСтрок();
	
	Ячейча.Текст = НСтр("ru = 'Объекты, поддерживающие согласование через 1С:Документооборот';
						|en = 'Objects that can be approved via 1C:Document Management'");
	Ячейча.Расшифровка = Неопределено;
	Ячейча.Шрифт = ШрифтыСтиля.КрупныйШрифтТекста;
	Ячейча.Отступ = 0;
	Если РедакцияДО = 2 Тогда
		Ячейча.Примечание.Текст = НСтр(
			...
			|en = 'In the integration rules, you can configure filling in the attribute that contains the status from the ""Approval state"" attribute of the 1C:Document Management object'");
	ИначеЕсли РедакцияДО = 3 Тогда
		Ячейча.Примечание.Текст = НСтр(
			...
			|en = 'In the integration rules, you can configure filling in the attribute that contains the status from the approval action state of the 1C:Document Management object'");
	КонецЕсли;
	...
	Для Каждого ОбъектМетаданных Из ОбъектыПоддерживающиеСогласование() Цикл
		ВывестиОбъектВТабличныйДокумент(ОбъектМетаданных, ТабличныйДокумент, Секция, Ячейча, "Согласование", РедакцияДО);
	КонецЦикла;
	...
	Ячейча.Текст = НСтр("ru = 'Объекты, поддерживающие утверждение через 1С:Документооборот';
						|en = 'Objects that can be confirmed via 1C:Document Management'");
	Ячейча.Расшифровка = Неопределено;
	Ячейча.Шрифт = ШрифтыСтиля.КрупныйШрифтТекста;
	Ячейча.Отступ = 0;
	Если РедакцияДО = 2 Тогда
		Ячейча.Примечание.Текст = НСтр(
			...
			|en = 'In the integration rules, you can configure filling in the attribute that contains the status from the ""Confirmation state"" attribute of the 1C:Document Management object'");
	ИначеЕсли РедакцияДО = 3 Тогда
		Ячейча.Примечание.Текст = НСтр(
			...
			|en = 'In the integration rules, you can configure filling in the attribute that contains the status from the confirmation action state of the 1C:Document Management object'");
	КонецЕсли;
	...
	Для Каждого ОбъектМетаданных Из ОбъектыПоддерживающиеУтверждение() Цикл
		ВывестиОбъектВТабличныйДокумент(ОбъектМетаданных, ТабличныйДокумент, Секция, Ячейча, "Утверждение", РедакцияДО);
	КонецЦикла;
...
&НаСервереБезКонтекста
Процедура ВывестиОбъектВТабличныйДокумент(ОбъектМетаданных, ТабличныйДокумент, Секция, Ячейча, ИмяОбласти, РедакцияДО)
	
	Ячейча.Текст = СтрШаблон("%1: %2",
	...
	Ячейча.Шрифт = ШрифтыСтиля.ОбычныйШрифтТекста;
	Ячейча.Отступ = 3;
	Ячейча.Примечание.Текст = "";
	
	Если РедакцияДО = 2 Тогда
		Ячейча.Расшифровка = "e1cib/list/Справочник.ПравилаИнтеграцииС1СДокументооборотом";
	ИначеЕсли РедакцияДО = 3 Тогда
		Ячейча.Расшифровка = ПолучитьНавигационнуюСсылку(
