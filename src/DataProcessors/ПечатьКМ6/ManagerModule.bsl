...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КМ6") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "КМ6", НСтр("ru = 'Отчет за смену';
																								|en = 'Shift report'", ОбщегоНазначения.КодОсновногоЯзыка()), СформироватьПечатнуюФормуКМ6(СтруктураТипов, ОбъектыПечати, ПараметрыПечати));
		ПараметрыВывода.Вставить("ЗаголовокФормы", НСтр("ru = 'Справка отчет кассира-операциониста (КМ-6)';
														|en = 'Cashier report (KM-6)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
		...
		Если НЕ (СтруктураОбъектов.Ключ = "Документ.ОтчетОРозничныхПродажах"
			...
			Для Каждого Документ Из СтруктураОбъектов.Значение Цикл
				...
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для документа %1 печать Справки отчета кассира-операциониста (КМ-6) не требуется.';
						|en = 'Printing of the cashier report (KM-6) is not required for document %1.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					Документ.Ссылка);
