	...
	Если МассивОбъектов.Количество() > 0 Тогда
		...
		Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПодробныйРасчетНачислений") Тогда
			...
			Если НЕ ТабличныйДокумент = Неопределено Тогда
				УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ПодробныйРасчетНачислений", НСтр("ru = 'Подробный расчет начислений';
																																|en = 'Detailed accrual calculation'"), ТабличныйДокумент);
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьТабличныйДокументПодробногоРасчетаНачислений(ПараметрыПечати, ДанныеДляПечати, ОбъектыПечати, ИмяМакета, ТабличныйДокумент)
	...
	Для Каждого Сотрудник Из ДанныеДокумента.МассивСотрудников Цикл
		...
		ЗначенияПараметров.Вставить("НомерДокумента",
			ЗарплатаКадрыОтчеты.НомерНаПечать(ДанныеДокумента.НомерДокумента, ДанныеДокумента.НомерПервичногоДокумента));
		...
		Если ДанныеДляПечати.ЕстьРасчетСреднегоЗаработка Тогда
			...
			Если НЕ ИмяОбластиСреднегоЗаработка = Неопределено Тогда
				...
				ВывестиСекциюИндексацияЗаработка(ТабличныйДокумент, ОбластиМакета.СекцияИндексацияЗаработка, ДанныеДляПечати.Показатели);
			...
			Если ДанныеДляПечати.ЕстьРасчетЗарплаты Тогда
				...
				Если НачисленияСекцииПоСотруднику.Количество() > 0 Тогда
					НазваниеСекции = НСтр("ru = 'Расчет оплаты труда';
											|en = 'Labor compensation calculation'");
					ВывестиСекциюНачислений(ТабличныйДокумент, ОбластиМакета, НомерРаздела, НачисленияСекцииПоСотруднику, ДанныеДляПечати.Показатели, НазваниеСекции);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ДанныеДокументовДляПодробногоРасчетаНачислений(Менеджер, МассивСсылок)
	...
	ДанныеДокументов.Колонки.Добавить("НомерПервичногоДокумента");
...
Функция ОбластиМакетаПодробногоРасчетаНачислений()
	...
	ОбластиМакета.Вставить("СекцияИндексацияЗаработка",  	Макет.ПолучитьОбласть("ИндексацияЗаработка"));
...
Процедура ВывестиСекциюИндексацияЗаработка(ТабличныйДокумент, СекцияИндексацияЗаработка, Показатели)
	
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьИндексациюЗаработка") Тогда 
		Возврат;
	КонецЕсли;
	
	ПоказательКоэффициентИндексации = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоэффициентИндексацииСреднегоЗаработка");
	Если ПоказательКоэффициентИндексации = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	
	СтрокаПоказателя = Показатели.Найти(ПоказательКоэффициентИндексации, "Показатель");
	Если СтрокаПоказателя = Неопределено Или СтрокаПоказателя.Значение = 1 Тогда 
		Возврат;
	КонецЕсли;
	
	СекцияИндексацияЗаработка.Параметры.КоэффициентИндексации = СтрокаПоказателя.Значение;
	
	ТабличныйДокумент.Вывести(СекцияИндексацияЗаработка);
