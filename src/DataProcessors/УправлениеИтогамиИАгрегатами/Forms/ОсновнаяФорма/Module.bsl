...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ru = 'Управление итогами и агрегатами недоступно в модели сервиса.';
								|en = 'Totals and aggregate management is unavailable in SaaS.'");
	КонецЕсли;
...
Процедура ПересчитатьТекущиеИтоги(Команда)
	...
	ПараметрыИтогов.Вставить("ОбрабатыватьЗаголовок",  НСтр("ru = 'Пересчет текущих итогов ...';
															|en = 'Recalculating the current totals …'"));
		...
		Иначе
			Результат.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 (регистр накопления)';
					|en = '%1 (accumulation register)'"),	ИмяРегистра);
		КонецЕсли;
	...
	Иначе
		...
		Результат.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Регистры накопления (%1)';
				|en = 'Accumulation registers (%1)'"), Успешно);
...
Функция УстановитьПараметрыРегистраНаСервере(Знач ВидРегистра,
	...
	Попытка
		...
		Иначе
			ВызватьИсключение НСтр("ru = 'Неправильное имя параметра';
									|en = 'Incorrect partner name'") + "(1): " + Действие;
		КонецЕсли;
