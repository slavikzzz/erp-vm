...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Состояние <> Перечисления.СостоянияМиграцииПриложения.Выполняется Тогда
		...
		Наименование = СтрШаблон(НСтр("ru = '%1 (Миграция приложения)';
										|en = '%1 (Application migration)'"), ВидыПриложенийСервер.ПредставлениеТекущегоВидаПриложения());
	...
	Если Не ЗначениеЗаполнено(Наименование) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не заполнено поле ""Наименование""';
				|en = 'The ""Name"" field is not filled in'"), , "Наименование", , Отказ);
	КонецЕсли;
...
Процедура СтраницаСозданиеПриложениеОткрытие(ЭтоПереходНазад)
	...
	Заголовок = СтрШаблон(НСтр("ru = 'Шаг %1 из %1: Параметры создания приложения';
								|en = 'Step %1 out of %1: Application creation parameters'"), КоличествоШагов);
...
Процедура СтраницаОжиданиеОткрытие(ЭтоПереходНазад)
	...
	Строки.Добавить(ПС);
	Строки.Добавить(СтрШаблон(НСтр("ru = 'Код абонента: %1';
									|en = 'Subscriber code: %1'"), КодАбонента) + ПС);
	Строки.Добавить(СтрШаблон(НСтр("ru = 'Код приложения: %1';
									|en = 'Application code: %1'"), КодПриложения) + ПС);
	Строки.Добавить(НСтр("ru = 'Контакты обслуживающей организации';
						|en = 'Service company contacts'") + ":" + ПС);
	Строки.Добавить(Таб);
	Строки.Добавить(Таб + СтрШаблон(НСтр("ru = 'e-mail: %1';
										|en = 'e-mail: %1'"), ОбслуживающаяОрганизацияЭлектроннаяПочта) + ПС);
	Строки.Добавить(Таб);
	Строки.Добавить(Таб + СтрШаблон(НСтр("ru = 'тел.: %1';
										|en = 'phone: %1'"), ОбслуживающаяОрганизацияТелефон) + ПС);
...
Процедура ОтменитьПереход()
	...
	Комментарий = НСтр("ru = 'Переход в сервис отменен';
						|en = 'Transfer to service canceled'");
...
Функция СостояниеПерехода()
	...
	Если (СостояниеПерехода.Состояние = Перечисления.СостоянияМиграцииПриложения.Выполняется 
		...
		ИначеЕсли ОставшеесяВремя <= 3600 Тогда
			СостояниеПерехода.ОсталосьВремени = СтрШаблон(НСтр("ru = '~ %1 мин.';
																|en = '~ %1 min.'"), Окр(ОставшеесяВремя / 60));
		Иначе
			СостояниеПерехода.ОсталосьВремени = СтрШаблон(НСтр("ru = '~ %1 ч.';
																|en = '~ %1 h.'"), Окр(ОставшеесяВремя / 3600, 1));
		КонецЕсли;
