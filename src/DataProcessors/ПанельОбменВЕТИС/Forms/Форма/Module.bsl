...
Процедура ОбновитьСпискиДокументов()
	...
	ДобавитьДокумент(
		...
		НСтр("ru = 'Запросы складского журнала ВетИС';
			|en = 'Запросы складского журнала ВетИС'"),
		Ложь,    // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
	...
	Если Проблемы.Количество() > 0 Тогда
		...
		Иначе
			ПерваяПроблема = НСтр("ru = 'Рассинхронизация обмена по записям складского журнала и ветеринарно-сопроводительным документам';
									|en = 'Рассинхронизация обмена по записям складского журнала и ветеринарно-сопроводительным документам'");
			Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаКрасный;
		КонецЕсли;
	КонецЕсли;
	...
	Если Проблемы.Количество() > 0 Тогда
		...
		Иначе
			ПерваяПроблема = НСтр("ru = 'Есть ошибки APLM0012 при синхронизации ветеринарно-сопроводительных документов';
									|en = 'Есть ошибки APLM0012 при синхронизации ветеринарно-сопроводительных документов'");
			Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаКрасный;
		КонецЕсли;
	КонецЕсли;
	...
	Если Проблемы.Количество() > 0 Тогда
		...
		Иначе
			ПерваяПроблема = НСтр("ru = 'Есть ошибки APLM0012 при синхронизации записей складского журнала';
									|en = 'Есть ошибки APLM0012 при синхронизации записей складского журнала'");
			Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаКрасный;
		КонецЕсли;
	КонецЕсли;
	...
	Если Проблемы.Количество() > 0 Тогда
		...
		Иначе
			ПерваяПроблема = НСтр("ru = 'Есть проблемы с синхронизацией ветеринарно-сопроводительных документов';
									|en = 'Есть проблемы с синхронизацией ветеринарно-сопроводительных документов'");
	...
	Если Проблемы.Количество() > 0 Тогда
		...
		Иначе
			ПерваяПроблема = НСтр("ru = 'Есть проблемы с синхронизацией записей складского журнала';
									|en = 'Есть проблемы с синхронизацией записей складского журнала'");
			Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаЖелтый;
		КонецЕсли;
	КонецЕсли;
	...
	Если Проблемы.Количество() > 0 Тогда
		...
		Иначе
			ПерваяПроблема = НСтр("ru = 'Есть проблемы с отправкой документов';
									|en = 'Есть проблемы с отправкой документов'");
			Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаКрасный;
		КонецЕсли;
	КонецЕсли;
	...
	Если Проблемы.Количество() > 0 Тогда
		...
		Иначе
			ПерваяПроблема = НСтр("ru = 'Ветеринарно-сопроводительные документы не актуальны';
									|en = 'Ветеринарно-сопроводительные документы не актуальны'");
			Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаЖелтый;
		КонецЕсли;
	КонецЕсли;
	...
	Если Проблемы.Количество() > 0 Тогда
		...
		Иначе
			ПерваяПроблема = НСтр("ru = 'Записи складского журнала не актуальны';
									|en = 'Записи складского журнала не актуальны'");
			Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаЖелтый;
		КонецЕсли;
	КонецЕсли;
	...
	Если Проблемы.Количество() > 0 Тогда
		...
		Иначе
			ПерваяПроблема = НСтр("ru = 'Требуется загрузка элементов классификаторов';
									|en = 'Требуется загрузка элементов классификаторов'");
			Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаКрасный;
		КонецЕсли;
	КонецЕсли;
		...
		Иначе
			...
			ПерваяПроблема = СтрШаблон(
				НСтр("ru = 'Есть сообщения (%1), ожидающие отправки. Выполните обмен.';
					|en = 'Есть сообщения (%1), ожидающие отправки. Выполните обмен.'"),
				Проблемы.Количество);
	...
	Если ОбменДаннымиВЕТИС.ЕстьПроблемыОтправкиПолученияДанныхВЕТИС(ОрганизацииВЕТИС) Тогда
		...
		Иначе
			ПерваяПроблема = НСтр("ru = 'Имеются проблемы обменов данными с ВетИС';
									|en = 'Имеются проблемы обменов данными с ВетИС'");
			Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаЖелтый;
		КонецЕсли;
	КонецЕсли;
			...
			Если Не ЗаданиеВключено Тогда
				...
				Иначе
					ПерваяПроблема = НСтр("ru = 'Рекомендуется настроить свертку регистра соответствия номенклатуры ВетИС';
											|en = 'Рекомендуется настроить свертку регистра соответствия номенклатуры ВетИС'");
					Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаЖелтый;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбновитьПредставлениеЛогирования()
	...
	ДанныеПодсказки.Добавить(НСтр("ru = 'техническую информацию по взаимодействию с ВетИС';
									|en = 'техническую информацию по взаимодействию с ВетИС'"));
	ДанныеПодсказки.Добавить(" ");
