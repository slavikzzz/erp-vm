			...
			НадписьСообщениеОбОшибкеПоОбъектуЗаголовок.Добавить(
				Новый ФорматированнаяСтрока(
					НСтр("ru = 'API-шлюз ФГИС Меркурий перегружен, попытки получения ВСД и записей 
					         ...
					         |en = 'API-шлюз ФГИС Меркурий перегружен, попытки получения ВСД и записей 
					         |складского журнала приводят к ошибке'")));
			НадписьСообщениеОбОшибкеПоОбъектуЗаголовок.Добавить(" ");
			НадписьСообщениеОбОшибкеПоОбъектуЗаголовок.Добавить(
				Новый ФорматированнаяСтрока(
				...
				Новый ФорматированнаяСтрока(
					СтрШаблон(
						НСтр("ru = '%1 записей складского журнала';
							|en = '%1 записей складского журнала'"),
			...
			НадписьСообщениеОбОшибкеПоОбъектуЗаголовок.Добавить(
				Новый ФорматированнаяСтрока(
					НСтр("ru = 'в ручном режиме с помощью сканера штрихкодов (внешнего файла)
					         ...
					         |en = 'в ручном режиме с помощью сканера штрихкодов (внешнего файла)
					         |или попробуйте загрузить'")));
			НадписьСообщениеОбОшибкеПоОбъектуЗаголовок.Добавить(" ");
			НадписьСообщениеОбОшибкеПоОбъектуЗаголовок.Добавить(
				Новый ФорматированнаяСтрока(
					НСтр("ru = 'непогашенные ВСД за период';
						|en = 'непогашенные ВСД за период'"),,,,
			...
			НадписьСообщениеОбОшибкеПоОбъектуЗаголовок.Добавить(
				Новый ФорматированнаяСтрока(
					НСтр("ru = 'При передаче запроса по документу';
						|en = 'При передаче запроса по документу'")));
			НадписьСообщениеОбОшибкеПоОбъектуЗаголовок.Добавить(" ");
			НадписьСообщениеОбОшибкеПоОбъектуЗаголовок.Добавить(
				Новый ФорматированнаяСтрока(
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ХранитьФайлыВТомахНаДиске" Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru = 'Вам запрещено выполнение данной операции';
											|en = 'Вам запрещено выполнение данной операции'"));
		КонецЕсли;
		...
		Элементы[ИмяЭлемента].Заголовок = Новый ФорматированнаяСтрока(
			СтрШаблон(
				НСтр("ru = 'Есть ошибки APLM0012 при синхронизации ветеринарно-сопроводительных документов (%1)';
					|en = 'Есть ошибки APLM0012 при синхронизации ветеринарно-сопроводительных документов (%1)'"),
				Выборка.Количество()),,,,
			ИдентификаторПроблемы);
		...
		Элементы[ИмяЭлемента].Заголовок = Новый ФорматированнаяСтрока(
			СтрШаблон(
				НСтр("ru = 'Есть ошибки APLM0012 при синхронизации записей складского журнала (%1)';
					|en = 'Есть ошибки APLM0012 при синхронизации записей складского журнала (%1)'"),
				Выборка.Количество()),,,,
			ИдентификаторПроблемы);
...
Процедура ПроверитьХранениеФайловВТомахНаДиске()
	...
	Если Элементы["Группа" + ИмяЭлемента].Видимость Тогда
		...
		ТекстПодсказки = НСтр("ru = 'Файлы обмена могут занимать значительный объем данных в базе.
			...
			|en = 'Файлы обмена могут занимать значительный объем данных в базе.
			|Для уменьшения объема базы данных, файлы необходимо хранить в томах на диске.'");
		...
		Элементы[ИмяЭлемента].Заголовок = Новый ФорматированнаяСтрока(
			СтрШаблон(
				НСтр("ru = 'Есть сообщения ожидающие отправки (%1)';
					|en = 'Есть сообщения ожидающие отправки (%1)'"),
				Выборка.Количество),,,,
			ИдентификаторПроблемы);
	...
	Если ОбменДаннымиВЕТИС.ЕстьПроблемыОтправкиПолученияДанныхВЕТИС(ОрганизацииВЕТИС) Тогда
		...
		ТекстЗаголовка = Новый ФорматированнаяСтрока(
			НСтр("ru = 'Рекомендуется настроить регламентные задания для обмена с ВетИС';
				|en = 'Рекомендуется настроить регламентные задания для обмена с ВетИС'"),,,,
		...
		ТекстПодсказки = НСтр("ru = 'Некорректные настройки могут привести к значительному увеличению времени выполнения обмена.';
								|en = 'Некорректные настройки могут привести к значительному увеличению времени выполнения обмена.'");
		...
		Если Элементы.ГруппаРегламентноеЗаданиеСверткаРегистраСоответствиеНоменклатурыВЕТИС.Видимость Тогда
			...
			ТекстЗаголовка = Новый ФорматированнаяСтрока(
				НСтр("ru = 'Рекомендуется настроить автоматическую свертку регистра соответствия номенклатуры ВетИС';
					|en = 'Рекомендуется настроить автоматическую свертку регистра соответствия номенклатуры ВетИС'"),,,,
				"АвтоматическийОбмен");
			Элементы[ИмяЭлемента].Заголовок = ТекстЗаголовка;
			
			ТекстПодсказки = НСтр("ru = 'Сворачивает остатки по сериям и записям складского журнала регистра соответствие номенклатуры.';
									|en = 'Сворачивает остатки по сериям и записям складского журнала регистра соответствие номенклатуры.'");
		...
		Элементы[ИмяЭлемента].Заголовок = Новый ФорматированнаяСтрока(
			СтрШаблон(
				НСтр("ru = 'Есть проблемы с синхронизацией ветеринарно-сопроводительных документов (%1)';
					|en = 'Есть проблемы с синхронизацией ветеринарно-сопроводительных документов (%1)'"),
				Выборка.Количество()),,,,
			ИдентификаторПроблемы);
		...
		Элементы[ИмяЭлемента].Заголовок = Новый ФорматированнаяСтрока(
			СтрШаблон(
				НСтр("ru = 'Есть проблемы с синхронизацией записей складского журнала (%1)';
					|en = 'Есть проблемы с синхронизацией записей складского журнала (%1)'"),
				Выборка.Количество()),,,,
			ИдентификаторПроблемы);
		...
		Элементы[ИмяЭлемента].Заголовок = Новый ФорматированнаяСтрока(
			СтрШаблон(
				НСтр("ru = 'Ветеринарно-сопроводительные документы не актуальны (%1)';
					|en = 'Ветеринарно-сопроводительные документы не актуальны (%1)'"),
				Выборка.Количество()),,,,
			ИдентификаторПроблемы);
		...
		Элементы[ИмяЭлемента].Заголовок = Новый ФорматированнаяСтрока(
			СтрШаблон(
				НСтр("ru = 'Записи складского журнала не актуальны (%1)';
					|en = 'Записи складского журнала не актуальны (%1)'"),
				Выборка.Количество()),,,,
			ИдентификаторПроблемы);
		...
		СтрокаЗаголовка = Новый ФорматированнаяСтрока(
			СтрШаблон(
				НСтр("ru = 'Обмен не выполнялся длительное время (%1)';
					|en = 'Обмен не выполнялся длительное время (%1)'"),
				Выборка.Количество()),,,,
			ИдентификаторПроблемы);
		...
		Элементы[ИмяЭлемента].Заголовок = Новый ФорматированнаяСтрока(
			СтрШаблон(
				НСтр("ru = 'Рассинхронизация обмена по записям складского журнала и ветеринарно-сопроводительным документам (%1)';
					|en = 'Рассинхронизация обмена по записям складского журнала и ветеринарно-сопроводительным документам (%1)'"),
				Выборка.Количество()),,,,
			ИдентификаторПроблемы);
