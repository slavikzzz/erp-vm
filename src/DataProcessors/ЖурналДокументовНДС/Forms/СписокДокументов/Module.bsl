...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	Если Не ПравоДоступа("Чтение", Метаданные.Документы.ТаможеннаяДекларацияЭкспорт) Тогда
		СвойстваСписка.ТекстЗапроса = 
			СтрЗаменить(СвойстваСписка.ТекстЗапроса, 
				"
				|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ТаможеннаяДекларацияЭкспорт КАК ДанныеПоСборуДокументов
				|		ПО (ДанныеПоСборуДокументов.Ссылка = РеестрДокументов.Ссылка)", 
				"");
	Иначе
		СвойстваСписка.ТекстЗапроса = 
			СтрЗаменить(СвойстваСписка.ТекстЗапроса, 
				"ЛОЖЬ КАК СборСопроводительныхДокументовЗавершен", 
				"ДанныеПоСборуДокументов.СборСопроводительныхДокументовЗавершен  КАК СборСопроводительныхДокументовЗавершен");
	КонецЕсли;
	//-- НЕ УТ
	...
	//++ Устарело_Переработка24
	ИспользуетсяТолькоПриемВПереработку2_4 = ПолучитьФункциональнуюОпцию("ИспользуетсяТолькоПриемВПереработку2_4");
	//-- Устарело_Переработка24
...
Функция ТекстЗапросаДокументыНДС()
	...
	Возврат 
		...
		|	ВЫБОР
		//++ НЕ УТ
		|		КОГДА РеестрДокументов.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтраженияДокументовВРеглУчете.ОтраженоВУчетеВручную)
		|			ТОГДА 6
		//-- НЕ УТ
...
Функция ОписаниеОперацийИТиповДокументов()
	...
	Строка.ЗаголовокРабочегоМеста		= НСтр("ru = 'Счета-фактуры выданные (аванс)';
												|en = 'Issued tax invoices (advance)'");
	...
	Строка.ЗаголовокРабочегоМеста		= НСтр("ru = 'Счета-фактуры выданные комиссионерам (агентам)';
												|en = 'Tax invoices issued to consignees'");
	...
	Строка.ЗаголовокРабочегоМеста		= НСтр("ru = 'Счета-фактуры полученные от комитентов (принципалов)';
												|en = 'Tax invoices received from consignors'");
	...
	Строка.ЗаголовокРабочегоМеста				= НСтр("ru = 'Счета-фактуры полученные налогового агента';
														|en = 'Received tax invoices of tax agent'");
	...
	Строка.ЗаголовокРабочегоМеста				= НСтр("ru = 'Иные документы, подтверждающие оплату сумм НДС';
														|en = 'Other documents confirming VAT amounts payment'");
	...
	Строка.ЗаголовокРабочегоМеста		= НСтр("ru = 'Счета-фактуры по неподтвержденной реализации по ставке 0%';
												|en = 'Tax invoices for unconfirmed sale at 0% rate'");
	...
	Строка.ЗаголовокРабочегоМеста = НСтр("ru = 'Таможенные декларации (импорт)';
										|en = 'Customs declarations (import)'");
	...
	СтрокаТаможеннаяДекларацияЭкспорт = ТЗХозОперацииИТипыДокументов.Добавить();
	Строка								= СтрокаТаможеннаяДекларацияЭкспорт;
	Строка.ХозяйственнаяОперация		= Перечисления.ХозяйственныеОперации.ОформлениеТаможеннойДекларацииЭкспорт;
	Строка.КлючНазначенияИспользования	= "ДокументыИсходящие";
	Строка.ЗаголовокРабочегоМеста		= НСтр("ru = 'Таможенные декларации на экспорт';
												|en = 'Customs declarations for export'");
	Строка.ПравоДоступаДобавление		= Истина;
	Строка.ТипДокумента					= Тип("ДокументСсылка.ТаможеннаяДекларацияЭкспорт");
	Строка.ПолноеИмяДокумента			= Метаданные.Документы.ТаможеннаяДекларацияЭкспорт.ПолноеИмя();
	Строка.ИспользуютсяСтатусы			= Ложь;
	Строка.ДобавитьКнопкуСоздать		= Истина;
	Строка.ИменаЭлементовРабочегоМеста = "ОтборТаможенныхДеклараций";
	Строка.МенеджерРасчетаГиперссылкиКОформлению = "Документ.ТаможеннаяДекларацияЭкспорт";
	...
	Строка.ЗаголовокРабочегоМеста = НСтр("ru = 'Заявления о ввозе товаров из ЕАЭС';
										|en = 'Applications for goods import from EAEU'");
...
Процедура УказатьДокументПриобретения(Команда)
	...
	Если ПараметрыСФ.Исправление Или ПараметрыСФ.Корректировочный Тогда
		ТекстПредупреждения = НСтр("ru = 'Команда не может быть выполнена для исправленных или корректировочных счетов-фактур.';
									|en = 'The command cannot be executed for adjusted or adjustment tax invoices.'");
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ОчиститьДокументПриобретения(Команда)
	...
	ИначеЕсли ВыделенныеСтроки.Количество() > 1 Тогда
		ТекстПредупреждения = НСтр("ru = 'Команда не может быть выполнена одновременно для нескольких строк.';
									|en = 'Command cannot be executed for multiple lines simultaneously.'");
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
