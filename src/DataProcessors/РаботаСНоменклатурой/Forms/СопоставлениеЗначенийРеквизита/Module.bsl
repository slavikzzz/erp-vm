...
Процедура УстановитьЗаголовокГиперссылкиОтбора()
	...
	Если РежимОтбораПоЗначению Тогда
		ОтборПохожихЗначений = СтрШаблон(НСтр("ru = 'Убрать отбор по значению %1';
												|en = 'Remove filter by value %1'"), ТекущееЗначениеРеквизита);	
	Иначе
		ОтборПохожихЗначений = СтрШаблон(НСтр("ru = 'Отобрать похожие на %1';
												|en = 'Select similar to %1'"), ТекущееЗначениеРеквизита);
	КонецЕсли;
...
Функция ТекстНеЗаполненоЗначение()
	...
	Возврат НСтр("ru = '<Выберите значение или создайте новое>';
				|en = '<Select a value or create a new one>'");
