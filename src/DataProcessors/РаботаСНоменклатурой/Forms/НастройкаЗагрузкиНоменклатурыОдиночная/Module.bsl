...
Процедура Создать(Команда)
	...
	Если Не ВыбраныОбъектыДляЗагрузки() Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Загрузка номенклатуры';
											|en = 'Import products'"),, НСтр("ru = 'Номенклатура не загружена';
																					|en = 'Products are not imported'"));
		Закрыть();
	Иначе
		...
		СтрокаЗаголовка = 
			СтрШаблон(
				НСтр("ru = 'Для номенклатуры <a href = ""Номенклатура"">%1</a> необходимо загрузить категорию
					...
					|en = 'Import the <a href = ""Категория"">%2</a>
					| category for the <a href = ""Номенклатура"">%1</a> product or map it with the product kind. 
					|This will help you save additional attributes and product characteristics.'"), 
		...
		СтрокаЗаголовка = 
			СтрШаблон(
				НСтр("ru = 'Для номенклатуры <a href = ""Номенклатура"">%1</a> необходимо загрузить 
					...
					|en = 'Import the 
					| <a href = ""Категория"">%2</a> category for the <a href = ""Номенклатура"">%1</a> product or map it with the product kind.'"), 
						НаименованиеНоменклатуры, НаименованиеКатегории);
	...
	ИначеЕсли ЗначениеЗаполнено(ВидНоменклатуры) И ИспользуютсяХарактеристикиВСервисе Тогда
		...
		СтрокаЗаголовка = СтрШаблон(
			НСтр("ru = 'Для номенклатуры <a href = ""Номенклатура"">%1</a> ведутся характеристики. 
				...
				|en = 'Characteristics are kept for the <a href = ""Номенклатура"">%1</a> product. 
				|Select characteristics to import.'"), 
...
Процедура ЗагрузитьКатегорииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ТекстОповещения = СтрШаблон(НСтр("ru = 'Создан вид номенклатуры ""%1""';
										|en = '""%1"" product kind is created'"), НовыеЭлементы[0]);
