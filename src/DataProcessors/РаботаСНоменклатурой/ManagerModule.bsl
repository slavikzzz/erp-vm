...
Функция ДобавитьКВыгрузкеНоменклатуруПоСписку(Организация, МассивНоменклатуры) Экспорт 
	...
	Если Добавлено > 0 Тогда
		ТекстЛога.Добавить(СтрШаблон(НСтр("ru = 'Добавлено к выгрузке %1';
											|en = 'Added to export %1'"), Добавлено));
	КонецЕсли;
	Если Исключено > 0 Тогда
		ТекстЛога.Добавить(СтрШаблон(НСтр("ru = 'Найдены в исключениях %1';
											|en = 'Found in exceptions %1'"), Исключено));
	КонецЕсли;
	Если МассивНоменклатуры.Количество() Тогда
		ТекстЛога.Добавить(СтрШаблон(НСтр("ru = 'Не добавлены в выгрузку (уже выгружаются) %1';
											|en = 'Not added to export (already being exported) %1'"), МассивНоменклатуры.Количество()));
	КонецЕсли;
...
Функция ДобавитьКВыгрузкеНоменклатуруПоОтбору(Организация, НастройкиОтбора, ЗапускИзРегламента = Ложь) Экспорт 
	...
	Иначе
		 ТекстЛога.Добавить(НСтр("ru = 'Нет новой номенклатуры, удовлетворяющей отбору';
								|en = 'There are no new items that satisfy the filter'"));
	КонецЕсли;
