...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность И НЕ ЗавершениеРаботы Тогда
		...
		ПоказатьВопрос(ОбработчикОтвета, НСтр("ru = 'Данные были изменены. Сохранить изменения в репозиторий?';
												|en = 'The data was changed. Do you want to save the changes to the repository?'"), РежимДиалогаВопрос.ДаНетОтмена);
	КонецЕсли;
...
#Если НЕ ВебКлиент Тогда
		...
		ФайловаяСистемаКлиент.ВыбратьКаталог(ОбработчикВыбораФайла, НСтр("ru = 'Укажите каталог SRC';
																		|en = 'Select a SRC directory'"), Объект.КаталогSRC);
...
#Если НЕ ВебКлиент Тогда
		...
		ФайловаяСистемаКлиент.ВыбратьКаталог(ОбработчикВыбораФайла, НСтр("ru = 'Укажите каталог файлов ошибок';
																		|en = 'Specify an error files directory'"), КаталогФайловОшибок);
#КонецЕсли
КонецПроцедуры
...
Процедура ЗаполнитьБыстрыеОтборы(Знач ОписаниеОбработчиков = Неопределено)
	...
	ТекстСтатуса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 (%2)", НСтр("ru = 'Все';
																							|en = 'All'"), ОписаниеОбработчиков.Количество());
	СтрокаГиперссылки = Новый ФорматированнаяСтрока(
		ТекстСтатуса, 
    	, 
		ЦветаСтиля.ЦветТекстаКнопки, ,
...
#Если НЕ ВебКлиент Тогда
	...
	Исключение
		...
		ТекстСообщения = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось прочитать файл %1.';
																									|en = 'Cannot read file %1.'"), ИмяФайла);
		ТекстСообщения = ТекстСообщения  + Символы.ПС + НСтр("ru = 'По причине';
															|en = 'Reason'")+":" + Символы.ПС;
...
Процедура ПрочитатьСтрокиYAML(Результат, ТекстФайла, НомерСтроки = 0, Уровень = 0, ЧтениеМногострочнойСтроки = Ложь)
	...
	Исключение
		ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось разобрать строку: %1';
																									|en = 'Cannot parse string: %1'"), СтрокаФайла);
		ТекстСообщения = ТекстСообщения  + Символы.ПС + НСтр("ru = 'По причине';
															|en = 'Reason'")+":" + Символы.ПС;
		ТекстСообщения = ТекстСообщения  + ТекстОшибки;
		ВызватьИсключение ТекстСообщения;
...
Функция ФильтрФайловРезервнойКопии()
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Описание обработчиков (*.handlers)|*.handlers|Все файлы (%1)|%1';
																		|en = 'Handler details (*.handlers)|*.handlers|All files (%1)|%1'"), ПолучитьМаскуВсеФайлы());
КонецФункции
