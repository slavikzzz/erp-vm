	...
	Если Не ЗакрытьБезВопроса Тогда
		...
		Если ЕстьВыгруженныеФайлы Тогда
			...
			Кнопки.Добавить(1, НСтр("ru = 'Удалить файлы и закрыть';
									|en = 'Delete files and close'"));
			Кнопки.Добавить(2, НСтр("ru = 'Закрыть без удаления';
									|en = 'Close without deletion'"));
			...
			ТекстВопроса = НСтр("ru = 'Выгруженные файлы могут быть несанкционированно изменены.
				...
				|en = 'Exported files can be changed without authorization.
				|Remove the files before closing the form.'");
			ПоказатьВопрос(
...
Процедура КаталогВыгрузкиПодтверждающихДокументовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбработчикЗавершения =
		Новый ОписаниеОповещения("КаталогВыгрузкиПодтверждающихДокументовЗавершение", ЭтотОбъект, Элемент);
	ЗаголовокОкнаВыбора= НСтр("ru = 'Выберите каталог для сохранения архивов подтверждающих документов';
								|en = 'Select a directory to save justification document archives to'");
	Каталог = Элементы.КаталогВыгрузкиПодтверждающихДокументов.ТекстРедактирования;
	ФайловаяСистемаКлиент.ВыбратьКаталог(ОбработчикЗавершения, ЗаголовокОкнаВыбора, Каталог);
	//-- НЕ УТ
...
Процедура КаталогВыгрузкиПодтверждающихДокументовЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныеФайлы <> "" Тогда
		КаталогВыгрузкиПодтверждающихДокументов = ВыбранныеФайлы;
		УстановитьЗначениеКаталогаВыгрузки(КаталогВыгрузкиПодтверждающихДокументов);
	КонецЕсли;
...
Процедура ПериодПриИзменении(Элемент)
	...
	ИначеЕсли Период = "ЗаДату" Тогда
		...
		ПоказатьВводДаты(Оповещение, ЛокальнаяТекущаяДата, НСтр("ru = 'Дата отбора платежей';
																|en = 'Payment filter date'"), ЧастиДаты.Дата);
		Возврат;
...
Процедура ПериодОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = "Дату" Тогда
		...
		ПоясняющийТекст = НСтр("ru = 'Выберите дату выгружаемых документов';
								|en = 'Select the date of exported documents'") + " ";
		ОткрытьФорму(
...
Процедура Выгрузить(Команда)
	...
	//++ НЕ УТ
	Объект.АдресаХранилищФайловПодтверждающихДокументов.Очистить();
	//-- НЕ УТ
			...
			Если Не ЗначениеЗаполнено(Счет.НастройкаОбмена) Тогда
				...
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Для банковского счета %1 не настроен прямой обмен. Выполните подключение к сервису 1С:ДиректБанк, или воспользуйтесь выгрузкой в файл.';
						|en = 'Direct exchange is not set up for the bank account: %1. Connect to 1C:DirectBank or export data to a file.'"),
					Счет.Ссылка);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ТекстСообщения,,
...
Процедура ЗаполнитьСписокВыбораПериодов()
	...
	СписокВыбораПериода.Добавить(НСтр("ru = 'Все время';
										|en = 'All time'"),     НСтр("ru = 'Все время';
																	|en = 'All time'"));
	СписокВыбораПериода.Добавить("Дату",                     НСтр("ru = 'Дату...';
																	|en = 'Date ...'"));
	СписокВыбораПериода.Добавить(НСтр("ru = 'Сегодня';
										|en = 'Today'"),       НСтр("ru = 'Сегодня';
																	|en = 'Today'"));
	СписокВыбораПериода.Добавить(НСтр("ru = 'Три дня';
										|en = 'Three days'"),       НСтр("ru = 'Три дня';
																	|en = 'Three days'"));
	СписокВыбораПериода.Добавить(НСтр("ru = 'Неделю';
										|en = 'Week'"),        НСтр("ru = 'Неделю';
																	|en = 'Week'"));
...
Процедура ОбработатьЗавершениеВыгрузки()
	...
	//++ НЕ УТ
	Если Объект.АдресаХранилищФайловПодтверждающихДокументов.Количество() > 0 Тогда
		СохранитьФайлыПодтверждающихДокументов();
	КонецЕсли;
	//-- НЕ УТ
		...
		Если ВозможностьВыбораФайлов Тогда
			Элементы.ДекорацияВыгружено.Заголовок = Выгружено + " " + Файлов + ". " +
				НСтр("ru = 'В целях безопасности проводится контроль соответствия файлов выгрузки и данных программы, пока эта форма открыта. При закрытии этой формы файлы будут удалены.
					...
					|en = 'For safety reasons, it is controlled whether export files correspond to the application data while this form is opened. When closing this form, the files will be deleted.
					|Check whether payment details in the Client Bank application are correct.'");
		Иначе
			Элементы.ДекорацияВыгружено.Заголовок = Выгружено + " " + Файлов + ". " +
				НСтр("ru = 'Контроль соответствия файлов выгрузки и данных программы не производится. В целях безопасности требуется установить расширение для работы с файлами.
					|Обязательно проверьте правильность платежных реквизитов в программе Клиент банка.';
					|en = 'It is not controlled whether export files correspond to the application data. For safety reasons, install 1C:Enterprise Extension.
					|Check whether payment details in the Client Bank application are correct.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура СформироватьСтрокуСуммыКВыгрузке()
	...
	Если СуммыПлатежейКВыгрузке.Количество() Тогда
		...
		СуммаКВыгрузке = НСтр("ru = 'На сумму:';
								|en = 'Amount:'") + " " + ?(СуммаКВыгрузке = "", "-", СуммаКВыгрузке);
	Иначе
		СуммаКВыгрузке = НСтр("ru = 'На сумму: -';
								|en = 'Amount:-'")
	КонецЕсли;
...
Процедура ВыборФайлаПриВыгрузке(Счет)
	...
	ДиалогВыбораФайла.Заголовок               =
		СтрШаблон(НСтр("ru = 'Укажите имя файла для выгрузки данных по счету %1';
						|en = 'Specify a file name for data export by %1 account'"), Строка(Счет.Ссылка));
...
&НаКлиенте
Процедура СохранитьФайлыПодтверждающихДокументов()
	
	Если ВозможностьВыбораФайлов Тогда
		ПолучитьАрхивФайловПодтверждающихДокументовССервера();
	Иначе
		
		ТекстСообщения =
			НСтр("ru = 'Для выгрузки файлов подтверждающих документов необходимо установить расширение работы с файлами.';
				|en = 'To export justification document files, install a file system extension.'");
		ОписаниеОповещения = Новый ОписаниеОповещения("СохранитьФайлыПодтверждающихДокументовЗавершение", ЭтотОбъект);
...
&НаСервереБезКонтекста
Функция КаталогПодтверждающихДокументов()
	
	Возврат Константы.КаталогВыгрузкиПодтверждающихДокументов.Получить();
	
КонецФункции

&НаКлиенте
Процедура ПолучитьАрхивФайловПодтверждающихДокументовССервера()
	
	КаталогВыгрузки = КаталогПодтверждающихДокументов();
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПроверкаСуществованияКаталогаПодтверждающихДокументовЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ПроверкаСуществованияКаталогаПодтверждающихДокументовЗавершение(Существует, ДополнительныеПараметры) Экспорт

	Если Существует Тогда
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ПроверкаКаталогаПодтверждающихДокументовЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		Каталог = Новый Файл(ДополнительныеПараметры.КаталогВыгрузки);
		Каталог.НачатьПроверкуЭтоКаталог(ОписаниеОповещения);
		
	Иначе
		
		ТекстОшибки = НСтр("ru = 'Каталог не обнаружен. Проверьте корректность пути или обратитесь к техническому специалисту.';
							|en = 'The directory is not found. Check whether the path to the directory is correct or contact the technical specialist.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки, , "КаталогВыгрузкиПодтверждающихДокументов");
...
&НаКлиенте
Процедура ПроверкаКаталогаПодтверждающихДокументовЗавершение(ЭтоКаталог, ДополнительныеПараметры) Экспорт

	Если ЭтоКаталог Тогда
		
		ПолучаемыеФайлы = Новый Массив;
	
		Для Каждого ТекущийАдрес Из Объект.АдресаХранилищФайловПодтверждающихДокументов Цикл
			ПолучаемыеФайлы.Добавить(Новый ОписаниеПередаваемогоФайла(ТекущийАдрес.Представление, ТекущийАдрес.Значение));
		КонецЦикла;
		
		Объект.АдресаХранилищФайловПодтверждающихДокументов.Очистить();
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыгрузитьФайлыПодтверждающихДокументовЗавершение", ЭтотОбъект);
		НачатьПолучениеФайловССервера(ОписаниеОповещения, ПолучаемыеФайлы, ДополнительныеПараметры.КаталогВыгрузки);
		
	Иначе
		
		ТекстОшибки = НСтр("ru = 'Не корректно заполнен каталог выгрузки.';
							|en = 'The export directory is filled incorrectly.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки, , "КаталогВыгрузкиПодтверждающихДокументов");
...
&НаКлиенте
Процедура ВыгрузитьФайлыПодтверждающихДокументовЗавершение(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	
	Если ПолученныеФайлы <> Неопределено Тогда
		
		Для Каждого ТекущееОписание Из ПолученныеФайлы Цикл
			УдалитьИзВременногоХранилища(ТекущееОписание.Хранение);
		КонецЦикла;
...
&НаКлиенте
Процедура СохранитьФайлыПодтверждающихДокументовЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
&НаСервереБезКонтекста
Процедура УстановитьЗначениеКаталогаВыгрузки(Значение)

	Константы.КаталогВыгрузкиПодтверждающихДокументов.Установить(Значение);
...
Процедура Подключаемый_ПодключитьРасширениеРаботыСФайламиИВыбратьФайл()
	...
	ТекстСообщения = НСтр("ru = 'Для выбора файла необходимо установить расширение работы с файлами.';
							|en = 'To select a file, install 1C:Enterprise Extension.'");
...
Процедура ПодключитьРасширениеРаботыСФайламиИВыбратьФайлПриВыгрузке(Счет)
	...
	ТекстСообщения = НСтр("ru = 'Для указания файла выгрузки платежей по счету %1 необходимо установить расширение работы с файлами.';
							|en = 'To specify a payment export file for account %1, install 1C:Enterprise Extension.'");
