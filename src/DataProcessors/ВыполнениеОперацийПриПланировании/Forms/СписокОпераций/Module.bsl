...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ПроизводственнаяОперация2_2" И Тип(Параметр) = Тип("Структура") И Параметр.Свойство("КлючОперации") 
...
Процедура ИнформационнаяНадписьДобавитьОперациюОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ЗначенияЗаполнения = ЗначенияЗаполненияОперации();
...
Процедура СписокОперацийИсполнительАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		ИсполнительПолучениеДанныхВыбора(ДанныеВыбора, Текст, Подразделение, Элементы.СписокОпераций.ТекущиеДанные.Дата);
...
&НаКлиенте
Функция ЗначенияЗаполненияОперации() 
	...
	ДанныеЗаполнения.Вставить("Количество", КоличествоСоздать);
	ДанныеЗаполнения.Вставить("Статус",
		?(РежимРаботы = РежимРаботыПринятьВРаботу(),
...
Процедура ЗаполнитьРеквизитыФормыСпискаПоДаннымОперации()
	...
	ИспользоватьБригадныеНаряды = Документы.ПроизводственнаяОперация2_2.НастройкиУчета(
		Подразделение, МаршрутнаяКарта).ИспользоватьБригадныеНаряды;
...
Процедура ЗаполнитьСписокОпераций()
	...
	Запрос = Новый Запрос(
		...
		|	И Операции.НомерПартии = &НомерПартии
