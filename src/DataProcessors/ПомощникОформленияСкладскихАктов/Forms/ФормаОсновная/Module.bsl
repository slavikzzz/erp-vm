	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") Тогда
		...
		Если Не ЗначениеЗаполнено(ЕдинственнаяОрганизация) Тогда
			ТекстОшибки = НСтр("ru = 'Невозможно оформить складские акты, т.к. в настройках программы не введены сведения об организации.';
								|en = 'Cannot register inventory adjustments as the company information is not entered in the application settings.'");
			Отказ = Истина;
	...
	Иначе
		...
		Элементы.ГруппаТоварыКПересортицеКПорче.Заголовок
			= НСтр("ru = 'Товары, зачитываемые по пересортице';
					|en = 'Goods offset by stock reclass'");
		Элементы.ТоварыКПересортицеКПорчеГруппаКолонокТоварКСписанию.Заголовок
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") Тогда
		Элементы.ДекорацияСтраницаОформлениеДокументов.Заголовок = 
			НСтр("ru = 'Сформированные согласно распределению по организациям складские акты. Если есть непроведенные акты - откройте их, заполните необходимые поля и проведите.';
				|en = 'Inventory adjustments generated according to assigned company stock. If there are unposted adjustments, open them, fill in the required fields and post.'");
	Иначе
		Элементы.ДекорацияСтраницаОформлениеДокументов.Заголовок = 
			НСтр("ru = 'Сформированные складские акты. Если есть непроведенные акты - откройте их, заполните необходимые поля и проведите.';
				|en = 'Generated inventory adjustments. If there are unposted adjustments, open them, fill in the required fields and post.'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Объект.Склад) Тогда
		Если Не СкладыСервер.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач(Объект.Склад) Тогда
			...
			ТекстСообщения = НСтр("ru = 'На складе %Склад% не используется ордерная схема при оформлении излишков, недостач и порчи, поэтому невозможно использовать помощник оформления складских актов.';
									|en = 'Warehouse management is not used in warehouse ""%Склад%"" while recording surpluses, shortages and damage, so it is impossible to use the inventory adjustment creation wizard.'");
	...
	ИначеЕсли Не Элементы.ОсновнаяПанель.ТекущаяСтраница = Элементы.СтраницаОформлениеДокументов Тогда
		...
		Если ЗавершениеРаботы Тогда
			ТекстПредупреждения = НСтр("ru = 'Процесс оформления складских актов не завершен.
				...
				|en = 'Registration of inventory adjustments is not completed. 
		...
		ТекстВопроса = НСтр("ru = 'Процесс оформления не завершен.';
							|en = 'Registration process is not completed.'");
	...
	Иначе
		...
		Если ЗавершениеРаботы Тогда
			ТекстПредупреждения = НСтр("ru = 'Работа помощника оформления складских актов будет прервана.';
										|en = 'Wizard of inventory adjustment registration will be closed.'");
	...
	Если Не СтандартнаяОбработка Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Вводимое значение должно быть не более текущего и не менее нуля.';
				|en = 'Entered value must be less than the current one and not less than zero.'"), , 
...
Процедура ЗачестьПоПересортицеПоПорче(Команда)
	...
	Если ТекущиеДанныеОприходования = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Выделите строку оприходования, которую нужно зачесть.';
								|en = 'Select a recording as received line which should be set off.'");
		ПоказатьПредупреждение(Неопределено, ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	ТекстСообщения = НСтр("ru = 'Укажите количество для зачета (максимальное значение - %МаксимальноеЗначение%)';
							|en = 'Specify a quantity to process (max value - %МаксимальноеЗначение%)'");
...
Процедура ЗачестьПоПересортицеАвто(Команда)
	...
	Если Результат = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Среди списываемых и приходуемых товаров нет товаров, по которым возможно провести автоматический зачет.';
									|en = 'In the list of stock shortage and surplus, there are no goods for which automatic processing is available.'"));
	КонецЕсли;
...
Процедура КнопкаНазад(Команда)
	...
	Если  Элементы.ОсновнаяПанель.ТекущаяСтраница = Элементы.СтраницаОформлениеДокументов Тогда
		...
		ТекстВопроса = НСтр("ru = 'Все сформированные акты будут удалены. Вернуться назад?';
							|en = 'All the generated certificates will be removed. Return?'");
		ПоказатьВопрос(Оповещение, ТекстВопроса, СписокКнопок);
	...
	Иначе 		
		...
		ТекстВопроса = НСтр("ru = 'Процесс оформления не завершен. При возврате к настройкам введенная информация будет утеряна. Вернуться?';
							|en = 'Registration process is not completed. If you return to settings, entered data will be lost. Return?'");
		ПоказатьВопрос(Оповещение, ТекстВопроса, СписокКнопок);
...
Процедура КнопкаДалее(Команда)
	...
	Если ТекущаяСтраница = Элементы.СтраницаНастройки Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Склад) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выберите склад.';
																	|en = 'Select a warehouse.'"), , "Объект.Склад");
			ЕстьОшибки = Истина;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ИнвентаризационныйПериод.ДатаНачала) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выберите дату начала инвентаризационного периода.';
																	|en = 'Select start date of physical inventory count period.'"), , "ИнвентаризационныйПериод");
			ЕстьОшибки = Истина;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ИнвентаризационныйПериод.ДатаОкончания) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выберите дату окончания инвентаризационного периода.';
																	|en = 'Select end date of physical inventory count period.'"), , "ИнвентаризационныйПериод");
			ЕстьОшибки = Истина;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ДатаОформленияСкладскихАктов) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выберите дату оформления складских актов.';
																	|en = 'Select inventory adjustment generation date.'"), , "ДатаОформленияСкладскихАктов");
			ЕстьОшибки = Истина;
		КонецЕсли;
		Если ДатаОформленияСкладскихАктов < ИнвентаризационныйПериод.ДатаНачала 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выберите дату оформления складских актов в рамках инвентаризационного периода.';
																	|en = 'Select inventory adjustment generation date under the physical inventory count period.'"), , "ДатаОформленияСкладскихАктов");
			ЕстьОшибки = Истина;
		КонецЕсли;
		Если КоличествоТоваровКОформлениюСУчетомОтбора = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Отсутствуют товары к оформлению складских актов.';
											|en = 'No goods to register inventory adjustments for.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаРаспределениеПоОрганизациям Тогда
		...
		Если Объект.ТоварыРаспределенные.Количество() = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Распределите товары по организациям.';
											|en = 'Assign company stock.'"), , НСтр("ru = 'Нет данных для оформления';
																								|en = 'No data for registration'"));
		Иначе
...
Процедура РаспределитьНаОрганизацию(Команда)
	...
	Если Не ЗначениеЗаполнено(Элементы.ТоварыНеРаспределенные.ВыделенныеСтроки) 
		...
		ТекстСообщения = НСтр("ru = 'Выделите одну или несколько строк для распределения.';
								|en = 'Select one or several lines for allocation.'");
		ПоказатьПредупреждение(Неопределено, ТекстСообщения);
...
Процедура РаспределитьПоВладельцамЗавершение(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнч(Результат) = Тип("Массив")
		...
		ТекстСообщения = НСтр("ru = 'Не выбрано организаций для распределения.';
								|en = 'Companies for allocation are not selected.'");
		ПоказатьПредупреждение(Неопределено, ТекстСообщения);
	...
	Если СозданоСтрок = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Не удалось провести автоматическое распределение по организациям. Возможно у выбранных организаций нет остатков по выбранным товарам.';
								|en = 'Cannot automatically assign company stock for adjustment. The selected companies may not have remaining selected goods.'");
		ПоказатьПредупреждение(Неопределено, ТекстСообщения);
	КонецЕсли;
...
Процедура УправлениеДекорациямиПоТоварам()
	...
	Если Не ЗначениеЗаполнено(Объект.Склад) И Не ЗначениеЗаполнено(ИнвентаризационныйПериод) Тогда 
		ЗаголовокДекорации = НСтр("ru = 'Заполните поля ""Склад"" и ""Период инвентаризационной описи"" для определения товаров, по которым необходимо оформить складские акты.';
									|en = 'Fill in fields ""Warehouse"" and ""Physical inventory register period"" to identify the goods for which it is required to register inventory adjustments.'");
	ИначеЕсли ЗначениеЗаполнено(Объект.Склад) И Не ЗначениеЗаполнено(ИнвентаризационныйПериод) Тогда 
		ЗаголовокДекорации = НСтр("ru = 'Заполните поле ""Период инвентаризационной описи"" для определения товаров, по которым необходимо оформить складские акты.';
									|en = 'Fill in field Physical inventory register period"" to determine goods for which it is required to register inventory adjustments.'");
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Склад) И ЗначениеЗаполнено(ИнвентаризационныйПериод) Тогда
		ЗаголовокДекорации = НСтр("ru = 'Заполните поле ""Склад"" для определения товаров, по которым необходимо оформить складские акты.';
									|en = 'Fill in field ""Warehouse"" to determine goods for which it is required to register inventory adjustments.'");
	ИначеЕсли КоличествоТоваровКОформлению = 0 Тогда 
		ЗаголовокДекорации = НСтр("ru = 'Не требуется оформление складских актов.';
									|en = 'Inventory adjustment registration is not required.'");
	ИначеЕсли КоличествоТоваровКОформлениюСУчетомОтбора = 0 Тогда
		МассивСтрок = Новый Массив;
		МассивСтрок.Добавить(НСтр("ru = 'По выбранному складу есть';
									|en = 'For the selected warehouse there is'") + " ");
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'товары';
																|en = 'goods'"),,,,"отчет"));
		МассивСтрок.Добавить(НСтр("ru = ', по которым необходимо оформить складские акты, но нет товаров удовлетворяющих установленному фильтру. Измените фильтр:';
									|en = ', which require inventory adjustments, but the set filter shows no goods. Change the filter:'"));
	...
	Иначе
		...
		МассивСтрок.Добавить(НСтр("ru = ', по которым необходимо оформить складские акты. Для ограничения списка обрабатываемой номенклатуры воспользуйтесь фильтрами:';
									|en = ', which require inventory adjustments. To restrict the list of processed products, use the filters:'"));		
		ЗаголовокДекорации = Новый ФорматированнаяСтрока(МассивСтрок);
	КонецЕсли;
	...
	Если СкладскойАкт.ПроверитьЗаполнение() Тогда
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось провести документ ""%Документ%"".';
									|en = 'Cannot post the ""%Документ%"" document.'");
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Оформление складских актов';
											|en = 'Inventory adjustments'", КодОсновногоЯзыка),
	...
	Если ЗначениеЗаполнено(Объект.Склад) Тогда
		...
		Если ФлагПоСкладуЕстьНезавершенныеПересчеты Тогда
			ТекстЗаголовка = НСтр("ru = 'Есть <a href=""Отчет"">пересчеты товаров</a>, по которым еще не завершено внесение результатов (они не проведены в статусе ""Выполнено"").';
									|en = 'There are <a href=""Отчет"">inventory counts</a> which are not entered yet (they are not posted in the ""Completed"" status).'");
		...
		Иначе
			Элементы.ДекорацияИнформированиеОНеоформленныхПересчетах.Заголовок = НСтр("ru = 'По выбранному складу все пересчеты товаров выполнены.';
																						|en = 'All inventory is recalculated for the selected warehouse.'");
		КонецЕсли;
...
Процедура УстановитьЗаголовокДекорацииОтбораНоменклатуры()
	...
	Иначе
		МассивСтрок.Добавить(НСтр("ru = '<не установлен>.';
									|en = '<not set>.'") + " ");
	КонецЕсли;
	МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Установить отбор';
															|en = 'Set filter'"),,,,"Фильтр"));
...
Процедура НастроитьФормуЛокализация()
	...
	Элементы.ДекорацияИнвентаризационныйПериод.Заголовок = НСтр("ru = 'Этот период будет подставлен в документ ""Инвентаризационная опись"", из которого осуществляется печать форм ИНВ-3 и ИНВ-19. Список товаров при печати будет взят из пересчетов товаров, оформленных за этот период, а данные о количестве будут рассчитаны по остаткам организаций на конец периода и данным складских актов..';
																|en = 'This period will be used in the ""Stocktaking sheet"" document which is the source for printing the INV-3 and INV-19 stocktaking forms. When printing, the list of goods will be received from the goods recounts created within this period, while goods quantity will be calculated by company remaining goods as of the period end and warehouse act data..'");
	//-- Локализация
	Возврат;
