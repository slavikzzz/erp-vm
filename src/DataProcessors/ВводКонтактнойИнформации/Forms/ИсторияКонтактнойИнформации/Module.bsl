...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.СписокКонтактнойИнформации) = Тип("Массив") Тогда
		...
		КонецЦикла;
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'История изменений (%1)';
																				|en = 'Change history (%1)'"), ПредставлениеКонтактнойИнформации);
...
Процедура ИсторияПередУдалением(Элемент, Отказ)
	...
	Если НЕ Отказ Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Удалить адрес действующий с';
										|en = 'Do you want to remove address registered on'") + " " + Формат(ДействуетС, "ДЛФ=DD")+ "?", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
	Отказ = Истина;
КонецПроцедуры
