...
Функция ВыполнитьПереносФайлов(ФайлыДляПереноса, Параметры) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Начало переноса файлов.';
									|en = 'Files.Start moving files.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,,
		НСтр("ru = 'Начат перенос файлов';
			|en = 'Started moving files'") + " " + ДействиеДляЖурнала);
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Завершение переноса файлов.';
									|en = 'Files.End moving files.'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПеренестиФайл(Файл, СвойстваТома, Параметры, ОшибкиПереноса, ПеренесеноФайлов)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Ошибка переноса файла.';
										|en = 'Files.File moving error.'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПеренестиМеждуТомами(ФайлОбъект, СвойстваТома, Параметры)
	...
	Если Не ФайлНаДиске.Существует() Тогда
		...
		НСтр("ru = 'Файл ""%1"" не найден.';
			|en = 'File ""%1"" is not found.'"), ТекущийПутьКФайлу);
	КонецЕсли;
