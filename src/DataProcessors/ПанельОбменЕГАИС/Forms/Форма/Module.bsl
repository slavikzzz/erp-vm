		...
		Иначе
			...
			ПерваяПроблема = СтрШаблон(
				НСтр("ru = 'Есть сообщения (%1), ожидающие отправки в ЕГАИС. Выполните обмен.';
					|en = 'Есть сообщения (%1), ожидающие отправки в ЕГАИС. Выполните обмен.'"),
		...
		Если Не ЗаданиеВключено Тогда
			...
			Иначе
				ПерваяПроблема = НСтр("ru = 'Рекомендуется настроить автоматический обмен с ЕГАИС';
										|en = 'Рекомендуется настроить автоматический обмен с ЕГАИС'");
				Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаЖелтый;
			КонецЕсли;
		КонецЕсли;
			...
			Если Не ЗаданиеВключено Тогда
				...
				Иначе
					ПерваяПроблема = НСтр("ru = 'Рекомендуется настроить свертку регистра соответствия номенклатуры ЕГАИС';
											|en = 'Рекомендуется настроить свертку регистра соответствия номенклатуры ЕГАИС'");
					Элементы.ГруппаДекорацияСостояниеОбмена.ТекущаяСтраница = Элементы.СостояниеОбменаЖелтый;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ВыполнитьОткрытьWebИнтерфейсУТМ(ВыбраннаяОрганизацияЕГАИС)
	...
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			СтрШаблон(
				НСтр("ru = 'Не заданы параметры подключения к УТМ для организации ЕГАИС %1';
					|en = 'Не заданы параметры подключения к УТМ для организации ЕГАИС %1'"), ВыбраннаяОрганизацияЕГАИС),,
			"ОрганизацияЕГАИС");
	КонецЕсли;
