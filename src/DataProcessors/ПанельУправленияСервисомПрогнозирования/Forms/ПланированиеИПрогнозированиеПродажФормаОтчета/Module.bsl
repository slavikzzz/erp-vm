...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтчетыКлиентСервер.ОтобразитьСостояниеОтчета(
		ЭтотОбъект, НСтр("ru = 'Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.';
						|en = 'Report not generated. To generate the report, click ""Generate"".'"));
...
#КонецЕсли
	...
	ИначеЕсли Не ВыполненоПодключение
		...
		ТекстОшибки = НСтр("ru = 'Сервис прогнозирования не настроен. Текущий пользователь не имеет прав для настройки сервиса.';
							|en = 'The forecasting service is not set up. The current user is not authorized to set up the service.'");
...
Процедура ОтчетСкомпоноватьРезультат(Команда)
	...
	Если СценарийПрогнозирования.Пустая() Тогда
		ТекстОшибки = НСтр("ru = 'Не заполнен сценарий планирования';
							|en = 'The planning scenario is not filled'");
...
Процедура КомандыАдминистратора(Команда)
	ТекстВопроса = НСтр("ru = 'Команды предназначены для администратора. Продолжить?';
						|en = 'The commands are for the administrator. Continue?'");
...
Процедура НастройкиСервиса(Команда)
	ТекстВопроса = НСтр("ru = 'Просмотр настроек предназначен для администратора. Продолжить?';
						|en = 'The settings can be viewed by the administrator. Continue?'");
...
Функция ПроверитьВыгрузкуДанных()
	Если СервисПрогнозирования.ИдетОбменДанными() Тогда
		ТекстОшибки = НСтр("ru = 'Активна выгрузка данных. До ее окончания запрещено обновление данных.';
							|en = 'Data export is active. Cannot update data before it is completed.'");
...
Функция РезультатФормированияОтчета(Знач ФормированиеПриОткрытии, Непосредственно, ПослеПредупреждения)
	...
	Иначе
		ОтчетыКлиентСервер.ОтобразитьСостояниеОтчета(
			ЭтотОбъект, НСтр("ru = 'Отчет формируется...';
							|en = 'Generating report...'"), БиблиотекаКартинок.ДлительнаяОперация48);
	КонецЕсли;
	Возврат Результат;
...
&НаСервере
Процедура УстановитьПараметрДанныхКомпоновки(ИмяПараметра, ЗначениеПараметра, УстанавливатьИспользование = Истина, Использование = Истина)
	НастройкиКомпоновкиДанных = Отчет.КомпоновщикНастроек.Настройки; // НастройкиКомпоновкиДанных -
	ДоступныйПараметрСценарий = НастройкиКомпоновкиДанных.ПараметрыДанных.ДоступныеПараметры.Элементы.Найти(ИмяПараметра);
	Если ДоступныйПараметрСценарий <> Неопределено Тогда
		ЗаданныйПараметрСценарий = НастройкиКомпоновкиДанных.ПараметрыДанных.Элементы.Найти(ИмяПараметра); // ЗначениеПараметраНастроекКомпоновкиДанных -
		Если ЗаданныйПараметрСценарий <> Неопределено Тогда
			ЗаданныйПараметрСценарий.Значение = ЗначениеПараметра;
			Если УстанавливатьИспользование Тогда
				ЗаданныйПараметрСценарий.Использование = Использование;
			КонецЕсли;
		Иначе
			НовыйПараметр = НастройкиКомпоновкиДанных.ПараметрыДанных.Элементы.Добавить();
...
Функция ПроверитьКорректностьНастроекСервиса()
	...
	Если Не ЗначениеЗаполнено(НастройкиСервиса.ТокенПриложения)
		...
		ТекстОшибки = НСтр("ru = 'Не произведена регистрация. Используйте мастер подключения к сервису.';
							|en = 'Cannot register. Use a service connection wizard.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
		Возврат Ложь;
	КонецЕсли;
	Возврат Истина;
...
Процедура УстановитьПараметрыОтчета()
	...
	Если ЗначениеЗаполнено(ВидПлана) Тогда
		НастройкиВидаПлана = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидПлана,
			"Периодичность, НачалоПрогнозирования");
		ДатаНачалаПрогнозирования = НастройкиВидаПлана.НачалоПрогнозирования;
		УстановитьПараметрДанныхКомпоновки("ДатаНачалаПрогнозирования",      ДатаНачалаПрогнозирования);
		УстановитьПараметрДанныхКомпоновки("ДатаОкончанияВыводаФактаПродаж", ДатаНачалаПрогнозирования);
		УстановитьПараметрДанныхКомпоновки("ДатаНачалаВыводаФактаПродаж",    ДобавитьМесяц(ДатаНачалаПрогнозирования, -1));
		УстановитьПараметрДанныхКомпоновки("ПериодичностьОтчета",            НастройкиВидаПлана.Периодичность);
	КонецЕсли;
...
Процедура УстановитьПараметрДетализацииПоУмолчанию()
	...
	УстановитьПараметрДанныхКомпоновки("Детализация", ДетализацияПоУмолчанию, Ложь);
...
Процедура НайтиПодходящиеЗначенияУниверсальногоПоискаБулево(ПодходящиеЗначения, ПоляПоискаПоТипам, ПараметрыПоиска)
	...
	Если СтрСравнить(ПараметрыПоиска.СтрокаПоиска, НСтр("ru = 'Да';
														|en = 'Yes'")) = 0 
		Или СтрСравнить(ПараметрыПоиска.СтрокаПоиска, НСтр("ru = 'Истина';
															|en = 'True'")) = 0 
		Или СтрСравнить(ПараметрыПоиска.СтрокаПоиска, НСтр("ru = 'Включено';
															|en = 'Enabled'")) = 0 Тогда
		Для Каждого ПолеПоиска Из ПоляПоиска Цикл
			СвойстваПоиска = СвойстваУниверсальногоПоиска(ПолеПоиска.Ключ, Истина, ПараметрыПоиска.УправляющиеСимволы);
			ВариантыПоиска = Новый Массив;
			...
			ПодходящиеЗначенияДляБулево.Добавить(ВариантыПоиска, ПредставлениеРезультатПоискаКлиент(
				ПолеПоиска.Значение, СвойстваПоиска.ВидСравнения, СвойстваПоиска.ПравоеЗначение));
		КонецЦикла;
	ИначеЕсли СтрСравнить(ПараметрыПоиска.СтрокаПоиска, НСтр("ru = 'Нет';
															|en = 'No'")) = 0 
		Или СтрСравнить(ПараметрыПоиска.СтрокаПоиска, НСтр("ru = 'Ложь';
															|en = 'False'")) = 0 
		Или СтрСравнить(ПараметрыПоиска.СтрокаПоиска, НСтр("ru = 'Выключено';
															|en = 'Disabled'")) = 0 Тогда
		Для Каждого ПолеПоиска Из ПоляПоиска Цикл
			СвойстваПоиска = СвойстваУниверсальногоПоиска(ПолеПоиска.Ключ, Ложь, ПараметрыПоиска.УправляющиеСимволы);
			ВариантыПоиска = Новый Массив;
...
Процедура НайтиПодходящиеЗначенияУниверсальногоПоискаСтроки(ПодходящиеЗначения, ПоляПоискаПоТипам, ПараметрыПоиска)
	...
	Если СтрСравнить(ПараметрыПоиска.СтрокаПоиска, НСтр("ru = 'Да';
		...
		Или СтрСравнить(ПараметрыПоиска.СтрокаПоиска, НСтр("ru = 'Включено';
															|en = 'Enabled'")) = 0
		Или СтрСравнить(ПараметрыПоиска.СтрокаПоиска, НСтр("ru = 'Нет';
															|en = 'No'")) = 0 
		Или СтрСравнить(ПараметрыПоиска.СтрокаПоиска, НСтр("ru = 'Ложь';
															|en = 'False'")) = 0 
		Или СтрСравнить(ПараметрыПоиска.СтрокаПоиска, НСтр("ru = 'Выключено';
...
Функция ПредставлениеУниверсальногоПоискаСсылки(Данные, ПолеПоиска, Условие, СтрокаПоиска)
	...
	Если ЗначениеЗаполнено(Данные.ДополнительныеДанные) Тогда
		ПредставлениеЗначения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 (%2)';
				|en = '%1 (%2)'"), Строка(Данные.Ссылка), Данные.ДополнительныеДанные);
	КонецЕсли;
...
Функция ШаблонФрагментаУсловия(Условие)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1<span style=""color: %2"">%3</span>%4';
			|en = '%1<span style=""color: %2"">%3</span>%4'"), " ", "СкрытыйВариантОтчетаЦвет", 
...
Функция ШаблонФрагментаСтрокиПоиска(СтрокаПоиска)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '<span style=""color: %1; font: %2"">%3</span>';
			|en = '<span style=""color: %1; font: %2"">%3</span>'"), "МоиВариантыОтчетовЦвет", "ВажнаяНадписьШрифт",
		СтрокаПоиска);
...
Процедура УстановитьВидимостьДоступность()
	...
	// Заголовок.
	ШаблонЗаголовка = НСтр("ru = 'Сервис прогнозирования продаж (%1)';
							|en = 'Sales forecasting service (%1)'"); // Сервис прогнозирования.
...
Процедура ПоказатьОшибкиФормирования(ИнформацияОбОшибке)
	Если ТипЗнч(ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда
		...
		ПодробноеПредставлениеОшибки = НСтр("ru = 'Отчет не сформирован по причине:';
											|en = 'The report is not generated due to:'") + Символы.ПС
	...
	Если Элементы.СохранитьВариант.Видимость Тогда
		...
		СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(
			...
			НСтр("ru = 'Сохранить вариант отчета?';
				|en = 'Do you want to save the report option?'"), Кнопки, ПараметрыВопроса);
		Возврат;
	Иначе
		ПараметрыФормы = Новый Структура;
	...
	Если ЗначениеЗаполнено(ВидПлана) Тогда
		...
		Если СервисПрогнозирования.ИдетОбменДанными() Тогда
			ТекстОшибки = НСтр("ru = 'Активна выгрузка данных. До ее окончания запрещен запуск прогнозирования.';
								|en = 'Data export is active. Cannot start forecasting before it is completed.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
			Возврат;
		КонецЕсли;
...
Функция ОтладочнаяИнформацияПоВидуПлана()
	...
	|%1
	|========================================
	|Model settings (can differ from the data on the server):
	|%2
	|========================================
	|General information (can differ from the data on the server):
	|Last known sales data (excluding filters): %3
	|Collection IDs:
	|%4
	|========================================
	...
	|en = 'Version: %1
	|Tokens are specified: %2
	|Token update date: %3
	|ID (application_uuid): %4'");
...
Функция ТекстОшибкиНеИспользуетсяПланированиеПродажПоНоменклатуре()
	...
	Возврат НСтр("ru = 'Прогнозирование по номенклатуре невозможно, поскольку в информационной базе не используется планирование продаж по номенклатуре.
		...
		|en = 'Cannot perform forecasting by items as sales forecasting by items is not used in the infobase.
		|Enable sales forecasts in Master data and settings -
...
Функция ТекстОшибкиНеИспользуетсяПланированиеПродажПоКатегориям()
	...
	Возврат НСтр("ru = 'Прогнозирование по категориям невозможно, поскольку в информационной базе не используется планирование продаж по категориям.
		...
		|Настройка НСИ и разделов - Бюджетирование и планирование - Планирование.';
		|en = 'Cannot perform forecasting by product categories as sales forecasting by categories is not used in the infobase.
		|Enable sales forecasts by categories in Master data and settings -
		|Master data and sections - Budgeting and planning - Planning.'");
