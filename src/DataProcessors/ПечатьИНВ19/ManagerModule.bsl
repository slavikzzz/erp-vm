...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ИНВ19") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru = 'Сличительная ведомость';
				|en = 'Inventory reconciliation report'"),
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ИНВ19_НаОснованииИнвентаризации") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"ИНВ19_НаОснованииИнвентаризации",
			НСтр("ru = 'Сличительная ведомость';
				|en = 'Inventory reconciliation report'"),
			СформироватьПечатнуюФормуИНВ19_НаОснованииИнвентаризации(СтруктураТипов, ОбъектыПечати, ПараметрыПечати));
...
Процедура ЗаполнитьТабличныйДокументИНВ19(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ИнвентаризационнаяОпись)
	...
	Иначе       		
		ТекстСообщения = НСтр("ru = 'Неверные данные для печати.';
								|en = 'Incorrect data for printing.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		Возврат;		
	КонецЕсли;
	...
	Если ТаблицаТоваров.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru = 'В рамках периода документа ""%Опись%"" не были оформлены складские акты. Нет данных для формирования печатной формы ""ИНВ-19""';
								|en = 'You have not created warehouse acts within the period of document ""%Опись%"". No data to generate INV-19 print form.'");
	...
	Для Каждого СтрокаТовар Из ТаблицаТоваровРезультат Цикл
		...
		Если НомерСтроки = 0 Тогда // первая строка
			...
			ТекстСтраница = НСтр("ru = 'Страница %НомерСтраницы% (продолжение)';
								|en = 'Page  %НомерСтраницы% (continue)'", Метаданные.Языки.Русский.КодЯзыка);
			ТекстСтраница = СтрЗаменить(ТекстСтраница,"%НомерСтраницы%",2);
...
Функция СформироватьПечатнуюФормуИНВ19_НаОснованииИнвентаризации(СтруктураТипов, ОбъектыПечати, ПараметрыПечати)
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		Если ВРег(СтруктураОбъектов.Ключ) <> ВРег("Документ.ИнвентаризацияТМЦВЭксплуатации") Тогда
			...
			ТекстСообщения = НСтр("ru = 'Формирование печатной формы ""ИНВ-19"" не доступно для данного вида документов %1.';
									|en = 'You cannot generate print form INV-19 for the %1 document kind.'");
		...
		Для Каждого ДокументОснование Из СтруктураОбъектов.Значение Цикл
			...
			ДанныеДляПечати = МенеджерОбъекта.ДанныеДляПечатнойФормыИНВ19(ПараметрыПечати, ДокументОснование);	
			Если НЕ ДанныеДляПечати = Неопределено Тогда
				ЗаполнитьТабличныйДокументИНВ19_НаОснованииИнвентаризации(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ДокументОснование);
			КонецЕсли;
...
//	ОбъектыПечати - см. УправлениеПечатьюПереопределяемый.ПриПечати.ОбъектыПечати.
//	ДокументОснование - ДокументСсылка.ИнвентаризацияТМЦВЭксплуатации - Документ
//
Процедура ЗаполнитьТабличныйДокументИНВ19_НаОснованииИнвентаризации(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ДокументОснование)
	
	Если ДанныеДляПечати.ТабличнаяЧасть.Пустой() Тогда
		ТекстСообщения = НСтр("ru = 'Не выявлено отклонений в инвентаризации %1';
								|en = 'No variance is found in the %1 physical inventory count'");
	...
	Пока СтрокаТовар.Следующий() Цикл
		...
		ДополнительныеПараметрыПолученияНаименованияДляПечати = НоменклатураКлиентСервер.ДополнительныеПараметрыПредставлениеНоменклатурыДляПечати();
		ДополнительныеПараметрыПолученияНаименованияДляПечати.ВозвратнаяТара = СтрокаТовар.ЭтоВозвратнаяТара;
		ДополнительныеПараметрыПолученияНаименованияДляПечати.КодОсновногоЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
		
		ПараметрыСтроки.НомерСтроки = НомерСтроки + 1;
		...
		ОбластьСтроки_Лист2.Параметры.Заполнить(СтрокаТовар);
		ОбластьСтроки_Лист2.Параметры.Заполнить(ПараметрыСтроки);
		...
		ИтоговыеСуммы.ИтогоРезультатИзлишекКолво = ИтоговыеСуммы.ИтогоРезультатИзлишекКолво + СтрокаТовар.КоличествоИзлишков;
		ИтоговыеСуммы.ИтогоРезультатИзлишекСумма = ИтоговыеСуммы.ИтогоРезультатИзлишекСумма + СтрокаТовар.СуммаИзлишков;
		ИтоговыеСуммы.ИтогоРезультатНедостачаКолво = ИтоговыеСуммы.ИтогоРезультатНедостачаКолво + СтрокаТовар.КоличествоНедостачи;
		ИтоговыеСуммы.ИтогоРезультатНедостачаСумма = ИтоговыеСуммы.ИтогоРезультатНедостачаСумма + СтрокаТовар.СуммаНедостачи;
