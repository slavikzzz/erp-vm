...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если БазаПодключена Тогда
		...
		Если Не ЗначениеЗаполнено(Email) Тогда
			Элементы.ДекорацияНеЗаполненEmailПользователя.Видимость = Истина;
			Элементы.ВвестиКодРегистрации.Доступность = Ложь;
			Элементы.ВвестиТокенДляРегистрации.Доступность = Ложь;
		КонецЕсли;
...
Процедура ПерейтиНаСтраницуРегистрации(Команда)
	...
	Если Не ЗначениеЗаполнено(АдресУСП) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Необходимо указать адрес службы поддержки';
				|en = 'Specify the technical support address'"), , "АдресУСП");
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Email) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Необходимо указать e-mail пользователя';
...
Процедура ОтменитьПодключение(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ОтменитьПодключениеПриПроверкеОтвета", ЭтотОбъект);
	...
	Если РезультатЗапроса.Свойство("ЕстьСтраницаСЗащитой")
		...
		ТекстСообщения = ТекстСообщения 
			...
			+ НСтр("ru = 'Код, отправленный в письме, пользователи могут использовать для работы с обращениями.';
					|en = 'You can use the code from the email to manage requests.'");
...
Процедура ВвестиТокенДляРегистрации(Команда)
	...
	ТекстНеверныйТокен = НСтр("ru = 'Неверный токен для регистрации.';
								|en = 'Incorrect registration token.'");
	...
	Если ДанныеРасшифровкиТокена.Данные.email <> Email Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Токен предназначен для пользователя с другим адресом электронной почты.';
				|en = 'The token is designed for a user with a different email address.'"),
			,
			"ТокенДляРегистрации");
		Возврат;
	КонецЕсли;
...
Функция ВвестиКодРегистрацииНаСервере()
	...
	Попытка
		...
		Если Ответ.КодСостояния <> 200 Тогда
			...
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка %1';
										|en = 'Error %1'", ОбщегоНазначения.КодОсновногоЯзыка()), Строка(Ответ.КодСостояния));
...
Процедура ЗафиксироватьОшибку(Результат, ПодробныйТекстОшибки, Знач КраткийТекстОшибки = "")
	...
	ЗаписьЖурналаРегистрации(
		СтрШаблон("%1.%2", ИмяСобытияЖурналаРегистрации(), 
			НСтр("ru = 'Ввод кода регистрации';
				|en = 'Enter registration code'", ОбщегоНазначения.КодОсновногоЯзыка())),
...
Процедура ОпределитьПоддерживаемыеБазыВФонеЗавершение(Операция, ДополнительныеПараметры) Экспорт
	...
	Если Операция.Статус = "Выполнено" Тогда
		...
		Иначе
			Элементы.НадписьСостояниеПодключения.Заголовок = НСтр("ru = 'Ошибка подключения: ';
																	|en = 'Connection error: '") + Результат.ТекстСообщения;
...
Процедура ПроверитьИВывестиОшибкиОтправкиПисем(АдресаНетПользователяУАбонента)
	...
	ШаблонСообщения = 
		НСтр("ru = 'Пользователя с адресом %1 нет в базе службы поддержки, поэтому ему не отправлено письмо.';
			|en = 'The user with %1 is not in the support database so the message was not sent to them.'");
