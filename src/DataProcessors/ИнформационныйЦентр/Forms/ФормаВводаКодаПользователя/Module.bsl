...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Email) Тогда
		...
		Элементы.ВвестиКод.Доступность = Ложь;
		Элементы.ЗапроситьКод.Доступность = Ложь;
	КонецЕсли;
...
Процедура ЗафиксироватьОшибку(ТекстОшибки)
	...
	ЗаписьЖурналаРегистрации(
		СтрШаблон("%1.%2", ИмяСобытияЖурналаРегистрации(), 
			НСтр("ru = 'Ввод кода пользователя';
				|en = 'Enter user code'", ОбщегоНазначения.КодОсновногоЯзыка())),
