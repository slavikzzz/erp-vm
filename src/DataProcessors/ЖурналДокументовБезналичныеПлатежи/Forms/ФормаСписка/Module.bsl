...
Процедура УстановитьПроведениеБанком(Команда)
	...
	ПоказатьВводДаты(ПриУстановкеПроведенияБанком, ТекущаяДата(), НСтр("ru = 'Укажите дату проведения банком';
																		|en = 'Specify a date of processing by bank'"), ЧастиДаты.Дата);
...
Процедура ЗаполнитьКомандыСозданияДокументов()
	...
	ХозОперацииПоФО.Вставить(Перечисления.ХозяйственныеОперации.ВнутренняяПередачаДенежныхСредств,              ИспользоватьОбособленныеПодразделения);
	//++ НЕ УТ
	ХозОперацииПоФО.Вставить(Перечисления.ХозяйственныеОперации.ОплатаАрендодателю,                             ИспользуетсяУправлениеВНА);
	ХозОперацииПоФО.Вставить(Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствОтАрендодателя,           ИспользуетсяУправлениеВНА);
	//-- НЕ УТ
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.ПоступлениеБезналичныхДенежныхСредств) Тогда
		...
		//++ НЕ УТ
		Операции.Добавить(Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствОтАрендодателя, НСтр("ru = 'Возврат от арендодателя';
																										|en = 'Lessor refund'"));
		//-- НЕ УТ
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.СписаниеБезналичныхДенежныхСредств) Тогда
		...
		//++ НЕ УТ
		Операции.Добавить(Перечисления.ХозяйственныеОперации.ОплатаАрендодателю, НСтр("ru = 'Оплата арендодателю';
																						|en = 'Payment to lessor'"));
		//-- НЕ УТ
	КонецЕсли;
	ГруппаОпераций = Новый Структура("Операции, ИмяГруппы", Операции, "Оплата");
	ГруппыОпераций.Добавить(ГруппаОпераций);
...
Процедура УстановитьОтборДинамическихСписков()
	...
	Если ЗначениеЗаполнено(БанковскийСчетОтбор) Тогда
		...
		Если РеквизитыСчета.ЭтоГруппа Тогда
			ТекстНадписи = СтрШаблон(НСтр("ru = 'Банковские счета каталога ""%1"" по организации %2';
											|en = 'Bank accounts of the ""%1"" directory by the %2 entity'"),
		...
		Иначе
			...
			Элементы.НадписьБанковскийСчет.Заголовок =
				СтрШаблон(НСтр("ru = 'Банковский счет %1, %2';
								|en = 'Bank account %1, %2'"), Строка(ВалютаСчета), СокрЛП(БанковскийСчетОтбор));
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НачалоПериода) И Не ЗначениеЗаполнено(КонецПериода) Тогда
		Элементы.НадписьПериодВыписки.Заголовок = НСтр("ru = 'За период <без ограничения>';
														|en = 'For period <without restriction>'")
	Иначе
...
Процедура ОбновитьГиперссылкиКОформлению()
	...
	Если ЭлементСписка <> Неопределено Тогда
		ЭлементСписка.Представление = Новый ФорматированнаяСтрока(
				НСтр("ru = 'Зачисление на лицевые счета';
					|en = 'Deposit into personal accounts'"), , ЦветаСтиля.ГиперссылкаЦвет, , "ЗачислениеДС");
	КонецЕсли;
...
Процедура ОбработкаПолученияОперацийПоЯндексКассеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекстСообщения = ?(СчетчикДокументов > 0, НСтр("ru = 'Операций по Яндекс.Кассе загружено: %1';
													|en = 'Yandex.Checkout transactions loaded: %1'")
		, НСтр("ru = 'Новых операций по Яндекс.Кассе не было';
				|en = 'There are no new Yandex.Checkout transactions'"));
...
Процедура ОбновитьГиперссылкиКОформлениюЛокализация()
		...
		Если ЭлементСписка <> Неопределено Тогда
			ЭлементСписка.Представление = Новый ФорматированнаяСтрока(
				НСтр("ru = 'Валютный контроль';
					|en = 'Foreign exchange control'"),, ЦветаСтиля.ГиперссылкаЦвет,, "ВалютныйКонтроль");
		КонецЕсли;
	КонецЕсли;
	...
	Если ЭлементСписка <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ЗаголовокПерепискаСБанками) Тогда
			Если ЗаголовокПерепискаСБанками = НСтр("ru = 'Переписка с банками';
													|en = 'Correspondence with banks'") Тогда
				ЭлементСписка.Представление = Новый ФорматированнаяСтрока(
