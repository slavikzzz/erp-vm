...
Процедура ВывестиТекстПредупреждения()
	...
	ШаблонПредупреждения = НСтр("ru = 'Не удалось выполнить действие, так как в %1 не проверены полномочия доверенностей.
		...
		|en = 'Cannot perform the action as authorization letter authorities are not checked in %1.
		|Request a document with another signature from the counterparty or check it manually and perform the action again if you trust the signer.'");
...
Процедура ЗаполнитьТаблицуДоверенностей(ДанныеПроверки)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(СтрокаДоверенности.Доверенность) = Тип("СправочникСсылка.МЧД003") Тогда
			СтрокаДоверенности.ТекстПолномочий =
				Справочники.МЧД003.ТекстПолномочий(СтрокаДоверенности.Доверенность);
		Иначе
			СтрокаДоверенности.ТекстПолномочий =
...
Процедура ОбновитьПрогресс()
	...
	ШаблонПрогресса = НСтр("ru = 'Проверено подписей %1 из %2.';
							|en = '%1 out of %2 signatures are checked.'");
