...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не МашиночитаемыеДоверенности.ИмеетсяПравоИзмененияМЧД() Тогда
		ТекстСообщенияНедостаткаПрав = НСтр("ru = 'Недостаточно прав для ручной проверки полномочий.';
											|en = 'Insufficient rights to check the authorities manually.'");
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ЭтоОперацияЭДО", Ложь) Тогда
		Заголовок = НСтр("ru = 'Действие не выполнено';
						|en = 'The action is not performed'");
	Иначе
	...
	ТекстПричины = НСтр("ru = 'В документе не проверены полномочия доверенности';
						|en = 'Authorization letter authorities are not checked in the document'");
	...
	Если Элементы.ДекорацияПредупреждение.Видимость Тогда
		...
		ШаблонПредупреждения = НСтр("ru = 'Не удалось выполнить действие. %1.
			...
			|en = 'Cannot perform the action. %1.
			|Check manually and try again.'");
