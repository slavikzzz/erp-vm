...
Процедура ВыгрузитьДанныеОбъектаМетаданных()
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Выгрузка загрузка данных. Выгрузка объекта метаданных';
			|en = 'Data Export and Import. Export the metadata object'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		ТекущийОбъектМетаданных,
		,
		СтрШаблон(НСтр("ru = 'Начало выгрузки данных объекта метаданных: %1';
						|en = 'Starting to export metadata object data: %1'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ЗавершитьЗаписьФайла(ПотокЗаписи)
	...
	ЗаписьЖурналаРегистрации(
		...
		|Exported objects: %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПроверитьДублиПредопределенных(ПоддерживаетПредопределенныеЭлементы)
	...
	Если РаботаВМоделиСервиса.РазделениеВключено() Тогда
		...
		|en = 'Contact the technical support'"); 
	Иначе
		ТекстРекомендации = НСтр(
