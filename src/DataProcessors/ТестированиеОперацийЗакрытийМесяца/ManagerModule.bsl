...
Функция РазделыТестирования() Экспорт
	...
	РазделыТестирования.Добавить("ВзаиморасчетыСПартнерами", " " + НСтр("ru = 'Взаиморасчеты с партнерами';
																		|en = 'AR/AP accounting with partners'"), Истина);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4") Тогда
		РазделыТестирования.Добавить("ВНА", " " + НСтр("ru = 'Формирование движений по ВНА';
														|en = 'Generation of CA records'"), Истина);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьРеглУчет") Тогда
		РазделыТестирования.Добавить("РеглУчет", " " + НСтр("ru = 'Отражение в регл. учете';
															|en = 'Record in local accounting'"), Истина);
	КонецЕсли;
	//-- НЕ УТ
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьМеждународныйФинансовыйУчет") Тогда
		РазделыТестирования.Добавить("МеждународныйУчет", " " + НСтр("ru = 'Отражение в международном учете';
																	|en = 'General ledger posting'"), Истина);
	КонецЕсли;
	//-- НЕ УТКА
...
Функция СоответствиеКонтрольныхРегистров(ВыбранныеРазделы = Неопределено) Экспорт
	...
	Если ВыбранныеРазделы.Найти("Себестоимость") <> Неопределено Тогда // Себестоимость
		...
		//++ Локализация
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.ПартииНезавершенногоПроизводства.ПолноеИмя(), "Себестоимость");
		//-- Локализация
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.ТрудозатратыНезавершенногоПроизводства.ПолноеИмя(), "Себестоимость");
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.ПрочиеРасходыНезавершенногоПроизводства.ПолноеИмя(), "Себестоимость");
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.МатериалыИРаботыВПроизводстве.ПолноеИмя(), "Себестоимость");
		РегистрыКонтроля.Вставить(Метаданные.РегистрыСведений.ДолиСписанияКосвенныхРасходов.ПолноеИмя(), "Себестоимость");
		//-- НЕ УТ
	КонецЕсли;
	...
	Если ВыбранныеРазделы.Найти("УСН") <> Неопределено Тогда //УСН
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.РасходыПриУСН.ПолноеИмя(), "УСН");
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.РасходыПриУСНМатериалы.ПолноеИмя(), "УСН");
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.РасходыПриУСНТовары.ПолноеИмя(), "УСН");
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.РасходыПриУСНОСиНМА.ПолноеИмя(), "УСН");
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.КнигаУчетаДоходовИРасходов.ПолноеИмя(), "УСН");
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.КнигаУчетаДоходовИРасходовРаздел2.ПолноеИмя(), "УСН");
	КонецЕсли;
	
	Если ВыбранныеРазделы.Найти("ВНА") <> Неопределено Тогда //ВНА
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.СтоимостьОС.ПолноеИмя(), "ВНА");
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.СтоимостьНМА.ПолноеИмя(), "ВНА");
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.АмортизацияНМА.ПолноеИмя(), "ВНА");
		РегистрыКонтроля.Вставить(Метаданные.РегистрыНакопления.АмортизацияОС.ПолноеИмя(), "ВНА");
	КонецЕсли;
	//-- НЕ УТ
	...
	Если ВыбранныеРазделы.Найти("РеглУчет") <> Неопределено Тогда //Регл. учет
		РегистрыКонтроля.Вставить(Метаданные.РегистрыБухгалтерии.Хозрасчетный.ПолноеИмя(), "РеглУчет");
	КонецЕсли;
	//-- НЕ УТ
	...
	Если ВыбранныеРазделы.Найти("МеждународныйУчет") <> Неопределено Тогда //Международный учет
		РегистрыКонтроля.Вставить(Метаданные.РегистрыБухгалтерии.Международный.ПолноеИмя(), "МеждународныйУчет");
	КонецЕсли;
	//-- НЕ УТКА
	...
	Для каждого СтрокаЭтапа Из ПланТестирования.Строки Цикл
		...
		Если СтрокаЭтапа.ИмяУзла = "ПереходНаНовыеВзаиморасчеты" Тогда
			...
			ДлительныеОперации.СообщитьПрогресс(15, НСтр("ru = 'Заполнение регистров по расчетным документам';
														|en = 'Fill registers by AR/AP documents'"));
...
Процедура СообщитьОбОшибке()
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Тестирование';
									|en = 'Testing'", ОбщегоНазначения.КодОсновногоЯзыка())
		 	+ "." + НСтр("ru = 'Ошибка тестирования';
							|en = 'Test error'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ЗакрытиеМесяцаСДаты(Параметры, Разделы = Неопределено)
	...
	Пока ТестируемыйМесяц <= Параметры.КонецРасчета Цикл
		...
		ТестированиеЗакрытияМесяца = НЕ (Разделы.Количество() = 1 И Разделы.Получить(0) = "РеглУчет");
		Если Не ТестированиеЗакрытияМесяца Тогда
			ПараметрыОтражения = РеглУчетПроведениеСервер.ПараметрыОтраженияВРеглУчете();
			ПараметрыОтражения.ПериодРасчета = КонецМесяца(ТестируемыйМесяц);
	...
	Для Каждого СтрокаПериода Из ТаблицаПериодов Цикл
		...
		Для Каждого РазделРасчета Из ВыбранныеРазделы Цикл
			...
			Если РазделРасчета = "ВНА" Тогда
				...
				НачалоЗамера4 = НачатьЗамерВремени(Параметры, "Установить-ВНА");
				ВыборкаЗаданийАмортизацияОС = ЗаданияАмортизацииОС(Период, ОрганизацииПоПериоду).Выбрать();
				РегистрыСведений.ЗаданияКРасчетуАмортизацииОС.СоздатьЗаписиРегистраПоДаннымВыборки(ВыборкаЗаданийАмортизацияОС);
				ВыборкаЗаданийАмортизацияНМА = ЗаданияАмортизацииНМА(Период, ОрганизацииПоПериоду).Выбрать();
				РегистрыСведений.ЗаданияКРасчетуАмортизацииНМА.СоздатьЗаписиРегистраПоДаннымВыборки(ВыборкаЗаданийАмортизацияНМА);
				ЗавершитьЗамерВремени(Параметры, "Установить-ВНА", НачалоЗамера4);
				
			КонецЕсли;
			
			Если РазделРасчета = "УСН" Тогда
				
				НачалоЗамера5 = НачатьЗамерВремени(Параметры, "Установить-УСН");
				...
				РегистрыСведений.ЗаданияКЗакрытиюМесяца.СоздатьЗаписьРегистра(Период,, ОрганизацииПоПериоду, ОперацииУСН);
				ЗавершитьЗамерВремени(Параметры, "Установить-УСН", НачалоЗамера5);
				
			КонецЕсли;
			
			Если РазделРасчета = "РеглУчет" Тогда
				НачалоЗамера6 = НачатьЗамерВремени(Параметры, "Установить-РеглУчет");
				...
				ИсключаемыеТипы = ОбщегоНазначения.СкопироватьРекурсивно(ТипыДокументовКПоследовательномуОтражению);
				...
				РегистрыСведений.ОтражениеДокументовВРеглУчете.ЗарегистрироватьДокументыКОтражению(
					НачалоМесяца(Период), КонецМесяца(Параметры.КонецРасчета), ОрганизацииПоПериоду, ТипыДляСнятияОтражения);
				ЗавершитьЗамерВремени(Параметры, "Установить-РеглУчет", НачалоЗамера6);
			КонецЕсли;
			//-- НЕ УТ
			...
			//++ НЕ УТКА
			Если РазделРасчета = "МеждународныйУчет" Тогда
				НачалоЗамера7 = НачатьЗамерВремени(Параметры, "Установить-МеждународныйУчет");
				МеждународныйУчетЗарегистрироватьДокументыКОтражению(НачалоМесяца(Период), КонецМесяца(Параметры.КонецРасчета), ОрганизацииПоПериоду);
				ЗавершитьЗамерВремени(Параметры, "Установить-МеждународныйУчет", НачалоЗамера7);
			КонецЕсли;
			//-- НЕ УТКА
...
Функция ЗаданияАмортизацииОС(Период, МассивОрганизаций)
	...
	|	НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|ИЗ
	|	РегистрНакопления.СтоимостьОС.Остатки(НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ), Организация В (&МассивОрганизаций)) КАК СтоимостьОСОстатки";
...
Функция ЗаданияАмортизацииНМА(Период, МассивОрганизаций)
	...
	|	НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|ИЗ
	|	РегистрНакопления.СтоимостьНМА.Остатки(НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ), Организация В (&МассивОрганизаций)) КАК СтоимостьНМАОстатки";
