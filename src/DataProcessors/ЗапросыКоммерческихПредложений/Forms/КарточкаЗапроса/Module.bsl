...
Процедура ЗаполнитьДанныеЗапроса(АдресРезультата)
	...
	Элементы.ТоварыМаксимальнаяЦена.Заголовок = СтрШаблон(НСтр("ru = 'Максимальная цена (%1)';
																|en = 'Maximum price (%1)'"), Валюта);
...
Процедура ЗаполнитьНаОснованииДерева(ДанныеВходящегоДокумента)
	...
	Если Валюта <> ВалютаДокумента И ЗначениеЗаполнено(ВалютаДокумента) Тогда
		Элементы.НадписьВалюта.Заголовок = СтрШаблон(НСтр("ru = 'Валюта: %1 (запрос размещен в %2)';
															|en = 'Currency: %1 (request placed in %2)'"), Валюта, ВалютаДокумента);
	Иначе
		Элементы.НадписьВалюта.Заголовок = СтрШаблон(НСтр("ru = 'Валюта: %1';
	...
	Если ДанныеКонтактногоЛица.Свойство("ЭлектроннаяПочта") И ДанныеКонтактногоЛица.ЭлектроннаяПочта <> Неопределено Тогда
		ПодстрокиКонтактнойИнформации.Добавить(СтрШаблон(НСтр("ru = 'email: %1';
																|en = 'email: %1'"), ДанныеКонтактногоЛица.ЭлектроннаяПочта));
	КонецЕсли;
	
	Если ДанныеКонтактногоЛица.Свойство("Телефон") И ДанныеКонтактногоЛица.Телефон <> Неопределено Тогда
		ПодстрокиКонтактнойИнформации.Добавить(СтрШаблон(НСтр("ru = 'телефон: %1';
																|en = 'phone: %1'"), ДанныеКонтактногоЛица.Телефон));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СведенияОФайлах.Значение) Тогда
		...
		Элементы.ДекорацияФайлы.Заголовок = СтрШаблон(НСтр("ru = 'Файлы (%1)';
															|en = 'Files (%1)'"), СведенияОФайлах.Строки.Количество());
...
Процедура СохранениеЗапросаКоммерческихПредложенийЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не удалось сохранить запрос коммерческих предложений';
															|en = 'Cannot save request for quotation'"))
		КонецЕсли;
	Иначе
	...
	Иначе
		ПоказатьОповещениеПользователя(НСтр("ru = 'Успешно';
											|en = 'Done'"),, НСтр("ru = 'Документ успешно обновлен';
																	|en = 'The document is successfully updated'"));
	КонецЕсли;
...
Процедура ВыполнитьЗапросВСервисе()
	...
	НаименованиеФоновогоЗаданияПолучениеДанных = НСтр("ru = 'Получение запроса коммерческих предложений из сервиса 1С:Бизнес-Сеть';
														|en = 'Get request for quotation from the service 1C:Business Network'");
