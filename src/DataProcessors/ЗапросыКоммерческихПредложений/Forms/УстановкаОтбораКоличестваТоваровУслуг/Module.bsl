...
Процедура ОК(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ЕдиницаИзмерения)
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Не выбрана единица измерения отбора.';
													|en = 'Filter unit of measure is not selected.'"), 60);
		Возврат;
	КонецЕсли;
	...
	Если НеверныйИнтервал Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Указан неверный диапазон значений.';
													|en = 'Invalid value range is specified.'"), 60);
		Возврат;
	КонецЕсли;
