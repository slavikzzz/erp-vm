...
Процедура ТребуетсяПомощьНажатие(Элемент)
	...
	РаботаСФайламиСлужебныйКлиент.ПоказатьОшибкуСканирования(ЭтотОбъект, НСтр("ru = 'Проблема при сканировании';
		...
		НСтр("ru = 'Вызвана помощь из окна сканирования.';
			|en = 'Help is called in the scanner window.'"), Истина);
...
Процедура ПреобразоватьПеречисленияВПараметрыИПолучитьПредставление()
	...
	Если Не ПоказыватьДиалогСканера Тогда
		...
		Если ВРег(ФорматКартинки) = "JPG" Тогда
			Представление = Представление +  НСтр("ru = 'Качество:';
													|en = 'Quality:'") + " " + Строка(КачествоJPG) + ". ";
		КонецЕсли;	
		
		Если ВРег(ФорматКартинки) = "TIF" Тогда
			Представление = Представление +  НСтр("ru = 'Сжатие:';
													|en = 'Compression:'") + " " + Строка(СжатиеTIFFПеречисление) + ". ";
		КонецЕсли;
		...
		Если НЕ ПоворотПеречисление.Пустая() Тогда
			Представление = Представление +  НСтр("ru = 'Поворот:';
													|en = 'Rotation:'")+ " " + Строка(ПоворотПеречисление) + ". ";
		КонецЕсли;	
		
		Если НЕ РазмерБумагиПеречисление.Пустая() Тогда
			Представление = Представление +  НСтр("ru = 'Размер бумаги:';
													|en = 'Paper size:'") + " " + Строка(РазмерБумагиПеречисление) + ". ";
		КонецЕсли;	
...
#Если НЕ ВебКлиент И НЕ МобильныйКлиент Тогда
	Если Источник = "TWAIN" Тогда
		РаботаСФайламиСлужебныйКлиент.ЗаписатьЖурналСканирования("СобытиеСканера", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Событие %1, данные %2';
																		|en = 'Event %1, data %2'"), Событие, Данные));
	КонецЕсли;
	...
	Если Источник = "TWAIN" И Событие = "ImageAcquired" Тогда
		...
		СтрокаТаблицы.Представление = НСтр("ru = 'Изображение';
											|en = 'Image'") + Строка(НарастающийНомерИзображения);
		...
		Если Открыта() Тогда
			...
			Если ТекущаяДата < НачалоСканирования + 3 И ТаблицаФайлов.Количество() = 0 Тогда
				ПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Сканирование отменено (событие %1) через %2 сек. См. %3';
						|en = 'Scanning is canceled (event %1) in %2 sec. See %3'"), Событие, 
				...
				РаботаСФайламиСлужебныйКлиент.ПоказатьОшибкуСканирования(ЭтотОбъект, 
					НСтр("ru = 'Не удалось отсканировать документ';
