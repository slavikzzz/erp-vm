...
&НаКлиенте
Процедура ПослеЗакрытияОбработкиПерехода(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ПроверитьВерсиюНаСервере()
	...
	Иначе
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось получить версию сервера ГИСМ, возможно, требуется обновление конфигурации.';
													|en = 'Не удалось получить версию сервера ГИСМ, возможно, требуется обновление конфигурации.'"));
	КонецЕсли;
...
Процедура ОбновитьВерсии()
	...
	Если Не ВерсияАктуальна Тогда
		ТекстСообщенияОВерсии = СтрШаблон(НСтр("ru = 'Текущая версия конфигурации не поддерживает актуальный формат обмена с ГИСМ (%1)';
												|en = 'Текущая версия конфигурации не поддерживает актуальный формат обмена с ГИСМ (%1)'"), ВерсииСхемОбмена.Сервер);
	Иначе
		ТекстСообщенияОВерсии = СтрШаблон(НСтр("ru = 'Обмен с ГИСМ выполняется с использованием актуального формата обмена (%1)';
												|en = 'Обмен с ГИСМ выполняется с использованием актуального формата обмена (%1)'"), ВерсииСхемОбмена.Сервер);
