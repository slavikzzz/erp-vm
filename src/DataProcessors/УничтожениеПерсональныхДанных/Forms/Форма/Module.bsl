...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(Не ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ(),
		НСтр("ru = 'Все операции по уничтожению персональных данных выполняются только в центральном узле.';
			|en = 'All operations for destroying personal data are performed only in the master node.'"));
...
Процедура СоздатьАкты(Команда)
	...
	Если СубъектыИОрганизации.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Для создания актов выделите строки в списке';
										|en = 'To create certificates, select lines in the list'"));
		Возврат;
	КонецЕсли;
...
Процедура ФормаСозданияАктовПослеЗакрытия(Результат, СубъектыИОрганизации) Экспорт
	...
	ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Формируются акты об уничтожении персональных данных';
											|en = 'Generating certificates of personal data destruction'");
