...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СостояниеПереходаИСМП = ИнтеграцияГИСМ.ПодсистемаНеИспользуется();
	...
	Элементы.РаботаСГИСМЗавершается.Видимость = Не СостояниеПереходаИСМП;
	Элементы.РаботаСГИСМЗавершена.Видимость   = СостояниеПереходаИСМП;
...
&НаКлиенте
Процедура РаботаСГИСМЗавершаетсяОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	Если НавигационнаяСсылкаФорматированнойСтроки = "ПерейтиВПанельОбменИСМП" Тогда
...
&НаКлиенте
Процедура РаботаСГИСМЗавершенаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	Если НавигационнаяСсылкаФорматированнойСтроки = "ПерейтиВПанельОбменИСМП" Тогда
		СтандартнаяОбработка = Ложь;
		ОткрытьФорму("Обработка.ПанельОбменИСМП.Форма.Форма", , ЭтотОбъект);
	КонецЕсли;
...
Процедура ОткрытьВидыНоменклатуры(Команда)
	...
	ИнтеграцияИСКлиентПереопределяемый.ОткрытьФормуСпискаВидыНоменклатуры(ЭтаФорма);
...
Процедура ОткрытьНоменклатуру(Команда)
	...
	ИнтеграцияИСКлиентПереопределяемый.ОткрытьФормуСпискаНоменклатуры(ЭтаФорма);
...
Процедура НастройкаМаркировки(Команда)
	...
	ОткрытьФорму("Обработка.ПанельАдминистрированияГИСМ.Форма.НастройкиГИСМ", , ЭтаФорма);
...
Процедура ОбновитьСпискиДокументов()
		...
		Если ДоступенОтчетОРозничныхПродажах Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаОтчетыОРозничныхПродажах" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьОтчетыОРозничныхПродажах, НСтр("ru = 'Уведомления о розничных продажах';
																							|en = 'Уведомления о розничных продажах'"));
			КонецЕсли;
		...
		Если ДоступенВозвратТоваровОтРозничногоКлиента Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаВозвратыТоваровОтРозничныхКлиентов" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьВозвратыТоваровОтРозничныхКлиентов, НСтр("ru = 'Уведомления о розничных возвратах';
																									|en = 'Уведомления о розничных возвратах'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОПоступленииМаркированныхТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОПоступлении" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОПоступленииМаркированныхТоваров, НСтр("ru = 'Уведомления о поступлении и возврате';
																											|en = 'Уведомления о поступлении и возврате'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОВвозеМаркированныхТоваровИзЕАЭСГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОВвозеИзЕАЭС" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОВвозеМаркированныхТоваровИзЕАЭС, НСтр("ru = 'Уведомления о ввозе из ЕАЭС';
																											|en = 'Уведомления о ввозе из ЕАЭС'"));
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстЗапроса) Тогда
		...
		Если Выборка.Следующий() И Выборка.КоличествоДокументов > 0 Тогда
			ТекстЗаголовка = СтрШаблон(НСтр("ru = 'Есть документы (%1), ожидающие отправки в ГИСМ. Выполните обмен.';
											|en = 'Есть документы (%1), ожидающие отправки в ГИСМ. Выполните обмен.'"), Выборка.КоличествоДокументов);
...
&НаКлиенте
Функция ПараметрыОткрытияФормыСпискаПрикладногоДокумента(ДальнейшееДействие)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Ответственный", Ответственный);
	ПараметрыФормы.Вставить("Организация", Организация);
	ПараметрыФормы.Вставить("ДальнейшееДействиеГИСМ", ДальнейшееДействие);
	ПараметрыФормы.Вставить("ОткрытьРаспоряжения", Ложь);
	
	ПараметрыФормы.Вставить("ИмяФормы", "");
	ПараметрыФормы.Вставить("ИмяПоляОтветственный");
	ПараметрыФормы.Вставить("ИмяПоляОрганизация");
...
Процедура ОткрытьОтчетыОРозничныхПродажах(Команда)
	...
	ПараметрыФормы = ПараметрыОткрытияФормыСпискаПрикладногоДокумента("ВсеТребующиеДействияИлиОжидания");
	ИнтеграцияГИСМКлиентПереопределяемый.ПараметрыОткрытияСпискаОтчетыОРозничныхПродажах(ПараметрыФормы);
	Если Не ЗначениеЗаполнено(ПараметрыФормы.ИмяФормы) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьОтчетыОРозничныхПродажахОтработайте(Команда)
	...
	ПараметрыФормы = ПараметрыОткрытияФормыСпискаПрикладногоДокумента("ВсеТребующиеДействия");
	ИнтеграцияГИСМКлиентПереопределяемый.ПараметрыОткрытияСпискаОтчетыОРозничныхПродажах(ПараметрыФормы);
	Если Не ЗначениеЗаполнено(ПараметрыФормы.ИмяФормы) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьОтчетыОРозничныхПродажахОжидайте(Команда)
	...
	ПараметрыФормы = ПараметрыОткрытияФормыСпискаПрикладногоДокумента("ВсеТребующиеОжидания");
	ИнтеграцияГИСМКлиентПереопределяемый.ПараметрыОткрытияСпискаОтчетыОРозничныхПродажах(ПараметрыФормы);
	Если Не ЗначениеЗаполнено(ПараметрыФормы.ИмяФормы) Тогда
		Возврат;
	КонецЕсли;
