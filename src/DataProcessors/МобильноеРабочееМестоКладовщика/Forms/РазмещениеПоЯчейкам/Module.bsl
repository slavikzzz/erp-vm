		...
		Если ИмяСобытия = "ScanData" И МенеджерОборудованияУТКлиент.ЕстьНеобработанноеСобытие() Тогда
			...
			Иначе
				...
				Иначе
					...
					СообщениеПользователю.Текст = НСтр("ru = 'Штрихкод не найден:';
														|en = 'Barcode is not found:'") + Штрихкод;
...
Процедура ЗакончитьРазмещение(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Завершить размещение?';
									|en = 'Finish put-away?'"), СписокЗначений, 0);
...
Процедура СформироватьЗаголовкиКомандМеню()
	Элементы.КомандаРазместить.Заголовок = СтрШаблон(НСтр("ru = 'Разместить (%1)';
															|en = 'Put away (%1)'"), ТоварыРазместить.Количество());
	Элементы.КомандаРазмещено.Заголовок = СтрШаблон(НСтр("ru = 'Размещено (%1)';
														|en = 'Put away (%1)'"), ТоварыРазмещено.Количество());
КонецПроцедуры
...
Функция ЗакончитьРазмещениеНаСервере()
	...
	Если ЗонаПриемки.Пустая() Тогда
		...
		СообщениеПользователю.Текст = НСтр("ru = 'Не выбрана зона приемки';
											|en = 'Inbound area is not selected'");
		СообщениеПользователю.Сообщить();
