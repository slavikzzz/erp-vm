...
Процедура ЗакончитьПриемку(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Завершить приемку?';
									|en = 'Finish goods receipt?'"), СписокЗначений, 0);
...
Процедура СобратьВсе(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Вкладка Принято будет очищена, продолжить?';
									|en = 'The Received tab will be cleared. Continue?'"),РежимДиалогаВопрос.ДаНет,0);
КонецПроцедуры
...
Процедура СформироватьЗаголовкиКомандМеню()
	Элементы.КомандаКПоступлению.Заголовок = СтрШаблон(НСтр("ru = 'К приемке (%1)';
															|en = 'For goods receipt (%1)'"), ТоварыКПоступлению.Количество());
	Элементы.КомандаПринято.Заголовок = СтрШаблон(НСтр("ru = 'Принято (%1)';
														|en = 'Received (%1)'"), ТоварыПринято.Количество());
КонецПроцедуры
...
Функция ЗакончитьПриемкуНаСервере()
	...
	Если МассивСообщений.Количество() > 0 Тогда
		ТекстСообщения = НСтр("ru = 'Укажите упаковки для позиций:';
								|en = 'Specify packaging units for the items:'");
	...
	Если МассивСообщений.Количество() > 0 Тогда
		ТекстСообщения = НСтр("ru = 'Укажите серии для позиций:';
								|en = 'Specify batches for the items:'");
		Для Каждого ЭлСообщения Из МассивСообщений Цикл
			ТекстСообщения = ТекстСообщения + Символы.ПС + ЭлСообщения;
		КонецЦикла;
