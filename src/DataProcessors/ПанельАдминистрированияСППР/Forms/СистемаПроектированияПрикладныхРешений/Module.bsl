...
Процедура ПроектСППРНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(НаборКонстант.АдресВебСервисаСППР) 
		...
		ТекстПредупреждения = НСтр("ru = 'Для выбора проекта нужно заполнить параметры соединения с базой СППР.
										...
										|en = 'To select a project, fill in the ASDS base connection parameters.
										|After successful connection, select a project from the list of ASDS base projects.
										|If connection fails, enter the project name manually.'");
...
Процедура КомандаПроверитьСоединение(Команда)
	...
	Если ПустаяСтрока(НаборКонстант.АдресВебСервисаСППР) 
		...
		ТекстПредупреждения = НСтр("ru = 'Перед проверкой необходимо заполнить параметры:
										...
										| - имя пользователя';
										|en = 'Fill in parameters before check:
										| - URL of ASDS infobase
										| - username'");
  		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	...
	ПоказатьПредупреждение(, НСтр("ru = 'Соединение успешно установлено.';
									|en = 'Connection is successfully established.'"),, НСтр("ru = 'Проверка соединения с СППР';
																					|en = 'Check connection to ASDS'"));
