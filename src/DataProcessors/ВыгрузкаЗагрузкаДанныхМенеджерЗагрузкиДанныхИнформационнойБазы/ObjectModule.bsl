	...
	Иначе
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Выгрузка объекта метаданных не поддерживается %1';
				|en = 'Metadata object %1 import not supported'"),
...
Процедура ЗагрузитьДанныеОбъектаИнформационнойБазы(Знач ОбъектМетаданных)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Выгрузка загрузка данных. Загрузка объекта метаданных';
			|en = 'Data Export and Import. Import the metadata object'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		ОбъектМетаданных,
		,
		СтрШаблон(НСтр("ru = 'Начало загрузки данных объекта метаданных: %1';
						|en = 'Starting to import metadata object data: %1'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ОбъектМетаданных.ПолноеИмя()));
	...
	Если ОбщегоНазначения.ЭтоПланОбмена(ОбъектМетаданных) Тогда
		...
		ЗаписьЖурналаРегистрации(
			...
			ОбъектМетаданных,
			,
			СтрШаблон(НСтр("ru = 'Очистка данных объекта метаданных: %1';
							|en = 'Clear metadata object data: %1'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ОбъектМетаданных.ПолноеИмя()));
	...
	Для Каждого КлючИЗначение Из ОтложенныеНаборы Цикл
		...
		Исключение
			...
			Комментарий = СтрШаблон(
				НСтр("ru = 'Объекта метаданных %1 с представлением ""%2"" не загружен по причине: %3';
					|en = 'Metadata object %1 with presentation ""%2"" is not imported. Reason: %3'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ОбъектМетаданных.ПолноеИмя(),
				Строка(Объект),
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Выгрузка загрузка данных. Загрузка объекта метаданных.Ошибка';
					|en = 'Data export and import.Metadata object import.Error'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Комментарий = СтрШаблон(
		НСтр("ru = 'Окончание загрузки данных объекта метаданных: %1
					...
					|Imported objects: %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		ОбъектМетаданных.ПолноеИмя(), 
		КоличествоОбъектов);
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Выгрузка загрузка данных. Загрузка объекта метаданных';
...
Процедура ЗаписатьОбъектВИнформационнуюБазуВнутр(Объект, АртефактыОбъекта, ОтложенныеНаборы, ОбъектМетаданных)
	...
	Если Не Отказ Тогда
		...
		Исключение
			...
			Комментарий = СтрШаблон(
				НСтр("ru = 'Объекта метаданных %1 с представлением ""%2"" не загружен по причине: %3';
					|en = 'Metadata object %1 with presentation ""%2"" is not imported. Reason: %3'", КодЯзыка),
				ОбъектМетаданных.ПолноеИмя(),
				Строка(Объект),
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Выгрузка загрузка данных. Загрузка объекта метаданных.Ошибка';
					|en = 'Data export and import. Metadata object import.Error'", КодЯзыка),
				УровеньЖурналаРегистрации.Ошибка,
				ОбъектМетаданных,
				,
				Комментарий);
