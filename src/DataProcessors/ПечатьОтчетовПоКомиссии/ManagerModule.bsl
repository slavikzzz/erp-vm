...
Процедура ЗаполнитьРеквизитыПодвалаОтчетПоКомисии(ДанныеПечати,
	...
	Если ВыборкаПоДокументам.СуммаПродажи < 0 Тогда
		ТекстИтоговаяСтрока = НСтр("ru = 'Всего возвращено наименований';
									|en = 'All returned items'", ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		ТекстИтоговаяСтрока = НСтр("ru = 'Всего продано наименований';
									|en = 'All sold items'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
	...
	СтруктураПараметров.Вставить(
		...
		+ ", " + НСтр("ru = 'на сумму';
						|en = 'in the amount of'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
	...
	Если ДанныеПечати.СуммаВознаграждения <> 0 Тогда
		СтруктураПараметров.Вставить(
			"СуммаВознаграждения",
			НСтр("ru = 'Сумма комиссионного вознаграждения составила';
...
Процедура ЗаполнитьРеквизитыПодвалаОтчетПоКомиссииОСписании(ДанныеПечати,
	...
	СтруктураПараметров.Вставить(
		"ИтоговаяСтрока",
		НСтр("ru = 'Всего списано наименований';
			|en = 'All written off items'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
...
Процедура ЗаполнитьТабличныйДокументОтчетПоЗакупкам(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати) 
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ВыборкаПоДокументам.НайтиСледующий(СтруктураПоиска) Тогда
			...
			СуммаВознаграждения = ?(ДанныеПечати.ЕстьСтавкаВознаграждения, НСтр("ru = 'Сумма (с НДС)';
			...
			Пока СтрокаТовары.Следующий() Цикл
				...
				ПредставлениеСчетаФактурыВыданного = ?(СтрокаТовары.СчетФактураВыданныйПредставлениеНомера = NULL,
														...
														СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
															НСтр("ru = '№ %1 от %2';
																|en = 'No. %1 from %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ЗаполнитьРеквизитыПодвалаОтчетПоЗакупкам(ДанныеПечати, ВыборкаПоДокументам, Макет, ТабличныйДокумент, ЕстьРНПТ)
	...
	ТекстИтоговаяСтрока = НСтр("ru = 'Всего закуплено наименований';
								|en = 'Total purchased descriptions'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если ДанныеПечати.СуммаВознаграждения <> 0 Тогда
		СтруктураПараметров.Вставить(
			...
			+ НСтр("ru = 'от суммы закупки)';
					|en = 'of the purchase amount)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыШапкиОтчетПоЗакупкам(ДанныеПечати, Заголовок, Макет, ТабличныйДокумент)
	...
	ПредставлениеДоговора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '№ %1 от %2';
			|en = 'No. %1 from %2'", ОбщегоНазначения.КодОсновногоЯзыка()), ДанныеПечати.Договор.Номер, Формат(ДанныеПечати.Договор.Дата, "ДЛФ=DD"));
