...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.ИдентификаторТранспорта) Тогда
		...
		Текст = НСтр("ru = 'Не заданы настройки подключения. Синхронизация не может быть продолжена.';
					|en = 'Connection settings are not specified. Cannot continue the synchronization.'", 
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьПодтверждениеЗакрытияПроизвольнойФормы(ЭтотОбъект, Отказ, ЗавершениеРаботы,
		НСтр("ru = 'Выйти из помощника?';
			|en = 'Do you want to exit the wizard?'"), "ЗакрытьФормуБезусловно");
...
Процедура ДополнениеВыгрузкиИсторияОтборовВыборИзМеню(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ПредставлениеНастройки)=Тип("Строка") Тогда
		...
		НСтр("ru = 'Восстановить настройки ""%1""?';
			|en = 'Do you want to restore ""%1"" settings?'"), ПредставлениеНастройки);
