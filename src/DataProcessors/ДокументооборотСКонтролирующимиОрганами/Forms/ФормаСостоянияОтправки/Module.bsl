...
Процедура ПодробнаяИнформацияНажатие(Элемент)
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Подробная информация отсутствует';
										|en = 'Подробная информация отсутствует'"));
	КонецЕсли;
...
Процедура ПечатьКвитанций(Команда)
	...
	ИначеЕсли НЕ ПротоколЕсть Тогда 
		ТекстПредупреждения = НСтр("ru = 'Отсутствуют протоколы для печати';
									|en = 'Отсутствуют протоколы для печати'");
	...
	Если ЗначениеЗаполнено(Отправка) Тогда
		...
		Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЗапросНаИнформационноеОбслуживаниеНалогоплательщика") Тогда
			ТекстВопроса = НСтр("ru = 'Выгрузить ответ с подписью?';
								|en = 'Выгрузить ответ с подписью?'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Выгрузить пакет документов для представления по месту требования?';
								|en = 'Выгрузить пакет документов для представления по месту требования?'");
		КонецЕсли;
...
Процедура Обновить(Команда)
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиРаспределенныйРеестр") Тогда
		...
		Если ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЗначениеРеквизитаОбъекта(
			Ссылка, "РежимыИспользования") = "ФТС" Тогда
...
Процедура ВыгрузитьЗаявлениеНаОтзыв(Команда)
	...
	Если ЗначенияРеквизитов.ФайлЗаявленияНаОтзыв = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Файл заявления на отзыв недоступен';
										|en = 'Файл заявления на отзыв недоступен'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыгрузитьПодписьЗаявленияНаОтзыв(Команда)
	...
	Если ЗначенияРеквизитов.ЭлектроннаяПодписьЗаявленияНаОтзыв = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Файл подписи заявления на отзыв недоступен';
										|en = 'Файл подписи заявления на отзыв недоступен'"));
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ВыгрузитьПодтверждениеДоставкиВСФР(Команда)
	
	ЗначенияРеквизитов = ЗначенияРеквизитовОбъекта(
		Отправка,
		"ИдентификаторОтправкиНаСервере, КвитанцияДоставкиВСФР",
		УникальныйИдентификатор);
	
	Если ЗначенияРеквизитов.КвитанцияДоставкиВСФР = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Подтверждение доставки оператором в СФР недоступно';
										|en = 'Подтверждение доставки оператором в СФР недоступно'"));
		Возврат;
	КонецЕсли;
	
	ИмяФайлаКвитанции = СтрШаблон(
		НСтр("ru = 'Доставлено в СФР %1.xml';
			|en = 'Доставлено в СФР %1.xml'"),
		ЗначенияРеквизитов.ИдентификаторОтправкиНаСервере);
	
	СохраняемыйФайл = Новый Структура;
	СохраняемыйФайл.Вставить("Адрес", 	ЗначенияРеквизитов.КвитанцияДоставкиВСФР);
	СохраняемыйФайл.Вставить("Имя", 	ИмяФайлаКвитанции);
	ОперацииСФайламиЭДКОКлиент.СохранитьФайлы(СохраняемыйФайл);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыгрузитьАрхивСОтчетомИКвитанциями(Команда)
	
	ЗначенияРеквизитов = ЗначенияРеквизитовОбъекта(
		Ссылка,
		"НомерДоверенности, Образ",
		УникальныйИдентификатор);
	
	Если ЗначенияРеквизитов.Образ = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Файл архива недоступен. Пометьте МЧД на удаление, затем загрузите МЧД из распределенного реестра';
										|en = 'Файл архива недоступен. Пометьте МЧД на удаление, затем загрузите МЧД из распределенного реестра'"));
		Возврат;
	КонецЕсли;
	
	СохраняемыйФайл = Новый Структура;
	СохраняемыйФайл.Вставить("Адрес", 	ЗначенияРеквизитов.Образ);
	СохраняемыйФайл.Вставить("Имя", 	ЗначенияРеквизитов.НомерДоверенности + ".zip");
	ОперацииСФайламиЭДКОКлиент.СохранитьФайлы(СохраняемыйФайл);
...
Процедура Подключаемый_ОсуществитьОбменПоОрганизацииИзЭтаповОтправки() Экспорт
	...
	ОписаниеОповещения 	= Новый ОписаниеОповещения("ПослеОбновления", ЭтотОбъект);
...
&НаКлиенте
Процедура ПослеОбновления(Результат, ДополнительныеПараметры) Экспорт
...
Процедура УстановитьШиринуТаблицыДляФТС()
	...
	Если ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ФТС
		И ТипЗнч(Ссылка) <> Тип("СправочникСсылка.МашиночитаемыеДоверенностиРаспределенныйРеестр") Тогда
...
Процедура ИзменитьОформлениеКнопкиПечать()
	...
	НеПоказыватьПечать = СтраницаЖурнала = Перечисления.СтраницыЖурналаОтчетность.ЕГРЮЛ
		...
		ИЛИ ДокументооборотСКОКлиентСервер.ПодсистемаЦПРРМЧДСуществует()
			И ТипЗнч(Ссылка) = Тип("СправочникСсылка." + ИмяТипаСправочникаМашиночитаемыеДоверенности);
...
Процедура УстановитьВидимостьКнопкиОбновить(КонтекстЭДОСервер, ТекущееСостояниеОтправки)
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиРаспределенныйРеестр") Тогда
		...
		Элементы.ПодробнаяИнформация.Видимость 	= (ТекущееСостояниеОтправки.КонтролирующийОрган =
			Перечисления.ТипыКонтролирующихОрганов.ФТС);
...
Процедура ВыгрузитьЦиклыОбмена()
	...
	ТекстВопроса = НСтр("ru = 'Выгрузить все файлы и подписи?';
						|en = 'Выгрузить все файлы и подписи?'");
...
Процедура УправлениеЭУ(Отказ = Ложь)
	...
	Если ТекущееСостояниеОтправки = Неопределено Тогда
		Сообщить(НСтр("ru = 'В базе отсутствуют файлы, необходимые для отображения состояния отправки. Проверьте, что файлы доступны и не были удалены.';
						|en = 'В базе отсутствуют файлы, необходимые для отображения состояния отправки. Проверьте, что файлы доступны и не были удалены.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ПрорисоватьТаблицу(ТекущееСостояниеОтправки)
	...
	Элементы.ДокументыРеализацииПолномочийНО.Заголовок = ?(КоличествоДокументовРеализацииПолномочийНО = 1,
		НСтр("ru = 'Квитанция о присвоении РНПТ';
			|en = 'Квитанция о присвоении РНПТ'"),
		СтрШаблон(
			НСтр("ru = 'Квитанции о присвоении РНПТ (%1)';
				|en = 'Квитанции о присвоении РНПТ (%1)'"),
...
Процедура ПрорисоватьТаблицуСостояний(ТаблицаСостояний)
	...
	ИмяТипаСправочникаМашиночитаемыеДоверенности = "МашиночитаемыеДоверенности";
	ЭтоМашиночитаемаяДоверенностьЕдиногоФормата = ДокументооборотСКОКлиентСервер.ПодсистемаЦПРРМЧДСуществует()
		И ТипЗнч(Ссылка) = Тип("СправочникСсылка." + ИмяТипаСправочникаМашиночитаемыеДоверенности);
	...
	Для каждого СтрокаТаблицыСостояний Из ТаблицаСостояний Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицыСостояний.КомментарийКСостоянию) И (НомерСтроки = НомерТекущегоЭтапа
			ИЛИ ЭтоМашиночитаемаяДоверенностьЕдиногоФормата) Тогда
		...
		Если ЗначениеЗаполнено(СтрокаТаблицыСостояний.НаименованиеПротокола) Тогда 
		 ...
		 Иначе
			...
			Если ЗначениеЗаполнено(СтрокаТаблицыСостояний.КомментарийКСостоянию) И (НомерСтроки = НомерТекущегоЭтапа
				ИЛИ ЭтоМашиночитаемаяДоверенностьЕдиногоФормата) Тогда // обход ошибки платформы
		...
		Если ЭтапПройден И НомерСтроки = НомерТекущегоЭтапа Тогда
			...
			ИначеЕсли СостояниеСдачиОтчетности = Перечисления.СостояниеСдачиОтчетности.ПоложительныйРезультатДокументооборота Тогда
				Если СтрокаТаблицыСостояний.ТекстНадписи = "Частично отозвано" Тогда
					ЦветФона = ЦветаСтиля.ЦветФонаЧастичногоОтзыва;
				Иначе
...
Процедура ПрорисоватьБлокЭтаповДокументовРеализацииПолномочий(ТекущееСостояниеОтправки)
	...
	Если Элементы.ГруппаПанельПриема.Видимость Тогда
		Элементы.КомандаОтказатьВПриеме.Видимость = НЕ ТребованияФНСВызовСервера.ПодтверждениеОтправилОператор(Ссылка);
	КонецЕсли;
...
Процедура ОтпределитьСоставМенюВыгрузка()
	...
	Элементы.ВыгрузитьПодтверждениеДоставкиВСФР.Видимость 		= Ложь;
	Элементы.ВыгрузитьАрхивСОтчетомИКвитанциями.Видимость 		= Ложь;
	...
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиРаспределенныйРеестр") Тогда
		СвойстваДоверенности = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "Статус, РежимыИспользования");
		СтатусОтправки = СвойстваДоверенности.Статус;
		ЭтоДоверенностьФТС = (СвойстваДоверенности.РежимыИспользования = "ФТС");
		...
		Элементы.ВыгрузитьОтправленныйОтчет.Видимость = НЕ ЭтоДоверенностьФТС И ОтчетОтправлен;
		Элементы.ВыгрузитьПодпись.Видимость = НЕ ЭтоДоверенностьФТС И ОтчетОтправлен;
		Элементы.ВыгрузитьЗаявлениеНаОтзыв.Видимость = НЕ ЭтоДоверенностьФТС И ОтзывОтчетаОтозван;
		Элементы.ВыгрузитьПодписьЗаявленияНаОтзыв.Видимость = НЕ ЭтоДоверенностьФТС И ОтзывОтчетаОтозван;
		Элементы.ВыгрузитьАрхивСОтчетомИКвитанциями.Видимость = НЕ ЭтоДоверенностьФТС И ОтчетОтправлен;
		Возврат;
	КонецЕсли;
			...
			Если ЭтоПодтверждениеВидаДеятельности
				...
				Иначе
					...
					Элементы.ВыгрузитьОтправленныйПакетДокументов.Заголовок = НСтр("ru = 'Отправленный отчет';
																					|en = 'Отправленный отчет'");
				КонецЕсли;
			КонецЕсли;
			
			Если ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ФСС Тогда
				Элементы.ВыгрузитьПодтверждениеДоставкиВСФР.Видимость = Отправка.ДоставленоВСФР;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьАдресФайлаПакета(
	...
	Иначе
		...
		ИмяФайлаПакета = НСтр("ru = 'Подпись';
								|en = 'Подпись'") + ОтправкаСсылка.ИмяФайлаВыгрузки;
...
Функция ПолучитьАдресФайлаКвитанции(
	...
	Иначе
		...
		Если ТипЗнч(СтрКвитанция) = Тип("ДвоичныеДанные") Тогда
			ИмяФайлаКвитанции = НСтр("ru = 'Квитанция';
									|en = 'Квитанция'") + ОтправкаСсылка.ИмяФайлаВыгрузки + ".bin";
			СтрКвитанция.Записать(ВременныйФайл);
			
		Иначе
			ИмяФайлаКвитанции = НСтр("ru = 'Квитанция';
									|en = 'Квитанция'") + ОтправкаСсылка.ИмяФайлаВыгрузки;
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, ИменаРеквизитов, УникальныйИдентификатор)
	...
	РазрешенныеРеквизитыДанных = СтрРазделить("ФайлВырузки,ЭлектроннаяПодпись,ФайлЗаявленияНаОтзыв,"
		+ "ЭлектроннаяПодписьЗаявленияНаОтзыв,Образ,КвитанцияДоставкиВСФР", ",");
	...
	Для каждого КлючИЗначение Из Результат Цикл
		Если ТипЗнч(КлючИЗначение.Значение) = Тип("ХранилищеЗначения")
			...
			Результат[КлючИЗначение.Ключ] = ?(ДанныеЗначения = Неопределено, Неопределено,
				ПоместитьВоВременноеХранилище(ДанныеЗначения, УникальныйИдентификатор));
		КонецЕсли;
	КонецЦикла;
