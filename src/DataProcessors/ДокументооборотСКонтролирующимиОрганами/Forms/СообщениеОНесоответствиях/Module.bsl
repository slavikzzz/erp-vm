...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка выгрузки сообщения о несоответствиях во временный файл:
                  |%1';
                  |en = 'Ошибка выгрузки сообщения о несоответствиях во временный файл:
	...
	Если НЕ ЗначениеЗаполнено(УзелСвНесоответ) Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Некорректная структура XML файла: не обнаружен узел ""СвНесоответ"".';
				|en = 'Некорректная структура XML файла: не обнаружен узел ""СвНесоответ"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
