...
Процедура ПолучатьУведомленияПриИзменении(Элемент)
	...
	Если ПолучатьСМСУведомления И ЗначениеЗаполнено(ТелефонМобильныйДляПаролей) Тогда
...
Процедура Подключаемый_ОбработчикОбратногоОтсчета()
	...
	Таймер = Таймер - 1;
	Если Таймер >= 0 Тогда
		НадписьОбратногоОтсчета = СтрШаблон(НСтр("ru = 'Запросить код повторно можно будет через %1 сек.';
												|en = 'Запросить код повторно можно будет через %1 сек.'"), Таймер);
		ПодключитьОбработчикОжидания("Подключаемый_ОбработчикОбратногоОтсчета", 1, Истина);		
	Иначе
...
Процедура УправлениеФормой(Форма)
	...
	Если Элементы.ПояснениеПриСмене.Видимость И ЗначениеЗаполнено(Форма.ТелефонМобильныйДляПаролей) Тогда
		...
		Представление = НСтр("ru = 'Изменения необходимо будет подтвердить, введя код отправленный на %1.';
							|en = 'Изменения необходимо будет подтвердить, введя код отправленный на %1.'");
...
Процедура ИзменитьОформлениеПодсказкиОбУведомлениях(Форма)
	...
	Элементы = Форма.Элементы;
	Элементы.ПолучатьСМСУведомления.Заголовок = НСтр("ru = 'Получать SMS-уведомления';
													|en = 'Получать SMS-уведомления'");
	
	Если Форма.ЭтоПереходВОблако Тогда
		
		Элементы.ПолучатьСМСУведомления.РасширеннаяПодсказка.Заголовок = НСтр("ru = 'SMS-уведомления о статусе отправки отчетов и входящих сообщениях';
																				|en = 'SMS-уведомления о статусе отправки отчетов и входящих сообщениях'");
		Элементы.ПолучатьСМСУведомления.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
			...
			Если ЗначениеЗаполнено(ТелефонМобильныйБезРазделителей) И Не ТелефоныРавны Тогда
				ТекстПодсказки = СтрШаблон(
					НСтр("ru = 'Вы указали отдельный номер для SMS-уведомлений - <a href = ""#ВводТелефонаДляОповещений"">%1</a>.';
						|en = 'Вы указали отдельный номер для SMS-уведомлений - <a href = ""#ВводТелефонаДляОповещений"">%1</a>.'"), 
					Форма.ТелефонМобильный);
				Элементы.ПолучатьСМСУведомления.Заголовок = НСтр("ru = 'Получать SMS-уведомления
                                                                  |на ';
                                                                  |en = 'Получать SMS-уведомления
                                                                  |на '") + Форма.ТелефонМобильный;
			Иначе
				ТекстПодсказки = НСтр("ru = 'Номера телефонов для SMS-уведомлений и для паролей совпадают.
		                               ...
		                               |en = 'Номера телефонов для SMS-уведомлений и для паролей совпадают.
		                               |Если вы хотите получать SMS-уведомления на другой номер, 
		                               |укажите его, перейдя по <a href = ""#ВводТелефонаДляОповещений"">ссылке</a>.'");
			КонецЕсли;
			Элементы.ПолучатьСМСУведомления.РасширеннаяПодсказка.Заголовок = СтроковыеФункцииКлиентСервер.ФорматированнаяСтрока(ТекстПодсказки);
			Элементы.ПолучатьСМСУведомления.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
		Иначе
			Элементы.ПолучатьСМСУведомления.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
		КонецЕсли;
...
Процедура СохранитьСведения(Результат, ВходящийКонтекст) Экспорт
	...
	Если НЕ ПроверкаТелефонДляПаролей.ЗначениеВведено И ЭтоПереходВОблако Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Укажите номер мобильного телефона';
				|en = 'Укажите номер мобильного телефона'"),, "ТелефонМобильныйДляПаролей");
		МастерДалее = Ложь;
	ИначеЕсли ПроверкаТелефонДляПаролей.ИсходноеЗначение <> ТелефонМобильныйДляПаролей И НЕ ЭтоПереходВОблако ИЛИ ЭтоПереходВОблако Тогда 
		...
		Если Не ПроверкаТелефонДляПаролей.ПодтверждениеВыполнено Тогда
			Если ПроверкаТелефонДляПаролей.ВыполняетсяПроверка Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					НСтр("ru = 'Введите код из SMS';
						|en = 'Введите код из SMS'"),, "КодПодтверждения");
				МастерДалее = Ложь;
			Иначе
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					НСтр("ru = 'Выполните проверку номера телефона';
						|en = 'Выполните проверку номера телефона'"),, "ТелефонМобильныйДляПаролей");
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если НЕ ПроверкаЭлектроннаяПочтаДляПаролей.ЗначениеВведено И ЭтоПереходВОблако Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Укажите адрес электронной почты';
				|en = 'Укажите адрес электронной почты'"),, "ЭлектроннаяПочтаДляПаролей");
		МастерДалее = Ложь;
	...
	ИначеЕсли ПроверкаЭлектроннаяПочтаДляПаролей.ЗначениеВведено 
		...
		Если Не ПроверкаЭлектроннаяПочтаДляПаролей.ПодтверждениеВыполнено Тогда
			Если ПроверкаЭлектроннаяПочтаДляПаролей.ВыполняетсяПроверка Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					НСтр("ru = 'Введите код из письма';
						|en = 'Введите код из письма'"),, "КодПодтверждения");
				МастерДалее = Ложь;
			Иначе
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					НСтр("ru = 'Выполните проверку адреса электронной почты';
						|en = 'Выполните проверку адреса электронной почты'"),, "ЭлектроннаяПочтаДляПаролей");
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
