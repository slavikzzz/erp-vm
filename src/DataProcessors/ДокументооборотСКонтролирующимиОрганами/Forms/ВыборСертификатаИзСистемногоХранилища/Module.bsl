...
Процедура ПересчитатьТаблицуСертификатов()
	...
	Для Каждого Серт Из ПолнаяТаблицаСертификатов Цикл
		...
		ИННы = ДокументооборотСКОКлиентСервер.ИННыИзСертификата(Серт.ВладелецСтруктура);
		
		НовСтр.ИННВладельца = ИННы.ФЛ;
		НовСтр.ИНН = ИННы.ЮЛ;
		...
		Если НЕ ЗначениеЗаполнено(НовСтр.ИНН) Тогда
			
			Представление = ОбработкаЗаявленийАбонентаКлиентСервер.ПредставлениеОрганизацииСертификата(НовСтр);
			НовСтр.Организация = Представление;
				
		КонецЕсли;
		
	КонецЦикла;
	Сертификаты.Сортировать("Организация, ИНН, ИмяВладельца, ДействителенС, ДействителенПо");
