...
Процедура ЗаполнитьСводку(РезультатыДиагностики, ВсегоОшибок, ВсегоПредупреждений)
	...
	ИначеЕсли НЕ РезультатыДиагностики.Отчет.Проверки.Свойство("ПроверкаКриптоопераций") Тогда
		Если КриптографияЭДКОКлиентСервер.ЭтоПодписьСервиса(РезультатыДиагностики.Параметры) Тогда
			...
			ВсегоПредупреждений = ВсегоПредупреждений + 1;
			Сообщение = НайтиОписаниеПоКоду("130_5", МакетПодсказки, Ссылка1,);
	...
	Если Нерасшифрованные <> Неопределено И Нерасшифрованные.Количество() > 0 Тогда
		...
		Если Безнадежных > 0 Тогда 			
			...
			Сообщение = НайтиОписаниеПоКоду("130_1", МакетПодсказки,Ссылка1,);
			ДобавитьНадпись(Сообщение, СсылкаГруппы,,,Ссылка1,,, Истина);
	...
	Если Результат.Свойство("КонтекстЭДО") Тогда 
		...
		Если ДополнительныеПараметры.УчетнаяЗаписьДокументооборота <> Неопределено Тогда 
			...
			КонтекстЭДОКлиент.АвтоматическаяНастройкаУчетнойЗаписи(
					...
					ВыполнятьРасшифровкуЗашифрованнойЧастиФайлаАвтонастроки);
		КонецЕсли;
	КонецЕсли;
		...
		Если ДополнительныеПараметры.ИспользоватьИнтерактивноеУведомление Тогда 
			Если РезультатНастройки = Истина Тогда
				Сообщение = НСтр("ru = 'Автоматическая настройка успешно завершена.';
								|en = 'Автоматическая настройка успешно завершена.'");
			...
			ИначеЕсли РезультатНастройки = Ложь Тогда
				Сообщение = НСтр("ru = 'Текущие настройки учетной записи актуальны!';
								|en = 'Текущие настройки учетной записи актуальны!'");
				ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ПоместитьИнформациюОбУчетнойЗаписиВОтчет()
	...
	РезультатыДиагностики.Отчет.ДополнительнаяИнформация.Вставить(
		"ПодключенныеНаправления", 
		РезультатыДиагностики.Параметры.ДополнительныеРеквизитыУчетнойЗаписиПолучатели);
...
Функция ПолучитьИнформациюФайлаАвтонастройки()
	...
	ДопРеквизиты = РезультатыДиагностики.Параметры.ДополнительныеРеквизитыУчетнойЗаписи;
	
	Если ДопРеквизиты <> Неопределено Тогда 
		СвойстваКриптопровайдера = КриптографияЭДКОКлиентСервер.СвойстваКриптопровайдераПоУмолчанию(ДопРеквизиты.ТипКриптопровайдера);
		...
		ДанныеАвтонастройки.ВладелецЭП.Вставить("Фамилия", ДопРеквизиты.ВладелецЭЦПФамилия);
		ДанныеАвтонастройки.ВладелецЭП.Вставить("Имя", ДопРеквизиты.ВладелецЭЦПИмя);
		ДанныеАвтонастройки.ВладелецЭП.Вставить("Отчество", ДопРеквизиты.ВладелецЭЦПОтчество);
		ДанныеАвтонастройки.ВладелецЭП.Вставить("СНИЛС", ДопРеквизиты.ВладелецЭЦПСНИЛС);
		ДанныеАвтонастройки.ВладелецЭП.Вставить("Должность", ДопРеквизиты.ВладелецЭЦПДолжность);
		...
		ДанныеАвтонастройки.ВладелецЭП.Вставить("КриптопровайдерENUM", ДопРеквизиты.ТипКриптопровайдера);
		
		ДанныеАвтонастройки.СведенияРегистрации.Вставить("КраткоеНаименование", ДопРеквизиты.КраткоеНаименование);
		ДанныеАвтонастройки.СведенияРегистрации.Вставить("ТелефонОсновной", ДопРеквизиты.ТелефонОсновной);
		ДанныеАвтонастройки.СведенияРегистрации.Вставить("ЭлектроннаяПочта", ДопРеквизиты.ЭлектроннаяПочта);
		ДанныеАвтонастройки.СведенияРегистрации.Вставить("ИНН", СокрП(ДопРеквизиты.ИНН));
		ДанныеАвтонастройки.СведенияРегистрации.Вставить("КПП", СокрП(ДопРеквизиты.КПП));
		ДанныеАвтонастройки.СведенияРегистрации.Вставить("ОГРН", ДопРеквизиты.ОГРН);
		ДанныеАвтонастройки.СведенияРегистрации.Вставить("РегНомерПФР", ДопРеквизиты.РегНомерПФР);
	Иначе
...
Процедура НачатьПроверку(Команда, АвтонастройкаВыполнена = Ложь) //0
	...
	Если Организация.Пустая() Тогда 
		Сообщение = НСтр("ru = 'Необходимо сначала выбрать организацию';
						|en = 'Необходимо сначала выбрать организацию'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение);
		Возврат;
	КонецЕсли;
	...
	Сертификат = МультирежимВызовСервера.СертификатПользователя(ОсновнаяУчетнаяЗапись.Ссылка);
	Если НЕ Сертификат.Найден Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(Сертификат.ТекстОшибки);
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьИнформациюОНерасшифрованных(Куда, Организация, УчетнаяЗаписьОбмена, УИД)
	...
	Запрос = Новый Запрос(
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ТранспортноеСообщение КАК ДокТранспортноеСообщение
		...
		|		ПО ЦиклыОбмена.Ссылка = ДокТранспортноеСообщение.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВладельцыОбъектов1СОтчетности КАК ВладельцыОбъектов1СОтчетности
		|		ПО (ЦиклыОбмена.Ссылка = ВладельцыОбъектов1СОтчетности.Объект)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВладельцыОбъектов1СОтчетности КАК ВладельцыОбъектов1СОтчетностиТС
		|		ПО (ТранспортныеКонтейнеры.ТранспортноеСообщение = ВладельцыОбъектов1СОтчетностиТС.Объект
		|			И ВладельцыОбъектов1СОтчетности.Владелец ЕСТЬ NULL)
		|ГДЕ
		|	(ДокТранспортноеСообщение.Отправитель = &ОтборОрганизация
		|		ИЛИ ДокТранспортноеСообщение.Получатель = &ОтборОрганизация)
		|	И ДокТранспортноеСообщение.УчетнаяЗапись = &УчетнаяЗапись
		|	И НЕ ДокТранспортноеСообщение.ПометкаУдаления
		|	И НЕ ДокТранспортноеСообщение.ЦиклОбмена.ПометкаУдаления
		|	И СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение ЕСТЬ NULL
		|	И (ВладельцыОбъектов1СОтчетности.Владелец = &ТекущийПользователь
		|		ИЛИ ВладельцыОбъектов1СОтчетности.Владелец ЕСТЬ NULL)
		|	И (ВладельцыОбъектов1СОтчетностиТС.Владелец = &ТекущийПользователь
		|		ИЛИ ВладельцыОбъектов1СОтчетностиТС.Владелец ЕСТЬ NULL)");
...
Функция ПолучитьФИОДоверенности(Доверенность, Субъект)
	...
	Если ДокументооборотСКОКлиентСервер.ПодсистемаЦПРРМЧДСуществует() И ТипЗнч(Доверенность.Ссылка) = 
		Тип("СправочникСсылка." + ИмяТипаСправочникаМашиночитаемыеДоверенности) Тогда
		Возврат Строка(Доверенность.Ссылка);
	КонецЕсли;
...
Функция ПолучитьСписокПроблемСДоверенностями(ОрганизацияСсылка, ВладелецЭП, СведенияОРегистрации, СертификатВыданНаПредставителя)
	...
	ВладелецЭП_ФЛ	= Мультирежим.ФизЛицоПоФИОиСНИЛС(
		ВладелецЭП.Фамилия, 
		ВладелецЭП.Имя, 
		ВладелецЭП.Отчество, 
		ВладелецЭП.СНИЛС);
	...
	ВсеРегистрации = ДокументооборотСКОВызовСервера.РегистрацииВНалоговомОргане(ОрганизацияСсылка);
	...
	Для Каждого Регистрация из ВсеРегистрации Цикл
		...
		РегистрацияСсылка = Регистрация.РегистрацияСсылка;
		
		Результат = Направления.НайтиСтроки(Новый Структура("КодПолучателя, КПП", РегистрацияСсылка.Код, РегистрацияСсылка.КПП));
		...
		Запись = Новый Структура("Направление, КодПолучателя, КПП, Регистрация, Доверенность, Проблемы", "ИФНС", 
						РегистрацияСсылка.Код, РегистрацияСсылка.КПП, РегистрацияСсылка, Неопределено, Новый Массив);
		...
		Иначе
			...
			Иначе
				...
				Если НЕ ЗначениеЗаполнено(ДоверенностьПредставитель_ФИО) Тогда
					ДоверенностьПредставитель_ФИО = НСтр("ru = '<не заполнено>';
														|en = '<не заполнено>'");
				КонецЕсли;
			...
			ИмяТипаСправочникаМашиночитаемыеДоверенности = "МашиночитаемыеДоверенности";
			Если ДокументооборотСКОКлиентСервер.ПодсистемаЦПРРМЧДСуществует() И
				ТипЗнч(Доверенность) = Тип("СправочникСсылка." + ИмяТипаСправочникаМашиночитаемыеДоверенности) Тогда
...
Процедура ДобавитьЗаписьИзменения(Вид, Знач Было, Знач Стало, Куда)	
	...
	Строчка.СтароеЗначение = СокрЛП_Врег(Было);
	Строчка.НовоеЗначение = СокрЛП_Врег(Стало);
...
Функция ПолучитьСписокИзменившихсяРеквизитовПодключенияК1СОтчетности(ОрганизацияСсылка, УчетнаяЗаписьОбменаСсылка)
	...
	КодРегионаФСРАР  = КонтекстЭДОСервер.КодРегионаФСРАР(ДанныеЗаполнения.АдресЮридическийЗначение);
	
	КодПФР = КодПФР(ДанныеОрганизации);
	
	Спецоператор = УчетнаяЗаписьОбменаСсылка.СпецоператорСвязи;
	
	ДопРеквизиты = ПолучитьДополнительныеРеквизитыУчетнойЗаписи(УчетнаяЗаписьОбменаСсылка);
	
	Если ДопРеквизиты <> Неопределено Тогда
		КраткоеНаименованиеИсходное 		= ДопРеквизиты.КраткоеНаименование;
		ИННИсходный 						= ДопРеквизиты.ИНН;
		КППИсходный 						= ДопРеквизиты.КПП;
		ОГРНИсходный 						= ДопРеквизиты.ОГРН;
		РегНомерПФРИсходный 				= ДопРеквизиты.РегНомерПФР;
		ЭлектроннаяПочтаИсходная 			= ДопРеквизиты.ЭлектроннаяПочта;
		ВладелецЭЦПФамилияИсходный 			= ДопРеквизиты.ВладелецЭЦПФамилия;
		ВладелецЭЦПИмяИсходный 				= ДопРеквизиты.ВладелецЭЦПИмя;
		ВладелецЭЦПОтчествоИсходный 		= ДопРеквизиты.ВладелецЭЦПОтчество;
		ВладелецЭЦППодразделениеИсходное 	= ДопРеквизиты.ВладелецЭЦППодразделение;
		ВладелецЭЦПСНИЛСИсходный 			= ДопРеквизиты.ВладелецЭЦПСНИЛС;
		ВладелецЭЦПДолжностьИсходная 		= ДопРеквизиты.ВладелецЭЦПДолжность;
		ТелефонОсновнойИсходный 			= ДопРеквизиты.ТелефонОсновной;
		КодРегионаФСРАРИсходный 			= ДопРеквизиты.КодРегионаФСРАР;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	
	Направления = ПолучитьДополнительныеРеквизитыУчетнойЗаписиПолучатели(УчетнаяЗаписьОбменаСсылка);
	...
	КраткоеНаименованиеИзменилось = 
		ЭтоЮрЛицо
		И СокрЛП_Врег(КраткоеНаименование) <> СокрЛП_Врег(КраткоеНаименованиеИсходное);
	
	Сертификат = Мультирежим.СертификатПользователя(УчетнаяЗаписьОбменаСсылка, "ЭлектроннаяПодписьВМоделиСервиса");
	
	ЭлектроннаяПочтаИзменилась = 
		НЕ Сертификат.ЭлектроннаяПодписьВМоделиСервиса // в этом случае берется не из регистра, а из сервиса
		И СокрЛП_Врег(ЭлектроннаяПочта) <> СокрЛП_Врег(ЭлектроннаяПочтаИсходная) 
		И ЗначениеЗаполнено(ЭлектроннаяПочта);
		
	ИсходныйВладелец = ОбработкаЗаявленийАбонента.ИсходныйВладелецСертификатаПоФИОиСНИЛС(
		Организация, 
	...
	ВладелецЭЦПИзменился = 
		((СокрЛП_Врег(ВладелецЭЦПФамилия) <> СокрЛП_Врег(ВладелецЭЦПФамилияИсходный) 
		ИЛИ СокрЛП_Врег(ВладелецЭЦПИмя) <> СокрЛП_Врег(ВладелецЭЦПИмяИсходный) 
		ИЛИ СокрЛП_Врег(ВладелецЭЦПОтчество) <> СокрЛП_Врег(ВладелецЭЦПОтчествоИсходный)))
		И ВладелецЭЦПИсходныйНайден;// если в базе не найден владелец - значит говорим, что он не изменился
							
	// КПП
	КППИзменился = 
		СокрЛП_Врег(КПП) <> СокрЛП_Врег(КППИсходный) 
		И ЗначениеЗаполнено(КПП);
		
	// ОГРН ***
	ОГРНИзменился = 
		СокрЛП_Врег(ОГРН) <> СокрЛП_Врег(ОГРНИсходный) 
		И ЗначениеЗаполнено(СтрокаБезДефисов(ОГРН));
		
	// Регистрационный номер в ПФР
	РегНомерПФРИзменился = 
		СокрЛП_Врег(РегНомерПФР) <> СокрЛП_Врег(РегНомерПФРИсходный) 
		И (СдаватьВПФРИсходный ИЛИ СдаватьВФССИсходный) 
		И ЗначениеЗаполнено(СтрокаБезДефисов(РегНомерПФР));
		
	КодРегионаФСРАРИзменился = 
		СокрЛП_Врег(КодРегионаФСРАРИсходный) <> СокрЛП_Врег(КодРегионаФСРАР) 
		И СдаватьВФСРАРИсходный 
		И ЗначениеЗаполнено(КодРегионаФСРАР) 
		И ЗначениеЗаполнено(КодРегионаФСРАРИсходный);
				
	ВладелецЭЦППодразделениеИзменилось = 
		СокрЛП_Врег(ВладелецЭЦППодразделение) <> СокрЛП_Врег(ВладелецЭЦППодразделениеИсходное) 
		И ВладелецЭЦПИсходныйНайден
		И ЗначениеЗаполнено(СокрЛП_Врег(ВладелецЭЦППодразделение));
		
	ВладелецЭЦПДолжностьИзменилась = 
		СокрЛП_Врег(ВладелецЭЦПДолжность) <> СокрЛП_Врег(ВладелецЭЦПДолжностьИсходная) 
		И ВладелецЭЦПИсходныйНайден
		И ЗначениеЗаполнено(СокрЛП_Врег(ВладелецЭЦПДолжность));
			
	ВладелецЭЦПСНИЛСИзменился = 
		СокрЛП_Врег(ВладелецЭЦПСНИЛС) <> СокрЛП_Врег(ВладелецЭЦПСНИЛСИсходный)
		...
		ИЛИ ВладелецЭЦПТипИсходный = Перечисления.ТипыВладельцевЭЦП.ГлавныйБухгалтер); // это бухгалтер или директор
		
	КодПФРИзменился = 
		СокрЛП_Врег(КодПФР) <> СокрЛП_Врег(КодПФРИсходный) 
		И (СдаватьВПФРИсходный ИЛИ СдаватьВФССИсходный) 
		И ЗначениеЗаполнено(СтрокаБезДефисов(КодПФР));
		
	ИННИзменился = СокрЛП_Врег(ИНН) <> СокрЛП_Врег(ИННИсходный);
		
	Перечисление = Перечисления.ПараметрыПодключенияК1СОтчетности;
		
	Если КППИзменился Тогда 
		ДобавитьЗаписьИзменения(Перечисление.КПП, КППИсходный, КПП, Изменения);
	КонецЕсли;
	
	Если ОГРНИзменился Тогда 
		ДобавитьЗаписьИзменения(Перечисление.ОГРН, ОГРНИсходный, ОГРН, Изменения);
	КонецЕсли;
	
	Если РегНомерПФРИзменился Тогда 
		ДобавитьЗаписьИзменения(Перечисление.РегНомерПФР, РегНомерПФРИсходный, РегНомерПФР, Изменения);
	КонецЕсли;
	
	// Код Региона ***
	Если КодРегионаФСРАРИзменился Тогда 
		ДобавитьЗаписьИзменения(Перечисление.КодРегионаФСРАР, КодРегионаФСРАРИсходный, КодРегионаФСРАР, Изменения);
	КонецЕсли;
	
	// Подразделение
	Если ВладелецЭЦППодразделениеИзменилось Тогда 
		ДобавитьЗаписьИзменения(
			Перечисление.ВладелецЭЦППодразделение, 
	...
	Если ВладелецЭЦПДолжностьИзменилась Тогда 
		ДобавитьЗаписьИзменения(
			Перечисление.ВладелецЭЦПДолжность, 
	...
	Если ВладелецЭЦПСНИЛСИзменился Тогда 
		ДобавитьЗаписьИзменения(Перечисление.ВладелецЭЦПСНИЛС, ВладелецЭЦПСНИЛСИсходный, ВладелецЭЦПСНИЛС, Изменения);
	КонецЕсли;
	
	// КодПФР
	Если КодПФРИзменился Тогда 
		ДобавитьЗаписьИзменения(Перечисление.КодПФР, КодПФРИсходный, КодПФР, Изменения);
	КонецЕсли;
	
	// ИНН ***
	Если ИННИзменился Тогда 
		ДобавитьЗаписьИзменения(Перечисление.ИНН, ИННИсходный, ИНН, Изменения);
	КонецЕсли;
	
	// Краткое наименование ***
	Если КраткоеНаименованиеИзменилось Тогда 
		ДобавитьЗаписьИзменения(Перечисление.КраткоеНаименование, КраткоеНаименованиеИсходное, КраткоеНаименование, Изменения);
	КонецЕсли;
	
	//Электронная почта ***
	Если ЭлектроннаяПочтаИзменилась Тогда 
		ДобавитьЗаписьИзменения(Перечисление.ЭлектроннаяПочта, ЭлектроннаяПочтаИсходная, ЭлектроннаяПочта, Изменения);
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьИнформациюЛичногоСертификата(Знач ОтпечатокСертификата)
	
	ПараметрыОповещения = Новый Структура("ОтпечатокСертификата", ОтпечатокСертификата);	
...
Функция НайтиСертификатИЭкспортироватьЕго(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		КонецЕсли;
	Иначе
		ДобавитьОшибкуСертификатНеНайден(ВходящийКонтекст);
	КонецЕсли;
...
Функция ВыполнитьПроверкуУчетнойЗаписи() //1
	...
	Сертификат = МультирежимВызовСервера.СертификатПользователя(РезультатыДиагностики.Параметры.УчетнаяЗаписьОбмена.Ссылка);
	Если Сертификат.Найден Тогда
		ЗаполнитьИнформациюЛичногоСертификата(Сертификат.Отпечаток);
	Иначе
		Контекст = Новый Структура("ОтпечатокСертификата", Сертификат.Отпечаток);
		ДобавитьОшибкуСертификатНеНайден(Контекст);
	КонецЕсли;
...
&НаКлиенте
Функция ДобавитьОшибкуСертификатНеНайден(ВходящийКонтекст) Экспорт
...
Функция ВыполнитьПроверкуПодключений() //2
	...
	Для Каждого Направление Из РезультатыДиагностики.Автонастройка.Получатели Цикл
		...
		Если Направление.Значение = "ФСРАР" Тогда 
			...
			ДанныеХоста = Новый Структура("Проверен, Адрес, Порт, Псевдоним", Ложь, Адрес, Порт, "Росалкогольтабакконтроль (производство)");
			...
			ДанныеХоста = Новый Структура("Проверен, Адрес, Порт, Псевдоним", Ложь, Адрес, Порт, "Росалкогольтабакконтроль (розница)");
			РезультатыДиагностики.Отчет.Проверки.ПроверяемыеХосты.Добавить(ДанныеХоста);
...
Процедура ПолучитьСписокОшибокПослеРасшифровки(СписокОшибок, ВходящийКонтекст) Экспорт
	...
	Если ВходящийКонтекст.ИнтерактивныеУведомления Тогда 
		Сообщение = НСтр("ru = 'Тестовая операция расшифровки завершилась неудачей';
						|en = 'Тестовая операция расшифровки завершилась неудачей'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение);		
	...
	Если Результат.Выполнено Тогда
		...
		Если ВходящийКонтекст.ИнтерактивныеУведомления Тогда 
			Сообщение = НСтр("ru = 'Тестовые криптооперации шифрования и расшифровки завершились успешно';
							|en = 'Тестовые криптооперации шифрования и расшифровки завершились успешно'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение);
...
&НаКлиенте
Процедура ВыполнитьПроверкуКриптоопераций(ИнтерактивныеУведомления = Ложь, ВходящийКонтекст = Неопределено) //5
	...
	Если НЕ Сертификат.Найден Тогда
		...
		Результат.Вставить("ОписаниеОшибки", Сертификат.ТекстОшибки);
		Результат.Вставить("ИнтерактивныйКонтекст", ИнтерактивныеУведомления);
		
		НайтиСертификатИПерейтиКШифрованию(Результат, Результат);
		Возврат;
	...
	МестоХраненияКлюча = КриптографияЭДКОКлиентСервер.КонтекстМоделиХраненияКлюча(РезультатыДиагностики.Параметры);
	НоваяСтрока = Новый Структура("Отпечаток", Отпечаток);
	КриптографияЭДКОКлиентСервер.ЗаполнитьМестоХраненияКлюча(МестоХраненияКлюча, НоваяСтрока);
	...
	ПараметрыОповещения = Новый Структура(
		"СертификатыХранятсяУдаленно, ОтпечатокСертификата, ИнтерактивныеУведомления, ИнтерактивныйКонтекст", 
		Ложь, Отпечаток, ИнтерактивныеУведомления, ВходящийКонтекст);	
	...
	КриптографияЭДКОКлиент.НайтиСертификаты(
		...
		СписокСертификатовДляПоиска, 
		Истина, 
		Ложь, 
		РезультатыПодключенияКК.Криптокомпонент);
...
Функция ОтправитьОтчетНаСервере(ОтчетДиагностики, ОписаниеОшибки = "", Знач ТолькоСформировать = Ложь)
	...
	Перем ЗащищенноеСоединение, Прокси;
	...
	РеквизитыПодключения = ЭлектронныйДокументооборотСКонтролирующимиОрганамиВнутренний.РеквизитыПодключенияСервисаОтчетов();
	...
	Соединение = Новый HTTPСоединение(ПолучательОтчета, 443, РеквизитыПодключения.Реквизит, РеквизитыПодключения.Значение, Прокси, Таймаут, ЗащищенноеСоединение);
...
Процедура ДинамическийЭлементОбработкаНавигационнойСсылки(Элемент, Ссылка, СтандартнаяОбработка)
	...
	Иначе
		Сообщение = НСтр("ru = 'Не удалось найти обработчик данного элемента';
						|en = 'Не удалось найти обработчик данного элемента'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение);
	КонецЕсли;
...
&НаКлиенте
Процедура ОткрытьНастройкуСистемы(Результат, ВхКонтекст) Экспорт
...
Процедура ДекорацияТекстОбщийНажатие(Ссылка, ВхПараметры) Экспорт 
	...
	Иначе
		Сообщение = НСтр("ru = 'Не удалось найти подробное описание данного элемента';
						|en = 'Не удалось найти подробное описание данного элемента'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение);
	КонецЕсли;
...
Функция ПолучитьДоступныеОрганизации()
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	И Организации.УчетнаяЗаписьОбмена.СпецоператорСвязи В (ЗНАЧЕНИЕ(Перечисление.СпецоператорыСвязи.КалугаАстрал), ЗНАЧЕНИЕ(Перечисление.СпецоператорыСвязи.Форус))
	               |	И Организации.УчетнаяЗаписьОбмена В(&УчетнаяЗаписьОбмена)";
	
	Запрос.УстановитьПараметр("УчетнаяЗаписьОбмена", ДокументооборотСКО.ТекущиеУчетныеЗаписиНалогоплательщика());
...
Функция ПолучитьДополнительныеРеквизитыУчетнойЗаписи(УчетнаяЗаписьОбмена)
	...
	КонтекстЭДОСервер = ДокументооборотСКО.ПолучитьОбработкуЭДО();
	ДопРеквизиты = КонтекстЭДОСервер.ДополнительныеРеквизитыУчетнойЗаписи(УчетнаяЗаписьОбмена); 
	...
	Колонки = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КолонкиСтрокой, ",", Ложь, Истина);
	
	Результат = Новый Структура();
	Для Каждого Колонка из Колонки цикл
		Значение = ДопРеквизиты[Колонка];
		Результат.Вставить(Колонка, Значение);
		Результат.Вставить(Колонка + "Исходный", Значение);
	КонецЦикла;
	
	Результат.Вставить("КраткоеНаименованиеИсходное", ДопРеквизиты.КраткоеНаименование);
	Результат.Вставить("ВладелецЭЦППодразделениеИсходное", ДопРеквизиты.ВладелецЭЦППодразделение);
	Результат.Вставить("ЭлектроннаяПочтаИсходная", ДопРеквизиты.ЭлектроннаяПочта);
	Результат.Вставить("ВладелецЭЦПДолжностьИсходная", ДопРеквизиты.ВладелецЭЦПДолжность);
