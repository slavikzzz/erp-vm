...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяДокументаСведенияОДоходахСотрудникаДляСоцВыплат =
		ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСерверПереопределяемый.ИмяОбъектаМетаданных(
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИмяСобытияЗаписиДокументаСведенияОДоходахСотрудникаДляСоцВыплат =
		?(НЕ ЗначениеЗаполнено(ИмяДокументаСведенияОДоходахСотрудникаДляСоцВыплат),
		"", "Запись_" + ИмяДокументаСведенияОДоходахСотрудникаДляСоцВыплат);
	...
	ИначеЕсли ИмяСобытия = "Запись_ЭлектронныеПредставленияРегламентированныхОтчетов" 
		...
		ИЛИ ИмяСобытия = ИмяСобытияЗаписиДокументаСведенияОДоходахСотрудникаДляСоцВыплат
	...
	ИначеЕсли ИмяСобытия = "Показать циклы обмена уведомления" 
		...
		Если ЗначениеЗаполнено(ИмяДокументаСведенияОДоходахСотрудникаДляСоцВыплат) Тогда
			Если Тип("ДокументСсылка." + ИмяДокументаСведенияОДоходахСотрудникаДляСоцВыплат) = ТипЗначенияПараметра Тогда
				СменитьЗакладкуУведомленийПриНеобходимости("СведенияОДоходахСотрудникаДляСоцВыплат", Ложь);
			КонецЕсли;
		КонецЕсли;
...
Процедура ВыгрузитьПакеты(ВидимыеЦиклы)
	...
	Если ВидимыеЦиклы.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбраны циклы обмена!';
										|en = 'Не выбраны циклы обмена!'"));
	Иначе
		Если ВидимыеЦиклы.Количество() = 1 Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выгрузить цикл обмена ""%1""?';
																						|en = 'Выгрузить цикл обмена ""%1""?'"), КонтекстЭДОКлиент.ПредставлениеЦиклаОбмена(ВидимыеЦиклы[0]));
		Иначе
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выгрузить выбранные %1 циклов обмена?';
																						|en = 'Выгрузить выбранные %1 циклов обмена?'"), Строка(ВидимыеЦиклы.Количество()));
		КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиФСС(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ТекстПредупреждения = ДокументооборотСКОКлиентСервер.ЗаменитьПФРиФССнаСФР(
			НСтр("ru = 'Не выбраны отправки ФСС!';
				|en = 'Не выбраны отправки ФСС!'"), Истина);
	...
	Если РежимВыгрузки = 0 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Зашифрованный пакет отсутствует.';
											|en = 'Зашифрованный пакет отсутствует.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ru = 'Выберите каталог для выгрузки отправленного пакета';
								|en = 'Выберите каталог для выгрузки отправленного пакета'");
...
&НаСервереБезКонтекста
Функция СуществуетРегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ()
	Если Метаданные.Отчеты.Найти("РегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ") <> Неопределено Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция СуществуетРегламентированныйОтчетФинОтчетностьМГК()
	Если Метаданные.Отчеты.Найти("РегламентированныйОтчетФинОтчетностьМГК") <> Неопределено Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура КомандаУведомленияСоздать(Команда)
	...
	Если ЗначениеЗаполнено(ИмяДокументаУведомлениеОКонтролируемыхСделках) Тогда
		СписокВариантовВыбора.Добавить("Документы", НСтр("ru = 'Уведомление о контролируемых сделках';
														|en = 'Уведомление о контролируемых сделках'"));
	КонецЕсли;
	...
	Если СуществуетРегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний() Тогда
		СписокВариантовВыбора.Добавить("УведомленияОбУчастииВМеждународнойГруппеКомпаний",
			НСтр("ru = 'Уведомление об участии в международной группе компаний';
				|en = 'Уведомление об участии в международной группе компаний'"));
	КонецЕсли;
	...
	Если СуществуетРегламентированныйОтчетДоходыОтДеятельностиМузеяТеатраБиблиотеки() Тогда
		СписокВариантовВыбора.Добавить("ДоходыОтДеятельностиМузеяТеатраБиблиотеки",
			НСтр("ru = 'Доходы от деятельности музея, театра, библиотеки';
				|en = 'Доходы от деятельности музея, театра, библиотеки'"));
	КонецЕсли;
	...
	Если СуществуетРегламентированныйОтчетДвижениеСредствПоСчетуВБанкеЗаПределамиРФ() Тогда
		СписокВариантовВыбора.Добавить("ДвижениеСредствПоСчетуВБанкеЗаПределамиРФ",
			НСтр("ru = 'Движение средств по счету в банке за пределами РФ';
				|en = 'Движение средств по счету в банке за пределами РФ'"));
	КонецЕсли;
	
	Если СуществуетРегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ() Тогда
		СписокВариантовВыбора.Добавить("ДвижениеСредствНерезидентаЗаПределамиРФ",
			НСтр("ru = 'Движение средств нерезидента за пределами РФ';
				|en = 'Движение средств нерезидента за пределами РФ'"));
	КонецЕсли;
	
	Если СуществуетРегламентированныйОтчетФинОтчетностьМГК() Тогда
		СписокВариантовВыбора.Добавить("ФинОтчетностьМГК",
			НСтр("ru = 'Консолидированная финансовая отчетность МГК';
				|en = 'Консолидированная финансовая отчетность МГК'"));
	КонецЕсли;
	...
	Если СуществуетРегламентированныйОтчетПрослеживаемыеТоварыОтчетОбОперациях() Тогда
		СписокВариантовВыбора.Добавить("ПрослеживаемыеТоварыОтчетОбОперациях",
			НСтр("ru = 'Отчет об операциях с товарами, подлежащими прослеживаемости';
				|en = 'Отчет об операциях с товарами, подлежащими прослеживаемости'"));
	КонецЕсли;
	...
	Если СуществуетРегламентированныйОтчетПрослеживаемыеТоварыВвоз()
		...
		СписокВариантовВыбора.Добавить("ПрослеживаемыеТоварыВвоз",
			НСтр("ru = 'Уведомление о ввозе товаров, подлежащих прослеживаемости';
				|en = 'Уведомление о ввозе товаров, подлежащих прослеживаемости'"));
	КонецЕсли;
	...
	Если СуществуетРегламентированныйОтчетПрослеживаемыеТоварыПеремещение()
		...
		СписокВариантовВыбора.Добавить("ПрослеживаемыеТоварыПеремещение",
			НСтр("ru = 'Уведомление о перемещении товаров, подлежащих прослеживаемости';
				|en = 'Уведомление о перемещении товаров, подлежащих прослеживаемости'"));
	КонецЕсли;
	...
	Если СуществуетРегламентированныйОтчетПрослеживаемыеТоварыОстатки()
		...
		СписокВариантовВыбора.Добавить("ПрослеживаемыеТоварыОстатки",
			НСтр("ru = 'Уведомление об остатках товаров, подлежащих прослеживаемости';
				|en = 'Уведомление об остатках товаров, подлежащих прослеживаемости'"));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИмяДокументаУведомлениеОПеремещенииПрослеживаемыхТоваров) Тогда
		СписокВариантовВыбора.Добавить("УведомленияОПеремещенииПрослеживаемыхТоваров",
			НСтр("ru = 'Уведомление о перемещении прослеживаемых товаров';
				|en = 'Уведомление о перемещении прослеживаемых товаров'"));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ИмяДокументаСведенияОДоходахСотрудникаДляСоцВыплат) Тогда
		СписокВариантовВыбора.Добавить("СведенияОДоходахСотрудникаДляСоцВыплат",
			НСтр("ru = 'Сведения о доходах физического лица, направляемые в целях назначения социальных выплат';
				|en = 'Сведения о доходах физического лица, направляемые в целях назначения социальных выплат'"));
	КонецЕсли;
...
Процедура КомандаУведомленияСоздатьЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено Тогда
		...
		ИначеЕсли РезультатВыбора.Значение = "ДвижениеСредствНерезидентаЗаПределамиРФ" Тогда
			СменитьЗакладкуУведомленийПриНеобходимости("ДвижениеСредствНерезидентаЗаПределамиРФ");
			
			ИсточникОтчета = "РегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ";
			
			ОткрытьФорму("Отчет." + ИсточникОтчета + ".Форма.ОсновнаяФорма", Новый Структура("Организация", Организация));
			
		ИначеЕсли РезультатВыбора.Значение = "ФинОтчетностьМГК" Тогда
			СменитьЗакладкуУведомленийПриНеобходимости("ФинОтчетностьМГК");
			
			ИсточникОтчета = "РегламентированныйОтчетФинОтчетностьМГК";
		...
		ИначеЕсли РезультатВыбора.Значение = "СведенияОДоходахСотрудникаДляСоцВыплат" Тогда
			СменитьЗакладкуУведомленийПриНеобходимости("СведенияОДоходахСотрудникаДляСоцВыплат");
			ОткрытьФорму("Документ." + ИмяДокументаСведенияОДоходахСотрудникаДляСоцВыплат + ".ФормаОбъекта",
				Новый Структура("Организация", Организация));
		...
		ИначеЕсли РезультатВыбора.Значение = "РеестрыФНС" Тогда
			...
			ТекстВопроса = НСтр("ru = 'Создание реестров НДС выполняется из формы ""1С-Отчетность"".
                                 ...
                                 |en = 'Создание реестров НДС выполняется из формы ""1С-Отчетность"".
                                 |Перейти к форме ""1С-Отчетность""?'");
		...
		ИначеЕсли РезультатВыбора.Значение = "РеестрыАкцизы" Тогда
			...
			ТекстВопроса = НСтр("ru = 'Создание реестров по акцизам выполняется из формы ""1С-Отчетность"".
                                 |Перейти к форме ""1С-Отчетность""?';
                                 |en = 'Создание реестров по акцизам выполняется из формы ""1С-Отчетность"".
                                 |Перейти к форме ""1С-Отчетность""?'");
...
Функция СодержимоеОтправкиФСРАРВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФСРАР, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ru = 'Протокол';
									|en = 'Протокол'") + " " + СсылкаНаОбъектОтправкиФСРАР["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
...
Процедура ВыгрузитьСодержимоеОтправкиФСРАР(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбраны отправки в Росалкогольтабакконтроль!';
										|en = 'Не выбраны отправки в Росалкогольтабакконтроль!'"));
		Возврат;
	КонецЕсли;
...
Функция СодержимоеОтправкиРПНВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиРПН, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ru = 'Протокол';
									|en = 'Протокол'") + " " + СсылкаНаОбъектОтправкиРПН["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиРПН(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Не выбраны отправки в Росприродназдор!';
									|en = 'Не выбраны отправки в Росприродназдор!'"));
		Возврат;
	КонецЕсли;
...
Функция СодержимоеОтправкиФТСВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФТС, ИмяФайлаДляКлиента)
	...
	Если РежимВыгрузки = 0 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ru = 'Подпись';
									|en = 'Подпись'") + СсылкаНаОбъектОтправкиФТС["ИмяФайлаВыгрузки"];
	ИначеЕсли РежимВыгрузки = 1 Тогда
		СсылкаНаСодержимое = СсылкаНаОбъектОтправкиФТС["Квитанция"];
		ИмяФайлаДляКлиента = НСтр("ru = 'Квитанция';
									|en = 'Квитанция'") + " " + СсылкаНаОбъектОтправкиФТС["ИмяФайлаВыгрузки"] + ".bin";
	ИначеЕсли РежимВыгрузки = 2 Тогда
		СсылкаНаСодержимое = СсылкаНаОбъектОтправкиФТС["Протокол"];
		ИмяФайлаДляКлиента = НСтр("ru = 'Протокол';
									|en = 'Протокол'") + " " + СсылкаНаОбъектОтправкиФТС["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
&НаКлиенте
Процедура КомандаОткрытьДвижениеСредствНерезидентаЗаПределамиРФ(Команда)
	УправлениеЗакладкойУведомлений("ДвижениеСредствНерезидентаЗаПределамиРФ");
КонецПроцедуры

&НаКлиенте
Процедура КомандаОткрытьФинОтчетностьМГК(Команда)
	УправлениеЗакладкойУведомлений("ФинОтчетностьМГК");
КонецПроцедуры
...
&НаКлиенте
Процедура КомандаОткрытьСведенияОДоходахСотрудникаДляСоцВыплат(Команда)
	УправлениеЗакладкойУведомлений("СведенияОДоходахСотрудникаДляСоцВыплат");
КонецПроцедуры
	...
	ИначеЕсли ВыбранныеОрганизации.Количество() = 1 Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Произвести обмен сообщениями по учетной записи ""%1""?';
				|en = 'Произвести обмен сообщениями по учетной записи ""%1""?'"), 
			ВыбранныеОрганизации[0]);
			
	Иначе
		ТекстВопроса = НСтр("ru = 'Произвести обмен сообщениями по выбранным учетным записям?';
							|en = 'Произвести обмен сообщениями по выбранным учетным записям?'");
	КонецЕсли;
...
Процедура УправлениеЗакладкойУведомлений(Вид = Неопределено)
	...
	Элементы.ТаблицаУведомленияКомандаОткрытьДвижениеСредствНерезидентаЗаПределамиРФ.Видимость = Истина; // движение средств нерезидента за пределами РФ
	Элементы.ТаблицаУведомленияКомандаОткрытьФинОтчетностьМГК.Видимость 					= Истина; // консолидированная финансовая отчетность МГК
	...
	Элементы.ТаблицаУведомленияКомандаОткрытьСведенияОДоходахСотрудникаДляСоцВыплат.Видимость = Истина; // сведения о доходах физического лица, направляемые в целях назначения социальных выплат
	...
	// движение средств нерезидента за пределами РФ
	Если НЕ СуществуетРегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ() Тогда
		Элементы.ТаблицаУведомленияКомандаОткрытьДвижениеСредствНерезидентаЗаПределамиРФ.Видимость = Ложь;
	КонецЕсли;
	
	// консолидированная финансовая отчетность МГК
	Если НЕ СуществуетРегламентированныйОтчетФинОтчетностьМГК() Тогда
		Элементы.ТаблицаУведомленияКомандаОткрытьФинОтчетностьМГК.Видимость = Ложь;
	КонецЕсли;
	...
	// документ Уведомление о перемещении прослеживаемых товаров
	Если НЕ ЗначениеЗаполнено(ИмяДокументаСведенияОДоходахСотрудникаДляСоцВыплат) Тогда
		Элементы.ТаблицаУведомленияКомандаОткрытьСведенияОДоходахСотрудникаДляСоцВыплат.Видимость = Ложь;
	КонецЕсли;
	...
	ИначеЕсли Вид = "РеестрыАкцизы" Тогда
		...
		// РегламентированныйОтчетРеестрАкцизы_КНД1150151, нажатость подменю
	...
	ИначеЕсли Вид = "ДвижениеСредствПоСчетуВБанкеЗаПределамиРФ" Тогда
		// документ РегламентированныйОтчет с источником РегламентированныйОтчетДвижениеСредствПоСчетуВБанкеЗаПределамиРФ,
		// нажатость подменю
		Элементы.ТаблицаУведомленияКомандаОткрытьДвижениеСредствПоСчетуВБанкеЗаПределамиРФ.Пометка = Истина;
	ИначеЕсли Вид = "ДвижениеСредствНерезидентаЗаПределамиРФ" Тогда
		// документ РегламентированныйОтчет с источником РегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ,
		// нажатость подменю
		Элементы.ТаблицаУведомленияКомандаОткрытьДвижениеСредствНерезидентаЗаПределамиРФ.Пометка = Истина;
	ИначеЕсли Вид = "ФинОтчетностьМГК" Тогда
		// документ РегламентированныйОтчет с источником РегламентированныйОтчетФинОтчетностьМГК,
		// нажатость подменю
		Элементы.ТаблицаУведомленияКомандаОткрытьФинОтчетностьМГК.Пометка = Истина;
	...
	ИначеЕсли Вид = "УведомленияОПеремещенииПрослеживаемыхТоваров" Тогда
		...
		Элементы.ТаблицаУведомленияНалоговыйОрган.Видимость = Ложь;
	ИначеЕсли Вид = "СведенияОДоходахСотрудникаДляСоцВыплат" Тогда
		// документ СведенияОДоходахСотрудникаДляСоцВыплат, нажатость подменю
		Элементы.ТаблицаУведомленияКомандаОткрытьСведенияОДоходахСотрудникаДляСоцВыплат.Пометка = Истина;
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаУведомлениеОКонтролируемыхСделках) И НЕ СуществуетРегламентированныйОтчетРозничныеЦеныНаТабак()
		...
		И НЕ ЗначениеЗаполнено(ИмяДокументаСведенияОДоходахСотрудникаДляСоцВыплат)
		...
		И НЕ СуществуетРегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ()
		И НЕ СуществуетРегламентированныйОтчетФинОтчетностьМГК()
	...
	// ДвижениеСредствНерезидентаЗаПределамиРФ, ФинОтчетностьМГК, ПрослеживаемыеТоварыОтчетОбОперациях,
	// ПрослеживаемыеТоварыВвоз, ПрослеживаемыеТоварыПеремещение и ПрослеживаемыеТоварыОстатки будет в любом случае
	...
	Если Вид = "РеестрыАкцизы" Тогда
		...
		МассивИсточниковОтчетов.Добавить("РегламентированныйОтчетРеестрАкцизы_КНД1150151");
	...
	// Если в системе существует документ СведенияОДоходахСотрудникаДляСоцВыплат
	// тогда добавляем в запрос для динамического списка по документу
	// Сведения о доходах физического лица, направляемые в целях назначения социальных выплат
	Если ЗначениеЗаполнено(ИмяДокументаСведенияОДоходахСотрудникаДляСоцВыплат)
		И Вид = "СведенияОДоходахСотрудникаДляСоцВыплат" Тогда
		
		ТекстЗапроса = ТекстЗапроса
			+ "ВЫБРАТЬ
			  |	ВЫБОР
			  |		КОГДА СведенияОДоходахСотрудникаДляСоцВыплат.ПометкаУдаления
			  |			ТОГДА ВЫБОР
			  |					КОГДА СтатусОтправки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтправки.Отправлен)
			  |						ТОГДА 10
			  |					ИНАЧЕ ВЫБОР
			  ...
			  |			КОНЕЦ
			  |	КОНЕЦ КАК СтатусКартинка,
			  |	СведенияОДоходахСотрудникаДляСоцВыплат.Дата КАК ДатаСоздания,
			  |	ВЫРАЗИТЬ(СведенияОДоходахСотрудникаДляСоцВыплат.Организация КАК Справочник.Организации) КАК Организация,
			  |	СведенияОДоходахСотрудникаДляСоцВыплат.НалоговыйПериод КАК ОтчетныйГод,
			  |	СведенияОДоходахСотрудникаДляСоцВыплат.Ссылка,
			  |	СведенияОДоходахСотрудникаДляСоцВыплат.КодИФНС КАК НалоговыйОрган,
			  |	ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.СведенияОДоходахСотрудникаДляСоцВыплат) КАК ВидУведомления
			  |ИЗ
			  |	Документ.СведенияОДоходахСотрудникаДляСоцВыплат КАК СведенияОДоходахСотрудникаДляСоцВыплат
			  |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
			  |			СтатусыОтправки.Объект КАК Объект,
			  |			СтатусыОтправки.Статус КАК Статус
			  |		ИЗ
			  |			РегистрСведений.СтатусыОтправки КАК СтатусыОтправки) КАК СтатусОтправки
			  |		ПО (СтатусОтправки.Объект = СведенияОДоходахСотрудникаДляСоцВыплат.Ссылка)
			  |ГДЕ
			  |	СведенияОДоходахСотрудникаДляСоцВыплат.Организация В(&Организация)";
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "СведенияОДоходахСотрудникаДляСоцВыплат", ИмяДокументаСведенияОДоходахСотрудникаДляСоцВыплат);
		
		Уведомления.ОсновнаяТаблица = "Документ.СведенияОДоходахСотрудникаДляСоцВыплат";
	КонецЕсли;
	...
	// Добавляем в запрос для динамического списка запрос по документу РегламентированныйОтчет
	// с источником РегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ.
	Если СуществуетРегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ()
		И Вид = "ДвижениеСредствНерезидентаЗаПределамиРФ" Тогда
	...
	// Добавляем в запрос для динамического списка запрос по документу РегламентированныйОтчет
	// с источником РегламентированныйОтчетФинОтчетностьМГК.
	Если СуществуетРегламентированныйОтчетФинОтчетностьМГК()
		И Вид = "ФинОтчетностьМГК" Тогда
	...
	Если Вид = "Файлы" Тогда
		...
		ВидыУведомлений.Добавить(Справочники.ВидыОтправляемыхДокументов.ДвижениеСредствНерезидентаЗаПределамиРФ);
		ВидыУведомлений.Добавить(Справочники.ВидыОтправляемыхДокументов.ФинОтчетностьМГК);
		...
		ВидыУведомлений.Добавить(Справочники.ВидыОтправляемыхДокументов.СведенияОДоходахСотрудникаДляСоцВыплат);
...
Функция ТекстЗапросаДляОтчетовНаЗакладкеИсходящиеУведомления(ИсточникОтчета)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	                ...
	                |			ИЛИ РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ""
	                |			ИЛИ РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетФинОтчетностьМГК""
	                ...
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетРеестрНДСДекларацииНаТоварыВвозРеэкспорт""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.РеестрНДСДекларацииНаТоварыВвозРеэкспорт)
	                ...
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетРеестрНДС_КНД1155221""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.РеестрНДС_КНД1155221)
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетРеестрНДС_КНД1155223""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.РеестрНДС_КНД1155223)
	                ...
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетРеестрАкцизы_КНД1150151""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.РеестрАкцизы_КНД1150151)
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетСтрановойОтчет""
	                ...
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.ДвижениеСредствНерезидентаЗаПределамиРФ)
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетФинОтчетностьМГК""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.ФинОтчетностьМГК)
	...
	Для ИндексИсточникаОтчета = 0 По МассивИсточниковОтчетов.Количество() - 1 Цикл
		НомерСтрокойИсточникаОтчета = ?(ИндексИсточникаОтчета > 0, Формат(ИндексИсточникаОтчета + 1, "ЧДЦ=; ЧН=; ЧГ="), "");
...
Процедура СообщитьОСменеРежимов(ПредыдущийРежим, НовыйРежим)
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Режим отображения списка был переключен с ""%1"" на ""%2""';
																		|en = 'Режим отображения списка был переключен с ""%1"" на ""%2""'"), ПредыдущийРежим, НовыйРежим);
...
Процедура СменитьЗакладкуУведомленийПриНеобходимости(ИмяСобытия, ВыводитьПредупреждение = Истина)
	...
	ИначеЕсли ИмяСобытия = "ДвижениеСредствПоСчетуВБанкеЗаПределамиРФ"
		...
		НужноВыполнитьСменуЗакладки = Истина;
	ИначеЕсли ИмяСобытия = "ДвижениеСредствНерезидентаЗаПределамиРФ"
		И НЕ Элементы.ТаблицаУведомленияКомандаОткрытьДвижениеСредствНерезидентаЗаПределамиРФ.Пометка Тогда
		НужноВыполнитьСменуЗакладки = Истина;
	ИначеЕсли ИмяСобытия = "ФинОтчетностьМГК"
		И НЕ Элементы.ТаблицаУведомленияКомандаОткрытьФинОтчетностьМГК.Пометка Тогда
		НужноВыполнитьСменуЗакладки = Истина;
	...
	ИначеЕсли ИмяСобытия = "УведомленияОПеремещенииПрослеживаемыхТоваров"
		...
		НужноВыполнитьСменуЗакладки = Истина;
	ИначеЕсли ИмяСобытия = "СведенияОДоходахСотрудникаДляСоцВыплат"
		И НЕ Элементы.ТаблицаУведомленияКомандаОткрытьСведенияОДоходахСотрудникаДляСоцВыплат.Пометка Тогда
		НужноВыполнитьСменуЗакладки = Истина;
