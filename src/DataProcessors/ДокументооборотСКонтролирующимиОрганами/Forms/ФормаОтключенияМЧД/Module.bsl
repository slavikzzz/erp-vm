...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТехническийСтатус = ДокументооборотСКОВызовСервера.СвойстваМЧДЕдиногоФормата(ДоверенностьСсылка).ТехническийСтатус;
	Если ТехническийСтатус = ДокументооборотСКОВызовСервера.ТехническиеСтатусыМЧДЕдиногоФормата("Просрочена") Тогда
...
Процедура СохранитьИзменения(Команда)
	...
	Если ОтключитьНедействительнуюМЧД(
		ДоверенностьСсылка,
		КодКонтролирующегоОргана,
		ОрганизацияСсылка,
		МассивПредставителей) Тогда
		
		Оповестить("ОтключениеМЧД",, ДоверенностьСсылка);
		Закрыть(Истина);
	КонецЕсли;
...
Функция ОтключитьНедействительнуюМЧД(
		ДоверенностьСсылка,
		КодКонтролирующегоОргана,
		ОрганизацияСсылка,
		МассивПредставителей,
	...
	Если НЕ ТолькоДобавитьВИгнорируемые Тогда
		Если ЗначениеЗаполнено(ОрганизацияСсылка) Тогда
			...
			Если ЭтоМультиРежим Тогда
				...
				Для каждого ПредставительФизическоеЛицо Из МассивПредставителей Цикл
					...
					Если ЗначениеЗаполнено(ПредставительПользователь) Тогда
						...
						Если СтрокаМЧД <> Неопределено И СтрокаМЧД.Доверенность = ДоверенностьСсылка Тогда
							ТаблицаМЧД.Удалить(СтрокаМЧД);
							ТаблицаМЧДИзменена = Истина;
						КонецЕсли;
					КонецЕсли;
			...
			Если ТаблицаМЧДИзменена Тогда
				КоличествоМЧД = ТаблицаМЧД.Количество();
				ДокументооборотСКОВызовСервера.ЗаписатьМЧДОбмена(
					ОрганизацияСсылка,
					КодКонтролирующегоОргана,
					?(ЭтоМультиРежим ИЛИ КоличествоМЧД = 0, 2, 1),
					ДоверенностьСсылка,
					ТаблицаМЧД);
			КонецЕсли;
	...
	Если ИгнорируемыеНедействительныеМЧД = Неопределено Тогда
		ИгнорируемыеНедействительныеМЧД = Новый Массив;
	КонецЕсли;
	Если ИгнорируемыеНедействительныеМЧД.Найти(ДоверенностьСсылка) = Неопределено Тогда
		ИгнорируемыеНедействительныеМЧД.Добавить(ДоверенностьСсылка);
		ХранилищеОбщихНастроек.Сохранить(
			"ДокументооборотСКонтролирующимиОрганами_ИгнорируемыеНедействительныеМЧД" + КодКонтролирующегоОргана,,
			ИгнорируемыеНедействительныеМЧД);
	КонецЕсли;
