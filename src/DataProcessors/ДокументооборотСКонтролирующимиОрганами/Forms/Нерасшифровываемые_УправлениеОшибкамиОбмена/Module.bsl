...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоМультиРежим 		= Истина; // для "Мультирежим.ПредставлениеПользователей"
	...
	Если Параметры.Свойство("Контекст", НерасшифрованныеСообщения) Тогда 
		...
		Если Параметры.Свойство("ПараметрыПоказа") Тогда
...
Процедура ОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли СтрНайти(НавигационнаяСсылкаФорматированнойСтроки, "#action") > 0 Тогда 
		...
		Если ЭлементыСтроки.Количество() > 1 Тогда 
			...
			Если КомандаСПараметром = "adjusting" Тогда 
			...
			ИначеЕсли КомандаСПараметром = "close_window" Тогда 
				ЭтотОбъект.Закрыть();			
			...
			ИначеЕсли СтрНайти(КомандаСПараметром, "user_list") > 0 Тогда
				...
				Если ТекущийПользовательВСписке <> Неопределено Тогда
					СписокПользователей.Удалить(ТекущийПользовательВСписке);
				КонецЕсли;
				КонтекстЭДО.ОткрытьФормуСпискаПользователей(, СписокПользователей, Истина);
			КонецЕсли;
		КонецЕсли;
...
Процедура СообщенияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "СообщенияПользователи" Тогда
		ТекДанные = Элементы.Сообщения.ТекущиеДанные;
		Если ТекДанные = Неопределено ИЛИ ТекДанные.СписокПользователей.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
		КонтекстЭДО.ОткрытьФормуСпискаПользователей(, ТекДанные.СписокПользователей, Истина);
...
Процедура Расшифровать(Команда, ИсхКонтекст = Неопределено) Экспорт 
	...
	КонтекстЭДО.РасшифроватьТранспортноеСообщение(ТС, ОО, , ПараметрыОперации);
...
Функция ИнформацияДляРасшифровкиСообщения(ТранспортноеСообщение, ОбщиеПараметры)
	...
	Результат.Вставить("Пользователи", 			"");
	Результат.Вставить("МассивПользователей", 	Новый Массив);
	
	Результат.МассивПользователей = Мультирежим.ВладельцыТранспортногоСообщения(ТранспортноеСообщение);
			...
			Если ФорматДокументооборота = Перечисления.ФорматыДокументооборотаСПФР.ЭДОК Тогда
				...
				Если ДеревоОписанияСведений = Неопределено Тогда
					Результат.Ошибка = НСтр("ru = 'Транспортный контейнер поврежден, некорректный формат описания';
											|en = 'Транспортный контейнер поврежден, некорректный формат описания'");
					ЧтениеZip.Закрыть();
					ЧтениеZip = Неопределено;
					Чтение.Закрыть();
					Чтение = Неопределено;
				...
				Если НЕ ЗначениеЗаполнено(УзелОСП) Тогда
					Результат.Ошибка = НСтр("ru = 'Транспортный контейнер поврежден, отсутствует узел ""ОСП""';
											|en = 'Транспортный контейнер поврежден, отсутствует узел ""ОСП""'");
					ЧтениеZip.Закрыть();
					ЧтениеZip = Неопределено;
					Чтение.Закрыть();
					Чтение = Неопределено;
...
Функция Нерасшифрованные(УчетнаяЗапись = Неопределено, ОбщиеПараметры)
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивПользователей, Сведения.МассивПользователей, Истина);
	КонецЦикла;
	СписокПользователей = Новый СписокЗначений;
...
Функция ДоступныеЛичныеСертификаты()
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |ГДЕ
	               |	(СертификатыЛичные.Ссылка.ЭтоМультиРежим
	               |				И СертификатыЛичные.Пользователь = &ТекущийПользователь
	               |			ИЛИ СертификатыЛичные.Пользователь = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка))";
...
Функция ЗаполнитьСообщенияИСертификаты(Знач ВхКонтекст = Неопределено)
	...
	ТекущийПользователь = Пользователи.ТекущийПользователь();
	МассивПользователей = СписокПользователей.ВыгрузитьЗначения();
	ФизическиеЛицаПользователей = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
		МассивПользователей, "ФизическоеЛицо", Истина);
	Для каждого ПользовательРасшифровки Из СписокПользователей Цикл
		СтрокаПользователя = Новый Структура;
		СтрокаПользователя.Вставить("Пользователь", ПользовательРасшифровки.Значение);
		СтрокаПользователя.Вставить("ФизическоеЛицо", ФизическиеЛицаПользователей[ПользовательРасшифровки.Значение]);
		ПользовательРасшифровки.Представление = Мультирежим.ПредставлениеПользователей(ЭтаФорма, СтрокаПользователя);
		Если ПользовательРасшифровки.Значение = ТекущийПользователь Тогда
			ПользовательРасшифровки.Картинка = БиблиотекаКартинок.Пользователь;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СведенияОТранспортномСообщении Из Контекст Цикл
		...
		МассивПользователей = СведенияОТранспортномСообщении.МассивПользователей;
		ПользователиСообщения = "";
		СписокПользователейСообщения = Неопределено;
		Если МассивПользователей.Количество() <> 0 Тогда
			Если МассивПользователей.Найти(ТекущийПользователь) = Неопределено Тогда
				СтрокаПользователя = Новый Структура;
				СтрокаПользователя.Вставить("Пользователь", МассивПользователей[0]);
				СтрокаПользователя.Вставить("ФизическоеЛицо", ФизическиеЛицаПользователей[МассивПользователей[0]]);
				ПользователиСообщения = Мультирежим.ПредставлениеПользователей(ЭтаФорма, СтрокаПользователя);
				
			Иначе
				ПользователиСообщения = НСтр("ru = 'Вы';
											|en = 'Вы'");
			КонецЕсли;
			
			Если МассивПользователей.Количество() > 1 Тогда
				КоличествоПользователей = МассивПользователей.Количество();
				КоличествоОстальныхПользователейСтрокой = Формат(КоличествоПользователей - 1, "ЧДЦ=; ЧН=; ЧГ=");
				ПользователиСообщения = ПользователиСообщения + " " + СтрШаблон(
					НСтр("ru = 'и еще %1';
						|en = 'и еще %1'"),
					КоличествоОстальныхПользователейСтрокой);
			КонецЕсли;
			
			СписокПользователейСообщения = Новый СписокЗначений;
			СписокПользователейСообщения.ЗагрузитьЗначения(МассивПользователей);
			Для каждого ПользовательСообщения Из СписокПользователейСообщения Цикл
				СтрокаПользователя = Новый Структура;
				СтрокаПользователя.Вставить("Пользователь", ПользовательСообщения.Значение);
				СтрокаПользователя.Вставить("ФизическоеЛицо", ФизическиеЛицаПользователей[ПользовательСообщения.Значение]);
				ПользовательСообщения.Представление = Мультирежим.ПредставлениеПользователей(ЭтаФорма, СтрокаПользователя);
				Если ПользовательСообщения.Значение = ТекущийПользователь Тогда
					ПользовательСообщения.Картинка = БиблиотекаКартинок.Пользователь;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		ЗаписьСообщение.Пользователи = ПользователиСообщения;
		ЗаписьСообщение.СписокПользователей = СписокПользователейСообщения;
...
Процедура ЗаполнитьРекомендации(ОписаниеОшибки, СводнаяИнформация)
	...
	СимволТочкиИПробел = Символ(9679) + " ";
	Если СводнаяИнформация.СертификатовНезаблокированных = 0 И СводнаяИнформация.ОшибокОбработкиКонтейнера = 0 Тогда
		...
		СтатичныйТекст = СимволТочкиИПробел + НСтр("ru = 'Ошибки обработаны и никаких действий не требуется, просто';
													|en = 'Ошибки обработаны и никаких действий не требуется, просто'") + " ";
		Рекомендации.Добавить(СтатичныйТекст);
		Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'закройте';
																|en = 'закройте'"),,,, "#action?close_window"));
		Рекомендации.Добавить(" " + НСтр("ru = 'это окно';
										|en = 'это окно'") + ".");
	...
	ИначеЕсли СводнаяИнформация.СертификатовНезаблокированных = 0 И СводнаяИнформация.ОшибокОбработкиКонтейнера > 0 Тогда
		...
		ПояснениеОшибки.Добавить(НСтр("ru = 'Транспортный контейнер был записан с ошибкой.';
										|en = 'Транспортный контейнер был записан с ошибкой.'"));
		Рекомендации.Добавить(СимволТочкиИПробел + НСтр("ru = 'При необходимости, исключите из расшифровки конкретные сообщения.';
														|en = 'При необходимости, исключите из расшифровки конкретные сообщения.'") + " ");
		Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Перейти к настройке';
																|en = 'Перейти к настройке'"),,,, "#action?configure_documents"));
	...
	ИначеЕсли СтрНайти(ОписаниеОшибки, "Ошибка расшифровки файла документа при распаковке пакета") > 0 Тогда 
		...
		ПояснениеОшибки.Добавить(НСтр("ru = 'Сообщение зашифровано сертификатом недоступного ключа.';
										|en = 'Сообщение зашифровано сертификатом недоступного ключа.'"));
		...
		Если СводнаяИнформация.СертификатовНеизвестных = 0
			...
			СтатичныйТекст = СимволТочкиИПробел + НСтр("ru = 'Попробуйте повторить обновление позже, при этом убедитесь, что пароль введен правильно.';
														|en = 'Попробуйте повторить обновление позже, при этом убедитесь, что пароль введен правильно.'");
			Рекомендации.Добавить(СтатичныйТекст);
		...
		Иначе
			...
			Если ПараметрыПоказа <> Неопределено И ПараметрыПоказа.ПоказДляДругихПользователей Тогда
				Рекомендации.Добавить(СимволТочкиИПробел + НСтр("ru = 'Попробуйте расшифровать сообщения с компьютеров';
																|en = 'Попробуйте расшифровать сообщения с компьютеров'") + " ");
				Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'других пользователей';
																		|en = 'других пользователей'"),,,,"#action?user_list"));
				Рекомендации.Добавить(", " + НСтр("ru = 'на которых есть доступ к закрытому ключу';
													|en = 'на которых есть доступ к закрытому ключу'") + ".");
				Рекомендации.Добавить(Символы.ПС);
				
			Иначе
				СтатичныйТекст = СимволТочкиИПробел + НСтр("ru = 'Вставьте токен (флешку) в USB-порт или удостоверьтесь, что есть доступ к закрытому ключу, и повторите обновление.';
															|en = 'Вставьте токен (флешку) в USB-порт или удостоверьтесь, что есть доступ к закрытому ключу, и повторите обновление.'");
				Рекомендации.Добавить(СтатичныйТекст);
				Рекомендации.Добавить(Символы.ПС);
				
				Рекомендации.Добавить(СимволТочкиИПробел + НСтр("ru = 'Выполните настройку рабочего места.';
																|en = 'Выполните настройку рабочего места.'") + " ");
				Рекомендации.Добавить(Новый ФорматированнаяСтрока("Перейти к настройке",,,, "#action?adjusting"));
				Рекомендации.Добавить(".");
				Рекомендации.Добавить(Символы.ПС);
				
				Если СводнаяИнформация.СертификатовНеизвестных = 0 Тогда
					СтатичныйТекст = СимволТочкиИПробел + НСтр("ru = 'При использовании 1С-Отчетности на нескольких компьютерах повторите обновление на том, где настроен';
																|en = 'При использовании 1С-Отчетности на нескольких компьютерах повторите обновление на том, где настроен'") + " ";
					Рекомендации.Добавить(СтатичныйТекст);
					Если СводнаяИнформация.СертификатовНеизвестных = 0
							И СводнаяИнформация.СертификатовКриптоПро > 0
							И СводнаяИнформация.СертификатовВипнет = 0 Тогда // однозначно КриптоПро
						Рекомендации.Добавить(НСтр("ru = 'КриптоПро';
													|en = 'КриптоПро'"));
					ИначеЕсли СводнаяИнформация.СертификатовНеизвестных = 0
							И СводнаяИнформация.СертификатовКриптоПро = 0
							И СводнаяИнформация.СертификатовВипнет > 0 Тогда // однозначно Випнет
						Рекомендации.Добавить(НСтр("ru = 'ViPNet';
													|en = 'ViPNet'"));
					КонецЕсли;
			...
			Если ПараметрыПоказа <> Неопределено И ПараметрыПоказа.ПоказДляДругихПользователей Тогда
				СтатичныйТекст = СимволТочкиИПробел + НСтр("ru = 'Перенесите закрытый ключ с компьютера другого пользователя.';
															|en = 'Перенесите закрытый ключ с компьютера другого пользователя.'") + " ";
			Иначе
				СтатичныйТекст = СимволТочкиИПробел + НСтр("ru = 'Перенесите закрытый ключ с другого компьютера.';
															|en = 'Перенесите закрытый ключ с другого компьютера.'") + " ";
			КонецЕсли;
			Рекомендации.Добавить(СтатичныйТекст);
			Если СводнаяИнформация.СертификатовНеизвестных > 0 Тогда // нераспознанный сертификат
				Рекомендации.Добавить(НСтр("ru = 'Инструкция по переносу для';
											|en = 'Инструкция по переносу для'") + " ");
				Рекомендации.Добавить(Новый ФорматированнаяСтрока("ViPNet CSP",,,, "https://its.1c.ru/bmk/elreps/key_trans_vipnet"));
				Рекомендации.Добавить(" " + НСтр("ru = 'и';
												|en = 'и'") + " ");
				Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'КриптоПро CSP';
																		|en = 'КриптоПро CSP'"),,,, "https://its.1c.ru/bmk/elreps/key_trans_cryptopro"));
			ИначеЕсли СводнаяИнформация.СертификатовНеизвестных = 0 
					И СводнаяИнформация.СертификатовКриптоПро > 0
					И СводнаяИнформация.СертификатовВипнет = 0 Тогда // однозначно КриптоПро
				Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Инструкция по переносу';
																		|en = 'Инструкция по переносу'"),,,, "https://its.1c.ru/bmk/elreps/key_trans_cryptopro"));
			ИначеЕсли СводнаяИнформация.СертификатовНеизвестных = 0 
					И СводнаяИнформация.СертификатовКриптоПро = 0
					И СводнаяИнформация.СертификатовВипнет > 0 Тогда // однозначно Випнет
				Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Инструкция по переносу';
																		|en = 'Инструкция по переносу'"),,,, "https://its.1c.ru/bmk/elreps/key_trans_vipnet"));
			КонецЕсли;
			Рекомендации.Добавить(".");
			Рекомендации.Добавить(Символы.ПС);
			
			Если ПараметрыПоказа <> Неопределено И ПараметрыПоказа.ПоказДляДругихПользователей Тогда
				СтатичныйТекст = СимволТочкиИПробел + НСтр("ru = 'Вставьте токен (флешку) в USB-порт или удостоверьтесь, что есть доступ к закрытому ключу, и повторите расшифровку. Ее вызов доступен для организации по гиперссылке ""Настройки обмена с контролирующими органами"" в разделе ""Настройки"" формы ""1С-Отчетность"".';
															|en = 'Вставьте токен (флешку) в USB-порт или удостоверьтесь, что есть доступ к закрытому ключу, и повторите расшифровку. Ее вызов доступен для организации по гиперссылке ""Настройки обмена с контролирующими органами"" в разделе ""Настройки"" формы ""1С-Отчетность"".'");
				Рекомендации.Добавить(СтатичныйТекст);
				Рекомендации.Добавить(Символы.ПС);
			КонецЕсли;
		КонецЕсли;
		...
		Если СводнаяИнформация.ЗашифрованоНаТекущем > 0 Тогда
			Рекомендации.Добавить(СимволТочкиИПробел);
			Если ПараметрыПоказа = Неопределено ИЛИ НЕ ПараметрыПоказа.ПоказДляДругихПользователей Тогда
				Рекомендации.Добавить(НСтр("ru = 'Некоторые сообщения зашифрованы действующим сертификатом.';
											|en = 'Некоторые сообщения зашифрованы действующим сертификатом.'") + " ");
			КонецЕсли;
			Рекомендации.Добавить(НСтр("ru = 'Если сертификат утерян';
										|en = 'Если сертификат утерян'") + ", ");
			Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'отправьте заявление';
																	|en = 'отправьте заявление'"),,,, "#action?show_master"));
			Рекомендации.Добавить(" " + НСтр("ru = 'на перевыпуск';
											|en = 'на перевыпуск'") + ".");
			Рекомендации.Добавить(Символы.ПС);
		КонецЕсли;
		
		Если СводнаяИнформация.СертификатовНезаблокированных > 0 Тогда
			Рекомендации.Добавить(СимволТочкиИПробел + НСтр("ru = 'Настройте исключения расшифровки по сертификатам, которые более недоступны.';
															|en = 'Настройте исключения расшифровки по сертификатам, которые более недоступны.'") + " ");
			Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Перейти к настройке';
																	|en = 'Перейти к настройке'"),,,, "#action?configure_certificates"));
			Рекомендации.Добавить(".");
			Рекомендации.Добавить(Символы.ПС);
		КонецЕсли;
	...
	ИначеЕсли СтрНайти(ОписаниеОшибки, "не найден файл описания пакета") > 0 Тогда 
		...
		СтатичныйТекст = НСтр("ru = 'Транспортный контейнер входящего сообщения содержит ошибки, его содержимое не может быть корректно извлечено и прочитано.';
								|en = 'Транспортный контейнер входящего сообщения содержит ошибки, его содержимое не может быть корректно извлечено и прочитано.'");
		ПояснениеОшибки.Добавить(СтатичныйТекст);
		
		Рекомендации.Добавить(СимволТочкиИПробел + НСтр("ru = 'Описание контейнера отсутствует.';
														|en = 'Описание контейнера отсутствует.'") + " ");
		Рекомендации.Добавить(НСтр("ru = 'Запросите повторную отправку сообщения в контролирующем органе по эл. почте или телефону.';
									|en = 'Запросите повторную отправку сообщения в контролирующем органе по эл. почте или телефону.'"));
	...
	ИначеЕсли СтрНайти(ОписаниеОшибки, "некорректная структура файла описания пакета") > 0 Тогда 
		...
		СтатичныйТекст = НСтр("ru = 'Транспортный контейнер входящего сообщения содержит ошибки, его содержимое не может быть правильно извлечено и прочитано.';
								|en = 'Транспортный контейнер входящего сообщения содержит ошибки, его содержимое не может быть правильно извлечено и прочитано.'");
		ПояснениеОшибки.Добавить(СтатичныйТекст);
		
		Рекомендации.Добавить(СимволТочкиИПробел + НСтр("ru = 'Расшифровка транспортного сообщения невозможна.';
														|en = 'Расшифровка транспортного сообщения невозможна.'") + " ");
		Рекомендации.Добавить(НСтр("ru = 'Запросите повторную отправку сообщения в контролирующем органе по эл. почте или телефону.';
									|en = 'Запросите повторную отправку сообщения в контролирующем органе по эл. почте или телефону.'"));
	...
	ИначеЕсли СтрНайти(ОписаниеОшибки, "не определен файл подписи под документом") > 0 Тогда
		...
		СтатичныйТекст = НСтр("ru = 'Транспортный контейнер входящего сообщения сформирован некорректно, его содержимое частично отсутствует.';
								|en = 'Транспортный контейнер входящего сообщения сформирован некорректно, его содержимое частично отсутствует.'");
		ПояснениеОшибки.Добавить(СтатичныйТекст);		
		
		Рекомендации.Добавить(СимволТочкиИПробел + НСтр("ru = 'Проверка содержимого транспортного сообщения завершилась с ошибкой.';
														|en = 'Проверка содержимого транспортного сообщения завершилась с ошибкой.'") + " ");
		Рекомендации.Добавить(НСтр("ru = 'Некоторые файлы в контейнере отсутствуют.';
									|en = 'Некоторые файлы в контейнере отсутствуют.'") + " ");
		Рекомендации.Добавить(НСтр("ru = 'Запросите повторную отправку сообщения в контролирующем органе по эл. почте или телефону.';
									|en = 'Запросите повторную отправку сообщения в контролирующем органе по эл. почте или телефону.'"));
	...
	ИначеЕсли СтрНайти(ОписаниеОшибки, "не удалось проверить подпись") > 0 Тогда 
		...
		СтатичныйТекст = НСтр("ru = 'Возникла проблема проверки подписи одного из файлов транспортного сообщения.';
								|en = 'Возникла проблема проверки подписи одного из файлов транспортного сообщения.'");
		ПояснениеОшибки.Добавить(СтатичныйТекст);
		
		Рекомендации.Добавить(СимволТочкиИПробел + НСтр("ru = 'Проверка содержимого транспортного сообщения завершилась с ошибкой.';
														|en = 'Проверка содержимого транспортного сообщения завершилась с ошибкой.'") + " ");
		Рекомендации.Добавить(НСтр("ru = 'Но на расшифровку сообщения она не влияет.';
									|en = 'Но на расшифровку сообщения она не влияет.'"));
...
Процедура ПоказатьФормуЗапросаБлокировкиСообщения(ОповещениеОЗакрытии, ФормаИнициатор = Неопределено)
	...
	Если ОбрабатываемыхСообщений = 1 Тогда
		ТекстСообщения =
			НСтр("ru = 'Подтвердите изменение настройки расшифровки для выбранного сообщения.
					   ...
					   |';
					   |en = 'Подтвердите изменение настройки расшифровки для выбранного сообщения.
					   |Программа более не будет пытаться его расшифровывать.
					   |'");
	Иначе
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Подтвердите изменение настройки расшифровки для выбранных сообщений (%1).
					   ...
					   |';
					   |en = 'Подтвердите изменение настройки расшифровки для выбранных сообщений (%1).
					   |Программа более не будет пытаться их расшифровывать.
					   |'"),
	...
	Части.Добавить(НСтр("ru = 'Обратите внимание, это действие применяется для всех пользователей и рабочих мест.';
						|en = 'Обратите внимание, это действие применяется для всех пользователей и рабочих мест.'"));
	...
	Если ОбрабатываемыхСообщений = 1 Тогда
		ТекстСообщения =
			НСтр("ru = 'Подтвердите активацию расшифровки для выбранного сообщения.
					   |
					   |';
					   |en = 'Подтвердите активацию расшифровки для выбранного сообщения.
					   |
					   |'");
	Иначе
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Подтвердите активацию расшифровки для выбранных сообщений (%1).
					   |
					   |';
					   |en = 'Подтвердите активацию расшифровки для выбранных сообщений (%1).
					   |
					   |'"),
			ОбрабатываемыхСообщений);
	КонецЕсли;
