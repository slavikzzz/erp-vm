...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка выгрузки вложения во временный файл:
                  |%1';
                  |en = 'Ошибка выгрузки вложения во временный файл:
	...
	Если НЕ ЗначениеЗаполнено(УзелСвОтметка) Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Некорректная структура XML файла: не обнаружен узел ""СвОтметка"".';
				|en = 'Некорректная структура XML файла: не обнаружен узел ""СвОтметка"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
