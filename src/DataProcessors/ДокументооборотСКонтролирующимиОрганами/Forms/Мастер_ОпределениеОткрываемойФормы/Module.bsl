...
Процедура АктивизироватьСтраницуНеверныйЛогинИлиПароль(ЭтотОбъект)
	...
	ЭтотОбъект.Заголовок = НСтр("ru = 'Авторизуйтесь на Портале 1С:ИТС';
								|en = 'Авторизуйтесь на Портале 1С:ИТС'"); 
...
Процедура АктивизироватьСтраницуПревышеноКоличествоПопыток(ЭтотОбъект)
	...
	ЭтотОбъект.Заголовок = НСтр("ru = 'Ошибка авторизации на Портале 1С:ИТС';
								|en = 'Ошибка авторизации на Портале 1С:ИТС'"); 
...
Процедура НачатьОпределениеФормыМастера(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ЭтоУпрощенноеЗаявление 
		И НужноПредложитьСоздатьВторичноеВместоПервичного() 
		И НЕ ЗаявлениеОтправлено Тогда
		
		// Вторичный или первичный упрощенный
		СпроситьПроСуществующееПодключение();
		
	ИначеЕсли ЕстьНезавершенноеЗаявление() 
		И НЕ ЗаявлениеОтправлено Тогда
...
Процедура СпроситьПроНеотправленноеЗаявление() Экспорт
	...
	#Если ВебКлиент Тогда
		СсылкаНаЗаявление = НСтр("ru = 'заявление';
								|en = 'заявление'");
	#Иначе
		СсылкаНаЗаявление = Новый ФорматированнаяСтрока(НСтр("ru = 'заявление';
															|en = 'заявление'"),,,,ПолучитьНавигационнуюСсылку(НезавершенноеЗаявление));
	#КонецЕсли
	...
	ТекстВопроса = Новый ФорматированнаяСтрока(
		...
		НСтр("ru = ' есть неотправленное ';
			|en = ' есть неотправленное '"),
		СсылкаНаЗаявление,
		НСтр("ru = ' на подключение к 1С-Отчетности';
			|en = ' на подключение к 1С-Отчетности'"),
		НСтр("ru = ' от ';
			|en = ' от '"),
		...
		НСтр("ru = 'Продолжить работу с этим заявлением или создать новое?';
			|en = 'Продолжить работу с этим заявлением или создать новое?'"));
		
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(1, НСтр("ru = 'Продолжить работу';
							|en = 'Продолжить работу'"));
...
Функция ОткрытьВторичноеЗаявление()
	...
	ОткрытьВторичноеЗаявление = 
		...
		И МультирежимВызовСервера.ПользовательПодключенК1СОтчетности(Организация)
...
Процедура НачатьОткрытьЗаявлениеНаПодключениеУпрощенное()
	...
	Иначе
		...
		РезультатПолученияТикета = ОбработкаЗаявленийАбонентаВызовСервера.ТикетАутентификацииНаПорталеПоддержки();
		...
		Иначе
			...
			ИначеЕсли НЕ ИгнорироватьОшибку И РезультатПолученияТикета.КодОшибки = "ПревышеноКоличествоПопыток" Тогда
				...
				Элементы.ПодсказкаПревышеноКоличествоПопыток.Заголовок = Новый ФорматированнаяСтрока(
					НСтр("ru = 'Подключение к 1С-Отчетности временно невозможно, т.к. не удалось авторизоваться на Портале 1С:ИТС.';
						|en = 'Подключение к 1С-Отчетности временно невозможно, т.к. не удалось авторизоваться на Портале 1С:ИТС.'"),Символы.ПС,
					НСтр("ru = 'Ответ сервера: превышено количество попыток авторизации.';
						|en = 'Ответ сервера: превышено количество попыток авторизации.'"),Символы.ПС,
...
Процедура НачатьОткрытьЗаявлениеНаПодключениеУпрощенное_ПослеПоискаКриптопровайдеров(Результат, ВходящийКонтекст) Экспорт
	...
	ИначеЕсли ПоказатьОшибку Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Не удалось создать заявление по причине:
                                    ...
                                    |en = 'Не удалось создать заявление по причине:
                                    |%1
                                    |
                                    |Что делать?
                                    |1. Попробуйте устранить проблему и повторите создание заявления.
		...
		ПоказатьПредупреждение(
			...
			НСтр("ru = 'Ошибка создания заявление по 1С-Отчетности';
				|en = 'Ошибка создания заявление по 1С-Отчетности'"));
...
Функция ЕстьНезавершенноеЗаявление()
	...
	Заявление = НезавершенноеЗаявление();
	
	Если ЗначениеЗаполнено(Организация) 
		И ЗначениеЗаполнено(УчетнаяЗаписьОрганизации) 
		И Мультирежим.ЭтоМультиРежимПоОрганизации(Организация)
		И НЕ МультирежимВызовСервера.ПользовательПодключенК1СОтчетности(Организация) Тогда
		
		Возврат Ложь;
		
	ИначеЕсли ЗначениеЗаполнено(Заявление) 
		И ЗначениеЗаполнено(Организация) 
		И ЗначениеЗаполнено(УчетнаяЗаписьОрганизации) 
		И Заявление.ЭтоМультиРежим <> УчетнаяЗаписьОрганизации.ЭтоМультиРежим Тогда
		
		Возврат Ложь;
		
	Иначе

		Возврат ЗначениеЗаполнено(Заявление);
...
Процедура ЗаполнитьСписокКриптопровайдеров(ВыполняемоеОповещение)
	...
	// Т.к. сначала мы спрашиваем, есть ли сертификат и для поиска сертификатов нам нужны расширения
