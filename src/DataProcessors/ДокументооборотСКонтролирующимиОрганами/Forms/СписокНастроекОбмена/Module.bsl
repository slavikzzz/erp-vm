...
Процедура ОбновитьНастройкиОбменаОрганизацийПослеПолученияНастроекОбмена(Результат, ДополнительныеПараметры) Экспорт
	...
	Для каждого ЭлементНастроекОбмена Из Результат.НастройкиОбмена Цикл
		...
		Иначе
			...
			Если ЗначениеЗаполнено(СтатусДействительности) Тогда
				Если ИнформацияОСтатусе <> Неопределено И ИнформацияОСтатусе.ТребуетПродления
					...
					И ЭлементНастроекОбмена.СпецоператорСвязи <> ПредопределенноеЗначение("Перечисление.СпецоператорыСвязи.Прочие") Тогда
					ЗаявлениеТекст = СтрШаблон(
						?(ДействияДляЭлемента.ПоказыватьСертификат, НСтр("ru = 'Продлить сертификат (%1)';
																		|en = 'Продлить сертификат (%1)'"), НСтр("ru = 'Продлить лицензию (%1)';
																													|en = 'Продлить лицензию (%1)'")),
						СтатусДействительности);
				Иначе
					ЗаявлениеТекст = СтрШаблон(
						?(ДействияДляЭлемента.ПоказыватьСертификат, НСтр("ru = 'Сертификат %1';
																		|en = 'Сертификат %1'"), НСтр("ru = 'Лицензия %1';
																										|en = 'Лицензия %1'")),
						СтатусДействительности);
				КонецЕсли;
		...
		Иначе
			ФНССтрокой = ?(ЭлементНастроекОбмена.ПредназначенаДляДокументооборотаСФНС, НСтр("ru = 'Да';
																							|en = 'Да'"), НСтр("ru = 'Нет';
																												|en = 'Нет'"));
		КонецЕсли;
		ПФРСтрокой = ?(ЭлементНастроекОбмена.ПредназначенаДляДокументооборотаСПФР, НСтр("ru = 'Да';
																						|en = 'Да'"), НСтр("ru = 'Нет';
																											|en = 'Нет'"));
		...
		Иначе
			РосстатСтрокой = ?(ЭлементНастроекОбмена.ПредназначенаДляДокументооборотаСФСГС, НСтр("ru = 'Да';
																								|en = 'Да'"), НСтр("ru = 'Нет';
																													|en = 'Нет'"));
		КонецЕсли;
		ФСССтрокой = ?(ЭлементНастроекОбмена.НастройкиОбменаФСС_ИспользоватьОбмен, НСтр("ru = 'Да';
																						|en = 'Да'"), НСтр("ru = 'Нет';
																											|en = 'Нет'"));
		...
		Если СтрокаНастроекОбмена <> Неопределено Тогда
			...
			СтрокаНастроекОбмена.ПометкаУдаленияСтрокой 		= ?(ЭлементНастроекОбмена.ПометкаУдаления, НСтр("ru = 'Да';
																												|en = 'Да'"), НСтр("ru = 'Нет';
																																	|en = 'Нет'"));
...
Функция ДействияДляЭлементаНастроекОбмена(ЭлементНастроекОбмена)
	...
	ПредлагатьПодключиться = (ЭлементНастроекОбмена.ВидОбменаСКонтролирующимиОрганами <>
		...
		ИЛИ ЭлементНастроекОбмена.ПравоЧтенияУчетнойЗаписиОбмена И ПоказыватьСертификат
		И ЭлементНастроекОбмена.СвойстваСертификатаРуководителя = Неопределено
		И НЕ ЗначениеЗаполнено(ЭлементНастроекОбмена.СертификатРуководителя) И ЭлементНастроекОбмена.ЭтоМультиРежим;
