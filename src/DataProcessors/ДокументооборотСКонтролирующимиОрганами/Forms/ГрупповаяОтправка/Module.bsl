...
Процедура ПоказатьРезультатОтправкиПоГиперссылке(ТекущиеДанные)
	...
	Если Ошибка = РезультатОтправкиПриПустойОшибке() Тогда
		Текст = РезультатОтправкиПриПустойОшибке() + "." + Символы.ПС + НСтр("ru = 'Подробности см. в окне сообщений.';
																			|en = 'Подробности см. в окне сообщений.'"); 
...
Процедура ПодтвердитьВыполнениеДействия(Команда, ОписаниеОповещения)
	...
	Иначе
		Кнопки.Добавить(НСтр("ru = 'Продолжить';
							|en = 'Продолжить'"), НСтр("ru = 'Начать проверку';
														|en = 'Начать проверку'"));
	КонецЕсли;
...
Функция ТекстПредупрежденияОбОтправке()
	...
	Текст = НСтр("ru = 'Сейчас начнется отправка отчетов.
          ...
          |en = 'Сейчас начнется отправка отчетов.
          |В процессе отправки могут быть запрошены пароли от закрытых ключей.'");
...
Функция ТекстПредупрежденияОПроверке()
	...
	Текст = НСтр("ru = 'Сейчас начнется проверка отчетов на специальном сервисе в Интернете.
          ...
          |en = 'Сейчас начнется проверка отчетов на специальном сервисе в Интернете.
          |
          |Если отчеты содержат персональные сведения физ. лиц, убедитесь, что получено согласие на передачу этих данных.'");
...
Процедура СпроситьПроСПАРК()
	...
	Текст = НСтр("ru = 'Среди выбранных отчетов есть бухгалтерская отчетность, копию которой можно опубликовать в крупнейшей информационной системе СПАРК.
                  |Опубликовать?';
                  |en = 'Среди выбранных отчетов есть бухгалтерская отчетность, копию которой можно опубликовать в крупнейшей информационной системе СПАРК.
                  |Опубликовать?'");
...
Процедура ВыполнитьПроверкуОдного_Завершение(Результат, ВходящийКонтекст) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		СертификатНедействителенНетПросроченности = Результат.СертификатНедействителен И НЕ Результат.СертификатПросрочен;
		
		Если СертификатНедействителенНетПросроченности Тогда
			ТекстОшибки = ДлительнаяОтправкаКлиентСервер.СообщениеСертификатНедействителен();
			ДобавитьОшибку(Ошибки, ТекущаяОрганизация, ТекстОшибки);
		ИначеЕсли Результат.ЛицензияПросрочена И Результат.СертификатПросрочен Тогда 
...
Функция ПроверитьЧтоЕстьУчетка(КонтекстЭДОСервер, ТекущаяОрганизация, ВидОргана, ПроверенныеОрганизации, Ошибки)
	...
	Иначе
		...
		Подключен = ИнтерфейсыВзаимодействияБРО.ПодключенДокументооборотСКонтролирующимОрганом(ТекущаяОрганизация, ВидОргана, Истина);
	...
	Если ПараметрыГрупповойОтправки = Неопределено Тогда
		Если Форма.ВыполняетсяОтправка Тогда
			Элементы.Состояние.Заголовок = НСтр("ru = 'Выполняется подготовка к отправке отчетов.';
												|en = 'Выполняется подготовка к отправке отчетов.'");
		Иначе
			Элементы.Состояние.Заголовок = НСтр("ru = 'Выполняется подготовка к проверке отчетов.';
												|en = 'Выполняется подготовка к проверке отчетов.'");
		КонецЕсли;
	Иначе
...
Функция ПодстрокаВремяСтарта(Форма)
	...
	Если Форма.ВыполняетсяОтправка Тогда
		Текст = СтрШаблон(НСтр("ru = 'Отправка отчетов запущена в %1.';
								|en = 'Отправка отчетов запущена в %1.'"), Формат(ВремяНачалаОтправки, "ДФ=HH:mm"));
	Иначе
		Текст = СтрШаблон(НСтр("ru = 'Проверка отчетов запущена в %1.';
								|en = 'Проверка отчетов запущена в %1.'"), Формат(ВремяНачалаОтправки, "ДФ=HH:mm"));
	КонецЕсли;
...
Функция ПодстрокаСостояниеДействия(Форма)
	...
	Если Форма.ВыполняетсяОтправка Тогда
		Подстрока1 = НСтр("ru = 'Отправляется отчет ';
							|en = 'Отправляется отчет '");
	Иначе
		Подстрока1 = НСтр("ru = 'Проверяется отчет ';
							|en = 'Проверяется отчет '");
	КонецЕсли;
...
Процедура ИзменитьОформлениеСчетчика(Форма)
	...
	Подстрока1 = НСтр("ru = 'Выберите отчеты к отправке, используя отборы и флажки.';
						|en = 'Выберите отчеты к отправке, используя отборы и флажки.'");
...
Процедура ПоказатьФормуРезультатаГрупповойОтправки_Завершение(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат = НСтр("ru = 'Показать неотправленные';
							|en = 'Показать неотправленные'") Тогда
...
Процедура УстановитьПараметрыЗапроса(Запрос)
	...
	Запрос.УстановитьПараметр("ТребуетУточнений", ДокументооборотСКОКлиентСервер.СтатусСданоТребуетУточнения());
...
Функция ПараметрВидКонтролирующегоОргана()
	...
	ВидКонтролирующегоОргана.Добавить(Перечисления.ТипыКонтролирующихОрганов.ЦБ);
...
Процедура ЗалогироватьДлительнуюОперацию(ПараметрыГрупповойОтправки, ЭтоСтарт, ВыполняетсяОтправка, ВыполняетсяПроверка)
	...
	Если ВыполняетсяОтправка Тогда
		Действие = НСтр("ru = 'групповой отправки';
						|en = 'групповой отправки'");
	Иначе
		Действие = НСтр("ru = 'групповой проверки';
						|en = 'групповой проверки'");
	КонецЕсли;
	
	Если ЭтоСтарт Тогда
		ТекстСообщения = НСтр("ru = 'Электронный документооборот с контролирующими органами. Старт ';
								|en = 'Электронный документооборот с контролирующими органами. Старт '") + Действие;
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'Электронный документооборот с контролирующими органами. Окончание ';
								|en = 'Электронный документооборот с контролирующими органами. Окончание '") + Действие;
...
Процедура ЗалогироватьОтправкуОтчета(ДанныеТекущегоОтчета, Результат, Ошибки, Время)
	...
	ТекстСообщения = НСтр("ru = 'Электронный документооборот с контролирующими органами. ';
							|en = 'Электронный документооборот с контролирующими органами. '") + Действие;
