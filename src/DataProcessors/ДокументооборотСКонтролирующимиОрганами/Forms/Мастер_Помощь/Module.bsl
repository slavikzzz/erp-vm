...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("СсылкаНаИнструкцию") Тогда
		СсылкаНаИнструкцию = Параметры.СсылкаНаИнструкцию;
	Иначе
		СсылкаНаИнструкцию = "https://astral.ru/help/1s-otchetnost";
	КонецЕсли;
	
	Если НЕ ИнтерфейсыВзаимодействияБРО.ПодключенДокументооборотСКонтролирующимОрганом() Тогда
...
&НаКлиенте
Процедура КоннектДействиеНажатие(Элемент)
	
	Если ПодключенКоннект = 2 Тогда
		ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку("https://sa.1c-connect.com/join/s/i8r8d6nj6fdc7yuunj9tr96qoy/");
		//ПерейтиПоНавигационнойСсылке("connect://s/i8r8d6nj6fdc7yuunj9tr96qoy");
	КонецЕсли;
...
Процедура ОтправитьСообщениеВСлужбуТехническойПоддержки(Команда)
	...
	Тема = НСтр("ru = 'Ошибка при работе с Облачной 1С-Отчетностью';
				|en = 'Ошибка при работе с Облачной 1С-Отчетностью'");
	
	Если Не ЗначениеЗаполнено(ТекстОбращения) Тогда
		ТекстОбращения = НСтр("ru = '<Опишите ошибку при работе с 1С-Отчетностью>';
								|en = '<Опишите ошибку при работе с 1С-Отчетностью>'");
	КонецЕсли;
...
Процедура Заказать(Команда)
	...
	Если СтрДлина(Телефон) <> 12 Тогда // Ожидается +79012345678
		...
		ТекстОшибки = НСтр("ru = 'Для продолжения необходимо указать номер телефона в формате +7 901 234-56-78';
							|en = 'Для продолжения необходимо указать номер телефона в формате +7 901 234-56-78'");
		ОбновитьСтатус(0);
		Возврат;
...
&НаКлиенте
Процедура ОтправитьЭлектронноеПисьмоВПоддержку(Команда)
	УправлениеКонтактнойИнформациейКлиент.СоздатьЭлектронноеПисьмо(
		"",
		"v8@1c.ru",
		ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты"));
	...
	ИначеЕсли ТипЗнч(ЗаданиеВыполнено) = Тип("Структура") И ОтложенноеЗавершение Тогда 
		...
		Если Результат = Неопределено Тогда
			ТекстОшибки = НСтр("ru = 'Извините, не удалось заказать обратный звонок, попробуйте снова через несколько минут.';
								|en = 'Извините, не удалось заказать обратный звонок, попробуйте снова через несколько минут.'");
			ОбновитьСтатус(0);
		...
		Иначе
			...
			ИначеЕсли Результат.Свойство("Код") И Результат.Код < 9 Тогда 
				ТекстОшибки = НСтр("ru = 'Извините, не удалось заказать обратный звонок из-за неполадок в интернет соединении.';
									|en = 'Извините, не удалось заказать обратный звонок из-за неполадок в интернет соединении.'");
				ОбновитьСтатус(0);
			КонецЕсли;
...
Процедура ОбновитьСтатус(ВПроцессе = 0)
	...
	Элементы.ГруппаКоннект.Видимость = ПодключенКоннект > 0;
	Если ПодключенКоннект = 1 Тогда
		Элементы.КоннектДействие.Заголовок = НСтр("ru = 'настроить';
													|en = 'настроить'");
	ИначеЕсли ПодключенКоннект = 2 Тогда
		Элементы.КоннектДействие.Заголовок = НСтр("ru = 'подключить';
													|en = 'подключить'");
	КонецЕсли;
