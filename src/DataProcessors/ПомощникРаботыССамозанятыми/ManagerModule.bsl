...
Процедура ЗаписатьДокумент(ДокументОбъект, Параметры)
	...
	Исключение
		...
		Если Не ЗначениеЗаполнено(ДокументОбъект.Ссылка) Тогда
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось создать документ ""%1""';
					|en = 'Cannot create the ""%1"" document.'"), ДокументОбъект.Метаданные().Синоним);
		Иначе
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось записать документ %1';
											|en = 'Cannot save document %1'"), ДокументОбъект);
		КонецЕсли;
	...
	Если Не ОшибокНет Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось провести документ %1';
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Контрагент не найден:
											|   ФИО: %1
											|   ИНН: %2';
											|en = 'The counterparty is not found:
											|   Full name: %1
											|   TIN: %2'"), ФИО, ИНН);
