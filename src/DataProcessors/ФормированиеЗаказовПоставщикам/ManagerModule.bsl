...
Процедура ЗаписатьПараметрыПоддержанияЗапаса(Таблица) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Запись параметров поддержания запаса';
										|en = 'Write inventory level control parameters'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция СоздатьЗаказы(Объект) Экспорт
	...
	Для Каждого Элемент Из МассивТаблиц Цикл
		...
		ТекстКомментарий = НСтр("ru = 'Сформирован автоматически обработкой ""Формирование заказов поставщикам"".';
								|en = 'Generated automatically using the ""Purchase order generation"" data processor.'");
...
//++ НЕ УТКА
		...
		|		КОНЕЦ КАК ПодразделениеПолучатель,
		|	
		|	Потребности.ЗаказНаПроизводство                        КАК Заказ,
		|	Потребности.Назначение                                 КАК Назначение,
		|	Потребности.ЗаказНаПроизводство.НачатьНеРанее          КАК ДатаОтгрузки,
		...
		|		- Потребности.Запланировано                        КАК Требуется,
		|	0 КАК Обеспечено
		|	
		|	ИЗ
...
Функция ТекстЗапросаЗаказов()
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЭтапПроизводства2_2)
			И ПравоДоступа("Чтение", Метаданные.Документы.ЗаказНаПроизводство2_2) Тогда
