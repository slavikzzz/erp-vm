...
Процедура ОформитьАктОРасхождениях(Команда)
	...
	Для Каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		...
		Если ТипЗнч(Строка.Ссылка) = Тип("ДокументСсылка.ЗаказНаПеремещение") Тогда
			Если Строка.СостояниеНакладной = 1 Тогда
				ТекстОшибки = НСтр("ru = 'Для распоряжения %1 невозможно оформить акт о расхождениях, т.к. нет ни одной накладной.';
									|en = 'Cannot prepare a discrepancy report for the %1 reference as there are no invoices.'");
		...
		Иначе
			ТекстОшибки = НСтр("ru = 'Для распоряжения %1 невозможно оформить акт о расхождениях, т.к. оно не является перемещением товаров.';
								|en = 'Cannot prepare a discrepancy report for the %1 reference as it is not a goods transfer.'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, Строка.Ссылка);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(Накладные) Или ЗначениеЗаполнено(Заказы) Тогда
		...
		НаименованиеЗадания = НСтр("ru = 'Журнал внутренних документов: Оформление актов о расхождениях';
									|en = 'Journal of internal documents: Registration of discrepancy reports'");
	...
	ИначеЕсли Элементы.СписокРаспоряженияНаОформление.ВыделенныеСтроки.Количество() > 1 Тогда
		...
		ТекстОшибки = НСтр("ru = 'Не выбрано ни одного документа, для которого можно было бы оформить акт о расхождениях.';
							|en = 'No documents are selected for which a discrepancy report can be created.'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СписокОшибок, "Объект.СписокРаспоряженияНаОформление", ТекстОшибки, Неопределено);
	...
	Иначе
		...
		Для каждого Идентификатор Из Элементы.СписокРаспоряженияНаОформление.ВыделенныеСтроки Цикл
			...
			Если ТипЗнч(СтрокаТаблицы.Ссылка) = Тип("ДокументСсылка.ЗаказНаВнутреннееПотребление")
				...
				//++ НЕ УТ
				ИЛИ ТипЗнч(СтрокаТаблицы.Ссылка) = Тип("ДокументСсылка.ЗаказМатериаловВПроизводство")
				//-- НЕ УТ

				//++ НЕ УТКА
				ИЛИ ТипЗнч(СтрокаТаблицы.Ссылка) = Тип("ДокументСсылка.ЗаказНаРемонт")
				//-- НЕ УТКА
		...
		Если СписокДокументов.Количество() = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Для выбранных документов отчет не поддерживается. Выполнение команды невозможно.';
											|en = 'Report is not supported for the selected documents. Cannot run the command.'"));
			Возврат;
		КонецЕсли;
...
Процедура СформироватьТекстыЗапросов()
	...
	Для каждого КЗ Из Состояния Цикл
		...
		ИначеЕсли КЗ.Ключ = "СостоянияПриходныхОрдеров" Тогда
			...
			|							КОНЕЦ
			//++ НЕ УТ
			|					КОГДА ТИПЗНАЧЕНИЯ(СостоянияПриходныхОрдеров.Распоряжение) = ТИП(Документ.ДвижениеПродукцииИМатериалов)
			...
			//++ НЕ УТ
			|	И НЕ (ТИПЗНАЧЕНИЯ(СостоянияПриходныхОрдеров.Распоряжение) = ТИП(Документ.ЗаказМатериаловВПроизводство)
			|			И ВЫРАЗИТЬ(СостоянияПриходныхОрдеров.Распоряжение КАК Документ.ЗаказМатериаловВПроизводство).УправлениеПроизводством2_2)
			|	И НЕ (ТИПЗНАЧЕНИЯ(СостоянияПриходныхОрдеров.Распоряжение) = ТИП(Документ.ДвижениеПродукцииИМатериалов)
			|			И ВЫРАЗИТЬ(СостоянияПриходныхОрдеров.Распоряжение КАК Документ.ДвижениеПродукцииИМатериалов).ХозяйственнаяОперация В(
			...
			//++ НЕ УТ
			|	ИЛИ (ТИПЗНАЧЕНИЯ(СостоянияПриходныхОрдеров.Распоряжение) = ТИП(Документ.ЗаказМатериаловВПроизводство)
			|			И ВЫРАЗИТЬ(СостоянияПриходныхОрдеров.Распоряжение КАК Документ.ЗаказМатериаловВПроизводство).УправлениеПроизводством2_2)
			|	ИЛИ (ТИПЗНАЧЕНИЯ(СостоянияПриходныхОрдеров.Распоряжение) = ТИП(Документ.ДвижениеПродукцииИМатериалов)
			|			И ВЫРАЗИТЬ(СостоянияПриходныхОрдеров.Распоряжение КАК Документ.ДвижениеПродукцииИМатериалов).ХозяйственнаяОперация В(
			|						ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаМатериаловВКладовую),
			|						ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзКладовой),
			|						ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратМатериаловИзКладовой),
	...
	ТекстЗапросаСпискаРаспоряжений =
		...
		|					И ВЫРАЗИТЬ(ОстаткиСостояния.Ссылка КАК Документ.ЗаказМатериаловВПроизводство).УправлениеПроизводством2_2 ТОГДА
		|				НЕ &ОтбиратьПоСкладу ИЛИ ОстаткиСостояния.Склад В (&Склад)
		|			КОГДА ТИПЗНАЧЕНИЯ(ОстаткиСостояния.Ссылка) = ТИП(Документ.ДвижениеПродукцииИМатериалов)
		|					И ВЫРАЗИТЬ(ОстаткиСостояния.Ссылка КАК Документ.ДвижениеПродукцииИМатериалов).ХозяйственнаяОперация В(
		|						ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаМатериаловВКладовую),
		|						ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзКладовой),
		|						ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратМатериаловИзКладовой),
		|						ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеремещениеПолуфабрикатов),
...
Функция ПараметрыФормыСозданныхДокументов(ОформляетсяАктОРасхождении = Ложь)
	...
	//++ НЕ УТ
	ПараметрыФормы.СобытияОбновления.Добавить("Запись_ПередачаМатериаловВПроизводство");
