...
Процедура ИмяФайлаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогОткрытияФайла.Заголовок = НСтр("ru = 'Выберите путь к файлу выгрузки данных из ТиС';
										|en = 'Select file path of data export from 1C:Trade and Warehouse'");
...
Процедура Подключаемый_СтраницаОжидания_ПриПереходеДалее(Отказ)
	...
	Состояние(НСтр("ru = 'Загрузка данных...';
					|en = 'Importing data…'"),, НСтр("ru = 'Выполняется загрузка данных из ТиС';
														|en = 'Data is being imported from 1C:Trade and Warehouse'"));
...
Функция ЗагрузитьНаСервере()
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Помощник перехода с Торговля и склад.Удаление временных файлов';
										|en = 'Wizard of migration from 1C:Trade and Warehouse.Delete temporary files'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
