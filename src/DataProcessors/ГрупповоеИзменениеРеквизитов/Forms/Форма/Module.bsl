	...
	Иначе
		Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
			ВызватьИсключение(НСтр("ru = 'Недостаточно прав доступа.';
									|en = 'Insufficient access rights.'"), КатегорияОшибки.НарушениеПравДоступа);
		КонецЕсли;
...
Процедура ОбработатьРезультатИзменения(РезультатИзменения = Неопределено, ПродолжитьОбработку = Неопределено)
	...
	Если ТекущееСостояниеИзменения.ПоказыватьПроцентОбработанных Тогда
		...
		Состояние(НСтр("ru = 'Обрабатываются элементы...';
						|en = 'Processing in progress…'"), ТекущийПроцент, НСтр("ru = 'Изменение выделенных элементов';
																					|en = 'Edit selected items'"));
	КонецЕсли;
	...
	Если ОбработкаЗавершена Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Изменение реквизитов элементов';
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Изменены элементы (%1).';
					|en = '%1 items have been edited.'"), ТекущееСостояниеИзменения.КоличествоИзмененных));
	...
	Если Объект.ИзменятьВТранзакции И Не ОбработкаЗавершена Тогда
		...
		Если КоличествоПропущенных > 0 И Не ТекущееСостояниеИзменения.ПрерватьИзменение Тогда
			...
			СтрокаТаблицы.Объект = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '... и другие элементы (%1)';
					|en = '… and other items (%1)'"), КоличествоПропущенных);
...
Процедура ПерейтиНаСтраницуВсеГотово()
	...
	Элементы.НадписьУспешноЗавершено.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Реквизиты выбранных элементов были изменены.
			...
			|Total items edited: %1.'"), ТекущееСостояниеИзменения.КоличествоИзмененных);
...
Процедура СформироватьПояснениеОНастроенныхИзменениях()
	...
	Иначе
		Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
			ШаблонПояснения = НСтр("ru = 'Изменить реквизит %1 в выбранных элементах';
									|en = 'Change the %1 attribute for the selected items'") // например, "Изменить реквизит ""Склад""..."
		ИначеЕсли ИзменяемыеРеквизиты.Количество() > 3 Тогда
			ШаблонПояснения = НСтр("ru = 'Изменить реквизиты (%1) в выбранных элементах';
									|en = 'Change %1 attributes for the selected items'"); // например, "Изменить реквизиты (5)..."
		ИначеЕсли ИзменяемыеРеквизиты.Количество() > 1 Тогда
			ШаблонПояснения = НСтр("ru = 'Изменить реквизиты %1 в выбранных элементах';
									|en = 'Change the %1 attributes for the selected items'"); // например, "Изменить реквизиты ""Склад"", ""Офис""..."
		Иначе	
			ШаблонПояснения = "";
		КонецЕсли;
			...
			Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
				ШаблонПояснения = ?(ПустаяСтрока(Пояснение),
					НСтр("ru = 'Изменить реквизит %1 в табличной части ""%2""';
						|en = 'Change the attribute %1 in tabular section ""%2""'"),
					НСтр("ru = 'реквизит %1 в табличной части ""%2""';
						|en = 'the %1 attribute in tabular section ""%2""'")); 
			ИначеЕсли ИзменяемыеРеквизиты.Количество() > 3 Тогда
				ШаблонПояснения = ?(ПустаяСтрока(Пояснение),
					НСтр("ru = 'Изменить реквизиты (%1) в табличной части ""%2""';
						|en = 'Change %1 attributes in tabular section ""%2""'"),
					НСтр("ru = 'реквизиты (%1) в табличной части ""%2""';
						|en = '%1 attributes in tabular section ""%2""'")); 
			Иначе 
				ШаблонПояснения = ?(ПустаяСтрока(Пояснение),
					НСтр("ru = 'Изменить реквизиты %1 в табличной части ""%2""';
						|en = 'Change the %1 attributes in tabular section ""%2""'"),
					НСтр("ru = 'реквизиты %1 в табличной части ""%2""';
						|en = 'the %1 attributes in tabular section ""%2""'")); 
			КонецЕсли;
	...
	Если ПустаяСтрока(КодАлгоритма) Тогда
		...
		|// %1 - an object to be processed.'"), "Объект") + Символы.ПС;
	КонецЕсли;
