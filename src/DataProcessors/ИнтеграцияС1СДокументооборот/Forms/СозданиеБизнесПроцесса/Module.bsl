...
Процедура ОбработатьРезультат(Дерево, РезультатДеревоШаблонов)
	...
	Для Каждого Раздел Из РезультатДеревоШаблонов.BusinessProcessTemplatesTree Цикл
		...
		Если РазделДерева.Наименование = НСтр("ru = 'Другие шаблоны';
												|en = 'Other templates'") Тогда
...
Процедура ОбработатьРезультат_2_1_18_1(Дерево, РезультатДеревоШаблонов)
	...
	Для Каждого Раздел Из РезультатДеревоШаблонов.BusinessProcessTemplatesTree Цикл
		...
		Если Раздел.name = НСтр("ru = 'Рекомендованные шаблоны';
								|en = 'Recommended templates'") Тогда
		...
		Иначе
			...
			Если РазделДерева.Наименование = НСтр("ru = 'Другие варианты';
													|en = 'Other options'") Тогда
				РазделДерева.Развернуть = Ложь;
			Иначе
				РазделДерева.Развернуть = Истина;
			КонецЕсли;
			...
			Если ЗаполнитьСтартоватьПроцесс(Данные.ТипПроцесса, ПараметрыФормы) Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Стартован бизнес-процесс по шаблону ""%1"".';
						|en = 'Business process started by template ""%1"".'"), Данные.Наименование);
