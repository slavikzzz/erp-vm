			...
			Элементы.ДекорацияИнфо.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
				СтрШаблон(
					НСтр("ru = 'Не настроено заполнение документа 1С:Документооборота из ""%1"". <a href = ""%2"">Настроить.</a>';
						|en = 'The population of document from 1C:Document Management from ""%1"" is not set up. <a href = ""%2"">Configure.</a>'"),
					ИмяПотребителя,
					МетаданныеПотребителя.ПолноеИмя()));
		Иначе
			Элементы.ДекорацияИнфо.Заголовок = СтрШаблон(
				НСтр("ru = 'Не настроено заполнение документа 1С:Документооборота из ""%1"".';
					|en = '1C:Document Management document population is not set up from ""%1"".'"), ИмяПотребителя);
		КонецЕсли;
...
Процедура Подписать(Команда)
	...
	СвойстваФайла.ДанныеПодписейФайла =
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ДанныеПодписей(ТекущиеДанные.ТаблицаПодписей);
...
&НаКлиенте
Процедура ДекорацияОбъектОчиститьНажатиеЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(ИдентификаторКорневойПапки1СДокументооборот) Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Не указана корневая папка для хранения файлов 1С:Документооборота.';
				|en = 'Root folder for 1C:Document Management file saving is not specified.'"));
		Возврат;
	КонецЕсли;
