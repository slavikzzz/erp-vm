			...
			Если ДетальныеСведенияЗадачи.ВключенХронометраж Тогда
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Включен хронометраж по ""%1""';
						|en = 'Timing by ""%1"" is enabled'"), Строка(СтрокаЗадачи.Задача));
				Состояние(Текст);
			КонецЕсли;
...
Функция ОбработатьФормуСогласноВерсииСервиса()
	...
	Заголовок = НСтр("ru = 'Документооборот: задачи мне';
					|en = 'Document Management: my tasks'");
...
Процедура ЗаполнитьПоляЧислаЗадач()
	...
	Заголовок =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Документооборот: задачи мне (Новых: %1 / Просроченных: %2 / Всего: %3)';
			|en = 'Document Management: my tasks (New: %1 / Overdue: %2 / Total: %3)'"),
...
Процедура ПереключитьХронометражЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Если ДетальныеСведенияЗадачи.ВключенХронометраж Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Включен хронометраж по ""%1""';
				|en = 'Timing by ""%1"" is enabled'"), Строка(ПараметрыОповещения.Задача));
		Состояние(Текст);
	КонецЕсли;
...
Процедура ВыполнитьЗадачуПерваяЗавершение(Ответ, ПараметрыОповещения) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Или Ответ = Истина Тогда
		ВыполнитьЗадачу(1);
...
Процедура ВыполнитьЗадачуЗавершение(ВыбранныйИсполнитель, НомерКнопки) Экспорт
	...
	Иначе
		...
		ДанныеЗадачи.Вставить("Задача", СтрокаЗадачи.Задача);
		ДанныеЗадачи.Вставить("ЗадачаID", СтрокаЗадачи.ЗадачаID);
		ДанныеЗадачи.Вставить("ЗадачаТип", СтрокаЗадачи.ЗадачаТип);
		ДанныеЗадачи.Вставить("РезультатID", СтрокаЗадачи.РезультатID);
	КонецЕсли;
	...
	Если ЗаписатьЗадачу(
		...
		ПараметрыОповещения = Новый Структура("name, ID, type",
			ДанныеЗадачи.Задача, ДанныеЗадачи.ЗадачаID, ДанныеЗадачи.ЗадачаТип);
		...
		Если ДобавлятьРаботуВЕжедневныйОтчетПриВыполненииЗадачи Тогда
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("Источник", ДанныеЗадачи.Задача);
			ПараметрыФормы.Вставить("ИсточникID", ДанныеЗадачи.ЗадачаID);
			ПараметрыФормы.Вставить("ИсточникТип", ДанныеЗадачи.ЗадачаТип);
			ОткрытьФорму("Обработка.ИнтеграцияС1СДокументооборот.Форма.ДобавлениеРаботы",
				ПараметрыФормы,
				ДанныеЗадачи.ЗадачаID);
		КонецЕсли;
...
&НаСервере
Функция ЗаписатьЗадачу(ДанныеЗадачи, Комментарий, ВыполнитьЗадачу, НомерКнопки, Знач ПредметыЗадачи,
		ВыбранныйИсполнитель, ПредметСсылка)
	...
	ЗаполнитьСвойстваОбъектаПоТипуЗадачи(Прокси, Задача, Комментарий, НомерКнопки, ДанныеЗадачи.РезультатID);
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, РезультатЗаписи, "DMError") Тогда
		Возврат Ложь;
	Иначе
		Если ДанныеЗадачи.ЗадачаТип = "DMBusinessProcessApprovalTaskApproval" И ВыполнитьЗадачу Тогда
