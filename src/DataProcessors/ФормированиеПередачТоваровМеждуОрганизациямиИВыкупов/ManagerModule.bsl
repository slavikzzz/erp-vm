...
Процедура СоздатьДокументыПередачи(Параметры, АдресВременногоХранилища) Экспорт
	...
	Иначе
		//++ НЕ УТКА
		Запрос.УстановитьПараметр("ФормируемыеДокументы", Параметры.ФормируемыеДокументы);
		//-- НЕ УТКА
...
Функция СоздатьДокументыПоПериоду(ТекущийПериод, ТаблицаНоменклатурыСОтрицательнымиОстатками, МенеджерВременныхТаблиц, ПараметрыСоздания, СозданныеДокументы)
	...
	//++ НЕ УТКА
	Запрос.УстановитьПараметр("ФормируемыеДокументы", ПараметрыСоздания.ФормируемыеДокументы);
	//-- НЕ УТКА
	...
	|			ИЛИ ВЫБОР
	//++ НЕ УТКА
	|					КОГДА &ФормируемыеДокументы = ""ТолькоВыкупыТоваровНаХранении""
	|						ТОГДА ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)
	|					КОГДА &ФормируемыеДокументы = ""ТолькоВыкупыТоваровДавальца""
	...
	|						ТОГДА ВЫБОР
	//++ НЕ УТКА
	|								КОГДА ВтОтрицательныеОстатки.ТипЗапасов В (
	|															ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.МатериалДавальца),
	|															ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца),
	|															ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца))
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				Иначе
					ТекстИсключения = НСтр("ru = 'По хозяйственной операции %ХозяйственнаяОперация% оформление документов не поддерживается.';
											|en = 'Document registration is not supported for the %ХозяйственнаяОперация% business transaction.'");
		...
		Если Не ЗначениеЗаполнено(ВыборкаПоОтрицательнымОстаткам.ВидЗапасовОтправителя) Тогда
			...
			Иначе
				ТекстИсключения = НСтр("ru = 'При определении источников передачи товаров не удалось определить вид запасов. Обратитесь к разработчикам.';
										|en = 'Cannot determine inventory owner attributes when determining goods transfer sources. Contact developers.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
		Иначе
...
Функция ТекстЗапросаОтправителиПоНастройкамПередачи(Запрос, ИмяВТ = "") Экспорт
	...
	Если Запрос.Параметры.ОформлятьВыкупы Тогда
		...
		|	ВЫБОР
		//++ НЕ УТКА
		|		КОГДА ВложенныйЗапрос.ТипЗапасов В (
		|									ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.МатериалДавальца),
		|									ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца),
		|									ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца))
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыкупТоваровДавальца)
		...
		//++ НЕ УТКА
		|						КОГДА &ФормируемыеДокументы = ""ТолькоВыкупыТоваровНаХранении""
		|							ТОГДА ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)
		|						КОГДА &ФормируемыеДокументы = ""ТолькоВыкупыТоваровДавальца""
...
Функция ХозяйственныеОперацииВыкупа() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья2_5") Тогда
		Массив.Добавить(Перечисления.ХозяйственныеОперации.ВыкупТоваровДавальца);
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура ПометитьНаУдалениеСозданныеДокументы(Параметры, АдресВременногоХранилища) Экспорт
	...
	Пока Индекс >0 Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации( НСтр("ru = 'Пакетное формирование документов';
											|en = 'Package document generation'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ПолучитьТаблицуПоНастройкамПередачи(ПараметрыЗапроса) Экспорт
	...
	//++ НЕ УТКА
	Запрос.УстановитьПараметр("ФормируемыеДокументы", ПараметрыЗапроса.ФормируемыеДокументы);
	//-- НЕ УТКА
...
Функция ЗаголовокИФормируемыеДокументыПоКлючуНазначенияИспользования(КлючНазначенияИспользования) Экспорт
	...
	Если (ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранениеВПроцессеЗакупки")
		...
		ИначеЕсли КлючНазначенияИспользования =
			...
			ФормируемыеДокументы = "ТолькоВыкупыТоваровДавальца";
		//-- НЕ УТКА
		...
		Заголовок = НСтр("ru = 'Помощник формирования передач и выкупов';
						|en = 'Intercompany documents generation wizard'");
	...
	ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранениеВПроцессеЗакупки")
		...
		Если Не ЗначениеЗаполнено(КлючНазначенияИспользования)
			...
			Заголовок = НСтр("ru = 'Помощник формирования выкупов';
							|en = 'Redemption generation wizard'");
