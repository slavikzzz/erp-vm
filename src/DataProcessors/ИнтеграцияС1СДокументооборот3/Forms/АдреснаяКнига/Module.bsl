...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользоватьПравилаКоммуникаций = ЗначениеЗаполнено(Параметры.КонтекстПравилКоммуникаций)
		И ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.15.41");
...
Процедура ЗаполнитьДеревоПодразделений(ВеткаДерева, ТипОбъектаВыбора, ИдентификаторПапки = Неопределено,
	...
	Если ИспользоватьПравилаКоммуникаций Тогда
		ИнтеграцияС1СДокументооборот3.ДополнитьОтборКонтекстомПравилКоммуникаций(
			Прокси,
			Параметры.КонтекстПравилКоммуникаций,
			СписокУсловийОтбора);
	КонецЕсли;
	...
	Для Каждого Элемент Из Результат.Items Цикл
		...
		Если ИспользоватьПравилаКоммуникаций Тогда
			НоваяСтрока.НедоступенПоПравиламКоммуникаций = Элемент.object.isNotAvailableAccordingCommunicationRules;
		КонецЕсли;
...
Процедура ЗаполнитьТаблицуПользователей(ПодразделениеИД)
	...
	Для Каждого ПользовательВСписке Из Результат.Items Цикл
		...
		Если ИспользоватьПравилаКоммуникаций Тогда
			НоваяСтрока.НедоступенПоПравиламКоммуникаций = ПользовательВСписке.object.isNotAvailableAccordingCommunicationRules;
		КонецЕсли;
	КонецЦикла;
...
Процедура ВыбратьВыполнить()
	...
	Если Элементы.ГруппаПользователиРоли.ТекущаяСтраница = Элементы.ГруппаПользователи Тогда
		...
		Если ЗначениеЗаполнено(Параметры.КонтекстПравилКоммуникаций)
				И Элементы.ТаблицаПользователей.ТекущиеДанные.НедоступенПоПравиламКоммуникаций Тогда
			ИнтеграцияС1СДокументооборот3Клиент.ПоказатьНарушениеПравилКоммуникаций();
			Возврат;
		КонецЕсли;
		
		ДанныеВозврата = Новый Структура;
		ДанныеВозврата.Вставить("Исполнитель", Элементы.ТаблицаПользователей.ТекущиеДанные.Пользователь);
		ДанныеВозврата.Вставить("ИсполнительID", Элементы.ТаблицаПользователей.ТекущиеДанные.ПользовательID);
		ДанныеВозврата.Вставить("ИсполнительТип", Элементы.ТаблицаПользователей.ТекущиеДанные.ПользовательТип);
	...
	ИначеЕсли Элементы.ГруппаПользователиРоли.ТекущаяСтраница = Элементы.ГруппаРоли Тогда
		...
		Если ЗначениеЗаполнено(Параметры.КонтекстПравилКоммуникаций)
				И Элементы.Роли.ТекущиеДанные.НедоступенПоПравиламКоммуникаций Тогда
			ИнтеграцияС1СДокументооборот3Клиент.ПоказатьНарушениеПравилКоммуникаций();
			Возврат;
		КонецЕсли;
