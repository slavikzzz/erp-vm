...
Процедура СформироватьОтчет()
	...
	Если Не ЗначениеЗаполнено(ВыборкаШапка) Тогда
		ВывестиОшибку(НСтр("ru = 'Данные документа недоступны';
							|en = 'Document data is not available'"), Макет);
		Возврат;
	КонецЕсли;
	
	Если ВыборкаШапка.ЕстьОшибкиПроведение Тогда
		ВывестиОшибку(НСтр("ru = 'Документ не проведен. Оформление документов по результатам приемки невозможно.';
							|en = 'Document is not posted. Cannot register documents according to the receiving results.'"), Макет);
		Возврат;
	КонецЕсли;
	
	Если ВыборкаШапка.ЕстьОшибкиСтатус Тогда
		ВывестиОшибку(НСтр("ru = 'Для оформления документов по результатам расхождений необходимо перевести документ в статус ""Отрабатывается"" или ""Отработано"".';
							|en = 'To register documents based on the result of differences, change the document status to ""Being processed"" or ""Processed"".'"), Макет);
		Возврат;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(СпособОтраженияРасхождений) Тогда 
		...
		ВывестиОшибку(НСтр("ru = 'В акте приемки не задан способ отражения расхождений';
							|en = 'Discrepancy record method is not specified in the receiving note'"), Макет);
		Возврат;
	...
	ИначеЕсли ТипЗнч(АктОРасхождениях) = Тип("ДокументСсылка.АктОРасхожденияхПослеПриемки") Тогда
		...
		ИначеЕсли ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПриемкаТоваровНаХранение
			...
			//++ НЕ УТКА
			ЧтениеЗаказПоставщику =
				ЧтениеЗаказПоставщику
				И ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПриемкаТоваровНаХранение;
			
			ЧтениеЗаказДавальца2_5 =
				ПравоДоступа("Чтение", Метаданные.Документы.ЗаказДавальца2_5)
				И ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПоступлениеОтДавальца;
			//-- НЕ УТКА
			...
			//++ НЕ УТКА
			ПраваДоступаНаТаблицы.Вставить("ЧтениеЗаказДавальца2_5", ЧтениеЗаказДавальца2_5);
			//-- НЕ УТКА
	...
	Если РезультатЗапроса[1].Пустой() Тогда
		ВывестиОшибку(НСтр("ru = 'По результатам приемки не выявлено расхождений, требующих оформления документов';
							|en = 'According to the receiving results, discrepancies requiring document registration are not detected'"), Макет);
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаДействия Из Действия Цикл
		...
		ИначеЕсли ТипЗнч(АктОРасхождениях) = Тип("ДокументСсылка.АктОРасхожденияхПослеПриемки") Тогда
			...
			//++ НЕ УТКА
			Если ХозяйственнаяОперацияАкта = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
				ХозяйственнаяОперация = ХозяйственнаяОперацияАкта;
			КонецЕсли;
			//-- НЕ УТКА
	...
	Если ТабличныйДокумент.ВысотаТаблицы <= 2 Тогда
		ТекстОшибки = НСтр("ru = 'Вам недоступно для оформления ни одного документа.';
							|en = 'Cannot register any documents.'");
		Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
			ТекстОшибки = ТекстОшибки + " " + НСтр("ru = 'Возможно нет необходимых прав.';
													|en = 'You may not have the required rights.'");
		КонецЕсли;
	...
	Если ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПриобретениеТоваровУслуг Тогда
		...
		Если СтрокаДействия.ВариантыДействий.НайтиПоЗначению(Перечисления.ВариантыДействийПоРасхождениямВАктеПослеПриемки.ОтнестиНедостачуНаПрочиеРасходы) <> Неопределено
			...
			Иначе
				...
				ВывестиОшибку(НСтр("ru = 'Документ ""Корректировка задолженности"" недоступен для оформления. Возможно нет необходимых прав.';
									|en = 'Cannot register the AR/AP adjustment document. You may not have the required rights.'"), Макет);
			КонецЕсли;
		...
		Если СтрокаДействия.ВариантыДействий.НайтиПоЗначению(Перечисления.ВариантыДействийПоРасхождениямВАктеПослеПриемки.ОформитьНедостачу) <> Неопределено Тогда
			...
			Если СтрокаДействия.ПоЗаказам
				...
				И (ПраваДоступаНаТаблицы.ЧтениеЗаказПоставщику
					//++ НЕ УТКА
					Или ПраваДоступаНаТаблицы.ЧтениеЗаказДавальца2_5
					//-- НЕ УТКА
					Или Ложь) Тогда
		...
		Если Описание.ИмяКоманды = "Изменить" Тогда
			...
			ИначеЕсли Результат.СостояниеДокумента = "ИзмененияНеТребуются" Тогда
				...
				ТекстГиперссылки = НСтр("ru = 'Изменения не требуются';
										|en = 'No modifications are required'");
			...
			ИначеЕсли Результат.СостояниеДокумента = "ОснованиеНеСоответствуетАкту" Тогда
				...
				ТекстГиперссылки = НСтр("ru = 'Основание не соответствует акту';
										|en = 'Base document does not match the certificate'");
			КонецЕсли;
		...
		ИначеЕсли Описание.ИмяКоманды = "Подтвердить" Тогда	
			...
			Если Описание.СтатусЭДО = "Получен" Тогда
				...
				Иначе
					...
					ТекстГиперссылки = НСтр("ru = 'Вам недоступно подтверждение электронного документа.';
											|en = 'You cannot confirm an electronic document.'");
			...
			ИначеЕсли Описание.СтатусЭДО = "ОтклонениеВОбработке" Тогда
				...
				ТекстГиперссылки = НСтр("ru = 'Инициировано отклонение электронного документа, что не соответствует выбранному способу отработки расхождений';
										|en = 'The electronic document rejection is initiated, which does not match the selected method of processing discrepancies'");
			...
			ИначеЕсли Описание.СтатусЭДО = "Отклонен" Тогда
				...
				ТекстГиперссылки = НСтр("ru = 'Электронный документ отклонен, что не соответствует выбранному способу отработки расхождений';
										|en = 'The electronic document is rejected, which does not match the selected method of processing discrepancies'");
		...
		ИначеЕсли Описание.ИмяКоманды = "Отклонить" Тогда
			...
			Если Описание.СтатусЭДО = "Получен" Тогда
				...
				Иначе
					...
					ТекстГиперссылки = НСтр("ru = 'Вам недоступно отклонение электронного документа.';
											|en = 'You cannot reject an electronic document.'");
			...
			ИначеЕсли Описание.СтатусЭДО = "УтверждениеВОбработке" Тогда
				...
				ТекстГиперссылки = НСтр("ru = 'Инициировано подтверждение электронного документа, что не соответствует выбранному способу отработки расхождений.';
										|en = 'The electronic document confirmation is initiated, which does not match the selected method of processing discrepancies.'");
				
			ИначеЕсли Описание.СтатусЭДО = "Утвержден" Тогда
				
				ЦветНадписи = ЦветаСтиля.ПоясняющийОшибкуТекст;
				ТекстГиперссылки = НСтр("ru = 'Электронный документ подтвержден, что не соответствует выбранному способу отработки расхождений';
										|en = 'The electronic document is confirmed, which does not match the selected method of processing discrepancies'");
				
			ИначеЕсли Описание.СтатусЭДО = "ОтклонениеВОбработке" Тогда
				
				ЦветНадписи = ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет;
				ТекстГиперссылки = НСтр("ru = 'Инициировано отклонение электронного документа.';
										|en = 'The electronic document rejection is initiated.'");
				
			ИначеЕсли Описание.СтатусЭДО = "Отклонен" Тогда
				
				ЦветНадписи = ЦветаСтиля.РезультатУспехЦвет;
				ТекстГиперссылки = НСтр("ru = 'Электронный документ отклонен.';
										|en = 'Electronic document rejected.'");
...
Функция ТекстШапки(ТипДокумента, Описание , ХозяйственнаяОперация = Неопределено)
	...
	Если ТипДокумента = "КорректировкаРеализации" Тогда
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ИсправлениеОшибок Тогда
			ТекстШапки = НСтр("ru = 'Документ ""Корректировка реализации (исправление ошибок)""';
								|en = 'Document ""Sales adjustment (error correction)""'");
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон Тогда
			ТекстШапки = НСтр("ru = 'Документ ""Корректировка реализации (по согласованию сторон)""';
								|en = 'Document ""Sales adjustment (as agreed by the parties)""'");
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияПерепоставленногоТовара Тогда
			ТекстШапки = НСтр("ru = 'Документ ""Корректировка реализации (реализация перепоставленного товара)""';
								|en = 'Document ""Sales adjustment (overdelivered goods sales)""'");
	...
	ИначеЕсли ТипДокумента = "ИсправлениеРеализации" Тогда
		ТекстШапки = НСтр("ru = 'Отразить расхождения в документе ""Реализация товаров и услуг""';
							|en = 'Record discrepancies in document ""Customer invoice""'");
	ИначеЕсли ТипДокумента = "ИсправлениеВозвратаТоваровПоставщику" Тогда
		ТекстШапки = НСтр("ru = 'Отразить расхождения в документе ""Возврат товаров поставщику""';
							|en = 'Record discrepancies in document ""Return of goods to vendor""'");
	ИначеЕсли ТипДокумента = "ОтгрузкаТоваровСХранения" Тогда
		...
		КонецЕсли;
		//-- НЕ УТКА
		ТекстШапки = СтрШаблон(НСтр("ru = 'Документ ""%1"" для довозврата недовозращенного товара';
									|en = 'The ""%1"" document for additional return of unreturned goods'"), ПредставлениеОбъекта);
	ИначеЕсли ТипДокумента = "ОтгрузкаТоваровСХраненияПоПриемке" Тогда
		...
		КонецЕсли;
		//-- НЕ УТКА
		ТекстШапки = СтрШаблон(НСтр("ru = 'Документ ""%1"" для возврата перепоставленного товара';
									|en = 'The ""%1"" document to return overdelivered goods'"), ПредставлениеОбъекта);
	ИначеЕсли ТипДокумента = "ИсправлениеОтгрузкаТоваровСХранения" Тогда
		...
		КонецЕсли;
		//-- НЕ УТКА
		ТекстШапки = СтрШаблон(НСтр("ru = 'Отразить расхождения в документе ""%1""';
									|en = 'Register discrepancies in document ""%1""'"), ПредставлениеОбъекта);
	ИначеЕсли ТипДокумента = "ИсправлениеПередачи" Тогда
		...
		ТекстШапки = СтрШаблон(НСтр("ru = 'Отразить расхождения в документе ""%1""';
									|en = 'Register discrepancies in document ""%1""'"), Обработчик.ПредставлениеОбъекта());
	ИначеЕсли ТипДокумента = "ИсправлениеПередачиНаКомиссию" Тогда
		ТекстШапки = НСтр("ru = 'Отразить расхождения в документе ""Передача на комиссию""';
							|en = 'Record discrepancies in the ""Consignment sales"" document'");
	ИначеЕсли ТипДокумента = "ПередачаТоваровХранителю" Тогда
		Обработчик = Документы.ПередачаТоваровХранителю.ОбработчикДействий(ХозяйственнаяОперацияАкта);
		ТекстШапки = СтрШаблон(НСтр("ru = 'Документ ""%1"" для передачи недопоставленного товара';
									|en = 'Document ""%1"" for underdelivered goods transfer'"), Обработчик.ПредставлениеОбъекта());
	ИначеЕсли ТипДокумента = "ПередачаТоваровНаКомиссию" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Передача товаров на комиссию"" для передачи недопоставленного товара';
							|en = 'The ""Customer stock fill-up documents"" document for underdelivered goods transfer'");
	ИначеЕсли ТипДокумента = "ПриемкаТоваровНаХранениеДопоставкаПоПриемке" Тогда
		//++ НЕ УТКА
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
			ПредставлениеОбъекта = ДавальческаяСхемаКлиентСервер.СинонимДокументаПриемкаТоваровНаХранение();
		Иначе
		//-- НЕ УТКА
		...
		КонецЕсли;
		//-- НЕ УТКА
		ТекстШапки = СтрШаблон(НСтр("ru = 'Документ ""%1"" для поступления недопоставленного товара';
									|en = 'The ""%1"" document to receive undelivered goods'"), ПредставлениеОбъекта);
	ИначеЕсли ТипДокумента = "ПриемкаТоваровНаХранение" Тогда
		...
		КонецЕсли;
		//-- НЕ УТКА
		ТекстШапки = СтрШаблон(НСтр("ru = 'Документ ""%1"" для поступления перепоставленного товара';
									|en = 'The ""%1"" document to receive overdelivered goods'"), ПредставлениеОбъекта);
	...
	ИначеЕсли ТипДокумента = "ИсправлениеВозвратаОтКомиссионера" Тогда
		ТекстШапки = НСтр("ru = 'Отразить расхождения в документе ""Поступление товаров от комиссионера""';
							|en = 'Record discrepancies in the ""Receipt of goods from consignee"" document'");
	ИначеЕсли ТипДокумента = "ВозвратТоваровОтХранителяДовозврат" Тогда
		Обработчик = Документы.ПоступлениеТоваровОтХранителя.ОбработчикДействий(ХозяйственнаяОперацияАкта);
		ТекстШапки = СтрШаблон(НСтр("ru = 'Документ ""%1"" по недовозвращенным товарам';
									|en = 'Document ""%1"" by unreturned goods'"), Обработчик.ПредставлениеОбъекта());
	ИначеЕсли ТипДокумента = "ВозвратТоваровОтКомиссионераДовозврат" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Поступление товаров от комиссионера"" по недовозвращенным товарам';
							|en = 'The ""Receipt of goods from consignee"" document by unreturned goods'");
	ИначеЕсли ТипДокумента = "ВозвратТоваровОтХранителяВозвратХранителю" Тогда
		...
		КонецЕсли;
		ТекстШапки = СтрШаблон(НСтр("ru = 'Документ ""%1"" по излишне возвращенным товарам';
									|en = 'The ""%1"" document of goods returned in excess'"), ПредставлениеДокумента);
	ИначеЕсли ТипДокумента = "ВозвратТоваровОтКомиссионераВозвратКомиссионеру" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Передача товаров комиссионеру"" по излишне возвращенным товарам';
							|en = 'The ""Goods transfer to consignee"" document by goods returned in excess'");
	ИначеЕсли ТипДокумента = "ИсправлениеВозвратаТоваровОтКлиента" Тогда
		ТекстШапки = НСтр("ru = 'Отразить расхождения в документе ""Возврат товаров от клиента""';
							|en = 'Record discrepancies in document ""Sales return""'");
	ИначеЕсли ТипДокумента = "ИсправлениеПеремещенияТоваров" Тогда
		ТекстШапки = НСтр("ru = 'Отразить расхождения в документе ""Перемещение товаров""';
							|en = 'Record discrepancies in document ""Goods transfer""'");
	ИначеЕсли ТипДокумента = "КорректировкаПеремещенияТоваров" Тогда
		ТекстШапки = НСтр("ru = 'Ввести корректировку документа ""Перемещение товаров""';
							|en = 'Enter ""Inventory transfer"" document adjustment'");
	ИначеЕсли ТипДокумента = "ДокументВозврата" Тогда
		Если ИспользоватьЗаявкиНаВозврат Тогда
			ТекстШапки = НСтр("ru = 'Документ ""Заявка на возврат товаров от клиента"" по перепоставленным товарам к возврату';
								|en = 'Document ""Sales return request"" for overdelivered goods for return'");
		Иначе
			ТекстШапки = НСтр("ru = 'Документ ""Возврат товаров от клиента"" по перепоставленным товарам к возврату';
								|en = 'Document ""Sales return"" for overdelivered goods for return'");
		КонецЕсли;
	ИначеЕсли ТипДокумента = "ДокументВозвратаПередачи" Тогда
		Если ИспользоватьЗаявкиНаВозврат
		   //++ НЕ УТ
		   И ХозяйственнаяОперацияАкта <> Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5
		   //-- НЕ УТ
		   Тогда
			ТекстШапки = НСтр("ru = 'Документ ""Заявка на возврат товаров от клиента"" по перепоставленным товарам к возврату';
								|en = 'Document ""Sales return request"" for overdelivered goods for return'");
		//++ НЕ УТ
		ИначеЕсли ХозяйственнаяОперацияАкта = Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5 Тогда
			ТекстШапки = НСтр("ru = 'Документ ""Поступление от переработчика"" по перепоставленным товарам к возврату';
								|en = 'The ""Goods receipt — External subcontracting"" document for overdelivered goods for return'");
		//-- НЕ УТ
		Иначе
			ТекстШапки = НСтр("ru = 'Документ ""Поступление товаров от хранителя"" по перепоставленным товарам к возврату';
								|en = 'The ""VMI return"" document by overdelivered goods for return'");
		КонецЕсли;
	ИначеЕсли ТипДокумента = "ДокументВозвратаКомиссионера" Тогда
		Если ИспользоватьЗаявкиНаВозврат Тогда
			ТекстШапки = НСтр("ru = 'Документ ""Заявка на возврат товаров от комиссионера"" по перепоставленным товарам к возврату';
								|en = 'The ""Consignment pick-up request"" document for overdelivered goods for return'");
		Иначе
			ТекстШапки = НСтр("ru = 'Документ ""Поступление товаров от комиссионера"" по перепоставленным товарам к возврату';
								|en = 'The ""Receipt of goods from consignee"" document by overdelivered goods for return'");
		КонецЕсли;
	ИначеЕсли ТипДокумента = "ВозвратТоваровОтКлиентаДовозврат" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Возврат товаров от клиента"" по недовозвращенным товарам';
							|en = 'Document ""Sales return"" for unreturned goods'");
	ИначеЕсли ТипДокумента = "ЗаказКлиента" Тогда
		ТекстШапки = НСтр("ru = 'Отменить строки документов ""Заказ клиента"" по недопоставленным товарам';
							|en = 'Cancel lines of documents ""Sales order"" for underdelivered goods'");
	ИначеЕсли ТипДокумента = "ЗаказПоставщику" Тогда
		ТекстШапки = НСтр("ru = 'Отменить строки документов ""Заказ поставщику"" по недопоставленным товарам';
							|en = 'Cancel lines of documents ""Purchase order"" for underdelivered goods'");
	//++ НЕ УТКА
	ИначеЕсли ТипДокумента = "ЗаказДавальца2_5" Тогда
		ТекстШапки = НСтр("ru = 'Отменить строки документов ""Заказ давальца"" по недопоставленным товарам';
							|en = 'Cancel lines of the ""Subcontracting sales order"" documents by undelivered goods'");
	//-- НЕ УТКА
	ИначеЕсли ТипДокумента = "ЗаказНаПеремещение" Тогда
		ТекстШапки = НСтр("ru = 'Отменить строки документов ""Заказ на перемещение"" по недопоставленным товарам';
							|en = 'Cancel lines of documents ""Transfer order"" for underdelivered goods'");
	ИначеЕсли ТипДокумента = "ЗаказПередачи" Тогда
		Обработчик = Документы.ПередачаТоваровХранителю.ОбработчикДействий(ХозяйственнаяОперацияАкта);	
		ТекстШапки = СтрШаблон(НСтр("ru = 'Отменить строки документов ""%1"" по недопоставленным товарам';
									|en = 'Cancel lines of documents ""%1"" for underdelivered goods'"), Обработчик.СинонимЗаказа());
	ИначеЕсли ТипДокумента = "ИсправлениеЗаявкаНаВозвратТоваров" Тогда
		ТекстШапки = НСтр("ru = 'Отменить строки документов ""Заявка на возврат товаров от клиента"" по недостачам при возврате';
							|en = 'Cancel lines of documents ""Sales return request"" by the shortages on return'");
	ИначеЕсли ТипДокумента = "ИсправлениеЗаявкаНаВозвратТоваровПередачи" Тогда
		Обработчик = Документы.ПоступлениеТоваровОтХранителя.ОбработчикДействий(ХозяйственнаяОперацияАкта);
		ТекстШапки = СтрШаблон(НСтр("ru = 'Отменить строки документов ""%1"" по недостачам при возврате';
									|en = 'Cancel the ""%1"" document lines by shortage on return'"), Обработчик.СинонимЗаказа());
	ИначеЕсли ТипДокумента = "ИсправлениеЗаявкаНаВозвратОтКомиссионера" Тогда
		ТекстШапки = НСтр("ru = 'Отменить строки документов ""Заявка на возврат товаров от комиссионера"" по недостачам при возврате';
							|en = 'Cancel lines of the ""Consignment pick-up request"" documents by shortages upon return'");
	ИначеЕсли ТипДокумента = "РеализацияТоваровИУслуг" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Реализация товаров и услуг"" для реализации недопоставленного товара';
							|en = 'Document ""Customer invoice"" for sales of underdelivered goods'");
	ИначеЕсли ТипДокумента = "РеализацияТоваровИУслугПоВозвратуОтКлиента" Тогда 
		ТекстШапки = НСтр("ru = 'Документ ""Реализация товаров и услуг"" для реализации излишков возврата от клиента';
							|en = 'Document ""Customer invoice"" for sales of surpluses of return from customer'");
	ИначеЕсли ТипДокумента = "ВозвратТоваровПоставщику" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Возврат товаров поставщику"" для довозврата недовозращенного товара';
							|en = 'Document ""Return of goods to vendor"" for additional return of unreturned goods'");
	ИначеЕсли ТипДокумента = "ПриобретениеТоваровУслуг" Тогда 
		ТекстШапки = НСтр("ru = 'Документ ""Приобретение товаров и услуг"" для поступления перепоставленного товара';
							|en = 'The ""Vendor invoice"" document for overdelivered goods receipt'");
	ИначеЕсли ТипДокумента = "ПриобретениеТоваровУслугДопоставкаПоПоступлению" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Приобретение товаров и услуг"" для поступления недопоставленного товара';
							|en = 'The ""Vendor invoice"" document for underdelivered goods receipt'");
	ИначеЕсли ТипДокумента = "ПоступлениеТоваровДопоставкаПоПоступлению" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Поступление товаров"" для поступления недопоставленного товара';
							|en = 'The ""Goods pick-up from customer"" for receiving the underdelivered goods'");
	ИначеЕсли ТипДокумента = "ПеремещениеТоваров" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Перемещение товаров"" на допоставку недопоставленного товара';
							|en = 'Document ""Goods transfer"" for additional delivery of underdelivered goods'");
	ИначеЕсли ТипДокумента = "ПеремещениеТоваровВозврат" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Перемещение товаров"" на возврат перепоставленного товара';
							|en = 'Document ""Goods transfer"" for return of overdelivered goods'");
	ИначеЕсли ТипДокумента = "ПоступлениеТоваровПерепроведениеПослеКорректировки" Тогда
		ТекстШапки = НСтр("ru = 'Требуется перепровести документ ""Поступление товаров"" для отражения изменений имеющихся после оформления документа ""Корректировка приобретения""';
							|en = 'Repost the ""Receipt of goods"" document to record the changes that occurred after registering the ""Purchase adjustment"" document'");
	ИначеЕсли ТипДокумента = "ПоступлениеТоваровПерепроведениеПослеПриобретения" Тогда
		ТекстШапки = НСтр("ru = 'Требуется перепровести документ ""Поступление товаров"" для отражения изменений внесенных в документ ""Приобретение товаров""';
							|en = 'Repost the ""Receipt of goods"" document to record the changes made in the ""Purchase of goods"" document'");
	ИначеЕсли ТипДокумента = "СписаниеНедостачТоваров" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Списание недостач товаров"" для списания непризнанных недостач';
							|en = 'The ""Stock shortage recognition"" document to write off unrecognized shortage'");
	ИначеЕсли ТипДокумента = "КорректировкаЗадолженности" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Корректировка задолженности"" для отражения задолженности сторонней компании';
							|en = 'Document ""AR/AP adjustment"" for recording a third party debt'");
	ИначеЕсли ТипДокумента = "ОприходованиеИзлишковТоваров" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Оприходование излишков товаров"" для оприходования излишков поставленных товаров';
							|en = 'Document ""Stock surplus recognition"" for recording as received of delivered goods surpluses'");
	ИначеЕсли ТипДокумента = "ИсправлениеПоступления" Тогда
		ТекстШапки = НСтр("ru = 'Отразить расхождения в документе ""Приобретение товаров и услуг""';
							|en = 'Record discrepancies in the ""Vendor invoice"" document'");
	...
	ИначеЕсли ТипДокумента = "КорректировкаПоступленияТоваровИсправления" Тогда
		ТекстШапки = НСтр("ru = 'Ввести корректировку документа ""Поступление товаров на склад""';
							|en = 'Enter ""Warehouse receipt"" document adjustment'");
	ИначеЕсли ТипДокумента = "СписаниеРасхожденийПоступлениеПриобретение" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Списание расхождений между поступлением и приобретением""';
							|en = 'Document: Write off discrepancies between receipt and purchase'");
	...
	ИначеЕсли ТипДокумента = "ВозвратТоваровПоставщикуПоПоступлению" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Возврат товаров поставщику"" для возврата перепоставленного товара';
							|en = 'Document ""Return of goods to vendor"" for return of overdelivered goods'");
	ИначеЕсли ТипДокумента = "Претензия" Тогда
		ТекстШапки = НСтр("ru = 'Претензия поставщику';
							|en = 'Complaint to vendor'");
	КонецЕсли;
	...
	Если ТипДокумента = "СписаниеНедостачТоваров"
		...
		Если Описание.ВыводитьСклад Тогда
			...
			СтрокаСклад = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для склада ""%1"".';
																						|en = 'for warehouse ""%1"".'"), Описание.Склад);
...
Функция ТекстЛегенды(ТипДокумента, ХозяйственнаяОперация = Неопределено)
	...
	Если ТипДокумента = "КорректировкаРеализации" Тогда
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ИсправлениеОшибок Тогда
			Возврат НСтр("ru = 'Необходимо оформить документ ""Корректировка реализации (исправление ошибок)"" для отражения излишков и недостач. Документ является основанием для выставления исправленных первичных документов и исправительного счета-фактуры.';
						|en = 'Register document ""Sales adjustment (error correction)"" for recording surpluses and shortages. This is the base document for issuing corrected source documents and a replacement tax invoice.'");
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон Тогда
			Возврат НСтр("ru = 'Необходимо оформить документ ""Корректировка реализации (по согласованию сторон)"" для отражения излишков и недостач. Документ является основанием для выставления исправленных первичных документов и корректировочного счета-фактуры.';
						|en = 'Register document ""Sales adjustment (as agreed by the parties)"" for recording surpluses and shortages. The document is a basis for issuing corrected source documents and an adjustment tax invoice.'");
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияПерепоставленногоТовара Тогда
			Возврат НСтр("ru = 'Необходимо оформить документ ""Корректировка реализации (реализация перепоставленного товара)"" для отражения в учете факта продажи излишне доставленных товаров. Документ является основанием для выставления первичных документов и счета-фактуры. Оформляется на строки акта приемки с вариантами отработки: ""Оформить"" и ""Оформить и вернуть"".';
						|en = 'Register document ""Sales adjustment (return of overdelivered goods)"" for recording sales of overdelivered goods. This is the base document for issuing source documents and tax invoice. It is registered for receiving note lines with processing options: ""Invoice the surplus"" and ""Invoice the surplus and create Goods return"".'");
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратНедопоставленногоТовара Тогда
			Возврат НСтр("ru = 'Необходимо зарегистрировать документ ""Корректировка реализации (возврат недопоставленного товара)"". Выполняется сторнирование в учете факта продажи товаров, которые были отражены при реализации, но не были доставлены. Документ регистрируется на основании первичных документов и счета-фактуры, полученных от клиента. Оформляется на строки акта о расхождениях с вариантами отработки: ""Оформить"" и ""Оформить и допоставить"".';
						|en = 'Register the ""Sales adjustment (return of underdelivered goods)"" document. Reversal is carried out in accounting of sale of goods which were recorded on sale, but were not delivered. The document is registered on the basis of source documents and the tax invoice received from the customer. It is registered for discrepancy report lines with processing options: ""Register"" and ""Register and supply additionally"".'");
		КонецЕсли;
	ИначеЕсли ТипДокумента = "КорректировкаПриобретения" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Корректировка приобретения"" для отражения излишков и недостач. Документ является основанием для регистрации исправленных первичных документов и исправительного счета-фактуры.';
					|en = 'Register the ""Purchase adjustment"" document to record surpluses and shortages. This is a base document to register corrected source documents and a replacement tax invoice.'");
	ИначеЕсли ТипДокумента = "ИсправлениеРеализации" Тогда
		Возврат НСтр("ru = 'Необходимо скорректировать документ ""Реализация товаров и услуг"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Оформить и вернуть"", ""Оформить"", ""Оформить и допоставить"".';
					|en = 'Correct document ""Customer invoice"" by recording an actual quantity in it. Make the changes to discrepancy report lines with processing options: ""Register and return"", ""Register"", ""Register and supply additionally"".'");
	ИначеЕсли ТипДокумента = "ИсправлениеВозвратаТоваровПоставщику" Тогда
		Возврат НСтр("ru = 'Необходимо скорректировать документ ""Возврат товаров поставщику"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Оформить и вернуть"", ""Оформить"", ""Оформить и допоставить"".';
					|en = 'Correct document ""Return of goods to vendor"" recording an actual quantity in it. Execute for discrepancy report lines with processing options: ""Register and return"", ""Register"", ""Register and supply additionally"".'");
	ИначеЕсли ТипДокумента = "ИсправлениеВозвратаТоваровОтКлиента" Тогда
		Возврат НСтр("ru = 'Необходимо скорректировать документ ""Возврат товаров поставщику"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Оформить и вернуть"", ""Оформить"", ""Оформить и допоставить"".';
					|en = 'Correct document ""Return of goods to vendor"" recording an actual quantity in it. Execute for discrepancy report lines with processing options: ""Register and return"", ""Register"", ""Register and supply additionally"".'");
	ИначеЕсли ТипДокумента = "ИсправлениеПеремещенияТоваров" Тогда
		Возврат НСтр("ru = 'Необходимо скорректировать документ ""Перемещение товаров"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Оформить и вернуть"", ""Оформить"", ""Оформить и допоставить"".';
					|en = 'Correct document ""Goods transfer"" by recording an actual quantity in it. Make the changes to the discrepancy report lines with tolling options: ""Register and return"", ""Register"", ""Register and supply additionally"".'");
	ИначеЕсли ТипДокумента = "КорректировкаПеремещенияТоваров" Тогда
		Возврат НСтр("ru = 'Необходимо ввести корректировочный документ ""Перемещение товаров"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Оформить и вернуть"", ""Оформить"", ""Оформить и допоставить"".';
					|en = 'Enter ""Inventory transfer"" correction document by recording an actual quantity in it. Make the changes to the discrepancy report lines with tolling options: ""Register and return"", ""Register"", ""Register and supply additionally"".'");
	ИначеЕсли ТипДокумента = "ДокументВозврата" Тогда
		Если ИспользоватьЗаявкиНаВозврат Тогда
			Возврат НСтр("ru = 'Необходимо оформить документ ""Заявка на возврат товаров от клиента"" по излишкам, отмеченным к возврату клиентом. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".';
						|en = 'Register document ""Sales return request"" for surpluses marked for return by the customer. It is registered in discrepancy report lines with processing option ""Register and return"".'");
		Иначе
			Возврат НСтр("ru = 'Необходимо оформить документ ""Возврат товаров от клиента"" по излишкам, отмеченным к возврату клиентом. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".';
						|en = 'Register document ""Sales return"" for surpluses marked for return by the customer. It is registered in discrepancy report lines with processing option ""Register and return"".'");
		КонецЕсли;
	ИначеЕсли ТипДокумента = "ДокументВозвратаПередачи" Тогда
		Если ИспользоватьЗаявкиНаВозврат
		   //++ НЕ УТ
		...
		//++ НЕ УТ
		ИначеЕсли ХозяйственнаяОперацияАкта = Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5 Тогда
			Возврат НСтр("ru = 'Необходимо оформить документ ""Поступление от переработчика"" по излишкам, отмеченным к возврату переработчиком. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".';
						|en = 'Register the ""Goods receipt — External subcontracting"" document for surplus marked for return by the subcontractor. It is registered for discrepancy report lines with the ""Register and return"" processing option.'");
		//-- НЕ УТ
		Иначе
			Возврат НСтр("ru = 'Необходимо оформить документ ""Поступление товаров от хранителя"" по излишкам, отмеченным к возврату клиентом. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".';
						|en = 'Register the ""VMI return"" document by surplus marked for return by the customer. It is registered in the discrepancy report lines with the ""Register and return"" processing option.'");
		КонецЕсли;
	ИначеЕсли ТипДокумента = "ДокументВозвратаКомиссионера" Тогда
		Если ИспользоватьЗаявкиНаВозврат Тогда
			Возврат НСтр("ru = 'Необходимо оформить документ ""Заявка на возврат товаров от комиссионера"" по излишкам, отмеченным к возврату клиентом. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".';
						|en = 'Register the ""Consignment pick-up request"" document for surplus marked for return by the customer. It is registered for discrepancy report lines with the ""Register and return"" processing option.'");
		Иначе
			Возврат НСтр("ru = 'Необходимо оформить документ ""Поступление товаров от комиссионера"" по излишкам, отмеченным к возврату клиентом. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".';
						|en = 'Register the ""Receipt of goods from consignee"" document for surplus marked for return by the customer. It is registered for discrepancy report lines with the ""Register and return"" processing option.'");
		КонецЕсли;
	ИначеЕсли ТипДокумента = "ВозвратТоваровОтКлиентаДовозврат" Тогда
			Возврат НСтр("ru = 'Необходимо оформить документ ""Возврат товаров от клиента"" по недостачам, отмеченным к довозврату клиентом. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и ожидать допоставку"".';
						|en = 'Register document ""Sales return"" by shortages selected by a customer for additional return. It is registered for discrepancy report lines with processing option ""Register and wait for subsequent delivery"".'");
	ИначеЕсли ТипДокумента = "ЗаказКлиента" Тогда
		Возврат НСтр("ru = 'Необходимо скорректировать документы ""Заказ клиента"", отменив строки с товарами, которые были отражены при реализации, но не были доставлены. Следует выполнить для строк акта о расхождениях с вариантом отработки ""Оформить"".';
					|en = 'Correct documents ""Sales order"" by canceling lines with goods which were recorded on sale, but were not delivered. Make the changes to discrepancy report lines with processing option ""Register"".'");
	ИначеЕсли ТипДокумента = "ЗаказПоставщику" Тогда
		Возврат НСтр("ru = 'Необходимо скорректировать документы ""Заказ поставщику"", отменив строки с товарами, которые были отражены при поступлении, но не были доставлены. Следует выполнить для строк акта о расхождениях с вариантом отработки ""Оформить"".';
					|en = 'Correct documents ""Purchase order"" by canceling lines with goods which were recorded on receipt, but were not delivered. Make these changes to the discrepancy report lines with processing option ""Register"".'");
	//++ НЕ УТКА
	ИначеЕсли ТипДокумента = "ЗаказДавальца2_5" Тогда
		Возврат НСтр("ru = 'Необходимо скорректировать документы ""Заказ давальца"", отменив строки с товарами, которые были отражены при поступлении, но не были доставлены. Следует выполнить для строк акта о расхождениях с вариантом отработки ""Оформить"".';
					|en = 'Correct the ""Subcontracting sales order"" documents by canceling lines with goods that were recorded at receipt but were not delivered. Make these changes to the discrepancy report lines with the ""Register"" option.'");
	//-- НЕ УТКА
	ИначеЕсли ТипДокумента = "ЗаказНаПеремещение" Тогда
		Возврат НСтр("ru = 'Необходимо скорректировать документы ""Заказ на перемещение"", отменив строки с товарами, которые были отражены при перемещении, но не были перемещены. Следует выполнить для строк акта о расхождениях с вариантом отработки ""Оформить"".';
					|en = 'Correct documents ""Transfer order"" canceling lines with goods which were recorded when moving, but were not transferred. Make the changes to discrepancy report lines with tolling option: ""Register"".'");
	ИначеЕсли ТипДокумента = "ЗаказПередачи" Тогда
		Обработчик = Документы.ПередачаТоваровХранителю.ОбработчикДействий(ХозяйственнаяОперацияАкта);
		Возврат СтрШаблон(НСтр("ru = 'Необходимо скорректировать документы ""%1"", отменив строки с товарами, которые были отражены при передачи, но не были доставлены. Следует выполнить для строк акта о расхождениях с вариантом отработки ""Оформить"".';
								|en = 'Correct documents ""%1"" by canceling lines with goods which were registered upon transfer but were not delivered. Make the changes to the discrepancy report lines with processing option ""Register"".'"), Обработчик.СинонимЗаказа());
	ИначеЕсли ТипДокумента = "ИсправлениеЗаявкаНаВозвратТоваров" Тогда
		Возврат НСтр("ru = 'Необходимо скорректировать документы ""Заявка на возврат товаров от клиента"", отменив строки с товарами, по которым были выявлены недостачи при возврате. Следует выполнить для строк акта о расхождениях с вариантом отработки ""Оформить"".';
					|en = 'Correct the ""Sales return request"" documents by canceling lines with goods for which shortages on return were detected. This is relevant to lines of discrepancy report with the ""Register"" processing option.'");
	ИначеЕсли ТипДокумента = "ИсправлениеЗаявкаНаВозвратТоваровПередачи" Тогда
		Обработчик = Документы.ПоступлениеТоваровОтХранителя.ОбработчикДействий(ХозяйственнаяОперацияАкта);
		Возврат СтрШаблон(НСтр("ru = 'Необходимо скорректировать документы ""%1"", отменив строки с товарами, по которым были выявлены недостачи при возврате. Следует выполнить для строк акта о расхождениях с вариантом отработки ""Оформить"".';
								|en = 'Correct the ""%1"" documents by canceling lines with goods for which shortages on return were detected. This is relevant to lines of discrepancy report with the ""Register"" processing option.'"), Обработчик.СинонимЗаказа());
	ИначеЕсли ТипДокумента = "ИсправлениеЗаявкаНаВозвратОтКомиссионера" Тогда
		Возврат НСтр("ru = 'Необходимо скорректировать документы ""Заявка на возврат товаров от комиссионера"", отменив строки с товарами, по которым были выявлены недостачи при возврате. Следует выполнить для строк акта о расхождениях с вариантом отработки ""Оформить"".';
					|en = 'Correct the ""Consignment pick-up request"" documents by canceling lines with goods for which shortages were detected upon return. This is relevant to lines of the discrepancy report with the ""Register"" processing option.'");
	ИначеЕсли ТипДокумента = "РеализацияТоваровИУслуг" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Реализация товаров и услуг"" по недостачам, отмеченным к допоставке клиенту. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и допоставить"".';
					|en = 'Register document ""Customer invoice"" for shortages marked for subsequent delivery to the customer. It is registered in discrepancy report lines with processing option ""Register and supply additionally"".'");
	ИначеЕсли ТипДокумента = "РеализацияТоваровИУслугПоВозвратуОтКлиента" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Реализация товаров и услуг"" по излишкам, отмеченным к возврату клиенту. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".';
					|en = 'Register document ""Customer invoice"" for surpluses marked for return to the customer. It is registered in discrepancy report lines with processing option ""Register and return"".'");
	ИначеЕсли ТипДокумента = "ВозвратТоваровПоставщику" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Возврат товаров поставщику"" по недостачам, отмеченным к допоставке поставщику. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и допоставить"".';
					|en = 'Register document ""Return of goods to vendor"" for shortages marked for additional delivery to the vendor. It is registered in discrepancy report lines with processing option ""Register and supply additionally"".'");
	ИначеЕсли ТипДокумента = "ПриобретениеТоваровУслуг" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Приобретение товаров и услуг"" по излишкам, отмеченным к возврату поставщиком. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".';
					|en = 'Register the ""Vendor invoice"" document for surpluses marked for return by the vendor. It is registered for discrepancy report lines with the ""Register and return"" processing option.'");
	ИначеЕсли ТипДокумента = "ОтгрузкаТоваровСХранения" Тогда
		//++ НЕ УТКА
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаДавальцу2_5 Тогда
			ПредставлениеОбъекта    =
				ДавальческаяСхемаКлиентСервер.СинонимДокументаОтгрузкаТоваровСХраненияПередачаДавальцу();
			ПредставлениеПоставщика = НСтр("ru = 'давальцу';
											|en = 'provider'");
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратДавальцу2_5 Тогда
			ПредставлениеОбъекта    =
				ДавальческаяСхемаКлиентСервер.СинонимДокументаОтгрузкаТоваровСХраненияВозвратДавальцу();
		...
		КонецЕсли;
		//-- НЕ УТКА
		Возврат СтрШаблон(НСтр("ru = 'Необходимо оформить документ ""%1"" по недостачам, отмеченным к допоставке %2. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и допоставить"".';
								|en = 'Register the ""%1"" document by the shortage marked for additional delivery %2. It is registered for discrepancy report lines with the ""Register and supply additionally"" option.'"), ПредставлениеОбъекта, ПредставлениеПоставщика);
	ИначеЕсли ТипДокумента = "ИсправлениеОтгрузкаТоваровСХранения" Тогда
		//++ НЕ УТКА
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаДавальцу2_5 Тогда
			ПредставлениеОбъекта =
		...
		КонецЕсли;
		//-- НЕ УТКА
		Возврат СтрШаблон(НСтр("ru = 'Необходимо скорректировать документ ""%1"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Оформить и вернуть"", ""Оформить"", ""Оформить и допоставить"".';
								|en = 'Correct document ""%1"" by recording an actual quantity in it. Make the changes to the discrepancy report lines with tolling options: ""Register and return"", ""Register"", ""Register and supply additionally"".'"), ПредставлениеОбъекта);
	ИначеЕсли ТипДокумента = "ОтгрузкаТоваровСХраненияПоПриемке" Тогда
		//++ НЕ УТКА
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
			ПредставлениеОбъекта    =
		...
		КонецЕсли;
		//-- НЕ УТКА
		Возврат СтрШаблон(НСтр("ru = 'Необходимо оформить документ ""%1"" по перепоставленному товару, отмеченному к возврату %2. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".';
								|en = 'Register the ""%1"" document for the overdelivered goods marked for return %2. It is registered for discrepancy report lines with the ""Register and return"" option.'"), ПредставлениеОбъекта, ПредставлениеПоставщика);
	ИначеЕсли ТипДокумента = "ИсправлениеПередачи" Тогда
		Обработчик = Документы.ПередачаТоваровХранителю.ОбработчикДействий(ХозяйственнаяОперацияАкта);
		Возврат СтрШаблон(НСтр("ru = 'Необходимо скорректировать документ ""%1"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Оформить и вернуть"", ""Оформить"", ""Оформить и допоставить"".';
								|en = 'Correct document ""%1"" by recording an actual quantity in it. Make the changes to the discrepancy report lines with tolling options: ""Register and return"", ""Register"", ""Register and supply additionally"".'"), Обработчик.ПредставлениеОбъекта());
	ИначеЕсли ТипДокумента = "ПередачаТоваровХранителю" Тогда
		Обработчик = Документы.ПередачаТоваровХранителю.ОбработчикДействий(ХозяйственнаяОперацияАкта);
		Возврат СтрШаблон(НСтр("ru = 'Необходимо оформить документ ""%1"" по недостачам, отмеченным к допоставке клиенту. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и допоставить"".';
								|en = 'Register document ""%1"" for shortages marked for subsequent delivery to the customer. It is registered in discrepancy report lines with processing option ""Register and supply additionally"".'"), Обработчик.ПредставлениеОбъекта());
	ИначеЕсли ТипДокумента = "ПриемкаТоваровНаХранение" Тогда
		//++ НЕ УТКА
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаДавальцу2_5
		 Или ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратДавальцу2_5 Тогда
		 	ПредставлениеОбъекта    = ДавальческаяСхемаКлиентСервер.СинонимДокументаПриемкаТоваровНаХранение();
		 	ПредставлениеПоставщика = НСтр("ru = 'давальцем';
											|en = 'provider'");
		Иначе
		//-- НЕ УТКА
			ПредставлениеОбъекта    = НСтр("ru = 'Приемка товаров на хранение';
		...
		КонецЕсли;
		//-- НЕ УТКА
		Возврат СтрШаблон(НСтр("ru = 'Необходимо оформить документ ""%1"" по излишкам, отмеченным к возврату %2. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".';
								|en = 'Register the ""%1"" document by the surplus marked for return %2. It is registered for discrepancy report lines with the ""Register and return"" option.'"), ПредставлениеОбъекта, ПредставлениеПоставщика);
	ИначеЕсли ТипДокумента = "ПриемкаТоваровНаХранениеДопоставкаПоПриемке" Тогда
		//++ НЕ УТКА
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
			ПредставлениеОбъекта = ДавальческаяСхемаКлиентСервер.СинонимДокументаПриемкаТоваровНаХранение();
		...
		КонецЕсли;
		//-- НЕ УТКА
		Возврат СтрШаблон(НСтр("ru = 'Необходимо оформить документ ""%1"" по недостачам, для которых требуется допоставка. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и ожидать допоставку"".';
								|en = 'Register the ""%1"" document by the shortage that requires additional delivery. It is registered for discrepancy report lines with the ""Register and wait for subsequent delivery"" option.'"), ПредставлениеОбъекта);
	ИначеЕсли ТипДокумента = "ИсправлениеВозвратаТоваровОтХранителя" Тогда
		Обработчик = Документы.ПоступлениеТоваровОтХранителя.ОбработчикДействий(ХозяйственнаяОперацияАкта);
		Возврат СтрШаблон(НСтр("ru = 'Необходимо скорректировать документ ""%1"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Оформить и вернуть"", ""Оформить"", ""Оформить и допоставить"".';
								|en = 'Correct document ""%1"" by recording an actual quantity in it. Make the changes to the discrepancy report lines with tolling options: ""Register and return"", ""Register"", ""Register and supply additionally"".'"), Обработчик.ПредставлениеОбъекта());
	ИначеЕсли ТипДокумента = "ВозвратТоваровОтХранителяДовозврат" Тогда
		Обработчик = Документы.ПоступлениеТоваровОтХранителя.ОбработчикДействий(ХозяйственнаяОперацияАкта);
		Возврат СтрШаблон(НСтр("ru = 'Необходимо оформить документ ""%1"" по недостачам, отмеченным к довозврату клиентом. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и ожидать допоставку"".';
								|en = 'Register document ""%1"" by shortages selected by a customer for additional return. It is registered for discrepancy report lines with processing option ""Register and wait for subsequent delivery"".'"), Обработчик.ПредставлениеОбъекта());
	ИначеЕсли ТипДокумента = "ВозвратТоваровОтХранителяВозвратХранителю" Тогда
		ПредставлениеДокумента = НСтр("ru = 'Передача товаров хранителю';
										|en = 'VMI fill-up'");
		Если ХозяйственнаяОперацияАкта = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5 Тогда
			ПредставлениеДокумента = НСтр("ru = 'Передача сырья переработчику';
											|en = 'Goods issue — Subcontracting services received'");
		КонецЕсли;
		Возврат СтрШаблон(НСтр("ru = 'Необходимо оформить документ ""%1"" по излишкам, отмеченным к возврату. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".';
								|en = 'Register the ""%1"" document of surpluses marked for return. Use the discrepancy report line with the ""Register and return"" processing option.'"), ПредставлениеДокумента);
	ИначеЕсли ТипДокумента = "ПриобретениеТоваровУслугДопоставкаПоПоступлению" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Приобретение товаров и услуг"" по недостачам, для которых требуется допоставка. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и ожидать допоставку"".';
					|en = 'Register the ""Vendor invoice"" document for shortages that require additional delivery. It is registered for discrepancy report lines with the ""Register and wait for subsequent delivery"" processing option.'");
	ИначеЕсли ТипДокумента = "ПоступлениеТоваровДопоставкаПоПоступлению" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Поступление товаров"" по недостачам, для которых требуется допоставка. Оформляется на строки акта о расхождениях с вариантами отработки ""Оформить и ожидать допоставку"", ""Ожидать допоставку без оформления"".';
					|en = 'Register the ""Warehouse receipt"" document for shortages that require additional delivery. It is registered for discrepancy report lines with the ""Register and wait for subsequent delivery"" and ""Wait for subsequent delivery without registration"" processing options.'");
	ИначеЕсли ТипДокумента = "ПеремещениеТоваров" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Перемещение товаров"" на допоставку недопоставленного товара на склад-получатель. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"", ""Оформить"".';
					|en = 'Register document ""Goods transfer"" for subsequent delivery of underdelivered products to destination warehouse. It is registered for discrepancy report lines with tolling option ""Register and return"", ""Register"".'");
	ИначеЕсли ТипДокумента = "ПеремещениеТоваровВозврат" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Перемещение товаров"" на возврат излишков со склада-получателя на склад-отправитель. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".';
					|en = 'Register document ""Goods transfer"" for surpluses return from a destination warehouse to an origin warehouse. It is registered for discrepancy report lines with tolling option ""Register and return"".'");
	ИначеЕсли ТипДокумента = "СписаниеНедостачТоваров" Тогда
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПриемНаХранениеСПравомПродажи Тогда
			Возврат НСтр("ru = 'Необходимо оформить документ ""Списание недостач товаров"" для списания  непризнанных недостач. Оформляется на строки акта о расхождениях с вариантом отработки ""Списать недостачи"".';
						|en = 'Register the ""Stock shortage recognition"" document to write-off unrecognized shortages. It is registered in discrepancy report lines with the ""Write-off shortages"" processing option.'");
		Иначе
			Возврат НСтр("ru = 'Необходимо оформить документ ""Списание недостач товаров"" для списания на прочие расходы непризнанных недостач. Оформляется на строки акта о расхождениях с вариантом отработки ""По вине сторонней компании"" или ""За наш счет"".';
						|en = 'Register ""Stock shortage recognition"" to write off unrecognized shortages as other expenses. It is registered for discrepancy report lines with processing option ""Due to the fault of the third party"" or ""At our expense"".'");
		КонецЕсли;
	ИначеЕсли ТипДокумента = "ПоступлениеТоваровПерепроведениеПослеКорректировки" Тогда
	...
	ИначеЕсли ТипДокумента = "КорректировкаЗадолженности" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Корректировка задолженности"" для отражения задолженности поставщика по недостачам, произошедшим по его вине. Оформляется на строки акта о расхождениях с вариантом отработки ""Списать за счет поставщика"".';
					|en = 'Register the ""AR/AP adjustment"" document to record vendor''s payables for shortages that occurred due to their fault. Register the document for discrepancy report lines with the ""Write off at vendor''s expense"" processing option.'");
	ИначеЕсли ТипДокумента = "ОприходованиеИзлишковТоваров" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Оприходование излишков товаров"" для оприходования излишков товаров. Оформляется на строки акта о расхождениях с вариантом отработки ""На прочие доходы"".';
					|en = 'Register document ""Stock surplus recognition"" to record as received goods surpluses. It is registered in discrepancy report lines with processing option ""For income"".'");
	ИначеЕсли ТипДокумента = "ИсправлениеПоступления" Тогда
		Возврат НСтр("ru = 'Необходимо скорректировать документ ""Приобретение товаров и услуг"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Оформить и вернуть"", ""Оформить"", ""Оформить и ожидать допоставку"".';
					|en = 'Correct the ""Vendor invoice"" document by recording an actual quantity in it. Make the changes to discrepancy report lines with processing options: ""Register and return"", ""Register"", ""Register and wait for subsequent delivery"".'");
	ИначеЕсли ТипДокумента = "ИзменениеПоступленияТоваров" Тогда
		Возврат НСтр("ru = 'Необходимо скорректировать документ ""Поступление товаров на склад"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Вернуть без оформления"", ""На прочие доходы"", ""Списать за наш счет"", ""Списать за счет сторонней компании"".';
					|en = 'The ""Goods receipt at the warehouse"" document must be adjusted by recording the actual quantity. This must be done for the discrepancy report lines with the processing options: ""Return the surplus"", ""For income"", ""Write off at our expense"", ""Write off at the expense of a third party"".'");
	ИначеЕсли ТипДокумента = "КорректировкаПоступленияТоваров" Тогда
		Возврат НСтр("ru = 'Необходимо ввести корректировочный документ ""Поступление товаров на склад"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Вернуть без оформления"", ""На прочие доходы"", ""Списать за наш счет"", ""Списать за счет сторонней компании"".';
					|en = 'The ""Goods receipt at the warehouse"" adjustment document must be entered with recording the actual quantity. This must be done for the discrepancy report lines with the processing options: ""Return the surplus"", ""For income"", ""Write off at our expense"", ""Write off at the expense of a third party"".'");
	ИначеЕсли ТипДокумента = "КорректировкаПоступленияТоваровИсправления" Тогда
		Возврат НСтр("ru = 'Необходимо ввести корректировочный документ ""Поступление товаров на склад"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Вернуть без оформления"", ""На прочие доходы"", ""Списать за наш счет"", ""Списать за счет сторонней компании"".';
					|en = 'The ""Goods receipt at the warehouse"" adjustment document must be entered with recording the actual quantity. This must be done for the discrepancy report lines with the processing options: ""Return the surplus"", ""For income"", ""Write off at our expense"", ""Write off at the expense of a third party"".'");
	ИначеЕсли ТипДокумента = "СписаниеРасхожденийПоступлениеПриобретение" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Списание расхождений между поступлением и приобретением"", отразив в нем расхождения. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""На прочие доходы"", ""Списать за наш счет"", ""Списать за счет сторонней компании"".';
					|en = 'Register the ""Write off discrepancies between receipt and purchase"" document to record discrepancies in it. Register the document for discrepancy report lines with the processing options: For income, Write off at our expense, Write off at the expense of a third party.'");
	ИначеЕсли ТипДокумента = "ИсправлениеПриемкиТоваровНаХранение" Тогда
		//++ НЕ УТКА
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
			ПредставлениеОбъекта = ДавальческаяСхемаКлиентСервер.СинонимДокументаПриемкаТоваровНаХранение();
		...
		КонецЕсли;
		//-- НЕ УТКА
		Возврат СтрШаблон(НСтр("ru = 'Необходимо скорректировать документ ""%1"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Оформить и вернуть"", ""Оформить"", ""Оформить и ожидать допоставку"".';
								|en = 'Correct the ""%1"" document by recording the actual quantity in it. Make the changes to the discrepancy report lines with the following options: ""Register and return"", ""Register"", and ""Register and wait for subsequent delivery"".'"), ПредставлениеОбъекта);
	ИначеЕсли ТипДокумента = "ВозвратТоваровПоставщикуПоПоступлению" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Возврат товаров поставщику"" по перепоставленному товару, отмеченному к возврату поставщику. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".';
					|en = 'Register document ""Return of goods to vendor"" for overdelivered goods marked for return to the vendor. It is registered in discrepancy report lines with processing option ""Register and return"".'");
	ИначеЕсли ТипДокумента = "ЭлектронныйДокументПодтверждение" Тогда
		Возврат НСтр("ru = 'Необходимо подтвердить полученный электронный документ, так как для отражения расхождения оформляется документ ""Корректировка приобретения"".';
					|en = 'Confirm the received electronic document, as the ""Purchase adjustment"" document is registered to record a discrepancy.'");
	ИначеЕсли ТипДокумента = "ЭлектронныйДокументОтклонение" Тогда
		Возврат НСтр("ru = 'Необходимо отклонить полученный электронный документ, так как для отражения расхождения изменяется исходный документ поступления.';
					|en = 'Reject received electronic document, as the original document for incoming payments is changed to record a discrepancy.'");
	ИначеЕсли ТипДокумента = "Претензия" Тогда
		Возврат НСтр("ru = 'Необходимо создать элемент справочника ""Претензии"" для отражения расчетов с поставщиком по расхождениям.';
					|en = 'Create a Complaints catalog item to record AR/AP with the vendor for discrepancies.'");
	КонецЕсли;
...
Функция ТекстЗапросаАктОРасхожденияхПослеПриемкиТоваровНаХранение(ПраваДоступаНаТаблицы, СоответствиеНаборовДанных)
	...
	Тогда
		...
		//++ НЕ УТКА
		Если ПраваДоступаНаТаблицы.ЧтениеЗаказПоставщику И ПраваДоступаНаТаблицы.ЧтениеЗаказДавальца2_5 Тогда
			
			ТекстОтборЗаказы =
			"НЕ АктОРасхожденияхПослеПриемкиТовары.ЗаказПоставщику В (
			|											ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка),
			|											ЗНАЧЕНИЕ(Документ.ЗаказДавальца2_5.ПустаяСсылка))
			|	И ТИПЗНАЧЕНИЯ(АктОРасхожденияхПослеПриемкиТовары.ЗаказПоставщику) В (
			|														ТИП(Документ.ЗаказПоставщику),
			|														ТИП(Документ.ЗаказДавальца2_5))";
			
		ИначеЕсли ПраваДоступаНаТаблицы.ЧтениеЗаказДавальца2_5 Тогда
			
			ТекстОтборЗаказы =
			"АктОРасхожденияхПослеПриемкиТовары.ЗаказПоставщику <> ЗНАЧЕНИЕ(Документ.ЗаказДавальца2_5.ПустаяСсылка)
			|	И ТИПЗНАЧЕНИЯ(АктОРасхожденияхПослеПриемкиТовары.ЗаказПоставщику) = ТИП(Документ.ЗаказДавальца2_5)";
			
		ИначеЕсли ПраваДоступаНаТаблицы.ЧтениеЗаказПоставщику Тогда
		//-- НЕ УТКА
...
Процедура ВывестиДокументПриемкаТоваровНаХранение(СтрокаДействия, Макет, Описание)
	...
	ХозяйственнаяОперация = Неопределено;
	//++ НЕ УТКА
	Если ХозяйственнаяОперацияАкта = Перечисления.ХозяйственныеОперации.ПередачаДавальцу2_5
	 Или ХозяйственнаяОперацияАкта = Перечисления.ХозяйственныеОперации.ВозвратДавальцу2_5 Тогда
		ХозяйственнаяОперация = ХозяйственнаяОперацияАкта;
	КонецЕсли;
	//-- НЕ УТКА
...
Функция ТекстЗапросаАктОРасхожденияхПослеПередачиТоваровХранителю()
	...
	//++ НЕ УТ
	|,
	|	АктОРасхожденияхПослеОтгрузкиТовары.Ссылка.ХозяйственнаяОперация КАК ХозяйственнаяОперация
	//-- НЕ УТ
	|ПОМЕСТИТЬ Действия
	|ИЗ
	|	Документ.АктОРасхожденияхПослеОтгрузки.Товары КАК АктОРасхожденияхПослеОтгрузкиТовары
	...
	|	&ИспользоватьЗаявкиНаВозврат
	|	И Поступление.Проведен
	|	И Действия.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПереработчику2_5)
	//-- НЕ УТ
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////// 3
