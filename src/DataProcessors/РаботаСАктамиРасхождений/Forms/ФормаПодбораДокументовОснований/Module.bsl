...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПоступлениеОтДавальца Тогда
		СоглашенияСКлиентамиПрименимы = Ложь;
	ИначеЕсли ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПередачаДавальцу
		...
		СоглашенияСПоставщикамиПрименимы = Ложь;
	//-- НЕ УТКА
	КонецЕсли;
		...
		Если Не ОчисткаПодтверждена И ТабличнаяЧастьНеПустая Тогда
			...
			Иначе
				ТекстВопроса = НСтр("ru = 'Табличная часть документа будет перезаполнена. Продолжить?';
									|en = 'Table of the document will be refilled. Continue?'");
			КонецЕсли;
...
Процедура СформироватьИнформационнуюНадписьОтборы()
	...
	Если НЕ ОбщегоНазначенияУТКлиентСервер.АвторизованВнешнийПользователь() Тогда
		ИнформационнаяНадписьОтборы = НСтр("ru = 'Отбор по: %Организация%, %Партнер%,%Контрагент% %Валюта%, %Соглашение%, %НалогообложениеНДС%,%Договор%%ХозяйственнаяОперация%, %ЦенаВключаетНДС%';
											|en = 'Filter by: %Организация%, %Партнер%,%Контрагент% %Валюта%, %Соглашение%, %НалогообложениеНДС%,%Договор%%ХозяйственнаяОперация%, %ЦенаВключаетНДС%'");
	Иначе
		ИнформационнаяНадписьОтборы = НСтр("ru = 'Отбор по: %Контрагент%%Договор%%Соглашение%';
											|en = 'Filter by: %Контрагент%%Договор%%Соглашение%'");
	КонецЕсли;
	...
	ИнформационнаяНадписьОтборы = СтрЗаменить(ИнформационнаяНадписьОтборы,"%ЦенаВключаетНДС%", ?(ЦенаВключаетНДС, НСтр("ru = 'Цена включает НДС';
...
Процедура ВывестиИнформационнуюНадписьКоличествоДокументов(Форма)
	...
	Если Форма.КоличествоДокументов <> 0 Тогда
		ТекстНадписи = НСтр("ru = 'Подобрано документов - %КоличествоДокументов%, на сумму %ДокументыНаСумму% %Валюта%.';
							|en = 'Documents selected - %КоличествоДокументов%, in the amount of %ДокументыНаСумму% %Валюта%.'");
