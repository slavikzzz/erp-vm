...
Процедура ОтключитьОрганизации(Команда)
	...
	Если ОдиночнаяРегистрация Тогда
		ТекстВопроса =
			...
			|en = 'The company will be disconnected from 1C:BusinessNetwork service in the application (infobase).
			|The company will be displayed as registered for other service participants.'");
	Иначе
		ТекстВопроса =
			...
			|en = 'Companies will be disconnected from 1C:BusinessNetwork service in the application (infobase).
			|The companies will be displayed as registered ones for other service participants.'");
	КонецЕсли;
...
Функция ПодключитьОрганизациюКСервисуНаСервере(Организация)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Подключение организации к сервису Бизнес-сеть';
															|en = 'Attach the company to the Business Network service'");
...
Процедура ОбработатьСтатусПодключенияОрганизации(СтатусПодключениеОрганизации, Организация, ВыполнятьПодключениеВФоне)
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Неизвестный статус подключения организации';
								|en = 'Unknown status of company attachment'")	
	КонецЕсли;
...
Функция ДанныеОрганизацииЗаполнены(Организация)
	...
	Если Не ЗначениеЗаполнено(ЗначениеИНН) Тогда
		...
		ТекстОшибки = НСтр("ru = 'Не заполнено свойство ""ИНН""';
							|en = '""TIN"" property is not filled in'");
...
Процедура ВыполнитьОтключениеОрганизаций()
	...
	Если Не ВсеЗарегистрированы Тогда
		ТекстСообщения = НСтр("ru = 'Отключение организаций в программе не выполнено. Выбрана незарегистрированная организация.';
								|en = 'Companies are not disabled in the application. Unregistered company is selected.'");
	...
	Если Не Отказ Тогда
		...
		ТекстОповещения = НСтр("ru = 'Отключение организаций в программе успешно выполнено.';
								|en = 'The companies are successfully disconnected in the application.'");
		ПоказатьОповещениеПользователя(НСтр("ru = '1С:Бизнес-сеть';
											|en = '1C:Business Network'"),, ТекстОповещения, БиблиотекаКартинок.БизнесСеть);
...
Процедура ВыполнитьОтключениеОрганизацииОдиночное()
	...
	Если Не ОрганизацияЗарегистрирована Тогда
		ТекстСообщения = НСтр("ru = 'Отключение организаций в программе не выполнено. Организация не зарегистрирована.';
								|en = 'Companies are not disabled in the application. Company is not registered.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если Не Отказ Тогда
		...
		ТекстОповещения = НСтр("ru = 'Отключение организации в программе успешно выполнено.';
								|en = 'The company is successfully disconnected in the application.'");
		ПоказатьОповещениеПользователя(НСтр("ru = '1С:Бизнес-сеть';
											|en = '1C:Business Network'"),, ТекстОповещения, БиблиотекаКартинок.БизнесСеть);
		Оповестить("БизнесСеть_РегистрацияОрганизаций",, ЭтотОбъект);
...
Процедура УстановкаВидимостиЭлементовПодсистемыТорговыеПредложения()
	...
	МассивПодстрок.Добавить(СтрШаблон(НСтр("ru = '<a href=""%1"">Торговая площадка</a>';
											|en = '<a href=""%1"">Trading platform</a>'"), БизнесСетьКлиентСервер.ГиперссылкаНаПромоСайтТорговаяПлощадка()));
	МассивПодстрок.Добавить(" - " 
		...
		|en = 'service in the 1C:Enterprise applications for suppliers and customers (buyers).
		|The service helps suppliers to simplify order processing and get new customers.
		|The service allows customers to find beneficial suppliers and reduce order registration time.'"));
...
Процедура ПоказатьПредупреждениеОПовторномПодключении(Организация)
	...
	Если ОдиночнаяРегистрация Тогда
		ШаблонЗаголовка = 
			...
			|en = 'Access of the %1 company to the 1C:Business network service was suspended. 
			|Please, reconnect the company.'");
	Иначе
		ШаблонЗаголовка = 
