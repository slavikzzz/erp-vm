...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РежимИсходящихДокументов Тогда
		...
		Заголовок = НСтр("ru = 'Исходящие документы 1С:Бизнес-сеть';
						|en = 'Outgoing 1C:Business Network documents'");
	...
	Иначе
		...
		Заголовок = НСтр("ru = 'Входящие документы 1С:Бизнес-сеть';
						|en = 'Incoming 1C:Business Network documents'");
...
Процедура СписокДокументовПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = НСтр("ru = 'Выделенные документы будут удалены в сервисе 1С:Бизнес-сеть.
							  ...
							  |en = 'The selected documents will be deleted in 1C:Business Network service.
							  |Infobase accounting documents will not be changed.
							  |Continue the transaction execution?'");
...
Процедура СохранитьДокументы(Команда)
	...
	ТекстПредложения = НСтр("ru = 'Для сохранения документов необходимо установить расширение для работы с файлами.';
							|en = 'To save documents, install the file system extension.'");
...
Процедура ОтклонитьДокументы(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Выделенные документы будут отклонены для загрузки.
							  |Продолжить выполнение операции?';
							  |en = 'The selected documents will be declined for import.
							  |Continue?'");
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура НастроитьПолеВыбораОрганизации()
	...
	Если Не ЗначениеЗаполнено(ПодключенныеОрганизации) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Ни одна организация не подключена к Бизнес-сети';
													|en = 'No company is connected to the Business Network'"));
		Возврат;
	КонецЕсли;
...
Процедура УдалитьДокументВСервисеПослеВопроса(Результат, МассивСтрок) Экспорт
	...
	Иначе
		...
		ТекстПояснения	= НСтр("ru = 'Удалены документы в сервисе 1С:Бизнес-сеть.';
								|en = 'Documents in 1C:Business Network service are deleted.'");
	КонецЕсли;
...
Функция ПолучитьДокументыФоне(СледующаяСтраница)
	...
	Задание.Наименование = НСтр("ru = '1С:Бизнес-сеть. Получение документов сервиса.';
								|en = '1C:Business network. Get the service documents.'");
...
Процедура ЗагрузитьДокументыПродолжение()
	...
	Если Не ЗначениеЗаполнено(Элементы.Список.ВыделенныеСтроки) Тогда
		БизнесСетьСлужебныйКлиент.ПоказатьОповещениеБизнесСети(НСтр("ru = 'Не выбраны документы для загрузки';
																	|en = 'No documents selected for load'"));
		Возврат;
	КонецЕсли;
...
Функция ЗагрузитьДокументыВФоне()
	...
	Если СписокДокументов.Количество() Тогда
		...
		Если НепроведенныеДокументы.Количество() <> СписокДокументов.Количество() Тогда
			ТекстОшибки = НСтр("ru = 'Перезапись проведенных документов запрещена.';
								|en = 'Cannot save posted documents again.'");
	...
	Задание.Наименование = НСтр("ru = '1С:Бизнес-сеть. Загрузка документов.';
								|en = '1C:Business Network. Document import.'");
	...
	Для каждого СтрокаЗагрузки Из ЗагружаемыеДокументы Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаЗагрузки.ТипОбъекта) Тогда
			Если ОписаниеПакета.СписокТипов.Количество() Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Поле ""Тип"" не заполнено';
															|en = 'The Type field is not filled in'"),, "ТипОбъекта",, Отказ);
				Возврат;
			Иначе
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Загрузка данного документа не поддерживается';
