...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("СформироватьАвтоматически") Тогда
		ЗагрузитьВДокументАвтоматически(Отказ, Параметры);
		Возврат;
	КонецЕсли;
	...
	Если ЗавершениеРаботы = Истина Тогда
		Если ТребуетсяПодтверждениеЗакрытияФормы Тогда
			...
			ТекстПредупреждения = НСтр("ru = 'Работа в форме не завершена.
				...
				|en = 'The form operations are not completed.
				|Close the window manually before you exit the application.'");
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура КонсолидироватьИСформироватьДекларациюПоНДС(Команда);
	...
	Если ФайлыВыгрузки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не добавлены файлы для консолидации';
										|en = 'Files for consolidation are not added'"));
		Возврат;
	КонецЕсли;
	...
	Если Организация.Пустая() Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не выбрана или не найдена организация %1 для формирования декларации по НДС';
				|en = 'Company %1 to generate the VAT declaration was not selected or it was not found'"), ПараметрОписания);
	...
	Если НЕ ПараметрыВыгрузки.Свойство("ПараметрыФормыОтчета", ПараметрыФормыОтчета) ИЛИ ПараметрыФормыОтчета = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru = 'Не сформированы параметры для создания декларации по НДС!';
								|en = 'Parameters for creating VAT declaration are not generated. '");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ОписаниеОшибки, , "");
		Возврат;
	КонецЕсли;
	...
	Если ФормаДекларации = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru = 'Декларация по НДС не сформирована!';
								|en = 'VAT declaration is not generated.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ОписаниеОшибки, , "");
		Возврат;
	КонецЕсли;
	...
	Если ЗаполнятьВФоне Тогда
		...
		ДополнительныеПараметры.Вставить("СохраненныйДокумент", СохраненныйДокумент);
		
		Если Результат.Статус = "Выполнено" Тогда
			КонсолидироватьВФоновомЗаданииЗавершение(Результат, ДополнительныеПараметры);
		ИначеЕсли Результат.Статус = "Выполняется" Тогда
			ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтаФорма);
			ПараметрыОжидания.ВыводитьСообщения = Истина;
			ОповещениеОЗавершении = Новый ОписаниеОповещения("КонсолидироватьВФоновомЗаданииЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ВыбратьФайлы(ЗаменитьСтроку = Ложь)
	...
	Если ПоддерживаетсяРасширениеРаботыСФайлами = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для множественного выбора файлов выгрузки рекомендуется установить расширение работы с файлами.';
								|en = 'It is recommended that you install the file system extension to select multiple files for export.'");
...
Процедура ВыбратьФайлыПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	СтрокаФильтра = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Файлы выгрузки (*.%1)|*.%1|Все файлы(%2)|%2';
			|en = 'Export files (*.%1)|*.%1|All files(%2)|%2'"), "xml", ПолучитьМаскуВсеФайлы());
	
	Если ПоддерживаетсяРасширениеРаботыСФайлами <> Истина Тогда
		
		Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
		Диалог.Заголовок = НСтр("ru = 'Выберите файл выгрузки декларации по НДС';
								|en = 'Select VAT declaration export file'");
		Диалог.МножественныйВыбор = Ложь;
		Диалог.Фильтр = СтрокаФильтра;
		
		ОбработкаПродолжения = Новый ОписаниеОповещения(
			"ВыбратьФайлыПриНачалеПомещенияФайла", ЭтотОбъект, Контекст);
		
		НачатьПомещениеФайла(ОбработкаПродолжения, , Диалог, Истина, УникальныйИдентификатор);
		Возврат;
	КонецЕсли;
	
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
	Диалог.Заголовок = НСтр("ru = 'Выберите файл(ы) выгрузки декларации по НДС';
							|en = 'Select VAT declaration export file(s)'");
...
Процедура ВыбратьФайлыПослеПомещенияФайлов(ПомещенныеФайлы, Контекст) Экспорт
	...
	Для Каждого ПомещенныйФайл Из ПомещенныеФайлы Цикл
		...
		СоставИмени = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ВРег(ПомещенныйФайл.ПолноеИмя));
	...
	Для Каждого ПомещенныйФайл Из ПомещенныеФайлы Цикл
		...
		Если ПараметрыФайлаВыгрузки = Неопределено Тогда
			...
			НСтр("ru = 'При получении параметров из файла выгрузки ""%1"" произошла ошибка:
			           |%2';
			           |en = 'When receiving parameters from export file ""%1"", an error occurred:
		...
		Если (НомКорр <> "0" И НомКорр <> "") И ПризнСвед = "1" Тогда
			...
			НСтр("ru = 'В файла выгрузки ""%1"" установлен признак актуальности ранее представленных сведений для корректирующего отчета:
			           |%2';
			           |en = 'The flag that shows relevance of the information provided earlier is selected in the ""%1"" export file for adjusting report:
		...
		Если ПараметрыВыгрузкиИзИДФайла = Неопределено ИЛИ ПустаяСтрока(ПараметрыВыгрузкиИзИДФайла.ИНН) Тогда
			...
			НСтр("ru = 'При получении параметров из идентификатора файла выгрузки ""%1"" произошла ошибка:
			           |%2';
			           |en = 'When receiving parameters from export file ""%1"" ID, an error occurred:
		...
		Если ФайлыВыгрузки.Количество() > 0 Тогда
			...
			Если НайденныеСтрокиФайлыВыгрузки.Количество() = 0 Тогда
				...
				НСтр("ru = 'При добавлении файла выгрузки ""%1"" произошла ошибка:
				           |%2';
				           |en = 'When adding export file ""%1"", an error occurred:
...
Процедура УдалитьФайлыНаСервере()
	...
	Пока ФайлыВыгрузки.Количество() > 0 Цикл
		УдалитьСтрокуТаблицыИФайлы(ЭтаФорма, ФайлыВыгрузки[0]);
	КонецЦикла;
...
Процедура ПоказатьОшибку(ОшибкаНаСервере, ИмяРеквизитаФормы = "")
	...
	Если ОшибкаНаСервере.Свойство("ОписаниеОшибки") Тогда
		...
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			ОбщегоНазначения.СообщитьПользователю(ОшибкаНаСервере.ОписаниеОшибки, , ИмяРеквизитаФормы);
		#Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(ОшибкаНаСервере.ОписаниеОшибки, , ИмяРеквизитаФормы);
		#КонецЕсли
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Заполнение консолидированной декларации по НДС';
				|en = 'Fill in consolidated VAT declaration'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗагрузитьВЭлектронноеПредставление(ФайлыРазделовДекларации)
	...
	Если КонтекстЭДОКлиент = Неопределено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'ТекстОшибкиИнициализацииКонтекстаЭДО';
														|en = 'EDIContextInitializationErrorText'"));
		Возврат;
	КонецЕсли;
...
Функция КонсолидироватьНаСервере(ТекстВыгрузки, ОшибкаНаСервере, ЗаполнитьДекларацию = Ложь, ЗаполнитьТолькоПараметры = Ложь, СохраненныйДок = Неопределено) Экспорт
	...
	ПараметрыВыгрузки.Свойство("ВидДокумента", ВидДокумента);
	
	ПараметрыВыгрузки.Очистить();
	
	ПараметрыВыгрузки.Вставить("ВидДокумента", ВидДокумента);
	ПараметрыВыгрузки.Вставить("ПравилаЗаполнения", "2015Кв1"); // по умолчанию
	
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы",     "NO_NDS_1_003_00_05_04_05.xsd");
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы2017", "NO_NDS_1_003_00_05_05_01.xsd");
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы2019", "NO_NDS_1_003_00_05_06_01.xsd");
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы2020", "NO_NDS_1_003_00_05_07_01.xsd");
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы2021", "NO_NDS_1_003_00_05_08_01.xsd");
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы2022", "NO_NDS_1_003_00_05_09_01.xsd");
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы2023", "NO_NDS_1_003_00_05_10_01.xsd");
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы2025", "NO_NDS_1_003_00_05_11_01.xsd");
	...
	Для Каждого ДанныеФайлаВыгрузки Из ФайлыВыгрузки Цикл
		...
		Если НЕ ЗначениеЗаполнено(ДанныеФайлаВыгрузки.Индекс) ИЛИ ДанныеФайлаВыгрузки.Индекс = "0000000" Тогда
			...
			ФайлНаСервере = Новый Файл(ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере);
			Если НЕ ФайлНаСервере.Существует() Тогда
				ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере = ПолучитьИмяВременногоФайла();
				ДвоичныеДанныеФайла = ПолучитьИзВременногоХранилища(ДанныеФайлаВыгрузки.Адрес);
				ДвоичныеДанныеФайла.Записать(ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере);
			КонецЕсли;
			ФайлНаСервере = Неопределено;
				...
				Если НЕ СформироватьПараметрыВыгрузки(ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере, ОписаниеОшибки) Тогда
					ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'При формировании параметров выгрузки из файла ""%1"" произошла ошибка:
						           |%2';
						           |en = 'When generating export parameters from file ""%1"", an error occurred:
			...
			Если НЕ ЗаполнитьДанныеРазделов1_7(СтруктураПараметров, ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере, ОписаниеОшибки) Тогда
				ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При заполнении данных разделов 1-7 из файла выгрузки ""%1"" произошла ошибка:
					           |%2';
					           |en = 'When filling in data of sections 1-7 from export file ""%1"", an error occurred:
					           |%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
		...
		Иначе
			...
			СтруктураПараметров.Вставить("ИД" + ДанныеФайлаВыгрузки.Индекс, ДанныеФайлаВыгрузки.НаименованиеФайла);
	...
	Если НЕ ПараметрыВыгрузки.Свойство("ИдФайл") Тогда
		ОшибкаНаСервере.Вставить("ОписаниеОшибки", НСтр("ru = 'Не сформированы параметры для консолидации. Нет файлов выгрузки разделов 1-7.';
														|en = 'Parameters for consolidation are not generated. No import files of sections 1-7.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ЗаполнитьДекларацию Тогда
		...
		Если СохраненныйДок <> Неопределено
			И (НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ПравилаЗаполнения)
				ИЛИ НЕ СтрЗаканчиваетсяНа(СохраненныйДок.ВыбраннаяФорма, ПараметрыВыгрузки.ПравилаЗаполнения)) Тогда
			ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Версия файла выгрузки ""%1"" пока не поддерживается';
					|en = 'Версия файла выгрузки ""%1"" пока не поддерживается'"), ПараметрыВыгрузки.ВерсФорм));
			Возврат Ложь;
		КонецЕсли;
	...
	СтруктураПараметров.Вставить("ПравилаЗаполнения", ПараметрыВыгрузки.ПравилаЗаполнения);
			...
			Если НЕ ЗаполнитьТаблицуРаздела8(СтруктураПараметров, ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере, ОписаниеОшибки) Тогда
				ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При заполнении данных раздела 8 из файла выгрузки ""%1"" произошла ошибка:
					           |%2';
					           |en = 'When filling in data of section 8 from export file ""%1"", an error occurred:
					           |%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
			...
			Если НЕ ЗаполнитьТаблицуРаздела8_1(СтруктураПараметров, ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере, ОписаниеОшибки) Тогда
				ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При заполнении данных приложения 1 к разделу 8 из файла выгрузки ""%1"" произошла ошибка:
					           |%2';
					           |en = 'An error occurred when filling in data of annex 1 to section 8 from export file ""%1"":
					           |%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
			...
			Если НЕ ЗаполнитьТаблицуРаздела9(СтруктураПараметров, ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере, ОписаниеОшибки) Тогда
				ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При заполнении данных раздела 9 из файла выгрузки ""%1"" произошла ошибка:
					           |%2';
					           |en = 'When filling in data of section 9 from export file ""%1"", an error occurred:
					           |%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
			...
			Если НЕ ЗаполнитьТаблицуРаздела9_1(СтруктураПараметров, ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере, ОписаниеОшибки) Тогда
				ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При заполнении данных приложения 1 к разделу 9 из файла выгрузки ""%1"" произошла ошибка:
					           |%2';
					           |en = 'An error occurred when filling in data of annex 1 to section 9 from export file ""%1"":
					           |%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
			...
			Если НЕ ЗаполнитьТаблицуРаздела10(СтруктураПараметров, ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере, ОписаниеОшибки) Тогда
				ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При заполнении данных раздела 10 из файла выгрузки ""%1"" произошла ошибка:
					           |%2';
					           |en = 'When filling in data of section 10 from export file ""%1"", an error occurred:
					           |%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
			...
			Если НЕ ЗаполнитьТаблицуРаздела11(СтруктураПараметров, ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере, ОписаниеОшибки) Тогда
				ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При заполнении данных раздела 11 из файла выгрузки ""%1"" произошла ошибка:
					           |%2';
					           |en = 'When filling in data of section 11 from export file ""%1"", an error occurred:
					           |%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
			...
			Если НЕ ЗаполнитьТаблицуРаздела12(СтруктураПараметров, ДанныеФайлаВыгрузки.ПолноеИмяФайлаНаСервере, ОписаниеОшибки) Тогда
				ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При заполнении данных раздела 12 из файла выгрузки ""%1"" произошла ошибка:
					           |%2';
					           |en = 'When filling in data of section 12 from export file ""%1"", an error occurred:
					           |%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
	...
	Если ЗаполнитьДекларацию Тогда
		...
		Если СохраненныйДок = Неопределено Тогда
			ОшибкаНаСервере.Вставить("ОписаниеОшибки", НСтр("ru = 'Не сформирован документ для заполнения консолидированными данными.';
															|en = 'Document for population with consolidated data is not created.'"));
			Возврат Ложь;
		КонецЕсли;
		...
		Если НЕ ЗаполнитьДаннымиСохраненныйДокумент(СохраненныйДок, ДеревоВыгрузки, СтруктураПараметров, ОписаниеОшибки) Тогда
			ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При заполнении сохраненного отчета ""%1"" произошла ошибка:
				           |%2';
				           |en = 'When filling in saved report ""%1"", an error occurred:
	...
	ИДРезультатаВыгрузки     = "НДС2015Разделы1_7";
	Если ПараметрыВыгрузки.ПравилаЗаполнения = "2017Кв1" Тогда
		ИДРезультатаВыгрузки = "НДС2017Разделы1_7";
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2019Кв1" Тогда
		ИДРезультатаВыгрузки = "НДС2019Разделы1_7";
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2020Кв4" Тогда
		ИДРезультатаВыгрузки = "НДС2020Разделы1_7";
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2021Кв3" Тогда
		ИДРезультатаВыгрузки = "НДС2021Разделы1_7";
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2022Кв1" Тогда
		ИДРезультатаВыгрузки = "НДС2022Разделы1_7";
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2023Кв1" Тогда
		ИДРезультатаВыгрузки = "НДС2023Разделы1_7";
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2025Кв1" Тогда
		ИДРезультатаВыгрузки = "НДС2025Разделы1_7";
	КонецЕсли;
	
	ТекстВыгрузки.Добавить(РезультатВыгрузкиВоВременномХранилище(ТекстВыгрузкиРазделов1_7, ПараметрыВыгрузки.ИдФайл + ".xml", ИДРезультатаВыгрузки));
...
Функция КонсолидироватьВФоновомЗадании(ТекстВыгрузки, ОшибкаНаСервере, ЗаполнитьДекларацию = Ложь, ЗаполнитьТолькоПараметры = Ложь, СохраненныйДок = Неопределено)
	...
	ПараметрыПроцедуры.Вставить("ТекстВыгрузки", ТекстВыгрузки);
	ПараметрыПроцедуры.Вставить("ОшибкаНаСервере", ОшибкаНаСервере);
	ПараметрыПроцедуры.Вставить("ЗаполнитьДекларацию", ЗаполнитьДекларацию);
	ПараметрыПроцедуры.Вставить("ЗаполнитьТолькоПараметры", ЗаполнитьТолькоПараметры);
	ПараметрыПроцедуры.Вставить("СохраненныйДок", СохраненныйДок);
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания =
		НСтр("ru = 'Выполняется заполнение отчета консолидированными данными. Пожалуйста, подождите...';
			|en = 'The report is being populated with consolidated data. Please wait...'");
...
&НаКлиенте
Процедура КонсолидироватьВФоновомЗаданииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Попытка
		Если Результат.Статус = "Выполнено" Тогда
			ОткрытьФормуЗаполненногоОтчета(ДополнительныеПараметры.СохраненныйДокумент);
		КонецЕсли;
	Исключение
		ЭтаФорма.Доступность = Истина;
		ВызватьИсключение;
...
Функция СформироватьПараметрыВыгрузки(ФайлВыгрузки, ОписаниеОшибки = Неопределено)
	...
	ПараметрыВыгрузки.Вставить("ВерсФорм",     "");
		...
		Пока ОбъектЧтениеXML.Прочитать() Цикл
			Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				Если ИмяУзла = "Файл" Тогда
					...
					ПараметрыВыгрузки.ВерсФорм = СокрЛП(АтрибутыУзла["ВерсФорм"]);
	...
	Если ПараметрыВыгрузки.ВерсФорм = "5.05" Тогда
		ПараметрыВыгрузки.Вставить("ПравилаЗаполнения", "2017Кв1");
	ИначеЕсли ПараметрыВыгрузки.ВерсФорм = "5.06" Тогда
		ПараметрыВыгрузки.Вставить("ПравилаЗаполнения", "2019Кв1");
	ИначеЕсли ПараметрыВыгрузки.ВерсФорм = "5.07" Тогда
		ПараметрыВыгрузки.Вставить("ПравилаЗаполнения", "2020Кв4");
	ИначеЕсли ПараметрыВыгрузки.ВерсФорм = "5.08" Тогда
		ПараметрыВыгрузки.Вставить("ПравилаЗаполнения", "2021Кв3");
	ИначеЕсли ПараметрыВыгрузки.ВерсФорм = "5.09" Тогда
		ПараметрыВыгрузки.Вставить("ПравилаЗаполнения", "2022Кв1");
	ИначеЕсли ПараметрыВыгрузки.ВерсФорм = "5.10" Тогда
		ПараметрыВыгрузки.Вставить("ПравилаЗаполнения", "2023Кв1");
	ИначеЕсли ПараметрыВыгрузки.ВерсФорм = "5.11" Тогда
		ПараметрыВыгрузки.Вставить("ПравилаЗаполнения", "2025Кв1");
	КонецЕсли;
...
Процедура ИнициализироватьТаблицыРазделов8_81(СтруктураПараметров)
	...
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
...
Процедура ИнициализироватьТаблицыРазделов9_91(СтруктураПараметров)
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		Раздел9.Колонки.Добавить("СтоимПродСФ7", ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
		Раздел9.Колонки.Добавить("СтоимПродСФ5", ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
	КонецЕсли;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		Раздел9.Колонки.Добавить("СумНДССФ7",    ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
		Раздел9.Колонки.Добавить("СумНДССФ5",    ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
	КонецЕсли;
	...
	ИтогиРаздел9.Вставить("СтПродБезНДС20",  0);
	ИтогиРаздел9.Вставить("СтПродБезНДС18",  0);
	ИтогиРаздел9.Вставить("СтПродБезНДС10",  0);
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ИтогиРаздел9.Вставить("СтПродБезНДС7",  0);
		ИтогиРаздел9.Вставить("СтПродБезНДС5",  0);
	КонецЕсли;
	ИтогиРаздел9.Вставить("СтПродБезНДС0",   0);
	ИтогиРаздел9.Вставить("СумНДСВсКПр20",   0);
	ИтогиРаздел9.Вставить("СумНДСВсКПр18",   0);
	ИтогиРаздел9.Вставить("СумНДСВсКПр10",   0);
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ИтогиРаздел9.Вставить("СумНДСВсКПр7",   0);
		ИтогиРаздел9.Вставить("СумНДСВсКПр5",   0);
	КонецЕсли;
	ИтогиРаздел9.Вставить("СтПродОсвВсКПр",  0);
	
	ИтогиРаздел91 = Новый Структура;
	ИтогиРаздел91.Вставить("ИтСтПродКПр20",   0);
	ИтогиРаздел91.Вставить("ИтСтПродКПр18",   0);
	ИтогиРаздел91.Вставить("ИтСтПродКПр10",   0);
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ИтогиРаздел91.Вставить("ИтСтПродКПр7",   0);
		ИтогиРаздел91.Вставить("ИтСтПродКПр5",   0);
	КонецЕсли;
	ИтогиРаздел91.Вставить("ИтСтПродКПр0",    0);
	ИтогиРаздел91.Вставить("СумНДСИтКПр20",   0);
	ИтогиРаздел91.Вставить("СумНДСИтКПр18",   0);
	ИтогиРаздел91.Вставить("СумНДСИтКПр10",   0);
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ИтогиРаздел91.Вставить("СумНДСИтКПр7",   0);
		ИтогиРаздел91.Вставить("СумНДСИтКПр5",   0);
	КонецЕсли;
	ИтогиРаздел91.Вставить("ИтСтПродОсвКПр",  0);
	ИтогиРаздел91.Вставить("СтПродВсП1Р9_20", 0);
	ИтогиРаздел91.Вставить("СтПродВсП1Р9_18", 0);
	ИтогиРаздел91.Вставить("СтПродВсП1Р9_10", 0);
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ИтогиРаздел91.Вставить("СтПродВсП1Р9_7", 0);
		ИтогиРаздел91.Вставить("СтПродВсП1Р9_5", 0);
	КонецЕсли;
	ИтогиРаздел91.Вставить("СтПродВсП1Р9_0",  0);
	ИтогиРаздел91.Вставить("СумНДСВсП1Р9_20", 0);
	ИтогиРаздел91.Вставить("СумНДСВсП1Р9_18", 0);
	ИтогиРаздел91.Вставить("СумНДСВсП1Р9_10", 0);
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ИтогиРаздел91.Вставить("СумНДСВсП1Р9_7", 0);
		ИтогиРаздел91.Вставить("СумНДСВсП1Р9_5", 0);
	КонецЕсли;
...
Процедура ИнициализироватьТаблицыРазделов10_12(СтруктураПараметров)
	...
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
		  ...
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		Раздел10.Колонки.Добавить("СвСчФОтПрод", Новый ОписаниеТипов("Массив"));
	КонецЕсли;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		Раздел12.Колонки.Добавить("КодВидОпер", Новый ОписаниеТипов("Массив"));
	КонецЕсли;
	
	Раздел12.Колонки.Добавить("СвПокуп");
	
	Раздел12.Колонки.Добавить("ДополнительныеСведения");
...
Функция ЗаполнитьТаблицуРаздела8(СтруктураПараметров, ФайлВыгрузки, ОписаниеОшибки = Неопределено)
	...
	НайденныеСтрокиФайлыВыгрузки = ФайлыВыгрузки.НайтиСтроки(Новый Структура("ПолноеИмяФайлаНаСервере", ФайлВыгрузки));
	Если НЕ ЗначениеЗаполнено(НайденныеСтрокиФайлыВыгрузки) Тогда
		ОписаниеОшибки = "Не найдена строка таблицы для файла: " + ФайлВыгрузки + ".";
		Возврат Ложь;
	КонецЕсли;
	
	ИмяФайлаРаздела = НайденныеСтрокиФайлыВыгрузки[0].НаименованиеФайла;
	...
	Попытка
			...
			Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				ИначеЕсли ИмяУзла = "КнПокСтр" Тогда
					...
					Для Каждого АтрибутУзла Из АтрибутыУзла Цикл
						...
						КонецЕсли;
					КонецЦикла;
				ИначеЕсли ИмяУзла = "РегНомТД"
					И (СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
				   ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
				   ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4") Тогда
					ОбъектЧтениеXML.Прочитать();
					ТекущиеДанные.РегНомТД.Добавить(ОбъектЧтениеXML.Значение);
				ИначеЕсли ИмяУзла = "СвРегНом"
					 И (СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
					ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
					ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
					ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1") Тогда
					АтрУзла = Новый Структура("РегНомПросл,ОКЕИ,КолТовПросл,СтоимТовПросл",
						СокрЛП(АтрибутыУзла["РегНомПросл"]), СокрЛП(АтрибутыУзла["ОКЕИ"]),
						ЧислоИзСтроки(АтрибутыУзла["КолТовПросл"]), ЧислоИзСтроки(АтрибутыУзла["СтоимТовПросл"]));
					ТекущиеДанные.СвРегНом.Добавить(АтрУзла);
		...
		Если СтруктураПараметров.ЗаполнитьДекларацию = Истина И ТекущиеДанные <> Неопределено Тогда
			...
			Если СтруктураПараметров.Свойство("ИД0000080", ИмяПоследнегоФайлаРаздела) И ИмяПоследнегоФайлаРаздела = ИмяФайлаРаздела Тогда
...
Функция ЗаполнитьТаблицуРаздела8_1(СтруктураПараметров, ФайлВыгрузки, ОписаниеОшибки = Неопределено)
	...
	Попытка
		...
		Если СтруктураПараметров.ЗаполнитьДекларацию = Истина И ТекущиеДанные <> Неопределено Тогда
			ИмяПоследнегоФайлаРаздела = Неопределено;
			Если СтруктураПараметров.Свойство("ИД0000081", ИмяПоследнегоФайлаРаздела) И ИмяПоследнегоФайлаРаздела = ИмяФайлаРаздела Тогда
...
Функция ЗаполнитьТаблицуРаздела9(СтруктураПараметров, ФайлВыгрузки, ОписаниеОшибки = Неопределено)
	...
	Попытка
			...
			Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				Если ИмяУзла = "КнигаПрод" Тогда
					Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
					 ...
					 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
					 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
						СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС20 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС20 + ЧислоИзСтроки(АтрибутыУзла["СтПродБезНДС20"]);
						СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр20  = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр20  + ЧислоИзСтроки(АтрибутыУзла["СумНДСВсКПр20"]);
					КонецЕсли;
					...
					Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
						СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС7 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС7 + ЧислоИзСтроки(АтрибутыУзла["СтПродБезНДС7"]);
						СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС5 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС5 + ЧислоИзСтроки(АтрибутыУзла["СтПродБезНДС5"]);
					КонецЕсли;
					...
					Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
						СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр7  = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр7  + ЧислоИзСтроки(АтрибутыУзла["СумНДСВсКПр7"]);
						СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр5  = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр5  + ЧислоИзСтроки(АтрибутыУзла["СумНДСВсКПр5"]);
					КонецЕсли;
				...
				ИначеЕсли ИмяУзла = "СвРегНом"
				   ...
				   ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1") Тогда
					АтрУзла = Новый Структура("РегНомПросл,ОКЕИ,КолТовПросл,СтоимТовПросл",
						...
						ЧислоИзСтроки(АтрибутыУзла["КолТовПросл"]), ЧислоИзСтроки(АтрибутыУзла["СтоимТовПросл"]));
					ТекущиеДанные.СвРегНом.Добавить(АтрУзла);
				ИначеЕсли ИмяУзла = "КодВидТовар" Тогда
					ОбъектЧтениеXML.Прочитать();
					ТекущиеДанные.КодВидТовар.Добавить(ОбъектЧтениеXML.Значение);
				ИначеЕсли ИмяУзла = "КодВидОпер" Тогда
					ОбъектЧтениеXML.Прочитать();
					ТекущиеДанные.КодВидОпер.Добавить(ОбъектЧтениеXML.Значение);
		...
		Если СтруктураПараметров.ЗаполнитьДекларацию = Истина И ТекущиеДанные <> Неопределено Тогда
			ИмяПоследнегоФайлаРаздела = Неопределено;
			Если СтруктураПараметров.Свойство("ИД0000090", ИмяПоследнегоФайлаРаздела) И ИмяПоследнегоФайлаРаздела = ИмяФайлаРаздела Тогда
...
Функция ЗаполнитьТаблицуРаздела9_1(СтруктураПараметров, ФайлВыгрузки, ОписаниеОшибки = Неопределено)
	...
	Попытка
			...
			Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				Если ИмяУзла = "КнигаПродДЛ" Тогда
					Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
					 ...
					 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
					 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
						СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр20   = СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр20   + ЧислоИзСтроки(АтрибутыУзла["ИтСтПродКПр20"]);
						СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр20   = СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр20   + ЧислоИзСтроки(АтрибутыУзла["СумНДСИтКПр20"]);
						СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_20 = СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_20 + ЧислоИзСтроки(АтрибутыУзла["СтПродВсП1Р9_20"]);
						СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_20 = СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_20 + ЧислоИзСтроки(АтрибутыУзла["СумНДСВсП1Р9_20"]);
					КонецЕсли;
					...
					Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
						СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр7   = СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр7 + ЧислоИзСтроки(АтрибутыУзла["ИтСтПродКПр7"]);
						СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр5   = СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр5 + ЧислоИзСтроки(АтрибутыУзла["ИтСтПродКПр5"]);
					КонецЕсли;
					...
					Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
						СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр7   = СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр7 + ЧислоИзСтроки(АтрибутыУзла["СумНДСИтКПр7"]);
						СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр5   = СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр5 + ЧислоИзСтроки(АтрибутыУзла["СумНДСИтКПр5"]);
					КонецЕсли;
					...
					Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
						СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_7 = СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_7 + ЧислоИзСтроки(АтрибутыУзла["СтПродВсП1Р9_7"]);
						СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_5 = СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_5 + ЧислоИзСтроки(АтрибутыУзла["СтПродВсП1Р9_5"]);
					КонецЕсли;
					...
					Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
						СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_7 = СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_7 + ЧислоИзСтроки(АтрибутыУзла["СумНДСВсП1Р9_7"]);
						СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_5 = СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_5 + ЧислоИзСтроки(АтрибутыУзла["СумНДСВсП1Р9_5"]);
					КонецЕсли;
		...
		Если СтруктураПараметров.ЗаполнитьДекларацию = Истина И ТекущиеДанные <> Неопределено Тогда
			ИмяПоследнегоФайлаРаздела = Неопределено;
			Если СтруктураПараметров.Свойство("ИД0000091", ИмяПоследнегоФайлаРаздела) И ИмяПоследнегоФайлаРаздела = ИмяФайлаРаздела Тогда
...
Функция ЗаполнитьТаблицуРаздела10(СтруктураПараметров, ФайлВыгрузки, ОписаниеОшибки = Неопределено)
	...
	Попытка
			...
			Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				Если ИмяУзла = "ЖУчВыстСчФСтр" Тогда
					...
					Для Каждого АтрибутУзла Из АтрибутыУзла Цикл
						...
						КонецЕсли;
					КонецЦикла;
				ИначеЕсли ИмяУзла = "СвРегНом" И ТипЗнч(АтрУзлаПД) = Тип("Структура")
					И (СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
				   ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
				   ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
				   ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1") Тогда
					АтрУзла = Новый Структура("РегНомПросл,ОКЕИ,КолТовПросл,СтоимТовПросл",
						СокрЛП(АтрибутыУзла["РегНомПросл"]), СокрЛП(АтрибутыУзла["ОКЕИ"]),
						ЧислоИзСтроки(АтрибутыУзла["КолТовПросл"]), ЧислоИзСтроки(АтрибутыУзла["СтоимТовПросл"]));
					АтрУзлаПД.СвРегНом.Добавить(АтрУзла);
				ИначеЕсли ИмяУзла = "КодВидОпер" Тогда
					ОбъектЧтениеXML.Прочитать();
					ТекущиеДанные.КодВидОпер.Добавить(ОбъектЧтениеXML.Значение);
				ИначеЕсли ИмяУзла = "СвПосрДеят" ИЛИ ИмяУзла = "СвСчФОтПрод" Тогда
					...
					Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
					 ...
					 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
					 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
						АтрУзлаПД.Вставить("СвРегНом", Новый Массив);
					КонецЕсли;
					ТекущиеДанные[ИмяУзла].Добавить(АтрУзлаПД);
				ИначеЕсли ИмяУзла = "СведЮЛ" Тогда
					АтрУзла = Новый Структура("ИННЮЛ,КПП", СокрЛП(АтрибутыУзла["ИННЮЛ"]), СокрЛП(АтрибутыУзла["КПП"]));
					Если ПредИмяУзла = "СвПрод" Тогда
		...
		Если СтруктураПараметров.ЗаполнитьДекларацию = Истина И ТекущиеДанные <> Неопределено Тогда
			ИмяПоследнегоФайлаРаздела = Неопределено;
			Если СтруктураПараметров.Свойство("ИД0000100", ИмяПоследнегоФайлаРаздела) И ИмяПоследнегоФайлаРаздела = ИмяФайлаРаздела Тогда
...
Функция ЗаполнитьТаблицуРаздела11(СтруктураПараметров, ФайлВыгрузки, ОписаниеОшибки = Неопределено)
	...
	Попытка
		...
		Если СтруктураПараметров.ЗаполнитьДекларацию = Истина И ТекущиеДанные <> Неопределено Тогда
			ИмяПоследнегоФайлаРаздела = Неопределено;
			Если СтруктураПараметров.Свойство("ИД0000110", ИмяПоследнегоФайлаРаздела) И ИмяПоследнегоФайлаРаздела = ИмяФайлаРаздела Тогда
...
Функция ЗаполнитьТаблицуРаздела12(СтруктураПараметров, ФайлВыгрузки, ОписаниеОшибки = Неопределено)
	...
	Попытка
		...
		Если СтруктураПараметров.ЗаполнитьДекларацию = Истина И ТекущиеДанные <> Неопределено Тогда
			ИмяПоследнегоФайлаРаздела = Неопределено;
			Если СтруктураПараметров.Свойство("ИД0000120", ИмяПоследнегоФайлаРаздела) И ИмяПоследнегоФайлаРаздела = ИмяФайлаРаздела Тогда
...
Функция ВыгрузкаРаздел8(СтруктураПараметров)
	...
	КомпоновщикXML_ВывестиАтрибут(КомпоновщикXML, "ВерсФорм", ПараметрыВыгрузки.ВерсФорм);
	...
	Если ПараметрыВыгрузки.ВидДок = "0" ИЛИ ПараметрыВыгрузки["АктуальностьСведений" + ИмяРаздела] = "0" Тогда
		...
		Для Каждого СтрокаДанных Из Данные.Записи Цикл
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
			 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
			 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
				Для Инд = 0 По СтрокаДанных.РегНомТД.Количество() - 1 Цикл
					КомпоновщикXML_СоздатьУзел(КомпоновщикXML, "РегНомТД");
					КомпоновщикXML_ВывестиЗначениеИЗакрытьУзел(КомпоновщикXML, СтрокаДанных.РегНомТД[Инд]);
				КонецЦикла;
			ИначеЕсли СтруктураПараметров.ПравилаЗаполнения <> "2015Кв1" Тогда // "2021Кв3", "2022Кв1", "2023Кв1", "2025Кв1"
				
				ВыгрузитьСведенияОРегНомереДеклНаТоварыИлиОПрослеживаемомТоваре(КомпоновщикXML, СтрокаДанных);
	...
	ПрефиксРезультатаВыгрузки = "НДС2015";
	Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1" Тогда
		ПрефиксРезультатаВыгрузки = "НДС2017";
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2019Кв1" Тогда
		ПрефиксРезультатаВыгрузки = "НДС2019";
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
		ПрефиксРезультатаВыгрузки = "НДС2020";
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3" Тогда
		ПрефиксРезультатаВыгрузки = "НДС2021";
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2022Кв1" Тогда
		ПрефиксРезультатаВыгрузки = "НДС2022";
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2023Кв1" Тогда
		ПрефиксРезультатаВыгрузки = "НДС2023";
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ПрефиксРезультатаВыгрузки = "НДС2025";
	КонецЕсли;
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", ПрефиксРезультатаВыгрузки + ИмяРаздела);
...
Функция ВыгрузкаРаздел9(СтруктураПараметров)
	...
	Если ПараметрыВыгрузки.ВидДок = "0" ИЛИ ПараметрыВыгрузки["АктуальностьСведений" + ИмяРаздела] = "0" Тогда
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтПродБезНДС20", Данные.Итоги.СтПродБезНДС20, "Ч,19,2");
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтПродБезНДС7", Данные.Итоги.СтПродБезНДС7, "Ч,19,2");
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтПродБезНДС5", Данные.Итоги.СтПродБезНДС5, "Ч,19,2");
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСВсКПр20", Данные.Итоги.СумНДСВсКПр20, "Ч,19,2");
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСВсКПр7",  Данные.Итоги.СумНДСВсКПр7,  "Ч,19,2");
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСВсКПр5",  Данные.Итоги.СумНДСВсКПр5,  "Ч,19,2");
		КонецЕсли;
		...
		Для Каждого СтрокаДанных Из Данные.Записи Цикл
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
			 ...
			 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
			 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
				КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтоимПродСФ20", СтрокаДанных.СтоимПродСФ20, "Ч,19,2");
			КонецЕсли;
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
				КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтоимПродСФ7", СтрокаДанных.СтоимПродСФ7, "Ч,19,2");
				КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтоимПродСФ5", СтрокаДанных.СтоимПродСФ5, "Ч,19,2");
			КонецЕсли;
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
			 ...
			 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
				...
				Если ВыгружатьНулевойАтрибут_СумНДССФ20 Тогда
					КомпоновщикXML_ВывестиАтрибут(КомпоновщикXML, "СумНДССФ20", СтрокаДанных.СумНДССФ20, "Ч,19,2");
				Иначе
					КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДССФ20", СтрокаДанных.СумНДССФ20, "Ч,19,2");
				КонецЕсли;
			КонецЕсли;
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
				...
				Если ВыгружатьНулевойАтрибут_СумНДССФ7 Тогда
					КомпоновщикXML_ВывестиАтрибут(КомпоновщикXML, "СумНДССФ7", СтрокаДанных.СумНДССФ7, "Ч,19,2");
				Иначе
					КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДССФ7", СтрокаДанных.СумНДССФ7, "Ч,19,2");
				КонецЕсли;
				...
				Если ВыгружатьНулевойАтрибут_СумНДССФ5 Тогда
					КомпоновщикXML_ВывестиАтрибут(КомпоновщикXML, "СумНДССФ5", СтрокаДанных.СумНДССФ5, "Ч,19,2");
				Иначе
					КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДССФ5", СтрокаДанных.СумНДССФ5, "Ч,19,2");
				КонецЕсли;
			КонецЕсли;
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
			 ...
			 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
				Для Инд = 0 По СтрокаДанных.РегНомТД.Количество() - 1 Цикл
					...
					КомпоновщикXML_ВывестиЗначениеИЗакрытьУзел(КомпоновщикXML, СтрокаДанных.РегНомТД[Инд]);
				КонецЦикла;
			ИначеЕсли СтруктураПараметров.ПравилаЗаполнения <> "2015Кв1" Тогда // "2021Кв3", "2022Кв1", "2023Кв1", "2023Кв1"
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
			 ...
			 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
				Для Инд = 0 По СтрокаДанных.КодВидТовар.Количество() - 1 Цикл
					КомпоновщикXML_СоздатьУзел(КомпоновщикXML, "КодВидТовар");
					КомпоновщикXML_ВывестиЗначениеИЗакрытьУзел(КомпоновщикXML, СтрокаДанных.КодВидТовар[Инд]);
				КонецЦикла;
			КонецЕсли;
...
Функция ВыгрузкаРаздел9_Прил1(СтруктураПараметров)
	...
	Если ПараметрыВыгрузки.ВидДок = "0" ИЛИ ПараметрыВыгрузки["АктуальностьСведений" + ИмяРаздела] = "0" Тогда
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "ИтСтПродКПр20",   Данные.Итоги.ИтСтПродКПр20, "Ч,19,2");
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "ИтСтПродКПр7",   Данные.Итоги.ИтСтПродКПр7, "Ч,19,2");
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "ИтСтПродКПр5",   Данные.Итоги.ИтСтПродКПр5, "Ч,19,2");
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСИтКПр20",   Данные.Итоги.СумНДСИтКПр20, "Ч,19,2");
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСИтКПр7",   Данные.Итоги.СумНДСИтКПр7,  "Ч,19,2");
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСИтКПр5",   Данные.Итоги.СумНДСИтКПр5,  "Ч,19,2");
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтПродВсП1Р9_20",   Данные.Итоги.СтПродВсП1Р9_20, "Ч,19,2");
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтПродВсП1Р9_7", Данные.Итоги.СтПродВсП1Р9_7, "Ч,19,2");
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтПродВсП1Р9_5", Данные.Итоги.СтПродВсП1Р9_5, "Ч,19,2");
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСВсП1Р9_20",   Данные.Итоги.СумНДСВсП1Р9_20, "Ч,19,2");
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСВсП1Р9_7", Данные.Итоги.СумНДСВсП1Р9_7,  "Ч,19,2");
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСВсП1Р9_5", Данные.Итоги.СумНДСВсП1Р9_5,  "Ч,19,2");
		КонецЕсли;
	...
	Если ПараметрыВыгрузки.ВидДок = "0" ИЛИ ПараметрыВыгрузки["АктуальностьСведений" + ИмяРаздела] = "0" Тогда
		...
		Для Каждого СтрокаДанных Из Данные.Записи Цикл
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
			 ...
			 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
			 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
				ИмяУзлаСв = "СвСчФОтПрод";
			КонецЕсли;
			
			Для Инд = 0 По СтрокаДанных[ИмяУзлаСв].Количество() - 1 Цикл
				ДанныеОДеятельности = СтрокаДанных[ИмяУзлаСв][Инд];
				
				КомпоновщикXML_СоздатьУзел(КомпоновщикXML, ИмяУзлаСв);
				...
				Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
				 ...
				 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
					...
					ВыгрузитьСведенияОРегНомереДеклНаТоварыИлиОПрослеживаемомТоваре(КомпоновщикXML, ДанныеОДеятельности);
...
&НаСервере
Процедура ВыгрузитьСведенияОРегНомереДеклНаТоварыИлиОПрослеживаемомТоваре(КомпоновщикXML, СтрокаДанных)
	
	Для Инд = 0 По СтрокаДанных.СвРегНом.Количество() - 1 Цикл
		
		КомпоновщикXML_СоздатьУзел(КомпоновщикXML, "СвРегНом");
		
		КомпоновщикXML_ВывестиАтрибут(КомпоновщикXML,
			"РегНомПросл", СтрокаДанных.СвРегНом[Инд].РегНомПросл, "Т,29");
		КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML,
...
Процедура ЗаполнитьДаннымиУзелИзДокумента(Узел, ОбъектDOM, СуммироватьЧисла = Ложь)
	...
	Для Каждого Стр Из Узел.Строки Цикл
		...
		Иначе
			Если ПустаяСтрока(Стр.ЗначениеПоУмолчанию) Тогда
				ЗначениеОбъекта = "";
				Если Стр.Тип = "П" Тогда
					ДочерниеОбъектыDOM = ОбъектDOM.ПолучитьЭлементыПоИмени(Стр.Код);
					Если ДочерниеОбъектыDOM.Количество() > 0 Тогда
						ЗначениеОбъекта = ДочерниеОбъектыDOM[0].ТекстовоеСодержимое;
					КонецЕсли;
				Иначе
					ЗначениеОбъекта = ОбъектDOM.ПолучитьАтрибут(Стр.Код);
				КонецЕсли;
				
				Если СуммироватьЧисла И Стр.Формат = "N" Тогда
					НовоеЗначениеАтрибута = ЧислоИзСтроки(Стр.Значение) + ЧислоИзСтроки(ЗначениеОбъекта);
					ВывестиПоказательВXML(Стр, НовоеЗначениеАтрибута);
				Иначе
					Стр.Значение = СокрЛП(ЗначениеОбъекта);
				КонецЕсли;
			ИначеЕсли Лев(Стр.ЗначениеПоУмолчанию, 1) = "&" Тогда
				Ключ = Сред(Стр.ЗначениеПоУмолчанию, 2);
				Если РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, Ключ) Тогда
...
Процедура ПостобработкаДереваВыгрузки(ДеревоВыгрузки)
	...
	Для Каждого УзелСведНалГод Из УзлыСведНалГод Цикл
		...
		Если УзелДатаИсп170 = Неопределено ИЛИ УзелПуст(УзелДатаИсп170) Тогда
			...
			Если УзелДоляНеОбл <> Неопределено И УзелПуст(УзелДоляНеОбл) Тогда
				УзелСведНалГод.Строки.Удалить(УзелДоляНеОбл);
			КонецЕсли;
			
			УзелНалГод = УзелСведНалГод.Строки.Найти("НалГод", "Код");
			Если УзелНалГод <> Неопределено И УзелПуст(УзелНалГод) Тогда
	...
	УзелСумПУ_173_5 = УзелСумПУ_173_1.Родитель.Строки.Найти("СумПУ_173.5", "Код");
	УзелОКТМО       = УзелСумПУ_173_1.Родитель.Строки.Найти("ОКТМО", "Код");
	
	Если ПараметрыВыгрузки.ПравилаЗаполнения = "2015Кв1" Тогда
		
		Если УзелПуст(УзелСумПУ_173_1) Тогда
			// если узел пуст и не выполняются условия его присутствия, то удаляем узел
			Если УзелСумПУ_173_5 <> Неопределено ИЛИ УзелОКТМО = Неопределено Тогда
				Если УзелНДС.Строки.Найти("СумУпл164", "Код") = Неопределено
					...
					РегламентированнаяОтчетность.УдалитьУзел(УзелСумПУ_173_1);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2017Кв1"
		  ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2019Кв1"
		  ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2020Кв4"
		  ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2021Кв3"
		  ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2022Кв1"
		  ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2023Кв1"
		  ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2025Кв1" Тогда
		
		Если УзелОКТМО <> Неопределено И ЗначениеЗаполнено(УзелОКТМО.Значение) Тогда
			Если НЕ УзелПуст(УзелСумПУ_173_5) И УзелПуст(УзелСумПУ_173_1) Тогда
				РегламентированнаяОтчетность.УдалитьУзел(УзелСумПУ_173_1);
			ИначеЕсли УзелПуст(УзелСумПУ_173_5) Тогда
				РегламентированнаяОтчетность.УдалитьУзел(УзелСумПУ_173_5);
			КонецЕсли;
		Иначе
			РегламентированнаяОтчетность.УдалитьУзел(УзелСумПУ_173_5);
			РегламентированнаяОтчетность.УдалитьУзел(УзелСумПУ_173_1);
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, ДополнительныеПараметры = Неопределено)
	...
	Для Каждого ДокументРазделов Из ДокументыРазделов Цикл
		...
		Если ДокУзлыСумУплНП.Количество() > 0 Тогда
			...
			Если ПараметрыВыгрузки.ПравилаЗаполнения = "2022Кв1"
			 ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2023Кв1"
			 ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2025Кв1" Тогда
				УстановитьАтрибутСуммаЗначений(УзелСумУплНП, ДокУзлыСумУплНП[0], "СумЗВ_176.1");
				
				УзелКодОснШаблон = ПолучитьПодчиненныйЭлемент(УзелСумУплНП, "КодОсн");
				ДокУзлыКодОсн = ДокУзлыСумУплНП[0].ПолучитьЭлементыПоИмени(УзелКодОснШаблон.Код);
				Для Каждого ДокУзелКодОсн Из ДокУзлыКодОсн Цикл
					УзлыКодОсн = УзелСумУплНП.Строки.НайтиСтроки(Новый Структура("Код,Значение", "КодОсн", ДокУзелКодОсн.ТекстовоеСодержимое));
					Если УзлыКодОсн.Количество() > 0 Тогда
						УзелКодОсн = УзлыКодОсн[0];
					Иначе
						УзелКодОсн = СкопироватьУзел(УзелСумУплНП, УзелКодОснШаблон);
					КонецЕсли;
					УзелКодОсн.Значение = ДокУзелКодОсн.ТекстовоеСодержимое;
				КонецЦикла;
			КонецЕсли;
			...
			Если ПараметрыВыгрузки.ПравилаЗаполнения = "2020Кв4"
			 ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2021Кв3"
			 ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2022Кв1"
			 ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2023Кв1"
			 ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2025Кв1" Тогда
				УстановитьАтрибутЕслиНеЗаполнен(УзелСумУплНП, ДокУзлыСумУплНП[0], "ПризнСЗПК");
				УстановитьАтрибутСуммаЗначений(УзелСумУплНП, ДокУзлыСумУплНП[0], "СумПУ_СЗПК");
			КонецЕсли;
		КонецЕсли;
		...
		Для Каждого ДокУзелСумУплНА Из ДокУзлыСумУплНА Цикл
			...
			Если ПараметрыВыгрузки.ПравилаЗаполнения = "2019Кв1"
			 ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2020Кв4"
			 ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2021Кв3"
			 ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2022Кв1"
			 ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2023Кв1"
			 ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2025Кв1" Тогда
				...
				АтрибутКодОпер = ДокУзелСумУплНА.ПолучитьАтрибут("КодОпер");
				Если АтрибутКодОпер = "1011715" Тогда
					РегламентированнаяОтчетность.УдалитьУзел(ПолучитьПодчиненныйЭлемент(УзелСумУплНА, "КППИно"));
		...
		Если ДокУзлыУзелНалПодтв0.Количество() > 0 Тогда
			...
			ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2017Кв1"
				...
				ДокУзлыКорНБВозврТов = ДокУзлыУзелНалПодтв0[0].ПолучитьЭлементыПоИмени(УзелКорНБВозврТовШаблон.Код);
				Для Каждого ДокУзелКорНБВозврТов Из ДокУзлыКорНБВозврТов Цикл
					АтрибутКодОпер = ДокУзелКорНБВозврТов.ПолучитьАтрибут("КодОпер");
					НайденУзел = Ложь;
					Если ЗначениеЗаполнено(АтрибутКодОпер) Тогда
						УзлыКодОпер = УзелНалПодтв0.Строки.НайтиСтроки(Новый Структура("Код,Значение", "КодОпер", АтрибутКодОпер), Истина);
						Для Каждого УзелКодОпер Из УзлыКодОпер Цикл
							Если УзелКодОпер.Родитель.Код = "КорНБВозврТов" Тогда
								УзелКорНБВозврТов = УзелКодОпер.Родитель;
								НайденУзел = Истина;
								Прервать;
							КонецЕсли;
						КонецЦикла;
					...
					КонецЕсли;
					ЗаполнитьДаннымиУзелИзДокумента(УзелКорНБВозврТов, ДокУзелКорНБВозврТов, НайденУзел);
				КонецЦикла;
				
				УзелКорНБИзмЦенШаблон = ПолучитьПодчиненныйЭлемент(УзелНалПодтв0, "КорНБИзмЦен");
				ДокУзлыКорНБИзмЦен = ДокУзлыУзелНалПодтв0[0].ПолучитьЭлементыПоИмени(УзелКорНБИзмЦенШаблон.Код);
				Для Каждого ДокУзелКорНБИзмЦен Из ДокУзлыКорНБИзмЦен Цикл
					АтрибутКодОпер = ДокУзелКорНБИзмЦен.ПолучитьАтрибут("КодОпер");
					НайденУзел = Ложь;
					Если ЗначениеЗаполнено(АтрибутКодОпер) Тогда
						УзлыКодОпер = УзелНалПодтв0.Строки.НайтиСтроки(Новый Структура("Код,Значение", "КодОпер", АтрибутКодОпер), Истина);
						Для Каждого УзелКодОпер Из УзлыКодОпер Цикл
							Если УзелКодОпер.Родитель.Код = "ДокУзелКорНБИзмЦен" Тогда
								УзелДокУзелКорНБИзмЦен = УзелКодОпер.Родитель;
								НайденУзел = Истина;
								Прервать;
							КонецЕсли;
						КонецЦикла;
					КонецЕсли;
					Если Не НайденУзел Тогда
						УзелДокУзелКорНБИзмЦен = СкопироватьУзел(УзелНалПодтв0, УзелКорНБИзмЦенШаблон);
					КонецЕсли;
					ЗаполнитьДаннымиУзелИзДокумента(УзелДокУзелКорНБИзмЦен, ДокУзелКорНБИзмЦен, НайденУзел);
				КонецЦикла;
		...
		Если ДокУзлыНалНеПодтв0.Количество() > 0 Тогда
			...
			ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2017Кв1"
				...
				ДокУзлыКорНалВозврТов = ДокУзлыНалНеПодтв0[0].ПолучитьЭлементыПоИмени(УзелКорНалВозврТовШаблон.Код);
				Для Каждого ДокУзелКорНалВозврТов Из ДокУзлыКорНалВозврТов Цикл
					АтрибутКодОпер = ДокУзелСумОпер.ПолучитьАтрибут("КодОпер");
					НайденУзел = Ложь;
					Если ЗначениеЗаполнено(АтрибутКодОпер) Тогда
						УзлыКодОпер = УзелНалНеПодтв0.Строки.НайтиСтроки(Новый Структура("Код,Значение", "КодОпер", АтрибутКодОпер), Истина);
						Для Каждого УзелКодОпер Из УзлыКодОпер Цикл
							Если УзелКодОпер.Родитель.Код = "КорНалВозврТов" Тогда
								УзелКорНалВозврТов = УзелКодОпер.Родитель;
								НайденУзел = Истина;
								Прервать;
							КонецЕсли;
						КонецЦикла;
					...
					КонецЕсли;
					ЗаполнитьДаннымиУзелИзДокумента(УзелКорНалВозврТов, ДокУзелКорНалВозврТов, НайденУзел);
				КонецЦикла;
				
				УзелКорНалИзмЦенШаблон = ПолучитьПодчиненныйЭлемент(УзелНалНеПодтв0, "КорНалИзмЦен");
				ДокУзлыКорНалИзмЦен = ДокУзлыНалНеПодтв0[0].ПолучитьЭлементыПоИмени(УзелКорНалИзмЦенШаблон.Код);
				Для Каждого ДокУзелКорНалИзмЦен Из ДокУзлыКорНалИзмЦен Цикл
					АтрибутКодОпер = ДокУзелКорНалИзмЦен.ПолучитьАтрибут("КодОпер");
					НайденУзел = Ложь;
					Если ЗначениеЗаполнено(АтрибутКодОпер) Тогда
						УзлыКодОпер = УзелНалНеПодтв0.Строки.НайтиСтроки(Новый Структура("Код,Значение", "КодОпер", АтрибутКодОпер), Истина);
						Для Каждого УзелКодОпер Из УзлыКодОпер Цикл
							Если УзелКодОпер.Родитель.Код = "КорНалИзмЦен" Тогда
								УзелКорНалИзмЦен = УзелКодОпер.Родитель;
								НайденУзел = Истина;
								Прервать;
							КонецЕсли;
						КонецЦикла;
					...
					КонецЕсли;
					ЗаполнитьДаннымиУзелИзДокумента(УзелКорНалИзмЦен, ДокУзелКорНалИзмЦен, НайденУзел);
				КонецЦикла;
...
Функция ИзвлечьСтруктуруXML()
	...
	Если ПараметрыВыгрузки.ПравилаЗаполнения = "2017Кв1" Тогда
		ИмяМакетаСтруктурыВыгрузки = "СхемаВыгрузки_NO_NDS_1_003_00_05_05_01";
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2019Кв1" Тогда
		ИмяМакетаСтруктурыВыгрузки = "СхемаВыгрузки_NO_NDS_1_003_00_05_06_01";
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2020Кв4" Тогда
		ИмяМакетаСтруктурыВыгрузки = "СхемаВыгрузки_NO_NDS_1_003_00_05_07_01";
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2021Кв3" Тогда
		ИмяМакетаСтруктурыВыгрузки = "СхемаВыгрузки_NO_NDS_1_003_00_05_08_01";
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2022Кв1" Тогда
		ИмяМакетаСтруктурыВыгрузки = "СхемаВыгрузки_NO_NDS_1_003_00_05_09_01";
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2023Кв1" Тогда
		ИмяМакетаСтруктурыВыгрузки = "СхемаВыгрузки_NO_NDS_1_003_00_05_10_01";
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2025Кв1" Тогда
		ИмяМакетаСтруктурыВыгрузки = "СхемаВыгрузки_NO_NDS_1_003_00_05_11_01";
	КонецЕсли;
...
&НаСервере
Процедура ЗагрузитьВДокументАвтоматически(Отказ, ПараметрыФормы)
	
	ДанныеДляЗагрузки = Неопределено;
	ПараметрыФормы.Свойство("ДанныеДляЗагрузки", ДанныеДляЗагрузки);
	Если НЕ ЗначениеЗаполнено(ДанныеДляЗагрузки) ИЛИ НЕ ДанныеДляЗагрузки.Свойство("СвойстваФайлов") Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	ПараметрыВыгрузки.Вставить("ВидДокумента", ДанныеДляЗагрузки.Версия);
	
	ДанныеПомещенныхФайлов = Новый Массив;
	Для Каждого СвойстваФайла Из ДанныеДляЗагрузки.СвойстваФайлов Цикл
		ДанныеПомещенныхФайлов.Добавить(Новый Структура("Адрес,ИмяФайла", СвойстваФайла.АдресДанных, СвойстваФайла.ИмяФайла));
	КонецЦикла;
	
	Контекст = Новый Структура;
	Контекст.Вставить("ЗаменитьСтроку",  Ложь);
	Контекст.Вставить("ПомещенныеФайлы", ДанныеПомещенныхФайлов);
	Контекст.Вставить("ОшибкаНаСервере", Новый Структура);
	
	Если НЕ ДобавитьСтрокиНаСервере(Контекст) Тогда
		ПоказатьОшибку(Контекст.ОшибкаНаСервере);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	ОшибкаНаСервере = Новый Структура;
	ТекстВыгрузки   = Неопределено;
	Успешно = КонсолидироватьНаСервере(ТекстВыгрузки, ОшибкаНаСервере, Истина, Ложь, ПараметрыФормы.СохраненныйДок);
	
	Если НЕ Успешно Тогда
		УдалитьДополнительныеФайлыДекларацииНДС(ПараметрыФормы.СохраненныйДок);
		ПоказатьОшибку(ОшибкаНаСервере);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	СохраненныйДокОбъект = ПараметрыФормы.СохраненныйДок.ПолучитьОбъект();
	Если ЗначениеЗаполнено(ПараметрыФормы.Комментарий) Тогда
		СохраненныйДокОбъект.Комментарий = ПараметрыФормы.Комментарий;
	КонецЕсли;
	СохраненныйДокОбъект.КодНалоговогоОргана = ПараметрыВыгрузки.КодИФНС;
	СохраненныйДокОбъект.КПП                 = ПараметрыВыгрузки.КППОрг;
	СохраненныйДокОбъект.Вид                 = ДанныеДляЗагрузки.Версия;
	СохраненныйДокОбъект.ПредставлениеВида   = РегламентированнаяОтчетность.ПредставлениеВидаДокумента(СохраненныйДокОбъект.Вид);
	
	Попытка
		СохраненныйДокОбъект.Записать();
	Исключение
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Ошибка записи заполненного документа отчета: %1.';
										|en = 'An error occurred while writing the filled report document: %1.'"),
...
&НаКлиенте
Процедура ОткрытьФормуЗаполненногоОтчета(СохраненныйДок = Неопределено) Экспорт
	
	ФормаОтчета = ПолучитьФормуОтчетаИзДокумента(СохраненныйДок);
	ФормаОтчета.Открыть();
	ФормаОтчета.СтруктураРеквизитовФормы.ОтчетСформированОбработкойКонсолидацияОтчетностиПоНДС = Истина;
...
Функция ПолучитьФормуОтчетаНаКлиенте(ПараметрыФормыОтчета)
	...
	ИначеЕсли ПараметрыФормыОтчета = "Открыть внутренний отчет-документ" Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Заполнение отчета-документа не предусмотрено!';
										|en = 'Report document cannot be filled in.'"));
		Возврат Неопределено;
	...
	Иначе
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Недостаточно параметров для создания отчета!';
										|en = 'Not enough parameters to create the report.'"));
		Возврат Неопределено;
	...
	Если ОсновнаяФормаОтчета = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Не удалось получить основную форму отчета!';
										|en = 'Cannot receive the main report form.'"));
		Возврат Неопределено;
		
	КонецЕсли;
	
	СтрВыбраннаяФорма = ОсновнаяФормаОтчета.мВыбраннаяФорма;
	Если СтрВыбраннаяФорма = "ФормаОтчета2017Кв1"
	   И ОсновнаяФормаОтчета.РезультирующаяТаблица.Количество() > 1
	   И ПараметрыФормыОтчета.мДатаКонцаПериодаОтчета >= '20190101' Тогда
		СвойстваФормы = ОсновнаяФормаОтчета.РезультирующаяТаблица.Получить(
			ОсновнаяФормаОтчета.РезультирующаяТаблица.Количество() - 1);
		СтрВыбраннаяФорма = СвойстваФормы.ФормаОтчета;
	КонецЕсли;
	
	ОсновнаяФормаОтчета = Неопределено;
	
	Если СтрВыбраннаяФорма <> "ФормаОтчета2015Кв1"
	   И СтрВыбраннаяФорма <> "ФормаОтчета2017Кв1"
	   И СтрВыбраннаяФорма <> "ФормаОтчета2019Кв1"
	   И СтрВыбраннаяФорма <> "ФормаОтчета2020Кв4"
	   И СтрВыбраннаяФорма <> "ФормаОтчета2021Кв3"
	   И СтрВыбраннаяФорма <> "ФормаОтчета2022Кв1"
	   И СтрВыбраннаяФорма <> "ФормаОтчета2023Кв1"
	   И СтрВыбраннаяФорма <> "ФормаОтчета2025Кв1" Тогда
		
		ПоказатьПредупреждение(, НСтр("ru = 'Консолидация отчетов по НДС реализована только для форм:
			|""Приложение № 1 к приказу ФНС России от 29 октября 2014 г. № ММВ-7-3/558@"",
			|""Приложение № 1 к приказу ФНС России от 29 октября 2014 г. № ММВ-7-3/558@
			|(в редакции приказа ФНС России от 20.12.2016 № ММВ-7-3/696@),
			|(в редакции приказа ФНС России от 28.12.2018 № СА-7-3/853@),
			|(в редакции приказа ФНС России от 19.08.2020 № ЕД-7-3/591@),
			|(в редакции приказа ФНС России от 26.03.2021 № ЕД-7-3/228@),
			|(в редакции приказа ФНС России от 24.12.2021 № ЕД-7-3/1149@),
			|(в редакции приказа ФНС России от 12.12.2022 № ЕД-7-3/1191@)"",
			|""Приложение № 1 к приказу ФНС России от 5 ноября 2024 г. № ЕД-7-3/989@.""!';
			|en = 'Консолидация отчетов по НДС реализована только для форм:
			|""Приложение № 1 к приказу ФНС России от 29 октября 2014 г. № ММВ-7-3/558@"",
			|""Приложение № 1 к приказу ФНС России от 29 октября 2014 г. № ММВ-7-3/558@
			|(в редакции приказа ФНС России от 20.12.2016 № ММВ-7-3/696@),
			|(в редакции приказа ФНС России от 28.12.2018 № СА-7-3/853@),
	...
	Если ВыбраннаяФормаОтчета = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Не удалось получить форму отчета по периоду!';
										|en = 'Cannot receive report form by period.'"));
		Возврат Неопределено;
	...
	Исключение
		...
		Если ИнформацияОбОшибке.Причина = Неопределено Тогда
			Сообщение.Текст = НСтр("ru = 'При открытии формы регламентированного отчета произошла ошибка!';
									|en = 'An error occurred when opening a local report form.'");
		ИначеЕсли Найти(ИнформацияОбОшибке.Причина.Описание, ".Форма.") > 0 Тогда
			Сообщение.Текст = НСтр("ru = 'Отчет не может быть открыт! Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.';
									|en = 'Cannot open report. Old version of the report form is not supported by the current configuration version.'");
		Иначе
...
Функция ПолучитьПараметрыОтчетаИзДокументаНаСервере(СохраненныйДок)
	...
	Если ОбъектОтчет = Неопределено Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось открыть сохраненные данные! Отчет не найден!';
								|en = 'Cannot open saved data. Report not found.'");
		Сообщение.Сообщить();
		...
		Для Каждого Раздел Из ПоказателиОтчета Цикл
			...
			Если ИмяРаздела = "Титульный" Тогда
				...
				Если ВидДокумента = Неопределено Тогда
					ВидДокумента = ЧислоИзСтроки(ПараметрыВыгрузки.ВидДок);
				КонецЕсли;
				ДанныеРаздела.НомерКорректировки = ВидДокумента;
				...
				Если ПараметрыВыгрузки.ПравилаЗаполнения <> "2020Кв4"
				   И ПараметрыВыгрузки.ПравилаЗаполнения <> "2021Кв3"
				   И ПараметрыВыгрузки.ПравилаЗаполнения <> "2022Кв1"
				   И ПараметрыВыгрузки.ПравилаЗаполнения <> "2023Кв1"
				   И ПараметрыВыгрузки.ПравилаЗаполнения <> "2025Кв1" Тогда
					Если ДанныеРаздела.Свойство("ОКВЭД") Тогда
				...
				Если ДанныеРаздела.Свойство("ОргУп") И ПараметрыВыгрузки.Свойство("НаимОргПредставителя") Тогда
			...
			Иначе
				...
				Для Каждого УзелРаздела Из УзлыРаздела Цикл
					Если ЗначениеЗаполнено(УзелРаздела.Ключ) И (УзелРаздела.Ключ <> "П0001М1005503") Тогда
						...
						ИначеЕсли Ключ = "П000600016003" Тогда
							Если Лев(Значение, 1) = "-" Тогда
								...
								Значение = Сред(Значение, 2);
							КонецЕсли;
						ИначеЕсли Ключ = "П000100009003" Тогда
							Если Лев(Значение, 1) = "-" Тогда
								Ключ = "П000100009503";
								Значение = Сред(Значение, 2);
							КонецЕсли;
				...
				Если УзлыСтрок.Количество() > 0 И ТипЗнч(ДанныеМногострочныхРазделов) = Тип("Структура") Тогда
					...
					// Распределяем строки многострочных разделов по группам.
					СтруктураСтрокРаздела = Новый Структура;
					Для Каждого УзелСтроки Из УзлыСтрок Цикл
						Если СтруктураСтрокРаздела.Свойство(УзелСтроки.Код) Тогда
							СтруктураСтрокРаздела[УзелСтроки.Код].Добавить(УзелСтроки);
						Иначе
							ЭлементСтроки = Новый Массив;
							ЭлементСтроки.Добавить(УзелСтроки);
							СтруктураСтрокРаздела.Вставить(УзелСтроки.Код, ЭлементСтроки);
						КонецЕсли;
					КонецЦикла;
					
					Для Каждого ЭлементСтруктуры Из СтруктураСтрокРаздела Цикл
						УзлыСтрокРаздела = ЭлементСтруктуры.Значение;
						
						Для Каждого ГруппаСтрокРаздела Из ДанныеМногострочныхРазделов Цикл
							ГруппаНайдена = Ложь;
							
							Если УзлыСтрокРаздела.Количество() = 0 Тогда
								Прервать;
							КонецЕсли;
							
							УзелСтрокиРаздела = УзлыСтрокРаздела[0];
							Если ЗначениеЗаполнено(УзелСтрокиРаздела.Ключ) Тогда
								ПоказателиСтрокиРаздела = Новый Массив;
								ПоказателиСтрокиРаздела.Добавить(УзелСтрокиРаздела);
							Иначе
								ПоказателиСтрокиРаздела = УзелСтрокиРаздела.Строки;
							КонецЕсли;
							
							Для Каждого УзелСтроки Из ПоказателиСтрокиРаздела Цикл
								ДлинаКлючаГруппы = СтрДлина(ГруппаСтрокРаздела.Ключ);
								Если ВРег(Лев(ГруппаСтрокРаздела.Ключ, ДлинаКлючаГруппы - 1)) = ВРег(Лев(УзелСтроки.Ключ, ДлинаКлючаГруппы - 1)) Тогда
						...
						Если ГруппаСтрок <> Неопределено Тогда
							ИндексСтроки = 0;
							Для Каждого УзелСтроки Из УзлыСтрокРаздела Цикл
								
								СтрокаТаблицы = ?(ГруппаСтрок.Количество() > ИндексСтроки, ГруппаСтрок[ИндексСтроки], ГруппаСтрок.Добавить());
								
								ПоказателиУзлаСтроки = УзелСтроки.Строки;
								Если ЗначениеЗаполнено(УзелСтроки.Ключ) Тогда
									ПоказателиУзлаСтроки = Новый Массив;
									ПоказателиУзлаСтроки.Добавить(УзелСтроки);
								КонецЕсли;
								
								Для Каждого УзелПоказатель Из ПоказателиУзлаСтроки Цикл
				...
				Для Каждого УзелСтраницыРаздела Из УзлыСтраницРаздела Цикл
					...
					Если УзлыСтрок.Количество() > 0 И ТипЗнч(ДанныеМногострочныхРазделов) = Тип("Структура") Тогда
						...
						Для Каждого ЭлементСтруктуры Из СтруктураСтрокРаздела Цикл
							...
							Для Каждого ГруппаСтрокРаздела Из ДанныеМногострочныхРазделов Цикл
								ГруппаНайдена = Ложь;
								Для Каждого УзелСтроки Из УзлыСтрокРаздела[0].Строки Цикл
									ДлинаКлючаГруппы = СтрДлина(ГруппаСтрокРаздела.Ключ);
									Если ВРег(Лев(ГруппаСтрокРаздела.Ключ, ДлинаКлючаГруппы - 1)) = ВРег(Лев(УзелСтроки.Ключ, ДлинаКлючаГруппы - 1)) Тогда
										ГруппаНайдена = Истина;
										Прервать;
							...
							Если ГруппаСтрок <> Неопределено Тогда
								...
								Если ИмяРаздела = "Раздел3_Прил1" Тогда
									...
									ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2017Кв1"
										  ...
										  ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2023Кв1"
										  ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2025Кв1" Тогда
										ДанныеСтраницы.Вставить("П000310008004", ГруппаСтрок.Итог("П000310007004"));
									КонецЕсли;
								КонецЕсли;
							КонецЕсли;
						КонецЦикла;
					...
					Если СтраницыРаздела.Колонки.Найти("ДанныеДопСтрок") <> Неопределено Тогда
						Если ГруппаСтрок <> Неопределено Тогда
							ТекущаяСтраница.ДанныеДопСтрок = ГруппаСтрок;
						Иначе
							ТекущаяСтраница.ДанныеДопСтрок = Новый Структура;
						КонецЕсли;
					КонецЕсли;
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела8(СтрокаРаздела8, СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП = 0 Тогда
		...
		ИмяМакета = "КнигаПокупок735";
		Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
			ИмяМакета = "КнигаПокупок1137_2017";
		ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			ИмяМакета = "КнигаПокупок534";
			ПараметрыРаздела.Вставить("СекцияСтрокаРНПТ");
		КонецЕсли;
		ПараметрыРаздела.Вставить("Макет", РеквизитФормыВЗначение("Объект").ПолучитьМакет(ИмяМакета));
		Если ПараметрыРаздела.Свойство("СекцияСтрокаРНПТ") Тогда
			ПараметрыРаздела.СекцияСтрокаРНПТ = ПараметрыРаздела.Макет.ПолучитьОбласть("СтрокаРНПТ");
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
			ПараметрыРаздела.СтрокиДляПовтора.Область("НомераГрафКнига").Видимость = Ложь;
		ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			ПараметрыРаздела.СтрокиДляПовтора.Область("НомераГрафКнига").Видимость = Ложь;
			ПараметрыРаздела.СтрокиДляПовтора.Область("НомераГрафДекларация81").Видимость = Ложь;
		КонецЕсли;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
	 ...
	 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		СтрОт = НСтр("ru = ' от ';
					|en = ' from'");
		ПараметрыСтроки.НомерОтДата =
			?(ЗначениеЗаполнено(СтрокаРаздела8.НомСчФПрод), "" + СтрокаРаздела8.НомСчФПрод + СтрОт, "")
			+ СтрокаРаздела8.ДатаСчФПрод;
		ПараметрыСтроки.НомерОтДатаИсправленный =
			?(ЗначениеЗаполнено(СтрокаРаздела8.НомИспрСчФ), "" + СтрокаРаздела8.НомИспрСчФ + СтрОт, "")
			+ СтрокаРаздела8.ДатаИспрСчФ;
		ПараметрыСтроки.НомерОтДатаКорректировочный =
			?(ЗначениеЗаполнено(СтрокаРаздела8.НомКСчФПрод), "" + СтрокаРаздела8.НомКСчФПрод + СтрОт, "")
			+ СтрокаРаздела8.ДатаКСчФПрод;
		ПараметрыСтроки.НомерОтДатаИсправленныйКорректировочный =
			?(ЗначениеЗаполнено(СтрокаРаздела8.НомИспрКСчФ), "" + СтрокаРаздела8.НомИспрКСчФ + СтрОт, "")
			+ СтрокаРаздела8.ДатаИспрКСчФ;
	Иначе
		ПараметрыСтроки.НомерДата =
			?(ЗначениеЗаполнено(СтрокаРаздела8.НомСчФПрод), "" + СтрокаРаздела8.НомСчФПрод + ";", "")
			+ СтрокаРаздела8.ДатаСчФПрод;
		ПараметрыСтроки.НомерДатаИсправления =
			?(ЗначениеЗаполнено(СтрокаРаздела8.НомИспрСчФ), "" + СтрокаРаздела8.НомИспрСчФ + ";", "")
			+ СтрокаРаздела8.ДатаИспрСчФ;
		ПараметрыСтроки.НомерДатаКорректировки =
			?(ЗначениеЗаполнено(СтрокаРаздела8.НомКСчФПрод), "" + СтрокаРаздела8.НомКСчФПрод + ";", "")
			+ СтрокаРаздела8.ДатаКСчФПрод;
		ПараметрыСтроки.НомерДатаИсправленияКорректировки =
			?(ЗначениеЗаполнено(СтрокаРаздела8.НомИспрКСчФ), "" + СтрокаРаздела8.НомИспрКСчФ + ";", "")
			+ СтрокаРаздела8.ДатаИспрКСчФ;
	КонецЕсли;
	...
	Для Каждого СведенияОПродавце Из СтрокаРаздела8.СвПрод Цикл
		...
		Если ЗначениеЗаполнено(СведенияОПродавце) Тогда
			СведенияОПродавце.Вставить("Контрагент",             Продавец);
			СведенияОПродавце.Вставить("КонтрагентНаименование", СокрЛП(ПродавецНаименование));
		КонецЕсли;
		
		Если СтрокаРаздела8.СвПрод.Количество() = 1 Тогда
			ДополнительныеСведения.Контрагент = Продавец;
			ДополнительныеСведения.ПредставлениеКонтрагента = СокрЛП(ПродавецНаименование);
		КонецЕсли;
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(СтрокаРаздела8.СвПос) Тогда
		СтрокаРаздела8.СвПос.Вставить("Контрагент",             Посредник);
		СтрокаРаздела8.СвПос.Вставить("КонтрагентНаименование", СокрЛП(ПараметрыСтроки.Посредник));
	КонецЕсли;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
	 ...
	 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
	 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ПараметрыСтроки.НомерОтДатаОплаты = СтрЗаменить(НомераДатыДокументов, ";", НСтр("ru = ' от ';
																						|en = ' from'"));
	Иначе
	...
	Валюта = ВалютаПоКодуСУчетомКэша(СтрокаРаздела8.ОКВ, НаименованиеВалюты, КэшВалют);
	...
	СвРегНомРНПТ = Новый Массив;
	ЗаполнитьЗначенияСвойств(ПараметрыСтроки, Новый Структура("РегНомер,КолПрослеж,СтоимПрослеж,ЕдПрослеж"));
	...
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
		ПараметрыСтроки.НомерГТД_ = ?(ЗначениеЗаполнено(СтрокаРаздела8.РегНомТД), МассивВСтрокуСРазделителями(СтрокаРаздела8.РегНомТД, ";"), Неопределено);
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		Если ЗначениеЗаполнено(СтрокаРаздела8.СвРегНом) Тогда
			Для ИндНом = 0 По СтрокаРаздела8.СвРегНом.ВГраница() Цикл
				ЭлементРНПТ = СтрокаРаздела8.СвРегНом[ИндНом];
				
				СоставСтроки = Новый Структура;
				СоставСтроки.Вставить("РегНомер",     ЭлементРНПТ.РегНомПросл);
				СоставСтроки.Вставить("КолПрослеж",   ЭлементРНПТ.КолТовПросл);
				СоставСтроки.Вставить("СтоимПрослеж", ЭлементРНПТ.СтоимТовПросл);
				СоставСтроки.Вставить("ЕдПрослеж",    ЭлементРНПТ.ОКЕИ);
				
				Если ИндНом = 0 Тогда
					ЗаполнитьЗначенияСвойств(ПараметрыСтроки, СоставСтроки);
				Иначе
					СвРегНомРНПТ.Добавить(СоставСтроки);
				КонецЕсли;
			КонецЦикла; 
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаРаздела8.НомКСчФПрод) Тогда
		ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
			...
			СтрокаРаздела8.НомКСчФПрод,
			Формат(СтрокаРаздела8.ДатаКСчФПрод, "ДФ=dd.MM.yyyy"),
			Формат(ЧислоИзСтроки(ПараметрыСтроки.ВсегоПокупок), "ЧДЦ=2; ЧН="),
			Строка(Валюта));
	Иначе
		ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
			НСтр("ru = 'Счет-фактура № %1 от %2 на сумму %3 %4';
				|en = 'Tax invoice No.%1 from %2 for the amount of %3 %4'"), 
			СтрокаРаздела8.НомСчФПрод,
			Формат(СтрокаРаздела8.ДатаСчФПрод, "ДФ=dd.MM.yyyy"),
			Формат(ЧислоИзСтроки(ПараметрыСтроки.ВсегоПокупок), "ЧДЦ=2; ЧН="),
			Строка(Валюта));
	КонецЕсли;
	ДополнительныеСведения.Документ = Неопределено;
	СтрокаРаздела8.ДополнительныеСведения = ДополнительныеСведения;
	
	Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
	 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
	 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
	 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ОбластьЗапись = ПараметрыРаздела.СекцияСтрока.Область("ОбластьЗаписьОсновная");
		ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии);
		Если НЕ ЗначениеЗаполнено(СтрокаРаздела8.СвРегНом)
			ИЛИ СтрокаРаздела8.СвРегНом.Количество() = 1 Тогда
			ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
		КонецЕсли;
	КонецЕсли;
	
	ПараметрыРаздела.СекцияСтрока.Область(1, ,
		ПараметрыРаздела.СекцияСтрока.ВысотаТаблицы).Имя = "Строка_" + Формат(ПараметрыРаздела.НомерПП, "ЧГ=");
		
	ПараметрыРаздела.ТабличныйДокумент.Вывести(ПараметрыРаздела.СекцияСтрока);
	
	Если ЗначениеЗаполнено(СвРегНомРНПТ) Тогда
		Для ИндНом = 0 По СвРегНомРНПТ.ВГраница() Цикл
			ОбластьЗапись = ПараметрыРаздела.СекцияСтрокаРНПТ.Область("ОбластьЗапись");
			Если ИндНом = СвРегНомРНПТ.ВГраница() Тогда
				ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
			Иначе
				ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии);
			КонецЕсли;
			
			ЗаполнитьЗначенияСвойств(ПараметрыРаздела.СекцияСтрокаРНПТ.Параметры, СвРегНомРНПТ[ИндНом]);
			
			ПараметрыРаздела.ТабличныйДокумент.Вывести(ПараметрыРаздела.СекцияСтрокаРНПТ);
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела81(СтрокаРаздела81, СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП = 0 Тогда
		...
		ИмяМакета = "ДополнительныйЛистКнигиПокупок735";
		Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
			ИмяМакета = "ДополнительныйЛистКнигиПокупок1137_2017";
		ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		...
		ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
			  ...
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			ПараметрыРаздела.СтрокиДляПовтора.Область("НомераГрафКнига").Видимость = Ложь;
			ПараметрыРаздела.СтрокиДляПовтора.Область("НомераГрафДекларация").Видимость = Ложь;
		КонецЕсли;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
	 ...
	 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		СтрОт = НСтр("ru = ' от ';
					|en = ' from'");
		ПараметрыСтроки.НомерОтДата =
			?(ЗначениеЗаполнено(СтрокаРаздела81.НомСчФПрод), "" + СтрокаРаздела81.НомСчФПрод + СтрОт, "")
			+ СтрокаРаздела81.ДатаСчФПрод;
		ПараметрыСтроки.НомерОтДатаИсправленный =
			?(ЗначениеЗаполнено(СтрокаРаздела81.НомИспрСчФ), "" + СтрокаРаздела81.НомИспрСчФ + СтрОт, "")
			+ СтрокаРаздела81.ДатаИспрСчФ;
		ПараметрыСтроки.НомерОтДатаКорректировочный =
			?(ЗначениеЗаполнено(СтрокаРаздела81.НомКСчФПрод), "" + СтрокаРаздела81.НомКСчФПрод + СтрОт, "")
			+ СтрокаРаздела81.ДатаКСчФПрод;
		ПараметрыСтроки.НомерОтДатаИсправленныйКорректировочный =
			?(ЗначениеЗаполнено(СтрокаРаздела81.НомИспрКСчФ), "" + СтрокаРаздела81.НомИспрКСчФ + СтрОт, "")
			+ СтрокаРаздела81.ДатаИспрКСчФ;
	Иначе
		ПараметрыСтроки.НомерДата =
			?(ЗначениеЗаполнено(СтрокаРаздела81.НомСчФПрод), "" + СтрокаРаздела81.НомСчФПрод + ";", "")
			+ СтрокаРаздела81.ДатаСчФПрод;
		ПараметрыСтроки.НомерДатаИсправления =
			?(ЗначениеЗаполнено(СтрокаРаздела81.НомИспрСчФ), "" + СтрокаРаздела81.НомИспрСчФ + ";", "")
			+ СтрокаРаздела81.ДатаИспрСчФ;
		ПараметрыСтроки.НомерДатаКорректировки =
			?(ЗначениеЗаполнено(СтрокаРаздела81.НомКСчФПрод), "" + СтрокаРаздела81.НомКСчФПрод + ";", "")
			+ СтрокаРаздела81.ДатаКСчФПрод;
		ПараметрыСтроки.НомерДатаИсправленияКорректировки =
			?(ЗначениеЗаполнено(СтрокаРаздела81.НомИспрКСчФ), "" + СтрокаРаздела81.НомИспрКСчФ + ";", "")
			+ СтрокаРаздела81.ДатаИспрКСчФ;
	КонецЕсли;
	...
	Для Каждого СведенияОПродавце Из СтрокаРаздела81.СвПрод Цикл
		...
		ПродавецНаименование = Неопределено;
		Продавец = КонтрагентПоИННСУчетомКэша(ИННПродавца, ПродавецНаименование, КэшКонтрагентов);
		ПродавцыНаименование = ПродавцыНаименование
			+ ?(ЗначениеЗаполнено(ПродавцыНаименование), "; ", "") + ПродавецНаименование;
		Если ЗначениеЗаполнено(СведенияОПродавце) Тогда
			СведенияОПродавце.Вставить("Контрагент",             Продавец);
			СведенияОПродавце.Вставить("КонтрагентНаименование", СокрЛП(ПродавецНаименование));
		КонецЕсли;
		
		Если СтрокаРаздела81.СвПрод.Количество() = 1 Тогда
			ДополнительныеСведения.Контрагент = Продавец;
			ДополнительныеСведения.ПредставлениеКонтрагента = СокрЛП(ПродавецНаименование);
		КонецЕсли;
	КонецЦикла;
	...
	ПараметрыСтроки.ПосредникИНН_КПП = СокрЛП(ИННПосредника)
		+ ?(ЗначениеЗаполнено(КПППосредника), "/" + СокрЛП(КПППосредника), "");
	Посредник = КонтрагентПоИННСУчетомКэша(ИННПосредника, ПараметрыСтроки.Посредник, КэшКонтрагентов);
	Если ЗначениеЗаполнено(СтрокаРаздела81.СвПос) Тогда
		СтрокаРаздела81.СвПос.Вставить("Контрагент",             Посредник);
		СтрокаРаздела81.СвПос.Вставить("КонтрагентНаименование", СокрЛП(ПараметрыСтроки.Посредник));
	КонецЕсли;
	...
	НаименованиеВалюты = Неопределено;
	Валюта = ВалютаПоКодуСУчетомКэша(СтрокаРаздела81.ОКВ, НаименованиеВалюты, КэшВалют);
	Если Валюта.Пустая() Тогда
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2015Кв1" Тогда
		ПараметрыСтроки.НомерГТД_ = ?(ЗначениеЗаполнено(СтрокаРаздела81.НомТД), СтрокаРаздела81.НомТД, Неопределено);
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
		ПараметрыСтроки.НомерГТД_ = ?(ЗначениеЗаполнено(СтрокаРаздела81.РегНомТД), МассивВСтрокуСРазделителями(СтрокаРаздела81.РегНомТД, ";"), Неопределено);
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		Если ЗначениеЗаполнено(СтрокаРаздела81.СвРегНом) Тогда
			Для ИндНом = 0 По СтрокаРаздела81.СвРегНом.ВГраница() Цикл
				ЭлементРНПТ = СтрокаРаздела81.СвРегНом[ИндНом];
	...
	Если ЗначениеЗаполнено(СтрокаРаздела81.НомКСчФПрод) Тогда
		ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
			НСтр("ru = 'Корректировочный счет-фактура № %1 от %2 на сумму %3 %4';
				|en = 'Corrective tax invoice No. %1 from %2 in the amount of %3 %4 '"), 
			СтрокаРаздела81.НомКСчФПрод,
			Формат(СтрокаРаздела81.ДатаКСчФПрод, "ДФ=dd.MM.yyyy"),
			Формат(ЧислоИзСтроки(ПараметрыСтроки.ВсегоПокупок), "ЧДЦ=2; ЧН="),
			Строка(Валюта));
	Иначе
		ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
			НСтр("ru = 'Счет-фактура № %1 от %2 на сумму %3 %4';
				|en = 'Tax invoice No.%1 from %2 for the amount of %3 %4'"), 
			СтрокаРаздела81.НомСчФПрод,
			Формат(СтрокаРаздела81.ДатаСчФПрод, "ДФ=dd.MM.yyyy"),
			Формат(ЧислоИзСтроки(ПараметрыСтроки.ВсегоПокупок), "ЧДЦ=2; ЧН="),
			Строка(Валюта));
	КонецЕсли;
	ДополнительныеСведения.Документ = Неопределено;
	СтрокаРаздела81.ДополнительныеСведения = ДополнительныеСведения;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
	 ...
	 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ОбластьЗапись = ПараметрыРаздела.СекцияСтрока.Область("ОбластьЗаписьОсновная");
		ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии);
		Если НЕ ЗначениеЗаполнено(СтрокаРаздела81.СвРегНом)
			ИЛИ СтрокаРаздела81.СвРегНом.Количество() = 1 Тогда
			ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыРаздела.СчетчикСтрокТекущегоСегмента = ПредельноеКоличествоЗаписейВРазделеДекларацииПоНДС Тогда
		...
		ВывестиШапкуРаздела81Декларации(ПараметрыРаздела.ТабличныйДокумент, ПараметрыРаздела.Макет, ТекущаяДатаСеанса(),
			Окр(ПараметрыРаздела.НомерПП / ПредельноеКоличествоЗаписейВРазделеДекларацииПоНДС) + 1);
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ru = 'Заполнен раздел 8 приложение 1';
														|en = 'Section 8 in annex 1 completed'"));
	КонецЕсли;
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела9(СтрокаРаздела9, СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП = 0 Тогда
		...
		ИмяМакета = "КнигаПродаж735";
		Если      СтруктураПараметров.ПравилаЗаполнения = "2017Кв1" Тогда
			ИмяМакета = "КнигаПродаж1137_2017";
		ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
			ИмяМакета = "КнигаПродаж15";
		ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
				ИмяМакета = "КнигаПродаж1448";
			Иначе
				ИмяМакета = "КнигаПродаж534";
			КонецЕсли;
			ПараметрыРаздела.Вставить("СекцияСтрокаРНПТ");
		КонецЕсли;
		ПараметрыРаздела.Вставить("Макет", РеквизитФормыВЗначение("Объект").ПолучитьМакет(ИмяМакета));
		...
		ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
			  ...
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			ПараметрыРаздела.СтрокиДляПовтора.Область("НомераГрафКнига").Видимость = Ложь;
			ПараметрыРаздела.СтрокиДляПовтора.Область("НомераГрафДекларация91").Видимость = Ложь;
		КонецЕсли;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1" Тогда
		ПараметрыСтроки.РегНомерТД = ?(ЗначениеЗаполнено(СтрокаРаздела9.РегНомТД), МассивВСтрокуСРазделителями(СтрокаРаздела9.РегНомТД, ";"), Неопределено);
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
		ПараметрыСтроки.РегНомерТД = ?(ЗначениеЗаполнено(СтрокаРаздела9.РегНомТД), МассивВСтрокуСРазделителями(СтрокаРаздела9.РегНомТД, ";"), Неопределено);
		ПараметрыСтроки.КодВидаТовара = ?(ЗначениеЗаполнено(СтрокаРаздела9.КодВидТовар), МассивВСтрокуСРазделителями(СтрокаРаздела9.КодВидТовар, ";"), Неопределено);
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		Если ЗначениеЗаполнено(СтрокаРаздела9.СвРегНом) Тогда
			Для ИндНом = 0 По СтрокаРаздела9.СвРегНом.ВГраница() Цикл
				ЭлементРНПТ = СтрокаРаздела9.СвРегНом[ИндНом];
	...
	ПараметрыСтрокиПром = Новый Структура;
	
	ПараметрыСтрокиПром.Вставить("НомерДата",
		?(ЗначениеЗаполнено(СтрокаРаздела9.НомСчФПрод), "" + СтрокаРаздела9.НомСчФПрод + ";", "")
		+ СтрокаРаздела9.ДатаСчФПрод);
	ПараметрыСтрокиПром.Вставить("НомерОтДата",
		?(ЗначениеЗаполнено(СтрокаРаздела9.НомСчФПрод), "" + СтрокаРаздела9.НомСчФПрод + " " + СтрОт + " ", "")
		+ СтрокаРаздела9.ДатаСчФПрод);
		
	ПараметрыСтрокиПром.Вставить("НомерДатаИсправления",
		?(ЗначениеЗаполнено(СтрокаРаздела9.НомИспрСчФ), "" + СтрокаРаздела9.НомИспрСчФ + ";", "")
		+ СтрокаРаздела9.ДатаИспрСчФ);
	ПараметрыСтрокиПром.Вставить("НомерОтДатаИсправленный",
		?(ЗначениеЗаполнено(СтрокаРаздела9.НомИспрСчФ), "" + СтрокаРаздела9.НомИспрСчФ + " " + СтрОт + " ", "")
		+ СтрокаРаздела9.ДатаИспрСчФ);
		
	ПараметрыСтрокиПром.Вставить("НомерДатаКорректировки",
		?(ЗначениеЗаполнено(СтрокаРаздела9.НомКСчФПрод), "" + СтрокаРаздела9.НомКСчФПрод + ";", "")
		+ СтрокаРаздела9.ДатаКСчФПрод);
	ПараметрыСтрокиПром.Вставить("НомерОтДатаКорректировочный",
		?(ЗначениеЗаполнено(СтрокаРаздела9.НомКСчФПрод), "" + СтрокаРаздела9.НомКСчФПрод + " " + СтрОт + " ", "")
		+ СтрокаРаздела9.ДатаКСчФПрод);
		
	ПараметрыСтрокиПром.Вставить("НомерДатаИсправленияКорректировки",
		?(ЗначениеЗаполнено(СтрокаРаздела9.НомИспрКСчФ), "" + СтрокаРаздела9.НомИспрКСчФ + ";", "")
		+ СтрокаРаздела9.ДатаИспрКСчФ);
	ПараметрыСтрокиПром.Вставить("НомерОтДатаИсправленныйКорректировочный",
		?(ЗначениеЗаполнено(СтрокаРаздела9.НомИспрКСчФ), "" + СтрокаРаздела9.НомИспрКСчФ + " " + СтрОт + " ", "")
		+ СтрокаРаздела9.ДатаИспрКСчФ);
	...
	Для Каждого СведенияОПокупателе Из СтрокаРаздела9.СвПокуп Цикл
		...
		ПокупателиНаименование = ПокупателиНаименование
			+ ?(ЗначениеЗаполнено(ПокупателиНаименование), "; ", "")
			+ ПокупательНаименование;
		Если ЗначениеЗаполнено(СведенияОПокупателе) Тогда
			СведенияОПокупателе.Вставить("Контрагент",             Покупатель);
			СведенияОПокупателе.Вставить("КонтрагентНаименование", СокрЛП(ПокупательНаименование));
		КонецЕсли;
		
		Если СтрокаРаздела9.СвПокуп.Количество() = 1 Тогда
			ДополнительныеСведения.Контрагент = Покупатель;
			ДополнительныеСведения.ПредставлениеКонтрагента = СокрЛП(ПокупательНаименование);
		КонецЕсли;
	КонецЦикла;
	...
	ПараметрыСтроки.ПосредникИНН_КПП = СокрЛП(ИННПосредника)
		+ ?(ЗначениеЗаполнено(КПППосредника), "/" + СокрЛП(КПППосредника), "");
	Посредник = КонтрагентПоИННСУчетомКэша(ИННПосредника, ПараметрыСтроки.Посредник, КэшКонтрагентов);
	Если ЗначениеЗаполнено(СтрокаРаздела9.СвПос) Тогда
		СтрокаРаздела9.СвПос.Вставить("Контрагент",             Посредник);
		СтрокаРаздела9.СвПос.Вставить("КонтрагентНаименование", СокрЛП(ПараметрыСтроки.Посредник));
	КонецЕсли;
	
	НомераДатыДокументов = "";
	НомераОтДатыДокументов = "";
	Для Каждого НомерДатаДок Из СтрокаРаздела9.ДокПдтвОпл Цикл
		...
		НомерДатаДокумента =
			?(ЗначениеЗаполнено(НомерДокумента), "" + НомерДокумента + ";", "") + ДатаДокумента;
		НомераДатыДокументов = НомераДатыДокументов
			+ ?(ЗначениеЗаполнено(НомераДатыДокументов), "," + Символы.ПС, "") + НомерДатаДокумента;
		НомерОтДатаДокумента =
			?(ЗначениеЗаполнено(НомерДокумента), "" + НомерДокумента + " " + СтрОт + " ", "") + ДатаДокумента;
		НомераОтДатыДокументов = НомераОтДатыДокументов
			+ ?(ЗначениеЗаполнено(НомераОтДатыДокументов), "," + Символы.ПС, "") + НомерОтДатаДокумента;
	КонецЦикла;
	ПараметрыСтрокиПром.Вставить("НомерДатаОплаты",   НомераДатыДокументов);
	ПараметрыСтрокиПром.Вставить("НомерОтДатаОплаты", НомераОтДатыДокументов);
	
	НаименованиеВалюты = Неопределено;
	Валюта = ВалютаПоКодуСУчетомКэша(СтрокаРаздела9.ОКВ, НаименованиеВалюты, КэшВалют);
	Если Валюта.Пустая() Тогда
	...
	ПараметрыСтрокиПром.Вставить("СуммаБезНДС20", ?(ЗначениеЗаполнено(СтрокаРаздела9.СтоимПродСФ20), СтрокаРаздела9.СтоимПродСФ20, Неопределено));
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ПараметрыСтроки.СуммаБезНДС7 = ?(ЗначениеЗаполнено(СтрокаРаздела9.СтоимПродСФ7), СтрокаРаздела9.СтоимПродСФ7, Неопределено);
		ПараметрыСтроки.СуммаБезНДС5 = ?(ЗначениеЗаполнено(СтрокаРаздела9.СтоимПродСФ5), СтрокаРаздела9.СтоимПродСФ5, Неопределено);
	КонецЕсли;
	ПараметрыСтроки.НДС0 = ?(ЗначениеЗаполнено(СтрокаРаздела9.СтоимПродСФ0), СтрокаРаздела9.СтоимПродСФ0, Неопределено);
	ПараметрыСтрокиПром.Вставить("НДС20", ?(ЗначениеЗаполнено(СтрокаРаздела9.СумНДССФ20), СтрокаРаздела9.СумНДССФ20, Неопределено));
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ПараметрыСтроки.НДС7 = ?(ЗначениеЗаполнено(СтрокаРаздела9.СумНДССФ7), СтрокаРаздела9.СумНДССФ7, Неопределено);
		ПараметрыСтроки.НДС5 = ?(ЗначениеЗаполнено(СтрокаРаздела9.СумНДССФ5), СтрокаРаздела9.СумНДССФ5, Неопределено);
	КонецЕсли;
	ПараметрыСтроки.СуммаСовсемБезНДС = ?(ЗначениеЗаполнено(СтрокаРаздела9.СтоимПродОсв), СтрокаРаздела9.СтоимПродОсв, Неопределено);
	
	ПараметрыСтроки.Заполнить(ПараметрыСтрокиПром);
	
	Если ЗначениеЗаполнено(СтрокаРаздела9.НомКСчФПрод) Тогда
		ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
			НСтр("ru = 'Корректировочный счет-фактура № %1 от %2 на сумму %3 %4';
				|en = 'Corrective tax invoice No. %1 from %2 in the amount of %3 %4 '"), 
			СтрокаРаздела9.НомКСчФПрод,
			Формат(СтрокаРаздела9.ДатаКСчФПрод, "ДФ=dd.MM.yyyy"),
			Формат(ЧислоИзСтроки(ПараметрыСтроки.ВсегоПродаж), "ЧДЦ=2; ЧН="),
			Строка(Валюта));
	Иначе
		ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
			НСтр("ru = 'Счет-фактура № %1 от %2 на сумму %3 %4';
				|en = 'Tax invoice No.%1 from %2 for the amount of %3 %4'"), 
			СтрокаРаздела9.НомСчФПрод,
			Формат(СтрокаРаздела9.ДатаСчФПрод, "ДФ=dd.MM.yyyy"),
			Формат(ЧислоИзСтроки(ПараметрыСтроки.ВсегоПродаж), "ЧДЦ=2; ЧН="),
			Строка(Валюта));
	КонецЕсли;
	ДополнительныеСведения.Документ = Неопределено;
	СтрокаРаздела9.ДополнительныеСведения = ДополнительныеСведения;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
	 ...
	 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ОбластьЗапись = ПараметрыРаздела.СекцияСтрока.Область("ОбластьЗаписьОсновная");
		ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии);
		Если НЕ ЗначениеЗаполнено(СтрокаРаздела9.СвРегНом)
			ИЛИ СтрокаРаздела9.СвРегНом.Количество() = 1 Тогда
			ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыРаздела.ПоследняяСтрока = Истина Тогда
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			СтруктураПараметров.ИтогиРаздел9.Свойство("СтПродБезНДС20", ЗначениеСвойства);
			СекцияВсего.Параметры.СуммаБезНДС20 = ?(ЗначениеЗаполнено(ЗначениеСвойства), ЗначениеСвойства, Неопределено);
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			СтруктураПараметров.ИтогиРаздел9.Свойство("СтПродБезНДС7", ЗначениеСвойства);
			СекцияВсего.Параметры.СуммаБезНДС7 = ?(ЗначениеЗаполнено(ЗначениеСвойства), ЗначениеСвойства, Неопределено);
			СтруктураПараметров.ИтогиРаздел9.Свойство("СтПродБезНДС5", ЗначениеСвойства);
			СекцияВсего.Параметры.СуммаБезНДС5 = ?(ЗначениеЗаполнено(ЗначениеСвойства), ЗначениеСвойства, Неопределено);
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			СтруктураПараметров.ИтогиРаздел9.Свойство("СумНДСВсКПр20", ЗначениеСвойства);
			СекцияВсего.Параметры.НДС20 = ?(ЗначениеЗаполнено(ЗначениеСвойства), ЗначениеСвойства, Неопределено);
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			СтруктураПараметров.ИтогиРаздел9.Свойство("СумНДСВсКПр7", ЗначениеСвойства);
			СекцияВсего.Параметры.НДС7 = ?(ЗначениеЗаполнено(ЗначениеСвойства), ЗначениеСвойства, Неопределено);
			СтруктураПараметров.ИтогиРаздел9.Свойство("СумНДСВсКПр5", ЗначениеСвойства);
			СекцияВсего.Параметры.НДС5 = ?(ЗначениеЗаполнено(ЗначениеСвойства), ЗначениеСвойства, Неопределено);
		КонецЕсли;
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела91(СтрокаРаздела91, СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП = 0 Тогда
		...
		ИмяМакета = "ДополнительныйЛистКнигиПродаж735";
		Если      СтруктураПараметров.ПравилаЗаполнения = "2017Кв1" Тогда
			ИмяМакета = "ДополнительныйЛистКнигиПродаж1137_2017";
		ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
			ИмяМакета = "ДополнительныйЛистКнигиПродаж15";
		ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			ИтСтПродКПр20 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС20;
			ПараметрыРаздела.СекцияИтого.Параметры.СуммаБезНДС20 = ?(ЗначениеЗаполнено(ИтСтПродКПр20), ИтСтПродКПр20, Неопределено);
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв4" Тогда
			ИтСтПродКПр7 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС7;
			ПараметрыРаздела.СекцияИтого.Параметры.СуммаБезНДС7 = ?(ЗначениеЗаполнено(ИтСтПродКПр7), ИтСтПродКПр7, Неопределено);
			
			ИтСтПродКПр5 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС5;
			ПараметрыРаздела.СекцияИтого.Параметры.СуммаБезНДС5 = ?(ЗначениеЗаполнено(ИтСтПродКПр5), ИтСтПродКПр5, Неопределено);
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			СумНДСИтКПр20 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр20;
			ПараметрыРаздела.СекцияИтого.Параметры.НДС20 = ?(ЗначениеЗаполнено(СумНДСИтКПр20), СумНДСИтКПр20, Неопределено);
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв4" Тогда
			СумНДСИтКПр7 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр7;
			ПараметрыРаздела.СекцияИтого.Параметры.НДС7 = ?(ЗначениеЗаполнено(СумНДСИтКПр7), СумНДСИтКПр7, Неопределено);
			
			СумНДСИтКПр5 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр5;
			ПараметрыРаздела.СекцияИтого.Параметры.НДС5 = ?(ЗначениеЗаполнено(СумНДСИтКПр5), СумНДСИтКПр5, Неопределено);
		КонецЕсли;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1" Тогда
		ПараметрыСтроки.РегНомерТД = ?(ЗначениеЗаполнено(СтрокаРаздела91.РегНомТД),
			МассивВСтрокуСРазделителями(СтрокаРаздела91.РегНомТД, ";"), Неопределено);
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
		ПараметрыСтроки.РегНомерТД = ?(ЗначениеЗаполнено(СтрокаРаздела91.РегНомТД),
			МассивВСтрокуСРазделителями(СтрокаРаздела91.РегНомТД, ";"), Неопределено);
		ПараметрыСтроки.КодВидаТовара = ?(ЗначениеЗаполнено(СтрокаРаздела91.КодВидТовар),
			МассивВСтрокуСРазделителями(СтрокаРаздела91.КодВидТовар, ";"), Неопределено);
		ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		Если ЗначениеЗаполнено(СтрокаРаздела91.СвРегНом) Тогда
			Для ИндНом = 0 По СтрокаРаздела91.СвРегНом.ВГраница() Цикл
				ЭлементРНПТ = СтрокаРаздела91.СвРегНом[ИндНом];
	...
	ПараметрыСтрокиПром.Вставить("НомерДата",
		?(ЗначениеЗаполнено(СтрокаРаздела91.НомСчФПрод), "" + СтрокаРаздела91.НомСчФПрод + ";", "")
		+ СтрокаРаздела91.ДатаСчФПрод);
	ПараметрыСтрокиПром.Вставить("НомерОтДата",
		?(ЗначениеЗаполнено(СтрокаРаздела91.НомСчФПрод), "" + СтрокаРаздела91.НомСчФПрод + " " + СтрОт + " ", "")
		+ СтрокаРаздела91.ДатаСчФПрод);
	
	ПараметрыСтрокиПром.Вставить("НомерДатаИсправления",
		?(ЗначениеЗаполнено(СтрокаРаздела91.НомИспрСчФ), "" + СтрокаРаздела91.НомИспрСчФ + ";", "")
		+ СтрокаРаздела91.ДатаИспрСчФ);
	ПараметрыСтрокиПром.Вставить("НомерОтДатаИсправленный",
		?(ЗначениеЗаполнено(СтрокаРаздела91.НомИспрСчФ), "" + СтрокаРаздела91.НомИспрСчФ + " " + СтрОт + " ", "")
		+ СтрокаРаздела91.ДатаИспрСчФ);
	
	ПараметрыСтрокиПром.Вставить("НомерДатаКорректировки",
		?(ЗначениеЗаполнено(СтрокаРаздела91.НомКСчФПрод), "" + СтрокаРаздела91.НомКСчФПрод + ";", "")
		+ СтрокаРаздела91.ДатаКСчФПрод);
	ПараметрыСтрокиПром.Вставить("НомерОтДатаКорректировочный",
		?(ЗначениеЗаполнено(СтрокаРаздела91.НомКСчФПрод), "" + СтрокаРаздела91.НомКСчФПрод + " " + СтрОт + " ", "")
		+ СтрокаРаздела91.ДатаКСчФПрод);
	
	ПараметрыСтрокиПром.Вставить("НомерДатаИсправленияКорректировки",
		?(ЗначениеЗаполнено(СтрокаРаздела91.НомИспрКСчФ), "" + СтрокаРаздела91.НомИспрКСчФ + ";", "")
		+ СтрокаРаздела91.ДатаИспрКСчФ);
	ПараметрыСтрокиПром.Вставить("НомерОтДатаИсправленныйКорректировочный",
		?(ЗначениеЗаполнено(СтрокаРаздела91.НомИспрКСчФ), "" + СтрокаРаздела91.НомИспрКСчФ + " " + СтрОт + " ", "")
		+ СтрокаРаздела91.ДатаИспрКСчФ);
	...
	Для Каждого СведенияОПокупателе Из СтрокаРаздела91.СвПокуп Цикл
		...
		Покупатель = КонтрагентПоИННСУчетомКэша(ИННПокупателя, ПокупательНаименование, КэшКонтрагентов);
		ПокупателиНаименование = ПокупателиНаименование
			+ ?(ЗначениеЗаполнено(ПокупателиНаименование), "; ", "") + ПокупательНаименование;
		Если ЗначениеЗаполнено(СведенияОПокупателе) Тогда
			СведенияОПокупателе.Вставить("Контрагент",             Покупатель);
			СведенияОПокупателе.Вставить("КонтрагентНаименование", СокрЛП(ПокупательНаименование));
		КонецЕсли;
		
		Если СтрокаРаздела91.СвПокуп.Количество() = 1 Тогда
			ДополнительныеСведения.Контрагент = Покупатель;
			ДополнительныеСведения.ПредставлениеКонтрагента = СокрЛП(ПокупательНаименование);
		КонецЕсли;
	КонецЦикла;
	...
	ПараметрыСтроки.ПосредникИНН_КПП = СокрЛП(ИННПосредника)
		+ ?(ЗначениеЗаполнено(КПППосредника), "/" + СокрЛП(КПППосредника), "");
	Посредник = КонтрагентПоИННСУчетомКэша(ИННПосредника, ПараметрыСтроки.Посредник, КэшКонтрагентов);
	Если ЗначениеЗаполнено(СтрокаРаздела91.СвПос) Тогда
		СтрокаРаздела91.СвПос.Вставить("Контрагент",             Посредник);
		СтрокаРаздела91.СвПос.Вставить("КонтрагентНаименование", СокрЛП(ПараметрыСтроки.Посредник));
	КонецЕсли;
	...
	НаименованиеВалюты = Неопределено;
	Валюта = ВалютаПоКодуСУчетомКэша(СтрокаРаздела91.ОКВ, НаименованиеВалюты, КэшВалют);
	Если Валюта.Пустая() Тогда
	...
	ПараметрыСтрокиПром.Вставить("СуммаБезНДС20", ?(ЗначениеЗаполнено(СтрокаРаздела91.СтоимПродСФ20), СтрокаРаздела91.СтоимПродСФ20, Неопределено));
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв4" Тогда
		ПараметрыСтроки.СуммаБезНДС7 = ?(ЗначениеЗаполнено(СтрокаРаздела91.СтоимПродСФ7), СтрокаРаздела91.СтоимПродСФ7, Неопределено);
		ПараметрыСтроки.СуммаБезНДС5 = ?(ЗначениеЗаполнено(СтрокаРаздела91.СтоимПродСФ5), СтрокаРаздела91.СтоимПродСФ5, Неопределено);
	КонецЕсли;
	ПараметрыСтроки.НДС0 = ?(ЗначениеЗаполнено(СтрокаРаздела91.СтоимПродСФ0), СтрокаРаздела91.СтоимПродСФ0, Неопределено);
	ПараметрыСтрокиПром.Вставить("НДС20", ?(ЗначениеЗаполнено(СтрокаРаздела91.СумНДССФ20), СтрокаРаздела91.СумНДССФ20, Неопределено));
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв4" Тогда
		ПараметрыСтроки.НДС7 = ?(ЗначениеЗаполнено(СтрокаРаздела91.СумНДССФ7), СтрокаРаздела91.СумНДССФ7, Неопределено);
		ПараметрыСтроки.НДС5 = ?(ЗначениеЗаполнено(СтрокаРаздела91.СумНДССФ5), СтрокаРаздела91.СумНДССФ5, Неопределено);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаРаздела91.НомКСчФПрод) Тогда
		ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
		НСтр("ru = 'Корректировочный счет-фактура № %1 от %2 на сумму %3 %4';
			|en = 'Corrective tax invoice No. %1 from %2 in the amount of %3 %4 '"), 
		СтрокаРаздела91.НомКСчФПрод,
		Формат(СтрокаРаздела91.ДатаКСчФПрод, "ДФ=dd.MM.yyyy"),
		Формат(ЧислоИзСтроки(ПараметрыСтроки.ВсегоПродаж), "ЧДЦ=2; ЧН="),
		Строка(Валюта));
	Иначе
		ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
		НСтр("ru = 'Счет-фактура № %1 от %2 на сумму %3 %4';
			|en = 'Tax invoice No.%1 from %2 for the amount of %3 %4'"), 
		СтрокаРаздела91.НомСчФПрод,
		Формат(СтрокаРаздела91.ДатаСчФПрод, "ДФ=dd.MM.yyyy"),
		Формат(ЧислоИзСтроки(ПараметрыСтроки.ВсегоПродаж), "ЧДЦ=2; ЧН="),
		Строка(Валюта));
	КонецЕсли;
	ДополнительныеСведения.Документ = Неопределено;
	СтрокаРаздела91.ДополнительныеСведения = ДополнительныеСведения;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
	 ...
	 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ОбластьЗапись = ПараметрыРаздела.СекцияСтрока.Область("ОбластьЗаписьОсновная");
		ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии);
		Если НЕ ЗначениеЗаполнено(СтрокаРаздела91.СвРегНом)
			ИЛИ СтрокаРаздела91.СвРегНом.Количество() = 1 Тогда
			ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыРаздела.ПоследняяСтрока = Истина Тогда
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			СтПродВсП1Р9_20 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС20 - ИтогиРаздел91.ИтСтПродКПр20 + ИтогиРаздел91.СтПродВсП1Р9_20;
			СекцияВсего.Параметры.СуммаБезНДС20 = ?(ЗначениеЗаполнено(СтПродВсП1Р9_20), СтПродВсП1Р9_20, Неопределено);
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв4" Тогда
			СтПродВсП1Р9_7 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС7 - ИтогиРаздел91.ИтСтПродКПр7 + ИтогиРаздел91.СтПродВсП1Р9_7;
			СекцияВсего.Параметры.СуммаБезНДС7 = ?(ЗначениеЗаполнено(СтПродВсП1Р9_7), СтПродВсП1Р9_7, Неопределено);
			
			СтПродВсП1Р9_5 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС5 - ИтогиРаздел91.ИтСтПродКПр5 + ИтогиРаздел91.СтПродВсП1Р9_5;
			СекцияВсего.Параметры.СуммаБезНДС5 = ?(ЗначениеЗаполнено(СтПродВсП1Р9_5), СтПродВсП1Р9_5, Неопределено);
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			СумНДСВсП1Р9_20 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр20 - ИтогиРаздел91.СумНДСИтКПр20 + ИтогиРаздел91.СумНДСВсП1Р9_20;
			СекцияВсего.Параметры.НДС20 = ?(ЗначениеЗаполнено(СумНДСВсП1Р9_20), СумНДСВсП1Р9_20, Неопределено);
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв4" Тогда
			СумНДСВсП1Р9_7 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр7 - ИтогиРаздел91.СумНДСИтКПр7 + ИтогиРаздел91.СумНДСВсП1Р9_7;
			СекцияВсего.Параметры.НДС7 = ?(ЗначениеЗаполнено(СумНДСВсП1Р9_7), СумНДСВсП1Р9_7, Неопределено);
			
			СумНДСВсП1Р9_5 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр5 - ИтогиРаздел91.СумНДСИтКПр5 + ИтогиРаздел91.СумНДСВсП1Р9_5;
			СекцияВсего.Параметры.НДС5 = ?(ЗначениеЗаполнено(СумНДСВсП1Р9_5), СумНДСВсП1Р9_5, Неопределено);
		КонецЕсли;
	...
	Если ПараметрыРаздела.СчетчикСтрокТекущегоСегмента = ПредельноеКоличествоЗаписейВРазделеДекларацииПоНДС Тогда
		...
		ВывестиШапкуРаздела91Декларации(ПараметрыРаздела.ТабличныйДокумент, ПараметрыРаздела.Макет, ТекущаяДатаСеанса(),
			Окр(ПараметрыРаздела.НомерПП / ПредельноеКоличествоЗаписейВРазделеДекларацииПоНДС) + 1);
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ru = 'Заполнен раздел 9 приложение 1';
														|en = 'Section 9 in annex 1 completed'"));
	КонецЕсли;
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела10(СтрокаРаздела10, СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП = 0 Тогда
		...
		ИмяМакета = "ЖурналУчетаСчетовФактур735";
		Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
			ИмяМакета = "ЖурналУчетаСчетовФактур1137_2017";
		ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2022Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			ИмяМакета = "ЖурналУчетаСчетовФактур534";
			ПараметрыРаздела.Вставить("Часть1СтрокаРНПТ");
		КонецЕсли;
		ПараметрыРаздела.Вставить("Макет", РеквизитФормыВЗначение("Объект").ПолучитьМакет(ИмяМакета));
		Если ПараметрыРаздела.Свойство("Часть1СтрокаРНПТ") Тогда
			ПараметрыРаздела.Часть1СтрокаРНПТ = ПараметрыРаздела.Макет.ПолучитьОбласть("Часть1СтрокаРНПТ");
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			ПараметрыРаздела.ШапкаТаблицыРаздел10.Область("НомераГрафЖурналЧ1").Видимость = Ложь;
		КонецЕсли;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2015Кв1" Тогда
		МассивСвПосрДеят = СтрокаРаздела10.СвПосрДеят;
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
		  ...
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		МассивСвПосрДеят = СтрокаРаздела10.СвСчФОтПрод;
	КонецЕсли;
	
	МаксимальныйИндекс = Макс(0, МассивСвПосрДеят.ВГраница());
	Для Инд = 0 По МаксимальныйИндекс Цикл
		...
		Если Инд = 0 Тогда
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
			 ...
			 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
				СтрОт = НСтр("ru = ' от ';
							|en = ' from'");
				ПараметрыСтроки.НомерОтДата =
					?(ЗначениеЗаполнено(СтрокаРаздела10.НомСчФПрод), "" + СтрокаРаздела10.НомСчФПрод + СтрОт, "")
					+ СтрокаРаздела10.ДатаСчФПрод;
				ПараметрыСтроки.НомерОтДатаИсправленный =
					?(ЗначениеЗаполнено(СтрокаРаздела10.НомИспрСчФ), "" + СтрокаРаздела10.НомИспрСчФ + СтрОт, "")
					+ СтрокаРаздела10.ДатаИспрСчФ;
				ПараметрыСтроки.НомерОтДатаКорректировочный =
					?(ЗначениеЗаполнено(СтрокаРаздела10.НомКСчФПрод), "" + СтрокаРаздела10.НомКСчФПрод + СтрОт, "")
					+ СтрокаРаздела10.ДатаКСчФПрод;
				ПараметрыСтроки.НомерОтДатаИсправленныйКорректировочный =
					?(ЗначениеЗаполнено(СтрокаРаздела10.НомИспрКСчФ), "" + СтрокаРаздела10.НомИспрКСчФ + СтрОт, "")
					+ СтрокаРаздела10.ДатаИспрКСчФ;
			Иначе
				ПараметрыСтроки.НомерДата =
					?(ЗначениеЗаполнено(СтрокаРаздела10.НомСчФПрод), "" + СтрокаРаздела10.НомСчФПрод + ";", "")
					+ СтрокаРаздела10.ДатаСчФПрод;
				ПараметрыСтроки.НомерДатаИсправления =
					?(ЗначениеЗаполнено(СтрокаРаздела10.НомИспрСчФ), "" + СтрокаРаздела10.НомИспрСчФ + ";", "")
					+ СтрокаРаздела10.ДатаИспрСчФ;
				ПараметрыСтроки.НомерДатаКорректировки =
					?(ЗначениеЗаполнено(СтрокаРаздела10.НомКСчФПрод), "" + СтрокаРаздела10.НомКСчФПрод + ";", "")
					+ СтрокаРаздела10.ДатаКСчФПрод;
				ПараметрыСтроки.НомерДатаИсправленияКорректировки =
					?(ЗначениеЗаполнено(СтрокаРаздела10.НомИспрКСчФ), "" + СтрокаРаздела10.НомИспрКСчФ + ";", "")
					+ СтрокаРаздела10.ДатаИспрКСчФ;
			КонецЕсли;
			...
			ПараметрыСтроки.КонтрагентИННКПП = СокрЛП(ИННКонтрагента)
				+ ?(ЗначениеЗаполнено(КППКонтрагента), "/" + СокрЛП(КППКонтрагента), "");
			
			Контрагент = КонтрагентПоИННСУчетомКэша(ИННКонтрагента, ПараметрыСтроки.КонтрагентНаименование, КэшКонтрагентов);
			
			Если ЗначениеЗаполнено(СтрокаРаздела10.СвПокуп) Тогда
				СтрокаРаздела10.СвПокуп.Вставить("Контрагент",             Контрагент);
				СтрокаРаздела10.СвПокуп.Вставить("КонтрагентНаименование", СокрЛП(ПараметрыСтроки.КонтрагентНаименование));
			КонецЕсли;
			
			ДополнительныеСведения.Контрагент = Контрагент;
			ДополнительныеСведения.ПредставлениеКонтрагента = СокрЛП(ПараметрыСтроки.КонтрагентНаименование);
			ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
				НСтр("ru = 'Счет-фактура выданный № %1 от %2 на сумму %3 %4';
					|en = 'Issued tax invoice No. %1 from %2 in the amount of %3 %4'"), 
				СтрокаРаздела10.НомСчФПрод, 
				Формат(СтрокаРаздела10.ДатаСчФПрод, "ДФ=dd.MM.yyyy"),
				Формат(0, "ЧДЦ=2; ЧН="), "");
		...
		Если Инд <= МассивСвПосрДеят.ВГраница() Тогда
			...
			СвПосрДеят = МассивСвПосрДеят[Инд];
			Если ТипЗнч(СвПосрДеят) = Тип("Структура") Тогда
				...
				ПараметрыСтроки.КомитентИНН_КПП = СокрЛП(ИННКомитента)
					+ ?(ЗначениеЗаполнено(КППКомитента), "/" + СокрЛП(КППКомитента), "");
				
				Комитент = КонтрагентПоИННСУчетомКэша(ИННКомитента, ПараметрыСтроки.Комитент, КэшКонтрагентов);
				
				Если ЗначениеЗаполнено(СвПрод) Тогда
					СвПрод.Вставить("Контрагент",             Комитент);
					СвПрод.Вставить("КонтрагентНаименование", СокрЛП(ПараметрыСтроки.Комитент));
				КонецЕсли;
				...
				Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
				 ...
				 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
					СтрОт = НСтр("ru = ' от ';
								|en = ' from'");
					ПараметрыСтроки.НомерОтДатаСчетаФактурыКомитента =
						?(ЗначениеЗаполнено(НомерСчетаФактурыКомитента), "" + НомерСчетаФактурыКомитента + СтрОт, "")
						+ ДатаСчетаФактурыКомитента;
				Иначе
					ПараметрыСтроки.НомерДатаСчетаФактурыКомитента =
						?(ЗначениеЗаполнено(НомерСчетаФактурыКомитента), "" + НомерСчетаФактурыКомитента + ";", "")
						+ ДатаСчетаФактурыКомитента;
				КонецЕсли;
					
				КодВалюты = Неопределено; НаименованиеВалюты = Неопределено;
				Валюта = ВалютаПоКодуСУчетомКэша(КодВалюты, НаименованиеВалюты, КэшВалют);
				Если СвПосрДеят.Свойство("ОКВ", КодВалюты) И НЕ Валюта.Пустая() Тогда
				...
				СтрокаРаздела10.СуммаПосрДеят = СтрокаРаздела10.СуммаПосрДеят + ЧислоИзСтроки(ПараметрыСтроки.СуммаДокумента);
				
				Если СвПосрДеят.Свойство("СвРегНом") И ЗначениеЗаполнено(СвПосрДеят.СвРегНом) Тогда
					Для ИндНом = 0 По СвПосрДеят.СвРегНом.ВГраница() Цикл
						ЭлементРНПТ = СвПосрДеят.СвРегНом[ИндНом];
				...
				ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
					...
					СтрокаРаздела10.НомСчФПрод, 
					Формат(СтрокаРаздела10.ДатаСчФПрод, "ДФ=dd.MM.yyyy"),
					Формат(СтрокаРаздела10.СуммаПосрДеят, "ЧДЦ=2; ЧН="),
					Строка(Валюта));
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			Если ПараметрыРаздела.Часть1Строка.Области.Найти("ОбластьЗаписьОсновнаяЧасть1") <> Неопределено Тогда
				ОбластьЗапись = ПараметрыРаздела.Часть1Строка.Область("ОбластьЗаписьОсновнаяЧасть1");
				ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии);
				Если НЕ ЗначениеЗаполнено(СвРегНомРНПТ) Тогда
					ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		ПараметрыРаздела.Часть1Строка.Область(1, ,
			ПараметрыРаздела.Часть1Строка.ВысотаТаблицы).Имя = "Строка_" + Формат(ПараметрыРаздела.НомерПП, "ЧГ=");
			
		ПараметрыРаздела.ТабличныйДокумент.Вывести(ПараметрыРаздела.Часть1Строка);
		
		Если ЗначениеЗаполнено(СвРегНомРНПТ) Тогда
			Для ИндНом = 0 По СвРегНомРНПТ.ВГраница() Цикл
				ОбластьЗапись = ПараметрыРаздела.Часть1СтрокаРНПТ.Область("ОбластьЗаписьЧасть1");
				Если ИндНом = СвРегНомРНПТ.ВГраница() Тогда
					ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
				Иначе
					ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии);
				...
				ЗаполнитьЗначенияСвойств(ПараметрыРаздела.Часть1СтрокаРНПТ.Параметры, СвРегНомРНПТ[ИндНом]);
				
				ПараметрыРаздела.ТабличныйДокумент.Вывести(ПараметрыРаздела.Часть1СтрокаРНПТ);
			КонецЦикла;
		КонецЕсли;
		
	КонецЦикла;
	
	СтрокаРаздела10.ДополнительныеСведения = ДополнительныеСведения;
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела11(СтрокаРаздела11, СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП = 0 Тогда
		...
		ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
			  ...
			  ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			ИмяМакета = "ЖурналУчетаСчетовФактур534";
			ПараметрыРаздела.Вставить("Часть2СтрокаРНПТ");
		КонецЕсли;
		ПараметрыРаздела.Вставить("Макет", РеквизитФормыВЗначение("Объект").ПолучитьМакет(ИмяМакета));
		Если ПараметрыРаздела.Свойство("Часть2СтрокаРНПТ") Тогда
			ПараметрыРаздела.Часть2СтрокаРНПТ = ПараметрыРаздела.Макет.ПолучитьОбласть("Часть2СтрокаРНПТ");
		КонецЕсли;
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
		 ...
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			ПараметрыРаздела.ШапкаТаблицыРаздел11.Область("НомераГрафЖурналЧ2").Видимость = Ложь;
		КонецЕсли;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
	 ...
	 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		СтрОт = НСтр("ru = ' от ';
					|en = ' from'");
		ПараметрыСтроки.НомерОтДата =
			?(ЗначениеЗаполнено(СтрокаРаздела11.НомСчФПрод), "" + СтрокаРаздела11.НомСчФПрод + СтрОт, "")
			+ СтрокаРаздела11.ДатаСчФПрод;
		ПараметрыСтроки.НомерОтДатаИсправленный =
			?(ЗначениеЗаполнено(СтрокаРаздела11.НомИспрСчФ), "" + СтрокаРаздела11.НомИспрСчФ + СтрОт, "")
			+ СтрокаРаздела11.ДатаИспрСчФ;
		ПараметрыСтроки.НомерОтДатаКорректировочный =
			?(ЗначениеЗаполнено(СтрокаРаздела11.НомКСчФПрод), "" + СтрокаРаздела11.НомКСчФПрод + СтрОт, "")
			+ СтрокаРаздела11.ДатаКСчФПрод;
		ПараметрыСтроки.НомерОтДатаИсправленныйКорректировочный =
			?(ЗначениеЗаполнено(СтрокаРаздела11.НомИспрКСчФ), "" + СтрокаРаздела11.НомИспрКСчФ + СтрОт, "")
			+ СтрокаРаздела11.ДатаИспрКСчФ;
	Иначе
		ПараметрыСтроки.НомерДата =
			?(ЗначениеЗаполнено(СтрокаРаздела11.НомСчФПрод), "" + СтрокаРаздела11.НомСчФПрод + ";", "")
			+ СтрокаРаздела11.ДатаСчФПрод;
		ПараметрыСтроки.НомерДатаИсправления =
			?(ЗначениеЗаполнено(СтрокаРаздела11.НомИспрСчФ), "" + СтрокаРаздела11.НомИспрСчФ + ";", "")
			+ СтрокаРаздела11.ДатаИспрСчФ;
		ПараметрыСтроки.НомерДатаКорректировки =
			?(ЗначениеЗаполнено(СтрокаРаздела11.НомКСчФПрод), "" + СтрокаРаздела11.НомКСчФПрод + ";", "")
			+ СтрокаРаздела11.ДатаКСчФПрод;
		ПараметрыСтроки.НомерДатаИсправленияКорректировки =
			?(ЗначениеЗаполнено(СтрокаРаздела11.НомИспрКСчФ), "" + СтрокаРаздела11.НомИспрКСчФ + ";", "")
			+ СтрокаРаздела11.ДатаИспрКСчФ;
	КонецЕсли;
	...
	ПараметрыСтроки.КонтрагентИННКПП = СокрЛП(ИННКонтрагента)
		+ ?(ЗначениеЗаполнено(КППКонтрагента), "/" + СокрЛП(КППКонтрагента), "");
	Контрагент = КонтрагентПоИННСУчетомКэша(ИННКонтрагента, ПараметрыСтроки.КонтрагентНаименование, КэшКонтрагентов);
	Если ЗначениеЗаполнено(СтрокаРаздела11.СвПрод) Тогда
		СтрокаРаздела11.СвПрод.Вставить("Контрагент",             Контрагент);
		СтрокаРаздела11.СвПрод.Вставить("КонтрагентНаименование", СокрЛП(ПараметрыСтроки.КонтрагентНаименование));
	КонецЕсли;
	...
	ПараметрыСтроки.СубкомиссионерИНН_КПП = СокрЛП(ИННСубкомиссионера)
		+ ?(ЗначениеЗаполнено(КППСубкомиссионера), "/" + СокрЛП(КППСубкомиссионера), "");
	Субкомиссионер = КонтрагентПоИННСУчетомКэша(ИННСубкомиссионера, ПараметрыСтроки.Субкомиссионер, КэшКонтрагентов);
	Если ЗначениеЗаполнено(СтрокаРаздела11.СвКомис) Тогда
		СтрокаРаздела11.СвКомис.Вставить("Контрагент",             Субкомиссионер);
		СтрокаРаздела11.СвКомис.Вставить("КонтрагентНаименование", СокрЛП(ПараметрыСтроки.Субкомиссионер));
	КонецЕсли;
	
	ПараметрыСтроки.КодВидаСделки = СтрокаРаздела11.КодВидСд;
	
	НаименованиеВалюты = Неопределено;
	Валюта = ВалютаПоКодуСУчетомКэша(СтрокаРаздела11.ОКВ, НаименованиеВалюты, КэшВалют);
	Если Валюта.Пустая() Тогда
	...
	ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
		НСтр("ru = 'Счет-фактура полученный № %1 от %2 на сумму %3 %4';
			|en = 'Received tax invoice No.%1 from %2 in the amount of %3 %4'"), 
		СтрокаРаздела11.НомСчФПрод, 
		Формат(СтрокаРаздела11.ДатаСчФПрод, "ДФ=dd.MM.yyyy"),
		Формат(СтрокаРаздела11.СтоимТовСчФВс, "ЧДЦ=2; ЧН="),
		Строка(Валюта));
	ДополнительныеСведения.Документ = Неопределено;
	СтрокаРаздела11.ДополнительныеСведения = ДополнительныеСведения;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
	 ...
	 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2023Кв1"
	 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		Если ЗначениеЗаполнено(СтрокаРаздела11.СвРегНом) Тогда
			Для ИндНом = 0 По СтрокаРаздела11.СвРегНом.ВГраница() Цикл
				ЭлементРНПТ = СтрокаРаздела11.СвРегНом[ИндНом];
		...
		ОбластьЗапись = ПараметрыРаздела.Часть2Строка.Область("ОбластьЗаписьОсновнаяЧасть2");
		ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии);
		Если НЕ ЗначениеЗаполнено(СтрокаРаздела11.СвРегНом)
			ИЛИ СтрокаРаздела11.СвРегНом.Количество() = 1 Тогда
			ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
		КонецЕсли;
	КонецЕсли;
	
	ПараметрыРаздела.Часть2Строка.Область(1, ,
		ПараметрыРаздела.Часть2Строка.ВысотаТаблицы).Имя = "Строка_" + Формат(ПараметрыРаздела.НомерПП, "ЧГ=");
		
	ПараметрыРаздела.ТабличныйДокумент.Вывести(ПараметрыРаздела.Часть2Строка);
	
	Если ЗначениеЗаполнено(СвРегНомРНПТ) Тогда
		Для ИндНом = 0 По СвРегНомРНПТ.ВГраница() Цикл
			ОбластьЗапись = ПараметрыРаздела.Часть2СтрокаРНПТ.Область("ОбластьЗаписьЧасть2");
			Если ИндНом = СвРегНомРНПТ.ВГраница() Тогда
				ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
			Иначе
				ОбластьЗапись.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии);
			...
			ЗаполнитьЗначенияСвойств(ПараметрыРаздела.Часть2СтрокаРНПТ.Параметры, СвРегНомРНПТ[ИндНом]);
			
			ПараметрыРаздела.ТабличныйДокумент.Вывести(ПараметрыРаздела.Часть2СтрокаРНПТ);
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела12(СтрокаРаздела12, СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП = 0 Тогда
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			ПараметрыРаздела.Вставить("Макет", РеквизитФормыВЗначение("Объект").ПолучитьМакет("СведенияИзСчетовФактур2025Кв1"));
		Иначе
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		ПараметрыСтроки.КодВидаОперации = МассивВСтрокуСРазделителями(СтрокаРаздела12.КодВидОпер, ";");
	КонецЕсли;
	
	НаименованиеВалюты = Неопределено;
	Валюта = ВалютаПоКодуСУчетомКэша(СтрокаРаздела12.ОКВ, НаименованиеВалюты, КэшВалют);
	Если Валюта.Пустая() Тогда
	...
	Если ЗначениеЗаполнено(СтрокаРаздела12.СвПокуп) Тогда
		СтрокаРаздела12.СвПокуп.Вставить("Контрагент",             Контрагент);
		СтрокаРаздела12.СвПокуп.Вставить("КонтрагентНаименование", СокрЛП(ПараметрыСтроки.КонтрагентНаименование));
	КонецЕсли;
	...
	ДополнительныеСведения.ПредставлениеДокумента = СтрШаблон(
		...
		Формат(СтрокаРаздела12.СтоимТовСНалВс, "ЧДЦ=2; ЧН="),
		Строка(Валюта));
	ДополнительныеСведения.Документ = Неопределено;
...
Процедура КорректироватьИтоги81_91(СтруктураПараметров)
	...
	СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_20 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС20 + СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_20 - СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр20;
	СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр20   = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС20;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_7 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС7 + СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_7 - СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр7;
		СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр7   = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС7;
		
		СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_5 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС5 + СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_5 - СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр5;
		СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр5   = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС5;
	КонецЕсли;
	...
	СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_20 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр20  + СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_20 - СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр20;
	СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр20   = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр20;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
		СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_7 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр7  + СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_7 - СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр7;
		СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр7   = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр7;
		
		СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_5 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр5  + СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_5 - СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр5;
		СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр5   = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр5;
	КонецЕсли;
...
Процедура ВывестиШапкуРаздела91Декларации(ТабличныйДокумент, Макет, ДатаОформления, НомерДополнительногоЛиста)
	...
	НайденнаяОбласть = Секция.Области.Найти("НомераГрафКнига");
	Если НайденнаяОбласть <> Неопределено Тогда
		НайденнаяОбласть.Видимость = Ложь;
	КонецЕсли;
	НайденнаяОбласть = Секция.Области.Найти("НомераГрафДекларация");
	Если НайденнаяОбласть <> Неопределено Тогда
		НайденнаяОбласть.Видимость = Ложь;
	КонецЕсли;
	ТабличныйДокумент.Вывести(Секция);
