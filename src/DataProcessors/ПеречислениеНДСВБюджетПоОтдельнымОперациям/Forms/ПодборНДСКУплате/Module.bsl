...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ ПеренестиВДокумент И Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru = 'Подобранные к уплате суммы налога не перенесены в документ.
			|
			|Перенести?';
			|en = 'Tax amounts selected for payment are not transferred to the document.
			|
			|Transfer them?'");
...
Процедура УстановитьТекстПодсказки(Форма)
	...
	ТекстСообщенияПодобрано = НСтр("ru = 'Всего подобрано к уплате [Подобрано] руб.';
									|en = 'Total selected for payment [Подобрано] rub.'");
...
Процедура ЗаполнитьЗадолженностьНаСервере()
	...
	//++ НЕ УТ
	Если ТипНалога = Перечисления.ТипыНалогов.НДС_ВвозимыеТовары Тогда
		ЗадолженностьПоСрокам = Обработки.ПеречислениеНДСВБюджетПоОтдельнымОперациям.ЗадолженностьПоУплатеНДСПоВвозуТоваров(РеквизитыДокумента);
	Иначе
		ЗадолженностьПоСрокам = Обработки.ПеречислениеНДСВБюджетПоОтдельнымОперациям.ЗадолженностьНалоговогоАгентаПоНДС(РеквизитыДокумента);
	КонецЕсли;
