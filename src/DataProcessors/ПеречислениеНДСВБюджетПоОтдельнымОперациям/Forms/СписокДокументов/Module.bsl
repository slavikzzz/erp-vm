...
Процедура СписокДокументыКОплатеПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
...
Процедура ОтправитьВКонтролирующийОрган(Команда)
	...
	//++ НЕ УТ
	Если Элементы.СписокДокументыКОплате.ТекущаяСтрока <> Неопределено Тогда
		
		ТекущиеДанные = Элементы.СписокДокументыКОплате.ТекущиеДанные;
...
Функция ОписаниеОперацийИТиповДокументов(ТолькоЗаявленияОВвозе = Ложь)
	...
	Если Не ТолькоЗаявленияОВвозе Тогда
		...
		Строка.ЗаголовокРабочегоМеста 		= НСтр("ru = 'Документы к уплате НДС в бюджет (Счета-фактуры налоговый агент)';
													|en = 'Documents on VAT payable to the budget (tax invoices tax agent)'");
		...
		Строка.ЗаголовокРабочегоМеста = НСтр("ru = 'Документы к уплате НДС в бюджет (Таможенные декларации на импорт)';
											|en = 'Documents on VAT payable to the budget (Customs import declarations)'");
	...
	Строка.ЗаголовокРабочегоМеста = НСтр("ru = 'Документы к уплате НДС в бюджет (Заявления о ввозе товаров)';
										|en = 'Documents on VAT payable to the budget (Applications for goods import)'");
...
Процедура СформироватьДокументыНаОсновании(ИмяДокумента);
	...
	Для Каждого ОрганизацияОснования Из МассивОрганизацийОснований Цикл
		...
		//++ НЕ УТ
		Если МассивСсылокСчетФактураНалоговыйАгент.Количество() <> 0 Тогда
			ОткрытьФорму(ИмяДокумента + ".ФормаОбъекта", Новый Структура("Основание", МассивСсылокСчетФактураНалоговыйАгент), , Истина);
		КонецЕсли;
		Если МассивСсылокЗаявлениеОВвозе.Количество() <> 0 Тогда
			ОткрытьФорму(ИмяДокумента + ".ФормаОбъекта", Новый Структура("Основание", МассивСсылокЗаявлениеОВвозе), , Истина);
		КонецЕсли;
...
Процедура НастроитьФормуПоВыбраннымОперациямИДокументам(ТЗХозОперацииИТипыДокументов)
	...
	ДанныеРабочегоМеста = ОбщегоНазначенияУТ.ДанныеРабочегоМеста(ТЗХозОперацииИТипыДокументов,
		КлючНазначенияФормыПоУмолчанию(), НСтр("ru = 'Документы к уплате НДС в бюджет (все)';
												|en = 'Documents on VAT payable to the budget (all)'"));
