...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СерииИспользуются = ОбщегоНазначенияИС.СерииИспользуются();
	
	Если Не ЗначениеЗаполнено(Документ) Тогда
		Документ = ПараметрыСканирования.СсылкаНаОбъект;
	КонецЕсли;
	
	ЭтоКонтекстОптовогоДокумента   = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ЭтоВидОперацииОптоваяПродажа(ПараметрыСканирования.ВидОперацииИСМП)
		Или ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ЭтоВидОперацииЗакупка(ПараметрыСканирования.ВидОперацииИСМП);
		
	ЭтоКонтекстРозничногоДокумента = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ЭтоВидОперацииРозничнаяПродажа(ПараметрыСканирования.ВидОперацииИСМП);
	...
	ДобавлятьБезКодаМаркировки = Ложь;
	
	Если ПоддерживаетсяОбъемноСортовойУчет Тогда
		ДобавлятьБезКодаМаркировки = Истина;
	ИначеЕсли Не ПараметрыСканирования.ТолькоМаркируемаяПродукция
		И Параметры.РазрешатьДобавлениеБезКодаМарки Тогда
		
		Если ОбщегоНазначенияИСМППовтИсп.РазрешенОборотНемаркируемыхОстатковПродукции(ВидМаркируемойПродукции) Тогда
			ДобавлятьБезКодаМаркировки = Истина;
		Иначе
			
			Если ЭтоКонтекстРозничногоДокумента
				И Не ОбщегоНазначенияИСМППовтИсп.ДействуетУведомительныйРежимПоВидуПродукции(ВидМаркируемойПродукции) Тогда
				
				ДобавлятьБезКодаМаркировки = Истина;
			
			ИначеЕсли ЭтоКонтекстОптовогоДокумента
				И Не ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПриПродажеОптомПоОСУ(ВидМаркируемойПродукции)
				И Не ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПриПродажеОптомПоштучно(ВидМаркируемойПродукции) Тогда
...
Процедура ДобавитьБезКодаМаркировки(Команда)
	...
	Если Не ПоддерживаетсяОбъемноСортовойУчет Тогда
		Комментарий = Нстр("ru = 'Отказ ввода кода маркировки по инициативе пользователя';
							|en = 'Отказ ввода кода маркировки по инициативе пользователя'");
...
Процедура ОбновитьПредставленияНоменклатуры()
	...
	Представление = ОбщегоНазначенияИСКлиентСервер.ПредставлениеНоменклатурыФорматированнойСтрокой(ПараметрыПредставленияНоменклатуры);
	...
	Если ПоддерживаетсяОбъемноСортовойУчет Тогда
		...
		Если ИнтеграцияИСМП.ЭтоТестовыйПериод(ВидМаркируемойПродукции) Тогда
			ДанныеТекстаПодсказки.Добавить(НСтр("ru = 'Если код маркировки не читается или отсутствует или требуется объемно-сортовой учет - используйте команду';
												|en = 'Если код маркировки не читается или отсутствует или требуется объемно-сортовой учет - используйте команду'"));
		Иначе
			ДанныеТекстаПодсказки.Добавить(НСтр("ru = 'При объемно-сортовом учете - используйте команду';
												|en = 'При объемно-сортовом учете - используйте команду'"));
		КонецЕсли;
		ДанныеТекстаПодсказки.Добавить(" ");
		ДанныеТекстаПодсказки.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru = 'Добавить без кода маркировки';
					|en = 'Добавить без кода маркировки'"),
...
Процедура ДобавитьСообщениеДляЖурналаРегистрацииОНевозможностиВводаКодаМарки(Комментарий, КодМаркировки="")
	...
	СтруктураСообщения.Вставить("ИмяСобытия",     НСтр("ru = 'Отказ от сканирования кода маркировки';
														|en = 'Отказ от сканирования кода маркировки'"));
