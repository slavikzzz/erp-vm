...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ВыполняетсяЗакрытие
		...
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ru = 'Не переносить и закрыть';
													|en = 'Do not transfer and close'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		ПоказатьВопрос(Новый ОписаниеОповещения("ПослеВопросаОЗакрытии", ЭтотОбъект),
			НСтр("ru = 'Выбранные транспортные средства не перенесены в список формируемых заданий. Перенести?';
				|en = 'The selected vehicles have not been transferred to the generated job list. Transfer?'"), Кнопки);
		...
		Для Каждого ВлСтр Из МассивЗаданийНаПеревозку Цикл
			Если НачалоДня(ВлСтр.ВремяС) = НачалоДня(ВлСтр.ВремяПо) Тогда
				СтрЗаданияНаПеревозку = СтрЗаданияНаПеревозку + ?(СтрЗаданияНаПеревозку = "","",Символы.ПС) + НСтр("ru = 'С';
																													|en = 'FROM'")
										+ Символы.НПП + Формат(ВлСтр.ВремяС, Нстр("ru = 'ДФ=""ЧЧ:мм""';
																					|en = 'DF=""HH:mm""'")) + Символы.НПП + НСтр("ru = 'до';
																															|en = 'to'")
										+ Символы.НПП + Формат(ВлСтр.ВремяПо, Нстр("ru = 'ДФ=""ЧЧ:мм""';
																					|en = 'DF=""HH:mm""'")) + " (" + ВлСтр.Статус + ")"
			Иначе
				СтрЗаданияНаПеревозку = СтрЗаданияНаПеревозку + ?(СтрЗаданияНаПеревозку = "","",Символы.ПС) + НСтр("ru = 'С';
																													|en = 'FROM'")
										+ Символы.НПП + Формат(ВлСтр.ВремяС, Нстр("ru = 'ДФ=""ЧЧ:мм""';
																					|en = 'DF=""HH:mm""'")) + Символы.НПП + НСтр("ru = 'весь день (';
																															|en = 'all day ('") + ВлСтр.Статус + ")"
			КонецЕсли;
		КонецЦикла;
