...
Процедура ЗаполнитьДерево()
	...
	СоответствиеАдрес = ПоместитьВоВременноеХранилище(СоответствиеУИД, УникальныйИдентификатор);
КонецПроцедуры
...
Процедура УведомленияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ИмяФормыУведомления) Тогда
		...
		ОткрытьФорму(ИмяФормыУведомления, ПараметрыОткрытия, ЭтотОбъект, ВыбраннаяСтрока);
	Иначе
	...
	Если ЗначениеЗаполнено(ВидСоздаваемогоДокумента) Тогда
		...
		Если ЗначениеЗаполнено(ИмяФормыСообщения) Тогда
			Если ЗначениеЗаполнено(Организация) Тогда 
				...
				ОткрытьФорму(ИмяФормыСообщения, ПараметрыОткрытия, ВладелецФормы);
			Иначе
				ПараметрыОткрытия = Новый Структура("Тип", ВидСоздаваемогоДокумента);
				ФормаОрганизации = ОткрытьФорму("Документ.УведомлениеОСпецрежимахНалогообложения.Форма.ФормаВыбораУведомления", ПараметрыОткрытия, ЭтотОбъект);
			КонецЕсли;
		КонецЕсли;
	Иначе 
		ФормаВыбор = ОткрытьФорму("Документ.УведомлениеОСпецрежимахНалогообложения.Форма.ФормаВыбораВидаСообщения", Новый Структура("Организация,Создание_УведомлениеОСпецрежимахНалогообложения", Организация), ЭтотОбъект);
	КонецЕсли;
