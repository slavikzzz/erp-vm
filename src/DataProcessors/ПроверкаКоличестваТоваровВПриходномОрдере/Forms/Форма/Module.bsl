	...
	Если Не ЗакрытьБезВопроса
		...
		ПоказатьВопрос(ОписаниеОповещения,
			НСтр("ru = 'Завершить проверку без переноса расхождений в документ?';
				|en = 'Finish checking without discrepancy transfer to the document?'"), КнопкиВопроса);
...
Процедура ЗаполнитьПоОрдеру(Команда)
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Список принятых товаров будет перезаполнен по документу.
			...
			|en = 'List of received goods will be refilled in the document.
			|Populated data will be cleared, continue?'");
		...
		Для Каждого ПровереннаяСтрока Из Товары Цикл
			...
			Если Остаток > 0 Тогда
				...
				НоваяСтрока.Комментарий = НСтр("ru = 'Строка добавлена по результатам проверки';
												|en = 'The line is added based on the check results'");
