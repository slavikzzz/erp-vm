...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("ru = 'Универсальный обмен данными в формате XML (%1)';
							|en = 'Conversion Rule Data Exchange in XML format (%1)'");
	...
	Если СохраненныйРежимЗагрузки
		...
		Если Объект.НастройкаАвтоматическойЗагрузкиДанных = 1 Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Выполнить загрузку данных из файла обмена?';
													|en = 'Do you want to import data from the exchange file?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура ИмяФайлаПравилПослеПроверкиСуществования(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Не Существует Тогда
		СообщитьПользователю(НСтр("ru = 'Не найден файл правил обмена';
									|en = 'Exchange rules file not found'"), "ИмяФайлаПравил");
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Загрузить правила обмена данными?';
											|en = 'Do you want to import data exchange rules?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура УдалениеУдалить(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Удалить выбранные данные в информационной базе?';
											|en = 'Do you want to delete the selected data from the infobase?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура ОткрытьФайлЗагрузкиНаСервере(АдресФайла)
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() Тогда
			...
			СообщитьПользователю(НСтр("ru = 'Не найден файл обмена на сервере.';
										|en = 'Exchange file not found on the server.'"), "ИмяФайлаОбмена");
			Возврат;
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Универсальный обмен данными в формате XML';
										|en = 'Conversion Rule Data Exchange in XML format'", ОбъектДляСервера.КодОсновногоЯзыка()),
...
Процедура ПолучитьФайлВыгрузкиНаКлиенте(Адрес)
	...
	ДиалогВыбора.Заголовок = НСтр("ru = 'Выберите путь для сохранения файла выгрузки';
									|en = 'Select a path to save the export file'");
...
Процедура ИзменитьРежимОбработки(РежимРаботы)
	...
	?(РежимРаботы, НСтр("ru = 'Режим работы (на клиенте)';
						|en = 'Mode (client)'"), НСтр("ru = 'Режим работы (на сервере)';
																	|en = 'Mode (server)'"));
...
Процедура ПослеПодключения(Подключено, УстановитьЕслиНеПодключено) Экспорт
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Не удалось установить или подключить расширение работы с файлами';
								|en = 'Cannot install or connect 1C:Enterprise Extension'");
