...
Функция одТипЗначенияСтрокой(ЗначениеИлиТип) Экспорт
	...
	Иначе
		...
		Если Менеджер = Неопределено Тогда
			...
			Текст= НСтр("ru = 'Неизвестный тип:';
						|en = 'Unknown type:'") + Строка(ТипЗнч(ТипЗначения));
...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаПользователю = ?(ТолькоПолучитьУзелСсылки, НСтр("ru = 'Конвертация ссылки на объект: %1';
																|en = 'Converting object reference: %1'"), НСтр("ru = 'Конвертация объекта: %1';
...
Процедура ПередатьИнформациюОЗаписиВПриемник(ИнформацияДляЗаписиВФайл, СтрокаОшибкиВБазеПриемнике = "") Экспорт
	...
	Если Не ПустаяСтрока(СтрокаОшибкиВБазеПриемнике) Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru = 'Загрузка в приемнике: %1';
															|en = 'Importing in destination: %1'"), СтрокаОшибкиВБазеПриемнике);
			...
			Исключение
				...
				ВызватьИсключение
					НСтр("ru = 'Ошибка при подключении внешней обработки отладки.';
						|en = 'An error occurred when attaching an external debug data processor.'");
	...
	Если Не ПустаяСтрока(ИмяВременногоФайла) Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Универсальный обмен данными в формате XML';
											|en = 'Conversion Rule Data Exchange in XML format'", КодОсновногоЯзыка()),
	...
	Если Не ПустаяСтрока(КодировкаФайлаПротоколаОбмена) Тогда
		Если СтрНачинаетсяС(ПредставлениеКодировки, "КодировкаТекста.") Тогда
			...
			Исключение
				...
				|The supported encoding is ANSI.'"), ПредставлениеКодировки);
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Универсальный обмен данными в формате XML';
												|en = 'Conversion Rule Data Exchange in XML format'", КодОсновногоЯзыка()),
...
Процедура КомментарииКЗагрузкеОбъекта(НПП, ИмяПравила, Источник, ТипОбъекта, ГНПП = 0)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		Если НПП <> 0 Тогда
			СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru = 'Загрузка объекта № %1';
																|en = 'Importing object #%1'"), НПП);
		Иначе
			СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru = 'Загрузка объекта № %1';
																|en = 'Importing object #%1'"), ГНПП);
		КонецЕсли;
...
Процедура ВыгрузкаОбъектаВыборки(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, ВыборкаДляВыгрузкиДанных = Неопределено)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru = 'Выгрузка объекта: %1';
															|en = 'Exporting object: %1'"), ПрОбъекта);
...
Процедура ВыгрузитьДанныеПоПравилу(Правило)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru = 'Правило выгрузки данных: %1 (%2)';
															|en = 'Data export rule: %1 (%2)'"), СокрЛП(Правило.Имя), СокрЛП(Правило.Наименование));
					...
					Попытка
						...
						Если Отказ = Истина Тогда
							...
							Если Не ПустаяСтрока(ПричинаОтказа) Тогда
								СтрокаИсключения = ПодставитьПараметрыВСтроку(НСтр("ru = 'Загрузка данных отменена по причине: %1';
																					|en = 'Data import canceled. Reason: %1'"), ПричинаОтказа);
...
Функция ВыполнитьДействияПередЧтениемДанных(СтрокаДанных = "") Экспорт
	...
	СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru = 'Начало загрузки: %1';
														|en = 'Import started at: %1'"), ТекущаяДатаСеанса());
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Окончание загрузки: %1';
			|en = 'Import finished at: %1'"), ТекущаяДатаСеанса()), , Ложь, , , Истина);
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Загружено объектов: %1';
			|en = 'Objects imported: %1'"), мСчетчикЗагруженныхОбъектов), , Ложь, , , Истина);
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Начало выгрузки: %1';
			|en = 'Export started at: %1'"), ТекущаяДатаСеанса()), , Ложь, , , Истина);
	...
	Исключение
		...
		ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при выгрузке данных: %1';
				|en = 'Data export error: %1'"), СтрокаОшибки), Неопределено, Истина, , , Истина);
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Окончание выгрузки: %1';
			|en = 'Export finished at: %1'"), ТекущаяДатаСеанса()), , Ложь, , ,Истина);
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Выгружено объектов: %1';
			|en = 'Objects exported: %1'"), мСчетчикВыгруженныхОбъектов), , Ложь, , , Истина);
...
Процедура ИнициализацияСообщений()
	...
	одСообщения.Вставить(66, НСтр("ru = 'Ошибка получения коллекции подчиненных объектов из входящих данных';
									|en = 'Cannot retrieve a collection of subordinate objects from incoming data'"));
	одСообщения.Вставить(67, НСтр("ru = 'Ошибка получения свойства подчиненного объекта из входящих данных';
									|en = 'Cannot retrieve a property of a subordinate object from incoming data'"));
	одСообщения.Вставить(68, НСтр("ru = 'Ошибка получения свойства объекта из входящих данных';
									|en = 'Cannot retrieve an object property from incoming data'"));
	...
	одСообщения.Вставить(72, НСтр("ru = 'Ошибка при выгрузке данных для узла плана обмена';
									|en = 'Cannot export data to the exchange plan node'"));
...
Функция ОбщийМодуль(Имя) Экспорт
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ru = 'Общий модуль ""%1"" не найден.';
															|en = 'Common module ""%1"" is not found.'"), Имя);
	КонецЕсли;
