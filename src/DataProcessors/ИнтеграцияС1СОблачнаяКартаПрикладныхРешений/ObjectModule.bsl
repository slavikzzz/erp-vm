...
Процедура ПолучитьДанныеФунциональнойМодели(Параметры, АдресРезультата, АдресДополнительногоРезультата) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Получение данных функциональной модели 1С:Облачная карта прикладных решений';
										|en = 'Receiving data of functional model 1C:Cloud Map of applications'", 
...
Функция ПолучитьПрокси()
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = '1С:Облачная карта прикладных решений';
										|en = '1C:Cloud Map of applications'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка,,, 
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Веб-сервис %1 в настоящий момент недоступен.
								...
								|en = 'Web service %1 is not available at the moment.
								|There might be no network connection, the web server may be overloaded or maintenance check is performed. Please try again later.
								|
								|IMPORTANT! Network connection is required to connect to ASDS OR/SR functional model.
	...
	Если ПустаяСтрока(Проект) Тогда
		...
		ВызватьИсключение 
			НСтр("ru = 'Ошибка подключения к СППР.
					...
					|en = 'An error occurred when connecting to ASDS.
					|
					|Project is not specified in settings of integration with ASDS.'");
