...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	Элементы.ИзменитьВалютуПоиска.Заголовок = СтрШаблон(НСтр("ru = 'Валюта: %1';
															|en = 'Currency: %1'"), Валюта);
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	Если Контрагенты.Количество() И ТекущиеДанные.КоличествоПредложений > 0 Тогда
		...
		МинЦена = ?(ТекущиеДанные.МинимальнаяЦена = 0, НСтр("ru = 'по запросу';
															|en = 'by request'"), ТекущиеДанные.МинимальнаяЦена);
		МаксЦена = ?(ТекущиеДанные.МаксимальнаяЦена = 0, НСтр("ru = 'по запросу';
																|en = 'by request'"), ТекущиеДанные.МаксимальнаяЦена);
		
		Если МинЦена = МаксЦена Тогда
			Если ТипЗнч(МаксЦена) = Тип("Число") Тогда
				ДиапазонЦен = СтрШаблон(НСтр("ru = 'цена %1 %2';
											|en = 'price %1 %2'"), МинЦена, Валюта);
			Иначе // по запросу.
				ДиапазонЦен = СтрШаблон(НСтр("ru = 'цена %1';
											|en = 'price %1'"), МинЦена);
			КонецЕсли;
		Иначе
			ДиапазонЦен = СтрШаблон(НСтр("ru = 'цены %1 - %2 %3';
										|en = 'prices %1 - %2 %3'"), МинЦена,
				МаксЦена, Валюта);
		КонецЕсли;
		
		МинСрокПоставки = ?(ТекущиеДанные.МинимальныйСрокПоставки = 0, НСтр("ru = 'по запросу';
																			|en = 'by request'"), ТекущиеДанные.МинимальныйСрокПоставки);
		МаксСрокПоставки = ?(ТекущиеДанные.МаксимальныйСрокПоставки = 0, НСтр("ru = 'по запросу';
																				|en = 'by request'"), ТекущиеДанные.МаксимальныйСрокПоставки);
		Если МинСрокПоставки = МаксСрокПоставки Тогда
			...
			ИначеЕсли ТипЗнч(МаксСрокПоставки) = Тип("Число") Тогда
				ДиапазонПоставки = СтрШаблон(НСтр("ru = 'поставка %1 дн.';
													|en = 'delivery %1 days'"), МинСрокПоставки);
			Иначе // по запросу.
				ДиапазонПоставки = СтрШаблон(НСтр("ru = 'поставка %1';
													|en = 'delivery %1'"), МинСрокПоставки);
			КонецЕсли;
		Иначе
			ДиапазонПоставки = СтрШаблон(НСтр("ru = 'поставка %1 - %2 дн.';
												|en = 'delivery %1 - %2 days'"), МинСрокПоставки,
				МаксСрокПоставки);
		КонецЕсли;
		
		ИнформацияПоТовару = СтрШаблон(НСтр("ru = 'Всего предложений %1, %2, %3';
											|en = 'Total offers %1, %2, %3'"), ТекущиеДанные.КоличествоПредложений,
	...
	Если Список.НайтиСтроки(Новый Структура("Пометка", Истина)).Количество() Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ИзменитьВалютуПоискаПродолжение", ЭтотОбъект),
			НСтр("ru = 'При изменении валюты, будут удалены подобранные товары и выполнен новый поиск торговых предложений.';
				|en = 'If currency is changed, selected goods are deleted and a new search of selling propositions is performed.'"),
...
Функция НайтиТорговыеПредложенияВФоне(Знач ИдентификаторФормы, Отказ)
	...
	Задание.Наименование = НСтр("ru = '1С:Бизнес-сеть. Поиск торговых предложений по списку.';
								|en = '1C:Business Network. Search for selling propositions by list.'");
...
Процедура ЗагрузитьРезультатыПоиска(АдресРезультата)
	...
	Для каждого ЭлементСписка Из Контрагенты Цикл
		...
		НовыйЭлемент.Заголовок = НСтр("ru = 'Группа выбора';
										|en = 'Selection group'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		НовыйЭлемент.Заголовок = НСтр("ru = 'Выбор';
										|en = 'Choice'", ОбщегоНазначения.КодОсновногоЯзыка());
		НовыйЭлемент.ОтображатьВШапке = Ложь;
		...
		НовыйЭлемент.Формат = "Ч " + НСтр("ru = 'дн.';
											|en = 'days'");
		НовыйЭлемент.Заголовок = НСтр("ru = 'Срок поставки';
										|en = 'Delivery period'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Для Каждого СтрокаСписка Из ВыбранныеСтроки Цикл
		...
		Если СтрокаСписка.Количество = 0 Тогда
			ПоказатьПредупреждение( , НСтр("ru = 'Не указано количество потребности';
											|en = 'Demand quantity is not specified'"));
			Продолжить;
		КонецЕсли;
...
Процедура СоздатьКорзинуДобавитьТовары(ДанныеПредложений)
	...
	ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Формирование корзины';
											|en = 'Generate a shopping cart'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементУсловногоОформления.Представление = НСтр("ru = 'Отключение доступности выбора характеристик для номенклатуры без характеристик';
													|en = 'Disabling the availability of characteristics selection for products without characteristics'");
...
Процедура НайтиТорговыеПредложения()
	...
	ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Поиск торговых предложений.';
											|en = 'Search for selling propositions.'");
...
Процедура ИзменитьВалютуПоискаПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		ПоказатьВводЗначения(Новый ОписаниеОповещения("ИзменитьВалютуПоискаЗавершение", ЭтотОбъект), Валюта,
			НСтр("ru = 'Валюта поиска';
				|en = 'Search currency'"), Тип("СправочникСсылка.Валюты"));
	КонецЕсли;
...
Процедура ПоказатьДиалогОшибкиПоиска(ОшибкиПараметров = Ложь)
	...
	Если ОшибкиПараметров Тогда
		...
		СтрокиСообщения.Добавить(НСтр("ru = 'Торговые предложения не найдены, причина:';
										|en = 'Trade offers are not found due to:'"));
