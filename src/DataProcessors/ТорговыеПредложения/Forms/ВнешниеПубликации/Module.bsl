...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПоказатьПредупреждение(, НСтр("ru = 'В форме отображается список внешних публикаций с возможностью удаления в сервисе 1С:Торговая площадка
		...
		|en = 'In the form, the list of external publications that can be deleted in 1C:Trading Platform service is displayed
		|You can view information about an external publication of trade offers
		|in the publication program (application) only.'"),,
		НСтр("ru = 'Внешняя публикация торговых предложений';
			|en = 'External publication of trade offers'"));
...
Процедура УдалитьПубликацию(Команда)
	...
	Если Элементы.Список.ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбраны строки для выполнения команды';
										|en = 'Lines to execute the command are not selected'"));
		Возврат;
	КонецЕсли;
	...
	ПоказатьВопрос(УдалитьПубликациюПродолжение,
		НСтр("ru = 'Выбранные публикации будут удалены в сервисе 1С:Торговая площадка.
			...
			|en = 'Selected publications will be deleted in 1C:Trading Platform service.
			|The operation cannot be reversed. Continue?'"), РежимДиалогаВопрос.ДаНет);
