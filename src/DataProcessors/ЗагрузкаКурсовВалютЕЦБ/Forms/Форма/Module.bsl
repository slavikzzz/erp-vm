...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(БазоваяВалюта) Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Для использования загрузки курсов с сайта ЕЦБ, в справочнике ""Валюты"" необходимо завести валюту Евро (EUR) с кодом 978';
				|en = 'To use exchange rate import from the ECB website, enter Euro (EUR) currency with code 978 in the ""Currencies"" catalog'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если НЕ Параметры.Свойство("ОткрытиеИзСписка") Тогда
		Если РаботаСКурсамиВалютУТ.ОтносительныеКурсыАктуальны(БазоваяВалюта) Тогда
...
Процедура ЗаполнитьВалюты()
	...
	ЗагружаемыеВалюты = РаботаСКурсамиВалютУТ.ЗагружаемыеВалюты();
...
Процедура ЗаполнитьДанныеСтрокиТаблицыНаОсновеВалюты(СтрокаТаблицы, Валюта);
	...
	ДанныеКурса = РаботаСКурсамиВалютУТ.ЗаполнитьДанныеКурсаЕЦБДляВалюты(Валюта, БазоваяВалюта);
	
	Если ТипЗнч(ДанныеКурса) = Тип ("Структура") Тогда
		СтрокаТаблицы.ДатаКурса       = ДанныеКурса.ДатаКурса;
		СтрокаТаблицы.КурсЧислитель   = ДанныеКурса.КурсЧислитель;
		СтрокаТаблицы.КурсЗнаменатель = ДанныеКурса.КурсЗнаменатель;
	КонецЕсли;
...
Процедура ПроверитьСписокЗагружаемыхВалют()
	Если Объект.СписокВалют.Количество() = 0 Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'В справочнике валют отсутствуют валюты, курсы которых можно загружать из сети Интернет.';
									|en = 'The list of currencies contains no currencies whose exchange rates are imported from the internet.'");
...
Функция ВыполнитьЗагрузкуКурсов()
	...
	РегламентноеЗадание = Метаданные.РегламентныеЗадания.ЗагрузкаКурсовВалютЕЦБ;
...
Процедура ОбработатьРезультатЗагрузки(РезультатЗагрузки)
	...
	Если ЕстьУспешноЗагруженныеКурсы Тогда
		...
		КонецЦикла;
		Оповестить("Запись_ЗагрузкаОтносительныхКурсовВалют", ПараметрыЗаписи, МассивОбновляемыхВалют);
	...
	Если КоличествоОшибок > 1 Тогда
		...
		Кнопки.Добавить("Продолжить", НСтр("ru = 'Продолжить';
											|en = 'Continue'"));
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось загрузить курсы валют ЕЦБ (%1).';
																					|en = 'Cannot import currency ECB exchange rates (%1).'"), КоличествоОшибок);
...
Процедура ОбработатьРезультатЗагрузкиПриОтветеНаВопрос(РезультатВопроса, ПредставлениеОшибки) Экспорт
	...
	ОткрытьФорму("Обработка.ЗагрузкаКурсовВалютЕЦБ.Форма.СообщенияОбОшибках", Новый Структура("Текст", ПредставлениеОшибки));	
КонецПроцедуры
	...
	Если Не ЗначениеЗаполнено(Объект.НачалоПериодаЗагрузки) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не задана дата начала периода загрузки.';
				|en = 'Please specify the start date of the import period.'"),
			,
	...
	Если Не ЗначениеЗаполнено(Объект.ОкончаниеПериодаЗагрузки) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не задана дата окончания периода загрузки.';
				|en = 'Import period end date is not set.'"),
			,
