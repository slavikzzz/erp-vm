			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При создании кадровых приказов не удалось провести приказ об увольнении сотрудника %1, необходимо выполнить проведение в ручном режиме.';
						|en = 'Cannot post a termination of employment order of employee %1 when creating personnel orders. Post it manually.'"),
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При создании кадровых приказов не удалось провести приказ о приеме сотрудника %1, необходимо выполнить проведение в ручном режиме.';
						|en = 'Cannot post a hiring order of employee %1 when creating personnel orders. Post it manually.'"),
					ДанныеСотрудников.Сотрудник);
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В функции ""%1"", в параметре № 2 ""%2"" не обслуживается значение ""%3"".';
				|en = 'The ""%3"" value is not supported in the ""%1"" function, the ""%2"" parameter No. 2.'"),
...
Процедура ЗаполнитьНастройкиПечатныхФормПоУмолчанию(ОписанияНастроек) Экспорт
	...
	КадровыйЭДО.ДобавитьНастройкуПечатнойФормыПоМетаданным(
		ОписанияНастроек,
		Метаданные.ОбщиеМакеты.ПФ_MXL_ТрудовойДоговорМикропредприятий,
	...
	КадровыйЭДО.ДобавитьНастройкуПечатнойФормыПоМетаданным(
		ОписанияНастроек,
		Метаданные.ОбщиеМакеты.ПФ_MXL_Т1,
		Перечисления.СодержимоеДокументов.СодержитТарифныеСтавки);
	
	КадровыйЭДО.ДобавитьНастройкуПечатнойФормыПоМетаданным(
		ОписанияНастроек,
		Метаданные.ОбщиеМакеты.ПФ_MXL_Т5,
		Перечисления.СодержимоеДокументов.СодержитТарифныеСтавки);
...
Функция ЗаполнитьДанныеДокументовПоИностраннымГражданам(ДанныеДляЗаполнения, СсылкаНаДокумент) Экспорт
	...
	Если ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ПриемНаРаботу") Тогда
		ДанныеДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаДокумент,
			"Организация,Сотрудник,ДатаПриема,Должность,РазрешениеНаРаботу");
		ДанныеДляЗаполнения.Организация = ДанныеДокумента.Организация;
		ДанныеДляЗаполнения.ДатаСведений = ДанныеДокумента.ДатаПриема;
		ДанныеДляЗаполнения.Сотрудник = ДанныеДокумента.Сотрудник;
		ДанныеДляЗаполнения.Должность = ДанныеДокумента.Должность;
		ДанныеДляЗаполнения.РазрешениеНаРаботу = ДанныеДокумента.РазрешениеНаРаботу;
		ДанныеДляЗаполнения.ТрудовойДоговор = Истина;
	ИначеЕсли ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ДоговорРаботыУслуги") Тогда
		ДанныеДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаДокумент,
	...
	ИначеЕсли ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.Увольнение") Тогда
		ДанныеДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаДокумент,
			"Организация,Сотрудник,ДатаУвольнения");
		ДанныеДляЗаполнения.Организация = ДанныеДокумента.Организация;
		ДанныеДляЗаполнения.ДатаСведений = ДанныеДокумента.ДатаУвольнения;
		ДанныеДляЗаполнения.Сотрудник = ДанныеДокумента.Сотрудник;
		ДанныеДляЗаполнения.ТрудовойДоговор = Истина;
		КадровыеДанные = КадровыйУчетВызовСервера.КадровыеДанныеСотрудника(
			ДанныеДляЗаполнения.Сотрудник, "ПриказОПриеме", ДанныеДляЗаполнения.ДатаСведений);
		Если ЗначениеЗаполнено(КадровыеДанные.ПриказОПриеме)
			И ТипЗнч(КадровыеДанные.ПриказОПриеме) = Тип("ДокументСсылка.ПриемНаРаботу") Тогда
		
			ДанныеПриказаОПриеме = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(КадровыеДанные.ПриказОПриеме,
				"РазрешениеНаРаботу");
			ДанныеДляЗаполнения.РазрешениеНаРаботу = ДанныеПриказаОПриеме.РазрешениеНаРаботу;
		КонецЕсли;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция КритерииПоискаСотрудниковПоКоллекцииОтборов(Отборы) Экспорт
	...
	ЕстьПодсистемаОбразованияФизическихЛиц = ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ОбразованияФизическихЛиц");
	Если ЕстьПодсистемаОбразованияФизическихЛиц Тогда
		МодульОбразованияФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("ОбразованияФизическихЛиц");
	КонецЕсли;
	
	ЕстьПодсистемаВоинскийУчет = ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ВоинскийУчет");
	Если ЕстьПодсистемаВоинскийУчет Тогда
	...
	Для каждого УсловиеПоиска Из Отборы Цикл
		...
		ИначеЕсли ДобавитьКритерийПоискаСправочникаФизическиеЛица(КритерииПоиска, УсловиеПоиска)
			...
			Или ЕстьПодсистемаБухучет
				И МодульОтражениеЗарплатыВБухучете.ДобавитьКритерийПоискаПоСведениямОбУчетеЗатрат(КритерииПоиска, УсловиеПоиска)
			Или ЕстьПодсистемаОбразованияФизическихЛиц
				И МодульОбразованияФизическихЛиц.ДобавитьКритерийПоискаПоСведениямОбОбразованииФизическихЛиц(КритерииПоиска, УсловиеПоиска)
			Или ЕстьПодсистемаОбразованияФизическихЛиц
				И МодульОбразованияФизическихЛиц.ДобавитьКритерийПоискаПоСведениямОЗнанииЯзыковФизическихЛиц(КритерииПоиска, УсловиеПоиска)
			Или ЕстьПодсистемаОбразованияФизическихЛиц
				И МодульОбразованияФизическихЛиц.ДобавитьКритерийПоискаПоСведениямОПрофессияхФизическихЛиц(КритерииПоиска, УсловиеПоиска)
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьКритерийПоискаПоСведениямОВоинскомУчетеФизическихЛиц(КритерииПоиска, УсловиеПоиска)
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьКритерийПоискаПоСведениямОБронированииГражданОрганизациями(КритерииПоиска, УсловиеПоиска) Тогда
		...
		Если ИмяПоля <> ВРег("ДатаНачала")
			...
			ТекстСообщенияИсключения = СтрШаблон(НСтр("ru = 'Неизвестное условие поиска - %1';
														|en = 'Unknown search condition: %1'"), ИмяПоля);
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Значение ""%1"" поля ""Таб. номер"" не уникально';
				|en = 'The ""%1"" value of the ""Pers. number"" field is not unique'"),
			ТабельныйНомер);
...
Процедура ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля) Экспорт
	...
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "ПодключенККЭДО";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru = 'Подключен к КЭДО';
										|en = 'Connected to HR EDI'");
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ИСТИНА";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".ПодключенККЭДО";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	
	УчетСтраховыхВзносовФизическиеЛица.ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
	УчетНДФЛФизическиеЛица.ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ОбразованияФизическихЛиц") Тогда
		МодульОбразованияФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("ОбразованияФизическихЛиц");
		МодульОбразованияФизическихЛиц.ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.СемейныеПоложенияФизическихЛиц") Тогда
		...
		МодульСемейныеПоложенияФизическихЛиц.ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ВоинскийУчет") Тогда
		МодульВоинскийУчет = ОбщегоНазначения.ОбщийМодуль("ВоинскийУчет");
		МодульВоинскийУчет.ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
	КонецЕсли;
...
Процедура ЗаполнитьПоляПредставленийКадровыхДанныхСотрудников(ДополнительныеПоляПредставлений, ПутьКПолямСотрудника, ПутьКПолямЛичныхДанных, ИсключаемыеПоля) Экспорт
	...
	КадровыйУчет.ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
	...
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "СогласиеНаПрисоединениеККЭДО";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru = 'Согласие на присоединение к КЭДО';
										|en = 'Consent to join HR EDI'");
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ЗНАЧЕНИЕ(Документ.СогласиеНаПрисоединениеККЭДО.ПустаяСсылка)";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".СогласиеНаПрисоединениеККЭДО";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "СогласиеНаПрисоединениеККЭДОСостояние";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru = 'Состояние';
										|en = 'State'");
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ЗНАЧЕНИЕ(Перечисление.СостоянияСогласийНаПрисоединениеККЭДО.ПустаяСсылка)";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".СогласиеНаПрисоединениеККЭДО.Состояние";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "СогласиеНаПрисоединениеККЭДОПодтвержден";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru = 'Подтвержден';
										|en = 'Confirmed'");
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ИСТИНА";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".СогласиеНаПрисоединениеККЭДО.Подтвержден";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "СогласиеНаПрисоединениеККЭДОДатаНачала";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru = 'Начало';
										|en = 'Start'");
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ДАТАВРЕМЯ(1, 1, 1)";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".СогласиеНаПрисоединениеККЭДО.ДатаНачала";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "СогласиеНаПрисоединениеККЭДОДатаОкончания";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru = 'Окончание';
										|en = 'End'");
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ДАТАВРЕМЯ(1, 1, 1)";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".СогласиеНаПрисоединениеККЭДО.ДатаОкончания";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	
	ЭлектронныеТрудовыеКнижки.ЗаполнитьПоляПредставленийКадровыхДанныхСотрудников(ДополнительныеПоляПредставлений, ПутьКПолямСотрудника, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
		...
		Пока ВыборкаКадроваяИстория.СледующийПоЗначениюПоля("Сотрудник") Цикл
			...
			Пока ВыборкаКадроваяИстория.Следующий() Цикл
				...
				СтруктураСтроки.Вставить("ОснованиеПеревода", НСтр("ru = 'Пр';
																	|en = 'Oth'") + ". № "
						+ НомерПриказа + " " + НСтр("ru = 'от';
													|en = 'dated'") + " " + Формат(ВыборкаКадроваяИстория.РегистраторДата, "ДЛФ=D"));
...
Процедура НастроитьВариантыОтчетаПоСотрудникам(Настройки, НастройкиОтчета) Экспорт
	...
	НастройкиВарианта.Описание =
		НСтр("ru = 'Паспортные данные, ИНН, СНИЛС, дата рождения и прочие личные данные.';
			|en = 'Passport data, TIN, SNILS, date of birth and other personal data.'");
...
Функция ЗапросВТПлановыеНачисленияСотрудников(ТолькоРазрешенные, ИмяВТПлановыеНачисленияСотрудников, ИмяВременнойТаблицыОтборовСотрудников, ПоляОтбораСотрудников, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень) Экспорт
	...
	Иначе
		...
		ТекстЗапроса =
			...
			|	ВЫБОР
			|		КОГДА НЕ КадровыеДанныеСотрудников.Подразделение ЕСТЬ NULL
			|			ТОГДА КадровыеДанныеСотрудников.Подразделение.РайонныйКоэффициент
			|		КОГДА НЕ КадровыеДанныеСотрудников.Организация ЕСТЬ NULL
			|			ТОГДА КадровыеДанныеСотрудников.Организация.РайонныйКоэффициент
			|		КОГДА НЕ КадровыеДанныеСотрудников.ГоловнаяОрганизация ЕСТЬ NULL
		...
		ТекстЗапроса =
			"ВЫБРАТЬ
			|	ДанныеПоСотрудникам.Период КАК Период,
			|	ДанныеПоСотрудникам.Период КАК ПериодЗаписи,
			|	ДанныеПоСотрудникам.Регистратор КАК Регистратор,
			...
			|	NULL КАК ОсновнойПоказатель,
			|	0 КАК ЗначениеОсновногоПоказателя,
			|	ДанныеПоСотрудникам.НачислениеТарифнойСтавки КАК НачислениеТарифнойСтавки,
...
Функция НеобходимыКадровыеДанныеФизическогоЛица(Знач ИмяПоля) Экспорт
	...
	Результат = КадровыйУчет.ЭтоОбязательноеПолеКадровыхДанныхФизическогоЛица(ИмяПоля)
	...
	Если Не Результат Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ОбразованияФизическихЛиц") Тогда
			МодульОбразованияФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("ОбразованияФизическихЛиц");
			Результат = МодульОбразованияФизическихЛиц.НеобходимыСведенияОСпециальностяхФизическихЛиц(ИмяПоля);
		КонецЕсли;
	КонецЕсли;
	...
	Если Не Результат Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.СемейныеПоложенияФизическихЛиц") Тогда
			МодульСемейныеПоложенияФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("СемейныеПоложенияФизическихЛиц");
			Результат = МодульСемейныеПоложенияФизическихЛиц.НеобходимыСведенияОСостоянияхВБракеФизическихЛиц(ИмяПоля)
				Или МодульСемейныеПоложенияФизическихЛиц.НеобходимыСведенияОСоставеСемейФизическихЛиц(ИмяПоля);
		КонецЕсли;
	КонецЕсли;
	...
	Если Не Результат Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ВоинскийУчет") Тогда
			МодульВоинскийУчет = ОбщегоНазначения.ОбщийМодуль("ВоинскийУчет");
			Результат = МодульВоинскийУчет.НеобходимыСведенияОВоинскомУчетеФизическихЛиц(ИмяПоля)
				Или МодульВоинскийУчет.НеобходимыСведенияОБронированииСотрудников(ИмяПоля);
		КонецЕсли;
	КонецЕсли;
...
Функция НеобходимыПостоянныеКадровыеДанныеФизическихЛиц(Знач ИмяПоля) Экспорт
	...
	Результат = КадровыйУчет.НеобходимыЗначенияРеквизитовСправочникаФизическиеЛица(ИмяПоля)
		Или НеобходимаКонтактнаяИнформацияФизическогоЛица(ИмяПоля)
		Или НеобходимыДатыСмертиСотрудников(ИмяПоля)
		Или ИнтеграцияКабинетСотрудника.НеобходимыДатыОтключенияФизическихЛиц(ИмяПоля)
		Или Метаданные.РегистрыСведений["ФизическиеЛицаПрисоединенныеККЭДО"] <> Неопределено
				И ОбщегоНазначения.ОбщийМодуль("РегистрыСведений.ФизическиеЛицаПрисоединенныеККЭДО").
					НеобходимыДанныеОПодключенииККЭДО(ИмяПоля);
	
	Если Не Результат Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ОбразованияФизическихЛиц") Тогда
			МодульОбразованияФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("ОбразованияФизическихЛиц");
			Результат = МодульОбразованияФизическихЛиц.НеобходимыСведенияОбОбразованияхФизическихЛиц(ИмяПоля)
				Или МодульОбразованияФизическихЛиц.НеобходимыСведенияОЗнанияхЯзыковФизическихЛиц(ИмяПоля)
				Или МодульОбразованияФизическихЛиц.НеобходимыСведенияОПрофессияхФизическихЛиц(ИмяПоля);
		КонецЕсли;
	КонецЕсли;
	...
	Если Не Результат Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.СводныеДанныеФизическихЛиц") Тогда
			...
			Результат = МодульСводныеДанныеФизическихЛиц.НеобходимыСтроковыеСведенияФизическихЛиц(ИмяПоля);
		КонецЕсли;
	КонецЕсли;
...
Функция ЗапросВТКадровыеДанныеСотрудников(ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, КадровыеДанные, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень, ИсточникиДанных, ИмяВТКадровыеДанныеСотрудников, ТаблицаОтборовСодержитПолеФизическоеЛицо, УничтожитьВспомогательныеТаблицы) Экспорт
	...
	Для каждого ИмяЗапрашиваемыхДанных Из ИменаКадровыхДанных Цикл
		...
		ИначеЕсли ДобавитьПолеСведенийКадровойИсторииСотрудников(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			...
			Или Метаданные.РегистрыСведений["СостояниеСогласияНаПрисоединениеККЭДО"] <> Неопределено
				И ОбщегоНазначения.ОбщийМодуль("РегистрыСведений.СостояниеСогласияНаПрисоединениеККЭДО").
					ДобавитьПолеСведенийОСогласияхПодключенийККЭДО(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных) Тогда
		...
		Иначе
			...
			ИсточникиДанных.Вставить("ЗапрашиваетсяТолькоКадроваяИсторияСотрудников", Ложь);
			ТекстСообщенияИсключения = НСтр("ru = 'Среди кадровых данных сотрудников нет данных с именем';
											|en = 'Among HR data of employees there is no data with name'") + " """ + ИмяКадровыхДанных + """";
			ВызватьИсключение ТекстСообщенияИсключения;
	...
	Иначе
		...
		Если Метаданные.РегистрыСведений["СостояниеСогласияНаПрисоединениеККЭДО"] <> Неопределено Тогда
			ОбщегоНазначения.ОбщийМодуль("РегистрыСведений.СостояниеСогласияНаПрисоединениеККЭДО").
				ДобавитьТекстЗапросаВТСведенияОСогласияхПодключенийККЭДО(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ИсточникиДанных);
		КонецЕсли;
		...
		КадровыйУчет.ДобавитьТекстыЗапросовВТСведенийЗависящихОтФункциональныхОпцийОрганизаций(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень, ИсточникиДанных);
...
Функция ДобавитьПолеСведенийТекущихКадровыхДанныхСотрудников(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
	...
	Если НеобходимыСведенияТекущихКадровыхДанныхСотрудников(ИмяПоля) Тогда
		...
		Если Не ФОИспользоватьКадровыйУчет И ВРег(ИмяПоля) = ВРег("ВидЗанятостиРегламентированный") Тогда
			ИсточникиДанных.Вставить("РегламентированныеСведенияОВидахЗанятостиСотрудников", Истина);
		КонецЕсли;
...
Функция ПутьКДаннымСведенийТекущихКадровыхДанныхСотрудников(ИмяПоля) Экспорт
	...
	ИначеЕсли Не ФОИспользоватьКадровыйУчет И ИмяПоляВВерхнемРегистре = ВРег("ВидЗанятостиРегламентированный") Тогда
		
		ПутьКДанным =
			"	ВЫБОР КОГДА ТекущиеКадровыеДанныеСотрудников.ТекущийВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ПустаяСсылка)
			|		ТОГДА &ЗначениеNull
			|		ИНАЧЕ 
			|			ВЫБОР КОГДА НЕ РегламентированныеТекущиеКадровыеДанныеСотрудников.ТекущийВидЗанятости ЕСТЬ NULL
			|				И ЕСТЬNULL(ТекущиеКадровыеДанныеСотрудников.ТекущийВидЗанятости, ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ВнутреннееСовместительство)
			|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.Совместительство)
			|			ИНАЧЕ ТекущиеКадровыеДанныеСотрудников.ТекущийВидЗанятости
			|		КОНЕЦ
			|	КОНЕЦ";
...
Процедура ДобавитьТекстЗапросаТекущихКадровыхДанныхСотрудников(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ИсточникиДанных)
	...
	Если ИсточникиДанных.Получить("ТекущиеКадровыеДанныеСотрудниковСоединениеУстановлено") <> Истина
		...
		Если Не ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") И ИсточникиДанных.Получить("РегламентированныеСведенияОВидахЗанятостиСотрудников") <> Неопределено Тогда
			
			ТекстЗапросаУникальныхФизическихЛиц =
				"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
				|	ТаблицаОтборов.Период КАК Период,
				|	ТаблицаОтборов.Сотрудник КАК Сотрудник,
				|	ТекущиеКадровыеДанныеСотрудников.ТекущийВидЗанятости КАК ТекущийВидЗанятости
				|ПОМЕСТИТЬ ВТРегламентированныеТекущиеКадровыеДанныеСотрудников
				|ИЗ
				|	ВТОтборовСотрудников КАК ТаблицаОтборов
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТекущиеКадровыеДанныеСотрудников КАК ТекущиеКадровыеДанныеСотрудников
				|		ПО ТаблицаОтборов.Сотрудник.ФизическоеЛицо = ТекущиеКадровыеДанныеСотрудников.ФизическоеЛицо
				|			И ТаблицаОтборов.Сотрудник.ГоловнаяОрганизация = ТекущиеКадровыеДанныеСотрудников.ГоловнаяОрганизация
				|			И (ТаблицаОтборов.Период > ДАТАВРЕМЯ(1, 1, 1))
				|			И (ТаблицаОтборов.Период МЕЖДУ ТекущиеКадровыеДанныеСотрудников.ДатаПриема И ВЫБОР
				...
				|			КОНЕЦ)
				|ГДЕ
				|	ТекущиеКадровыеДанныеСотрудников.ТекущийВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.Совместительство)
				|
				|ОБЪЕДИНИТЬ ВСЕ
				|
				|ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			ТекстЗапросаУникальныхФизическихЛиц = СтрЗаменить(ТекстЗапросаУникальныхФизическихЛиц,
				"ТаблицаОтборов.Сотрудник", "ТаблицаОтборов." + ОписательВременнойТаблицыОтборов.ИмяПоляСотрудник);
			...
			ЧастиЗапроса.Вставить(0, ТекстЗапросаУникальныхФизическихЛиц);
			
			ЧастиЗапросаСоединений.Добавить(
				"		ЛЕВОЕ СОЕДИНЕНИЕ ВТРегламентированныеТекущиеКадровыеДанныеСотрудников КАК РегламентированныеТекущиеКадровыеДанныеСотрудников
				|		ПО ТаблицаОтборов." + ОписательВременнойТаблицыОтборов.ИмяПоляСотрудник + " = РегламентированныеТекущиеКадровыеДанныеСотрудников.Сотрудник");
...
Функция НеобходимыСведенияПриказаОПриеме(Знач ИмяПоля) Экспорт
	...
	Результат = ИмяПоля = ВРег("ПриказОПриеме")
	...
	Если Не Результат И Не ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.КадровыйУчетРасширенная.КонтрактыДоговорыСотрудников") Тогда
		
		Результат = ИмяПоля = ВРег("ТрудовойДоговорНомер")
...
Функция ЗапросВТСведенийПриказовОПриеме(ИмяВТКадроваяИсторияСотрудников, ТолькоРазрешенные, ИмяВТСведенийПриказовОПриеме) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ТрудовойДоговорНомер", Метаданные.Документы.ПриемНаРаботу) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПриемНаРаботу.Проведен КАК Проведен", "ПриемНаРаботу.Проведен КАК Проведен,
...
Функция ПутьКДаннымСведенийПриказаОПриемеОбУвольнении(Знач ИмяПоля) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
		...
		ИначеЕсли Не ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.КадровыйУчетРасширенная.КонтрактыДоговорыСотрудников") Тогда
			
			Если ИмяПоляВВерхнемРегистре = ВРег("ТрудовойДоговорНомер") Тогда
				ПутьКДанным = "	ПриказыОПриеме.ТрудовойДоговорНомер";
			ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("ТрудовойДоговорДата") Тогда
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") И КадровыйУчет.НеобходимыСведенияКадровойИсторииСотрудников(ИмяПоля) Тогда
		...
		Иначе
			...
			ИначеЕсли ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.РабочиеМестаОхраныТруда")
				...
				ТекстыОписанияПолей.Добавить("	NULL КАК " + ИмяПоля);
			Иначе
...
Функция НеобходимыСведенияКадровойИсторииСотрудников(ИмяПоля) Экспорт
	...
	Возврат ИмяПоля = ВРег("РабочееМестоПериодРегистрации")
		...
		Или ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет")
			И Не ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы")
...
Функция ДобавитьПолеСведенийОбОплатеТруда(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
	...
	ДобавленоПолеСведений = Ложь;
	Если КадровыйУчет.НеобходимыСведенияОбОплатеТруда(ИмяПоля)
...
Функция ДобавитьПолеСведенийОВидахЗанятостиСотрудников(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
	...
	ДобавленоПолеСведений = Ложь;
	Если НеобходимыСведенияОВидахЗанятостиСотрудников(ИмяПоля)
		И (ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет")
			Или КадровыйУчет.НастройкиКадровогоУчета().ИспользуютсяДанныеСотрудниковВнешнихИсточников)  Тогда
		...
		Если ВРег(ИмяПоля) = ВРег("ВидЗанятостиРегламентированный") Тогда
			ИсточникиДанных.Вставить("РегламентированныеСведенияОВидахЗанятостиСотрудников", Истина);
		КонецЕсли;
...
Функция НеобходимыСведенияОВидахЗанятостиСотрудников(Знач ИмяПоля)
	...
	Возврат ИмяПоля = ВРег("ВидЗанятостиПериодРегистрации")
		...
		Или ИмяПоля = ВРег("ВидЗанятостиРегламентированный")
...
Функция ПутьКДаннымСведенийОВидахЗанятостиСотрудников(ИмяПоля)
	...
	ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("ВидЗанятостиРегламентированный") Тогда
		ПутьКДанным =
			"	ВЫБОР КОГДА НЕ РегламентированныеСведенияОВидахЗанятостиСотрудников.ВидЗанятости ЕСТЬ NULL
			|		И ЕСТЬNULL(СведенияОВидахЗанятостиСотрудников.ВидЗанятости, ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ВнутреннееСовместительство)
			|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.Совместительство)
			|		ИНАЧЕ СведенияОВидахЗанятостиСотрудников.ВидЗанятости
			|	КОНЕЦ";
	Иначе
...
Процедура ДобавитьТекстЗапросаВТСведенийОВидахЗанятостиСотрудников(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных, ФормироватьСПериодичностьДень)
	...
	ЧастиЗапроса = Новый Массив;
	Если ИсточникиДанных.Получить("РегламентированныеСведенияОВидахЗанятостиСотрудников") <> Неопределено Тогда
		...
		ТекстЗапросаУникальныхФизическихЛиц =
			...
			|	ТекущиеКадровыеДанныеСотрудников.Сотрудник КАК Сотрудник,
			|	ТекущиеКадровыеДанныеСотрудников.ФизическоеЛицо КАК ФизическоеЛицо
			|ПОМЕСТИТЬ ВТОтборовВнешнихСовместителей
			|ИЗ
			|	ВТОтборовСотрудников КАК ТаблицаОтборов
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТекущиеКадровыеДанныеСотрудников КАК ТекущиеКадровыеДанныеСотрудников
			|		ПО ТаблицаОтборов.ФизическоеЛицо = ТекущиеКадровыеДанныеСотрудников.ФизическоеЛицо
			|			И ТаблицаОтборов.Сотрудник.ГоловнаяОрганизация = ТекущиеКадровыеДанныеСотрудников.ГоловнаяОрганизация
			|			И (ТаблицаОтборов.Период > ДАТАВРЕМЯ(1, 1, 1))
			|			И (ТаблицаОтборов.Период МЕЖДУ ТекущиеКадровыеДанныеСотрудников.ДатаПриема И ВЫБОР
			|				КОГДА ТекущиеКадровыеДанныеСотрудников.ДатаУвольнения = ДАТАВРЕМЯ(1, 1, 1)
		...
		ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра(
			"ВТОтборовВнешнихСовместителей", "Сотрудник");
		...
		ЗапросВТИмяРегистраСрез = ЗарплатаКадрыОбщиеНаборыДанных.ЗапросВТИмяРегистраСрез(
			...
			ПараметрыПостроения,
			Истина,
			"ВТРегламентированныеВидыЗанятостиСотрудников");
		
		ЗарплатаКадрыОбщиеНаборыДанных.СкопироватьПараметрыЗапроса(Запрос, ЗапросВТИмяРегистраСрез);
		
		ТекстЗапросДляРегламентированныхСведений =
			"ВЫБРАТЬ
			|	ОтборВнешнихСовместителей.Период КАК Период,
			|	ОтборВнешнихСовместителей.Сотрудник КАК Сотрудник,
			|	ОтборВнешнихСовместителей.ФизическоеЛицо КАК ФизическоеЛицо,
			|	ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.Совместительство) КАК ВидЗанятости
			|ПОМЕСТИТЬ ВТРегламентированныеСведенияОВидахЗанятостиСотрудников
			|ИЗ
			|	ВТОтборовВнешнихСовместителей КАК ОтборВнешнихСовместителей
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТРегламентированныеВидыЗанятостиСотрудников КАК СведенияОВидахЗанятости
			|		ПО ОтборВнешнихСовместителей.ФизическоеЛицо = СведенияОВидахЗанятости.ФизическоеЛицо
			|			И ОтборВнешнихСовместителей.Сотрудник <> СведенияОВидахЗанятости.Сотрудник
			|			И ОтборВнешнихСовместителей.Период = СведенияОВидахЗанятости.Период
			|			И (СведенияОВидахЗанятости.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.Совместительство))";
		
		ЧастиЗапроса.Добавить(ТекстЗапросаУникальныхФизическихЛиц);
		ЧастиЗапроса.Добавить(ЗарплатаКадрыОбщиеНаборыДанных.РазделительЗапросов());
		ЧастиЗапроса.Добавить(ЗапросВТИмяРегистраСрез.Текст);
		ЧастиЗапроса.Добавить(ЗарплатаКадрыОбщиеНаборыДанных.РазделительЗапросов());
		ЧастиЗапроса.Добавить(ТекстЗапросДляРегламентированныхСведений);
		ЧастиЗапроса.Добавить(ЗарплатаКадрыОбщиеНаборыДанных.РазделительЗапросов());
	...
	Если ИсточникиДанных.Получить("РегламентированныеСведенияОВидахЗанятостиСотрудников") <> Неопределено Тогда
		ЧастиЗапроса.Добавить(
			"		{ЛЕВОЕ СОЕДИНЕНИЕ ВТРегламентированныеСведенияОВидахЗанятостиСотрудников КАК РегламентированныеСведенияОВидахЗанятостиСотрудников
				|		ПО ТаблицаОтборов." + ОписательВременнойТаблицыОтборов.ИмяПоляСотрудник + " = РегламентированныеСведенияОВидахЗанятостиСотрудников.Сотрудник
				|			И ТаблицаОтборов." + ОписательВременнойТаблицыОтборов.ИмяПоляПериод + " = РегламентированныеСведенияОВидахЗанятостиСотрудников.Период}");
	КонецЕсли;
	...
	Если ИсточникиДанных.Получить("РегламентированныеСведенияОВидахЗанятостиСотрудников") <> Неопределено Тогда
		КадровыйУчет.ДобавитьВКоллекциюИмяКадровыхДанных(ИсточникиДанных, "ВТКУничтожению", "ВТОтборовВнешнихСовместителей");
		КадровыйУчет.ДобавитьВКоллекциюИмяКадровыхДанных(ИсточникиДанных, "ВТКУничтожению", "ВТРегламентированныеВидыЗанятостиСотрудников");
		КадровыйУчет.ДобавитьВКоллекциюИмяКадровыхДанных(ИсточникиДанных, "ВТКУничтожению", "ВТРегламентированныеСведенияОВидахЗанятостиСотрудников");
	КонецЕсли;
...
Функция ДобавитьПолеСведенийОДолеНеполногоРабочегоВремениСотрудников(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
	...
	Если НеобходимыСведенияОДолеНеполногоРабочегоВремениСотрудников(ИмяПоля) Тогда
		...
		Если Не ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
			КадровыйУчет.ДобавитьВКоллекциюИмяКадровыхДанных(ИсточникиДанных, "ПостоянныеКадровыеДанныеСотрудников", "КоличествоСтавок");
		КонецЕсли;
...
Функция ДобавитьПолеСведенийОКлассахУсловийТруда(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
	...
	Если НеобходимыСведенияОКлассахУсловийТруда(ИмяПоля) Тогда
		...
		ПутьКДанным = КадровыйУчет.ПутьКДаннымСведенийОКлассахУсловийТруда(ИмяПоля);
		ТекстыОписанияПолей.Добавить(ПутьКДанным + " КАК " + ИмяПоля);
...
Функция ЗапросВТСведенияОКлассахУсловийТруда(ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ИмяВТОтборовДолжностей, ИмяПоляДолжность, ИсточникиДанных, ИмяВТСведенияОКлассахУсловийТруда = "ВТСведенияОКлассахУсловийТруда") Экспорт
	...
	КадровыйУчет.ПриПолученииЗапросаВТСведенияОКлассахУсловийТруда(Запрос, ТолькоРазрешенные, ИмяВТСведенияОКлассахУсловийТруда);
...
Процедура КадроваяИсторияСотрудниковПриЗаписи(НаборЗаписей, Отказ) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет")
		Или КадровыйУчет.НастройкиКадровогоУчета().ИспользуютсяДанныеСотрудниковВнешнихИсточников Тогда
...
Функция ТекстыОписанияПолейЗапросПоляКадровыхДанныхФизическихЛиц(КадровыеДанные, ИсточникиДанных, ОписательВТОтборов)
	...
	ЕстьПодсистемаСемейныеПоложенияФизическихЛиц = ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.СемейныеПоложенияФизическихЛиц");
	Если ЕстьПодсистемаСемейныеПоложенияФизическихЛиц Тогда
		МодульСемейныеПоложенияФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("СемейныеПоложенияФизическихЛиц");
	КонецЕсли;
	...
	Для каждого ИмяЗапрашиваемыхДанных Из КадровыеДанные Цикл
		...
		ИначеЕсли ДобавитьПолеСведенийОДокументахУдостоверяющихЛичность(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			...
			Или ДобавитьПолеСведенийОВозрасте(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных, ОписательВТОтборов)
			Или ЕстьПодсистемаОбразованияФизическихЛиц
				И МодульОбразованияФизическихЛиц.ДобавитьПолеСведенийОСпециальностяхФизическихЛиц(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаСемейныеПоложенияФизическихЛиц
				И МодульСемейныеПоложенияФизическихЛиц.ДобавитьПолеСведенийОСостоянияхВБракеФизическихЛиц(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаСемейныеПоложенияФизическихЛиц
				И МодульСемейныеПоложенияФизическихЛиц.ДобавитьПолеСведенийОСоставеСемейФизическихЛиц(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьПолеСведенийОВоинскомУчетеФизическихЛиц(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьПолеСведенийОБронированииСотрудников(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных) Тогда
		...
		Иначе
			...
			ТекстСообщенияИсключения = НСтр("ru = 'Среди кадровых данных физических лиц нет данных с именем';
											|en = 'Among HR data of individuals there is no data with name'") + " """ + ИмяКадровыхДанных + """";
			ВызватьИсключение ТекстСообщенияИсключения;
...
Процедура ДобавитьВЗапросСоединенияСИсточникамиКадровыхДанныхФизическихЛиц(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных, ИмяВТКадровыеДанныеФизическихЛиц) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ОбразованияФизическихЛиц") Тогда
		МодульОбразованияФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("ОбразованияФизическихЛиц");
		МодульОбразованияФизическихЛиц.ДобавитьТекстЗапросаВТСведенияОСпециальностяхФизическихЛиц(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ИсточникиДанных);
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.СемейныеПоложенияФизическихЛиц") Тогда
		МодульСемейныеПоложенияФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("СемейныеПоложенияФизическихЛиц");
		МодульСемейныеПоложенияФизическихЛиц.ДобавитьТекстЗапросаВТСведенияОСостоянияхВБракеФизическихЛиц(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных);
		МодульСемейныеПоложенияФизическихЛиц.ДобавитьТекстЗапросаВТСведенияОСоставеСемейФизическихЛиц(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ИсточникиДанных);
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ВоинскийУчет") Тогда
		МодульВоинскийУчет = ОбщегоНазначения.ОбщийМодуль("ВоинскийУчет");
		МодульВоинскийУчет.ДобавитьТекстЗапросаВТСведенияОВоинскомУчетеФизическихЛиц(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных);
		МодульВоинскийУчет.ДобавитьТекстЗапросаВТСведенияОБронированииСотрудников(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных);
	КонецЕсли;
...
Процедура ДобавитьВЗапросПоляПостоянныхКадровыхДанныхФизическихЛиц(Запрос, КадровыеДанные, ИсточникиДанных, ИмяВТПостоянныеКадровыеДанныеФизическихЛиц) Экспорт
	...
	ЕстьПодсистемаСводныеДанныеФизическихЛиц = ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.СводныеДанныеФизическихЛиц");
	Если ЕстьПодсистемаСводныеДанныеФизическихЛиц Тогда
		МодульСводныеДанныеФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("СводныеДанныеФизическихЛиц");
	КонецЕсли;
	...
	Для каждого ИмяЗапрашиваемыхДанных Из КадровыеДанные Цикл
		...
		ИначеЕсли ДобавитьПолеРеквизитаСправочникаФизическиеЛица(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			...
			Или ДобавитьПолеСведенийОДатахСмертиСотрудников(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или Метаданные.РегистрыСведений["ФизическиеЛицаПрисоединенныеККЭДО"] <> Неопределено
				И ОбщегоНазначения.ОбщийМодуль("РегистрыСведений.ФизическиеЛицаПрисоединенныеККЭДО").
					ДобавитьПолеСведенийОПодключенииККЭДО(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаОбразованияФизическихЛиц
				И МодульОбразованияФизическихЛиц.ДобавитьПолеСведенийОбОбразованииФизическихЛиц(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаОбразованияФизическихЛиц
				И МодульОбразованияФизическихЛиц.ДобавитьПолеСведенийОЗнанияхЯзыковФизическихЛиц(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаОбразованияФизическихЛиц
				И МодульОбразованияФизическихЛиц.ДобавитьПолеСведенийОПрофессияхФизическихЛиц(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаСводныеДанныеФизическихЛиц
				И МодульСводныеДанныеФизическихЛиц.ДобавитьПолеСтроковыхСведенийФизическихЛиц(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ИнтеграцияКабинетСотрудника.ДобавитьПолеСведенийОДатахОтключенияФизическихЛиц(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных) Тогда
...
Процедура ДобавитьВЗапросСоединенияСИсточникамиПостоянныхКадровыхДанныхФизическихЛиц(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ИсточникиДанных) Экспорт
	...
	ИнтеграцияКабинетСотрудника.ДобавитьТекстЗапросаВТСведенияОДатахОтключенияФизическихЛиц(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ИсточникиДанных);
	Если Метаданные.РегистрыСведений["ФизическиеЛицаПрисоединенныеККЭДО"] <> Неопределено Тогда
		ОбщегоНазначения.ОбщийМодуль("РегистрыСведений.ФизическиеЛицаПрисоединенныеККЭДО").
			ДобавитьТекстЗапросаВТСведенияОПодключенииККЭДО(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ИсточникиДанных);
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ОбразованияФизическихЛиц") Тогда
		МодульОбразованияФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("ОбразованияФизическихЛиц");
		МодульОбразованияФизическихЛиц.ДобавитьТекстЗапросаВТСведенияОбОбразованииФизическихЛиц(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ИсточникиДанных);
		МодульОбразованияФизическихЛиц.ДобавитьТекстЗапросаВТСведенияОЗнанияхЯзыковФизическихЛиц(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ИсточникиДанных);
		МодульОбразованияФизическихЛиц.ДобавитьТекстЗапросаВТСведенияОПрофессияхФизическихЛиц(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ИсточникиДанных);
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.СводныеДанныеФизическихЛиц") Тогда
		МодульСводныеДанныеФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("СводныеДанныеФизическихЛиц");
		МодульСводныеДанныеФизическихЛиц.ДобавитьТекстЗапросаВТСтроковыеСведенияФизическихЛиц(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ИсточникиДанных);
	КонецЕсли;
...
Функция ДанныеОтсутствийСотрудникаДляПособияПоБезработице(КадровыеДанныеСотрудников) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.КадровыйУчет") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("КадровыйУчетДляНебольшихОрганизаций");
		ДанныеОтсутствий = Модуль.ДанныеОтсутствийСотрудникаДляПособияПоБезработице(КадровыеДанныеСотрудников);
	КонецЕсли;
...
Функция СведенияОСреднемЗаработкеДляСправкиДетямСиротам(КадровыеДанныеСотрудников, ДатаСправки) Экспорт
	...
	СведенияОСреднемЗаработке = Новый Структура;
	...
	ДанныеОВремени.Колонки.Добавить("Период", Новый ОписаниеТипов("Дата"));
	ДанныеОВремени.Колонки.Добавить("ОтработаноДней", Новый ОписаниеТипов("Число"));
	ДанныеОВремени.Колонки.Добавить("НормаДней", Новый ОписаниеТипов("Число"));
	ДанныеОВремени.Колонки.Добавить("ОтработаноЧасов", Новый ОписаниеТипов("Число"));
	ДанныеОВремени.Колонки.Добавить("НормаЧасов", Новый ОписаниеТипов("Число"));
	СведенияОСреднемЗаработке.Вставить("ДанныеОВремени", ДанныеОВремени);
	
	ДанныеОНачислениях = Новый ТаблицаЗначений;
	ДанныеОНачислениях.Колонки.Добавить("Сотрудник", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ДанныеОНачислениях.Колонки.Добавить("Период", Новый ОписаниеТипов("Дата"));
	ДанныеОНачислениях.Колонки.Добавить("Сумма", Новый ОписаниеТипов("Число"));
	СведенияОСреднемЗаработке.Вставить("ДанныеОНачислениях", ДанныеОНачислениях);
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.КадровыйУчет") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("КадровыйУчетДляНебольшихОрганизаций");
		Модуль.ЗаполнитьСведенияОСреднемЗаработкеДляСправкиДетямСиротам(
			СведенияОСреднемЗаработке, КадровыеДанныеСотрудников, ДатаСправки);
	КонецЕсли;
...
Процедура ОпределитьПроверкиВеденияУчета(ГруппыПроверок, Проверки) Экспорт
	...
	ГруппаПроверок.Наименование					= НСтр("ru = 'Оформление кадровых приказов';
															|en = 'Staffing orders registration'");
	...
	Проверка.Причины						= НСтр("ru = 'Отмена проведений кадровых приказов задним числом (не в обратной хронологической последовательности)';
													|en = 'Cancel post factum HR orders posting (not in the reverse chronological order)'");
	Проверка.Рекомендация					= НСтр("ru = 'Восстановить хронологическую последовательность кадровых приказов';
													|en = 'Restore HR orders chronological sequence'");
	...
	Проверка.Наименование					= НСтр("ru = 'Дата рождения сотрудников, учет которых начался с начала прошлого месяца';
													|en = 'Date of birth of employees whose accounting began from the beginning of the last month'");
	Проверка.Причины						= НСтр("ru = 'Не введена дата рождения (или введена «странная») дата рождения';
													|en = 'Date of birth is empty (or invalid)'");
	Проверка.Рекомендация					= НСтр("ru = 'Ввести (исправить) дату рождения';
													|en = 'Enter (correct) date of birth'");
	...
	Проверка.Наименование					= НСтр("ru = 'ИНН сотрудников, учет которых начался с начала прошлого месяца';
													|en = 'TIN of employees whose accounting began from the beginning of the last month'");
	Проверка.Причины						= НСтр("ru = 'Не введен (введен некорректный) ИНН';
													|en = 'TIN is empty (or invalid)'");
	Проверка.Рекомендация					= НСтр("ru = 'Ввести (исправить) ИНН';
													|en = 'Enter (correct) TIN'");
	...
	Проверка.Наименование					= НСтр("ru = 'СНИЛС сотрудников, учет которых начался с начала прошлого месяца';
													|en = 'SNILS of employees whose accounting began from the beginning of the last month'");
	Проверка.Причины						= НСтр("ru = 'Не введен (введен некорректный) СНИЛС';
													|en = 'SNILS is empty (or invalid)'");
	Проверка.Рекомендация					= НСтр("ru = 'Ввести (исправить) СНИЛС';
													|en = 'Enter (correct) SNILS'");
	...
	Проверка.Наименование					= НСтр("ru = 'Статус застрахованного физического лица на дату начала учета';
													|en = 'The insured individual status as of the accounting start date'");
	Проверка.Причины						= НСтр("ru = 'Не указан (указан некорректно) страховой статус';
													|en = 'Insurance status is empty (invalid)'");
	Проверка.Рекомендация					= НСтр("ru = 'Ввести (исправить) страховой статус';
													|en = 'Enter (correct) insurance status'");
	...
	Проверка.Причины						= НСтр("ru = 'Документ не проведен и не помечен на удаление';
													|en = 'Document is not posted and not marked for deletion'");
	Проверка.Рекомендация					= НСтр("ru = 'Провести или удалить соответствующий документ';
													|en = 'Post or delete the relevant document'");
...
Процедура СоздатьВТОбложениеДоговоровГПХНСПЗ(МенеджерВременныхТаблиц) Экспорт
	...
	Запрос.Текст =
		...
		|	ДоговорыГПХ.ДоговорГПХ.ОблагаетсяФСС_НС КАК ОблагаетсяНСПЗ
