...
Процедура ВыполнитьОбменДаннымиПоСценарию(КодСценарияОбмена) Экспорт
	...
	Если Не ОчередьЗадачВыполнена(Сценарий) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Не запустилось выполнение синхронизации по сценарию,
                               ...
                               |en = 'Cannot start the synchronization scenario.
                               |The previous synchronization session is not completed.'",
...
Процедура УдалитьУстаревшиеЗадачи(Сценарий = Неопределено, РучнойОбмен = Ложь)
	...
	Исключение
		...
		Событие = НСтр("ru = 'Обмен данными.Удаление записей регистра задач';
						|en = 'Data exchange.Delete task register records'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если Выборка.Следующий() Тогда
		...
		Иначе
			...
			Шаблон = НСтр("ru = 'Не обнаружено активных задач обмена в корреспонденте. 
                                ...
                                |en = 'No active exchange tasks are found in the peer infobase. 
                                |See the peer infobase event log (%1).'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗаполнитьЗадачиПоСценарию(Сценарий, ПерваяЗадача = Неопределено) 
	...
	Пока Выборка.Следующий() Цикл
		...
		Запись.Режим = НСтр("ru = 'Автоматический';
							|en = 'Automatic'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗаполнитьЗадачиДляОбменаВручную(УзелИнформационнойБазы, ИдентификаторОбмена, ПерваяЗадача, ДополнениеВыгрузки)
	...
	Запись.ОбластьДанныхКорреспондента = НастройкиТранспорта.ОбластьДанныхКорреспондента;
	Запись.Режим = НСтр("ru = 'Ручной';
						|en = 'Manual'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОтветныйВызов(ИмяПланаОбмена, КодУзлаИнформационнойБазы, ИдентификаторЗадачи, Ошибка = "")
	...
	Если Прокси = Неопределено Тогда
		ТекстИсключения = НСтр("ru = 'Не удалось создать подключение к базе корреспонденту';
								|en = 'Cannot create a connection to the peer infobase'",
			ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьЗадачу(Задача, ПараметрыОбмена, Отказ) Экспорт
	...
	Шаблон = НСтр("ru = 'Выполнение шага сценария синхронизации.
                   ...
                   |en = 'Execute synchronization scenario step.
                   |SequenceNumber: %1, Application: %2, Action: %3, Mode: %4.'");
...
Функция СобытиеЖурналаРегистрацииОтключениеСценария()
	...
	Возврат НСтр("ru = 'Обмен данными.Отключение сценария';
				|en = 'Data exchange.Disable scenario'", ОбщегоНазначения.КодОсновногоЯзыка());
