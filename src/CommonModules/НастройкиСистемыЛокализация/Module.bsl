...
Процедура ДополнитьЗначенияКонстантКонтролируемыхВБазовойВерсии(Результат) Экспорт 
	...
	//++ НЕ УТКА
	Результат.Вставить(Метаданные.Константы.ИспользоватьВнутреннююПереработку.Имя, Ложь);
	...
	Результат.Вставить(Метаданные.Константы.ИспользоватьАктыРасхожденийПослеОтгрузкиПоОтгрузкамТоваровДавальцу2_5.Имя, Ложь);
	Результат.Вставить(Метаданные.Константы.ИспользоватьАктыРасхожденийПослеПриемкиПоПриемкамТоваровДавальца2_5.Имя, Ложь);
	Результат.Вставить(Метаданные.Константы.ИспользоватьЗаказыДавальцев2_5.Имя, Ложь);
	Результат.Вставить(Метаданные.Константы.ИспользоватьОтчетДавальцуПоНесколькимЗаказам2_5.Имя, Ложь);
	Результат.Вставить(Метаданные.Константы.ИспользоватьПроизводствоИзДавальческогоСырья2_5.Имя, Ложь);
	Результат.Вставить(Метаданные.Константы.ИспользоватьНесколькоОрганизацийПроизводствоИзДавальческогоСырья2_5.Имя, Ложь);
	Результат.Вставить(Метаданные.Константы.ИспользоватьСтатусыЗаказовДавальцев2_5.Имя, Ложь);
	...
	//++ НЕ УТКА
	Результат.Вставить(Метаданные.Константы.ИспользоватьНесколькоОрганизацийПроизводствоНаСтороне2_5.Имя, Ложь);
	//-- НЕ УТКА
...
Процедура СинхронизироватьЗначенияПодчиненныхКонстант(ИмяКонстанты, ЗначениеКонстанты, Отказ) Экспорт
	...
	Если ИмяКонстанты = Метаданные.Константы.ИспользоватьСтатусыЗаказовДавальцев2_5.Имя Тогда
		
		Если НЕ Константы.ИспользоватьСтатусыЗаказовДавальцев2_5.Получить() Тогда
			НастройкиСистемы.УстановитьЗначениеКонстанты(Константы.НеЗакрыватьЗаказыДавальцевБезПолнойОплаты2_5,    Ложь);
			НастройкиСистемы.УстановитьЗначениеКонстанты(Константы.НеЗакрыватьЗаказыДавальцевБезПолнойОтработки2_5, Ложь);
		КонецЕсли;
	...
	Если Найти(ИмяКонстанты, "ИспользоватьПроизводствоИзДавальческогоСырья") > 0 Тогда
		...
		// Настройка интерфейса в зависимости от режима совместного использования приема в переработку 2.4 и 2.5
		НастройкиСистемы.УстановитьЗначениеКонстанты(
							Константы.ИспользуетсяТолькоПриемВПереработку2_4,
							ПриемВПереработку24 И Не ПриемВПереработку25);
		
		НастройкиСистемы.УстановитьЗначениеКонстанты(
							Константы.ИспользуетсяПриемВПереработку24и25,
							ПриемВПереработку24 И ПриемВПереработку25);
		
		// настройка интерфейса для отображения исторических данных
		НастройкиСистемы.УстановитьЗначениеКонстанты(
							Константы.ОтображатьПроизводствоИзДавальческогоСырья2_4,
							Не ПриемВПереработку24
							И ПриемВПереработку25
							И ПроизводствоИзДавальческогоСырья24ИспользовалосьРанее());
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ПоддержкаБанковскогоИКазначейскогоСопровожденияГосконтрактов" Или РеквизитПутьКДанным = "" Тогда
		...
		Если Форма.НаборКонстант.ПоддержкаБанковскогоИКазначейскогоСопровожденияГосконтрактов Тогда
			СтрокаЗаголовка = НСтр("ru = 'Поддержка банковского и казначейского сопровождения госконтрактов (используется)';
									|en = 'Support of banking and treasury state contracts (used)'");
		Иначе
			СтрокаЗаголовка = НСтр("ru = 'Поддержка банковского и казначейского сопровождения госконтрактов (не используется)';
									|en = 'Support of banking and treasury state contracts (not used)'");
		КонецЕсли;
...
Процедура ПриСозданииНаСервере_НастройкиНоменклатуры(Форма) Экспорт
	...
	ЭлементыФормы.ОткрытьЕдиницыИзмерения.РасширеннаяПодсказка.Заголовок = НСтр("ru = 'Классификатор единиц измерения, основывающийся на Общероссийском классификаторе единиц измерения.';
																				|en = 'The UOM classifier (units of measure classifier) stores all units of measure that can apply to products you sell or buy.'");
		...
		//-- НЕ УТ
			...
			ТекстКомментария = ТекстКомментария
								...
								РазделительТекстов + НСтр("ru = '""Учет зерна и продуктов переработки"" (находится в разделе ""Настройка интеграции с ФГИС ""Зерно"");';
															|en = '""Accounting of grain and grain processing products"" (see ""Set up integration with FSIS Zerno"")'"),
								"");
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьНоменклатуруПартнеров"
		...
		ТекстКомментария = ?(КоличествоВключенныхОпций = 1,
							НСтр("ru = 'Невозможно отключение использования номенклатуры поставщиков, потому что включена опция';
								|en = 'Cannot disable vendor part numbers as the option is enabled'"),
							НСтр("ru = 'Невозможно отключение использования номенклатуры поставщиков, потому что включены опции:';
								|en = 'Cannot disable vendor part numbers as the following options are enabled:'"));
		
		ТекстКомментария = ТекстКомментария
							+ ?(ИспользоватьОбменЭД,
								РазделительТекстов + НСтр("ru = '""Сервисы ЭДО"" (находится в разделе ""Обмен электронными документами"");';
															|en = '""EDI services"" (located in the ""Electronic document exchange"" section)'"),
								"");
		...
		ТекстКомментария = ТекстКомментария
							+ ?(ИспользоватьЭлектронноеАктированиеВЕИС,
								РазделительТекстов + НСтр("ru = 'Обмен с ЕИС для целей электронного актирования поставщика;';
															|en = 'Exchange with UIS for electronic certification of the vendor'"),
								"");
		...
		Если КоличествоВключенныхОпций > 0 Тогда
			...
			ТекстКомментария = ТекстКомментария
								...
								РазделительТекстов + НСтр("ru = '""Маркировка товаров"" (находится в разделе ""Настройка интеграции с ИС МП"");';
															|en = '""Goods marking"" (located in ""Integration with IS MT settings"" section)'"),
								"");
...
Процедура ПриСозданииНаСервере_Продажи(Форма) Экспорт
	...
	//++ НЕ УТКА
	Элементы.ГруппаПроизводствоИзДавальческогоСырья.Видимость = Истина;
	
	//++ Устарело_Переработка24
	ВидимостьПроизводствоИзДавальческогоСырья2_4 = НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья
												   Или ПроизводствоИзДавальческогоСырья24ИспользовалосьРанее();
	
	Если ВидимостьПроизводствоИзДавальческогоСырья2_4 Тогда
		
		Элементы.ИспользоватьПроизводствоИзДавальческогоСырья2_5.Заголовок =
					НСтр("ru = 'Производство из давальческого сырья (версия 2.5)';
						|en = 'Subcontracting (version 2.5)'");
		
	Иначе
		Элементы.ИспользоватьПроизводствоИзДавальческогоСырья.Видимость = Ложь;
		Элементы.ГруппаЗаказыДавальца.Видимость                         = Ложь;
	КонецЕсли;
	//-- Устарело_Переработка24
...
Процедура УстановитьДоступность_Продажи(РеквизитПутьКДанным, Форма) Экспорт
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьДоговорыСКлиентами"
		...
		//++ НЕ УТКА
		ИЛИ РеквизитПутьКДанным = "НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья2_5"
		//-- НЕ УТКА
		ИЛИ РеквизитПутьКДанным = "" Тогда
		...
		//++ НЕ УТКА
		ЗначениеКонстантыПриемаВПереработку = НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья2_5;
		//-- НЕ УТКА
		...
		ДоступноРедактированиеДоговорыСКлиентами =
			...
			//++ НЕ УТКА
			И Не ЗначениеКонстантыПриемаВПереработку
			//-- НЕ УТКА
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья"
		...
		ЕстьОбменыСФормированиемДоговоровПоЗаказам =
			Константы.ИспользоватьСинхронизациюДанных.Получить()
			И ОбменДаннымиУТУП.ЕстьОбменыСФормированиемДоговоровПоЗаказам();
		
		Элементы.ИспользоватьПроизводствоИзДавальческогоСырья.Доступность =
			НаборКонстант.ИспользоватьПроизводство;
		
		Элементы.ИспользоватьСтатусыЗаказовДавальцев.Доступность =
			НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья;
		
		Элементы.ИспользоватьОтчетДавальцуПоНесколькимЗаказам.Доступность =
			НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья
			И НЕ ЕстьОбменыСФормированиемДоговоровПоЗаказам;
		
		Элементы.ГруппаКомментарийИспользоватьОтчетДавальцуПоНесколькимЗаказам.Видимость =
			ЕстьОбменыСФормированиемДоговоровПоЗаказам;
			
		ОбщегоНазначенияУТКлиентСервер.ОтображениеПредупрежденияПриРедактировании(
			Элементы.ИспользоватьПроизводствоИзДавальческогоСырья,
			НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья);
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья"
		...
		Элементы.НеЗакрыватьЗаказыДавальцевБезПолнойОтработки.Доступность =
			ИспользоватьПроизводствоИзДавальческогоСырья
			И ИспользоватьСтатусыЗаказовДавальцев;
		
		Элементы.НеЗакрыватьЗаказыДавальцевБезПолнойОплаты.Доступность =
			ИспользоватьПроизводствоИзДавальческогоСырья
			И ИспользоватьСтатусыЗаказовДавальцев;
		
		Элементы.ГруппаКомментарийКонтролироватьЗакрытиеЗаказовДавальцев.Видимость =
			ИспользоватьПроизводствоИзДавальческогоСырья
			И НЕ ИспользоватьСтатусыЗаказовДавальцев;
		
	КонецЕсли;
	//-- Устарело_Переработка24
	
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьПроизводство"
		Или РеквизитПутьКДанным = "НаборКонстант.ИспользоватьУправлениеПроизводством2_2"
		Или РеквизитПутьКДанным = "НаборКонстант.НоваяАрхитектураВзаиморасчетов"
		Или РеквизитПутьКДанным = "" Тогда
		
		Если НаборКонстант.ИспользоватьПроизводствоИзДавальческогоСырья
			И (Не НаборКонстант.ИспользоватьУправлениеПроизводством2_2
				Или Не НаборКонстант.НоваяАрхитектураВзаиморасчетов) Тогда
			
			ШаблонЗаголовка = НСтр("ru = 'Для использования производства из давальческого сырья (версии 2.5) необходимо включить опции:
									|%1';
									|en = 'To use subcontracting (version 2.5), enable the following options:
									|%1'");
			
			Опции = Новый Массив;
			Если Не НаборКонстант.ИспользоватьУправлениеПроизводством2_2 Тогда
				Если НаборКонстант.ИспользоватьУправлениеПроизводством Тогда
					Опции.Добавить(НСтр("ru = '- ""Использовать производство (версия 2.2)"" в разделе ""НСИ и администрирование"" - ""Производство"".';
										|en = '- Use production (version 2.2) in Master data and settings - Manufacturing.'"));
				Иначе
					Опции.Добавить(НСтр("ru = '- ""Использовать производство"" в разделе ""НСИ и администрирование"" - ""Производство"".';
										|en = '- Use production in Master data and settings - Manufacturing.'"));
				КонецЕсли;
			КонецЕсли;
			...
			Элементы.КомментарийИспользоватьПроизводствоИзДавальческогоСырья2_5.Заголовок =
...
Процедура ОтображениеПредупрежденияПриРедактировании_Продажи(СтруктураКонстант) Экспорт
	...
	//++ Устарело_Переработка24
	СтруктураКонстант.Вставить("ИспользоватьСтатусыЗаказовДавальцев");
	//-- Устарело_Переработка24
...
Процедура ПриСозданииНаСервере_Производство(Форма) Экспорт
	...
	Элементы.ГруппаУчетТрудозатратВРазрезеСотрудников.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет");
	...
	Если Элементы.ЛокализацияГруппаИспользоватьПроизводствоНаСтороне.Видимость Тогда
		Элементы.ИспользоватьПроизводствоНаСтороне2_5.Заголовок = НСтр("ru = 'Производство на стороне (версия 2.5)';
																		|en = 'External subcontracting (version 2.5)'");
	КонецЕсли;
	//-- Устарело_Переработка24
	//-- Локализация
	Возврат
...
Процедура УстановитьДоступность_Производство(РеквизитПутьКДанным, Форма) Экспорт
	...
	Если РеквизитПутьКДанным = ""
		...
		ИЛИ РеквизитПутьКДанным = "НаборКонстант.ИспользоватьУправлениеПроизводством"
		ИЛИ РеквизитПутьКДанным = "НаборКонстант.ИспользоватьУправлениеПроизводством2_2"
		//-- НЕ УТКА
		Тогда
		...
		Элементы.ЛокализацияГруппаСтатусыЗаказовПереработчика.Видимость =
			//++ НЕ УТКА
			НаборКонстант.ИспользоватьУправлениеПроизводством Или
			//-- НЕ УТКА
			НаборКонстант.ИспользоватьПроизводствоНаСтороне;
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьПроизводствоНаСтороне"
		...
		Если НаборКонстант.ИспользоватьПроизводствоНаСтороне
			...
			ШаблонЗаголовка = НСтр("ru = 'Для использования производства на стороне (версии 2.5) необходимо включить опции:
									|%1';
									|en = 'To use production by third party (version 2.5), enable the following options:
									|%1'");
	...
	Если РеквизитПутьКДанным = ""
		ИЛИ РеквизитПутьКДанным = "УчетТрудозатратВРазрезеСотрудников" Тогда
		Элементы.ДатаНачалаВеденияУчетаТрудозатратПоСотрудникам.Видимость = Булево(Форма.УчетТрудозатратВРазрезеСотрудников);
	КонецЕсли;
...
Процедура СохранитьЗначениеРеквизита_Производство(КонстантаИмя, КонстантаЗначение, Форма) Экспорт
	...
	Если КонстантаИмя = "ДатаНачалаВеденияУчетаТрудозатратПоСотрудникам"
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Указанный период должен быть не раньше следующего месяца: %1';
										|en = 'The selected period must not be earlier than the next month: %1'"),
...
Функция ПроизводствоИзДавальческогоСырья24ИспользовалосьРанее() Экспорт
	...
	|	Таблица.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ПоступлениеСырьяОтДавальца КАК Таблица
	|ГДЕ
	|	НЕ Таблица.ПометкаУдаления
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	Таблица.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ВозвратСырьяДавальцу КАК Таблица
	|ГДЕ
	|	НЕ Таблица.ПометкаУдаления
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	Таблица.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ПередачаДавальцу КАК Таблица
	|ГДЕ
	|	НЕ Таблица.ПометкаУдаления
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	Таблица.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ОтчетДавальцу КАК Таблица
	|ГДЕ
	|	НЕ Таблица.ПометкаУдаления
	|";
