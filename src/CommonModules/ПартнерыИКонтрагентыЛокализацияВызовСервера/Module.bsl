	...
	Если ЗначениеЗаполнено(ИНН) Тогда
		...
		Если Не РезультатПоиска.ДоступноСоздание Тогда
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|en = 'Cannot change the main counterparty for %1.
				|These TIN and CRTR is already specified for the counterparty with code %2, responsible person is %3.
				|Try to specify a correct CRTR and then select the main counterparty.'"),
				Контрагент,
	...
	Исключение
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось изменить головного контрагента для %1.
				...
				|en = 'Cannot change the main counterparty for %1.
				|Maybe, the counterparty is being edited.'"),
			Контрагент);
		Возврат;
	КонецПопытки;
	...
	Попытка
		РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			НСтр("ru = 'Работа с контрагентами: получение реквизитов по ИНН.';
				|en = 'Operations with counterparties: receipt of attributes by TIN.'"));
	Исключение
