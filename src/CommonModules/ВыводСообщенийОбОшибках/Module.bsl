...
Функция ДобавитьПростоеОписаниеОшибки(Ошибки, Описание = "", Ссылка = Неопределено) Экспорт
	...
	Ошибка.Данные.Вставить("Ссылка",              Ссылка);   // Ссылка или НавигационнаяСсылка. Переход по ссылке - первый шаг к исправлению
...
Функция ПредставлениеСсылки(Ссылка)
	...
	Если ТипЗнч(Ссылка) = Тип("Строка") И Не ПустаяСтрока(Ссылка) Тогда
		
		// Это может быть навигационная ссылка.
		// Попробуем получить ее представление.
		Ссылки = Новый Массив;
		Ссылки.Добавить(Ссылка);
		Представления = ПолучитьПредставленияНавигационныхСсылок(Ссылки);
		
		// Результат ПолучитьПредставленияНавигационныхСсылок не для всех значений параметров
		// специфицирован, поэтому тщательно его проверим на валидность.
		// При любых подозрениях вернем стандартное представление ссылки,
		// а не результат этого метода.
		
		Если ТипЗнч(Представления) = Тип("Массив") 
			И Представления.Количество() = 1 Тогда
			
			Представление = Представления[0];
			Если ТипЗнч(Представление) = Тип("ПредставлениеНавигационнойСсылки") Тогда
				Если Не ПустаяСтрока(Представление.Текст) Тогда
					Возврат Представление.Текст;
				КонецЕсли;
			КонецЕсли;
	...
	// Это не навигационная ссылка или она ссылается на несуществующий объект.
	// Выведем стандартное представление.
	Возврат "" + Ссылка;
...
Функция ДанныеДляВыводаСсылки(Ссылка)
	...
	ДанныеДляВывода.Вставить("Представление", ПредставлениеСсылки(Ссылка));
	ДанныеДляВывода.Вставить("Ссылка",        Ссылка);
...
Функция ДобавитьРаздел(Ошибка, ИмяРаздела, Текст) Экспорт
	...
	ИначеЕсли ИмяРаздела = "ГдеПроблема" Тогда 
		...
		Секция = ДобавитьСекцию(Ошибка, "Раздел", НСтр("ru = 'Где проблема и что делать';
														|en = 'Where the issue is and what to do'"));
		// Текст раздела
		ДобавитьСекцию(Секция, "Текст", Текст);
