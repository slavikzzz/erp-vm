...
Процедура ВыгрузитьКлиентовДляОбмена(ЗаписьXML, УзелОбмена, ДопСвойства, ВыгружатьПолноеДосье = Ложь) Экспорт
	...
	ЗаголовокЗаписи = НСтр("ru = 'Обмен партнерами';
							|en = 'Exchange of partners'") + "." + НСтр("ru = 'Начало пакета выгрузки партнеров';
																	|en = 'Beginning of partners export package'", КодЯзыка);
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Количество строк клиентов на начало: %1';
									|en = 'Quantity of customer lines as of the beginning: %1'", КодЯзыка), Формат(ТаблицаИзмененных.Количество(), "ЧГ="));
	...
	ЗаголовокЗаписи = НСтр("ru = 'Обмен партнерами';
							|en = 'Exchange of partners'") + "." + НСтр("ru = 'Конец пакета выгрузки партнеров';
																	|en = 'End of partners export package'", КодЯзыка);
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Количество строк клиентов на конец: %1';
									|en = 'Quantity of customer lines as of the end: %1'", КодЯзыка), Формат(ТаблицаИзмененных.Количество(), "ЧГ="));
	ЗаписьЖурналаРегистрации(ЗаголовокЗаписи, УровеньЖурналаРегистрации.Информация,,, ТекстСообщения);
КонецПроцедуры
...
Процедура ВыгрузитьПрайсЛистДляОбмена(ЗаписьXML, СообщениеОбмена, УзелОбмена, НомерСообщения, ВсеЦены, ДопСвойства) Экспорт
	...
	Если ТаблицаПрайсЛиста.Количество() = 0 Тогда
		...
		СообщениеОбмена = НСтр("ru = 'Нет доступных цен.';
								|en = 'No available prices.'", ОбщегоНазначения.КодОсновногоЯзыка());
		Возврат;
	КонецЕсли;
	ВремяНачалаОбмена = ТекущаяДатаСеанса();
	ТребуетсяПродолжение = Ложь;
...
Функция НайтиУзелОбмена(СтруктураОтвета, ИдентификаторМобильногоУстройства, ТекущийПользователь = Неопределено) Экспорт
	...
	Если Результат.Пустой() Тогда
		ТекстОтвета = НСтр("ru = 'Синхронизация с устройством не настроена.
									...
									|en = 'Synchronization with the device is not configured.
									|Exchange node was not found for user: %Пользователь%'",
	...
	Если ВыбранныеУзлы.НовыйУзел Тогда
		...
		Исключение
			...
			|en = 'Another application user is already defined for this device.
			|Concurrent usage of one device by different users is not supported
			|by the current configuration version. If that user no longer uses this device,
			|contact your infobase administrator to detach the setting of another user from this device.'",
			ОбщегоНазначения.КодОсновногоЯзыка());
			СтруктураОтвета.ResultMessage = ТекстОтвета;
			СтруктураОтвета.Success = Ложь;
...
Процедура ЗаписатьСправочник(ЗаписьXML, Знач СтруктураСправочника, Знач ИмяСправочника)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Обмен мобильное приложение 1С:Заказы';
				|en = 'Exchange mobile application 1C:Orders'") + "." + НСтр("ru = 'Запись справочника';
																			|en = 'Catalog record'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ЗаписатьВидЦеныСтроки(ЗаписьXML, Знач СтрокаДанных)
	...
	Если ИнформацияНДС = 0 Тогда
		Если СтруктураВидаЦены["ЦенаВключаетНДС"] Тогда //@NON-NLS-1
			ТекстПроНДС = НСтр("ru = '(с НДС)';
								|en = '(including VAT)'", КодОсновногоЯзыка);
		Иначе
			ТекстПроНДС = НСтр("ru = '(без НДС)';
								|en = '(excluding VAT)'", КодОсновногоЯзыка);
		КонецЕсли;
...
Функция КомпоновщикНастроекВыгрузкиКлиентов(Узел)
	...
	ПараметрСКД.Значение = НСтр("ru = 'Клиент входит в сегмент партнеров, которым отгрузка запрещена.';
								|en = 'Customer belongs to the segment of partners to whom the shipment is prohibited.'",
		КодОсновногоЯзыка);
	ПараметрСКД.Использование = Истина;
	...
	ПараметрСКД.Значение = НСтр("ru = 'Клиент помечен на удаление.';
								|en = 'The customer is marked for deletion.'",
		КодОсновногоЯзыка);
	ПараметрСКД.Использование = Истина;
...
Процедура ЗаполнитьИнформациюПоДоставке(ДокументОбъект, МобильныеДанные, ТипДокумента)
	...
	Иначе
		ДокументОбъект.АдресДоставки = НСтр("ru = '<адрес не указан/>';
											|en = '<address is not specified/>'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
Процедура СоздатьОбновитьДоверенность(СтруктураОплаты, Сообщение)
	...
	ДоверенностьКомментарий = СтрШаблон(НСтр("ru = 'Оплата по доверенности №%1 от %2';
											|en = 'Payment under the authorization letter No. %1 of %2'"),
...
Процедура ЗаписатьДополнительныеРеквизитыОбъекта(ИнформацияПоОбъекту, Знач ДопРеквизиты, Знач ЛокальноеИмяОбъекта)
	...
	ЛокальноеИмяОбъекта = СтрЗаменить(ЛокальноеИмяОбъекта, НСтр("ru = 'Партнер';
																|en = 'Partner'"), НСтр("ru = 'Клиент';
																						|en = 'Customer'"));
