...
Функция ПредставлениеНоменклатурыДляПечати(НаименованиеНоменклатурыДляПечати,
	...
	Если ТипЗнч(НаименованиеНоменклатурыДляПечати) <> Тип("Строка") Тогда
		ТекстИсключения = НСтр("ru = 'В функцию %1 не передано наименование номенклатуры для печати.';
								|en = 'Item name for printing is not passed to the %1 function.'");	
	...
	Если ЗначениеЗаполнено(НаименованиеХарактеристикиДляПечати) 
		...
		ТекстИсключения = НСтр("ru = 'В функцию %1 не передано наименование характеристики для печати.';
								|en = 'Item variant name for printing is not passed to the %1 function.'");	
		ТекстИсключения = СтрШаблон(ТекстИсключения, "НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати");
	...
	Иначе
		...
		Если ДополнительныеПараметры.ВозвратнаяТара Тогда
			ТекстВСкобках.Добавить(НСтр("ru = 'возвратная тара';
										|en = 'returnable packaging'", ДополнительныеПараметры.КодОсновногоЯзыка));
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ДополнительныеПараметры.КодТНВЭД) Тогда
			ТекстВСкобках.Добавить(НСтр("ru = 'Код ТН ВЭД:';
										|en = 'FEACN code:'", ДополнительныеПараметры.КодОсновногоЯзыка)+ Символы.НПП + ДополнительныеПараметры.КодТНВЭД);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекстВСкобкахСтрока) Тогда
			ПредставлениеНоменклатуры = НСтр("ru = '%НаименованиеНоменклатурыДляПечати% (%ТекстВСкобках%)';
											|en = '%НаименованиеНоменклатурыДляПечати% (%ТекстВСкобках%)'", ДополнительныеПараметры.КодОсновногоЯзыка);
...
Функция ИмяПоляШапкиТовара(ИмяПоля) Экспорт
	...
	Если ПустаяСтрока(ИмяПоля) Тогда
		ТекстИсключения = СтрШаблон(НСтр("ru = 'Не указан параметр ""%1"".';
										|en = '""%1"" parameter is not specified.'"),
									"ИмяПоля");
	...
	Если ПустаяСтрока(ИмяПоляШапкиТовара) Тогда
		ТекстИсключения = СтрШаблон(НСтр("ru = 'Отсутствует поле шапки товара для параметра ""%1"".';
										|en = 'Goods header field for parameter ""%1"" is missing.'"),
									ИмяПоля);
...
Процедура ПересчитатьСтатусУказанияСерийПриОбработке(ПараметрыУказанияСерий, ТекущийСтатус, СерииУказаныПолностью, КоличествоСерий, ВариантОбеспечения = Неопределено, ТекущиеДанные = Неопределено) Экспорт
	...
	//++ НЕ УТ
	ЭтоПередачаИзПроизводстваОтгруженоНоНеПринято = 
		(
			ПараметрыУказанияСерий.СкладскиеОперации.Найти(ПредопределенноеЗначение("Перечисление.СкладскиеОперации.ВозвратМатериаловИзПроизводстваОтгрузка")) <> Неопределено
			И ПараметрыУказанияСерий.СкладскиеОперации.Найти(ПредопределенноеЗначение("Перечисление.СкладскиеОперации.ВозвратМатериаловИзПроизводстваПриемка")) = Неопределено
		)
		ИЛИ
		(
			ПараметрыУказанияСерий.СкладскиеОперации.Найти(ПредопределенноеЗначение("Перечисление.СкладскиеОперации.ОтгрузкаПродукцииИзПроизводства")) <> Неопределено
			И ПараметрыУказанияСерий.СкладскиеОперации.Найти(ПредопределенноеЗначение("Перечисление.СкладскиеОперации.ПриемкаПродукцииИзПроизводства")) = Неопределено
		);
	//-- НЕ УТ
	
	//++ НЕ УТКА
	ЭтоВыходныеИзделияЭтапа = 
		(
			ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ЭтапПроизводства2_2"
			ИЛИ ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Обработка.РедактированиеПоследовательностиЭтаповПроизводства"
			ИЛИ ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Обработка.ДелениеПартииПроизводства"
		) 
		И ПараметрыУказанияСерий.СкладскиеОперации.Найти(ПредопределенноеЗначение("Перечисление.СкладскиеОперации.ПриемкаПродукцииИзПроизводства")) <> Неопределено
		И НЕ ТекущиеДанные.Произведено;
	ЭтоМатериалыМЛИлиПроизвОперации = 
		ПараметрыУказанияСерий.СкладскиеОперации.Найти(ПредопределенноеЗначение("Перечисление.СкладскиеОперации.ПотреблениеМатериаловПриПроизводстве")) <> Неопределено
		И (ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ПроизводственнаяОперация2_2"
			//++ Устарело_Производство21
			ИЛИ ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.МаршрутныйЛистПроизводства"
			//-- Устарело_Производство21 
			);
	//-- НЕ УТКА
	...
	ПрименятьСтатусМожноУказать = (ПараметрыУказанияСерий.ПодготовкаОрдера
									//++ НЕ УТ
									Или ЭтоПередачаИзПроизводстваОтгруженоНоНеПринято
									//-- НЕ УТ

									//++ НЕ УТКА
									Или ЭтоВыходныеИзделияЭтапа
									Или ЭтоМатериалыМЛИлиПроизвОперации
