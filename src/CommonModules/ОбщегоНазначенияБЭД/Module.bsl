...
Процедура ЗаписатьВЖурналРегистрации(Комментарий, Подсистема, Уровень = Неопределено,
	...
	ПредставлениеОбщаяПодсистема = НСтр("ru = 'Общая подсистема';
										|en = 'Common subsystem'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Иначе
		Если ОбщегоНазначения.ПодсистемаСуществует(Подсистема) Тогда
			...
			ПредставлениеПодсистемы = СтрШаблон(НСтр("ru = '%1';
	...
	ПолноеИмяСобытия = СтрШаблон(НСтр("ru = 'Электронное взаимодействие.%1';
										|en = 'Electronic interaction.%1'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
// Возвращает результат выполнения длительной операции.
// 
// Параметры:
//  ДлительнаяОперация - См. ДлительныеОперации.ВыполнитьФункцию
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  ВидОперации - Строка
//  ПодсистемаБЭД - Строка - См. ПодсистемыБЭД
//  
// Возвращаемое значение:
//  Произвольный
//
Функция РезультатДлительнойОперации(ДлительнаяОперация, КонтекстДиагностики, ВидОперации, ПодсистемаБЭД = "") Экспорт
	...
	ИначеЕсли ДлительнаяОперация.Статус = "Ошибка" Тогда
		ПодсистемаОшибки = ?(ЗначениеЗаполнено(ПодсистемаБЭД), ПодсистемаБЭД,
		...
		ОбработкаНеисправностейБЭД.ДобавитьОшибку(КонтекстДиагностики, Ошибка, ПодсистемаОшибки);
	КонецЕсли;
...
// 
// Возвращаемое значение:
//  Строка - Представление локальной даты со смещением. Пример 19.01.2024 15:37:18 GMT+03:00
Функция ПредставлениеЛокальнойДатыСоСмещением(Дата, Формат) Экспорт
	...
	Если Смещение <> 0 Тогда
		ЛокализованныйФорматВремя = НСтр("ru = 'ДФ=''HH:mm''';
										|en = 'DF=''HH:mm'''");
		ПредставлениеСмещения = СтрШаблон("%1%2", ПредставлениеСмещения, Формат('00010101' + Смещение, ЛокализованныйФорматВремя));
	КонецЕсли;
	
	Возврат СтрШаблон("%1 GMT%2", Формат(Дата, Формат), ПредставлениеСмещения);
...
Функция ДобавитьПространствоИмен(Знач ДанныеXML, Знач ПространствоИмен, Знач Кодировка = "windows-1251") Экспорт
	...
	КодировкаПроверена = Ложь;
	
	ПараметрыЧтенияXML = Новый ПараметрыЧтенияXML(,,,,,,,,Ложь);
	ЧтениеXML = Новый ЧтениеXML;
	ЧтениеXML.ОткрытьПоток(ПотокЧтения, ПараметрыЧтенияXML, , Кодировка);
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если Не КодировкаПроверена 
			И ЗначениеЗаполнено(ЧтениеXML.КодировкаXML)
			И НРег(ЧтениеXML.КодировкаXML) <> НРег(Кодировка) Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Ошибка проверки кодировки: требуется %1, кодировка данных %2';
											|en = 'Encoding check error: %1 required, the %2 data encoding'"), 
				Кодировка, ЧтениеXML.КодировкаXML);
		ИначеЕсли Не КодировкаПроверена Тогда
			КодировкаПроверена = Истина;
		КонецЕсли;		
...
// 
// Параметры:
//  РегламентноеЗадание - ОбъектМетаданных - Метаданные регламентного задания.
//  Использование       - Булево           - Признак использования регламентного задания.
Процедура УстановитьАктивностьРегламентногоЗадания(Знач РегламентноеЗадание, Знач Использование) Экспорт
	...
	ОтборЗаданий.Вставить("Метаданные", РегламентноеЗадание);
...
Функция ПроверитьФормулу(Формула, ДоступныеПоля, ТекстОшибки = "") Экспорт
	...
	Для Каждого Операнд Из Операнды Цикл
		...
		Если ДоступныеПоля.НайтиПоле(Новый ПолеКомпоновкиДанных(Операнд)) = Неопределено Тогда
			...
			Формула = СтрЗаменить(Формула, ТекстОперанда,
				СтрШаблон(НСтр("ru = '!""Поле не найдено: %1""';
								|en = '!""Field is not found: %1""'"), Операнд));
		КонецЕсли;
	КонецЦикла;
	...
	Если ЕстьНедоступныеПоля Тогда
		...
		ТекстОшибки = НСтр("ru = 'Операндами должны быть реквизиты источника доступные для выбора.';
							|en = 'Source attributes available to select must be operands.'");
	КонецЕсли;
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Проверьте формулу. Формулы должны составляться по правилам написания выражений на встроенном языке 1С:Предприятия.';
							|en = 'CHeck the formula. Formulas must be created based on the expression rules of 1C:Enterprise script.'");
	КонецПопытки;
	...
	Если Не Результат Тогда
		ТекстОшибки = НСтр("ru = 'В формуле обнаружены ошибки.';
							|en = 'Errors were found in the formula.'") + " " + ТекстОшибки;
	КонецЕсли;
...
// Возвращаемое значение:
//  Строка - маска формата оператора ПОДОБНО
Функция МаскаКППГоловнойОрганизации() Экспорт
	Возврат "____01001";
КонецФункции

// Возвращаемое значение:
//  Строка - маска формата оператора ПОДОБНО
Функция МаскаКППКрупнейшегоНалогоплательщика() Экспорт
	Возврат "____50001";
КонецФункции
...
Функция ЗначенияРеквизитовОбъектаИлиСсылки(Объект, Знач Реквизиты) Экспорт
	...
	Иначе
		...
		ИначеЕсли ТипЗнч(Реквизиты) = Тип("Массив") Или ТипЗнч(Реквизиты) = Тип("ФиксированныйМассив") Тогда
			...
			КонецЦикла;
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неверный тип второго параметра Реквизиты: %1';
																							|en = 'Invalid Attributes parameter type: %1.'"), Строка(ТипЗнч(Реквизиты)));
		КонецЕсли;
	...
	Если Результат = Неопределено И ВызыватьИсключение Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректное имя ""%1"" значения перечисления ""%2"".
				 ...
				 |Available values: %3'"), ИмяЗначения, МетаданныеПеречисления, СтрСоединить(ДоступныеЗначения, ", "));
	КонецЕсли;
...
Функция ОписаниеТипаВсеСсылки() Экспорт
	...
	Возврат ОбщегоНазначенияБЭДПовтИсп.ОписаниеТипаВсеСсылки();
