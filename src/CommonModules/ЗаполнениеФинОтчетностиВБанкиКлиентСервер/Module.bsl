...
Функция ЭтоОтчетБРО(ВидОтчета) Экспорт
	...
	Возврат СпособЗаполнения = "ОтчетБРО";

КонецФункции

Функция ЭтоОтчетПоФормеБанка(ВидОтчета) Экспорт
	Перем СпособЗаполнения;
	
	ЗаполнениеФинОтчетностиВБанкиКлиентСерверПереопределяемый.ОпределитьСпособЗаполненияВидаОтчета(ВидОтчета, СпособЗаполнения);

	Возврат СпособЗаполнения = "ОтчетПоФормеБанка";
...
Функция ЭтоСтандартныйОтчет(ВидОтчета) Экспорт
	...
	Возврат СпособЗаполнения = "СтандартныйОтчет";
...
Функция ЭтоВнешнийФайл(ВидОтчета) Экспорт
	...
	Возврат Не ЗначениеЗаполнено(СпособЗаполнения)
			Или СпособЗаполнения = "ПроизвольныйФайл"
			Или СпособЗаполнения = "ВнешнийФайл";
...
Функция СформироватьНаименованиеСубъекта(Субъект, ВключатьОбособленныеПодразделения) Экспорт
	...
	Если ЗначениеЗаполнено(Субъект) Тогда
		Если ВключатьОбособленныеПодразделения Тогда
			Возврат СтрШаблон(НСтр("ru = '%1 с обособленными подразделениями';
									|en = '%1 with branch offices'"), Субъект);
		Иначе
			Возврат Субъект;
		КонецЕсли;
	КонецЕсли;
