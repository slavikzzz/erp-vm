...
// КлиентЛицензированияКлиент.ПодключитьЗапросНастроекКлиентаЛицензирования.
//
...
Процедура ПриЗапросеНастроекКлиентаЛицензирования() Экспорт
	...
	Если Не ИнтернетПоддержкаПользователейКлиент.ЗначениеПараметраПриложения("БазоваяФункциональностьБИП\ПравоПодключенияИПП", Ложь) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Недостаточно прав для подключения Интернет-поддержки пользователей.
			...
			|en = 'Insufficient rights to connect to online user support.
	...
	ПараметрОповещенияПроверка = Новый Структура("Активизирована", Ложь);
	ЗаполнитьЗначенияСвойств(
		ПараметрОповещенияПроверка,
		ПараметрОповещения);
	
	Если ПараметрОповещенияПроверка.Активизирована = Истина Тогда
		Возврат;
	КонецЕсли;
