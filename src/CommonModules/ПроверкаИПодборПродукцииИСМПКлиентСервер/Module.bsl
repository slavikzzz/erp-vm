...
Процедура УстановитьИндексКартинкиСостояниеПодбораКодаМаркировки(ТекущаяСтрока, СвойстваВидаПродукции) Экспорт
	...
	Если СвойстваВидаПродукции <> Неопределено Тогда
		
		ОтдельноеПолеКоличестваКодов = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СвойстваВидаПродукции, "ОтдельноеПолеКоличестваКодов")
			И СвойстваВидаПродукции.ОтдельноеПолеКоличестваКодов;
			
		РазрешенОборотНемаркируемыхОстатков = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СвойстваВидаПродукции, "РазрешенОборотНемаркируемыхОстатков")
			И СвойстваВидаПродукции.РазрешенОборотНемаркируемыхОстатков;
		
	Иначе
		ОтдельноеПолеКоличестваКодов        = Ложь;
		РазрешенОборотНемаркируемыхОстатков = Ложь;
	КонецЕсли;
	...
	ИначеЕсли РазрешенОборотНемаркируемыхОстатков Тогда
		
		Если ТекущаяСтрока.КоличествоПодобрано < ТекущаяСтрока.Количество
			И ТекущаяСтрока.КоличествоПодобрано > 0 Тогда
			
			ТекущаяСтрока.ИндексКодаМаркировки = 5;
...
Процедура СформироватьПредставлениеСодержимогоУпаковки(ТекущаяСтрока) Экспорт
	...
	Если (ЗначениеЗаполнено(ТекущаяСтрока.ВидУпаковки)
		...
		ИначеЕсли ТекущаяСтрока.ЧастичноеВыбытие Тогда // Для частичного выбытия представление количества будет сформировано ранее: 1 - Наименование
...
Процедура ПроверитьДопустимостьКодаМаркировки(ТекущаяСтрока, ПараметрыПроверкиКодовМаркировки, ЕстьПодчиненныеСтроки = Неопределено, ЭтоОСУ = Ложь) Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.ОрганыБлокировкиКМ) Тогда
			ТекущаяСтрока.НедопустимыйКодМаркировки           = Истина;
			ТекущаяСтрока.ПредставлениеПроверкиКодаМаркировки = СтрШаблон(
				НСтр("ru = 'Заблокирован по решению %1';
...
Функция ДопустимыеВложенияАгрегатовПоВидамПродукции() Экспорт
	...
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.ДезинфицирующиеСредства"), ДопустимыеВложенияАгрегатов);
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.БАДы"),            ДопустимыеВложенияАгрегатов);
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.БАДы2025"),        ДопустимыеВложенияАгрегатов);
	...
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.ТелефоныИНоутбуки"),                   ДопустимыеВложенияАгрегатов);
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.ПечатныеПлаты"),                       ДопустимыеВложенияАгрегатов);
	...
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.СоковаяПродукция"),      ДопустимыеВложенияАгрегатов);
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.БезалкогольныеНапитки"), ДопустимыеВложенияАгрегатов);
...
Функция СвойстваВидаПродукцииПоДаннымФормы(Форма)
	...
	СвойстваВидаПродукции.Вставить("РазрешенОборотНемаркируемыхОстатков", ОбщегоНазначенияИСМПВызовСервера.РазрешенОборотНемаркируемыхОстатковПродукции(ВидПродукции));
...
Функция ПоддерживаетсяОбъемноСортовойУчет(ПараметрыСканирования, ВидПродукции, ЭтоВнутреннееПеремещение = Ложь) Экспорт
	
	Возврат ПараметрыСканирования.ПоддерживаетсяОбъемноСортовойУчет
		И ОбщегоНазначенияИСКлиентСерверПовтИсп.ВидПродукцииПоддерживаетОбъемноСортовойУчет(ВидПродукции, ЭтоВнутреннееПеремещение);
...
Функция ВладелецМожетОтличатьсяПоДатеПроизводства(СтрокаДанных, ПараметрыПроверкиКодовМаркировки)
	...
	Если Не ЗначениеЗаполнено(ПараметрыПроверкиКодовМаркировки.ДатаКонтроляВладельцаПоДатеПроизводства)
		Или Не (СтрокаДанных.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Потребительская")
			Или СтрокаДанных.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Групповая")
			Или СтрокаДанных.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Набор")) Тогда
		Возврат Ложь;
	Иначе
		...
		ДатаНачалаКонтроляПриПродажеОптомПоштучно = ПараметрыПроверкиКодовМаркировки.ДатаКонтроляВладельцаПоДатеПроизводства;
