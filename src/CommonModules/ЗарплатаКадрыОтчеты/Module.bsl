		...
		Если СтрНайти(ПутьКВариантуОтчета, ИмяОтчета) = 1 Тогда
			...
			Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, ИмяМакетаПечатнойФормы) Тогда
				...
				Иначе
					...
					ПараметрыКомпоновщика = Новый Структура;
					Для Каждого ЭлементПараметров Из ОбъектОтчета.КомпоновщикНастроек.ПолучитьНастройки().ПараметрыДанных.Элементы Цикл
						Если ЭлементПараметров.Использование Тогда
							Если ЭлементПараметров.Параметр = Новый ПараметрКомпоновкиДанных("СсылкиНаОбъекты")
								Или ЭлементПараметров.Параметр = Новый ПараметрКомпоновкиДанных("КлючВарианта") Тогда
								Продолжить;
							КонецЕсли;
							ПараметрыКомпоновщика.Вставить(Строка(ЭлементПараметров.Параметр), ЭлементПараметров.Значение);
						КонецЕсли;
					КонецЦикла;
					...
					Для каждого Параметр Из МакетКомпоновкиДанных.ЗначенияПараметров Цикл
						...
						Если ПараметрыКомпоновщика.Свойство(Параметр.Имя) Тогда
							ЗначениеПараметраКомпоновщика = ПараметрыКомпоновщика[Параметр.Имя];
							Если ТипЗнч(ЗначениеПараметраКомпоновщика) = Тип("СтандартнаяДатаНачала") Тогда
								Параметр.Значение = ЗначениеПараметраКомпоновщика.Дата;
							ИначеЕсли ТипЗнч(Параметр.Значение) = ТипЗнч(ЗначениеПараметраКомпоновщика) Тогда
								Параметр.Значение = ЗначениеПараметраКомпоновщика;
							КонецЕсли;
						КонецЕсли;
...
Функция ТарифнаяСтавкаНаПечать(Знач ТарифнаяСтавка, Группировка = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ТарифнаяСтавка) Тогда
		...
		Возврат Формат(Цел(ТарифнаяСтавка), ФорматнаяСтрока) + " " + НСтр("ru = 'руб';
																			|en = 'rub'") + ". " + Формат((ТарифнаяСтавка - Цел(ТарифнаяСтавка)), "ЧЦ=2; ЧДЦ=0; ЧС=-2; ЧН=; ЧВН=") + " " + НСтр("ru = 'коп';
																																															|en = 'cents'") + ".";
		
	КонецЕсли;
	
	Возврат "_______________ " + НСтр("ru = 'руб';
										|en = 'rub'") + ". ____ " + НСтр("ru = 'коп';
																	|en = 'cents'") + ".";
...
Функция ПросклоненноеПредставление(Знач ПредставлениеОбъекта, Падеж) Экспорт
	
	Возврат СклонениеПредставленийОбъектов.ПросклонятьПредставление(ПредставлениеОбъекта, Падеж);
...
Процедура ПриКомпоновкеРезультатаВТабличныйДокумент(ОбъектОтчета, ДокументРезультат, ДанныеРасшифровки, 
	СтандартнаяОбработка, ВстраиватьДополнительныеПоляПредставлений = Истина, ВнешниеНаборыДанных = Неопределено) Экспорт
...
Процедура НастроитьРасчетныйЛисток(ОтчетОбъект, ВариантОтчета, КлючПредопределенногоВарианта)
	...
	НастройкиОтчетаКДПоУмолчанию = Неопределено;
	...
	Если НастройкиОтчета <> Неопределено Тогда
		Если ТипЗнч(НастройкиОтчета) <> Тип("Структура") Тогда
			ПрименитьНастройкиКОтчету(ОтчетОбъект, СвойстваВарианта.КлючВарианта, НастройкиОтчета, Неопределено);
			НастройкиОтчетаКД = НастройкиОтчета;
		КонецЕсли;
	КонецЕсли;
	
	Если НастройкиОтчетаКД <> Неопределено Тогда
		КлючВариантаОтчета = "";
		ПараметрОтчета = ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("КлючВарианта");
		Если ПараметрОтчета <> Неопределено И ЗначениеЗаполнено(ПараметрОтчета.Значение) Тогда
			КлючВариантаОтчета = ПараметрОтчета.Значение;
		КонецЕсли;
		КлючВариантаНастроек = "";
		ПараметрНастроек = НастройкиОтчетаКД.ПараметрыДанных.Элементы.Найти("КлючВарианта");
		Если ПараметрНастроек <> Неопределено И ЗначениеЗаполнено(ПараметрНастроек.Значение) Тогда
			КлючВариантаНастроек = ПараметрНастроек.Значение;
		КонецЕсли;
		Если КлючВариантаОтчета = КлючВариантаНастроек Тогда
			ОтчетОбъект.КомпоновщикНастроек.ЗагрузитьНастройки(НастройкиОтчетаКД);
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипЗнч(НастройкиОтчета) = Тип("НастройкиКомпоновкиДанных") Тогда
		НастройкиОтчетаКДПоУмолчанию = ОтчетОбъект.КомпоновщикНастроек.ПолучитьНастройки();
	КонецЕсли;
	
	МакетКомпоновкиДанных = МакетКомпоновкиДанныхОтчета(
		ОтчетОбъект, СвойстваВарианта, , НастройкиОтчетаКДПоУмолчанию);
...
Функция МакетКомпоновкиДанныхОтчетаПоКлючуВарианта(ОтчетОбъект, КлючВарианта, ПодготовитьМакетЕслиНеНайден = Истина) Экспорт
	МетаданныеОтчета = ОтчетОбъект.Метаданные();
	Отчет = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(МетаданныеОтчета);
...
Процедура ОчиститьМакетыКомпоновкиДанных() Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Макеты компоновки данных';
				|en = 'Data composition templates'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция НастройкиПечатнойФормы(Отчет, КлючВарианта) Экспорт
	...
	ВариантОтчета = ВариантыОтчетов.ВариантОтчета(Отчет, КлючВарианта);
	Если ВариантОтчета <> Неопределено Тогда
		Возврат НастройкиВариантаОтчета(ВариантОтчета);
	КонецЕсли;
	Возврат Неопределено;
	
КонецФункции

Функция НоваяСтруктураНастроекПечатнойФормы() Экспорт
	НоваяСтруктура = Новый Структура("Настройки,ПользовательскиеНастройки,СхемаКомпоновкиДанных");
...
Процедура ЗагрузитьНастройкиПечатнойФормы(КомпоновщикНастроекКД, НастройкиПечатнойФормы) Экспорт
	...
	Если НастройкиПечатнойФормы <> Неопределено Тогда
		ОтчетыКлиентСервер.ЗагрузитьНастройки(КомпоновщикНастроекКД, НастройкиПечатнойФормы.Настройки, НастройкиПечатнойФормы.ПользовательскиеНастройки);
...
Процедура ОчиститьМакетОтчетаПоСсылке(Отчет, КлючВарианта = Неопределено) Экспорт
...
Процедура ОчиститьМакетОтчета(МетаданныеОтчета, КлючВарианта = Неопределено) Экспорт
	...
	Отчет = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(МетаданныеОтчета);
	ОчиститьМакетОтчетаПоСсылке(Отчет, КлючВарианта);
	
КонецПроцедуры

Процедура ОчиститьНастройкиВариантаОтчета(ВариантОтчета) Экспорт
	
	ЗапомнитьНастройкиВариантаОтчета(ВариантОтчета, Неопределено);
	
КонецПроцедуры

Процедура ЗапомнитьНастройкиВариантаОтчета(ВариантОтчета, НастройкиВарианта) Экспорт
	
	ВариантОтчетаОбъект = ВариантОтчета.ПолучитьОбъект();
	ВариантОтчетаОбъект.Настройки = Новый ХранилищеЗначения(НастройкиВарианта);
...
Процедура ПриКомпоновкеРезультатаТ49(ОбъектОтчета, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка, НаАванс = Ложь) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'В настройку отчета Т-49 внесены критичные изменения. Отчет не будет сформирован.';
								|en = 'Critical changes have been made to T-49 report setting. Cannot generate the report.'")
...
Процедура ПриКомпоновкеРезультатаТ51(ОбъектОтчета, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка, НаАванс = Ложь) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'В настройку отчета Т-51 внесены критичные изменения. Отчет не будет сформирован.';
								|en = 'Critical changes have been made to T-51 report setting. Cannot generate the report.'")
			+ " " + КраткоеПредставлениеОшибки(Инфо);
...
Процедура ВывестиСотрудникаТ51(НомерСтроки, СтрокаГоловногоСотрудника, ДокументРезультат, РезультатКомпоновки, ПромежуточныеИтоги, КВыплатеПоВедомости, ЕстьОплатаВНатуральнойФорме, ДанныеНаПечать)
	...
	Для каждого СтрокаСотрудника Из СтрокаГоловногоСотрудника.Строки Цикл
		...
		// Обновление итогов
		ДобавитьВПромежуточныйИтог(ПромежуточныеИтогиПоГоловномуСотруднику, ОбластьСтроки.Параметры, ИсключаемыеИтоги);
		ИсключаемыеИтоги = "ДолгЗаОрганизацией,ДолгЗаСотрудником,ВсегоНачислено,НДФЛ,ПрочиеУдержания,ВсегоУдержано,КВыплате";
	...
	Если НастройкиВывода.ВыводитьОсобенностиРасчетаНДФЛ И СтрокаДанныхПоНДФЛ <> Неопределено
		...
		Если НастройкиВывода.ВыводитьИнформациюОНачисленныхВзносахВПФР Тогда
			...
			Если ВсегоВзносов <> 0 Тогда
				...
				Если СтрокаПервогоГоловногоСотрудника.МесяцНачисления < УчетСтраховыхВзносов.ДатаОбъединенияВзносов() Тогда
					ТекстСекции = ТекстСекции
						+ СтрокаСНепереносимымиПробелами(НСтр("ru = 'Страховые взносы в ПФР';
																|en = 'Insurance contributions to Pension Fund'") + ": " + Формат(ВсегоВзносов, "ЧДЦ=2; ЧН="));
				Иначе
					ТекстСекции = ТекстСекции
						+ СтрокаСНепереносимымиПробелами(НСтр("ru = 'Взносы по единому тарифу';
																|en = 'Contributions at joint rate'") + ": " + Формат(ВсегоВзносов, "ЧДЦ=2; ЧН="));
				КонецЕсли;
		...
		Если НастройкиВывода.ВыводитьОсобенностиРасчетаНДФЛ Тогда
			...
			ТекстСекции = ТекстСекции
				+ СтрокаСНепереносимымиПробелами(НСтр("ru = 'Общий облагаемый доход';
														|en = 'Total taxable income'") + ": " + Формат(СтрокаДанныхПоНДФЛ.ОблагаемыйДоход, "ЧДЦ=2; ЧН="));
			...
			Если ЗначениеЗаполнено(СтрокаДанныхПоНДФЛ.АвансовыеПлатежи) Тогда
				...
				ТекстСекции = ТекстСекции
					+ СтрокаСНепереносимымиПробелами(НСтр("ru = 'Зачтено авансовых платежей';
															|en = 'Credited advance payments'") + ": " + Формат(СтрокаДанныхПоНДФЛ.АвансовыеПлатежи, "ЧДЦ=2"));
			...
			Если СтрокаДанныхПоНДФЛ.ПоложеныЛичныеВычеты = Истина
				...
				ТекстСекции = ТекстСекции
					+ СтрокаСНепереносимымиПробелами(НСтр("ru = 'Вычетов личных';
															|en = 'Personal deductions'") + ": " + Формат(СтрокаДанныхПоНДФЛ.ВычетНаФизлицо, "ЧДЦ=2; ЧН="));
			...
			Если СтрокаДанныхПоНДФЛ.ПоложеныВычетыНаДетей = Истина
				...
				ТекстСекции = ТекстСекции
					+ СтрокаСНепереносимымиПробелами(НСтр("ru = 'Вычетов на детей';
															|en = 'Child-related deductions'") + ": " + Формат(СтрокаДанныхПоНДФЛ.ВычетНаДетей, "ЧДЦ=2; ЧН="));
			...
			Если ЗначениеЗаполнено(СтрокаДанныхПоНДФЛ.ВычетИмущественный) Тогда
				...
				ТекстСекции = ТекстСекции
					+ СтрокаСНепереносимымиПробелами(НСтр("ru = 'Вычетов имущественных';
															|en = 'Property-related deduction'") + ": " + Формат(СтрокаДанныхПоНДФЛ.ВычетИмущественный, "ЧДЦ=2; ЧН="));
			...
			Если ЗначениеЗаполнено(СтрокаДанныхПоНДФЛ.ВычетСоциальный) Тогда
				...
				ТекстСекции = ТекстСекции
					+ СтрокаСНепереносимымиПробелами(НСтр("ru = 'Вычетов социальных';
															|en = 'Social deductions'") + ": " + Формат(СтрокаДанныхПоНДФЛ.ВычетСоциальный, "ЧДЦ=2; ЧН="));
				...
				Для каждого СтрокаНачислений Из СтрокаГруппы.Строки Цикл
					...
					Если НастройкиВывода.ВыводитьПериодыНачислений Тогда
						...
						Если ЗначениеЗаполнено(СтрокаНачислений.ДатаНачала) И ЗначениеЗаполнено(СтрокаНачислений.ДатаОкончания) Тогда
							...
							Если Год(СтрокаНачислений.ДатаНачала) <> Год(СтрокаНачислений.ПериодДействия)
								Или Год(СтрокаНачислений.ДатаОкончания) <> Год(СтрокаНачислений.ПериодДействия) Тогда
								
								СтрокаФормата = "ДФ=дд.ММ.гг";
							Иначе
								СтрокаФормата = "ДФ=дд.ММ";
							КонецЕсли;
							...
							Если СтрокаНачислений.ДатаНачала <> СтрокаНачислений.ПериодДействия
								...
								Если СтрокаНачислений.ДатаНачала = СтрокаНачислений.ДатаОкончания Тогда
									СтрокаКоллекции.ПериодДействияПредставление = Формат(СтрокаНачислений.ДатаНачала, СтрокаФормата);
								Иначе
									
									СтрокаКоллекции.ПериодДействияПредставление = Формат(СтрокаНачислений.ДатаНачала, СтрокаФормата) + "-"
										+ Формат(СтрокаНачислений.ДатаОкончания, СтрокаФормата);
...
Процедура ДобавитьСтрокуДанныхРасчетногоЛистка(ДанныеРасчетныхЛистков, СтрокаОтчета, СотрудникОтчета = Неопределено)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаОтчета, "ИмяОбласти") Тогда
		Если СтрНачинаетсяС(СтрокаОтчета.ИмяОбласти, "СтрокаРасшифровка") Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция ПредставлениеВыплаты(ВидВзаиморасчетов, СвойстваРегистратора)
	...
	Если СвойстваРегистратора.Свойство("СпособВыплаты")
		...
		ИначеЕсли ВидВзаиморасчетов = Перечисления.ВидыВзаиморасчетовССотрудниками.ВыплатаЗарплаты
			...
			Если СтрНайти(ВРег(Представление), ВРег(НСтр("ru = 'зарплата';
														|en = 'salary'"))) = 0 Тогда
		...
		ИначеЕсли ВидВзаиморасчетов = Перечисления.ВидыВзаиморасчетовССотрудниками.ВыплатаВМежрасчетныйПериод Тогда
			...
			Если СтрНайти(ВРег(Представление), ВРег(НСтр("ru = 'межрасчет';
														|en = 'outside the payroll period'"))) = 0 Тогда
				Представление = Представление + ", " + НСтр("ru = 'межрасчет';
															|en = 'outside the payroll period'");
			КонецЕсли;
...
Процедура ПриКомпоновкеРезультатаРасчетныйЛистокПоРабочимМестам(ОбъектОтчета, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка, НаАванс = Ложь)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'В настройку отчета ""Расчетный листок по рабочим местам"" внесены критичные изменения. Отчет не будет сформирован.';
								|en = 'Critical changes have been made to the ""Payslip by workplaces"" report settings. Cannot generate the report.'")
			+ " " + КраткоеПредставлениеОшибки(Инфо);
...
Процедура ВывестиСотрудникаРасчетныйЛистокПоРабочимМестам(СтрокиГоловныхСотрудников, ДокументРезультатПромежуточный, Макеты, СоответствиеПользовательскихПолей, ДанныеПоНДФЛИВзносам, ГруппироватьПоИсточникамФинансирования)
	...
	Если ЗначениеЗаполнено(ИтогиПоГруппам.СуммаДолгаНаКонецМесяца) И ИтогиПоГруппам.СуммаДолгаНаКонецМесяца >= 0 Тогда
		Макеты.Сальдо.Параметры.ТекстДолгНаКонецПериода = НСтр("ru = 'Долг за предприятием на конец месяца';
																|en = 'Enterprise debt at the end of the month'");
	...
	ИначеЕсли ЗначениеЗаполнено(ИтогиПоГруппам.СуммаДолгаНаКонецМесяца) И ИтогиПоГруппам.СуммаДолгаНаКонецМесяца < 0 Тогда
		Макеты.Сальдо.Параметры.ТекстДолгНаКонецПериода = НСтр("ru = 'Долг за работником на конец месяца';
																|en = 'Employee''s debt at the end of the month'");
		Макеты.Сальдо.Параметры.СуммаДолгНаКонецПериода = -ИтогиПоГруппам.СуммаДолгаНаКонецМесяца;
	Иначе
		Макеты.Сальдо.Параметры.ТекстДолгНаКонецПериода = НСтр("ru = 'Долг за предприятием на конец месяца';
																|en = 'Enterprise debt at the end of the month'");
	...
	Если ЗначениеЗаполнено(ИтогиПоГруппам.СуммаДолгаНаНачалоМесяца) И ИтогиПоГруппам.СуммаДолгаНаНачалоМесяца >= 0 Тогда
		Макеты.Сальдо.Параметры.ТекстДолгНаНачалоПериода = НСтр("ru = 'Долг за предприятием на начало месяца';
																|en = 'Enterprise debt at the beginning of the month'");
	...
	ИначеЕсли ЗначениеЗаполнено(ИтогиПоГруппам.СуммаДолгаНаНачалоМесяца) И ИтогиПоГруппам.СуммаДолгаНаНачалоМесяца < 0 Тогда
		Макеты.Сальдо.Параметры.ТекстДолгНаНачалоПериода = НСтр("ru = 'Долг за работником на начало месяца';
																|en = 'Employee''s debt at the beginning of the month'");
		Макеты.Сальдо.Параметры.СуммаДолгНаНачалоПериода = -ИтогиПоГруппам.СуммаДолгаНаНачалоМесяца;
	Иначе
		Макеты.Сальдо.Параметры.ТекстДолгНаНачалоПериода = НСтр("ru = 'Долг за предприятием на начало месяца';
																|en = 'Enterprise debt at the beginning of the month'");
...
Функция ВывестиГруппировкиПоСтатьямФинансированияПоРабочимМестам(ДокументРезультат, СтрокиГоловныхСотрудников,  Макеты, СоответствиеПользовательскихПолей)
	...
	Макеты.СтатьяФинансирования.Параметры.СтатьяФинансирования =
		НСтр("ru = 'Итого по всем статьям финансирования';
			|en = 'Totals by all financing items'");
	ДокументРезультат.Вывести(Макеты.СтатьяФинансирования);
				...
				Если НомерСтроки < СтрокиВыплачено.Количество() Тогда
					...
					Для каждого СвойстваРегистратора Из Регистраторы Цикл
						...
						ПредставлениеРегистраторов = ?(ПустаяСтрока(ПредставлениеРегистраторов), "", ПредставлениеРегистраторов + ", ")
							...
							+ " " + НСтр("ru = 'от';
										|en = 'dated'") + " " + Формат(СвойстваРегистратора.Значение.Дата, "ДФ=dd.MM.yy");
...
Процедура КонтрольНастроекОтчетовПриЗаписиРассылкиОтчетов(Рассылка, Отказ) Экспорт
	...
	РезультатПроверки.ИмяСобытия  = НСтр("ru = 'Рассылка отчетов. Проверка настроек';
										|en = 'Report bulk mail. Check settings'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Исключение
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуре %1 не удалось проверить настройки рассылки отчетов %2:';
				|en = 'Cannot check the %2 report distribution settings in the %1 procedure:'"),
		...
		Если ТребоватьПерсонализацииОтчетаПриРассылке(ИнформацияОВарианте.ИмяОтчета, КлючВарианта(НастройкиКД))
			...
			Результат.ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Отчет ""%1"" запрещено рассылать без отбора по физическому лицу';
					|en = 'Cannot send the ""%1"" report without filter by individual'"),
...
//
Процедура ДобавитьВПромежуточныйИтог(ПромежуточныеИтоги, ДанныеСтроки, ИсключаемыеИтоги = "") Экспорт
	
	ИсключаемыеКлючи = Новый Массив;
	Если ЗначениеЗаполнено(ИсключаемыеИтоги) Тогда
		Если ТипЗнч(ИсключаемыеИтоги) = Тип("Строка") Тогда
			ИсключаемыеКлючи = СтрРазделить(ИсключаемыеИтоги, ", ", Ложь);
		КонецЕсли;
	КонецЕсли;
	Для каждого ПромежуточныйИтог Из ПромежуточныеИтоги Цикл
		Если ИсключаемыеКлючи.Найти(ПромежуточныйИтог.Ключ) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ИнициализироватьНастройкиОтчета(ОтчетОбъект, КлючСхемы, КлючВарианта, НовыеНастройкиКД = Неопределено, НовыеПользовательскиеНастройкиКД = Неопределено) Экспорт
	...
	Если КлючСхемы <> КлючВарианта Тогда
		...
		Если КлючВариантаНастроек = КлючВарианта И ВариантНастраиваемойПечатнойФормы(ОтчетОбъект, КлючВариантаНастроек) Тогда
			
			НастройкиУстановлены = ПрименитьНастройкиКОтчету(ОтчетОбъект, КлючВариантаНастроек, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД);
...
Функция ПрименитьНастройкиКОтчету(ОтчетОбъект, КлючВариантаНастроек, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД)
	...
	Если Не НастройкиУстановлены Тогда
		...
		Если ВариантОтчета <> Неопределено Тогда
			...
			НастройкиВарианта = НастройкиВариантаОтчета(ВариантОтчета);
			Если ТипЗнч(НастройкиВарианта) = Тип("Структура") Тогда
				...
				КонецЕсли;
				Если НастройкиВарианта.ПользовательскиеНастройки <> Неопределено Тогда
					Если НовыеПользовательскиеНастройкиКД <> Неопределено
						И НовыеПользовательскиеНастройкиКД.Элементы.Количество() > 0 Тогда
						
						Для Каждого ЭлементНастройкиВарианта Из НастройкиВарианта.ПользовательскиеНастройки.Элементы Цикл
							Если ТипЗнч(ЭлементНастройкиВарианта) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
								...
								ПараметрВарианта = Неопределено;
								Для Каждого ВариантНастроек Из ОтчетОбъект.СхемаКомпоновкиДанных.ВариантыНастроек Цикл
									Если ВариантНастроек.Имя = КлючВариантаНастроек Тогда
										ПараметрВарианта = ВариантНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(
							...
							ИначеЕсли ТипЗнч(ЭлементНастройкиВарианта) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
								...
								ЭлементВарианта = Неопределено;
								Для Каждого ВариантНастроек Из ОтчетОбъект.СхемаКомпоновкиДанных.ВариантыНастроек Цикл
									Если ВариантНастроек.Имя = КлючВариантаНастроек Тогда
										ЭлементВарианта = НайтиЭлементНастройкиПоИдентификаторПользовательскойНастройки(
											ВариантНастроек.Настройки.Отбор.Элементы, ЭлементНастройкиВарианта.ИдентификаторПользовательскойНастройки);
										Прервать;
									КонецЕсли;
								КонецЦикла;
						...
						КонецЦикла;
					КонецЕсли;
					НовыеПользовательскиеНастройкиКД = НастройкиВарианта.ПользовательскиеНастройки;
				КонецЕсли;
			КонецЕсли;
...
Функция НастройкиВариантаОтчета(ВариантОтчета)
	
	НастройкиВарианта = ВариантОтчета.Настройки.Получить();
	Если ТипЗнч(НастройкиВарианта) = Тип("Структура") Тогда
		
		СтруктураНастроекПечатнойФормы = НоваяСтруктураНастроекПечатнойФормы();
		ЗаполнитьЗначенияСвойств(СтруктураНастроекПечатнойФормы, НастройкиВарианта);
		Если НастройкиВарианта.Свойство("ПользовательскиеНастройкиКД")
			И Не НастройкиВарианта.Свойство("ПользовательскиеНастройки") Тогда
			
			СтруктураНастроекПечатнойФормы.ПользовательскиеНастройки = НастройкиВарианта.ПользовательскиеНастройкиКД;
		КонецЕсли;
...
Функция НастройкиШаблонаДляКабинетаСотрудников(КлючВарианта, НастройкиПоУмолчанию = Ложь) Экспорт
	
	ИмяОтчета = Метаданные.Отчеты.ШаблоныДокументовКабинетСотрудника.ПолноеИмя();
	Отчет = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ИмяОтчета);
	НастройкиШаблона = Неопределено;
	Если Не НастройкиПоУмолчанию Тогда
		НастройкиШаблона = НастройкиПечатнойФормы(Отчет, КлючВарианта);
	КонецЕсли;
	
	Если НастройкиШаблона = Неопределено Тогда
		НастройкиШаблона = НоваяСтруктураНастроекПечатнойФормы();
	КонецЕсли;
	
	Если Не НастройкиШаблона.Свойство("СхемаКомпоновкиДанных")
		Или ТипЗнч(НастройкиШаблона.СхемаКомпоновкиДанных) <> Тип("СхемаКомпоновкиДанных") Тогда
		
		ОбъектОтчета = Отчеты.ШаблоныДокументовКабинетСотрудника.Создать();
		НастройкиШаблона.Вставить("СхемаКомпоновкиДанных", ОбъектОтчета.СхемаКомпоновкиДанных);
	КонецЕсли;
	
	Если НастройкиШаблона.Настройки = Неопределено Тогда
		НастройкиШаблона.Настройки =
			НастройкиШаблона.СхемаКомпоновкиДанных.ВариантыНастроек.ПоУмолчанию.Настройки;
	КонецЕсли;
...
Процедура ЗагрузитьНастройкиВКомпоновщикКД(КомпоновщикНастроекКД, ОбъектОтчета, КлючВарианта) Экспорт
	...
	Если НастройкиПечатнойФормы = Неопределено Тогда
		НастройкиПечатнойФормы = НоваяСтруктураНастроекПечатнойФормы();
	КонецЕсли;
	
	Если НастройкиПечатнойФормы.Настройки = Неопределено Тогда
		НастройкиПечатнойФормы.Настройки =
			ОбъектОтчета.СхемаКомпоновкиДанных.ВариантыНастроек[КлючВарианта].Настройки;
	КонецЕсли;
	
	ИнициализироватьИЗагрузитьНастройкиВКомпоновщикКД(КомпоновщикНастроекКД, НастройкиПечатнойФормы, ОбъектОтчета.СхемаКомпоновкиДанных);
	
КонецПроцедуры

Процедура ИнициализироватьИЗагрузитьНастройкиВКомпоновщикКД(КомпоновщикНастроекКД, НастройкиПечатнойФормы, СхемаКомпоновкиДанных = Неопределено) Экспорт
	
	Если СхемаКомпоновкиДанных = Неопределено Тогда
		ИнициализироватьКомпоновщикНастроекКД(КомпоновщикНастроекКД, НастройкиПечатнойФормы.СхемаКомпоновкиДанных);
	Иначе
		ИнициализироватьКомпоновщикНастроекКД(КомпоновщикНастроекКД, СхемаКомпоновкиДанных);
	КонецЕсли;
...
Процедура ИнициализироватьКомпоновщикНастроекКД(КомпоновщикНастроекКД, Схема)
...
Процедура ЗагрузитьНастройкиВКомпоновщик(КомпоновщикНастроекКД, НастройкиКД, ПользовательскиеНастройкиКД = Неопределено) Экспорт
	
	НастройкиПечатнойФормы = НоваяСтруктураНастроекПечатнойФормы();
	НастройкиПечатнойФормы.Настройки = НастройкиКД;
	НастройкиПечатнойФормы.ПользовательскиеНастройки = ПользовательскиеНастройкиКД;
