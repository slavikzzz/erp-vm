...
Функция ДокументМожетФормироватьЗадания(ИмяДокумента) Экспорт
	...
	//++ НЕ УТ
	Если Метаданные.РегистрыСведений.ЗаданияКРасчетуАмортизацииОС.Измерения.Документ.Тип.СодержитТип(ТипДокумента)
	 ИЛИ Метаданные.РегистрыСведений.ЗаданияКРасчетуАмортизацииНМА.Измерения.Документ.Тип.СодержитТип(ТипДокумента)
	 ИЛИ Метаданные.РегистрыСведений.ЗаданияКРасчетуСтоимостиВНА.Измерения.Документ.Тип.СодержитТип(ТипДокумента)
	 ИЛИ Метаданные.РегистрыСведений.ЗаданияКФормированиюДвиженийПоВНА.Измерения.Документ.Тип.СодержитТип(ТипДокумента) Тогда
		Возврат Истина;
	КонецЕсли;
	//-- НЕ УТ
	...
	Если МетаРегистр = Неопределено Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось определить имя контрольного регистра по имени таблицы: %1';
				|en = 'Cannot identify the control register name by the table name: %1'"),
			ИмяТаблицы);
		...
		Если КонтрольныеРегистры.Получить(МетаРегистр) = Неопределено Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 не входит в перечень контрольных регистров для формирования заданий к закрытию месяца (см. %2)';
					|en = '%1 is not included in the list of control registers for generating month-end closing jobs (see %2)'"),
