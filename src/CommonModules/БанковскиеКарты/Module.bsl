...
Функция МаскированныйНомерКарты(Знач Карта) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неверный тип значения параметра (%1)';
																						|en = 'Invalid parameter value type: %1.'"), Строка(ТипЗнч(Карта)));
	КонецЕсли;	
КонецФункции
...
Функция МаскированныеНомераКарт(Знач Карты) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неверный тип значения параметра (%1)';
																						|en = 'Invalid parameter value type: %1.'"), Строка(ТипЗнч(Карты)));
	КонецЕсли;	
КонецФункции
...
Функция Добавить(Знач ДержательКарты, Знач Карта, Знач Свойства = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДержательКарты) Тогда
		ВызватьИсключение НСтр("ru = 'Не указан держатель карты';
								|en = 'Cardholder not specified'")
	КонецЕсли;
	
	Если ТипЗнч(ДержательКарты) <> Тип("СправочникСсылка.БанковскиеКартыКонтрагентов") Тогда
		ВызватьИсключение 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неверный тип значения параметра ДержательКарты (%1)';
					|en = 'Incorrect type of CardHolder parameter value (%1)'"), Строка(ТипЗнч(Карта)));
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неверный тип значения параметра Карта (%1)';
					|en = 'Incorrect type of Card parameter value (%1)'"), Строка(ТипЗнч(Карта)));
	КонецЕсли;
...
Процедура ПриЗаполненииСведенийОбУничтожаемыхПерсональныхДанных(ТаблицаСведений) Экспорт
	Справочники.БанковскиеКартыКонтрагентов.ПриЗаполненииСведенийОбУничтожаемыхПерсональныхДанных(ТаблицаСведений);
КонецПроцедуры
