...
Функция ОписаниеРегистра(Знач ИмяРегистра, Знач ИсключатьНеИспользуемые = Истина) Экспорт
	...
	Если МетаданныеРегистра = Неопределено Тогда
		...
		Если МетаданныеРегистра = Неопределено Тогда
			...
			Если МетаданныеРегистра = Неопределено Тогда
				ВызватьИсключение НСтр("ru = 'Неизвестное имя регистра';
										|en = 'Unknown register name'") + " """ + ИмяРегистра + """";
			КонецЕсли;	
		Иначе
...
Процедура СоздатьВТИмяРегистра(Знач ИмяРегистра, МенеджерВременныхТаблиц, Знач ТолькоРазрешенные, Знач ОписаниеФильтра, ПараметрыПостроения = Неопределено, Знач ИмяРезультирующейТаблицы = Неопределено) Экспорт
	...
	ДобавитьИндексированиеПоСотруднику(ИмяРегистра, ПараметрыПостроения, Ложь, ИмяРезультирующейТаблицы);
...
Процедура СоздатьВТИмяРегистраСрезПервых(Знач ИмяРегистра, МенеджерВременныхТаблиц, Знач ТолькоРазрешенные, Знач ОписаниеФильтра, Знач ПараметрыПостроения = Неопределено, Знач ИмяРезультирующейТаблицы = Неопределено) Экспорт
	...
	ДобавитьИндексированиеПоСотруднику(ИмяРегистра, ПараметрыПостроения, Истина, ИмяРезультирующейТаблицы);
...
Функция ЗапросВТИмяРегистра(Знач ИмяРегистра, Знач ТолькоРазрешенные, Знач ОписаниеФильтра, ПараметрыПостроения = Неопределено, Знач ИмяСоздаваемойТаблицы = Неопределено) Экспорт
	...
	ДобавитьИндексированиеПоСотруднику(ИмяРегистра, ПараметрыПостроения, Ложь, ИмяСоздаваемойТаблицы);
...
Функция ЗапросВТИмяРегистраСрез(Знач ИмяРегистра, Знач ТолькоРазрешенные, Знач ОписаниеФильтра, Знач ПараметрыПостроения = Неопределено, Знач СрезПоследних = Истина, Знач ИмяСоздаваемойТаблицы = Неопределено) Экспорт
	...
	ДобавитьИндексированиеПоСотруднику(ИмяРегистра, ПараметрыПостроения, Истина, ИмяСоздаваемойТаблицы);
...
Функция ОписаниеДополнительногоПоляПредставлений() Экспорт
	Возврат Новый Структура(
		"ИмяПоля,
		|ПустоеЗначениеНаЯзыкеЗапросов,
		|ЗаголовокПоляСКД,
		|ПутьПоляСКД,
		|ПсевдонимыПолей,
		|Соединения,
...
// Добавляет в коллекцию описание дополнительного поля представления
Процедура ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля) Экспорт
	
	Если ИсключаемыеПоля <> Неопределено Тогда
		Для Каждого ИсключаемоеПоле Из ИсключаемыеПоля Цикл
			Если СтрНайти(ОписаниеПоля.ПутьПоляСКД, ИсключаемоеПоле) > 0 Тогда
				Возврат;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	СтрокаПредставления = ДополнительныеПоляПредставлений.Добавить();
	СтрокаПредставления.ИмяПоля = ОписаниеПоля.ИмяПоля;
	СтрокаПредставления.ПустоеЗначениеНаЯзыкеЗапросов = ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов;
	СтрокаПредставления.ПутьПоляСКД = ОписаниеПоля.ПутьПоляСКД;
	
	Если ЗначениеЗаполнено(ОписаниеПоля.ЗаголовокПоляСКД) Тогда
		СтрокаПредставления.ЗаголовокПоляСКД = ОписаниеПоля.ЗаголовокПоляСКД;
	КонецЕсли;
	Если ОписаниеПоля.ДляПоиска = Истина Тогда
		СтрокаПредставления.ДляПоиска = Истина;
	КонецЕсли;
	Если ЗначениеЗаполнено(ОписаниеПоля.ПсевдонимыПолей) Тогда
		СтрокаПредставления.ПсевдонимыПолей = ОписаниеПоля.ПсевдонимыПолей;
	КонецЕсли;
	Если ЗначениеЗаполнено(ОписаниеПоля.Соединения) Тогда
		СтрокаПредставления.Соединения = ОписаниеПоля.Соединения;
	КонецЕсли;
...
Функция ЗапросВТПериоды(Знач ОписаниеНачалаИнтервала, Знач ОписаниеОкончанияИнтервала, Периодичность = "МЕСЯЦ", ИмяПоляПериод = "Период", ИмяВТ = "ВТПериоды", ИспользоватьКонецПериода = Ложь) Экспорт
		...
		Иначе
			...
			Если ЛетВИнтервале > 100 Тогда
				...
				ТекстИсключения = НСтр("ru = 'Попытка получить данные за слишком большой интервал времени';
										|en = 'Attempt to receive data for a too long time interval'")
					+ " (%1 " + НСтр("ru = 'лет';
									|en = 'years'") + ": " + НСтр("ru = 'с';
																|en = 'from'")+ " %2 " + НСтр("ru = 'по';
																						|en = 'to'") + " %3)";
	...
	Если ВРег(Периодичность) <> "ГОД"
		...
		ВызватьИсключение НСтр("ru = 'Невозможно сформировать запрос с переданными параметрами.';
								|en = 'The query with the passed parameters cannot be constructed.'");
		...
		Если СообщатьОРасхожденияхВПутяхКДанным = Истина
			...
			Если ПутиНабора.Получить(Поле.ПутьКДанным) = Неопределено Тогда
				...
				Если Поле.ПутьКДанным <> ОписаниеПоля.ПутьПоляСКД Тогда
					...
					ТекстСообщения = ИмяПоля + " " + НСтр("ru = 'в отчете используется путь';
															|en = 'path is used in the report'") + " - " + Поле.ПутьКДанным;
...
Процедура ДобавитьИндексированиеПоСотруднику(ИмяРегистра, ПараметрыПостроения, ЭтоПараметрыСреза, ИмяРезультирующейТаблицы)
	
	Если Не ЗначениеЗаполнено(ИмяРезультирующейТаблицы) Тогда
		Возврат;
	КонецЕсли;
	
	ПолеИндексирования = "";
	Описание = ОписаниеРегистра(ИмяРегистра);
	Если Описание.Измерения.Найти("Сотрудник") <> Неопределено
		Или Описание.Ресурсы.Найти("Сотрудник") <> Неопределено Тогда
		
		ПолеИндексирования = "Сотрудник";
	ИначеЕсли Описание.Измерения.Найти("ФизическоеЛицо") <> Неопределено
		Или Описание.Ресурсы.Найти("ФизическоеЛицо") <> Неопределено Тогда
		
		ПолеИндексирования = "ФизическоеЛицо";
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ПолеИндексирования) Тогда
		Возврат;
	КонецЕсли;
	
	Если ПараметрыПостроения = Неопределено Тогда
		Если ЭтоПараметрыСреза Тогда
		...
		КонецЕсли;
	Иначе
		ПсевдонимПоля = ПараметрыПостроения.СоответствиеПсевдонимовПолей.Получить(ПолеИндексирования);
		Если ПсевдонимПоля <> Неопределено Тогда
			ПолеИндексирования = ПсевдонимПоля;
		КонецЕсли;
		Если ПараметрыПостроения.ИндексироватьПо <> Неопределено
			И ПараметрыПостроения.ИндексироватьПо.Найти(ПолеИндексирования) <> Неопределено Тогда
			
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	Если ПараметрыПостроения.ИндексироватьПо = Неопределено Тогда
		ПараметрыПостроения.ИндексироватьПо = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПолеИндексирования);
	Иначе
		ПараметрыПостроения.ИндексироватьПо.Добавить(ПолеИндексирования);
	КонецЕсли;
