...
Функция АдресПубликацииИнформационнойБазыВИнтернете() Экспорт
	...
	Если РазделениеВключено() И ДоступноИспользованиеРазделенныхДанных() Тогда 
		Если ПустаяСтрока(Результат)
			...
			Исключение
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Адрес публикации приложения';
												|en = 'Publication address'", КодОсновногоЯзыка()), // АПК:154 Не считаем ошибкой проблемы недоступности Менеджера сервиса. 
...
Функция ОбщиеПараметрыБазовойФункциональности(ВернутьЗакешированноеЗначение = Истина) Экспорт
	...
	Если МинимальнаяИРекомендуемаяВерсияПлатформыЗаполненаНеПравильно(Минимальная, Рекомендуемая) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Базовая функциональность';
										|en = 'Core'", КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,,, 
			ТекстСообщения);		
	КонецЕсли;
...
Функция ВидОбъектаПоТипу(ТипОбъекта) Экспорт
	...
	Иначе
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверный тип значения параметра %1 функции %2: %3.';
				|en = 'Invalid data type of parameter ""%1"" in function ""%2"": ""%3"".'"), Строка(ТипОбъекта)),
...
Функция ПолучитьВерсииИнтерфейсаЧерезВнешнееСоединение(ВнешнееСоединение, Знач ИмяИнтерфейса) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Получение версий интерфейса';
										|en = 'Getting interface versions'", КодОсновногоЯзыка()),
...
Процедура ЗаписатьДанныеВБезопасноеХранилище(Владелец, Данные, Ключ = "Пароль") Экспорт
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.Проверить(ТипЗнч(Данные) = Тип("Структура"),
			...
			|en = 'Incorrect value of the %1 parameter in %2.
...
#КонецЕсли
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Подключение внешней компоненты на сервере';
										|en = 'Attaching add-in on the server'", КодОсновногоЯзыка()),
		...
		Если ЗначениеЗаполнено(ОшибкаСовместимостиКомпонентыИзМакета) Тогда
			Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"" на сервере по причине:
					 ...
					 |%3
					 |Method ""%4"" returned ""False"".'"), Идентификатор, ОшибкаСовместимостиКомпонентыИзМакета, Местоположение,
		...
		Иначе
			Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"" на сервере.
					 ...
					 |Technical information:
					 |%2
					 |The method ""%3"" returned ""False"".'"), Идентификатор, Местоположение, "ПодключитьВнешнююКомпоненту");
		КонецЕсли;
		...
		Если Не ПолеИсточникаСхемыЗапросаДоступно(Источник, ТекущееВыражение) Тогда 
			ТекстОшибки = ТекстОшибки + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Поле объекта ""%1"" не существует.';
					|en = 'The ""%1"" object field does not exist.'"), ТекущееВыражение);
		КонецЕсли;
	...
	Для Каждого МестоИспользования Из Результат.МестаИспользования Цикл
		...
		Иначе
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Замена ссылок в ""%1"" недоступна.';
					|en = 'Cannot replace references in ""%1"".'"), Информация.ПолноеИмя);
...
Функция СобытиеЖурналаРегистрацииЗаменыСсылок()
	...
	Возврат НСтр("ru = 'Поиск и удаление ссылок';
				|en = 'Searching for references and deleting them'", КодОсновногоЯзыка());
...
Процедура ЗарегистрироватьОшибкуВТаблицу(Результат, Дубль, Оригинал, Данные, Информация, ТипОшибки, ИнформацияОбОшибке)
	...
	ИначеЕсли ТипОшибки = "ЗаписатьНаборОригинала" Тогда
		НовыйШаблон = НСтр("ru = 'Не удалось обновить сведения в %1: %2';
							|en = 'Cannot update additional data in %1: %2'");
		Ошибка.ТипОшибки = "ОшибкаЗаписи";
	Иначе
...
Процедура ПроверитьИмяПроцедурыКонфигурации(Знач ИмяПроцедуры)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Безопасное выполнение метода';
										|en = 'Executing method in safe mode'", КодОсновногоЯзыка()),
...
Функция МенеджерОбъектаПоИмени(Имя)
	...
	ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Неверное значение параметра %1 в %2. Менеджер для объекта ""%3"" не существует.';
			|en = 'Invalid value of parameter ""%1"" in ""%2"". Manager for object ""%3"" doesn''t exist.'"), Имя),
		...
		Если МинимальнаяИРекомендуемаяВерсияПлатформыЗаполненаНеПравильно(Минимальная, Рекомендуемая) Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Минимальная и рекомендуемая версии платформы ""1С:Предприятие"", полученные с сервиса обновления приложений, не соответствуют следующим требованиям:
					...
					|en = 'The minimum and recommended 1C:Enterprise versions received from the update service do not meet the following requirements:
					| - Minimum version must be specified.
					| - Minimum version cannot be earlier than the minimum SSL version (see %1).
					| - Minimum version cannot be earlier than the recommended version.
					|Minimum version of the update service: %2
					|Minimum SSL version: %3
					|Recommended version of the update service: %4
					|
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось изменить минимальную и рекомендуемую версию платформы ""1С:Предприятие"", полученную с сервиса обновления приложений, по причине:
				|%1';
				|en = 'Cannot change the minimum and recommended 1C:Enterprise versions received from the update service. Reason:
				|%1'",
...
Процедура ПроверитьАлгоритм(Знач Алгоритм)
	...
	Если НайденныеВызовы.Количество() > 0 Тогда
		ТекстОшибки = НСтр("ru = 'Вызовы недопустимых свойств глобального контекста:
			|%1';
			|en = 'Calls to invalid global context properties:
			|%1'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, СтрСоединить(НайденныеВызовы, Символы.ПС));
