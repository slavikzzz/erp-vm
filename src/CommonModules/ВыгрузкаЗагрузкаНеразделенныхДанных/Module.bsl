...
Процедура ПриРегистрацииОбработчиковВыгрузкиДанных(ТаблицаОбработчиков) Экспорт
	...
	ОбъектыДляКонтроляНеразделенныхДанныхПриВыгрузке
...
Процедура КонтрольИспользованияСсылокНаНеразделенныеДанныеВРазделенных() Экспорт
	...
	Исключение
		...
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Обнаружены ошибки в структуре метаданных конфигурации: %1';
										|en = 'Errors found in the structure of metadata configuration: %1'", Метаданные.ОсновнойЯзык.КодЯзыка),
			ТекстОшибки);
...
Процедура ПроверитьПоляЕстественногоКлюча(Знач ОбъектМетаданных, Знач ПоляЕстественногоКлюча)
	...
	Если ПоляЕстественногоКлюча = Неопределено Или ПоляЕстественногоКлюча.Количество() = 0 Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Для типа данных %1 не указаны естественные ключи для замены ссылок.
                  ...
                  |en = 'Natural keys for reference replacement are not specified for %1 data type.
                  |Check the %2 handler.'"),
	...
	Для Каждого ПолеДляКонтроляСсылокНаНеразделенныеДанные Из ПоляДляКонтроляСсылокНаНеразделенныеДанные Цикл
		...
		Иначе
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Объект метаданных %1 не поддерживается.';
											|en = 'Metadata object %1 is not supported.'", Метаданные.ОсновнойЯзык.КодЯзыка),
				ПолноеИмяОбъектаМетаданных);
		КонецЕсли;
...
Процедура КонтрольСсылкиНаНеразделенныеДанныеПриВыгрузке(Контейнер, Знач ПроверяемаяСсылка, Знач ИсходныйОбъект, Знач ИсходныйОбъектМетаданных, Знач ИмяПоля)
	...
	ШаблонОшибки =
		НСтр("ru = 'Объект метаданных %1 включен в перечень объектов, для которых не требуется сопоставление ссылок при выгрузке / загрузке
              ...
              |%6
              |---------------------------------------------------------------------------------------------------------------------------';
              |en = 'The %1 metadata object is included in the list of objects, for which reference mapping is not required when importing/exporting
              |data in overridable procedure
              |%2. The requirement for no unmapped references is not satisfied for it on export.
              |
              | Unmapped reference is detected when exporting the %3object, which has a reference to the %1object set as the %4
              |attribute value, which cannot be correctly mapped when importing data. 
              | Revise the logic of the %1 object usage and make sure it has no unmapped references
              |in exported data.
              |
              |Diagnostic data:
              | 1. Exported object serialization:
              |---------------------------------------------------------------------------------------------------------------------------
              |%5
              |---------------------------------------------------------------------------------------------------------------------------
              |2. Serialization of unmapped reference object
              |---------------------------------------------------------------------------------------------------------------------------
              |%6
              |---------------------------------------------------------------------------------------------------------------------------'");
