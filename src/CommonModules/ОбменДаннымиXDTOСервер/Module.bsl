...
Функция ПКОПоИмени(КомпонентыОбмена, Имя) Экспорт
	...
	Если ПравилоКонвертации = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найдено ПКО с именем %1';
																						|en = 'Object conversion rule ""%1"" is not found.'"), Имя);
...
Процедура ПроизвестиВыгрузкуДанных(КомпонентыОбмена) Экспорт
	...
	Если КомпонентыОбмена.ФлагОшибки Тогда
		...
		ВызватьИсключение НСтр("ru = 'При формировании сообщения обмена данными произошли ошибки. Подробнее см. в журнале регистрации.';
								|en = 'Errors generating data exchange message. For details, see Event log.'");
		...
		Если (ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента
			...
			Если СообщениеОтНеобновленнойНастройки(ЧтениеXML) Тогда
				СтрокаСообщенияОбОшибке = НСтр("ru = 'Получение данных от источника, в котором не выполнено
					...
					|en = 'Receive data from a source where the data synchronization
					|settings are not updated. Perform the following actions:
					|1) For the Internet transport kind:
					|	- Repeat the data synchronization after some time.
					|2) For other transport kinds:
					|	- Synchronize the data on the source side.
					|	After that, synchronize the data again in this infobase.'");
				ЗаписатьВПротоколВыполнения(КомпонентыОбмена, СтрокаСообщенияОбОшибке);
			Иначе
	...
	Если Не РазрешенаЗаписьОбъекта(Объект, КомпонентыОбмена) Тогда
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Попытка изменения неразделенных данных (%1: %2) в разделенном режиме.';
				|en = 'Attempt to modify shared data (%1: %2) in the separated mode.'"),
...
Функция ОтменитьПроведениеОбъектаВИБ(Объект, Отправитель, КомпонентыОбмена = Неопределено) Экспорт
	...
	Исключение
		...
		ШаблонТекста = НСтр("ru = 'При отмене проведения документа:
			...
			|en = 'When unposting the
			|%1
			|document, an error occurred before reimporting it:
			|%2. 
			|The issue can be ignored (hidden) if the document is posted after import.
			|For error details, see the event log'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура КонвертацияСвойстваXDTOВЭлементСтруктуры(Источник, Свойство, Приемник,
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка чтения объекта XDTO, имя свойства: <%1>.';
																					|en = 'XDTO object reading error. Property: <%1>.'"), ИмяСвойства)
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Исключение
		ПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка конвертации свойства объекта XDTO, имя свойства: <%1>.';
																					|en = 'XDTO object property conversion error. Property: <%1>.'"), ПКС.СвойствоКонфигурации)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Если ЭлементыФормата.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неканоническое имя формата обмена <%1>';
																						|en = 'Non-canonical exchange format name: ""%1""'"), ФорматОбмена);
	КонецЕсли;
...
Процедура ОбновитьНастройкиXDTOКорреспондента(КомпонентыОбмена)
	...
	Если МаксимальнаяОбщаяВерсия <> НомерВерсииКорреспондента Тогда
		...
		ЗаписатьВПротоколВыполнения(КомпонентыОбмена, 
			НСтр("ru = 'Изменен номер версии формата обмена.';
				|en = 'Exchange format version has been changed.'"), , Ложь, , , Истина);
	КонецЕсли;
...
Функция ПОДПоИмени(КомпонентыОбмена, Имя)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найдено ПОД с именем %1';
																						|en = 'Cannot find a DER with name ""%1""'"), Имя);
...
Функция ПредставлениеКоллекцииСвойствДляПротокола(КоллекцияСвойств)
		...
		Если КоллекцияСвойств.Свойство("Дата", ЗначениеСвойстваДата) // @Non-NLS
			...
			ЭлементыПредставления.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '№%1 от %2';
																			|en = '#%1, %2'"), ЗначениеСвойства, Формат(ЗначениеСвойстваДата, "ДЛФ=D")));
		Иначе
			ЭлементыПредставления.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '№%1';
																			|en = '#%1'"), ЗначениеСвойства));
		КонецЕсли;
	...
	ИначеЕсли КоллекцияСвойств.Свойство("Дата", ЗначениеСвойства) // @Non-NLS
		...
		ЭлементыПредставления.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'от %1';
																		|en = ', %1'"), ЗначениеСвойства));
...
Процедура ПроверитьВерсию(Знач Версия)
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неканоническое представление версии формата обмена: <%1>.';
																						|en = 'Non-canonical presentation of the exchange format v.%1.'"), Версия);
	КонецЕсли;
