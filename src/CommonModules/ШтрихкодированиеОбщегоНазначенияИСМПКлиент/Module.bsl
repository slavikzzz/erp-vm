	...
	Если ПараметрыПроверки.ЕстьДанныеДляПроверкиИСМП Тогда
		...
		Если Не ЗначениеЗаполнено(ПараметрыПроверки.ИдентификаторУстройства) Тогда
			ВызватьИсключение НСтр("ru = 'Внутреняя ошибка. Не заполнен идентификатор устройства ККТ';
									|en = 'Внутреняя ошибка. Не заполнен идентификатор устройства ККТ'");
		КонецЕсли;
...
Функция ЗавершитьОбработкуВводаШтрихкода(ПараметрыЗавершенияВводаШтрихкода, ВыполнятьОбработчикОповещения = Истина) Экспорт
	...
	Если ТребуетсяПроверкаРазрешительногоРежимаНаЛокальномМодуле Тогда
	...
	ИначеЕсли Результат[Организация] = Неопределено Тогда
		...
		Если ПараметрыСканирования.РежимБРМК Тогда
			...
			РезультатОбработкиШтрихкода.ТекстОшибки                 = НСтр("ru = 'Авторизация в сервисе ГИС МТ не пройдена';
																			|en = 'Авторизация в сервисе ГИС МТ не пройдена'");
			РезультатОбработкиШтрихкода.ЕстьОшибки                  = Истина;
...
// 
// Параметры:
//  СтрокаОбработки                   - Структура - (См. ШтрихкодированиеОбщегоНазначенияИС.ИнициализироватьДанныеШтрихкода)
...
Асинх Функция ЗапросРазрешительногоРежимаНаЛокальномМодулеЧестныйЗнак(СтрокаОбработки, ПараметрыСканирования = Неопределено, ПараметрыЗавершенияВводаШтрихкода = Неопределено) Экспорт
	...
	Состояние(ТекстВыполненияЗапроса, 0,, БиблиотекаКартинок.ДлительнаяОперация16);
	
	Если Не ПараметрыЗавершенияВводаШтрихкода = Неопределено Тогда
		ПараметрыСканирования       = ПараметрыЗавершенияВводаШтрихкода.ПараметрыСканирования;
	КонецЕсли;
	
	Если СтрокаОбработки.ТребуетсяПроверкаРазрешительногоРежимаНаЛокальномМодуле Тогда
		...
		#Если ВебКлиент Тогда
			...
			РезультатОффлайнЗапроса = Ждать ОбщегоНазначенияИСМПВебКлиент.ДанныеЛокальногоМодуляЧЗПоКИПриРозничнойПродаже(
				...
				СтрокаОбработки.СписокЛокальныхМодулейДляОбработки);
		...
		Если РезультатОффлайнЗапроса <> Неопределено Тогда
			...
			ИначеЕсли РезультатОффлайнЗапроса.ВидыПродукцииБезНастроенныхЛокальныхМодулей.Найти(СтрокаОбработки.ВидПродукции) <> Неопределено Тогда
				...
				Если ПараметрыСканирования.РасширеннаяВерсияГосИС Тогда
					...
					ВидДополнения.ВидОперацииИСМП             = ПараметрыСканирования.ВидОперацииИСМП;
					ВидДополнения.Организация                 = ПараметрыСканирования.Организация;
					ВидДополнения.ВидПродукцииИС              = СтрокаОбработки.ВидПродукции;
				...
				СтрокаОбработки.ТекстОшибки = СтрШаблон(ШаблонТекстаОшибки, СтрокаОбработки.ВидПродукции);
	...
	Состояние(ТекстВыполненияЗапроса, 100,, БиблиотекаКартинок.ДлительнаяОперация16);
...
Асинх Функция ОбработатьАсинхронноШтрихкодНаЛокальномМодулеЧестныйЗнак(ПараметрыЗавершенияВводаШтрихкода) Экспорт
	...
	Иначе
		...
		ПередаваемыеЗначенияПараметровЗавершения = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ПараметрыЗавершенияВводаШтрихкода);
		РезультатОбработки = Ждать ЗавершитьОбработкуКодаМаркировкиАсинхронноПослеВыполненияЗапросаНаЛокальномМодулеЧестныйЗнак(ПередаваемыеЗначенияПараметровЗавершения);
			...
			Если Не ПараметрыСканирования.ТребуетсяПроверкаСредствамиККТ
				...
				Если Не (КодВключаетКриптоХвост Или КодВключаетМРЦ) Тогда
					СтрокаРезультат.ТекстОшибки = НСтр("ru = 'Требуется полный код маркировки (включая МРЦ)';
														|en = 'Требуется полный код маркировки (включая МРЦ)'");
		...
		Если ЭлементПроверки.ЧастичноеВыбытиеОстаток < 0 Тогда
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Недостаточно остатка во вскрытой потребительской упаковке: %1 %2';
					|en = 'Недостаточно остатка во вскрытой потребительской упаковке: %1 %2'"),
...
Процедура ПроверкаИдентификатораГИСМТПоКодуМаркировки(ПараметрыПроверки)
	...
	Если ПараметрыПроверки.ВыполняетсяЛогирование Тогда
		...
		ТекстЛога = ТекстДляЗаписиВЛогЗапросов(
			НСтр("ru = 'Получение идентификаторов разрешительного режима';
				|en = 'Получение идентификаторов разрешительного режима'"),
			ПараметрыПроверки);
	...
	ПараметрыОжидания.ТекстСообщения                 = НСтр("ru = 'Получение идентификаторов запроса ГИС МТ при розничной продаже.';
															|en = 'Получение идентификаторов запроса ГИС МТ при розничной продаже.'");
	...
	Если ПараметрыПроверки.ВыполняетсяЛогирование
		...
		ТекстЛога = ТекстДляЗаписиВЛогЗапросов(
			НСтр("ru = 'Локальная проверка средствами ККТ';
				|en = 'Локальная проверка средствами ККТ'"),
			ПараметрыПроверки,
		...
		Если ДанныеДляЛогирования.Количество() Тогда
			ТекстЛога = ТекстДляЗаписиВЛогЗапросов(
				НСтр("ru = 'Проверка статуса товара ОИСМ средствами ККТ';
					|en = 'Проверка статуса товара ОИСМ средствами ККТ'"),
				ПараметрыПроверки,,
	...
	Если ПараметрыПроверки.ВыполняетсяЛогирование Тогда
		...
		ТекстЛога = ТекстДляЗаписиВЛогЗапросов(
			НСтр("ru = 'Подтверждение кода маркировки при выбытии';
				|en = 'Подтверждение кода маркировки при выбытии'"),
			ПараметрыПроверки,
...
Асинх Процедура ПолучениеИдентификатораГИСМТЗавершение(Результат, ПараметрыПроверки) Экспорт
	...
	Если ДанныеОтвета.ОтсутствуютУстановленныеЛМЧЗ Тогда
	...
	Для Каждого КлючИЗначение Из ДанныеОтвета.ДанныеИдентификаторов Цикл
		...
		Если ДанныеПоКМ.ТребуетсяПроверкаРазрешительногоРежимаНаЛокальномМодуле Тогда
			
			РезультатОффлайнРежима = Ждать ЗапросРазрешительногоРежимаНаЛокальномМодулеЧестныйЗнак(ДанныеПоКМ, ПараметрыПроверки.ПараметрыСканирования);
						...
						Если ЭлементПроверки.Количество > ДанныеПоКМ.ЧастичноеВыбытиеОстатокГИСМТ Тогда
							...
							ТекстОшибкиОстатокВКеге = СтрШаблон(
								НСтр("ru = 'Остаток в кеге по данным ГИС МТ: %1 л., требуется: %2 л.';
									|en = 'Остаток в кеге по данным ГИС МТ: %1 л., требуется: %2 л.'"),
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(КлючИЗначение.Значение, "ТекстОшибки")
			И ЗначениеЗаполнено(КлючИЗначение.Значение.ТекстОшибки)
			И Не ЗначениеЗаполнено(РезультатЭлементаПроверки.ТекстОшибкиГИСМТ) Тогда
			
			РезультатЭлементаПроверки.ТекстОшибкиГИСМТ   = КлючИЗначение.Значение.ТекстОшибки;
			ПараметрыПроверки.Результат.ЕстьОшибки       = Истина;
...
Процедура ОбновлениеCDNПлощадокГИСМТЗавершение(Результат, ПараметрыПроверки) Экспорт
	...
	ИначеЕсли Результат.Результат.Свойство("ОтказОтАвторизации") И Результат.Результат.ОтказОтАвторизации Тогда
		...
		ПараметрыПроверки.Результат.ТекстОшибкиГИСМТ  = НСтр("ru = 'Произошла ошибка при получении CDN-площадок ГИС МТ.';
															|en = 'Произошла ошибка при получении CDN-площадок ГИС МТ.'");
...
Функция ЗавершитьОбработкуКодаМаркировкиПоУточнениямИПроверкам(ПараметрыЗавершенияВводаШтрихкода, РезультатОбработкиШтрихкода, ВыполнятьОбработчикОповещения)
...
Асинх Функция ЗавершитьОбработкуКодаМаркировкиАсинхронноПослеВыполненияЗапросаНаЛокальномМодулеЧестныйЗнак(ПараметрыЗавершенияВводаШтрихкода)
	...
	Если Не ПараметрыЗавершенияВводаШтрихкода = Неопределено Тогда
		...
		ПараметрыЗавершенияВводаШтрихкода.РезультатОбработкиШтрихкода.ДанныеШтрихкода.ТребуетсяПроверкаРазрешительногоРежимаНаЛокальномМодуле = Ложь;
		
		ПараметрыЗавершенияВводаШтрихкода.РезультатОбработкиШтрихкода.ТребуетсяПроверкаРазрешительногоРежимаНаЛокальномМодуле = Ложь;
