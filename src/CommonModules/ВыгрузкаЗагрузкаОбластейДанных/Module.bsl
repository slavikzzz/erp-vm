...
Процедура ПроверитьВыгрузкаВАрхивеСовместимаСТекущейКонфигурацией(Знач ИмяАрхива) Экспорт
	...
	Попытка
		...
		Если ЭлементОписанияВыгрузки = Неопределено Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'В файле выгрузки отсутствует файл %1';
											|en = '%1 is missing from the export file'"), "DumpInfo.xml");
		КонецЕсли;
...
Функция ЗагрузитьТекущуюОбласть(
	...
	Если Не ПараметрыЗагрузкиВнутр.Свойство("КоличествоПотоков") Тогда
		ПараметрыЗагрузкиВнутр.Вставить("КоличествоПотоков", 0);
	КонецЕсли;
	
	Если ПараметрыЗагрузкиВнутр.КоличествоПотоков < 1 Тогда
		ПараметрыЗагрузкиВнутр.КоличествоПотоков = 1;
	КонецЕсли;
