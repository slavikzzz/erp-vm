...
Функция ПоместитьФайлВХранилищеВСервисе(Прокси, Знач ИмяФайла, 
	...
	Если Прокси = Неопределено Тогда
		...
		ВызватьИсключение НСтр("ru = 'Не определен WS-прокси передачи файла выгрузки в базу приемник. 
			...
			|Contact the administrator.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииИзменениеТранспортаНаWS() Экспорт
	...
	Возврат НСтр("ru = 'Обмен данными.Изменение транспорта на WS';
				|en = 'Data exchange.Change transport to WS'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУстановкаПодключенияКWebСервису()
	...
	Возврат НСтр("ru = 'Обмен данными.Установка подключения к web-сервису';
				|en = 'Data exchange.Establish web service connection'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОжиданиеЗавершенияОперации(СтруктураНастроекОбмена, ПараметрыОбмена, Прокси, ДействиеПриОбменеВЭтойИБ = Неопределено) Экспорт
	...
	Если ПараметрыОбмена.ИнтервалОжиданияНаСервере = 0 Тогда
		...
		Если ДействиеПриОбменеВЭтойИБ <> Неопределено Тогда
			...
			Если ДействиеПриОбменеВЭтойИБ = Перечисления.ДействияПриОбмене.ЗагрузкаДанных Тогда
				...
				ДействиеВКорреспондентеСтрокой = НСтр("ru = 'выгрузка';
														|en = 'export'", ОбщегоНазначения.КодОсновногоЯзыка());
				
			Иначе
				
				ДействиеВКорреспондентеСтрокой = НСтр("ru = 'загрузка';
														|en = 'import'", ОбщегоНазначения.КодОсновногоЯзыка());
				
			КонецЕсли;
			
			ШаблонСообщения = НСтр("ru = 'Ожидание выполнения операции (%1 данных в базе-корреспонденте)...';
									|en = 'Waiting for the operation to be executed (%1 of the data in the peer infobase)…'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Пока ПараметрыОбмена.ДлительнаяОперация Цикл // Замена рекурсии
		...
		Иначе
			...
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Ошибка в базе-корреспонденте:%1 %2';
											|en = 'Peer infobase error:%1 %2'"), Символы.ПС, СтрокаСообщенияОбОшибке);
