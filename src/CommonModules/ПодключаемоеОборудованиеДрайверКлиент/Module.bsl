...
Асинх Функция ОбъектДрайвераАсинх(ПараметрыПодключения) Экспорт
	...
	ТекстСообщения = СтрШаблон(
		НСтр("ru = 'Для продолжения работы требуется установка внешней компоненты ""%1"".';
			|en = 'To continue, set the ""%1"" add-in.'"), 
	...
	Если ПараметрыПодключения.ПодключениеИзМакета Или ОбщегоНазначенияБПОКлиентСервер.ЭтоМобильныйКлиент() Тогда
		...
		Операция = НСтр("ru = 'Подключить из макета';
						|en = 'Подключить из макета'");
		Если Не ПараметрыПодключения.МакетДоступен Тогда
			
			ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Для внешней компоненты макет %1 не обнаружен.';
											|en = 'Для внешней компоненты макет %1 не обнаружен.'"), ПараметрыПодключения.ИмяМакетаДрайвера);
		...
		Иначе
			...
			ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Подключение внешней компоненты из макета %1 не допустимо.';
											|en = 'Подключение внешней компоненты из макета %1 не допустимо.'"), ПараметрыПодключения.ИмяМакетаДрайвера);
			ЗаписатьОшибкуВЛог(ПараметрыПодключения, Операция, ОписаниеОшибки);
	...
	#Иначе                   
		Операция = НСтр("ru = 'Подключить из реестра Windows';
						|en = 'Подключить из реестра Windows'");
			...
			Попытка                         
				...
				Если Не РезультатВызова.Значение Тогда
					ОбщегоНазначенияБПОКлиент.СообщитьПользователю(НСтр("ru = 'Ошибка локализации драйвера.';
																		|en = 'Driver localization error.'"));
					ПолучитьОшибкуДрайвера(ОбъектДрайвера, ЗаписьЛога);
				КонецЕсли
			Исключение
				ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, Неопределено);
			КонецПопытки;   
		КонецЕсли;
		...
		Если ОписаниеДрайвера.РевизияИнтерфейса >= 4000 И ОписаниеДрайвера.ПоддерживаетсяЛокализация Тогда   
			...
			Исключение
				...
				ОбщегоНазначенияБПОКлиент.СообщитьПользователю(НСтр("ru = 'Ошибка получения шаблон локализации драйвера.';
																	|en = 'An error occurred when getting the driver localization template.'") + Символы.ПС + ОписаниеОшибки);
			КонецПопытки;
			Если РезультатВызова.Значение <> Истина Тогда
				ПолучитьОшибкуДрайвера(ОбъектДрайвера, ЗаписьЛога);
			КонецЕсли;
...
Асинх Функция ПараметрыДрайвераАсинх(ОбъектДрайвера, ОписаниеДрайвера, ПараметрыПодключения) Экспорт
	...
	Если ПараметрыПодключения.РевизияИнтерфейса > 4002 Тогда
		...
		Исключение
			ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, Неопределено);
			ОписаниеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ОбщегоНазначенияБПОКлиент.СообщитьПользователю(НСтр("ru = 'Ошибка получения параметров драйвера.';
																|en = 'An error occurred when receiving the driver parameters.'") + Символы.ПС + ОписаниеОшибки);
			Возврат Ложь;
		КонецПопытки;
	Иначе
		Попытка
	...
	Исключение
		...
		ОбщегоНазначенияБПОКлиент.СообщитьПользователю(НСтр("ru = 'Ошибка получения описания драйвера.';
															|en = 'An error occurred when receiving the driver description.'"));
		Возврат Ложь;
	КонецПопытки;
...
Асинх Функция ПодключитьУстройствоАсинх(ПараметрыПодключения) Экспорт
	...
	Если ПустаяСтрока(ПараметрыПодключения.РевизияИнтерфейса) Или ПараметрыПодключения.РевизияИнтерфейса = 0 Тогда
		ОписаниеОшибки = НСтр("ru = 'Ревизия интерфейса драйвера не определена.';
								|en = 'Driver interface revision is not defined.'");
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
	Иначе
...
Асинх Функция ТестУстройстваАсинх(ПараметрыПодключения) Экспорт
	...
	Если РевизияИнтерфейса > 4002 Тогда
		...
		Исключение                                                       
			ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, Неопределено);
			Возврат ОшибкаВызоваДрайвера("ТестированиеОборудования");
		КонецПопытки;
...
Асинх Функция АвтонастройкаУстройстваАсинх(ПараметрыПодключения) Экспорт
	...
	Если РевизияИнтерфейса > 4002 Тогда
		...
		Попытка
			...
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "EquipmentAutoSetup", 
				ВМассив(,ПараметрыПодключения.ПараметрыXML,,ПоказыватьОкноАвтонастройки,Таймаут));
	...
	Иначе
		ОписаниеОшибки = НСтр("ru = 'Драйвер не поддерживает операцию.';
								|en = 'The operation is not supported by the driver.'"); 
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
	КонецЕсли;
	...
	Если Результат Тогда
		...
		Исключение 
			...
			ОписаниеОшибки = НСтр("ru = 'Ошибка выполнения дополнительного действия.';
									|en = 'An error occurred when performing an additional action.'");
			РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
			Возврат РезультатВыполнения; 
		КонецПопытки;
		РезультатВыполнения = РезультатОперацииНаОборудовании(Истина);
...
Функция ОбработатьСобытие(ОбъектДрайвера, ПараметрыПодключения, Событие, Данные) Экспорт
	...
	Операция = СтрШаблон(НСтр("ru = 'Событие: %1';
								|en = 'Событие: %1'"), Событие);
...
Функция ОшибкаЗагрузкиДрайвера(ПараметрыПодключения, ОписаниеОшибки = "") Экспорт
	...
	Текст = НСтр("ru = '%1: Не удалось загрузить драйвер устройства.
				...
				|%2';
				|en = '%1: Cannot import the device driver.
				|Check if the driver is correctly installed and registered in the application.
				|%2'");
...
Функция ОшибкаВызоваДрайвера(МетодДрайвера = Неопределено, ОписаниеОшибки = Неопределено, ЗаписьВЖурналРегистрации = Ложь) Экспорт
	...
	Если НЕ ПустаяСтрока(МетодДрайвера) Тогда
		СообщениеОбОшибке = НСтр("ru = 'Ошибка вызова метода драйвера <%1>.';
								|en = 'An error occurred when calling the <%1> driver method.'");
	...
	Иначе
		СообщениеОбОшибке = НСтр("ru = 'Данный тип оборудования не поддерживает данную команду.';
								|en = 'This peripherals type does not support the command.'");
	КонецЕсли;
	...
	Если ЗаписьВЖурналРегистрации Тогда
		ОбщегоНазначенияБПОКлиент.ЗаписатьОшибкуВЖурналРегистрации(НСтр("ru = 'Подключаемое оборудование.';
																		|en = 'Peripherals.'"), СообщениеОбОшибке);
	КонецЕсли;
...
Асинх Функция УстановитьИнформациюПриложенияАсинх(ОбъектДрайвера, ПараметрыПодключения) 
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ru = 'Ошибка установки информации приложения для драйвера.';
								|en = 'Error installing the application information for the driver.'");
	...
	ИначеЕсли ПараметрыПодключения.ТипыОборудования.ККТ Тогда
		...
		Попытка
			...
			Иначе // Версия стандарта 2.2 (ОФД 1.05)
				...
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "ProcessCheck", ВМассив(,ДанныеОперации.Электронно, Экранировать(ДанныеОперации.ДанныеЧекаXML)));
	...
	Попытка
		Если ДанныеОперации.Свойство("ТестовыеЧеки") И ДанныеОперации.ТестовыеЧеки.Количество() > 0 Тогда
			Для Каждого ТекстПечати Из ДанныеОперации.ТестовыеЧеки Цикл
				...
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PrintTextDocument", ВМассив(,ТекстПечати));
		...
		Если ДанныеОперации.Свойство("КопияЧека") И ДанныеОперации.КопияЧека.Количество() > 0 Тогда
			Для Каждого Копия Из ДанныеОперации.КопияЧека Цикл
				...
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PrintTextDocument", ВМассив(,Копия));
...
Асинх Функция Инкассация(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	ИначеЕсли ПараметрыПодключения.ТипыОборудования.ККТ Тогда
		...
		Попытка
			...
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "CashInOutcome", ВМассив(,ДанныеОперации.ДанныеЧекаXML, СуммаЧека));
...
Асинх Функция НапечататьКопиюЧека(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ДанныеОперации.Свойство("Аппаратно") И ДанныеОперации.Аппаратно Тогда
		...
		Попытка
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PrintCheckCopy", ВМассив(, НомерЧека));
...
Асинх Функция ОткрытьСессиюРегистрацииКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если НЕ ПустаяСтрока(ИдентификаторСессии) Тогда  
		...
		ОписаниеОшибки = НСтр("ru = 'Сессия проверки кодов маркировки уже открыта.';
								|en = 'Session of marking code check is opened.'");
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "OpenSessionRegistrationKM", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
...
Асинх Функция ЗапросКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если НЕ ПустаяСтрока(ИдентификаторЗапроса) Тогда
		...
		Попытка
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "ConfirmKM", ВМассив(, ИдентификаторЗапроса, 0));
	...
	Попытка
		...
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "RequestKM", ВМассив(, Экранировать(ДанныеОперации.ПараметрыXML)));
...
Асинх Функция ПодтвердитьКМ(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ПустаяСтрока(ИдентификаторЗапроса) Тогда
		ОписаниеОшибки = НСтр("ru = 'Идентификатор запроса КМ не указан.';
								|en = 'MC request ID is not specified.'");
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "ConfirmKM", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Попытка
		...
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "ConfirmKM", ВМассив(, ИдентификаторЗапроса, ТипПодтверждения));
	...
	Если НЕ ПараметрыПодключения.ТипыОборудования.ККТ Или ПараметрыПодключения.РевизияИнтерфейса < 3004 Тогда
		ОписаниеОшибки = СтрШаблон(
			НСтр("ru = 'Драйвер %1 (версия %2) (рев.%3) не поддерживает операцию проверки кода маркировки. Для корректной работы требуется обновить драйвер.';
				|en = 'The %1 driver (version %2) (rev. %3) does not support the marking code check. Update the driver.'"),
			ПараметрыПодключения.Наименование,
...
Асинх Функция ФискализацияЧекаПакетом(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации);
	...
	Попытка
		...
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "ProcessCheck", ВМассив(, Ложь, ДанныеОперации.ДанныеЧекаXML));
...
Асинх Функция ПараметрыКарты(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если РевизияИнтерфейса > 3004 Тогда
		...
		Попытка                  
			Если РевизияИнтерфейса >= 4000 Тогда    
				...
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "GetCardParametrs", ВМассив(, РеквизитыКартыQR, ОтПоследнейОперации));
			...
			ИначеЕсли РевизияИнтерфейса >= 3007 Тогда
				...
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "GetCardParametrs", ВМассив(,ОтПоследнейОперации));
	...
	Иначе     
		ОписаниеОшибки = НСтр("ru = 'Терминал не поддерживает данную операцию.';
								|en = 'The terminal does not support this transaction.'");
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, "GetCardParametrs", ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки); 
	КонецЕсли;
...
Асинх Функция ОперацииПоКартам(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ПараметрыПодключения.РевизияИнтерфейса < 4000 Тогда    
		...
		ОписаниеОшибки = НСтр("ru = 'Команда не поддерживается драйвером.';
								|en = 'The command is not supported by the driver.'");
	...
	Попытка        
		Если РевизияИнтерфейса >= 4000 Тогда
			...
			Если Команда = "AuthorizeSales" Тогда
				...
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PayByPaymentCard", 
					ВМассив(,НомерМерчанта,РеквизитыКартыQR,СуммаОперации,НомерКарты,НомерЧека));
			...
			ИначеЕсли Команда = "AuthorizeRefund" Тогда
				...
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "ReturnPaymentByPaymentCard", 
					ВМассив(,НомерМерчанта,РеквизитыКартыQR,СуммаОперации,НомерКарты,НомерЧека,СсылочныйНомер,КодАвторизации));
			...
			ИначеЕсли Команда = "AuthorizeVoid" Тогда
				...
				Если СуммаОригинальнойОперации > 0 И НЕ ПараметрыПодключения.ЧастичнаяОтмена Тогда
					ОписаниеОшибки = НСтр("ru = 'Частичная отмена не поддерживается драйвером.';
											|en = 'The driver does not support partial cancellation.'");
			...
			ИначеЕсли Команда = "PayByPaymentCardWithCashWithdrawal" Тогда
				...
				Если ПараметрыПодключения.ВыдачиНаличныхДенежныхСредств Тогда
					ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PayByPaymentCardWithCashWithdrawal", 
						ВМассив(,НомерМерчанта,РеквизитыКартыQR,СуммаОперации,СуммаНаличных));
		...
		ИначеЕсли РевизияИнтерфейса >= 3005 Тогда
			...
			Если Команда = "AuthorizeSales" Тогда
				...
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PayByPaymentCard", ВМассив(, НомерМерчанта, СуммаОперации, НомерКарты, НомерЧека));
			...
			ИначеЕсли Команда = "AuthorizeRefund" Тогда
				...
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "ReturnPaymentByPaymentCard", 
					ВМассив(,НомерМерчанта,СуммаОперации,НомерКарты,НомерЧека,СсылочныйНомер,КодАвторизации));
			...
			ИначеЕсли РевизияИнтерфейса >= 3006 И Команда = "PayByPaymentCardWithCashWithdrawal" Тогда
				...
				Если ПараметрыПодключения.ВыдачиНаличныхДенежныхСредств Тогда
					ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PayByPaymentCardWithCashWithdrawal", 
						ВМассив(,НомерМерчанта,СуммаОперации,СуммаНаличных));
		...
		Иначе
			...
			Если Команда = "AuthorizeSales" Тогда
				...
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PayByPaymentCard", ВМассив(,НомерКарты, СуммаОперации, НомерЧека));
			...
			ИначеЕсли Команда = "AuthorizeRefund" Тогда
				...
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "ReturnPaymentByPaymentCard", 
					ВМассив(,НомерКарты,СуммаОперации,НомерЧека,СсылочныйНомер,КодАвторизации));
...
Асинх Функция ЭквайринговаяОперацияXML(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации, Команда)
	...
	Попытка        
		Если Команда = "AuthorizeSales" Тогда
			...
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "Pay", ВМассив(,ВходяшиеПараметрыXML));
		...
		ИначеЕсли Команда = "AuthorizeRefund" Тогда
			...
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "ReturnPayment", ВМассив(,ВходяшиеПараметрыXML));
		...
		ИначеЕсли Команда = "AuthorizeVoid" Тогда
			...
			Если СуммаОригинальнойОперации > 0 И НЕ ПараметрыПодключения.ЧастичнаяОтмена Тогда
				...
				Возврат РезультатВыполнения;
			Иначе
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "CancelPayment", ВМассив(,ВходяшиеПараметрыXML));
		...
		ИначеЕсли Команда = "PayByPaymentCardWithCashWithdrawal" Тогда
			...
			Если ПараметрыПодключения.ВыдачиНаличныхДенежныхСредств Тогда
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PayWithCashWithdrawal", ВМассив(,ВходяшиеПараметрыXML));
		...
		ИначеЕсли Команда = "PurchaseWithEnrollment" Тогда
			...
			Если ПараметрыПодключения.ПокупкаСЗачислениемНаКарту Тогда
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PurchaseWithEnrollment", ВМассив(,ВходяшиеПараметрыXML));
		...
		ИначеЕсли Команда = "AuthorizePreSales" Тогда
			...
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "Authorisation", ВМассив(,ВходяшиеПараметрыXML));
			РезультатВызова = Ждать ОбъектДрайвера.ПреавторизацияАсинх(ПараметрыПодключения.ИДУстройства, ВходяшиеПараметрыXML, ИсходящиеПараметрыXML, ТекстСлипЧека); // Структура
			ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
			
		ИначеЕсли Команда = "AuthorizeCompletion" Тогда 
			
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "AuthConfirmation", ВМассив(,ВходяшиеПараметрыXML));
			РезультатВызова = Ждать ОбъектДрайвера.ЗавершитьПреавторизациюАсинх(ПараметрыПодключения.ИДУстройства, ВходяшиеПараметрыXML, ИсходящиеПараметрыXML, ТекстСлипЧека); // Структура
			ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
			
		ИначеЕсли Команда = "AuthorizeVoidPreSales" Тогда
			
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "CancelAuthorisation", ВМассив(,ВходяшиеПараметрыXML));
			РезультатВызова = Ждать ОбъектДрайвера.ОтменитьПреавторизациюАсинх(ПараметрыПодключения.ИДУстройства, ВходяшиеПараметрыXML, ИсходящиеПараметрыXML, ТекстСлипЧека); // Структура
			ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
...
Асинх Функция ЭквайринговаяОперацияСЭлектроннымСертификатом(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации, Команда)
	...
	Попытка
		Если ПараметрыПодключения.РевизияИнтерфейса >= 4000 Тогда
			Если Команда = "PayElectronicCertificate" Тогда
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PayElectronicCertificate", 
					ВМассив(,НомерМерчанта, РеквизитыКартыQR, ИдентификаторКорзины, СуммаЭлектронногоСертификата, СуммаСобственныхСредств));
			...
			ИначеЕсли Команда = "ReturnElectronicCertificate" Тогда
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PayElectronicCertificate", 
					ВМассив(,НомерМерчанта, РеквизитыКартыQR, ИдентификаторКорзины, СуммаЭлектронногоСертификата, СуммаСобственныхСредств, НомерКарты, НомерЧека, СсылочныйНомер));
		...
		Иначе
			Если Команда = "PayElectronicCertificate" Тогда
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PayElectronicCertificate", 
					ВМассив(,НомерМерчанта, ИдентификаторКорзины, СуммаЭлектронногоСертификата, СуммаСобственныхСредств));
			...
			ИначеЕсли Команда = "ReturnElectronicCertificate" Тогда
				ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PayElectronicCertificate", 
					ВМассив(,НомерМерчанта, ИдентификаторКорзины, СуммаЭлектронногоСертификата, СуммаСобственныхСредств, НомерКарты, НомерЧека, СсылочныйНомер));
...
Асинх Функция ЭквайринговаяОперацияСЭлектроннымСертификатомXML(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации, Команда)
	...
	Если ПараметрыПодключения.РевизияИнтерфейса < 4003 Тогда      
		ОписаниеОшибки = НСтр("ru = 'Команда не поддерживается драйвером.';
								|en = 'Команда не поддерживается драйвером.'");
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки); 
		Возврат РезультатВыполнения;   
	КонецЕсли;
	...
	Если НЕ ПараметрыПодключения.ОплатаЭлектроннымиСертификатами Тогда
		ОписаниеОшибки = НСтр("ru = 'Терминал не поддерживает данную операцию.';
								|en = 'Терминал не поддерживает данную операцию.'"); 
		ЗаписатьОшибкуВЛог(ПараметрыПодключения, Команда, ОписаниеОшибки);
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки); 
		Возврат РезультатВыполнения;
	КонецЕсли;
	...
	Попытка        
		Если Команда = "PayElectronicCertificate" Тогда
			...
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PayElectronicCertificate", ВМассив(,ВходяшиеПараметрыXML));
			РезультатВызова = Ждать ОбъектДрайвера.ОплатитьСертификатомАсинх(ПараметрыПодключения.ИДУстройства, ВходяшиеПараметрыXML, ИсходящиеПараметрыXML, ТекстСлипЧека); // Структура
			ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
			
		ИначеЕсли Команда = "ReturnElectronicCertificate" Тогда
			
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "ReturnElectronicCertificate", ВМассив(,ВходяшиеПараметрыXML));
			РезультатВызова = Ждать ОбъектДрайвера.ВернутьСертификатомАсинх(ПараметрыПодключения.ИДУстройства, ВходяшиеПараметрыXML, ИсходящиеПараметрыXML, ТекстСлипЧека); // Структура
			ЗавершитьЗаписьЛогаОперации(ЗаписьЛога, РезультатВызова);
...
Асинх Функция ВывестиСтрокуНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	СтрокиТекста = ?(ДанныеОперации.Свойство("СтрокиТекста"), ДанныеОперации.СтрокиТекста, НСтр("ru = 'Данные для вывода не указаны';
																								|en = 'Output data is not specified'"));
	Попытка
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "StringOutputOnCustomerDisplay", ВМассив(,СтрокиТекста));
...
Асинх Функция ВывестиQRКодНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "QRCodeOutputOnCustomerDisplay", ВМассив(,ЗначениеQRКода, КартинкаQRКода));
...
Асинх Функция ВывестиHTMLНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "OutputHTML", ВМассив(,ТекстHTML));
...
Асинх Функция ВывестиСтатусНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ПараметрыПодключения.РевизияИнтерфейса < 3009  Тогда
		ОписаниеОшибки = НСтр("ru = 'Устройство не поддерживает данную операцию.';
								|en = 'The device does not support this transaction.'");
	...
	Попытка                                                                                    
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "DisplayOperationStatus", ВМассив(,СтатусОперации));
...
Асинх Функция ВывестиСтатусРабочегоМестаНаДисплейПокупателя(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "DisplayWorkplaceStatus", ВМассив(,СтатусРабочегоМеста, ЗвуковойСигнал));
...
Асинх Функция Тарировать(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)   
	...
	Если ПараметрыПодключения.РевизияИнтерфейса >= 4001 И НЕ ПараметрыПодключения.ВесыПоддерживаютТарирование Тогда
		ОписаниеОшибки = НСтр("ru = 'Оборудование не поддерживает данную операцию.';
								|en = 'The peripheral does not support this transaction.'"); 
	...
	Попытка
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "Calibrate", ВМассив(,ВесТары));
...
Асинх Функция ВыгрузитьXMLНаВесы(ОбъектДрайвера, ПараметрыПодключения, ДанныеДляВыгрузки, СтатусПакета)
	...
	Попытка
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "UploadGoods", ВМассив(, ДанныеДляВыгрузки, СтатусПакета));
...
Асинх Функция ВыгрузитьТовары(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Для Каждого ДанныеДляВыгрузки Из ДанныеОперации.ПакетыXML  Цикл
		...
		Состояние(НСтр("ru = 'Выгрузка данных...';
						|en = 'Exporting data…'"), Окр(ТекущийПроцент));
	КонецЦикла; 
...
Асинх Функция ВыгрузитьТоварыПакетом(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Если ПустаяСтрока(ДанныеДляВыгрузки) Тогда
		ОписаниеОшибки = НСтр("ru = 'XML пакет для выгрузки пустой.';
								|en = 'The XML package for export is empty.'");
		РезультатВыполнения = РезультатОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		Возврат РезультатВыполнения;
	КонецЕсли;
	СтатусПакета = "last";                                                                                
...
Асинх Функция ВыгрузитьИзображенияТоваров(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Для Каждого ДанныеДляВыгрузки Из ДанныеОперации.ПакетыXML  Цикл
		...
		Попытка
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "UploadProductImages", ВМассив(, ДанныеДляВыгрузки, СтатусПакета));
...
Асинх Функция РаспознатьАсинх(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "Recognize", ВМассив(,Обучение, Варианты));
...
Асинх Функция ПодтвердитьРаспознаваниеАсинх(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)   
	...
	Попытка
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "ConfirmRecognize", ВМассив(,Код));
...
Асинх Функция ВыгрузитьТаблицу(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Для Каждого ДанныеДляВыгрузки Из ДанныеОперации.ПакетыXML  Цикл
		...
		Попытка
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "UploadTable", ВМассив(, ДанныеДляВыгрузки, СтатусПакета));
...
Асинх Функция ПечатьЭтикеток(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Для Счетчик = 0 По КоличествоПакетов Цикл
		...
		Попытка
			ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "PrintLabels", ВМассив(, ДанныеДляВыгрузки, СтатусПакета));
...
Функция ПолучитьДанныеМетокRFID(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации) 
	...
	Попытка       
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "GetDataTagsRFID", ВМассив(, ИДПакета, ТаблицаМеток));
...
Асинх Функция ЗаписатьДанныеВМеткуRFID(ОбъектДрайвера, ПараметрыПодключения, ДанныеОперации)
	...
	Попытка       
		ЗаписьЛога = НачатьЗаписьЛогаОперации(ПараметрыПодключения, "SaveDataTagRFID", ВМассив(, TID, EPC, Данные, БанкПамяти, Таймаут));
