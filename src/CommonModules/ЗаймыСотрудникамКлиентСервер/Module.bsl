...
Функция ГрафикПогашенияЗайма(ПараметрыПостроения) Экспорт
	...
	ИзмененияУсловий = ПараметрыПостроения.ИзмененияУсловий;
	...
	Пока ТекущийМесяц <= МесяцОкончания Цикл
		...
		КоличествоДнейМесяца = ЗарплатаКадрыКлиентСервер.КоличествоДнейМесяца(ТекущийМесяц);
				
		Если ТекущийМесяц = МесяцНачала И (ИзмененияУсловий.Количество() > 0) Тогда
			
			НачислениеПроцентов     = 0;
			НачисленоОсновногоДолга = 0;
			
			// В первом месяце могло произойти несколько изменений условий займа.
			// Рассчитаем погашение первого месяца с учётом текущих параметров графика.
			ПоследняяСтрокаИзменениеУсловий = ИзмененияУсловий[ИзмененияУсловий.ВГраница()];
			ПоследняяСтрокаИзменениеУсловий.ДатаИзменений		= ПараметрыПостроения.ДатаИзменений;
			ПоследняяСтрокаИзменениеУсловий.РазмерПогашения		= ПараметрыПостроения.РазмерПогашения;
			ПоследняяСтрокаИзменениеУсловий.РазмерПлатежа		= ПараметрыПостроения.РазмерПлатежа;
			ПоследняяСтрокаИзменениеУсловий.ПроцентнаяСтавка	= ПараметрыПостроения.ГодоваяСтавка;
			
			ВГраница = ИзмененияУсловий.ВГраница();
			Для Индекс = 0 ПО ВГраница-1 Цикл
				ТекущаяСтрока = ИзмененияУсловий[Индекс];
				ТекущаяСтрока.КоличествоДней = День(ИзмененияУсловий[Индекс+1].ДатаИзменений) - День(ТекущаяСтрока.ДатаИзменений);
			КонецЦикла;
			ИзмененияУсловий[Индекс].КоличествоДней = День(КонецМесяца(ПараметрыПостроения.ДатаИзменений)) + 1 
													- День(ПараметрыПостроения.ДатаИзменений);
			ЭтоПервыйПроход = Истина;
			Для Каждого Изменение ИЗ ИзмененияУсловий Цикл
				// Рассчитываем проценты за месяц.
				ДнейВМесяцеПроценты		= Изменение.КоличествоДней;
				ДнейВМесяцеОсновнойДолг	= Изменение.КоличествоДней;
				
				// Займ выдан не с начала месяца и изменён в тот же месяц. В этом случае проценты начисляются на следующий день
				Если ЭтоПервыйПроход И День(Изменение.ДатаИзменений) <> 1 Тогда
					ДнейВМесяцеПроценты = ДнейВМесяцеПроценты - 1;
				КонецЕсли;
				ЭтоПервыйПроход = Ложь;
				
				ТекущиеПроценты		= Макс(ОстатокДолга, 0) * Изменение.ПроцентнаяСтавка * 0.01 * ДнейВМесяцеПроценты / ДнейВГоду;
				НачислениеПроцентов	= НачислениеПроцентов + ТекущиеПроценты; 
				
				Если ВидыПлатежейРазмерПогашения.Найти(ВидПлатежей) <> Неопределено Тогда
					НачисленоОсновногоДолга	= НачисленоОсновногоДолга + Изменение.РазмерПогашения
					                                                  * (ДнейВМесяцеОсновнойДолг) / КоличествоДнейМесяца;
				Иначе
					НачисленоОсновногоДолга	= НачисленоОсновногоДолга + Изменение.РазмерПлатежа 
					                                                  * (ДнейВМесяцеОсновнойДолг) / КоличествоДнейМесяца - ТекущиеПроценты;
				КонецЕсли;
			КонецЦикла;
		Иначе
			// Рассчитываем проценты за месяц.
			...
			ИначеЕсли ТекущийМесяц = ДатаПредоставления Тогда
				// Это изменение займа, проценты считаем в тот же день
				ДнейВМесяцеПроценты = День(КонецМесяца(ТекущийМесяц)) - День(ДатаПредоставления);
			...
			Если ВидыПлатежейРазмерПогашения.Найти(ВидПлатежей) <> Неопределено Тогда
				НачисленоОсновногоДолга	= РазмерПогашения * (ДнейВМесяцеОсновнойДолг) / КоличествоДнейМесяца;
			Иначе
				НачисленоОсновногоДолга	= РазмерПлатежа * (ДнейВМесяцеОсновнойДолг) / КоличествоДнейМесяца - НачислениеПроцентов;
			КонецЕсли;
		КонецЕсли;
		...
		Если ПараметрыПостроения.СпособПогашения = ПредопределенноеЗначение("Перечисление.СпособыПогашенияЗаймаСотруднику.ВТечениеСрока") Тогда
			...
			Если ЗначениеЗаполнено(ПараметрыПостроения.МесяцНачалаПогашения) Тогда
				...
				Иначе
					...
					Если ВидПлатежей <> ПредопределенноеЗначение("Перечисление.ВидыПлатежейПогашенияЗаймаСотруднику.ТолькоЗаем") Тогда
						ПогашениеПроцентов = НачислениеПроцентов + НакопленоПроцентов;
					КонецЕсли;
					Если ВидПлатежей <> ПредопределенноеЗначение("Перечисление.ВидыПлатежейПогашенияЗаймаСотруднику.ТолькоПроценты")
						И ВидПлатежей <> ПредопределенноеЗначение("Перечисление.ВидыПлатежейПогашенияЗаймаСотруднику.ТолькоЗаем") Тогда
		...
		ПогашениеДолга     = Окр(ПогашениеДолга,2);
		ПогашениеПроцентов = Окр(ПогашениеПроцентов,2);
...
Функция РазмерАннуитетногоПлатежа(ДанныеЗайма) Экспорт
	...
	// Если займ выдан только на месяц и не с начала месяца, то сразу рассчитаем размер аннуитетного платежа
	Если День(ДанныеЗайма.ДатаВыдачи) <> 1 И
		ДанныеЗайма.МесяцПредоставления = НачалоМесяца(ДанныеЗайма.МесяцОкончания) Тогда
		
		ДнейВГоду = КоличествоДнейВГоду(Год(ДанныеЗайма.МесяцПредоставления));
		// Количество дней определяем от даты фактической выдачи займа (т.к. проценты начисляются на следующий день).
		ДнейВМесяцеПроценты = День(КонецМесяца(ДанныеЗайма.ДатаВыдачи)) - День(ДанныеЗайма.ДатаВыдачи);
		
		НачислениеПроцентов  = ДанныеЗайма.СуммаЗайма * ДанныеЗайма.ГодоваяСтавка * 0.01 * ДнейВМесяцеПроценты / ДнейВГоду;
		ТекущийРазмерПлатежа = ДанныеЗайма.СуммаЗайма + НачислениеПроцентов;
...
Функция СальдоВзаиморасчетовПоЗавершению(РазмерПлатежа, МесяцОкончания, ДанныеЗайма, КорректировкаСрока = Неопределено)
	...
	ПараметрыГрафика.ДатаПредоставления = ДанныеЗайма.ДатаВыдачи;
	...
	ПараметрыГрафика.ДатаИзменений = ДанныеЗайма.ДатаВыдачи;
	ПараметрыГрафика.ИзмененияУсловий = ДанныеЗайма.ИзмененияУсловий;
...
Функция ПараметрыПостроенияГрафикаПогашенияЗайма() Экспорт
	...
	//	- ДатаПредоставления
	//	- ДатаИзменений - дата, с которой вносится изменение в условия договора займа
	...
	//	- ИзмененияУсловий - (необязательный) коллекция истории изменений условий займа за месяц.
	...
	СтруктураПараметров.ДатаИзменений = Дата(1, 1, 1);
	...
	СтруктураПараметров.ИзмененияУсловий = Новый Массив;
...
Процедура УстановитьЭлементыВводаОтсрочки(Форма, ДатаНачалаПогашенияПоУмолчанию) Экспорт
	...
	Иначе
		Если Не ЗначениеЗаполнено(Форма.Объект.ДатаНачалаПогашения) Тогда
			Форма.Объект.ДатаНачалаПогашения = НачалоМесяца(ДатаНачалаПогашенияПоУмолчанию);
		КонецЕсли;
	КонецЕсли;
	...
	Если ВыдачаТраншами Тогда 
		...
		Иначе
			...
			Форма.Элементы.ЗаемПоДоговоруВыданПолностью.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Транш за %1 выдан в полном объеме';
						|en = 'Tranche for %1 is issued fully'"), НРег(Формат(Форма.Объект.ДатаПредоставления, "ДФ=""ММММ гггг""")));
		КонецЕсли;
	Иначе
		Форма.Элементы.ВыдачаЗаймаДокументомДоговораСтраницы.ТекущаяСтраница = Форма.Элементы.ЗаемВыданПолностьюСтраница;
		Форма.Элементы.ЗаемПоДоговоруВыданПолностью.Заголовок = НСтр("ru = 'Заем по договору выдан полностью';
																	|en = 'Contract loan is fully granted'");
	КонецЕсли;
...
Процедура ЗаполнитьПредставлениеТранша(Форма, СтрокаТранша) Экспорт
	...
	СтрокаТранша.ПредставлениеТранша = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1 %2 (%3)';
			|en = '%1 %2 (%3)'"), СтрокаТранша.Сумма, Форма.Валюта, СтрокаТранша.МесяцПредоставленияСтрокой);
	
КонецПроцедуры

Процедура ЗаполнитьРазмерПогашения(РазмерПогашения, Срок, Сумма, ДанныеЗайма, ИзмененияУсловий = Неопределено) Экспорт
	...
	Если Срок = 0 
		Или ВидыПлатежей.Найти(ДанныеЗайма.ВидПлатежей) = Неопределено Тогда
		РазмерПогашения = 0;
		Возврат;
	КонецЕсли;
	
	Если ДанныеЗайма.МесяцНачалаПогашения <> '00010101' Тогда
		// Если предоставлена отсрочка следует пересчитать срок и не учитывать дату выдачи
		СрокСОтсрочкой = СрокПоДатеОкончания(ДанныеЗайма.МесяцОкончания,ДанныеЗайма.МесяцНачалаПогашения);
		Если СрокСОтсрочкой = 0 Тогда
			РазмерПогашения = 0;
			Возврат;
		КонецЕсли;
		РазмерПогашения = ?(ДанныеЗайма.ОграничениеПлатежа = 0, 
							Сумма / СрокСОтсрочкой, 
							Мин(Сумма / СрокСОтсрочкой, ДанныеЗайма.ОграничениеПлатежа));
	ИначеЕсли День(ДанныеЗайма.ДатаВыдачи) = 1 
		Или Срок = 1 Тогда
		РазмерПогашения = ?(ДанныеЗайма.ОграничениеПлатежа = 0, Сумма / Срок, Мин(Сумма / Срок, ДанныеЗайма.ОграничениеПлатежа));
	Иначе
		КоличествоДнейМесяца = ЗарплатаКадрыКлиентСервер.КоличествоДнейМесяца(ДанныеЗайма.ДатаВыдачи);
		Если ИзмененияУсловий = Неопределено Или ИзмененияУсловий.Количество() = 0 Тогда
			ДнейВМесяцеОсновнойДолг     = День(КонецМесяца(ДанныеЗайма.ДатаВыдачи)) - День(ДанныеЗайма.ДатаВыдачи) + 1;
			ДоляДнейВМесяцеОсновнойДолг = ДнейВМесяцеОсновнойДолг / КоличествоДнейМесяца;
			РазмерПогашения = Сумма / (Срок - 1 + ДоляДнейВМесяцеОсновнойДолг);
		Иначе
			НачисленоОсновногоДолга = 0;
			...
			// Рассчитаем погашение первого месяца с учётом текущих параметров графика.
			ПоследняяСтрокаИзменениеУсловий = ИзмененияУсловий[ИзмененияУсловий.ВГраница()];
			ПоследняяСтрокаИзменениеУсловий.ДатаИзменений = ДанныеЗайма.ДатаВыдачи;
			
			ВГраница = ИзмененияУсловий.ВГраница();
			Для Индекс = 0 ПО ВГраница-1 Цикл
				ТекущаяСтрока = ИзмененияУсловий[Индекс];
				ТекущаяСтрока.КоличествоДней = День(ИзмененияУсловий[Индекс+1].ДатаИзменений) - День(ТекущаяСтрока.ДатаИзменений);
				НачисленоОсновногоДолга	= НачисленоОсновногоДолга + ТекущаяСтрока.РазмерПогашения
																  * (ТекущаяСтрока.КоличествоДней) / КоличествоДнейМесяца;
			КонецЦикла;
			
			ДнейВМесяцеОсновнойДолг     = День(КонецМесяца(ДанныеЗайма.ДатаВыдачи)) - День(ДанныеЗайма.ДатаВыдачи) + 1;
			ДоляДнейВМесяцеОсновнойДолг = ДнейВМесяцеОсновнойДолг / КоличествоДнейМесяца;
			РазмерПогашения = Макс(0,Сумма - НачисленоОсновногоДолга) / (Срок - 1 + ДоляДнейВМесяцеОсновнойДолг);
...
// Конструирует структуру для описания строки массива изменения ставок
//
Функция ОписаниеСтрокиМассиваИзмененияУсловий() Экспорт
	
	ПоляОписания = Новый Соответствие;
	ПоляОписания.Вставить("ДатаИзменений",		Новый ОписаниеТипов("Дата"));
	ПоляОписания.Вставить("ПроцентнаяСтавка",	Новый ОписаниеТипов("Число"));
	ПоляОписания.Вставить("РазмерПлатежа",		Новый ОписаниеТипов("Число"));
	ПоляОписания.Вставить("РазмерПогашения",	Новый ОписаниеТипов("Число"));
	ПоляОписания.Вставить("КоличествоДней",		Новый ОписаниеТипов("Число"));
