...
// см. ИнтеграцияИСМПКлиентСерверПовтИсп.ВидыПродукцииТестовогоПериода
...
// Используемые виды маркируемой продукции ИСМП.
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.ВидыПродукцииИС - используемые виды маркируемой продукции ИСМП
...
Процедура ВыполнитьОбменРегламентноеЗадание() Экспорт
	...
	ОбменДаннымиИСМПВызовСервера.ВыполнитьОбмен();
...
// Установить условное оформление полей количества.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
//  ГрупповыеУпаковкиОтдельно - Булево - Групповые упаковки отдельно
...
// Настроить видимость колонки количества кодов маркировки.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
...
// Запрещено использование объекта ВИСМП.
// 
// Параметры:
//  Объект - ДанныеФормыСтруктура
// 
// Возвращаемое значение:
//  Булево - Запрещено использование объекта ВИСМП
	...
	Если Не ОбщегоНазначенияИСМП.ИспользуетсяМаркируемаяПродукция()
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Для работы с объектом необходимо включить ведение учета маркируемой продукции ИС МП';
				|en = 'Для работы с объектом необходимо включить ведение учета маркируемой продукции ИС МП'"));
		Возврат Истина;
	КонецЕсли;
...
// 
// Параметры:
//  ДанныеВскрытия - см. ДанныеВскрытияПотребительскойУпаковки
// 
// Возвращаемое значение:
...
Процедура ОбработатьОжиданиеПриОтправкеДанныхВСервис(ПараметрыОтправкиHTTPЗапросов) Экспорт
	...
	Если ПараметрыОтправкиHTTPЗапросов.Свойство("ДатаПоследнегоЗапроса") Тогда
		ВремяОжиданияСледующегоЗапроса = ИнтеграцияИСМПСлужебный.ВремяОжиданияСледующегоЗапросаСУЗ(ПараметрыОтправкиHTTPЗапросов);
...
// Доступна работа с кодами маркировки пива для внутреннего перемещения ИС МП.
// 
// Параметры:
//  Отказ - Булево - Отказ от открытия форм перемещения между МОД
// 
// Возвращаемое значение:
//  Булево - Доступна работа с кодами маркировки ИСМП (пива, слабоалкогольной продукции)
//
Функция ДоступнаРаботаСКодамиМаркировкиПивнойСлабоалкогольнойПродукции(Отказ = Неопределено) Экспорт
	
	УчитываемыеВидыПродукции                    = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции();
	ВидыПродукцииВнутреннегоПеремещенияМеждуМОД = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ВидыПродукцииВнутреннегоПеремещенияМеждуМОД();
	
	ВключенУчетВидовПродукцииПеремещенияМеждуМОД = Ложь;
	
	Для Каждого ВидПродукции Из ВидыПродукцииВнутреннегоПеремещенияМеждуМОД Цикл
		Если УчитываемыеВидыПродукции.Найти(ВидПродукции) <> Неопределено Тогда
			ВключенУчетВидовПродукцииПеремещенияМеждуМОД = Истина;
		КонецЕсли;
	КонецЦикла;
	
	Если ВключенУчетВидовПродукцииПеремещенияМеждуМОД Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если Не (Отказ = Неопределено) Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Для работы с документом перемещения между МОД
				 |требуется включить использование хотя бы одного вида алкогольной продукции
				 |в панели администрирования ИС МП';
				 |en = 'Для работы с документом перемещения между МОД
				 |требуется включить использование хотя бы одного вида алкогольной продукции
				 |в панели администрирования ИС МП'"),,,,
			Отказ);
		Возврат Ложь;
...
Процедура ПроверкаЗаполненияКоличестваПотребительскихУпаковок(ДокументОбъект, Отказ) Экспорт
	...
	ШаблонСообщения = НСтр("ru = 'Не заполнена колонка ""Коды маркировки"" в строке %1 таблицы ""Товары""';
							|en = 'Не заполнена колонка ""Коды маркировки"" в строке %1 таблицы ""Товары""'");
...
// Дополняет статусы ИСМП, цвет текста которых "требует внимания"
// 
// Параметры:
//  СтатусыЦветТекстаТребуетВнимания - Массив из ПеречислениеСсылка - статусы цвет текста требует внимания
// 
Процедура ДополнитьСтатусыЦветТекстаТребуетВнимания(СтатусыЦветТекстаТребуетВнимания) Экспорт
	...
	Для Каждого Статус Из Документы.ОтчетИСМП.СтатусыОшибок() Цикл
		СтатусыЦветТекстаТребуетВнимания.Добавить(Статус);
	КонецЦикла;
	Для Каждого Статус Из Документы.ПеремещениеМеждуМОДИСМП.СтатусыОшибок() Цикл
		СтатусыЦветТекстаТребуетВнимания.Добавить(Статус);
	КонецЦикла;
	Для Каждого Статус Из Документы.РазрешениеНаОтгрузкуИСМП.СтатусыОшибок() Цикл
		СтатусыЦветТекстаТребуетВнимания.Добавить(Статус);
	КонецЦикла;
...
Функция ОписаниеОформленныхДокументов(МетаданныеДокумента, ДокументОснование, ДокументыПоОснованию, СтатусыОформления) Экспорт
	...
	ЦветНадписи                    = Неопределено;
	ПредставлениеСтатусаОформления = СокрЛП(СтатусОформления);
	...
	Если МетаданныеДокумента = Метаданные.Документы.ЗаказНаЭмиссиюКодовМаркировкиСУЗ Тогда
		...
		Если АрхивныйСтатус Тогда
			ТекстНадписи = НСтр("ru = 'Оформление заказов на эмиссию кодов маркировки завершено';
								|en = 'Оформление заказов на эмиссию кодов маркировки завершено'");
		...
		ИначеЕсли (МассивДокументов.Количество()
			...
			Если СостояниеЗаказаИтоги.КоличествоПечатать > 0
				...
				Если СостояниеЗаказаИтоги.КоличествоЗаказать > 0 Тогда
					ЭлементыТекстаТребуется.Добавить(СтрШаблон(НСтр("ru = 'заказать: %1';
																	|en = 'заказать: %1'"), СостояниеЗаказаИтоги.КоличествоЗаказать));
				КонецЕсли;
				Если СостояниеЗаказаИтоги.КоличествоРезервировать > 0 Тогда
					ЭлементыТекстаТребуется.Добавить(СтрШаблон(НСтр("ru = 'зарезервировать: %1';
																	|en = 'зарезервировать: %1'"), СостояниеЗаказаИтоги.КоличествоРезервировать));
				КонецЕсли;
				Если СостояниеЗаказаИтоги.КоличествоПечатать > 0 Тогда
					ЭлементыТекстаТребуется.Добавить(СтрШаблон(НСтр("ru = 'распечатать: %1';
																	|en = 'распечатать: %1'"), СостояниеЗаказаИтоги.КоличествоПечатать));
				КонецЕсли;
				ЭлементыТекста.Добавить(СтрШаблон(НСтр("ru = 'Требуется %1 кодов маркировки';
														|en = 'Требуется %1 кодов маркировки'"), СтрСоединить(ЭлементыТекстаТребуется, ", ")));
			КонецЕсли;
			
			Если СостояниеЗаказаИтоги.КоличествоЗаказано > 0 Тогда
				ЭлементыТекста.Добавить(
					НСтр("ru = 'Ожидается получение кодов маркировки';
						|en = 'Ожидается получение кодов маркировки'"));
			КонецЕсли;
		...
		ИначеЕсли (МассивДокументов.Количество()
			...
			ИмяКоманды = ШаблонПредставленияДокумента.ИмяКомандыПулКодовМаркировки;
			ТекстНадписи = СтрШаблон(НСтр("ru = 'Распечатано кодов маркировки: %1';
											|en = 'Распечатано кодов маркировки: %1'"), СостояниеЗаказаИтоги.КоличествоРаспечатано)
	...
	ИначеЕсли МассивДокументов.Количество() > 0 Тогда
		...
		Если МассивДокументов.Количество() = 1 Тогда
			...
			Если АрхивныйСтатус Тогда
				ИмяКоманды = ШаблонПредставленияДокумента.ИмяКомандыОткрытьПротоколОбмена;
			ИначеЕсли СтатусОформления <> Перечисления.СтатусыОформленияДокументовГосИС.Оформлено
				И МетаданныеДокумента <> Метаданные.Документы.РазрешениеНаОтгрузкуИСМП Тогда
				ИмяКоманды = ШаблонПредставленияДокумента.ИмяКомандыОткрытьПротоколОбмена;
			Иначе
			...
			Если МетаданныеДокумента = Метаданные.Документы.РазрешениеНаОтгрузкуИСМП
				И (СтатусОформления <> Перечисления.СтатусыОформленияДокументовГосИС.Оформлено
					Или Документы.РазрешениеНаОтгрузкуИСМП.КонечныеСтатусы(Ложь).Найти(Статус) = Неопределено) Тогда
				ЦветГиперссылки = ЦветаСтиля.ЦветТекстаТребуетВниманияГосИС;
				ЦветНадписи     = ЦветаСтиля.ЦветТекстаТребуетВниманияГосИС;
				Если СтатусОформления = Перечисления.СтатусыОформленияДокументовГосИС.ЕстьОшибкиОформления Тогда
					ПредставлениеСтатусаОформления = НСтр("ru = 'Отмените разрешение на отгрузку';
															|en = 'Отмените разрешение на отгрузку'");
				ИначеЕсли СтатусОформления = Перечисления.СтатусыОформленияДокументовГосИС.НеОформлено Тогда
					ТекстНадписи = ШаблонПредставленияДокумента.ПредставлениеКомандыСоздать;
					ИмяКоманды   = ШаблонПредставленияДокумента.ИмяКомандыСоздать;
				КонецЕсли;
			КонецЕсли;
		...
		Иначе
			...
			Если МетаданныеДокумента = Метаданные.Документы.РазрешениеНаОтгрузкуИСМП Тогда
				
				ЕстьКонечныйСтатус = Ложь;
				КоличествоАктивных = 0;
				КонечныеСтатусы    = Документы.РазрешениеНаОтгрузкуИСМП.КонечныеСтатусы(Ложь);
				СтатусАктивного    = Неопределено;
				Для Каждого ДанныеСтатуса Из МассивДокументов Цикл
					Если (СтатусОформления = Перечисления.СтатусыОформленияДокументовГосИС.ЕстьОшибкиОформления
						Или СтатусОформления = Перечисления.СтатусыОформленияДокументовГосИС.ОформленоЧастично
						Или СтатусОформления = Перечисления.СтатусыОформленияДокументовГосИС.Оформлено)
						И КонечныеСтатусы.Найти(ДанныеСтатуса.Статус) <> Неопределено Тогда
						ЕстьКонечныйСтатус = Истина;
					КонецЕсли;
					Если ДанныеСтатуса.Статус <> Перечисления.СтатусыОбработкиРазрешенийНаОтгрузкуИСМП.РазрешениеОтменено Тогда
						КоличествоАктивных = КоличествоАктивных + 1;
						СтатусАктивного    = ДанныеСтатуса.Статус;
					КонецЕсли;
				КонецЦикла;
				...
				Если КоличествоАктивных = 1 Тогда
					...
					ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ШаблонПредставленияДокумента.ПредставлениеДокумента,
						СтатусАктивного);
					НуженРазделительВТексте = ЗначениеЗаполнено(СтатусАктивного);
				ИначеЕсли КоличествоАктивных = 0 Тогда
					ТекстНадписи = ШаблонПредставленияДокумента.ПредставлениеКомандыСоздать;
					ИмяКоманды   = ШаблонПредставленияДокумента.ИмяКомандыСоздать;
				КонецЕсли;
				
				Если Не ЕстьКонечныйСтатус
					Или СтатусОформления = Перечисления.СтатусыОформленияДокументовГосИС.ЕстьОшибкиОформления Тогда
					ЦветГиперссылки = ЦветаСтиля.ЦветТекстаТребуетВниманияГосИС;
					ЦветНадписи     = ЦветаСтиля.ЦветТекстаТребуетВниманияГосИС;
				КонецЕсли;
		...
		Если СтатусОформления <> Перечисления.СтатусыОформленияДокументовГосИС.Оформлено
			...
			Если Не (МетаданныеДокумента = Метаданные.Документы.РазрешениеНаОтгрузкуИСМП
				И ИмяКоманды = ШаблонПредставленияДокумента.ИмяКомандыСоздать) Тогда
				ТекстНадписи = ТекстНадписи + ?(НуженРазделительВТексте, ", ", "") + ПредставлениеСтатусаОформления
			КонецЕсли;
	...
	Иначе
		...
		Если Не АрхивныйСтатус
			И ИнтеграцияИС.ДокументПоддерживаетКорректировку(МетаданныеДокумента)
		...
		ИначеЕсли МетаданныеДокумента = Метаданные.Документы.РазрешениеНаОтгрузкуИСМП Тогда
			ЦветГиперссылки = ЦветаСтиля.ЦветТекстаТребуетВниманияГосИС;
			ЦветНадписи     = ЦветаСтиля.ЦветТекстаТребуетВниманияГосИС;
		КонецЕсли;
	...
	ФорматированныйТекстНадписи = Новый ФорматированнаяСтрока(
		ТекстНадписи,
		,
		?(ЗначениеЗаполнено(ИмяКоманды), ЦветГиперссылки, ЦветНадписи),
		,
		ИмяКоманды);
...
Функция ШаблонПредставленияДокументаДляПоляИнтеграции(МетаданныеДокумента, ДокументОснование) Экспорт
	...
	Шаблон.Вставить("ПредставлениеКомандыСоздать",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Создать %1';
																	|en = 'Создать %1'"), ФормыИмениДокумента.Склоненная));
	Шаблон.Вставить("ПредставлениеКомандыОткрыть",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Открыть %1';
																	|en = 'Открыть %1'"), ФормыИмениДокумента.Склоненная));
	...
	Шаблон.Вставить("ДокументНеОформлен",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 не %2';
			...
			?(ФормыИмениДокумента.РодИмени = "М", НСтр("ru = 'оформлен';
														|en = 'оформлен'"),
				?(ФормыИмениДокумента.РодИмени = "Ж", НСтр("ru = 'оформлена';
															|en = 'оформлена'"), НСтр("ru = 'оформлено';
																					|en = 'оформлено'")))));
	Шаблон.Вставить("ДокументНеСоздан",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 не %2';
			...
			?(ФормыИмениДокумента.РодИмени = "М", НСтр("ru = 'создан';
														|en = 'создан'"),
				?(ФормыИмениДокумента.РодИмени = "Ж", НСтр("ru = 'создана';
															|en = 'создана'"), НСтр("ru = 'создано';
																					|en = 'создано'")))));
	Шаблон.Вставить("ДокументНеПолучен",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 не %2';
			...
			?(ФормыИмениДокумента.РодИмени = "М", НСтр("ru = 'получен';
														|en = 'получен'"),
				?(ФормыИмениДокумента.РодИмени = "Ж", НСтр("ru = 'получена';
															|en = 'получена'"), НСтр("ru = 'получено';
																					|en = 'получено'")))));
				
	// корректировочный документ
	Если ИнтеграцияИС.ДокументПоддерживаетКорректировку(МетаданныеДокумента) Тогда
		
		ПредставлениеДокумента = ИнтеграцияИС.ПредставлениеДокументаПоддерживающегоКорректировку(МетаданныеДокумента);
...
//
// Параметры:
//  Организации - Массив из ОпределяемыйТип.Организация, ОпределяемыйТип.Организация - Организации для обмена.
//  ДатыПоследнегоЗапуска - Соответствие из КлючИЗначение - Даты последнего запуска обменов на клиенте по расписанию.
// 
// Возвращаемое значение:
//  Структура - Структура со свойствами:
//   * ОбменНаСервере - Соответствие из КлючИЗначение - Настройки обмена на сервере, см. функцию ИнтерфейсСУЗСлужебный.НоваяНастройкаОбменаСУЗ()
//   * ОбменНаКлиенте - Соответствие из КлючИЗначение - Настройки обмена на клиенте, см. функцию ИнтерфейсСУЗСлужебный.НоваяНастройкаОбменаСУЗ()
...
Функция ДанныеДляУточненияКоэффициентовУпаковок(ВходящиеДанные, ИспользоватьХарактеристики = Истина, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Для Каждого GTIN Из ПроверяемыеGTIN Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаДанных, ДанныеПоGTIN);
...
//
//Параметры:
//   КоллекцияИНН - Массив из Строка - список ИНН контрагентов
//
//Возвращаемое значение:
//   Соответствие из КлючИЗначение - ссылки на элементы справочника контрагентов:
...
//
//Параметры:
//   КоллекцияНалоговыхНомеров - Массив из Строка - список налоговых номеров контрагентов
//
//Возвращаемое значение:
//   Соответствие из КлючИЗначение - ссылки на элементы справочника контрагентов:
...
// Процедура дополняет список представлений документов по подсистеме ИС МП.
// 
// Параметры:
//  Представления - Соответствие из КлючИЗначение:
//   * Ключ - ОбъектМетаданных - Метаданные документа
//   * Значение - Структура:
//   	** Основной - Строка - представление для обычного документа
//   	** Корректировочный - Строка - представление для корректировочного документа.
Процедура ДополнитьПредставленияДокументовПоддерживающихКорректировку(Представления) Экспорт
	
	// Перемещение между МОД ИСМП
	МетаданныеДокументаПеремещениеМеждуМОДИСМП = Метаданные.Документы.ПеремещениеМеждуМОДИСМП;
	СтруктураПредставлений = Новый Структура("Основной, Корректировочный", "", "");
	
	СтруктураПредставлений.Основной         = МетаданныеДокументаПеремещениеМеждуМОДИСМП.Синоним;
	СтруктураПредставлений.Корректировочный = НСтр("ru = 'Обратное перемещение между МОД ИС МП';
													|en = 'Обратное перемещение между МОД ИС МП'");
	
	Представления.Вставить(МетаданныеДокументаПеремещениеМеждуМОДИСМП, СтруктураПредставлений);
	
КонецПроцедуры

// Дополнить список документов, поддерживающих корректировку.
// 
// Параметры:
//  Документы - Массив из ОбъектМетаданных
Процедура ДополнитьСписокДокументовПоддерживающихКорректировку(Документы) Экспорт
	
	Документы.Добавить(Метаданные.Документы.ПеремещениеМеждуМОДИСМП);
...
Процедура ЗаполнитьДоступныеВидыДокументовСертификации(Элемент, ВидПродукции) Экспорт
	...
	Если ВидПродукции <> Перечисления.ВидыПродукцииИС.БАДы
		И ВидПродукции <> Перечисления.ВидыПродукцииИС.БАДы2025
		...
		Если ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхПодконтрольныеВЕТИС
			Или ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхВлажныеПодконтрольныеВЕТИС Тогда
			...
			КонецЕсли;
		ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхБезВЕТИС
		...
		ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.ИгрыИИгрушкиДляДетей Тогда
			Если ИгрыИИгрушкиДляДетейДоступенСертификатСоответствия() Тогда
				Элемент.СписокВыбора.Добавить(Перечисления.ВидыДокументовОбязательнойСертификацииИС.СертификатСоответствия);
			КонецЕсли;
		ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.РадиоэлектроннаяПродукция
			Или ВидПродукции = Перечисления.ВидыПродукцииИС.ТелефоныИНоутбуки
			Или ВидПродукции = Перечисления.ВидыПродукцииИС.ПечатныеПлаты Тогда
		...
		ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.ПолимерныеТрубы Тогда
			Если ПолимерныеТрубыДоступенСертификатСоответствия() Тогда
				Элемент.СписокВыбора.Добавить(Перечисления.ВидыДокументовОбязательнойСертификацииИС.СертификатСоответствия);
			КонецЕсли;
		ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.СладостиИКондитерскиеИзделия Тогда
			Если СладостиИКондитерскиеИзделияДоступенСертификатСоответствия() Тогда
				Элемент.СписокВыбора.Добавить(Перечисления.ВидыДокументовОбязательнойСертификацииИС.СертификатСоответствия);
			КонецЕсли;
		Иначе
			Элемент.СписокВыбора.Добавить(Перечисления.ВидыДокументовОбязательнойСертификацииИС.СертификатСоответствия);
	...
	Если ВидПродукции <> Перечисления.ВидыПродукцииИС.БАДы
		И ВидПродукции <> Перечисления.ВидыПродукцииИС.БАДы2025 Тогда
		Если ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхПодконтрольныеВЕТИС
			Или ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхВлажныеПодконтрольныеВЕТИС Тогда
		...
		ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.МоторныеМасла Тогда
			...
			КонецЕсли;
		ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.ПолимерныеТрубы Тогда
			Если ПолимерныеТрубыДоступнаДекларацияСоответствия() Тогда
				Элемент.СписокВыбора.Добавить(Перечисления.ВидыДокументовОбязательнойСертификацииИС.ДекларацияСоответствия);
			КонецЕсли;
		ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.СладостиИКондитерскиеИзделия Тогда
			Если СладостиИКондитерскиеИзделияДоступнаДекларацияСоответствия() Тогда
				Элемент.СписокВыбора.Добавить(Перечисления.ВидыДокументовОбязательнойСертификацииИС.ДекларацияСоответствия);
			КонецЕсли;
		Иначе
			Элемент.СписокВыбора.Добавить(Перечисления.ВидыДокументовОбязательнойСертификацииИС.ДекларацияСоответствия);
	...
	ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
		...
		СтрШаблон(
			НСтр("ru = 'В табличной части ""Товары"" дублируется GTIN %1.';
				|en = 'В табличной части ""Товары"" дублируется GTIN %1.'"),
	...
	ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
		...
		СтрШаблон(
			НСтр("ru = 'В табличной части ""Товары"" не найден GTIN %1.';
				|en = 'В табличной части ""Товары"" не найден GTIN %1.'"),
			ПараметрыОтбора.GTIN));
КонецПроцедуры
...
//
// Возвращаемое значение:
//  см. ОбщегоНазначенияИС.ИнициализироватьДанныеОписанияНоменклатуры
...
Процедура ЗапронитьПротоколОбменаПоОснованиюДокумента(Форма) Экспорт
	
	Документ     = Форма.Документ;
	ТипДокумента = ТипЗнч(Документ);
	
	Если Метаданные.ОпределяемыеТипы.ОснованиеМаркировкаТоваровИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаМаркировкаТоваровИСМП(Форма);
	...
	Если Метаданные.ОпределяемыеТипы.ОснованиеЗаказНаЭмиссиюКодовМаркировкиИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		...
		ЗаполнитьПоОснованиюДокументаЗаказНаЭмиссиюКодовМаркировкиСУЗ(Форма);
		
	КонецЕсли;
	
	Если Метаданные.ОпределяемыеТипы.ОснованиеВыводаИзОборотаИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаВыводИзОборотаИСМП(Форма);
		
	КонецЕсли;
	
	Если Метаданные.ОпределяемыеТипы.ОснованиеСписаниеКодовМаркировкиИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаСписаниеКодовМаркировкиИСМП(Форма);
		
	КонецЕсли;
	
	Если Метаданные.ОпределяемыеТипы.ОснованиеПеремаркировкиТоваровИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаПеремаркировкаТоваровИСМП(Форма);
		
	КонецЕсли;
	
	Если Метаданные.ОпределяемыеТипы.ОснованиеВозвратВОборотИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаВозвратВОборотИСМП(Форма);
		
	КонецЕсли;
	
	Если Метаданные.ОпределяемыеТипы.ОснованиеОтгрузкаТоваровИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаОтгрузкаТоваровИСМП(Форма);
		
	КонецЕсли;
	
	Если Метаданные.ОпределяемыеТипы.ОснованиеПриемкаТоваровИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаПриемкаТоваровИСМП(Форма);
		
	КонецЕсли;
	
	Если Метаданные.ОпределяемыеТипы.ОснованиеОтчетИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаОтчетИСМП(Форма);
		
	КонецЕсли;
	
	Если Метаданные.ОпределяемыеТипы.ОснованиеПеремещениеМеждуМОДИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоПеремещениюМеждуМОД_ИСМП(Форма);
		
	КонецЕсли;
	
	Если Метаданные.ОпределяемыеТипы.ОснованиеРазрешениеНаОтгрузкуИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаРазрешениеНаОтгрузкуИСМП(Форма);
		
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьПоОснованиюДокументаЗаказНаЭмиссиюКодовМаркировкиСУЗ(Форма)
	
	МетаданныеДокумента = Метаданные.Документы.ЗаказНаЭмиссиюКодовМаркировкиСУЗ;
	
	ЗаполнитьПоОснованиюДокументаИСМП(МетаданныеДокумента, Форма);
	
КонецПроцедуры

Процедура ЗаполнитьПоОснованиюДокументаМаркировкаТоваровИСМП(Форма)
...
Процедура ЗаполнитьПоОснованиюДокументаВыводИзОборотаИСМП(Форма)
...
Процедура ЗаполнитьПоОснованиюДокументаСписаниеКодовМаркировкиИСМП(Форма)
...
Процедура ЗаполнитьПоОснованиюДокументаПеремаркировкаТоваровИСМП(Форма)
...
Процедура ЗаполнитьПоОснованиюДокументаОтгрузкаТоваровИСМП(Форма)
...
Процедура ЗаполнитьПоОснованиюДокументаПриемкаТоваровИСМП(Форма)
	
	МетаданныеДокумента = Метаданные.Документы.ПриемкаТоваровИСМП;
	
	ЗаполнитьПоОснованиюДокументаИСМП(МетаданныеДокумента, Форма, Ложь);
	
КонецПроцедуры

Процедура ЗаполнитьПоОснованиюДокументаВозвратВОборотИСМП(Форма)
...
Процедура ЗаполнитьПоОснованиюДокументаОтчетИСМП(Форма)
...
Процедура ЗаполнитьПоПеремещениюМеждуМОД_ИСМП(Форма)
	
	МетаданныеДокумента = Метаданные.Документы.ПеремещениеМеждуМОДИСМП;
	
	ЗаполнитьПоОснованиюДокументаИСМП(МетаданныеДокумента, Форма);
	
КонецПроцедуры

Процедура ЗаполнитьПоОснованиюДокументаРазрешениеНаОтгрузкуИСМП(Форма)
	
	МетаданныеДокумента = Метаданные.Документы.РазрешениеНаОтгрузкуИСМП;
	
	ЗаполнитьПоОснованиюДокументаИСМП(МетаданныеДокумента, Форма);
	
КонецПроцедуры

Процедура ЗаполнитьПоОснованиюДокументаИСМП(МетаданныеДокументаИСМП, Форма, ДобавитьГиперссылку = Истина)
	
	ДокументОснование = Форма.Документ;
	...
	ТаблицаДокументы = ПротоколОбменаИСМП.ТаблицаДокументы();
	...
	ПротоколОбменаИСМП.ЗаполнитьПоДокументу(Форма, ТаблицаДокументы, Истина);
	...
	Если СтатусыОформления.Свойство(МетаданныеДокументаИСМП.Имя)
		...
		Шаблон = ШаблонПредставленияДокументаДляПоляИнтеграции(
		...
		Если Не ПравоДобавления Тогда
			...
			ИмяКоманды   = Неопределено;
		ИначеЕсли ИнтеграцияИС.ДокументПоддерживаетКорректировку(МетаданныеДокументаИСМП)
			И СтатусыОформления[МетаданныеДокументаИСМП.Имя].Статус = Перечисления.СтатусыОформленияДокументовГосИС.ЕстьОшибкиОформления Тогда
		...
		Если ЗначениеЗаполнено(ТекстНадписи) Тогда
			...
			Если ЗначениеЗаполнено(Форма.ТекстДокументаИСМП) Тогда
				Строки.Добавить(Форма.ТекстДокументаИСМП);
				Строки.Добавить(", ");
			КонецЕсли;
			Строки.Добавить(ФорматированнаяСтрока);
			
			Форма.ТекстДокументаИСМП = Новый ФорматированнаяСтрока(Строки);
...
Функция ПолимерныеТрубыДоступенСертификатСоответствия()
	
	Возврат Истина;
	
КонецФункции

Функция СладостиИКондитерскиеИзделияДоступенСертификатСоответствия()
...
Функция ПолимерныеТрубыДоступнаДекларацияСоответствия()
	
	Возврат Истина;
	
КонецФункции

Функция СладостиИКондитерскиеИзделияДоступнаДекларацияСоответствия()
