	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Классификатор видов алкогольной продукции ЕГАИС';
				|en = 'Классификатор видов алкогольной продукции ЕГАИС'"),
	...
	Исключение
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Ошибка подготовки исходных файлов для загрузки классификатора ЕГАИС %1:
			           |%2';
			           |en = 'Ошибка подготовки исходных файлов для загрузки классификатора ЕГАИС %1:
			           |%2'",
		...
		ОбщегоНазначенияИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			ТекстОшибки,
			НСтр("ru = 'Работа с классификаторами ЕГАИС';
				|en = 'Работа с классификаторами ЕГАИС'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ДополнитьДанныеПоНоменклатуреДляПоискаМинимальныхЦен(Товары, ПараметрыПроверки)
	...
	Если Товары.Колонки.Найти("ИННКонтрагента") = Неопределено Тогда
...
Процедура ИнициализироватьДанныеКонтрагентовПередПоискомЦен(ПараметрыПроверки)
    ...
    Если Контрагент <> Неопределено Тогда	
		...
		Если Не ЗначениеЗаполнено(ДанныеКонтрагента.ИНН) Тогда
			МассивОшибок.Добавить(СтрШаблон(НСтр("ru = 'У контрагента ""%1"" не указан ИНН';
												|en = 'У контрагента ""%1"" не указан ИНН'"), Контрагент));
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДанныеКонтрагента.КПП) Тогда
			МассивОшибок.Добавить(СтрШаблон(НСтр("ru = 'У контрагента ""%1"" не указан КПП';
												|en = 'У контрагента ""%1"" не указан КПП'"), Контрагент));
		КонецЕсли;
	...
	Если Организация <> Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(ДанныеОрганизации.ИНН) Тогда
			МассивОшибок.Добавить(СтрШаблон(НСтр("ru = 'У организации ""%1"" не указан ИНН';
												|en = 'У организации ""%1"" не указан ИНН'"), Организация));
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДанныеОрганизации.КПП) Тогда
			МассивОшибок.Добавить(СтрШаблон(НСтр("ru = 'У организации ""%1"" не указан КПП';
												|en = 'У организации ""%1"" не указан КПП'"), Организация));
		КонецЕсли;
...
Процедура МинимальныеЦеныАлкогольнойПродукции(Товары, ПараметрыПроверки)
	...
	|				И втТоварыПолная.ОбъемДАЛ * 10 > 0.375
	...
	Если ПараметрыПроверки.ПроверятьЗаполнениеНоменклатуры Тогда
		...
		Пока ВыборкаНоменклатуры.Следующий() Цикл
			Если ВыборкаНоменклатуры.Крепость = 0 Тогда
				МассивОшибок.Добавить(СтрШаблон(НСтр("ru = 'У номенклатуры ""%1"" не указана крепость';
													|en = 'У номенклатуры ""%1"" не указана крепость'"), ВыборкаНоменклатуры.Номенклатура));
			КонецЕсли;
		
			Если ВыборкаНоменклатуры.Объем = 0 Тогда
				МассивОшибок.Добавить(СтрШаблон(НСтр("ru = 'У номенклатуры ""%1"" не указан объем';
													|en = 'У номенклатуры ""%1"" не указан объем'"), ВыборкаНоменклатуры.Номенклатура));
			КонецЕсли;
			
			Если Не ПараметрыПроверки.РозничнаяПродажа Тогда
				Если ВыборкаНоменклатуры.Производитель = ПроизводительНеУказан Тогда
					МассивОшибок.Добавить(СтрШаблон(НСтр("ru = 'У номенклатуры ""%1"" не указан производитель';
														|en = 'У номенклатуры ""%1"" не указан производитель'"),
				...
				Иначе
					Если ВыборкаНоменклатуры.ПроизводительИНН = "" Тогда
						МассивОшибок.Добавить(СтрШаблон(НСтр("ru = 'У номенклатуры ""%1"" в производителе не указан ИНН';
															|en = 'У номенклатуры ""%1"" в производителе не указан ИНН'"),
							ВыборкаНоменклатуры.Номенклатура));
					КонецЕсли;
					...
					Если ВыборкаНоменклатуры.ПроизводительКПП = "" Тогда
						МассивОшибок.Добавить(СтрШаблон(НСтр("ru = 'У номенклатуры ""%1"" в производителе не указан КПП';
															|en = 'У номенклатуры ""%1"" в производителе не указан КПП'"),
							ВыборкаНоменклатуры.Номенклатура));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			...
			Если ВыборкаНоменклатуры.ВидПродукции = Справочники.ВидыАлкогольнойПродукции.ПустаяСсылка() Тогда
				МассивОшибок.Добавить(СтрШаблон(НСтр("ru = 'У номенклатуры ""%1"" не указан вид алкогольной продукции';
													|en = 'У номенклатуры ""%1"" не указан вид алкогольной продукции'"), ВыборкаНоменклатуры.Номенклатура));
			КонецЕсли;
