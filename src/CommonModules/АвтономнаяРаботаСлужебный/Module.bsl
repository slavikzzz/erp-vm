...
Функция СобытиеЖурналаРегистрацииСозданиеАвтономногоРабочегоМеста() Экспорт
	...
	Возврат НСтр("ru = 'Автономная работа.Создание автономного рабочего места';
				|en = 'Standalone mode.Create standalone workstation'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУдалениеАвтономногоРабочегоМеста() Экспорт
	...
	Возврат НСтр("ru = 'Автономная работа.Удаление автономного рабочего места';
				|en = 'Standalone mode.Delete standalone workstation'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура УдалитьНеактуальныеСообщенияОбмена(АвтономноеРабочееМесто) Экспорт 
	...
	СобытиеЖурналаРегистрации = НСтр("ru = 'Обмен данными';
...
Процедура ЗаписатьЭлементДанныхНачальногоОбраза(ЭлементДанных, СвойстваМетаданных, Получатель) Экспорт 
	...
	Исключение
		...
		Если ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(МетаданныеЭлемента) Тогда
			Шаблон = НСтр("ru = 'Навигационная ссылка: %1';
							|en = 'URL: %1'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистр(МетаданныеЭлемента) Тогда
			Шаблон = НСтр("ru = 'Отбор: %1';
							|en = 'Filter: %1'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Шаблон = НСтр(
			"ru = 'Ошибка при записи элемента ""%1"" при создании начального образа
		     ...
		     |en = 'An error occurred when saving the ""%1"" item when creating the initial image
		     |%2 
