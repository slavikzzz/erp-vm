...
Процедура УточнитьДанныеРазбораПоВидуПродукции(СтрокаДанных, ВидПродукции, ПараметрыСканирования, ОбщиеМодули = Неопределено, СканированиеФормыПроверкиИПодбора = Ложь) Экспорт
	...
	Если ДопустимаТабачнаяПродукция(ПараметрыСканирования)
		...
		//@skip-check wrong-string-literal-content
		МодульШтрихкодированиеИСМП = ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСМП");
		МодульШтрихкодированиеИСМП.УстановитьПризнакОтказаОтПроверкиКодовМаркировкиПоСтроке(СтрокаДанных, ПараметрыСканирования, СканированиеФормыПроверкиИПодбора);
...
Процедура ЗаполнитьСведенияПоШтрихкодамEANСУчетомВерсии(ДанныеПоШтрихкодам, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(СтрокаДанных.Номенклатура) И Не СтрокаДанных.ЭтоКодВводаОстатков Тогда
...
Функция ИнициализацияДанныхПоШтрихкодам(ИндексироватьТаблицы = Истина) Экспорт
	...
	ДанныеПоШтрихкодам.Вставить("ОтсутствуютУстановленныеЛМЧЗ",               Ложь);
...
Функция ИнициализацияТаблицыДанныхКодовМаркировки(ИндексироватьТаблицы = Истина) Экспорт
	...
	ДанныеКодовМаркировки.Колонки.Добавить("ОтключитьЗапросВСервисПоОперацииИПродукции",   Новый ОписаниеТипов("Булево"));
	ДанныеКодовМаркировки.Колонки.Добавить("ПодлежитПроверкеРазрешительногоРежима",        Новый ОписаниеТипов("Булево"));
...
// Выполняет поиск в массиве ВидыПродукции пивной продукции.
//
// Параметры:
//  ВидыПродукции - Массив Из ПеречислениеСсылка.ВидыПродукцииИС - Коллекция видов продукции ИС.
// Возвращаемое значение:
//  Булево - Истина, если найдена пивная продукция.
Функция ПрисутствуетПивнаяПродукция(ВидыПродукции) Экспорт
...
// Параметры:
//  ВидыПродукции - Массив Из ПеречислениеСсылка.ВидыПродукцииИС - Коллекция видов продукции ИС.
//  НаДату        - Дата - дата проверки контроля перемещений между МОД.
// Возвращаемое значение:
//  Булево - Истина, если найдена пивная продукция.
Функция ПрисутствуетПрослеживаемаяПоМОДПивнаяПродукция(ВидыПродукции, НаДату = Неопределено) Экспорт
	
	МассивПивнойПродукции = ОбщегоНазначенияИСКлиентСервер.ВидыПродукцииПрослеживаемогоПеремещенияМеждуМОД();
	
	Для Каждого ВидПродукции Из МассивПивнойПродукции Цикл
		Если ВидыПродукции.Найти(ВидПродукции) <> Неопределено
			И ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПеремещенийМеждуМОД(ВидПродукции, НаДату) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьСтрокуДанныхНаВхождениеНедопустимыхСимволов(СтрокаДанных) Экспорт
	...
	ИначеЕсли СтрНайти(СтрокаДанных.Штрихкод, Символы.Таб)
		...
		СтрокаДанных.ТекстОшибки = НСтр("ru = 'Код не может содержать переносы строк и табуляции';
										|en = 'Код не может содержать переносы строк и табуляции'");
	КонецЕсли;
...
Функция ДанныеПоШтрихкодамEAN(ШтрихкодыEAN, ПараметрыСканирования = Неопределено, GTINПотребительскихУпаковок = Неопределено) Экспорт
	...
	Если ПараметрыСканирования <> Неопределено
		...
		КонецЕсли;
	КонецЕсли;
	ШтрихкодированиеИСПереопределяемый.ПриЗаполненииИнформацииПоШтрихкодамEAN(ДанныеПоШтрихкодамEAN, ПараметрыПоискаРМК, ПараметрыСканирования);
	...
	Для Каждого ЭлементДанных Из ДанныеПоШтрихкодамEAN Цикл
		...
		Если УникальныеGTIN[GTINСтроки] = Неопределено Тогда
			ДанныеПоGTIN.Вставить(GTINСтроки, ЭлементДанных);
	...
	ОписанияGTIN = ОписанияGTINПоДаннымШтрихкодовСУчетомВерсии(ПараметрыСканирования, НаборGTIN, ДанныеПоGTIN);
			...
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'GTIN %1 найден в составе кода маркировки потребительской упаковки.
					           ...
					           |en = 'GTIN %1 найден в составе кода маркировки потребительской упаковки.
					           |В регистре Описание GTIN ИС %1 соответствует виду упаковки: %2.
					           |GTIN логистических, групповых и потребительских упаковок должны быть различными.'"),
...
Функция ТребуетсяОткрытьФормуВводаКодаМаркировки(СтрокаДанных, ПараметрыСканирования) Экспорт
	...
	Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции, Истина) Тогда
		...
		ТребуетсяВводМарок = ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПоВидуОперацииИПродукции(
			ПараметрыСканирования.ВидОперацииИСМП,
			СтрокаДанных.ВидПродукции);
			
	Иначе
		
		ТребуетсяВводМарок = Истина;
		
	КонецЕсли;
	
	Если (ЭтоЗапросКодаМаркировкиПослеСчитыванияEAN
		Или ЭтоЗапросКодаМаркировкиПослеСчитыванияRFID) И ТребуетсяВводМарок Тогда
		...
		Если ЗначениеЗаполнено(СтрокаДанных.Номенклатура)
			...
			СтрокаДанных.ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Номенклатура %1 является немаркируемой';
					|en = 'Номенклатура %1 является немаркируемой'"),
...
Функция ТекстОшибкиТребуетВзвешиванияПриГрупповойОбработке()
	...
	ТекстОшибки =  НСтр("ru = 'Для кода маркировки требуется указание индивидуального количества';
						|en = 'Для кода маркировки требуется указание индивидуального количества'");
	Возврат ТекстОшибки;
...
Функция ТекстОшибкиНеПоддерживаетсяВидПродукции(СтрокаДанных, ПоддерживаемыеВидыПродукции, ВидПродукцииШтрихкода)
	...
	Фрагмент3 = СтрШаблон(НСтр("ru = 'опознан как %1.';
								|en = 'опознан как %1.'"), НРег(ПредставлениеВидаПродукции));
...
Функция ТекстОшибкиШтрихкодСоответствуетВидамПродукции(ВидыПродукцииКодаМаркировки, ВыключенУчет = Ложь)
	...
	Если ВыключенУчет Тогда
		ШаблонОшибки = НСтр("ru = 'В информационной базе выключен учет по видам продукции: %1';
							|en = 'В информационной базе выключен учет по видам продукции: %1'");
	Иначе
		ШаблонОшибки = НСтр("ru = 'Значение штрихкода соответствует недопустимым видам продукции: %1';
							|en = 'Значение штрихкода соответствует недопустимым видам продукции: %1'");
	КонецЕсли;
...
Функция ТекстОшибкиНеопределеннойУпаковки()
	...
	Возврат НСтр("ru = 'Отсканируйте код логистической упаковки в соответствующей форме
	                   ...
	                   |en = 'Отсканируйте код логистической упаковки в соответствующей форме
	                   |проверки и подбора маркируемой продукции'");
...
Функция ТекстОшибкиТребуетсяУточнениеКоличества() Экспорт
	...
	Возврат НСтр("ru = 'Требуется индивидуальное считывание с указанием веса';
				|en = 'Требуется индивидуальное считывание с указанием веса'");
...
Функция ТекстОшибкиТолькоВесоваяПродукция(СтрокаДанных) Экспорт
	...
	Возврат СтрШаблон(НСтр("ru = 'Считанный код %1 не требует уточнения веса.
	                             |Отсканируйте код весовой продукции.';
	                             |en = 'Считанный код %1 не требует уточнения веса.
...
Функция ОбработатьШтрихкоды(ДанныеШтрихкодаСписок, ПараметрыСканирования, КэшированныеЗначения,
	...
	Если ВнешниеДанныеПоШтрихкодам = Неопределено Тогда
		...
		ДанныеПоШтрихкодам = ИнициализацияДанныхПоШтрихкодам(Ложь);
		ДанныеПоШтрихкодам.ЭтоСканированиеВФормеПроверкиИПодбора = (ТипЗнч(ФормаУникальныйИдентификатор) = Тип("УникальныйИдентификатор"));
...
Процедура ОтметитьНовыеИИзмененныеШтрихкодыУпаковок(ДанныеПоШтрихкодам, ЭлементыДляИзменения, ПараметрыСканирования)
	...
	Если ПараметрыСканирования.РасширеннаяВерсияГосИС Тогда
		...
		//@skip-check wrong-string-literal-content
		Модуль = ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеИС");
		Модуль.ОтметитьНовыеИИзмененныеШтрихкодыУпаковок(ПараметрыСканирования, ДанныеПоШтрихкодам, ЭлементыДляИзменения);
...
Функция ОписанияGTINПоДаннымШтрихкодовСУчетомВерсии(ПараметрыСканирования, СписокGTIN, ДанныеПоGTIN)
	...
	Если ОбщегоНазначенияИС.ЭтоРасширеннаяВерсияГосИС() Тогда
		...
		//@skip-check wrong-string-literal-content
		Модуль = ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеИС");
		ОписанияGTIN = Модуль.ОписанияGTINПоДаннымШтрихкодовРасширеннойВерсии(СписокGTIN);
		
	Иначе
		ОписанияGTIN = ОписанияGTINПоДаннымШтрихкодов(ПараметрыСканирования, ДанныеПоGTIN);
	КонецЕсли;
...
Функция ОписанияGTINПоДаннымШтрихкодов(ПараметрыСканирования, ДанныеПоGTIN)
	
	ОписанияGTIN = Новый Соответствие;
	ШтрихкодированиеИСПереопределяемый.ПриЗаполненииОписанияGTINПоДаннымШтрихкодов(ПараметрыСканирования, ОписанияGTIN, ДанныеПоGTIN);
...
Процедура ДополнитьДанныеШтрихкодовРасширеннойСпецификойМОТП(ПараметрыСканирования, ДанныеПоШтрихкодам)
	...
	МодульШтрихкодированиеОбщегоНазначенияМОТП.ДобавитьКолонкиТаблицыДанныеКодовМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования);
	МодульШтрихкодированиеОбщегоНазначенияИСМП.ЗаполнениеПолногоКодаМаркировкиПоДаннымСоставаКодаМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования);
	...
	Иначе
		...
		МодульШтрихкодированиеОбщегоНазначенияИСМП.ЗаполнитьПоДаннымСокращеннойИнформации(ДанныеПоШтрихкодам, ПараметрыСканирования);
...
Функция ДанныеПоШтрихкодуУжеПолучены(ДанныеШтрихкода) 
	...
	Возврат ДанныеШтрихкода.Свойство("ТипУпаковки") И
		(ЗначениеЗаполнено(ДанныеШтрихкода.Номенклатура)
			Или ДанныеШтрихкода.МаркируемаяПродукция = Истина);
...
Функция ОбработатьДанныеПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования, ЕстьОшибки, ФормаУникальныйИдентификатор, РезультатыПроверок) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		ДополнитьРезультатОбработкиДополнительнымиПризнакамиРазрешительногоРежима(РезультатОбработки, ДанныеПоШтрихкодам);
...
Функция НастроеннаяНоменклатураЧастичногоВыбытия(СписокНоменклатуры) Экспорт
	...
	Возврат РегистрыСведений.НастройкиЧастичногоВыбытияПродукцииИС.НастроеннаяНоменклатураЧастичногоВыбытия(СписокНоменклатуры);
	
КонецФункции

Процедура ДополнитьРезультатОбработкиДополнительнымиПризнакамиРазрешительногоРежима(РезультатОбработкиШтрихкода, ДанныеПоШтрихкодам)
	...
	Если ПрисутствуетТабачнаяПродукция(РезультатОбработкиШтрихкода.ВидыПродукции)
		...
		МодульШтрихкодированиеИСМПСлужебный = ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСМП");
		МодульШтрихкодированиеИСМПСлужебный.ДополнитьРезультатОбработкиДополнительнымиПризнакамиРазрешительногоРежима(РезультатОбработкиШтрихкода, ДанныеПоШтрихкодам);
