...
Процедура ЗаполнитьРезультатПроверкиСертификатНеквалифицированный(ПараметрыПроверки, Результат) Экспорт
	Если ПараметрыПроверки.ПроверятьСертификат = ТолькоКвалифицированные() Тогда
		ТекстОшибки = НСтр("ru = 'Сертификат неквалифицированный, подпись таким сертификатом не является юридически значимой.';
							|en = 'The certificate is unqualified. Signing with such certificate is not legally relevant.'");
...
Функция РезультатПоискаКриптопровайдеров(КриптопровайдерыРезультат, ИмяСервера = "") Экспорт
	...
	Иначе
		...
		ИначеЕсли КриптопровайдерыРезультат.ПроверкаВыполнена Тогда
			...
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'На сервере %1 не установлены приложения электронной подписи и шифрования.';
					|en = 'No digital signing and encryption apps are installed on the server %1.'"), ИмяСервера);
			КонецЕсли;
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ПользователиСертификатаСтрокой(Пользователь1, Пользователь2, КоличествоПользователей) Экспорт
	...
	Если КоличествоПользователей > 2 Тогда
		СтрокаПользователей = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 и другие (всего %2)';
				|en = '%1 and other (total %2)'"), СтрокаПользователей, Формат(КоличествоПользователей, "ЧГ=0"));
	КонецЕсли;
...
Функция ОпределитьПрограмму(СвойстваСертификата,
	...
	Если ПрограммаНайдена = Неопределено Тогда
		...
		ШаблонОшибки = НСтр("ru = 'Не предусмотрено использование ни одного приложения
			...
			|en = 'Cannot find any applications that are
			|configured for this signature algorithm: %1.'");
...
Функция ОшибкаВызоваМетодаКомпоненты(ИмяМетода, ИнформацияОбОшибке) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка вызова метода %1 компоненты %2.';
			|en = 'Error when calling method ""%1"" of add-in ""%2"".'"), ИмяМетода, "ExtraCryptoAPI")
	...
	ИначеЕсли Программа <> Неопределено Тогда
		...
		Если Не ПрограммаНайдена Тогда
			МенеджерКриптографииДобавитьОшибку(Ошибки, Программа, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Приложение %1 не предусмотрено для использования (отключено администратором).';
					|en = 'The %1 application cannot be used (disabled by the administrator).'"), Программа), Истина);
			Возврат Неопределено;
		КонецЕсли;
...
Функция МенеджерКриптографииСвойстваПрограммы(ОписаниеПрограммы, ЭтоLinux, Ошибки, ЭтоСервер,
	...
	Если Не ЗначениеЗаполнено(ОписаниеПрограммы.ИмяПрограммы) Тогда
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка,
			НСтр("ru = 'Не указано название приложения.';
				|en = 'App name required.'"), Истина);
		Возврат Неопределено;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ОписаниеПрограммы.ТипПрограммы) Тогда
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка,
			НСтр("ru = 'Не указан тип приложения.';
				|en = 'App type required.'"), Истина);
		Возврат Неопределено;
	КонецЕсли;
				...
				Если ПутиКМодулям.Количество() = 1 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Файл не существует: ""%1"".';
							|en = 'File does not exist: ""%1"".'"), ПутиКМодулям[0]);
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция МенеджерКриптографииАлгоритмыУстановлены(ОписаниеПрограммы, Менеджер, Ошибки, АлгоритмШифрования = "") Экспорт
	...
	Исключение
		...
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Выбран неизвестный алгоритм подписи ""%1"".';
				|en = 'Unknown signature algorithm ""%1"" is selected.'"), АлгоритмПодписи), Истина);
		Возврат Ложь;
	КонецПопытки;
	...
	Исключение
		...
		// Платформа использует обобщенное сообщение "Неизвестный алгоритм криптографии". Требуется более конкретное.
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Выбран неизвестный алгоритм хеширования ""%1"".';
				|en = 'Unknown hash algorithm ""%1"" is selected.'"), АлгоритмХеширования), Истина);
		Возврат Ложь;
	КонецПопытки;
	...
	Исключение
		...
		// Платформа использует обобщенное сообщение "Неизвестный алгоритм криптографии". Требуется более конкретное.
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Выбран неизвестный алгоритм шифрования ""%1"".';
				|en = 'Unknown encryption algorithm ""%1"" is selected.'"), АлгоритмШифрования), Истина);
		Возврат Ложь;
	КонецПопытки;
...
Процедура МенеджерКриптографииПрограммаНеНайдена(ОписаниеПрограммы, Ошибки, ЭтоСервер) Экспорт
	...
	МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка,
		НСтр("ru = 'Приложение не установлено на компьютере.';
			|en = 'Приложение не установлено на компьютере.'"), ЭтоСервер, Истина);
	...
	Если ОписаниеОшибок.Ошибки.Количество() = 0 Тогда
		Если Не ЗначениеЗаполнено(АлгоритмПодписи) Тогда
			ТекстОшибки = НСтр("ru = 'Не предусмотрено использование ни одного приложения электронной подписи и шифрования.';
								|en = 'No digital signing and encryption app is configured.'");
		Иначе
			ШаблонОшибки = НСтр("ru = 'Не предусмотрено использование ни одного приложения электронной подписи и шифрования
			                          |с алгоритмом подписи %1.';
			                          |en = 'No digital signing and encryption application is configured
			                          |for this signing algorithm: %1.'");
...
Функция СертификатПросрочен(Сертификат, НаДату, РазницаСУниверсальнымВременем) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'На %1 просрочен сертификат.';
			|en = 'Certificate is overdue on %1.'"), Формат(НачалоДня(НаДату), "ДЛФ=D"));
...
Функция ЗакрытыйКлючПросрочен(СвойстваСертификата, НаДату) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'На %1 просрочен закрытый ключ сертификата.';
			|en = 'As of %1, the private certificate key has expired.'"), Формат(НачалоДня(НаДату), "ДЛФ=D"));
...
Функция НастройкиСозданияПодписи(ТипПодписи, АдресаСерверовМетокВремени) Экспорт
	...
	Если ТипПодписи = ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.СМеткойДоверенногоВремениCAdEST") 
		...
		Если Результат.АдресаСерверовМетокВремени.Количество() = 0 Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для создания подписи с типом ""%1"" заполните адреса серверов меток времени.';
					|en = 'To create a signature with type ""%1"", fill in timestamp server addresses.'"), ТипПодписи);
		КонецЕсли;
	...
	ИначеЕсли ТипПодписи <> ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.БазоваяCAdESBES")
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Создание подписи типа ""%1"" не поддерживается.';
				|en = 'Creating a signature of type ""%1"" is not supported.'"), ТипПодписи);
	КонецЕсли;
...
Процедура ЗаполнитьОписаниеДанныхСертификата(Таблица, СвойстваСертификата) Экспорт
	...
	Иначе
		Строка.Значение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 (срок действия закрытого ключа)';
				|en = '%1 (private key validity period)'"), Формат(СвойстваСертификата.ДействителенДо, "ДЛФ=D"));
	КонецЕсли;
	 ...
	 Или Не ЗначениеЗаполнено(ДанныеАлгоритмаПодписания.ВыбранныйАлгоритмХеширования) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не указаны алгоритмы подписания и хеширования для создания подписи,
			           ...
			           |en = 'Signing and hash algorithms to create a signature
	...
	Если СвойстваКонвертаXML.АлгоритмПодписи.Идентификатор
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В документе XML указанный алгоритм подписи
			           ...
			           |en = 'The specified signing algorithm
			           |%1 (%2 OID)
...
Функция ДиагностическаяИнформацияОКомпьютере(ДляКлиента = Ложь) Экспорт
	...
	Если Не ПустаяСтрока(ПрограммаПросмотра) Тогда
		ПрограммаПросмотра = Символы.ПС + НСтр("ru = 'Приложение:';
												|en = 'App:'") + " " + ПрограммаПросмотра;
	КонецЕсли;
	
	Возврат НСтр("ru = 'Операционная система:';
				|en = 'Operating system:'") + " " + СисИнфо.ВерсияОС
		+ Символы.ПС + НСтр("ru = 'Версия:';
							|en = 'Version:'") + " " + СисИнфо.ВерсияПриложения
		+ Символы.ПС + НСтр("ru = 'Тип платформы:';
							|en = 'Platform type:'") + " " + СисИнфо.ТипПлатформы
		+ ПрограммаПросмотра;
...
Функция СертификатВЛокальномХранилищеТекстРешения() Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '<a href = ""%1"">Удалите сертификат</a> из локального хранилища компьютера.';
																		|en = '<a href = ""%1"">Uninstall certificate</a> from local storage on your computer.'"),
...
Функция ДиагностикаСерверовМетокВремени(АдресаСерверовМетокВремени, СтрокаПодключения) Экспорт
	...
	Для Каждого Адрес Из АдресаСерверовМетокВремени Цикл
		...
		Иначе
			Массив.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 - доступен.';
																						|en = '%1 is available.'"), Адрес));
		КонецЕсли;
		...
		Попытка
			...
			Если ОтветHTTP.КодСостояния < 200 Или ОтветHTTP.КодСостояния >= 300 Тогда
				...
				Если ОтветHTTP.КодСостояния < 200 Или ОтветHTTP.КодСостояния >= 300 И ОтветHTTP.КодСостояния < 400 Тогда
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Неподдерживаемый ответ сервера (%1)';
							|en = 'Unsupported server response (%1)'"), ОтветHTTP.КодСостояния);
				...
				ИначеЕсли ОтветHTTP.КодСостояния >= 400 И ОтветHTTP.КодСостояния < 500 Тогда
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка при выполнении запроса (%1)';
							|en = 'Request error (%1)'"), ОтветHTTP.КодСостояния);
					ВызватьИсключение ТекстОшибки;

				Иначе

					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка сервера при обработке запроса к ресурсу (%1)';
							|en = 'A server error occurred while processing a request (%1)'"), ОтветHTTP.КодСостояния);
...
Функция ОбластьXML(СтрокаXML, ИмяЭлемента, НомерSignature = 1) Экспорт
	...
	Если Позиция = 0 Тогда
		...
		Если Позиция = 0 Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не найдена элемент ""%1"" в документе XML.';
					|en = 'The %1 element is not found in the XML document.'"), ИмяЭлемента);
	...
	Если Позиция = 0 Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не найдено окончание элемента ""%1"" в документе XML.';
				|en = '%1 element end is not found in the XML document.'"), ИмяЭлемента);
		Результат.ТекстОшибки = ТекстОшибки;
	КонецЕсли;
	...
	Если Позиция = 0 Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не найдено окончание заголовка элемента ""%1"" в документе XML.';
				|en = 'The %1 element title end is not found in the XML document.'"), ИмяЭлемента);
		Результат.ТекстОшибки = ТекстОшибки;
	КонецЕсли;
	...
	Если СоответствующийOIDПодписи <> Неопределено И OIDАлгоритмаПодписи <> СоответствующийOIDПодписи Тогда
		...
		АлгоритмПодписиНеСоответствуетГОСТ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Алгоритм %1, указанный в подписи, не соответствует стандарту, так как должен быть указан %2, соответствующий алгоритму хеширования %3.';
				|en = 'The %1 algorithm specified in the signature does not comply with the standard. Specify the %2 algorithm that corresponds to the %3 hash algorithm.'"),
			...
			ОбщегоНазначенияКлиентСервер.Проверить(Ложь,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Недопустимый тип значения ""%1""
					           ...
					           |at the temporary storage address specified in the Data parameter'") + Символы.ПС,
