	...
	Для Каждого ОписаниеКолонки Из ОписаниеСоответствияКолонок Цикл
		Если КолонкиТаблицыВыплаченнойЗарплаты.Все.Найти(ОписаниеКолонки.Ключ) = Неопределено Тогда
			ВызватьИсключение НСтр("ru = 'НоваяТаблицаВыплаченнойЗарплатыПоТабличнойЧасти: недопустимое имя колонки таблицы выплаченной зарплаты в описании соответствия колонок';
									|en = 'НоваяТаблицаВыплаченнойЗарплатыПоТабличнойЧасти: invalid name of a paid salary table column in column mapping description'")
		КонецЕсли;	
				...
				СообщениеОбОшибке = 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Документ %1 не содержит сотрудника %2';
							|en = 'Document %1 does not contain employee %2'"), 
						Ошибка.Ведомость, 
				...
				СообщениеОбОшибке = 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Выплата сотруднику %2 по документу %1 уже произведена документом %3';
							|en = 'Payment to employee %2 against document %1 is already performed by document %3'"), 
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Начальная установка настройки Отражение взаиморасчетов с сотрудниками.';
									|en = 'Initial value of the ""Record of mutual settlements with employees"" setting.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Очистка видов дохода исполнительного производства выплат прошлых периодов ведомостей в банк.';
									|en = 'Clearing of enforcement proceedings income kinds of the last periods payments of statements to the bank.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Очистка видов дохода исполнительного производства выплат прошлых периодов ведомостей в кассу.';
									|en = 'Clearing of enforcement proceedings income kinds of the last periods payments of statements to the cash account.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка вида взаиморасчетов незачисленной зарплаты при обмене с банками.';
									|en = 'Setting kind of non-credited payroll mutual settlements on EDI with banks.'");
...
Функция ОписательВременныхТаблицДляСоздатьВТСостояниеВыплат(МенеджерВременныхТаблиц, ТаблицаОтборов) Экспорт
	...
	Иначе	
		ВызватьИсключение НСтр("ru = 'Недопустимый тип параметра ТаблицаОтборов';
								|en = 'Invalid type of the ТаблицаОтборов parameter'");
	КонецЕсли;
...
Процедура СоздатьВТСостояниеВыплат(ОписательВременныхТаблиц, ТолькоРазрешенные, Организация, ПериодРегистрации, ИгнорируемыеРегистраторы = Неопределено, ИмяВТСостояниеВыплат = "ВТСостояниеВыплат") Экспорт
	...
	|ИЗ
	|	ВТОтборДокументовДляЗарплатаКВыплате КАК Выплаты";
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		ИмяФункциональнойОпции = "ИспользоватьРасчетПервойПоловиныМесяца";
		ЧитатьРегистрЗарплатаКВыплатеАвансом = ПолучитьФункциональнуюОпцию(ИмяФункциональнойОпции);
	...
	КоличествоЗаписейЗапроса = РезультатЗапроса.Количество();
	ЧитатьРегистрЗарплатаКВыплате = КоличествоЗаписейЗапроса > 0;
	
	Если КоличествоЗаписейЗапроса = 1 Тогда
	...
	Если ЧитатьРегистрЗарплатаКВыплатеАвансом Тогда
		...
		|	Выплаты.ДокументОснование КАК ДокументОснование
		|ИЗ
		|	ВТОтборДокументовДляЗарплатаКВыплатеАвансом КАК Выплаты";
		Запрос.Текст = СтрЗаменить(ТекстЗапроса, "#ВТОтборов", ОписательВременныхТаблиц.ИмяВТОтборов);
		РезультатЗапроса = Запрос.Выполнить().Выбрать();
		КоличествоЗаписейЗапроса = РезультатЗапроса.Количество();
		ЧитатьРегистрЗарплатаКВыплатеАвансом = КоличествоЗаписейЗапроса > 0;
		Если КоличествоЗаписейЗапроса = 1 Тогда
			РезультатЗапроса.Следующий();
			ОтборДокументОснованиеАванс = РезультатЗапроса.ДокументОснование;
		Иначе
			ОтборДокументОснованиеАванс = Ложь;
		КонецЕсли;
	...
	КонецЕсли;	
	ФрагментЗапросаСОснованиемВложенныйЗапрос =
	"(ЗарплатаКВыплате.ДокументОснование, ЗарплатаКВыплате.ФизическоеЛицо) В
	|			(ВЫБРАТЬ
	|				Отборы.ДокументОснование КАК ДокументОснование,
	|				Отборы.ФизическоеЛицо КАК ФизическоеЛицо
	|			ИЗ
	|				#ВТОтборДокументов КАК Отборы)";
	ФрагментЗапросаБезОснованияВложенныйЗапрос =
	"(ЗарплатаКВыплате.ФизическоеЛицо) В
	|			(ВЫБРАТЬ
	|				Отборы.ФизическоеЛицо КАК ФизическоеЛицо
	|			ИЗ
	|				#ВТОтборДокументов КАК Отборы)";
	...
	Если ЧитатьРегистрЗарплатаКВыплате Тогда
		...
		Если ОтборДокументОснование = Ложь Тогда
			Подзапрос = СтрЗаменить(Подзапрос,"&ОтборВложенныйЗапрос",ФрагментЗапросаСОснованиемВложенныйЗапрос);
			Подзапрос = СтрЗаменить(Подзапрос,"&ОтборДокументОснование","Истина");
		Иначе
			Подзапрос = СтрЗаменить(Подзапрос,"&ОтборВложенныйЗапрос",ФрагментЗапросаБезОснованияВложенныйЗапрос);
			Подзапрос = СтрЗаменить(Подзапрос,"&ОтборДокументОснование","ЗарплатаКВыплате.ДокументОснование = &ОтборДокументОснование");
	...
	Если ЧитатьРегистрЗарплатаКВыплатеАвансом Тогда
		Подзапрос = ШаблонПодзапроса;
		Если ОтборДокументОснованиеАванс = Ложь Тогда
			Подзапрос = СтрЗаменить(Подзапрос,"&ОтборВложенныйЗапрос",ФрагментЗапросаСОснованиемВложенныйЗапрос);
			Подзапрос = СтрЗаменить(Подзапрос,"&ОтборДокументОснование","Истина");
		Иначе
			Подзапрос = СтрЗаменить(Подзапрос,"&ОтборВложенныйЗапрос",ФрагментЗапросаБезОснованияВложенныйЗапрос);
			Подзапрос = СтрЗаменить(Подзапрос,"&ОтборДокументОснование","ЗарплатаКВыплате.ДокументОснование = &ОтборДокументОснованиеАванс");
			Запрос.УстановитьПараметр("ОтборДокументОснованиеАванс",ОтборДокументОснованиеАванс);
		КонецЕсли;
