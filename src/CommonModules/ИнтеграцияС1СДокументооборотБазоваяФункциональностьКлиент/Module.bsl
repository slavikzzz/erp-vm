...
Процедура ВыбратьЗначениеДополнительногоРеквизита(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	Если СписокДоступныхТипов.Количество() > 1 Тогда
		...
		ЗаголовокФормы = НСтр("ru = 'Тип реквизита ""%Свойство%""';
								|en = 'The ""%Свойство%"" attribute type'");
...
Асинх Процедура ВыполнитьПроверкуЗаписиИПродолжить(Знач СсылкаНаОбъектИС, Знач ФормаОбъектаИС,
	...
	Если СсылкаНаОбъектИС.Пустая() Тогда
		...
		ТекстПредупреждения = СтрШаблон(
			...
			|You can perform the ""%1"" action only after you save the data.'"),
			Действие);
		...
		Иначе
			ПоказатьПредупреждение(,
				НСтр("ru = 'Просмотр объектов такого типа не поддерживается в этой версии 1С:Документооборота.';
					|en = 'View of this type objects is not supported in this version of 1C:Document Management.'"));
		КонецЕсли;
...
Функция ПоказатьВопросДаНетАсинх(ТекстВопроса, ПараметрыВопроса = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Недопустимое значение кнопки по умолчанию: %1';
										|en = 'Invalid value for default button: %1'"),
...
Процедура ПроверитьПодключение(ОписаниеОповещения, Владелец, ПовторноПоказыватьОкноАвторизации = Ложь,
	...
	ВыполнитьЗапросАсинхронно(
		...
		НСтр("ru = 'Подключение к 1С:Документооборот.';
			|en = 'Connect to 1C:Document Management.'"),
		СкрыватьИнтерфейс);
		...
		Исключение
			...
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ЗаписатьПредупреждение(
				НСтр("ru = 'Не найдены сохраненные учетные данные пользователя 1С:Документооборота';
					|en = 'Saved accounting data of 1C:Document Management user is not found'"));
		КонецПопытки;
		...
		Инфо = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			СтрШаблон(
				НСтр("ru = 'Не удалось заполнить объект по данным из 1С:Документооборот.
					...
					|en = 'Cannot fill the object by data from 1C:Document Management.
					|Check the <a href = ""%1"">integration rule</a>.'"),
...
Асинх Процедура ЗакончитьРедактированиеФайла(Знач ID, Знач Наименование, Знач Расширение,
	...
	Если Не СведенияОРедактировании.Сохранен Или СведенияОРедактировании.НаЧтение Тогда
		Ответ = Ждать ПоказатьВопросДаНетАсинх(НСтр("ru = 'Файл не найден в рабочем каталоге. Освободить его?';
													|en = 'File was not found in the working directory. Unlock the file?'"));
		Если Ответ = КодВозвратаДиалога.Да Тогда
	...
	ТекстПояснения = СтрШаблон(НСтр("ru = 'Идет сохранение файла ""%1"" (%2). Пожалуйста, подождите...';
									|en = 'Saving the ""%1"" file (%2). Please wait...'"),
...
Функция НеобходимоДобавитьСканКопиюАсинх(СостояниеРазрешаетДобавлениеСканКопии,
	...
	Если СостояниеРазрешаетДобавлениеСканКопии И Не СостояниеРазрешаетДобавлениеФайла Тогда
		ТекстВопроса = НСтр(
			...
			|en = 'In the current state, you can add only a scanned copy of the original document. Proceed?'");
		ПараметрыВопросаДаНет = ПараметрыВопросаДаНет(КодВозвратаДиалога.Да);
...
Процедура ОткрытьКаталогФайла(ID) Экспорт
	...
	Если Не РасширениеРаботыСФайламиПодключено() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Открывать каталог файла можно только после подключения
			...
			|en = 'You can open the file directory only after the
			|file system extension is attached.'"));
		Возврат;
	КонецЕсли;
	...
	Если СведенияОРедактировании.Сохранен Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru = 'Файл не найден в рабочем каталоге.';
											|en = 'File is not found in the working directory.'"));
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Файл не найден в рабочем каталоге.';
										|en = 'File is not found in the working directory.'"));
...
Асинх Процедура ОтменитьРедактированиеФайла(Знач ID, Знач ОписаниеОповещенияОЗавершении = Неопределено,
	...
	Ответ = Ждать ПоказатьВопросДаНетАсинх(
		НСтр("ru = 'Отмена редактирования файла может привести к потере изменений. Продолжить?';
			|en = 'Canceling file editing will result in the loss of changes. Continue?'"));
...
Функция ПолучитьФильтрДляДиалогаВыбораФайла() Экспорт
	...
	Фильтр = НСтр("ru = 'Все файлы (*.*)|*.*';
		...
		+ НСтр("ru = '|Документы MS Office(*.doc;*.docx;*.xls;*.xlsx;*.ppt;*.pptx)|*.doc;*.docx;*.xls;*.xlsx;*.ppt;*.pptx';
				|en = '|MS Office documents(*.doc;*.docx;*.xls;*.xlsx;*.ppt;*.pptx)|*.doc;*.docx;*.xls;*.xlsx;*.ppt;*.pptx'")
		+ НСтр("ru = '|Документы Open Office(*.odt;*.odf;*.ods;*.odp)|*.odt;*.odf;*.ods;*.odp';
				|en = '|Open Office documents(*.odt;*.odf;*.ods;*.odp)|*.odt;*.odf;*.ods;*.odp'")
		+ НСтр("ru = '|Текстовые файлы(*.txt)|*.txt';
				|en = '|Text files(*.txt)|*.txt'")
		+ НСтр("ru = '|Файлы PDF(*.pdf)|*.pdf';
				|en = '|PDF files(*.pdf)|*.pdf'")
		+ НСтр("ru = '|Файлы HTML(*.htm;*.html)|*.htm;*.html';
				|en = '|HTML files(*.htm;*.html)|*.htm;*.html'")
		+ НСтр("ru = '|Картинки (*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf)|*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf';
				|en = '|Images (*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf)|*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf'");
	...
	Иначе
		Если ТекущаяПометкаУдаления = Истина Тогда
			ТекстВопроса = НСтр("ru = 'Снять с выделенных файлов пометку на удаление?';
								|en = 'Unmark selected files for deletion?'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Пометить выделенные файлы на удаление?';
								|en = 'Mark selected files for deletion?'");
		КонецЕсли;
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.СделатьВерсиюАктивной(
		...
		Если ТекстСообщения = "" Тогда
			ТекстСообщения = НСтр("ru = 'Активная версия изменена.';
									|en = 'The active version is changed.'");
		КонецЕсли;
...
Асинх Процедура СохранитьИзмененияРедактируемогоФайла(Знач ID, Знач Наименование, Знач Расширение,
	...
	Если Не СведенияОРедактировании.Сохранен Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не удалось сохранить изменения файла,
			...
			|en = 'Cannot save changes in the file
			|as it is not locked for editing now.'"));
		Возврат;
	КонецЕсли;
	Если СведенияОРедактировании.НаЧтение Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не удалось сохранить изменения файла,
			|так как он сейчас сохранен только для просмотра.';
			|en = 'Cannot save changes
			|as the file is saved only for reading.'"));
		Возврат;
	КонецЕсли;
	Параметры.Вставить("СведенияОРедактировании", СведенияОРедактировании);
	...
	Если Не РасширениеРаботыСФайламиПодключено() Тогда
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Идет сохранение файла ""%1"". Пожалуйста, подождите...';
				|en = 'Saving the ""%1"" file. Please wait...'"),
			Параметры.ИмяФайла);
		Состояние(ТекстПояснения);
	...
	Если Параметры.ОставшиесяФайлы.Количество() = 0 Тогда
		...
		Диалог.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Все файлы (*.%1)|*.%1';
				|en = 'All files (*.%1)|*.%1'"), Параметры.ТекущийФайл.Расширение, Параметры.ТекущийФайл.Расширение);
	Иначе
...
Процедура ИнформироватьОПодписании(Представление, ДобавленныеПодписиОбъектов, ДокументID,
	...
	Состояние(СтрШаблон(НСтр("ru = 'Установлена подпись для ""%1""';
							|en = 'Signature is set for ""%1""'"), Представление));
	...
	Для Каждого Элемент Из ДобавленныеПодписиОбъектов Цикл
		...
		Если ДанныеВладельцаПодписи.Тип <> "DMFile" Тогда
			Продолжить;
		КонецЕсли;
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Установлена подпись для ""%1""';
										|en = 'Signature is set for ""%1""'"), ДанныеВладельцаПодписи.Наименование);
		ИнформироватьОбИзмененииФайла(
			"ИзменениеЭПФайла",
...
Процедура НачатьФормированиеПодписиОбъекта(Форма, ОписаниеОповещенияОЗакрытии) Экспорт
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Отсутствует стандартная подсистема ""Электронная подпись"".';
										|en = 'The standard ""Digital signature"" subsystem is missing.'"));
		Возврат;
	КонецЕсли;
	...
	Для Каждого ОбъектДляПодписи Из МассивОбъектовДляПодписи Цикл
		...
		ИначеЕсли ОбъектДляПодписи.ОбъектТип = "DMFile" Тогда
			...
			Если Строки.Количество() = 1 Тогда
				ПредставлениеФайла = СтрШаблон(НСтр("ru = 'Файл: %1';
													|en = 'File: %1'"), Строки[0].Наименование);
				ДанныеДляПодписи.Вставить("Представление",
	...
	Для Каждого ДанныеСтроки Из ТаблицаПодписей Цикл
		...
		Если Не ЕстьПодсистемаЭП Тогда
			Статус = НСтр("ru = 'Недействительна (%1): отсутствует стандартная подсистема ""Электронная подпись""';
							|en = 'Invalid (%1): the standard ""Digital signature"" subsystem is missing'");
...
Асинх Процедура УдалитьПодписи(Знач УдаленныеСтроки, Знач ТаблицаПодписей, Знач ВладелецФайла,
	...
	Для Каждого ЭлементСоответствия Из ПодписиОбъектов Цикл
		...
		Если ДанныеВладельцаПодписи.Тип <> "DMFile" Тогда
			Продолжить;
		КонецЕсли;
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Удалена подпись файла ""%1""';
										|en = 'The %1 file signature is deleted.'"), ДанныеВладельцаПодписи.Наименование);
		ИнформироватьОбИзмененииФайла(
			"ИзменениеЭПФайла",
			ДанныеВладельцаПодписи.Наименование,
			ДанныеВладельцаПодписи.ID,
	...
	Если ПараметрыПриложения["ИнтеграцияС1СДокументооборотКомпонентаПолученияКартинкиИзБуфера"] = Неопределено Тогда
		...
		Если КодВозврата Тогда
			Состояние(НСтр("ru = 'Компонента сканирования уже установлена.';
							|en = 'The scanner add-in is already installed.'"));
		Иначе
	...
	Если Результат = Истина Тогда
		...
		Иначе
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Буфер обмена не содержит картинки';
											|en = 'Clipboard contains no pictures'"));
...
Процедура ЗакончитьРедактированиеСРасширениемПослеВопросаОбОсвобождении(Параметры)
	...
	ТекстСообщения = СтрШаблон(
		НСтр("ru = 'Файл %1 обновлен и освобожден.';
			|en = 'The %1 file is updated and unlocked.'"),
	...
	Если ДатаМодификацииВыбранная < Параметры.ТекущийФайл.ДатаМодификации Тогда // В хранилище более новый.
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Файл ""%1""
				...
				|Операция прервана.';
				|en = 'The %1 file
				|in the file storage was changed later (%2)
	...
	Если Настройки.ИспользоватьИнтеграциюДО2 Тогда
		...
		Если ВладелецФайла = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Не указан владелец файла в интегрированной системе.';
											|en = 'A file owner is not specified in the integrated system.'"));
			Возврат;
		КонецЕсли;
		...
		Исключение
			ПоказатьПредупреждение(,
				НСтр("ru = 'Не удалось заменить файл на диске файлом из 1С:Документооборота.
					...
					|en = 'Cannot replace a file on the disk with the file from 1C:Document Management.
			...
			Исключение
				ПоказатьПредупреждение(,
					НСтр("ru = 'Не удалось открыть файл для редактирования. Возможно,
						...
						|en = 'Cannot open the file to edit.
...
Процедура ПродолжитьСохранитьКакПослеВыбораИсточника(ПолучаемыеФайлы, Параметры) Экспорт
	...
	Состояние(НСтр("ru = 'Файл сохранен.';
					|en = 'File is saved.'"), , Параметры.ПолныйПуть);
	...
	Если Подключено Тогда
		...
		Если Параметры.ЯвляетсяСканКопией Тогда
			ВыборФайла.Заголовок = НСтр("ru = 'Выбор скан-копии оригинала документа';
										|en = 'Select the scanned copy of the original document'");
			...
			Если ЗначениеЗаполнено(СтрокаРасширений) Тогда
				...
				ВыборФайла.Фильтр = СтрШаблон(НСтр("ru = 'Скан-копии (%1)|%2|Все файлы (*.*)|*.*';
													|en = 'Scanned copies (%1)|%2|All files (*.*)|*.*'"),
...
Процедура СохранитьИзмененияСРасширениемПослеВопросаОбОсвобождении(Параметры)
	...
	ТекстСообщения = СтрШаблон(
		НСтр("ru = 'Изменения файла %1 сохранены.';
			|en = 'The %1 file changes are saved.'"),
		Параметры.ТекущийФайл.Наименование);
	Состояние(ТекстСообщения);
	ИнформироватьОбИзмененииФайла(
		"РедактированиеФайлаЗавершено",
	...
	Если МассивПодписей.Количество() = 1 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Добавлена подпись из файла для ""%1""';
				|en = 'The signature from file was added for ""%1""'"), СвойстваФайла.ИмяФайла);
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Добавлены подписи из файлов для ""%1""';
				|en = 'Signatures from files were added for ""%1""'"), СвойстваФайла.ИмяФайла);
	КонецЕсли;
	ИнформироватьОбИзмененииФайла(
		"ИзменениеЭПФайла",
		СвойстваФайла.ИмяФайла,
...
Процедура НачатьДобавлениеЭПИзФайлаПослеПодключенияРасширения(Подключено, Параметры) Экспорт
	...
	Если Не Подключено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Для выполнения операции нужно установить расширение работы с файлами.';
										|en = 'To perform the operation, install the file system extension.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПодписатьФайлЗавершение(ОписаниеДанных, СвойстваФайла) Экспорт
	...
	Если ТипЗнч(ОписаниеДанных) = Тип("Структура") И ОписаниеДанных.Свойство("СвойстваПодписи", СвойстваПодписи) Тогда
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Установлена подпись для ""%1""';
										|en = 'Signature is set for ""%1""'"), СвойстваФайла.ИмяФайла);
		ИнформироватьОбИзмененииФайла(
			"ИзменениеЭПФайла",
			СвойстваФайла.ИмяФайла,
			СвойстваФайла.ИдентификаторФайла,
...
#Если МобильныйКлиент Тогда
	...
	ПоказатьПредупреждение(, НСтр("ru = 'Печать файлов данного типа не поддерживается в мобильном клиенте.';
									|en = 'The mobile client does not support printing files of this type.'"));
	Возврат;
		...
		Если Не ПустаяСтрока(СписокРасширенийСканКопийОригиналов)
			...
			ТекстПредупреждения = СтрШаблон(
				НСтр("ru = 'Файл ""%1"" не является скан-копией. Выберите файл с расширением: %2.';
					|en = 'The %1 file is not a scanned copy. Select a file with the extension: %2.'"),
				Файл.Имя,
					...
					ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ЗаписатьПредупреждение(
						СтрШаблон(
							НСтр("ru = 'Нет доступа к рабочей папке: %1';
								|en = 'No access to the working folder: %1'"),
...
#КонецЕсли
	...
	Если Не Успех Тогда
		ВызватьИсключение НСтр("ru = 'Не удается создать файл в каталоге временных файлов.';
								|en = 'Cannot create file in the temporary directory.'");
	КонецЕсли;
	...
	Если РазницаДат > 0 Тогда
		...
		ПараметрыФормы.Вставить("Сообщение",
			НСтр("ru = 'Файл на диске новее, чем файл в 1С:Документообороте.
				...
				|en = 'Disk file is newer than the file in 1C:Document Management.
				|It might have been modified.
				|
				|Use the existing file or replace it with the file from 1C:Document Management
				|and possibly lose the changes?'"));
	Иначе
		// файл на диске старше
		ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'В 1С:Документообороте новый файл';
													|en = 'There is a new file in 1C:Document Management.'"));
		ПараметрыФормы.Вставить("Сообщение",
			НСтр("ru = 'Файл в 1С:Документообороте новее, чем файл на диске.
				...
				|en = 'File in 1C:Document Management is newer than the one on the disk.
				|It might have been modified.
				|
				|Take the file from 1C:Document Management replacing the file on the disk and lose the
				|changes or use the existing file?'"));
	КонецЕсли;
...
Процедура СоздатьФайлСДискаЗавершение(Параметры)
	...
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Создан файл ""%1""';
									|en = 'The ""%1"" file is created'"), Параметры.ТекущийФайл.Наименование);
	ИнформироватьОбИзмененииФайла(
		"СозданиеФайла",
		Параметры.ТекущийФайл.Наименование,
		Параметры.ТекущийФайл.ID,
