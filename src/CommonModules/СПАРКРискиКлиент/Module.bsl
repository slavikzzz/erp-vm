		...
		Если УслугаПодключена <> Истина Тогда
			...
			РезультатИндексыКонтрагента.Вставить("ИнформацияОбОшибке", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось загрузить индексы 1СПАРК Риски.
					...
					|en = 'Cannot import 1C-SPARK Risks indexes.
...
Процедура ПоказатьЗначенияИндексовКонтрагента(ИндексыСПАРКРиски, ВладелецФормы = Неопределено) Экспорт
	...
	Если ИндексыСПАРКРиски = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Нет информации для отображения.';
										|en = 'No information available to display.'"));
		Возврат;
	ИначеЕсли ИндексыСПАРКРиски.ПодлежитПроверке <> Истина Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выбранный контрагент не подлежит проверке в сервисе 1СПАРК Риски.';
										|en = 'The selected counterparty is not subject to checking in 1C-SPARK Risks service.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПерейтиВВебВерсиюСПАРК(
	...
	Если Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ВнутренняяОшибкаСервиса") Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<b>Не удалось подключиться к сервису 1СПАРК Риски </b>
				...
				|en = '<b>Cannot connect to 1C-SPARK Risks service </b>
				|
				|Cannot connect to 1CPARK Risks service. The service is temporarily unavailable. Try again later.'"));
	...
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ИспользованиеЗапрещено") Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<b>Переход в веб-версию СПАРК недоступен </b>
				...
				|en = '<b>Navigation to SPARK online is unavailable </b>
				|
				|1C-SPARK Risks service cannot be used in the current mode.'"));
	...
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ОшибкаПодключения") Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			...
			|• Firewall or another middleware (antivirus, etc.) prevents the application from connecting to the Internet;
			|• Internet connection is executed via proxy server but its parameters are not set in the application.'"));
	...
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ИнтернетПоддержкаНеПодключена")
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<b>Переход в веб-версию СПАРК недоступен </b>
				...
				|en = '<b>Navigation to SPARK online is unavailable </b>
				|
				|To use 1C-SPARK Risks service, connect to online user support. Contact the administrator.'"));
		...
		Иначе
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				НСтр("ru = '<b>Переход в веб-версию СПАРК недоступен </b>
					...
					|en = '<b>Navigation to SPARK online is unavailable </b>
					|
					|There is not enough limit to navigate.
					|To replenish the limit, <a href=""%1"">purchase a service plan</a>,
					| that includes SPARK online.'"),
	...
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.НеПодлежитПроверке") Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<b>Переход в веб-версию СПАРК недоступен </b>
				...
				|en = '<b>Navigation to SPARK online is unavailable </b>
				|
				|The counterparty is not subject to check, navigation to online version is not allowed.'"));
	...
	ИначеЕсли ЗначениеЗаполнено(Результат.ВидОшибки) Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<b>Не удалось перейти в веб-версию СПАРК </b>
				...
				|en = '<b>Cannot go to SPARK online </b>
				|
				|Errors occurred while receiving a navigation link. Contact the administrator.'"));
		...
		Если Источник.Ссылка.Пустая() Или Форма.Модифицированность Тогда
			...
			Если Команда.Имя = "СПАРКРиски_ОткрытьСписокСправокКонтрагента" Тогда
				ТекстВопроса = НСтр("ru = 'Данные еще не записаны.
					...
					|You can go to ""information sheets"" only when data is written.
			...
			ИначеЕсли Команда.Имя = "СПАРКРиски_СобытияМониторингаПоКонтрагенту" Тогда
				ТекстВопроса = НСтр("ru = 'Данные еще не записаны.
					...
					|Данные будут записаны.';
					|en = 'Data is not written yet.
					|You can go to ""Monitoring events"" only when data is written.
					|Data will be written.'");
			ИначеЕсли Команда.Имя = "СПАРКРиски_ВключитьМониторингСобытий" Тогда
				ТекстВопроса = НСтр("ru = 'Данные еще не записаны.
					...
					|Данные будут записаны.';
					|en = 'Data is not written yet.
					|Event monitoring can be enabled only when data is written.
					|Data will be written.'");
			ИначеЕсли Команда.Имя = "СПАРКРиски_ПерейтиВСПАРК" Тогда
				ТекстВопроса = НСтр("ru = 'Данные еще не записаны.
					...
					|You can go to SPARK online only after saving the data.
					|Data will be saved.'");
			КонецЕсли;
...
Процедура ПерейтиКПокупкеСервиса()
	...
	ПоказатьВопрос(
		...
		НСтр("ru = 'Вы можете заказать сервис у партнера, либо купить сервис непосредственно в фирме 1С';
			|en = 'You may order the service from the parter or buy the service directly in the 1C company'"),
		КнопкиВопроса,
		,
		,
		НСтр("ru = 'Покупка сервиса';
			|en = 'Purchase of the service'"));
