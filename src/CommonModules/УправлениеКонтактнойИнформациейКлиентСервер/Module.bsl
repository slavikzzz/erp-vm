...
Функция СформироватьПредставлениеТелефона(КодСтраны, КодГорода, НомерТелефона, Добавочный, Комментарий) Экспорт
	...
	Если НЕ ПустаяСтрока(Добавочный) Тогда
		Представление = Представление + ?(ПустаяСтрока(Представление), "", ", ") + НСтр("ru = 'доб.';
																						|en = 'ext.'") + " " + СокрЛП(Добавочный);
	КонецЕсли;
...
Функция РасширеннаяПодсказкаАдреса(КомандыДляВывода, ПредставлениеАдреса, Комментарий) Экспорт
	...
	Если КомандыДляВывода.Свойство("ПоказатьНаКартеЯндекс") И КомандыДляВывода.Свойство("ПоказатьНаКартеGoogle") Тогда
		ПоказатьНаКарте = Новый ФорматированнаяСтрока(НСтр("ru = 'На карте';
															|en = 'On the map'"),,WebЦвета.Серый, , "ПоказатьНаКарте");
