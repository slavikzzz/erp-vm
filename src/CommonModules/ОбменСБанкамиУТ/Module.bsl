	...
	Если ЗначениеЗаполнено(Организация) И ЗначениеЗаполнено(БанковскийСчет) Тогда
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(НСтр("ru = '%1
				...
				|en = '%1 
				|Errors occurred when posting.'"), ОписаниеОшибки.Описание);
				
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Данные.Проведение';
					|en = 'Data.Posting'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура СоздатьФайлРеестраОбменаСБанком(ВладелецФайла, ДанныеДокумента, СтруктураПараметров) Экспорт
	...
	Если СтруктураПараметров.ВидОперации = "ОплатаСамозанятому" Тогда
		...
		ДанныеДокумента.ИмяФайла = СтрШаблон(
			НСтр("ru = '%1.%2';
				|en = '%1.%2'"), ДанныеДокумента.ИмяФайла, ?(ВыгрузкаФорматаСбербанк, "csv", "xls"));
	...
	Если ЗначениеЗаполнено(ДанныеЗаполнения.ПервичныйДокумент) Тогда
		...
		Иначе
			...
			ОбменСБанкамиПоЗарплатнымПроектамПереопределяемый.ВедомостиПлатежногоДокументаПеречисленияЗарплаты(
				ДанныеЗаполнения.ПервичныйДокумент, Ведомости);
			
			Если Ведомости.Количество() <> 0 Тогда
				
				Ведомость = Ведомости[0];
				ЗарплатныйПроект = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ведомость, "ЗарплатныйПроект");
				ДанныеЗаполнения.Вставить("ЗарплатныйПроект", ЗарплатныйПроект);
