	...
	Исключение
		...
		РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
			...
			НСтр("ru = 'Фоновое задание по обработке непроверенных контрагентов после включения';
				|en = 'Background job to process unchecked counterparties after enabling'",
...
Процедура УстановитьНадписиВПанелиОтчетаПриСозданииНаСервере(Форма) Экспорт
	...
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Проверка контрагентов по данным ФНС выполнена успешно';
																	|en = 'Counterparties check in FTS base was successful'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Есть некорректные контрагенты.
	ЭлементФормы = Форма.Элементы.Найти("ТекстПриНекорректныхКонтрагентах");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Обнаружены недействующие контрагенты по данным ФНС';
																	|en = 'Inactive counterparties (according to FTS) are detected'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Выполняется проверка
	ЭлементФормы = Форма.Элементы.Найти("ТекстПриПроверкеВПроцессеВыполнения");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Выполняется проверка контрагентов согласно данным ФНС';
																	|en = 'Checking counterparties against FTS data '"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Нет доступа к сервису
	ЭлементФормы = Форма.Элементы.Найти("ТекстНетДоступаКСервису");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Не удалось произвести проверку контрагентов: сервис ФНС временно недоступен';
																	|en = 'Cannot check counterparties: FTS server is temporarily unavailable'"), СсылкаНаИнструкцию);
	КонецЕсли;
		...
		Попытка
		 	ФоновоеЗадание = ФоновыеЗадания.Выполнить("ПроверкаКонтрагентов.ПроверитьКонтрагентовВДокументе", 
				Параметры, , НСтр("ru = 'Проверка контрагентов в документе';
									|en = 'Counterparty check in document'"));
		...
		Исключение
			...
			РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
				...
				НСтр("ru = 'Фоновое задание в документе';
					|en = 'Background job in the document'",
					ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверитьКонтрагентовВОтчете(Форма, СтруктураПараметров) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Проверка контрагентов в отчете';
															|en = 'Checking counterparties in report'");
	...
	Иначе
		...
		Элемент.Заголовок =
			НСтр("ru = 'В настоящее время веб-сервис ФНС работает в тестовом режиме.';
				|en = 'FTS web service is operating in test mode now.'");
	КонецЕсли;
...
Функция ПроверкаДоступностиСервисов() Экспорт
	...
	ДлительныеОперации.СообщитьПрогресс(
		...
		НСтр("ru = 'Проверка наличия доступа к веб-сервису ФНС.';
			|en = 'Check access to the FTS web service.'")); 
	...
	Если Не ПустаяСтрока(ОписаниеОшибкиФНС) Тогда
		ОписаниеОшибкиФНС = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Проверка доступа к сервису ""Проверка контрагентов ФНС"".
					|%1';
					|en = 'Check access to the FTS counterparty check service.
					|%1'"),
				ОписаниеОшибкиФНС);
	КонецЕсли;
	...
	ДлительныеОперации.СообщитьПрогресс(
		...
		НСтр("ru = 'Проверка наличия доступа к веб-сервису 1С:Контрагент.';
			|en = 'Check access to the 1C:Counterparty web service.'"));
	ОписаниеОшибкиSOAP = "";
		...
		МассивОшибокСервисаКонтрагент.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Проверка доступа к сервису ""1С:Контрагент"".
					|%1';
					|en = 'Check access to the 1C:Counterparty service.
					|%1'"),
				ОписаниеОшибкиSOAP));
	КонецЕсли;
	...
	ДлительныеОперации.СообщитьПрогресс(
		...
		НСтр("ru = 'Проверка наличия доступа к веб-сервису 1С:Контрагент (REST).';
			|en = 'Check access to the 1C:Counterparty web service (REST).'"));
	
	ОписаниеОшибкиREST = "";
	РаботаСКонтрагентами.ПроверитьДоступностьRESTСервисаКонтрагент(ОписаниеОшибкиREST);
	Если Не ПустаяСтрока(ОписаниеОшибкиREST) Тогда
		МассивОшибокСервисаКонтрагент.Добавить(
			"Проверка доступа к сервису ""1С:Контрагент (REST)"""
			+ Символы.ПС
			+ ОписаниеОшибкиREST);
	...
	Исключение
		...
		РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
			...
			НСтр("ru = 'Ошибка при проверке из карточки контрагента';
				|en = 'An error occurred while checking from the counterparty profile'",
				ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Исключение
		...
		РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
			...
			НСтр("ru = 'Запись результатов проверки контрагентов в регистр';
				|en = 'Save counterparty check results to the register'",
				ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция НовоеПолеКартинки(Форма, ТаблицаФормы, Родитель, ПолеСКонтрагентом)
	...
	НовоеПоле.Заголовок					= НСтр("ru = 'Есть ошибка по данным ФНС';
													|en = 'There is an error in FTS data'");
		...
		Если СвойстваСправочника = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отсутствует описание справочника с именем ""%1"" в методе ПриОпределенииСвойствСправочниковКонтрагентов()
					...
					|method of the РаботаСКонтрагентамиПереопределяемый common module.'"),
				ИмяСправочника);
		КонецЕсли;
