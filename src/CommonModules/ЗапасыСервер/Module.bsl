			...
			Иначе
				...
				Если ОбновлениеИнформационнойБазы.ЭтоВызовИзОбработчикаОбновления("Отложенно") Тогда
					ВызватьИсключение НСтр("ru = 'Создание новых видов запасов в режиме отложенного обновления запрещено.';
											|en = 'Cannot create new inventory owner attributes in deferred update mode.'");
				КонецЕсли;
...
Процедура ЗаполнитьВидыЗапасовПоТоварамОрганизаций(ДокументОбъект,
	...
	Если Не ПараметрыЗаполненияВидовЗапасов.ТаблицаОстатковУжеСформирована
		...
		ТекстИсключения = НСтр("ru = 'Процедуру %1 можно вызывать только с таблицей остатков %2 или %3';
								|en = 'The %1 procedure can be called only with the %2 balance table or with %3'");
...
Процедура ЗаполнитьВидыЗапасовПоОстаткамКОформлению(ДокументОбъект,
	...
	Если ПараметрыЗаполненияВидовЗапасов.ИмяТаблицыОстатков = "ТаблицаВидыЗапасов"
		...
		ТекстИсключения = НСтр("ru = '%1 нужна для заполнения видов запасов по умолчанию, поэтому %2 должен быть выстален в ИСТИНА.';
								|en = '%1 is needed to fill in the inventory owner attributes by default, therefore, %2 should be set to TRUE.'");
...
Процедура СформироватьВТДоступныеВидыЗапасов(ДокументОбъект,
	...
	//++ НЕ УТКА
	Если ЗначениеЗаполнено(ОтборыВидовЗапасов.ОрганизацияДавалец) Тогда
		Запрос.УстановитьПараметр("ОрганизацияДавалец", ОтборыВидовЗапасов.ОрганизацияДавалец);
	Иначе
		Запрос.УстановитьПараметр("ОрганизацияДавалец", Неопределено);
	КонецЕсли;
	//-- НЕ УТКА
	...
	Пока Выборка.Следующий() Цикл
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не заполнена колонка ""Номер ГТД"" в строке %1 списка ""%2""';
						|en = 'Column ""CCD number"" in line %1 of the ""%2"" list is not filled in'"),
					Выборка.НомерСтроки,
					ПредставлениеТЧ);
...
Процедура ПроверитьНаличиеИзделийВШапкеСУчетомПоГТД(ДокументОбъект, Отказ) Экспорт
	...
	Если РеквизитыИзделия.ВестиУчетПоГТД = Истина
		...
		ШаблонСообщения = НСтр("ru = 'В документе выбрана номенклатура, для которой ведется учет по ГТД.
							|При включенной опции ""Запретить оформление операций с импортными товарами без номеров ГТД"" выбор такой номенклатуры запрещен.';
							|en = 'Items with CCD records are selected in the document.
							|If the ""Prohibit registration of operations with imported goods without CCD numbers"" option is selected, you cannot select these items.'");
...
Функция ИсправитьОстаткиПоТоварамКПередачеВФоновомЗадании(ПараметрыЗадания) Экспорт
	...
	НаименованиеЗадания = НСтр("ru = 'Исправление отрицательных остатков по товарам к передаче';
								|en = 'Correct negative stock balance for transfer'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если КоличествоОтрицательное <> 0 Тогда
			Если КоличествоОтрицательное > КоличествоВКорректировках Тогда
				...
				|en = 'An error occurred in the correction algorithm of stock balance to transfer.
			...
			Иначе
				...
				|en = 'An error occurred while correcting the stock balance for transfer:
				|there are excessive expense movements in register adjusting entries by dimensions 
				|%Организация% - %АналитикаУчетаНоменклатуры% - %ВидЗапасов% - %НомерГТД%,
				|excessive expense %КоличествоОтрицательное%. Adjust document register records and post manually.'");
...
Функция ИмяСобытияВыключенКонтрольОстатков() Экспорт
	...
	Возврат НСтр("ru = 'Контроль остатков товаров организаций';
				|en = 'Control of stock balance of companies'", КодОсновногоЯзыка);
...
Процедура ТаблицаОстатковМатериаловВПодразделении(МенеджерВременныхТаблиц, СтруктураПараметров) Экспорт
	...
	|						ИЛИ &ПоВсейНоменклатуре)
	|	
	|	) КАК МатериалыИРаботыВПроизводстве
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|				И (Движения.Подразделение В(&Подразделение)
	...
	ИначеЕсли ТипЗнч(Подразделение) = Тип("Массив")
		И Подразделение.Количество() > 0 Тогда
	...
	Если Результат.Пустой() Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(ПараметрыОбработчика,
															НСтр("ru = 'Дублей видов запасов не найдено.';
																|en = 'Duplicate of inventory owner attributes are not found.'",
																ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Иначе
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
			...
			НСтр("ru = 'Обнаружены дубли видов запасов. Требуется выполнить замену дублей и удалить помеченные виды запасов.';
				|en = 'Duplicates of inventory owner attributes are found. Replace duplicates and remove the selected inventory owner attributes.'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
			,
			,
...
Процедура ПроверкаКорректностьРеализацииУказаннойВВозврате(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Реализация",					НСтр("ru = 'Документ реализации';
																|en = 'Sales document'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Обнаружены возвращенные от клиента товары, отсутствующие в документе реализации организации ""%1""';
			|en = 'Goods returned from the customer that are missing in the sales document of the ""%1"" company are detected'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"Возврат");
...
Процедура ПроверкаКорректностиЗаполненияСуммВОтчетахПоКомиссииМеждуОрганизациями(ПараметрыПроверки) Экспорт
	...
	ШаблонТекстаОшибки  = НСтр("ru = 'Обнаружены отчеты по комиссии между организациями с некорректными суммовыми показателями по организации ""%1"" за период %2';
								|en = '""Commission reports — Intercompany"" with incorrect sum indicators for the company ""%1"" for the period %2 are found'",
								ОбщегоНазначения.КодОсновногоЯзыка());
	
	СписокПолей = Новый СписокЗначений;
	СписокПолей.Добавить("Организация",  НСтр("ru = 'Организация';
												|en = 'Company'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить(ИмяПоляОбъекта, НСтр("ru = 'Отчет по комиссии между организациями';
												|en = 'Commission sales report — Intercompany'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ИсправлениеОтрицательныхОстатковТоваровОрганизаций(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Отрицательные остатки в регистре ""Товары организаций""';
			|en = 'Negative stock balance in the ""Company goods"" register'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Отрицательных остатков быть не должно.';
			|en = 'There must be no negative inventory.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаОтрицательныхОстатковТоваровОрганизаций(ПараметрыПроверки) Экспорт
	...
	Если ДатаЗапрета > ПараметрыПроверки.ДополнительныеПараметры.КонецПериода Тогда
		ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
									...
									НСтр("ru = 'Рекомендуется исправить отрицательные остатки товаров организации ""%1"" на конец периода %2 в открытом периоде';
										|en = 'Correct the negative stock balance of the %1 company at the end of the period %2 in an open period'", ОбщегоНазначения.КодОсновногоЯзыка()),
									СписокПолей);
	...
	Если ЗакрытиеМесяцаСервер.РазмерВременнойТаблицы(Запрос, "ВТКОформлениюПередач", ПараметрыОбработчика) = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет остатков в регистре ""Резервы товаров организаций""';
				|en = 'No balance in the ""Company goods reserves"" register'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ОформлениеПередачТоваров(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Не оформленные передачи товаров между организациями';
			|en = '""Customer invoices — Intercompany"" that are not registered'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Должны быть оформлены документы ""Передача товаров между организациями"".';
			|en = 'The ""Customer invoice  — Intercompany"" documents must be registered.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаНеобходимостиОформленияПередачТоваров(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Требуется оформить передачу товаров в организацию ""%1"" на конец периода %2';
			|en = 'Register goods transfer to the ""%1"" company as of the %2 period end'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
	...
	Если ЗакрытиеМесяцаСервер.РазмерВременнойТаблицы(Запрос, "ОстаткиКПередаче", ПараметрыОбработчика) = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет остатков в регистре ""Товары организаций к передаче""';
				|en = 'No balance in the ""Company goods for transfer"" register'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ОформлениеОстатковТоваровКПередаче(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Не оформленные остатки передач товаров между организациями';
			|en = 'Non-registered balance of intercompany goods transfers'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Должны быть оформлены документы ""Передача товаров между организациями"".';
			|en = 'The ""Customer invoice  — Intercompany"" documents must be registered.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаОстатковПередачТоваров(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Обнаружены товары, требующие оформления передачи/возврата от организации ""%1"" на конец периода %2';
			|en = 'Goods that require transfer/return from the ""%1"" company to be registered are detected at the end of %2 period'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
	...
	Если ЗакрытиеМесяцаСервер.РазмерВременнойТаблицы(Запрос, "ВТКОформлениюВыкупов", ПараметрыОбработчика) = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет остатков товаров к выкупу в регистре ""Резервы товаров организаций""';
				|en = 'No stock balance to redemption in the ""Company goods reserves"" register'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ОформлениеВыкупаТоваров(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Не оформленные выкупы товаров, принятых на ответственное хранение';
			|en = 'Non-registered redemptions of available VMI'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Должны быть оформлены документы ""Выкуп товаров с хранения"".';
			|en = 'The ""Purchase invoice — VMI"" documents should be registered.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаОстатковТоваровКВыкупу(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Обнаружены товары, требующие оформления выкупа по организации ""%1"" на конец периода %2';
			|en = 'Goods that require redemption registration are detected for the ""%1"" company at the end of %2 period'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
	...
	Если РазмерыВременныхТаблиц.ВТОстаткиКомитента = 0 И РазмерыВременныхТаблиц.ВТОстаткиКомиссионера = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет остатков в регистре ""Товары к оформлению отчетов комитенту"" по схеме Интеркампани';
				|en = 'No balance in the ""Goods for consignment notifications issue"" register under Intercompany'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ОформлениеОтчетовПоКомиссииМеждуОрганизациями(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Не оформленные отчеты по комиссии между организациями';
			|en = '""Commission reports — Intercompany"" that are not registered'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Должны быть оформлены документы ""Отчет по комиссии между организациями"" и ""Отчет по комиссии между организациями о списании"".';
			|en = 'The ""Commission sales report — Intercompany"" and ""Commission write-off report — Intercompany"" documents must be registered.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаОформленияОтчетовКомитентуМеждуОрганизациями(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Комиссионер", 			   НСтр("ru = 'Организация-комиссионер';
															|en = 'Consignee company'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("АналитикаУчетаНоменклатуры", НСтр("ru = 'Аналитика номенклатуры';
															|en = 'Item dimension'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ВидЗапасов", 				   НСтр("ru = 'Вид запасов';
																|en = 'Inventory owner attributes'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Обнаружены товары, требующие оформления отчетов комитенту ""%1"" на конец периода %2';
			|en = 'Goods that require consignment notifications issue to the ""%1"" consignor were detected as of the end of %2 period'"),
		СписокПолей);
	...
	СписокПолей.Добавить("Комитент", 				   НСтр("ru = 'Организация-комитент';
															|en = 'Consignor company'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("АналитикаУчетаНоменклатуры", НСтр("ru = 'Аналитика номенклатуры';
															|en = 'Item dimension'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ВидЗапасов", 				   НСтр("ru = 'Вид запасов';
																|en = 'Inventory owner attributes'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗакрытиеМесяцаСервер.ДополнитьПараметрыРегистрацииПроблемПроверки(ПараметрыРегистрации,
		...
		НСтр("ru = 'Обнаружены товары, требующие оформления отчетов комитенту от организации ""%1"" на конец периода %2';
			|en = 'Goods were detected that require consignment notifications issue to the consignor from the ""%1"" company as of the end of %2 period'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Процедура Использование_ОформлениеОтчетовКомитентамОСписании(ПараметрыОбработчика) Экспорт
	...
	Если ЗакрытиеМесяцаСервер.РазмерВременнойТаблицы(Запрос, "ВТКОформлению", ПараметрыОбработчика) = 0 Тогда
		Пояснение = НСтр("ru = 'Нет остатков в регистре ""Товары к оформлению отчетов комитенту"" по сторонним комитентам';
						|en = 'No balance in the ""Goods for consignment notifications issue"" register for third-party consignors'",
						ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОписаниеПроверок_ОформлениеОтчетовКомитентамОСписании(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Не оформленные отчеты комитентам о списании';
			|en = 'Non-registered vendor consignment stock adjustments'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Должны быть оформлены документы ""Отчет комитенту о списании"".';
			|en = 'The ""Vendor consignment stock adjustment"" documents should be registered.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаОформленияОтчетовКомитентуОСписании(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Комитент", 				   НСтр("ru = 'Комитент';
															|en = 'Consignor'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("АналитикаУчетаНоменклатуры", НСтр("ru = 'Аналитика номенклатуры';
															|en = 'Item dimension'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ВидЗапасов", 				   НСтр("ru = 'Вид запасов';
																|en = 'Inventory owner attributes'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Обнаружены товары, требующие оформления отчетов комитенту о списании от организации ""%1"" на конец периода %2';
			|en = 'Goods were detected that require consignment notifications from the ""%1"" company as of the end of %2 period'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Функция ОрганизацияДокумента(ДокументОбъект, ПараметрыЗаполненияВидовЗапасов)
	...
	Иначе
		ТекстИсключения = НСтр("ru = 'Ошибка в программе: невозможно определить организацию документа.';
								|en = 'An error occurred in the application: cannot identify the document company.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
...
Функция ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение(ПараметрыЗаполненияВидовЗапасов, ВыборкаТоваров)
	...
	ИначеЕсли ВРег(ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение) = ВРег("ПоХозяйственнойОперации") Тогда
		...
		Если ДействияПодбораТоваровПринятыхНаОтветственноеХранениеПоХозяйственнойОперации = Неопределено Тогда
			ТекстИсключения = НСтр("ru = 'В параметрах заполнения видов запасов не определено подставлять или нет в табличную часть товары, принятые на ответственное хранение для типа запасов %ТипЗапасов%.';
									|en = 'In the parameters for filling the inventory owner attributes, it is not defined whether to insert the available VMI in the table for the %ТипЗапасов% inventory type.'");
		...
		Если ВРег(ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение) <> ВРег("Всегда")
			...
			ТекстИсключения = НСтр("ru = 'В параметрах заполнения видов запасов не определено подставлять или нет в табличную часть товары, принятые на ответственное хранение для хозяйственной операции %ХозяйственнаяОперация%.';
									|en = 'In the parameters for filling in the inventory owner attributes, it is not defined whether to insert the available VMI in the table for the business transaction %ХозяйственнаяОперация%.'");
	...
	Иначе
		ТекстИсключения = НСтр("ru = 'В параметрах заполнения видов запасов не определено подставлять или нет в табличную часть товары, принятые на ответственное хранение.';
								|en = 'In the filling parameters for inventory owner attributes, it is not defined whether to add the available VMI to the table.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатковСформироватьВТНовыеРезервы(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровОтКлиента
		...
		ТекстИсключения = НСтр("ru = 'Функция %Функция% не предназначена для вызова с хозяйственной операцией %ХозяйственнаяОперация%';
								|en = 'The %Функция% function cannot be called with business transaction %ХозяйственнаяОперация%'");
	...
	Пока ВыборкаОстаткиПоСтрокам.Следующий() Цикл
		...
		//++ НЕ УТКА
		ОрганизацияДавалец			= ВыборкаОстаткиПоСтрокам.ОрганизацияДавалец;
		//-- НЕ УТКА
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(Выборка.ВидЗапасов) Тогда
				...
				Если (Выборка.ОрганизацияОстатка = Организация
						//++ НЕ УТКА
						Или Выборка.ОрганизацияОстатка = ОрганизацияДавалец
						//-- НЕ УТКА
						Или Ложь)
					И (ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение = Неопределено
						Или (ВРег(ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение) = ВРег("ПоНастройкамДоговора")
							...
							//++ НЕ УТ
							Или ВРег(ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение) = ВРег("ПоНастройкамДоговораПроизводство")
								И Выборка.ПорядокОформленияСписанияВПроизводствоТоваровПринятыхНаХранение =
									Перечисления.ПорядокОформленияСписанияТоваровПринятыхНаХранение.ОформлятьСписание
							//-- НЕ УТ
				...
				//++ НЕ УТКА
				ИначеЕсли Выборка.ОрганизацияОстатка = ОрганизацияДавалец
					И ЗначениеЗаполнено(Выборка.ВладелецТовара) Тогда
			...
			ИначеЕсли (КоличествоТовара > 0
				...
				Иначе
					ТекстИсключения = НСтр("ru = 'Ненулевое количество при пустом виде запасов.';
											|en = 'Non-zero quantity with empty inventory owner attributes.'");
				...
				Если Выборка.ВидЗапасов <> ВидЗапасов Тогда
					...
					//++ НЕ УТКА
					Если Выборка.ОрганизацияОстатка = ОрганизацияДавалец Тогда
						ДвижениеРезерваПродавца.Организация		= ОрганизацияДавалец;
					Иначе
					//-- НЕ УТКА
			...
			Если ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию
				...
				Если Не ПараметрыЗаполненияВидовЗапасов.ВладелецТовараВШапке
					...
					ТекстИсключения = НСтр("ru = 'Установлен флаг получения владельца товаров из таблицы
												...
												|en = 'Check box of receiving goods owner from the inventory owner attribute table
...
Процедура ДобавитьВТаблицуСторноРезервовСторноПоРасходу(ДокументОбъект,
	...
	Если ГлубинаРекурсии > 100 Тогда
		ТекстИсключения = НСтр("ru = 'Превышена максимальная глубина рекурсии при формировании сторно резервов по расходу. Сохраните текущее состояние информационной базы и передайте ее разработчикам программы.';
								|en = 'The maximum recursion depth is exceeded while generating expense reserve storno. Save the current infobase state and pass it to the application developers.'");
		...
		Для Каждого СтрокаТаблицы Из СтрокиСПустымВидомЗапасовКомитента Цикл
			...
			Если СтрокаТаблицы.Количество <> 0 Тогда
				ТекстСообщения = НСтр("ru = 'По комитенту %Комитент% не хватает %Количество% %ЕдиницаИзмерения% товара %ПредставлениеНоменклатуры%, реализованного комиссионером %Комиссионер%';
										|en = 'By the %Комитент% consignor, not enough %Количество% %ЕдиницаИзмерения% of the %ПредставлениеНоменклатуры% goods sold by the %Комиссионер% consignee'");
			...
			Если ЭтоОтчетОПродажах
				...
				ТекстСообщения = НСтр("ru = 'Товар: %ПредставлениеНоменклатуры% Сумма продажи превышает сумму реализованного товара комиссионером на %Сумма% %Валюта%';
										|en = 'Goods: %ПредставлениеНоменклатуры% The sale amount exceeds the amount of goods sold by the consignee by %Сумма% %Валюта%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения,
											"%ПредставлениеНоменклатуры%",
											НоменклатураКлиентСервер.ПредставлениеНоменклатуры(
												РеквизитыАналитики.Номенклатура,
...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатковКомиссионныхТоваровИнтеркампани(ДокументОбъект,
	...
	Если ТаблицаОшибок.Количество() > 0
		...
		ТекстСообщения = НСтр("ru = 'Документ %Ссылка% проведен с выключенным контролем остатков товаров организаций.';
								|en = 'The %Ссылка% document is posted with enabled company stock balance control.'",
...
Функция ЗапросРаспределенияСУчетомПриоритетовТаблицыТоваровНаТаблицуОстатков(МенеджерВременныхТаблиц,
	...
	|	ТаблицаДокумента.Серия							КАК Серия,
	//++ НЕ УТКА
	|	ТаблицаДокумента.ОрганизацияДавалец				КАК ОрганизацияДавалец,
	//-- НЕ УТКА
	|	ТаблицаДокумента.Количество						КАК Количество,
	|	ВЫРАЗИТЬ(ТаблицаДокумента.КоличествоПоРНПТ КАК ЧИСЛО(23, 11))	КАК КоличествоПоРНПТ,
	|	ТаблицаДокумента.ВидЦены						КАК ВидЦены,
	...
	//++ НЕ УТ
	|	ОстаткиСгруппированные.ПорядокОформленияСписанияВПроизводствоТоваровПринятыхНаХранение КАК ПорядокОформленияСписанияВПроизводствоТоваровПринятыхНаХранение,
	//-- НЕ УТ
	...
	|				ТОГДА 1
	//++ НЕ УТКА
	|			КОГДА Остатки.Организация В(ВЫБРАТЬ ОрганизацииДавальца.ОрганизацияДавалец ИЗ ОрганизацииДавальца КАК ОрганизацииДавальца)
	...
	//++ НЕ УТ
	|		ВидыЗапасов.Договор.ПорядокОформленияСписанияВПроизводствоТоваровПринятыхНаХранение КАК ПорядокОформленияСписанияВПроизводствоТоваровПринятыхНаХранение,
	//-- НЕ УТ
	|		Остатки.НомерГТД							КАК НомерГТД,
	|		Остатки.ДатаПоступления						КАК ДатаПоступления,
	...
	//++ НЕ УТ
	|	ОстаткиСгруппированные.ПорядокОформленияСписанияВПроизводствоТоваровПринятыхНаХранение,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	ЕСТЬNULL(Остатки.ПорядокОформленияСписанияВПроизводствоТоваровПринятыхНаХранение, Неопределено) КАК ПорядокОформленияСписанияВПроизводствоТоваровПринятыхНаХранение,
	//-- НЕ УТ
	|	ЕСТЬNULL(Остатки.НомерГТД, Неопределено)			КАК НомерГТД,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ВестиУчетПоГТД
	...
	|
	|///////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ОрганизацииДавальца
	//-- НЕ УТКА
	|;
	|
	|///////////////////////////////////////////////////////////////////////////////
		...
		Если Не ОбщегоНазначенияУТКлиентСервер.СтруктурыРавны(СтруктураПоиска, ВыборкаОстатков) Тогда
			...
			Если НовыйРезервВсегоНеРаспределено <> 0 Тогда
				ТекстИсключения = НСтр("ru = 'Ошибка распределения резервов по месяцам';
										|en = 'An error occurred when allocating reserves by months'");
	...
	Если ПараметрыЗаполненияВидовЗапасов.СообщатьОбОшибкахЗаполнения
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
			...
			ТекстСообщения = НСтр("ru = 'По организации %Организация% %МестоХраненияНаименование% %Склад% не хватает %Количество% %ЕдиницаИзмерения% товара %ПредставлениеНоменклатуры% с указанными номерами ГТД по налогообложению %НалогообложениеНДС%';
									|en = '%Количество% %ЕдиницаИзмерения% of the %ПредставлениеНоменклатуры% goods items with specified CCD numbers by %НалогообложениеНДС% taxation are missing %МестоХраненияНаименование% %Склад% of the %Организация% company'");
			...
			Если Не СтрокаТаблицы.НеУказанНомерГТД
				...
				ТекстСообщения = НСтр("ru = 'По организации %Организация% %МестоХраненияНаименование% %Склад% не хватает %Количество% %ЕдиницаИзмерения% товара %ПредставлениеНоменклатуры%';
										|en = '%Количество% %ЕдиницаИзмерения% of the %ПредставлениеНоменклатуры% goods items are missing %МестоХраненияНаименование% %Склад% of the %Организация% company'");
			...
			ИначеЕсли Не СтрокаТаблицы.НеУказанНомерГТД Тогда
				ТекстСообщения = НСтр("ru = 'По организации %Организация% %МестоХраненияНаименование% %Склад% не хватает %Количество% %ЕдиницаИзмерения% товара %ПредставлениеНоменклатуры% по налогообложению %НалогообложениеНДС%';
										|en = '%Количество% %ЕдиницаИзмерения% of the %ПредставлениеНоменклатуры% goods items by %НалогообложениеНДС% taxation are missing %МестоХраненияНаименование% %Склад% of the %Организация% company'");
			ИначеЕсли Не КонтролироватьНалогообложениеНДС Тогда
				ТекстСообщения = НСтр("ru = 'По организации %Организация% %МестоХраненияНаименование% %Склад% не хватает %Количество% %ЕдиницаИзмерения% товара %ПредставлениеНоменклатуры% с указанными номерами ГТД';
										|en = '%Количество% %ЕдиницаИзмерения% of the %ПредставлениеНоменклатуры% goods items with specified CCD numbers are missing %МестоХраненияНаименование% %Склад% of the %Организация% company'");
			КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Склад)
				...
				ТекстСообщения = НСтр("ru = 'По организации %Организация% не хватает %ТипДвижения% %Количество% %ЕдиницаИзмерения% на сумму %Сумма%%Валюта% товара: %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД%';
										|en = '%ТипДвижения% %Количество% %ЕдиницаИзмерения% of goods in the amount of %Сумма%%Валюта% are missing in the %Организация% company: %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД%'");
			ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицы.Склад) Тогда
				ТекстСообщения = НСтр("ru = 'По организации %Организация% не хватает %ТипДвижения% %Количество% %ЕдиницаИзмерения% товара: %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД%';
										|en = '%ТипДвижения% %Количество% %ЕдиницаИзмерения% of goods are missing in the %Организация% company: %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД%'");
			ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицы.Сумма)
				...
				ТекстСообщения = НСтр("ru = 'По организации %Организация% не хватает %ТипДвижения% %Количество% %ЕдиницаИзмерения% на сумму %Сумма%%Валюта% товара: %ПредставлениеНоменклатуры% %КонтрольГТД%';
										|en = '%ТипДвижения% %Количество% %ЕдиницаИзмерения% of goods in the amount of %Сумма%%Валюта% are missing in the %Организация% company: %ПредставлениеНоменклатуры% %КонтрольГТД%'");
			Иначе
				ТекстСообщения = НСтр("ru = 'По организации %Организация% не хватает %ТипДвижения% %Количество% %ЕдиницаИзмерения% товара: %ПредставлениеНоменклатуры% %КонтрольГТД%';
										|en = '%ТипДвижения% %Количество% %ЕдиницаИзмерения% of goods are missing in the %Организация% company: %ПредставлениеНоменклатуры% %КонтрольГТД%'");
			КонецЕсли;	
...
Процедура СообщитьОКонтролеНалогообложенияПриПодбореВидовЗапасов(Знач ДокументОбъект, Знач ПараметрыЗаполненияВидовЗапасов)
	...
	Если КонтролироватьНалогообложениеНДС(ПараметрыЗаполненияВидовЗапасов) Тогда
		...
		Если НалогообложениеНДСДокумента <> Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД
			...
			|en = 'Note that the ""%НалогообложениеНДС%"" taxation type is not supported by company policy. In this case, only consignment goods can be written off.'");
...
Функция НаименованиеМестаХраненияДляСообщенияОбОшибкахКонтроляОстатковПоОрганизациям(СтрокаТаблицы, ХозяйственнаяОперация = Неопределено)
	...
	Иначе
		ТекстИсключения = НСтр("ru = 'В процедуре %1 не учтен тип места хранения ""%2""';
								|en = 'The ""%2"" storage location type is not recorded in the %1 procedure'");
...
Процедура СформироватьВТТаблицаОстатков(ИмяТаблицыОстатков,
	...
	Если ИмяТаблицыОстатков = "ТоварыОрганизаций" Тогда
		Если ВычитатьДвиженияПоТоварамОрганизаций = Неопределено Тогда
			ТекстИсключения = НСтр("ru = 'Ошибка формирования таблицы остатков товаров организаций: не передан параметр %1.';
									|en = 'An error occurred while generating company stock balance: the %1 parameter is not transferred.'");
	...
	ИначеЕсли ИмяТаблицыОстатков = "ВозвратТоваровПереданныхПереработчику" Тогда
		ТаблицаОстатковДляВозвратаПереданныхПереработчикуТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов, ДатаОстатков);
	//-- Устарело_Переработка24
	...
	Иначе
		ТекстИсключения = НСтр("ru = 'Неизвестное имя таблицы остатков.';
								|en = 'Unknown name of the remaining quantity table.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
...
Процедура ТаблицаОстатковРеализованныхТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|ИЗ
	|	Документ.ОтчетДавальцу2_5.ВидыЗапасов КАК ТЧВидыЗапасов
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаДанныхДокумента КАК ТаблицаДанныхДокумента
	|	ПО ТЧВидыЗапасов.Ссылка.Организация = ТаблицаДанныхДокумента.Организация
	|	
...
//
Процедура ТаблицаОстатковДляВозвратаПереданныхПереработчикуТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов, ДатаОстатков)
	...
	|
	|ГДЕ
	|	ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковМатериаловПереданныхПереработчикам)
	...
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(ПереданныеПереработчикамТовары.КоличествоПоРНПТ КАК ЧИСЛО(23, 11))
	|	КОНЕЦ КАК КоличествоПоРНПТ,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаРеализации
	|ИЗ
	...
	|	НЕОПРЕДЕЛЕНО КАК ДокументРеализации,
	|	Аналитика.КлючАналитики КАК АналитикаУчетаНоменклатуры,
	|	ВводОстатковПереданныхПереработчикамТоваров.ВидЗапасов КАК ВидЗапасов,
	|	ВводОстатковПереданныхПереработчикамТоваров.НомерГТД КАК НомерГТД,
	|	ВводОстатковПереданныхПереработчикамТоваров.Количество КАК Количество,
	|	ВЫБОР
	|		КОГДА НЕ &ИспользоватьУчетПрослеживаемыхИмпортныхТоваров
	|				ИЛИ НАЧАЛОПЕРИОДА(ВводОстатковПереданныхПереработчикамТоваров.Ссылка.Дата, МЕСЯЦ) < &ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(ВводОстатковПереданныхПереработчикамТоваров.КоличествоПоРНПТ КАК ЧИСЛО(23, 11))
	|	КОНЕЦ КАК КоличествоПоРНПТ,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаРеализации
	|ИЗ
	|	Документ.ВводОстатковТоваров.Товары КАК ВводОстатковПереданныхПереработчикамТоваров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДокументыВводаОстатков КАК ДокументыВводаОстатков
	|		ПО ВводОстатковПереданныхПереработчикамТоваров.Ссылка = ДокументыВводаОстатков.Ссылка
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК Аналитика
	|		ПО ВводОстатковПереданныхПереработчикамТоваров.АналитикаУчетаНоменклатуры.Номенклатура = Аналитика.Номенклатура
	|			И ВводОстатковПереданныхПереработчикамТоваров.АналитикаУчетаНоменклатуры.Характеристика = Аналитика.Характеристика
	|			И Аналитика.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|			И Аналитика.МестоХранения = &Партнер
	|			И Аналитика.Назначение = Аналитика.Назначение
	|			И Аналитика.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	...
	|	ВЫБОР
	|		КОГДА НЕ &ИспользоватьУчетПрослеживаемыхИмпортныхТоваров
	|				ИЛИ НАЧАЛОПЕРИОДА(Остатки.ДокументПередачи.Дата, МЕСЯЦ) < &ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(Остатки.КоличествоПоРНПТОстаток КАК ЧИСЛО(23, 11))
	|	КОНЕЦ КАК КоличествоПоРНПТ,
	...
	|	ВЫБОР
	|		КОГДА НЕ &ИспользоватьУчетПрослеживаемыхИмпортныхТоваров
	|				ИЛИ НАЧАЛОПЕРИОДА(Движения.ДокументПередачи.Дата, МЕСЯЦ) < &ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(Движения.КоличествоПоРНПТ КАК ЧИСЛО(23, 11))
	|	КОНЕЦ КАК КоличествоПоРНПТ,
	...
	ТаблицаОстаткиПоГТД = Запрос.ВыполнитьПакет()[4].Выгрузить(); // ТаблицаЗначений
	...
	|	ВЫРАЗИТЬ(Таблица.КоличествоПоРНПТОстаток КАК ЧИСЛО(23, 11))	КАК КоличествоПоРНПТОстаток
	...
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ТаблицаНомеровГТД.РаспределеноПоРНПТ КАК ЧИСЛО(23, 11)), 0) КАК КоличествоПоРНПТОстаток,
	|	0 КАК СуммаОстаток
	|ПОМЕСТИТЬ ТаблицаОстатков
	|ИЗ
	...
	|	ВЫРАЗИТЬ(ТаблицаНомеровГТД.КоличествоПоРНПТОстаток КАК ЧИСЛО(23, 11)) КАК КоличествоПоРНПТОстаток,
	|	0 КАК СуммаОстаток
	|
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		Если Выборка.Период > ДобавитьМесяц(ТекущаяДатаСеанса(), 12) Тогда
			...
			ТекстИсключения = НСтр("ru = 'Документ %Регистратор% проведен по регистру %Регистр1% с датой, которая больше текущей на 12 месяцев. Отмените проведение этого документа и исправьте дату.';
									|en = '%Регистратор% document is posted in the %Регистр1% register with a date that exceeds the current date by 12 months. Cancel the posting of this document and correct the date.'");
			ТекстИсключения = СтрЗаменить(ТекстИсключения, "%Регистр1%", "ТоварыОрганизаций");
...
Функция ОписаниеТаблицСОбязательнымиКолонками()
	...
	//++ НЕ УТКА
	ОписаниеТаблиц.ТаблицаТоваров.Вставить("ОрганизацияДавалец", "ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)");
	//-- НЕ УТКА
...
Процедура ПередУдалениемРегистратораПроверитьНаличиеЗаписейВРезервахТоваровОрганизаций(Источник, Отказ) Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		...
		ТекстИсключения = НСтр("ru = 'По документу %Документ% есть движения в регистре %Регистр1%. Удаление документа невозможно.';
								|en = 'The %Документ% contains records in the %Регистр1% register. Cannot remove the document.'");
...
Функция ТекстЗапросаОформленияПоРезервамТоваровОрганизаций(Запрос,
	...
	|				И РезервыТоваровОрганизаций.КорОрганизация = РезервыТоваровОрганизаций.Организация
	|			ТОГДА ВЫБОР
	|					КОГДА РезервыТоваровОрганизаций.АналитикаУчетаНоменклатуры.ТипМестаХранения В (
	...
	|						ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыкупТоваровДавальца)
	|					ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПустаяСсылка)
	|				  КОНЕЦ
	//-- НЕ УТКА
	...
	Если Не Запрос.Параметры.Свойство("ТипыЗапасовВыкупа")
	 Или Не ЗначениеЗаполнено(Запрос.Параметры.ТипыЗапасовВыкупа) Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья2_5") Тогда
			ТипыЗапасовВыкупа.Добавить(Перечисления.ТипыЗапасов.МатериалДавальца);
			ТипыЗапасовВыкупа.Добавить(Перечисления.ТипыЗапасов.ПолуфабрикатДавальца);
			ТипыЗапасовВыкупа.Добавить(Перечисления.ТипыЗапасов.ПродукцияДавальца);
		КонецЕсли;
		//-- НЕ УТКА
