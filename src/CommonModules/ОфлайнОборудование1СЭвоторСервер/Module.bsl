...
Процедура ВыполнитьЗагрузкуТоваров(Настройки, Товары, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Загрузка товаров из ККТ.';
							|en = 'Import of goods from the cash register.'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
...
Процедура ВыполнитьВыгрузкуТоваров(Настройки, Товары, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Выгрузка товаров на ККТ.';
							|en = 'Export of goods to the cash register.'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
...
Процедура ВыполнитьОчисткуТоваров(Настройки, Товары, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Очистка товаров на ККТ.';
							|en = 'Clear goods in the cash register.'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
...
Процедура ВыполнитьЗагрузкуСпискаДокументов(Настройки, КассовыеДокументы, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Загрузка кассовых документов на ККТ.';
							|en = 'Import of cash documents to cash register.'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
...
Процедура ВыполнитьЗагрузкуМагазинов(Настройки, СписокМагазинов, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Загрузка магазинов Эвотор.';
							|en = 'Import Evotor stores.'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
...
Функция HTTPСоединение(Настройки)
	...
	Исключение
		СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Ошибка при создании HTTP-соединения с сервером Эвотор';
											|en = 'An error occurred when establishing the HTTP connection to the Evotor server'"));
...
Процедура ВыполнитьЗапросКСерверуЭвотор(Соединение, Настройки, Данные, Отказ, СообщениеОбОшибке)
	...
	Исключение
		...
		Отказ = Истина;
	#Если Не МобильноеПриложениеСервер Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'НТТР запрос к облаку Эвотор.';
										|en = 'НТТР request to the Evotor cloud.'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, СообщениеОбОшибке);
		Возврат;
	#КонецЕсли
	КонецПопытки;
...
Процедура ОбработатьОтветСервера(Результат, Настройки, Отказ, СообщениеОбОшибке)
	...
	Если Результат.КодСостояния = 401 Тогда
		СообщениеОбОшибке = НСтр("ru = 'Ключ авторизации неверный.';
								|en = 'Authorization key is incorrect.'");
		Отказ = Истина;
	...
	ИначеЕсли Результат.КодСостояния <> 200 Тогда
		СообщениеОбОшибке = НСтр("ru = 'Произошла ошибка при выгрузке данных. Код ответа: %КодСостояния%.';
								|en = 'An error occurred when exporting data. Response code: %КодСостояния%.'");
	...
	Если Отказ Тогда
	#Если Не МобильноеПриложениеСервер Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ответ сервера Эвотор.';
										|en = 'Evotor server response.'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, СообщениеОбОшибке);
	#КонецЕсли
	КонецЕсли;
...
Функция ЗаполнитьНалогиТовара(СтруктураДокумента, Товар)
	...
	ИначеЕсли НалогиТовара.type = "VAT_5" Тогда
		НалогиТовара.tax_percent = "5"	
	ИначеЕсли НалогиТовара.type = "VAT_5_105" Тогда
		НалогиТовара.tax_percent = "5/105"
	ИначеЕсли НалогиТовара.type = "VAT_7" Тогда
		НалогиТовара.tax_percent = "7"
	ИначеЕсли НалогиТовара.type = "VAT_7_107" Тогда
		НалогиТовара.tax_percent = "7/107"
	КонецЕсли;
...
Функция ПреобразоватьВСтрокуИзJSON(Строка, Отказ, СообщениеОбОшибке)
	...
	Исключение
		...
		Отказ = Истина;
	#Если Не МобильноеПриложениеСервер Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Чтение JSON.';
										|en = 'Read JSON.'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()),
...
Функция СтрокаJSON(Знач ПараметрыМетода, Отказ, СообщениеОбОшибке)
	...
	Исключение
		...
		Отказ = Истина;
	#Если Не МобильноеПриложениеСервер Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Запись JSON.';
										|en = 'JSON record.'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	#КонецЕсли
	КонецПопытки;
