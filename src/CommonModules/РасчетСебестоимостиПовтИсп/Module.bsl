...
Функция ПравилаЗаполненияПоляТипЗаписи(ИмяДокументаДляОтбора = "") Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		...
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.ВозвратПрошлыхПериодов;
		НовоеПравило.ДокументИсточникВРасходе = Истина;
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		...
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Потребление;
		
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Потребление;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.Перемещение;
		...
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцуСписаниеНаРасходы;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Потребление;
		...
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцуКорректировкаПрошлогоПериода;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.ВозвратПрошлыхПериодов;
		//-- НЕ УТКА
	...
	ТекущийДокумент = Метаданные.Документы.ОтчетДавальцу2_5.Имя;
	...
	ТекущийДокумент = Метаданные.Документы.ОтчетДавальцуМеждуОрганизациями.Имя;
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцуМеждуОрганизациями;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.Партия;
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		...
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтражениеПлановойСтоимостиВыпуска;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.Партия;
	...
	//++ НЕ УТ
	#Область ОтчетПереработчика2_5
	
	ТекущийДокумент = Метаданные.Документы.ОтчетПереработчика2_5.Имя;
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДляОтбора) ИЛИ НРег(ИмяДокументаДляОтбора) = НРег(ТекущийДокумент) Тогда
		...
		НовоеПравило = ТаблицаПравил.Добавить();
		НовоеПравило.ИмяДокумента = ТекущийДокумент;
		НовоеПравило.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаПродукцииИзПроизводстваФиксированнаяСтоимость;
		НовоеПравило.ТипЗаписиПриход = Перечисления.ТипыЗаписейПартий.Партия;
		НовоеПравило.ТипЗаписиРасход = Перечисления.ТипыЗаписейПартий.Дополнение;
...
Функция РаспределятьПостатейныеЗатратыРекурсивно() Экспорт
	Возврат Ложь; // временно не используется, требуется оптимизация алгоритма
КонецФункции
...
Функция ЕстьРасчетныеДвиженияДокумента(ИмяДокумента, ИмяРегистра) Экспорт
	...
	//++ НЕ УТКА
	Если МетаданныеДокумента = Метаданные.Документы.ЭтапПроизводства2_2 Тогда
		Возврат Истина;
	КонецЕсли;
	//-- НЕ УТКА
