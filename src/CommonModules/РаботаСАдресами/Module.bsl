...
Функция РегионАдресаКонтактнойИнформации(Знач Адрес) Экспорт
	...
	ИначеЕсли ТипЗнч(Адрес) <> Тип("Структура") Тогда
		...
		ВызватьИсключение НСтр("ru = 'Невозможно определить субъект РФ, ожидается адрес.';
								|en = 'Cannot recognize territorial entity. Address expected.'");
...
Функция ГородАдресаКонтактнойИнформации(Знач Адрес) Экспорт
	...
	ИначеЕсли ТипЗнч(Адрес) <> Тип("Структура") Тогда
		...
		ВызватьИсключение НСтр("ru = 'Невозможно определить город, ожидается адрес.';
								|en = 'Cannot recognize city. Address expected.'");
...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	...
	Проверка.Наименование                 = НСтр("ru = 'Исправление адресов содержащих устаревший тип здания Литер.';
												|en = 'Correct addresses with the obsolete building type Liter.'");
	Проверка.Причины                      = НСтр("ru = 'В октябре 2019 г. в федеральной информационной адресной системе тип здания ""Литер"" был переименован в ""Литера"".';
												|en = 'In October 2019, it was renamed to Letter in the Federal Information Address System.'");
	Проверка.Рекомендация                 = НСтр("ru = 'Исправить в адресе Литер на Литера (для этого нажать ссылку ниже).
		...
		|en = 'To correct an address, click the link below.
		|At the same time, legally relevant addresses must be left unchanged (address of the Unified State Register of Legal Entities, place of registration, birth, and so on.)
		|
		|If you work in a distributed infobase (DIB), start corrections in the master node only.
		|Then synchronize it with subordinate nodes.'");
	...
	Проверка.Наименование                 = НСтр("ru = 'Выявление устаревших адресов контактной информации';
												|en = 'Identifying obsolete contact information addresses'");
	Проверка.Причины                      = НСтр("ru = 'Переподчинение адресных объектов (улиц, населенных пунктов), их переименование или упразднение влечет за собой внесение изменений в 
		...
		|en = 'Reassignment of address objects (streets, localities), their renaming or abolition results in making amendments to the 
		|State Address Register. After that, previously entered addresses cease to be tested for compliance with the address classifier.'");
	Проверка.Рекомендация                 = НСтр("ru = 'Для автоматической замены устаревших адресов на актуальные нажать ссылку ниже.
		|При этом следует оставлять без изменений юридически значимые адреса (адрес ЕГРЮЛ, место регистрации, рождения и др.).
		|
		|Если работа ведется в распределенной информационной базе (РИБ), то исправление следует запускать только в главном узле.
		|Затем выполнить синхронизацию с подчиненными узлами.';
		|en = 'To automatically replace obsolete addresses with relevant ones, click the link below.
		|At the same time, legally relevant addresses must be left unchanged (address of the Unified State Register of Legal Entities, place of registration, birth, and so on.)
		|
		|If you work in a distributed infobase (DIB), start corrections in the master node only.
		|Then synchronize it with subordinate nodes.'");
	...
	Проверка.Наименование                 = НСтр("ru = 'Проверка адресов на соответствие адресному классификатору.';
												|en = 'Check addresses for compliance with the address classifier.'");
	Проверка.Причины                      = НСтр("ru = 'Адрес контактной информации введен некорректно, устарел, был некорректно загружен из других источников или еще не внесен в Федеральную информационную адресную систему.';
												|en = 'The contact information address was entered incorrectly, is obsolete, was incorrectly imported from other sources, or has not yet been entered into the Federal Information Address System.'");
	Рекомендация                          = НСтр("ru = 'В зависимости от ситуации следует выбрать один из вариантов:
		...
		|Затем выполнить синхронизацию с подчиненными узлами.';
		|en = 'Choose a suitable option:
		|•	Validate the address on the FTS website: %1
		|To report invalid addresses, contact the local authorities or the FTS online support.
		|•	If the address is outdated due to a renaming, reassignment, or removal, enter the up-to-date address.
		|•	If you couldn''t autofill the address, open the form and enter it manually.
		|
		|If an address classifier is added:
		|•	Update it to the latest version from the ""Administration"" menu.
		|•	Import the address change history to automatically find and fix outdated addresses.
		|Quick link: %2
		|
		|If you''re using a distributed infobase, make changes only in the master node.
...
Процедура ЗаполнитьМаскиНомераТелефона(СписокВыбора) Экспорт
	...
	МаскаНомера = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '+7(999) 999-99-99, <span style=""color: %1""> (Пример: +7(910) 123-23-23)</span>';
						|en = '+7(999) 999-99-99, <span style=""color: %1""> (Example: +7(910) 123-23-23)</span>'"),
					"ПоясняющийТекст");
	...
	МаскаНомера = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '##(99999) 999-99-99, <span style=""color: %1""> (Пример: 8(4822) 12-34-56)</span>';
						|en = '##(99999) 999-99-99, <span style=""color: %1""> (Example: 8(4822) 12-34-56)</span>'"),
					"ПоясняющийТекст");
	...
	МаскаНомера = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '999999999, <span style=""color: %1""> (Пример: 9001234567)</span>';
						|en = '999999999, <span style=""color: %1""> (Example: 9001234567)</span>'"),
					"ПоясняющийТекст");
...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	Элемент.Код                       = "643";
	Элемент.Наименование              = НСтр("ru = 'РОССИЯ';
											|en = 'RUSSIA'", ОбщегоНазначения.КодОсновногоЯзыка());
	Элемент.НаименованиеПолное        = НСтр("ru = 'Российская Федерация';
											|en = 'Russian Federation'", ОбщегоНазначения.КодОсновногоЯзыка());
	Элемент.КодАльфа2                 = "RU";
	Элемент.КодАльфа3                 = "RUS";
	Элемент.УчастникЕАЭС              = Истина;
	Элемент.МеждународноеНаименование = НСтр("ru = 'The Russian Federation';
											|en = 'The Russian Federation'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОбработкаЧастыхСокращенийВАдресах(Знач ДанныеАнализа) Экспорт
	...
	СубъектРФ.Вставить(НСтр("ru = 'САНКТ-ПЕТЕРБУРГ';
							|en = 'SAINT PETERSBURG'"), "Санкт-Петербург");
	СубъектРФ.Вставить(НСтр("ru = 'МОСКВА';
							|en = 'MOSCOW'"), "Москва");
	СубъектРФ.Вставить(НСтр("ru = 'МСК';
							|en = 'MSK'"), "Москва");
	СубъектРФ.Вставить(НСтр("ru = 'СПБ';
							|en = 'SPB'"), "Санкт-Петербург");
...
Процедура ПроверитьАдресаНаТипЗданияЛитер(Проверка, ПараметрыПроверки) Экспорт
	...
	Пока РезультатЗапроса.Следующий() Цикл
		...
		Если Проверки[РезультатЗапроса.Вид] = Неопределено Тогда
			...
			ПараметрыВыполненияПроверки = МодульКонтрольВеденияУчета.ПараметрыВыполненияПроверки(НСтр("ru = 'Устаревший тип здания в адресах контактной информации';
																										|en = 'An obsolete building type in contact information addresses'", ОбщегоНазначения.КодОсновногоЯзыка()), РезультатЗапроса.Вид);
		...
		Проблема.УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В адресе ""%1"" используется устаревший тип здания Литер.';
				|en = 'The obsolete building type Liter is used in address ""%1"".'"), РезультатЗапроса.Представление);
...
Процедура ВыявитьУстаревшиеАдреса(Проверка, ПараметрыПроверки) Экспорт
	...
	Пока СтрокаСАдресом.Следующий() Цикл
		...
		Если Проверки[СтрокаСАдресом.Вид] = Неопределено Тогда
			...
			ПараметрыВыполненияПроверки = МодульКонтрольВеденияУчета.ПараметрыВыполненияПроверки(НСтр("ru = 'Адреса контактной информации';
																										|en = 'Contact information addresses'", ОбщегоНазначения.КодОсновногоЯзыка()), СтрокаСАдресом.Вид);
			МодульКонтрольВеденияУчета.ОчиститьРезультатыПредыдущихПроверок(Проверка, ПараметрыВыполненияПроверки);
			ВидПроверки = МодульКонтрольВеденияУчета.ВидПроверки(ПараметрыВыполненияПроверки);
...
Процедура ЗаменитьАдресНаАктуальный(ОбъектКИ, СтрокаВидаКИ, АдресПоПолям, СвойстваВида, ЕстьДействуетС, ЗаменаУстаревшего = Истина)
	...
	Если ЗаменаУстаревшего Тогда
		ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Адрес ""%1"" устарел и был переименован в ""%2""';
																						|en = 'Address ""%1"" is obsolete and it was renamed to ""%2""'"),
	...
	Иначе
		ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Адрес ""%1"" был изменен автоматически на ""%2""';
																						|en = 'The ""%1"" address was automatically changed to ""%2""'"),
			СтрокаВидаКИ.Представление, НовоеПредставление);
	КонецЕсли;
...
Процедура ОбработкаУстаревшегоАдресаПослеПроверки(Адреса, СоответствиеАдресаИСсылки, ПараметрыПроверки, Проверки)
	...
	Для каждого КлючЗначение Из Адреса Цикл
		...
		УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Адрес %1 устарел';
				|en = 'Address %1 is obsolete'"), ОписаниеАдреса.Представление);
...
Процедура ПроверитьАдресаНаСоответствиеАдресномуКлассификатору(Проверка, ПараметрыПроверки) Экспорт
	...
	Пока СтрокаСАдресом.Следующий() Цикл
		...
		Если Проверки[СтрокаСАдресом.ИмяОбъекта] = Неопределено Тогда
			...
			ПараметрыВыполненияПроверки = МодульКонтрольВеденияУчета.ПараметрыВыполненияПроверки(НСтр("ru = 'Адреса контактной информации';
																										|en = 'Contact information addresses'", ОбщегоНазначения.КодОсновногоЯзыка()), СтрокаСАдресом.ИмяОбъекта);
			МодульКонтрольВеденияУчета.ОчиститьРезультатыПредыдущихПроверок(Проверка, ПараметрыВыполненияПроверки);
			ВидПроверки = МодульКонтрольВеденияУчета.ВидПроверки(ПараметрыВыполненияПроверки);
			...
			Если ЗначениеЗаполнено(ОписаниеАдреса.Представление) Тогда
				УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1 ""%2"" не соответствует адресному классификатору, по причине: %3';
						|en = '%1 %2 does not comply with the address classifier due to: %3'"), 
			...
			Иначе
				УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1 не соответствует адресному классификатору, по причине: %2';
						|en = '%1 does not comply with the address classifier due to: %2'"), 
					Строка(ОписаниеАдреса.Вид), ПроверенныйАдрес.Ошибки[0].Текст);
			КонецЕсли;
	
		Иначе
			УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 ""%2"" не соответствует адресному классификатору';
					|en = '%1 %2 does not comply with the address classifier'"), Строка(ОписаниеАдреса.Вид), ОписаниеАдреса.Представление);
		КонецЕсли;
