...
Функция ДополнительнаяКолонкаПечатныхФормДокументов(КодЯзыка = Неопределено) Экспорт
	...
	Если ДопКолонка = Перечисления.ДополнительнаяКолонкаПечатныхФормДокументов.Артикул Тогда
		...
		Результат.ПредставлениеКолонки = НСтр("ru = 'Артикул';
												|en = 'Item ID'", КодЯзыка);
	ИначеЕсли ДопКолонка = Перечисления.ДополнительнаяКолонкаПечатныхФормДокументов.Код Тогда
		Результат.ИмяКолонки = "Код";
		Результат.ПредставлениеКолонки = НСтр("ru = 'Код';
												|en = 'Code'", КодЯзыка);
	КонецЕсли;
...
Функция ОписаниеОрганизации(СписокСведений, Список = "", СПрефиксом = Истина) Экспорт
	...
	ПрефиксыРеквизитов.Вставить("ИНН",			НСтр("ru = 'ИНН %1';
														|en = 'TIN %1'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ПрефиксыРеквизитов.Вставить("КПП",			НСтр("ru = 'КПП %1';
														|en = 'KPP %1'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПрефиксыРеквизитов.Вставить("Телефоны",		НСтр("ru = 'тел.: %1';
														|en = 'phone: %1'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ПрефиксыРеквизитов.Вставить("НомерСчета",	НСтр("ru = '%1 %2 в банке %3 %4 %5 %6 %7';
														|en = '%1 %2 in the bank %3 %4 %5 %6 %7'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ПрефиксыРеквизитов.Вставить("КодПоОКПО",	НСтр("ru = 'Код по ОКПО %1';
													|en = 'OKPO code %1'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ПрефиксыРеквизитов.Вставить("КодОКВЭД",		НСтр("ru = 'Код ОКВЭД %1';
														|en = 'OKVED code %1'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если ИспользоватьСкидки Тогда
		...
		Если ЕстьНаценки И ЕстьСкидки Тогда
			СтруктураШапки.Вставить("Скидка", НСтр("ru = 'Скидка (Наценка)';
													|en = 'Discount (markup)'", ОбщегоНазначения.КодОсновногоЯзыка()));
			СтруктураШапки.Вставить("СуммаСкидки", НСтр("ru = 'Сумма';
														|en = 'Amount'", ОбщегоНазначения.КодОсновногоЯзыка())
				+ Символы.ПС + НСтр("ru = 'без скидки (наценки)';
									|en = 'without discount (markups)'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		ИначеЕсли ЕстьНаценки И НЕ ЕстьСкидки Тогда
			СтруктураШапки.Вставить("Скидка", НСтр("ru = 'Наценка';
													|en = 'Markup'", ОбщегоНазначения.КодОсновногоЯзыка()));
			СтруктураШапки.Вставить("СуммаСкидки", НСтр("ru = 'Сумма';
														|en = 'Amount'", ОбщегоНазначения.КодОсновногоЯзыка())
				+ Символы.ПС + НСтр("ru = 'без наценки';
									|en = 'without markup'", ОбщегоНазначения.КодОсновногоЯзыка()));
			СтруктураШапки.Вставить("ТолькоНаценка", Истина);
		ИначеЕсли ЕстьСкидки Тогда
			СтруктураШапки.Вставить("Скидка", НСтр("ru = 'Скидка';
													|en = 'Discount'", ОбщегоНазначения.КодОсновногоЯзыка()));
			СтруктураШапки.Вставить("СуммаСкидки", НСтр("ru = 'Сумма';
														|en = 'Amount'", ОбщегоНазначения.КодОсновногоЯзыка())
				+ Символы.ПС + НСтр("ru = 'без скидки';
									|en = 'without discount'", ОбщегоНазначения.КодОсновногоЯзыка()));
			СтруктураШапки.Вставить("ТолькоНаценка", Ложь);
		КонецЕсли;
	КонецЕсли;
...
Функция ПроверитьЗаполнениеНоменклатуры(СтрокаТовары, НомерСтроки) Экспорт
	...
	Если Не ЗначениеЗаполнено(СтрокаТовары.Номенклатура) Тогда
		...
		ТекстОшибки = НСтр("ru = 'В строке %НомерСтроки% не заполнено значение номенклатуры - строка при печати пропущена.';
							|en = 'Item value is required in line %НомерСтроки%. Skipped the line when printing.'");
...
Функция ТекстНДСПоСтавке(СтавкаНДС, ЦенаВключаетНДС) Экспорт
	...
	ТекстНДСПоСтавке = ?(ЦенаВключаетНДС, НСтр("ru = 'В т.ч. НДС (%СтавкаНДС%):';
												|en = 'Including VAT (%СтавкаНДС%):'"), НСтр("ru = 'НДС (%СтавкаНДС%):';
																						|en = 'VAT (%СтавкаНДС%):'"));
...
// Заполняет объекты, интегрированные с подсистемой ФормированиеПечатныхФорм
//
// Параметры:
//  УправляющиеМодули - Соответствие Из КлючИЗначение - Управляющие модули объектов:
//   * Ключ     - Строка                        - Полное имя метаданных встраиваемого объекта
//   * Значение - ОбщийМодуль, ДокументМенеджер - Объект, в котором расположена логика интеграции 
//                                                с механизмом ФормированиеПечатныхФорм
//
// Комментарий:
//  Если управляющим модулем является модуль менеджера соответствующего документа, 
//  то значение соответствия можно не заполнять.
//  Если же управляющий модуль отличается от модуля менеджера документа, 
//  то в значениии следует указать ссылку на нужный модуль.
//
// Пример 1:
//  // Документ АктВыполненныхРабот интегрирован в механизм ФормированиеПечатныхФорм.
//  // Методы интеграции находятся в модуле менеджера документа АктВыполненныхРабот.
//  УправляющиеМодули.Вставить(Метаданные.Документы.АктВыполненныхРабот.ПолноеИмя());
//
// Пример 2:
//  // Документ КоммерческоеПредложениеКлиенту интегрирован в механизм ФормированиеПечатныхФорм.
//  // Методы интеграции находятся в модуле КоммерческоеПредложениеКлиентуУТ.
//  УправляющиеМодули.Вставить(Метаданные.Документы.КоммерческоеПредложениеКлиенту.ПолноеИмя(), 
//  	КоммерческоеПредложениеКлиентуУТ);
//
// В управляющем модуле будет вызван метод ЗаполнитьСтруктуруПолучателейПечатныхФорм.
Процедура ПриЗаполненииУправляющихМодулей(УправляющиеМодули) Экспорт

	УправляющиеМодули.Вставить(Метаданные.Справочники.Партнеры.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Справочники.Претензии.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.АктВыполненныхРабот.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.АктОРасхожденияхПослеОтгрузки.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.АктОРасхожденияхПослеПриемки.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ВзаимозачетЗадолженности.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ВозвратТоваровМеждуОрганизациями.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ВозвратТоваровОтКлиента.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ВозвратТоваровПоставщику.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.КоммерческоеПредложениеКлиенту.ПолноеИмя(), 
		КоммерческоеПредложениеКлиентуУТ);
	УправляющиеМодули.Вставить(Метаданные.Документы.ЗаказКлиента.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ЗаказПоставщику.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ЗаявкаНаВозвратТоваровОтКлиента.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.КорректировкаРеализации.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ОтгрузкаТоваровСХранения.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ВыкупПринятыхНаХранениеТоваров.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ПриемкаТоваровНаХранение.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ПоступлениеТоваровОтХранителя.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ПередачаТоваровХранителю.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ПорчаТоваровУХранителя.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ПересортицаТоваровУХранителя.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ОтчетКомиссионера.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ОтчетКомиссионераОСписании.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ОтчетКомитенту.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ОтчетКомитентуОСписании.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ОтчетПоКомиссииМеждуОрганизациями.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ОтчетОСписанииТоваровУХранителя.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ПередачаТоваровМеждуОрганизациями.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ПеремещениеТоваров.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ПриобретениеТоваровУслуг.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ПриходныйКассовыйОрдер.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.РасходныйКассовыйОрдер.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.РеализацияТоваровУслуг.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.РеализацияУслугПрочихАктивов.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.СверкаВзаиморасчетов.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.СверкаВзаиморасчетов2_5_11.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.СписаниеБезналичныхДенежныхСредств.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.СчетНаОплатуКлиенту.ПолноеИмя());
	УправляющиеМодули.Вставить(Метаданные.Документы.ОприходованиеИзлишковТоваровУХранителя.ПолноеИмя());
...
Процедура ЗаполнитьДанныеСтруктурыПолучателяПоТипуОбъекта(ПолноеИмяОбъектаМетаданных, СтруктураДанныхОбъектаПечати) Экспорт
	...
	УправляющийМодуль = ФормированиеПечатныхФормПовтИсп.УправляющиеМодули().Получить(ПолноеИмяОбъектаМетаданных);
	
	Если УправляющийМодуль = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	УправляющийМодуль.ЗаполнитьСтруктуруПолучателейПечатныхФорм(СтруктураДанныхОбъектаПечати);
