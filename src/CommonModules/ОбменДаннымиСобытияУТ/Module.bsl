...
Процедура ДокументСчетФактураВыданныйПередОбработкойПРО(Объект, Отказ, ИмяПланаОбмена) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.КорректировкаРеализации") Тогда 
			Если ОснованиеДокументаОснования <> Неопределено Тогда
				Если ТипЗнч(ОснованиеДокументаОснования) = Тип("ДокументСсылка.ПервичныйДокумент") 
					//++ НЕ УТКА
					ИЛИ ТипЗнч(ОснованиеДокументаОснования) = Тип("ДокументСсылка.ОтчетДавальцу2_5")
					//-- НЕ УТКА
					Тогда Отказ = Истина; 
					Возврат;
				КонецЕсли;	
		...
		ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.ВозвратТоваровОтКлиента") Тогда 
			...
			КонецЕсли;
			//++ НЕ УТКА
		ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.ОтчетДавальцуМеждуОрганизациями") Тогда 
			Отказ = Истина; 
			Возврат;
			//-- НЕ УТКА
		КонецЕсли;
...
Процедура ДокументСчетФактураПолученныйПередОбработкойПРО(Объект, Отказ) Экспорт
	...
	//-- Устарело_Переработка24
	ДопустимыеТипыДокументовОснований.Добавить(Тип("ДокументСсылка.ОтчетПереработчика2_5"));
	//-- НЕ УТ
...
Процедура ДокументПроизводствоБезЗаказаПередОбработкойПРО(Объект, Отказ) Экспорт 
	...
	Если НЕ Объект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоДокументу Тогда
		...
		|	И ПроизводствоБезЗаказаВыходныеИзделия.ВнутренняяПереработка
		|";
