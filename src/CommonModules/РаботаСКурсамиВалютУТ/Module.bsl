...
Функция ПолучитьКурсВалютыИзКэша(Знач Валюта, Знач ДатаКурса, КэшКурсов, Знач БазоваяВалюта = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Валюта) Тогда
		ТекстСообщения = НСтр("ru = 'Ошибка получения курса валют: передана пустая валюта';
								|en = 'An error occurred upon receiving the exchange rate: empty currency is submitted'");
	...
	Если СтруктураКурса.КурсЧислитель = 0 Или СтруктураКурса.КурсЗнаменатель = 0 Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Получение курса валют: обнаружен нулевой курс валюты %1 относительно базовой валюты %2.';
										|en = 'Get an exchange rate: a zero exchange rate %1 relative to base currency %2 was detected.'"), Валюта, БазоваяВалюта);
