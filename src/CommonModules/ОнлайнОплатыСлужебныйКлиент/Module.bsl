...
Процедура ОткрытьФормуПлатежнойСсылкиПродолжение(Знач ПодключенаИПП, Знач Параметры) Экспорт
	...
	НастройкаОнлайнОплаты = ОнлайнОплатыСлужебныйВызовСервера.НастройкаОнлайнОплатыОснованияПлатежа(
		Параметры.ОснованиеПлатежа);
	
	Если НастройкаОнлайнОплаты = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Не обнаружены настройки подключения к ЮKassa.';
									|en = 'YooMoney connection settings are not found.'"));
		Возврат;
	КонецЕсли;
	
	Если Не ОнлайнОплатыСлужебныйВызовСервера.ПроверитьЗаполнениеОснованияПлатежа(Параметры.ОснованиеПлатежа) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьВопросПодключенияИПП(Знач ОбработкаЗавершения = Неопределено) 
	...
	ТекстВопроса = НСтр("ru = 'Для использования функций взаимодействия с сервисом ЮKassa,
		...
		|Подключиться сейчас?';
		|en = 'To use functions of interaction with YooMoney service,
