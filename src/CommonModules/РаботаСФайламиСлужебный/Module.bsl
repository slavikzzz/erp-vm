...
Процедура ИзвлечьТекстИзФайлов() Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Извлечение текста';
									|en = 'Files.Extract text'", ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			Иначе
				...
				Если Файл.Существует() И ФайлБазы.Существует() Тогда
					Результат.ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Файлы ""%1"" и ""%2"" уже существуют.
							           ...
							           |Do you want to overwrite them?'"), ПутьКАрхивуСФайламиТомов, ПолноеИмяФайловойБазы);
				...
				ИначеЕсли Файл.Существует() Тогда
					Результат.ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Файл ""%1"" уже существует.
							           ...
							           |Do you want to overwrite it?'"), ПутьКАрхивуСФайламиТомов);
					Результат.ТребуетсяПодтверждение = Истина;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если Результат.ДанныеПодготовлены Тогда
			Если ФайлБазы.Существует() И НЕ Результат.ТребуетсяПодтверждение Тогда
				Результат.ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Файл ""%1"" уже существует.
						           |Заменить существующий файл?';
						           |en = 'File ""%1"" already exists.
						           |Do you want to overwrite it?'"), ПолноеИмяФайловойБазы);
				Результат.ТребуетсяПодтверждение = Истина;
			КонецЕсли;
...
Процедура ОчиститьНенужныеФайлы(Параметры = Неопределено, АдресРезультата = Неопределено) Экспорт
	...
	ЗаписатьВЖурналСобытийОчисткиФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Начата регламентная очистка ненужных файлов (%1).';
			|en = 'Scheduled cleanup of unwanted files is started (%1).'"), ОчищатьНенужныеФайлы));
	...
	ЗаписатьВЖурналСобытийОчисткиФайлов(НСтр("ru = 'Завершена регламентная очистка ненужных файлов.';
											|en = 'Scheduled cleanup of unwanted files is completed.'"));
...
Процедура ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов(ВозможныеПрава) Экспорт
	...
	Право.Подсказка     = НСтр("ru = 'Пометка удаления файлов в папке';
								|en = 'Mark files in a folder for deletion.'");
	// Права, требуемые для этого права.
	Право.ТребуемыеПрава.Добавить("ИзменениеФайлов");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Перенос информации о файлах в регистр сведений Сведения о файлах.';
									|en = 'Move file data to the ""File properties"" information register.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление признака наличия присоединенного файла без учета служебных файлов.';
									|en = 'Update the flag that indicates whether there is an attachment without taking into account service files.'");
...
Функция СобытиеЖурналаРегистрацииОчисткаФайлов()
	...
	Возврат НСтр("ru = 'Файлы.Очистка файлов';
				|en = 'Files.File cleanup'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если ОбработатьПолученныеФайлы Тогда
		...
		Если НовыйТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			...
			Если ДвоичныеДанные = Неопределено Тогда
				...
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Добавление файла в том';
												|en = 'Files.Add file to volume'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ИмяСправочникаХраненияФайлов(ВладелецФайлов, ИмяСправочника = "",
	...
	ШаблонПричиныОшибки = 
		НСтр("ru = 'У владельца файлов ""%1"" типа ""%2""
			...
			|en = 'File location ""%1"" of type ""%2""
			|does not have a main file storage catalog.'") + Символы.ПС;
...
Функция ИмяСправочникаХраненияВерсийФайлов(ВладелецФайлов, ИмяСправочника = "",
	...
	ШаблонПричиныОшибки = 
		НСтр("ru = 'У владельца версий файлов ""%1""
			...
			|en = 'The main version of the file storage catalog is not specified
			|for file owner ""%1"".'") + Символы.ПС;
...
Процедура СообщитьОбОшибкеФайлНеНайден(ФайлОбъект, ВызыватьИсключение) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Открытие файла';
									|en = 'Files.Open file'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция СобытиеЖурналаРегистрацииДляОбмена() 
	...
	Возврат НСтр("ru = 'Файлы.Не удалось отправить файл при обмене данными';
				|en = 'Files.Cannot send file during data exchange'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПодготовитьФайлКУдалению(ПрисоединенныйФайл)
	...
	ЗаписатьВЖурналСобытийОчисткиФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Удаляется ненужный файл ""%1""...';
			|en = 'Deleting unwanted file ""%1""...'"), ОбщегоНазначения.ПредметСтрокой(ПрисоединенныйФайл)),,
		ПрисоединенныйФайл);
	Блокировка = Новый БлокировкаДанных();
...
Функция ЗагрузитьФайлССервера(ПараметрыФайла, ЭтоФайл = Неопределено)
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'Не удалось загрузить файл ""%1"" из облачного сервиса по причине:';
								|en = 'Cannot upload file ""%1"" from the cloud service. Reason:'") + " " + Символы.ПС + РезультатЗагрузки.ТекстОшибки;
...
Функция СобытиеЖурналаРегистрацииСинхронизация()
	...
	Возврат НСтр("ru = 'Файлы.Синхронизация с облачным сервисом';
				|en = 'Files.Synchronization with cloud service'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Для Каждого ПараметрыФайла Из Подписи Цикл 
		...
		Иначе
			ТекстСообщения = НСтр("ru = 'Не удалось загрузить подпись ""%1"" из облачного сервиса по причине:';
									|en = 'Cannot upload signature ""%1"" from the cloud service. Reason:'") + " "
...
Функция СинхронизироватьФайлы(ТаблицаФайлов, ПараметрыСинхронизации, АдресСервера, Подписи, ЭтоУдалениеФайлов = Ложь)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
		...
		Исключение
			...
			СинхронизацияВыполнена = Ложь;
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось синхронизировать файл ""%1"" по причине:';
					|en = 'Cannot synchronize file ""%1"". Reason:'"), Строка(СтрокаТаблицы.ФайлСсылка))
...
Функция УдалитьФайлВОблачномСервисе(Знач ПараметрыСинхронизации, Знач СтрокаТаблицы)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ЗаписатьВЖурналСобытийСинхронизацииФайлов(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удален файл: %1';
																		|en = 'File is not deleted: %1'"), СтрокаТаблицы.Наименование), 
			ПараметрыСинхронизации.УчетнаяЗапись, УровеньЖурналаРегистрации.Ошибка);
		Возврат Ложь;
	...
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Удален файл: %1';
																	|en = 'File is deleted: %1'"), СтрокаТаблицы.Наименование), 
		ПараметрыСинхронизации.УчетнаяЗапись);
...
Процедура ВыполнитьПроверкуПодключения(УчетнаяЗапись, РезультатПроверки) Экспорт 
	...
	ТекстСобытия = НСтр("ru = 'Начата проверка синхронизации файлов';
						|en = 'File synchronization check started'") + " " + НаименованиеУчетнойЗаписи;
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации.УчетнаяЗапись);
	...
	ТекстСобытия = НСтр("ru = 'Завершена проверка синхронизации файлов';
						|en = 'File synchronization check completed'") + " " + НаименованиеУчетнойЗаписи;
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации.УчетнаяЗапись);
