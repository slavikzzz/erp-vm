...
Функция СообщениеЗапросаGLN(Организация, ИНН, КПП) Экспорт
	...
	Если ЗначениеЗаполнено(КПП) Тогда
		СообщениеXML.Описание = СтрШаблон(НСтр("ru = 'Получение GLN по ИНН %1, КПП %2';
												|en = 'Получение GLN по ИНН %1, КПП %2'"), ИНН, КПП);
	Иначе
		СообщениеXML.Описание = СтрШаблон(НСтр("ru = 'Получение GLN по ИНН %1';
												|en = 'Получение GLN по ИНН %1'"), ИНН);
	КонецЕсли;
...
Функция СообщенияКПередачеЗапросыСписковВходящихДокументовИКвитанцийXML() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СообщениеXML = ИнтеграцияГИСМКлиентСервер.СтруктураСообщенияXML();
		СообщениеXML.Описание = СтрШаблон(НСтр("ru = 'Получение входящих документов организации %1';
												|en = 'Получение входящих документов организации %1'"), Выборка.Организация);
	...
	Пока ДанныеДляПолученияКвитанцииИзОчереди.Следующий() Цикл
		...
		СообщениеXML.Описание = СтрШаблон(
			НСтр("ru = 'Получение квитанции по операции %1 документа %2 по организации %3';
				|en = 'Получение квитанции по операции %1 документа %2 по организации %3'"),
	...
	Для Каждого Сообщение Из Сообщения Цикл
		...
		Если Категории.ПолучениеСпискаВходящихДокументов.Получить(Сообщение.Операция) <> Неопределено Тогда
			...
			Для Каждого ДанныеДляПолученияДокумента Из РезультатПередачиИОбработки.РезультатОбработкиОтвета.Документы Цикл
				...
				СообщениеXML = ИнтеграцияГИСМКлиентСервер.СтруктураСообщенияXML();
				СообщениеXML.Описание = СтрШаблон(НСтр("ru = 'Получение документа ""%1"" по организации %2';
														|en = 'Получение документа ""%1"" по организации %2'"), ТипДокумента.Описание, Сообщение.Организация);
...
Функция ПередатьДанные(ИсходящееСообщение, СертификатыДляПодписанияНаСервере = Неопределено) Экспорт
	...
	Если ОбновитьСостояниеПодтверждения Тогда
		...
		Исключение
			...
			ВозвращаемоеЗначение.РезультатОбработкиОтвета.ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось заблокировать документ %1.';
																						|en = 'Не удалось заблокировать документ %1.'"), ДокументСсылка);
			Возврат ВозвращаемоеЗначение;
		КонецПопытки;
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'При загрузке квитанции о фиксации по документу %1 возникла ошибка:
				           |Текст ошибки: %2';
				           |en = 'При загрузке квитанции о фиксации по документу %1 возникла ошибка:
				           |Текст ошибки: %2'"),
				ДокументСсылка,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'Квитанция о фиксации не получена:
					       ...
					       |По причине:
					       |%2';
					       |en = 'Квитанция о фиксации не получена:
					       |Переданные данные: %1
					       |По причине:
					       |%2'"), ОписаниеОперации, РезультатОтправкиЗапроса.ТекстОшибки));
		Иначе
...
Функция ЗапросКвитанцииОФиксацииПоСсылкеXML(Ссылка, Операция) Экспорт
	...
	ВозвращаемоеЗначение.Вставить("Описание", СтрШаблон(НСтр("ru = 'Запрос квитанции о фиксации документа %1';
															|en = 'Запрос квитанции о фиксации документа %1'"), Ссылка));
	ВозвращаемоеЗначение.Вставить("СообщенияXML", Новый Массив);
...
Функция ПодготовитьСообщениеКПередаче(ТекстСообщенияXML, Реквизиты) Экспорт
	...
	Если ОбновитьСостояниеПодтверждения Тогда
		...
		Исключение
			// Выполнить блокировку объекта не удалось, операция будет выполнена в следующей итерации.
			ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось заблокировать документ %1.';
																|en = 'Не удалось заблокировать документ %1.'"), Реквизиты.Документ);
			Возврат ВозвращаемоеЗначение;
		КонецПопытки;
...
Функция ДобавитьЗаписьВПротоколОбмена(КонвертSOAP, Реквизиты) Экспорт
	...
	Если Реквизиты.ТипСообщения = Перечисления.ТипыСообщенийГИСМ.Исходящее Тогда
		ЗаголовокОшибки = СтрШаблон(НСтр("ru = 'Ошибка при проверке электронной подписи отправляемого в ГИСМ сообщения %1:';
										|en = 'Ошибка при проверке электронной подписи отправляемого в ГИСМ сообщения %1:'"), ПрисоединенныйФайл);
	ИначеЕсли Реквизиты.ТипСообщения = Перечисления.ТипыСообщенийГИСМ.Входящее Тогда
		ЗаголовокОшибки = СтрШаблон(НСтр("ru = 'Ошибка при проверке электронной подписи загруженного из ГИСМ сообщения %1:';
										|en = 'Ошибка при проверке электронной подписи загруженного из ГИСМ сообщения %1:'"), ПрисоединенныйФайл);
	КонецЕсли;
...
Функция ТекстУведомлениеОбОтгрузкеМаркированнойПродукции(Ссылка)
	...
	Если ПравоДобавления Тогда
		ТекстНадписи =  НСтр("ru = 'Создать уведомление об отгрузке ГИСМ';
							|en = 'Создать уведомление об отгрузке ГИСМ'");
	...
	Иначе
		ТекстНадписи = НСтр("ru = 'Уведомление об отгрузке ГИСМ не создано';
							|en = 'Уведомление об отгрузке ГИСМ не создано'");
		ИмяКоманды   = "";
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи = СтрШаблон(НСтр("ru = 'Уведомление об отгрузке ГИСМ: %1';
										|en = 'Уведомление об отгрузке ГИСМ: %1'"), Выборка.Статус);
...
Функция ТекстУведомлениеОСписанииКиЗ(Ссылка)
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи  = СтрШаблон(НСтр("ru = 'Уведомление о списании КиЗ ГИСМ: %1';
										|en = 'Уведомление о списании КиЗ ГИСМ: %1'"), Выборка.Статус);
		ИмяКоманды    = "#ГИСМ#ОткрытьПротоколОбмена";
...
Функция ТекстМаркировкаТоваров(Ссылка)
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи  = СтрШаблон(НСтр("ru = 'Маркировка товаров: %1';
										|en = 'Маркировка товаров: %1'"), НРег (Строка(Выборка.Статус)));
		ИмяКоманды    = "#ГИСМ#ОткрытьПротоколОбмена";
...
Функция ТекстЗаявкаНаВыпускКиЗГИСМ(Ссылка)
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи = СтрШаблон(НСтр("ru = 'Заявка на выпуск КиЗ: %1';
										|en = 'Заявка на выпуск КиЗ: %1'"), Выборка.СостояниеЗаявкиНаВыпускКиЗ);
		ИмяКоманды    = "#ГИСМ#ОткрытьПротоколОбмена";
...
Функция ТекстУведомлениеОбИмпорте(Ссылка)
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи = СтрШаблон(НСтр("ru = 'Уведомление об импорте ГИСМ: %1';
										|en = 'Уведомление об импорте ГИСМ: %1'"), Выборка.Статус);
		ИмяКоманды    = "#ГИСМ#ОткрытьПротоколОбмена";
...
Функция ТекстУведомлениеОВвозе(Ссылка)
	...
	Если ПравоДобавления Тогда
		ТекстНадписи = НСтр("ru = 'Создать уведомление о ввозе из ЕАЭС ГИСМ';
							|en = 'Создать уведомление о ввозе из ЕАЭС ГИСМ'");
	...
	Иначе
		ТекстНадписи = НСтр("ru = 'Уведомление о ввозе из ЕАЭС ГИСМ не создано';
							|en = 'Уведомление о ввозе из ЕАЭС ГИСМ не создано'");
		ИмяКоманды   = "";
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи = СтрШаблон(НСтр("ru = 'Уведомление о ввозе из ЕАЭС ГИСМ: %1';
										|en = 'Уведомление о ввозе из ЕАЭС ГИСМ: %1'"), Выборка.Статус);
		ИмяКоманды    = "#ГИСМ#ОткрытьПротоколОбмена";
		...
		Если ПервичныеДанные = Неопределено Тогда
			...
			ОбработатьОшибку(
				НСтр("ru = 'При разборе XML ответа на запрос получения квитанции из ГИСМ возникла ошибка:
				           |Текст ошибки: %1';
				           |en = 'При разборе XML ответа на запрос получения квитанции из ГИСМ возникла ошибка:
			...
			Если ОбъектXDTO.request_query_error <> Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Квитанция получена. Ошибка: Неверный запрос.
					           ...
					           |en = 'Квитанция получена. Ошибка: Неверный запрос.
					           |Дополнительная информация: Код ошибки: %1. Текст ошибки: %2.'"),
					КодОшибки,
					ОписаниеОшибки);
			...
			ИначеЕсли ОбъектXDTO.request_system_failure <> Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'При получения квитанции из ГИСМ произошел системный сбой:
					           |%1.';
					           |en = 'При получения квитанции из ГИСМ произошел системный сбой:
					           |%1.'"),
					ОписаниеОшибки);
				...
				ИначеЕсли ОбъектXDTO.result.system_failure <> Неопределено Тогда
					...
					ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
						НСтр("ru = 'Квитанция получена. Ошибка: Системный сбой.
						           ...
						           |en = 'Квитанция получена. Ошибка: Системный сбой.
						           |Дополнительная информация: %1.'"),
						ОписаниеОшибки);
				...
				ИначеЕсли ОбъектXDTO.result.reject <> Неопределено Тогда
					...
					Если ОбъектXDTO.result.reject.denied_action <> Неопределено Тогда
						...
						Если ОписаниеТипаДокумента <> Неопределено Тогда
							ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Недостаточно прав на выполнение операции ""%1""';
															|en = 'Недостаточно прав на выполнение операции ""%1""'"), ОписаниеТипаДокумента.Описание);
						Иначе
							ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Недостаточно прав на выполнение операции (action id: %1)';
															|en = 'Недостаточно прав на выполнение операции (action id: %1)'"), ТипДокумента);
						КонецЕсли;
					...
					ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
						НСтр("ru = 'Квитанция получена. Ошибка: Отказ на проводку запроса.
						           |Дополнительная информация: %1.';
						           |en = 'Квитанция получена. Ошибка: Отказ на проводку запроса.
						           |Дополнительная информация: %1.'"),
						ОписаниеОшибки);
				...
				ИначеЕсли ОбъектXDTO.result.moderation <> Неопределено Тогда
					...
					ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
						НСтр("ru = 'Запрос ожидает рассмотрения и утверждения.
						           |Дополнительная информация: %1.';
						           |en = 'Запрос ожидает рассмотрения и утверждения.
						           |Дополнительная информация: %1.'"),
						ОписаниеОшибки);
	...
	Исключение
		...
		ОбработатьОшибку(
			НСтр("ru = 'При разборе объекта XDTO ответа на запрос получения квитанции из ГИСМ возникла ошибка:
			           |Текст ошибки: %1';
			           |en = 'При разборе объекта XDTO ответа на запрос получения квитанции из ГИСМ возникла ошибка:
			           |Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
		...
		Если ПервичныеДанные = Неопределено Тогда
			...
			ОбработатьОшибку(
				НСтр("ru = 'При разборе XML ответа на запрос передачи данных в ГИСМ возникла ошибка:
				           |Текст ошибки: %1';
				           |en = 'При разборе XML ответа на запрос передачи данных в ГИСМ возникла ошибка:
				           |Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
			...
			Если ОбъектXDTO.query_error <> Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'При передаче данных в ГИСМ возникла ошибка
					           ...
					           |en = 'При передаче данных в ГИСМ возникла ошибка
					           |Код ошибки: %1. Текст ошибки: %2.'"),
					КодОшибки,
					ОписаниеОшибки);
			...
			ИначеЕсли ОбъектXDTO.system_failure <> Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'При передаче данных в ГИСМ произошел системный сбой.
					           |Дополнительная информация: %1.';
					           |en = 'При передаче данных в ГИСМ произошел системный сбой.
					           |Дополнительная информация: %1.'"),
					ОписаниеОшибки);
	...
	Исключение
		...
		ОбработатьОшибку(
			НСтр("ru = 'При разборе объекта XDTO ответа на запрос передачи данных в ГИСМ возникла ошибка:
			           |Текст ошибки: %1';
			           |en = 'При разборе объекта XDTO ответа на запрос передачи данных в ГИСМ возникла ошибка:
			           |Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
		...
		Если ПервичныеДанные = Неопределено Тогда
			...
			ОбработатьОшибку(
				НСтр("ru = 'При разборе XML ответа на запрос получения документа из ГИСМ возникла ошибка:
				           |Текст ошибки: %1';
				           |en = 'При разборе XML ответа на запрос получения документа из ГИСМ возникла ошибка:
				           |Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
			...
			Если ОбъектXDTO.request_query_error <> Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'При получении документа %5 (action_id=%3, output_id=%4) из ГИСМ возникла ошибка:
					           |Код ошибки: %1. Текст ошибки: %2.';
					           |en = 'При получении документа %5 (action_id=%3, output_id=%4) из ГИСМ возникла ошибка:
					           |Код ошибки: %1. Текст ошибки: %2.'"),
					КодОшибки,
					ОписаниеОшибки,
			...
			ИначеЕсли ОбъектXDTO.request_system_failure <> Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'При получении документа %4 (action_id=%2, output_id=%3) из ГИСМ произошел системный сбой.
					           |Дополнительная информация: %1.';
					           |en = 'При получении документа %4 (action_id=%2, output_id=%3) из ГИСМ произошел системный сбой.
					           |Дополнительная информация: %1.'"),
					ОписаниеОшибки,
					ДанныеДляПолученияДокумента.action_id,
					ДанныеДляПолученияДокумента.output_id,
	...
	Исключение
		...
		ОбработатьОшибку(
			НСтр("ru = 'При разборе объекта XDTO ответа на запрос получения документа из ГИСМ возникла ошибка:
			           |Текст ошибки: %1';
			           |en = 'При разборе объекта XDTO ответа на запрос получения документа из ГИСМ возникла ошибка:
			           |Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
		...
		Если ПервичныеДанные = Неопределено Тогда
			...
			ОбработатьОшибку(
				НСтр("ru = 'При разборе XML ответа на запрос получения списка новых документов из ГИСМ возникла ошибка:
				           |Текст ошибки: %1';
				           |en = 'При разборе XML ответа на запрос получения списка новых документов из ГИСМ возникла ошибка:
				           |Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
			...
			Если ОбъектXDTO.request_query_error <> Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'При получении списка новых документов из ГИСМ возникла ошибка:
					           |Код ошибки: %1. Текст ошибки: %2.';
					           |en = 'При получении списка новых документов из ГИСМ возникла ошибка:
					           |Код ошибки: %1. Текст ошибки: %2.'"),
					КодОшибки,
					ОписаниеОшибки);
			...
			ИначеЕсли ОбъектXDTO.request_system_failure <> Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'При получении списка новых документов из ГИСМ произошел системный сбой.
					           |Дополнительная информация: %1.';
					           |en = 'При получении списка новых документов из ГИСМ произошел системный сбой.
					           |Дополнительная информация: %1.'"),
					ОписаниеОшибки);
	...
	Исключение
		...
		ОбработатьОшибку(
			НСтр("ru = 'При разборе объекта XDTO ответа на запрос получения списка новых документов из ГИСМ возникла ошибка:
			           |Текст ошибки: %1';
			           |en = 'При разборе объекта XDTO ответа на запрос получения списка новых документов из ГИСМ возникла ошибка:
			           |Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
		...
		Если ПервичныеДанные = Неопределено Тогда
			...
			ОбработатьОшибку(
				НСтр("ru = 'При разборе XML ответа на запрос получения данных реестра из ГИСМ возникла ошибка:
				           |Текст ошибки: %1';
				           |en = 'При разборе XML ответа на запрос получения данных реестра из ГИСМ возникла ошибка:
				           |Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
			...
			Если ОбъектXDTO.query_error <> Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'При получении данных реестра из ГИСМ возникла ошибка:
					           |Код ошибки: %1. Текст ошибки: %2.';
					           |en = 'При получении данных реестра из ГИСМ возникла ошибка:
					           |Код ошибки: %1. Текст ошибки: %2.'"),
					КодОшибки,
					ОписаниеОшибки);
			...
			ИначеЕсли ОбъектXDTO.system_failure <> Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'При получении данных реестра из ГИСМ произошел системный сбой.
					           |Дополнительная информация: %1.';
					           |en = 'При получении данных реестра из ГИСМ произошел системный сбой.
					           |Дополнительная информация: %1.'"),
					ОписаниеОшибки);
	...
	Исключение
		...
		ОбработатьОшибку(
			НСтр("ru = 'При разборе объекта XDTO ответа на запрос получения данных реестра из ГИСМ возникла ошибка:
			           |Текст ошибки: %1';
			           |en = 'При разборе объекта XDTO ответа на запрос получения данных реестра из ГИСМ возникла ошибка:
			           |Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
// 
// Возвращаемое значение:
//  Неопределено, Строка - Текст входящего сообщения XML
...
// см. ИнтеграцияГИСМ.ОтправитьЗапросДляПередачиДанных
...
// см. ИнтеграцияГИСМ.ОтправитьЗапросДляПолученияДанных
...
// см. ИнтеграцияГИСМ.ОтправитьЗапросДляПолученияРеестров
...
// 
// Возвращаемое значение:
//  Структура - Получить информацию ГИСМПо номеру ки З:
// * TID - Строка
// * GTIN - Строка
