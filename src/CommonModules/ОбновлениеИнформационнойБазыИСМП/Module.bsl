...
#Область Монопольно
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение настроек интеграции МОТП.';
									|en = 'Заполнение настроек интеграции МОТП.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение настроек интеграции ИС МП (обувь).';
									|en = 'Заполнение настроек интеграции ИС МП (обувь).'");
...
#Область Отложенно
	...
	Обработчик.Комментарий = НСтр("ru = 'Включение настроек обращений к ГИС МТ при наличии включенных товарных групп разрешительного режима';
									|en = 'Включение настроек обращений к ГИС МТ при наличии включенных товарных групп разрешительного режима'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "10.1.35.17";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Процедура = "РегистрыСведений.НастройкиУчетаМаркируемойПродукцииИСМП.ОбработатьДанныеДляПереходаНаНовуюВерсиюЗаполнениеДатМаркировки";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("699cd462-bcca-4ad3-89e2-a0de9b835642");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "РегистрыСведений.НастройкиУчетаМаркируемойПродукцииИСМП.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюЗаполнениеДатМаркировки";
	Обработчик.ОчередьОтложеннойОбработки = 1;
	...
	Обработчик.БлокируемыеОбъекты = "";
	Обработчик.Комментарий = НСтр("ru = 'Заполнение даты контроля прослеживаемости перемещений между МОД (места осуществления деятельности)';
									|en = 'Заполнение даты контроля прослеживаемости перемещений между МОД (места осуществления деятельности)'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	...
	Если ЭтоРасширеннаяВерсия Тогда
		...
		Обработчик.Комментарий = НСтр("ru = 'Заполняет новое измерение регистра ""Вид маркируемой продукции"" ссылками на перечисление ""ВидыПродукцииИС.Табак"".';
										|en = 'Заполняет новое измерение регистра ""Вид маркируемой продукции"" ссылками на перечисление ""ВидыПродукцииИС.Табак"".'");
		Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
		...
		Обработчик.Комментарий = НСтр("ru = 'Переход на настройки раздельного контроля статусов и владельцев кодов маркировки';
										|en = 'Переход на настройки раздельного контроля статусов и владельцев кодов маркировки'");
		Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
		...
		Обработчик.Комментарий = НСтр("ru = 'Обработка кодов маркировки табачной продукции (нормализация без МРЦ)';
										|en = 'Обработка кодов маркировки табачной продукции (нормализация без МРЦ)'");
		Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
Функция ВключитьПроверкуРозничнымМетодомДляОпределенныхТоварныхГрупп() Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось обновить исключения по товарным группам, требующим обязательного онлайн-контроля при продаже в розницу, по причине: %Причина%';
								|en = 'Не удалось обновить исключения по товарным группам, требующим обязательного онлайн-контроля при продаже в розницу, по причине: %Причина%'");
...
Процедура ЗаполнитьНастройкуУчетаВидаПродукции(НастройкаУчета, ВидМаркируемойПродукции)
	...
	СтруктураДатКонтроля = РегистрыСведений.НастройкиУчетаМаркируемойПродукцииИСМП.ДатыРаздельногоКонтроляМаркируемойПродукцииПоУмолчанию(ВидМаркируемойПродукции);
	...
	Если ВидМаркируемойПродукции = Перечисления.ВидыПродукцииИС.Табак Тогда
		...
		Если Не Метаданные.Константы.Найти(ИмяКонстанты) = Неопределено Тогда
			НастройкаУчета.ДатаКонтрольПроизводства = Константы[ИмяКонстанты].Получить();
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(НастройкаУчета, СтруктураДатКонтроля,, "ДатаКонтрольПроизводства");
		
		Если НастройкаУчета.ДатаКонтрольПроизводства = '00010101' Тогда
			НастройкаУчета.ДатаКонтрольПроизводства = СтруктураДатКонтроля.ДатаКонтрольПроизводства;
		КонецЕсли;
	...
	Иначе
		...
		ЗаполнитьЗначенияСвойств(НастройкаУчета, СтруктураДатКонтроля);
