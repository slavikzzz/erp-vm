...
Функция ЗашифроватьВстроеннымКриптопровайдером(Данные, Сертификат) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru = 'Не настроен встроенный криптопровайдер.';
								|en = 'The built-in сryptographic service provider is not configured.'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		ВызватьИсключение  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					|%1';
					|en = 'Built-in сryptographic service provider:
					|%1'"), ОписаниеОшибки);
	КонецЕсли;
			...
			Для Каждого ФайлДоверенности Из ФайлыДоверенности Цикл
				Если ЗначениеЗаполнено(ФайлДоверенности.ОписаниеОшибки) Тогда
					...
					НСтр("ru = 'Не удалось сохранить файл доверенности %1: %2';
						|en = 'Cannot save the file of the %1 letter of authority: %2'"),
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			...
			Если ЭтоМодельСервисаСДоступнымУсовершенствованием Тогда
				...
				Элементы.ТипПодписиКриптографииПоУмолчаниюРасширеннаяПодсказка.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
					...
					|en = 'In the web application, archival signatures are unavailable by default. You can select this signature type when signing with a <a href=%1>certificate</a> installed on the computer with a <a href=Программы>digital signing application</a>.'"), "Сертификаты");
			Иначе
				...
				Элементы.ТипПодписиКриптографииПоУмолчанию1РасширеннаяПодсказка.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
					...
					|en = 'In the web application, the default signature type is ""Basic"". You can select signature types with timestamps when signing with a <a href=%1>certificate</a> installed on the computer with a <a href=Программы>digital signing application</a>.'"), "Сертификаты");
			КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает стандартные адреса серверов меток времени ФНС России и Удостоверяющего Центра ООО «Научно-производственный центр ""1С""» для удобной работы с метками доверенного времени в электронной подписи.';
									|en = 'Sets standard timestamp server addresses of the Federal Tax Service of the Russian Federation and the certification authority ""Research and development center 1C"" LLC for convenient operation with trusted timestamps in the digital signature.'");
	...
	Обработчик.Комментарий =
		НСтр("ru = 'Устанавливает режим использования для приложений электронной подписи и шифрования.';
			|en = 'Sets the usage mode for digital signing and encryption apps.'");
...
Процедура ПриДобавленииКлассификаторов(Классификаторы) Экспорт
	...
	Описание.Наименование = НСтр("ru = 'Список аккредитованных удостоверяющих центров';
								|en = 'List of accredited certification authorities'");
		...
		Если ЭлектроннаяПодпись.ОбщиеНастройки().ЗаявлениеНаВыпускСертификатаДоступно Тогда
			...
			Для Каждого Раздел Из Разделы Цикл
				...
				Дело.Представление  = НСтр("ru = 'Заявления на выпуск сертификата в работе';
											|en = 'Submitted applications for certificate issuance'");
	...
	Если ВидимостьСсылкиНаИнструкциюПоТипичнымПроблемамПриРаботеСПрограммами() Тогда
		Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'При возникновении затруднений ознакомьтесь со списком <a href = %1>типичных проблем при работе с приложениями электронной подписи и их решений</a>.
				...
				|en = 'Facing issues? Learn to troubleshoot <a href = %1>digital signing applications</a> (in Russian).
				|
	...
	Иначе
		Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = '<a href = %1>Обратитесь в службу поддержки фирмы ""1С""</a>';
				|en = '<a href = %1>Contact the 1C support service</a> '"),
...
Функция ОшибкаСертификатПомеченКакОтозванный() Экспорт
	...
	ПредставлениеОшибки.Вставить("Решение", СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = 'В меню <b>Еще</b> в <a href=""%1"">карточке сертификата</a> можно снять пометку <b>Сертификат отозван</b>. Но если сертификат отозван в удостоверяющем центре, сделать подпись таким сертификатом будет все равно невозможно.';
			|en = 'In the <b>More actions</b> menu, in <a href=""%1"">certificate card</a>, you can clear the <b>Certificate revoked</b> mark. However, if a certificate is revoked by a certification authority, you will not be able to sign with this certificate.'"),
		"ОткрытьСертификат"));
...
Функция МенеджерКриптографии(Операция, ПараметрыСозданияМенеджераКриптографии = Неопределено) Экспорт
	...
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		ЗаголовокОшибки = НСтр("ru = 'Не удалось проверить подпись на сервере %1 по причине:';
								|en = 'Cannot check the signature on server %1 due to:'");
	...
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		ЗаголовокОшибки = НСтр("ru = 'Не удалось проверить сертификат на сервере %1 по причине:';
								|en = 'Cannot check the certificate on server %1 due to:'");
	...
	ИначеЕсли Операция <> "" Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка вызова функции %1.
			           ...
			           |The ""Operation"" parameter has invalid value ""%2"".'"), "МенеджерКриптографии", Операция);
	...
	Иначе
		СведенияОПодписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Дата подписи: %1';
				|en = 'Signed on: %1'"), Формат(ДатаПодписи, "ДЛФ=DT")) + Символы.ПС;
	КонецЕсли;
	...
	Если ТребуетсяУстановкаСпискаОтзыва Тогда
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|%2.';
				|en = 'Не удалось установить список отзыва на сервере при проверке сертификата: %1
				|%2.'"),
			...
			ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Электронная подпись.Обновление списков отзыва';
						|en = 'Digital signature.Revocation list update'",
					ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ЗаписатьОтметкуОбОтзывеСертификата(Отпечаток) Экспорт
	...
	Исключение
		...
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось записать отметку об отзыве сертификата %1: %2';
				|en = 'Cannot save the %1 certificate revocation mark: %2'"), СсылкаНаСертификат, 
			ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись.Удаление временного файла';
				|en = 'Digital signature.Remove temporary file'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , ,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура ЗарегистрироватьОшибкуУсовершенствованияПодписиВЖурнале(ОписаниеОшибки, СвойстваПодписи = Неопределено) 
	...
	Если СвойстваПодписи <> Неопределено Тогда
		ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Порядковый номер подписи %1';
				|en = 'Signature sequence number %1'"), СвойстваПодписи.ПорядковыйНомер);
		ПодписанныйОбъект = СвойстваПодписи.ПодписанныйОбъект;
		ПодписанныйОбъектМетаданные = ПодписанныйОбъект.Метаданные();
	Иначе
...
Функция УсовершенствоватьПодписьВСервисе(Подпись, ПараметрыВыполнения)
	...
	Если ТипПодписи <> Перечисления.ТипыПодписиКриптографии.БазоваяCAdESBES Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Нельзя усовершенствовать подпись с типом %1 в приложении в Интернете';
				|en = 'Cannot enhance the signature with type %1 in the web application'"), ТипПодписи);
	...
	Для Каждого КлючИЗначение Из СвойстваКонвертаXML.ОбязательныеУзлы Цикл
		...
		Если ЧастиИмени.Количество() > 1 Тогда
			СвойстваКонвертаXML.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В документе XML не найден узел ""%1"" в узле ""%2"".';
					|en = '%1 node is not found in %2 node in XML document.'"), КлючИЗначение.Ключ);
		Иначе
			СвойстваКонвертаXML.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В документе XML не найден узел ""%1"".';
					|en = '%1 node is not found in XML document.'"), ЧастиИмени[0]);
		КонецЕсли;
		Возврат ВозвращаемыеСвойстваКонвертаXML(СвойстваКонвертаXML);
	КонецЦикла;
...
Функция ПрограммаПоСертификату(Знач Сертификат, ТребуетсяЗакрытыйКлюч = Неопределено,  ОбъектКомпоненты = Неопределено) Экспорт
	...
	Если ОбъектКомпоненты = Неопределено Тогда
		...
		Исключение
			Результат.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось подключить компоненту %1 на сервере %2.';
					|en = 'Cannot attach the %1 add-in on the %2 server.'"), "ExtraCryptoAPI", ИмяКомпьютера());
			Возврат Результат;
		КонецПопытки;
	КонецЕсли;
	...
	Если ТребуетсяЗакрытыйКлюч = Истина Тогда
		Результат.Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось определить приложение по закрытому ключу сертификата на сервере %1.';
				|en = 'Couldn''t determine app by the private key on the server %1.'"), ИмяКомпьютера());
		Возврат Результат;
	КонецЕсли;
		...
		Если Элементы.ГруппаУсовершенствоватьПодписи.Доступность Тогда
			...
			Элементы.ДекорацияУсовершенствованиеРасширеннаяПодсказка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Подписи, добавленные ранее и полученные извне, будут усовершенствованы для длительного хранения до выбранного типа: %1.';
					|en = 'Previously added and external signatures will be improved for long-term storage to the selected type: %1.'"),
				ТипПодписи);
		...
		Иначе
			Элементы.ДекорацияУсовершенствованиеРасширеннаяПодсказка.Заголовок = 
				НСтр("ru = 'Подписи, добавленные ранее и полученные извне, будут усовершенствованы для длительного хранения до выбранного типа.';
					|en = 'Previously added and external signatures will be improved for long-term storage to the selected type.'");
		КонецЕсли;
...
Процедура УстановитьЗаголовокЭлектроннаяПодписьНаСервере(Форма)
	...
	Если НаборКонстант.ИспользоватьЭлектронныеПодписи Тогда
		ЗаголовокФлажка = НСтр("ru = 'Проверять подписи и сертификаты на сервере';
								|en = 'Check signatures and certificates on server'");
		ПодсказкаФлажка =
	...
	Иначе
		ПодсказкаНаСервере = НСтр("ru = 'Важно: на каждый компьютер, где работает сервер 1С:Предприятия, должно быть установлено хотя бы одно <a href=%1>приложение электронной подписи</a> из списка.';
									|en = 'Important: At least one <a href=%1>digital signing app</a> from the list must be installed on each computer running 1C:Enterprise server.'");
	КонецЕсли;
	...
	Если Не НаборКонстант.ИспользоватьЭлектронныеПодписи Тогда
		ЗаголовокФлажка = НСтр("ru = 'Шифровать и расшифровывать на сервере';
								|en = 'Encrypt and decrypt on server'");
		ПодсказкаФлажка =
	...
	Если ИнформационнаяБазаФайловая Тогда
		ПодсказкаНаСервере = НСтр("ru = 'Важно: на компьютер, где работает веб-сервер, подключенный к файловой информационной базе, должно быть установлено <a href=%1>приложение электронной подписи</a> и <a href=%2>сертификат</a> с закрытым ключом.';
									|en = 'Важно: на компьютер, где работает веб-сервер, подключенный к файловой информационной базе, должно быть установлено <a href=%1>приложение электронной подписи</a> и <a href=%2>сертификат</a> с закрытым ключом.'");
	Иначе
		ПодсказкаНаСервере = НСтр("ru = 'Важно: на каждый компьютер, где работает сервер 1С:Предприятия, должно быть установлено <a href=%1>приложение электронной подписи</a> и <a href=%2>сертификат</a> с закрытым ключом.';
									|en = 'Важно: на каждый компьютер, где работает сервер 1С:Предприятия, должно быть установлено <a href=%1>приложение электронной подписи</a> и <a href=%2>сертификат</a> с закрытым ключом.'"); 
	КонецЕсли;
...
Процедура ДополнитьТехническойИнформациейОСервере(ТехническаяИнформация,
	...
	Если Не ЭлектроннаяПодпись.СоздаватьЭлектронныеПодписиНаСервере()
		...
		ТехническаяИнформация = ТехническаяИнформация + Символы.ПС
			+ НСтр("ru = 'Электронная подпись на сервере не используется.';
					|en = 'Digital signature is not used on the server.'") + Символы.ПС;
	Иначе
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		РежимРаботы = ?(ОбщегоНазначения.КлиентПодключенЧерезВебСервер(),
			НСтр("ru = 'Файловый через веб';
				|en = 'File mode via web'"), НСтр("ru = 'Файловый';
													|en = 'File'"));
	Иначе
	...
	ТехническаяИнформация = ТехническаяИнформация
		+ Символы.ПС + НСтр("ru = 'Режим работы информационной базы';
							|en = 'Infobase operation mode'")+ " - " + РежимРаботы
...
Функция ТехническаяИнформацияОКомпьютере()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Сервер (%1):';
			|en = 'Server (%1):'"), ИмяКомпьютера() + "\" + ИмяПользователяОС) + Символы.ПС
...
Функция ТехническаяИнформацияОКомпоненте()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Версия компоненты %1 на сервере - %2';
			|en = 'Add-in ""%1"" version on server: %2'"), "ExtraCryptoAPI", ВерсияКомпоненты); 
...
Функция ТехническаяИнформацияОПрограммах()
	...
	Иначе
		...
		Иначе
			ДиагностическаяИнформация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Автоматически определенные приложения электронной подписи и шифрования на сервере ""%1"":';
					|en = 'Digital signing and encryption apps auto-determined on the server ""%1"":'"), ИмяКомпьютера()) + Символы.ПС;
	...
	ДиагностическаяИнформация = ДиагностическаяИнформация + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Программы на сервере из справочника ""%1"":';
			|en = 'Applications on the server from the ""%1"" catalog:'"), ИмяКомпьютера()) + Символы.ПС;
...
Функция ТехническаяИнформацияОНастройкахПрограммВСправочнике(ПроверенныеПутиКМодулямПрограммНаКлиенте)
	...
	ДиагностическаяИнформация = Символы.ПС
		+ НСтр("ru = 'Настройки приложений электронной подписи и шифрования:';
				|en = 'Digital signing and encryption app settings:'") + Символы.ПС;
				...
				Иначе
					РезультатЗагрузки.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибки при попытке загрузить файл(ы):
							 ...
							 |%1
							 |For more information, see the event log.'"), СтрСоединить(Адреса, Символы.ПС));
				КонецЕсли;
				...
				Если ПериодДействияСпискаОтзыва = Неопределено Тогда
					...
					РезультатЗагрузки.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Файл %1 не является списком отзыва.';
							|en = 'This file is not a revocation list: %1.'"), РезультатЗагрузки.АдресФайла);
				Иначе
			...
			Иначе
				...
				РезультатЗагрузки.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибки при загрузке списка отзыва:
						|%1
						|Подробности см. в Журнале регистрации.';
						|en = 'An error occurred when importing the revocation list:
						|%1
...
Функция ЦепочкаСертификатов(Знач Сертификат, ИдентификаторФормы = Неопределено, ОбъектКомпоненты = Неопределено) Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Ошибка) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось получить цепочку сертификатов: %1 (%2).';
					|en = 'Cannot receive the certificate chain: %1 (%2).'"), Ошибка, ИмяКомпьютера());
		КонецЕсли;
		
		Если СертификатыРезультат = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось получить цепочку сертификатов (%1).';
					|en = 'Cannot receive the certificate chain (%1).'"), ИмяКомпьютера());
		КонецЕсли;
	...
	Если ОшибкаПоКлассификатору.ДействияДляУстранения.Найти(
		...
		Если ЗначениеЗаполнено(ИздательСертификата) Тогда
			...
			Решение.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Удостоверяющий центр, выдавший сертификат: %1.';
								|en = 'Certificate authority that issued the certificate: %1.'"), ИздательСертификата));
		КонецЕсли;
	КонецЕсли;
		...
		Если ОбщегоНазначения.ЭтоВебКлиент() Тогда
			...
			Для Каждого АдресСервера Из АдресаСерверовМетокВремени Цикл
				...
				Массив.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<a href = ""%1"">%1</a>';
						|en = '<a href = ""%1"">%1</a>'"), АдресСервера));
...
Функция СертификатВЛокальномХранилищеТекстРешения(Место)
	...
	Если Место = "Сервер" Тогда
		...
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Удалите сертификат из локального хранилища сервера %1.';
				|en = 'Uninstall the certificate from the local storage on the server %1.'"), ИмяКомпьютера());
...
Процедура УстановитьСписокОтзываСертификата(Сертификат, Адреса = Неопределено, АдресВнутренний = Неопределено) Экспорт
	...
	Если СвойстваСпискаОтзыва = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Файл %1 не является списком отзыва.';
				|en = 'This file is not a revocation list: %1.'"), Результат.АдресФайла);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СвойстваСпискаОтзыва.ДатаОкончания) И СвойстваСпискаОтзыва.ДатаОкончания < ТекущаяУниверсальнаяДата() Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Список отзыва %1 просрочен. Обратитесь в удостоверяющий центр, выдавший сертификат.';
				|en = 'The certificate revocation list %1 is expired. Contact the certificate issuer.'"), Результат.АдресФайла);
	КонецЕсли;
