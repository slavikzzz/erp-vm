...
Функция ЭтоБазоваяВерсияКонфигурации() Экспорт
	...
	ЭтоБазоваяВерсияКонфигурации = СтрНайти(ВРег(Метаданные.Имя), НСтр("ru = 'БАЗОВАЯ';
																		|en = 'BASE'")) > 0;
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Ошибка выполнения';
					|en = 'Runtime error'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПроверитьБезопасныйРежимПередЗаписью(Источник, Отказ) Экспорт
	...
	ВызватьИсключение НСтр("ru = 'Действие недоступно в безопасном режиме.';
							|en = 'The action is unavailable in safe mode.'");
...
Функция КомпонентаПоследнейВерсии(Идентификатор, Местоположение, ВнешняяКомпонента = Неопределено) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не существует внешняя компонента с идентификатором %1';
			|en = 'Add-in with ID %1 does not exist'"), Идентификатор);
...
Функция ТехническаяИнформацияОВерсияхПодсистемИРасширениях() Экспорт
	...
	ТехническаяИнформацияОВерсияхПодсистемИРасширениях = НСтр("ru = 'Версии подсистем';
																|en = 'Subsystem versions'") + ":" + Символы.ПС;
	...
	Для Каждого Расширение Из Расширения Цикл
		...
		ТехническаяИнформацияОВерсияхПодсистемИРасширениях = ТехническаяИнформацияОВерсияхПодсистемИРасширениях
			...
			+ Формат(Расширение.Активно, НСтр("ru = 'БЛ=Отключено; БИ=Включено';
												|en = 'BF=Disabled; BT=Enabled'")) + Символы.ПС;
			...
			Если Счет.ВидыСубконто[Индекс].ВидСубконто <> ВидСубконто.ВидСубконто Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Обмен данными.Отключение связи с главным узлом';
						|en = 'Data exchange.Disconnection from the master node'", ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Счет.Метаданные(),
					Счет,
