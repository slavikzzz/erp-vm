...
Процедура СформироватьДвиженияДенежныеСредстваКонтрагентПоКлиентам(МассивДокументов, ИзмененныеДокументы = Неопределено, ДляОтладки = Неопределено) Экспорт
	...
	Попытка
		...
		ЗаписатьДвиженияДенежныеСредстваКонтрагент(ДанныеДвижений, ИзмененныеДокументы, ДляОтладки);
	...
	Исключение
		ТекстСообщения = НСтр("ru = 'Не удалось выполнить отложенное проведение по регистру ""Движения Денежные средства - Контрагент""
						|при расчетах с клиентами по причине';
						|en = 'Cannot execute deferred posting for the ""Records – Cash – Counterparty"" register
						|when performing customer AR/AP. Reason:'") + ":" + Символы.ПС + "%Причина%";
...
Процедура СформироватьДвиженияДенежныеСредстваКонтрагентПоПоставщикам(МассивДокументов, ИзмененныеДокументы = Неопределено, ДляОтладки = Неопределено) Экспорт
	...
	Исключение
		ТекстСообщения = НСтр("ru = 'Не удалось выполнить отложенное проведение по регистру ""Движения Денежные средства - Контрагент""
						|при расчетах с поставщиками по причине';
						|en = 'Cannot execute deferred posting for the ""Records – Cash – Counterparty"" register
						|when performing vendor AR/AP. Reason:'") + ":" + Символы.ПС + "%Причина%";
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Ошибка, , ТекстСообщения);
		ВызватьИсключение ТекстСообщения;
...
Процедура СформироватьДвиженияКонтрагентКонтрагент(МассивДокументовРасчетов, ИзмененныеДокументы = Неопределено, ДляОтладки = Неопределено) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Отложенное проведение взаимозачета';
				|en = 'Deferred offsetting'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ЗаписатьДвиженияДенежныеСредстваКонтрагент(ДанныеДвижений, ИзмененныеДокументы, ДляОтладки = Неопределено)
...
Процедура РаспределитьОплатуПоРасчетнымДокументам(НаборЗаписей)
	...
	ПоляОтбора = "Организация,СтавкаНДС,ОтложенноеПроведение,Сторно,"
...
#Область ТекстЗапросаРезервыПоСомнительнымДолгам

	ТекстЗапросаРезервыПоСомнительнымДолгам = 
	"ВЫБРАТЬ
	|	Резервы.Период,
	...
	|	ЗНАЧЕНИЕ(Перечисление.ТипыСуммЗадолженности.РезервДолга) КАК ТипСуммы,
	...
	|			ПО Резервы.Регистратор = Т.Документ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК КлючиАналитики
	|			ПО Резервы.АналитикаУчетаПоПартнерам = КлючиАналитики.КлючАналитики
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(Резервы.Регистратор) = ТИП(Документ.КорректировкаЗадолженности)
	|
	|СГРУППИРОВАТЬ ПО
	|	Резервы.Период,
	|	Резервы.ВидДвижения,
...
//Документ СписаниеЗадолженности
		#Область РасходЗадолженностиПриСписании
		РасходЗадолженностиПриСписании = 
		...
		|	РасчетыСПартнерами.ОбъектРасчетов КАК ОбъектРасчетов,
		|	РасчетыСПартнерами.Валюта,
		|	Резервы.Сумма КАК СуммаВВалюте,
		|	Резервы.СуммаУпр КАК Сумма,
		|	Резервы.СуммаРегл КАК СуммаРегл,
		|	РасчетыСПартнерами.ТипСуммы КАК ТипСуммы,
		|	РасчетыСПартнерами.ХозяйственнаяОперация КАК ХозОперацияРегистра,
		...
		|	РасчетыСПартнерами.Валюта КАК ВалютаВзаиморасчетов,
		|	Резервы.СуммаРегл / 
		|		ВЫБОР КОГДА Резервы.Сумма = 0 
		|			ТОГДА ВЫБОР КОГДА Резервы.СуммаРегл = 0 
		|							ТОГДА 1 
		|							ИНАЧЕ Резервы.СуммаРегл
		|					КОНЕЦ
		|			ИНАЧЕ Резервы.Сумма
		|		КОНЕЦ КАК КурсВалютыВзаиморасчетов,
		|	Резервы.Сумма КАК СуммаВВалютеВзаиморасчетов,
		|	РасчетыСПартнерами.ОбъектРасчетов КАК ИсточникГФУРасчетов,
		|	""РасходЗадолженностиПриСписании"" КАК ИмяТаблицы,
		|	РасчетыСПартнерами.ЭтоРасчетыСКлиентами КАК ЭтоРасчетыСКлиентами,
		|	РасчетыСПартнерами.ЭтоГоловнаяОрганизация КАК ЭтоГоловнаяОрганизация,
		|	ЛОЖЬ КАК ЭтоВозвратАванса,
		|	РасчетыСПартнерами.Сторно КАК Сторно,
		...
		|ИЗ
		|	РасчетыСПартнерами КАК РасчетыСПартнерами
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РезервыПоСомнительнымДолгам КАК Резервы
		|	ПО РасчетыСПартнерами.Регистратор = Резервы.Регистратор
		|		И РасчетыСПартнерами.АналитикаУчетаПоПартнерам = Резервы.АналитикаУчетаПоПартнерам
		|		И РасчетыСПартнерами.ОбъектРасчетов = Резервы.ОбъектРасчетов
		|		И РасчетыСПартнерами.Валюта = Резервы.Валюта
		|ГДЕ
		|	РасчетыСПартнерами.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|	И ТИПЗНАЧЕНИЯ(РасчетыСПартнерами.Регистратор) = ТИП(Документ.КорректировкаЗадолженности)";
		#КонецОбласти
//-- НЕ УТ
...
//Дисконтирование
		...
		//++ НЕ УТ
		РасходЗадолженности.Добавить(РасходЗадолженностиПриСписании);
		//-- НЕ УТ
...
//Документ СписаниеЗадолженности
		#Область ПриходЗадолженностиПриСписании
		ПриходЗадолженностиПриСписании = 
		"//При списании задолженности (таблица прихода точно такая же как таблица расхода) 
		|ВЫБРАТЬ
		|	РасчетыСПартнерами.Период,
		|	РасчетыСПартнерами.ВидДвижения КАК ВидДвижения,
		|	РасчетыСПартнерами.Регистратор КАК Регистратор,
		...
		|	Резервы.Сумма КАК СуммаВВалютеВзаиморасчетов,
		|	РасчетыСПартнерами.ОбъектРасчетов КАК ИсточникГФУРасчетов,
		|	""ПриходЗадолженностиПриСписании"" КАК ИмяТаблицы,
		|	РасчетыСПартнерами.ЭтоРасчетыСКлиентами КАК ЭтоРасчетыСКлиентами,
		|	РасчетыСПартнерами.ЭтоГоловнаяОрганизация КАК ЭтоГоловнаяОрганизация,
		|	ЛОЖЬ КАК ЭтоВозвратАванса,
		|	РасчетыСПартнерами.Сторно КАК Сторно,
...
//Дисконтирование
		...
		//++ НЕ УТ
		ПриходЗадолженности.Добавить(ПриходЗадолженностиПриСписании);
		//-- НЕ УТ
...
#КонецОбласти
	...
	//++ НЕ УТ
	ТекстыЗапросов.Добавить(ТекстЗапросаРезервыПоСомнительнымДолгам);
	//-- НЕ УТ
