...
Процедура ПроверитьАктивностьСервисов(АктивностьСервисов, ПроверяемыеСервисы) 
	...
	ШаблонСообщенияОбОшибке = НСтр("ru = 'Ошибка проверки активности сервисов:
										|%1
										|%2';
										|en = 'Error checking service activity:
										|%1
										|%2'");
	...
	Иначе
		Ошибка = СтрШаблон(ШаблонСообщенияОбОшибке,
			НСтр("ru = 'Получение файла из интернета:';
				|en = 'Getting a file from the Internet:'"),
...
Процедура ПроверитьДоступностьСервисов(РезультатыДиагностикиСоединения, ПроверяемыеСервисы)
	...
	ТекстПодробнойОшибки = НСтр("ru = 'На сервере ""%1"" нет доступа к сервисам.
		...
		|в сети предприятия.';
		|en = 'The server ""%1"" does not have access to services.
		|The ports for access to the 1C and/or Taxcom servers may be blocked.
		|Their availability must be checked. We also recommend that you check
		|the correct proxy server settings, if it is used
...
Процедура ПроверитьКорневыеСертификаты(НаличиеКорневыхСертификатов, ПроверяемыеСервисы, Контекст) 
	...
	Если ДоступнаУстановкаСертификатов() Тогда
		КраткаяРекомендация = НСтр("ru = 'Необходимо установить корневые сертификаты.';
									|en = 'You must install root certificates.'");
	Иначе
	...
	|en = 'You must install root certificates.
	|Installation manual: http://1c-edo.ru/handbook/24/2516/'");
...
Функция ИнтернетРесурсДоступен(АдресРесурса)
	...
	РезультатВыполненияЗапроса = ИнтернетСоединениеБЭД.ВызватьHTTPМетод(ОписаниеСоединения, HTTPЗапрос,
		ИнтернетСоединениеБЭД.HTTPМетоды().HEAD, НСтр("ru = 'Диагностика ЭДО: проверка доступности интернет-ресурса';
														|en = 'EDI troubleshooting: check Internet resource availability'"),
		КонтекстДиагностики);
...
Функция ОтчетДляАдминистратораИнтернетСоединение(РезультатВидаДиагностики, ПараметрыВыполнения)
	...
	ДобавитьПроверку(ПредставленияПроверок, "НаличиеИнтернетСоединения", НСтр("ru = '* Проверка интернет-соединения';
																				|en = '* Checking internet connection'"));
	...
	ДобавитьПроверку(ПредставленияПроверок, "АктивностьСервисов",
		НСтр("ru = '* Проверка активности сервиса';
			|en = '* Checking service activity'"),, ВложенныеПроверки, "Сервисы");
	...
	КонецЦикла;
	ДобавитьПроверку(ПредставленияПроверок, "ДоступностьСервисов", НСтр("ru = '* Проверка доступа к сервису';
																		|en = '* Checking service access'"),, ВложенныеПроверки, "Сервисы");
	...
	Если РезультатВидаДиагностики.Результаты.НаличиеКорневыхСертификатов.Результат <> Неопределено Тогда
		...
		ДобавитьПроверку(ПредставленияПроверок, "НаличиеКорневыхСертификатов",
			...
			|в хранилище операционной системы';
			|en = '* Checking root certificate availability
			|in the operating system storage'"),, ВложенныеПроверки, "Сервисы");
	КонецЕсли;
...
Процедура ДиагностикаКриптографии(РезультатДиагностики, ПараметрыВыполненияДиагностики, Контекст)
	...
	ОпределитьДоступностьСлужбМетокВремени(РезультатДиагностики, ПараметрыВыполненияДиагностики);
...
Процедура ОпределитьНаличиеКорневогоСертификатаГУЦ(РезультатДиагностики, ПараметрыВыполненияДиагностики, Контекст)
		...
		Если РезультатЗагрузки.Статус Тогда
			...
			Если ТипКонтента <> Неопределено И СтрНайти(НРег(ТипКонтента), "text/html") > 0 Тогда // Загрузилась страница с ошибкой
				СообщениеОбОшибке = НСтр("ru = 'Не удалось загрузить корневой сертификат по адресу %1';
										|en = 'Cannot import root certificate at %1'");
	...
	|en = 'Install MCA root certificate
	|Manual: http://1c-edo.ru/handbook/28/2655/'");
...
//  РезультатДиагностики - См. НовыйРезультатДиагностики
//  ПараметрыВыполненияДиагностики - см. ДиагностикаЭДОКлиентСервер.НовыеПараметрыВыполненияДиагностики
Процедура ОпределитьДоступностьСлужбМетокВремени(РезультатДиагностики, ПараметрыВыполненияДиагностики)
	
	Если Не ПараметрыВыполненияДиагностики.ПараметрыФормы.Свойство("Ошибки")
		Или ПараметрыВыполненияДиагностики.ПараметрыФормы.Ошибки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ЕстьТипПодписиСМеткойВремени = Ложь;
	МеткаДоверенногоВремениCAdEST = ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.СМеткойДоверенногоВремениCAdEST");
	Для Каждого Ошибка Из ПараметрыВыполненияДиагностики.ПараметрыФормы.Ошибки Цикл
		Если ТипЗнч(Ошибка.ДополнительныеДанные) = Тип("Структура")
			И Ошибка.ДополнительныеДанные.Свойство("ТипПодписи")
			И Ошибка.ДополнительныеДанные.ТипПодписи = МеткаДоверенногоВремениCAdEST Тогда
			ЕстьТипПодписиСМеткойВремени = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если Не ЕстьТипПодписиСМеткойВремени Тогда
		Возврат;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(АдресаСерверовМетокВремени) Тогда
		СлужбыАдресовМетокВремени = СтрРазделить(АдресаСерверовМетокВремени, ", ;" + Символы.ПС);
	Иначе
		СлужбыАдресовМетокВремени = Новый Массив;
	КонецЕсли;
	
	Граница = СлужбыАдресовМетокВремени.ВГраница();
	Для Счетчик = 0 По Граница Цикл
		ТекущийИндекс = Граница - Счетчик;
		СтруктураURI = ОбщегоНазначенияКлиентСервер.СтруктураURI(СлужбыАдресовМетокВремени[ТекущийИндекс]);
		Если СтруктураURI.ИмяСервера = "" Тогда
			СлужбыАдресовМетокВремени.Удалить(ТекущийИндекс);
		КонецЕсли;
	КонецЦикла;
	
	РезультатКриптография.Результаты.АдресаСлужбМетокВремени.Результат = СлужбыАдресовМетокВремени.Количество() > 0;
	
	Для Каждого Строка Из СлужбыАдресовМетокВремени Цикл
		Результат = ПолучениеФайловИзИнтернета.СкачатьФайлНаСервере(Строка);
		Если Результат.КодСостояния = 200 Тогда
			РезультатКриптография.Результаты.ДоступностьСлужбМетокВремени.Сервисы.Добавить(Строка);
		КонецЕсли;
	КонецЦикла;
	
	РезультатКриптография.Результаты.ДоступностьСлужбМетокВремени.Результат =
		РезультатКриптография.Результаты.ДоступностьСлужбМетокВремени.Сервисы.Количество() > 0;
...
Процедура ЗаполнитьРезультатыПолученияОтпечатков(ПараметрыВыполненияДиагностики) 
	...
	Если ЕстьКриптографияНаСервере И ЕстьМенеджерКриптографииНаСервере Тогда
		...
		ПараметрыВыполненияДиагностики.РезультатыПолученияОтпечатков = КриптографияБЭД.ПолучитьОтпечаткиСертификатов(
			НСтр("ru = 'Выполнение диагностики ЭДО';
				|en = 'Running EDI diagnostics'"), КонтекстДиагностики,
			ПараметрыВыполненияДиагностики.РезультатыПолученияОтпечатков);
	КонецЕсли;
...
Функция ОтчетДляАдминистратораКриптография(РезультатДиагностикиКриптографии, ПараметрыВыполнения) Экспорт
	...
	ДобавитьПроверку(ПредставленияПроверок, "НепривязанныеСертификаты",
		НСтр("ru = '* Проверка наличия сертификатов, непривязанных к учетным записям';
			|en = '* Checking certificates not bound to accounts'"));
	ДобавитьПроверку(ПредставленияПроверок, "ПроверкиСертификатов",
		НСтр("ru = '* Проверка наличия ошибок сертификатов';
			|en = '* Checking certificate errors'"),, ВложенныеПроверки, "Сертификаты");
	ДобавитьПроверку(ПредставленияПроверок, "АдресаСлужбМетокВремени",
		НСтр("ru = '* Проверка заполнения адресов меток времени';
			|en = '* Check whether timestamp addresses are filled'"));
	ДобавитьПроверку(ПредставленияПроверок, "ДоступностьСлужбМетокВремени",
		НСтр("ru = '* Проверка доступности серверов меток времени';
			|en = '* Check whether timestamp servers are available'"));
...
Процедура ПроверитьВнедрениеБИП(РезультатДиагностики, ПараметрыВыполненияДиагностики) 
	...
	РезультатИнтернетПоддержка.Результаты.ВнедрениеБИП.Рекомендация.Подробная = НСтр("ru = 'Внедрите в конфигурацию библиотеку интернет-поддержки.
		...
		|en = 'Deploy Online Support Library to the configuration.
		|Manual: https://its.1c.ru/db/uisldoc'");
...
Процедура ПроверитьПравильностьЛогинаПароля(РезультатДиагностики, ПараметрыВыполненияДиагностики) 
	...
	Если ЗначениеЗаполнено(Результат.КодОшибки) И Результат.КодОшибки = "НеверныйЛогинИлиПароль" Тогда
		...
		РезультатИнтернетПоддержка.Результаты.КорректностьДанныхИнтернетПоддержки.Рекомендация.Подробная = НСтр("ru = 'Введите корректные данные интернет-поддержки.
			...
			|en = 'Enter correct online support data.
			|Manual: https://its.1c.ru/db/uisldoc#content:172:hdoc:issogl2_подключение_интернет-поддержки'");
	Иначе 
...
Функция ОтчетДляАдминистратораИнтернетПоддержка(РезультатВидаДиагностики, ПараметрыВыполнения)
	...
	ДобавитьПроверку(ПредставленияПроверок, "ВнедрениеБИП", НСтр("ru = '* Проверка наличия библиотеки интернет-поддержки в конфигурации';
																|en = '* Checking for Online Support Library presence in the configuration'"));
	ДобавитьПроверку(ПредставленияПроверок, "КорректностьДанныхИнтернетПоддержки",
		НСтр("ru = '* Проверка правильности логина/пароля интернет-поддержки';
			|en = '* Checking for correct online support login/password'"));
...
Процедура ПроверитьДлинуПутиКаталогаВременныхФайлов(РезультатДиагностики, ПараметрыВыполненияДиагностики) 
	...
	ДлинаПутиКФайлу.Рекомендация.Подробная = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Путь к файлам в каталоге временных файлов должен быть короче %1 символов.
					 ...
					 |en = 'The path to the files in the temporary files directory must be shorter than %1characters.
					 |Change the path to temporary files
					 |to a shorter one in the environment variables of operating system settings.'"),
...
Функция ОтчетДляАдминистратораРаботаСФайлами(РезультатВидаДиагностики, ПараметрыВыполнения)
	...
	ДобавитьПроверку(ПредставленияПроверок, "ДлинаПутиКФайлу", НСтр("ru = '* Проверка длины пути к файлу в каталоге временных файлов';
																	|en = '* Checking the length of the file path in the temporary files directory'"));
...
Функция НовыйРезультатДиагностикиКриптографии() 
	...
	Результаты.Вставить("АдресаСлужбМетокВремени", НовыйРезультатПроверки());
	Результаты.Вставить("ДоступностьСлужбМетокВремени", НовыйРезультатДиагностикиСервисов());
...
Функция ОтчетДляАдминистратора(РезультатВидаДиагностики, ВидДиагностики, ПараметрыВыполнения, ПредставленияПроверок)
	...
	Если ПараметрыВыполнения.ИнформационнаяБазаФайловая И ЗначениеЗаполнено(ПараметрыВыполнения.ИмяКомпьютераКлиент) Тогда
		ПредставлениеКомпьютера = СтрШаблон(НСтр("ru = 'на компьютере ""%1""';
												|en = 'on computer ""%1""'"), ПараметрыВыполнения.ИмяКомпьютераКлиент);
	Иначе
		ИмяКомпьютера = ИмяКомпьютера();
		ПредставлениеКомпьютера = СтрШаблон(НСтр("ru = 'на сервере ""%1""';
												|en = 'on server ""%1""'"), ИмяКомпьютера);
	КонецЕсли;
	
	Шапка = СтрШаблон(НСтр("ru = 'Результаты диагностики %1 %2';
							|en = 'Diagnostics results %1 %2'"), ПредставлениеВидаДиагностики, ПредставлениеКомпьютера);
	...
	ОтчетДляАдминистратора.Вставить("Заголовок", СтрШаблон(НСтр("ru = 'Диагностика %1. Отчет для администратора';
																|en = 'Diagnostics %1. Report for administrator'"), ПредставлениеВидаДиагностики));
