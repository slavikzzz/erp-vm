	...
	Если ТипЗнч(ЛентаНовостейКод) <> ТипСтрока
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'ПолучитьЛентуНовостейПоКоду: параметр [ЛентаНовостейКод] (%1) недопустимого типа [%2] или пустая строка или строка длиной более 20 символов [%3].';
				|en = 'ПолучитьЛентуНовостейПоКоду: the [ЛентаНовостейКод] parameter (%1) of an invalid type [%2] or an empty string, or a string of over 20 characters long [%3].'"),
				ЛентаНовостейКод,
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка получения имени пользователя ИБ (ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()) по причине:
				|%1';
				|en = 'An error occurred while receiving IB username (ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()) as:
				|%1'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Разное';
				|en = 'ISL:News.Miscellaneous'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Разное. Ошибка работы с пользователями';
				|en = 'News. Miscellaneous. User operation error'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка получения пользователя из справочника Пользователи по его имени (%1) в ИБ (ОбработкаНовостейПовтИсп.ПолучитьПользователяПоИмениПользователяИБ()) по причине:
					|%2';
					|en = 'An error occurred while receiving a user from the Users catalog by their name (%1) in the infobase (ОбработкаНовостейПовтИсп.ПолучитьПользователяПоИмениПользователяИБ()). Reason:
					|%2'"),
				ИмяПользователяИБ,
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка поиска пользователя (%1) по уникальному идентификатору:
						...
						|Причина:
						|%5';
						|en = 'An error occurred when searching for the user (%1) by a UUID:
						|Short username: %2
						|Full username: %3
						|UUID: %4
						| Reason:
						|%5'"),
					?(ПолныеПрава = Истина, НСтр("ru = 'доступны полные права';
												|en = 'full rights are available'"), НСтр("ru = 'роль РедакторНовостей';
																					|en = 'role РедакторНовостей'")),
