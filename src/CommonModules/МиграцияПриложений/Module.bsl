...
Процедура НачатьВыгрузку(АдресПриложения, Логин, Пароль, ВыгружатьНастройкиПользователей, ЗавершитьМиграциюАвтоматически, ДополнительныеСвойства) Экспорт
	...
	Попытка
		...
		НовыйУзел.Наименование = СтрШаблон(НСтр("ru = 'Миграция в %1 (%2)';
												|en = 'Migration in %1 (%2)'"), Метаданные.Представление(), АдресПриложения);
		...
		Если ЗначениеЗаполнено(Запись.УзелОбмена) Тогда
			...
			Исключение
				...
				СообщениеПользователю.Текст = СтрШаблон(
					ШаблонСообщения, ТехнологияСервиса.КраткийТекстОшибки(ИнформацияОбОшибке()));
...
// ВидПриложения - Структура
//
Процедура НачатьЗагрузку(ПользователиОбласти, ВидПриложения = Неопределено) Экспорт
	...
	Попытка
		...
		СостояниеЗагрузки.ВидПриложения = Новый ХранилищеЗначения(ВидПриложения);
...
Процедура ЗаданиеВыгрузка(ЗавершитьМиграцию = Ложь) Экспорт
	...
	Если Не СостояниеВыгрузки.Выбран() Тогда
		...
		РегистрацияОшибки(НСтр("ru = 'Выгрузка';
								|en = 'Export'", ОбщегоНазначения.КодОсновногоЯзыка()), ПредставлениеОшибки);
		РегламентныеЗаданияСервер.УдалитьЗадание(Метаданные.РегламентныеЗадания.МиграцияПриложенийВыгрузка);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ПредставлениеОшибки = ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке());
		Шаблон = НСтр("ru = 'Не удалось получить состояние загрузки, возможно сервис временно недоступен: %1 %2';
						|en = 'Cannot get import status, the service might be temporarily unavailable: %1 %2'", ОбщегоНазначения.КодОсновногоЯзыка());
		Комментарий = СтрШаблон(Шаблон, Символы.ПС, ПредставлениеОшибки);
		РегистрацияПримечания(НСтр("ru = 'Выгрузка';
									|en = 'Export'", ОбщегоНазначения.КодОсновногоЯзыка()), Комментарий);
	...
	Исключение
		...
		Если СостояниеВыгрузки.КоличествоПопыток >= 3 Тогда
			...
			ЗавершитьВыгрузкуСОшибкой(СостояниеВыгрузки,
				ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке), 
				ТехнологияСервиса.КраткийТекстОшибки(ИнформацияОбОшибке));
			
		Иначе
			
			СостояниеВыгрузки.Комментарий = ТехнологияСервиса.КраткийТекстОшибки(ИнформацияОбОшибке);
			СостояниеВыгрузки.Записать();
...
Процедура ЗаданиеЗагрузка() Экспорт
	...
	Если Не СостояниеЗагрузки.Выбран() Тогда
		Комментарий = НСтр("ru = 'Состояние загрузки не найдено. Загрузка прервана';
							|en = 'Import state not found. Import canceled'", ОбщегоНазначения.КодОсновногоЯзыка());
		РегистрацияОшибки(НСтр("ru = 'Загрузка';
								|en = 'Import'", ОбщегоНазначения.КодОсновногоЯзыка()), Комментарий);
		УдалитьОчередь();
	...
	Если Не Файл.Существует() Тогда
		ПодробноеПредставлениеОшибки = СтрШаблон(НСтр("ru = 'Файл сообщения %1 не найден.';
														|en = '%1 message file not found.'"), Выборка.ИмяФайла);
	...
	Исключение
		...
		Если СостояниеЗагрузки.КоличествоПопыток >= 3 Тогда
			ЗавершитьЗагрузкуСОшибкой(СостояниеЗагрузки, 
				ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке), 
				ТехнологияСервиса.КраткийТекстОшибки(ИнформацияОбОшибке));
		Иначе 
			РегистрацияОшибки(НСтр("ru = 'Загрузка';
									|en = 'Import'", ОбщегоНазначения.КодОсновногоЯзыка()), 
				ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке));
			...
			СостояниеЗагрузки.ОписаниеОшибки = ТехнологияСервиса.КраткийТекстОшибки(ИнформацияОбОшибке);
			СостояниеЗагрузки.Записать();
		КонецЕсли;
	...
	Исключение
		ПредставлениеОшибки = ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке());
		РегистрацияОшибки(НСтр("ru = 'Загрузка';
								|en = 'Import'", ОбщегоНазначения.КодОсновногоЯзыка()), ПредставлениеОшибки);
	КонецПопытки;
	...
	Для Каждого Состав Из Метаданные.ПланыОбмена.МиграцияПриложений.Состав Цикл
		Если Состав.АвтоРегистрация = АвтоРегистрацияИзменений.Разрешить Тогда
			ВызватьИсключение НСтр("ru = 'Некорректно настроен план обмена ""Миграция приложений"": авторегистрация должна быть выключена.';
									|en = 'The ""Application migration"" exchange plan is set up incorrectly: auto registration must be disabled.'");
		КонецЕсли;
...
Функция АбонентыПользователя(ИсточникПараметровДоступа) Экспорт
	...
	Попытка
		...
		КонецЦикла;
	Исключение
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Неверный формат ответа внешнего программного интерфейса: ""%1""';
...
Функция ВызватьМетодПрограммногоИнтерфейса(ИсточникПараметровДоступа, ПараметрыМетода, Метод = Неопределено) Экспорт
	...
	СтруктураURI = ОбщегоНазначенияКлиентСервер.СтруктураURI(ПараметрыДоступа.АдресПрограммногоИнтерфейса);
	
	АдресРесурса = СтруктураURI.ПутьНаСервере + "/execute";
	...
	Соединение = Новый HTTPСоединение(СтруктураURI.Хост, СтруктураURI.Порт,
		ПараметрыДоступа.Логин, ПараметрыДоступа.Пароль,
		ПолучениеФайловИзИнтернета.ПолучитьПрокси(СтруктураURI.Схема), 30, ЗащищенноеСоединение);
...
Функция СервисПоддерживаетМиграцию(ИмяСервера, АдресПрограммногоИнтерфейса, АдресРегистрации, АдресВосстановления, РегистрацияРазрешена) Экспорт
	...
	ЗащищенноеСоединение = ОбщегоНазначенияКлиентСервер.НовоеЗащищенноеСоединение(, Новый СертификатыУдостоверяющихЦентровОС);
	Соединение = Новый HTTPСоединение(ИмяСервера, , , , ПолучениеФайловИзИнтернета.ПолучитьПрокси("https"), 30, ЗащищенноеСоединение);
	...
	Исключение
		...
		ОбщегоНазначения.СообщитьПользователю(
			СтрШаблон(Шаблон, ТехнологияСервиса.КраткийТекстОшибки(ИнформацияОбОшибке())));
		Возврат Ложь;
	КонецПопытки;
...
//
Функция СоздатьПриложениеДляМиграции(ИсточникПараметровДоступа, Наименование, ЧасовойПояс, ПраваПользователей, 
		РасширенияДляВосстановления = Неопределено) Экспорт
	...
	Если РасширенияДляВосстановления <> Неопределено И ПараметрыДоступа.ВерсияПрограммногоИнтерфейса >= 24 Тогда
		ПараметрыМетода.general.version = 24;
		Расширения = Новый Массив;
		Для Каждого СтрокаРасширение Из РасширенияДляВосстановления Цикл
			Расширения.Добавить(Новый Структура("id, version", СтрокаРасширение.Имя, СтрокаРасширение.Версия));
		КонецЦикла;
		ПараметрыМетода.Вставить("extensions", Расширения);
	КонецЕсли;
	
	ПараметрыМетода.Вставить("id", ПараметрыДоступа.КодАбонента);
	ПараметрыМетода.Вставить("application", ВидыПриложенийСервер.ИмяТекущегоВидаПриложения());
...
Функция ДанныеОбслуживающейОрганизации(ИсточникПараметровДоступа) Экспорт
	...
	Исключение
		...
		Результат.ОписаниеОшибки = ТехнологияСервиса.КраткийТекстОшибки(ИнформацияОбОшибке());
...
Функция Зарегистрироваться(ИмяСервера, Наименование, Логин, ЭлектроннаяПочта, Пароль, Телефон) Экспорт
	...
	Если Ответ.КодСостояния <> 200 Тогда
		ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Не удалось выполнить запрос, код ответа: %1';
															|en = 'Cannot run the query, response code: %1'"), Ответ.КодСостояния));
		Возврат Ложь;
	КонецЕсли;
...
Функция Описание(ИдентификаторХранилища, ИдентификаторДанных) Экспорт
	...
	Иначе
		...
		Результат.Комментарий = СтрШаблон(НСтр("ru = 'Неподдерживаемый идентификатор данных: %1.';
												|en = 'Unsupported data ID: %1.'"), ИдентификаторДанных);
	КонецЕсли;
...
Функция Загрузить(ОписаниеДанных) Экспорт
	...
	Если ОписаниеДанных.ИмяФайла = "message" Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			РегистрацияОшибки(НСтр("ru = 'Загрузка сообщения';
									|en = 'Message import'", ОбщегоНазначения.КодОсновногоЯзыка()), 
				ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке));
			Результат.Успешно = Ложь;
			Результат.Комментарий = ТехнологияСервиса.КраткийТекстОшибки(ИнформацияОбОшибке);
		КонецПопытки;
	...
	Иначе
		...
		Результат.Успешно = Ложь;
		Результат.Комментарий = СтрШаблон(НСтр("ru = 'Загрузка файла ""%1"" не поддерживается.';
												|en = 'Cannot import the ""%1"" file.'"), ОписаниеДанных.ИмяФайла);
...
Процедура УдалитьВременныхПользователей() Экспорт
	...
	Для Каждого ПользовательИБ Из ВсеПользователи Цикл
		Если ПользовательИБ.Имя = "migration" И ПользовательИБ.РазделениеДанных.Свойство("ОбластьДанныхОсновныеДанные") Тогда
...
// Получить данные измененных объектов.
// 
// Параметры:
//  Узел - ПланОбменаСсылка.МиграцияПриложений - Узел плана обмена
// 
// Возвращаемое значение:
//  Структура - Данные измененных объектов:
//  * ТаблицаОбъектов - ТаблицаЗначений - Таблица измененных объектов:
//     ** ПолноеИмя - Строка - Полное имя объекта метаданных
//     ** КоличествоОбъектов - Число - Количество объектов
//  * КоличествоОбъектов - Число - Количество измененных объектов
Функция ПолучитьДанныеИзмененныхОбъектов(Узел) Экспорт
	
	ТаблицаОбъектов = Новый ТаблицаЗначений();
	ТаблицаОбъектов.Колонки.Добавить(
		"ПолноеИмя",
		Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(256, ДопустимаяДлина.Переменная)));
	ТаблицаОбъектов.Колонки.Добавить(
		"КоличествоОбъектов",
	...
	ТекстыЗапросов = ТекстыЗапросовИзмененныхОбъектов();
	...
	Для Каждого ТекстЗапроса Из ТекстыЗапросов Цикл
		...
		Если Выборка.Следующий() Тогда
			ЗаполнитьЗначенияСвойств(ТаблицаОбъектов.Добавить(), Выборка);
		КонецЕсли;
		
	КонецЦикла;
	
	Результат = Новый Структура();
	Результат.Вставить("ТаблицаОбъектов", ТаблицаОбъектов);
	Результат.Вставить("КоличествоОбъектов", ТаблицаОбъектов.Итог("КоличествоОбъектов"));
...
Функция ЗагрузитьДанныеИзКаталога(КаталогЗагрузки, СоответствиеСсылок, ОбъектыДляОчистки)
	...
	Если ВидыПриложенийСервер.СвойстваВидаПриложенияПоИмени(Манифест.ИмяКонфигурации, Ложь) = Неопределено Тогда
	...
	СостояниеЗагрузки = РегистрыСведений.МиграцияПриложенийСостояниеЗагрузки.СоздатьМенеджерЗаписи();
	СостояниеЗагрузки.Прочитать();
	СвойстваВидаПриложения = СостояниеЗагрузки.ВидПриложения.Получить();
	
	ИмяВидаПриложения = ВидыПриложенийСервер.БазовыйВидПриложения().Имя;
	Если СвойстваВидаПриложения <> Неопределено Тогда
		ИмяВидаПриложения = СвойстваВидаПриложения.Name;
	КонецЕсли;
	
	Если Манифест.ИмяКонфигурации <> ИмяВидаПриложения Тогда
		ВызватьИсключение НСтр("ru = 'Имя конфигурации не совпадает.';
								|en = 'Configuration name does not match.'");
	КонецЕсли;
	...
	Если Манифест.Свойство("ОбъектыВыгруженныеПолностью", ОбъектыВыгруженныеПолностью) Тогда
		Шаблон = НСтр("ru = 'Не удалось выгрузить порциями объекты: %1.';
						|en = 'Cannot export objects in portions: %1.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ВыгрузитьДанныеВКаталог(КаталогВыгрузки, УзелОбмена, НомерСообщения, ПоследнийОбъектМетаданных, ВыгружатьНастройкиПользователей, ЗавершитьМиграциюАвтоматически)
	...
	Если ВыгруженоОбъектов < ПараметрыВыгрузки.РазмерПорцииДанных Тогда
		Если ЗавершитьМиграциюАвтоматически Тогда
			...
			Исключение
				Комментарий = НСтр("ru = 'Не удалось установить монопольный режим.';
									|en = 'Cannot enable exclusive mode.'", ОбщегоНазначения.КодОсновногоЯзыка());
				РегистрацияПримечания(НСтр("ru = 'Выгрузка';
											|en = 'Export'"), Комментарий);
			КонецПопытки;
		КонецЕсли;
	...
	Манифест.Вставить("ИмяКонфигурации", ВидыПриложенийСервер.ИмяТекущегоВидаПриложения());
...
Процедура ЗагрузитьСообщение(Источник)
	...
	Если Выборка.Следующий() И Выборка.ОчередьЗагрузки >= МаксимальнаяОчередьЗагрузки() Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Очередь загрузки больше %1';
										|en = 'Import queue is more than %1'"), МаксимальнаяОчередьЗагрузки());
	КонецЕсли;
...
Функция ВыгрузитьПользовательскиеДанные(УзелОбмена, НомерСообщения, ПоследнийОбъектМетаданных, ПараметрыВыгрузки)
	...
	ШаблонНачалоВыгрузки = НСтр("ru = 'Выгрузка объекта: %1';
								|en = 'Exporting object: %1'", ОбщегоНазначения.КодОсновногоЯзыка());
	ШаблонОкончаниеВыгрузки = НСтр("ru = 'Выгружен объект: %1, Количество: %2, Длительность (сек): %3';
									|en = 'Exported object: %1, Quantity: %2, Duration (sec): %3'", ОбщегоНазначения.КодОсновногоЯзыка());
	ВсегоОбъектов = 0;
	...
	Для Каждого ОбъектМетаданных Из ПорядокВыборкиМетаданных(ПоследнийОбъектМетаданных) Цикл
		...
		РегистрацияПримечания(НСтр("ru = 'Статистика выгрузки';
									|en = 'Export statistics'"), Комментарий);
		...
		Если ТипЗнч(Объект) <> Тип("УдалениеОбъекта") Тогда
			...
			Если ЭтоСправочникФайлов И Не Объект.ЭтоГруппа Тогда
				...
				Исключение
					...
					РегистрацияОшибки(
						НСтр("ru = 'Выгрузка справочника файлов';
							|en = 'Export file catalog'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ВыгрузитьНаборЗаписей(ОбъектМетаданных, Узел, НомерСообщения, ОсталосьВыгрузить, ПараметрыВыгрузки) Экспорт
	...
	Пока Истина Цикл
		...
		Пока ВыборкаДанных.Следующий() Цикл
			...
			Если ТипЗнч(Объект) <> Тип("УдалениеОбъекта") И РеквизитыСХранилищем.Количество() Тогда
	...
	Для Каждого КлючИЗначение Из СсылкиПоТипам Цикл
		...
		Для Каждого Ссылка Из КлючИЗначение.Значение Цикл
			...
			Если ПараметрыВыгрузки.ТипыСопоставляемыхОбщихДанныхПоПредопределенномуИмени[КлючИЗначение.Ключ] <> Неопределено
				...
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Обнаружена ссылка на общие данные без поля поиска. Тип данных: %1';
												|en = 'Reference to common data without search field was found. Data type: %1'"), Объект.Метаданные().ПолноеИмя());
			КонецЕсли;
...
Функция ЗагрузитьПользовательскиеДанные(ПараметрыЗагрузки)
	...
	ШаблонНачалоЗагрузки = НСтр("ru = 'Загружается объект: %1';
								|en = 'Object is being imported: %1'", ОбщегоНазначения.КодОсновногоЯзыка());
	ШаблонОкончаниеЗагрузки = НСтр("ru = 'Загружен объект: %1, Количество: %2, Длительность (сек): %3';
									|en = 'Imported object: %1, Quantity: %2, Duration (sec): %3'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Для Каждого СтрокаДанных Из ПараметрыЗагрузки.ТаблицаДанных Цикл
		...
		РегистрацияПримечания(НСтр("ru = 'Статистика загрузки';
									|en = 'Import statistics'"), Комментарий);
...
Функция ЗагрузитьСсылочныйТип(ОбъектМетаданных, Идентификатор, ПараметрыЗагрузки)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Или ЧтениеXML.Имя <> "Data" Тогда
		ВызватьИсключение(НСтр("ru = 'Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента ""Data"".';
								|en = 'An error occurred while reading XML. Invalid file format. Start of the Data item is expected.'"));
	КонецЕсли;	
	
	ПоддерживаютсяПредопределенные = 
		ОбщегоНазначенияБТС.ЭтоСсылочныеДанныеПоддерживающиеПредопределенныеЭлементы(ОбъектМетаданных);
	Иерархический = ПоддерживаютсяПредопределенные И Не ОбщегоНазначенияБТС.ЭтоПланВидовРасчета(ОбъектМетаданных) 
	...
	Пока ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		Объект.ОбменДанными.Загрузка = Истина;
		Объект.ДополнительныеСвойства.Вставить("ОтключитьМеханизмРегистрацииОбъектов");
		Дубли = ДублиПредопределенных(Объект, ОбъектМетаданных, ПоддерживаютсяПредопределенные);
		
		НачатьТранзакцию();
		Попытка
			
			Для Каждого ДубльПредопределенных Из Дубли Цикл
				Если Иерархический Тогда
					ЗаменитьСсылкиНаРодителя(ДубльПредопределенных, Объект.ПолучитьСсылкуНового(), ОбъектМетаданных);
				КонецЕсли;
				ДубльПредопределенныхОбъект = ДубльПредопределенных.ПолучитьОбъект(); 
...
Процедура ЗаменитьСсылкиНаРодителя(Исходная, Замена, ОбъектМетаданных)
	Запрос = Новый Запрос;
	Запрос.Текст = СтрЗаменить("ВЫБРАТЬ
		...
		|ГДЕ 
		|	Т.Родитель = &Ссылка
		|	И Т.Предопределенный",
		"&Таблица",
		ОбъектМетаданных.ПолноеИмя());
	Запрос.УстановитьПараметр("Ссылка", Исходная);
	Подчиненные = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	Для Каждого Подчиненный Из Подчиненные Цикл
		ПодчиненныйОбъект = Подчиненный.ПолучитьОбъект();
		ПодчиненныйОбъект.Родитель = Замена;
...
Функция ДублиПредопределенных(Объект, ОбъектМетаданных, ПоддерживаютсяПредопределенные)
	Дубли = Новый Массив;
	
	Если ПоддерживаютсяПредопределенные И Объект.Предопределенный Тогда
...
Процедура ЗагрузитьНастройкиПользователей(ХранилищеНастроек, ИмяФайла, ПараметрыЗагрузки)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Или ЧтениеXML.Имя <> "Настройки" Тогда
		ВызватьИсключение(НСтр("ru = 'Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента ""Настройки"".';
								|en = 'Error reading XML. Incorrect file format. Settings item start is expected.'"));
	КонецЕсли;
...
Функция ТипПоляПоиска(ОбъектМетаданных, ИмяПоля) Экспорт
	...
	ВызватьИсключение СтрШаблон(НСтр("ru = 'Поле %1 не может использоваться в качестве поля естественного ключа объекта %2:
		...
		|the object field is not found'", ОбщегоНазначения.КодОсновногоЯзыка()),
		ИмяПоля,
		ОбъектМетаданных.ПолноеИмя());
...
Процедура ОчиститьПользовательскиеДанные(ОбъектыДляОчистки) Экспорт
	...
	МетаданныеДляИнициализацииПредопределенных = Новый Массив;
	Для Каждого КлючИЗначение Из ОбъектыДляОчистки Цикл
		...
		ИначеЕсли Метаданные.Справочники.Содержит(ОбъектМетаданных)
			...
			ОчиститьСсылочныйТип(ОбъектМетаданных);
			МетаданныеДляИнициализацииПредопределенных.Добавить(ОбъектМетаданных);
			
		ИначеЕсли Метаданные.Задачи.Содержит(ОбъектМетаданных) 
			Или Метаданные.Документы.Содержит(ОбъектМетаданных)
			Или Метаданные.БизнесПроцессы.Содержит(ОбъектМетаданных) Тогда
	...
	Для Каждого ОбъектСПредопределенными Из МетаданныеДляИнициализацииПредопределенных Цикл
		Менеджер = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ОбъектСПредопределенными.ПолноеИмя());
...
Функция ПолучитьСсылкуПриЗагрузку(ТипЗначения, СтароеЗначение, ПараметрыЗагрузки)
	...
	ИначеЕсли ПараметрыЗагрузки.ЗапрещенныеТипыОбщихДанных[ТипЗначения] <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(СтараяСсылка) Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'В объекте обнаружена недопустимая ссылка. Тип данных: %1';
											|en = 'Invalid link is found in the object. Data type: %1'"), СтараяСсылка.Метаданные().ПолноеИмя());
		КонецЕсли;
...
Процедура ЗаполнитьСостояниеВыгрузки(СостояниеВыгрузки, СостояниеЗагрузки, УзелОбмена)
	...
	Если СостояниеЗагрузки.ЗавершеноСОшибками Тогда
		...
		СостояниеВыгрузки.Комментарий = НСтр("ru = 'При загрузке сообщения возникла ошибка:';
											|en = 'An error occurred when importing the message:'") + Символы.ПС + СостояниеЗагрузки.ОписаниеОшибки;
	КонецЕсли;
...
Функция КоличествоИзмененных(Узел) Экспорт
	...
	Для Каждого ТекстЗапроса Из ТекстыЗапросов Цикл
		...
		Если Выборка.Следующий() Тогда
			КоличествоОбъектов = КоличествоОбъектов + Выборка.КоличествоОбъектов;
		КонецЕсли;
...
// Тексты запросов измененных объектов.
// 
// Возвращаемое значение:
//  Массив из Строка - Тексты запросов измененных объектов
Функция ТекстыЗапросовИзмененныхОбъектов()
	...
	Для Каждого Состав Из МиграцияПриложенийПовтИсп.ВыгружаемыеОбъекты() Цикл
		...
		Если Не ОбъектРасширенияВходитВСоставПланаОбмена(ОбъектМетаданных) Тогда
			Продолжить;
		КонецЕсли;
		
		ПолноеИмяОбъектаМетаданных = ОбъектМетаданных.ПолноеИмя();
		ИмяТаблицы = СтрЗаменить(ПолноеИмяОбъектаМетаданных, ".Перерасчет.", ".") + "." + "Изменения";
...
Процедура ЗавершитьВыгрузкуСОшибкой(СостояниеВыгрузки, ПодробноеПредставлениеОшибки, КраткоеПредставлениеОшибки)
	...
	РегистрацияОшибки(НСтр("ru = 'Выгрузка';
							|en = 'Export'", ОбщегоНазначения.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки);
...
Процедура ЗавершитьЗагрузкуСОшибкой(СостояниеЗагрузки, ПодробноеПредставлениеОшибки, КраткоеПредставлениеОшибки)
	...
	РегистрацияОшибки(НСтр("ru = 'Загрузка';
							|en = 'Import'", ОбщегоНазначения.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки);
...
Процедура ЗавершитьЗагрузку(СостояниеЗагрузки)
	...
	ВидПриложения = СостояниеЗагрузки.ВидПриложения.Получить();
	ВидыПриложенийСлужебный.УстановитьВидПриложенияПоСообщению(ВидПриложения);
	...
	КонецЕсли;
	РаботаВМоделиСервисаБТС.ОбновитьКодПредопределенныхУзлов();
...
Процедура РегистрацияОшибки(Событие, Комментарий, ОбъектМетаданных = Неопределено, Данные = Неопределено)
	...
	ИмяСобытия = СтрШаблон(НСтр("ru = 'Миграция приложений.%1';
								|en = 'Application migration.%1'", ОбщегоНазначения.КодОсновногоЯзыка()), Событие);
...
Процедура ВызватьИсключениеНеизвестныйОбъектМетаданных(Знач ОбъектМетаданных) Экспорт
	...
	ВызватьИсключение СтрШаблон(НСтр("ru = 'Неизвестный объект метаданных: %1';
									|en = 'Unknown metadata object: %1'"), ОбъектМетаданных.ПолноеИмя());
