...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	//++ НЕ УТ
	Если Не Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваровУПереработчика Тогда
	//-- НЕ УТ
...
#Область ОтражениеПревышенияЦеныНадФактическойСебестоимостью // (Дт ХХ.ХХ :: Кт 91.01)
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Отражение превышения прайсовой цены над фактической себестоимостью товара (Дт ХХ.ХХ :: Кт 91.01)
	...
	|	КОНЕЦ + Стоимости.СуммаПР КАК СуммаПРКт,
	|	Стоимости.СуммаВР КАК СуммаВРКт,
	|	&ТекстОтражениеПревышения КАК Содержание
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.ОтчетОСписанииТоваровУХранителя КАК Операция
	|	ПО
	...
	|		И Стоимости.РазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаХраненииСПравомПродажиПереданныеПартнерам)
	// критерий того, что отклонения отражаем в доходах
	|		И Стоимости.НастройкаХозяйственнойОперации = ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ОтклонениеВСтоимостиТоваровДоходы)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ТекстОтражениеПревышения", """"+ НСтр("ru = 'Отражение превышения прайсовой цены над фактической себестоимостью товара';
												|en = 'Display the price excess over the actual inventory cost'") + """");
	
	ТекстыОтражения.Добавить(ТекстЗапроса);
#КонецОбласти

#Область ОтражениеПревышенияФактическойСебестоимостьюНадЦеной // (Дт 91.02 :: Кт ХХ.ХХ)
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Отражение превышения фактической себестоимости товара над прайсовой ценой (Дт 91.02 :: Кт ХХ.ХХ)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	"""" КАК ИдентификаторСтроки,
	...
	|		ТОГДА 0
	|		ИНАЧЕ Стоимости.СуммаНУ
	|	КОНЕЦ + Стоимости.СуммаПР КАК СуммаПРДт,
	|	Стоимости.СуммаВР КАК СуммаВРДт,
	|
	...
	|		И Стоимости.РазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаХраненииСПравомПродажиПереданныеПартнерам)
	// критерий того, что отклонения отражаем в доходах
	|		И Стоимости.НастройкаХозяйственнойОперации = ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ОтклонениеВСтоимостиТоваровРасходы)
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ТекстОтражениеПревышения", """"+ НСтр("ru = 'Отражение превышения фактической себестоимости товара над прайсовой ценой';
												|en = 'Display of actual inventory cost excess over the price'") + """");
...
#КонецОбласти
	...
	ТекстыОтражения.Добавить(ТекстСписаниеТоваровНаРасходы()); // Списание на расходы собственных товаров (Дт 20, 2Х, 44, 91.02 :: Кт 20, 41.01, 45.01)
	ТекстыОтражения.Добавить(ТекстСписаниеТоваровНаСтатьиАктивовПассивов()); // Списание на прочие активы собственных товаров (Дт ХХ.ХХ :: Кт 20, 41.01, 45.01)
	ТекстыОтражения.Добавить(ТекстПеремещениеТоваровЗаБалансом()); // Списание товаров, принятых на ответственное хранение (Дт 002, 003, 004.01 :: Кт 002, 003, 004.01, 004.02)
...
Функция ТекстЗапросаТаблицаПрослеживаемыеТоварыВСоставеОС(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтВидыЗапасов", ТекстыЗапроса) Тогда
		Документы.ОтчетОСписанииТоваровУХранителя.ТекстЗапросаВтВидыЗапасов(Запрос, ТекстыЗапроса);
	КонецЕсли;
	...
	|	&АналитикаРасходов							КАК ОсновноеСредство
	|ИЗ
	...
	|			ТОГДА &СтатьяАктивовПассивов = ЗНАЧЕНИЕ(Перечисление.ТипыРасходов.ФормированиеСтоимостиВНА)
	|		ИНАЧЕ &СтатьяРасходов = ЗНАЧЕНИЕ(Перечисление.ТипыРасходов.ФормированиеСтоимостиВНА)
	|	КОНЕЦ	
	|	И ТИПЗНАЧЕНИЯ(&АналитикаРасходов)= ТИП(Справочник.ОбъектыЭксплуатации)
	...
	|	ТаблицаВидыЗапасов.Количество * ПрослеживаемыеКомплектующие.КоличествоПоРНПТ КАК КоличествоПоРНПТ,
	|	ТаблицаВидыЗапасов.Номенклатура				КАК Номенклатура,
	|	ТаблицаВидыЗапасов.Характеристика			КАК Характеристика,
	|	&АналитикаРасходов							КАК ОсновноеСредство
	|ИЗ
...
Функция ТекстСписаниеТоваровНаРасходы()
	...
	ТекстЗапроса = ПроизводствоБезЗаказаЛокализация.ТекстСписаниеТоваровНаРасходы(, Ложь, Истина);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборПоУмолчанию", "НЕ Строки.НастройкаХозяйственнойОперации В
	|			(ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ОтклонениеВСтоимостиТоваровДоходы),
	|			ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ОтклонениеВСтоимостиТоваровРасходы))
	|			И Строки.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Содержание", """Списание товаров на расходы""");
...
Функция ТекстПеремещениеТоваровЗаБалансом()
	...
	|		ТОГДА ""Списание товаров у хранителя""
