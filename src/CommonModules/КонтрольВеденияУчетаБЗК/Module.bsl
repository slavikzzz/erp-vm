	...
	Если ДекорацияГруппа <> Неопределено Тогда
		...
		Для НомерПроблемы = 1 По ТаблицаПроблем.Количество() Цикл
			...
			Иначе
				...
				УточнениеПроблемы = Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ru = 'Еще проблемы (%1)';
																				|en = 'More problems (%1)'"), ОставшеесяКоличествоПроблем), , , , "Основной");
		...
		Если ТаблицаПроблем.Количество() < 4 Тогда
			...
			ДекорацияНадпись.Заголовок                    = Новый ФорматированнаяСтрока(НСтр("ru = 'Отчет по проблемам';
																							|en = 'Report on problems'"),,,, "Основной");
