...
Функция ЗаполнитьЗаписиСкладскогоЖурнала(Товары, ХозяйствующийСубъект, Предприятие, СписокСтрок, ПараметрыЗаполнения) Экспорт
	...
	РезультатЗаполнения.Вставить("СообщениеПользователю", Неопределено);
	...
	Для каждого СтрокаДокумента Из Товары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДокумента.ЗаписьСкладскогоЖурнала)
			ИЛИ (ПараметрыЗаполнения.ЭтоИнвентаризация
				И СтрокаДокумента.Операция = Перечисления.ОперацииИнвентаризацииПродукцииВЕТИС.Удаление) Тогда
			Продолжить;
		...
		Если НЕ ЗначениеЗаполнено(СтрокаДокумента.Номенклатура) И Не ЗначениеЗаполнено(СтрокаДокумента.Продукция) Тогда
	...
	|	ТаблицаТовары.НоменклатураЗаполнена КАК НоменклатураЗаполнена,
	|	ТаблицаТовары.ПродукцияЗаполнена    КАК ПродукцияЗаполнена,
	|	ТаблицаТовары.СерияЗаполнена        КАК СерияЗаполнена
	...
	|	ТаблицаЗаписи.Характеристика          КАК Характеристика,
	|	ТаблицаЗаписи.Продукция               КАК Продукция,
	|	ТаблицаЗаписи.Серия                   КАК Серия,
	|	ТаблицаЗаписи.ЗаписьСкладскогоЖурнала КАК ЗаписьСкладскогоЖурнала,
	|	ТаблицаТовары.СерияЗаполнена И ТаблицаТовары.Серия = ТаблицаЗаписи.Серия КАК ПолноеСоответствиеПоСерии
	|ПОМЕСТИТЬ ТаблицаСоответствияЗаписиЖурнала
	|ИЗ ТаблицаТовары КАК ТаблицаТовары
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыВЕТИС КАК ТаблицаЗаписи
	|		ПО ТаблицаЗаписи.ЗаписьСкладскогоЖурнала <> ЗНАЧЕНИЕ (Справочник.ЗаписиСкладскогоЖурналаВЕТИС.ПустаяСсылка)
	|		И (Не ТаблицаТовары.НоменклатураЗаполнена
	|			Или (ТаблицаТовары.Номенклатура = ТаблицаЗаписи.Номенклатура
	|				И ТаблицаТовары.Характеристика = ТаблицаЗаписи.Характеристика))
	|		И (Не ТаблицаТовары.СерияЗаполнена
	|			Или ТаблицаТовары.Серия = ТаблицаЗаписи.Серия
	|			Или ТаблицаЗаписи.Серия В (&НезаполненныеЗначенияСерии))
	|		И (Не Не ТаблицаТовары.ПродукцияЗаполнена
	|			Или ТаблицаТовары.Продукция = ТаблицаЗаписи.Продукция)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ПОМЕСТИТЬ ТаблицаСоответствияНоменклатуры
	|ИЗ ТаблицаТовары КАК ТаблицаТовары
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыВЕТИС КАК ТаблицаСоответствия
	|		ПО ТаблицаСоответствия.ЗаписьСкладскогоЖурнала = ЗНАЧЕНИЕ (Справочник.ЗаписиСкладскогоЖурналаВЕТИС.ПустаяСсылка)
	|		И (Не ТаблицаТовары.НоменклатураЗаполнена
	|			Или (ТаблицаТовары.Номенклатура = ТаблицаСоответствия.Номенклатура
	|				И ТаблицаТовары.Характеристика = ТаблицаСоответствия.Характеристика))
	|		И (Не Не ТаблицаТовары.ПродукцияЗаполнена
	|			Или ТаблицаТовары.Продукция = ТаблицаСоответствия.Продукция)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаСоответствияНоменклатуры.Продукция КАК Продукция
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаЗаписи.ЗаписьСкладскогоЖурнала КАК ЗаписьСкладскогоЖурнала
	|ПОМЕСТИТЬ НеподходящиеЗаписиЖурнала
	|ИЗ ТаблицаСоответствияНоменклатуры КАК ТаблицаСоответствияНоменклатуры
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыВЕТИС КАК ТаблицаЗаписи
	|		ПО ТаблицаЗаписи.Номенклатура  = ТаблицаСоответствияНоменклатуры.Номенклатура
	|		И ТаблицаЗаписи.Характеристика = ТаблицаСоответствияНоменклатуры.Характеристика
	|		И ТаблицаЗаписи.Продукция      = ТаблицаСоответствияНоменклатуры.Продукция
	|		И ТаблицаЗаписи.ЗаписьСкладскогоЖурнала <> ЗНАЧЕНИЕ (Справочник.ЗаписиСкладскогоЖурналаВЕТИС.ПустаяСсылка)
	|	ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаСоответствияЗаписиЖурнала КАК ТаблицаСоответствияЗаписиЖурнала
	|		ПО ТаблицаСоответствияЗаписиЖурнала.ЗаписьСкладскогоЖурнала = ТаблицаЗаписи.ЗаписьСкладскогоЖурнала
	|ГДЕ
	|	ТаблицаСоответствияЗаписиЖурнала.ЗаписьСкладскогоЖурнала ЕСТЬ NULL
	|ИНДЕКСИРОВАТЬ ПО
	|	ЗаписьСкладскогоЖурнала
	|;
	|
	...
	|	ТаблицаОстатков.ЗаписьСкладскогоЖурнала В (ВЫБРАТЬ ЗаписьСкладскогоЖурнала ИЗ ТаблицаСоответствияЗаписиЖурнала) КАК ТочноеСоответствие
	...
	|	И НЕ ТаблицаОстатков.ЗаписьСкладскогоЖурнала В (ВЫБРАТЬ ЗаписьСкладскогоЖурнала ИЗ НеподходящиеЗаписиЖурнала)
	|ИНДЕКСИРОВАТЬ ПО
	|	ЗаписьСкладскогоЖурнала
	|;
	|
	...
	|	ТаблицаОстатков.ТочноеСоответствие КАК ТочноеСоответствие
	|ИЗ
	...
	|	ТаблицаСоответствия.ПолноеСоответствиеПоСерии КАК ПолноеСоответствиеПоСерии
	|
	|ИЗ
	|	ТаблицаСоответствияЗаписиЖурнала КАК ТаблицаСоответствия
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ЗНАЧЕНИЕ(Справочник.ЗаписиСкладскогоЖурналаВЕТИС.ПустаяСсылка),
	|	Ложь
	|ИЗ
	|	ТаблицаСоответствияНоменклатуры КАК ТаблицаСоответствия
	|
	|УПОРЯДОЧИТЬ ПО
	|	ТаблицаСоответствия.Продукция,
	|	ТаблицаСоответствия.Номенклатура,
	|	ТаблицаСоответствия.Характеристика,
	|	ТаблицаСоответствия.Серия Убыв
	|";
	...
	СписокКолонок = "Номенклатура, Характеристика, Продукция, Серия, ЗаписьСкладскогоЖурнала, ЕдиницаИзмеренияВЕТИС, "
		+ ПараметрыЗаполнения.ИмяПоляКоличествоВЕТИС;
	ТаблицаТовары = Товары.Выгрузить(, СписокКолонок);
	ТаблицаТовары.Колонки[ПараметрыЗаполнения.ИмяПоляКоличествоВЕТИС].Имя = "КоличествоВЕТИС";
	ТаблицаТовары.Колонки.Добавить("НоменклатураЗаполнена", Новый ОписаниеТипов("Булево"));
	ТаблицаТовары.Колонки.Добавить("СерияЗаполнена",        Новый ОписаниеТипов("Булево"));
	ТаблицаТовары.Колонки.Добавить("ПродукцияЗаполнена",    Новый ОписаниеТипов("Булево"));
	Для Каждого СтрокаТаблицы Из ТаблицаТовары Цикл
		СтрокаТаблицы.НоменклатураЗаполнена = ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура);
		СтрокаТаблицы.СерияЗаполнена = ЗначениеЗаполнено(СтрокаТаблицы.Серия);
		СтрокаТаблицы.ПродукцияЗаполнена = ЗначениеЗаполнено(СтрокаТаблицы.ПродукцияЗаполнена);
	КонецЦикла;
	...
	ТаблицаОстатки.Индексы.Добавить("Продукция");
...
Процедура ЗаполнитьВСтрокахЗаписиСкладскогоЖурнала(СписокСтрокДокумента, ПараметрыЗаполнения, РезультатЗаполнения, СтрокиДляЗаполнения)
	...
	Для каждого СтрокаДокумента Из СписокСтрокДокумента Цикл
		...
		Если ОсталосьЗаполнить > 0 Тогда
			...
			// а затем по соответствию продукции и номенклатуры.
			
			СтруктураПоиска = Новый Структура;
			СтруктураПоиска.Вставить("Номенклатура",   СтрокаДокумента.Номенклатура);
			СтруктураПоиска.Вставить("Характеристика", СтрокаДокумента.Характеристика);
			СтруктураПоиска.Вставить("Серия",          ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа(Метаданные.ОпределяемыеТипы.СерияНоменклатурыВЕТИС));
			Если ЗначениеЗаполнено(СтрокаДокумента.Продукция) Тогда
				СтруктураПоиска.Вставить("Продукция", СтрокаДокумента.Продукция);
			КонецЕсли;
			СписокСтрокСоответствия = ТаблицаСоответствия.НайтиСтроки(СтруктураПоиска);
...
Процедура ЗаполнитьЗаписиСкладскогоЖурналаПоСтрокеДокументаИСоответствию(СтрокаДокумента, СписокСтрокСоответствия, ПараметрыЗаполнения, ОсталосьЗаполнить, РезультатЗаполнения)
	...
	Для каждого СтрокаСоответствие Из СписокСтрокСоответствия Цикл
		Если ЗначениеЗаполнено(СтрокаСоответствие.ЗаписьСкладскогоЖурнала) Тогда
			СтруктураПоиска = Новый Структура;
	...
	Для каждого СтрокаСоответствие Из СписокСтрокСоответствия Цикл
		Если Не ЗначениеЗаполнено(СтрокаСоответствие.ЗаписьСкладскогоЖурнала) Тогда
		СтруктураПоиска = Новый Структура;
		СтруктураПоиска.Вставить("Продукция",               СтрокаСоответствие.Продукция);
...
Процедура ЗаполнитьКоличествоВСтрокеДокументаПоОстатку(СтрокаДокумента, СтрокаОстатки, ПараметрыЗаполнения, ОсталосьЗаполнить, ОсталосьЗаполнитьВЕТИС)
	...
	Если НЕ ПараметрыЗаполнения.ЭтоИнвентаризация
		...
		Количество = ИнтеграцияВЕТИСКлиентСервер.ПересчитатьКоличествоЕдиниц(
		...
		СтрокаДокумента[ПараметрыЗаполнения.ИмяПоляКоличество] = 
			?(ПараметрыЗаполнения.ЭтоИнвентаризация, -Количество, Количество);
		ОсталосьЗаполнить = ОсталосьЗаполнить - Количество;
...
Процедура ПодготовитьСообщениеОЗавершенииЗаполненияЗаписейСкладскогоЖурнала(РезультатЗаполнения)
	...
	Если РезультатЗаполнения = Неопределено Тогда
		...
		ЗаголовокСообщения = НСтр("ru = 'Заполнение не выполнено';
									|en = 'Заполнение не выполнено'");
		ТекстСообщения = НСтр("ru = 'Требуется заполнить реквизиты Продукция и Номенклатура';
								|en = 'Требуется заполнить реквизиты Продукция и Номенклатура'");
	...
	ИначеЕсли РезультатЗаполнения.ВсегоСтрок = 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Заполнение не требуется';
								|en = 'Заполнение не требуется'");
		КартинкаСообщения = БиблиотекаКартинок.Информация32ГосИС;
	...
	ИначеЕсли РезультатЗаполнения.ВсегоСтрок = РезультатЗаполнения.ЗаполненоСтрок Тогда
		...
		ТекстСообщения = НСтр("ru = 'Заполнение успешно завершено';
								|en = 'Заполнение успешно завершено'");
	...
	ИначеЕсли РезультатЗаполнения.ПроблемныхСтрок = РезультатЗаполнения.ВсегоСтрок Тогда
		...
		Если РезультатЗаполнения.НоменклатураНеЗаполнена > 0 И РезультатЗаполнения.КоличествоНеЗаполнено > 0 Тогда
			ЧастиСтрок.Добавить(НСтр("ru = 'реквизиты Продукция, Номенклатура и Количество';
									|en = 'реквизиты Продукция, Номенклатура и Количество'"));
	...
	ИначеЕсли РезультатЗаполнения.ЗаполненоСтрок <> 0 Тогда
		...
		ЗаголовокСообщения = НСтр("ru = 'Заполнение выполнено частично';
									|en = 'Заполнение выполнено частично'");
		...
		Если РезультатЗаполнения.ПроблемныхСтрок <> 0 Тогда
			ТекстСообщения = НСтр("ru = 'Недостаточно остатков продукции и не заполнены обязательные реквизиты';
									|en = 'Недостаточно остатков продукции и не заполнены обязательные реквизиты'");
		Иначе
			ТекстСообщения = НСтр("ru = 'Недостаточно остатков продукции';
									|en = 'Недостаточно остатков продукции'");
		КонецЕсли; 
		КартинкаСообщения = БиблиотекаКартинок.Предупреждение32;
	...
	РезультатЗаполнения.СообщениеПользователю = СообщениеПользователю;
