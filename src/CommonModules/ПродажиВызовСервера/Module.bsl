...
Функция СформироватьДанныеЗаполненияРеализации(МассивСсылок, ИмяДокумента, РеквизитыШапки) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказДавальца2_5) Тогда
		...
		|	НЕОПРЕДЕЛЕНО                                                     КАК ЭтоЗаказКакСчет,
		|	ВЫБОР
...
Функция СформироватьДанныеЗаполненияАктовВыполненныхРабот(МассивСсылок, РеквизитыШапки) Экспорт
	...
	ТекстСообщения = НСтр("ru = 'У выделенных распоряжений отличается поле ""%ПредставлениеПоля%""';
							|en = 'The ""%ПредставлениеПоля%"" field of the selected references differs.'");
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияЭтоЗаказКакСчет Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Выделенные распоряжения были созданы при разных системных настройках использования заказов.';
				|en = 'The selected references were created using different system order settings.'"),
			,
			,
			,
			Отказ);
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияПартнер 
		...
		ТекстОшибки = НСтр("ru = 'Ввод одного акта выполненных работ на основании выделенных распоряжений невозможен';
							|en = 'Cannot register one ""Customer invoice — Services"" based on the selected references'");
			...
			Пока ВыборкаЗаказы.Следующий() Цикл
				...
				ИначеЕсли ВыборкаЗаказы.ЕстьОшибкиХозяйственнаяОперация Тогда
					...
					ТекстОшибки = НСтр("ru = 'Ввод на основании документа с хозяйственной операцией %ХозяйственнаяОперация% невозможен.';
										|en = 'Generation from a base document with business transaction %ХозяйственнаяОперация% is not allowed.'");
...
Функция СформироватьДанныеЗаполненияОтчетаДавальцу(МассивСсылок, РеквизитыШапки) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	МИНИМУМ(ЗаказДавальца.НаправлениеДеятельности) КАК НаправлениеДеятельности,
		|	МИНИМУМ(ЗаказДавальца.Организация)             КАК Организация,
		|	МИНИМУМ(ЗаказДавальца.Подразделение)           КАК Подразделение,
		|	МИНИМУМ(ЗаказДавальца.ВалютаВзаиморасчетов)    КАК ВалютаВзаиморасчетов,
		|	МИНИМУМ(ЗаказДавальца.НалогообложениеНДС)      КАК НалогообложениеНДС,
		...
		|	МИНИМУМ(ЗаказДавальца.Содержание)              КАК Содержание,
		|	МИНИМУМ(ЗаказДавальца.Сделка)                  КАК Сделка,
		|
		|	ВЫБОР
		|		КОГДА
		|			КОЛИЧЕСТВО(РАЗЛИЧНЫЕ
		...
		|	КОНЕЦ КАК ЕстьОтличияДоговор,
		|
		|	ВЫБОР КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказДавальца.НаправлениеДеятельности) > 1 ТОГДА
		|		ИСТИНА
		|	ИНАЧЕ
		|		ЛОЖЬ
		|	КОНЕЦ КАК ЕстьОтличияНаправлениеДеятельности,
		...
		|	КОНЕЦ КАК ЕстьОтличияОрганизация,
		|
		|	ВЫБОР КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказДавальца.Подразделение) > 1 ТОГДА
		|		ИСТИНА
		|	ИНАЧЕ
		|		ЛОЖЬ
		...
		|	КОНЕЦ КАК ЕстьОтличияВалютаВзаиморасчетов,
		|
		|	ВЫБОР КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказДавальца.НалогообложениеНДС) > 1 ТОГДА
		|		ИСТИНА
		|	ИНАЧЕ
		|		ЛОЖЬ
		|	КОНЕЦ КАК ЕстьОтличияНалогообложениеНДС,
		...
		|	ГДЕ
		|		Договоры.Ссылка В(&МассивСсылок)) КАК ЗаказДавальца
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияПартнер Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru = 'Партнер';
													|en = 'Partner'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияКонтрагент Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru = 'Контрагент';
													|en = 'Counterparty'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияОрганизация Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru = 'Организация';
													|en = 'Company'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияПодразделение Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru = 'Подразделение';
													|en = 'Business unit'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияДоговор Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru = 'Договор';
													|en = 'Contract'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияНаправлениеДеятельности Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru = 'Направление деятельности';
													|en = 'Line of business'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияВалютаВзаиморасчетов Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru = 'Валюта взаиморасчетов';
													|en = 'AR/AP currency'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияНалогообложениеНДС Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru = 'Налогообложение НДС';
													|en = 'VAT taxation'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияЦенаВключаетНДС Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru = 'Цена включает НДС';
													|en = 'Price includes VAT'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияСтавкаНДС Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru = 'Ставка НДС';
													|en = 'VAT rate'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияНоменклатура Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru = 'Номенклатура';
													|en = 'Items'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияХарактеристика Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru = 'Характеристика';
													|en = 'Variant'"), Отказ);
	КонецЕсли;
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияПартнер Или
		...
		ВыборкаРеквизитыШапки.ЕстьОтличияПодразделение Или
	...
	Иначе
			...
			Пока ВыборкаЗаказы.Следующий() Цикл
				...
				ИначеЕсли ВыборкаЗаказы.ЕстьОшибкиСтатус Тогда
					...
					ТекстОшибки = НСтр("ru = 'Документ %Документ% находится в статусе ""%Статус%"". Ввод на основании разрешен только в статусах ""К отгрузке"", ""Закрыт"".';
										|en = 'Document %Документ% is in the ""%Статус%"" status. Generation is only allowed from reference documents in ""For shipment"" or ""Closed"" statuses.'");
		...
		РеквизитыШапки.Вставить("Содержание",					ВыборкаРеквизитыШапки.Содержание);
			...
			Если МассивПартнеров.Найти(ПартнерДоговора) = Неопределено Тогда
				...
				СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Указанный партнер в договоре ""%1"" должен быть ""%2"" или головной партнер';
						|en = 'The partner specified in the ""%1"" contract must be ""%2"" or the main partner'"),
		...
		ИначеЕсли ПартнерДоговора <> ПартнерДокумента Тогда
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Указанный партнер в договоре ""%1"" должен быть ""%2""';
					|en = 'The partner specified in the ""%1"" contract must be ""%2""'"),
				Договор, ПартнерДокумента);
		КонецЕсли;
