	...
	Для Каждого Элемент Из ОбъектыПоТипам Цикл
		...
		Если ЕстьОшибки Тогда
			ВызватьИсключение НСтр("ru = 'Ошибка обновления словаря сопоставления номенклатуры.';
									|en = 'An error occurred when updating the item mapping dictionary.'");
		КонецЕсли;
	КонецЦикла;
...
Функция СозданиеПерезаполнениеНоменклатурыКонтрагентаИзДерева(СтрокаОбъекта, ДеревоРазбора) Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаОбъекта.СсылкаНаОбъект) Тогда
		...
		Исключение
			...
			|en = 'Cannot change data on products of the ""%1"" counterparty.
			|Maybe, products of this counterparty are being edited by another user'"),
	...
	Исключение
		Текст = НСтр("ru = 'Создание элемента справочника ""Номенклатура контрагентов""';
					|en = 'Creating ""Product cross references"" catalog item'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПодобратьВариантыСопоставленияПоНоменклатуреКонтрагента(ТаблицаНоменклатурыКонтрагентов,
	...
	ШаблонИсключения = НСтр("ru = 'Номенклатура контрагента %1 с идентификатором %2 передана в поиск более 1 раза';
							|en = 'The %1 partner product with ID %2 is passed to search more than once'");
...
Функция ТекстУсловияПроверкиПустойНоменклатуры(ОграничениеТипаНоменклатуры)
	...
	ШаблонИсключения = НСтр("ru = 'Ошибка встраивания БЭД: определяемый тип НоменклатураБЭД содержит тип %1';
							|en = 'EDL embedding error: the НоменклатураБЭД flexible type contains the %1 type'");
