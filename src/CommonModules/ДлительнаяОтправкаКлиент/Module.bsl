...
Процедура ОповеститьОбУдачнойОтправке(Параметры = Неопределено) Экспорт
	...
	Если ДокументооборотСКОКлиент.ЗапоминаниеОшибок(,, Истина) <> 0 И ТипЗнч(Параметры) = Тип("Структура")
		И Параметры.Свойство("ВыполняемоеОповещение") И Параметры.ВыполняемоеОповещение <> Неопределено Тогда
		
		ВыполнитьОбработкуОповещения(Параметры.ВыполняемоеОповещение, Истина);
		Параметры.ВыполняемоеОповещение = Неопределено;
		Возврат;
	КонецЕсли;
...
Процедура ОповеститьОНеудачнойОтправке(Параметры = Неопределено) Экспорт
	...
	Если ФормаГрупповойОтправкиЕстьСредиОткрытых(Истина) Тогда
		Если ДокументооборотСКОКлиент.ЗапоминаниеОшибок(,, Истина) <> 0 И ТипЗнч(Параметры) = Тип("Структура")
			...
			ВыполнитьОбработкуОповещения(Параметры.ВыполняемоеОповещение, Ложь);
			Параметры.ВыполняемоеОповещение = Неопределено;
			Возврат;
		КонецЕсли;
...
Функция ФормаГрупповойОтправкиИзОткрытых(УчитыватьВыполнениеОтправкиИлиПроверки = Истина) Экспорт
	...
	Если ДокументооборотСКОКлиент.ЗапоминаниеОшибок(,, Истина) <> 0 Тогда
		...
		Результат.Вставить("ВыполняетсяОтправка", Истина);
		Результат.Вставить("ВыполняетсяПроверка", Истина);
		Результат.Вставить("ОповещениеОЗавершенииДействия", Неопределено);
		Результат.Вставить("УникальныйИдентификатор", Новый УникальныйИдентификатор);
		Результат.Вставить("РезультатыАвтонастройки", Новый Соответствие);
		Возврат Результат;
	КонецЕсли;
...
Процедура ЗакрытьФормуОжиданияЗагрузкиМодуля(ОставитьОткрытой = Ложь, УдалосьОбновитьМодуль = Неопределено) Экспорт
	...
	Если ОставитьОткрытой Тогда
		...
		ОсновнойТекст 		= НСтр("ru = 'Открывается помощник подключения к 1С-Отчетности...';
									|en = 'Открывается помощник подключения к 1С-Отчетности...'"); 
...
Процедура ОбновитьНадписьИЗаголовок(
	...
	Оповестить(НСтр("ru = 'Длительное действие. Обновить надпись и заголовок';
					|en = 'Длительное действие. Обновить надпись и заголовок'"), ДополнительныеПараметры); 
	...
	Если ВходящийКонтекст.Свойство("ЭтоОбновлениеМодуля") 
		...
		Иначе
			ОсновнойТекст = НСтр("ru = 'Выполняется обновление модуля 1С-Отчетности...';
								|en = 'Выполняется обновление модуля 1С-Отчетности...'");
		КонецЕсли;
...
Функция ЭтоФормаОтправкиОтчетности(КраткоеИмяФормы, ИмяФормы) Экспорт
	...
	Если КраткоеИмяФормы = "РегламентированнаяОтчетность" Тогда
		Возврат Истина;
	КонецЕсли;
	
	ПрикладныеФормы = Новый Массив;
	РегламентированнаяОтчетностьКлиентПереопределяемый.ДобавитьПрикладныеФормыОтправкаОтчетов(ПрикладныеФормы);
	Возврат ПрикладныеФормы.Найти(ИмяФормы) <> Неопределено;
