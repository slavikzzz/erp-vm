...
Функция ПараметрыРазбиенияСтроки(Количество) Экспорт
	...
	Результат.Вставить("Заголовок", НСтр("ru = 'Введите количество товара в новой строке';
										|en = 'Введите количество товара в новой строке'"));
...
// Внешнее событие преобразовать данные со сканера в структуру.
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  Неопределено, Структура - Внешнее событие преобразовать данные со сканера в структуру:
// * TID - Строка - 
// * EPC - Строка - 
// * EPC_BIN - Строка - 
// * Формат - Строка - 
...
Функция ВнешнееСобытиеПреобразоватьДанныеСоСканераВСтруктуру(Форма, Источник, Событие, Данные) Экспорт
	...
	Если Результат <> Неопределено
		...
		ДанныеСоСканера = ШтрихкодированиеОбщегоНазначенияИСКлиент.ПреобразоватьДанныеСоСканераВСтруктуру(Результат.Параметр);
...
Процедура ВнешнееСобытиеПолученыШтрихкоды(ЗавершениеОбработки, Форма, Источник, Событие, Данные, ПараметрыСканирования = Неопределено) Экспорт
	...
	Если Результат <> Неопределено
		...
		ДанныеШтрихкода = ШтрихкодированиеОбщегоНазначенияИСКлиент.ПреобразоватьДанныеСоСканераВСтруктуру(Результат.Параметр);
...
Процедура ОтработатьТаймаутОжиданияСчитыванияRFID(Форма) Экспорт
	...
	ОбщегоНазначенияКлиент.СообщитьПользователю(
		НСтр("ru = 'Истекло время ожидания чтения RFID-метки.
		           ...
		           |en = 'Истекло время ожидания чтения RFID-метки.
		           |Возможные проблемы: метка находится вне зоны считывания,
		           |метка повреждена или чтение заводской метки в настройках оборудования не включено.'"));
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			СтрШаблон(
				НСтр("ru = 'Номер серии должен быть сгенерирован по TID RFID-метки.
				           ...
				           |en = 'Номер серии должен быть сгенерирован по TID RFID-метки.
				           |При генерации произошла ошибка: %1. Обратитесь к администратору.'"),
...
Процедура ОбработкаВнешнегоСобытияПолученыШтрихкоды(ЗавершениеОбработки, Форма, Результат, Данные, ПараметрыСканирования) Экспорт
	
	Если ДляОбработкиRFIDНуженСерверныйВызов(Результат.Параметр, Форма) Тогда
...
Функция НайтиПоИдентификатору(ДанныеФормыДерево, Идентификатор) Экспорт
	...
	Если Не РезультатВыполнения.Результат Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Операция считывателя RFID завершилась с ошибкой: %1';
				|en = 'Операция считывателя RFID завершилась с ошибкой: %1'"),
			РезультатВыполнения.ОписаниеОшибки);
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
