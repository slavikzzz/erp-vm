...
Процедура ДополнитьСловоСсылкойНаПримечание(Слово, НомерПримечания = 1) Экспорт
	...
	Слово = СтрШаблон(НСтр("ru = '%1%2%3';
							|en = '%1%2%3'"), Слово, Разделитель, СимволыПримечания(НомерПримечания));
...
Функция ДоступныеПользователюНаборыСуммовыхПоказателей(ИмяОтчета, НачалоПериода, КонецПериода, Организация) Экспорт
	...
	Если Не БухгалтерскиеОтчетыВызовСервера.ИспользоватьСуммыРазниц(Организация, НачалоПериода, КонецПериода) Тогда
...
#Область СправкиРасчетыПоИмущественнымНалогам
...
//
Процедура ДобавитьГруппировкиТаблицыКомпоновкиДанныхПоШаблону(
	Знач ГруппировкаТаблицы, ГруппировкаШаблон, НастройкаГруппировок, ИмяКорневойГруппировки, ВыбранныеПоляКорневойТаблицы) Экспорт
	
	КоличествоГруппировок = 0;
	Для каждого ПолеВыбраннойГруппировки Из НастройкаГруппировок Цикл
		Если ПолеВыбраннойГруппировки.Использование Тогда
			
			СтруктураШаблон = НайтиСтрокуТаблицыРекурсивно(ГруппировкаШаблон, ПолеВыбраннойГруппировки.Поле);
			// Если группировка не предусмотрена в предопределенных настройках, то она не требуется.
			Если СтруктураШаблон = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			// На первой итерации добавляется строка (группировка) в корневую таблицу,
			// а на следующих итерациях внутри добавляются следующие уровни группировки
			// (ГруппировкаТаблицы подменяется на вложенную группировку - см. ниже) 
			...
			Для каждого ПолеГруппировкиШаблон Из СтруктураШаблон.ПоляГруппировки.Элементы Цикл
				...
				// устанавливаем вид иерархии из настроек.
			...
			// Также для верхнего уровня устанавливаем:
			// - вывод итогов только в начале
			// - имя группировки, как у корневой группировки, чтобы применился макет заголовка группировки (шапка таблицы).
			НастройкаВыводаИтогов = Структура.ПараметрыВывода.Элементы.Найти("РасположениеИтогов");
			
			Если КоличествоГруппировок = 0 Тогда
				Структура.Имя = ИмяКорневойГруппировки;
				НастройкаВыводаИтогов.Использование = Истина;
				НастройкаВыводаИтогов.Значение = РасположениеИтоговКомпоновкиДанных.Начало;
			Иначе
				НастройкаВыводаИтогов.Использование = Истина;
...
Процедура НастроитьПолеВариантНалогообложения(Форма, Организация = Неопределено) Экспорт
	
	Если Организация = Неопределено Тогда
		Организация = СправкиРасчетыКлиентСервер.Организация(Форма);
	КонецЕсли;
	
	Если Форма.Элементы.Найти("ВариантНалогообложенияПрибыли") <> Неопределено Тогда
		БазовыеРеквизиты = СправкиРасчетыКлиентСервер.БазовыеРеквизиты(Форма);
		ИспользуетсяРаздельныйУчет = РаздельныйУчетПоНалогуНаПрибыль.ИспользуетсяРаздельныйУчет(
			Организация, БазовыеРеквизиты.НачалоПериода);
		Форма.Элементы.ВариантНалогообложенияПрибыли.Видимость = ИспользуетсяРаздельныйУчет;
		Если Форма.Элементы.Найти("ПроверятьЗаполнениеВариантаНО") <> Неопределено Тогда
			Форма.Элементы.ВариантНалогообложенияПрибыли.АвтоОтметкаНезаполненного = ИспользуетсяРаздельныйУчет;
			Форма.ПроверятьЗаполнениеВариантаНО = ИспользуетсяРаздельныйУчет;
		КонецЕсли;
	КонецЕсли;
...
Процедура НастроитьПоОрганизации(Форма)
	...
	НастроитьПолеВариантНалогообложения(Форма, Организация);
...
Процедура НастроитьПериод(Форма, ТребуетсяВызовСервера) Экспорт
	...
	Если ТребуетсяВызовСервера.Свойство("НастроитьЭлементВариантНалогообложенияПрибыли") Тогда
		НастроитьПолеВариантНалогообложения(Форма);
	КонецЕсли;
...
Функция КонтекстФормированияОтчета(Форма)
	...
	Если Форма.Элементы.Найти("ВариантНалогообложенияПрибыли") <> Неопределено Тогда
		Если РаздельныйУчетПоНалогуНаПрибыль.ИспользуетсяРаздельныйУчет(БазовыеРеквизиты.Организация, БазовыеРеквизиты.НачалоПериода) Тогда
			Контекст.Вставить("ВариантНО", Форма["ВариантНалогообложенияПрибыли"]);
		Иначе
			Контекст.Вставить("ВариантНО", Справочники.ВариантыНалогообложенияПрибыли.ПустаяСсылка());
		КонецЕсли;
	КонецЕсли;
...
// В ГруппировкаТаблицыКомпоновкиДанных добавляет выбранные поля на основании шаблонной группировки.
// Если в настройках отчета группировка указана как предопределенная, то вместо автополя заполняются
// поля из корневой таблицы компоновки данных
//
// Параметры:
//  ВыбранныеПоля                  - КоллекцияВыбранныхПолейКомпоновкиДанных - коллекция для заполнения 
//  ВыбранныеПоляГруппировкиШаблон - КоллекцияВыбранныхПолейКомпоновкиДанных - шаблон-источник
//  ПредопределеннаяГруппировка    - Булево - признак предопределенной группировки в настройках отчета
//  ВыбранныеПоляТаблицы           - КоллекцияВыбранныхПолейКомпоновкиДанных - выбранные поля на уровне корневой таблицы компоновки данных
//
Процедура ДобавитьВыбранныеПоляПоШаблонуРекурсивно(
	ВыбранныеПоля, ВыбранныеПоляГруппировкиШаблон, ПредопределеннаяГруппировка = Ложь, ВыбранныеПоляТаблицы = Неопределено) Экспорт
	...
	Для каждого ВыбранноеПоле Из ВыбранныеПоляГруппировкиШаблон Цикл
		...
		ИначеЕсли ТипЗнч(ВыбранноеПоле) = Тип("ГруппаВыбранныхПолейКомпоновкиДанных") Тогда
			...
			ДобавитьВыбранныеПоляПоШаблонуРекурсивно(КоллекцияПолей.Элементы, ВыбранноеПоле.Элементы, ПредопределеннаяГруппировка, ВыбранныеПоляТаблицы);
		ИначеЕсли ТипЗнч(ВыбранноеПоле) = Тип("АвтоВыбранноеПолеКомпоновкиДанных") И ПредопределеннаяГруппировка Тогда
			ДобавитьВыбранныеПоляПоШаблонуРекурсивно(ВыбранныеПоля, ВыбранныеПоляТаблицы);
