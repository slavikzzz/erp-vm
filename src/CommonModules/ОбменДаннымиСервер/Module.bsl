		...
		Если Реквизит <> Неопределено Тогда
			...
			Если ТипыРеквизита.Количество() <> 1 Тогда
				...
				СтрокаСообщения = НСтр("ru = 'Составной тип данных для значений по умолчанию не поддерживается.
					|Реквизит ""%1"".';
					|en = 'Default values don''t support composite data types.
					|Attribute: %1.'");
...
Функция ПредставлениеДатыСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Последняя синхронизация: %1';
																		|en = 'Last synchronized on: %1'"), ОтносительнаяДатаСинхронизации(ДатаСинхронизации));
КонецФункции
...
Функция ОтносительнаяДатаСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	Иначе
		...
		Если КоличествоДнейРазницы = 0 Тогда // сегодня
			...
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сегодня, %1';
																					|en = 'Today, %1'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
			
		ИначеЕсли КоличествоДнейРазницы = 1 Тогда // вчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Вчера, %1';
																					|en = 'Yesterday, %1'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
			
		ИначеЕсли КоличествоДнейРазницы = 2 Тогда // позавчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Позавчера, %1';
																					|en = 'Day before yesterday, %1'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазы(
	...
	Иначе
		...
		Сообщение = НСтр("ru = 'Не определен идентификатор транспорта';
						|en = 'Transport ID is no'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьДействиеОбмена(
	...
	СтрокаСообщения = НСтр("ru = 'Начало процесса обмена данными для узла %1';
							|en = 'Data exchange started. Node: %1.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗагрузитьПриоритетныеДанныеВПодчиненныйУзелРИБ(Отказ = Ложь) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Обмен данными.Загрузка приоритетных данных';
				|en = 'Data exchange.Priority data import'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция КлючФоновогоЗадания(ИмяПланаОбмена, Действие) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'План обмена: %1, действие: %2';
			|en = 'Exchange plan: %1, action: %2'"), ИмяПланаОбмена, Действие);
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
			...
			Если МодульРаботаВМоделиСервиса.СтатусОбластиДанных(НомерОбластиДанных) <> Перечисления["СтатусыОбластейДанных"]["Используется"] Тогда
				...
				ТекстИсключения = НСтр("ru = 'Синхронизация невозможна, потому что область данных отмечена как не рабочая.
					...
					|Contact the administrator or technical service.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПриЗаполненииПоставляемыхПрофилейГруппДоступа(ОписанияПрофилей, ПараметрыОбновления) Экспорт
	...
	ОписаниеПрофиля.Наименование =
		НСтр("ru = 'Синхронизация данных с другими программами';
			|en = 'Synchronization with other applications'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииЗагрузкаПравилДляОбменаДанными() Экспорт
	...
	Возврат НСтр("ru = 'Обмен данными.Загрузка правил';
				|en = 'Data exchange.Load rules'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииСозданиеОбменаДанными() Экспорт
	...
	Возврат НСтр("ru = 'Обмен данными.Создание обмена данными';
				|en = 'Data exchange.Create data exchange'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУдалениеОбменаДанными() Экспорт
	...
	Возврат НСтр("ru = 'Обмен данными.Удаление обмена данными';
				|en = 'Data exchange.Delete data exchange'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииРегистрацияДанныхДляНачальнойВыгрузки() Экспорт
	...
	Возврат НСтр("ru = 'Обмен данными.Регистрация данных для начальной выгрузки';
				|en = 'Data exchange.Register data for initial export'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииВыгрузкаДанныхДляСопоставления() Экспорт
	...
	Возврат НСтр("ru = 'Обмен данными.Выгрузка данных для сопоставления';
				|en = 'Data exchange.Export data for mapping'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУдалениеВременногоФайла() Экспорт
	...
	Возврат НСтр("ru = 'Обмен данными.Удаление временного файла';
				|en = 'Data exchange.Delete temporary file'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииОбменДанными() Экспорт
	...
	Возврат НСтр("ru = 'Обмен данными';
				|en = 'Data exchange'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииВыгрузкаДанныхВСервисПередачиФайлов() Экспорт
	...
	Возврат НСтр("ru = 'Обмен данными.Сервис передачи файлов.Выгрузка данных';
				|en = 'Data exchange.File transfer service.Export data'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииЗагрузкаДанныхИзСервисаПередачиФайлов() Экспорт
	...
	Возврат НСтр("ru = 'Обмен данными.Сервис передачи файлов.Загрузка данных';
				|en = 'Data exchange.File transfer service.Import data'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбмена) Экспорт
	...
	Иначе
		СтрокаСообщения = НСтр("ru = '%1, %2; Объектов обработано: %3';
								|en = '%1, %2. Objects processed: %3'", ОбщегоНазначения.КодОсновногоЯзыка());
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
							СтруктураНастроекОбмена.РезультатВыполненияОбмена,
							СтруктураНастроекОбмена.ДействиеПриОбмене,
...
Функция КлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, ДействиеПриОбмене) Экспорт
	...
	КлючСообщения = НСтр("ru = 'Обмен данными.[ИмяПланаОбмена].[ДействиеПриОбмене]';
						|en = 'Data exchange.[ИмяПланаОбмена].[ДействиеПриОбмене]'",
		ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗаблокироватьУзелОбмена(УзелОбмена, Отказ) Экспорт
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ru = 'Не удалось заблокировать узел плана обмена [%1].
			...
			|Data exchange is probably already in progress. Try again later.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если ВыполняемоеДействие = Перечисления.ДействияПриОбмене.ЗагрузкаДанных Тогда
		...
		Если ЕстьПоддержкаСопоставления Тогда
			...
			Если ПолученоСообщениеДляСопоставления Тогда 
				...
				|en = 'Для продолжения выполните загрузку сообщения для сопоставления данных.
				|Выполнение обмена данными отменено.'");
...
Функция ЭтоОшибкаНомерСообщенияМеньшеИлиРавенНомеруРанееПринятогоСообщения(ОписаниеОшибки)
	...
	Возврат СтрНайти(НРег(ОписаниеОшибки), НРег(НСтр("ru = 'Номер сообщения меньше или равен';
...
Процедура ВыполнитьКонтрольЗаписиНеразделенныхДанных(Знач Данные) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		ТекстИсключения = НСтр("ru = 'Недостаточно прав для выполнения действия.';
								|en = 'Insufficient rights for this operation.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СтрокаСообщения = НСтр("ru = 'Начало процесса обмена данными по настройке %1';
									|en = 'Data exchange started. Setting: %1'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура УстановитьПравилаОбменаВыгрузкиДанных(ОбработкаОбменаДаннымиXML, СтруктураНастроекОбмена)
	...
	Если ПравилаКонвертацииОбъектов = Неопределено Тогда
		...
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не заданы правила конвертации для плана обмена %1. Выгрузка данных отменена.';
				|en = 'Conversion rules are not specified. Exchange plan: %1. The data export is canceled.'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ВыполнитьОбновлениеВерсииТиповыхПравилДляОбменаДанными(ПравилаОбменаЗагруженныеИзФайла, ПравилаРегистрацииЗагруженныеИзФайла)
	...
	Для Каждого ЗаписьПланаОбмена Из Результат Цикл
		...
		Если ОбменДаннымиПовтИсп.ЕстьМакетПланаОбмена(ИмяПланаОбмена, "ПравилаОбмена")
			...
			Если ПравилаОбменаЗагруженныеИзФайла.Найти(ИмяПланаОбмена) = Неопределено Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Выполняется обновление правил конвертации данных для плана обмена %1';
						|en = 'Updating data conversion rules. Exchange plan: %1'"), ИмяПланаОбмена);
				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииОбменДанными(),
		...
		Иначе
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Выполняется обновление правил регистрации данных для плана обмена %1';
					|en = 'Updating data registration rules. Exchange plan: %1'"), ИмяПланаОбмена);
			...
			Попытка
				...
				Если Отказ Тогда
					ВызватьИсключение НСтр("ru = 'Получение данных из главного узла завершилось с ошибками.';
											|en = 'Error receiving data from the master node.'");
				КонецЕсли;
			Исключение
				УстановитьПривилегированныйРежим(Истина);
				УстановитьРежимЗагрузкиСообщенияОбменаДаннымиПередЗапуском("ЗагрузкаРазрешена", Ложь);
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Иначе
			...
			Дело.ПараметрыФормы = Новый Структура("Заголовок,ИмяМакета",
				НСтр("ru = 'Установка обновления';
					|en = 'Install update'"), "ИнструкцияКакВыполнитьУстановкуОбновленияВручную");
		КонецЕсли;
		Дело.Владелец       = Раздел;
...
Функция ДеревоМетаданныхКонфигурации(Отбор = Неопределено) Экспорт
	...
	НоваяСтрокаКоллекцииОбъектовМетаданных("Константы",               НСтр("ru = 'Константы';
																			|en = 'Constants'"),                 БиблиотекаКартинок.Константа,              БиблиотекаКартинок.Константа,                    КоллекцииОбъектовМетаданных);
	...
	НоваяСтрокаКоллекцииОбъектовМетаданных("ПланыВидовХарактеристик", НСтр("ru = 'Планы видов характеристик';
																			|en = 'Charts of characteristic types'"), БиблиотекаКартинок.ПланВидовХарактеристик, БиблиотекаКартинок.ПланВидовХарактеристикОбъект, КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("ПланыСчетов",             НСтр("ru = 'Планы счетов';
																			|en = 'Charts of accounts'"),              БиблиотекаКартинок.ПланСчетов,             БиблиотекаКартинок.ПланСчетовОбъект,             КоллекцииОбъектовМетаданных);
	...
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыСведений",        НСтр("ru = 'Регистры сведений';
																			|en = 'Information registers'"),         БиблиотекаКартинок.РегистрСведений,        БиблиотекаКартинок.РегистрСведений,              КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыНакопления",      НСтр("ru = 'Регистры накопления';
																			|en = 'Accumulation registers'"),       БиблиотекаКартинок.РегистрНакопления,      БиблиотекаКартинок.РегистрНакопления,            КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыБухгалтерии",     НСтр("ru = 'Регистры бухгалтерии';
																			|en = 'Accounting registers'"),      БиблиотекаКартинок.РегистрБухгалтерии,     БиблиотекаКартинок.РегистрБухгалтерии,           КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыРасчета",         НСтр("ru = 'Регистры расчета';
																			|en = 'Calculation registers'"),          БиблиотекаКартинок.РегистрРасчета,         БиблиотекаКартинок.РегистрРасчета,               КоллекцииОбъектовМетаданных);
...
Процедура НастроитьЭлементыФормыЗацикливания(Форма)
	...
	ШаблонТекста = НСтр("ru = '<br>Обнаружено зацикливание синхронизаций. Для получения подробной информации перейдите по 
			  ...
			  |en = '<br>Обнаружено зацикливание синхронизаций. Для получения подробной информации перейдите по 
			  |<a href=""%1"">ссылке</a>.
			  |<br><br>
			  |<a href=""%2"">Объекты, не зарегистрированные при зацикливании</a>.'");
...
Процедура УстановитьСообщениеОбменаИзАрхива(ИмяВременногоФайлаАрхива, УзелИнформационнойБазы)
	...
	Если РаспаковатьZipФайл(ИмяВременногоФайлаАрхива, ИмяВременногоКаталога) Тогда
		...
		Если СписокРаспакованныхФайлов.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ru = 'При распаковке архива не найден файл с правилами.';
									|en = 'При распаковке архива не найден файл с правилами.'");
		КонецЕсли;
		...
		Если СписокРаспакованныхФайлов.Количество() > 1 Тогда
			ВызватьИсключение НСтр("ru = 'Ожидается, что в архиве будет один файл.';
									|en = 'Ожидается, что в архиве будет один файл.'");
		КонецЕсли;
	...
	Если ЕстьОшибки Тогда
		...
		ВызватьИсключение НСтр("ru = 'Не удалось распаковать архив с сообщением обмена.';
								|en = 'Не удалось распаковать архив с сообщением обмена.'");
	КонецЕсли;
