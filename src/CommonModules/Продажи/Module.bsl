...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Если Свойства.РежимЗаписи <> РежимЗаписиДокумента.Запись Тогда
		...
		Если Не Свойства.ЭтоНовый
			...
			Параметры.КонтрольныеРегистрыИзменений.Добавить(Метаданные.РегистрыНакопления.ЗаказыКлиентов);
		КонецЕсли;
...
Функция ТекстыЗапросовСторнирования(МетаданныеДокумента) Экспорт
	...
	МетаданныеРегистра = Метаданные.РегистрыНакопления.ЗаказыКлиентов;
...
Процедура ИнициализироватьДанныеКонтроляИзменений(Запрос, ТекстыЗапроса, Документ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияЗаказыКлиентовИзменение") Тогда
		...
		Если ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров = 0 Тогда
			...
			ТекстЗапроса =
				...
				|	ТаблицаОстатков.КПередачеОстаток               КАК КПередаче
				|ИЗ
				...
				|	ИЛИ ТаблицаОстатков.КПередачеОстаток < 0
				|";
		...
		Иначе
			...
			ТекстЗапроса = 
				...
				|	ИЛИ ЗаказыОстатки.КПередачеОстаток + ЕСТЬNULL(ДопустимыеОтклонения.ДопустимоеОтклонениеКПередаче,0) < 0
				|";
			
		КонецЕсли;
		
		ТекстыЗапроса.Добавить(ТекстЗапроса, "ОшибкиЗаказыКлиентов");
...
Процедура СообщитьОРезультатахКонтроляИзменений(РезультатыКонтроля, Документ, Отказ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияЗаказыКлиентовИзменение") Тогда
		...
		ШаблонСообщенияСоСкладом = НСтр("ru = 'Номенклатура %1, склад %2
			...
			|en = 'Item %1, warehouse %2
			|Registered more than specified in reference %3 by %4 %5'");
		...
		Для каждого СтрокаОшибки Из РезультатыКонтроля.ОшибкиЗаказыКлиентов Цикл
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияЗаявкиНаВозвратТоваровОтКлиентовИзменение") Тогда
		...
		ШаблонСообщения = НСтр("ru = 'Номенклатура %1
			...
			|en = 'Items %1
			|Number of the returned items exceeds the number of items specified in the return request by %2 %3'");
