...
Процедура ОбработкаЗаполненияОтчетОПроизводствеЕГАИС(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПроизводствоБезЗаказа") Тогда
		...
		ЗаполнитьОтчетПроизводстваЕГАИСНаОснованииПроизводствоБезЗаказа(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	//-- НЕ УТ
	
	//++ НЕ УТКА
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		
		ЗаполнитьОтчетПроизводстваЕГАИСНаОснованииЭтапПроизводство2_2(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	//-- НЕ УТКА
	...
	Пока ОбъектыДляОбработки.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось обработать %ИмяОбъекта%: %Ссылка% по причине: %Причина%';
									|en = 'Не удалось обработать %ИмяОбъекта%: %Ссылка% по причине: %Причина%'");
...
Процедура СформироватьДанныеДокументаОснования(ДокументОснование, ДокументЕГАИС, ТаблицаДанных) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПроизводствоБезЗаказа") Тогда
		
		СформироватьДанныеОснованияПроизводствоБезЗаказа(ДокументОснование, ДокументЕГАИС, ТаблицаДанных);	
	...
	//++ НЕ УТКА
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		
		СформироватьДанныеОснованияЭтапПроизводства2_2(ДокументОснование, ДокументЕГАИС, ТаблицаДанных);	
...
//++ НЕ УТ
Процедура СформироватьДанныеОснованияПроизводствоБезЗаказа(ДокументОснование, ДокументЕГАИС, ТаблицаДанных)
	...
	Запрос.Текст = "
		...
		|		ЕСТЬNULL(Серии.Серия, ВыходныеИзделия.Серия) КАК Серия,
		|		ЕСТЬNULL(Серии.Количество, ВыходныеИзделия.Количество) КАК Количество
		|	ИЗ
		|		Документ.ПроизводствоБезЗаказа.ВыходныеИзделия КАК ВыходныеИзделия
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
		|			ПО ВыходныеИзделия.Номенклатура = СправочникНоменклатура.Ссылка
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыАлкогольнойПродукции КАК СправочникВидыАлкогольнойПродукции
		|			ПО СправочникНоменклатура.ВидАлкогольнойПродукции = СправочникВидыАлкогольнойПродукции.Ссылка
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа.ВыходныеИзделияСерии КАК Серии
		|			ПО Серии.Ссылка = &ДокументОснование
		|			И Серии.Номенклатура      = ВыходныеИзделия.Номенклатура
		|			И Серии.Характеристика    = ВыходныеИзделия.Характеристика
		|			И Серии.Назначение        = ВыходныеИзделия.Назначение
		|			И Серии.Получатель        = ВыходныеИзделия.Получатель
		|			И Серии.НомерГруппыЗатрат = ВыходныеИзделия.НомерГруппыЗатрат
		|	ГДЕ
		...
		|		-ОформленныеТовары.Количество
		|	ИЗ
		|		Документ.ПроизводствоБезЗаказа.ВыходныеИзделия КАК ОформленныеТовары
		|	ГДЕ
		|		ОформленныеТовары.Ссылка В
		|				(ВЫБРАТЬ
		|					Т.Ссылка
...
Процедура СформироватьДанныеОснованияЭтапПроизводства2_2(ДокументОснование, ДокументЕГАИС, ТаблицаДанных)
	...
	Запрос.Текст = "
		...
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыАлкогольнойПродукции КАК СправочникВидыАлкогольнойПродукции
		|			ПО СправочникНоменклатура.ВидАлкогольнойПродукции = СправочникВидыАлкогольнойПродукции.Ссылка
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ВыходныеИзделияСерии КАК Серии
		|			ПО (Серии.Ссылка = &ДокументОснование)
		|				И (Серии.Номенклатура = ВыходныеИзделия.Номенклатура)
		|				И (Серии.Характеристика = ВыходныеИзделия.Характеристика)
		|				И (Серии.Назначение = ВыходныеИзделия.Назначение)
	...
	|				ИЛИ СправочникНоменклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.АлкогольнаяПродукцияДо9Процентов))
		...
		|	И НЕ ВыходныеИзделия.Получатель = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		-ОформленныеТовары.Количество
		|	ИЗ
		|		Документ.ЭтапПроизводства2_2.ВыходныеИзделия КАК ОформленныеТовары
		|	ГДЕ
		|		ОформленныеТовары.Ссылка В
		|				(ВЫБРАТЬ
		|					Т.Ссылка
	...
	Пока Выборка.Следующий() Цикл
			...
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'Позиция номенклатуры %1 не сопоставлена с элементом классификатора алкогольной продукции ЕГАИС в строке %2 списка ""Товары"".';
						|en = 'Позиция номенклатуры %1 не сопоставлена с элементом классификатора алкогольной продукции ЕГАИС в строке %2 списка ""Товары"".'"),
			...
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'Не заполнен объем для алкогольной продукции %1 в строке %2 списка ""Товары"".';
						|en = 'Не заполнен объем для алкогольной продукции %1 в строке %2 списка ""Товары"".'"),
					Выборка.Номенклатура,
					Выборка.НомерСтроки),
				Объект,
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", Выборка.НомерСтроки, "Номенклатура"),
		...
		Если Выборка.Количество <> Выборка.КоличествоМарок Тогда
			...
			ТекстОшибки = НСтр("ru = 'В строке %1 для %2 должно быть указано акцизных марок - %3, а указано - %4.';
								|en = 'В строке %1 для %2 должно быть указано акцизных марок - %3, а указано - %4.'");
...
Процедура ЗаполнитьАктСписанияЕГАИСНаОснованииВнутреннегоПотребленияТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Запрос.Текст = ТекстЗапросаЗаполненияПоШаблонуОснования(ВозможныеДокументыЕГАИС, "ВнутреннееПотребление",
		...
		НастройкиФормированияЗапросаЗаполнения());
	...
	Товары = РезультатЗапроса[РезультатЗапроса.Количество() - 1];
...
Процедура ЗаполнитьАктСписанияЕГАИСНаОснованииСборкиТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаЗаполненияПоШаблонуОснования(ВозможныеДокументыЕГАИС, "СборкаТоваров", ТекстЗапроса);
...
Процедура ЗаполнитьЧекЕГАИСНаОснованииСборкиТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
		...
		"(ТаблицаТовары.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СборкаТоваров) И ЛОЖЬ)");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, 
		"ТаблицаТовары.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РазборкаТоваров)",
		"(ТаблицаТовары.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РазборкаТоваров)
		|	И ТаблицаТовары.Номенклатура.ВидАлкогольнойПродукции.Маркируемый)");
		...
		Если РеквизитыДокумента.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеремещениеТоваровМеждуФилиалами Тогда
			...
			Если РеквизитыДокумента.Организация = РеквизитыДокумента.ОрганизацияПолучатель Тогда
				...
				ТекстОшибки = НСтр("ru = 'В %1 одинаковые организация и организация-получатель - %2. Ввод на основании невозможен.';
									|en = 'В %1 одинаковые организация и организация-получатель - %2. Ввод на основании невозможен.'");
...
//++ НЕ УТ
Процедура ЗаполнитьОтчетПроизводстваЕГАИСНаОснованииПроизводствоБезЗаказа(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ДокументПроизводствоБезЗаказа.Ссылка                                      КАК ДокументОснование,
	|	ДокументПроизводствоБезЗаказа.Дата                                        КАК Дата,
	|	ДокументПроизводствоБезЗаказа.Дата                                        КАК ДатаПроизводства,
	|	НЕ ДокументПроизводствоБезЗаказа.Проведен                                 КАК ЕстьОшибкиПроведен,
	|	ДокументПроизводствоБезЗаказа.Организация                                 КАК Организация,
	...
	|ГДЕ
	|	ДокументПроизводствоБезЗаказа.Ссылка = &ДокументОснование");
	...
	|	ТаблицаТовары.Характеристика            КАК Характеристика,
	|	ТаблицаТовары.Серия                     КАК Серия,
	|	ТаблицаТовары.НомерГруппыЗатрат         КАК ИдентификаторСтроки,
	|	""""                                    КАК ИдентификаторСтрокиТовары,
	|	СУММА(ТаблицаТовары.Количество)         КАК Количество,
	|	0                                       КАК Сумма
	|ПОМЕСТИТЬ ТоварыКОформлению
	|ИЗ
	...
	|	И ТаблицаТовары.Получатель ССЫЛКА Справочник.Склады
	|	И ТаблицаСерии.Номенклатура ЕСТЬ NULL
	|	И НЕ &ДанныеПоСырью
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ТабличнаяЧасть.Характеристика            КАК Характеристика,
	|	ТабличнаяЧасть.Серия                     КАК Серия,
	|	ТаблицаТоваров.НомерГруппыЗатрат         КАК ИдентификаторСтроки,
	|	""""                                    КАК ИдентификаторСтрокиТовары,
	|	СУММА(ТабличнаяЧасть.Количество)         КАК Количество,
	|	0                                        КАК Сумма
	|ИЗ
	...
	|	И ТабличнаяЧасть.Назначение        = ТаблицаТоваров.Назначение
	|	И ТабличнаяЧасть.Получатель        = ТаблицаТоваров.Получатель
	|	И ТабличнаяЧасть.НомерГруппыЗатрат = ТаблицаТоваров.НомерГруппыЗатрат
	|
	|ГДЕ
	|	ТабличнаяЧасть.Ссылка = &ДокументОснование
	|	И СправочникНоменклатура.АлкогольнаяПродукция
	...
	|		ИЛИ СправочникНоменклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПивоВПотребительскихУпаковках)
	|		ИЛИ СправочникНоменклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.АлкогольнаяПродукцияДо9Процентов))
	|	И ТаблицаТоваров.НаправлениеВыпуска = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукцииНаСклад)
	|	И ТаблицаТоваров.Получатель ССЫЛКА Справочник.Склады
	|	И НЕ &ДанныеПоСырью
	|
	|СГРУППИРОВАТЬ ПО
	...
	|		И ТаблицаСерии.НомерГруппыЗатрат = ТаблицаТовары.НомерГруппыЗатрат
	//++ НЕ УТКА
	|		И ТаблицаСерии.ВладелецИзделия   = ТаблицаТовары.ВладелецИзделия
	//-- НЕ УТКА
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &ДокументОснование
	|	И СправочникНоменклатура.АлкогольнаяПродукция
	...
	|	ТаблицаТовары.НомерГруппыЗатрат         КАК ИдентификаторСтрокиТовары,
	|	СУММА(ТаблицаТовары.Количество)         КАК Количество,
	|	0                                       КАК Сумма
	|ИЗ
	...
	|		ИЛИ СправочникНоменклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.АлкогольнаяПродукцияДо9Процентов))
	|	И &ДанныеПоСырью
	|	И ТаблицаТовары.Склад = &ОтборПоСкладу
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТовары.Склад,
	|	ТаблицаТовары.Номенклатура,
	...
	|	ОформленныеТовары.Характеристика             КАК Характеристика,
	|	ОформленныеТовары.Серия                      КАК Серия,
	|	ОформленныеТовары.ИдентификаторСтроки        КАК ИдентификаторСтроки,
	|	""""                                         КАК ИдентификаторСтрокиТовары,
	|	-СУММА(ОформленныеТовары.Количество)         КАК Количество,
	|	0                                            КАК Сумма
	|ИЗ
	|	Документ.ОтчетОПроизводствеЕГАИС.Товары КАК ОформленныеТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетОПроизводствеЕГАИС КАК ТаблицаДокумента
	|		ПО ОформленныеТовары.Ссылка = ТаблицаДокумента.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторОрганизацийЕГАИС КАК КлассификаторОрганизацийЕГАИС
	|		ПО ТаблицаДокумента.ОрганизацияЕГАИС = КлассификаторОрганизацийЕГАИС.Ссылка
	|ГДЕ
	...
	|	ОформленныеТовары.Серия                      КАК Серия,
	|	""""                                         КАК ИдентификаторСтроки,
	|	ОформленныеТовары.ИдентификаторСтрокиТовары  КАК ИдентификаторСтрокиТовары,
	|	-СУММА(ОформленныеТовары.Количество)         КАК Количество,
	|	0                                            КАК Сумма
	|ИЗ
	...
	|	ОформленныеТовары.Ссылка В (ВЫБРАТЬ Т.Ссылка ИЗ ОформленныеДокументыОтчетОПроизводствеЕГАИС КАК Т)
	|	И &ДанныеПоСырью
	|	И ВЫРАЗИТЬ(КлассификаторОрганизацийЕГАИС.ТорговыйОбъект КАК Справочник.Склады) = &ОтборПоСкладу
	|
	|СГРУППИРОВАТЬ ПО
	|	КлассификаторОрганизацийЕГАИС.ТорговыйОбъект,
	|	ОформленныеТовары.Номенклатура,
	|	ОформленныеТовары.Характеристика,
	|	ОформленныеТовары.Серия,
	|	ОформленныеТовары.ИдентификаторСтрокиТовары");
	...
	|	ТоварыКОформлению.ИдентификаторСтрокиТовары         КАК ИдентификаторСтрокиТовары,
	|	СопоставленныеПозиции.АлкогольнаяПродукция          КАК АлкогольнаяПродукция,
	|	СопоставленныеПозиции.АлкогольнаяПродукция.Крепость КАК Крепость,
	|	СУММА(ТоварыКОформлению.Количество)                 КАК Количество,
	|	СУММА(ТоварыКОформлению.Количество)                 КАК КоличествоУпаковок,
	...
	|	СопоставленныеПозиции.АлкогольнаяПродукция,
	|	СопоставленныеПозиции.АлкогольнаяПродукция.Крепость,
	|	ТоварыКОформлению.ИдентификаторСтрокиТовары,
	...
	Запрос.УстановитьПараметр("ОтборПоСкладу",                  Справочники.Склады.ПустаяСсылка());
	
	Запрос.УстановитьПараметр("КонечныеСтатусыОтчетОПроизводствеЕГАИС", 
		Документы.ОтчетОПроизводствеЕГАИС.КонечныеСтатусы());
	...
	Если ВыборкаПоСкладам.Следующий() Тогда
	
		ДокументОбъект.ОрганизацияЕГАИС = ОрганизацияЕГАИСПоОрганизацииИСкладу(РеквизитыДокумента.Организация, ВыборкаПоСкладам.Склад);	
		ИнтеграцияЕГАИС.ЗаполнитьТабличнуюЧастьТовары(ДокументОбъект, ВыборкаПоСкладам, ДанныеЗаполнения, Истина);
		
	КонецЕсли;	
	
	Запрос.УстановитьПараметр("ДанныеПоСырью", Истина);
	Запрос.УстановитьПараметр("ОтборПоСкладу", ВыборкаПоСкладам.Склад);
...
//++ НЕ УТКА
Процедура ЗаполнитьОтчетПроизводстваЕГАИСНаОснованииЭтапПроизводство2_2(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ДокументЭтапПроизводства2_2.Ссылка                                      КАК ДокументОснование,
	|	ДокументЭтапПроизводства2_2.Дата                                        КАК Дата,
	|	ДокументЭтапПроизводства2_2.Дата                                        КАК ДатаПроизводства,
	|	НЕ ДокументЭтапПроизводства2_2.Проведен                                 КАК ЕстьОшибкиПроведен,
	|	ДокументЭтапПроизводства2_2.Организация                                 КАК Организация,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыДокументовЕГАИС.ОтчетОПроизводствеПроизводство) КАК ВидДокумента
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК ДокументЭтапПроизводства2_2
	|ГДЕ
	|	ДокументЭтапПроизводства2_2.Ссылка = &ДокументОснование");
	...
	|	СУММА(ТаблицаТовары.Количество)         КАК Количество,
	|	СУММА(0)                                КАК Сумма
	|ПОМЕСТИТЬ ТоварыКОформлениюПоСырью
	|ИЗ
	...
	|ГДЕ
	|	ОформленныеТовары.Ссылка В (ВЫБРАТЬ Т.Ссылка ИЗ ОформленныеДокументыОтчетОПроизводствеЕГАИС КАК Т)
	|	И КлассификаторОрганизацийЕГАИС.ТорговыйОбъект = &ОтборПоСкладу
	|
	|СГРУППИРОВАТЬ ПО
	|	ОформленныеТовары.Номенклатура,
	|	ОформленныеТовары.Характеристика,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ДанныеТЧ_Товары.АлкогольнаяПродукция   КАК АлкогольнаяПродукция,
	|	ТоварыКОформлению.Номенклатура            КАК Номенклатура,
	|	ТоварыКОформлению.Характеристика          КАК Характеристика,
	|	ТоварыКОформлению.Серия                   КАК Серия,
	...
	|ИЗ
	|	ВТ_ДанныеТЧ_Товары КАК ВТ_ДанныеТЧ_Товары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТоварыКОформлениюПоСырью КАК ТоварыКОформлению
	|		ПО ИСТИНА";
	
	Запрос.УстановитьПараметр("ОтборПоСкладу",                  ВыборкаПоСкладам.Склад);
	Запрос.УстановитьПараметр("ТаблицаТоваров",                 ДокументОбъект.Товары.Выгрузить());
	Запрос.УстановитьПараметр("ДокументОснование",              ДанныеЗаполнения);
	
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	Сырье = РезультатЗапроса[РезультатЗапроса.Количество()-1];
	ЗаполнитьСырьеОтчетаОПроизводствеЕГАИС(ДокументОбъект, Сырье);
