...
Функция ИнициализироватьСоединениеССерверомСдачиОтчетности(ОписаниеОшибки)
	...
	Попытка
		Возврат ОбщегоНазначенияЭДКО.СоединениеССерверомИнтернета(АдресСервера);
	Исключение
...
Функция СобытиеЖурналаРегистрации(Объект)
	...
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ПолучениеРезультатовОтправкиОтчетности Тогда
		Возврат НСтр("ru = 'Получение результатов отправки отчетности';
					|en = 'Получение результатов отправки отчетности'");
	КонецЕсли;
...
Функция ПолучитьКомментарийКСобытиюНачалаРегламентногоЗадания(Объект)
	...
	Если Объект = Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности Тогда
		Возврат НСтр("ru = 'Начато обновление информации о направлениях сдачи отчетности';
					|en = 'Начато обновление информации о направлениях сдачи отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ОтправкаОтчетности Тогда
		Возврат НСтр("ru = 'Начата отправка отчетности';
					|en = 'Начата отправка отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ПолучениеРезультатовОтправкиОтчетности Тогда
		Возврат НСтр("ru = 'Начато получение результатов отправки отчетности';
					|en = 'Начато получение результатов отправки отчетности'");
	КонецЕсли;
...
Функция ПолучитьКомментарийКСобытиюОкончанияРегламентногоЗадания(Объект)
	...
	Если Объект = Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности Тогда
		Возврат НСтр("ru = 'Завершено обновление информации о направлениях сдачи отчетности';
					|en = 'Завершено обновление информации о направлениях сдачи отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ОтправкаОтчетности Тогда
		Возврат НСтр("ru = 'Завершена отправка отчетности';
					|en = 'Завершена отправка отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ПолучениеРезультатовОтправкиОтчетности Тогда
		Возврат НСтр("ru = 'Завершено получение результатов отправки отчетности';
					|en = 'Завершено получение результатов отправки отчетности'");
	КонецЕсли;
...
Функция СсылкаПоСтрокеСтатуса(СтрокаСтатуса, ОписаниеОшибки = "")
	...
	Иначе
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось определить статус сдачи отчетности по ответу сервера (%1)';
																						|en = 'Не удалось определить статус сдачи отчетности по ответу сервера (%1)'"), ТекстИзФайла);
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПроверитьПередРегистрациейЗаявкиНаОтправкуОтчетности(Отчет, КонтролирующийОрган = "ФНС", ТекстВопроса = "") Экспорт
	...
	СведенияОрг = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
		Организация, , "КодОрганаФСГС, КодОрганаПФР");
	...
	ИначеЕсли КонтролирующийОрган = "ФСГС" Тогда
		...
		КодНаправления = СокрЛП(СведенияОрг.КодОрганаФСГС);
	Иначе
		ТипНаправления = Перечисления.ТипыНаправленийСдачиОтчетности.ПФР;
		КодНаправления = СокрЛП(СведенияОрг.КодОрганаПФР);
	КонецЕсли;
	...
	Если НЕ НаправлениеСдачиОтчетностиОткрыто(Организация, ТипНаправления, КодНаправления) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Направление сдачи отчетности по организации закрыто. Отправка невозможна.';
													|en = 'Направление сдачи отчетности по организации закрыто. Отправка невозможна.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ТипОтчета = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
		...
		Если АналогичныеОтправки.Количество() > 0 Тогда
			ТекстВопроса = НСтр("ru = 'Вы уже отправляли аналогичный отчет с аналогичным признаком корректировки,
										...
										|en = 'Вы уже отправляли аналогичный отчет с аналогичным признаком корректировки,
										|за тот же период, по той же организации и в ту же инспекцию.
										|
										|Вы уверены, что хотите отправить отчет заново?'");
			Возврат Ложь;
	...
	Иначе
		...
		Если ОтправкиОтчета.Количество() > 0 Тогда
			ТекстВопроса = НСтр("ru = 'Вы уже отправляли данный отчет.
										|Вы уверены, что хотите отправить отчет заново?';
										|en = 'Вы уже отправляли данный отчет.
										|Вы уверены, что хотите отправить отчет заново?'");
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗарегистрироватьОтправкуОтчетностиВСправочнике(ДанныеПакета, ИмяФайлаПакета, Отчет) Экспорт
	...
	Если ТипНаправления = Перечисления.ТипыНаправленийСдачиОтчетности.ФНС Тогда
		...
		ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодНаправления) Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Код инспекции-получателя отчета должен состоять строго из четырех цифр.';
														|en = 'Код инспекции-получателя отчета должен состоять строго из четырех цифр.'"));
			Возврат Ложь;
		КонецЕсли;
	ИначеЕсли ТипНаправления = Перечисления.ТипыНаправленийСдачиОтчетности.ФСГС Тогда
		...
		ИЛИ Сред(КодНаправления, 3, 1) <> "-" Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Код органа ФСГС, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XX-XXZZZZZ, где X - любая цифра.';
														|en = 'Код органа ФСГС, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XX-XXZZZZZ, где X - любая цифра.'"));
			Возврат Ложь;
		КонецЕсли;
	Иначе
		...
		ИЛИ Сред(КодНаправления, 4, 1) <> "-" Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Код органа ПФР, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XXX-XXX, где X - любая цифра.';
														|en = 'Код органа ПФР, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XXX-XXX, где X - любая цифра.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ВидОтчета) Тогда
		ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В справочнике ""Регламентированные отчеты"" не найдено определение отчета вида %1.';
																											|en = 'В справочнике ""Регламентированные отчеты"" не найдено определение отчета вида %1.'"), ИсточникОтчета));
		Возврат Ложь;
	КонецЕсли;
	...
	Исключение
		...
		ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось запланировать отправку отчета:
				|
				|%1';
				|en = 'Не удалось запланировать отправку отчета:
				|
...
Функция ОпеределениеReporting(ВебАдрес, ОписаниеОшибки = "") Экспорт
	...
	Попытка
		...
		Возврат Новый WSОпределения(ВебАдрес + "/ws/Reporting?wsdl", ИмяИПароль.Имя, ИмяИПароль.Пароль, Прокси, 30);
	Исключение
		ОписаниеОшибки = ИнформацияОбОшибке().Описание;
		Возврат Неопределено;
	КонецПопытки;
...
Функция ИнициализироватьВебСервисReporting(ОпределениеСервиса, ОписаниеОшибки = "")
	...
	СтруктураURI = ОбщегоНазначенияКлиентСервер.СтруктураURI(ОпределениеСервиса);
	Схема = ?(ЗначениеЗаполнено(СтруктураURI.Схема), СтруктураURI.Схема, "http");
	Прокси = ПолучениеФайловИзИнтернета.ПолучитьПрокси(Схема);
	
	Попытка
		Прокси = Новый WSПрокси(ОпределениеСервиса, "http://www.1c.ru/SaaS/1.0/WS", "Reporting", "ReportingSoap", Прокси, 30);
		ИмяИПароль = ИмяПользователяИПарольУдаленногоДоступаКВебСервисамМС();
	...
	Если НЕ ЗначениеЗаполнено(ВебАдрес) Тогда
		ЗаписьЖурналаРегистрации(СобытиеЖурнала,
								...
								НСтр("ru = 'Не задан адрес веб-сервиса для получения сведений о направлениях сдачи отчетности.';
									|en = 'Не задан адрес веб-сервиса для получения сведений о направлениях сдачи отчетности.'"));
		Возврат Неопределено;
	КонецЕсли;
		...
		Иначе
			Если НЕ ЗаписатьТекстВФайл(ЭлТекстВыгрузки, ПолноеИмяФайлаВыгрузки, ЭлКодировкаТекстаВыгрузки) Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось запланировать отправку отчета: не удалось записать временный файл с отчетностью.';
															|en = 'Не удалось запланировать отправку отчета: не удалось записать временный файл с отчетностью.'"));
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если КонтролирующийОрган = "ФНС" Тогда
		...
		Иначе
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось запланировать отправку отчета: отсутствуют сведения о доверенности.';
														|en = 'Не удалось запланировать отправку отчета: отсутствуют сведения о доверенности.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура СформироватьФайлInfoПакета(Отчет, КонтролирующийОрган, ПолноеИмяФайлаInfo)
	...
	// извлекаем параметры
	СведенияОрг = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
		Отчет.Организация, , "ИННЮЛ, КодОрганаФСГС, КодОрганаПФР");
	
	ИНН = СокрЛП(СведенияОрг.ИННЮЛ);
	Если КонтролирующийОрган = "ФНС" Тогда
		ТипНаправления = "fns";
		КодНаправления = ПолучитьКодИФНСПоСсылкеНаОтчет(Отчет);
	ИначеЕсли КонтролирующийОрган = "ФСГС" Тогда
	...
	Если нрег(РасширениеФайла) = ".xml" Тогда // формат xml - меняем ИдФайл и имя файла
		...
		Если КомпонентыИмени.Количество() < 5 Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Невозможно изменить реквизиты отправителя в имени файла отчета: некорректное исходное имя файла выгрузки.';
														|en = 'Невозможно изменить реквизиты отправителя в имени файла отчета: некорректное исходное имя файла выгрузки.'"));
			Возврат Ложь;
		КонецЕсли;
...
Функция ПолучитьКодИФНСПоСсылкеНаОтчет(Отчет)
	...
	Иначе
		Возврат РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(Отчет.Организация, ,"КодНО");
	КонецЕсли;
...
Функция ВыгрузитьСведенияОДоверенностиПоНаправлениюОтчета(Отчет, ДанныеДоверенности, ИмяФайлаДоверенности, Доверенность = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(РегистрацияВИФНС) Тогда
		ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось запланировать отправку отчета:
				|не найдена запись в справочнике ""Регистрации в налоговом органе"", соответствующая направлению отправки (%1).';
				|en = 'Не удалось запланировать отправку отчета:
...
Функция ПолучитьСоответствиеРеквизитовКодамПолномочийДоверенности()
	...
	СоответствиеРеквизитовКодамПолномочий.Вставить("Признак40", "40");
