	...
	Если ОбщегоНазначенияБТС.ЭтоСсылочныеДанныеПоддерживающиеПредопределенныеЭлементы(Объект.Метаданные()) Тогда
		...
		Иначе
			...
			НСтр("ru = 'Объект метаданных %1 не может быть обработан обработчиком
                  ...
                  |en = 'The %1 metadata object cannot be processed by the 
                  |%2 handler,
                  |as reference mapping for its predefined items is not required.'", Метаданные.ОсновнойЯзык.КодЯзыка),
	...
	Иначе
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Объект метаданных %1 не может быть обработан обработчиком
                  ...
                  |en = 'The %1 metadata object cannot be processed by the 
                  |%2 handler 
                  | as it cannot contain predefined items.'", Метаданные.ОсновнойЯзык.КодЯзыка),
			МетаданныеОбъекта.ПолноеИмя(),
			"ВыгрузкаЗагрузкаПредопределенныхДанных.ПередВыгрузкойОбъекта");
		...
		Если ЗначениеЗаполнено(СтрокаТаблицыИсходныхСсылок.ИмяПредопределенныхДанных) Тогда
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				Иначе
					...
					ВызватьИсключение СтрШаблон(
						НСтр("ru = 'Обнаружено дублирование предопределенных элементов %1 в таблице %2.';
							|en = 'Duplicate predefined items %1 are found in table %2.'", Метаданные.ОсновнойЯзык.КодЯзыка),
