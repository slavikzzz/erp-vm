		...
		Для Каждого ВидЦены Из Форма.ВыбранныеЦены Цикл
			...
			Если ВидЦены.Выбрана
				...
				Если ПодборТоваровКлиентСервер.ЭтоФормаПрайсЛиста(Форма) Или
					...
					КонецЕсли;
					ЗаголовокКолонкиСтараяЦена = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Цена на %1';
																												|en = 'Price as of %1'"), Формат(ДатаДействующихЦен, "ДЛФ=D"));
					
				Иначе
					
					ЗаголовокКолонкиСтараяЦена = ?(ВидЦены.Выбрана, НСтр("ru = 'Действующая цена';
																		|en = 'Effective price'"), НСтр("ru = 'Цена';
																										|en = 'Price'"));
					
				КонецЕсли;
				
				Если ВидЦены.Выбрана Тогда
					НоваяГруппаТехническая = ДобавитьГруппуФормы(Форма, "ГруппаТехническая" + ИмяКолонки, НСтр("ru = 'Группа';
																												|en = 'Group'"), Ложь, Неопределено, ГруппировкаКолонок.Горизонтальная, НоваяГруппа);
					...
					НовоеПоле.Подсказка = НСтр("ru = 'Текущая установленная цена';
												|en = 'Current set price'");
					НовоеПоле.ЦветТекста = WebЦвета.Серый;
					
					Если Не ЭтоРегистрацияЦенПоставщика Тогда
						НоваяГруппаФормула = ДобавитьГруппуФормы(Форма, "ГруппаТехническаяФормула" + ИмяКолонки, НСтр("ru = 'Группа формула';
																														|en = 'Formula group'"), Ложь, Неопределено, ГруппировкаКолонок.ВЯчейке, НоваяГруппаТехническая);
						...
						НовоеПоле.Подсказка = НСтр("ru = 'Формула для расчета цены';
													|en = 'Formula for price calculation'");
						НовоеПоле.ЦветТекста = WebЦвета.Серый;
					КонецЕсли;
					...
					НовоеПоле.Подсказка = НСтр("ru = 'Процент изменения цены';
												|en = 'Price change percentage'");
					
					НоваяГруппаЦена = ДобавитьГруппуФормы(Форма, "ГруппаЦена" + ИмяКолонки, НСтр("ru = 'Группа цена';
																								|en = 'Group price'"), Ложь, Неопределено, ГруппировкаКолонок.ВЯчейке, НоваяГруппа);
					...
					Если Не ЭтоРегистрацияЦенПоставщика Тогда
						НоваяГруппаВалюта = ДобавитьГруппуФормы(Форма, "ГруппаВалюта" + ИмяКолонки, НСтр("ru = 'Группа валюта';
																										|en = 'Currency group'"), Ложь, Неопределено, ГруппировкаКолонок.Горизонтальная, НоваяГруппаТехническая);
				...
				Если Форма.ИспользоватьУпаковкиНоменклатуры Тогда
					НоваяГруппаУпаковка = ДобавитьГруппуФормы(Форма, "ГруппаУпаковки" + ИмяКолонки, НСтр("ru = 'Ед. изм.';
																										|en = 'Unit'"), Ложь, Неопределено, ГруппировкаКолонок.ВЯчейке, НоваяГруппа);
		...
		Исключение
			...
			Если ЗначениеЗаполнено(СтрокаТаблицыЦен.Характеристика) Тогда
				ТекстСообщения = НСтр("ru = 'Для номенклатуры ""%Номенклатура%"" с характеристикой ""%Характеристика%"" не рассчитана цена по виду цен ""%ВидЦены%"" по причине: %ОписаниеОшибки%. Проверьте корректность формулы.';
										|en = 'Price is not calculated for the ""%Характеристика%"" variant of the ""%Номенклатура%"" item by the ""%ВидЦены%"" price type. Reason: %ОписаниеОшибки%. Check whether the formula is correct.'");
			Иначе
				ТекстСообщения = НСтр("ru = 'Для номенклатуры ""%Номенклатура%"" не рассчитана цена по виду цен ""%ВидЦены%"" по причине: %ОписаниеОшибки%. Проверьте корректность формулы.';
										|en = 'Price is not calculated for the ""%Номенклатура%"" item by the ""%ВидЦены%"" price type. Reason: %ОписаниеОшибки%. Check whether the formula is correct.'");
			КонецЕсли;
...
Процедура ОбновитьСтарыеЦеныНоменклатуры(Форма, КэшДанных = Неопределено, ТаблицаНоменклатуры = Неопределено) Экспорт
	...
	Если Форма.ВыбранныеЦеныИзменены Тогда
		...
		КэшДанных.Вставить("ТаблицаСоответствияНоменклатурыСтрокамДереваТоваров", Неопределено);
...
Процедура ЗагрузитьСтарыеЦеныНоменклатурыПредприятия2_5(Форма, ТаблицаНоменклатуры, КэшДанных, ТаблицаНоменклатурыЗапрещеннойКРедактированию = Неопределено) Экспорт
	...
	|ГДЕ
	|	(ЗапрещенныеКРедактированиюТовары.Номенклатура, ЗапрещенныеКРедактированиюТовары.ХарактеристикаЦО, ЗапрещенныеКРедактированиюТовары.ВидЦены) В
	|			(ВЫБРАТЬ
	|				ВременнаяТаблицаТовары.Номенклатура,
		...
		Исключение
			...
			РезультатЗагрузки.ОписаниеОшибки =
				НСтр("ru = 'Файл не может быть загружен.
				            ...
				            |You may only import the files exported with command ""Excel -> Export..."".'");
		...
		Исключение
			...
			РезультатЗагрузки.ОписаниеОшибки =
				НСтр("ru = 'Файл не может быть загружен.
				            ...
				            |en = 'The file cannot be imported.
				            |File column structure might have been corrupted.'");
			Возврат РезультатЗагрузки;
		КонецПопытки;
		...
		Если ТекЭлементНоменклатура.ХарактеристикиИспользуются Тогда
			...
			Если Не ДобавленыХарактеристики Тогда
				ТекстОшибки = НСтр("ru = 'Для номенклатуры ""%1"" в строке ""%2"" списка ""Таблица цен"" включено использование характеристик,
				                       ...
				                       |en = 'Usage of variants is enabled for item ""%1"" product in line ""%2"" of the ""Price table"" list,
				                       |but no variant is specified. Add variants and specify their prices, or delete this product from the list.'");
	...
	Для Каждого СтрокаВидЦены Из УстановкаЦенКлиентСервер.ВыбранныеСтрокиТаблицыВидовЦен(Форма) Цикл
		...
		Если Ценообразование.ПроверитьСхемуКомпоновкиДанныхПоОбязательнымПолям(
			...
			СтрЗаменить(НСтр("ru = 'Вид цены ""%ВидЦены%""';
							|en = 'Price type ""%ВидЦены%""'"), "%ВидЦены%", СтрокаВидЦены.Ссылка)) Тогда
...
Процедура ВыполнитьОбновлениеЦен(ВидыЦен, ПараметрыОбновленияЦен) Экспорт
	...
	//++ Локализация
	ИнтеграцияСМаркетплейсамиСерверЛокализация.ВыполнитьОбновлениеЦен(СтруктураФормы.ВыбранныеЦены);
	//-- Локализация
...
Процедура УстановитьЦены(Знач МассивВидовЦен, Знач СтруктураФормы, Знач ВидОбновленияЦен = "Обновление")
	...
	ТекстГруппаСобытий = НСтр("ru = '%ВидОбновленияЦен% цен. Запуск по расписанию';
								|en = '%ВидОбновленияЦен% of prices. Scheduled start'", ОбщегоНазначения.КодОсновногоЯзыка());	
	...
	ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Примечание, "", НСтр("ru = 'Запуск';
																					|en = 'Start'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Попытка
		...
		ТекстКомментария = НСтр("ru = '%ВидОбновленияЦен% цен (автоматическое по расписанию)';
								|en = '%ВидОбновленияЦен% of prices (scheduled automatic)'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Исключение
		...
		ЗаписьЖурнала(ПараметрыЖурнала, 
		              ...
		              "",
		              НСтр("ru = 'Ошибка обновления цен';
							|en = 'Price update error'", ОбщегоНазначения.КодОсновногоЯзыка()) + " '"+ МассивВидовЦен +"'",
		              ИнформацияОбОшибке());
		
	КонецПопытки;
	
	ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Примечание, "",НСтр("ru = 'Завершение';
																					|en = 'Complete'", ОбщегоНазначения.КодОсновногоЯзыка()));
КонецПроцедуры
...
Функция ПеренестиВидЦенНаЦенообразованиеВерсии2_5(Форма, ВидЦен)
	...
	Если ДанныеОтчета.Количество() = 0 Тогда
		...
		ПараметрыЖурнала = Новый Структура("ГруппаСобытий, Метаданные, Данные");
		ТекстГруппаСобытий = НСтр("ru = 'Перенос цен на ценообразование 2.5.';
									|en = 'Transfer of prices to pricing 2.5.'", ОбщегоНазначения.КодОсновногоЯзыка());	
		
		ПараметрыЖурнала.ГруппаСобытий = ТекстГруппаСобытий;
		ПараметрыЖурнала.Метаданные    = Метаданные.Справочники.ВидыЦен;
		ПараметрыЖурнала.Данные        = ВидЦен;

		ТекстОшибки = НСтр("ru = 'Ошибка обновления цен. Цены не найдены';
							|en = 'Price update error. Prices not found'", ОбщегоНазначения.КодОсновногоЯзыка()) + " '"+ ВидЦен +"'";
	...
	Иначе
		...
		//	заполняем и записываем документ
		ТекстКомментария = НСтр("ru = 'Автоматический перенос цен на ценообразование 2.5 по виду цен << %ВидЦен% >>';
								|en = 'Automatic transfer of prices to pricing 2.5 by price type << %ВидЦен% >>'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Если РезультатУстановкиЦен = Неопределено Тогда
			...
			ТекстОшибки = НСтр("ru = 'Ошибка обновления цен';
								|en = 'Price update error'", ОбщегоНазначения.КодОсновногоЯзыка()) + " '"+ ВидЦен +"'";
...
Функция СоздатьИПЛ(ВладелецИПЛ, Дата) Экспорт
	...
	Если Не ЗначениеЗаполнено(ИндивидуальныйВидЦены) Тогда
		...
		Исключение
			...
			ПараметрыЖурнала = Новый Структура("ГруппаСобытий, Метаданные, Данные");
			ТекстГруппаСобытий = НСтр("ru = 'Создание ИПЛ.';
										|en = 'Generating IPL.'", ОбщегоНазначения.КодОсновногоЯзыка());	
			
			ПараметрыЖурнала.ГруппаСобытий = ТекстГруппаСобытий;
			ПараметрыЖурнала.Метаданные    = Метаданные.Справочники.СоглашенияСКлиентами;
			ПараметрыЖурнала.Данные        = ВладелецИПЛ;
	
			ТекстОшибки = НСтр("ru = 'Ошибка создания ИПЛ. По владельцу';
								|en = 'Error creating IPL. By owner'", ОбщегоНазначения.КодОсновногоЯзыка()) + " '"+ ВладелецИПЛ +"'";
	...
	Если ЗначениеЗаполнено(ИндивидуальныйВидЦены) Тогда
		...
		//	заполняем и записываем документ
		ТекстКомментария = НСтр("ru = 'Создание ИПЛ на основании соглашений с клиентами. Владелец << %ВладелецИПЛ% >>';
								|en = 'Creation of IPL based on agreements with clients. Owner << %ВладелецИПЛ% >>'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Если РезультатУстановкиЦен = Неопределено Тогда
			...
			ТекстОшибки = НСтр("ru = 'Ошибка обновления цен';
								|en = 'Price update error'", ОбщегоНазначения.КодОсновногоЯзыка()) + " '"+ ИндивидуальныйВидЦены +"'";
	...
	Иначе
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка создания ИПЛ. По владельцу ""%1""';
									|en = 'Error creating IPL. By the ""%1"" owner'", ОбщегоНазначения.КодОсновногоЯзыка()), ВладелецИПЛ);
	...
	Если ДополнительныеПараметры = Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(РеквизитыОбъекта.НастройкиКлючаЦенПоХарактеристике) Тогда
			...
			Исключение
				...
				РезультатОбновления.ОписаниеОшибки = НСтр("ru = 'Ошибка при обновлении вида номенклатуры';
															|en = 'Error when updating item kind'", ОбщегоНазначения.КодОсновногоЯзыка());
			КонецПопытки;
		...
		ИначеЕсли РеквизитыОбъекта.НастройкиКлючаЦенПоХарактеристике <> Перечисления.ВариантОтбораДляКлючаЦен.НеИспользовать
			...
			Если Не ОбновитьХарактеристкиЦенообразования(ВидНоменклатуры) Тогда
				...
				РезультатОбновления.Результат = Ложь;
				РезультатОбновления.ОписаниеОшибки = НСтр("ru = 'Ошибка при обновлении характеристик ценообразования';
															|en = 'Error when updating pricing variants'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Если РеквизитыОбъекта.НастройкиКлючаЦенПоСерии <> Перечисления.ВариантОтбораДляКлючаЦен.НеИспользовать 
			...
			Если Не ОбновитьСерииЦенообразования(ВидНоменклатуры) Тогда
				...
				РезультатОбновления.Результат = Ложь;
				РезультатОбновления.ОписаниеОшибки = РезультатОбновления.ОписаниеОшибки + НСтр("ru = 'Ошибка при обновлении серий ценообразования';
																								|en = 'Error when updating pricing batch'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВычислитьЗначенияЦеныПоДаннымИБ(Форма,
	...
	Если ТаблицаНоменклатуры.Количество() > 0 Тогда
		...
		Если СтрокаВидЦены.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБ
			...
			Если Ценообразование.ПроверитьСхемуКомпоновкиДанных(СхемаКомпоновкиДанных, СтрЗаменить(НСтр("ru = 'Вид цены ""%ВидЦены%""';
																										|en = 'Price type ""%ВидЦены%""'"), "%ВидЦены%", СтрокаВидЦены.Ссылка)) Тогда
...
Процедура ВычислитьЗначенияЦеныПоДаннымИБ2_5(Форма,
	...
	Если ТаблицаНоменклатуры.Количество() > 0 Тогда
		...
		Если СтрокаВидЦены.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБ
			...
			ПроверкаСКДПройдена = Ценообразование.ПроверитьСхемуКомпоновкиДанных(СхемаКомпоновкиДанных, СтрЗаменить(НСтр("ru = 'Вид цены ""%ВидЦены%""';
																														|en = 'Price type ""%ВидЦены%""'"), "%ВидЦены%", СтрокаВидЦены.Ссылка), Ложь);
			ПроверкаСКДПройдена2_5 = Ценообразование.ПроверитьСхемуКомпоновкиДанных2_5(СхемаКомпоновкиДанных, СтрЗаменить(НСтр("ru = 'Вид цены ""%ВидЦены%""';
																																|en = 'Price type ""%ВидЦены%""'"), "%ВидЦены%", СтрокаВидЦены.Ссылка), Ложь);
			Если ПроверкаСКДПройдена2_5 Тогда
		...
		Иначе
			...
			Иначе
				ВызватьИсключение НСтр("ru = 'В документ введено слишком много товаров.
										...
										|en = 'You have entered too many goods into the document.
										|Try to split the document into several separate documents.'");
			КонецЕсли;
		КонецЕсли;
