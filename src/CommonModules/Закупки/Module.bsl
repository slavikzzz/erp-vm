	...
	Если Свойства.РежимЗаписи <> РежимЗаписиДокумента.Запись Тогда
		...
		Если Не Свойства.ЭтоНовый
			...
			Или Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение
				...
				//++ НЕ УТ
				Или ТипДокумента = Тип("ДокументОбъект.ОтчетПереработчика2_5")
				//++ Устарело_Переработка24
				...
				Или ТипДокумента = Тип("ДокументОбъект.ПоступлениеТоваровОтХранителя")
					И Документ.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5
				//-- НЕ УТ
...
Процедура СообщитьОРезультатахКонтроляИзменений(РезультатыКонтроля, Документ, Отказ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияУслугиКОформлениюОтчетовПринципалуИзменение") Тогда
		...
		ШаблонСообщения = НСтр("ru = 'По организации %1 не хватает к оформлению %2 %3 на сумму %4 %5 услуг: %6, у партнера %7';
								|en = 'There is a lack of registration %2%3 for the amount of %4%5services: %6at the partner %7 by company %1'");
