...
Функция ТекстЗапросаСтоимостьОС(Запрос)
	...
	|	И ТИПЗНАЧЕНИЯ(Таблица.Документ) <> ТИП(Документ.РаспределениеНДС)
	|	И НЕ &ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА
	|	И НЕ &ДвиженияЗаписываютсяПриАктуализацияДвиженийПоВНА
	|	И НЕ &ДвиженияЗаписываютсяПриОтложенномФормированииДвиженийВНА
	|";
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не удалось выполнить по причине %1';
							|en = 'Cannot execute. Reason: %1'"),
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
						
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Формирование движений по НДС.';
						|en = 'Generation of VAT records.'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Процедура ВыполнитьРасчет(ПараметрыРасчета)
	...
	//++ НЕ УТКА
	ВернутьДокументыКОтражениюВМеждународномУчете(ПараметрыРасчета);
	//-- НЕ УТКА
...
Процедура ЗафиксироватьРезультат(ПараметрыРасчета)
	...
	//++ НЕ УТ
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НАЧАЛОПЕРИОДА(РасчетыПоЛизингу.Период, МЕСЯЦ) КАК Месяц,
	|	&НомерЗадания                                    КАК НомерЗадания,
	|	Задания.Организация                              КАК Организация,
	|	Задания.СчетФактура                              КАК СчетФактура
	|ИЗ
	|	ИсходныеЗадания КАК Задания
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыПоФинансовымИнструментам КАК РасчетыПоЛизингу
	|	ПО РасчетыПоЛизингу.Период > &КонецПериода
	|		И РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Организация = Задания.Организация
	|		И РасчетыПоЛизингу.РасчетныйДокумент = Задания.СчетФактура
	...
	|		И РасчетыПоЛизингу.Активность
	|		И РасчетыПоЛизингу.СуммаРегл > 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	//-- НЕ УТ
...
Процедура ЗаписатьНДСПредъявленный(ПараметрыРасчета, ИменаТаблицЗаписейНДСПредъявленный)
	...
	Если МассивНедоступныхРегистраторов.Количество() Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									...
									|%1. 
									|Эти документы не могут быть основанием для записей в книгах. Для исправления проблемы оформите по нему ""Счет-фактуру полученный"" или не используйте эти документы в качестве документа партии НДС.';
									|en = 'An error occurred when calculating VAT: an attempt to generate purchase/sales ledger entries by documents:
									|%1. 
									|These documents cannot be base documents for entries in the ledgers. To eliminate the issue, register a Received tax invoice for these documents or do not use them as VAT lot documents.'", ОбщегоНазначения.КодОсновногоЯзыка()),
									СтрСоединить(МассивНедоступныхРегистраторов, ","+Символы.ПС));
		
		ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Формирование движений по НДС.';
						|en = 'Generate VAT records.'", ОбщегоНазначения.КодОсновногоЯзыка()), 
					УровеньЖурналаРегистрации.Ошибка, , , ТекстСообщения); 
...
// В данном сценарии движение сформируется в расчете себестоимости.
	...
	//++ НЕ УТ
	|	СписанияНДСНаРасходы.НастройкаСчетовУчета      КАК НастройкаСчетовУчета,
	//-- НЕ УТ
...
Процедура РассчитатьДвиженияПоПрочимАктивамПассивамСохранение(ПараметрыРасчета)
	...
	//++ НЕ УТ
	|	ДвиженияПоПрочимАктивамПассивам.НастройкаСчетовУчета КАК НастройкаСчетовУчета,
	//-- НЕ УТ
...
Процедура ЗаписатьНДСДвиженияПоПрочимАктивамПассивам(ПараметрыРасчета)
	...
	|	ДвиженияПоПрочимАктивамПассивам.Аналитика,
	//++ НЕ УТ
	|	ДвиженияПоПрочимАктивамПассивам.НастройкаСчетовУчета,
	//-- НЕ УТ
...
Процедура ВернутьДокументыКОтражениюВМеждународномУчете(ПараметрыРасчета)
...
#КонецОбласти
	...
	Если ПараметрыРасчета.РасчетАвансовПолученных Тогда
		...
		|		ПО ОтражениеДокументовВМеждународномУчете.Регистратор = ДокументыВыборочнойРегистрации.Документ
		|ГДЕ
		|	ОтражениеДокументовВМеждународномУчете.Активность
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
...
Процедура ПодготовитьВТ_АвансыВыданные_Возникновения(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	|
	|ВЫБРАТЬ
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Организация             КАК Организация,
	|	РасчетыПоЛизингу.РасчетныйДокумент                                 КАК РасчетныйДокумент,
	|	РасчетыПоЛизингу.Регистратор                                       КАК ДокументРегистратор,
	|	РасчетыПоЛизингу.Договор                                           КАК ОбъектРасчетов,
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Контрагент              КАК Контрагент,
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Договор                 КАК Договор,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(РасчетыПоЛизингу.Договор) = ТИП(Справочник.ДоговорыАренды)
	|			ТОГДА ВЫРАЗИТЬ(РасчетыПоЛизингу.Договор КАК Справочник.ДоговорыАренды).НалогообложениеНДС
	|		ИНАЧЕ
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|	КОНЕЦ КАК ТипНалогообложения,
	|	НАЧАЛОПЕРИОДА(РасчетыПоЛизингу.Период, ДЕНЬ)                       КАК ДатаАванса,
	|	МАКСИМУМ(РасчетыПоЛизингу.Валюта)                                  КАК ВалютаДокумента,
	|	СУММА(РасчетыПоЛизингу.СуммаРегл)                                  КАК СуммаАванса,
	|	СУММА(РасчетыПоЛизингу.Сумма)                                      КАК СуммаАвансаВал,
	|	СУММА(РасчетыПоЛизингу.СуммаУпр)                                   КАК СуммаАвансаУпр
	|ИЗ
	|	РегистрНакопления.РасчетыПоФинансовымИнструментам КАК РасчетыПоЛизингу
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|	ПО
	|		РасчетыПоЛизингу.РасчетныйДокумент = ДанныеПервичныхДокументов.Документ
	|	
	|ГДЕ
	|	НЕ &СоздатьПустуюТаблицу
	|	И &ВключитьДанныеФинансовыхИнструментов
	|	И РасчетыПоЛизингу.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	И РасчетыПоЛизингу.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И РасчетыПоЛизингу.ТипСуммы В (
	|		ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.УслугаПоАрендеАванс),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ВыкупнаяСтоимостьАванс))
	|	И (РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Организация В (&Организации_АвансыВыданные))
	|	И (&ТекстОтборПоПараметрам)
	|	И ТИПЗНАЧЕНИЯ(РасчетыПоЛизингу.РасчетныйДокумент) В (&ТипыРасчетныхДокументовВыданных)
	|	И РасчетыПоЛизингу.Сумма > 0
	|	И ВЫБОР КОГДА &ИсключитьИУДА
	|		ТОГДА РасчетыПоЛизингу.НастройкаХозяйственнойОперации <> ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ИзменениеУсловийДоговораАренды)
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ
	|
	|СГРУППИРОВАТЬ ПО
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Организация,
	|	РасчетыПоЛизингу.РасчетныйДокумент,
	|	РасчетыПоЛизингу.Регистратор,
	|	РасчетыПоЛизингу.Договор,
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.НаправлениеДеятельности,
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Контрагент,
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Договор,
	|	НАЧАЛОПЕРИОДА(РасчетыПоЛизингу.Период, ДЕНЬ)
	//-- НЕ УТ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	РасчетныйДокумент
...
Процедура ПодготовитьВТ_АвансыВыданные_Погашения(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	|	РасчетыПоЛизингу.Регистратор                                       КАК ДокументРегистратор,
	|	НАЧАЛОПЕРИОДА(ДанныеПервичныхДокументов.ДатаРегистратора, ДЕНЬ)    КАК ДатаАванса,
	|	РасчетыПоЛизингу.Период                                            КАК ДатаПогашения,
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Контрагент              КАК Контрагент,
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Договор                 КАК Договор,
	|	РасчетыПоЛизингу.Договор                                           КАК ОбъектРасчетов,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС) КАК ТипНалогообложения,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗачетАвансаКлиента)    КАК ВидСобытия,
	|	МАКСИМУМ(РасчетыПоЛизингу.Валюта)                                  КАК ВалютаДокумента,
	|	1                                                                  КАК СобытиеЗнак,
	|	СУММА(РасчетыПоЛизингу.СуммаРегл)                                  КАК СуммаПогашения,
	|	СУММА(РасчетыПоЛизингу.Сумма)                                      КАК СуммаПогашенияВал,
	|	СУММА(РасчетыПоЛизингу.СуммаУпр)                                   КАК СуммаПогашенияУпр
	|ИЗ
	|	РегистрНакопления.РасчетыПоФинансовымИнструментам КАК РасчетыПоЛизингу
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	...
	|	И &ВключитьДанныеФинансовыхИнструментов
	|	И РасчетыПоЛизингу.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	И РасчетыПоЛизингу.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	И РасчетыПоЛизингу.ТипСуммы В (
	|		ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.УслугаПоАрендеАванс),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ВыкупнаяСтоимостьАванс))
	...
	|	И (&ТекстОтборПоПараметрам)
	|	И ТИПЗНАЧЕНИЯ(РасчетыПоЛизингу.РасчетныйДокумент) В (&ТипыРасчетныхДокументовВыданных)
	|	И ТИПЗНАЧЕНИЯ(РасчетыПоЛизингу.Регистратор) <> ТИП(Документ.ИзменениеУсловийДоговораАренды)
	|	И РасчетыПоЛизингу.Сумма > 0
	|
	|СГРУППИРОВАТЬ ПО
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Организация,
...
Процедура ПодготовитьВТ_ПереносыАвансовВыданных(МенеджерВременныхТаблиц, ПараметрыРасчета, ТаблицаНастроек = Неопределено) Экспорт
	...
	|ВЫБРАТЬ
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Организация                    КАК Организация,
	|	РасчетыПоЛизингу.Регистратор                                              КАК ДокументПереноса,
	|	РасчетыПоЛизингу.РасчетныйДокумент                                        КАК ДокументОплаты,
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам                                КАК АналитикаУчетаПоПартнерам,
	|	РасчетыПоЛизингу.Договор                                                  КАК ОбъектРасчетов,
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Контрагент                     КАК Контрагент,
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.НаправлениеДеятельности        КАК НаправлениеДеятельности,
	|	НЕОПРЕДЕЛЕНО                                                              КАК Подразделение,
	|	РасчетыПоЛизингу_Расход.АналитикаУчетаПоПартнерам                         КАК КорАналитикаУчетаПоПартнерам,
	|	РасчетыПоЛизингу_Расход.Договор                                           КАК КорОбъектРасчетов,
	|	РасчетыПоЛизингу_Расход.АналитикаУчетаПоПартнерам.Контрагент              КАК КорКонтрагент,
	|	РасчетыПоЛизингу_Расход.АналитикаУчетаПоПартнерам.НаправлениеДеятельности КАК КорНаправлениеДеятельности,
	|	НЕОПРЕДЕЛЕНО                                                              КАК КорПодразделение,
	|	РасчетыПоЛизингу.Период                                                   КАК Период,
	|	РасчетыПоЛизингу.НастройкаХозяйственнойОперации                           КАК НастройкаХозяйственнойОперации,
	|	РасчетыПоЛизингу.ИдентификаторФинЗаписи                                   КАК ИдентификаторФинЗаписи,
	|	МАКСИМУМ(РасчетыПоЛизингу.Валюта)                                         КАК Валюта,
	|	СУММА(РасчетыПоЛизингу.Сумма)                                             КАК Предоплата,
	|	СУММА(РасчетыПоЛизингу.СуммаРегл)                                         КАК ПредоплатаРегл,
	|	СУММА(РасчетыПоЛизингу.СуммаУпр)                                          КАК ПредоплатаУпр
	|ИЗ
	|	РегистрНакопления.РасчетыПоФинансовымИнструментам КАК РасчетыПоЛизингу
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	...
	|	ПО
	|		РасчетыПоЛизингу.Регистратор = ИзменениеУсловийДоговораАренды.Ссылка
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		РегистрНакопления.РасчетыПоФинансовымИнструментам КАК РасчетыПоЛизингу_Расход
	|	ПО РасчетыПоЛизингу_Расход.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		И РасчетыПоЛизингу_Расход.Регистратор = РасчетыПоЛизингу.Регистратор
	|		И РасчетыПоЛизингу_Расход.РасчетныйДокумент = РасчетыПоЛизингу.РасчетныйДокумент
	|		И РасчетыПоЛизингу_Расход.ИдентификаторФинЗаписи = РасчетыПоЛизингу_Расход.ИдентификаторФинЗаписи
	|		И (РасчетыПоЛизингу_Расход.Договор <> РасчетыПоЛизингу.Договор
	|			ИЛИ РасчетыПоЛизингу_Расход.АналитикаУчетаПоПартнерам.Контрагент <> РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Контрагент
	|			ИЛИ РасчетыПоЛизингу_Расход.АналитикаУчетаПоПартнерам.НаправлениеДеятельности <> РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.НаправлениеДеятельности)
	|ГДЕ
	|	&ВключитьДанныеФинансовыхИнструментов
	|	И РасчетыПоЛизингу.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	И РасчетыПоЛизингу.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И РасчетыПоЛизингу.НастройкаХозяйственнойОперации = ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ИзменениеУсловийДоговораАренды)
	|	И РасчетыПоЛизингу.ТипСуммы В (
	|		ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.УслугаПоАрендеАванс),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ВыкупнаяСтоимостьАванс))
	...
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам.Контрагент,
	|	РасчетыПоЛизингу.Договор,
	|	РасчетыПоЛизингу.АналитикаУчетаПоПартнерам,
	|	РасчетыПоЛизингу_Расход.АналитикаУчетаПоПартнерам.НаправлениеДеятельности,
	|	РасчетыПоЛизингу_Расход.АналитикаУчетаПоПартнерам.Контрагент,
	|	РасчетыПоЛизингу_Расход.Договор,
	|	РасчетыПоЛизингу_Расход.АналитикаУчетаПоПартнерам,
	|	РасчетыПоЛизингу.НастройкаХозяйственнойОперации,
	|	РасчетыПоЛизингу.ИдентификаторФинЗаписи
	//-- НЕ УТ
	|
	|;
	|
