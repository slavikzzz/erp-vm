...
Функция ПараметрыТоваровЯчейкиWS(ПараметрыЯчейки, ТоварыЯчейки, Знач ВерсияСервиса)
	...
	Если (ПараметрыЯчейки = Неопределено)
		...
		ПараметрыТоваровЯчейкиWS.ТекстСообщения = НСтр("ru = 'Не удалось получить информацию по штрихкоду';
														|en = 'Cannot receive barcode information'");
...
Функция ПроксиВебСервиса(ИмяПользователя, Пароль, Знач МестоположениеWSDL, Знач ВерсияСервиса = "") Экспорт
	...
	Если ПустаяСтрока(МестоположениеWSDL) Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка подключения к веб-сервису.
			...
			|en = 'An error occurred while connecting to the web service.
			|
			|Infobase URL is not specified in the connection parameters'");
	КонецЕсли;
	...
	Если ПустаяСтрока(ИмяПользователя) Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка подключения к веб-сервису.
			...
			|en = 'An error occurred while connecting to the web service.
			|
			|User is not specified in the connection parameters'");
	КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Подключение к информационной базе';
										|en = 'Connect to infobase'", КодОсновногоЯзыка),
		...
		ВызватьИсключение НСтр("ru = 'Ошибка подключения к веб-сервису.
			...
			|en = 'An error occurred when connecting to web service.
			|
			|Authorization might fail, the web service address is incorrect,
			|or infobase is not published on web server.
			|
