...
Процедура ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, Ссылка) Экспорт
	...
	Если Не ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Ссылка)) Тогда
		...
		НСтр("ru = 'Недопустимое значение параметра ""Ссылка"",
			...
			|Reference type value was expected, actual value: ""%1"" (type: %2)'"), Ссылка, ТипЗнч(Ссылка));
		Попытка // Такая конструкция нужна для проброса стека в журнал регистрации.
			ВызватьИсключение ТекстСообщения;
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Печать';
											|en = 'Print'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , ,
...
Функция ИспользуетсяПользовательскийМакет(ПутьКМакету) Экспорт
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Макет ""%1"" не существует. Операция прервана.';
			|en = 'Template ""%1"" does not exist. The operation is canceled.'"), ПутьКМакету);
...
Функция ОбластьМакета(СсылкаНаМакет, ОписаниеОбласти) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Тип области не указан или указан некорректно: %1.';
				|en = 'Area type is not specified or invalid: %1.'"), ОписаниеОбласти.ТипОбласти);
	КонецЕсли;
...
Функция ДанныеQRКода(QRСтрока, УровеньКоррекции, Размер) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Формирование QR-кода';
										|en = 'QR code generation'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция СписокОператоров(ДополнительныеПоля = Неопределено) Экспорт
	...
	ДобавитьОператорВГруппу(Группа, ИмяМодуляПечати() + РазделительКоманды() + "СтрокаЛатиницей", НСтр("ru = 'Строка латиницей';
																										|en = 'Latin string'"), Новый ОписаниеТипов("Строка"), Истина);
	...
	Для Каждого ИмяМакета Из ИменаМакетов Цикл
		...
		Если Отказ Тогда
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				...
				|en = 'Cannot generate the ""%1"" print form. Contact the administrator.'"), ИмяМакета);
			ВызватьИсключение ТекстСообщенияОбОшибке;
		КонецЕсли;
...
Процедура СообщитьПечатнаяФормаНедоступна(Объект)
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Печать %1 не выполнена: выбранная печатная форма недоступна.';
																					|en = 'Cannot print %1: the print form is unavailable.'"), Объект);
...
Функция ИмяФайлаПечатнойФормыПоУмолчанию(ОбъектПечати, НазваниеПечатнойФормы)
	...
	Иначе
		...
		Шаблон = НСтр("ru = '[НазваниеПечатнойФормы] - [ПредставлениеОбъекта] - [ТекущаяДата]';
						|en = '[НазваниеПечатнойФормы] - [ПредставлениеОбъекта] - [ТекущаяДата]'");
...
Процедура ДобавитьГруппуОператоровФункцииДляТабличныхЧастей(СписокОператоров)
	...
	ДобавитьОператорВГруппу(Группа, Префикс + "СуммаПоКолонке", НСтр("ru = 'Сумма по колонке';
																	|en = 'Amount by column'"), Тип, Истина);
	ДобавитьОператорВГруппу(Группа, Префикс + "КоличествоСтрок", НСтр("ru = 'Количество строк';
																		|en = 'Line quantity'"), Тип, Истина);
	ДобавитьОператорВГруппу(Группа, Префикс + "МаксимумПоКолонке", НСтр("ru = 'Максимум по колонке';
																		|en = 'Maximum value by column'"), Тип, Истина);
	ДобавитьОператорВГруппу(Группа, Префикс + "МинимумПоКолонке", НСтр("ru = 'Минимум по колонке';
																		|en = 'Minimum value by column'"), Тип, Истина);
	ДобавитьОператорВГруппу(Группа, Префикс + "СреднееПоКолонке", НСтр("ru = 'Среднее по колонке';
																		|en = 'Average value by column'"), Тип, Истина);
