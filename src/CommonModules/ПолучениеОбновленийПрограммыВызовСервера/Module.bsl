...
Функция ПараметрыПроверкиВерсииПлатформыПриЗапуске() Экспорт
	...
	Если ПараметрыБазовойФункциональности.РаботаВПрограммеЗапрещена Тогда
		ШаблонСообщения = НСтр("ru = 'Для работы с программой необходимо использовать версию платформы ""1С:Предприятие 8"" не ниже: <b>%1</b>
			...
			|en = 'For the proper functioning of the application, the minimum 1C:Enterprise 8 version must be: <b>%1</b>
			|Current version: %2.'");
	Иначе
	...
	Если Не Результат.ЭтоАдминистраторСистемы Тогда
		...
		ТекстСообщения = ТекстСообщения
			...
			+ НСтр("ru = 'Вход в программу невозможен.
				...
				|en = 'Logon to the application is not available.
				|Contact the Administrator for update of the 1C:Enterprise platform version.'");
	...
	ИначеЕсли ПараметрыБазовойФункциональности.РаботаВПрограммеЗапрещена Тогда
		...
		ТекстСообщения = ТекстСообщения
			...
			+ НСтр("ru = 'Вход в программу невозможен.
				...
				|en = 'Logon to the application is not available.
				|Update 1C:Enterprise platform version first.'");
	...
	Иначе
		...
		ТекстСообщения = ТекстСообщения + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Символы.ПС
			...
			+ НСтр("ru = 'Рекомендуется обновить версию платформы 1С:Предприятия. Новая версия платформы содержит исправления ошибок,
				...
				|en = 'It is recommended that you update the 1C:Enterprise platform version. The new platform version contains bug fixes
				|which improve the application stability.
				|
				|
				|You can also continue using the current platform version. %1
				|
				|
				|<span style=""font: ПояснениеВерсийПлатформыШрифт"">Platform version required by the application: %2, recommended version: %3 or higher, current version: %4.</span>'"),
			ТекущаяВерсияПлатформы,
