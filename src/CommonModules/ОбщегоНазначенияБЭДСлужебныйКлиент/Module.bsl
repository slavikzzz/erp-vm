...
// Выполняет проведение документов в случае согласия пользователя.
// 
// Параметры:
...
//   * ДокументыТребующиеПроведение - Массив Из ОпределяемыйТип.ОснованияЭлектронныхДокументовДокументОбъектЭДО
//   * ФормаИсточник                - ФормаКлиентскогоПриложения
//                                  - Неопределено
//   * ДокументыМассив              - Массив Из ОпределяемыйТип.ОснованияЭлектронныхДокументовДокументОбъектЭДО
//
Процедура ВыполнитьПроверкуПроведенияДокументовПродолжить(Результат, ДополнительныеПараметры) Экспорт
	
	ДокументыМассив = ДополнительныеПараметры.ДокументыМассив;
	ОбработкаПродолжения = ДополнительныеПараметры.ОбработкаПродолжения;
	ДокументыТребующиеПроведение = ДополнительныеПараметры.ДокументыТребующиеПроведение;
	
	Если Результат = КодВозвратаДиалога.Да И ЗначениеЗаполнено(ДокументыТребующиеПроведение) Тогда
		...
		Если КоличествоНепроведенныхДокументов > 0 Тогда
			...
			ПараметрыЗавершения = Новый Структура("ОбработкаПродолжения, ДокументыМассив");
			ПараметрыЗавершения.ОбработкаПродолжения = ОбработкаПродолжения;
			ПараметрыЗавершения.ДокументыМассив = ДокументыМассив;
			
			Обработчик = Новый ОписаниеОповещения("ВыполнитьПроверкуПроведенияДокументовЗавершить", ЭтотОбъект,
				ПараметрыЗавершения);
...
// Вызывается после уведомления пользователя о документах, которые не удалось провести, выполняет обработку оповещения.
// 
// Параметры:
//  Результат - КодВозвратаДиалога 
//            - Неопределено
...
//   * ОбработкаПродолжения - ОписаниеОповещения
//
Процедура ВыполнитьПроверкуПроведенияДокументовЗавершить(Результат, ДополнительныеПараметры) Экспорт
	
	ДокументыМассив = ДополнительныеПараметры.ДокументыМассив;
	ОбработкаПродолжения = ДополнительныеПараметры.ОбработкаПродолжения;
	
	Если Результат = КодВозвратаДиалога.Пропустить И ЗначениеЗаполнено(ДокументыМассив) Тогда
		ВыполнитьОбработкуОповещения(ОбработкаПродолжения, ДокументыМассив);
	Иначе
		ВыполнитьОбработкуОповещения(ОбработкаПродолжения);
	КонецЕсли;
