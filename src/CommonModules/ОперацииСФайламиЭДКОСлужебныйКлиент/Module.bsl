...
Процедура ДанныеССервераВФайл(ОповещениеОЗавершении, Адрес, ИмяФайлаИлиРасширение, ВыводитьСообщения, ДвоичныеДанные) Экспорт
	...
	Если Результат.Продолжение.Количество() = 0 Тогда
		Если Результат.СтрокаBase64 = Неопределено ИЛИ Результат.СтрокаBase64 = "" Тогда
			ОписаниеОшибки = НСтр("ru = 'Попытка передачи пустого файла.';
									|en = 'Попытка передачи пустого файла.'");
...
Процедура ЗапуститьПриложениеНаКлиентеПослеПодключенияРасширенияРаботыСФайлами(Подключено, ВходящийКонтекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru = 'Не удалось запустить приложение,
									...
									|en = 'Не удалось запустить приложение,
									|т.к. расширение для работы с файлами не подключено.'");
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(ОписаниеОшибки);
	КонецЕсли;
...
Процедура СохранитьФайлы(Знач СохраняемыеФайлы, КаталогСохранения, ВыводитьСообщения) Экспорт
	...
	Если ТипЗнч(СохраняемыеФайлы) = Тип("Массив") И СохраняемыеФайлы.Количество() = 0 Тогда
		...
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(НСтр("ru = 'Отсутствуют файлы для сохранения';
															|en = 'Отсутствуют файлы для сохранения'"));
		Возврат;
	КонецЕсли;
	...
	Если Подключено Тогда
		...
		Если ТребуетсяПоказатьДиалогВыбораКаталога Тогда
			...
			Диалог.Заголовок = НСтр("ru = 'Выберите каталог для сохранения';
									|en = 'Выберите каталог для сохранения'");
			...
			Если ВходящийКонтекст.ВыводитьСообщения Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось выгрузить на диск файл 
                          |%1!
                          |%2';
                          |en = 'Не удалось выгрузить на диск файл 
                          |%1!
		...
		Если НЕ ОбщегоНазначенияКлиент.ЭтоLinuxКлиент() И НЕ ОбщегоНазначенияКлиент.ЭтоMacOSКлиент() Тогда
			Если СтрДлина(Имя) > 260 Тогда
				ПоказатьПредупреждение(, НСтр("ru = 'Не удается сохранить файлы, потому что длина пути превышает 260 символов.
                                               ...
                                               |en = 'Не удается сохранить файлы, потому что длина пути превышает 260 символов.
                                               |Сохраните файлы в другую папку, например в корень диска С.'"));
				Возврат;
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		...
		Если ВходящийКонтекст.Свойство("ЗапроситьПодтверждениеОбновитьФайлы") Тогда
			...
			ТекстВопроса = НСтр("ru = 'В выбранном каталоге присутствуют файлы с такими же именами.
				...
				|en = 'В выбранном каталоге присутствуют файлы с такими же именами.
				|Продолжить сохранение с заменой файлов?'");
...
Процедура СохранитьФайлыПослеПолученияФайлов(ПолученныеФайлы, ВходящийКонтекст) Экспорт
	...
	Если ВходящийКонтекст.ВыводитьСообщения Тогда
		...
		Если КоличествоСохраняемыхФайлов <> 0 Тогда
			...
			СообщениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выгружено файлов: %1.';
																							|en = 'Выгружено файлов: %1.'"), КоличествоСохраненныхФайлов);
...
Процедура ДобавитьФайлыПослеПодключенияРасширенияРаботыСФайлами(Подключено, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru = 'Не установлено расширение для работы с файлами';
															|en = 'Не установлено расширение для работы с файлами'"));
...
Функция ТекстОшибкиРазмера(ОписанияФайлов, Знач МаксимальныйРазмерФайла)
	...
	Если НЕ ПустаяСтрока(ТекстОшибкиРазмера) Тогда
		...
		ШаблонСтроки       = НСтр("ru = 'Размер следующих файлов превышает максимально допустимый размер - %1:
                                             |%2';
                                             |en = 'Размер следующих файлов превышает максимально допустимый размер - %1:
                                             |%2'");
...
Функция ТекстОшибкиТипа(ОписанияФайлов, Знач ДопустимыеТипыФайлов)
	...
	Если НЕ ПустаяСтрока(ТекстОшибкиТипа) Тогда
		...
		ШаблонСтроки       = НСтр("ru = 'Типы следующих файлов не соответствуют требованиям (допустимые типы - ""%1""):
                                             |%2';
                                             |en = 'Типы следующих файлов не соответствуют требованиям (допустимые типы - ""%1""):
                                             |%2'");
		...
		Для Каждого ОписаниеФайла Из ФайлыДокумента Цикл
			Если ТипыФайлов.Найти(НРег(ОписаниеФайла.Расширение)) = Неопределено Тогда
				...
				ШаблонСтроки = НСтр("ru = 'Тип файла ""%1"" (%2) не соответствует требованиям. 
	                                 ...
	                                 |en = 'Тип файла ""%1"" (%2) не соответствует требованиям. 
	                                 |Допустимые типы файлов - ""%3"".'");
...
Функция ОписаниеОшибкиНеПолученКаталогВременныхФайлов()
	...
	Возврат НСтр("ru = 'Не удалось получить каталог временных файлов на клиенте.';
				|en = 'Не удалось получить каталог временных файлов на клиенте.'");
...
Функция ОписаниеОшибкиНеУдалосьСоздатьКаталог()
	...
	Возврат НСтр("ru = 'Не удалось создать каталог на клиенте.';
				|en = 'Не удалось создать каталог на клиенте.'");
...
Функция ОписаниеОшибкиРасширениеДляРаботыСФайламиНеПодключено()
	...
	Возврат НСтр("ru = 'Расширение для работы с файлами не подключено. 
                  ...
                  |en = 'Расширение для работы с файлами не подключено. 
                  |Дальнейшее выполнение операции невозможно.'");
...
Функция ОписаниеОшибкиОшибкаЧтенияФайла(ИмяФайла)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка чтения файла %1.';
			|en = 'Ошибка чтения файла %1.'"), ИмяФайла);
...
Функция ОписаниеОшибкиОшибкаСохраненияФайла(ИмяФайла)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Во время сохранения файла %1 возникла ошибка.';
			|en = 'Во время сохранения файла %1 возникла ошибка.'"), ИмяФайла);	
...
Функция ОписаниеОшибкиНеУдалосьУдалитьФайл(ИмяФайла, ИнформацияОбОшибке)
	...
	Шаблон = НСтр("ru = 'Не удалось удалить файл или каталог
                  ...
                  |по причине:
                  |%2';
                  |en = 'Не удалось удалить файл или каталог
                  |%1
                  |по причине:
                  |%2'");
...
Функция ОписаниеОшибкиФайлНеСуществует(ИмяФайла)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка обращения к файлу %1 - файл не существует.';
			|en = 'Ошибка обращения к файлу %1 - файл не существует.'"), ИмяФайла);	
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при вызове ""%1"" объекта для работы с файлами.
				|%2';
				|en = 'Ошибка при вызове ""%1"" объекта для работы с файлами.
				|%2'"),
...
Функция ИмяСобытия()
	...
	Возврат НСтр("ru = 'Электронный документооборот с контролирующими органами. Операции с файлами';
				|en = 'Электронный документооборот с контролирующими органами. Операции с файлами'");
