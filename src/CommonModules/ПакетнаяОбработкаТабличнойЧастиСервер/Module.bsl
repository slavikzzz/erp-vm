		...
		Если КэшированныеЗначения.ЕстьЗависимыеОбработчики
			И Не КэшированныеЗначения.Свойство("ОбработкаКоллекции") Тогда
				...
				ТекстИсключения = СтрШаблон(НСтр("ru = 'Некорректно встроен механизм пакетной обработки строк табличных частей.
										...
										|%1, передав в параметры только строки, требующие обработки.';
										|en = 'The tool for batch processing of table rows is built in incorrectly.
										|The action structure contains actions that depend on each other.
										|To process the collection rows in this case, use the 
										|%1 method, passing only rows that require processing to the parameters.'"), ИмяМетода);
...
Процедура ЗаполнитьНазначениеМатериалыВСтрокеСверхЗаказа(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если ТекущаяСтрока.КодСтроки = 0 Тогда
		...
		Если ТекущаяСтрока.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар")
			...
			ДанныеДляЗаполнения = ДанныеДляОбработкиСтроки("НазначениеМатериаловПоЗаказуДавальца", КэшированныеЗначения)[0];
				
			Если ЗначениеЗаполнено(ТекущаяСтрока.ЗаказДавальца) Тогда
				ТекущаяСтрока.Назначение = ДанныеДляЗаполнения.Назначение;
			Иначе
				ТекущаяСтрока.Назначение = ДанныеДляЗаполнения.НазначениеПоЗаказуДавальцаИзПараметров;
			КонецЕсли;
		Иначе
...
Процедура ЗаполнитьПризнакДоступноОформлениеОтчетаДавальцу(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	ДанныеДляОбработки = ДанныеДляОбработкиСтроки("ОформлениеОтчетаДавальцу", КэшированныеЗначения);
	
	ТекущаяСтрока.ДоступноОформлениеОтчетаДавальцу = ДанныеДляОбработки[0].ДоступноОформлениеОтчетаДавальцу;
	ТекущаяСтрока.ОформитьОтчетДавальцу            = ДанныеДляОбработки[0].ОформитьОтчетДавальцу;
...
// Заполняет получателя по данным направление выпуска текущей строки табличной части документа "Производство без заказа"
//
// Параметры:
...
Процедура ПроверитьЗаполнитьДолюСтоимости(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если ПроизводствоКлиентСервер.ДоляСтоимостиРассчитывается(ТекущаяСтрока, ПараметрыРаспределенияЗатрат) Тогда
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
			РезультатРасчета = ДанныеДляОбработкиСтроки("ДолиСтоимости", КэшированныеЗначения);
		КонецЕсли;
...
//++ НЕ УТКА
Процедура ПроверитьЗаполнитьОбеспечениеВЭтапеПроизводства(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Перем ПараметрыДействия;
	
	СтруктураДействий.Свойство("ПроверитьЗаполнитьОбеспечениеВЭтапеПроизводства", ПараметрыДействия);
	
	ПараметрыОбеспечения = ДанныеДляОбработкиСтроки("ПараметрыОбеспеченияВЭтапеПроизводства", КэшированныеЗначения);
	Если ПараметрыОбеспечения <> Неопределено Тогда
		
		Если ПараметрыДействия.ЗаполнитьВариантОбеспечения Тогда
			ТекущаяСтрока.ВариантОбеспечения = ПараметрыОбеспечения.ВариантОбеспечения;
		КонецЕсли;
		
		Если ПараметрыДействия.ЗаполнитьСклад Тогда
			ТекущаяСтрока.Склад = ПараметрыОбеспечения.Склад;
		КонецЕсли;
		
		Если ПараметрыДействия.ЗаполнитьОбособленно Тогда
...
Процедура ПроверитьЗаполнитьСкладПроизводства(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Перем ПараметрыДействия;
	
	СтруктураДействий.Свойство("ПроверитьЗаполнитьСкладПроизводства", ПараметрыДействия);
	
	ПараметрыЗаполнения = ДанныеДляОбработкиСтроки("ПараметрыЗаполненияСкладаПроизводства", КэшированныеЗначения);
	Если ПараметрыЗаполнения <> Неопределено Тогда
		
		СписокПолей = СтрРазделить(ПараметрыДействия.ИмяПоляСклад, ",");
		Для каждого ИмяПоляСклад Из СписокПолей Цикл
			ТекущаяСтрока[ИмяПоляСклад] = ПараметрыЗаполнения.Склад;
		КонецЦикла;
...
Процедура ЗаполнитьЦенуПродажиПоАссортименту(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	СтруктураДействий.Свойство("ЗаполнитьЦенуПродажиПоАссортименту", ПараметрыДействия);
...
Процедура ЗаполнитьУсловияПродаж(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	СтруктураДействий.Свойство("ЗаполнитьУсловияПродаж", ПараметрыДействия);
	...
	СтруктураДействий.Свойство("ЗаполнитьУсловияПродажУслуги", ПараметрыДействия);
...
Процедура ЗаполнитьУсловияРозничныхПродаж(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	СтруктураДействий.Свойство("ЗаполнитьУсловияРозничныхПродаж", ПараметрыДействия);
...
Процедура ЗаполнитьНоменклатуруПартнераПоНоменклатуре(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	СтруктураДействий.Свойство("ЗаполнитьНоменклатуруПартнераПоНоменклатуре", ПараметрыДействия);
...
Процедура ЗаполнитьНоменклатуруПоНоменклатуреПартнера(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	СтруктураДействий.Свойство("ЗаполнитьНоменклатуруПоНоменклатуреПартнера", ПараметрыДействия);
...
Процедура ЗаполнитьЦенуЗакупки(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	СтруктураДействий.Свойство("ЗаполнитьЦенуЗакупки", ПараметрыДействия);
...
Процедура ЗаполнитьУсловияЗакупок(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	СтруктураДействий.Свойство("ЗаполнитьУсловияЗакупок", ПараметрыДействия);
...
Процедура ЗаполнитьУчетноеКоличествоНоменклатуры(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	СтруктураДействий.Свойство("ЗаполнитьУчетноеКоличествоНоменклатуры", ПараметрыДействия);
...
Процедура ПересчитатьКоличествоУпаковок(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	СтруктураДействий.Свойство("ПересчитатьКоличествоУпаковок", ПараметрыДействия);
...
Процедура ПриДобавленииОбработчиковСтрокКоллекции(ОбработчикиСтрок) Экспорт
	...
	//++ НЕ УТКА
	ОбработчикиСтрок.Добавить("ПроверитьЗаполнитьСкладПроизводства",
		 "ПакетнаяОбработкаТабличнойЧастиСервер.ПроверитьЗаполнитьСкладПроизводства");
	//-- НЕ УТКА
	...
	//++ НЕ УТКА
	ОбработчикиСтрок.Добавить("ПроверитьЗаполнитьОбеспечениеВЭтапеПроизводства",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ПроверитьЗаполнитьОбеспечениеВЭтапеПроизводства");
	//-- НЕ УТКА
	...
	//++ НЕ УТКА
	ОбработчикиСтрок.Добавить("ЗаполнитьДатуПроизводства",
		 "ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьДатуПроизводства");

	ОбработчикиСтрок.Добавить("УстановитьПризнакИспользованияПлановойСтоимости",
		 "ПакетнаяОбработкаТабличнойЧастиСервер.УстановитьПризнакИспользованияПлановойСтоимостиВСтрокеТЧ");
	//-- НЕ УТКА

	//++ НЕ УТ
	ОбработчикиСтрок.Добавить("ЗаполнитьЦенуПередачиПродукцииДавальцу",
		 "ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьЦенуПередачиПродукцииДавальцуВСтрокеТЧ");
	...
	//++ НЕ УТКА
	ОбработчикиСтрок.Добавить("ПересчитатьСуммуУслугиЗаказДавальца",
		 "ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПересчитатьСуммуУслугиВСтрокеТЧЗаказДавальца");

	ОбработчикиСтрок.Добавить("ПересчитатьСуммуЗаказДавальца",
		 "ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПересчитатьСуммуВСтрокеТЧЗаказДавальца");
	...
	//++ НЕ УТКА
	ОбработчикиСтрок.Добавить("ПересчитатьЦенуПоСуммеЗаказДавальца",
		 "ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПересчитатьЦенуПоСуммеВСтрокеТЧЗаказДавальца");
	...
	//++ НЕ УТКА
	ОбработчикиСтрок.Добавить("ЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства",
		 "ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства");

	ОбработчикиСтрок.Добавить("ЗаполнитьНазначениеВЭтапеПроизводства",
		 "ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьНазначениеВЭтапеПроизводства");
	...
	//++ НЕ УТКА
	ОбработчикиСтрок.Добавить("ПроверитьЗаполнитьКлючСвязиСпецификации",
		 "ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПроверитьЗаполнитьКлючСвязиСпецификации");

	//++ Устарело_Переработка24
	ОбработчикиСтрок.Добавить("ЗаполнитьНазначениеМатериалыВСтрокеСверхЗаказа",
		 "ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьНазначениеМатериалыВСтрокеСверхЗаказа");

	ОбработчикиСтрок.Добавить("ЗаполнитьПризнакДоступноОформлениеОтчетаДавальцу",
		 "ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьПризнакДоступноОформлениеОтчетаДавальцу");

	//-- Устарело_Переработка24
	ОбработчикиСтрок.Добавить("ЗаполнитьДействиеПоПерерабатываемымМатериалам",
		 "ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьДействиеПоПерерабатываемымМатериаламВСтрокеТЧ");
	...
	//++ НЕ УТКА
	ОбработчикиСтрок.Добавить("ПроверитьЗаполнитьЦенуВыпуска",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ПроверитьЗаполнитьЦенуВыпуска");
	//-- НЕ УТКА
...
Процедура ДополнитьТекстыЗапросовИсточниковДанных(Действия, КэшированныеЗначения)
	...
	//++ Устарело_Переработка24
	ПродажиСервер.ДополнитьТекстЗапросаНазначениеМатериаловПоЗаказуДавальца(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	//-- Устарело_Переработка24
	...
	//++ НЕ УТКА
	ДавальческаяСхема.ДополнитьТекстЗапросаОформлениеОтчетаДавальцу(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	//-- НЕ УТКА
	...
	ПроизводствоСервер.ДополнитьТекстЗапросаЗаполнитьДолюСтоимостиПакетно(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	ПроизводствоСервер.ДополнитьТекстЗапросаРеквизитыПоНазначению(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	//-- НЕ УТ

	//++ НЕ УТКА
	ОбеспечениеПроизводства.ДополнитьТекстЗапросаЗаполнитьОбеспечениеВЭтапеПроизводстваПакетно(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	ОбеспечениеПроизводства.ДополнитьТекстЗапросаЗаполнитьСкладПроизводства(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	//-- НЕ УТКА
...
Процедура ПроверитьЗависимостьОбработчиков(ОбработчикЗависимый, ОбработчикВедомый, ОписанияЗависимостей, Цепочка)
	...
	Если ИндексВЦепочке <> Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru = 'Некорректно встроен механизм пакетной обработки строк табличных частей.
									...
									|%1';
									|en = 'The tool for batch processing of table rows is built in incorrectly.
									|Cycling is found in descriptions of dependencies between row handlers:
									|
									|%1'");
