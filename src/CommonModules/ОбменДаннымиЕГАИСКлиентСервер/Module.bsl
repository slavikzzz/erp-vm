...
Функция НоваяНастройкаОбменаЕГАИС() Экспорт
	...
	НастройкиОбменаЕГАИС.Вставить("ИспользоватьЗащищенноеСоединение", Ложь);
...
// * ТипЗапроса - Строка -
// * АдресЗапроса - Строка -
// * ТелоЗапроса - Строка, ДвоичныеДанные -
// * ТекстXML - Строка -
	...
	Иначе
		...
		Иначе
			...
			ЗаголовокОшибки = СтрШаблон(НСтр("ru = 'Код состояния HTTP: %1.';
											|en = 'Код состояния HTTP: %1.'"), РезультатОтправкиHTTPЗапроса.КодСостояния);
		КонецЕсли;
