...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
		"Документ.ОбращениеСФРКСтрахователю");
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
		"Документ.ОтветСтрахователяНаОбращениеСФР");
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
	...
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
		"РегистрСведений.МашиночитаемыеДоверенностиОбмена");
	...
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
		"РегистрСведений.ЗапросыДляПовторнойОтправкиСЭДО");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("РегламентированнаяОтчетность.ЕдиныйНалоговыйСчет") Тогда
		...
		МодульЕдиныйНалоговыйСчетИнтеграция.ПриЗаполненииСписковСОграничениемДоступа(Списки);
	КонецЕсли;
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени("Документ.ОбращениеСФРКСтрахователю");
	Если НЕ ОбъектМетаданных = Неопределено Тогда
	...
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени("Документ.ОтветСтрахователяНаОбращениеСФР");
	Если НЕ ОбъектМетаданных = Неопределено Тогда
	...
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени("РегистрСведений.ЗапросыДляПовторнойОтправкиСЭДО");
	Если НЕ ОбъектМетаданных = Неопределено Тогда
	...
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени("РегистрСведений.МашиночитаемыеДоверенностиОбмена");
	Если НЕ ОбъектМетаданных = Неопределено Тогда
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("РегламентированнаяОтчетность.ЕдиныйНалоговыйСчет") Тогда
		МодульЕдиныйНалоговыйСчетИнтеграция = ОбщегоНазначения.ОбщийМодуль("ЕдиныйНалоговыйСчетИнтеграция");
		МодульЕдиныйНалоговыйСчетИнтеграция.ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание);
	КонецЕсли;
	// Конец ЕдиныйНалоговыйСчетИнтеграция
...
Процедура ПриДобавленииВидовРегламентированныхОтчетов(ВидыОтчетов, ДополнительныеПараметры) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеРегламентированныхОтчетов") Тогда
		...
		Описатель = МодульПолучениеРегламентированныхОтчетов.ОписаниеВидаРегламентированногоОтчета();
		
		Описатель.ИдентификаторВида = "EXT_REPORT_ARC";
		ВидыОтчетов.Добавить(Описатель);
...
Процедура ПриЗагрузкеРегламентированногоОтчета(
	...
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеРегламентированныхОтчетов")
		Тогда
	...
	Если ОписаниеФайла.ИдентификаторВидаОтчета = "EXT_REPORT_ARC" Тогда
		
		Справочники.РегламентированныеОтчеты.ОбновитьДанныеОтчетаИзФайла(
			ОписаниеФайла, Обработан, ДополнительныеПараметры);
...
Функция ПустаяТаблицаОписанияОбъектовРегламентированнойОтчетности() Экспорт
	...
	ТаблицаОписания.Колонки.Добавить("Хеш", ОбщегоНазначения.ОписаниеТипаСтрока(255));
	
	Возврат ТаблицаОписания;
	
КонецФункции

// Заполняет наименование и ключевые слова поиска таблицы для получения данных из переопределяемого метода.
//
// Параметры:
...
Процедура ЗаполнитьНаименованиеВТаблицеОписанияОбъектовРегламентированнойОтчетности(ТаблицаОписания) Экспорт
	Для Каждого Стр Из ТаблицаОписания Цикл
		Если НЕ ЗначениеЗаполнено(Стр.Наименование) Тогда
			Стр.Наименование = Метаданные.НайтиПоТипу(Стр.ТипОбъекта).Представление();
		КонецЕсли;
		Если Стр.ВидДокумента = Перечисления.СтраницыЖурналаОтчетность.Уведомления И Не ЗначениеЗаполнено(Стр.Хеш) Тогда
			Стр.Хеш = НРег(СтрЗаменить(Строка(Стр.ВидКонтролирующегоОргана) + Стр.Наименование, " ", ""));
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ДобавитьКнопкуПрисоединенныеФайлы(Форма) Экспорт
	...
	Если ТипЗнч(Форма) = Тип("ФормаКлиентскогоПриложения") Тогда
		Если СтрНайти(Форма.ИмяФормы, "Отчет.РегламентированныйОтчет") > 0
			Или СтрНайти(Форма.ИмяФормы, "Отчет.EF_РегламентированныйОтчет") > 0
			Или СтрНайти(Форма.ИмяФормы, "Отчет.РегламентированноеУведомление") > 0
			Или СтрНайти(Форма.ИмяФормы, "Отчет.EF_РегламентированноеУведомление") > 0 Тогда
...
Процедура СохранитьСтатусОтправкиРеглОтчета(Форма) Экспорт
	Если ТипЗнч(Форма.СтруктураРеквизитовФормы.мСохраненныйДок) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда 
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			Форма.СтруктураРеквизитовФормы.мСохраненныйДок, "Ссылка, ИсточникОтчета");
	Иначе
		Реквизиты = Новый Структура("Ссылка, ИсточникОтчета", 
			Форма.СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка, 
			Форма.СтруктураРеквизитовФормы.мСохраненныйДок.ИсточникОтчета);
	КонецЕсли;
	
	ВыборкаЖОС = РегистрыСведений.ЖурналОтчетовСтатусы.Выбрать(Новый Структура("Ссылка", Реквизиты.Ссылка));
	ЭтоОтчетМинобороны = Реквизиты.ИсточникОтчета = "РегламентированныйОтчетИсполнениеКонтрактовГОЗ"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетКассовыйПланГОЗ"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетЦенаПродукцииГОЗ";
	...
	Если ВыборкаЖОС.Следующий() Тогда
		ТекСтатус = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																				|en = '%1'"), Форма.Элементы.НаименованиеЭтапа.Заголовок);
		ТекСтатусМодифицирован = ДокументооборотСКОКлиентСервер.СтатусСданМодифицирован();
		
		Если РегламентированнаяОтчетностьКлиентСервер.ОбъектНеОтправлялсяЧерез1СОтчетность(ВыборкаЖОС.СостояниеСдачиОтчетности)
			ИЛИ ТекСтатус = ТекСтатусМодифицирован
			ИЛИ ЭтоОтчетМинобороны Тогда
			
			СтатусыОбъектов = РегламентированнаяОтчетностьКлиентСервер.СтатусыОбъектовРеглОтчетностиПриРучномВводе(Форма, КонтроллирующийОрган);
			Если ЭтоОтчетМинобороны И ТекСтатус = ДокументооборотСКОКлиентСервер.СтатусПереданоВМинобороны() Тогда
				ТекСтатус = "Передано";
			КонецЕсли;
...
Функция СвойстваРегламентированногоОтчета(Ссылка) Экспорт
	...
	Если СтрЧислоВхождений(ОрганизацияОтчета, НСтр("ru = '<Объект не найден>';
													|en = '<Object is not found>'")) > 0 Тогда
		ОрганизацияОтчета = "";
	КонецЕсли;
	...
	ИначеЕсли ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ФНС Тогда
		...
		Если Ссылка.ИсточникОтчета = "РегламентированныйОтчетОбученныеПоОхранеТрудаЛица"
			ИЛИ Ссылка.ИсточникОтчета = "РегламентированныйОтчетСоответствиеУсловийТруда"
	...
	ИначеЕсли ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ФСГС Тогда
		...
		Попытка
			...
			Иначе
				...
				Если Признак Тогда
					КодКонтролирующегоОргана = ПолучитьКодОрганаФСГССтатотчета(Ссылка);
				Иначе
	...
	ИначеЕсли ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ФСС Тогда
		...
		Если СтрДлина(КодОрганаФСС) <> 5 И ЗначениеЗаполнено(ОрганизацияОтчета) Тогда
			ИсточникОтчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "ИсточникОтчета");
			Если ИсточникОтчета = "РегламентированныйОтчетПодтверждениеВидаДеятельности" Тогда
				КонтекстЭДОСервер = ДокументооборотСКО.ПолучитьОбработкуЭДО();
				Если КонтекстЭДОСервер <> Неопределено Тогда
					КодОрганаФСС = КонтекстЭДОСервер.СвойстваОрганизацииДляОбмена(ОрганизацияОтчета,,, Истина).КодПодчиненностиФСС;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		КодКонтролирующегоОргана = ?(ПустаяСтрока(КодОрганаФСС) И ЗначениеЗаполнено(ОрганизацияОтчета),
			Лев(ОрганизацияОтчета.КодПодчиненностиФСС, 5), КодОрганаФСС);
...
Функция ПолучитьСсылкуНаРеглОтчет(ИсточникОтчета, Организация, ДатаОкончания, КорректирующийОтчет = Ложь, РегистрацияВНалоговомОргане = Неопределено, ФормаОтчета = "") Экспорт
	...
	ОсновныеКПП = Новый Массив;
	// В разных декларациях используются разные подходы по указанию КПП
	// Вариант 1. КПП всегда указывает на регистрацию по месту нахождения организации (например ХХХХ01001) - 
	//   тогда определить принадлежность декларации в регистрации в налоговом органе можно по коду налогового органа.
	//   Например, такой подход используется в декларации по налогу на имущество.
	// Вариант 2. КПП может указывать на регистрацию по месту нахождения организации
	//   или на регистрацию по иным основаниям (например, обособленного подразделения) - 
	//   тогда определить регистрацию можно по этому КПП.
	//   Например, такой подход используется в декларации по налогу на прибыль.
	СведенияОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
		Организация,
		ДатаОкончания,
		"КППЮЛ, КППЮЛКрупнейший");
		
	Если СведенияОрганизации.Свойство("КППЮЛ") Тогда
		ОсновныеКПП.Добавить(СведенияОрганизации.КППЮЛ);
	КонецЕсли;
	Если СведенияОрганизации.Свойство("КППЮЛКрупнейший") Тогда
		ОсновныеКПП.Добавить(СведенияОрганизации.КППЮЛКрупнейший);
	КонецЕсли;
	// В качестве пустого значения используем строку из пробелов, так как в документе этот реквизит имеет фиксированную длину
	ОсновныеКПП.Добавить(СтроковыеФункцииКлиентСервер.СформироватьСтрокуСимволов(
		" ",
		Метаданные.Документы.РегламентированныйОтчет.Реквизиты.КПП.Тип.КвалификаторыСтроки.Длина));
	...
	|	РегламентированныйОтчет.КПП В (&ОсновныеКПП) КАК ОсновнойКПП,
	...
	" 	И (РегламентированныйОтчет.КПП В (&ОсновныеКПП)
	|				И РегламентированныйОтчет.КодНалоговогоОргана В
	|					(ВЫБРАТЬ
	|						НалоговыйОрган.Код
	|					ИЗ
	|						НалоговыйОрган)
	|			ИЛИ НЕ РегламентированныйОтчет.КПП В (&ОсновныеКПП) И (РегламентированныйОтчет.КПП) В
	|				(ВЫБРАТЬ
	|					НалоговыйОрган.КПП
	|				ИЗ
	|					НалоговыйОрган))", "");
	...
	Запрос.УстановитьПараметр("ОсновныеКПП",    ОсновныеКПП);
	
	РезультатЗапроса = Запрос.Выполнить().Выгрузить();
	
	РезультатЗапроса.Сортировать("ОсновнойКПП Убыв, Дата Убыв, Вид Убыв, Ссылка Убыв", Новый СравнениеЗначений);
...
Функция ТребованияРосстатаПоОКУД(МассивОКУД) Экспорт
	...
	// Необходимо установить соответствие между кодом ОКУД формы Росстата и конкретными правилами списка задач (требованиями),
	// которые определяют периодичность и сроки сдачи этой формы. По одноЙ форме (коду ОКУД) может быть несколько правил,
	// т.к. для разных групп респондентов форма может быть одна, но периодичность ее сдачи разная. Поэтому и правил,
	// связанных с одной формой, может быть несколько.
	// 
	// Алгоритм получения соответствия следующий:
	// Шаг 1.Получить соответствие правила и регл. отчета (исполнителя правила)
	// Шаг 2 По каждому регл. отчету из шага 1 определить ОКУД этого отчета. К найденному ОКУД в таблице результата
	//       дописать правило с шага 1:
	//   Шаг 2.1 Определить ОКУД через метод модуля менеджера регл. отчета.
	//   Шаг 2.2 По найденному ОКУД находим строку результирующей таблицы
	//   Шаг 2.3 Добавляем в требования по данному ОКУД правило из соответствия с шага 1
	// Шаг 3 Дополнить результат данными прямого сопоставления - на тот случай, если отчет не реализован в конфигурации,
	//       а правило (требование) есть
	
	Если Не ЗначениеЗаполнено(МассивОКУД) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ТЗРезультат = Новый ТаблицаЗначений;
	ТЗРезультат.Колонки.Добавить("ОКУД");
	ТЗРезультат.Колонки.Добавить("Требования"); // массив
	
	Для Каждого ОКУД Из МассивОКУД Цикл
		СтрокаРезультата = ТЗРезультат.Добавить();
		СтрокаРезультата.ОКУД = ОКУД;
		СтрокаРезультата.Требования = Новый Массив;
	КонецЦикла;
	
	// Шаг 1
	ИсполнителиТребований = РегламентированнаяОтчетностьПовтИсп.ИсполнителиТребованийРосстата();
	
	ТЗИсполнителиТребований = Новый ТаблицаЗначений;
	ТЗИсполнителиТребований.Колонки.Добавить("ИДОтчета");
	ТЗИсполнителиТребований.Колонки.Добавить("Требование");
	
	Для Каждого КлючИЗначение Из ИсполнителиТребований Цикл
		НовСтр = ТЗИсполнителиТребований.Добавить();
		НовСтр.ИДОтчета   = КлючИЗначение.Значение;
		НовСтр.Требование = КлючИЗначение.Ключ;
	КонецЦикла;
	
	// Шаг 2
	ТЗОтчеты = ТЗИсполнителиТребований.Скопировать();
	ТЗОтчеты.Свернуть("ИДОтчета");
	
	Для Каждого Отчет Из ТЗОтчеты Цикл
		
		ИмяОтчета = Отчет.ИДОтчета;
		
		Если Метаданные.Отчеты.Найти(ИмяОтчета) <> Неопределено Тогда
			
			ОКУДыОтчета = Новый ТаблицаЗначений;
			ОКУДыОтчета.Колонки.Добавить("ОКУД");
			
			// Шаг 2.1
			Для Каждого СтрДФ Из Отчеты[ИмяОтчета].ДеревоФормИФорматов().Строки Цикл
				НовСтр = ОКУДыОтчета.Добавить();
				НовСтр.ОКУД = ?(СтрДлина(СтрДФ.Код) = 6, "0" + СтрДФ.Код, СтрДФ.Код);
			КонецЦикла;
			
			ОКУДыОтчета.Свернуть("ОКУД");
			
			Для Каждого ОКУДОтчета Из ОКУДыОтчета Цикл
				
				// Шаг 2.2
				СтрокаРезультата = ТЗРезультат.Найти(ОКУДОтчета.ОКУД, "ОКУД");
				Если СтрокаРезультата = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				// Шаг 2.3
				НайденныеСтроки = ТЗИсполнителиТребований.НайтиСтроки(Новый Структура("ИДОтчета", ИмяОтчета));
				Для Каждого СтрокаТребования Из НайденныеСтроки Цикл
					Если СтрокаРезультата.Требования.Найти(СтрокаТребования.Требование) = Неопределено Тогда
						СтрокаРезультата.Требования.Добавить(СтрокаТребования.Требование);
					КонецЕсли;
				КонецЦикла;
	...
	// Шаг 3
	Для Каждого ТребованияПоОКУД Из РегламентированнаяОтчетностьПовтИсп.НеподдерживаемыеТребованияРосстата() Цикл
		СтрокаРезультата = ТЗРезультат.Найти(ТребованияПоОКУД.Ключ, "ОКУД");
		Если СтрокаРезультата = Неопределено Или СтрокаРезультата.Требования.Количество() > 0 Тогда
			// Не требуется или уже заполнена по данным отчета-исполнителя требования
			Продолжить;
		КонецЕсли;
		СтрокаРезультата.Требования = ТребованияПоОКУД.Значение;
	КонецЦикла;
...
Процедура УдалитьСтарыеПечатныеБланкиРегламентированныхОтчетов() Экспорт
	Попытка
		...
		|ГДЕ
		|	ПечатныеБланкиРегламентированныхОтчетов.ДатаЗаписи < &ДатаОтбора";
		
		ДатаОтбора = ДобавитьМесяц(ТекущаяДатаСеанса(), -1);
		...
		Пока Выборка.Следующий() Цикл 
			...
			НаборЗаписей.Отбор.РегламентированныйОтчет.Установить(Выборка.РегламентированныйОтчет);
			...
			Пока Индекс > 0 Цикл 
				...
				Если Запись.ДатаЗаписи < ДатаОтбора Тогда 
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("ОчисткаПечатныеБланкиРегламентированныхОтчетов",
...
Процедура Автозаполнение_РеестрПоНДС_КНД1155221(ПереданныеПараметры, НеиспользуемыйАдресХранилища) Экспорт
...
Процедура Автозаполнение_РеестрПоНДС_Приложение1(ПереданныеПараметры, НеиспользуемыйАдресХранилища) Экспорт
	...
	Если ВРег(СокрЛП(ПереданныеПараметры.ИДРедакцииОтчета)) = ВРег("ФормаОтчета2024Кв1") Тогда
	...
	Иначе// "ФормаОтчета2021Кв2", "ФормаОтчета2020Кв2", "ФормаОтчета2015Кв4"
...
Функция КоллекцияЦветовРаскраски() Экспорт
	
	Результат = Новый Структура;
	
	// Используется в механизмах автозаполнения по сведениям информационной базы для оформления показателя
	// значение которого заменяется на значение введенное пользователем ("_1").
	Результат.Вставить("ЦветЗаполняетсяТолькоВручную", ЦветаСтиля.ЦветМакетаБРОАвтозаполнениеРучное);
	
	// Используется в механизмах автозаполнения по сведениям информационной базы для оформления показателя
	// значение которого корректируется на величину, введенную пользователем ("_2").
	Результат.Вставить("ЦветЗаполняетсяАвтоКорректир", ЦветаСтиля.ЦветМакетаБРОАвтозаполнениеСКорректировкой);
	
	// Используется в механизмах автозаполнения по сведениям информационной базы для оформления показателя
	// значение которого получено из сведений ИБ и не подвергается изменениям ("_3").
	Результат.Вставить("ЦветЗаполняетсяАвтоБезКорректир", ЦветаСтиля.ЦветМакетаБРОАвтозаполнение);
	
	// Используется для оформления показателей, значение которых получено на основании других показателей ("_4").
	Результат.Вставить("ЦветЗаполняетсяАвтоАрифметика", ЦветаСтиля.ЦветМакетаБРОАвторасчет);
	
	// Используется для оформления показателей, которые пользователь должен заполнить вручную ("_0").
	Результат.Вставить("ЦветЗаполняетсяОднозначноВручную", ЦветаСтиля.ЦветМакетаБРОРучноеЗаполнение);
	
	Результат.Вставить("ЦветЗаполняетсяАвтоматически", ЦветаСтиля.ЦветМакетаБРОФон);
	Результат.Вставить("ЦветАвто", ЦветаСтиля.ЦветМакетаБРОТекст);
...
Процедура СформироватьСтруктуруЦветовРаскраски(Форма) Экспорт
	Цвета = КоллекцияЦветовРаскраски();
	...
	Форма.СтруктураРеквизитовФормы.Вставить("ЦветЗаполняетсяАвтоАрифметика", Цвета.ЦветЗаполняетсяАвтоАрифметика);
КонецПроцедуры

Процедура Автозаполнение_ОбученныеПоОхранеТрудаЛица(ПереданныеПараметры, НеиспользуемыйАдресХранилища) Экспорт
	...
	Для Инд = 0 По ДеревоДанныхРеестр.Строки.Количество() - 1 Цикл
		...
		ТекВетка.Данные["ПризнакИзмененияДанных"] = 1;
		ТабличныйДокумент.Область("ПризнакИзмененияДанных").Значение = ТекВетка.Данные["ПризнакИзмененияДанных"];
		...
		Для Инд_П10000 = 0 По Дерево_П10000.Строки.Количество() - 1 Цикл
			...
			НумераторСквозной = НумераторСквозной + 1;
			ТекВетка_П10000.Данные["П1000001"] = Формат(НумераторСквозной, "ЧГ=");
...
Функция СформироватьСтруктуруОбязательныхРеквизитовСтатистическогоОтчета(Организация = Неопределено) Экспорт 
	...
	СтруктураРеквизитовФормы.Вставить("КонтрольныеСоотношенияДополнятьРасшифровкой", Ложь);
...
Функция ЭтоСлужебнаяОбласть(Имя) Экспорт
	...
	Возврат Имя = "ПолеНомерКорректировки" Или Имя = "ШапкаОбособленныеПодразделения"
		Или Имя = "ШапкаРегистрацияРеспондента" Или Имя = "ДанныеПредыдущегоПериода"
...
Процедура УдалитьНомерКорректировки(ТитульнаяСтраница) Экспорт 
	...
	Если НК <> Неопределено Тогда
		ТитульнаяСтраница.УдалитьОбласть(НК, ТипСмещенияТабличногоДокумента.ПоВертикали);
	КонецЕсли;
	НК = ТитульнаяСтраница.Области.Найти("ШапкаРегистрацияРеспондента");
	Если НК <> Неопределено Тогда
		ТитульнаяСтраница.УдалитьОбласть(НК, ТипСмещенияТабличногоДокумента.ПоВертикали);
	КонецЕсли;
...
Процедура ЗагрузитьСправочникиШаблонаВРегистр(ИмяФормы, ИсточникОтчета, МакетШаблона) Экспорт
	...
	НачатьТранзакцию();
	ЗаписиРегистра = РегистрыСведений.СправочникиШаблоновОтчетовСтатистики.СоздатьНаборЗаписей();
...
Функция ПолучитьКодОрганаФСГСРегистрацииРеспондента(РегистрацияРеспондента)
	КодОрганаФСГС = "";
	Если ИспользуютсяРегистрацииВОрганеСтатистики() Тогда
		...
		КодОрганаФСГС = ?(НЕ ЗначениеЗаполнено(РегистрацияРеспондента), "",
			СокрЛП(ОбщийМодульРегистрацииВОрганеСтатистикиВызовСервера.СвойстваРегистрацииВОрганеСтатистики(РегистрацияРеспондента, "КодОрганаФСГС").КодОрганаФСГС));
	КонецЕсли;
...
Функция ПолучитьКодОрганаФСГССтатотчета(СсылкаНаОтчет) Экспорт
	...
	ДанныеОтчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаОтчет, "ДанныеОтчета").Получить();
	
	РегистрацияРеспондента = Неопределено;
	ДанныеОтчета.Свойство("РегистрацияРеспондента", РегистрацияРеспондента);
	Если ЗначениеЗаполнено(РегистрацияРеспондента) Тогда
		КодФСГС = ПолучитьКодОрганаФСГСРегистрацииРеспондента(РегистрацияРеспондента);
	Иначе
		...
		Если Не ЗначениеЗаполнено(СокрЛП(КодФСГС)) Тогда
			КодФСГС = ПолучитьКодОрганаФСГСПоОрганизации(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаОтчет, "Организация"));
		КонецЕсли;
	КонецЕсли;
...
//
Функция ВидКонтролирующегоОргана(Знач ИмяОтчета) Экспорт
	Если СтрНачинаетсяС(ИмяОтчета, "EF_") И ТипЗнч(Метаданные.Отчеты.Найти(ИмяОтчета)) <> Тип("ОбъектМетаданных") Тогда
		ИмяОтчета = Сред(ИмяОтчета, 4);
	КонецЕсли;
	...
	Иначе
		Попытка
			// см. описание в ОписаниеВыпускаНовогоОтчетаВВидеПатча
			Если СтрНачинаетсяС(ИмяОтчета, "EF_") И ТипЗнч(Метаданные.Отчеты.Найти(ИмяОтчета)) = Тип("ОбъектМетаданных") Тогда 
				Возврат Отчеты[ИмяОтчета].ТипКонтролирующегоОргана();
			КонецЕсли;
		Исключение
		КонецПопытки;
		...
		ИначеЕсли СтрНайти(ИмяОтчета, "ОкрСреда") > 0
				...
				ИЛИ СтрНайти(ИмяОтчета, "ЭкологическийКонтроль") > 0 
		...
		ИначеЕсли СтрНайти(ИмяОтчета, "ИсполнениеКонтрактовГОЗ") > 0 
			Или СтрНайти(ИмяОтчета, "КассовыйПланГОЗ") > 0 
			Или СтрНайти(ИмяОтчета, "ЦенаПродукцииГОЗ") > 0  Тогда
...
Функция ОпределитьСвойстваОбъектовДляОтображенияВФормеОтчетность(Ссылки) Экспорт
	...
	ТаблицаСвойств.Колонки.Добавить("ВидКонтролирующегоОргана", Новый ОписаниеТипов("ПеречислениеСсылка.ТипыКонтролирующихОрганов"));
...
Процедура ОпределениеСвойстваОбъектаДляОтображенииВФормеОтчетность(Ссылка, ЗначенияЗаполнения)
	...
	Если СвойстваСсылки <> Неопределено Тогда
		...
		КонецЕсли;
		Если ЗначенияЗаполнения.Свойство("ВидКонтролирующегоОргана")
			И НЕ ЗначениеЗаполнено(ЗначенияЗаполнения.ВидКонтролирующегоОргана) Тогда
			ЗначенияЗаполнения.ВидКонтролирующегоОргана = СвойстваСсылки.ВидКонтролирующегоОргана;
		КонецЕсли;
...
Процедура ЗаписьОбъектовРегламентированнойОтчетности(Источник, Отказ) Экспорт
	...
	ИначеЕсли ДокументооборотСКОКлиентСервер.ПодсистемаЦПРРМЧДСуществует()
		...
		ЭлектронныйДокументооборотСКонтролирующимиОрганами.ЗаписатьМашиночитаемыеДоверенностиВРегистрЖурналОтправокВКонтролирующиеОрганы(Ссылка);
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ВходящийДокументСЭДОФСС") Тогда
		ЭлектронныйДокументооборотСКонтролирующимиОрганами.ЗаписатьВходящийДокументСЭДОФСС(Ссылка);
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЗаявлениеНаФормированиеСправкиОРасчетах") Тогда
		ЭлектронныйДокументооборотСКонтролирующимиОрганами.ЗаписатьЗаявлениеНаФормированиеСправкиОРасчетах(Ссылка);
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ОтветСтрахователяНаТребованиеПроверкиФСС") Тогда
		ЭлектронныйДокументооборотСКонтролирующимиОрганами.ЗаписатьОтветСтрахователяНаТребованиеПроверкиФСС(Ссылка);
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ОбращениеСФРКСтрахователю") Тогда
		ЭлектронныйДокументооборотСКонтролирующимиОрганами.ЗаписатьОбращениеСФРКСтрахователю(Ссылка);
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ОтветСтрахователяНаОбращениеСФР") Тогда
		ЭлектронныйДокументооборотСКонтролирующимиОрганами.ЗаписатьОтветСтрахователяНаОбращениеСФР(Ссылка);
	Иначе
...
Процедура РегламентированныйОтчетОтправляетсяВКонтролирующиеОрганы(ДокСсылка, ОбъектОтправляется) Экспорт
	...
	Если РеглОтчетыНеОтправляютсяВКонтролирующиеОрганы.Найти(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокСсылка, "ИсточникОтчета")) <> Неопределено Тогда
...
Функция КоличествоОбъектовНеЗаписанныхВРегистр(ПараметрыПроцедуры) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания =
		НСтр("ru = 'Проверка полноты данных журнала регламентированной отчетности';
			|en = 'Check completeness of local reporting log data'");
...
Функция ДобавлениеОбъектовОтсутствующихВРегистре(ПараметрыПроцедуры) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Заполнение данных журнала регламентированной отчетности';
															|en = 'Fill in data of local reporting log'");
	ПараметрыВыполнения.ОжидатьЗавершение = 0;
	ПараметрыВыполнения.ЗапуститьВФоне = Истина;
...
Процедура ОбработатьСобытие1СОтчетности(ИмяСобытия, СсылкаНаИсточник = Неопределено, ДопПараметрСтруктура = Неопределено) Экспорт
	...
	Если ИмяСобытия = НСтр("ru = 'Регламентированный отчет. Создание';
							|en = 'Local report. Creation'") Тогда
		
		СтатистикаБРОКлиентСервер.ДобавитьСобытие(ИмяСобытия + ". " + КомментарийКСозданиюРеглОтчета(СсылкаНаИсточник));
		
	ИначеЕсли ИмяСобытия = НСтр("ru = 'Регламентированный отчет. Выгрузка';
								|en = 'Local report. Export'") Тогда
		
		СтатистикаБРОКлиентСервер.ДобавитьСобытие(ИмяСобытия + ". " + КомментарийКВыгрузкеРеглОтчета(СсылкаНаИсточник));
		
	ИначеЕсли ИмяСобытия = НСтр("ru = 'Регламентированный отчет. Проверка выгрузки';
								|en = 'Local report. Export check'") Тогда
...
Функция ОтчетыОценкиРисков(Организация, ДатаНачала, ДатаОкончания)
	...
	ВидыОтчетов = РегламентированнаяОтчетностьПовтИсп.ВидыОтчетовОценкиРисков();
...
Процедура ВставитьПоказательВТаблицуОценкиРисков(ТаблицаПоказателей, ОписаниеОтчета)
	Если РегламентированнаяОтчетностьПовтИсп.ВидыОтчетовОценкиРисков().Найти(ОписаниеОтчета.ИсточникОтчета) <> Неопределено Тогда 
		Отчеты[ОписаниеОтчета.ИсточникОтчета].ПоказателиОценкиРискаВыезднойНалоговойПроверки(ТаблицаПоказателей, ОписаниеОтчета);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьДанныеВШапкеОбособленныхПодразделений(Форма, ИмяТД = "ПолеТабличногоДокументаФормаОтчета") Экспорт
	...
	Если ТабДок.Области.Найти("РегистрацияРеспондента") <> Неопределено Тогда
		ТабДок.Области.РегистрацияРеспондента.Значение = Форма.РегистрацияРеспондента;
		ОбщийМодульРегистрацииВОрганеСтатистикиКлиентСервер = ОбщегоНазначения.ОбщийМодуль("РегистрацииВОрганеСтатистикиКлиентСервер");
		ОбщийМодульРегистрацииВОрганеСтатистикиКлиентСервер.ОбновитьОбластьИсключитьТОСП(Форма, ИмяТД);
	КонецЕсли;
	
	Если ТабДок.Области.Найти("ОбособленноеПодразделение") <> Неопределено Тогда
		Цвета = КоллекцияЦветовРаскраски();
		...
		Если ИсключитьДанныеОбособленныхПодразделений Тогда
			...
			ОблОбособленноеПодразделение.ЦветФона = Цвета.ЦветЗаполняетсяТолькоВручную;
		Иначе
			ОблОбособленноеПодразделение.Значение = Форма.ОбособленноеПодразделение;
			ОблОбособленноеПодразделение.Защита = Ложь;
			ОблОбособленноеПодразделение.ЦветФона = Цвета.ЦветЗаполняетсяОднозначноВручную;
		КонецЕсли;
	КонецЕсли;
...
// Обработчик проверки возможности выгрузки в фоновом задании.
// 
// Параметры:
//   ОтчетыСФоновойПроверкойВыгрузки - Массив Из Структура:
//     * МетодПроверки - Строка
//     * РегламентированныйОтчет - ДокументСсылка.РегламентированныйОтчет
//   ДополнительныеПараметры - Структура - Используется для проброса значений через фоновое задание
// 
// Возвращаемое значение:
//   Структура
//
Функция ПроверитьВозможностьВыгрузкиВФоне(ОтчетыСФоновойПроверкойВыгрузки, ДополнительныеПараметры) Экспорт
	
	ОшибкаВыгрузки = Ложь;
	ОписаниеОшибок = Новый ТаблицаЗначений;
	
	Для Каждого ОписаниеМетода Из ОтчетыСФоновойПРоверкойВыгрузки Цикл
		Параметры = Новый Структура("РегламентированныйОтчет", ОписаниеМетода.РегламентированныйОтчет);
		РезультатПроверки = ОбщегоНазначения.ВычислитьВБезопасномРежиме(ОписаниеМетода.МетодПроверки
			+ "(Параметры.РегламентированныйОтчет)", Параметры);
			
		ОшибкаВыгрузки = ОшибкаВыгрузки ИЛИ НЕ РезультатПроверки.ВыгрузкаВозможна;
		
		ОшибкиОтчета = РезультатПроверки.ОписаниеОшибок;
		
		// Колонки для группировки ошибок по отчетам.
		ОшибкиОтчета.Колонки.Добавить("ОтчетДок", Новый ОписаниеТипов("ДокументСсылка.РегламентированныйОтчет"));
		ОшибкиОтчета.ЗаполнитьЗначения(ОписаниеМетода.РегламентированныйОтчет, "ОтчетДок");
		ОшибкиОтчета.Колонки.Добавить("Отчет", Новый ОписаниеТипов("Строка"));
		ОшибкиОтчета.ЗаполнитьЗначения(РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(ОписаниеМетода.РегламентированныйОтчет), "Отчет");
		
		Если ОписаниеОшибок.Колонки.Найти("ОтчетДок") = Неопределено Тогда
			ОписаниеОшибок = ОшибкиОтчета.Скопировать();
		Иначе
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ОшибкиОтчета, ОписаниеОшибок);
		КонецЕсли;
		
	КонецЦикла;
	
	ВозвращаемоеЗначение = ОбщегоНазначения.СкопироватьРекурсивно(ДополнительныеПараметры);
	
	ВозвращаемоеЗначение.Вставить("ОшибкаВыгрузки", ОшибкаВыгрузки);
	ВозвращаемоеЗначение.Вставить("ОписаниеОшибок", ОписаниеОшибок);
...
Функция ПолучитьОрганизациюПоУмолчанию() Экспорт
	...
	РегламентированнаяОтчетностьПереопределяемый.УстановитьОрганизациюПоУмолчанию(Организация);
...
Функция РеглОтчеты(Наим, НеЗапоминатьВКэше = Ложь, ВыводитьСообщение = Истина) Экспорт
	...
	Если НЕ ПризнакИспользованияВнешнегоОтчетаСУчетомКэша(НайденныйЭлемент, НеЗапоминатьВКэше) ИЛИ НайденныйЭлемент.ВнешнийОтчетХранилище = Неопределено Тогда // используется внутренний отчет
		...
		Иначе
			Если ВыводитьСообщение Тогда
				ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найден отчет %1!';
																													|en = 'The %1 report is not found.'"), СокрП(Наим)));
			КонецЕсли;
	...
	Иначе // используется внешний отчет
		...
		Исключение
			Если ВыводитьСообщение Тогда
				ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось запустить внешний отчет %1!';
																													|en = 'Cannot run the external %1 report.'"), СокрП(Наим)));
			КонецЕсли;
...
Функция ФормаРеглОтчета(Наим, ВыбраннаяФорма = Неопределено, Владелец = Неопределено, КлючУникальности = Неопределено) Экспорт
	...
	Если СтрНачинаетсяС(Наим, "EF_") И ТипЗнч(Метаданные.Отчеты.Найти(Наим)) = Тип("ОбъектМетаданных") Тогда
		Возврат Отчеты[Наим];
	КонецЕсли;
...
Функция ЗаблокироватьДокумент(мСохраненныйДок) Экспорт
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Не удается заблокировать документ!%1Возможно, документ уже открыт другим пользователем!';
			|en = 'Cannot lock the document.%1Maybe, the document is already opened by another user.'"), Символы.ПС));
		Возврат Ложь;
	КонецПопытки;
КонецФункции
	...
	Если ДатамВерсияФормы < ДатаВерсияФормы Тогда
		ОбщегоНазначения.СообщитьПользователю(
			...
			НСтр("ru = 'Сохраненные данные не могут быть открыты текущей версией отчета!%1Версия отчета - от %2Версия данных - от %3';
				|en = 'Saved data cannot be opened by the current report version.%1 Report version from %2 Data version from %3'"),
...
Функция СуществуетДокументСАналогичнымиРеквизитами(Форма, КодИФНС, КПП = "") Экспорт	
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("мСохраненныйДок", 
		?(Форма.СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено, Неопределено, Форма.СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка));
...
Процедура ОперацииПриВосстановленииРегламентированногоОтчета(Форма) Экспорт
	...
	Если ИспользуютсяРегистрацииВОрганеСтатистики()
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "РегистрацияРеспондента") Тогда
		СписокСохранения = Форма.СтруктураРеквизитовФормы.мСохраненныйДок.ДанныеОтчета.Получить();
		СписокСохранения.Свойство("РегистрацияРеспондента", Форма.РегистрацияРеспондента);
	КонецЕсли;
...
Функция ВывестиСведенияСлужебнойЧасти30(Текст, Организация = Неопределено, Знач ДатаДок = Неопределено, Знач ОргИНН = Неопределено, Знач ОргКПП = Неопределено, Знач КодИнсп = Неопределено, ТипИнф = "ОТЧЕТНОСТЬНП", ВерсияФормата = "3.00") Экспорт
	...
	Если ПустаяСтрока(ОргИНН) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Невозможно определить ИНН организации! Выгрузка прервана!';
													|en = 'Cannot determine company TIN. Export is canceled.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(КодИНСП) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Невозможно определить код налоговой инспекции! Выгрузка прервана!';
													|en = 'Cannot determine tax inspection code. Export is canceled.'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция ВывестиСведенияОбОтправителе30(Текст, Организация = Неопределено, Знач ОргНазв = Неопределено, Знач ОргТелефон = Неопределено, Знач ОргБухгалтер = Неопределено, ВыгрузкаДекларацииПоАлкоголю = Ложь) Экспорт
	...
	Если ПустаяСтрока(ИНН) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Невозможно определить вид налогоплательщика! Выгрузка прервана!';
													|en = 'Cannot determine a taxpayer kind. Export is canceled.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(ОргНазв) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Невозможно определить полное название организации! Выгрузка прервана!';
													|en = 'Cannot determine the full company name. Export is canceled.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если КатегорияНП = 1 И ПустаяСтрока(ОргБухгалтер) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Невозможно определить ФИО главного бухгалтера организации! Выгрузка прервана!';
													|en = 'Cannot define Chief Accountant full name. Export is canceled.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Иначе
		...
		Если ПустаяСтрока(ФИООтпрФЛ) Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Невозможно определить ФИО индивидуального предпринимателя! Выгрузка прервана!';
														|en = 'Cannot determine the name of an individual entrepreneur! Export is canceled!'"));
			Возврат Ложь;
		КонецЕсли;
		...
		Если БылаЦифра Тогда
			...
			Исключение
				ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибки преобразования номера из окончания: %1 === %2';
						|en = 'Errors occurred when converting numbers from the end: %1 === %2'"), Корень, Номер));
			КонецПопытки;
			Стр.Имя = Корень;
		КонецЕсли;
...
Процедура СообщитьВТаблицуСообщений(Владелец, Описание, Раздел, ИмяЯчейки = "", ФИО = "") Экспорт
	...
	НовСтр.Описание  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																					|en = '%1'"), Описание);
	НовСтр.Страница  = ФИО;
...
Функция НастройкиВидимостиЭлементовФормы(Форма, НастройкиВидимости = Неопределено) Экспорт
	...
	РегламентированнаяОтчетностьПереопределяемый.ПолучитьНастройкиВидимостиЭлементовФормы(Разложение[1], Разложение[3], НастройкиВидимости);
...
Процедура ПолучитьСведенияОПоказателяхУведомления(Форма, ПараметрыОтчета = Неопределено) Экспорт
	...
	Разложение = СтрРазделить(Форма.ИмяФормы, ".");
	АвтозаполняемыеПоказателиОтчета = РегламентированнаяОтчетностьПереопределяемый.ПолучитьСведенияОПоказателяхОтчета(
...
Процедура ПереносДанныхЭДОПриОбновлении10158() Экспорт
	...
	ТекстСообщения = НСтр("ru = 'Обновление данных информационной базы по ЭДО с КО';
							|en = 'Updating infobase data according to EDI with RA'");
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Исключение
            ТекстСообщения = НСтр("ru = 'Электронный документооборот с контролирующими органами';
									|en = 'Electronic data interchange with regulatory authorities'");
...
Процедура ЗаполнитьСтатусыОтправки10158(СоответствиеКонвертированныхОбъектов)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	ТИПЗНАЧЕНИЯ(СтатусыОтправки.Объект) = ТИП(Документ.УдалитьЗапросНаВыпискуИзЕГРЮЛ_ЕГРИП)";
...
Процедура ЗаполнитьЦиклыОбмена10158(СоответствиеКонвертированныхОбъектов)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	(ТИПЗНАЧЕНИЯ(ЦиклыОбмена.Предмет) = ТИП(Документ.УдалитьЗапросНаВыпискуИзЕГРЮЛ_ЕГРИП)
...
//
// Параметры:
//  СсылкаНаСохрРегОтчет    - ДокументСсылка.РегламентированныйОтчет - ссылка на сохраненный регламентированный
//                            отчет, данные которого нужно получить.
//  ДополнительныеПараметры - Структура - дополнительные параметры, если необходимо уточнение получения данных
//                            (необязательный параметр), при этом соответствующий регламентированный отчет
//                            должен поддерживать работу с этим параметром (пример см. в модуле объекта отчета
//                            "РегламентированныйОтчет6_НДФЛ", функция "ДанныеРегОтчетаСУчетомКонвертации").
//
Функция ПолучитьДанныеСохрРегОтчета(СсылкаНаСохрРегОтчет, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		И ДополнительныеПараметры.Свойство("ДанныеРегОтчетаСУчетомКонвертации")
		И ДополнительныеПараметры["ДанныеРегОтчетаСУчетомКонвертации"] = Истина
		И СокрЛП(СсылкаНаСохрРегОтчет.ИсточникОтчета) = "РегламентированныйОтчет6_НДФЛ" Тогда
		
		ИсточникОтчета = СокрЛП(СсылкаНаСохрРегОтчет.ИсточникОтчета);
		ИмяФормыОтчета = СокрЛП(СсылкаНаСохрРегОтчет.ВыбраннаяФорма);
		...
		Возврат ОтчетОбъект.ДанныеРегОтчетаСУчетомКонвертации(СсылкаНаСохрРегОтчет);
...
Процедура СохранитьДанныеРасшифровкиСтатистическогоОтчета(Форма, СписокСохранения) Экспорт
	Если Форма.СтруктураРеквизитовФормы.АдресВременногоХранилищаРасшифровки <> Неопределено 
	   И Форма.СтруктураРеквизитовФормы.мСохранятьРасшифровку = Истина Тогда
			...
			Если СтрДлина(ОбщегоНазначения.ЗначениеВСтрокуXML(ТаблицаРасшифровки)) < (9 * Pow(10,8)) Тогда
...
Процедура СохранитьДанныеРасшифровкиСтатистическогоОтчетаВФайл(Форма, СписокСохранения) Экспорт
	
	Если Форма.СтруктураРеквизитовФормы.АдресВременногоХранилищаРасшифровки <> Неопределено 
	   И Форма.СтруктураРеквизитовФормы.мСохранятьРасшифровку = Истина Тогда
		
		ТаблицаРасшифровки = ПолучитьИзВременногоХранилища(Форма.СтруктураРеквизитовФормы.АдресВременногоХранилищаРасшифровки);
		СохраненныйДокумент = Форма.СтруктураРеквизитовФормы.мСохраненныйДок;
		
		Если РегламентированнаяОтчетностьВызовСервера.ВозможноПрисоединениеФайловКОбъекту(СохраненныйДокумент) Тогда
			
			ИмяСправочника = РаботаСФайламиСлужебный.ИмяСправочникаХраненияФайлов(СохраненныйДокумент);
			ТаблицаРасшифровкиСтрокаXML = ОбщегоНазначения.ЗначениеВСтрокуXML(ТаблицаРасшифровки);
			...
			ТекстЗапроса =
				...
				|ГДЕ
				|	Файлы.Наименование = ""РасшифровкаОтчета""
				|	И Файлы.Служебный = ИСТИНА
			...
			Запрос.УстановитьПараметр("Владелец", СохраненныйДокумент);
			
			Выборка = Запрос.Выполнить().Выбрать();
			
			Если СписокСохранения.Свойство("Расшифровка") Тогда
			...
			Иначе
				...
				Текст = Новый ЗаписьТекста(ВременныйФайлТекст);
				Текст.ЗаписатьСтроку(ТаблицаРасшифровкиСтрокаXML);
				Текст.Закрыть();
				
				ВременныйФайлАрхив = ПолучитьИмяВременногоФайла("zip");
				Архив = Новый ЗаписьZipФайла(ВременныйФайлАрхив);
				Архив.Добавить(ВременныйФайлТекст);
				Архив.Записать();
				
				ДвоичныеДанные = Новый ДвоичныеДанные(ВременныйФайлАрхив);
				...
				Иначе
					...
					ПараметрыФайла.ВладелецФайлов = СохраненныйДокумент;
					ПараметрыФайла.ИмяБезРасширения = "РасшифровкаОтчета";
	...
	Если ТаблицаРасшифровки = Неопределено Тогда
		...
		Если РегламентированнаяОтчетностьВызовСервера.ВозможноПрисоединениеФайловКОбъекту(СохраненныйДокумент) Тогда
			...
			Если Выборка.Следующий() Тогда
				...
				Результат = РаботаСФайлами.ДанныеФайла(Выборка.Ссылка);
				ДанныеФайла = ПолучитьИзВременногоХранилища(Результат.СсылкаНаДвоичныеДанныеФайла);
				
				ВФАрх = ПолучитьИмяВременногоФайла("zip");
				ДанныеФайла.Записать(ВФАрх);
				...
				ЧтениеТекст = Новый ЧтениеТекста(ИтоговыйФайл);
				Стр = ЧтениеТекст.Прочитать();
				ЧтениеТекст.Закрыть();
				
				ТаблицаРасшифровки = ОбщегоНазначения.ЗначениеИзСтрокиXML(Стр);
...
Функция СформироватьСтруктуруОбязательныхРеквизитовУведомления() Экспорт
	...
	СтруктураРеквизитовФормы.Вставить("ОтображатьКнопкуЗаполнить", Ложь);
	СтруктураРеквизитовФормы.Вставить("ОтображатьКнопкуРасшифровать", Ложь);
	СтруктураРеквизитовФормы.Вставить("ЦветАвтоЗаполнение", ЦветаСтиля.ЦветМакетаБРОАвтозаполнение);
	СтруктураРеквизитовФормы.Вставить("ЦветАвтоРасчет", ЦветаСтиля.ЦветМакетаБРОАвторасчет);
	СтруктураРеквизитовФормы.Вставить("ЦветВручную", ЦветаСтиля.ЦветМакетаБРОРучноеЗаполнение);
...
Функция ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении(ОбъектРегОтчет, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если СокрЛП(ОбъектРегОтчет.НаименованиеОтчета) = "Доходы, выплаченные иностранным организациям" Тогда
		...
		Если ВРег(СокрЛП(ОбъектРегОтчет.ВыбраннаяФорма)) = ВРег("ФормаОтчета2023Кв4")
			ИЛИ ВРег(СокрЛП(ОбъектРегОтчет.ВыбраннаяФорма)) = ВРег("ФормаОтчета2020Кв1")
			ИЛИ ВРег(СокрЛП(ОбъектРегОтчет.ВыбраннаяФорма)) = ВРег("ФормаОтчета2016Кв1") Тогда
			...
			Описание = НСтр("ru = 'Отчет сохранен в конфигурации предыдущей версии с неуникальными значениями показателя ""Уникальный номер"" в Разделе 3.1 и(или) неуникальными сочетаниями значений показателей ""Уникальный номер"" и ""Порядковый номер"" в Разделе 3.2.';
							|en = 'The report is saved in configuration of the previous version with non-unique values of the ""Unique number"" indicator in Section 3.1 and (or) non-unique value combinations of the ""Unique number"" and ""Sequence number"" indicators in Section 3.2. '");
			...
			Описание = НСтр("ru = 'В отчете, сохраненном в конфигурации предыдущей версии, не соблюдена последовательность в нумерации значений показателя ""Уникальный номер"" в Разделе 3.1 и(или) сочетаний значений показателей ""Уникальный номер"" и ""Порядковый номер"" в Разделе 3.2 и(или) Разделе 3.3.';
							|en = 'In the report saved in the previous version configuration, the sequence in numbering of the ""Unique number"" indicator value is not observed in Section 3.1 and(or) combinations of ""Unique number"" and ""Sequence number"" indicator values in Section 3.2 and(or) Section 3.3. '");
			...
			Если ВРег(СокрЛП(ОбъектРегОтчет.ВыбраннаяФорма)) = ВРег("ФормаОтчета2023Кв4") Тогда
				
				Описание = НСтр("ru = 'Отчет сохранен в конфигурации предыдущей версии с неуникальными значениями показателя ""Уникальный номер"" в Разделе 4.1 и(или) неуникальными сочетаниями значений показателей ""Уникальный номер"" и ""Порядковый номер"" в Разделе 4.2.';
								|en = 'The report is saved in the configuration of the previous version with non-unique values of the ""Unique number"" indicator in Section 4.1 and (or) non-unique value combinations of the ""Unique number"" and ""Sequence number"" indicators in Section 4.2.'");
				ДопПараметры = Новый Структура("ТекстСообщения", Описание);
				// Подразделы 4.1.
				...
				КонецЦикла;
				ТЗПров41_ = ТЗПров41.Скопировать();
				ТЗПров41_.Свернуть("УникНом");
				Если ТЗПров41.Количество() <> ТЗПров41_.Количество() Тогда
					// Не пройдена проверка на неуникальность значений показателя
					// "Уникальный номер" в разделе 4.1.
					ДополнительныеПараметры = ДопПараметры;
					Возврат Ложь;
				КонецЕсли;
				// Подразделы 4.2.
				...
				КонецЦикла;
				ТЗПров42_ = ТЗПров42.Скопировать();
				ТЗПров42_.Свернуть("УникНом,ПорядНом");
				Если ТЗПров42.Количество() <> ТЗПров42_.Количество() Тогда
					// Не пройдена проверка на неуникальность сочетаний значений показателей
					// "Уникальный номер" и "Порядковый номер дохода" в разделе 4.2.
					ДополнительныеПараметры = ДопПараметры;
					Возврат Ложь;
				КонецЕсли;
				
				Описание = НСтр("ru = 'В отчете, сохраненном в конфигурации предыдущей версии, не соблюдена последовательность в нумерации значений показателя ""Уникальный номер"" в Разделе 4.1 и(или) сочетаний значений показателей ""Уникальный номер"" и ""Порядковый номер"" в Разделе 4.2.';
								|en = 'In the report saved in the previous version configuration, the sequence in numbering of the ""Unique number"" indicator value is not observed in Section 4.1 and (or) combinations of ""Unique number"" and ""Sequence number"" indicator values in Section 4.2.'");
				ДопПараметры.Вставить("ТекстСообщения", Описание);
				// Подразделы 4.1.
				ТЗПров41.Сортировать("УникНом");
				Для Инд = 0 По ТЗПров41.Количество() - 1 Цикл
					Если Инд = 0 Тогда
						Если ТЗПров41[Инд].УникНом > 1 Тогда
							// Не пройдена проверка на первый номер среди
							// значений "Уникальный номер" в разделе 4.1.
							// (Нумерация начинается со значения больше 1.)
							ДополнительныеПараметры = ДопПараметры;
							Возврат Ложь;
						КонецЕсли;
					Иначе
						Если ТЗПров41[Инд].УникНом - ТЗПров41[Инд - 1].УникНом > 1 Тогда
							// Не пройдена проверка на последовательность в нумерации
							// среди значений "Уникальный номер" в разделе 4.1.
							// Например, после номера 3 идет номер 5, а должен 4.
							ДополнительныеПараметры = ДопПараметры;
							Возврат Ложь;
						КонецЕсли;
				...
				КонецЦикла;
				// Подразделы 4.2.
				ТЗПров42_.Свернуть("УникНом");
				ТЗПров42_.Сортировать("УникНом");
				Для Инд = 0 По ТЗПров42_.Количество() - 1 Цикл
					Если Инд = 0 Тогда
						Если ТЗПров42_[Инд].УникНом > 1 Тогда
							// Не пройдена проверка на первый номер среди
							// значений "Уникальный номер" в разделе 4.2.
							// (Нумерация начинается со значения больше 1.)
							ДополнительныеПараметры = ДопПараметры;
							Возврат Ложь;
						КонецЕсли;
					Иначе
						Если ТЗПров42_[Инд].УникНом - ТЗПров42_[Инд - 1].УникНом > 1 Тогда
							// Не пройдена проверка на последовательность в нумерации
							// среди значений "Уникальный номер" в разделе 4.2.
							// Например, после номера 3 идет номер 5, а должен 4.
							ДополнительныеПараметры = ДопПараметры;
							Возврат Ложь;
						КонецЕсли;
				...
				ТЗПров42_ = ТЗПров42.Скопировать();
				ТЗПров42_.Свернуть("УникНом,ПорядНом");
				ТЗПров42_.Сортировать("УникНом,ПорядНом");
				ПредУникНом = Неопределено;
				Для Инд = 0 По ТЗПров42_.Количество() - 1 Цикл
					ТекУникНом = ТЗПров42_[Инд].УникНом;
					Если ТекУникНом <> ПредУникНом Тогда
						Если ТЗПров42_[Инд].ПорядНом > 1 Тогда
							// Не пройдена проверка на первый номер среди
							// значений "Порядковый номер" в разделе 4.2.
							// (Нумерация начинается со значения больше 1.)
							ДополнительныеПараметры = ДопПараметры;
							Возврат Ложь;
						КонецЕсли;
					Иначе
						Если ТЗПров42_[Инд].ПорядНом - ТЗПров42_[Инд - 1].ПорядНом > 1 Тогда
							// Не пройдена проверка на последовательность в нумерации
							// среди значений "Порядковый номер" в разделе 4.2.
							// Например, после номера 3 идет номер 5, а должен 4.
							ДополнительныеПараметры = ДопПараметры;
							Возврат Ложь;
						КонецЕсли;
		...
		ЗаписьЖурналаРегистрации(
			?(ВернутьРазмерТарифа,
				НСтр("ru = 'Получение размера тарифа СВ в процентах по коду категории';
					|en = 'Receive IC tariff amount in percent by category code'"),
				НСтр("ru = 'Получение кода тарифа СВ по коду категории';
					|en = 'Receive IC tariff code by category code'")),
...
Процедура ДобавитьНадписьВыгрузкаНедоступна(Форма)
	
	НовыйЭлемент = Форма.Элементы.Добавить("НадписьВыгрузкаНедоступна", Тип("ДекорацияФормы"));
	НовыйЭлемент.Вид = ВидДекорацииФормы.Надпись;
	НовыйЭлемент.Заголовок = "Возможность выгрузки, отправки, проверки контрольных соотношений будет доступна после публикации Росстатом шаблона электронного представления";
	НовыйЭлемент.ЦветТекста = Новый Цвет(255, 0, 0);
	НовыйЭлемент.АвтоМаксимальнаяШирина = Ложь;
	
	МестоРасположения = ?(Форма.Элементы.Найти("ГруппаПанельОтправки") <> Неопределено,
						  Форма.Элементы.Найти("ГруппаПанельОтправки"),
						  Форма.Элементы.Найти("Отчет"));
	
	Если МестоРасположения = Неопределено Тогда
		Форма.Элементы.Переместить(НовыйЭлемент, Форма);
	Иначе
		Форма.Элементы.Переместить(НовыйЭлемент, Форма, МестоРасположения);
	КонецЕсли;
...
Процедура БлокировкаКнопокПриОтстутствииФорматаРосстата(Форма) Экспорт
	...
	Если ВерсияФорматаВыгрузки <> Перечисления.ВерсииФорматовВыгрузки.ВерсияФСГС Тогда 
		...
		ДобавитьНадписьВыгрузкаНедоступна(Форма);
	КонецЕсли;
КонецПроцедуры
...
Функция АдресВJSON(КонтактнаяИнформация, ВидАдреса) Экспорт
...
// Конструктор дерева форм и форматов регламентированного отчета, используемого в механизмах онлайн-блокировки.
//
// Возвращаемое значение:
//  ДеревоЗначений:
//    * Код - Строка
...
Функция ПолучитьТекущуюТаблицуФорматов() Экспорт 
	Попытка
		...
		Таблица = ШаблоныЭВФОтчетовСтатистики.СписокШаблоновСервисаВебСбора(СвойстваМодуля, Истина, Ложь, Истина);
		Возврат Таблица;
	Исключение
		Возврат Неопределено;
	КонецПопытки;
...
Процедура ОбновитьШаблоныЧерезВебСервисНаСервере(Контекст, ИмяМакета) Экспорт
	Попытка
		...
		ПараметрыЗапуска.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение актуальной версии макета через веб-сервис Росстата';
															|en = 'Receive the relevant template version via Russian Federal State Statistics web service'");
...
Процедура ОбновитьШаблоныСпискомЧерезВебСервисНаСервереВФоне(ПараметрыКоманды, АдресРезультата = Неопределено) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Получение списка XML-шаблонов через веб-сервис Росстата';
										|en = 'Retrieving a XML templates list through the Rosstat web service'"),
...
Процедура ИзменитьВерсиюФорматаСПроверкойСовместимостиШаблонов(Форма, ИмяМакетаАттрибутов, ИмяМакетаШаблона, ПараметрыВыгрузки) Экспорт
	Попытка
		...
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Версия формата выгрузки была изменена на ';
													|en = 'Export format version was changed to'") + Выборка.Версия);
		Комментарий = НСтр("ru = 'Для отчета ';
							|en = 'For report'") + Форма.ИмяФормы + НСтр("ru = ' версия формата выгрузки была изменена с ';
																		|en = ' export format version was changed from'") + ДопАтрибуты.version + НСтр("ru = ' на ';
																																						|en = ' to'") + Выборка.Версия;
...
Процедура УправлениеДоступностьюРеквизитов(Форма, БлокируемыеЭлементы, ТолькоПросмотр) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПроцессыОбработкиДокументов") Тогда
		...
		Если НЕ Форма.Элементы.Найти("ФормаСохранить") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ФормаСохранить");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("ФормаЗаполнить") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ФормаЗаполнить");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("Проверка") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("Проверка");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("Проверить") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("Проверить");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("ФормаПроверитьВыгрузку") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ФормаПроверитьВыгрузку");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("ПроверитьВИнтернете") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ПроверитьВИнтернете");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("Печать") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("Печать");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("ФормаПечатьБРО") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ФормаПечатьБРО");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("ФормаПоказатьСДвухмернымШтрихкодомPDF417") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ФормаПоказатьСДвухмернымШтрихкодомPDF417");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("ОтправитьВКонтролирующийОрган") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ОтправитьВКонтролирующийОрган");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("ФормаСформироватьXML") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ФормаСформироватьXML");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("ФормаРучнойВвод") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ФормаРучнойВвод");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("ФормаОчисткаОтчета") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ФормаОчисткаОтчета");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("ФормаРазрешитьВыгружатьСОшибками") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ФормаРазрешитьВыгружатьСОшибками");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("ФормаЗагрузитьИзФайлаВФормуУведомление") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ФормаЗагрузитьИзФайлаВФормуУведомление");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("ЗаполнениеИзФайлов") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ЗаполнениеИзФайлов");
		КонецЕсли;
		
		Если НЕ Форма.Элементы.Найти("ПредставлениеУведомления") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ПредставлениеУведомления");
		КонецЕсли;
		...
		Если НЕ Форма.Элементы.Найти("ПредставлениеУведомления") = Неопределено Тогда
			Форма.Элементы["ПредставлениеУведомления"].ЧерноБелыйПросмотр = ТолькоПросмотр;
		КонецЕсли;
...
Процедура ПриЧтенииНаСервере(Форма, Объект)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПроцессыОбработкиДокументов") Тогда
		...
		Если ТипЗнч(Объект) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			
			ГруппаДляКнопкиПроцесс = Форма.КоманднаяПанель;
			
		Иначе
			
			ГруппаДляКнопкиПроцесс = Неопределено;
			
		КонецЕсли;
		
		МодульПроцессыОбработкиДокументов = ОбщегоНазначения.ОбщийМодуль("ПроцессыОбработкиДокументов");
	
		МодульПроцессыОбработкиДокументов.ПриЧтенииНаСервере(Форма, Объект, , ГруппаДляКнопкиПроцесс);
...
Процедура ПриСозданииНаСервере(Форма, Объект)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПроцессыОбработкиДокументов") Тогда
		...
		МодульПроцессыОбработкиДокументов.ПриСозданииНаСервере(Форма, Источник, , ГруппаДляКнопкиПроцесс);
...
#Область БлокировкаОткрытыхВДругомСеансеОтчетов

Процедура ЗапретРедактированияОткрытогоОтчета(Форма, МассивНедоступных)
	Форма.СтруктураРеквизитовФормы.Вставить("ЗаблокированоВДругомСеансе", Истина);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗапретРедактированияРеквизитовОбъектов") Тогда
		...
		МодульЗапретРедактированияРеквизитовОбъектов.ЗаблокироватьРеквизиты(Форма,,
			НСтр("ru = '<Не используется>';
				|en = '<Not used>'"), Форма.СтруктураРеквизитовФормы.мСохраненныйДок);
	КонецЕсли;
	
	ПараметрыЗапретаРедактированияРеквизитов = Неопределено;
	РеквизитыФормы = Форма.ПолучитьРеквизиты();
	Для Каждого РеквизитФормы Из РеквизитыФормы Цикл
	...
	Если ПараметрыЗапретаРедактированияРеквизитов = Неопределено Тогда
		БлокируемыеРеквизиты = ОбщегоНазначения.МенеджерОбъектаПоСсылке(Форма.СтруктураРеквизитовФормы.мСохраненныйДок).ПолучитьБлокируемыеРеквизитыОбъекта();
	...
	Для Каждого Элт Из СтрРазделить("Обновить;РазрешитьРедактированиеРеквизитовОбъекта;ОтправитьВКонтролирующийОрган;Выгрузить", ";") Цикл
		Элемент = Форма.Элементы.Найти(Элт);
		Если Элемент <> Неопределено Тогда
			Элемент.Видимость = Ложь; 
		КонецЕсли;
	КонецЦикла;
	
	Если ТипЗнч(МассивНедоступных) = Тип("Массив") Тогда 
		Для Каждого Элт Из МассивНедоступных Цикл 
			Элемент = Форма.Элементы.Найти(Элт);
			Если Элемент <> Неопределено Тогда
				Элемент.Доступность = Ложь; 
			КонецЕсли;
...
// Блокирует отчет если он открыт другим пользователем
// Либо установлена дата запрета изменения
// Форма - форма рег.отчета
// СписокНедоступных - массив из имен элементов, которые надо сделать недоступными, если стандартная обработка их не видит
//                     или строка с разделителями ; (точка с запятой)
Процедура БлокировкаИлиЗапретРедактированияОтчета(Форма, СписокНедоступных = Неопределено) Экспорт 
	Попытка
		ЗаблокироватьДанныеДляРедактирования(Форма.СтруктураРеквизитовФормы.мСохраненныйДок, , Форма.УникальныйИдентификатор);
		
		НеобходимаБлокировка = Ложь;
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДатыЗапретаИзменения")
			И ТипЗнч(Форма.СтруктураРеквизитовФормы.мСохраненныйДок) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			МодульДатыЗапретаИзменения = ОбщегоНазначения.ОбщийМодуль("ДатыЗапретаИзменения");
			РегОтчетОбъект = Форма.СтруктураРеквизитовФормы.мСохраненныйДок.ПолучитьОбъект();
			ОписаниеОшибки = "";
			Если Не РегОтчетОбъект.ЭтоНовый() И МодульДатыЗапретаИзменения.ИзменениеЗапрещено(РегОтчетОбъект, Неопределено, ОписаниеОшибки) Тогда 
				ОписаниеОшибки = "Отчет открыт в режиме ""только просмотр""" + Символы.ПС + ОписаниеОшибки;
				НеобходимаБлокировка = Истина;
			КонецЕсли;
		КонецЕсли;
	Исключение
		НеобходимаБлокировка = Истина;
	...
	Если НеобходимаБлокировка Тогда 
		ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки);
		Если ТипЗнч(СписокНедоступных) = Тип("Массив") Тогда 
			БлокируемыеРеквизиты = СписокНедоступных;
		ИначеЕсли ТипЗнч(СписокНедоступных) = Тип("Строка") Тогда 
			БлокируемыеРеквизиты = СтрРазделить(СписокНедоступных, ";", Ложь);
		Иначе
			БлокируемыеРеквизиты = Неопределено;
		КонецЕсли;
		ЗапретРедактированияОткрытогоОтчета(Форма, БлокируемыеРеквизиты);
	КонецЕсли;
КонецПроцедуры
	...
	Если ЗапретитьРедактированиеПользователем Тогда
		Попытка
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗапретРедактированияРеквизитовОбъектов") Тогда
				МодульЗапретРедактированияРеквизитовОбъектов = ОбщегоНазначения.ОбщийМодуль("ЗапретРедактированияРеквизитовОбъектов");
				МодульЗапретРедактированияРеквизитовОбъектов.ЗаблокироватьРеквизиты(Форма,,
					НСтр("ru = 'Разрешить редактирование отчета';
						|en = 'Allow report editing'"), СохраненныйДок);
			КонецЕсли;
			...
			ТекущееСостояние = ТекущееСостояниеОтправки(СохраненныйДок, КонтролирующийОрган);
			...
			Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СвойстваФормы.СтруктураРеквизитовФормы, "ЗаблокированоВДругомСеансе", Ложь) Тогда
				ЭлементРазрешитьРедактирование = Форма.Элементы.Найти("РазрешитьРедактированиеРеквизитовОбъекта");
				Если ЭлементРазрешитьРедактирование <> Неопределено Тогда
					ЭлементРазрешитьРедактирование.Видимость = Ложь; 
				КонецЕсли;
			КонецЕсли;
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Установка запрета редактирования сданного регламентированного отчета';
					|en = 'Prevent the submitted local accounting report from being edited'"), 
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ЗаполнитьСведенияОПредставителеАвансовогоПлатежаАкциза(
		Форма,
		РегистрацииВНалоговомОргане = Неопределено, 
		КПП = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(РегистрацииВНалоговомОргане) Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Если ЗначениеЗаполнено(ДанныеПредставителя.Представитель) Тогда
				ТипПредставителя = ТипЗнч(ДанныеПредставителя.Представитель);
				Если НЕ РегламентированнаяОтчетность.ПредставительЯвляетсяФизЛицом(ДанныеПредставителя.Представитель) Тогда
					СтруктураРеквизитовФормы.УполномоченноеЛицоПредставителя = СокрЛП(ДанныеПредставителя.УполномоченноеЛицоПредставителя);
					СтруктураРеквизитовФормы.НаименованиеОрганизацииПредставителя = СокрЛП(ДанныеПредставителя.Представитель);
					СтруктураРеквизитовФормы.ИННПредставителя = СокрЛП(ДанныеПредставителя.Представитель.ИНН);
					СтруктураРеквизитовФормы.КПППредставителя = СокрЛП(ДанныеПредставителя.Представитель.КПП);
				Иначе
					СтруктураРеквизитовФормы.УполномоченноеЛицоПредставителя = СокрЛП(ДанныеПредставителя.Представитель);
					СтруктураРеквизитовФормы.ИННПредставителя = СокрЛП(ДанныеПредставителя.Представитель.ИНН);
				КонецЕсли;
				ДокументПредставителя = СокрЛП(ДанныеПредставителя.ДокументПредставителя);
				ТипПодписанта = "2";
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипПодписанта = "1" Тогда
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(СтруктураРеквизитовФормы.Организация, Форма.ДатаПодписи, "ФИОРук, ФИО");
...
Функция ПолучитьПоКодамСведенияОПредставителеПрибыль(Организация, КодНО, КПП = Неопределено) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Если ЗначениеЗаполнено(ДанныеПредставителя.Представитель) Тогда
			...
			ДокументПредставителя = ДанныеПредставителя.ДокументПредставителя;
...
Процедура ВывестиШапкуРегистрацияРеспондента(ТабличноеПоле) Экспорт
	...
	ПустаяСтрока = Шапка.ПолучитьОбласть("R1");
	ПустаяСтрока.Область(1, 1).ШиринаКолонки = 1;
	ПустаяСтрока.Область(1, 2).ШиринаКолонки = 4;
	ПустаяСтрока.Область(1, 3).ШиринаКолонки = 2;
	ПустаяСтрока.Область(1, 4).ШиринаКолонки = 38;
	ПустаяСтрока.Область(1, 5).ШиринаКолонки = 4;
	ПустаяСтрока.Область(1, 6).ШиринаКолонки = 21;
	ПустаяСтрока.Область(1, 7).ШиринаКолонки = 56;
	ПустаяСтрока.Область(1, 8).ШиринаКолонки = 11;
	
	Цвета = КоллекцияЦветовРаскраски();
	
	ОбластьСтроки = Шапка.ПолучитьОбласть("R2");
	
	ОбластьФлаг = ОбластьСтроки.Область(1, 3);
	ОбластьФлаг.СодержитЗначение = Истина;
	ОбластьФлаг.Имя = "ИсключитьДанныеТОСП";
	ОбластьФлаг.ТипЗначения = Новый ОписаниеТипов("Булево");
	ОбластьФлаг.Формат = "БЛ=' '; БИ=V";
	ОбластьФлаг.Значение = ОбластьФлаг.ТипЗначения.ПривестиЗначение();

	ОбластьИсключить = ОбластьСтроки.Область(1, 4);
	ОбластьИсключить.Имя = "ИсключитьДанныеТОСП_Заголовок";	
	ОбластьИсключить.ГоризонтальноеПоложение = ГоризонтальноеПоложение.Право;
	
	ОбластьЗаголовок = ОбластьСтроки.Область(1, 6);
	ОбластьЗаголовок.Текст = НСтр("ru = 'Регистрация респондента';
									|en = 'Register a respondent'");
	
	ОбластьРегистрация = ОбластьСтроки.Область(1, 7);
	ОбластьРегистрация.СодержитЗначение = Истина;
	ОбластьРегистрация.Имя = "РегистрацияРеспондента";
	ОбластьРегистрация.ТипЗначения = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(0));
	ОбластьРегистрация.Значение = ОбластьРегистрация.ТипЗначения.ПривестиЗначение();
	Попытка
		ОбластьРегистрация.УстановитьЭлементУправления(Тип("ПолеФормы"));
	Исключение
		ОбластьРегистрация.УстановитьЭлементУправления(Тип("ПолеВвода"));
	КонецПопытки;
	ОбластьРегистрация.Защита = Ложь;
	ОбластьРегистрация.ЦветФона = Цвета.ЦветЗаполняетсяОднозначноВручную;
	ОбластьРегистрация.ГраницаСнизу  = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
	ОбластьРегистрация.ГраницаСверху = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
	ОбластьРегистрация.ГраницаСправа = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
	ОбластьРегистрация.ГраницаСлева  = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
	
	ОбластьСостав = ОбластьСтроки.Область(1, 8);
	ОбластьСостав.Имя = "СоставРегистрации";
	ОбластьСостав.Текст = НСтр("ru = 'См. состав';
								|en = 'See content'");
	ОбластьСостав.Гиперссылка = Истина;
	ОбластьСостав.Защита = Ложь;
	ОбластьСостав.ЦветТекста = ЦветаСтиля.ЦветГиперссылкиБРО;
	ОбластьСостав.Шрифт = Новый Шрифт(ОбластьСостав.Шрифт,,,,,Истина);
	ОбластьСостав.ГоризонтальноеПоложение = ГоризонтальноеПоложение.Центр;
	
	ОбластьВывода = Шапка.Вывести(ПустаяСтрока);
	ОбластьВывода = Шапка.Вывести(ОбластьСтроки);
	ОбластьВывода = Шапка.Вывести(ПустаяСтрока);
	
	Шапка.Область("R1:R3").Имя = "ШапкаРегистрацияРеспондента";
	ТабличноеПоле.Вывести(Шапка);
	
КонецПроцедуры

Процедура ОбновитьОбластьРегистрацииРеспондента(Форма, ИмяТД = "ПолеТабличногоДокументаФормаОтчета") Экспорт
	
	Если ВидимостьОбластиРегистрацииРеспондента(Форма, ИмяТД) Тогда
		ОбщийМодульРегистрацииВОрганеСтатистикиКлиентСервер = ОбщегоНазначения.ОбщийМодуль("РегистрацииВОрганеСтатистикиКлиентСервер");
		ОбщийМодульРегистрацииВОрганеСтатистикиКлиентСервер.ОбновитьОбластьИсключитьТОСП(Форма, ИмяТД);
	КонецЕсли;
...
Функция ВидимостьОбластиРегистрацииРеспондента(Форма, ИмяТД = "ПолеТабличногоДокументаФормаОтчета") Экспорт
	...
	ТабДок = Форма[ИмяТД];
	ОблВыборРегистрации = ТабДок.Области.Найти("ШапкаРегистрацияРеспондента");
	ОблВыборТОСП = ТабДок.Области.Найти("ШапкаОбособленныеПодразделения");
	Если ОблВыборРегистрации <> Неопределено И ОблВыборТОСП <> Неопределено Тогда
		Если Форма.РегистрацияРеспондента <> Неопределено Тогда
			ТабДок.УдалитьОбласть(ОблВыборТОСП, ТипСмещенияТабличногоДокумента.ПоВертикали);
			Результат = Истина;
		Иначе
			ТабДок.УдалитьОбласть(ОблВыборРегистрации, ТипСмещенияТабличногоДокумента.ПоВертикали);
		КонецЕсли;
	КонецЕсли;
...
#Область ПодключениеОтсутствующихВКонфигурацииОтчетовВВидеПатчей

Функция СинонимОтчетаДляДереваОтчетов(ИмяОтчета) Экспорт 
	ОтчетМенеджер = Метаданные.Отчеты.Найти(ИмяОтчета);
	Если ТипЗнч(ОтчетМенеджер) = Тип("ОбъектМетаданных") Тогда 
		Возврат ?(ТипЗнч(ОтчетМенеджер.ОсновнаяФорма) = Тип("ОбъектМетаданных"),
			ОтчетМенеджер.ОсновнаяФорма.Синоним, ОтчетМенеджер.Синоним);
	КонецЕсли;
КонецФункции

Процедура ДополнитьДеревоОтчетовЭлементамиПатчей(ДеревоВсехИспользуемыхОтчетов) Экспорт 
	Попытка
		ТаблицаОписания = ПустаяТаблицаОписанияОбъектовРегламентированнойОтчетности();
		Для Каждого Отчет Из Метаданные.Отчеты Цикл 
			Если ТипЗнч(Отчет.РасширениеКонфигурации()) = Тип("РасширениеКонфигурации")
				...
				Попытка
					Отчеты[Отчет.Имя].ДополнитьДеревоОтчетовЭлементамиПатчей(ТаблицаОписания, Отчет.Имя);
				Исключение
				КонецПопытки;
			КонецЕсли;
		КонецЦикла;
		
		ТаблицаОписанияОтчетов = ТаблицаОписания.Скопировать(Новый Структура("ВидДокумента, ЯвляетсяАктуальным", Перечисления.СтраницыЖурналаОтчетность.Отчеты, Истина));
		Для Каждого Стр Из ТаблицаОписанияОтчетов Цикл
			ЗначенияКолонок = Новый Структура;
			ЗначенияКолонок.Вставить("Наименование", Стр.Наименование);
			ЗначенияКолонок.Вставить("ПолноеИмя", Метаданные.НайтиПоТипу(Стр.ТипОбъекта).ПолноеИмя());
			ЗначенияКолонок.Вставить("ТипПолучателя", Стр.ВидКонтролирующегоОргана);
			ЗначенияКолонок.Вставить("Категория", Стр.ГруппаВДереве);
			ЗначенияКолонок.Вставить("ЭтоГруппа", Ложь);
			ЗначенияКолонок.Вставить("Ссылка", Справочники.РегламентированныеОтчеты.ПустаяСсылка());
			
			ДобавитьОтчетВДеревоРегламентированныхОтчетов(ДеревоВсехИспользуемыхОтчетов, ЗначенияКолонок);
		КонецЦикла;
	Исключение
		ЗаписьЖурналаРегистрации("НеУдалосьПоказатьНовыеРегОтчеты");
	КонецПопытки;
КонецПроцедуры

Функция ИсточникОтчетаПоСинониму(Отчет) Экспорт
	СтрОтб = Новый Структура("Наименование, ПолноеИмя, Ссылка");
	ЗаполнитьЗначенияСвойств(СтрОтб, Отчет);
	Если СтрОтб.Ссылка = Справочники.РегламентированныеОтчеты.ПустаяСсылка()
		И СтрОтб.ПолноеИмя = "Документ.РегламентированныйОтчет"
		И ЗначениеЗаполнено(СтрОтб.Наименование) Тогда 
		
		Для Каждого Отч Из Метаданные.Отчеты Цикл 
			Если СтрНачинаетсяС(Отч.Имя, "EF_") И СинонимОтчетаДляДереваОтчетов(Отч.Имя) = СтрОтб.Наименование Тогда
				Возврат Отч.Имя;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
#Область НовыеМеханизмыПечати

Функция МакетыВАрхиве(ИмяФормы, ИмяМакетаАрхива) Экспорт
	СоответствиеИспользуемыхМакетов = Новый Соответствие;
	Если ЭтоВнешнийОтчет(ИмяФормы) Тогда 
		ДвоичныеДанныеМакета = РегламентированнаяОтчетностьВызовСервера.ОбъектОтчета(ИмяФормы).ПолучитьМакет(ИмяМакетаАрхива);
	Иначе
		АрхивТабличныхДокументов = Неопределено;
		ИмяОтчета = СтрРазделить(ИмяФормы, ".")[1];
		...
		Иначе
			ДвоичныеДанныеМакета = Отчеты[ИмяОтчета].ПолучитьМакет(АрхивТабличныхДокументов.Имя);
		КонецЕсли;
	КонецЕсли;
	
	ЧтениеZipФайла = Новый ЧтениеZipФайла(Новый ПотокВПамяти(ПолучитьБуферДвоичныхДанныхИзДвоичныхДанных(ДвоичныеДанныеМакета)));
...
Функция СтраницаЗаполнена(Данные, ИсключаемыеДанные = "") Экспорт 
	МассивИсключаемых = СтрРазделить(ИсключаемыеДанные, ",", Ложь);
	Для Каждого КЗ Из Данные Цикл
		Если МассивИсключаемых.Найти(КЗ.Ключ) = Неопределено И ЗначениеЗаполнено(КЗ.Значение) Тогда 
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
	Возврат Ложь;
...
Функция ПустаяСкопированнаяСтруктура(Исходник)
	Результат = Новый Структура;
	Для Каждого КЗ Из Исходник Цикл 
		Результат.Вставить(КЗ.Ключ, "");
	КонецЦикла;
	Возврат Результат;
КонецФункции

Процедура МногострочнаяЧастьВДанные(Данные, МногострочнаяЧасть, Начало, Конец) Экспорт 
	ДанныеИсходные = ПустаяСкопированнаяСтруктура(МногострочнаяЧасть[Начало - 1].Значение);
	Для Инд = Начало По Конец Цикл 
		ТекущиеДанные = ?(Инд > МногострочнаяЧасть.Количество(), ДанныеИсходные, МногострочнаяЧасть[Инд - 1].Значение);
		Постфикс = "_" + (1 + Инд - Начало);
		Для Каждого КЗ Из ТекущиеДанные Цикл 
			Данные.Вставить(КЗ.Ключ + Постфикс, ТекущиеДанные[КЗ.Ключ]);
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры

Процедура ДополнитьДанныеДляПечати(Данные, ДляДополнения) Экспорт 
	Для Каждого КЗ Из ДляДополнения Цикл 
		Данные.Вставить(КЗ.Ключ, ДляДополнения[КЗ.Ключ]);
	КонецЦикла;
КонецПроцедуры

Функция ПустоеЗначениеТипа(Значение)
	...
	Иначе
		...
		ТекущийТип.Добавить(ТипЗнч(Значение));
		ОТ = Новый ОписаниеТипов(ТекущийТип);
		Возврат ОТ.ПривестиЗначение(Неопределено);
	КонецЕсли;
КонецФункции

Процедура ОчисткаДанныхДляПечати(Данные) Экспорт 
	Для Каждого КЗ Из Данные Цикл 
		Если ЗначениеЗаполнено(КЗ.Значение) Тогда 
			Данные.Вставить(КЗ.Ключ, ПустоеЗначениеТипа(КЗ.Значение));
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

Функция ЗаполненнаяМногострочнаяЧасть(МногострочнаяЧасть) Экспорт 
	Результат = Новый СписокЗначений;
	Для Каждого Стр Из МногострочнаяЧасть Цикл 
		Если СтраницаЗаполнена(Стр.Данные) Тогда 
			Результат.Добавить(Стр.Данные);
		КонецЕсли;
	КонецЦикла;
	Возврат Результат;
КонецФункции

Функция ПустаяМногострочнаяЧасть(МногострочнаяЧасть) Экспорт 
	Результат = Новый СписокЗначений;
	Для Каждого Стр Из МногострочнаяЧасть Цикл 
		ПустаяСтруктура = ОбщегоНазначения.СкопироватьРекурсивно(Стр.Данные);
		ОчисткаДанныхДляПечати(ПустаяСтруктура);
		Результат.Добавить(ПустаяСтруктура);
		Прервать;
	КонецЦикла;
	Возврат Результат;
КонецФункции

Функция НовыйПустойЛист()
	ПечатнаяФорма = Новый ТабличныйДокумент;
	ПечатнаяФорма.ОтображатьЗаголовки = Ложь;
	ПечатнаяФорма.ОтображатьСетку     = Ложь;
	ПечатнаяФорма.ЧерноБелыйПросмотр  = Истина;
...
Функция ПустойМакетИзСписка(ИмяФормы, СписокИспользуемыхМакетов, ИмяМакета) Экспорт 
	Если ТипЗнч(СписокИспользуемыхМакетов) = Тип("Соответствие") Тогда 
		...
		Если ТипЗнч(МакетПФ) = Тип("ТабличныйДокумент") Тогда
			Результат = НовыйПустойЛист();
			Результат.Вывести(МакетПФ);
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
	
	Результат = НовыйПустойЛист();
	Результат.Вывести(РегламентированнаяОтчетностьВызовСервера.ОбъектОтчета(ИмяФормы).ПолучитьМакет(ИмяМакета));
	Возврат Результат;
КонецФункции

Процедура НапечататьСтруктуру(ПечатнаяФорма, Данные, КонтекстПечати) Экспорт 
	Для Каждого КЗ Из Данные Цикл 
		Если ЗначениеЗаполнено(КЗ.Значение) Тогда 
			ТипЗнчКЗЗначение = ТипЗнч(КЗ.Значение);
			Если ТипЗнчКЗЗначение = Тип("Число") Тогда 
				ЧислоВОбластиМакета(КЗ.Значение, КЗ.Ключ, ПечатнаяФорма, КонтекстПечати)
			ИначеЕсли ТипЗнчКЗЗначение = Тип("Строка") Тогда
				СтрокаВОбластиМакета(КЗ.Значение, КЗ.Ключ, ПечатнаяФорма, КонтекстПечати);
			ИначеЕсли ТипЗнчКЗЗначение = Тип("Дата") Тогда
				ДатаВОбластиМакета(КЗ.Значение, КЗ.Ключ, ПечатнаяФорма, КонтекстПечати);
			Иначе
				СтрокаВОбластиМакета(КонтекстПечати.ОТС.ПривестиЗначение(КЗ.Значение), КЗ.Ключ, ПечатнаяФорма, КонтекстПечати);
			КонецЕсли;
		Иначе
			СтрокаВОбластиМакета("", КЗ.Ключ, ПечатнаяФорма, КонтекстПечати);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

Процедура ЧислоВОбластиМакета(Число, ПрефиксПараметра, ПечатнаяФорма, КонтекстПечати)
	ЦелЧисло = Цел(Число);
	Если Число = ЦелЧисло Тогда 
		Строка = ?(ЗначениеЗаполнено(ЦелЧисло), Лев(Формат(ЦелЧисло, "ЧГ=") + КонтекстПечати["ОстатокСтроки"], 32), "");
		СтрокаВОбластиМакета(Строка, ПрефиксПараметра, ПечатнаяФорма, КонтекстПечати);
	Иначе
		ТочкаПройдена = Ложь;
		ДоЗапятой = 0;
		ПослеЗапятой = 0;
		Индекс = 0;
		ПраваяЧастьОбласти = -1;
		
		ПрПарам = ПрефиксПараметра + "_";
		Пока Истина Цикл 
			Индекс = Индекс + 1;
			Обл = ПечатнаяФорма.Области.Найти(ПрПарам + Индекс);
			Если Обл = Неопределено Тогда 
				Прервать;
			КонецЕсли;
			
			Если ТочкаПройдена Тогда 
				ПослеЗапятой = ПослеЗапятой + 1;
			ИначеЕсли Индекс > 1 Тогда
				Если (ПраваяЧастьОбласти + 1 <> Обл.Лево) Тогда 
					ТочкаПройдена = Истина;
					ПослеЗапятой = 1;
				Иначе
					ПраваяЧастьОбласти = Обл.Право;
					ДоЗапятой = ДоЗапятой + 1;
				КонецЕсли;
			Иначе
				ПраваяЧастьОбласти = Обл.Право;
				ДоЗапятой = 1;
		...
		Если Не ЗначениеЗаполнено(ДоЗапятой) Или Не ЗначениеЗаполнено(Число) Тогда 
			СтрокаВОбластиМакета("", ПрефиксПараметра, ПечатнаяФорма, КонтекстПечати);
		Иначе
			Остаток = (Число - ЦелЧисло);
			Остаток = ?(Остаток < 0, -1*Остаток, Остаток);
			ДЧ = "";
			Пока ЗначениеЗаполнено(Остаток - Цел(Остаток)) И СтрДлина(ДЧ) < ПослеЗапятой Цикл 
				Остаток = Остаток * 10;
				ДЧ = ДЧ + Формат(Цел(Остаток), "ЧЦ=1; ЧН=0");
				Остаток = (Остаток - Цел(Остаток));
				Остаток = ?(Остаток < 0, -1*Остаток, Остаток);
			КонецЦикла;
			Строка = Лев(Формат(ЦелЧисло, "ЧН=0; ЧГ=") + КонтекстПечати["ОстатокСтрокиЧисло"], ДоЗапятой) 
				+ Лев(ДЧ + КонтекстПечати["ОстатокСтрокиЧисло"], ПослеЗапятой);
			СтрокаВОбластиМакета(Строка, ПрефиксПараметра, ПечатнаяФорма, КонтекстПечати);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Процедура ДатаВОбластиМакета(Дата, ПрефиксПараметра, ПечатнаяФорма, КонтекстПечати)
	СтрокаВОбластиМакета(Формат(Дата, КонтекстПечати["ФорматДатыПоУмолчанию"]), ПрефиксПараметра, ПечатнаяФорма, КонтекстПечати);
КонецПроцедуры

Процедура СтрокаВОбластиМакета(Строка, ПрефиксПараметра, ПечатнаяФорма, КонтекстПечати)
	СтрокаКПечати = Строка + КонтекстПечати["ОстатокСтроки"];
	...
	Пока Истина Цикл 
		...
		Если Обл = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		Обл.Текст = Сред(СтрокаКПечати, Индекс, 1);
	КонецЦикла;
КонецПроцедуры

Процедура ПоложитьПФВСписокЛистов(Листы, ПечатнаяФорма, Наименование, КонтекстПечати) Экспорт 
	Если КонтекстПечати.Автонумерация Тогда 
		СтрокаВОбластиМакета(Прав("000" + Формат(Листы.Количество() + 1, "ЧГ="), 3), "НомСтр", ПечатнаяФорма, КонтекстПечати);
	КонецЕсли;
...
Функция КонтекстПечатиДекларацииПоУмолчанию() Экспорт 
	КонтекстПечати = Новый Структура;
	КонтекстПечати.Вставить("ОТС", Новый ОписаниеТипов("Строка"));
	КонтекстПечати.Вставить("СимволПоУмолчанию", "-");
	КонтекстПечати.Вставить("ФорматДатыПоУмолчанию", "ДФ=ddMMyyyy");
	КонтекстПечати.Вставить("Автонумерация", Истина);
	
	ОстатокСтроки = КонтекстПечати["СимволПоУмолчанию"];
	Для Инд = 1 По 600 Цикл 
		ОстатокСтроки = ОстатокСтроки + КонтекстПечати["СимволПоУмолчанию"];
	КонецЦикла;
	
	КонтекстПечати.Вставить("ОстатокСтроки", ОстатокСтроки);
	КонтекстПечати.Вставить("ОстатокСтрокиЧисло", Лев(ОстатокСтроки, 20));
