	...
	Если КонтекстФормы.СтруктураРеквизитовФормы.мКодШаблона <> ПараметрыReport["code"] Тогда
		Если Сред(КонтекстФормы.СтруктураРеквизитовФормы.мКодОтчета, 2) = Лев(ПараметрыReport["code"], 6) Тогда 
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не соответствует код шаблона. Возможно, файл от другого отчета';
														|en = 'Не соответствует код шаблона. Возможно, файл от другого отчета'"));
		Иначе
...
Процедура ПодготовитьДанныеОтчетаПоШаблону(КонтекстФормы, ДанныеШаблонаОтчета, Отказ = Ложь) Экспорт 
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось получить текст XML-шаблона:%1';
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете(ПараметрыДляПроверки) Экспорт 
	...
	Для каждого КонтрольноеСоотношение Из ПараметрыДляПроверки.ТаблицаКонтрольныхСоотношений Цикл
		...
		Если ПараметрыДляПроверки.КвоОшибокКС > 0 Тогда
			Стр1 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																				|en = '%1'"), "Не удалось вычислить условие! Соотношение не используется.");
			Стр2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																				|en = '%1'"), ?(ОбязательностьКонтроля, "", "Контролировать не обязательно"));
...
Функция НомерПериодаИзСправочника(ПараметрыДляПроверки, СтрКодПериода)
	Если СтрДлина(СтрКодПериода) = 4 Тогда 
		Возврат СтрКодПериода;
	КонецЕсли;
...
Функция ПорядковыйНомерПериодаВГоду(ДатаВПериоде, Периодичность)
	...
	ИначеЕсли Периодичность = 2 Тогда
		Возврат "020" + (ДатаМесяц / 6);
	ИначеЕсли Периодичность = 4 Тогда
		Возврат "040" + (ДатаМесяц / 3);
	ИначеЕсли Периодичность = 12 Тогда
		Возврат "12" + Формат(ДатаМесяц, "ЧЦ=2; ЧВН=");
	ИначеЕсли Периодичность = 36 Тогда
		Возврат "36" + Формат(Окр((ДеньГода(ДатаВПериоде) - 2) / 10, 0), "ЧЦ=2; ЧВН=");
	ИначеЕсли Периодичность = 52 Тогда
		Возврат "52" + Формат(НеделяГода(ДатаВПериоде), "ЧЦ=2; ЧВН=");
	Иначе
		Возврат "040" + (ДатаМесяц / 3);
	КонецЕсли;
КонецФункции
...
Функция ПолучитьШаблон(ИмяФормы, ИмяМакетаШаблона) Экспорт 
	...
	ЧтениеZip = Новый ЧтениеZipФайла(Новый ПотокВПамяти(
		ПолучитьБуферДвоичныхДанныхИзДвоичныхДанных(РегламентированнаяОтчетностьВызовСервера.ОбъектОтчета(ИмяФормы).ПолучитьМакет(ИмяМакетаШаблона))));
	КаталогРазархивирования = КаталогВременныхФайлов();
	ЧтениеZip.Извлечь(ЧтениеZip.Элементы[0], КаталогРазархивирования, РежимВосстановленияПутейФайловZIP.НеВосстанавливать);
...
Функция ПолучитьПостроительПоПараметру(Шаблон)
	...
	ИначеЕсли ТипЗнч(Шаблон) = Тип("ДвоичныеДанные") Тогда
		КаталогРазархивирования = КаталогВременныхФайлов();
		ЧтениеZip = Новый ЧтениеZipФайла(Новый ПотокВПамяти(ПолучитьБуферДвоичныхДанныхИзДвоичныхДанных(Шаблон)));
