...
Функция ЗаписатьИзмененияОбъектов(Сообщение) Экспорт
	...
	Попытка
		...
		ПараметрыСеанса.ИнтеграцияС1СДокументооборотСхемаXML = Сообщение.DMXMLSchema;
		ФабрикаDM = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ФабрикаDM();
...
Процедура ЗагрузитьСсылочныйОбъект(ОбъектСсылка, ОбъектXDTO, УзелДокументооборота, СоставПланаОбмена)
	...
	Если ПодходящиеПравила = Неопределено Или ПодходящиеПравила.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Не найдены подходящие правила интеграции';
								|en = 'Suitable integration rules are not found'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
