...
Процедура ОбновитьПоставляемыеДрайвера(ДрайвераОборудования) Экспорт
	...
	Драйвер.Наименование = НСтр("ru = 'Гексагон:Принтера этикеток Zebra,Proton,Toshiba-TEC,Datamax-O neil';
								|en = 'Hexagon:Zebra,Proton,Toshiba-TEC,Datamax-O neil label printers'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'Скансити:Принтер печати этикеток TSC';
								|en = 'Scancity:TSC label printer'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'Сканкод:Принтера этикеток Godex EZPL8 (Native API)';
								|en = 'Scancode:Godex EZPL8 label printers (Native API)'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'АТОЛ:Принтера этикеток';
								|en = 'ATOL:Label printers'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
...
#Если НЕ МобильноеПриложениеСервер Тогда
	...
	ШаблонСообщенияОбОшибке = НСтр("ru = 'При чтении файла атрибута %1 элемента %2 произошла ошибка.';
									|en = 'An error occurred when reading a file of the %2 attribute of the %1 item.'");
					...
					Если ПрочитатьАтрибут(ЧтениеXML, "FontSize", ЗначениеАтрибута) Тогда
						Если ПодписьШтрихкода И ЗначениеАтрибута=Неопределено Тогда
							...
							ШаблонДополнение = НСтр("ru = 'При значении атрибута %1 = %2 указание значения атрибута %3 обязательно.';
													|en = 'If the attribute value is %1 =%2, the attribute value %3 is required.'");
...
Функция ПрочитатьЭлементXML(ЧтениеXML, ТекстСообщенияОбОшибке)
	...
	Исключение
		ТекстСообщенияОбОшибке = НСтр("ru = 'При чтении XML-описания макета произошла ошибка.';
										|en = 'An error occurred when reading XML description of the template.'");
		Результат = Ложь;
	КонецПопытки;
