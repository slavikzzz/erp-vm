...
Функция ЭтоПроверкаПиваВРозницуСЧастичнымВыбытием(ПараметрыСканирования, СтрокаДанных) Экспорт
	...
	ПроверкаПродажиПиваВРозницуСЧастичнымВыбытием = (СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.Пиво
		...
		И (ОбщегоНазначенияИС.ИспользуетсяЧастичноеВыбытие(Перечисления.ВидыПродукцииИС.Пиво)
			Или ОбщегоНазначенияИС.ИспользуетсяЧастичноеВыбытие(Перечисления.ВидыПродукцииИС.ПивоВПотребительскихУпаковках))
...
Функция НовыеПараметрыРаспределенияШтрихкодовУпаковок() Экспорт
	...
	Параметры.Вставить("ЭтоПробитиеЧека",                 Ложь);
...
Функция РаспределениеШтрихкодовУпаковокПоТоварам(ПараметрыРаспределения, Товары, МассивУпаковок) Экспорт
	...
	Для Каждого СтрокаШтрихкод Из ТаблицаШтрихкоды Цикл
		...
		Для Каждого СтрокаТовары Из НайденныеСтрокиТовары Цикл
			...
			ДобавитьСтрокуРаспределенияВРезультат(ВозвращаемоеЗначение, Товары, СтрокаТовары, РезультатСписания);
			...
			Если ЭтоОшибкаТребуетВзвешивания Тогда
				РезультатСписания.ТекстОшибки = НСтр("ru = 'Требуется указание веса товара';
													|en = 'Требуется указание веса товара'");
				Прервать;
			КонецЕсли;
			...
			Если СтрокаШтрихкод.ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет
				...
				РезультатСписания.ТекстОшибки = НСтр("ru = 'Пробитие чеков при объемно-сортовом учете не поддерживается.';
													|en = 'Пробитие чеков при объемно-сортовом учете не поддерживается.'");
			КонецЕсли;
			...
			ВозвращаемоеЗначение.Ошибки.Добавить(
				СтрШаблон(
					НСтр("ru = 'Не удалось распределить код маркировки по товарам:
						      |%1 (%2)';
						      |en = 'Не удалось распределить код маркировки по товарам:
						      |%1 (%2)'"),
...
Процедура ДобавитьСтрокуРаспределенияВРезультат(ВозвращаемоеЗначение, Товары, СтрокаТовары, РезультатСписания)
...
Процедура ДополнитьРаспределениеПоТоварамБезШтрихкодов(ВозвращаемоеЗначение, ПараметрыРаспределения, ПараметрыСканирования, Дата, ТаблицаТовары, Товары)
	...
	Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
		...
		Если ВидПродукции = Неопределено Тогда
			ВидПродукции = ОбщегоНазначенияИС.ВидПродукцииПоНоменклатуре(СтрокаТовары.Номенклатура);
		...
		Если ЭтоПродукцияИСМП Тогда
			
			Если ПараметрыРаспределения.ЭтоПробитиеЧека Тогда
				ТребуетсяКонтрольМарок = ОбщегоНазначенияИСМППовтИсп.ДействуетУведомительныйРежимПоВидуПродукции(ВидПродукции, Дата)
					И Не ОбщегоНазначенияИСМППовтИсп.РазрешенОборотНемаркируемыхОстатковПродукции(ВидПродукции, Дата);
			Иначе
				ТребуетсяКонтрольМарок = ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПоВидуОперацииИПродукции(ОперацияИС, ВидПродукции, Дата);
			КонецЕсли;
			
		Иначе
			ТребуетсяКонтрольМарок = Ложь;
		КонецЕсли;
		
		Если Не ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(ВидПродукции, Истина)
			Или Не ТребуетсяКонтрольМарок
		...
		РезультатСписания.ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Требуется код маркировки для товара: %1';
				|en = 'Требуется код маркировки для товара: %1'"),
			Представление);
