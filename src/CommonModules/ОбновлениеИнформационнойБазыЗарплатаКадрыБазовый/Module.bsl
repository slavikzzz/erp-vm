...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка параметров для набора дополнительных свойств объектов библиотеки Зарплата и кадры (базовая)';
									|en = 'Setting of parameters for collection of additional parameters of the Salary and employees (basic) library objects'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Переименовать существующий способ округления ""Без округления"" в ""Округление до копеек""';
										|en = 'Rename the existing rounding method ""No rounding"" to ""Rounding to kopecks""'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Создать новый способ округления ""Без округления""';
										|en = 'Create a new rounding method ""No Rounding""'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Установить способ формирования интервальных регистров';
										|en = 'Set the method for generating interval registers'");
	...
	ИнтеграцияКабинетСотрудника.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	
	ИнтеграцияУправлениеПерсоналом.ЗарегистрироватьОбработчикиОбновления(Обработчики);

	ЗаявкиСотрудников.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПодсистемы.ЗащитаПерсональныхДанных") Тогда
		...
		МодульЗащитаПерсональныхДанныхЗарплатаКадры.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	КонецЕсли;
...
Функция ОписаниеБлокируемыхДанных(Знач МетаданныеОбъекта, Знач ПоляБлокировки = Неопределено) Экспорт
		...
		Иначе
			...
			Иначе
				...
				Если Описание.ПоляБлокировки.Количество() = 0 Тогда
					Шаблон = НСтр("ru = 'Неверное имя полей блокировки %1';
									|en = 'Incorrect name of lock fields %1'", ОбщегоНазначения.КодОсновногоЯзыка());
			...
			Для Каждого ПолеБлокировки Из Описание.ПоляБлокировки Цикл
				Если МассивИзмерений.Найти(ПолеБлокировки.Ключ) = Неопределено Тогда
					Шаблон = НСтр("ru = 'Неверное имя поля блокировки %1';
									|en = 'Incorrect name of lock field %1'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	ИначеЕсли ОбщегоНазначения.ЭтоКонстанта(МетаданныеОбъекта) Тогда
		...
		Описание.ПоляБлокировки = Новый Структура;
	Иначе
		ВызватьИсключение НСтр("ru = 'Недопустимый тип объекта';
								|en = 'Invalid object type'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
Функция ВыполнитьЗапросПолученияОбновляемыхДанных(Запрос, Знач ПараметрыОбновления, Знач ИмяВТОбновляемыеДанные = "", КоличествоОбрабатываемыхОбъектов = 1000) Экспорт
	...
	Иначе
		...
		Если ЗапросВыбораНеНайден Тогда
			ВызватьИсключение НСтр("ru = 'Не определен запрос выбора данных. Ошибка в имени временной таблицы получения данных';
									|en = 'Data selection query is not determined. An error occurred in the name of temporary table of data receipt'", ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
	КонецЕсли;
