...
// ВозможнаПроверкаПоИНН и ВозможнаПроверкаПоСНИЛС заполняются из параметра РезультатПоиска.
//
// Параметры:
//	РезультатПоиска	- Структура - содержит результаты проверки уникальности физического лица
...
//	 * СсылкаНаОригинал - СправочникСсылка, Неопределено - 
//	 * НайденыДубли - Булево
Функция ПараметрыОткрытияФормыФизическиеЛицаСПохожимиДанными(РезультатПоиска = Неопределено) Экспорт
	...
	ПараметрыОткрытия.Вставить("СсылкаНаОригинал", Неопределено);
	ПараметрыОткрытия.Вставить("НайденыДубли", Ложь);
	...
	Если РезультатПоиска <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыОткрытия, РезультатПоиска);
	КонецЕсли;
...
Процедура СформироватьНаименованиеСотрудника(Форма) Экспорт
	...
	Форма.Сотрудник.Наименование = КадровыйУчетКлиентСервер.ПолноеНаименованиеСотрудника(Форма.ФИОФизическихЛиц.Фамилия,
		Форма.ФИОФизическихЛиц.Имя, Форма.ФИОФизическихЛиц.Отчество, Форма.ФизическоеЛицо.УточнениеНаименования,
...
Процедура УстановитьФизическоеЛицоВФормеСотрудника(Форма, ФизическоеЛицо) Экспорт
	
	Если ФизическоеЛицо = Неопределено Или ФизическоеЛицо.Пустая() Тогда
		Возврат;
	КонецЕсли;
	
	Форма.Сотрудник.ФизическоеЛицо = ФизическоеЛицо;
	Форма.ФизическоеЛицоСсылка = ФизическоеЛицо;
...
//  Форма - ФормаКлиентскогоПриложения
//  ФизическоеЛицо - СправочникСсылка.ФизическиеЛица
Процедура ОбработатьВыборФизическогоЛицаОднофамильца(Форма, ФизическоеЛицо) Экспорт
	
	Форма.Модифицированность = Ложь;
	Форма.Закрыть();
	
	Если Форма.РежимВыбора Тогда
		Оповестить("СозданоФизическоеЛицо", ФизическоеЛицо, Форма.ВладелецФормы);
	Иначе
		ОткрытьФорму("Справочник.ФизическиеЛица.ФормаОбъекта", Новый Структура("Ключ", ФизическоеЛицо));
	КонецЕсли;
			...
			Если СообщатьОНевозможностиБлокировки Тогда
				...
				Если ЗарплатаКадрыВызовСервера.СБазойРаботаетЕдинственныйПользователь() Тогда
					ПоказатьПредупреждение(, НСтр("ru = 'Не удается внести изменения в личные данные сотрудника. Возможно, уже открыта форма с личными данными.';
													|en = 'Cannot change employee personal data. Form with personal data may have been opened.'"));
				Иначе
					ПоказатьПредупреждение(, НСтр("ru = 'Не удается внести изменения в личные данные сотрудника. Возможно, уже открыта форма с личными данными или личные данные редактируются другим пользователем.';
													|en = 'Cannot change employee personal data. The form with personal data may have been opened or personal data is being edited by another user.'"));
				КонецЕсли;
...
Процедура НачатьЗапись(Форма, ЗакрытьПослеЗаписи = Ложь, ОповещениеЗавершения = Неопределено) Экспорт
	...
	ТекстВопроса = НСтр("ru = 'Данные физического лица не записаны.
						|Записать и продолжить?';
						|en = 'Individual''s data is not saved.
						|Save and continue?'");
...
Процедура ВвестиНовыеЕжегодныеСтандартныеВычеты(ФизическоеЛицо, Организация) Экспорт
	
	ЗначенияЗаполнения = Новый Структура;
	ЗначенияЗаполнения.Вставить("Сотрудник", ФизическоеЛицо);
	ЗначенияЗаполнения.Вставить("Организация", Организация);
	ЗначенияЗаполнения.Вставить("ПрименятьВычетыС", Макс(НачалоМесяца(ОбщегоНазначенияКлиент.ДатаСеанса()),'20250101'));
	ЗначенияЗаполнения.Вставить("НалоговыйПериод", Макс(Год(ОбщегоНазначенияКлиент.ДатаСеанса()),2025));

	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗначенияЗаполнения", ЗначенияЗаполнения);
	ПараметрыФормы.Вставить("ИзФормыСотрудника", Истина);
	
	ОткрытьФорму("Документ.ЕжегодныеСтандартныеВычеты.ФормаОбъекта", ПараметрыФормы);
...
Процедура ПриИзмененииФИО(Форма, ФизическоеЛицо, МенеджерЗаписиФИО)
	...
	ФИОПолные = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ФИОИзЧастей(МенеджерЗаписиФИО);
	ОкончаниеФИО = СтрЗаменить(ФИО, ФИОПолные, "");
	Если Не ПустаяСтрока(ОкончаниеФИО) Тогда
		МенеджерЗаписиФИО.Отчество = МенеджерЗаписиФИО.Отчество + " " + СокрЛП(ОкончаниеФИО);
	КонецЕсли;
	
	Если СтрНайти(ФИО, ФИОПолные) = 0 Тогда
		ФизическоеЛицо.ФИО = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ФИОИзЧастей(МенеджерЗаписиФИО);
		ПоказатьПредупреждение(, НСтр("ru = 'Возможно, не корректно введено полное имя!
			...
			|en = 'The full name may have been entered incorrectly!
			|The full name does not match the one retrieved from the last name, first name and patronymic data.'"));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(МенеджерЗаписиФИО.Фамилия) Тогда
		ФизическоеЛицо.Пол = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ПолПоЧастямФИО(МенеджерЗаписиФИО);
		Если ФизическоеЛицо.Пол = ПредопределенноеЗначение("Перечисление.ПолФизическогоЛица.ПустаяСсылка") Тогда
			...
			ТекстСостояния = НСтр("ru = 'Не удалось установить пол сотрудника.
				...
				|en = 'Cannot identify the employee gender.
				|Assigned by default - %1.'");
		Иначе
			ТекстСостояния = НСтр("ru = 'Установлен пол сотрудника - %1.';
									|en = 'The employee gender is specified - %1.'");
		КонецЕсли;
	...
	Если ИзменениеЗаднимЧислом Тогда 
		...
		ТекстПредупреждения = СтрШаблон(
			НСтр("ru = 'Изменение ФИО выполнено ""задним числом"". Введенное данные действуют с %1 по %2.
			           ...
			           |en = 'The full name is changed post factum. The entered data is valid from %1 until %2.
			           |Editing full name directly in the form is not available now and will be available only after writing.   '"),
...
Процедура ИзменитьФИОФизическогоЛицаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если Результат.ДатаИзменения >= МенеджерЗаписиФИО.Период Тогда
			...
			НовоеНаименование = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ФИОИзЧастей(Результат);
		Иначе
			// Вернем прежнее имя если это - ввод хронологически не последней записи регистра ФИО.
			НовоеНаименование = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ФИОИзЧастей(МенеджерЗаписиФИО);
		КонецЕсли;
...
Процедура ПроверитьОднофамильцев(Форма, ИзФормыСотрудника, ОповещениеЗавершения = Неопределено)
	...
	ИНН = Форма.ФизическоеЛицо.ИНН;
	СтраховойНомерПФР = Форма.ФизическоеЛицо.СтраховойНомерПФР;
	СНИЛСЗаполнен = КадровыйУчетКлиентСервер.СНИЛСЗаполнен(СтраховойНомерПФР);
	
	ЗаполненыИННИлиСНИЛС = Не ПустаяСтрока(ИНН) Или СНИЛСЗаполнен;
	ИННУказанПравильноИлиПустой = Форма.ИННУказанПравильно Или ПустаяСтрока(ИНН);
	СНИЛСУказанПравильноИлиПустой = Форма.СНИЛСУказанПравильно Или Не СНИЛСЗаполнен;
		
	Если ЗаполненыИННИлиСНИЛС И ИННУказанПравильноИлиПустой И СНИЛСУказанПравильноИлиПустой
		И СотрудникиВызовСервера.ИННИСНИЛСУникальны(ИНН, СтраховойНомерПФР) Тогда
		Если ОповещениеЗавершения <> Неопределено Тогда 
	...
	РезультатПоискаПохожих = СотрудникиВызовСервера.ПохожиеФизическиеЛица(ПараметрыПоиска);
	ФизическоеЛицоУникально = РезультатПоискаПохожих.ФизическоеЛицоУникально;
	КоличествоПохожих = РезультатПоискаПохожих.ДанныеФизическихЛиц.Количество();
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
		МодульПодборПерсоналаКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПодборПерсоналаКлиент");
		МодульПодборПерсоналаКлиент.ПроверитьОднофамильцевСотрудникаСредиКандидатов(Форма, РезультатПоискаПохожих,
			ФизическоеЛицоУникально, КоличествоПохожих);
	КонецЕсли;
	
	Если ФизическоеЛицоУникально Тогда
		Если ОповещениеЗавершения <> Неопределено Тогда 
			ВыполнитьОбработкуОповещения(ОповещениеЗавершения);
		КонецЕсли;
		Возврат;
	...
	ПараметрыОткрытия = ПараметрыОткрытияФормыФизическиеЛицаСПохожимиДанными(РезультатПоискаПохожих);
	...
	Если КоличествоПохожих > 0 Тогда
		Оповещение = Новый ОписаниеОповещения("ПроверитьОднофамильцевДанныеФизическихЛиц", ЭтотОбъект,
			ДополнительныеПараметры);
		ОткрытьФорму("Справочник.ФизическиеЛица.Форма.ФизическиеЛицаСПохожимиДанными", ПараметрыОткрытия, Форма, 
			Форма.ФизическоеЛицоСсылка, , , Оповещение);
	Иначе
		Если РезультатПоискаПохожих.ВозможнаПроверкаПоИНН ИЛИ РезультатПоискаПохожих.ВозможнаПроверкаПоСНИЛС Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Найден человек с похожим именем.
				...
				|en = 'Person with a similar name is found.
				|Contact the information system administrator to fix this issue.'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
			Если Не РезультатПоискаПохожих.ДоступнаРольСохранениеДанныхЗадвоенныхФизическихЛиц Тогда
				Форма.Модифицированность = Ложь;
				Форма.Закрыть();
			КонецЕсли;
			Если ОповещениеЗавершения <> Неопределено Тогда 
...
Процедура ПроверитьОднофамильцевДанныеФизическихЛиц(РезультатВыбора, ДополнительныеПараметры) Экспорт 
	...
	Если РезультатВыбора <> Неопределено Тогда
		...
		Если ИзФормыСотрудника Тогда
			...
			Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
				...
				МодульПодборПерсоналаКлиент.ПроверитьОднофамильцевСотрудникаСредиКандидатовЗавершение(РезультатВыбора,
					ДополнительныеПараметры);
			Иначе
				УстановитьФизическоеЛицоВФормеСотрудника(Форма, РезультатВыбора.ПохожийЧеловек);
			КонецЕсли;
		...
		Иначе
			...
			Иначе
				ОбработатьВыборФизическогоЛицаОднофамильца(Форма, РезультатВыбора.ПохожийЧеловек);
			КонецЕсли;
...
Процедура ПроверитьОднофамильцевИНН_СНИЛС(РезультатПроверки, ДополнительныеПараметры) Экспорт 
	...
	Если РезультатПроверки = Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Проверка уникальности не пройдена.
			|Обратитесь к администратору информационной системы для устранения проблемы.';
			|en = 'Uniqueness check failed.
			|Contact your infobase administrator to resolve issues.'");
	...
	Если ФормаИсточник.ДоступенПросмотрДанныхФизическихЛиц Тогда
		...
		ТекстВопроса = СтрШаблон(
				НСтр("ru = 'При редактировании изменены сведения о гражданстве. 
				            ...
				            |en = 'When editing, citizenship information was changed.
				            |If only citizenship entry is corrected (it was incorrect), click ""Error is corrected"".
				            |If the employee''s citizenship changed from %1, click ""%2""'"), 
	...
	Если ФормаИсточник.ДоступенПросмотрДанныхФизическихЛиц Тогда
		...
		ТекстВопроса = СтрШаблон(
				НСтр("ru = 'При редактировании изменены сведения о документе, удостоверяющем личность.
				            ...
				            |en = 'When editing, identity document information was changed.
				            |If the previous document entry is corrected (it was incorrect), click ""Error is corrected"".
				            |If the employee''s identity document changed from %1, click ""%2""'"),
				Формат(ДатаИзменения, "ДФ='д ММММ гггг ""г""'"),
				ТекстКнопкиДа);
