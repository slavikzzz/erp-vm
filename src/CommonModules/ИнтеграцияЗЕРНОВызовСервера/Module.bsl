...
Функция ПодготовитьКПередаче(ВходящиеДанные, УникальныйИдентификатор = Неопределено) Экспорт
	...
	Если УникальныйИдентификатор <> Неопределено И Не ОбщегоНазначенияИС.РежимРаботыБезФоновыхЗаданий() Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Выполнение обмена с ФГИС ""Зерно""';
																|en = 'Выполнение обмена с ФГИС ""Зерно""'");
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'При записи подписанного сообщения возникла ошибка:
			           |Текст ошибки: %1';
			           |en = 'При записи подписанного сообщения возникла ошибка:
			           |Текст ошибки: %1'"),
			...
			Если СведенияЮрЛицаПоДаннымСервиса <> Неопределено Тогда
				Если ЗначениеЗаполнено(СведенияЮрЛицаПоДаннымСервиса.ОписаниеОшибки) Тогда
					...
					Если СтрНайти(
						...
						НСтр("ru = 'не подключена.';
							|en = 'не подключена.'")) > 0 Тогда
...
Функция СписокВыбораИдентификаторовАдресовОбъектов(Объекты) Экспорт
	...
	Для Каждого Объект Из Объекты Цикл
		...
		ДанныеАдреса = ИнтеграцияИСВызовСервера.ДанныеАдресаОбъекта(Объект, Истина);
		...
		Если ЗначениеЗаполнено(ДанныеАдреса.Идентификатор) Тогда
			ДополнительныеДанныеПредставления.Добавить(СтрШаблон("%1: %2", НСтр("ru = 'ГАР';
																				|en = 'ГАР'"), ДанныеАдреса.Идентификатор));
		КонецЕсли;
		Если ЗначениеЗаполнено(ДанныеАдреса.ОКТМО) Тогда
			ДополнительныеДанныеПредставления.Добавить(СтрШаблон("%1: %2", НСтр("ru = 'ОКТМО';
																				|en = 'ОКТМО'"), ДанныеАдреса.ОКТМО));
		КонецЕсли;
