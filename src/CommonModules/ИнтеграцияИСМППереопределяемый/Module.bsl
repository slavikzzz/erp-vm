...
Процедура ПриОпределенииКомплектующихНаборов(Наборы, КомплектующиеНаборов) Экспорт
	...
	// получение данных из ресурсных спецификаций
	МассивНомеровСтрок = КомплектующиеНаборов.ВыгрузитьКолонку("НомерСтрокиНабора");
	КоличествоСтрок = ТаблицаТоваров.Количество();
	Для Счетчик = 1 По КоличествоСтрок Цикл
		СтрокаТаблицы = ТаблицаТоваров[КоличествоСтрок - Счетчик];
		Если МассивНомеровСтрок.Найти(СтрокаТаблицы.НомерСтроки) <> Неопределено Тогда
	...
	Если ТаблицаТоваров.Количество() > 0 Тогда
		...
		Для каждого СтрокаТаблицы Из ТаблицаТоваров Цикл
			...
			ЗаполнитьЗначенияСвойств(ДанныеОбИзделии, СтрокаТаблицы);
		...
		ДанныеСпецификаций = Справочники.РесурсныеСпецификации.ДанныеСпецификацииПоСпискуНоменклатуры(
			СписокНоменклатуры,
			Справочники.РесурсныеСпецификации.ПараметрыВыборкиДанных("ВыходныеИзделия,МатериалыИУслуги"));
		
		МатериалыПоСпецификациям = КомплектующиеНаборов.СкопироватьКолонки();
		
		Для Каждого ДанныеСпецификации Из ДанныеСпецификаций Цикл
			
			Если ДанныеСпецификации = Неопределено
				Или ДанныеСпецификации.ВыходныеИзделия.Количество() <> 1
				Или Не ЗначениеЗаполнено(ДанныеСпецификации.ВыходныеИзделия[0].Номенклатура) Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокаВыходногоИзделия = ДанныеСпецификации.ВыходныеИзделия[0];
			КоличествоВыходногоИзделия = СтрокаВыходногоИзделия.КоличествоУпаковокНаЕдиницуПартииВыпуска *
				СтрокаВыходногоИзделия.ДанныеУпаковки.Знаменатель /
				СтрокаВыходногоИзделия.ДанныеУпаковки.Числитель;
			
			Для Каждого СтрокаМатериала Из ДанныеСпецификации.МатериалыИУслуги Цикл
				КоличествоМатериала = СтрокаМатериала.КоличествоУпаковокНаЕдиницуПартииВыпуска *
					СтрокаМатериала.ДанныеУпаковки.Знаменатель /
					СтрокаМатериала.ДанныеУпаковки.Числитель;
				Строка = МатериалыПоСпецификациям.Добавить();
				Строка.НоменклатураНабора = СтрокаВыходногоИзделия.Номенклатура;
				Строка.ХарактеристикаНабора = СтрокаВыходногоИзделия.Характеристика;
				Строка.УпаковкаНабора = СтрокаВыходногоИзделия.Упаковка;
				Строка.НомерСтрокиКомплектации = СтрокаМатериала.НомерСтроки;
				Строка.Номенклатура = СтрокаМатериала.Номенклатура;
				Строка.Характеристика = СтрокаМатериала.Характеристика;
				Строка.Упаковка = СтрокаМатериала.Упаковка;
				Строка.КоличествоУпаковок = СтрокаМатериала.КоличествоУпаковокНаЕдиницуПартииВыпуска /
					СтрокаВыходногоИзделия.КоличествоУпаковокНаЕдиницуПартииВыпуска;
				Строка.Количество = КоличествоМатериала / КоличествоВыходногоИзделия;
			КонецЦикла;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаМатериалов.НоменклатураНабора КАК НоменклатураНабора,
		|	ТаблицаМатериалов.ХарактеристикаНабора КАК ХарактеристикаНабора,
		|	ТаблицаМатериалов.УпаковкаНабора КАК УпаковкаНабора,
		|	ТаблицаМатериалов.НомерСтрокиКомплектации КАК НомерСтрокиКомплектации,
		...
		|	ТаблицаМатериалов.ХарактеристикаНабора КАК ХарактеристикаНабора,
		|	ТаблицаМатериалов.УпаковкаНабора КАК УпаковкаНабора,
		|	МИНИМУМ(ТаблицаМатериалов.НомерСтрокиКомплектации) КАК НомерСтрокиКомплектации,
		|	ТаблицаМатериалов.Номенклатура КАК Номенклатура,
		|	ТаблицаМатериалов.Характеристика КАК Характеристика,
		|	ТаблицаМатериалов.Упаковка КАК Упаковка,
		|	ТаблицаТоваров.КоличествоУпаковокНабора * СУММА(ТаблицаМатериалов.КоличествоУпаковок) КАК КоличествоУпаковок,
		|	ТаблицаТоваров.КоличествоНаборов * СУММА(ТаблицаМатериалов.Количество) КАК Количество
		|ИЗ
		|	ТаблицаТоваров КАК ТаблицаТоваров
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК НоменклатураНаборов
		|		ПО ТаблицаТоваров.НоменклатураНабора = НоменклатураНаборов.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаМатериалов КАК ТаблицаМатериалов
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК НоменклатураМатериалов
		|			ПО ТаблицаМатериалов.Номенклатура = НоменклатураМатериалов.Ссылка
		|		ПО ТаблицаТоваров.НоменклатураНабора = ТаблицаМатериалов.НоменклатураНабора
		|		И ТаблицаТоваров.ХарактеристикаНабора = ТаблицаМатериалов.ХарактеристикаНабора
		|		И ТаблицаТоваров.УпаковкаНабора = ТаблицаМатериалов.УпаковкаНабора
		|		И НоменклатураНаборов.ОсобенностьУчета = НоменклатураМатериалов.ОсобенностьУчета
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаТоваров.НомерСтрокиНабора,
		|	ТаблицаМатериалов.НоменклатураНабора,
		|	ТаблицаМатериалов.ХарактеристикаНабора,
		|	ТаблицаМатериалов.УпаковкаНабора,
		...
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("ТаблицаТоваров",    ТаблицаТоваров);
		Запрос.УстановитьПараметр("ТаблицаМатериалов", МатериалыПоСпецификациям);
		Запрос.Текст = ТекстЗапроса;
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
...
Процедура ОбработкаЗаполненияДокумента(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РазрешениеНаОтгрузкуИСМП") Тогда
		ИнтеграцияИСМПУТ.ОбработкаЗаполненияДокументаРазрешениеНаОтгрузкуИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПеремещениеМеждуМОДИСМП") Тогда
		ИнтеграцияИСМПУТ.ОбработкаЗаполненияДокументаПеремещениеМеждуМОДИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	КонецЕсли;
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") Тогда
		Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаказНаЭмиссиюКодовМаркировкиСУЗ")
			...
			Или ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ОтчетИСМП")
			Или ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РазрешениеНаОтгрузкуИСМП")
...
Процедура ИспользуетсяПереработкаНаСтороне(Используется) Экспорт
	...
	Используется = ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне")
