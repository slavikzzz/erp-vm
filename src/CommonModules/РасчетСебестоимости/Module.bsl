...
Процедура РассчитатьВсе(ПараметрыЗапуска, ПараметрыРасчета = Неопределено, ПараметрыОтладки = Неопределено) Экспорт
	...
	Если СхемаРасчета.Количество() = 0 Тогда
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'За период ""%1"" по ""%2"" расчет партий версии 2.2 не требуется.';
				|en = 'Lot calculation of version 2.2 for the period from ""%1"" to ""%2"" is not required.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		ЗаписьЖурналаРегистрации(
			РасчетСебестоимостиПрикладныеАлгоритмы.ИмяСобытияЖурналаРегистрации(
				...
				НСтр("ru = 'Расчет не требуется';
					|en = 'Calculation not required'", ОбщегоНазначения.КодОсновногоЯзыка())),
	...
	ИначеЕсли НачалоМесяца(СхемаРасчета[0].Дата) < ДатаПереходаНаПартионныйУчетВерсии22 Тогда
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'За период ""%1"" по ""%2"" расчет партий версии 2.2 невозможен - расчет партий версии 2.1 за предыдущие периоды выполнен с ошибками.';
				|en = 'Cannot execute lot calculation of version 2.2 for the period from ""%1"" to ""%2"" - errors occurred when executing lot calculation of version 2.1 for the previous periods.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			РасчетСебестоимостиПротоколРасчета.ПредставлениеПериодаРасчета(ПараметрыЗапуска.Дата),
			РасчетСебестоимостиПрикладныеАлгоритмы.ПредставлениеОрганизаций(ПараметрыЗапуска.МассивОрганизаций, ", ", Истина));
		ЗаписьЖурналаРегистрации(
			РасчетСебестоимостиПрикладныеАлгоритмы.ИмяСобытияЖурналаРегистрации(
				Новый Структура("ЗапущенРасчетПартий", Истина),
				НСтр("ru = 'Расчет невозможен';
					|en = 'Cannot calculate'", ОбщегоНазначения.КодОсновногоЯзыка())),
	...
	Для Каждого СтрокаСхемыРасчета Из СхемаРасчета Цикл
		...
		РасчетСебестоимостиЛокализация.РаспределениеТрудозатрат(ПараметрыРасчета);
		...
		// Этапы распределения материальных затрат в локализованной версии.
		...
		РасчетСебестоимостиЛокализация.РаспределениеТрудозатратНаВыпуск(ПараметрыРасчета);
...
//		// - ВТСтоимостьПартийТоваров
		...
		// Этап 14
		// Формирует временные таблицы:
		// - ВтИсходныеДанныеУзловТранзитаРасходовМеждуОВЗ
		// - ВтТранзитРасходовМеждуОВЗ
		...
		// Этап 15.1
		// Формирует временные таблицы:
		// - ПрямыеПроизводственныеРасходы (используется при расчете с/с)
		...
		РасчетСебестоимостиРешениеСЛУ.ПодготовкаДанныхДляРешенияСЛУПостатейныеРасходыУчетНДС(ПараметрыРасчета);
		...
		// Этап расчета себестоимости:
		//   - для расчета постатейных производственных расходов
		//   - для расчета постоянной и временной разницы
		// Заполняет временные таблицы:
		// - ВТСтоимостьПартийТоваров
		РасчетСебестоимостиЛокализация.ВыполнитьРегламентнуюОперацию(ПараметрыРасчета);
...
Процедура РассчитатьПредварительнуюСебестоимость(Период = Неопределено, ПараметрыРасчета = Неопределено, ПараметрыОтладки = Неопределено) Экспорт
	...
	Если СтатьиКРаспределению.Количество() > 0 Тогда // нет данных для распределения
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Расчет предварительной себестоимости';
					|en = 'Calculate preliminary cost'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция РассчитатьФоновымЗаданием(Период, Организация = Неопределено) Экспорт
	...
	НаименованиеЗадания = НСтр("ru = 'Расчет партий и себестоимости';
								|en = 'Lot and cost calculation'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Если НЕ ЗначениеЗаполнено(ДанныеДвижения.Регистратор) Тогда
			...
			ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не заполнено свойство Регистратор в данных для формирования движений по регистру ""%1""';
					|en = 'Recorder property in data for generating records of the ""%1"" register is not filled in'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ТаблицаПриемник);
		Иначе
...
Функция ВходящиеДанныеМеханизма(ВходящиеДанные = Неопределено, ТолькоТребующиеПерерасчета = Ложь) Экспорт
	...
	Если НЕ ТолькоТребующиеПерерасчета Тогда 
		...
		//-- Устарело_Переработка24
		ВходящиеДанные.Вставить(Метаданные.Документы.ЗаказПереработчику2_5, Значение);
		//++ Устарело_Переработка24
		ВходящиеДанные.Вставить(Метаданные.Документы.ОтчетПереработчика, Значение);
		//-- Устарело_Переработка24
		ВходящиеДанные.Вставить(Метаданные.Документы.ОтчетПереработчика2_5, Значение);
		//++ Устарело_Переработка24
		...
		//-- Устарело_Переработка24
		ВходящиеДанные.Вставить(Метаданные.Документы.ЗаказДавальца2_5, Значение);
		//-- НЕ УТКА
		...
		//++ НЕ УТ
		ВходящиеДанные.Вставить(Метаданные.Справочники.ПартииПроизводства, Значение);
	...
	ВходящиеДанные.Вставить(Метаданные.Документы.ЗаказНаПроизводство2_2, Значение);
	//-- НЕ УТКА
...
Функция ЭтапыСРаспределениемПартий() Экспорт
	...
	//++ НЕ УТ
	РасчетСебестоимостиМатериальныеЗатраты.ДополнитьЭтапыСРаспределениемПартий(СписокЭтапов);
	РасчетСебестоимостиТрудозатраты.ДополнитьЭтапыСРаспределениемПартий(СписокЭтапов);
	//-- НЕ УТ
...
Функция ТекстЗапросаДляРасчетаЭтапа(ИмяЭтапа, ПараметрыРасчета = Неопределено) Экспорт
	...
	//++ НЕ УТ
	РасчетСебестоимостиМатериальныеЗатраты.ТекстЗапросаДляРасчетаЭтапа(ИмяЭтапа, ПараметрыРасчета, ТекстЗапроса);
	РасчетСебестоимостиТрудозатраты.ТекстЗапросаДляРасчетаЭтапа(ИмяЭтапа, ПараметрыРасчета, ТекстЗапроса);
	//-- НЕ УТ
