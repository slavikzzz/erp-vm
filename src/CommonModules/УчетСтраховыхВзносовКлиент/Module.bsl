...
Функция ФормаРасшифровкиРегламентированногоОтчета(ИДОтчета, ИДРедакцииОтчета, ИДИменПоказателей, ПараметрыОтчета) Экспорт
	...
	Если ИДОтчета = "РегламентированныйОтчетРасчетПоСтраховымВзносам" Тогда
		...
		ИначеЕсли Год = 2023 Тогда	
			...
			ДополнитьПараметрыОтчетаРСВза2023год(ПараметрыОтчета, ИмяПоказателя, ИДРедакцииОтчета = "ФормаОтчета2023Кв3");
		ИначеЕсли Год = 2025 Тогда	
			ИмяРасчета = "РасчетПоказателей_РСВ_2025Кв1";
			ДополнитьПараметрыОтчетаРСВза2025год(ПараметрыОтчета, ИмяПоказателя);
		КонецЕсли;
	...
	Если Раздел = "21" И НомерКолонки > "03" Тогда	
		Если НомерСтроки = "001" Или НомерСтроки = "002" Или НомерСтроки = "004" Тогда
			...
			НСтр("ru = 'Подраздел 2.1, строка %1, графа %2';
				|en = 'Subsection 2.1, line %1, column %2'"),
...
Процедура ДополнитьПараметрыОтчетаРСВза2025год(ПараметрыОтчета, ИмяПоказателя)

	ИмяНабораДанных = "";
	ПараметрыОтчета.Вставить("ИмяСКД",ИмяНабораДанных);
	ПараметрыОтчета.Вставить("ВыделятьМеждународныеДоговоры", Истина);
	...
	ИначеЕсли Раздел = "0013" Тогда	
		...
		КонецЕсли;
	ИначеЕсли Раздел = "0014" Тогда	
		ОписаниеРаздела = "Подраздел 4 Раздела 1";
		Если НомерСтроки = "061" Или НомерСтроки = "062" Или НомерСтроки = "071" Или НомерСтроки = "072" Или НомерСтроки = "081" Или НомерСтроки = "082" Тогда
			ИмяНабораДанных = "Подраздел4Взносы2023"
		ИначеЕсли НомерСтроки = "030" Или НомерСтроки = "040" Или НомерСтроки = "045" Или НомерСтроки = "051" Или НомерСтроки = "052" Тогда
			ИмяНабораДанных = "Подраздел4Доходы2023"
		ИначеЕсли НомерСтроки = "010" Тогда
			ИмяНабораДанных = "Подраздел4ЧисленностьЗастрахованных"
		КонецЕсли;
...
Процедура ДополнитьПараметрыОтчетаРСВза2023год(ПараметрыОтчета, ИмяПоказателя, ВыделятьМеждународныеДоговоры)

	ИмяНабораДанных = "";
	ПараметрыОтчета.Вставить("ИмяСКД",ИмяНабораДанных);
	ПараметрыОтчета.Вставить("ВыделятьМеждународныеДоговоры", ВыделятьМеждународныеДоговоры);
	...
	ИначеЕсли Раздел = "0013" Тогда	
		...
		ИначеЕсли НомерСтроки = "020" Или НомерСтроки = "030" Тогда
			ИмяНабораДанных = "Подраздел3Доходы2023"
		КонецЕсли;
	ИначеЕсли ВыделятьМеждународныеДоговоры И Раздел = "0014" Тогда	
		ОписаниеРаздела = "Подраздел 4 Раздела 1";
		Если НомерСтроки = "061" Или НомерСтроки = "062" Или НомерСтроки = "071" Или НомерСтроки = "072" Или НомерСтроки = "081" Или НомерСтроки = "082" Тогда
			ИмяНабораДанных = "Подраздел4Взносы2023"
		ИначеЕсли НомерСтроки = "030" Или НомерСтроки = "040" Или НомерСтроки = "045" Или НомерСтроки = "051" Или НомерСтроки = "052" Тогда
		...
		Если НомерСтроки = "ВЭД" Тогда
			Если НомерКолонки > "08" Тогда
				...
				НСтр("ru = 'Табл. 1.1, строка основного ВЭД, графа %1';
					|en = 'Tabl. 1.1, the main FEACN row, column %1'"),
				Формат(Число(НомерКолонки), "ЧГ="));
			КонецЕсли;
