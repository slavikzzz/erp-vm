...
Функция КраткоеПредставлениеРазмера(Знач Размер) Экспорт
	...
	ИначеЕсли Размер < 1024 * 10 Тогда // < 10 Кб.
		Возврат СтрШаблон(НСтр("ru = '%1 Кб';
								|en = '%1 KB'"), Формат(РазмерВКб(Размер), "ЧГ=0"));
	...
	ИначеЕсли Размер < 1024 * 1024 * 10 Тогда // < 10 Мб.
		Возврат СтрШаблон(НСтр("ru = '%1 Мб';
								|en = '%1 MB'"), Формат(РазмерВМб(Размер), "ЧГ=0"));
	...
	Если Размер > МаксРазмерФайла Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Размер файла ""%1"" (%2) превышает максимально допустимый (%3).';
				|en = 'The size of file ""%1"" (%2) exceeds the maximum allowable size (%3).'"),
			ИмяФайла,
			КраткоеПредставлениеРазмера(Размер),
...
Функция СообщениеОбОтсутствииПредпросмотра(ЗаголовокСообщенияВОбластиПредпросмотра) Экспорт
	...
	Возврат СообщениеВПредпросмотр(
		НСтр("ru = 'Данные предпросмотра отсутствуют.';
			|en = 'Preview data is missing.'"), ЗаголовокСообщенияВОбластиПредпросмотра);
...
Функция СтрокаСообщенияОНедопустимостиПодписанияЗашифрованногоФайла(ФайлСсылка = Неопределено) Экспорт
	...
	Иначе
		Возврат СтрШаблон(НСтр("ru = 'Нельзя подписать зашифрованный файл: %1.';
								|en = 'Cannot sign the file %1 because it is encrypted.'"), ФайлСсылка);
	КонецЕсли;
	...
	Если ДанныеСтроки.Зашифрован Тогда
		...
		ДанныеСтроки.Статус =
			НСтр("ru = 'Проверка подписи зашифрованного файла доступна только на стороне 1С:Документооборот';
				|en = 'You can check the signature of the encrypted file only in 1C:Document Management'");
...
Функция СтрокаСообщенияОНедопустимостиПодписанияЗанятогоФайла(ФайлСсылка = Неопределено) Экспорт
	...
	Иначе
		Возврат СтрШаблон(НСтр("ru = 'Нельзя подписать занятый файл: %1.';
								|en = 'Cannot sign the file %1 because it is locked.'"), ФайлСсылка);
	КонецЕсли;
