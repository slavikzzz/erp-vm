...
Процедура ФормированиеСообщенийПоОповещениямКлиентов() Экспорт
	...
	ТекстЗаписи = НСтр("ru = 'Начало формирования сообщений по очереди оповещений для клиентов';
						|en = 'Start generating messages according to the customer notification queue'",
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ТекстЗаписи = НСтр("ru = 'Ошибка при формировании сообщений по оповещениям клиентов';
								|en = 'An error occurred while generating messages of customer notifications'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + " : ";
	...
	ТекстЗаписи = НСтр("ru = 'Окончание формирования сообщений по рассылкам клиентов';
						|en = 'End generating customer mailout messages'",
	                   ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ВыполнитьЗаписьЖурналаРегистрации(ТекстЗаписи ,УровеньЖурналаРегистрации.Информация);
...
Процедура ФормированиеСообщенийПоРассылкамКлиентам() Экспорт
	...
	ТекстЗаписи = НСтр("ru = 'Начало формирования сообщений по рассылкам клиентов';
						|en = 'Start generating messages for customer mailouts'",
	                   ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ВыполнитьЗаписьЖурналаРегистрации(ТекстЗаписи ,УровеньЖурналаРегистрации.Информация);
	...
	Пока ВыборкаРассылки.Следующий() Цикл
		...
		Исключение
			...
			ТекстЗаписи = НСтр("ru = 'Ошибка при формировании сообщений по рассылкам клиентов';
								|en = 'An error occurred while generating messages of customer mailouts'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + " : ";
			ТекстЗаписи = ТекстЗаписи + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВыполнитьЗаписьЖурналаРегистрации(ТекстЗаписи ,УровеньЖурналаРегистрации.Ошибка);
			Возврат;
	...
	ТекстЗаписи = НСтр("ru = 'Окончание формирования сообщений по рассылкам клиентов. Обработано рассылок %1.';
						|en = 'End generating customer mailout messages. Processed mailouts: %1.'",
	                   ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура УдалениеВзаимодействийПоРассылкамИОповещениям() Экспорт
	...
	ТекстЗаписи = НСтр("ru = 'Начало установки пометки удаления для взаимодействий по рассылке клиентам';
						|en = 'Start setting a deletion mark for interactions on customer mailout'",
	                   ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ВыполнитьЗаписьЖурналаРегистрации(ТекстЗаписи ,УровеньЖурналаРегистрации.Информация);
...
Функция ТекстПоясненияКонтактноеЛицоОбъектаОповещения() Экспорт
	...
	Возврат НСтр("ru = 'Когда это возможно, оповещение будет отправлено контактному лицу, указанному в объекте, являющемся
	                   ...
	                   |en = 'When it is possible, a notification will be sent to a contact person specified in
	                   |the notification source. For example, a notification of the changed ""Sales order"" state
	                   |will be sent to the contact person specified in the document.'");
...
Функция ТекстКомуБудутОтправленыСообщения(Данные, Принудительная = Ложь) Экспорт
	...
	Если Данные.ОтправлятьКонтактномуЛицуОбъектаОповещения Тогда
		...
		ТекстАдресаты = ТекстАдресаты + НСтр("ru = 'контактное лицо объекта оповещения';
											|en = 'contact person of a notification object'");
	КонецЕсли;
	...
	Если Принудительная Тогда
		...
		Если Данные.ОтравлятьКонтактнымЛицаРоли Тогда
			...
			ТекстКЛ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'контактные лица роли ""%1""';
																					|en = 'contact persons of the ""%1"" role'"),
...
Процедура АнализПросроченнаяЗадолженность(ВидОповещения)
	...
	Если Выборка.Принудительная Тогда
		...
		|	
		|	ВЫБРАТЬ
		|		РасчетыСКлиентамиПоСрокамОстатки.АналитикаУчетаПоПартнерам.Партнер КАК ИсточникОповещения,
		|		РасчетыСКлиентамиПоСрокамОстатки.ОбъектРасчетов КАК ОбъектРасчетов,
		|		РасчетыСКлиентамиПоСрокамОстатки.ДолгОстаток КАК СуммаОстаток,
		|		РасчетыСКлиентамиПоСрокамОстатки.ДолгОстаток КАК КОплатеОстаток
		|	ИЗ
		|		РегистрНакопления.РасчетыСКлиентамиПоСрокам.Остатки(, ДатаПлановогоПогашения < НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ)) КАК РасчетыСКлиентамиПоСрокамОстатки
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОчередьСобытийДляОповещенийКлиентам КАК ОчередьСобытийДляОповещенийКлиентам
		|				ПО РасчетыСКлиентамиПоСрокамОстатки.АналитикаУчетаПоПартнерам.Партнер = ОчередьСобытийДляОповещенийКлиентам.ИсточникОповещения
		|					И (ОчередьСобытийДляОповещенийКлиентам.ВидОповещения = &ВидОповещения)
		|	ГДЕ
		...
		|	
		|	ВЫБРАТЬ
		|		РасчетыСКлиентамиПланОплатОстатки.АналитикаУчетаПоПартнерам.Партнер КАК ИсточникОповещения,
		|		РасчетыСКлиентамиПланОплатОстатки.ОбъектРасчетов КАК ОбъектРасчетов,
		|		РасчетыСКлиентамиПланОплатОстатки.КОплатеОстаток КАК СуммаОстаток,
		|		РасчетыСКлиентамиПланОплатОстатки.КОплатеОстаток КАК КОплатеОстаток
		|	ИЗ
		|		РегистрНакопления.РасчетыСКлиентамиПланОплат.Остатки(, ДатаПлановогоПогашения < НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ)) КАК РасчетыСКлиентамиПланОплатОстатки
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОчередьСобытийДляОповещенийКлиентам КАК ОчередьСобытийДляОповещенийКлиентам
		|				ПО РасчетыСКлиентамиПланОплатОстатки.АналитикаУчетаПоПартнерам.Партнер = ОчередьСобытийДляОповещенийКлиентам.ИсточникОповещения
		|					И (ОчередьСобытийДляОповещенийКлиентам.ВидОповещения = &ВидОповещения)
		|	ГДЕ
		|		&НоваяАрхитектураВзаиморасчетов
		|		И ОчередьСобытийДляОповещенийКлиентам.ИсточникОповещения ЕСТЬ NULL 
		...
		|ИМЕЮЩИЕ
		|	СУММА(ВложенныйЗапрос.КОплатеОстаток) > 0"
	...
	Иначе
		...
		|					И (ПодпискиНаРассылкиИОповещенияКлиентам.ОтправлятьПартнеру
		|						ИЛИ ПодпискиНаРассылкиИОповещенияКлиентам.КоличествоКонтактныхЛицАдресатов > 0)
		|					И ОчередьСобытийДляОповещенийКлиентам.ИсточникОповещения ЕСТЬ NULL )) КАК РасчетыСКлиентамиПоСрокамОстатки
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|					И (ПодпискиНаРассылкиИОповещенияКлиентам.ОтправлятьПартнеру
		|						ИЛИ ПодпискиНаРассылкиИОповещенияКлиентам.КоличествоКонтактныхЛицАдресатов > 0)
		|					И ОчередьСобытийДляОповещенийКлиентам.ИсточникОповещения ЕСТЬ NULL )) КАК РасчетыСКлиентамиПланОплатОстатки
		|	) КАК ВложенныйЗапрос
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.ИсточникОповещения
		|ИМЕЮЩИЕ
...
Процедура АнализПлановыеПлатежи(ВидОповещения)
	...
	Если Выборка.Принудительная Тогда
		...
		Запрос.Текст = "
		|ВЫБРАТЬ
		|	ДанныеРасчетов.Партнер КАК ИсточникОповещения,
		|	СУММА(ДанныеРасчетов.КОплатеОстаток) КАК СуммаОстаток
		|ИЗ (ВЫБРАТЬ
		...
		|		РасчетыСКлиентамиПоСрокамОстатки.ДолгОстаток КАК КОплатеОстаток
		|	ИЗ
		|		РегистрНакопления.РасчетыСКлиентамиПоСрокам.Остатки(, ДатаПлановогоПогашения < НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(&ТекущаяДата, ДЕНЬ, 2), ДЕНЬ)) КАК РасчетыСКлиентамиПоСрокамОстатки
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОчередьСобытийДляОповещенийКлиентам КАК ОчередьСобытийДляОповещенийКлиентам
		|			ПО РасчетыСКлиентамиПоСрокамОстатки.АналитикаУчетаПоПартнерам.Партнер = ОчередьСобытийДляОповещенийКлиентам.ИсточникОповещения
		|				И (ОчередьСобытийДляОповещенийКлиентам.ВидОповещения = &ВидОповещения)
		|	ГДЕ
		...
		|		РасчетыСКлиентамиПланОплатОстатки.КОплатеОстаток КАК КОплатеОстаток
		|	ИЗ
		|		РегистрНакопления.РасчетыСКлиентамиПланОплат.Остатки(, ДатаПлановогоПогашения < НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(&ТекущаяДата, ДЕНЬ, 2), ДЕНЬ) ) КАК РасчетыСКлиентамиПланОплатОстатки
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОчередьСобытийДляОповещенийКлиентам КАК ОчередьСобытийДляОповещенийКлиентам
		|			ПО РасчетыСКлиентамиПланОплатОстатки.АналитикаУчетаПоПартнерам.Партнер = ОчередьСобытийДляОповещенийКлиентам.ИсточникОповещения
		|				И (ОчередьСобытийДляОповещенийКлиентам.ВидОповещения = &ВидОповещения)
		|	ГДЕ
		...
		|
		|ИМЕЮЩИЕ
		|	СУММА(ДанныеРасчетов.КОплатеОстаток) > 0";
	...
	Иначе
		...
		|ВЫБРАТЬ
		|	ДанныеРасчетов.Партнер КАК ИсточникОповещения,
		|	СУММА(ДанныеРасчетов.КОплатеОстаток) КАК КОплатеОстаток
		|ИЗ (ВЫБРАТЬ
		|		РасчетыСКлиентамиОстатки.АналитикаУчетаПоПартнерам.Партнер КАК Партнер,
		|		РасчетыСКлиентамиОстатки.КОплатеОстаток КАК КОплатеОстаток
		|	ИЗ
		...
		|		РегистрНакопления.РасчетыСКлиентамиПоСрокам.Остатки(
		|				,
		|				ДатаПлановогоПогашения < НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(&ТекущаяДата, ДЕНЬ, 2), ДЕНЬ)
	...
	Для каждого ВложениеПечатнаяФорма Из ВложенияПечатныеФормы Цикл 
		...
		Если ВложениеПечатнаяФорма.Статус = "ПечатнаяФорма" Тогда
			...
			Исключение
				...
				ТекстОшибки = НСтр("ru = 'Ошибка при создании внешней печатной формы. По причине:';
									|en = 'An error occurred when creating an external print form. Reason:'") + Символы.ПС
...
Функция КомментарийДляСообщенияПоОповещению(ДанныеВидаОповещения)
	...
	ТекстКомментария =  НСтр("ru = 'Создано по виду оповещения ""%1""';
							|en = 'Created by notification kind ""%1""'");
...
Функция КомментарийДляСообщенияПоРассылке(Данные)
	...
	ТекстКомментария = НСтр("ru = 'Создано по рассылке ""%1"" № %2 от %3.';
							|en = 'Created for mailout ""%1"" No. %2 dated %3.'");
...
Функция ВыполнитьЗаписьЖурналаРегистрации(ТекстЗаписи ,УровеньВажности)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Рассылки и оповещения клиентам';
			|en = 'Mailouts and notifications to customers'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		УровеньВажности, , ,
		ТекстЗаписи);
