	...
	Если НесколькоДокументов Тогда
		Для Каждого ЭлементОрганизация Из НеЗарегистрированныеОрганизации Цикл
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'Не удалось отправить документ по причине отсутствия подключения организации %1 к сервису 1С:Бизнес-сеть.';
								|en = 'Cannot send the document, because the company %1 is not connected to 1C:Business Network service.'"), ЭлементОрганизация));
		КонецЦикла;
...
Процедура ОтправитьДокументИПолучитьQRКодЧерезБизнесСетьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекстОповещения = НСтр("ru = 'Отправка выполнена.';
							|en = 'Sending is completed.'");
	ТекстПояснения  = НСтр("ru = 'Отправлен документ через сервис 1С:Бизнес-сеть.';
							|en = 'The document was sent via 1C:Business Network service.'");
