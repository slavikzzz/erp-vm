...
//
Функция АктуализироватьДанныеСервиса_СФлагамиОшибок(Сервис)
	...
	Если РезультатАвторизациии.ОшибкаАвторизацииПоЛогину
	 Или РезультатАвторизациии.ОшибкаПриПолученииТикета Тогда // ошибка определенного вида
		Возврат РезультатАвторизациии;
	ИначеЕсли Не РезультатАвторизациии.Выполнено Тогда // неклассифицированная ошибка
		РезультатАвторизациии.Вставить("Ошибка", Истина);
		Возврат РезультатАвторизациии;
	КонецЕсли;
	...
	Если НЕ Результат.Выполнено Тогда // ошибка уже во время передачи данных
		РезультатАвторизациии.Вставить("ОшибкаПриПолученииДанных", Истина);
		Возврат РезультатАвторизациии;
	КонецЕсли;
...
Функция АктуализироватьДанныеСервиса(Сервис) Экспорт
	...
	РезультатОбновленияДанных = УниверсальныйОбменСБанкамиКлиентСервер.РезультатОбновленияДанныхСервиса();
	
	РезультатАктуализации = АктуализироватьДанныеСервиса_СФлагамиОшибок(Сервис);
	
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РезультатАктуализации, "ОшибкаАвторизацииПоЛогину", Ложь) Тогда
		// Нет подключения к интернет-поддержке (например, не подходит пароль).
		РезультатОбновленияДанных.ИмяСобытия = "ОшибкаАвторизацииПоЛогину";
	ИначеЕсли ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РезультатАктуализации, "ОшибкаПриПолученииТикета", Ложь) Тогда
		// Не выдан доступ для сеанса сервером ИТС.
		РезультатОбновленияДанных.ИмяСобытия = "ОшибкаПриПолученииТикета";
	ИначеЕсли ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РезультатАктуализации, "ОшибкаПриПолученииДанных", Ложь) Тогда
		// Во время получения данных с сервера произошла ошибка.
		РезультатОбновленияДанных.ИмяСобытия = "ОшибкаПриПолученииДанных";
	ИначеЕсли ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РезультатАктуализации, "Ошибка", Ложь) Тогда
		// Неклассифицированная ошибка.
		РезультатОбновленияДанных.ИмяСобытия = "Ошибка";
	КонецЕсли;
		
	Если РезультатОбновленияДанных.ИмяСобытия <> "ДанныеАктуальны" Тогда
		РезультатОбновленияДанных.ОписаниеСобытия = РезультатАктуализации.ОписаниеОшибки;
	КонецЕсли;
...
Процедура ЗапуститьУниверсальныйОбменСБанкамиПоСервису(Сервис, АдресПолученныхТранзакций = Неопределено, Отбор = Неопределено) Экспорт
	...
	Если Сервисы.Найти(Сервис) = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Автоматическая проверка входящих транзакций для сервиса %1 не поддерживается';
										|en = 'Automatic check of incoming transactions for the %1 service is not supported'"), Сервис);
	КонецЕсли;
	...
	Если НЕ РезультатАвторизации.Выполнено Тогда
		ОписаниеОшибки = НСтр("ru = 'Ошибка авторизации на сервере обмена.';
								|en = 'An error of authorization on the exchange server.'");
...
Функция ХарактеристикиУслугБанков(Сервис, Характеристики = Неопределено, Банки = Неопределено, ВозвращатьНеактивные = Ложь) Экспорт
	...
	|	УслугиБанков.НачалоДействия КАК НачалоДействия,
	|	ВЫБОР
	|		КОГДА Банки.АктивенДо = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА УслугиБанков.КонецДействия
	|		КОГДА УслугиБанков.КонецДействия = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА Банки.АктивенДо
	|		КОГДА УслугиБанков.КонецДействия > Банки.АктивенДо
	|			ТОГДА Банки.АктивенДо
	|		ИНАЧЕ УслугиБанков.КонецДействия
	|	КОНЕЦ КАК КонецДействия
	|ИЗ
	|	Справочник.УслугиБанков КАК УслугиБанков
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.УслугиБанков.Характеристики КАК ХарактеристикиУслугБанка
	|		ПО УслугиБанков.Ссылка = ХарактеристикиУслугБанка.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.БанкиУниверсальногоОбмена КАК Банки
	|		ПО УслугиБанков.Банк = Банки.Ссылка
	|ГДЕ
	|	УслугиБанков.Сервис = &Сервис
	|	И &ВозвращатьНеактивные";
	
	Если ВозвращатьНеактивные = Истина Тогда
	
		Запрос.Текст = СтрЗаменить(ТекстЗапроса, "&ВозвращатьНеактивные", "ИСТИНА");	
	
	Иначе
	
		Запрос.Текст = СтрЗаменить(ТекстЗапроса, "&ВозвращатьНеактивные", 
		"	(Банки.АктивенДо = ДАТАВРЕМЯ(1, 1, 1)
		|			ИЛИ Банки.АктивенДо >= &ТекущаяДата)
		|	И УслугиБанков.НачалоДействия <= &ТекущаяДата
		|	И (УслугиБанков.КонецДействия = ДАТАВРЕМЯ(1, 1, 1)
		|			ИЛИ УслугиБанков.КонецДействия >= &ТекущаяДата)");
...
Функция РеквизитыБанков(Банки, Знач Поля = "") Экспорт
	...
	Результат.Колонки.Добавить("НаименованиеСокращенное", Новый ОписаниеТипов("Строка"));
	...
	|	Банки.НаименованиеСокращенное КАК НаименованиеСокращенное,
	|	Банки.ИНН КАК ИНН,
	|	Банки.КПП КАК КПП,
	|	Банки.ОГРН КАК ОГРН,
...
Функция ДатаПоследнегоОбновленияСервиса(Сервис) Экспорт
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ДанныеСервисовОбменаСБанками.ДатаОбновления КАК ДатаОбновления
	|ИЗ
	|	Справочник.ДанныеСервисовОбменаСБанками КАК ДанныеСервисовОбменаСБанками
	|ГДЕ
	|	ДанныеСервисовОбменаСБанками.Сервис = &Сервис
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДанныеСервисовОбменаСБанками.ДатаОбновления УБЫВ";
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если Выборка.Следующий() Тогда
			Результат = Выборка.ДатаОбновления;
		КонецЕсли;
	КонецЕсли;
...
Функция ХарактеристикиСервиса(Сервис) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ДанныеСервисовОбменаСБанками.Характеристики
	|ИЗ
	|	Справочник.ДанныеСервисовОбменаСБанками КАК ДанныеСервисовОбменаСБанками
	|ГДЕ
	|	ДанныеСервисовОбменаСБанками.Сервис = &Сервис";
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.Характеристики;
	КонецЕсли;
...
Функция СервисСуществует(ИмяСервиса) Экспорт
	...
	Возврат Метаданные.Перечисления.СервисыОбменаСБанками.ЗначенияПеречисления.Найти(ИмяСервиса) <> Неопределено;
...
Функция СоставВерсииКонфигурации(ВерсияКонфигурации) Экспорт
	...
	СоставВерсииКонфигурации = НовыйСоставВерсииКонфигурации();
	
	ДопустимыеСимволы = "0123456789.";
	ИмяРеквизитаСостава = "ВерсияКонфигурации";
	
	Для Позиция = 1 По СтрДлина(ВерсияКонфигурации) Цикл
		
		ТекСимвол = Сред(ВерсияКонфигурации, Позиция, 1);
		
		Если СтрНайти(ДопустимыеСимволы, ТекСимвол) = 0 Тогда
			ИмяРеквизитаСостава = "ВерсияОтраслевогоРешения";
			Продолжить;
		КонецЕсли;
		
		СоставВерсииКонфигурации[ИмяРеквизитаСостава] = СоставВерсииКонфигурации[ИмяРеквизитаСостава] + ТекСимвол;
		
	КонецЦикла;
	
	Возврат СоставВерсииКонфигурации;
...
Функция ОтправитьТранзакцииНаСервер(Сервис, Транзакции, ПараметрыПрогрессаВыполнения = Неопределено) Экспорт
	...
	Если ПараметрыПрогрессаВыполнения <> Неопределено Тогда
		...
		ДлительныеОперации.СообщитьПрогресс(1, НСтр("ru = 'Отправка транзакций...';
													|en = 'Sending transactions...'"), ПараметрыПрогрессаВыполнения);
	...
	Для каждого Транзакция Из Транзакции Цикл
		...
		Если Транзакция.Пустая() Тогда
			ОписаниеОшибки = НСтр("ru = 'Передана пустая ссылка на транзакцию.';
									|en = 'Empty reference to the transaction is sent.'");
		...
		Исключение
			...
			ОписаниеОшибки = НСтр("ru = 'Ошибка вызова предопределенного метода. Описание ошибки:
				|%1';
				|en = 'Predefined method call error. Error description:
				|%1'");
...
Функция ПротоколОбработкиОтчетностиБанком(ПредметИлиТранзакция, ТипыДокументовПротокола, ТипыТранзакцийПротокола = Неопределено) Экспорт
	...
	Иначе
		Если ТипыТранзакцийПротокола = Неопределено Тогда
			ВызватьИсключение НСтр("ru = 'Не определен тип транзакции протокола.';
									|en = 'Protocol transaction type is not defined.'");
		КонецЕсли;
		...
		Если Транзакции.Количество() = 0 Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'Не обнаружено ответных транзакций по отчету.';
											|en = 'Response transactions of the report are not found.'");
			Возврат Результат;
		КонецЕсли;
	...
	Если ФайлыТранзакции.Количество() = 0 Тогда
		...
		Результат.ОписаниеОшибки = НСтр("ru = 'Данные транзакции не расшифрованы.';
										|en = 'Transaction data is not decrypted.'");
		Возврат Результат;
	...
	Если НЕ ПротоколНайден Тогда
		...
		Результат.ОписаниеОшибки = НСтр("ru = 'В данных транзакции не найдено протоколов.';
										|en = 'Protocols are not found in the transaction data.'");
		Возврат Результат;
...
//
Функция ТранзакцииТребующиеРасшифровки(Сервисы = Неопределено, Организации = Неопределено, Транзакции = Неопределено, 
	...
	|	ТранзакцииОбмена.ДатаТранспорта КАК ДатаТранспорта
	|ИЗ
	|	РегистрСведений.СвойстваТранзакцийОбменаСБанками КАК СвойстваТранзакцийОбменаСБанками
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ТранзакцииОбменаСБанками КАК ТранзакцииОбмена
	|		ПО СвойстваТранзакцийОбменаСБанками.Транзакция = ТранзакцииОбмена.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДокументооборотыОбменаСБанками КАК Документообороты
	|		ПО (ТранзакцииОбмена.Документооборот = Документообороты.Ссылка)
	|ГДЕ
	|	СвойстваТранзакцийОбменаСБанками.ТребуетРасшифровки = ИСТИНА
	|	И ТранзакцииОбмена.ПометкаУдаления = ЛОЖЬ
	|	И Документообороты.ПометкаУдаления = ЛОЖЬ
	|	И &УсловиеПоОтветственному
	|
	|УПОРЯДОЧИТЬ ПО
	|	СвойстваТранзакцийОбменаСБанками.Организация,
	|	ТранзакцииОбмена.ДатаТранспорта";

	Запрос = Новый Запрос(ТекстЗапроса);
	
	Если Организации = Неопределено Тогда
		
		ПроверяемыеОрганизации = ОбщегоНазначенияБПВызовСервераПовтИсп.ВсеОрганизацииДанныеКоторыхДоступныПоRLS(Истина);
		
	Иначе
		
		ПроверяемыеОрганизации = Организации;
	
	КонецЕсли;
	
	Отбор = Новый Структура;
	Отбор.Вставить("Сервисы", Сервисы);
	Отбор.Вставить("Организации", ПроверяемыеОрганизации);
	Отбор.Вставить("Транзакции", Транзакции);
	
	УстановитьУсловиеИПараметрЗапросаИзОтбора(Отбор,
		"Сервисы",
		Запрос,
		"Документообороты.Сервис В (&Сервисы)");
	...
	Если ЗначениеЗаполнено(Ответственный) Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоОтветственному",
			"ЕСТЬNULL(Документообороты.Предмет.Ответственный, ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)) 
			|	В (&Ответственный, ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка))");
		Запрос.УстановитьПараметр("Ответственный", Ответственный);
		
	Иначе
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоОтветственному", "Истина");
...
Функция СоздатьТранзакцииИзДанныхЖурналаОперацийСФайлами(Параметры) Экспорт
	...
	ИмяСобытия = НСтр("ru = 'Создание архива транзакции';
						|en = 'Generate transaction archive'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если НЕ ЗначениеЗаполнено(ТипТранзакции) Тогда
		Результат.ОписаниеОшибки = НСтр("ru = 'Не определен тип транзакции.';
										|en = 'Transaction type is not defined.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если Вложения.Количество() = 0 Тогда
		ОписаниеОшибки = НСтр("ru = 'Отсутствуют вложения транзакции с идентификатором %1.';
								|en = 'Transaction attachments with %1 ID are absent.'");
	...
	// Инициализируем вывод прогресса.
	Если ПараметрыПрогрессаВыполнения <> Неопределено Тогда
		ДлительныеОперации.СообщитьПрогресс(1, НСтр("ru = 'Формирование транзакций...';
													|en = 'Generating transactions...'"), ПараметрыПрогрессаВыполнения);
		ШаблонСообщенияПрогрессаВложение = НСтр("ru = 'Обрабатывается вложение %1 для %2...';
												|en = 'Processing %1 attachment for %2...'");
		ШаблонСообщенияПрогрессаСохранениеБД = НСтр("ru = 'Сохраняем данные в базе данных для %1...';
													|en = 'Saving data in the database for %1...'");
	КонецЕсли;
...
#Область ПолучениеИД_ДокументооборотаИТранзакцииНаСервере
		...
		Если ДокументооборотыПолучателей = Неопределено Тогда
			...
			Если НЕ РезультатСозданияДокументооборота.Выполнено Тогда
				ОписаниеОшибки = НСтр("ru = 'Ошибка при создании документооборота на сервере для получателя %2:
										|%1';
										|en = 'An error occurred when generating EDI on the server for the %2 recipient:
										|%1'");
				ОписаниеОшибки = СтрШаблон(ОписаниеОшибки,
					РезультатСозданияДокументооборота.ОписаниеОшибки,
					Получатель);
				Результат.ОписаниеОшибки = ОписаниеОшибки;
				Возврат Результат;
			КонецЕсли;
		...
		Иначе
			...
			Если Документооборот = Неопределено Тогда
				ОписаниеОшибки = НСтр("ru = 'Документооборот для получателя %1 не определен.';
										|en = 'EDI for %1 receiver is not defined.'");
				ОписаниеОшибки = СтрШаблон(ОписаниеОшибки, Получатель);
				Результат.ОписаниеОшибки = ОписаниеОшибки;
				Возврат Результат;
			КонецЕсли;
		...
		Если НЕ РезультатСозданияТранзакции.Выполнено Тогда
			...
			ОписаниеОшибки = НСтр("ru = 'Ошибка при создании транзакции на сервере для получателя %2:
									|%1';
									|en = 'An error occurred when generating a transaction on the server for the %2 recipient:
									|%1'");
			ОписаниеОшибки = СтрШаблон(ОписаниеОшибки,
				РезультатСозданияТранзакции.ОписаниеОшибки,
				Получатель);
...
#Область ОбработкаВложений
			...
			Исключение
				...
				ОписаниеОшибки = СтрШаблон(
					НСтр("ru = 'Произошла ошибка при добавлении данных в архив:
					      |%1.';
					      |en = 'An error occurred upon adding data to the archive:
					      |%1.'"),
					ОписаниеОшибки);
				Результат.ОписаниеОшибки = ОписаниеОшибки;
				Возврат Результат;
		...
		Если ВложенияКонтейнера.Количество() = 0 Тогда
			...
			ОписаниеОшибки = НСтр("ru = 'Нет файлов для помещения в контейнер.';
									|en = 'No files to put in the container.'");
			ЗаписатьСобытиеВЖурнал(ОписаниеОшибки, ИмяСобытия);
...
#Область ФормированиеОписанияКонтейнера
		...
		Если ПустаяСтрока(ПолноеИмяФайлаОписанияВСоставеПакета) Тогда
			...
			Шаблон = НСтр("ru = 'Ошибка создания файла описания пакета для получателя %1.';
							|en = 'An error occurred when generating a batch details file for the %1 recipient.'");
			ОписаниеОшибки = СтрШаблон(Шаблон, Получатель);
			ЗаписатьСобытиеВЖурнал(ОписаниеОшибки, ИмяСобытия);
...
#КонецОбласти
		...
		УдалитьВременныйФайл(ПолноеИмяФайлаКонтейнера);
		УдалитьВременныйФайл(КаталогВременныхФайлов);
		
#Область СозданиеОбъектовБД
		ИмяСобытия = НСтр("ru = 'Запись транзакции и документооборота';
							|en = 'Transaction and EDI record'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Для каждого ПолучательСтроки Из ПараметрыОписанияТранзакции.Получатели Цикл
			СтрокаПолучатель = ТранзакцияОбъект.Банки.Добавить();
			СтрокаПолучатель.Банк = ПолучательСтроки;
		КонецЦикла;
		...
		Если НЕ РезультатСохранения Тогда
			...
			ОписаниеОшибки = НСтр("ru = 'Ошибка при сохранении данных транспортного контейнера в базе данных';
									|en = 'An error occurred upon saving transport container data in the database'");
			ЗаписатьСобытиеВЖурнал(ОписаниеОшибки, ИмяСобытия);
			Результат.ОписаниеОшибки = ОписаниеОшибки;
			Возврат Результат;
		...
		Для каждого Вложение Из ВложенияСИсходнымиДанными Цикл
			...
			Если НЕ РезультатПомещения Тогда
				ОписаниеОшибки = НСтр("ru = 'Ошибка сохранения в базе данных файла %1.';
										|en = 'Failed to save the %1 file to the database.'");
		...
		Исключение
			...
			Результат.ОписаниеОшибки = НСтр("ru = 'Ошибка записи в БД:
											|%1';
											|en = 'An error occurred upon recording to the database:
											|%1'");
			Результат.ОписаниеОшибки = СтрШаблон(Результат.ОписаниеОшибки, ОписаниеОшибки);
			Возврат Результат;
		КонецПопытки;
...
Функция ПолучитьСписокТиповДокументовТранзакции(Сервис, ТипТранзакции) Экспорт
	...
	Если СтрокаТранзакции = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Тип тразакции не обнаружен в описании.';
								|en = 'Transaction type is not found in the details.'");
	КонецЕсли;
...
Функция ПоместитьВЖурналОперацияСФайлами(Параметры) Экспорт
	...
	Если НЕ Параметры.РежимРасшифровки И Параметры.БанкиПолучатели.Количество() = 0 Тогда
		...
		ОписаниеОшибки = НСтр("ru = 'Не указаны банки-получатели транзакции.';
								|en = 'Receiving banks of the transaction are not specified.'");
	...
	Если НЕ ЗначениеЗаполнено(Параметры.ТипДокумента) Тогда
		...
		Комментарий = НСтр("ru = 'Не заполнен тип документа для файла ""%1"".';
							|en = 'Document type for the ""%1"" file is not filled in.'");
	...
	Исключение
		...
		Комментарий = НСтр("ru = 'Тип документа %1 не принадлежит сервису %2.';
							|en = '%1 document type does not belong to %2 service.'");
	...
	Если НЕ ПроверкаИмениФайла(ИсходноеИмяФайла) Тогда
		...
		Комментарий = НСтр("ru = 'В имени файла ""%1"" обнаружены недопустимые символы.';
							|en = 'Invalid characters are detected in the ""%1"" file name.'");
		Комментарий = СтрШаблон(Комментарий, ИсходноеИмяФайла);
		ЗаписатьСобытиеВЖурнал(Комментарий, , Метаданные.РегистрыСведений.ЖурналОперацийСФайламиОбменаСБанками);
		Результат.ОписаниеОшибки = Комментарий;
		Возврат Результат;
...
Функция УдалитьЗаписиЖурналаОперацийСФайламиПоИдентификатору(Идентификатор) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Удаление записей журнала операций с файлами';
				|en = 'Delete file operation log records'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		...
		Если НЕ МенеджерЗаписи.Выбран() Тогда
			...
			ТекстыОшибок.Добавить(СтрШаблон(
				НСтр("ru = 'Не найдены данные файла %1 во временном хранилище транзакций обмена с банками.';
					|en = '%1 file data is not found in the temporary storage of bank exchange transactions.'"),
...
Функция ПодготовитьАрхивыВЖурналеОперацийСФайлами(Идентификатор, ОтпечатокСертификата, ПараметрыПрогрессаВыполнения) Экспорт
	...
	ДлительныеОперации.СообщитьПрогресс(1, НСтр("ru = 'Подписание архивов...';
												|en = 'Signing archives...'"), ПараметрыПрогрессаВыполнения);
	КоличествоДействийПрогресса = ИдентификаторыСоздаваемыхАрхивов.Количество();
	ВыполненоДействийПрогресса = 0;
	ШаблонСообщенияПрогресса = НСтр("ru = 'Подписан архив %1';
									|en = '%1 archive is signed'");
		...
		Для Каждого ФайлАрхива Из ЖурналОперацийСФайлами Цикл
			...
			Если ФайлАрхива.Сжать Тогда
				...
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Некорректный вызов ПодготовитьАрхивыВЖурналеОперацийСФайлами() для набора файлов с установленным флагом ""Сжать"".
                                        |Идентификатор архива %1';
                                        |en = 'Incorrect call ПодготовитьАрхивыВЖурналеОперацийСФайлами() for a set of files with the selected ""Compress"" check box.
                                        |Archive ID %1'"), ИдентификаторАрхива);
				Прервать;
			...
			ИначеЕсли Зашифровать <> ФайлАрхива.Зашифровать Тогда
				...
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Некорректный вызов ПодготовитьАрхивыВЖурналеОперацийСФайлами() для набора файлов, имеющих различные значения флага ""Зашифровать"".
                                        |Идентификатор архива %1';
                                        |en = 'Incorrect call ПодготовитьАрхивыВЖурналеОперацийСФайлами() for a set of files with different values of the ""Encrypt"" check box.
                                        |Archive ID %1'"), ИдентификаторАрхива);
				Прервать;
			...
			Если Не РезультатПомещения.Выполнено Тогда
				...
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Ошибка при записи архива %1 во временное хранилище транзакций обмена с банками: %2';
						|en = 'An error occurred upon recording the %1 archive to the temporary storage of bank exchange transactions: %2'"),
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Ошибка при подготовке архива %1 во временном хранилище транзакций обмена с банками: %2';
					|en = 'An error occurred upon preparing the %1 archive in the temporary storage of bank exchange transactions: %2'"),
	...
	Если Не РезультатПодготовки.Выполнено Тогда
		...
		Комментарий = СтрШаблон(
			НСтр("ru = 'Ошибки при подготовке архивов во временном хранилище транзакций с Идентификатором=%1
						|%2';
						|en = 'Errors occurred upon preparing archives in the temporary storage of transactions with ID=%1
						|%2'"),
			Идентификатор, РезультатПодготовки.ОписаниеОшибки);

		ИмяСобытия = НСтр("ru = 'ПодготовитьАрхивыВЖурналеОперацийСФайлами';
							|en = 'ПодготовитьАрхивыВЖурналеОперацийСФайлами'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ПодписатьИЗашифроватьДанныеВЖурналеОперацийСФайлами(
	...
	Если НЕ ДоступнаОблачнаяКриптография() Тогда
		Результат.ОписаниеОшибки = НСтр("ru = 'Электронная подпись в модели сервиса не доступна.';
										|en = 'Electronic signature in SaaS is unavailable.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ПараметрыПроверкиСертификата <> Неопределено Тогда
		...
		Если ДанныеСертификата = Неопределено Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'Облачный сертификат не найден в хранилище.';
											|en = 'Cloud certificate is not found in the storage.'");
			Возврат Результат;
		КонецЕсли;
		
		Сертификат = Новый СертификатКриптографии(ДанныеСертификата.Сертификат);
		РезультатПроверки = УниверсальныйОбменСБанкамиКлиентСервер.СертификатСоответствуетОтбору(Сертификат, ПараметрыПроверкиСертификата);
		...
		Если НЕ РезультатПроверки.ПризнакСоответствия Тогда
			...
			ЗаписатьСобытиеВЖурнал(ОписаниеОшибки,
				НСтр("ru = 'Сертификаты подписей';
					|en = 'Signature certificates'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
			Результат.ОписаниеОшибки = ОписаниеОшибки;
			Возврат Результат;
		КонецЕсли;
	...
	Если ПараметрыПрогрессаВыполнения <> Неопределено Тогда
		...
		ДлительныеОперации.СообщитьПрогресс(51, НСтр("ru = 'Подписание файлов...';
													|en = 'Sign files...'"), ПараметрыПрогрессаВыполнения);
	...
	Если ТекстыОшибок.Количество() > 0 Тогда
		...
		ИмяСобытия = НСтр("ru = 'ПодписатьИЗашифроватьДанныеВЖурналеОперацийСФайлами';
							|en = 'ПодписатьИЗашифроватьДанныеВЖурналеОперацийСФайлами'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
				
		Комментарий = СтрШаблон(
			НСтр("ru = 'Ошибки при подписании и шифровании данных во временном хранилище транзакций с Идентификатором=%1:%2';
				|en = 'Errors occurred upon signing and encrypting data in the temporary storage of transactions with ID=%1:%2'"),
			Идентификатор,
...
Функция РасшифроватьИПроверитьПодписьВЖурналеОперацийСФайлами(Идентификатор, Транзакция) Экспорт
		...
		Если МенеджерЗаписи.Подписать Тогда
			Если РезультатОперации.ПодписьНеДействительна Тогда
				ТекстПредупреждения = НСтр("ru = 'В файле %1 подпись не действительна.';
											|en = '%1 file signature is invalid.'");
			...
			Если РезультатОперации.НеизвестныйСертификатПодписи Тогда
				ТекстПредупреждения = НСтр("ru = 'В файле %1 подпись выполнена неизвестным сертификатом.';
											|en = '%1 file is signed by an unknown certificate.'");
				ТекстПредупреждения = СтрШаблон(ТекстПредупреждения, Выборка.ИсходноеИмяФайла);
				ТекстыПредупреждений.Добавить(ТекстПредупреждения);
	...
	Если ТекстыОшибок.Количество() > 0 Тогда
		...
		ИмяСобытия = НСтр("ru = 'ПодписатьИЗашифроватьДанныеВоВременномХранилищеТранзакций';
							|en = 'ПодписатьИЗашифроватьДанныеВоВременномХранилищеТранзакций'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Комментарий = СтрШаблон(
			НСтр("ru = 'Ошибки при подписании и шифровании данных во временном хранилище транзакций с Идентификатором=%1:%2';
				|en = 'Errors occurred upon signing and encrypting data in the temporary storage of transactions with ID=%1:%2'"),
...
Функция ПолучитьТранзакцииДокументооборота(Документооборот,
	...
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
	                      ...
	                      |	И (&ВсеТипыТранзакций
	                      |			ИЛИ Транзакция.ТипТранзакции В (&ТипТранзакции))
	...
	Запрос.УстановитьПараметр("ВсеТипыТранзакций", НЕ ЗначениеЗаполнено(ТипТранзакции));
...
Процедура ДобавитьГруппуСобытий(ДанныеГрупп, Группа) Экспорт
	...
	Если Группа.ПодсчетКоличестваНовыхСобытий
		...
		ВызватьИсключение(НСтр("ru = 'Не заполнен шаблон ненулевого количества событий.';
								|en = 'Template of non-zero number of events is not filled in.'"));
	КонецЕсли;
...
Функция ГруппыСобытийСервиса(Сервис) Экспорт
	...
	УниверсальныйОбменСБанкамиСлужебныйПереопределяемый.ГруппыСобытийСервиса(Сервис, Результат);
	Возврат Результат;
...
Функция ТекущееСостояниеДокументооборота(Знач Документооборот, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Исключение
		...
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Получение текущего состояния отправки, критических ошибок';
				|en = 'Receive current state of sending and critical errors'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		НСтр("ru = 'Сервер обмена в рамках технологии 1С:БанкКоннект';
			|en = 'Exchange server under 1C:BankConnect technology'"));
...
Процедура СброситьВерсиюУслугБанка(Сервис = Неопределено) Экспорт
	...
	Иначе
		Услуги = Справочники.УслугиБанков.Выбрать();
	КонецЕсли;
...
// Процедура устанавливает значение реквизита "Версия" справочника "БанкиУниверсальногоОбмена" 
// и справочника "Услуги банков" равным 0.
// Может быть использована для принудительного обновления данных в этом справочнике при
// следующей синхронизации с сервером обмена.
...
Процедура УдалитьВременныйФайл(ИмяФайла, Маска = Неопределено) Экспорт
	...
	Исключение
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				ИмяФайла,
				Представление);
		ЗаписатьСобытиеВЖурнал(Комментарий, , ,
...
Функция ОбработатьРезультатРасшифровкиТранзакции(Транзакция, РезультатРасшифровки) Экспорт
	...
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Ошибка в переопределяемом методе ПриРасшифровкеТранзакции.
			|%1';
			|en = 'An error in overridable method ПриРасшифровкеТранзакции.
			|%1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписатьСобытиеВЖурнал(ТекстСообщения,
			НСтр("ru = 'ПриРасшифровкеТранзакции';
				|en = 'ПриРасшифровкеТранзакции'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Транзакция);
		Возврат Ложь;
	КонецПопытки;
...
Функция ПоместитьДокументыКонтейнераВЖурналОперацийСФайлами(Транзакция, ТранспортнаяИнформация, КаталогСДаннымиКонтейнера)
	...
	Сервис = ОпределитьСервисПоИдентификаторуНаСервере(ТранспортнаяИнформация.ИдентификаторСервиса);
	...
	Для каждого Документ Из ЗначениеВКоллекцию(ТранспортнаяИнформация.Документы.Документ) Цикл
		...
		Если НайденныеФайлы.Количество() = 0 Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'В контейнере отсутсвует файл %1.';
											|en = 'The container does not contain the %1 file.'");
			Результат.ОписаниеОшибки = СтрШаблон(Результат.ОписаниеОшибки, ИмяФайла);
			ЗаписатьСобытиеВЖурнал(Результат.ОписаниеОшибки,
				НСтр("ru = 'Транспортный контейнер';
					|en = 'Transport container'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
			Возврат Результат;
		КонецЕсли;
		...
		Если СвойстваДокумента.Подписать Тогда
			...
			Если НайденныеФайлы.Количество() = 0 Тогда
				Результат.ОписаниеОшибки = НСтр("ru = 'В контейнере отсутсвует подпись файла %1.';
												|en = 'The container does not contain the %1 file signature.'");
				Результат.ОписаниеОшибки = СтрШаблон(Результат.ОписаниеОшибки, ИмяФайла);
				ЗаписатьСобытиеВЖурнал(Результат.ОписаниеОшибки,
					НСтр("ru = 'Транспортный контейнер';
						|en = 'Transport container'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		...
		Если СвойстваДокумента.Зашифровать Тогда
			...
			Если СвойстваКриптоСообщения.Тип <> "EnvelopedData" Тогда
				Результат.ОписаниеОшибки = НСтр("ru = 'Зашифрованный файл %1 имеет неверный формат.';
												|en = 'Encrypted %1 file has an invalid format.'");
				Результат.ОписаниеОшибки = СтрШаблон(Результат.ОписаниеОшибки, ИмяФайла);
				ЗаписатьСобытиеВЖурнал(Результат.ОписаниеОшибки,
					НСтр("ru = 'Транспортный контейнер';
						|en = 'Transport container'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		...
		Если НЕ РезультатПомещения.Выполнено Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'Ошибка при помещении данных файла %1 в журнал операций: %2.';
											|en = 'An error occurred upon transferring the %1 file data to the operation log: %2.'");
		...
		Если Вложение.Подписать Тогда
			...
			Если НЕ РезультатПомещения Тогда
				ОписаниеОшибки = НСтр("ru = 'Ошибка сохранения в базе данных подписи файла %1.';
										|en = 'Failed to save the %1 file signature to the database.'");
				Результат.ОписаниеОшибки = СтрШаблон(ОписаниеОшибки, ИмяВложенияБД);
				Возврат Результат;
			КонецЕсли;
		КонецЕсли;
...
Функция ПоместитьДанныеТранзакцииВЖурналОперацийСФайлами(Транзакция) Экспорт
	...
	Если ТаблицаДанныхКонтейнера.Количество() > 1 Тогда
		...
		ЗаписатьСобытиеВЖурнал(НСтр("ru = 'Контейнер хранится более чем в одном файле.';
									|en = 'The container is stored in more than one file.'"),
			НСтр("ru = 'Транспортный контейнер';
				|en = 'Transport container'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			Транзакция,
	...
	Если МассивФайлов = Неопределено Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось получить из информационной базы транспортный контейнер %1!%2';
				|en = 'Failed to receive the %1 transport container from the infobase.%2'"),
		...
		Если МассивФайлов = Неопределено Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось сохранить в информационной базе транспортный контейнер %1!%2';
					|en = 'Failed to save the %1 transport container to the infobase.%2'"),
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не удалось сохранить в информационной базе содержимое транспортного контейнера транзакции ""%1""!
                  |%2';
                  |en = 'Failed to save the transport container content of the ""%1"" transaction to the infobase.
                  |%2'"),
...
Функция ПолучитьСписокТиповТранзакцийДокументооборота(Сервис, ТипДокументооборота) Экспорт
	...
	Если СтрокаДокументооборота = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Документооборот не обнаружен в описании.';
								|en = 'EDI is not found in the details.'");
	КонецЕсли;
...
Функция ЗаписатьXMLВФайл(ТекстДляЗаписи, ФайлДляЗаписи) Экспорт
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Не удалось записать файл XML:%1';
								|en = 'Cannot write XML file:%1'"), Символы.ПС + ИнформацияОбОшибке().Описание);
		ЗаписатьСобытиеВЖурнал(ТекстСообщения,
			НСтр("ru = 'Транспортный контейнер';
				|en = 'Transport container'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		Возврат Ложь;
...
Функция ПредставлениеПредмета(Сервис, Предмет, БезИмениБанка = Ложь) Экспорт
	...
	Представление = "";
	УниверсальныйОбменСБанкамиСлужебныйПереопределяемый.ПредставлениеПредмета(
		Сервис,
		Предмет,
		БезИмениБанка,
		Представление);
	...
	Если ДокументооборотОбъект.Модифицированность() Тогда
		...
		Исключение
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось обновить свойства родительского документооборота %1%2';
											|en = 'Failed to update parent EDI properties %1%2'"),
...
Процедура ОпределитьНеобходимостьРегЗаданияОбменаСБанками(Сервис, УстановитьМинимальныйПериодПовтора = Ложь, Отбор = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(Сервис) Тогда
		ВызватьИсключение НСтр("ru = 'ОпределитьНеобходимостьРегЗаданияОбменаСБанками : необходимо указать сервис, для которого используется регламентное задание';
								|en = 'ОпределитьНеобходимостьРегЗаданияОбменаСБанками : specify the service for which a scheduled job is used'");
	КонецЕсли;
	...
	Если ИспользованиеРегЗадания Тогда // добавим новое рег.задание
		...
		НаименованиеЗадания = СтрШаблон(НСтр("ru = 'Универсальный обмен с банками. %1';
											|en = 'Universal bank exchange. %1'"), Ключ);
...
Функция АвторизацияНаСервере(Сервис = Неопределено) Экспорт
	...
	Результат.Вставить("ОписаниеОшибки", "");
	Результат.Вставить("ОшибкаПриПолученииТикета", Ложь);
	Результат.Вставить("ОшибкаАвторизацииПоЛогину", Ложь);
	
	// Получаем тикет с портала ИТС.
	РезультатПолученияТикета = ПолучитьТикет(Сервис);
	
	ИмяСобытия = НСтр("ru = 'Ошибка авторизации на сервере обмена с банками';
						|en = 'Authorization error on the bank exchange server'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
	Если НЕ ПустаяСтрока(РезультатПолученияТикета.ИнформацияОбОшибке) Тогда
		
		// Возникла ошибка при получении тикета.
		УстановитьПривилегированныйРежим(Истина);
		ДанныеАутентификации = ИнтернетПоддержкаПользователей.ДанныеАутентификацииПользователяИнтернетПоддержки();
		УстановитьПривилегированныйРежим(Ложь);
		
		Если (РезультатПолученияТикета.КодОшибки = "ОшибкаПодключения"
				ИЛИ РезультатПолученияТикета.КодОшибки = "НеверныйЛогинИлиПароль")
			И НЕ ОбщегоНазначения.РазделениеВключено()
			И НЕ ДанныеАутентификации = Неопределено Тогда
			
			// Выполняем авторизацию по логину и паролю.
			ПараметрыЗапроса = ПараметрыВыполнитьЗапросКСерверу("", "auth/by-login");
			
			ДанныеТела = Новый Структура;
			ДанныеТела.Вставить("login", ДанныеАутентификации.Логин);
			ДанныеТела.Вставить("password", ДанныеАутентификации.Пароль);
			...
			Если НЕ РезультатВыполненияЗапроса.Выполнено Тогда
				Шаблон = НСтр("ru = 'Ошибка при авторизации по логину ИТС: %1';
								|en = 'An error occurred during authorization using the ITS username: %1'");
				Результат.ОписаниеОшибки = СтрШаблон(Шаблон, РезультатВыполненияЗапроса.ОписаниеОшибки);
				...
				Результат.ОшибкаАвторизацииПоЛогину = Истина;
				Возврат Результат;
			КонецЕсли;
		...
		Иначе
			...
			Результат.ОписаниеОшибки = СтрШаблон(
				НСтр("ru = 'Ошибка при получении тикета ИТС:
						|%1';
						|en = 'ITS ticket receipt error:
						|%1'"),
				РезультатПолученияТикета.ИнформацияОбОшибке);
			ЗаписатьСобытиеВЖурнал(Результат.ОписаниеОшибки, ИмяСобытия);
			Результат.ОшибкаПриПолученииТикета = Истина;
			Возврат Результат;
...
Функция ЗагрузитьИнформациюОСервисеИзСтруктуры(ДанныеСервисаНаСервере)
	...
	Исключение
		// Пропускаем неизвестные сервисы. Считаем, что данные успешно загружены.
	КонецПопытки;
	
	Если Не ЗначениеЗаполнено(Сервис) Тогда
		// Данный сервис не поддерживается в текущей конфигурации. Его поставляемые данные загружать не требуется.
		Результат.Выполнено = Истина;
		Возврат Результат;
	КонецЕсли;
	...
	Если ДанныеСервиса.Пустая() Тогда
		...
		ДанныеСервисаОбъект.Наименование = НСтр("ru = 'Данные сервиса обмена с банками ';
												|en = 'Bank exchange service data '") + Сервис;
	Иначе
		ДанныеСервисаОбъект = ДанныеСервиса.ПолучитьОбъект();
	КонецЕсли;
	
	ДанныеОбщихХарактеристикСервера = ДанныеСервисаНаСервере.metadata.value;
	
	РасшифрованныеДанныеСервиса = РасшифроватьДанныеСервиса(Сервис, ДанныеОбщихХарактеристикСервера);
	ДанныеСервисаОбъект.Характеристики = Новый ХранилищеЗначения(РасшифрованныеДанныеСервиса, Новый СжатиеДанных(9));
	ДанныеСервисаОбъект.ДатаОбновления = ТекущаяДатаСеанса();
...
#Область ПолученияДанныхСервиса
	...
	Иначе
		ЗарегистироватьДатуОбращенияКСервису(Сервис);
	КонецЕсли;
		...
		Если Банк <> Неопределено Тогда
			...
			Если Строка = Неопределено Тогда
				ТекстОшибки = НСтр("ru = 'На сервере не найден банк с идентификатором %1.';
									|en = 'Bank with %1 ID is not found on the server.'");
...
Функция ПолучитьПараметрыСледующейЧастиКонтейнераССервера(Токен,
	...
	Если НЕ РезультатВыполненияЗапроса.Выполнено Тогда
		...
		ЗаписатьСобытиеВЖурнал(РезультатВыполненияЗапроса.ОписаниеОшибки,
			НСтр("ru = 'HTTP-запрос';
				|en = 'HTTP request'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		Возврат Результат;
	КонецЕсли;
...
Функция ОтправитьТранспортныйКонтейнерНаСервер(Параметры)
	...
	Исключение
		ОписаниеОшибки = НСтр("ru = 'Ошибка при промежуточной записи транзакции отправки транспортного контейнера.
                               |%1';
                               |en = 'An error occurred upon the intermediate recording of the transport container sending transaction.
                               |%1'");
	...
	Исключение
		ОписаниеОшибки = НСтр("ru = 'Ошибка при записи транзакции отправки транспортного контейнера.
                               |%1';
                               |en = 'An error occurred upon recording the transaction of transport container sending.
                               |%1'");
		Результат.ОписаниеОшибки = СтрШаблон(ОписаниеОшибки, ОписаниеОшибки());
		ЗаписатьСобытиеВЖурнал(Результат.ОписаниеОшибки,
			НСтр("ru = 'Транспортный контейнер';
	...
	Для каждого Контейнер Из Данные Цикл
		...
		Исключение
			...
			Шаблон = НСтр("ru = 'Ошибка определения типа транзакции по коду %1. Сервис %2, банк %3, документооборот %4, транзакция %5.';
							|en = 'Failed to define the transaction type by the %1 code. %2 service, %3 bank, %4 EDI, %5 transaction.'");
			ОписаниеОшибки = СтрШаблон(Шаблон,
				...
				Контейнер.id);
			ЗаписатьСобытиеВЖурнал(ОписаниеОшибки,
				НСтр("ru = 'Получение списка входящих транзакций';
					|en = 'Receive incoming transaction list'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
			Продолжить;
		КонецПопытки;
...
Функция СоздатьДокументооборотНаСервере(Токен, Сервис, Банк, ИНН, КПП)
	...
	Если УслугаБанка = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru = 'Услуга для банка ""%1"" сервиса ""%2"" не найдена.';
								|en = 'The service for the ""%1"" bank of the ""%2"" service is not found.'");
...
Функция ВыполнитьЗапросКСерверу(ПараметрыЗапроса, ПараметрыСервера = Неопределено) Экспорт
	...
	Исключение
		...
		Если НЕ ПустаяСтрока(ИмяФайлаТела) Тогда
			УдалитьВременныйФайл(ИмяФайлаТела);
		КонецЕсли;
		Возврат Результат;
	...
	Если НЕ РезультатАнализа.Выполнено Тогда
		...
		ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Ошибка при выполнении HTTP-запроса к %1.
			|%2';
			|en = 'An error occurred upon HTTP request to %1.
...
Функция ПроверитьДоступностьСервера() Экспорт
	...
	Если Ответ.status <> "UP" Тогда
		Шаблон = НСтр("ru = 'Сервер обмена с банками %1 не активен.';
						|en = '%1 bank exchange server is not active.'");
		ОписаниеОшибки = СтрШаблон(Шаблон, АдресСервиса);
		ЗаписатьСобытиеВЖурнал(ОписаниеОшибки,
			НСтр("ru = 'Проверка доступности сервера';
				|en = 'Server availability check'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		Результат.ОписаниеОшибки = ОписаниеОшибки;
		Возврат Результат;
	КонецЕсли;
...
Функция ЗагрузитьТранзакциюССервера(Параметры, ПараметрыСервера = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Документооборот) ИЛИ НЕ ТипЗнч(Документооборот) = Тип("СправочникСсылка.ДокументооборотыОбменаСБанками") Тогда
		Результат.ОписаниеОшибки = НСтр("ru = 'Не определено значение документооборота.';
										|en = 'EDI value is not determined.'");
		Возврат Результат;
	КонецЕсли;
	...
	Иначе
		...
		Если НЕ РезультатПолученияДанных.Выполнено И НЕ ЗакачатьЗаново Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'Ошибка получения содержимого контейнера из базы данных.';
											|en = 'Failed to receive the container content from the database.'");
			Возврат Результат;
		КонецЕсли;
...
#Область СкачиваниеКонтейнераССервера
	...
	Если НЕ РезультатПолученияАдреса.Выполнено Тогда
		ОписаниеОшибки = НСтр("ru = 'Ошибка получения адреса контейнера на сервере:
			|%1';
			|en = 'Failed to receive the container address on the server:
			|%1'");
...
#Область РаспаковкаКонтейнера
	...
	Исключение
		...
		Шаблон = НСтр("ru = 'Ошибка извлечения файлов транзакции %1 документооборота %2 из архива: %3.';
						|en = 'Failed to extract the %1 transaction files of %2 EDI from the archive: %3.'");
...
#Область РазборОписанияКонтейнера
	...
	Если НайденныеФайлы.Количество() = 0 Тогда
		ОписаниеОшибки = НСтр("ru = 'Контейнер не содержит файла описания содержимого %1.';
								|en = 'The container does not contain %1 content details file.'");
	...
	СервисТранспортнаяИнформация = ОпределитьСервисПоИдентификаторуНаСервере(ТранспортнаяИнформация.ИдентификаторСервиса);
	...
	Если ИдентификаторТранзакцииТранспортнаяИнформация <> ИдентификаторТранзакции Тогда
		ОписаниеОшибки = НСтр("ru = 'Идентификатор транзакции в файле ""%1"" не совпадает с идентификатором на сервере ""%2"".';
								|en = 'Transaction ID in the ""%1"" file differs from ID on the ""%2"" server.'");
	...
	Если СервисТранспортнаяИнформация <> Сервис Тогда
		ОписаниеОшибки = НСтр("ru = 'Сервис транзакции в файле ""%1"" не совпадает с сервисом документооборота ""%2"".';
								|en = 'Transaction service in the ""%1"" file differs from ""%2"" EDI service.'");
	...
	Если Организация = Неопределено Тогда
		Результат.ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Организация с ИНН %1 и КПП %2 не найдена.';
													|en = 'Company with TIN %1 and CRTR %2 is not found.'"), ИННОрганизации, КППОрганизации);
	...
	Если Организация <> Документооборот.Организация Тогда
		Результат.ОписаниеОшибки = СтрШаблон(
			НСтр("ru = 'Организация документооборота ""%1"" отличается от организации транзакции ""%2"". Возможно есть дубли по ИНН, КПП.';
				|en = '""%1"" EDI company differs from ""%2"" transaction company. There might be TIN and CRTR duplicates.'"),
	...
	Если ИдентификаторДокументооборота <> Документооборот.Идентификатор Тогда
		Результат.ОписаниеОшибки = НСтр("ru = 'Идентификатор документооборота в контейнере отличается от идентификатора на сервере.';
										|en = 'EDI ID in the container differs from ID on the server.'");
		ЗаписатьСобытиеВЖурнал(Результат.ОписаниеОшибки, , Документооборот);
	...
	Если ИдентификаторТранзакции <> ИдентификаторТранзакцииТранспортнаяИнформация Тогда
		Результат.ОписаниеОшибки = НСтр("ru = 'Идентификатор транзакции в контейнере отличается от идентификатора на сервере.';
										|en = 'Transaction ID in the container differs from ID on the server.'");
		ЗаписатьСобытиеВЖурнал(Результат.ОписаниеОшибки, , Документооборот);
	...
	Для каждого Документ Из ЗначениеВКоллекцию(ТранспортнаяИнформация.Документы.Документ) Цикл
		...
		Если Подписан Тогда
			...
			Если НайденныеФайлы.Количество() = 0 Тогда
				...
				ОписаниеОшибки = НСтр("ru = 'В контейнере отсутсвует файл подписи %1.';
										|en = 'The container does not contain the %1 signature file.'");
		...
		Иначе
			...
			Если Сжат Тогда
				...
				УдалитьВременныйФайл(КаталогСДаннымиДокумента, "*");
				...
				ЧтениеZip.Закрыть();
				УдалитьВременныйФайл(ПолноеИмяФайлаСодержимого);
			...
			Если НЕ РезультатСохранения Тогда
				...
				ОписаниеОшибки = НСтр("ru = 'Ошибка записи содержимого контейнера в базу данных.';
										|en = 'Failed to record the container to the database.'");
				Результат.ОписаниеОшибки = ОписаниеОшибки;
				ЗаписатьСобытиеВЖурнал(Результат.ОписаниеОшибки,
					НСтр("ru = 'Транспортный контейнер';
						|en = 'Transport container'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		...
		Если НЕ Зашифрован И Подписан Тогда
			...
			Если НЕ РезультатСохранения Тогда
				...
				ОписаниеОшибки = НСтр("ru = 'Ошибка сохранения файла с подписью.';
										|en = 'An error occurred upon saving the file with signature.'");
				Результат.ОписаниеОшибки = ОписаниеОшибки;
				ЗаписатьСобытиеВЖурнал(Результат.ОписаниеОшибки,
					НСтр("ru = 'Транспортный контейнер';
						|en = 'Transport container'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
#Область СохранениеДанныхТранзакцииВБД
	...
	Если НЕ РезультатСохранения Тогда
		Результат.ОписаниеОшибки = НСтр("ru = 'Ошибка сохранения контейнера в базе данных.';
										|en = 'An error occurred upon saving the container in the database.'");
		ЗаписатьСобытиеВЖурнал(Результат.ОписаниеОшибки,
			НСтр("ru = 'Транспортный контейнер';
				|en = 'Transport container'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			ТранзакцияСсылка);
...
Функция ЗагрузитьТранзакцииССервера(Токен, Сервис, Отбор = Неопределено) Экспорт
	...
	Для каждого ДанныеДокументооборота Из ОжидающиеОтветаДокументообороты Цикл
		...
		Если НЕ РезультатПолученияСпискаКонтейнеров.Выполнено Тогда
			...
			ОписаниеОшибки = НСтр("ru = 'Ошибка при получении списка входящих сообщений по документообороту %1:
				|%2';
				|en = 'An error occurred upon receiving the list of %1 EDI incoming messages:
				|%2'");
			ОписаниеОшибки = СтрШаблон(ОписаниеОшибки,
				Документооборот.Идентификатор,
		...
		Для каждого Контейнер Из РезультатПолученияСпискаКонтейнеров.ТаблицаКонтейнеров Цикл
			...
			Исключение
				...
				ТекстОшибки = НСтр("ru = 'Ошибка загрузки сообщения с сервера.';
									|en = 'An error occurred upon importing a server message.'");
			...
			Если Транзакция <> Неопределено И РезультатЗагрузкиКонтейнера.НоваяТранзакция Тогда
				...
				Исключение
					...
					ТекстОшибки = НСтр("ru = 'Ошибка обработки данных транзакции сервисом.';
										|en = 'An error occurred upon processing transaction data by the service.'");
					ДлительныеОперацииОбменаСБанками.ВывестиОшибку(ТекстОшибки);
					Продолжить;
				КонецПопытки;
	...
	Если БылиОшибки Тогда
		ОписаниеОшибок.Вставить(0, НСтр("ru = 'При загрузке транзакций возникли следующие ошибки:';
										|en = 'The following errors occurred upon importing the transactions:'"));
		...
		Результат.ОписаниеОшибки = НСтр("ru = 'При загрузке транзакций возникли ошибки.';
										|en = 'Errors occurred upon transaction import.'");
	Иначе
		Результат.Выполнено = Истина;
...
Функция ОтправитьТранзакцию(Токен, Знач Транзакция, ПараметрыСервера = Неопределено) Экспорт
	...
	Если Транзакция.Пустая() Тогда
		ТекстСообщения = НСтр("ru = 'На задана транзакция для отправки.';
								|en = 'Transaction for sending is not set.'");
	...
	ПризнакОшибки = Ложь;
	ИмяСобытия = НСтр("ru = 'Транспортный контейнер';
						|en = 'Transport container'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если Транзакция.Статус = Перечисления.СтатусыТранзакцийОбменаСБанками.Получена Тогда
		...
		ТекстСообщения = НСтр("ru = 'Отправка полученных транзакций запрещена.';
								|en = 'It is prohibited to send received transactions.'");
...
#Область ПроверкиПередОтправкой
	...
	Если НЕ ЗначениеЗаполнено(Транзакция.ТипТранзакции) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Ошибка отправки транзакции: тип транзакции не определен.';
								|en = 'Transaction sending error: transaction type is not defined.'");
	...
	Если НЕ ЗначениеЗаполнено(Транзакция.Организация) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Ошибка отправки транзакции: не задана организация.';
								|en = 'Transaction sending error: company is not specified.'");
		ЗаписатьСобытиеВЖурнал(ТекстСообщения, ИмяСобытия, Транзакция);
		ПризнакОшибки = Истина;
	...
	Если НЕ ЗначениеЗаполнено(Транзакция.Банк) И Транзакция.Банки.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Ошибка отправки транзакции: не задан банк-получатель.';
								|en = 'Transaction sending error: receiving bank is not specified.'");
		ЗаписатьСобытиеВЖурнал(ТекстСообщения, ИмяСобытия, Транзакция);
		ПризнакОшибки = Истина;
	...
	Если НЕ ЗначениеЗаполнено(Транзакция.Документооборот) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Ошибка отправки транзакции: документооборот не задан.';
								|en = 'Transaction sending error: EDI is not specified.'");
		ЗаписатьСобытиеВЖурнал(ТекстСообщения, ИмяСобытия, Транзакция);
		ПризнакОшибки = Истина;
...
#КонецОбласти
	...
	Если ОписаниеТранспортныхКонтейнеров.Количество() = 0 Тогда
		...
		ОписаниеОшибки = НСтр("ru = 'Не удалось отправить транзакцию по причине отсутствия сформированного пакета.';
								|en = 'Failed to send the transaction as there is no generated package.'");
		ЗаписатьСобытиеВЖурнал(ОписаниеОшибки,
			НСтр("ru = 'Работа с архивом';
				|en = 'Operations with archive'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			Транзакция);
...
//
// Параметры:
//	Параметры - Структура - Необходима для совместимости с БСП, содержит ключи:
...
Процедура ОбновитьДанныеСервиса(Параметры, АдресХранилища) Экспорт
	...
	Попытка
		АктуализироватьДанныеСервиса(Параметры.СервисОбменаСБанками);
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'УниверсальныйОбменСБанками.ОбновитьДанныеСервиса';
				|en = 'УниверсальныйОбменСБанками.ОбновитьДанныеСервиса'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция НеотправленныеИзвещения(Сервис, Отбор = Неопределено) Экспорт
	...
	УниверсальныйОбменСБанкамиСлужебныйПереопределяемый.НеотправленныеИзвещения(
		Сервис,
		Отбор,
		Результат);
...
Функция ЗагрузитьИнформациюОБанкеИзСтруктуры(Данные)
	...
	БанкОбъект.НаименованиеСокращенное = Данные.shortName;
	
	НаименованиеИнтерфейс = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Данные, "publicName", "");
	
	Если ЗначениеЗаполнено(НаименованиеИнтерфейс) Тогда
		БанкОбъект.Наименование = НаименованиеИнтерфейс;
	Иначе
	...
	БанкОбъект.АктивенДо = ПрочитатьДатуИзJSON(Данные.dateTo);
	...
	Если ПустаяСтрока(ОтпечатокДействующегоСертификата) Тогда
		БанкОбъект.АктивенДо = ТекущаяДатаСеанса();
	КонецЕсли;
...
Функция ЗагрузитьИнформациюОбУслугеБанкаИзСтруктуры(Данные)
	...
	Если Банк = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru = 'Банк услуги не найден по идентификатору %1.';
								|en = 'Service bank is not found by ID %1.'");
		Результат.ОписаниеОшибки = СтрШаблон(ОписаниеОшибки, Данные.bankId);
		ЗаписатьСобытиеВЖурнал(Результат.ОписаниеОшибки,
			НСтр("ru = 'Загрузка данных услуги банка';
				|en = 'Import bank service data'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УслугаБанка);
		Возврат Результат;
	КонецЕсли;
	
	ИмяСервиса = Данные.serviceId;
	
	Попытка
		Сервис = ОпределитьСервисПоИдентификаторуНаСервере(ИмяСервиса);
	Исключение
		ОписаниеОшибки = НСтр("ru = 'Сервис услуги не найден по идентификатору %1.';
								|en = 'The service is not found by ID %1.'");
		Результат.ОписаниеОшибки = СтрШаблон(ОписаниеОшибки, ИмяСервиса);
		ЗаписатьСобытиеВЖурнал(Результат.ОписаниеОшибки,
			НСтр("ru = 'Загрузка данных услуги банка';
				|en = 'Import bank service data'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УслугаБанка);
	...
	УслугаБанкаОбъект.НачалоДействия = ПрочитатьДатуИзJSON(Данные.dateFrom);
	УслугаБанкаОбъект.КонецДействия = ПрочитатьДатуИзJSON(Данные.dateTo);
	...
	Для каждого Характеристика Из ЗначениеВКоллекцию(Данные.characteristicValues) Цикл
		...
		ДанныеХарактеристики = Характеристика.value;
		РасшифрованныеДанные = РасшифроватьДанныеХарактеристики(Сервис, Ключ, ДанныеХарактеристики);

		Строка.Значение = Новый ХранилищеЗначения(РасшифрованныеДанные, Новый СжатиеДанных(9));
	КонецЦикла;
	...
	Исключение
		ОписаниеОшибки = НСтр("ru = 'Ошибка при записи данных услуги банка:
			|%1.';
			|en = 'An error occurred upon recording bank service data:
			|%1.'");
	...
	УниверсальныйОбменСБанкамиСлужебныйПереопределяемый.ПослеЗагрузкиДанныхУслугиБанка(УслугаБанкаОбъект.Ссылка, ИмяСервиса);
...
Функция ЗагрузитьСтраницыССервера(Токен, ПараметрыЗапроса, Страница = 0, Данные = Неопределено)
	...
	Если СтраницаВТелеОтвета <> Страница Тогда
		ТекстСообщения = НСтр("ru = 'Получена страница с номером %1, ожидалась страница %2.';
								|en = 'Page with # %1 is received. %2 page was expected.'");
...
Функция СкачатьДанныеИзХранилищаS3(URL, ИмяФайлаКонтейнера)
	...
	Если Ответ.КодСостояния = 200 Тогда
		Результат.Выполнено = Истина;
	Иначе
		ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Ошибка скачивания данных транзакции. Код состояния: %1.';
										|en = 'Transaction data download error. Status code: %1.'"), Ответ.КодСостояния);
		ЗаписатьСобытиеВЖурнал(ОписаниеОшибки,
			НСтр("ru = 'HTTP-запрос';
				|en = 'HTTP request'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		Результат.ОписаниеОшибки = ОписаниеОшибки;
...
Функция ZIPАрхивВДвоичныеДанные(ИсходныеДанные, ИсходноеИмяФайла, ОписаниеОшибки)
	...
	Результат      = Неопределено;
	ОписаниеОшибки = "";
	ИмяСобытия = НСтр("ru = 'ZIPАрхивВДвоичныеДанные';
						|en = 'ZIPАрхивВДвоичныеДанные'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если ИмяВременногоКаталога = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru = 'Ошибка при создании каталога с временными файлами.';
								|en = 'An error occurred upon generating the temporary file directory.'");
		ЗаписатьСобытиеВЖурнал(ОписаниеОшибки, ИмяСобытия);
		Возврат Результат;
	КонецЕсли;
	...
	Исключение
		...
		ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Ошибка подготовки zip-архива файла %1: %2.';
										|en = 'An error occurred upon preparing the %1 file ZIP archive: %2.'"),
	...
	УдалитьВременныйФайл(ИмяВременногоКаталога);
...
Функция ПодписатьДанныеВЖурналеОперацийСФайлами(МенеджерЗаписи, ОтпечатокСертификата)
	...
	Если ТипЗнч(ИсходныеДанные) <> Тип("ДвоичныеДанные") Тогда
		Результат.ОписаниеОшибки = СтрШаблон(
			НСтр("ru = 'Некорректный формат исходных данных файла %1 для подписания';
				|en = 'Invalid format of source data of the %1 file for signing'"),
	...
	Иначе
		ОписаниеОшибки = НСтр("ru = 'Ошибка подписывания файла, код возврата: %1.';
								|en = 'File signing error. Return code: %1.'");
...
Функция ЗашифроватьДанныеВЖурналеОперацийСФайлами(МенеджерЗаписи, КэшСертификатовБанков, СертификатШифрования = Неопределено)
	...
	Если ТипЗнч(ДанныеДляШифрования) <> Тип("ДвоичныеДанные") Тогда
		Результат.ОписаниеОшибки = СтрШаблон(
			НСтр("ru = 'Некорректный формат данных файла %1 для шифрования';
				|en = 'Invalid format of data of the %1 file for encryption'"),
			МенеджерЗаписи.ИсходноеИмяФайла);
		Возврат Результат;
	КонецЕсли;
	...
	Если ТипЗнч(Банки) <> Тип("Массив") Тогда
		Результат.ОписаниеОшибки = СтрШаблон(
			НСтр("ru = 'Не указан список банков-получателей файла %1';
				|en = 'The list of the %1 file receiving banks is not specified'"),
			МенеджерЗаписи.ИсходноеИмяФайла);
		Возврат Результат;
	КонецЕсли;
	...
	Для каждого Банк Из Банки Цикл
		...
		Если СертификатБанка = Неопределено Тогда
			ТекстыОшибок.Добавить(СтрШаблон(НСтр("ru = 'Не найден действующий сертифика для банка %1';
												|en = 'Valid certificate for %1 bank is not found'"), Банк));
		Иначе
...
Функция РасшифроватьИПроверитьПодписьЗаписиВЖурналеОперацийСФайлами(МенеджерЗаписи, СертификатыБанка)
	...
	Если ТипЗнч(ДанныеДляРасшифровки) <> Тип("ДвоичныеДанные") Тогда
		ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Некорректный формат данных файла %1 для расшифровки';
										|en = 'Incorrect format of data of the %1 file for decryption'"), МенеджерЗаписи.ИсходноеИмяФайла);
		ЗаписатьСобытиеВЖурнал(ОписаниеОшибки,
			НСтр("ru = 'Работа с архивом';
				|en = 'Operations with archive'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		Результат.ОписаниеОшибки = ОписаниеОшибки;
	...
	Если ТипЗнч(РасшифрованныеДанные) = Тип("ДвоичныеДанные") Тогда
		...
		КонецЕсли;
	Иначе
		ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Некорректный формат файла %1';
										|en = 'Incorrect format of the %1 file'"), МенеджерЗаписи.ИсходноеИмяФайла);
		ЗаписатьСобытиеВЖурнал(ОписаниеОшибки,
			НСтр("ru = 'Работа с архивом';
				|en = 'Operations with archive'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		Результат.ОписаниеОшибки = ОписаниеОшибки;
	...
	Если МенеджерЗаписи.Подписать Тогда
		...
		Исключение
			ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Ошибка при проверке подписи для файла %1.
				|%2';
				|en = 'An error occurred upon the %1 file signature check.
		...
		Если Результат.ПодписьНеДействительна Тогда
			ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Подпись для файла %1 является недействительной';
											|en = 'Signature for the %1 file is invalid'"), МенеджерЗаписи.ИсходноеИмяФайла);
			ЗаписатьСобытиеВЖурнал(ОписаниеОшибки,
				НСтр("ru = 'Сертификаты подписей';
					|en = 'Signature certificates'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ,
				УровеньЖурналаРегистрации.Предупреждение);
		...
		Если НЕ СертификатНайден Тогда
			...
			ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Неизвестный сертификат подписи для файла %1';
											|en = 'Unknown signature certificate for the %1 file'"), МенеджерЗаписи.ИсходноеИмяФайла);
			ЗаписатьСобытиеВЖурнал(ОписаниеОшибки,
				НСтр("ru = 'Сертификаты подписей';
					|en = 'Signature certificates'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ,
				УровеньЖурналаРегистрации.Предупреждение);
...
Функция РаспаковатьДвоичныеДанные(Данные, ИмяФайла, Идентификатор = Неопределено) Экспорт
	...
	Исключение
		...
		ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Ошибка при распаковке файла %1 на сервере.
			|%2';
			|en = 'An error occurred upon unpacking the %1 file on the server.
			|%2'"), ИмяФайла, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписатьСобытиеВЖурнал(ОписаниеОшибки,
			НСтр("ru = 'Работа с архивом';
				|en = 'Operations with archive'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Идентификатор);
		Результат.ОписаниеОшибки = ОписаниеОшибки;
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				ОписаниеОшибки = НСтр("ru = 'Ошибка при записи данных файла %1 в базу данных.';
										|en = 'An error occurred upon recording the %1 file data to the database.'");
...
// Предназначена для отражения даты и времени обращения к сервису.
// 
// Параметры:
//	Сервис - ПеречислениеСсылка.СервисыОбменаСБанками
//
// Возвращаемое значение:
//	Булево - Истина, если успешно обновлено
//
Функция ЗарегистироватьДатуОбращенияКСервису(Сервис)
...
Функция НовыйСоставВерсииКонфигурации()

	СоставВерсииКонфигурации = Новый Структура;
	СоставВерсииКонфигурации.Вставить("ВерсияКонфигурации",       "");
	СоставВерсииКонфигурации.Вставить("ВерсияОтраслевогоРешения", "");
	
	Возврат СоставВерсииКонфигурации;

КонецФункции

Функция РасшифроватьДанныеСервиса(Сервис, Данные)
	
	Если ТребуетсяРасшифроватьДанныеСервиса(Сервис) Тогда
		Возврат ИзвлечьИзАрхиваBase64(Данные);
	Иначе
		Возврат Данные;
	КонецЕсли;
...
Функция РасшифроватьДанныеХарактеристики(Сервис, Характеристика, Данные)
	
	Если ТребуетсяРасшифроватьДанныеХарактеристики(Сервис, Характеристика) Тогда
		Возврат ИзвлечьИзАрхиваBase64(Данные);
	Иначе
		Возврат Данные;
	КонецЕсли;
...
Функция ТребуетсяРасшифроватьДанныеСервиса(Сервис)
	
	Расшифровать = Ложь;
	УниверсальныйОбменСБанкамиСлужебныйПереопределяемый.ТребуетсяРасшифроватьДанныеСервиса(Сервис, Расшифровать);

	Возврат Расшифровать;

КонецФункции

Функция ТребуетсяРасшифроватьДанныеХарактеристики(Сервис, Характеристика)
	
	Расшифровать = Ложь;
	УниверсальныйОбменСБанкамиСлужебныйПереопределяемый.
		ТребуетсяРасшифроватьДанныеХарактеристики(Сервис, Характеристика, Расшифровать);

	Возврат Расшифровать;

КонецФункции

Функция ИзвлечьИзАрхиваBase64(Характеристика)

	Попытка
		
		ДанныеАрхива = Base64Значение(Характеристика);
	...
	Иначе
		...
		ИмяФайла = Архив.Элементы[0].Имя;
		
		Архив.Извлечь(Архив.Элементы[0], ВременныйКаталог, РежимВосстановленияПутейФайловZIP.НеВосстанавливать);
		Архив.Закрыть();
		
		ЧтениеТекста = Новый ЧтениеТекста;
		ЧтениеТекста.Открыть(ВременныйКаталог + ИмяФайла);
