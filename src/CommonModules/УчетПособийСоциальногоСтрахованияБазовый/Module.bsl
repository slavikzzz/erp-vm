...
Процедура ЗаполнитьНастройкиПечатныхФормПоУмолчанию(ОписанияНастроек) Экспорт
	...
	КадровыйЭДО.ДобавитьНастройкуПечатнойФормыПоМетаданным(
		ОписанияНастроек,
		Метаданные.Документы.ИсходящаяСправкаОЗаработкеДляРасчетаПособий.Макеты.ПФ_MXL_СправкаРасширенная,
		Перечисления.СодержимоеДокументов.СодержитЗарплату);
	
	КадровыйЭДО.ДобавитьНастройкуПечатнойФормыПоМетаданным(
		ОписанияНастроек,
		Метаданные.Документы.ИсходящаяСправкаОЗаработкеДляРасчетаПособий.Макеты.ПФ_MXL_Справка_2022,
		Перечисления.СодержимоеДокументов.СодержитЗарплату);
...
Процедура СоздатьВТДанныеОЗаработкеДляЗаполнения(МенеджерВременныхТаблиц, ПараметрыЗаполнения)
	...
	ТекстЗапроса =
		...
		|	СУММА(СведенияОДоходах.БазаДляРасчетаПособий - СведенияОДоходах.СуммаПревысившаяПределДляРасчетаПособий) КАК Заработок
		...
		|
		|ИМЕЮЩИЕ
		|	СУММА(СведенияОДоходах.БазаДляРасчетаПособий - СведенияОДоходах.СуммаПревысившаяПределДляРасчетаПособий) <> 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Процедура СоздатьВТДанныеДокументовДляРасчетаСреднегоЗаработкаФСС(МенеджерВременныхТаблиц, МассивСсылок) Экспорт
	...
	|	НЕОПРЕДЕЛЕНО КАК ИсточникСреднего,
	...
	|	ДанныеДокументаРасчетаСреднего.ИсточникСреднего КАК ИсточникСреднего,
...
Функция ТаблицыДанныхСреднегоЗаработкаФСС(ИмяДокумента, МассивСсылок) Экспорт
	...
	|	СреднийЗаработокФСС.Начисление КАК Начисление,
