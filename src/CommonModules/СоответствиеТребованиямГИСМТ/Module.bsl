...
Функция СостояниеКомандыСоответствиеТребованиямГИСМТ(Документ, РезультатПроверкиПоДокументу = Неопределено) Экспорт
	...
	Если РезультатПроверкиПоДокументу.ПроверкаОтключена Тогда
		СостояниеКоманды.ЦветТекста = ЦветаСтиля.ЦветТекстаНеТребуетВниманияГосИС;
...
Функция ИнициализироватьПараметрыОбработки(Документ, Параметры) Экспорт
	...
	ПараметрыОбработки.Вставить("ПередачаПивнойПродукции",             ДанныеШтрихкодов.ПередачаПивнойПродукции);
...
Функция ПолучитьИРазобратьШтрихкодыДокумента(Документ, ПараметрыСканирования, НаДату) Экспорт
	...
	ДанныеШтрихкодов.Вставить("ПередачаПивнойПродукции",        Ложь);
	...
	ДанныеШтрихкодов.ПередачаПивнойПродукции = ШтрихкодированиеОбщегоНазначенияИС.ПрисутствуетПрослеживаемаяПоМОДПивнаяПродукция(ДанныеШтрихкодов.ВидыПродукцииВДокументе, НаДату);
...
Функция РеквизитыДокумента(Документ) Экспорт
	...
	РеквизитыДокумента.Вставить("ДанныеПокупателя",                ЭлектронноеВзаимодействиеИСМП.КонструкторАдресУчастникаОбмена());
	РеквизитыДокумента.Вставить("ДанныеПродавца",                  ЭлектронноеВзаимодействиеИСМП.КонструкторАдресУчастникаОбмена());
	РеквизитыДокумента.Вставить("ДанныеЭлектронногоДокумента",     Неопределено);
	
	СоответствиеТребованиямГИСМТПереопределяемый.ПриОпределенииРеквизитовДокумента(Документ, РеквизитыДокумента);
	
	Если Не РеквизитыДокумента.ДанныеЭлектронногоДокумента = Неопределено Тогда
	
		СтруктураАдресов = ЭлектронноеВзаимодействиеИСМП.СтруктураАдресовУчастниковОбмена();
		ЭлектронноеВзаимодействиеИСМППереопределяемый.ЗаполнитьСведенияОбАдресахУчастниковОбмена_5_02(РеквизитыДокумента.ДанныеЭлектронногоДокумента, Документ, СтруктураАдресов, Ложь, Ложь);
		
		ЗаполнитьЗначенияСвойств(РеквизитыДокумента.ДанныеПокупателя, СтруктураАдресов.Покупатель);
		ЗаполнитьЗначенияСвойств(РеквизитыДокумента.ДанныеПродавца, СтруктураАдресов.Продавец);
	...
	РеквизитыДокумента.Вставить("ОрганизацияКПП", ИННКПППоОрганизации.КПП);
	
	РеквизитыДокумента.Вставить("КонтрагентИНН",       "");
	РеквизитыДокумента.Вставить("КонтрагентКПП",       "");
	РеквизитыДокумента.Вставить("ГрузополучательИНН",  "");
	РеквизитыДокумента.Вставить("ГрузополучательКПП",  "");
	РеквизитыДокумента.Вставить("ГрузоотправительИНН", "");
	РеквизитыДокумента.Вставить("ГрузоотправительКПП", "");
	...
	Если ЗначениеЗаполнено(РеквизитыДокумента.Контрагент) Тогда
		...
		РеквизитыДокумента.КонтрагентКПП = ИННКПППоКонтрагенту.КПП;
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(РеквизитыДокумента.ДанныеПокупателя.УполномоченноеЛицоЗаПоставкуГруза) Тогда
		
		ИННКПППоГрузополучателю = РаботаСКонтрагентамиИСВызовСервера.ИННКПППоОрганизацииКонтрагенту(РеквизитыДокумента.ДанныеПокупателя.УполномоченноеЛицоЗаПоставкуГруза);
		
		РеквизитыДокумента.ГрузополучательИНН = ИННКПППоГрузополучателю.ИНН;
		РеквизитыДокумента.ГрузополучательКПП = ИННКПППоГрузополучателю.КПП;
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(РеквизитыДокумента.ДанныеПродавца.УполномоченноеЛицоЗаПоставкуГруза) Тогда
		
		ИННКПППоГрузоотправителю = РаботаСКонтрагентамиИСВызовСервера.ИННКПППоОрганизацииКонтрагенту(РеквизитыДокумента.ДанныеПродавца.УполномоченноеЛицоЗаПоставкуГруза);
		
		РеквизитыДокумента.ГрузоотправительИНН = ИННКПППоГрузоотправителю.ИНН;
		РеквизитыДокумента.ГрузоотправительКПП = ИННКПППоГрузоотправителю.КПП;
		
	КонецЕсли;
	
	РеквизитыДокумента.Вставить("ИдентификаторГАР_МОД_Отправителя", "");
	РеквизитыДокумента.Вставить("ИдентификаторГАР_МОД_Получателя",  "");
	
	АдресОтправителяЗначение = "";
	АдресПолучателяЗначение  = "";
	
	УчастникОбменаПродавец = ?(ЗначениеЗаполнено(РеквизитыДокумента.ДанныеПродавца.УполномоченноеЛицоЗаПоставкуГруза), 
		РеквизитыДокумента.ДанныеПродавца.УполномоченноеЛицоЗаПоставкуГруза,
		РеквизитыДокумента.ДанныеПродавца.УчастникОбмена);
		
	УчастникОбменаПокупатель = ?(ЗначениеЗаполнено(РеквизитыДокумента.ДанныеПокупателя.УполномоченноеЛицоЗаПоставкуГруза), 
		РеквизитыДокумента.ДанныеПокупателя.УполномоченноеЛицоЗаПоставкуГруза,
		РеквизитыДокумента.ДанныеПокупателя.УчастникОбмена);
	
	Если ЗначениеЗаполнено(УчастникОбменаПродавец) Тогда
		УчастникОбменаПродавецИндивидуальныйПредприниматель = ОбщегоНазначенияИСВызовСервера.ПартнерОрганизацияЭтоИндивидуальныйПредприниматель(УчастникОбменаПродавец);
	Иначе
		УчастникОбменаПродавецИндивидуальныйПредприниматель = Ложь;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(УчастникОбменаПокупатель) Тогда
		УчастникОбменаПокупательИндивидуальныйПредприниматель = ОбщегоНазначенияИСВызовСервера.ПартнерОрганизацияЭтоИндивидуальныйПредприниматель(УчастникОбменаПокупатель);
	Иначе
		УчастникОбменаПокупательИндивидуальныйПредприниматель = Ложь;
	КонецЕсли;
	
	Если УчастникОбменаПродавецИндивидуальныйПредприниматель Тогда
		
		Если РеквизитыДокумента.ДанныеПродавца.АвтоматическоеЗаполнение Тогда
			
			Если ЗначениеЗаполнено(РеквизитыДокумента.ДанныеПродавца.ВладелецКонтактнойИнформации)
				И ЗначениеЗаполнено(РеквизитыДокумента.ДанныеПродавца.ВидКонтактнойИнформации) Тогда
				
				КонтактнаяИнформация = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(
					РеквизитыДокумента.ДанныеПродавца.ВладелецКонтактнойИнформации, РеквизитыДокумента.ДанныеПродавца.ВидКонтактнойИнформации, РеквизитыДокумента.Дата, Ложь);
		...
		ИначеЕсли ЗначениеЗаполнено(РеквизитыДокумента.ДанныеПродавца.АдресЗначенияПолей) Тогда
			АдресОтправителяЗначение = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВJSON(РеквизитыДокумента.ДанныеПродавца.АдресЗначенияПолей);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(АдресОтправителяЗначение) Тогда
		...
		Если АдресПоПриказуМинфина.КодСтраны = "643" Тогда // Россия
			Если ЗначениеЗаполнено(АдресПоПриказуМинфина.ИдентификаторДома) Тогда
				РеквизитыДокумента.ИдентификаторГАР_МОД_Отправителя = АдресПоПриказуМинфина.ИдентификаторДома;
			Иначе
				РеквизитыДокумента.ИдентификаторГАР_МОД_Отправителя = АдресПоПриказуМинфина.ИдентификаторАдресногоОбъекта;
			КонецЕсли;
		КонецЕсли;
		
		РеквизитыДокумента.ДанныеПродавца.АдресСтрокой = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформации(АдресОтправителяЗначение);
		
		Если УчастникОбменаПродавецИндивидуальныйПредприниматель Тогда
			ЭлектронноеВзаимодействиеИСМП.ВыполнитьУточнениеКодаФИАСПоМОДам(
				РеквизитыДокумента.ИдентификаторГАР_МОД_Отправителя,
				РеквизитыДокумента.ДанныеПродавца.АдресСтрокой);
		КонецЕсли;
		
	КонецЕсли;
	
	Если УчастникОбменаПокупательИндивидуальныйПредприниматель Тогда
	
		Если РеквизитыДокумента.ДанныеПокупателя.АвтоматическоеЗаполнение Тогда
			
			Если ЗначениеЗаполнено(РеквизитыДокумента.ДанныеПокупателя.ВладелецКонтактнойИнформации)
				И ЗначениеЗаполнено(РеквизитыДокумента.ДанныеПокупателя.ВидКонтактнойИнформации) Тогда
				
				КонтактнаяИнформация = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(
					РеквизитыДокумента.ДанныеПокупателя.ВладелецКонтактнойИнформации, РеквизитыДокумента.ДанныеПокупателя.ВидКонтактнойИнформации, РеквизитыДокумента.Дата, Ложь);
		...
		ИначеЕсли ЗначениеЗаполнено(РеквизитыДокумента.ДанныеПокупателя.АдресЗначенияПолей) Тогда
			АдресПолучателяЗначение = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВJSON(РеквизитыДокумента.ДанныеПокупателя.АдресЗначенияПолей);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(АдресПолучателяЗначение) Тогда
		...
		Если АдресПоПриказуМинфина.КодСтраны = "643" Тогда // Россия
			Если ЗначениеЗаполнено(АдресПоПриказуМинфина.ИдентификаторДома) Тогда
				РеквизитыДокумента.ИдентификаторГАР_МОД_Получателя = АдресПоПриказуМинфина.ИдентификаторДома;
			Иначе
				РеквизитыДокумента.ИдентификаторГАР_МОД_Получателя = АдресПоПриказуМинфина.ИдентификаторАдресногоОбъекта;
			КонецЕсли;
		КонецЕсли;
		
		РеквизитыДокумента.ДанныеПокупателя.АдресСтрокой = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформации(АдресПолучателяЗначение);
		
		Если УчастникОбменаПокупательИндивидуальныйПредприниматель Тогда
			ЭлектронноеВзаимодействиеИСМП.ВыполнитьУточнениеКодаФИАСПоМОДам(
				РеквизитыДокумента.ИдентификаторГАР_МОД_Получателя,
				РеквизитыДокумента.ДанныеПокупателя.АдресСтрокой);
		КонецЕсли;
...
Функция ПараметрыСканирования(Документ)
	...
	СтруктураЗаполнения = Новый Структура("ДатаДокумента", ДанныеШапки.Дата);
	ЗаполнитьЗначенияСвойств(ПараметрыСканирования, СтруктураЗаполнения);
...
Функция ДокументСодержитШтрихкоды(Документ) Экспорт
	...
	СтруктураРеквизитов = Новый Структура("ДатаДокумента", Неопределено);
	ЗаполнитьЗначенияСвойств(СтруктураРеквизитов, ПараметрыСканирования);
	
	ШтрихкодыДокумента = ПолучитьИРазобратьШтрихкодыДокумента(Документ, ПараметрыСканирования, СтруктураРеквизитов.ДатаДокумента);
