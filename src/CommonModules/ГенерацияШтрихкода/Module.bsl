...
Функция ИзображениеШтрихкода(ПараметрыШтрихкода) Экспорт
	...
	Если ВнешняяКомпонента = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru = 'Ошибка подключения внешней компоненты печати штрихкода.';
								|en = 'An error occurred while attaching the barcode printing add-in.'", МодульОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если ПараметрыШтрихкода.Свойство("Инвертирован") Тогда      
		...
		#Если НЕ МобильноеПриложениеСервер Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Используется старая версия компоненты генерации штрихкода';
											|en = 'The old version of the barcode generation add-in is used'", 
				МодульОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, 
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		#КонецЕсли
