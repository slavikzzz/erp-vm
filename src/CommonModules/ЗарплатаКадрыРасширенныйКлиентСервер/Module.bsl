...
Функция ПараметрыРедактированияСоставаНачисленийИУдержаний() Экспорт
	...
	ПараметрыОткрытия.Вставить("РедактироватьБухучетНачислений", Ложь);
...
Процедура УстановитьКомментарииДействийСНачислением(СтрокаНачисления, МаксимальноеКоличествоПоказателей, РежимРаботы, ОписаниеТаблицыВидовРасчета = Неопределено, ДатаСобытия = Неопределено, ДатаОкончания = Неопределено) Экспорт
			...
			Если ЗначениеЗаполнено(ДатаОкончания) Тогда
				...
				Если ДатаОкончания < СтрокаНачисления.ДатаОкончанияНачисления
					...
					СтрокаНачисления.Комментарий = СтрокаНачисления.Комментарий + " "
						+ НСтр("ru = 'по';
								|en = 'to'") + " " + Формат(ДатаОкончания, "ДЛФ=D");
					
					Если ЗначениеЗаполнено(СтрокаНачисления.ДатаОкончанияНачисления)
						И СтрокаНачисления.ИспользуетсяПоОкончании Тогда
						
						СтрокаНачисления.Комментарий = СтрокаНачисления.Комментарий + ", "
							+ НСтр("ru = 'затем начисляется до';
									|en = 'then accrued to'") + " " + ПредставлениеДатаОкончанияНачисления;
	...
	ИначеЕсли Не СтрокаНачисления.ДействующийВидРасчета И РасшифровыватьИспользованиеНачисления И ЗначениеЗаполнено(СтрокаНачисления.ДатаОкончанияНачисления) Тогда
		...
		СтрокаНачисления.Комментарий = НСтр("ru = 'Будет начисляться с';
											|en = 'It will be accrued from'") + " " + ПредставлениеДатаОкончанияНачисления;
	...
	Иначе
			...
			Иначе
				...
				Если Значение <> ТекущееЗначение Тогда
					...
					Если НачисляетсяВЦеломЗаМесяц И ДатаСобытия <> Неопределено Тогда
						НачалоДействия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = ' с %1';
																										|en = ' since %1'"), Формат(НачалоМесяца(ДатаСобытия), "ДФ=""д ММММ"""));
					КонецЕсли;
		...
		Если Не ПустаяСтрока(КомментарийТекущегоЗначения)
			...
			Комментарий = Комментарий + НСтр("ru = 'по';
											|en = 'to'") + " " + Формат(ДатаОкончания, "ДЛФ=D")
				+ ", " + НСтр("ru = 'затем';
								|en = 'then'") + " " + КомментарийТекущегоЗначения
		...
		Если РасшифровыватьИспользованиеНачисления Тогда
			...
			Если ЗначениеЗаполнено(ДатаОкончания) Тогда
				...
				ИначеЕсли ДатаОкончания < СтрокаНачисления.ДатаОкончанияНачисления
					...
					Комментарий = Комментарий + " " + НСтр("ru = 'до';
															|en = 'to'") + " " + ПредставлениеДатаОкончанияНачисления;
	...
	Если Значение <> ТекущееЗначение Тогда
		Если ТекущееЗначение Тогда
			Комментарий = НСтр("ru = '(Ранее начисления действовали)';
								|en = '(Previously accruals were relevant)'");
		Иначе	
			Комментарий = НСтр("ru = '(Ранее начисления не действовали)';
								|en = '(Accruals were not valid before)'");
		КонецЕсли;
	КонецЕсли;
	Возврат Комментарий;
КонецФункции 
...
Функция ПрименятьНадбавкуЗаОсобыеКлиматическиеУсловияПоТерриториальнымУсловиям(ТерриториальныеУсловия) Экспорт
	...
	Если ЗначениеЗаполнено(ТерриториальныеУсловия) Тогда
		ПрименятьНадбавкуЗаОсобыеКлиматическиеУсловия = ТерриториальныеУсловияПримененияСевренойНадбавки(ТерриториальныеУсловия)
			Или ТерриториальныеУсловияВКоторыхМожетПрименятьсяСевренаяНадбавка(ТерриториальныеУсловия);
	КонецЕсли;
	Возврат ПрименятьНадбавкуЗаОсобыеКлиматическиеУсловия;
	
КонецФункции

Процедура УстановитьПроцентЗаОсобыеКлиматическиеУсловияОбособленногоПодразделения(Форма, ПроцентЗаОсобыеКлиматическиеУсловия, ТерриториальныеУсловия) Экспорт
	
	Если ТерриториальныеУсловияПримененияСевренойНадбавки(ТерриториальныеУсловия.ТерриториальныеУсловияПФР) Тогда
		ТерриториальныеУсловия.ПрименятьСевернуюНадбавку = Истина;
	ИначеЕсли Не ТерриториальныеУсловияВКоторыхМожетПрименятьсяСевренаяНадбавка(ТерриториальныеУсловия.ТерриториальныеУсловияПФР) Тогда
		ТерриториальныеУсловия.ПрименятьСевернуюНадбавку = Ложь;
	КонецЕсли;
	
	УстановитьПроцентЗаОсобыеКлиматическиеУсловия(Форма, ПроцентЗаОсобыеКлиматическиеУсловия,
...
Процедура УстановитьПроцентЗаОсобыеКлиматическиеУсловия(Форма, ПроцентЗаОсобыеКлиматическиеУсловия, ТерриториальныеУсловия, ПрименятьСевернуюНадбавку = Истина) Экспорт
	...
	Если ПрименятьСевернуюНадбавку Тогда
		...
		ИначеЕсли (ТерриториальныеУсловия = ПредопределенноеЗначение("Справочник.ТерриториальныеУсловияПФР.МКС")
			Или ТерриториальныеУсловия = ПредопределенноеЗначение("Справочник.ТерриториальныеУсловияПФР.РКСР")
			Или ТерриториальныеУсловия = ПредопределенноеЗначение("Справочник.ТерриториальныеУсловияПФР.МКСРКСР")
		...
		ИначеЕсли ТерриториальныеУсловия = ПредопределенноеЗначение("Справочник.ТерриториальныеУсловияПФР.ОКУ")
			Или ТерриториальныеУсловия = ПредопределенноеЗначение("Справочник.ТерриториальныеУсловияПФР.ПРОЧ") Тогда
				
			Если ПрименятьСевернуюНадбавку И ПроцентЗаОсобыеКлиматическиеУсловия = 0 Тогда
	...
	УстановитьОтображениеПолейВводаПроцентаЗаОсобыеКлиматическиеУсловия(Форма, ТерриториальныеУсловия, ПрименятьСевернуюНадбавку);
...
Процедура УстановитьОтображениеПолейВводаПроцентаЗаОсобыеКлиматическиеУсловия(Форма, ТерриториальныеУсловия, ПрименятьСевернуюНадбавку = Истина) Экспорт
	
	Если Форма.ПолучитьФункциональнуюОпциюФормы("ИспользоватьШтатноеРасписание") Тогда
		ПоказыватьПоляВвода = ПрименятьНадбавкуЗаОсобыеКлиматическиеУсловияПоТерриториальнымУсловиям(ТерриториальныеУсловия);
	Иначе
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"Видимость",
		ТерриториальныеУсловияВКоторыхМожетПрименятьсяСевренаяНадбавка(ТерриториальныеУсловия));
...
Функция ТерриториальныеУсловияПримененияСевренойНадбавки(ТерриториальныеУсловия) Экспорт
	Возврат ТерриториальныеУсловияПФРВКоторыхПрименяетсяСевренаяНадбавка().Найти(ТерриториальныеУсловия) <> Неопределено;
КонецФункции

Функция ТерриториальныеУсловияВКоторыхМожетПрименятьсяСевренаяНадбавка(ТерриториальныеУсловия) Экспорт
	Возврат ТерриториальныеУсловияПФРВКоторыхМожетПрименятьсяСевренаяНадбавка().Найти(ТерриториальныеУсловия) <> Неопределено;
КонецФункции

Функция ТерриториальныеУсловияПФРВКоторыхПрименяетсяСевренаяНадбавка()
	СписокУсловий = Новый Массив;
	СписокУсловий.Добавить(ПредопределенноеЗначение("Справочник.ТерриториальныеУсловияПФР.РКСМ"));
	СписокУсловий.Добавить(ПредопределенноеЗначение("Справочник.ТерриториальныеУсловияПФР.МКС"));
	СписокУсловий.Добавить(ПредопределенноеЗначение("Справочник.ТерриториальныеУсловияПФР.РКСР"));
	СписокУсловий.Добавить(ПредопределенноеЗначение("Справочник.ТерриториальныеУсловияПФР.МКСРКСР"));
	СписокУсловий.Добавить(ПредопределенноеЗначение("Справочник.ТерриториальныеУсловияПФР.МКСР"));
	Возврат СписокУсловий
КонецФункции

Функция ТерриториальныеУсловияПФРВКоторыхМожетПрименятьсяСевренаяНадбавка()
	СписокУсловий = Новый Массив;
	СписокУсловий.Добавить(ПредопределенноеЗначение("Справочник.ТерриториальныеУсловияПФР.ОКУ"));
	СписокУсловий.Добавить(ПредопределенноеЗначение("Справочник.ТерриториальныеУсловияПФР.ПРОЧ"));
	Возврат СписокУсловий
КонецФункции
...
Процедура ПланируемаяДатаВыплатыУстановитьОтображениеПредупреждения(Форма, ЭлементФормы, СтрокиДокументаПоФизическимЛицам = Неопределено) Экспорт
	
	Если Не Форма.ПолучитьФункциональнуюОпциюФормы("ИспользоватьРасчетЗарплатыРасширенная") Тогда
		Возврат;
	КонецЕсли; 

	Если СтрокиДокументаПоФизическимЛицам = Неопределено ИЛИ СтрокиДокументаПоФизическимЛицам.Количество() = 0 Тогда
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			...
			ЭлементФормы,
			"ОтображениеПредупрежденияПриРедактировании",
			ОтображатьПредупреждениеПриРедактировании);
КонецПроцедуры
...
Функция ТекстПредупрежденияОписаниеНачисленийИзменилось() Экспорт
	...
	ТекстПредупреждения = НСтр("ru = 'За время работы с документом существенно изменилось описание начислений. 
                                ...
                                |en = 'Description of accruals has significantly changed while working with the document.
                                |Now you should close the document and reopen it, or read the document information again (the Reread command in the More menu).'");
		...
		Если ВключатьВПредставлениеНаименованиеСтажа Тогда
			ПредставлениеКоманды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Стаж ""%1"" не задан';
					|en = 'The ""%1"" service is not specified'"),
				Строка(ВидСтажа));;
		Иначе
			ПредставлениеКоманды = НСтр("ru = 'Стаж не задан';
										|en = 'Length of service is not specified'");
		КонецЕсли;
...
Функция ПредставлениеСтажа(Лет, Месяцев, Дней) Экспорт 
	...
	Иначе 
		...
		Если ПоследняяЦифра = "1" Тогда 
			КоличествоЛетШаблон = ?(ПоследниеДвеЦифры = "11", НСтр("ru = '%1 лет';
																	|en = '%1 years'"), НСтр("ru = '%1 год';
																							|en = '%1'"));
		Иначе
			КоличествоЛетШаблон = ?(Найти("12,13,14", ПоследниеДвеЦифры) = 0, НСтр("ru = '%1 года';
																					|en = '%1'"), НСтр("ru = '%1 лет';
																											|en = '%1 years'"));
		КонецЕсли;
	КонецЕсли;
	...
	Иначе 
		...
		ПоследниеДвеЦифры = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(ПоследниеДвеЦифры, 2, "0");
		Если ПоследняяЦифра = "1" Тогда 
			КоличествоМесяцевШаблон = ?(ПоследниеДвеЦифры = "11", НСтр("ru = '%1 месяцев';
																		|en = '%1 months'"), НСтр("ru = '%1 месяц';
																									|en = '%1 month'"));
		Иначе
			КоличествоМесяцевШаблон = ?(Найти("12,13,14", ПоследниеДвеЦифры) = 0, НСтр("ru = '%1 месяца';
																						|en = '%1 months'"), НСтр("ru = '%1 месяцев';
																												|en = '%1 months'"));
		КонецЕсли;
	КонецЕсли;
	...
	Иначе 
		...
		ПоследниеДвеЦифры = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(ПоследниеДвеЦифры, 2, "0");
		Если ПоследняяЦифра = "1" Тогда 
			КоличествоДнейШаблон = ?(ПоследниеДвеЦифры = "11", НСтр("ru = '%1 дней';
																	|en = '%1 days'"), НСтр("ru = '%1 день';
																							|en = '%1 day'"));
		Иначе
			КоличествоДнейШаблон = ?(Найти("12,13,14", ПоследниеДвеЦифры) = 0, НСтр("ru = '%1 дня';
																					|en = '%1 days'"), НСтр("ru = '%1 дней';
																											|en = '%1 days'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ПредставлениеСтажа <> "" И Дней <> 0 Тогда 
		ПредставлениеСтажа = ПредставлениеСтажа + " " + НСтр("ru = 'и';
															|en = 'and'") + " " + КоличествоДнейТекст;
	КонецЕсли;
...
// Возвращаемое значение:
//  Структура:
// * КраткоеНазваниеИменительныйПадеж - Строка -
// * КраткоеНазваниеРодительныйПадеж - Строка -
// * ИмяРеквизитаСотрудник - Строка -
// * ИмяРеквизитаОтсутствующийСотрудник - Строка -
// * ИмяРеквизитаДатаНачалаСобытия - Строка -
// * ИмяРеквизитаДатаОкончанияСобытия - Строка -
// * ИмяРеквизитаДокументОснование - Строка -
// * ИмяРеквизитаФормыТекущаяПозицияШР - Строка -
// * ИмяТабличнойЧастиСотрудники - Строка -
// * ИмяРеквизитаИдентификаторСтроки - Строка - 
Функция СтруктураОписанияДокумента() Экспорт
	...
	ОписаниеДокумента.Вставить("ИмяРеквизитаФормыТекущаяПозицияШР", "");
	ОписаниеДокумента.Вставить("ИмяТабличнойЧастиСотрудники", "");
	ОписаниеДокумента.Вставить("ИмяРеквизитаИдентификаторСтроки", "");
...
Функция ОбновитьПредставлениеПараметровМодификации(Элементы, СтрокаПараметра) Экспорт
	...
	Если ЕстьДругиеПараметры Тогда
		Строки.Добавить(" ");
		Строки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Еще...';
														|en = 'More...'"),,,,"ДругиеПараметры"));
	КонецЕсли;
