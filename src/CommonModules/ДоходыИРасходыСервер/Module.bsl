	...
	Для каждого Параметры Из ДанныеВыборСтатейИАналитик.МассивПараметровВыбора Цикл
		...
		Если ЗначениеЗаполнено(Параметры.ЭлементыФормы.АналитикаАктивовПассивов) Тогда
			...
			Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<выберите статью активов/пассивов>';
																		|en = '<choose an assets/liabilities item>'"));
			Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
			#КонецОбласти
		КонецЕсли; 
	КонецЦикла;
		...
		Если ВыборкаПоИндексу.База = 0 Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отсутствует база для распределения расходов по правилу %1 для аналитики %2. Укажите другое правило или удалите строку из документа.';
					|en = 'No base for expense allocation according to rule %1 for dimension %2. Specify another rule or remove the line from the document.'"),
...
Функция ДополнитьТекстЗапросаТаблицаОтражениеДокументовВРеглУчете(ЕстьВтПартииПрочихРасходов = Истина, ИмяПериода = "Период") Экспорт
	...
	ШаблонЗапроса = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	&ПереопределяемоеИмяПериодаДокумента КАК Период,
	|	Строки.ОрганизацияПолучатель КАК Организация,
	|	НАЧАЛОПЕРИОДА(Строки.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	&ПереопределяемоеИмяТаблицы КАК Строки
	|ГДЕ
	|	Строки.ОрганизацияПолучатель <> Строки.Организация";
	
	ТекстыЗапроса = Новый Массив;
	ТекстыЗапроса.Добавить(СтрЗаменить(ШаблонЗапроса, "&ПереопределяемоеИмяТаблицы", "ВтПрочиеРасходы"));
	Если ЕстьВтПартииПрочихРасходов Тогда
		ТекстыЗапроса.Добавить(СтрЗаменить(ШаблонЗапроса, "&ПереопределяемоеИмяТаблицы", "ВтПартииПрочихРасходов"));
		
		ТекстЗапроса =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	&ПереопределяемоеИмяПериодаДокумента КАК Период,
		|	Строки.Организация КАК Организация,
		|	НАЧАЛОПЕРИОДА(СтрокиОфлайн.Период, ДЕНЬ) КАК ДатаОтражения
		|ИЗ
		|	ВтПартииПрочихРасходов КАК Строки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПартииПрочихРасходов КАК СтрокиОфлайн
		|			ПО СтрокиОфлайн.Регистратор = &Ссылка
		|				И СтрокиОфлайн.Период <> Строки.Период
		|				И СтрокиОфлайн.РасчетПартий";
		ТекстыЗапроса.Добавить(ТекстЗапроса);
	КонецЕсли;
	
	ТекстЗапроса = СтрСоединить(ТекстыЗапроса, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении(Истина));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПереопределяемоеИмяПериодаДокумента", "&" + ИмяПериода);
...
Процедура Использование_РаспределениеРасходовБудущихПериодов(ПараметрыОбработчика) Экспорт
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьУчетПрочихДоходовРасходов") Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеОтключено(
			...
			НСтр("ru = 'Учет прочих доходов и расходов не ведется';
				|en = 'Accounting of income and expenses is not kept'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ТекстЗапроса = 
		...
		//++ НЕ УТ
		|	ИЛИ (Данные.СуммаНУ <> 0 ИЛИ Данные.БезСуммНУ) И РаспределениеНУ.Ссылка ЕСТЬ NULL
		//-- НЕ УТ
		|";
	...
	Если РазмерыВременныхТаблиц.ВТНераспределенныеРБП = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет незакрытых оборотов по статьям расходов с вариантом распределения ""На расходы будущих периодов""';
				|en = 'No unclosed turnovers for expense item with the ""For deferred expenses"" allocation option'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_РаспределениеРасходовБудущихПериодов(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Нераспределенные расходы будущих периодов';
			|en = 'Unallocated deferred expenses'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Обнаружены нераспределенные расходы по статьям с вариантом распределения ""На расходы будущих периодов"".';
			|en = 'Unallocated expenses of items with the ""To deferred expenses"" allocation option are detected.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаНераспределенныеРасходыБудущихПериодов(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Сумма",		 			НСтр("ru = 'Сумма (с НДС)';
															|en = 'Amount (including VAT)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("СуммаУпр",		 		НСтр("ru = 'Сумма (без НДС)';
															|en = 'Amount (excluding VAT)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("СуммаРегл",		 		НСтр("ru = 'Сумма (БУ)';
															|en = 'Amount (AC)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ НЕ УТ
	СписокПолей.Добавить("СуммаНУ",		 			НСтр("ru = 'Сумма (НУ)';
															|en = 'TA amount'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ПостояннаяРазница",		НСтр("ru = 'Сумма (ПР)';
															|en = 'Amount (PD)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ВременнаяРазница",		НСтр("ru = 'Сумма (ВР)';
														|en = 'Amount (TD)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
	СписокПолей.Добавить("БезСуммУпр",		 		НСтр("ru = 'Автосумма (УУ)';
															|en = 'Autosum (MA)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("БезСуммРегл",		 		НСтр("ru = 'Автосумма (БУ)';
															|en = 'Autosum (AC)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ НЕ УТ
	СписокПолей.Добавить("БезСуммНУ",		 		НСтр("ru = 'Автосумма (НУ)';
															|en = 'Autosum (TA)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
	
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ВТНераспределенныеРБП",
		НСтр("ru = 'В периоде %2 имеются нераспределенные расходы будущих периодов по организации ""%1""';
			|en = 'There are unallocated deferred expenses for %1 company in the %2 period.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Процедура ПроверитьПараметрыВыбораСтатейИАналитик(Форма, МассивПараметровВыбора)
	...
	Для каждого ПараметрыВыбора Из МассивПараметровВыбора Цикл
		...
		Если НЕ ПоискПоТЧ И ЗначениеЗаполнено(ПараметрыВыбора.УсловияДоступностиСтатьиВСтроках) Тогда
			...
			ШаблонСообщения = НСтр("ru = 'Статья располагается в шапке документа. Использование параметра ""%1"" не допускается.';
									|en = 'The item is located at the document header. The parameter ''%1'' cannot be used.'");
		...
		Если ПараметрыВыбора.ВыборСтатьиАктивовПассивов + ПараметрыВыбора.ВыборСтатьиДоходов + ПараметрыВыбора.ВыборСтатьиРасходов > 1 Тогда
			Если НЕ ЗначениеЗаполнено(ПараметрыВыбора.ТипСтатьи) Тогда 
				...
				ШаблонСообщения = НСтр("ru = 'Не указано значение параметра ""%1"" в параметрах выбора статей и аналитик .';
										|en = 'The value of the parameter ""%1"" in parameters of item selection and analytics is not specified.'");
					...
					ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
							...
							СтрШаблон(НСтр("ru = 'Отсутствует реквизит %1 в %2 формы объекта.';
										...
										?(ПоискПоТЧ, СтрШаблон(НСтр("ru = 'табличной части %1';
																	|en = 'table %1'"), ИмяТЧПоиска), НСтр("ru = 'списке реквизитов';
			...
			Для каждого ИмяЭлемента Из ЭлементФормы.Значение Цикл 
				...
				Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Элементы, ИмяЭлемента) Тогда
					ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
							Ошибки,
							"",
							СтрШаблон(НСтр("ru = 'Отсутствует элемент управления %1 на форме.';
											|en = 'No management item %1 on the form.'"), ИмяЭлемента),
							"");
					...
					Если НЕ СтрНайти(ЭлементУправленияФормы.ПутьКДанным, ПутьКДаннымСтатьи)
						...
						ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
							...
							НСтр("ru = 'Путь к данным элемента формы %1 указан некорректно: %2, ожидается %3.';
								|en = 'The path to the data in the item of the form %1 is not correct: %2, expected %3.'"),
								ЭлементФормы.Ключ,
	...
	Если НЕ Ошибки = Неопределено Тогда
		ТекстИсключения = НСтр("ru = 'Найдены ошибки при проверке параметров выбора статей и аналитик';
								|en = 'Errors are found during checking parameters of item and analytics selection'");
