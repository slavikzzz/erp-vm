		...
		Если ПараметрыВыполнения.АдресРезультата = Неопределено Тогда
			Если Не ЗначениеЗаполнено(ПараметрыВыполнения.ИдентификаторФормы) И ОбщегоНазначения.РежимОтладки() Тогда
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(НСтр("ru = 'Длительные операции.Диагностика';
													|en = 'Long-running operations.Diagnostics'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОтменитьВыполнениеЗадания(Знач ИдентификаторЗадания) Экспорт 
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Длительные операции.Отмена выполнения фонового задания';
										|en = 'Long-running operations.Cancel background job'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ОперацияВыполнена(Знач ИдентификаторЗадания, Задание = Неопределено) Экспорт
	...
	Если Задание = Неопределено Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Длительные операции.Фоновое задание не найдено';
										|en = 'Long-running operations.Background job not found'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , Результат.ПодробноеПредставлениеОшибки);
		Результат.Статус = "Ошибка";
		Возврат Результат;
	КонецЕсли;
...
Процедура ВыполнитьСКонтекстомКлиента(ВсеПараметры) Экспорт
	...
	Исключение
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, "8.3.22.2009") < 0 Тогда
			СтекОшибки = НСтр("ru = 'Стек ошибки фонового задания:';
								|en = 'The stack of the background job error:'") + Символы.ПС
...
Процедура ВызватьПроцедуру(ИмяПроцедуры, ПараметрыВызова, ПараметрыВыполнения)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Неверный формат параметра %2 (переданное значение: %1)';
			|en = 'Invalid format of the %2 parameter (passed value: %1).'"), ИмяПроцедуры, "ИмяПроцедуры");
...
Процедура ВызватьФункцию(ИмяФункции, ПараметрыПроцедуры, ПараметрыВыполнения)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Неверный формат параметра %2 (переданное значение: %1)';
			|en = 'Invalid format of the %2 parameter (passed value: %1).'"), ИмяФункции, "ИмяФункции");
...
Функция ИдентификаторВидаОповещения(ВидОповещения)
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Неизвестный вид оповещения длительной операции ""%1"".';
			|en = 'Unknown notification type of the long-running operation: ""%1"".'"), ВидОповещения);
...
Функция ПервыйИдентификаторЗаданияУправляющегоПотока(ИдентификаторПроцесса)
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось найти запись управляющего потока многопоточной длительной операции %1';
			|en = 'Cannot find a record for the thread of control of the %1 multithreaded long-running operation'"),
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При попытке перезапуска фонового задания %1
			           ...
			           |
			           |%3';
			           |en = 'An error occurred while restarting the %1 background job
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Многопоточные длительные операции';
				|en = 'Multithreaded long-running operations'", ОбщегоНазначения.КодОсновногоЯзыка());
