...
Процедура ЗаполнитьПроверкиДляРегистрации(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Корректность перехода на партионный учет версии 2.2';
			|en = 'Whether migration to batch accounting 2.2 is correct'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Проверяется корректность движений по регистрам, сформированных при переходе на партионный учет версии 2.2:
			...
			|records of the ""Data transfer"" kind should be generated as of the end of the month which preceded the migration.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета) Экспорт
	...
	РасшифровкаКодовОшибок.Вставить(20, НСтр("ru = 'Не корректно заполнен кор. вид деятельности НДС для операции ""Возврат товаров между организациями"" для организации на УСН';
											|en = 'The cor VAT activity kind is filled incorrectly for the ""Goods return between companies"" operation for the companies under STS'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(21, НСтр("ru = 'Не корректно заполнен тип записи в движениях документа ""Заявление о ввозе товаров из ЕАЭС""';
											|en = 'Record type in the ""Application for goods import from EAEU"" document register records is invalid'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	ДополнительныеПоля = Новый Структура("ХозяйственнаяОперация",
		НСтр("ru = 'Хозяйственная операция';
			|en = 'Business transaction'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		НСтр("ru = 'переход на партионный учет версии 2.2';
			|en = 'transfer to batch accounting 2.2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		ДополнительныеПоля);
	...
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Различается заполненность ресурсов НДС (упр.) и НДС (регл.) в регистре НДСПредъявленный';
											|en = 'Fullness of the VAT (man.) and VAT (compl.) resources is different in the Charged VAT register '", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(2, НСтр("ru = 'При отключенном управленческом учете заполнен ресурс НДС (упр.) в регистре НДСПредъявленный';
											|en = 'VAT (man.) resource is filled in with disabled management accounting in the Charged VAT register'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		НСтр("ru = 'некорректное заполнение НДС (упр.)';
			|en = 'VAT is filled in incorrectly (man.)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Отсутствуют движения документа ""Счет-фактура полученный (налоговый агент)"" по регистру ""Прочие расходы""';
											|en = 'The ""Received tax invoices (tax agent)"" document register records of the ""Other expenses"" register are missing'"));
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		НСтр("ru = 'отсутствуют движения по регистру ""Прочие расходы""';
			|en = 'records of the ""Other expenses"" register are missing'"));
	...
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Некорректные движения документа ""Оприходование (за счет расходов)"" по регистру ""Прочие расходы""';
											|en = 'Incorrect records of the ""Recording as received (by means of outflows)"" document under the ""Other expenses"" register'"));
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		НСтр("ru = 'некорректные движения по регистру ""Прочие расходы""';
			|en = 'incorrect records under the ""Other expenses"" register'"));
	...
	РасшифровкаКодовОшибок = Новый Соответствие;
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Некорректные движения по регистру ""Детализация партий товаров для НДС и УСН (ФИФО скользящая оценка)""';
											|en = 'Invalid records in the register ""Goods lot details for VAT and STS (FIFO rolling valuation)""'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументы(
		...
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru = 'исправление некорректных движений документа ввода остатков';
			|en = 'correction of invalid records in balance input document'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	// Заполнение направления деятельности в регистре накопления ПартииПрочихРасходов.
	РасшифровкаКодовОшибок = Новый Соответствие;
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Не заполнено направление деятельности в движениях документа ""Резервы отпусков"" по регистру ""Партии прочих расходов"".';
											|en = 'The line of business in the ""Leave reserves"" document records by the ""Material overheads for allocation"" register is not filled.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	|	Т.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И Т.Организация В(&МассивОрганизаций)
	|	И ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.НачислениеОценочныхОбязательствПоОтпускам)
	|	И НЕ Т.РасчетПартий
	|	И НЕ Т.РасчетСебестоимости
	...
	|	ПО Т.Регистратор = ПрочиеРасходы.Регистратор
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru = 'некорректное заполнение направления деятельности в регистре ""Партии прочих расходов""';
			|en = 'incorrect filling of the line of business in the ""Material overheads for allocation"" register'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ДополнитьТипыРегистраторовСНестандартнойЛогикойФормированияДвижений(ТипыРегистраторов) Экспорт
	...
	ТипыРегистраторов.Добавить(Тип("ДокументСсылка.НачислениеОценочныхОбязательствПоОтпускам"));
	ТипыРегистраторов.Добавить(Тип("ДокументСсылка.ПерерасчетИмущественныхНалогов"));
	//-- НЕ УТ
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В регистре накопления ""%1"" по организации ""%2"" на начало периода %3 есть остатки по пустой аналитике учета номенклатуры.';
				|en = 'There is balance of the empty dimension of product accounting in the ""%1""  accumulation register for the ""%2"" company as of the %3 period start.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			Выборка.ИмяРегистра,
		...
		РасчетСебестоимостиПрикладныеАлгоритмы.ЗарегистрироватьПроблемуВыполненияРасчета(
			...
			НСтр("ru = 'При переходе на партионный учет версии 2.2 диагностированы ошибки';
				|en = 'Errors occurred when migrating to batch accounting 2.2'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ТекстДляПротокола);
...
Процедура КорректировкаПереходНаНовыйРегистрДетализацияСебестоимостиТоваров(ПараметрыРасчета, ПараметрыКорректировки)
	...
	Выборка = РасчетСебестоимостиПрикладныеАлгоритмы.ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос,,,, НСтр("ru = 'Организации для корректировки';
																															|en = 'Companies for adjustment'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Если ЗначениеЗаполнено(Выборка.ПериодНовыйРегистр) Тогда
			...
			Если ЗначениеЗаполнено(Выборка.ПериодСтарыйРегистр1)
			 И Выборка.ПериодСтарыйРегистр1 >= Выборка.ПериодНовыйРегистр Тогда
				...
				ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'По организации ""%1"" в предыдущих периодах есть некорректные движения по регистрам партий НДС.';
						|en = 'There are incorrect records by VAT lot registers for the %1 company in previous periods.'", ОбщегоНазначения.КодОсновногоЯзыка()),
					Выборка.Организация);
		...
		Исключение
			...
			ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось создать ключ аналитики без назначения для аналитики учета номенклатуры ""%1"":
					|%2';
					|en = 'Cannot generate the analytics key without the purpose for the nomenclature accounting analytics ""%1"":
		...
		Исключение
			...
			ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось создать ключ аналитики для номенклатуры ""%1"":
					|%2';
					|en = 'Cannot generate the analytics key for the nomenclature ""%1"":
					|%2'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПроверкаКорректностиПереходаНаПартионныйУчетВерсии22(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Обнаружены некорректные движения ""Перенос данных"" по организации ""%1"" в периоде %2';
			|en = 'Incorrect ""Data transfer"" records for the ""%1"" company in %2 period'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"Регистратор");
