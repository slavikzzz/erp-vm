...
Процедура НастроитьКомандуПоказатьСообщения(Объект, ЭлементФормы, Видимость = Истина, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если НепрочитанныхСообщений > 0 Тогда
		ЭлементФормы.Заголовок = СтрШаблон(НСтр("ru = 'Показать непрочитанные сообщения (%1)';
												|en = 'Show unread messages (%1)'"), НепрочитанныхСообщений);
	Иначе
		ЭлементФормы.Заголовок = СтрШаблон(НСтр("ru = 'Показать сообщения (%1)';
												|en = 'Show messages (%1)'"), СтруктураСообщений.Сообщения.Количество());
	КонецЕсли;
