...
Функция СтавкаНДСПоУмолчанию(Знач Организация, Знач Дата = Неопределено, Знач РасчетнаяСтавка = Ложь, Знач НалогообложениеНДС = Неопределено) Экспорт
	...
	УчетНДСЛокализация.УстановитьСтавкуНДСПоУмолчанию(СтавкаНДС, ДатаСтавки, РасчетнаяСтавка, НалогообложениеНДС, Организация);
...
Функция ТекстЗапросаСтавкаНДС(ВыражениеНалогообложениеНДС, ВыражениеНоменклатура = Неопределено, ВыражениеВозвращатьМногооборотнуюТару = Неопределено) Экспорт
	...
	Если ВыражениеНоменклатура <> Неопределено Тогда
		...
		УчетНДСЛокализация.ДополнитьТекстЗапросаСтавкаНДС(ТекстЗапроса,ВыражениеНоменклатура);
...
Функция СтавкаНДСПоУмолчаниюПакетно(КэшированныеЗначения, Знач РасчетнаяСтавка = Ложь, Знач НалогообложениеНДС = Неопределено) Экспорт
	...
	УчетНДСЛокализация.УстановитьСтавкуНДСПоУмолчанию(СтавкаНДС, ДатаСтавки, РасчетнаяСтавка, НалогообложениеНДС, КэшированныеЗначения.ОрганизацияДляСтавкиНДС);
...
Процедура ЗаполнитьНалогообложениеНДСЗакупки(НалогообложениеНДС, ПараметрыЗаполнения, КэшированныеЗначенияПараметров = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыЗаполнения.Договор) Тогда
		...
		|
		|ВЫБРАТЬ
		|	ДоговорыАренды.НалогообложениеНДС КАК НалогообложениеНДС
		|ИЗ
		...
		Если Элемент.Значение = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС Тогда
			...
			ИначеЕсли ПараметрыЗаполнения.ВозвратТоваровПоставщику Тогда
				Элемент.Представление = НСтр("ru = 'Возврат товаров облагается НДС';
											|en = 'Return of goods is subject to VAT'");
			КонецЕсли;
		ИначеЕсли Элемент.Значение = Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС Тогда
			...
			ИначеЕсли ПараметрыЗаполнения.ВозвратТоваровПоставщику Тогда
				Элемент.Представление = НСтр("ru = 'Возврат товаров не облагается НДС';
											|en = 'Return of goods is not subject to VAT'");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
		...
		Если Элемент.Значение = Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС Тогда
			Если ПараметрыЗаполнения.ВозвратТоваровОтКлиента ИЛИ ПараметрыЗаполнения.ВозвратТоваровОтКомиссионера Тогда
				Элемент.Представление = НСтр("ru = 'Возврат не облагается НДС';
											|en = 'Return is not subject to VAT'");
			КонецЕсли;
		КонецЕсли;
...
Функция ТекстыЗапросовСторнирования(МетаданныеДокумента) Экспорт
	...
	Если ДвиженияДокумента.Содержит(МетаданныеРегистра) Тогда
		ТекстЗапроса = ПроведениеДокументов.ТекстСторнирующегоЗапроса(
				МетаданныеРегистра,
				МетаданныеДокумента);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "- ДанныеРегистра.НДСРегл", "0");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "- ДанныеРегистра.НДСУпр", "0");
...
Процедура ОтразитьПриобретениеУПоставщика(Запрос, ТекстыЗапроса, Регистры, ТекстЦенности) Экспорт
	...
	Если Запрос.Параметры.Свойство("ОрганизацияПолучатель") И Запрос.Параметры.Свойство("Период") Тогда
		...
		Запрос.УстановитьПараметр("ИспользоватьРаздельныйУчетВНАПоНалогообложению",
			НастройкиНалоговУчетныхПолитикПовтИсп.РаздельныйУчетВНАПоНалогообложениюНДС(Запрос.Параметры.ОрганизацияПолучатель, Запрос.Параметры.Период));
		Запрос.УстановитьПараметр("ИспользоватьРаздельныйУчетПостатейныхПроизводственныхЗатратПоНалогообложениюНДС",
			НастройкиНалоговУчетныхПолитикПовтИсп.РаздельныйУчетПостатейныхПроизводственныхЗатратПоНалогообложениюНДС(Запрос.Параметры.ОрганизацияПолучатель, Запрос.Параметры.Период));
		//-- НЕ УТ
	ИначеЕсли Запрос.Параметры.Свойство("Организация") И Запрос.Параметры.Свойство("Период") Тогда
		...
		//++ НЕ УТ
		Запрос.УстановитьПараметр("ИспользоватьРаздельныйУчетВНАПоНалогообложению",
			НастройкиНалоговУчетныхПолитикПовтИсп.РаздельныйУчетВНАПоНалогообложениюНДС(Запрос.Параметры.Организация, Запрос.Параметры.Период));
		Запрос.УстановитьПараметр("ИспользоватьРаздельныйУчетПостатейныхПроизводственныхЗатратПоНалогообложениюНДС",
			НастройкиНалоговУчетныхПолитикПовтИсп.РаздельныйУчетПостатейныхПроизводственныхЗатратПоНалогообложениюНДС(Запрос.Параметры.Организация, Запрос.Параметры.Период));
		//-- НЕ УТ
	Иначе
		...
		Запрос.УстановитьПараметр("ИспользоватьРаздельныйУчетПостатейныхПроизводственныхЗатратПоНалогообложениюНДС", Ложь);
		//-- НЕ УТ
	КонецЕсли;
...
#Область НДСПредъявленный
	...
	|						  И &ИспользоватьРаздельныйУчетВНАПоНалогообложению
...
#Область ПартииНДСКРаспределению
	...
	// ПартииПрочихРасходов.ТекстЗапросаТаблицаВтПартииПрочихРасходов()
	|	И НЕ (ЕСТЬNULL(СтатьиРасходов.ВариантРаспределенияРасходовРегл, НЕОПРЕДЕЛЕНО) В (
	...
	|			И &ИспользоватьРаздельныйУчетПостатейныхПроизводственныхЗатратПоНалогообложениюНДС)
	//-- НЕ УТ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|			И ЕСТЬNULL(СтатьиРасходов.ВидЦенностиНДС, НЕОПРЕДЕЛЕНО) В (ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ОС),
	|			                                                           ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.НМА)))
	|			И &ИспользоватьРаздельныйУчетВНАПоНалогообложению)
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ожидает отражения в учете НДС: %1';
					|en = 'Awaiting recording in VAT accounting: %1'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если НЕ ЕстьОжидающиеОтражения И НЕ ЕстьОтраженныеВУчете Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет документов для обработки.';
				|en = 'No documents for processing.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ДополнитьТекстЗапросаЗаданийКЗакрытиюМесяца(Запрос, ТекстЗапроса, ТекстЗапросаВременныхТаблиц, ИменаВременныхТаблиц, ИсточникСвойствДокумента) Экспорт
	...
	//++ НЕ УТ
	ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА = Ложь;
	ДвиженияЗаписываютсяПриОтложенномФормированииДвиженийВНА = Ложь;
	ДвиженияЗаписываютсяПриАктуализацияДвиженийПоВНА = Ложь;
	Если ИсточникСвойствДокумента <> Неопределено
		И ТипЗнч("ИсточникСвойствДокумента") = Тип("Структура") Тогда
			
		ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА = 
			ИсточникСвойствДокумента.Свойство(РасчетСтоимостиВНА.ИмяПризнакаВыполненияОперации());
		
		ДвиженияЗаписываютсяПриОтложенномФормированииДвиженийВНА = 
			ИсточникСвойствДокумента.Свойство(ОтложенноеФормированиеДвиженийВНА.ИмяПризнакаВыполненияОперации());
		
		ДвиженияЗаписываютсяПриАктуализацияДвиженийПоВНА = 
			ИсточникСвойствДокумента.Свойство(РасчетСтоимостиВНА.ИмяПризнакаВыполненияОперацииАктуализацияДвиженийПоВНА());
		
	ИначеЕсли ИсточникСвойствДокумента <> Неопределено Тогда
	
		ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА = 
			РасчетСтоимостиВНА.ДвиженияЗаписываютсяПриВыполненииОперации(ИсточникСвойствДокумента);
			
		ДвиженияЗаписываютсяПриОтложенномФормированииДвиженийВНА = 
			ОтложенноеФормированиеДвиженийВНА.ДвиженияЗаписываютсяПриВыполненииОперации(ИсточникСвойствДокумента);
			
		ДвиженияЗаписываютсяПриАктуализацияДвиженийПоВНА = 
			РасчетСтоимостиВНА.ДвиженияЗаписываютсяПриАктуализацияДвиженийПоВНА(ИсточникСвойствДокумента);
			
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА", ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА); 
	Запрос.УстановитьПараметр("ДвиженияЗаписываютсяПриОтложенномФормированииДвиженийВНА", ДвиженияЗаписываютсяПриОтложенномФормированииДвиженийВНА); 
	Запрос.УстановитьПараметр("ДвиженияЗаписываютсяПриАктуализацияДвиженийПоВНА", ДвиженияЗаписываютсяПриАктуализацияДвиженийПоВНА); 
	//-- НЕ УТ
...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	ОписаниеВарианта.Описание =
		НСтр("ru = 'Анализ учета НДС по зачетам полученных авансов в разрезе контрагентов и документов оплаты';
			|en = 'Analyze VAT accounting by prepayments from customers with a breakdown by counterparties and AR/AP transactions'");
	#КонецОбласти
	...
	ОписаниеВарианта.Описание =
		НСтр("ru = 'Обороты по входящему и исходящему НДС за период.';
			|en = 'Input and output VAT turnovers for the period.'");
	#КонецОбласти 
...
Функция ОписаниеПараметровОперацииЗакрытияМесяца() Экспорт
	...
	ОписаниеПараметра.Наименование 		  = НСтр("ru = 'Количество заданий для порционного расчета НДС';
													|en = 'Number of tasks for batch VAT calculation'");
	...
	ОписаниеПараметра.Описание =
		НСтр("ru = 'Расчет НДС в закрытии месяца выполняется не для всех заданий разом, а обсчитываются отдельные порции.
              ...
              |en = 'VAT calculation in the month-end closing is not performed for all tasks at once, but separate batches are calculated.
              |These are requests in a loop, so performance is degraded. The required amount of server memory for calculation is significantly reduced.
              |You can increase the calculation batch size to find a compromise between the server memory and the stage calculation duration.'");
	...
	ОписаниеПараметра.Наименование 		  = НСтр("ru = 'Порог списания погрешностей округления';
													|en = 'Threshold of error rounding write-off'");
	...
	ОписаниеПараметра.Описание =
		НСтр("ru = 'При расчете партий НДС могут возникать погрешности округления, из-за которых в регистре “НДС предъявленный” могут оставаться копеечные остатки.
              ...
              |en = 'When calculating VAT lots, rounding errors may occur, which may result in pennyworth balances in the ""Charged VAT"" register.
              |If you specify a non-zero threshold for writing off a rounding error, then all balances within this threshold will be written off as expenses to the ""Goods value variance"" item.
              |If the threshold is equal to zero, then the rounding error write-off is not performed.'");
