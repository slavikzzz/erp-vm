...
Функция ОтчетОбъект(Идентификатор) Экспорт
	...
	Иначе
		ВызватьИсключение СтрЗаменить(НСтр("ru = 'Некорректное полное имя отчета ""%1"".';
											|en = 'Report %1 has invalid name.'"), "%1", ПолноеИмя);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтрЗаменить(НСтр("ru = '""%1"" не является отчетом.';
											|en = '%1 is not a report.'"), "%1", ПолноеИмя);
	КонецЕсли;
КонецФункции
...
Процедура ПерегруппироватьЭлементыФормыНастроек(Форма, Знач УзелИерархииЭлементов, СвойстваЭлементов, ИменаРеквизитов, ВидыСтилизованныхЭлементов)
	...
	Для Каждого ИдентификаторГруппы Из ИдентификаторыГрупп Цикл 
		...
		Если Группа = Неопределено Тогда 
			...
			Группа.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Строка %1';
																							|en = 'Row %1'"), НомерГруппы);
		КонецЕсли;
...
Процедура ДобавитьОтступКолонок(Форма, ЭлементыНастроек, УзелИерархииЭлементов)
	...
	Для НомерКолонки = 2 По КоличествоКолонок Цикл 
		...
		ГруппаПолейКолонки.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поля колонки %1';
																									|en = 'The %1 column fields'"), НомерКолонки);
...
Функция ИерархияЭлементовФормыНастроек(Форма, Родитель, Признаки, НомерСтроки, НомерКолонки)
	...
	Строка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Строка %1.%2';
			|en = 'Row %1.%2'"), НомерКолонки, НомерСтроки);
	...
	Колонка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Колонка %1.%2.1';
			|en = 'Column %1.%2.1'"), НомерКолонки, НомерСтроки);
...
Функция ЗаголовокЭлементаПериода(Свойство, ЗаголовокЭлементаНастройки, ВыводитьЗаголовкиПериодов)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		?(СтрЗаканчиваетсяНа(НРег(Свойство), НРег("ДатаНачала")), НСтр("ru = 'начала';
																		|en = 'start'"), НСтр("ru = 'окончания';
																								|en = 'end'")));
