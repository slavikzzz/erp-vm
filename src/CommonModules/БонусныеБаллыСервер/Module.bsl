	...
	Пока ПовторитьОбработку Цикл
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			ТекстСообщения = НСтр("ru = 'При выполнении обработки возвратов списаний бонусных баллов произошла ошибка: %1';
									|en = 'An error occurred while processing refunds of bonus point write-offs: %1'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1", ТекстОшибки);
			
			ЗаписьЖурналаРегистрации(
				НСтр(
					"ru = 'Обработка возврата списания бонусных баллов';
					|en = 'Process the refund of bonus point write-off'",
	...
	Пока ПовторитьПостановку Цикл
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			ТекстСообщения = НСтр("ru = 'При выполнении постановке к обработке документов начисения и списания произошла ошибка: %1';
									|en = 'An error occurred when processing accumulation and write-off documents: %1'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1", ТекстОшибки);
			
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Постановка к обработке документов начисления и списания';
											|en = 'Process accumulation and write-off documents'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
			Прервать;
		КонецЕсли;
