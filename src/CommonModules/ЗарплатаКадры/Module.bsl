...
Процедура ОрганизацииФормаЭлементаПриСозданииНаСервере(Форма) Экспорт
	...
	Если Форма.Параметры.Свойство("ИмяРеквизита") И Не ПустаяСтрока(Форма.Параметры.ИмяРеквизита) Тогда
		...
		Если Не ПустаяСтрока(Форма.Параметры.ИмяРеквизита) Тогда
			...
			ИначеЕсли Форма.Параметры.ИмяРеквизита = "РегНомерСФР" Тогда
				ИмяРеквизита = "ОрганизацияРегистрационныйНомерСФР";
				ИмяСтраницы = "ГруппаФонды";
...
Функция РегистрацияВНалоговомОрганеПодразделения(ПодразделениеОрганизации, Знач ДатаАктуальности = Неопределено) Экспорт
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		ПодразделениеОрганизации, "ОбособленноеПодразделение,Родитель,Владелец");
	
	Если ЗначенияРеквизитов.ОбособленноеПодразделение Тогда
		Возврат Справочники.РегистрацииВНалоговомОргане.ПустаяСсылка();
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ЗначенияРеквизитов.Родитель) Тогда
		Возврат РегистрацияВНалоговомОргане(ЗначенияРеквизитов.Родитель, ДатаАктуальности);
	ИначеЕсли ЗначениеЗаполнено(ЗначенияРеквизитов.Владелец) Тогда
		Возврат РегистрацияВНалоговомОргане(ЗначенияРеквизитов.Владелец, ДатаАктуальности);
	КонецЕсли;
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ПроцессыОбработкиДокументовЗарплата") Тогда	
		...
		МодульПроцессыОбработкиДокументовЗарплата.ПараметрыРаботыКлиентаПриЗапуске(Параметры);
	КонецЕсли;
	// Конец ПроцессыОбработкиДокументов
	Модуль = Модуль();
	// Установка неразделенных параметров работы клиента.
	Модуль.ПриДобавленииНеразделенныхПараметровРаботыКлиентаПриЗапуске(Параметры);
	// Установка разделенных параметров работы клиента.
...
Процедура ПриНачальномЗаполненииРолейИсполнителей(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	БизнесПроцессыЗаявокСотрудников.ПриНачальномЗаполненииРолейИсполнителей(КодыЯзыков, Элементы, ТабличныеЧасти);
...
Процедура ПриЗаполненииРеквизитовОрганизации(Организация, Реквизиты, СтандартнаяОбработка) Экспорт
	Если ТипЗнч(Организация) <> Тип("СправочникСсылка.Организации") Или Не ЗначениеЗаполнено(Организация) Тогда
		Возврат;
	КонецЕсли;
	...
	СведенияОбОрганизации = СведенияОбОрганизации(Организация, ИменаПолей, Период);
	
	// Передача сведений об организации.
	Реквизиты.НаименованиеСокращенное          = СведенияОбОрганизации.НаимЮЛСокр;
	Реквизиты.НаименованиеПолное               = СведенияОбОрганизации.НаимЮЛПол;
	Реквизиты.ЭтоИндивидуальныйПредприниматель = Не СведенияОбОрганизации.ЭтоЮридическоеЛицо;
	Реквизиты.ЭтоФилиал                        = СведенияОбОрганизации.ГоловнаяОрганизация <> Организация;
	...
	Реквизиты.ЭлектроннаяПочта                 = СведенияОбОрганизации.АдрПочт;
	Реквизиты.ЭлектроннаяПочтаЗначение         = СведенияОбОрганизации.АдрПочт_JSON;
	Реквизиты.Телефон                          = СведенияОбОрганизации.ТелОрганизации;
	Реквизиты.ТелефонЗначение                  = СведенияОбОрганизации.ТелОрганизации_JSON;
	Реквизиты.ЮридическийАдрес                 = СведенияОбОрганизации.АдрЮР;
	Реквизиты.ЮридическийАдресЗначение         = СведенияОбОрганизации.АдрЮР_JSON;
	...
	Иначе
		Реквизиты.ЛицоБезДоверенности = СведенияОбОрганизации.РукСсылка;
	КонецЕсли;
	Если ЗначениеЗаполнено(Реквизиты.ЛицоБезДоверенности) Тогда
		РеквизитыЛицаБезДоверенности = МашиночитаемыеДоверенностиФНС.РеквизитыПредставителяОрганизации(
			Реквизиты.ЛицоБезДоверенности, Реквизиты.ЭтоИндивидуальныйПредприниматель, СведенияОбОрганизации.ДолжнРук);
		ПриЗаполненииРеквизитовФизическогоЛица(Реквизиты.ЛицоБезДоверенности, РеквизитыЛицаБезДоверенности);
...
Процедура ПриЗаполненииРеквизитовФизическогоЛица(ФизическоеЛицо, Реквизиты) Экспорт
	Если ТипЗнч(ФизическоеЛицо) <> Тип("СправочникСсылка.ФизическиеЛица") Или Не ЗначениеЗаполнено(ФизическоеЛицо) Тогда
		Возврат;
	КонецЕсли;
	...
	ДанныеФизическогоЛица = КадровыйУчет.КадровыеДанныеФизическогоЛица(Ложь, ФизическоеЛицо, СписокПолей, ТекущаяДатаСеанса());
	...
	Реквизиты.БезГражданства = Не ЗначениеЗаполнено(Реквизиты.Гражданство);
	...
	Если ЗначениеЗаполнено(ДанныеФизическогоЛица.ДокументВид) Тогда
		Реквизиты.ДокументВид = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеФизическогоЛица.ДокументВид, "КодМВД");
	КонецЕсли;
	Реквизиты.ДокументНомер = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1 %2';
			|en = '%1 %2'"), ДанныеФизическогоЛица.ДокументСерия, ДанныеФизическогоЛица.ДокументНомер);
	Реквизиты.ДокументКемВыдан = ДанныеФизическогоЛица.ДокументКемВыдан;
	Реквизиты.ДокументКодПодразделения = ДанныеФизическогоЛица.ДокументКодПодразделения;
	Реквизиты.ДокументДатаВыдачи = ДанныеФизическогоЛица.ДокументДатаВыдачи;
	
	Если ЗначениеЗаполнено(ДанныеФизическогоЛица.EMail) Тогда
		Реквизиты.ЭлектроннаяПочтаЗначение = ДанныеФизическогоЛица.EMail;
		Реквизиты.ЭлектроннаяПочта = ДанныеФизическогоЛица.EMailПредставление;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДанныеФизическогоЛица.ТелефонРабочий) Тогда
		Реквизиты.ТелефонЗначение = ДанныеФизическогоЛица.ТелефонРабочий;
		Реквизиты.Телефон         = ДанныеФизическогоЛица.ТелефонРабочийПредставление;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДанныеФизическогоЛица.АдресПоПрописке) Тогда
		Реквизиты.АдресРегистрацииЗначение = ДанныеФизическогоЛица.АдресПоПрописке;
		Реквизиты.АдресРегистрации = ДанныеФизическогоЛица.АдресПоПропискеПредставление;
	КонецЕсли;
...
Процедура ПриОпределенииНастроекПечати(Настройки) Экспорт
	...
	ЗарплатаКадрыВнутренний.ПриОпределенииНастроекПечати(Настройки);
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Для Каждого СтрокаИсточник Из Источники.Строки Цикл
		...
		ДобавитьКомандыБЗК = ЭтоОбъектЗарплатноКадровойБиблиотеки(СтрокаИсточник.ПолноеИмя)
			Или ОбъектМетаданныхВключенВПодсистемы(СтрокаИсточник.ПолноеИмя, "СтандартныеПодсистемы")
			Или ОбъектМетаданныхВключенВПодсистемы(СтрокаИсточник.ПолноеИмя, "РегламентированнаяОтчетность");
			
		Если ДобавитьКомандыБЗК Тогда
			ЗарплатаКадрыВнутренний.ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники,
...
Процедура ПриПолученииСпискаШаблоновОчередиЗаданий(Шаблоны) Экспорт
	...
	ИнтеграцияКабинетСотрудника.ПриПолученииСпискаШаблоновОчередиЗаданий(Шаблоны);
	Интеграция1СПерсонал.ПриПолученииСпискаШаблоновОчередиЗаданий(Шаблоны);
...
Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	...
	ИнтеграцияКабинетСотрудника.ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам);
	Интеграция1СПерсонал.ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам);
...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	ИнтеграцияКабинетСотрудника.ПриОпределенииНастроекРегламентныхЗаданий(Настройки);
	Интеграция1СПерсонал.ПриОпределенииНастроекРегламентныхЗаданий(Настройки);
...
Процедура ПриПолученииНаименованийНаборовСвойств(Наименования, КодЯзыка) Экспорт
	ЗарплатаКадрыВнутренний.ПриПолученииНаименованийНаборовСвойств(Наименования, КодЯзыка);
КонецПроцедуры
...
Процедура ЗаполнитьНаборыСвойствОбъекта(Объект, ТипСсылки, НаборыСвойств, СтандартнаяОбработка, КлючНазначения) Экспорт
	ЗарплатаКадрыВнутренний.ЗаполнитьНаборыСвойствОбъекта(Объект, ТипСсылки, НаборыСвойств, СтандартнаяОбработка, КлючНазначения);
КонецПроцедуры
...
Процедура ПриЗаполненииТиповИсключаемыхИзВыгрузкиЗагрузки(Типы) Экспорт
	...
	Типы.Добавить(Метаданные.РегистрыСведений.МакетыКомпоновкиДанных);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КонфигурацииЗарплатаКадры") Тогда
		...
		МодульКонфигурацииЗарплатаКадры.ПриЗаполненииТиповИсключаемыхИзВыгрузкиЗагрузки(Типы);
	КонецЕсли;
...
// См. ЭлектронныйДокументооборотСКонтролирующимиОрганамиПереопределяемый.ПолучитьФизЛицоПоСНИЛС.
Процедура ПриПолученииФизическогоЛицаПоСНИЛС(СНИЛС, ФизическоеЛицо) Экспорт
...
Процедура ПроверитьНеобходимостьЗапускаРегламентногоЗаданияСЭДО(Страхователи, ТребуетсяЗапуск) Экспорт
...
Процедура ПослеПолученияОтветаНаПодтверждениеОПрочтении(Страхователь, РезультатыПодтверждений, Результат) Экспорт
	...
	УчетПособийСоциальногоСтрахования.ПослеПолученияОтветаНаПодтверждениеОПрочтении(Страхователь, РезультатыПодтверждений, Результат);
...
Процедура ПослеПолученияПодтвержденияДоставкиОператоромСообщенияСЭДО(Страхователь, ИсходноеСообщение, СтатусПодтверждения, Результат) Экспорт
	...
	УчетПособийСоциальногоСтрахования.ПослеПолученияПодтвержденияДоставкиОператоромСообщенияСЭДО(
		Страхователь,
		ИсходноеСообщение,
		СтатусПодтверждения,
		Результат);
...
Процедура ПослеПолученияОшибкиДоставкиОператоромСообщенияСЭДО(Страхователь, ИсходноеСообщение, ТекстОшибки, Результат) Экспорт
	...
	УчетПособийСоциальногоСтрахования.ПослеПолученияОшибкиДоставкиОператоромСообщенияСЭДО(
		Страхователь,
		ИсходноеСообщение,
		ТекстОшибки,
		Результат);
...
// См. ЭлектронныйДокументооборотСФССПереопределяемый.ЗаполнитьИдентификаторыПроактивБезОтвета.
Процедура ЗаполнитьИдентификаторыПроактивБезОтвета(ТаблицаЗапросов, СписокИдентификаторов, МинимальнаяДатаОтвета) Экспорт
	Если ЗначениеЗаполнено(МинимальнаяДатаОтвета) Тогда
		МинимальнаяДатаСозданияОтвета = МинимальнаяДатаОтвета;
	Иначе
		МинимальнаяДатаСозданияОтвета = НачалоДня(ТекущаяДатаСеанса() - 3*86400);
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("МинимальнаяДатаСозданияОтвета", МинимальнаяДатаСозданияОтвета);
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВходящийЗапросФонда.ИдентификаторСообщения КАК ИдентификаторФСС,
	|	ВходящийЗапросФонда.Страхователь КАК Организация,
	|	ВходящийЗапросФонда.Представление КАК ВидДокумента,
	|	ОтветНаЗапросФонда.ИдентификаторСообщения КАК ИдентификаторСообщенияОтвета,
	|	ОтветНаЗапросФонда.ДатаОтправки КАК ДатаОтправкиВФСС,
	|	ОтветНаЗапросФонда.ДатаСоздания КАК Дата
	|ИЗ
	|	Документ.ОтветНаЗапросФССДляРасчетаПособия КАК ОтветНаЗапросФонда
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВходящийЗапросФССДляРасчетаПособия КАК ВходящийЗапросФонда
	|		ПО ОтветНаЗапросФонда.Ссылка = ВходящийЗапросФонда.ОтветНаЗапрос
	|ГДЕ
	|	ОтветНаЗапросФонда.ДатаСоздания >= &МинимальнаяДатаСозданияОтвета
	|	И ОтветНаЗапросФонда.ДатаОтправки = ДАТАВРЕМЯ(1, 1, 1)
	|	И ОтветНаЗапросФонда.ИдентификаторСообщения = """"";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		СтрокаЗапроса = ТаблицаЗапросов.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаЗапроса, Выборка);
		СтрокаЗапроса.НомерПроблемы = 2;
		СписокИдентификаторов.Добавить(Выборка.ИдентификаторФСС);
	КонецЦикла;
КонецПроцедуры
...
Процедура ПолучитьНастройкиВидимостиЭлементовФормы(ИДОтчета, ИДРедакцииОтчета, НастройкиВидимости) Экспорт
	...
	Если ЭтоНоваяФормаОтчета Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Автоматическое заполнение этой редакции формы будет реализовано в будущих версиях программы.';
									|en = 'Automatic filling of this form revision will be implemented in future application versions.'");
...
Процедура ОпределитьНастройки(Настройки) Экспорт
	...
	ЗарплатаКадрыВнутренний.ОпределитьНастройки(Настройки);
...
Процедура ДополнитьДанныеСубъектовПерсональныхДанных(СубъектыПерсональныхДанных, ДатаАктуальности) Экспорт
	...
	ЗащитаПерсональныхДанныхЗарплатаКадры.ДополнитьДанныеСубъектовПерсональныхДанных(СубъектыПерсональныхДанных,
		ДатаАктуальности);
...
Процедура ДополнитьДанныеОрганизацииОператораПерсональныхДанных(Организация, ДанныеОрганизации, ДатаАктуальности) Экспорт
	...
	ЗащитаПерсональныхДанныхЗарплатаКадры.ДополнитьДанныеОрганизацииОператораПерсональныхДанных(Организация,
		ДанныеОрганизации, ДатаАктуальности);
...
Процедура ПередСкрытиемПерсональныхДанныхСубъектов(Субъекты, ТаблицаИсключений, ОтказОтСкрытия) Экспорт
	...
	ЗащитаПерсональныхДанныхЗарплатаКадры.ПередСкрытиемПерсональныхДанныхСубъектов(Субъекты, ТаблицаИсключений,
		ОтказОтСкрытия);
...
Процедура ПередУничтожениемПерсональныхДанных(Объект, Субъекты, ВыполнитьЗаписьОбъекта) Экспорт
	...
	ЗащитаПерсональныхДанныхЗарплатаКадры.ПередУничтожениемПерсональныхДанных(Объект, Субъекты,
...
Процедура ПриОпределенииБизнесПроцессов(ПодключенныеБизнесПроцессы) Экспорт
	...
	ЗаявкиСотрудников.ПриОпределенииБизнесПроцессов(ПодключенныеБизнесПроцессы);
...
// Возвращает признак заполнения и отправки регистрационного номера СФР в исходящих документах.
//
// Параметры:
//   Дата - Дата - Дата, на которую нужна проверка возможности использования реквизита РегистрационныйНомерСФР.
//       В справочнике Организации и связанных с ним формах редактирования периодических сведений дата не указывается.
//       В документах может указываться. Например, для печатных форм можно передавать дату документа.
//         Для электронных документов можно указывать дату последнего автозаполнения регистрационного номера СФР,
//         или дату, максимально близкую к дате последнего автоматического заполнения регномера СФР.
//       Если дата не указана, то проверяется текущая дата Фонда (см. СЭДОФСС.ТекущаяДатаФонда).
//         При этом допустимо в каком-либо виде сохранять видимость регномера в документе (например,
//         в виде версии спецификации, или в виде такого косвенного признака как заполненость регномера).
//   Организация - СправочникСсылка.Организации, Неопределено - Организация, для которой выполняется проверка.
//       Если у страхователя организации не заполнен регномер ПФР, то функция возвращает Истина.
//
// Возвращаемое значение:
//   Булево - Истина, если регистрационный номер СФР можно заполнять / отправлять в Фонд.
//
Функция ИспользованиеРегистрационногоНомераСФР(Знач Дата = Неопределено, Знач Организация = Неопределено) Экспорт
	
	Если Не ЗначениеЗаполнено(Дата) Тогда
		Дата = СЭДОФСС.ТекущаяДатаФонда();
	КонецЕсли;
	
	Возврат Дата >= ЗарплатаКадрыПовтИсп.ДатаВступленияВСилуНА("НачалоПереходаНаРегистрационныйНомерСФР")
		Или (ЗначениеЗаполнено(Организация) И Не ЗарплатаКадрыПовтИсп.РегномерПФРЗаполнен(Организация));
			...
			Исключение
				ТекстИсключенияЗаписи = СтрШаблон(
					НСтр("ru = 'Не удалось изменить подчиненное подразделение %1 при записи %2.
					           ...
					           |en = 'Cannot change subordinate business unit %1 when writing %2.
					           |Maybe business unit is being edited by another user.'"),
					ПодразделениеОбъект.Наименование,
					?(ИсточникДанных = Неопределено, НСтр("ru = 'всех подразделений';
															|en = 'all business units'"), ИсточникДанных.Наименование));
...
Функция КраткийСоставСотрудниковПоСпискуФизическихЛиц(ФизическиеЛица, ДатаАктуальности) Экспорт
	...
	Для Каждого Физлицо Из ФизическиеЛица Цикл
		...
		Если Не ЗначениеЗаполнено(Физлицо) Или УникальныеФизлица[Физлицо] <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗащитаПерсональныхДанных") Тогда
			...
			Если МодульЗащитаПерсональныхДанных.ЭтоОбъектСУничтоженнымиПерсональнымиДанными(Физлицо) Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбработкаПолученияДанныхВыбораСправочникаОрганизации(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	ЗарплатаКадрыВнутренний.ОбработкаПолученияДанныхВыбораСправочникаОрганизации(ДанныеВыбора, Параметры, СтандартнаяОбработка);
КонецПроцедуры
...
Функция СведенияОбОрганизации(Знач Организация, Знач ИменаПолей, Знач ДатаПолучения = Неопределено) Экспорт
	...
	Если РезультатБРО <> Неопределено Тогда
	...
	Если Результат.Свойство("НомерСтрахователяФСС") И Результат.НомерСтрахователяФСС = Неопределено Тогда
		ЭтоГоловнаяОрганизация = ГоловнаяОрганизация(Организация) = Организация;
		...
		КонецЕсли;
	КонецЕсли;
	Если Результат.Свойство("ГоловнаяОрганизация") И Результат.ГоловнаяОрганизация = Неопределено Тогда
		Результат.ГоловнаяОрганизация = ГоловнаяОрганизация(Организация);
	КонецЕсли;
...
Процедура НастроитьОтображениеГруппыПодписей(Группа, 
	...
	Если ИОФамилии.Количество() > 0 Тогда
		Группа.ЗаголовокСвернутогоОтображения = ЗаголовокСвернутогоОтображенияГруппы(НСтр("ru = 'Подписи';
																							|en = 'Signatures'"), ИОФамилии);
	Иначе	
		Группа.ЗаголовокСвернутогоОтображения = НСтр("ru = 'Подписи не указаны';
													|en = 'Signatures are not defined'");
	КонецЕсли	
...
Функция ЗаголовокСвернутогоОтображенияГруппы(Знач ЗаголовокГруппы, ОтображаемыеЗначения, Знач ПредставлениеНезаполненного = Неопределено, Знач ПредставлениеЧастичноЗаполненного = Неопределено)
	...
	Если ПредставлениеЧастичноЗаполненного = Неопределено Тогда
		ПредставлениеЧастичноЗаполненного = НСтр("ru = '(не все поля заполнены)';
												|en = '(not all fields are filled in)'")	
	КонецЕсли;	
...
Функция КлючевыеРеквизитыПредставлениеПредупреждения(ПредставлениеРеквизита, ДействиеПриИзмененииОчистка)
	...
	Если ДействиеПриИзмененииОчистка Тогда
		ПредставлениеПредупреждения = НСтр("ru = 'Редактирование %1 приведет к очистке табличных частей документа';
											|en = 'Editing of %1 will result in clearing of the document tabular sections'");
	Иначе 
		ПредставлениеПредупреждения = НСтр("ru = 'Редактирование %1 приведет к перезаполнению табличных частей документа';
											|en = 'Editing of %1 will result in repopulation of the document tabular sections'");
	КонецЕсли;
				...
				Если ЗначениеЗаполнено(СтрокаДанных.ДатаЗавершенияРаботы) Тогда
					...
					ВызватьИсключение СтрШаблон(
						НСтр("ru = 'Сотрудник ""%1"" был оформлен на работу с %2, уволен %3';
							|en = 'Employee ""%1"" was hired from %2, dismissed %3'"),
						ДанныеЗаполнения,
					...
					Если ЗначениеЗаполнено(СтрокаДанных.ДатаПриема) Тогда
						...
						ВызватьИсключение СтрШаблон(
							НСтр("ru = 'Сотрудник ""%1"" уже оформлен на работу с %2';
								|en = 'Employee ""%1"" is already hired since %2'"),
							ДанныеЗаполнения,
							Формат(СтрокаДанных.ДатаПриема, "ДЛФ=DD"));
					...
					Иначе
						...
						ВызватьИсключение СтрШаблон(
							НСтр("ru = 'Сотрудник ""%1"" уже оформлен на работу';
								|en = 'Employee ""%1"" is already hired'"),
							ДанныеЗаполнения);
...
Процедура ДобавитьПараметрОтбора(СтруктураПараметровОтбора, ИмяПараметра, ОписаниеТипа,	Представление = Неопределено,
	ИмяМодификации = Неопределено) Экспорт
...
Процедура ПриСозданииНаСервереФормыСДинамическимСписком(Форма, НаименованиеРеквизитаСписок, ГруппаНастроек = "СписокНастройкиОтбора",
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ИмяЗаменяемойФормы") Тогда
		ИмяФормы = Форма.ИмяЗаменяемойФормы;
	Иначе
		ИмяФормы = Форма.ИмяФормы;
	КонецЕсли;
	ТипОбъекта = ТипЗнч(ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ИмяФормы));
	...
	Если КоличествоВыведенныхСтрок > ОграничениеСтрок Тогда
		...
		ТекстНадписи = СтрШаблон(
				НСтр("ru = 'Продолжение таблицы см. в дополнении к документу №%1.';
					|en = 'See continued table in addition to document No. %1'"),
...
Процедура СоздатьВТПоЗапросу(МенеджерВременныхТаблиц, Запрос, ИмяВТ, ИндексВТ = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Запрос, "МенеджерВременныхТаблиц") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ru = 'Менеджер временных таблиц отличается от установленного в запросе';
				|en = 'TT manager differs from the one specified in the request'"),
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		НСтр("ru = 'В запросе нет оператора ВЫБРАТЬ';
			|en = 'No SELECT statement in request'"),
		"ЗарплатаКадры.СоздатьВТПоЗапросу");
		...
		Если ЗагружаемыеСведения.Версия < ТекущаяВерсия Тогда
			Если Не ПолучатьДанныеИзСервиса Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Нормативно-правовая информация БЗК';
						|en = 'PCL regulatory information'", ОбщегоНазначения.КодОсновногоЯзыка()),
					...
					СтрШаблон(
						НСтр("ru = 'Версия %1 классификатора ""%2"" не загружена, т.к. уже загружена более новая версия %3.
						           ...
						           |en = 'The version %1 of the classifier ""%2"" is not uploaded because the newer version %3 is already uploaded.
						           |It is recommended to check the presence of configuration updates.'"),
		...
		Если КоличествоБудетЗагружено = 0 Тогда
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Классификатор ""%1"" не обновлен по причине:
				           ...
				           |
				           |Рекомендуется проверить наличие обновлений конфигурации.';
				           |en = 'Classifier ""%1"" is not updated due to:
				           |%2.
				           |
				           |It is recommended that you check availability of configuration updates.'"),
				ОбъектМетаданных.Представление(),
		...
		Иначе
			...
			ТекстЖурнала = СтрШаблон(
				НСтр("ru = 'Классификатор ""%1"" обновлен частично: Загружено %2 из %3 записей; %4 записей не загружено по причине:
				           ...
				           |
				           |Рекомендуется проверить наличие обновлений конфигурации.';
				           |en = 'Classifier ""%1"" is partially updated: %2 out of  %3 records are imported; %4 records were not imported due to:
				           |%5.
				           |
				           |It is recommended that you check the availability of configuration updates.'"),
				ОбъектМетаданных.Представление(),
	...
	Иначе
		...
		ТекстЖурнала = СтрШаблон(
			НСтр("ru = 'Обновлен классификатор ""%1"" (загружено %2 записей).';
				|en = 'Classifier ""%1"" is updated (%2 records are imported).'"),
			ОбъектМетаданных.Представление(),
			КоличествоБудетЗагружено);
	КонецЕсли;
		...
		Для Каждого ИмяКолонки Из ИменаКолонокДляПреобразованияСсылок Цикл
			...
			Если ЗначениеССсылками = Неопределено Тогда
				Если ТаблицаОшибок <> Неопределено Тогда
					...
					СтрокаОшибки.ТекстОшибки = СтрШаблон(НСтр("ru = '""%1"" отсутствует в данных';
																|en = '%1 is missing in data'"), ЗначениеБезСсылок);
				КонецЕсли;
...
// Устарела. В карточке организации следует отказаться от использования. В документах следует перейти на функцию
//   ИспользованиеРегистрационногоНомераСФР.
//
// Параметры:
//   Дата - Дата - Дата, на которую нужна проверка видимости реквизита РегистрационныйНомерСФР.
//       В справочнике Организации и связанных с ним формах редактирования периодческих сведений дата не указывается.
//       В документах может указываться. Например, для печатных форм можно передавать дату документа.
//         Для электронных документов может быть актуальна проверка на дату заполнения регистрационного номера СФР,
//         или на дату, максимально близкую к дате автоматического заполнения регномера СФР.
//       Если дата не указана, то проверяется текущая дата Фонда (см. СЭДОФСС.ТекущаяДатаФонда).
//         При этом допустимо в каком-либо виде сохранять видимость регномера в документе (например,
//         в виде версии спецификации, или в виде такого косвенного признака как заполненость регномера).
//
// Возвращаемое значение:
//   Булево - Истина, если регистрационный номер СФР можно показать пользователю.
//
Функция ВидимостьРегистрационногоНомераСФР(Знач Дата = Неопределено) Экспорт
	Возврат ИспользованиеРегистрационногоНомераСФР(Дата);
КонецФункции
...
Процедура ПроверитьКорректностьМесяца(Ссылка, ПроверяемыйМесяц, ПутьКПроверяемомуМесяцу, Отказ, ПредставлениеПроверяемогоМесяца = "", МинимальноеЗначение = '19000101', ПредставлениеМинимальногоЗначенияМесяца = "", ПроверятьПустыеМесяцы = Истина) Экспорт
	...
	Если (ПроверятьПустыеМесяцы Или ЗначениеЗаполнено(ПроверяемыйМесяц))
		...
		Если ПустаяСтрока(ПредставлениеПроверяемогоМесяца) Тогда
			ТекстСообщения = НСтр("ru = 'Месяц';
									|en = 'Month'") + " " + Формат(ПроверяемыйМесяц, "ДФ='ММММ гггг'");
		Иначе
...
Процедура СообщитьОНевозможностиИППриниматьНаРаботуСамогоСебя() Экспорт
	...
	ВызватьИсключение НСтр("ru = 'В качестве сотрудника указан индивидуальный предприниматель.
		...
		|en = 'Individual entrepreneur is specified as employee.
		|Individual entrepreneur does not have to accept the work for themselves.'");
...
Процедура ОбновитьНаименованияФизЛицаИСотрудника(ТаблицаФизЛиц) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
		...
		Модуль.ПриИзмененииНаименованияФизическогоЛица(
			МенеджерВременныхТаблиц.Таблицы.Найти("ВТФИОФизЛицПоследнее").ПолучитьДанные().Выгрузить());
	КонецЕсли;
	...
	Пока ДанныеФИО.Следующий() Цикл
		...
		Исключение
			...
			НСтр("ru = 'Не удалось изменить имя физического лица %1.
			           ...
			           |en = 'Cannot change person''s name %1.
			           |Maybe employee''s data is being edited by another user.'"),
	...
	Пока ДанныеФИО.Следующий() Цикл
		...
		Исключение
			...
			НСтр("ru = 'Не удалось изменить имя сотрудника %1.
			           |Возможно, данные сотрудника редактируются другим пользователем';
			           |en = 'Cannot change name for employee %1.
			           |Employee''s data may be edited by another user.'"),
...
Функция ПолучитьДанныеКлассификатораИзСервиса(Идентификатор)
	...
	Если Значение = Неопределено Тогда
		...
		ТекстЖурнала = НСтр("ru = 'При получении классификатора %1 из веб-сервиса возникла ошибка:
			...
			|%4';
			|en = 'An error occurred when receiving classifier %1 from web service:
			|Code: %2
			|Details: %3
			|More details:
			|%4'");
	...
	Иначе
		УровеньЖурнала = УровеньЖурналаРегистрации.Информация;
		ТекстЖурнала = СтрШаблон(НСтр("ru = 'Файл классификатора %1 получен из веб-сервиса.';
										|en = 'Classifier file %1 is received from web service.'"), Идентификатор);
	КонецЕсли;
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Нормативно-правовая информация БЗК';
			|en = 'PCL regulatory information'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция Модуль()
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы") Тогда
		Возврат ОбщегоНазначения.ОбщийМодуль("ЗарплатаКадрыРасширенный");
	Иначе
		Возврат ЗарплатаКадрыБазовый;
	КонецЕсли;
КонецФункции

// Получает информацию о виде расчета.
Функция ИнформацияОВидеРасчета(ВидРасчета) Экспорт
	Кэш = ЗарплатаКадрыПовтИсп.КэшИнформацииОВидахРасчета();
	СтруктураИзКэша = Кэш[ВидРасчета];
	Если СтруктураИзКэша <> Неопределено Тогда
		Возврат СтруктураИзКэша;
	КонецЕсли;
	Массив = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВидРасчета);
	Если ТипЗнч(ВидРасчета) = Тип("ПланВидовРасчетаСсылка.Начисления") Тогда
		ЗарплатаКадрыВнутренний.ПриОпределенииИнформацииОНачислениях(Кэш, Новый Соответствие, Массив);
	ИначеЕсли ТипЗнч(ВидРасчета) = Тип("ПланВидовРасчетаСсылка.Удержания") Тогда
		ЗарплатаКадрыВнутренний.ПриОпределенииИнформацииОбУдержаниях(Кэш, Новый Соответствие, Массив);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	Возврат Кэш[ВидРасчета];
КонецФункции

// Получает информацию о видах расчета.
Функция ИнформацияОВидахРасчета(ВидыРасчета) Экспорт
	...
	НачисленияДляЗапроса = Новый Массив;
	УдержанияДляЗапроса  = Новый Массив;
	
	Для Каждого ВидРасчета Из ВидыРасчета Цикл
		СтруктураИзКэша = Кэш[ВидРасчета];
		Если СтруктураИзКэша <> Неопределено Тогда
			Результат.Вставить(ВидРасчета, СтруктураИзКэша);
		Иначе
			Если ТипЗнч(ВидРасчета) = Тип("ПланВидовРасчетаСсылка.Начисления") Тогда
				НачисленияДляЗапроса.Добавить(ВидРасчета);
			ИначеЕсли ТипЗнч(ВидРасчета) = Тип("ПланВидовРасчетаСсылка.Удержания") Тогда
				УдержанияДляЗапроса.Добавить(ВидРасчета);
			Иначе // исключение.
				Результат.Вставить(ВидРасчета, Неопределено);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Если НачисленияДляЗапроса.Количество() > 0 Тогда
		ЗарплатаКадрыВнутренний.ПриОпределенииИнформацииОНачислениях(Кэш, Результат, НачисленияДляЗапроса);
	КонецЕсли;
	Если УдержанияДляЗапроса.Количество() > 0 Тогда
		ЗарплатаКадрыВнутренний.ПриОпределенииИнформацииОбУдержаниях(Кэш, Результат, УдержанияДляЗапроса);
	КонецЕсли;
