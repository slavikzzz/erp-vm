...
Процедура ПроверкаКэшаСостоянийФНС() Экспорт
	...
	Результат = ПроксиСервиса();
	Прокси = Результат.Прокси;
	Если Прокси = Неопределено Тогда
		ТекстИсключения = НСтр("ru = 'Не удалось проверить контрагентов в сервисе ФНС. Не удалось подключиться к
			...
			|en = 'Cannot check counterparties in FTS service. Cannot connect to
			|the service. (see error details in the event log).'");
...
Функция ПроверитьДоступностьСервиса() Экспорт
	...
	Таймаут = 15;
	Результат = ПроксиСервиса(Таймаут);
	Прокси = Результат.Прокси;
	Если Прокси = Неопределено Тогда
		Возврат ОбработкаОшибок.ПодробноеПредставлениеОшибки(Результат.ИнформацияОбОшибке);
	КонецЕсли;
	...
	РезультатЗапроса = ПолучитьСостоянияКонтрагентов(Прокси, КонтрагентыЗапроса);
	
	Если РезультатЗапроса.NdsResponse = Неопределено Тогда
		Возврат ОбработкаОшибок.ПодробноеПредставлениеОшибки(РезультатЗапроса.ИнформацияОбОшибке);
	Иначе
		Возврат "";
	КонецЕсли;
...
Функция ПолучитьСостоянияКонтрагентов(Прокси, КонтрагентыЗапроса)
	...
	Результат.Вставить("NdsResponse", Неопределено);
	Результат.Вставить("ИнформацияОбОшибке", "");
	
	Попытка
		Результат.NdsResponse = Прокси.NdsRequest2(КонтрагентыЗапроса);
	Исключение
		...
		СообщениеЖурналаРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось проверить контрагентов в сервисе ФНС. Ошибка при вызове операции сервиса.
				|%1';
				|en = 'Cannot check counterparties in FTS service. An error occurred when calling service operation.
				|%1'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ЗаписатьОшибкуВЖурналРегистрации(
			СообщениеЖурналаРегистрации,
			НСтр("ru = 'NdsRequest2';
				|en = 'NdsRequest2'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Исключение
		...
		РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
			...
			НСтр("ru = 'Формирование данных для обращения к сервису ФНС';
				|en = 'Generate data to contact FTS service'",
		...
		Исключение
			...
			ЗаписатьОшибкуВЖурналРегистрации(
				...
				НСтр("ru = 'Обработка ответа от веб-сервиса';
					|en = 'Process response from web service'",
					ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Сервис ФНС получил информацию не по всем контрагентам.
					...
					|en = 'FTS service has not received the information about all contractors.
					|The amount of contractors in the query %1, the amount of contractors in the response %2.'"),
				КоличествоЗапрошенных,
...
Функция ПроксиСервиса(Таймаут = 120)
	...
	Попытка
		...
		Результат.Прокси = ИнтернетПоддержкаПользователей.СоздатьWSПрокси(ПараметрыПодключения);
	...
	Исключение
		...
		СообщениеЖурналаРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при подключении к сервису ФНС для проверки контрагентов по ЕГРН.
				|%1';
				|en = 'An error occurred when connecting to FTS service to check counterparties by USRT.
				|%1'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ЗаписатьОшибкуВЖурналРегистрации(
			СообщениеЖурналаРегистрации,
			НСтр("ru = 'Ошибка доступа к веб-сервису';
				|en = 'Web service access error'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура СохранитьРезультатыПроверки(ДанныеДляПроверки)
	...
	Для Каждого ОписаниеКонтрагента Из ДанныеДляПроверки Цикл
		Если НачалоДня(ОписаниеКонтрагента.Дата) > ГраницаАктуальностиКэша
			...
			ЗаполнитьЗначенияСвойств(
				...
				ОписаниеКонтрагента,
				"ИНН, КПП, Дата, Состояние, ПричинаОтключенияПроверкиФНС");
	...
	ОписаниеДанныхПроверок.Свернуть("ИНН, КПП, Дата, Состояние, Проверен, ПричинаОтключенияПроверкиФНС");
	...
	Запрос.Текст = 
		...
		|	ОписаниеДанныхПроверок.ПричинаОтключенияПроверкиФНС КАК ПричинаОтключения,
		...
		|	ВТ_ДанныеПроверки.ПричинаОтключения КАК ПричинаОтключения,
		...
		|			И (КэшСостоянийКонтрагентовФНС.ПричинаОтключения = ВТ_ДанныеПроверки.ПричинаОтключения)
...
Процедура ВыполнитьПроверкуКэшаСостояний(
	...
	Запрос.Текст =
		...
		|	КэшСостоянийКонтрагентовФНС.Состояние КАК Состояние,
		|	КэшСостоянийКонтрагентовФНС.ПричинаОтключения КАК ПричинаОтключения
		|ИЗ
		|	РегистрСведений.КэшСостоянийКонтрагентовФНС КАК КэшСостоянийКонтрагентовФНС
		|ГДЕ
...
Процедура ОбновитьЗаписиКэшаСостояний(
	...
	РезультатЗапроса = ПолучитьСостоянияКонтрагентов(Прокси, КонтрагентыЗапроса);
	NdsResponse = РезультатЗапроса.NdsResponse;
...
Функция ОписаниеДанныхПроверок(Состояние = Ложь)
	...
	Если Состояние Тогда
		...
		ОписаниеДанных.Колонки.Добавить("ПричинаОтключенияПроверкиФНС",
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'ПроверкаКонтрагентов.';
				|en = 'ПроверкаКонтрагентов.'", ОбщегоНазначения.КодОсновногоЯзыка());
