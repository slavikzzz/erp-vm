	...
	Если ИдентификаторУстройства = Неопределено Или  ПустаяСтрока(ИдентификаторУстройства) Тогда
		...
		МенеджерОборудованияКлиент.ВыбратьУстройство(Оповещение, ПоддерживаемыеТипы,
			НСтр("ru = 'Выберите электронные весы';
				|en = 'Select digital retail scales'"),
			НСтр("ru = 'Электронные весы не подключены.';
				|en = 'Digital retail scales are not connected.'"), 
			НСтр("ru = 'Электронные весы не выбраны.';
				|en = 'Digital retail scales are not selected.'"));
	Иначе
	...
	Если ИдентификаторУстройства = Неопределено Или ПустаяСтрока(ИдентификаторУстройства) Тогда
		...
		МенеджерОборудованияКлиент.ВыбратьУстройство(Оповещение, ПоддерживаемыеТипы,
			НСтр("ru = 'Выберите весы с печатью этикеток';
				|en = 'Select label printing scales'"),
			НСтр("ru = 'Весы с печатью этикеток не подключены.';
				|en = 'Label printing scales are not connected.'"), 
			НСтр("ru = 'Весы с печатью этикеток не выбраны.';
				|en = 'Label printing scales are not selected.'"));
	Иначе
		НачатьОперациюНаЭлектронныхВесахЗавершение(КонтекстОперации, КонтекстОперации);
	КонецЕсли
...
Процедура НачатьВыгрузкуДанныеВВесыСПечатьюЭтикетокПакетом(ОповещениеПриЗавершении, ИдентификаторКлиента, ИдентификаторУстройства, ПараметрыОперации, ДополнительныеПараметры = Неопределено) Экспорт
	...
	КонтекстОперации.Вставить("ПодготовитьДанные"      , Ложь); 
	КонтекстОперации.Вставить("ОбработатьДанные"       , Ложь); 
	КонтекстОперации.Вставить("Команда"  , "UploadGoodsPacket"); 
	КонтекстОперации.Вставить("Результат", Истина);
