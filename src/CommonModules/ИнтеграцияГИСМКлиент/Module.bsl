...
Процедура ПодтвердитьПолучениеДляДокументовИзСписка(ДинамическийСписок) Экспорт
	...
	Иначе
		...
		ПоказатьПредупреждение(, НСтр("ru = 'В указанных строках нет документов, для которых требуется подтверждение поступления КиЗ.';
										|en = 'В указанных строках нет документов, для которых требуется подтверждение поступления КиЗ.'"));
...
Процедура АрхивироватьДокументы(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		ИнтеграцияГИСМВызовСервера.АрхивироватьДокументы(ДополнительныеПараметры.ДокументыКАрхивированию);
		ОповеститьОбИзменении(Тип("РегистрСведенийКлючЗаписи.СтатусыИнформированияГИСМ"));
		ОповеститьОбИзменении(ТипЗнч(ДополнительныеПараметры.ДокументыКАрхивированию[0]));
	КонецЕсли;
...
Процедура АрхивироватьРаспоряжения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Распоряжения.Количество() > 0 Тогда
		
		ИнтеграцияГИСМВызовСервера.АрхивироватьРаспоряженияКОформлению(
			ДополнительныеПараметры.Распоряжения,
			ДополнительныеПараметры.ПустаяСсылка);
		
		ОповеститьОбИзменении(Тип("РегистрСведенийКлючЗаписи.СтатусыИнформированияГИСМ"));
		ОповеститьОбИзменении(ТипЗнч(ДополнительныеПараметры.Распоряжения[0]));
...
Процедура ПолучитьGLNПоИННКПП(Форма) Экспорт
	...
	Данные = РаботаСКонтрагентамиИСВызовСервера.ИННКПППоОрганизацииКонтрагенту(Контрагент);
	...
	Если Не ЗначениеЗаполнено(Контрагент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Контрагент""';
																|en = 'Не заполнено поле ""Контрагент""'"), Форма.Объект.Ссылка, "Объект.Контрагент");
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Данные.ИНН) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнен ИНН контрагента';
																|en = 'Не заполнен ИНН контрагента'"), Форма.Объект.Ссылка, "Объект.Контрагент");
		Возврат;
	КонецЕсли;
	...
	Если Данные = Неопределено Тогда
		...
		Для Каждого ЭлементДанных Из Результат.Изменения Цикл
			...
			Если Выполнено Тогда
				...
				Оповестить("#ГИСМ#ИзменениеСостоянияГИСМ", ПараметрОповещения);
		...
		Если Результат.ИнформацияОВерсиях <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(Результат.ИнформацияОВерсиях.ТекстСообщения) Тогда
				...
				Иначе
					ПоказатьОповещениеПользователя(Результат.ИнформацияОВерсиях.ТекстСообщения,,НСтр("ru = 'Версия обмена обновлена';
																									|en = 'Версия обмена обновлена'"), БиблиотекаКартинок.Информация32ГосИС);
				КонецЕсли;
			КонецЕсли;
		...
		Если ВыполнитьОбновлениеСписков Тогда
			...
			ПараметрОповещения = Новый Структура;
			ПараметрОповещения.Вставить("ОбновлятьСтатусВФормахДокументов", Ложь);
			Оповестить("#ГИСМ#ВыполненОбменГИСМ", ПараметрОповещения);
...
//  ОповещениеПриЗавершении  - ОписаниеОповещения - описание обработчика, который будет вызван при закрытии формы
//                                                  проверки подписи.
//
...
// Из соответствия с сообщениями организаций, исключает первое, как отработанное.
//
// Параметры:
...
Функция СообщенияПоОрганизациям(Сообщения) Экспорт
	...
	Для Каждого Сообщение Из Сообщения Цикл
		...
		Если ЗначениеЗаполнено(Сообщение.ТекстОшибки) Тогда
			ЗаголовокСообщения = СтрШаблон(НСтр("ru = 'Сообщение ""%1"" не подготовлено к отправке в ГИСМ:';
												|en = 'Сообщение ""%1"" не подготовлено к отправке в ГИСМ:'"), Сообщение.Описание) + Символы.ПС;
...
Процедура ПодготовитьСообщениеКПередаче(ДокументСсылка, Операция) Экспорт
	...
	Если Сообщения = Неопределено И Операция <> Неопределено Тогда
		ПоказатьПредупреждение(,СтрШаблон(НСтр("ru = 'По операции ""%1"" передача данных в ГИСМ не требуется';
												|en = 'По операции ""%1"" передача данных в ГИСМ не требуется'"), Операция));
	КонецЕсли;
	Если Сообщения = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ЭлементДанных Из Результат Цикл
		...
		Если Не Выполнено Тогда
			...
			ТекстЗаголовка = НСтр("ru = 'При подготовке сообщения к передаче в ГИСМ возникла ошибка:';
									|en = 'При подготовке сообщения к передаче в ГИСМ возникла ошибка:'");
		...
		Иначе
			...
			ТекстЗаголовка = НСтр("ru = 'Сообщение подготовлено к передаче в ГИСМ';
									|en = 'Сообщение подготовлено к передаче в ГИСМ'");
			Если Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаДанных") Тогда
				ТекстСообщения = НСтр("ru = 'Для документа %Документ% подготовлено к отправке сообщение передачи данных';
										|en = 'Для документа %Документ% подготовлено к отправке сообщение передачи данных'");
			ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаЗакрытияЗаявки") Тогда
				ТекстСообщения = НСтр("ru = 'Для документа %Документ% подготовлено к отправке сообщение о закрытии заявки';
										|en = 'Для документа %Документ% подготовлено к отправке сообщение о закрытии заявки'");
			ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаПодтверждения") Тогда
				ТекстСообщения = НСтр("ru = 'Для документа %Документ% подготовлено к отправке подтверждение о поступлении товаров';
										|en = 'Для документа %Документ% подготовлено к отправке подтверждение о поступлении товаров'");
			КонецЕсли;
...
Процедура ОповеститьПользователяОВыполненнойОперации(СписокДокументов, КоличествоОбработанных, КоличествоВсего, Операция) Экспорт
	...
	ТекстЗаголовка = НСтр("ru = 'Подготовка сообщения к передаче в ГИСМ';
							|en = 'Подготовка сообщения к передаче в ГИСМ'");
	...
	Если КоличествоОбработанных > 0 Тогда
		...
		Если Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаЗакрытияЗаявки") Тогда
			ТекстСообщения = НСтр("ru = 'Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов закрытие заявки подготовлено к отправке';
									|en = 'Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов закрытие заявки подготовлено к отправке'");
		ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаПодтверждения") Тогда
			ТекстСообщения = НСтр("ru = 'Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов подтверждение о поступлении подготовлено к отправке';
									|en = 'Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов подтверждение о поступлении подготовлено к отправке'");
		Иначе
			ТекстСообщения = НСтр("ru = 'Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов данные подготовлены к отправке';
									|en = 'Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов данные подготовлены к отправке'");
		КонецЕсли;
...
Процедура ОповеститьПользователяОРезультатахВыполненияОбмена(Изменения) Экспорт
	...
	Если СтатусыДокументов.Количество() = 0 Тогда
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Документов с измененным статусом: %1.';
										|en = 'Документов с измененным статусом: %1.'"), СтатусыДокументов.Количество());
		...
		Для Каждого КлючИЗначение Из СтатусыДокументов Цикл
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Для документа %1 изменен статус ГИСМ: %2.';
					|en = 'Для документа %1 изменен статус ГИСМ: %2.'"),
				КлючИЗначение.Ключ,
	...
	ПоказатьОповещениеПользователя(ТекстЗаголовка, НавигационнаяСсылка, ТекстСообщения, БиблиотекаКартинок.Информация32ГосИС);
