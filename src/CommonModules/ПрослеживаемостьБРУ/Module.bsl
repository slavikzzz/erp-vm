...
Процедура ПроверитьДублированиеНомераКорректировкиВУведомлениях(УведомлениеОбъект) Экспорт
	...
	Если Не Результат.Пустой() Тогда
		...
		ТекстИсключения = НСтр("ru = 'Корректировка №%НомерКорректировки% первичного уведомления об остатках прослеживаемых товаров уже существует';
								|en = 'Correction #%НомерКорректировки% of a source notification of traceable goods balance already exists'");
	...
	Если Не Результат.Пустой() Тогда
		...
		ТекстИсключения = 
			НСтр(
				...
				|en = 'Correction No. %НомерКорректировки% of the source notification of traceable goods import already exists'"
				);
...
Процедура ВывестиСообщениеОбОшибках(ТаблицаОшибок, Регистратор, Отказ)
	...
	Для Каждого Ошибка Из ТаблицаОшибок Цикл
		...
		ТекстОшибки = НСтр("ru = 'По контрагенту %1, указанное количество превышает количество по сопроводительному документу. Количество: %2; Не хватает: %3';
							|en = 'For %1 counterparty the specified quantity exceeds the quantity according to the supporting document. Quantity: %2; Missing: %3'");
...
Функция ПолучитьПечатнуюФормуУведомлениеОбОстаткахПрослеживаемыхТоваров(ДокументСсылка) Экспорт
	
	ПараметрыПечати = ПараметрыУведомленияОбОстатках(ДокументСсылка);
...
Процедура ЗаполнитьСодержательнуюЧастьУведомлениеОбОстатках(СодержательнаяЧасть, ДокументСсылка)
	...
	СодержательнаяЧасть.П000010000101 = НСтр("ru = 'Инвентаризация товаров';
											|en = 'Physical inventory count'"); // НаимПервичДок
...
Функция ПолучитьСписокПечатныхФормУведомлениеОПеремещенииПрослеживаемыхТоваров(ДокументСсылка) Экспорт
	
	ПараметрыПечати = ПараметрыУведомленияОПеремещении(ДокументСсылка);
	...
	Для каждого ТекущаяСтрокаРезультат Из Результат Цикл
		...
		ТекстПечатнойформы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Уведомление о перемещении. Лист %1';
				|en = 'Transfer notification. Sheet %1'"), НомерЛиста);
