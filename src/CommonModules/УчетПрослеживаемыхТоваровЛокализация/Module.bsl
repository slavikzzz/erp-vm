...
Функция ТекстыЗапросовСторнирования(МетаданныеДокумента) Экспорт
	...
	МетаданныеРегистра = Метаданные.РегистрыСведений.ОперацииСПрослеживаемымиТоварамиРасширенный;
	...
	Если ДвиженияДокумента.Содержит(МетаданныеРегистра) Тогда
		...
		|	ОперацииСПрослеживаемымиТоварамиРасширенный.Организация КАК Организация,
		|	ОперацииСПрослеживаемымиТоварамиРасширенный.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|	ОперацииСПрослеживаемымиТоварамиРасширенный.РНПТ КАК РНПТ,
		|	ОперацииСПрослеживаемымиТоварамиРасширенный.ВидЗапасов КАК ВидЗапасов,
		|	ОперацииСПрослеживаемымиТоварамиРасширенный.ОтражениеВОтчетности КАК ОтражениеВОтчетности,
		|	ОперацииСПрослеживаемымиТоварамиРасширенный.КодОперации КАК КодОперации,
		|	ОперацииСПрослеживаемымиТоварамиРасширенный.ИдентификаторСтроки КАК ИдентификаторСтроки,
		|	-ОперацииСПрослеживаемымиТоварамиРасширенный.КоличествоПоРНПТ КАК КоличествоПоРНПТ,
		|	ОперацииСПрослеживаемымиТоварамиРасширенный.Контрагент КАК Контрагент,
		|	ОперацииСПрослеживаемымиТоварамиРасширенный.ТипДокументаВПрослеживаемости КАК ТипДокументаВПрослеживаемости
		|ИЗ
		...
		|		ПО ОперацииСПрослеживаемымиТоварамиРасширенный.Регистратор = ДокументыСторно.СторнируемыйДокумент
...
Процедура ИнициализироватьДанныеКонтроляИзменений(Запрос, ТекстыЗапроса, Документ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияПрослеживаемыеТоварыВСоставеОСИзменение") Тогда
		...
		Если Документ.ДополнительныеСвойства.Свойство("КонтролироватьСписаниеВНоль")  
			...
			|   ПрослеживаемыеТоварыВСоставеОС.КодТНВЭД.ЕдиницаИзмерения КАК ЕдиницаИзмеренияТНВЭД,
...
Процедура СообщитьОРезультатахКонтроляИзменений(РезультатыКонтроля, Документ, Отказ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияПрослеживаемыеТоварыВСоставеОСИзменение") Тогда
		...
		ШаблонСообщенияНеотрицательныйОстаток = НСтр("ru = 'Товар с номером ГТД %1 с кодом ТН ВЭД %2 в составе основного средства %3:
			...
			|en = 'Goods with CCD number %1 and with FEACN code %2 in fixed asset %3:
			|Written off more than there is in stock balance of %4 company as of %5 %6'");
		...
		ШаблонСообщенияНеНулевойОстаток = НСтр("ru = 'Товар с номером ГТД %1 с кодом ТН ВЭД %2 в составе основного средства %3:
			...
			|en = 'Goods with CCD number %1 and with FEACN code %2 in fixed asset %3:
			|Written off less than there is in stock balance of %4 company as of %5 %6'");		
...
Процедура ПроверитьЗаполнениеКоличестваПоРНПТ(Объект, Отказ, ПараметрыПроверки) Экспорт
	...
	ОшибкаЗаполненияКоличестваПоРНПТ = НСтр("ru = 'Не заполнена колонка ""Количество по РНПТ"" в строке %НомерСтроки% списка ""%ТаблицаТовары%"".';
											|en = 'Column ""Quantity by GBRN"" in line %НомерСтроки% of the ""%ТаблицаТовары%"" list is not filled in.'");
...
Процедура УстановитьУсловноеОформлениеУчетаПоГТДВДокументахКомиссионера(Форма, ЭтоОтчетКомиссионера = Истина) Экспорт
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<не ведется учет по РНПТ>';
																|en = '<records by RNPT are not kept>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Ложь);
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
...
Процедура ПроверитьЗаполнениеНомераГТДВДокументахКомиссионера(Объект, Отказ, КомиссионерВедетУчетПоРНПТ = Истина) Экспорт
	...
	ШаблонТекстаОшибки	= НСтр("ru = 'Не заполнена колонка ""Номер ГТД"" в строке %НомерСтроки% списка ""Товары"".';
								|en = 'Column ""CCD number"" in line %НомерСтроки% of ""Goods"" list is not filled in.'");
	КлючДанных			= ОбщегоНазначенияУТ.КлючДанныхДляСообщенияПользователю(Объект);
...
Процедура ПроверитьКорректностьНастроекТоваровРНПТ(Объект,
	...
	Если ИспользоватьУчетПрослеживаемыхИмпортныхТоваров Тогда
		...
		ШаблонТекстаОшибки = НСтр("ru = 'Для номенклатуры ""%1"" задан прослеживаемый код ТН ВЭД, но выключен учет по номерам ГТД.
									...
									|en = 'For product ""%1"", a traceable FEACN code is specified, but accounting by CCD numbers is disabled. 
									|To account the goods as traced, set the flag of accounting by CCD numbers.'");
...
Процедура ПроверитьДанныеПрослеживаемостиНомеровГТД(Объект,
	...
	Если ИмяТабличнойЧасти = "ВидыЗапасов" Тогда
		...
		ТекстОшибки = НСтр("ru = 'Документ содержит номера ГТД, в которых некорректно заполнены стоимостные показатели комплектующих.
									...
									|en = 'The document contains CCD numbers where the component cost is filled incorrectly.
									|For more information, open the form to edit inventory owner attributes of the document.'");
	...
	Иначе
		...
		ШаблонТекстаОшибки = НСтр("ru = 'Не заполнены стоимостные показатели комплектующих номера ГТД ""%1"" в строке ""%2"" списка ""%3""';
									|en = 'Component cost is not filled for CCD number %1 in line %2 of the %3 list'");
		ИмяТабличнойЧастиТекст = ?(ИмяТабличнойЧасти = "ТаблицаКорректировки", НСтр("ru = 'Данные корректировки';
																					|en = 'Correction data'"), ИмяТабличнойЧасти); 
		...
		Для Каждого Стр Из Объект[ИмяТЧ] Цикл
			...
			Если Не ЗначениеЗаполнено(Выборка.РНПТ) Тогда
				ВызватьИсключение НСтр("ru = 'Обнаружены проблемы в подборе номеров РНПТ.';
										|en = 'Issues detected upon selecting goods batch registration numbers.'");
			КонецЕсли;
		...
		Если КоличествоРаспоряженийКОформлению = 1 Тогда
			...
			Иначе
				ТекстРаспоряженияКОформлению = НСтр("ru = 'Уведомление о ввозе ПТ не оформлено';
													|en = 'TG import notification is not registered'"); 
			КонецЕсли;
		ИначеЕсли КоличествоРаспоряженийКОформлению > 1 Тогда
			Если ЕстьПравоИнтерактивногоДобавленияДокументаПоОснованию Тогда
				ТекстРаспоряженияКОформлению = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru = 'Оформить уведомления по ПТ (%1)';
												|en = 'Register TG notifications (%1)'"), 
			...
			Иначе
				ТекстРаспоряженияКОформлению = НСтр("ru = 'Уведомления о ввозе ПТ не оформлены';
													|en = 'TG import notifications are not registered'"); 
			КонецЕсли;				
		КонецЕсли;
		...
		Если КоличествоРаспоряженийКОформлению > 0 И ТаблицаУведомленийОПеремещении.Количество() = 0 Тогда
			...
			Иначе
				ТекстРаспоряженияКОформлению = НСтр("ru = 'Уведомление о перемещении ПТ не оформлено';
													|en = 'TG export notifications are not registered'"); 
			КонецЕсли;
		КонецЕсли;
...
Процедура ПроверитьКорректностьУказанияНоменклатурыПорчиТоваров(Объект, Отказ, ПараметрыПроверки = Неопределено) Экспорт
	...
	ШаблонСообщения = НСтр("ru = 'Некорректно заполнены данные в строке %1 списка ""%2"".
							|%3.';
							|en = 'Incorrect data in line %1 of the ""%2"" list.
							|%3.'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ПрослеживаемыйТовар Тогда
			ТекстОшибки = НСтр("ru = 'Списание прослеживаемого по РНПТ товара исходного качества и оприходование непрослеживаемого испорченного товара';
								|en = 'Write off an item of the original quality traced by RNPT and receive untraceable stock of changed quality'");
		Иначе
			ТекстОшибки = НСтр("ru = 'Списание непрослеживаемого по РНПТ товара исходного качества и оприходование прослеживаемого испорченного товара';
								|en = 'Write off an item of the original quality that is not traced by RNPT and receive traceable stock of changed quality'");
		КонецЕсли;
...
Функция ШаблонСообщенияНепрослеживаемогоКомплекта(ТипНоменклатуры)
	...
	Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Набор Тогда
		ТекстШаблона = НСтр("ru = 'Непрослеживаемый по РНПТ набор ""%1"" не должен содержать прослеживаемые комплектующие:
							|%2.';
							|en = 'Non-traceable by goods batch registration number set ""%1"" must not contain traceable components:
							|%2.'");
	Иначе
		ТекстШаблона = НСтр("ru = 'Непрослеживаемый по РНПТ комплект ""%1"" не должен содержать прослеживаемые комплектующие:
							|%2.';
							|en = 'Non-traceable by goods batch registration number kit ""%1"" must not contain traceable components:
							|%2.'");
	КонецЕсли;
...
Функция ШаблонСообщенияНепрослеживаемыеКомплектующие()
	...
	ТекстШаблона = НСтр("ru = 'Прослеживаемый по РНПТ комплект ""%1"" не содержит прослеживаемую комплектующую.
						...
						|en = 'The ""%1"" kit traceable by a goods batch registration number does not contain a traceable component.
						|To correctly account for the kits traceable by goods batch registration numbers, specify the component with the FEACN code equal to the kit code.'");
	...
	Если ЭтоКомплект Тогда
		Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Набор Тогда
			ТекстШаблона = НСтр("ru = 'Для набора ""%1"" задан прослеживаемый код ТН ВЭД, но выключен учет по номерам ГТД.
								...
								|en = 'For the ""%1"" set a traceable FEACN code is specified but accounting by CCD numbers is disabled. 
								|To account for the set as traceable, enable accounting by CCD numbers.'");
		Иначе
			ТекстШаблона = НСтр("ru = 'Для комплекта ""%1"" задан прослеживаемый код ТН ВЭД, но выключен учет по номерам ГТД.
								...
								|en = 'For the ""%1"" kit a traceable FEACN code is specified but accounting by CCD numbers is disabled. 
								|To account for the kit as traceable, enable accounting by CCD numbers.'");
		КонецЕсли;
	Иначе
		ТекстШаблона = НСтр("ru = 'Для комплектующей ""%1"" задан прослеживаемый код ТН ВЭД, но выключен учет по номерам ГТД.
								...
								|en = 'For the ""%1"" component a traceable FEACN code is specified but accounting by CCD numbers is disabled. 
								|To account for the component as traceable, enable accounting by CCD numbers.'");
	КонецЕсли;
...
Функция ШаблонСообщенияКорректностиСоответствияТипаНомераГТДИНастроекНоменклатуры()
	...
	ТекстШаблона = НСтр("ru = 'Для не прослеживаемой комплектующей ""%1"" задан прослеживаемый номер ГТД (РНПТ) ""%2"".
					...
					|en = 'For the ""%1"" non-traceable component the traceable CCD (RNPT) number ""%2"" is specified.
					|To account for the goods as traceable, select the check box for accounting by traceable goods.'");
	Возврат ТекстШаблона;
...
Функция ШаблонСтрокиПрослеживаемойКомплектующей(ВыводитьНомера)
	...
	Если Не ВыводитьНомера Тогда
		ТекстШаблона = НСтр("ru = '""%1"" списка ""Комплектующие""';
							|en = '""%1"" ""Components"" list'");
	Иначе
		ТекстШаблона = НСтр("ru = '""%1"" в строке %2 списка ""Комплектующие""';
							|en = '""%1"" in line %2 of the ""Components"" list'");
	КонецЕсли;
