...
Функция ПакетыСтраниц(НосительМакетов) Экспорт
	...
	Для НомерСтроки = 3 По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		Если НРег(СтрокаТаблицы.Раздел) = НРег(НСтр("ru = 'Реклама';
													|en = 'Ads'")) Тогда // АПК:1391 Раздел локализуется.
...
Функция ИзвлечьФайлыПакета(НосительМакетов, ИмяМакета) Экспорт
	...
	Исключение
		...
		Если НосительМакетов.Метаданные().Макеты.Найти(ЛокализованноеИмяМакета) = Неопределено Тогда
			ТекстСообщения = ТекстСообщения + НСтр("ru = 'Макет с таким именем не существует.';
													|en = 'Template with this name does not exist.'") + Символы.ПС;
		КонецЕсли;
		ТекстСообщения = ТекстСообщения + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Информация при запуске';
				|en = 'Notification at startup'", ОбщегоНазначения.КодОсновногоЯзыка()),
