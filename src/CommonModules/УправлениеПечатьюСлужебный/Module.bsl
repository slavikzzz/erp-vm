		...
		Для Каждого СтрокаПечатнойФормы Из ТаблицаПечатныхФорм Цикл
			Если СтрокаПечатнойФормы.ФормироватьЗаново Тогда
				...
				Иначе
					...
					НСтр("ru = 'Ошибка: Не удалось сформировать %1';
						|en = 'Error: Cannot generate %1'"), СтрокаПечатнойФормы.Представление);
				КонецЕсли;
			КонецЕсли;
...
Процедура ЗакрытьСоединение(ПечатнаяФорма) Экспорт
	...
	Исключение
		...
		ВызватьИсключение(НСтр("ru = 'Не удалось удалить временный каталог шаблона печатной формы по причине:';
								|en = 'Failed to delete temporary directory where print form template is stored. Reason:'") + Символы.ПС 
...
Процедура НайтиОбласти(СтруктураДокумента, ИменаТабличныхЧастейОбъекта) Экспорт
	...
	Для Каждого ИндексНачалаУсловнойОбласти Из ИндексыНачалУсловныхОбластей Цикл
		...
		Если ОкончаниеУсловнойОбласти = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В макете документа нет окончания условной области ""%1"".';
					|en = 'In the document template, the end of the %1 conditional area is not specified.'"), УсловиеОбласти);
		КонецЕсли;
...
Процедура СобратьКонтейнерДокументаDOCX(Знач ПолноеИмяФайла, Знач ПутьКСтруктуреФайла)
	...
	Исключение
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение(НСтр("ru = 'Не удалось создать файл документа по причине:';
								|en = 'Cannot create MS Word document. Reason:'") + Символы.ПС 
			+ ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	Исключение
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение(НСтр("ru = 'Не удалось сформировать файл документа по причине:';
								|en = 'Cannot generate MS Word document. Reason:'") + Символы.ПС 
			+ ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Печать';
				|en = 'Print'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ОпределитьРасширениеФайлаДанныхПоСигнатуре(ДанныеИлиСтруктура) Экспорт
	...
	ИначеЕсли Символ1 = 80 И Символ2 = 75 И Символ3 = 3 И Символ4 = 4 И Символ5 = 20
		...
		Исключение
			...
			ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение(НСтр("ru = 'Не удалось открыть файл шаблона по причине:';
									|en = 'Cannot open the template file. Reason:'") + Символы.ПС 
				+ ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
