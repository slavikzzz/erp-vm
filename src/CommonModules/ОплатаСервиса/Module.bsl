...
Процедура ЗагрузитьТарифыСервиса() Экспорт
	...
	Исключение
		...
		ИмяСобытия = СобытиеЖурналаРегистрации(НСтр("ru = 'Загрузка тарифов сервиса';
													|en = 'Import service plans'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ОтправитьОтветВУчетнуюСистемуБиллинга(ИдентификаторСчета, Данные) Экспорт
	...
	Если Ответ.КодСостояния = 404 Тогда
		...
		Если Ответ.КодСостояния = 404 Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Ошибка 404 при отправке ответа по счету %1 на адрес %2. 
				...
				|en = 'Error 404 occurred when sending the response on proforma invoice %1 to %2. 
				|The HTTP service %3 might not be published in the internal Service Manager publication.'"),
	...
	Если Ответ.КодСостояния = 401 Тогда
		ВызватьИсключение СтрШаблон(ШаблонТекстаОшибки, Ответ.КодСостояния, ИдентификаторСчета, АдресСчета,
			СтрШаблон(НСтр("ru = 'Пользователь %1 не авторизован.';
							|en = 'User %1 is not authorized.'"), ИмяПользователяУчетнойСистемы));
	ИначеЕсли Ответ.КодСостояния = 403 Тогда
		ВызватьИсключение СтрШаблон(ШаблонТекстаОшибки, Ответ.КодСостояния, ИдентификаторСчета, АдресСчета,
			СтрШаблон(НСтр("ru = 'Запрос %1 не разрешен пользователю %2.';
							|en = 'The %1 request is restricted for user %2.'"), МетодHTTP, ИмяПользователяУчетнойСистемы)); 
	ИначеЕсли Ответ.КодСостояния = 405 Тогда
		ВызватьИсключение СтрШаблон(ШаблонТекстаОшибки, Ответ.КодСостояния, ИдентификаторСчета, АдресСчета,
			СтрШаблон(НСтр("ru = 'Метод %1 не поддерживается.';
							|en = 'The %1 method is not supported.'"), МетодHTTP));
...
Процедура УстановитьНастройкиОплатыСервиса(КодАбонента) Экспорт
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ИмяСобытия = СобытиеЖурналаРегистрации(НСтр("ru = 'Установка настроек оплаты сервиса';
													|en = 'Configure service payment settings'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, , ,
			ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке));
		ВызватьИсключение СтрШаблон(
	...
	Если ПоддерживаетсяЗагрузкаТарифов() Тогда
...
Процедура УдалитьНастройкиОплатыСервиса(КодАбонента) Экспорт
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ИмяСобытия = СобытиеЖурналаРегистрации(НСтр("ru = 'Удаление настроек оплаты сервиса';
													|en = 'Delete service payment settings'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, , ,
			ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке));
		ВызватьИсключение СтрШаблон(
...
//
// Параметры:
//  ДанныеЗапроса - см. ДанныеЗапросаСчетаНаОплату
//
Процедура ПодготовитьСчетНаОплату(ДанныеЗапроса) Экспорт
		...
		Если ПечатнаяФорма = Неопределено И ДанныеСчета = Неопределено Тогда
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Не заданы или не возвращают данных переопределяемые методы %1 и %2.';
					|en = 'Overridable methods %1 and %2 are not specified or do not return data.'"),
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ИмяСобытия = СобытиеЖурналаРегистрации(НСтр("ru = 'Подготовка счета на оплату';
													|en = 'Prepare a commercial invoice'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, , ,
			ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке));
		Данные = ШаблонДанныхОтвета(КодВозвратаВнутренняяОшибка(), ТехнологияСервиса.КраткийТекстОшибки(ИнформацияОбОшибке));
		ОтправитьОтветВУчетнуюСистемуБиллинга(ИдентификаторСчета, Данные);
...
Процедура ЛогироватьHTTPЗапрос(Запрос, Ответ, ДвоичныеДанные = Ложь) Экспорт
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(НСтр("ru = 'HTTP-запрос';
															|en = 'HTTP request'")), УровеньЖурналаРегистрации.Информация,
...
// Владелец пароля авторизации в сервисе.
// 
// Параметры:
		...
		Если Ответ.КодСостояния = 500 Тогда
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Ошибка 500 при помещении файла в счет на оплату %1 по причине: %2';
					|en = 'Error 500 occurred when placing a file to proforma invoice %1. Reason: %2'"),
...
Функция СобытиеЖурналаРегистрации(Уточнение) Экспорт
	...
	Возврат СтрШаблон("%1.%2", НСтр("ru = 'Оплата сервиса';
									|en = 'Service payments'", ОбщегоНазначения.КодОсновногоЯзыка()), Уточнение);
...
Функция ОписаниеЗапроса(Запрос, ДвоичныеДанные = Ложь)
	...
	Если СтрДлина(ТелоЗапроса) > 1000 Тогда
		ТелоЗапроса = СтрШаблон(НСтр("ru = '%1...';
									|en = '%1...'"), Лев(ТелоЗапроса, 1000));
	КонецЕсли; 
...
Функция ОписаниеОтвета(Ответ, ДвоичныеДанные = Ложь)
	...
	Если СтрДлина(ТелоОтвета) > 1000 Тогда
		ТелоОтвета = СтрШаблон(НСтр("ru = '%1...';
									|en = '%1...'"), Лев(ТелоОтвета, 1000));
	КонецЕсли; 
