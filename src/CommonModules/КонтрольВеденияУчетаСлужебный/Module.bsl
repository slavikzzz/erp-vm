...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий                         = НСтр("ru = 'Отключение автоматического запуска системных проверок в подсистеме контроля ведения учета.';
															|en = 'Disabling automatic running of system checks in the accounting audit subsystem.'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ru = 'Проверка ведения учета давно не выполнялась';
									|en = 'Accounting has not been checked for a while'");
	...
	Если Не ЗначениеЗаполнено(ОшибкаПроверки.ВидПроверки) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При записи проблемы по проверке ""%1"" не указан вид проверки.';
				|en = 'Check type is not specified when saving the issue for the ""%1"" check.'"), 
...
Процедура ПроверкаУникальностиЗаданныхЭлементов(ГруппыПроверок, Проверки)
	...
	Пока Результат.Следующий() Цикл
		...
		ТекстИсключения = ТекстИсключения + ?(ЗначениеЗаполнено(ТекстИсключения), Символы.ПС, "")
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Дублирующийся идентификатор: ""%1""';
																			|en = 'Duplicate ID: ""%1""'"), Результат.Идентификатор);
...
Процедура ПроверитьНаличиеПредопределенныхУзловПлановОбмена(Проверка, ПараметрыПроверки) Экспорт
	...
	Для Каждого МетаданныеПланОбмена Из МетаданныеПланыОбмена Цикл
		...
		Проблема.УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В плане обмена ""%1"" отсутствует предопределенный узел (%2 = Неопределено).';
																									|en = 'Predefined node is missing from exchange plan ""%1"" (%2 = Undefined).'"), МетаданныеПланОбмена.Имя, "ЭтотУзел()");
...
Функция РазместитьГруппуИндикатораОшибки(Форма, КлючУникальностиИмен, ИмяРодителяГруппы = Неопределено, ВыводитьСнизу = Ложь) Экспорт
	...
	Иначе
		...
		Если РодительГруппы = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не существует указанная группа формы: ""%1""';
																							|en = 'The specified form group ""%1"" does not exist'"), ИмяРодителяГруппы);
		КонецЕсли;
...
Функция СформироватьОбщуюСтрокуИндикатор(Форма, СсылкаНаОбъект, Настройки) Экспорт
	...
	Иначе
		ПоясняющаяНадпись = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В этом %1';
																						|en = 'In this %1'"), ПредставлениеОбъектаПоТипу(СсылкаНаОбъект));
	КонецЕсли;
	
	Если ГиперссылкаИндикатораПроблем <> Неопределено Тогда
		Гиперссылка = ГиперссылкаИндикатораПроблем;
	Иначе
		Гиперссылка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'найдены проблемы (%1)';
																					|en = 'issues found (%1)'"), Формат(КоличествоПроблемПоОбъекту, "ЧГ=0"));
	КонецЕсли;
...
Процедура ВыполнитьПроверку(Проверка, ПараметрыВыполненияПроверки = Неопределено, ПроверяемыеОбъекты = Неопределено) Экспорт
	...
	Если ПараметрыСеанса.СчетчикПроблемВеденияУчета > 0
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Контроль ведения учета';
										|en = 'Accounting audit'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			Если ЕщеПроблем > 0 Тогда
				УточнениеПроблемы = УточнениеПроблемы + Символы.ПС + НСтр("ru = 'и еще проблем';
																			|en = 'More issues'") + ": " + Строка(ЕщеПроблем);
			КонецЕсли;
			...
			Проблема.УточнениеПроблемы = НСтр("ru = 'Нарушена ссылочная целостность:';
												|en = 'Reference integrity violation:'") + Символы.ПС + УточнениеПроблемы;
...
Функция ИнформацияОНезависимомРегистре(ОбъектМетаданных, ПолучитьИзмерения = Истина, ПолучитьРесурсы = Истина, ПолучитьРеквизиты = Истина)
		...
		Для Каждого Измерение Из Измерения Цикл
			...
			ЗаполнитьЗначенияСвойств(ИнформацияОРегистре.Добавить(),
				Новый Структура("ТипМетаданныхВИменительномПадеже, ТипМетаданныхВТворительномПадеже, ИмяМетаданных", 
					"Измерение", НСтр("ru = 'измерении';
										|en = 'dimension'"), ИмяИзмерения));
		КонецЦикла;
	КонецЕсли;
	...
	Если ПолучитьРесурсы Тогда
		...
		Для Каждого Ресурс Из Ресурсы Цикл
			...
			ЗаполнитьЗначенияСвойств(ИнформацияОРегистре.Добавить(),
				Новый Структура("ТипМетаданныхВИменительномПадеже, ТипМетаданныхВТворительномПадеже, ИмяМетаданных", 
					"Ресурс", НСтр("ru = 'ресурсе';
									|en = 'resource'"), ИмяРесурса));
		КонецЦикла;
	КонецЕсли;
	...
	Если ПолучитьРеквизиты Тогда
		...
		Для Каждого Реквизит Из Реквизиты Цикл
			...
			ЗаполнитьЗначенияСвойств(ИнформацияОРегистре.Добавить(),
				Новый Структура("ТипМетаданныхВИменительномПадеже, ТипМетаданныхВТворительномПадеже, ИмяМетаданных", 
					"Реквизит", НСтр("ru = 'реквизите';
									|en = 'attribute'"), ИмяРеквизита));
		КонецЦикла;
	КонецЕсли;
...
Процедура НайтиНезаполненныеОбязательныеРеквизиты(ОбъектМетаданных, ПараметрыПроверки)
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			Проблема.УточнениеПроблемы = НСтр("ru = 'Не заполнены реквизиты, обязательные к заполнению:';
												|en = 'Attributes required:'") + Символы.ПС + ОшибкиЗаполненияОбъекта();
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			Проблема.УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У записи с полями:
				...
				" • " + НСтр("ru = 'Регистратор:';
							|en = 'Recorder:'") + " = """ + СтрокаРезультата.РегистраторРеквизитСсылка, Символы.ПС + ОшибкиЗаполненияОбъекта());
			Проблема.ДополнительнаяИнформация = Новый ХранилищеЗначения(ДополнительнаяИнформация);
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			Проблема.УточнениеПроблемы        = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У записи с полями:
				...
				|en = 'Error in record with the following fields:
				|%1
				|Some values are required: %2'"), ПредставлениеОтбораНабораЗаписей, Символы.ПС + ОшибкиЗаполненияОбъекта());
			Проблема.ДополнительнаяИнформация = Новый ХранилищеЗначения(ДополнительнаяИнформация);
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			ПредставлениеОтбораНабораЗаписей = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(" • %1 = ""%2""",
				НСтр("ru = 'Период';
					|en = 'Period'"), СтрокаРезультата.Период);
			Для Каждого Измерение Из Измерения Цикл
			...
			Проблема.УточнениеПроблемы        = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У записи с полями:
				...
				|en = 'Error in record with the following fields:
				|%1
				|Some values are required: %2'"), ПредставлениеОтбораНабораЗаписей, Символы.ПС + УточнениеПроблемы);
...
Процедура НайтиОтсутствующиеПредопределенныеЭлементы(ОбъектМетаданных, ПараметрыПроверки)
	...
	Для Каждого ПредопределенныйЭлемент Из ПредопределенныеЭлементы Цикл
		...
		Проблема = ОписаниеПроблемы(ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ОбъектМетаданных), ПараметрыПроверки); // @skip-check query-in-loop - порционная проверка ведения учета
		Проблема.УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Предопределенный элемент ""%1"" отсутствует в информационной базе.';
																									|en = 'Predefined item ""%1"" missing in information database.'"), ПредопределенныйЭлемент);
		ЗаписатьПроблему(Проблема, ПараметрыПроверки); // @skip-check query-in-loop - порционная проверка ведения учета
...
Процедура НайтиДублиПредопределенныхЭлементов(ВидМетаданных, ПараметрыПроверки)
	...
	Пока Результат.Следующий() Цикл
		...
		Пока РезультатПоИмениПредопределенного.Следующий() Цикл
			...
			УточнениеПроблемы = УточнениеПроблемы + ?(ЗначениеЗаполнено(УточнениеПроблемы), Символы.ПС, "")
				+ НСтр("ru = 'Имя предопределенного элемента:';
						|en = 'Name of the predefined item:'") + " """ + ИмяПредопределенныхДанных + """"
...
Функция ОшибкиЗаполненияОбъекта()
	...
	Возврат ?(ПустаяСтрока(УточнениеПроблемы), НСтр("ru = 'Для подробной информации откройте форму объекта.';
													|en = 'For more information, open the object form.'"), УточнениеПроблемы);
...
Функция ПредставлениеОбъектаПоТипу(СсылкаНаОбъект)
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неверный тип значения параметра (%1)';
																						|en = 'Invalid parameter value type (%1)'"), Строка(ТипОбъекта));
