...
Процедура УстановитьViPNetCSP(ОповещениеОЗавершении, ВладелецФормы, ТихийРежим = Ложь) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru = 'Установка ViPNet CSP возможно только на операционных системах семейства Windows.';
								|en = 'Установка ViPNet CSP возможно только на операционных системах семейства Windows.'");
		ПоказатьПредупреждение(, ОписаниеОшибки,, НСтр("ru = 'Установка ViPNet CSP';
														|en = 'Установка ViPNet CSP'"));
	...
	Если РезультатВызова.Выполнено Тогда
		...
		Если РезультатВызова.СистемнаяИнформация.ВерсияОС = "10.0"
			...
			ОписаниеОшибки = СтрШаблон(
								НСтр("ru = 'Работа ViPNet CSP на операционной системе %1 не поддерживается.';
									|en = 'Работа ViPNet CSP на операционной системе %1 не поддерживается.'"),
		...
		Если Не ЗначениеЗаполнено(ПараметрыДистрибутива) Тогда
			ОписаниеОшибки = НСтр("ru = 'Сервис временно недоступен. Обратитесь в службу поддержки или повторите попытку позже.';
									|en = 'Сервис временно недоступен. Обратитесь в службу поддержки или повторите попытку позже.'");
			ПоказатьПредупреждение(, ОписаниеОшибки);
			РезультатВыполнения = Новый Структура;
			РезультатВыполнения.Вставить("Выполнено", Ложь);
			РезультатВыполнения.Вставить("ОписаниеОшибки", ОписаниеОшибки);
		...
		ПараметрыВыполнения = Новый Структура("ПоясняющийТекст", НСтр("ru = 'Выполняется загрузка программы...';
																		|en = 'Выполняется загрузка программы...'"));
...
Процедура УстановитьViPNetCSPПослеПолученияПодтвержденияНаУстановку(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru = 'Пользователь отклонил установку ViPNet CSP';
															|en = 'Пользователь отклонил установку ViPNet CSP'"));
...
Процедура УстановитьViPNetCSPПослеСохраненияСкрипта(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru = 'Ошибка при установке ViPNet CSP';
															|en = 'Ошибка при установке ViPNet CSP'"));
	...
	Если Результат.Выполнено Тогда
		Если ЗначениеЗаполнено(Результат.КодВозврата) Тогда
			...
			РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru = 'Установка ViPNet CSP не выполнена';
																|en = 'Установка ViPNet CSP не выполнена'"));
...
Процедура УстановитьCryptoProCSP(ОповещениеОЗавершении, ВладелецФормы, ТихийРежим = Ложь) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru = 'Автоматическая установка CryptoPro CSP возможно только на операционных системах семейства Windows.';
								|en = 'Автоматическая установка CryptoPro CSP возможно только на операционных системах семейства Windows.'");
		ПоказатьПредупреждение(, ОписаниеОшибки,, НСтр("ru = 'Установка CryptoPro CSP';
														|en = 'Установка CryptoPro CSP'"));
		РезультатВыполнения = Новый Структура;
		РезультатВыполнения.Вставить("Выполнено", Ложь);
		РезультатВыполнения.Вставить("ОписаниеОшибки", ОписаниеОшибки);
...
Процедура УстановитьCryptoProCSPПослеВводаРегистрационныхДанных(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru = 'Пользователь прервал операцию';
															|en = 'Пользователь прервал операцию'"));
		ВыполнитьОбработкуОповещения(ВходящийКонтекст.ОповещениеОЗавершении, РезультатВыполнения);
	КонецЕсли;
...
Процедура УстановитьCryptoProCSPПослеПолученияДистрибутива(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	Если ДлительнаяОперация = Неопределено Тогда
		...
		РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru = 'Вызов API сервиса выдачи дистрибутивов CryptoPro CSP не был завершен штатно.';
															|en = 'Вызов API сервиса выдачи дистрибутивов CryptoPro CSP не был завершен штатно.'"));
		ВыполнитьОбработкуОповещения(ВходящийКонтекст.ОповещениеОЗавершении, РезультатВыполнения);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьCryptoProCSPПослеПолученияПодтвержденияНаУстановку(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru = 'Ошибка при установке CryptoPro CSP';
															|en = 'Ошибка при установке CryptoPro CSP'"));
	...
	Если Результат.Выполнено Тогда
		Если ЗначениеЗаполнено(Результат.КодВозврата) Тогда
			...
			РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru = 'Установка CryptoPro CSP не выполнена';
																|en = 'Установка CryptoPro CSP не выполнена'"));
...
Процедура УстановитьДрайверыРутокена(ОповещениеОЗавершении) Экспорт
	...
	Иначе
		...
		ОписаниеОшибки = НСтр("ru = 'Автоматическая установка драйвера возможно только на операционных системах семейства Windows.';
								|en = 'Автоматическая установка драйвера возможно только на операционных системах семейства Windows.'");
...
Процедура УстановитьДрайверыРутокена_ПослеСозданияВременногоКаталога(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		ПараметрыВыполнения = Новый Структура("ПоясняющийТекст", НСтр("ru = 'Выполняется загрузка драйверов из Интернета...';
																		|en = 'Выполняется загрузка драйверов из Интернета...'"));
	...
	Иначе
		...
		РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru = 'Не удалось создать временный каталог';
															|en = 'Не удалось создать временный каталог'"));
...
Процедура УстановитьДрайверы_ПослеУстановки(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Если ЭтоОшибкаУстановки(Результат.КодВозврата) Тогда
			...
			РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru = 'При установке возникла ошибка или пользователь отказался от установки';
																|en = 'При установке возникла ошибка или пользователь отказался от установки'"));
	...
	Иначе
		...
		РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru = 'Установка драйверов не выполнена';
															|en = 'Установка драйверов не выполнена'"));
...
Процедура УстановитьДрайверДжакарта(ОповещениеОЗавершении) Экспорт
	...
	Иначе
		...
		ОписаниеОшибки = НСтр("ru = 'Установка Драйверов JaCarta возможно только на операционных системах семейства Windows.';
								|en = 'Установка Драйверов JaCarta возможно только на операционных системах семейства Windows.'");
	...
	Иначе
		...
		Иначе
			...
			ОписаниеОшибки = НСтр("ru = 'Не удалось получить иформацию о разрядности ОС.';
									|en = 'Не удалось получить иформацию о разрядности ОС.'");
	...
	Если Результат.Выполнено Тогда
		...
		Если Путь = "" Тогда
			...
			РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru = 'Не удалось определить ссылку для загрузки драйвера';
																|en = 'Не удалось определить ссылку для загрузки драйвера'"));
