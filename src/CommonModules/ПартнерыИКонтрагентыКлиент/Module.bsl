...
Процедура ВывестиСообщениеПользователю(ТекстСообщения, ИспользуютсяТолькоПартнеры, Поле="", ОчищатьСообщения = Ложь) Экспорт
	...
	Если ИспользуютсяТолькоПартнеры Тогда
		ТекстСообщения = СтрЗаменить(ТекстСообщения, НСтр("ru = 'партнера';
															|en = 'partner'"), НСтр("ru = 'контрагента';
																					|en = 'counterparty'"));
	КонецЕсли;
...
Процедура ВосстановитьОтображениеСпискаПослеПолнотекстовогоПоиска(Форма) Экспорт
	...
	Если Форма.РасширенныйПоиск 
		...
		Если Форма.РежимОтображенияДоПримененияПолнотекстовогоПоиска = НСтр("ru = 'Иерархический список';
																			|en = 'Hierarchical list'") Тогда
			Форма.Элементы.Список.Отображение = ОтображениеТаблицы.ИерархическийСписок;
		ИначеЕсли Форма.РежимОтображенияДоПримененияПолнотекстовогоПоиска = НСтр("ru = 'Дерево';
																				|en = 'Tree'") Тогда
	...
	Если Копирование Тогда
		Если НЕ Форма.УпрощенныйВводДоступен Тогда
			...
			ТекстСообщения = НСтр("ru = 'Копирование %1 запрещено.';
									|en = '%1 copying is prohibited.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			            ТекстСообщения, 
			            ?(Форма.ИспользоватьПартнеровКакКонтрагентов, НСтр("ru = 'контрагентов';
																			|en = 'counterparties'"), НСтр("ru = 'партнеров';
																										|en = 'partners'")));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Копирование партнеров запрещено.';
																	|en = 'Partner copying is prohibited.'"));
		КонецЕсли;
	Иначе
		Отказ = Истина;
	...
	Если ИсторияНаименований.Количество() > 0 Тогда
		...
		Для Счетчик = 0 По ИсторияНаименований.Количество() - 1 Цикл
			...
			Если Не ЭтоЮрЛицо Тогда
				СтрокаИсторииНаименований[ИмяРеквизитаСокращенноеНаименование] = СтрШаблон(НСтр("ru = 'ИП %1';
																								|en = 'IE %1'"), СтрокаНаименование.НаименованиеСокращенное);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
