...
Процедура ПроверитьВыгрузкуНоменклатуры() Экспорт 
	...
	Если ТипЗнч(Оповещения) = Тип("Массив") Тогда
		Для каждого Организация Из Оповещения Цикл
			...
			Пояснение = СтрШаблон(НСтр("ru = 'Завершена выгрузка по организации: %1';
										|en = 'Export by company is completed: %1'"), Организация);
			ПоказатьОповещениеПользователя(НСтр("ru = 'Выгрузка номенклатуры';
												|en = 'Products export'"), Обработчик, Пояснение, 
