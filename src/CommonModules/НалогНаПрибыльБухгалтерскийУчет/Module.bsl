...
Функция ВыражениеПоляЗапроса(ОписаниеЗапроса, ОператорЗапроса, ИмяПоля)
	...
	Если Поле = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Запрос не содержит поле %1';
				|en = 'Query does not contain %1 field'", Метаданные.ОсновнойЯзык.КодЯзыка),
			ИмяПоля);
	КонецЕсли;
...
Процедура ЗаписатьИзменениеОтложенногоНалога(Проводки, ТаблицаРасчета, КонтекстРасчета)
	...
	Для Каждого ОбщийРасчет Из ТаблицаРасчета Цикл
		...
		Представления.Увеличение.Вставить(
			ВидСчета.Активный,
			НСтр("ru = 'Признание отложенного налогового актива';
				|en = 'Deferred tax asset recognition'", Метаданные.ОсновнойЯзык.КодЯзыка));
			
		Представления.Уменьшение.Вставить(
			ВидСчета.Активный,
			НСтр("ru = 'Погашение отложенного налогового актива';
				|en = 'Repayment of deferred tax asset'", Метаданные.ОсновнойЯзык.КодЯзыка));
			
		Представления.Уменьшение.Вставить(
			ВидСчета.Пассивный,
			НСтр("ru = 'Признание отложенного налогового обязательства';
				|en = 'Deferred tax liability recognition'", Метаданные.ОсновнойЯзык.КодЯзыка));
			
		Представления.Увеличение.Вставить(
			ВидСчета.Пассивный,
			НСтр("ru = 'Погашение отложенного налогового обязательства';
				|en = 'Repayment of deferred tax liability'", Метаданные.ОсновнойЯзык.КодЯзыка));
			
		ШаблонПредставленияПересчетПоСтавке = НСтр(
			"ru = 'Пересчет %1 по ставке %2%%';
			|en = 'Recalculation %1 at a rate of %2%%'",
			Метаданные.ОсновнойЯзык.КодЯзыка);
		
		ПредставлениеПересчетПоставке = СтрШаблон(
			ШаблонПредставленияПересчетПоСтавке,
			НСтр("ru = 'отложенного налогового актива';
				|en = 'of deferred tax asset'", Метаданные.ОсновнойЯзык.КодЯзыка),
		...
		ПредставлениеПересчетПоставке = СтрШаблон(
			ШаблонПредставленияПересчетПоСтавке,
			НСтр("ru = 'отложенного налогового обязательства';
				|en = 'of deferred tax liability'", Метаданные.ОсновнойЯзык.КодЯзыка),
			СтавкаБудущегоНалога * 100);
...
Процедура РассчитатьРасходПоНалогу(ТаблицаРасчета, КонтекстРасчета)
	...
	Для каждого ОбщийРасчет Из ТаблицаРасчета Цикл
		...
		ПорядокРасчета = СтрШаблон(
			НСтр("ru = 'Оборот по кредиту счета %1 в корреспонденции со счетом %2 (со знаком ""минус"")';
				|en = 'Account %1 credit turnover in correspondence with account %2 (with negative sign)'", Расчет.КодЯзыка),
...
Процедура РассчитатьТекущийНалог(ТаблицаРасчета, КонтекстРасчета)
	...
	Для Каждого ОбщийРасчет Из ТаблицаРасчета Цикл
		...
		ВидУчета = НСтр("ru = 'данные по учету постоянных разниц';
						|en = 'permanent difference accounting data'", Расчет.КодЯзыка);
...
Функция ПорядокРасчетаФинансовыйРезультат(КодЯзыка, ВидУчета = "")
	...
	ПорядокРасчета = СтрШаблон(
		НСтр("ru = 'Оборот по счету %1 в корреспонденции со счетами %2,%3';
			|en = 'Account %1 turnover in correspondence with accounts %2,%3'", КодЯзыка),
	...
	Шаблон = НСтр("ru = '%1 (%2)';
					|en = '%1 (%2)'", КодЯзыка); // Оборот (по такому-то виду учета)
...
Процедура ДобавитьОписаниеРасчетаОтложенногоНалога(Расчет, ОтложенныйНалог, КонтекстРасчета)
	...
	ПорядокРасчета = СтрШаблон(
		НСтр("ru = 'Сальдо на начало периода по дебету счета %1';
			|en = 'Opening balance for account debit %1'", Расчет.КодЯзыка),
	...
	ПорядокРасчета = СтрШаблон(
		НСтр("ru = 'Сальдо на начало периода по кредиту счета %1';
			|en = 'Opening balance for account credit %1'", Расчет.КодЯзыка),
	...
	ПорядокРасчета = СтрШаблон(
		НСтр("ru = 'Сальдо на конец периода по дебету счета %1';
			|en = 'Closing balance for account debit %1'", Расчет.КодЯзыка),
		КодСчетаОтложенныеНалоговыеАктивы);
	ДобавитьЗаписьПротоколаРасчета(Расчет, "ОтложенныйНалоговыйАктивКонецПериода", ПорядокРасчета);
		
	ПорядокРасчета = СтрШаблон(
		НСтр("ru = 'Сальдо на конец периода по кредиту счета  %1';
			|en = 'Closing balance for account credit %1'", Расчет.КодЯзыка),
		КодСчетаОтложенныеНалоговыеОбязательства);
	...
	Если ЗначениеЗаполнено(Расчет.ВлияниеУбыткаТекущегоПериода) Тогда
		...
		ПорядокРасчета = СтрШаблон(
			НСтр("ru = 'Сальдо на конец периода по дебету счета  %1 - вид актива %2';
				|en = 'Closing balance for account debit %1 - asset type %2'", Расчет.КодЯзыка),
			КодСчетаОтложенныеНалоговыеАктивы,
...
Процедура ЗаписатьРасчетНалога(Проводки, ТаблицаРасчета, КонтекстРасчета)
	...
	ВариантПроводки.ПредставлениеКт = НСтр("ru = 'Условный расход по налогу на прибыль';
											|en = 'Provisional corporate profit tax expense'", Метаданные.ОсновнойЯзык.КодЯзыка);
	ВариантПроводки.КорСчетКт       = ПланыСчетов.Хозрасчетный.УсловныйРасходПоНалогуНаПрибыль;
	ВариантПроводки.ПредставлениеДт = НСтр("ru = 'Условный доход по налогу на прибыль';
											|en = 'Provisional corporate profit tax income'", Метаданные.ОсновнойЯзык.КодЯзыка);
	...
	ВариантПроводки.ПредставлениеКт = НСтр("ru = 'Постоянный налоговый расход';
											|en = 'Constant tax expense'", Метаданные.ОсновнойЯзык.КодЯзыка);
	ВариантПроводки.КорСчетКт       = ПланыСчетов.Хозрасчетный.ПостоянноеНалоговоеОбязательство;
	ВариантПроводки.ПредставлениеДт = НСтр("ru = 'Постоянный налоговый доход';
											|en = 'Constant tax income'", Метаданные.ОсновнойЯзык.КодЯзыка);
	...
	ВариантПроводки.ПредставлениеКт = НСтр("ru = 'Корректировка отложенного налога в связи с выбытием активов и обязательств по деятельности, не облагаемой налогом на прибыль';
											|en = 'Deferred tax adjustment for the disposal of assets and liabilities for business activity not subject to income tax'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
Процедура ЗаписатьВлияниеУбыткаТекущегоПериода(Проводки, ТаблицаРасчета, КонтекстРасчета)
	...
	Для каждого ОбщийРасчет Из ТаблицаРасчета Цикл
		...
		Представление = НСтр("ru = 'Убыток текущего периода';
							|en = 'Current period loss'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
Процедура ЗакрытьСчетРасчетаНалога(Проводки, КонтекстРасчета)
	...
	// Представления записываются в ИБ, поэтому локализуются основным языком конфигурации.
	ПредставлениеПроводки = НСтр("ru = 'Перенос сальдо счета 68.04.2';
								|en = 'Transfer balance of account 68.04.2'", Метаданные.ОсновнойЯзык.КодЯзыка);
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ХозрасчетныйОстатки.Субконто1    КАК ВариантНалогообложенияПрибыли,
	...
	СоответствиеСальдо = Новый Соответствие();
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		СоответствиеСальдо.Вставить(Выборка.ВариантНалогообложенияПрибыли, Выборка.Сальдо);
	КонецЦикла;
	...
	Для Каждого Проводка Из Проводки Цикл
		...
		Если Проводка.СчетДт = СчетРасчетаНалога Тогда
			...
			ВариантНалогообложенияПрибыли = Проводка.СубконтоДт[ВидыСубконтоВариантыНалогообложения];
			Сальдо = СоответствиеСальдо.Получить(ВариантНалогообложенияПрибыли);
			Если Не ЗначениеЗаполнено(Сальдо) Тогда
				Сальдо = 0;
			КонецЕсли;
			СоответствиеСальдо.Вставить(ВариантНалогообложенияПрибыли, Сальдо + Проводка.Сумма);
			
		КонецЕсли;
		
		Если Проводка.СчетКт = СчетРасчетаНалога Тогда
			
			ВариантНалогообложенияПрибыли = Проводка.СубконтоКт[ВидыСубконтоВариантыНалогообложения];
			Сальдо = СоответствиеСальдо.Получить(ВариантНалогообложенияПрибыли);
			Если Не ЗначениеЗаполнено(Сальдо) Тогда
				Сальдо = 0;
			КонецЕсли;
	...
	Для каждого КлючИЗначениеСальдо Из СоответствиеСальдо Цикл
	
		ВариантНалогообложенияПрибыли = КлючИЗначениеСальдо.Ключ;
		Сальдо                        = КлючИЗначениеСальдо.Значение;
		...
		Если Сальдо > 0 Тогда
			...
			Проводка.СубконтоКт.Вставить(ВидыСубконтоВариантыНалогообложения, ВариантНалогообложенияПрибыли);
		...
		Иначе
			...
			Проводка.СубконтоДт.Вставить(ВидыСубконтоВариантыНалогообложения, ВариантНалогообложенияПрибыли);
