...
Функция ПолучитьСписокПериодов(Знач НачалоПериода, Знач ВидПериода) Экспорт
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Полугодие") Тогда
		...
		Для Сч = ТекущийГод - 1 По ТекущийГод + 1 Цикл
			СписокПериодов.Добавить(Дата(Сч, 1, 1), СтрШаблон(НСтр("ru = 'I полугодие %1';
																	|en = 'Half-year I %1'"), Формат(Сч, "ЧГ=0")));
			СписокПериодов.Добавить(Дата(Сч, 7, 1), СтрШаблон(НСтр("ru = 'II полугодие %1';
																	|en = 'Half-year II %1'"), Формат(Сч, "ЧГ=0")));
		КонецЦикла;
		СписокПериодов.Добавить(Дата(ТекущийГод + 2, 1, 1), Формат(ТекущийГод + 2, "ЧГ=0") + "...");
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Квартал") Тогда
		...
		Для Сч = ТекущийГод - 1 По ТекущийГод Цикл
			СписокПериодов.Добавить(Дата(Сч, 1, 1),	 СтрШаблон(НСтр("ru = '1 квартал %1';
																		|en = 'Quarter 1 %1'"), Формат(Сч, "ЧГ=0")));
			СписокПериодов.Добавить(Дата(Сч, 4, 1),	 СтрШаблон(НСтр("ru = '2 квартал %1';
																		|en = 'Quarter 2 %1'"), Формат(Сч, "ЧГ=0")));
			СписокПериодов.Добавить(Дата(Сч, 7, 1),	 СтрШаблон(НСтр("ru = '3 квартал %1';
																		|en = 'Quarter 3 %1'"), Формат(Сч, "ЧГ=0")));
			СписокПериодов.Добавить(Дата(Сч, 10, 1), СтрШаблон(НСтр("ru = '4 квартал %1';
																	|en = 'Quarter 4 %1'"), Формат(Сч, "ЧГ=0")));
		КонецЦикла;
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Декада") Тогда
		...
		Пока Сч >0 Цикл
			СписокПериодов.Добавить(Дата(СчГод, СчМесяц, 1),  СтрШаблон(НСтр("ru = 'I дек. %1';
																			|en = 'Ten-day period 1, %1'"), Нрег(Формат(Дата(СчГод, СчМесяц, 1), "ДФ='MMMM yyyy'"))));
			СписокПериодов.Добавить(Дата(СчГод, СчМесяц, 11), СтрШаблон(НСтр("ru = 'II дек. %1';
																			|en = 'Ten-day period 2, %1'"), Нрег(Формат(Дата(СчГод, СчМесяц, 1), "ДФ='MMMM yyyy'"))));
			СписокПериодов.Добавить(Дата(СчГод, СчМесяц, 21), СтрШаблон(НСтр("ru = 'III дек. %1';
																			|en = 'Ten-day period 3, %1'"), Нрег(Формат(Дата(СчГод, СчМесяц, 1), "ДФ='MMMM yyyy'"))));
			СчМесяц = СчМесяц + 1;
...
//
// Параметры:
//  Текст   - Строка - текст, содержащий данные периода отчета.
//
// Возвращаемое значение:
