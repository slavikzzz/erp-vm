...
Функция ДанныеСокращения(ВходящийСписок, КоличествоЭлементов = 2) Экспорт
	...
	Если СколькоЕще = 0 Тогда
		ТекстЕще = "";
	Иначе
		ТекстЕще = СтрШаблон(НСтр("ru = '%1( + еще %2)';
									|en = '%1( + еще %2)'"), " ", СколькоЕще);
	КонецЕсли;
...
Процедура ИсключитьДействующиеТоварныеГруппыОбязательногоРозничногоКонтроля(ГруппаПараметровНастройки)
	...
	ДействующиеТоварныеГруппыОбязательногоРозничногоКонтроля = ОбщегоНазначенияИСМПВызовСервера.СписокУчитываемойПродукцииТребующейОбязательнойОнлайнПроверкиПриПродажеВРозницуНаДату();
