...
Процедура ВвестиУведомлениеОЗаключенииДоговораСИностранцем(ПараметрКоманды, ДополнительныеПараметры) Экспорт
	
	ВвестиДокументПоИностранномуГражданину(
		ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ЗаключениеТрудовогоДоговора"),
		ПараметрКоманды,
		ДополнительныеПараметры);
	
КонецПроцедуры

Процедура ВвестиУведомлениеОХодатайствеПривлеченияИностранца(ПараметрКоманды, ДополнительныеПараметры) Экспорт
	
	ВвестиДокументПоИностранномуГражданину(
		ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ХодатайствоОПривлечении"),
		ПараметрКоманды,
		ДополнительныеПараметры);
	
КонецПроцедуры

Процедура ВвестиУведомлениеОРасторженииДоговораСИностранцем(ПараметрКоманды, ДополнительныеПараметры) Экспорт
	
	ВвестиДокументПоИностранномуГражданину(
		ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.РасторжениеТрудовогоДоговора"),
		ПараметрКоманды,
		ДополнительныеПараметры);
	
КонецПроцедуры

Процедура ВвестиУведомлениеОбИсполненииОбязательствПередИностраннымСпециалистом(ПараметрКоманды, ДополнительныеПараметры) Экспорт
	
	ВвестиДокументПоИностранномуГражданину(
		ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ОбязательстваВыплатаЗП"),
		ПараметрКоманды,
		ДополнительныеПараметры);
	
КонецПроцедуры

Процедура ВвестиДокументПоИностранномуГражданину(ВидУведомления, Основание, ПараметрыКоманды)
	
	Если Основание = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ДокументыИностранца = КадровыйУчетВызовСервера.ДокументыПоИностраннымГражданам(ВидУведомления, Основание);
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ВидУведомления", ВидУведомления);
	ДополнительныеПараметры.Вставить("Основание", Основание);
	ДополнительныеПараметры.Вставить("ДокументыИностранца", ДокументыИностранца);
	ДополнительныеПараметры.Вставить("ПараметрыКоманды", ПараметрыКоманды);
	
	Оповещение = Новый ОписаниеОповещения("ВвестиДокументПоИностранномуГражданинуЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	Если ДокументыИностранца.Количество() > 0 Тогда
		
		Кнопки = Новый СписокЗначений;
		Если ДокументыИностранца.Количество() = 1 Тогда
			ТекстВопроса = НСтр("ru = 'Найден документ, введенный на основании.';
								|en = 'A generated document is found.'");
			Кнопки.Добавить("ОткрытьСуществующий", НСтр("ru = 'Открыть документ';
														|en = 'Open document'"));
		Иначе
			ТекстВопроса = НСтр("ru = 'Найдены документы, введенные на основании.';
								|en = 'Generated documents are found.'");
			Кнопки.Добавить("ПоказатьСписок", НСтр("ru = 'Показать список';
													|en = 'Show list'"));
		КонецЕсли;
...
Процедура ВвестиДокументПоИностранномуГражданинуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = "СоздатьНовый" Тогда
		
		ДанныеДляЗаполнения = КадровыйУчетВызовСервера.ДанныеЗаполненияДокументовПоИностраннымГражданам(
			ДополнительныеПараметры.ВидУведомления,
			ДополнительныеПараметры.Основание);
		
		УведомлениеОСпецрежимахНалогообложенияИнтеграцияКлиент.ОткрытьТрудовойДоговорСИностранцем(
			ДанныеДляЗаполнения.Организация,
			ДополнительныеПараметры.ВидУведомления,
			ДанныеДляЗаполнения.ДанныеЗаполнения,
			РежимОткрытияОкнаФормы.БлокироватьОкноВладельца,
			ДополнительныеПараметры.ПараметрыКоманды.Форма);
		
	ИначеЕсли Результат = "ОткрытьСуществующий" Тогда
		ПоказатьЗначение(, ДополнительныеПараметры.ДокументыИностранца[0]);
	ИначеЕсли Результат = "ПоказатьСписок" Тогда
		ПараметрыОткрытия = Новый Структура(
			"Отбор", Новый Структура("Ссылка", ДополнительныеПараметры.ДокументыИностранца));
		ОткрытьФорму("Документ.УведомлениеОСпецрежимахНалогообложения.ФормаСписка", ПараметрыОткрытия, ДополнительныеПараметры.ПараметрыКоманды.Форма);
	КонецЕсли;
