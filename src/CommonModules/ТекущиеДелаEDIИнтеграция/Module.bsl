...
Процедура ВывестиИнформациюПоВиджетуКонтрольПоступлений(Форма, ПараметрыВыводаВиджетов, Виджет)
	...
	Для Каждого РазделВиджета Из ТекущиеДелаEDIКлиентСервер.РазделыВиджета(Виджет) Цикл
		...
		Заголовок = Новый ФорматированнаяСтрока(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2)';
																											|en = '%1 (%2)'"), Заголовок, ДанныеРаздела.Количество),
		                                        ,
		                                        ЦветТекста,
		                                        ,
...
Процедура ВывестиИнформациюПоРазделуАрхив(Форма, ПараметрыВывода, ЕстьРазделыДляОтображения)
	...
	Если ПараметрыВывода.ДанныеРаздела.ЕстьВыполнен Тогда
		...
		ТекущиеДелаEDI.ВывестиДекорацию(Форма, НСтр("ru = 'Выполнен';
													|en = 'Completed'"), ПараметрыВывода.ПрефиксГиперссылки + "Выполнен", ИмяТекущейДекорации);
		ТекущиеДелаEDI.ОпределитьИмяСледующейДекорации(ИмяТекущейДекорации, ИмяДекорации, ПостфиксДекорации);
	...
	Если ПараметрыВывода.ДанныеРаздела.ЕстьОтменен Тогда
		...
		ТекущиеДелаEDI.ВывестиДекорацию(Форма, НСтр("ru = 'Отменен';
													|en = 'Canceled'"), ПараметрыВывода.ПрефиксГиперссылки + "Отменен", ИмяТекущейДекорации);
		ТекущиеДелаEDI.ОпределитьИмяСледующейДекорации(ИмяТекущейДекорации, ИмяДекорации, ПостфиксДекорации);
...
Процедура ВывестиИнформациюПоРазделуСтатусы(Форма, ПараметрыВывода, ЕстьРазделыДляОтображения)
	...
	Для Каждого Статус Из ПараметрыВывода.МассивСтатусов Цикл
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2)';
																				|en = '%1 (%2)'"), ПредставлениеСтатуса, КоличествоПоСтатусу);
	...
	ЗаголовокГруппы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2)';
																					|en = '%1 (%2)'"), ПараметрыВывода.ИмяИтоговойГруппы, КоличествоВсего);
...
Процедура ВывестиИнформациюПоРазделуЗаказыПоставщикуДоступныеДляОтправки(Форма, ДанныеРаздела, Виджет, ЕстьРазделыДляОтображения)
	...
	Элементы.ДекорацияЗаказПоставщикуДоступныеДляОтправки.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Заказов, готовых к отправке (%1)';
																																	|en = 'Orders ready to ship (%1)'"), ДанныеРаздела);
