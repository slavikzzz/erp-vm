...
Процедура ДобавитьОтборДляПоказателяКонтроль(ПараметрыОтчета, КомпоновщикНастроек, ВПользовательскиеНастройки = Истина) Экспорт
	...
	Если Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыОтчета, "ПоказательКонтроль", Ложь)
...
Функция ИспользоватьСуммыРазниц(Организация, НачалоПериода, КонецПериода) Экспорт
	...
	Если НалогНаПрибыльБухгалтерскийУчет.ИспользоватьСуммыРазниц(Организация, КонецПериода) Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Возврат НалогНаПрибыльБухгалтерскийУчет.ИспользоватьСуммыРазниц(Организация, НачалоПериода);
	...
	Если Проверки.Свойство("ВыборПоказателя") И Проверки.ВыборПоказателя Тогда
		...
		Если КоличествоПоказателей = 0 И НаборПоказателей.Количество() > 0 Тогда
			ТекстСообщения = НСтр("ru = 'Не выбран показатель(и)';
									|en = 'Indicator(s) is not selected '");
	...
	Если Проверки.Свойство("СписокВидовСубконто") И Проверки.СписокВидовСубконто Тогда
		...
		Если ОтчетОбъект.СписокВидовСубконто.Количество() > МаксКоличествоСубконто Тогда
			ТекстСообщения = НСтр("ru = 'Выбрано слишком много видов субконто, максимально допустимо %1';
									|en = 'Too many extra dimension types are selected, maximum allowed number is %1'");
...
Процедура ОбработкаПроверкиЗаполненияОтборов(ОтчетОбъект, Отказ) Экспорт
	...
	Для каждого Отбор Из ОтборыВерхнегоУровня Цикл
		...
		ТекстСообщения = НСтр("ru = 'Поле с реквизитами субконто ""%1"" нельзя использовать в групповом отборе с иными полями.';
								|en = 'The field with attributes of extra dimension ""%1"" cannot be used in group filter with other fields.'");
...
Функция ОписаниеОтборовОтчета(ПараметрыОтчета, КомпоновщикНастроек)
	...
	Если ПараметрыОтчета.Свойство("Подразделение") И ЗначениеЗаполнено(ПараметрыОтчета.Подразделение) Тогда
		ТекстПодразделение = СтрШаблон(НСтр("ru = 'Подразделение Равно ""%1""';
											|en = 'Business unit Equals ""%1""'"), ПараметрыОтчета.Подразделение);
	КонецЕсли;
...
Процедура ДополнитьТекстОтбора(ОсновнойТекстОтбор, ДополнительныйТекстОтбор)
	...
	Если Не ПустаяСтрока(ОсновнойТекстОтбор) И Не ПустаяСтрока(ДополнительныйТекстОтбор) Тогда
		ОсновнойТекстОтбор = ДополнительныйТекстОтбор + " " + НСтр("ru = 'И';
																	|en = 'AND'") + " " + ОсновнойТекстОтбор;
...
Функция ПолучитьПараметрыИсполненияОтчета(ПараметрыОтчета, МенеджерОтчета)
	...
	ПараметрыПоУмолчанию.Вставить("ВременныеТаблицыЗапроса",                    Ложь);
...
//
// Возвращаемое значение:
//  Неопределено, ПараметрСхемыКомпоновкиДанных - Скопировать параметр схемы компоновки данных
...
Процедура СохранитьТипФайлаТабличногоДокументаПоУмолчанию(ТипФайла) Экспорт
	...
	ОписаниеНастройки.Представление = НСтр("ru = 'Значение по-умолчанию для формата в диалоге выбора файла при сохранении отчета';
											|en = 'Default format value in the file selection dialog box when saving the report'");
...
Функция ПодготовитьОтчет(ПараметрыОтчета) Экспорт
	...
	Попытка
		...
		Если ПараметрыИсполненияОтчета.ИспользоватьВременныеТаблицыЗапроса Тогда
			МенеджерВременныхТаблицЗапроса = МенеджерОтчета.МенеджерВременныхТаблицОтчета(ПараметрыОтчета, 
				МакетКомпоновки);
		КонецЕсли;
		...
		Если ВнешниеНаборыДанных = Неопределено Тогда
			ПроцессорКомпоновки.Инициализировать(МакетКомпоновки, , ДанныеРасшифровкиОбъект,
				Истина, , МенеджерВременныхТаблицЗапроса);
		Иначе
			ПроцессорКомпоновки.Инициализировать(МакетКомпоновки, ВнешниеНаборыДанных,
				ДанныеРасшифровкиОбъект, Истина, , МенеджерВременныхТаблицЗапроса);
		КонецЕсли;
...
// Описание аналитики счета для использования в настройке отчетов
//
// Параметры:
...
//
Функция ОписаниеАналитикиСчета(Счет, ДополнитьПодразделением, ВключенныеСубконто = Неопределено) Экспорт
	...
	ОписаниеАналитики.Вставить("КоличествоВключеннойАналитики", 0);
	ОписаниеАналитики.Вставить("ШифрАналитики",                 "");
	ОписаниеАналитики.Вставить("Представление",                 "");
	// В поле ШифрАналитики в виде позиционной строки закодированы используемые субконто - 
	// см. БухгалтерскиеОтчетыКлиент.ТабличноеПолеПоСчетамПредставлениеНачалоВыбора,
	// БухгалтерскиеОтчетыКлиент.ТабличноеПолеПоСчетамПредставлениеОбработкаВыбора
	
	НеиспользуемаяАналитика = ПланыСчетов.Хозрасчетный.НеиспользуемаяАналитика();
	
	// ДоступнаяАналитика - вся, которая может физически быть записана в регистр бухгалтерии по этому счету.
	// При этом в списке субконто должны быть все субконто счета, включая неиспользуемую аналитику - 
	// см. БухгалтерскиеОтчетыКлиент.ТабличноеПолеПоСчетамПредставлениеНачалоВыбора
	ДоступнаяАналитика     = ВсяДоступнаяАналитикаСчета(Счет, ДополнитьПодразделением);
	
	ЭлементыПредставления  = Новый Массив; // представление содержит включенную аналитику
	ИспользованиеАналитики = Новый Массив; // ключ - строка с номером субконто, значение - вид субконто
	
	Для Каждого Аналитика Из ДоступнаяАналитика Цикл
		
		Если НеиспользуемаяАналитика.Найти(Аналитика.КлючАналитики) = Неопределено
			И (ВключенныеСубконто = Неопределено ИЛИ ВключенныеСубконто[Аналитика.КлючАналитики] <> Неопределено) Тогда
			ИспользованиеАналитики.Добавить("+" + Аналитика.КлючШифра);
			ЭлементыПредставления.Добавить(Аналитика.Представление);
		Иначе
			ИспользованиеАналитики.Добавить("-" + Аналитика.КлючШифра);
		КонецЕсли;
	...
	ОписаниеАналитики.КоличествоВключеннойАналитики = ЭлементыПредставления.Количество();
	ОписаниеАналитики.Представление = СтрСоединить(ЭлементыПредставления, ", ");
	ОписаниеАналитики.ШифрАналитики = СтрСоединить(ИспользованиеАналитики);
	
	Возврат ОписаниеАналитики;
	
КонецФункции

Функция ВсяДоступнаяАналитикаСчета(Счет, ВключатьПризнакиУчета = Истина)
	
	СвойстваСчета = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(Счет);
	
	АналитикаСчета = Новый Массив;
	ПараметрыСтрокой = "КлючШифра, КлючАналитики, Представление";
	
	Если ВключатьПризнакиУчета И СвойстваСчета.УчетПоНаправлениямДеятельности Тогда
		Параметры = Новый Структура(ПараметрыСтрокой, "!", "НаправлениеДеятельности", НСтр("ru = 'Направление деятельности';
																							|en = 'Line of business'"));
		АналитикаСчета.Добавить(Параметры);
	КонецЕсли;
	
	Если ВключатьПризнакиУчета И СвойстваСчета.УчетПоПодразделениям Тогда
		Параметры = Новый Структура(ПараметрыСтрокой, "0", "Подразделение", НСтр("ru = 'Подразделения';
																				|en = 'Business units'"));
		АналитикаСчета.Добавить(Параметры);
	КонецЕсли;
	...
	Для НомерСубконто = 1 По СвойстваСчета.КоличествоСубконто Цикл
		...
		Параметры = Новый Структура(ПараметрыСтрокой,
			КлючСубконто, СвойстваСчета["ВидСубконто" + КлючСубконто],
			СвойстваСчета["ВидСубконто" + КлючСубконто + "Наименование"]);
		АналитикаСчета.Добавить(Параметры);
	КонецЦикла;
...
Функция ВозможныеРасшифровки(Знач ИмяОтчета, Знач АдресРасшифровки, Знач ИдентификаторРасшифровки) Экспорт
	...
	ДанныеРасшифровки = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Расшифровка, "ДанныеРасшифровки");
