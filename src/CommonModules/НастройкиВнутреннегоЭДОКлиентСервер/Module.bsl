
#Область СлужебныйПрограммныйИнтерфейс

// Возвращает описание полей ключа настроек отправки, см. ТекстЗапросаДополнениеНастройкамиВнутреннегоЭДО.
// 
// Возвращаемое значение:
// 	Структура:
// * Организация - Строка
// * ВидДокумента - Строка
Функция НовоеОписаниеПолейКлючаНастройкиВнутреннегоЭДО() Экспорт
	
	ПоляКлючаНастройки = Новый Структура;
	ПоляКлючаНастройки.Вставить("Организация", "");
	ПоляКлючаНастройки.Вставить("ВидВнутреннегоДокумента", "");
	
	Возврат ПоляКлючаНастройки;
	
КонецФункции

Функция ВидЭлектроннойПодписиПоВидамДокументов() Экспорт
	
	Возврат НСтр("ru = 'По видам документов';
				|en = 'By document kinds'");
	
КонецФункции

Функция ВидЭлектроннойПодписиПЭП_УНЭП() Экспорт
	
	Возврат НСтр("ru = 'Простая и усиленная неквалифицированная';
				|en = 'Simple and enhanced non-certified'");
	
КонецФункции

#КонецОбласти




