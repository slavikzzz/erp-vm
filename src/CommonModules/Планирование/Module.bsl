...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Если Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		//++ НЕ УТ
		Параметры.ПодчиненныеРегистры.Добавить(Метаданные.РегистрыНакопления.ПланыВыпускаИзделий);
		Параметры.ПодчиненныеРегистры.Добавить(Метаданные.РегистрыНакопления.ПланыПроизводства);
		//-- НЕ УТ
...
Процедура ОтразитьДвижения(ТаблицыДляДвижений, Движения, Отказ) Экспорт
	...
	//++ НЕ УТ
	ПроведениеДокументов.ОтразитьДвижения(ТаблицыДляДвижений, Движения, "ПланыВыпускаИзделий");
	ПроведениеДокументов.ОтразитьДвижения(ТаблицыДляДвижений, Движения, "ПланыПроизводства");
	//-- НЕ УТ
...
Функция ПолучитьПериодыПлановОстатков(Параметры) Экспорт 
	...
	|
	|ВЫБРАТЬ
	|	ПериодПланыПроизводства.Период
	|ИЗ
	|	(ВЫБРАТЬ ПЕРВЫЕ 1
	|		ПланПроизводства.НачалоПериода КАК Период
	|	ИЗ
	|		Документ.ПланПроизводства КАК ПланПроизводства
	|	ГДЕ
	|		ПланПроизводства.НачалоПериода <= &КонецПериода
	|		И ПланПроизводства.Ссылка <> &Ссылка
	|		И ПланПроизводства.Сценарий = &Сценарий
	|		И ПланПроизводства.Статус В(&СтатусыСценария)
	|	
	|	УПОРЯДОЧИТЬ ПО
	|		Период) КАК ПериодПланыПроизводства
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ПериодПланыПотребленияМатериаловОбороты.Период
	|ИЗ
	|	(ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|					И ПланПроизводства <> &Ссылка) КАК ПланыПотребленияМатериаловОбороты
	|	
	|	УПОРЯДОЧИТЬ ПО
	|		ПланыПотребленияМатериаловОбороты.Период) КАК ПериодПланыПотребленияМатериаловОбороты
	//-- НЕ УТ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Процедура ОбработкаПроверкиЗаполненияВФорме(Таблица, СтруктураДействий, Отказ, ПроверяемыеРеквизиты, Знач КроссТаблица = Истина) Экспорт 
	...
	Если Таблица.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Не заполнен список ""%ПредставлениеТаблицы%""';
								|en = 'List ""%ПредставлениеТаблицы%"" is not filled in'");
	...
	ТекстОшибкиКомплектация     = НСтр("ru = 'Не заполнена колонка ""Комплектация""';
										|en = 'Column ""Product kit"" is not filled in'");
	//++ НЕ УТ
	ТекстОшибкиСпецификация 	= НСтр("ru = 'Не заполнена колонка ""Спецификация""';
										|en = 'Column ""BOM"" is not filled in'");
	//-- НЕ УТ
	
	ШаблонАдресаОшибки = " " + НСтр("ru = 'в строке %НомерСтроки% списка ""%Товары%""';
									|en = 'in line %НомерСтроки% of the ""%Товары%"" list'");
	...
	Для Каждого СтрокаДублей Из ТаблицаПроверкиДублирующихСтрок Цикл
		Если СтрокаДублей.КоличествоДублирующихСтрок > 1 Тогда
			ТекстОшибкиКоличества = НСтр("ru = 'Найдены дублирующиеся по ключевым полям строки:';
										|en = 'Duplicate lines are found by key fields:'");
			...
			Для Каждого Строка Из Таблица.НайтиСтроки(Отбор) Цикл
				ТекстОшибкиКоличества = ТекстОшибкиКоличества + " " + НСтр("ru = 'строка';
																			|en = 'string'") + " " + Строка.НомерСтроки + ";";
			КонецЦикла;
...
Процедура ОбработкаПроверкиЗаполненияПоСценариюВидуПлана(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт 
	...
	Если Объект.Периодичность <> ПараметрыСценария.Периодичность Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Периодичность документа не соответствует периодичности сценария.';
				|en = 'Document frequency does not correspond to the scenario frequency.'"), Объект, "Сценарий",,Отказ);
	КонецЕсли; 
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПланЗакупок") Тогда
		...
		Если Объект.ПланироватьПоСумме <> ПараметрыСценария.ПланЗакупокПланироватьПоСумме Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Настройка планирования по количеству и сумме в документе не соответствует настройке сценария.';
					|en = 'Setting of planning according to the quantity and document amount does not correspond to the script setting.'"), Объект, "Сценарий",,Отказ);
		КонецЕсли;
			
		Если ПараметрыСценария.ПланЗакупокПланироватьПоСумме И Объект.Валюта <> ПараметрыСценария.Валюта Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Валюта в документе не соответствует валюте сценария.';
					|en = 'The currency in the document does not match the scenario currency.'"), Объект, "Сценарий",,Отказ);
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.ПланПродаж") Тогда
		...
		Если Объект.ПланироватьПоСумме <> ПараметрыСценария.ПланПродажПланироватьПоСумме Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Настройки планирования по количеству и сумме в документе не соответствует настройкам сценария.';
					|en = 'Settings of planning by quantity and amount in the document do not correspond to the script settings.'"), Объект, "Сценарий",,Отказ);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(Объект.ВидПлана) Тогда
		...
		Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПланЗакупок") Тогда
			...
			Если Объект.ПланироватьПоСумме И Объект.ЗаполнятьПланОплат <> НастройкиЗаполнения.ЗаполнятьПланОплат Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru = 'Настройки заполнения плана оплат в документе не соответствует настройкам вида плана.';
						|en = 'Settings of payment plan population in the document do not correspond to the plan profile settings.'"), 
					Объект, 
					"ВидПлана",
					,
					Отказ);
			...
			Если Не ПараметрыСценария.ОтражаетсяВБюджетировании И ВидПланаОтражаетсяВБюджетировании Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru = 'Настройки отражения бюджетирования в сценарии не соответствую настройкам вида плана.';
						|en = 'Settings of budgeting recording in the script do not correspond to the plan profile settings.'"),
					Объект, 
					"Сценарий",
					,
					Отказ);
		...
		Если НастройкиЗаполнения.ЗаполнятьПоДефициту Тогда
			...
			//++ НЕ УТ
			ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.ПланПроизводства") Тогда
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПланЗакупок", "ПланПроизводства");
			//-- НЕ УТ
			КонецЕсли;
			...
			Если Запрос.Выполнить().Выгрузить().Количество()>0 Тогда
				...
				НСтр("ru = 'В рамках вида плана, заполняемого по дефицитам, может быть создан только один план в периоде.';
					|en = 'Only one plan in the period can be created within the plan type filled in by shortages'"),
				Объект,
				,
				, 
				Отказ);
...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	//++ НЕ УТ
	ИначеЕсли ТипЗнч(Форма.Объект.Ссылка) = Тип("ДокументСсылка.ПланПроизводства") Тогда
		Форма.ТипПлана = Перечисления.ТипыПланов.ПланПроизводства;
	//-- НЕ УТ
	КонецЕсли;
...
Функция ДополнительныеПараметрыДляРасчетаПоФормуле(Параметры) Экспорт
	...
	//++ НЕ УТ
							Или ИспользоватьДляПланированияМатериалов
	//-- НЕ УТ
...
Функция ЗаполнитьПоФормуле(Параметры) Экспорт 
	...
	Если ТаблицаРезультата.Колонки.Найти("ПланыПродаж") <> Неопределено
		//++ НЕ УТ
		Или ТаблицаРезультата.Колонки.Найти("ПланыПроизводстваМатериалы") <> Неопределено
		//-- НЕ УТ
	...
	Если ТаблицаРезультата.Колонки.Найти("ПланыПродажСОтбором") <> Неопределено
		//++ НЕ УТ
		Или ТаблицаРезультата.Колонки.Найти("ПланыПроизводстваМатериалыСОтбором") <> Неопределено
		//-- НЕ УТ
	...
	Если ТаблицаРезультата.Колонки.Найти("ПланыЗакупок") <> Неопределено
		//++ НЕ УТ
		Или ТаблицаРезультата.Колонки.Найти("ПланыВыпуска") <> Неопределено
		//-- НЕ УТ
	...
	Если ТаблицаРезультата.Колонки.Найти("ПланыЗакупокСОтбором") <> Неопределено
		//++ НЕ УТ
		Или ТаблицаРезультата.Колонки.Найти("ПланыВыпускаСОтбором") <> Неопределено
		//-- НЕ УТ
...
Процедура РассчитатьПланыПриходаПланыРасхода(ТекущиеДанные, Разделитель)
	...
	Если ТекущиеДанные.Свойство("ПланыРасхода"+Разделитель) Тогда
		...
		//++ НЕ УТ
		+ ?(ТекущиеДанные.Свойство("ПланыПроизводстваМатериалы"+Разделитель),
		ТекущиеДанные["ПланыПроизводстваМатериалы"+Разделитель],0)
		+ ?(ТекущиеДанные.Свойство("ПланыПроизводстваПотребностьВПолуфабрикатах"+Разделитель),
		ТекущиеДанные["ПланыПроизводстваПотребностьВПолуфабрикатах"+Разделитель],0)
		//-- НЕ УТ
	...
	Если ТекущиеДанные.Свойство("ПланыПрихода"+Разделитель) Тогда
		...
		//++ НЕ УТ
		+ ?(ТекущиеДанные.Свойство("ПланыВыпуска"+Разделитель),
		ТекущиеДанные["ПланыВыпуска"+Разделитель],0)
		//-- НЕ УТ
...
Функция ПолучитьЗнакКорректировки(Форма) Экспорт
	...
	ИначеЕсли ТипЗнч(Форма.Объект.Ссылка) = Тип("ДокументСсылка.ПланПродаж")
		...
		//++ НЕ УТКА
		Или (ТипЗнч(Форма.Объект.Ссылка) = Тип("ДокументСсылка.ПланПроизводства")
		И Форма.Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Разборка)
		//-- НЕ УТКА
...
Функция ОсновнойОперандПлана(Форма, СОтбором = Ложь) Экспорт
	...
	//++ НЕ УТ
	ИначеЕсли ТипЗнч(Форма.Объект.Ссылка) = Тип("ДокументСсылка.ПланПроизводства") Тогда
		Если Форма.Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Ремонт Тогда
			Возврат ?(СОтбором,"ПланыПроизводстваРемонтСОтбором","ПланыПроизводстваРемонт");
		ИначеЕсли Форма.Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Разборка Тогда
			Возврат ?(СОтбором,"ПланыПроизводстваРазборкаСОтбором","ПланыПроизводстваРазборка");
		Иначе
			Возврат ?(СОтбором,"ПланыПроизводстваСОтбором","ПланыПроизводства");
		КонецЕсли;
	//-- НЕ УТ
	КонецЕсли;
...
Функция ПодсистемаПланирование() Экспорт
	...
	Иначе
		...
		Результат = Метаданные.Подсистемы.БюджетированиеИПланирование;
...
Процедура ПроверитьНаличиеБюджетнойЗадачи(Отказ, Ссылка, ЭтотОбъект) Экспорт
	...
	Если Не ЭтотОбъект.ОтражаетсяВБюджетировании Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗапускВыполненияФоновогоПроведенияПлана() Экспорт
	...
	НаименованиеЗадания = НСтр("ru = 'Замещение/отмена замещения планов';
								|en = 'Plan substitution/substitution cancellation'");
...
Процедура ВыполнитьФоновоеПроведенияПлана(ПараметрыЗадания) Экспорт
	...
	Если ПараметрыЗадания.Свойство("КоличествоОшибок") Тогда
		...
		Если ПараметрыЗадания.КоличествоОшибок >= МаксимальноеКоличествоОшибок Тогда
			...
			ТекстСообщения = НСтр("ru = 'Фоновое замещение/отмена замещения планов остановлена. Количество неудачных попыток: %КоличествоОшибок%';
									|en = 'Background plan substitution/substitution cancellation is stopped. Number of failed attempts: %КоличествоОшибок%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОшибок%", ПараметрыЗадания.КоличествоОшибок);
			
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Фоновое замещение/отмена замещения плана';
											|en = 'Background plan substitution/substitution cancellation'", КодОсновногоЯзыка),
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось выполнить блокировку регистра ""Замещение планов"" фоновым заданием. Количество попыток: %КоличествоОшибок%';
								|en = 'Cannot lock the ""Plan substitution"" register with the background job. Number of attempts: %КоличествоОшибок%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОшибок%", ПараметрыЗадания.КоличествоОшибок);
		
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Фоновое замещение плана';
											|en = 'Background plan substitution'", КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ТекстСообщения);
...
Функция ВыполнитьПроведенияПлана(ПараметрыЗадания)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось выполнить замещение/отмену замещения плана %ЗамещенныйПлан% фоновым заданием по причине: %Причина%. Количество попыток: %КоличествоОшибок%';
								|en = 'Cannot perform/cancel substitution for the %ЗамещенныйПлан% plan with the background job. Reason: %Причина%. Number of attempts: %КоличествоОшибок%'");
...
Процедура ОбновитьГиперссылкуЗамещение(Форма) Экспорт
	...
	Если ТЗЗамещенныеПланы.Количество() > 0 Тогда
		МассивСтроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Замещенные (';
																|en = 'Substituted ('") + ТЗЗамещенныеПланы.Количество() + ")",,ЦветаСтиля.ЦветТекстаУспех,,"Замещенные"));
	КонецЕсли;
	Если ТЗЗамещающиеПланы.Количество() > 0 Тогда
		Если МассивСтроки.Количество()>0 Тогда
			МассивСтроки.Добавить(", ");
		КонецЕсли;
		МассивСтроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Замещающие (';
																|en = 'Substitute ('") + ТЗЗамещающиеПланы.Количество() + ")",,ЦветаСтиля.ЦветТекстаПроблема,,"Замещающие"));
	КонецЕсли;
	Если МассивСтроки.Количество()= 0 Тогда
		МассивСтроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Без замещения';
																|en = 'Without substitution'"),,ЦветаСтиля.ГиперссылкаЦвет));
	КонецЕсли;
...
Процедура ОтменитьПроведениеПланов(МассивСсылок, ИмяДокумента) Экспорт
	...
	ШаблонОшибки = НСтр("ru = 'Не удалось выполнить отмену проведения документа %Документ%. %ОписаниеОшибки%';
						|en = 'Cannot cancel the %Документ% document posting. %ОписаниеОшибки%'");
...
Функция ПроверитьПравилоЗаполнения(Знач ПравилоЗаполнения, Знач АдресСхемыПреобразованияДанных, Знач ВидПлана = Неопределено, Знач ИмяРеквизитаПравила = "", ОтборПоРесурсам) Экспорт 
	...
	Если НЕ СоответствиеПолямДокумента ИЛИ НЕ СоответствиеПолямИсточников ИЛИ НЕ НаличиеОбязательныхПолей ИЛИ НЕ НаличиеОбязательногоПоляПериод Тогда
		ПравилоКорректно = Ложь;
	КонецЕсли;
	...
	ТекстОшибки = НСтр("ru = 'Состав полей, возвращаемых источниками заполнения количества, не совпадает с данными документа. Отредактируйте ""Группировки"" и ""Выбранные поля"", исключив ненужные поля %1.';
						|en = 'Fields returned by the quantity filling sources are different from the document data. Edit ""Groups"" and ""Selected fields"" by removing unnecessary fields%1.'");
	Если НЕ СоответствиеПолямДокумента Тогда
		...
		Если НомераСтрокПоляДокумента.Количество() > 1 Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, НСтр("ru = 'в строках:';
																									|en = 'in lines:'") + " " + СтрСоединить(НомераСтрокПоляДокумента, ", "));
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, НСтр("ru = 'в строке';
																									|en = 'in line'") + " " + СтрСоединить(НомераСтрокПоляДокумента, ", "));
		КонецЕсли;
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки,,ИмяРеквизитаПравила);
	...
	ТекстОшибки = НСтр("ru = 'В списке полей источника отсутствует обязательное поле ""%1"". Отредактируйте группировки и выбранные поля %2.';
						|en = 'The source field list does not contain the required ""%1"" field. Edit groupings and selected fields %2.'");
	
	Если НЕ НаличиеОбязательныхПолей Тогда
		Если НомераСтрокОбязательныеПоля.Количество() > 1 Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ОбязательныеПоля[0], НСтр("ru = 'в строках:';
																														|en = 'in lines:'") + " " + СтрСоединить(НомераСтрокОбязательныеПоля, ", "));
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ОбязательныеПоля[0], НСтр("ru = 'в строке';
																														|en = 'in line'") + " " + СтрСоединить(НомераСтрокОбязательныеПоля, ", "));
		КонецЕсли;
	...
	ТекстОшибки = НСтр("ru = 'При использовании сезонных коэффициентов необходимо в списке полей источника указать поле ""Период"". Отредактируйте группировки и выбранные поля %1.';
						|en = 'When using seasonal indexes, specify the Period field in the list of source fields. Edit groupings and selected fields %1.'");
	
	Если НЕ НаличиеОбязательногоПоляПериод Тогда
		Если НомераСтрокОбязательноеПолеПериод.Количество() > 1 Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, НСтр("ru = 'в строках:';
																									|en = 'in lines:'") + " " + СтрСоединить(НомераСтрокОбязательноеПолеПериод, ", "));
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, НСтр("ru = 'в строке';
																									|en = 'in line'") + " " + СтрСоединить(НомераСтрокОбязательноеПолеПериод, ", "));
		КонецЕсли;
	...
	ТекстОшибки = НСтр("ru = 'Состав полей, возвращаемых источниками заполнения цены, не совпадает с данными документа. Отредактируйте ""Группировки"" и ""Выбранные поля"", исключив ненужные поля %1.';
						|en = 'Fields returned by the price population sources are different from the document data. Edit ""Groups"" and ""Selected fields"" by removing unnecessary fields%1.'");
	Если НЕ СоответствиеПолямДокумента Тогда
	...
	ТекстОшибки = НСтр("ru = 'В составе полей, возвращаемых источниками заполнения цены, отсутствует поле ""Цена"" %1. Отредактируйте ""Выбранные поля"" или используйте другой источник';
						|en = 'Fields returned by the price population sources do not contain the ""Price"" field %1. Edit ""Selected fields"" or use another source'");
	Если НЕ ЕстьПолеЦены Тогда
		
		Если НомераСтрокБезПолеЦены.Количество() > 1 Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, НСтр("ru = 'в строках:';
																									|en = 'in lines:'") + " " + СтрСоединить(НомераСтрокБезПолеЦены, ", "));
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, НСтр("ru = 'в строке';
																									|en = 'in line'") + " " + СтрСоединить(НомераСтрокБезПолеЦены, ", "));
		КонецЕсли;
...
Процедура СохранитьНовыйПериод(Форма) Экспорт
	...
	Если Не Объект.Периодичность.Пустая() Тогда
		Форма.НадписьПериодичностьВалюта = Форма.НадписьПериодичностьВалюта +НСтр("ru = 'Периодичность';
																					|en = 'Frequency'") + ": " + Строка(Объект.Периодичность);
	КонецЕсли;
	Если Объект.ПланироватьПоСумме Тогда
		Форма.НадписьПериодичностьВалюта = Форма.НадписьПериодичностьВалюта + ", " + НСтр("ru = 'Валюта';
																							|en = 'Currency'") + ": " + Строка(Объект.Валюта);
	КонецЕсли; 
...
Процедура СбалансированностьПлановОбработкаПроверкиЗаполнения(КомпоновщикНастроек, Отказ) Экспорт 
		...
		Если ПериодыПлановОстаткаЗаПериодБольшеНачалоПериода <> Неопределено Тогда
			Если ПериодыПлановОстаткаЗаПериодБольшеНачалоПериода.Количество() > 0 Тогда
				...
				ТекстСообщения = НСтр("ru = 'В пределах периода отчета найдены планы остатков в следующих периодах: ""%СтрокаПериодыПланаОстатков%"". Сбалансированность можно отобразить, если планы остатков не разрывают период отчета.';
										|en = 'Goods stock plans are found within the report period limits in the following periods: ""%СтрокаПериодыПланаОстатков%"". Balance can be displayed if the plans of goods stock do not break the report plan.'");
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнено обязательное поле ""Сценарий"".';
																|en = 'Required ""Scenario"" is not filled in.'"), , , , Отказ);
	КонецЕсли;
...
Функция ПолучитьПользовательскиеНастройкиСОтборомПоДокументам(ОтборПоДокументам)
	...
	|	
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		ДвиженияПланаТовары.Период,
	|		ДвиженияПланаТовары.Период,
	|		ДвиженияПланаТовары.Сценарий
	|	ИЗ
	|		РегистрНакопления.ПланыВыпускаИзделий.Обороты(,,Запись,ПланПроизводства В (&МассивСсылок)) КАК ДвиженияПланаТовары
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		ДвиженияПланаТовары.Сценарий
	|	ИЗ
	|		РегистрНакопления.ПланыПотребленияМатериалов.Обороты(,,Запись,ПланПроизводства В (&МассивСсылок)) КАК ДвиженияПланаТовары
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	//-- НЕ УТ
	|	
	...
	|	
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		ДвиженияПланаТовары.Номенклатура
	|	ИЗ
	|		РегистрНакопления.ПланыВыпускаИзделий.Обороты(,,Запись,ПланПроизводства В (&МассивСсылок)) КАК ДвиженияПланаТовары
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
...
Процедура ПроверитьСтатусУтвержден(Объект, Отказ, РежимЗаписи, ТипПлана) Экспорт 
	...
	ИначеЕсли ТипПлана = Перечисления.ТипыПланов.ПланВнутреннихПотреблений И Пользователи.РолиДоступны("УтверждениеПлановВнутреннихПотреблений") Тогда
		Возврат;
		//++ НЕ УТ
	ИначеЕсли ТипПлана = Перечисления.ТипыПланов.ПланПроизводства И Пользователи.РолиДоступны("УтверждениеПлановПроизводства") Тогда
		Возврат;
		//-- НЕ УТ
		...
		Если Объект.Статус = Перечисления.СтатусыПланов.Утвержден 
			...
			Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
				ТекстСообщения = НСтр("ru = 'Недостаточно прав для проведения документа в статусе ""Утвержден"".';
										|en = 'Insufficient rights to post the document with the Confirmed status.'")
			Иначе
				ТекстСообщения = НСтр("ru = 'Недостаточно прав для отмены проведения документа в статусе ""Утвержден"".';
										|en = 'Insufficient rights to cancel posting the document with the Confirmed status.'")
			КонецЕсли;
...
Функция ПроверитьСуществованиеУжеСозданныхПланов(ДанныеФормы, ИмяТипаДокумента) Экспорт
	...
	//++ НЕ УТ
	ИначеЕсли ИмяТипаДокумента = "ПланПроизводства" Тогда
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В базе уже создан документ за текущий период: %1 в статусе %2, который необходимо утвердить.';
																					|en = 'Document for the current period is already created in the infobase: %1 in the %2 status which should be approved.'"), 
...
#Область Отчет_ПлановаяПотребностьВВидахРЦ

// Функция возвращает самую раннюю и позднюю дату планов занятости видов рабочих центров
//  у планов производства по заданному сценарию товарного планирования.
//
// Параметры:
//  Параметры - Структура - содержит:
//   Сценарий - СправочникСсылка.СценарииТоварногоПланирования - заданный сценарий отчета.
//   СтатусыСценария - ПеречислениеСсылка.СтатусыПланов - статусы планов производства.
//   ПланПроизводства - ДокументСсылка.ПланПроизводства - перечень планов производства, соответствующих условиям.
//   ОтборПоСтатусуИспользуется - Булево - проверка на использование отбора по статусам планов.
//   ОтборПоПлануИспользуется - Булево - проверка на использование отбора по плану производства.
//
// Возвращаемое значение:
//  Структура - даты крайних периодов планов занятости видов рабочих центров
//   ПериодДатаНачала - Дата - дата первого периода планов занятости видов РЦ.
//   ПериодДатаКонца - Дата - дата последнего периода планов занятости видов РЦ.
//
Функция ПолучитьКрайниеПериодыПлановЗанятостиВидовРЦ(Параметры) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	МИНИМУМ(ПланыЗанятостиВидовРабочихЦентровОбороты.Период) КАК ПериодДатаНачала,
	|	МАКСИМУМ(ПланыЗанятостиВидовРабочихЦентровОбороты.Период) КАК ПериодДатаКонца
	|ИЗ
	...
	|			Сценарий = &Сценарий
	|				И (Статус В (&СтатусыСценария)
	|					ИЛИ НЕ &ОтборПоСтатусуИспользуется)
	|				И (ПланПроизводства = &ПланПроизводства
	|					ИЛИ НЕ &ОтборПоПлануИспользуется)) КАК ПланыЗанятостиВидовРабочихЦентровОбороты";
	
	Запрос.УстановитьПараметр("Сценарий", Параметры.Сценарий);
	Запрос.УстановитьПараметр("СтатусыСценария", Параметры.СтатусыСценария);
	Запрос.УстановитьПараметр("ОтборПоСтатусуИспользуется", Параметры.ОтборПоСтатусуИспользуется <> Ложь);
	Запрос.УстановитьПараметр("ПланПроизводства", Параметры.ПланПроизводства);
	Запрос.УстановитьПараметр("ОтборПоПлануИспользуется", Параметры.ОтборПоПлануИспользуется <> Ложь);
	
	ВыборкаДатНачалаИКонцаПериода = Запрос.Выполнить().Выбрать();
	
	Если ВыборкаДатНачалаИКонцаПериода.Следующий() Тогда
		ПериодДатаНачала = ВыборкаДатНачалаИКонцаПериода.ПериодДатаНачала;
		ПериодДатаКонца = ВыборкаДатНачалаИКонцаПериода.ПериодДатаКонца;
	КонецЕсли;
	
	Если ТипЗнч(ПериодДатаНачала) <> Тип("Дата") И ТипЗнч(ПериодДатаКонца) <> Тип("Дата") Тогда
		ПериодДатаНачала = '00010101';
		ПериодДатаКонца = '00010101';
	КонецЕсли;
	
	Результат = Новый Структура;
	Результат.Вставить("ПериодДатаНачала", ПериодДатаНачала);
	Результат.Вставить("ПериодДатаКонца", ПериодДатаКонца);
...
// 	ДокументРезультат - ТабличныйДокумент - 
// 	СтандартнаяОбработка - Булево -
Процедура ПлановаяПотребностьВВидахРЦПриКомпоновкеРезультата(КомпоновщикНастроек, ДокументРезультат, СтандартнаяОбработка) Экспорт
	
	ОтборСтатус                = ПолучитьОтбор(КомпоновщикНастроек, "Статус", "");
	ОтборПланПроизводства      = ПолучитьОтбор(КомпоновщикНастроек, "ПланПроизводства", "");
	СтатусыСценария            = Неопределено;
	ПланПроизводства           = Неопределено;
	ОтборПоСтатусуИспользуется = Ложь;
	ОтборПоПлануИспользуется   = Ложь;
	
	Для каждого ЭлементНастройки Из КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
		Если ОтборСтатус <> Неопределено И ЭлементНастройки.ИдентификаторПользовательскойНастройки = ОтборСтатус.ИдентификаторПользовательскойНастройки Тогда
			Если ЭлементНастройки.Использование Тогда
				СтатусыСценария = Новый Массив;
				Если ЭлементНастройки.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке
					ИЛИ ЭлементНастройки.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии Тогда
					Для Каждого Элемент Из ЭлементНастройки.ПравоеЗначение Цикл
						СтатусыСценария.Добавить(Элемент.Значение);
					КонецЦикла;
				ИначеЕсли ЭлементНастройки.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Тогда
					СтатусыСценария.Добавить(ЭлементНастройки.ПравоеЗначение);
				КонецЕсли;
				
				Если ЗначениеЗаполнено(СтатусыСценария) Тогда
					ОтборПоСтатусуИспользуется = Истина;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		Если ОтборПланПроизводства <> Неопределено И ЭлементНастройки.ИдентификаторПользовательскойНастройки = ОтборПланПроизводства.ИдентификаторПользовательскойНастройки Тогда
			Если ЭлементНастройки.Использование Тогда
				Если ЭлементНастройки.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Тогда
					ОтборПоПлануИспользуется = Истина;
					ПланПроизводства = ЭлементНастройки.ПравоеЗначение;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Если Период <> Неопределено
	   И ТипЗнч(Период) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных")
	   И Период.Использование Тогда
		
		ПериодЗначение = Период.Значение; // СтандартныйПериод - 
		НастроитьДополнениеПериода(КомпоновщикНастроек.Настройки, Сценарий.Значение.Периодичность, ПериодЗначение.ДатаНачала, ПериодЗначение.ДатаОкончания);
		
	Иначе
		
		ПараметрыРасчетаКрайнихПериодовПлановЗанятостиВидовРЦ = Новый Структура();
		ПараметрыРасчетаКрайнихПериодовПлановЗанятостиВидовРЦ.Вставить("Сценарий", Сценарий.Значение);
		ПараметрыРасчетаКрайнихПериодовПлановЗанятостиВидовРЦ.Вставить("СтатусыСценария", СтатусыСценария);
		ПараметрыРасчетаКрайнихПериодовПлановЗанятостиВидовРЦ.Вставить("ОтборПоСтатусуИспользуется", ОтборПоСтатусуИспользуется);
		ПараметрыРасчетаКрайнихПериодовПлановЗанятостиВидовРЦ.Вставить("ПланПроизводства", ПланПроизводства);
		ПараметрыРасчетаКрайнихПериодовПлановЗанятостиВидовРЦ.Вставить("ОтборПоПлануИспользуется", ОтборПоПлануИспользуется);
		
		Результат = ПолучитьКрайниеПериодыПлановЗанятостиВидовРЦ(ПараметрыРасчетаКрайнихПериодовПлановЗанятостиВидовРЦ);
		
		НачальныйПериод = Результат.ПериодДатаНачала;
		КонечныйПериод = Результат.ПериодДатаКонца;
		
		НастроитьДополнениеПериода(КомпоновщикНастроек.Настройки, Сценарий.Значение.Периодичность, НачальныйПериод, КонечныйПериод);
...
Процедура ПереопределитьНастройкиРасшифровкиОтчета(СхемаКомпоновкиДанных, КомпоновщикНастроек, ДанныеРасшифровки) Экспорт
	...
	//++ НЕ УТКА
	ПолеПодразделениеДиспетчер = Новый ПолеКомпоновкиДанных("ПодразделениеДиспетчер");
	//-- НЕ УТКА
	...
	Для Каждого ЭлементОтбор Из ОтборыОтчета.Элементы Цикл
		...
		Если ИдентификаторВариантаОтчета = "ПоПодразделениям" Тогда
			...
			КонецЕсли;
		//++ НЕ УТКА
		ИначеЕсли ИдентификаторВариантаОтчета = "ПоПодразделениямДиспетчерам" Тогда
			Если ТипЗнч(ЭлементОтбор) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
				Если ЭлементОтбор.ЛевоеЗначение = ПолеПодразделениеДиспетчер Тогда
					ЭлементОтбораДетализация = ЭлементОтбор; // ЭлементОтбораКомпоновкиДанных - 
					ПолеДетализации = ПолеПодразделениеДиспетчер;
				КонецЕсли;
			КонецЕсли;
		//-- НЕ УТКА
...
Процедура ПроверитьЗаполнениеСклада(Объект, Отказ, ПараметрыПроверки)
	...
	ШаблонСообщения = НСтр("ru = 'Не заполнена колонка ""Склад"" в строке %1 списка ""%2""';
							|en = 'Column ""Warehouse"" in line %1 of list ""%2"" is not filled in'");
...
Процедура ПолучитьДлительностьСпецификацийИЗаполнитьДатыПоГрафику(КоллекцияСтрок, ДатаОтсчетаИмя, ДатаПоГрафикуИмя, СценарийПланирования, ИскатьВперед, СпособРасчетаМатериалов) Экспорт
	
	СписокСпецификаций = Новый Массив();
	ДатыОтсчета = Новый ТаблицаЗначений;
	ДатыОтсчета.Колонки.Добавить("Спецификация", Новый ОписаниеТипов("СправочникСсылка.РесурсныеСпецификации"));
	ДатыОтсчета.Колонки.Добавить(ДатаОтсчетаИмя, Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты));
	ДатыОтсчета.Колонки.Добавить("ЧислоДней", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла));
	ДатыОтсчета.Колонки.Добавить("Индекс", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла));
	
	Для Индекс = 0 По КоллекцияСтрок.Количество()-1 Цикл
		
		Строка = КоллекцияСтрок[Индекс];
		
		Если ЗначениеЗаполнено(Строка.Спецификация)
			И ЗначениеЗаполнено(Строка[ДатаОтсчетаИмя]) Тогда
			
			СписокСпецификаций.Добавить(Строка.Спецификация);
			НоваяСтрока = ДатыОтсчета.Добавить();
			НоваяСтрока.Индекс = Индекс;
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка, "Спецификация," + ДатаОтсчетаИмя);
			
		КонецЕсли;
		
	КонецЦикла;
	
	Если ДатыОтсчета.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	НормативнаяДлительность = РегистрыСведений.НормативнаяДлительностьПроизводства.ПолучитьДлительность(СписокСпецификаций);
	ПолеДлительность = ?(СпособРасчетаМатериалов = Перечисления.СпособыРасчетаМатериалов.МинимальноеПотребление,
		"ПлановаяДлительность",
		"МаксимальнаяДлительность");
	Для каждого Строка Из ДатыОтсчета Цикл
		Строка.ЧислоДней = НормативнаяДлительность.Получить(Строка.Спецификация)[ПолеДлительность];
	КонецЦикла;
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДатыОтсчета." + ДатаОтсчетаИмя + " КАК ДатаОтсчета,
		|	ДатыОтсчета.ЧислоДней КАК ЧислоДней
		|ПОМЕСТИТЬ ВтДатыОтсчета
		|ИЗ
		|	&ДатыОтсчета КАК ДатыОтсчета");
	
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ДатыОтсчета", ДатыОтсчета);
	Запрос.Выполнить();
	
	ДатыЗапуска = ПолучитьДатыПоГрафику(Запрос.МенеджерВременныхТаблиц, СценарийПланирования, ИскатьВперед);
	ДатыЗапуска.Колонки.ДатаОтсчета.Имя = ДатаОтсчетаИмя;
	
	ДатыОтсчета.Индексы.Добавить("ЧислоДней," + ДатаОтсчетаИмя);
	СтруктураПоиска = Новый Структура("ЧислоДней," + ДатаОтсчетаИмя);
	
	Для каждого СтрокаЗапуск Из ДатыЗапуска Цикл
		
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаЗапуск);
		НайденныеСтроки = ДатыОтсчета.НайтиСтроки(СтруктураПоиска);
		
		Для каждого СтрокаВыпуск Из НайденныеСтроки Цикл
			
			СтрокаКоллекции = КоллекцияСтрок[СтрокаВыпуск.Индекс];
			СтрокаКоллекции[ДатаПоГрафикуИмя] = СтрокаЗапуск.ДатаПоГрафику;
...
// Рассчитывает даты запуска, вычитая заданное количество дней от переданных дат выпуска по календарю
// работы сценария (либо основному календарю). Если рассчитать дату по календарю не удалось,
// то результат будет определен путем простого вычитания.
//
// Параметры:
//  Источник	 - ТаблицаЗначений, МенеджерВременныхТаблиц - таблица или менеджер с таблицей ВтДатыОтсчета, колонки:
//			* ДатаОтсчета - Дата - дата, от которой необходимо выполнить расчет.
//			* ЧислоДней - Число - количество дней, которое необходимо отложить влево от даты выпуска.
//  Сценарий - СправочникСсылка.СценарииТоварногоПланирования - сценарий, календарь которого
//		используется для расчета дат запуска.
//  ИскатьВперед - Булево               - если истина, то поиск вперед, иначе поиск назад.
// 
// Возвращаемое значение:
//  ТаблицаЗначений - состав:
//    * ДатаОтсчета - Дата -
//    * ЧислоДней - Число -
//    * ДатаПоГрафику - Дата -
//
Функция ПолучитьДатыПоГрафику(Источник, Сценарий, ИскатьВперед) Экспорт
	
	Календарь = Справочники.СценарииТоварногоПланирования.Календарь(Сценарий);
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	Таблица.ДатаОтсчета КАК ДатаОтсчета,
		|	Таблица.ЧислоДней   КАК ЧислоДней
		...
		|////////////////////////////////////////////////////////////////////////////////
		|"
		+ ПроизводствоСервер.ТекстЗапросаДатПоКалендарю(ИскатьВперед)
		+ "
		|ВЫБРАТЬ
		|	Таблица.ДатаОтсчета КАК ДатаОтсчета,
		|	Таблица.ЧислоДней   КАК ЧислоДней,
		|	ЕСТЬNULL(ДатыГрафика.Дата, ДОБАВИТЬКДАТЕ(Таблица.ДатаОтсчета, ДЕНЬ, Таблица.ЧислоДней * &Направление)) КАК ДатаПоГрафику
		|ИЗ
		|	ВтПараметрыПоиска КАК Таблица
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтДатыГрафика КАК ДатыГрафика
		|		ПО Таблица.ДатаОтсчета = ДатыГрафика.ДатаОтсчета
		|			И Таблица.ЧислоДней = ДатыГрафика.ЧислоДней
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("Направление", ?(ИскатьВперед, 1, -1));
	
	Если ТипЗнч(Источник) = Тип("МенеджерВременныхТаблиц") Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Таблица", "ВтДатыОтсчета");
		Запрос.МенеджерВременныхТаблиц = Источник;
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Таблица", "&ДатыОтсчета");
		Запрос.УстановитьПараметр("ДатыОтсчета", Источник);
	КонецЕсли;
...
Функция РассчитатьДатуЗапуска(Сценарий, ДатаВыпуска, Спецификация, СпособРасчетаМатериалов) Экспорт
	...
	ПолучитьДлительностьСпецификацийИЗаполнитьДатыПоГрафику(
		Таблица,
		"ДатаВыпуска",
		"ДатаЗапуска",
		Сценарий,
...
Процедура ЗапускВыполненияФоновойПроверкиРасчетаДефицитаПоЭтапам(Сценарий, ВидПлана) Экспорт
	...
	НаименованиеЗадания = НСтр("ru = 'Проверка расчета дефицитов планов по этапам для вида плана';
								|en = 'Verification of plan shortages calculation by stages for plan profile'") + Строка(ВидПлана);
...
Процедура ВыполнитьФоновуюПроверкуРасчетаДефицитовПоЭтапам(ПараметрыЗадания) Экспорт
	...
	Если ПараметрыЗадания.Свойство("КоличествоОшибок") Тогда
		...
		Если ПараметрыЗадания.КоличествоОшибок >= МаксимальноеКоличествоОшибок Тогда
			...
			ТекстСообщения = НСтр("ru = 'Фоновая проверка расчета дефицитов планов по этапам остановлена. Количество неудачных попыток: %КоличествоОшибок%';
									|en = 'Background verification of plan shortages calculation by stages was stopped. Number of unsuccessful attempts: %КоличествоОшибок%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОшибок%", ПараметрыЗадания.КоличествоОшибок);
			
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Фоновая проверка расчета дефицитов по этапам';
											|en = 'Background verification of shortages calculation by stages'", КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось выполнить блокировку регистра ""Очередь расчета дефицитов по этапам"" фоновым заданием. Количество попыток: %КоличествоОшибок%';
								|en = 'Failed to lock the register ""Queue for calculation of shortages by plans"" with background job. Number of attempts: %КоличествоОшибок%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОшибок%", ПараметрыЗадания.КоличествоОшибок);
...
Функция ТребуетсяПересчитатьДефициты(Сценарий, ВидПлана) Экспорт
	...
	|
	|ВЫБРАТЬ
	|	ПланыВыпускаИзделийОбороты.Номенклатура,
	|	ПланыВыпускаИзделийОбороты.Характеристика,
	|	ПланыВыпускаИзделийОбороты.Назначение,
	|	ПланыВыпускаИзделийОбороты.Сценарий,
	|	ПланыВыпускаИзделийОбороты.ВидПлана,
	|	ПланыВыпускаИзделийОбороты.Период,
	|	0,
	|	ПланыВыпускаИзделийОбороты.КоличествоОборот,
	|	0,
	|	0,
	|	0,
	|	0,
	...
	|			,
	|			МЕСЯЦ,
	|			&ТипПлана = ЗНАЧЕНИЕ(Перечисление.ТипыПланов.ПланПроизводства)
	|				И Сценарий = &Сценарий
	|				И ВидПлана = &ВидПлана
	|				И Статус В (&СтатусыСценария)) КАК ПланыВыпускаИзделийОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ПланыПроизводстваОбороты.ВидПлана,
	|	ПланыПроизводстваОбороты.Период,
	|	0,
	|	0,
	|	ПланыПроизводстваОбороты.КоличествоОборот,
	|	0,
	|	0,
	|	ПланыПроизводстваОбороты.ЗаданИнтервалПотребностиОборот,
	|	ПланыПроизводстваОбороты.ИнтервалПотребностиОтОборот,
	|	ПланыПроизводстваОбороты.ИнтервалПотребностиДоОборот,
	|	ЛОЖЬ
	|ИЗ
	...
	|				И Сценарий = &Сценарий
	|				И ВидПлана = &ВидПлана
	|				И Статус В (&СтатусыСценария)) КАК ПланыПроизводстваОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ПланыПотребленияМатериаловОбороты.Номенклатура,
	|	ПланыПотребленияМатериаловОбороты.Характеристика,
	|	ПланыПотребленияМатериаловОбороты.Назначение,
	|	ПланыПотребленияМатериаловОбороты.Сценарий,
	|	ПланыПотребленияМатериаловОбороты.ВидПлана,
	|	ПланыПотребленияМатериаловОбороты.Период,
	|	0,
	|	0,
	|	0,
	|	ПланыПотребленияМатериаловОбороты.КоличествоОборот,
	|	0,
	|	0,
	|	0,
	|	0,
	...
	|				И Сценарий = &Сценарий
	|				И ВидПлана = &ВидПлана
	|				И Статус В (&СтатусыСценария)) КАК ПланыПотребленияМатериаловОбороты
	//-- НЕ УТ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Процедура ЗапускВыполненияФоновогоРасчетаДефицитаПоЭтапам(Сценарий) Экспорт
	...
	НаименованиеЗадания = НСтр("ru = 'Расчет дефицитов планов по этапам';
								|en = 'Calculation of plan shortages by stages'");
...
Процедура ВыполнитьФоновыйРасчетДефицитовПоЭтапам(ПараметрыЗадания) Экспорт
	...
	Если ПараметрыЗадания.Свойство("КоличествоОшибок") Тогда
		...
		Если ПараметрыЗадания.КоличествоОшибок >= МаксимальноеКоличествоОшибок Тогда
			...
			ТекстСообщения = НСтр("ru = 'Фоновый расчет дефицитов планов по этапам остановлен. Количество неудачных попыток: %КоличествоОшибок%';
									|en = 'Background calculation of plan shortages by stages was stopped. Number of unsuccessful attempts: %КоличествоОшибок%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОшибок%", ПараметрыЗадания.КоличествоОшибок);
			
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Фоновый расчет дефицитов планов по этапам';
											|en = 'Background calculation of plan shortages by stages'", КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось выполнить блокировку регистра ""Очередь расчета дефицитов по планам"" фоновым заданием. Количество попыток: %КоличествоОшибок%';
								|en = 'Failed to lock registers ""Queue for calculation of shortages by plans"" with background job. Number of attempts: %КоличествоОшибок%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОшибок%", ПараметрыЗадания.КоличествоОшибок);
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru = 'Не удалось выполнить формирование планов по дефициту по причине %Причина%';
										|en = 'Cannot form plans by shortage. Reason: %Причина%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Формирование планов по дефициту';
												|en = 'Form plans by shortage'", КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ТекстСообщения);
...
Функция ВыполнитьРасчетДефицитовПоЭтапам(Сценарий, Период) Экспорт
	...
	|	РесурсныеСпецификацииВыходныеИзделия.Номенклатура.ИспользованиеХарактеристик КАК ИспользованиеХарактеристик,
	|	РесурсныеСпецификацииВыходныеИзделия.ВидНоменклатуры КАК ВидНоменклатуры,
	|	РесурсныеСпецификацииВыходныеИзделия.Номенклатура.ВладелецХарактеристик КАК ВладелецХарактеристик
	|ПОМЕСТИТЬ РесурсныеСпецификации
	|ИЗ
	...
	|
	|ВЫБРАТЬ
	|	РесурсныеСпецификации.ВидНоменклатуры КАК ВидНоменклатуры,
	|	РесурсныеСпецификации.Номенклатура КАК Номенклатура,
	|	ХарактеристикиНоменклатуры.Ссылка КАК Характеристика,
	|	РесурсныеСпецификации.ТипТовара КАК ТипТовара
	|ИЗ
	|	РесурсныеСпецификации КАК РесурсныеСпецификации
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК ХарактеристикиНоменклатуры
	|		ПО (ВЫБОР
	|				КОГДА РесурсныеСпецификации.ИспользованиеХарактеристик = ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ИндивидуальныеДляНоменклатуры)
	|					ТОГДА РесурсныеСпецификации.Номенклатура
	|				КОГДА РесурсныеСпецификации.ИспользованиеХарактеристик = ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеДляВидаНоменклатуры)
	|					ТОГДА РесурсныеСпецификации.ВидНоменклатуры
	|				КОГДА РесурсныеСпецификации.ИспользованиеХарактеристик = ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеСДругимВидомНоменклатуры)
	|					ТОГДА РесурсныеСпецификации.ВладелецХарактеристик
	|				ИНАЧЕ НЕОПРЕДЕЛЕНО
	|			КОНЕЦ = ХарактеристикиНоменклатуры.Владелец)
	|ГДЕ
	|	РесурсныеСпецификации.Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
	|	И (РесурсныеСпецификации.ИспользованиеХарактеристик <> ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.НеИспользовать)
	|		ИЛИ РесурсныеСпецификации.ИспользованиеХарактеристик ЕСТЬ NULL)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	РесурсныеСпецификации.ВидНоменклатуры КАК ВидНоменклатуры,
	|	РесурсныеСпецификации.Номенклатура,
	|	РесурсныеСпецификации.Характеристика,
	|	РесурсныеСпецификации.ТипТовара
	|ИЗ
	|	РесурсныеСпецификации КАК РесурсныеСпецификации
	|ГДЕ
	|	РесурсныеСпецификации.Характеристика <> ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
	|	ИЛИ РесурсныеСпецификации.ИспользованиеХарактеристик = ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.НеИспользовать)
	//-- НЕ УТКА
	|;
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|						Т.Назначение КАК Назначение
	|					ИЗ
	|						ОчередьРасчетаДефицитовПоЭтапам КАК Т)) КАК ПланыВыпускаИзделийОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ПланыПроизводстваИзделийОбороты.Период,
	|	ПланыПроизводстваИзделийОбороты.Сценарий,
	|	ПланыПроизводстваИзделийОбороты.Номенклатура,
	|	ПланыПроизводстваИзделийОбороты.Характеристика,
	|	ПланыПроизводстваИзделийОбороты.Назначение,
	|	ПланыПроизводстваИзделийОбороты.ВидПлана,
	|	0,
	|	0,
	|	0,
	|	0,
	|	ПланыПроизводстваИзделийОбороты.ЗаданИнтервалПотребностиОборот,
	|	ПланыПроизводстваИзделийОбороты.ИнтервалПотребностиОтОборот,
	|	ПланыПроизводстваИзделийОбороты.ИнтервалПотребностиДоОборот,
	|	ПланыПроизводстваИзделийОбороты.КоличествоОборот
	|ИЗ
	|	РегистрНакопления.ПланыПроизводства.Обороты(
	|			&НачалоПериода,
	|			&КонецПериода,
	...
	|						Т.Назначение КАК Назначение
	|					ИЗ
	|						ОчередьРасчетаДефицитовПоЭтапам КАК Т)) КАК ПланыПроизводстваИзделийОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|						Т.Назначение КАК Назначение
	|					ИЗ
	|						ОчередьРасчетаДефицитовПоЭтапам КАК Т)) КАК ПланыПотребленияМатериаловОбороты
	|
	//-- НЕ УТ
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Процедура ПроверкаЗавершенияРасчетаПлановПроизводстваИЗамещенияПланов(Сценарий)
	...
	Если Запрос.Выполнить().Выбрать().Следующий() Тогда
		Ключ = "ОчередьРасчетаПланаПроизводства.ОбработкаОчереди";
...
Процедура ЗаполнитьПланыПоДефициту(Сценарий, ВидПлана, НачалоПериода, ОкончаниеПериода, ПланОбъект = Неопределено) Экспорт
	...
	Попытка
		...
		//++ НЕ УТ
		"ОтражаетсяВБюджетировании, СтатьяБюджетов, ОтражаетсяВБюджетированииОплаты,
		|СтатьяБюджетовОплат, ОтражаетсяВБюджетированииОплатыКредит, СтатьяБюджетовОплатКредит,
		|ПланироватьПолуфабрикатыАвтоматически, ТипПроизводственногоПроцесса, ЗаполнятьСпецификациюПоПериодам,"+
		//-- НЕ УТ
		"Родитель";
		...
		Если ОбщегоНазначения.ИнформационнаяБазаФайловая()
			И ПараметрыВидаПлана.ТипПлана = Перечисления.ТипыПланов.ПланПроизводства Тогда
		...
		Если ПланОбъект = Неопределено Тогда
			...
			|	Документ.ПланПроизводства КАК ПланПроизводства
			|ГДЕ
			|	ПланПроизводства.НачалоПериода <= &ОкончаниеПериода
			|	И ПланПроизводства.ОкончаниеПериода >= &НачалоПериода
		...
		Если ПланОбъект = Неопределено Тогда
			...
			ИначеЕсли ТипПлана = Перечисления.ТипыПланов.ПланПроизводства Тогда
				ПланОбъект = Документы.ПланПроизводства.СоздатьДокумент();
			//-- НЕ УТ
		...
		//++ НЕ УТ
		ИначеЕсли ПараметрыВидаПлана.ТипПлана = Перечисления.ТипыПланов.ПланПроизводства Тогда
			ТаблицаТовары = ПланОбъект.Продукция;
			ИмяКолонкиПериода = "ДатаВыпуска";
		...
		//++ НЕ УТ
		Если ПараметрыВидаПлана.ТипПлана = Перечисления.ТипыПланов.ПланПроизводства Тогда
			
			ПараметрыВыбораСпецификаций = УправлениеДаннымиОбИзделиях.ПараметрыВыбораСпецификаций(ПланОбъект, Документы.ПланПроизводства);
			
			МассивСтрок = Новый Массив;
			МассивНазначений = Новый Массив;
			Для Каждого СтрокаТовара Из ПланОбъект.Продукция Цикл
				МассивСтрок.Добавить(СтрокаТовара);
				МассивНазначений.Добавить(СтрокаТовара.Назначение);
			КонецЦикла;
			...
			ОписаниеТаблицы = Новый Структура("ТЧ, ИмяТЧ, ПутьКДанным");
			ОписаниеТаблицы.ПутьКДанным = "Объект.Продукция";
			ОписаниеТаблицы.ТЧ          = ПланОбъект.Продукция;
			ОписаниеТаблицы.ИмяТЧ       = "Продукция";
			...
			Для каждого Строка Из МассивСтрок Цикл
				...
				ДанныеОбИзделии = УправлениеДаннымиОбИзделияхКлиентСервер.СобратьДанныеОбИзделииДляВыбораСпецификации(
					ПланОбъект, Строка, ПараметрыВыбораСпецификаций,, Новый Структура("КлючПутейКДанным", ОписаниеТаблицы.ПутьКДанным));
			...
			ПолучитьДлительностьСпецификацийИЗаполнитьДатыПоГрафику(
				ПланОбъект.Продукция,
				"ДатаВыпуска",
				"ДатаЗапуска",
				ПланОбъект.Сценарий,
				Ложь,
				ПланОбъект.СпособРасчетаПотребностейВМатериалах);
...
Процедура РасчитатьИЗаполнитьПланыПоДефицитуАвтоматически(Сценарий) Экспорт
	...
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ЕСТЬNULL(ПланПроизводства.Ссылка, ЗНАЧЕНИЕ(Документ.ПланПроизводства.ПустаяСсылка)),
	|	ВидыПлановПоПериодамДляПересчета.ВидПлана,
	|	ВидыПлановПоПериодамДляПересчета.ПериодПланирования,
	|	ЕСТЬNULL(ПланПроизводства.ЗаполнятьАвтоматически, ВидыПлановПоПериодамДляПересчета.ЗаполнятьАвтоматически),
	...
	|		ПО ВидыПлановПоПериодамДляПересчета.ВидПлана = ПланПроизводства.ВидПлана
	|			И ВидыПлановПоПериодамДляПересчета.ПериодПланирования = ПланПроизводства.НачалоПериода
	|			И ПланПроизводства.Проведен
	|ГДЕ
	|	ВидыПлановПоПериодамДляПересчета.ТипПлана = ЗНАЧЕНИЕ(Перечисление.ТипыПланов.ПланПроизводства)
	|	И ЕСТЬNULL(ПланПроизводства.ЗаполнятьАвтоматически, ВидыПлановПоПериодамДляПересчета.ЗаполнятьАвтоматически)
	|
	//-- НЕ УТ
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Процедура ЗапускВыполненияФоновогоПересчетаДефицитаПоЭтапам(Сценарий) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось выполнить блокировку регистров ""Очередь расчета дефицитов по планам"" и ""Расчет дефицитов по этапам""';
								|en = 'Failed to lock registers ""Queue for calculation of shortages by plans"" and ""Calculation of shortages by stages""'");
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Запуск полного пересчета по дефицитам';
										|en = 'Run full shortages recalculation'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			ТекстСообщения);
...
Процедура СопоставитьЗагружаемыеДанные(АдресЗагружаемыхДанных, АдресТаблицыСопоставления, СписокНеоднозначностей, ПолноеИмяТабличнойЧасти, ДополнительныеПараметры) Экспорт
	...
	КонецЕсли;
	//++ НЕ УТ
	Если ДополнительныеПараметры.ИспользуютсяСпецификации Тогда
		ТекстыЗапроса.Добавить(ТекстЗапросаВтДанныеСпецификаций());
	КонецЕсли;
	//-- НЕ УТ
	...
	Если ДополнительныеПараметры.КроссТаблица Тогда
		...
		//++ НЕ УТ
		ИначеЕсли ДополнительныеПараметры.ТипПлана = "ПланПроизводства" Тогда
			ИмяПериодаПлана = "ДатаВыпуска";
		//-- НЕ УТ
...
Процедура СоздатьЗаписиОНеоднозначности(СписокНеоднозначностей, Строка, ДополнительныеПараметры)
		...
		//++ НЕ УТ
		Если ДополнительныеПараметры.ИспользуютсяСпецификации И Строка.КоличествоСпецификаций > 1 Тогда
			ЗаписьОНеоднозначности               = СписокНеоднозначностей.Добавить();
			ЗаписьОНеоднозначности.Идентификатор = Строка.Идентификатор;
			ЗаписьОНеоднозначности.Колонка       = "Спецификация";
		КонецЕсли;
		//-- НЕ УТ
...
Процедура ЗаполнитьСписокНеоднозначностей(СписокНеоднозначностей, ИмяКолонки, ЗагружаемыеЗначенияСтрока, ДополнительныеПараметры) Экспорт
	...
	//++ НЕ УТ
	Если ИмяКолонки = "Спецификация" Тогда
		...
		|ГДЕ
		|	НЕ &Спецификация = """"
		|	И Спецификации.Наименование = &Спецификация
		|";
		
		Запрос.УстановитьПараметр("Спецификация", ЗагружаемыеЗначенияСтрока.Спецификация);
...
Функция СоздатьНастроитьЗапрос(ТекстыЗапроса, ЗагружаемыеДанные, ДополнительныеПараметры)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗагружаемыеДанныеСпецификация,",
		?(ДополнительныеПараметры.ИспользуютсяСпецификации, "ЗагружаемыеДанные.Спецификация КАК Спецификация,", ""));
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Спецификация,", ?(ДополнительныеПараметры.ИспользуютсяСпецификации, "
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ДанныеСпецификаций.КоличествоСпецификаций, 0) = 1
		|			ТОГДА ДанныеСпецификаций.Спецификация
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.РесурсныеСпецификации.ПустаяСсылка)
		|	КОНЕЦ КАК Спецификация,", ""));
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&КоличествоСпецификаций,", 
		?(ДополнительныеПараметры.ИспользуютсяСпецификации, "ЕСТЬNULL(ДанныеСпецификаций.КоличествоСпецификаций, 0) КАК КоличествоСпецификаций,", ""));
	
	ТекстЗапроса = ТекстЗапроса + ?(ДополнительныеПараметры.ИспользуютсяСпецификации, "
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВтДанныеСпецификаций КАК ДанныеСпецификаций
		|	ПО ДанныеСпецификаций.Идентификатор = ЗагружаемыеДанные.Идентификатор", "");
	//-- НЕ УТ
	...
	ИначеЕсли ДополнительныеПараметры.ТипПлана = "ПланПроизводства" Тогда
		...
		ТекстДляЗамены = ?(ДополнительныеПараметры.ИспользуютсяДатыВыпуска,"ЗагружаемыеДанные.ДатаВыпуска КАК ДатаВыпуска," + Символы.ПС,"")
			+ ?(ДополнительныеПараметры.ИспользуютсяДатыВыпускаПродукцииПолуфабриката,"ЗагружаемыеДанные.ДатаВыпускаПродукцииПолуфабриката КАК ДатаВыпускаПродукцииПолуфабриката," + Символы.ПС,"")
			+ "ЗагружаемыеДанные.ДатаЗапуска КАК ДатаЗапуска,";
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗагружаемыеДанныеДаты,", ТекстДляЗамены);
	//-- НЕ УТ
	ИначеЕсли ДополнительныеПараметры.ТипПлана = "ПланВнутреннихПотреблений"
		ИЛИ ДополнительныеПараметры.ТипПлана = "ПланПродаж" Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗагружаемыеДанныеДаты,", 
...
// Возвращаемое значение:
//  Строка - Текст запроса вт данные спецификаций
Функция ТекстЗапросаВтДанныеСпецификаций()
	ТекстЗапроса = "
	|ВЫБРАТЬ
	|	Спецификации.Идентификатор                      КАК Идентификатор,
	|	МАКСИМУМ(Спецификации.Спецификация)             КАК Спецификация,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Спецификации.Спецификация) КАК КоличествоСпецификаций
	...
	|		ПО Спецификации.Наименование = ЗагружаемыеДанные.Спецификация
	|
	|	ГДЕ
	|		НЕ ЗагружаемыеДанные.Спецификация = """"
	|	) КАК Спецификации
	|
	|СГРУППИРОВАТЬ ПО
