...
Функция ТекстОбработаноСтрок(Количество) Экспорт
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Обработано строк: %1';
																		|en = 'Processed lines: %1'"), Количество);
КонецФункции
...
Функция ЗаполнитьВариантОбеспеченияПоОстаткамДляВыделенныхСтрокОбъекта(ДокументОбъект, КодДействия, ПараметрыЗаполнения, Строки = Неопределено, ПараметрыВстраивания = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	//++ НЕ УТКА
	Если ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		ДанныеЗаполненияСтатуса = УправлениеПроизводством.ДанныеЗаполненияПриИзмененииСтатуса(ДокументОбъект);
		Документы.ЭтапПроизводства2_2.ПослеЗаполненияОбеспечения(ДокументОбъект, ТаблицаИзменений, Режим, ПараметрыЗаполненияТекущие,
			ДанныеЗаполненияСтатуса);
	Иначе
	//-- НЕ УТКА
...
Функция ТекстОшибкиНетТоваровДоступныхДляОтгрузки() Экспорт
	...
	ТекстОшибки = НСтр("ru = 'Отгрузка по документу недоступна, так как в документе не выполнены необходимые действия. Для начала процесса отгрузки необходимо
		...
		|en = 'Shipment by the document is unavailable as the required actions are not performed in the document. To start the shipment process,
		|specify the ""Ship"" action in the goods list of product items to ship.'");
	...
	Иначе
		...
		Если Таблица.Количество() = 0 Тогда
			...
			Иначе
				СвойстваГиперссылки.Заголовок      = НСтр("ru = 'Отгружается/Отгружено';
															|en = 'Shipment in progress/Shipped'");
				СвойстваГиперссылки.ИмяСтраницы    = "СтраницаГиперссылкаСостояниеОбеспеченияОтгружается";
				СвойстваГиперссылки.ИмяГиперссылки = "ГиперссылкаСостояниеОбеспеченияОтгружается";
			КонецЕсли;
		...
		ИначеЕсли Таблица[0].ДатаОтгрузкиНеИсполнима Тогда
			...
			СвойстваГиперссылки.Заголовок      = НСтр("ru = 'Дата отгрузки не исполнима';
														|en = 'Shipment date cannot be met'");
		...
		ИначеЕсли ЗначениеЗаполнено(Таблица[0].ВозможнаяДатаОтгрузки) Тогда
			...
			ШаблонЗаголовка = НСтр("ru = 'Возможная дата отгрузки: %ВозможнаяДатаОтгрузки%';
									|en = 'Available shipment date: %ВозможнаяДатаОтгрузки%'");
		...
		Иначе
			...
			СвойстваГиперссылки.Заголовок      = НСтр("ru = 'Плановая дата обеспечения не определена';
														|en = 'Scheduled supply date is not defined'");
			СвойстваГиперссылки.ИмяСтраницы    = "СтраницаГиперссылкаСостояниеОбеспеченияСЗаголовком";
			СвойстваГиперссылки.ИмяГиперссылки = "ГиперссылкаСостояниеОбеспеченияСЗаголовком";
...
Функция РаспределениеЗапасовДляРедактируемогоДокумента(ВызовИзФормы, ИсточникДанных, ОтборСостояний = Неопределено) Экспорт
	...
	ОбъектФормы = ИсточникДанных.Объект; // ДокументОбъект
...
Процедура РезервироватьИзДругихЗаказов(ПараметрыЗаполнения, АдресРезультата) Экспорт
	...
	Попытка
		...
		Если Не НедостаточноДоступногоОстатка Тогда
			...
			Запрос.Текст =
				...
				|		КОГДА ЛОЖЬ
				//++ НЕ УТКА
				|			ИЛИ НЕ ДинамическийЭтап.Ссылка ЕСТЬ NULL
				//-- НЕ УТКА
				|			ТОГДА ИСТИНА
				|		ИНАЧЕ ЛОЖЬ
				...
				|		
				//++ НЕ УТКА
				|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2 КАК ДинамическийЭтап
				|		ПО ДинамическийЭтап.Ссылка = Заказы.Заказ
				|		 И ДинамическийЭтап.ДинамическаяСтруктура
				//-- НЕ УТКА
				|ГДЕ
				|	ВЫБОР
				|		КОГДА Заказы.КСнятиюРезерваПоМереПоступления < ЕСТЬNULL(РезервыПоМереПоступленияВЗаказах.Количество, 0)
			...
			Для Каждого ТекущийЗаказ Из ЗаказыНаОтгрузку Цикл
				...
				Если ДокументОбъект.Проведен Тогда
					...
					Если ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
						...
						ДанныеЗаполненияСтатуса = УправлениеПроизводством.ДанныеЗаполненияПриИзмененииСтатуса(ДокументОбъект);
						Документы.ЭтапПроизводства2_2.ПослеЗаполненияОбеспечения(
							ДокументОбъект,
							ТаблицаИзменений,
							Режим,
							Неопределено,
					...
					Иначе
						...
						ВызватьИсключение СтрШаблон(НСтр("ru = 'Документ %1 не проведен и не будет обработан.';
														|en = 'Document %1 has not been posted and will not be processed.'"), ЗаказНаОтгрузку);
					КонецЕсли;
					...
					Для Каждого ТекущийЗаказ Из ЗаказыНаОтгрузку Цикл
						...
						Если ДокументОбъект.Проведен Тогда
							...
							Иначе
								СообщенияПользователю = ПолучитьСообщенияПользователю(Истина);
								ВызватьИсключение СтрШаблон(НСтр("ru = 'Документ %1 не проведен и не будет обработан.';
																|en = 'Document %1 has not been posted and will not be processed.'"), СтрокаТаблицы.Заказ);
							КонецЕсли;
			...
			ИначеЕсли Не ДосчитыватьРегламентнымЗаданием Тогда
				...
				//++ НЕ УТКА
				СтруктураЗаказаПроведениеДокументов.СформироватьЗаданияКРаспределениюЗапасовПриЗаписиДокумента(Документ, МенеджерВременныхТаблиц);
				//-- НЕ УТКА
				УстановитьПривилегированныйРежим(Ложь);
	...
	Исключение
		...
		ИмяСобытияЖурнала = НСтр("ru = 'Состояние обеспечения: Резервировать на складе';
								|en = 'Supply forecast: Reserve at warehouse'", ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурнала, УровеньЖурналаРегистрации.Ошибка);
		
		Если Не ОбъектнаяБлокировкаУстановлена Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось заблокировать объект %1, возможно он редактируется.';
										|en = 'Cannot lock the %1 object. It may be being edited.'"), ТекущийЗаказ);
		ИначеЕсли Не УправляемаяБлокировкаНаСсылкиУстановлена Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось заблокировать объект %1, через некоторое время попробуйте снова. Подробности см. в журнале регистрации.';
										|en = 'Cannot lock the %1 object. Please try again later. For details, see the event log.'"), ТекущийЗаказ);
		...
		ИначеЕсли Не УправляемаяБлокировкаНаОстаткиУстановлена Тогда
			ТекстОшибки = НСтр("ru = 'Не удалось заблокировать остатки товаров, через некоторое время попробуйте снова. Подробности см. в журнале регистрации.';
								|en = 'Cannot lock the goods balance. Please try again later. For details, see the event log.'");
		Иначе
...
Процедура ОбменыПослеЗагрузкиДанных(НоменклатураСоответствие) Экспорт
	...
	Если Не РаспределениеЗапасов.ДосчитыватьРегистрРегламентнымЗаданием() Тогда
		...
		//++ НЕ УТКА
		СтруктураЗаказаПроведениеДокументов.СформироватьЗаданияКРаспределениюЗапасовПриЗаписиДокумента(ЭмуляцияДокумента, МенеджерВременныхТаблиц);
		//-- НЕ УТКА
	КонецЕсли;
	ПроверитьЗапуститьФоновоеЗаданиеРаспределенияЗапасов();
...
Функция ИмяСобытияВыключенКонтрольРезервовТоваровПоЗаказам() Экспорт
	...
	Возврат НСтр("ru = 'Контроль резервов товаров по заказам';
				|en = 'Control of goods reserves under orders'", КодОсновногоЯзыка);
...
Функция ЭтапыПоЗаказамПереработчику(Регистраторы)
	...
	Для Каждого Ссылка Из Регистраторы Цикл
		Если ЛОЖЬ
				...
				Или ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЗаказПереработчику")
				
				//-- Устарело_Переработка24
				
				Или ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЗаказПереработчику2_5") Тогда
			ЗаказыПереработчику.Добавить(Ссылка);
		КонецЕсли;
	КонецЦикла;
	Если ЗаказыПереработчику.Количество() > 0 Тогда
		Запрос = Новый Запрос();
		Запрос.УстановитьПараметр("ЗаказыПереработчику", ЗаказыПереработчику);
		Запрос.Текст =
		"ВЫБРАТЬ
		|	Таблица.Ссылка КАК Ссылка
		|ИЗ
		...
		|		И Таблица.ЗаказПереработчику В(&ЗаказыПереработчику)";
		УстановитьПривилегированныйРежим(Истина);
