...
Процедура ШаблонИнфоКвитанция(Шаблон, Данные, Параметры) Экспорт
	...
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ИНН:';
										|en = 'TIN:'"), Данные.ИНН,  ФорматЛево);
	Модуль.ДобавитьКолонку(Шаблон, НСтр("ru = 'КПП:';
										|en = 'KPP:'"), Данные.КПП,  ФорматПраво);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'КАССА:';
										|en = 'CASH REGISTER:'"), Данные.НомерКассы, ФорматЛево);
	Модуль.ДобавитьКолонку(Шаблон, НСтр("ru = 'СМЕНА:';
										|en = 'SHIFT:'"), Данные.НомерСмены, ФорматПраво);
	
	ДатаВремя = Формат(ОбщегоНазначенияБПО.ДатаСеанса(), "ДФ=""дд.ММ.гггг ЧЧ:мм""");
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ЧЕК:';
										|en = 'RECEIPT:'"), Данные.НомерЧека, ФорматЛево);
	Модуль.ДобавитьКолонку(Шаблон, НСтр("ru = 'ДАТА:';
										|en = 'DATE:'"), ДатаВремя, ФорматПраво);
...
Процедура ПодготовитьКассовыйЧек(Данные, ФискальнаяОперация, ВерсияФФД)
	...
	Для Каждого ПозицияЧека Из Данные.ПозицииЧека Цикл
		...
		Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
			...
			ИначеЕсли СтавкаНДС < 100 Тогда
				ТекстНалога = СтрШаблон(НСтр("ru = 'НДС %1%%';
											|en = 'VAT %1%%'"), СтавкаНДС);
				СуммаНДС = ?(СуммаНДС = Неопределено, 0, СуммаНДС);
				СуммаВТаблицеПоКлючу(ТаблицаНДС, СтавкаНДС, СуммаНДС);
				СуммаВТаблицеПоКлючу(ТаблицаСумма, СтавкаНДС, Сумма);
			ИначеЕсли СтавкаНДС >= 100 Тогда
				ТекстНалога = СтрШаблон(НСтр("ru = 'НДС %1/%2%%';
											|en = 'НДС %1/%2%%'"), СтавкаНДС-100, СтавкаНДС);
				СуммаНДС = ?(СуммаНДС = Неопределено, 0, СуммаНДС);
				СуммаВТаблицеПоКлючу(ТаблицаНДС, СтавкаНДС, СуммаНДС);
				СуммаВТаблицеПоКлючу(ТаблицаСумма, СтавкаНДС, Сумма);
			КонецЕсли;
	...
	Для Каждого СтрокаНДС Из ТаблицаНДС Цикл
		...
		Если СтавкаНДС = "БЕЗНДС" Тогда
			...
			СуммаНДС  = ?(Сумма > 0, Сумма, 0);
		ИначеЕсли СтавкаНДС = 0 Тогда
			Заголовок = " "+НСтр("ru = 'СУММА С НДС 0%';
								|en = 'AMOUNT WITH VAT 0%'");
			Сумма = ТаблицаСумма.Получить(СтавкаНДС);
			СуммаНДС  = ?(Сумма > 0, Сумма, 0);
		ИначеЕсли СтавкаНДС < 100 Тогда
			Заголовок = СтрШаблон(НСтр("ru = 'СУММА НДС %1%%';
										|en = 'VAT AMOUNT %1%%'"), СтавкаНДС);
			Сумма = ТаблицаСумма.Получить(СтавкаНДС);
			СуммаНДС  = ?(СуммаНДС > 0, СуммаНДС, Сумма / (100+СтавкаНДС) * СтавкаНДС );
		ИначеЕсли СтавкаНДС >= 100 Тогда
			Заголовок = СтрШаблон(НСтр("ru = 'СУММА НДС %1/%2%%';
										|en = 'СУММА НДС %1/%2%%'"), СтавкаНДС-100, СтавкаНДС);
			Сумма = ТаблицаСумма.Получить(СтавкаНДС);
	...
	Если ЗначениеЗаполнено(ФискальнаяОперация) Тогда
		...
		Данные.QRКод.ЗначениеКода = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.СформироватьQRКодЧекаККТ(ПараметрыQRКода);
	КонецЕсли;
...
Процедура ШаблонОтчет(Шаблон, Данные, Параметры, НазваниеДокумента) Экспорт
	...
	Модуль.ДобавитьСтроку (Шаблон, Параметры.Заголовок,  ФорматЦентр);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'НЕФИСКАЛЬНЫЙ РЕЖИМ';
										|en = 'NON-FISCAL MODE'"),  ФорматЦентр);
	...
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'КАССИР:';
										|en = 'CASHIER:'"), Данные.Кассир, ФорматЛево);
	Если Параметры.Подпись Тогда
		Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ПОДПИСЬ:';
											|en = 'SIGNATURE:'"), ФорматПодпись);
	КонецЕсли;
	Модуль.ДобавитьСтроку (Шаблон, "=", ФорматРазделитель);
...
Процедура ШаблонОперацияИнкассации(Шаблон, Данные, Параметры, НазваниеОперации) Экспорт
	...
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'СУММА=';
										|en = 'AMOUNT='"), Формат(Данные.Сумма, ФорматЧисла), ФорматПраво);
	Модуль.ДобавитьСтроку (Шаблон, "=", ФорматРазделитель);
...
Процедура ШаблонКассовыйЧекШапка(Шаблон, Данные, Параметры)
	...
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'КАССОВЫЙ ЧЕК';
										|en = 'SALES SLIP'"),  ФорматЦентр);
	...
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ИНН';
										|en = 'TIN'")+" ", Данные.ОрганизацияИНН, ФорматЛево);
	...
	ДатаВремя  = Формат(ДатаВремя, НСтр("ru = 'ДФ=""дд.ММ.гггг ЧЧ:мм""';
										|en = 'DF=""MM/dd/yyyy HH:mm""'"));
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ЧЕК №';
										|en = 'RECEIPT No.'"), Данные.НомерЧека, ФорматЛево);
	...
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ПОКУПАТЕЛЬ:';
										|en = 'CUSTOMER:'"),     Покупатель,    ФорматПраво);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ИНН ПОКУПАТЕЛЯ:';
										|en = 'CUSTOMER TIN:'"), ПокупательИНН, ФорматПраво);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'АДРЕС ПОКУПАТЕЛЯ:';
										|en = 'CUSTOMER ADDRESS:'"), ПокупательАдрес, ФорматПраво);
	
	Модуль.ДобавитьСтроку (Шаблон, ВРег(Данные.ПризнакАгента), ФорматЛево);

	// ++ Локализация
	ДанныеПоставщика = Данные.ДанныеПоставщика;
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ПОСТАВЩИК:';
										|en = 'ПОСТАВЩИК:'"),      ДанныеПоставщика.Наименование, ФорматПраво);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ТЛФ.ПОСТ.:';
										|en = 'ТЛФ.ПОСТ.:'"),      ДанныеПоставщика.Телефон,      ФорматПраво);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ИНН ПОСТАВЩИКА:';
										|en = 'ИНН ПОСТАВЩИКА:'"), ДанныеПоставщика.ИНН,          ФорматПраво);
	...
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ПЛ.АГЕНТ ОПЕР.:';
										|en = 'ПЛ.АГЕНТ ОПЕР.:'"),    ПлатежныйАгент.Операция,       ФорматПраво);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ПЛ.АГЕНТ.ТЛФ.:';
										|en = 'ПЛ.АГЕНТ.ТЛФ.:'"),     ПлатежныйАгент.Телефон,        ФорматПраво);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ОПЕРАТОР ПЕРЕВОДА:';
										|en = 'ОПЕРАТОР ПЕРЕВОДА:'"), ОператорПеревода.Наименование, ФорматПраво);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ТЛФ.ОП.ПЕРЕВОДА:';
										|en = 'ТЛФ.ОП.ПЕРЕВОДА:'"),   ОператорПеревода.Телефон,      ФорматПраво);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'АДР.ОП.ПЕРЕВОДА:';
										|en = 'АДР.ОП.ПЕРЕВОДА:'"),   ОператорПеревода.Адрес,        ФорматПраво);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ИНН ОП.ПЕРЕВОДА:';
										|en = 'ИНН ОП.ПЕРЕВОДА:'"),   ОператорПеревода.ИНН,          ФорматПраво);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ОПЕР.ПО ПРИЕМУ ПЛАТ.ТЛФ:';
										|en = 'ОПЕР.ПО ПРИЕМУ ПЛАТ.ТЛФ:'"), ОператорПоПриемуПлатежей.Телефон, ФорматПраво);
	// -- Локализация
...
Процедура ШаблонКассовыйЧекПозицияЧекаФискальнаяСтрока(Шаблон, ПозицияЧека)
	...
	Если СуммаСкидок > 0 Тогда
		Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'СКИДКА=';
											|en = 'DISCOUNT='"), Формат(СуммаСкидок, ФорматЧисла2), ФорматЛево);
	ИначеЕсли СуммаСкидок < 0 Тогда
		Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'НАДБАВКА=';
											|en = 'STANDARD BONUS='"), Формат(СуммаСкидок, ФорматЧисла2), ФорматЛево);
	Иначе
	...
	Если НЕ ПустаяСтрока(ПозицияЧека.КонтрольнаяМарка) Тогда 
		Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = '[М]';
											|en = '[М]'"), ФорматЛево);
	Иначе
		...
		Если ОбщегоНазначенияБПО.ИспользуетсяМаркировка() Тогда
			...
			Если КодТовараИдентифицируетЭкземпляр Тогда
				КодТовараТекст = НСтр("ru = '[М]';
										|en = '[М]'") + КодТовараТекст + Символы.НПП;
			КонецЕсли;
		КонецЕсли;
	...
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'КОД СТРАНЫ:';
										|en = 'COUNTRY CODE:'"),  ПозицияЧека.КодСтраныПроисхожденияТовара, ФорматЛево);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ДЕКЛАРАЦИЯ:';
										|en = 'DECLARATION:'"),  ПозицияЧека.НомерТаможеннойДекларации,    ФорматЛево);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'АКЦИЗ:';
										|en = 'EXCISE:'"),       ПозицияЧека.СуммаАкциза,                  ФорматЛево);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ЕД.ИЗМЕРЕНИЯ:';
										|en = 'UNIT OF MEASURE:'"),ПозицияЧека.ЕдиницаИзмерения,             ФорматЛево);
	...
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ДОП.РЕК.:';
										|en = 'ADD. ATTR.:'"), ПозицияЧека.ДополнительныйРеквизит, ФорматПраво);
...
Процедура ШаблонКассовыйЧекПодвал(Шаблон, Данные, ФискальнаяОперация, Параметры)
	...
	Модуль.ДобавитьСтроку (Шаблон, "-", ФорматРазделитель);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ИТОГ=';
										|en = 'TOTAL='"), Формат(СуммаЧека, ФорматЧисла2), ФорматПраво);
	Модуль.ДобавитьСтроку (Шаблон, "-", ФорматРазделитель);
	Модуль.ДобавитьСтроку (Шаблон, НСтр("ru = 'ОПЛАТА';
										|en = 'PAYMENT'"), ФорматЛево);
	...
	Если СуммаОплаты - СуммаЧека > 0 Тогда
		Модуль.ДобавитьСтроку(Шаблон, НСтр("ru = 'ПОЛУЧЕНО=';
											|en = 'RECEIVED='"), Формат(СуммаОплаты, ФорматЧисла2), ФорматПраво);
		Модуль.ДобавитьСтроку(Шаблон, НСтр("ru = 'СДАЧА=';
											|en = 'CHANGE='"), Формат(СуммаОплаты - СуммаЧека, ФорматЧисла2), ФорматПраво);
	КонецЕсли; 
	...
	Модуль.ДобавитьСтроку(Шаблон, НСтр("ru = 'СНО:';
										|en = 'СНО:'"), СНО, ФорматЛево);
	// -- Локализация
	...
	Если Параметры.ВыводитьФискальнуюИнформацию И ФискальнаяОперация <> Неопределено Тогда
		Модуль.ДобавитьСтроку(Шаблон, " ");
		Модуль.ДобавитьСтроку(Шаблон, НСтр("ru = 'РН ККТ:';
											|en = 'Cash register registration number:'"), ФискальнаяОперация.РегистрационныйНомерККТ, ФорматПраво);
		Модуль.ДобавитьСтроку(Шаблон, НСтр("ru = 'ФН №:';
											|en = 'Fiscal memory device N.:'"), ФискальнаяОперация.ЗаводскойНомерФН, ФорматПраво);
		Модуль.ДобавитьСтроку(Шаблон, НСтр("ru = 'ФД №:';
											|en = 'Fiscal document No:'"), Формат(ФискальнаяОперация.НомерЧекаККМ,"ЧГ="), ФорматПраво);
		Модуль.ДобавитьСтроку(Шаблон, НСтр("ru = 'ФП:';
											|en = 'Fiscal flag:'"), ФискальнаяОперация.ФискальныйПризнак, ФорматПраво);
		Модуль.ДобавитьСтроку(Шаблон, " ");
