...
Функция ДвоичныеДанныеДокументаЭПД(Объект, Данные = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	Исключение
		...
		ОбщийТекстОшибки = НСтр("ru = 'Не все обязательные реквизиты документа заполнены';
								|en = 'Not all required document attributes are filled'");
...
Процедура ПолучитьТокеныЭПД(КлючиСинхронизации, КонтекстДиагностики) Экспорт
	...
	Для Каждого КиЗ Из КлючиСинхронизации Цикл
		...
		ПараметрыВыполненияОперации.ПредставлениеОперации = НСтр("ru = 'Получение токена ЭПД';
																|en = 'Receive an electronic payment document token'");
...
Процедура ПредставлениеИсходящихВидовЭлектронныхДокументов(ПредставленияЭПД) Экспорт
	...
	ПредставленияЭПД.Вставить(Перечисления.ТипыДокументовЭДО.ЭДФ, НСтр("ru = 'Электронные договоры фрахтования';
																		|en = 'Electronic charter contracts'"));
...
Процедура СпособыОтраженияВУчетеТипаЭлектронногоДокумента(ТипДокумента, СпособыОтраженияВУчете) Экспорт
	...
	Если ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭТрН Тогда	
		СпособыОтраженияВУчете.Добавить("ЭлектроннаяТранспортнаяНакладная", НСтр("ru = 'Электронная транспортная накладная';
																				|en = 'Electronic waybill'"), Истина);
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗН Тогда	
		СпособыОтраженияВУчете.Добавить("ЭлектронныйЗаказНаряд", НСтр("ru = 'Электронный заказ-наряд';
																		|en = 'Electronic work order'"), Истина);
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭСВ Тогда	
		СпособыОтраженияВУчете.Добавить("ЭлектроннаяСопроводительнаяВедомость", НСтр("ru = 'Электронная сопроводительная ведомость';
																					|en = 'Electronic payment summary'"), Истина);
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭЗЗ Тогда	
		СпособыОтраженияВУчете.Добавить("ЭлектронныйЗаказЗаявка", НСтр("ru = 'Электронный заказ (заявка)';
																		|en = 'Electronic order (request)'"), Истина);
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭПЛ Тогда	
		СпособыОтраженияВУчете.Добавить("ЭлектронныйПутевойЛист", НСтр("ru = 'Электронный путевой лист';
																		|en = 'Electronic waybill'"), Истина);
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ЭДФ Тогда	
		СпособыОтраженияВУчете.Добавить("ЭлектронныйДоговорФрахтования", НСтр("ru = 'Электронный договор фрахтования';
																				|en = 'Electronic charter contract'"), Истина);
	КонецЕсли;
		...
		Если РезультатыЗапроса[0].Пустой()
			ИЛИ РезультатыЗапроса[1].Пустой() Тогда
				ВызватьИсключение НСтр("ru = 'Не удалось установить отметку об исправлении, т.к. сообщение не найдено';
										|en = 'Cannot set a correction mark as the message is not found'");
		КонецЕсли;
...
Процедура ЗаписатьОшибкуВЖурнал(ИмяСобытия, ИнформацияОбОшибке) Экспорт
	...
	ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Обмен с ГИС ЭПД.Установление соединения с сервером интернета';
				|en = 'Exchange with State Information System electronic payment document.Establish connection with the Internet server'",
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СоздатьВерсиюПользователяМП Тогда	
			...
			Если Принято = Ложь Тогда
				...
				Иначе
					МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Документ отклонен в мобильном приложении.';
																			|en = 'The document is rejected in the mobile application.'"), , ЦветСостоянияСерый));	
				КонецЕсли;
				ЗамечанияКомментарий = Неопределено;
				ДанныеСообщения.Свойство("ЗамечанияКомментарий", ЗамечанияКомментарий);	
				МассивСтрок.Добавить(Символы.ПС);
				//@skip-check bsl-nstr-string-literal-format
				МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Комментарий: ';
																		|en = 'Comment:'"), , ЦветСостоянияСерый));
			...
			Если ПредыдущийПарныйТитул <> Неопределено Тогда
				...
				Если ЗначениеЗаполнено(Форма.Объект[ИнформацияПоПрефиксамПарногоТитула.ВПрограмме + "ИдентификаторФайла"]) = Ложь Тогда
					МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Для формирования титула сначала требуется выполнить обмен';
																			|en = 'To generate a title, perform an exchange first'")));
			...
			Если Принято <> Ложь И ПарныйТитулПолучен = Истина Тогда
				МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Проверьте данные, в случае необходимости дополните и отправьте титул.';
																		|en = 'Check the data, complete it if necessary, and send the title.'"), , ЦветСостоянияСерый));
			КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Обмен с ГИС ЭПД. Слияние элементов в разрезе типов документов ЭПД.';
									|en = 'Exchange with State Information System of Electronic Shipment Documents. Merge items broken down by document types of electronic shipment documents.'");
...
Функция ПолучитьАдресаДоставки(ЗначениеОтбора) Экспорт
	...
	Если ТипЗнч(ЗначениеОтбора) = Тип("СправочникСсылка.Организации") Тогда
		...
		ПредставленияВыбора = НСтр("ru = '<Фактический адрес организации>';
									|en = '<Business address of the company>'");
	Иначе
...
Функция ПолучитьQRКодОтОператора(Знач Отправитель, Знач УИДМинтранс, Знач Токен, СоединениеHTTP = Неопределено)
	...
	Если СоединениеHTTP = Неопределено Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'ЭПД.Соединение с сервером';
										|en = 'Electronic payment document.Connect to the server'", ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка, , , 
					НСтр("ru = 'Не удалось установить соединение';
						|en = 'Cannot connect'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ОтветHTTP.КодСостояния <> 200 Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'ЭПД.Получение QR';
										|en = 'Electronic payment document.Retrieving QR code'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если ЭлементZipФайлаQR = Неопределено Тогда
		ТекстОшибки = НСтр("ru = 'Оператор вернул неверный формат QR-кода';
							|en = 'The operator returned an incorrect QR code format'", ОбщегоНазначения.КодОсновногоЯзыка());	
	Иначе	
		Попытка
		...
		Исключение
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось извлечь файл %1 из архива по причине:';
										|en = 'Cannot extract the %1 file from the archive due to:'"),
...
Функция ПолучитьДанныеJSON(СтрокаJSON)
	...
	Попытка
		...
		ЧтениеJSON.Закрыть();
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'ЭПД.Получение токена авторизации';
										|en = 'Electronic payment document.Receive an authorization token'", ОбщегоНазначения.КодОсновногоЯзыка()),
						УровеньЖурналаРегистрации.Ошибка, , , 
						НСтр("ru = 'Получены неверные данные от оператора';
							|en = 'Incorrect data is received from the service provider'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат Неопределено;
	КонецПопытки;
...
Функция КлючеваяИнформацияПоТитулу(ОбъектXDTO)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'ЭПД.Чтение XDTO';
										|en = 'Electronic payment document.Read XDTO'", ОбщегоНазначения.КодОсновногоЯзыка()),
