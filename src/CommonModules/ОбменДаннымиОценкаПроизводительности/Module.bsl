...
Процедура Инициализировать(КомпонентыОбмена, Анализ = Ложь) Экспорт
	...
	ШаблонНаименования = НСтр("ru = '%1 в %2 для ""%3"" (%4)';
								|en = '%1 at %2 for %3 (%4)'", КодОсновногоЯзыка); // Отправка в 18.07.2022 15:09 для "Бухгалтерия предприятия 3.0"
	...
	Если Анализ Тогда
		Наименование = Наименование + " " + НСтр("ru = '(Анализ)';
												|en = '(Analysis)'", КодОсновногоЯзыка);
	КонецЕсли;
	...
	ЗаголовокЛога = НСтр("ru = 'Начало замера; Время выполнения; Тип события; Событие; Комментарий';
						|en = 'Measurement start; Execution time; Event type; Event; Comment'");
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ru = 'Ошибка при попытке удаления сеанса обмена: %1. Описание ошибки: %2';
								|en = 'An error occurred when trying to delete the exchange session: %1. Error details: %2'",
...
Процедура Завершить(КомпонентыОбмена) Экспорт
	...
	// Шапка
	ЗаписьИтогов.ЗаписатьСтроку(НСтр("ru = '--- Сеанс обмена ---';
									|en = '--- Exchange session ---'", КодОсновногоЯзыка));
	ЗаписьИтогов.ЗаписатьСтроку(НСтр("ru = 'Направление обмена:';
									|en = 'Exchange direction:'", КодОсновногоЯзыка) + КомпонентыОбмена.НаправлениеОбмена);
	
	Шаблон = НСтр("ru = 'Узел: %1 (%2)';
					|en = 'Node: %1 (%2)'", КодОсновногоЯзыка); 
	ЗаписьИтогов.ЗаписатьСтроку(СтрШаблон(Шаблон, 
		КомпонентыОбмена.УзелКорреспондента,
		КомпонентыОбмена.УзелКорреспондента.Код));
	
	ЗаписьИтогов.ЗаписатьСтроку(НСтр("ru = 'Начало:';
									|en = 'Start:'", КодОсновногоЯзыка) + ВремяНачала);
	ЗаписьИтогов.ЗаписатьСтроку(НСтр("ru = 'Окончание:';
									|en = 'End:'", КодОсновногоЯзыка) + ВремяОкончания);
	ЗаписьИтогов.ЗаписатьСтроку(НСтр("ru = 'Время выполнения (сек):';
									|en = 'Execution time (sec.):'", КодОсновногоЯзыка) + (ВремяОкончания - ВремяНачала));
	ЗаписьИтогов.ЗаписатьСтроку("");
	
	// Итоги по типу событий
	ЗаписьИтогов.ЗаписатьСтроку(НСтр("ru = '--- Итоговые значения по типам ---';
									|en = '--- Final values by types ---'", КодОсновногоЯзыка));
	ЗаписьИтогов.ЗаписатьСтроку(НСтр("ru = 'Тип события; Время выполнения';
									|en = 'Event type; Execution time'", КодОсновногоЯзыка));
	...
	// Итоги по событиям
	ЗаписьИтогов.ЗаписатьСтроку(НСтр("ru = '--- Итоговые значения по событиям ---';
									|en = '--- Final values by events ---'", КодОсновногоЯзыка));
	ЗаписьИтогов.ЗаписатьСтроку(НСтр("ru = 'Процент; Ср. время; Время; Кол-во событий; Тип события; Событие';
									|en = 'Percent; Avg time; Time; Number of events; Event type; Event'", КодОсновногоЯзыка));
...
Функция ТипСобытияПравило() Экспорт
	...
	Возврат НСтр("ru = 'Правила обменов';
				|en = 'Exchange rules'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция ТипСобытияБиблиотека() Экспорт

	Возврат НСтр("ru = 'Подсистема обменов';
				|en = 'Exchange subsystem'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция ТипСобытияПрикладное() Экспорт

	Возврат НСтр("ru = 'Остальная конфигурация';
				|en = 'The rest of the configuration'", ОбщегоНазначения.КодОсновногоЯзыка());
