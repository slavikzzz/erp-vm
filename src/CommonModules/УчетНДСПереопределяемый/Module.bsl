...
Процедура ДополнитьИнформациейОРНПТ(ДополнительнаяИнформация, СтруктураПараметров, Запрос, ПоискРНПТПоОснованию = Ложь) Экспорт
	...
	//++ НЕ УТ
	УчетНДСРФ.ДополнитьИнформациейОРНПТ(ДополнительнаяИнформация, СтруктураПараметров, Запрос, ПоискРНПТПоОснованию);
	//-- НЕ УТ
...
Функция ОпределитьДатуИНомерСчетаФактурыДляПечати(ЗаписьКниги) Экспорт
		...
		Если ТипЗнч(ЗаписьКниги.СчетФактура) = Тип("ДокументСсылка.ЗаявлениеОВвозеТоваров")
			...
			Представление = СтрШаблон(НСтр("ru = '%1 № %2 от %3';
				...
				НСтр("ru = 'Заявление о ввозе товаров';
					|en = 'Application for goods import'"), НомерСФ, ДатаСФ);
		...
		ИначеЕсли ТипЗнч(ЗаписьКниги.СчетФактура) = Тип("ДокументСсылка.ТаможеннаяДекларацияИмпорт") Тогда
			...
			ПредставлениеСчетаФактуры.Представление = СтрШаблон(НСтр("ru = '%1 № %2 от %3';
				...
				НСтр("ru = 'Таможенная декларация';
					|en = 'Customs declaration'"), НомерСФ, ДатаСФ);
	...
	Иначе
		...
		ИначеЕсли ТипЗнч(ЗаписьКниги.СчетФактура) = Тип("ДокументСсылка.ТаможеннаяДекларацияИмпорт") Тогда
			...
			ПредставлениеСчетаФактуры.Представление = СтрШаблон(НСтр("ru = '%1 № %2 от %3';
																	|en = '%1 No. %2 from %3'"),
				НСтр("ru = 'Таможенная декларация';
					|en = 'Customs declaration'"), ЗаписьКниги.НомерСчетаФактуры, Формат(ДатаГТД, "ДФ=dd.MM.yyyy"));
	...
	Если ЗначениеЗаполнено(СокрЛП(НомерСФ)) Тогда
		ПредставлениеСчетаФактуры.ДатаНомер   = СтрШаблон(НСтр("ru = '%1;%2';
																|en = '%1;%2'"), ДатаСФ, НомерСФ);
		ПредставлениеСчетаФактуры.НомерДата   = СтрШаблон(НСтр("ru = '%1;%2';
																|en = '%1;%2'"), НомерСФ, ДатаСФ);
		ПредставлениеСчетаФактуры.НомерОтДата = СтрШаблон(НСтр("ru = '%1 от %2';
																|en = '%1 from %2'"), НомерСФ, ДатаСФ);
	КонецЕсли;
...
Процедура ДополнитьСведенияОПоказателяхОтчетаНДСФормаОтчета2017Кв1Раздел7(ПоказателиОтчета) Экспорт
	...
	РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "П000700001001", Истина, Истина);
	
КонецПроцедуры

Процедура ЗаполнитьОтчетНДСФормаОтчета2017Кв1Раздел7(ПараметрыЗапросов, Контейнер) Экспорт
	
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьОсобыйПорядокУчетаНДСПриПроизводствеДЦ") Тогда
		Возврат;
	КонецЕсли;
	...
	НастройкиНалоговУчетныхПолитик.ДополнитьМенеджерВременныхТаблицДействующимиПараметрамиНалоговУчетныхПолитик(
		...
		НачалоМесяца(ПараметрыЗапросов.НачалоПериода),
		"ВТНастройкиУчетаНДС",
		ПараметрыЗапросов.Организация);
	...
	Если НоваяАрхитектураВзаиморасчетов Тогда
		...
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
		|		ВТНастройкиУчетаНДС КАК УчетныеПолитики
		|	ПО
		|		РасчетыСКлиентами.АналитикаУчетаПоПартнерам.Организация = УчетныеПолитики.Организация
		|		И УчетныеПолитики.УчетНДСДлительногоЦиклаПроизводства
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
		|	ПО
		|		РасчетыСКлиентами.ОбъектРасчетов = ДоговорыКонтрагентов.Ссылка
		|		И ДоговорыКонтрагентов.НалогообложениеНДС = &ПроизводствоСДЦ
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		Документ.ЗаказКлиента КАК ЗаказыКлиентов
		|	ПО
		|		РасчетыСКлиентами.ОбъектРасчетов = ЗаказыКлиентов.Ссылка
		|		И ЗаказыКлиентов.НалогообложениеНДС = &ПроизводствоСДЦ
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		Документ.РеализацияТоваровУслуг КАК РеализацииТоваровУслуг
		|	ПО
		|		РасчетыСКлиентами.ОбъектРасчетов = РеализацииТоваровУслуг.Ссылка
		|		И РеализацииТоваровУслуг.НалогообложениеНДС = &ПроизводствоСДЦ
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		Документ.АктВыполненныхРабот КАК АктВыполненныхРабот
		|	ПО
		|		РасчетыСКлиентами.ОбъектРасчетов = АктВыполненныхРабот.Ссылка
		|		И АктВыполненныхРабот.НалогообложениеНДС = &ПроизводствоСДЦ
		|
		|ГДЕ
		...
		|	И (
		|		НЕ ДоговорыКонтрагентов.Ссылка ЕСТЬ NULL ИЛИ
		|		НЕ ЗаказыКлиентов.Ссылка ЕСТЬ NULL ИЛИ
		|		НЕ РеализацииТоваровУслуг.Ссылка ЕСТЬ NULL ИЛИ
		|		НЕ АктВыполненныхРабот.Ссылка ЕСТЬ NULL
		|	)
		|";
	Иначе
		Запрос.Текст = 
		...
		|		Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
		|	ПО
		|		РасчетыСКлиентами.ЗаказКлиента = ДоговорыКонтрагентов.Ссылка
		|		И ДоговорыКонтрагентов.НалогообложениеНДС = &ПроизводствоСДЦ
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		Документ.ЗаказКлиента КАК ЗаказыКлиентов
		|	ПО
		|		РасчетыСКлиентами.ЗаказКлиента = ЗаказыКлиентов.Ссылка
		|		И ЗаказыКлиентов.НалогообложениеНДС = &ПроизводствоСДЦ
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		Документ.РеализацияТоваровУслуг КАК РеализацииТоваровУслуг
		|	ПО
		|		РасчетыСКлиентами.ЗаказКлиента = РеализацииТоваровУслуг.Ссылка
		|		И РеализацииТоваровУслуг.НалогообложениеНДС = &ПроизводствоСДЦ
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		Документ.АктВыполненныхРабот КАК АктВыполненныхРабот
		|	ПО
		|		РасчетыСКлиентами.ЗаказКлиента = АктВыполненныхРабот.Ссылка
		|		И АктВыполненныхРабот.НалогообложениеНДС = &ПроизводствоСДЦ
		|
		|ГДЕ
		|	РасчетыСКлиентами.Период МЕЖДУ &НачалоПериода И &КонецПериода
	...
	Запрос.УстановитьПараметр("НачалоПериода",   ПараметрыЗапросов.НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода",    ПараметрыЗапросов.КонецПериода);
	Запрос.УстановитьПараметр("ПроизводствоСДЦ", Перечисления.ТипыНалогообложенияНДС.ПроизводствоСДЦ);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Контейнер.Раздел7.Вставить("П000700001001", Выборка.ПредоплатаРегл);
	КонецЕсли;
...
Процедура СуммаНДССМежценовойРазницы(МенеджерВременныхТаблиц, ПараметрыОсновногоЗапроса) Экспорт
	...
	"ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА СчетФактураВыданныйТовары.СтавкаНДС.ПеречислениеСтавкаНДС = &ОбщаяРасчетнаяСтавкаНДС
	|			ТОГДА СчетФактураВыданныйТовары.СуммаНДС
	|	КОНЕЦ КАК СуммаНДСМежценоваяРазницаОсновнаяРасчетнаяСтавка,
	|	ВЫБОР
	|		КОГДА СчетФактураВыданныйТовары.СтавкаНДС.ПеречислениеСтавкаНДС = &СтавкаНДС10_110
	|			ТОГДА СчетФактураВыданныйТовары.СуммаНДС
	...
	|	И СчетФактураВыданныйТовары.МежценоваяРазница > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ПриСозданииНаСервереДокументаНДС(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	УчетНДСУПСлужебный.НастроитьСовместныйВыборКонтрагентовОрганизаций(Элементы.НеоблагаемыеНДСОперацииКонтрагент);
...
Функция РеквизитыОрганизацииДляОтчетностиПоНДС(Организация) Экспорт
	...
	ПараметрыОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		Организация, "ОтделениеИностраннойОрганизации,КПП");
		
	Возврат Новый Структура("ИностраннаяОрганизация,КПП", 
		ПараметрыОрганизации.ОтделениеИностраннойОрганизации, ПараметрыОрганизации.КПП);	
...
Процедура ТекстЗапросаРеестрНДСПриложение(Запрос, ПараметрыОтчета) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ПодтвержденныеОтгрузки.ДокументОтгрузки ССЫЛКА Документ.РеализацияТоваровУслуг
	|			ТОГДА ВЫРАЗИТЬ(ПодтвержденныеОтгрузки.ДокументОтгрузки КАК Документ.РеализацияТоваровУслуг).Договор
	|		КОГДА ПодтвержденныеОтгрузки.ДокументОтгрузки ССЫЛКА Документ.РеализацияУслугПрочихАктивов
	|			ТОГДА ВЫРАЗИТЬ(ПодтвержденныеОтгрузки.ДокументОтгрузки КАК Документ.РеализацияУслугПрочихАктивов).Договор
	|		КОГДА ПодтвержденныеОтгрузки.ДокументОтгрузки ССЫЛКА Документ.АктВыполненныхРабот
	|			ТОГДА ВЫРАЗИТЬ(ПодтвержденныеОтгрузки.ДокументОтгрузки КАК Документ.АктВыполненныхРабот).Договор
	//++ НЕ УТКА
	|		КОГДА ПодтвержденныеОтгрузки.ДокументОтгрузки ССЫЛКА Документ.ОтчетДавальцу
	|			ТОГДА ВЫРАЗИТЬ(ПодтвержденныеОтгрузки.ДокументОтгрузки КАК Документ.ОтчетДавальцу).Договор
	|		КОГДА ПодтвержденныеОтгрузки.ДокументОтгрузки ССЫЛКА Документ.ОтчетДавальцу2_5
	|			ТОГДА ВЫРАЗИТЬ(ПодтвержденныеОтгрузки.ДокументОтгрузки КАК Документ.ОтчетДавальцу2_5).Договор
	//-- НЕ УТКА
	
	|		КОГДА ПодтвержденныеОтгрузки.ДокументОтгрузки ССЫЛКА Документ.ОтчетПереработчика
	|			ТОГДА ВЫРАЗИТЬ(ПодтвержденныеОтгрузки.ДокументОтгрузки КАК Документ.ОтчетПереработчика).Договор
	|		КОГДА ПодтвержденныеОтгрузки.ДокументОтгрузки ССЫЛКА Документ.ОтчетПереработчика2_5
	|			ТОГДА ВЫРАЗИТЬ(ПодтвержденныеОтгрузки.ДокументОтгрузки КАК Документ.ОтчетПереработчика2_5).Договор
	...
	|	ЕСТЬNULL(ДоговорыКонтрагентов.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаКонтракта,
	|	ЕСТЬNULL(ДоговорыКонтрагентов.Номер, """") КАК НомерКонтракта
	|ИЗ
	...
	|		ПО НалоговаяБазаПоДекларациям.ДоговорКонтрагента = ДоговорыКонтрагентов.Ссылка
	|ИТОГИ
	|	МАКСИМУМ(Примечание),
	|	МАКСИМУМ(ПодтвержденнаяНалоговаяБаза),
	|	МАКСИМУМ(ДатаКонтракта),
	|	МАКСИМУМ(НомерКонтракта)
	|ПО
	|	КодОперации,
	|	НомерТаможеннойДекларации,
	|	КодТС";
