...
Асинх Функция УстановитьКомпонентуАсинх(Контекст) Экспорт
	...
	Если СимволическоеИмя = Неопределено Тогда 
		...
		ТекстПояснения =  ?(ПустаяСтрока(Контекст.ТекстПояснения),
			НСтр("ru = 'Установить внешнюю компоненту?';
				|en = 'Do you want to install the add-in?'"), Контекст.ТекстПояснения);
...
#Если ВебКлиент Тогда
	Результат = НСтр("ru = 'Веб-клиент';
					|en = 'Web client'") + СистемнаяИнформация.ИнформацияПрограммыПросмотра;
	...
	Иначе 
		...
		ИначеЕсли Не Контекст.ПредложитьУстановить Тогда
			...
			ТекстОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|It might not have been installed.'"), Контекст.Идентификатор);
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		ТекстОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"".
						 ...
						 |Technical details:
						 |%3
						 |Method %4 returned False.'"), Контекст.Идентификатор, Результат,
	...
	Иначе
		ТекстОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"".
						 ...
						 |Technical details:
						 |%3
						 |Method %4 returned False.'"), Контекст.Идентификатор, ВидПриложения(), Контекст.Местоположение,
			...
			Если ЗначениеЗаполнено(ОшибкаСовместимостиКомпоненты) Тогда
				ТекстОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"".
						 ...
						 |Technical details:
						 |%3
						 |Method %4 returned False.'"), Контекст.Идентификатор, ОшибкаСовместимостиКомпоненты,
	...
	Иначе 
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось зарегистрировать компоненту comcntr.
			           ...
			           |en = 'Cannot register the comcntr add-in.
		...
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru = 'Регистрация компоненты comcntr';
				|en = 'Registration of comcntr component'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
			"Ошибка",
			ТекстСообщения,,
			Истина);
