...
Процедура ПроверитьЗаписьВФормеПоСтруктуре(Форма, ИмяРегистра, СтруктураВедущихОбъектов, Отказ)
	...
	Если ИзменилисьДанные Тогда
		...
		ПроверитьЗаписьРегистраВФорме(Форма, МенеджерЗаписи, ИмяРегистра, СтруктураВедущихОбъектов, Отказ);
	ИначеЕсли Форма[ИмяРегистра + "НаборЗаписейПрочитан"] Тогда
		Для Каждого Запись Из Форма[ИмяРегистра + "НаборЗаписей"] Цикл
			ПроверитьЗаписьРегистраВФорме(Форма, Запись, ИмяРегистра, СтруктураВедущихОбъектов, Отказ);
		КонецЦикла;
	КонецЕсли;
...
Процедура ПроверитьЗаписьРегистраВФорме(Форма, МенеджерЗаписи, ИмяРегистра, СтруктураВедущихОбъектов, Отказ)
...
Процедура ПроверитьЗаполнениеПоляЗаписиРегистраВФорме(МенеджерЗаписи, ИмяРегистра, ОписаниеПоля, Синоним, Отказ, ПутьКРеквизитуФормы = "")
	...
	Если ОписаниеПоля.ПроверкаЗаполнения = ПроверкаЗаполнения.ВыдаватьОшибку И Не ЗначениеЗаполнено(МенеджерЗаписи[ОписаниеПоля.Имя]) Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1: не заполнено поле ""%2"".';
																						|en = '%1: ""%2"" is required.'"), Синоним, ?(ЗначениеЗаполнено(ОписаниеПоля.Синоним), ОписаниеПоля.Синоним, ОписаниеПоля.Имя));
