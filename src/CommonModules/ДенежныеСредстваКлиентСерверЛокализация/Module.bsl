...
Процедура ПроверитьУникальныйНомерВалютногоКонтракта(Объект, НомерКонтракта, Отказ = Ложь) Экспорт
	...
	Если ВсегоЧастей = 1
		...
		ТекстСообщения = НСтр("ru = '- номер не соответствует формату ХХХХХХХХ/ХХХХ/ХХХХ/Х/Х';
								|en = '- the number does not match the ХХХХХХХХ/ХХХХ/ХХХХ/Х/Х format'");
	...
	Пока ТекущаяЧасть <= ВсегоЧастей Цикл
		...
		Если КонтрольДлины[ТекущаяЧасть] <> ДлинаЧастиНомера Тогда
			ТекстСообщения = СтрШаблон(
				НСтр("ru = '- количество разрядов в %1 части должно соответствовать %2';
					|en = '- the number of digits in part %1 must match %2'"),
				ТекущаяЧасть,
		...
		Если ТекущаяЧасть = 1
			...
			Если ОбщегоНазначенияКлиентСервер.ЭтоЧисло(МесяцНомера)
				...
				ТекстСообщения = НСтр("ru = '- ошибка в 3 и 4 разрядах 1 части номера';
										|en = '- ошибка в 3 и 4 разрядах 1 части номера'");
				СписокОшибок.Добавить(ТекстСообщения);
		...
		Если НедопустимыеСимволы <> "" Тогда
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = '- в %1 части введены недопустимые символы (%2)';
											|en = '- в %1 части введены недопустимые символы (%2)'"), ТекущаяЧасть, НедопустимыеСимволы);
			СписокОшибок.Добавить(ТекстСообщения);
		...
		Если ТекущаяЧасть = 3
			...
			Если (СтрНайти(ВРег(ЧастьНомера), "G") > 0
				...
				ТекстСообщения = НСтр("ru = '- в 3 части допустимо использовать символы GU только совместно и в 1 и 2 разрядах';
										|en = '- в 3 части допустимо использовать символы GU только совместно и в 1 и 2 разрядах'");
				СписокОшибок.Добавить(ТекстСообщения);
	...
	Если СписокОшибок.Количество() <> 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(НСтр("ru = 'Значение в поле ""УНК"" введено не корректно: 
								|	%1';
								|en = 'Incorrect ""Unique contract number"" field value: 
	...
	#Иначе
		...
		Если КоллекцияСвойствXDTOФайл.Получить("РезультатЗачисленияЗарплаты") = Неопределено Тогда
			...
			ТекстСообщения = НСтр("ru = '<Нет сведений о результатах зачисления денежных средств.>';
									|en = '<No information about the results of cash receipt.>'");
			Возврат;
		...
		Если Не СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(ОбъектXDTOФайл.ИдПервичногоДокумента)
			...
			ТекстСообщения = НСтр("ru = '<Неверный ИдПервичногоДокумента в загружаемом файле (ожидается не пустой GUID).>';
									|en = '<Incorrect ИдПервичногоДокумента in the file to be imported (not empty GUID is expected).>'");
			Возврат;
