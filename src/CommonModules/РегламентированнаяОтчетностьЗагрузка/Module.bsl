...
Функция ЗагрузкаОтчетаИзФайла(ФормаОтчета, ПараметрыОтчета, ДанныеДляЗагрузки, ДеревоДляЗагрузки) Экспорт
	...
	Если ОсновнойФайл = Неопределено ИЛИ НЕ ЭтоАдресВременногоХранилища(ОсновнойФайл.АдресТекста) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось открыть файл выгрузки!';
													|en = 'Не удалось открыть файл выгрузки!'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ТипЗнч(НастройкиЗагрузки) <> Тип("Структура")
		...
		СообщениеОбОшибке.ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Возможность загрузки из файла отчета ""%1"" за выбранный период не поддерживается.';
				|en = 'Возможность загрузки из файла отчета ""%1"" за выбранный период не поддерживается.'"),
	...
	Если ТипЗнч(ДеревоДляЗагрузки) <> Тип("ДеревоЗначений") И ТипЗнч(ДеревоДляЗагрузки) <> Тип("Структура") Тогда
		...
		СообщениеОбОшибке.ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Возможность загрузки из файла отчета ""%1"" не реализована.';
				|en = 'Возможность загрузки из файла отчета ""%1"" не реализована.'"),
			ДанныеДляЗагрузки.ВидОтчета);
		Возврат Ложь;
	КонецЕсли;
	...
	ИначеЕсли ДанныеДляЗагрузки.ТипДокумента = Перечисления.ТипыОтправляемыхДокументов.ПрочаяОтчетностьФНС
		...
		Тогда 
			Если ТипЗнч(ДеревоДляЗагрузки) = Тип("Структура") Тогда
				...
				Если СтрНайти(ОсновнойФайл.ИмяФайла, ".2_") > 0 ИЛИ СтрНайти(ОсновнойФайл.ИмяФайла, "-2_") > 0 Тогда // это файл сведений
				...
				Если ВыбранноеДеревоДляЗагрузки = Неопределено Тогда
					...
					СообщениеОбОшибке.ТекстСообщения = НСтр("ru = 'Не определена схема для загрузки реестра НДС из файла.';
															|en = 'Не определена схема для загрузки реестра НДС из файла.'");
					Возврат Ложь;
				КонецЕсли;
			...
			Если ТипЗнч(ДеревоДляЗагрузки) = Тип("Структура") Тогда
				...
				Если ВыбранноеДеревоДляЗагрузки = Неопределено Тогда
					...
					СообщениеОбОшибке.ТекстСообщения = НСтр("ru = 'Не определена схема для загрузки реестра по акцизам из файла.';
															|en = 'Не определена схема для загрузки реестра по акцизам из файла.'");
					Возврат Ложь;
				КонецЕсли;
				ДеревоДляЗагрузки = ВыбранноеДеревоДляЗагрузки;
			КонецЕсли;
	...
	ИначеЕсли ДанныеДляЗагрузки.ТипДокумента = Перечисления.ТипыОтправляемыхДокументов.КомплектОтчетностиПФР Тогда
		...
		СообщениеОбОшибке.ТекстСообщения = НСтр("ru = 'Возможность загрузки из файла отчетов ПФР не поддерживается.';
												|en = 'Возможность загрузки из файла отчетов ПФР не поддерживается.'");
		Возврат Ложь;
		...
		Если ДанныеДляЗагрузки.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.СтатистикаБухОтчетность Тогда
			...
			СообщениеОбОшибке.ТекстСообщения = НСтр(
				"ru = 'Возможность загрузки из файла отчета ""Бухгалтерская (финансовая) отчетность в Росстат"" не поддерживается.
					  ...
					  |en = 'Возможность загрузки из файла отчета ""Бухгалтерская (финансовая) отчетность в Росстат"" не поддерживается.
					  |Выберите соответствующий вид отчета ""Бухгалтерская отчетность..."" для ФНС.'");
			Возврат Ложь;
	...
	ПараметрыЗапуска.НаименованиеФоновогоЗадания = НСтр("ru = 'Загрузка регламентированного отчета из файла электронного представления';
														|en = 'Загрузка регламентированного отчета из файла электронного представления'");
...
Процедура ЗагрузкаОтчетаИзФайлаДлительнаяОперация(ПараметрыПроцедуры, АдресРезультата) Экспорт
	...
	Попытка
		...
		// Если в обработчике были определены параметры выполнения метода конфигурации или объекта - выполняем метод с привилегиями конфигурации.
		МетодКонфигурацииИлиОбъекта = Неопределено;
		Если ПараметрыОбработчика.ПараметрыОтчета.Свойство("ВыполнитьМетодКонфигурацииИлиОбъекта", МетодКонфигурацииИлиОбъекта)
		   И ТипЗнч(МетодКонфигурацииИлиОбъекта) = Тип("Структура") Тогда
			Если МетодКонфигурацииИлиОбъекта.Свойство("Объект") Тогда
				ОбщегоНазначения.ВыполнитьМетодОбъекта(
					МетодКонфигурацииИлиОбъекта.Объект,
					МетодКонфигурацииИлиОбъекта.ИмяМетода,
					МетодКонфигурацииИлиОбъекта.Параметры);
			Иначе
				ОбщегоНазначения.ВыполнитьМетодКонфигурации(
					МетодКонфигурацииИлиОбъекта.ИмяМетода,
					МетодКонфигурацииИлиОбъекта.Параметры);
			КонецЕсли;
			ПараметрыОбработчика.ПараметрыОтчета.Удалить("ВыполнитьМетодКонфигурацииИлиОбъекта");
		КонецЕсли;
	...
	Исключение
		...
		ОбщегоНазначения.СообщитьПользователю(
			СтрШаблон(НСтр("ru = 'Не удалось заполнить схему данных из файла! %1';
							|en = 'Не удалось заполнить схему данных из файла! %1'"), ОписаниеОшибки));
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка данных регламентированного отчета';
										|en = 'Загрузка данных регламентированного отчета'"),
	...
	Исключение
		ОписаниеОшибки = ИнформацияОбОшибке().Описание;
		ОбщегоНазначения.СообщитьПользователю(
			СтрШаблон(НСтр("ru = 'Не удалось заполнить и сохранить документ отчета! %1';
							|en = 'Не удалось заполнить и сохранить документ отчета! %1'"), ОписаниеОшибки));
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка данных регламентированного отчета';
										|en = 'Загрузка данных регламентированного отчета'"),
			УровеньЖурналаРегистрации.Ошибка,,, ОписаниеОшибки);
		ПоместитьВоВременноеХранилище(Ложь, АдресРезультата);
...
Процедура ПолучитьДеревоДляЗагрузкиПоФормату(ФормаОтчета, ДеревоДляЗагрузки, Настройки, ВерсияФормата)
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(
			СтрШаблон(НСтр("ru = 'Не удалось получить схему загрузки для версии данных %1. Используется схема по умолчанию.';
							|en = 'Не удалось получить схему загрузки для версии данных %1. Используется схема по умолчанию.'"), ВерсияФормата));
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Получение схемы для загрузки';
										|en = 'Получение схемы для загрузки'"),
...
Процедура ЗаполнитьДаннымиДерево(ДеревоДляЗагрузки, ИмяФайла, РазмерТекста = 0);
	...
	ДеревоДляЗагрузкиПрототипы.Колонки.Добавить("ТекИндДУ", Новый ОписаниеТипов("Число"));
	...
	Исключение
		...
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Ошибка чтения XML: %1.';
										|en = 'Ошибка чтения XML: %1.'"), ИнформацияОбОшибке().Описание);
	КонецПопытки;
	...
	Пока ОбъектЧтениеXML.Прочитать() Цикл
		...
		Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Если СтрокиДереваЭлементы.Количество() = 0
				...
				Иначе
					...
					// Запишем индекс предыдущего дочернего узла в дерево прототипов для ускорения поиска при копировании узлов.
					ПрототипДобавляемогоУзла.ТекИндДУ = УзелДерева.Строки.Индекс(ПредыдущийДочернийУзелДерева);
			...
			Если НЕ ЭлементНайден Тогда
				...
				ТекстСообщения = СтрШаблон(НСтр("ru = 'В схеме загрузки не найден узел с именем ""%1""!';
												|en = 'В схеме загрузки не найден узел с именем ""%1""!'"), ИмяТекущегоЭлемента);
		...
		Если Счетчики.РазмерТекста > 0 Тогда
			...
			Если Счетчики.Процент < Процент Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'загружено элементов %1';
												|en = 'загружено элементов %1'"), Формат(Счетчики.ОбработаноУзлов, "ЧГ="));
...
Функция ВсеЭлементыИАтрибутыУзла(Узел, Атрибуты = Неопределено)
	...
	Для Каждого ПодчиненныйУзел Из Узел.Строки Цикл
		...
		Иначе
			ТекстСообщения = СтрШаблон(НСтр("ru = 'В схеме загрузки не распознан тип узла ""%1""!';
											|en = 'В схеме загрузки не распознан тип узла ""%1""!'"), ПодчиненныйУзел.Код);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
	КонецЦикла;
...
Функция СкопированныйУзел(Родитель, Узел, ПолеПоиска = "", ЗначениеПоиска = Неопределено)
	...
	РодительСтроки = Родитель.Строки;
	...
	Если ЗначениеПоиска <> Неопределено Тогда
		...
		Если ПолеПоиска = "Прототип" И ЗначениеПоиска.ТекИндДУ > 0 Тогда
			НайденнаяСтрока = РодительСтроки[ЗначениеПоиска.ТекИндДУ];
			Если НайденнаяСтрока.Код <> Узел.Код Тогда
				НайденнаяСтрока = РодительСтроки.Найти(ЗначениеПоиска, ПолеПоиска, Ложь);
			КонецЕсли;
		Иначе
			НайденнаяСтрока = РодительСтроки.Найти(ЗначениеПоиска, ПолеПоиска, Ложь);
		КонецЕсли;
		...
		КонецЕсли;
	Иначе
		СтрокиСТемЖеКодом = РодительСтроки.НайтиСтроки(Новый Структура("Код", Узел.Код), Ложь);
	КонецЕсли;
	...
	Если МаксИндексМассиваСтрок >= 0 Тогда
		ИндексПоследнейСтроки = РодительСтроки.Индекс(СтрокиСТемЖеКодом[МаксИндексМассиваСтрок]);
		Если ИндексПоследнейСтроки >= (МаксИндексМассиваСтрок + ИндексКопируемогоУзла) Тогда
			МаксИндекс = ИндексПоследнейСтроки;
		Иначе
			Для Каждого СтрокаСТемЖеКодом Из СтрокиСТемЖеКодом Цикл
				ТекИндекс = РодительСтроки.Индекс(СтрокаСТемЖеКодом);
	...
	Если ИндексСоздаваемогоУзла >= РодительСтроки.Количество() ИЛИ ИндексСоздаваемогоУзла = 0 Тогда
		НовыйУзел = РодительСтроки.Добавить();
	Иначе
		НовыйУзел = РодительСтроки.Вставить(ИндексСоздаваемогоУзла);
	КонецЕсли;
...
Функция ЗаполнитьДаннымиСохраненныйДокумент(СохраненныйДок, ДеревоДляЗагрузки, ПараметрыОтчета, ДанныеДляЗагрузки, ТекстыПравилЗагрузки)
	...
	Исключение
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Ошибка записи заполненного документа отчета: %1.';
										|en = 'Ошибка записи заполненного документа отчета: %1.'"),
...
Процедура ЗаполнитьПоказательДокументаИзУзлаДерева(Узел, ДанныеОтчета, ПараметрыОтчета, НомерСтраницы = 0, НомерСтроки = 0)
	...
	Если ТипЗнч(НомерСтраницы) = Тип("Массив") Тогда // раздел многоуровневый (многостраничный)
		...
		Если ДанныеОтчета.ДанныеМногоуровневыхРазделов = Неопределено Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Не найдены данные многоуровневой части документа отчета. Ошибка в разделе: ""%1"".';
											|en = 'Не найдены данные многоуровневой части документа отчета. Ошибка в разделе: ""%1"".'"), Раздел);
		КонецЕсли;
		...
		Если ТипЗнч(МногоуровневыйРаздел) <> Тип("ДеревоЗначений") ИЛИ МногоуровневыйРаздел.Строки.Количество() = 0 Тогда
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'В данных документа отчета не найден многоуровневый раздел ""%1"" для показателя ""%2"".';
								|en = 'В данных документа отчета не найден многоуровневый раздел ""%1"" для показателя ""%2"".'"),
				Раздел, Узел.Ключ));
			Возврат;
		КонецЕсли;
		...
		Если ВетвьДерева = МногоуровневыйРаздел Тогда
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'В данных документа отчета не найдена страница многоуровневого раздела ""%1"" для показателя ""%2"".';
								|en = 'В данных документа отчета не найдена страница многоуровневого раздела ""%1"" для показателя ""%2"".'"),
				Раздел, Узел.Ключ));
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли НомерСтраницы > 0 Тогда // раздел многостраничный
		...
		Если ДанныеОтчета.ДанныеМногостраничныхРазделов = Неопределено Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Не найдены данные многостраничной части документа отчета. Ошибка в разделе: ""%1"".';
											|en = 'Не найдены данные многостраничной части документа отчета. Ошибка в разделе: ""%1"".'"), Раздел);
		КонецЕсли;
		...
		Если ТипЗнч(МногостраничныйРаздел) <> Тип("ТаблицаЗначений") ИЛИ МногостраничныйРаздел.Количество() = 0 Тогда
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'В данных документа отчета не найден многостраничный раздел ""%1"" для показателя ""%2"".';
								|en = 'В данных документа отчета не найден многостраничный раздел ""%1"" для показателя ""%2"".'"),
				Раздел, Узел.Ключ));
			Возврат;
		КонецЕсли;
		...
		Если ТипЗнч(СтраницаРаздела) <> Тип("Структура") Тогда
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'В данных документа отчета не найден раздел ""%1"" для показателя ""%2"".';
								|en = 'В данных документа отчета не найден раздел ""%1"" для показателя ""%2"".'"),
				Раздел, Узел.Ключ));
			Возврат;
		КонецЕсли;
	...
	Если НомерСтроки = 0 ИЛИ (ТипЗнч(НомерСтроки) = Тип("Массив") И ЕстьДанныеМногострочныхЧастейМнУр <> Ложь) Тогда // обычные и многоуровневые разделы
		Если СтраницаРаздела.Свойство(Узел.Ключ, ЗначениеПоказателя) Тогда
			...
			Если ЗначениеЗаполнено(Узел.Значение) Тогда
				ЭлементЗаполненногоРаздела = ДанныеОтчета.СписокЗаполненныхРазделов.НайтиПоЗначению(Раздел);
				Если ЭлементЗаполненногоРаздела = Неопределено Тогда
					ДанныеОтчета.СписокЗаполненныхРазделов.Добавить(Раздел, Раздел);
				ИначеЕсли ЗначениеЗаполнено(НомерСтраницы)
						И ТипЗнч(НомерСтраницы) = Тип("Массив") И НомерСтраницы[НомерСтраницы.ВГраница()] = 10000 Тогда
					ЭлементЗаполненногоРаздела.Пометка = Истина; // отмечаем раздел, если в нем 10000 и более страниц
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	Иначе
		...
		Если СтраницаРаздела.Свойство(КодПоказателяДляПоиска, ЗначениеПоказателя)
			...
			Если ЗначениеЗаполнено(Узел.Значение) Тогда
				...
				Если ЭлементЗаполненногоРаздела = Неопределено Тогда
					ДанныеОтчета.СписокЗаполненныхРазделов.Добавить(Раздел, Раздел);
				ИначеЕсли ЗначениеЗаполнено(НомерСтраницы)
					И ?(ТипЗнч(НомерСтраницы) = Тип("Массив"), НомерСтраницы[НомерСтраницы.ВГраница()], НомерСтраницы) = 10000 Тогда
					ЭлементЗаполненногоРаздела.Пометка = Истина; // отмечаем раздел, если в нем 10000 и более страниц
				КонецЕсли;
			КонецЕсли;
		Иначе
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'В данных раздела ""%1"" документа отчета не найден показатель ""%2"" многострочной части.';
								|en = 'В данных раздела ""%1"" документа отчета не найден показатель ""%2"" многострочной части.'"),
				Раздел, Узел.Ключ));
		КонецЕсли;
	КонецЕсли;
...
Функция ВключитьЗаполненныеСтраницыВДеревеНастройкиСтраниц(ДанныеОтчета)
	...
	Если ТипЗнч(ДанныеОтчета.ДеревоНастройкиСтраниц) = Тип("ДеревоЗначений") Тогда
		...
		Для Каждого ОписаниеСтраницыРаздела Из ДанныеОтчета.ДеревоНастройкиСтраниц.Строки Цикл
			ЭлементСписка = СписокЗаполненныхРазделов.НайтиПоЗначению(ОписаниеСтраницыРаздела.ИмяСтраницы);
			Если ЭлементСписка <> Неопределено Тогда
				Если ЗначениеЗаполнено(ОписаниеСтраницыРаздела.ПоказатьСтраницу) Тогда
		...
		Если НастройкиЗагрузки.Свойство("ИмяФайлаНастроек", ИмяФайлаНастроек) И ЗначениеЗаполнено(ИмяФайлаНастроек) Тогда
			Если Пользователи.ЭтоПолноправныйПользователь(, Ложь, Ложь) = Истина Тогда
				...
				Попытка
					...
					ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Используются обработчики из файла настроек загрузки ""%1"".';
																		|en = 'Используются обработчики из файла настроек загрузки ""%1"".'"), ИмяФайлаНастроек));
				Исключение
					ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Не найден файл настроек загрузки ""%1"": %2.';
																		|en = 'Не найден файл настроек загрузки ""%1"": %2.'"), ИмяФайлаНастроек,
						РегламентированнаяОтчетностьКлиентСервер.СформироватьТекстСообщения(ИнформацияОбОшибке().Описание)));
...
Функция УзелУдовлетворяетВыражениюПредиката(Узел, МаксИндекс, Индекс, Знач Предикат)
	...
	Если НЕ ПустаяСтрока(Предикат) Тогда
		...
		Исключение
			...
			ОписаниеОшибки = ИнформацияОбОшибке().Описание;
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'Не удалось вычислить выражение XPath для дерева загрузки! %1';
								|en = 'Не удалось вычислить выражение XPath для дерева загрузки! %1'"), ОписаниеОшибки));
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка данных регламентированного отчета';
											|en = 'Загрузка данных регламентированного отчета'"),
				УровеньЖурналаРегистрации.Ошибка,,, ОписаниеОшибки);
