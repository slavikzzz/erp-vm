...
Функция РасшифроватьПереборомСертификатов(ЗашифрованныеДанные, ТипШифрования = "CMS", ПараметрыШифрования = Неопределено) Экспорт
	...
	Если СвойстваСообщения.Тип = "envelopedData" Тогда
		...
		КонецЦикла;
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Недопустимое значение параметра %1 - файл не является криптосообщением';
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(Сертификат) Тогда
			ВызватьИсключение(НСтр("ru = 'Сертификат подписанта не найден в хранилище сертификатов.';
									|en = 'Signer certificate is not found in the certificate storage.'"));
		КонецЕсли;
...
Функция ВызватьHTTPМетод(Соединение, МетодHTTP, Запрос)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Сервис криптографии.Выполнение запроса';
				|en = 'Digital signature in SaaS.Cryptography service.Query execution'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Процедура ЗашифроватьПроверкаВходящихПараметров(Данные, Получатели, ТипШифрования, ПараметрыШифрования)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		ТипШифрования = "CMS",
		СтрШаблон(НСтр("ru = 'Недопустимое значение параметра %1 (неизвестный тип шифрования)';
	...
	Для Каждого Элемент Из Данные Цикл
		...
		Если ТипЗнч(Элемент) = Тип("Строка") Тогда
			ОбщегоНазначенияКлиентСервер.Проверить(
				...
				СтрШаблон(НСтр("ru = 'Недопустимое значение параметра Данные[%1] (указан адрес, который не является адресом временного хранилища)';
								|en = 'Invalid value of the Data [%1] parameter (the specified address is not a temporary storage address).'"), Индекс), 
				"СервисКриптографии.Зашифровать");
		КонецЕсли;
		Индекс = Индекс + 1;
	КонецЦикла;
...
Процедура РасшифроватьПроверкаВходящихПараметров(ЗашифрованныеДанные, ТипШифрования, ПараметрыШифрования)
	...
	Если ТипЗнч(ЗашифрованныеДанные) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			СтрШаблон(НСтр("ru = 'Недопустимое значение параметра %1 (указан адрес, который не является адресом временного хранилища)';
...
Процедура РасшифроватьБлокПроверкаВходящихПараметров(ЗашифрованныеДанные, Получатель, КлючеваяИнформация, ПараметрыШифрования)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		СтрШаблон(НСтр("ru = 'Отсутствует одно и/или более обязательных свойств параметра %1 (%2)';
						|en = 'One or more required properties of the %1 (%2) parameter are missing'"),
			"КлючеваяИнформация", "ephemeral_key|session_key|iv_data"),
		"СервисКриптографии.РасшифроватьБлок");
...
Процедура ХешированиеДанныхПроверкаВходящихПараметров(Данные, АлгоритмХеширования, ПараметрыХеширования)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		СтрШаблон(НСтр("ru = 'Недопустимое значение параметра %1 (неизвестный алгоритм хеширования)';
						|en = 'Incorrect value of the %1 parameter (unknown hash algorithm)'"),
			"АлгоритмХеширования"),
		"СервисКриптографии.ХешированиеДанных");
...
Процедура ПодписатьПроверкаВходящихПараметров(Данные, Подписант, ТипПодписи, ПараметрыПодписания)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		СтрШаблон(НСтр("ru = 'Недопустимое значение параметра %1 (неизвестный тип подписи)';
						|en = 'Incorrect value of the %1 parameter (unknown signature type)'"),
			"ТипПодписи"),
		"СервисКриптографии.Подписать");
...
Процедура ПолучитьВременныйПарольПроверкаВходящихПараметров(ИдентификаторСертификата, ПовторнаяОтправка, СпособДоставкиПаролей)
	...
	Если ТипЗнч(СпособДоставкиПаролей) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			СтрШаблон(НСтр("ru = 'Недопустимое значение параметра %1, допустимые значения ""%2"" или ""%3""';
							|en = 'Incorrect value of the %1 parameter, possible values are ""%2"" or ""%3""'"),
				"СпособДоставкиПаролей", "phone", "email"),
			"СервисКриптографии.ПолучитьВременныйПароль");
	КонецЕсли;
...
Функция ИмяСобытияШифрование()
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Сервис криптографии.Шифрование';
				|en = 'Cryptography service.Encryption'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИмяСобытияШифрованиеБлока()
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Сервис криптографии.Шифрование блока';
				|en = 'Cryptography service.Block encryption'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИмяСобытияРасшифровка()
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Сервис криптографии.Расшифровка';
				|en = 'Cryptography service.Decryption'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИмяСобытияРасшифровкаБлока()
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Сервис криптографии.Расшифровка блока';
				|en = 'Cryptography service.Block decryption'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИмяСобытияАутентификация()
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Сервис криптографии.Аутентификация';
				|en = 'Cryptography service.Authentication'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИмяСобытияСвойстваКриптосообщения()
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Сервис криптографии.Свойства криптосообщения';
				|en = 'Cryptography service.Crypto message properties'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИмяСобытияПроверкаСертификата()
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Сервис криптографии.Проверка сертификата';
				|en = 'Cryptography service.Certificate check'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИмяСобытияСвойстваСертификата()
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Сервис криптографии.Свойства сертификата';
				|en = 'Cryptography service.Certificate properties'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИмяСобытияПроверкаПодписи()
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Сервис криптографии.Проверка подписи';
				|en = 'Cryptography service.Signature check'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИмяСобытияХеширование()
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Сервис криптографии.Хеширование';
				|en = 'Cryptography service.Hashing'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИмяСобытияПодписание()
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Сервис криптографии.Подписание';
				|en = 'Cryptography service.Signing'", ОбщегоНазначения.КодОсновногоЯзыка());
