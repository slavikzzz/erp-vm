	...
	Для Каждого Макет Из Макеты Цикл // Структура
		...
		Попытка
			...
			Исключение
				...
				|%1';
				|en = 'Cannot create a new label template:
...
Функция КонвертироватьШаблонВXML(Макет, ЭтоКонвертацияШаблона = Ложь) Экспорт
	...
	Попытка
		...
		XMLТекстНовогоШаблона = ОборудованиеПринтерыЭтикеток.СохранитьXMLФайл(Окр(ШиринаМакета/КоличествоПикселейНаЕдиницуШирины), 
