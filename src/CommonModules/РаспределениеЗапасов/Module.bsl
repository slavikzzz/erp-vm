...
Процедура ВыполнитьРаспределениеВФоне(
	...
	Если НаборЗаписанЦеликом Тогда
		...
		Если ДинамическаяСтруктураЗаказовНаПроизводство Тогда
			...
			|			РаспределениеЗапасовПередЗаписью.ДатаПоступления    КАК ДатаПоступления,
			...
			|					ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе),
			|					ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОжидаемоеПоступление),
			|					ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ПоступлениеНеПодтвержденное))) КАК ТаблицаИзменений
			|
			|	СГРУППИРОВАТЬ ПО
			...
			|
			|	ИМЕЮЩИЕ
			|		СУММА(ТаблицаИзменений.Излишек) <> 0) КАК ТаблицаЗаданий";
		...
		Если ДинамическаяСтруктураЗаказовНаПроизводство И Не Результаты[0].Пустой() Тогда
...
Процедура РаспределениеЗапасовНаЗаказыВыбраннойОчереди(Заказы, Приоритет, МенеджерВременныхТаблиц) Экспорт
	...
	ВременнаяТаблицаЗапасыИДоступностьПоДатам(Запрос);
	ВременнаяТаблицаРезультатРаспределенияЗапасовНаПотребности(Запрос);
	Набор = ДанныеДляЗаписиТоваровВПроизводительномРежиме(Запрос, "ТоварыДляРаспределенияЗапасов", "", Ложь);
	...
	Для Каждого ОшибкаКонтроля Из РезультатыКонтроля Цикл
		...
		Если Количество > 0 Тогда
			...
			ТекстНазначениеХарактеристика = НСтр("ru = 'Для отгрузки/резервирования товара ""%1"",""%2"" на складе ""%3"" по назначению ""%4"" недостаточно свободного остатка в количестве %5 %6.';
												|en = 'Insufficient inventory available at warehouse ""%3"" to destination ""%4"" to ship/reserve goods ""%1"",""%2"" in the amount of %5 %6.'");
			ТекстНазначение = НСтр("ru = 'Для отгрузки/резервирования обособленного товара ""%1"" на складе ""%2"" по назначению ""%3"" недостаточно свободного остатка в количестве %4 %5';
									|en = 'Insufficient inventory available at warehouse ""%2"" to destination ""%3"" to ship/reserve assigned goods ""%1"", in the amount of %4 %5'");
			ТекстБезНазначенияХарактеристика = НСтр("ru = 'Для отгрузки/резервирования необособленного товара ""%1"",""%2"" на складе ""%3"" недостаточно свободного остатка в количестве %4 %5.';
													|en = 'Insufficient inventory available at warehouse ""%3"" to ship/reserve non-assigned goods ""%1"",""%2"" in the amount of %4 %5.'");
			ТекстБезНазначения = НСтр("ru = 'Для отгрузки/резервирования необособленного товара ""%1"" на складе ""%2"" недостаточно свободного остатка в количестве %3 %4';
										|en = 'Insufficient inventory available at warehouse ""%2"" to ship/reserve non-assigned goods ""%1"" in the amount of %3 %4'");
