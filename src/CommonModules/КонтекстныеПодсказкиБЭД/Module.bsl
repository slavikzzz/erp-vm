...
Процедура КонтекстныеПодсказки_ПриСозданииНаСервере(Форма, МестоРазмещенияПанели = Неопределено, 
	...
	Если Не МеханизмПодсказокПоддерживается(Форма.ИмяФормы) Тогда
		ВызватьИсключение НСтр("ru = 'Имя формы не добавлено в метод КонтекстныеПодсказкиБЭД.ФормыСПоддержкойКонтекстныхПодсказок';
								|en = 'The form name it not added to the КонтекстныеПодсказкиБЭД.ФормыСПоддержкойКонтекстныхПодсказок method'");
	КонецЕсли;
...
Функция ФормыСПоддержкойКонтекстныхПодсказок()
	...
	СписокФорм.Вставить("Документ.ТранспортныйКонтейнерЭДО.Форма.ФормаДокумента", "Документ.ТранспортныйКонтейнерЭДО");
	СписокФорм.Вставить("Документ.ЭлектронныйДокументИсходящийЭДО.Форма.ФормаПросмотра", "Документ.ЭлектронныйДокументИсходящийЭДО");
	СписокФорм.Вставить("Документ.ЭлектронныйДокументВходящийЭДО.Форма.ФормаПросмотра", "Документ.ЭлектронныйДокументВходящийЭДО");
	СписокФорм.Вставить("Обработка.ИнтерфейсДокументовЭДО.Форма.АрхивЭлектронныхДокументов", "Обработка.ИнтерфейсДокументовЭДО");
	СписокФорм.Вставить("Обработка.НастройкиЭДО.Форма.НастройкаОбменаСКонтрагентом", "Обработка.НастройкиЭДО");
	СписокФорм.Вставить("Обработка.НастройкиЭДО.Форма.НастройкиЭДО", "Обработка.НастройкиЭДО");
	СписокФорм.Вставить("Обработка.ИнтерфейсДокументовЭДО.Форма.СписокПроизвольныхЭлектронныхДокументов", "Обработка.ИнтерфейсДокументовЭДО");
	СписокФорм.Вставить("Обработка.ИнтерфейсДокументовЭДО.Форма.ТекущиеДелаПоЭДО", "Обработка.ИнтерфейсДокументовЭДО");
...
Процедура ЗадатьЗначениеСчетчикаКоличестваНепрочитанныхСтатей(Форма)
	...
	Если КоличествоНовостейДляСчетчика > 0 Тогда
		...
		ШаблонСтроки = НСтр("ru = ';%1 совет;;%1 совета;%1 советов;%1 советов';
							|en = ';%1 tip;;%1 tips;%1 tips;%1 tips'");
		Представление = СтрокаСЧислом(ШаблонСтроки, КоличествоНовостейДляСчетчика, ВидЧисловогоЗначения.Количественное);
			
		Форма.Элементы.ПанельКонтекстныхНовостей_СчетчикНепрочитанных.Заголовок = НСтр("ru = 'еще';
																						|en = 'more'") + " " + Представление;		
...
Процедура ДобавитьПодключаемыеКоманды(Форма, МестаРазмещения)
	...
	ДобавитьКоманду(Форма, НСтр("ru = 'Все советы';
								|en = 'All tips'"), "КомандаПоказатьВсеНовости");
	ДобавитьКоманду(Форма, НСтр("ru = 'Советы для текущего состояния';
								|en = 'Tips for current state'"), "КомандаПоказатьВсеКонтекстныеНовостиНовости");  
	ДобавитьКоманду(Форма, НСтр("ru = 'Панель советов';
								|en = 'Tips panel'"), "ВидимостьПанелиКонтекстныхНовостей");
...
Функция Подключаемый_ДоступныеКатегорииФормы(ПолноеИмяФормы) Экспорт
	...
	Если ПолноеИмяФормы = "Документ.ТранспортныйКонтейнерЭДО.Форма.ФормаДокумента" Тогда
		ДоступныеКатегории.Добавить(КонтекстныеПодсказкиБЭДКатегоризацияВызовСервера.Категория_ОператорАбонента());
	...
	ИначеЕсли ПолноеИмяФормы = "Документ.ЭлектронныйДокументВходящийЭДО.Форма.ФормаПросмотра" Тогда
	...
	ИначеЕсли ПолноеИмяФормы = "Документ.ЭлектронныйДокументИсходящийЭДО.Форма.ФормаПросмотра" Тогда
		ДоступныеКатегории.Добавить(КонтекстныеПодсказкиБЭДКатегоризацияВызовСервера.Категория_СтатусДокументооборота());
		ДоступныеКатегории.Добавить(КонтекстныеПодсказкиБЭДКатегоризацияВызовСервера.Категория_ВидЭлектронногоДокумента());
		ДоступныеКатегории.Добавить(КонтекстныеПодсказкиБЭДКатегоризацияВызовСервера.Категория_ОператорАбонента());
		ДоступныеКатегории.Добавить(КонтекстныеПодсказкиБЭДКатегоризацияВызовСервера.Категория_КодОператораУчетнойЗаписиОрганизации());
		ДоступныеКатегории.Добавить(КонтекстныеПодсказкиБЭДКатегоризацияВызовСервера.Категория_СуществуютНеверныеПодписиФайла());
	ИначеЕсли ПолноеИмяФормы = "Обработка.НастройкиЭДО.Форма.НастройкаОбменаСКонтрагентом" Тогда
		ДоступныеКатегории.Добавить(КонтекстныеПодсказкиБЭДКатегоризацияВызовСервера.Категория_КодОператораУчетнойЗаписиОрганизации());
		ДоступныеКатегории.Добавить(КонтекстныеПодсказкиБЭДКатегоризацияВызовСервера.Категория_ОператорАбонента());
