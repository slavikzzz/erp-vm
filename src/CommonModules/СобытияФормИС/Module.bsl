...
Процедура ПриОпределенииПараметровИнтеграцииФормы(Форма, МодулиМодификацииФормыГосИС)
	...
	Если Форма.ИмяФормы = "Документ.ОтчетОПроизводствеЕГАИС.Форма.ФормаДокумента"
		Или Форма.ИмяФормы = "Документ.ОтчетОбИмпортеЕГАИС.Форма.ФормаДокумента"
		Или Форма.ИмяФормы = "Документ.ПеремещениеМеждуМОДИСМП.Форма.ФормаДокумента" Тогда
...
Процедура ЗаполнениеРеквизитовГосИС(Форма)
	...
	МодулиЗаполнения = Новый СписокЗначений;
	Для Каждого КлючИЗначение Из Форма.ПараметрыИнтеграцииГосИС Цикл
		Если ТипЗнч(КлючИЗначение.Значение) = Тип("Структура") И КлючИЗначение.Значение.Свойство("МодульЗаполнения") Тогда
			ИмяМодуляЗаполнения = КлючИЗначение.Значение.МодульЗаполнения;
			Если ЗначениеЗаполнено(ИмяМодуляЗаполнения) И МодулиЗаполнения.НайтиПоЗначению(ИмяМодуляЗаполнения) = Неопределено Тогда
				МодулиЗаполнения.Добавить(ИмяМодуляЗаполнения);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	МодулиЗаполнения.СортироватьПоЗначению();
	Для Каждого ЭлементСписка Из МодулиЗаполнения Цикл
		МодульЗаполнения = ОбщегоНазначения.ОбщийМодуль(ЭлементСписка.Значение);
...
Процедура ПодключитьФоновоеЗаданиеРасчетаСтатусовОформления(Форма)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Расчет статусов оформления документа-основания.';
															|en = 'Расчет статусов оформления документа-основания.'");
