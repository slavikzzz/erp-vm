...
Функция ПолучитьПартнераПоКонтрагенту(Контрагент, ХозяйственнаяОперация = Неопределено) Экспорт
	...
	РезультатЗапроса =
		РезультатЗапросаПартнерыКонтрагентовПоОперации(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Контрагент), ХозяйственнаяОперация);
