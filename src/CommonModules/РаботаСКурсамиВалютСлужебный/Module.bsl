...
Процедура ЗагрузитьКурсы() Экспорт
	...
	Если Дескрипторы.Descriptor.Количество() < 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			...
			|en = 'The service manager has no data of type ""%1""'"), "КурсыВалют");
	КонецЕсли;
...
Процедура КопироватьКурсыВалюты(Знач КодВалюты) Экспорт
	...
	Если ВалютаСсылка.Пустая() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Валюта с кодом %1 не найдена в справочнике, загрузка курсов отменена.';
																					|en = 'Currency with code %1 is not found in the catalog. Exchange rate import is canceled.'"), КодВалюты);
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Поставляемые данные.Распространение курсов валют по областям данных';
										|en = 'Default master data.Distribution of exchange rates in data areas'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если Курсы.Количество() = 0 Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отсутствуют курсы для валюты с кодом %1 в поставляемых данных.';
																					|en = 'There are no exchange rates for the currency with code %1 in the default master data.'"), КодВалюты);
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Поставляемые данные.Распространение курсов валют по областям данных';
										|en = 'Default master data.Distribution of exchange rates in data areas'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ТекстОшибки);
	...
	Пока ВыборкаВалюты.Следующий() Цикл
		...
		Пока ВыборкаОбщий.Следующий() Цикл
			...
			Иначе
				...
				ИмяСобытия = НСтр("ru = 'Поставляемые данные.Отмена загрузки курсов валюты';
									|en = 'Default master data.Cancel exchange rates import'", ОбщегоНазначения.КодОсновногоЯзыка());
