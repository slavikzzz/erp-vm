...
Функция СвойстваВидаПриложенияПоИмени(Имя, ВызыватьИсключение = Истина) Экспорт
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не найден вид приложения по имени ""%1""';
				|en = 'Cannot find the application type by name ""%1""'"), Имя);
	ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Найдено несколько видов приложений по имени ""%1""';
				|en = 'Multiple application types are found by name ""%1""'"), Имя);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТекстСообщения) Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Виды приложений. Поиск вида приложения';
				|en = 'Application types. Search for application type'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не найден вид приложения по списку услуг: %1';
				|en = 'Cannot find the application type by service list: %1'"),
			ПредставлениеСпискаУслуг(Услуги, Символы.ПС + "- "));
	ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Найдено несколько видов приложений по списку услуг:%1';
				|en = 'Multiple application types are found by service list: %1'"),
			ПредставлениеСпискаУслуг(Услуги, Символы.ПС + "- "));
	КонецЕсли;
...
Функция НовыйПараметрыУведомленияОЗавершенииПерехода() Экспорт
	...
	ПараметрыУведомления.Вставить("Заголовок", НСтр("ru = 'Выполнен переход на новый вид приложения';
													|en = 'Migration to a new application type is successfully completed'"));
