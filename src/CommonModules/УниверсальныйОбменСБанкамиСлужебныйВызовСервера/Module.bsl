			...
			Исключение
				ИмяСобытия = ?(НастройкиВызова.ЭтоЭлектроннаяПодписьВМоделиСервиса,
					НСтр("ru = 'Электронная подпись в модели сервиса.Сервис криптографии.Чтение сертификата';
						|en = 'Digital signature in SaaS mode.Cryptography service.Reading certificate'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					НСтр("ru = 'Сервис криптографии.Чтение сертификата';
						|en = 'Cryptography service.Reading certificate'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	...
	Исключение
		ИмяСобытия = ?(НастройкиВызова.ЭтоЭлектроннаяПодписьВМоделиСервиса,
			НСтр("ru = 'Электронная подпись в модели сервиса.Сервис криптографии.Чтение PKCS7';
				|en = 'Digital signature in SaaS mode.Cryptography service.Reading PKCS7'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			НСтр("ru = 'Сервис криптографии.Чтение PKCS7';
				|en = 'Cryptography service.Reading PKCS7'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(
			ИмяСобытия,
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПрочитатьIssuerAndSerialNumber(ЧтениеДанных, СвойстваКриптосообщения, КонецБлока)
	...
	Пока ЧтениеДанных.ИсходныйПоток().ТекущаяПозиция() < Граница Цикл
		...
		Имя = СоответствиеOIDИмя.Получить(Идентификатор);
...
Функция СоответствиеOIDИмя()
	...
	СоответствиеOID.Вставить("1.2.643.3.131.1.1", "INN"); // ИНН физического лица, до 01.09.2021 использовалось и для юридического лица
	СоответствиеOID.Вставить("1.2.643.100.4",     "INNLE"); // ИНН юридического лица с 01.09.2021
