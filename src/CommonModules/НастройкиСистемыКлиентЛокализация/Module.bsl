...
Процедура ПриИзмененииРеквизита_Производство(Элемент, Форма) Экспорт
	...
	Если Элемент.Имя = "ИспользоватьПроизводство21" Тогда
		...
		Если ТребуетсяВывестиПредупреждениеПриОтключении21(Форма) Тогда
			...
			Элемент = Новый Структура("Имя", Неопределено);
			Возврат;
...
Процедура УчетВнеоборотныхАктивовПриИзменении(Форма, Отказ) Экспорт
	...
	ИначеЕсли Форма.УчетВнеоборотныхАктивов = "2_2" И Форма.НаличиеУчета.ИспользуетсяУУНаПланеСчетовХозрасчетный Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для включения учета версии 2.2 необходимо выключить опцию ""Управленческий учет на плане счетов регламентированного учета"" (раздел ""Регламентированный учет"" - ""Отражение операций"")';
								|en = 'To enable accounting 2.2, disable the ""Management accounting on the chart of accounts of local accounting"" option (in ""Local accounting"" - ""Record operations"") '");
	...
	ИначеЕсли Форма.УчетВнеоборотныхАктивов = ""
		...
		ТекстСообщения = НСтр("ru = 'Для отключения учета необходимо выключить опцию ""Регламентированный учет"" (раздел ""Регламентированный учет"")';
								|en = 'To disable the accounting, clear the ""Local accounting"" check box (section ""Local accounting"")'");
	...
	ИначеЕсли Форма.УчетВнеоборотныхАктивов = "2_2" И Форма.НаборКонстант.ИспользоватьВнеоборотныеАктивы2_4 Тогда
		...
		Если Форма.НаличиеУчета.ЕстьУчет2_4 Тогда
			ТекстВопроса = НСтр("ru = 'Отключать учет внеоборотных активов версии 2.4 после начала работы с системой не рекомендуется.
				                 |Продолжить редактирование?';
				                 |en = 'It is not recommended that you disable capital asset accounting 2.4 after you start working with the application.
				                 |Continue editing? '");
		КонецЕсли;
