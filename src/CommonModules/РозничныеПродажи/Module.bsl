	...
	Если Выборка.Следующий() Тогда
		ПокупательПерсональныеДанные = СокрЛП(Выборка.Серия) + " " + СокрЛП(Выборка.Номер) + " " + СокрЛП(Выборка.КемВыдан)
			+ " " + Формат(Выборка.ДатаВыдачи, НСтр("ru = 'ДФ=dd.MM.yyyy';
													|en = 'DF=MM/dd/yyyy'")) + " " + Формат(Выборка.СрокДействия, НСтр("ru = 'ДФ=dd.MM.yyyy';
																													|en = 'DF=MM/dd/yyyy'"));
	КонецЕсли;
...
Функция ЗаписатьФискальнуюОперациюНаСервере(Форма, ПараметрыФискализации, ПараметрыЗаписи) Экспорт
	...
	Попытка
		...
		Если Не УспешноЗаписанДокумент Тогда
			Если ЭтоЧекККМКоррекции Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось записать документ: %1';
																								|en = 'Cannot save the document: %1'"), Объект.Ссылка);
				ВызватьИсключение ОписаниеОшибки;
			Иначе
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Данные фискальной операции записаны, не удалось записать документ: %1';
																								|en = 'Fiscal transaction data is not saved. Cannot save the document: %1'"), Объект.Ссылка);
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Розничные продажи';
												|en = 'Retail sales'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , ОписаниеОшибки);
			КонецЕсли;
		КонецЕсли;
	...
	Исключение
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ОписаниеОшибки, Объект.Ссылка, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Розничные продажи';
										|en = 'Retail sales'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , ОписаниеОшибки);
	...
	Для Каждого СтрокаТЧ Из Объект.ОплатаПлатежнымиКартами Цикл
		...
		Если (ТипЗнч(Объект) = Тип("ДокументОбъект.ОтчетОРозничныхВозвратах")
			...
			Текст = НСтр("ru = 'В строке %НомерСтроки% табличной части ""%ИмяТабличнойЧасти%"" необходимо указать вид оплаты.';
						|en = 'In line %НомерСтроки% of the %ИмяТабличнойЧасти% table, enter the payment kind.'");
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.ЭквайринговыйТерминал)
			...
			Текст = НСтр("ru = 'В строке %НомерСтроки% табличной части ""%ИмяТабличнойЧасти%"" необходимо указать эквайринговый терминал.';
						|en = 'In line %НомерСтроки% of the %ИмяТабличнойЧасти% table, enter the POS terminal.'");
			Текст = СтрЗаменить(Текст, "%НомерСтроки%",       СтрокаТЧ.НомерСтроки);
			Текст = СтрЗаменить(Текст, "%ИмяТабличнойЧасти%", ИмяТабличнойЧастиОплатаПлатежнымиКартами);
		...
		Если ТипЗнч(Объект) = Тип("ДокументОбъект.ЧекККМ") 
			...
			Если Не ЗначениеЗаполнено(СтрокаТЧ.ДоговорПодключения)
				...
				Текст = НСтр("ru = 'В строке %НомерСтроки% табличной части ""%ИмяТабличнойЧасти%"" необходимо указать договор подключения.';
							|en = 'In line %НомерСтроки% of the %ИмяТабличнойЧасти% table, enter the payment system contract.'");
				Текст = СтрЗаменить(Текст, "%НомерСтроки%",       СтрокаТЧ.НомерСтроки);
				Текст = СтрЗаменить(Текст, "%ИмяТабличнойЧасти%", ИмяТабличнойЧастиОплатаПлатежнымиКартами);
...
Процедура АрхивированиеЧековККМ() Экспорт
	...
	Если НЕ Результат.Пустой() Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Архивирование чеков ККМ';
										|en = 'Archive receipts'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация, , ,
			НСтр("ru = 'Начато регламентное архивирование чеков ККМ.';
				|en = 'Scheduled archiving of receipts has started.'"));
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Архивирование чеков ККМ';
			...
			НСтр("ru = 'Закончено регламентное архивирование чеков ККМ.';
				|en = 'Scheduled archiving of receipts is finished.'"));
	КонецЕсли;
...
Процедура УдалениеЧековККМ() Экспорт
	...
	Если НЕ Результат.Пустой() Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление чеков ККМ';
										|en = 'Delete receipts'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация, , ,
			НСтр("ru = 'Начато регламентное удаление чеков ККМ.';
				|en = 'Scheduled removal of receipts has started.'"));
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление чеков ККМ';
			...
			НСтр("ru = 'Закончено регламентное удаление чеков ККМ.';
				|en = 'Scheduled deletion of receipts is finished.'"));
...
Процедура УдалениеОтложенныхЧековККМ() Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление отложенных чеков ККМ';
									|en = 'Deletion of held receipts'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	                         УровеньЖурналаРегистрации.Информация, , ,
	                         НСтр("ru = 'Начато регламентное удаление отложенных чеков ККМ.';
									|en = 'Scheduled removal of held receipts has started.'"));
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление отложенных чеков ККМ';
		                         ...
		                         НСтр("ru = 'Во время регламентного удаления отложенных чеков ККМ произошла ошибка.';
										|en = 'An error occurred during scheduled removal of held receipts.'")
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление отложенных чеков ККМ';
	                         ...
	                         НСтр("ru = 'Закончено регламентное удаление отложенных чеков ККМ.';
									|en = 'Scheduled removal of held receipts is completed.'"));
...
//  ВключатьНепроведенныеЧеки - Булево - По умолчанию - ложь. Будут обработаны проведенные и непроведенные чеки непомеченные на удаление
//
Процедура ВыполнитьАрхивациюЧековККМ(ПараметрыЗадания, АдресХранилища="", ВключатьНепроведенныеЧеки = Ложь) Экспорт
	...
	|	НЕ ЧекККМ.Архивный
	|	И (ЧекККМ.Проведен
	|		ИЛИ &ВключатьНепроведенныеЧеки И НЕ ЧекККМ.ПометкаУдаления)
	...
	|		ИЛИ &ВключатьНепроведенныеЧеки И НЕ ЧекККМВозврат.ПометкаУдаления)
	...
	|		ИЛИ &ВключатьНепроведенныеЧеки И НЕ ЧекККМКоррекции.ПометкаУдаления)
		...
		Попытка
			...
			Пока ВыборкаДокументов.Следующий() Цикл
				...
				ИначеЕсли Не ТипЗнч(ВыборкаДокументов.Ссылка) = Тип("ДокументСсылка.ОтчетОРозничныхВозвратах") Тогда
					Отказ = Истина;
					ОписаниеОшибки = НСтр("ru = 'По документу %КассоваяСмена% не создано документов %Синоним%';
											|en = 'Documents %Синоним% failed to be created by document %КассоваяСмена%'",  ОбщегоНазначения.КодОсновногоЯзыка());
					...
					ЗаписьЖурналаРегистрации(НСтр("ru = 'Архивирование чеков ККМ';
													|en = 'Archive receipts'", ОбщегоНазначения.КодОсновногоЯзыка()),
							УровеньЖурналаРегистрации.Ошибка, , ,
							НСтр("ru = 'Во время регламентного архивирования чеков ККМ произошла ошибка.';
								|en = 'An error occurred during scheduled archiving of the receipts.'", ОбщегоНазначения.КодОсновногоЯзыка())
		...
		Исключение
			...
			Если ПараметрыЗадания.ПоказатьОшибкуПользователю Тогда
				ОписаниеОшибки = НСтр("ru = 'При архивации чеков ККМ произошла ошибка.
			                      ...
			                      |en = 'An error occurred when archiving the receipt.
			                      |Receipt archiving is not completed.
			                      |Additional details:
			                      |%ДополнительноеОписание%'");
				ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
...
//  ПараметрыЗадания  - см. НовыйПараметрыЗаданияЗакрытияКассовойСмены
//  АдресХранилища - Строка - Адрес во временном хранилище.
//  ВключатьНепроведенныеЧеки - Булево - По умолчанию - ложь. При выполнении операции будут удалены непроведенные чеки, в том числе помеченные на удаление.
//
	...
	Пока ВыборкаКассовыхСмен.Следующий() Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Удаление чеков ККМ';
					|en = 'Delete receipts'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка, , ,
				ПодробноеПредставлениеОшибки);
			Если ТранзакцияАктивна() Тогда
				ВызватьИсключение ПодробноеПредставлениеОшибки;
			...
			Если ПараметрыЗадания.ПоказатьОшибкуПользователю Тогда
				ШаблонОшибки = НСтр("ru = 'При удалении чеков ККМ произошла ошибка.
			                      ...
			                      |Дополнительное описание:
			                      |%ДополнительноеОписание%';
			                      |en = 'An error occurred while removing receipts. 
...
Функция ЗакрытьКассовуюСмену(Объект, ОписаниеОшибки = "") Экспорт
	...
	Если СтруктураСостояниеКассовойСмены.СтатусКассовойСмены = Перечисления.СтатусыКассовойСмены.Открыта
		...
		Иначе
			...
			ОписаниеОшибки = НСтр("ru = 'При закрытии кассовой смены произошла ошибка.
			                            ...
			                            |en = 'An error occurred when closing the POS shift.
			                            |Detailed description:
			                            |Date of the POS shift closing is less than the date of the last POS shift status change.'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(СтруктураСостояниеКассовойСмены.КассоваяСмена) Тогда
		...
		|en = 'An error occurred upon the POS shift-end closing.
		|More information:
		|No POS shift is found.'");
...
Функция ЗаполнитьОтчетОРозничныхПродажах(ОтчетОРозничныхПродажахОбъект, СостояниеКассовойСмены, НалогообложениеНДС, МенеджерВременныхТаблиц, ТолькоЗаполнить = Ложь, ОписаниеОшибки = "") Экспорт
	...
	Попытка
		...
		|		СписокТоваров.ЧекККМКассоваяСмена) КАК СписокТоваров
		...
		|	ГДЕ
		|		ЧекККМВозвратОплатаПлатежнымиКартами.ОплатаОтменена) КАК ОплатаПлатежнымиКартами
		|		ЛЕВОЕ СОЕДИНЕНИЕ ИндексыПоАналитике КАК ИндексыПоАналитике
		...
		|	ГДЕ
		|		ЧекиККМ.ПоЧекуКоррекции = ЛОЖЬ) КАК ПодарочныеСертификаты
		|		ЛЕВОЕ СОЕДИНЕНИЕ ИндексыПоАналитике КАК ИндексыПоАналитике
		...
		Если Не ТолькоЗаполнить Тогда
			...
			Иначе
				ОписаниеОшибки = НСтр("ru = 'При проверке заполнения перед проведением документа %Документ% по кассовой смене %КассоваяСмена% произошла ошибка. Документ записан, но не проведен.';
										|en = 'An error occurred when posting the %Документ% document for the %КассоваяСмена% POS shift. The document is saved but not posted.'");
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ru = 'При создании отчета о розничных продажах по кассовой смене %КассоваяСмена% произошла ошибка.
		                            |Дополнительное описание:
		                            |%ДополнительноеОписание%';
		                            |en = 'An error occurred when generating the retail sales report for the %КассоваяСмена% POS shift.
		                            |Additional details:
		                            |%ДополнительноеОписание%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%КассоваяСмена%", СостояниеКассовойСмены.КассоваяСмена);
...
Функция ЗаполнитьОтчетОРозничныхВозвратах(ОтчетОРозничныхВозвратахОбъект, СостояниеКассовойСмены, НалогообложениеНДС, МенеджерВременныхТаблиц, ТолькоЗаполнить = Ложь, ОписаниеОшибки = "") Экспорт
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ru = 'При создании отчета о розничных возвратах по кассовой смене %КассоваяСмена% произошла ошибка.
		                            |Дополнительное описание:
		                            |%ДополнительноеОписание%';
		                            |en = 'An error occurred when generating the retail return report for the %КассоваяСмена% POS shift.
		                            |Additional details:
		                            |%ДополнительноеОписание%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%КассоваяСмена%", СостояниеКассовойСмены.КассоваяСмена);
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
//  СостояниеКассовойСмены - см. ОписаниеКассовойСмены
//	МенеджерВременныхТаблиц - МенеджерВременныхТаблиц -
//  ВключатьНепроведенныеЧеки - Булево - По умолчанию - ложь. Будут подобраны проведенные и непроведенные чеки непомеченные на удаление.
//
// Возвращаемое значение:
...
Функция ПодготовитьДанныеПоЧекамКассовойСмены(СостояниеКассовойСмены, МенеджерВременныхТаблиц, ВключатьНепроведенныеЧеки = Ложь) Экспорт
	...
	Запрос.Текст = "
		|ВЫБРАТЬ
		|	ЧекККМТовары.Ссылка КАК Ссылка,
		|	ЧекККМТовары.Ссылка.ЧекККМ КАК ЧекККМ,
		|	ЧекККМТовары.НоменклатураНабора КАК НоменклатураНабора,
		|	ЧекККМТовары.ХарактеристикаНабора КАК ХарактеристикаНабора,
		|	ЧекККМТовары.Номенклатура КАК Номенклатура,
		|	ЧекККМТовары.Характеристика КАК Характеристика,
		...
		|ПОМЕСТИТЬ ТоварыРозничныхВозвратовВСменуПродажи
		|ИЗ
		...
		|			ИЛИ &ВключатьНепроведенныеЧеки И НЕ ЧекККМТовары.Ссылка.ПометкаУдаления)
		...
		|	ЧекиККМ.КассаККМ КАК КассаККМ,
		|	ЧекиККМ.Организация КАК Организация,
		|	ЧекиККМ.НалогообложениеНДС КАК НалогообложениеНДС,
		|	ЧекиККМ.КассоваяСмена КАК КассоваяСмена
		|ПОМЕСТИТЬ ЧекиККМПоКоторымОформленВозвратВСмену
		|ИЗ
		|	ТоварыРозничныхВозвратовВСменуПродажи КАК ТоварыРозничныхПродаж
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧекККМ КАК ЧекиККМ
		|		ПО (ЧекиККМ.Ссылка = ТоварыРозничныхПродаж.ЧекККМ)
		|ГДЕ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СписокПроблемныхВозвратовВСмену.ЧекККМ КАК ЧекККМ
		|ПОМЕСТИТЬ ЧекиККМПоКоторымОформленыПроблемныеВозвратыВСмену
		|ИЗ
		|	(ВЫБРАТЬ
		|		СписокПродажВСмену.ЧекККМ КАК ЧекККМ,
		|		СписокПродажВСмену.НоменклатураНабора КАК НоменклатураНабора,
		|		СписокПродажВСмену.ХарактеристикаНабора КАК ХарактеристикаНабора,
		|		СписокПродажВСмену.Номенклатура КАК Номенклатура,
		|		СписокПродажВСмену.Характеристика КАК Характеристика,
		|		СписокПродажВСмену.Серия КАК Серия,
		|		СписокПродажВСмену.Упаковка КАК Упаковка,
		|		СписокПродажВСмену.Цена КАК Цена,
		|		СписокПродажВСмену.СтавкаНДС КАК СтавкаНДС,
		|		СУММА(СписокПродажВСмену.КоличествоУпаковок) КАК КоличествоУпаковок,
		|		СУММА(СписокПродажВСмену.Количество) КАК Количество,
		|		СУММА(СписокПродажВСмену.Сумма) КАК Сумма,
		|		СУММА(СписокПродажВСмену.СуммаНДС) КАК СуммаНДС,
		|		СписокПродажВСмену.Склад КАК Склад,
		|		СписокПродажВСмену.Партнер КАК Партнер,
		|		СписокПродажВСмену.Помещение КАК Помещение,
		|		СписокПродажВСмену.Продавец КАК Продавец,
		|		СписокПродажВСмену.Валюта КАК Валюта,
		|		СписокПродажВСмену.ВидЦены КАК ВидЦены,
		|		СписокПродажВСмену.КассаККМ КАК КассаККМ,
		|		СписокПродажВСмену.Организация КАК Организация,
		|		СписокПродажВСмену.НалогообложениеНДС КАК НалогообложениеНДС
		|	ИЗ
		|		(ВЫБРАТЬ
		...
		|			ЧекиККМ.НалогообложениеНДС КАК НалогообложениеНДС
		|		ИЗ
		|			ЧекиККМПоКоторымОформленВозвратВСмену КАК ЧекиККМ
		|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЧекККМ.Товары КАК ЧекККМТовары
		|				ПО ЧекиККМ.ЧекККМ = ЧекККМТовары.Ссылка
		|		
		|		ОБЪЕДИНИТЬ ВСЕ
		|		
		|		ВЫБРАТЬ
		...
		|			ЧекККМТовары.Склад,
		|			ЧекККМТовары.Партнер,
		|			ЧекККМТовары.Помещение,
		|			ЧекККМТовары.Продавец,
		|			ЧекККМТовары.Валюта,
		|			ЧекККМТовары.ВидЦены,
		|			ЧекККМТовары.КассаККМ,
		|			ЧекККМТовары.Организация,
		|			ЧекККМТовары.НалогообложениеНДС
		|		ИЗ
		|			ТоварыРозничныхВозвратовВСменуПродажи КАК ЧекККМТовары) КАК СписокПродажВСмену
		|	
		|	СГРУППИРОВАТЬ ПО
		|		СписокПродажВСмену.ЧекККМ,
		|		СписокПродажВСмену.НоменклатураНабора,
		|		СписокПродажВСмену.ХарактеристикаНабора,
		|		СписокПродажВСмену.Номенклатура,
		|		СписокПродажВСмену.Характеристика,
		|		СписокПродажВСмену.Серия,
		|		СписокПродажВСмену.Упаковка,
		|		СписокПродажВСмену.Цена,
		|		СписокПродажВСмену.СтавкаНДС,
		|		СписокПродажВСмену.Склад,
		|		СписокПродажВСмену.Партнер,
		|		СписокПродажВСмену.Помещение,
		|		СписокПродажВСмену.Продавец,
		|		СписокПродажВСмену.Валюта,
		|		СписокПродажВСмену.ВидЦены,
		|		СписокПродажВСмену.КассаККМ,
		|		СписокПродажВСмену.Организация,
		|		СписокПродажВСмену.НалогообложениеНДС
		|	
		|	ИМЕЮЩИЕ
		|		СУММА(СписокПродажВСмену.Количество) < 0) КАК СписокПроблемныхВозвратовВСмену
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЧекККМ
		|;
		...
		|	ЧекККМТовары.Помещение,
		|	ВЫБОР
		|		КОГДА ЧекиККМПоКоторымОформленыПроблемныеВозвратыВСмену.ЧекККМ ЕСТЬ NULL
		|			ТОГДА ЧекККМТовары.Продавец
		...
		|	НЕОПРЕДЕЛЕНО
		|ИЗ
		|	ТоварыРозничныхВозвратовВСменуПродажи КАК ЧекККМТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ ЧекиККМПоКоторымОформленыПроблемныеВозвратыВСмену КАК ЧекиККМПоКоторымОформленыПроблемныеВозвратыВСмену
		|		ПО ЧекККМТовары.ЧекККМ = ЧекиККМПоКоторымОформленыПроблемныеВозвратыВСмену.ЧекККМ
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|			ТОГДА ЧекККМТовары.Ссылка.НалогообложениеНДС
		...
		|	И ЧекККМКоррекции.КассоваяСмена = &КассоваяСмена
		|	И (ЧекККМКоррекции.Проведен
		|			ИЛИ &ВключатьНепроведенныеЧеки И НЕ ЧекККМКоррекции.Ссылка.ПометкаУдаления)
		|	И ЧекККМКоррекции.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробит)
		|	И ЧекККМКоррекции.Ссылка.ВидКоррекции = 0
		|;
		|
		...
		|ИЗ
		|	ЧекиККМНалогообложениеНДС КАК ЧекиККМНалогообложениеНДС
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах КАК ОтчетОРозничныхПродажах
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЧекиККМНалогообложениеНДС.НалогообложениеНДС КАК НалогообложениеНДС
		|ИЗ
		|	(ВЫБРАТЬ
		|		ВЫРАЗИТЬ(ВЫБОР
		...
		|		ЧекиККМ.Приоритет
		|	ИЗ
		|		ЧекиККМНалогообложениеНДС КАК ЧекиККМ) КАК ЧекиККМНалогообложениеНДС
		|
		|СГРУППИРОВАТЬ ПО
		|	ЧекиККМНалогообложениеНДС.НалогообложениеНДС
		|
		|УПОРЯДОЧИТЬ ПО
		|	МИНИМУМ(ЧекиККМНалогообложениеНДС.Приоритет)";
...
Функция ВыполнитьОперациюЗакрытияКассовойСмены(Объект, ОшибкаПриСнятииZОтчета, ОписаниеОшибки = "") Экспорт
	...
	Если Константы.ОперацияПриЗакрытииКассовойСмены.Получить() = Перечисления.ОперацииПриЗакрытииКассовойСмены.Нет Тогда
		ШаблонРекомендации = НСтр("ru = 'Согласно текущим настройкам при закрытии кассовой смены архивирование чеков не производится.
							...
							|en = 'According to the current settings, sales slips are not archived when the POS shift is closed.
							|This mode is not recommended and is no longer supported.
							|We recommend that you enable ""%1"" mode when closing the POS shift.
							|When archiving sales slips, all cash flows and changes in inventory balances, which are registered in the ""%2""
							| and ""%3"" documents during the shift, are aggregated in the ""%4"" and ""%5"" documents after shift-end closing.'");
	...
	Исключение
		...
		ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Закрытие кассовой смены';
										|en = 'POS shift-end closing'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки);
		...
		КраткоеПредставлениеОшибки = НСтр("ru = 'При закрытии кассовой смены произошла ошибка. Подробное описание находится в журнале регистрации.';
											|en = 'An error occurred upon the POS shift-end closing. For more information, see the event log.'");
...
Процедура СформироватьДокументыДляОперацииЗакрытияКассовойСмены(СостояниеКассовойСмены, ОписаниеОшибки, ВключатьНепроведенныеЧеки = Ложь) Экспорт
	...
	Если Не ПарыДокументовСформированы Тогда
		...
		КонецЕсли;
		МассивСтрокОписанияОшибки.Добавить(СтрШаблон(НСтр("ru = 'При формировании документов для операции закрытия кассовой смены %1 возникла ошибка.';
															|en = 'An error occurred when generating documents for the %1 POS shift-end closing operation.'"), СостояниеКассовойСмены.КассоваяСмена));
...
Функция ПолучитьТекстОшибкиЗаполненияОтчета()
	...
	Возврат НСтр("ru = 'Проверьте настройки розничного склада и кассы ККМ.';
				|en = 'Check settings of the retail warehouse and cash register.'");
...
Функция УдалитьОтложенныеЧеки(КассоваяСмена, ОписаниеОшибки)
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ru = 'При удалении отложенных чеков произошла ошибка.
		                            |Дополнительное описание:
		                            |%ДополнительноеОписание%';
		                            |en = 'An error occurred while removing deferred receipts. 
		                            |Additional description:
		                            |%ДополнительноеОписание%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ПодобратьЗаполнитьДокументРеализацииВСтроке(СтрокаТабличнойЧастиТовары, ТаблицаДокументовРеализации, СостояниеКассовойСмены, ВыборкаДетальныеЗаписи, СообщатьОбОшибке = Ложь)
	Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ЧекККМКассоваяСмена) 
		И ВыборкаДетальныеЗаписи.ЧекККМКассоваяСмена <> СостояниеКассовойСмены.КассоваяСмена Тогда
		...
		Если НайденныеДокументы.Количество() = 0 Тогда
			...
			ОписаниеОшибки = НСтр("ru = 'При создании отчета о розничных продажах по кассовой смене %1 в строке %2 по чеку коррекции не удалось подобрать документ реализации по кассовой смене %3.';
									|en = 'Cannot select a sales document of the %3 POS shift by the adjustment voucher in line %2 when creating a retail sales report for the %1 POS shift.'");
			Если ТипЗнч(СтрокаТабличнойЧастиТовары) = Тип("ДокументТабличнаяЧастьСтрока.ОтчетОРозничныхВозвратах.Товары") Тогда
				СтрокаТабличнойЧастиТовары.СпособОпределенияСебестоимости = Перечисления.СпособыОпределенияСебестоимости.ИзТекущегоДокумента;
				ОписаниеОшибки = НСтр("ru = 'При создании отчета о розничных возвратах по кассовой смене %1 в строке %2 не удалось подобрать документ реализации по кассовой смене %3. Способ определения себестоимости в строке изменен на %4.';
										|en = 'Cannot select the sales document for the %3 POS shift when creating the retail return report for the %1 POS shift in line %2. The cost defining method in the line is changed to %4.'");
				ОписаниеОшибки = СтрШаблон(ОписаниеОшибки, СостояниеКассовойСмены.КассоваяСмена, СтрокаТабличнойЧастиТовары.НомерСтроки, ВыборкаДетальныеЗаписи.ЧекККМКассоваяСмена, Перечисления.СпособыОпределенияСебестоимости.ИзТекущегоДокумента);
			Иначе
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Розничные продажи';
											|en = 'Retail sales'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение, , , ОписаниеОшибки);
...
Функция ОписаниеОшибки24Часа() Экспорт
	...
	Возврат НСтр("ru = 'С момента открытия кассовой смены истекло более 24 часов. Необходимо выполнить закрытие кассовой смены.';
				|en = 'More than 24 hours have passed since the POS shift was opened. Close the POS shift.'");
...
Процедура ЗаписьВЖурналРегистрации(Знач ТекстОшибки, Знач Событие, Данные = Неопределено, Знач Уровень = "Ошибка") Экспорт
	...
	ЗаписьЖурналаРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Розничные продажи.%1';
				|en = 'Retail sales.%1'", ОбщегоНазначения.КодОсновногоЯзыка()), 
			Событие), 
		УровеньЖР, 
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ШаблонКомментария = НСтр("ru = 'Установка константы ""%1"". Если включена функциональная опция ""%2"", то значение константы влияет на вид чека, который будет устанавливается по умолчанию во всех фискальных формах.';
							|en = 'Set the %1 constant. If the %2 functional option is enabled, the constant value affects the receipt type that will be set by default in all fiscal forms.'");
