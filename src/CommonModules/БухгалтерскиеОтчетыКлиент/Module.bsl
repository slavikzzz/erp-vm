...
Процедура ОбработчикКомандыВыгрузитьБСП(ФормаОтчета, Команда) Экспорт
	...
	ПараметрыОтчета.Вставить("РучнаяНастройка",
		БухгалтерскиеОтчетыВызовСервера.ЕстьОтличияНастроекОтЭталонныхБСП(
			ФормаОтчета.НастройкиОтчета.ПараметрыРежимаВыгрузки, ФормаОтчета.Отчет.КомпоновщикНастроек));
	//-- НЕ УТ
...
Функция ЕстьОтличияНастроекОтЭталонныхБП(ПараметрыРежимаВыгрузки, ТекущиеНастройки) Экспорт
	...
	Если ТекущиеНастройки.Свойство("КомпоновщикНастроек") Тогда
		...
		Если ПараметрыРежимаВыгрузки.Свойство("Отбор") Тогда // сверяемся с заданным в настройках отбором
			//++ НЕ УТ
			Если БухгалтерскиеОтчетыВызовСервера.ЕстьОтличияОтборовОтЭталонных(
					ПараметрыРежимаВыгрузки.Отбор, ТекущиеНастройки.КомпоновщикНастроек.Настройки.Отбор) Тогда
				Возврат Истина;
			КонецЕсли;
			//-- НЕ УТ
...
Процедура ТабличноеПолеПоСчетамСчетПриИзменении(ФормаОтчета, ИмяЭлемента, Элемент) Экспорт
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.Счет) Тогда
		Возврат;
	КонецЕсли;
	
	ДополнитьПодразделением = (ИмяЭлемента = "ГруппировкаКор");
		
	БухгалтерскиеОтчетыКлиентСервер.ИнициализироватьНастройкуАналитики(
		ТекущиеДанные,
		ТекущиеДанные.Счет,
		ДополнитьПодразделением);
	...
	Иначе
		...
		Для Индекс = 1 По КоличествоСубконто Цикл
			...
			ИначеЕсли ИндексСубконто = "!" Тогда
				СписокВидовСубконто.Добавить("НаправлениеДеятельности", НСтр("ru = 'Направление деятельности';
																			|en = 'Line of business'"), ?(Сред(СтрокаПоСубконто, Индекс * 2 - 1, 1) = "+", Истина, Ложь));
			Иначе
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СписокЗначений") Тогда 
		...
		Для Каждого ЭлементСписка Из ВыбранноеЗначение Цикл
			Если ЭлементСписка.Пометка Тогда
				...
				СтрокаПредставление = СтрокаПредставление + Строка(ЭлементСписка.Представление) + ", ";
			Иначе
				СтрокаПоСубконто = СтрокаПоСубконто + "-";
			КонецЕсли;
...
Процедура ОтчетСохранитьКак(Форма, ИмяТабличногоДокумента = "Результат") Экспорт
	...
	ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботыСФайлами(ОповещениеОРаботеСФайлами,
		НСтр("ru = 'Для сохранения файла требуется установка расширения для работы с 1С:Предприятием.';
			|en = 'Install 1C:Enterprise extension to save the file.'"),
		Истина);
...
Процедура ОбработатьРезультатПроверкиАктуальности(Результат, Форма) Экспорт
	...
	Если РезультатПроверки.Свойство("Статус") И РезультатПроверки.Статус = "Ошибка" Тогда
		...
		Иначе
			МассивПодстрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Не удалось получить информацию об актуальности данных отчета';
																	|en = 'Cannot get information about the report data relevance.'")));
		КонецЕсли;
	КонецЕсли;
	...
	Если РезультатПроверки.Состояние = "НеВыполнено" И РезультатПроверки.Свойство("ДатаАктуальности") Тогда
		...
		ТекстДанныеУчетаНеАктуальны = НСтр("ru = 'Данные учета неактуальны по';
											|en = 'Accounting data is not relevant till'") + " ";
		...
		МассивПодстрок.Добавить(Новый ФорматированнаяСтрока(" " + НСтр("ru = 'Рекомендуется выполнить';
																		|en = 'It is recommended to perform'") + " "));
		МассивПодстрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'закрытие месяца';
																|en = 'month-end closing'"),,,, "e1cib/app/Обработка.ОперацииЗакрытияМесяца"));
		МассивПодстрок.Добавить(Новый ФорматированнаяСтрока(" " + НСтр("ru = 'и сформировать отчет повторно.';
																		|en = 'and generate the report again.'")));
	КонецЕсли;
...
Процедура ОтобразитьСостояниеАктуализации(Форма) Экспорт
	...
	ТекстПриАктуализации = НСтр("ru = 'Данные этого отчета могут измениться.';
								|en = 'Data of this report can change. '");
