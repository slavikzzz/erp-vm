	...
	Если ПустаяСтрока(АдресПочты) Тогда
		...
		ИначеЕсли ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты") Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Нельзя создать письмо по контактной информацию с типом ""%1"".';
					|en = 'Cannot create an email from contact information of the ""%1"" type.'"), ТипИнформации);
		Иначе
...
Процедура ПерейтиПоВебСсылке(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено) Экспорт
	...
	Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.ВебСтраница") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Нельзя открыть ссылку по контактной информации с типом ""%1""';
																						|en = 'Cannot follow a link from contact information of the ""%1"" type.'"), ТипИнформации);
	КонецЕсли;
...
Процедура ПередТелефоннымЗвонком(КонтактнаяИнформация, ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(КонтактнаяИнформация.Представление) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Для совершения звонка введите номер телефона.';
				|en = 'To start a call, enter a phone number.'"), , ДополнительныеПараметры.ИмяРеквизита);
		Возврат;
	КонецЕсли;
...
Процедура ПередСозданиемSMS(КонтактнаяИнформация, ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(КонтактнаяИнформация.Представление) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Для отправки SMS введите номер телефона.';
				|en = 'To send a text message, enter a phone number.'"), , ДополнительныеПараметры.ИмяРеквизита);
		Возврат;
	КонецЕсли;
	ПараметрыSMS = ПараметрыПисьмаSMS();
