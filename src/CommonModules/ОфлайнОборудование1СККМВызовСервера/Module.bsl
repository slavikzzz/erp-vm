...
Функция ТекстXMLНастроек(НастройкиККМ, ВерсияФорматаОбмена) Экспорт
	...
	Иначе
		ОбъектОбмена = ОбъектНастройкиККМ1000(НастройкиККМ, ВерсияФорматаОбмена)
	КонецЕсли;
...
Функция ТекстXMLПрайсЛиста(ПрайсЛист, ВерсияФорматаОбмена) Экспорт
	...
	Иначе
		ОбъектОбмена = ОбъектПрайсЛист1000(ПрайсЛист, ВерсияФорматаОбмена);
	КонецЕсли;
...
Функция ЗагружаемыеДанныеИзККМ(Отказ, XMLТекст, ВыходныеПараметры, ВерсияФорматаОбмена) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'При чтении файла-отчета произошла ошибка.';
								|en = 'An error occurred while reading the report file.'") 
		...
		Если НЕ ОтчетОПродажахXTDO.Чеки = Неопределено Тогда
			...
			Для Каждого ЧекXDTO Из ОтчетОПродажахXTDO.Чеки.Чек Цикл
				...
				Если НЕ ЧекXDTO.КодСистемыНалогообложения = Неопределено Тогда
					ЧекККМ.СистемаНалогообложения = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.СистемаНалогообложенияККТПоКоду(ЧекXDTO.КодСистемыНалогообложения);
				КонецЕсли;
...
Процедура ЗаполнитьОтчетыВскрытияТары(ВскрытияТарыXDTO, ВскрытияТары, ВерсияФорматаОбмена) Экспорт
	...
	Для Каждого ВскрытиеТарыXTDO Из ВскрытияТарыXDTO.ВскрытиеАлкогольнойТары Цикл
		...
		Если ЗначениеЗаполнено(ВскрытиеТарыXTDO.НомерДокумента) Тогда
			ВскрытиеТарыККМ.НомерДокумента = ВскрытиеТарыXTDO.НомерДокумента;
		КонецЕсли; 
		
		Если ЗначениеЗаполнено(ВскрытиеТарыXTDO.ПостановкаНаКран) Тогда
			ВскрытиеТарыККМ.ПостановкаНаКран = ВскрытиеТарыXTDO.ПостановкаНаКран;
		КонецЕсли;
...
Процедура ЗаполнитьОтчетыОПродажах1000(ОтчетыОПродажахККМ, ОтчетыОПродажах, ВерсияФорматаОбмена) Экспорт
	...
	Для Каждого ОтчетОПродажахXTDO Из ОтчетыОПродажахККМ.ОтчетОПродажах Цикл
		...
		Для Каждого СтрокаТоварXDTO Из ОтчетОПродажахXTDO.ОтчетОПродажахТовары.ОтчетОПродажахТоварыЗапись Цикл
			...
			СвойстваСтрокаТоварXDTO = СтрокаТоварXDTO.Свойства(); // ОбъектXDTO
			Если НЕ СвойстваСтрокаТоварXDTO.Получить("ШтрихкодАлкогольнойПродукции") = Неопределено Тогда
	...
	Для Каждого ТекПакет Из Пакеты Цикл
		...
		Исключение
			...
			СообщениеОбОшибке = НСтр("ru = 'При проверке файла на обработанность произошла ошибка.';
									|en = 'An error occurred when checking the file for processing.'")
...
// 
// Параметры:
//  ВыгружаемыеНастройкиККМ - см. МенеджерОфлайнОборудованияКлиентСервер.НастройкиККМ
//  ВерсияФорматаОбмена - Строка
//  ЕстьВыгружаемыеНастройки - Булево
//
// Возвращаемое значение:
//  ЗначениеXDTO
	...
	Если ЗначениеЗаполнено(ВыгружаемыеНастройкиККМ.СистемыНалогообложения) Тогда
		...
		Для Каждого СистемаНалогообложенияККТ Из ВыгружаемыеНастройкиККМ.СистемыНалогообложения Цикл
			...
			ОбъектСистемаНалогообложенияЗапись.КодСистемыНалогообложения =
				ОборудованиеЧекопечатающиеУстройстваКлиентСервер.КодСистемыНалогообложенияККТ(СистемаНалогообложенияККТ);
...
// 
// Параметры:
//  СтруктураНастроек - см. МенеджерОфлайнОборудованияКлиентСервер.НастройкиККМ
//  ВерсияФорматаОбмена - Строка
//
// Возвращаемое значение:
//  ЗначениеXDTO
Функция ОбъектНастройкиККМ1000(СтруктураНастроек, ВерсияФорматаОбмена)
...
// Возвращает объект прайс-листа
// 
// Параметры:
//  ВыгружаемыйПрайсЛист - см. МенеджерОфлайнОборудованияКлиентСервер.ПрайсЛистККМ
//  ВерсияФорматаОбмена - Строка
//  ЕстьВыгружаемыеДанные - Булево
//
// Возвращаемое значение:
//  ЗначениеXDTO
Функция ОбъектПрайсЛист(ВыгружаемыйПрайсЛист, ВерсияФорматаОбмена, ЕстьВыгружаемыеДанные = Ложь)
	...
	Если ЗначениеЗаполнено(ВыгружаемыйПрайсЛист.Товары) Тогда
		...
		Товары = ВыгружаемыйПрайсЛист.Товары;
		Для Каждого Товар Из Товары Цикл
			...
			Если ЗначениеЗаполнено(Товар.СтавкаНДС) Тогда
				...
				Если ВерсияФорматаОбмена < 3000 И Товар.СтавкаНДС = МенеджерОфлайнОборудования.СтавкаНДС20() Тогда
					ТекстСообщения = НСтр("ru = 'Для использования НДС 20% необходимо использовать формат обмена 3.0 и выше';
											|en = 'To use a 20% VAT, use the exchange format 3.0 and later'");
			...
			Если Товар.ИмеетУпаковки Тогда
				...
				Если Товар.ИмеетХарактеристики Тогда
					...
					|Флаг ""Имеет Упаковки"" не может быть ""Истина"", если у товара имеются характеристики (флаг Имеет Характеристики).';
					|en = 'An error occurred when embedding PEL (Offline peripherals).
					|The ""ИмеетУпаковки"" check box cannot be True if goods have variants (the ИмеетХарактеристики check box).'");
			...
			Если Товар.ЭтоМаркированнаяПродукция Тогда
				...
				Если ВерсияФорматаОбмена < 3001 Тогда
					ТекстСообщения = НСтр("ru = 'Для работы с маркированной продукцией необходимо использовать формат обмена 3.1 и выше';
											|en = 'To work with labeled products, you must use the exchange format 3.1 and above.'");
					ВызватьИсключение ТекстСообщения;
				КонецЕсли;
			...
			КонецЕсли;
				...
				Если ВерсияФорматаОбмена < 3002 Тогда
					ТекстСообщения = НСтр("ru = 'Для работы с продукцией использующей номенклатурную классификацию необходимо использовать формат обмена 3.2 и выше';
											|en = 'To work with manufactured products that use product classification, you must use the exchange format 3.2 and above'");
					ВызватьИсключение ТекстСообщения;
				КонецЕсли;
		...
		Для Каждого ДанныеАгента Из ВыгружаемыйПрайсЛист.ДанныеАгентов Цикл
			...
			Если ЗначениеЗаполнено(ДанныеАгента.ПризнакАгента) Тогда
				ЗаписьДанныеАгента.ПризнакАгента =
					ОборудованиеЧекопечатающиеУстройстваРФКлиентСервер.КодПризнакаАгента(ДанныеАгента.ПризнакАгента);
			КонецЕсли;
			// -- Локализация
...
// 
// Параметры:
//  СтруктураПрайсЛиста - см. МенеджерОфлайнОборудованияКлиентСервер.ПрайсЛистККМ
//  ВерсияФорматаОбмена - Строка
//
// Возвращаемое значение:
//  ЗначениеXDTO
Функция ОбъектПрайсЛист1000(СтруктураПрайсЛиста, ВерсияФорматаОбмена)
...
// Возвращает объект характеристики
//
// Параметры:
//  Товар - МенеджерОфлайнОборудования.ЗаписьТовараПрайсЛиста
//  ВерсияФорматаОбмена - Строка
// 
// Возвращаемое значение:
//  ЗначениеXDTO
Функция ОбъектХарактеристики(Товар, ВерсияФорматаОбмена)
	...
	Характеристики = Товар.Характеристики;
...
// Возвращает объект упаковки
//
// Параметры:
//  Источник - МенеджерОфлайнОборудования.ЗаписьТовараПрайсЛиста
//  ВерсияФорматаОбмена - Строка
// 
// Возвращаемое значение:
//  ЗначениеXDTO
Функция ОбъектУпаковки(Источник, ВерсияФорматаОбмена)
	...
	Упаковки = Источник.Упаковки;  //Массив Из См. МенеджерОфлайнОборудования.ЗаписьУпаковкиПрайсЛиста
...
Процедура ВызватьИсключениеЗаполнения(ИмяОбъекта, ИмяПоля)
	...
	ТекстСообщения = НСтр("ru = 'Ошибка встраивания БПО (Офлайн-оборудование).
		...
		|en = 'An error occurred when embedding PEL (Offline peripherals).
		|Required field is not filled: %ИмяПоля%, object: %ИмяОбъекта%.'");
...
// Записывает товар в прайс лист
//
// Параметры:
//  ОбъектПрайсЛист - см. МенеджерОфлайнОборудованияКлиентСервер.ПрайсЛистККМ
//  Запись - ЗначениеXDTO
//  Источник - см. МенеджерОфлайнОборудования.ЗаписьТовараПрайсЛиста
//  ВерсияФорматаОбмена - Строка
...
Процедура ЗаписатьТовар1000(ОбъектПрайсЛист, Запись, Источник, ВерсияФорматаОбмена, ЕдиницыИзмерения)
	...
	Если НЕ (Источник.ИмеетУпаковки ИЛИ Источник.ИмеетХарактеристики) Тогда
		...
		Запись.Штрихкод = РазложенныйШтрихкод(Источник.Штрихкоды);
...
// Записывает объект характеристики
//
// Параметры:
//  Запись - ЗначениеXDTO
//  Источник - см. МенеджерОфлайнОборудования.ЗаписьТовараПрайсЛиста
...
//  ЕдиницыИзмерения - Массив из см. МенеджерОфлайнОборудования.ЗаписьЕдиницыИзмеренияПрайсЛиста
//
Процедура ЗаписатьХарактеристики1000(Запись, Источник, ВерсияФорматаОбмена, ЕдиницаИзмерения)
	...
	Для Каждого СтрокаХарактеристика Из Источник.Характеристики Цикл
		...
		Если НЕ СтрокаХарактеристика.ИмеетУпаковки Тогда
			...
			ОбъектХарактеристикиЗапись.Штрихкод = РазложенныйШтрихкод(СтрокаХарактеристика.Штрихкоды);
...
Процедура ЗаписатьУпаковки1000(Запись, Источник, ВерсияФорматаОбмена, ЕдиницаИзмерения)
	...
	Для Каждого СтрокаУпаковка Из Источник.Упаковки Цикл
		...
		ОбъектУпаковкиЗапись.Штрихкод = РазложенныйШтрихкод(СтрокаУпаковка.Штрихкоды);
...
Функция СоздатьЧтениеXML(ТекстПакета, ЧтениеXML, ВыходныеПараметры)
	...
	Исключение
		СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ru = 'При чтении файла произошла ошибка';
														|en = 'An error occurred when reading the file'"));
		Результат = Ложь;
	КонецПопытки;
