...
Функция ТестовоеПодключение(ПараметрыОперации) Экспорт
	...
	СписокОшибок = ВнешняяКомпонента.ПолучитьСписокОшибок();
...
Функция ПредварительноеОдобрениеИспользования(ПараметрыОперации) Экспорт
	...
	Параметры.СписокОшибок         = ВнешняяКомпонента.ПолучитьСписокОшибок();
...
Функция ПередачаДанныхФискальногоЧека(ПараметрыОперации) Экспорт
	...
	Если ПараметрыОперации.ПротоколОбмена = Перечисления.ПротоколОбменаФЭСНСПК.ВерсияV2 Тогда 
...
Функция ПрочитатьJSONРезультатОперации(СтрокаJSON, СписокОшибок = Неопределено, ОснованиеФискальнойОперации = Неопределено) Экспорт 
	...
	Если Не ПустаяСтрока(СтрокаJSON) 
		...
		ЧтениеJSON.Закрыть();
	Иначе                      
		РезультатВыполнения.ОписаниеОшибки = ?(ПустаяСтрока(ОписаниеОшибки), НСтр("ru = 'Ответ не получен';
																					|en = 'Response is not received'"), ОписаниеОшибки);
	КонецЕсли;
...
Функция ПодготовитьКомпонентуИнтеграцииНСПК(ПараметрыОперации = Неопределено)
	...
	Если ВнешняяКомпонента = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Ошибка подключения внешней компоненты НСПК.';
								|en = 'An error occurred while attaching the NSPK add-in.'");
		ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Ошибка компоненты НСПК';
				|en = 'NSPK add-in error'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()),
			ТекстСообщения);
	...
	Если ПараметрыОперации <> Неопределено Тогда
		...
		ВнешняяКомпонента.УстановитьПараметры(ПараметрыJSON);
	КонецЕсли;
...
Процедура ЗаписатьИсториюПлатежнойОперации(ТипПлатежнойОперации, ДанныеОперации, РезультатВыполнения, ТекстОтветаJSON)
	...
	Если Не ПустаяСтрока(ТекстОтветаJSON) 
		И Не СтрНачинаетсяС(ТекстОтветаJSON, "<!DOCTYPE") 
		И Не СтрНачинаетсяС(ТекстОтветаJSON, "<html>") Тогда
...
Функция ПроверитьПараметрыОперации(ПараметрыОперации)   
	
	РезультатВыполнения = ЭлектронныеСертификатыНСПККлиентСервер.ПараметрыВыполненияОперации(Истина);
	
	Если ПараметрыОперации.ПротоколОбмена = Перечисления.ПротоколОбменаФЭСНСПК.ВерсияV2
