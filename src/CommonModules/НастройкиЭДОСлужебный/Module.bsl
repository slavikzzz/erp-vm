...
Функция ПредупрежденияПоНастройкеОбменаНаДату(КлючНастроекОтправки, ИдентификаторПолучателя = "",
	...
	Пока АбонентыЭДО.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(АбонентыЭДО.КраткоеОписание) Тогда
			ТекстИдентификатора = СтрШаблон(НСтр("ru = '%1 (%2)';
												|en = '%1 (%2)'"), АбонентыЭДО.ИдентификаторЭДО, АбонентыЭДО.КраткоеОписание);
		Иначе
...
Процедура ДобавитьПредупреждениеСрокДействияСертификатов(Предупреждения, УчетнаяЗапись)
	...
	Предупреждение.ТекстОшибки = СтрШаблон(НСтр("ru = 'У учетной записи %1 отсутствуют действующие сертификаты. Документ можно отправить, но он не будет получен, пока контрагент не перевыпустит хотя бы один сертификат. Рекомендуем связаться с контрагентом и сообщить о данной проблеме.';
												|en = 'The account %1 has no valid certificates. The document can be sent but it will not be received until the counterparty reissues at least one certificate. We recommend to contact the counterparty and to report this issue.'"), УчетнаяЗапись);
...
Процедура ДобавитьПредупреждениеДатаПоследнейАктивности(Предупреждения, УчетнаяЗапись)
	...
	Предупреждение.ТекстОшибки = СтрШаблон(НСтр("ru = 'Контрагент последний раз использовал  учетную запись %1  более 2 месяцев назад. Возможно он ей уже не пользуется';
												|en = 'Last time the counterparty used the account %1 was more than 2 month ago. Perhaps they don''t use it anymore'"), УчетнаяЗапись);
	Предупреждения.Добавить(Предупреждение);
...
Процедура ДобавитьПредупреждениеУстаревшийФорматПолучателя(Предупреждения, УстаревшийФорматПолучателя)
	...
	Шаблон = НСтр("ru = 'Контрагент использует для обмена документами ""<b>%1</b>"" устаревший формат ""%2"". Документ будет отправлен в формате ""%3"", вследствие чего у получателя могут возникнуть проблемы при отображении документа. По возможности свяжитесь контрагентом и порекомендуйте обновить программное обеспечение';
					|en = 'The counterparty uses the obsolete format ""%2"" for exchange of documents ""<b>%1</b>"". The document will be sent in the ""%3"" format, the receiver might have document display issues. Contact the counterparty and recommend updating the software'");
...
#Область ОбменСКонтрагентами_ВключитьИспользованиеОзнакомленийСЭД
	...
	|en = '1С:Exchange with counterparties: enable sending electronic documents for review.
	|Sending electronic documents for review is temporarily unavailable.'");
...
#Область НастройкиЭДОСлужебный_УстановкаЗначенияКонстантыСоздаватьКонтрагентовАвтоматическиБЭД
	...
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: Установка константы разрешающей автоматическое создание контрагентов при работе с ЭДО';
									|en = '1C:Exchange with counterparties: Install the constant enabling automatic creation of counterparties when working with EDI'");
...
#Область Справочники_УдалитьСоглашенияОбИспользованииЭД_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|en = '1С:Exchange with counterparties: change parameters of the ""EDI settings"" catalog.
...
#Область НастройкиЭДОСлужебный_ВключитьУтверждениеВходящихДокументов
	...
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: Включает необходимость утверждения входящих документов, если включен обмен с контрагентами';
									|en = '1C:Exchange with counterparties: Enables approval of incoming documents if exchange with counterparties is enabled'");
...
#Область НастройкиЭДОСлужебный_ТипПодписиЭД
	...
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: Включает тип подписи по умолчанию для электронных документов';
									|en = '1C:Exchange with counterparties: Enables the default signature type for electronic documents'");
...
#Область НастройкиЭДОСлужебный_ТипПодписиМЧД
	...
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: Включает тип подписи по умолчанию для машиночитаемых доверенностей';
									|en = '1C:Exchange with counterparties: Enables the default signature type for machine-readable letters of authority'");
...
Процедура УстановкаЗначенияКонстантыСоздаватьКонтрагентовАвтоматическиБЭД(Параметры) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось обработать константу ""Создавать контрагентов автоматически БЭД"" по причине:';
								|en = 'Cannot process the constant ""Create counterparties automatically BED"" due to:'") 
	...
	Если ОбработанныхОбъектов = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = НСтр("ru = 'Не удалось обработать константу ""Создавать контрагентов автоматически БЭД"".';
								|en = 'Cannot process the constant ""Create counterparties automatically BED"".'");
		ВызватьИсключение ТекстСообщения;
	Иначе
		ТекстСообщения = НСтр("ru = 'Обработана константа ""Создавать контрагентов автоматически БЭД"".';
								|en = 'The constant ""Create counterparties automatically BED"" is processed.'");
...
Процедура НастроитьАвтоПереходНаНовыеВерсииФорматовЭД() Экспорт
	...
	Исключение
		...
		Операция = НСтр("ru = 'Обновление подсистемы обмена с контрагентами';
						|en = 'Updating subsystem of exchange with counterparties'");
		...
		ТекстСообщения = НСтр("ru = 'При обновлении подсистемы обмена с контрагентами произошла ошибка';
								|en = 'An error occurred when updating the subsystem of exchange with counterparties'");
...
Процедура ВключитьИспользованиеОзнакомленийСЭД(Параметры) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось обработать константу ""Использовать ознакомление с электронными документами"" по причине:';
								|en = 'Cannot process the ""Use preview electronic documents"" constant due to:'") 
			+ Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
			МетаданныеОбъекта,, ТекстСообщения);
	...
	Если ОбработанныхОбъектов = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = НСтр("ru = 'Не удалось обработать константу ""Использовать ознакомление с электронными документами"".';
								|en = 'Cannot process the ""Use preview electronic documents"" constant.'");
		ВызватьИсключение ТекстСообщения;
	Иначе
		ТекстСообщения = НСтр("ru = 'Обработана константа ""Использовать ознакомление с электронными документами"".';
								|en = 'The ""Use preview electronic documents"" constant is processed.'");
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			МетаданныеОбъекта,, ТекстСообщения);
	КонецЕсли;
...
Процедура ВключитьУтверждениеВходящихДокументов(Параметры) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось обработать константу ""Отправлять входящие документы на утверждение"" по причине:';
								|en = 'Cannot to process the ""Send incoming documents for approval"" constant. Reason:'") 
			+ Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
			МетаданныеОбъекта,, ТекстСообщения);
	...
	Если ОбработанныхОбъектов = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = НСтр("ru = 'Не удалось обработать константу ""Отправлять входящие документы на утверждение"".';
								|en = 'Cannot to process the ""Send incoming documents for approval"" constant.'");
		ВызватьИсключение ТекстСообщения;
	Иначе
		ТекстСообщения = НСтр("ru = 'Обработана константа ""Отправлять входящие документы на утверждение"".';
								|en = '""Send incoming documents for approval"" constant processed.'");
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			МетаданныеОбъекта,, ТекстСообщения);
	КонецЕсли;
