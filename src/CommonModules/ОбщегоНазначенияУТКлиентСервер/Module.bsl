	...
	ИначеЕсли СпособПредставленияДокумента = ПредопределенноеЗначение("Перечисление.СпособыПредставленияВходящихДокументов.Внешнее") Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(Данные.НаименованиеВходящегоДокумента) Тогда
			Шаблон = НСтр("ru = 'вх. %1 %2 от %3, вн. %4 от %5';
							|en = 'incoming %1 %2 dated %3, # %4 dated %5'");
...
Функция ДоступныеЗначенияПериодаПоВидуПериода(Знач НачалоПериода, Знач ВидПериода) Экспорт
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Полугодие") Тогда
		...
		Для Сч = ТекущийГод - 1 По ТекущийГод + 1 Цикл
			СписокПериодов.Добавить(Дата(Сч, 1, 1), НСтр("ru = 'I полугодие';
														|en = 'Half-year I'")  + " " + Формат(Сч, "ЧГ=0"));
			СписокПериодов.Добавить(Дата(Сч, 7, 1), НСтр("ru = 'II полугодие';
														|en = 'Half-year II'") + " " + Формат(Сч, "ЧГ=0"));
		КонецЦикла;
		СписокПериодов.Добавить(Дата(ТекущийГод + 2, 1, 1), Формат(ТекущийГод + 2, "ЧГ=0") + "...");
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Квартал") Тогда
		...
		Для Сч = ТекущийГод - 1 По ТекущийГод Цикл
			СписокПериодов.Добавить(Дата(Сч, 1, 1), НСтр("ru = 'I квартал';
														|en = 'Quarter I'")   + " " + Формат(Сч, "ЧГ=0"));
			СписокПериодов.Добавить(Дата(Сч, 4, 1), НСтр("ru = 'II квартал';
														|en = 'Quarter II'")  + " " + Формат(Сч, "ЧГ=0"));
			СписокПериодов.Добавить(Дата(Сч, 7, 1), НСтр("ru = 'III квартал';
														|en = 'Quarter III'") + " " + Формат(Сч, "ЧГ=0"));
			СписокПериодов.Добавить(Дата(Сч, 10, 1),НСтр("ru = 'IV квартал';
														|en = 'Quarter IV'")  + " " + Формат(Сч, "ЧГ=0"));
		КонецЦикла;
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Декада") Тогда
		...
		СписокПериодов.Добавить(Период, Формат(Период, НСтр("ru = 'ДФ=''MMMM yyyy''';
															|en = 'DF=''MMMM yyyy'''")) + "...");
		Пока Сч >0 Цикл
			СписокПериодов.Добавить(Дата(СчГод, СчМесяц, 1),  НСтр("ru = 'I дек.';
																	|en = 'Ten-day period 1'")   + " " + Нрег(Формат(Дата(СчГод, СчМесяц, 1), НСтр("ru = 'ДФ=''MMMM yyyy''';
																																		|en = 'DF=''MMMM yyyy'''"))));
			СписокПериодов.Добавить(Дата(СчГод, СчМесяц, 11), НСтр("ru = 'II дек.';
																	|en = 'Ten-day period 2'")  + " " + Нрег(Формат(Дата(СчГод, СчМесяц, 1), НСтр("ru = 'ДФ=''MMMM yyyy''';
																																		|en = 'DF=''MMMM yyyy'''"))));
			СписокПериодов.Добавить(Дата(СчГод, СчМесяц, 21), НСтр("ru = 'III дек.';
																	|en = 'Ten-day period 3'") + " " + Нрег(Формат(Дата(СчГод, СчМесяц, 1), НСтр("ru = 'ДФ=''MMMM yyyy''';
																																		|en = 'DF=''MMMM yyyy'''"))));
			СчМесяц = СчМесяц + 1;
	...
	ИначеЕсли ВидПериода = ПредопределенноеЗначение("Перечисление.ДоступныеПериодыОтчета.Неделя") Тогда
		...
		Для Сч = 0 По 6 Цикл
			...
			СписокПериодов.Добавить(НачНедели, Формат(НачНедели, НСтр("ru = 'ДФ=''dd.MM''';
																		|en = 'DF=''dd.MM'''")) + " - " + Формат(КонНедели, НСтр("ru = 'ДФ=''dd.MM''';
																															|en = 'DF=''dd.MM'''")));
		КонецЦикла;
...
Процедура ПроверитьНаличиеДублейЗначенийМассива(Массив, Отказ, ПредставлениеМассива = "", ПредставлениеЗначений = "", ИсключатьПустые = Ложь) Экспорт
	...
	ШаблонСообщения = НСтр("ru = 'В списке ""%ПредставлениеМассива%"" повторяется значение ""%ПовторяемоеЗначение%"" в поле %ПредставлениеЗначений%.';
							|en = 'In the ""%ПредставлениеМассива%"" list, the ""%ПовторяемоеЗначение%"" value is repeated in field %%ПредставлениеЗначений%%.'");
