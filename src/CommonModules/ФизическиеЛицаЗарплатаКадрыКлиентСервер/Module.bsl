...
// Функция определяет пол физлица по его фамилии, имени и отчеству.
// Параметр:
// 		ОбъектСЧастямиФИО - объект содержащий поля (реквизиты, ресурсы, свойства ...) Фамилия, Имя Отчество.
//
// Возвращаемое значение:
// 	ПеречислениеСсылка.ПолФизическогоЛица, Неопределено - если данных недостаточно
//
Функция ПолПоЧастямФИО(ОбъектСЧастямиФИО) Экспорт
	
	Пол = Неопределено;
	ЧастиФИО = ЧастиФИОИзОбъекта(ОбъектСЧастямиФИО);
	Если ОбщегоНазначенияБЗККлиентСервер.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПодборФИО") Тогда
		ОбщийМодульПодборФИО = ОбщегоНазначенияБЗККлиентСервер.ОбщийМодуль("ПодборФИОВызовСервера");
		ПолЧислом = ОбщийМодульПодборФИО.ОпределитьПол(ЧастиФИО);
		Если ПолЧислом = 1 Тогда
	...
	Если Пол = Неопределено И ЧастиФИО.Свойство("Отчество") Тогда
		Возврат ОпределитьПолПоОтчеству(ЧастиФИО.Отчество);
	КонецЕсли;
...
Функция ФИОИзЧастей(ЧастиИмени) Экспорт
	Если ЗначениеЗаполнено(ЧастиИмени.Отчество) Тогда
		Возврат СокрЛП(СтрСоединить(
		    ОбщегоНазначенияБЗККлиентСервер.ЗначенияВМассиве(
		        СокрЛП(ЧастиИмени.Фамилия),
		        СокрЛП(ЧастиИмени.Имя),
		        СокрЛП(ЧастиИмени.Отчество)),
		    " "));
	КонецЕсли;
	Возврат СокрЛП(СтрСоединить(
	    ОбщегоНазначенияБЗККлиентСервер.ЗначенияВМассиве(
...
Функция ПредставлениеВозраста(ДатаРождения, ДатаВозраста) Экспорт
	...
	Если Возраст >= 1 Тогда
		Возврат СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(НСтр("ru = ';%1 год;;%1 года;%1 лет;';
																				|en = ';%1 year;;%1 years;%1 years;'"), Возраст);
	КонецЕсли;
	...
	Возврат СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(НСтр("ru = ';%1 месяц;;%1 месяца;%1 месяцев;';
																			|en = ';%1 month;;%1 month;%1 months;'"), Возраст);
...
Функция ПустыеЧастиФИО() Экспорт
...
Функция ЧастиФИОИзОбъекта(ОбъектСЧастямиФИО)
	
	ЧастиФИО = ПустыеЧастиФИО();
	ЗаполнитьЗначенияСвойств(ЧастиФИО, ОбъектСЧастямиФИО);
	ПустыеКлючи = Новый Массив;
	Для Каждого КлючИЗначение Из ЧастиФИО Цикл
		Если Не ЗначениеЗаполнено(КлючИЗначение.Значение) Тогда
			ПустыеКлючи.Добавить(КлючИЗначение.Ключ);
		КонецЕсли;
	КонецЦикла;   
	Для Каждого ПустойКлюч Из ПустыеКлючи Цикл
		ЧастиФИО.Удалить(ПустойКлюч);
	КонецЦикла;
	Возврат ЧастиФИО;
