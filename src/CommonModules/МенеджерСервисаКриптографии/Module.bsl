...
Процедура УстановитьСертификатВКонтейнерИХранилище(ПараметрыЗаявления, АдресРезультата) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Менеджер сервиса криптографии';
				|en = 'Digital signature SaaS.Cryptography service manager'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Функция ПоискСертификатаПоОтпечаткуИлиСерийномуНомеру(
	...
	Если Не ЗначениеЗаполнено(Отпечаток) И Не ЗначениеЗаполнено(СерийныйНомер) Тогда 
		Возврат Новый Структура("Выполнено, КодОшибки, ОписаниеОшибки", 
						...
						НСтр("ru = 'Необходимо заполнить thumbprint или serialnumber.';
							|en = 'Fill in thumbprint or serialnumber.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
