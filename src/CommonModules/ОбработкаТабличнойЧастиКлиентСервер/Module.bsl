...
//		* УказаннаяСтавкаНДС - СправочникСсылка.СтавкиНДС - Указанная пользователем в строке ТЧ ставка НДС.
//
...
Функция ПолучитьСтруктуруЗаполненияСодержанияУслугиВСтрокеТЧ(Объект, ЗаполнятьДляВсехУслуг) Экспорт
	...
	Если Объект.Свойство("ХозяйственнаяОперация")
		 И (Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья2_5")
...
Функция СуммаССуффиксом(Суффикс) Экспорт
	...
	ИначеЕсли Суффикс = "Забаланс" Тогда
		...
		СтруктураВозврата.СуммаБезНДСУпр = "";		
	Иначе
		ВызватьИсключение НСтр("ru = 'Неизвестный суффикс для суммы:';
								|en = 'Unknown suffix for amount:'") + " " + Суффикс;
	КонецЕсли;
...
Функция СтавкаССуффиксом(Суффикс) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Неизвестный суффикс для ставки:';
								|en = 'Unknown suffix for the rate:'") + " " + Суффикс;
	КонецЕсли;
...
Процедура ЗаполнитьДействиеПоПерерабатываемымМатериаламВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьДействиеПоПерерабатываемымМатериалам", Действие) Тогда
		Если ЗначениеЗаполнено(Действие) Тогда
			ТекущаяСтрока.Действие = Действие;
		Иначе
			ТекущаяСтрока.Действие = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоПерерабатываемымМатериалам.ПринятьНаХранение");
		КонецЕсли;
...
Функция КоличествоССуффиксом(Суффикс) Экспорт
	...
	ИначеЕсли Суффикс = "Забаланс" Тогда
		...
		СтруктураВозврата.КоличествоПоРНПТ   = "";
	Иначе
		ВызватьИсключение НСтр("ru = 'Неизвестный суффикс для количества:';
								|en = 'Unknown suffix for quantity:'") + " " + Суффикс;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
		...
		#Иначе
			ТекстИсключения = НСтр("ru = 'Попытка заполнения спецификации на клиенте.';
									|en = 'Attempt to fill in BOM on client.'");
			ВызватьИсключение ТекстИсключения;
		#КонецЕсли
	Иначе
		ТекущаяСтрока[ИмяПоля] = Неопределено;
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСпособОпределенияСебестоимостиСтрокой", СтруктураПараметров)
		...
		Если ТекущаяСтрока.СпособОпределенияСебестоимости = ПредопределенноеЗначение("Перечисление.СпособыОпределенияСебестоимости.ИзТекущегоДокумента") Тогда
			Представление = НСтр("ru = 'Равна сумме в данной строке';
								|en = 'Equals the amount in this line'");
		ИначеЕсли ТекущаяСтрока.СпособОпределенияСебестоимости = ПредопределенноеЗначение("Перечисление.СпособыОпределенияСебестоимости.ИзДокументаПродажи") Тогда
			Представление = НСтр("ru = 'Определяется из документа продажи';
								|en = 'Determined from the sales document'");
		ИначеЕсли ТекущаяСтрока.СпособОпределенияСебестоимости = ПредопределенноеЗначение("Перечисление.СпособыОпределенияСебестоимости.ИзДокументаПередачи") Тогда
			Представление = НСтр("ru = 'Определяется из документа передачи';
								|en = 'Determined from the transfer document'");
				...
				Если СтруктураПараметров.ИспользоватьРеглУчет Тогда
					Если СтруктураПараметров.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС") Тогда
						Представление = НСтр("ru = 'Себестоимость с НДС: %1% (%2%), Себестоимость без НДС: %3% (%4%)';
											|en = 'Cost including VAT: %1% (%2%), Cost excluding VAT: %3% (%4%)'");
					Иначе
				...
				Если СтруктураПараметров.ИспользоватьРеглУчет Тогда
					Если СтруктураПараметров.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС") Тогда
						Представление = НСтр("ru = 'Упр. с НДС: %1% (%2%), Регл. без НДС: %3% (%4%)';
											|en = 'Manag. including VAT: %1% (%2%), Local excluding VAT: %3% (%4%)'");
					Иначе
						Представление = НСтр("ru = 'Упр.: %1% (%2%), Регл.: %3% (%4%)';
											|en = 'Management: %1% (%2%), Local: %3% (%4%)'");
					КонецЕсли;
				Иначе
					Если СтруктураПараметров.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС") Тогда
						Представление = НСтр("ru = 'Упр. с НДС: %1% (%2%)';
											|en = 'Management including VAT: %1% (%2%)'");
					Иначе
...
Процедура ЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если Не СтруктураДействий.Свойство("ЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства", ПараметрыДействия) Тогда
		Возврат;
	КонецЕсли;
	
	Если ТекущаяСтрока.Производится Тогда
		ПараметрыДействия.Свойство("НазначениеПолуфабрикаты", ТекущаяСтрока.НазначениеОбеспечения);
	Иначе
		ПараметрыДействия.Свойство("НазначениеМатериалы", ТекущаяСтрока.НазначениеОбеспечения);
	КонецЕсли;
	
	ТекущаяСтрока.Назначение = ?(ТекущаяСтрока.Обособленно,
		ТекущаяСтрока.НазначениеОбеспечения,
...
Процедура ЗаполнитьНазначениеВЭтапеПроизводства(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения = Неопределено) Экспорт
	...
	Если Не СтруктураДействий.Свойство("ЗаполнитьНазначениеВЭтапеПроизводства") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПолучателяПоНаправлениюВыпуска(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		...
		ИначеЕсли ТекущаяСтрока.НаправлениеВыпуска = 
				...
				Если ПараметрыДействия.Свойство("ИспользоватьНесколькоСкладов")
				   И ПараметрыДействия.ИспользоватьНесколькоСкладов = Истина Тогда
					
					ТекущаяСтрока.Получатель = ЗначениеНастроекПовтИсп.ПолучитьЦеховуюКладовуюПоУмолчанию(
													ПараметрыДействия.Подразделение, ТекущаяСтрока.Получатель);
	...
	#Иначе
		ТекстИсключения = НСтр("ru = 'Попытка заполнения получателя на основании направления выпуска на клиенте.';
								|en = 'Attempt to fill in the recipient based on the direction of the issue on the client.'");
		ВызватьИсключение ТекстИсключения;
	#КонецЕсли
		...
		Если Кэш = Неопределено Тогда
			...
			#Иначе
				ТекстИсключения = НСтр("ru = 'Попытка получения коэффициента упаковки на клиенте.';
										|en = 'Attempt to receive packaging unit factor on client.'");
				ВызватьИсключение ТекстИсключения;
			#КонецЕсли
		Иначе
			Результат = Кэш;
...
Процедура ПроверитьЗаполнитьКлючСвязиСпецификации(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Если НЕ СтруктураДействий.Свойство("ПроверитьЗаполнитьКлючСвязиСпецификации") Тогда
		Возврат;
	КонецЕсли;
