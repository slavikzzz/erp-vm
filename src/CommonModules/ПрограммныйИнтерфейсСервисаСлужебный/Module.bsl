...
Функция СвойстваАвторизации(КодАбонента = Неопределено,
	...
	Если ТекущийПользователь = Неопределено И Токен = Неопределено Тогда
	...
	Если ТекущийПользователь = Неопределено Тогда
		...
		СохраняемоеЗначениеПароля = Токен;
		
		Если ИдентификаторПользователя <> ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор() Тогда
			ИмяПользователя = ПользователиИнформационнойБазы.ТекущийПользователь().Имя;
		Иначе
			ИмяПользователя = СтрШаблон("service user (%1)", Лев(Новый УникальныйИдентификатор, 8));
		КонецЕсли;
	...
	СвойстваАвторизации.Вставить("tenant", РаботаВМоделиСервиса.ЗначениеРазделителяСеанса());
	СвойстваАвторизации.Вставить("key", Константы.КлючОбластиДанных.Получить());
	
	Если Не (РаботаВМоделиСервиса.АутентификацияТокеномДоступаВнутреннихВызововПрограммногоИнтерфейса()
		И Токен = Неопределено) Тогда
		СвойстваАвторизации.Вставить("login", ИмяПользователя);
...
Функция АдресСервисаИзЛокальнойБазы() Экспорт
	...
	Если Не ЗначениеЗаполнено(АдресСервиса) Тогда
		...
		|%1'");
		СсылкаНаНастройку = "e1cib/data/Обработка.ПанельАдминистрированияБТС.Форма.НастройкиПодключенияКСервису1СФреш";
