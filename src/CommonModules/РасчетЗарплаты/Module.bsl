...
// Создаются виды расчета оплаты долей РК и СН среднего заработка.
// Если виды расчета требуются, они создаются.
Процедура СформироватьНачисленияОплатыДолейРКиСН() Экспорт
	РасчетЗарплатыВнутренний.СформироватьНачисленияОплатыДолейРКиСН();
КонецПроцедуры
...
Процедура СформироватьДвиженияНачисленийПоДоговорам(Движения, Отказ, Организация, ПериодРегистрации, Начисления) Экспорт
	...
	Для Каждого Строка Из Начисления Цикл
		...
		ИначеЕсли Не ЭтоСторнирование Тогда
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет даты запрета редактирования документов ""Отпуск"".';
									|en = 'Fills in closing dates of editing the Leave documents.'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Установить порядок определения месяца дохода в виде оплаты труда по умолчанию';
										|en = 'Set the procedure for determining the month of income in the form of wages by default'");
