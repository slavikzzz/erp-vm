...
Процедура ПриНажатииНаКнопкуОтправкиВКонтролирующийОрган(
	...
	ИначеЕсли КонтролирующийОрган = "ЦПРР" ИЛИ ТипЗнч(СсылкаНаОтчет) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиРаспределенныйРеестр") Тогда
...
Процедура ОтправитьВКонтролирующийОрганНеИспользуется1СОтчетность(СсылкаНаОтчет, КонтролирующийОрган, Форма, ЭтоОтправкаИзФормыОтчетность) Экспорт
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Поддерживается отправка только регламентированных отчетов. Отправка отчета прервана.';
								|en = 'Поддерживается отправка только регламентированных отчетов. Отправка отчета прервана.'");
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(ТекстСообщения);
		Возврат;
...
Процедура ПоказатьФормуПредложениеОформитьЗаявлениеНаПодключение(Организация = Неопределено, ПоказыватьКнопки = Неопределено) Экспорт
	...
	Иначе
		ЭтоРежимОблачной1СО = ПолучитьРежимОблачной1СО();
		Если ЭтоРежимОблачной1СО Тогда
			ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентПереопределяемый.ОткрытьФормуПредложениеОформитьЗаявлениеНаПодключениеОблачной1СО(
				ПарметрыОткрытия,
				ОписаниеОповещения,
...
Процедура ОткрытьФормуМастераЗаявленияНаПодключениеСлужебный(ДополнительныеПараметры) Экспорт
	...
	Если ПутьКОбъекту = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Подключение к электронному документообороту невозможно: недостаточно прав.';
										|en = 'Подключение к электронному документообороту невозможно: недостаточно прав.'"));
	Иначе
...
Процедура ОбновитьОтправкуИзПанелиОтправкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если СтрНайти(Форма.ИмяФормы, "Отчет.РегламентированныйОтчетПодтверждениеВидаДеятельности") > 0 Тогда
		КонтекстДлительнойОперации.Вставить("ТипСообщенияОтвета", 16);
	Иначе
		КонтекстДлительнойОперации.Вставить("ТипСообщенияОтвета", Неопределено);
	КонецЕсли;
...
Процедура ОсуществитьОбменПоОрганизацииИзПанелиОтправки() Экспорт
	...
	ТипСообщенияОтвета 	= ?(ДополнительныеПараметры.Свойство("ТипСообщенияОтвета"),
		ДополнительныеПараметры.ТипСообщенияОтвета,
		Неопределено);
...
Процедура ОтправитьПояснение(
	...
	Если СведенияОЗаполнении.ЕстьИзмененияПоНДС Тогда
		МассивСообщений.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Внимание!';
																	|en = 'Внимание!'"), , Новый Цвет(225, 40, 40)));
	...
	Если СведенияОЗаполнении.ЕстьНеОтвеченныеСтроки Тогда
		...
		МассивСообщений.Добавить(Новый ФорматированнаяСтрока(
								...
								|en = 'В ФНС будут отправлены все те строки пояснения, по которым указан ответ.
								|Уточнение по остальным строкам требования можно будет отправить позже в другом пояснении.'")));
	КонецЕсли;
...
Процедура ПроверитьВладельцаПриОткрытииПрисоединенныхФайловИзСпискаОтчетов(ПараметрыПроверки) Экспорт
	...
	Если ТипЗнч(ПараметрыПроверки.Владелец) = Тип("СправочникСсылка.МакетыПенсионныхДел") Тогда
		...
		ПараметрыПроверки.ТекстПредупреждения = НСтр("ru = 'Присоединение файлов к ""Макету пенсионного дела"" должно выполняться из основной формы.';
													|en = 'Присоединение файлов к ""Макету пенсионного дела"" должно выполняться из основной формы.'");
	КонецЕсли;
	...
	Если ТипЗнч(ПараметрыПроверки.Владелец) = Тип("СправочникСсылка.ЗаявлениеОНазначенииПенсии") Тогда
		...
		ПараметрыПроверки.ТекстПредупреждения = НСтр("ru = 'Присоединение файлов к ""Заявлению о назначении и доставке пенсии"" должно выполняться из основной формы.';
													|en = 'Присоединение файлов к ""Заявлению о назначении и доставке пенсии"" должно выполняться из основной формы.'");
	КонецЕсли;
...
Функция СвойстваОбменаПоСЭДОЧерезОператора(Организация = Неопределено, СброситьБезПолучения = Ложь) Экспорт
	...
	СвойстваОбмена 	= Неопределено;
	
	Если НЕ СброситьБезПолучения ИЛИ Организация <> Неопределено Тогда
		СвойстваОбмена =
			ПараметрыПриложения["ЭлектронныйДокументооборотСКонтролирующимиОрганами.СвойстваОбменаПоСЭДОЧерезОператора"];
		Если СвойстваОбмена = Неопределено Тогда
			СвойстваОбмена = Новый Соответствие;
		КонецЕсли;
		
		Если НЕ СброситьБезПолучения Тогда
			Результат = СвойстваОбмена[Организация];
			
			Если Результат <> Неопределено Тогда
				ДатаСеанса = ОбщегоНазначенияКлиент.ДатаСеанса();
				
				Если Результат.ДатаВремяПоследнегоОбновления + 10 * 60 > ДатаСеанса
					И Результат.ДатаВремяПоследнегоОбновления <= ДатаСеанса Тогда
					
					Возврат Результат;
				КонецЕсли;
			КонецЕсли;
			
			Результат = ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.СвойстваОбменаПоСЭДОЧерезОператора(
				Организация);
		КонецЕсли;
		
		Если СброситьБезПолучения И (ТипЗнч(Организация) = Тип("Массив")
			ИЛИ ТипЗнч(Организация) = Тип("ФиксированныйМассив")) Тогда
			
			Для каждого ЭлементМассиваОрганизаций Из Организация Цикл
				СвойстваОбмена[ЭлементМассиваОрганизаций] = Неопределено;
			КонецЦикла;
			
		Иначе
			СвойстваОбмена[Организация] = Результат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолучитьСообщенияСЭДОЗаПериодПослеПолученияКонтекста(Результат, ДополнительныеПараметры) Экспорт
	...
	ПараметрыПолучения 			= ДополнительныеПараметры.ПараметрыПолучения;
	...
	КонтекстЭДОКлиент.ПолучитьСообщенияСЭДОЗаПериод(
		ОповещениеОбратногоВызова,
		Организации,
		ДатаНачала,
		ДатаОкончания,
	...
	Если ВыполнитьСтандартнуюОтправкуОтчета Тогда
		...
		Если ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
			...
			Текст = НСтр("ru = 'Отправка отчета в электронном виде не предусмотрена законодательством.
												...
												|en = 'Отправка отчета в электронном виде не предусмотрена законодательством.
												|Отчет сдается в бумажном виде'");
		...
		Иначе
			...
			Если ЗначениеЗаполнено(КонтролирующийОрган) Тогда
				...
				Иначе
					...
					ЭтоОтправкаИзФормы1СОтчетность = ДлительнаяОтправкаКлиент.ЭтоФормаОтправкиОтчетности(КраткоеИмяФормы, Форма.ИмяФормы);
			...
			Иначе
				...
				Текст = НСтр("ru = 'Не указан контролирующий орган. Отправка невозможна';
							|en = 'Не указан контролирующий орган. Отправка невозможна'");
				РегламентированнаяОтчетностьКлиент.ПоказатьИВернутьОшибку(
					Форма, 
					Текст);
...
Функция ТребованияКИзображениямВМакетахПенсионныхДел(ДопустимыеТипыФайлов, МаксимальныйРазмерФайла, ИгнорироватьОшибки = Ложь) Экспорт
	...
	ПараметрыМетода.Вставить("Пояснение", 					НСтр("ru = 'Письма, содержащие изображения, не соответствующие указанным требованиям, могут быть не приняты СФР (бывш. ПФР).';
																	|en = 'Письма, содержащие изображения, не соответствующие указанным требованиям, могут быть не приняты СФР (бывш. ПФР).'"));
	ПараметрыМетода.Вставить("ИгнорироватьОшибки",          ИгнорироватьОшибки);
...
Функция ПолучитьРежимОблачной1СО()
	
	Возврат ДокументооборотСКОВызовСервера.ЭтоРежимОблачной1СО();
