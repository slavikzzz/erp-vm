...
Процедура ПриФормированииФайлаСИнформациейДляТехподдержки(Текст, ТехническаяИнформация) Экспорт
	...
	Текст = Текст + НСтр("ru = 'Версия прикладного решения: [ВерсияПрикладногоРешения];
					   ...
					   |en = 'Application version: [ВерсияПрикладногоРешения];
					   |EDL version: [ВерсияБЭД];
					   |1C:Enterprise platform version: [ВерсияПлатформы];
					   |Mode: [Режим];
					   |Cryptographic service provider (client): [КриптопровайдерКлиент];
					   |Cryptographic service provider (server): [КриптопровайдерСервер];
					   |Online support connection username: [ЛогинИнтернетПоддержка];
					   |Online support connection password: [ПарольИнтернетПоддержкаЗаполнен];
					   |Client parameters: [ПараметрыКлиента];'"); 
	...
	ТехническаяИнформация.Вставить("Режим", ?(ЭтоФайловаяИБ, НСтр("ru = 'Файловый';
																	|en = 'File'"), НСтр("ru = 'Серверный';
																							|en = 'Server'")));
	ТехническаяИнформация.Вставить("ЛогинИнтернетПоддержка", Логин);
	ТехническаяИнформация.Вставить("ПарольИнтернетПоддержкаЗаполнен", ?(ПарольЗаполнен, НСтр("ru = 'Заполнен';
																							|en = 'Filled'"), НСтр("ru = 'Не заполнен';
																														|en = 'Not filled in'")));
