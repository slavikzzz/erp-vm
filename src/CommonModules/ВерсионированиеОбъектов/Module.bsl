...
Процедура ВключитьВерсионированиеОбъекта(ИмяОбъекта, Знач ВариантВерсионирования = Неопределено) Экспорт
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Неизвестный вариант версионирования: ""%1""';
			|en = 'Unknown versioning option: %1.'"), ВариантВерсионирования);
	...
	Попытка
		...
		Если НомерВерсииСКоллизией = Неопределено Тогда
			...
			Запись.ПредупреждениеСинхронизации = НСтр("ru = 'Отклоненная версия (автоматическое разрешение конфликта).';
														|en = 'The rejected version (automatic conflict resolving).'", ОбщегоНазначения.КодОсновногоЯзыка());
			НаборЗаписей.Записать();
...
Функция ОтчетПоВерсииОбъекта(СсылкаНаОбъект, Знач ВерсияОбъекта = Неопределено, ПользовательскийНомерВерсии = Неопределено) Экспорт
	...
	Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '№ %1 / (%2) / %3';
																			|en = '#%1 / (%2) / %3'"), ПользовательскийНомерВерсии,
...
Функция РазмерДанныхСтрокой(Знач РазмерДанных)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 %2';
																		|en = '%1 %2'"), РазмерДанных, ЕдиницаИзмерения);
...
Функция ВывестиРеквизитыПоРазобранномуОбъекту(Результат, ВерсияОбъекта, СсылкаНаОбъект)
	...
	ВывестиТекстВОтчет(Результат, Секция, "R1C2", НСтр("ru = 'Реквизиты';
														|en = 'Attributes'"), ШрифтыСтиля.КрупныйШрифтТекста);
