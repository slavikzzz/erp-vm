...
Функция СоответсвиеПеречисленийСтатусам() Экспорт
	...
	СоответствиеВрем.Вставить(ПредопределенноеЗначение("Перечисление.СтатусыОтправки.ПринятЕстьОшибки"), 
		ДокументооборотСКОКлиентСервер.СтатусСданоЕстьЗамечания());
	...
	Для каждого КлючЗначение Из СоответствиеВрем Цикл
		Статус = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																				|en = '%1'"), КлючЗначение.Значение);
...
Функция СоответсвиеСтатусовПеречислениям() Экспорт
	...
	СоответствиеВрем.Вставить(ДокументооборотСКОКлиентСервер.СтатусСданоЕстьЗамечания(),
		ПредопределенноеЗначение("Перечисление.СтатусыОтправки.ПринятЕстьОшибки"));
...
Функция СформироватьИмяФайлаПакета(Параметры) Экспорт
	...
	ТипФайлаВыгрузки = Параметры.ТипФайлаВыгрузки;
	Если ТипФайлаВыгрузки = ТипФайлаВыгрузкиИсполнениеКонтрактовГОЗ() Тогда
	...
	ИначеЕсли ТипФайлаВыгрузки = ТипФайлаВыгрузкиКассовыйПланГОЗ() Тогда
		
		ДатаСоставленияСтрока = Формат(Параметры.ДатаОтчета, ФорматДаты);
		ИНН = Параметры.ИНН;
		ИдентификаторФайла = Параметры.ИдентификаторФайла;
		ИмяФайла = СтрШаблон("%1_%2_%3.zip",
							ИНН, ДатаСоставленияСтрока, ИдентификаторФайла);
							
	ИначеЕсли ТипФайлаВыгрузки = ТипФайлаВыгрузкиЦенаПродукцииГОЗ() Тогда
		
		Если ЗначениеЗаполнено(Параметры.КПП) Тогда 
			ИмяФайла = СтрШаблон("%1_%2_%3_%4_01.zip", Параметры.ИНН, Параметры.КПП, Параметры.Type, Параметры.ИдентификаторФайла);
		Иначе
			ИмяФайла = СтрШаблон("%1_000000000_%2_%3_01.zip", Параметры.ИНН, Параметры.Type, Параметры.ИдентификаторФайла);
		КонецЕсли;
...
Функция ТипФайлаВыгрузкиИсполнениеКонтрактовГОЗ() Экспорт
	
	Возврат "ИсполнениеКонтрактовГОЗ";
	
КонецФункции

Функция ТипФайлаВыгрузкиКассовыйПланГОЗ() Экспорт
	
	Возврат "КассовыйПланГОЗ";
	
КонецФункции

Функция ТипФайлаВыгрузкиЦенаПродукцииГОЗ() Экспорт
	
	Возврат "ЦенаПродукцииГОЗ";
	
КонецФункции

Функция ЭтоСертификатФизическогоЛица(ДанныеСертификата) Экспорт
	
	Результат = Ложь;
	
	Если ТипЗнч(ДанныеСертификата) = Тип("Структура")
		И ДанныеСертификата.Свойство("ВладелецСтруктура") Тогда
		Результат = НЕ ДанныеСертификата.ВладелецСтруктура.Свойство("OID1_2_643_100_4");
	ИначеЕсли ТипЗнч(ДанныеСертификата) = Тип("СертификатКриптографии") Тогда
		Результат = НЕ ДанныеСертификата.Субъект.Свойство("OID1_2_643_100_4");
	КонецЕсли;
...
Функция НовыеПараметрыАрхивации() Экспорт
	
	ПараметрыАрхивации = Новый Структура("СтрокаДанныхПодписи,
											|ИмяФайлаПодписи,
											|АдресФайлаВыгрузки,
											|ИмяФайлаВыгрузки,
											|ОтчетСсылка");
	ПараметрыАрхивации.Вставить("Доверенность", Неопределено);
	ПараметрыАрхивации.Вставить("ИНН", "");
	ПараметрыАрхивации.Вставить("ТипФайлаВыгрузки", "");
	
	Возврат ПараметрыАрхивации;
	
КонецФункции

Функция ПредставленияСправочниковМЧД() Экспорт
	
	Представления = Новый Соответствие;
	Представления.Вставить("МашиночитаемыеДоверенностиРаспределенныйРеестр", "Машиночитаемая доверенность (ФНС, распределенный реестр)");
	Представления.Вставить("МашиночитаемыеДоверенностиФНС", "Машиночитаемая доверенность (ФНС)");
	Представления.Вставить("МашиночитаемыеДоверенности", "Машиночитаемая доверенность (единый формат)");
