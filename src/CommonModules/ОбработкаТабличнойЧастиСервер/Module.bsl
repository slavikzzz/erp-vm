			...
			Если Выборка.Следующий() Тогда
				...
				Если ЗначениеЗаполнено(Выборка.Характеристика) Тогда
					...
					ТекстОшибки = НСтр("ru = 'Номенклатуре ""%Номенклатура%"" с характеристикой ""%Характеристика%"" уже сопоставлена другая номенклатура поставщика (%КоличествоНоменклатурыПартнера%)';
										|en = 'Another vendor part number is already matched to variant ""%Характеристика%"" of item ""%Номенклатура%"" (%КоличествоНоменклатурыПартнера%)'");
				...
				Иначе
					...
					ТекстОшибки = НСтр("ru = 'Номенклатуре ""%Номенклатура%"" уже сопоставлена другая номенклатура поставщика (%КоличествоНоменклатурыПартнера%)';
										|en = 'Another vendor part number is already matched to item ""%Номенклатура%"" (%КоличествоНоменклатурыПартнера%)'");
					ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Номенклатура%",                     Выборка.Номенклатура);
					ТекстОшибки = СтрЗаменить(ТекстОшибки, "%КоличествоНоменклатурыПартнера%", Выборка.КоличествоНоменклатурыПартнера);
...
Процедура УстановитьПризнакИспользованияПлановойСтоимостиВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт

	Перем ПараметрыДействия;
	
	Если Не СтруктураДействий.Свойство("УстановитьПризнакИспользованияПлановойСтоимости", ПараметрыДействия) Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыУчетнойПолитики = НастройкиНалоговУчетныхПолитик.ДействующиеПараметрыНалоговУчетныхПолитикНаДату(
		...
		ТекущаяСтрока.ДатаПроизводства);
	
	Если НЕ ПараметрыУчетнойПолитики = Неопределено Тогда
		ТекущаяСтрока.ИспользоватьПлановуюСтоимость = ПараметрыУчетнойПолитики.УчетГотовойПродукцииПоПлановойСтоимости;
	КонецЕсли;

КонецПроцедуры

Процедура ПроверитьЗаполнитьЦенуВыпускаВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Перем СтруктураПараметровДействия, ЗначениеСвойства;
	
	Если Не СтруктураДействий.Свойство("ПроверитьЗаполнитьЦенуВыпуска", СтруктураПараметровДействия) Тогда
		Возврат;
	КонецЕсли;
	Если Не СтруктураПараметровДействия.Свойство("ВидЦены", ЗначениеСвойства) ИЛИ Не ЗначениеЗаполнено(ЗначениеСвойства) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьЗаполнитьОбеспечениеВСтрокеТЧЭтапаПроизводства(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Перем ПараметрыДействия;
	
	Если НЕ СтруктураДействий.Свойство("ПроверитьЗаполнитьОбеспечениеВЭтапеПроизводства", ПараметрыДействия) Тогда
		Возврат;
	КонецЕсли;
	
	Если ТекущаяСтрока.Производится
		И ТекущаяСтрока.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.МногооборотнаяТара Тогда
		ТекущаяСтрока.Производится = Ложь;
	КонецЕсли;
	
	ПараметрыОбеспечения = ОбеспечениеПроизводства.ПараметрыОбеспеченияМатериалаЭтапа(
								ТекущаяСтрока.Номенклатура,
								ТекущаяСтрока.Характеристика,
								ТекущаяСтрока.Производится,
								ПараметрыДействия.Подразделение,
								ТекущаяСтрока.Спецификация,
								ПараметрыДействия.ЗаказНаПроизводство);
	
	Если ПараметрыДействия.Свойство("ВариантОбеспечения") И ЗначениеЗаполнено(ПараметрыДействия.ВариантОбеспечения) Тогда // отражение выполненного этапа производства
		ТекущаяСтрока.ВариантОбеспечения = ПараметрыДействия.ВариантОбеспечения;
		
		// Проверим, что в отражении выполненного этапа складом является цеховая кладовая текущего подразделения.
		Если ЗначениеЗаполнено(ТекущаяСтрока.Склад) Тогда
			РеквизитыСклада = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущаяСтрока.Склад, "Подразделение, ЦеховаяКладовая");
			Если Не РеквизитыСклада.ЦеховаяКладовая
				Или Не РеквизитыСклада.Подразделение = ПараметрыДействия.Подразделение Тогда
				ТекущаяСтрока.Склад = Неопределено;
			КонецЕсли;
		КонецЕсли;
		
	Иначе
		ТекущаяСтрока.ВариантОбеспечения = ПараметрыОбеспечения.ВариантОбеспечения;
	КонецЕсли;
	
	Если ПараметрыДействия.Свойство("Склад") И ЗначениеЗаполнено(ПараметрыДействия.Склад) Тогда
		ТекущаяСтрока.Склад = ПараметрыДействия.Склад;
	Иначе
		ТекущаяСтрока.Склад = ПараметрыОбеспечения.Склад;
	КонецЕсли;
	
	Если ПараметрыДействия.Свойство("Обособленно") И ЗначениеЗаполнено(ПараметрыДействия.Обособленно) Тогда
		ТекущаяСтрока.Обособленно = ПараметрыДействия.Обособленно;
	Иначе
		ТекущаяСтрока.Обособленно = ПараметрыОбеспечения.Обособленно Или ТекущаяСтрока.Производится;
	КонецЕсли;
	
КонецПроцедуры

Процедура ПроверитьЗаполнитьСкладПроизводстваВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт

	Перем ПараметрыДействия;

	Если НЕ СтруктураДействий.Свойство("ПроверитьЗаполнитьСкладПроизводства", ПараметрыДействия) Тогда
		Возврат;
	КонецЕсли;
	
	ОбеспечениеПроизводства.ЗаполнитьСкладВСтрокеЭтапаПроизводства(
		ТекущаяСтрока,
		ПараметрыДействия);
...
Процедура ПроверитьЗаполнитьДолюСтоимости(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если НЕ СтруктураДействий.Свойство("ПроверитьЗаполнитьДолюСтоимости", ПараметрыРаспределенияЗатрат) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ПроизводствоКлиентСервер.ДоляСтоимостиРассчитывается(ТекущаяСтрока, ПараметрыРаспределенияЗатрат) Тогда
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
			РезультатРасчета = ПроизводствоСервер.РассчитатьДолюСтоимостиНоменклатуры(ТекущаяСтрока, ПараметрыРаспределенияЗатрат);
		Иначе
...
Процедура ЗаполнитьНастройкуСчетовУчетаПоУмолчанию(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения = Неопределено) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьНастройкуСчетовУчетаПоУмолчанию", СтруктураПараметровДействия) Тогда
		НастройкаСчетовУчетаСервер.ЗаполнитьНастройкуСчетовУчетаПоУмолчанию(ТекущаяСтрока, СтруктураПараметровДействия);
	КонецЕсли;
	//-- НЕ УТ
...
Процедура ЗаполнитьЦенуПередачиПродукцииДавальцуВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если ЗначениеЗаполнено(ВидЦеныПередачи) Тогда
		...
		ТекущаяСтрока.Цена = ЦеныПредприятияЗаполнениеСервер.ПолучитьЦенуПоОтбору(СтруктураПараметровОтбора);
...
Процедура ЗаполнитьПризнакЦеховаяКладовая(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	ПроизводствоСервер.ЗаполнитьПризнакЦеховаяКладовая(ТекущаяСтрока, ПараметрыЗаполнения, КэшированныеЗначения);	
...
Функция МетодыКВыполнению(СтруктураДействий)
	...
	//++ НЕ УТКА
	Соответствие.Добавить("ПроверитьЗаполнитьСкладПроизводства",						"ОбработкаТабличнойЧастиСервер.ПроверитьЗаполнитьСкладПроизводстваВСтрокеТЧ");
	//-- НЕ УТКА
	...
	//++ НЕ УТКА
	Соответствие.Добавить("ПроверитьЗаполнитьОбеспечениеВЭтапеПроизводства",			"ОбработкаТабличнойЧастиСервер.ПроверитьЗаполнитьОбеспечениеВСтрокеТЧЭтапаПроизводства");
	//-- НЕ УТКА
	
	//++ НЕ УТ
	Соответствие.Добавить("ЗаполнитьНаправлениеВыпуска",								"ОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьНаправлениеВыпуска");
	Соответствие.Добавить("ЗаполнитьПолучателяПоНаправлениюВыпуска",					"ОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьПолучателяПоНаправлениюВыпуска");
	Соответствие.Добавить("ЗаполнитьПризнакЦеховаяКладовая",							"ОбработкаТабличнойЧастиСервер.ЗаполнитьПризнакЦеховаяКладовая");
	//-- НЕ УТ
	...
	//++ НЕ УТКА
	Соответствие.Добавить("ЗаполнитьДатуПроизводства",									"ОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьДатуПроизводства");
	Соответствие.Добавить("УстановитьПризнакИспользованияПлановойСтоимости",			"ОбработкаТабличнойЧастиСервер.УстановитьПризнакИспользованияПлановойСтоимостиВСтрокеТЧ");
	Соответствие.Добавить("ПроверитьЗаполнитьЦенуВыпуска",								"ОбработкаТабличнойЧастиСервер.ПроверитьЗаполнитьЦенуВыпускаВСтрокеТЧ");
	//-- НЕ УТКА

	//++ НЕ УТ
	Соответствие.Добавить("ЗаполнитьЦенуПередачиПродукцииДавальцу",						"ОбработкаТабличнойЧастиСервер.ЗаполнитьЦенуПередачиПродукцииДавальцуВСтрокеТЧ");
	//-- НЕ УТ
	...
	//++ НЕ УТКА
	Соответствие.Добавить("ПересчитатьСуммуУслугиЗаказДавальца",						"ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьСуммуУслугиВСтрокеТЧЗаказДавальца");
	Соответствие.Добавить("ПересчитатьСуммуЗаказДавальца",								"ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьСуммуВСтрокеТЧЗаказДавальца");
	//-- НЕ УТКА
	...
	//++ НЕ УТКА
	Соответствие.Добавить("ПересчитатьЦенуПоСуммеЗаказДавальца",						"ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьЦенуПоСуммеВСтрокеТЧЗаказДавальца");
	//-- НЕ УТКА
	...
	//++ НЕ УТ
	Соответствие.Добавить("ПроверитьЗаполнитьСпецификацию",								"ОбработкаТабличнойЧастиКлиентСервер.ПроверитьЗаполнитьСпецификацию");
	Соответствие.Добавить("ПроверитьЗаполнитьДолюСтоимости",							"ОбработкаТабличнойЧастиСервер.ПроверитьЗаполнитьДолюСтоимости");
	Соответствие.Добавить("ПересчитатьДолюСтоимости",									"ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьДолюСтоимостиВСтрокеТЧ");
	//-- НЕ УТ
	...
	//++ НЕ УТКА
	Соответствие.Добавить("ЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства",			"ОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства");
	Соответствие.Добавить("ЗаполнитьНазначениеВЭтапеПроизводства",						"ОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьНазначениеВЭтапеПроизводства");
	//-- НЕ УТКА
	...
	//++ НЕ УТКА
	Соответствие.Добавить("ПроверитьЗаполнитьКлючСвязиСпецификации",					"ОбработкаТабличнойЧастиКлиентСервер.ПроверитьЗаполнитьКлючСвязиСпецификации");
	//++ Устарело_Переработка24
	Соответствие.Добавить("ЗаполнитьНазначениеМатериалыВСтрокеСверхЗаказа",				"ОбработкаТабличнойЧастиСервер.ЗаполнитьНазначениеМатериалыВСтрокеСверхЗаказа");
	Соответствие.Добавить("ЗаполнитьПризнакДоступноОформлениеОтчетаДавальцу",			"ОбработкаТабличнойЧастиСервер.ЗаполнитьПризнакДоступноОформлениеОтчетаДавальцу");
	//-- Устарело_Переработка24
	Соответствие.Добавить("ЗаполнитьДействиеПоПерерабатываемымМатериалам",				"ОбработкаТабличнойЧастиКлиентСервер.ЗаполнитьДействиеПоПерерабатываемымМатериаламВСтрокеТЧ");
	Соответствие.Добавить("ЗаполнитьВладелецИзделияДоступен",							"ОбработкаТабличнойЧастиСервер.ЗаполнитьВладелецИзделияДоступен");
	//-- НЕ УТКА
...
Процедура ЗаполнитьНазначениеМатериалыВСтрокеСверхЗаказа(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьНазначениеМатериалыВСтрокеСверхЗаказа") И ТекущаяСтрока.КодСтроки = 0 Тогда
		...
		Если ТекущаяСтрока.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар")
			...
			Назначение = КэшированныеЗначения.НазначенияМатериала.Получить(ЗаказДавальца);
			Если Назначение = Неопределено Тогда
				
				Назначение = Документы.ЗаказДавальца.НазначениеМатериалы(ЗаказДавальца);
				КэшированныеЗначения.НазначенияМатериала.Вставить(ЗаказДавальца, Назначение);
...
Процедура ЗаполнитьВладелецИзделияДоступен(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Если Не СтруктураДействий.Свойство("ЗаполнитьВладелецИзделияДоступен") Тогда
		Возврат;
	КонецЕсли;
	
	ВнутренняяПереработкаСервер.ЗаполнитьВладелецИзделияДоступен(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
