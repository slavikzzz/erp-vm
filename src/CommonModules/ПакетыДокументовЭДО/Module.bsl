...
Функция ЗаголовокОтображенияПакетов(КоличествоДокументов) Экспорт
	Если КоличествоДокументов Тогда
		ШаблонСтроки = НСтр("ru = ';Пакет из %1 документа;;Пакет из %1 документов;Пакет из %1 документов;Пакет из %1 документов';
							|en = ';Batch from %1 document;;Batch from %1 documents;Batch from %1 documents;Batch from %1 documents'");
...
Функция СоздатьПакетДокументов(ЭлектронныеДокументы, КонтекстДиагностики) Экспорт
	...
	Исключение
		...
		ЗаголовокОшибки = НСтр("ru = 'Не удалось создать пакет документов';
								|en = 'Cannot create a document batch'");
...
Функция УдалитьДокументИзПакета(ИдентификаторПакета, ЭлектронныйДокумент, КонтекстДиагностики = Неопределено) Экспорт
	...
	ВидОперации = НСтр("ru = 'Удаление электронного документа из пакета';
						|en = 'Delete the electronic document from the batch'");
...
Функция ДобавитьДокументВПакет(ИдентификаторПакета, ЭлектронныйДокумент, КонтекстДиагностики = Неопределено) Экспорт
	...
	ВидОперации = НСтр("ru = 'Добавление электронного документа в пакет';
						|en = 'Add an electronic document to the batch'");
	...
	Исключение
		...
		ВидОперации = НСтр("ru = 'Добавление электронного документа из пакета';
							|en = 'Add an electronic document from the batch'");
		ТекстОшибки = ОбщегоНазначенияБЭД.ПодробноеПредставлениеИнформацииОбОшибке(ИнформацияОбОшибке());
		ДобавитьОшибкуИзмененияСоставаПакетаИсходящегоЭДО(КонтекстДиагностики, ВидОперации,
			ИдентификаторПакета, ЭлектронныйДокумент, ТекстОшибки);
	КонецПопытки;
...
Функция СостоянияДокументовПакетаОднородны(МассивСостояний, ИдентификаторПакета, Действие, КонтекстДиагностики) Экспорт
	...
	ВидОперации =  СтрШаблон(НСтр("ru = 'Действие %1';
									|en = 'Action %1'"), ПредставлениеДействия);
	...
	КраткоеПредставление = СтрШаблон(НСтр("ru = 'Операция ""%1"" недоступна для документов пакета.
		|Состояния документов пакета неоднородны.';
		|en = 'The ""%1"" operation is unavailable for the batch documents.
		|Batch document states are not the same.'"), ПредставлениеДействия);
	
	МассивСтрок = Новый Массив; // Массив из Строка
	МассивСтрок.Добавить(КраткоеПредставление);
	МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Идентификатор пакета: %1';
										|en = 'Batch ID: %1'"), ИдентификаторПакета));
	ПодробноеПредставление = СтрСоединить(МассивСтрок, Символы.ПС);
...
Функция ТекстОПревышенииМаксимальногоРазмераПакетаПриСозданииДокумента(ОграниченияОбъемаПакетаДокументов) Экспорт
	...
	Возврат
		СтрШаблон(НСтр("ru = 'Не удалось создать документ из-за ограничений оператора %1 (размер файла не более %2 Кб).';
						|en = 'Cannot create the document due to the limitations of the %1 service operator (file size is not more than %2 KB).'"), 
...
Функция ТекстОПревышенииМаксимальногоКоличестваДокументовПередОтправкой(ПредставлениеДокумента,
	...
	Возврат
		...
		|Удалите документы из пакета.';
		|en = 'Cannot send the %1 package due to the limitations of the %2 service operator (no more than %3 documents in the package).
		|Delete the documents from the package.'"), ПредставлениеДокумента,
...
Функция ТекстОПревышенииМаксимальногоРазмераПакетаПередОтправкой(ПредставлениеДокумента,
	...
	Возврат
		...
		|Удалите документы из пакета.';
		|en = 'Cannot send the %1 package due to the limitations of the %2 service operator (the package size is not more than %3 KB).
		|Delete the documents from the package.'"), ПредставлениеДокумента,
		ОграниченияОбъемаПакетаДокументов.ОператорЭДО, ОграниченияОбъемаПакетаДокументов.МаксимальныйРазмерПакетаЭД);
...
Функция ТекстОПревышенииМаксимальногоРазмераДокументаПередОтправкой(ПредставлениеДокумента,
	...
	Возврат
		...
		|Переформируйте документ меньшим размером.';
		|en = 'Cannot send the %1 document due to the limitations of the %2 service operator (the package size is not more than %3 KB).
		|Reformat the document with a smaller size.'"), ПредставлениеДокумента,
		ОграниченияОбъемаПакетаДокументов.ОператорЭДО, ОграниченияОбъемаПакетаДокументов.МаксимальныйРазмерПакетаЭД);
...
Функция ИзменениеСоставаПакетаНедоступно(СостоянияДокументовПоЭДО, КонтекстДиагностики = Неопределено)
	...
	Если КонтекстДиагностики <> Неопределено И Результат Тогда
		...
		ПодробноеПредставление = СтрШаблон(
			...
			|en = 'Batch editing is available for documents with state: %1.
			|The state of the %2 documents is different.'"),
...
Функция ВидОшибкиСостоянияДокументовПакетаНеоднородны()
	...
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Состояние документов пакета неоднородно';
										|en = 'Batch document states are not the same'");
	ВидОшибки.ОписаниеРешения = НСтр("ru = 'Выполнить действие отдельно для электронного документа';
									|en = 'Perform an action separately for the electronic document'");
	Возврат ВидОшибки;
КонецФункции
...
Процедура ДобавитьОшибкуДокументыВСоставеДругогоПакета(ДокументыВСоставеДругогоПакета, КонтекстДиагностики)
	...
	КраткоеПредставление = НСтр("ru = 'Не удалось создать пакет документов по причине:
		...
		|en = 'Cannot create a document batch. Reason:
		|electronic documents belong to another batch.'");
...
Процедура ДобавитьОшибкуИзмененияСоставаПакетаВходящегоЭДО(КонтекстДиагностики, ВидОперации, ЭлектронныйДокумент)
	...
	МассивСтрок.Добавить(НСтр("ru = 'Изменение состава пакета входящих документов недоступно.';
								|en = 'Cannot change the incoming document batch.'"));
	ПодробноеПредставление = СтрСоединить(МассивСтрок, Символы.ПС);
...
Процедура ДобавитьОшибкуИзмененияСоставаПакетаИсходящегоЭДО(КонтекстДиагностики, ВидОперации, ИдентификаторПакета, ЭлектронныйДокумент, ТекстОшибки)
	...
	МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Идентификатор пакета: %1';
										|en = 'Batch ID: %1'"), ИдентификаторПакета));
	МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Внутренний номер документа: %1';
										|en = 'Internal document number: %1'"), СвойстваДокумента.Номер));
