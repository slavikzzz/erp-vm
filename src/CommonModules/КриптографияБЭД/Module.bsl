...
#Область РегистрыСведений_ПодписываемыеВидыЭД_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = '1С:БЭД: обновление подписываемых видов электронных документов';
									|en = '1C:EDL: update of electronic document kinds being signed'");
...
#Область Версия_1_9_10
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		...
		Обработчик.Комментарий = НСтр(
			...
			|en = '1C:EDL: create digital signature and encryption applications (initial filling).'");
...
// дополняя их клиентскими отпечатками (в качестве параметра РезультатыПолученияОтпечатков необходимо передать
// результат выполнения метода См. КриптографияБЭДКлиент.ПолучитьОтпечаткиСертификатов).
// 
// Параметры:
//  ВидОперации - Строка
//  КонтекстДиагностики - Неопределено - ошибка получения отпечатков не будет добавлена в контекст диагностики,
...
// 
// Возвращаемое значение:
//  См. КриптографияБЭДКлиентСервер.ПроверитьКвалифицировнныйСертификат
...
Функция ПроверитьПодпись(МенеджерКриптографии, ИсходныеДанные, Подпись, КонтекстДиагностики) Экспорт
	...
	РезультатПроверки = ПроверитьПодписьЧерезСервисDSS(ИсходныеДанные, Подпись);
...
Функция Зашифровать(Данные, СертификатКриптографии, КонтекстДиагностики) Экспорт
	...
	Если МенеджерКриптографии = Неопределено Тогда
		КраткийТекстОшибки = НСтр("ru = 'Не удалось получить менеджер криптографии';
									|en = 'Cannot get a crypto manager'");
...
Процедура УстановитьПрограммуСертификата(Сертификат, Программа) Экспорт
	...
	Исключение
		...
		ОбработкаНеисправностейБЭД.ОбработатьОшибку(НСтр("ru = 'Изменение программы криптографии сертификата';
														|en = 'Change certificate cryptography software'"),
...
Процедура ДобавитьОшибкуДляСертификатаНетПодписываемогоВидаДокумента(КонтекстДиагностики, ВидОперации, Сертификат, ВидДокумента) Экспорт
	...
	ТекстОшибки = НСтр("ru = 'Для сертификата в списке ""Подписываемые виды документов"" нет данного вида документа.';
						|en = 'There is no such type of document for the certificate in the “Signed document types” list.'");
	...
	Если ЕстьОшибкаПолученияОтпечатков(РезультатыПолученияОтпечатков, ТекстОшибки) Тогда
		Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации,
			...
			НСтр("ru = 'Ошибка при получении отпечатков сертификатов.';
				|en = 'An error occurred while receiving the certificate thumbprints.'"));
		ОбработкаНеисправностейБЭД.ДобавитьОшибку(КонтекстДиагностики, Ошибка,
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().Криптография);
	КонецЕсли;
...
Функция ВидОшибкиПодписьНеверна() Экспорт
	...
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Не удалось обработать объект';
										|en = 'Cannot process the object'");
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Объект содержит невалидные подписи';
										|en = 'The object contains invalid signatures'");
	ОписаниеРешения = НСтр("ru = '<a href = ""Выполните"">Выполните</a> диагностику криптографии. Если это не помогло решить проблему,
			...
			|en = '<a href = ""Выполните"">Perform</a> cryptography diagnostics. If that did not solve the problem,
			|read the articles <a href = ""http://1c-edo.ru/handbook/24/2595/#1"">here</a> and <a href = ""https://its.1c.ru/db/metbud81#content:5784:hdoc"">here</a>.'");
...
Функция ШтампЭлектроннойПодписи(ДанныеДляФормирования) Экспорт
	...
	Если ДанныеДляФормирования.ЭтоИнформацияОтправителя Тогда
		...
		Если ЭтоИсходящийДокумент
			...
			ТекстШтампа = НСтр("ru = 'Документ подписан в 1С-ЭДО';
								|en = 'The document is signed in 1C:EDI'");
		ИначеЕсли ЭтоИсходящийДокумент
			...
			ТекстШтампа = НСтр("ru = 'Документ подписан и передан через 1С-ЭДО';
								|en = 'The document is signed and transmitted via 1C:EDI'");
		...
		ИначеЕсли Не ЭтоИсходящийДокумент И ДанныеДляФормирования.ЕстьОтветнаяПодпись Тогда
			ТекстШтампа = НСтр("ru = 'Документ получен и подписан в 1С-ЭДО';
								|en = 'The document received and signed in 1C:EDI'");
	...
	Если ВидПодписи = Перечисления.ВидыЭлектронныхПодписей.Простая Тогда
		ТекстШтампа = НСтр("ru = 'Документ подписан простой электронной подписью';
							|en = 'The document is signed with a simple electronic signature'");
	КонецЕсли;
...
// Возвращаемое значение:
// 	см. КриптографияБЭДКлиентСервер.НовыйРезультатПроверкиПодписи
Функция ПроверитьПодписьЧерезСервисDSS(ИсходныеДанные, Подпись, НастройкиСервисаОблачнойПодписи = Неопределено)
	...
	Иначе
		...
		РезультатПроверки.ОписаниеОшибки = НСтр("ru = 'Настройки сервиса DSS не доступны.';
												|en = 'The DSS service settings are not available.'");
		Возврат РезультатПроверки;
	КонецЕсли;
...
Функция ПроверитьПодписьЧерезКриптопровайдер(МенеджерКриптографии, ИсходныеДанные, Подпись, КонтекстДиагностики)
	...
	Если РезультатПроверкиПодписи.ПодписьВерна Тогда
		СертификатКриптографии = Новый СертификатКриптографии(РезультатПроверкиПодписи.Сертификат);
		РезультатПроверкиКЭП = ПроверитьСертификат(МенеджерКриптографии, СертификатКриптографии, КонтекстДиагностики, Подпись);
		Если Не РезультатПроверкиКЭП.Успех Тогда
			РезультатПроверки.СвойстваПодписи.ПодписьВерна = Ложь;
			Ошибка = КриптографияБЭДКлиентСервер.СформироватьОшибкуПоПротоколуПроверкиКвалификации(СертификатКриптографии, РезультатПроверкиКЭП.Протокол);
			РезультатПроверки.СвойстваПодписи.Комментарий = Ошибка.ПодробноеПредставлениеОшибки;
			РезультатПроверки.ОписаниеОшибки = Ошибка.ПодробноеПредставлениеОшибки;
		КонецЕсли;
	КонецЕсли;
...
Функция НастройкиСервисаОблачнойПодписи()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодписьСервисаDSS") Тогда
		МодульСервисКриптографииDSS = ОбщегоНазначения.ОбщийМодуль("СервисКриптографииDSS");
		Если МодульСервисКриптографииDSS.ИспользоватьСервисОблачнойПодписи() Тогда
			НастройкипоУмолчанию = МодульСервисКриптографииDSS.НастройкиПодключенияСлужебнойУчетнойЗаписи();
			Если НастройкипоУмолчанию.Свойство("Результат") Тогда
				Возврат НастройкипоУмолчанию.Результат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
