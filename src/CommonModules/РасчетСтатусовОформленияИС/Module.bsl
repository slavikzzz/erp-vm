...
Процедура ПередЗаписьюДокумента(ИмяОпцииИспользованияПодсистемы, Источник, Отказ) Экспорт
	...
	Если ЗначениеЗаполнено(Источник.Ссылка)
			И Не Источник.ДополнительныеСвойства.Свойство("ПредыдущийДокументОснование") Тогда
...
Процедура ПриЗаписиДокумента(ИмяОпцииИспользованияПодсистемы, Источник, Отказ, Модуль) Экспорт
		...
		Если Источник.ДополнительныеСвойства.Свойство("ПредыдущийДокументОснование")
		 ...
		 И Источник.ДополнительныеСвойства.ПредыдущийДокументОснование <> Источник.ДокументОснование Тогда
			...
			Если ИнтеграцияИС.РасчетСтатусовОформленияВыполняетсяВнеТранзакцииЗаписи() Тогда
				РегистрыСведений.СтатусыОформленияКРасчетуИС.ДобавитьОснование(Источник.ДополнительныеСвойства.ПредыдущийДокументОснование);
			Иначе
	...
	Иначе
		...
		Если ИнтеграцияИС.РасчетСтатусовОформленияВыполняетсяВнеТранзакцииЗаписи() Тогда
			ОтметитьДокументОснованиеБезРасчетаСтатусов(Источник);
		Иначе
			Модуль.РассчитатьСтатусОформленияДокумента(Источник);
		КонецЕсли;
...
Процедура РасчетСтатусовОформленияИСДляОснований(Источники) Экспорт
	...
	ТипИсточников = ТипЗнч(Источники[0]);
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ЕГАИС") Тогда
		ИмяОпции = "ВестиСведенияДляДекларацийПоАлкогольнойПродукции";
		ИмяМодуля = "РасчетСтатусовОформленияЕГАИС";
		РассчитатьДляОснованийДляПодсистемы(ИмяОпции, ИмяМодуля, Источники, ТипИсточников);
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ВетИС") Тогда
		ИмяОпции = "ВестиУчетПодконтрольныхТоваровВЕТИС";
		ИмяМодуля = "РасчетСтатусовОформленияВЕТИС";
		РассчитатьДляОснованийДляПодсистемы(ИмяОпции, ИмяМодуля, Источники, ТипИсточников);
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП") Тогда
		ИмяОпции = "ВестиУчетМаркируемойПродукцииИСМП";
		ИмяМодуля = "РасчетСтатусовОформленияИСМП";
		РассчитатьДляОснованийДляПодсистемы(ИмяОпции, ИмяМодуля, Источники, ТипИсточников);
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ЗЕРНО") Тогда
		ИмяОпции = "ВестиУчетЗернаИПродуктовПереработкиЗЕРНО";
		ИмяМодуля = "РасчетСтатусовОформленияЗЕРНО";
		РассчитатьДляОснованийДляПодсистемы(ИмяОпции, ИмяМодуля, Источники, ТипИсточников);
	КонецЕсли;
...
Процедура РасчетСтатусовОформленияПослеЗаписиДокумента(Параметры, АдресРезультата) Экспорт
	...
	Источники = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Ссылка);
	
	НачатьТранзакцию();
	Попытка
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.СтатусыОформленияКРасчетуИС");
		ЭлементБлокировки.УстановитьЗначение("Основание", Ссылка);
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		Блокировка.Заблокировать();
		
		Если Не ЗначениеЗаполнено(РегистрыСведений.СтатусыОформленияКРасчетуИС.ТекстНадписиВОбъекте(Ссылка)) Тогда
			ЗафиксироватьТранзакцию();
			Возврат;
		КонецЕсли;
		
		РасчетСтатусовОформленияИСДляОснований(Источники);
		РегистрыСведений.СтатусыОформленияКРасчетуИС.УдалитьОснования(Источники);
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
	КонецПопытки;
...
Процедура РасчетСтатусовОформленияИС() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.РасчетСтатусовОформленияИС);
	УстановитьПривилегированныйРежим(Истина);
	
	Выборка = РегистрыСведений.СтатусыОформленияКРасчетуИС.ВыборкаОбъектовКРасчету();
	Пока Выборка.Следующий() Цикл
		
		ВыборкаОснованияПоТипу = Выборка.Выбрать();
		
		ТаблицаБлокировки = Новый ТаблицаЗначений;
		ТаблицаБлокировки.Колонки.Добавить("Основание", Новый ОписаниеТипов(Выборка.ТипОснования));
		ОснованияПоТипу = Новый Массив;
		Пока ВыборкаОснованияПоТипу.Следующий() Цикл
			ОснованияПоТипу.Добавить(ВыборкаОснованияПоТипу.Основание);
			ТаблицаБлокировки.Добавить().Основание = ВыборкаОснованияПоТипу.Основание;
		КонецЦикла;
		...
		Попытка
			...
			ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Основание", "Основание");
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			Блокировка.Заблокировать();
		
			РасчетСтатусовОформленияИСДляОснований(ОснованияПоТипу);
			РегистрыСведений.СтатусыОформленияКРасчетуИС.УдалитьОснования(ОснованияПоТипу);
			ЗафиксироватьТранзакцию();
		Исключение
			ОтменитьТранзакцию();
		КонецПопытки;
...
Функция РассчитатьДляДокумента(ИмяОпцииИспользованияПодсистемы, Источник, Модуль) Экспорт
	...
	Если ИсточникЭтоСсылка Тогда
		...
		Если ИсточникЭтоДокументИС Тогда
			УстановитьПривилегированныйРежим(Истина);
			//@skip-warning Схема подразумевает наличие реквизита "ДокументОснование"
	...
	Если ТекстЗапроса = Неопределено Тогда
		...
		УточнениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неправильное встраивание расчета статуса оформления документа ""%1"" для документа ""%2"".';
				|en = 'Неправильное встраивание расчета статуса оформления документа ""%1"" для документа ""%2"".'"),
				МетаданныеДокументаОснования.Имя,
				МетаданныеДокументаИС.Имя);
		
		ВызватьИсключение ОбщегоНазначенияИСКлиентСервер.ТекстОшибки(,УточнениеОшибки);
	...
	Если Не Модуль.ТипОснование().СодержитТип(ТипЗнч(ДокументОснование)) Тогда
		...
		УточнениеОшибки = СтрШаблон(
			НСтр("ru = 'Ссылочный тип ""%1"" не входит в состав определяемого типа ""ОснованиеСтатусыОформленияДокументов"".';
				|en = 'Ссылочный тип ""%1"" не входит в состав определяемого типа ""ОснованиеСтатусыОформленияДокументов"".'"),
	...
	Если НЕ ЗначениеЗаполнено(ДокументыИС) Тогда
		...
		УточнениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ссылочный тип ""%1"" не входит в состав ни одного из определяемых типов оснований.';
				|en = 'Ссылочный тип ""%1"" не входит в состав ни одного из определяемых типов оснований.'"),
			СокрЛП(ТипДокументОснование));
		
		ВызватьИсключение ОбщегоНазначенияИСКлиентСервер.ТекстОшибки(, УточнениеОшибки); // ошибка составов определяемых типов Основание<ИмяДокументаВЕТИС>
...
Процедура РассчитатьДляОснованийДляПодсистемы(ИмяОпции, ИмяМодуля, Источники, ТипИсточников)
	Если ПолучитьФункциональнуюОпцию(ИмяОпции) Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль(ИмяМодуля);
		Если Модуль.ТипОснование().СодержитТип(ТипИсточников) Тогда
			РассчитатьДляДокументов(ИмяОпции, Источники, Модуль)
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтметитьДокументОснованиеБезРасчетаСтатусов(Источник)
	
	Если Источник.ДополнительныеСвойства.Свойство("РасчетСтатусовОформленияИС") Тогда
		Возврат;
	КонецЕсли;
	Источник.ДополнительныеСвойства.Вставить("РасчетСтатусовОформленияИС");
	РегистрыСведений.СтатусыОформленияКРасчетуИС.ДобавитьОснование(Источник.Ссылка);
