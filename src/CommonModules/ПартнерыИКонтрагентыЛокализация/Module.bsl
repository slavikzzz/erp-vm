...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Справочники.ДоговорыАренды.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	...
	Команда = Документы.ЗаказПереработчику2_5.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	Если Команда <> Неопределено Тогда
		Команда.РежимЗаписи = "";
	КонецЕсли;
			...
			Если ЗначениеЗаполнено(СвойстваГоловногоКонтрагента.ИНН) И СвойстваГоловногоКонтрагента.ИНН <> Объект.ИНН Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'ИНН контрагента не равен указанному в головном подразделении';
						|en = 'Counterparty TIN is not equal to the one specified in the main business unit'"),
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			                                СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			                                  НСтр("ru = 'Данные ИНН и КПП уже указаны для другого контрагента, ответственный - %1.';
													|en = 'These TIN and KPP are already specified for another counterparty, responsible person - %1.'"),
...
Процедура ДополнитьДанныеКонтрагентаПриЗаписи(ОбъектКонтрагент, Форма) Экспорт
	...
	ОбъектКонтрагент.РегистрационныйНомер  = ?(ОбъектКонтрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо
													Или ОбъектКонтрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент,
	...
	Если Не ЕстьОшибка Тогда
		...
		Если ЗначениеЗаполнено(Форма.БИКБанка) Тогда
			...
			Если Не ЗначениеЗаполнено(Банк) Тогда
				...
				ТекстОшибки = НСтр("ru = 'В справочнике Банки не найден банк по БИК';
									|en = 'The bank was not found by BIC in the Banks catalog'") +" = " + Форма.БИКБанка;
		...
		Если ЗначениеЗаполнено(Форма.БИКБанкаДляРасчетов) Тогда
			...
			Если Не ЗначениеЗаполнено(БанкДляРасчетов) Тогда
				...
				ТекстОшибки = НСтр("ru = 'В справочнике Банки не найден банк по БИК';
									|en = 'The bank was not found by BIC in the Banks catalog'") +" = " + Форма.БИКБанкаДляРасчетов;
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
			КонецЕсли;
		КонецЕсли;
	...
	Если Партнер <> Неопределено Тогда
		Если НЕ Форма.ИспользоватьПартнеровКакКонтрагентов Тогда
			Форма.КонтрагентыПартнера = НСтр("ru = 'Контрагенты';
											|en = 'Counterparties'") + " (" + Строка(ДанныеПартнера.КоличествоКонтрагентов) + ")";
	...
	Если Не РезультатПоиска.ДоступноСоздание Тогда 
		...
		СтрСообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'ИНН\КПП - %1 уже указаны для контрагента %2 с кодом %3, ответственный - %4.';
				|en = 'TIN/CRTR - %1 already specified for counterparty %2 with code %3, responsible employee - %4.'"),
