...
Функция ОписаниеОформленныхДокументовЗЕРНО(МетаданныеДокументаЗЕРНО, ДокументОснование, ДокументыПоОснованию, СтатусыОформления) Экспорт
	...
	Иначе
		...
		Если Не АрхивныйСтатус
			И ИнтеграцияИС.ДокументПоддерживаетКорректировку(МетаданныеДокументаЗЕРНО)
...
Функция ШаблонПредставленияДокументаЗЕРНОДляПоляИнтеграции(МетаданныеДокументаЗЕРНО, ДокументОснование) Экспорт
	...
	Если ИнтеграцияИС.ДокументПоддерживаетКорректировку(МетаданныеДокументаЗЕРНО) Тогда
		
		ПредставлениеДокумента = ИнтеграцияИС.ПредставлениеДокументаПоддерживающегоКорректировку(МетаданныеДокументаЗЕРНО);
	...
	Исключение
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Ошибка подготовки исходных файлов для загрузки классификатора %1:
			           |%2';
			           |en = 'Ошибка подготовки исходных файлов для загрузки классификатора %1:
			           |%2'",
		...
		ОбщегоНазначенияИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			ТекстОшибки,
			НСтр("ru = 'Работа с классификаторами ЗЕРНО';
				|en = 'Работа с классификаторами ЗЕРНО'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстОшибки = НСтр("ru = 'При записи соответствия номенклатуры ЗЕРНО произошла ошибка:';
								|en = 'При записи соответствия номенклатуры ЗЕРНО произошла ошибка:'")
			+ Символы.ПС + "%1";
...
Функция ТребуетсяСозданиеКорректировочногоДокумента(МетаданныеДокументаЗЕРНО, Документ) Экспорт
	...
	Если Не ИнтеграцияИС.ДокументПоддерживаетКорректировку(МетаданныеДокументаЗЕРНО) Тогда
...
Функция ПриОпределенииДанныхАдресаОбъекта(Ссылка, ДанныеАдреса) Экспорт
	
	ВыполненРазбор = Ложь;
	
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.ЗемельныеУчасткиИС") Тогда
		
		ДанныеАдресаЗемельногоУчастка = ДанныеЗемельногоУчастка(Ссылка);
		ЗаполнитьЗначенияСвойств(ДанныеАдреса, ДанныеАдресаЗемельногоУчастка);
		
		ВыполненРазбор = Истина;
...
// Процедура дополняет список представлений документов по подсистеме ЗЕРНО.
// 
// Параметры:
...
Процедура ДополнитьПредставленияДокументовПоддерживающихКорректировку(Представления) Экспорт
	...
	Представления.Вставить(МетаданныеДокументаОформлениеСДИЗ, СтруктураПредставлений);
	...
	Представления.Вставить(МетаданныеДокументаПогашениеСДИЗ, СтруктураПредставлений);
...
Процедура ДополнитьСписокДокументовПоддерживающихКорректировку(Документы) Экспорт
	...
	Документы.Добавить(Метаданные.Документы.ОформлениеСДИЗЗЕРНО);
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Реестр элеваторов (ФГИС ""Зерно"")';
				|en = 'Реестр элеваторов (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Назначения партий (ФГИС ""Зерно"")';
				|en = 'Назначения партий (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Потребительские свойства (ФГИС ""Зерно"")';
				|en = 'Потребительские свойства (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Причины списания (ФГИС ""Зерно"")';
				|en = 'Причины списания (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Виды отбора проб (ФГИС ""Зерно"")';
				|en = 'Виды отбора проб (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Виды транспортных средств (ФГИС ""Зерно"")';
				|en = 'Виды транспортных средств (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Единицы измерения (ФГИС ""Зерно"")';
				|en = 'Единицы измерения (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Причины расхождения веса (ФГИС ""Зерно"")';
				|en = 'Причины расхождения веса (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Назначения потребительских свойств (ФГИС ""Зерно"")';
				|en = 'Назначения потребительских свойств (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Документы на партию (ФГИС ""Зерно"")';
				|en = 'Документы на партию (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Причины возврата партии (ФГИС ""Зерно"")';
				|en = 'Причины возврата партии (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Документы права собственности (ФГИС ""Зерно"")';
				|en = 'Документы права собственности (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Способы переработки (ФГИС ""Зерно"")';
				|en = 'Способы переработки (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Сведения о собственности (ФГИС ""Зерно"")';
				|en = 'Сведения о собственности (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Цели использования партии (ФГИС ""Зерно"")';
				|en = 'Цели использования партии (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Допустимые значения потребительских свойств (ФГИС ""Зерно"")';
				|en = 'Допустимые значения потребительских свойств (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Виды с/х культур (ФГИС ""Зерно"")';
				|en = 'Виды с/х культур (ФГИС ""Зерно"")'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Типы услуг элеватора (ФГИС ""Зерно"")';
				|en = 'Типы услуг элеватора (ФГИС ""Зерно"")'"),
				...
				ДанныеПодсказки.Добавить(
					СтрШаблон(
						НСтр("ru = 'Требуется загрузка из реестра элеваторов ФГИС ""Зерно"" (%1 шт.): %2';
							|en = 'Требуется загрузка из реестра элеваторов ФГИС ""Зерно"" (%1 шт.): %2'"),
				...
				ДанныеПодсказки.Добавить(
					СтрШаблон(
						НСтр("ru = 'Требуется загрузка %1 (%2 шт.): %3';
							|en = 'Требуется загрузка %1 (%2 шт.): %3'"),
	...
	Если ЗначениеЗаполнено(КоличествоЭлементовТребуетсяЗагрузка) Тогда
		ВозвращаемоеЗначение.ТребуетсяЗагрузкаОписание = СтрШаблон(
			НСтр("ru = 'Требуется загрузка поставляемых данных классификаторов (%1 шт.)';
				|en = 'Требуется загрузка поставляемых данных классификаторов (%1 шт.)'"),
			КоличествоЭлементовТребуетсяЗагрузка);
		ДанныеПодсказки.Добавить(Символы.ПС);
			ДанныеПодсказки.Добавить(
				Новый ФорматированнаяСтрока(
					НСтр("ru = 'Информация';
						|en = 'Информация'"),,,, "ПросмотрОписанияОкружения"));
			ДанныеПодсказки.Добавить(" ");
			ДанныеПодсказки.Добавить(НСтр("ru = 'для службы поддержки';
											|en = 'для службы поддержки'"));
