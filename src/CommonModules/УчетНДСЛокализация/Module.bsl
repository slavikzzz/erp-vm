...
Процедура ДополнитьНастройкиУчета(НастройкиУчета) Экспорт
	...
	//++ НЕ УТ
	НастройкиУчета.Вставить("ОсобыйПорядокУчетаНДСПриПроизводствеДЦ", ПолучитьФункциональнуюОпцию("ИспользоватьОсобыйПорядокУчетаНДСПриПроизводствеДЦ"));
	//-- НЕ УТ
	...
	Иначе
		...
		Если ДействующиеНастройки.РаздельныйУчетТоваровПоНалогообложениюНДС Тогда
			МассивСтрокОписания.Добавить(СтрШаблон(СтрокаШаблонБулево, НСтр("ru = 'Используется раздельный учет НДС';
																			|en = 'Используется раздельный учет НДС'")));
		КонецЕсли;
...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	ОписаниеВарианта.Описание =
		НСтр("ru = 'Анализ причин блокировки вычета НДС';
			|en = 'Analysis of VAT deduction lock reasons'"); 
	#КонецОбласти
