...
Процедура СформироватьЗаданияНаОтложенныеДвижения(Документ, МенеджерВременныхТаблиц) Экспорт
	...
	//++ НЕ УТ
	ДанныеТаблиц = ДанныеТаблицСтаройСтруктуры(Документ, МенеджерВременныхТаблиц);
			...
			Если НЕ ЗначениеЗаполнено(ТекстОшибки) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '№%1, описание отсутствует';
						|en = 'No.%1, no description'", ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ЗначениеЗаполнено(СокрЛП(Выборка.ИмяРегистра)) Тогда
					ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = '%1 (%2), организация ""%3"", регистр ""%4""%5';
							|en = '%1 (%2), company ""%3"", register ""%4""%5'", ОбщегоНазначения.КодОсновногоЯзыка()),
				...
				Иначе
					ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = '%1 (%2), организация ""%3""%4';
							|en = '%1 (%2), the ""%3"" company %4'", ОбщегоНазначения.КодОсновногоЯзыка()),
						СокрЛП(Выборка.Ссылка),
						ПолучитьНавигационнуюСсылку(Выборка.Ссылка),
						СокрЛП(Выборка.Организация),
						ТекстДополнительныхПолей);
				...
				Если ЗначениеЗаполнено(ПараметрыПроведения.ОперацииЗакрытияМесяца) Тогда
					...
					ЗарегистрироватьПроблемуВЗакрытииМесяца(
						...
						НСтр("ru = 'Не удалось исправить проблемы в движениях документов';
							|en = 'Cannot fix the issues in the document records'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ПерепровестиДокументыПоРегиструНакопления(ПараметрыПроведения, ВыборкаДокументов, ИмяРегистраНакопления) Экспорт
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		//++ НЕ УТКА
		Если ПараметрыПроведения.ЭтапПроизводстваТолькоВТекущемПериоде
			И ТипЗнч(Регистратор) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
			
			ДопПараметры.ДополнительныеСвойства = Новый Структура;
			ДопПараметры.ДополнительныеСвойства.Вставить(
				"ПериодПерезаписиДвижений",
				ПараметрыПроведения.ПериодРасчета);
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обработать документ ""%1"" (%2)по причине:
					|		%3';
					|en = 'Cannot process the document ""%1"" (%2). Reason:
...
Процедура ЗарегистрироватьПроблемуВЗакрытииМесяца(ПараметрыПроведения, Организация, ТекстПроблемы, ТекстУточнения, Объект)
	...
	ПолныйТекстПроблемы = ТекстПроблемы + " " + НСтр("ru = 'по организации ""%1"" за период ""%2""';
