...
Процедура ПроверитьНазначениеРолей(НазначениеРолей = Неопределено, ПроверитьВсе = Ложь, СписокОшибок = Неопределено) Экспорт
	...
	ПроверитьСоставПравРолей(НедоступныеПрава, Назначение.ТолькоДляВнешнихПользователей, ТекстОшибки,
		НСтр("ru = 'При проверке ролей только для внешних пользователей найдены ошибки:';
			|en = 'Errors were found while checking external user roles:'"), СписокОшибок);
	...
	ПроверитьСоставПравРолей(НедоступныеПрава, Назначение.СовместноДляПользователейИВнешнихПользователей, ТекстОшибки,
		НСтр("ru = 'При проверке ролей совместно для пользователей и внешних пользователей найдены ошибки:';
			|en = 'Errors were found while checking both user and external user roles:'"), СписокОшибок);
	...
	Если ОбщегоНазначения.РазделениеВключено() Или ПроверитьВсе Тогда
		...
		ПроверитьСоставПравРолей(НедоступныеПрава, Роли, ТекстОшибки,
			НСтр("ru = 'При проверке ролей для пользователей приложения найдены ошибки:';
				|en = 'Errors were found while checking application user roles:'"), СписокОшибок, НеразделенныеДанные);
	КонецЕсли;
	Если Не ОбщегоНазначения.РазделениеВключено() Или ПроверитьВсе Тогда
		...
		ПроверитьСоставПравРолей(НедоступныеПрава, Роли, ТекстОшибки,
			НСтр("ru = 'При проверке ролей для пользователей найдены ошибки:';
				|en = 'Errors were found while checking user roles:'"), СписокОшибок);
...
Функция ПредставлениеСсылки(Ссылка) Экспорт
	...
	Если Не ЗначениеЗаполнено(Результат) Тогда
		Результат = "<" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Пустая ссылка %1';
				|en = 'Empty reference: ""%1""'", ОбщегоНазначения.КодОсновногоЯзыка()), ТипЗнч(Ссылка)) + ">";
	КонецЕсли;
...
Процедура ПроверитьБезопасныйРежимОтключен(ИмяПроцедурыИлиФункции) Экспорт
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Выполнение ""%1"" недоступно в безопасном режиме.';
			|en = 'Cannot execute ""%1"" in safe mode.'"),
...
Процедура ПриЗаполненииВнешнихРеквизитов(ВидыИзменяемыхОбъектов, ВнешниеРеквизиты) Экспорт
	...
	ВнешниеРеквизиты.Добавить(НовыйВнешнийРеквизит(Префикс + "АутентификацияСтандартная",
		НСтр("ru = 'Аутентификация 1С:Предприятия';
			|en = '1C:Enterprise authentication'"), ТипБулево));
	
	ВнешниеРеквизиты.Добавить(НовыйВнешнийРеквизит(Префикс + "АутентификацияOpenID",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Аутентификация по протоколу %1';
				|en = '%1 protocol authentication'"), "OpenID"), ТипБулево));
	
	ВнешниеРеквизиты.Добавить(НовыйВнешнийРеквизит(Префикс + "АутентификацияOpenIDConnect",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Аутентификация по протоколу %1';
				|en = '%1 protocol authentication'"), "OpenID-Connect"), ТипБулево));
	...
	ВнешниеРеквизиты.Добавить(НовыйВнешнийРеквизит(Префикс + "СрокДействияНеОграничен",
		НСтр("ru = 'Без ограничения срока доступа к приложению';
			|en = 'Non-expiring access'"), ТипБулево));
...
Функция ВозможноИзменитьПароль(Пользователь, ДополнительныеПараметры = Неопределено) Экспорт
	   ...
	   И Пользователь <> Пользователи.АвторизованныйПользователь() Тогда
		...
		Если Не ДействияСПользователемСервиса.ИзменениеПароля Тогда
			ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Сервис: Недостаточно прав для изменения пароля пользователя ""%1"".';
					|en = 'Service: insufficient rights to change the password for user ""%1.""'"), Пользователь));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ДополнительныеПараметры.Свойство("ПроверятьДействительностьПользователя")
	   И РеквизитыПользователя.Недействителен <> Ложь Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Пользователь ""%1"" недействителен.';
				|en = 'User ""%1"" is invalid.'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ДополнительныеПараметры.Свойство("ПроверятьНаличиеПользователяИБ")
	   И ПользовательИБ = Неопределено Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не существует учетная запись пользователя ""%1"".';
				|en = 'There is no account for user ""%1.""'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	   И Не УровеньДоступа.НастройкиДляВхода Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недостаточно прав для изменения пароля пользователя ""%1"".';
				|en = 'Insufficient rights to change password for user ""%1"".'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
			...
			Если ДополнительныеПараметры.Свойство("УчитыватьСвойствоЗапрещеноИзменятьПароль") Тогда
				ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Установлен запрет изменения пароля пользователя ""%1"".';
						|en = 'User ""%1"" cannot change password.'"), Пользователь));
				Возврат Ложь;
			КонецЕсли;
		Иначе
			ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Установлен запрет изменения пароля пользователя ""%1"".
				           ...
				           |Please contact the administrator.'"), Пользователь));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Пароль можно будет сменить только через %1.';
			|en = 'You can change the password in %1.'"), ЧислоИПредмет));
		...
		Если ЕстьРазделАдминистрирование Тогда
			Подсказка =
				НСтр("ru = 'Требования к длине и сложности пароля задаются отдельно.
				           ...
				           |en = 'You can specify password length and complexity requirements.
		...
		Если ПустаяСтрока(Имя) Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Не заполнено Имя (для входа).';
					|en = 'The username is required.'"),, "Имя",,Отказ);
		...
		ИначеЕсли СтрДлина(Имя) > 64 Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Имя (для входа) превышает 64 символа.';
					|en = 'The username exceeds 64 characters.'"),,"Имя",,Отказ);
			
		ИначеЕсли СтрНайти(Имя, ":") > 0 Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Имя (для входа) содержит запрещенный символ "":"".';
					|en = 'The username contains an illegal character "":"".'"),,"Имя",,Отказ);
		Иначе
			УстановитьПривилегированныйРежим(Истина);
	 ...
	 Или Не ЗначениеЗаполнено(ОписаниеПользователяИБ.Пароль) Тогда
		...
		Если ЗначениеЗаполнено(МинимальнаяДлинаПаролей) Тогда
			...
			Если ПарольЗадан Или ПроверитьПустойПароль Тогда
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru = 'Установите пароль.';
						|en = 'Set a password.'"),, "СменитьПароль",, Отказ);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
			...
			Исключение
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru = 'Пользователь ОС должен быть в формате
					           ...
					           |""\\Domain Name\Username"".'"),,"ПользовательОС",,Отказ);
			КонецПопытки;
			УстановитьПривилегированныйРежим(Ложь);
		КонецЕсли;
					   ...
					   И ПользовательИБ.АдресЭлектроннойПочты <> АдресЭлектроннойПочты Тогда
						...
						Если ЗаписьВБезопасномРежиме Тогда
							ТекстОшибки = НСтр("ru = 'Изменение основного адреса электронной почты недоступно в безопасном режиме.';
												|en = 'Изменение основного адреса электронной почты недоступно в безопасном режиме.'");
							ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
						КонецЕсли;
				...
				Если ПометкаУдаленияОбъектаАвторизации И ВнешнийПользовательОбъект.Недействителен = Ложь Тогда
					Если ЗаписьВБезопасномРежиме Тогда
						ТекстОшибки = НСтр("ru = 'Изменение действительности пользователя недоступно в безопасном режиме.';
											|en = 'Изменение действительности пользователя недоступно в безопасном режиме.'");
						ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
					КонецЕсли;
...
Функция ИмяСобытияОшибкаВходаВПрограммуДляЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Пользователи.Ошибка входа в приложение';
				|en = 'Users.Authorization error'", ОбщегоНазначения.КодОсновногоЯзыка());
