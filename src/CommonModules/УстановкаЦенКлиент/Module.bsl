...
Процедура ВыгрузитьВExcel(Форма, ПараметрыПечати) Экспорт
	...
	Исключение
		СообщениеОбОшибке = НСтр("ru = 'Не удалось выгрузить цены в Excel.
		                               ...
		                               |Подробности:';
		                               |en = 'Cannot export prices to Excel.
...
Функция ПолучитьИнформационнуюСтрокуДляЯчейки(Элементы,
	...
	Если ПозицияПодстроки <> 0 Тогда
		...
		ФорматированнаяИсторияИзмененияЦены = Новый ФорматированнаяСтрока(НСтр("ru = 'история';
																				|en = 'history'"),Новый Шрифт(,,Истина),,,"СсылкаГруппыИсторииИзмененияЦены");
...
Процедура ЗагрузитьИзExcelБезСсылки(Форма) Экспорт
	...
	Исключение
		СообщениеОбОшибке = НСтр("ru = 'Не удалось загрузить цены из файла Excel.
		                               ...
		                               |Подробности:';
		                               |en = 'Cannot import prices from Excel file.
		                               |Make sure Microsoft Excel is installed on the server. 
		                               |More information:'")
		                    + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение СообщениеОбОшибке;
	КонецПопытки;
	...
	Исключение
		...
		СообщениеОбОшибке = СтрШаблон(
			НСтр("ru = 'Файл не может быть загружен.
			           ...
			           |Подробности: %1';
			           |en = 'The file cannot be imported.
			           |You may only import the files exported with the ""Excel -> Export..."" command.
			           |Details: %1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение СообщениеОбОшибке;
	КонецПопытки;
...
Процедура ВыгрузитьВExcelПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Попытка
		...
		Sheet.Protection.AllowEditRanges.Add(
			НСтр("ru = 'Цены номенклатуры';
				|en = 'Item prices'"),                                           // Title
	...
	Исключение
		...
		СообщениеОбОшибке = НСтр("ru = 'Не удалось записать данные в файл. Подробности:';
								|en = 'Cannot save data to file. More information:'") + " "
		                    + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение СообщениеОбОшибке;
	КонецПопытки;
