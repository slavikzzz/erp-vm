...
Функция УстановитьДанныеСервисаОтправкиФинансовойОтчетностиВБанки(Знач Данные, ИмяПараметра = "") Экспорт
	...
	Если ПустаяСтрока(ИмяПараметра) И ТипЗнч(Данные) <> Тип("Структура") Тогда
		...
		ВызватьИсключение НСтр("ru = 'Передан неверный тип данных.';
								|en = 'Incorrect data type is transmitted.'");
	КонецЕсли;
...
Процедура ПриПолученииТранзакции(РеквизитыТранзакции, Результат) Экспорт
	...
	ПоследнийДокументооборот = УниверсальныйОбменСБанками.ПоследнийДокументооборотПоПредмету(РеквизитыТранзакции.Предмет);
	Если РеквизитыТранзакции.Документооборот <> ПоследнийДокументооборот Тогда
		// Транзакция отправлена на предыдущую отправку, игнорируем её.
		Возврат;
	КонецЕсли;
	
	ТипТранзакции = РеквизитыТранзакции.ТипТранзакции;
	...
	Если ЗначениеЗаполнено(НовыйСтатус)
		ИЛИ ЭтоОшибка Тогда
			...
			ЗаполнитьЗначенияСвойств(Параметры, РеквизитыТранзакции);
	...
	Если ЗначениеЗаполнено(НовыйСтатус)
	   И НовыйСтатус = ФинОтчетностьВБанкиКлиентСервер.СтатусСдано() Тогда
		ЗаполнениеФинОтчетностиВБанкиПереопределяемый.ЗарегистрироватьИзменениеСтатусаЗадачиФинОтчетности(РеквизитыТранзакции.Предмет, НовыйСтатус);
	КонецЕсли;
...
Процедура ПриОтправкеТранзакции(РеквизитыТранзакции, Результат) Экспорт
	...
	Если Не Результат.Выполнено
	 Или РеквизитыТранзакции.ТипТранзакции <> Перечисления.ТипыТранзакцийОбменаСБанкамиФинОтчетность.Отчетность Тогда
		Возврат;
	КонецЕсли;
		
	Параметры = ПараметрыУстановкиСтатусаПредмета();
	ЗаполнитьЗначенияСвойств(Параметры, РеквизитыТранзакции);
	Параметры.ДатаОтправки = РеквизитыТранзакции.ДатаТранспорта;
...
Процедура НачатьОбновлениеДанныхСервиса(Форма) Экспорт 
	...
	СведенияОДлительнойОперации.ДлительнаяОперацияОбновленияДанныхСервиса = ДлительныеОперации.ВыполнитьФункцию(
...
Функция ПолучитьНадписьВремяОбновления(ДатаПоследнегоОбновления) Экспорт
	...
	ИначеЕсли ВременнаяРазница < 15 * 60 Тогда
		НадписьВремяОбновления = НСтр("ru = 'Обновлено менее 15 минут назад';
										|en = 'Updated less than 15 minutes ago'");
...
Функция ОбновитьДанныеСервисаФинОтчетности() Экспорт
	...
	Если СведенияОСервисе.ТребуетсяПодключениеИнтернетПоддержки Тогда
		// Нет подключения к интернет-поддержке.
		РезультатОбновленияДанных.ИмяСобытия = "ТребуетсяПодключениеИнтернетПоддержки";
		Возврат РезультатОбновленияДанных;
	КонецЕсли;

	РезультатОбновленияДанных = УниверсальныйОбменСБанками.АктуализироватьДанныеСервиса(Перечисления.СервисыОбменаСБанками.ФинансоваяОтчетность);
		
	Если РезультатОбновленияДанных.ИмяСобытия <> "ДанныеАктуальны" Тогда
		
		ЗаписьЖурналаРегистрации("УниверсальныйОбменСБанками.ОбновитьДанныеСервиса",
...
Функция НаименованиеОтчетностиВСбербанк() Экспорт
	...
	Возврат НСтр("ru = 'Бухгалтерская отчетность для Сбербанка';
				|en = 'Sberbank accounting reporting'");
...
Процедура УстановитьСтатусПредметаВЖурнале(ПараметрыУстановкиСтатуса, Статус, Непрочтенное = Ложь)
	...
	Если ЗначениеЗаполнено(ДатаНачала) И ЗначениеЗаполнено(ДатаОкончания) Тогда
		Менеджер.ФинансовыйПериод = РегламентированнаяОтчетностьВызовСервера.ПредставлениеФинансовогоПериода(
													ДатаНачала, 
