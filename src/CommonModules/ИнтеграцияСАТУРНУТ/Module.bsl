...
//++ НЕ УТ
Функция ТекстТоварыДокументаБезСерийСоСкладом(ИмяДокумента, ИмяТЧТовары = ".Товары")
	...
	Шаблон = СтрЗаменить(Шаблон, "ПроизводствоБезЗаказа", ИмяДокумента);
	Шаблон = СтрЗаменить(Шаблон, ".МатериалыИРаботы", ИмяТЧТовары);
	Возврат Шаблон;
...
Процедура ОбработкаЗаполненияДокументаПроизводственнаяОперацияСАТУРН(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЭтапПроизводства2_2")Тогда
		ЗаполнитьПроизводственнуюОперациюСАТУРННаОснованииЭтапаПроизводства2_2(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	//-- НЕ УТКА
	
	//++ НЕ УТ
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПроизводствоБезЗаказа")Тогда
		ЗаполнитьПроизводственнуюОперациюСАТУРННаОснованииПроизводстваБезЗаказа(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОтчетПереработчика2_5")Тогда
		ЗаполнитьПроизводственнуюОперациюСАТУРННаОснованииОтчетаПереработчика2_5(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	//-- НЕ УТ
	КонецЕсли;
...
Процедура ЗаполнитьТабличнуюЧастьСырьеПроизводственнойОперации(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента)
	...
	//++ НЕ УТ
	УстановитьПараметрЗапросаТаблицаСырье(Запрос, ДанныеЗаполнения);
	//-- НЕ УТ
...
//++ НЕ УТКА
Процедура ЗаполнитьПроизводственнуюОперациюСАТУРННаОснованииЭтапаПроизводства2_2(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	ДокументСсылка.Организация   КАК Организация,
	|	ДокументСсылка.Подразделение КАК Подразделение,
	|	ЕСТЬNULL(Товары1.Получатель, Товары2.Получатель) КАК Склад,
	|	Неопределено                 КАК Контрагент,
	|	Неопределено                 КАК Партнер,
	|	ДокументСсылка.Номер         КАК НомерПервичногоДокумента,
	|	""Этап производства ""       КАК НаименованиеПервичногоДокумента
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК ДокументСсылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ВыходныеИзделия КАК Товары1
	|	ПО Товары1.Ссылка = &ДокументОснование
	|	И Товары1.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияСАТУРН)
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ПобочныеИзделия КАК Товары2
	|	ПО Товары2.Ссылка = &ДокументОснование
	|	И Товары2.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияСАТУРН)
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование");
	...
	|ПОМЕСТИТЬ ТоварыСерии
	|ИЗ
	|	ТаблицаТовары1 КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаСерии1 КАК ТаблицаСерии
	|		ПО ТаблицаТовары.Номенклатура = ТаблицаСерии.Номенклатура
	|		И ТаблицаТовары.Характеристика = ТаблицаСерии.Характеристика
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО ТаблицаТовары.Номенклатура = СправочникНоменклатура.Ссылка
	...
	|	СУММА(ЕСТЬNULL(ТаблицаСерии.Количество, ТаблицаТовары.Количество)) КАК Количество
	|ИЗ
	|	ТаблицаТовары2 КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаСерии2 КАК ТаблицаСерии
	|		ПО ТаблицаТовары.Номенклатура = ТаблицаСерии.Номенклатура
	|		И ТаблицаТовары.Характеристика = ТаблицаСерии.Характеристика
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО ТаблицаТовары.Номенклатура = СправочникНоменклатура.Ссылка
	...
	ТекстТоварыДокумента = ИнтеграцияИСУТ.ТекстЗапросаЭтапПроизводства2_2()
		+ ИнтеграцияИС.РазделительЗапросовВПакете()
		+ ТекстЗапросаСырье("ТаблицаТоварыЭтапПроизводства2_2");
...
//++ НЕ УТ
Процедура ЗаполнитьПроизводственнуюОперациюСАТУРННаОснованииПроизводстваБезЗаказа(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа.ВыходныеИзделия КАК Товары1
	|	ПО Товары1.Ссылка = &ДокументОснование
	|	И Товары1.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияСАТУРН)
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа.ПобочныеИзделия КАК Товары2
	|	ПО Товары2.Ссылка = &ДокументОснование
	|	И Товары2.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияСАТУРН)
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование");
	...
	ТекстТоварыДокумента = ИнтеграцияИСУТ.ТекстЗапросаПроизводствоБезЗаказа()
		+ ИнтеграцияИС.РазделительЗапросовВПакете()
		+ ТекстЗапросаСырье("ТаблицаТоварыПроизводствоБезЗаказа");
	
	ЗаполнитьТабличнуюЧастьСырьеПроизводственнойОперации(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента);
	
КонецПроцедуры

Процедура ЗаполнитьПроизводственнуюОперациюСАТУРННаОснованииОтчетаПереработчика2_5(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	ДокументСсылка.Номер         КАК НомерПервичногоДокумента,
	|	""Отчет переработчика""      КАК НаименованиеПервичногоДокумента
	|ИЗ
	|	Документ.ОтчетПереработчика2_5 КАК ДокументСсылка
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование");
	...
	ТекстТоварыДокумента = ТекстТоварыДокументаБезСерий("ОтчетПереработчика2_5", ".МатериалыИРаботы");
	ЗаполнитьТабличнуюЧастьСырьеПроизводственнойОперации(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента);
...
Процедура ОбработкаЗаполненияДокументаАктПримененияСАТУРН(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	//++ НЕ УТ
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПроизводствоБезЗаказа")Тогда
		ЗаполнитьАктПримененияСАТУРННаОснованииПроизводстваБезЗаказа(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	//-- НЕ УТ
	//++ НЕ УТКА
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЭтапПроизводства2_2")Тогда
		ЗаполнитьАктПримененияСАТУРННаОснованииЭтапаПроизводства2_2(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	//-- НЕ УТКА
	КонецЕсли;
...
//++ НЕ УТ
Процедура ЗаполнитьАктПримененияСАТУРННаОснованииПроизводстваБезЗаказа(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ДополнительныеПараметры.Вставить("ЗаполнитьСкладИзТабличнойЧасти", Истина);
	ДополнительныеПараметры.Вставить("Организация",                    Справочники.Организации.ПустаяСсылка());
	ДополнительныеПараметры.Вставить("ДокументОбъект",                 ДокументОбъект);
	
	ЗаполнитьШапкуАктаПрименения(ДокументОбъект, ДанныеЗаполнения, Запрос, ДополнительныеПараметры);
	
	ТекстТоварыДокумента = ТекстТоварыДокументаБезСерийСоСкладом("ПроизводствоБезЗаказа", ".МатериалыИРаботы");
	ЗаполнитьТабличнуюЧастьАктаПрименения(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента, ДополнительныеПараметры);
	
КонецПроцедуры
//-- НЕ УТ

//++ НЕ УТКА
Процедура ЗаполнитьАктПримененияСАТУРННаОснованииЭтапаПроизводства2_2(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	
	|ВЫБРАТЬ
	|	ЕСТЬNULL(СкладыОбеспечения.Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)) КАК Склад,
	|	ТаблицаТовары.Подразделение     КАК Подразделение,
	|	ТаблицаТовары.Номенклатура      КАК Номенклатура,
	|	ТаблицаТовары.Характеристика    КАК Характеристика,
	|	ТаблицаТовары.Серия             КАК Серия,
	...
	|		ПО СкладыОбеспечения.Номенклатура = ТаблицаТовары.Номенклатура
	|			И СкладыОбеспечения.Характеристика = ТаблицаТовары.Характеристика
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &ДокументОснование
	|	И СправочникНоменклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияСАТУРН)
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(СкладыОбеспечения.Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)),
	|	ТаблицаТовары.Подразделение,
	|	ТаблицаТовары.Номенклатура,
	|	ТаблицаТовары.Характеристика,
	|	ТаблицаТовары.Серия
...
Процедура ОбработкаЗаполненияДокументаНакладнаяСАТУРН(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	//++ НЕ УТКА
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЭтапПроизводства2_2")Тогда
		ЗаполнитьНакладнуюСАТУРННаОснованииЭтапаПроизводства2_2(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	//-- НЕ УТКА
	//++ НЕ УТ
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ДвижениеПродукцииИМатериалов")Тогда
		ЗаполнитьНакладнуюСАТУРННаОснованииДвиженияПродукцииИМатериалов(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОтчетПереработчика2_5")Тогда
		ЗаполнитьНакладнуюСАТУРННаОснованииОтчетаПереработчика2_5(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПроизводствоБезЗаказа")Тогда
		ЗаполнитьНакладнуюСАТУРННаОснованииПроизводстваБезЗаказа(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	//-- НЕ УТ
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ВнутреннееПотребление")Тогда
...
//++ НЕ УТ
Процедура ЗаполнитьНакладнуюСАТУРННаОснованииДвиженияПродукцииИМатериалов(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	ДокументСсылка.Номер            КАК НомерПервичногоДокумента,
	|	ДокументСсылка.Дата             КАК ДатаРегистрации,
	|	""Движение продукции и материалов"" КАК НаименованиеПервичногоДокумента,
	|	НЕ ДокументСсылка.Проведен      КАК ЕстьОшибкиПроведен,
	|	ДокументСсылка.Ответственный    КАК Ответственный,
	|	ДокументСсылка.Организация      КАК Организация,
	|	ДокументСсылка.Отправитель      КАК Подразделение,
	|	ВЫБОР
	|		КОГДА ДокументСсылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеремещениеТоваровМеждуФилиалами)
	|			ТОГДА ДокументСсылка.ОрганизацияПолучатель
	|		КОГДА ДокументСсылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеремещениеПолуфабрикатовМеждуФилиалами)
	|			ТОГДА ДокументСсылка.ОрганизацияПолучатель
	|		ИНАЧЕ ДокументСсылка.Организация
	|	КОНЕЦ                           КАК Контрагент,
	|	ДокументСсылка.Получатель       КАК Партнер,
	|	ДокументСсылка.Отправитель      КАК ГрузоотправительСклад,
	|	Неопределено                    КАК ГрузоотправительКонтрагент,
	|	Неопределено                    КАК ГрузоотправительПартнер,
	|	ДокументСсылка.Получатель       КАК ГрузополучательСклад,
	|	Неопределено                    КАК ГрузополучательКонтрагент,
	|	Неопределено                    КАК ГрузополучательПартнер,
	|	ДокументСсылка.Получатель       КАК ГрузополучательПодразделение,
	|	Неопределено                    КАК ПеревозчикПартнер,
	|	Неопределено                    КАК ГосударственныйКонтракт,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСАТУРН.НакладнаяПеревозка) КАК Операция
	|ИЗ
	...
	ТекстТоварыДокумента = ИнтеграцияИСУТ.ТекстЗапросаДвижениеПродукцииИМатериалов() + "
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаТовары.Номенклатура      КАК Номенклатура,
	...
	|ПОМЕСТИТЬ ТоварыСерии
	|ИЗ
	|	ТаблицаТоварыДвижениеПродукцииИМатериалов КАК ТаблицаТовары
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТовары.Номенклатура,
	|	ТаблицаТовары.Характеристика,
	|	ТаблицаТовары.Серия
...
Процедура ЗаполнитьНакладнуюСАТУРННаОснованииОтчетаПереработчика2_5(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
...
Процедура ЗаполнитьНакладнуюСАТУРННаОснованииПроизводстваБезЗаказа(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	Неопределено                   КАК Контрагент,
	|	Неопределено                   КАК Партнер,
	|	Товары.Склад                   КАК ГрузоотправительСклад,
	|	Неопределено                   КАК ГрузоотправительКонтрагент,
	|	Неопределено                   КАК ГрузоотправительПартнер,
	|	Неопределено                   КАК ГрузополучательСклад,
	|	Неопределено                   КАК ГрузополучательКонтрагент,
	...
	|ИЗ
	|	Документ.ПроизводствоБезЗаказа КАК ДокументСсылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа.МатериалыИРаботы КАК Товары
	|	ПО Товары.Ссылка = &ДокументОснование
	|	И Товары.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияСАТУРН)
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование");
	...
	|ПОМЕСТИТЬ ТоварыСерии
	|ИЗ
	|	ТаблицаТоварыПроизводствоБезЗаказа КАК ТаблицаТовары
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТовары.Номенклатура,
	|	ТаблицаТовары.Характеристика,
	|	ТаблицаТовары.Серия
...
//++ НЕ УТКА
Процедура ЗаполнитьНакладнуюСАТУРННаОснованииЭтапаПроизводства2_2(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК ДокументСсылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ОбеспечениеМатериаламиИРаботами КАК Товары
	|	ПО Товары.Ссылка = &ДокументОснование
	|	И Товары.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияСАТУРН)
	|ГДЕ
	|	ДокументСсылка.Ссылка = &ДокументОснование");
	...
	|ПОМЕСТИТЬ ТоварыСерии
	|ИЗ
	|	ТаблицаТоварыЭтапПроизводства2_2 КАК ТаблицаТовары
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТовары.Номенклатура,
	|	ТаблицаТовары.Характеристика,
	|	ТаблицаТовары.Серия
...
Функция ТаблицаСырьеПоСпецификациям(ДокументСсылка, ИмяДокумента, ИмяТЧ)
	
	КвалификаторЧисла = Новый КвалификаторыЧисла(10, 2, ДопустимыйЗнак.Любой);
	ТаблицаСырьеПоСпецификациям = Новый ТаблицаЗначений;
	ТаблицаСырьеПоСпецификациям.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаСырьеПоСпецификациям.Колонки.Добавить("Характеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	ТаблицаСырьеПоСпецификациям.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число", КвалификаторЧисла));
	...
	ШаблонТаблицыЗапроса = "Документ.%1.%2";
	ШаблонТаблицыЗапроса = СтрШаблон(ШаблонТаблицыЗапроса, ИмяДокумента, ИмяТЧ);
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяТаблицы", ШаблонТаблицыЗапроса);
	
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
	
	Если Не Запрос.Выполнить().Пустой() Тогда
		Возврат ТаблицаСырьеПоСпецификациям;
	КонецЕсли;
	...
	ШаблонТаблицыЗапроса = "Документ.%1.ВыходныеИзделия";
	ШаблонТаблицыЗапроса = СтрШаблон(ШаблонТаблицыЗапроса, ИмяДокумента);
	...
	ЗаполнитьСырьеПоСпецификациям(РезультатЗапроса, ТаблицаСырьеПоСпецификациям);
	
	Возврат ТаблицаСырьеПоСпецификациям;
	
КонецФункции

Процедура ЗаполнитьСырьеПоСпецификациям(Спецификации, ТаблицаСырьеПоСпецификациям)
	
	СписокНоменклатуры = Справочники.РесурсныеСпецификации.СписокНоменклатуры();
	
	Выборка = Спецификации.Выбрать();
	...
	Для Каждого ДанныеСпецификации Из ДанныеСпецификаций Цикл
		Если ДанныеСпецификации <> Неопределено И ДанныеСпецификации.Свойство("МатериалыИУслуги") Тогда
			Для Каждого СтрокаМатериала Из ДанныеСпецификации.МатериалыИУслуги Цикл
				ЗаполнитьЗначенияСвойств(ТаблицаСырьеПоСпецификациям.Добавить(), СтрокаМатериала);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
Функция ТекстЗапросаСырье(ИмяВТ)
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаСырьеПоСпецификациям.Номенклатура,
	|	ТаблицаСырьеПоСпецификациям.Характеристика,
	|	ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка),
	|	ТаблицаСырьеПоСпецификациям.Количество
	|ПОМЕСТИТЬ ТаблицаСырьеПоСпецификациям
	|ИЗ
	|	&ТаблицаСырьеПоСпецификациям КАК ТаблицаСырьеПоСпецификациям
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ ТаблицаСырье
	|ИЗ
	|	&ИмяВТ КАК ВременнаяТаблица
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	СУММА(ТаблицаСырье.Количество) КАК Количество
	|ПОМЕСТИТЬ ТоварыСерии
	|ИЗ
	|	ТаблицаСырье КАК ТаблицаСырье
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО ТаблицаСырье.Номенклатура = СправочникНоменклатура.Ссылка
	|ГДЕ
	|	СправочникНоменклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияСАТУРН)
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаСырье.Номенклатура,
	|	ТаблицаСырье.Характеристика,
	|	ТаблицаСырье.Серия
	|;
	|";
...
Процедура УстановитьПараметрЗапросаТаблицаСырье(Запрос, ДанныеЗаполнения)
	...
	ТаблицаСырьеПоСпецификациям = ТаблицаСырьеПоСпецификациям(ДанныеЗаполнения, ИмяДокумента, ИмяТЧ);
	
	Запрос.УстановитьПараметр("ТаблицаСырьеПоСпецификациям", ТаблицаСырьеПоСпецификациям);
