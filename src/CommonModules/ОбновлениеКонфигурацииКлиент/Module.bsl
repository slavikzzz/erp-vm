	...
	Если ПустаяСтрока(КаталогСкрипта) Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
			"Предупреждение", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Журнал обновления см. в папке временных файлов %1.(цифры).';
					|en = 'See the update log in the temporary file folder: %1.(digits).'"), "%temp%\1Cv8Update"),, Истина);
		РезультатОбновления = Истина; // считаем обновление успешным.
	Иначе 
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
			"Информация", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Папка с журналом обновления: %1';
					|en = 'Update log folder: %1'"), КаталогСкрипта),, Истина);
...
Функция ИмяЗадачиScheduleService(Знач КодЗадачи)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Обновление конфигурации (%1)';
																		|en = 'Update configuration (%1)'"), Формат(КодЗадачи, "ЧГ=0"));
...
Функция СобытиеЖурналаРегистрации() Экспорт
	Возврат НСтр("ru = 'Обновление конфигурации';
				|en = 'Configuration update'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
КонецФункции
...
Процедура ЗапуститьСкриптОбновленияЗавершение(Параметры, ПараметрыАдминистрирования)
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Информация",
		НСтр("ru = 'Выполняется процедура обновления конфигурации:';
			|en = 'Updating the configuration:'") + " " + ИмяГлавногоФайлаСкрипта);
