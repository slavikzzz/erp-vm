...
Функция ПредставлениеПрогрессаПодписания(ОбъектМаршрута, КоличествоУстановленныхПодписей) Экспорт
	...
	Разделитель = ?(ОбщегоНазначенияБЭД.КонфигурацияИспользуетНесколькоЯзыков(), "/", НСтр("ru = 'из';
																							|en = 'from'", ЯзыкКонфигурации));
	Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '(%1 %2 %3)';
																			|en = '(%1 %2 %3)'", ЯзыкКонфигурации), 
...
// * Порядок - Число
// * Требование - ПеречислениеСсылка.ТребованияКПодписаниюЭД
...
// Параметры:
//  СсылкаНаОбъект - см. РегистрСведений.СостояниеПодписанияЭД.Объект
// 
// Возвращаемое значение:
//  См. ПустаяТаблицаТребованийКПодписанию
Функция ТаблицаМаршрутаПоСсылке(СсылкаНаОбъект) Экспорт
...
Функция ПредставлениеМаршрутаВыбранногоВДокументе(ЗадаватьМаршрутВручную, Подписанты = Неопределено, 
	...
	Если ЗадаватьМаршрутВручную Тогда
		ПредставлениеНастроек = СтрШаблон(НСтр("ru = 'Подписи по списку (%1)';
												|en = 'Signatures by list (%1)'"), Подписанты.Количество());
	Иначе
...
//  ИсточникДанных - ДокументСсылка.СообщениеЭДО
...
Процедура ЗаполнитьДеревоМаршрутаНаФорме(Форма, ИсточникДанных, ИмяРеквизитаДерева = "ДеревоТребований",
	...
	Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ИсточникДанных) Тогда
		...
		ТаблицаМаршрута = ТаблицаМаршрутаПоСсылке(ИсточникДанных);
	Иначе
		...
		Если ЕстьОбменСКонтрагентами Тогда
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр(
					...
					|en = 'The signature kind in the route does not correspond to the signature kind in intra-company EDI settings for the companies:'"));
...
Процедура ЗафиксироватьФактОповещенияУчастникаМаршрута(ОбъектПодписания, Подписант) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ИмяСобытия = НСтр("ru = 'Запись факта оповещения о подписании электронного документа';
							|en = 'Recording of the fact of notification of the electronic document signing'", ОбщегоНазначения.КодОсновногоЯзыка());
...
#Область РегистрыСведений_СостояниеПодписанияЭД_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = '1С:БЭД: обновление состояний подписания электронных документов';
									|en = '1C:EDL: update of electronic document signing states'");
...
#Область Справочники_МаршрутыПодписания_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|en = '1C:EDL: Fills in service properties of the predefined routes.
	|Operations with signing routes are temporarily unavailable.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
Функция ПредставлениеОшибкиНесоответствияМаршрутаНастройкам(ИдентификаторОшибки)
	...
	Ошибки.Вставить("НетДоступныхСертификатов", 
		НСтр("ru = 'Нет доступных сертификатов для подписания документов ""%1""';
			|en = 'No available certificates to sign the ""%1"" documents'"));
	Ошибки.Вставить("ДлинаМаршрутаБольшеКоличестваДоступныхСертификатов", 
		НСтр("ru = 'Возможное количество подписей по маршруту ""%2"" больше количества доступных сертификатов по документам ""%1""';
			|en = 'Possible number of signatures by route ""%2"" is more than the number of available certificates by documents ""%1""'"));
	Ошибки.Вставить("НетДоступныхСертификатовДляНекоторыхШаговМаршрута", 
		НСтр("ru = 'Нет доступных сертификатов для подписания документов ""%1"" на некоторых шагах маршрута ""%2""';
			|en = 'No available certificates for signing the ""%1"" documents on some steps of the ""%2"" route'"));
...
Функция ВидОшибкиМаршрутПодписания()
	...
	ОписаниеРешения = НСтр("ru = '<a href = ""Проверьте"">Проверьте:</a>
		...
		|en = '<a href = ""Проверьте"">Check:</a>
		|1. Availability of certificates, specified in signing route.
		|2. Users specified in the certificates.'");
