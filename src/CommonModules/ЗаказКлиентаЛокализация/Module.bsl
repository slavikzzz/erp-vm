...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	//++ Локализация 
	СписокНалогообложенийНДСДляПроверки = Новый СписокЗначений();
	СписокНалогообложенийНДСДляПроверки.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС);
	СписокНалогообложенийНДСДляПроверки.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС);
	СписокНалогообложенийНДСДляПроверки.Добавить(Перечисления.ТипыНалогообложенияНДС.ЛьготноеНалогообложениеНДСНаУСН);
	
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту 
			И СписокНалогообложенийНДСДляПроверки.НайтиПоЗначению(Объект.НалогообложениеНДС) <> Неопределено
		...
		Если Объект.НеОтгружатьЧастями Тогда
			...
			Если НастройкиСистемыНалогообложения.ПрименяетсяУСН Тогда
				...
				Если Объект.НалогообложениеНДС <> НалогообложениеОрганизации Тогда
					...
					|en = 'The taxation as of the shipment date does not correspond to the document taxation. 
					|Correct the taxation of the current document.'");
		...
		Иначе
			...
			|en = 'The taxation as of the shipment date in line %1 does not correspond to the document taxation. 
			|For line %1, create a separate document or correct the taxation of the current document.'");
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
		...
		//++ НЕ УТ
		Документы.ВыпускПродукции.ДобавитьКомандуСоздатьОбеспечивающийЗаказ(КомандыСозданияНаОсновании);
		//-- НЕ УТ
