...
Процедура ПараметрыПодключения(
	...
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'У пользователя недостаточно прав для выполнения настройки.';
														|en = 'Insufficient rights to perform the setting.'"));
	КонецЕсли;
...
Процедура ПерейтиКДаннымЧеков(Документ) Экспорт
	...
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'У пользователя недостаточно прав для получения данных чеков.';
														|en = 'The user has insufficient rights to get receipt data.'"));
	КонецЕсли;
	...
	Если Не ОФДВызовСервера.ЭтоПолноправныйПользователь() Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Недостаточно прав доступа для просмотра журнала регистрации.';
				|en = 'Insufficient access rights to view the event log.'"));
