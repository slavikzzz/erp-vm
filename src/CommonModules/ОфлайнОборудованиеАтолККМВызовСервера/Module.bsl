...
Функция ЗагружаемыеДанныеИзККМ(ТекстовыйДокумент, ФорматОбмена, ВыходныеПараметры) Экспорт
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Загрузка невозможна. Неизвестный формат отчета.';
							|en = 'Cannot import. Unknown report format.'");
		Результат = Ложь;
	КонецЕсли;
	...
	Иначе
		...
		Если НЕ Результат Тогда
			ТекстОшибки = НСтр("ru = 'Операция прервана. Ошибка при загрузке транзакции №%1%.';
								|en = 'The operation is aborted. An error occurred when importing transaction No.%1%.'");
...
Функция ЗакрытьЧек(Смена, Транзакция, ТекстОшибки)
	...
	Если ТекущийЧек = Неопределено Тогда
		ТекстОшибки = НСтр("ru = 'Неверный формат файла. Невозможно найти чек №%1%';
							|en = 'Incorrect file format. Cannot find receipt No.%1%'");
...
Функция НомераНалоговНаККМПоУмолчанию() Экспорт
	...
	Налоги.Вставить("НомерНалога5", 7);
	Налоги.Вставить("НомерНалога7", 8);
...
Функция СпискиНалоговИКомбинацийНалогов(НалогиНаККМ)
	...
	// 5%
	СтавкаНалога5 = Новый Структура;
	СтавкаНалога5.Вставить("Код", 7);
	СтавкаНалога5.Вставить("Текст", НСтр("ru = 'НДС 5%';
										|en = 'НДС 5%'"));
	СтавкаНалога5.Вставить("Значение", 5);
	СтавкаНалога5.Вставить("НомерНалогаККМ", НалогиНаККМ.НомерНалога5);
	НалогНДС.Ставки.Добавить(СтавкаНалога5);
	
	// 7%
	СтавкаНалога7 = Новый Структура;
	СтавкаНалога7.Вставить("Код", 8);
	СтавкаНалога7.Вставить("Текст", НСтр("ru = 'НДС 7%';
										|en = 'НДС 7%'"));
	СтавкаНалога7.Вставить("Значение", 7);
	СтавкаНалога7.Вставить("НомерНалогаККМ", НалогиНаККМ.НомерНалога7);
	НалогНДС.Ставки.Добавить(СтавкаНалога7);
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Источник.ЭтоМаркированнаяПродукция) И Источник.ЭтоМаркированнаяПродукция Тогда
				Если ФорматОбмена = 8 Тогда
					...
					ИначеЕсли Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.БезалкогольныеНапитки") 
						ИЛИ Источник.ВидМаркированнойПродукции = ПредопределенноеЗначение("Перечисление.ВидыМаркированнойПродукцииБПО.СоковаяПродукция") Тогда
						ДанныеДляЗаполнения.Вставить("КодТипаНоменклатуры", 23);
					Иначе
