...
Процедура ПроверитьЗаполнениеАкцизныхМарокРегистры13(Объект, Отказ, ПроверятьТолькоЕслиЗаполнены, РазрешеноНеУказыватьПартионныеМаркиВСтроке)
	...
	Для Каждого СтрокаТЧ Из АлкогольнаяПродукция Цикл
		...
		ТекстОшибки = НСтр("ru = 'В строке %1 для алкогольной продукции """"%2"""" должно быть указано количество продукции по акцизным маркам - %3, а указано - %4.';
							|en = 'В строке %1 для алкогольной продукции """"%2"""" должно быть указано количество продукции по акцизным маркам - %3, а указано - %4.'");
		ТекстОшибки = СтрШаблон(
			ТекстОшибки,
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Если СтрокаТЧ.Поштучная = Истина Тогда
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'В строке %1 для алкогольной продукции """"%2"""" указана поштучная Справка 2 которая не может быть учтена в регистре 2 ЕГАИС.';
					|en = 'В строке %1 для алкогольной продукции """"%2"""" указана поштучная Справка 2 которая не может быть учтена в регистре 2 ЕГАИС.'"),
				СтрокаТЧ.НомерСтроки,
				СтрокаТЧ.АлкогольнаяПродукция);
			ОбщегоНазначения.СообщитьПользователю(
				ТекстОшибки,
		...
		Если НайденныеСтроки.Количество() Тогда
			ТекстОшибки = НСтр("ru = 'В строке %1 для алкогольной продукции """"%2"""" указана поштучная акцизная марка которая не может быть учтена в регистре 2 ЕГАИС.';
								|en = 'В строке %1 для алкогольной продукции """"%2"""" указана поштучная акцизная марка которая не может быть учтена в регистре 2 ЕГАИС.'");
			ТекстОшибки = СтрШаблон(ТекстОшибки,
				СтрокаТЧ.НомерСтроки,
				СтрокаТЧ.АлкогольнаяПродукция);
			ОбщегоНазначения.СообщитьПользователю(
