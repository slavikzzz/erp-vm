	...
	Для Каждого ДанныеПроверки Из ДанныеОбмена Цикл
		...
		Если ЗначениеЗаполнено(ДанныеПроверки.ЛокальныйОтветКМ) Тогда
			ДанныеЗаписи.Добавить(НСтр("ru = 'Локальный ответ ККТ на запрос КМ:';
										|en = 'Локальный ответ ККТ на запрос КМ:'"));
...
// 
// Параметры:
// 	РезультатЗапроса - см. ОбменДаннымиИСМПКлиентСервер.СтруктураДанныхЛогирования
Процедура ВывестиДанныеДляПротоколаОбмена(РезультатЗапроса) Экспорт
	...
	Для Каждого КлючИЗначение Из РезультатЗапроса.HTTPЗапросЗаголовки Цикл
	...
	ЗаписьПротокола.Запрос            = ОбщегоНазначенияИСМП.URLЗапроса(, ПараметрыОтправкиHTTPЗапросов, HTTPМетод, РезультатЗапроса.HTTPЗапросАдресРесурса);
	ЗаписьПротокола.ЗапросЗаголовки   = СтрСоединить(ЗапросЗаголовки, Символы.ПС);
	ЗаписьПротокола.ЗапросТело        = РезультатЗапроса.HTTPЗапросТело;
	
	Если ЗначениеЗаполнено(РезультатЗапроса.HTTPОтветКодСостояния) Тогда
		
		ЗаписьПротокола.ОтветЗаголовки = ОбменДаннымиИСМПКлиентСервер.ЗаголовкиИзHTTPОтвета(РезультатЗапроса.HTTPОтветЗаголовки);
		ЗаписьПротокола.ОтветТело      = РезультатЗапроса.HTTPОтветТело;
		ЗаписьПротокола.КодСостояния   = Строка(РезультатЗапроса.HTTPОтветКодСостояния);
