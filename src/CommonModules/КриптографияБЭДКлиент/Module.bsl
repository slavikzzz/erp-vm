...
Процедура Подписать(ОписаниеДанных, КонтекстДиагностики, Форма = Неопределено, Оповещение = Неопределено,
	...
	СледующийОбработчик = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОписаниеДанных, "ПередВыполнением");
	ОписаниеДанных.Вставить("ПередВыполнением", КриптографияБЭДСлужебныйКлиент.ОбработчикВыбораСертификата(
		СледующийОбработчик, КонтекстДиагностики));
	...
	ПараметрыПодписи = ЭлектроннаяПодписьКлиент.НовыйТипПодписи(ТипПодписи);
	ПараметрыПодписи.ПроверятьСертификат = РежимыПроверки.НеПроверятьСертификат;
	ЭлектроннаяПодписьКлиент.Подписать(ОписаниеДанных, Форма, СлужебноеОповещение, ПараметрыПодписи);
	
КонецПроцедуры

// Проверяет сертификат средствами БСП, а также дополнительно проверяет на условия призннания квалифицироанным.
// 
// Параметры:
//  ОбработчикРезультатаПроверки  - См. ЭлектроннаяПодписьКлиент.ПроверитьСертификат.Оповещение
//  Сертификат - См. ЭлектроннаяПодписьКлиент.ПроверитьСертификат.Сертификат
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики
//  Подпись - ДвоичныеДанные
//  МенеджерКриптографии - См. ЭлектроннаяПодписьКлиент.ПроверитьСертификат.МенеджерКриптографии
// 
Процедура ПроверитьСертифкат(ОбработчикРезультатаПроверки, Сертификат, КонтекстДиагностики, Подпись = Неопределено,
	МенеджерКриптографии = Неопределено) Экспорт
	КриптографияБЭДСлужебныйКлиент.ПроверитьСертификат(ОбработчикРезультатаПроверки, Сертификат, КонтекстДиагностики,
		МенеджерКриптографии, Подпись);
КонецПроцедуры
...
Процедура ЗаполнитьПрограммуВСертификате(ОповещениеОЗавершении, ОписаниеДанных) Экспорт
	...
	ЗаголовокФормы = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОписаниеДанных, "Операция",
		НСтр("ru = 'Заполнение программы в сертификате ЭП';
			|en = 'Fill an application in the digital signature certificate'"));
...
Процедура УстановитьCryptoPRO(ОповещениеОЗавершении, Форма) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru = 'Автоматическая установка CryptoPro CSP возможно только на операционных системах семейства Windows.';
								|en = 'Automatic installation of CryptoPro CSP is possible only on Window operational systems.'");
		ПоказатьПредупреждение(, ОписаниеОшибки,, НСтр("ru = 'Установка CryptoPro CSP';
														|en = 'CryptoPro CSP installation'"));
