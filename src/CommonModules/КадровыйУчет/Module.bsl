...
Функция НовоеФизическоеЛицо(Знач ФИО, Знач ДанныеФизическогоЛица = Неопределено, Знач ПериодДанных = Неопределено,
	...
	Если ДанныеФизическогоЛица = Неопределено Тогда
		ЗаполнитьНаименованиеФИОПол(ФизическоеЛицоОбъект, ФИО, ПериодДанных);
	Иначе
		ЗаполнитьФизическоеЛицо(ФизическоеЛицоОбъект, ФИО, ДанныеФизическогоЛица, ПериодДанных);
	КонецЕсли;
...
Функция НастройкиКадровогоУчета() Экспорт
	Возврат КадровыйУчетПовтИсп.НастройкиКадровогоУчета();
КонецФункции
		...
		Если СтрокаНеРаботающиеСотрудники.НачалоПериода = СтрокаНеРаботающиеСотрудники.ОкончаниеПериода
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Сотрудник %1 не работает в организации на дату %2г.';
					|en = 'Employee %1 does not work for the company on %2.'"),
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекстСообщенияПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По сотруднику %1, уже зарегистрировано событие ""%2"" (%3), документ противоречит кадровому приказу %4';
					|en = 'The ""%2"" event (%3) of the %1 employee is already registered, the document contradicts with the %4 personnel order'"),
				Выборка.Сотрудник,
				Выборка.ВидСобытия,
				...
				Если НачалоДня(Выборка.ДатаПриема) > НачалоДня(Выборка.ДатаСобытия) Тогда
					...
					ТекстСообщенияПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'По сотруднику %1, невозможно зарегистрировать событие ""%2"" ранее, чем дата приема на работу (%3 - %4)';
							|en = 'Cannot register the ""%2"" event for the %1 employee earlier than the hiring date (%3 - %4)'"),
						Выборка.Сотрудник,
						Выборка.ВидСобытия,
				...
				ИначеЕсли Выборка.ДатаУвольнения < Выборка.ДатаСобытия И ЗначениеЗаполнено(Выборка.ДатаУвольнения) Тогда
					...
					ТекстСообщенияПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'По сотруднику %1, невозможно зарегистрировать событие ""%2"" позднее, чем дата увольнения (%3 - %4)';
							|en = 'Cannot register the ""%2"" event for the %1 employee later than the termination of employment date (%3 - %4)'"),
						Выборка.Сотрудник,
						Выборка.ВидСобытия,
			...
			Пока Выборка.Следующий() Цикл
				...
				ТекстСообщенияПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Сотрудника %1 необходимо принять на работу ранее кадрового события ""%2"" (%3 - %4)';
						|en = 'Employee %1 should be hired earlier than HR event ""%2"" (%3 - %4)'"),
					Выборка.Сотрудник,
					Выборка.ВидСобытия,
					Формат(Выборка.Период, "ДЛФ=D"),
					Выборка.Регистратор);
			...
			Пока Выборка.Следующий() Цикл
				...
				ТекстСообщенияПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Сотрудника %1 необходимо уволить позднее кадрового события ""%2"" (%3 - %4)';
						|en = 'Employee %1 should be dismissed after HR event ""%2"" (%3 - %4)'"),
					Выборка.Сотрудник,
					Выборка.ВидСобытия,
					Формат(Выборка.Период, "ДЛФ=D"),
					Выборка.Регистратор);
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекстСообщенияПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По сотруднику %1, дата кадрового события (%2) противоречит кадровому приказу %3';
					|en = 'Date of HR event (%2) of the %1 employee contradicts with the %3 personnel order '"),
				Выборка.Сотрудник,
				Формат(Выборка.Период, "ДЛФ=D"),
				Выборка.Регистратор);
...
Процедура ПроверитьЗаполнениеДатыРожденияПоВременнойТаблице(МенеджерВременныхТаблиц, ИмяВТОтбораСотрудников, ДатаРожденияВТаблице = Истина) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'У сотрудника %1 не заполнена дата рождения';
				|en = 'Date of birth is not populated for employee %1'"), Выборка.Сотрудник);
	...
	Если ВсегоОшибок > КоличествоСообщений Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '..и еще у %1 сотрудников';
				|en = '..and other %1 employees'"), ВсегоОшибок - КоличествоСообщений);
	...
	ВызватьИсключение НСтр("ru = 'У одного или нескольких сотрудников не заполнена дата рождения.';
							|en = 'Date of birth is not populated for one or several employees.'");
...
Функция КадровыеДанныеОсновногоСотрудникаФизическогоЛица(Организация, ФизическоеЛицо, ИменаПолей, ДатаПолучения = '00010101', ТолькоРазрешенные = Истина, ТекстОшибки = "") Экспорт
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Не удалось получить кадровые данные основного сотрудника ""%1"".';
							|en = 'Cannot receive the main employee ""%1"" HR data.'");
...
Функция ДоговорыФизическихЛиц(ТаблицаФизическихЛиц) Экспорт
	...
	ТаблицаДоговоров.Колонки.Добавить("НСПЗ",			 	Новый ОписаниеТипов("Булево"));
	...
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	РабочиеМестаСотрудников.ДокументОснование КАК ДоговорГПХ
		|ПОМЕСТИТЬ ВТДоговорыГПХ
		|ИЗ
		|	ВТРабочиеМестаСотрудников КАК РабочиеМестаСотрудников
		|ГДЕ
		|	ТИПЗНАЧЕНИЯ(РабочиеМестаСотрудников.ДокументОснование) В (ТИП(Документ.ДоговорРаботыУслуги), &ТипДоговораАвторскогоЗаказа)";
	Запрос.Выполнить();
	
	Если ЗарплатаКадры.ВТСодержитСтроки(Запрос.МенеджерВременныхТаблиц, "ВТДоговорыГПХ") Тогда
		СоздатьВТОбложениеДоговоровГПХНСПЗ(Запрос.МенеджерВременныхТаблиц);
	Иначе
		Запрос.Текст =
			"ВЫБРАТЬ
			|	НЕОПРЕДЕЛЕНО КАК ДоговорГПХ,
			|	ЛОЖЬ КАК ОблагаетсяНСПЗ
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ОбложениеДоговоровГПХНСПЗ.ОблагаетсяНСПЗ, НЕОПРЕДЕЛЕНО) КАК НСПЗ,
		|	ВЫБОР
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОбложениеДоговоровГПХНСПЗ КАК ОбложениеДоговоровГПХНСПЗ
		|		ПО РабочиеМестаСотрудников.ДокументОснование = ОбложениеДоговоровГПХНСПЗ.ДоговорГПХ
		|ГДЕ
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	Договоры.Период КАК Период,
		|	Договоры.Сотрудник КАК Сотрудник,
		|	Договоры.ФизическоеЛицо КАК ФизическоеЛицо,
		|	Договоры.ВидДоговора КАК ВидДоговора,
		|	Договоры.ВидДоговораГПХ КАК ВидДоговораГПХ,
		|	Договоры.НСПЗ КАК НСПЗ,
		|	Договоры.Номер КАК Номер,
		|	Договоры.Дата КАК Дата,
		|	Договоры.Организация КАК Организация,
		|	Договоры.Начало КАК Начало,
		|	Договоры.Окончание КАК Окончание,
		...
		|	ДоговорыСОкончанием.НСПЗ КАК НСПЗ,
...
Процедура СоздатьВТОбложениеДоговоровГПХНСПЗ(МенеджерВременныхТаблиц)
	
	КадровыйУчетВнутренний.СоздатьВТОбложениеДоговоровГПХНСПЗ(МенеджерВременныхТаблиц);
...
Процедура СоздатьВТОсновныеСотрудникиФизическихЛицВПериодах(МенеджерВременныхТаблиц, ТолькоРазрешенные, Знач ГоловнаяОрганизация, Знач ИмяВТФизическиеЛицаПериоды, Знач ИмяВТОсновныеСотрудникиФизическихЛицВПериодах = "ВТОсновныеСотрудникиФизическихЛицВПериодах") Экспорт
	...
	Если ФОИспользоватьНачисленияПоДоговорам Тогда
		...
		Если Не ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.УправлениеШтатнымРасписанием") Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ОсновныеСотрудникиФизическихЛиц.ДолжностьПоШтатномуРасписанию", "NULL");
		КонецЕсли;
		
		Если Не ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.КадровыйУчетРасширенная") Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ОсновныеСотрудникиФизическихЛиц.ВидДоговора", "NULL");
		КонецЕсли;
...
Функция НеобходимыСведенияЗависящиеОтФункциональныхОпцийОрганизации(Знач ИмяПоля) Экспорт
	...
	Возврат КадровыйУчетВнутренний.НеобходимыСведенияЗависящиеОтФункциональныхОпцийОрганизации(ИмяПоля);
...
Процедура ДобавитьТекстыЗапросовВТСведенийЗависящихОтФункциональныхОпцийОрганизаций(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень, ИсточникиДанных) Экспорт
	
	КадровыйУчетВнутренний.ДобавитьТекстыЗапросовВТСведенийЗависящихОтФункциональныхОпцийОрганизаций(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень, ИсточникиДанных);
...
Функция ПутьКДаннымСведенийОКлассахУсловийТруда(ИмяПоля) Экспорт
	...
	Возврат КадровыйУчетВнутренний.ПутьКДаннымСведенийОКлассахУсловийТруда(ИмяПоля);
	
КонецФункции

Процедура ПриПолученииЗапросаВТСведенияОКлассахУсловийТруда(Запрос, ТолькоРазрешенные, ИмяСоздаваемойТаблицы) Экспорт
	
	КадровыйУчетВнутренний.ПриПолученииЗапросаВТСведенияОКлассахУсловийТруда(Запрос, ТолькоРазрешенные, ИмяСоздаваемойТаблицы);
...
Процедура ДополнитьПоляЗапросаВТКадровыхДанных(Организация, Запрос) Экспорт
	...
	КадровыйУчетВнутренний.ДополнитьПоляЗапросаВТКадровыхДанных(Организация, Запрос);
...
Процедура ЗаполнитьПараметрыКадровогоПриказа(Параметры, КадровыеДанные, УчитыватьНастройкиФормированияПечатныхФорм = Истина) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(КадровыеДанные, "НомерДок") Тогда
		ПолеНомер = "НомерДок";
	Иначе
		ПолеНомер = "Номер";
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(КадровыеДанные, "НомерПервичногоДокумента") Тогда
		Параметры.НомерДок = ЗарплатаКадрыОтчеты.НомерНаПечать(КадровыеДанные[ПолеНомер], КадровыеДанные.НомерПервичногоДокумента);
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(КадровыеДанные, "НомерПриказа") Тогда
		Параметры.НомерДок = ЗарплатаКадрыОтчеты.НомерНаПечать(КадровыеДанные[ПолеНомер], КадровыеДанные.НомерПриказа);
	Иначе
...
Процедура ИнициализироватьОтчетТ2(ОтчетОбъект) Экспорт
	...
	КадровыйУчетВнутренний.ИнициализироватьОтчетТ2(ОтчетОбъект);
...
Функция ЗапросВТСотрудникиОрганизацииПоРабочимМестам(ТолькоРазрешенные, ИмяВТСотрудникиОрганизации, ИмяВТСотрудникиПериоды, Параметры, МенеджерВременныхТаблиц = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.Подработки") Тогда
		...
		ИспользоватьПодработки = ПолучитьФункциональнуюОпцию(ИмяФункциональнойОпции)
			Или НастройкиКадровогоУчета().ИспользуютсяДанныеСотрудниковВнешнихИсточников;
	КонецЕсли;
	...
	Иначе
		...
		Если РаботникиПоДоговорамГПХ = Неопределено Тогда
			...
			Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет")
				Или НастройкиКадровогоУчета().ИспользуютсяДанныеСотрудниковВнешнихИсточников Тогда
...
Процедура СоздатьВТРабочиеМестаСотрудников(МенеджерВременныхТаблиц, ТолькоРазрешенные, Параметры, ИмяВТРабочиеМестаСотрудников = "ВТРабочиеМестаСотрудников") Экспорт
	...
	Если ПривилегированныйРежим() Тогда
		...
		Если Параметры.Свойство("ИмяВТСотрудникиПериоды") Тогда
			СоздатьВТРабочиеМестаСотрудниковПоВременнойТаблице(МенеджерВременныхТаблиц, Ложь, Параметры, ИмяВТРабочиеМестаСотрудников);
		Иначе
			Запрос = ЗапросВТРабочиеМестаСотрудниковПоСпискуСотрудников(Ложь, ИмяВТРабочиеМестаСотрудников, Параметры, МенеджерВременныхТаблиц);
			Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
			Запрос.Выполнить();
		КонецЕсли;
...
Процедура ОбновитьОсновныхСотрудниковФизическихЛицПоТрудовымДоговорам(Сотрудники) Экспорт
	...
	Запрос.Текст =
		...
		|	ВидыЗанятостиСотрудников.ВидЗанятости КАК ВидЗанятости,
		|	ВЫБОР
		|		КОГДА ТекущиеДанные.ДатаПриема > ВидыЗанятостиСотрудников.Период
		|			ТОГДА ТекущиеДанные.ДатаПриема
		...
		|		ПО ВидыЗанятостиСотрудников.Сотрудник = ТекущиеДанные.Сотрудник
		|ГДЕ
		...
		|	КОНЕЦ КАК ДатаОкончания
		|ИЗ
		|	ВТВидыЗанятостиОсновныхСотрудников КАК ВидыЗанятостиСотрудников
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТВидыЗанятостиОсновныхСотрудников КАК ВидыЗанятостиСотрудниковПредыдущие
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТВидыЗанятостиОсновныхСотрудников КАК ВидыЗанятостиСотрудниковСледующие
	...
	Для Каждого ДанныеСотрудников Из ДанныеСсылок Цикл
		...
		Для Каждого ДанныеНаПечать Из ДанныеСотрудников.Строки Цикл
			...
			Иначе
				...
				ВыводитьРасшифровку = ДанныеНаПечать.Строки.Количество() > 1;
				Если Не ВыводитьРасшифровку
					И ДанныеНаПечать.Строки.Количество() = 1
					И ЗначениеЗаполнено(ДанныеНаПечать.СсылкаНаОбъектПродолжительностьОсновногоОтпуска) Тогда
				    ВыводитьРасшифровку = ДанныеНаПечать.Строки[0].СсылкаНаОбъектДанныеОтпусковВидОтпускаОтпускЯвляетсяЕжегодным;
				КонецЕсли;
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет даты запрета редактирования документов ""Кадровый перевод"".';
									|en = 'Fills in closing dates of editing the ""Employee transfer"" document.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет даты запрета редактирования документов ""Прием на работу"".';
									|en = 'Fills in closing dates of editing the ""Hiring"" document.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет даты запрета редактирования документов ""Увольнение"".';
									|en = 'Fills in closing dates of editing the ""Termination of employment"" document.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет служебные данные регистра сведений ""Фамилия, имя, отчество физического лица"".';
									|en = 'Populates internal data of the ""Individual last name, first name, middle name"" information register.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет инициалы в справочнике ""Физические лица"".';
									|en = 'Fills in the initials in the  ""Individuals"" catalog.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет инициалы в регистре сведений ""Фамилия, имя, отчество физического лица"".';
									|en = 'Fills in the initials in the ""Individual last name, first name, middle name"" information register.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет не верно сформированные движения регистра сведений ""Кадровая история сотрудников"".';
									|en = 'Populates the incorrectly generated records of the ""Employment record of employees"" information register.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает признак используемости оснований увольнения';
									|en = 'Indicates that grounds for termination of employment are used'");
	Обработчик.Процедура = "Справочники.ОснованияУвольнения.ОбновитьОснованияВАрхиве";
	
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.Справочники.ОснованияУвольнения) Тогда
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.29.52";
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.Справочники.ОснованияУвольнения) Тогда
		...
		Обработчик.Процедура = "Справочники.ОснованияУвольнения.ИсправитьОшибочнуюСтатьюОснования";
		Обработчик.ОбщиеДанные = Истина;
	КонецЕсли;
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.30.51";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("8876f68d-7165-438b-a497-b9b9bbf0b4fe");
	Обработчик.Процедура = "РегистрыСведений.ФИОФизическихЛиц.ЗаполнитьФИОСлужебныеСБуквамиЕЁ";
	Обработчик.Комментарий = НСтр("ru = 'Заполняет служебные данные регистра сведений ""Фамилия, имя, отчество физического лица"".';
									|en = 'Populates internal data of the ""Individual last name, first name, middle name"" information register.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.30.96";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("ef5fd17a-cb9f-4216-8909-c70a2e0856ed");
	Обработчик.Процедура = "Справочники.ФизическиеЛица.ЗаполнитьФИОСлужебныеСБуквамиЕЁ";
	Обработчик.Комментарий = НСтр("ru = 'Заполняет служебные данные справочника ""Физические лица"".';
									|en = 'Заполняет служебные данные справочника ""Физические лица"".'");
...
Процедура ЗаполнитьДокументыФизическихЛицПоКлассификаторуМВД() Экспорт
	...
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.ПаспортСССР,			"", 						"01", НСтр("ru = 'ПАСПОРТ';
																															|en = 'PASSPORT'"));		// R-ББ 999999
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.ЗагранпаспортСССР,		"", 						"02", НСтр("ru = 'ЗГПАСПОРТ';
																															|en = 'FOREIGN PASSPORT'"));		// 99 0999999
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.СвидетельствоОРождении,	"", 						"03", НСтр("ru = 'СВИД О РОЖД';
																																|en = 'BIRTH CERTIFICATE'"));	// R-ББ 999999
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.УдостоверениеОфицера,	"", 						"04", НСтр("ru = 'УДОСТ ОФИЦЕРА';
																															|en = 'OFFICER ID'"));	// ББ 9999990
	ОписатьДокумент(, НСтр("ru = 'Справка об освобождении';
							|en = 'Release certificate'"),														"05", НСтр("ru = 'СПРАВКА ОБ ОСВ';
																																|en = 'RELEASE CERTIFICATE'"));	// -
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.ПаспортМинморфлота, 	"",							"06", НСтр("ru = 'ПАСПОРТ МОРФЛТ';
																																|en = 'MINISTRY OF MARINE PASSPORT'"));	// ББ 999999
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.ВоенныйБилет,
		НСтр("ru = 'Военный билет солдата (матроса, сержанта, старшины)';
			|en = 'Military ID card of a soldier  (sailor, sergeant, first sergeant)'"), 										"07", НСтр("ru = 'ВОЕННЫЙ БИЛЕТ';
																															|en = 'MILITARY ID'"));	// ББ 0999999
	// -
	ОписатьДокумент(, НСтр("ru = 'Временное удостоверение, выданное взамен военного билета';
							|en = 'Temporary certificate issued instead of the military ID'"),					"08", НСтр("ru = 'ПРОЧЕЕ';
																															|en = 'OTHER'"));
	// 99 9999999
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.ДипломатическийПаспорт, "",							"09", НСтр("ru = 'ДИППАСПОРТ РФ';
																																|en = 'RF DIPLOMATIC PASSPORT'"));
	ОписатьДокумент(, НСтр("ru = 'Иностранный паспорт';
							|en = 'Foreign passport'"),															"10", НСтр("ru = 'ИНПАСПОРТ';
																																|en = 'FOREIGN PASSPORT'"));		// -
	// -
	ОписатьДокумент(, НСтр("ru = 'Свидетельство о регистрации ходатайства иммигранта о признании его беженцем';
							|en = 'Certificate of registration of immigrant''s request for recognition as a refugee'"), "11", НСтр("ru = 'СВИД БЕЖЕНЦА';
																															|en = 'REFUGEE CERTIFICATE'"));
	ОписатьДокумент(, НСтр("ru = 'Вид на жительство';
							|en = 'Lawful permanent residence'"),															"12", НСтр("ru = 'ВИД НА ЖИТЕЛЬ';
																															|en = 'LAWFUL PERMANENT RESID'"));	// -
	ОписатьДокумент(, НСтр("ru = 'Удостоверение беженца';
							|en = 'Refugee certificate'"),														"13", НСтр("ru = 'УДОСТ БЕЖЕНЦА';
																															|en = 'REFUGEE CERTIFICATE'"));	// -
	// -
	ОписатьДокумент(, НСтр("ru = 'Временное удостоверение личности гражданина РФ';
							|en = 'Temporary identity card of the Russian Federation citizen'"),								"14", НСтр("ru = 'ВРЕМ УДОСТ';
																															|en = 'TEMP CERTIF'"));
	// -
	ОписатьДокумент(, НСтр("ru = 'Разрешение на временное проживание в Российской Федерации';
							|en = 'Permit for temporary residence in the Russian Federation'"),					"15", НСтр("ru = 'ПРОЧЕЕ';
																															|en = 'OTHER'"));
	ОписатьДокумент(,
		// ББ-999 9999999
		НСтр("ru = 'Свидетельство о предоставлении временного убежища на территории Российской Федерации';
			|en = 'Certificate on granting temporary asylum in the territory of the Russian Federation'"), 		"18", НСтр("ru = 'ПРОЧЕЕ';
																																|en = 'OTHER'"));
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.ПаспортРФ,				"",							"21", НСтр("ru = 'ПАСПОРТ РОССИИ';
																																|en = 'RUSSIAN PASSPORT'"));	// 99 99 9999990
	// 99 9999999
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.ЗагранпаспортРФ,		"",							"22", НСтр("ru = 'ЗГПАСПОРТ РФ';
																																|en = 'RF FOREIGN PASSPORT'"));
	ОписатьДокумент(, 
		// -
		НСтр("ru = 'Свидетельство о рождении, выданное уполномоченным органом иностранного государства';
			|en = 'Birth certificate issued by an authorized body of foreign state'"), 		"23", НСтр("ru = 'ПРОЧЕЕ';
																															|en = 'OTHER'"));
	ОписатьДокумент(, НСтр("ru = 'Удостоверение личности военнослужащего Российской Федерации';
							|en = 'RF military ID '"),					"24", НСтр("ru = 'ПРОЧЕЕ';
																																|en = 'OTHER'"));			// ББ 9999999
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.ПаспортМоряка,			"",							"26", НСтр("ru = 'ПАСПОРТ МОРЯКА';
																																|en = 'SAILOR''S PASSPORT'"));	// ББ 0999999
	ОписатьДокумент(Справочники.ВидыДокументовФизическихЛиц.ВоенныйБилетОфицераЗапаса, "",						"27", НСтр("ru = 'БИЛЕТ ОЗ';
																															|en = 'RO MILITARY ID '"));		// ББ 0999999
	ОписатьДокумент(, НСтр("ru = 'Иные документы, выдаваемые органами МВД';
							|en = 'Other documents issued by MIA authorities'"),										"91", НСтр("ru = 'ПРОЧЕЕ';
																																|en = 'OTHER'"));			// -
...
Функция ПоляПредставленийКадровыхДанныхФизическихЛиц(Знач ПутьКПолямЛичныхДанных = "ЛичныеДанные", ИсключаемыеПоля = Неопределено) Экспорт
	
	ДополнительныеПоляПредставлений = ЗарплатаКадрыОбщиеНаборыДанных.ПустаяТаблицаДополнительныхПолейПредставлений();
	ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
	
	Возврат ДополнительныеПоляПредставлений;
	
КонецФункции

Функция ПоляПредставленийКадровыхДанныхСотрудников(Знач ПутьКПолямСотрудника = "Работа", Знач ПутьКПолямЛичныхДанных = "ЛичныеДанные", ИсключаемыеПоля = Неопределено) Экспорт
	
	ДополнительныеПоляПредставлений = ЗарплатаКадрыОбщиеНаборыДанных.ПустаяТаблицаДополнительныхПолейПредставлений();
	ЗаполнитьПоляПредставленийКадровыхДанныхСотрудников(ДополнительныеПоляПредставлений, ПутьКПолямСотрудника, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
...
Процедура ЗаполнитьКлассификаторОснованийУвольнения(ИмяСправочника, Макет, НормативныйДокумент = Неопределено) Экспорт
	...
	Для Каждого СтрокаМакета Из ДанныеМакетаКлассификатора Цикл
		...
		Если ЗначениеЗаполнено(СтрокаМакета.ExpirationDate) Тогда
		...
		Если ЗначениеЗаполнено(СтрокаМакета.CodePFR) Тогда
...
Функция ОсновныеСотрудникиИнформационнойБазы(ТолькоРазрешенные, ФизическиеЛица, ДатаСведений) Экспорт
	...
	Запрос.Текст =
		...
		|		ПО (ОсновныеСотрудникиФизическихЛиц.ГоловнаяОрганизация = Организации.Ссылка
...
// Возвращает информацию о действующих приостановлениях трудовых договоров.
//
// Параметры:
//	ТолькоРазрешенные - Булево
...
// Возвращает пустую таблицу значений с периодами приостановления трудовых договоров.
//
// Возвращаемое значение:
//	ТаблицаЗначений:
...
Функция ЗаполнитьДанныеДокументовПоИностраннымГражданам(ДанныеДляЗаполнения, СсылкаНаДокумент) Экспорт
	...
	Возврат КадровыйУчетВнутренний.ЗаполнитьДанныеДокументовПоИностраннымГражданам(ДанныеДляЗаполнения, СсылкаНаДокумент);
	
КонецФункции

// Заполняет физическое лицо по данным переданным в структуре.
// 
// Параметры:
//  ФизическоеЛицо - СправочникОбъект.ФизическиеЛица
//  ФИО - см. НовоеФизическоеЛицо.ФИО
//  ДанныеФизическогоЛица - см. НовыеДанныеФизическогоЛица
//  ПериодДанных - см. НовоеФизическоеЛицо.ПериодДанных
Процедура ЗаполнитьФизическоеЛицо(ФизическоеЛицо, Знач ФИО, Знач ДанныеФизическогоЛица,
	Знач ПериодДанных = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ФизическоеЛицо.Отчество) Тогда
	...
	Если ДанныеФизическогоЛица.Свойство("УточнениеНаименования") Тогда
		...
		Если ЗначениеЗаполнено(ДанныеФизическогоЛица.УточнениеНаименования) Тогда
			ФизическоеЛицо.УточнениеНаименования = ДанныеФизическогоЛица.УточнениеНаименования;
		КонецЕсли;
	КонецЕсли;
	...
	ЗаполнитьЗначенияСвойств(ФизическоеЛицо, ДанныеФизическогоЛица, , ИсключаемыеСвойства);
	
	Если ДанныеФизическогоЛица.Свойство("Ссылка") 
		И ЗначениеЗаполнено(ДанныеФизическогоЛица.Ссылка) Тогда
		
		ФизическоеЛицо.УстановитьСсылкуНового(ДанныеФизическогоЛица.Ссылка);
		
	КонецЕсли;
	
	ФизическоеЛицо.ДополнительныеСвойства.Вставить("СвязанныеОбъекты", СвязанныеОбъектыДанныхФизическогоЛица(
		ДанныеФизическогоЛица));
		
	ЗаполнитьНаименованиеФИОПол(ФизическоеЛицо, ФИО, ПериодДанных);
...
Функция СоздатьВТОбразованияФизическихЛиц(МенеджерВременныхТаблиц, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ИмяВТОбразованияФизическихЛиц = "ВТОбразованияФизическихЛиц") Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ОбразованияФизическихЛиц") Тогда
		...
		Возврат МодульОбразованияФизическихЛиц.СоздатьВТОбразованияФизическихЛиц(МенеджерВременныхТаблиц, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ИмяВТОбразованияФизическихЛиц);
	Иначе
		...
		|	NULL КАК Ссылка,
		|	NULL КАК ВидОбразования,
		|	NULL КАК ВидПослевузовскогоОбразования,
		|	NULL КАК ВидДополнительногоОбучения,
		|	NULL КАК УчебноеЗаведение,
		|	NULL КАК Начало,
		|	NULL КАК Окончание,
		|	NULL КАК Специальность,
		|	NULL КАК Квалификация,
		|	NULL КАК НаименованиеКурса,
		|	NULL КАК КоличествоЧасов,
		|	NULL КАК ВидДокумента,
		|	NULL КАК Серия,
		|	NULL КАК Номер,
		|	NULL КАК ДатаВыдачи,
		|	NULL КАК ЗаПределамиРФ,
		|	NULL КАК Основание
...
Функция ОснованиеДляРеестра(Дата, Номер, НомерПриказа = Неопределено) Экспорт 
	
	НомерПриказаНаПечать = ЗарплатаКадрыОтчеты.НомерНаПечать(Номер, НомерПриказа);
	Возврат ОснованиеДляРеестраПоДатеИНомеруПриказа(Дата, НомерПриказаНаПечать);
	
КонецФункции

Функция ОснованиеДляРеестраПоДатеИНомеруПриказа(ДатаПриказа, НомерПриказа) Экспорт 
	
	Возврат СтрШаблон(НСтр("ru = 'Пр. № %1 от %2';
							|en = 'Order No. %1, %2'"), НомерПриказа, Формат(ДатаПриказа, "ДЛФ=D"));
		...
		Если Не ЗначениеЗаполнено(ДанныеСотрудника.ВидЗанятости) Тогда
			...
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Сотрудник %1 еще не оформлен на работу';
					|en = '%1 employee is not recruited yet'"),
		...
		ИначеЕсли Не ЗначениеЗаполнено(ДанныеСотрудника.ДатаУвольнения) Тогда
			...
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По сотруднику %1 еще не оформлено увольнение';
					|en = 'Termination of employment is not registered for the %1 employee '"),
				ДанныеСотрудника.Сотрудник);
...
Функция ДанныеОтсутствийСотрудникаДляПособияПоБезработице(КадровыеДанныеСотрудников) Экспорт
	...
	Возврат КадровыйУчетВнутренний.ДанныеОтсутствийСотрудникаДляПособияПоБезработице(КадровыеДанныеСотрудников);
...
Процедура ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля) Экспорт
	...
	КадровыйУчетВнутренний.ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
	
КонецПроцедуры

Процедура ЗаполнитьПоляПредставленийКадровыхДанныхСотрудников(ДополнительныеПоляПредставлений, ПутьКПолямСотрудника, ПутьКПолямЛичныхДанных, ИсключаемыеПоля)
	
	КадровыйУчетВнутренний.ЗаполнитьПоляПредставленийКадровыхДанныхСотрудников(ДополнительныеПоляПредставлений, ПутьКПолямСотрудника, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
	...
	Иначе
		...
		Если (РаботникиПоТрудовымДоговорам = Неопределено И ПодработкиРаботниковПоТрудовымДоговорам <> Истина И РаботникиПоДоговорамГПХ = Ложь)
			...
			ВызватьИсключение НСтр("ru = 'Не верно заданы параметры получения сотрудников организации';
									|en = 'Parameters of company employee receipt are specified incorrectly'");
		...
		КонецЕсли;
		Если УстановленыОтборы Или СписокФизическихЛиц <> Неопределено Тогда
			ТекстыЗапроса.Добавить(
				"ИНДЕКСИРОВАТЬ ПО
				|	Сотрудник");
		КонецЕсли;
...
Функция ЗапросВТСотрудникиОрганизацииПоРабочимМестамСКадровымУчетом(ТолькоРазрешенные, ИмяВТСотрудникиОрганизации, ИмяВТСотрудникиПериоды, Параметры, МенеджерВременныхТаблиц)
	...
	ТекстЗапросаСотрудникиОрганизацииДляКадровойИстории =
		...
		|	РабочиеМестаСотрудников.Сотрудник КАК Сотрудник,
		|	МАКСИМУМ(РабочиеМестаСотрудников.ПериодЗаписи) КАК ПериодЗаписи
		|ПОМЕСТИТЬ ВТМаксимальныеПериодыЗаписиРабочихМестСотрудников
		|ИЗ
		|	ВТРабочиеМестаСотрудниковДляВТСотрудникиОрганизации КАК РабочиеМестаСотрудников
		|
		|СГРУППИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	МаксимальныеПериодыЗаписи.Сотрудник КАК Сотрудник,
		|	МаксимальныеПериодыЗаписи.ПериодЗаписи КАК ПериодЗаписи,
		|	РабочиеМестаСотрудников.Организация КАК Организация
		|ПОМЕСТИТЬ ВТМаксимальныеПериодыЗаписиОрганизацииФункциональныхОпций
		|ИЗ
		|	ВТМаксимальныеПериодыЗаписиРабочихМестСотрудников КАК МаксимальныеПериодыЗаписи
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТРабочиеМестаСотрудниковДляВТСотрудникиОрганизации КАК РабочиеМестаСотрудников
		|		ПО МаксимальныеПериодыЗаписи.Сотрудник = РабочиеМестаСотрудников.Сотрудник
		|			И МаксимальныеПериодыЗаписи.ПериодЗаписи = РабочиеМестаСотрудников.ПериодЗаписи
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	РабочиеМестаСотрудников.Сотрудник КАК Сотрудник,
		|	МаксимальныеПериодыЗаписиОрганизацииФункциональныхОпций.Организация КАК ОрганизацияФункциональныхОпций,
		|	РабочиеМестаСотрудников.ГоловнойСотрудник КАК ГоловнойСотрудникТаблицыРазличных,
		|	РабочиеМестаСотрудников.ФизическоеЛицо КАК ФизическоеЛицо,
		|	МаксимальныеПериодыЗаписиОрганизацииФункциональныхОпций.ПериодЗаписи КАК ПериодЗаписи,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСотрудникиПериоды КАК СотрудникиПериоды
		|		ПО РабочиеМестаСотрудников.Сотрудник = СотрудникиПериоды.Сотрудник
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТМаксимальныеПериодыЗаписиОрганизацииФункциональныхОпций КАК МаксимальныеПериодыЗаписиОрганизацииФункциональныхОпций
		|		ПО РабочиеМестаСотрудников.Сотрудник = МаксимальныеПериодыЗаписиОрганизацииФункциональныхОпций.Сотрудник";
	...
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьЗапросУничтоженияВременнойТаблицы(Запрос.Текст, "ВТМаксимальныеПериодыЗаписиРабочихМестСотрудников");
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьЗапросУничтоженияВременнойТаблицы(Запрос.Текст, "ВТМаксимальныеПериодыЗаписиОрганизацииФункциональныхОпций");
...
// Заполняет наименование, ФИО и пол физического лица по данныи о ФИО
//
// Параметры:
//  ФизическоеЛицо - СправочникОбъект.ФизическиеЛица
//  ФИО - см. НовоеФизическоеЛицо.ФИО
//  ПериодДанных - см. НовоеФизическоеЛицо.ПериодДанных
Процедура ЗаполнитьНаименованиеФИОПол(ФизическоеЛицо, Знач ФИО, Знач ПериодДанных = Неопределено)
	...
	ФизическоеЛицо.ФИО = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ФИОИзЧастей(ФамилияИмяОтчество);
	ЗаполнитьЗначенияСвойств(ФизическоеЛицо, ФамилияИмяОтчество, "Фамилия, Имя, Отчество");
	ФизическоеЛицо.ДополнительныеСвойства.Вставить("ФИОУстановлены", Истина);
	
	Если Не ЗначениеЗаполнено(ФизическоеЛицо.Пол) Тогда
		ФизическоеЛицо.Пол = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ПолПоЧастямФИО(ФамилияИмяОтчество);
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Наименование) Тогда
		...
		ДанныеФИО.Вставить("ФИО", ФамилияИмяОтчество);
		ДанныеФИО.Вставить("Период", ПериодДанных);
		
		Если ФизическоеЛицо.ДополнительныеСвойства.Свойство("СвязанныеОбъекты") Тогда
			СвязанныеОбъекты = ФизическоеЛицо.ДополнительныеСвойства.СвязанныеОбъекты;
		Иначе
			СвязанныеОбъекты = Новый Структура;
		КонецЕсли;
		СвязанныеОбъекты.Вставить("ФИО", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДанныеФИО));
		ФизическоеЛицо.ДополнительныеСвойства.Вставить("СвязанныеОбъекты", СвязанныеОбъекты);
