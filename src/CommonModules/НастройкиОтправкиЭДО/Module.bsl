...
Функция ИдентификаторыПолучателейОтправителя(ИдентификаторОтправителя, Отправитель, ВидыДокументов) Экспорт
	...
	|ВЫБРАТЬ
	|	НастройкиВТ.Получатель КАК Получатель,
	|	ПРЕДСТАВЛЕНИЕ(НастройкиВТ.Получатель) КАК Представление,
...
Процедура СоздатьНастройкиОтправки(Знач Организация, Знач Контрагент, Знач ДоговорКонтрагента,
	...
	ЭтоОбменЧерезОператора = РегламентыЭДО.ЭтоОбменЧерезОператора(Данные.СпособОбменаЭД);
	...
	Для каждого СтрокаНастроек Из ТекущаяТаблица Цикл
		...
		Если ЭтоОбменЧерезОператора Тогда
			ТипДокумента = ТипыДокументовПоВиду[СтрокаНастроек.ВидДокумента];
			СтрокаНастроек.ТребуетсяИзвещениеОПолучении = СтрокаНастроек.ТребуетсяИзвещениеОПолучении
				И НЕ НастройкиЭДО.ОбменПоТипуДокументаМожноЗавершитьБезИзвещения(ТипДокумента);
		КонецЕсли;
	...
	Исключение
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Создание настроек отправки электронных документов';
														|en = 'Create electronic document dispatch settings'"),
...
Функция НастройкаЗаполненияДополнительныхПолей(КлючНастроек, Формат = Неопределено, ТекстОшибки = Неопределено) Экспорт
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Произошла ошибка при чтении настроек заполнения дополнительных полей.';
							|en = 'An error occurred while reading the settings of additional fields filling in.'");
		
		МассивСтрок = Новый Массив;
		МассивСтрок.Добавить(ТекстОшибки);
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Отправитель: %1';
											|en = 'Sender: %1'"), КлючНастроек.Отправитель));
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Получатель: %1';
											|en = 'Recipient: %1'") , КлючНастроек.Получатель));
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Договор: %1';
											|en = 'Contract: %1'")    , КлючНастроек.Договор));
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Вид электронного документа: %1';
											|en = 'Electronic document kind: %1'"), КлючНастроек.ВидДокумента));
		...
		ОбработкаНеисправностейБЭД.ОбработатьОшибку(НСтр("ru = 'Чтение настроек заполнения дополнительных полей.';
														|en = 'Read additional field filling settings.'"),
...
#Область РегистрыСведений_НастройкиЗаполненияДополнительныхПолей_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление настроек заполнения дополнительных полей.';
									|en = 'Update settings of additional field populating.'");
...
#Область РегистрыСведений_НастройкиОтправкиЭлектронныхДокументовПоВидам_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = 
		НСтр("ru = '1С:Обмен с контрагентами: обновление настроек отправки электронных документов по видам';
			|en = '1C:Exchange with counterparties: update of settings of electronic document sending by kinds'");
...
#Область РегистрыСведений_НастройкиОтправкиЭлектронныхДокументов_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = 
		НСтр("ru = '1С:Обмен с контрагентами: обновление настроек отправки электронных документов';
			|en = '1C:Exchange with counterparties: update electronic document sending settings'");
