...
Функция ПолучитьФайлИзИнтернет(Знач URL, Знач НастройкаСохранения, Знач НастройкаСоединения,
			...
			Если ОтветHTTP.КодСостояния = 301 // 301 Moved Permanently
				...
				Если Перенаправления.Количество() > 7 Тогда
					ВызватьИсключение(НСтр("ru = 'Превышено количество перенаправлений.';
											|en = 'Redirections limit exceeded.'"), КатегорияОшибки.ОшибкаСети);
				КонецЕсли;
	...
	Иначе
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не указано место для сохранения файла при вызове %1.';
				|en = 'File save location is not specified for ""%1"".'"), "ПолучитьФайлИзИнтернет"),
