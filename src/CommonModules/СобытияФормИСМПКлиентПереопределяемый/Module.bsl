...
Процедура ОткрытьФормуПодбораНоменклатуры(Форма, ОповещениеПриЗавершении = Неопределено) Экспорт
	...
	Если ТипЗначенияСсылки = Тип("ДокументСсылка.ПеремещениеМеждуМОДИСМП") Тогда
		...
		ВидыПродукцииПеремещенияМеждуМОД = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ВидыПродукцииВнутреннегоПеремещенияМеждуМОД();
		ОсобенностиУчета                 = Новый СписокЗначений;
		
		Для Каждого ВидПродукции Из ВидыПродукцииПеремещенияМеждуМОД Цикл
			ОсобенностиУчета.Добавить(ИнтеграцияИСУТКлиентСервер.ОсобенностьУчетаПоВидуПродукции(ВидПродукции));
		КонецЦикла;
		
		ПараметрыФормы.Вставить("ОсобенностьУчета", ОсобенностиУчета);
