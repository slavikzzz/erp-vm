		...
		Если МассивНеНайденныхРолей.Количество() = 1 Тогда
			ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Не найдено описания для подписей роли %1';
						|en = 'Description for signatures of the %1 role not found'"), 
					ИмяРоли);
		ИначеЕсли МассивНеНайденныхРолей.Количество() > 1 Тогда
			ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Не найдено описания для подписей ролей: %1';
						|en = 'Description for signatures of the following roles not found: %1'"), 
...
//
Процедура ЗаполнитьСведенияОПодписяхДокументовПоОрганизации(ЗаполняемыеЗначения, ДатаЗначений = Неопределено) Экспорт
	...
	ДатаСведений = ДатаЗначений;
	УстановитьПривилегированныйРежим(Истина);
	Если ДатаСведений <> Неопределено И Константы.ИспользоватьПоследниеПодписиДокументов.Получить() Тогда
		ДатаСведений = Неопределено;
	КонецЕсли;
	УстановитьПривилегированныйРежим(Ложь);
	
	СведенияОПодписях = СведенияОПодписяхДокументов(ОписаниеПолейПодписантов, Организация, ДатаСведений);
...
Процедура СоздатьВТОснованияПолномочийФизическихЛиц(МенеджерВременныхТаблиц, ИмяОтбора = "ВТОрганизацииФизическиеЛица") Экспорт
	...
	Запрос.Текст = 
		...
		|			И ОснованияПолномочийОтветственныхЛиц.ДатаНачала <= ОрганизацииФизическиеЛица.Период
		|			И (ВЫБОР
		|				КОГДА ОснованияПолномочийОтветственныхЛиц.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)
		|					ТОГДА ИСТИНА
		|				ИНАЧЕ ОснованияПолномочийОтветственныхЛиц.ДатаОкончания >= ОрганизацииФизическиеЛица.Период
		|			КОНЕЦ)";
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение реквизита Должность в основаниях полномочий из сведений об ответственных лицах.';
									|en = 'Fill in the Position attribute in the authority sources from the information about responsible persons.'");
...
// Возвращает сведения об ответственных лицах организации, их должностях и основаниях подписи для документа объекта
//
// Параметры:
//   Объект -  ДоументОбъект - Заполняемый документ объект.
//   ДатаСведений  - Дата    - Дата получения среза ответственных лиц.
//
// Возвращаемое значение:
//   Структура - Для каждой роли возвращается 3 поля:
//       * <ИмяРоли>                 - СправочникСсылка.ФизическиеЛица, Неопределено
//       * <ИмяРоли>Должность        - СправочникСсылка.Должности, Неопределено
//       * <ИмяРоли>ОснованиеПодписи - Строка, Неопределено
Функция ОтветственныеЛицаДляЗаполнения(Объект, ДатаСведений) Экспорт
	
	Организация = Неопределено;
	СведенияОПодписях = Новый Структура;
	
	Менеджер = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(Объект.Метаданные().ПолноеИмя());
	ОписаниеПолей = Менеджер.ОписаниеПодписейДокумента();
	
	Для Каждого РеквизитыПодписи Из ОписаниеПолей Цикл
		Организация = Объект[РеквизитыПодписи.Организация];
		СведенияОПодписях.Вставить(РеквизитыПодписи.ФизическоеЛицо);
		СведенияОПодписях.Вставить(РеквизитыПодписи.Должность);
		СведенияОПодписях.Вставить(РеквизитыПодписи.ОснованиеПодписи);
	КонецЦикла;
	
	Если ЗначениеЗаполнено(Организация) Тогда
		ОписаниеПолей = Общегоназначения.ТаблицаЗначенийВМассив(ОписаниеПолей);
		ОписаниеПолей = ПодписиДокументовКлиентСервер.СоответствиеОписанийИзКоллекцииОписаний(ОписаниеПолей);
		СведенияОПодписях = СведенияОПодписяхДокументов(ОписаниеПолей, Организация, ДатаСведений, ТипЗнч(Объект.Ссылка));
	КонецЕсли;
	
	Возврат СведенияОПодписях;
...
Функция СоответствиеОписанийИменРеквизитовПоСтруктуре(ЗаполняемыеЗначения, Организация)
	...
	ПодписиДокументовПереопределяемый.ЗаполнитьСоответствиеИдентификаторамиОтветственныхЛицОрганизаций(ИдентификаторыКЗаполнению, ЗаполненныеИдентификаторы);
...
Процедура ЗаполнитьСведенияПоОснованиямПолномочийВПодписяхДокументов(ОписаниеПолейПодписантов, Организация, ЗначенияПодписейДокументов, ДатаСведений = Неопределено)
	...
	Для Каждого ОписаниеПолейПодписанта Из ОписаниеПолейПодписантов Цикл
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(ДанныеФизическогоЛица.ОснованиеПодписи)
			И ЗначениеЗаполнено(ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураИменРеквизитовОбъекта, "ОснованиеПодписи")) Тогда
