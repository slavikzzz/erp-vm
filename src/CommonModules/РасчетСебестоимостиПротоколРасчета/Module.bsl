		...
		ТекстОшибки =
			РасчетСебестоимостиПрикладныеАлгоритмы.СлужебныйСимвол_ПроблемаУжеЗарегистрирована()
		 	+ НСтр("ru = 'Расчет себестоимости завершился аварийно.
				...
				|en = 'Cost calculation aborted.
				|See details in the event log.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
#Область ЗаголовокПротокола
	...
	Если НЕ ПротоколРасчета.РасчетЗавершен ИЛИ БылиОшибки Тогда
		Протокол.ДобавитьСтроку(НСтр("ru = 'ВНИМАНИЕ !';
									|en = 'WARNING!'", ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
	
	Если НЕ ПротоколРасчета.РасчетЗавершен Тогда
		Протокол.ДобавитьСтроку(НСтр("ru = 'Это промежуточный протокол расчета - расчет еще не завершен!';
									|en = 'This is an intermediate calculation log - the calculation is not yet complete!'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если БылиОшибки Тогда
		...
		Протокол.ДобавитьСтроку(ПредставлениеОшибокРасчета(
			...
			НСтр("ru = 'В ходе расчета были диагностированы ошибки:';
				|en = 'Errors were found during calculation:'", ОбщегоНазначения.КодОсновногоЯзыка()),
			Символы.ПС + "	- "));
		Протокол.ДобавитьСтроку(НСтр("ru = 'Подробное описание ошибок можно найти (Ctrl-F) в тексте протокола по символу ""!"".';
									|en = 'You can find (Ctrl+F) detailed error description by the ""!"" character in the protocol text.'", ОбщегоНазначения.КодОсновногоЯзыка()));
		
		Если БылиОшибкиДвижений ИЛИ БылиОшибкиЗаписи Тогда
			Протокол.ДобавитьСтроку(НСтр("ru = 'Расчет завершился аварийно.';
										|en = 'Calculation aborted.'", ОбщегоНазначения.КодОсновногоЯзыка()));
		КонецЕсли;
	...
	Протокол.ДобавитьСтроку(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Протокол расчета (идентификатор %1)';
			|en = 'Calculation protocol (ID %1)'", ОбщегоНазначения.КодОсновногоЯзыка()),
		ПротоколРасчета.Идентификатор));
	Протокол.ДобавитьСтроку(НСтр("ru = 'Платформа:';
								|en = 'Platform:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + СистемнаяИнформация.ВерсияПриложения);
	Протокол.ДобавитьСтроку(НСтр("ru = 'Конфигурация:';
								|en = 'Configuration:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + ПредставлениеЗначения(Метаданные.Имя)
		+ ", " + НСтр("ru = 'версия:';
						|en = 'version:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + ПредставлениеЗначения(Метаданные.Версия));
	Протокол.ДобавитьСтроку(НСтр("ru = 'Информационная база:';
								|en = 'Infobase:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " 
		+ ПредставлениеЗначения(ПараметрыРасчета.СтрокаСоединенияИБ));
	Протокол.ДобавитьСтроку(НСтр("ru = 'Начало расчета:';
								|en = 'Calculation start:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + ПредставлениеЗначения(ПротоколРасчета.ДатаНачала)
		+ ", " + НСтр("ru = 'длительность:';
						|en = 'duration:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + ПредставлениеЗначения(ПротоколРасчета.Длительность));
	Протокол.ДобавитьСтроку(НСтр("ru = 'Место вызова расчета:';
								|en = 'Calculation call place:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " 
		+ ПредставлениеЗначения(ПараметрыРасчета.МестоВызоваРасчета));
	
	Протокол.ДобавитьСтроку("");
	Протокол.ДобавитьСтроку(НСтр("ru = 'Вариант расчета:';
								|en = 'Calculation option:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " 
		+ ПредставлениеЗначения(ПараметрыРасчета.ВариантРасчета));
	Протокол.ДобавитьСтроку(НСтр("ru = 'Режим закрытия месяца:';
								|en = 'Month-end closing mode:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " 
		+ ПредставлениеЗначения(ПараметрыРасчета.РежимЗакрытияМесяца));
	Протокол.ДобавитьСтроку(НСтр("ru = 'Период расчета:';
								|en = 'Calculation period:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
	...
	Протокол.ДобавитьСтроку(НСтр("ru = 'Организации для расчета:';
								|en = 'Companies for calculation:'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если ЗначениеЗаполнено(ПараметрыРасчета.ИзмененоДокументов) Тогда
		Протокол.ДобавитьСтроку(НСтр("ru = 'Изменено документов в ИБ после прошлого расчета:';
									|en = 'Changed documents in the infobase after the previous calculation:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
	...
	ИменаРасширений = НСтр("ru = 'Активные расширения конфигурации:';
							|en = 'Active configuration extensions:'")
	...
	Протокол.ДобавитьСтроку(НСтр("ru = 'Функциональные опции, влияющие на результат расчета:';
								|en = 'Functional options which influence the calculation result:'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Для Каждого ТекущийПараметр Из ПараметрыГруппы Цикл
			...
			Если ТекущееЗначениеПараметра <> ТекущийПараметр.ЗначениеПоУмолчанию Тогда
				ДополнениеКОписанию = "; (" + НСтр("ru = 'значение по умолчанию';
													|en = 'default value'", ОбщегоНазначения.КодОсновногоЯзыка()) + ": "
	...
	Если ПараметрыРасчета.Свойство("Движения") Тогда
		...
		Протокол.ДобавитьСтроку(НСтр("ru = 'Обслуживаемые регистры:';
									|en = 'Maintained registers:'", ОбщегоНазначения.КодОсновногоЯзыка()));
			...
			Протокол.ДобавитьСтроку("	- " + ОписаниеРегистра.Значение.ПолноеИмяРегистра
				+ ?(ПараметрыРасчета.РегистрыСРасчетнымиОборотами.Свойство(ОписаниеРегистра.Ключ),
					" (" + НСтр("ru = 'кэш оборотов';
								|en = 'turnover cache'", ОбщегоНазначения.КодОсновногоЯзыка()) + ")", "")
				+ ?(ПараметрыРасчета.РегистрыСРасчетнымиОстатками.Свойство(ОписаниеРегистра.Ключ),
					" (" + НСтр("ru = 'кэш остатков';
								|en = 'balance cache'", ОбщегоНазначения.КодОсновногоЯзыка()) + ")", ""));
		КонецЦикла;
...
#Область ЭтапыРасчета
	...
	Протокол.ДобавитьСтроку(НСтр("ru = 'Протокол выполнения этапов расчета:';
								|en = 'Protocol of settlement step execution:'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Для Каждого ОписаниеЭтапа Из ПротоколРасчета.Этапы Цикл
		...
		Протокол.ДобавитьСтроку(НСтр("ru = 'Начало этапа:';
									|en = 'Stage start:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + ПредставлениеЗначения(ОписаниеЭтапа.ДатаНачала)
			+ ", " + НСтр("ru = 'длительность:';
							|en = 'duration:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + ПредставлениеВремениВыполнения(ОписаниеЭтапа, ПротоколРасчета));
		Протокол.ДобавитьСтроку(НСтр("ru = 'Время от начала расчета до начала этапа:';
									|en = 'Time from calculation start to stage start:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + ПредставлениеВремени(ОписаниеЭтапа.ДатаНачала - ПротоколРасчета.ДатаНачала));
		...
		Если ЗначениеЗаполнено(ОписаниеЭтапа.СформированныеДвижения) Тогда
			Протокол.ДобавитьСтроку(НСтр("ru = 'Сформированы движения по регистрам (всего / уникальных):';
										|en = 'Records in registers are generated (total / unique):'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Если ЗначениеЗаполнено(ОписаниеЭтапа.СформированныеСтрокиВТ) Тогда
			Протокол.ДобавитьСтроку(НСтр("ru = 'Сформированы строки вспомогательных временных таблиц (всего / уникальных):';
										|en = 'Rows of auxiliary temporary tables are generated (total / unique):'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Если ЗначениеЗаполнено(ОписаниеЭтапа.ВременныеТаблицы) Тогда
			...
			Протокол.ДобавитьСтроку(НСтр("ru = 'Сформированы временные таблицы (размер / время (% времени этапа) / уточнение / исходные таблицы):';
										|en = 'Temporary tables are generated (size / time (% of step time) / clarification / original tables):'", ОбщегоНазначения.КодОсновногоЯзыка()));
			...
			Если ЗначениеЗаполнено(ВремяФормированияВсехТаблиц.ОбщееВремя) Тогда
				Протокол.ДобавитьСтроку(НСтр("ru = 'Общее время формирования таблиц:';
											|en = 'Total time of table generation:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
		...
		Если ЗначениеЗаполнено(ОписаниеЭтапа.ЗамерыВыполненияКода) Тогда
			...
			Протокол.ДобавитьСтроку(НСтр("ru = 'Выполнены замеры фрагментов кода (время (% времени этапа) / уточнение):';
										|en = 'Measurements of code fragments are completed (time (% of the stage time) / specification):'", ОбщегоНазначения.КодОсновногоЯзыка()));
			...
			Протокол.ДобавитьСтроку(НСтр("ru = 'Общее время замеров выполнения кода:';
										|en = 'Total time for measurements of code completion:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
		...
		Если ЗначениеЗаполнено(ОписаниеЭтапа.ДополнительнаяИнформация) Тогда
			...
			Протокол.ДобавитьСтроку(НСтр("ru = 'Дополнительная информация об этапе:';
										|en = 'Additional step information:'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Если ЗначениеЗаполнено(ОписаниеЭтапа.ИнформацияОбОшибках) Тогда
			ВывестиИнформациюОбОшибках(
				...
				НСтр("ru = 'При выполнении этапа расчета обнаружены ошибки';
					|en = 'Errors are detected while performing the calculation step'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
#Область ИтоговыеПоказатели
	...
	Протокол.ДобавитьСтроку(НСтр("ru = 'Итоговая информация о длительности этапов (абсолютная (относительная)):';
								|en = 'Summary stage duration (absolute (relative)):'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Для Каждого ОписаниеЭтапа Из ПротоколРасчета.Этапы Цикл
		...
		НоваяСтрока.Этап = НСтр("ru = 'Этап';
								|en = 'Stage'", ОбщегоНазначения.КодОсновногоЯзыка()) + " №" + СокрЛП(Счетчик) + ". " + ОписаниеЭтапа.ПредставлениеЭтапа;
	...
	Протокол.ДобавитьСтроку(НСтр("ru = 'ТОП-10 самых длительных этапов:';
								|en = 'TOP 10 longest stages:'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Протокол.ДобавитьСтроку(НСтр("ru = 'Итоговая информация о сформированных движениях по регистрам (всего / уникальных):';
								|en = 'Summary of generated records in registers (total / unique):'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Для Каждого ОписаниеДвижений Из ПротоколРасчета.СформированныеДвижения Цикл
			...
			Если ОписаниеДвижений.Значение.Ошибочных > 0 Тогда
				...
				Протокол.ДобавитьСтроку("		- " + НСтр("ru = 'в т.ч. ошибочных:';
															|en = 'including error:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
	...
	Иначе
		Протокол.ДобавитьСтроку(НСтр("ru = 'Нет сформированных движений.';
									|en = 'No generated movements.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
	...
	Если БылиОшибкиДвижений Тогда
		ВывестиИнформациюОбОшибках(
			...
			НСтр("ru = 'Формирование движений выполнено с ошибками';
				|en = 'Movement generation is completed with errors'", ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
	...
	Если ПараметрыРасчета.ОбщееКоличествоФЗЗаписи > 0 Тогда
		...
		Протокол.ДобавитьСтроку(НСтр("ru = 'Всего запущено фоновых заданий записи движений:';
									|en = 'Total started background jobs for saving movements:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
	...
	Протокол.ДобавитьСтроку("");
	Протокол.ДобавитьСтроку(НСтр("ru = 'Итоговая информация о записанных движениях по регистрам (документов / записей / время записи):';
								|en = 'Summary of saved records in registers (documents / records / record time):'", ОбщегоНазначения.КодОсновногоЯзыка())); // количество записей - с учетом "первичных"
	...
	Если НЕ БылиОшибкиЗаписи Тогда
		...
		Если КоличествоЗаписей > 0 ИЛИ КоличествоДокументов > 0 Тогда
			Протокол.ДобавитьСтроку(НСтр("ru = 'Всего записано движений по регистрам:';
										|en = 'Total records saved for registers:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
		...
		ИначеЕсли КоличествоЗаписей = 0 И КоличествоДокументов = 0 Тогда
			Протокол.ДобавитьСтроку(НСтр("ru = 'Запись движений не выполнялась - нет измененных движений.';
										|en = 'Records have not been saved - no changed records.'", ОбщегоНазначения.КодОсновногоЯзыка()));
		КонецЕсли;
	...
	Иначе
		...
		ВывестиИнформациюОбОшибках(
			...
			НСтр("ru = 'Запись движений выполнена с ошибками';
				|en = 'Errors occurred when saving records'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если ЗначениеЗаполнено(ПротоколРасчета.ЗаписьДвижений) Тогда
		...
		Протокол.ДобавитьСтроку(НСтр("ru = 'Длительность операций записи:';
									|en = 'Duration of saving operations:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
	...
	Если ПараметрыРасчета.Свойство("ЛогЗаписиДвижений") И ЗначениеЗаполнено(ПараметрыРасчета.ЛогЗаписиДвижений) Тогда
		Протокол.ДобавитьСтроку("");
		Протокол.ДобавитьСтроку(НСтр("ru = 'Лог работы механизма записи движений:';
									|en = 'Лог работы механизма записи движений:'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
#Область СохранениеПротоколаРасчета
	...
	ЗаписьПротокола.ОписаниеОшибок 			 = ?(ПротоколРасчета.РасчетЗавершен, ПредставлениеОшибокРасчета(ПротоколРасчета), НСтр("ru = 'Расчет не завершен';
																																		|en = 'Calculation is not completed'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура НачалоЭтапаРасчета(ПараметрыРасчета, ПредставлениеЭтапа, УровеньЭтапа = 1) Экспорт
	...
	ПолноеПредставлениеЭтапа =
		?(ПараметрыРасчета.ИдетРасчетПартий, НСтр("ru = 'Партионный учет';
													|en = 'Lot accounting'", ОбщегоНазначения.КодОсновногоЯзыка()), НСтр("ru = 'Расчет себестоимости';
																														|en = 'Cost calculation'", ОбщегоНазначения.КодОсновногоЯзыка()))
		+ ": " + ПредставлениеЭтапа;
...
Процедура ОтметкаВЖурналеРегистрации(ПараметрыРасчета, ПредставлениеЭтапа, Уровень = Неопределено, Комментарий = "")
	...
	Если НЕ ЗначениеЗаполнено(Комментарий) Тогда
		Комментарий = НСтр("ru = 'Начало выполнения этапа.';
							|en = 'Step execution start.'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
	
	ЗаписьЖурналаРегистрации(
		РасчетСебестоимостиПрикладныеАлгоритмы.ИмяСобытияЖурналаРегистрации(
			ПараметрыРасчета, НСтр("ru = 'Этап';
									|en = 'Stage'", ОбщегоНазначения.КодОсновногоЯзыка()) + "." + ПредставлениеЭтапа),
		Уровень,
		,
		,
		Комментарий);
...
Функция ПредставлениеЗначения(Значение, ПредставлениеДатыКакПериодРасчета = Ложь) Экспорт
	...
	Если ТипЗнч(Значение) = Тип("Булево") Тогда
		...
		Возврат Формат(Значение, НСтр("ru = 'БЛ=Нет; БИ=Да';
										|en = 'BF=No; BT=Yes'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("ПеречислениеСсылка.МетодыОценкиСтоимостиТоваров") Тогда
		...
		Если НЕ ЗначениеЗаполнено(Значение) Тогда
			Возврат "<" + НСтр("ru = 'Не используется';
								|en = 'Not used'", ОбщегоНазначения.КодОсновногоЯзыка()) + ">";
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("ПеречислениеСсылка.ТипыЗаписейПартий") Тогда
		...
		Если НЕ ЗначениеЗаполнено(Значение) Тогда
			Возврат "<" + НСтр("ru = 'Пустой тип записи партии';
								|en = 'Empty lot record type'", ОбщегоНазначения.КодОсновногоЯзыка()) + ">";
		КонецЕсли;
...
Функция ПредставлениеОшибокРасчета(ПротоколРасчета, Заголовок = Неопределено, Разделитель = Неопределено)
    ...
    ТипыОшибок.Вставить(Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаВИсходныхДанныхДляРасчета,
        НСтр("ru = 'в исходных данных для расчета';
			|en = 'in source data for calculation'", ОбщегоНазначения.КодОсновногоЯзыка()));
    ТипыОшибок.Вставить(Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаИнициализацииПараметровРасчета,
        НСтр("ru = 'при инициализации параметров расчета';
			|en = 'when initializing calculation parameters'", ОбщегоНазначения.КодОсновногоЯзыка()));
    ТипыОшибок.Вставить(Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаВУчетныхПолитикахОрганизаций,
        НСтр("ru = 'в учетных политиках организаций';
			|en = 'in accounting policies of companies'", ОбщегоНазначения.КодОсновногоЯзыка()));
    ТипыОшибок.Вставить(Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаВосстановленияДвиженийДокументов,
        НСтр("ru = 'при восстановлении движений документов';
			|en = 'when restoring migrations of documents'", ОбщегоНазначения.КодОсновногоЯзыка()));
    ТипыОшибок.Вставить(Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаИнициализацииДокументовРасчетаСебестоимости,
        НСтр("ru = 'при инициализации документов расчета себестоимости';
			|en = 'when initializing cost calculation documents'", ОбщегоНазначения.КодОсновногоЯзыка()));
    ТипыОшибок.Вставить(Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаРаспределенияПартий,
        НСтр("ru = 'при распределении партий';
			|en = 'when allocating lots'", ОбщегоНазначения.КодОсновногоЯзыка()));
    ТипыОшибок.Вставить(Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаРаспределенияНДС,
        НСтр("ru = 'при распределении НДС';
			|en = 'when allocating VAT'", ОбщегоНазначения.КодОсновногоЯзыка()));
    ТипыОшибок.Вставить(Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаВыполненияРегламентныхОпераций,
        НСтр("ru = 'при выполнении связанных регламентных операций закрытия месяца';
			|en = 'when performing related by-the-book month-end closing transactions'", ОбщегоНазначения.КодОсновногоЯзыка()));
    ТипыОшибок.Вставить(Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаФормированияДвиженийПоРегистрам,
        НСтр("ru = 'при формировании движений по регистрам';
			|en = 'when forming migrations by registers'", ОбщегоНазначения.КодОсновногоЯзыка()));
    ТипыОшибок.Вставить(Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаЗаписиДвиженийПоРегистрам,
        НСтр("ru = 'при записи движений по регистрам';
			|en = 'when saving migrations by registers'", ОбщегоНазначения.КодОсновногоЯзыка()));
    ТипыОшибок.Вставить(Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаВОстаткахПослеРасчета,
        НСтр("ru = 'в остатках после расчета';
			|en = 'in balance after calculation'", ОбщегоНазначения.КодОсновногоЯзыка()));
    ТипыОшибок.Вставить(Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаВремениВыполнения,
        НСтр("ru = 'при выполнения кода (run-time error)';
			|en = 'when performing code (run-time error)'", ОбщегоНазначения.КодОсновногоЯзыка()));
    ТипыОшибок.Вставить(Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаВНачальныхОстаткахПУ22,
        НСтр("ru = 'в начальных остатках при переходе на партионный учет версии 2.2';
			|en = 'in opening balance when migrating to lot accounting 2.2'", ОбщегоНазначения.КодОсновногоЯзыка()));	
...
Функция ЗаголовокПротоколаПриВыводе(ПериодРасчета) Экспорт
	...
	ТекстЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Протокол расчета периода %1';
			|en = 'Period calculation protocol %1'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ВывестиИнформациюОбОшибках(ПараметрыРасчета, Протокол, ДанныеОшибок, Заголовок, ПредставлениеЭтапа = Неопределено)
	...
	Для Каждого КлючИЗначение Из СоответствиеОшибок Цикл
		...
		Если СоответствиеОшибок.Количество() > 1 Тогда
			...
			ОписаниеОшибок = ОписаниеОшибок + ?(ОписаниеОшибок = "", "", "
				|") + НСтр("ru = 'Тип ошибки';
							|en = 'Error type'", ОбщегоНазначения.КодОсновногоЯзыка()) + ": """ + ПредставлениеЗначения(КлючИЗначение.Ключ) + """.";
	...
	ОписаниеОшибок =
		...
		+ ?(НомерОшибки > 0,  " (" + ПредставлениеЗначения(НомерОшибки) + " " + НСтр("ru = 'шт.';
																					|en = 'pcs.'", ОбщегоНазначения.КодОсновногоЯзыка()) + ")", "")
