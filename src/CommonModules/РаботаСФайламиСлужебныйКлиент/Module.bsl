...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
	...
	ФильтрДиалога = ?(ПараметрыВыполнения.Свойство("ФильтрДиалогаВыбора"),
		ПараметрыВыполнения.ФильтрДиалогаВыбора, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Все файлы (%1)|%1';
...
Процедура ПоказатьПредупреждениеОНеобходимостиРасширенияРаботыСФайлами(ОбработчикРезультата, ПредставлениеКоманды = "") Экспорт
	Если Не КлиентПоддерживаетСинхронныеВызовы() Тогда
		ТекстПредупреждения = НСтр("ru = 'Действие ""%1"" доступно только в клиентском приложении.';
									|en = 'The ""%1"" action is only available in the client application.'");
	Иначе
		ТекстПредупреждения = НСтр("ru = 'Для выполнения команды ""%1"" установите расширение для работы с 1С:Предприятием.';
									|en = 'To run the ""%1"" command, install 1C:Enterprise Extension.'");
	КонецЕсли;
	...
	Если Файл.Существует() Тогда
		...
		Исключение
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
				НСтр("ru = 'Работа с файлами.Ошибка установки времени изменения';
					|en = 'Работа с файлами.Ошибка установки времени изменения'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
...
Процедура ОбновитьСостояниеОСохраненииФайлов(Знач ВыбранныеФайлы,
	...
	Если ВыбранныеФайлы.Количество() > 1 Тогда
		Если ТекущаяПозиция = Неопределено Тогда
			ПоказатьОповещениеПользователя(НСтр("ru = 'Сохранение файлов';
												|en = 'Save files'"),, НСтр("ru = 'Сохранение файлов успешно завершено';
	...
	Иначе
		Если ТекущаяПозиция = Неопределено Тогда
			...
			ПоказатьОповещениеПользователя(НСтр("ru = 'Сохранение файла';
												|en = 'Save files'"), , ТекстПояснения, БиблиотекаКартинок.ДиалогИнформация);
		КонецЕсли;
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Файлы';
				|en = 'Files'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура ОбновитьИзФайлаНаДиске(ОбработчикРезультата, ДанныеФайла, ИдентификаторФормы,
	...
	Если ПараметрыДобавленияФайла <> Неопределено
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Размер файла превышает %1 Мб.';
				|en = 'The file size exceeds %1 MB.'"), ПараметрыДобавленияФайла.МаксимальныйРазмер);
	...
	ПоказатьОповещениеПользователя(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Файл ""%1""
			           ...
			           |en = 'File ""%1""
			           |is locked for editing.'"), Строка(ДанныеФайла.Ссылка)),
		БиблиотекаКартинок.ДиалогИнформация);
...
Функция РабочийКаталогДоступен(РабочийКаталогВладельца, Владелец)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Рабочий каталог %1 для папки файлов %2 не существует или нет права на запись. Восстановлены настройки по умолчанию.
			...
			|%3';
			|en = 'Рабочий каталог %1 для папки файлов %2 не существует или нет права на запись. Восстановлены настройки по умолчанию.
			|Технические подробности:
			|%3'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, 
			РабочийКаталогВладельца, Владелец, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru = 'Работа с файлами';
				|en = 'File operations'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
...
Процедура ПолучитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Контекст.Вставить("ЗаголовокОшибки",
		НСтр("ru = 'Не удалось получить файл на компьютер из приложения по причине:';
			|en = 'Cannot get the file from the application. Reason:'") + Символы.ПС);
	...
	Если Контекст.ДляРедактирования
	   И Контекст.ДанныеФайла.Редактирует <> ПользователиКлиент.АвторизованныйПользователь() Тогда
		...
		Результат.Вставить("ОписаниеОшибки", Контекст.ЗаголовокОшибки 
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Файл уже редактирует %1.';
					|en = 'The file is locked by %1.'"), Строка(Контекст.ДанныеФайла.Редактирует)));
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
		Возврат;
	КонецЕсли;
...
Процедура ПоместитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Контекст.Вставить("ЗаголовокОшибки",
		НСтр("ru = 'Не удалось поместить файл с компьютера в приложение по причине:';
			|en = 'Cannot store the local file to the application. Reason:'") + Символы.ПС);
...
#Если ВебКлиент Тогда
	ВернутьРезультатПослеПоказаПредупреждения(ОбработчикРезультата, НСтр("ru = 'Очистка рабочего каталога возможна только в приложении (тонкий клиент).';
																		|en = 'You can clear the working directory only in the thin client.'"), Неопределено);
	Возврат;
#КонецЕсли
...
Процедура ИмпортФайловПослеПроверкиРазмеров(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.КоличествоСуммарное = 0 Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, НСтр("ru = 'Нет файлов для добавления';
																								|en = 'There are no files to add.'"), Неопределено);
		Возврат;
	КонецЕсли;
...
Процедура ИмпортФайловПослеЦиклаПродолжение(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество() > 1 Тогда
		ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Импорт файлов завершен. Импортировано файлов: %1';
				|en = 'The upload is completed. Files uploaded: %1.'"), Строка(ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество()) );
...
Процедура СохранитьКакСРасширениемПослеРасшифровки(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Все файлы (*.%1)|*.%1';
			|en = 'All files (*.%1)|*.%1'"), Расширение);
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Файл успешно сохранен';
										|en = 'File saved'"), , ПолноеИмяФайла);
...
Процедура ПроверитьПредельныйРазмерФайлов(ОбработчикРезультата, ПараметрыПроверки)
	...
	Если МассивСлишкомБольшихФайлов.Количество() <> 0 Тогда 
		...
		Для Каждого Файл Из МассивСлишкомБольшихФайлов Цикл
			...
			ТекстСтроки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2 МБ)';
																						|en = '%1 (%2 MB)'"), Строка(Файл), Строка(РазмерФайлаВМб));
...
Процедура ПоказатьИнформациюФайлНеБылИзменен(ОбработчикРезультата)
	...
	Если ПерсональныеНастройки.ПоказыватьИнформациюЧтоФайлНеБылИзменен Тогда
		ТекстНапоминания = НСтр("ru = 'Версия не была создана, т.к. файл не изменен. Комментарий не сохранен.';
								|en = 'Cannot create a version as the file is not changed. The comment is discarded.'");
...
Процедура Зашифровать(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор) Экспорт
	...
	Если ПараметрыВыполнения.ДанныеФайла.КоличествоВерсий > 1 Тогда
		ПредставлениеФайла = ПредставлениеФайла + " (" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Версий: %1';
				|en = 'Versions: %1'"), ПараметрыВыполнения.ДанныеФайла.КоличествоВерсий) + ")";
	КонецЕсли;
...
Процедура Расшифровать(ОбработчикРезультата, ФайлСсылка, УникальныйИдентификатор, ДанныеФайла) Экспорт
	...
	Если ДанныеФайла.КоличествоВерсий > 1 Тогда
		ПредставлениеФайла = ПредставлениеФайла + " (" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Версий: %1';
				|en = 'Versions: %1'"), ДанныеФайла.КоличествоВерсий) + ")";
	КонецЕсли;
	СписокПредставлений = Новый СписокЗначений;
...
Процедура ДобавитьФайл(
	...
	Если ПараметрыДобавления = Неопределено
		...
		ПараметрыВыполнения.Вставить("ФильтрДиалогаВыбора",  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Все файлы (%1)|%1';
																															|en = 'All files (%1)|%1'"), ПолучитьМаскуВсеФайлы()));
	...
	Для Каждого ВыбранныйФайл Из ВыбранныеФайлы Цикл
		...
		Попытка
			...
			ТекстСостояния = НСтр("ru = 'Загрузка файлов с компьютера...';
									|en = 'Importing files from computer...'");
...
Процедура ВыполнитьСравнениеФайлов(ПутьКФайлу1, ПутьКФайлу2, СпособСравненияВерсийФайлов, ЗаголовокЛевый = "", ЗаголовокПравый = "") Экспорт
	...
	Исключение
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru = 'Файлы.Сравнение файлов';
				|en = 'Files.Compare files'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
...
Процедура СравнитьТабличныеДокументы(ПутьКФайлу1, ПутьКФайлу2, ЗаголовокЛевый, ЗаголовокПравый)
	...
	ПараметрыОткрытияФормы.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Сравнение %1 с %2';
			|en = 'Compare %1 to %2'"), ЗаголовокЛевый, ЗаголовокПравый);
...
Процедура ОбработатьФайлПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ТипЗнч(ПолученныеФайлы) <> Тип("Массив") Или ПолученныеФайлы.Количество() = 0 Тогда
		ОбработатьФайлПослеОшибки(НСтр("ru = 'Получение файла было отменено.';
										|en = 'Getting the file was canceled.'"), Неопределено, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьФайлПослеПомещенияФайлов(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ТипЗнч(ПомещенныеФайлы) <> Тип("Массив") Или ПомещенныеФайлы.Количество() = 0 Тогда
		ОбработатьФайлПослеОшибки(НСтр("ru = 'Помещение файла было отменено.';
										|en = 'Storing the file was canceled.'"), Неопределено, Контекст);
		Возврат;
	КонецЕсли;
...
Функция ПолучитьФайлВРабочийКаталог(Знач АдресДвоичныхДанныхФайла,
	...
	Исключение
		СообщениеОбОшибке = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		СообщениеОбОшибке = НСтр("ru = 'Не удалось создать каталог на компьютере:';
								|en = 'Не удалось создать каталог на компьютере:'") + " " + СообщениеОбОшибке;
		ОбщегоНазначенияКлиент.СообщитьПользователю(СообщениеОбОшибке);
		Возврат Ложь;
	КонецПопытки;
...
Функция ПодключенныеУстройства(Форма, ПодключаемыйМодуль)
	...
	Исключение   
		...
		ПоказатьОшибкуСканирования(Форма, НСтр("ru = 'Не удалось получить список устройств';
												|en = 'Cannot get the list of devices'"), ПредставлениеОшибки);
	КонецПопытки;
...
Функция НастройкаСканера(Форма, ПодключаемыйМодуль, ИмяУстройства, ИмяНастройки) Экспорт
	...
	Исключение
		...
		ПоказатьОшибкуСканирования(Форма, НСтр("ru = 'Не удалось получить настройки устройства.';
												|en = 'Cannot get the device settings.'"), ПредставлениеОшибки);
		Возврат -1;
	КонецПопытки;
...
Функция ЕстьУстройства(Форма, ПодключаемыйМодуль, ПоказыватьОшибку = Истина) Экспорт
	...
	Исключение
		...
		Если ПоказыватьОшибку Тогда
			ПоказатьОшибкуСканирования(Форма, НСтр("ru = 'Не удалось проверить наличие устройств';
													|en = 'Cannot check for devices'"), ПредставлениеОшибки);
		КонецЕсли;
		ЕстьУстройства = Ложь;
	КонецПопытки;
...
Процедура НачатьСканирование(Форма, ПодключаемыйМодуль, ПараметрыСканирования) Экспорт
	...
	Исключение
		...
		ПоказатьОшибкуСканирования(Форма, НСтр("ru = 'Не удалось отсканировать документ';
												|en = 'Cannot scan the document'"), ПредставлениеОшибки);
	КонецПопытки;
	...
	Если Не Результат.Успешно Тогда
		...
		ЗаписатьЖурналСканирования("ФайлКомпоненты", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Каталог журнала сканирования недоступен %1. Использование каталога отменено.';
				|en = 'The scan log directory is unavailable %1. Directory usage is cancelled.'"), 
...
Процедура ЗаписатьЖурналСканирования(Событие, Комментарий = "", ЭтоОшибка = Ложь) Экспорт 
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
		НСтр("ru = 'Сканирование изображений';
			|en = 'Scan images'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()) + "." + Событие,
...
Процедура ПолучитьТехническуюИнформацию(ПодробноеПредставлениеОшибки, ОповещениеПриЗавершении = Неопределено) Экспорт
	...
	Фильтр = НСтр("ru = 'Отчет о проблеме';
...
#Если Не ВебКлиент Тогда	
	...
	Если ИмяЛогФайла = Неопределено Тогда
		ЗаписатьЖурналСканирования("ФайлКомпоненты", НСтр("ru = 'Не задано имя файла журнала компоненты сканирования.';
															|en = 'Specify the name of the scanning add-in log file.'"), 
			Истина);
	Иначе
	...
	Текст =  НСтр("ru = 'Информация о компьютере:';
					|en = 'Computer information:'") + Символы.ПС + СтандартныеПодсистемыКлиент.ИнформацияДляПоддержки();
	...
	Если РезультатИнициализации.Подключено Тогда
		Попытка
			...
			ИнформацияОКомпоненте = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Версия компоненты %1: %2';
					|en = '%1 add-in version: %2'"), "ImageScan", ВерсияКомпоненты);
		Исключение
		...
		Для Каждого Предупреждение Из Предупреждения Цикл
			ТекстПредупреждения = ?(ТекстПредупреждения = "", "", Символы.ПС)
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1: %2';
																				|en = '%1: %2'"), Предупреждение.Ключ, Предупреждение.Значение);
		КонецЦикла;
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Если ПодробностиВыполнения.Обработано < ПодробностиВыполнения.Всего Тогда
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				Нстр("ru = 'Восстановлено %1 из %2 поврежденных сведений о файлах. Подробности см. в журнале регистрации.';
					|en = 'Restored file info: %1 out of %2. For details, see the Event log.'"),
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru = 'Нет поврежденных файлов для восстановления.';
											|en = 'No damaged files to restore.'"));
			Возврат;
		КонецЕсли;
