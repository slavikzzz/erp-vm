		...
		Если ФайлыВКаталоге.Количество() = 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Удаляется пустой подкаталог ""%1""';
					|en = 'Удаляется пустой подкаталог ""%1""'"), 
				КаталогФайла);
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Информация,,, СообщениеЖурналаРегистрации(ТекстСообщения));
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Файлы.Удаление файлов в томе';
				|en = 'Файлы.Удаление файлов в томе'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Если Не Том.Существует() Или Не Том.ЭтоКаталог() Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не существует сетевой каталог для тома хранения файлов ""%1"": %2
					...
					|Contact your administrator.'"), СвойстваФайла.Том, ПолныйПутьТома);
		КонецЕсли;
		ВызватьИсключение;
	КонецПопытки	
КонецФункции
	...
	Для Каждого Порция Из Порции Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Файлы.Восстановление файлов в томе';
					|en = 'Files.File recovery in volume'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,,
...
Процедура ЗаписатьДанныеФайлаВТом(ПрисоединенныйФайл, ДвоичныеДанныеИлиПуть)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Добавление файла';
										|en = 'Files.Add file'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
...
Процедура ОчиститьУдаленныеФайлыВТоме(Каталог)
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Работа с файлами.Очистка файлов';
										|en = 'File management.File cleanup'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
...
Функция ПредставлениеСтатусаПроверки(Знач СтатусПроверки) Экспорт
	...
	ИначеЕсли СтатусПроверки = "ЛишнийФайлВТоме" Тогда 
		Возврат НСтр("ru = 'Лишние файлы (есть в томе, но сведения о них отсутствуют)';
					|en = 'Extraneous files (files in the volume that are not registered in the application)'");
