	...
	Исключение
		...
		Если ЗначениеЗаполнено(СообщениеЖурналаРегистрации) Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Ошибка добавления подписи';
					|en = 'Digital signature. An error occurred while adding a signature'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура УдалитьПодпись(Объект, ПорядковыйНомер, ИдентификаторФормы = Неопределено,
	...
	Исключение
		...
		Если ЗначениеЗаполнено(СообщениеЖурналаРегистрации) Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Ошибка удаления подписи';
					|en = 'Digital signature.Signature deletion error'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				Объект.Метаданные(),
				Объект.Ссылка,
				СообщениеЖурналаРегистрации + "
...
Функция ЗаписатьСертификатВСправочник(Знач Сертификат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ПравоДоступаДобавление И Не ЗначениеЗаполнено(СертификатСсылка) Тогда
		ВызватьИсключение(НСтр("ru = 'Недостаточно прав для использования сертификата.';
								|en = 'insufficient rights to use the certificate.'"), КатегорияОшибки.НарушениеПравДоступа);
	КонецЕсли;
		...
		Если СертификатОбъект.ДанныеСертификата.Получить() <> ДвоичныеДанныеСертификата Тогда
			Если Не ПравоДоступаДобавление Тогда
				...
				ВызватьИсключение(НСтр("ru = 'Недостаточно прав для изменения данных сертификата.';
										|en = 'Insufficient rights to modify certificate data.'"), КатегорияОшибки.НарушениеПравДоступа);
			КонецЕсли;
			СертификатОбъект.ДанныеСертификата = Новый ХранилищеЗначения(ДвоичныеДанныеСертификата);
		КонецЕсли;
...
Функция УсовершенствоватьПодпись(Подпись, ТипПодписи, ДобавитьАрхивнуюМеткуВремени = Ложь,
	...
	Исключение
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось прочитать данные подписи: %1';
				|en = 'Cannot read the signature data: %1'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Результат;
	КонецПопытки;
	...
	Если Не ПараметрыУсовершенствования.ИгнорироватьСрокДействияСертификата
		...
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Истек срок действия подписи: %1';
				|en = 'The signature expired: %1'"), ПараметрыПодписиКриптографии.СрокДействияПоследнейМеткиВремени);
		Возврат Результат;
	КонецЕсли;
	...
	Если РезультатПроверкиСертификата Тогда
		Если ЗначениеЗаполнено(ТипПодписи) И ЭлектроннаяПодписьСлужебныйКлиентСервер.ПодлежитУсовершенствованию(
			...
			Исключение
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не удалось усовершенствовать подпись: %1';
							|en = 'Couldn''t enhance the signature: %1'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(
					ИнформацияОбОшибке()));
				Возврат Результат;
			КонецПопытки;
		ИначеЕсли ДобавитьАрхивнуюМеткуВремени И (ПараметрыПодписиКриптографии.ТипПодписи
			...
			Исключение
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не удалось добавить архивную метку времени в подпись: %1';
							|en = 'Cannot add an archive timestamp to the signature: %1'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(
					ИнформацияОбОшибке()));
				Возврат Результат;
			КонецПопытки;
	...
	Исключение
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось прочитать данные усовершенствованной подписи: %1';
					|en = 'Couldn''t read the enhanced signature data: %1'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(
			ИнформацияОбОшибке()));
		Возврат Результат;
	КонецПопытки;
	...
	Иначе
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не прошел проверку сертификат полученной метки времени: %1
					 |%2';
					 |en = 'The certificate of the received timestamp failed the check: %1
					 |%2'"), ОписаниеОшибки, СведенияОСертификате);
...
Функция УсовершенствоватьПодписьОбъекта(ПодписанныйОбъект, ПорядковыйНомер, ТипПодписи, ДобавитьАрхивнуюМеткуВремени = Ложь,
	...
	Если УстановленныеПодписи.Количество() = 0 Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось найти подпись с порядковым номером %1 у %2';
				|en = 'Cannot find a signature with sequence number %1 for %2'"), ПорядковыйНомер, ПодписанныйОбъект);
		Возврат Результат;
	КонецЕсли;
		...
		Исключение
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось получить данные из файла подписи по причине:
						|%1';
						|en = 'Cannot receive data from the signature file due to:
...
Функция Зашифровать(Данные, Сертификат, АлгоритмШифрования = "") Экспорт
			...
			Если МенеджерКриптографии = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось выполнить шифрование сертификатом ""%1"":
						...
						|en = 'Cannot encrypt with the ""%1"" certificate:
						|Cannot create a crypto manager'"), Сертификат);
			КонецЕсли;
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось выполнить шифрование сертификатом ""%1""';
						|en = 'Cannot encrypt with the ""%1"" certificate'"), Сертификат);
КонецФункции
...
Процедура ДобавитьСтрокиПодписи(ОбъектДанных, СвойстваПодписей, СообщениеЖурналаРегистрации)
	...
	Для Каждого СвойстваПодписи Из МассивСвойствПодписи Цикл
		...
		ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Добавление подписи';
					|en = 'Digital signature.Add signature'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Попытка
		...
		Если ВыборкаДетальныеЗаписи.Количество() <> МассивПорядковыхНомеров.Количество() Тогда
			ВызватьИсключение НСтр("ru = 'Строка с подписью не существует.';
									|en = 'A signature line does not exist.'");
		КонецЕсли;
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Если Не ЕстьПрава Тогда
				ВызватьИсключение(НСтр("ru = 'Недостаточно прав на удаление подписи.';
										|en = 'Insufficient rights to delete the signature.'"), КатегорияОшибки.НарушениеПравДоступа);
			КонецЕсли;
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Электронная подпись.Удаление подписи';
											|en = 'Digital signature.Delete signature'", ОбщегоНазначения.КодОсновногоЯзыка()),
