...
Функция ТекстЗапросаДляПартийТоваров(ПараметрыРасчета = Неопределено) Экспорт
	...
	ТекстЗапроса = ""
		...
		+ ОбщегоНазначения.РазделительПакетаЗапросов() + ТекстКорректировкиПрошлыхОтчетовДавальцу() // вт ПрошлыеОтчетыПродукция
		...
		+ ОбщегоНазначения.РазделительПакетаЗапросов() + ТекстВозвратныеОтходы() // вт ВозвратныеОтходы
		...
		//++ НЕ УТКА
		+ ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении() + ТекстНачальныеОстаткиОтчетыДавальцу()
		//-- НЕ УТКА
...
Функция ТекстКорректировкиПрошлыхОтчетовДавальцу() // вт ПрошлыеОтчетыПродукция
	Возврат 
		"ВЫБРАТЬ
		|	Корректировки.Ссылка КАК Корректировка,
		|	ОтчетыДавальцу.Ссылка КАК ОтчетДавальцу
		|ПОМЕСТИТЬ ПрошлыеОтчеты
		|ИЗ
		|	Документ.КорректировкаРеализации КАК Корректировки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцу2_5 КАК ОтчетыДавальцу
		|		ПО Корректировки.ДокументОснование = ОтчетыДавальцу.Ссылка
		|ГДЕ
		|	Корректировки.Дата МЕЖДУ &НачалоПериода И &КонецПериода
		|	И Корректировки.Проведен
		|	И ОтчетыДавальцу.Дата < &НачалоПериода
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ОтчетДавальцу
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ОтчетыДавальцу.Корректировка КАК Корректировка,
		|	Т.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|	Т.ВидЗапасов КАК ВидЗапасов,
		...
		|ПОМЕСТИТЬ ПрошлыеОтчетыПродукция
		|ИЗ
		|	РегистрНакопления.СебестоимостьТоваров КАК Т
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПрошлыеОтчеты КАК ОтчетыДавальцу
		|		ПО Т.Регистратор = ОтчетыДавальцу.ОтчетДавальцу
		|ГДЕ
		|	Т.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца)
		|	И Т.Активность
		|
		|СГРУППИРОВАТЬ ПО
		|	ОтчетыДавальцу.Корректировка,
		|	Т.АналитикаУчетаНоменклатуры,
		|	Т.ВидЗапасов
		|ИМЕЮЩИЕ 
...
Функция ТекстНачальныеОстаткиОтчетыДавальцу()
	// Если в прошлом периода товар был передан давальцу, то остатков товара не будет.
	// В этом случае в качстве остатков используем движения самого отчета давальцу.
	Возврат 
		"ВЫБРАТЬ
		|	""ТекстНачальныеОстаткиОтчетыДавальцу""		 	  КАК ЗапросИсточник,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Остаток) КАК ТипЗаписи,
		|	ЛОЖЬ											 КАК ЭтоТочноРасчетноеДвижение,
		|	ЛОЖЬ 							 				 КАК ЭтоСторно,
		|	90 												 КАК Приоритет,
		...
		|	И Т.Организация В (&МассивОрганизаций)
		|	И Т.Регистратор В (ВЫБРАТЬ ОтчетыДавальцу.ОтчетДавальцу ИЗ ПрошлыеОтчеты КАК ОтчетыДавальцу)
		|	И Т.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца)
		|	И Т.Активность
		|	И &ВосстанавливатьХронологическуюПоследовательностьСписанияПартий
		|";
КонецФункции
...
Функция ТекстТекущиеОборотыПартийТоваров() // использует ВозвратныеОтходы
	Возврат 
		...
		// Для потребления производством без заказа период поступления берем из даты документа.
		|		КОГДА Т.ТипЗаписи В (
		|			ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Потребление),
		|			ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.СписаниеНаДругиеПартии))
		|		 И Т.Регистратор ССЫЛКА Документ.ПроизводствоБезЗаказа
		|		 И Т.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
		|			ТОГДА ПроизводствоБезЗаказа.Дата
		//++ Устарело_Переработка24
		|		КОГДА ВозвратыСырьяДавальцу.ДокументПоступления.Дата ЕСТЬ НЕ NULL
		|			ТОГДА ВозвратыСырьяДавальцу.ДокументПоступления.Дата
		//-- Устарело_Переработка24
		...
		//++ Устарело_Переработка24
		|		КОГДА ВозвратыСырьяДавальцу.ДокументПоступления ЕСТЬ НЕ NULL
		|				И Т.Организация В (&ОрганизацииСФИФОСкользящая)
		|			ТОГДА ВозвратыСырьяДавальцу.ДокументПоступления
		//-- Устарело_Переработка24
		...
		|	Т.СтоимостьБезНДС,
		//++ НЕ УТКА
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ПрошлыеОтчеты.СтоимостьЗабалансовая * Т.Количество / ПрошлыеОтчеты.Количество КАК ЧИСЛО (31,2)),
		//-- НЕ УТКА
		|		Т.СтоимостьЗабалансовая
		//++ НЕ УТКА
		|	)
		...
		|	Т.СтоимостьРегл,
		//++ НЕ УТКА
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ПрошлыеОтчеты.СтоимостьЗабалансоваяРегл * Т.Количество / ПрошлыеОтчеты.Количество КАК ЧИСЛО (31,2)),
		//-- НЕ УТКА
		|		Т.СтоимостьЗабалансоваяРегл
		//++ НЕ УТКА
		|	)
		...
		//++ Устарело_Переработка24
		|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратСырьяДавальцу.ВидыЗапасов КАК ВозвратыСырьяДавальцу
		|		ПО ВозвратыСырьяДавальцу.Ссылка = Т.Регистратор
		|		И Т.ИдентификаторСтроки = ВозвратыСырьяДавальцу.ИдентификаторСтроки
		//-- Устарело_Переработка24
		...
		|	ЛЕВОЕ СОЕДИНЕНИЕ ПрошлыеОтчетыПродукция КАК ПрошлыеОтчеты
		|		ПО Т.Регистратор = ПрошлыеОтчеты.Корректировка
		|		И Т.АналитикаУчетаНоменклатуры = ПрошлыеОтчеты.АналитикаУчетаНоменклатуры
		|		И Т.ВидЗапасов = ПрошлыеОтчеты.ВидЗапасов
		|		И Т.Количество < 0
		//-- НЕ УТКА
		|	ЛЕВОЕ СОЕДИНЕНИЕ ИсправленныеДокументы КАК ИсправленныеДокументы
		|		ПО ИсправленныеДокументы.Регистратор = Т.Регистратор
...
Функция ТекстСторноДвиженияРасчетаПартийСебестоимостьТоваров() // использует ИсправленныеДокументы
	Возврат 
		...
		// Сторно количественных приходных движений в незавершенное производство, по материальным затратам,
		// при сторнировании Отчета давальцу между организациями, оформленному на партию производства с докуменом = ЗаказНаПроизводство2_2
		// Стоимостные движения формируются в см. РасчетСебестоимостиКорректировкаСтоимости.СкорректироватьСтоимостьСторноДвиженийИсправленныхДокументов.
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|		ПО ДанныеРегистра.Партия = СпрПартииПроизводства.Ссылка
		...
		|	И ТИПЗНАЧЕНИЯ(ДанныеРегистра.Регистратор) = ТИП(Документ.ОтчетДавальцуМеждуОрганизациями)
