...
Процедура ПодключитьОповещениеОНовыхДокументахВСервисе(Форма, ВидыДокументов, ГруппаФормы = Неопределено) Экспорт
	...
	Для каждого ВидДокумента Из ВидыДокументов Цикл
		Если ВидыДокументовСервиса().НайтиПоЗначению(ВидДокумента) = Неопределено Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Вид документа ""%1"" не найден в сервисе ""1С:Бизнес-сеть"".';
											|en = 'Document kind ""%1"" is not found in the service ""1C:Business-network"".'"), ВидДокумента);
...
Процедура АктуализироватьКэшДанныхКонтрагентов() Экспорт
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), 
		...
		НСтр("ru = 'Начало обновления идентификаторов контрагентов Бизнес-сети';
			|en = 'Starting Business Network counterparty IDs update'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	Результат = РегистрыСведений.КонтрагентыБизнесСеть.РезультатАктуализацииКешаКонтрагентов();
	
	ШаблонСообщения = НСтр("ru = 'Окончание обновления идентификаторов контрагентов Бизнес-сети. Обновлено записей - %1. %2';
							|en = 'Business Network counterparty IDs update finished. Records updated- %1. %2'", ОбщегоНазначения.КодОсновногоЯзыка());
	ТекстОшибки = ?(Не Результат.Успешно, НСтр("ru = 'Выполнено с ошибками.';
...
Функция ИдентификаторыКонтрагентов(Знач Контрагенты, ИспользоватьТолькоКэш = Ложь) Экспорт
			...
			Результат.Ошибки.Вставить(ДанныеКонтрагента.Контрагент,
				НовыйОписаниеОшибкиПоискаКонтрагентов(ТипОшибкиСервисНеДоступен(),
					НСтр("ru = 'Не удалось получить данные контрагента из сервиса.';
						|en = 'Cannot receive counterparty data from the service.'")));
		КонецЦикла;
	...
	Если ЗначениеЗаполнено(НекорректныеЗаписи) Тогда
		...
		ТекстОшибки = НСтр("ru = 'Контрагент не подключен к Бизнес Сети.';
							|en = 'Counterparty is not connected to Business Network.'");
		Для Каждого НекорректнаяЗапись Из НекорректныеЗаписи Цикл
			Результат.Ошибки.Вставить(
...
Функция КонтрагентыПоИдентификаторам(Знач ИдентификаторыКонтрагентов) Экспорт
	...
	Если Отказ Тогда
		...
		ТекстОшибки = НСтр("ru = 'Не удалось получить ИНН/КПП из сервиса.';
							|en = 'Cannot receive TIN/KPP from service.'");
		Для Каждого ДанныеКонтрагента Из ДанныеКонтрагентов Цикл
			Результат.Ошибки.Вставить(
	...
	Если ЗначениеЗаполнено(ОшибочныеЗаписи) Тогда
		...
		ТекстОшибки = НСтр("ru = 'Идентификатор отсутствует в сервисе Бизнес Сеть.';
							|en = 'ID is missing in the Business Network service.'");
	...
	Если ЗначениеЗаполнено(НеНайденныеЗаписи) Тогда
		...
		ШаблонТекстаОшибки = 
			НСтр("ru = 'По %1, полученным от Бизнес Сети, не найдена запись в базе данных.';
				|en = 'Database record not found for %1 received from Business Network.'");
...
Функция РеквизитыУчастника(ПараметрыМетода, Отказ = Неопределено) Экспорт
	...
	Если Отказ Тогда
		...
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(НСтр("ru = 'Ошибка чтения данных в 1С:Бизнес-сеть:';
															|en = 'An error occurred when reading data in 1C:Business Network:'") + " " + ТекстОшибки, 
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
...
Функция ПараметрыКомандыОтправитьДокумент(ПараметрыМетода, Отказ = Неопределено) Экспорт
	...
	Если Отказ Тогда
		...
		ТекстЖурналаРегистрации = НСтр("ru = 'Ошибка отправки документов через 1С:Бизнес-сеть:';
										|en = 'Error sending documents via 1C:Business Network:'") + " " + ТекстОшибки;
...
Функция ИдентификаторыУчастника(Знач ИНН, Знач КПП, Знач Наименование = Неопределено, Отказ = Неопределено,
	...
	Если Не РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(ИНН, ЭтоЮридическоеЛицо, ТекстСообщения) Тогда
		ШаблонОшибки = НСтр("ru = 'Значение ИНН ""%1"" не соответствует требованиям к ИНН: %2';
							|en = 'TIN value ""%1"" does not meet the TIN requirements: %2'", КодЯзыка);
	...
	ИначеЕсли Не РегламентированныеДанныеКлиентСервер.КППСоответствуетТребованиям(КПП, ТекстСообщения) Тогда
		ШаблонОшибки = НСтр("ru = 'Значение КПП ""%1"" не соответствует требованиям к КПП: %2';
							|en = 'KPP value ""%1"" does not meet the KPP requirements: %2'", КодЯзыка);
		...
		Если ЕстьПравоОбменаЭД Тогда
			...
			Если ИспользоватьОбменБизнесСеть И СоставКоманд.БезПодписи.Найти(ПолноеИмя) <> Неопределено Тогда
				...
				КомандаЭДО.Представление = НСтр("ru = 'Загрузить документ без электронной подписи';
												|en = 'Import the document without digital signature'");
				...
				КомандаЭДО.Представление = НСтр("ru = 'Найти по QR-коду электронный документ';
												|en = 'Find an electronic document by QR code'");
				КомандаЭДО.Порядок       = 62;
	...
	Если СоставКоманд.Исходящие.Найти(ПолноеИмя) <> Неопределено 
		...
		Если ПравоДоступа("Вывод", Метаданные) И ИспользоватьОбменБизнесСеть
			...
			КомандаЭДО.Представление = НСтр("ru = 'Отправить документ без электронной подписи';
											|en = 'Send the document without a digital signature'");
	...
	Для Каждого ЗначениеМассива Из Результат Цикл
		...
		Если ТипЗнч(ЗначениеМассива) <> Тип("Структура") ИЛИ Не ЗначениеМассива.Свойство("documentTitle") Тогда
			ВидОперации = НСтр("ru = 'Чтение списка документов';
								|en = 'Read document list'");
			ПодробныйТекстОшибки = НСтр("ru = 'Внутренняя ошибка чтения данных 1С:Бизнес-сеть. Ошибка формата.';
										|en = 'An internal error occurred when reading 1C:Business Network data. Format error.'");
			ТекстОшибки = НСтр("ru = 'Внутренняя ошибка чтения данных 1С:Бизнес-сеть.';
								|en = 'An internal error occurred when reading 1C:Business Network data.'");
	...
	Для Каждого СтрокаТаблицы Из ДанныеДляВыгрузки Цикл
		...
		Если ВидыДокументовСервиса.НайтиПоЗначению(СтрокаТаблицы.ТипДокумента) = Неопределено Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Отправка данного вида документа не поддерживается.';
														|en = 'You cannot send documents of this kind.'"));
			Возврат Неопределено;
		КонецЕсли;
...
Функция ОтправитьДанныеДокументовЧерезБизнесСетьВФоне(Знач УникальныйИдентификатор, Знач ИдентификаторЗадания, Знач ДополнительныеПараметры) Экспорт
	...
	НаименованиеЗадания = НСтр("ru = '1С:Бизнес-сеть. Отправка документов.';
								|en = '1C:Business Network. Document sending.'");
...
Функция ОбновитьКлючДоступа(ИдентификаторОрганизации, ТокенОбновления, Отказ = Ложь)
	...
	Если РегламентныеЗаданияСервер.РаботаСВнешнимиРесурсамиЗаблокирована() Тогда
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
			НСтр("ru = 'Обновление ключа доступа. Работа с внешними ресурсами заблокирована.';
				|en = 'Updating an access key. Operations with external resources are blocked.'"), 
			Подсистема, 
			УровеньЖурналаРегистрации.Информация);
	Иначе
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
			НСтр("ru = 'Обновление ключа доступа. Работа с внешними ресурсами не заблокирована.';
				|en = 'Updating an access key. Operations with external resources are not blocked.'"), 
			Подсистема, 
			УровеньЖурналаРегистрации.Информация);
	КонецЕсли;
	...
	ШаблонЗаписи = НСтр("ru = 'Обновление ключа доступа. Запрос: %1 %2';
						|en = 'Updating an access key. Request: %1 %2'");
	ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
	...
	Если ТипЗнч(ДанныеСервиса) <> Тип("Структура") Тогда
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
			НСтр("ru = 'Обновление ключа доступа. Не верный формат данных в ответе на запрос';
				|en = 'Access key update. Incorrect data format in the request response'", 
	...
	ШаблонЗаписи = НСтр("ru = 'Обновление ключа доступа. Ответ: 
		...
		|en = 'Access key update. Response: 
		| Access token validity period is %1; 
		| Update token validity period is %2'");
	...
	ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
		НСтр("ru = 'Обновление ключа доступа. Ключ записан.';
			|en = 'Access key update. The key is saved.'"), 
		Подсистема, 
		УровеньЖурналаРегистрации.Информация);
...
Функция ПараметрыКомандыДанныеОрганизацииПоНатуральномуИдентификатору(ПараметрыМетода)
	...
	ПараметрыКоманды.Наименование             = НСтр("ru = 'Получение данных организации по натуральному идентификатору';
													|en = 'Receipt of company data by natural identifier'");
...
Функция ПараметрыКомандыДанныеОрганизацииДоступныхПользователю()
	...
	ПараметрыКоманды.Наименование = НСтр("ru = 'Получение данных организаций, доступных пользователю';
										|en = 'Receive data of companies available to the user'");
...
Функция ОбновитьИнформациюОНовыхДокументахВСервисеАсинхронно(Знач ПараметрыОбновления) Экспорт
	...
	НаименованиеЗадания = НСтр("ru = 'Обновление информации о новых документах в сервисе ""1С:Бизнес-сеть"".';
								|en = 'Update the information about new documents in the service ""1C:Business Network"".'");
...
Функция ПараметрыКомандыУдалитьДокументы(ПараметрыМетода, Отказ = Неопределено) Экспорт
	...
	Если Отказ Тогда
		...
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(НСтр("ru = 'Ошибка удаления документов в 1С:Бизнес-сеть:';
															|en = 'Error deleting documents in 1C:Business Network:'") + " " + ТекстОшибки, 
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПараметрыКомандыОтправкаПриглашенийКонтрагентам(ПараметрыМетода, Отказ = Неопределено) Экспорт
	...
	Для каждого РеквизитыКонтрагента Из ПараметрыМетода.Данные Цикл
		...
		Если Отказ Тогда
			...
			ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(НСтр("ru = 'Ошибка отправки приглашения в 1С:Бизнес-сеть:';
																|en = 'Error sending invitation to 1C:Business Network:'") + " " + ТекстОшибки, 
				ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
...
Функция ПараметрыКомандыСписокВходящихДокументов(ПараметрыМетода, Отказ = Неопределено) Экспорт
	...
	Если ПараметрыМетода.Свойство("Контрагент") Тогда
		...
		Если Отказ Тогда
			...
			ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(НСтр("ru = 'Ошибка отбора по контрагенту при чтении документов 1С:Бизнес-сеть:';
																|en = 'An error occurred while filtering by counterparty when reading 1C:Business Network documents:'") + " " + ТекстОшибки, 
				ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
...
Функция ТекстОшибкиСервисаИзДанныхОтвета(КодСостояния, ОписаниеОшибки, ПараметрыКоманды, ПараметрыСоединения, Данные)
	...
	Если ТипЗнч(Данные) = Тип("Структура") Тогда
		...
		ТекстОшибкиСервиса = НСтр("ru = 'Описание ошибки';
									|en = 'Error description'") + ": " + ТекстОшибкиСервиса;
		...
		Если Данные.Свойство("validationError") И ТипЗнч(Данные.validationError) = Тип("Массив") Тогда
			...
			Если ЗначениеЗаполнено(ДополнительноеОписание) Тогда
				ДанныеПоОшибке.Добавить(НСтр("ru = 'Дополнительное описание';
											|en = 'Additional details'") + ": " + ДополнительноеОписание);
			КонецЕсли;
			
		КонецЕсли;
		
		Если Данные.Свойство("requestTraceId") Тогда
			ДанныеПоОшибке.Добавить(НСтр("ru = 'Идентификатор запроса';
										|en = 'Query ID'") + ": " + Данные.requestTraceId);
		ИначеЕсли Данные.Свойство("traceId") Тогда
			ДанныеПоОшибке.Добавить(НСтр("ru = 'Идентификатор запроса';
										|en = 'Query ID'") + ": " + Данные.traceId);
		КонецЕсли;
		
		Если Данные.Свойство("type") Тогда
			ДанныеПоОшибке.Добавить(НСтр("ru = 'Тип запроса';
										|en = 'Query type'") + ": " + Данные.type);
		КонецЕсли;
...
Функция АдресаВременныхХранилищПолученныхДокументов(Знач Результат, Знач УникальныйИдентификатор, Знач ЗагрузкаПредставления, Отказ) Экспорт
	...
	Если Отказ Тогда
		ВидОперации = НСтр("ru = 'Чтение электронного документа';
							|en = 'Reading electronic document'");
		ПодробныйТекстОшибки = НСтр("ru = 'Внутренняя ошибка чтения данных 1С:Бизнес-сеть. Ошибка формата.';
									|en = 'An internal error occurred when reading 1C:Business Network data. Format error.'");
		ТекстОшибки = НСтр("ru = 'Внутренняя ошибка чтения данных 1С:Бизнес-сеть.';
							|en = 'An internal error occurred when reading 1C:Business Network data.'");
...
Функция КлючДоступаБизнесСети(
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		ВызватьИсключение НСтр("ru = 'Не найдена организация по идентификатору.';
								|en = 'The company is not found by identifier.'");
	КонецЕсли;
	...
	ИначеЕсли НастройкиРегионов.РежимОтбора = 1 Тогда
		...
		Если ПустаяСтрока(ПредставлениеАдресовРегионов) Тогда
			РегионЗаголовок = НСтр("ru = 'Регионы профиля (не выбраны)';
									|en = 'Profile regions (not selected)'");
		Иначе
			РегионЗаголовок = СтрШаблон(НСтр("ru = 'Регионы профиля (%1)';
											|en = 'Profile regions (%1)'"), ПредставлениеАдресовРегионов);
		КонецЕсли;
...
Процедура СообщитьОбОшибкеЗаписатьВЖурнал(ТипСообщения, Отказ = Ложь, ИнформацияОбОшибке = "")
	...
	Если ТипСообщения = "ТокенОбновленияИнвалидирован" Тогда
		...
		ПричинаОшибки = НСтр("ru = 'Токен обновления Бизнес-сети инвалидирован.';
							|en = 'Business Network update token is disabled.'");
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
			ПричинаОшибки, 
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
	...
	ИначеЕсли ТипСообщения = "ЗакончилсяСрокТокенаОбновления" Тогда
		...
		ПричинаОшибки = НСтр("ru = 'Закончен период действия токена обновления Бизнес-сети.';
							|en = 'Business Network update token has expired.'");
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
			ПричинаОшибки, 
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
	...
	ИначеЕсли ТипСообщения = "НеУдалосьПрочитатьОбновитьКлючДоступа" Тогда
		...
		ПричинаОшибки = НСтр("ru = 'Не удалось прочитать или обновить ключ доступа Бизнес-сети.';
							|en = 'Cannot read or update 1C:Business Network access key.'");
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
			ПричинаОшибки, 
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть);
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Не удалось выполнить запрос к сервису.';
				|en = 'Cannot complete the service query.'") 
				+ Символы.ПС
				+ ПричинаОшибки
				+ Символы.ПС 
...
Процедура СоздатьГиперссылкуПодбораДокументовИзСервиса(Форма, ГруппаФормы)
	...
	НоваяКомандаФормы.Подсказка = НСтр("ru = 'Подобрать новые документы из сервиса ""1С:Бизнес-сеть""';
										|en = 'Select new documents from the service ""1C:Business Network""'");
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'БизнесСеть';
				|en = 'БизнесСеть'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ЗаписатьТабличныйДокумент(ТабличныйДокумент, ТипФайла, ИмяФайла)
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'Не удалось сформировать табличный документ.
									...
									|en = 'Cannot generate a spreadsheet document.
...
Функция ПараметрыКомандыСозданияQRКода(ПараметрыМетода)
	...
	ПараметрыКоманды.Права = "bn_user";
	ПараметрыКоманды.Ошибки.Вставить(404,
		СтрШаблон(НСтр("ru = 'Ошибка создания QR-кода по документу %1.';
						|en = 'Error generating QR code for document %1.'"), ПараметрыМетода.СсылкаНаДокумент));
	ПараметрыКоманды.ЗаполнитьАутентификациюПоШаблону = Ложь;
...
Функция ПараметрыКомандыПолученияQRКода(ПараметрыМетода)
	...
	ПараметрыКоманды.Наименование = НСтр("ru = 'Получение QR-кода';
										|en = 'Retrieving QR code'");
	ПараметрыКоманды.Сервис = "EDI";
	ПараметрыКоманды.Метод = "GET";
	ПараметрыКоманды.Права = "bn_user";
	ПараметрыКоманды.РазрешенныеСостояния.Добавить(404);
	ПараметрыКоманды.Ошибки.Вставить(400,
		СтрШаблон(НСтр("ru = 'Ошибка получения QR-кода по документу %1.';
						|en = 'Error retrieving QR code for document %1.'"), ПараметрыМетода.СсылкаНаДокумент));
	ПараметрыКоманды.ЗаполнитьАутентификациюПоШаблону = Ложь;
...
Функция ПараметрыКомандыПолученияПолнойПубличнойСсылки(Знач АдресРесурса)
	...
	ПараметрыКоманды.Наименование = НСтр("ru = 'Получить полную публичную ссылку на документ по ссылке с QR-кодом.';
										|en = 'Get the full public link to the document using the link with the QR code.'");
	...
	ПараметрыКоманды.Ошибки.Вставить(404, НСтр("ru = 'Не найден документ по ссылке QR-кода.';
												|en = 'No document found by QR code link.'"));
