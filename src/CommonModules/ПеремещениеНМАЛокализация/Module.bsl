...
Процедура ТекстЗапросаПеремещениеРезерваДооценкиИсходящиеДвижения(СписокЗапросов)
	...
	ТекстЗапроса = 
	"////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Передача резерва дооценки исходящие проводки (Дт СчетДооценки :: Кт 79.01)
	|ВЫБРАТЬ
	|
	|	Строки.Регистратор КАК Ссылка,
	|	Строки.Период КАК Период,
	...
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтРезервДооценкиНМА КАК Строки
	|	ПО Строки.Регистратор = ДокументыКОтражению.Ссылка
	|	
	|ГДЕ
	|	Строки.Организация <> Строки.КорОрганизация";
...
Процедура ТекстЗапросаПеремещениеРезерваДооценкиВходящиеДвижения(СписокЗапросов)
	...
	ТекстЗапроса = 
	"////////////////////////////////////////////////////////////////////////////////////////////////////
	|// Передача резерва дооценки НМА входящие проводки (Дт 79.01 :: Кт СчетДооценки)
	|ВЫБРАТЬ
	|
	|	Строки.Регистратор КАК Ссылка,
	|	Строки.Период КАК Период,
...
Функция ТекстЗапросаВтРезервДооценки()
	...
	|	ГДЕ
	|		СтоимостьНМА.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И СтоимостьНМА.РезервПереоценкиСтоимостиРегл <> 0
	|		И СтоимостьНМА.Регистратор В
	|				(ВЫБРАТЬ
	|					ДокументыКОтражению.Ссылка
	|				ИЗ
