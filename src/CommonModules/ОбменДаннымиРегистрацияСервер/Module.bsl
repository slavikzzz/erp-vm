...
Функция ОбъектМодифицированДляПланаОбмена(Источник, ОбъектМетаданных, ИмяПланаОбмена, РежимЗаписи, ЗарегистрироватьОбъектКВыгрузке) Экспорт
	...
	Исключение
		...
		СтрокаШаблона = НСтр("ru = 'Ошибка определения модифицированности объекта: %1';
							|en = 'Cannot determining whether the object was modified: %1'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ИмяСобытияПравилаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Обмен данными.Правила регистрации объектов';
				|en = 'Data exchange.Object registration rules'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРеквизитовРегистрации Цикл
		...
		Исключение
			...
			СтрокаСообщения = НСтр("ru = 'Тип объекта не определен: %1';
									|en = 'Undefined object type: %1'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Иначе
			...
			Если МетаТаблицы = Неопределено Тогда
				...
				СтрокаСообщения = НСтр("ru = 'Табличная часть (стандартная табличная часть, движения) ""%1"" объекта ""%2"" не существует.';
										|en = 'The ""%1"" tabular section (or a standard tabular section, or a list of register records) of the ""%2"" object does not exist.'");
			...
			Для Каждого Реквизит Из СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Если Не РеквизитНайден Тогда
					...
					СтрокаСообщения = НСтр("ru = 'Реквизит ""%3"" не существует в табличной части (стандартной табличной части, движениях) ""%1"" объекта ""%2"".';
											|en = 'The ""%3"" attribute is not found in the tabular section (or a standard tabular section, or a list of register records) ""%1"" of the ""%2"" object.'");
	...
	Для Каждого ПКС Из МассивСтрокПКС Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен данными.Загрузка правил конвертации';
											|en = 'Data exchange.Import conversion rules'", ОбщегоНазначения.КодОсновногоЯзыка()),
