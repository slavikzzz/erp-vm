		...
		Если ХарактеристикиНеЗаполнены Тогда
			...
			Если ДокументОбъект.Товары.Количество() = 1 Тогда
				ТекстОшибки = НСтр("ru = 'Не заполнена характеристика.';
									|en = 'Не заполнена характеристика.'");
...
Процедура УстановитьПараметрыВыбораНоменклатуры(Форма, ИмяПоляВвода = "ТоварыНоменклатура") Экспорт
	...
	ОсобенностиУчетаНоменклатурыВетИС.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.КормаДляЖивотныхВлажныеПодконтрольныеВЕТИС);
...
Процедура ПриГенерацииСерий(Объект, Товары, ВыделенныеСтроки, ПараметрыУказанияСерий, ТоварыУточнение, РезультатЗаполненияСерий) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Неправильный тип параметра Товары метода СгенерироватьСерии';
								|en = 'Неправильный тип параметра Товары метода СгенерироватьСерии'");
	КонецЕсли;
...
Процедура ПриИзмененииДокументаОснованияПроизводственнойОперации(Объект) Экспорт
	...
	//++ НЕ УТКА
	Если ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		Данные = ИнтеграцияВЕТИСУТ.ДанныеПартииПроизводстваПоДокументуОснования(Объект.ДокументОснование);
		Объект.ИдентификаторПроизводственнойТранзакции = Данные.ИдентификаторПроизводственнойТранзакции;
		Объект.ЗавершениеПроизводственнойТранзакции = Данные.ЗавершениеПроизводственнойТранзакции;
	КонецЕсли;
	//-- НЕ УТКА
