	...
	Если ВзаиморасчетыССотрудниками.ЕстьОплатаПоВедомости(ДокументОбъект.Ссылка) Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'По ведомости произведены оплаты, изменения запрещены';
				|en = 'There are payments based on the statement, changes are unavailable'"), 
			ДокументОбъект);
...
Функция ЗарплатаКВыплатеФизическихЛиц(
	...
	Иначе
		Запрос.УстановитьПараметр("УсловиеПоУволенным", Истина);
	КонецЕсли;
...
Функция ВзысканияПоИсполнительнымДокументамМежрасчет(ЗарплатаКВыплате)
	...
	Взыскания = ИсполнительныеЛисты.УдержанныеСуммыФизическихЛицПоДокументам(УчитываемыеВыплаты.Прямые, Ложь, УчитываемыеВыплаты.Сторно.Количество()>0);
	Взыскания.Свернуть("ФизическоеЛицо", "Сумма");
...
Функция ВзысканияПоИсполнительнымДокументамЗарплата(ЗарплатаКВыплате)
	...
	Взыскания = ИсполнительныеЛисты.УдержанныеСуммыФизическихЛицПоДокументам(УчитываемыеВыплаты.Прямые, Истина, УчитываемыеВыплаты.Сторно.Количество()>0);
	Взыскания.Свернуть("ФизическоеЛицо", "Сумма");
...
Функция ВзысканияПоИсполнительнымДокументамУчитываемыеВыплаты(ЗарплатаКВыплате, НачисленнаяЗарплата)
	...
	Для Каждого Выплата Из Выплаты Цикл
		Если Не ЗначениеЗаполнено(Выплата.ДокументОснование) Тогда
			// выплаты без оснований не интересны (при нулевой сумме к выплате могло быть взыскание)
