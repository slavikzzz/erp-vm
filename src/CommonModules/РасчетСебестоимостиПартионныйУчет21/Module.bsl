...
Функция ПроверитьНеобходимостьЗаполненияРеквизитаРасчетПартий(ПараметрыРасчета)
	...
	// Проверим наличие движений партий незавершенного производства с незаполненным расчетом партий
	ТекстЗапроса = ТекстЗапроса
	+ ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении() + "
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	""%1"" КАК РегистрСоСтарымиДвижениями
...
#Область ОчисткаОстатков21
	...
	// Очистим остатки регистра МатериалыИРаботыВПроизводстве в части переработки
	ОписаниеРегистра = Новый Соответствие;
	ОписаниеРегистра.Вставить(Метаданные.РегистрыНакопления.МатериалыИРаботыВПроизводстве, Истина);
	Для Каждого КлючИЗначение Из ОписаниеРегистра Цикл
		Прервать;
	КонецЦикла;
	
	МетаРегистр = КлючИЗначение.Ключ; // ОбъектМетаданныхРегистрНакопления -
	 
	ТекстОтборОстатков = "ТИПЗНАЧЕНИЯ(Подразделение) = ТИП(Справочник.Партнеры)";
	
	Запрос.Текст = ТекстЗапросаОчисткаОстатковРегистраПУВерсии21(
		КлючИЗначение,
		ТекстОтборОстатков);
...
#Область ОчисткаДвижений21
			...
			РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Очищено устаревших движений по регистру %1: %2';
						|en = 'Obsolete records by %1 register cleared: %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
					ИмяРегистра,
	...
	//++ НЕ УТ
	#Область ОчисткаДвиженийРаспределениеПрочихЗатрат

	// Очистим движения документа РаспределениеПрочихЗатрат по регистру ПрочиеРасходы, сформированные партионным учетом
	// версии 2.1. В партионном учете версии 2.2 все движения этого документа по этому регистру формируются механизмом
		...
		РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Сформировано движений сторнирования остатков по регистру %1: %2';
					|en = 'Generated balance reversal records for the %1 register: %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ИмяРегистра,
				РасчетСебестоимостиПротоколРасчета.ПредставлениеЗначения(Выборка.Количество())));
		...
		РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Очищено движений сторнирования остатков по регистру %1: %2';
					|en = 'Cleared balance reversal records for the register %1: %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ИмяРегистра,
				РасчетСебестоимостиПротоколРасчета.ПредставлениеЗначения(Выборка.Количество())));
...
Функция ПартионныйУчетВерсии22НельзяПонизитьДоВерсии21(ЕстьДвиженияПУ22) Экспорт
	...
	|	Константы КАК Константы
	|ГДЕ
	|	Константы.ИспользоватьВнеоборотныеАктивы2_4
	//-- НЕ УТ
	|
	|УПОРЯДОЧИТЬ ПО
	|	КодПричины
		...
		Если Выборка.КодПричины = 1 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
				|    - " + НСтр("ru = 'есть движения по себестоимости товаров, сформированные партионным учетом версии 2.2';
								|en = 'There are goods cost movements generated by batch accounting 2.2'", ОбщегоНазначения.КодОсновногоЯзыка());
		//++ НЕ УТ
		ИначеЕсли Выборка.КодПричины = 2 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
				|    - " + НСтр("ru = 'используется управление производством версии 2.2';
								|en = 'Production management, version 2.2 is used'", ОбщегоНазначения.КодОсновногоЯзыка());
		//-- НЕ УТ
		ИначеЕсли Выборка.КодПричины = 3 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
				|    - " + НСтр("ru = 'в данной информационной базе партионный учет версии 2.1 не использовался';
								|en = 'batch accounting of version 2.1 was not used in this infobase '", ОбщегоНазначения.КодОсновногоЯзыка());
		//++ НЕ УТ
		ИначеЕсли Выборка.КодПричины = 4 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
				|    - " + НСтр("ru = 'используется учет внеоборотных активов версии 2.4';
								|en = 'Capital asset accounting 2.4 is used'", ОбщегоНазначения.КодОсновногоЯзыка());
		//-- НЕ УТ
		Иначе
			ВызватьИсключение НСтр("ru = 'Неизвестный код ошибки.';
									|en = 'Unknown error code.'", ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстПредупреждения) Тогда
		ТекстПредупреждения = НСтр("ru = 'Невозможен возврат к партионному учету версии 2.1:';
									|en = 'You cannot return to batch accounting 2.1:'", ОбщегоНазначения.КодОсновногоЯзыка())
			+ ТекстПредупреждения;;
	КонецЕсли;
...
Процедура СкорректироватьСтоимостьСписанияЗапасовПоФИФОСкользящая(ПараметрыРасчета)
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли СтрокаОстатка.Регистратор = Выборка.Регистратор
			...
			Если Ложь
			 //++ Устарело_Производство21
			 ИЛИ ТипЗнч(Выборка.Регистратор) = Тип("ДокументСсылка.ВыпускПродукции")
			 //-- Устарело_Производство21
			 ...
			 Тогда
				...
				СуммыРаспределения.ДопРасходы = 0;
				СуммыРаспределения.ДопРасходыБезНДС = 0;
...
Процедура СкорректироватьСтоимостьСписанияНезавершенногоПроизводстваПоФИФОСкользящая(ПараметрыРасчета)
	...
	РасчетСебестоимостиПротоколРасчета.НачалоЭтапаРасчета(ПараметрыРасчета, "СкорректироватьСтоимостьСписанияНезавершенногоПроизводстваПоФИФОСкользящая");
	...
	|	Распределение.НаправлениеДеятельности КАК КорНаправлениеДеятельности,
	...
	|	СУММА(Таб.ДопРасходы) КАК ДопРасходы,
	|	СУММА(Таб.ДопРасходыБезНДС) КАК ДопРасходыБезНДС,
	...
	|		0. КАК ДопРасходыБезНДС,
	|		СУММА(ВЫБОР
	|			КОГДА СтатьиРасходов.ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
	...
	|		И (ЛОЖЬ
	//++ Устарело_Производство21
	|			ИЛИ ДД.Регистратор ССЫЛКА Документ.ВыпускПродукции И ДД.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	//-- Устарело_Производство21
	...
	|		И (СтатьиРасходов.ВариантРаспределенияРасходовУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
	...
	|				КОГДА СтатьиРасходов.ВариантРаспределенияРасходовУпр <> ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
	|					ТОГДА 0
	|				КОГДА ДД.АналитикаУчетаПартий.НалогообложениеНДС В (&ТипыНалогообложенияНДСУчитываетсяВСтоимости)
	|			ТОГДА
	...
	|		0. КАК Стоимость,
	|		0. КАК СтоимостьБезНДС,
	|		-СУММА(ДД.Стоимость) КАК ДопРасходы,
	|		-СУММА(ДД.СтоимостьБезНДС) КАК ДопРасходыБезНДС,
	|		-СУММА(
	|			ВЫБОР
	|				КОГДА ДД.АналитикаУчетаПартий.НалогообложениеНДС В (&ТипыНалогообложенияНДСУчитываетсяВСтоимости)
	|			ТОГДА
	...
	|ДоляСтоимости, Стоимость, СтоимостьБезНДС, ДопРасходы, ДопРасходыБезНДС,
	|СтоимостьРегл, ПостояннаяРазница, ВременнаяРазница");
	
	СтрокаПрихода = Новый Структура("
	|Организация, СтатьяРасходов, АналитикаРасходов, НаправлениеДеятельности, КорНаправлениеДеятельности, ИсходноеПодразделение, КорПодразделение, ДокументДвижения, Период,
	|ДоляСтоимости, Стоимость, СтоимостьБезНДС, ДопРасходы, ДопРасходыБезНДС,
	|СтоимостьРегл, ПостояннаяРазница, ВременнаяРазница");
	...
	|СтоимостьУпр, ДопРасходыУпр, ТрудозатратыУпр");
	...
	Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ВидДвижения = ВидДвиженияНакопления.Расход Тогда
				Если СтрокаПрихода.Организация <> Выборка.Организация
				 ...
				 ИЛИ СтрокаПрихода.ИсходноеПодразделение <> Выборка.ИсходноеПодразделение Тогда
					...
					СтрокаПрихода.ДопРасходы 		 = 0;
					СтрокаПрихода.ДопРасходыБезНДС = 0;
			...
			ИначеЕсли Выборка.ВидДвижения = ВидДвиженияНакопления.Приход Тогда
				Если СтрокаПрихода.Организация = Выборка.Организация
				 ...
				 И СтрокаПрихода.ИсходноеПодразделение = Выборка.ИсходноеПодразделение Тогда
					...
					СтрокаОстатка.ДопРасходы 		 = СтрокаОстатка.ДопРасходы - СтрокаПрихода.ДопРасходы;
					СтрокаОстатка.ДопРасходыБезНДС = СтрокаОстатка.ДопРасходыБезНДС - СтрокаПрихода.ДопРасходыБезНДС;
		...
		ИначеЕсли СтрокаОстатка.Регистратор = Выборка.Регистратор
		 ...
		 И СтрокаОстатка.ИсходноеПодразделение = Выборка.ИсходноеПодразделение Тогда
			...
			Если СтрокаОстатка.ДоляСтоимости <> 0 Тогда
				...
				Суммы.ДопРасходы 		 = Окр(Выборка.ДоляСтоимости * СтрокаОстатка.ДопРасходы / СтрокаОстатка.ДоляСтоимости, 2, 1);
				Суммы.ДопРасходыБезНДС = Окр(Выборка.ДоляСтоимости * СтрокаОстатка.ДопРасходыБезНДС / СтрокаОстатка.ДоляСтоимости, 2, 1);
			...
			СтрокаОстатка.ДопРасходы 		 = СтрокаОстатка.ДопРасходы - Суммы.ДопРасходы;
			СтрокаОстатка.ДопРасходыБезНДС = СтрокаОстатка.ДопРасходыБезНДС - Суммы.ДопРасходыБезНДС;
			...
			Если Выборка.ВидДвижения = ВидДвиженияНакопления.Расход Тогда
				...
				СтрокаПрихода.ДопРасходы 		 = СтрокаПрихода.ДопРасходы + Суммы.ДопРасходы;
				СтрокаПрихода.ДопРасходыБезНДС = СтрокаПрихода.ДопРасходыБезНДС + Суммы.ДопРасходыБезНДС;
			...
			ЕстьНенулеваяСумма =
				...
				ИЛИ Суммы.ДопРасходы <> 0 ИЛИ Суммы.ДопРасходыБезНДС <> 0 
		...
		Иначе
			ЗаполнитьЗначенияСвойств(СтрокаПрихода, Выборка);
			СтрокаПрихода.ДокументДвижения       = Выборка.ДокументИсточник;
			СтрокаПрихода.ДоляСтоимости			 = Выборка.ДоляСтоимости;
			СтрокаПрихода.Стоимость 			 = 0;
			СтрокаПрихода.СтоимостьБезНДС 		 = 0;
			СтрокаПрихода.ДопРасходы 		 	 = 0;
			СтрокаПрихода.ДопРасходыБезНДС 		 = 0;
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.УничтожитьВременныеТаблицы(ПараметрыРасчета, "ПрочиеРасходыНЗПДолиСтоимости");
...
Процедура СформироватьДвиженияПрочиеРасходыНезавершенногоПроизводства(ПараметрыРасчета, ДанныеДвижения, Суммы)
	...
	Запись.Стоимость 	   = Суммы.Стоимость + Суммы.ДопРасходы;
	Запись.СтоимостьБезНДС = Суммы.СтоимостьБезНДС + Суммы.ДопРасходыБезНДС;
	Запись.СтоимостьРегл   = Суммы.СтоимостьРегл;
			
	Если Суммы.Свойство("РезервРегл") И Суммы.РезервРегл > 0 Тогда
		Запись.СтоимостьРегл = Макс(Запись.СтоимостьРегл - Суммы.РезервРегл, 0);
	КонецЕсли;
...
Процедура СформироватьДвиженияСебестоимостьТоваровСписаниеНЗП(ПараметрыРасчета, ДанныеДвижения, Суммы)
	...
	Запись.ВидЗапасов 				  = ДанныеДвижения.ВидЗапасов;
...
Процедура СформироватьДвиженияПрочиеРасходыСписаниеНЗП(ПараметрыРасчета, ДанныеДвижения, Суммы)
	...
	Запись.Сумма 			  	 = Суммы.Стоимость + Суммы.ДопРасходы;
	Запись.СуммаБезНДС 	  		 = Суммы.СтоимостьБезНДС + Суммы.ДопРасходыБезНДС;
	...
	Если ПараметрыРасчета.ФО.ИспользоватьУчетПрочихДоходовРасходовРегл Тогда
		...
		Если Суммы.Свойство("РезервРегл") И Суммы.РезервРегл > 0 Тогда
			Запись.СуммаРегл = Макс(Запись.СуммаРегл - Суммы.РезервРегл, 0);
		КонецЕсли;
...
Процедура СкорректироватьСтоимостьПрочегоСписанияНезавершенногоПроизводстваПоФИФОСкользящая(ПараметрыРасчета)
	...
	РасчетСебестоимостиПротоколРасчета.НачалоЭтапаРасчета(ПараметрыРасчета, "СкорректироватьСтоимостьПрочегоСписанияНезавершенногоПроизводстваПоФИФОСкользящая");
	...
	|	Партии.НаправлениеДеятельностиПолучатель КАК КорНаправлениеДеятельности,
	...
	Пока Выборка.Следующий() Цикл
		...
		РасчетСебестоимостиКорректировкаСтоимости.СформироватьДвиженияПрочиеРасходыПрочееСписаниеНЗП(ПараметрыРасчета, Выборка);
...
Процедура ПолучитьСебестоимостьПартийТоваров(ПараметрыРасчета, ДляСтоимостиПродаж = Ложь)
	...
	//++ НЕ УТКА
	|		КОГДА Партии.Регистратор ССЫЛКА Документ.ОтчетДавальцу
	|		 И Партии.ВидЗапасов = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга)
	//-- НЕ УТКА
	...
	|				И (Затраты.СтатьяРасходовСписания = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)
	|					ИЛИ  Затраты.СтатьяРасходовСписания = НЕОПРЕДЕЛЕНО))
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|		И ЕСТЬNULL(Затраты.ВидЗапасов.ТипЗапасов, НЕОПРЕДЕЛЕНО) <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца)
	|		И Затраты.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|		И (ЛОЖЬ
	//++ Устарело_Производство21
	|			ИЛИ Затраты.Регистратор ССЫЛКА Документ.ВыпускПродукции
	//-- Устарело_Производство21
	...
	|			ПО Аналитика.Ссылка = Затраты.АналитикаУчетаНоменклатуры
	|	ГДЕ
	|		НЕ Затраты.СлужебноеВидДвиженияПриход
	|		И ЕСТЬNULL(Затраты.ВидЗапасов.ТипЗапасов, НЕОПРЕДЕЛЕНО) <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|		И ЕСТЬNULL(Затраты.ВидЗапасов.ТипЗапасов, НЕОПРЕДЕЛЕНО) <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца)
	|		И (Затраты.ВидЗапасов <> ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	|			ИЛИ Аналитика.Номенклатура.ТипНоменклатуры В (
	|					ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа),
	|					ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга))
	|			ИЛИ Затраты.ДокументПоступления ССЫЛКА Документ.ОтчетКомитентуОСписании
	|			ИЛИ Затраты.ДокументПоступления ССЫЛКА Документ.ОтчетПоКомиссииМеждуОрганизациямиОСписании)
	|		И (ЛОЖЬ
	//++ Устарело_Производство21
	|			ИЛИ Затраты.Регистратор ССЫЛКА Документ.ВыпускПродукции
	//-- Устарело_Производство21
	...
	|		И (ЛОЖЬ
	//++ НЕ УТКА
	|			ИЛИ Выпуск.Регистратор ССЫЛКА Документ.ОтчетДавальцу
	//-- НЕ УТКА

	//++ Устарело_Производство21
	|			ИЛИ НЕ ВыпускВПодразделение.Ссылка ЕСТЬ NULL
	//-- Устарело_Производство21
	|)
	//-- НЕ УТ
	|
...
Процедура РассчитатьСебестоимостьПоГруппеОрганизаций(ПараметрыЗапуска, ПараметрыРасчета, ПараметрыОтладки)
	...
	РасчетСебестоимостиКорректировкаСтоимости.РаспределитьПрочиеРасходыНезавершенногоПроизводства(ПараметрыРасчета);
	//-- НЕ УТ
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.УничтожитьВременныеТаблицы(ПараметрыРасчета,
		...
		//++ НЕ УТ
		|ПриемникиРезервов, ДетальныеРезервыПоУзлам, ЕстьПроизводственныеУзлы,
		//-- НЕ УТ
	...
	Если НЕ ПараметрыРасчета.ПредварительныйРасчет Тогда
		...
		СтоимостьСписанияРезервов(ПараметрыРасчета);
	КонецЕсли;
	//-- НЕ УТ
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.УничтожитьВременныеТаблицы(ПараметрыРасчета,
		...
		//++ НЕ УТ
		|ВтНезавершенноеПроизводство,
		|РезервыПоРасходам, ПокрываемыеРезервамиСтатьиРасходов,
		|ПриемникиРезервов, ДетальныеРезервыПоУзлам, ЕстьПроизводственныеУзлы,
		//-- НЕ УТ
		|ВтТаблицаРешений, ВтПеремещенияСписания");
	...
	Если НЕ ПараметрыРасчета.ПредварительныйРасчет Тогда
		...
		Если ПараметрыРасчета.ФО.ИспользоватьУчетПрочихДоходовРасходов Тогда
			...
			РасчетСебестоимостиКорректировкаСтоимости.СкорректироватьСтоимостьПрочегоСписанияНезавершенногоПроизводства(ПараметрыРасчета);
...
Процедура СтоимостьНезавершенногоПроизводства(ПараметрыРасчета)
	...
	РасчетСебестоимостиПротоколРасчета.НачалоЭтапаРасчета(ПараметрыРасчета, "СтоимостьНезавершенногоПроизводства");
	...
	Если ПараметрыРасчета.КоличествоУзлов <> 0 Тогда
		...
		|	СУММА(ТаблицаРешений.СтоимостьГраф_1
		|		* ТаблицаРешений.СтоимостьЗнакГраф_1) 			КАК Стоимость,
		|	СУММА(ТаблицаРешений.ДопРасходыГраф_1)				КАК СтоимостьДопРасходы,
		|	0													КАК Трудозатраты,
		|	0													КАК ПостатейныеПостоянныеСНДС,
		|	0													КАК ПостатейныеПеременныеСНДС,
		|	СУММА(ТаблицаРешений.СтоимостьЗабалансовая)			КАК СтоимостьЗабалансовая,
		...
		|	СУММА(ТаблицаРешений.СтоимостьГраф_2
		|		* ТаблицаРешений.СтоимостьЗнакГраф_2) 			КАК СтоимостьБезНДС,
		|	СУММА(ТаблицаРешений.ДопРасходыГраф_2)				КАК СтоимостьДопРасходыБезНДС,
		...
		|	И УзлыКорректировки.РазделУчета В (ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты),
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос, Истина, "ВтСтоимостиНезавершенногоПроизводства");
...
Процедура СтоимостьНезавершенногоПроизводстваРегл(ПараметрыРасчета) Экспорт
	...
	РасчетСебестоимостиПротоколРасчета.НачалоЭтапаРасчета(ПараметрыРасчета, "СтоимостьНезавершенногоПроизводстваРегл");
	...
	Если ПараметрыРасчета.КоличествоУзловРегл <> 0 Тогда
		...
		|	СУММА(ТаблицаРешений.СтоимостьГраф_1
		|		* ТаблицаРешений.СтоимостьЗнакГраф_1)			КАК СтоимостьРегл,
		|	0									 				КАК ДопРасходыРегл,
		|	0										 			КАК ТрудозатратыРегл,
		|	0												 	КАК ПостатейныеПостоянныеРегл,
		|	0												 	КАК ПостатейныеПеременныеРегл,
		|	0												 	КАК РезервРегл,
		|	СУММА(ТаблицаРешений.СтоимостьЗабалансовая) 		КАК СтоимостьЗабалансоваяРегл,
		|	СУММА(ТаблицаРешений.СтоимостьГраф_1
		|		* ТаблицаРешений.СтоимостьЗнакГраф_1)
		|	- СУММА(ТаблицаРешений.ПостояннаяРазница
		|		* ТаблицаРешений.ПостояннаяРазницаЗнак)
		|	- СУММА(ТаблицаРешений.ВременнаяРазница
		|		* ТаблицаРешений.ВременнаяРазницаЗнак)			КАК СтоимостьНУ,
		|	СУММА(ТаблицаРешений.ПостояннаяРазница
		|		* ТаблицаРешений.ПостояннаяРазницаЗнак)			КАК ПостояннаяРазница,
		|	СУММА(ТаблицаРешений.ВременнаяРазница
		|		* ТаблицаРешений.ВременнаяРазницаЗнак) 			КАК ВременнаяРазница,
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос, Истина, "ВтСтоимостиНезавершенногоПроизводстваРегл");
	
КонецПроцедуры

Процедура СтоимостьСписанияРезервов(ПараметрыРасчета)

	РасчетСебестоимостиПротоколРасчета.НачалоЭтапаРасчета(ПараметрыРасчета, "ВтСтоимостиСписанияРезервов");
	...
	Запрос.Текст =
		...
		|	ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)				КАК Организация,
		|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)	КАК НаправлениеДеятельности,
		|	ЗНАЧЕНИЕ(Справочник.Резервы.ПустаяСсылка)					КАК ВидРезервов,
		|	ЗНАЧЕНИЕ(Справочник.ОбъектыУчетаРезервовПредстоящихРасходов.ПустаяСсылка) КАК ОбъектУчетаРезервов,
		|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)		КАК Подразделение,
		|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)КАК СтатьяРасходов,
		|	НЕОПРЕДЕЛЕНО												КАК АналитикаРасходов,
		|	0															КАК СуммаРегл,
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос, Истина, "РезервыПредстоящихРасходов");
...
Процедура СкорректироватьСтоимостьСписанияНезавершенногоПроизводства21(ПараметрыРасчета)
	...
	|	Движения.СтатьяРасходов.ВариантРаспределенияРасходовНУ = 
	|		ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаНаправленияДеятельности) КАК КосвенныеЗатратыНУ,
	...
	|		КАК ЧИСЛО(31,2)) КАК ПостояннаяРазница,
	|
	|	ВЫРАЗИТЬ(
	|		ВЫРАЗИТЬ(ТаблицаКорректировки.Количество КАК ЧИСЛО(23,10))
	|		* (ВЫБОР
	...
	|Стоимость, СтоимостьБезНДС, ДопРасходы, ДопРасходыБезНДС, СтоимостьРегл, ПостояннаяРазница, ВременнаяРазница,
	|СтоимостьЗабалансовая, СтоимостьЗабалансоваяРегл");
	
	Суммы = Новый Структура("Стоимость, СтоимостьБезНДС, ДопРасходы, ДопРасходыБезНДС, СтоимостьРегл,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПараметрыРасчета.ФО.ИспользоватьУчетПрочихДоходовРасходов
		 И Выборка.ВидДвижения = ВидДвиженияНакопления.Приход Тогда
		 	...
		 	СуммыПрочихРасходов = Новый Структура(Новый ФиксированнаяСтруктура(Суммы));
			ЗаполнитьЗначенияСвойств(СуммыПрочихРасходов, Выборка);
			
			СформироватьДвиженияПрочиеРасходыСписаниеНЗП(ПараметрыРасчета, Выборка, СуммыПрочихРасходов);
...
Процедура СформироватьУзлыКорректировкиСписанияСтоимости(ПараметрыРасчета)
	...
	//++ НЕ УТ
	|	ВложенныйЗапрос.ВидРезервов					КАК ВидРезервов,
	|	ВложенныйЗапрос.ОбъектУчетаРезервов			КАК ОбъектУчетаРезервов,
	...
	|	ВложенныйЗапрос.ПравилоОтнесенияНаВыпуск	КАК ПравилоОтнесенияНаВыпуск,
	//-- НЕ УТ
	...
	|		Таблица.ПравилоОтнесенияНаВыпуск							КАК ПравилоОтнесенияНаВыпуск,
		// Показатели графа 1. Стоимость.
	|		Таблица.Количество                 							КАК ВесГраф_1,
	|		Таблица.Сумма			                      				КАК СтоимостьГраф_1,
	|		0                                  							КАК ДопРасходыГраф_1,
	|		0                                  							КАК СтоимостьЗабалансовая,
		// Показатели графа 2. СтоимостьБезНДС.
	|		Таблица.Количество                 							КАК ВесГраф_2,
	|		Таблица.СуммаБезНДС                							КАК СтоимостьГраф_2,
	|		0                                  							КАК ДопРасходыГраф_2
	|	ИЗ
	...
	//++ НЕ УТ
	|	ВложенныйЗапрос.ВидРезервов,
	|	ВложенныйЗапрос.ОбъектУчетаРезервов,
	|	ВложенныйЗапрос.СтатьяКалькуляции,
	|	ВложенныйЗапрос.Этап,
	|	ВложенныйЗапрос.ГруппаПродукции,
	|	ВложенныйЗапрос.ПравилоОтнесенияНаВыпуск,
	//-- НЕ УТ
...
Процедура СформироватьУзлыКорректировкиСписанияСтоимостиРегл(ПараметрыРасчета)
	...
	|		Таблица.ПравилоОтнесенияНаВыпуск							КАК ПравилоОтнесенияНаВыпуск,
	        // Показатели граф 1.
	|		Таблица.Количество                 							КАК ВесГраф_1,
	|		Таблица.СуммаРегл                  							КАК СтоимостьГраф_1,
	|		0                                  							КАК ДопРасходыГраф_1,
	|		0                                  							КАК СтоимостьЗабалансовая,
	|		Таблица.ПостояннаяРазница          							КАК ПостояннаяРазница,
	|		Таблица.ВременнаяРазница           							КАК ВременнаяРазница,
...
Процедура ПодготовитьДанныеДляРасчетаСтоимостиПоФИФО(ПараметрыРасчета, Запрос)
	...
	//++ НЕ УТ
	|	ТаблицаУзлыКорректировки.ВидРезервов				КАК ВидРезервов,
	|	ТаблицаУзлыКорректировки.ОбъектУчетаРезервов		КАК ОбъектУчетаРезервов,
	|	ТаблицаУзлыКорректировки.СтатьяКалькуляции			КАК СтатьяКалькуляции,
	|	ТаблицаУзлыКорректировки.Этап 						КАК Этап,
	|	ТаблицаУзлыКорректировки.ГруппаПродукции			КАК ГруппаПродукции,
	|	ТаблицаУзлыКорректировки.ПравилоОтнесенияНаВыпуск	КАК ПравилоОтнесенияНаВыпуск,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	Т.ВидРезервов КАК ВидРезервов,
	|	Т.ОбъектУчетаРезервов КАК ОбъектУчетаРезервов,
...
Процедура РассчитатьСтоимость(ПараметрыРасчета, РасчетСтоимостиРегл)
	...
	//++ НЕ УТ
	РасчетСебестоимостиПрикладныеАлгоритмы.УничтожитьВременныеТаблицы(ПараметрыРасчета, "Выпуски");
	//-- НЕ УТ
...
Функция ТекстЗапросаДвиженияСтоимости(ПараметрыРасчета)
	...
	|			ИНАЧЕ УчетСебестоимости.Количество КОНЕЦ) КАК ВесДугиГраф_1,
	|		(ВЫБОР
	|			КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход ТОГДА -УчетСебестоимости.Количество
	|			ИНАЧЕ УчетСебестоимости.Количество КОНЕЦ) КАК ВесДугиГраф_2
	|	ИЗ
	|		ВТКэшРасчетныеОборотыСебестоимостьТоваров КАК УчетСебестоимости
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|		ПО
	|			УчетСебестоимости.КорОрганизация = СпрОрганизации.Ссылка
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|			ВтУзлыКорректировки КАК УзлыКорректировкиПриемник
	|		ПО
	|			УчетСебестоимости.СтатьяРасходовСписания = УзлыКорректировкиПриемник.СтатьяРасходов
	...
	|			И УчетСебестоимости.Подразделение = УзлыКорректировкиПриемник.Подразделение
	...
	|		Расходы.ДоляСтоимости 					КАК ВесДугиГраф_1,
	|		Расходы.ДоляСтоимости 					КАК ВесДугиГраф_2
	|	ИЗ
	...
	|			ВтУзлыКорректировки КАК УзлыКорректировкиИсточник
	|		ПО
	|			УзлыКорректировкиИсточник.Организация = Расходы.Организация
	...
	|			И УзлыКорректировкиИсточник.Подразделение = Расходы.ДокументИсточник.Подразделение
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
...
Функция ТекстЗапросаДвиженияСтоимостиРегл(ПараметрыРасчета)
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Списание материалов на статьи расходов (в пределах одной организации)
	|	ВЫБРАТЬ
	|		УзлыКорректировкиИсточник.НомерУзла  КАК НомерУзлаИсточник,
	|		УзлыКорректировкиПриемник.НомерУзла  КАК НомерУзлаПриемник,
	|		ВЫБОР
	|			КОГДА ЛОЖЬ
	//++ Локализация
	|				ИЛИ НЕ &УчетПоФСБУ5_2019 
	|				И (УчетСебестоимости.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаВЭксплуатациюБУНУ)
	|				ИЛИ ЕСТЬNULL(Категория.СпособПогашенияСтоимостиБУ, ЗНАЧЕНИЕ(Перечисление.СпособыПогашенияСтоимостиТМЦ.ПустаяСсылка))
	...
	|				(ВЫБОР
	|				КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход ТОГДА -УчетСебестоимости.Количество
	|				ИНАЧЕ УчетСебестоимости.Количество КОНЕЦ)
	|		КОНЕЦ КАК ВесДугиГраф_1,
	|		(ВЫБОР
	|			КОГДА УчетСебестоимости.СлужебноеВидДвиженияПриход ТОГДА -УчетСебестоимости.Количество
	|			КОГДА НЕ УчетСебестоимости.КорОрганизация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|					И НЕ УчетСебестоимости.КорОрганизация = УчетСебестоимости.Организация
	|				ТОГДА 0
	|			ИНАЧЕ УчетСебестоимости.Количество КОНЕЦ) КАК ВесДугиГраф_2,
	|		(ВЫБОР
	...
	|			ВтУзлыКорректировки КАК УзлыКорректировкиПриемник
	|		ПО
	|			УчетСебестоимости.Организация = УзлыКорректировкиПриемник.Организация
	|			И УчетСебестоимости.Подразделение = УзлыКорректировкиПриемник.Подразделение
	|			И УчетСебестоимости.КорНаправлениеДеятельности = УзлыКорректировкиПриемник.НаправлениеДеятельности
	|			И УчетСебестоимости.СтатьяРасходовСписания = УзлыКорректировкиПриемник.СтатьяРасходов
	|			И УчетСебестоимости.АналитикаРасходов = УзлыКорректировкиПриемник.АналитикаРасходов
	|			И УзлыКорректировкиПриемник.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПокрываемыеРезервамиСтатьиРасходов КАК РасходыПоРезервам
	|		ПО РасходыПоРезервам.Организация = УзлыКорректировкиПриемник.Организация
	|			И РасходыПоРезервам.Подразделение = УзлыКорректировкиПриемник.Подразделение
	|			И РасходыПоРезервам.НаправлениеДеятельности = УзлыКорректировкиПриемник.НаправлениеДеятельности
	|			И РасходыПоРезервам.СтатьяРасходов = УзлыКорректировкиПриемник.СтатьяРасходов
	|			И РасходыПоРезервам.АналитикаРасходов = УзлыКорректировкиПриемник.АналитикаРасходов
	|			
	//++ Локализация
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВнутреннееПотребление.Товары КАК ПередачиВЭксплуатацию
	...
	|													 ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаВЭксплуатациюБУНУ),
	|													 ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаПрочиеЦели))
	// Расчет статей расходов, которые распределяются на производственные затраты или покрываются резервами.
	|		И (Статьи.ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
	|			ИЛИ (&ПартионныйУчетВерсии22 И Статьи.ВариантРаспределенияРасходовУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты))
	|			ИЛИ НЕ РасходыПоРезервам.СтатьяРасходов ЕСТЬ NULL)	
	// Исключение сторно списания расходов с возвратом прошлого периода, т.к. стоимость рассчитывается на шаге СкорректироватьСтоимостьВозвратовПрошлыхПериодов
	|		И НЕ (УчетСебестоимости.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СторноСписанияНаРасходы) 
	|			И УчетСебестоимости.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ВозвратПрошлыхПериодов))
	|		И НЕ (УчетСебестоимости.Сторно И УчетСебестоимости.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ИсправлениеПрошлогоПериода))
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// Списание материалов на статьи расходов (перенос стоимости на другую организацию)
	|	ВЫБРАТЬ
	|		УзлыКорректировкиИсточник.НомерУзла  КАК НомерУзлаИсточник,
	|		УзлыКорректировкиПриемник.НомерУзла  КАК НомерУзлаПриемник,
	|		0 КАК ВесДугиГраф_1,
	...
	|			ВтУзлыКорректировки КАК УзлыКорректировкиПриемник
	|		ПО
	|			УчетСебестоимости.КорОрганизация = УзлыКорректировкиПриемник.Организация
	|			И УчетСебестоимости.Подразделение = УзлыКорректировкиПриемник.Подразделение
	|			И УчетСебестоимости.КорНаправлениеДеятельности = УзлыКорректировкиПриемник.НаправлениеДеятельности
	|			И УчетСебестоимости.СтатьяРасходовСписания = УзлыКорректировкиПриемник.СтатьяРасходов
	|			И УчетСебестоимости.АналитикаРасходов = УзлыКорректировкиПриемник.АналитикаРасходов
	...
	|													 ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаВЭксплуатациюБУНУ),
	|													 ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаПрочиеЦели))
	|		И НЕ УчетСебестоимости.КорОрганизация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|		И НЕ УчетСебестоимости.КорОрганизация = УчетСебестоимости.Организация
	// Расчет статей расходов, которые распределяются на производственные затраты или покрываются резервами.
	|		И Статьи.ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
	// Исключение сторно списания расходов с возвратом прошлого периода, т.к. стоимость рассчитывается на шаге СкорректироватьСтоимостьВозвратовПрошлыхПериодов
...
Процедура РешитьСЛУПлатформойБазовыеЗначенияГраф_1(ПараметрыРасчета)
	...
	ПояснениеДляЗамера = НСтр("ru = 'Решение для базовых значений (граф 1)';
								|en = 'Solution for basic values (chart 1)'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	СтруктураПолейСЛУ.Вставить("ВесДугиСтоимостьГраф_1", "СтоимостьГраф_1");
	СтруктураПолейСЛУ.Вставить("ВесДугиДопРасходыГраф_1", "ДопРасходыГраф_1");
...
Процедура РешитьСЛУПлатформойБазовыеЗначенияГраф_2(ПараметрыРасчета)
	...
	ПояснениеДляЗамера = НСтр("ru = 'Решение для базовых значений (граф 2)';
								|en = 'Solution for basic values (chart 2)'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	// корректируем стоимости узлов с нулевым весом
	СтруктураПолейСЛУ = Новый Структура;
	СтруктураПолейСЛУ.Вставить("ВесДугиСтоимостьГраф_2", "СтоимостьГраф_2");
	СтруктураПолейСЛУ.Вставить("ВесДугиДопРасходыГраф_2", "ДопРасходыГраф_2");
...
Процедура РешитьСЛУПлатформойРазницыШаг(ПараметрыРасчета, РасчетСтоимостиРегл, НомерШага)
	...
	ШаблонПояснениеДляЗамера = НСтр("ru = 'Решение для %1 (шаг %2)';
									|en = 'Solution for %1 (step %2)'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	ПояснениеДляЗамера = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПояснениеДляЗамера, НСтр("ru = 'ПР';
																												|en = 'PD'", ОбщегоНазначения.КодОсновногоЯзыка()), НомерШага);
	...
	ПояснениеДляЗамера = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПояснениеДляЗамера, НСтр("ru = 'ВР';
																												|en = 'TD'", ОбщегоНазначения.КодОсновногоЯзыка()), НомерШага);
	
	// корректируем стоимости узлов с нулевым весом
	СтруктураПолейСЛУ = Новый Структура;
	СтруктураПолейСЛУ.Вставить("ВесДугиВременнаяРазница", "ВременнаяРазница");
...
Функция АнализСебестоимостиВыпущеннойПродукции_ТекстЗапросаМатериальные() Экспорт
	ТекстЗапроса = "
		...
		|	РегистрНакопления.СебестоимостьТоваров КАК ДД
		|ГДЕ
		|	(ДД.Период МЕЖДУ &НачалоПериода И &КонецПериода ИЛИ &КонецПериода = ДАТАВРЕМЯ(1,1,1))
		...
		|	Партии.КорАналитикаУчетаНоменклатуры,
		|	СУММА(
		|		ВЫБОР КОГДА &ДанныеПоСебестоимости = 1 ТОГДА
		|			Партии.Стоимость
		|		КОГДА &ДанныеПоСебестоимости = 2 ИЛИ &ДанныеПоСебестоимости = 3 ТОГДА
		|			Партии.СтоимостьБезНДС
		|		КОГДА &ДанныеПоСебестоимости = 4 ТОГДА
		|			Партии.СтоимостьРегл
		|			+(ВЫБОР
		...
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РасчетСебестоимостиТоваров.Организации КАК РасчетСебестоимости
		|		ПО НАЧАЛОПЕРИОДА(Партии.Период, МЕСЯЦ) = НАЧАЛОПЕРИОДА(РасчетСебестоимости.Ссылка.Дата, МЕСЯЦ)
		|		И Партии.Организация = РасчетСебестоимости.Организация
		|		И РасчетСебестоимости.Ссылка.Проведен
		|		И РасчетСебестоимости.Ссылка.МетодОценки = ЗНАЧЕНИЕ(Перечисление.МетодыОценкиСтоимостиТоваров.ФИФОСкользящаяОценка)
		|ГДЕ
		|	(Партии.Период МЕЖДУ &НачалоПериода И &КонецПериода ИЛИ &КонецПериода = ДАТАВРЕМЯ(1,1,1))
		|	И Партии.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|	И (Партии.Регистратор ССЫЛКА Документ.ПоступлениеОтПереработчика
		//++ Устарело_Производство21
		|		ИЛИ Партии.Регистратор ССЫЛКА Документ.ВыпускПродукции
		//-- Устарело_Производство21

		//++ НЕ УТКА
		|		ИЛИ Партии.Регистратор ССЫЛКА Документ.ОтчетДавальцу
		//-- НЕ УТКА
		|		)
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	Партии.КорВидЗапасов,
		|	Партии.Продукция,
		|	Партии.ХарактеристикаПродукции,
		|	СУММА(Партии.Количество) КАК Количество,
		|	СУММА(
		|		ВЫБОР КОГДА &ДанныеПоСебестоимости = 1 ТОГДА
		|			Партии.Стоимость
		...
		|		КОНЕЦ) КАК Стоимость,
		|	СУММА(
		|		ВЫБОР КОГДА &ДанныеПоСебестоимости = 4 ТОГДА
		|			Партии.ВременнаяРазница
		|		ИНАЧЕ
		|			0
		|		КОНЕЦ) КАК ВременнаяРазница,
		...
		|	(Партии.Период МЕЖДУ &НачалоПериода И &КонецПериода ИЛИ &КонецПериода = ДАТАВРЕМЯ(1,1,1))
		|	И Партии.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|	И (Партии.ВидЗапасов = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка) ИЛИ (Партии.ВидЗапасов.ТипЗапасов <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.МатериалДавальца)
		|				И Партии.ВидЗапасов.ТипЗапасов <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца)))
		|	И (Партии.Регистратор ССЫЛКА Документ.ПоступлениеОтПереработчика
		//++ Устарело_Производство21
		|		ИЛИ Партии.Регистратор ССЫЛКА Документ.ВыпускПродукции
		//-- Устарело_Производство21
		...
		|		ИЛИ Партии.Период < Константы.ДатаПереходаНаПартионныйУчетВерсии22)
		|
		|СГРУППИРОВАТЬ ПО
		|	Партии.Период,
		|	Партии.Регистратор,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ЗатратыНЗП.Организация КАК Организация,
		|	НЕОПРЕДЕЛЕНО КАК ПодразделениеПродукции,
		|	ЗатратыНЗП.АналитикаУчетаПродукции КАК АналитикаПродукции,
		...
		|	АналитикаВыпуска.Серия КАК СерияПродукции,
		|	ЗатратыНЗП.СтатьяКалькуляции КАК СтатьяКалькуляции,
		...
		|	КОГДА &ЕдиницыКоличества = 1 ТОГДА
		|		Аналитика.Номенклатура.ЕдиницаДляОтчетов
		|	КОНЕЦ КАК ЕдиницаИзмеренияЗатраты,
		|	
		|	Аналитика.МестоХранения КАК Подразделение,
		|	(ВЫБОР
		|		КОГДА ЗатратыНЗП.ЗаказНаПроизводство ЕСТЬ NULL ТОГДА НЕОПРЕДЕЛЕНО
		|		КОГДА ЗатратыНЗП.ЗаказНаПроизводство ССЫЛКА Справочник.РесурсныеСпецификации Тогда НЕОПРЕДЕЛЕНО
		//++ НЕ УТКА
		|		КОГДА ЗатратыНЗП.Регистратор ССЫЛКА Документ.ОтчетДавальцу ТОГДА ЗатратыНЗП.Регистратор
		//-- НЕ УТКА
		|		ИНАЧЕ ЗатратыНЗП.ЗаказНаПроизводство КОНЕЦ) КАК ЗаказНаПроизводство,
		|	ЕСТЬNULL(ЗатратыНЗП.ВидЗапасов.ГруппаПродукции, ЗНАЧЕНИЕ(Справочник.ГруппыАналитическогоУчетаНоменклатуры.ПустаяСсылка)) КАК ГруппаПродукции,
		|	ЗатратыНЗП.КорВидЗапасов КАК ВидЗапасов,
		|	АналитикаВыпуска.Назначение КАК НазначениеПродукции,
		|	
		|	ВЫБОР КОГДА &ЕдиницыКоличества = 0 ТОГДА
		|		ЗатратыНЗП.Количество
		|	КОГДА &ЕдиницыКоличества = 1 ТОГДА
		|		ВЫБОР КОГДА Аналитика.Номенклатура.КоэффициентЕдиницыДляОтчетов <> 0 ТОГДА
		|			ЗатратыНЗП.Количество / Аналитика.Номенклатура.КоэффициентЕдиницыДляОтчетов
		|		ИНАЧЕ 0
		|		КОНЕЦ
		|	КОНЕЦ КАК КоличествоЗатрат,
		|	
		|	ВЫБОР КОГДА ЗатратыНЗП.Регистратор ССЫЛКА Документ.ПоступлениеОтПереработчика
		|	 И ЗатратыНЗП.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга) ТОГДА
		|		ЗатратыНЗП.Стоимость
		|	КОГДА РасчетСебестоимости.Ссылка ЕСТЬ NULL ТОГДА	
		|		ВЫБОР КОГДА &ДанныеПоСебестоимости = 1 ТОГДА
		|			ЗатратыНЗП.Количество * (СтоимостьЗатрат.Стоимость + СтоимостьЗатрат.СтоимостьДопРасходы)
		|		КОГДА &ДанныеПоСебестоимости = 2 ТОГДА
		|			ЗатратыНЗП.Количество * (СтоимостьЗатрат.СтоимостьБезНДС + СтоимостьЗатрат.СтоимостьДопРасходыБезНДС)
		|		КОГДА &ДанныеПоСебестоимости = 3 ТОГДА
		|			ЗатратыНЗП.Количество * (СтоимостьЗатрат.СтоимостьУпр + СтоимостьЗатрат.ДопРасходыУпр)
		|		КОГДА &ДанныеПоСебестоимости = 4 ТОГДА
		|			ЗатратыНЗП.Количество * СтоимостьЗатрат.СтоимостьРегл
		|		ИНАЧЕ
		|			ВЫБОР КОГДА &ПоПредприятию
		|				ТОГДА ЗатратыНЗП.Количество * (СтоимостьЗатрат.СтоимостьБезНДС + СтоимостьЗатрат.СтоимостьДопРасходыБезНДС)
		|				ИНАЧЕ ЗатратыНЗП.Количество * (СтоимостьЗатрат.СтоимостьУпр + СтоимостьЗатрат.ДопРасходыУпр)
		|			КОНЕЦ
		|		КОНЕЦ
		|	ИНАЧЕ
		|		ЗатратыНЗП.Стоимость
		|		+ ЕСТЬNULL(ПартииРасходов.Стоимость, 0)
		|		+ ЕСТЬNULL(ПартииЗатрат.Стоимость, 0)
		|	КОНЕЦ КАК СтоимостьЗатрат,
		|	ВЫБОР КОГДА РасчетСебестоимости.Ссылка ЕСТЬ NULL ТОГДА	
		|		ВЫБОР КОГДА &ДанныеПоСебестоимости = 4 ТОГДА
		|			ЗатратыНЗП.Количество * СтоимостьЗатрат.ВременнаяРазница
		|		ИНАЧЕ
		|			0
		|		КОНЕЦ
		|	ИНАЧЕ
		|		ЗатратыНЗП.ВременнаяРазница
		|	КОНЕЦ КАК ВременнаяРазница,
		|	ВЫБОР КОГДА РасчетСебестоимости.Ссылка ЕСТЬ NULL ТОГДА	
		|		ВЫБОР КОГДА &ДанныеПоСебестоимости = 4 ТОГДА
		|			ЗатратыНЗП.Количество * СтоимостьЗатрат.ПостояннаяРазница
		|		ИНАЧЕ
		|			0
		|		КОНЕЦ
		|	ИНАЧЕ
		|		ЗатратыНЗП.ПостояннаяРазница
		|	КОНЕЦ КАК ПостояннаяРазница,
		// В РН ПартииЗатратНаВыпуск нет ресурса по НДД, поэтому 0
		|	0 КАК НДД,
		|	ЗатратыНЗП.Регистратор КАК Регистратор
		|ИЗ
		|	ПартииНЗП КАК ЗатратыНЗП
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВозвратныеОтходы КАК ВозвратныеОтходы
		|		ПО ЗатратыНЗП.АналитикаУчетаНоменклатуры = ВозвратныеОтходы.АналитикаУчетаНоменклатуры 
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
		|		ПО ЗатратыНЗП.АналитикаУчетаНоменклатуры = Аналитика.Ссылка
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК АналитикаВыпуска
		|		ПО ЗатратыНЗП.АналитикаУчетаПродукции = АналитикаВыпуска.Ссылка
		|	ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
		|		ПО (ИСТИНА)
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтоимостьТоваров КАК СтоимостьЗатрат
		|		ПО ВЫБОР КОГДА Константы.УчитыватьСебестоимостьТоваровПоНазначениям
		|			И Константы.ДатаВключенияОбособленногоУчетаСебестоимостиПоНазначениям <= &НачалоПериода
		|			ТОГДА ЗатратыНЗП.АналитикаУчетаНоменклатуры = СтоимостьЗатрат.АналитикаУчетаНоменклатуры
		|			ИНАЧЕ
		|				ЗатратыНЗП.АналитикаУчетаНоменклатуры.Номенклатура = СтоимостьЗатрат.АналитикаУчетаНоменклатуры.Номенклатура
		|				И ЗатратыНЗП.АналитикаУчетаНоменклатуры.Характеристика = СтоимостьЗатрат.АналитикаУчетаНоменклатуры.Характеристика
		|				И ЗатратыНЗП.АналитикаУчетаНоменклатуры.Серия = СтоимостьЗатрат.АналитикаУчетаНоменклатуры.Серия
		|				И ЗатратыНЗП.АналитикаУчетаНоменклатуры.МестоХранения = СтоимостьЗатрат.АналитикаУчетаНоменклатуры.МестоХранения
		|				И ЗатратыНЗП.АналитикаУчетаНоменклатуры.СтатьяКалькуляции = СтоимостьЗатрат.АналитикаУчетаНоменклатуры.СтатьяКалькуляции
		|			КОНЕЦ
		|		И ЗатратыНЗП.Организация = СтоимостьЗатрат.Организация
		|		И (СтоимостьЗатрат.Период = НАЧАЛОПЕРИОДА(ЗатратыНЗП.Период, МЕСЯЦ))
		|		И ЗатратыНЗП.ВидЗапасов = СтоимостьЗатрат.ВидЗапасов
		|		И (ВЫБОР
		|			КОГДА ЗатратыНЗП.Количество < 0 И НЕ ВозвратныеОтходы.АналитикаУчетаНоменклатуры ЕСТЬ NULL И СтоимостьЗатрат.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПустаяСсылка)
		|				ТОГДА ИСТИНА
		|		 	КОГДА (ЗатратыНЗП.Количество > 0 ИЛИ ВозвратныеОтходы.АналитикаУчетаНоменклатуры ЕСТЬ NULL) И СтоимостьЗатрат.РазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПереданныеПереработчику) И СтоимостьЗатрат.РазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПустаяСсылка)
		|		 		ТОГДА ИСТИНА
		|		 	ИНАЧЕ ЛОЖЬ КОНЕЦ)
		|	ЛЕВОЕ СОЕДИНЕНИЕ ПартииРасходов КАК ПартииРасходов
		|		ПО ПартииРасходов.Период = ЗатратыНЗП.Период
		|		И ПартииРасходов.Регистратор = ЗатратыНЗП.Регистратор
		|		И ПартииРасходов.АналитикаУчетаНоменклатуры = ЗатратыНЗП.АналитикаУчетаНоменклатуры
		|		И ПартииРасходов.ВидЗапасов = ЗатратыНЗП.ВидЗапасов
		|		И ПартииРасходов.ДокументПоступления = ЗатратыНЗП.ДокументПоступления
		|		И ПартииРасходов.КорАналитикаУчетаНоменклатуры = ЗатратыНЗП.АналитикаУчетаПродукции
		|	ЛЕВОЕ СОЕДИНЕНИЕ ПартииЗатрат КАК ПартииЗатрат
		|		ПО ПартииЗатрат.Период = ЗатратыНЗП.Период
		|		И ПартииЗатрат.Регистратор = ЗатратыНЗП.Регистратор
		|		И ПартииЗатрат.АналитикаУчетаПродукции = ЗатратыНЗП.АналитикаУчетаНоменклатуры
		|		И ПартииЗатрат.ДокументВыпуска = ЗатратыНЗП.ДокументПоступления
		|		И ПартииЗатрат.КорАналитикаУчетаПродукции = ЗатратыНЗП.АналитикаУчетаПродукции
		|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РасчетСебестоимостиТоваров.Организации КАК РасчетСебестоимости
		|		ПО НАЧАЛОПЕРИОДА(ЗатратыНЗП.Период, МЕСЯЦ) = НАЧАЛОПЕРИОДА(РасчетСебестоимости.Ссылка.Дата, МЕСЯЦ)
		|		И ЗатратыНЗП.Организация = РасчетСебестоимости.Организация
		|		И РасчетСебестоимости.Ссылка.Проведен
		|		И РасчетСебестоимости.Ссылка.МетодОценки = ЗНАЧЕНИЕ(Перечисление.МетодыОценкиСтоимостиТоваров.ФИФОСкользящаяОценка)
		|{ГДЕ
		|	((ЗатратыНЗП.Продукция, ЗатратыНЗП.ХарактеристикаПродукции) В
		|			(ВЫБРАТЬ
		|				ОтборПоСегментуНоменклатуры.Номенклатура,
		|				ОтборПоСегментуНоменклатуры.Характеристика
		|			ИЗ
