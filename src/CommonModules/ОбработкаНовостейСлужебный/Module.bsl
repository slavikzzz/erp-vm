...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
		...
		Обработчик.Комментарий         = НСтр("ru = 'Запускается при первом запуске, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).';
												|en = 'Started in common data on the first start. Set constants, import standard values, clear information registers (common data).   '");
	...
	Обработчик = Обработчики.Добавить();
		...
		Обработчик.Комментарий         = НСтр("ru = 'Запускается при первом запуске, для каждой области данных. Установка настроек пользователей (разделенных данных).';
												|en = 'Started for each data area on the first start. Configure user settings (separated data).'");
		Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик = Обработчики.Добавить();
		...
		Обработчик.Комментарий         = НСтр("ru = 'Запускается при переходе на новую версию, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).';
												|en = 'Started in common data on migration to a new version. Set constants, import standard values, clear information registers (common data).'");
		Обработчик.РежимВыполнения     = "Оперативно";
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		Обработчик = Обработчики.Добавить();
			...
			Обработчик.Комментарий             = НСтр("ru = 'Запускается при переходе на любую новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';
														|en = 'Started for each data area on migration to any new version. Configure user settings (separated data).'");
			Обработчик.РежимВыполнения         = "Оперативно";
	Иначе
		...
		Если ДатаПоследнегоОбновления <= ПятьДнейНазад Тогда
			...
			Если ДатаПоследнегоОбновления = '00010101' Тогда
				ТекстСообщения = НСтр("ru = 'ОбработкаНовостейСлужебный.ПередЗапускомПрограммы: пересчет периодических реквизитов не осуществлялся ни разу.
					...
					|en = 'ОбработкаНовостейСлужебный.ПередЗапускомПрограммы: Periodic attributes have never been recalculated.
					|The News subsystem has been switched from [Активна] to [ТребуетсяПересчетОтборовПослеДлительногоОжидания].
					|This is not an error. But important and urgent news will not pop up, because they may have already expired.
					|Soon an automatic recalculation of filters will be performed and the subsystem will return to the [Активна] state.'");
			Иначе
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'ОбработкаНовостейСлужебный.ПередЗапускомПрограммы: прошло длительное время после последнего пересчета периодических реквизитов (%1).
						...
						|Это не является ошибкой. Но важные и очень важные новости не будут всплывать, т.к. возможно, что они уже просрочены.
						|В самое ближайшее время будет произведен автоматический пересчет отборов и подсистема вернется в состояние [Активна].';
						|en = 'ОбработкаНовостейСлужебный.ПередЗапускомПрограммы: a long time has passed since the last recalculation of periodic atributes (%1).
						|The News subsystem has been switched from [Активна] to [ТребуетсяПересчетОтборовПослеДлительногоОжидания].
						|This is not an error. But important and very important news will not pop up, because they may have already expired.
						|Soon an automatic recalculation of filters will be performed and the subsystem will return to [Активна] status.'"),
					Формат(ДатаПоследнегоОбновления, "ДЛФ=DT"));
			КонецЕсли;
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru = 'БИП:Новости.Сервис и регламент';
					|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
				НСтр("ru = 'Новости. Сервис и регламент. УстановитьСостояниеПодсистемы';
					|en = 'News. Service and regulation. УстановитьСостояниеПодсистемы'", КодОсновногоЯзыка), // ИдентификаторШага.
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		НСтр("ru = 'Веб-сервисы получения новостей';
			|en = 'Web services of news receipt'"));
	...
	Исключение
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При чтении файла с именем %1 произошла ошибка:
				|%2';
				|en = 'An error occurred when reading the file with name %1:
				|%2 '"),
			ПолноеИмяФайла,
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru = 'Обновление периодических свойств новостей';
												|en = 'Update periodic news properties'")); // Идентификатор.
		...
		Если НЕ Результат.Пустой() Тогда
			...
			ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Найдено %1 новостей для обновления периодических свойств';
					|en = '%1 news is found to update periodic properties'"),
				ВсегоСтрок);
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru = 'Обновление привязок к метаданным';
												|en = 'Update metadata links'")); // Идентификатор.
		...
		Если НЕ Результат.Пустой() Тогда
			...
			ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Найдено %1 новостей для обновления привязок к метаданным';
					|en = '%1 news is found to update metadata links'"),
				ВсегоСтрок);
			Пока Выборка.Следующий() Цикл
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_ВыполнениеЗапроса
	...
	РезультатЗапроса1 = ?(РезультатыЗапроса[1].Пустой(), НСтр("ru = 'пустой';
																|en = 'empty'"), НСтр("ru = 'есть данные';
																					|en = 'there is data'"));
	РезультатЗапроса2 = ?(РезультатыЗапроса[2].Пустой(), НСтр("ru = 'пустой';
																|en = 'empty'"), НСтр("ru = 'есть данные';
																					|en = 'there is data'"));
	Если ОптимизироватьОтборыПоНовостямОбщие = Истина Тогда
		РезультатЗапроса3 = ?(РезультатыЗапроса[3].Пустой(), НСтр("ru = 'пустой';
																	|en = 'empty'"), НСтр("ru = 'есть данные';
																						|en = 'there is data'"));
	Иначе
...
#Область Оптимизация_УдалениеЗначенийОтборов_ДляОбластиДанных
	...
	Иначе
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "", НСтр("ru = 'ОптимизироватьОтборыПоНовостямДляОбластиДанных - отключено';
															|en = 'ОптимизироватьОтборыПоНовостямДляОбластиДанных - disabled'"));
...
#Область Оптимизация_УдалениеЗначенийОтборов_Пользовательские
	...
	Иначе
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "", НСтр("ru = 'ОптимизироватьОтборыПоНовостямПользовательские - отключено';
															|en = 'ОптимизироватьОтборыПоНовостямПользовательские - disabled'"));
...
#Область Проверки
	...
	Если (НЕ ПривилегированныйРежим())
		...
		ТекстСообщения = НСтр("ru = 'Отсутствует право на изменение для РегистрыСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся.';
								|en = 'No rights to change РегистрыСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся.'");
	...
	Если (КатегорияВерсияПродукта.Пустая())
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'ПересчитатьОтборыПоНовостям_РедкоМеняющиеся, не заполнены значения плана видов характеристик КатегорииНовостей:
				...
				|en = 'ПересчитатьОтборыПоНовостям_РедкоМеняющиеся, values of the chart of characteristic types КатегорииНовостей are not populated:
				|КатегорияВерсияПродукта: %1
				|КатегорияВерсияПлатформы: %2
				|They might not been updated from the server yet upon classifiers update, or might not been populated in the СтандартныеЗначения template of chart of characteristic types КатегорииНовостей'"),
...
#Область ОсновныеДействия
	...
	Если НЕ Результат.Пустой() Тогда
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, 0, НСтр("ru = 'Редко-меняющиеся отборы обновлены';
															|en = 'Rarely-changing filters are updated'"));
	Иначе
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, 0,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Пересчет пользовательских отборов (%1 штук) (по текущему пользователю (%2)).';
					|en = 'User-defined filter recalculation (%1) for the current user (%2).'"),
				КоличествоОтборов,
				ТекущийПользователь),
	...
	Иначе
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, 0,
			НСтр("ru = 'Пересчет пользовательских отборов (по всем пользователям)';
				|en = 'User-defined filter recalculation for all users'"),
			КонтекстВыполненияВложенный);
	КонецЕсли;
...
Процедура ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения, ПринудительнаяЗапись = Ложь) Экспорт
	...
	Если ПустаяСтрока(ИмяСобытияЛокальный) Тогда
		ИмяСобытияЛокальный = НСтр("ru = 'БИП:Новости.Сервис и регламент';
									|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка);
	КонецЕсли;
...
Функция Проверка_ДоступныНеразделенныеДанные(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если КонтекстВыполнения <> Неопределено Тогда
		НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru = 'Проверка:ДоступныНеразделенныеДанные';
													|en = 'Проверка:ДоступныНеразделенныеДанные'")); // Идентификатор.
	КонецЕсли;
			...
			ЗавершитьШагВыполнения(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При работе в модели сервиса доступ к неразделенным данным возможен только в неразделенном сеансе.
						...
						|en = 'When using SaaS mode, you can only access shared data in an shared session.
						|ОбщегоНазначения.РазделениеВключено() = %1
						|ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() = %2
						|ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() = %3'"),
...
Функция Проверка_ДоступныРазделенныеДанные(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если КонтекстВыполнения <> Неопределено Тогда
		НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru = 'Проверка:ДоступныРазделенныеДанные';
													|en = 'Check: Separated data is available'")); // Идентификатор.
	КонецЕсли;
	...
	Если КонтекстВыполнения <> Неопределено Тогда
			...
			ЗавершитьШагВыполнения(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Необходим доступ к разделенным данным.
						...
						|ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() = %2
						|ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() = %3';
						|en = 'Access to separated data is required.
						|ОбщегоНазначения.РазделениеВключено() = %1
						|ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() = %2
						|ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() = %3'"),
					ОбщегоНазначения.РазделениеВключено(),
		...
		Иначе
			ЗавершитьШагВыполнения(КонтекстВыполнения, "", НСтр("ru = 'Доступны разделенные данные';
																|en = 'Separated data is available'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ((ВРег(Протокол) <> ВРег("http")))
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Протокол получения новостей [%1] отличается от http и https.
				...
				|en = 'The [%1] protocol for news receipt is different from HTTP and HTTPS.
				|Connection with the news server was not established.'"),
			Протокол);
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка определения порта из имени сервера %1 (%2).';
					|en = 'An error occurred while determining a port from the %1 (%2) server name.'"),
				ИмяСервера,
				Протокол);
			...
			ЗавершитьШагВыполнения(КонтекстВыполнения, 0,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Попытка %1 / %2: Протокол %3, Имя сервера %4:%5. Соединение успешно.';
						|en = 'Attempt %1 / %2: Protocol %3, Server name %4:%5. Connected successfully.'"),
					ПопыткаСоединения,
			...
			ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.CONNECTION.NOT_CONNECTED",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Попытка %1 / %2: Протокол %3, Имя сервера %4:%5. Ошибка соединения по причине:
						|%6';
						|en = 'Attempt %1 / %2: Protocol %3, Server name %4:%5. Connection error due to:
						|%6'"),
					ПопыткаСоединения,
					НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения,
					Протокол,
...
Процедура СбросДатыПоследнейУспешнойЗагрузкиНовостей(ЛентыНовостей, ПомечатьНовостиНаУдаление = Ложь) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
			ТекстСообщения = НСтр(
				...
				|en = 'ОбработкаНовостейСлужебный.СбросДатыПоследнейУспешнойЗагрузкиНовостей: Must run in a shared session.'",
				КодОсновногоЯзыка);
	...
	Если ЛокальныеЛентыНовостей.Количество() = 0 Тогда
		ТекстСообщения = НСтр(
			...
			|en = 'ОбработкаНовостейСлужебный.СбросДатыПоследнейУспешнойЗагрузкиНовостей: Cannot generate a news feed list'",
			КодОсновногоЯзыка);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
#Область Шаблоны
	...
	Результат = Символы.ПС
		+ НСтр("ru = '# ОТЛАДОЧНАЯ ИНФОРМАЦИЯ О НОВОСТЯХ
			...
			|
			|----------';
			|en = '# DEBUG NEWS INFORMATION 
			|
			|Report date: **%ТекущаяУниверсальнаяДата%**
			|
			|The News catalog items with UIN **%УИННовости%** are found: %КоличествоНайденныхНовостей%
			|
			|----------
			|## NEWS DATA
			|%ДанныеОНовостях%
	...
	ШаблонРезультатаНовости = Символы.ПС
		+ НСтр("ru = '### Новость: %НомерПоПорядку% / %КоличествоНайденныхНовостей%
			...
			|%ПоставляемыеДанные%';
			|en = '### News: %НомерПоПорядку% / %КоличествоНайденныхНовостей%
			|
			|### News catalog item
			|%ЭлементСправочникаНовости%
			|
			|### Auxiliary information registers
			|##### РегистрСведений.КатегорииНовостейИнтервалыВерсий
			|%РегистрСведений.КатегорииНовостейИнтервалыВерсий%
			|
			|##### РегистрСведений.КатегорииНовостейПростые
			...
			|%РегистрСведений.ОтключенныеЛентыНовостей%
			|
			|##### РегистрСведений.ОтключенныеЛентыНовостейДляОбластиДанных (for data area) %РегистрСведений.ОтключенныеЛентыНовостейДляОбластиДанных%
			|
			|##### РегистрСведений.СостоянияНовостей
			|%РегистрСведений.СостоянияНовостей%
			|
			|#### Data in the main supplied data template
			|%ПоставляемыеДанные%'")
			+ Символы.ПС
			+ Символы.ПС;
	...
	ШаблонРезультатаПоставляемыеДанные = Символы.ПС
		+ НСтр("ru = '##### Поставляемые данные, идентификатор %ИдентификаторПоставляемыхДанных%
			...
			|en = '##### Supplied data, ID %ИдентификаторПоставляемыхДанных%
			|
			|Text with UIN (+-200 characters):
			|
			|%ТекстПоставляемыхДанных%'")
			+ Символы.ПС
			+ Символы.Таб
	...
	ШаблонРезультатаЛентыНовостей = Символы.ПС
		+ НСтр("ru = '### Лента новостей: %НомерПоПорядку% / %КоличествоНайденныхЛентНовостей%
			...
			|en = '### News feed: %НомерПоПорядку% / %КоличествоНайденныхЛентНовостей%
			|
			|#### The News feed catalog item
			|%ЭлементСправочникаЛентыНовостей%'")
			+ Символы.ПС
			+ Символы.ПС;
	...
	ШаблонРезультатаРегламентныеОперации = Символы.ПС
		+ НСтр("ru = '### РегистрСведений.ДатыЗагрузкиПоследнихНовостей
			...
			|%РегистрСведений.ДатыОбновленияСтандартныхСписковНовостей%';
			|en = '### РегистрСведений.ДатыЗагрузкиПоследнихНовостей
			|%РегистрСведений.ДатыЗагрузкиПоследнихНовостей%
			|
			|### РегистрСведений.ДатыОбновленияСтандартныхСписковНовостей
			|%РегистрСведений.ДатыОбновленияСтандартныхСписковНовостей%'")
	...
	ШаблонРезультатаПрограммноеОкружение = Символы.ПС
		+ НСтр("ru = '### Константы
			...
			|
			|### Расписание регламентного задания (неразделенное)
			|%РасписаниеРегламентногоЗадания%
			|
			|### Расписание регламентного задания (для области данных)
			|%РасписаниеРегламентногоЗаданияДляОбластиДанных%';
			|en = '### Константы
			|%Константы%
			|
			|### Другие параметры программного окружения
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(
					...
					НСтр("ru = 'В модели сервиса для администратора системы нельзя получить пользовательские настройки или отборы.';
						|en = 'Cannot receive user settings or filters for the system administrator in SaaS.'"));
			КонецЕсли;
...
#Область ЗаполнениеШаблонов_Новости_ПоставляемыеДанные
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(РезультатТекущейНовости, "%ПоставляемыеДанные%", Символы.Таб + НСтр("ru = 'Не найдено в поставляемых данных.';
																															|en = 'Not found in the supplied data'"));
			КонецЕсли;
...
#КонецОбласти
	...
	Иначе
		Результат = СтрЗаменить(Результат, "%ДанныеОНовостях%", НСтр("ru = 'Не найдено ни одной новости с таким UIN';
																	|en = 'No news with this UIN is found'"));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ЛентыНовостей
	...
	Иначе
		Результат = СтрЗаменить(Результат, "%ДанныеОЛентахНовостей%", НСтр("ru = 'Не найдено ни одной ленты новостей для новости с таким UIN';
																			|en = 'No news feed with this UIN is found'"));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_Константы
	...
	Иначе
		ПредставлениеНастройкиНовостей = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Настройки новостей необрабатываемого типа %1';
				|en = 'News settings of unprocessable type %1'"),
	...
	РезультатДанныеОПрограммномОкружении_Константы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Константа.НастройкиНовостей:
			...
			|en = 'Константа.НастройкиНовостей:
			|%1
			|РазрешенаРаботаСНовостями: %2
			|РазрешенаРаботаСНовостямиЧерезИнтернет: %3'"),
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_ДругиеПараметрыПрограммногоОкружения
	...
	ВременныйТекст = НСтр("ru = 'Строка подключения: %СтрокаСоединенияИнформационнойБазы%
		...
		|en = 'Connection string: %СтрокаСоединенияИнформационнойБазы%
		|ТекущийРежимЗапуска: %ТекущийРежимЗапуска%
		|This is a separated configuration: %ЭтоРазделеннаяКонфигурация%
		|Separation is enabled: %РазделениеВключено%
		|Separated data usage is available: %ДоступноИспользованиеРазделенныхДанных%
		|Session is started without separators: %СеансЗапущенБезРазделителей%
		|Configuration (metadata): %Метаданные_Имя%, %Метаданные_Версия%
		|Configuration (override): %Переопределение_Имя%, %Переопределение_Версия%
		|Platform: %Платформа_Версия%
		|Functional option ""Work with news is allowed"": %ОбработкаНовостейПовтИсп_РазрешенаРаботаСНовостями%
		|Functional option ""Work with news via the Internet is allowed"":%ОбработкаНовостейПовтИсп_РазрешенаРаботаСНовостямиЧерезИнтернет%
		|Current infobase user:
		|	Name: %ТекущийПользователь_Имя%
		|	UID: %ТекущийПользователь_УникальныйИдентификатор%
		|	Data separation:
		|		%ТекущийПользователь_РазделениеДанных%
		|Data area (transferred as a parameter): %ОбластьДанных%
		|Selected data area (used for calculations):%ВыбраннаяОбластьДанных%
		|Launch parameter /C:%ПараметрЗапуска%'")
		+ Символы.ПС;
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_ПараметрыСеанса
	...
	Иначе
		ПредставлениеПараметрыОкруженияБИП_Новости = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Параметры окружения подсистемы новостей необрабатываемого типа %1';
				|en = 'Participant parameters of the news subsystem of unprocessable type %1'"),
	...
	РезультатДанныеОПрограммномОкружении_ПараметрыСеанса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Параметры окружения подсистемы новостей:
			|%1';
			|en = 'News subsystem participant parameters:
			|%1'"),
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_ДанныеОКонфигурацииИзМетаданных
	...
	РезультатДанныеОПрограммномОкружении_ДанныеОКонфигурацииИзМетаданных = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Имя конфигурации: %1
			...
			|Данные компьютера: %7';
			|en = 'Configuration name: %1
			|Configuration version: %2
			|ISL subsystem version: %3
			|Platform version: %4
			|Operating system: %5
			|Viewer: %6
			|Computer data: %7'"),
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_РасписаниеРегламентногоЗадания
	...
	ТекущееРегламентноеЗадание = ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейНеразделенное("ОбработкаНовостейСлужебный.ПолучитьИнформациюОНовости");
	Если ТекущееРегламентноеЗадание = Неопределено Тогда
		ПредставлениеРасписания = НСтр("ru = 'Регламентное задание (неразделенное) не найдено';
										|en = 'Регламентное задание (неразделенное) не найдено'");
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_РасписаниеРегламентногоЗаданияДляОбластиДанных

	ТекущееРегламентноеЗадание = ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейДляОбластиДанных("ОбработкаНовостейСлужебный.ПолучитьИнформациюОНовости");
	Если ТекущееРегламентноеЗадание = Неопределено Тогда
		ПредставлениеРасписания = НСтр("ru = 'Регламентное задание (для области данных) не найдено';
										|en = 'Регламентное задание (для области данных) не найдено'");
		РегламентноеЗадание_Использование = Ложь;
		ПредставлениеРегламентноеЗадание_ПоследнееЗадание = НСтр("ru = 'Никогда не выполнялось';
																|en = 'Never run'");
	Иначе
	...
	РезультатДанныеОПрограммномОкружении_РасписаниеРегламентногоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Расписание регламентного задания (для области данных):
			|%1
			|Использование: %2
			|Последнее задание:
			|%3
			|Примечание:
			|В модели сервиса:
			| - расписание из предопределенного регламентного задания копируется в элементы справочника ОчередьЗаданий автоматически при обновлении ИБ;
			| - расписание из метаданных регламентного задания в предопределенное регламентное задание НЕ копируется автоматически.
			|В этом отчете выведено расписание предопределенного регламентного задания.';
			|en = 'Расписание регламентного задания (для области данных):
			|%1
			|Использование: %2
			|Последнее задание:
			|%3
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'ОбработкаНовостейСлужебный.НайтиНовости: произошла ошибка поиска новостей, строка поиска [%1] по причине:
						|%2';
						|en = 'ОбработкаНовостейСлужебный.НайтиНовости: an error occurred while searching for news, search string [%1] due to:
						|%2'"),
					СтруктураПараметровПоискаЭталон.СтрокаПоиска,
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru = 'БИП:Новости.Разное';
						|en = 'ISL:News.Miscellaneous'", КодОсновногоЯзыка), // ИмяСобытия.
					НСтр("ru = 'Новости. Разное. Ошибка полнотекстового поиска';
						|en = 'News. Miscellaneous. Full-text search error'", КодОсновногоЯзыка), // ИдентификаторШага.
...
#Область ИнициализацияМетода
	...
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru = 'БИП:Новости.Все обновления новостей';
															|en = 'ISL: News.All news updates'", КодОсновногоЯзыка);
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, Операция, "ПолучитьИОбработатьНовостиПоЛентамНовостей"); // Идентификатор.
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Массив лент новостей для получения новостей. Размер = %1. Состав: %2';
						|en = 'News feed array to receive news. Size = %1. Content: %2'"),
		...
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru = 'Нет лент новостей для регламентного обновления';
											|en = 'No news feeds for scheduled update'"));
		КонецЕсли;
...
#Область ИнициализацияМетода
	...
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
	КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru = 'БИП:Новости.Все обновления новостей для области данных';
															|en = 'OSL:News. All news updates for the data area'", КодОсновногоЯзыка);
...
#Область Обработка_ПолучитьСписокДанныхДляУдаления
	...
	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru = 'УдалениеНовостей_ПолучитьСписокДанныхДляУдаления';
												|en = 'УдалениеНовостей_ПолучитьСписокДанныхДляУдаления'")); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
		...
		ЛогШаговВыполнения.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Собраны данные для удаления:
					...
					|en = 'Data for deletion was assembled:
					|News (for deletion): %1
					|News (for setting a deletion mark): %2
					|КатегорииНовостейПростыеНеобработанные: %3
					|КатегорииНовостейИнтервалыВерсийНеобработанные: %4'"),
...
#Область Обработка_Удаление
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(КонтекстВыполнения, НСтр("ru = 'УдалениеНовостей_Удаление';
																						|en = 'УдалениеНовостей_Удаление'")); // Идентификатор.
	ЛогШаговВыполнения = Новый Массив;
	...
	ДобавитьШагВыполнения(КонтекстВыполнения, "Получение:РазрешенаРаботаСНовостямиЧерезИнтернет", "",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Массив лент новостей для обработки: %1
				...
				|en = 'News feed array for processing: %1
				|Work via the Internet is allowed: %2'"),
			ОбработкаНовостейКлиентСервер.ПредставлениеЗначения(МассивЛентНовостей, ";"),
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Запрос выполнен. Количество лент новостей, которые будут обработаны: %1';
					|en = 'The query is executed. The number of news feeds to process: %1'"),
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.QUERY.EMPTY",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'ПолучитьИОбработатьНовостиПоЛентамНовостей, не удалось определить список лент новостей
					...
					|en = 'ПолучитьИОбработатьНовостиПоЛентамНовостей, cannot define the news feed list
					|Parameter passed with the number of values: %1
					|Query execution result: Empty
					|The news feed list may be empty, or all news feeds may have the ЛокальнаяЛентаНовостей
					|flag set (which means that this news does not have to be imported),
					|or all news feeds may have been marked for deletion.'"),
				МассивЛентНовостей.Количество()));
	КонецЕсли;
...
#Область Обработка_ОбработкаЗапроса
	...
	Если ЕстьЛентыНовостей = Истина Тогда
		...
		ЗавершитьШагВыполнения(КонтекстВыполнения, "",
			НСтр("ru = 'Загрузка и парсинг новостей завершены.';
				|en = 'News are imported and parsed.'"), КонтекстВыполненияВложенный);
	...
	ЛогШаговВыполнения.Добавить(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'РазрешенаРаботаСНовостями: %1
				...
				|en = 'РазрешенаРаботаСНовостями: %1
				|РазрешенаРаботаСНовостямиЧерезИнтернет: %2'"),
...
#КонецОбласти
	...
	Если РазрешенаРаботаСНовостямиЧерезИнтернет Тогда
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Обновление классификаторов запущено.';
										|en = 'Update of classifiers has been started.'"));
		...
		ЛогШаговВыполнения.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Обновление классификаторов завершено. Результат: %1';
					|en = 'Classifiers update complete. Result: %1'"),
				Результат));
	Иначе
		ЛогШаговВыполнения.Добавить(
			НСтр("ru = 'ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, включен режим [Работа только с локальными новостями],
				...
				|en = 'ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, mode [Operations only with local news] is enabled,
				|that is why classifiers cannot be updated from 1C servers.
				|You can update the classifiers either from metadata (while updating the configuration), or manually.'"));
	КонецЕсли;
...
#Область Обработка_ОбновлениеРасписания
	...
	Если Результат = Истина Тогда
		...
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Классификаторы обновлены без ошибок/';
										|en = 'Classifiers are updated without errors/'"));
	Иначе
		...
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Были ошибки обновления классификаторов. Время следующего запуска изменено.';
										|en = 'Errors occurred while updating classifiers. The next launch time is changed.'"));
	КонецЕсли;
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Результаты[0] - непустой. ТаблицаКатегорийИЗначенийПростые.Количество() = %1';
						|en = 'Results[0] - not empty. ТаблицаКатегорийИЗначенийПростые.Количество() = %1'"),
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Результаты[1] - непустой. ТаблицаКатегорийИЗначенийИнтервалыВерсий.Количество() = %1';
						|en = 'Results[1] - not empty. ТаблицаКатегорийИЗначенийИнтервалыВерсий.Количество() = %1'"),
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Результаты[2] - непустой. ТаблицаДоступныхКатегорий.Количество() = %1';
						|en = 'Results[2] - not empty. AvailableCategoriesTable.Quantity() = %1'"),
				...
				ЛогШаговВыполнения.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Удалена запись из регистра сведений КатегорииНовостейПростыеНеобработанные с ключами:
							...
							| - ЛентаНовостей: %4
							| - УИННовости: %5';
							|en = 'Record is deleted from information register КатегорииНовостейПростыеНеобработанные with keys:
							| - КатегорияНовостейСтрока: %1
							| - ЗначениеКатегорииНовостейСтрока:%2
							| - Filter condition: %3
							| - ЛентаНовостей:%4
				...
				ЛогШаговВыполнения.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Удалена запись из регистра сведений КатегорииНовостейИнтервалыВерсийНеобработанные с ключами:
							...
							| - ЛентаНовостей: %5
							| - УИННовости: %6';
							|en = 'Record is deleted from information register КатегорииНовостейИнтервалыВерсийНеобработанные with keys:
							| - КатегорияНовостейСтрока: %1
							| - Продукт: %2
							| - ВерсияОТ: %3
							| - ВерсияДО: %4
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Перенесено %1 значений простых категорий';
						|en = '%1 values of simple categories transferred'"),
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Перенесено %1 значений категорий интервалов версий';
						|en = '%1 values of version interval categories transferred'"),
					ТаблицаЗаполненныхКатегорийИЗначений.Количество()));
		КонецЕсли;
		...
		Если НЕ Результат.Пустой() Тогда
			ЛогШаговВыполнения.Добавить(
				НСтр("ru = 'Есть необработанные простые категории, не привязанные к новостям:';
					|en = 'There are unprocessed simple categories not linked to news:'"));
			Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
			Пока Выборка.Следующий() Цикл
		...
		Иначе
			ЛогШаговВыполнения.Добавить(
				НСтр("ru = 'Нет необработанных простых категорий, не привязанных к новостям.';
					|en = 'There are no unprocessed simple categories that are not linked to the news.'"));
		КонецЕсли;
		...
		Если НЕ Результат.Пустой() Тогда
			ЛогШаговВыполнения.Добавить(
				НСтр("ru = 'Есть необработанные категории интервалов версий, не привязанные к новостям:';
					|en = 'There are unprocessed version interval categories not linked to news:'"));
			Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
			Пока Выборка.Следующий() Цикл
		...
		Иначе
			ЛогШаговВыполнения.Добавить(
				НСтр("ru = 'Нет необработанных категорий интервалов версий, не привязанных к новостям.';
					|en = 'There are no unprocessed version interval categories that are not linked to the news.'"));
		КонецЕсли;
		ТКонец = ТекущаяУниверсальнаяДатаВМиллисекундах();
		ЛогШаговВыполнения.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		Если НЕ Результат.Пустой() Тогда
			ЛогШаговВыполнения.Добавить(
				НСтр("ru = 'Есть необработанные категории интервалов версий, которые уже перенесены:';
					|en = 'There are unprocessed version interval categories which are already moved:'"));
			Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
			Пока Выборка.Следующий() Цикл
				...
				ЛогШаговВыполнения.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'УИН: %1, Категория %2, Продукт %3, %4..%5';
							|en = 'Payment ID: %1, Category %2, Product %3, %4..%5'"),
						Выборка.УИННовости,
		...
		Иначе
			ЛогШаговВыполнения.Добавить(
				НСтр("ru = 'Нет необработанных категорий интервалов версий, которые уже перенесены.';
					|en = 'No unprocessed version interval categories which are already moved.'"));
		КонецЕсли;
		ТКонец = ТекущаяУниверсальнаяДатаВМиллисекундах();
		ЛогШаговВыполнения.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		Если МенеджерЗаданийАктивен = Истина Тогда
			Если МассивВыполняемыхФоновыхЗаданий.Количество() = 0 Тогда
					...
					ФоновыеЗадания.Выполнить(
						...
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся в связи с изменением версии платформы с %1 на %2';
								|en = 'ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся due to platform version change from %1 to %2'"),
				...
				ЛогШаговВыполнения.Добавить(НСтр("ru = 'Редко меняющиеся отборы будут пересчитаны (запущено фоновое задание)';
												|en = 'Rarely changed filters will be recalculated (background job is started) '"));
			Иначе
				ЛогШаговВыполнения.Добавить(НСтр("ru = 'Редко меняющиеся отборы уже пересчитываются (запущено фоновое задание из другого сеанса)';
												|en = 'Rarely changed filters are already recalculated (background job from another session is started)'"));
			КонецЕсли;
		Иначе
			ЛогШаговВыполнения.Добавить(НСтр("ru = 'Редко меняющиеся отборы будут пересчитаны (явный запуск ПересчитатьОтборыПоНовостям_РедкоМеняющиеся())';
											|en = 'Rarely changed filters will be recalculated (explicit start ПересчитатьОтборыПоНовостям_РедкоМеняющиеся())'"));
			ПересчитатьОтборыПоНовостям_РедкоМеняющиеся(КонтекстВыполненияВложенный); // здесь сохранится последняя версия платформы
		КонецЕсли;
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Сохраненная версия платформы:';
										|en = 'Saved platform version:'") + " " + ЛокальнаяПоследняяВерсия);
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Текущая версия платформы:';
										|en = 'Current platform version:'") + " " + ЛокальнаяТекущаяВерсия);
	Иначе
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Сохраненная версия платформы:';
										|en = 'Saved platform version:'") + " " + ЛокальнаяПоследняяВерсия);
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Текущая версия платформы:';
		...
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Редко меняющиеся отборы останутся как есть';
										|en = 'Rarely changed filters will remain as is'"));
	КонецЕсли;
		...
		Пока Выборка.Следующий() Цикл
			...
			ЛогШаговВыполнения.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Лента новостей %1, частота обновления ленты новостей: %2, дата последней успешной загрузки: %3, неудач: %4, последняя попытка загрузки: %5. %6';
					|en = 'News feed %1, news feed update frequency: %2 last successful import date: %3, failures: %4, last import attempt: %5. %6'"),
				Выборка.ЛентаНовостей,
				...
				Если СтруктураНовости.Ссылка.Пустая() Тогда
					...
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Действие';
						...
						+ НСтр("ru = 'Лента новостей активна, новости о неактивности нет';
								|en = 'News feed is active, no news about inactivity'"));
				ИначеЕсли СтруктураНовости.ПометкаУдаления = Истина Тогда
					...
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Действие';
						...
						+ НСтр("ru = 'Лента новостей активна, новость о неактивности уже была помечена на удаление';
								|en = 'News feed is active, news about inactivity was marked for deletion'"));
				Иначе
					...
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Действие';
						...
						+ НСтр("ru = 'Лента новостей активна, новость о неактивности будет помечена на удаление';
								|en = 'News feed is active, news about inactivity will be marked for deletion'"));
				КонецЕсли;
			Иначе
				...
				Если СтруктураНовости.Ссылка.Пустая() Тогда
					...
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Действие';
						...
						+ НСтр("ru = 'Лента новостей НЕактивна, новость о неактивности будет создана';
								|en = 'News feed in NOT active, news about inactivity will be created'"));
				Иначе
					НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Действие';
													|en = 'Action'") + ": "
						+ НСтр("ru = 'Лента новостей НЕактивна, новость о неактивности будет обновлена';
								|en = 'News feed in NOT active, news about inactivity will be updated'"));
				КонецЕсли;
				...
				НовостьОбъект.Наименование                       = НСтр("ru = 'Лента новостей неактивна';
																		|en = 'News feed is not active'");
				НовостьОбъект.Подзаголовок                       = НСтр("ru = 'Лента новостей неактивна. Рекомендуется исключить ее из автообновления';
																		|en = 'News feed is not active. It is recommended that you remove it from auto update'");
					...
					Исключение
						...
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Произошла ошибка при создании / обновлении новости о неактивности ленты новостей %1
								|Подробно:
								|%2';
								|en = 'An error occurred when creating / updating the news about inactivity of the %1 news feed.
								|Details: 
								|%2'"),
							Выборка.ЛентаНовостей,
							ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				...
				Иначе
					ЛогШаговВыполнения.Добавить(НСтр("ru = 'Отменено создание / изменение новости про отсутствие активности для ленты новостей';
													|en = 'Generation / change of the news about lack of activity for the news feed is canceled'"));
				КонецЕсли;
			КонецЕсли;
	...
	ЗавершитьШагВыполнения(КонтекстВыполнения, "",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Окончание проверки лент новостей на актуальность.
				...
				|en = 'End of news feed relevance check.
				|News feed is considered IRrelevant if
				| - There were more than 10 unsuccessful attempts to receive news.
				| - The last successful attempt was more than 24 hours ago.
				| - This is not a required news feed.
				| - This is not a news feed imported from 1C servers.
				|Query execution result:%1'"),
			?(Результат.Пустой(), НСтр("ru = 'Запрос выполнен.';
										|en = 'The query is executed.'"), НСтр("ru = 'Запрос пустой.';
																		|en = 'Request is empty.'"))),
			КонтекстВыполненияВложенный);
...
#Область Проверки
	...
	Если ТипЗнч(Использовать) <> ТипБулево Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'ОбработкаНовостейСлужебный.ИзменитьИспользованиеРегламентныхЗаданий:
				...
				|en = 'ОбработкаНовостейСлужебный.ИзменитьИспользованиеРегламентныхЗаданий:
				|value of an incorrect type is passed in the Use parameter: %1'"),
			ТипЗнч(Использовать));
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';
				|en = 'News. Service and regulation. ИзменениеИспользованияРегламентныхЗаданий'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
	...
	// Регламентное задание (неразделенное)
	РегламентноеЗаданиеНеразделенное = ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейНеразделенное("ОбработкаНовостейСлужебный.ИзменитьИспользованиеРегламентныхЗаданий");
	Если РегламентноеЗаданиеНеразделенное = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Не найдено предопределенное регламентное задание ВсеОбновленияНовостей.';
								|en = 'Predefined scheduled job ВсеОбновленияНовостей was not found.'");
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';
	...
	// Регламентное задание (для области данных)
	РегламентноеЗаданиеДляОбластиДанных = ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейДляОбластиДанных("ОбработкаНовостейСлужебный.ИзменитьИспользованиеРегламентныхЗаданий");
	Если РегламентноеЗаданиеДляОбластиДанных = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Не найдено предопределенное регламентное задание ВсеОбновленияНовостейДляОбластиДанных.';
								|en = 'Не найдено предопределенное регламентное задание ВсеОбновленияНовостейДляОбластиДанных.'");
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';
				|en = 'News. Service and regulation. ИзменениеИспользованияРегламентныхЗаданий'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостейДляОбластиДанных, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		Возврат;
...
#Область Обработка
	...
	РегламентноеЗаданиеНеразделенное.Использование    = Использовать;
	РегламентноеЗаданиеНеразделенное.Наименование     = НСтр("ru = 'Все обновления новостей';
															|en = 'Все обновления новостей'");
	РегламентноеЗаданиеНеразделенное.Расписание       = ПолучитьРасписаниеОбновленийНовостейНеразделенное(Истина);
	РегламентноеЗаданиеНеразделенное.Записать();

	РегламентноеЗаданиеДляОбластиДанных.Использование = Использовать;
	РегламентноеЗаданиеДляОбластиДанных.Наименование  = НСтр("ru = 'Все обновления новостей (для области данных)';
															|en = 'Все обновления новостей (для области данных)'");
	РегламентноеЗаданиеДляОбластиДанных.Расписание    = ПолучитьРасписаниеОбновленийНовостейДляОбластиДанных();
	РегламентноеЗаданиеДляОбластиДанных.Записать();
...
//
Функция ПолучитьРасписаниеОбновленийНовостейНеразделенное(АвтоопределениеЧастотыЗапуска = Ложь)
	...
	Иначе
		...
		РезультатЗапроса = Запрос.Выполнить(); // ПолучитьРасписаниеОбновленийНовостейНеразделенное.
		Если НЕ РезультатЗапроса.Пустой() Тогда
			Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
			Если Выборка.Следующий() Тогда
...
//
Функция ПолучитьРасписаниеОбновленийНовостейДляОбластиДанных()

	ИнтервалЗапускаСекунд = 30*60; // 1 раз в 30 минут.

	// В конфигурации есть общие реквизиты с разделением и включена ФО РаботаВМоделиСервиса.
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ИнтервалЗапускаСекунд = 6*60*60; // 1 раз в 6 часов.
	КонецЕсли;
...
//
Функция ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейНеразделенное(ИсточникВызова)
	...
	Попытка
		...
		ТекущееРегламентноеЗадание = РегламентныеЗаданияСервер.ПолучитьРегламентноеЗадание(
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей);
	Исключение
		ЕстьОшибка = Истина;
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не найдено предопределенное регламентное задание ВсеОбновленияНовостей по причине:
				...
				|
				|Источник вызова: %2';
				|en = 'Predefined scheduled job ВсеОбновленияНовостей is not found due to:
				|%1
				|
				|Call source: %2'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке),
		...
		Если ТипЗнч(ТекущееРегламентноеЗадание) <> ТипРегламентноеЗадание Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не найдено предопределенное регламентное задание ВсеОбновленияНовостей.
					|Источник вызова: %1';
					|en = 'Predefined scheduled job ВсеОбновленияНовостей is not found.
					|Call source: %1'"),
				ИсточникВызова);
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьОшибка = Истина Тогда
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейНеразделенное';
				|en = 'Новости. Сервис и регламент. ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейНеразделенное'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
//   РегламентноеЗадание - регламентное задание "ВсеОбновленияНовостей", сохраненное в информационной базе.
//
Функция ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейДляОбластиДанных(ИсточникВызова)
	...
	Попытка
		...
		//  использует механизм очередей, а если это не так (как в нашем случае), то приводит к исключению.
		ТекущееРегламентноеЗадание = РегламентныеЗаданияСервер.ПолучитьРегламентноеЗадание(
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостейДляОбластиДанных);
	Исключение
		ЕстьОшибка = Истина;
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не найдено предопределенное регламентное задание ВсеОбновленияНовостейДляОбластиДанных по причине:
				|%1
				|
				|Источник вызова: %2';
				|en = 'Не найдено предопределенное регламентное задание ВсеОбновленияНовостейДляОбластиДанных по причине:
				|%1
				|
				|Источник вызова: %2'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке),
	...
	Если ЕстьОшибка = Ложь Тогда
		Если ТипЗнч(ТекущееРегламентноеЗадание) <> ТипРегламентноеЗадание Тогда
			...
			ЕстьОшибка = Истина;
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не найдено предопределенное регламентное задание ВсеОбновленияНовостейДляОбластиДанных.
					|Источник вызова: %1';
					|en = 'Не найдено предопределенное регламентное задание ВсеОбновленияНовостейДляОбластиДанных.
					|Источник вызова: %1'"),
				ИсточникВызова);
	...
	Если ЕстьОшибка = Истина Тогда
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'БИП:Новости.Сервис и регламент'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейДляОбластиДанных';
				|en = 'Новости. Сервис и регламент. ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейДляОбластиДанных'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостейДляОбластиДанных, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Найдено %1 новостей для сброса признака [ТребуетсяПересчетОтборов]';
				|en = 'Found %1 news to reset [ТребуетсяПересчетОтборов] flag'"),
			ВсегоСтрок);
		Пока Выборка.Следующий() Цикл
...
#Область Обработка_УдалитьНовости
	...
	Если (ТипЗнч(ДанныеДляУдаленияЭталон.НовостиДляУдаления) = ТипМассив)
		...
		ТекстСообщения = СтрЗаменить(
			НСтр("ru = 'Есть помеченные на удаление новости (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% новостей).
				...
				|en = 'Some news are marked for deletion (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% of news).
				|You should delete them manually in All functions / Standard / Deletion of marked objects.'"),
			"%ДанныеДляУдаления.НовостиДляУдаления.Количество()%",
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'Нет помеченных на удаление новостей';
								|en = 'No news marked for deletion'");
	КонецЕсли;
		...
		Для Каждого РегистрДляОчистки Из МассивРегистровДляОчистки Цикл
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Очистка регистра сведений [%1] (%2 мс).';
					|en = 'Clearing information register [%1] (%2 ms).'"),
				РегистрДляОчистки,
				(Т2-Т1));
		...
		Для Каждого РегистрДляОчистки Из МассивРегистровДляОчистки Цикл
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Очистка регистра сведений [%1] не проводилась - нет новостей для удаления.';
					|en = 'Clearing information register [%1] was not performed - no news to delete.'"),
				РегистрДляОчистки);
			ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%" + РегистрДляОчистки + "_НовостиДляУдаления%", ТекстСообщения);
		КонецЦикла;
	КонецЕсли;
...
#Область Обработка_ПометкаНовостейНаУдаление
	...
	Если (ТипЗнч(ДанныеДляУдаленияЭталон.НовостиДляУстановкиПометкиУдаления) = ТипМассив)
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Все устаревшие новости успешно помечены на удаление (%1 новостей).';
				|en = 'All obsolete news are successfully marked for deletion (%1 новостей).'"),
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'Нет устаревших новостей, которые надо пометить на удаление.';
								|en = 'No old news to mark for deletion.'");
	КонецЕсли;
...
#Область Обработка_Очистка_КатегорииНовостейПростыеНеобработанные
	...
	Если (ТипЗнч(ДанныеДляУдаленияЭталон.КатегорииНовостейПростыеНеобработанные) = ТипМассив)
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Было удалено %1 необработанных простых категорий (%2 мс).';
				|en = '%1 unprocessed simple categories were deleted (%2 ms).'"),
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'Простых необработанных категорий не было найдено.';
								|en = 'No simple unprocessed categories were found.'");
	КонецЕсли;
...
#Область Обработка_Очистка_КатегорииНовостейИнтервалыВерсийНеобработанные
	...
	Если (ТипЗнч(ДанныеДляУдаленияЭталон.КатегорииНовостейИнтервалыВерсийНеобработанные) = ТипМассив)
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Было удалено %1 необработанных категорий интервалов версий (%2 мс).';
				|en = '%1 of unprocessed version interval categories were deleted (%2 ms).'"),
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'Необработанных категорий интервалов версий не было найдено.';
								|en = 'No unprocessed version interval categories.'");
	КонецЕсли;
		...
		Для Каждого РегистрДляОчистки Из МассивРегистровДляОчистки Цикл
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Очистка регистра сведений [%1] не проводилась - нет новостей для пометки удаления.';
					|en = 'Clearing information register [%1] was not performed - no news to mark for deletion.'"),
				РегистрДляОчистки);
			ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%" + РегистрДляОчистки + "_НовостиДляУстановкиПометкиУдаления%", ТекстСообщения);
		КонецЦикла;
	КонецЕсли;
...
#Область ИнициализацияМетода
	...
	КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонтекстВыполнения.ЖурналРегистрации.ИмяСобытия = НСтр("ru = 'БИП:Новости.Обновление ИБ';
															|en = 'ISL:News.IB update'", КодОсновногоЯзыка);
	...
	Если НЕ ПустаяСтрока(Результат) Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Обнаружены ошибки внедрения подсистемы ИнтернетПоддержкаПользователей.Новости.
				...
				|%1';
				|en = 'Errors occurred while implementing the ИнтернетПоддержкаПользователей.Новости subsystem.
...
#Область ОсновныеДействия_УстановкаРасписания_Неразделенное
	НачатьШагВыполнения(КонтекстВыполнения, "Заполнение:РасписаниеНеразделенное"); // Идентификатор.
	РегламентноеЗаданиеНеразделенное = ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейНеразделенное("ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск");
	Если РегламентноеЗаданиеНеразделенное <> Неопределено Тогда
		РегламентноеЗаданиеНеразделенное.Наименование = НСтр("ru = 'Все обновления новостей';
															|en = 'All news updates'");
		РегламентноеЗаданиеНеразделенное.Расписание   = ПолучитьРасписаниеОбновленийНовостейНеразделенное(Ложь);
		РегламентноеЗаданиеНеразделенное.Записать();
		ЗавершитьШагВыполнения(КонтекстВыполнения, 0, НСтр("ru = 'Успешная установка расписания регламентного задания (неразделенного).';
															|en = 'Успешная установка расписания регламентного задания (неразделенного).'"));
	Иначе
		ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.SCHEDULED_JOB.ABSENT/UPDATE", НСтр("ru = 'Регламентное задание ВсеОбновленияНовостей не найдено.';
																							|en = 'The ВсеОбновленияНовостей scheduled job was not found.'"));
	КонецЕсли;
#КонецОбласти
#Область ОсновныеДействия_УстановкаРасписания_ДляОбластиДанных
	НачатьШагВыполнения(КонтекстВыполнения, "Заполнение:РасписаниеДляОбластиДанных"); // Идентификатор.
	РегламентноеЗаданиеДляОбластиДанных = ПолучитьРегламентноеЗаданиеВсеОбновленияНовостейДляОбластиДанных("ОбработкаНовостейСлужебный.ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск");
	Если РегламентноеЗаданиеДляОбластиДанных <> Неопределено Тогда
		// Установка расписания для этого регламентного задания в модели сервиса означает лишь то,
		//  что это расписание будет использовано для заполнения элемента справочника ШаблоныЗаданийОчереди.
		РегламентноеЗаданиеДляОбластиДанных.Наименование = НСтр("ru = 'Все обновления новостей (для области данных)';
																|en = 'Все обновления новостей (для области данных)'");
		РегламентноеЗаданиеДляОбластиДанных.Расписание   = ПолучитьРасписаниеОбновленийНовостейДляОбластиДанных();
		РегламентноеЗаданиеДляОбластиДанных.Записать();
		ЗавершитьШагВыполнения(КонтекстВыполнения, 0, НСтр("ru = 'Успешная установка расписания регламентного задания (для области данных).';
															|en = 'Успешная установка расписания регламентного задания (для области данных).'"));
	Иначе
		ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.SCHEDULED_JOB.ABSENT/UPDATE", НСтр("ru = 'Регламентное задание ВсеОбновленияНовостейДляОбластиДанных не найдено.';
																							|en = 'Регламентное задание ВсеОбновленияНовостейДляОбластиДанных не найдено.'"));
	КонецЕсли;
#КонецОбласти
#КонецОбласти
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
		...
		Иначе
			ЗавершитьШагВыполнения(КонтекстВыполнения, "WARN.SUBSYSTEM.OFF/UPDATE",
				НСтр("ru = 'Работа с новостями отключена. Обработчик пропущен.';
					|en = 'Work with news is disabled. Handler is skipped.'"), КонтекстВыполненияВложенный);
		КонецЕсли;
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Запуск:" + НаименованиеМетода
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Переход на версию %1 (общие данные)';
				|en = 'Migration to version %1 (common data)'"),
...
#КонецОбласти
	...
	Иначе
		ОписаниеРезультата = НСтр("ru = 'Работа с новостями отключена.';
									|en = 'Work with news is disabled.'") + " ";
		// Записать об отключении подсистемы в журнал регистрации.
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Обновление ИБ';
				|en = 'ISL:News.IB update'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru = 'Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию';
				|en = 'News. Infobase Update. ОбщиеДанные. ПерейтиНаВерсию'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			0, // Данные
			НСтр("ru = 'Работа с новостями отключена. Новости, включая важные, не будут поступать.';
				|en = 'Operations with news are disabled. You will not receive news, including important ones.'"), // Комментарий
			Истина); // Записывать всегда.
	КонецЕсли;

	// В зависимости от текущего значения константы отключить или включить
	//  использование регламентных заданий "ВсеОбновленияНовостей" и "ВсеОбновленияНовостейДляОбластиДанных".
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Запуск:" + НаименованиеМетода
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Переход на версию %1 (разделенные данные)';
				|en = 'Migration to version %1 (separated data)'"),
			ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки())); // Идентификатор.
	...
	Если ВариантПроверки = "Все" Тогда
		...
		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru = 'Ошибки настройки общего реквизита ОбластьДанныхОсновныеДанные:';
									|en = 'Errors occurred while setting up the ОбластьДанныхОсновныеДанные common attribute:'"));
		...
		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru = 'Ошибки настройки общего реквизита ОбластьДанныхВспомогательныеДанные:';
									|en = 'Errors occurred while setting up the ОбластьДанныхВспомогательныеДанные common attribute:'"));
			Для Каждого ТекущаяСтрока Из РезультатЛокальный Цикл
				Результат.Добавить(ТекущаяСтрока);
			КонецЦикла;
		КонецЕсли;
	...
	Если ВариантПроверки = "Все" Тогда
		...
		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru = 'Ошибки настройки прав для роли АдминистраторСистемы:';
									|en = 'Errors occurred while setting up rights for the АдминистраторСистемы role:'"));
			Для Каждого ТекущаяСтрока Из РезультатЛокальный Цикл
				Результат.Добавить(ТекущаяСтрока);
			КонецЦикла;
		КонецЕсли;
		...
		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru = 'Ошибки настройки прав для роли ПолныеПрава:';
									|en = 'Errors occurred while setting up rights for the ПолныеПрава role:'"));
			Для Каждого ТекущаяСтрока Из РезультатЛокальный Цикл
				Результат.Добавить(ТекущаяСтрока);
			КонецЦикла;
		КонецЕсли;
	...
	Если (ВариантПроверки = "Все") ИЛИ (ВариантПроверки = "ТолькоПереопределения") Тогда
		...
		Если ВРег(ИмяКонфигурации) = ВРег("Unknown") Тогда
			РезультатЛокальный.Добавить(НСтр("ru = 'Не настроен идентификатор конфигурации.
				...
				|en = 'Configuration ID is not set up.
				|To set it up, change
				|the ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории
				|common module, for the ProductName condition line.
				|Otherwise, news for this configuration will NOT be received.'"));
		КонецЕсли;
		...
		Если СписокЗапрещенныхСимволов.Количество() > 0 Тогда
			РезультатЛокальный.Добавить(НСтр("ru = 'Неправильно настроен идентификатор конфигурации.
				...
				|en = 'Configuration ID is configured incorrectly.
				|To configure it, change the
				|ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории common module,
				| for the ProductName condition line. 
				|Configuration ID should contain English letters, digits, a minus, and an underscore.'"));
		КонецЕсли;

		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru = 'Ошибки настройки кода конфигурации:';
									|en = 'Errors of configuration code setting:'"));
			Для Каждого ТекущаяСтрока Из РезультатЛокальный Цикл
				Результат.Добавить(ТекущаяСтрока);
			КонецЦикла;
		КонецЕсли;
	...
	Если (ВариантПроверки = "Все") ИЛИ (ВариантПроверки = "ТолькоПереопределения") Тогда
		...
		Если МассивСлов.Количество() <> 4 Тогда
			РезультатЛокальный.Добавить(НСтр("ru = 'Неправильно настроена версия конфигурации (количество чисел отлично от 4).
				...
				|en = 'Configuration version is set up incorrectly (the number of digits differs from 4).
				|In metadata, in the Version field, there should be data of the A.B.C.D kind (four digits separated by dots).
				|If for some reasons the version in metadata is specified in the format which differs from the A.B.C.D format,
				|it is required to make changes to common module
				|ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,
				|for the ProductVersion condition line.'"));
		Иначе
				...
				РезультатЛокальный.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Неправильно настроена версия конфигурации (есть неправильные символы).
							...
							|en = 'Configuration version is set up incorrectly (there are incorrect characters).
							|In metadata, in the Version field, there should be data of the A.B.C.D kind (four digits separated by dots).
							|If for some reasons the version in metadata is specified in the format which differs from the A.B.C.D format,
							|it is required to make changes to common module
							|ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,
							|for the ProductVersion condition line.
							|List of incorrect characters: %1'"),
						СНайденныеНеправильныеСимволы));
			КонецЕсли;
		КонецЕсли;
		...
		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru = 'Ошибки настройки версии конфигурации:';
									|en = 'Errors of configuration version setting:'"));
			Для Каждого ТекущаяСтрока Из РезультатЛокальный Цикл
				Результат.Добавить(ТекущаяСтрока);
			КонецЦикла;
		КонецЕсли;
	...
	Если ВариантПроверки = "Все" Тогда
		...
		Если РезультатЛокальный.Количество() > 0 Тогда
			Результат.Добавить(НСтр("ru = 'Ошибки настройки истории данных:';
									|en = 'Data history customization errors:'"));
			Для Каждого ТекущаяСтрока Из РезультатЛокальный Цикл
				Результат.Добавить(ТекущаяСтрока);
			КонецЦикла;
		КонецЕсли;
...
Процедура Проверка_ОбщиеРеквизиты(
		...
		Иначе
			ТекущийРезультат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неправильно установлено использование для %1 в общем реквизите %2.
					...
					|en = 'Use is specified incorrectly for %1 in common attribute %2.
					|Specified: %3, expected: %4.
					|It is also possible that Autouse is set for %1, but then Autouse should be set in common attribute %2 in %4.'"),
	...
	Иначе
		ТекущийРезультат = НСтр("ru = 'Не найден требуемый объект метаданных: %ПолноеИмяМетаданных%';
								|en = 'Required metadata object was not found: %ПолноеИмяМетаданных%'");
...
Процедура Проверка_Права(
	...
	Если ТекущееЗначениеПрава <> ПравильноеЗначениеПрава Тогда
		ТекущийРезультат = НСтр("ru = 'Неправильно установлено право %Право% в роли %Роль% для %ПолноеИмяМетаданных%. Установлено: %ТекущееЗначениеПрава%, ожидается: %ПравильноеЗначениеПрава%';
								|en = 'Right %Право% is specified incorrectly for role %Роль% for %ПолноеИмяМетаданных%. Set: %ТекущееЗначениеПрава%, expected: %ПравильноеЗначениеПрава%'");
		ТекущийРезультат = СтрЗаменить(
			ТекущийРезультат,
			"%ПолноеИмяМетаданных%",
			ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(ЛокальныйОбъектМетаданных) + "." + ЛокальныйОбъектМетаданных.Имя);
	...
	Если ЛокальныйОбъектМетаданных.ИсторияДанных = ИсторияДанныхВключена Тогда
		Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'У объекта метаданных [%1] включена работа с историей данных. Должна быть отключена.';
				|en = 'The [%1] metadata object has data history operations enabled. Disable it.'"),
			ЛокальныйОбъектМетаданных));
	КонецЕсли;
	// 2. Права
		...
		Если ТекущееЗначениеПрава = Истина Тогда
			Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У объекта метаданных [%1] для роли [%2] включено право [%3]. Должно быть отключено.';
					|en = 'The [%1] metadata object has the [%3] right enabled for the [%2] role. Disable it.'"),
				ЛокальныйОбъектМетаданных,
				РольАС,
				ТекущееПраво));
		КонецЕсли;
					...
					Архив = Новый ЗаписьZipФайла(
						...
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Выгрузка журнала регистраций библиотеки БИП за период %1...%2';
								|en = 'Export of the ISL library event log for the period %1...%2'"),
				...
				Результат.Вставить("ТекстОшибки",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Файл с выгрузкой событий журнала регистрации не существует.
							...
							|en = 'File with export of event log events does not exist. 
							|You might not have events for the specified period (%1...%2).'"),
						Формат(ПараметрыОтбораПравильный.ДатаНачала, "ДЛФ=DT"),
						Формат(ПараметрыОтбораПравильный.ДатаОкончания, "ДЛФ=DT")));
			КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось экспортировать события журнала регистрации в файл по причине:
				|%1';
				|en = 'Cannot export events of the Event log to file due to:
				|%1'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ВыгрузитьВсеСобытияЖурналаРегистрации';
				|en = 'News. Service and regulation. ВыгрузитьВсеСобытияЖурналаРегистрации'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при получении xml-представления объекта ХДТО:
					|%1';
					|en = 'An error occurred while receiving xml presentation of XDTO object:
					|%1'"),
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru = 'БИП:БазовыйФункционал.Отладка';
					|en = 'БИП:БазовыйФункционал.Debug'", КодОсновногоЯзыка), // ИмяСобытия.
				НСтр("ru = 'БазовыйФункционал. Отладка. ПредставлениеОбъектаХДТО';
					|en = 'Core. Debug. XDTOObjectPresentation'", КодОсновногоЯзыка), // ИдентификаторШага.
...
Функция Проверка_РазрешенаРаботаСНовостями(КонтекстВыполнения)
	...
	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru = 'Проверка:РазрешенаРаботаСНовостями';
												|en = 'Проверка:РазрешенаРаботаСНовостями'")); // Идентификатор.
...
Функция Проверка_МонопольныйРежимОтключен(КонтекстВыполнения)
	...
	НачатьШагВыполнения(КонтекстВыполнения, НСтр("ru = 'Проверка:МонопольныйРежим';
												|en = 'Проверка:МонопольныйРежим'")); // Идентификатор.
	...
	Если Результат = Истина Тогда
		ЗавершитьШагВыполнения(КонтекстВыполнения, "ERR.EXTERNAL_USER_PROHIBITED",
			НСтр("ru = 'Работа с новостями не поддерживается для внешнего пользователя.';
				|en = 'Work with news is not supported for an external user.'"));
		КонтекстВыполнения.ЖурналРегистрации.Уровень = УровеньЖурналаРегистрации.Предупреждение; // Идентификатор.
		ЗаписатьКонтекстВыполненияВЖурналРегистрации(КонтекстВыполнения);
	Иначе
		ЗавершитьШагВыполнения(КонтекстВыполнения, "", НСтр("ru = 'Это обычный пользователь.';
															|en = 'This is a regular user.'"));
	КонецЕсли;
...
Функция КомментарийДляЖурналаРегистрации(ОписаниеШага, ШагВыполнения, КонтекстВыполнения, Вариант = "Простой") Экспорт
	...
	Если Вариант = "Расширенный" Тогда
		...
		ТекстСообщения = НСтр("ru = '%ОписаниеШага%
			...
			|%РезультатПоШагам%';
			|en = '%ОписаниеШага%
			|Connection string: %СтрокаСоединенияИнформационнойБазы%
			|ТекущийРежимЗапуска: %ТекущийРежимЗапуска%
			|This is a separated configuration: %ЭтоРазделеннаяКонфигурация%
			|Separation is enabled: %РазделениеВключено%
			|Separated data usage is available: %ДоступноИспользованиеРазделенныхДанных%
			...
			|	Data separation:
			|		%ТекущийПользователь_РазделениеДанных%
			|Start time (ms): %ВремяНачала%
			|End time (ms): %ВремяОкончания%
			|Duration (ms): %Длительность%
			|Result by steps:
			|%РезультатПоШагам%'")
			+ Символы.ПС;
...
#Область УдалениеНеправильныхКатегорий
	...
	Для Каждого ТекущаяКатегория Из НеправильныеКатегории Цикл
		...
		ЛогШаговВыполнения.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Найдена задвоенная категория с кодом [%1]';
					|en = 'Found duplicate category with code [%1]'", КодОсновногоЯзыка),
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Попытка записи категории новостей с повторяющимся кодом (замена кода и пометка удаления):
						|Новый код [%1] (%2)';
						|en = 'An attempt was made to write a news category with a duplicate code (code replacement and deletion mark):
						|New code [%1] (%2)'", КодОсновногоЯзыка),
					НовыйКод,
					ТекущаяКатегория.УникальныйИдентификатор()));
			Объект.ОбменДанными.Загрузка = Истина;
			ЛогШаговВыполнения.Добавить(НСтр("ru = 'Блокировка объекта.';
											|en = 'Object lock.'", КодОсновногоЯзыка));
			Объект.Заблокировать();
			ЛогШаговВыполнения.Добавить(НСтр("ru = 'Начало записи объекта.';
											|en = 'Object saving start.'", КодОсновногоЯзыка));
			Объект.Записать();
			ЛогШаговВыполнения.Добавить(НСтр("ru = 'Запись успешна.';
											|en = 'Saving successful.'", КодОсновногоЯзыка));
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка записи по причине:
						|%1';
						|en = 'Saving error. Reason:
						|%1'", КодОсновногоЯзыка),
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
		КонецПопытки;
		Если Объект.Заблокирован() Тогда // Разблокировать в любом случае - было исключение, или нет.
			ЛогШаговВыполнения.Добавить(НСтр("ru = 'Начало разблокировки объекта.';
											|en = 'Object unlock start.'", КодОсновногоЯзыка));
			Объект.Разблокировать();
		КонецЕсли;
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ЗадвоенныеКатегории. УдалениеНеправильныхКатегорий';
				|en = 'News. Service and regulations. ЗадвоенныеКатегории. УдалениеНеправильныхКатегорий'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
		...
		Пока Выборка.Следующий() Цикл
			...
			Для Каждого ТекущаяДоступнаяКатегория Из Объект.ДоступныеКатегорииНовостей Цикл
				Если НеправильныеКатегории.Найти(ТекущаяДоступнаяКатегория.КатегорияНовостей) <> Неопределено Тогда
					ЛогШаговВыполнения.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'В Ленте новостей [%1] категория [%2] (%3) будет заменена на [%4] (%5).';
								|en = 'The category [%2] (%3) will be replaced with [%4] (%5) in the News feed [%1] .'", КодОсновногоЯзыка),
							Выборка.ЛентаНовостей,
			...
			Попытка
				ЛогШаговВыполнения.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Попытка записи ленты новостей [%1].';
							|en = 'Attempting to save news feed [%1].'", КодОсновногоЯзыка),
						Выборка.ЛентаНовостей));
				Объект.ОбменДанными.Загрузка = Истина;
				Объект.Записать();
				ЛогШаговВыполнения.Добавить(НСтр("ru = 'Запись успешна.';
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru = 'БИП:Новости.Сервис и регламент';
					|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
				НСтр("ru = 'Новости. Сервис и регламент. ЗадвоенныеКатегории. ЗаменаНеправильныхКатегорий';
					|en = 'News. Service and regulations. ЗадвоенныеКатегории. ЗаменаНеправильныхКатегорий'", КодОсновногоЯзыка), // ИдентификаторШага.
				УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
...
#Область ЗаменаНеправильныхКатегорийВДанных_КатегорииНовостейПростые
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Добавление правильных категорий';
										|en = 'Adding correct categories'", КодОсновногоЯзыка));
		Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
		Пока Выборка.Следующий() Цикл
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Замена категории [%1] (%2) в [%3] для новости [%4], [%5].';
						|en = 'Replacing category [%1] (%2) in [%3] for news [%4], [%5].'", КодОсновногоЯзыка),
		...
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Удаление неправильных категорий';
										|en = 'Removing incorrect categories'", КодОсновногоЯзыка));
...
#Область ЗаменаНеправильныхКатегорийВДанных_ЗначенияКатегорийДляОбластиДанных
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Замена категории [%1] (%2) в [%3].';
						|en = 'Replacing category [%1] (%2) in [%3].'", КодОсновногоЯзыка),
					Выборка.КатегорияНовостей,
					СокрЛП(Выборка.КатегорияНовостей.УникальныйИдентификатор()),
					ОбъектМетаданных));
		КонецЦикла;
...
#Область ЗаменаНеправильныхКатегорийВДанных_ОтборыПоЛентамНовостейДляОбластиДанных
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Замена категории [%1] (%2) в [%3] для ленты новостей [%4].';
						|en = 'Replacing category [%1] (%2) in [%3] for news feed [%4].'", КодОсновногоЯзыка),
					Выборка.КатегорияНовостей,
					СокрЛП(Выборка.КатегорияНовостей.УникальныйИдентификатор()),
					ОбъектМетаданных,
					Выборка.ЛентаНовостей));
...
#Область УдалениеНеправильныхЗначенийКатегорий
	...
	Для Каждого ТекущееЗначениеКатегории Из НеправильныеЗначенияКатегорий Цикл
		...
		ЛогШаговВыполнения.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Найдены дубликаты значений категории с кодом [%1]';
					|en = 'Duplicates of category values with code [%1] are found'", КодОсновногоЯзыка),
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Попытка записи значения категории новостей с повторяющимся кодом (замена кода и пометка удаления):
						|Новый код [%1] (%2)';
						|en = 'An attempt was made to write a news category value with a duplicate code (code replacement and deletion mark):
						|New code [%1] (%2)'", КодОсновногоЯзыка),
					НовыйКод,
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ЗадвоенныеЗначенияКатегорий. УдалениеНеправильныхЗначенийКатегорий';
				|en = 'News. Service and regulation. ЗадвоенныеЗначенияКатегорий. УдалениеНеправильныхЗначенийКатегорий'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
...
#Область ЗаменаНеправильныхЗначенийКатегорийВДанных_КатегорииНовостейПростые
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Добавление правильных значений категорий';
										|en = 'Adding correct category values'", КодОсновногоЯзыка));
		Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
		Пока Выборка.Следующий() Цикл
			Запись = РегистрыСведений[ОбъектМетаданных].СоздатьМенеджерЗаписи();
				Запись.Новость                   = Выборка.Новость;
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Замена значения категории [%1] (%2) в [%3] для новости [%4], [%5].';
						|en = 'Replacing category value [%1] (%2) in [%3] for news [%4], [%5].'", КодОсновногоЯзыка),
		...
		ЛогШаговВыполнения.Добавить(НСтр("ru = 'Удаление неправильных значений категорий';
										|en = 'Removing incorrect category values'", КодОсновногоЯзыка));
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. ЗадвоенныеЗначенияКатегорий. ЗаменаНеправильныхЗначенийКатегорий';
				|en = 'News. Service and regulation. ЗадвоенныеЗначенияКатегорий. ЗаменаНеправильныхЗначенийКатегорий'", КодОсновногоЯзыка), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений[ОбъектМетаданных], // ОбъектМетаданных
			, // Данные
			СтрСоединить(ЛогШаговВыполнения, Символы.ПС), // Комментарий
...
#Область ЗаменаНеправильныхЗначенийКатегорийВДанных_ЗначенияКатегорийДляОбластиДанных
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Замена значения категории [%1] (%2) в [%3].';
						|en = 'Replacing category value [%1] (%2) in [%3].'", КодОсновногоЯзыка),
					Выборка.ЗначениеКатегорииНовостей,
					СокрЛП(Выборка.ЗначениеКатегорииНовостей.УникальныйИдентификатор()),
					ОбъектМетаданных));
		КонецЦикла;
...
#Область ЗаменаНеправильныхЗначенийКатегорийВДанных_ОтборыПоЛентамНовостейДляОбластиДанных
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Замена значения категории [%1] (%2) в [%3] для ленты новостей [%4].';
						|en = 'Replacing category value [%1] (%2) in [%3] for news feed [%4].'", КодОсновногоЯзыка),
					Выборка.ЗначениеКатегорииНовостей,
					СокрЛП(Выборка.ЗначениеКатегорииНовостей.УникальныйИдентификатор()),
					ОбъектМетаданных,
					Выборка.ЛентаНовостей));
	...
	Если (Результат.ТекущееСостояние <> "Активна")
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'ОбработкаНовостейСлужебный.УстановитьСостояниеПодсистемы: ожидается состояние подсистемы:
				...
				|Передано значение: [%1].';
				|en = 'ОбработкаНовостейСлужебный.УстановитьСостояниеПодсистемы: expected subsystem status:
				| - Активна;
				| - ТребуетсяПересчетОтборовПослеДлительногоОжидания;
				| - СейчасЗагружаютсяНовости.
				|Passed value: [%1].'", КодОсновногоЯзыка),
			СостояниеПодсистемы);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
#Область Завершение
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Подсистема Новости переводится в состояние [%1].';
			|en = 'The News subsystem is switched to [%1] status.'"),
		СостояниеПодсистемы);
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru = 'БИП:Новости.Сервис и регламент';
			|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
...
Процедура ЗаполнитьЗначенияАвтоматическихОтборов_Общие(КонтекстВыполнения = Неопределено)
	...
	АвтоматическийРасчет = Истина; // Если ИСТИНА, то вызвать РассчитатьОтборПоКатегории (Возможен вход в область данных).
...
#Область МножественноеЗначениеОтбора
		...
		Запрос.Текст = "
			...
			|;
			|
			|// 3. Итоги по РегистрСведений.ЗначенияКатегорий, чтобы увидеть - были сохраненные значения по категории или нет.
			|ВЫБРАТЬ
			|	Рег.КатегорияНовостей                     КАК КатегорияНовостей,
			|	КОЛИЧЕСТВО(Рег.ЗначениеКатегорииНовостей) КАК ИтогоСохраненныхЗначений
			|ПОМЕСТИТЬ ВТ_СохраненныеЗначения_Итоги
			|ИЗ
			|	РегистрСведений.ЗначенияКатегорийОбщие КАК Рег
			|СГРУППИРОВАТЬ ПО
			|	Рег.КатегорияНовостей
			...
			|;
			|
			|// 4. Общий список лент новостей и значений категорий (для заполнения общего отбора)
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|	ВТ.ЛентаНовостей                     КАК ЛентаНовостей,
			|	ВЫБОР
			|		КОГДА ВТ_СохраненныеЗначения_Итоги.ИтогоСохраненныхЗначений ЕСТЬ NULL ТОГДА
			|			ЛОЖЬ
			|		КОГДА ВТ_СохраненныеЗначения_Итоги.ИтогоСохраненныхЗначений = 0 ТОГДА
			|			ЛОЖЬ
			|		ИНАЧЕ
			|			ИСТИНА
			...
			|
			|	ЛЕВОЕ СОЕДИНЕНИЕ
			|	ВТ_СохраненныеЗначения_Итоги
			|	ПО
			|		ВТ.КатегорияНовостей = ВТ_СохраненныеЗначения_Итоги.КатегорияНовостей
			|УПОРЯДОЧИТЬ ПО
			|	КатегорияНовостей,
						...
						Пока Выборка.СледующийПоЗначениюПоля("ЛентаНовостей") Цикл
							...
							Пока Выборка.Следующий() Цикл
								Если РезультатРасчета = Истина Тогда
									...
									КонецЕсли;
									НоваяЗапись.УстановленоПрограммно = Истина;
								Иначе
		...
		Запрос.Текст = "
			...
			Если ИспользоватьНомерОбластиДанных = Истина Тогда
				Запрос.Текст = Запрос.Текст + "
					...
					|	РегистрСведений.ЗначенияКатегорийДляОбластиДанных КАК Рег
					|СГРУППИРОВАТЬ ПО
					|	Рег.ОбластьДанныхВспомогательныеДанные,
					|	Рег.КатегорияНовостей
					|ИНДЕКСИРОВАТЬ ПО
					|	ОбластьДанныхВспомогательныеДанные,
					|	КатегорияНовостей
					|;
					|
					|// 4. Общий список лент новостей и значений категорий (для заполнения отбора области данных)
					|ВЫБРАТЬ РАЗЛИЧНЫЕ
					...
					|	ВТ_СохраненныеЗначения_Итоги
					|	ПО
					|		РегОбласти.ОбластьДанныхВспомогательныеДанные = ВТ_СохраненныеЗначения_Итоги.ОбластьДанныхВспомогательныеДанные
					|		И ВТ.КатегорияНовостей = ВТ_СохраненныеЗначения_Итоги.КатегорияНовостей
					|ГДЕ
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка получения значения функциональной опции %1 для категории %2.
					|Подробности:
					|%3';
					|en = 'An error occurred while receiving values of the functional option %1 for %2 category.
					|Details:
					|%3'"),
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru = 'БИП:Новости.Сервис и регламент';
					|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
				НСтр("ru = 'Новости. Сервис и регламент. РассчитатьОтборПоКатегории. Общие';
					|en = 'News. Service and regulation. РассчитатьОтборПоКатегории. Общие'", КодОсновногоЯзыка), // ИдентификаторШага.
				УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
				Метаданные.Справочники.Новости, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
		...
		Если ИспользоватьНомерОбластиДанных = Истина Тогда
			...
			Исключение
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru = 'БИП:Новости.Сервис и регламент';
						|en = 'ISL:News.Service and regulation'", КодОсновногоЯзыка), // ИмяСобытия.
					НСтр("ru = 'Новости. Сервис и регламент. РассчитатьОтборПоКатегории. ДляОбластиДанных';
						|en = 'News. Service and regulation. РассчитатьОтборПоКатегории. ДляОбластиДанных'", КодОсновногоЯзыка), // ИдентификаторШага.
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
