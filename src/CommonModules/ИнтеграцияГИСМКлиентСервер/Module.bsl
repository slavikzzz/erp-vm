...
Функция ПредставлениеПодписи(СертификатКриптографии, Сообщение, ВыводитьГиперссылки, СтруктураЦветаСтиля) Экспорт
	...
	Если СертификатКриптографии = Неопределено Тогда
		Подпись = Новый ФорматированнаяСтрока(НСтр("ru = 'Не подписано';
													|en = 'Не подписано'"),,СтруктураЦветаСтиля.ЦветТекстаТребуетВниманияГосИС);
		Возврат Подпись;
	КонецЕсли;
	...
	Если Сообщение.ПодписьВерна Тогда
		СтрокаПодписьВерна = Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ru = 'Подпись верна (Дата проверки: %1).';
																		|en = 'Подпись верна (Дата проверки: %1).'"), Формат(Сообщение.ДатаПроверкиПодписи, "ДЛФ=D")));
	Иначе
		Если ЗначениеЗаполнено(Сообщение.ДатаПроверкиПодписи) Тогда
			СтрокаПодписьВерна = Новый ФорматированнаяСтрока(
				СтрШаблон(НСтр("ru = 'Подпись не верна (Дата проверки: %1).';
								|en = 'Подпись не верна (Дата проверки: %1).'"), Формат(Сообщение.ДатаПроверкиПодписи, "ДЛФ=D")),,
				СтруктураЦветаСтиля.ЦветТекстаТребуетВниманияГосИС);
		Иначе
			СтрокаПодписьВерна = Новый ФорматированнаяСтрока(
				НСтр("ru = 'Проверка подписи не выполнялась.';
					|en = 'Проверка подписи не выполнялась.'"),,
				СтруктураЦветаСтиля.ЦветТекстаТребуетВниманияГосИС);
		КонецЕсли;
	КонецЕсли;
	
	СтрокаПроверить = Новый ФорматированнаяСтрока(НСтр("ru = 'Проверить';
														|en = 'Проверить'"),,СтруктураЦветаСтиля.ЦветГиперссылкиГосИС,,"ПроверитьПодпись");
			...
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
				...
				СтрШаблон(
					НСтр("ru = 'Не заполнен GLN организации %1 (подразделение %2).';
						|en = 'Не заполнен GLN организации %1 (подразделение %2).'"),
			...
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
				...
				СтрШаблон(
					НСтр("ru = 'Указан не корректный GLN организации %1 (подразделение %2).';
						|en = 'Указан не корректный GLN организации %1 (подразделение %2).'"),
					РеквизитыШапки.Организация, РеквизитыШапки.Подразделение));
		КонецЕсли;
	Иначе
		Если Не ЗначениеЗаполнено(GLN) Тогда
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
				СообщениеXML,
				СтрШаблон(НСтр("ru = 'Не заполнен GLN организации %1.';
								|en = 'Не заполнен GLN организации %1.'"), РеквизитыШапки.Организация));
		Иначе
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
				СообщениеXML,
				СтрШаблон(НСтр("ru = 'Указан не корректный GLN организации %1.';
								|en = 'Указан не корректный GLN организации %1.'"), РеквизитыШапки.Организация));
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьТекстОшибкиУказанНеКорректныйКППОрганизации(СообщениеXML, КПП, Организация) Экспорт
	...
	ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Указан не корректный КПП организации %1.';
																				|en = 'Указан не корректный КПП организации %1.'"), Организация));
...
Процедура ДобавитьТекстОшибкиНеЗаполненоПолноеНаименованиеКонтрагента(СообщениеXML, Контрагент) Экспорт
	...
	ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Не заполнено полное наименование контрагента %1.';
																				|en = 'Не заполнено полное наименование контрагента %1.'"), Контрагент));
...
Процедура ДобавитьТекстОшибкиНеЗаполненGLNПартнера(СообщениеXML, GLN, Партнер) Экспорт
	...
	Если Не ЗначениеЗаполнено(GLN) Тогда
		ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Не заполнен GLN партнера %1.';
																					|en = 'Не заполнен GLN партнера %1.'"), Партнер));
	Иначе
		ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Указан не корректный GLN партнера %1.';
																					|en = 'Указан не корректный GLN партнера %1.'"), Партнер));
	КонецЕсли;
...
// 
// Возвращаемое значение:
//  Булево - Необходим отбор по дальнейшему действию ГИСМПеред загрузкой из настроек
