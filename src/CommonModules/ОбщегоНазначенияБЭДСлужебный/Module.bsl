...
// Возвращает версию платформы 1С:Предприятия без номера сборки
//
// Возвращаемое значение:
//  Строка - Версия платформы без номера сборки в формате РР.ПП.ЗЗ.
//
Функция ВерсияПлатформыБезНомераСборки()
...
Процедура ДобавитьЗапросВПакет(Схема1, ЗапросСхемы2) Экспорт
	...
	// На платформе 8.3.25 ошибка при выполнении метода ЗаполнитьЗначенияСвойств из-за свойства "ТаблицаДляДобавления"
	// поэтому для платформ версий выше 24-ой временно добавляем свойство в исключение.
	// Проверка на платформу нужна для обратной совместимости. 
	
	ИсключаемыеПоля = 
		?(ОбщегоНазначенияКлиентСервер.СравнитьВерсииБезНомераСборки(ВерсияПлатформыБезНомераСборки(), "8.3.24") > 0
			И ТипЗнч(ЗапросСхемы2) <> Тип("ЗапросУничтоженияТаблицыСхемыЗапроса"),
				"ТаблицаДляДобавления", 
				""); 
	
	ЗаполнитьЗначенияСвойств(ЗапросСхемы1, ЗапросСхемы2,, ИсключаемыеПоля);
