...
Функция ПравоДоступаКРегламентированномуОтчету(ИдентификаторОтчета) Экспорт
	...
	ИначеЕсли Метаданные.Отчеты.Найти(ИдентификаторОтчета) <> Неопределено Тогда // это отчет
		Если НЕ ПравоДоступа("Использование", Метаданные.Отчеты[ИдентификаторОтчета]) Тогда
			Возврат Ложь;
		КонецЕсли;
	ИначеЕсли СтрНачинаетсяС(ИдентификаторОтчета, "EF_")
		И Метаданные.Отчеты.Найти(ИдентификаторОтчета) = Неопределено
		И Метаданные.Отчеты.Найти(СтрЗаменить(ИдентификаторОтчета, "EF_", "")) <> Неопределено Тогда
		
		Если НЕ ПравоДоступа("Использование", Метаданные.Отчеты[СтрЗаменить(ИдентификаторОтчета, "EF_", "")]) Тогда
			Возврат Ложь;
		КонецЕсли;
	Иначе
...
Функция ЗаполнитьСведенияОбОрганизацииНаСервере(Форма, Знач СписокСведений, Дополнения = "", РазделДляВывода = "Титульный") Экспорт
	...
	ОргСведения = ПолучитьСведенияОбОрганизации(Форма.СтруктураРеквизитовФормы.Организация, Форма.ДатаПодписи, СписокСведений);

	Результат = "";
	ПБОЮЛ = НЕ ЭтоЮридическоеЛицо(Форма.СтруктураРеквизитовФормы.Организация);
	...
	Если (Титульный.Области.Найти("ИННУп") <> Неопределено)
	 ИЛИ (Титульный.Области.Найти("ИННУпП") <> Неопределено) Тогда
		...
		ДанныеПредставителя = ПолучитьСведенияОПредставителе(Форма.СтруктураРеквизитовФормы.Организация, Форма.ДатаПодписи, Ложь, КодНО, КПП);
...
Функция ПолучитьСведенияОПредставителе(ФормаОрганизация, ФормаДатаПодписи, Знач МожетБытьТолькоЮРЛицо = Неопределено, Знач ИФНС = Неопределено, КПП = Неопределено) Экспорт
	...
	ДанныеКонтактнойИнформации = ПолучитьСведенияОбОрганизации(
		ФормаОрганизация, 
		ФормаДатаПодписи, 
		"ФИОУпПред"
		+ ",ИННУпПред"
	...
	Представитель = РегистрацияВНОСервер.Представитель(СправочникИФНС[0].Ссылка);
	...
	Если РегламентированнаяОтчетность.ПредставительЯвляетсяФизЛицом(Представитель) Тогда
		...
		Иначе
			...
			СформироватьАдрес(Результат[0].ЗначенияПолей, СтруктураАдреса);
			...
			ДанныеКонтактнойИнформации.Вставить("КодСубъектМЖУпПред", Формат(КодРегионаПоНазванию(СтруктураАдреса.Регион), "ЧЦ=2; ЧВН="));
...
Процедура ПолучитьСведения(ОргСведения, ПБОЮЛ, Организация, ДатаПодписи, СписокСведений, ДанныеПредставителя, КодФНС, КПП) Экспорт
	...
	ОргСведения = ПолучитьСведенияОбОрганизации(Организация, ДатаПодписи, СписокСведений);
	...
	ДанныеПредставителя = ПолучитьСведенияОПредставителе(Организация, ДатаПодписи, Ложь, КодНО, КПП);
...
Функция КПППоКомбинацииОрганизацииИКодаИФНС(Организация, КодИФНС) Экспорт
	...
	Иначе
		...
		СведенияОбОрганизации = ПолучитьСведенияОбОрганизации(Организация, ТекущаяДатаСеанса(), "КППЮЛ");
		...
		Иначе
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Невозможно определить КПП для организации %1 и ИФНС с кодом %2!';
																							|en = 'Cannot determine CRTR for the %1 company and IFTS with the %2 code.'"), Организация, КодИФНС);
...
Процедура ПолучитьШаблонПоИмени(Выгрузка) Экспорт
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'ошибка при получении шаблона из регистра';
													|en = 'error when receiving template from register'"));
	КонецПопытки;
КонецПроцедуры
...
Функция ПолучитьШаблоныМашиночитаемыхФорм(Знач ИмяОтчета, ПрефиксИмениМакетаШаблона, КоличествоВыгруженныхШаблонов, ТочныеПараметрыШаблонаЗаданы, ПрефиксИмениШаблона, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм) Экспорт
	...
	Для Каждого МакетОтчета Из МакетыОтчета Цикл
		...
		И Лев(нрег(МакетОтчета.Имя), СтрДлина(ПрефиксИмениМакетаШаблона)) = нрег(ПрефиксИмениМакетаШаблона) Тогда
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось получить шаблон машиночитаемой формы ""%1""!%2';
																								|en = 'Cannot receive the ""%1"" machine-readable form template.%2'"), ИмяФайлаШаблона, Символы.ПС + ОписаниеОшибки());
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось получить шаблон машиночитаемой формы ""%1""!%2';
																								|en = 'Cannot receive the ""%1"" machine-readable form template.%2'"), Выборка.ИмяФайлаШаблона, Символы.ПС + ОписаниеОшибки());
	...
	ПолучитьДистрибутивКомпонентыПечатиМашиночитаемыхФорм(ДистрибутивКомпонентыПечатиМашиночитаемыхФорм);
...
Функция СформироватьСтруктуруПараметровФайлаВыгрузкиНаСервере(Адрес, ВерФормВыгрузки) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление временного файла регламентированной отчетности';
										|en = 'Delete a temporary file of local reporting'"), УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Функция ПолучитьСведенияОПредставителеОрганизации(Организация, КодИФНС, КПП, ДатаПодписиОтчета)	Экспорт
	...
	ДанныеПредставителя = РегистрацияВНОСервер.ДанныеРегистрации(СправочникИФНС[0].Ссылка);
	
	// В справочнике регистрации ИФНС может быть только информация по одной ИФНС!
	Представитель = ДанныеПредставителя.Представитель;
	...
	КонецЕсли;
	Результат.Вставить("ДокументПредставителя", ДанныеПредставителя.ДокументПредставителя);
...
Функция ФормаРеглОтчетаСоответствуетФормату(ИсточникОтчета, ДатаОкончания, ВерсияФормата, ИмяФормыОтчета) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка регламентированной отчетности';
										|en = 'Import local reporting'"), УровеньЖурналаРегистрации.Ошибка, , , ИсточникОтчета + Символы.ПС + ОписаниеОшибки());
...
Функция ПроверитьВозможностьВыводаМашиночитаемойФормыЧерезВебСервис(АдресПараметровПроксиВоВременномХранилище, ПолучитьОпределенияВебСервиса = Истина, ЭтоВызовИзВебКлиента = Ложь, ВебБраузер = "") Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Невозможно подключиться к веб-сервису формирования машиночитаемых бланков.';
								|en = 'Cannot connect to the web service of machine-readable form generation.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Исключение
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось получить подключение к веб-сервису формирования машиночитаемых бланков!';
								|en = 'Cannot receive connection to the web service of machine-readable form generation.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецПопытки;
...
Функция ВывестиМашиночитаемуюФормуЧерезВебСервис(ИмяФормыОтчета, НаименованиеИФНС, ФайлВыгрузкиВоВременномХранилище, ПараметрыСформированногоОтчета, АдресПараметровПроксиВоВременномХранилище) Экспорт
	...
	Исключение
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось получить подключение к веб-сервису! Проверьте адрес точки подключения, имя, пароль или доступность веб-сервиса.';
								|en = 'Cannot receive connection to web service. Check address of connection point, name, password or availability of web service.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецПопытки;
	...
	Если НЕ ЭтоАдресВременногоХранилища(АдресФайлаВыгрузкиВоВременномХранилище) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось получить файл выгрузки отчета на сервере!';
								|en = 'Cannot receive the report export file on the server.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Если МакетНайден Тогда
		...
		Если КоличествоСтрок > 0 Тогда
			...
			Если ОбластьНастройки <> Неопределено Тогда
				...
				Исключение
					ЗаписьЖурналаРегистрации(НСтр("ru = 'Регламентированная отчетность';
													|en = 'Local reporting'"), УровеньЖурналаРегистрации.Ошибка, , , ПутьКФорме + Символы.ПС + ОписаниеОшибки());
				КонецПопытки;
				ЧтениеJSON.Закрыть();
			КонецЕсли;
		КонецЕсли;
...
Функция ПравилаЗагрузкиДляФормы(ПутьКФорме, НастройкиЗагрузки = Неопределено) Экспорт
	...
	МакетНайден = Ложь;
	Если ОбъектОтчет.Метаданные().Макеты.Найти("EF_" + ИмяМакета) <> Неопределено
		И Не РегламентированнаяОтчетность.ЭтоВнешнийОтчет(ПутьКФорме) Тогда
		МакетНайден = Истина;
		ИмяМакета = "EF_" + ИмяМакета;
	ИначеЕсли ОбъектОтчет.Метаданные().Макеты.Найти(ИмяМакета) <> Неопределено Тогда
		МакетНайден = Истина;
	КонецЕсли;
...
Процедура УдалитьСохраненныйДокумент(ДокументСсылка) Экспорт
	...
	Исключение
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при удалении документа: ""%1"".';
				|en = 'An error occurred when deleting the document: ""%1"".'"), Строка(ДокументСсылка));
		Сообщение.Сообщить();
	КонецПопытки;
КонецПроцедуры
...
Функция ИсточникОтчета(ИмяФормыСтр) Экспорт
	...
	Если СтрНачинаетсяС(МассивИмениФормы[1], "РегламентированныйОтчет")
		Или СтрНачинаетсяС(МассивИмениФормы[1], "EF_РегламентированныйОтчет") Тогда
...
Функция ОбъектОтчета(ЭтаФормаИмя) Экспорт
	Если СтрНачинаетсяС(ЭтаФормаИмя, "Внешний") Тогда
		Попытка
			Возврат ВнешниеОтчеты.Создать(СтрРазделить(ЭтаФормаИмя, ".")[1]);
		Исключение
			Возврат ВнешнийОтчетВФоновомЗадании(СтрРазделить(ЭтаФормаИмя, ".")[1]);
		КонецПопытки;
	ИначеЕсли СтрНачинаетсяС(ЭтаФормаИмя, "Документ") Тогда
		Возврат Документы[СтрРазделить(ЭтаФормаИмя, ".")[1]].СоздатьДокумент();
	Иначе
		Возврат Отчеты[СтрРазделить(ЭтаФормаИмя, ".")[1]].Создать();
	КонецЕсли;
КонецФункции
...
Процедура СпециальногоДобавленияСтроки(ТекТабличноеПоле, ТекТабличноеПолеИмя, ИмяГруппы, НомерВерхЯчейкиОбласти, НомерНижнЯчейкиОбласти) Экспорт
	...
	// Отобрать в список рисунки подсказок вставляемой области.
	СписокПодсказок = Новый СписокЗначений;
	ВставляемаяОбластьДок = ТекТабличноеПоле.ПолучитьОбласть(ВставляемаяОбласть.Имя);
	Для Каждого РисунокДок Из ВставляемаяОбластьДок.Рисунки Цикл
		Если СтрНачинаетсяС(РисунокДок.Имя, "Подсказка") И РисунокДок.Гиперссылка Тогда
			СписокПодсказок.Добавить(ТекТабличноеПоле.Области.Найти(РисунокДок.Имя), РисунокДок.Имя);
		КонецЕсли;
	КонецЦикла;
	...
	// Назначить новые имена рисункам подсказок после вставки.
	Для Каждого ЭлементСпискаПодсказок Из СписокПодсказок Цикл
		ПодсказкаВИсходнойСтроке = ЭлементСпискаПодсказок.Значение;
		ПодсказкаВоВставленнойСтроке = ТекТабличноеПоле.Области.Найти(ЭлементСпискаПодсказок.Представление);
		Если ПодсказкаВоВставленнойСтроке <> Неопределено И ПодсказкаВИсходнойСтроке <> Неопределено Тогда
			ЭлементыИмени = СтрРазделить(ПодсказкаВоВставленнойСтроке.Имя, "_");
			ИндексПоследнегоЭлемента = ЭлементыИмени.ВГраница();
			Если РегламентированнаяОтчетностьКлиентСервер.ЯвляетсяЧислом(ЭлементыИмени[ИндексПоследнегоЭлемента]) Тогда
				ЭлементыИмени[ИндексПоследнегоЭлемента] = Формат(Число(ЭлементыИмени[ИндексПоследнегоЭлемента]) + 1, "ЧГ=");
				ПодсказкаВоВставленнойСтроке.Имя = СтрСоединить(ЭлементыИмени, "_");
				ПодсказкаВИсходнойСтроке.Имя = ЭлементСпискаПодсказок.Представление;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОтразитьВЖурналеОтчетов(ПараметрыОтчета) Экспорт
	...
	Если ТипЗнч(ДокСсылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
		...
		ИначеЕсли ДокСсылка.ИсточникОтчета = "РегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ" Тогда
			ПредставлениеФинПериода = РегламентированнаяОтчетностьКлиентСервер.ПредставлениеПериодаРасширенное(
				НачалоДня(ДатаНачалаОП),
				КонецДня(ДатаОкончанияОП),
	...
	Попытка
		...
		ИначеЕсли ВидКонтролирующегоОргана = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.Минобороны") Тогда
			...
			Если ТекущийСтатус = ДокументооборотСКОКлиентСервер.СтатусПереданоВМинобороны() Тогда
	...
	Исключение
		...
		Если ОшибкаУстановкиБлокировки Тогда
			Возврат;
		Иначе
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Журнал отчетов (статусы): ошибка записи';
											|en = 'Report log (statuses) : writing error'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Если ФП = "Ложь"
		   И ДатаНачалаОП = НачалоГода(ДатаНачалаОП) Тогда
			...
			Если СтрНайти(ПредставлениеФинПериода, "1 квартал") > 0 Тогда
				...
				ПредставлениеФинПериода = СтрЗаменить(ПредставлениеФинПериода, НСтр("ru = '1 квартал';
																					|en = '1 quarter'"), НСтр("ru = 'Январь - Март';
																											|en = 'January - March'"));
				
			ИначеЕсли ПредставлениеФинПериода = ПредставлениеФинПериодаГод Тогда
				
				ПредставлениеФинПериода = НСтр("ru = 'Январь - Декабрь ';
												|en = 'January - December'") + ПредставлениеФинПериодаГод;
...
Функция ПредставлениеКонтролирующегоОргана(ЗначенияЗаполнения, ДобавлятьБывш = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Вид) Тогда
		...
		Иначе
			Если Вид = Перечисления.ТипыКонтролирующихОрганов.ПФР И СокрЛП(КодКонтролирующегоОргана) = "ЕЦП" Тогда
...
Функция ВозможноПрисоединениеФайловКОбъекту(ВладелецФайлов) Экспорт
	...
	ИмяОпределяемогоТипа = "ПрисоединенныйФайл";
	ОпределяемыйТипПрисоединенныйФайл = Метаданные.ОпределяемыеТипы.Найти(ИмяОпределяемогоТипа);
	Если ОпределяемыйТипПрисоединенныйФайл = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ТипыХранимыхФайлов = Метаданные.ОпределяемыеТипы[ИмяОпределяемогоТипа].Тип;
...
// Проверяет значение КПП на соответствие требованиям, с кодом причины состоящим из цифр и букв от A до Z.
// 
// Параметры:
//  КПП - Строка
// 
...
//   Булево
//
Функция КППСоответствуетТребованиямФНС(Знач КПП) Экспорт
	
	// Регулярное выражение соооветствует шаблону типа "КППТип" из XSD-схем форматов выгрузки.
	Возврат СтрПодобнаПоРегулярномуВыражению(КПП,
...
Функция ИмяФайлаСданногоОбъекта1СОтчетности(Ссылка) Экспорт 
	ВсеОтправки = СведенияПоОтправкам.СведенияПоВсемОтправкам(Ссылка);
	Для Инд = 0 По ВсеОтправки.ВГраница() Цикл 
		Отправка = СведенияПоОтправкам.СведенияПоОтправке(Ссылка, ВсеОтправки[Инд].ИдентификаторОтправки);
		Если Отправка.Статус = Перечисления.СтатусыОтправки.Сдан Тогда 
			Если СтрЗаканчиваетсяНа(НРег(Отправка.ИмяФайла), ".xml") Тогда 
				Возврат Лев(Отправка.ИмяФайла, СтрДлина(Отправка.ИмяФайла) - 4);
			Иначе
				Возврат Отправка.ИмяФайла;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
КонецФункции
...
Функция ОписаниеРегОтчета(ДокументРегОтчет) Экспорт
	...
	Если ТипЗнч(ДокументРегОтчет) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
		...
		ИначеЕсли СокрЛП(ОбъектРегОтч.НаименованиеОтчета) = "Доходы, выплаченные иностранным организациям" Тогда
			...
			Если ОписаниеРегОтч.ВыбраннаяФорма = "ФормаОтчета2023Кв4"
			 ИЛИ ОписаниеРегОтч.ВыбраннаяФорма = "ФормаОтчета2020Кв1"
...
Функция ТекстСообщенияОНепройденномКонтролеНомеровКорректировок(ВыбОрганизация,
	...
	ОбъектОтчет = ОтчетОбъект("РегламентированныйОтчетНДС");
	...
	Если ШаблоныМашиночитаемыхФорм.Количество() > 0 Тогда
		...
		Если Прокси.UploadTemplates(WSСписокШаблонов) <> Истина Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не удалось обновить шаблон машиночитаемой формы на веб-сервисе';
									|en = 'Cannot update machine-readable form template on the web service'");
			Сообщение.Сообщить();
			Возврат Ложь;
		КонецЕсли;
...
Функция ДополнитьПоРегистрацииРеспондента(СведенияОрганизации, СведенияРегистрации, Знач РегистрацияРеспондента, Знач Организация, Знач ДатаЗначения) Экспорт
	...
	Если Не РегламентированнаяОтчетность.ИспользуютсяРегистрацииВОрганеСтатистики() Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	РегистрацияРеспондента = ОбщийМодульРегистрацииВОрганеСтатистикиВызовСервера.ДополнитьПоРегистрацииРеспондента(
		СведенияОрганизации, СведенияРегистрации, РегистрацияРеспондента, Организация, ДатаЗначения);
...
Функция НеотправляемыйРеглОтчетИмеетВозможностьИзмененияСтатусаОтправкиИзФормыОтчетаВручную(ИмяОбъекта)
	...
	МассивИмен.Добавить("РегламентированныйОтчетКассовыйПланГОЗ");
	МассивИмен.Добавить("РегламентированныйОтчетЦенаПродукцииГОЗ");
	МассивИмен.Добавить("РегламентированныйОтчетОбученныеПоОхранеТрудаЛица");
...
Процедура ОпределитьНаличиеКодаПодразделенияВСправочникеОрганыФСГС(Знач Подразделение) Экспорт
	...
	Если Подключен Тогда
		Попытка
			...
			КонецЕсли;
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка получения кода ФСГС подразделения';
											|en = 'The error is occurred when the FSO business unit code  is received'"), УровеньЖурналаРегистрации.Предупреждение, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	КонецЕсли;
...
Процедура УведомлениеОСпецрежимахНалогообложения_ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка) Экспорт 
	...
	ИначеЕсли Параметры.Свойство("ЗначениеКопирования") И ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		РеквизитыКлюча = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Параметры.ЗначениеКопирования, "ВидУведомления,Организация,ИмяФормы,ИмяОтчета");
		ВидСообщения = РеквизитыКлюча.ВидУведомления;
		Организация = РеквизитыКлюча.Организация;
		ИмяФормы = РеквизитыКлюча.ИмяФормы;
		ИмяОтчета = РеквизитыКлюча.ИмяОтчета;
		Ключ = Параметры.ЗначениеКопирования;
		
		УведомлениеОСпецрежимахНалогообложения.ЗаменитьИмяОтчетаПередОткрытием(ВидСообщения, ИмяФормы, ИмяОтчета);
		Если ОбщегоНазначения.ОбщийМодуль("Отчеты.ВизуализацияУстаревшихУведомлений").РедакцияУдалена(ИмяОтчета, ИмяФормы)
			Или Не ЗначениеЗаполнено(Отчеты[ИмяОтчета].ПолучитьТаблицуФорм().НайтиСтроки(Новый Структура("ИмяФормы", ИмяФормы))) Тогда 
			СтандартнаяОбработка = Ложь;
			ВыбраннаяФорма = "ФормаДокумента";
			Параметры.Вставить("ЗапретКопирования", Истина);
			Возврат;
		КонецЕсли;
	Иначе
	...
	Если ВидСообщения = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ФормаР11001
		Или ВидСообщения = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ФормаР21001 Тогда
	...
	Если ЗначениеЗаполнено(ИмяФормы) И ЗначениеЗаполнено(ИмяОтчета) Тогда
		...
		Параметры.Свойство("Данные", Данные);
	ИначеЕсли ЗначениеЗаполнено(ВидСообщения) И (Не ПроверятьЗаполнениеОрганизации Или ЗначениеЗаполнено(Организация)) Тогда
		СтандартнаяОбработка = Ложь;
