...
Функция ДеревоУпаковок(Значение, Детализация = Неопределено, ПараметрыСканирования = Неопределено) Экспорт
	...
	Для Каждого КлючИЗначение Из СтатусыКодовМаркировки Цикл
		...
		Для Каждого СтрокаВложенногоКодаМаркировки Из ВложенныеУпаковки Цикл
			...
			СтрокаКодаМаркировки = СтрокаВложенногоКодаМаркировки.Значение;
					...
					Если ДетализироватьДоПачекРасширенно Тогда
						...
						Если СтрокаКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая
							...
							Для Каждого КлючИЗначение Из ВложенныеУпаковки Цикл
								...
								ВложеннаяСтрокаКодаМаркировки = КлючИЗначение.Значение;
			...
			Для Каждого ВидПродукции Из ВидыПродукции Цикл
				...
				Иначе
					...
					Иначе
						...
						НормализованныйКодМаркировки = ОбменДаннымиИСМП.КодМаркировкиДляПередачиИСМП(СтрокаТаблицы, ПараметрыНормализацииИСМПУкороченныйФормат);
						МассивДобавляемыхКодов.Добавить(НормализованныйКодМаркировки);
						
						НормализованныйКодМаркировки = ОбменДаннымиИСМП.КодМаркировкиДляПередачиИСМП(СтрокаТаблицы, ПараметрыНормализацииИСМПЛидирующиеНули);
						МассивДобавляемыхКодов.Добавить(НормализованныйКодМаркировки);
...
Процедура ЗаполнитьДеревоУпаковокРекурсивно(ПараметрыЗаполнения)
	...
	Если ТипЗнч(КодыМаркировкиУпаковок) <> Тип("Число")
		...
		Для Каждого ЗначениеВложенногоКода Из КодыМаркировкиУпаковок Цикл
			СтрокаКодаМаркировки = ЗначениеВложенногоКода.Значение;
	...
	ИначеЕсли ТипЗнч(КодыМаркировкиУпаковок) <> Тип("Число")
		...
		Для Каждого ЗначениеВложенногоКода Из КодыМаркировкиУпаковок Цикл
			...
			Если СтрокаКодаМаркировки = Неопределено
				И ТипЗнч(ЗначениеВложенногоКода.Ключ) = Тип("Структура") Тогда
			...
			Если ТипЗнч(ЗначениеВложенногоКода.Значение) = Тип("Структура")
				И ЗначениеВложенногоКода.Значение.Свойство("Статус") Тогда
				Если ДанныеСтатуса = Неопределено Тогда
