...
Функция НастройкиПодключаемогоОбъекта(ПолноеИмя, НастройкиПрограммногоИнтерфейса = Неопределено) Экспорт
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось прочитать настройки объекта %1 из модуля менеджера:';
				|en = 'Cannot read %1 object settings from the manager module:'"), ПолноеИмя);
		...
		Если ЗаписыватьОшибку Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Подключаемые команды';
					|en = 'Attachable commands'", ОбщегоНазначения.КодОсновногоЯзыка()),
