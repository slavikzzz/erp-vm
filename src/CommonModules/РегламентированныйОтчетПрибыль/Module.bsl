...
Функция ВерсияКонтейнера(ИмяРедакции) Экспорт
	...
	Если ИмяРедакции = "ФормаОтчета2013Кв4"
		...
		Возврат '2016-10-01';
	ИначеЕсли ИмяРедакции = "ФормаОтчета2019Кв4"
		Или ИмяРедакции = "ФормаОтчета2020Кв4"
		Или ИмяРедакции = "ФормаОтчета2021Кв4"
		Или ИмяРедакции = "ФормаОтчета2022Кв4" Тогда
		Возврат '2019-10-01';
	КонецЕсли;
...
Процедура ВключитьСтраницу(Страница, НомерЛиста, НомерПриложения, ИмяРедакции, ПризнакНалогоплательщика = "") Экспорт
	...
	Если ЗначениеЗаполнено(ПризнакНалогоплательщика) Тогда
		
		ПризнакНалогоплательщикаПоУмолчанию = ПризнакНалогоплательщика;
		
	ИначеЕсли ВерсияКонтейнера(ИмяРедакции) < '2019-10-01' Тогда
