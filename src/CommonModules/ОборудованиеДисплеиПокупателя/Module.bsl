...
Процедура ОбновитьПоставляемыеДрайвера(ДрайвераОборудования) Экспорт
	...
	Драйвер.Наименование = НСтр("ru = '1С:Дисплей покупателя';
								|en = '1C:Customer display'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'АТОЛ:Дисплеи покупателя 8.X';
								|en = 'ATOL:Customer displays 8.X'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'POSUA:Дисплеи покупателя';
								|en = 'POSUA:Customer displays'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = '1С-Рарус:Дисплеи покупателя';
								|en = '1C-Rarus:Customer displays'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	Драйвер.ИдентификаторОбъекта = "d_android1c83"; 
	
	Драйвер = ДрайвераОборудования.Добавить();
	Драйвер.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ДисплейПокупателя;
	Драйвер.ИмяДрайвера  = "ДрайверСканкодДисплеиПокупателяNative";
	Драйвер.Наименование = НСтр("ru = 'Сканкод:Дисплеи покупателя (Native API)';
								|en = 'Scancode:Customer displays (Native API)'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	Драйвер.ИдентификаторОбъекта = "DSPPromag8"; 
	
	Драйвер = ДрайвераОборудования.Добавить();
	Драйвер.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ДисплейПокупателя;
	Драйвер.ИмяДрайвера  = "ДрайверКристаллСервисДисплеиПокупателяVikiVision";
	Драйвер.Наименование = НСтр("ru = 'Кристалл Сервис:Дисплей покупателя Viki Vision';
								|en = 'Crystal Service:Viki Vision customer display'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'ККС:Дисплеи покупателя';
								|en = 'KKS:Customer displays'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'Эвотор:Дисплей покупателя';
								|en = 'Evotor:Customer display'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
...
Процедура СтраницаСервисныйРежим(Знач ПараметрыПодключения, Знач ДанныеОперации, Знач ПараметрыОперации)
	...
	Если ПараметрыОперации.ГотовностьКРаботе Тогда
		Параметры.Операция = НСтр("ru = 'ГОТОВ К ПРОДАЖЕ';
									|en = 'READY FOR SALE'");
		Параметры.Информация = НСтр("ru = 'ДОБРО ПОЖАЛОВАТЬ!';
									|en = 'WELCOME'"); // АПК: 374 - интерфейсная особенность
	...
	ИначеЕсли НЕ ПараметрыОперации.ГотовностьКРаботе Тогда
		Параметры.Операция = НСтр("ru = 'НЕДОСТУПНО';
									|en = 'UNAVAILABLE'");
		Параметры.Информация = НСтр("ru = 'КАССА НЕ РАБОТАЕТ!';
									|en = 'THE CASH REGISTER IS DOWN.'"); // АПК: 374 - интерфейсная особенность
	...
	Параметры.ТекстПриложениеМойНалог = НСтр("ru = 'Для поверки чеков используйте<br>Мои чеки онлайн';
											|en = 'To check receipts, use<br>My receipts online'");
	// -- Локализация
...
Процедура СтраницаФискальныйЧек(Знач ПараметрыПодключения, Знач ДанныеОперации, Знач ПараметрыОперации)
	...
	Параметры.Заголовок = НСтр("ru = 'Получите электронный чек';
								|en = 'Receive an electronic receipt'");
	...
	Параметры.Описание = НСтр("ru = 'Сканируйте QR-код <br> в мобильном приложении';
								|en = 'Scan the QR code<br> in the mobile application'");
...
Процедура СтраницаСписокТоваров(Знач ПараметрыПодключения, Знач ДанныеОперации, Знач ПараметрыОперации)
	...
	Для Каждого КолонкаТабличнойЧасти Из ПараметрыОперации.КолонкиТабличнойЧасти Цикл                                        
		Если ВРег(КолонкаТабличнойЧасти.Имя) = НСтр("ru = 'СКИДКА';
													|en = 'DISCOUNT'") Тогда
	...
	// Текст на обычный дисплей покупателя
	Если ПустаяСтрока(ПараметрыОперации.СтрокиТекста) Тогда
		ДанныеОперации.СтрокиТекста = НСтр("ru = 'ИТОГО:';
											|en = 'TOTAL:'") + " " + Формат(ПараметрыОперации.ПодвалСумма, ФорматЧисла);  
	КонецЕсли;
...
Процедура СтраницаПлатежнаяОперация(Знач ПараметрыПодключения, Знач ДанныеОперации, Знач ПараметрыОперации)
	...
	ИначеЕсли ПараметрыОперации.ТипПлатежнойСистемы = Перечисления.ТипыПлатежнойСистемыККТ.ЮКасса Тогда
		Параметры.Комментарий = НСтр("ru = 'Для входа в личный кабинет отсканируйте QR-код';
									|en = 'To log in to your personal account, scan the QR code'");
		ОплатаЧерезСервис = Истина;
	КонецЕсли;
	...
	// Текст на обычный дисплей покупателя
	Если ПустаяСтрока(ПараметрыОперации.СтрокиТекста) Тогда
		ДанныеОперации.СтрокиТекста = НСтр("ru = 'ПЛАТЕЖНАЯ ОПЕРАЦИЯ';
											|en = 'PAYMENT TRANSACTION'") + Символы.ПС + Параметры.Операция;
	КонецЕсли;
