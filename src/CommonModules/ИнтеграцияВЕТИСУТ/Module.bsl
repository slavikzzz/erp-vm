...
Функция СоздатьЕдиницуИзмеренияПоКлассификаторуОКЕИ(КодОКЕИ) Экспорт
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru = 'Создание единицы измерения по классификатору.';
							|en = 'Создание единицы измерения по классификатору.'", КодОсновногоЯзыка);
...
Процедура ОбработатьВводПоГруппеСкладов(РеквизитыДокумента)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РеквизитыДокумента, "ЭтоГруппаСкладов")
		...
		ТекстОшибки = НСтр("ru = 'Документ %Документ% оформлен по группе складов. Ввод на основании не поддерживается.';
							|en = 'Документ %Документ% оформлен по группе складов. Ввод на основании не поддерживается.'");
	...
	Если Выборка.Количество() = 0 Тогда
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'В %1 отсутствует продукция для заполнения.';
				|en = 'В %1 отсутствует продукция для заполнения.'"),
			ДанныеЗаполнения);
		...
		Иначе
			...
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Выборка, "Номенклатура") Тогда
				...
				ТекстОшибки = НСтр("ru = 'Не выполнено сопоставление %Номенклатура% классификатору номенклатуры ВетИС.';
									|en = 'Не выполнено сопоставление %Номенклатура% классификатору номенклатуры ВетИС.'");
...
Процедура ОбработкаЗаполненияДокументаИсходящаяТранспортнаяОперацияВЕТИС(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ПередачаДавальцу")Тогда 
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ВозвратСырьяДавальцу")Тогда 
...
Функция СтруктураЗапросаДанныеДокументаПроизводствоБезЗаказа()
	...
	|	МИНИМУМ(
	|		ВЫБОР
	|			КОГДА НазначенияДавальца.Договор.Контрагент ЕСТЬ НЕ NULL
	|				ТОГДА НазначенияДавальца.Договор.Контрагент
	//++ НЕ УТКА
	|			ИНАЧЕ ВЫБОР
	//++ Устарело_Переработка24
	|				КОГДА НазначенияДавальца.Заказ ЕСТЬ НЕ NULL
	|					И ТИПЗНАЧЕНИЯ(НазначенияДавальца.Заказ) = ТИП(Документ.ЗаказДавальца)
	|					ТОГДА ВЫРАЗИТЬ(НазначенияДавальца.Заказ КАК Документ.ЗаказДавальца).Контрагент
	//-- Устарело_Переработка24
	|				КОГДА НазначенияДавальца.Заказ ЕСТЬ НЕ NULL
	|					И ТИПЗНАЧЕНИЯ(НазначенияДавальца.Заказ) = ТИП(Документ.ЗаказДавальца2_5)
	|					ТОГДА ВЫРАЗИТЬ(НазначенияДавальца.Заказ КАК Документ.ЗаказДавальца2_5).Контрагент
	//-- НЕ УТКА
	|				КОГДА ИСТИНА
	...
	|ПОМЕСТИТЬ Штрихкоды
	|ИЗ
	|	Документ.ПроизводствоБезЗаказа.ШтрихкодыУпаковок КАК ТабличнаяЧасть
	|
	|ГДЕ
	|	ТабличнаяЧасть.Ссылка = &ДокументОснование
	|";
...
Функция СтруктураЗапросаДанныеДокументаЭтапПроизводства2_2()
	...
	//-- Устарело_Переработка24
	|		КОГДА Таблица.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья2_5)
	|			ТОГДА
	|				Таблица.Договор.Контрагент
...
Функция ДанныеПартииПроизводстваПоДокументуОснования(ДокументОснования) Экспорт
	...
	Если ТипЗнч(ДокументОснования) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ
			...
			|	ДокументОснование.ПартияПроизводства
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
		...
		Запрос.УстановитьПараметр("ДокументОснование", ДокументОснования);
...
Процедура ПроверитьЗаполнениеИдентификатораПартии(ДокументОбъект, Отказ, МассивНепроверяемыхРеквизитов) Экспорт
	...
	ШаблонСообщения = НСтр("ru = 'Не заполнено поле ""Идентификатор партии"" в строке %1 списка ""Продукция"".';
							|en = 'Не заполнено поле ""Идентификатор партии"" в строке %1 списка ""Продукция"".'");
...
Процедура ЗаполнитьДвижениеПродукцииИМатериаловНаОснованииИсходящейТранспортнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
...
Процедура ЗаполнитьПередачуСырьяПереработчикуНаОснованииИсходящейТранспортнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
...
Процедура ЗаполнитьВозвратСырьяОтПереработчикаНаОснованииВходящейТранспортнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
...
Процедура ЗаполнитьПоступлениеОтПереработчикаНаОснованииВходящейТранспортнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
...
Процедура ЗаполнитьПоступлениеСырьяОтДавальцаНаОснованииВходящейТранспортнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
...
Процедура ЗаполнитьВозвратСырьяДавальцуНаОснованииИсходящейТранспортнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
...
Процедура ЗаполнитьПередачуДавальцуНаОснованииИсходящейТранспортнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
...
Процедура ЗаполнитьПриемкуТоваровНаХранениеНаОснованииВходящейТранспортнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
...
Процедура ЗаполнитьОтгрузкуТоваровСХраненияНаОснованииИсходящейТранспортнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
...
Процедура ЗаполнитьВыкупПринятыхНаХранениеТоваровНаОснованииВходящейТранспортнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
	...
	СтруктураЗаполненияТЧ = СтруктураЗаполненияТабличныхЧастейПоВходящейТранспортнойОперации(ПакетРезультатовЗапроса);
	ЗаполнитьТабличныеЧастиПрикладногоДокументаНаОснованииВходящейТранспортнойОперации(ДокументОбъект,СтруктураЗаполненияТЧ);
	ДокументОбъект.ЗаполнитьУсловияЗакупокПоУмолчанию();
...
Процедура ЗаполнитьСборкуТоваровНаОснованииПроизводственнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
	...
	Если Комплект.Количество() > 1 Тогда
		...
		ТекстОшибки = НСтр("ru = 'Документ %Документ% содержит несколько строк в табличной части ""Продукция"". Ввод на основании документа ""Сборка (разборка) товаров"" невозможен.';
							|en = 'Документ %Документ% содержит несколько строк в табличной части ""Продукция"". Ввод на основании документа ""Сборка (разборка) товаров"" невозможен.'");
	...
	Если ТаблицаШапка.Количество() > 0 Тогда
		...
		Если НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(ДокументОбъект.СтатусУказанияСерий, ПараметрыУказанияСерий.Шапка)
			...
			ТекстОшибки = НСтр("ru = 'Документ %Документ% содержит несколько серий в табличной части ""Продукция"". Ввод на основании документа ""Сборка (разборка) товаров"" невозможен.';
								|en = 'Документ %Документ% содержит несколько серий в табличной части ""Продукция"". Ввод на основании документа ""Сборка (разборка) товаров"" невозможен.'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
...
Процедура ЗаполнитьПроизводствоБезЗаказаНаОснованииПроизводственнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения) Экспорт
...
Процедура ЗаполнитьСоответствиеШапкиДляВходящейТранспортнойОперацииВЕТИС(ДокументОснование, ДокументОбъект)
	...
	//++ Устарело_Переработка24
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ПоступлениеОтПереработчика")
		Или ТипОснования = Тип("ДокументОбъект.ПоступлениеОтПереработчика")
		Или ТипОснования = Тип("ДокументСсылка.ВозвратСырьяОтПереработчика")
		Или ТипОснования = Тип("ДокументОбъект.ВозвратСырьяОтПереработчика")
	//++ НЕ УТКА
		Или ТипОснования = Тип("ДокументОбъект.ПоступлениеСырьяОтДавальца")
	//-- НЕ УТКА
	Тогда
			РеквизитыШапки = Реквизиты(ДокументОснование,
				"Организация,Контрагент,Партнер,Подразделение",
				Не ЗаписьДокументаВЕТИС);
			Справочники.ХозяйствующиеСубъектыВЕТИС.СопоставитьСПрикладнымиРеквизитами(
				РеквизитыВЕТИС.ГрузоотправительХозяйствующийСубъект,
				РеквизитыВЕТИС.ГрузоотправительПредприятие,
			...
			Справочники.ХозяйствующиеСубъектыВЕТИС.СопоставитьСПрикладнымиРеквизитами(
				...
				Истина);
	//-- Устарело_Переработка24
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ДвижениеПродукцииИМатериалов")
		Или ТипОснования = Тип("ДокументОбъект.ДвижениеПродукцииИМатериалов") Тогда
			РеквизитыШапки = Реквизиты(ДокументОснование,
				"Организация,ОрганизацияПолучатель,Отправитель,Получатель",
				Не ЗаписьДокументаВЕТИС);
			Справочники.ХозяйствующиеСубъектыВЕТИС.СопоставитьСПрикладнымиРеквизитами(
				РеквизитыВЕТИС.ГрузоотправительХозяйствующийСубъект,
				РеквизитыВЕТИС.ГрузоотправительПредприятие,
				РеквизитыШапки.Организация,
				РеквизитыШапки.Отправитель,
				Истина);
			Справочники.ХозяйствующиеСубъектыВЕТИС.СопоставитьСПрикладнымиРеквизитами(
				РеквизитыВЕТИС.ГрузополучательХозяйствующийСубъект,
				РеквизитыВЕТИС.ГрузополучательПредприятие,
				РеквизитыШапки.ОрганизацияПолучатель,
				РеквизитыШапки.Получатель,
				Истина);
	//-- НЕ УТ
	КонецЕсли;
...
Процедура ЗаполнитьСоответствиеШапкиДляИсходящейТранспортнойОперацииВЕТИС(ДокументОснование, ДокументОбъект)
	...
	//++ Устарело_Переработка24
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ПередачаСырьяПереработчику")
		Или ТипОснования = Тип("ДокументОбъект.ПередачаСырьяПереработчику")
	//++ НЕ УТКА
		Или ТипОснования = Тип("ДокументОбъект.ВозвратСырьяДавальцу")
		Или ТипОснования = Тип("ДокументОбъект.ПередачаДавальцу")
	//-- НЕ УТКА
	Тогда
			РеквизитыШапки = Реквизиты(ДокументОснование,
				"Организация,Контрагент,Партнер,Подразделение",
...
Процедура ЗаполнитьСоответствиеШапкиДляПроизводственнойОперацииВЕТИС(ДокументОснование, ДокументОбъект)
	...
	//++ НЕ УТ
		Или ТипОснования = Тип("ДокументСсылка.ПроизводствоБезЗаказа")
		Или ТипОснования = Тип("ДокументОбъект.ПроизводствоБезЗаказа")
	//-- НЕ УТ

	//++ НЕ УТКА
		Или ТипОснования = Тип("ДокументСсылка.ЭтапПроизводства2_2")
		Или ТипОснования = Тип("ДокументОбъект.ЭтапПроизводства2_2")
	//-- НЕ УТКА
		Тогда
			РеквизитыШапки = Реквизиты(ДокументОснование,
...
Функция ЗаполнитьСгенерироватьСерии(ТабличнаяЧастьТовары,
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Неправильный тип параметра ТабличнаяЧастьТовары метода ИнтеграцияВЕТИСУТ.ЗаполнитьСгенерироватьСерии';
								|en = 'Неправильный тип параметра ТабличнаяЧастьТовары метода ИнтеграцияВЕТИСУТ.ЗаполнитьСгенерироватьСерии'");
	КонецЕсли;
	...
	Для Каждого СтрТабл из ТаблицаОшибок Цикл
		...
		Если СтрТабл.ТипОшибки = "ОшибкаНетУточнения" Тогда
			...
			ТекстСообщения = НСтр("ru = 'По строке %НомерСтроки% указано, что должны быть уточнения, но строки уточнения не введены.';
									|en = 'По строке %НомерСтроки% указано, что должны быть уточнения, но строки уточнения не введены.'");
		...
		Иначе
			...
			Если СтрТабл.ТипОшибки = "ОшибкаНетНоменклатуры" Тогда
				...
				Если СтрТабл.ЭтоНомерСтрокиУточнений Тогда
					...
					ТекстСообщения = НСтр("ru = 'В одной из строк уточнения по строке %НомерСтроки% не указана номенклатура.';
											|en = 'В одной из строк уточнения по строке %НомерСтроки% не указана номенклатура.'");
				Иначе
					ПолеОшибки = "Объект.Товары.Номенклатура";
					ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% не указана номенклатура.';
											|en = 'В строке %НомерСтроки% не указана номенклатура.'");
				КонецЕсли;
			...
			ИначеЕсли СтрТабл.ТипОшибки = "ОшибкаНеНужноГенерировать" Тогда
				...
				Если СтрТабл.ЭтоНомерСтрокиУточнений Тогда
					...
					ТекстСообщения = НСтр("ru = 'В одной из строк уточнения по строке %НомерСтроки% выбрана номенклатура, по которой не настроена автоматическая генерация серий.';
											|en = 'В одной из строк уточнения по строке %НомерСтроки% выбрана номенклатура, по которой не настроена автоматическая генерация серий.'");
				Иначе
					ПолеОшибки = "Объект.Товары.Номенклатура";
					ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% выбрана номенклатура, по которой не настроена автоматическая генерация серий';
											|en = 'В строке %НомерСтроки% выбрана номенклатура, по которой не настроена автоматическая генерация серий'");
				КонецЕсли;
			...
			ИначеЕсли СтрТабл.ТипОшибки = "ОшибкаНетХарактеристики" Тогда
				...
				Если СтрТабл.ЭтоНомерСтрокиУточнений Тогда
					...
					ТекстСообщения = НСтр("ru = 'В одной из строк уточнения по строке %НомерСтроки% не указана характеристика - серии нужно генерировать после указания характеристики.';
											|en = 'В одной из строк уточнения по строке %НомерСтроки% не указана характеристика - серии нужно генерировать после указания характеристики.'");
				Иначе
					...
					ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% не указана характеристика - серии нужно генерировать после указания характеристики.';
											|en = 'В строке %НомерСтроки% не указана характеристика - серии нужно генерировать после указания характеристики.'");
				КонецЕсли;
			...
			ИначеЕсли СтрТабл.ТипОшибки = "ОшибкаНетЗаписиСкладскогоЖурнала" Тогда
				...
				ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% не указана запись складского журнала ВетИС - серии нужно генерировать после загрузки записи складского журнала из ВетИС.';
										|en = 'В строке %НомерСтроки% не указана запись складского журнала ВетИС - серии нужно генерировать после загрузки записи складского журнала из ВетИС.'");
			...
			ИначеЕсли СтрТабл.ТипОшибки = "ОшибкаНетДатыПроизводства" Тогда
				...
				ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% не указана запись складского журнала ВетИС, в которой не заполнена дата начала производства. Создайте серию вручную.';
										|en = 'В строке %НомерСтроки% не указана запись складского журнала ВетИС, в которой не заполнена дата начала производства. Создайте серию вручную.'");
			...
			ИначеЕсли СтрТабл.ТипОшибки = "ОшибкаНетГоденДо" Тогда
				...
				ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% указана запись складского журнала ВетИС, в которой не заполнена дата начала периода срока годности. Создайте серию вручную.';
										|en = 'В строке %НомерСтроки% указана запись складского журнала ВетИС, в которой не заполнена дата начала периода срока годности. Создайте серию вручную.'");
			...
			ИначеЕсли СтрТабл.ТипОшибки = "ОшибкаНетПроизводителя" Тогда
				...
				ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% указана запись складского журнала ВетИС, в которой не заполнен производитель. Создайте серию вручную.';
										|en = 'В строке %НомерСтроки% указана запись складского журнала ВетИС, в которой не заполнен производитель. Создайте серию вручную.'");
			...
			ИначеЕсли СтрТабл.ТипОшибки = "ОшибкаМногоПроизводителей" Тогда
				...
				ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% указана запись складского журнала ВетИС, в которой указано более одного производителя. Создайте серию вручную.';
										|en = 'В строке %НомерСтроки% указана запись складского журнала ВетИС, в которой указано более одного производителя. Создайте серию вручную.'");
			...
			ИначеЕсли СтрТабл.ТипОшибки = "ОшибкаНетИдентификатораПартии" Тогда
				...
				Если СтрТабл.ЭтоНомерСтрокиУточнений Тогда
					...
					ТекстСообщения = НСтр("ru = 'В одной из строк уточнения по строке %НомерСтроки% не указан идентификатор партии ВетИС. Создайте серию вручную.';
											|en = 'В одной из строк уточнения по строке %НомерСтроки% не указан идентификатор партии ВетИС. Создайте серию вручную.'");
				Иначе
					...
					ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% не указан идентификатор партии ВетИС. Создайте серию вручную.';
											|en = 'В строке %НомерСтроки% не указан идентификатор партии ВетИС. Создайте серию вручную.'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция СоздатьСерию(Реквизиты, Результат)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'ВетИС';
										|en = 'ВетИС'", ОбщегоНазначения.КодОсновногоЯзыка()),
