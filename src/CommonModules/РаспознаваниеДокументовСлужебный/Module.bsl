	...
	Попытка
		ФоновыеЗадания.Выполнить("РаспознаваниеДокументов.ЗагрузитьРаспознанныеДокументы", ПараметрыЗадания, КлючЗадания,
			НСтр("ru = 'Загрузка распознанных документов';
				|en = 'Import recognized documents'"));
	Исключение
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Невозможно выполнить фоновое задание загрузки распознанных докуменетов
			           |по причине:
			           |%1';
			           |en = 'Cannot execute the background job of recognized document import
			           |due to:
			           |%1'"),
...
Функция УстановитьИПроверитьВыполняетсяЗагрузкаРаспознанныхДокументов() Экспорт 
	...
	Попытка
		...
		Если РаспознаваниеДокументовСлужебный.ВыполняетсяЗагрузкаРаспознанныхДокументов(СтарыеПараметрыСеанса) Тогда
			ВызватьИсключение НСтр("ru = 'Загрузка распознанных документов уже началасть в другом сеансе.';
									|en = 'Import of recognized documents has already started in another session.'");
		КонецЕсли;
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Не удалось проверить и обновить Параметры сеанса загрузки распознанных документов.';
							|en = 'Cannot check and update Parameters of recognized document import session.'")
	...
	Если ПараметрыЗаполнения.Свойство("Товары") Тогда
		Для Каждого СтрокаТаблицы Из ПараметрыЗаполнения.Товары Цикл
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура) Тогда
				...
				ЭтоСообщение.Текст = НСтр("ru = 'Не заполнена колонка ""Номенклатура"" в строке ';
											|en = 'The ""Items"" column is not filled in line '") + СтрокаТаблицы.ПорядокСтроки;
