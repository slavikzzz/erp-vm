...
Функция СоединениеССерверомИнтернета(URL, ПараметрыСоединения = Неопределено) Экспорт
	...
	Если ТипЗнч(ПараметрыСоединения) = Тип("Структура") Тогда
		...
		КонецЕсли;
		Если ПараметрыСоединения.Свойство("ЗащищенноеСоединение") Тогда
			ЗащищенноеСоединение = ПараметрыСоединения.ЗащищенноеСоединение;
		КонецЕсли;
	КонецЕсли;
	ЗащищенноеСоединение = ?(НРег(Схема) = "http", Неопределено,
		?(ЗащищенноеСоединение = Неопределено,
...
// Возвращает значение перечисления, если перечисление и значение в нем существуют.
//
// Параметры:
//  ИмяТипаПеречисления - Строка.
//  ИмяЗначения         - Строка - при пустом значении возвращает пустую ссылку.
//  ЗначениеПоУмолчанию.
//
Функция БезопасноеПолучениеЗначенияПеречисления(
		ИмяТипаПеречисления,
		ИмяЗначения = "",
	...
	Если Метаданные.Перечисления.Найти(ИмяТипаПеречисления) <> Неопределено Тогда
		ОбъектПеречисления = Перечисления[ИмяТипаПеречисления];
		Если НЕ ЗначениеЗаполнено(ИмяЗначения) Тогда
			Результат = ОбъектПеречисления.ПустаяСсылка();
		ИначеЕсли Метаданные.Перечисления[ИмяТипаПеречисления].ЗначенияПеречисления.Найти(ИмяЗначения) <> Неопределено Тогда
			Результат = ОбъектПеречисления[ИмяЗначения];
		КонецЕсли;
	КонецЕсли;
...
// Возвращает ссылку на элемент объекта метаданных, переданного в значении по умолчаню, по фильтру.
//
// Параметры:
//  ЗначениеПоУмолчанию - например, Справочники.Организации.ПустаяСсылка().
//  ФильтрыПоиска       - Соответствие.
//
Функция НайтиЭлементИБ(ЗначениеПоУмолчанию, ФильтрыПоиска) Экспорт
	...
	Построитель.Текст = СтрЗаменить(Построитель.Текст, "Справочник.МашиночитаемыеДоверенностиФНС",
