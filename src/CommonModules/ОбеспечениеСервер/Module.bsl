...
Процедура ИспользоватьУправлениеПеремещениемОбособленныхТоваровВычислитьИЗаписать() Экспорт
	...
	Запрос.Текст =
		...
		|	1
		|ИЗ
		|	Константа.ИспользоватьПроизводствоИзДавальческогоСырья КАК ИспользоватьПроизводствоИзДавальческогоСырья
		|ГДЕ
		|	ИспользоватьПроизводствоИзДавальческогоСырья.Значение
		//-- Устарело_Переработка24
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	1
		|ИЗ
		|	Константа.ИспользоватьПроизводствоИзДавальческогоСырья2_5 КАК ИспользоватьПроизводствоИзДавальческогоСырья2_5
		|ГДЕ
		|	ИспользоватьПроизводствоИзДавальческогоСырья2_5.Значение
		//-- НЕ УТКА
		|;
		|/////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'Не удалось записать значение константы ""Использовать управление перемещением обособленных товаров""';
								|en = 'Cannot save ""Use management of assigned goods transfer"" constant value'");
	КонецПопытки;
...
Процедура ИспользоватьНазначенияБезЗаказаВычислитьИЗаписать() Экспорт
	...
	Запрос.Текст =
		...
		|	1
		|ИЗ
		|	Константа.ИспользоватьПроизводствоНаСтороне КАК ИспользоватьПроизводствоНаСтороне
		|ГДЕ
		|	ИспользоватьПроизводствоНаСтороне.Значение
		//-- Устарело_Переработка24
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'Не удалось записать значение константы ""Использовать назначения без заказа""';
								|en = 'Cannot save ""Use assignments without order"" constant value'");
	КонецПопытки;
...
Процедура НеИспользоватьНазначенияБезЗаказаВычислитьИЗаписать() Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'Не удалось записать значение константы ""Не использовать назначения без заказа""';
								|en = 'Cannot save ""Do not use assignments without order"" constant value'");
	КонецПопытки;
...
Процедура ДобавитьВременныеТаблицыОстатковРасходаИВозвратов(Запрос)
	...
	Запрос.Текст =
		...
		//++ НЕ УТ
		|				КОГДА НЕ ПроизводственныеПеремещения.Ссылка ЕСТЬ NULL ТОГДА
		|					ПроизводственныеПеремещения.Отправитель
		//-- НЕ УТ
		...
		|			
		//++ НЕ УТ
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ДвижениеПродукцииИМатериалов КАК ПроизводственныеПеремещения
		|			ПО ПроизводственныеПеремещения.Ссылка = Приемка.Регистратор
		|			И ПроизводственныеПеремещения.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратМатериаловИзКладовой)
		//-- НЕ УТ
		|	ГДЕ
		|		Приемка.Активность
		...
		//++ НЕ УТ
		|					И ПроизводственныеПеремещения.Ссылка ЕСТЬ NULL
		//-- НЕ УТ
		|				)) КАК Набор
		|СГРУППИРОВАТЬ ПО
		|	Набор.Номенклатура,
		...
		|							= ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратИзЭксплуатации)
		//++ НЕ УТ
		|					КОГДА ТИПЗНАЧЕНИЯ(Возврат.Регистратор) = ТИП(Документ.ДвижениеПродукцииИМатериалов) ТОГДА
		|						ВЫРАЗИТЬ(Возврат.Регистратор КАК Документ.ДвижениеПродукцииИМатериалов).ХозяйственнаяОперация
		|							= ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратМатериаловИзПроизводства)
		//-- НЕ УТ

		//++ НЕ УТКА
		|					КОГДА ТИПЗНАЧЕНИЯ(Возврат.Регистратор) = ТИП(Документ.ЭтапПроизводства2_2) ТОГДА
		|						ИСТИНА В(
		|							ВЫБРАТЬ ПЕРВЫЕ 1
		|								ИСТИНА
		|							ИЗ
		...
		|							ГДЕ
		|								Экономия.Ссылка = Возврат.Регистратор
		|									И Экономия.Номенклатура = Возврат.Номенклатура
		|									И Экономия.Характеристика = Возврат.Характеристика
		|									И Экономия.Получатель = Возврат.Склад)
		//-- НЕ УТКА
		|					ИНАЧЕ
