...
Процедура ПроверитьНачалоЭлемента(Знач ЧтениеXML, Знач Имя)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Или ЧтениеXML.Имя <> Имя Тогда
		ИмяСобытия = НСтр("ru = 'Календарные графики.Обработка файла классификатора';
							|en = 'Calendar schedules.Process classifier file'", ОбщегоНазначения.КодОсновногоЯзыка());
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверный формат файла данных. Ожидается начало элемента %1';
				|en = 'Invalid data file format. Beginning of the ""%1"" element is expected.'"), 
			Имя);
