...
Функция ПолучитьФайлыКлассификаторов(Идентификаторы) Экспорт
	...
	РезультатОбновления.Вставить("СообщениеОбОшибке",     "");
	РезультатОбновления.Вставить("ИнформацияОбОшибке",    "");
	РезультатОбновления.Вставить("ДанныеКлассификаторов", ОписаниеДанныхКлассификаторов());
...
//    * ВариантОбновленияКлассификаторов - Неопределено - интерактивная загрузка не доступна.
//                                       - Число 
//    * ФайлКлассификаторов - Неопределено - интерактивная загрузка не доступна.
//                          - Строка
//    * ПредставлениеРасписания - Неопределено - интерактивная загрузка не доступна.
//                              - Строка
//
Функция ПараметрыОкруженияФормыИнтернетПоддержкаИСервисы() Экспорт
	
	ИнтерактивнаяЗагрузкаДоступна = ИнтерактивнаяЗагрузкаКлассификаторовДоступна();
	
	Результат = Новый Структура();
	Результат.Вставить("ИнтерактивнаяЗагрузкаДоступна"   , ИнтерактивнаяЗагрузкаДоступна);
	Результат.Вставить("ВариантОбновленияКлассификаторов", Неопределено);
	Результат.Вставить("ФайлКлассификаторов"             , Неопределено);
	Результат.Вставить("ПредставлениеРасписания"         , Неопределено);
	
	Если Не ИнтерактивнаяЗагрузкаДоступна Тогда
		Возврат Результат;
	КонецЕсли;
	
	Результат.ВариантОбновленияКлассификаторов = Константы.ВариантОбновленияКлассификаторов.Получить();
	Результат.ФайлКлассификаторов              = Константы.ФайлКлассификаторов.Получить();
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Обработчик.Комментарий         = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1. Обновление списка классификаторов областей данных.';
				|en = '%1. Updating list of classifiers of data areas.'"),
			ИмяСобытияЖурналаРегистрации());
		...
		Обработчик.Комментарий         = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1. Перенос кэша поставляемых данных в подсистему ""Работа с классификаторами"".';
				|en = '%1. Moving the default master data cache to the ""Classifier operations"" subsystem.'"),
			ИмяСобытияЖурналаРегистрации());
	Иначе
...
Процедура ОбновитьНастройкиРаботыСКлассификаторами() Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Обновление настроек подсистемы ""Работа с классификаторами"". Начало обновления.';
			|en = 'Update the Classifier operation subsystem settings. Update started.'"),
		Ложь);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Обновление настроек подсистемы ""Работа с классификаторами"". Успешно завершено.';
			|en = 'Updating Classifier operation subsystem settings. Completed successfully.'"),
		Ложь);
...
Процедура ОбновитьНастройкиРаботыСКлассификаторамиОбластейДанных(Параметры) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Обновление настроек областей подсистемы ""Работа с классификаторами"". Начало обновления.';
			|en = 'Update settings of the ""Classifier operations"" subsystem area. Update start.'"),
		Ложь);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Обновление настроек областей подсистемы ""Работа с классификаторами"". Успешно завершено.';
			|en = 'Update settings of the ""Operations with classifiers"" subsystem areas. Succeeded.'"),
		Ложь);
...
Процедура ЗаполнитьДанныеРегистраВерсииКлассификаторовОбластейДанных(Параметры) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Перенос настроек в регистр сведений ВерсииКлассификаторовОбластейДанных. Начало обновления.';
			|en = 'Transfer settings to the DataAreasClassifiersVersions information register. Update start.'"),
		Ложь);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Перенос настроек в регистр сведений ВерсииКлассификаторовОбластейДанных. Успешно завершено.';
			|en = 'Moving settings to the DataAreasClassifiersVersions information register. Successfully completed.'"),
		Ложь);
...
Процедура НастроитьОбновленияКлассификаторов() Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Установка значения константы ВариантОбновленияКлассификаторов и добавление
			...
			|en = 'Setting value of the ClassifiersUpdateOption constant
			|and adding the UpdateClassifiers scheduled job. Update start.'"),
		Ложь);
	...
	Если ЗаданияОбновления.Количество() <> 0 Тогда
		Если ЗаданияОбновления[0].Использование
			...
			ВключитьАвтоматическоеОбновлениеКлассификаторовИзСервиса();
		КонецЕсли;
	Иначе
		ДобавитьРегламентноеЗаданиеОбновленияКлассификаторов(Ложь);
	КонецЕсли;
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Установка значения константы ВариантОбновленияКлассификаторов и добавление
			...
			|en = 'Setting value of the ClassifiersUpdateOption
			|and adding the UpdateClassifiers scheduled job. Completed successfully.'"),
		Ложь);
...
Процедура ПриОпределенииОбработчиковПоставляемыхДанных(Обработчики) Экспорт
	...
	ВидыДанных = ВидыПоставляемыхДанныхКлассификаторы();
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ТекущиеДела")
		...
		Или ПользовательскиеНастройкиОбновлений().ОтключитьНапоминания
		Или Не ЗагрузкаКлассификаторовДоступна() Тогда
		
		Возврат;
	КонецЕсли;
	
	ИдентификаторДела = "ВключениеРегламентногоОбновленияКлассификаторов";
	...
	Если Подсистема = Неопределено
		...
		Разделы = МодульТекущиеДелаСервер.РазделыДляОбъекта(Метаданные.Обработки.ОбновлениеКлассификаторов.ПолноеИмя());
	Иначе
	...
	Если Разделы.Количество() > 0 Тогда
		...
		ЕстьДела = (ДатаИнформирования <= ТекущаяДатаСеанса()
			И Не АвтоматическаяЗагрузкаОбновленийВключена());
		...
		Для Каждого Раздел Из Разделы Цикл
			...
			Дело.Представление  = НСтр("ru = 'Автоматическое обновление классификаторов';
										|en = 'Automatic classifier update'");
			Дело.Форма          = "Обработка.ОбновлениеКлассификаторов.Форма.НастройкаЗагрузкиОбновлений";
...
Процедура ПриИзмененииДанныхАутентификации(Логин, Пароль) Экспорт
	...
	Если ЗначениеЗаполнено(Логин) Тогда
		Если Не АвтоматическаяЗагрузкаОбновленийВключена() Тогда
			ВключитьАвтоматическоеОбновлениеКлассификаторовИзСервиса()
		Иначе
...
Процедура ПриЗаполненииХостовСервисовИнтернетПоддержки(ХостыСервисовИнтернетПоддержки) Экспорт
	...
	ХостыСервисовИнтернетПоддержки.Вставить(
		ХостСервисаКлассификаторов(0),
		НСтр("ru = 'Получение классификаторов';
			|en = 'Get classifiers'"));
	ХостыСервисовИнтернетПоддержки.Вставить(
		ХостСервисаКлассификаторов(1),
		НСтр("ru = 'Получение классификаторов';
...
Процедура ПараметрыРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Параметры.Вставить("ОповещениеОбновленияКлассификаторов", Ложь);
	
	Если ОбщегоНазначения.РазделениеВключено()
		Или ОбщегоНазначения.ЭтоАвтономноеРабочееМесто()
		Или ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ТекущиеДела")
		Или ПользовательскиеНастройкиОбновлений().ОтключитьНапоминания
		Или Не ЗагрузкаКлассификаторовДоступна()
		Или АвтоматическаяЗагрузкаОбновленийВключена() Тогда
		Возврат;
	КонецЕсли;
	...
	ДатаИнформирования = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		...
		РаботаСКлассификаторамиКлиентСервер.КлючНастройкиДатаИнформированияОВключенииЗагрузкиОбновлений(),
		'00010101');
	Параметры.ОповещениеОбновленияКлассификаторов = (ДатаИнформирования <= ТекущаяДатаСеанса());
...
//
Функция ВидПоставляемыхДанныхКлассификаторы(Идентификатор = "") Экспорт
...
Процедура ОбновлениеКлассификаторов() Экспорт
	...
	Если РежимОбновления = ВариантОбновленияИзФайла() Тогда
	...
	ИначеЕсли РежимОбновления = ВариантОбновленияИзСервиса() Тогда
		...
		Если Не ИнтернетПоддержкаПользователей.ЗаполненыДанныеАутентификацииПользователяИнтернетПоддержки() Тогда
			ЗаписатьИнформациюВЖурналРегистрации(
				НСтр("ru = 'Не заполнены данные аутентификации Интернет-поддержки пользователей.
					...
					|en = 'Authentication data for Online user support is blank.
					|Update of classifiers from the service is impossible.'"),
			Ложь,
			Метаданные.РегламентныеЗадания.ОбновлениеКлассификаторов);
			Возврат;
		КонецЕсли;
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ЗаданияОбновления.Количество() = 0 Тогда
			...
			Попытка
				...
				Если Использование
					И Не АвтоматическаяЗагрузкаОбновленийВключена() Тогда
					ВключитьАвтоматическоеОбновлениеКлассификаторовИзСервиса();
				КонецЕсли;
				ЗафиксироватьТранзакцию();
			Исключение
...
Процедура ВключитьАвтоматическоеОбновлениеКлассификаторовИзСервиса() Экспорт
	...
	Константы.ВариантОбновленияКлассификаторов.Установить(ВариантОбновленияИзСервиса());
...
Функция ИнформацияОбАктуальныхВерсияхКлассификаторов(
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Начало получения информации об актуальных версиях классификаторов: %1';
			|en = 'Start receiving information on latest classifier versions: %1'"),
		...
		РезультатОперации.ИнформацияОбОшибке = Новый ФорматированнаяСтрока(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось получить актуальные версии классификаторов.
					...
					|%4';
					|en = 'Failed to get relevant versions of external components.
					|
					|%1
					|
					|Technical information about the error:
					| When getting information about current versions of external components, the service returned an error.
					|URL: %2
					|Error code: %3
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Завершено получение актуальных версий классификаторов: %1';
			|en = 'Relevant classifier versions received: %1'"),
		СтрСоединить(Идентификаторы, ","));
...
Функция ЗагрузитьФайлыКлассификаторов(ДанныеКлассификаторов, ДанныеАутентификации)
	...
	Для Каждого ОписательКлассификатора Из ДанныеКлассификаторов Цикл
		...
		ПараметрыОтправки.Таймаут = ИнтернетПоддержкаПользователей.ТаймаутЗагрузкиФайла(
			ОписательКлассификатора.Размер);
...
//
// Параметры:
//  ИдентификаторыОтбор - Массив - отбор идентификаторов классификаторов.
//
// Возвращаемое значение:
//  Массив - содержит настройки классификаторов.
//
Функция ДанныеКлассификаторовДляИнтерактивногоОбновления(ИдентификаторыОтбор = Неопределено) Экспорт
	
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ru = 'Использование функции при работе в модели сервиса запрещено.';
								|en = 'You cannot use the function while working in SaaS mode.'");
	КонецЕсли;
	...
	Запрос.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Запрос.Текст,
		?(ИдентификаторыОтбор = Неопределено,
			"",
			"И ВерсииКлассификаторов.Идентификатор В (&ИдентификаторыОтбор)"));
	Запрос.УстановитьПараметр("ИдентификаторыОтбор", ИдентификаторыОтбор);
...
Функция ДанныеКлассификаторовИзКэша(Идентификаторы, ЗагрузитьФайлы = Истина)
	...
	ПолеДанныеФайла = ?(ЗагрузитьФайлы, "КэшДанныхКлассификаторов.ДанныеФайла", """""");
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДанныеФайла", ПолеДанныеФайла);
	Запрос.УстановитьПараметр("Идентификаторы", Идентификаторы);
	...
	ИначеЕсли КодОшибки = "НетДоступаКПрограмме" Тогда
		Возврат СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Доступ к обновлению классификатора невозможен, так как ваша программа не находится на <a href = ""https://portal.1c.ru/support/"">официальной поддержке</a>.';
				|en = 'Access to the classifier update is not possible because your application is not <a href = ""https://portal.1c.ru/support/"">officially supported</a>.'"));
...
Функция ДанныеАутентификации()
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		ВызватьИсключение НСтр("ru = 'При работе в модели сервиса информация о классификаторах
			...
			|en = 'When operating in SaaS, classifier information
			|is imported from the default master data.'");
		...
		Если Результат.ДанныеАутентификации = Неопределено Тогда
			...
			Результат.ИнформацияОбОшибке =
				НСтр("ru = 'Для получения обновлений классификаторов необходимо подключить Интернет-поддержку пользователей.';
					|en = 'To update classifiers, connect to online user support.'");
...
// Определяет виды данных классификаторов для поставляемых данных.
//
// Возвращаемое значение:
//  Массив из Строка- наименования видов данных.
//
Функция ВидыПоставляемыхДанныхКлассификаторы()
	...
	Идентификаторы = Новый Массив;
	Для Каждого Описатель Из Классификаторы Цикл
		Идентификаторы.Добавить(Описатель.Идентификатор);
	КонецЦикла;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.РаботаВМоделиСервиса.РаботаСКлассификаторами") Тогда
		МодульРаботаСКлассификаторамиСлужебныйВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль(
			"РаботаСКлассификаторамиСлужебныйВМоделиСервиса");
		МодульРаботаСКлассификаторамиСлужебныйВМоделиСервиса.ДополнитьИдентификаторыКлассификаторов(
			Идентификаторы);
	КонецЕсли;
	
	Для Каждого Идентификатор Из Идентификаторы Цикл
		ВидДанных = ВидПоставляемыхДанныхКлассификаторы(Идентификатор);
...
//
Функция ВариантОбновленияОтключено()
	Возврат 0;
КонецФункции
...
//  Число - Значение режима обновления.
//
Функция ВариантОбновленияИзФайла()
	Возврат 2;
КонецФункции

// Возвращает пользовательские настройки обновления классификаторов.
//
// Возвращаемое значение:
//  Структура - Настройки уведомлений:
//    * ОтключитьНапоминания - Булево - Истина, если необходимо отключить напоминание о включении автоматической
//        загрузки классификаторов в подсистеме БСП.ТекущиеДела и не показывать оповещение пользователю
//        при старте системы, если подсистема БСП.ТекущиеДела отсутствует в конфигурации.
//        Значение по умолчанию - Ложь.
//
Функция ПользовательскиеНастройкиОбновлений()
...
// Возвращает признак использования автоматического обновления классификаторов.
//
// Возвращаемое значение:
//  Булево - Ложь, когда отключено.
//
Функция АвтоматическаяЗагрузкаОбновленийВключена()
	Возврат Константы.ВариантОбновленияКлассификаторов.Получить() <> ВариантОбновленияОтключено();
КонецФункции
