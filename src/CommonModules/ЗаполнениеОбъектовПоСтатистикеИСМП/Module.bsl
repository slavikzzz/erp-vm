...
Функция ДанныеЗаполненияПеремещениеМеждуМОД_ИСМП(Организация) Экспорт
	
	ДанныеЗаполнения = Новый Структура;
	ДанныеЗаполнения.Вставить("Организация", ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("Организация"));
	ДанныеЗаполнения.Вставить("Отправитель", ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("Подразделение"));
	ДанныеЗаполнения.Вставить("Получатель",  ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа("Подразделение"));
	...
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 3
	|	1                                    КАК Количество,
	|	ПеремещениеМеждуМОД_ИСМП.Организация КАК Организация,
	|	ПеремещениеМеждуМОД_ИСМП.Отправитель КАК Отправитель,
	|	ПеремещениеМеждуМОД_ИСМП.Получатель  КАК Получатель
	|ИЗ
	|	Документ.ПеремещениеМеждуМОДИСМП КАК ПеремещениеМеждуМОД_ИСМП
	|ГДЕ
	|	ПеремещениеМеждуМОД_ИСМП.Организация = &Организация Или &БезУчетаОрганизации
	|	И ПеремещениеМеждуМОД_ИСМП.Проведен
	|УПОРЯДОЧИТЬ ПО
	|	ПеремещениеМеждуМОД_ИСМП.Дата УБЫВ");
	...
	ЗаполнитьПоСтатистике(
		"Организация, Отправитель, Получатель",
		ДанныеЗаполнения, ДанныеПоследнихДокументов, "");
...
Процедура ЗаполнитьДанныеПоТоварамЗаказаНаЭмиссиюКодовМаркировкиСУЗ(ОбъектЗаполнения, МассивСтрок = Неопределено, Знач ДополнительныеПоляПоиска = "") Экспорт
	...
	Для Каждого СтрокаТовары Из СтрокиТовары Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТовары.Шаблон) Тогда
			...
			ИначеЕсли Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.Антисептики
				Или Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.ДезинфицирующиеСредства Тогда
				СтрокаТовары.Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.АнтисептикиСтандартныйКМ;
			ИначеЕсли Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.БАДы
				Или Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.БАДы2025 Тогда
			...
			ИначеЕсли Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхПодконтрольныеВЕТИС
				Или Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхВлажныеПодконтрольныеВЕТИС Тогда
				...
				КонецЕсли;
			ИначеЕсли Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхБезВЕТИС
				Или Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхВлажныеБезВЕТИС Тогда
			...
			ИначеЕсли Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.ИгрыИИгрушкиДляДетей Тогда
				...
				КонецЕсли;
			ИначеЕсли Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.РадиоэлектроннаяПродукция
				Или Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.ТелефоныИНоутбуки
				Или Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.ПечатныеПлаты Тогда
			...
			ИначеЕсли Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.АлкогольнаяПродукцияДо9Процентов Тогда
				...
				КонецЕсли;
			ИначеЕсли Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.ПолимерныеТрубы Тогда
				Если ОбщегоНазначенияИСМПКлиентСервер.ПолимерныеТрубыИспользуетСтандартныйКМ() Тогда
					СтрокаТовары.Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.ПолимерныеТрубыСтандартныйКМ;
				Иначе
					СтрокаТовары.Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.ПолимерныеТрубы;
				КонецЕсли;
			ИначеЕсли Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.СладостиИКондитерскиеИзделия Тогда
				Если ОбщегоНазначенияИСМПКлиентСервер.СладостиИКондитерскиеИзделияИспользуетСтандартныйКМ() Тогда
					СтрокаТовары.Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.СладостиИКондитерскиеИзделияСтандартныйКМ;
				Иначе
					СтрокаТовары.Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.СладостиИКондитерскиеИзделия;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьПоСтатистике(Поля, ДанныеЗаполнения, ДанныеПоследнихДокументов, ПоляОтбора = "ВидПродукции")
	...
	ОбщегоНазначенияИСМП.ЗаполнитьПоСтатистике(Поля, ДанныеЗаполнения, ДанныеПоследнихДокументов, ПоляОтбора);
