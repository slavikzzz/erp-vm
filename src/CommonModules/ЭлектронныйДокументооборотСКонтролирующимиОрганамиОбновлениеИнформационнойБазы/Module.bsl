...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Выполняет обновление информации в форме 1С-Отчетность во всех разделах, кроме Отчеты. До завершения выполнения данные в форме 1С-Отчетность могут отображаться некорректно';
									|en = 'Выполняет обновление информации в форме 1С-Отчетность во всех разделах, кроме Отчеты. До завершения выполнения данные в форме 1С-Отчетность могут отображаться некорректно'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Выполняет обновление наименования загруженных отчетов.';
									|en = 'Выполняет обновление наименования загруженных отчетов.'");
	...
	|en = 'Преобразуются файлы избражений сканированных документов, предназначенных для отправки в ФНС. 
	|До завершения обработки рекомендуется воздержаться от формирования и отправки документов в ответ на требование о представлении документов в ФНС'");
	...
	|До завершения обработки рекомендуется воздержаться от формирования и отправки документов в ответ на требование о представлении документов в ФНС';
	|en = 'Заполняются данные единого списка документов для представления в ФНС. 
	|До завершения обработки рекомендуется воздержаться от формирования и отправки документов в ответ на требование о представлении документов в ФНС'");
	...
	|en = 'Добавляются новые виды документов для ответа на требование ФНС. 
	|До завершения обработки рекомендуется воздержаться от формирования и отправки ответов на требование о представлении документов в ФНС'");
	...
	|en = 'Заполнение даты отправки отчетов, уведомлений, выписок ЕГРЮЛ, сверок, писем. 
	|До завершения обработки дата отправки может быть заполнена не везде'");
	Обработчик.НачальноеЗаполнение = Истина;
	...
	Обработчик.Комментарий = НСтр("ru = 'Заменяются коды ТОГС в организациях и обособленных подразделениях';
									|en = 'Заменяются коды ТОГС в организациях и обособленных подразделениях'");
	Обработчик.НачальноеЗаполнение = Истина;
	...
	|en = 'Конвертация параметров хранения закрытого ключа в учетной записи.
	|До завершения обработки может быть заполнена не везде'");
	Обработчик.РежимВыполнения = "Отложенно";
	...
	|До завершения обработки может быть заполнена не везде';
	|en = 'Конвертация параметров хранения закрытого ключа в заявлениях.
	|До завершения обработки может быть заполнена не везде'");
	Обработчик.РежимВыполнения = "Отложенно";
	...
	|До завершения обработки может быть заполнена не везде';
	|en = 'Копирование сертификатов в режиме подписи в модели сервиса в отдельный регистр.
	|До завершения обработки может быть заполнена не везде'");
	Обработчик.РежимВыполнения = "Отложенно";
	...
	Обработчик.Комментарий         = НСтр("ru = 'Дублирование измерения по идентификатору учетной записи.';
											|en = 'Дублирование измерения по идентификатору учетной записи.'");
	Обработчик.РежимВыполнения = "Отложенно";
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение вида входящего документа 1С-Отчетности';
									|en = 'Заполнение вида входящего документа 1С-Отчетности'");
	Обработчик.НачальноеЗаполнение = Истина;
	...
	Обработчик.Комментарий = НСтр("ru = 'Формирование требований ФСС из входящих сообщений';
									|en = 'Формирование требований ФСС из входящих сообщений'");
	Обработчик.НачальноеЗаполнение = Ложь;
	...
	Обработчик.Комментарий = НСтр("ru = 'Формирование данных в регистре сведений журнал отправок в контролирующие органы';
									|en = 'Формирование данных в регистре сведений журнал отправок в контролирующие органы'");
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.РежимВыполнения = "Оперативно";
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "1.2.1.203";
	Обработчик.Процедура           = "Справочники.ШаблоныПоясненийДляФНС.ЗаполнениеТиповыхШаблонов";
	Обработчик.НачальноеЗаполнение = Истина; // Процедура должна запускаться при переходе с 2БП на 3БП
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("431D4EFB-434C-4939-8E7D-60D1515B026B");
	Обработчик.Комментарий = НСтр("ru = 'Заполняет типовые шаблоны ответов на требования ФНС.';
									|en = 'Заполняет типовые шаблоны ответов на требования ФНС.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия     = "1.2.1.211";
	Обработчик.Процедура  = "ДокументооборотСКО.ПереносЗаявленийГосРегистрацииВЕГРЮЛ";
	Обработчик.Комментарий = НСтр("ru = 'Перенос заявлений по гос. регистрации в раздел ЕГРЮЛ';
									|en = 'Перенос заявлений по гос. регистрации в раздел ЕГРЮЛ'");
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор();
	...
	Обработчик.Процедура  = "Справочники.ВидыОтправляемыхДокументов.ИсправитьВидыОтправляемыхДокументовРеестровНДС";
	Обработчик.Комментарий = НСтр("ru = 'Изменение наименований видов отправляемых документов реестров по НДС';
									|en = 'Изменение наименований видов отправляемых документов реестров по НДС'");
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор();
	...
	Обработчик.Процедура  = "Справочники.ВидыОтправляемыхДокументов.ИсправитьВидыОтправляемыхДокументовРеестровАкцизовСФ";
	Обработчик.Комментарий = НСтр("ru = 'Изменение наименований видов отправляемых документов реестров по акцизам, счетов-фактур';
									|en = 'Изменение наименований видов отправляемых документов реестров по акцизам, счетов-фактур'");
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор();
	#КонецОбласти
...
Процедура РазнестиДублиПредопределенныхЭлементовВИБ()
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'БРО. Восстановление предопределенных элементов';
										|en = 'БРО. Восстановление предопределенных элементов'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,
...
Процедура ДобавитьОбработчикиОбновленияОблачнойПодписи(Обработчики, Принудительно = Ложь) Экспорт
	...
	Если КриптографияЭДКО.РаботаВМоделиСервиса() И Принудительно Тогда
		...
		Обработчик.Комментарий         = НСтр("ru = 'Проверяем установку константы ЭП';
												|en = 'Проверяем установку константы ЭП'");
