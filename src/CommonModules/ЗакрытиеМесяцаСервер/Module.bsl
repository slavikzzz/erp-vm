...
Процедура ЗаполнитьОписаниеГруппЭтаповЗакрытияМесяца(ТаблицаГруппЭтапов) Экспорт
	...
	НоваяСтрока.Наименование = НСтр("ru = 'Взаиморасчеты и переоценка валютных средств';
									|en = 'AR/AP accounting and currency revaluation'");
	...
	НоваяСтрока.Наименование = НСтр("ru = 'Формирование финансового результата';
									|en = 'Profitability & cost generation'");
...
Процедура ЗаполнитьОписаниеЭтаповЗакрытияМесяца(ТаблицаЭтапов) Экспорт
	...
	//++ НЕ УТ
	РезервыПредстоящихРасходов.ДобавитьЭтап_НачислениеСписаниеРезервовПредстоящихРасходов(ТаблицаЭтапов, ТекущийРодитель);
	ПроизводствоСервер.ДобавитьЭтап_ОформлениеПроизводстваБезЗаказов(ТаблицаЭтапов, ТекущийРодитель);
	//++ Устарело_Переработка24
	ПереработкаНаСтороне.ДобавитьЭтап_ОстаткиПереработкиНаСтороне2_4(ТаблицаЭтапов, ТекущийРодитель);
	//-- Устарело_Переработка24
	ПереработкаНаСтороне.ДобавитьЭтап_ОстаткиКОформлениюОтчетовПереработчиков(ТаблицаЭтапов, ТекущийРодитель);
	//++ НЕ УТКА
	...
	УправлениеПроизводством.ДобавитьЭтап_ПланированиеПроизводстваПродукцииБезСпецификаций(ТаблицаЭтапов,ТекущийРодитель);
	//-- НЕ УТКА
	ЗатратыСервер.ДобавитьЭтап_РаспределениеМатериаловИРаботНаСебестоимостьПродукции22(ТаблицаЭтапов,ТекущийРодитель);
	ЗатратыСервер.ДобавитьЭтап_НастройкаПоказателейДляРаспределенияРасходовНаСебестоимостьПродукции(ТаблицаЭтапов, ТекущийРодитель);
	ЗатратыСервер.ДобавитьЭтап_НастройкаПоказателейДляРаспределенияРасходовМеждуОВЗ(ТаблицаЭтапов, ТекущийРодитель);
	//-- НЕ УТ
	...
	//++ НЕ УТ
	ПроизводствоСервер.ДобавитьЭтап_ОформлениеВыработкиСотрудников(ТаблицаЭтапов, ТекущийРодитель);
	...
	Документы.РасчетКурсовыхРазниц.ДобавитьЭтап_ПереоценкаАренды(ТаблицаЭтапов,ТекущийРодитель);
	НачислениеСписаниеРезервовПоСомнительнымДолгамСервер.ДобавитьЭтап_ФормированиеРезервовПоСомнительнымДолгам(ТаблицаЭтапов,
		ТекущийРодитель);
	//-- НЕ УТ
	#КонецОбласти
	...
	//++ НЕ УТ
	Документы.РасчетРезервовПодОбесценениеЗапасов.ДобавитьЭтап_ФормированиеРезервовПодОбесценениеЗапасов(
		ТаблицаЭтапов,
		ТекущийРодитель);
	//-- НЕ УТ
...
Процедура Использование_ПереходНаНовыеРежимыРаботы(ПараметрыОбработчика) Экспорт
	...
	ТекстГиперссылкиПоУмолчанию = НСтр("ru = 'Перейти';
										|en = 'Navigate'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если НЕ ПолучитьФункциональнуюОпцию("БазоваяВерсия") И РасчетСебестоимостиПовтИсп.ПартионныйУчетВерсии21() Тогда
		...
		УстановитьСостояниеВыполненСОшибками(
			...
			НСтр("ru = 'Партионный учет (версия 2.2)';
				|en = 'Lot accounting (version 2.2)'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если НЕ ПолучитьФункциональнуюОпцию("БазоваяВерсия") И НЕ ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		УстановитьСостояниеВыполненСОшибками(
			...
			НСтр("ru = 'Взаиморасчеты (онлайн)';
				|en = 'Mutual settlements (online)'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ТекстГиперссылкиПоУмолчанию,
	...
	Если ПроизводствоСервер.НастройкиПодсистемыПроизводство().ИспользуетсяПроизводство21 Тогда
		...
		Если ПроизводствоСервер.НастройкиПодсистемыПроизводство().ИспользуетсяПроизводство22 Тогда
			ТекстПояснения = НСтр("ru = 'Завершить переход на управление производством версии 2.2 и выключить функциональную опцию ""Производство (версия 2.1)""';
									|en = 'End the transfer to v. 2.2. production management and disable the functional option ""Production (v. 2.1.)""'", ОбщегоНазначения.КодОсновногоЯзыка());
		Иначе
			ТекстПояснения = НСтр("ru = 'Производство (версия 2.2)';
									|en = 'Production (v. 2.2)'", ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне")
	 И НЕ ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне2_5") Тогда
		...
		УстановитьСостояниеВыполненСОшибками(
			ПараметрыОбработчика,
			НСтр("ru = 'Передача в переработку (версия 2.5)';
				|en = 'Subcontracting (version 2.5)'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ТекстГиперссылкиПоУмолчанию,
			ОписаниеДействия_ОткрытьФорму(Метаданные.Обработки.ПанельАдминистрированияКА.Формы.Производство.ПолноеИмя()),
			СтатусПоУмолчанию);
	 ...
	 И НЕ ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4") Тогда
		...
		УстановитьСостояниеВыполненСОшибками(
			...
			НСтр("ru = 'Учет внеоборотных активов (версия 2.4)';
				|en = 'Fixed and intangible assets (version 2.4)'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ТекстГиперссылкиПоУмолчанию,
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья")
	 И НЕ ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья2_5") Тогда
		
		УстановитьСостояниеВыполненСОшибками(
			ПараметрыОбработчика,
			НСтр("ru = 'Прием в переработку (версия 2.5)';
				|en = 'Subcontracting services delivered (version 2.5)'"),
			ТекстГиперссылкиПоУмолчанию,
			ОписаниеДействия_ОткрытьФорму(Метаданные.Обработки.ПанельАдминистрированияУТ.Формы.Продажи.ПолноеИмя()),
			СтатусПоУмолчанию);
	...
	Если РасчетСебестоимостиПовтИсп.ПартионныйУчетВерсии22() Тогда
		ШаблонТекста = НСтр("ru = 'Для организации ""%1"" использовать метод оценки стоимости ""ФИФО (скользящая оценка)""';
							|en = 'For the ""%1"" company, use the ""FIFO (rolling valuation)"" method of valuation'", ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		ШаблонТекста = НСтр("ru = 'Для организации ""%1"" использовать метод оценки стоимости ""Средняя за месяц""';
							|en = 'For the ""%1"" company, use the ""Average per month"" method of valuation'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
	...
	Если НЕ СостояниеЭтапаОпределено(ПараметрыОбработчика.ДанныеЭтапа) Тогда
		...
		УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Устаревшие режимы работы не используются.';
				|en = 'Obsolete operation modes are not used.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ИсправлениеОшибокВИсходныхДанных(ТаблицаПроверок)
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Корректность реализации, указанной в возврате.';
			|en = 'Whether the sale specified in the return is correct.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'При изменении задним числом документа реализации товарный состав документа возврата от клиента может рассинхронизироваться с реализацией.
			|В этом случае в документе возврата необходимо или указать корректный документ реализации или вообще не указывать его.';
			|en = 'If sales document is changed post factum, goods content of customer return document can be different from the sale. 
			|In this case, specify correct document in the return document or do not specify it at all.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Корректность указания номеров ГТД, указанных в возврате.';
			|en = 'Correctness of indication of CFD numbers specified in the return.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Были оформлены возвраты товаров, для которых включен учет по номерам ГТД, но номера ГТД не были указаны.
			...
			|In this case you should specify CFD numbers in documents of return.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Актуализация регистра сведений ""Устаревшие виды запасов с остатками""';
			|en = 'Update the ""Obsolete inventory owner attributes with balance"" information register'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		|en = 'Information register contains a list of obsolete inventory owner attributes
		|by which there is balance in the ""Company goods"" register and which should be written off at release.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ОписаниеПроверки = ДобавитьОписаниеНовойПроверки(ТаблицаПроверок,
		...
		Перечисления.ОперацииЗакрытияМесяца.ИсправлениеОшибокВИсходныхДанных,
		Перечисления.МоментЗапускаПроверкиОперацииЗакрытияМесяца.ДоРасчета,
		"ПереработкаНаСтороне.ПроверкаЦеныВозвратныхОтходовВОтчетеПереработчика");
	
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Цены возвратных отходов в документе ""Отчет переработчика""';
			|en = 'Prices of recyclable waste in the ""Subcontractor report"" document'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Цена должна быть заполнена.';
			|en = 'Price should be filled in.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- Устарело_Переработка24
	
	// Выпуск по фиксированной стоимости без выпуска по расчетной в отчете переработчика 2.5
	ОписаниеПроверки = ДобавитьОписаниеНовойПроверки(ТаблицаПроверок,
		"ПобочныйВыпускБезОсновногоВОтчетеПереработчика25",
		Перечисления.ОперацииЗакрытияМесяца.ИсправлениеОшибокВИсходныхДанных,
		Перечисления.МоментЗапускаПроверкиОперацииЗакрытияМесяца.ДоРасчета,
		"ПереработкаНаСтороне.ПроверкаНаПобочныйВыпускБезОсновногоВОтчетеПереработчика25");
	
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Наличие отчетов переработчика с выпуском по фиксированной стоимости без выпуска по расчетной';
			|en = 'Availability of subcontracting services notifications received with release at the fixed cost without release at the estimated cost'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Без выпуска по расчетной стоимости в НЗП могут остаться незакрытые суммы затрат.';
			|en = 'Without release at the estimated cost, unclosed cost amounts might remain in WIP.'", ОбщегоНазначения.КодОсновногоЯзыка()));

	// Указаны сторнированные документы производства без заказа в документах распределения расходов.
	ОписаниеПроверки = ДобавитьОписаниеНовойПроверки(ТаблицаПроверок,
		"КорректностьУказанияПартийПроизводства",
		Перечисления.ОперацииЗакрытияМесяца.ИсправлениеОшибокВИсходныхДанных,
		Перечисления.МоментЗапускаПроверкиОперацииЗакрытияМесяца.ДоРасчета,
		"ЗатратыСервер.ПроверкаКорректностиУказанияПартийПроизводства");
	
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Корректность указания документов производства без заказов в документах распределения расходов.';
			|en = 'Accuracy of backflush production documents specification in the expenses allocation documents.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Указаны документы производства без заказов для которых введено Сторно.';
			|en = 'Specifies backflush production documents with Storno.'", ОбщегоНазначения.КодОсновногоЯзыка())); 	
		
	// Указаны неактуальные партии производства в документах распределения расходов.

	ОписаниеПроверки = ДобавитьОписаниеНовойПроверки(ТаблицаПроверок,
		"АктуальностьУказанныхПартийПроизводства",
		Перечисления.ОперацииЗакрытияМесяца.ИсправлениеОшибокВИсходныхДанных,
		Перечисления.МоментЗапускаПроверкиОперацииЗакрытияМесяца.ДоРасчета,
		"ЗатратыСервер.ПроверкаАктуальностиУказанныхПартийПроизводства");
	
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Корректность указания партий производства в документах распределения расходов.';
			|en = 'Correctness of specification of production lots in the expense allocation documents.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Указаны партии производства, не соответствуюшие документам ""Производство без заказа""..';
			|en = 'You have specified production lots that do not correspond to the ""Backflush production"" documents.'", ОбщегоНазначения.КодОсновногоЯзыка()));
		
	// Указаны одинаковые статьи кулькуляции для материалов и побочного выхода по фиксированной стоимости.
	ОписаниеПроверки = ДобавитьОписаниеНовойПроверки(ТаблицаПроверок,
		"ПравильностьУказанияСтатейКалькуляцийДляПобочногоВыхода",
		Перечисления.ОперацииЗакрытияМесяца.ИсправлениеОшибокВИсходныхДанных,
		Перечисления.МоментЗапускаПроверкиОперацииЗакрытияМесяца.ДоРасчета,
		"ЗатратыСервер.ПроверкаПравильностиУказанияСтатейКалькуляцийДляПобочногоВыхода");
		
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Корректность указания статей калькуляций для побочных изделий.';
			|en = 'Accuracy of product cost element for by-products.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Указаны одинаковые статьи кулькуляции для материалов и побочного выхода по фиксированной стоимости.';
			|en = 'Identical product cost elements are specified for materials and side products at a fixed cost.'", ОбщегоНазначения.КодОсновногоЯзыка())); 	
	
	// В партиях указаны группы продукции, несмотря на отключенную опцию учета по группам
	Если НЕ ПолучитьФункциональнуюОпцию("АналитическийУчетПоГруппамПродукции") Тогда
		ОписаниеПроверки = ДобавитьОписаниеНовойПроверки(ТаблицаПроверок,
			"КорректностьУказанияГруппПродукцииВПартияхПроизводства",
			Перечисления.ОперацииЗакрытияМесяца.ИсправлениеОшибокВИсходныхДанных,
			Перечисления.МоментЗапускаПроверкиОперацииЗакрытияМесяца.ДоИПослеРасчета,
			"ЗатратыСервер.ПроверкаПравильностиУказанияГруппПродукцииВПартияхПроизводства");
			
		ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
			НСтр("ru = 'Корректность указания групп продукции в партиях производства.';
				|en = 'Correctness of specifying manufactured product groups in production lots.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			НСтр("ru = 'В партиях производства используются группы продукции, несмотря на отключенную опцию учета по группам продукции.';
				|en = 'Manufactured product groups are disabled but still manufactured product groups are used in production lots. '", ОбщегоНазначения.КодОсновногоЯзыка())); 	
	КонецЕсли;
	
	// Соответствие отчета переработчика и корректировки отчета переработка.
	ОписаниеПроверки = ДобавитьОписаниеНовойПроверки(ТаблицаПроверок,
		"СоответствиеОтчетаПереработчикаИКорректировкиОтчетаПереработчика",
		Перечисления.ОперацииЗакрытияМесяца.ИсправлениеОшибокВИсходныхДанных,
		Перечисления.МоментЗапускаПроверкиОперацииЗакрытияМесяца.ДоРасчета,
		"ЗатратыСервер.ПроверитьСоответствиеОтчетаПереработчикаИКорректировкиОтчетаПереработчика");
	
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Соответствие отчета переработчика и корректировки отчета переработчика.';
			|en = 'Mapping of the ""Subcontracting services notification received"" and the adjustment of ""Subcontracting services notification received"".'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Заполнение корректировки отчета переработчика должно соответствовать отчету переработчика.';
			|en = 'Filling of the adjustment of ""Subcontracting services notification received"" must correspond to the ""Subcontracting services notification received"".'", ОбщегоНазначения.КодОсновногоЯзыка())); 	
	//-- НЕ УТ
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Корректность заполнения суммовых показателей в документе ""Отчет по комиссии между организациями"".';
			|en = 'Correctness of filling sum indicators in the ""Commission sales report — Intercompany"".'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Оформлены отчеты по комиссии между организациями, у которых суммовые показатели табличной части ""Товары"" не соответствуют данным табличной части ""Виды запасов"".
			...
			|Проблемные документы необходимо перепровести, после этого, возможно, потребуется повторно выполнить закрытие периода.';
			|en = '""Commission reports — Intercompany"" are generated. Sum indicators of the ""Goods"" table do not match the data in the ""Inventory owner attributes"" table.
			|This is due to error No. 00-00190359 that was fixed earlier.
			|To fix this issue, you need to repost the problem documents and afterwards close the period again.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	//++ НЕ УТ

	// Соответствие пар организация/подразделение в документах и ОВЗ
	ОписаниеПроверки = ДобавитьОписаниеНовойПроверки(ТаблицаПроверок,
		"СоответствиеОрганизацияПодразделениеДаннымОВЗ",
		Перечисления.ОперацииЗакрытияМесяца.ИсправлениеОшибокВИсходныхДанных,
		Перечисления.МоментЗапускаПроверкиОперацииЗакрытияМесяца.ДоИПослеРасчета,
		"ЗатратыСервер.ПроверкаСоответствияОрганизацияПодразделениеДаннымОВЗ");
	
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Соответствие организации и подразделения в первичных документах и настройках объектов возникновения затрат.';
			|en = 'Correspondence of the company and business unit in the source documents and settings of cost centers.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'В документе регистрации расхода и указанном объекте возникновения затрат должны совпадать организация и подразделение.';
			|en = 'The company and business unit must match in the expense record document and the specified cost center.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
КонецПроцедуры
...
Процедура ПроверкаКорректностьПередачиУказаннойВВозврате(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Возврат",						НСтр("ru = 'Документ возврата';
																|en = 'Returns document'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("Передача",					НСтр("ru = 'Документ передачи продукции';
															|en = 'Product transfer document'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРегистрации = ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Обнаружены возвращенные от организации ""%1"" товары, отсутствующие в документе передачи товаров';
			|en = 'The products returned from the company ""%1"" were found that were absent in the product transfer document'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"Возврат");
...
Процедура ПроверкаКорректностьЗаполненияНомеровГТДВВозвратах(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Обнаружены возвращенные товары без номеров ГТД в организацию 
			...
			|""%1"" were found without CFD numbers'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"Возврат");
...
Процедура Использование_ЗаблокироватьОтИзменений(ПараметрыОбработчика) Экспорт
	...
	Если НЕ НастройкиБлокировок.ИспользоватьМеханизмБлокировкиОтИзменений Тогда
		...
		УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Механизм блокировки изменений не используется.';
				|en = 'Lock from changes is not used.'"));
	...
	ИначеЕсли НастройкиБлокировок.ПериодМожноЗаблокировать Тогда
		...
		Если ЕстьПравоУправленияБлокировкойПериода() Тогда
			УстановитьСостояниеВыполненСОшибками(
				ПараметрыОбработчика,
				НСтр("ru = 'Основные операции закрытия месяца выполнены, период можно заблокировать от изменений.';
					|en = 'Main month-end closing operations are completed. You can lock the period from changes.'", ОбщегоНазначения.КодОсновногоЯзыка()),
				,
				,
		...
		Иначе
			УстановитьСостояниеНеТребуется(
				...
				НСтр("ru = 'Нет прав на управление блокировкой периодов.';
					|en = 'No rights to manage the period lock.'"));
		КонецЕсли;
	...
	Иначе
		...
		УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Есть невыполненные операции, блокировка невозможна.';
				|en = 'There are uncompleted operations. The lock is unavailable.'"));
		...
		УстановитьСостояниеНеВыполнен(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Требуется пересчет операции %1 %2';
					|en = 'Recalculation of the %1 %2 operation is required'", ОбщегоНазначения.КодОсновногоЯзыка()),
				?(ВыполнятьССамогоРаннего, НСтр("ru = 'начиная с периода';
												|en = 'starting from the period'", ОбщегоНазначения.КодОсновногоЯзыка()), НСтр("ru = 'за период';
																														|en = 'over period'", ОбщегоНазначения.КодОсновногоЯзыка())),
		...
		УстановитьСостояниеНеТребуется(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'За период %1 нет заданий для расчета данной операции и отсутствуют документы ""%2"".';
					|en = 'No jobs for calculating this operation and no ""%2"" documents for period %1.'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ПроверитьНаличиеДокументаРегламентнаяОперация(ПараметрыОбработчика, Знач ТипОперации = Неопределено,
	...
	Результат = ЗакрытиеМесяцаЛокализация.ПроверитьНаличиеДокументаРегламентнаяОперация(ПараметрыОбработчика, 
		ТипОперации,
		ИмяТаблицыОтбораОрганизаций,
		ДляЭтапаИспользуетсяРегистрЗаданий);
...
Функция ОпределитьСостояниеЭтаповРасчета(Этапы, Период, МассивОрганизаций = Неопределено,
	...
	ПараметрыЗапускаОбновленияСостояния.ОбновляемыеЭтапы = ОбщегоНазначенияУТКлиентСервер.Массив(Этапы);
	Если ЗначениеЗаполнено(ОбновитьТолькоОдинЭтап) Тогда
		ОбщегоНазначенияУТКлиентСервер.ДобавитьНовоеЗначениеВМассив(ПараметрыЗапускаОбновленияСостояния.ОбновляемыеЭтапы, ОбновитьТолькоОдинЭтап);
	КонецЕсли;
	...
	МассивЭтапов = ПараметрыЗапускаОбновленияСостояния.ОбновляемыеЭтапы;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
		    ИмяСобытияЖурналаРегистрации(НСтр("ru = 'Увеличение номера задания для расчета';
												|en = 'Increase the number of job for calculation'", ОбщегоНазначения.КодОсновногоЯзыка())),
...
Процедура ОтразитьЗаданияКЗакрытиюМесяца(ИсточникСвойствДокумента, МенеджерВременныхТаблиц) Экспорт
	...
	//++ НЕ УТ
	НачислениеСписаниеРезервовПоСомнительнымДолгамСервер.ДополнитьТекстЗапросаЗаданийКЗакрытиюМесяца(
		Запрос,
		ТекстВложенногоЗапроса,
		ТекстЗапросаВременныхТаблиц,
		ИменаВременныхТаблиц,
	...
	//++ НЕ УТ
	РезервыПредстоящихРасходов.ДополнитьТекстЗапросаЗаданийКЗакрытиюМесяца(
		Запрос,
		ТекстВложенногоЗапроса,
		ТекстЗапросаВременныхТаблиц,
		ИменаВременныхТаблиц);
	...
	//++ НЕ УТ
	УчетРасходовПоИсследованиямИРазработкам.ДополнитьТекстЗапросаЗаданийКЗакрытиюМесяца(
		Запрос,
		ТекстВложенногоЗапроса,
		ТекстЗапросаВременныхТаблиц,
		ИменаВременныхТаблиц);

	ОбесценениеВНАСервер.ДополнитьТекстЗапросаЗаданийКЗакрытиюМесяца(
		Запрос,
		ТекстВложенногоЗапроса,
		ТекстЗапросаВременныхТаблиц,
		ИменаВременныхТаблиц,
	...
	//++ НЕ УТКА
	МеждународныйУчетЗакрытиеМесяца.ДополнитьТекстЗапросаЗаданийКЗакрытиюМесяца(
		Запрос,
		ТекстВложенногоЗапроса,
		ТекстЗапросаВременныхТаблиц,
		ИменаВременныхТаблиц);
...
Функция ДатаЗаданияНаСледующийПериод(Операция, Организация, ДокументЗадания, Знач СледующийМесяц) Экспорт
	...
	//++ НЕ УТ
	Если Операция = Перечисления.ОперацииЗакрытияМесяца.ФормированиеРезервовПоСомнительнымДолгам Тогда
		
		ДатаЗадания = Документы.НачислениеСписаниеРезервовПоСомнительнымДолгам.ПериодРасчетаРезервовПоСомнительнымДолгам(Организация, СледующийМесяц);
				
	ИначеЕсли Операция = Перечисления.ОперацииЗакрытияМесяца.НачислениеПроцентовПоАренде Тогда
		
		ДатаЗадания = Документы.НачислениеПроцентовПоАренде.ПериодНачисленияПроцентовПоАренде(Организация, СледующийМесяц);
				
	ИначеЕсли Операция = Перечисления.ОперацииЗакрытияМесяца.НачислениеПроцентовПоДоходнойАренде Тогда
		
		ДатаЗадания = Документы.НачислениеПроцентовПоАренде.ПериодНачисленияПроцентовПоДоходнойАренде(Организация, СледующийМесяц);
				
	ИначеЕсли Операция = Перечисления.ОперацииЗакрытияМесяца.ЗакрытиеРасходовОтВыбытияОС Тогда
		
		ДатаЗадания = ЗакрытиеРасходовОтВыбытияОС.СледующийПериодОперации(Организация, СледующийМесяц);
				
	ИначеЕсли Операция = Перечисления.ОперацииЗакрытияМесяца.РасчетСтоимостиИнвестицииВАренду Тогда
		
		ДатаЗадания = РасчетСтоимостиИнвестицииВАренду.СледующийПериодОперации(Организация, СледующийМесяц);
...
Функция ТекстЗапросаСостояниеОтраженияДокументов() Экспорт
	...
	Возврат ЗакрытиеМесяцаЛокализация.ТекстЗапросаСостояниеОтраженияДокументов();
...
Функция ИмяСобытияЖурналаРегистрации(ДополнениеИмени) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Закрытие месяца.%1';
			|en = 'Month-end closing.%1'", ОбщегоНазначения.КодОсновногоЯзыка()),
		ДополнениеИмени);
...
Функция ОбработчикиОбновленияБлокирующиеЗакрытиеМесяца() Экспорт
	...
	//++ НЕ УТ
	|РегистрыСведений.ПервоначальныеСведенияНМА.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.ПараметрыАмортизацииНМАБУ.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.ПараметрыАмортизацииНМАУУ.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.ПорядокУчетаНМАБУ.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|Документы.НачислениеПроцентовПоАренде.ОбработатьДанныеДляПереходаНаНовуюВерсию
	//-- НЕ УТ
	|";
...
Функция ОбработчикиОбновленияНеВлияющиеНаЗакрытиеМесяца() Экспорт
	...
	|Документы.ВводОстатковВнеоборотныхАктивов2_4.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|Документы.ПринятиеКУчетуНМА2_4.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|Документы.ИзменениеУсловийДоговораАренды.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|Справочники.Резервы.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.ДокументыПоОС.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.АрендованныеОС.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыНакопления.ДвиженияПоПрочимАктивамПассивам.ОбработатьДанныеДляПереходаНаНовуюВерсию
	//-- НЕ УТ
	|
	//++ НЕ УТКА
	|Документы.ЭтапПроизводства2_2.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.НастройкиСчетовМеждународногоУчетаПоОбъектам.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.ОчередьПроизводственныхОпераций.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыСведений.ЗаданияКОтражениюВБюджетировании.ОбработатьДанныеДляПереходаНаНовуюВерсию
	|РегистрыНакопления.ОбеспечениеПроизводственныхПроцессов.ОбработатьДанныеДляПереходаНаНовуюВерсию
	//-- НЕ УТКА
	|
	|";
...
Процедура ВыполнитьЗакрытиеМесяцаВФоновомЗадании(ОписаниеЗадания, ИдентификаторРасчета) Экспорт
	...
	Для Каждого ТекущийПериод Из ЗакрываемыеПериоды Цикл
		...
		ИмяСобытия = НСтр("ru = 'Закрытие месяца';
		...
		Если СтруктураДанных.ИзмененияЗапрещены Тогда
			...
			Если ЗначениеЗаполнено(ОписаниеЗадания.Организация) Тогда
				...
				ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Регламентное задание закрытия месяца %1 по организации %2 не выполнялось.';
						|en = 'The %1 scheduled job of month-end closing by the %2 company has not been executed.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			Иначе
				...
				ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Регламентное задание закрытия месяца %1 по организациям %2 не выполнялось.';
						|en = 'The %1 scheduled job of month-end closing by the %2 companies has not been executed.'", ОбщегоНазначения.КодОсновногоЯзыка()),
					ЗакрытиеМесяцаКлиентСервер.ПредставлениеПериодаМесяцГод(ТекущийПериод),
		...
		ИначеЕсли ЗначениеЗаполнено(СтруктураДанных.ЗаблокированныеОрганизации) Тогда
			...
			ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Регламентное задание закрытия месяца %1 не выполнялось по заблокированным организациям: %2';
					|en = 'The %1 scheduled job of month-end closing has not been executed by the locked companies: %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ЗакрытиеМесяцаКлиентСервер.ПредставлениеПериодаМесяцГод(ТекущийПериод),
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Удаление помеченных объектов';
					|en = 'Deletion of marked objects'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
...
Процедура ЗаданияКЗакрытиюМесяцаПередЗаписью(Источник, Отказ, Замещение) Экспорт
	...
	//++ НЕ УТ
	СтарыеЗначенияПолейИсточника = Неопределено;
	//-- НЕ УТ
	...
	Для Каждого ТекСтр Из ТаблицаЗаданий Цикл
		...
		Если ЗначениеЗаполнено(ЗаблокироватьПо) И НачалоМесяца(ТекСтр[ИмяПоляПериод]) <= ЗаблокироватьПо Тогда
			...
			//++ НЕ УТ
			РегистрироватьОшибку = ПроверитьИзменениеСтатусаВЗаписи(Источник, СтарыеЗначенияПолейИсточника, ТекСтр.Организация, ТекСтр[ИмяПоляПериод]);
			//-- НЕ УТ
...
Процедура ЗаполнитьПроверкиДляРегистрации(ТаблицаПроверок) Экспорт
	...
	//++ НЕ УТ
	РезервыПредстоящихРасходов.ОписаниеПроверок_НачислениеСписаниеРезервовПредстоящихРасходов(ТаблицаПроверок);
	ПроизводствоСервер.ОписаниеПроверок_ОформлениеПроизводстваБезЗаказов(ТаблицаПроверок);
	//++ Устарело_Переработка24
	ПереработкаНаСтороне.ОписаниеПроверок_ОстаткиПереработкиНаСтороне2_4(ТаблицаПроверок);
	//-- Устарело_Переработка24
	ПереработкаНаСтороне.ОписаниеПроверок_ОстаткиКОформлениюОтчетовПереработчиков(ТаблицаПроверок);
	//++ НЕ УТКА
	...
	УправлениеПроизводством.ОписаниеПроверок_ПланированияПроизводстваПродукцииБезСпецификаций(ТаблицаПроверок);
	//-- НЕ УТКА
	ЗатратыСервер.ОписаниеПроверок_РаспределениеМатериаловИРаботНаСебестоимостьПродукции22(ТаблицаПроверок);
	ЗатратыСервер.ОписаниеПроверок_НастройкаПоказателейДляРаспределенияРасходовНаСебестоимостьПродукции(ТаблицаПроверок);
	ЗатратыСервер.ОписаниеПроверок_НастройкаПоказателейДляРаспределенияРасходовМеждуОВЗ(ТаблицаПроверок);
	//-- НЕ УТ
		...
		ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Проблемы, диагностированные при выполнении операции ""%1""';
					|en = 'Issues found when executing the ""%1"" operation'", ОбщегоНазначения.КодОсновногоЯзыка()),
				МетаЗначение.Синоним),
			НСтр("ru = 'Служебная контекстная проверка для регистрации проблем, диагностированных в ходе расчета.';
				|en = 'Service context check to register issues found during the calculation.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ОписаниеГруппПараметровОперацииЗакрытияМесяца() Экспорт
	...
	ОписаниеГруппы.Наименование = НСтр("ru = 'Настройки для технического специалиста';
										|en = 'Settings for technical specialist'");
	ОписаниеГруппы.Описание 	=
	...
	ОписаниеГруппы.Наименование = НСтр("ru = 'Предварительное закрытие месяца';
										|en = 'Preliminary month-end closing'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Скрыть		= НЕ РасчетСебестоимостиПовтИсп.ПартионныйУчетВерсии22();
	ОписаниеГруппы.Описание 	=
		НСтр("ru = 'Включение / выключение отдельных этапов предварительного закрытия месяца.';
			|en = 'Enable/disable individual stages of preliminary month-end closing.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
	ОписаниеГруппы = ОписаниеГруппПараметров.Добавить();
	ОписаниеГруппы.Имя 			= "ЭтапыОкончательногоЗакрытияМесяца";
	ОписаниеГруппы.Наименование = НСтр("ru = 'Окончательное закрытие месяца';
										|en = 'Final month-end closing'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Скрыть		= НЕ РасчетСебестоимостиПовтИсп.ПартионныйУчетВерсии22();
	ОписаниеГруппы.Описание 	=
		НСтр("ru = 'Параметры этапов окончательного закрытия месяца.';
			|en = 'Parameters of the final month-end closing stages.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
	ОписаниеГруппы = ОписаниеГруппПараметров.Добавить();
	ОписаниеГруппы.Имя 			= "БлокировкаОтИзменений";
	ОписаниеГруппы.Наименование = НСтр("ru = 'Блокировка от изменений';
										|en = 'Lock from changes'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Описание 	=
		НСтр("ru = 'Настройки механизма блокировки закрываемого периода от изменений.';
			|en = 'Settings for the lock of a closing period from changes.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ОписаниеТехнологическихПараметровЗакрытияМесяца() Экспорт
	...
	ОписаниеПараметра.Описание =
		...
		|en = 'If an error of locking data writing occurs when you save the check results, the month-end closing stage is set to the ""Execution is required"" state even if this stage does not have any errors.
		|To solve the issue, you can try saving the check results as many times as the parameter value specifies.'");
	...
	ОписаниеПараметра.Наименование 		  = НСтр("ru = 'Игнорировать ошибку аварийного завершения проверки';
													|en = 'Ignore an abnormal check termination error'");
	ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
	...
	ОписаниеПараметра.Описание =
		...
		|en = 'If an error occurs during the check, for example, due to an error in the check code, the month-end closing stage is set to the ""Execution is required"" state even if this stage does not have any errors.
		|To ignore the issue until the configuration patch is released, you can set this parameter to ""Yes"".'");
	...
	ОписаниеПараметра.Описание =
		...
		|en = 'Allows you to set internal parameters for month-end close
		|Internal parameters are intended for investigating calculation problems only.
		|Save the parameters and reopen the parameter settings form for this parameter to take effect.'");
	...
	#Область ЭтапыПредварительногоЗакрытияМесяца
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru = 'Распределение дополнительных расходов по товарам';
														|en = 'Additional expenses allocation by goods'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Истина;
		...
		ОписаниеПараметра.Описание =
			...
			|Позволяет получить себестоимость товаров с учетом зарегистрированных дополнительных расходов.';
			|en = 'Execution of the additional expenses allocation by goods.
			|Allows you to get the inventory cost taking into account the registered additional expenses.'", ОбщегоНазначения.КодОсновногоЯзыка());
			
		// РаспределениеПостатейныхРасходовНаПродажу
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "РаспределениеПостатейныхРасходовНаПродажу";
		ОписаниеПараметра.Наименование 		  = НСтр("ru = 'Распределение расходов на продажу';
														|en = 'Sales expense allocation'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Истина;
		ОписаниеПараметра.Скрыть			  = НЕ РасчетСебестоимостиПовтИсп.ПартионныйУчетВерсии22();
		ОписаниеПараметра.Описание =
			НСтр("ru = 'Выполнение операции распределения постатейных расходов на продажу.
			|Позволяет получить валовую прибыль с учетом зарегистрированных расходов на продажу.';
			|en = 'Execution of the itemized sales expense allocation.
			|Allows you to get a gross profit taking into account the registered sales expenses.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	#Область ЭтапыОкончательногоЗакрытияМесяца
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru = 'Восстанавливать хронологическую последовательность списания партий по ""ФИФО""';
														|en = 'Restore the chronological sequence of writing off lots based on ""FIFO""'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Истина;
		ОписаниеПараметра.Описание =
			...
			|привело к нарушению хронологической последовательности списания партий.';
			|en = 'The full lot allocation will be performed without saving the results of the preliminary calculation.
			|Allows you to restore the chronological sequence of writing off the lots based on ""FIFO"".
			|Makes sense if there were ""retroactive"" changes in documents in the current month after the preliminary month-end closing and this
			| caused a violation of the chronological sequence of writing off lots.'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru = 'Выделять отклонения себестоимости окончательного расчета от результата предварительного расчета';
														|en = 'Highlight cost deviations in the final calculation from the preliminary calculation result'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь;
		ОписаниеПараметра.Описание =
			...
			|Включение данного параметра увеличивает количество движений в регистрах и может приводить к увеличению времени закрытия месяца.';
			|en = 'The deviations of the final calculation from the preliminary calculation result will be calculated when calculating the cost.
			|The deviations will be recorded in separate records in the accumulation registers:
			|""Inventory cost"", ""Revenue and cost of goods sold"", ""Other expenses"" with reference to the internal document ""Cost calculation recorder"".
			|Enabling this parameter increases the number of register records in the registers and may cause the month-end closing time to increase.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	ОписаниеПараметра.Наименование 		  = НСтр("ru = 'Использовать механизм блокировки закрытия месяца от изменений';
													|en = 'Use the lock of month-end closing from changes'");
	ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
	ОписаниеПараметра.ЗначениеПоУмолчанию = Истина;
	ОписаниеПараметра.Описание =
		НСтр("ru = 'Позволяет включить/отключить использование механизма блокировки';
			|en = 'Allows you to enable or disable the lock'");
...
Процедура ДобавитьИнформациюОНезарегистрированныхОбъектахКЗафиксированнойПроблеме(ПараметрыПроверки, ОписаниеПроблемы, КоличествоОбъектов) Экспорт
	...
	Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Еще найдено аналогичных проблем: %1';
			|en = 'Similar issues found: %1'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурналаРегистрации(НСтр("ru = 'Отмена выполнения фонового задания';
													|en = 'Cancel background job'", ОбщегоНазначения.КодОсновногоЯзыка())),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ТекстОшибки);
...
Функция РегистрыЗаданийДляЗакрытияМесяца() Экспорт
	...
	//++ НЕ УТ
	Результат.Вставить(Метаданные.РегистрыСведений.ЗаданияКРасчетуАмортизацииНМА);
	Результат.Вставить(Метаданные.РегистрыСведений.ЗаданияКРасчетуАмортизацииОС);
	Результат.Вставить(Метаданные.РегистрыСведений.ЗаданияКРасчетуСтоимостиВНА);
	Результат.Вставить(Метаданные.РегистрыСведений.ЗаданияКФормированиюДвиженийПоВНА);
	//++ НЕ УТКА
	Результат.Вставить(Метаданные.РегистрыСведений.ОтражениеДокументовВМеждународномУчете, ПолучитьФункциональнуюОпцию("ИспользоватьМеждународныйФинансовыйУчет"));
	//-- НЕ УТКА
	
	//++ Локализация
	Результат.Вставить(Метаданные.РегистрыСведений.ЗаданияКПогашениюСтоимостиТМЦВЭксплуатации);
	Результат.Вставить(Метаданные.РегистрыСведений.ОтражениеДокументовВРеглУчете, ПолучитьФункциональнуюОпцию("ИспользоватьРеглУчет"));
	//-- Локализация
...
Процедура ТестированиеЗакрытияМесяца(Период) Экспорт
	...
	Если СтруктураДанных.ИзмененияЗапрещены Тогда
		 ...
		 Шаблон1 = НСтр("ru = '%Парам1%';
						|en = '%Парам1%'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Иначе
			...
			Задание = ФоновыеЗадания.Выполнить(
				...
				НСтр("ru = 'Тестирование этапа закрытия месяца';
					|en = 'Test month-end closing stage'"));
			...
			Попытка
				...
				Если Задание.Состояние <> СостояниеФоновогоЗадания.Завершено Тогда
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						|%3';
						|en = 'Background job for stage calculation %1 %2:
...
Процедура ДобавитьИнформациюОЗаданииВЗаблокированномПериоде(Организация, ПериодЗадания, ПериодБлокировки, ИмяРегистраЗаданий, Отказ)
	...
	Если СтруктураДанных = Неопределено Тогда
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Изменение данных в заблокированном периоде';
				|en = 'Change data in the locked period'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
...
Процедура ПроверитьСформированныеЗаданияПриПроведенииДокумента(Документ, Отказ) Экспорт
	...
	ТекстОшибки = НСтр("ru = 'Невозможно изменение документа';
						|en = 'Cannot change the document'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
...
Функция ТекстОшибкиЗаписиЗаданияВЗаблокированномПериоде(Организация, ПериодЗадания, ПериодБлокировки, ИмяРегистраЗаданий = Неопределено, ИзменяемыйДокумент = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИмяРегистраЗаданий) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно изменение документа ""%1"" организации %2 в периоде %3: установлена блокировка от изменения до периода %4';
				|en = 'Cannot change the %1 document of the %2 company in the period %3: the lock from changes is set until %4'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ИзменяемыйДокумент,
			Организация,
	...
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно добавить запись в регистр %1 по организации %2 в периоде %3: установлена блокировка от изменения до периода %4';
				|en = 'Cannot add a record to the %1 register by the %2 company in the period %3: the lock from changes is set until %4'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ИмяРегистраЗаданий,
			Организация,
			ЗакрытиеМесяцаКлиентСервер.ПредставлениеПериодаМесяцГод(ПериодЗадания),
			ЗакрытиеМесяцаКлиентСервер.ПредставлениеПериодаМесяцГод(КонецМесяца(ПериодБлокировки) + 1));
...
// Проверить изменение статуса в записи.
// 
// Параметры:
//  Источник - РегистрСведенийНаборЗаписей.ЗаданияКРасчетуСебестоимости, РегистрСведенийНаборЗаписей.ЗаданияКЗакрытиюМесяца, РегистрСведенийНаборЗаписей.ЗаданияКРасчетуАмортизацииОС, РегистрСведенийНаборЗаписей.ЗаданияКРасчетуСтоимостиВНА, РегистрСведенийНаборЗаписей.ЗаданияКПогашениюСтоимостиТМЦВЭксплуатации, РегистрСведенийНаборЗаписей.ЗаданияКРаспределениюРасчетовСКлиентами, РегистрСведенийНаборЗаписей.ОтражениеДокументовВРеглУчете, РегистрСведенийНаборЗаписей.ЗаданияКФормированиюДвиженийПоВНА, РегистрСведенийНаборЗаписей.ЗаданияКРаспределениюРасчетовСПоставщиками, РегистрСведенийНаборЗаписей.ОтражениеДокументовВМеждународномУчете, РегистрСведенийНаборЗаписей.ЗаданияКФормированиюДвиженийПоНДС, РегистрСведенийНаборЗаписей.ЗаданияКРасчетуАмортизацииНМА - Источник
//  СтарыеЗначенияПолейИсточника - ТаблицаЗначений - Старые значения полей источника
//  Организация - СправочникСсылка.организации - Организация
//  Период - Дата - Период
// 
// Возвращаемое значение:
//  Булево - Проверить изменение статуса в записи
Функция ПроверитьИзменениеСтатусаВЗаписи(Источник, СтарыеЗначенияПолейИсточника, Организация, Период)
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("РегистрСведенийНаборЗаписей.ОтражениеДокументовВРеглУчете") Тогда
		
		ИмяРегистра = "РегистрСведений.ОтражениеДокументовВРеглУчете";
		
	//-- Локализация
	
	//++ НЕ УТКА
	
	ИначеЕсли ТипЗнч(Источник) = Тип("РегистрСведенийНаборЗаписей.ОтражениеДокументовВМеждународномУчете") Тогда

		ИмяРегистра = "РегистрСведений.ОтражениеДокументовВМеждународномУчете";
	...
	Если СтарыеЗначенияПолейИсточника = Неопределено Тогда
		...
		"ВЫБРАТЬ
		|	Т.Организация КАК Организация,
		|	Т.ДатаОтражения КАК ДатаОтражения,
		|	Т.Статус КАК Статус
		|ИЗ
		|	ИмяРегистра КАК Т
		|ГДЕ
		...
		СтарыеЗначенияПолейИсточника = Запрос.Выполнить().Выгрузить();
		
	КонецЕсли;
	
	СтарыеСтроки = СтарыеЗначенияПолейИсточника.НайтиСтроки(Новый Структура("Организация, ДатаОтражения", Организация, Период));
	
	Если СтарыеСтроки.Количество() = 1 Тогда
		
		ТаблицаИсточника = Источник.Выгрузить(, "Организация, ДатаОтражения, Статус");
		НовыеСтроки = ТаблицаИсточника.НайтиСтроки(Новый Структура("Организация, ДатаОтражения", Организация, Период));

		Если НовыеСтроки.Количество() = 1 Тогда
			Если СтарыеСтроки[0].Статус = НовыеСтроки[0].Статус Тогда
				Результат = Ложь;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Очищает неиспользуемые данные в служебных регистрах механизма закрытия месяца';
									|en = 'Clears unused data in internal registers of the month-end closing'");
