...
Функция ПолучитьСведенияОПоказателяхОтчета(ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета = Неопределено) Экспорт
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетУСН" Тогда
		...
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2021Кв1" Или ИДРедакцииОтчета = "ФормаОтчета2023Кв1"
			Или ИДРедакцииОтчета = "ФормаОтчета2024Кв1" Тогда
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетИмущество" Тогда
		...
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2019Кв1" 
			...
			РегламентированнаяОтчетностьУП.ПолучитьСведенияОПоказателяхОтчетаИмуществоФормаОтчета2019_2024(ПоказателиОтчета, ИДРедакцииОтчета, ПараметрыОтчета);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетДоходыИностраннымОрганизациям" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОтчета2023Кв4" Тогда
...
Процедура ЗаполнитьОтчет(ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета, Контейнер) Экспорт
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетИмущество" Тогда
		...
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2022Кв4" Тогда
			...
			Иначе
				...
				РегламентированнаяОтчетностьУП.ЗаполнитьОтчетИмуществоФормаОтчета2022_НовыйРасчет(ПараметрыОтчета, Контейнер, ИДРедакцииОтчета);
			КонецЕсли;
			
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2024Кв4" Тогда
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетДоходыИностраннымОрганизациям" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОтчета2023Кв4" Тогда
			РегламентированнаяОтчетностьУП.ЗаполнитьРегламентированныйОтчетДоходыИностраннымОрганизациям(ПараметрыОтчета, Контейнер);
		КонецЕсли;	
...
Функция ЭтоЮридическоеЛицо(Организация) Экспорт
	...
	РегламентированнаяОтчетностьУП.ПолучитьЗначениеРеквизитаОрганизации(Организация, "ЮридическоеФизическоеЛицо", Значение, Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо);
...
Функция ЭтоИностраннаяОрганизация(Организация) Экспорт
	...
	РегламентированнаяОтчетностьУП.ПолучитьЗначениеРеквизитаОрганизации(Организация, "ОтделениеИностраннойОрганизации", Значение, Ложь);
		...
		Пока Выборка.Следующий() Цикл
			...
			Для Каждого СтрокаОбъекта Из СтрокиОбъектов Цикл
				...
				СтрокаОбъекта.Наименование = НСтр("ru = 'Журнал учета счетов-фактур для передачи в электронном виде';
													|en = 'Tax invoice register for electronic filing'");
		...
		Пока Выборка.Следующий() Цикл
			...
			Для Каждого СтрокаОбъекта Из СтрокиОбъектов Цикл
				...
				СтрокаОбъекта.Наименование = НСтр("ru = 'Заявление о ввозе товаров (на основании поступления)';
													|en = 'Application for goods import (based on receipt)'");
...
//       НастраиваемаяБухгалтерскаяОтчетность.ДополнитьСтруктуруМногострочныхЧастей(
