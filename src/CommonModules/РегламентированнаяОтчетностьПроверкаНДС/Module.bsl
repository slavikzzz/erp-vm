...
// декларации по НДС. Предназначена для редакций декларации по НДС, соответствующих формам "ФормаОтчета2025Кв1",
// "ФормаОтчета2023Кв1","ФормаОтчета2022Кв1", "ФормаОтчета2021Кв3", "ФормаОтчета2020Кв4", "ФормаОтчета2019Кв1" и
// "ФормаОтчета2017Кв1" отчета "РегламентированныйОтчетНДС".
// Параметры:
...
Процедура ВыявитьОшибкиЗаполненияПрепятствующиеФормированиюЭлектронногоПредставления(Отчет, МакетОформления, НаименованиеДекларации, АдресХранилища) Экспорт
	...
	ДанныеОтчета = СформироватьСтруктуруПоказателей(Отчет);
	ПараметрыВыгрузки = ПараметрыВыгрузки(Отчет, ДанныеОтчета);
	
	КонтекстВыполнения = Новый Структура;
	КонтекстВыполнения.Вставить("Отчет", Отчет);
	КонтекстВыполнения.Вставить("ИмяФормы", СокрЛП(Отчет.ВыбраннаяФорма));
	...
	ПроверитьЗаполнение_ТитульныйЛист(КонтекстВыполнения);
	ПроверитьЗаполнение_Раздел1(КонтекстВыполнения);
	ПроверитьЗаполнение_Раздел2(КонтекстВыполнения);
	ПроверитьЗаполнение_Раздел3Приложение1(КонтекстВыполнения);
	ПроверитьЗаполнение_Раздел3Приложение2(КонтекстВыполнения);
	ПроверитьЗаполнение_Раздел4(КонтекстВыполнения);
	ПроверитьЗаполнение_Раздел5(КонтекстВыполнения);
	ПроверитьЗаполнение_Раздел6(КонтекстВыполнения);
	ПроверитьЗаполнение_Раздел7(КонтекстВыполнения);
	
	ПроверитьРазделыНДС(КонтекстВыполнения);
	
	СформироватьПредставлениеОшибок(КонтекстВыполнения);
...
Функция СформироватьСтруктуруПоказателей(Отчет)
	...
	ЭтоОтчетСтарогоОбразца = СохрСтруктура.Свойство("ОкружениеСохранения");
	...
	Для Каждого Раздел Из ПоказателиОтчета Цикл
		Если ЭтоОтчетСтарогоОбразца Тогда
			...
			ВосстановитьСтруктуруДанныхРазделаСтарогоОбразца(
			Отчет, ПоказателиОтчета, Раздел.Ключ, ИмяРаздела);
		КонецЕсли;
...
Процедура ВосстановитьСтруктуруДанныхРазделаСтарогоОбразца(Отчет, ПоказателиОтчета, ИмяСтруктурыДанных, ИмяРаздела)
...
Функция ПараметрыВыгрузки(Отчет, ДанныеОтчета)
	
	ИмяФормы = СокрЛП(Отчет.ВыбраннаяФорма);
	
	Титульный = ДанныеОтчета.ПолеТабличногоДокументаТитульный;
	
	ПараметрыВыгрузки = Новый Структура;
	
	Если ВРег(ИмяФормы) = ВРег("ФормаОтчета2019Кв1")
		ИЛИ ВРег(ИмяФормы) = ВРег("ФормаОтчета2017Кв1") Тогда
	...
	НЕ РегламентированнаяОтчетностьВызовСервера.ЭтоЮридическоеЛицо(Отчет.Организация));
	
	СведенияОбОрганизации
	= РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
	Отчет.Организация, ТекущаяДатаСеанса(), "ФИОБух, ТелБух");
	...
	ПараметрыВыгрузки.Вставить("АктуальностьСведенийРаздел8",
	Формат(СохраненныеДанные.АктуальностьСведенийРаздел8, "ЧН="));
	ПараметрыВыгрузки.Вставить("АктуальностьСведенийРаздел8_Прил1",
	Формат(СохраненныеДанные.АктуальностьСведенийРаздел8_Прил1, "ЧН="));
	ПараметрыВыгрузки.Вставить("АктуальностьСведенийРаздел9",
	Формат(СохраненныеДанные.АктуальностьСведенийРаздел9, "ЧН="));
	ПараметрыВыгрузки.Вставить("АктуальностьСведенийРаздел9_Прил1",
	Формат(СохраненныеДанные.АктуальностьСведенийРаздел9_Прил1, "ЧН="));
	ПараметрыВыгрузки.Вставить("АктуальностьСведенийРаздел10",
	Формат(СохраненныеДанные.АктуальностьСведенийРаздел10, "ЧН="));
	ПараметрыВыгрузки.Вставить("АктуальностьСведенийРаздел11",
	Формат(СохраненныеДанные.АктуальностьСведенийРаздел11, "ЧН="));
	ПараметрыВыгрузки.Вставить("АктуальностьСведенийРаздел12",
	Формат(СохраненныеДанные.АктуальностьСведенийРаздел12, "ЧН="));
	...
	Если СохраненныеДанные.Свойство("ОкружениеСохранения")
		И НЕ ПараметрыВыгрузки.ДанныеСегментированы И ОтчетЗаполнялсяСДелениемНаСекции(Отчет) Тогда
		ПараметрыВыгрузки.Вставить("ДанныеСегментированы", Истина);
	КонецЕсли;
...
Процедура ПроверитьЗаполнение_ТитульныйЛист(КонтекстВыполнения)
	...
	РазрешенныеКодыПериода
	= "01,02,03,04,05,06,07,08,09,10,11,12,21,22,23,24,51,54,55,56,71,72,73,74,75,76,77,78,79,80,81,82";
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2019Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2017Кв1") Тогда
...
Процедура ПроверитьЗаполнение_Раздел1(КонтекстВыполнения)
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2023Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2022Кв1") Тогда
		
		Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1")
			ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2023Кв1") Тогда
			
			РазрешенныеКодыОснования = "01, 02, 03, 04, 05, 06, 07, 08";
			
		Иначе// "ФормаОтчета2022Кв1"
			
			РазрешенныеКодыОснования = "01, 02, 03, 04, 05, 06, 07";
			
		КонецЕсли;
		
		ВозможныеКодыОснования = СтрРазделить(РазрешенныеКодыОснования, ", ", Ложь);
		
		ЗаполненКодОснования = Ложь;
		
		НомерСтроки = 1;
		Пока ДанныеРаздела.Свойство("П0001М1005503_" + СтрокаЧГ0(НомерСтроки)) Цикл
			Постфикс = "_" + СтрокаЧГ0(НомерСтроки);
			СтрокаЗаполнена = ЗначениеЗаполнено(ДанныеРаздела["П0001М1005503" + Постфикс]);
			ЗаполненКодОснования = ЗаполненКодОснования ИЛИ СтрокаЗаполнена;
			Если СтрокаЗаполнена
				И ВозможныеКодыОснования.Найти(СокрЛП(ДанныеРаздела["П0001М1005503" + Постфикс])) = Неопределено Тогда
				ОписаниеОшибки
				= "В строке 055 раздела 1 указан некорректный код основания применения заявительного порядка возмещения налога."
				+ " Возможны значения: " + РазрешенныеКодыОснования + ".";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел1", "П0001М1005503" + Постфикс);
			КонецЕсли;
			НомерСтроки = НомерСтроки + 1;
		КонецЦикла;
		
		Если НЕ ЗаполненКодОснования Тогда
			
			Если ЗначениеЗаполнено(Окр(ДанныеРаздела.П000100005603, 0)) Тогда
				ОписаниеОшибки = "Строка 056 раздела 1 не должна быть заполнена, если не заполнена строка 055.";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел1", "П000100005603");
			КонецЕсли;
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2023Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2022Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2021Кв3")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2020Кв4") Тогда
		
		ПризнакСЗПК = СокрЛП(ДанныеРаздела.П000100008503);
		
		Если НЕ ЗначениеЗаполнено(ПризнакСЗПК) Тогда
			ОписаниеОшибки = "В строке 085 раздела 1 не указан признак СЗПК.";
			ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел1", "П000100008503");
		ИначеЕсли НЕ (ПризнакСЗПК = "1" ИЛИ ПризнакСЗПК = "2") Тогда
			ОписаниеОшибки = "В строке 085 раздела 1 указан некорректный признак СЗПК. Возможны значения: 1, 2.";
			ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел1", "П000100008503");
		КонецЕсли;
		
		ЗначениеСтроки090 = Окр(ДанныеРаздела.П000100009003, 0);
		ЗначениеСтроки095 = Окр(ДанныеРаздела.П000100009503, 0);
		
		Если ЗначениеЗаполнено(ЗначениеСтроки090) И ЗначениеЗаполнено(ЗначениеСтроки095) Тогда
			
			ОписаниеОшибки = "Не допускается одновременное заполнение строк 090 и 095 раздела 1.";
			ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел1", "П000100009003");
			
		ИначеЕсли ПризнакСЗПК = "2" Тогда
			
			Если ЗначениеЗаполнено(ЗначениеСтроки090) Тогда
				ОписаниеОшибки = "Строка 090 раздела 1 не должна быть заполнена при значении признака СЗПК, равном 2.";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел1", "П000100009003");
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ЗначениеСтроки095) Тогда
				ОписаниеОшибки = "Строка 095 раздела 1 не должна быть заполнена при значении признака СЗПК, равном 2.";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел1", "П000100009503");
			КонецЕсли;
...
Процедура ПроверитьЗаполнение_Раздел2(КонтекстВыполнения)
	...
	Если ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2025Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2023Кв1") Тогда
		
		ВозможныеКодыОпераций_Стр
		= "1011703, 1011705, 1011707, 1011709, 1011714, 1011715, 1011716, 1011717, 1011718, 1011719, 1011720, 1011721, 1011722";
		
	ИначеЕсли ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2022Кв1") Тогда
		
		Если КонтекстВыполнения.Отчет.ДатаОкончания < '20220701' Тогда
			ВозможныеКодыОпераций_Стр
			= "1011703, 1011705, 1011707, 1011709, 1011711, 1011712, 1011714, 1011715, 1011716, 1011717, 1011718";
		ИначеЕсли КонтекстВыполнения.Отчет.ДатаОкончания < '20221001' Тогда
			ВозможныеКодыОпераций_Стр
			= "1011703, 1011705, 1011707, 1011709, 1011711, 1011712, 1011714, 1011715, 1011716, 1011717, 1011718, 1011719";
		Иначе
			ВозможныеКодыОпераций_Стр
			= "1011703, 1011705, 1011707, 1011709, 1011711, 1011712, 1011714, 1011715, 1011716, 1011717, 1011718, 1011719, 1011720";
		КонецЕсли;
		
	ИначеЕсли ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2021Кв3")
		ИЛИ ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2020Кв4")
		ИЛИ (ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2019Кв1")
		И Год(КонтекстВыполнения.Отчет.ДатаОкончания) >= 2020) Тогда
	...
	Иначе
		...
		ВозможныеКодыОпераций_Стр = "1011703, 1011705, 1011707, 1011709, 1011711, 1011712, 1011714, 1011715";
		
	КонецЕсли;
	
	ВозможныеКодыОпераций = СтрРазделить(ВозможныеКодыОпераций_Стр, ", ");
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела Цикл
		...
		Если НЕ ЗаполненаСтраница_Раздел2(ДанныеСтраницы) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2025Кв1")
			ИЛИ ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2023Кв1")
			ИЛИ ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2022Кв1")
			ИЛИ ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2021Кв3")
			ИЛИ ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2020Кв4")
			ИЛИ ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2019Кв1") Тогда
			
			Если КодОперации <> "1011715" Тогда
				...
				Если ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2025Кв1")
					ИЛИ ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2023Кв1") Тогда
					
					ДолжноБытьЗаполненоНаименованиеПродавца
					= КодОперации = "1011721" ИЛИ КодОперации = "1011703" ИЛИ КодОперации = "1011722";
					
				Иначе// "ФормаОтчета2022Кв1", "ФормаОтчета2021Кв3", "ФормаОтчета2020Кв4", "ФормаОтчета2019Кв1"
					
					ДолжноБытьЗаполненоНаименованиеПродавца = КодОперации = "1011712" ИЛИ КодОперации = "1011703";
					
				КонецЕсли;
				
				// Наименование налогоплательщика-продавца.
				Если ДолжноБытьЗаполненоНаименованиеПродавца И НЕ ЗначениеЗаполнено(ДанныеСтраницы.П000200002003) Тогда
		...
		Если ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2025Кв1")
			...
			// Заполнение ИНН при кодах операции 1011703, 1011722.
			Если (КодОперации = "1011703" ИЛИ КодОперации = "1011722") И НЕ ЗначениеЗаполнено(ДанныеСтраницы.П000200003003) Тогда
				ОписаниеОшибки = "В разделе 2 на странице " + НомерСтраницы + " не указан ИНН налогоплательщика - продавца.";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел2", "П000200003003", НомерСтраницы);
			КонецЕсли;
			
		Иначе// "ФормаОтчета2022Кв1", "ФормаОтчета2021Кв3", "ФормаОтчета2020Кв4", "ФормаОтчета2019Кв1", "ФормаОтчета2017Кв1"
			
			// Заполнение ИНН при коде операции 1011703.
			Если КодОперации = "1011703" И НЕ ЗначениеЗаполнено(ДанныеСтраницы.П000200003003) Тогда
				ОписаниеОшибки = "В разделе 2 на странице " + НомерСтраницы + " не указан ИНН налогоплательщика - продавца.";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел2", "П000200003003", НомерСтраницы);
			КонецЕсли;
		...
		Иначе
			...
			Если ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2025Кв1")
				...
				Если ВозможныеКодыОпераций.Найти(КодОперации) = Неопределено Тогда
					ОписаниеОшибки = "В разделе 2 на странице " + НомерСтраницы
					+ " указан некорректный код операции. Возможны значения: " + ВозможныеКодыОпераций_Стр + ".";
					ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел2", "П000200007003", НомерСтраницы);
				КонецЕсли;
				
			Иначе
				
				Если СтрДлина(КодОперации) <> 7 Тогда
...
Функция ЗаполненаСтраница_Раздел2(ДанныеСтраницы)
	...
	СтраницаЗаполнена = ЗначениеЗаполнено(ДанныеСтраницы.П000200001003)
	                ...
	                ИЛИ ЗначениеЗаполнено(ДанныеСтраницы.П000200004003)
...
Процедура ПроверитьЗаполнение_Раздел3Приложение1(КонтекстВыполнения)
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела Цикл
		...
		Если НЕ ЗаполненаСтраница_Раздел3_Прил1(ДанныеСтраницы) Тогда
			Продолжить;
		...
		НаимООС = СокрЛП(ДанныеСтраницы.П000310001001);
		Если ПустаяСтрока(НаимООС) Тогда
			ОписаниеОшибки = "На странице " + НомерСтраницы + " приложения 1 к разделу 3 не указано наименование объекта основного средства.";
			ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел3_Прил1", "П000310001001", НомерСтраницы);
		КонецЕсли;
		
		КодОперации = СокрЛП(ДанныеСтраницы.П000310002001);
		Если НЕ ЗначениеЗаполнено(КодОперации) Тогда
			ОписаниеОшибки = "На странице " + НомерСтраницы + " приложения 1 к разделу 3 не указан код операции по объекту основного средства.";
		...
		ИначеЕсли СтрДлина(КодОперации) <> 7 Тогда
			ОписаниеОшибки = "На странице " + НомерСтраницы + " приложения 1 к разделу 3 указан код операции по объекту основного средства, не состоящий из 7 символов.";
			ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел3_Прил1", "П000310002001", НомерСтраницы);
		КонецЕсли;
		
		ПроверяемаяДата = ДанныеСтраницы.П000310003001;
		Если НЕ ЗначениеЗаполнено(ПроверяемаяДата) Тогда
			ОписаниеОшибки = "На странице " + НомерСтраницы + " приложения 1 к разделу 3 не указана дата ввода объекта основного средства в эксплуатацию для исчисления амортизации.";
			ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел3_Прил1", "П000310003001");
		Иначе
			ОшибкиВДате = ОшибкиТипа_ДатаТип(ПроверяемаяДата);
			Если ОшибкиВДате = "ВыходитЗаПериод" Тогда
				ОписаниеОшибки = "На странице " + НомерСтраницы + " приложения 1 к разделу 3 дата ввода объекта основного средства в эксплуатацию для исчисления амортизации значительно (более, чем на 80 лет) отличается от текущей.";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел3_Прил1", "П000310003001");
			КонецЕсли;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ПроверяемаяДата) Тогда
			ОписаниеОшибки = "На странице " + НомерСтраницы + " приложения 1 к разделу 3 не указана дата начала амортизационных отчислений по объекту основного средства.";
			ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел3_Прил1", "П000310004001", НомерСтраницы);
		Иначе
			ОшибкиВДате = ОшибкиТипа_ДатаТип(ПроверяемаяДата);
			Если ОшибкиВДате = "ВыходитЗаПериод" Тогда
				ОписаниеОшибки = "На странице " + НомерСтраницы + " приложения 1 к разделу 3 дата дата начала амортизационных отчислений по объекту основного средства значительно (более, чем на 80 лет) отличается от текущей.";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел3_Прил1", "П000310004001", НомерСтраницы);
			КонецЕсли;
		КонецЕсли;
		
		Если МногострочнаяЧастьПуста(ДанныеСтраницы, "П000310007001, П000310007002, П000310007003, П000310007004", "П000310007004") Тогда
			...
			ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел3_Прил1", "П000310007001_1", НомерСтраницы);
		КонецЕсли;
		
		НомерСтроки = 1;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеСтраницы, "П000310007001_" + Формат(НомерСтроки, "ЧГ=")) Цикл
			П000310007001 = СокрЛП(ДанныеСтраницы["П000310007001_" + СтрокаЧГ0(НомерСтроки)]);
			П000310007002 = ДанныеСтраницы["П000310007002_" + СтрокаЧГ0(НомерСтроки)];
			П000310007003 = ДанныеСтраницы["П000310007003_" + СтрокаЧГ0(НомерСтроки)];
			П000310007004 = ДанныеСтраницы["П000310007004_" + СтрокаЧГ0(НомерСтроки)];
			
			Если ЗначениеЗаполнено(П000310007001) ИЛИ ЗначениеЗаполнено(П000310007002) ИЛИ ЗначениеЗаполнено(П000310007003) ИЛИ ЗначениеСуммыЗаполнено(П000310007004) Тогда
				Если НЕ ЗначениеЗаполнено(П000310007001) Тогда
					ОписаниеОшибки = "На странице " + НомерСтраницы + " приложения 1 к разделу 3 в строке " + НомерСтроки + " не указан год.";
					ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел3_Прил1", "П000310007001_" + СтрокаЧГ0(НомерСтроки), НомерСтраницы);
				ИначеЕсли СтрДлина(П000310007001) <> 4 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(П000310007001) Тогда
					ОписаниеОшибки = "На странице " + НомерСтраницы + " приложения 1 к разделу 3 в строке " + НомерСтроки + " указан год, не состоящий из 4 цифр.";
					ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел3_Прил1", "П000310007001_" + СтрокаЧГ0(НомерСтроки), НомерСтраницы);
				КонецЕсли;
				
				ПроверяемаяДата = П000310007002;
				Если ЗначениеЗаполнено(ПроверяемаяДата) Тогда
					ОшибкиВДате = ОшибкиТипа_ДатаТип(ПроверяемаяДата);
					Если ОшибкиВДате = "ВыходитЗаПериод" Тогда
						ОписаниеОшибки = "На странице " + НомерСтраницы + " приложения 1 к разделу 3 в строке " + НомерСтроки + " дата начала использования объекта основного средства значительно (более, чем на 80 лет) отличается от текущей.";
						ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел3_Прил1", "П000310007002_" + СтрокаЧГ0(НомерСтроки), НомерСтраницы);
					КонецЕсли;
				КонецЕсли;
...
Функция ЗаполненаСтраница_Раздел3_Прил1(ДанныеСтраницы)
	
	СтраницаЗаполнена = ЗначениеЗаполнено(ДанныеСтраницы.П000310001001)
	                ...
	                ИЛИ ЗначениеСуммыЗаполнено(ДанныеСтраницы.П000310005001)
	                ИЛИ ЗначениеСуммыЗаполнено(ДанныеСтраницы.П000310006001);
	
	НомерСтроки = 1;
	Пока НЕ СтраницаЗаполнена И ДанныеСтраницы.Свойство("П000310007001_" + СтрокаЧГ0(НомерСтроки)) Цикл
		Постфикс = "_" + СтрокаЧГ0(НомерСтроки);
		СтраницаЗаполнена = ЗначениеЗаполнено(ДанныеСтраницы["П000310007001" + Постфикс])
		                ИЛИ ЗначениеЗаполнено(ДанныеСтраницы["П000310007002" + Постфикс])
		                ИЛИ ЗначениеЗаполнено(ДанныеСтраницы["П000310007003" + Постфикс])
		                ИЛИ ЗначениеСуммыЗаполнено(ДанныеСтраницы["П000310007004" + Постфикс]);
		НомерСтроки = НомерСтроки + 1;
	КонецЦикла;
...
Процедура ПроверитьЗаполнение_Раздел3Приложение2(КонтекстВыполнения)
...
Процедура ПроверитьЗаполнение_Раздел4(КонтекстВыполнения)
	
	ДанныеОтчета = КонтекстВыполнения.ДанныеОтчета;
	ДанныеРаздела = ДанныеОтчета.ПолеТабличногоДокументаРаздел4;
	
	НомерСтроки = 1;
	Пока ДанныеРаздела.Свойство("П0004М1001003_" + СтрокаЧГ0(НомерСтроки)) Цикл
		Постфикс = "_" + СтрокаЧГ0(НомерСтроки);
		СтрокаЗаполнена = ЗначениеЗаполнено(ДанныеРаздела["П0004М1001003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0004М1002003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0004М1003003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0004М1004003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0004М1005003" + Постфикс]);
		Если СтрокаЗаполнена Тогда
			КодОперации = СокрЛП(ДанныеРаздела["П0004М1001003" + Постфикс]);
			Если НЕ ЗначениеЗаполнено(КодОперации) Тогда
				ОписаниеОшибки = "В разделе 4 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " группы строк 010-050 не указан код операции.";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки,
				"Раздел4", "П0004М1001003" + Постфикс);
			ИначеЕсли СтрДлина(КодОперации) <> 7 Тогда
				ОписаниеОшибки = "В разделе 4 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " группы строк 010-050 указан код операции, не состоящий из 7 символов.";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки,
				"Раздел4", "П0004М1001003" + Постфикс);
			КонецЕсли;
		КонецЕсли;
	...
	НомерСтроки = 1;
	Пока ДанныеРаздела.Свойство("П0004М2006003_" + СтрокаЧГ0(НомерСтроки)) Цикл
		Постфикс = "_" + СтрокаЧГ0(НомерСтроки);
		СтрокаЗаполнена = ЗначениеЗаполнено(ДанныеРаздела["П0004М2006003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0004М2007003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0004М2008003" + Постфикс]);
		Если СтрокаЗаполнена Тогда
			КодОперации = СокрЛП(ДанныеРаздела["П0004М2006003" + Постфикс]);
			Если НЕ ЗначениеЗаполнено(КодОперации) Тогда
				ОписаниеОшибки = "В разделе 4 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " группы строк 060-080 не указан код операции.";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки,
				"Раздел4", "П0004М2006003" + Постфикс);
			ИначеЕсли СтрДлина(КодОперации) <> 7 Тогда
				ОписаниеОшибки = "В разделе 4 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " группы строк 060-080 указан код операции, не состоящий из 7 символов.";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки,
				"Раздел4", "П0004М2006003" + Постфикс);
			КонецЕсли;
		КонецЕсли;
	...
	НомерСтроки = 1;
	Пока ДанныеРаздела.Свойство("П0004М3009003_" + СтрокаЧГ0(НомерСтроки)) Цикл
		Постфикс = "_" + СтрокаЧГ0(НомерСтроки);
		СтрокаЗаполнена = ЗначениеЗаполнено(ДанныеРаздела["П0004М3009003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0004М3010003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0004М3011003" + Постфикс]);
		Если СтрокаЗаполнена Тогда
			КодОперации = СокрЛП(ДанныеРаздела["П0004М3009003" + Постфикс]);
			Если НЕ ЗначениеЗаполнено(КодОперации) Тогда
				ОписаниеОшибки = "В разделе 4 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " группы строк 090-110 не указан код операции.";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки,
				"Раздел4", "П0004М3009003" + Постфикс);
			ИначеЕсли СтрДлина(КодОперации) <> 7 Тогда
				ОписаниеОшибки = "В разделе 4 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " группы строк 090-110 указан код операции, не состоящий из 7 символов.";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки,
				"Раздел4", "П0004М3009003" + Постфикс);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПроверитьЗаполнение_Раздел5(КонтекстВыполнения)
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела Цикл
		...
		Если НЕ ЗаполненаСтраница_Раздел5(ДанныеСтраницы) Тогда
			Продолжить;
		КонецЕсли;
		...
		Иначе
				...
				Если ЗначениеСуммыЗаполнено(ДанныеСтраницы["П000500004003_" + СтрИндексСтроки])
				 ...
				 ИЛИ ЗначениеЗаполнено(ДанныеСтраницы["П000500003003_" + СтрИндексСтроки]) Тогда
					...
					Если ЗначениеСуммыЗаполнено(ДанныеСтраницы["П000500006003_" + СтрИндексСтроки])
						...
						ОписаниеОшибки = "В разделе 5 на странице " + НомерСтраницы + " в строке " + СтрИндексСтроки + " не задана сумма налоговых вычетов (строка 070).";
			...
			// Итог в строке 080 не может быть пуст, если соотв. данные заполнены хотя бы в одной строке (условие из формата).
			Если НЕ ЗначениеСуммыЗаполнено(ДанныеСтраницы.П000500008003) Тогда
				...
				Пока ДанныеСтраницы.Свойство("П000500005003_" + Формат(ИндексСтроки, "ЧГ=")) Цикл
					Если ЗначениеСуммыЗаполнено(ДанныеСтраницы["П000500005003_" + Формат(ИндексСтроки, "ЧГ=")]) Тогда
						ОписаниеОшибки = "В разделе 5 на странице " + НомерСтраницы + " не указана итоговая сумма (строка 080).";
						ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел5", "П000500008003", НомерСтраницы);
						Прервать;
					КонецЕсли;
					ИндексСтроки = ИндексСтроки + 1;
				КонецЦикла;
			...
			// Итог в строке 090 не может быть пуст, если соотв. данные заполнены хотя бы в одной строке (условие из формата).
			Если НЕ ЗначениеСуммыЗаполнено(ДанныеСтраницы.П000500009003) Тогда
				...
				Пока ДанныеСтраницы.Свойство("П000500007003_" + Формат(ИндексСтроки, "ЧГ=")) Цикл
					Если ЗначениеСуммыЗаполнено(ДанныеСтраницы["П000500007003_" + Формат(ИндексСтроки, "ЧГ=")]) Тогда
						ОписаниеОшибки = "В разделе 5 на странице " + НомерСтраницы + " не указана итоговая сумма (строка 090).";
						ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел5", "П000500009003", НомерСтраницы);
						Прервать;
					КонецЕсли;
					ИндексСтроки = ИндексСтроки + 1;
				КонецЦикла;
...
Функция ЗаполненаСтраница_Раздел5(ДанныеСтраницы)
...
Процедура ПроверитьЗаполнение_Раздел6(КонтекстВыполнения)
	
	ДанныеОтчета = КонтекстВыполнения.ДанныеОтчета;
	ДанныеРаздела = ДанныеОтчета.ПолеТабличногоДокументаРаздел6;
	
	НомерСтроки = 1;
	Пока ДанныеРаздела.Свойство("П0006М1001003_" + СтрокаЧГ0(НомерСтроки)) Цикл
		Постфикс = "_" + СтрокаЧГ0(НомерСтроки);
		СтрокаЗаполнена = ЗначениеЗаполнено(ДанныеРаздела["П0006М1001003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0006М1002003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0006М1003003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0006М1004003" + Постфикс]);
		Если СтрокаЗаполнена Тогда
			КодОперации = СокрЛП(ДанныеРаздела["П0006М1001003" + Постфикс]);
			Если НЕ ЗначениеЗаполнено(КодОперации) Тогда
				ОписаниеОшибки = "В разделе 6 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " группы строк 010-040 не указан код операции";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки,
				"Раздел6", "П0006М1001003" + Постфикс);
			ИначеЕсли СтрДлина(КодОперации) <> 7 Тогда
				ОписаниеОшибки = "Код операции в разделе 6 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " группы строк 010-040 должен состоять из 7 символов";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки,
				"Раздел6", "П0006М1001003" + Постфикс);
			КонецЕсли;
		КонецЕсли;
	...
	// Если заполнена хотя бы одна строка 040, то должен быть заполнен итог в строке 060.
	Если НЕ ЗначениеСуммыЗаполнено(ДанныеРаздела.П000600006003) И НЕ МногострочнаяЧастьПуста(ДанныеРаздела, "", "П0006М1004003") Тогда
	...
	НомерСтроки = 1;
	Пока ДанныеРаздела.Свойство("П0006М2007003_" + СтрокаЧГ0(НомерСтроки)) Цикл
		Постфикс = "_" + СтрокаЧГ0(НомерСтроки);
		СтрокаЗаполнена = ЗначениеЗаполнено(ДанныеРаздела["П0006М2007003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0006М2008003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0006М2009003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0006М2010003" + Постфикс]);
		Если СтрокаЗаполнена Тогда
			КодОперации = СокрЛП(ДанныеРаздела["П0006М2007003" + Постфикс]);
			Если НЕ ЗначениеЗаполнено(КодОперации) Тогда
				ОписаниеОшибки = "В разделе 6 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " группы строк 070-100 не указан код операции";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки,
				"Раздел6", "П0006М2007003" + Постфикс);
			ИначеЕсли СтрДлина(КодОперации) <> 7 Тогда
				ОписаниеОшибки = "Код операции в разделе 6 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " группы строк 070-100 должен состоять из 7 символов";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки,
				"Раздел6", "П0006М2007003" + Постфикс);
			КонецЕсли;
		КонецЕсли;
	...
	НомерСтроки = 1;
	Пока ДанныеРаздела.Свойство("П0006М3011003_" + СтрокаЧГ0(НомерСтроки)) Цикл
		Постфикс = "_" + СтрокаЧГ0(НомерСтроки);
		СтрокаЗаполнена = ЗначениеЗаполнено(ДанныеРаздела["П0006М3011003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0006М3012003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0006М3013003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0006М3014003" + Постфикс])
		ИЛИ ЗначениеСуммыЗаполнено(ДанныеРаздела["П0006М3015003" + Постфикс]);
		Если СтрокаЗаполнена Тогда
			КодОперации = СокрЛП(ДанныеРаздела["П0006М3011003" + Постфикс]);
			Если НЕ ЗначениеЗаполнено(КодОперации) Тогда
				ОписаниеОшибки = "В разделе 6 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " группы строк 110-150 не указан код операции";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки,
				"Раздел6", "П0006М3011003" + Постфикс);
			ИначеЕсли СтрДлина(КодОперации) <> 7 Тогда
				ОписаниеОшибки = "Код операции в разделе 6 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " группы строк 110-150 должен состоять из 7 символов";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки,
				"Раздел6", "П0006М3011003" + Постфикс);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПроверитьЗаполнение_Раздел7(КонтекстВыполнения)
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
		ДанныеРаздела, "П000700000101_" + Формат(НомерСтроки, "ЧГ=")) Цикл
		...
		Если ЗначениеЗаполнено(П000700000101) ИЛИ ЗначениеСуммыЗаполнено(П000700000102)
			ИЛИ ЗначениеСуммыЗаполнено(П000700000103) ИЛИ ЗначениеСуммыЗаполнено(П000700000104) Тогда
			ДлинаП000700000101 = СтрДлина(П000700000101);
			Если ДлинаП000700000101 = 0 Тогда
				ОписаниеОшибки = "В разделе 7 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " не указан код операции";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки,
				"Раздел7", "П000700000101_" + СтрокаЧГ0(НомерСтроки));
			ИначеЕсли ДлинаП000700000101 <> 7 Тогда
				ОписаниеОшибки = "Код операции в разделе 7 в строке " + СтрокаЧГ0(НомерСтроки)
				+ " должен состоять из 7 символов";
				ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел7",
				"П000700000101_" + СтрокаЧГ0(НомерСтроки));
			КонецЕсли;
		КонецЕсли;
		НомерСтроки = НомерСтроки + 1;
	КонецЦикла;
...
Процедура ПроверитьРазделыНДС(КонтекстВыполнения)
	...
	Если ПараметрыВыгрузки.ПоМесту = "231" Тогда
		...
		Для Каждого ЛистРаздела Из Раздел2 Цикл
			...
			Если ЗаполненаСтраница_Раздел2(ДанныеЛиста) Тогда
	...
	Если ПроверятьРаздел8 Тогда
		ПроверитьЗаполнение_Раздел8(КонтекстВыполнения, ДанныеРаздел8);
	КонецЕсли;
	ПроверитьЗаполнение_Раздел8_Прил1(КонтекстВыполнения, ДанныеРаздел8_Прил1);
	
	Если ПроверятьРаздел9 Тогда
		ПроверитьЗаполнение_Раздел9(КонтекстВыполнения, ДанныеРаздел9);
	КонецЕсли;
	ПроверитьЗаполнение_Раздел9_Прил1(КонтекстВыполнения, ДанныеРаздел9_Прил1);
	
	ПроверитьЗаполнение_Раздел10(КонтекстВыполнения, ДанныеРаздел10);
	ПроверитьЗаполнение_Раздел11(КонтекстВыполнения, ДанныеРаздел11);
	ПроверитьЗаполнение_Раздел12(КонтекстВыполнения, ДанныеРаздел12);
	
КонецПроцедуры

Процедура ПроверитьЗаполнение_Раздел8(КонтекстВыполнения, ДанныеРаздела)
	...
	ДопустимыеКодыОперации = СтрРазделить(КодыОпераций("Раздел8", КонтекстВыполнения), ",");
	КодыПоПродавцам        = СтрРазделить(КодыОперацийПоПродавцам(КонтекстВыполнения), ",");
	КодыПоНепродавцам      = СтрРазделить(КодыОперацийПоНепродавцам(КонтекстВыполнения), ",", Ложь);
	...
	КонтрольныеСписки.Вставить("КодыПоНепродавцам",      КодыПоНепродавцам);
	...
	Если ТипЗнч(ДанныеРаздела) = Тип("Массив") Тогда
		...
		Для Каждого НомерПервойСтрокиСекции Из ДанныеРаздела Цикл
			...
			Для Каждого СтрокаДанных ИЗ СтрокиСегментаРаздела Цикл
				...
				ПроверитьЗаполнениеСтрокиДанных_Раздел8(СтрокаДанных, НомерСтроки, КонтекстВыполнения);
...
Процедура ПроверитьЗаполнение_Раздел8_Прил1(КонтекстВыполнения, ДанныеРаздела)
	...
	ДопустимыеКодыОперации = СтрРазделить(КодыОпераций("Раздел8_Прил1", КонтекстВыполнения), ",");
	КодыПоПродавцам        = СтрРазделить(КодыОперацийПоПродавцам(КонтекстВыполнения), ",");
	КодыПоНепродавцам      = СтрРазделить(КодыОперацийПоНепродавцам(КонтекстВыполнения), ",", Ложь);
...
Процедура ПроверитьЗаполнениеСтрокиДанных_Раздел8(СтрокаДанных, НомерСтроки, КонтекстВыполнения)
	...
	КодыПоНепродавцам = КонтрольныеСписки.КодыПоНепродавцам;
	...
	ТребуетсяНеУказатьПродавца = Истина;
	...
	Для Каждого Операция Из СтрокаДанных.КодВидОпер Цикл
		...
		УказанНекорректныйКодВидаОперации
		= УказанНекорректныйКодВидаОперации ИЛИ ДопустимыеКодыОперации.Найти(Операция) = Неопределено;
		ТребуетсяУказатьПродавца = ТребуетсяУказатьПродавца ИЛИ КодыПоПродавцам.Найти(Операция) <> Неопределено;
		ТребуетсяНеУказатьПродавца = ТребуетсяНеУказатьПродавца И КодыПоНепродавцам.Найти(Операция) <> Неопределено;
	КонецЦикла;
	
	Если СтрокаДанных.КодВидОпер.Количество() = 0 Тогда
		ТребуетсяНеУказатьПродавца = Ложь;
	КонецЕсли;
	...
	ИначеЕсли УказанНекорректныйКодВидаОперации Тогда
		ЗафиксироватьОшибку812(КонтекстВыполнения, "НекорректныйКодОперации" + ИмяРаздела);
	КонецЕсли;
	...
	Для каждого ПроверяемаяДата Из СтрокаДанных.ДатаУчТов Цикл
		ОшибкиВДате = ОшибкиТипа_ДатаТип(ПроверяемаяДата);
		Если ОшибкиВДате = "ВыходитЗаПериод" Тогда
			ЗафиксироватьОшибку812(КонтекстВыполнения, "ДатаВнеДиапазона");
		КонецЕсли;
	...
	Для Каждого СведенияОПродавце Из СтрокаДанных.СвПрод Цикл
		...
		Если НЕ ТребуетсяНеУказатьПродавца Тогда
	...
	Если ТребуетсяУказатьПродавца И НЕ УказаныСведенияОПродавце Тогда
		...
		Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1")
			...
			ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2022Кв1")
			ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2021Кв3")
			ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2020Кв4")
			ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2019Кв1") Тогда
			
			Ошибка = "НеУказаныРеквизитыКонтрагентаРФ";
			
		Иначе// "ФормаОтчета2017Кв1"
			
			Если Год(КонтекстВыполнения.Отчет.ДатаОкончания) >= 2018 Тогда
				Ошибка = "НеУказаныРеквизитыКонтрагентаРФ_20180101";
			Иначе
				Ошибка = "НеУказаныРеквизитыКонтрагентаРФ";
			КонецЕсли;
	...
	Если ТребуетсяНеУказатьПродавца И УказаныСведенияОПродавце Тогда
		Ошибка = "УказаныРеквизитыКонтрагентаРФ";
		ЗафиксироватьОшибку812(КонтекстВыполнения, Ошибка);
	КонецЕсли;
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2023Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2022Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2021Кв3") Тогда
		
		ПроверитьСведенияОРегНомереДеклНаТоварыИлиОПрослеживаемомТоваре(СтрокаДанных, КонтекстВыполнения);
		
	Иначе// "ФормаОтчета2020Кв4", "ФормаОтчета2019Кв1", "ФормаОтчета2017Кв1"
		
		УказанНекорректныйРегистрационныйНомерТаможеннойДекларации = Ложь;
		
		Для Каждого РегистрационныйНомерТД Из СтрокаДанных.РегНомТД Цикл
			УказанНекорректныйРегистрационныйНомерТаможеннойДекларации
			= УказанНекорректныйРегистрационныйНомерТаможеннойДекларации
			ИЛИ НекорректныйРегистрационныйНомерТаможеннойДекларации(РегистрационныйНомерТД, КонтекстВыполнения);
		КонецЦикла;
		
		Если УказанНекорректныйРегистрационныйНомерТаможеннойДекларации Тогда
			ЗафиксироватьОшибку812(КонтекстВыполнения, "НекорректныйРегистрационныйНомерТаможеннойДекларации");
		КонецЕсли;
...
Процедура ПроверитьЗаполнение_Раздел9(КонтекстВыполнения, ДанныеРаздела)
	...
	СписокДопустимыхКодовОпераций = КодыОпераций("Раздел9", КонтекстВыполнения);
	...
	Если ТипЗнч(ДанныеРаздела) = Тип("Массив") Тогда
		...
		Для Каждого НомерПервойСтрокиСекции Из ДанныеРаздела Цикл
			...
			Для Каждого СтрокаДанных ИЗ СтрокиСегментаРаздела Цикл
				НомерСтроки = СтрокаЧГ0(ОбщийНомерСтроки);
				ПроверитьЗаполнениеСтрокиДанных_Раздел9(СтрокаДанных, НомерСтроки, КонтекстВыполнения);
...
Процедура ПроверитьЗаполнение_Раздел9_Прил1(КонтекстВыполнения, ДанныеРаздела)
	...
	СписокДопустимыхКодовОпераций = КодыОпераций("Раздел9_Прил1", КонтекстВыполнения);
	ДопустимыеКодыОперации = СтрРазделить(СписокДопустимыхКодовОпераций, ",");
...
Процедура ПроверитьЗаполнениеСтрокиДанных_Раздел9(СтрокаДанных, НомерСтроки, КонтекстВыполнения)
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1")
		...
		УказанНекорректныйКодВидаТовара = Ложь;
		Для Каждого КодВидаТовара Из СтрокаДанных.КодВидТовар Цикл
			УказанНекорректныйКодВидаТовара = УказанНекорректныйКодВидаТовара ИЛИ НекорректныйКодВидаТовара(КодВидаТовара);
		КонецЦикла;
		Если УказанНекорректныйКодВидаТовара Тогда
			ЗафиксироватьОшибку812(КонтекстВыполнения, "НекорректныйКодВидаТовара");
		КонецЕсли;
...
Процедура ПроверитьЗаполнение_Раздел10(КонтекстВыполнения, ДанныеРаздела)
	...
	СписокДопустимыхКодовОпераций = КодыОпераций("Раздел10", КонтекстВыполнения);
	ДопустимыеКодыОперации = СтрРазделить(СписокДопустимыхКодовОпераций, ",");
	...
	Если ТипЗнч(ДанныеРаздела) = Тип("Массив") Тогда
		...
		Для Каждого НомерПервойСтрокиСекции Из ДанныеРаздела Цикл
			...
			Для Каждого СтрокаДанных ИЗ СтрокиСегментаРаздела Цикл
				НомерСтроки = СтрокаЧГ0(ОбщийНомерСтроки);
				ПроверитьЗаполнениеСтрокиДанных_Раздел10(СтрокаДанных, НомерСтроки, КонтекстВыполнения);
...
Процедура ПроверитьЗаполнениеСтрокиДанных_Раздел10(СтрокаДанных, НомерСтроки, КонтекстВыполнения)
	...
	Если ЗначениеЗаполнено(СтрокаДанных.СвСчФОтПрод) Тогда
		
		Для Каждого СведенияОПосредническойДеятельности Из СтрокаДанных.СвСчФОтПрод Цикл
			...
			Если НЕ ЗначениеЗаполнено(СведенияОПосредническойДеятельности.НомСчФОтПрод)
				...
				ЗафиксироватьОшибку812(КонтекстВыполнения, "НетСФОтПродавца");
			ИначеЕсли НЕ ЗначениеЗаполнено(СведенияОПосредническойДеятельности.НомСчФОтПрод) Тогда
				ЗафиксироватьОшибку812(КонтекстВыполнения, "НеУказанНомерСФОтПродавца");
			Иначе
				Если НЕ ЗначениеЗаполнено(СведенияОПосредническойДеятельности.ДатаСчФОтПрод) Тогда
					ЗафиксироватьОшибку812(КонтекстВыполнения, "НеУказанаДатаСФОтПродавца");
				Иначе
					ОшибкиВДате = ОшибкиТипа_ДатаТип(СведенияОПосредническойДеятельности.ДатаСчФОтПрод);
					Если ОшибкиВДате = "ВыходитЗаПериод" Тогда
						ЗафиксироватьОшибку812(КонтекстВыполнения, "ДатаВнеДиапазона");
					КонецЕсли;
				КонецЕсли;
			...
			Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1")
				...
				ПроверитьСведенияОРегНомереДеклНаТоварыИлиОПрослеживаемомТоваре(
				СведенияОПосредническойДеятельности, КонтекстВыполнения);
	...
	Иначе
		...
		ЗафиксироватьОшибку812(КонтекстВыполнения, "НетСведенийИзСФПолученныхОтПродавцов");
		
	КонецЕсли;
	
КонецПроцедуры

Процедура ПроверитьЗаполнение_Раздел11(КонтекстВыполнения, ДанныеРаздела)
	...
	СписокДопустимыхКодовОпераций = КодыОпераций("Раздел11", КонтекстВыполнения);
	ДопустимыеКодыОперации = СтрРазделить(СписокДопустимыхКодовОпераций, ",");
	...
	Если ТипЗнч(ДанныеРаздела) = Тип("Массив") Тогда
		...
		Для Каждого НомерПервойСтрокиСекции Из ДанныеРаздела Цикл
			...
			Для Каждого СтрокаДанных ИЗ СтрокиСегментаРаздела Цикл
				НомерСтроки = СтрокаЧГ0(ОбщийНомерСтроки);
				ПроверитьЗаполнениеСтрокиДанных_Раздел11(СтрокаДанных, НомерСтроки, КонтекстВыполнения);
...
Процедура ПроверитьЗаполнениеСтрокиДанных_Раздел11(СтрокаДанных, НомерСтроки, КонтекстВыполнения)
...
Процедура ПроверитьЗаполнение_Раздел12(КонтекстВыполнения, ДанныеРаздела)
	...
	СписокДопустимыхКодовОпераций = КодыОпераций("Раздел12", КонтекстВыполнения);
	ДопустимыеКодыОперации = СтрРазделить(СписокДопустимыхКодовОпераций, ",");
	...
	Если ТипЗнч(ДанныеРаздела) = Тип("Массив") Тогда
		...
		Для Каждого НомерПервойСтрокиСекции Из ДанныеРаздела Цикл
			...
			Для Каждого СтрокаДанных ИЗ СтрокиСегментаРаздела Цикл
				НомерСтроки = СтрокаЧГ0(ОбщийНомерСтроки);
				ПроверитьЗаполнениеСтрокиДанных_Раздел12(СтрокаДанных, НомерСтроки, КонтекстВыполнения);
...
Процедура ПроверитьЗаполнениеСтрокиДанных_Раздел12(СтрокаДанных, НомерСтроки, КонтекстВыполнения)
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1") Тогда
		...
		Если НЕ УказанКодВидаОперации Тогда
			ЗафиксироватьОшибку812(КонтекстВыполнения, "НеУказанКодОперации");
		ИначеЕсли УказанНекорректныйКодВидаОперации Тогда
			ЗафиксироватьОшибку812(КонтекстВыполнения, "НекорректныйКодОперацииРаздел12");
		КонецЕсли;
...
Процедура СформироватьПредставлениеОшибок(КонтекстВыполнения)
	...
	Иначе
		ВывестиОшибки(КонтекстВыполнения, ПредставлениеОшибок, "ОшибкиТитульногоЛиста");
		ВывестиОшибки(КонтекстВыполнения, ПредставлениеОшибок, "ОшибкиОтчета");
		ВывестиОшибки(КонтекстВыполнения, ПредставлениеОшибок, "ОшибкиРазделов17");
...
Процедура ВывестиОшибки(КонтекстВыполнения, ПредставлениеОшибок, ИмяТаблицыОшибок)
...
Процедура ПроверитьСведенияОРегНомереДеклНаТоварыИлиОПрослеживаемомТоваре(СтрокаДанных, КонтекстВыполнения)
	
	ОшибкиСпискаСвРегНом = Новый Структура;
	ОшибкиСпискаСвРегНом.Вставить("НеУказанРНДТИлиРНПТ", Ложь);
	ОшибкиСпискаСвРегНом.Вставить("НекорректныйРНДТИлиРНПТ", Ложь);
	ОшибкиСпискаСвРегНом.Вставить("НекорректныйКодПоОКЕИ", Ложь);
	
	Для Каждого Сведения Из СтрокаДанных.СвРегНом Цикл
		
		Если НЕ ЗначениеЗаполнено(Сведения.РегНомПросл) Тогда
			ОшибкиСпискаСвРегНом.Вставить("НеУказанРНДТИлиРНПТ", Истина);
		ИначеЕсли НекорректныйРегистрационныйНомерТаможеннойДекларации(Сведения.РегНомПросл, КонтекстВыполнения) Тогда
			ОшибкиСпискаСвРегНом.Вставить("НекорректныйРНДТИлиРНПТ", Истина);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Сведения.ОКЕИ) Тогда
			КодПоОКЕИ = СокрЛП(Сведения.ОКЕИ);
			ДлинаКодаПоОКЕИ = СтрДлина(КодПоОКЕИ);
			Если ДлинаКодаПоОКЕИ < 3 ИЛИ ДлинаКодаПоОКЕИ > 4
				ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодПоОКЕИ)Тогда
				ОшибкиСпискаСвРегНом.Вставить("НекорректныйКодПоОКЕИ", Истина);
			КонецЕсли;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ОшибкиСпискаСвРегНом.НеУказанРНДТИлиРНПТ Тогда
		ЗафиксироватьОшибку812(КонтекстВыполнения, "НеУказанРНДТИлиРНПТ");
	КонецЕсли;
	
	Если ОшибкиСпискаСвРегНом.НекорректныйРНДТИлиРНПТ Тогда
		ЗафиксироватьОшибку812(КонтекстВыполнения, "НекорректныйРНДТИлиРНПТ");
	КонецЕсли;
	
	Если ОшибкиСпискаСвРегНом.НекорректныйКодПоОКЕИ Тогда
		ЗафиксироватьОшибку812(КонтекстВыполнения, "НекорректныйКодПоОКЕИ");
	КонецЕсли;
	
КонецПроцедуры

Функция КодыОпераций(ИмяРаздела, КонтекстВыполнения)
	
	ГодОтчета = Год(КонтекстВыполнения.Отчет.ДатаОкончания);
	
	Если ИмяРаздела = "Раздел8" ИЛИ ИмяРаздела = "Раздел8_Прил1" Тогда
		
		Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1") Тогда
			
			Возврат "01,02,06,13,15,16,17,18,19,20,22,23,24,25,26,27,28,32,34,36,41,42,43,44,45,46";
			
		ИначеЕсли ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2023Кв1") Тогда
			
			Возврат "01,02,06,13,15,16,17,18,19,20,22,23,24,25,26,27,28,32,34,36,41,42,43,44,45,46,48,49,50,51";
			
		ИначеЕсли ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2022Кв1")
			ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2021Кв3")
			ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2020Кв4")
			ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2019Кв1") Тогда
			
			Возврат "01,02,06,13,15,16,17,18,19,20,22,23,24,25,26,27,28,32,34,36,41,42,43,44,45";
			
		Иначе// "ФормаОтчета2017Кв1"
			
			Если ГодОтчета >= 2018 Тогда
				Возврат "01,02,06,13,15,16,17,18,19,20,22,23,24,25,26,27,28,32,34,36,41,42,43,44";
			Иначе
				Возврат "01,02,06,13,15,16,17,18,19,20,22,23,24,25,26,27,28,32";
			КонецЕсли;
			
		КонецЕсли;
		
	ИначеЕсли ИмяРаздела = "Раздел9" ИЛИ ИмяРаздела = "Раздел9_Прил1" Тогда
		
		Если НачалоДня(КонтекстВыполнения.Отчет.ДатаОкончания) > '20240331' Тогда
			Возврат "01,02,06,10,13,14,15,18,21,26,27,28,29,30,31,33,34,35,37,38,39,40,41,42,43,44,47";
		ИначеЕсли ГодОтчета >= 2018 Тогда
			Возврат "01,02,06,10,13,14,15,18,21,26,27,28,29,30,31,33,34,35,37,38,39,40,41,42,43,44";
		Иначе
			Возврат "01,02,06,10,13,14,15,18,21,26,27,28,29,30,31";
		КонецЕсли;
		
	ИначеЕсли ИмяРаздела = "Раздел10" ИЛИ ИмяРаздела = "Раздел11" Тогда
		
		Если ГодОтчета >= 2018 Тогда
			Возврат "01,02,13,15,18,19,20,27,28,29,30,33,34,37,38,39,40";
		Иначе
			Возврат "01,02,13,15,18,19,20,27,28,29,30";
		КонецЕсли;
...
Функция КодыОперацийПоПродавцам(КонтекстВыполнения)
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1")
		...
		Возврат "01,02,13,15,16,17,18,22,24,25,26,32,34,36,41,42,43,44,45";
	...
	Иначе// "ФормаОтчета2017Кв1"
		...
		Если ГодОтчета >= 2018 Тогда
			Возврат "01,02,13,15,16,17,18,22,24,25,26,27,28,32,34,36,41,42,43,44";
		Иначе
			Возврат "01,02,13,15,16,17,18,22,24,25,26,27,28,32";
		КонецЕсли;
...
Функция КодыОперацийПоНепродавцам(КонтекстВыполнения)
	...
	Иначе// "ФормаОтчета2019Кв1", "ФормаОтчета2017Кв1"
...
Функция ОтчетЗаполнялсяСДелениемНаСекции(СсылкаНаРеглОтчет) Экспорт
	
	Возврат РегламентированнаяОтчетность.СегментДанныхРазделаДекларацииНДС(
	СсылкаНаРеглОтчет, "Раздел8", 1) <> Неопределено
	ИЛИ РегламентированнаяОтчетность.СегментДанныхРазделаДекларацииНДС(
	СсылкаНаРеглОтчет, "Раздел8_Прил1", 1) <> Неопределено
	ИЛИ РегламентированнаяОтчетность.СегментДанныхРазделаДекларацииНДС(
	СсылкаНаРеглОтчет, "Раздел9", 1) <> Неопределено
	ИЛИ РегламентированнаяОтчетность.СегментДанныхРазделаДекларацииНДС(
	СсылкаНаРеглОтчет, "Раздел9_Прил1", 1) <> Неопределено
	ИЛИ РегламентированнаяОтчетность.СегментДанныхРазделаДекларацииНДС(
	СсылкаНаРеглОтчет, "Раздел10", 1) <> Неопределено
	ИЛИ РегламентированнаяОтчетность.СегментДанныхРазделаДекларацииНДС(
	СсылкаНаРеглОтчет, "Раздел11", 1) <> Неопределено
	ИЛИ РегламентированнаяОтчетность.СегментДанныхРазделаДекларацииНДС(
	СсылкаНаРеглОтчет, "Раздел12", 1) <> Неопределено;
...
Функция НекорректныйРегистрационныйНомерТаможеннойДекларации(ПроверяемоеЗначение, КонтекстВыполнения)
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1")
		...
		Возврат (ДлинаЗначения < 13 ИЛИ ДлинаЗначения > 29);
		
	ИначеЕсли ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2020Кв4")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2019Кв1") Тогда
		
		Возврат (ДлинаЗначения < 13 ИЛИ ДлинаЗначения > 27);
...
Функция НекорректныйКодВидаТовара(ПроверяемоеЗначение)
	
	ДлинаЗначения = СтрДлина(СокрЛП(ПроверяемоеЗначение));
	
	Возврат (ДлинаЗначения < 4 ИЛИ ДлинаЗначения > 10);
