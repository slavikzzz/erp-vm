...
Процедура ПриИзмененииДанныхАутентификации() Экспорт
	...
	Если Не ИнтернетПоддержкаПользователей.ДоступноПодключениеИнтернетПоддержки() Тогда
		ВызватьИсключение НСтр("ru = 'Отключение интернет-поддержки пользователей не доступно.';
								|en = 'Cannot disable online user support.'");
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Обработчик.Комментарий         = НСтр("ru = 'Запускается при переходе на любую новую версию, для каждой области данных.
			...
			|en = 'Started for each data area on migration to any new version.
			|Set separated data.'");
...
Процедура ПриСозданииНаСервере(Форма, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДополнительныеПараметры.Идентификатор) Тогда
		ТекстИсключения = НСтр("ru = 'Не заполнен идентификатор сервиса сопровождения.';
								|en = 'Maintenance service ID is empty.'");
...
Процедура ПриПодключенииТестовогоПериодаНаФорме(Форма, Действие, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Передано некорректное действие при подключении тестового периода.';
								|en = 'An incorrect action is passed when activating the trial period.'");
	КонецЕсли;
...
Процедура ОбновитьДанныеСервисовСопровождения() Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Обновление данных идентификаторов сервисов сопровождения. Начало обновления.';
			|en = 'Productivity Tools details update started.'"),
		Ложь);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Обновление данных идентификаторов сервисов сопровождения. Успешно завершено.';
			|en = 'Productivity Tools details update completed.'"),
		Ложь);
...
Процедура СоздатьРегламентноеЗаданиеПроверкиПодключения(
	...
	Если Не РазрешеноПодключениеТестовыхПериодов() Тогда
		ВызватьИсключение НСтр("ru = 'Подключение тестового периода недоступно.';
								|en = 'Cannot activate trial periods.'");
	КонецЕсли;
	...
	Попытка
		...
		ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Создано регламентное задание для проверки запроса на подключение: %1';
				|en = 'A scheduled job to check trial activation request is created: %1'"),
			ИдентификаторЗапроса);
	...
	Исключение
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|%1';
			|en = 'Cannot create a scheduled job to check trial activation request:
			|%1'"),
...
Процедура УдалитьРегламентноеЗаданиеПроверкиПодключения(ИдентификаторЗапроса)
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Удалено регламентное задание проверки запроса на подключение: %1';
			|en = 'A scheduled job to check trial activation request is deleted: %1'"),
		ИдентификаторЗапроса);
...
Функция ОбновитьКэшТестовыхПериодов(Знач Идентификаторы)
	...
	Если ЗначениеЗаполнено(НеКорректныеИдентификаторы) Тогда
		...
		РезультатОбновления.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Описания сервисов: %1, не добавлены.';
				|en = 'Cannot add descriptions for the tools: %1.'"),
			НеКорректныеИдентификаторы);
	...
	Если Не РезультатОперации.СодержаниеОтвета.ПодключениеДоступно Тогда
		...
		|en = 'Cannot activate trials. Request is being processed.
		|Try again later.'");
		Возврат РезультатОбновления;
	КонецЕсли;
		...
		Если ИдентификаторыЗапросовНабор.Количество() = 0 Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не обнаружен идентификатор запроса на подключение.
					...
					|Идентификатор: %2';
					|en = 'Activation request ID not found.
					|Productivity tool: %1
					|ID: %2'"),
				СервисСопровождения,
				ИдентификаторЗапроса);
	...
	Если Не ПустаяСтрока(РезультатПроверки.ИмяОшибки) Тогда
		...
		ПодробнаяИнформацияОбОшибке          = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось проверить доступность сервиса тестовых периодов:
				...
				|Причина:
				|%2';
				|en = 'Cannot check the availability of Trial Period Service:
				|%1.
				|Reason:
				|%2'"),
			URLОперацииPing,
...
Функция ИнформацияОДоступныхТестовыхПериодах(ИдентификаторыСервисов)
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Начало обновление кэша для сервисов: %1';
			|en = 'Cache update started. Tools: %1'"),
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при получении доступных тестовых периодов:
				|%1';
				|en = 'Error receiving available trial periods:
				|%1'"),
		...
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось получить доступные тестовые периоды.
				...
				|Причина:
				|%2';
				|en = 'Cannot receive available trial periods.
				|The service returned an error: %1.
				|Reason:
				|%2'"),
			URLОперации,
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Завершено обновление кэша для сервисов: %1';
			|en = 'Cache update complete. Tools: %1'"),
		СтрСоединить(ИдентификаторыСервисов, ","));
	ЗаписатьИнформациюВЖурналРегистрации(
		ТекстЖурналРегистрации,
		Ложь);
...
Функция ПрочитатьДанные_availableList(ТелоJSON)
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Получен ответ сервиса Подключения тестовых периодов:
			|%1';
			|en = 'Trial Period Service responded:
			|%1'"),
		ТелоJSON);
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при подключении тестового периода:
				|%1';
				|en = 'Error activating trial period:
				|%1'"),
			РезультатОтправки.СообщениеОбОшибке);
		...
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось подключить тестовый период.
				...
				|Причина:
				|%2';
				|en = 'Cannot activate trial period.
				|The service returned an error:%1.
				|Reason:
				|%2'"),
			URLОперации,
...
Функция СостояниеЗапросаНаПодключение(
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Проверка состояния запроса на подключение тестового периода:
			...
			|Идентификатор тестового периода: %2';
			|en = 'Trial activation request status:
			|Request ID: %1
			|Test period ID: %2'"),
		ИдентификаторЗапроса,
		ИдентификаторТестовогоПериода);
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при проверке запроса на подключение:
				|%1';
				|en = 'Error checking activation request:
				|%1'"),
			РезультатОтправки.СообщениеОбОшибке);
		...
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось проверить состояние запроса на подключение.
				...
				|Причина:
				|%2';
				|en = 'Cannot check the status of the trial activation request.
				|The service returned an error: %1.
				|Reason:
				|%2'"),
			URLОперации,
...
Процедура ОбработатьРезультатПроверкиПодключения(
	...
	ИначеЕсли РезультатПодключения.КодОшибки = "UNKNOWN_CAUSE" Тогда
		РезультатПодключения.ИнформацияОбОшибке = НСтр("ru = 'Неизвестная ошибка при обработке запроса.
			|Подробнее см. в журнале регистрации.';
			|en = 'An unknown error occurred when processing the request.
			|See the Event Log for details.'");
	Иначе
		РезультатПодключения.ИнформацияОбОшибке = НСтр("ru = 'Внутренняя ошибка при обработке запроса.
			|Подробнее см. в журнале регистрации.';
			|en = 'An internal error occurred when processing the request.
			|See the Event Log for details.'");
	КонецЕсли;
...
Функция ТестовыйПериодПодключен(ИдентификаторТестовогоПериода, ПараметрыПодключения)
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		|%1';
		|en = 'Checking status of trial period:
		|%1'"),
		ИдентификаторТестовогоПериода);
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось проверить активность тестового периода.
				...
				|Причина:
				|%2';
				|en = 'Cannot check the status of the trial period.
				|The service returned an error:%1.
				|Reason:
				|%2'"),
			URLОперации,
...
Функция ПодключениеТестовогоПериода(
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Подключение тестового периода (activate).';
															|en = 'Connect test period (activate).'");
...
Функция ПроверитьПодключениеТестовогоПериода(
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Проверка состояния подключения тестового периода.';
															|en = 'Check test period connection status.'");
		...
		Если Результат.ДанныеАутентификации = Неопределено Тогда
			...
			Результат.ИнформацияОбОшибке =
				НСтр("ru = 'Для использования тестовых периодов, необходимо подключить Интернет-поддержку пользователей.';
					|en = 'Connect to Online support to activate trial periods.'");
			ЗаписатьИнформациюВЖурналРегистрации(
				Результат.ИнформацияОбОшибке,
				Истина);
		КонецЕсли;
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Подключение тестовых периодов.';
				|en = 'Trial period activation.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Иначе
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось прочитать значение свойства %1.
				...
				|en = 'Failed to read property: %1.
				|Invalid property type: %2.'"),
			ИмяСвойства,
...
Процедура СоздатьЭлементыПодключенияТестовогоПериода(Форма, ДополнительныеПараметры)
	...
	ПараметрыСтраницы.ТекстНадписи = НСтр("ru = 'Подключение сервиса на тестовый период,
		...
		|en = 'Connecting service for a test period. 
		|Please wait...'");
	...
	ПараметрыСтраницы.ТекстНадписи = НСтр("ru = 'Сервис успешно подключен.';
											|en = 'Service successfully connected.'");
	СоздатьСтраницу(Форма, ПараметрыСтраницы);
	...
	ПараметрыСтраницы.ТекстНадписи = НСтр("ru = 'Запрос на подключение сервиса отправлен. Ожидайте подтверждение подключения.';
											|en = 'The service connection request is sent. Please wait for the connection confirmation.'");
	СоздатьСтраницу(Форма, ПараметрыСтраницы);
