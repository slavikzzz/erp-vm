...
Процедура ИнициализироватьНастройкуАналитики(ЭлементНастройкиАналитики, Счет, ДополнитьПодразделением, ВключенныеСубконто = Неопределено) Экспорт
	...
	ОписаниеАналитикиСчета = БухгалтерскиеОтчетыВызовСервера.ОписаниеАналитикиСчета(
		Счет,
		ДополнитьПодразделением,
		ВключенныеСубконто);
	
	Если ОписаниеАналитикиСчета = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ЭлементНастройкиАналитики.Счет          = Счет;
	ЭлементНастройкиАналитики.Использование = Истина;
	ЭлементНастройкиАналитики.ПоСубсчетам   = (ОписаниеАналитикиСчета.КоличествоВключеннойАналитики = 0);
	ЭлементНастройкиАналитики.ПоСубконто    = ОписаниеАналитикиСчета.ШифрАналитики;
	ЭлементНастройкиАналитики.Представление = ОписаниеАналитикиСчета.Представление;
