	...
	Иначе
		...
		Если КоличествоДоступныхСервисов = 0 Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не включены сервисы доставки товаров';
															|en = 'Goods delivery services are not enabled'"));
			Возврат;
...
Процедура ПроверитьОрганизациюБизнесСетиСВопросом(Параметры, Отказ) Экспорт
	...
	Если Не СервисДоставкиВызовСервера.ОрганизацияПодключена(ОрганизацияБизнесСетиСсылка) Тогда
		ТекстВопроса = НСтр("ru = 'Организация ""%1"" не подключена в сервисе 1С:Бизнес-сеть. Подключить сейчас?';
							|en = 'Company ""%1"" is not connected in 1C:Business Network service. Connect now?'") + Символы.ПС + Символы.ПС
		+ НСтр("ru = 'При нажатии ""Да"" откроется форма подключения организации,';
				|en = 'Clicking ""Yes"" will open the company connection form,'") + Символы.ПС
		+ НСтр("ru = 'при нажатии ""Нет"" откроется форма выбора подключенной организации,';
				|en = 'clicking ""No"" will open the connected company choice form,'") + Символы.ПС
		+ НСтр("ru = 'при нажатии ""Отмена"" текущее действие будет отменено.';
				|en = 'clicking ""Cancel"" will cancel the current action.'");
