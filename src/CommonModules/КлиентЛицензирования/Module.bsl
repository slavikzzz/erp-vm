...
Процедура ЗаписатьДанныеАутентификацииВНастройкиКлиентаЛицензирования(Логин, Пароль)
	...
	УстановитьНастройкиКлиентаЛицензирования(
		Логин,
		Пароль,
		ЗначениеДополнительногоПараметра(
			НастройкиПодключения.ДоменРасположенияСерверовИПП));
...
//
Функция ЗначениеДополнительногоПараметра(ДоменнаяЗона)
...
Процедура ПередЗапускомПрограммы() Экспорт
	...
	Попытка
		...
		// Получение параметров клиента лицензирования
		// в разделенном режиме не доступно.
		...
		Если Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
			...
			НовоеЗначениеДопПараметра = ЗначениеДополнительногоПараметра(ДоменнаяЗона);
			Если ПолучитьДополнительныйПараметрКлиентаЛицензирования() <> НовоеЗначениеДопПараметра Тогда
				УстановитьНастройкиКлиентаЛицензирования(
					,
					,
					НовоеЗначениеДопПараметра);
			КонецЕсли;
