...
Процедура ЗапуститьДоставкуСерверныхОповещенийСОтсрочкой(Запущено = Ложь)
	...
	НаименованиеЗадания =
		НСтр("ru = 'Автозапуск';
			|en = 'Autostart'", ОбщегоНазначения.КодОсновногоЯзыка()) + ": "
		+ НСтр("ru = 'Доставка серверных оповещений с отсрочкой';
				|en = 'Delayed server notification delivery'", ОбщегоНазначения.КодОсновногоЯзыка()) + " ("
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'из сеанса %1 от %2';
				|en = 'from session %1 started on %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось создать служебного пользователя ""%1"" по причине:
				           |%2';
				           |en = 'Cannot create internal user ""%1"" due to:
				           |%2'"),
