...
Функция ИмяФормыВыбора() Экспорт
	...
	ИмяФормыВыбора = НСтр("ru = 'Справочник.УчетныеЗаписиМаркетплейсов.Форма.ВыборЗагружаемыхВидовЦен';
							|en = 'Справочник.УчетныеЗаписиМаркетплейсов.Форма.ВыборЗагружаемыхВидовЦен'");
...
// 
// Параметры:
//   ВыбранныеЦены - ТаблицаЗначений - таблица видов цен, cм. ВыбранныеЦены из УстановкаЦенСервер.ИнициализироватьСтруктуруФормы.
//
Процедура ВыполнитьОбновлениеЦен(ВыбранныеЦены) Экспорт
	
	Если ФункциональнаяОпцияИспользоватьИнтеграциюСЯндексМаркет() Тогда
		Отбор = Новый Структура;
		Отбор.Вставить("Выбрана", Истина);
		Отбор.Вставить("СпособЗаданияЦены", Перечисления.СпособыЗаданияЦен.ЗагружаетсяИзЯндексМаркет);
		
		СтрокиВидовЦен = ВыбранныеЦены.НайтиСтроки(Отбор);
		
		Если СтрокиВидовЦен.Количество() > 0 Тогда
			Для Каждого СтрокаВидаЦен Из СтрокиВидовЦен Цикл
				СтрокаВидаЦен.Выбрана       = Ложь;
				СтрокаВидаЦен.Редактировать = Ложь;
				СтрокаВидаЦен.Влияет        = Истина;
			КонецЦикла;
			
			ТаблицаВидовЦен = ВыбранныеЦены.Скопировать(СтрокиВидовЦен, "Ссылка");
			ТаблицаВидовЦен.Колонки.Ссылка.Имя = "ВидЦены";
	...
	Если ФункциональнаяОпцияИспользоватьИнтеграциюСOzon() Тогда
		...
		Отбор = Новый Структура;
		Отбор.Вставить("Выбрана", Истина);
		Отбор.Вставить("СпособЗаданияЦены", Перечисления.СпособыЗаданияЦен.ЗагружаетсяСOzon);
		...
		Если СтрокиВидовЦен.Количество() > 0 Тогда
			Для Каждого СтрокаВидаЦен Из СтрокиВидовЦен Цикл
				...
				НастройкиСпособаЗаданияЦены =
					ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаВидаЦен.Ссылка, "ХранилищеНастроекПараметровСпособаЗаданияЦены");
				
				Если НастройкиСпособаЗаданияЦены <> Неопределено Тогда
					Настройки = НастройкиСпособаЗаданияЦены.Получить();
					Если ТипЗнч(Настройки) = Тип("Структура") Тогда
						ВидыЦенOzon.Добавить(СтрокаВидаЦен.Ссылка);
				...
				ГиперссылкаЖурналПродажи.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'См. также: <a href=""%1"">Документы поставок (оформленные накладные)</a>';
							|en = 'See also: <a href=""%1"">Supply documents (registered invoices)</a>'"),
						"ДокументыЗакупки"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
