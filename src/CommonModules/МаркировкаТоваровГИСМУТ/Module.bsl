	...
	Для каждого СтрТабл Из ДокументОбъект.Товары Цикл
		Если СтрТабл.СтатусУказанияСерий = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Для товара %Товар% не настроено указание серий на складе %Склад%.';
									|en = 'Для товара %Товар% не настроено указание серий на складе %Склад%.'");
...
Процедура ЗаполнитьСпискиВыбораРеквизитовШапки(Форма) Экспорт
	...
	Если Форма.Элементы.ОперацияМаркировки.СписокВыбора.НайтиПоЗначению(Форма.Объект.ОперацияМаркировки) = Неопределено Тогда
		...
		ВызватьИсключение НСтр("ru = 'Выбранная операция маркировки отличается от доступных для выбора операций маркировки';
								|en = 'Выбранная операция маркировки отличается от доступных для выбора операций маркировки'");
...
Процедура ЗаполнитьМаркировкуТоваровПоВозвратуТоваровОтКлиента(ДанныеЗаполнения, ДокументОбъект)
	...
	Если ПакетРезультатов[1].Пустой() Тогда
		ТекстОшибки = НСтр("ru = 'Документ %Документ% не содержит маркируемых товаров для ГИСМ. Ввод на основании такого документа запрещен.';
							|en = 'Документ %Документ% не содержит маркируемых товаров для ГИСМ. Ввод на основании такого документа запрещен.'");
...
Процедура ЗаполнитьМаркировкуТоваровПоВыпускуПродукции(ДанныеЗаполнения, ДокументОбъект)
	...
	|	И ВыпускПродукцииТовары.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПродукцияИзНатуральногоМеха)");
...
Процедура ЗаполнитьМаркировкуТоваровПоДвижениюПродукцииИМатериалов(ДанныеЗаполнения, ДокументОбъект)
	...
	Запрос = Новый Запрос("
		...
		|	И Товары.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПродукцияИзНатуральногоМеха);");
	...
	Если ВыборкаШапка.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ВыпускПродукцииНаСклад
		...
		ТекстОшибки = НСтр("ru = 'Создание маркировки доступно только для документа с хозяйственной операцией ""Выпуск продукции"".';
							|en = 'Создание маркировки доступно только для документа с хозяйственной операцией ""Выпуск продукции"".'");
		ВызватьИсключение ТекстОшибки;
...
Процедура ЗаполнитьМаркировкуТоваровПоПроизводствуБезЗаказа(ДанныеЗаполнения, ДокументОбъект)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
		...
		|ГДЕ
		|	ВыходныеИзделия.Ссылка = &Ссылка
		|	И ВыходныеИзделия.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПродукцияИзНатуральногоМеха)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ПобочныеИзделия.Ссылка = &Ссылка
		|	И ПобочныеИзделия.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПродукцияИзНатуральногоМеха);
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Если ВыборкаШапка.НаправлениеВыпуска = Перечисления.ХозяйственныеОперации.ВыпускПродукцииНаСклад Тогда
		...
		Если СкладДляЗаполнения = Неопределено Тогда
			ТекстОшибки = НСтр("ru = 'В документе определено поступление на разные склады.
									...
									|en = 'В документе определено поступление на разные склады.
									|Ввод одного документа на основании выбранного распоряжения невозможен.'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Процедура ЗаполнитьМаркировкуТоваровПоМаршрутномуЛистуПроизводства(ДанныеЗаполнения, ДокументОбъект)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
		...
		|ГДЕ
		|	ВозвратныеОтходы.Ссылка = &Ссылка
		|	И ВозвратныеОтходы.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПродукцияИзНатуральногоМеха);
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВыходныеИзделия.Получатель КАК Склад
	...
	Если ВыборкаШапка.НаправлениеВыпуска = Перечисления.ХозяйственныеОперации.ВыпускПродукцииНаСклад Тогда
		...
		Если СкладДляЗаполнения = Неопределено Тогда
			ТекстОшибки = НСтр("ru = 'В маршрутном листе определено поступление на разные склады.
									|Ввод одного документа на основании выбранного распоряжения невозможен.';
									|en = 'В маршрутном листе определено поступление на разные склады.
									|Ввод одного документа на основании выбранного распоряжения невозможен.'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Функция СгенерироватьСериюКиЗ(Данные, ВыборкаДопРеквизиты)
	...
	ШаблонОшибкиЗаписать = НСтр("ru = 'Не удалось записать серию КиЗ. %ОписаниеОшибки%';
								|en = 'Не удалось записать серию КиЗ. %ОписаниеОшибки%'");
		...
		Если Не ЗначениеЗаполнено(СтрокаСерии.Серия) Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для КиЗ %1 отсутствуют серии.';
					|en = 'Для КиЗ %1 отсутствуют серии.'"),
...
Функция АдаптированныйТекстЗапросаДвиженийПоРегиструМаркировкаТоваров(ИмяРегистра) Экспорт
	...
	Иначе
		ТекстИсключения = НСтр("ru = 'В документе %ПолноеИмяДокумента% не реализована адаптация текста запроса формирования движений по регистру %ИмяРегистра%.';
								|en = 'В документе %ПолноеИмяДокумента% не реализована адаптация текста запроса формирования движений по регистру %ИмяРегистра%.'");
		...
		Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса(ДополнительныеТаблицы[0], ТекстыЗапроса) Тогда
			...
			Иначе
				ВызватьИсключение НСтр("ru = 'Некорректный тип документа ГИСМ';
										|en = 'Некорректный тип документа ГИСМ'");
			КонецЕсли;
...
//++ НЕ УТ
#Область ПроведениеРегламентированныйУчетERP
...
//
Функция ТекстОтраженияВРеглУчете(ДокументИмя) Экспорт
	
	ТекстыОтражения = Новый Массив;
	
	Если ДокументИмя = "МаркировкаТоваровГИСМ" Тогда
		ТекстыОтражения.Добавить(ТекстМаркировкаТоваровГИСМСписаниеТоваровНаРасходы());
		ТекстыОтражения.Добавить(ТекстМаркировкаТоваровГИСМВключениеИсключениеНДСВСтоимостьТоваров());
	ИначеЕсли ДокументИмя = "ПеремаркировкаТоваровГИСМ" Тогда
		ТекстыОтражения.Добавить(ТекстПеремаркировкаТоваровГИСМСписаниеТоваровНаРасходы());
		ТекстыОтражения.Добавить(ТекстПеремаркировкаТоваровГИСМВключениеИсключениеНДСВСтоимостьТоваров());
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для документа %1 в общем модуле ""МаркировкаТоваровГИСМ"" отсутствует функция ТекстОтраженияВРеглУчете()';
				|en = 'Для документа %1 в общем модуле ""МаркировкаТоваровГИСМ"" отсутствует функция ТекстОтраженияВРеглУчете()'"),
			ДокументИмя);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
//  Строка - Текст запроса
//
Функция ТекстЗапросаВТОтраженияВРеглУчете(ДокументИмя) Экспорт
	
	Возврат "";
	
КонецФункции

#Область ОтражениеМаркировкаТоваровГИСМ

Функция ТекстМаркировкаТоваровГИСМСписаниеТоваровНаРасходы()
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.МаркировкаТоваровГИСМ КАК Операция
	|	ПО
...
Функция ТекстМаркировкаТоваровГИСМВключениеИсключениеНДСВСтоимостьТоваров()
...
#Область ОтражениеПеремаркировкаТоваровГИСМ

Функция ТекстПеремаркировкаТоваровГИСМСписаниеТоваровНаРасходы()
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.ПеремаркировкаТоваровГИСМ КАК Операция
	|	ПО
	|		ДокументыКОтражению.Ссылка = Операция.Ссылка
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстПеремаркировкаТоваровГИСМВключениеИсключениеНДСВСтоимостьТоваров()
