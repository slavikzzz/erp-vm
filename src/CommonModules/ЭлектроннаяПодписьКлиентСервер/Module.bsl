...
Процедура ЗаполнитьСтатусПодписи(СвойстваПодписи, ДатаСеанса) Экспорт
				...
				Если ЗначениеЗаполнено(СвойстваПодписи.СрокДействияПоследнейМеткиВремени)
					...
					СвойстваПодписи.РезультатПроверкиКраткий = 
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '%1. Документ не изменен, сертификат был действителен на дату подписи.';
								|en = '%1. The document wasn''t modified, and the certificate was valid at the signing date.'"), Статус);
				Иначе
					СвойстваПодписи.РезультатПроверкиКраткий =  
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '%1. Документ не изменен, сертификат действителен на дату проверки.';
								|en = '%1. The document wasn''t modified, and the certificate was valid at the verification date.'"), Статус);
				КонецЕсли;
			Иначе
				СвойстваПодписи.РезультатПроверкиКраткий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1. Документ не изменен, сертификат действителен на дату метки времени.';
						|en = '%1. The document wasn''t modified, and the certificate was valid at the timestamp date.'"), Статус);
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(РезультатПроверки.ОшибкаМатематическойПроверкиПодписи) И ЗначениеЗаполнено(
		...
		СвойстваПодписи.РезультатПроверкиКраткий =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1. Документ изменен и сертификат не прошел проверку. %2 %3';
				|en = '%1. The document was modified, and the certificate validation failed. %2 %3'"), Статус,
	...
	ИначеЕсли ЗначениеЗаполнено(РезультатПроверки.ОшибкаМатематическойПроверкиПодписи) Тогда
		...
		СвойстваПодписи.РезультатПроверкиКраткий =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1. Документ изменен. %2';
						|en = '%1. The document was modified. %2'"), Статус, РезультатПроверки.ОшибкаМатематическойПроверкиПодписи);
	ИначеЕсли ЗначениеЗаполнено(РезультатПроверки.ОшибкаПроверкиДополнительныхАтрибутов) Тогда
		...
		Если СвойстваПодписи.ТипПодписи = ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.ОбычнаяCMS")
			...
			СвойстваПодписи.РезультатПроверкиКраткий =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1. Документ не изменен, но сертификат не прошел проверку. %2';
						|en = '%1. The document was not modified, but the certificate validation failed. %2'"), Статус, 
				РезультатПроверки.ОшибкаПроверкиДополнительныхАтрибутов);
		Иначе
			СвойстваПодписи.РезультатПроверкиКраткий =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1. Документ не изменен, но один из сертификатов не прошел проверку. %2';
						|en = '%1. The document was not modified, but one of the certificates failed validation. %2'"), Статус,
				РезультатПроверки.ОшибкаПроверкиДополнительныхАтрибутов);
		КонецЕсли;
	Иначе
		СвойстваПодписи.РезультатПроверкиКраткий =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
