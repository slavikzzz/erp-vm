	...
	Если ЗначениеЗаполнено(ЯзыкПеревода)
		...
		Иначе
			...
			Если РезультатПеревода.ПереведенныйТекст <> ТекстПеревода Тогда
				...
				ТекстВопроса = СтрШаблон(НСтр("ru = 'В результате перевода текст на языке ""%1"" будет изменен, продолжить?';
												|en = 'After the translation, the text in the %1 language will be changed. Continue?'"), ЯзыкПеревода);
				ПоказатьВопрос(ОповещениеВопросПослеПеревода, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
				
				
			Иначе
				
				ТексСообщения = СтрШаблон(НСтр("ru = 'После перевода текст на языке ""%1"" изменен не был';
												|en = 'After the translation, the text in the %1 language was not changed'"), ЯзыкПеревода);
