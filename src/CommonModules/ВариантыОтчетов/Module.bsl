...
Функция ХранилищеВариантовДополнительногоОтчетаКорректное(МетаданныеОтчета, ТекстПредупреждения = "") Экспорт 
	...
	Если МетаданныеОтчета.ХранилищеВариантов <> Неопределено Тогда 
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|Valid value: %1.'"), Метаданные.ХранилищаНастроек.ХранилищеВариантовОтчетов.ПолноеИмя());
	Иначе
				...
				Исключение
					Если ОбщегоНазначения.РазделениеВключено() Тогда
						ШаблонТекстаОшибки = НСтр("ru = 'Не удалось прочитать список вариантов отчета %1 в неразделенном сеансе,
							...
							|%2';
							|en = 'Cannot read the %1 report option list in a separated session,
...
Процедура УстановитьРежимВыводаОтчетаВПанеляхОтчетов(Настройки, Отчет, ГруппироватьПоОтчетам)
	...
	Иначе
		...
		Если ОписаниеОтчета.Количество() <> 1 Тогда
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Предупреждение, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отчет ""%1"" не подключен к подсистеме.';
						|en = 'Report %1 is not attached to the subsystem.'"), Отчет.Имя));
			Возврат;
		КонецЕсли;
...
Процедура ЗаписатьВЖурналЗапускПроцедуры(ИмяПроцедуры)
	...
	ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(НаименованиеПодсистемы(Неопределено),
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Запуск процедуры ""%1"".';
																	|en = 'Starting %1.'"), ИмяПроцедуры)); 
...
Процедура ЗаписатьВЖурналЗавершениеПроцедуры(ИмяПроцедуры, ИзмененоОбъектов = Неопределено)
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Завершение процедуры ""%1"".';
																		|en = 'Finishing %1.'"), ИмяПроцедуры);
	Если ИзмененоОбъектов <> Неопределено Тогда
		Текст = Текст + " " 
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Изменено %1 объектов.';
																			|en = '%1 objects have been modified.'"), ИзмененоОбъектов);
	КонецЕсли;
	ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(НаименованиеПодсистемы(Неопределено),
...
Процедура ОперативноеОбновлениеСлужебногоПользователя() Экспорт 
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Варианты отчетов.Обновление служебного пользователя';
				|en = 'Report options.Update service user'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
...
Функция ОбновитьИндексПоиска(Режим, ИндексироватьСхему)
	...
	Уточнение = НРег(ПредставлениеРежима(Режим)) + ", " + ?(ИндексироватьСхему, НСтр("ru = 'полное';
																					|en = 'full'"), НСтр("ru = 'по изменениям';
																											|en = 'by changes'"));
	
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Обновление индекса поиска (%1)';
																							|en = 'Updating search index (%1)'"), Уточнение);
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура АктуализироватьКлючиПредопределенных(Режим, Результат)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		?(Режим = "ОбщиеДанныеКонфигурации", НСтр("ru = 'метаданные конфигурации';
													|en = 'configuration metadata'"), НСтр("ru = 'метаданные расширений';
		...
		Для Каждого Раздел Из ОписаниеВарианта.Размещение Цикл
			Если Раздел.Ключ = Неопределено Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для варианта отчета ""%1"" (%2) 
					...
					|en = 'A subsystem to place the %1 report option (%2) is not specified.
					|See the %3 and %4 procedures.'"), ОписаниеВарианта.Наименование,
...
Функция ПараметрыЗаполненияПредставленийВариантовОтчетов(Языки = Неопределено, ИндексТекущегоЯзыка = 0)
	...
	НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Заполнение представлений предопределенных вариантов отчетов для языка %1';
			|en = 'Populating presentations of predefined report options for the %1 language'"), КодЯзыка);
...
Функция СлужебныйПользователь(Знач КодЯзыка)
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Варианты отчетов.Создание служебного пользователя';
				|en = 'Report options.Create service user'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Метаданные.Справочники.Пользователи,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Неопределено;
...
Функция ЗаполнитьПоляДляПоиска(ВариантОбъект, СведенияОбОтчете = Неопределено) Экспорт
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		Если ТипЗнч(ВариантОбъект) = Тип("СправочникОбъект.ПредопределенныеВариантыОтчетов")
			...
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось прочитать настройки предопределенного варианта отчета ""%1"".';
						|en = 'Cannot read settings of a predefined report option: %1.'"), ВариантОбъект.КлючЗамеров),
	...
	Если ОтчетОбъект = Неопределено Тогда
		...
		Иначе
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось получить настройки отчета ""%1"":';
					|en = 'Failed to get settings for report %1:'") + Символы.ПС + Подключение.ТекстОшибки,
				ОтчетСсылка);
...
#Область ЧтениеФайлаОписанияНастроек
	...
	ШапкаОписанияОшибки = НСтр("ru = 'Некорректный формат описания настроек:';
								|en = 'Invalid settings description format:'") + Символы.ПС;
	...
	Исключение
		ОписаниеНастроек.ОписаниеОшибки = ШапкаОписанияОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'отсутствует файл %1.';
				|en = 'File %1 is missing.'"), ИмяФайла);
...
#Область ЧтениеОбщегоОписанияНастроек
	...
	Если ИмяОтчета = Неопределено Тогда 
		ОписаниеНастроек.ОписаниеОшибки = ШапкаОписанияОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'отсутствует атрибут %1.';
				|en = 'Attribute ""%1"" is missing.'"), ИмяЭлемента);
		Возврат ОписаниеНастроек;
	КонецЕсли;
...
#Область ЧтениеОписанияОсновныхНастроек
	...
	Если Состав.Количество() = 0 Тогда 
		ОписаниеНастроек.ОписаниеОшибки = ШапкаОписанияОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'отсутствует элемент %1.';
				|en = 'Item ""%1"" is missing.'"), ИмяЭлемента);
		Возврат ОписаниеНастроек;
	КонецЕсли;
	...
	Если КлючВарианта = Неопределено Тогда 
		ОписаниеНастроек.ОписаниеОшибки = ШапкаОписанияОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'отсутствует атрибут %1 элемента %2.';
				|en = 'Item ""%2"" is missing attribute ""%1"".'"), ИмяАтрибута, ИмяЭлемента);
		Возврат ОписаниеНастроек;
	КонецЕсли;
...
Функция ОбновитьВариантОтчетаПоОписанию(Знач ОписаниеВариантаОтчета, Знач ВариантОтчетаОснование = Неопределено)
	...
	Если Не ВариантыОтчетовПовтИсп.ПравоДобавления() Тогда 
		ВызватьИсключение(НСтр("ru = 'Недостаточно прав для выполнения операции.';
								|en = 'Insufficient rights to perform the operation.'"), КатегорияОшибки.НарушениеПравДоступа);
	КонецЕсли;
...
Процедура ПоделитьсяПользовательскимиНастройками(ВыбранныеПользователи, ШаблонОписанияНастроек) Экспорт 
	...
	Если СвойстваПользователей.Действительных.Количество() = 0 Тогда 
		...
		ШаблонОписанияНастроек.Вставить("Предупреждение", НСтр("ru = 'Выбранные пользователи недействительны';
																|en = 'The selected users are invalid.'"));
		Возврат;
	...
	Если СвойстваПользователей.Недействительных.Количество() > 0 Тогда 
		...
		ШаблонПояснения = НСтр("ru = 'Некоторые из выбранных пользователей недействительны: %1';
								|en = 'Some of the selected users are invalid: %1.'");
...
#Область ГенерацияОповещенияНовыхПользователейНастроек
	...
	Если ЗначениеЗаполнено(ОписаниеНастроек.Представление) Тогда 
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Предоставлены настройки отчета ""%1""';
				|en = 'The report settings are available: ""%1.""'"), ОписаниеНастроек.Представление);
	Иначе
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Варианты отчетов.Оповещение о предоставлении доступа к настройкам отчета';
				|en = 'Report options.Report settings availability notification'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,
