...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	ОписаниеВарианта.Описание = НСтр("ru = 'Выявление и анализ расхождений между входящей транспортной операцией ВетИС и поступлением товаров.';
									|en = 'Выявление и анализ расхождений между входящей транспортной операцией ВетИС и поступлением товаров.'");
	...
	ОписаниеВарианта.Описание = НСтр("ru = 'Выявление и анализ расхождений между инвентаризацией продукции ВетИС и складскими операциями.';
									|en = 'Выявление и анализ расхождений между инвентаризацией продукции ВетИС и складскими операциями.'");
	ОписаниеВарианта.НастройкиДляПоиска.НаименованияПолей = НСтр("ru = 'ВетИС';
																|en = 'ВетИС'");
	...
	ОписаниеВарианта.Описание = НСтр("ru = 'Выявление и анализ расхождений между исходящей транспортной операцией ВетИС и складскими операциями.';
									|en = 'Выявление и анализ расхождений между исходящей транспортной операцией ВетИС и складскими операциями.'");
	ОписаниеВарианта.НастройкиДляПоиска.НаименованияПолей = НСтр("ru = 'ВетИС';
																|en = 'ВетИС'");
	...
	ОписаниеВарианта.Описание = НСтр("ru = 'Выявление и анализ расхождений между производственной операцией ВетИС и складскими операциями.';
									|en = 'Выявление и анализ расхождений между производственной операцией ВетИС и складскими операциями.'");
	ОписаниеВарианта.НастройкиДляПоиска.НаименованияПолей = НСтр("ru = 'ВетИС';
																|en = 'ВетИС'");
	...
	Если НастройкиОбменаВЕТИС <> Неопределено
		...
		ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(
			НСтр("ru = 'Пользователь %1 не включен в список пользователей хозяйствующего субъекта %2.';
				|en = 'Пользователь %1 не включен в список пользователей хозяйствующего субъекта %2.'"),
			Пользователи.ТекущийПользователь(),
			ХозяйствующийСубъект));
	Иначе
		ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(
			НСтр("ru = 'Для хозяйствующего субъекта %1 не найдены настройки подключения к ВетИС.';
				|en = 'Для хозяйствующего субъекта %1 не найдены настройки подключения к ВетИС.'"), ХозяйствующийСубъект));
	КонецЕсли;
...
Функция СообщениеОперацияВЕТИСНедоступнаДляПользователя(СообщенияXML, СообщениеXML, ОперацияВЕТИС) Экспорт
	...
	ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(
		НСтр("ru = 'Операция ""%1"" недоступна для текущего пользователя. Обратитесь к администратору.';
			|en = 'Операция ""%1"" недоступна для текущего пользователя. Обратитесь к администратору.'"), ОперацияВЕТИС));
	...
	Иначе
		ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
			...
			НСтр("ru = 'Не найдена настройка для подключения к ВетИС.';
				|en = 'Не найдена настройка для подключения к ВетИС.'"));
	КонецЕсли;
		...
		Иначе
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Сообщение ""%1"" не подготовлено к отправке в ВетИС:
				           |%2';
				           |en = 'Сообщение ""%1"" не подготовлено к отправке в ВетИС:
				           |%2'"),
				Сообщение.Описание,
...
Функция ТекстОшибкиОбработкиРезультата(ДанныеДокумента, ХозяйствующийСубъект = Неопределено, Предприятие = Неопределено, ВыводитьЗаголовок = Ложь) Экспорт
	...
	Если ДанныеДокумента.Ошибки <> Неопределено Тогда
		...
		Для Каждого Ошибка Из ДанныеДокумента.Ошибки Цикл
			Ошибки.Добавить(СтрШаблон(НСтр("ru = '%1: %2';
											|en = '%1: %2'"), Ошибка.Код, Ошибка.ТекстОшибки));
		КонецЦикла;
		
		Ошибки.Добавить(НСтр("ru = 'Текст XML см. в протоколе обмена.';
							|en = 'Текст XML см. в протоколе обмена.'"));
...
// Текст запроса движение серий товаров.
// 
// Параметры:
//  МетаданныеДокумента - ОбъектМетаданных - Метаданные документа
// 
// Возвращаемое значение:
//  Строка - Текст запроса движение серий товаров
...
Процедура ПроверитьЗаполнениеТипаПроисхождения(ДокументОбъект, Отказ, МассивНепроверяемыхРеквизитов) Экспорт
	...
	ШаблонСообщения = НСтр("ru = 'Не заполнено поле ""Происхождение"" в строке %1 списка ""Товары"".';
							|en = 'Не заполнено поле ""Происхождение"" в строке %1 списка ""Товары"".'");
		...
		Если ДанныеДокумента.Объект <> Неопределено Тогда
			...
			Если СписокСтрок.Количество() <> 0 Тогда
				ТекстОшибки = НСтр("ru = 'Не удалось заполнить все ссылки на добавленные записи журнала';
									|en = 'Не удалось заполнить все ссылки на добавленные записи журнала'");
...
Процедура ЗаполнитьСопоставленнуюПродукциюВДокументе(ТабличнаяЧасть, Отбор = Неопределено, ПроверятьЗаполненнуюПродукцию = Ложь) Экспорт
	...
	Пока ВыборкаСтроки.Следующий() Цикл
		...
		Если КоличествоСопоставлено > 1 Тогда
			СтрокаТЧ.СопоставлениеТекст = СтрШаблон(НСтр("ru = '<Несколько позиций (%1)>';
...
Процедура ПроверитьОчиститьЕдиницуИзмеренияВЕТИС(СтрокаТабличнойЧасти, РеквизитыПродукции = Неопределено) Экспорт
	...
	Иначе
		Если РеквизитыПродукции <> Неопределено Тогда
			РеквизитыСтруктурой = Новый Структура("ВидПродукцииИдентификатор, ПродукцияИдентификатор, ТипПродукции");
			ЗаполнитьЗначенияСвойств(РеквизитыСтруктурой, РеквизитыПродукции);
		Иначе 
			РеквизитыСтруктурой = Неопределено;
		КонецЕсли;
		ДоступныеЕдиницыИзмерения = ИнтеграцияВЕТИСПовтИсп.ДоступныеЕдиницыИзменения(Продукция, РеквизитыСтруктурой);
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстОшибки = НСтр("ru = 'При записи соответствия номенклатуры ВетИС произошла ошибка:';
								|en = 'При записи соответствия номенклатуры ВетИС произошла ошибка:'")
			+ Символы.ПС + "%1";
...
Процедура СверткаРегистраСоответствиеНоменклатурыВЕТИС() Экспорт
	...
	Если НЕ Результат.Пустой() Тогда
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Свертка регистра Соответствие номенклатуры ВЕТИС';
				|en = 'Свертка регистра Соответствие номенклатуры ВЕТИС'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация, , ,
			НСтр("ru = 'Начато регламентное задание ""Свертка регистра Соответствие номенклатуры ВЕТИС"".';
				|en = 'Начато регламентное задание ""Свертка регистра Соответствие номенклатуры ВЕТИС"".'"));
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Свертка регистра Соответствие номенклатуры ВЕТИС';
										|en = 'Свертка регистра Соответствие номенклатуры ВЕТИС'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация, , ,
			НСтр("ru = 'Завершено регламентное задание ""Свертка регистра Соответствие номенклатуры ВЕТИС"".';
				|en = 'Завершено регламентное задание ""Свертка регистра Соответствие номенклатуры ВЕТИС"".'"));
	...
	Исключение
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Ошибка подготовки исходных файлов для загрузки классификатора ВетИС %1:
			           |%2';
			           |en = 'Ошибка подготовки исходных файлов для загрузки классификатора ВетИС %1:
			           |%2'",
		...
		ОбщегоНазначенияИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			ТекстОшибки,
			НСтр("ru = 'Работа с классификаторами ВетИС';
				|en = 'Работа с классификаторами ВетИС'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ДатаФормированияРезультата(ЭлементОчереди, ПараметрыОбмена, ДанныеВыполненияЗаявки = Неопределено) Экспорт
		...
		Если ТипЗнч(ЭлементОчереди.Сообщение) = Тип("СправочникСсылка.ВЕТИСПрисоединенныеФайлы") Тогда
			...
			Если РеквизитыСообщения.Следующий() Тогда
				...
				Если РеквизитыСообщения.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийВЕТИС.ОшибкаОтправки Тогда
					...
					Источник = СтрШаблон(НСтр("ru = 'Реквизит ДатаМодификацииУниверсальная сообщения %1';
												|en = 'Реквизит ДатаМодификацииУниверсальная сообщения %1'"), ЭлементОчереди.Сообщение);
				...
				Иначе
					...
					Источник = СтрШаблон(НСтр("ru = 'Реквизит rcvDate ответа на передачу данных %1, приведен к универсальному времени';
												|en = 'Реквизит rcvDate ответа на передачу данных %1, приведен к универсальному времени'"), РеквизитыСообщения.Ссылка);
			...
			Иначе
				...
				Если ДанныеВыполненияЗаявки = Неопределено Тогда
					...
					Источник = НСтр("ru = 'Реквизит ДатаСоздания элемента очереди, приведен к универсальному времени';
									|en = 'Реквизит ДатаСоздания элемента очереди, приведен к универсальному времени'");
				Иначе
					...
					Источник = НСтр("ru = 'Реквизит issueDate данных выполненения заявки, приведен к универсальному времени';
									|en = 'Реквизит issueDate данных выполненения заявки, приведен к универсальному времени'");
				КонецЕсли;
	...
	Иначе
		...
		Источник = НСтр("ru = 'Реквизит ДатаФормированияРезультата входящего сообщения';
						|en = 'Реквизит ДатаФормированияРезультата входящего сообщения'");
...
Функция ДатаФормированияРезультатаПоСсылке(ДокументСсылка, ПараметрыОбмена) Экспорт
	...
	Если РеквизитыСообщения.Следующий() Тогда
		...
		Если РеквизитыСообщения.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийВЕТИС.ОшибкаОтправки Тогда
			...
			ДатаФормированияРезультата = РеквизитыСообщения.ДатаМодификацииУниверсальная;
			Источник = СтрШаблон(НСтр("ru = 'Реквизит ДатаМодификацииУниверсальная сообщения %1';
										|en = 'Реквизит ДатаМодификацииУниверсальная сообщения %1'"), РеквизитыСообщения.Ссылка);
...
Функция ШаблонПредставленияДокументаВЕТИСДляПоляИнтеграции(МетаданныеДокументаВЕТИС, ДокументОснование) Экспорт
	...
	Если МетаданныеДокументаВЕТИС = Метаданные.Документы.ВходящаяТранспортнаяОперацияВЕТИС Тогда
		...
		ФормыИмениДокумента.Склоненная = НСтр("ru = 'входящую транспортную операцию ВетИС';
												|en = 'входящую транспортную операцию ВетИС'");
...
// 
// Параметры:
//  ОбъектXDTOИсточник - ОбъектXDTO, Произвольный - Объект XDTOИсточник
//  ОбъектXDTOПриемник - ОбъектXDTO - Объект XDTOПриемник
// 
// Возвращаемое значение:
...
Функция ОписаниеОперации(Описание, ДокументСсылка, НомерВерсии = Неопределено) Экспорт
	...
	ИначеЕсли НомерВерсии = Неопределено И ДокументСсылка <> Неопределено Тогда
		Возврат СтрШаблон(НСтр("ru = '%1 по документу ""%2""';
								|en = '%1 по документу ""%2""'"), Описание, ДокументСсылка);
	ИначеЕсли НомерВерсии <> Неопределено И ДокументСсылка = Неопределено Тогда
		Возврат СтрШаблон(НСтр("ru = '%1. Версия %2';
								|en = '%1. Версия %2'"), Описание, НомерВерсии);
	Иначе
		Возврат СтрШаблон(НСтр("ru = '%1 по документу ""%2"". Версия %3';
				...
				ЗаписатьОшибкуСервисаВЖурналРегистрации(
					СтрШаблон(
						НСтр("ru = '[ПРОВЕРКА] Запущена проверка результатов обработки заявки,
						           ...
						           |Источник: %2';
						           |en = '[ПРОВЕРКА] Запущена проверка результатов обработки заявки,
						           |Дата результата заявки: %1
						           |Источник: %2'"),
...
Функция ПоляПодбораДокументовПриВосстановлении(ТипДокумента) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Неверный тип документа для определения полей подбора.';
								|en = 'Неверный тип документа для определения полей подбора.'");
	КонецЕсли;
...
// Выполнить запрос элемента классификатора.
// 
// Параметры:
//  Запрос - Запрос
//  ХозяйствующийСубъект - Неопределено, СправочникСсылка.ХозяйствующиеСубъектыВЕТИС - Хозяйствующий субъект
//  ПараметрыОбмена - см. ПараметрыОбмена
// 
// Возвращаемое значение:
//  см. ИнтеграцияВЕТИСКлиентСервер.РезультатВыполненияЗапросаЭлементаКлассификатора 
	...
	Если НастройкаОбмена = Неопределено Тогда
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Не удалось получить ответ на %1 по причине
			           |%2';
			           |en = 'Не удалось получить ответ на %1 по причине
			           |%2'"),
			...
			СтрШаблон(
				НСтр("ru = 'Не найдены настройки подключения к ВетИС. Проверьте, что пользователь %1 сопоставлен с пользователем ВетИС.';
					|en = 'Не найдены настройки подключения к ВетИС. Проверьте, что пользователь %1 сопоставлен с пользователем ВетИС.'"),
				Пользователи.ТекущийПользователь()));
...
// Выполнить запрос элементов классификатора.
// 
// Параметры:
//  Запрос - Запрос
//  ХозяйствующийСубъект - Неопределено, СправочникСсылка.ХозяйствующиеСубъектыВЕТИС - Хозяйствующий субъект
...
// 
// Возвращаемое значение:
//  см. ИнтеграцияВЕТИСКлиентСервер.РезультатВыполненияЗапросаЭлементовКлассификатора
...
Функция ОбработатьОтветНаЗапросЭлементаКлассификатора(РезультатОтправкиЗапроса, Запрос, ОбработатьСписок = Ложь) Экспорт
	...
	Исключение
		...
		ПредставлениеОшибки = ПредставлениеОшибкиXDTO(ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()), ЧтениеXML, ЧтениеXML.КонтекстПространствИмен.Глубина);
			...
			ОбработатьОшибку(
				СтрШаблон(
					НСтр("ru = 'При разборе XML ответа на %1 возникла ошибка:
					           ...
					           |en = 'При разборе XML ответа на %1 возникла ошибка:
					           |Текст ошибки: %ТекстОшибки%'"),
					Запрос.Представление),
		...
		ОбработатьОшибку(
			СтрШаблон(
				НСтр("ru = 'При разборе объекта XDTO ответа на %1 возникла ошибка:
				           |Текст ошибки: %ТекстОшибки%';
				           |en = 'При разборе объекта XDTO ответа на %1 возникла ошибка:
				           |Текст ошибки: %ТекстОшибки%'"),
				Запрос.Представление),
			ИнформацияОбОшибке(),
			ВозвращаемоеЗначение);
...
Функция ОбработатьОтветНаПередачуДанных(РезультатОтправкиЗапроса, Запрос, Операция = Неопределено, ПараметрыОбмена) Экспорт
	...
	Попытка
		...
		Если Запрос.Сервис = Перечисления.СервисыВЕТИС.ОбработкаЗаявок Тогда
			...
			Иначе
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Неизвестный статус обработки заявки: %1';
												|en = 'Неизвестный статус обработки заявки: %1'"), Статус);
			КонецЕсли;
	...
	Если РезультатОтправкиЗапроса.КодСостояния = 500 Тогда
		...
		ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Код состояния HTTP: %1. Код ошибки: %2. Текст ошибки: %3.';
				|en = 'Код состояния HTTP: %1. Код ошибки: %2. Текст ошибки: %3.'"),
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 401 Тогда
			...
			ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Код состояния HTTP: %1. Описание ошибки: %2.';
					|en = 'Код состояния HTTP: %1. Описание ошибки: %2.'"),
				РезультатОтправкиЗапроса.КодСостояния,
				НСтр("ru = 'Нет доступа';
					|en = 'Нет доступа'"));
		Иначе
			...
			Если ЗначениеЗаполнено(РезультатОтправкиЗапроса.ТекстОшибки) Тогда
				ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Код состояния HTTP: %1. Текст ошибки: %2.';
						|en = 'Код состояния HTTP: %1. Текст ошибки: %2.'"),
					РезультатОтправкиЗапроса.КодСостояния,
					РезультатОтправкиЗапроса.ТекстОшибки);
			Иначе
				ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Код состояния HTTP: %1. Текст сообщения HTTP: %2.';
						|en = 'Код состояния HTTP: %1. Текст сообщения HTTP: %2.'"),
					РезультатОтправкиЗапроса.КодСостояния,
					РезультатОтправкиЗапроса.ТекстВходящегоСообщенияSOAP);
			КонецЕсли;
		КонецЕсли;
		...
		ВызватьИсключение
			СтрШаблон(
				НСтр("ru = 'Неизвестный способ изменения: %1';
					|en = 'Неизвестный способ изменения: %1'"),
				СпособИзменения);
	КонецЕсли;
		...
		ВызватьИсключение
			СтрШаблон(
				НСтр("ru = 'Неизвестный тип хозяйствующего субъекта с кодом: %1';
					|en = 'Неизвестный тип хозяйствующего субъекта с кодом: %1'"),
				Код);
	КонецЕсли;
		...
		ВызватьИсключение
			СтрШаблон(
				НСтр("ru = 'Неизвестный тип хозяйствующего субъекта: %1';
					|en = 'Неизвестный тип хозяйствующего субъекта: %1'"),
		...
		ВызватьИсключение
			СтрШаблон(
				НСтр("ru = 'Неизвестный тип предприятия: %1';
					|en = 'Неизвестный тип предприятия: %1'"),
				ТипПредприятия);
	КонецЕсли;
...
Функция ТаблицаОрганизацияПредприятие(ОрганизацииВЕТИС) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru = 'Некорректный параметр функции ИнтеграцияВЕТИС.ТаблицаОрганизацияПредприятие().';
								|en = 'Некорректный параметр функции ИнтеграцияВЕТИС.ТаблицаОрганизацияПредприятие().'");
		...
		Для Каждого ХозяйствующийСубъект Из ХозяйствующиеСубъекты Цикл
			...
			Если Не ПараметрыОбмена.ХозяйствующиеСубъектыБезНастроек.Найти(ХозяйствующийСубъект) = Неопределено Тогда
				ТекстОшибки = Новый ФорматированнаяСтрока(
					СтрШаблон(
						НСтр("ru = 'Для хозяйствующего субъекта %1 не найдены настройки подключения к ВетИС.';
							|en = 'Для хозяйствующего субъекта %1 не найдены настройки подключения к ВетИС.'"),
						ХозяйствующийСубъект));
				Прервать;
			КонецЕсли;
...
// Параметры:
//  Идентификатор  - ОпределяемыйТип.УникальныйИдентификаторИС - идентификатор по которому производится загрузка.
//  ПредприятиеGLN - Структура, Неопределено -
// Возвращаемое значение:
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Допустимые цели и виды продукции (Аттестованный специалист) ВетИС';
				|en = 'Допустимые цели и виды продукции (Аттестованный специалист) ВетИС'"),
	...
	Идентификаторы.Вставить(
		...
		Новый Структура(
			...
			НСтр("ru = 'Допустимые цели и виды продукции (Уполномоченное лицо) ВетИС';
				|en = 'Допустимые цели и виды продукции (Уполномоченное лицо) ВетИС'"),
	...
	Иначе
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Не определены поля строка данных классификатора %1.';
				|en = 'Не определены поля строка данных классификатора %1.'"),
			ИмяМетаданных);
	КонецЕсли;
...
// Обработать записи складского журнала.
// 
// Параметры:
//  ПараметрыОбмена - см. ПараметрыОбмена
//  ХозяйствующийСубъект - СправочникСсылка.ХозяйствующиеСубъектыВЕТИС
//  Предприятие - СправочникСсылка.ПредприятияВЕТИС
//  ИсключаяПоследнююДату - Булево
//  ДокументДляВосстановления - Булево
// 
// Возвращаемое значение:
//  Массив из см. ИнтеграцияВЕТИСКлиентСервер.СтруктураИзменения
