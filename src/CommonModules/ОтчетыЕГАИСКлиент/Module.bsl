...
Процедура ОбработкаРасшифровкиОтчета(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Если СтруктураРасшифровки.Свойство("ДокументОснование") И СтруктураРасшифровки.ДокументОснование = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'В базе данных отсутствует документ-основание.';
										|en = 'В базе данных отсутствует документ-основание.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗапроситьОтсутствующиеТТН(ФормаОтчета) Экспорт
	...
	Если ДанныеЗапросов.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Все ТТН ЕГАИС загружены, операция не требуется';
														|en = 'Все ТТН ЕГАИС загружены, операция не требуется'"),, "ЗапроситьОтсутствующиеТТН");
		Возврат;
	КонецЕсли;
...
Процедура ЗапроситьВыделенныеДанные(ФормаОтчета) Экспорт
		...
		Для НомерСтроки = Область.Верх По Область.Низ Цикл
			Для НомерКолонки = Область.Лево По Область.Право Цикл
				...
				Если ТипЗнч(ТекущаяОбласть) = Тип("ОбластьЯчеекТабличногоДокумента")
					И ТекущаяОбласть.ТипОбласти <> ТипОбластиЯчеекТабличногоДокумента.Прямоугольник Тогда
					Продолжить;
				КонецЕсли;
	...
	Если ДанныеЗапросов.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Операция недоступна для выделенных объектов';
														|en = 'Операция недоступна для выделенных объектов'"),, "ЗапроситьВыделенныеДанные");
		Возврат;
	КонецЕсли;
