...
Процедура ПоказатьВопросДаНет(ОписаниеОповещенияОЗавершении, ТекстВопроса, ПараметрыВопроса = Неопределено) Экспорт
	...
	Если ПараметрыВопроса = Неопределено Тогда
		ПараметрыВопроса = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПараметрыВопросаДаНет();
	КонецЕсли;
	...
	Если ПараметрыВопроса.КнопкаПоУмолчанию = Неопределено Тогда
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки,,, ПараметрыВопроса.Заголовок);
	ИначеЕсли ПараметрыВопроса.КнопкаПоУмолчанию = КодВозвратаДиалога.Да Тогда
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки,, КодВозвратаДиалога.Да, ПараметрыВопроса.Заголовок);
	ИначеЕсли ПараметрыВопроса.КнопкаПоУмолчанию = КодВозвратаДиалога.Нет Тогда
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки,, КодВозвратаДиалога.Нет, ПараметрыВопроса.Заголовок);
	Иначе
...
Процедура НачатьСозданиеСвязанногоОбъектаДО(ОбъектИС, ОписаниеОповещения, ОткрытьФорму = Ложь) Экспорт
	...
	ИначеЕсли Правила.Количество() = 0 Тогда
		...
		ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'Создать в 1С:Документообороте';
													|en = 'Create in 1C:Document Management'"));
		...
		Если ТипЗнч(ОбъектДОИлиСообщение) = Тип("Строка") Тогда // Сообщение об ошибке.
			...
			ПоказатьПредупреждение(ОписаниеОповещения, ОбъектДОИлиСообщение,,
				НСтр("ru = 'Не удалось создать объект 1С:Документооборота';
					|en = 'Cannot create a 1C:Document Management object'"));
...
Процедура НачатьВыборШаблонаБизнесПроцесса(ОписаниеОповещенияОЗакрытии, Форма) Экспорт
	...
	Иначе
		ТекстПредупреждения = НСтр("ru = 'Не найдено ни одного подходящего шаблона бизнес-процесса.';
									|en = 'No suitable business process template is found.'");
...
Процедура НачатьСогласованиеПослеВопроса(Результат, Параметры) Экспорт
	...
	Если ПодходящиеШаблоны.Количество() = 0 Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Подходящих шаблонов не найдено.';
											|en = 'No suitable templates found.'"));
...
Процедура ПрерватьСогласование(Параметры) Экспорт
	...
	Если СвязанныйОбъектДО = Неопределено Тогда
		ТекстПредупреждения = НСтр("ru = 'В 1С:Документообороте не запущено согласование.';
									|en = 'Approval is not started in 1C:Document Management.'");
		ПоказатьПредупреждение(ОписаниеОповещенияПослеПредупреждения, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьСогласование(Параметры) Экспорт
	...
	Если ЗадачиСогласования.Количество() = 0 Тогда
		ТекстВопроса = НСтр("ru = 'В 1С:Документообороте нет задач согласования, адресованных на текущего пользователя %1';
							|en = '1C:Document Management does not contain any approval tasks addressed to the current user %1'");
		Кнопки = Новый СписокЗначений;
...
Процедура ДобавитьПредметПослеВыбораТипа(Результат, Параметры) Экспорт
	...
	Если Результат.Вариант = "ФайлСДиска" Тогда
		...
		ПараметрыНачалаСоздания.ВладелецID = Параметры.ИдентификаторВладельца;
		ПараметрыНачалаСоздания.ВладелецТип = Параметры.ТипВладельца;
		ПараметрыНачалаСоздания.УникальныйИдентификаторФормы = ФормаВладельца.УникальныйИдентификатор;
		ПараметрыНачалаСоздания.ВладелецПредставление =
			?(Параметры.Свойство("Представление"),
				Параметры.Представление,
				ФормаВладельца.Представление);
		ПараметрыНачалаСоздания.ОписаниеОповещения = Новый ОписаниеОповещения(
		...
		Если СтрокаПредмета.РольПредмета = "Заполняемый" //@NON-NLS-1
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не введен заполняемый предмет ""%1"". Заполнить предмет можно на стороне 1С:Документооборот.';
					|en = 'The ""%1"" object is not entered. You can fill the object in 1C:Document Management.'"),
...
Процедура НачатьДобавлениеСвязиПослеВыбораТипаСвязи(Результат, Параметры) Экспорт
	...
	Если Результат = "ПовторитьВыборДокумента" Тогда // Нажата кнопка "Назад".
		ОповещениеПродолжения = Новый ОписаниеОповещения(
			"НачатьДобавлениеСвязиПослеВыбораДокумента",
			ИнтеграцияС1СДокументооборотКлиент,
			Параметры);
...
//     структуры (ИсходныйДокумент, СвязанныйДокумент, ТипСвязи).
//
Асинх Процедура НачатьУдалениеСвязи(Знач ИсходныйДокумент, Знач СвязанныйДокумент, Знач ТипСвязи,
		Знач ОписаниеОповещения = Неопределено) Экспорт
...
Процедура СогласоватьАсинхронно(Форма, ПредметСогласования, ОповещениеОЗавершении) Экспорт
	...
	Если Форма.Объект.Ссылка.Пустая() Тогда
		...
		ТекстВопроса = НСтр("ru = 'Данные еще не записаны.
							...
							|en = 'Data not saved yet.
							|Document approval can be performed only after data is saved.
							|Data will be saved.'");
...
Процедура ПрерватьСогласованиеАсинхронно(Форма, ПредметСогласования, ОповещениеОЗавершении) Экспорт
	...
	Если Форма.Объект.Ссылка.Пустая() Тогда
		...
		ТекстВопроса = НСтр("ru = 'Данные еще не записаны.
							...
							|Aborting document approval can only be performed after data is saved.
							|Data will be saved.'");
	...
	Если Результат.Статус = "Ошибка" Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось определить возможность согласования документа по причине:
						|%1';
						|en = 'Cannot determine the document approval capability. Reason:
			...
			ВыполнитьОбработкуОповещения(
				...
				Новый Структура("ВыполненоУспешно, ОписаниеОшибки",
					...
					НСтр("ru = 'Предмет согласования не записан.';
						|en = 'The approval subject has not been saved.'")));
	...
	Если Результат.Статус = "Ошибка" Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось отправить документ на согласование по причине:
						|%1';
						|en = 'Cannot send the document for approval. Reason:
						|%1'"), Результат.КраткоеПредставлениеОшибки);
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если Результат.Статус = "Ошибка" Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось определить возможность прерывания согласования документа по причине:
						|%1';
						|en = 'Cannot determine the document approval aborting capability. Reason:
						|%1'"), Результат.КраткоеПредставлениеОшибки);
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если Результат.Статус = "Ошибка" Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось прервать согласование документа по причине:
						|%1';
						|en = 'Cannot abort document approval. Reason:
						|%1'"), Результат.КраткоеПредставлениеОшибки);
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если Результат.Статус = "Ошибка" Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось получить состояние согласования документа по причине:
						|%1';
						|en = 'Cannot get document approval state. Reason:
						|%1'"), Результат.КраткоеПредставлениеОшибки);
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьЗначениеРеквизитаСоставногоТипаВТаблицеОбработка(ИмяТипа, Форма, ТекущиеДанные, Реквизит)
	...
	Если ИмяТипа = "integer"
		...
		ПоказатьВводЗначения(Оповещение, ТекущиеДанные[Реквизит], НСтр("ru = 'Введите значение';
																		|en = 'Enter a value'"), ТипЗначения);
...
Процедура ПереключитьХронометражЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если ПараметрыХронометража.ВключенХронометраж Тогда
			...
			НСтр("ru = 'Включен хронометраж по ""%1""';
				|en = 'Timing by ""%1"" is enabled'"), Строка(ПараметрыОповещения.ОбъектИС));
			Состояние(Текст);
		КонецЕсли;
	...
	Иначе
		...
		Если ПараметрыХронометража.ВключенХронометраж Тогда
			...
			Если ПараметрыХронометража.ВключенХронометраж Тогда
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Включен хронометраж по ""%1""';
						|en = 'Timing by ""%1"" is enabled'"), Строка(Параметры.ОбъектИС));
				Состояние(Текст);
			КонецЕсли;
			...
			Иначе
				...
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Включен хронометраж по другим объектам:
						...
						|en = 'Timing by other objects is enabled:
						|%1
						|Disable timing and record labor costs before enabling new timing?'"),
						СтрокаОбъектовХронометража);
				Кнопки = Новый СписокЗначений;
				Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Да';
															|en = 'Yes'"));
