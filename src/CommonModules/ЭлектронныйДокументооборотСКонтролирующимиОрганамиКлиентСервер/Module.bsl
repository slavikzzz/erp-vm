...
Процедура ПрименитьПараметрыПрорисовкиПанелиОтправки(Форма, ПараметрыПрорисовки) Экспорт
		...
		Если БлокСостоянияОтправкиРЭЦ <> Неопределено Тогда  
			...
			Иначе
				...
				Если ЭлементФормыНаименованиеЭтапаРЭЦ <> Неопределено Тогда
					...
					НСтр("ru = '%1';
						|en = '%1'"), СостояниеОтправкиРЭЦ.ТекстНадписи);
	...
	Если ПараметрыПрорисовки.КонтролирующийОрган <> "РЭЦ" Тогда
		...
		Иначе
			...
			// Определяем цвет фона.
			БлокСостоянияОтправки.ЦветФона = ДокументооборотСКОВызовСервера.ЦветФонаПанелиОтправкиПоСтатусу(
				СостояниеСдачиОтчетности,
				СостояниеОтправки.ТекстСтатуса);
			...
			Если ЭлементФормыНаименованиеЭтапа <> Неопределено Тогда
				...
				НСтр("ru = '%1';
					|en = '%1'"), СостояниеОтправки.ТекстНадписи);
...
Функция МассивВидовПрочихУведомленийПоддерживающихДокументооборот(
	...
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(
		ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.УведомлениеСколковоОсвобождениеОтОбязанностейНалогоплательщика")) <>
		Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(
			ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.УведомлениеУчастникаСколково"));
	КонецЕсли;
	Если МассивВидовОтправляемыхДокументов.Найти(
		ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.УведомлениеПорядокПредставленияДекларацииИмущество")) <>
		Неопределено Тогда
		МассивВидовПрочихУведомленийПоддерживающихДокументооборот.Добавить(
			ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.УведомлениеПорядокПредставленияНалоговыхДеклараций"));
	КонецЕсли;
...
Функция НаименованияОтчетаБезПрефикса(НаименованиеОтчета, МаксимальнаяДлина = 25) Экспорт
	...
	МассивПрефиксов.Добавить("Журнал справок ");
	МассивПрефиксов.Добавить("Журнал ");
	МассивПрефиксов.Добавить("Консолидированная ");
	
	МассивДополнительныхПрефиксов = Новый Массив;
	МассивДополнительныхПрефиксов.Добавить("о ");
	МассивДополнительныхПрефиксов.Добавить("об ");
	МассивДополнительныхПрефиксов.Добавить("по ");
	МассивДополнительныхПрефиксов.Добавить("в ");
	МассивДополнительныхПрефиксов.Добавить("для ");
	МассивДополнительныхПрефиксов.Добавить("на ");
	МассивДополнительныхПрефиксов.Добавить("об ");
	МассивДополнительныхПрефиксов.Добавить("за ");
	МассивДополнительныхПрефиксов.Добавить("порядке ");
	МассивДополнительныхПрефиксов.Добавить("представлении ");
	МассивДополнительныхПрефиксов.Добавить("представления ");
	МассивДополнительныхПрефиксов.Добавить("выборе ");
	МассивДополнительныхПрефиксов.Добавить("налогового органа ");
	МассивДополнительныхПрефиксов.Добавить("налогового органа, ");
	МассивДополнительныхПрефиксов.Добавить("в который представляется ");
	МассивДополнительныхПрефиксов.Добавить("налоговая ");
	МассивДополнительныхПрефиксов.Добавить("налоговых ");
	МассивДополнительныхПрефиксов.Добавить("декларация ");
	МассивДополнительныхПрефиксов.Добавить("деклараций ");
	МассивДополнительныхПрефиксов.Добавить("(расчетов) ");
	
	Для каждого ТекстПрефикса Из МассивПрефиксов Цикл
		ДлинаПрефикса = СтрДлина(ТекстПрефикса);
		Если Лев(Результат[0], ДлинаПрефикса) = ТекстПрефикса Тогда
	...
	Пока Истина Цикл
		ПрефиксНайден = Ложь;
		Для каждого ТекстПрефикса Из МассивДополнительныхПрефиксов Цикл
			ДлинаПрефикса = СтрДлина(ТекстПрефикса);
			Если Лев(Результат[0], ДлинаПрефикса) = ТекстПрефикса Тогда
				Результат[0] = Сред(Результат[0], ДлинаПрефикса + 1);
				Результат[0] = СокрЛП(Результат[0]);
				ПрефиксНайден = Истина;
			КонецЕсли;
		КонецЦикла;
		Если НЕ ПрефиксНайден Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого Элемент Из МассивЭлементов Цикл
		...
		Если ДобавлятьРазмер Тогда
			ПредставлениеФайла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 (%2)';
					|en = '%1 (%2)'"), Элемент.ИмяФайла, ТекстовоеПредставлениеРазмераФайла(Элемент.Размер));
		Иначе
...
Функция ЭтоФайлВыгрузкиРеестраНДС(КороткоеИмяФайла) Экспорт
	...
	Префиксы.Добавить("KO_RRTD165-22"); // 	РегламентированныйОтчетРеестрНДСДекларацииНаТоварыВвозРеэкспорт
	...
	Префиксы.Добавить("KO_RRTD165-21"); // 	РегламентированныйОтчетРеестрНДС_КНД1155221
	Префиксы.Добавить("KO_RRDT165.7.1"); // РегламентированныйОтчетРеестрНДС_КНД1155223
...
Функция ЭтоФайлВыгрузкиРеестраАкцизов(КороткоеИмяФайла) Экспорт
	...
	Префиксы.Добавить("ON_REESTR11.1.201"); // 	РегламентированныйОтчетРеестрАкцизы_КНД1150151
...
Функция РазобратьIP(IPАдресКлиента) Экспорт
	...
	Результат = СтрРазделить(IPАдресКлиента, ".");
	Если Результат.Количество() <> КоличествоЭлементов Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Для ИндексЭлемента = 0 По КоличествоЭлементов - 1 Цикл
		Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Результат[ИндексЭлемента]) Тогда
			Возврат Неопределено;
		КонецЕсли;
		
		ТипЧисло = Новый ОписаниеТипов("Число");
		Результат[ИндексЭлемента] = ТипЧисло.ПривестиЗначение(Результат[ИндексЭлемента]);
		
		Если Результат[ИндексЭлемента] < 0 ИЛИ Результат[ИндексЭлемента] > 255 Тогда
			Возврат Неопределено;
		КонецЕсли;
	КонецЦикла;
...
Функция ЭтоЧастныйIP(ЭлементыIP) Экспорт
	...
	// https://ru.m.wikipedia.org/wiki/Частный_IP-адрес (с дополнениями)
	Возврат ЭлементыIP[0] = 0
		ИЛИ ЭлементыIP[0] = 10
		ИЛИ ЭлементыIP[0] = 100 И ЭлементыIP[1] >= 64 И ЭлементыIP[1] <= 127
		ИЛИ ЭлементыIP[0] = 127
		ИЛИ ЭлементыIP[0] = 169 И ЭлементыIP[1] = 254
		ИЛИ ЭлементыIP[0] = 172 И ЭлементыIP[1] >= 16 И ЭлементыIP[1] <= 31
		ИЛИ ЭлементыIP[0] = 192 И ЭлементыIP[1] = 0 И (ЭлементыIP[2] = 0 ИЛИ ЭлементыIP[2] = 2)
		ИЛИ ЭлементыIP[0] = 192 И ЭлементыIP[1] = 168
		ИЛИ ЭлементыIP[0] = 198 И ЭлементыIP[1] >= 18 И ЭлементыIP[1] <= 19
		ИЛИ ЭлементыIP[0] = 198 И ЭлементыIP[1] = 51 И ЭлементыIP[2] = 100
		ИЛИ ЭлементыIP[0] = 203 И ЭлементыIP[1] = 0 И ЭлементыIP[2] = 113
		ИЛИ ЭлементыIP[0] = 255 И ЭлементыIP[1] = 255 И ЭлементыIP[2] = 255 И ЭлементыIP[2] = 255;
...
Функция ПредставлениеСчетаФактурыПояснения(СтрокаТаблицы, ТипРеквизитов) Экспорт
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы["НомерИсправленияКорректировочногоСчетаФактуры"+ТипРеквизитов]) Тогда
			РеквизитыКоррСчетаФактуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Корр. %1 от %2 (испр.%3 от %4) по с/ф ';
										|en = 'Корр. %1 от %2 (испр.%3 от %4) по с/ф '"),
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																НСтр("ru = '%1%2';
																	|en = '%1%2'"), РеквизитыКоррСчетаФактуры, РеквизитыСчетаФактуры);
...
Процедура ОбработкаПроверкиЗаполненияЗаявленияОНазначенииПенсии(Форма, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если НЕ СведенияКорректны Тогда
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Вид пенсии""';
														|en = 'Заполните обязательные реквизиты раздела ""Вид пенсии""'"),,"УказательВидПенсии");
		#Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Вид пенсии""';
															|en = 'Заполните обязательные реквизиты раздела ""Вид пенсии""'"),,"УказательВидПенсии");
		#КонецЕсли
		Отказ = Истина;
	КонецЕсли;
	...
	Если НЕ СведенияКорректны Тогда
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Паспорт, контакты""';
														|en = 'Заполните обязательные реквизиты раздела ""Паспорт, контакты""'"),,"УказательСведенияОПолучателе");
		#Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Паспорт, контакты""';
															|en = 'Заполните обязательные реквизиты раздела ""Паспорт, контакты""'"),,"УказательСведенияОПолучателе");
		#КонецЕсли
		Отказ = Истина;
	КонецЕсли;
	...
	Если НЕ СведенияКорректны Тогда
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Начисление пенсии""';
														|en = 'Заполните обязательные реквизиты раздела ""Начисление пенсии""'"),,"УказательДопСведения");
		#Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Начисление пенсии""';
															|en = 'Заполните обязательные реквизиты раздела ""Начисление пенсии""'"),,"УказательДопСведения");
		#КонецЕсли
		Отказ = Истина;
	КонецЕсли;	
	...
	Если НЕ СведенияКорректны Тогда
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Представитель""';
														|en = 'Заполните обязательные реквизиты раздела ""Представитель""'"),,"Объект.ЕстьПредставитель");
		#Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Представитель""';
															|en = 'Заполните обязательные реквизиты раздела ""Представитель""'"),,"Объект.ЕстьПредставитель");
		#КонецЕсли
		Отказ = Истина;
	КонецЕсли;
	...
	Если НЕ СведенияКорректны Тогда
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Cпособ получения""';
														|en = 'Заполните обязательные реквизиты раздела ""Cпособ получения""'"),,"УказательСпособПолучения");
		#Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Cпособ получения""';
															|en = 'Заполните обязательные реквизиты раздела ""Cпособ получения""'"),,"УказательСпособПолучения");
		#КонецЕсли
		Отказ = Истина;
	КонецЕсли;
...
Функция ПроверитьЭлектронныйПочтуПолучателя(Форма, МастерДалее = Истина, ВыводитьСообщения) Экспорт
	...
	Если (Форма.КогоУведомлятьПочта = ПредопределенноеЗначение("Перечисление.ВидыПолучателейПенсии.Пенсионер") 
		...
		ПроверитьПочту(РезультатПроверкиРеквизитов, Форма, "ЭлектроннаяПочта", НСтр("ru = 'Электронная почта получателя пенсии';
																					|en = 'Электронная почта получателя пенсии'"));
	...
	Если Форма.ЭтоЗаявлениеОДоставкеПенсии Тогда
		...
		ПроверитьНаПустое(
			...
			НСтр("ru = 'Через (раздел ""До заключения договора"")';
				|en = 'Через (раздел ""До заключения договора"")'"));
		...
		ПроверитьНаПустое(
			...
			НСтр("ru = 'Наименование (раздел ""До заключения договора"")';
				|en = 'Наименование (раздел ""До заключения договора"")'"));
		...
		Если ЭтоКредитнаяОрганизация Тогда
			...
			ПроверитьНаПустое(
				...
				НСтр("ru = 'Счет (раздел ""До заключения договора"")';
					|en = 'Счет (раздел ""До заключения договора"")'"));
		Иначе
			ПроверитьНаПустое(
				РезультатПроверкиРеквизитов, 
				Форма, 
				...
				НСтр("ru = 'Как (раздел ""До заключения договора"")';
					|en = 'Как (раздел ""До заключения договора"")'"));
		КонецЕсли;
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(Форма.ПредставительАдресМестаЖительства)
			...
			РезультатПроверки.ТекстОшибки = НСтр("ru = 'Заполните хотя бы один из адресов';
												|en = 'Заполните хотя бы один из адресов'");
		...
		Если НЕ МастерДалее И ВыводитьСообщения Тогда
			#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Проверьте корректность заполнения сведений о детях';
															|en = 'Проверьте корректность заполнения сведений о детях'"));
			#Иначе
		...
		Если НЕ МастерДалее И ВыводитьСообщения Тогда
			#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Проверьте корректность заполнения сведений об инвалидах и пожилых';
															|en = 'Проверьте корректность заполнения сведений об инвалидах и пожилых'"));
			#Иначе
		...
		Если НЕ МастерДалее И ВыводитьСообщения Тогда
			#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Проверьте корректность заполнения сведений военной службы по призыву';
															|en = 'Проверьте корректность заполнения сведений военной службы по призыву'"));
			#Иначе
		...
		Если НЕ МастерДалее И ВыводитьСообщения Тогда
			#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Проверьте корректность заполнения сведений военной службы';
															|en = 'Проверьте корректность заполнения сведений военной службы'"));
			#Иначе
	...
	Если Проверять Тогда
		...
		Для каждого СтрокаРебенка Из Форма.Дети Цикл
			...
			Если НЕ ЗначениеЗаполнено(СтрокаРебенка.ФИО) Тогда
				...
				РезультатПроверки.ТекстОшибки = НСтр("ru = 'Заполните ФИО ребенка в строке ';
													|en = 'Заполните ФИО ребенка в строке '") + НомерСтроки;
				РезультатПроверки.Пустой	  = Истина;
			...
			Если НЕ ЗначениеЗаполнено(СтрокаРебенка.ДатаРождения) Тогда
				...
				РезультатПроверки = ДокументооборотСКОКлиентСервер.РезультатПроверкиРеквизитов();
				РезультатПроверки.Поле = Поле;
				РезультатПроверки.ТекстОшибки = НСтр("ru = 'Заполните дату рождения ребенка в строке ';
													|en = 'Заполните дату рождения ребенка в строке '") + НомерСтроки;
				РезультатПроверки.Пустой	  = Истина;
			...
			Если НЕ ПустаяСтрока(СНИЛСБезРазделителей) Тогда
				...
				Если НЕ ДокументооборотСКОКлиентСервер.ПроверитьСНИЛС(СтрокаРебенка.СНИЛС) Тогда
    				РезультатПроверки.ТекстОшибки = НСтр("ru = 'Некорректно указан СНИЛС в строке %1. Не соответствует маске ХХХ-ХХХ-ХХХ ХХ, где X - любая цифра';
														|en = 'Некорректно указан СНИЛС в строке %1. Не соответствует маске ХХХ-ХХХ-ХХХ ХХ, где X - любая цифра'");
    			...
    			ИначеЕсли НЕ ДокументооборотСКОКлиентСервер.ПроверитьСНИЛС(СтрокаРебенка.СНИЛС, Ложь, Истина) Тогда
    				РезультатПроверки.ТекстОшибки = НСтр("ru = 'Некорректно указан СНИЛС в строке %1. Не сошлось контрольное число (СНИЛС не существует)';
														|en = 'Некорректно указан СНИЛС в строке %1. Не сошлось контрольное число (СНИЛС не существует)'");
					РезультатПроверки.ТекстОшибки = СтрШаблон(РезультатПроверки.ТекстОшибки, НомерСтроки);
    			КонецЕсли;
	...
	Если Проверять Тогда
		...
		Если ЗначениеЗаполнено(Форма.ВоеннаяСлужбаПоПризывуНачало) Тогда
			ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, "ВоеннаяСлужбаПоПризывуОкончание", НСтр("ru = 'Дата окончания службы по призыву';
																											|en = 'Дата окончания службы по призыву'"));
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Форма.ВоеннаяСлужбаПоПризывуОкончание) Тогда
			ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, "ВоеннаяСлужбаПоПризывуНачало", НСтр("ru = 'Дата начала службы по призыву';
																										|en = 'Дата начала службы по призыву'"));
		КонецЕсли;
...
Функция СведенияОПолучателеПенсииУказаныКорректно(Форма, МастерДалее = Истина, ВыводитьСообщения) Экспорт
	...
	ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, "ПолучаетПенсиюДругогоГосударства", НСтр("ru = 'Получает пенсию другого государства';
																									|en = 'Получает пенсию другого государства'"));
	...
	Если ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.ЭтоСтраховаяПенсияПоСтаростиНакопительная(Форма) Тогда
		ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, "ЗамещаетГосударственнуюДолжность", НСтр("ru = 'Замещает государственную должность';
																										|en = 'Замещает государственную должность'"));
	КонецЕсли;
	...
	Если ЭтоСтраховаяПенсияПоПотереКормильца(Форма) Тогда
		...
		ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, "УмершийКормилецПолучалПенсиюДругогоГосударства", НСтр("ru = 'Умерший кормилец получает пенсию другого государства';
																													|en = 'Умерший кормилец получает пенсию другого государства'"));
		Если Форма.УмершийКормилецПолучалПенсиюДругогоГосударства = ПредопределенноеЗначение("Перечисление.ОтветыНаВопросыОПенсии.Да") Тогда
			ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, "СтранаВыплачивающаяПенсиюУмершемуКормильцу", НСтр("ru = 'Государство, выплачивающее пенсию умершему кормилецу';
																													|en = 'Государство, выплачивающее пенсию умершему кормилецу'"));
		КонецЕсли;
		...
		ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, "УмершийКормилецПолучаетПенсиюПоФЗ4468_1", НСтр("ru = 'Умерший кормилец получал пенсию по ФЗ РФ № 4468-1';
																												|en = 'Умерший кормилец получал пенсию по ФЗ РФ № 4468-1'"));
		Если Форма.УмершийКормилецПолучаетПенсиюПоФЗ4468_1 = ПредопределенноеЗначение("Перечисление.ОтветыНаВопросыОПенсии.ДаВПрошлом") Тогда
			ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, "ОрганВыплачивающийПенсиюУмершемуКормильцу", НСтр("ru = 'Орган, выплачивавший пенсию умершему кормильцу';
																													|en = 'Орган, выплачивавший пенсию умершему кормильцу'"));
		КонецЕсли;
		...
		ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, "УмершийКормилецПолучаетПенсиюПоФЗ3132_1", НСтр("ru = 'Умерший кормилец получал содержание по ФЗ РФ № 3132-1';
																												|en = 'Умерший кормилец получал содержание по ФЗ РФ № 3132-1'"));
		Если ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.ЭтоСтраховаяПенсияПоСтаростиНакопительная(Форма) Тогда
			ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, "УмершийКормилецЗамещалГосударственнуюДолжность", НСтр("ru = 'Умерший кормилец замещает государственную должность';
																														|en = 'Умерший кормилец замещает государственную должность'"));
		КонецЕсли;
	...
	Если Проверять Тогда
		...
		Для каждого СтрокаВоеннаяСлужба Из Форма.ВоеннаяСлужба Цикл
			...
			Если ЗначениеЗаполнено(СтрокаВоеннаяСлужба.ДатаОкончания) И НЕ ЗначениеЗаполнено(СтрокаВоеннаяСлужба.ДатаНачала)
				...
				РезультатПроверки = ДокументооборотСКОКлиентСервер.РезультатПроверкиРеквизитов();
				РезультатПроверки.Поле = Поле;
				РезультатПроверки.ТекстОшибки = НСтр("ru = 'Заполните дату начала службы в строке ';
													|en = 'Заполните дату начала службы в строке '") + НомерСтроки;
				РезультатПроверки.Пустой	  = Истина;
			...
			Если ЗначениеЗаполнено(СтрокаВоеннаяСлужба.ДатаНачала) И НЕ ЗначениеЗаполнено(СтрокаВоеннаяСлужба.ДатаОкончания)
				...
				РезультатПроверки = ДокументооборотСКОКлиентСервер.РезультатПроверкиРеквизитов();
				РезультатПроверки.Поле = Поле;
				РезультатПроверки.ТекстОшибки = НСтр("ru = 'Заполните дату окончания службы в строке ';
													|en = 'Заполните дату окончания службы в строке '") + НомерСтроки;
				РезультатПроверки.Пустой	  = Истина;
			...
			Если ЗначениеЗаполнено(СтрокаВоеннаяСлужба.ДатаНачала) 
				...
				РезультатПроверки = ДокументооборотСКОКлиентСервер.РезультатПроверкиРеквизитов();
				РезультатПроверки.Поле = Поле;
				РезультатПроверки.ТекстОшибки = НСтр("ru = 'Дата окончания службы меньше даты начала в строке ';
													|en = 'Дата окончания службы меньше даты начала в строке '") + НомерСтроки;
				РезультатПроверки.Пустой	  = Истина;
    ...
    Если Проверять Тогда
    	...
    	Для каждого СтрокаИнвалидыПожилые Из Форма.ИнвалидыПожилые Цикл
			...
			Если НЕ ЗначениеЗаполнено(СтрокаИнвалидыПожилые.ДатаНачала) Тогда
				...
				РезультатПроверки = ДокументооборотСКОКлиентСервер.РезультатПроверкиРеквизитов();
				РезультатПроверки.Поле = Поле;
				РезультатПроверки.ТекстОшибки = НСтр("ru = 'Заполните дату начала ухода за инвалидом, пожилым в строке ';
													|en = 'Заполните дату начала ухода за инвалидом, пожилым в строке '") + НомерСтроки;
				РезультатПроверки.Пустой	  = Истина;
...
Функция ПроверитьТелефон(РезультатПроверкиРеквизитов, Форма, Поле, Представление)
	...
	Если НЕ ЗначениеЗаполнено(ЭлектроннаяПодписьВМоделиСервисаКлиентСервер.ПолучитьПредставлениеТелефона(Форма[Поле])) Тогда
		...
		РезультатПроверки = ДокументооборотСКОКлиентСервер.РезультатПроверкиРеквизитов();
		РезультатПроверки.ТекстОшибки = СтрШаблон(НСтр("ru = 'Заполните поле ""%1""';
														|en = 'Заполните поле ""%1""'"), Представление);
		РезультатПроверки.Пустой      = Истина;
		РезультатПроверки.Поле        = Поле;
	...
	Если ЗначениеЗаполнено(Форма.МестоПроживания) Тогда
		...
		Если ЕстьОшибка Тогда
			...
			ТекстОшибки = НСтр("ru = 'Не может быть указано значение ""Проживает за пределами РФ"", так как в разделе ""Получение, информирование"" может быть указан только российский адрес';
								|en = 'Не может быть указано значение ""Проживает за пределами РФ"", так как в разделе ""Получение, информирование"" может быть указан только российский адрес'"); 
	...
	Если ПроверитьНаПустое(РезультатПроверкиРеквизитов, Форма, Поле, Представление)
		...
		Если НЕ СтрНайти(Форма[Поле], "@") Тогда
			РезультатПроверки.ТекстОшибки = 
				СтрШаблон(НСтр("ru = 'Некорректно заполнено поле ""%1"". Отсутствует символ @';
								|en = 'Некорректно заполнено поле ""%1"". Отсутствует символ @'"), Представление);
		Иначе 
			РезультатПроверки.ТекстОшибки = 
				СтрШаблон(НСтр("ru = 'Поле ""%1"" содержит некорректные сочетания символов';
								|en = 'Поле ""%1"" содержит некорректные сочетания символов'"), Представление);
		КонецЕсли;
		РезультатПроверкиРеквизитов.Добавить(РезультатПроверки);
...
Функция ПредставлениеДетиИнвалиды(Таблица) Экспорт
	...
	ИначеЕсли Количество = 2 Тогда
		Представление = Таблица[0].ФИО + НСтр("ru = ' и ';
												|en = ' и '") + Таблица[1].ФИО;
	Иначе
		Представление = Таблица[0].ФИО + НСтр("ru = ' и еще ';
												|en = ' и еще '") + Строка(Количество - 1);
	КонецЕсли;
...
Функция ПредставлениеВоеннаяСлужба(Таблица) Экспорт
	...
	ИначеЕсли Количество = 1 Тогда
		...
		Если Количество = 2 Тогда
			...
			Представление = Первый + НСтр("ru = ' и ';
											|en = ' и '") + Второй;
			
		ИначеЕсли Количество > 2 Тогда
			
			Представление = Первый + НСтр("ru = ' и еще ';
											|en = ' и еще '") + Строка(Количество - 1);
	...
	Иначе
		...
		Иначе
			...
			Если Количество = 2 Тогда
				...
				Представление = ПервыйИнтервал + НСтр("ru = ' и ';
														|en = ' и '") + ВторойИнтервал;
			Иначе
				Представление = ПервыйИнтервал + НСтр("ru = ' и еще ';
														|en = ' и еще '") + Строка(Количество - 1);
			КонецЕсли;
