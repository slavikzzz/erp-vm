...
Процедура ВнешнееСобытиеПолученыШтрихкоды(ЗавершениеОбработки, Форма, Источник, Событие, Данные, ПараметрыСканирования = Неопределено) Экспорт
	...
	Если Результат <> Неопределено
		...
		ЭтоРасширеннаяВерсияГосИС = ОбщегоНазначенияИСКлиент.ЭтоРасширеннаяВерсияГосИС();
		...
		Если ПараметрыСканирования <> Неопределено
			...
			Если Форма.СоответствиеШтрихкодовСтрокДерева.Получить(НормализованныйШтрихкод) <> Неопределено Тогда
				Если ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияИСМП(ВидПродукции)
					...
					СтрокаДерева = МодульСобытияФормИСКлиент.НайтиПоИдентификатору(Форма.ДеревоМаркированнойПродукции,
						Форма.СоответствиеШтрихкодовСтрокДерева.Получить(НормализованныйШтрихкод));
					Если СтрокаДерева <> Неопределено Тогда
						ЕстьВложенныеСтроки = Ложь;
						МодульИнтеграцияИСКлиентСервер = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияИСКлиентСервер");
						Если МодульИнтеграцияИСКлиентСервер.ЭтоУпаковка(СтрокаДерева.ТипУпаковки) Тогда
	...
	ИначеЕсли Результат <> Неопределено
		...
		И ОбщегоНазначенияИСКлиент.ЭтоРасширеннаяВерсияГосИС()
		И Найти(Форма.ПоддерживаемыеТипыПодключаемогоОборудования, "СчитывательRFID") > 0
		И Не Форма.ИдетЗаписьRFID Тогда
		
		МодульСобытияФормИСКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СобытияФормИСКлиент");
		МодульСобытияФормИСКлиент.ОбработкаВнешнегоСобытияПолученыШтрихкоды(ЗавершениеОбработки, Форма, Результат, Данные, ПараметрыСканирования);
...
Функция ПреобразоватьДанныеСоСканераВСтруктуру(Параметр) Экспорт
	...
	ОбщегоНазначенияСобытияФормИСКлиентПереопределяемый.ПреобразоватьДанныеСоСканераВСтруктуру(Результат, Параметр);
	Возврат Результат;
		...
		Если ЗначениеЗаполнено(РезультатВыбораДанныеШтрихкода.Характеристика) Тогда
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Код маркировки не соответствует номенклатуре %1 (%2)';
					|en = 'Код маркировки не соответствует номенклатуре %1 (%2)'"),
				ДанныеУточнения.Номенклатура, ДанныеУточнения.Характеристика);
		Иначе
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Код маркировки не соответствует номенклатуре %1';
											|en = 'Код маркировки не соответствует номенклатуре %1'"), ДанныеУточнения.Номенклатура);
		КонецЕсли;
		...
		Если ПараметрыСканирования.РежимБРМК
			И (РезультатОбработки.ДанныеШтрихкода <> Неопределено
				И РезультатОбработки.ДанныеШтрихкода.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Логистическая")
					Или ЗначениеЗаполнено(РезультатОбработки.ТекстОшибки))
			И Форма.ИмяФормы <> "РегистрСведений.ВскрытыеПотребительскиеУпаковкиИС.Форма.ФормаЗаписи" Тогда
...
Асинх Процедура ОбработатьАсинхронноШтрихкод(ПараметрыЗавершенияОбработкиШтрихкода, ОписаниеПослеЗавершения = Неопределено) Экспорт
	
	ПараметрыСканирования = ПараметрыЗавершенияОбработкиШтрихкода.ПараметрыСканирования;
	РезультатОбработки    = ПараметрыЗавершенияОбработкиШтрихкода.РезультатОбработкиШтрихкода;
	
	Если ПрисутствуетПродукцияИСМП(ПараметрыСканирования.ДопустимыеВидыПродукции, Истина)
		И ПрисутствуетПродукцияИСМП(РезультатОбработки.ВидыПродукции, Истина)
		И Не РезультатОбработки.ТребуетсяУточнениеДанных Тогда
		
		//@skip-check wrong-string-literal-content
		МодульОбработки = ОбщегоНазначенияКлиент.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСМПКлиент");
		Результат       = Ждать МодульОбработки.ОбработатьАсинхронноШтрихкодНаЛокальномМодулеЧестныйЗнак(ПараметрыЗавершенияОбработкиШтрихкода);
		
		РезультатОбработки = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(Результат.ПараметрыЗавершенияВводаШтрихкода.РезультатОбработкиШтрихкода.ДанныеШтрихкода);
		
		Если ОписаниеПослеЗавершения <> Неопределено И Не Результат.Отказ Тогда
			ВыполнитьОбработкуОповещения(ОписаниеПослеЗавершения, Новый Структура("ДанныеВыбора, ЗапомнитьВыбор", РезультатОбработки, Ложь));
		КонецЕсли;
