...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение версии схем обмена с ГИСМ.';
									|en = 'Заполнение версии схем обмена с ГИСМ.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление настроек обмена с ГИСМ.';
									|en = 'Обновление настроек обмена с ГИСМ.'");
	...
	Если ТипОрганизация <> Тип("Строка") Тогда
		...
		Обработчик.Комментарий = НСтр("ru = 'Перенос GLN из справочника организаций в регистр ""Организации для обмена с ГИСМ"".';
										|en = 'Перенос GLN из справочника организаций в регистр ""Организации для обмена с ГИСМ"".'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение ресурса GLN в регистре ""Очередь получения квитанций о фиксации ГИСМ"".';
									|en = 'Заполнение ресурса GLN в регистре ""Очередь получения квитанций о фиксации ГИСМ"".'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Очищает в документе ""Уведомление об импорте маркированных товаров"" виды меха не соответствующие новому классификатору.';
									|en = 'Очищает в документе ""Уведомление об импорте маркированных товаров"" виды меха не соответствующие новому классификатору.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление версии схем обмена с ГИСМ до 2.41';
									|en = 'Обновление версии схем обмена с ГИСМ до 2.41'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Обработка ошибочно присвоенного статуса документа ""Уведомление об отгрузке маркированной продукции"".';
									|en = 'Обработка ошибочно присвоенного статуса документа ""Уведомление об отгрузке маркированной продукции"".'");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП") Тогда
		...
		Обработчик.Комментарий = НСтр("ru = 'Включение вида продукции Продукция из натурального меха.';
										|en = 'Включение вида продукции Продукция из натурального меха.'");
...
Процедура ВерсииСхемОбменаГИСМ_ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось обработать константу ВерсииСхемОбменаГИСМ по причине: %Причина%';
								|en = 'Не удалось обработать константу ВерсииСхемОбменаГИСМ по причине: %Причина%'");
...
Процедура НастройкиОбменаГИСМ_ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось обработать константу НастройкиОбменаГосИС по причине: %Причина%';
								|en = 'Не удалось обработать константу НастройкиОбменаГосИС по причине: %Причина%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Предупреждение,
...
Процедура ОтключениеГИСМ_ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если ИспользуетсяПодсистемаГИСМ Тогда
		...
		Попытка
			...
			Настройка = Новый Структура("ВидПродукции, ВестиУчетПродукции, ДатаКонтрольПроизводства", 
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось включить вид продукции Продукция из натурального меха по причине: %Причина%';
									|en = 'Не удалось включить вид продукции Продукция из натурального меха по причине: %Причина%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Предупреждение,
