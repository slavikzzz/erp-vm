...
Процедура ОбъектНеНайден(Штрихкод, ИскатьПоВсемОбъектам = Истина) Экспорт
	...
	Если ИскатьПоВсемОбъектам Тогда
		Состояние(НСтр("ru = 'Выполняется поиск документа по штрихкоду во всех документах информационной базы..';
						|en = 'Searching for the document by barcode in all documents of the infobase..'"));
	...
	Если ВывестиСообщениеОбъектНеНайден Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Объект со штрихкодом %1 не найден';
																														|en = 'Object with barcode %1 is not found'"), Штрихкод));
	КонецЕсли;
