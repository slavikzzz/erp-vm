...
Процедура ПриДобавленииКлассификаторов(Классификаторы) Экспорт
	...
	Описатель.Наименование           = НСтр("ru = 'Данные налоговой нагрузки и рентабельности по отраслям';
											|en = 'Data of tax burden and profitability by industries'");
	...
	Исключение
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru = 'Некорректный формат файла классификатора, обработка прервана';
				|en = 'Incorrect classifier file format, processing is canceled'"),
			Истина,
...
Процедура НачальноеЗаполнение() Экспорт
	...
	Исключение
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru = 'Некорректный формат макета классификатора, обработка прервана';
				|en = 'Incorrect classifier template format, processing is canceled'"),
			Истина,
			Метаданные.РегистрыСведений.ПредельныеЗначенияОКВЭД2);
	...
	Иначе
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = '""Data of tax burden and profitability by industries"" classifier version %1 is not imported
			| as newer version %2 is imported. Check for configuration updates.'"),
	...
	Если Не ЗначениеЗаполнено(ДанныеКлассификатора)
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru = 'Отсутствуют данные или данные не являются структурой';
				|en = 'No data or the data is not a structure'"),
			Истина,
			Метаданные.РегистрыСведений.ПредельныеЗначенияОКВЭД2);
	...
	Если ОжидаемыеДанные.version = Неопределено Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru = 'Отсутствует свойство ""version""';
				|en = 'The ""version"" property is missing'"),
			Истина,
			Метаданные.РегистрыСведений.ПредельныеЗначенияОКВЭД2);
	...
	ИначеЕсли ОжидаемыеДанные.classifierData = Неопределено Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru = 'Отсутствует свойство ""classifierData""';
				|en = 'The ""classifierData"" property is missing'"),
			Истина,
			Метаданные.РегистрыСведений.ПредельныеЗначенияОКВЭД2);
	...
	ИначеЕсли ТипЗнч(ДанныеКлассификатора.version) <> Тип("Число") Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru = 'Неправильно заполнено свойство ""version""';
				|en = 'The ""version"" property is filled incorrectly'"),
			Истина,
			Метаданные.РегистрыСведений.ПредельныеЗначенияОКВЭД2);
	...
	ИначеЕсли ТипЗнч(ДанныеКлассификатора.classifierData) <> Тип("Массив") Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru = 'Неправильно заполнено свойство ""classifierData""';
				|en = 'The ""classifierData"" property is filled incorrectly'"),
			Истина,
			Метаданные.РегистрыСведений.ПредельныеЗначенияОКВЭД2);
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Данные нагрузки и рентабельности';
				|en = 'Data of tax burden and profitability'",
