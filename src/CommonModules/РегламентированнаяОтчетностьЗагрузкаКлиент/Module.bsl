...
Процедура ЗагрузитьИзФайлаОтчетПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Исключение
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не предусмотрена загрузка данных в отчет ""%1"".';
					|en = 'Не предусмотрена загрузка данных в отчет ""%1"".'"), ПредставлениеДокументаРеглОтч);
			Сообщение.Сообщить();
			Возврат;
		КонецПопытки;
		...
		Если НЕ ЗначениеЗаполнено(ИмяФормыОтчета) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В выбранном отчетном периоде не найдена действующая форма отчета ""%1"".';
					|en = 'В выбранном отчетном периоде не найдена действующая форма отчета ""%1"".'"), ПредставлениеДокументаРеглОтч);
			Сообщение.Сообщить();
			Возврат;
		КонецЕсли;
	...
	Исключение
		ОписаниеОшибки = НСтр("ru = 'Ошибка: ';
								|en = 'Ошибка: '") + ИнформацияОбОшибке().Описание + ".";
	КонецПопытки;
	Если ФормаОтчета = Неопределено Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось получить заполненную по умолчанию форму отчета ""%1"". %2';
				|en = 'Не удалось получить заполненную по умолчанию форму отчета ""%1"". %2'"), ПредставлениеДокументаРеглОтч, ОписаниеОшибки);
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	ТекстКомментария = НСтр("ru = 'Данные отчета загружены из файла.';
							|en = 'Данные отчета загружены из файла.'");
	ТекстДопКомментария = ?(ПустаяСтрока(Результат.Комментарий), "", НСтр("ru = ' Примечание: ';
																			|en = ' Примечание: '") + СокрЛП(Результат.Комментарий) +  ".");
	...
	Если НовыйСохраненныйДок = Неопределено ИЛИ НовыйСохраненныйДок.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось создать документ отчета для загрузки данных из файла в ""%1"".';
				|en = 'Не удалось создать документ отчета для загрузки данных из файла в ""%1"".'"), ПредставлениеДокументаРеглОтч);
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Если Результат.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.НДС") Тогда
		...
		Если ФормаКонсолидацииОтчетностиПоНДС = Неопределено Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не удалось загрузить данные с помощью обработки ""Консолидация отчетности по НДС"".';
									|en = 'Не удалось загрузить данные с помощью обработки ""Консолидация отчетности по НДС"".'");
			Сообщение.Сообщить();
			Возврат;
		КонецЕсли;
	...
	Если РезультатЗагрузки И Результат.Свойство("РезультатДлительнойОперации", РезультатДлительнойОперации) Тогда
		...
		ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Выполняется загрузка отчета из файла';
												|en = 'Выполняется загрузка отчета из файла'");
					...
					Исключение
						...
						Исключение
							...
							Сообщение.Текст = НСтр("ru = 'Не предусмотрена очистка экранной формы без запроса.';
													|en = 'Не предусмотрена очистка экранной формы без запроса.'");
							Сообщение.Сообщить();
						КонецПопытки;
					КонецПопытки;
				КонецЕсли;
		...
		Если ДанныеДляЗагрузки.Свойство("СообщениеОбОшибке", СообщениеОбОшибке) И СообщениеОбОшибке <> Неопределено Тогда
			Если ТипЗнч(СообщениеОбОшибке) = Тип("Структура") Тогда
				...
				Иначе
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Загрузка данных из файла в ""%1"" не выполнена.';
							|en = 'Загрузка данных из файла в ""%1"" не выполнена.'"), ПредставлениеОтчета);
					Сообщение.Сообщить();
				КонецЕсли;
			Иначе
				Сообщение = Новый СообщениеПользователю;
...
Процедура ПолучениеФайловДляЗагрузкиНачало(Адрес, ФайлыДляЗагрузки = "", ДополнительныеПараметры = Неопределено) Экспорт
	...
	ТекстСообщения = НСтр("ru = 'Для загрузки файлов электронных представлений необходимо установить расширение работы с файлами.';
							|en = 'Для загрузки файлов электронных представлений необходимо установить расширение работы с файлами.'");
		   ...
		   И ОсновнойФайлНаДиске.Размер > МаксимальныйРазмер Тогда
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Согласно требованиям ФНС файл формата 5.01 больше %2 Мб должен быть разбит на файлы формата 5.02.%1Загрузить файл с разбивкой?';
					|en = 'Согласно требованиям ФНС файл формата 5.01 больше %2 Мб должен быть разбит на файлы формата 5.02.%1Загрузить файл с разбивкой?'"),
				Символы.ПС,
				Строка(РазмерВМб));
	...
	Если ФайлПолучен Тогда
		...
		Иначе
			...
			Если ФормаЭлектронногоПредставления = Неопределено Тогда
				Если ТипЗнч(ВходящийКонтекст.ОповещениеОЗакрытииФормы) = Тип("ОписаниеОповещения")
					И ТипЗнч(ВходящийКонтекст.ОповещениеОЗакрытииФормы.ДополнительныеПараметры) = Тип("Структура") Тогда 
					
					КнопкаЗагрузить = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ВходящийКонтекст.ОповещениеОЗакрытииФормы.ДополнительныеПараметры, "КнопкаЗагрузить");
					Если ТипЗнч(КнопкаЗагрузить) = Тип("КнопкаФормы") Тогда 
						КнопкаЗагрузить.Доступность = Истина;
					КонецЕсли;
				КонецЕсли;
				
				ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Не удалось открыть форму электронного представления для файла: ""%1"".';
																			|en = 'Не удалось открыть форму электронного представления для файла: ""%1"".'"),
...
Функция ПолучитьДиалогВыбораФайла()
	...
	ДиалогВыбора.Заголовок = НСтр("ru = 'Выберите файл(ы) для загрузки';
									|en = 'Выберите файл(ы) для загрузки'");
...
Функция ПолучитьЗаполненнуюПоУмолчаниюФормуОтчета(Знач ПолноеИмяФормыОтчета, ПараметрыФормыОтчета)
	...
	ИначеЕсли ПараметрыФормыОтчета = "Форма отчета не найдена" Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Форма отчета не найдена в выбранном периоде!';
										|en = 'Форма отчета не найдена в выбранном периоде!'"));
		Возврат Неопределено;
	ИначеЕсли ТипЗнч(ПараметрыФормыОтчета) <> Тип("Структура") Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Недостаточно параметров для создания отчета!';
										|en = 'Недостаточно параметров для создания отчета!'"));
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ОсновнаяФормаОтчета = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не удалось получить основную форму отчета!';
										|en = 'Не удалось получить основную форму отчета!'"));
		Возврат Неопределено;
	КонецЕсли;
	   ...
	   И (ПараметрыФормыОтчета.Свойство("ОрганизацияЮЛ"))Тогда
		Если ПараметрыФормыОтчета.ОрганизацияЮЛ = Неопределено ИЛИ ПараметрыФормыОтчета.ОрганизацияЮЛ.Пустая() Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Не удалось подобрать организацию для формы отчета!';
											|en = 'Не удалось подобрать организацию для формы отчета!'"));
			Возврат Неопределено;
		Иначе
	...
	Если ВыбраннаяФормаОтчета = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не удалось получить заполненную по умолчанию форму отчета!';
										|en = 'Не удалось получить заполненную по умолчанию форму отчета!'"));
		Возврат Неопределено;
	КонецЕсли;
			...
			Исключение
				...
				Исключение
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка добавления файла %1 с именем ""%2"".';
							|en = 'Ошибка добавления файла %1 с именем ""%2"".'"), СтрВидДополнительногоФайла, СвойстваФайла.ИмяФайла);
...
Функция РеквизитОбъектаЕслиСуществует(Объект, ИмяРеквизита)
	...
	Если НЕ РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Объект, ИмяРеквизита) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не найдено свойство ""%1"" объекта ""%2"".';
				|en = 'Не найдено свойство ""%1"" объекта ""%2"".'"), ИмяРеквизита, Строка(Объект));
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
...
Процедура ВыполнитьПреобразованиеИзПрошлойРедакции(ФормаОтчета, РезультатВопроса) Экспорт
	...
	ПараметрыФормыДляВыгрузки.Вставить("мВыбраннаяФорма", СтруктураРеквизитовФормы.мСкопированаФорма);
	...
	ФормаОтчета.Комментарий = НСтр("ru = 'Данные отчета сконвертированы из другой редакции.';
									|en = 'Данные отчета сконвертированы из другой редакции.'");
	...
	Если РезультатЗагрузки И ДанныеДляЗагрузки.Свойство("РезультатДлительнойОперации", РезультатДлительнойОперации) Тогда
		...
		ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Выполняется конвертация отчета из другой редакции';
												|en = 'Выполняется конвертация отчета из другой редакции'");
		ПараметрыОжидания.ВыводитьПрогрессВыполнения = Истина;
