...
Функция СтатусДокументооборота(Объект) Экспорт
	...
	ТекстСообщенияОбОшибкеВоВходящихПараметрах = НСтр("ru = 'Неверный первый параметр ""Объект"": 
		...
		|en = 'The first parameter ""Object"" is invalid: 
		|- The value must be a link to the electronic document or an object included in the ""ОснованияЭлектронныхДокументов"" flexible type collection.'");
...
Функция ОписаниеСпециальногоОбстоятельстваФНС(КодСпециальногоОбстоятельства) Экспорт
	Возврат РегистрыСведений.ПереченьСпециальныхОбстоятельствДокументаЭДО.ОписаниеСпециальногоОбстоятельстваФНС(
		КодСпециальногоОбстоятельства);
КонецФункции
