		...
		Если ЗначениеЗаполнено(ЗаполненВнутреннийИдентификаторЕИС) Тогда
			Если Не ЗначениеЗаполнено(ТекущийОбъект.ЭтапГосконтрактаЕИС) Тогда
				...
				ТекстСообщения = НСтр("ru = 'Не заполнен этап госконтракта.';
										|en = 'The state contract step is not filled.'");
	...
	Если Результат.Следующий() Тогда
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru = 'Не удалось записать справочник ""Договоры""
				|%1';
				|en = 'Cannot save the ""Contracts"" catalog
				|%1'");
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Изменение договоров контрагента';
					|en = 'Change counterparty contracts'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ЗаполнитьТабличнуюЧастьДокументаПоДаннымГосконтракта(ДанныеФормы, ЗаполнениеАктаВыполненныхРабот = Ложь) Экспорт
	...
	Если ДанныеДляЗаполнения = Неопределено Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'В договоре не указан государственный контракт.';
													|en = 'A state contract is not specified in the contract.'"));
		Возврат;
	КонецЕсли;
	...
	Если ДанныеДляЗаполнения.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Номенклатура контракта не сопоставлена с данными БД.';
													|en = 'The contract item is not mapped with the database data.'"));
		Возврат;
	КонецЕсли;
	...
	Если ЗаполнениеАктаВыполненныхРабот И Не ЕстьНоменклатураПоАкту Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'В контракте нет номенклатуры, подходящей для акта выполненных работ.';
													|en = 'The contract does not contain an item suitable for the Customer invoice — Services.'"));
	КонецЕсли;
	...
	Если ВладелецКонтракта <> ДоговорСсылка Тогда
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru = 'Не удалось записать справочник ""Государственные контракты""
				|%1';
				|en = 'Cannot save the ""State contracts"" catalog
				|%1'");
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Изменение государственного контракта';
					|en = 'Change state contract'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.Справочники.ДоговорыКонтрагентов, 
				Госконтракт,
				ТекстСообщения);
...
Процедура ЗаполнитьДанныеДереваДляЭлектронногоАктированияЕИС(ДеревоДанных,
	...
	Если ДанныеЭлектронногоАктированияЕИС.ДанныеГосконтракта.Количество() = 0 Тогда
		ОписаниеОшибки = НСтр("ru = 'Не заполнены данные для электронного актирования в форме документа.';
								|en = 'Data for electronic certification in the document form is not filled.'");
	...
	Если ЗначениеЗаполнено(ВыборкаШапки.Кладовщик) Тогда
		...
		Иначе
			...
			//++ НЕ УТ			
			КладовщикФИО = ВыборкаШапки.Кладовщик.ФИО;
			//-- НЕ УТ			
		КонецЕсли;
	...
	Если Не ОтветственныеЗаОформление.Количество() Тогда
		ОписаниеОшибки = НСтр("ru = 'Не заполнен менеджер ответственный за оформление УПД.';
								|en = 'The employee in charge of UTD registration is not filled.'");
		ЭлектронноеАктированиеЕИС.ДобавитьОшибкуЗаполненияПриложения(
			ДанныеПриложенияЕИС, ОписаниеОшибки);
		Возврат;
	КонецЕсли;
	...
	Если Не ВыборкаПодписантов.Количество() Тогда
		ОписаниеОшибки = НСтр("ru = 'Менеджер ответственный за оформление УПД не найден в списке подписантов личного кабинета ЕИС.';
								|en = 'The employee in charge of UTD registration is not found in the list of signers of the personal UIS account.'");
		ЭлектронноеАктированиеЕИС.ДобавитьОшибкуЗаполненияПриложения(
			ДанныеПриложенияЕИС, ОписаниеОшибки);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьДанныеПриложенияУПД(ДанныеЭлектронногоАктированияЕИС, ДанныеПриложенияЕИС) Экспорт
	...
	Если ДанныеЭлектронногоАктированияЕИС.ДанныеГосконтракта.Количество() = 0 Тогда
		ОписаниеОшибки = НСтр("ru = 'Не заполнены данные государственного контракта для электронного актирования.';
								|en = 'State contract data for electronic certification is not filled.'");
		ЭлектронноеАктированиеЕИС.ДобавитьОшибкуЗаполненияПриложения(
			ДанныеПриложенияЕИС, ОписаниеОшибки);
		Возврат;
	Иначе
...
Процедура ПроверитьДоговорЭД(Настройки, ДокументОтгрузки, ДеревоДанных, ЭтоУКД) Экспорт
	...
	Если ЭтоУКД И ЭтоВозврат Тогда
		ТекстОшибки = НСтр("ru = 'Возврат товаров от клиента не отправляется при актировании. Используйте корректировку реализации.';
							|en = 'Return of goods from the customer is not sent upon certification. Use the sales adjustment.'");
	Иначе
		Шаблон = НСтр("ru = 'Выбран договор, привязанный к контракту %1. Отправка документа в ЕИС не поддерживается, для отправки контрагенту используйте договор без привязки к госконтракту.';
						|en = 'The contract linked to the %1 contract is selected. Sending the document to UIS is not supported. To send it to a counterparty, use a contract without a link to a state contract.'");
...
Процедура ЗаполнитьМестаПоставкиПриложенияЕИСПоДаннымУчастника(СведенияОбУчастнике, СтрокаУчастника, ВидУчастника, ДанныеПриложенияЕИС, ДатаСведений = Неопределено) Экспорт
	...
	Если ВРег(ВидУчастника) = ВРег("СведенияОПокупателе") Тогда
		КлючУчастника = "Покупатель";
	ИначеЕсли ВРег(ВидУчастника) = ВРег("СведенияОГрузополучателе") Тогда
		КлючУчастника = "Грузополучатель";
	Иначе
		КлючУчастника = ВидУчастника;
	КонецЕсли;
	СтрокаУчастника[КлючУчастника].ИнформацияДляУчастника = ИдентификаторУчастника;
	...
	Если ЗначениеЗаполнено(АдресУчастника) Тогда
		...
		Попытка
			ПолучитьСведенияАдресаПоГАР	= ПолучитьСведенияАдресаПоГАР(СведенияОбУчастнике, "Факт");
			МестоПоставки.КодГАР		= ПолучитьСведенияАдресаПоГАР.КодГар;
			МестоПоставки.КодСтраны		= ПолучитьСведенияАдресаПоГАР.КодСтраны;
		Исключение
		КонецПопытки;
...
Функция ПолучитьСведенияАдресаПоГАР(СведенияОбУчастнике, ВидАдреса)
	
	КодГАРИКодСтраны = Новый Структура;
	КодГАРИКодСтраны.Вставить("КодГАР", "");
	КодГАРИКодСтраны.Вставить("КодСтраны", "");
	
	Ссылка = СведенияОбУчастнике.Ссылка;
	
	Если ВидАдреса = "Юр" Тогда
		Вид = ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ЮрАдресКонтрагента");
	Иначе
		Вид = ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ФактАдресКонтрагента");
	КонецЕсли; 
	
	// Определения вида адреса аналогично ОбменСКонтрагентамиБП.ПолучитьАдресСтруктурой.
	...
	КонтактнаяИнформация = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(
		Ссылка,
		Вид, ТекущаяДатаСеанса(), Ложь);
		
	Если КонтактнаяИнформация.Количество() = 0 Тогда
		
		КодОсновногоЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
		ИмяСобытия = НСтр("ru = 'ЭлектронноеАктированиеЕИС.ПриФормированииАдресаМестаПоставки';
							|en = 'ЭлектронноеАктированиеЕИС.ПриФормированииАдресаМестаПоставки'",
			КодОсновногоЯзыка);
		ПодробноеПредставлениеОшибки = НСтр("ru = 'Не удалось получить адрес участника.';
											|en = 'Не удалось получить адрес участника.'",
			КодОсновногоЯзыка);
		ЗаписьЖурналаРегистрации(ИмяСобытия,
			УровеньЖурналаРегистрации.Ошибка, , ,
			ПодробноеПредставлениеОшибки);
			
		Возврат КодГАРИКодСтраны;
		
	КонецЕсли;
	
	АдресСтрока = КонтактнаяИнформация[0];
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("КодыАдреса", Истина);
	Сведения = РаботаСАдресами.СведенияОбАдресе(АдресСтрока.Значение, ДополнительныеПараметры);
	КодГАРИКодСтраны.КодСтраны = Сведения.КодСтраны;
	Если ЗначениеЗаполнено(Сведения.ИдентификаторДома) Тогда
		КодГАРИКодСтраны.КодГАР = Строка(Сведения.ИдентификаторДома);
	Иначе
		КодГАРИКодСтраны.КодГАР = Строка(Сведения.ИдентификаторАдресногоОбъекта);
	КонецЕсли;
...
Процедура ЗаполнитьДанныеПоСтрокеТоваровУПД_2019(НоваяСтрокаЭД, СтрокаВыборки, СтруктураДанных, ДанныеПриложенияЕИС) Экспорт
	...
	НоваяСтрокаЭД.Код = СтрокаДокумента.КодТовараДляЕИС;
	НоваяСтрокаЭД.Наименование = СтрокаДокумента.Наименование;
	...
	Если ДанныеКонтракта.ИнформацияОЦенах.СпособУказанияЦены = СпособМаксимальнаяЦена
		...
		НоваяСтрокаЭД.Цена = Окр(ЦенаИзКонтрактаСНДС * МножительНДС, 11);
...
Процедура ЗаполнитьДанныеПоСтрокеТоваровУКД(НоваяСтрокаЭД, СтрокаВыборки, СтруктураДанных, ДанныеПриложенияЕИС) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента) Тогда
		Шаблон = НСтр("ru = 'Не найдена строка госконтракта по номенклатуре партнера строки документа %1.';
						|en = 'The state contract line by partner items numbers of the %1 document line is not found.'");
		ТекстОшибки = СтрШаблон(Шаблон, ИдентификаторСтроки);
		ЭлектронноеАктированиеЕИС.ДобавитьОшибкуЗаполненияПриложения(
			ДанныеПриложенияЕИС, ТекстОшибки);
		Возврат;
