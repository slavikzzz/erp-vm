...
Функция ОперандыТекстовойФормулы(Формула) Экспорт
	...
	Если (МассивОперандов.Найти("ПланируемыеНачальныеОстатки") <> Неопределено
		...
		//++ НЕ УТ
		Если МассивОперандов.Найти("ПланыПроизводства") = Неопределено Тогда
			МассивОперандов.Добавить("ПланыПроизводства");
		КонецЕсли;
		Если МассивОперандов.Найти("ПланыВыпуска") = Неопределено Тогда
			МассивОперандов.Добавить("ПланыВыпуска");
		КонецЕсли;
		//-- НЕ УТ
	...
	Если (МассивОперандов.Найти("ПланируемыеНачальныеОстатки") <> Неопределено
		...
		//++ НЕ УТ
		Если МассивОперандов.Найти("ПланыПроизводстваМатериалы") = Неопределено Тогда
			МассивОперандов.Добавить("ПланыПроизводстваМатериалы");
		КонецЕсли;
		Если МассивОперандов.Найти("ПланыПроизводстваПотребностьВПолуфабрикатах") = Неопределено Тогда
			МассивОперандов.Добавить("ПланыПроизводстваПотребностьВПолуфабрикатах");
		КонецЕсли;
		//-- НЕ УТ
	...
	Если (МассивОперандов.Найти("ПланируемыеНачальныеОстаткиСОтбором") <> Неопределено
		...
		//++ НЕ УТ
		Если МассивОперандов.Найти("ПланыПроизводстваСОтбором") = Неопределено Тогда
			МассивОперандов.Добавить("ПланыПроизводстваСОтбором");
		КонецЕсли;
		Если МассивОперандов.Найти("ПланыВыпускаСОтбором") = Неопределено Тогда
			МассивОперандов.Добавить("ПланыВыпускаСОтбором");
		КонецЕсли;
		//-- НЕ УТ
	...
	Если (МассивОперандов.Найти("ПланируемыеНачальныеОстаткиСОтбором") <> Неопределено
		...
		//++ НЕ УТ
		Если МассивОперандов.Найти("ПланыПроизводстваМатериалыСОтбором") = Неопределено Тогда
			МассивОперандов.Добавить("ПланыПроизводстваМатериалыСОтбором");
		КонецЕсли;
		Если МассивОперандов.Найти("ПланыПроизводстваПотребностьВПолуфабрикатахСОтбором") = Неопределено Тогда
			МассивОперандов.Добавить("ПланыПроизводстваПотребностьВПолуфабрикатахСОтбором");
		КонецЕсли;
		//-- НЕ УТ
...
Функция СформироватьЗаголовокПериода(Знач Периодичность, Знач ДатаНачала, Знач ДатаОкончания, Знач ОтображатьНомерПериода = Ложь, Замещен = Ложь) Экспорт
	...
	ИначеЕсли Периодичность = ПредопределенноеЗначение("Перечисление.Периодичность.Квартал") Тогда
		Если ОтображатьНомерПериода Тогда
			Заголовок = Формат(ДатаНачала, НСтр("ru = 'ДФ=''q''';
												|en = 'DF=''q'''")) + " " + НСтр("ru = 'квартал';
																				|en = 'quarter'");
		Иначе
	...
	ИначеЕсли Периодичность = ПредопределенноеЗначение("Перечисление.Периодичность.Год") Тогда
		Заголовок = Формат(ДатаНачала, НСтр("ru = 'ДФ=yyyy';
											|en = 'DF=yyyy'")) + НСтр("ru = ' г.';
																		|en = ' year'");
	Иначе 
...
Процедура ПроверитьДатуПериодаТЧ(Объект, Отказ, Параметры) Экспорт 
	...
	Если ЗначениеЗаполнено(Параметры.Периодичность) И ЗначениеЗаполнено(Параметры.ДатаНачала) И ЗначениеЗаполнено(Параметры.ДатаОкончания) Тогда
		...
		ТекстОшибки = НСтр("ru = '%1 должна соответствовать периоду плана ""%2"" - ""%3""';
							|en = '%1 must correspond to the period plan ""%2"" - ""%3""'");
...
Функция ОтборДляЗаполненияПоказателей(ТипПлана, Параметры, ПараметрыВидаПлана) Экспорт
	...
	//++ НЕ УТ
	Если ТипПлана = ПредопределенноеЗначение("Перечисление.ТипыПланов.ПланПроизводства") Тогда 
		
		Если ПараметрыВидаПлана.Свойство("ЗаполнятьПодразделение") И ПараметрыВидаПлана.ЗаполнятьПодразделение Тогда
			Отбор.Вставить("ОтборПодразделениеДиспетчер", Параметры.Подразделение);
		КонецЕсли;
		
		//++ НЕ УТКА
		Отбор.Вставить("ОтборТипПроизводственногоПроцесса", Параметры.ТипПроизводственногоПроцесса);
		//-- НЕ УТКА
...
Функция ТекстУстановкиФормулыПолуфабриката() Экспорт 
	...
	Возврат НСтр("ru = 'Выберите строку с продукцией';
				|en = 'Select a line with products'");
