...
// Оставляет только Организации не применяющие ЛьготноеНалогообложениеНДСПриУСН
//
// Параметры:
//	Организации - СправочникСсылка.Организации - Список организаций, который нужно обработать.
//	Период - Дата - Дата, на которую необходимо определить применяется ли льготное налогообложение НДС.
//
// Возвращаемое значение:
//	Список - список организаций не применяющих ЛьготноеНалогообложениеНДСПриУСН..
//

Функция ОрганизацииНеНаЛьготномНалогообложении(Организации, Период) Экспорт
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиУчетаНДСПриУСН.СрезПоследних(&Период,ОсновноеНалогообложение = Значение(Перечисление.ТипыНалогообложенияНДС.ЛьготноеНалогообложениеНДСНаУСН)) КАК НастройкиУчетаНДСПриУСНСрезПоследних
	|		ПО НастройкиУчетаНДСПриУСНСрезПоследних.Организация = Организации.Ссылка
	|ГДЕ
...
Функция СоответствиеСтавокНДССтроковымЗначениям(СоответствиеСтавокНДС = Неопределено) Экспорт
	...
	Соответствие.Вставить(Справочники.СтавкиНДС.БезНДС, Перечисления.СтавкиНДС.БезНДС);
...
Функция СчетаФактурыПолученныеПоДокументуЗакупки(ПараметрыРегистрации)
	...
	Иначе
		ТекстОчистить = НСтр("ru = 'Очистить документ приобретения в счете-фактуре';
							|en = 'Clear the purchase document in the tax invoice'");
...
Процедура Использование_ОформлениеСчетовФактурИТаможенныхДокументов(ПараметрыОбработчика) Экспорт
	...
	Если ОрганизацииКОбработке.Количество() = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Для управленческой организации данные документы не оформляются.';
				|en = 'These documents are not registered for management company.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ТекстВыполнить = НСтр("ru = 'Оформить';
							|en = 'Register'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Для Каждого Организация Из ОрганизацииКОбработке Цикл 
		...
		Если Результат.ЕстьОбязательныеОперацииКВыполнению Тогда     
			...
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По организации ""%1"" требуется оформление документов по учету НДС (%2 шт.)';
					|en = 'Registration of documents for VAT accounting for %1 company is required (%2 pcs.)'", ОбщегоНазначения.КодОсновногоЯзыка()), 
				Организация,
		...
		Если Результат.ЕстьСчетФактураПолученныйАвансОшибкиСумм Тогда 
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По организации ""%1"" в этом квартале есть расхождения в суммах выданных авансов по данным взаиморасчетов и по данным СФ поставщиков';
					|en = 'There are discrepancies in amounts of prepayments to vendors according to AR/AP accounting data and according to the data of tax invoices from vendors for the ""%1"" company in the current quarter'",
					ОбщегоНазначения.КодОсновногоЯзыка()), 
				Организация);
			ТекстВыполнить = НСтр("ru = 'Подробнее';
		...
		Если Результат.ЕстьСчетФактураВыданныйАвансОшибкиСумм Тогда
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По организации ""%1"" в этом квартале есть расхождения в суммах полученных авансов по данным взаиморасчетов и по данным СФ';
					|en = 'There are discrepancies in amounts of prepayments from customers according to AR/AP accounting data and according to tax invoice data for the ""%1"" company in the current quarter'",
					ОбщегоНазначения.КодОсновногоЯзыка()), 
				Организация);
			ТекстВыполнить = НСтр("ru = 'Подробнее';
									|en = 'Details'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если НЕ ЕстьОбязательныеОперацииКВыполнению Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет неоформленных документов.';
				|en = 'No unregistered documents.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
// 	ТаблицаЭтапов - (См. Обработки.ОперацииЗакрытияМесяца.ЗаполнитьОписаниеЭтаповЗакрытияМесяца)
// 	ТекущийРодитель - Строка - идентификатор группы.
Процедура ДобавитьЭтап_ПеречислениеНДСВБюджет(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
	...
	НоваяСтрока.ТекстВыполнить     = НСтр("ru = 'Оплатить';
											|en = 'Pay'");
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
		"УчетНДСРФ.Использование_ПеречислениеНДСВБюджет");
...
Процедура Использование_ПеречислениеНДСВБюджет(ПараметрыОбработчика) Экспорт
	...
	ЗакрытиеМесяцаСервер.УвеличитьКоличествоОбработанныхДанныхДляЗамера(ПараметрыОбработчика, КоличествоДокументовКОплате);
	...
	Иначе
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Требуется оплата НДС в бюджет по счетам-фактурам налогового агента (%1 шт.)';
				|en = 'VAT payment to budget by tax invoices of the tax agent is required (%1 pcs.)'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПолучитьПартии(ПараметрыРасчета) Экспорт
	...
	|			ТОГДА ЕСТЬNULL(НастройкиУчетаНДСПриУСН.ОсновноеНалогообложение, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС) )
	...
	|		ПО НастройкиУчетаНДСПриУСН.Организация = Партии.Организация 
	|		И НастройкиУчетаНДСПриУСН.ОсновноеНалогообложение = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЛьготноеНалогообложениеНДСНаУСН)
	|	
	|ГДЕ
...
Процедура ВернутьДокументыКОтражениюВРеглУчете(ПараметрыРасчета) Экспорт
	
#Область ТекстНДСПредъявленный
	...
	|ИЗ
	|	НДСЗаписиКнигиПокупокСторноПоИсправленнымСчетамФактурам КАК НДСЗаписиКнигиПокупок
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ИЗ
	|	НДСЗаписиКнигиПродажСторноПоИсправленнымСчетамФактурам КАК НДСЗаписиКнигиПродаж
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПогрешностиОкругления.Период КАК Период,
	|	ПогрешностиОкругления.Регистратор КАК Документ,
	|	ПогрешностиОкругления.Организация КАК Организация,
	|	НАЧАЛОПЕРИОДА(&КонецПериода, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСПредъявленныйПогрешностиОкругления КАК ПогрешностиОкругления
	|ГДЕ
	|	ПогрешностиОкругления.НДС <> 0
	|";
	
#КонецОбласти
	
#Область ТекстНДСАвансыПолученные
	
	ТекстНДСАвансыПолученные = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НДСАвансыПолученные.Период КАК Период,
	|	НДСАвансыПолученные.Регистратор КАК Документ,
	|	НДСАвансыПолученные.Организация КАК Организация,
	|	НАЧАЛОПЕРИОДА(НДСАвансыПолученные.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСАвансыПолученныеПоИСФ КАК НДСАвансыПолученные
	|ГДЕ
	|	НДСАвансыПолученные.НДС <> 0
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	НАЧАЛОПЕРИОДА(НДСАвансыПолученные.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСАвансыПолученныеЗачеты КАК НДСАвансыПолученные
	|ГДЕ
	|	НДСАвансыПолученные.НДС <> 0
	|
	|ОБЪЕДИНИТЬ
	...
	|	НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПокупок.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСЗаписиКнигиПокупокСторноПоИсправленнымСчетамФактурамВыданнымНаАванс КАК НДСЗаписиКнигиПокупок
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПродаж.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСЗаписиКнигиПродажСторноПоИсправленнымСчетамФактурамВыданнымНаАванс КАК НДСЗаписиКнигиПродаж
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НДСАвансыПолученныеПереносы.Период КАК Период,
	|	НДСАвансыПолученныеПереносы.Регистратор КАК Документ,
	|	НДСАвансыПолученныеПереносы.Организация КАК Организация,
	|	НАЧАЛОПЕРИОДА(НДСАвансыПолученныеПереносы.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСАвансыПолученныеПереносыПриход КАК НДСАвансыПолученныеПереносы
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	НАЧАЛОПЕРИОДА(НДСАвансыПолученныеПереносы.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСАвансыПолученныеПереносыРасход КАК НДСАвансыПолученныеПереносы
	|";
#КонецОбласти

#Область ТекстНДСАвансыВыданные
	
	ТекстНДСАвансыВыданные = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НДСАвансыВыданные.Период КАК Период,
	|	НДСАвансыВыданные.Регистратор КАК Документ,
	|	НДСАвансыВыданные.Организация КАК Организация,
	|	НАЧАЛОПЕРИОДА(НДСАвансыВыданные.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСАвансыВыданныеПоИСФ КАК НДСАвансыВыданные
	|ГДЕ
	|	НДСАвансыВыданные.НДС <> 0
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	НАЧАЛОПЕРИОДА(НДСАвансыВыданные.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСАвансыВыданныеЗачеты КАК НДСАвансыВыданные
	|ГДЕ
	|	НДСАвансыВыданные.НДС <> 0
	|
	|ОБЪЕДИНИТЬ
	...
	|	НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПокупок.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСЗаписиКнигиПокупокСторноПоИсправленнымСчетамФактурамПолученнымНаАванс КАК НДСЗаписиКнигиПокупок
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПродаж.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСЗаписиКнигиПродажСторноПоИсправленнымСчетамФактурамПолученнымНаАванс КАК НДСЗаписиКнигиПродаж
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	НАЧАЛОПЕРИОДА(НДСАвансыПолученныеПереносы.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСАвансыВыданныеПереносыПриход КАК НДСАвансыПолученныеПереносы
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	НАЧАЛОПЕРИОДА(НДСАвансыПолученныеПереносы.Период, ДЕНЬ) КАК ДатаОтражения
	|ИЗ
	|	НДСАвансыВыданныеПереносыРасход КАК НДСАвансыПолученныеПереносы
	|";
...
#КонецОбласти
	...
	Если ПараметрыРасчета.РасчетАвансовПолученных Тогда
		ТекстыОтражения.Добавить(ТекстНДСАвансыПолученные);
	КонецЕсли;
	Если ПараметрыРасчета.РасчетАвансовВыданных Тогда
		ТекстыОтражения.Добавить(ТекстНДСАвансыВыданные);
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = СтрСоединить(ТекстыОтражения, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении(Истина));
	...
	РеглУчетСервер.ВернутьДокументыКОтражению(ПараметрыРасчета.МенеджерВременныхТаблиц);
	...
	Если ПараметрыРасчета.РасчетАвансовПолученных Тогда
		...
		ОбщегоНазначенияУТ.УничтожитьВременныеТаблицы(
			ПараметрыРасчета.МенеджерВременныхТаблиц,
			"ТаблицаВыборочнойРегистрации,ТаблицаТекущейРегистрации");
...
Процедура ОписаниеПроверок_ФормированиеДвиженийПоНДС(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Есть расхождения сумм торговых документов и счетов-фактур';
			|en = 'There are variances in amounts of commercial documents and tax invoices'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаРасхожденияСумм(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("СчетФактура", НСтр("ru = 'Счет-фактура полученный';
											|en = 'Received tax invoice'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		|Убедитесь в корректности сумм счета-фактуры полученного';
		|en = 'A variance in amounts of the tax invoice and the commercial base document for the company %1 is found.
		|Make sure that amounts of the received tax invoice is correct'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"СчетФактура");
...
Процедура Использование_ФормированиеСводныхСправокНДС(ПараметрыОбработчика) Экспорт
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиУчетаНДСПриУСН.СрезПоследних(&НачалоПериода, Организация В
	|			(ВЫБРАТЬ
	|				ГоловныеОрганизации.Организация
	|			ИЗ
	|				ВтГоловныеОрганизации КАК ГоловныеОрганизации)) КАК НастройкиУчетаНДСПриУСНСрезПоследних
	|		ПО ГоловныеОрганизации.Организация = НастройкиУчетаНДСПриУСНСрезПоследних.Организация
	...
	|	ИЛИ НастройкиУчетаНДСПриУСНСрезПоследних.ОсновноеНалогообложение В 
	|													(ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЛьготноеНалогообложениеНДСНаУСН),
	|													ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)))
	...
	Если ПараметрыОбработчика.ПараметрыРасчета.НачалоПериода < Константы.ДатаНачалаПримененияСводныхСправок.Получить()
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru = 'Сводные справки не применяются';
				|en = 'Consolidated certificates are not applied'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ИначеЕсли ЗакрытиеМесяцаСервер.ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика, Ложь, "ОрганизацииОСНО")
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
			ПараметрыОбработчика,
			НСтр("ru = 'Необходимо сформировать сводные справки';
				|en = 'Generate consolidated certificates'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Пока Выборка.Следующий() Цикл
			...
			ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для организации ""%1"" не установлена учетная политика по НДС на начало квартала';
						|en = 'For the %1 company, a VAT accounting policy is not set at the start of the quarter'", ОбщегоНазначения.КодОсновногоЯзыка()),
					Выборка.Организация));
		КонецЦикла;
	Иначе
		...
		Исключение
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр(
					...
					|%2';
					|en = 'Generation of VAT consolidated certificates for the period %1 is made with the error:
					|%2'", ОбщегоНазначения.КодОсновногоЯзыка()),
				РасчетСебестоимостиПротоколРасчета.ПредставлениеПериодаРасчета(НачалоПериода),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПроверкаРучныеКорректировкиСводныхСправок(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Организация",    НСтр("ru = 'Организация';
												|en = 'Company'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("СводнаяСправка", НСтр("ru = 'Сводная справка НДС';
												|en = 'VAT reconciliated certificate'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"вт_РучныеКорректировкиСводныхСправок",
		НСтр("ru = 'По организации %1 оформлены сводные справки НДС, в которых вручную отредактированы суммы.';
			|en = 'Consolidated VAT certificates are registered by the company %1 where amounts are edited manually.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"СводнаяСправка");
		...
		ИначеЕсли НЕ СтрокаОрганизации.ПрименитьПравило5Процентов И СтрокаОценки.Доля <= 5 Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У организации ""%1"" расходы по реализации не облагаемой НДС не превышают 5% от общих расходов. НДС может быть принят к вычету в полном объеме.';
					|en = 'The ""%1"" company expenses for sale not subject to VAT do not exceed 5% of total expenses. VAT can be fully accepted for deduction.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		ИначеЕсли СтрокаОрганизации.ПрименитьПравило5Процентов И СтрокаОценки.Доля > 5 Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У организации ""%1"" расходы по реализации не облагаемой НДС превышают 5% от общих расходов. Необходимо выполнить распределение НДС между видами деятельности.';
					|en = 'The ""%1"" company expenses for sale not subject to VAT exceed 5% of total expenses. Allocate VAT among activity categories.'", ОбщегоНазначения.КодОсновногоЯзыка()),
				СтрокаОрганизации.Организация);
...
Функция ЗаписатьДвиженияЖурналаСФ(ДокументДвижений, НаборЗаписей, РучнаяКорректировка, Отказ) Экспорт
	...
	Если РучнаяКорректировка Тогда
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Ручная корректировка журнала СФ';
					|en = 'Manual adjustment of tax invoice register'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Иначе
		...
		Исключение
			...
			ШаблонОшибки = НСтр("ru = 'Не удалось перепровести документ по причине %1';
								|en = 'Cannot repost document. Reason: %1'") 
...
Процедура ДополнитьИнформациейОРНПТ(Знач ДополнительнаяИнформация, Знач СтруктураПараметров, Запрос, ПоискРНПТПоОснованию = Ложь) Экспорт
	...
	|	СводнаяСправкаНДС.Организация КАК Организация,
	|	СводнаяСправкаНДС.Ссылка КАК Документ,
	...
	|		ПО СписокСчетовФактур.СчетФактура = СводнаяСправкаНДС.Ссылка
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация
	|;
	...
	|	ВложенныйЗапрос.СводнаяСправка КАК СводнаяСправка
	|ПОМЕСТИТЬ ВТ_РозничныеПродажи
	|ИЗ
	|	(ВЫБРАТЬ
	|		ЧекККМ.Ссылка КАК Документ,
	|		ПериодыРозничныхПродаж.Документ КАК СводнаяСправка
	|	ИЗ
	...
	|				Т.КассоваяСмена
	|			ИЗ
	|				КассовыеСменыСОтчетами КАК Т)//ДополнительныеТаблицы
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	|		ОтчетОРозничныхПродажахТовары.Ссылка,
	|		ПериодыРозничныхПродаж.Документ
	|	ИЗ
	...
	|			И ОтчетОРозничныхПродажахТовары.Ссылка.Дата
	|				МЕЖДУ ПериодыРозничныхПродаж.НачалоПериода И ПериодыРозничныхПродаж.КонецПериода
	|	ГДЕ
	|		ОтчетОРозничныхПродажахТовары.Ссылка.Проведен
	|		И ОтчетОРозничныхПродажахТовары.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа),
	...
	|			И ОтчетКомиссионераТовары.Ссылка.Дата
	|				МЕЖДУ ПериодыРозничныхПродаж.НачалоПериода И ПериодыРозничныхПродаж.КонецПериода
	|	ГДЕ
	|		ОтчетКомиссионераТовары.Ссылка.Проведен
	|		И ОтчетКомиссионераТовары.ВидЗапасов.ТипЗапасов В (&ТипыЗапасовСобственные)
	|		И (НЕ ОтчетКомиссионераТовары.Покупатель.ЮрФизЛицо В (ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ЮрЛицо),
	...
	|	ГДЕ
	|		ОтчетКомиссионераТовары.Ссылка.Проведен
	|		И ОтчетКомиссионераТовары.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа),
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга))
	|		И (НЕ ОтчетКомиссионераТовары.Покупатель.ЮрФизЛицо В (ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ЮрЛицо),
	|			ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ИндивидуальныйПредприниматель))
	|		ИЛИ ОтчетКомиссионераТовары.Покупатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))
	...
	|			И ОтчетОРозничныхВозвратахТовары.Ссылка.Дата
	|				МЕЖДУ ПериодыРозничныхПродаж.НачалоПериода И ПериодыРозничныхПродаж.КонецПериода
	|			И ОтчетОРозничныхВозвратахТовары.ДокументРеализации.Дата
	|				МЕЖДУ ПериодыРозничныхПродаж.НачалоПериода И ПериодыРозничныхПродаж.КонецПериода
	|	ГДЕ
	|		ОтчетОРозничныхВозвратахТовары.Ссылка.Проведен
	|		И ОтчетОРозничныхВозвратахТовары.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа),
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга))
	...
	|		И ОтчетОРозничныхВозвратахТовары.ПоЧекуКоррекции) КАК ВложенныйЗапрос
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Документ
	|;
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЧекККМВозврат) Тогда
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//ДополнительныеТаблицы", "
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЧекККМКоррекции) Тогда
	...
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(СписокСчетовФактур.СчетФактура) = ТИП(Документ.СчетФактураВыданный)
	|			ТОГДА ВЫРАЗИТЬ(СписокСчетовФактур.СчетФактура КАК Документ.СчетФактураВыданный).ДокументОснование
	|		КОГДА ТИПЗНАЧЕНИЯ(СписокСчетовФактур.СчетФактура) = ТИП(Документ.СчетФактураКомиссионеру)
	|			ТОГДА ВЫРАЗИТЬ(СписокСчетовФактур.СчетФактура КАК Документ.СчетФактураКомиссионеру).ДокументОснование 
	|		КОГДА ТИПЗНАЧЕНИЯ(СписокСчетовФактур.СчетФактура) = ТИП(Документ.СчетФактураПолученный)
	|			ТОГДА СФДокументыОснования.ДокументОснование
	|		ИНАЧЕ ЕСТЬNULL(ВТ_РозничныеПродажи.Документ, СписокСчетовФактур.СчетФактура)
	|	КОНЕЦ КАК Документ,
	|	ЕСТЬNULL(ВТ_РозничныеПродажи.Документ, СписокСчетовФактур.СчетФактура) КАК СчетФактура,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(СписокСчетовФактур.СчетФактура) = ТИП(Документ.СчетФактураВыданный)
	|			ТОГДА ВЫРАЗИТЬ(СписокСчетовФактур.СчетФактура КАК Документ.СчетФактураВыданный).Корректировочный
	|		ИНАЧЕ ЛОЖЬ 
	|	КОНЕЦ КАК Корректировочный,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(СписокСчетовФактур.СчетФактура) = ТИП(Документ.СчетФактураВыданный)
	|			ТОГДА ВЫРАЗИТЬ(СписокСчетовФактур.СчетФактура КАК Документ.СчетФактураВыданный).Исправление
	|		ИНАЧЕ ЛОЖЬ 
	|	КОНЕЦ КАК Исправление
	|ПОМЕСТИТЬ ВТ_СопоставлениеСФОснованийПромежуточный
	|ИЗ
	|	СписокСчетовФактур КАК СписокСчетовФактур
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_РозничныеПродажи КАК ВТ_РозничныеПродажи
	|		ПО СписокСчетовФактур.СчетФактура = ВТ_РозничныеПродажи.СводнаяСправка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученный.ДокументыОснования КАК СФДокументыОснования
	|		ПО СписокСчетовФактур.СчетФактура = СФДокументыОснования.Ссылка
    |ГДЕ
	|	НЕ СписокСчетовФактур.НаАванс
	|	И НЕ ТИПЗНАЧЕНИЯ(СписокСчетовФактур.СчетФактура) = ТИП(Документ.СчетФактураКомитента)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(СчетФактураКомитентаПокупатели.СчетФактураВыданный) = ТИП(Документ.СчетФактураВыданный)
	|			ТОГДА ВЫРАЗИТЬ(СчетФактураКомитентаПокупатели.СчетФактураВыданный КАК Документ.СчетФактураВыданный).ДокументОснование
	|		ИНАЧЕ СписокСчетовФактур.СчетФактура
	|	КОНЕЦ,
	...
	|ИЗ
	|	СписокСчетовФактур КАК СписокСчетовФактур
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураКомитента.Покупатели КАК СчетФактураКомитентаПокупатели
	|		ПО СписокСчетовФактур.СчетФактура = СчетФактураКомитентаПокупатели.Ссылка
	|ГДЕ
	|	НЕ СписокСчетовФактур.НаАванс
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|	ВЫБОР 
	|	   КОГДА ВТ_СопоставлениеСФОснований.ИСПРАВЛЕНИЕ ТОГДА
	|            ВТ_СопоставлениеСФОснований.СчетФактура
	|	   ИНАЧЕ ВТ_СопоставлениеСФОснований.Документ
	|   КОНЕЦ КАК ДокументДляСвязи,
	|   ВТ_СопоставлениеСФОснований.Документ,
	|   ВТ_СопоставлениеСФОснований.Корректировочный,
	|   ВТ_СопоставлениеСФОснований.Исправление
	|
	|ПОМЕСТИТЬ ВТ_СопоставлениеСФОснований
	|ИЗ
	|	ВТ_СопоставлениеСФОснованийПромежуточный КАК ВТ_СопоставлениеСФОснований
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Документ;	
	|	
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_СопоставлениеСФОснованийПромежуточный
	|;
	|";

	
	// В книге покупок невозможно отразить Счет-фактуру комиссионеру. Уберем его из запроса, чтобы не было ошибки
	Если НЕ СтруктураПараметров.МенеджерВременныхТаблиц.Таблицы.Найти("СписокСчетовФактур").Колонки.СчетФактура.ТипЗначения.СодержитТип(Тип("ДокументСсылка.СчетФактураКомиссионеру")) Тогда
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЕСТЬNULL(ВТ_РозничныеПродажи.СводнаяСправка, ВТ_СопоставлениеСФОснований.СчетФактура) КАК СчетФактура,
	|	ДанныеРНПТ.РНПТ КАК РегНомПросл,
	|	ДанныеРНПТ.ОКЕИ КАК ОКЕИ,
	|	СУММА(ДанныеРНПТ.КоличествоРНПТ) КАК КолТовПросл,
	|	СУММА(ДанныеРНПТ.СуммаБезНДС) КАК СтоимТовПросл
	|ИЗ
	|	ДанныеРНПТ КАК ДанныеРНПТ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СопоставлениеСФОснований КАК ВТ_СопоставлениеСФОснований
	|		ПО ДанныеРНПТ.Документ = ВТ_СопоставлениеСФОснований.ДокументДляСвязи
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_РозничныеПродажи КАК ВТ_РозничныеПродажи
	|		ПО ДанныеРНПТ.Документ = ВТ_РозничныеПродажи.Документ
	|
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(ВТ_РозничныеПродажи.СводнаяСправка, ВТ_СопоставлениеСФОснований.СчетФактура),
	|	ДанныеРНПТ.РНПТ,
	|	ДанныеРНПТ.ОКЕИ
	|ИТОГИ ПО
	|	СчетФактура
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_СписокДокументов
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_СопоставлениеСФОснований
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ДанныеРНПТ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_РозничныеПродажи";
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Пока ВыборкаПоРНПТ.Следующий() Цикл 
			...
			Если ЗначениеЗаполнено(ВыборкаПоРНПТ.РегНомПросл) Тогда
				...
				СтруктураСвРегНом.КолТовПросл=Окр(СтруктураСвРегНом.КолТовПросл,0);
				СтруктураСвРегНом.СтоимТовПросл=Окр(СтруктураСвРегНом.СтоимТовПросл,0);
...
//++ НЕ УТ
Процедура ЗаполнениеДанныхПоДоговорамДляПроверкиВРЭЦ(Параметры) Экспорт
	...
	|			ПО ДокументыОтгрузки.СчетФактура = РеализацияТоваровУслуг.Ссылка
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|			ПО ДокументыОтгрузки.СчетФактура = РеализацияУслугПрочихАктивов.Ссылка) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Контрагент,
	|	ВложенныйЗапрос.ДоговорКонтрагента";
	...
	РеквизитыДекларацииНДС = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		Параметры.ДекларацияНДС, "ДатаНачала, ДатаОкончания, ДанныеОтчета");
	
	СписокДоступныхОрганизаций = Новый СписокЗначений;
	РегламентированнаяОтчетность.ПолучитьСписокДоступныхОрганизаций(СписокДоступныхОрганизаций);
	Запрос.УстановитьПараметр("НачалоПериода",               НачалоДня(РеквизитыДекларацииНДС.ДатаНачала));
	Запрос.УстановитьПараметр("КонецПериодаГраница",         Новый Граница(КонецДня(РеквизитыДекларацииНДС.ДатаОкончания), ВидГраницы.Включая));
	Запрос.УстановитьПараметр("КонецПериода",                КонецДня(РеквизитыДекларацииНДС.ДатаОкончания));
	...
	ТаблицаДоговоровСФайлами = Новый ТаблицаЗначений;
	ТаблицаДоговоровСФайлами.Колонки.Добавить("Контрагент", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
	ТаблицаДоговоровСФайлами.Колонки.Добавить("ДоговорКонтрагента", Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));
	ТаблицаДоговоровСФайлами.Колонки.Добавить("Файл", Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентовПрисоединенныеФайлы"));
	ТаблицаДоговоровСФайлами.Колонки.Добавить("ИндексКартинки", Новый ОписаниеТипов("Число"));
	ТаблицаДоговоровСФайлами.Колонки.Добавить("Наименование", Новый ОписаниеТипов("Строка"));
	...
	Для Каждого Строка Из Результат Цикл
		ЕстьПодходящиеВложенияКДоговору = Ложь;
		Файлы = Новый Массив;
		РаботаСФайлами.ЗаполнитьПрисоединенныеФайлыКОбъекту(Строка.ДоговорКонтрагента, Файлы);
		Для Каждого Файл Из Файлы Цикл
			
			ДанныеФайла = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Файл, "Наименование, ПометкаУдаления, ИндексКартинки, Расширение");
			Если ДанныеФайла.ПометкаУдаления = Ложь И ВозможныеРасширения.Найти(ДанныеФайла.Расширение) <> Неопределено Тогда
				НоваяСтрока = ТаблицаДоговоровСФайлами.Добавить();
				НоваяСтрока.ДоговорКонтрагента = Строка.ДоговорКонтрагента;
				НоваяСтрока.Контрагент         = Строка.Контрагент;
				НоваяСтрока.Файл               = Файл;
				НоваяСтрока.ИндексКартинки     = ДанныеФайла.ИндексКартинки;
				НоваяСтрока.Наименование       = ДанныеФайла.Наименование;
				ЕстьПодходящиеВложенияКДоговору = Истина;
			КонецЕсли;
		КонецЦикла;
		
		Если Не ЕстьПодходящиеВложенияКДоговору Тогда
			НоваяСтрока = ТаблицаДоговоровСФайлами.Добавить();
			НоваяСтрока.ДоговорКонтрагента = Строка.ДоговорКонтрагента;
			НоваяСтрока.Контрагент         = Строка.Контрагент;
		КонецЕсли;
	...
	ПоместитьВоВременноеХранилище(ТаблицаДоговоровСФайлами, Параметры.АдресВоВременномХранилище);
	
КонецПроцедуры

Функция ПолучитьПризнакВеденияУчетаТаможенныхДекларацийЭкспорт() Экспорт
	
	Возврат ПолучитьФункциональнуюОпцию("ВестиУчетТаможенныхДекларацийНаЭкспорт");
