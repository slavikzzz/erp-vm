			...
			Иначе
				...
				Если СвойстваОбъекта.Количество() > 0 Тогда
					...
					ИначеЕсли ДопРеквизиты.Количество() > 1 Тогда
						...
						|en = 'Cannot calculate the name under the template.
						|The infobase stores several records of additional attributes and data with the ""%ИмяРеквизита%"" ID included into the set of additional attributes and data of the recorded directory item ""%ИмяОбъекта%"".'");
		...
		Если Параметры.ТоварВШапке Тогда
			...
			Если НоменклатураКлиентСервер.СтатусыСерийСерияНеУказана().Найти(ДокументОбъект.СтатусУказанияСерий) <> Неопределено
				...
				ТекстСообщения = НСтр("ru = 'Количество товара ""%Товар%"" не соответствует указанному для него количеству серий. Исправьте серии.';
										|en = 'Quantity of the %Товар% goods does not correspond to the quantity of batch specified for them. Correct the batch.'");
				...
				Пока Выборка.Следующий() Цикл
					...
					Если Выборка.ОшибкаКоличества Тогда
						Если Выборка.ЭтоТоварноеМесто Тогда
							ШаблонСообщения = НСтр("ru = 'Политика учета серий товара ""%1"" предусматривает, что количество товарных мест будет соответствовать количеству товара равному 1, т.е. %2';
													|en = 'According to the batch accounting policy of the ""%1"" item, the number of cargo packagings matches the number of items equal to 1, that is, %2'");
						...
						Иначе
							ШаблонСообщения = НСтр("ru = 'Указанное количество товара ""%1"" с серией ""%2"" равно %3. Политика учета серий данного товара предусматривает, что количество по любой серии этого товара всегда будет равно 1.';
													|en = 'The specified quantity of the ""%1"" item with batch ""%2"" is equal to %3. According to the batch accounting policy of this item, the quantity by any item batch always equals 1.'");
					...
					Если Выборка.ОшибкаУпаковки Тогда
						ШаблонСообщения = НСтр("ru = 'Политика учета серий товара ""%1"" предусматривает, что серии указываются для каждого экземпляра. Укажите единичную упаковку в строке %2 списка ""%3"".';
												|en = 'According to the batch accounting policy of the ""%1"" item, batches are specified for each item. Specify a single packaging unit in line %2 of the ""%3"" list.'");
					...
					Если Выборка.ОшибкаНеУказанаСерия
						...
						Если (Выборка.СтатусУказанияСерий = 13
							...
							ШаблонСообщения = НСтр("ru = 'Не заполнена колонка ""Серия"" в строке %1 списка ""%2""';
													|en = 'The ""Batch"" column is not filled in line %1 of the ""%2"" list'");
						...
						Иначе
							ШаблонСообщения = НСтр("ru = 'Для товара ""%1"" указано по сериям %2 %4. Необходимо указать %3 %4. Исправьте серии.';
													|en = 'For the ""%1"" item, %2 %4 is specified by batches. Specify %3 %4. Correct the batches.'");
...
Процедура ПроверитьВозможностьОткрытияФормыУказанияСерий(НастройкиИспользованияСерий, ПараметрыУказанияСерий, ЗначенияПолейСвязи) Экспорт
	...
	Если Не НастройкиИспользованияСерий.ИспользоватьСерии Тогда
		ТекстИсключения = НСтр("ru = 'Для вида номенклатуры ""%ВидНоменклатуры%"" серии не используются';
								|en = 'Batches are not used for the ""%ВидНоменклатуры%"" item kind'");
	...
	Пока ВыборкаПоСерии.Следующий() Цикл 
		...
		Если ОписанияИзлишков.Количество() <> 0 Или ОписанияНедостач.Количество() <> 0 Тогда
			...
			СообщениеПользователю = НСтр("ru = 'Из следующих товарных мест нельзя собрать целое количество %ЕдИзм% товара ""%Товар%"": %ОписаниеИзлишков%. Необходимо или удалить эти товарные места, или дополнить товарными местами: %ОписаниеНедостач%.';
										|en = 'Cannot assemble the entire quantity %ЕдИзм% of ""%Товар%"" goods from the following cargo packagings: %ОписаниеИзлишков%. Either delete these cargo packagings, or add the following cargo packagings: %ОписаниеНедостач%.'");
	...
	Если ИмяТЧ = "Объект" Тогда
		...
		ИначеЕсли Объект.Количество = 0 Или Объект.КоличествоУпаковок = 0 Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнено поле ""%ПредставлениеПоляКоличество%""';
									|en = '""%ПредставлениеПоляКоличество%"" is required'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения,
	...
	Если ИмяТЧ = "Объект" Тогда
		...
		Если Не ПараметрыПроверки.ПроверитьВозможностьОкругления Тогда
			ШаблонОшибкаОкругления = НСтр("ru = 'Количество в ""%Упаковка%"" переведено в количество в ""%БазоваяЕдиницаИзмерения%"" с погрешностью - %КоличествоОтклонение% %Упаковка%, что недопустимо, что свидетельствует об ошибке в работе программы. Обратитесь к администратору.';
											|en = 'Quantity in ""%Упаковка%"" is transferred to quantity in ""%БазоваяЕдиницаИзмерения%"" with error – %КоличествоОтклонение% %Упаковка%, which is invalid and shows that there is an application error. Please contact the administrator.'");
		ИначеЕсли ДопустимыйПроцентОтклонения > 0 Тогда
			ШаблонОшибкаОкругления = НСтр("ru = 'После округления количество товара будет равно %КоличествоОкругленное% %БазоваяЕдиницаИзмерения%, при этом погрешность округления составит %КоличествоОтклонение% %Упаковка%, что превышает максимальную погрешность - %КоличествоМаксимальнаяПогрешность% %Упаковка%.';
											|en = 'After rounding the quantity of goods will be %КоличествоОкругленное% %БазоваяЕдиницаИзмерения%; but the rounding error will be %КоличествоОтклонение% %Упаковка%, it exceeds the maximum rounding error - %КоличествоМаксимальнаяПогрешность% %Упаковка%.'");
		Иначе
			ШаблонОшибкаОкругления = НСтр("ru = 'Количество в ""%Упаковка%"" некратно количеству в ""%БазоваяЕдиницаИзмерения%"". Отклонение - %КоличествоОтклонение% %Упаковка%.';
											|en = 'Quantity in ""%Упаковка%"" is not a multiple of quantity in ""%БазоваяЕдиницаИзмерения%"". Variance - %КоличествоОтклонение% %Упаковка%.'");
		КонецЕсли;
	...
	Иначе
		...
		Если Не ПараметрыПроверки.ПроверитьВозможностьОкругления Тогда
			ШаблонОшибкаОкругления = НСтр("ru = 'В строке %НомерСтроки% списка ""%ПредставлениеТЧ%"" количество в ""%Упаковка%"" переведено в количество в ""%БазоваяЕдиницаИзмерения%"" с отклонением - %КоличествоОтклонение% %Упаковка%, что свидетельствует об ошибке в работе программы. Обратитесь к администратору.';
											|en = 'Quantity in ""%Упаковка%"" is transferred to quantity in ""%БазоваяЕдиницаИзмерения%"" in line %НомерСтроки% of list ""%ПредставлениеТЧ%"" with variance – %КоличествоОтклонение% %Упаковка%, that shows the application error. Please contact the administrator.'");
		ИначеЕсли ДопустимыйПроцентОтклонения > 0 Тогда
			ШаблонОшибкаОкругления = НСтр("ru = 'После округления количество товара будет равно %КоличествоОкругленное% %БазоваяЕдиницаИзмерения% в строке %НомерСтроки% списка ""%ПредставлениеТЧ%"", при этом погрешность округления составит %КоличествоОтклонение% %Упаковка%, что превышает максимальную погрешность - %КоличествоМаксимальнаяПогрешность% %Упаковка%.';
											|en = 'After rounding, quantity of goods will be %КоличествоОкругленное% %БазоваяЕдиницаИзмерения% in line %НомерСтроки% of the ""%ПредставлениеТЧ%"" list; but the rounding error will be %КоличествоОтклонение% %Упаковка%, it exceeds the maximum rounding error - %КоличествоМаксимальнаяПогрешность% %Упаковка%.'");
		Иначе 
			ШаблонОшибкаОкругления = НСтр("ru = 'В строке %НомерСтроки% списка ""%ПредставлениеТЧ%"" количество в ""%Упаковка%"" некратно количеству в ""%БазоваяЕдиницаИзмерения%"". Отклонение - %КоличествоОтклонение% %Упаковка%.';
											|en = 'Quantity in ""%Упаковка%"" is not a multiple of quantity  in ""%БазоваяЕдиницаИзмерения%"" in line %НомерСтроки% of list ""%ПредставлениеТЧ%"". Discrepancy - %КоличествоОтклонение% %Упаковка%.'");
		КонецЕсли;
...
Процедура ВызватьИсключениеПоОшибкеШаблонаНаименования(ИмяОперанда, ИмяРеквизита = "")
	...
	ШаблонСообщенияОбОшибке =
		НСтр("ru = 'Невозможно рассчитать наименование по шаблону.
				   ...
				   |en = 'Cannot calculate a name according to the template.
				   |Error in the ""%ИмяОперанда%"" operand name.'");
	ТекстСообщенияОбОшибке  =
...
Функция ОтборПолитикУчетаСерийПоПараметрам(Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(ВидНоменклатуры)
		И (ИспользоватьНеотфактурованныеПоставки
			...
			//++ НЕ УТ
			Или ИспользоватьПроизводствоНаСтороне
			//-- НЕ УТ
...
Функция ОтборВладельцаПодчиненныхСправочников(ВидНоменклатуры, ИмяРеквизитаВладелец) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Получение отбора владельца подчиненных справочников для реквизита %1 не определено';
										|en = 'Receipt of the subordinate catalogs owner filter for attribute %1 is not defined'"), ИмяРеквизитаВладелец);
	КонецЕсли;
...
Процедура ЗаполнитьСерииПоFEFOВТЧТовары(Объект, ПараметрыУказанияСерий)
	...
	Если Не (
			...
			//++ НЕ УТКА
			Или ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ЭтапПроизводства2_2"
			Или ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Обработка.РедактированиеПоследовательностиЭтаповПроизводства"
			//-- НЕ УТКА
			)Тогда
		ТекстИсключения = НСтр("ru = 'Ошибка заполнения серий по FEFO. Процедура заполнения не поддерживает данный документ.';
								|en = 'An error occurred while populating FEFO batch. Population procedure does not support this document.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	//++ НЕ УТКА
	ЭтоОрдер = ПараметрыУказанияСерий.ЭтоОрдер;
	...
	Пока ВыборкаНомераСтрок.Следующий() Цикл
		...
		//++ НЕ УТКА
		Иначе
			ОстаткиСерийПоТовару = ОстаткиСерийПоТоваруЭтапа(СтрокаТоваров, СоответствиеОстатки, ВыборкаНомераСтрок.Назначение);
		КонецЕсли;
		//-- НЕ УТКА
	...
	//++ НЕ УТКА
	Иначе
		Запрос.Текст = ТекстЗапросаПроверкаЗаполненияСерийПоFEFOВЭтапе(Объект, ПараметрыУказанияСерий);
	КонецЕсли;
	//-- НЕ УТКА
	...
	Пока ВыборкаНезаполненные.Следующий() Цикл
		...
		Если ВыборкаНезаполненные.ЭтоТоварноеМесто Тогда
			...
			ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% для товара ""%ПредставлениеТовара%"" серии не заполнены, т.к. указана упаковка - товарное место. Заполнение серий FEFO по товарным местам не поддерживается.';
									|en = 'Batches are not filled in for the ""%ПредставлениеТовара%"" goods item in line %НомерСтроки%. Cargo packaging is specified as packaging unit. Cannot fill in FEFO batch according to the cargo packagings.'");
		...
		ИначеЕсли ВыборкаНезаполненные.Количество = 0 Тогда
			...
			ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% для товара ""%ПредставлениеТовара%"" серии не заполнены, т.к. не указано количество товаров.';
									|en = 'Batches are not filled in for the ""%ПредставлениеТовара%"" goods items in line %НомерСтроки%. Goods quantity is not specified.'");
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% для товара ""%ПредставлениеТовара%"" серии не заполнены. На складе ""%ПредставлениеСклада%"" не хватает ""%Количество%"" ""%ЕдиницаИзмерения%""';
									|en = 'Batches are not filled in for the ""%ПредставлениеТовара%"" goods item in line %НомерСтроки%. The ""%Количество%"" ""%ЕдиницаИзмерения%"" is missing in the ""%ПредставлениеСклада%"" warehouse'");
...
//
Функция ИспользуетсяСправочноеУказаниеСерийВМаршрутныхЛистах(Спецификация = Неопределено, ВидСтрокиСпецификации = Неопределено) Экспорт
...
Процедура ЗаполнитьСерииПоFEFOВМаршрутномЛисте(Объект, ПараметрыУказанияСерий)
	...
	Для каждого СтрокаТоваров Из Объект[ПараметрыУказанияСерий.ИмяТЧТовары] Цикл
		...
		Если СтрокаТоваров.КоличествоФакт = 0 Тогда
			...
			ТекстСообщения = НСтр("ru = 'Номенклатура ""%ПредставлениеТовара%"" - серии не заполнены, т.к. не указано количество.';
									|en = 'Products ""%ПредставлениеТовара%"" - series are not filled in as quantity is not specified.'");
			
		Иначе
			
			ТекстСообщения = НСтр("ru = 'Номенклатура ""%ПредставлениеТовара%"" - серии не заполнены.
				|В подразделении ""%ПредставлениеСклада%"" не хватает ""%Количество%"" ""%ЕдиницаИзмерения%"".';
				|en = 'Products ""%ПредставлениеТовара%"" - series are not filled in. 
				|Not enough ""%Количество%"" ""%ЕдиницаИзмерения%"" in business unit ""%ПредставлениеСклада%"".'");
...
Функция ДанныеДляЗаполненияСерийПоFEFOВЭтапе(Объект, ПараметрыУказанияСерий, Запрос)
	...
	|	ДанныеДокумента.Характеристика                                             КАК Характеристика,
	|	ВЫРАЗИТЬ(ДанныеДокумента.Упаковка КАК Справочник.УпаковкиЕдиницыИзмерения) КАК Упаковка,
	|	ДанныеДокумента.Обособленно                                                КАК Обособленно,
	|	ВЫРАЗИТЬ(ДанныеДокумента.Назначение КАК Справочник.Назначения)             КАК Назначение
	|ПОМЕСТИТЬ ТаблицаТоваров
	|ИЗ
	|	&ТаблицаТоваров КАК ДанныеДокумента
	|ГДЕ
	...
	|	ДанныеДокумента.Характеристика КАК Характеристика,
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.Обособленно
	|				И ЕСТЬNULL(ДанныеДокумента.Назначение.ДвиженияПоСкладскимРегистрам, ЛОЖЬ)
	...
	|					ТаблицаТоваровДляЗапроса.Назначение
	|				ИЗ
	|					ТаблицаТоваровДляЗапроса)) КАК СерииТоваров
	|ГДЕ
	|	СерииТоваров.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ЭтапПроизводства2_2" Тогда
		Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
	ИначеЕсли ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Обработка.РедактированиеПоследовательностиЭтаповПроизводства" Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Ссылка", "ДанныеДокумента.Ссылка");
	КонецЕсли;
...
Функция ОстаткиСерийПоТоваруЭтапа(СтрокаТоваров, СоответствиеОстатки, Назначение)
	
	Если СоответствиеОстатки[СтрокаТоваров.Склад] = Неопределено			
		Или СоответствиеОстатки[СтрокаТоваров.Склад][СтрокаТоваров.Номенклатура] = Неопределено
		Или СоответствиеОстатки[СтрокаТоваров.Склад][СтрокаТоваров.Номенклатура][СтрокаТоваров.Характеристика] = Неопределено
		Или СоответствиеОстатки[СтрокаТоваров.Склад][СтрокаТоваров.Номенклатура][СтрокаТоваров.Характеристика][Назначение] = Неопределено Тогда
		
		Остатки = Неопределено;
		
	Иначе
		
		Остатки = СоответствиеОстатки[СтрокаТоваров.Склад][СтрокаТоваров.Номенклатура][СтрокаТоваров.Характеристика][Назначение];
...
Функция ТекстЗапросаПроверкаЗаполненияСерийПоFEFOВЭтапе(Объект, ПараметрыУказанияСерий)
...
Процедура УпаковкиЕдиницыИзмеренияОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	Если Параметры.Свойство("ВыборРодителя") Тогда
		...
		УсловиеПоСтрокеПоиска=?(НеИспользуютсяДопЯзыки, "УпаковкиНоменклатуры.Наименование ПОДОБНО &СтрокаПоиска", "(УпаковкиНоменклатуры.Наименование ПОДОБНО &СтрокаПоиска 
		|		ИЛИ УпаковкиНоменклатуры.НаименованиеЯзык1 ПОДОБНО &СтрокаПоиска
		|		ИЛИ УпаковкиНоменклатуры.НаименованиеЯзык2 ПОДОБНО &СтрокаПоиска)");
...
Процедура ЗаполнитьДанныеВыбораУпаковками(Номенклатура, ДанныеВыбора, Параметры, Знач ПоказыватьМерныеЕдиницыИзмерения, ДобавлятьЕдиницуХранения, ПоказыватьТоварныеМеста)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
		"&УсловиеПоСтрокеПоиска",
		?(Параметры.СтрокаПоиска = Неопределено, "ИСТИНА", " УпаковкиЕдиницыИзмерения.Наименование ПОДОБНО &СтрокаПоиска"));
...
Процедура ХарактеристикиНоменклатурыОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	|			СпрХарактеристики.Наименование ПОДОБНО &СтрокаПоиска
	|		КОГДА &ИспользуютсяДопЯзыки = ИСТИНА И &СтрокаПоискаЗаполнена = ИСТИНА ТОГДА
	|			(СпрХарактеристики.Наименование ПОДОБНО &СтрокаПоиска
	|			ИЛИ СпрХарактеристики.НаименованиеЯзык1 ПОДОБНО &СтрокаПоиска
	|			ИЛИ СпрХарактеристики.НаименованиеЯзык2 ПОДОБНО &СтрокаПоиска)
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ
	|УПОРЯДОЧИТЬ ПО
