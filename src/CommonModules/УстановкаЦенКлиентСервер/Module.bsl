...
Функция НайтиСтрокуВалюты(СоответствиеВалют, Валюта) Экспорт
	...
	Если СтруктураВалюты = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрЗаменить(НСтр("ru = 'Не установлен курс валюты: %Валюта%';
																			|en = 'Exchange rate is not specified: %Валюта%'"), "%Валюта%", Валюта));
	КонецЕсли;
