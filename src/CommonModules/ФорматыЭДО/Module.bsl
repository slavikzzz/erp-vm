...
Функция ПоддерживаемыеФорматы() Экспорт
	...
	Форматы.ФНС.Вставить("УПД_5_02", Новый Структура);
	Форматы.ФНС.УПД_5_02.Вставить("ИнформацияПродавца", ФорматыФНС.УПД_5_02.ИнформацияПродавца);
	Форматы.ФНС.УПД_5_02.Вставить("ИнформацияПокупателя", ФорматыФНС.УПД_5_02.ИнформацияПокупателя);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		Форматы.Вставить("ЭПД", Новый Структура);
		Форматы.ЭПД.Вставить("ЭТрН", "ETRN");
		Форматы.ЭПД.Вставить("ЭЗН", "EZN");
		Форматы.ЭПД.Вставить("ЭСВ", "ESV");
		Форматы.ЭПД.Вставить("ЭЗЗ", "EZZ");
		Форматы.ЭПД.Вставить("ЭПЛ", "EPL");
		Форматы.ЭПД.Вставить("ЭДФ", "EDF");	
	КонецЕсли;
...
Процедура ОбновитьФорматыЭлектронныхДокументов(Форматы, ДатаИзменения) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ПоддерживаемыеФорматы.ИдентификаторФормата КАК СТРОКА(50)) КАК Формат
	|ПОМЕСТИТЬ ПоддерживаемыеФорматы
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ИСТИНА КАК ЕстьНеподдержанныеФорматы
	|ИЗ
	|	РегистрСведений.ФорматыЭлектронныхДокументов КАК ФорматыЭлектронныхДокументов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПоддерживаемыеФорматы КАК ПоддерживаемыеФорматы
	|		ПО ФорматыЭлектронныхДокументов.Формат = ПоддерживаемыеФорматы.Формат
	|ГДЕ
	|	НЕ ФорматыЭлектронныхДокументов.ПоддерживаетсяКонфигурацией
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	Запрос.УстановитьПараметр("ПоддерживаемыеФорматы", ТаблицаИзМакета);
	
	УстановитьПривилегированныйРежим(Истина);
	РезультатыЗапроса = Запрос.ВыполнитьПакет();
	УстановитьПривилегированныйРежим(Ложь);
	
	ЕстьНеподдержанныеФорматы = Не РезультатыЗапроса[1].Пустой();
	ЕстьЗаписи = Не РезультатыЗапроса[2].Пустой();
	
	Если Форматы.ДатаПоследнегоИзменения <> ДатаИзменения Или ЕстьНеподдержанныеФорматы Или Не ЕстьЗаписи Тогда
		...
		Исключение
			...
			ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Обновление форматов ЭДО из сервиса настроек';
															|en = 'Updating EDI formats from the settings service'"),
...
Функция ЭтоСводныйУПД(ДвоичныеДанные) Экспорт
	...
	Если ПараметрыДокумента = Неопределено
		ИЛИ ПараметрыДокумента.ИсходныйФормат <> ПоддерживаемыеФорматы().ФНС.УПД2019.ИнформацияПродавца
		ИЛИ ПараметрыДокумента.ИсходныйФормат <> ПоддерживаемыеФорматы().ФНС.УПД_5_02.ИнформацияПродавца Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ЭтоСлужебноеИмяДополнительногоПоля(ИмяПоля, Формат) Экспорт
	...
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УПД.ИнформацияПродавца
		...
		Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца
		...
		Если Обработка <> Неопределено Тогда
			...
			Исключение
				...
				Шаблон = НСтр(
					...
					|%2';
					|en = 'Data processor: %1. Cannot identify a classifier of tax documents. Reason:
					|%2'");
	...
	Для Каждого Обработка Из ОбработкиФорматов() Цикл
		...
		Исключение
			...
			Шаблон = НСтр(
				...
				|%2';
				|en = 'Data processor: %1. Cannot search for an ED format manager. Reason:
				|%2'");
			ТекстСообщения = СтрШаблон(Шаблон, ТипЗнч(Обработка), ОписаниеОшибки());
			ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
...
Процедура ПодключитьИспользованиеФорматаДоговорныхДокументовXML() Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось установить использование формата договорных документов xml.';
								|en = 'Cannot set the XML contractual document format.'");
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
			УровеньЖурналаРегистрации.Ошибка);
...
Функция ЗаполнитьДанныеДоверенностейДокумента(ОписаниеФайла, Организация, Доверенности, СодержаниеДокумента) Экспорт
	...
	Если ДанныеФайлаЭД.Свойства().Получить("ИдФайл") <> Неопределено Тогда
		...
		ДвоичныеДанные = РаботаСФайламиБЭД.XDTOВДвоичныеДанные(ДанныеФайлаЭД, Ложь, , ЛокальноеИмя);
...
Функция ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла,
	...
	Если (Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца
		...
		Если ПустаяСтрока(ОписаниеОшибки) Тогда
			...
			Если (ЕстьСвойствоXDTO(Подписант, "ИП") И Подписант.ИП <> Неопределено)
				Или (ЕстьСвойствоXDTO(Подписант, "ЮЛ") И Подписант.ЮЛ <> Неопределено)
				Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца Тогда
	...
	Если Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца
		...
		Иначе
			...
			ОснованиеПолномочий = НСтр("ru = 'Свидетельство ОГРН';
										|en = 'Registration number certificate'");
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.Подписант[ИндексПодписанта],
	...
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца
		Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПокупателя Тогда	
		...
		Если ЭлектронныеДокументыЭДО.ТребуетсяМашиночитаемаяДоверенность(Организация, СертификатПодписи) Тогда
			...
			Если ЗначениеЗаполнено(ВыбраннаяМЧД) Тогда
				...
				СвойствоСвДоверЭл = Подписант.Свойства().Получить("СвДоверЭл");
				СвДоверЭл = РаботаСФайламиБЭД.ПолучитьОбъектТипаCML(СвойствоСвДоверЭл.Тип, ПространствоИменСхемы);
				...
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СвДоверЭл, "ДатаВыдДовер", 
					Формат(РеквизитыМЧД.ДатаВыдачи, "ДФ=dd.MM.yyyy"), Истина, Ошибки);
					
				ИнформационнаяСистема = "-";	
				ДанныеФайлаМЧД = МашиночитаемыеДоверенности.ДанныеФайлаДоверенностиИПодписи(ВыбраннаяМЧД);
				ПараметрыФайлаМЧД = КонвертацияЭДО.ПараметрыФайлаПроизвольногоДокумента(ДанныеФайлаМЧД.ДанныеФайла);
				Если ЗначениеЗаполнено(ПараметрыФайлаМЧД) И ПараметрыФайлаМЧД.Свойство("ИнформационнаяСистема") Тогда
					ИнформационнаяСистема = ПараметрыФайлаМЧД.ИнформационнаяСистема;
				КонецЕсли;	
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СвДоверЭл, "ИдСистХран", ИнформационнаяСистема, Истина, Ошибки);
		...
		Если ЭтоФизЛицо Тогда
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(Подписант, "Должн", НСтр("ru = 'ИП';
																			|en = 'IE'"), Истина, Ошибки);
		Иначе
		...
		Если Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПокупателя 
			И РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "Документ.СодФХЖ4.СвПрин.СвЛицПрин") = Неопределено Тогда
			ПутьКТипуСвЛицПрин = СтрШаблон("{%1}.Файл.Документ.СодФХЖ4.СвПрин.СвЛицПрин", ПространствоИменСхемы);
		
			РабОргПокТип = ПолучитьТипОбъектаCML(ПутьКТипуСвЛицПрин + ".РабОргПок", ВерсияСхемы);
			РабОргПок = ПолучитьОбъектТипаCML(РабОргПокТип, ПространствоИменСхемы);
			Если ЭтоФизЛицо Тогда
				Должность = НСтр("ru = 'ИП';
								|en = 'IE'");
				ОснованиеПолномочий = НСтр("ru = 'Свидетельство ОГРН';
			...
			Иначе
				...
				ОснованиеПолномочий = НСтр("ru = 'Должностные обязанности';
											|en = 'Duties'");
			КонецЕсли;
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(РабОргПок, "Должность", Должность, Истина, Ошибки);
			...
			СвЛицПринТип = ПолучитьТипОбъектаCML(ПутьКТипуСвЛицПрин, ВерсияСхемы);
			СвЛицПрин = ПолучитьОбъектТипаCML(СвЛицПринТип, ПространствоИменСхемы);

			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СвЛицПрин, "РабОргПок", РабОргПок, Истина, Ошибки);
			
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.СодФХЖ4.СвПрин, "СвЛицПрин", СвЛицПрин,
				Истина, Ошибки);
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.АктОРасхождениях Тогда
		...
		Если Формат = ПоддерживаемыеФорматы.ФНС.АктОРасхождениях.ДополнительныеСведения Тогда
			...
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(Подписант, "ОснПолн", НСтр("ru = 'Должностные обязанности';
																				|en = 'Duties'"), Истина,
				Ошибки);
	...
	Иначе
		...
		Исключение
			...
			Шаблон = НСтр(
				...
				|%2';
				|en = 'Data processor: %1. Cannot fill the signer data. Reason:
				|%2'");
			ТекстСообщения = СтрШаблон(Шаблон, ТипЗнч(Обработка), ОписаниеОшибки());
			ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
				УровеньЖурналаРегистрации.Ошибка);
...
Функция ЗаполнениеДанныхПодписантаДоступно(СодержаниеДокумента) Экспорт
	...
	Если ТипЗнч(Обработка) = Тип("Неопределено") Тогда
		Возврат Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца
			...
			Или Формат = ПоддерживаемыеФорматы.ФНС.УПД2019.ИнформацияПокупателя
			Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца
			Или Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПокупателя
			Или Формат = ПоддерживаемыеФорматы.ФНС.ПередачаТоваров.ИнформацияПродавца
	...
	Иначе
		...
		Исключение
			...
			Шаблон = НСтр(
				...
				|%2';
				|en = 'Data processor: %1. Cannot identify whether the signer data can be filled. Reason:
				|%2'");
			ТекстСообщения = СтрШаблон(Шаблон, ТипЗнч(Обработка), ОписаниеОшибки());
			ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
				УровеньЖурналаРегистрации.Ошибка);
...
Функция СведенияОПодписанте(ОписаниеФайла, ОписаниеОшибки = "") Экспорт
	...
	Если ЕстьСвойствоXDTO(ДанныеФайлаЭД, "СвУчДокОбор") Тогда
	    Результат.ИдентификаторОрганизации = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
			ДанныеФайлаЭД, "СвУчДокОбор.ИдОтпр", "");
	ИначеЕсли ЕстьСвойствоXDTO(ДанныеФайлаЭД, "ИдФайл")
		И СтрНайти(ДанныеФайлаЭД.ИдФайл, "ON_NSCHFDOPPR") > 0 Тогда
			
			ВерсияФорматаУПД = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "ВерсФорм", "");
			Если ВерсияФорматаУПД = "5.02" Тогда
				ЭлементыИдентификатора = СтрРазделить(ДанныеФайлаЭД.ИдФайл, "_");
				Результат.ИдентификаторОрганизации = ЭлементыИдентификатора[3];
			КонецЕсли;
	...
	Если ЕстьСвойствоXDTO(Подписант, "ИП") И Подписант.ИП <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(Результат, Подписант.ИП.ФИО);
		Результат.Вид = "ИП";
	ИначеЕсли ЕстьСвойствоXDTO(Подписант, "ЮЛ") И Подписант.ЮЛ <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(Результат, Подписант.ЮЛ.ФИО);
		Результат.Вид = "ЮЛ";
	ИначеЕсли ЕстьСвойствоXDTO(Подписант, "ФЛ") И Подписант.ФЛ <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(Результат, Подписант.ФЛ.ФИО);
		Результат.Вид = "ФЛ";
	ИначеЕсли ЕстьСвойствоXDTO(Подписант, "ФИО") И Подписант.ФИО <> Неопределено Тогда	
		ЗаполнитьЗначенияСвойств(Результат, Подписант.ФИО);
		Результат.Вид = "";
	Иначе 
		Результат.Вид = "";
	КонецЕсли;
...
Функция ПодписантЗаполнен(Подписант, Организация, ОписаниеОшибки = "") Экспорт
	Если ПустаяСтрока(Подписант.Имя) Или ПустаяСтрока(Подписант.Фамилия) Тогда
		Возврат Ложь;
	КонецЕсли;
	Если Подписант.Имя = "-" И Подписант.Фамилия = "-" Тогда
		Возврат Ложь;
	КонецЕсли;
		...
		Если Подписант.Вид = "ЮЛ" Тогда
			ОписаниеОшибки = СтрШаблон(
				НСтр("ru = 'Подписант %1 %2 %3 должен быть заполнен в разделе <ИП>. Данные подписанта будут заполнены при подписании.';
					|en = 'Signer %1 %2 %3 must be filled in the <IE> section. Signer data will be filled upon signing.'"),
		...
		Если Подписант.Вид = "ИП" Тогда
			ОписаниеОшибки = СтрШаблон(
				НСтр("ru = 'Подписант %1 %2 %3 должен быть заполнен в разделе <ЮЛ>. Данные подписанта будут заполнены при подписании.';
					|en = 'Signer %1 %2 %3 must be filled in the <Legal entity> section. Signer data will be filled upon signing.'"),
				Подписант.Фамилия, Подписант.Имя, Подписант.Отчество);
		КонецЕсли;
	КонецЕсли;
...
Функция ВУчетнойЗаписиЕстьСертификатПодписанта(Подписант, ОписаниеОшибки = "") Экспорт
	...
	ОписаниеОшибки = СтрШаблон(НСтр(
			...
			|en = 'No certificate %1 %2 %3 for the signer. Signer section will be filled after signing.'"),
			Подписант.Фамилия, Подписант.Имя, Подписант.Отчество);
	Возврат Ложь;
КонецФункции
...
Процедура СброситьСведенияОПодписанте(ОписаниеФайла, ОписаниеОшибки = "") Экспорт
	...
	КонецЕсли;
	Если ЕстьСвойствоXDTO(Подписант, "ЮЛ") И Подписант.ЮЛ <> Неопределено Тогда
	...
	Если ПространствоИмен = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца Тогда
		ВерсияСхемы = "4.01";
		ЛокальноеИмя = "Файл";
		...
		СвойствоФИО = Подписант.Свойства().Получить("ФИО");
		ФИО = РаботаСФайламиБЭД.ПолучитьОбъектТипаCML(СвойствоФИО.Тип, ПространствоИмен);
		
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ФИО, "Фамилия", "-");
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ФИО, "Имя", "-");
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(Подписант, "ФИО", ФИО, Истина);
	Иначе
...
Функция ЗапросКонструктораДополнительныхПолей(Знач ТипДокумента, Знач Формат, Знач ТипРаздела) Экспорт
	...
	Если ТипРаздела = ТипРазделов.Шапка Тогда
		...
		Если Не ЗначениеЗаполнено(ТекстЗапроса) Тогда
			ТекстОшибки = НСтр("ru = 'Необходимо проверить процедуру ОбменСКонтрагентамиПереопределяемый.ПолучитьЗапросКонструктораДополнительныхПолейШапки.';
								|en = 'Check the ExchangeWithCounterpartiesOverridable procedure.ReceiveAdditionalHeaderFieldsDesignerRequest.'");
		КонецЕсли;
	...
	ИначеЕсли ТипРаздела = ТипРазделов.Таблица Тогда
		...
		Если Не ЗначениеЗаполнено(ТекстЗапроса) Тогда
			ТекстОшибки = НСтр("ru = 'Необходимо проверить определяемые типы ""НоменклатураБЭД"", ""ХарактеристикаНоменклатурыБЭД"", ""УпаковкаНоменклатурыБЭД"".';
								|en = 'It is necessary to check the types defined ""НоменклатураБЭД"", ""ХарактеристикаНоменклатурыБЭД"", ""УпаковкаНоменклатурыБЭД"".'");
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ТекстЗапроса) Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Не определен запрос конструктора дополнительных полей для вида электронного документа ""%1"".';
										|en = 'The additional fields designer request is not defined for the ""%1"" electronic document kind.'"),
			ТипДокумента);
...
Функция ЗагружаемыйФайлСоответсвуетЗагруженномуФайлу(Организация, ЗагружаемыйФайл, ЗагруженныйФайл, ИмяФайла) Экспорт
	...
	Исключение
		ОписаниеОшибки = СтрШаблон(
			...
			|en = 'The document is not imported as the %1 file contains errors (see the Event log).
	...
	ПространствоИменЗагружаемыхДанных = ЗагружаемыеДанные.Тип().URIПространстваИмен;
	ПространствоИменЗагруженныхДанных = ЗагруженныеДанные.Тип().URIПространстваИмен;
	
	Если ПространствоИменЗагружаемыхДанных <> ПространствоИменЗагруженныхДанных Тогда
		Если ФорматыНеСовместимы(ПространствоИменЗагружаемыхДанных, ПространствоИменЗагруженныхДанных) Тогда
					ОписаниеОшибки = СтрШаблон(
			...
			|Выберите другой файл и повторите загрузку.';
			|en = 'The document is not imported as the %1 file format violates the current accounting document regulation.
			|Importing can lead to the service operator errors.
			|Select another file and try again.'"), 
			ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ЗагружаемыйФайл.Имя).Имя);
			Ошибки.Вставить(ОписаниеОшибки, Ложь);
			Возврат Ошибки;
		Иначе
			...
			ДанныеСравнения.Вставить("ВИсходномДокументе", ПространствоИменЗагруженныхДанных);
			ДанныеСравнения.Вставить("ВЗагружаемомДокументе", ПространствоИменЗагружаемыхДанных);
			Ошибки.Вставить(НСтр("ru = 'Формат файла';
								|en = 'File format'"), ДанныеСравнения);
		КонецЕсли;
	КонецЕсли;
	
	Если ПространствоИменЗагружаемыхДанных = ФорматыЭДО_ФНС.ПространствоИмен_УПД_5_02_ИнформацияПродавца() Тогда
		
		ЧастиИмениФайлаЗагружаемыеДанные = СтрРазделить(ЗагружаемыеДанные.ИдФайл, "_");
		Если ЧастиИмениФайлаЗагружаемыеДанные.Количество() >= 3 Тогда
			ИдОтправителяВЗагружаемомДокументе = ЧастиИмениФайлаЗагружаемыеДанные[3];
			ИдПолучателяВЗагружаемомДокументе  = ЧастиИмениФайлаЗагружаемыеДанные[2];
		Иначе
			ИдОтправителяВЗагружаемомДокументе = "";
			ИдПолучателяВЗагружаемомДокументе  = "";
		КонецЕсли;
		
		ЧастиИмениФайлаВИсходномДокументе = СтрРазделить(ЗагруженныеДанные.ИдФайл, "_");
		Если ЧастиИмениФайлаВИсходномДокументе.Количество() >= 3 Тогда
			ИдОтправителяВИсходномДокументе = ЧастиИмениФайлаВИсходномДокументе[3];
			ИдПолучателяВИсходномДокументе  = ЧастиИмениФайлаВИсходномДокументе[2];
		Иначе
			ИдОтправителяВИсходномДокументе = "";
			ИдПолучателяВИсходномДокументе  = "";
		КонецЕсли;
		
		Если ИдОтправителяВЗагружаемомДокументе <> ИдОтправителяВИсходномДокументе Тогда
			ДанныеСравнения = Новый Структура;
			ДанныеСравнения.Вставить("ВИсходномДокументе", ИдОтправителяВИсходномДокументе);
			ДанныеСравнения.Вставить("ВЗагружаемомДокументе", ИдОтправителяВЗагружаемомДокументе);
			Ошибки.Вставить("ИдФайл[ИдентификаторОтправителя]", ДанныеСравнения);
		КонецЕсли;
		
		Если ИдПолучателяВИсходномДокументе <> ИдПолучателяВЗагружаемомДокументе Тогда
			ДанныеСравнения = Новый Структура;
			ДанныеСравнения.Вставить("ВИсходномДокументе", ИдПолучателяВИсходномДокументе);
			ДанныеСравнения.Вставить("ВЗагружаемомДокументе", ИдПолучателяВЗагружаемомДокументе);
			Ошибки.Вставить("ИдФайл[ИдентификаторПолучателя]", ДанныеСравнения);
		КонецЕсли;
	...
	Иначе
		...
		Если Не ЕстьСвойствоXDTO(ЗагружаемыеДанные, "ИдФайл") Или Не ЕстьСвойствоXDTO(ЗагружаемыеДанные, "СвУчДокОбор") Тогда
			...
			ОписаниеОшибки = СтрШаблон(
				...
				|Выберите другой файл и повторите загрузку.';
				|en = 'The document is not imported as the %1 file is not a formalized EDI document.
				|Select another file and try again.'"), 
				ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ЗагружаемыйФайл.Имя).Имя);
			Ошибки.Вставить(ОписаниеОшибки, Ложь);
			Возврат Ошибки;
	...
	ИначеЕсли ЗагружаемыйДокумент = Неопределено Тогда 
		...
		ДанныеСравнения.Вставить("ВИсходномДокументе", 
			СтрШаблон(НСтр("ru = 'данные формата &1';
							|en = 'format data &1'"), ЗагруженныеДанные.Тип().URIПространстваИмен));
...
Функция ДанныеФайлаЭД(ДанныеФайла, ОписаниеОшибки, ПространствоИменФНС = Неопределено, ИмяКорневогоУзла = "", МягкоеЧтение = Ложь) Экспорт
	...
	ИначеЕсли НРег(Прав(ДанныеФайла.ИмяФайла, 4)) = ".pdf" Тогда
		...
		Если ДвоичныеДанныеФайла = Неопределено Тогда
			...
			|en = 'An error occurred while recognizing the machine-readable file part:
			|%1. The file may mismatch the supported format.'");
	...
	Если ЗначениеЗаполнено(ИдФайл) И Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Если Не МягкоеЧтение Тогда
			...
			ИначеЕсли СтрНайти(ИдФайл, "ON_NSCHFDOPPR") Тогда
				Если ВерсФорм = "5.02" Тогда
					URIПространстваИмен = "ON_NSCHFDOPPR_1_997_01_05_02_01";
				Иначе	
			...
			ИначеЕсли СтрНайти(ИдФайл, "ON_NSCHFDOPPOK") Тогда
				Если ВерсФорм = "5.02" Тогда
					URIПространстваИмен = "ON_NSCHFDOPPOK_1_997_02_05_02_01";
				Иначе
				...
				Если Обработка <> Неопределено Тогда
					...
					Исключение
						...
						Шаблон = НСтр(
							...
							|%2';
							|en = 'Data processor: %1. Cannot identify a namespace. Reason:
							|%2'");
						ТекстСообщения = СтрШаблон(Шаблон, ТипЗнч(Обработка), ОписаниеОшибки());
						ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
							УровеньЖурналаРегистрации.Ошибка);
			...
			Если Обработка <> Неопределено Тогда
				...
				Исключение
					...
					Шаблон = НСтр(
						...
						|%2';
						|en = 'Manager %1. Cannot identify a namespace due to:
						|%2'");
					ТекстСообщения = СтрШаблон(Шаблон, ТипЗнч(Обработка), ОписаниеОшибки());
					ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
						УровеньЖурналаРегистрации.Ошибка);
	...
	Если Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		...
		Попытка
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(ПространствоИменФайла) Тогда
...
// Возвращаемое значение:
//  - Неопределено
//  - См. НовоеОписаниеФайлаДокумента
//
...
Функция ИдентификаторыДатаНомерДокументаПоДаннымФайла(ОписаниеФайла, ОписаниеОшибки = "") Экспорт
	...
	Исключение
		Если Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
			ШаблонСообщения = НСтр("ru = 'Ошибка: %1 
									...
									|%2';
									|en = 'An error (%1) 
									|occurred while reading data from the file:
									|%2'");
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
	...
	Если РаспознаниеТекущего.Свойство("ИсходныйФормат") Тогда
		ПоддерживаемыеФорматы = ПоддерживаемыеФорматы();
		
		Если РаспознаниеТекущего.ИсходныйФормат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца Тогда
			Если РаспознаниеТекущего.Свойство("ИдентификаторДокумента") Тогда
				ЧастиИмениФайла = СтрРазделить(РаспознаниеТекущего.ИдентификаторДокумента, "_");
			Иначе
				ЧастиИмениФайла = СтрРазделить(ОписаниеФайла.ИмяФайла, "_");
			КонецЕсли;

			Если ЧастиИмениФайла.Количество() >= 3 Тогда
				НастройкаОтправки.Вставить("ИдентификаторОрганизации", ЧастиИмениФайла[3]);
				НастройкаОтправки.Вставить("ИдентификаторКонтрагента", ЧастиИмениФайла[2]);
			Иначе
				Возврат Неопределено;
			КонецЕсли;
		КонецЕсли;
...
Функция НовоеОписаниеФайлаДокумента() Экспорт
	...
	Содержание.Вставить("ВалютаДокумента", "");
	Содержание.Вставить("СтавкаНДСДокумента", "");
	Содержание.Вставить("СуммаНДСДокумента", 0);
	...
	Если СтруктураРазбора = Неопределено
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Неизвестный тип электронного документа при чтении данных из файла %1.';
				|en = 'Unknown type of electronic document when reading data from the %1 file.'"),
...
Функция ИнформацияОТовареИзФайла(Параметры, ИменаДопКолонок = "") Экспорт
		...
		Если ТаблицаВозврата = Неопределено Тогда
			Если Пользователи.ЭтоПолноправныйПользователь(, , Ложь) Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Неизвестный тип при чтении данных из ЭД %1.';
						|en = 'Unknown type when reading data from ED %1.'"), Параметры.ОсновнойФайл.ИмяФайла);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
		КонецЕсли;
	...
	Исключение
		ВидОперации = НСтр("ru = 'Чтение информации о товаре электронного документа';
							|en = 'Read the information on electronic document goods'");
		...
		ТекстОшибкиКратко = СтрШаблон(НСтр(
			...
			|en = 'Cannot read information on electronic document goods: %1'"),
...
Функция РазделыДополнительныхПолейФорматаЭлектронногоДокумента(ТипЭлектронногоДокумента, Формат) Экспорт
	...
	Исключение
		...
		Шаблон = НСтр(
			...
			|%2';
			|en = 'Data processor: %1. Cannot identify additional field sections. Reason:
			|%2'");
		ТекстСообщения = СтрШаблон(Шаблон, ТипЗнч(Обработка), ОписаниеОшибки());
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
			УровеньЖурналаРегистрации.Ошибка);
...
Функция РасшифровкаВариантовЗаполненияФорматовЭДО() Экспорт
	...
	НоваяСтрока = ВариантыЗаполнения.Добавить();
	НоваяСтрока.Формат = "ON_NSCHFDOPPR_1_997_01_05_02_01";
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.УПД;
	НоваяСтрока.ВариантЗаполнения = "СЧФДОП";
...
#Область ПравилаПреобразованияФорматов_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: заполнение правил преобразования форматов.
								  ...
								  |en = '1C:Exchange with counterparties: fill in format conversion rules. 
								  |Setup and exchange of electronic documents with counterparties are temporarily unavailable.'");
	...
	Обработчик.Комментарий = НСтр(
		...
		|en = '1C:Exchange with counterparties: update format conversion rules (initial filling).'");
	...
	Изменяемые.Добавить(Метаданные.РегистрыСведений.ФорматыДляПреобразования.ПолноеИмя());
	Изменяемые.Добавить(Метаданные.РегистрыСведений.ПравилаПреобразованияФорматов.ПолноеИмя());
	Изменяемые.Добавить(Метаданные.РегистрыСведений.ПереченьСпециальныхОбстоятельствДокументаЭДО.ПолноеИмя());
	Изменяемые.Добавить(Метаданные.Константы.ДатаЗагрузкиОписанияФорматовДляПреобразования.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
#КонецОбласти

#Область ПереченьСпециальныхОбстоятельствДокументаЭДО_ОбработатьДанныеДляПереходаНаНовуюВерсию
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "КонвертацияЭДО.ЗаполнитьПереченьСпециальныхОбстоятельствДокументаЭДО";
	Обработчик.Версия = "1.9.12.43";
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("c9bf16c0-bbdb-4f91-9b44-600230c1897e");
	Обработчик.ОбщиеДанные = Истина;
	Обработчик.ЗапускатьИВПодчиненномУзлеРИБСФильтрами = Истина;
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: обновление 
		|переченя кодов (значений) для формирования информации о специальных обстоятельствах документа.';
		|en = '1С:Обмен с контрагентами: обновление 
		|переченя кодов (значений) для формирования информации о специальных обстоятельствах документа.'");
	...
	Если НастройкаЗаполнения = Неопределено Тогда
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Не удалось получить настройки дополнительных полей на основании %1 по причине:';
												|en = 'Cannot get additional fields settings based on %1 due to:'"),
				ОбъектУчета));
			...
			МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Проверьте корректность настроек заполнения дополнительных полей
				...
				|en = 'Check whether settings of 
				| additional fields filling are correct in the EDI setting for the ""%1"" electronic document kind and for the ""%2"" format'"),
...
Функция ВычислитьЗначенияДополнительныхПолейШапки(ПараметрыЗаполнения, ОшибкиВычисления)
	...
	Для Каждого НастройкаПоФормуле Из МассивНастроекПоФормуле Цикл
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось вычислить значение поля ""%1"".';
											|en = 'Cannot calculate the ""%1"" field value.'"), ПредставлениеПоля);
		...
		Для Каждого НастройкаПоФормуле Из МассивНастроекПоФормуле Цикл
			...
			Исключение
				...
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось вычислить значение поля ""%1"" в строке %2 списка ""%3"".';
												|en = 'Cannot calculate the ""%1"" field value in the %2 string of the ""%3"" list.'"),
...
Процедура ОбработатьОшибкуВыполненияЗапросаКонструктораДополнительныхПолей(ИнформацияОбОшибке, ПараметрыЗаполнения)
	...
	Операция = НСтр("ru = 'Заполнение дополнительных полей.';
					|en = 'Additional fields filling in.'");
	...
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось получить значения операндов формулы для вычисления значений дополнительных полей раздела ""%1"".';
									|en = 'Cannot receive operands values of the formula for calculating the values of the ""%1"" additional fields section.'"),
		ПараметрыЗаполнения.Раздел.Имя);
	
	МассивСтрок = Новый Массив;
	МассивСтрок.Добавить(НСтр("ru = 'Не удалось выполнить запрос конструктора дополнительных полей.';
								|en = 'Cannot execute the additional field designer request.'"));
	...
	МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Настройка ЭДО: %1';
										|en = 'EDI setup: %1'"), СсылкаНаНастройкуЭДО));
		
	МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Вид электронного документа: %1';
										|en = 'Electronic document kind: %1'"),
	...
	МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Раздел дополнительных полей: %1';
										|en = 'Section of additional fields: %1'"), ПараметрыЗаполнения.Раздел.Имя));
...
Функция РезультатФормированияСОшибкойПолученияДанныхПоУчету()
	...
	ОбщегоНазначенияБЭД.ДобавитьОшибку(Результат.Ошибки.ЗаполнениеДанных, НСтр(
		...
		|en = 'An error occurred while getting data for electronic document generation'"));
	...
	Если ЭтоОбработкаФормата(Данные) Тогда
		...
		Исключение
			...
			Шаблон = НСтр(
				...
				|%2';
				|en = 'Manager %1. Cannot fill the document data due to:
				|%2'");
...
Функция ДанныеЭлектронногоДокументаОсновныхТипов(ПараметрыПолученияДанных)
	...
	Если ДанныеФайлаЭД.Свойства().Получить("ИдФайл") <> Неопределено Тогда //чтение файлов без namespace по форматам ФНС
		...
		ИначеЕсли СтрНайти(ДанныеФайлаЭД.ИдФайл, "ON_NSCHFDOPPR") > 0 Тогда
			ВерсияФорматаУПД = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "ВерсФорм", "");
			Если ВерсияФорматаУПД = "5.02" Тогда
				ФорматыЭДО_ФНС.ПрочитатьИнформациюПродавцаXDTO_УПД_5_02(ДанныеФайлаЭД, ДеревоРазбора, НовыйЭД, ОшибкаРазбора);
				НовыйЭД.ВерсияФормата = "ON_NSCHFDOPPR_1_997_01_05_02_01";
			ИначеЕсли ВерсияФорматаУПД = "5.01" Тогда
		...
		ИначеЕсли СтрНайти(ДанныеФайлаЭД.ИдФайл, "ON_NSCHFDOPPOK") > 0 Тогда
			ВерсияФорматаУПД = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "ВерсФорм", "");
			Если ВерсияФорматаУПД = "5.02" Тогда
				ФорматыЭДО_ФНС.ПрочитатьИнформациюПокупателяXDTO_УПД_5_02(ДанныеФайлаЭД, ДеревоРазбора, НовыйЭД, ОшибкаРазбора);
				НовыйЭД.ВерсияФормата = "ON_NSCHFDOPPR_1_997_01_05_02_01";
			ИначеЕсли ВерсияФорматаУПД = "5.01" Тогда
			...
			Если Обработка <> Неопределено Тогда
				...
				Исключение
					...
					Шаблон = НСтр(
						...
						|%2';
						|en = 'Data processor: %1. Cannot read the exchange file. Reason:
						|%2'");
					ТекстСообщения = СтрШаблон(Шаблон, ТипЗнч(Обработка), ОписаниеОшибки());
					ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ПодсистемаРегистрацииОшибок,
						УровеньЖурналаРегистрации.Ошибка);
	...
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ДанныеФайлаЭД, "Документ") <> Неопределено Тогда //CML_2_08
		...
		Если ВРег(РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЭДокумент, "ХозОперация")) = ВРег(НСтр("ru = 'Отчет о продажах комиссионного товара';
																								|en = 'Account sales'")) Тогда
			
			ФорматыЭДО_CML.ПрочитатьОтчетОПродажахКомиссионногоТовараXDTO(ЭДокумент, ДанныеРазбора, НовыйЭД, ОшибкаРазбора); 
			
		ИначеЕсли ВРег(РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЭДокумент, "ХозОперация")) = ВРег(НСтр("ru = 'Отчет о списании комиссионного товара';
																									|en = 'Report on consigned goods write-off'")) Тогда
			ФорматыЭДО_CML.ПрочитатьОтчетОСписанииКомиссионногоТовараXDTO(ЭДокумент, ДанныеРазбора, НовыйЭД, ОшибкаРазбора); 
	
		ИначеЕсли ВРег(РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЭДокумент, "ХозОперация")) = ВРег(НСтр("ru = 'Счет на оплату';
																									|en = 'Commercial invoice'")) Тогда
			ФорматыЭДО_CML.ПрочитатьСчетНаОплатуXDTO(ЭДокумент, ДанныеРазбора, НовыйЭД, ОшибкаРазбора);
			
		ИначеЕсли ВРег(РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЭДокумент, "ХозОперация")) = ВРег(НСтр("ru = 'Заказ товара';
																									|en = 'Goods order'")) Тогда
			ФорматыЭДО_CML.ПрочитатьЗаказXDTO(ЭДокумент, ДанныеРазбора, НовыйЭД, ОшибкаРазбора);
			
		ИначеЕсли ВРег(РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(ЭДокумент, "ХозОперация")) = ВРег(НСтр("ru = 'Передача прав';
																									|en = 'Transfer of rights'")) Тогда
...
Функция ТипКорневогоУзла(URIПространстваИмен)
	...
	Если URIПространстваИмен = "ON_AKTSVEROTP_1_972_01_05_01_01" Или URIПространстваИмен = "ON_AKTSVERPOL_1_972_02_05_01_01"
		...
		Или URIПространстваИмен = "ON_NSCHFDOPPR_1_997_01_05_02_01" Или URIПространстваИмен = "ON_NSCHFDOPPOK_1_997_02_05_02_01"
	...
	Если СтрНайти(СтрокаXML, "<Данные>") > 0 Тогда
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Возникла ошибка при чтении данных из строки доп.данных.
										...
										|en = 'An error occurred while reading data from the string of addit.data.
...
Функция СоответствиеТитулов()
	...
	СоответствиеТитулов.Вставить(Форматы.ФНС.УПД_5_02.ИнформацияПродавца, Форматы.ФНС.УПД_5_02.ИнформацияПокупателя);
	...
	Для Каждого Менеджер Из ОбработкиФорматов() Цикл
		...
		Исключение
			...
			Шаблон = НСтр(
				...
				|%2';
				|en = 'Manager %1. Cannot identify whether the titles match due to:
				|%2'");
		...
		Если ЭтоОбработкаФормата(ОписаниеДанныхЭД) Тогда
			...
			Исключение
				...
				Шаблон = НСтр(
					...
					|%2';
					|en = 'Manager %1. Cannot fill the additional field data due to:
					|%2'");
...
// Преобразует XML формата ДопФайлУниверсальный в ДеревоЗначений. 
...
Функция РазобратьУниверсальныйДопФайл(ДанныеФайла)
	...
	ДопФайлУниверсальный = ПолучитьТипЗначенияCML("ДопФайл", "ДопФайлУниверсальный");
	Попытка
		ДанныеДопФайла = ФабрикаXDTO.ПрочитатьXML(ОбъектXML, ДопФайлУниверсальный);
	Исключение
		ОбъектXML.Закрыть();
		Возврат ДеревоДопДанных;
	КонецПопытки;
	...
	Попытка
		Если ДанныеДопФайла.Данные <> Неопределено Тогда
			РекурсивноРазобратьДопФайл(ДанныеДопФайла.Данные, ДеревоДопДанных);
		КонецЕсли;
	Исключение
