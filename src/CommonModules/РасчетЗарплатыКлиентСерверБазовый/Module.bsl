...
Функция ПредставлениеСпособаРасчетаАванса(СпособРасчетаАванса, СжатыйФормат) Экспорт 
	...
	Если СпособРасчетаАванса = ПредопределенноеЗначение("Перечисление.СпособыРасчетаАванса.ПроцентомОтТарифа") Тогда
		СтрокаПодстановки = ?(СжатыйФормат, НСтр("ru = '%2%3';
												|en = '%2%3'"), НСтр("ru = 'Ранее получал аванс %2%3 от тарифа';
																		|en = 'Previously received advance %2%3 of tariff '"));
	Иначе
		СтрокаПодстановки = ?(СжатыйФормат, НСтр("ru = '%2 %3';
												|en = '%2 %3'"), НСтр("ru = 'Ранее получал аванс %2 %3';
																		|en = 'Previously received advance %2 %3'"));
	КонецЕсли;
