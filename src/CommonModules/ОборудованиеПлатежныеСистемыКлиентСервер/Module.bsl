...
Функция ТекстСлипЧекаПоУмолчанию(ПараметрыСлипЧека, КороткийСлипЧек = Истина) Экспорт
	...
	Если ПараметрыСлипЧека.Результат Тогда
		Текст = НСтр("ru = 'ПЛАТЕЖНАЯ ОПЕРАЦИЯ';
					|en = 'PAYMENT TRANSACTION'") + Символы.ПС;
		Текст = Текст + СтрШаблон(НСтр("ru = 'Код авторизации: %1';
										|en = 'Authorization code: %1'"), ПараметрыСлипЧека.КодАвторизации) + Символы.ПС;
		Текст = Текст + СтрШаблон(НСтр("ru = 'Номер карты: %1';
										|en = 'Card number: %1'"),     ПараметрыСлипЧека.НомерКарты)     + Символы.ПС;
		Текст = Текст + СтрШаблон(НСтр("ru = 'Номер чека ЭТ: %1';
										|en = 'POS terminal receipt number: %1'"),   ПараметрыСлипЧека.НомерЧекаЭТ)    + Символы.ПС;
		Текст = Текст + СтрШаблон(НСтр("ru = 'Ссылочный номер: %1';
										|en = 'Reference number: %1'"), ПараметрыСлипЧека.СсылочныйНомер) + Символы.ПС;
		Если ПараметрыСлипЧека.Свойство("СуммаОперации") Тогда
			Текст = Текст + СтрШаблон(НСтр("ru = 'Сумма: %1';
											|en = 'Amount: %1'"), ПараметрыСлипЧека.СуммаОперации) + Символы.ПС;
		КонецЕсли;
		Если Не КороткийСлипЧек И ПараметрыСлипЧека.Свойство("ТекстСлипЧека") Тогда
			Текст = Текст + ПараметрыСлипЧека.ТекстСлипЧека;
		КонецЕсли;
	Иначе
		Текст = НСтр("ru = 'ОПЕРАЦИЯ ОТКЛОНЕНА';
					|en = 'TRANSACTION IS REJECTED'") + Символы.ПС;  
