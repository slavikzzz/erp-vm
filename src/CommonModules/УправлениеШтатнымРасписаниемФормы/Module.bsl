...
Процедура ПроверкаШтатногоРасписания(Форма, ДатаВступленияВСилу) Экспорт
	...
	Иначе
		...
		КомментарийШтатногоРасписанияТекст = НСтр("ru = 'Штатное расписание проверено. Ошибок нет';
													|en = 'Headcount is checked. No errors'");
	КонецЕсли;
		...
		Если НЕ РезультатПроверки.ИзмененияВозможны Тогда
			...
			ЗаголовокДекорации = Новый ФорматированнаяСтрока(
				НСтр("ru = 'Документ недоступен для редактирования, т.к. зарегистрированы более поздние изменения штатного расписания.
					|См';
					|en = 'The document cannot be edited as later changes to the headcount were made.
					|See'") + ". ",
...
Процедура ПрочитатьДополнительныеДанныеДолжностиДляПозицииШтатногоРасписания(Форма) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
		...
		МодульОхранаТрудаФормы.ПрочитатьДополнительныеЗначенияПозицииШтатногоРасписания(Форма);
	КонецЕсли;
...
Процедура УстановитьУсловноеОформлениеСпискаДолжностей(Форма, ИмяСписка = "Список", ИмяРеквизитаВведена="ВведенаВШтатноеРасписание", ИмяРеквизитаИсключена = "ИсключенаИзШтатногоРасписания") Экспорт
	...
	ЭлементОформления.Представление = НСтр("ru = 'Зачеркивать исключенные должности.';
											|en = 'Cross out excluded positions.'");
	...
	ЭлементОформления.Представление = НСтр("ru = 'Выделять не введенные должности.';
											|en = 'Point out positions not introduced.'");
...
Процедура УстановитьУсловноеОформлениеСпискаПодразделений(Форма, ИмяСписка = "Список", ИмяРеквизитаСформировано = "Сформировано", ИмяРеквизитаРасформировано = "Расформировано") Экспорт
	...
	ЭлементОформления.Представление = НСтр("ru = 'Зачеркивать расформированные подразделения.';
											|en = 'Cross out dissolved business units.'");
	...
	ЭлементОформления.Представление = НСтр("ru = 'Выделять жирным не сформированные подразделения.';
											|en = 'Type bold unformed business units.'");
...
Процедура ПрочитатьДанныеПозицииВФорму(Форма, Объект, АдресДанныхПозиции) Экспорт
	...
	УдалитьНачисленияИЕжегодныеОтпуска(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Форма.ИдентификаторРедактируемойСтроки), Объект);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ЛьготыСотрудников") Тогда
		МодульЛьготыСотрудников = ОбщегоНазначения.ОбщийМодуль("ЛьготыСотрудников");
		МодульЛьготыСотрудников.ПрочитатьДанныеПозицииВФорму(Форма, Объект, НовыеДанныеПозиции);
	КонецЕсли;
...
Процедура ДобавитьДополнительныеСведенияПозиций(Форма, ТекущийОбъект)
	...
	Если Форма.Модифицированность Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ЛьготыСотрудников") Тогда
			МодульЛьготыСотрудников = ОбщегоНазначения.ОбщийМодуль("ЛьготыСотрудников");
			МодульЛьготыСотрудников.ДобавитьДополнительныеСведенияОЛьготахПозиций(Форма, ТекущийОбъект);
		КонецЕсли;
	...
	Если Форма.ПолучитьФункциональнуюОпциюФормы("ИспользоватьИсториюИзмененияШтатногоРасписания") Тогда
		...
		Для Каждого СтрокаПозиции Из ПозицииДанныеФормы Цикл
			...
			Иначе
				...
				ДополнительныеПараметры.Вставить("ПолноеНаименованиеДолжности", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					СтрокаПозиции.Должность, "НаименованиеДляЗаписейОТрудовойДеятельности"));
				ДополнительныеПараметры.Вставить("ПолноеНаименованиеПодразделения", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					СтрокаПозиции.Подразделение, "НаименованиеДляЗаписейОТрудовойДеятельности"));
...
Процедура ЗаполнитьТерриториальныеУсловияВСтрокахПозиций(Форма, ДатаВступленияВСилу, ОписаниеТаблицыПозиций)
	...
	Для каждого СтрокаПозиции Из ПозицииДанныеФормы Цикл
		...
		Если ЗначениеЗаполнено(СтрокаПозиции.Подразделение) Тогда
			...
			ТерриториальныеУсловияПФРПодразделения = Форма.СоответствиеТерриториальныхУсловийПФРПодразделениям.Получить(СтрокаПозиции.Подразделение);
			Если ТерриториальныеУсловияПФРПодразделения = Неопределено Тогда
				СтрокаПозиции.ТерриториальныеУсловияПФР = Неопределено;
			Иначе
				СтрокаПозиции.ТерриториальныеУсловияПФР = ТерриториальныеУсловияПФРПодразделения.ТерриториальныеУсловияПФР;
			КонецЕсли;
...
Функция ТерриториальныеУсловияПФРПодразделений(Подразделения, ДатаСведений) Экспорт
	...
	Запрос.Текст =
		...
		|	ТерриториальныеУсловия.ПрименятьСевернуюНадбавку КАК ПрименятьСевернуюНадбавку
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		СоответствиеУсловийПодразделениям.Вставить(Выборка.СтруктурнаяЕдиница,
			Новый Структура("ТерриториальныеУсловияПФР,ПрименятьСевернуюНадбавку",
				Выборка.ТерриториальныеУсловияПФР,
				Выборка.ПрименятьСевернуюНадбавку));
	КонецЦикла; 
...
Процедура ЗаполнитьНачисленияПоказателиСтрокиПозиции(Форма, СтрокаПозиции, Начисления, Показатели, СведенияОПоказателяхФормы) Экспорт
...
Процедура УстановитьТекущиеЗначенияПоказателейСтрокиПозиции(Форма, СтрокаПозиции, ИнфоОПоказателе, СтрокаПоказателя) Экспорт
	...
	Если Форма.ПолучитьФункциональнуюОпциюФормы("ИспользоватьВилкуСтавокВШтатномРасписании") Тогда
		...
		СтрокаПозиции[ИмяПоказателяМин + "ТекущееЗначение"] = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'было: %1';
																															|en = 'previous value: %1'"), СуммаПодстановкиМин);
		СтрокаПозиции[ИмяПоказателяМакс + "ТекущееЗначение"] = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'было: %1';
																															|en = 'previous value: %1'"), СуммаПодстановкиМакс);
	Иначе
		...
		СтрокаПозиции[ИмяПоказателя + "ТекущееЗначение"] = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'было: %1';
