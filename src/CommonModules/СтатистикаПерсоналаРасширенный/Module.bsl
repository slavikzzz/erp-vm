...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка настройки расчета среднесписочной численности';
									|en = 'Set up average number calculation'");
	Обработчик.НачальноеЗаполнение = Истина;
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка настройки статистики персонала для организации';
									|en = 'Установка настройки статистики персонала для организации'");
	Обработчик.НачальноеЗаполнение = Истина;
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка вид формы мониторинга в строках отчетности мониторинга работников социальной сферы';
									|en = 'Установка вид формы мониторинга в строках отчетности мониторинга работников социальной сферы'");
	Обработчик.НачальноеЗаполнение = Истина;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.ПодключаемыеХарактеристики") Тогда
		...
		Обработчик.Комментарий = НСтр("ru = 'Очистка некорректных подключаемых характеристик строк отчетности форм мониторинга';
										|en = 'Clear incorrect connected characteristics of monitoring form report lines'");
		...
		Обработчик.Комментарий = НСтр("ru = 'Очистить  подключаемые характеристики строк отчетности форм мониторинга';
										|en = 'Clear the connected characteristics in lines of monitoring form reports'");
		Обработчик.НачальноеЗаполнение = Истина;
		...
		Обработчик.Комментарий = НСтр("ru = 'Отключение некорректных подключаемых характеристик строк отчетности форм мониторинга';
										|en = 'Disable incorrect connected characteristics of monitoring form report lines'");
		...
		Обработчик.Комментарий = НСтр("ru = 'Обновление подключаемых характеристик строк отчетности форм мониторинга';
										|en = 'Update the connected characteristics in lines of monitoring form reports'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Порядок заполнения отчета устарел, рекомендуется обновить конфигурацию.
				|Подробности: %1';
				|en = 'Report filling procedure is outdated, it is recommended that you update configuration.
				|More details: %1'"),
...
Функция СотрудникиДляЗаполненияРаздела2Формы57Т_2021(Организация, ПериодОтчета, ВсеСотрудникиПоКатегориям)
	...
	Запрос.Текст =
		...
		|	Форма57ТРаздел2Сотрудники.Сотрудник.ФизическоеЛицо КАК ФизическоеЛицо
		|ИЗ
		|	РегистрСведений.Форма57ТРаздел2Сотрудники КАК Форма57ТРаздел2Сотрудники
		|ГДЕ
		|	Форма57ТРаздел2Сотрудники.Организация = &Организация
...
Процедура ДополнитьФорма57ТРаздел2СотрудникиДаннымиОНачислениях_2021(Форма57ТРаздел2Сотрудники, ДанныеОрганизации, УчтенаКвартальнаяПремия, УчестьТолькоКвартальнуюПремию = Ложь)
	...
	ИсходнаяТаблицаСотрудников.Свернуть("Сотрудник,Должность,Пол,ГодРождения,Образование,КодКатегории,КодПоОКПДТР,Стаж,
		|Наименование,Организация,Подразделение,Отобран,НомерСтрокиОтчета,ФизическоеЛицо");
	...
	Для Каждого СтрокаФормы Из ИсходнаяТаблицаСотрудников Цикл
		...
		СтрокиНачислений = ДанныеОрганизации.Строки.НайтиСтроки(Новый Структура("ФизическоеЛицо", СтрокаФормы.ФизическоеЛицо));   
		ПерваяСтрока = Истина;
		Для Каждого СтрокаНачислений Из СтрокиНачислений Цикл
			НачисленоРаботникам = СтрокаНачислений.НачисленоРаботникамСписочногоСостава;
...
Функция ВнешнийИсточникСтатистикиФормы1(ПараметрыОтчета)
	...
	ТаблицаСотрудников = Новый ТаблицаЗначений;
	ТаблицаСотрудников.Колонки.Добавить("Счетчик");
	ТаблицаСотрудников.Колонки.Добавить("Начислено");
...
Функция ВнешнийИсточникСтатистикиФормы57Т(ПараметрыОтчета)
	...
	СотрудникиПоКатегориям.Колонки.Добавить("ФизическоеЛицо");
	...
	ПараметрыОтчетаДляЗаполненияЗарплата = СкопироватьКоллекцию(ПараметрыОтчета);
	ПараметрыОтчетаДляЗаполненияЗарплата.Вставить("ТолькоОсновноеМестоРаботы", Ложь);
	ПараметрыОтчетаДляЗаполненияЗарплата.Вставить("ИсключаяПерерасчеты", Истина);
	ДанныеДляЗаполненияРегламентированнойОтчетностиЗарплата(
		"РегламентированныйОтчетСтатистикаФорма57Т",
		ПараметрыОтчетаДляЗаполненияЗарплата,
...
Функция КритерииОтбораПоРегистрацииРеспондента(ПараметрыОтчета)
	...
	Если СвойстваРеспондента.ЭтоОрганизация Тогда
		...
		// Запрос 2 - исключаемые территории и сотрудники, которые весь отчетный период были на территории не входящей в регистрацию.
		...
		ЗапросДанных = Новый Запрос(
			...
			|ИЗ
			|	РегистрСведений.ИспользованиеРегистрацийВОрганеСтатистики.СрезПоследних(&ДатаНачалаПериодаОтчета, ) КАК ИспользованиеРегистрацийВОрганеСтатистикиСрезПоследних
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТТерриторииСотрудниковПериоды КАК ТерриторииСотрудников
			|		ПО (ТерриторииСотрудников.Территория = ИспользованиеРегистрацийВОрганеСтатистикиСрезПоследних.СтруктурнаяЕдиница)
			|			И (ТерриторииСотрудников.НачалоПериода <= &ДатаНачалаПериодаОтчета)
			|			И (ТерриторииСотрудников.КонецПериода >= &ДатаКонцаПериодаОтчета)
			|ГДЕ
			|	ЕСТЬNULL(ИспользованиеРегистрацийВОрганеСтатистикиСрезПоследних.Регистрация, &РегистрацияРеспондента) <> &РегистрацияРеспондента
		...
		Если Не РезультатЗапроса[ИндексПоследнего - 1].Пустой() Тогда
			ИсключаемыеРегистрации = РезультатЗапроса[ИндексПоследнего - 1].Выгрузить().ВыгрузитьКолонку("Регистрация");
		...
		Если Не РезультатЗапроса[ИндексПоследнего].Пустой() Тогда
			ИсключаемыеПоТерриториям = РезультатЗапроса[ИндексПоследнего].Выгрузить();
			
			Территории = ИсключаемыеПоТерриториям.Скопировать(, "Территория");
			Территории.Свернуть("Территория");
			
			Результат.ИсключаемыеСотрудники = ИсключаемыеПоТерриториям.ВыгрузитьКолонку("Сотрудник");
			Результат.ИсключаемыеТерритории = Территории.ВыгрузитьКолонку("Территория");
		КонецЕсли;
	...
	Иначе
		...
		// Запрос 1 - включаемые сотрудники, которые были на территории входящей в регистрацию весь отчетный период или его часть.
		// Запрос 2 - исключаемые территории и сотрудники, которые весь отчетный период были на территории не входящей в регистрацию.
		...
		ЗапросДанных.УстановитьПараметр("Регистрация", ПараметрыОтчета.РегистрацияРеспондента);
		ЗапросДанных.УстановитьПараметр("ГоловнаяОрганизация", ГоловнаяОрганизация);
		ЗапросДанных.УстановитьПараметр("ДатаНачалаПериодаОтчета", ПараметрыОтчета.ДатаНачалаПериодаОтчета);
		ЗапросДанных.УстановитьПараметр("ДатаКонцаПериодаОтчета", ПараметрыОтчета.ДатаКонцаПериодаОтчета);
		ЗарплатаКадрыОбщиеНаборыДанных.ОбъединитьЗапросы(Запрос, ЗапросДанных);
		...
		Если Не РезультатЗапроса[ИндексПоследнего - 1].Пустой() Тогда
			ВключаемыеПоТерриториям = РезультатЗапроса[ИндексПоследнего - 1].Выгрузить();
			
			Территории = ВключаемыеПоТерриториям.Скопировать(, "Территория");
			Территории.Свернуть("Территория");
			
			Результат.ВключаемыеСотрудники = ВключаемыеПоТерриториям.ВыгрузитьКолонку("Сотрудник");
			Результат.ВключаемыеТерритории = Территории.ВыгрузитьКолонку("Территория");
		КонецЕсли;
...
Процедура НастроитьОтборТОСП(Отбор, ИмяПоляПодразделение, КритерииОтбора)
	...
	Если КритерииОтбора.ПоИсключаемым Тогда
		...
		Иначе
			ГруппаИ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
				Отбор, НСтр("ru = 'Исключаемые подразделения и сотрудники';
							|en = 'Business units and employees subject to exclusion'"), ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаИ, ИмяПоляПодразделение, ВидСравненияКомпоновкиДанных.НеВСписке, КритерииОтбора.ИсключаемыеПодразделения);
	...
	ИначеЕсли КритерииОтбора.ПоВключаемым Тогда
		...
		Если КритерииОтбора.ИсключаемыеСотрудники = Неопределено И КритерииОтбора.ВключаемыеСотрудники = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор, ИмяПоляПодразделение,	ВидСравненияКомпоновкиДанных.ВСписке, КритерииОтбора.ВключаемыеПодразделения);
		Иначе
			Если КритерииОтбора.ВключаемыеПодразделения <> Неопределено И КритерииОтбора.ВключаемыеСотрудники <> Неопределено Тогда
				МестоОтбора = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
					Отбор, НСтр("ru = 'Подразделения или сотрудники';
								|en = 'Business units or employees'"), ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
			Иначе
				МестоОтбора = Отбор;
			КонецЕсли;
			...
			Иначе
				ГруппаИ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
					МестоОтбора, НСтр("ru = 'Подразделение';
										|en = 'Business unit'"), ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
			...
			Если КритерииОтбора.ВключаемыеСотрудники <> Неопределено Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(МестоОтбора, ИмяПоляСотрудник, ВидСравненияКомпоновкиДанных.ВСписке, КритерииОтбора.ВключаемыеСотрудники);
			КонецЕсли;
...
Функция ЗаполнитьКомпоновщикНастроек(КлючВарианта, ПараметрыОтчета, СхемаКД) Экспорт
	...
	КонецЕсли;
	НастроитьОтборТОСП(Отбор, ИмяПоляПодразделение, КритерииОтбораТОСП);
...
Процедура ДобавитьПоказателиФормы7Травматизм_2020(ПоказателиОтчета)
	...
	ЗарплатаКадры.ДобавитьПоказательРегламентированнойОтчетности(ПоказателиОтчета, "П001001002304", Истина, Ложь);
	ЗарплатаКадры.ДобавитьПоказательРегламентированнойОтчетности(ПоказателиОтчета, "П001001002404", Истина, Ложь);
...
Процедура ЗаполнитьПоказателиФормы7Травматизм_2020(ПараметрыОтчета, Контейнер)
	...
	Исключение
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Порядок заполнения отчета устарел, рекомендуется обновить конфигурацию.
				|Подробности: %1';
				|en = 'Порядок заполнения отчета устарел, рекомендуется обновить конфигурацию.
				|Подробности: %1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
