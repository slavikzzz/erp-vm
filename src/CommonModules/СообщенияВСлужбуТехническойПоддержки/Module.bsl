...
Функция ТекстТехническойИнформации() Экспорт
	...
	ШаблонТекста = НСтр("ru = 'Техническая информация о программе:
		...
		|en = 'Technical information about the program:
		|Program name: %1
		|Program name %2 <%3>; 
		|Program version: %4;
		|Vendor: %5; 
		|1C: Enterprice version: %6 <%7>'");
	...
	ШаблонТекста = НСтр("ru = 'Вид приложения: %1;
		...
		|en = 'Application type: %1;
		|Mode: %2;
		|Full rights: %3;
		|Administrator rights: %4;
		| Username for connecting to the Internet support: %5; 
		|Password for connecting to the Internet support: %6;
		|Setting of connection with the servers of the Internet support:
		|	Domain zone: %7 
		|Proxy server setting: %8'");
		...
		Если Не ВременныйФайл.Существует() Тогда
			...
			Результат.СообщениеОбОшибке =
				НСтр("ru = 'Файл выгрузки журнала регистрации не обнаружен. Обратитесь к администратору.';
					|en = 'The event log export file was not found. Contact the administrator.'");
			Возврат Результат;
		КонецЕсли;
		...
		Если ВременныйФайл.Размер() > МаксимальныйРазмерФайла() Тогда
			...
			Результат.СообщениеОбОшибке =
				НСтр("ru = 'Превышен максимально допустимый размер файла журнала регистрации. Измените параметры выгрузки данных.';
					|en = 'The maximum allowed file size of the event log is exceeded. Change data export parameters.'");
			Возврат Результат;
		КонецЕсли;
	...
	Исключение
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось выгрузить события журнала регистрации в файл по причине:
					|%1';
					|en = 'Cannot export event log events to the file due to:
					|%1'"),
		...
		Результат.СообщениеОбОшибке =
			НСтр("ru = 'Не удалось выгрузить события журнала регистрации. Обратитесь к администратору.';
				|en = 'Cannot export event log events. Contact the administrator.'");
		...
		ОписаниеВерсийПодсистем.Добавить(
			...
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1; %2; %3; Это конфигурация: %4';
					|en = '%1; %2; %3; This is configuration: %4'"),
	...
	Для Каждого ДанныеРасширения Из ДанныеРасширений Цикл
		...
		Иначе
			ВызватьИсключение НСтр("ru = 'Неизвестный вариант назначения расширения';
									|en = 'Unknown extension assignment option'");
		КонецЕсли;
		...
		ОписаниеРасширений.Добавить(
			...
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 (%2); Акт.: %3; Распр. ИБ: %4; БР: %5; Это патч: %6';
					|en = '%1 (%2); Act.: %3; Ext. IB: %4; BR: %5; This is patch %6'"),
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось отправить данные сообщения в службу технической поддержки.
					|%1';
					|en = 'Cannot send message data to technical support service.
					|%1'"),
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось отправить данные сообщения в службу технической поддержки.
					...
					|Тело ответа: %2';
					|en = 'Cannot send message data to technical support service.
					|An error occurred while processing service response. Incorrect response 
					|of send message service.
					|%1
					|Response body: %2'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
	...
	Если ТипЗнч(РезультатВызова) <> Тип("Структура")
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			...
			НСтр("ru = 'Не удалось отправить данные сообщения в службу технической поддержки.
				...
				|Ответ сервиса: %1';
				|en = 'Cannot send message data to the technical support service.
		...
		Результат.СообщениеОбОшибке =
			НСтр("ru = 'Не удалось отправить данные сообщения в службу технической поддержки.
				|Ошибка при передаче сообщения в службу технической поддержки.';
				|en = 'Cannot send message data to technical support service. 
				|An error occurred while sending the message to technical support service.'");
