					...
					Исключение
						ЗаписьЖурналаРегистрации(НСтр("ru = 'Центр мониторинга.Статистика конфигурации переопределяемая';
							...
							НСтр("ru = 'Не удалось установить разделение сеанса. Область данных';
								|en = 'Couldn''t set session separation. Data area'", ОбщегоНазначения.КодОсновногоЯзыка()) 
...
Функция ПолучитьКаталогНастроекТехнологическогоЖурнала()
	...
	Пока ПоискКаталога = Истина Цикл
		...
		Если КаталогиНастроек.Найти(ПутьКаталога) <> Неопределено Тогда
			...
			КаталогНастроек.ОписаниеОшибки = НСтр("ru = 'Обнаружена циклическая ссылка';
													|en = 'Circular ref is found'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПолучитьКаталогИзФайлаКонфигурацииНастроек(ФайлКонфигурацииНастроек)
	...
	Иначе
		...
		КаталогНастроек.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не найдена секция %1 в файле %2';
				|en = 'Section %1 is not found in file %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция СобытиеЖурналаРегистрацииЦентрМониторингаУдалениеДампа()
	Возврат НСтр("ru = 'Центр мониторинга.Удаление дампа';
				|en = 'Monitoring center.Removing the dump'", ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции

Функция СобытиеЖурналаРегистрацииРазобратьБуферОперацийСтатистики()
	Возврат НСтр("ru = 'Центр мониторинга.Разобрать буфер операций статистики';
				|en = 'Monitoring center.Parse the buffer of statistics operations'", ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
#КонецОбласти
