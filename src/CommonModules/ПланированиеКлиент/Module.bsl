		...
		Если АктивныеПериоды.Количество() > 1  Тогда
			...
			Если Элементы[ИмяЭлементаТаблицы].ТекущиеДанные["Отменено_"+Период.ИмяКолонки] Тогда
				ТекстВопроса = НСтр("ru = 'Снять пометку отмены ячеек плана для всех периодов?';
									|en = 'Clear a cancellation mark for plan cells for all periods?'");
			Иначе
				ТекстВопроса = НСтр("ru = 'Установить пометку отмены ячеек плана для всех периодов?';
									|en = 'Set a mark of plan cell cancellation for all periods?'");
			КонецЕсли; 
			...
			Кнопки.Добавить(КодВозвратаДиалога.Да,     НСтр("ru = 'Да, для всех';
															|en = 'Yes, for all'"));
			Кнопки.Добавить(КодВозвратаДиалога.Нет,    НСтр("ru = 'Только';
...
Процедура ПоказатьВопросПриИзмененииСценария(Форма, ОповещениеОЗакрытии = Неопределено) Экспорт 
	...
	Если ЗначениеЗаполнено(Форма["РеквизитыДоИзменения"].Сценарий) Тогда
		...
		ТекстВопроса = НСтр("ru = 'Сценарий изменен. Правило заполнения будет обновлено. Обновить?';
							|en = 'Scenario is changed. Filling rule will be updated. Update?'");
		Кнопки = Новый СписокЗначений;
...
Процедура ПоказатьВопросПриИзмененииВидПлана(Форма, ОповещениеОЗакрытии = Неопределено) Экспорт 
	...
	Если ЗначениеЗаполнено(Форма["РеквизитыДоИзменения"].Сценарий) Тогда
		...
		ТекстВопроса = НСтр("ru = 'Вид плана изменен. Правило заполнения будет обновлено. Обновить?';
							|en = 'Plan profile is changed. Filling rule will be updated. Update?'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Обновить';
													|en = 'Update'"));
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ru = 'Отменить изменение вида плана';
													|en = 'Cancel plan profile change'"));
		ПоказатьВопрос(Оповещение, ТекстВопроса, Кнопки);
...
Процедура ПриИзмененииПериодаПлана(Форма, Знач ИмяКроссТаблицы, ОповещениеОбновленияИнтерфейса) Экспорт 
	...
	Если ПревышеноКоличествоПериодов Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Превышено рекомендуемое количество периодов (%1).
			...
			|Продолжить?';
			|en = 'You have exceeded the recommended number of periods (%1).
			|It might slow down the system operation.
			|Continue?'"), СтрокаПериод);
		...
		Если Объект.КроссТаблица
			...
			Если ЗадаватьВопрос Тогда
				...
				ТекстВопроса = НСтр("ru = 'Изменение периода плана приведет к удалению данных в некоторых периодах. Удалить периоды?';
									|en = 'Plan period change will lead to data deletion in some periods. Delete the periods?'");
		...
		Если СтрокаТЧ.Свойство("Номенклатура") Тогда
			...
			Если НЕ СтрокаТЧ.Характеристика.Пустая() Тогда
				...
				Возврат СтроковыеФункцииКлиент.ФорматированнаяСтрока(НСтр("ru = '<a href=""%1"">%2</a>, <a href=""%3"">%4</a>: %5';
																			|en = '<a href=""%1"">%2</a>, <a href=""%3"">%4</a>: %5'"),
...
Функция ФункцииИзОбщегоМодуля() Экспорт
	...
	ОписаниеФункции.Пояснение = НСтр("ru = 'Вычисление номера текущей колонки';
									|en = 'Calculating current column number'");
...
Процедура ВвестиПроцентИИзменитьФормулу(Результат, ДополнительныеПараметры) Экспорт
	...
	ПоказатьВводЧисла(Оповещение, 0, НСтр("ru = 'Введите процент';
											|en = 'Enter the percentage'"), 5, 2);
...
Процедура ВыбратьТочностьОкругленияИИзменитьФормулуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ШаблонФормулы = НСтр("ru = 'Окр((%ТекущаяФормула%), %ТочностьОкругления%)';
						|en = 'Round((%ТекущаяФормула%), %ТочностьОкругления%)'");
...
Функция ОписаниеКолонокМакетаДляЗагрузкиНоменклатуры(ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ИспользуютсяНазначения Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("Назначение", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(150)), НСтр("ru = 'Назначение';
																																							|en = 'Assignment'"), 15);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	...
	Если ДополнительныеПараметры.ИспользуютсяСпецификации Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("Спецификация", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(150)), НСтр("ru = 'Спецификация';
																																								|en = 'Bill of materials'"), 15);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	//-- НЕ УТ
	...
	Если ДополнительныеПараметры.ИспользуютсяСклады Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("Склад", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(150)), НСтр("ru = 'Склад';
																																						|en = 'Warehouse'"), 15);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	...
	Если ДополнительныеПараметры.ИспользуютсяПартнеры Тогда
		Заголовок = ?(ДополнительныеПараметры.ТипПлана = "ПланЗакупок", НСтр("ru = 'Поставщик';
																			|en = 'Vendor'"), НСтр("ru = 'Клиент';
																										|en = 'Customer'"));
	...
	Если ДополнительныеПараметры.ИспользуютсяСоглашения Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("Соглашение", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(150)), НСтр("ru = 'Соглашение';
																																							|en = 'Agreement'"), 15);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	...
	Если ДополнительныеПараметры.ИспользуютсяВидыЦен Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("ВидЦены", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(150)), НСтр("ru = 'Вид цены';
																																							|en = 'Price type'"), 10);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	...
	Если НЕ КроссТаблица И ДополнительныеПараметры.ПланироватьПоСумме Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("Цена", Новый ОписаниеТипов("Число",, Новый КвалификаторыЧисла(15, 3)), НСтр("ru = 'Цена';
																																						|en = 'Price'"), 10);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
		
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("Сумма", Новый ОписаниеТипов("Число",, Новый КвалификаторыЧисла(15, 3)), НСтр("ru = 'Сумма';
																																						|en = 'Amount'"), 10);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	...
	Если ДополнительныеПараметры.ТипПлана = "ПланСборкиРазборки" Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("ВариантКомплектации", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(150)), НСтр("ru = 'Комплектация';
																																										|en = 'Product kit'"), 15);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
		
		Если НЕ КроссТаблица Тогда
			Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("ДатаСборкиРазборки", Новый ОписаниеТипов("Дата",, Новый КвалификаторыДаты()), НСтр("ru = 'Дата сборки (разборки)';
																																									|en = 'Date of kitting'"), 15);
			Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
			КолонкиМакета.Добавить(Колонка);
		КонецЕсли;
	КонецЕсли;
	...
	//++ НЕ УТ
	Если НЕ КроссТаблица И ДополнительныеПараметры.ИспользуютсяДатыВыпуска Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("ДатаВыпуска", Новый ОписаниеТипов("Дата",, Новый КвалификаторыДаты()), НСтр("ru = 'Дата выпуска';
																																						|en = 'Release date'"), 15);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	
	Если НЕ КроссТаблица И ДополнительныеПараметры.ИспользуютсяДатыВыпускаПродукцииПолуфабриката Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("ДатаВыпускаПродукцииПолуфабриката", Новый ОписаниеТипов("Дата",, Новый КвалификаторыДаты()), НСтр("ru = 'Дата выпуска продукции полуфабриката';
																																											|en = 'Release date of products or semi-finished product'"), 20);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	
	Если НЕ КроссТаблица И ДополнительныеПараметры.ТипПлана = "ПланПроизводства" Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("ДатаЗапуска", Новый ОписаниеТипов("Дата",, Новый КвалификаторыДаты()), НСтр("ru = 'Дата запуска';
																																						|en = 'Launch date'"), 15);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	...
	Если НЕ КроссТаблица И ДополнительныеПараметры.ТипПлана = "ПланЗакупок" Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("ДатаПоступления", Новый ОписаниеТипов("Дата",, Новый КвалификаторыДаты()), НСтр("ru = 'Дата поступления';
																																							|en = 'Date of receipt'"), 15);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
		
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("ДатаЗаказа", Новый ОписаниеТипов("Дата",, Новый КвалификаторыДаты()), НСтр("ru = 'Дата заказа';
																																						|en = 'Order date'"), 10);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
		
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("СрокИсполненияЗаказа", Новый ОписаниеТипов("Число",, Новый КвалификаторыЧисла(8, 0)), НСтр("ru = 'Срок исполнения';
																																										|en = 'Due date'"), 13);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	...
	Если НЕ КроссТаблица И (ДополнительныеПараметры.ТипПлана = "ПланВнутреннихПотреблений"
		...
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("ДатаОтгрузки", Новый ОписаниеТипов("Дата",, Новый КвалификаторыДаты()), НСтр("ru = 'Дата отгрузки';
																																						|en = 'Shipment date'"), 15);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	...
	Если КроссТаблица Тогда
		Для Каждого Период Из ТаблицаПериоды Цикл
			...
			Если ДополнительныеПараметры.ПланироватьПоСумме Тогда
				...
				Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета(ИмяКолонки, Новый ОписаниеТипов("Число",, Новый КвалификаторыЧисла(15, 3)), НСтр("ru = 'Количество';
																																									|en = 'Quantity'") + " " + Период.Заголовок, 15);
				Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
				КолонкиМакета.Добавить(Колонка);
				
				ИмяКолонки = "Цена_" + Период.ИмяКолонки;
				Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета(ИмяКолонки, Новый ОписаниеТипов("Число",, Новый КвалификаторыЧисла(15, 3)), НСтр("ru = 'Цена';
																																									|en = 'Price'") + " " + Период.Заголовок, 15);
				Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
				КолонкиМакета.Добавить(Колонка);
				
				ИмяКолонки = "Сумма_" + Период.ИмяКолонки;
				Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета(ИмяКолонки, Новый ОписаниеТипов("Число",, Новый КвалификаторыЧисла(15, 3)), НСтр("ru = 'Сумма';
																																									|en = 'Amount'") + " " + Период.Заголовок, 15);
				Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
				КолонкиМакета.Добавить(Колонка);
			Иначе
				ИмяКолонки = "КоличествоУпаковок_" + Период.ИмяКолонки;
	...
	Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("Комментарий", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(0)), НСтр("ru = 'Комментарий';
																																						|en = 'Comment'"), 20);
	Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
	КолонкиМакета.Добавить(Колонка);
...
Функция ОписаниеКолонокМакетаДляЗагрузкиТоварныхКатегорий(ДополнительныеПараметры) Экспорт
	КолонкиМакета = Новый Массив;
	Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("ТоварнаяКатегория", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(150)), НСтр("ru = 'Товарная категория';
																																								|en = 'Product category'"), 15);
	Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
	КолонкиМакета.Добавить(Колонка);
	...
	Если ДополнительныеПараметры.ИспользуетсяРасчетПоСкоростиПродаж Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("РейтингПродаж", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(100)), НСтр("ru = 'Рейтинг продаж';
																																								|en = 'Sales rank'"), 15);
		Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
...
Функция ДополнительныеПараметрыЗагрузкиИзФайла() Экспорт
	...
	//++ НЕ УТ
	ДополнительныеПараметры.Вставить("ИспользуютсяСпецификации",                      Ложь);
	ДополнительныеПараметры.Вставить("ИспользуютсяДатыВыпуска",                       Ложь);
	ДополнительныеПараметры.Вставить("ИспользуютсяДатыВыпускаПродукцииПолуфабриката", Ложь);
	//-- НЕ УТ
