...
Функция ПолучитьОператоровЭлектронногоДокументооборота(КонтекстДиагностики, ПолучатьНеобработанныеДанные = Ложь) Экспорт
	...
	ПараметрыОперации = Новый Структура("Наименование",
		НСтр("ru = 'Получение данных об операторах электронного документооборота';
			|en = 'Receive EDI service provider data'"));
	
	РезультатПолученияДанныхФайла = ДанныеФайлаОблачногоХранилищаНастроек("/operators/catalog.json",
		ПараметрыОперации.Наименование, , КонтекстДиагностики);
	Если РезультатПолученияДанныхФайла.ДанныеФайла = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Если ПолучатьНеобработанныеДанные Тогда
		Возврат РезультатПолученияДанныхФайла.ДанныеФайла;	
	Иначе
		Отказ = Ложь;
		ПолученныеДанные = ОбъектИзJSON(ПараметрыОперации, РезультатПолученияДанныхФайла.ДанныеФайла, Отказ,
			КонтекстДиагностики);
	КонецЕсли;
...
Функция ПолучитьСовместимостьОператоровЭлектронногоДокументооборота(КонтекстДиагностики, ПолучатьНеобработанныеДанные = Ложь) Экспорт
	...
	ПараметрыОперации = Новый Структура("Наименование",
		НСтр("ru = 'Получение данных о совместимости операторов электронного документооборота';
			|en = 'Receive EDI service provider compatibility data'"));
	
	РезультатПолученияДанныхФайла = ДанныеФайлаОблачногоХранилищаНастроек(
...
Функция ПолучитьКонтрагентовСоздаваемыхБезусловно(КонтекстДиагностики, ПолучатьНеобработанныеДанные = Ложь) Экспорт
	...
	ПараметрыОперации = Новый Структура("Наименование",
		НСтр("ru = 'Получение данных контрагентов создаваемых безусловно';
			|en = 'Receive data on the counterparties created unconditionally'"));
...
Процедура ОбновитьСовместимостьОператоровЭлектронногоДокументооборота(СовместимостьОператоров,
	...
	Исключение
		...
		ВидОперации = НСтр("ru = 'Обновление данных о совместимости операторов электронного документооборота.';
							|en = 'Update EDI service provider compatibility data.'");
		ЭлектронноеВзаимодействие.ОбработатьОшибку(ВидОперации,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
			НСтр("ru = 'Не удалось обновить данные о совместимости операторов ЭДО.';
				|en = 'Cannot update EDI service provider compatibility data.'"));
	КонецПопытки;
...
Функция ОбновитьИнформациюОбАктивностиУчетнойЗаписи(ИдентификаторУчетнойЗаписи, КонтекстДиагностики) Экспорт
	...
	ПараметрыКоманды.Наименование        = НСтр("ru = 'Отправка данных об активности абонента в сервис 1С:ЭДО';
												|en = 'Sending subscriber activity data to 1C:EDI service'");
...
#Область ПереченьСпециальныхОбстоятельств

// Получает из сервиса файл с данными перечня специальных обстоятельств.
// 
// Параметры:
...
// Возвращаемое значение:
// 	- Неопределено - форматы получить не удалось
// 	- См. РезультатПолученияДанныхФайлаОблачногоХранилищаНастроек
Функция ПолучитьПереченьСпециальныхОбстоятельств(ДатаИзмененияФайла, КонтекстДиагностики) Экспорт
	
	ПараметрыОперации = Новый Структура("Наименование",
		НСтр("ru = 'Получение перечня специальных обстоятельств ФНС';
			|en = 'Получение перечня специальных обстоятельств ФНС'"));
	Путь = "/special_circumstances/special_circumstances.json";
	...
	РезультатПолученияДанныхФайла = ДанныеФайлаОблачногоХранилищаНастроек(Путь, ПараметрыОперации.Наименование,
		ПараметрыПолучения, КонтекстДиагностики);
	
	Если РезультатПолученияДанныхФайла.ДанныеФайла = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат РезультатПолученияДанныхФайла;
...
//  Структура - данные, полученные из файла.:
//   * ДатаПоследнегоИзменения - Дата
...
//
Функция ОбработкаРезультатаКлассификаторПолномочийФНС_МЧД002(Знач ПолученныеДанные,
	ДатаПоследнегоИзменения = Неопределено, Отказ = Ложь, КонтекстДиагностики = Неопределено) Экспорт
	...
	// Если функция вызывается из обработчика обновления, то даты последнего редактирования файла нет.
	// Считаем датой последнего изменения значение атрибута внутри данных.
	Если ДатаПоследнегоИзменения = Неопределено Тогда
		
		ДатаПоследнегоИзмененияИзДанных = ДатаИзJsonСПроверкой(ПолученныеДанные.lastChangeDate, ФорматДатыJSON.ISO);
		ПараметрыОперации = СинхронизацияЭДО.ЗаполнитьПараметрыСинхронизацииКлассификатораПолномочий_МЧД003();
		
		Если ПараметрыОперации.ДатаПоследнегоИзменения >= ДатаПоследнегоИзмененияИзДанных Тогда
			Возврат Неопределено;
		Иначе
			ДатаПоследнегоИзменения = ДатаПоследнегоИзмененияИзДанных;
		КонецЕсли;
	КонецЕсли;
...
// Возвращает данные классификатора полномочий (МЧД 003).
// 
// Параметры:
//  ПолученныеДанные - Структура
//                   - Строка
...
//  Структура - данные, полученные из файла.:
//   * ДатаПоследнегоИзменения - Дата
//   * КлассификаторПолномочийМЧД003 - ТаблицаЗначений:
//    ** КодКлассификатора - Строка
//    ** Наименование      - Строка
//    ** Полномочие        - Строка
//    ** Мнемоника         - Строка
...
//    ** ГруппаПодгруппаПолномочия - Строка
//    ** МнемоникаСистемыВладельцаПолномочия   - Строка
//    ** ДатаВключенияПолномочияВКлассификатор - Дата
//    ** ДатаОтзываПолномочия                  - Дата
//    ** МаксимальныйСрокДействияДоверенности  - Число
//    ** ОбластьВидимостиПолномочия            - Строка
//    ** ПравоПолученияДоверенностиСДаннымПолномочием - Строка
//    ** ПравоНаВыдачуДоверенностиСДаннымПолномочием  - Строка
//    ** КонтекстПолномочия - Строка
//    ** Правоотношения     - Строка
//    ** РеквизитыНПА       - Строка
//    ** Автоключ           - Строка
Функция ОбработкаРезультатаКлассификаторПолномочий_МЧД003(Знач ПолученныеДанные, ДатаПоследнегоИзменения = Неопределено,
	Отказ = Ложь, КонтекстДиагностики = Неопределено) Экспорт
	
	Если ТипЗнч(ПолученныеДанные) = Тип("Строка") Или ТипЗнч(ПолученныеДанные) = Тип("ДвоичныеДанные") Тогда
		ПолученныеДанные = ОбъектИзJSON(Неопределено, ПолученныеДанные, Отказ, КонтекстДиагностики);
		Если Отказ Тогда
			Возврат Неопределено;
		КонецЕсли;
	...
	Если ДатаПоследнегоИзменения = Неопределено Тогда
		...
		ПринудительноеОбновление = (ПолученныеДанные.forcedUpdate = "1");
	...
	Полномочия.Колонки.Добавить("УдалитьТипОтношений", Типы().Строка);
	Полномочия.Колонки.Добавить("ДатаИзменения",       Типы().Дата);
	Полномочия.Колонки.Добавить("Скрипт",              Типы().Строка);
	Полномочия.Колонки.Добавить("НастройкиПроверки",   Типы().Таблица);
	Полномочия.Колонки.Добавить("ОписаниеПолномочия",  Типы().Строка);
	Полномочия.Колонки.Добавить("ГруппаПодгруппаПолномочия",             ТипСтрока255);
	Полномочия.Колонки.Добавить("МнемоникаСистемыВладельцаПолномочия",   ТипСтрока255);
	Полномочия.Колонки.Добавить("ДатаВключенияПолномочияВКлассификатор", Типы().Дата);
	Полномочия.Колонки.Добавить("ДатаОтзываПолномочия",                  Типы().Дата);
	Полномочия.Колонки.Добавить("МаксимальныйСрокДействияДоверенности",  Типы().Число);
	Полномочия.Колонки.Добавить("ОбластьВидимостиПолномочия",            ТипСтрока255);
	Полномочия.Колонки.Добавить("ПравоПолученияДоверенностиСДаннымПолномочием", ТипСтрока255);
	Полномочия.Колонки.Добавить("ПравоНаВыдачуДоверенностиСДаннымПолномочием",  ТипСтрока255);
	Полномочия.Колонки.Добавить("КонтекстПолномочия",  Типы().Строка);
	Полномочия.Колонки.Добавить("Правоотношения",      ТипСтрока255);
	Полномочия.Колонки.Добавить("РеквизитыНПА",        ТипСтрока255);
	Полномочия.Колонки.Добавить("Автоключ",            ТипСтрока255);
	...
	Для Каждого ЭлементДанных Из ПолученныеДанные.elements Цикл
		...
		НоваяСтрока.Полномочие          = ЭлементДанных.NAME;
		НоваяСтрока.Родитель            = ЭлементДанных.parent;
		НоваяСтрока.Мнемоника           = ЭлементДанных.MNEMONIC;
		НоваяСтрока.УИДРодителя         = Новый УникальныйИдентификатор(ЭлементДанных.parentUUID);
		НоваяСтрока.УдалитьТипОтношений = ЭлементДанных.type;
		НоваяСтрока.ДатаИзменения       = ДатаИзJsonСПроверкой(ЭлементДанных.lastChangeDate, ФорматДатыJSON.ISO);
		НоваяСтрока.Скрипт              = ЭлементДанных.script;
		НоваяСтрока.ОписаниеПолномочия        = ЭлементДанных.DESCRIPTION;
		НоваяСтрока.ГруппаПодгруппаПолномочия = ЭлементДанных.GROUP;
		НоваяСтрока.МнемоникаСистемыВладельцаПолномочия   = ЭлементДанных.NSI_ID;
		НоваяСтрока.ДатаВключенияПолномочияВКлассификатор = ДатаИзJsonСПроверкой(ЭлементДанных.STARTED, ФорматДатыJSON.ISO);
		НоваяСтрока.ДатаОтзываПолномочия                  = ДатаИзJsonСПроверкой(ЭлементДанных.REVOKED, ФорматДатыJSON.ISO);
		НоваяСтрока.МаксимальныйСрокДействияДоверенности  = ЭлементДанных.EXP;
		НоваяСтрока.ОбластьВидимостиПолномочия            = ЭлементДанных.PWR_VIS;
		НоваяСтрока.ПравоПолученияДоверенностиСДаннымПолномочием = ЭлементДанных.PWR_READ;
		НоваяСтрока.ПравоНаВыдачуДоверенностиСДаннымПолномочием  = ЭлементДанных.PWR_ISS;
		НоваяСтрока.КонтекстПолномочия = ЭлементДанных.CXT;
		НоваяСтрока.Правоотношения     = ЭлементДанных.LEGAL_RELATIONS;
		НоваяСтрока.РеквизитыНПА       = ЭлементДанных.LAW_DETAILS;
		НоваяСтрока.Автоключ           = ЭлементДанных.autokey;
	...
	Возврат Новый Структура("ДатаПоследнегоИзменения, КлассификаторПолномочийМЧД003",
		ДатаПоследнегоИзменения, Полномочия);
...
//
Функция ПолучитьКлассификаторПолномочийФНС_МЧД002(КонтекстДиагностики, ДатаИзмененияФайла,
	ПолучатьНеобработанныеДанные = Ложь) Экспорт
	
	ПараметрыОперации = Новый Структура("Наименование",
		НСтр("ru = 'Получение данных классификатора полномочий ФНС (МЧД 002)';
			|en = 'Receive data from the FTS authority classifier (machine-readable authorization letter 002)'"));
	...
	Если РезультатПолученияДанныхФайла.ДанныеНеИзменялись И ЗначениеЗаполнено(ДатаИзмененияФайла) Тогда
		ИзменитьСостояниеСинхронизации(
			Перечисления.ТипыСинхронизацииСервисаНастроекЭДО.ОбновлениеКлассификатораПолномочийФНСМЧД002,
			ДатаИзмененияФайла);
		Возврат Неопределено;
	КонецЕсли;
	...
	ДатаПоследнегоИзменения = РезультатПолученияДанныхФайла.ДатаИзмененияФайла;
	Результат = ОбработкаРезультатаКлассификаторПолномочийФНС_МЧД002(ПолученныеДанные, ДатаПоследнегоИзменения, Отказ,
		КонтекстДиагностики);
...
// Получает классификатор полномочий (МЧД 003).
// 
// Параметры:
//  КонтекстДиагностики          - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  ДатаИзмененияФайла           - Дата
...
//  - Неопределено - не удалось получить данные.
//  - ДвоичныеДанные - необработанные данные
//  - См. ОбработкаРезультатаКлассификаторПолномочий_МЧД003
//
Функция ПолучитьКлассификаторПолномочий_МЧД003(КонтекстДиагностики, ДатаИзмененияФайла,
	ПолучатьНеобработанныеДанные = Ложь) Экспорт
	
	ПараметрыОперации = Новый Структура("Наименование",
		НСтр("ru = 'Получение данных классификатора полномочий (МЧД 003)';
			|en = 'Get the power classifier data (machine-readable letter of authority 003)'"));
	
	Путь  = "/mchd/rights_classifier/mchd_003/POWER_PWR_CLASS.zip";
	ПараметрыПолучения = ПолучениеФайловИзИнтернетаКлиентСервер.ПараметрыПолученияФайла();
	...
	Если РезультатПолученияДанныхФайла.ДанныеНеИзменялись И ЗначениеЗаполнено(ДатаИзмененияФайла) Тогда
		ИзменитьСостояниеСинхронизации(
			Перечисления.ТипыСинхронизацииСервисаНастроекЭДО.ОбновлениеКлассификатораПолномочийМЧД003,
			ДатаИзмененияФайла);
		Возврат Неопределено;
	КонецЕсли;
	...
	РаспакованныеФайлы = РаботаСФайламиБЭД.РаспаковатьАрхив(РезультатПолученияДанныхФайла.ДанныеФайла,
		КаталогРаспаковки, КонтекстДиагностики, НСтр("ru = 'Распаковка архива данных классификатора полномочий (МЧД 003).';
													|en = 'Unpack the power classifier data archive (machine-readable letter of authority 003).'"));
	
	Если РаспакованныеФайлы = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ИмяФайлаДанныхКлассификатора = "POWER_PWR_CLASS.json";
	ДанныеКлассификатора = Неопределено;
	Для Каждого РаспакованныйФайл Из РаспакованныеФайлы Цикл
		Если НРег(РаспакованныйФайл.Имя) = НРег(ИмяФайлаДанныхКлассификатора) Тогда
			ДанныеКлассификатора = Новый ДвоичныеДанные(РаспакованныйФайл.ПолноеИмя);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Если ПолучатьНеобработанныеДанные Тогда
		Возврат РезультатПолученияДанныхФайла.ДанныеФайла;
	Иначе
		ПолученныеДанные = ОбъектИзJSON(ПараметрыОперации, ДанныеКлассификатора, Отказ, КонтекстДиагностики);
	КонецЕсли;
	...
	Результат = ОбработкаРезультатаКлассификаторПолномочий_МЧД003(ПолученныеДанные, ДатаПоследнегоИзменения, Отказ,
		КонтекстДиагностики);
...
Функция ШаблонСоглашенияНаРоуминг(ИдентификаторОператора) Экспорт
	...
	ВидОперации = НСтр("ru = 'Получение шаблона соглашения на роуминг для электронного документооборота';
						|en = 'Receive the roaming agreement template for EDI'");
	РезультатПолученияДанныхФайла = ДанныеФайлаОблачногоХранилищаНастроек(ПутьКШаблону, ВидОперации);
...
#Область Версия_1_9_1
	...
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: обновление операторов ЭДО.';
									|en = '1C:Exchange with counterparties: update EDI service providers.'");
	...
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: обновление операторов ЭДО (начальное заполнение).';
									|en = '1C:Exchange with counterparties: update EDI service providers (initial filling).'");
	...
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: обновление совместимостей операторов ЭДО.';
									|en = '1C:Exchange with counterparties: update EDI service provider compatibility.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	...
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: обновление совместимостей операторов ЭДО (начальное заполнение).';
									|en = '1C:Exchange with counterparties: update EDI service provider compatibility (initial filling).'");
...
//  см. РезультатПолученияДанныхФайлаОблачногоХранилищаНастроек
//
Функция ДанныеФайлаОблачногоХранилищаНастроек(ПутьКФайлу, ВидОперации = "", ПараметрыПолучения = Неопределено,
	...
	РезультатПолученияФайла = ПолучениеФайловИзИнтернета.СкачатьФайлВоВременноеХранилище(
		ПолныйАдрес, ПараметрыПолучения, Не ЗначениеЗаполнено(ВидОперации));
	
	РезультатПолученияДанныхФайла = РезультатПолученияДанныхФайлаОблачногоХранилищаНастроек();
	Если НЕ РезультатПолученияФайла.Статус Тогда
		Если РезультатПолученияФайла.КодСостояния = 304 Тогда
			РезультатПолученияДанныхФайла.ДанныеНеИзменялись = Истина;
			Возврат РезультатПолученияДанныхФайла;
		Иначе
			ТекстОшибки = НСтр("ru = 'Ошибка при получении данных из облачного хранилища.';
								|en = 'Error when retrieving data from cloud storage.'");
			ПодробныйТекстОшибки = ТекстОшибки + Символы.ПС + ПолныйАдрес + Символы.ПС
				+ РезультатПолученияФайла.СообщениеОбОшибке;
			ВидОперации = ?(ВидОперации = "", НСтр("ru = 'Получение данных из облачного хранилища';
													|en = 'Retrieving data from cloud storage'"), ВидОперации);
	...
	РезультатПолученияДанныхФайла.ДанныеФайла = ПолучитьИзВременногоХранилища(РезультатПолученияФайла.Путь);
	УдалитьИзВременногоХранилища(РезультатПолученияФайла.Путь);
	
	ДатаИзмененияФайлаRFC1123 = РезультатПолученияФайла.Заголовки["Last-Modified"];
	Если ЗначениеЗаполнено(ДатаИзмененияФайлаRFC1123) Тогда
		РезультатПолученияДанныхФайла.ДатаИзмененияФайла =
			ОбщегоНазначенияКлиентСервер.ДатаRFC1123(ДатаИзмененияФайлаRFC1123);
	КонецЕсли;
...
// Возвращаемое значение:
//  Структура - Результат получения данных файла облачного хранилища настроек:
// * ДанныеФайла        - Неопределено
// * ДатаИзмененияФайла - Дата
// * ДанныеНеИзменялись - Булево - сервис возвращает ответ, что данные не изменились
Функция РезультатПолученияДанныхФайлаОблачногоХранилищаНастроек()
	
	Результат = Новый Структура;
	Результат.Вставить("ДанныеФайла", Неопределено);
	Результат.Вставить("ДатаИзмененияФайла", Дата(1,1,1));
	Результат.Вставить("ДанныеНеИзменялись", Ложь);
...
Функция ПараметрыКомандыПолученияДанныхАбонентовВСервисНастроек(ПараметрыЗапроса)
	...
	ПараметрыКоманды.Наименование        = НСтр("ru = 'Получение данных абонента в сервисе 1С:ЭДО';
												|en = 'Receive the subscriber data in 1C:EDW service'");
	ПараметрыКоманды.Метод               = ИнтернетСоединениеБЭД.HTTPМетоды().POST;
...
Функция ПараметрыКомандыПолученияДанныхОрганизацийСОбязательнойОтправкойЗаявкиНаРоуминг(НаборОтборовПоКонтрагентам, Отказ)
	...
	ПараметрыКоманды.Наименование = НСтр("ru = 'Получение данных абонента с принудительной отправкой приглашения';
										|en = 'Receive subscriber data with forced sending of the invitation'");
	ПараметрыКоманды.Метод = ИнтернетСоединениеБЭД.HTTPМетоды().POST;
...
Функция ПараметрыПолученияДанныхФорматовЭДО(ПараметрыЗапроса)
	...
	ПараметрыКоманды.Наименование        = НСтр("ru = 'Получение данных о форматах ЭДО из сервиса настроек 1С:ЭДО';
												|en = 'Receive data about EDI formats from 1C:EDI settings service'");
	ПараметрыКоманды.Метод               = ИнтернетСоединениеБЭД.HTTPМетоды().POST;
...
Функция ПараметрыПолученияСвязиТиповИФорматовЭДО(ПараметрыЗапроса)
	...
	ПараметрыКоманды.Наименование        = НСтр("ru = 'Получение данных о связи типов и форматов ЭД из сервиса настроек 1С:ЭДО';
												|en = 'Get the data about connection of ED types and formats from 1C:EDW setting service'");
	ПараметрыКоманды.Метод               = ИнтернетСоединениеБЭД.HTTPМетоды().POST;
...
Функция ПараметрыПолученияВидовЭДСервиса(ПараметрыЗапроса)
	...
	ПараметрыКоманды.Наименование        = НСтр("ru = 'Получение данных видов ЭД из сервиса настроек 1С:ЭДО';
												|en = 'Get the data of ED types from 1C:EDW setting service'");
	ПараметрыКоманды.Метод               = ИнтернетСоединениеБЭД.HTTPМетоды().POST;
...
Функция ПараметрыКомандыОтправкаДанныхАбонентаВСервисНастроек(ПараметрыЗапроса, Отказ,
	...
	ПараметрыКоманды.Наименование        = НСтр("ru = 'Отправка данных абонента в сервис 1С:ЭДО';
												|en = 'Sending subscriber data to 1C:EDI service'");
	ПараметрыКоманды.Метод               = ИнтернетСоединениеБЭД.HTTPМетоды().POST;
	ПараметрыКоманды.ОбработкаРезультата = Неопределено;
	ПараметрыКоманды.Данные              = ТекстJSON;
	ПараметрыКоманды.Заголовки.Вставить("Content-Type", "application/json");
...
Функция ВыполнитьКомандуСервиса(ПараметрыКоманды, Отказ, КонтекстДиагностики)
	...
	ВидОперации = НСтр("ru = 'Выполнение запроса к сервису настроек обмена электронными документами';
						|en = 'Run the query for the service of electronic document exchange settings'");
	РезультатВызова = 
...
Функция ОбработатьРезультатОтветаСервиса(Знач ПараметрыКоманды, Ответ, Отказ, КонтекстДиагностики)
	...
	Если ПараметрыКоманды.Ошибки.Получить(КодСостояния) = Неопределено
		...
		ПараметрыКоманды.Ошибки.Вставить(КодСостояния, СтрШаблон(НСтр("ru = 'Внутренняя ошибка сервиса: %1';
																		|en = 'Internal service error: %1'"), ТелоОтвета));
	КонецЕсли;
			...
			Если ЗначениеЗаполнено(ТелоОтвета) Тогда
				...
				Исключение
					ЗаписьЖурналаРегистрации(
						НСтр("ru = 'Преобразование ошибки сервиса в структуру';
							|en = 'Convert service error to structure'", ОбщегоНазначения.КодОсновногоЯзыка()),
						УровеньЖурналаРегистрации.Ошибка,,, НСтр("ru = 'Не удалось преобразовать ошибку сервиса:';
																|en = 'Failed to convert the service error:'") + ТелоОтвета);
				КонецПопытки;
			КонецЕсли;
...
Функция ОбъектИзJSON(ПараметрыКоманды, Знач Данные, Отказ, КонтекстДиагностики = Неопределено,
	ПрочитатьВСоответствие = Ложь)
	
	Попытка
		ПолученныеДанные = ОбщегоНазначенияБЭД.JSONЗначение(Данные, ПрочитатьВСоответствие);
	Исключение
		
		Отказ = Истина;
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		
		ТекстСообщения = НСтр("ru = 'Не удалось прочитать ответ от сервиса по причине:';
								|en = 'Cannot read service answer due to:'") + Символы.ПС + ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
			
		Если ПараметрыКоманды = Неопределено Тогда
			НаименованиеКоманды = НСтр("ru = 'Чтение данных сервиса настроек';
										|en = 'Read settings service data'") ;
		Иначе
...
Функция ВидОшибкиНетПравНаИзменениеИнформацииПоУчетнойЗаписи()
	...
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Нет прав на изменение информации по учетной записи';
										|en = 'Insufficient rights to change account information'");
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Идентификатор ЭДО привязан к логину интернет-поддержки, отличному от текущего';
										|en = 'EDI ID is bound to online support usename, which is different from the current one'");
	ОписаниеРешения = НСтр("ru = 'Способ устранения описан <a href = ""http://1c-edo.ru/handbook/24/4358/"">здесь</a>';
							|en = 'Removal method is described <a href = ""http://1c-edo.ru/handbook/24/4358/"">here</a>'");
