...
Процедура ПриАктивизацииРезультатаОтчета(Форма, Элемент) Экспорт 
	...
	Если Элементы.КонтекстноеМенюОбластиЗаголовка.Видимость Тогда 
		...
		ЗаголовокКнопки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Сгруппировать по полю %1';
				|en = 'Group by the %1 field'"), СвойстваЗаголовка.Текст);
...
Процедура УстановитьВысотуСтроки(Форма, Команда, СвойстваЗаголовка = Неопределено,
	...
	Если ВысотаСтроки = Неопределено Тогда 
		...
		ПоказатьВводЧисла(Обработчик, ПараметрыВысотыСтроки.Размер, НСтр("ru = 'Высота строки';
																		|en = 'Row height'"), 5);
...
Процедура УстановитьШиринуКолонки(Форма, Команда, СвойстваЗаголовка = Неопределено,
	...
	Если ШиринаКолонки = Неопределено Тогда 
		...
		ПоказатьВводЧисла(Обработчик, ПараметрыШириныКолонки.Размер, НСтр("ru = 'Ширина колонки';
																			|en = 'Column width'"), 5);
...
Процедура СнятьФильтр(Форма, СвойстваЗаголовка, ДанныеРасшифровки = Неопределено) Экспорт 
	...
	Если Фильтр = Неопределено Тогда 
		...
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'По полю ""%1"" отсутствуют фильтры';
				|en = 'No filters by the ""%1"" field'"), СвойстваЗаголовка.Текст);
...
Функция КонтекстноеМенюОбластиЗаголовка()
	...
	КонтекстноеМеню.Добавить("СнятьФильтр", НСтр("ru = 'Снять фильтр';
												|en = 'Clear filter'"));
	КонтекстноеМеню.Добавить("Фильтровать", НСтр("ru = 'Фильтровать...';
												|en = 'Filter…'"),, БиблиотекаКартинок.ОтборКомпоновкиДанных);
	
	// Сортировать
	КонтекстноеМеню.Добавить(НаправлениеСортировкиКомпоновкиДанных.Возр, НСтр("ru = 'Сортировать по возрастанию';
																				|en = 'Sort ascending'"),, БиблиотекаКартинок.СортироватьСтрокиПоВозрастанию);
	КонтекстноеМеню.Добавить(НаправлениеСортировкиКомпоновкиДанных.Убыв, НСтр("ru = 'Сортировать по убыванию';
																				|en = 'Sort descending'"),, БиблиотекаКартинок.СортироватьСтрокиПоУбыванию);
	
	// Оформить
	ПодменюОформления = Новый СписокЗначений;
	ПодменюОформления.Добавить("УстановитьВысотуСтроки", НСтр("ru = 'Высота строки...';
																|en = 'Row height…'"),, БиблиотекаКартинок.ВысотаСтроки);
	ПодменюОформления.Добавить("УстановитьШиринуКолонки", НСтр("ru = 'Ширина колонки...';
																|en = 'Column width…'"),, БиблиотекаКартинок.ШиринаКолонки);
	
	КонтекстноеМеню.Добавить(ПодменюОформления, НСтр("ru = 'Оформить';
													|en = 'Format'"),, БиблиотекаКартинок.УсловноеОформлениеКомпоновкиДанных);
...
Функция КонтекстноеМенюОбластиДанных(СвойстваЗаголовка, ДоступныеВидыСравнения)
	...
	КонтекстноеМеню.Добавить(ДоступныеВидыСравнения, НСтр("ru = 'Фильтровать';
															|en = 'Filter'"),, БиблиотекаКартинок.ОтборКомпоновкиДанных);
	...
	ПодменюОформления.Добавить("ОчиститьОформление", НСтр("ru = 'Очистить оформление';
															|en = 'Clear appearance'"));
	ПодменюОформления.Добавить("ОформитьКрасным", НСтр("ru = 'Красным';
														|en = 'Red'"),, КартинкиОформления.ОформитьКрасным);
	ПодменюОформления.Добавить("ОформитьЖелтым", НСтр("ru = 'Желтым';
														|en = 'Yellow'"),, КартинкиОформления.ОформитьЖелтым);
	ПодменюОформления.Добавить("ОформитьЗеленым", НСтр("ru = 'Зеленым';
														|en = 'Green'"),, КартинкиОформления.ОформитьЗеленым);
	...
	Если СвойстваОбласти.ЭтоЗаголовок Тогда 
		...
		Если ТипЗнч(ВыполненноеДействие) = Тип("Строка")
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Действие ""%1"" недоступно';
					|en = 'Action ""%1"" is unavailable'"), Меню.НайтиПоЗначению(ВыполненноеДействие));
...
Процедура ОткрытьФормуОтчета(Форма, ПараметрыОткрытия)
	...
	ПредставлениеВарианта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1 (Расшифровка)';
			|en = '%1 (Details)'"), Форма.ПредставлениеТекущегоВарианта);
...
Функция ДействиеНадПолемДоступно(Действие, СвойстваЗаголовка)
	...
	Если Не СвойстваЗаголовка[Действие] Тогда 
		...
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Действие недоступно для поля %1';
				|en = 'Action is unavailable for the %1 field'"), СвойстваЗаголовка.Текст);
