...
Функция СтандартныеРасписанияДляНапоминания()
	...
	Результат.Вставить(НСтр("ru = 'по понедельникам, в 9:00';
							|en = 'on Mondays at 9.00 AM'"), Расписание);
	...
	Расписание.ДниНедели = ДниНедели;
	Результат.Вставить(НСтр("ru = 'по пятницам, в 15:00';
							|en = 'on Fridays at 3.00 PM'"), Расписание);
	...
	Расписание.ПериодНедель = 1;
	Расписание.ВремяНачала = '00010101090000';
	Результат.Вставить(НСтр("ru = 'каждый день, в 9:00';
							|en = 'every day at 9:00 AM'"), Расписание);
...
Процедура АктуализироватьСписокНапоминаний(ПриЗапуске)
	...
	Для Каждого Напоминание Из СписокНапоминаний Цикл
		...
		Если ТребуетсяПереподключитьПоРасписанию Тогда
			...
			Исключение
				ОтменитьТранзакцию();
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Напоминания пользователя';
												|en = 'User reminders'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПриСозданииНаСервере(Форма, ПараметрыРазмещения) Экспорт
	...
	Если Форма.Элементы.Найти(ИмяГруппыЭлементов) = Неопределено Тогда
		...
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(ИмяПоляНапоминатьОСобытии,
			Новый ОписаниеТипов("Булево"), , НСтр("ru = 'Напомнить';
													|en = 'Remind'"), Истина));
			
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(ИмяПоляИнтервалВремениНапоминания,
			Новый ОписаниеТипов("Строка"), ,  НСтр("ru = 'Интервал времени напоминания';
													|en = 'Reminder interval'"), Истина));
		...
		Если НастройкиНапоминания.ДобавлятьФлажок Тогда
			...
			Иначе
				Форма[ИмяПоляИнтервалВремениНапоминания] = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'за %1';
							|en = '%1 before'"), ПредставлениеВремени(НастройкиНапоминания.ИнтервалВремениНапоминания, , Ложь));
			КонецЕсли;
		Иначе
