...
Функция ОписаниеФиксируемогоРеквизита() Экспорт
	...
	ФиксацияРеквизита.Вставить("СтрокаПредупреждения",     НСтр("ru = 'Эти данные заполнены автоматически';
																|en = 'This data is populated automatically'"));
...
//   БезСимволовПереноса - Булево - если Истина сравнение значений строковых реквизитов будет проведено без учета регистра, символов переноса и пробелов
//       См. СтроковыеФункцииБЗККлиентСервер.УбратьПробелы()
//
Функция ОбновитьДанныеШапки(НовыеЗначенияРеквизитов, Объект, ПараметрыФиксацииВторичныхДанных = Неопределено, БезСимволовПереноса = Ложь) Экспорт
	...
	Для Каждого КлючИЗначение Из НовыеЗначенияРеквизитов Цикл
		...
		Если БезСимволовПереноса И ТипЗнч(Объект[ИмяРеквизита]) = Тип("Строка") Тогда
			ЗначениеИзменено = СтроковыеФункцииБЗККлиентСервер.УбратьПробелы(ВРег(СтароеЗначение), Ложь) 
				<> СтроковыеФункцииБЗККлиентСервер.УбратьПробелы(ВРег(Объект[ИмяРеквизита]), Ложь);
		Иначе
			ЗначениеИзменено = СтароеЗначение <> Объект[ИмяРеквизита]; // Приведенное значение сравнивается со старым значением.
		КонецЕсли;
