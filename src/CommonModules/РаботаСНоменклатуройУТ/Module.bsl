	...
	Если ДанныеВидаНоменклатуры.ТипНоменклатуры <> ТипНоменклатурыСервиса Тогда
		РаботаСНоменклатурой.ДобавитьСтрокуВТаблицуОтличий(ТаблицаОтличийРеквизитов, 
			...
			НСтр("ru = 'Тип номенклатуры';
				|en = 'Item type'"),,НЕ ЗначениеЗаполнено(ДанныеВидаНоменклатуры.ТипНоменклатуры));
	КонецЕсли; 
КонецПроцедуры
...
Процедура ПоискНоменклатурыПоШтрихкодуПриСозданииНаСервере(Форма) Экспорт
	...
	НовыйЭлемент.Заголовок   = НСтр("ru = 'Алкогольная продукция';
									|en = 'Alcohol containing products'");
	...
	Если Выборка.Следующий() Тогда // Штрихкод уже записан в БД
		...
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Номенклатура%", """" + Выборка.НоменклатураПредставление + """"
			...
			+ ?(ЗначениеЗаполнено(Выборка.ЕдиницаИзмерения), " """ + НСтр("ru = 'с единицей измерения';
																			|en = 'with Unit'") + " " + Выборка.ЕдиницаИзмеренияПредставление + """", ""));
...
Процедура ИнициализацияЗапросаВыборкиДанныхДляВыгрузки(Запрос) Экспорт
	...
	|			ТОГДА &НДС20
	|		КОГДА СправочникНоменклатура.СтавкаНДС.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5)
	|			ТОГДА ""5""
	|		КОГДА СправочникНоменклатура.СтавкаНДС.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5_105)
	|			ТОГДА ""5""
	|		КОГДА СправочникНоменклатура.СтавкаНДС.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
	|			ТОГДА ""7""
	|		КОГДА СправочникНоменклатура.СтавкаНДС.ПеречислениеСтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7_107)
	|			ТОГДА ""7""
...
Процедура ДобавитьИдентификаторыВетИС(Запрос)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ 
	|	ЕСТЬNULL(ПроизводителиДляВЕТИС.Производитель.Идентификатор, """") КАК Идентификатор,
	|	ПроизводителиДляВЕТИС.Ссылка
	|ПОМЕСТИТЬ ПроизводителиВЕТИС	
	|ИЗ
	|	ПродукцияВЕТИС КАК ПродукцияВЕТИС
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПродукцияВЕТИС.Производители КАК ПроизводителиДляВЕТИС
	|		ПО ПродукцияВЕТИС.Продукция = ПроизводителиДляВЕТИС.Ссылка 
	|
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ЕСТЬNULL(СправочникПродукцияВЕТИС.ТипПродукции.Идентификатор, """") 					КАК ИдентификаторТипПродукции,
	|	ЕСТЬNULL(СправочникПродукцияВЕТИС.Продукция.Идентификатор, """") 						КАК ИдентификаторПродукция,	
	|	ЕСТЬNULL(СправочникПродукцияВЕТИС.ВидПродукции.Идентификатор, """") 					КАК ИдентификаторВидПродукции,
	|	ЕСТЬNULL(СправочникПродукцияВЕТИС.ХозяйствующийСубъектПроизводительИдентификатор, """") КАК ХСПроизводитель,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПроизводителиВЕТИС КАК Производитель
	...
	Если Запрос.Параметры.Свойство("Организация") или Запрос.Параметры.Свойство("СписокСостояний") Тогда
		...
		Пока Выборка.Следующий() Цикл
			Если ЗначениеЗаполнено(Выборка.ИдентификаторПродукция) Тогда		
				...
				ИдентификаторТипПродукции = ПродукцияВЕТИСВызовСервера.ТипПродукции(Выборка.ТипПродукции);			
