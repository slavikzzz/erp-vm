...
Процедура МодификацияЭлементовФормы(Форма) Экспорт
	...
	Для Каждого ВидПродукции Из ПараметрыИнтеграции.ВидыПродукции Цикл
		...
		Для Каждого Настройки Из НастройкиМассивом Цикл
			...
			ТребуетсяКонтрольМарок = ОбщегоНазначенияИСМППовтИсп.ДействуетУведомительныйРежимПоВидуПродукции(ВидПродукции)
				Или ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПриПродажеОптомПоОСУ(ВидПродукции)
				Или ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПриПродажеОптомПоштучно(ВидПродукции);
...
Процедура ДобавитьНастройкиРазделенияДатИСМП(Форма, ПараметрыИнтеграции, ДобавляемыеРеквизиты) Экспорт
	...
	ПараметрыИнтеграцииФормыПроверкиИПодбора = ПараметрыИнтеграцииФормыПроверкиИПодбора(Форма);
...
Процедура ЗаполнитьПризнакАвтоматическийОСУИСВСтроке(СтрокаТовары, ПараметрыСканирования = Неопределено, НаДату = Неопределено) Экспорт
	...
	ВидПродукцииПоддерживаетОСУ    = ОбщегоНазначенияИСКлиентСерверПовтИсп.ВидПродукцииПоддерживаетОбъемноСортовойУчет(СтрокаТовары.ВидПродукцииИС, ДокументВнутреннегоПеремещения);
	...
	ВидОперацииИСМП = ?(ПараметрыСканирования = Неопределено,
		ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ВидОперацииОптоваяПродажа(),
...
Процедура ЗаполнитьПризнакАвтоматическийОСУИСВТаблице(ТаблицаТовары, ЗаполнитьКолонкуGTIN = Ложь, ПараметрыСканирования = Неопределено, НаДату = Неопределено) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаТовары Цикл
		...
		ВидПродукцииПоддерживаетОСУ    = ОбщегоНазначенияИСКлиентСерверПовтИсп.ВидПродукцииПоддерживаетОбъемноСортовойУчет(СтрокаТаблицы.ВидПродукцииИС, ДокументВнутреннегоПеремещения);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаТовары Цикл
		...
		Если ДокументВнутреннегоПеремещения Тогда
			ДействуетОСУПоДатам = Истина;
		Иначе
			ДействуетОСУПоДатам = ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПриПродажеОптомПоОСУ(СтрокаТаблицы.ВидПродукцииИС)
				И Не ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПриПродажеОптомПоштучно(СтрокаТаблицы.ВидПродукцииИС)
				И ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ЭтоВидОперацииОптоваяПродажа(ВидОперацииИСМП);
		КонецЕсли;
...
Процедура ЗаполнитьПризнакиКонтроляОборотаМарокПоСтроке(СтрокаТаблицы, ПараметрыСканирования, НаДату = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(СтрокаТаблицы.ВидПродукцииИС)
		Или ОбщегоНазначенияИСКлиентСервер.ВидыПродукцииИСМП(Истина, Истина, Истина).Найти(СтрокаТаблицы.ВидПродукцииИС) = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьПризнакиКонтроляОборотаМарок(Форма, ТабличнаяЧасть) Экспорт
	...
	Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
		...
		СтруктураОтбора = Новый Структура("ВидПродукцииИС", СтрокаТабличнойЧасти.ВидПродукцииИС);
		...
		Иначе
			...
			ДействуетКонтрольОборотаМарок = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ДействуетКонтрольПоРазделеннымДатам(ПараметрыСканирования.ВидОперацииИСМП,
				...
				ПараметрыСканирования.ДатаДокумента);
...
Функция ПараметрыИнтеграцииФормыПроверкиИПодбора(Форма = Неопределено, ВидПродукции = Неопределено) Экспорт
	...
	Иначе
		ВидПродукцииРодительный = НСтр("ru = 'продукции';
										|en = 'продукции'");
		ВидПродукцииВинительный = НСтр("ru = 'продукцию';
										|en = 'продукцию'");
	КонецЕсли;
	...
	ПараметрыИнтеграции.Заголовки[0] = СтрШаблон(НСтр("ru = 'Проверить поступившую %1';
														|en = 'Проверить поступившую %1'"),             ВидПродукцииВинительный);
	ПараметрыИнтеграции.Заголовки[1] = СтрШаблон(НСтр("ru = 'Подобрать и проверить %1';
														|en = 'Подобрать и проверить %1'"),             ВидПродукцииВинительный);
	ПараметрыИнтеграции.Заголовки[2] = СтрШаблон(НСтр("ru = 'Продолжить проверку поступившей %1';
														|en = 'Продолжить проверку поступившей %1'"),   ВидПродукцииРодительный);
	ПараметрыИнтеграции.Заголовки[3] = СтрШаблон(НСтр("ru = 'Продолжить подбор и проверку %1';
														|en = 'Продолжить подбор и проверку %1'"),      ВидПродукцииРодительный);
	ПараметрыИнтеграции.Заголовки[4] = СтрШаблон(НСтр("ru = 'Результаты проверки %1';
														|en = 'Результаты проверки %1'"),               ВидПродукцииРодительный);
	ПараметрыИнтеграции.Заголовки[5] = СтрШаблон(НСтр("ru = 'Результаты подбора %1';
														|en = 'Результаты подбора %1'"),                ВидПродукцииРодительный);
	ПараметрыИнтеграции.Заголовки[6] = СтрШаблон(НСтр("ru = 'Промежуточные результаты проверки %1';
														|en = 'Промежуточные результаты проверки %1'"), ВидПродукцииРодительный);
	ПараметрыИнтеграции.Заголовки[7] = СтрШаблон(НСтр("ru = 'Промежуточные результаты подбора %1';
														|en = 'Промежуточные результаты подбора %1'"),  ВидПродукцииРодительный);
	ПараметрыИнтеграции.Заголовки[8] = СтрШаблон(НСтр("ru = 'Возобновить проверку %1';
														|en = 'Возобновить проверку %1'"),              ВидПродукцииРодительный);
	...
	Если ИнтеграцияИСКлиентСервер.ПредставлениеВидаПродукцииВоМножественномЧисле(ВидПродукции) Тогда
		ПараметрыИнтеграции.Заголовки[0] = СтрШаблон(НСтр("ru = 'Проверить поступившие %1';
															|en = 'Проверить поступившие %1'"),           ВидПродукцииВинительный);
		ПараметрыИнтеграции.Заголовки[2] = СтрШаблон(НСтр("ru = 'Продолжить проверку поступивших %1';
															|en = 'Продолжить проверку поступивших %1'"), ВидПродукцииРодительный);
	ИначеЕсли ОбщегоНазначенияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ВидПродукции)
		...
		ПараметрыИнтеграции.Заголовки[0] = СтрШаблон(НСтр("ru = 'Проверить поступившее %1';
															|en = 'Проверить поступившее %1'"),            ВидПродукцииВинительный);
		ПараметрыИнтеграции.Заголовки[2] = СтрШаблон(НСтр("ru = 'Продолжить проверку поступившего %1';
															|en = 'Продолжить проверку поступившего %1'"), ВидПродукцииРодительный);
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуДатКонтролейМаркировкиГосИС(Форма) Экспорт
	...
	МассивДатНастроек = РегистрыСведений.НастройкиУчетаМаркируемойПродукцииИСМП.ДатыРаздельногоКонтроля();
	
	Для Каждого НастройкаДат Из МассивДатНастроек Цикл
	
		НоваяСтрока = Форма.ТаблицаКонтролейДатМаркировкиГосИС.Добавить();
		
		НоваяСтрока.ВидПродукцииИС = НастройкаДат.ВидПродукции;
...
Функция ТаблицаМаркируемойПродукцииДокумента(Контекст, ВидМаркируемойПродукции, Сценарий = 0, ПараметрыСканирования = Неопределено) Экспорт
	...
	Иначе
		...
		Если МетаданныеДокумента.Имя = "ПеремещениеМеждуМОДИСМП" Тогда
		...
		ИначеЕсли ИнтеграцияИС.СодержитсяВПодсистеме("ГосИС.ИСМП", МетаданныеДокумента) Тогда
			МенеджерДокумента = ОбщегоНазначенияИС.МенеджерОбъектаПоПолномуИмени(МетаданныеДокумента.ПолноеИмя());
...
Функция ВидПродукцииРодительный(ВидПродукции) Экспорт
	...
	Если ВидПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукцияПодконтрольнаяВЕТИС Тогда
		Возврат НСтр("ru = 'молока и молочной продукции (ВетИС)';
					|en = 'молока и молочной продукции (ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукцияБезВЕТИС Тогда
		Возврат НСтр("ru = 'молока и молочной продукции (неподконтрольной ВетИС)';
					|en = 'молока и молочной продукции (неподконтрольной ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхПодконтрольныеВЕТИС Тогда
		Возврат НСтр("ru = 'сухих кормов для животных (ВетИС)';
					|en = 'сухих кормов для животных (ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхБезВЕТИС Тогда
		Возврат НСтр("ru = 'сухих кормов для животных (неподконтрольных ВетИС)';
					|en = 'сухих кормов для животных (неподконтрольных ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхВлажныеПодконтрольныеВЕТИС Тогда
		Возврат НСтр("ru = 'влажных кормов для животных (ВетИС)';
					|en = 'влажных кормов для животных (ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхВлажныеБезВЕТИС Тогда
		Возврат НСтр("ru = 'влажных кормов для животных (неподконтрольных ВетИС)';
					|en = 'влажных кормов для животных (неподконтрольных ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.КонсервированнаяПродукцияПодконтрольнаяВЕТИС Тогда
		Возврат НСтр("ru = 'консервированной продукции (ВетИС)';
					|en = 'консервированной продукции (ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.КонсервированнаяПродукцияБезВЕТИС Тогда
		Возврат НСтр("ru = 'консервированной продукции (неподконтрольной ВетИС)';
					|en = 'консервированной продукции (неподконтрольной ВетИС)'");
...
Функция ВидПродукцииВинительный(ВидПродукции) Экспорт
	Если ВидПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукцияПодконтрольнаяВЕТИС Тогда
		Возврат НСтр("ru = 'молоко и молочную продукцию (ВетИС)';
					|en = 'молоко и молочную продукцию (ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукцияБезВЕТИС Тогда
		Возврат НСтр("ru = 'молоко и молочную продукцию (неподконтрольную ВетИС)';
					|en = 'молоко и молочную продукцию (неподконтрольную ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхПодконтрольныеВЕТИС Тогда
		Возврат НСтр("ru = 'сухие корма для животных (ВетИС)';
					|en = 'сухие корма для животных (ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхБезВЕТИС Тогда
		Возврат НСтр("ru = 'сухие корма для животных (неподконтрольные ВетИС)';
					|en = 'сухие корма для животных (неподконтрольные ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхВлажныеПодконтрольныеВЕТИС Тогда
		Возврат НСтр("ru = 'влажные корма для животных (ВетИС)';
					|en = 'влажные корма для животных (ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.КормаДляЖивотныхВлажныеБезВЕТИС Тогда
		Возврат НСтр("ru = 'влажные корма для животных (неподконтрольные ВетИС)';
					|en = 'влажные корма для животных (неподконтрольные ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.КонсервированнаяПродукцияПодконтрольнаяВЕТИС Тогда
		Возврат НСтр("ru = 'консервированную продукции (ВетИС)';
					|en = 'консервированную продукции (ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.КонсервированнаяПродукцияБезВЕТИС Тогда
		Возврат НСтр("ru = 'консервированную продукции (неподконтрольную ВетИС)';
					|en = 'консервированную продукции (неподконтрольную ВетИС)'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.БАДы Тогда
		Возврат НСтр("ru = 'БАДы';
					|en = 'БАДы'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.БАДы2025 Тогда
		Возврат НСтр("ru = 'БАДы 2025';
					|en = 'БАДы 2025'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.МедицинскиеИзделия20 Тогда
		Возврат НСтр("ru = 'медицинские изделия 2.0';
					|en = 'медицинские изделия 2.0'");
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.Духи Тогда
		Возврат НСтр("ru = 'духи и туалетную воду';
					|en = 'духи и туалетную воду'");
	КонецЕсли;
...
Процедура ПриОпределенииПараметровИнтеграцииФормыПроверкиИПодбора(Форма, ПараметрыИнтеграции, ВидПродукции)
	...
	ИначеЕсли Форма.ИмяФормы = "Документ.ОтчетОПроизводствеЕГАИС.Форма.ФормаДокумента" Тогда
		...
		ПараметрыИнтеграции.ЕстьПравоИзменение                         = ПравоДоступа("Изменение", Форма.Объект.Ссылка.Метаданные());
		ПараметрыИнтеграции.ИмяТабличнойЧастиСерии                     = Неопределено;
	ИначеЕсли Форма.ИмяФормы = "Документ.ПеремещениеМеждуМОДИСМП.Форма.ФормаДокумента" Тогда
		ПараметрыИнтеграции.ИмяРодительскойГруппыФормы                 = "ГруппаТоварыСписком";
		ПараметрыИнтеграции.РазмещатьЭлементыИнтерфейса                = Истина;
		ПараметрыИнтеграции.ИспользоватьБезМаркируемойПродукции        = Истина;
		ПараметрыИнтеграции.БлокироватьТабличнуюЧастьТоварыПриПроверке = Ложь;
...
Процедура ДополнитьТаблицуПодобраннойМаркируемойПродукции(ТаблицаМаркируемойПродукции, ДанныеДокумента, ВидПродукции, ПараметрыСканирования) Экспорт
	...
	ДокументВнутреннегоПеремещения = ОбщегоНазначенияИСВызовСервера.ЭтоДокументВнутреннегоПеремещения(ДанныеДокумента.ПроверяемыйДокумент);
	ВидПродукцииПоддерживаетОСУ    = ОбщегоНазначенияИСКлиентСерверПовтИсп.ВидПродукцииПоддерживаетОбъемноСортовойУчет(ВидПродукции, ДокументВнутреннегоПеремещения);
