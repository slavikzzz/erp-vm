...
Функция ПозицииНоменклатурыПоДокументу(ДокументСсылка) Экспорт
	...
	Если МодульЛокализации = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не настроен модуль локализации по документу.';
								|en = 'Localization module for the document is not configured.'"); 
	Иначе
...
Процедура ОбновитьПараметрыФискальногоЧекаСБП(ПараметрыФискальногоЧека, ОплатыСБП) Экспорт
	...
	Если ОплатыСБП.Количество() > 0 Тогда
		...
		ПараметрыФискальногоЧека.QRКод.ТекстПользователя = НСтр("ru = 'Сохраните чек для возврата';
																|en = 'Save the receipt for refund'");
	КонецЕсли;
	//-- Локализация
КонецПроцедуры
...
Функция ПараметрыФискальныхСтрокЧека(ПредметыРасчетов, ПараметрыФискальногоЧека)
	...
	Для Каждого СтрокаДляДобавленияВЧек Из ПредметыРасчетов Цикл
		...
		Если ОплатаЭлектроннымиСертификатамиФСС И Не ПустаяСтрока(СтрокаПозицииЧека.NTIN) Тогда
			СтрокаТекста = СтрШаблон(НСтр("ru = 'ТРУ (КОД): %1';
											|en = 'Goods/works/services (CODE): %1'"), СтрокаПозицииЧека.NTIN);
			ТекстоваяСтрокаЧека =
		...
		Если ТипРасчетаДенежнымиСредствами = Перечисления.ТипыРасчетаДенежнымиСредствами.ПриходДенежныхСредств
			...
			Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств")
				...
				НаименованиеОплаты = НСтр("ru = 'Оплата на р/с от:';
											|en = 'Payment to the account from:'");
...
Процедура ОбновитьПараметрыФискальногоЧекаПозицийЧекаПоДокументамОплатыВсяСуммаОднойПозициейНеАвансДляПриведенияКФормату(ПараметрыФискальногоЧека, ПризнакСпособаРасчета, ПозицииЧека, СтавкаНДС, СуммаОплаты)
	...
	Иначе
		НаименованиеОплаты = НСтр("ru = 'Выдано:';
									|en = 'Issued:'") + " " + ПолучательНаименование;
	КонецЕсли;
