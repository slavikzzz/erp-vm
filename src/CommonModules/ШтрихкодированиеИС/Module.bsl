...
Процедура ОбработатьШтрихкодыУпаковокПоДаннымБазы(ПараметрыСканирования, ДанныеПоШтрихкодам, ШтрихкодыМаркируемойПродукции, ТипыУпаковок, ЗаполнитьПоСсылкам) Экспорт
	...
	Если ШтрихкодированиеОбщегоНазначенияИС.ДопустимаТабачнаяПродукция(ПараметрыСканирования)
		...
		СканированиеФормыПроверкиИПодбора = МодульШтрихкодированиеИСМП.ЭтоСканированиеВФормеПроверкиИПодбора(ДанныеПоШтрихкодам, ПараметрыСканирования);
		
	Иначе
		СканированиеФормыПроверкиИПодбора = Ложь;
	КонецЕсли;
	...
	Для Каждого НайденнаяУпаковка Из НайденныеУпаковки Цикл
		...
		Если ЗначениеЗаполнено(НайденнаяУпаковка.Номенклатура) Тогда
			...
			Если СтрокаДанныхКодаМаркировки.ВидПродукции <> НайденнаяУпаковка.ВидПродукции Тогда
				...
				ШтрихкодированиеОбщегоНазначенияИС.УточнитьДанныеРазбораПоВидуПродукции(
					СтрокаДанныхКодаМаркировки, НайденнаяУпаковка.ВидПродукции, ПараметрыСканирования, ОбщиеМодули, СканированиеФормыПроверкиИПодбора);
		...
		Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанныхКодаМаркировки.ВидПродукции, Истина) Тогда
			
			//@skip-check wrong-string-literal-content
			МодульШтрихкодированиеИСМП = ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСМП");
			МодульШтрихкодированиеИСМП.УстановитьПризнакОтказаОтПроверкиКодовМаркировкиПоСтроке(СтрокаДанныхКодаМаркировки, ПараметрыСканирования, СканированиеФормыПроверкиИПодбора);
			
			ТребуетсяКонтрольМарок = СтрокаДанныхКодаМаркировки.ТребуетсяКонтрольМарок;
			
		Иначе
			ТребуетсяКонтрольМарок = Истина;
		КонецЕсли;

		Если Не ТребуетсяКонтрольМарок Тогда
			СтрокаДанныхКодаМаркировки.ШтрихкодУпаковки = Неопределено;
		КонецЕсли;
...
Функция ОбщиеКолонкиКоллекцийВложенныхШтрихкодов()
	...
	КоллекцияОбязательныхКолонок.Вставить("РазрешительныйРежимИдентификаторЗапросаГИСМТ", Новый ОписаниеТипов("Строка"));
	КоллекцияОбязательныхКолонок.Вставить("РазрешительныйРежимДатаЗапросаГИСМТ",          Новый ОписаниеТипов("Строка"));
	КоллекцияОбязательныхКолонок.Вставить("РазрешительныйРежимАдресСервера",              Новый ОписаниеТипов("Строка"));
	КоллекцияОбязательныхКолонок.Вставить("РазрешительныйРежимТелоЗапросаJSON",           Новый ОписаниеТипов("Строка"));
	КоллекцияОбязательныхКолонок.Вставить("РазрешительныйРежимТелоОтветаJSON",            Новый ОписаниеТипов("Строка"));
	КоллекцияОбязательныхКолонок.Вставить("РазрешительныйРежимКодОтвета",                 Новый ОписаниеТипов("Число"));
	КоллекцияОбязательныхКолонок.Вставить("ПодлежитПроверкеРазрешительногоРежима",        Новый ОписаниеТипов("Булево"));
...
Процедура ЗаполнитьСведенияПоШтриходамДаннымПулаGTIN(ДанныеПоШтрихкодам, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено) Экспорт
	...
	Если ДопустимаПродукцияИСМП
		Или ДопустимаТабачнаяПродукция Тогда
		
		МодульШтрихкодированиеИСМПСлужебный = ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеОбщегоНазначенияИСМП");
		СканированиеФормыПроверкиИПодбора = МодульШтрихкодированиеИСМПСлужебный.ЭтоСканированиеВФормеПроверкиИПодбора(ДанныеПоШтрихкодам, ПараметрыСканирования);
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая И Не ЗначениеЗаполнено(СтрокаДанных.GTIN) Тогда
			
			ШтрихкодДляПоиска = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.Base64ВШтрихкод(СтрокаДанных.ШтрихкодBase64);
			Если УникальныеШтрихкодыEAN[ШтрихкодДляПоиска] = Неопределено Тогда
				ШтрихкодыEAN.Добавить(ШтрихкодДляПоиска);
				УникальныеШтрихкодыEAN.Вставить(ШтрихкодДляПоиска, Истина);
			КонецЕсли;
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(СтрокаДанных.GTIN)
				И (СтрокаДанных.ВидыПродукцииКодаМаркировки.Найти(Перечисления.ВидыПродукцииИС.Алкогольная) <> Неопределено
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
			...
			Если Не ДанныеПулаПоКоду.СохраненоПриСканировании Тогда
				...
				Если Не ЗначениеЗаполнено(СтрокаДанных.ВидУпаковки) Тогда
					...
					Если СтрокаДанных.ВидПродукции <> ДанныеПулаПоКоду.ВидПродукции Тогда
						...
						ШтрихкодированиеОбщегоНазначенияИС.УточнитьДанныеРазбораПоВидуПродукции(
							СтрокаДанных, ДанныеПулаПоКоду.ВидПродукции, ПараметрыСканирования, ОбщиеМодули, СканированиеФормыПроверкиИПодбора);
				...
				Если Не СтрокаДанных.НоменклатураСопоставленаПоУПД
					...
					ПоляЗаполнения = Новый Структура("Номенклатура, Характеристика");
					ПоляЗаполнения.Номенклатура = ДанныеПулаПоКоду.Номенклатура;
					ПоляЗаполнения.Характеристика = ДанныеПулаПоКоду.Характеристика;
					Если Не ВидыПродукцииПринадлежатТоварнойГруппе(
						СтрокаДанных.ВидПродукции,
						ДанныеПулаПоКоду.ВидПродукции,
						КешТоварныхГрупп) Тогда
		...
		Если СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
			И Не ЗначениеЗаполнено(СтрокаДанных.GTIN) Тогда // EAN13+5
			
			ЗначениеПоиска = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.Base64ВШтрихкод(СтрокаДанных.ШтрихкодBase64);
		...
		Для Каждого СтрокаИнформации Из НайденныеШтрихкодыEAN Цикл
			...
			// Штрихкод номенклатуры вместо штрихкода, распознанного как штрихкод упаковки
			Если СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
				И Не ЗначениеЗаполнено(СтрокаДанных.GTIN) Тогда
				СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.ПустаяСсылка();
				СтрокаДанных.Штрихкод = ЗначениеПоиска;
			КонецЕсли;
			...
			ИначеЕсли Не СтрокаДанных.НоменклатураСопоставленаПоУПД
				...
				Если ЗначениеЗаполнено(СтрокаДанных.ВидПродукции) Тогда
					...
					Если УточнитьПоДаннымРазбора Тогда
						...
						ШтрихкодированиеОбщегоНазначенияИС.УточнитьДанныеРазбораПоВидуПродукции(
							СтрокаДанных, СтрокаДанных.ВидПродукции, ПараметрыСканирования,, СканированиеФормыПроверкиИПодбора);
		...
		Если ДанныеПулаПоКоду <> Неопределено
			...
			Если Не ЗначениеЗаполнено(СтрокаДанных.Номенклатура)
				...
				Если СтрокаДанных.ВидПродукции <> ДанныеПулаПоКоду.ВидПродукции Тогда
					...
					ШтрихкодированиеОбщегоНазначенияИС.УточнитьДанныеРазбораПоВидуПродукции(
						СтрокаДанных, ДанныеПулаПоКоду.ВидПродукции, ПараметрыСканирования,, СканированиеФормыПроверкиИПодбора);
		...
		Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции, Истина)
			И МодульИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции(СтрокаДанных.ВидПродукции)
...
Функция ДобавитьИзменитьКодМаркировкиВскрытойПотребительскойУпаковки(ПараметрыСканирования, Товары, ШтрихкодыУпаковок = Неопределено) Экспорт
	...
	СерииИспользюутся = ОбщегоНазначенияИС.СерииИспользуются();
...
Процедура ПоместитьДанныеДокументаВКэшМаркируемойПродукции(Форма, ПараметрыСканирования, ОчиститьОтложенныеКоды = Ложь) Экспорт
	...
	Если ОбработанныеКодыМаркировки.Количество() = 0 Тогда
		...
		ДанныеПоШтрихкодам = ШтрихкодыУпаковокПоДаннымТоваровИМаркируемойПродукции(Форма, ПараметрыСканирования);
		КоллекцияВсехШтрихкодовДерева(
			ДанныеПоШтрихкодам.ВложенныеШтрихкоды.ДеревоУпаковок.Строки,
			ОбработанныеКодыМаркировки, ПараметрыСканирования, ДанныеПоШтрихкодам);
...
Процедура ОбработатьУточнениеДанныхДляФормыПроверкиИПодбора(РезультатВыбора, РезультатОбработки, ПараметрыСканирования, КэшированныеЗначения) Экспорт
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ДанныеШтрихкода.ВидПродукции) Тогда
			ДанныеШтрихкода.ВидПродукции = ОбщегоНазначенияИС.ВидПродукцииПоНоменклатуре(ДанныеШтрихкода.Номенклатура);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ДанныеШтрихкода.ШтрихкодУпаковки)
			...
			Исключение
				...
				ИмяСобытияЖурналРегистрации = НСтр("ru = 'ИС МП: Обработка выбора серии';
													|en = 'ИС МП: Обработка выбора серии'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(ДанныеШтрихкода.ВидПродукции, Истина) Тогда
		...
		Модуль = ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеИСМПСлужебный");
		Модуль.ОбработатьУточнениеРезультатаОбработки(ДанныеШтрихкода, РезультатОбработки);
...
Процедура ПоместитьДеревоУпаковокВоВременноеХранилище(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	НормализоватьИСМП  = Ложь;
	НормализоватьМОТП  = Ложь;
	
	// Если выключены запросы к ГИС МТ, то ненайденные упаковки необходимо преобразовать и добавить в дерево
	...
	Если Не ПараметрыСканирования.РазрешенаОбработкаНеНайденныхЛогистическихУпаковок
		И (ПараметрыСканирования.ИспользуетсяСоответствиеШтрихкодовСтрокДерева
			Или ДанныеПоШтрихкодам.ИерархическаяЗагрузкаИзТСД) Тогда
		
		// Создать и нормализовать дерево
			...
			Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
				Если СтрокаДанных.ТипУпаковки = Перечисления.ТипыУпаковок.МонотоварнаяУпаковка Тогда
					Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции) Тогда
						НормализоватьИСМП = Истина;
					ИначеЕсли ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(СтрокаДанных.ВидПродукции) Тогда
						НормализоватьМОТП = Истина;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
			Если НормализоватьИСМП Или НормализоватьМОТП Тогда
				ВложенныеШтрихкоды = ИнициализацияВложенныхШтрихкодов();
				Если НормализоватьИСМП Тогда
					Модуль = ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеИСМПСлужебный");
					Модуль.НормализоватьДанныеВложенныхШтрихкодов(ВложенныеШтрихкоды, ПараметрыСканирования);
				КонецЕсли;
				Если НормализоватьМОТП Тогда
					Модуль = ОбщегоНазначения.ОбщийМодуль("ШтрихкодированиеМОТП");
					Модуль.НормализоватьДанныеВложенныхШтрихкодов(ВложенныеШтрихкоды, ПараметрыСканирования);
				КонецЕсли;
			КонецЕсли;
		...
		// Дополнить дерево
		Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
			Если СтрокаДанных.ТипУпаковки = Перечисления.ТипыУпаковок.МонотоварнаяУпаковка
	...
	// Сортировать данные
	Если ПараметрыСканирования.ИспользуетсяСоответствиеШтрихкодовСтрокДерева Тогда
		...
		Если ДанныеПоШтрихкодам.ВложенныеШтрихкоды.Свойство("МаркированныеТовары")
			...
			Если СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая Тогда
				ДанныеПоШтрихкодам.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Запрещено сканировать логистические упаковки в форму документа с детализацией %1.
					           ...
					           |en = 'Запрещено сканировать логистические упаковки в форму документа с детализацией %1.
					           |Сканирование таких кодов необходимо выполнять в форме проверки и подбора.'"),
			...
			ИначеЕсли СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
				ДанныеПоШтрихкодам.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Запрещено сканировать групповые упаковки в форму документа с детализацией %1.
					           |Сканирование таких кодов необходимо выполнять в форме проверки и подбора.';
					           |en = 'Запрещено сканировать групповые упаковки в форму документа с детализацией %1.
					           |Сканирование таких кодов необходимо выполнять в форме проверки и подбора.'"),
					ПараметрыСканирования.ДетализацияСтруктурыХранения);
			ИначеЕсли СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Набор Тогда
				ДанныеПоШтрихкодам.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Запрещено сканировать упаковки наборов в форму документа с детализацией %1.
					           |Сканирование таких кодов необходимо выполнять в форме проверки и подбора.';
					           |en = 'Запрещено сканировать упаковки наборов в форму документа с детализацией %1.
					           |Сканирование таких кодов необходимо выполнять в форме проверки и подбора.'"),
					ПараметрыСканирования.ДетализацияСтруктурыХранения);
			КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось записать или обновить элементы справочника Штрихкоды упаковок товаров по причине: %1';
					|en = 'Не удалось записать или обновить элементы справочника Штрихкоды упаковок товаров по причине: %1'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));

		ЗаписьЖурналаРегистрации(
				НСтр("ru = 'ГосИС: Запись элементы справочника Штрихкоды упаковок товаров';
					|en = 'ГосИС: Запись элементы справочника Штрихкоды упаковок товаров'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция СоздатьМаркированныйТоварПоДаннымШтрихкода(ДанныеШтрихкода, ПараметрыСканирования, РезультатОбработки = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(ДанныеШтрихкода.ВидПродукции, Истина) Тогда
		ТребуетсяКонтрольМарок = ДанныеШтрихкода.ТребуетсяКонтрольМарок;
	Иначе
		ТребуетсяКонтрольМарок = Истина;
	КонецЕсли;
	
	Если Не ТребуетсяКонтрольМарок Тогда
		СоздаватьШтрихкодУпаковки = Ложь;
	КонецЕсли;
...
Функция ТоварныеГруппыВидовПродукции() Экспорт
	...
	ТоварныеГруппыВидовПродукции.Вставить(Перечисления.ВидыПродукцииИС.МедицинскиеИзделия20, Перечисления.ВидыПродукцииИС.КреслаКоляски);
	...
	ТоварныеГруппыВидовПродукции.Вставить(Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность2025, Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность);
	...
	ТоварныеГруппыВидовПродукции.Вставить(Перечисления.ВидыПродукцииИС.КормаДляЖивотныхВлажныеБезВЕТИС, Перечисления.ВидыПродукцииИС.КормаДляЖивотныхПодконтрольныеВЕТИС);
	ТоварныеГруппыВидовПродукции.Вставить(Перечисления.ВидыПродукцииИС.КормаДляЖивотныхВлажныеПодконтрольныеВЕТИС, Перечисления.ВидыПродукцииИС.КормаДляЖивотныхПодконтрольныеВЕТИС);
...
Функция ТекстОшибкиНеСоответствуетСтандартнойУпаковке(КодУпаковки, СтандартныйРазмер, ВложеноПоДаннымСервиса)
	...
	ШаблонОшибки = НСтр("ru = 'По данным ГИС МТ в упаковке %3 ед. из %2';
						|en = 'По данным ГИС МТ в упаковке %3 ед. из %2'");
	Возврат СтрШаблон(
		ШаблонОшибки,
...
Процедура ПроверитьДоступныеОстаткиМаркируемойПродукции(ПараметрыСканирования, ДанныеПоШтрихкодам, ЕстьОшибки)
	...
	Если ПараметрыСканирования.ОперацияКонтроляАкцизныхМарок = "Продажа" Тогда
		ТекстОшибки = НСтр("ru = 'Продана ранее по данным информационной базы';
							|en = 'Продана ранее по данным информационной базы'");
	Иначе
		ТекстОшибки = НСтр("ru = 'Уже в наличии по данным информационной базы';
							|en = 'Уже в наличии по данным информационной базы'");
	КонецЕсли;
...
Функция ОбработатьУточнениеНоменклатуры(ФормаПараметрыСканирования, РезультатВыбора, РезультатОбработки, КэшированныеЗначения)
	...
	СвойстваМаркируемойПродукции = ОбщегоНазначенияИС.СвойстваМаркируемойПродукции(ДанныеШтрихкода.Номенклатура);
...
Процедура ОтметитьНовыеИИзмененныеШтрихкодыУпаковок(ПараметрыСканирования, ДанныеПоШтрихкодам, ЭлементыДляИзменения) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции, Истина) Тогда
			...
			ИспользоватьМарки = ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПоВидуОперацииИПродукции(
				ПараметрыСканирования.ВидОперацииИСМП,
				СтрокаДанных.ВидПродукции);
				
		Иначе
			
			ИспользоватьМарки = Истина;
			
		КонецЕсли;
		
		Если Не ИспользоватьМарки Тогда
			Продолжить;
		КонецЕсли;
...
//  ПараметрыСканирования - см. ШтрихкодированиеОбщегоНазначенияИС.ПараметрыСканирования
//  ПараметрыДополнения - см. ПараметрыДополненияВложенныхШтрихкодовУпаковок
// Возвращаемое значение:
//  см. ШтрихкодированиеОбщегоНазначенияИС.ИнициализацияДанныхПоШтрихкодам
Функция ВложенныеШтрихкодыУпаковокПоДаннымМенеджераВТ(ВыборкаПерваяИтерация, МенеджерВременныхТаблиц, ПараметрыСканирования, ПараметрыДополнения) 
	
	ДанныеПоШтрихкодам = ШтрихкодированиеОбщегоНазначенияИС.ИнициализацияДанныхПоШтрихкодам();
	ДанныеПоШтрихкодам.ВложенныеШтрихкоды = ИнициализацияВложенныхШтрихкодов();
	
	ШтрихкодированиеОбщегоНазначенияИС.НормализоватьДанныеВложенныхШтрихкодов(ДанныеПоШтрихкодам, ПараметрыСканирования);
	...
	Пока ПродолжатьПоиск Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			РодительскиеДанные = ДанныеПоШтрихкодам.ВложенныеШтрихкоды.ДеревоУпаковок;
	...
	ИменаМодулейПараметровДополнения = Новый Соответствие;
	...
	Для Каждого КлючЗначение Из СоответствиеСтрокДереваУпаковок Цикл
		...
		Если КлючЗначение.Значение.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар Тогда
			...
			НоваяСтрока = ДанныеПоШтрихкодам.ВложенныеШтрихкоды.МаркированныеТовары.Добавить();
			...
			Если ПараметрыДополнения.ДанныеЧастичногоВыбытия[НоваяСтрока.ШтрихкодУпаковки] <> Неопределено Тогда
				Если НоваяСтрока.ВидПродукции = Перечисления.ВидыПродукцииИС.Алкогольная Тогда
					ИмяМодуля = "ШтрихкодированиеЕГАИС";
					Если ИменаМодулейПараметровДополнения[ИмяМодуля] = Неопределено Тогда
						ИменаМодулейПараметровДополнения[ИмяМодуля] = ОбщегоНазначенияИС.ПодсистемаСуществует("ЕГАИС");
					КонецЕсли;
				ИначеЕсли ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(НоваяСтрока.ВидПродукции, Истина) Тогда
					ИмяМодуля = "ШтрихкодированиеИСМПСлужебный";
					Если ИменаМодулейПараметровДополнения[ИмяМодуля] = Неопределено Тогда
						ИменаМодулейПараметровДополнения[ИмяМодуля] = ОбщегоНазначенияИС.ПодсистемаСуществует("ИСМП");
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	Для Каждого КлючЗначение Из ИменаМодулейПараметровДополнения Цикл
		Если Не КлючЗначение.Значение Тогда
			Продолжить;
		КонецЕсли;
		Модуль = ОбщегоНазначения.ОбщийМодуль(КлючЗначение.Ключ);
		Модуль.ДополнитьДанныеПоШтрихкодамПараметрамиДополнения(ДанныеПоШтрихкодам, ПараметрыСканирования, ПараметрыДополнения);
	КонецЦикла;
...
Функция ШтрихкодыУпаковокПоДаннымТоваровИМаркируемойПродукции(Форма, ПараметрыСканирования)
	...
	ЕстьКолонкаЧастичноеВыбытиеВариантУчета = Неопределено;
	ПараметрыДополнения = ПараметрыДополненияВложенныхШтрихкодовУпаковок();
	...
	Для Каждого СтрокаТЧ Из ТабличнаяЧастьШтрихкоды Цикл
		...
		КонецЕсли;
		Если ЕстьКолонкаЧастичноеВыбытиеВариантУчета = Неопределено Тогда
			Если ЗначениеЗаполнено(ПараметрыСканирования.ИмяКолонкиЧастичноеВыбытиеНоменклатура) Тогда
				ЕстьКолонкаЧастичноеВыбытиеВариантУчета = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
					СтрокаТЧ, ПараметрыСканирования.ИмяКолонкиЧастичноеВыбытиеНоменклатура);
			Иначе
				ЕстьКолонкаЧастичноеВыбытиеВариантУчета = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Для Каждого ИмяКолонки Из МассивИменКолонокШтрихкодовУпаковки Цикл
			...
			КонецЕсли;
			Если ЕстьКолонкаЧастичноеВыбытиеВариантУчета Тогда
				ЭлементЧастичногоВыбытия = НовыйЭлементДополненияВложенныхШтрихкодовУпаковокЧастичноеВыбытие();
				Если ЗначениеЗаполнено(ПараметрыСканирования.ИмяКолонкиЧастичноеВыбытиеВариантУчета) Тогда
					ЭлементЧастичногоВыбытия.ЧастичноеВыбытиеВариантУчета = СтрокаТЧ[ПараметрыСканирования.ИмяКолонкиЧастичноеВыбытиеВариантУчета];
				КонецЕсли;
				Если ЗначениеЗаполнено(ПараметрыСканирования.ИмяКолонкиЧастичноеВыбытиеНоменклатура) Тогда
					ЭлементЧастичногоВыбытия.ЧастичноеВыбытиеНоменклатура = СтрокаТЧ[ПараметрыСканирования.ИмяКолонкиЧастичноеВыбытиеНоменклатура];
				КонецЕсли;
				Если ЗначениеЗаполнено(ПараметрыСканирования.ИмяКолонкиЧастичноеВыбытиеХарактеристика) Тогда
					ЭлементЧастичногоВыбытия.ЧастичноеВыбытиеХарактеристика = СтрокаТЧ[ПараметрыСканирования.ИмяКолонкиЧастичноеВыбытиеХарактеристика];
				КонецЕсли;
				Если ЗначениеЗаполнено(ПараметрыСканирования.ИмяКолонкиЧастичноеВыбытиеКоличество) Тогда
					ЭлементЧастичногоВыбытия.Количество = СтрокаТЧ[ПараметрыСканирования.ИмяКолонкиЧастичноеВыбытиеКоличество];
				КонецЕсли;
				Если ЗначениеЗаполнено(ЭлементЧастичногоВыбытия.ЧастичноеВыбытиеВариантУчета) Тогда
					ПараметрыДополнения.ДанныеЧастичногоВыбытия.Вставить(НоваяСтрока.ШтрихкодУпаковки, ЭлементЧастичногоВыбытия);
				КонецЕсли;
			КонецЕсли;
	...
	ДанныеПоШтрихкодам = ВложенныеШтрихкодыУпаковокПоДаннымМенеджераВТ(
		РезультатыЗапроса.ВложенныеШтрихкоды.Выбрать(), МенеджерВременнойТаблицы, ПараметрыСканирования, ПараметрыДополнения);
...
Процедура КоллекцияВсехШтрихкодовДерева(СтрокиДереваУпаковок, ОбработанныеКодыМаркировки, ПараметрыСканирования, ДанныеПоШтрихкодам)
	...
	Для Каждого СтрокаДерева Из СтрокиДереваУпаковок Цикл
		...
		Если ШтрихкодированиеОбщегоНазначенияИС.ТребуетсяЧастичноеВыбытие(ПараметрыСканирования, ДанныеПоШтрихкодам)
			...
			ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.ОперацияИС(ПараметрыСканирования, СтрокаДерева.ВидПродукции)) Тогда
			КолонкиКэша = "Штрихкод,ТекстОшибки,ТипШтрихкода,ТипУпаковки,ЧастичноеВыбытие";
		Иначе
			КолонкиКэша = "Штрихкод,ТекстОшибки,ТипШтрихкода,ТипУпаковки";
		КонецЕсли;
		ОбработанныеКодыМаркировки.Вставить(
			СтрокаДерева.Штрихкод,
			ШтрихкодированиеОбщегоНазначенияИС.СтрокаТаблицыДанныеКодовМаркировкиВСтруктуру(СтрокаДерева, КолонкиКэша));
		
		КоллекцияВсехШтрихкодовДерева(СтрокаДерева.Строки, ОбработанныеКодыМаркировки, ПараметрыСканирования, ДанныеПоШтрихкодам);
...
Процедура ЗаполнитьПоДокументуИОснованию(ДанныеПоШтрихкодам, ПараметрыСканирования, Форма) Экспорт
	...
	ОбновлятьСерии = ОбщегоНазначенияИС.СерииИспользуются();
...
Функция ОписанияGTINПоДаннымШтрихкодовРасширеннойВерсии(НаборGTIN) Экспорт
