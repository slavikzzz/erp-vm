	...
	Если Не Форма.ЗарегистрированыДоходы Тогда
		ТекстПредупреждения = СтрШаблон(
			НСтр("ru = 'По %1 %2 доходы не зарегистрированы.';
				|en = 'Income is not registered for %1 %2.'"), 
	...
	ИначеЕсли ЗначениеЗаполнено(ТекстКодов) Тогда
		ИнфоРегистрацияВНО = СтрШаблон(
			НСтр("ru = 'За %1 год зарегистрированы доходы по следующим кодам %2 %3. %4';
				|en = 'Income of the %2 %3 codes are registered for %1. %4'"),
			Формат(Форма.Объект.НалоговыйПериод,"ЧЦ=4; ЧДЦ=0; ЧГ=0"), 
			ЗаголовокРегистрации,
			ТекстКодов, 
			ТекстПредупреждения);
	Иначе
		Если ОписаниеДоходовОрганизации.Количество() > 0 Тогда
			Шаблон = НСтр("ru = 'За %1 год не зарегистрировано доходов. 
			                    ...
			                    |en = 'No income recorded for %1.
			                    |The registration with the tax authority may be not specified for the company.'");
		Иначе	
			Шаблон = НСтр("ru = 'За %1 год не зарегистрировано доходов.';
							|en = 'Income is not registered for %1.'");
		КонецЕсли;	
...
Процедура УстановитьСписокВыбораРегистрацийВНОПоФизическомуЛицу(Форма, ФизическоеЛицо) Экспорт 
	...
	ОтсутствуютЗаписиПоДоходам = Истина;
	Если Не ФормироватьСводно Тогда
		Запрос.Текст = 
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТДействующиеРегистрации";
		Выборка = Запрос.Выполнить().Выбрать();
		
		ОтсутствуютЗаписиПоДоходам = Выборка.Количество() = 0;
		
	КонецЕсли;
	
	Если ФормироватьСводно Или ОтсутствуютЗаписиПоДоходам Тогда 
	...
	Если Не Форма.Объект.РегистрацияВНалоговомОргане.Пустая()
		...
		Если Форма.РегистрацияВНОПредставление = "" Тогда
			УстановитьПредставлениеРегистрацииВНО(Форма);
		КонецЕсли;
		СписокРегистраций.Добавить(Форма.Объект.РегистрацияВНалоговомОргане, Форма.РегистрацияВНОПредставление);
	КонецЕсли;
...
Процедура УстановитьЗаголовокПоляРегистрацияВНО(Форма, Подробно = Ложь) Экспорт
	Если УчетНДФЛКлиентСервер.СправкиНДФЛДокументИспользуетКодОКТМО(Форма.Объект) Тогда	
		ТекстПодробно = ?(Подробно, " " + НСтр("ru = 'при выплате доходов';
												|en = 'on income payments'"), "");
		Заголовок = СтрШаблон(НСтр("ru = 'ОКТМО/КПП %1';
									|en = 'RNCMT/KPP %1'"), ТекстПодробно);
	Иначе	
