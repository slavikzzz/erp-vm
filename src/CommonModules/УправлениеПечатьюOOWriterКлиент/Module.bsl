...
#Если Не МобильныйКлиент Тогда
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при связи с сервисом ""%1"".';
				|en = 'An error occurred when connecting to service ""%1"".'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), ИмяОбъекта)
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при запуске сервиса ""%1"".';
				|en = 'An error occurred when starting service ""%1"".'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), ИмяОбъекта)
			+ Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Функция ПолучитьПозициюНачалаОбласти(Знач xDocument, Знач ИмяОбласти)
	...
	Если xFound = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не найдено начало области макета:';
								|en = 'Cannot find where template begins:'") + " " + ИмяОбласти;	
	КонецЕсли;
	Возврат xFound.End;
...
Функция ПолучитьПозициюОкончанияОбласти(Знач xDocument, Знач ИмяОбласти)
	...
	xFound = xDocument.findFirst(xSearchDescr);
	Если xFound = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не найден конец области макета:';
								|en = 'Cannot find where template ends:'") + " " + ИмяОбласти;	
	КонецЕсли;
