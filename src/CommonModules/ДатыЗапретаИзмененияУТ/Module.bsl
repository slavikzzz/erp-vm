...
Процедура ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения(Знач ИсточникиДанных) Экспорт
	...
	//++ НЕ УТКА
	УправлениеРемонтами.ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения(ИсточникиДанных);
	//-- НЕ УТКА
...
#Область ВводОстатков
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										...
										"Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										Метаданные.Документы.НачальныеОстаткиНЗППоПодразделениям.ПолноеИмя(),
										"Дата",
										"ВводОстатков",
										"Организация");
	//-- НЕ УТКА
...
#Область Продажи
	...
	//-- Устарело_Переработка24
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										Метаданные.Документы.ОтчетДавальцу2_5.ПолноеИмя(),
										"Дата",
										"Продажи",
										"Организация");
	//-- НЕ УТКА
...
#Область ПродажиМеждуОрганизациями
	...
	//++ НЕ УТКА
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										Метаданные.Документы.ОтчетДавальцуМеждуОрганизациями.ПолноеИмя(),
										"Дата",
										"ПродажиМеждуОрганизациями",
										"Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
...
#Область Производство
	...
	//++ НЕ УТКА
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										Метаданные.Документы.ПроизводственнаяОперация2_2.ПолноеИмя(),
										"ОкончаниеВыполнения",
										"Производство",
										"Этап.Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных,
										Метаданные.Документы.СменноеЗадание.ПолноеИмя(),
										"Дата",
										"Производство",
										"");
	//-- НЕ УТКА
...
Процедура ПередПроверкойЗапретаИзменения(Объект, ВыполнятьПроверку, УзелПроверки, ВерсияОбъекта) Экспорт
	...
	// Для этапов производства проверка запрета изменений реализована в модуле объекта.
	Если ТипОбъекта = Тип("ДокументОбъект.ЭтапПроизводства2_2")
	 ИЛИ ТипРегистратора = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		ВыполнятьПроверку = Ложь;
	КонецЕсли;
	
	//++ Устарело_Производство21
	// Для маршрутных листов проверка запрета изменений реализована в модуле объекта.
	Если ТипОбъекта = Тип("ДокументОбъект.МаршрутныйЛистПроизводства")
	 ИЛИ ТипРегистратора = Тип("ДокументСсылка.МаршрутныйЛистПроизводства") Тогда
		ВыполнятьПроверку = Ложь;
	КонецЕсли;
	//-- Устарело_Производство21
	
	// Производственные операции проверяются только в финальных статусах.
	Если ТипОбъекта = Тип("ДокументОбъект.ПроизводственнаяОперация2_2") Тогда
		
		ПроверятьНовуюВерсию  = Объект.Статус = Перечисления.СтатусыПроизводственныхОпераций.Выполнена;
		СтарыйСтатус 		  = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "Статус");
		ПроверятьСтаруюВерсию = СтарыйСтатус = Перечисления.СтатусыПроизводственныхОпераций.Выполнена;
		
		Если Не ПроверятьНовуюВерсию И Не ПроверятьСтаруюВерсию Тогда
			ВыполнятьПроверку = Ложь;
	...
	Если ТипОбъекта = Тип("РегистрНакопленияНаборЗаписей.РасчетыСКлиентами")
	 И (ТипРегистратора = Тип("ДокументСсылка.ЗаказКлиента")
		...
		ИЛИ ТипРегистратора = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента")
		 ...
		 //++ Устарело_Переработка24
		 ИЛИ ТипРегистратора = Тип("ДокументСсылка.ЗаказДавальца")
		 //-- Устарело_Переработка24
		 ИЛИ ТипРегистратора = Тип("ДокументСсылка.ЗаказДавальца2_5")
		 //-- НЕ УТКА
		) Тогда
