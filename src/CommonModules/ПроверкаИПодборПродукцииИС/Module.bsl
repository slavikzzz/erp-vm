...
Процедура ПрименитьКешШтрихкодовУпаковок(Форма, Настройки = Неопределено, ОбновитьТовары = Ложь, ДополнительныеКлючи = "") Экспорт
	...
	ОбновитьПризнакиКонтроляОборотаМарок(Форма, ТабличнаяЧастьТовары);
	...
	Для Каждого СтрокаТовары Из ТабличнаяЧастьТовары Цикл
		...
		Если (Настройки.ВсеМаркируемые
			Или СтрокаТовары.МаркируемаяПродукция)
			И ДействуетКонтрольОборотаМарок Тогда
			...
			ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТовары, "РазрешенОборотНемаркированныхОстатков")
				И СтрокаТовары.РазрешенОборотНемаркированныхОстатков
				И КоличествоПоКлючу > КоличествоВКеше
				И КоличествоВКеше > 0
				И (Не ЕстьКоличествоПотребительскихКодов Или КоличествоПотребительскихКодовРазница > 0) Тогда
				СтрокаТовары.СтатусПроверкиГосИС = 5;
			Иначе
...
Функция КартинкаТипыУпаковкиПоВидуПродукции(ВидПродукции) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ВидПродукции) Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Для вида продукции: %1, не задана картинка с типами упаковок.';
										|en = 'Для вида продукции: %1, не задана картинка с типами упаковок.'"), ВидПродукции);
	Иначе
...
Процедура ЗаполнитьПризнакМаркируемаяПродукцияГосИС(ТабличнаяЧасть, Настройки)
	...
	Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
		...
		ИначеЕсли ЕстьКолонкаДействуетКонтрольОборотаМарок Тогда
		...
		ИначеЕсли ЕстьКолонкаРазрешенОборотНемаркированныхОстатков Тогда
			РазрешенОборотНемаркированныхОстатков = СтрокаТабличнойЧасти.РазрешенОборотНемаркированныхОстатков;
		Иначе
			РазрешенОборотНемаркированныхОстатков = Ложь;
		КонецЕсли;
		
		Если (ВсеМаркируемые Или СтрокаТабличнойЧасти.МаркируемаяПродукция) И ДействуетКонтрольОборотаМарок Тогда
...
Процедура ОбновитьПризнакиКонтроляОборотаМарок(Форма, ТабличнаяЧасть)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПараметрыИнтеграцииГосИС") Тогда
		...
		Если Форма.ПараметрыИнтеграцииГосИС.Получить("ИСМП") <> Неопределено Тогда
			...
			ПроверкаИПодборПродукцииИСМП.ОбновитьПризнакиКонтроляОборотаМарок(Форма, ТабличнаяЧасть);
