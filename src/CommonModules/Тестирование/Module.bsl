...
Функция СохранитьЭталонныеДанные(КонтрольныеРегистры, ПараметрыЗапроса = Неопределено, МассивРегистраторов = Неопределено, ЭталонныеДанные = Неопределено) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Тестирование';
		...
		+ "." + НСтр("ru = 'Начато сохранение эталонных данных';
					|en = 'Saving reference data started'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ЗапросДанных.ВыполнитьПакет();
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Тестирование';
									|en = 'Testing'", ОбщегоНазначения.КодОсновногоЯзыка())
		+ "." + НСтр("ru = 'Завершено сохранение эталонных данных';
					|en = 'Finished saving reference data'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция СравнитьСЭталоннымиДанными(ВременныеТаблицы, КонтрольныеРегистры, ПараметрыЗапроса = Неопределено, МассивРегистраторов = Неопределено, ЭталонныеДанные = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Тестирование';
									|en = 'Testing'", ОбщегоНазначения.КодОсновногоЯзыка())
		+ "." + НСтр("ru = 'Начато сравнение c эталонными данными';
					|en = 'Started comparison with reference data'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	Для каждого КонтрольныйРегистр Из КонтрольныеРегистры Цикл
		
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Выгрузили';
										|en = 'Exported'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + КонтрольныйРегистр);
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Тестирование';
									|en = 'Testing'", ОбщегоНазначения.КодОсновногоЯзыка())
		+ "." + НСтр("ru = 'Завершено сравнение c эталонными данными';
					|en = 'Finished comparison with reference data'", ОбщегоНазначения.КодОсновногоЯзыка()));
