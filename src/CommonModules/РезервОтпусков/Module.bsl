...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'В регистрах учета резервов заполняется новая аналитика ""Резерв"" предопределенным значением ""Резерв ежегодных отпусков""';
									|en = 'In leave reserve accounting registers, the new ""Reserve"" dimension is filled with the ""Annual leave reserve"" predefined value'");
...
Процедура СобратьДанныеДляНачислениеРезерваОтпусков(Регистратор, Организация, ПериодРегистрации, РасчетРезерваОтпусков, ОценочныеОбязательства, ОценочныеОбязательстваПоСотрудникам) Экспорт
	...
	ТаблицаНакопленныхРезервов   = РезервыПоОплатеТруда.НакопленоРезерваПоВсейОрганизации(ПериодРегистрации, Организации, Резерв, Регистратор, Истина, Истина);
	...
	СтруктураПараметровРасчета.Вставить("Начисление",                                   Не Инвентаризация);
	...
	// Распределение ERP по выработке
	ОтражениеЗарплатыВФинансовомУчетеУП.ЗаполнитьТаблицыОценочныхОбязательств(
		Организация, ПериодРегистрации, ПараметрыРасчетаОбязательств, ОценочныеОбязательства, ОценочныеОбязательстваПоСотрудникам);
...
Функция ДолиРезервовОтпусков(Период, Организация, Резерв, Регистратор = Неопределено)
	...
	ТаблицаНакопленныхРезервов = РезервыПоОплатеТруда.НакопленоРезерваПоВсейОрганизации(Период, Организации, Резерв, Регистратор, Ложь, Истина);
