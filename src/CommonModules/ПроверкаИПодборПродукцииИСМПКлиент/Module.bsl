...
Функция ОбновитьПризнакиКонтроляОборотаМарокПоСтроке(Форма, ДанныеСтроки) Экспорт
	...
	Если ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияИСМП(ВидПродукцииИС, Истина, Истина) Тогда 
		
		ВидОперации = ПараметрыСканирования.ВидОперацииИСМП;
		
		ТаблицаДатРазделенногоКонтроля = Форма.ТаблицаКонтролейДатМаркировкиГосИС;
		СтруктураОтбора                = Новый Структура("ВидПродукцииИС", ВидПродукцииИС);
	...
	Иначе
		...
		Иначе
			ТекстОшибки = НСтр("ru = 'Для маркируемой продукции ""%1"" проверка и подбор в отдельной форме не предусмотрен.';
								|en = 'Для маркируемой продукции ""%1"" проверка и подбор в отдельной форме не предусмотрен.'");
