...
Процедура УправлениеНастройкойГОЗ(Форма) Экспорт
	...
	СЗаказчиком =
		...
		//-- Устарело_Переработка24
		Или Объект.ТипДоговора = ПредопределенноеЗначение("Перечисление.ТипыДоговоров.СДавальцем2_5")
		//-- НЕ УТКА
		Или Ложь;
	...
	Иначе
		Форма.Элементы.ДекорацияНастроитьГК.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Настроить договор для исполнения госконтракта';
																						|en = 'Configure the contract for state contract fulfillment'"),,,, "НастроитьГОЗ");
	КонецЕсли;	
КонецПроцедуры
