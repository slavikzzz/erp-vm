...
Процедура ПодключениеОбработчикаОжиданияАвтообмена(ПоказыватьФормуСоСтарымиСообщениями = Истина, ИнициироватьСчетчикЦиклов = Истина) Экспорт
	...
	// Если у пользователя нет права СохранениеДанныхПользователя возвращается Неопределено.
...
Процедура СохранитьПротоколПослеСохранения(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ПолученныеФайлы = Неопределено Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Сохранение в файл';
											|en = 'Save to file'"),,
			НСтр("ru = 'При сохранении данных возникла ошибка.';
				|en = 'An error occurred upon saving data.'"), БиблиотекаКартинок.Ошибка32);
		Возврат;
	КонецЕсли;
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Сохранение в файл';
		...
		НСтр("ru = 'Протокол успешно сохранен.';
			|en = 'The protocol is saved.'"), БиблиотекаКартинок.Успешно32);
