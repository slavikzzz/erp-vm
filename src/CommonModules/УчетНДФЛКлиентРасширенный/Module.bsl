...
Процедура НДФЛПередУдалением(Форма, НДФЛВыделенныеСтроки, Отказ) Экспорт
	...
	Если МассивИдентификаторовСтрокНДФЛСОшибками = Неопределено Тогда
		...
		Если ОписаниеПанелиВычеты.ТабличнаяЧастьНДФЛ.ИспользуетсяФиксРасчет Тогда
			...
			Для Каждого ИдентификаторСтроки Из НДФЛВыделенныеСтроки Цикл
				
				Если МассивИдентификаторовСтрокНДФЛСОшибками = Неопределено
					Или МассивИдентификаторовСтрокНДФЛСОшибками.Найти(ИдентификаторСтроки) = Неопределено Тогда
					
					НДФЛТекущиеДанные = ДанныеНДФЛ.НайтиПоИдентификатору(ИдентификаторСтроки);
					
					Если НДФЛТекущиеДанные <> Неопределено И Не (НДФЛТекущиеДанные.ФиксРасчет Или НДФЛТекущиеДанные.ФиксСтрока) Тогда
					
						ТекстПредупреждения = НСтр("ru = 'НДФЛ рассчитан автоматически, его редактирование не рекомендуется.
							|Редактирование следует выполнять только в том случае, если вы полностью уверены в своих действиях.';
							|en = 'PIT is calculated automatically, it is not recommended that you edit it.
							|You can edit it only if you are completely sure.'");
	...
	УчетНДФЛКлиентБазовый.НДФЛПередУдалением(Форма, НДФЛВыделенныеСтроки, Отказ);
		...
		Если МассивИдентификаторовСтрокНДФЛСОшибками = Неопределено
			...
			Если ОписаниеПанелиВычеты.ТабличнаяЧастьНДФЛ.ИспользуетсяФиксРасчет И НЕ (НДФЛТекущиеДанные.ФиксРасчет ИЛИ НДФЛТекущиеДанные.ФиксСтрока) Тогда
				...
				ТекстПредупреждения = НСтр("ru = 'НДФЛ рассчитан автоматически, его редактирование не рекомендуется. Редактирование следует выполнять только в том случае, если вы полностью уверены в своих действиях.';
											|en = 'PIT is calculated automatically, it is not recommended that you edit it. You can edit it only if you are completely sure.'");
				ТекстЗаголовка = НСтр("ru = '1С:Предприятие';
										|en = '1C:Enterprise'");
...
Процедура УдалитьПримененныеВычеты(Форма) Экспорт
	...
	ИдентификаторыУдаляемыхСтрокСтрокиНДФЛ = Форма[ОписаниеПанелиВычеты.ИмяГруппыФормыПанелиВычеты 
		+ "ИдентификаторыУдаляемыхСтрокСтрокиНДФЛ"];
