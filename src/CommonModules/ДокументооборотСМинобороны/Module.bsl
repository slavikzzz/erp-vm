...
Функция ПолучитьНастройки(Организация) Экспорт
	...
	Настройки.Вставить("Доверенность", Неопределено);
	...
	|	НастройкиОбменаМинобороны.Доверенность КАК Доверенность
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		...
		Настройки.Доверенность = Выборка.Доверенность;
...
Функция СформироватьАрхивПакета(ПараметрыАрхивации) Экспорт
	...
	Доверенность = ПараметрыАрхивации.Доверенность;
	ТипФайлаВыгрузки = ПараметрыАрхивации.ТипФайлаВыгрузки;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Электронный документооборот с минобороны. Запись файлов пакета';
										|en = 'Электронный документооборот с минобороны. Запись файлов пакета'", 
			ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ТекстОшибки = НСтр("ru = 'Ошибка записи файла отчета на сервере.';
							|en = 'Ошибка записи файла отчета на сервере.'");
	...
	Если ЗначениеЗаполнено(Доверенность) Тогда
		ПараметрыВыгрузкиМЧД = НовыеПараметрыМашиночитаемыеДоверенностиПакета(Доверенность);
		ПараметрыВыгрузкиМЧД.ИНН = ПараметрыАрхивации.ИНН;
		ПараметрыВыгрузкиМЧД.ВыгрузкаПоПриказу564 = ПараметрыАрхивации.ТипФайлаВыгрузки
			= ДокументооборотСМинобороныКлиентСервер.ТипФайлаВыгрузкиЦенаПродукцииГОЗ();
		РезультатВыгрузкиДоверенностей = МашиночитаемыеДоверенностиПакета(ПараметрыВыгрузкиМЧД);
		Если НЕ РезультатВыгрузкиДоверенностей.Выполнено Тогда
			Результат.ОписаниеОшибки = РезультатВыгрузкиДоверенностей.ОписаниеОшибки;
			Возврат Результат;
		КонецЕсли;
		Если РезультатВыгрузкиДоверенностей.РеквизитыМЧД.Количество() < 2 Тогда
			Результат.ОписаниеОшибки = НСтр("ru = 'Отсутствует файл доверенности или подпись доверенности.';
											|en = 'Отсутствует файл доверенности или подпись доверенности.'");
			Возврат Результат;
		КонецЕсли;
		Для каждого ФайлДоверенности Из РезультатВыгрузкиДоверенностей.РеквизитыМЧД Цикл
			Если ПустаяСтрока(ФайлДоверенности.ИмяФайла) Тогда
				Результат.ОписаниеОшибки = НСтр("ru = 'Отсутствует файл доверенности или подпись доверенности.';
												|en = 'Отсутствует файл доверенности или подпись доверенности.'");
				Возврат Результат;
			КонецЕсли;
			ИмяфайлаДоверенности = КаталогВременныхФайловНаСервере + ФайлДоверенности.ИмяФайла;
			Попытка
				ФайлДоверенности.ДанныеФайла.Записать(ИмяфайлаДоверенности);
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ОписаниеОшибки = ОбщегоНазначенияЭДКОКлиентСервер.ПолучитьИнформациюОбОшибке(ИнформацияОбОшибке);
				Результат.ОписаниеОшибки = ОписаниеОшибки;
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Электронный документооборот с минобороны. Запись zip архива пакета';
												|en = 'Электронный документооборот с минобороны. Запись zip архива пакета'", 
					ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,,,
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				Возврат Результат;
			КонецПопытки;
			ФайлыДоверенностей.Добавить(ИмяфайлаДоверенности);
		КонецЦикла;
	КонецЕсли;
	...
	Для каждого ИмяФайла Из ФайлыДоверенностей Цикл
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Ошибка записи файла архива на сервере.';
							|en = 'Ошибка записи файла архива на сервере.'");
		Результат.ОписаниеОшибки = ТекстОшибки;
		Возврат Результат;
	КонецПопытки;
...
Функция СохранитьОтвет(Параметры) Экспорт
	...
	Если Отправки.Количество() = 0 Тогда
		...
		Результат.ОписаниеОшибки = НСтр("ru = 'Ошибка поиска данных о передаче пакета.';
										|en = 'Ошибка поиска данных о передаче пакета.'", "ru");
		Возврат Результат;
	КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Электронный документооборот с минобороны. Сохранение отправки';
										|en = 'Электронный документооборот с минобороны. Сохранение отправки'", 
			ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		Результат.Выполнено = Ложь;
		Результат.ОписаниеОшибки = НСтр("ru = 'Ошибка записи инфомации в базу данных.';
										|en = 'Ошибка записи инфомации в базу данных.'", "ru");
		Возврат Результат;
	КонецПопытки;
...
Функция СохранитьСтатусОтправки(Отправка, Статус) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Электронный документооборот с минобороны. Определение статуса протокола.';
										|en = 'Электронный документооборот с минобороны. Определение статуса протокола.'", 
			ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ИнформацияОбОшибке);
		Возврат Ложь;
...
Функция СтатусУстановленВручную(Отправка, Знач СтатусИзЖурнала) Экспорт
	...
	Если СтатусИзЖурнала = ДокументооборотСКОКлиентСервер.СтатусПереданоВМинобороны() Тогда
...
Процедура ОтключитьОбмен(Организация, ОтключенОбмен) Экспорт
	...
	НаборЗаписей = РегистрыСведений.НастройкиОбменаМинобороны.СоздатьНаборЗаписей();
...
Функция ИмяСобытияВЖурналеРегистрации()
	...
	Возврат НСтр("ru = 'Документооборот с Минобороны.';
				|en = 'Документооборот с Минобороны.'");
	
КонецФункции

Функция НовыеПараметрыМашиночитаемыеДоверенностиПакета(Доверенность)
	
	Результат = Новый Структура;
	Результат.Вставить("ИНН", "");
	// Признак выгрузки по формату, описанному в приказе Министра обороны от 25.09.2024 №564.
	Результат.Вставить("ВыгрузкаПоПриказу564", Ложь);
...
Функция МашиночитаемыеДоверенностиПакета(Параметры)
	
	РезультатПодготовки = Новый Структура;
	РезультатПодготовки.Вставить("РеквизитыМЧД", Неопределено);
	РезультатПодготовки.Вставить("Выполнено", Ложь);
	РезультатПодготовки.Вставить("ОписаниеОшибки", "");
	
	Доверенность = Параметры.Доверенность;
	
	РеквизитыМЧД = Новый ТаблицаЗначений;
	РеквизитыМЧД.Колонки.Добавить("Доверенность",      Метаданные.ОпределяемыеТипы.МашиночитаемыеДоверенностиБРО.Тип);
	...
	РезультатПодготовки.РеквизитыМЧД = РеквизитыМЧД;
	
	СокращенныйПериодБезУИД = Ложь;
	РасширенияЭП = РасширенияПодписей();
	
	ТипыСправочниковМЧД = Новый Структура;
	ТипыСправочниковМЧД.Вставить("РегламентированнаяОтчетность",
		ДокументооборотСМинобороныВызовСервера.ДоступныеСправочникиМЧД("РегламентированнаяОтчетность"));
	ТипыСправочниковМЧД.Вставить("СтандартныеПодсистемы",
		ДокументооборотСМинобороныВызовСервера.ДоступныеСправочникиМЧД("СтандартныеПодсистемы"));
		
	ТекущаяДоверенность = РеквизитыМЧД.Добавить();
	ТекущаяДоверенность.Доверенность = Доверенность;
	
	// Реквизиты доверенности.
	ДатаОкончанияСрокаДействия = "ДатаОкончания";
	РеквизитыДоверенности = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		ТекущаяДоверенность.Доверенность, "НомерДоверенности,ДатаВыдачи," + ДатаОкончанияСрокаДействия, Истина);
	...
	ТекущаяДоверенность.ДатаОкончания     = РеквизитыДоверенности[ДатаОкончанияСрокаДействия];
	
	ТипСправочникаМЧД = ТипЗнч(ТекущаяДоверенность.Доверенность);
	ИндексТекущейДоверенности = 0;
	КоличествоДоверенностей = 0;
	...
	Если ТипыСправочниковМЧД.РегламентированнаяОтчетность.Найти(ТипСправочникаМЧД) <> Неопределено Тогда
		...
		РасширениеФайлаДоверенности = "xml";
		...
		Если Не ЗначениеЗаполнено(РезультатВыгрузки)
		 Или ТипЗнч(РезультатВыгрузки) <> Тип("Массив")
			И НРег(ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(РезультатВыгрузки.ИмяФайла)) <> РасширениеФайлаДоверенности Тогда
			
			РезультатПодготовки.ОписаниеОшибки = НСтр("ru = 'Не удалось сформировать машиночитаемую доверенность налогоплательщика';
		...
		Если ТипЗнч(РезультатВыгрузки) = Тип("Массив") Тогда
			...
			Для Каждого ФайлРезультатаВыгрузки Из РезультатВыгрузки Цикл
				...
				Если РасширенияЭП.Найти(РасширениеФайла) = Неопределено
					И РасширениеФайла <> РасширениеФайлаДоверенности Тогда
					Продолжить;
				КонецЕсли;
				
				Если НРег(РасширениеФайла) = РасширениеФайлаДоверенности Тогда
					ИмяФайлаДоверенности = ФайлРезультатаВыгрузки.Имя;
				КонецЕсли;
				
				ИмяФайлаСРасширением = ИмяФайлаДоверенностиИлиПодписи(
					ФайлРезультатаВыгрузки.Имя, Параметры, , ИмяФайлаДоверенности);
				...
				ТекущаяСтрокаДоверенности.СодержитИнформациюДоверенности = (РасширениеФайла = РасширениеФайлаДоверенности);
		...
		Иначе
			...
			ИмяФайлаСРасширением = ИмяФайлаДоверенностиИлиПодписи(
					РезультатВыгрузки.ИмяФайла, Параметры);
		...
		Для Каждого ДоверенностиДляВыгрузки Из РезультатВыгрузки Цикл
			...
			Для Каждого ФайлРезультатаВыгрузки Из ФайлыДляВыгрузки Цикл
				...
				Если ЗначениеЗаполнено(ФайлРезультатаВыгрузки.ОписаниеОшибки) Тогда
					...
					РезультатПодготовки.ОписаниеОшибки = ФайлРезультатаВыгрузки.ОписаниеОшибки;
					Возврат РезультатПодготовки;
					
				КонецЕсли;
				
				ТипФайлаДоверенности = "Доверенность";
				Если ФайлРезультатаВыгрузки.ТипФайла = ТипФайлаДоверенности Тогда
					ИмяФайлаДоверенности = ФайлРезультатаВыгрузки.ИмяФайла;
				КонецЕсли;
				
				РасширениеФайла = НРег(ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(
					ФайлРезультатаВыгрузки.ИмяФайла));
				ИмяФайлаСРасширением = ИмяФайлаДоверенностиИлиПодписи(
					ФайлРезультатаВыгрузки.ИмяФайла, Параметры, РасширениеФайла, ИмяФайлаДоверенности);
	...
	РезультатПодготовки.Выполнено = Истина;
	Возврат РезультатПодготовки;
...
Функция РасширенияПодписей()
	...
	РасширенияПодписей.Добавить("sign");
	РасширенияПодписей.Добавить("bin");
	
	Возврат РасширенияПодписей;
	
КонецФункции

Функция ИмяФайлаДоверенностиИлиПодписи(ИмяФайлаИзПодсистемы, Параметры, Знач Расширение = "", ИмяФайлаДоверенности = "")
	
	Результат = "";
	ИмяБезРасширения = "";
	
	Файл = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ИмяФайлаИзПодсистемы);
	ИмяБезРасширения = Файл.ИмяБезРасширения;
	
	Если ПустаяСтрока(Расширение) Тогда
		Расширение = ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(ИмяФайлаИзПодсистемы);
	КонецЕсли;
	Расширение = НРег(Расширение);
	
	РасширенияПодписей = РасширенияПодписей();
	ЭтоФайлПодписи = НЕ РасширенияПодписей.Найти(Расширение) = Неопределено;
	РасширениеПодписиДоверенности = "sig";
	Если ЭтоФайлПодписи Тогда
		Расширение = РасширениеПодписиДоверенности;
	КонецЕсли;
	
	Если Параметры.ВыгрузкаПоПриказу564 Тогда
		Результат = Параметры.ИНН + "." + Расширение;
	Иначе
		Если НЕ ПустаяСтрока(ИмяФайлаДоверенности)
			И ЭтоФайлПодписи Тогда
			// делаем имя файла подписи совпадающим с файлом доверенности, за исключением расширения.
			ФайлДоверенности = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ИмяФайлаДоверенности);
			ИмяБезРасширения = ФайлДоверенности.ИмяБезРасширения;
		КонецЕсли;
		Результат = ИмяБезРасширения + "." + Расширение;
	КонецЕсли;
