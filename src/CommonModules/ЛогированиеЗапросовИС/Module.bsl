...
// * HTTPЗапросОтветЗаголовки  - Неопределено, Соответствие из КлючИЗначение - Заголовки запроса HTTP
// * HTTPЗапросОтветТело       - Неопределено, Строка                - Тело запроса или ответа HTTP 
// * HTTPОтветКодСостояния     - Неопределено, Число                 - Код состояния ответа HTTP
...
// * ЛогироватьHTTPЗапрос      - Булево                              - Истина, если фиксируются данные HTTP-запроса
// * ЛогироватьHTTPОтвет       - Булево                              - Истина, если фиксируются данные HTTP-ответа
...
Функция НоваяСтруктураДанныхЗаписи() Экспорт
	...
	ПараметрыЗаписиЛога.Вставить("HTTPЗапросОтветЗаголовки",  Неопределено);
	ПараметрыЗаписиЛога.Вставить("HTTPЗапросОтветТело",       Неопределено);
	ПараметрыЗаписиЛога.Вставить("HTTPОтветКодСостояния",     Неопределено);
	...
	ПараметрыЗаписиЛога.Вставить("ЛогироватьHTTPЗапрос",      Ложь);
	ПараметрыЗаписиЛога.Вставить("ЛогироватьHTTPОтвет",       Ложь);
	ПараметрыЗаписиЛога.Вставить("ЭмуляцияЗапроса",           Ложь);
...
Процедура Вывести(ДанныеЗаписи, ПараметрыЛогирования) Экспорт
	...
	ИначеЕсли ДанныеЗаписи.ЛогироватьHTTPЗапрос И Не ЗначениеЗаполнено(ТекстОшибки) Тогда
		
		ЗаписьТекста.ЗаписатьСтроку("");
		ЗаписьТекста.ЗаписатьСтроку(ДанныеЗаписи.АдресРесурса);
		
		Если ДанныеЗаписи.ЭмуляцияЗапроса Тогда
			ЗаписьТекста.ЗаписатьСтроку("");
			ЗаписьТекста.ЗаписатьСтроку(НСтр("ru = 'Запрос проксирован через сервис интернет-поддержки';
											|en = 'Запрос проксирован через сервис интернет-поддержки'"));
		КонецЕсли;
		
		Для Каждого КлючИЗначение Из ДанныеЗаписи.HTTPЗапросОтветЗаголовки Цикл
			ЗначениеЗаголовка = КлючИЗначение.Значение;
			ЗаписьТекста.ЗаписатьСтроку(СтрШаблон("%1: %2", КлючИЗначение.Ключ, ЗначениеЗаголовка));
		КонецЦикла;
		
		Тело = ДанныеЗаписи.HTTPЗапросОтветТело;
		
		Если Не ПустаяСтрока(Тело) Тогда
			ЗаписьТекста.ЗаписатьСтроку(Тело);
		КонецЕсли;
		
	ИначеЕсли ДанныеЗаписи.ЛогироватьHTTPОтвет И Не ЗначениеЗаполнено(ТекстОшибки) Тогда
		
		ЗаписьТекста.ЗаписатьСтроку("");
		ЗаписьТекста.ЗаписатьСтроку(СтрШаблон("Код состояния: %1", ДанныеЗаписи.HTTPОтветКодСостояния));
	...
	ИначеЕсли Штрихкод <> Неопределено Тогда
		...
		ДанныеДляЛогирования = ОбщегоНазначения.СкопироватьРекурсивно(Штрихкод);
		ПреобразоватьРекурсивноСоответствияДляПреобразованияВJSON(ДанныеДляЛогирования);

		ПараметрыЗаписи = Новый ПараметрыЗаписиJSON(ПереносСтрокJSON.Нет);
		ЗаписьJSON = Новый ЗаписьJSON();
	
		ЗаписьJSON.УстановитьСтроку(ПараметрыЗаписи);
		ЗаписатьJSON(ЗаписьJSON, ДанныеДляЛогирования, НастройкиСериализации, "ПреобразоватьВФорматДляЗаписиJSON", ЛогированиеЗапросовИС);

		СтрокаJSON = ЗаписьJSON.Закрыть();
	
		ЗаписьТекста.ЗаписатьСтроку("");
		ЗаписьТекста.ЗаписатьСтроку(НСтр("ru = 'Входящие данные с торгового оборудования:';
										|en = 'Входящие данные с торгового оборудования:'"));
		ЗаписьТекста.ЗаписатьСтроку(СтрокаJSON);

	ИначеЕсли РезультатРазбораШтрихКода <> Неопределено Тогда
		
		ДанныеДляЛогирования = ОбщегоНазначения.СкопироватьРекурсивно(РезультатРазбораШтрихКода);
		ПреобразоватьРекурсивноСоответствияДляПреобразованияВJSON(ДанныеДляЛогирования);
		...
		ЗаписьТекста.ЗаписатьСтроку("");
		ЗаписьТекста.ЗаписатьСтроку(НСтр("ru = 'Результаты разбора штрихкодов:';
										|en = 'Результаты разбора штрихкодов:'") + Символы.ПС);
		ЗаписьТекста.ЗаписатьСтроку(СтрокаJSON);
	...
	Если ТипЗнч(СтруктураДанных) = Тип("Структура")
		...
		Для Каждого ЗначениеСтруктуры Из СтруктураДанных Цикл
			...
			ИначеЕсли ТипЗнч(ЗначениеСтруктуры.Ключ) <> Тип("Строка") Тогда
				...
				ЗаписьJSON.УстановитьСтроку(ПараметрыЗаписи);
				ЗаписатьJSON(ЗаписьJSON, ЗначениеСтруктуры.Ключ, НастройкиСериализации, "ПреобразоватьВФорматДляЗаписиJSON", ЛогированиеЗапросовИС);
