	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Розничные продажи';
		                         ...
		                         НСтр("ru = 'Во время создания документа внесения денежных средств в кассу ККМ произошла ошибка.';
										|en = 'An error occurred while creating the document of putting cash into the cash register.'")
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Розничные продажи';
		                         ...
		                         НСтр("ru = 'Во время создания документа выемки денежных средств из кассы ККМ произошла ошибка.';
										|en = 'An error occurred while creating the document of taking cash out of the cash register.'")
		                         + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция УдалитьДокументПриОтменеФискальнойОперации(Ссылка) Экспорт
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		ШаблонТекстаЗаголовкаОшибки = НСтр("ru = 'При отмене фискальной операции не удалось удалить документ %1';
											|en = 'При отмене фискальной операции не удалось удалить документ %1'");

		ЕстьПравоДобавленияИзменения = (ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЧекККМ")
											Или ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЧекККМВозврат"))
										И Пользователи.РолиДоступны("ДобавлениеИзменениеЧековККМ")
									Или (ТипЗнч(Ссылка) = Тип("ДокументСсылка.РеализацияПодарочныхСертификатов")
											Или ТипЗнч(Ссылка) = Тип("ДокументСсылка.ВозвратПодарочныхСертификатов"))
										И Пользователи.РолиДоступны("ДобавлениеИзменениеРеализацийВозвратовПодарочныхСертификатов");
										
		НастройкиПродажДляПользователя = НастройкиПродажДляПользователейСервер.ПраваДоступаРМК(Пользователи.ТекущийПользователь());
		Если ЕстьПравоДобавленияИзменения И НастройкиПродажДляПользователя.КорректировкаСтрок Тогда
			
			ДокументСтатус = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, Ссылка.Метаданные().Реквизиты.Статус.Имя, Истина);
			Если ДокументСтатус = Неопределено Тогда
				
				Результат = Ложь;
				СтрокиТекстаОшибки.Добавить(СтрШаблон(ШаблонТекстаЗаголовкаОшибки, Ссылка));
				СтрокиТекстаОшибки.Добавить(НСтр("ru = 'по причине: Документ не существует или у пользователя недостаточно прав';
												|en = 'по причине: Документ не существует или у пользователя недостаточно прав'"));
				
			ИначеЕсли ДокументСтатус = Перечисления.СтатусыЧековККМ.Пробит Тогда
					
				Результат = Ложь;
				СтрокиТекстаОшибки.Добавить(СтрШаблон(ШаблонТекстаЗаголовкаОшибки, Ссылка));
				СтрокиТекстаОшибки.Добавить(НСтр("ru = 'по причине: Пробитый чек не может быть удален';
												|en = 'по причине: Пробитый чек не может быть удален'"));
			...
			Иначе
				...
				Если Не ДокументОбъект = Неопределено Тогда
					...
					ДокументОбъектНомер = Строка(ДокументОбъект.Номер);
					ДокументОбъектДата = Строка(ДокументОбъект.Дата);
					...
					Исключение
						...
						Результат = Ложь;
						СтрокиТекстаОшибки.Добавить(СтрШаблон(ШаблонТекстаЗаголовкаОшибки, ДокументОбъектТип));
						СтрокиТекстаОшибки.Добавить(ДокументОбъектНомер);
						СтрокиТекстаОшибки.Добавить(ДокументОбъектДата);
						СтрокиТекстаОшибки.Добавить(СтрШаблон(НСтр("ru = 'по причине: %4';
																	|en = 'по причине: %4'", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()))));
		...
		Иначе
			...
			Результат = Ложь;
			СтрокиТекстаОшибки.Добавить(СтрШаблон(ШаблонТекстаЗаголовкаОшибки, Ссылка));
			СтрокиТекстаОшибки.Добавить(НСтр("ru = 'по причине: У пользователя недостаточно прав';
											|en = 'по причине: У пользователя недостаточно прав'"));
			
		КонецЕсли;
		
		Если СтрокиТекстаОшибки.Количество() Тогда
			ИмяСобытияЖурнала = НСтр("ru = 'Розничные продажи';
									|en = 'Retail sales'", ОбщегоНазначения.КодОсновногоЯзыка());
			ТекстОшибки = СтрСоединить(СтрокиТекстаОшибки, " ");
...
// 
// Возвращаемое значение:
//  Неопределено, см. РозничныеПродажиКлиентСерверЛокализация.ДанныеДляВозвратаПоДокументуОплатыСБП
//
...
// Устанавливает значение константы, вызывается из клиентского контекста.
//
//Параметры:
//	НовоеЗначение - см. РозничныеПродажиКлиентСерверЛокализация.ПараметрыПодключенияНСПК
//
Процедура УстановитьЗначениеКонстантыНастройкиОбменаНСПК(НовоеЗначение) Экспорт
	
	НастройкиОбменаНСПК = ЭлектронныеСертификатыНСПКУТ.ОбщиеПараметрыПодключенияНСПК();
	
	Если Не (ОбщегоНазначения.КоллекцииИдентичны(НовоеЗначение, НастройкиОбменаНСПК, , "ПараметрыПроксиНСПК") И
			ОбщегоНазначения.КоллекцииИдентичны(НовоеЗначение.ПараметрыПроксиНСПК, НастройкиОбменаНСПК.ПараметрыПроксиНСПК)) Тогда
		
		Константы.НастройкиОбменаНСПК.Установить(Новый ХранилищеЗначения(НовоеЗначение, Новый СжатиеДанных(9)));
	КонецЕсли;
