					...
					Если ИнтервалСекундЧислом < 2 Тогда
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Показ важных и очень важных новостей будет отложен на 2 секунды
								...
								|en = 'Show important and very important news in 2 seconds
								|(%1 value transferred in the parameter has been increased to 2)'"),
							ИнтервалСекундЧислом);
						ПоказатьОповещениеПользователя(
							НСтр("ru = 'Изменено время первого показа новостей';
								|en = 'Time of the first news display was changed'"),
							,
							ТекстСообщения);
					...
					ИначеЕсли ИнтервалСекундЧислом > 999 Тогда
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Показ важных и очень важных новостей будет отложен на 999 секунд
								...
								|en = 'Show important and very important news in 999 seconds
								|(%1 value transferred in the parameter has been decreased to 999)'"),
							ИнтервалСекундЧислом);
						ПоказатьОповещениеПользователя(
							НСтр("ru = 'Изменено время первого показа новостей';
								|en = 'Time of the first news display was changed'"),
					...
					Иначе
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Показ важных и очень важных новостей будет отложен на %1 секунд';
								|en = 'Show important and very important news in %1 seconds'"),
							ИнтервалСекундЧислом);
						ПоказатьОповещениеПользователя(
							НСтр("ru = 'Изменено время первого показа новостей';
								|en = 'Time of the first news display was changed'"),
				...
				Исключение
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не удалось преобразовать параметр запуска (сдвиг показа новостей): %1.
							...
							|en = 'Cannot convert launch parameter (news display shift): %1. 
							|Parameter line: %2.
							|Specify time in seconds, from 2 to 999'"),
					...
					ПоказатьОповещениеПользователя(
						НСтр("ru = 'Ошибка изменения времени первого показа новостей';
							|en = 'An error occurred while changing time of the first news display'"),
						,
						ТекстСообщения);
				КонецПопытки;
			Иначе
				ТекстСообщения = НСтр("ru = 'Для сдвига показа важных и очень важных новостей,
					...
					|en = 'To shift display of important and very important news,
					|transfer ОтложитьПоказНовостей=ХХ
					|in the launch parameter /C;
					|where XX is the number of seconds from 2 to 99.
					|The parameter must be terminated by a semi-colon.'");
				ПоказатьОповещениеПользователя(
					НСтр("ru = 'Ошибка изменения времени первого показа новостей';
						|en = 'An error occurred while changing time of the first news display'"),
					,
				...
				Исключение
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Произошла ошибка при обработке действия %1 с параметрами %2.
							...
							|%3';
							|en = 'An error occurred when processing the %1 action with the %2 parameters.
					...
					ОбработкаНовостейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru = 'БИП:Новости.Клиент';
							|en = 'ISL:News.Client'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИмяСобытия.
						НСтр("ru = 'Новости. Клиент. ОбработкаНовостейКлиент. ОбработкаНавигационнойСсылки';
							|en = 'News. Client. ОбработкаНовостейКлиент. ОбработкаНавигационнойСсылки'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИдентификаторШага.
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Действие %1 с параметрами %2 не обрабатывается.';
				|en = 'Action %1 with parameters %2 is not processed.'"),
			Действие,
			СписокПараметров);
		ОбработкаНовостейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Клиент';
			...
			ОшибкиПереходаПоСсылке.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка получения BaseURI по причине:
						|%1';
						|en = 'Error retrieving BaseURI. Reason:
						|%1'"),
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
		КонецПопытки;
			...
			ИнформацияПереходаПоСсылке.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Только ссылка без about:blank = %1';
						|en = 'Only link without about:blank = %1'"),
					Href));
		КонецЕсли;
			...
			ИнформацияПереходаПоСсылке.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Попытка поиска элемента с идентификатором [%1]';
						|en = 'Attempt to search for item with ID [%1]'"),
			...
			ОшибкиПереходаПоСсылке.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка получения элемента якоря с идентификатором [%1] по причине:
						|%2';
						|en = 'An error occurred when receiving the anchor item with ID [%1]. Reason:
						|%2'"),
					ИдентификаторЭлемента,
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
		КонецПопытки;
				...
				ИнформацияПереходаПоСсылке.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Попытка перехода к элементу с идентификатором [%1]';
							|en = 'Attempt to go to the item with ID [%1]'"),
						ИдентификаторЭлемента));
				...
				ОшибкиПереходаПоСсылке.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка перехода к якорю с идентификатором [%1] по причине:
							|%2';
							|en = 'An error occurred when going to the anchor with ID [%1]. Reason:
							|%2'"),
						ИдентификаторЭлемента,
						ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
			КонецПопытки;
	...
	Если ОшибкиПереходаПоСсылке.Количество() > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Произошла ошибка при переходе по ссылке [%1].
				...
				|
				|%3';
				|en = 'An error occurred when navigating the link [%1].
				|Detailed error information:
				|%2
				|
				|%3'"),
		...
		ОбработкаНовостейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Клиент';
				|en = 'ISL:News.Client'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Клиент. ОбработкаНовостейКлиент. ПерейтиПоИнтернетСсылке';
				|en = 'News. Client. ОбработкаНовостейКлиент. ПерейтиПоИнтернетСсылке'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИдентификаторШага.
			"Ошибка", // УровеньЖурналаРегистрации строкой
			"", // ОбъектМетаданных строкой
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Произошла ошибка при переходе по ссылке %1.
				|Подробная информация об ошибке:
				|%2';
				|en = 'An error occurred when clicking the %1 link.
				|Detailed error information:
				|%2'"),
			ИнтернетСсылка,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
