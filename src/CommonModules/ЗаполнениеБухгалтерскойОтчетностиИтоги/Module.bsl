...
Функция Счет(УсловныйКодСчета) Экспорт
	...
	Если Результат = ПС.ПустаяСсылка() Тогда
		ОписаниеОшибки = НСтр("ru = 'Не определен счет с условным кодом %1 в соответствии счетов';
								|en = 'Account with conditional code %1 in the account correspondence is not determined'");
...
// Разделенный оборот межотчетного периода по счету 02.01.
// 
// Параметры:
//  НачалоПериода - Дата
//  КонецПериода - Дата
...
//  Структура - Разделенный оборот межотчетного периода по счету 02.01:
// * МОП_ОБ_02_01__84__01_01 - Число -
// * МОП_ОБ_84__02_01__01_01 - Число -
// * МОП_ОБ_02_01__84__01_08 - Число -
// * МОП_ОБ_84__02_01__01_08 - Число -
Функция РазделенныйОборотМежотчетногоПериодаПоСчету_02_01(НачалоПериода, КонецПериода, Организация) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачПериода, &КонПериода, , Счет В (&Счета02_01), &ВидыСубконто, Организация = &Организация, КорСчет В (&Счета84), ) КАК ХозрасчетныйОбороты
	               |
	               |СГРУППИРОВАТЬ ПО
	               ...
	               |				КОГДА ВТ_Сальдо_01_01.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |					ТОГДА 0
	               |				ИНАЧЕ ВТ_КорректировкаАмортизации.СуммаОборотДт
	               |			КОНЕЦ), 0) КАК МОП_ОБ_02_01__84__01_01,
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ВТ_Сальдо_01_01.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |					ТОГДА 0
	               |				ИНАЧЕ ВТ_КорректировкаАмортизации.СуммаОборотКт
	               |			КОНЕЦ), 0) КАК МОП_ОБ_84__02_01__01_01,
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ВТ_Сальдо_01_01.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |						И НЕ ВТ_Сальдо_01_08.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |					ТОГДА ВТ_КорректировкаАмортизации.СуммаОборотДт
	               |				ИНАЧЕ 0
	               |			КОНЕЦ), 0) КАК МОП_ОБ_02_01__84__01_08,
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ВТ_Сальдо_01_01.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |						И НЕ ВТ_Сальдо_01_08.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |					ТОГДА ВТ_КорректировкаАмортизации.СуммаОборотДт
	               |				ИНАЧЕ 0
	               |			КОНЕЦ), 0) КАК МОП_ОБ_84__02_01__01_08
	               |ИЗ
	               |	ВТ_КорректировкаАмортизации КАК ВТ_КорректировкаАмортизации
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Сальдо_01_01 КАК ВТ_Сальдо_01_01
	               |		ПО ВТ_КорректировкаАмортизации.ОС = ВТ_Сальдо_01_01.ОС
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Сальдо_01_08 КАК ВТ_Сальдо_01_08
	               |		ПО ВТ_КорректировкаАмортизации.ОС = ВТ_Сальдо_01_08.ОС";
	...
	Запрос.УстановитьПараметр("Счета01_08", БухгалтерскийУчетПовтИсп.СчетаВИерархии(Счет("01.08")));
	Запрос.УстановитьПараметр("Счета02_01", БухгалтерскийУчетПовтИсп.СчетаВИерархии(Счет("02.01")));
	Запрос.УстановитьПараметр("Счета84", БухгалтерскийУчетПовтИсп.СчетаВИерархии(Счет("84")));
	
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	
	ВозвращаемоеЗначение = Новый Структура;
	ВозвращаемоеЗначение.Вставить("МОП_ОБ_02_01__84__01_01", Выборка.МОП_ОБ_02_01__84__01_01);
	ВозвращаемоеЗначение.Вставить("МОП_ОБ_84__02_01__01_01", Выборка.МОП_ОБ_84__02_01__01_01);
	ВозвращаемоеЗначение.Вставить("МОП_ОБ_02_01__84__01_08", Выборка.МОП_ОБ_02_01__84__01_08);
	ВозвращаемоеЗначение.Вставить("МОП_ОБ_84__02_01__01_08", Выборка.МОП_ОБ_84__02_01__01_08);
	
	Возврат ВозвращаемоеЗначение;
	
КонецФункции

// Разделенный оборот межотчетного периода по счету 02.02.
// 
// Параметры:
//  НачалоПериода - Дата
//  КонецПериода - Дата
...
// 
// Возвращаемое значение:
//  Структура - Разделенный оборот межотчетного периода по счету 02.02:
// * МОП_ОБ_02_02__84__03_01 - Число -
// * МОП_ОБ_84__02_02__03_01 - Число -
// * МОП_ОБ_02_02__84__03_02 - Число -
// * МОП_ОБ_84__02_02__03_02 - Число -
// * МОП_ОБ_02_02__84__03_03 - Число -
// * МОП_ОБ_84__02_02__03_03 - Число -
// * МОП_ОБ_02_02__84__03_04 - Число -
// * МОП_ОБ_84__02_02__03_04 - Число -
Функция РазделенныйОборотМежотчетногоПериодаПоСчету_02_02(НачалоПериода, КонецПериода, Организация) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ПОМЕСТИТЬ ВТ_КорректировкаАмортизации
	               |ИЗ
	               |	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачПериода, &КонПериода, , Счет В (&Счета02_02), &ВидыСубконто, Организация = &Организация, КорСчет В (&Счета84), ) КАК ХозрасчетныйОбороты
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ХозрасчетныйОбороты.Субконто1
	               |
	               ...
	               |					ТОГДА 0
	               |				ИНАЧЕ ВТ_КорректировкаАмортизации.СуммаОборотДт
	               |			КОНЕЦ), 0) КАК МОП_ОБ_02_02__84__03_01,
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ВТ_Сальдо_03_01.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |					ТОГДА 0
	               |				ИНАЧЕ ВТ_КорректировкаАмортизации.СуммаОборотКт
	               |			КОНЕЦ), 0) КАК МОП_ОБ_84__02_02__03_01,
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ВТ_Сальдо_03_01.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |						И НЕ ВТ_Сальдо_03_02.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |					ТОГДА ВТ_КорректировкаАмортизации.СуммаОборотДт
	               |				ИНАЧЕ 0
	               |			КОНЕЦ), 0) КАК МОП_ОБ_02_02__84__03_02,
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ВТ_Сальдо_03_01.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |						И НЕ ВТ_Сальдо_03_02.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |					ТОГДА ВТ_КорректировкаАмортизации.СуммаОборотКт
	               |				ИНАЧЕ 0
	               |			КОНЕЦ), 0) КАК МОП_ОБ_84__02_02__03_02,
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ВТ_Сальдо_03_01.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |						И ВТ_Сальдо_03_02.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |						И НЕ ВТ_Сальдо_03_03.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |					ТОГДА ВТ_КорректировкаАмортизации.СуммаОборотДт
	               |				ИНАЧЕ 0
	               |			КОНЕЦ), 0) КАК МОП_ОБ_02_02__84__03_03,
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ВТ_Сальдо_03_01.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |						И ВТ_Сальдо_03_02.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |						И НЕ ВТ_Сальдо_03_03.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |					ТОГДА ВТ_КорректировкаАмортизации.СуммаОборотКт
	               |				ИНАЧЕ 0
	               |			КОНЕЦ), 0) КАК МОП_ОБ_84__02_02__03_03,
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ВТ_Сальдо_03_01.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |						И ВТ_Сальдо_03_02.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |						И ВТ_Сальдо_03_03.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               ...
	               |					ТОГДА ВТ_КорректировкаАмортизации.СуммаОборотДт
	               |				ИНАЧЕ 0
	               |			КОНЕЦ), 0) КАК МОП_ОБ_02_02__84__03_04,
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ВТ_Сальдо_03_01.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |						И ВТ_Сальдо_03_02.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               |						И ВТ_Сальдо_03_03.СуммаКонечныйОстатокДт ЕСТЬ NULL
	               ...
	               |					ТОГДА ВТ_КорректировкаАмортизации.СуммаОборотКт
	               |				ИНАЧЕ 0
	               |			КОНЕЦ), 0) КАК МОП_ОБ_84__02_02__03_04
	               |ИЗ
	               |	ВТ_КорректировкаАмортизации КАК ВТ_КорректировкаАмортизации
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Сальдо_03_01 КАК ВТ_Сальдо_03_01
	               |		ПО ВТ_КорректировкаАмортизации.ОС = ВТ_Сальдо_03_01.ОС
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Сальдо_03_02 КАК ВТ_Сальдо_03_02
	               |		ПО ВТ_КорректировкаАмортизации.ОС = ВТ_Сальдо_03_02.ОС
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Сальдо_03_03 КАК ВТ_Сальдо_03_03
	               |		ПО ВТ_КорректировкаАмортизации.ОС = ВТ_Сальдо_03_03.ОС
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Сальдо_03_04 КАК ВТ_Сальдо_03_04
	               |		ПО ВТ_КорректировкаАмортизации.ОС = ВТ_Сальдо_03_04.ОС";
	...
	ВозвращаемоеЗначение = Новый Структура;
	ВозвращаемоеЗначение.Вставить("МОП_ОБ_02_02__84__03_01", Выборка.МОП_ОБ_02_02__84__03_01);
	ВозвращаемоеЗначение.Вставить("МОП_ОБ_84__02_02__03_01", Выборка.МОП_ОБ_84__02_02__03_01);
	ВозвращаемоеЗначение.Вставить("МОП_ОБ_02_02__84__03_02", Выборка.МОП_ОБ_02_02__84__03_02);
	ВозвращаемоеЗначение.Вставить("МОП_ОБ_84__02_02__03_02", Выборка.МОП_ОБ_84__02_02__03_02);
	ВозвращаемоеЗначение.Вставить("МОП_ОБ_02_02__84__03_03", Выборка.МОП_ОБ_02_02__84__03_03);
	ВозвращаемоеЗначение.Вставить("МОП_ОБ_84__02_02__03_03", Выборка.МОП_ОБ_84__02_02__03_03);
	ВозвращаемоеЗначение.Вставить("МОП_ОБ_02_02__84__03_04", Выборка.МОП_ОБ_02_02__84__03_04);
	ВозвращаемоеЗначение.Вставить("МОП_ОБ_84__02_02__03_04", Выборка.МОП_ОБ_84__02_02__03_04);
...
Функция РазделенноеСальдоСчета_76_07(КонецПериода, Организация) Экспорт
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&КонПериода, Счет В (&Счета), &ВидыСубконто, Организация = &Организация) КАК ХозрасчетныйОстатки
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Субконто2
	|;
	...
	|	СУММА(вт_СиО_Аренда.СуммаОстатокДт - вт_СиО_Аренда.СуммаОстатокКт) КАК СуммаКонечныйОстатокДт,
	...
	|ВЫБРАТЬ
	|	вт_СиО_Аренда.Счет КАК Счет,
	|	СУММА(вт_СиО_Аренда.СуммаОстатокКт - вт_СиО_Аренда.СуммаОстатокДт) КАК СуммаКонечныйОстатокКт,
	|	вт_СиО_Аренда.ВидСчета КАК ВидСчета
	|ИЗ
	|	вт_СиО_Аренда КАК вт_СиО_Аренда
...
//     * ВидАктива - Перечисления.ВидыАктивовДляРБП - 
