...
Функция ДанныеВходящегоОбъекта(ТранспортныйКонтейнер,
	...
	Если НЕ ЗначениеЗаполнено(ШапкаПакетаЭД.Контрагент)
		...
		ОписаниеОшибки = НСтр("ru = 'Не определен контрагент электронного документа ЕИС.';
								|en = 'Electronic UIS document counterparty is not determined.'");
...
Процедура ПолучитьНовыеДокументыЕИС(
	...
	Если НЕ РезультатПроверки.Выполнено Тогда
		Если РезультатПроверки.Статус = Статусы.ПараметрыАутентификацииНеЗаполнены Тогда
			...
			ПодробноОбОшибке = НСтр("ru = 'Не заполнены параметры авторизации на сервере интернет-поддержки';
									|en = 'Authorization parameters on the online support server are not filled'");
			КраткоОбОшибке = НСтр("ru = 'Не заполнены параметры авторизации';
									|en = 'Authorization parameters are not filled'");
		...
		Иначе
			...
			ПодробноОбОшибке = НСтр("ru = 'Не выполнены условия сопровождения для конфигурации';
									|en = 'Configuration maintenance terms are not fulfilled'");
	...
	ИмяСобытия = НСтр("ru = 'Получение транспортных контейнеров из ЕИС';
						|en = 'Transport containers receipt from the Unified Information System'",
		КодЯзыка);
	Попытка
	...
	Исключение
		ЗаголовокПроблемы = НСтр("ru = 'Ошибка при получении документов поставщика';
								|en = 'An error occurred when receiving the vendor documents'");
		...
		ПодробноОбОшибке = НСтр("ru = 'Произошла критическая ошибка при получении документов поставщика';
								|en = 'A critical error occurred when receiving the vendor documents'");
	...
	Исключение
		ЗаголовокПроблемы = НСтр("ru = 'Ошибка при получении документов заказчика';
								|en = 'An error occurred when receiving the customer documents'");
		ОписаниеОшибки = ИнформацияОбОшибке();
		КраткоеОписание = ОбработкаОшибок.КраткоеПредставлениеОшибки(ОписаниеОшибки);
		...
		ПодробноОбОшибке = НСтр("ru = 'Произошла критическая ошибка при получении документов заказчика';
								|en = 'A critical error occurred when receiving the customer documents'");
...
Процедура СпособыОтраженияВУчетеТипаЭлектронногоДокумента(ТипДокумента, СпособыОтраженияВУчете) Экспорт
	...
	Если ТипДокумента = Перечисления.ТипыДокументовЭДО.КонтрактЕИС Тогда
		СпособыОтраженияВУчете.Добавить("ГосударственныеКонтрактыЕИС", НСтр("ru = 'Государственный контракт ЕИС';
																			|en = 'UIS state contract'"), Истина);
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.ДокументПриемкиЕИСУПД
		...
		СпособыОтраженияВУчете.Добавить("ДокументПриемкиЕИС", НСтр("ru = 'Документ о приемке из ЕИС';
																	|en = 'UIS receipt document'"), Истина);
	КонецЕсли;
...
Функция ОпределитьКонтрагентаЕИС(ДанныеКонтрагента, ИдентификаторУчастника)
	...
	Если НЕ ЗначениеЗаполнено(Контрагент) Тогда
		Шаблон = НСтр("ru = 'Не удалось найти контрагента с ИНН %1 КПП %2.';
						|en = 'Cannot find the counterparty with TIN %1 KPP %2.'");
...
Функция ПолучитьШапкуПакетаЕИСЗаказчика(ОбъектЭД,
	...
	Иначе
		...
		Иначе
			ОписаниеОшибки = НСтр("ru = 'Неизвестный формат документа ЕИС.';
									|en = 'Unknown UIS document format.'");
			ВызватьИсключение(ОписаниеОшибки);
		КонецЕсли;
	КонецЕсли;
	...
	Если ПустаяСтрока(ИдентификаторЗаказчика) Тогда
		...
		Если НЕ ЗначениеЗаполнено(НастройкиОбмена) Тогда
			Шаблон = НСтр("ru = 'Не заполнены настройки обмена с ЕИС для организации %1';
							|en = 'UIS exchange settings are not filled for the %1 company'");
	...
	Если ПустаяСтрока(ИдентификаторУчастника) Тогда
		Шаблон = НСтр("ru = 'Не задан идентификатор участника документооборота организации %1.';
						|en = 'ID of the %1 company data interchange participant is not set.'");
		ОписаниеОшибки = СтрШаблон(Шаблон, Организация);
		ВызватьИсключение(ОписаниеОшибки);
	КонецЕсли;
...
Функция ПолучитьСведенияОЗаказчикеИПодписантах(НастройкиОбмена)
	...
	Если НЕ РезультатЗапросаОЗаказчике.Выполнено Тогда
		...
		ИмяСобытия = НСтр("ru = 'Ошибка при получении данных заказчика из ЛК ЕИС';
							|en = 'An error occurred when receiving the customer data from the personal UIS account'",
			КодОсновногоЯзыка);
...
Процедура ПолучитьДокументыЗаказчика(
	...
	Если НЕ ЗначениеЗаполнено(СтрокаНастройкаОбменаЕИС.ДатаПоследнегоПолученияДокументов) Тогда
		ДатаНачальнаяЗапросаПоДокументам = ЭлектронноеАктированиеЕИС.СтартоваяДатаПолученияДокументовПоУмолчанию();
	Иначе
		ДатаНачальнаяЗапросаПоДокументам = СтрокаНастройкаОбменаЕИС.ДатаПоследнегоПолученияДокументов;
	КонецЕсли;
	...
	КодОсновногоЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
	ИмяСобытия = НСтр("ru = 'Получение из ЕИС документов заказчика';
						|en = 'Receiving customer documents from Unified Information System'", КодОсновногоЯзыка);
	ТекстОшибки = "";
	Пока НЕ ПолученыВсеДокументы Цикл
		...
		Если НЕ РезультатПолученияСпискаДокументов.Выполнено Тогда
			ТекстОшибки = НСтр("ru = 'Ошибка при получении из ЕИС документов заказчика.';
								|en = 'An error occurred when receiving the customer documents from Unified Information System.'");
			Шаблон = НСтр("ru = 'Ошибка при получении из ЕИС документов заказчика %1 за период с %2 по %3.';
							|en = 'An error occurred when receiving the %1 customer documents for the period from %2 to %3 from Unified Information System.'");
			Шаблон = ТекстОшибки + Символы.ПС + "%4";
			...
			ТекстОшибкиЖурнал = СтрШаблон(Шаблон,
				СтрокаНастройкаОбменаЕИС.КодПоСПЗ,
				СтрокаДатаНачальная,
				СтрокаДатаКонечная,
				РезультатПолученияСпискаДокументов.ОписаниеОшибки);
			ЗаписьЖурналаРегистрации(ИмяСобытия,
				УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстОшибкиЖурнал);
			Прервать;
		...
		Если МассивОписанийДокументов = Неопределено Тогда
			ТекстОшибки = НСтр("ru = 'Список объектов для заказчика не определен.';
								|en = 'The customer object list is not specified.'");
			Шаблон = НСтр("ru = 'Ошибка при получении документов заказчика из ЕИС за период с %1 по %2:';
							|en = 'An error occurred when receiving the customer documents for the period from %1 to %2 from Unified Information System:'");
			ТекстОшибкиЖурнал = СтрШаблон(Шаблон,
				ДатаНачальнаяЗапросаПоДокументам,
				ДатаКонечнаяЗапросаПоДокументам);
			ТекстОшибкиЖурнал = ТекстОшибкиЖурнал + Символы.ПС + НСтр("ru = 'Список объектов заказчика не определен.';
																		|en = 'The customer object list is not specified.'");
			ЗаписьЖурналаРегистрации(ИмяСобытия,
				УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстОшибкиЖурнал);
		...
		ИсключаемыеВидыДокументов = ИсключаемыеПриЗагрузкеВидыДокументовДляЗаказчика();
		Для Каждого ОписаниеДокумента Из МассивОписанийДокументов Цикл

			Если НЕ ИсключаемыеВидыДокументов.Найти(ОписаниеДокумента.ВидДокумента) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			Если НЕ ПроверитьОписаниеДокументаЗаказчика(ОписаниеДокумента) Тогда
				ТекстОшибки = НСтр("ru = 'Не заполнен идентификатор эл.документа заказчика по гос.контракту.';
									|en = 'ID of the electronic customer document for the state contract is not filled.'");
				ТекстОшибкиЖурнал = НСтр("ru = 'Не задан идентификатор электронного документа в данных, полученных c сервера ЛК ЕИС';
										|en = 'The electronic document ID is not specified in the data received from the personal UIS account server'");
				ЗаписьЖурналаРегистрации(ИмяСобытия,
					УровеньЖурналаРегистрации.Ошибка, , ,
					ТекстОшибкиЖурнал);
				Прервать;
			...
			Если НЕ РезультатПолученияЭД.Выполнено Тогда
				Шаблон = НСтр("ru = 'Не удалось получить документ заказчика от %1:';
								|en = 'Cannot get the customer document dated %1:'")
					 + Символы.ПС + РезультатПолученияЭД.ОписаниеОшибки;
				ТекстОшибки = СтрШаблон(Шаблон, ОписаниеДокумента.ДатаДокумента);
				Шаблон = НСтр("ru = 'Не удалось получить документ заказчика %1 от %2 вида %3:';
								|en = 'Cannot get the %1 customer document dated %2 of the %3 kind:'")
					+ Символы.ПС + "%4";
				ТекстОшибкиЖурнал = СтрШаблон(Шаблон,
					ОписаниеДокумента.Идентификатор,
					ОписаниеДокумента.ДатаДокумента,
					ОписаниеДокумента.ВидДокумента,
					РезультатПолученияЭД.ОписаниеОшибки);
				ЗаписьЖурналаРегистрации(ИмяСобытия,
					УровеньЖурналаРегистрации.Ошибка, , ,
					ТекстОшибкиЖурнал);
				Прервать;
			...
			Если ДвоичныеДанныеДокумента = Неопределено Тогда
				ТекстОшибки = НСтр("ru = 'Отсутствуют данные электронного документа заказчика.';
									|en = 'Electronic customer document data is missing.'");
				Шаблон = НСтр("ru = 'Отсутствуют данные электронного документа заказчика с ИД %1';
								|en = 'No data of the electronic customer document with ID %1'");
				ТекстОшибкиЖурнал = СтрШаблон(Шаблон, ОписаниеДокумента.Идентификатор);
				ЗаписьЖурналаРегистрации(ИмяСобытия,
					УровеньЖурналаРегистрации.Ошибка, , ,
					ТекстОшибкиЖурнал);
				Прервать;
			...
			Если ШапкаПакетаЭД.ТипЭлементаВерсииЭД = ТипЭлементаВерсииИнформацияПокупателяУПД()
				...
				Если ЗначениеЗаполнено(КонтейнерТитулаПоставщика) Тогда
					...
					Если НЕ РезультатИзмененияСтатуса Тогда
						ТекстОшибки = НСтр("ru = 'Не удалось изменить статус транспортного контейнера заказчика.';
											|en = 'Cannot change the customer transport container status.'");
						ЗаписьЖурналаРегистрации(ИмяСобытия,
							УровеньЖурналаРегистрации.Ошибка, , ,
							ТекстОшибки);
						Прервать;
			...
			Исключение
				...
				ТекстОшибки = НСтр("ru = 'Ошибка сохранение транспортного контейнера ЕИС:';
									|en = 'An error occurred when saving the transport container of Unified Information System:'")
					+ Символы.ПС + ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				Комментарий = НСтр("ru = 'Ошибка при сохранении транспортного контейнера документа ЕИС в сфере закупок:';
									|en = 'An error occurred when saving the transport container of the UIS document in procurement:'")
					+ Символы.ПС
				...
				Прервать;
			КонецПопытки;
			ДатаЗагруженногоДокумента = УниверсальнаяДатаДокументаЗаказчика(ОписаниеДокумента.ДатаДокумента);
		КонецЦикла;
...
Функция ПолучитьШапкуПакетаЕИС(ОбъектЭД, Организация, Контрагент = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(НастройкиОбменаЕИС) Тогда
		Шаблон = НСтр("ru = 'Не определены настройки обмена с ЕИС для организации %1.';
						|en = 'UIS exchange settings are not determined for %1 company.'");
		ОписаниеОшибки = СтрШаблон(Шаблон, Организация);
		ВызватьИсключение(ОписаниеОшибки);
	КонецЕсли;
	...
	Для Каждого СтрокаНастройкаОбменаЕИС Из ТаблицаНастроекОбменаЕИС Цикл
		...
		Если НЕ РезультатЗапросаОПоставщике.Выполнено Тогда
			...
			ИмяСобытия = НСтр("ru = 'Ошибка при получении данных поставщика из ЛК ЕИС';
								|en = 'An error occurred when receiving the vendor data from the personal UIS account'",
				КодОсновногоЯзыка);
...
Функция ИсключаемыеПриЗагрузкеВидыДокументовДляЗаказчика()
	...
	Пока НЕ ПолученыВсеДокументы Цикл
		...
		Если НЕ РезультатПолученияСпискаДокументов.Выполнено Тогда
			...
			ИмяСобытия = НСтр("ru = 'Ошибка при получении из ЕИС документов поставщика %1 за период с %2 по %3.';
								|en = 'An error occurred when receiving the %1 vendor documents for the period from %2 to %3 from UIS.'",
				КодОсновногоЯзыка);
		...
		Если МассивОписанийДокументов = Неопределено Тогда
			...
			ИмяСобытия = НСтр("ru = 'Ошибка при получении документов поставщика из ЕИС за период с %1 по %2';
								|en = 'An error occurred when receiving the vendor documents for the period from %1 to %2 from UIS.'",
				КодОсновногоЯзыка);
			ИмяСобытия = СтрШаблон(ИмяСобытия,
				ДатаНачальнаяЗапросаПоДокументам,
				ДатаКонечнаяЗапросаПоДокументам);
			Комментарий = НСтр("ru = 'Список объектов не определен.';
								|en = 'Object list is not identified.'");
			ЗаписьЖурналаРегистрации(ИмяСобытия,
				УровеньЖурналаРегистрации.Ошибка, , ,
				Комментарий);
			Прервать;
		...
		Для Каждого ОписаниеДокумента Из МассивОписанийДокументов Цикл
			...
			Если НЕ ЗначениеЗаполнено(ОписаниеДокумента.Идентификатор) Тогда
				...
				ИмяСобытия = НСтр("ru = 'Не заполнен идентификатор эл.документа поставщика по гос.контракту.';
									|en = 'ID of the digital vendor document for the state contract is not filled.'",
					КодОсновногоЯзыка);
			...
			Если НЕ РезультатПолученияЭД.Выполнено Тогда
				...
				ИмяСобытия =
					НСтр("ru = 'Произошла ошибка при получении из ЕИС данных электронного документа вида %1 с ИД %2.';
						|en = 'An error occurred when receiving the data of the electronic document of the %1 kind with ID %2 from UIS.'",
						КодОсновногоЯзыка);
			...
			Если ДвоичныеДанныеДокумента = Неопределено Тогда
				...
				ИмяСобытия = НСтр("ru = 'Отсутствуют данные электронного документа из ЕИС с ИД %1';
									|en = 'No data of the electronic document with ID %1 from UIS'",
					КодОсновногоЯзыка);
...
Процедура ПолучитьКонтракты(МассивКонтейнеров,
	...
	Если НЕ ЗначениеЗаполнено(СтрокаНастройкаОбменаЕИС.ДатаПоследнегоПолученияКонтрактов) Тогда
		ДатаВремяЗапросаКонтрактов = ЭлектронноеАктированиеЕИС.СтартоваяДатаПолученияКонтрактовПоУмолчанию();
	Иначе
		ДатаВремяЗапросаКонтрактов = СтрокаНастройкаОбменаЕИС.ДатаПоследнегоПолученияКонтрактов;
	КонецЕсли;
	...
	Пока Не ПолученыВсеКонтракты Цикл
		...
		Если НЕ РезультатПолученияСпискаКонтрактов.Выполнено Тогда
			...
			ИмяСобытия = НСтр("ru = 'Ошибка при получении контрактов из ЕИС в сфере закупок';
								|en = 'An error occurred when receiving the contracts from UIS in procurement'",
				КодОсновногоЯзыка);
			...
			Если НЕ ОписаниеКонтракта.ДанныеКонтракта = Неопределено Тогда
				...
				Исключение
					...
					ИмяСобытия = НСтр("ru = 'Сохранение транспортного контейнера контракта ЕИС в сфере закупок';
										|en = 'Save the UIS contract transport container in procurement'",
						КодЯзыка);
					ИнформацияОбОшибке = ИнформацияОбОшибке();
					Комментарий = ОбработкаОшибок.ПодробноеПредставлениеОшибки(
						ИнформацияОбОшибке);
...
Функция УниверсальнаяДатаДокументаЗаказчика(ДатаДокумента)
	
	УниверсальнаяДатаДокумента = УниверсальноеВремя(ДатаДокумента);
	ЧасовойПоясМСК = "Europe/Moscow";
	Возврат УниверсальноеВремя(УниверсальнаяДатаДокумента, ЧасовойПоясМСК);
	...
	Пока ВыборкаОтправок.Следующий() Цикл
			...
			Если КонтейнерНайденный = Неопределено Тогда
				...
				Исключение
					...
					ТекстОшибки = НСтр("ru = 'Ошибка при получении пакета с квитанцией ЕИС: %1';
										|en = 'An error occurred when receiving the package with the UIS receipt: %1'");
		...
		Иначе
			...
			ТекстОшибки = НСтр("ru = 'Ошибка при получении пакета %2 с квитанцией ЕИС: %1';
								|en = 'An error occurred when receiving the %2 package with the UIS receipt: %1'");
...
Функция ВидОшибкиНеВыполненыУсловияСопровождения()
	...
	ВидОшибки.ОписаниеРешения = НСтр("ru = '<a href = ""Проверка"">Проверьте</a> параметры интернет-поддержки';
									|en = '<a href = ""Check "">Check </a> online support parameters'");
...
Функция ВидОшибкиСбойПриПолученииДокументов(Идентификатор, ЗаголовокПроблемы, ТекстОшибки)
	...
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Произошла ошибка:';
										|en = 'Error occurred:'") + Символы.ПС + ТекстОшибки;
...
Функция ВидОшибкиНетДанныхАвторизации()
	...
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Не заполнены параметры подключения к интернет-поддержке';
										|en = 'Online support connection parameters are not filled'");
	ВидОшибки.ОписаниеРешения = НСтр("ru = '<a href = ""Выполните"">Выполните</a> диагностику интернет-поддержки';
									|en = '<a href = ""Выполните"">Diagnose</a> online support'");
