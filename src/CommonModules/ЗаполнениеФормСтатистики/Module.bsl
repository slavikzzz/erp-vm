...
Процедура ЗаполнитьФормуСтатистики(Контейнер, Знач ИмяОтчета, Знач ИмяРедакцииОтчета, ПараметрыОтчета) Экспорт
	...
	Иначе	
		...
		ЗаполнениеФормСтатистикиПоОбособленнымПодразделениямПереопределяемый.ДобавитьФормыЗаполняемыеВЦеломПоОрганизации(ФормыЗаполняемыеПоВсейОрганизации);
	...
	Иначе	
		...
		Если ФормыЗаполняемыеПоКаждомуОП.Найти(ИмяОтчета) <> Неопределено Тогда	
			...
			Если ПараметрыОтчета.Свойство("ОбособленноеПодразделение") 
				...
				ЗаполнениеФормСтатистикиПоОбособленнымПодразделениямПереопределяемый.ЗаполнитьОтборПоОбособленнымПодразделениям(
...
Процедура ЗаполнитьСписокАвтозаполняемыхПолей(ЗаполняемыеПоля, Знач ИмяОтчета, ИмяРедакцииОтчета, Период) Экспорт
	...
	// В наименовании ячеек многострочных разделов используется суффикс вида "_[номер строки]".
	// В то же время в справочнике полей форм статистики имена заполняемых полей (ИмяПоля) указаны без суффикса.
	// Сделаем поправку на это: очистим суффикс в именах незаполняемых ячеек.
	Для каждого ИмяПоля Из НеЗаполняемыеПоля Цикл
		НачалоНомераСтроки = СтрНайти(ИмяПоля, "_");
		Если НачалоНомераСтроки > 0 Тогда
			ИмяПоля = Лев(ИмяПоля, НачалоНомераСтроки - 1);
		КонецЕсли;	
...
Функция ПодготовитьВнешниеДанные(МакетКомпоновки, ВнешниеДанные) Экспорт
	...
	Для Каждого ОписаниеОбъекта Из ОбъектыВнешнихНаборовДанных(МакетКомпоновки.НаборыДанных) Цикл
		...
		ОбъектВнешнихДанных = ЗаполнениеФормСтатистикиПереопределяемый.ВнешниеДанные(ИмяОбъекта, МакетКомпоновки);
...
Функция СериализоватьРезультатНастройки(КомпоновщикНастроек) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Сохранение настроек форм статистики';
				|en = 'Save statistics form settings'", Метаданные.ОсновнойЯзык.КодЯзыка),
