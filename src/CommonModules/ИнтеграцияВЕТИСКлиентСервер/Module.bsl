...
Функция ПараметрыФормыРедактированияПериода() Экспорт 
	...
	Результат.Вставить("Заголовок",               НСтр("ru = 'Редактирование даты или периода';
														|en = 'Редактирование даты или периода'"));
...
Функция СформироватьНадписьПоДаннымТабличнойЧасти(Данные, ПараметрыНадписи) Экспорт
	...
	Если СчетчикСтрок > ПараметрыНадписи.Количество Тогда
		СтрокаРезультата = СтрШаблон(НСтр("ru = '%1 (+ еще %2...)';
											|en = '%1 (+ еще %2...)'"), СтрокаРезультата, СчетчикСтрок - ПараметрыНадписи.Количество);
	КонецЕсли;
...
Функция СтруктураПараметрыПередачи(ДальнейшееДействие = Неопределено) Экспорт
	...
	Возврат ИнтеграцияИСКлиентСервер.ИнициализироватьПараметрыПередачиДанных(ДальнейшееДействие);
...
// 
// Параметры:
//  ДеревоХозяйствующихСубъектовПредприятийВЕТИС - ДеревоЗначений
//  ВсеСтроки - Булево - Все строки
//  ФормироватьХэш - Булево - Формировать хэш
//  ВсеПредприятияВыбраны - Булево - Все предприятия выбраны
// 
// Возвращаемое значение:
//  Структура - Дерево отбора организации ВЕТИСВМассив структур:
// * ВыбранныеЗначения - Массив из Структура - 
// * ХэшСтроки - Строка - 
// * ПредставлениеОрганизации - Число, Произвольный - 
// * ПредставлениеПредприятия - Число, Произвольный - 
...
Функция ПредставлениеВыбранныхНастроек(ДеревоОтбора, ХэшСтроки = "", СписокВыбора = Неопределено) Экспорт 
	...
	Если ТипЗнч(ОписаниеВыбора.ПредставлениеОрганизации) = Тип("Число") Тогда
		...
		Иначе
			ПредставлениеОрганизации = НСтр("ru = ';%1 организация;;%1 организации;%1 организаций;%1 организаций';
											|en = ';%1 организация;;%1 организации;%1 организаций;%1 организаций'");
	...
	Если ТипЗнч(ОписаниеВыбора.ПредставлениеПредприятия) = Тип("Число") Тогда
		...
		Иначе
			ПредставлениеПредприятия = НСтр("ru = ';%1 предприятие;;%1 предприятия;%1 предприятий;%1 предприятий';
											|en = ';%1 предприятие;;%1 предприятия;%1 предприятий;%1 предприятий'");
	...
	Иначе
		Возврат СтрШаблон(НСтр("ru = '%1 (%2)';
								|en = '%1 (%2)'"), ПредставлениеОрганизации, ПредставлениеПредприятия);
	КонецЕсли;
...
// Результат выполнения запроса элемента классификатора.
// 
// Возвращаемое значение:
//  Структура - Результат выполнения запроса элемента классификатора:
// * Элемент - Произвольный
// * ТекстОшибки - Строка
// * СтатусОбработки - ПеречислениеСсылка
// * РезультатОтправкиЗапроса - Структура
// * РезультатОбработкиОтвета - Структура - :
...
// Результат выполнения запроса элементов классификатора.
// 
// Возвращаемое значение:
//  Структура - Результат выполнения запроса элементов классификатора:
// * Список - Произвольный
// * ОбщееКоличество - Число
// * НомерСтраницы - Число
// * КоличествоСтраниц - Число
// * ТекстОшибки - Строка
// * РезультатОтправкиЗапроса - Произвольный
// * РезультатОбработкиОтвета - Структура - :
// ** Результат - Произвольный
// ** ТекстОшибки - Строка
...
//
// Возвращаемое значение:
//  Булево - Необходим отбор по дальнейшему действию ВЕТИСПеред загрузкой из настроек
...
Функция ЭтоШтриховойКодЕАN13(ШтрихКод) Экспорт
	...
	ИначеЕсли МенеджерОборудованияКлиентСервер.РассчитатьКонтрольныйСимволGTIN13(ШтрихКод) <> Прав(ШтрихКод, 1) Тогда
		...
		СтруктураВозврата.ТекстОшибки = НСтр("ru = 'Контрольная цифра не соответствует рассчитанной';
											|en = 'Контрольная цифра не соответствует рассчитанной'");
...
Функция ПредставлениеПоляДатаПроизводства(ТипЖивыеЖивотные) Экспорт
	...
	Возврат ?(ТипЖивыеЖивотные = Истина, НСтр("ru = 'Дата рождения';
												|en = 'Дата рождения'"), НСтр("ru = 'Дата производства';
...
Функция ПодсказкаКСостояниюОбмена(ИдентификаторПроблемы, Элемент = Неопределено) Экспорт
		...
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru = 'Синхронизация данных может занять неприемлемое время (в зависимости от количества предприятий - от 1 часа
				         ...
				         |en = 'Синхронизация данных может занять неприемлемое время (в зависимости от количества предприятий - от 1 часа
				         |до нескольких дней). Рекомендуется заново'")));
		СтрокаЗаголовка.Добавить(" ");
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				Нстр("ru = 'запросить остатки и непогашенные ВСД';
					|en = 'запросить остатки и непогашенные ВСД'"),,,, "ЗапроситьОстаткиИНепогашенныеВСД"));
		СтрокаЗаголовка.Добавить(".");
	...
	ИначеЕсли ИдентификаторПроблемы = "ЕстьПроблемыAPLM0012ПриСинхронизацииЗаписейСкладскогоЖурнала" Тогда
		...
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru = 'Последние попытки получения записей складского журнала приводят к ошибке';
					|en = 'Последние попытки получения записей складского журнала приводят к ошибке'")));
		СтрокаЗаголовка.Добавить(" ");
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				"APLM0012",
		...
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru = 'Попробуйте увеличить количество попыток, уменьшить количество элементов в пакете
				         ...
				         |en = 'Попробуйте увеличить количество попыток, уменьшить количество элементов в пакете
				         |и уменьшить интервал получения изменений в панели'")));
		СтрокаЗаголовка.Добавить(" ");
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				Нстр("ru = 'Параметры оптимизации';
					|en = 'Параметры оптимизации'"),,,, "ПараметрыОптимизации"));
		СтрокаЗаголовка.Добавить(" ");
		...
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				Нстр("ru = 'выполнить обмен';
					|en = 'выполнить обмен'"),,,, "ВыполнитьОбмен"));
		СтрокаЗаголовка.Добавить(".");
		...
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru = 'Последние попытки получения ветеринарно-сопроводительных документов приводят к ошибке';
					|en = 'Последние попытки получения ветеринарно-сопроводительных документов приводят к ошибке'")));
		СтрокаЗаголовка.Добавить(" ");
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				"APLM0012",
		...
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru = 'Возможно, что в процессе обмена происходят ошибки времени выполнения. Рекомендуется провести анализ';
					|en = 'Возможно, что в процессе обмена происходят ошибки времени выполнения. Рекомендуется провести анализ'")));
		СтрокаЗаголовка.Добавить(" ");
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				Нстр("ru = 'журнала регистрации';
					|en = 'журнала регистрации'"),,,, "e1cib/app/Обработка.ЖурналРегистрации"));
		СтрокаЗаголовка.Добавить(Символы.ПС);
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru = 'по событиям ВетИС на предмет наличия ошибок.';
					|en = 'по событиям ВетИС на предмет наличия ошибок.'")));
		...
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru = 'Синхронизация записей складского журнала требуется для поддержания журнала в актуальном состоянии,
				           ...
				           |en = 'Синхронизация записей складского журнала требуется для поддержания журнала в актуальном состоянии,
				           |особенно если ведется работа в Web-интерфейсе ФГИС Меркурий. Рекомендуется'")));
		СтрокаЗаголовка.Добавить(" ");
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
		...
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru = 'Синхронизация ветеринарно-сопроводительных документов используется для получения входящих
				           ...
				           |Рекомендуется';
				           |en = 'Синхронизация ветеринарно-сопроводительных документов используется для получения входящих
				           |и возвратных документов, для обновления статусов исходящих ВСД при их гашении получателями.
				           |Рекомендуется'")));
		СтрокаЗаголовка.Добавить(" ");
		СтрокаЗаголовка.Добавить(
		...
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru = 'Не все подготовленные для отправки сообщения доставлены в ФГИС Меркурий.
					       |Рекомендуется';
					       |en = 'Не все подготовленные для отправки сообщения доставлены в ФГИС Меркурий.
					       |Рекомендуется'")));
		СтрокаЗаголовка.Добавить(" ");
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
		...
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				Нстр("ru = 'Запись синхронизации не используется для обмена с ФГИС Меркурий. Рекомендуется';
					|en = 'Запись синхронизации не используется для обмена с ФГИС Меркурий. Рекомендуется'")));
		СтрокаЗаголовка.Добавить(" ");
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				Нстр("ru = 'Удалить запись';
					|en = 'Удалить запись'"),,,, "УдалитьЗаписьСинхронизации"));
		СтрокаЗаголовка.Добавить(".");
		...
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				Нстр("ru = 'Синхронизация классификатора не выполнялась длительное время. Рекомендуется';
					|en = 'Синхронизация классификатора не выполнялась длительное время. Рекомендуется'")));
		СтрокаЗаголовка.Добавить(" ");
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				Нстр("ru = 'выполнить синхронизацию';
					|en = 'выполнить синхронизацию'"),,,, "ВыполнитьОбмен"));
		СтрокаЗаголовка.Добавить(".");
		...
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				Нстр("ru = 'Не заполнен регистр сведений ""Виды продукции по группам ВетИС""
				         ...
				         |en = 'Не заполнен регистр сведений ""Виды продукции по группам ВетИС""
				         |или регистр сведений ""Допустимые цели по группам ВетИС"".
				         |При оформлении исходящих транспортных операций ВетИС недоступны цели.'")));
		СтрокаЗаголовка.Добавить("");
		СтрокаЗаголовка.Добавить(
			Новый ФорматированнаяСтрока(
				Нстр("ru = 'Заполнить';
					|en = 'Заполнить'"),,,, "ЗаполнитьСлужебныеРегистры"));
		СтрокаЗаголовка.Добавить(".");
