...
//   ФормированиеЭлектронногоДокумента - Булево - если Истина, то проверка при формировании ЭД, иначе - при пробитии чека.
Процедура ПроверитьЗаполнениеАкцизныхМарокПоСсылкам(Источники, Отказ, НаДату = Неопределено, СообщатьОбОшибках = Истина, ФормированиеЭлектронногоДокумента = Ложь) Экспорт
...
//   СообщатьОбОшибках - Булево - Сообщать об ошибках.
//   ФормированиеЭлектронногоДокумента - Булево - если Истина, то проверка при формировании ЭД, иначе - при пробитии чека.
Процедура ПроверитьЗаполнениеАкцизныхМарок(Объект, Отказ, НаДату = Неопределено, РеквизитыОбъекта = Неопределено, СообщатьОбОшибках = Истина, ФормированиеЭлектронногоДокумента = Ложь) Экспорт
	...
	Для Каждого СтрокаТовары Из Объект[ИмяТаблицыТовары] Цикл
		...
		Если КоличествоМарок <> КоличествоЕдиницТоваров Тогда
			...
			Шаблон = НСтр("ru = 'Товара ""%1"" по документу - %2 %4, по кодам маркировки - %3 %4.';
							|en = 'Товара ""%1"" по документу - %2 %4, по кодам маркировки - %3 %4.'");
			...
			Для Каждого СтрокаТовар Из СтрокиТоваров Цикл
				...
				Если КоличествоМарок <> КоличествоЕдиницТоваров Тогда
					...
					Шаблон = НСтр("ru = 'Для маркировки ""%1"" по серии должно быть указано единиц - %2, а указано - %3.';
									|en = 'Для маркировки ""%1"" по серии должно быть указано единиц - %2, а указано - %3.'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						Шаблон,
						ПредставлениеНоменклатуры,
						КоличествоЕдиницТоваров,
...
// Возвращает признак, является ли участник обмена индивидуальным предпринимателем.
// 
// Параметры:
//  ОрганизацияКонтрагент - СправочникСсылка.Контрагенты, СправочникСсылка.Организации - участник обмена
// 
// Возвращаемое значение:
//  Булево - Истина, если партнер является индивидуальным предпринимателем.
Функция ПартнерОрганизацияЭтоИндивидуальныйПредприниматель(ОрганизацияКонтрагент) Экспорт
	
	ЭтоИндивидуальныйПредприниматель = Ложь;
	
	Если ТипЗнч(ОрганизацияКонтрагент) = Тип("СправочникСсылка.Склады") Тогда
		
		ЭтоИндивидуальныйПредприниматель = Ложь;
		Возврат ЭтоИндивидуальныйПредприниматель;
		
	ИначеЕсли ТипЗнч(ОрганизацияКонтрагент) = Тип("СправочникСсылка.РегистрацииВНалоговомОргане") Тогда
	...
	ЮрФизЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОрганизацияКонтрагент, "ЮрФизЛицо");
...
Процедура ОпределитьШаблоныКодовМаркировкиСУЗТекстаЗапроса(ТекстЗапроса,
	...
	|		КОГДА %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ЛегкаяПромышленность2025)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.ЛегкаяПромышленность)
	...
	|			ИЛИ %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ДезинфицирующиеСредства)
	...
	|			ИЛИ %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.БАДы2025)
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.КреслаКоляски)
	|		КОГДА %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.МедицинскиеИзделия20)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.КреслаКоляски)
	...
	|		КОГДА %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.КормаДляЖивотныхВлажныеБезВЕТИС)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.КормаДляЖивотныхБезВЕТИС)
	|		КОГДА %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.КормаДляЖивотныхВлажныеПодконтрольныеВЕТИС)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.КормаДляЖивотных)
	...
	|			ИЛИ %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ТелефоныИНоутбуки)
	|			ИЛИ %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПечатныеПлаты)
	...
	|		КОГДА %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПолимерныеТрубы)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.ПолимерныеТрубы)
	|		КОГДА %1.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.СладостиИКондитерскиеИзделия)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.СладостиИКондитерскиеИзделия)
...
Функция ТекстЗапросаДвижениеПродукцииИМатериалов() Экспорт
	
	ТекстЗапроса = ТекстЗапросаРеализацияТоваровУслуг();
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РеализацияТоваровУслуг", "ДвижениеПродукцииИМатериалов");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТаблицаТовары.Склад", "ИСТИНА");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТаблицаСерии.Склад", "ИСТИНА");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Ссылка.ЗаказКлиента", "Ссылка.Распоряжение");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ЗаказКлиента", "ЗаказМатериаловВПроизводство");
	Возврат ТекстЗапроса;
...
#Область ТоварыОрдера

Функция ТекстЗапросаПроизводствоБезЗаказа() Экспорт
	...
	|	ТаблицаТовары.Серия             КАК Серия,
	|	СУММА(ТаблицаТовары.Количество) КАК Количество
	|ПОМЕСТИТЬ ТоварыПроизводствоБезЗаказа
	|ИЗ
	...
	|/////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТоварыПроизводствоБезЗаказа.Ссылка КАК Ссылка,
	|	ТоварыПроизводствоБезЗаказа.Номенклатура КАК Номенклатура,
	|	ТоварыПроизводствоБезЗаказа.Характеристика КАК Характеристика
	|ПОМЕСТИТЬ ПроизводствоБезЗаказаБезСерий
	|ИЗ
	|	ТоварыПроизводствоБезЗаказа КАК ТоварыПроизводствоБезЗаказа
	|ГДЕ
	|	ТоварыПроизводствоБезЗаказа.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|;
	|
	|/////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТоварыПоРаспоряжениям.Ссылка КАК Ссылка,
	|	ПроизводствоБезЗаказаБезСерий.Ссылка КАК ПроизводствоБезЗаказа
	|ПОМЕСТИТЬ РасходныеОрдераПроизводствоБезЗаказа
	|ИЗ
	|	ПроизводствоБезЗаказаБезСерий КАК ПроизводствоБезЗаказаБезСерий
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РасходныйОрдерНаТовары.ТоварыПоРаспоряжениям КАК ТоварыПоРаспоряжениям
	|		ПО ПроизводствоБезЗаказаБезСерий.Ссылка = ТоварыПоРаспоряжениям.Распоряжение
	|;
	|
	|/////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РасходныеОрдера.ПроизводствоБезЗаказа КАК Ссылка,
	|	РасходныеОрдера.ПроизводствоБезЗаказа.Организация КАК ОрганизацияПолучатель,
	|	ДвиженияСерийТоваров.Номенклатура КАК Номенклатура,
	|	ДвиженияСерийТоваров.Характеристика КАК Характеристика,
	|	ДвиженияСерийТоваров.Серия КАК Серия,
	|	СУММА(ДвиженияСерийТоваров.КоличествоОборот) КАК Количество
	|ПОМЕСТИТЬ СкладскиеДвиженияПроизводствоБезЗаказа
	|ИЗ
	|	РегистрНакопления.ДвиженияСерийТоваров.Обороты(,,, Документ В
	|		(ВЫБРАТЬ
	|			Ссылка
	|		Из
	|			РасходныеОрдераПроизводствоБезЗаказа)) КАК ДвиженияСерийТоваров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РасходныеОрдераПроизводствоБезЗаказа КАК РасходныеОрдера
	|		ПО РасходныеОрдера.Ссылка = ДвиженияСерийТоваров.Документ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПроизводствоБезЗаказаБезСерий КАК ПроизводствоБезЗаказаБезСерий
	|		ПО ДвиженияСерийТоваров.Номенклатура = ПроизводствоБезЗаказаБезСерий.Номенклатура
	|		И ДвиженияСерийТоваров.Характеристика = ПроизводствоБезЗаказаБезСерий.Характеристика
	|		И РасходныеОрдера.ПроизводствоБезЗаказа = ПроизводствоБезЗаказаБезСерий.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	РасходныеОрдера.ПроизводствоБезЗаказа,
	|	ДвиженияСерийТоваров.Номенклатура,
	|	ДвиженияСерийТоваров.Характеристика,
	|	ДвиженияСерийТоваров.Серия
	|ИМЕЮЩИЕ
	...
	|	Товары.Серия                 КАК Серия,
	|	Товары.Количество            КАК Количество
	|ПОМЕСТИТЬ ТаблицаТоварыПроизводствоБезЗаказа
	|ИЗ
	|	ТоварыПроизводствоБезЗаказа КАК Товары
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	СкладскиеДвиженияПроизводствоБезЗаказа.ОрганизацияПолучатель,
	|	СкладскиеДвиженияПроизводствоБезЗаказа.Номенклатура,
	|	СкладскиеДвиженияПроизводствоБезЗаказа.Характеристика,
	|	СкладскиеДвиженияПроизводствоБезЗаказа.Серия,
	|	СкладскиеДвиженияПроизводствоБезЗаказа.Количество
	|ИЗ
	|	СкладскиеДвиженияПроизводствоБезЗаказа КАК СкладскиеДвиженияПроизводствоБезЗаказа
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ТекстЗапросаЭтапПроизводства2_2() Экспорт
	
	Результат = ТекстЗапросаКорректировкаРеализации();
	Результат = СтрЗаменить(Результат, "Расхождения", "РасходМатериаловИРабот");
	Результат = СтрЗаменить(Результат, "КорректировкаРеализации", "ЭтапПроизводства2_2");
	Возврат Результат;
...
Функция СерииУказываютсяПоFEFO(Основание) Экспорт
	...
	Если ТипОснование = Тип("ДокументСсылка.ВнутреннееПотребление")
		...
		//++ НЕ УТ
		Или ТипОснование = Тип("ДокументСсылка.ВыкупПринятыхНаХранениеТоваров")
		Или ТипОснование = Тип("ДокументСсылка.ДвижениеПродукцииИМатериалов")
		Или ТипОснование = Тип("ДокументСсылка.ОтгрузкаТоваровСХранения")
		//++ Устарело_Переработка24
		Или ТипОснование = Тип("ДокументСсылка.ПередачаСырьяПереработчику")
		//-- Устарело_Переработка24
		...
		//++ Устарело_Переработка24
		Или ТипОснование = Тип("ДокументСсылка.ВозвратСырьяДавальцу")
		Или ТипОснование = Тип("ДокументСсылка.ПередачаДавальцу")
		//-- Устарело_Переработка24
	...
	//++ НЕ УТ
	ИначеЕсли ТипОснование = Тип("ДокументСсылка.ПроизводствоБезЗаказа")
		Тогда ИмяТабличнойЧасти = ".МатериалыИРаботы";
	//-- НЕ УТ

	//++ НЕ УТКА
	ИначеЕсли ТипОснование = Тип("ДокументСсылка.ЭтапПроизводства2_2")
		Тогда ИмяТабличнойЧасти = ".РасходМатериаловИРабот";
	//-- НЕ УТКА
	КонецЕсли;
	...
	Если Не РезультатЗапроса[0].Пустой() Тогда
		ПараметрыКопированияДополнительныхДанных.Добавить("КопироватьСвязьСКлассификаторомАлкогольнойПродукцииЕГАИС",
			НСтр("ru = 'Копировать связь с классификатором алкогольной продукции ЕГАИС';
				|en = 'Копировать связь с классификатором алкогольной продукции ЕГАИС'"),
			Истина);
	КонецЕсли;
	...
	Если Не РезультатЗапроса[1].Пустой() Тогда 
		ПараметрыКопированияДополнительныхДанных.Добавить("КопироватьСвязьСКлассификаторомПродукцииВЕТИС",
			НСтр("ru = 'Копировать связь с классификатором продукции ВЕТИС';
				|en = 'Копировать связь с классификатором продукции ВЕТИС'"),
			Истина);
	КонецЕсли;
...
Процедура ЗаполнитьПоПараметрамСоздания(Объект, ПараметрыСоздания) Экспорт
	...
	Если Не ЗначениеЗаполнено(Объект.ВидНоменклатуры)
		...
		|	ИЛИ ВидыНоменклатуры.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.КормаДляЖивотныхВлажныеПодконтрольныеВЕТИС)
...
#Область Команды
	...
	КомандаФормы.Заголовок = НСтр("ru = 'Настроить параметры подключения к ЕГАИС';
									|en = 'Настроить параметры подключения к ЕГАИС'");
...
#Область Элементы
	...
	КолонкаИнтерфейса.Заголовок = НСтр("ru = 'Параметры подключения к ЕГАИС';
										|en = 'Параметры подключения к ЕГАИС'");
...
#Область Реквизиты
	...
	Если ЕстьЕГАИС Или ЕстьИСМП Тогда
		...
		Если ОбщегоНазначенияИСВызовСервера.ВидыПродукцииИспользующиеВскрытыеПотребительскихУпаковок(,, Ложь).Количество() Тогда
			Реквизит = Новый РеквизитФормы("ЕстьВскрытыеПотребительскиеУпаковкиИС", Новый ОписаниеТипов("Булево"), "Объект.Товары");
			ДобавляемыеРеквизиты.Добавить(Реквизит);
			ПараметрыИнтеграцииГосИС.Вставить("ВедетсяУчетВидовПродукцииСЧастичнымВыбытиемИС", Истина);
		Иначе
			ПараметрыИнтеграцииГосИС.Вставить("ВедетсяУчетВидовПродукцииСЧастичнымВыбытиемИС", Ложь);
		КонецЕсли;
...
#Область Команды
	...
	КомандаФормы.Заголовок = НСтр("ru = 'Заполнить по документам маркировки';
									|en = 'Заполнить по документам маркировки'");
	КомандаФормы.ИзменяетСохраняемыеДанные = Истина;
	
	ИмяПодменюЗаполнить = "ТоварыЗаполнить";
	//++ НЕ УТ
	Если Форма.ИмяФормы = "Документ.ПроизводствоБезЗаказа.Форма.ФормаДокумента" Тогда
		ИмяПодменюЗаполнить = "ВыходныеИзделияГруппаЗаполнить";
	КонецЕсли;
	//-- НЕ УТ

	//++ НЕ УТКА
	Если Форма.ИмяФормы = "Документ.ЭтапПроизводства2_2.Форма.ФормаДокумента" Тогда
		ИмяПодменюЗаполнить = "ВыходныеИзделия_Заполнить";
	КонецЕсли;
	//-- НЕ УТКА
		...
		Если Не Справочники.УпаковкиЕдиницыИзмерения.ЭтоМернаяЕдиница(Объект.ЕдиницаИзмерения, ТипИзмеряемойВеличины)
			...
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'При продаже пива на разлив необходимо использовать единицу хранения ""л (дм3)""';
					|en = 'При продаже пива на разлив необходимо использовать единицу хранения ""л (дм3)""'"),,
			...
			ИначеЕсли Форма.ПотребительскаяУпаковкаИС = Форма.УпаковкаЧастичногоВыбытияИС Тогда
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru = 'Упаковка частичного выбытия не может совпадать с потребительской';
						|en = 'Упаковка частичного выбытия не может совпадать с потребительской'"),,,
			...
			ИначеЕсли КоэффициентыПоВидамУпаковок.КоэффициентУпаковкиЧастичногоВыбытия >= КоэффициентыПоВидамУпаковок.КоэффициентПотребительскойУпаковки Тогда
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru = 'Упаковка частичного выбытия должна иметь меньшее количество товара чем потребительская';
						|en = 'Упаковка частичного выбытия должна иметь меньшее количество товара чем потребительская'"),,,
					"УпаковкаЧастичногоВыбытияИС",
					Отказ);
			КонецЕсли;
...
Функция КоэффициентыПоВидамУпаковок(Форма)
	...
	Если ЗначениеЗаполнено(Форма.ПотребительскаяУпаковкаИС)
		...
		РеквизитыПотребительской = ЗначенияРеквизитов.Получить(Форма.ПотребительскаяУпаковкаИС);
		Если ЗначениеЗаполнено(РеквизитыПотребительской.Знаменатель) Тогда
			ВозвращаемоеЗначение.КоэффициентПотребительскойУпаковки = РеквизитыПотребительской.Числитель / РеквизитыПотребительской.Знаменатель;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(РеквизитыПотребительской.КоличествоУпаковок) Тогда
			ВозвращаемоеЗначение.КоэффициентПотребительскойУпаковки = ВозвращаемоеЗначение.КоэффициентПотребительскойУпаковки * РеквизитыПотребительской.КоличествоУпаковок;
		КонецЕсли;
	КонецЕсли;
		...
		Если Форма.ИспользуетсяЧастичноеВыбытиеИС Тогда
			...
			Иначе
				ВызватьИсключение НСтр("ru = 'Коэффициент упаковки частичного выбытия не должен = 0.';
										|en = 'Коэффициент упаковки частичного выбытия не должен = 0.'")
			КонецЕсли;
		Иначе
...
Процедура ДобавитьЭлементыНастройкиВидаУпаковокПриСоздании(Форма) Экспорт
		...
		ТекстПодсказки = Новый ФорматированнаяСтрока(
			СтрШаблон(
				...
				НСтр("ru = 'не используется, для настройки необходимо использовать упаковки.';
					|en = 'не используется, для настройки необходимо использовать упаковки.'")),,
	...
	Если ЭтоНоменклатура Тогда
		...
		Команда.Заголовок = НСтр("ru = 'Заполнить по данными ГИС МТ';
								|en = 'Заполнить по данными ГИС МТ'");
		...
		Команда.Подсказка = НСтр("ru = 'Выполняет получение емкости упаковки ГИС МТ и подбор подходящей упаковки номенклатуры.';
								|en = 'Выполняет получение емкости упаковки ГИС МТ и подбор подходящей упаковки номенклатуры.'");
		...
		НовыйЭлемент.Заголовок = Новый ФорматированнаяСтрока(
			НСтр("ru = 'Виды упаковок по GTIN';
				|en = 'Виды упаковок по GTIN'"),,,, "ГиперссылкаОткрытьФормуНастройкиВидовУпаковокПоGTINИСМП");
