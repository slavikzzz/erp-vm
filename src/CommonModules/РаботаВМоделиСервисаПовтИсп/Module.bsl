...
Функция РазделенныеОбъектыМетаданных(Знач Разделитель) Экспорт
	...
	Если МетаданныеОбщегоРеквизита = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Общий реквизит %1 не обнаружен в конфигурации.';
																						|en = 'Common attribute %1 is not found in the configuration.'"), Разделитель);
	КонецЕсли;
	...
	Если МетаданныеОбщегоРеквизита.РазделениеДанных = Метаданные.СвойстваОбъектов.РазделениеДанныхОбщегоРеквизита.Разделять Тогда
		...
		НеИспользоватьОбщийРеквизит = (МетаданныеОбщегоРеквизита.АвтоИспользование = Метаданные.СвойстваОбъектов.АвтоИспользованиеОбщегоРеквизита.НеИспользовать);
		...
		Для Каждого ЭлементСостава Из СоставОбщегоРеквизита Цикл
			...
			Использование = ЭлементСостава.Использование;
			
			// При отключенном разделении для всех объектов расширения использование общего реквизита Авто
			Если Не РазделениеВключено
				И ЭлементСостава.Метаданные.РасширениеКонфигурации() <> Неопределено Тогда
				
				Использование = Авто;
				
			КонецЕсли;
			
			Если НеИспользоватьОбщийРеквизит И Использование = Авто ИЛИ Использование = НеИспользовать Тогда
				
				Продолжить;
				
			КонецЕсли;
			
			ДополнительныеДанные = НовыйДополнительныеДанные(ЭлементСостава.Метаданные.Имя, Разделитель, Неопределено);
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для общего реквизита %1 не используется разделение данных.';
																						|en = 'Data separation is not used for the %1 common attribute.'"), Разделитель);
	...
	Для Каждого МетаданныеПоследовательности Из Метаданные.Последовательности Цикл
		...
		ДополнительныеДанные = НовыйДополнительныеДанные(МетаданныеПоследовательности.Имя, Разделитель, Неопределено);
	...
	Для Каждого МетаданныеЖурналаДокументов Из Метаданные.ЖурналыДокументов Цикл
		...
		ДополнительныеДанные = НовыйДополнительныеДанные(МетаданныеЖурналаДокументов.Имя, Разделитель, Неопределено);
...
// Новый дополнительные данные.
// 
// Параметры:
//  Имя - Строка
//  Разделитель - Число
//  УсловноеРазделение - Строка
// 
// Возвращаемое значение:
//  Структура:
// * Имя - Строка
// * Разделитель - Число
// * УсловноеРазделение - Строка
Функция НовыйДополнительныеДанные(Имя, Разделитель, УсловноеРазделение = Неопределено)
	
	ДополнительныеДанные = Новый Структура;
	ДополнительныеДанные.Вставить("Имя", Имя);
	ДополнительныеДанные.Вставить("Разделитель", Разделитель);
	ДополнительныеДанные.Вставить("УсловноеРазделение", УсловноеРазделение);
