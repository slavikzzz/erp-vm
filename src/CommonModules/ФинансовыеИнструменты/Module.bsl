	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыКредитовИДепозитов") Тогда
		ЗакрытиеМесяцаСервер.УстановитьСостояниеОтключено(
			...
			НСтр("ru = 'Договоры кредитов и депозитов не используются.';
				|en = 'Loan and deposit contracts are not used.'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат;
	КонецЕсли;
	...
	Если РазмерыВременныхТаблиц.Графики = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет графиков начислений и оплат по договорам кредитов и депозитов.';
				|en = 'No schedules of accruals and payments under credit and deposit contracts.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Иначе
		...
		ЗакрытиеМесяцаСервер.ЗафиксироватьРезультатыОбработчикаИспользованияПоОрганизациям(
			...
			НСтр("ru = 'По организации ""%1"" за период %2 есть отклонения в плановых и фактических начислениях по договорам кредитов и депозитов.';
				|en = 'There are variances in the planned and actual accruals under credit and deposit contracts of the ""%1"" company for period %2.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			Запрос,
			"Отклонения",
			Ложь,
			Истина);
...
Функция ПроверитьВходящиеПараметрыЧИСТВНДОХ(Платежи)
	...
	Если Не (ЕстьПоложительнаяСумма И ЕстьОтрицательнаяСумма) Тогда
		ТекстСообщения = НСтр("ru = 'Невозможно рассчитать приблизительную годовую ставку IRR. Проверьте корректность графика платежей: по крайней мере один платеж в графике должен быть положительный и один отрицательный.';
								|en = 'Cannot calculate an approximate yearly IRR rate. Check the correctness of the payment schedule: at least one payment in the schedule must be positive and one negative.'");
