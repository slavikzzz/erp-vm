...
Процедура ОбработкаРасшифровки(ЭтаФорма, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	//++ НЕ УТ
	ИначеЕсли ПолноеИмяОтчета = "Отчет.СправкаРасчетДисконтированияРезервовПредстоящихРасходов" Тогда
		
		СтандартнаяОбработка = Ложь;		
		
		СтруктураРасшифровки = ОтчетыУТВызовСервераПереопределяемый.СформироватьДанныеРасшифровки(Расшифровка, ЭтаФорма.ОтчетДанныеРасшифровки);
		
		Если СтруктураРасшифровки.Свойство("ОбъектУчетаРезервов") И ЗначениеЗаполнено(СтруктураРасшифровки.ОбъектУчетаРезервов) Тогда
			
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("ОбъектУчетаРезервов", СтруктураРасшифровки.ОбъектУчетаРезервов);
			
			ПараметрПериод = ЭтаФорма.Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Период"));
			ПараметрыФормы.Вставить("Период", ПараметрПериод.Значение);
						
			ПараметрРегистратор = КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(ЭтаФорма.Отчет.КомпоновщикНастроек.Настройки, "Регистратор");
			
			Если ПараметрРегистратор.Значение = Неопределено Тогда
				ПараметрРегистратор = КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(ЭтаФорма.Отчет.КомпоновщикНастроек.ПользовательскиеНастройки, "Регистратор");				
			КонецЕсли;
			
			Если ПараметрРегистратор <> Неопределено Тогда
				ПараметрыФормы.Вставить("Регистратор", ПараметрРегистратор.Значение);				
			КонецЕсли;
...
Процедура ОбработкаДополнительнойРасшифровки(ЭтаФорма, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ПолноеИмяОтчета = "Отчет.СебестоимостьТоваров" Тогда
		...
		ПараметрыОтчета.Вставить("Заголовок", НСтр("ru = 'Ведомость по партиям товаров - только при ФИФО(скользящая оценка)';
													|en = 'Lot list - only with FIFO (rolling valuation)'"));
	...
	ИначеЕсли ПолноеИмяОтчета = "Отчет.ТоварыСхемыОбеспечения" Тогда
		...
		ПараметрыДействия.Вставить("Заголовок", НСтр("ru = 'Изменить способ обеспечения потребностей';
													|en = 'Change the procurement strategy'"));
	...
	Если Не ЗначениеЗаполнено(Документ) Тогда
		...
		Если ТипЗнч(Документ) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Договор не записан. Переход к отчету возможен только после записи договора.';
																	|en = 'The contract is not saved. You can go to the report only after you save the contract.'"));
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Документ не проведен. Переход к отчету возможен только после проведения документа.';
																	|en = 'The document is not posted. You can access the report only after you post the document.'"));
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьРасшифровкуКонтрольОформленияДокументовТовародвижения(
	...
	ИначеЕсли СтруктураРекомендаций.ОформитеВозвратМатериаловИзПроизводства Тогда
		...
		ПараметрыОткрытияФормы.Вставить("Распоряжения", ПараметрыОбработкиРасшифровки.Распоряжения.ВыгрузитьЗначения());
		ПараметрыОткрытияФормы.Вставить("СтруктураБыстрогоОтбора", Новый Структура);
		ПараметрыОткрытияФормы.СтруктураБыстрогоОтбора.Вставить("ОтборХозяйственныеОперации", Новый СписокЗначений);
		СписокОтборХозяйственныеОперации = ПараметрыОткрытияФормы.СтруктураБыстрогоОтбора.ОтборХозяйственныеОперации; // СписокЗначений - 
		СписокОтборХозяйственныеОперации.Добавить(
			ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратМатериаловИзПроизводства"));
		ПараметрыОткрытияФормы.СтруктураБыстрогоОтбора.Вставить("ОтборТипыДокументов", Новый СписокЗначений);
		СписокОтборТипыДокументов = ПараметрыОткрытияФормы.СтруктураБыстрогоОтбора.ОтборТипыДокументов; // СписокЗначений - 
		СписокОтборТипыДокументов.Добавить(
			ОтчетыУТВызовСервераПереопределяемый.ИдентификаторДвижениеПродукцииИМатериалов());
	...
	ИначеЕсли СтруктураРекомендаций.ОформитеВыпускПродукции Тогда
		...
		// В рабочем месте оформления выпуска (производство 2.1) не поддерживается отбор по заказам на производство
		ОткрытьФорму("Документ.ВыпускПродукции.Форма.ФормаСпискаДокументов",, ЭтаФорма,,,, ОписаниеОповещения);
		//-- Устарело_Производство21
	...
	ИначеЕсли СтруктураРекомендаций.ОформитеВозвратСырьяОтПереработчика Тогда
		...
		ПараметрыОткрытияФормы = Новый Структура;
		ПараметрыОткрытияФормы.Вставить("Распоряжения", ПараметрыОбработкиРасшифровки.Распоряжения.ВыгрузитьЗначения());
		ПараметрыОткрытияФормы.Вставить("ИмяТекущейСтраницы", "СтраницаВозвратыСырья");
	...
	ИначеЕсли СтруктураРекомендаций.ОформитеПоступлениеОтПереработчика Тогда
		...
		ПараметрыОткрытияФормы = Новый Структура;
		ПараметрыОткрытияФормы.Вставить("Распоряжения", ПараметрыОбработкиРасшифровки.Распоряжения.ВыгрузитьЗначения());
		ПараметрыОткрытияФормы.Вставить("ИмяТекущейСтраницы", "СтраницаПоступления");
		ОткрытьФорму("Обработка.ЖурналДокументовПереработки.Форма.СписокДокументыПередачиКОформлению",
			ПараметрыОткрытияФормы, ЭтаФорма,,,, ОписаниеОповещения);
		//-- Устарело_Переработка24
	...
	ИначеЕсли СтруктураРекомендаций.ОформитеПоступлениеСырьяОтДавальца Тогда
		...
		ПараметрыОткрытияФормы = Новый Структура;
		ПараметрыОткрытияФормы.Вставить("Распоряжения", ПараметрыОбработкиРасшифровки.Распоряжения.ВыгрузитьЗначения());
		ПараметрыОткрытияФормы.Вставить("ИмяТекущейСтраницы", "СтраницаПоступленияСырья");
	...
	ИначеЕсли СтруктураРекомендаций.ОформитеПередачуМатериаловВПроизводствоВСтатусеПринято Тогда
		...
		ПараметрыОткрытияФормы.Вставить("Отбор", Новый Структура);
		ПараметрыОткрытияФормы.Отбор.Вставить("Ссылка", ПараметрыОбработкиРасшифровки.Распоряжения);
		ОткрытьФорму("Документ.ПередачаМатериаловВПроизводство.Форма.ФормаСписка",
			ПараметрыОткрытияФормы, ЭтаФорма,,,, ОписаниеОповещения);
			//-- Устарело_Производство21
