	...
	Если ЗначениеЗаполнено(КодСтроки) Тогда
		...
		Иначе
			...
			ТекстОшибки = НСтр("ru = 'Дублируется значение реквизита ""Код строки"" строки %1 и строки %2 заказа.
				...
				|en = 'The ""Line code"" attribute value of line %1 and line %2 of the order has a duplicate.
...
Функция СкорректироватьСтрокиЗаказа(ЗаказОбъект, СтруктураДополнительныхПараметров) Экспорт
	...
	ИначеЕсли ТипЗнч(ЗаказОбъект) = Тип("ДокументОбъект.ЗаказПереработчику") Тогда
		...
		СтруктураКорректировкиСтрокЗаказа.ПроверятьОстатки = 
			ЗаказОбъект.Статус <> Перечисления.СтатусыЗаказовПереработчикам.НеСогласован
			И ЗаказОбъект.Статус <> Перечисления.СтатусыЗаказовПереработчикам.КОбеспечению;
		...
		КонецЕсли;
	//-- Устарело_Переработка24
	ИначеЕсли ТипЗнч(ЗаказОбъект) = Тип("ДокументОбъект.ЗаказПереработчику2_5") Тогда
	...
	ИначеЕсли ТипЗнч(ЗаказОбъект) = Тип("ДокументОбъект.ЗаказДавальца") Тогда
		...
		СтруктураКорректировкиСтрокЗаказа.ПроверятьОстатки = 
			ЗаказОбъект.Статус <> Перечисления.СтатусыЗаказовДавальцев.НеСогласован
			И ЗаказОбъект.Статус <> Перечисления.СтатусыЗаказовДавальцев.Согласован;
		...
		КонецЕсли;
	//-- Устарело_Переработка24
	ИначеЕсли ТипЗнч(ЗаказОбъект) = Тип("ДокументОбъект.ЗаказДавальца2_5") Тогда
	...
	ИначеЕсли ТипЗнч(ЗаказОбъект) = Тип("ДокументОбъект.ЗаказНаРемонт") Тогда
		
		СтруктураКорректировкиСтрокЗаказа.ИмяТабличнойЧасти = "МатериалыИРаботы";
		СтруктураКорректировкиСтрокЗаказа.ПроверятьОстатки = 
			ЗаказОбъект.Статус <> Перечисления.СтатусыЗаказовНаРемонт.Создан;
	...
	ИначеЕсли ТипЗнч(ЗаказОбъект) = Тип("ДокументОбъект.ЭтапПроизводства2_2") Тогда
		...
		СтруктураКорректировкиСтрокЗаказа.ПроверятьОстатки = 
			ЗаказОбъект.Статус <> Перечисления.СтатусыЭтаповПроизводства2_2.Формируется;
...
Функция ОтменитьНеотработанныеСтрокиПоПриемке(ДокументОбъект, ПараметрыЗаполнения, ПараметрыОтмены) Экспорт
	...
	Для Каждого Строка Из ДокументОбъектТЧ Цикл
		Если (ПараметрыОтмены.УдалятьСтроки Или Не Строка.Отменено)
			//++ НЕ УТКА
			И Не (ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Строка, "Действие")
				И Строка.Действие = Перечисления.ВариантыДействийПоПерерабатываемымМатериалам.ИспользоватьИмеющиеся)
			//-- НЕ УТКА
			Тогда
			НоваяСтрока = ТаблицаВыбранныхСтрок.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
...
Функция РезультатОтменыНеотработанныхСтрок(ОтмененоСтрок, Модифицированность = Ложь) Экспорт
	...
	Если ОтмененоСтрок = Неопределено Тогда
		РезультатОтмены.Оповещение = НСтр("ru = 'Отмена неотработанных строк не выполнена';
											|en = 'Cannot cancel unprocessed lines'");
	...
	ИначеЕсли ОтмененоСтрок = 0 Тогда
		РезультатОтмены.Оповещение = НСтр("ru = 'Строки не отменены, поскольку полностью отработаны';
											|en = 'Lines were not canceled as they are not fully processed'");
		РезультатОтмены.КоличествоСтрок = 0;
	Иначе
		РезультатОтмены.Оповещение = СтрШаблон(НСтр("ru = 'Отменено неотработанных строк: %1';
													|en = 'Canceled unprocessed lines: %1'"), ОтмененоСтрок);
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Не удалось заблокировать объект при автоматической корректировке строк мерных товаров.';
					|en = 'Cannot lock the object on automatic adjustment of lines with measured goods.'", КодОсновногоЯзыка),
		...
		Если ЗаказИзменен Тогда
			Попытка
				...
				ТекстСообщения = НСтр("ru = 'Выполнена корректировка строк мерных товаров по заказу %Заказ%';
										|en = 'Adjustment of lines with measured goods against the %Заказ% order is executed'", КодОсновногоЯзыка);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Заказ%", Выборка.Заказ);
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Окончание корректировки строк мерных товаров.';
						|en = 'End of adjustment of lines with measured goods.'", КодОсновногоЯзыка),
					УровеньИнформация,
					,
					Выборка.Заказ,
					ТекстСообщения);
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru = 'Не удалось выполнить корректировку строк
				                            ...
				                            |against the %Заказ% order. Reason: %Причина%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Заказ%", Выборка.Заказ);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Фоновая корректировка строк заказов мерных товаров';
												|en = 'Background adjustment of lines of orders with measured goods'", КодОсновногоЯзыка),
		...
		Иначе
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru = 'Не удалось удалить %Заказ% из очереди на корректировку строк
				                            ...
				                            |en = 'Cannot delete %Заказ% from adjustment of lines with measured goods
