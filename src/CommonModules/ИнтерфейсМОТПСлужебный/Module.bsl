	...
	ВызватьИсключение
		СтрШаблон(
			НСтр("ru = 'Неизвестный статус участника: %1';
				|en = 'Неизвестный статус участника: %1'"),
			ЗначениеПоиска);
	...
	ВызватьИсключение
		СтрШаблон(
			НСтр("ru = 'Неизвестный тип операции движения кода маркировки: %1';
				|en = 'Неизвестный тип операции движения кода маркировки: %1'"),
			ЗначениеПоиска);
...
Функция РасшифроватьТокенJWT(Токен) Экспорт
	...
	ВозвращаемоеЗначение.Данные = ОбменДаннымиИСМПКлиентСервер.ТекстJSONВОбъект(
...
Функция ОписаниеНомераТипаДанных(МетаданныеXDTO)
	...
	Иначе
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Внутренняя ошибка. Не описан номер типа данных для %1';
				|en = 'Внутренняя ошибка. Не описан номер типа данных для %1'"), МетаданныеXDTO);
	КонецЕсли;
