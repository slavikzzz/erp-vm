...
Процедура ЗаписатьНастройкиВнутреннегоЭДО(Организация, Настройки, НеактуальныеВидыДокументов = Неопределено, Отказ = Ложь) Экспорт 
	...
	Исключение
		ОтменитьТранзакцию();
		Отказ = Истина;
		ИмяСобытия = НСтр("ru = 'Запись настроек внутреннего электронного документооборота';
							|en = 'Write intra-company EDI settings'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура УдалитьНастройкуВнутреннегоЭДО(Организация) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ИмяСобытия = НСтр("ru = 'Удаление настройки внутреннего документооборота';
							|en = 'Delete intra-company EDI setting'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ДобавитьНастройкуВнутреннегоЭДО(ПараметрыНастройки) Экспорт
	...
	Исключение
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Создание настроек внутреннего электронного документооборота';
														|en = 'Create intra-company EDI settings'"),
...
Функция ШаблонУведомленияОбИспользованииПЭП_УНЭП(ПараметрыЗаполнения) Экспорт
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Не удалось заполнить значения параметров шаблона положения об использовании простой ЭП причине:';
							|en = 'Cannot fill in values of the parameters of the simple electronic signature practice statement template due to:'")
...
Функция ШаблонПоложенияОбИспользованииПЭП_УНЭП(Параметры) Экспорт
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Не удалось заполнить значения параметров шаблона положения об использовании простой ЭП причине:';
							|en = 'Cannot fill in values of the parameters of the simple electronic signature practice statement template due to:'") + Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
