		...
		Иначе
			...
			Элемент.Заголовок = ?(Параметры.ИспользоватьНакладныеПоНесколькимЗаказам,
				...
				НСтр("ru = 'Подобрать товары по заказу';
					|en = 'Pick goods against the order'"));
...
Процедура ЗаполнитьСостоянияНакладной(СписокВыбора, ТолькоАктивные = Ложь) Экспорт
	...
	Иначе
		СписокВыбора.Добавить("0", НСтр("ru = 'Все оформлено';
										|en = 'Fully issued/received'"),          Ложь, БиблиотекаКартинок.ВыполненоУспешно);
		СписокВыбора.Добавить("1", НСтр("ru = 'Создать накладную';
										|en = 'Create invoice'"),      Ложь, БиблиотекаКартинок.СоздатьНакладную);
		СписокВыбора.Добавить("2", НСтр("ru = 'Дооформить накладную';
										|en = 'Pending issue/receipt'"),   Ложь, БиблиотекаКартинок.ДооформитьНакладную);
		СписокВыбора.Добавить("4", НСтр("ru = 'Не используются заказы';
										|en = 'Not applicable'"), Ложь, БиблиотекаКартинок.ПустаяКартинка);
	КонецЕсли;
...
Процедура ЗаполнитьСостоянияОрдера(СписокВыбора, ТолькоАктивные = Ложь) Экспорт
	...
	Иначе
		СписокВыбора.Добавить("0", НСтр("ru = 'Соответствуют накладным';
										|en = 'Notes correspond to the invoices'"),    Ложь, БиблиотекаКартинок.ВыполненоУспешно);
		СписокВыбора.Добавить("1", НСтр("ru = 'Не оформлены';
										|en = 'Pending creation'"),               Ложь, БиблиотекаКартинок.СоздатьНакладную);
		СписокВыбора.Добавить("3", НСтр("ru = 'Не соответствуют накладным';
										|en = 'Received vs invoiced mismatch'"), Ложь, БиблиотекаКартинок.НесоответствиеОрдерНакладная);
		СписокВыбора.Добавить("4", НСтр("ru = 'Не используются';
										|en = 'Not applicable'"),            Ложь, БиблиотекаКартинок.ПустаяКартинка);
	КонецЕсли;
...
Функция ТекстЗапросаСостояний(
	...
	//++ НЕ УТ
	ИначеЕсли ИмяТаблицыСостояний = "СостоянияПередачВПроизводство" Тогда
		Возврат ТекстЗапросаСостоянияПередачВПроизводство(ПараметрыОтбора, Распоряжения, ТолькоНаличиеКОформлению, ПараметрыЗамены);
	//++ Устарело_Переработка24
	ИначеЕсли ИмяТаблицыСостояний = "СостоянияОтчетовПереработчика" Тогда
		Возврат ТекстЗапросаСостоянияОтчетовПереработчика(ПараметрыОтбора, ТолькоНаличиеКОформлению);
	//-- Устарело_Переработка24
	ИначеЕсли ИмяТаблицыСостояний = "СостоянияОтчетовПереработчика2_5" Тогда
		Возврат ТекстЗапросаСостоянияОтчетовПереработчика2_5(ПараметрыОтбора, Распоряжения, ТолькоНаличиеКОформлению);
	ИначеЕсли ИмяТаблицыСостояний = "СостоянияПоступленийОтПереработчика2_5" Тогда
		Возврат ТекстЗапросаСостоянияПоступленийОтПереработчика2_5(ПараметрыОтбора, Распоряжения, ТолькоНаличиеКОформлению);
	//++ НЕ УТКА
	ИначеЕсли ИмяТаблицыСостояний = "СостоянияПередачИзПроизводства" Тогда
		Возврат ТекстЗапросаСостоянияПередачИзПроизводства(ПараметрыОтбора, Распоряжения, ТолькоНаличиеКОформлению, ПараметрыЗамены);
	//++ Устарело_Переработка24
	ИначеЕсли ИмяТаблицыСостояний = "СостоянияОтчетовДавальцам" Тогда
		Возврат ТекстЗапросаСостоянияОтчетовДавальцам(ПараметрыОтбора, ТолькоНаличиеКОформлению);
	//-- Устарело_Переработка24
	ИначеЕсли ИмяТаблицыСостояний = "СостоянияПередач" Тогда
		Возврат ТекстЗапросаСостоянияПередач(ПараметрыОтбора, Распоряжения, ТолькоНаличиеКОформлению);
	ИначеЕсли ИмяТаблицыСостояний = "СостоянияОтчетовДавальцам2_5" Тогда
		Возврат ТекстЗапросаСостоянияОтчетовДавальцам2_5(ПараметрыОтбора, Распоряжения, ТолькоНаличиеКОформлению);
	//-- НЕ УТКА

	//-- НЕ УТ
	Иначе
		ВызватьИсключение НСтр("ru = 'Ошибка формирования текста запроса состояний: некорректное имя таблицы состояний.';
								|en = 'An error occurred while generating state request text: incorrect state table name.'");;
	КонецЕсли;
...
Функция СостоянияПоХозОперациям(ХозОперации) Экспорт
	...
	Для каждого ХО Из ХозОперации Цикл
		...
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.СписаниеТоваровПоТребованию Тогда
			...
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ЗаказНаВнутреннееПотребление);
			//++ НЕ УТКА
			ДобавитьСостояние(Состояния, "СостоянияВнутреннихПотреблений", Метаданные.Документы.ЗаказНаРемонт);
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ЗаказНаРемонт);
			//-- НЕ УТКА
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ВнутреннееПотребление);
		...
		//++ НЕ УТ
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПередачаВПроизводство Тогда
			ДобавитьСостояние(Состояния, "СостоянияПередачВПроизводство", Метаданные.Документы.ЗаказМатериаловВПроизводство);
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ЗаказМатериаловВПроизводство);
			//++ Устарело_Производство21

			//++ НЕ УТКА
			ДобавитьСостояние(Состояния, "СостоянияПередачВПроизводство", Метаданные.Документы.ЗаказНаПроизводство);
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ЗаказНаПроизводство);
			//-- НЕ УТКА
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ПередачаМатериаловВПроизводство);
			//-- Устарело_Производство21
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПередачаМатериаловВКладовую Тогда
			ДобавитьСостояние(Состояния, "СостоянияПередачВПроизводство", Метаданные.Документы.ЗаказМатериаловВПроизводство);
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ЗаказМатериаловВПроизводство);
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ДвижениеПродукцииИМатериалов);
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПередачаПродукцииИзКладовой
			Или ХО = Перечисления.ХозяйственныеОперации.ВозвратМатериаловИзКладовой Тогда
			ДобавитьСостояние(Состояния, "СостоянияПриходныхОрдеров", Метаданные.Документы.ДвижениеПродукцииИМатериалов);
		//++ Устарело_Переработка24
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПередачаПереработчику Тогда
			
			ДобавитьСостояние(Состояния, "СостоянияОтгрузок",         Метаданные.Документы.ЗаказПереработчику);
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ЗаказПереработчику);
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ПередачаСырьяПереработчику);
			
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика Тогда
			
			ДобавитьСостояние(Состояния, "СостоянияПоступлений",      Метаданные.Документы.ЗаказПереработчику);
			ДобавитьСостояние(Состояния, "СостоянияПриходныхОрдеров", Метаданные.Документы.ЗаказПереработчику);
			ДобавитьСостояние(Состояния, "СостоянияПриходныхОрдеров", Метаданные.Документы.ПоступлениеОтПереработчика);
			
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ВозвратОтПереработчика Тогда
			
			ДобавитьСостояние(Состояния, "СостоянияПриходныхОрдеров", Метаданные.Документы.ВозвратСырьяОтПереработчика);
			
		//-- Устарело_Переработка24
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5 Тогда
			
			ДобавитьСостояние(Состояния, "СостоянияОтгрузок",         Метаданные.Документы.ЗаказПереработчику2_5);
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ЗаказПереработчику2_5);
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ПередачаТоваровХранителю);
			
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5 Тогда
			
			ДобавитьСостояние(Состояния, "СостоянияПоступленийОтПереработчика2_5", Метаданные.Документы.ЗаказПереработчику2_5);
			ДобавитьСостояние(Состояния, "СостоянияПриходныхОрдеров", Метаданные.Документы.ЗаказПереработчику2_5);
			ДобавитьСостояние(Состояния, "СостоянияПриходныхОрдеров", Метаданные.Документы.ПоступлениеТоваровОтХранителя);
			
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ВозвратОтПереработчика2_5 Тогда
			
			ДобавитьСостояние(Состояния, "СостоянияПриходныхОрдеров", Метаданные.Документы.ПоступлениеТоваровОтХранителя);
			
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчика2_5
			Или ХО = Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчикаВСтранахЕАЭС2_5 Тогда
			ДобавитьСостояние(Состояния, "СостоянияОтчетовПереработчика2_5", Метаданные.Документы.ЗаказПереработчику2_5);
		//++ НЕ УТКА
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПередачаМатериаловВПроизводство Тогда
			ДобавитьСостояние(Состояния, "СостоянияПередачВПроизводство", Метаданные.Документы.ЭтапПроизводства2_2);
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ЭтапПроизводства2_2);
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПередачаПродукцииИзПроизводства
			Или ХО = Перечисления.ХозяйственныеОперации.ВозвратМатериаловИзПроизводства Тогда
			ДобавитьСостояние(Состояния, "СостоянияПередачИзПроизводства", Метаданные.Документы.ЭтапПроизводства2_2);
			ДобавитьСостояние(Состояния, "СостоянияПриходныхОрдеров", Метаданные.Документы.ЭтапПроизводства2_2);
			ДобавитьСостояние(Состояния, "СостоянияПриходныхОрдеров", Метаданные.Документы.ДвижениеПродукцииИМатериалов);
		//++ Устарело_Переработка24
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца Тогда
			ДобавитьСостояние(Состояния, "СостоянияПоступлений", Метаданные.Документы.ЗаказДавальца);
			ДобавитьСостояние(Состояния, "СостоянияПриходныхОрдеров", Метаданные.Документы.ЗаказДавальца);
			ДобавитьСостояние(Состояния, "СостоянияПриходныхОрдеров", Метаданные.Документы.ПоступлениеСырьяОтДавальца);
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПередачаДавальцу Тогда
			ДобавитьСостояние(Состояния, "СостоянияОтгрузок", Метаданные.Документы.ЗаказДавальца);
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ЗаказДавальца);
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ВозвратДавальцу Тогда
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ВозвратСырьяДавальцу);
		//-- Устарело_Переработка24
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
			
			ДобавитьСостояние(Состояния, "СостоянияПоступлений",	  Метаданные.Документы.ЗаказДавальца2_5);
			ДобавитьСостояние(Состояния, "СостоянияПриходныхОрдеров", Метаданные.Документы.ЗаказДавальца2_5);
			ДобавитьСостояние(Состояния, "СостоянияПриходныхОрдеров", Метаданные.Документы.ПриемкаТоваровНаХранение);
			
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПередачаДавальцу2_5 Тогда
			
			ДобавитьСостояние(Состояния, "СостоянияПередач",		  Метаданные.Документы.ЗаказДавальца2_5);
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ЗаказДавальца2_5);
			
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ВозвратДавальцу2_5 Тогда
			
			ДобавитьСостояние(Состояния, "СостоянияРасходныхОрдеров", Метаданные.Документы.ОтгрузкаТоваровСХранения);
			
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5 Тогда
			
			ДобавитьСостояние(Состояния, "СостоянияОтчетовДавальцам2_5", Метаданные.Документы.ЗаказДавальца2_5);
...
Функция ИменаВременныхТаблицПоСостояниям() Экспорт
	...
	//++ Устарело_Переработка24
	ИменаТаблиц.Вставить("СостоянияОтчетовПереработчика",          "ВтСостоянияОтчетовПереработчика");
	//-- Устарело_Переработка24
	ИменаТаблиц.Вставить("СостоянияОтчетовПереработчика2_5",       "ВтСостоянияОтчетовПереработчика2_5");
	ИменаТаблиц.Вставить("СостоянияПоступленийОтПереработчика2_5", "ВтСостоянияПоступленийОтПереработчика2_5");
	//++ НЕ УТКА

	//++ Устарело_Переработка24
	ИменаТаблиц.Вставить("СостоянияОтчетовДавальцам",              "ВтСостоянияОтчетовДавальцам");
	//-- Устарело_Переработка24
	ИменаТаблиц.Вставить("СостоянияПередач",                       "ВтСостоянияПередач");
	ИменаТаблиц.Вставить("СостоянияОтчетовДавальцам2_5",           "ВтСостоянияОтчетовДавальцам2_5");
		...
		Если МассивДопустимыхСтатусов.Количество() = 1 Тогда
			Если Статус <> Неопределено Тогда
				ТекстОшибки = НСтр("ru = 'Документ находится в статусе ""%Статус%"". Ввод на основании разрешен только в статусе %СтрокаДопустимыхСтатусов%.';
									|en = 'Document is in the ""%Статус%"" status. Generation is only allowed from base documents in %СтрокаДопустимыхСтатусов% status.'");
			Иначе
				ТекстОшибки = НСтр("ru = 'Ввод на основании разрешен только в статусе %СтрокаДопустимыхСтатусов%.';
									|en = 'Generation is only allowed from base documents in %СтрокаДопустимыхСтатусов% status.'");
			КонецЕсли;
		Иначе
			Если Статус <> Неопределено Тогда
				ТекстОшибки = НСтр("ru = 'Документ находится в статусе ""%Статус%"". Ввод на основании разрешен только в статусах %СтрокаДопустимыхСтатусов%.';
									|en = 'Document is in the ""%Статус%"" status. Generation is only allowed from base documents in %СтрокаДопустимыхСтатусов% statuses.'");
			Иначе
				ТекстОшибки = НСтр("ru = 'Ввод на основании разрешен только в статусах %СтрокаДопустимыхСтатусов%.';
									|en = 'Generation is only allowed from base documents in %СтрокаДопустимыхСтатусов% statuses.'");
			КонецЕсли;
		КонецЕсли;
...
Функция ТекстЗапросаСостоянияРасходныхОрдеров(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению, ПараметрыЗамены = Неопределено)
	...
	//++ НЕ УТКА
	Если ПараметрыОтбора.ХозОперация Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РеестрДокументов.ХозяйственнаяОперация",
			"ВЫБОР
			//++ Устарело_Производство21 
...
Функция ТекстЗапросаСостоянияОтгрузок(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	|				РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|			ГДЕ
	|				РеестрДокументов.Ссылка = Остатки.ЗаказКлиента
	|				И (НЕ &ОтбиратьПоХозОперации ИЛИ РеестрДокументов.ХозяйственнаяОперация В (&ХозОперация))
	|				И (НЕ &ОтбиратьПоОрганизации ИЛИ РеестрДокументов.Организация В (&Организация))
	|				И (НЕ &ОтбиратьПоПодразделению ИЛИ РеестрДокументов.Подразделение В (&Подразделение))
	|				И (НЕ &ОтбиратьПоМенеджеру ИЛИ РеестрДокументов.Ответственный В (&Менеджер))
	...
	|			ИМЕЮЩИЕ
	|				СУММА(ЗаказыКлиентов.КОформлению) > 0)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ЗаказКлиента,
	|	Склад
...
Функция ТекстЗапросаСостоянияВнутреннихПотреблений(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	Если ПараметрыОтбора.ХозОперация Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РеестрДокументов.ХозяйственнаяОперация",
			"ВЫБОР КОГДА РеестрДокументов.Ссылка ССЫЛКА Документ.ЗаказНаРемонт ТОГДА
			|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию)
			|ИНАЧЕ
			|	РеестрДокументов.ХозяйственнаяОперация
			|КОНЕЦ");
...
Функция ТекстЗапросаСостоянияПередачВПроизводство(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению, ПараметрыЗамены = Неопределено)
	
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.ЗаказыМатериаловВПроизводство) Тогда
	...
	|		Остатки.Получатель 			  КАК Получатель
	|	ИЗ
	|		РегистрНакопления.ЗаказыМатериаловВПроизводство.Остатки(, &УсловиеОтбораОстатков) КАК Остатки
	|	ГДЕ
	|		Остатки.КОформлениюОстаток <> 0
	|		И ИСТИНА В(
	|			ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	&ВтЕстьНакладныеПоля КАК ВтЕстьНакладныеПоля,
	|	Распоряжения.Склад        КАК Склад,
	|	Распоряжения.Получатель   КАК Получатель
	|ПОМЕСТИТЬ ВтЕстьНакладные
	|ИЗ
	...
	|	&ВтСостоянияПередачВПроизводствоПоля КАК ВтСостоянияПередачВПроизводствоПоля,
	|	ВтЕстьРаспоряжения.Склад        КАК Склад,
	|	ВтЕстьРаспоряжения.Получатель   КАК Получатель,
	|	ВЫБОР
	...
	|			И &ВтСостоянияПередачВПроизводствоУсловие
	|			И ВтЕстьРаспоряжения.Склад = ВтЕстьНакладные.Склад
	...
	НастроитьПолеНоменклатуры(
		...
		"&ВтСостоянияПередачВПроизводствоПоля КАК ВтСостоянияПередачВПроизводствоПоля,",
		"ВтЕстьРаспоряжения",
		"&ВтСостоянияПередачВПроизводствоУсловие",
		"ВтЕстьНакладные");
	
	ДопУсловие = ?(ПараметрыОтбора.Подразделение, "Подразделение = &Подразделение", "");
	ДополнитьТекстОтборомПоОстаткам(ТекстЗапроса, "Распоряжение", ТипыРаспоряжений, ПараметрыОтбора.Склад, ДопУсловие);
	//++ НЕ УТКА
	Если ПараметрыОтбора.ХозОперация Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РеестрДокументов.ХозяйственнаяОперация",
			"ВЫБОР
	...
	КонецЕсли;
	//-- НЕ УТКА
	ПреобразоватьТекстЗапросаВНаличиеКОформлению(ТекстЗапроса, "ВтСостоянияПередачВПроизводство", ТолькоНаличиеКОформлению);
...
Функция ТекстЗапросаСостоянияОтчетовПереработчика(ПараметрыОтбора, ТолькоНаличиеКОформлению)
	
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.ТоварыПолученныеОтПереработчика)
		Или Не ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.ЗаказыПоставщикам) Тогда
		...
		Возврат
			...
			|	NULL КАК Партнер
			|ПОМЕСТИТЬ ВтСостоянияОтчетовПереработчика";
	...
	|		NULL КАК Склад
	|	ИЗ
	|		РегистрНакопления.ТоварыПолученныеОтПереработчика.Остатки(, Распоряжение <> НЕОПРЕДЕЛЕНО) КАК Остатки
	|	ГДЕ
	...
	|		Остатки.Склад
	|	ИЗ
	|		РегистрНакопления.ЗаказыПоставщикам.Остатки(, ЗаказПоставщику ССЫЛКА Документ.ЗаказПереработчику) КАК Остатки
	|	ГДЕ
	|		Остатки.КОформлениюОстаток <> 0
	|		И НЕ ВЫРАЗИТЬ(Остатки.ЗаказПоставщику КАК Документ.ЗаказПереработчику).Ссылка ЕСТЬ NULL
	|	) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РаспоряженияПоСкладам.Распоряжение КАК Распоряжение
	|ПОМЕСТИТЬ ВтЕстьОтчеты
	|ИЗ
	|	ВтЕстьРаспоряжения КАК РаспоряженияПоСкладам
	|ГДЕ
	|	ИСТИНА В
	|			(ВЫБРАТЬ
	|				ИСТИНА
	...
	|			ГДЕ
	|				ТоварыПолученныеОтПереработчика.Активность
	|				И ТоварыПолученныеОтПереработчика.Распоряжение = РаспоряженияПоСкладам.Распоряжение
	|				И ТоварыПолученныеОтПереработчика.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|			ИМЕЮЩИЕ
	|				СУММА(ТоварыПолученныеОтПереработчика.Количество) > 0)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Распоряжение
	|;
	...
	|	ВтЕстьРаспоряжения.Распоряжение КАК Распоряжение,
	|	ВЫБОР
	|		КОГДА ВтЕстьОтчеты.Распоряжение ЕСТЬ NULL
	|			ТОГДА 1
	|		ИНАЧЕ 2
	|	КОНЕЦ                           КАК Состояние,
	|	NULL                            КАК Организация,
	...
	|ИЗ
	|	ВтЕстьРаспоряжения КАК ВтЕстьРаспоряжения
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтЕстьОтчеты КАК ВтЕстьОтчеты
	|		ПО ВтЕстьРаспоряжения.Распоряжение = ВтЕстьОтчеты.Распоряжение
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		Остатки.КодСтроки КАК КодСтроки,
	|		Остатки.ТипСтоимости КАК ТипСтоимости,
	|		Остатки.АналитикаУчетаНоменклатуры.МестоХранения КАК Партнер
	|	ИЗ
	|		РегистрНакопления.ТоварыПолученныеОтПереработчика.Остатки(, &УсловиеОтбораОстатков) КАК Остатки
	|	ГДЕ
	|		Остатки.КоличествоОстаток > 0
	|	) КАК ВложенныйЗапрос
	|;
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВтЕстьОтчеты";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеОтбораОстатков",
		?(ПараметрыОтбора.Организация, "Распоряжение = НЕОПРЕДЕЛЕНО И Организация В (&Организация)", "Распоряжение = НЕОПРЕДЕЛЕНО"));
	ПреобразоватьТекстЗапросаВНаличиеКОформлению(ТекстЗапроса, "ВтСостоянияОтчетовПереработчика", ТолькоНаличиеКОформлению);
...
Функция ТекстЗапросаСостоянияОтчетовПереработчика2_5(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.ЗаказыПоставщикам)
		...
		Возврат
			...
			|	NULL КАК Распоряжение,
			|	NULL КАК Состояние
			|ПОМЕСТИТЬ ВтСостоянияОтчетовПереработчика2_5";
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		Остатки.ЗаказПоставщику    КАК Распоряжение,
	|		Остатки.Номенклатура       КАК Номенклатура,
	|		Остатки.Характеристика     КАК Характеристика,
	|		Остатки.Склад              КАК Склад,
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Остатки товаров к оформлению без заказа
	|	ВЫБРАТЬ
	|		Остатки.Распоряжение                     КАК Распоряжение,
	|		Остатки.Номенклатура                     КАК Номенклатура,
	|		Остатки.Характеристика                   КАК Характеристика,
	|		ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка) КАК Склад,
	|		СУММА(Остатки.КОформлению)               КАК КОформлению
	|	ИЗ
	|		(ВЫБРАТЬ
	|			Остатки.АналитикаУчетаНоменклатуры.Договор        КАК Распоряжение,
	...
	|			-Остатки.КоличествоОстаток                        КАК КОформлению
	|		ИЗ
	|			РегистрНакопления.ТоварыОрганизаций.Остатки(,
	|				НЕ &ОтбиратьПоОрганизации ИЛИ Организация В (&Организация)) КАК Остатки
	|		ГДЕ
	|			Остатки.КоличествоОстаток < 0
	...
	|			И (ЕСТЬNULL(Остатки.АналитикаУчетаНоменклатуры.Договор.ТипДоговора, ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.ПустаяСсылка)) =
	|				ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СПереработчиком2_5)
	|				ИЛИ ЕСТЬNULL(Остатки.АналитикаУчетаНоменклатуры.Договор.ТипДоговора, ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.ПустаяСсылка)) =
	|				ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СПереработчиком2_5_ЕАЭС))
	|			И (НЕ &ОтбиратьПоМенеджеру
	|				ИЛИ Остатки.АналитикаУчетаНоменклатуры.Договор.Менеджер В (&Менеджер))
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	|			РеестрДокументов.Договор         КАК Распоряжение,
	|			ТоварыПоступления.Номенклатура   КАК Номенклатура,
	|			ТоварыПоступления.Характеристика КАК Характеристика,
	|			-ТоварыПоступления.Количество    КАК КОформлению
	|		ИЗ
	|			РегистрНакопления.ЗаказыПоставщикам.Обороты(,,
	|				Авто,
	|				ТИПЗНАЧЕНИЯ(ЗаказПоставщику) = ТИП(Документ.ЗаказПереработчику2_5)) КАК ОборотыПоЗаказам
	|			
	...
	|			
	|		ГДЕ
	|			(НЕ &ОтбиратьПоОрганизации ИЛИ РеестрДокументов.Организация В (&Организация))
	|			И (НЕ &ОтбиратьПоМенеджеру
	|				ИЛИ РеестрДокументов.Договор.Менеджер В (&Менеджер))
	|	) КАК Остатки
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|		ГДЕ
	|			ЗаказыПоставщикам.Активность
	|			И ЗаказыПоставщикам.ЗаказПоставщику = Распоряжения.Распоряжение
	...
	ДополнитьТекстОтборомПоОстаткам(ТекстЗапроса, "ЗаказПоставщику", ТипыРаспоряжений);
	
	ПреобразоватьТекстЗапросаВНаличиеКОформлению(
		ТекстЗапроса,
		"ВтСостоянияОтчетовПереработчика2_5",
		ТолькоНаличиеКОформлению);
	
	Возврат ТекстЗапроса;
	
КонецФункции

Функция ТекстЗапросаСостоянияПоступленийОтПереработчика2_5(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.ЗаказыПоставщикам) Тогда
		...
		Возврат
			...
			|	NULL КАК Склад,
			|	NULL КАК Состояние
			|ПОМЕСТИТЬ ВтСостоянияПоступленийОтПереработчика2_5";
	...
	|				И ЗаказыПоставщикам.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|			ИМЕЮЩИЕ
	|				СУММА(ЗаказыПоставщикам.КПоступлению) > 0)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Распоряжение,
	|	Склад
	...
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтЕстьНакладные КАК ВтЕстьНакладные
	|	ПО ВтЕстьНакладные.Распоряжение = ВтЕстьРаспоряжения.Распоряжение
	|	И ВтЕстьНакладные.Склад = ВтЕстьРаспоряжения.Склад
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВтЕстьРаспоряжения
	...
	ДополнитьТекстОтборомПоОстаткам(ТекстЗапроса, "ЗаказПоставщику", ТипыРаспоряжений, ПараметрыОтбора.Склад);
	ПреобразоватьТекстЗапросаВНаличиеКОформлению(ТекстЗапроса, "ВтСостоянияПоступленийОтПереработчика2_5", ТолькоНаличиеКОформлению);
...
Функция ТекстЗапросаСостоянияПередачИзПроизводства(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению, ПараметрыЗамены = Неопределено)
	
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.РаспоряженияНаПередачуИзПроизводства) Тогда
		...
		|	NULL КАК ХозОперация,
		|	NULL КАК Состояние
		|ПОМЕСТИТЬ ВтСостоянияПередачИзПроизводства";
	...
	|		Остатки.КодСтроки             КАК КодСтроки
	|	ИЗ
	|		РегистрНакопления.РаспоряженияНаПередачуИзПроизводства.Остатки(, &УсловиеОтбораОстатков) КАК Остатки
	|	ГДЕ
	|		Остатки.КоличествоОстаток <> 0
	|		И ИСТИНА В(
	|			ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	Распоряжения.Получатель   КАК Получатель,
	|	Распоряжения.Отправитель  КАК Отправитель,
	|	Распоряжения.Операция     КАК Операция
	|ПОМЕСТИТЬ ВтЕстьНакладные
	|ИЗ
	|	ВтЕстьРаспоряжения КАК Распоряжения
	|ГДЕ
	...
	|				ИСТИНА
	|			ИЗ
	|				РегистрНакопления.РаспоряженияНаПередачуИзПроизводства КАК РаспоряженияНаПередачуИзПроизводства
	|			ГДЕ
	|				РаспоряженияНаПередачуИзПроизводства.Активность
	|				И РаспоряженияНаПередачуИзПроизводства.Распоряжение = Распоряжения.Распоряжение
	|				И &ВтЕстьНакладныеУсловие
	|				И РаспоряженияНаПередачуИзПроизводства.Получатель = Распоряжения.Получатель
	|				И РаспоряженияНаПередачуИзПроизводства.Отправитель = Распоряжения.Отправитель
	|				И РаспоряженияНаПередачуИзПроизводства.Операция = Распоряжения.Операция
	|				И РаспоряженияНаПередачуИзПроизводства.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|			ИМЕЮЩИЕ
	|				СУММА(РаспоряженияНаПередачуИзПроизводства.Количество) > 0)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Распоряжение,
	|	ВтЕстьНакладныеПоля,
	...
	|ВЫБРАТЬ
	|	ВтЕстьРаспоряжения.Распоряжение КАК Распоряжение,
	|	&ВтСостоянияПередачИзПроизводстваПоля КАК ВтСостоянияПередачИзПроизводстваПоля,
	|	ВтЕстьРаспоряжения.Получатель   КАК Склад,
	|	ВтЕстьРаспоряжения.Отправитель  КАК Отправитель,
	|	ВтЕстьРаспоряжения.Операция     КАК ХозОперация,
	|	ВЫБОР
	|		КОГДА ВтЕстьНакладные.Распоряжение ЕСТЬ NULL
	|			ТОГДА 1
	|		ИНАЧЕ 2
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтЕстьНакладные КАК ВтЕстьНакладные
	|		ПО ВтЕстьРаспоряжения.Распоряжение = ВтЕстьНакладные.Распоряжение
	|			И &ВтСостоянияПередачИзПроизводстваУсловие
	|			И ВтЕстьРаспоряжения.Получатель = ВтЕстьНакладные.Получатель
	|			И ВтЕстьРаспоряжения.Отправитель = ВтЕстьНакладные.Отправитель
	|			И ВтЕстьРаспоряжения.Операция = ВтЕстьНакладные.Операция
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВтЕстьРаспоряжения
	...
	НастроитьПолеНоменклатуры(
		...
		"&ВтСостоянияПередачИзПроизводстваПоля КАК ВтСостоянияПередачИзПроизводстваПоля,",
		"ВтЕстьРаспоряжения",
		"&ВтСостоянияПередачИзПроизводстваУсловие",
		"ВтЕстьНакладные");
	
	ДопУсловие = "ТИПЗНАЧЕНИЯ(Получатель) = ТИП(Справочник.Склады)";
	Если ПараметрыОтбора.ХозОперация Тогда
		ДопУсловие = ДопУсловие + " И Операция В (&ХозОперация)";
	КонецЕсли;
	Если ПараметрыОтбора.Склад Тогда
		ДопУсловие = ДопУсловие + " И Получатель В (&Склад)";
	КонецЕсли;
	ДополнитьТекстОтборомПоОстаткам(ТекстЗапроса, "Распоряжение", ТипыРаспоряжений, Ложь, ДопУсловие);
	ПреобразоватьТекстЗапросаВНаличиеКОформлению(ТекстЗапроса, "ВтСостоянияПередачИзПроизводства", ТолькоНаличиеКОформлению);
...
//++ Устарело_Переработка24
Функция ТекстЗапросаСостоянияОтчетовДавальцам(ПараметрыОтбора, ТолькоНаличиеКОформлению)
	
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.УслугиДавальцуКОформлению) Тогда
		...
		Возврат
			...
			|	NULL КАК Распоряжение,
			|	NULL КАК Состояние
			|ПОМЕСТИТЬ ВтСостоянияОтчетовДавальцам";
	...
	|	ВложенныйЗапрос.ЗаказДавальца КАК ЗаказДавальца
	|ПОМЕСТИТЬ ВтЕстьРаспоряжения
	|ИЗ
	|	(ВЫБРАТЬ
	|		Остатки.Договор КАК Договор,
	|		Остатки.ЗаказДавальца КАК ЗаказДавальца,
	|		Остатки.Номенклатура КАК Номенклатура,
	|		Остатки.Характеристика КАК Характеристика
	|	ИЗ
	|		РегистрНакопления.УслугиДавальцуКОформлению.Остатки КАК Остатки
	|	ГДЕ
	|		&ОтборПоРеестру ИЛИ &ОтборПоДоговору
	|	) КАК ВложенныйЗапрос
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РаспоряженияПоСкладам.Договор КАК Договор,
	|	РаспоряженияПоСкладам.ЗаказДавальца КАК ЗаказДавальца
	|ПОМЕСТИТЬ ВтЕстьОтчеты
	|ИЗ
	|	ВтЕстьРаспоряжения КАК РаспоряженияПоСкладам
	|ГДЕ
	...
	|			ГДЕ
	|				УслугиДавальцуКОформлению.Активность
	|				И УслугиДавальцуКОформлению.Договор = РаспоряженияПоСкладам.Договор
	|				И УслугиДавальцуКОформлению.ЗаказДавальца = РаспоряженияПоСкладам.ЗаказДавальца
	|				И УслугиДавальцуКОформлению.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|			ИМЕЮЩИЕ
	|				СУММА(УслугиДавальцуКОформлению.КОформлению) > 0)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Договор,
	|	ЗаказДавальца
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВтЕстьРаспоряжения.Договор КАК Договор,
	|	ВтЕстьРаспоряжения.ЗаказДавальца КАК Распоряжение,
	|	ВЫБОР
	|		КОГДА ВтЕстьОтчеты.ЗаказДавальца ЕСТЬ NULL
	|			ТОГДА 1
	|		ИНАЧЕ 2
	|	КОНЕЦ                            КАК Состояние
	|ПОМЕСТИТЬ ВтСостоянияОтчетовДавальцам
	...
	|	ВтЕстьРаспоряжения КАК ВтЕстьРаспоряжения
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтЕстьОтчеты КАК ВтЕстьОтчеты
	|		ПО ВтЕстьРаспоряжения.Договор = ВтЕстьОтчеты.Договор
	|		 И ВтЕстьРаспоряжения.ЗаказДавальца = ВтЕстьОтчеты.ЗаказДавальца
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВтЕстьРаспоряжения
	...
	ДополнитьТекстОтбором(
		ТекстЗапроса,
		"&ОтборПоРеестру",
		"РегистрСведений.РеестрДокументов",
		"ЗаказДавальца",
		ПоляОтбораРеестрДокументов(),
		ПараметрыОтбора,
		"НЕ Таблица.ДополнительнаяЗапись");
	...
	ДополнитьТекстОтбором(
		...
		"Договор",
		ПоляОтбораДоговорыКонтрагентов(),
		ПараметрыОтбора,
		"Остатки.ЗаказДавальца = ЗНАЧЕНИЕ(Документ.ЗаказДавальца.ПустаяСсылка)");
			
	ПреобразоватьТекстЗапросаВНаличиеКОформлению(ТекстЗапроса, "ВтСостоянияОтчетовДавальцам", ТолькоНаличиеКОформлению);
...
Функция ТекстЗапросаСостоянияПередач(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	|				И (НЕ &ОтбиратьПоОрганизации   ИЛИ РеестрДокументов.Организация В (&Организация))
	|				И (НЕ &ОтбиратьПоПодразделению ИЛИ РеестрДокументов.Подразделение В (&Подразделение))
	|				И (НЕ &ОтбиратьПоМенеджеру     ИЛИ РеестрДокументов.Ответственный В (&Менеджер)))) КАК Заказы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			И ЗаказыКлиентов.Склад        = Распоряжения.Склад
	|		ИМЕЮЩИЕ
	|			СУММА(ЗаказыКлиентов.КПередаче) > 0)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ЗаказКлиента,
	|	Склад
	...
	ДополнитьТекстОтборомПоОстаткам(ТекстЗапроса, "ЗаказКлиента", ТипыРаспоряжений, ПараметрыОтбора.Склад);
	ПреобразоватьТекстЗапросаВНаличиеКОформлению(ТекстЗапроса, "ВтСостоянияПередач", ТолькоНаличиеКОформлению);
	
	Возврат ТекстЗапроса;
	
КонецФункции

Функция ТекстЗапросаСостоянияОтчетовДавальцам2_5(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.ЗаказыКлиентов)
	 Или Не ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.ТоварыОрганизаций) Тогда
		...
		Возврат
			...
			|	NULL КАК Склад,
			|	NULL КАК Состояние
			|ПОМЕСТИТЬ ВтСостоянияОтчетовДавальцам2_5";
	...
	|	(ВЫБРАТЬ
	|		Остатки.ЗаказКлиента                                                 КАК ЗаказКлиента,
	|		ВЫРАЗИТЬ(Остатки.ЗаказКлиента КАК Документ.ЗаказДавальца2_5).Договор КАК Договор,
	|		Остатки.Номенклатура                                                 КАК Номенклатура,
	|		Остатки.Характеристика                                               КАК Характеристика,
	|		Остатки.КодСтроки                                                    КАК КодСтроки,
	|		Остатки.Склад                                                        КАК Склад,
	...
	|		Остатки.КОформлениюОстаток                                           КАК КОформлению
	|	ИЗ
	|		РегистрНакопления.ЗаказыКлиентов.Остатки(, &УсловиеОтбораОстатков И КодСтроки <> 0) КАК Остатки
	|	ГДЕ
	|		Остатки.КОформлениюОстаток > 0
	|		И НЕ Остатки.ЗаказКлиента.Ссылка ЕСТЬ NULL // для отработки RLS
	|		И НЕ Остатки.Склад.Ссылка ЕСТЬ NULL
	...
	|	
	|	ВЫБРАТЬ
	|		ЗНАЧЕНИЕ(Документ.ЗаказДавальца2_5.ПустаяСсылка) КАК ЗаказКлиента,
	|		ОстаткиБезЗаказа.Договор                         КАК Договор,
	...
	|ВЫБРАТЬ
	|	Распоряжения.ЗаказКлиента КАК ЗаказКлиента,
	|	Распоряжения.Договор      КАК Договор,
	|	Распоряжения.Склад        КАК Склад
	|ПОМЕСТИТЬ ВтЕстьНакладные
	|ИЗ
	|	ВтЕстьРаспоряжения КАК Распоряжения
	...
	|	ВтЕстьРаспоряжения КАК Распоряжения
	|ГДЕ
	|	Распоряжения.ЗаказКлиента = ЗНАЧЕНИЕ(Документ.ЗаказДавальца2_5.ПустаяСсылка)
	|	И ИСТИНА В
	|			(ВЫБРАТЬ
	|				ИСТИНА
	|			ИЗ
	...
	|				И ТоварыОрганизаций.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца)
	|				И ВЫРАЗИТЬ(ТоварыОрганизаций.ВидЗапасов.Договор КАК Справочник.ДоговорыКонтрагентов).ТипДоговора =
	|											ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СДавальцем2_5)
	|				И ТоварыОрганизаций.АналитикаУчетаНоменклатуры.ТипМестаХранения =
	|													ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.Склад)
	|				И ТоварыОрганизаций.ВидЗапасов.Договор                       = Распоряжения.Договор
	|				И ТоварыОрганизаций.АналитикаУчетаНоменклатуры.МестоХранения = Распоряжения.Склад
	|			ИМЕЮЩИЕ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтЕстьНакладные КАК ВтЕстьНакладные
	|		ПО ВтЕстьРаспоряжения.ЗаказКлиента = ВтЕстьНакладные.ЗаказКлиента
	|		 И ВтЕстьРаспоряжения.Договор      = ВтЕстьНакладные.Договор
	|		 И ВтЕстьРаспоряжения.Склад        = ВтЕстьНакладные.Склад
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	ТекстЗапросаОстаткиБезЗаказа = СтрЗаменить(ТекстЗапросаСостоянияОтчетовДавальцам2_5ОстаткиБезЗаказа(),
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОстаткиБезЗаказа", СтрШаблон("(%1)", ТекстЗапросаОстаткиБезЗаказа));
	
	ДополнитьТекстОтборомПоОстаткам(ТекстЗапроса, "ЗаказКлиента", ТипыРаспоряжений, ПараметрыОтбора.Склад);
	
	ПреобразоватьТекстЗапросаВНаличиеКОформлению(
		ТекстЗапроса, "ВтСостоянияОтчетовДавальцам2_5", ТолькоНаличиеКОформлению);
	
	Возврат ТекстЗапроса;
	
КонецФункции

// Возвращает текст запроса остатков без заказа для оформления отчетов давальцев
// 
// Возвращаемое значение:
//  Строка - Текст запроса
//
Функция ТекстЗапросаСостоянияОтчетовДавальцам2_5ОстаткиБезЗаказа() Экспорт
	...
	|	ОстаткиБезЗаказа.Склад              КАК Склад,
	|	ОстаткиБезЗаказа.Серия              КАК Серия,
	|	СУММА(ОстаткиБезЗаказа.КОформлению) КАК КОформлению
	|ИЗ
	|	(ВЫБРАТЬ
	|		ТоварыОрганизаций.ВидЗапасов.Договор                        КАК Договор,
	...
	|		-ТоварыОрганизаций.КоличествоОстаток                        КАК КОформлению
	|	ИЗ
	|		РегистрНакопления.ТоварыОрганизаций.Остатки(,
	...
	|				И (НЕ &ОтбиратьПоОрганизации ИЛИ Организация В (&Организация))
	|				И (НЕ &ОтбиратьПоСкладу ИЛИ АналитикаУчетаНоменклатуры.МестоХранения В (&Склад))
	|				И &ДополнительныеУсловияТоварыОрганизаций) КАК ТоварыОрганизаций
	|	ГДЕ
	...
	|
	|	ВЫБРАТЬ
	|		ВЫРАЗИТЬ(Обороты.ЗаказКлиента КАК Документ.ЗаказДавальца2_5).Договор КАК Договор,
	...
	|	ИЗ
	|		РегистрНакопления.ЗаказыКлиентов.Обороты(,,, &УсловиеОтбораОстатков) КАК Обороты
	|
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтгрузкаТоваровСХранения.Товары КАК Товары
	|			ПО Обороты.ЗаказКлиента = Товары.ЗаказКлиента
	|			 И Обороты.КодСтроки    = Товары.КодСтроки
	...
	|				И (НЕ &ОтбиратьПоМенеджеру     ИЛИ РеестрДокументов.Ответственный В (&Менеджер))
	|				И НЕ РеестрДокументов.ДополнительнаяЗапись)
	|		И &ДополнительныеУсловияЗаказыКлиентов) КАК ОстаткиБезЗаказа
	|
	|СГРУППИРОВАТЬ ПО
	|	ОстаткиБезЗаказа.Договор,
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(ОстаткиБезЗаказа.КОформлению) > 0
	|";
...
Процедура ДополнитьТекстОтборомПоОстаткам(ТекстЗапроса, ПолеСвязи, ТипыРаспоряжений, Склад = Ложь, ДопУсловие = "", ВключаяОтправитель = Ложь, ВключаяПолучатель = Ложь)
	...
	Если ЗначениеЗаполнено(ТипыРаспоряжений) Тогда
		...
		Если ТипыРаспоряжений.Количество() = 1 Тогда
			...
			Подстановка = "ВЫРАЗИТЬ(Остатки.ЗаказКлиента КАК Документ.ЗаказНаВнутреннееПотребление).Ссылка";
			Подстановка = СтрЗаменить(Подстановка, "Документ.ЗаказНаВнутреннееПотребление", ТипыРаспоряжений[0]);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Остатки.ЗаказКлиента.Ссылка", Подстановка);
		КонецЕсли;
...
Процедура ДополнитьТекстОтбором(
	ТекстЗапроса, ПараметрЗамены, Источник, ПолеСвязи, ПоляОтбора, ПараметрыОтбора, ПроизвольныйОтбор = "")
	...
	Если ЗначениеЗаполнено(ПараметрыОтбора) Тогда
		...
		Для каждого ПараметрОтбора Из ПараметрыОтбора Цикл
			...
			Если ПараметрОтбора.Значение И ПоляОтбора.Свойство(ПараметрОтбора.Ключ, ПолеОтбора) Тогда
				ТекстыУсловий.Добавить(СтрШаблон(ШаблонУсловий, ПолеОтбора, ПараметрОтбора.Ключ));
			КонецЕсли;
	...
	Если ТекстыУсловий.Количество() > 0 Тогда
		...
		ТекстОтбора = СтрШаблон(
						...
						Источник,
						ПолеСвязи,
						СтрСоединить(ТекстыУсловий, "И "),
						?(ЗначениеЗаполнено(ПроизвольныйОтбор), ПроизвольныйОтбор, "ИСТИНА"));
		
	Иначе
		ТекстОтбора = "ИСТИНА";
	КонецЕсли;
		
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ПараметрЗамены, ТекстОтбора);	
...
//++ Устарело_Переработка24
#Область СоответствиеИменПолейОтбора

Функция ПоляОтбораРеестрДокументов()
	
	ПоляОтбора = Новый Структура;
	ПоляОтбора.Вставить("ХозОперация",	 "Таблица.ХозяйственнаяОперация");
	ПоляОтбора.Вставить("Организация",	 "Таблица.Организация");
	ПоляОтбора.Вставить("Подразделение", "Таблица.Подразделение");
	ПоляОтбора.Вставить("Склад",		 "Таблица.МестоХранения.Ключ");
	ПоляОтбора.Вставить("Менеджер",		 "Таблица.Ответственный");
	
	Возврат ПоляОтбора;
	
КонецФункции

Функция ПоляОтбораДоговорыКонтрагентов()
	
	ПоляОтбора = Новый Структура;
	ПоляОтбора.Вставить("Организация",	 "Таблица.Организация");
	ПоляОтбора.Вставить("Подразделение", "Таблица.Подразделение");
	ПоляОтбора.Вставить("Менеджер",		 "Таблица.Менеджер");
