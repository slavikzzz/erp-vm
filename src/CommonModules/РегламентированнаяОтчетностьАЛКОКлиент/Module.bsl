...
Процедура ВыполнитьКомандуФормыПоИмениАЛКО(Форма, ИмяКоманды) Экспорт
	...
	ИначеЕсли ИмяКоманды = "ДобавитьСтраницу" Тогда
		...
		Если (
				(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '20131101')
				И ЭтоРазделЛицензии
				И (Не Форма.ЭтоЛицензируемаяДеятельность)
			)
			ИЛИ
			ЭтоТитульный
			Тогда
			...
			ТекстПредупреждения = НСтр("ru = 'Добавление страницы запрещено!';
										|en = 'Добавление страницы запрещено!'");
	...
	ИначеЕсли ИмяКоманды = "УдалитьСтраницу" Тогда
		...
		Если Форма.ОднаСтраницаВРазделе Тогда
			
			ТекстПредупреждения = НСтр("ru = 'Нельзя удалить единственную страницу!';
										|en = 'Нельзя удалить единственную страницу!'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
	...
	ИначеЕсли ИмяКоманды = "ЭкспортXLS" Тогда
		
		// При экспорте используется печать, поэтому проверяем именно возможность печати.
		ОписаниеОтчета = РегламентированнаяОтчетностьАЛКОВызовСервера.ОписаниеРегОтчета(Форма.СтруктураРеквизитовФормы.мСохраненныйДок);
		НужнаДальнейшаяОбработка = ОбработкаДействияСАлкоОтчетом(ОписаниеОтчета, "Печать", Ложь);
	...
	ИначеЕсли ИмяКоманды = "ЭкспортXLSX" Тогда
	...
	Если ЗадаватьВопросПередОчисткой Тогда
		...
		Если ПустаяСтрока(ТекстВопроса) Тогда
			ТекстВопроса = НСтр("ru = 'Внимание! Будут очищены все разделы отчета.
				|Продолжить?';
				|en = 'Внимание! Будут очищены все разделы отчета.
				|Продолжить?'");
		КонецЕсли;
...
Процедура ОбособленноеПодразделениеНажатиеАЛКО(Форма, Элемент = Неопределено,
		СтандартнаяОбработка = Ложь, ПараметрыИзФормы = Неопределено) Экспорт

	СтандартнаяОбработка = Ложь;
	ВводРеквизитовОП_АЛКО(Форма, ПараметрыИзФормы);

КонецПроцедуры

Процедура ФлагГоловногоОППриИзмененииАЛКО(Форма, Элемент = Неопределено) Экспорт
		...
		// Нужно убедиться, что отчет не слишком большой.
			...
			Если КолСтрокВОтчете > 20000 Тогда
				...
				ТекстВопроса = НСтр("ru = 'Слишком большой отчет для печати с показом бланка.
					...
					|en = 'Слишком большой отчет для печати с показом бланка.
					|Продолжить в режиме печати в выбранном формате в ZIP архив?'");
		...
		Иначе
			...
			Если НужноВыбратьФормат Тогда
				...
				Если ВидПечати = "ПечатьФайлZIP" Тогда
					ПечатьФайлZIPВыборФорматаАЛКО(Форма);
				ИначеЕсли ВидПечати = "Экспорт" Тогда
					ЭкспортВыборФорматаАЛКО(Форма);
				КонецЕсли;
...
Функция ОбработкаДействияСАлкоОтчетом(ОписаниеОтчета, Действие,
	...
	Если НЕ ЗначениеЗаполнено(Действие) Тогда
		ВызватьИсключение НСтр("ru = 'В функцию ОбработаноДействиеДляАлкоОтчета() НЕ передано Действие!';
								|en = 'В функцию ОбработаноДействиеДляАлкоОтчета() НЕ передано Действие!'");
	Иначе
		...
		Если НайденноеДействие = Неопределено Тогда
			ВызватьИсключение НСтр("ru = 'В функцию ОбработаноДействиеДляАлкоОтчета() передано неверное Действие!';
									|en = 'В функцию ОбработаноДействиеДляАлкоОтчета() передано неверное Действие!'");
		КонецЕсли;
	...
	// Урожайная 2022.
	СтруктураАлкоОтчетов.Вставить("РегламентированныйОтчетАлкоПриложение32", Новый Структура);
	
	// Виноградные 2022.
	СтруктураАлкоОтчетов.Вставить("РегламентированныйОтчетАлкоПриложение33", Новый Структура);
	СтруктураАлкоОтчетов.Вставить("РегламентированныйОтчетАлкоПриложение34", Новый Структура);
	
	// Алко новые субъекты РФ 2023.
	СтруктураАлкоОтчетов.Вставить("РегламентированныйОтчетАлкоПриложение35", Новый Структура);
	СтруктураАлкоОтчетов.Вставить("РегламентированныйОтчетАлкоПриложение36", Новый Структура);
	СтруктураАлкоОтчетов.Вставить("РегламентированныйОтчетАлкоПриложение37", Новый Структура);
	СтруктураАлкоОтчетов.Вставить("РегламентированныйОтчетАлкоПриложение38", Новый Структура);
	СтруктураАлкоОтчетов.Вставить("РегламентированныйОтчетАлкоПриложение39", Новый Структура);
	
	// Алко 2025.
	СтруктураАлкоОтчетов.Вставить("РегламентированныйОтчетАлкоПриложение40", Новый Структура);
	...
	Алко25.Вставить("ФормаОтчета2025Кв1", Новый Структура);
	Алко25.ФормаОтчета2025Кв1.Вставить("ТабличныйОтчет", Ложь);
	Алко25.ФормаОтчета2025Кв1.Вставить("Утверждена", "Приказом Росалкогольтабакконтроля от 26.11.2024 г. № 461");
	Алко25.ФормаОтчета2025Кв1.Вставить("НачалоДействия", "2 квартал 2025 года");
	УстановитьДляФормыДействие(СписокДействий, Алко25.ФормаОтчета2025Кв1, Неопределено, Истина);
	...
	// Формы Алко32. Урожайная.
	Алко32 = СтруктураАлкоОтчетов.РегламентированныйОтчетАлкоПриложение32;

	Алко32.Вставить("ФормаОтчета2022Кв3", Новый Структура);
	Алко32.ФормаОтчета2022Кв3.Вставить("ТабличныйОтчет", Ложь);
	Алко32.ФормаОтчета2022Кв3.Вставить("Утверждена", "Постановлением Правительства РФ от 21.12.2020 г. № 2195");
	Алко32.ФормаОтчета2022Кв3.Вставить("НачалоДействия", "3 квартал 2022 года");
	// Запрещаем все кроме печати.
	УстановитьДляФормыДействие(СписокДействий, Алко32.ФормаОтчета2022Кв3, Неопределено, Ложь);
	УстановитьДляФормыДействие(СписокДействий, Алко32.ФормаОтчета2022Кв3, "Печать", Истина);
	
	// Виноградные 2022.
	// Формы Алко33.
	Алко33 = СтруктураАлкоОтчетов.РегламентированныйОтчетАлкоПриложение33;

	Алко33.Вставить("ФормаОтчета2022Кв4", Новый Структура);
	Алко33.ФормаОтчета2022Кв4.Вставить("ТабличныйОтчет", Ложь);
	Алко33.ФормаОтчета2022Кв4.Вставить("Утверждена", "Приказом Минсельхоза России от 01.10.2021 № 687");
	Алко33.ФормаОтчета2022Кв4.Вставить("НачалоДействия", "2022 год");
	УстановитьДляФормыДействие(СписокДействий, Алко33.ФормаОтчета2022Кв4, Неопределено, Истина);
	
	// Формы Алко34.
	Алко34 = СтруктураАлкоОтчетов.РегламентированныйОтчетАлкоПриложение34;

	Алко34.Вставить("ФормаОтчета2022Кв4", Новый Структура);
	Алко34.ФормаОтчета2022Кв4.Вставить("ТабличныйОтчет", Ложь);
	Алко34.ФормаОтчета2022Кв4.Вставить("Утверждена", "Приказом Минсельхоза России от 01.10.2021 № 687");
	Алко34.ФормаОтчета2022Кв4.Вставить("НачалоДействия", "2022 год");
	УстановитьДляФормыДействие(СписокДействий, Алко34.ФормаОтчета2022Кв4, Неопределено, Истина);
	УстановитьДляФормыДействие(СписокДействий, Алко34.ФормаОтчета2022Кв4, "ЗагрузкаИзФайла", Ложь);
	
	// Алко новые субъекты РФ 2023.
	// Формы Алко35.
	Алко35 = СтруктураАлкоОтчетов.РегламентированныйОтчетАлкоПриложение35;

	Алко35.Вставить("ФормаОтчета2023Кв3", Новый Структура);
	Алко35.ФормаОтчета2023Кв3.Вставить("ТабличныйОтчет", Ложь);
	Алко35.ФормаОтчета2023Кв3.Вставить("Утверждена", "Образец формы опубликован на сайте ФСРАР");
	Алко35.ФормаОтчета2023Кв3.Вставить("НачалоДействия", "июль 2023 года");
	УстановитьДляФормыДействие(СписокДействий, Алко35.ФормаОтчета2023Кв3, Неопределено, Ложь);
	УстановитьДляФормыДействие(СписокДействий, Алко35.ФормаОтчета2023Кв3, "Печать", Истина);
	
	// Формы Алко36.
	Алко36 = СтруктураАлкоОтчетов.РегламентированныйОтчетАлкоПриложение36;

	Алко36.Вставить("ФормаОтчета2023Кв3", Новый Структура);
	Алко36.ФормаОтчета2023Кв3.Вставить("ТабличныйОтчет", Ложь);
	Алко36.ФормаОтчета2023Кв3.Вставить("Утверждена", "Образец формы опубликован на сайте ФСРАР");
	Алко36.ФормаОтчета2023Кв3.Вставить("НачалоДействия", "июль 2023 года");
	УстановитьДляФормыДействие(СписокДействий, Алко36.ФормаОтчета2023Кв3, Неопределено, Ложь);
	УстановитьДляФормыДействие(СписокДействий, Алко36.ФормаОтчета2023Кв3, "Печать", Истина);
	
	// Формы Алко37.
	Алко37 = СтруктураАлкоОтчетов.РегламентированныйОтчетАлкоПриложение37;

	Алко37.Вставить("ФормаОтчета2023Кв3", Новый Структура);
	Алко37.ФормаОтчета2023Кв3.Вставить("ТабличныйОтчет", Ложь);
	Алко37.ФормаОтчета2023Кв3.Вставить("Утверждена", "Образец формы опубликован на сайте ФСРАР");
	Алко37.ФормаОтчета2023Кв3.Вставить("НачалоДействия", "июль 2023 года");
	УстановитьДляФормыДействие(СписокДействий, Алко37.ФормаОтчета2023Кв3, Неопределено, Ложь);
	УстановитьДляФормыДействие(СписокДействий, Алко37.ФормаОтчета2023Кв3, "Печать", Истина);
	
	// Формы Алко38.
	Алко38 = СтруктураАлкоОтчетов.РегламентированныйОтчетАлкоПриложение38;

	Алко38.Вставить("ФормаОтчета2023Кв3", Новый Структура);
	Алко38.ФормаОтчета2023Кв3.Вставить("ТабличныйОтчет", Ложь);
	Алко38.ФормаОтчета2023Кв3.Вставить("Утверждена", "Образец формы опубликован на сайте ФСРАР");
	Алко38.ФормаОтчета2023Кв3.Вставить("НачалоДействия", "июль 2023 года");
	УстановитьДляФормыДействие(СписокДействий, Алко38.ФормаОтчета2023Кв3, Неопределено, Ложь);
	УстановитьДляФормыДействие(СписокДействий, Алко38.ФормаОтчета2023Кв3, "Печать", Истина);
	
	// Формы Алко39.
	Алко39 = СтруктураАлкоОтчетов.РегламентированныйОтчетАлкоПриложение39;

	Алко39.Вставить("ФормаОтчета2023Кв3", Новый Структура);
	Алко39.ФормаОтчета2023Кв3.Вставить("ТабличныйОтчет", Ложь);
	Алко39.ФормаОтчета2023Кв3.Вставить("Утверждена", "Образец формы опубликован на сайте ФСРАР");
	Алко39.ФормаОтчета2023Кв3.Вставить("НачалоДействия", "июль 2023 года");
	УстановитьДляФормыДействие(СписокДействий, Алко39.ФормаОтчета2023Кв3, Неопределено, Ложь);
	УстановитьДляФормыДействие(СписокДействий, Алко39.ФормаОтчета2023Кв3, "Печать", Истина);
	
	// АЛКО 2025
	// Формы Алко26.
	Алко40 = СтруктураАлкоОтчетов.РегламентированныйОтчетАлкоПриложение40;

	Алко40.Вставить("ФормаОтчета2025Кв1", Новый Структура);
	Алко40.ФормаОтчета2025Кв1.Вставить("ТабличныйОтчет", Ложь);
	Алко40.ФормаОтчета2025Кв1.Вставить("Утверждена", "Приказом Росалкогольтабакконтроля от 26.11.2024 г. № 461");
	Алко40.ФормаОтчета2025Кв1.Вставить("НачалоДействия", "2 квартал 2025 года");
	УстановитьДляФормыДействие(СписокДействий, Алко40.ФормаОтчета2025Кв1, Неопределено, Истина);
	...
	ИначеЕсли НЕ СтруктураРазрешенийФормы[Действие] Тогда
		...
		Если ПериодНачалаДействия = Неопределено Тогда
			...
			Иначе
				...
				НомерОтчета = Число(СтрЗаменить(ИсточникОтчета, "РегламентированныйОтчетАлкоПриложение", ""));
				
				Если НомерОтчета >= 35 И НомерОтчета <= 39 Тогда
					
					// Специфические отчеты по новым регионам РФ.
					
					МассивСообщения = Новый Массив;
					МассивСообщения.Добавить( НСтр("ru = '" + ПредставлениеДействия
						+ " редакции формы не предусмотрена.
						|
						|Образец формы опубликован на сайте ФСРАР 
						|'"));
					МассивСообщения.Добавить(
						Новый ФорматированнаяСтрока("https://fsrar.gov.ru/gorjachaja_linija_po_voprosam_predostavlenija_otchetnosti_v_novyh_subektah_rossijskoj_federacii",
						, Новый Цвет(28, 85, 174), 
						, "https://fsrar.gov.ru/gorjachaja_linija_po_voprosam_predostavlenija_otchetnosti_v_novyh_subektah_rossijskoj_federacii"));
					МассивСообщения.Добавить( НСтр("ru = '
						|
						|" + ПредставлениеДействия
						+ ", возможно, будет реализована в будущих версиях программы.
						|
						|Для отчетов, утвержденных для новых регионов РФ,"
						+ " процедура сдачи отчетности состоит в передаче из личного кабинета"
						+ " федеральной службы по контролю за алкогольным и табачным рынками" 
						+ " (Росалкогольтабакконтроль) файла декларации в формате MS Excel (xls).
						|
						|Файл декларации в формате MS Excel (xls) можно получить из меню отчета
						|Выгрузка -> Экспорт в MS Excel до 2007 (XLS).
						|В процессе экспорта никакие проверки данных отчета на ошибки не проводятся.'"));
						
					ТекстПредупреждения = Новый ФорматированнаяСтрока(МассивСообщения);	
...
Процедура ВызватьФормуВводаАдресаАЛКО(СохраненныеДанныеАдреса, ЗаголовокФормыВвода,
	...
	Если НЕ ТипЗнч(СохраненныеДанныеАдреса) = Тип("Структура") Тогда
		...
		ВызватьИсключение НСтр("ru = 'В процедуру РегламентированнаяОтчетностьАЛКОКлиент.ВызватьФормуВводаАдресаАЛКО 
			...
			|en = 'В процедуру РегламентированнаяОтчетностьАЛКОКлиент.ВызватьФормуВводаАдресаАЛКО 
			|передан неверный параметр СохраненныеДанныеАдреса, параметр должен быть структурой.'");
...
Функция ПроверитьПодключениеОрганизации(СтруктураРеквизитовФормы,
	...
	// Это БРО 1.1.10.
		...
		ИначеЕсли (НЕ Подключение.ФСРАР) и (НЕ ТолькоОтчетность) Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'По организации отчета не включена возможность электронного документооборота с Росалкогольтабакконтролем!';
											|en = 'По организации отчета не включена возможность электронного документооборота с Росалкогольтабакконтролем!'"));
		КонецЕсли;
...
Процедура ПриОткрытииЗавершениеАЛКО(Отказ, Форма) Экспорт
	...
	Если Отказ Тогда
		...
		КонецЕсли;
		Форма.ОбщийПараметрПроцедур.Вставить("ОнлайнБлокировка", Истина);
		
		ОтказЗакрытия = Ложь;
		ПередЗакрытиемАЛКО(Форма, ОтказЗакрытия, Ложь, "", Истина);
		Если НЕ ОтказЗакрытия Тогда
...
Процедура ПередЗакрытиемРегламентированногоОтчетаАЛКО(Форма, Отказ,
	...
	Перем ОнлайнБлокировка;
	
	Если Форма.ОбщийПараметрПроцедур = Неопределено Тогда
		Форма.ОбщийПараметрПроцедур = Новый Структура();
	КонецЕсли;
	Форма.ОбщийПараметрПроцедур.Свойство("ОнлайнБлокировка", ОнлайнБлокировка);
	ОнлайнБлокировка = ?(ОнлайнБлокировка = Неопределено, Ложь, ОнлайнБлокировка);
	
	Если ОнлайнБлокировка Тогда
...
Процедура ПослеВыполненияДлительнойОперацииАЛКО(Форма,
	...
	ИначеЕсли ВидДлОперации = ВидДлОперацииПечать
		...
		Если ПечатьУспешна и (НЕ (ВидПечати = "ПечатьФайлZIP" ИЛИ ВидПечати = "Экспорт")) Тогда
		...
		Если ПечатьУспешна и (ВидПечати = "ПечатьФайлZIP" ИЛИ ВидПечати = "Экспорт") Тогда
			...
			Если ВидПечати = "ПечатьФайлZIP" Тогда			
			    ...
			    СохранитьФайлыZIP(АдресВоВременномХранилище, ТипЭкспорта, "ALKO_" + Строка(Форма.НомерАлкоОтчета), 
					Форма.ОтчетныйПериодСГодом, Форма.ОрганизацияНаименованиеСокращенное);
					
			ИначеЕсли ВидПечати = "Экспорт" Тогда
				
				СохранитьФайлЭкспорта(АдресВоВременномХранилище, ТипЭкспорта, "ALKO_" + Строка(Форма.НомерАлкоОтчета), 
					Форма.ОтчетныйПериодСГодом, Форма.ОрганизацияНаименованиеСокращенное);
		...
		Форма.СтруктураРеквизитовФормы.Вставить("ТипЭкспорта", Неопределено);
		Форма.ОбщийПараметрПроцедур.Вставить("ВидПечати", Неопределено);
...
Процедура ВводРеквизитовОП_АЛКО(Форма, ПараметрыИзФормы = Неопределено)
    
	СведенияОбОП = ТекущееОбособленноеПодразделение(Форма);
	
	ЭтоВиноградная = НЕ (Форма.Элементы.Найти("ГруппаВиноградныхДанных") = Неопределено);
	ЕстьДобавитьВиноградник = (Форма.Элементы.Найти("ДобавитьВиноградник") <> Неопределено);
	...
	ПараметрыФормы.Вставить("ЭтоВиноградная", ЭтоВиноградная);
	ПараметрыФормы.Вставить("ЕстьДобавитьВиноградник", ЕстьДобавитьВиноградник);
	ПараметрыФормы.Вставить("УникальностьФормы", Форма.УникальностьФормы);
	ПараметрыФормы.Вставить("ПараметрыИзФормы", ПараметрыИзФормы);

	ИмяФормыВводРеквизитовОП = "ОбщаяФорма.АЛКОВводРеквизитовОП";

	ДополнительныеПараметры = Новый Структура("Форма, ИмяЭлементаСтруктуры, СведенияОбОП, ПараметрыИзФормы",
											Форма, ИмяЭлементаСтруктуры, СведенияОбОП, ПараметрыИзФормы);
...
Процедура ВводРеквизитовОПЗавершение(РезультатВвода,
	...
	// В случае заполнения реквизитов ОП по головному предприятию,
	// при наличии в данных СведенияОбОП Приложения (например Приложения 7 с 2025 года)
	// реквизита ВидРозничнойПродажи - НЕЛЬЗЯ позволять пользователю просто закрыть
	// форму ввода реквизитов по ОП без сохранения значения ВидРозничнойПродажи.
	//
	// Поэтому проверяем и если ВидРозничнойПродажи = 0 то
	// запускаем ввод реквизитов, до тех пор пока не сохранит данные.
	ПараметрыИзФормы = ДополнительныеПараметры.ПараметрыИзФормы;
	
	Если РегламентированнаяОтчетностьАЛКОКлиентСервер.ЭтоСтруктура(ПараметрыИзФормы)
		И
		ПараметрыИзФормы.Свойство("НуженВидПродажи")
		И
		ПараметрыИзФормы.НуженВидПродажи
		И
		(
			(
				РегламентированнаяОтчетностьАЛКОКлиентСервер.ЭтоСтруктура(РезультатВвода)
				И
				(НЕ ЗначениеЗаполнено(РезультатВвода.ВидРозничнойПродажи))
				И
				(
					ЗначениеЗаполнено(РезультатВвода.ПредставлениеАдреса)
					ИЛИ
					ЗначениеЗаполнено(РезультатВвода.КПП)
					ИЛИ
					ЗначениеЗаполнено(РезультатВвода.Наименование)
				)
			)
			ИЛИ
			(
				(
					НЕ (
						ТипЗнч(РезультатВвода) = Тип("Структура") 
						ИЛИ ТипЗнч(РезультатВвода) = Тип("ФиксированнаяСтруктура")
						)
				)
				И 
				(НЕ ЗначениеЗаполнено(СведенияОбОП.ВидРозничнойПродажи))
				И
				(
					ЗначениеЗаполнено(СведенияОбОП.ПредставлениеАдреса)
					ИЛИ
	...
	Параметры = Новый Структура("ИмяЭлементаСтруктуры, СведенияОбОП, ПараметрыИзФормы", 
								ИмяЭлементаСтруктуры, СведенияОбОП, ПараметрыИзФормы);
...
Процедура ВыборДеятельностиПоЛицензииЗавершение(РезультатВыбора,
	...
	Если ЗначениеЗаполнено(ВыбранныйКод) И (НЕ РегламентированнаяОтчетностьАЛКОКлиентСервер.ВСтрокеВсеЦифры(ВыбранныйКод)) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Код вида деятельности должен состоять из цифр!
											...
											|en = 'Код вида деятельности должен состоять из цифр!
											|Изменение вида деятельности не принято.'"));
		Возврат;
	...
	// Если выбран код больше 7 - т.е нелицензируемая деятельность
		...
		Если (КоличествоСтраницЛицензий > 1)
			...
			ТекстВопроса = НСтр("ru = 'Выбор этого вида деятельности приведет к удалению
				...
				|Продолжить?';
				|en = 'Выбор этого вида деятельности приведет к удалению
				|всех страниц этого раздела, кроме текущей. 
				|Если в отчете есть нелицензионный вид деятельности, 
				|он должен быть единственным.
				|Продолжить?'");
...
Процедура ПересчетИтоговПослеСохраненияАЛКО(Результат, Форма) Экспорт
	...
	// отказ от сохранения или ошибка сохранения
		ПоказатьПредупреждение(, НСтр("ru = 'Отчет не сохранен, продолжение пересчета итогов невозможно!';
										|en = 'Отчет не сохранен, продолжение пересчета итогов невозможно!'"));
		ВыполнитьСледующееДействиеИзОчередиАЛКО(Форма);
...
Процедура КомандаОчиститьРегистрыОтМусораАЛКО(Форма)
	...
	ТекстВнимание = Новый ФорматированнаяСтрока(НСтр("ru = 'Внимание!';
													|en = 'Внимание!'"), ПолужирныйНаклонныйШрифт, WebЦвета.Красный);
	ТекстПредупреждение = Новый ФорматированнаяСтрока(НСтр("ru = 'После применения данной процедуры восстановление удаленных страниц станет невозможным!';
															|en = 'После применения данной процедуры восстановление удаленных страниц станет невозможным!'"), ПолужирныйНаклонныйШрифт);
	ТекстПредупреждение2 = Новый ФорматированнаяСтрока(НСтр("ru = 'Если Отчет изменен, перед очисткой изменения будут сохранены!';
															|en = 'Если Отчет изменен, перед очисткой изменения будут сохранены!'"), ПолужирныйНаклонныйШрифт);
	...
	ТекстЧастичнаяОчистка = Новый ФорматированнаяСтрока(НСтр("ru = 'Частичная очистка';
															|en = 'Частичная очистка'"), ПолужирныйШрифт);
	ТекстЧастичнаяОчистка = Новый ФорматированнаяСтрока(ТекстЧастичнаяОчистка, НСтр("ru = ' - очистка регистров от данных удаленных страниц.
		...
		|
		|';
		|en = ' - очистка регистров от данных удаленных страниц.
		|Может выполняться длительное время, если удаленные страницы содержали большое количество строк.
		|
		|'"));

	ТекстПолнаяОчистка = Новый ФорматированнаяСтрока(НСтр("ru = 'Полная очистка';
															|en = 'Полная очистка'"), ПолужирныйШрифт);
	ТекстПолнаяОчистка = Новый ФорматированнаяСтрока(ТекстПолнаяОчистка, НСтр("ru = ' - очистка регистров от данных удаленных страниц и 
		...
		|
		|';
		|en = ' - очистка регистров от данных удаленных страниц и 
		|дополнительно удаляются строки, помеченные на удаление в активных (неудаленных) страницах.
		|Выполняется дольше частичной очистки. 
		|
		|Если помеченных на удаление строк немного, рекомендуется частичная очистка.
	...
	ТекстЗавершение = НСтр("ru = 'Очистка не влияет на работоспособность отчета, она необходима лишь в целях уменьшения 
		...
		|en = 'Очистка не влияет на работоспособность отчета, она необходима лишь в целях уменьшения 
		|размера информационной базы, особенно если база файловая.
		|
		|Для файловой ИБ возможна выдача ошибок о превышении максимального размера 
		|внутреннего файла 1Cv8.1CD в Журнале регистрации при операциях Заполнения, 
...
Процедура ОчисткаОтМусораПослеСохраненияАЛКО(Результат, Форма) Экспорт
	...
	// отказ от сохранения или ошибка сохранения
		ПоказатьПредупреждение(, НСтр("ru = 'Отчет не сохранен, продолжение очистки невозможно!';
										|en = 'Отчет не сохранен, продолжение очистки невозможно!'"));
		ВыполнитьСледующееДействиеИзОчередиАЛКО(Форма);
...
Процедура ПечатьФайлZIPВыборФорматаАЛКО(Форма)
	...
	СписокТиповЭкспорта.Добавить(ТипФайлаТабличногоДокумента.MXL, "Табличный документ 1С (*.mxl)");
	СписокТиповЭкспорта.Добавить(ТипФайлаПакетаОтображаемыхДокументов.XLSX, "Таблица MS Excel 2007-... (*.xlsx)");
	СписокТиповЭкспорта.Добавить(ТипФайлаПакетаОтображаемыхДокументов.ODS, "Таблица OpenDocument (*.ods)");
	
	СписокТиповЭкспорта.Добавить(ТипФайлаПакетаОтображаемыхДокументов.PDF, "Документ PDF (*.pdf)");
	СписокТиповЭкспорта.Добавить(ТипФайлаПакетаОтображаемыхДокументов.HTML4, "Документ HTML (*.html)");
...
//								
//
Процедура СохранитьФайлыZIP(АдресВоВременномХранилище, ТипЭкспорта,
		СокращенноеИмяОтчета, ОтчетныйПериодСГодом,
		ОрганизацияНаименованиеСокращенное)

	СоответсвиеРасширений = РегламентированнаяОтчетностьАЛКОКлиентСервер.СоответсвиеРасширенийЭкспорта();
	...
	СокращенноеНаименованиеОрг = 
		РегламентированнаяОтчетностьАЛКОКлиентСервер.ПравильноеИмяФайла(СокращенноеНаименованиеОрг);
	...
	Если ЭтоАдресВременногоХранилища(АдресВоВременномХранилище) Тогда
		...
		ПолучитьФайл(АдресВоВременномХранилище, ИмяКонечногоФайла, Истина);
...
Процедура УстановитьДляФормыДействие(СписокДействий, СтруктураФормыАлкоОтчета,
	...
	Если НЕ ТипЗнч(СписокДействий) = Тип("СписокЗначений") Тогда
		ВызватьИсключение НСтр("ru = 'В процедуру УстановитьДляФормыДействие() передан неверный список действий!';
								|en = 'В процедуру УстановитьДляФормыДействие() передан неверный список действий!'");
	КонецЕсли;
	...
	ИначеЕсли ЗначениеЗаполнено(Действие) Тогда
		ВызватьИсключение НСтр("ru = 'В процедуру УстановитьДляФормыДействие() передано неверное действие!';
								|en = 'В процедуру УстановитьДляФормыДействие() передано неверное действие!'");
	Иначе
...
Процедура ЭкспортXLS(Форма)

	Форма.СтруктураРеквизитовФормы.Вставить("ТипЭкспорта", ТипФайлаПакетаОтображаемыхДокументов.XLS);
	ПечатьАЛКО(Форма, "Экспорт");

КонецПроцедуры 

Процедура ЭкспортXLSX(Форма)

	Форма.СтруктураРеквизитовФормы.Вставить("ТипЭкспорта", ТипФайлаПакетаОтображаемыхДокументов.XLSX);
	ПечатьАЛКО(Форма, "Экспорт");

КонецПроцедуры

Процедура ЭкспортВыборФорматаАЛКО(Форма)

	Оповещение = Новый ОписаниеОповещения("ЭкспортЗавершениеАЛКО", ЭтотОбъект, Форма);

	СписокТиповЭкспорта = Новый СписокЗначений;
	
	СписокТиповЭкспорта.Добавить(ТипФайлаПакетаОтображаемыхДокументов.XLSX, "Таблица MS Excel 2007-... (*.xlsx)");
	СписокТиповЭкспорта.Добавить(ТипФайлаПакетаОтображаемыхДокументов.XLS, "Таблица MS Excel до 2007 (*.xls)");
	СписокТиповЭкспорта.Добавить(ТипФайлаПакетаОтображаемыхДокументов.ODS, "Таблица OpenDocument (*.ods)");
	
	СписокТиповЭкспорта.Добавить(ТипФайлаПакетаОтображаемыхДокументов.PDF, "Документ PDF (*.pdf)");
	СписокТиповЭкспорта.Добавить(ТипФайлаПакетаОтображаемыхДокументов.HTML4, "Документ HTML (*.html)");
	
	СписокТиповЭкспорта.ПоказатьВыборЭлемента(Оповещение, "Выберите формат экспорта");

КонецПроцедуры

Процедура ЭкспортЗавершениеАЛКО(ТипЭкспорта, Форма) Экспорт
...
// Сохраняет экспорт табличного документа в выбранном пользователем формате, 
// предварительно сохраненный во временном хранилище, на клиентском компьютере.
// При этом определяется имя файла по умолчанию для сохраняемого файла на основании переданных реквизитов 
// отчета.
//
...
//								
//
Процедура СохранитьФайлЭкспорта(АдресВоВременномХранилище, ТипЭкспорта,
		СокращенноеИмяОтчета, ОтчетныйПериодСГодом,
		ОрганизацияНаименованиеСокращенное)
...
Процедура ПечатьПослеСохраненияАЛКО(Отказ, Форма) Экспорт
	...
	ТекстПечати = ?(ВидПечати = "Экспорт", "экспорта", "печати");
	ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Отчет не сохранен, продолжение %1 невозможно!';
			|en = 'Отчет не сохранен, продолжение %1 невозможно!'"), ТекстПечати);
	...
	Если ВидПечати = "Экспорт" Тогда
		
		ТипЭкспорта = Форма.СтруктураРеквизитовФормы.ТипЭкспорта;
		
		КартинкаСостояния = БиблиотекаКартинок.Печать;
		
		Если ТипЭкспорта = ТипФайлаПакетаОтображаемыхДокументов.XLSX
			ИЛИ ТипЭкспорта = ТипФайлаПакетаОтображаемыхДокументов.XLS
			Тогда
			КартинкаСостояния = БиблиотекаКартинок.ЭкспортExcel;
		ИначеЕсли ТипЭкспорта = ТипФайлаПакетаОтображаемыхДокументов.ODS
			Тогда
			КартинкаСостояния = БиблиотекаКартинок.ЭкспортOpenOfficeCalc;
		ИначеЕсли ТипЭкспорта = ТипФайлаПакетаОтображаемыхДокументов.PDF
			Тогда
			КартинкаСостояния = БиблиотекаКартинок.ЭкспортPDF;
		ИначеЕсли ТипЭкспорта = ТипФайлаПакетаОтображаемыхДокументов.HTML4
			...
			Тогда
			КартинкаСостояния = БиблиотекаКартинок.ЭкспортHTML;
		КонецЕсли;
		
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1. Экспорт...';
				|en = '%1. Экспорт...'"), Форма.Заголовок), , , 
			КартинкаСостояния);
	...
	Иначе
		...
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1. Формируется печатная форма...';
				|en = '%1. Формируется печатная форма...'"), Форма.Заголовок), , , 
...
Процедура Печать_ДлительнаяОперацияАЛКО(Форма)
	...
	// отказ от сохранения или ошибка сохранения
		ПоказатьПредупреждение(, НСтр("ru = 'Отчет не сохранен, продолжение печати невозможно!';
										|en = 'Отчет не сохранен, продолжение печати невозможно!'"));
		ВыполнитьСледующееДействиеИзОчередиАЛКО(Форма);
...
Процедура ОткрытьФормуВыбораСтраницОтчетаАЛКО(Форма, ВариантВыбора)
	...
	мПараметры.Вставить("ЗначенияИзСписка", Форма.ЕстьСпискиВыбора);
...
Процедура ОрганизацияОткрытиеЗавершениеАЛКО(Результат, Форма) Экспорт
	...
	ТекстВопроса = НСтр("ru = 'Обновить данные организации в отчете?';
						|en = 'Обновить данные организации в отчете?'");
...
Процедура ПроверитьВыгрузкуОтчетаАЛКО(Форма)
	...
	Если НЕ ЗначениеЗаполнено(Форма.СтруктураРеквизитовФормы.Организация) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Организация не выбрана! Проверка выгрузки невозможна.';
								|en = 'Организация не выбрана! Проверка выгрузки невозможна.'");
		Сообщение.Сообщить();
...
Процедура ПроверитьВыгрузкуОтчетаПослеСохраненияАЛКО(Результат, Форма) Экспорт
	...
	// отказ от сохранения или ошибка сохранения
		ПоказатьПредупреждение(, НСтр("ru = 'Отчет не сохранен, продолжение проверки выгрузки невозможно!';
										|en = 'Отчет не сохранен, продолжение проверки выгрузки невозможно!'"));
		ВыполнитьСледующееДействиеИзОчередиАЛКО(Форма);
...
Процедура ВыгрузитьОтчетПослеСохраненияАЛКО(Результат, Форма) Экспорт
	...
	// отказ от сохранения или ошибка сохранения
		ПоказатьПредупреждение(, НСтр("ru = 'Отчет не сохранен, продолжение выгрузки невозможно!';
										|en = 'Отчет не сохранен, продолжение выгрузки невозможно!'"));
...
Процедура РезультатСохраненияАЛКО(Форма, Автосохранение = Ложь, КодИФНС, КПП,
	...
	Если Форма.ПользовательСохранилОтчет
		...
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сохраняется %1...';
																				|en = 'Сохраняется %1...'"), Форма.Заголовок), , , БиблиотекаКартинок.Записать);
	Иначе
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сохраняется временный %1...';
																				|en = 'Сохраняется временный %1...'"), Форма.Заголовок), , , БиблиотекаКартинок.Записать);
	КонецЕсли;
...
Процедура СкопироватьДанныеРегистров_ДлительнаяОперацияАЛКО(Форма)
	...
	// отказ от сохранения или ошибка сохранения
		ПоказатьПредупреждение(, НСтр("ru = 'Отчет не сохранен, копирование данных регистров невозможно!';
										|en = 'Отчет не сохранен, копирование данных регистров невозможно!'"));
		ВыполнитьСледующееДействиеИзОчередиАЛКО(Форма);
...
Процедура ЗакрытьФормуОшибокПриИзмененииОтчета(Форма)
	...
	Если (НЕ Форма.ФормаОшибок = Неопределено)
		...
		Сообщение.Текст = НСтр("ru = 'Отчет изменен, поэтому открытая форма ошибок проверки выгрузки отчета закрыта, 
			...
			|en = 'Отчет изменен, поэтому открытая форма ошибок проверки выгрузки отчета закрыта, 
			|поскольку может содержать неактуальные или некорректные данные об ошибках.
			|Запустите проверку выгрузки отчета еще раз после сохранения отчета.'");
		Сообщение.Сообщить();
