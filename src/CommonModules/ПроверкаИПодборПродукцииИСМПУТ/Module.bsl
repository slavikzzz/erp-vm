...
Процедура ЗаполнитьМаркируемуюПродукциюДокумента(Контекст, ВидМаркируемойПродукции, ТаблицаМаркируемойПродукции) Экспорт
	...
	//++ НЕ УТ
	ИначеЕсли ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(Контекст, "ПроизводствоБезЗаказа") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииПроизводствоБезЗаказа(Контекст, ВидМаркируемойПродукции);
	//-- НЕ УТ

	//++ НЕ УТКА
	ИначеЕсли ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(Контекст, "ЭтапПроизводства2_2") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииЭтапПроизводства(Контекст, ВидМаркируемойПродукции);
	//-- НЕ УТКА
	Иначе
		Возврат;
	КонецЕсли;
...
//++ НЕ УТ
Функция ЗапросМаркируемойПродукцииПроизводствоБезЗаказа(Документ, ВидМаркируемойПродукции)
	...
	|		И Товары.НомерГруппыЗатрат = Серии.НомерГруппыЗатрат
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|			ПО Товары.Номенклатура = СправочникНоменклатура.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	|			ПО Товары.Номенклатура = ШтрихкодыНоменклатуры.Номенклатура
	|		И Товары.Характеристика = ШтрихкодыНоменклатуры.Характеристика
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОписаниеНоменклатурыИС КАК ОписаниеИС
	|			ПО Товары.Номенклатура = ОписаниеИС.Номенклатура
	|ГДЕ
	|	Товары.Ссылка = &ДокументСсылка
	|	И &УсловиеМаркируемаяПродукция
	|	И &УсловиеНефасованнаяПродукция
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа.ПобочныеИзделияСерии КАК Серии
	|			ПО Серии.Ссылка = &ДокументСсылка
	|			И Товары.Номенклатура = Серии.Номенклатура
	|			И Товары.Характеристика = Серии.Характеристика
	|			И Товары.Назначение = Серии.Назначение
	...
	|			И Товары.НомерГруппыЗатрат = Серии.НомерГруппыЗатрат
	//++ НЕ УТКА
	|			И Товары.ВладелецИзделия = Серии.ВладелецИзделия
	//-- НЕ УТКА
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|			ПО Товары.Номенклатура = СправочникНоменклатура.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
...
//++ НЕ УТКА
Функция ЗапросМаркируемойПродукцииЭтапПроизводства(Документ, ВидМаркируемойПродукции)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ПобочныеИзделияСерии КАК Серии
	|		ПО Серии.Ссылка = &ДокументСсылка
	|			И Товары.Номенклатура = Серии.Номенклатура
	|			И Товары.Характеристика = Серии.Характеристика
	|			И Товары.Назначение = Серии.Назначение
