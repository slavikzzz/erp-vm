...
Функция НовыеДанныеОбъектов() Экспорт
	...
	ДанныеОбъектов.Колонки.Добавить("Объект", Новый ОписаниеТипов(ОбщегоНазначенияБЭД.ОписаниеТипаВсеСсылки(), "Строка",,,
...
Процедура ИзменитьСостояниеОбмена(ИдентификаторУчетнойЗаписи, Свойство, Значение) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Недопустимый параметр процедуры СинхронизацияЭДО.ИзменитьСостояниеОбмена';
								|en = 'Invalid parameter of the СинхронизацияЭДО.ИзменитьСостояниеОбмена procedure'");
	КонецЕсли;
...
Функция ДатыЗапросаДанныхОператораЭДО(ИдентификаторыУченыхЗаписей, ТипДанных) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Неверный параметр ""Тип данных"" в функции ""ДатыЗапросаДанныхОператораЭДО""';
								|en = 'Invalid parameter ""Data type"" in the ""ДатыЗапросаДанныхОператораЭДО"" function'");
	КонецЕсли;
...
Функция ПроверитьОбменЧерезКаталог(КаталогОбмена) Экспорт
	...
	Исключение
		ОписаниеОшибки = НСтр("ru = 'При проверке подключения возникли ошибки:';
								|en = 'Errors occurred when checking connection:'")
...
Функция ОбновитьКешиОператоровЭДОИФорматов(КонтекстДиагностики = Неопределено) Экспорт
	...
	ОбновлениеКлассификатораПолномочийМЧД003       = ТипыСинхронизации.ОбновлениеКлассификатораПолномочийМЧД003;
	ОбновлениеПеречняСпециальныхОбстоятельств      = ТипыСинхронизации.ОбновлениеПеречняСпециальныхОбстоятельств;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Попытка
			...
			ИначеЕсли ВыборкаДетальныеЗаписи.ТипСинхронизации = ОбновлениеКлассификатораПолномочийМЧД003 Тогда
				
				ПараметрыСинхронизации = ПараметрыСинхронизацииКлассификатораПолномочий_МЧД003();
				ПараметрыСинхронизации.ДатаПоследнегоИзменения = ВыборкаДетальныеЗаписи.ДатаПоследнегоИзменения;
				ПараметрыСинхронизации.КонтекстДиагностики     = КонтекстДиагностики;
				
				СинхронизироватьКлассификаторПолномочий_МЧД003(ПараметрыСинхронизации);
			ИначеЕсли ВыборкаДетальныеЗаписи.ТипСинхронизации = ОбновлениеПеречняСпециальныхОбстоятельств Тогда
				СинхронизироватьПереченьСпециальныхОбстоятельств(ОбновлениеПеречняСпециальныхОбстоятельств, 
					ВыборкаДетальныеЗаписи.ДатаПоследнегоИзменения, КонтекстДиагностики);
			КонецЕсли;
		...
		Исключение
			...
			ВидОперации = НСтр("ru = 'Обновление кеша настроек ЭДО';
								|en = 'Updating the EDI settings cache'");
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Исключение
			...
			ВидОперации = НСтр("ru = 'Обновление учетной записи в сервисе настроек';
								|en = 'Updating your account in the settings service'");
			ЭлектронноеВзаимодействие.ОбработатьОшибку(ВидОперации,
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ОписаниеОшибки));
...
Процедура ПрочитатьДополнительныеДанныеДокумента(ДополнительныеФайлы, Знач ОписаниеДополнительныхДанных, КонтекстДиагностики = Неопределено) Экспорт
	...
	ВидОперации = НСтр("ru = 'Распаковка дополнительных файлов ЭДО';
						|en = 'Unpack additional EDI files'", ОбщегоНазначения.КодОсновногоЯзыка());
	ТекстСообщения = НСтр("ru = 'Ошибка при распаковке дополнительных файлов ЭДО';
							|en = 'An error occurred when unpacking additional EDI files'", ОбщегоНазначения.КодОсновногоЯзыка());
	Попытка
	...
	Для Каждого СтрокаФайл Из РезультатРаспаковки Цикл
		Если Не СтрокаФайл.ЭтоФайл() Тогда
			Продолжить;
		КонецЕсли;
		ДополнительныйФайл = ДополнительныеФайлы.Добавить();
...
Функция ВидОшибкиЕстьНеполученныеЭлектронныеДокументы() Экспорт
	...
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Дата запроса получения данных оператора ЭДО ограничивает получение документов';
										|en = 'The date of the request for receiving data from the EDI service provider limits the receipt of documents'");
	ВидОшибки.ОписаниеРешения = НСтр("ru = '<a href = ""Выберите"">Выберите</a> дальнейшее действие';
									|en = '<a href = ""Выберите"">Select</a> further action'");
...
Функция ВидОшибкиЕстьНеполученныеПриглашения() Экспорт
	...
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Получены не все приглашения к обмену электронными документами';
										|en = 'Not all invitations to exchange electronic documents received'");
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Дата запроса получения данных оператора ЭДО ограничивает получение приглашений к обмену электронными документами';
										|en = 'The date of the request for receiving data from the EDI service provider limits the receipt of invitations to the electronic documents exchange'");
	ВидОшибки.ОписаниеРешения = НСтр("ru = '<a href = ""Выберите"">Выберите</a> дальнейшее действие';
									|en = '<a href = ""Выберите"">Select</a> further action'");
	ВидОшибки.ОбработчикиНажатия.Вставить("Выберите",
...
Процедура ДобавитьОшибкуОтсутствияСертификатовДляПодписания(КонтекстДиагностики, ВидОперации) Экспорт
	...
	ТекстОшибки = НСтр("ru = 'В процессе подписания электронного документа не найден ни один из доступных сертификатов ЭП.';
						|en = 'No available electronic signature certificate is found upon signing the electronic document.'");
	Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации,
...
Процедура ПриОпределенииСпискаКомандЭДО(Знач СоставКоманд, Знач ПолноеИмя, Знач НаправлениеЭД, КомандыЭДО) Экспорт
	...
	Если СоставКоманд.Контрагенты.Найти(ПолноеИмя) <> Неопределено И ИспользоватьОбменЭД Тогда
		...
		КомандаЭДО.Представление = НСтр("ru = 'Пригласить к обмену электронными документами';
										|en = 'Invite for electronic document exchange'");
	...
	Если СоставКоманд.Входящие.Найти(ПолноеИмя) <> Неопределено 
		...
		Если ИспользоватьОбменЭД Тогда
			...
			КомандаЭДО.Представление = НСтр("ru = 'Перезаполнить по электронному документу';
											|en = 'Repopulate by electronic document'");
			КомандаЭДО.Порядок       = 30;
...
// 
// Параметры:
//  ПринудительноеОбновление - Булево - Если Истина, то не учитывается дата последнего изменения данных и обновление
//                                      происходит всегда.
//  
// Возвращаемое значение:
//  см. ПараметрыСинхронизацииКлассификатораПолномочийФНС_МЧД002
//
Функция ЗаполнитьПараметрыСинхронизацииКлассификатораПолномочийФНС_МЧД002(ПринудительноеОбновление = Ложь) Экспорт
...
// Заполнить параметры синхронизации классификатора (МЧД 003).
// 
// Параметры:
//  ПринудительноеОбновление - Булево - Если Истина, то не учитывается дата последнего изменения данных и обновление
//                                      происходит всегда.
//  
// Возвращаемое значение:
//  см. ПараметрыСинхронизацииКлассификатораПолномочий_МЧД003
//
Функция ЗаполнитьПараметрыСинхронизацииКлассификатораПолномочий_МЧД003(ПринудительноеОбновление = Ложь) Экспорт
	
	ПараметрыОперации = ПараметрыСинхронизацииКлассификатораПолномочий_МЧД003();
	...
	Если Не ПринудительноеОбновление Тогда
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	СостоянияСинхронизацииСервисНастроекЭДО.ТипСинхронизации =
			|		ЗНАЧЕНИЕ(Перечисление.ТипыСинхронизацииСервисаНастроекЭДО.ОбновлениеКлассификатораПолномочийМЧД003)";
...
Процедура СинхронизироватьКлассификаторПолномочийФНС_МЧД002(ПараметрыОперации, АдресРезультата = Неопределено) Экспорт
	...
	ДанныеКлассификатора = СервисНастроекЭДО.ПолучитьКлассификаторПолномочийФНС_МЧД002(
		ПараметрыОперации.КонтекстДиагностики, ПараметрыОперации.ДатаПоследнегоИзменения);
...
// Синхронизировать классификатор полномочий (МЧД 003).
// 
// Параметры:
//  ПараметрыОперации - см. ПараметрыСинхронизацииКлассификатораПолномочий_МЧД003
//  АдресРезультата   - Строка - 
Процедура СинхронизироватьКлассификаторПолномочий_МЧД003(ПараметрыОперации, АдресРезультата = Неопределено) Экспорт
	
	ДанныеКлассификатора = СервисНастроекЭДО.ПолучитьКлассификаторПолномочий_МЧД003(
		ПараметрыОперации.КонтекстДиагностики, ПараметрыОперации.ДатаПоследнегоИзменения);
	Если ДанныеКлассификатора = Неопределено Тогда
		Возврат;
	КонецЕсли;
	НачатьТранзакцию();
	Попытка
		МашиночитаемыеДоверенности.ОбновитьКлассификаторПолномочий_МЧД003(ДанныеКлассификатора);
		СервисНастроекЭДО.ИзменитьСостояниеСинхронизации(ПараметрыОперации.ТипСинхронизации,
			ДанныеКлассификатора.ДатаПоследнегоИзменения);
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
		ИмяСобытия = НСтр("ru = 'Синхронизация классификатора полномочий (МЧД 003) из сервиса настроек';
...
// Синхронизировать перечень специальных обстоятельств.
// 
// Параметры:
//  ТипСинхронизации - ПеречислениеСсылка.ТипыСинхронизацииСервисаНастроекЭДО - 
//  ДатаПоследнегоИзменения - Дата - Дата последнего изменения.
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура СинхронизироватьПереченьСпециальныхОбстоятельств(ТипСинхронизации,
	ДатаПоследнегоИзменения, КонтекстДиагностики) Экспорт
	
	ДанныеСервиса = СервисНастроекЭДО.ПолучитьПереченьСпециальныхОбстоятельств(
	...
	НачатьТранзакцию();
	Попытка
		РегистрыСведений.ПереченьСпециальныхОбстоятельствДокументаЭДО.ОбновитьПереченьСпецОбстоятельств(
			ДанныеСервиса.ДанныеФайла);
		СервисНастроекЭДО.ИзменитьСостояниеСинхронизации(ТипСинхронизации, ДанныеСервиса.ДатаИзмененияФайла);
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
		ИмяСобытия = НСтр("ru = 'Обновление перечня специальных обстоятельств ФНС';
							|en = 'Обновление перечня специальных обстоятельств ФНС'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
...
Процедура СинхронизироватьФорматыЭлектронныхДокументовИзСервисаНастроек(ТипСинхронизации, ДатаПоследнегоИзменения,
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru = 'Синхронизация форматов электронных документов из сервиса настроек';
							|en = 'Synchronize electronic document formats from the settings service'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
...
Процедура СинхронизироватьСвязьВидовИФорматовЭлектронныхДокументовИзСервисаНастроек(ТипСинхронизации,
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru = 'Синхронизация связи видов и форматов электронных документов из сервиса настроек';
							|en = 'Synchronize the link of electronic document kinds and formats from the settings service'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
...
Процедура СинхронизироватьАбонентовЭлектронногоДокументооборотаИзСервисаНастроек(ТипСинхронизации,
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru = 'Синхронизация абонентов электронного документооборота из сервиса настроек';
							|en = 'EDI subscriber synchronization from the setting service'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
...
Функция ВидОшибкиОтсутствуетСертификатДляПодписания()
	...
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Отсутствует доступный сертификат для подписания документов';
										|en = 'There is no available certificate for signing documents'");
	...
	ОписаниеРешения = СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru = '<a href = ""Проверьте"">Проверьте</a> настройки электронных подписей.';
																	|en = '<a href = ""Проверьте"">Check</a> electronic signature settings.'"));
...
// Параметры синхронизации классификатора (МЧД 003).
// 
// Возвращаемое значение:
//  Структура - Параметры операции обновления классификатора:
// * ТипСинхронизации        - ПеречислениеСсылка.ТипыСинхронизацииСервисаНастроекЭДО
...
// * КонтекстДиагностики     - Неопределено
//
Функция ПараметрыСинхронизацииКлассификатораПолномочий_МЧД003()
	
	ПараметрыОперации = Новый Структура;
	ПараметрыОперации.Вставить("ТипСинхронизации",        Перечисления.ТипыСинхронизацииСервисаНастроекЭДО.ОбновлениеКлассификатораПолномочийМЧД003);
	ПараметрыОперации.Вставить("ДатаПоследнегоИзменения", Дата(1, 1, 1));
	ПараметрыОперации.Вставить("КонтекстДиагностики",     Неопределено);
