...
Функция КомпонентыОбмена(НаправлениеОбмена, ВерсияФормата, РасширениеФормата, ВерсияФорматаОбменаПриЗагрузке = "", РасширениеФорматаПриЗагрузке = "") 
	...
	Если ОбщегоНазначенияБПО.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
		...
		КомпонентыОбмена.КлючСообщенияЖурналаРегистрации = НСтр("ru = 'Обмен ККМ офлайн';
																|en = 'Cash register offline exchange'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Если ЭлементыФормата.Количество() = 0 Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Неканоническое имя формата обмена <%1>';
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Неканоническое представление версии формата обмена: <%1>.';
										|en = 'Non-canonical presentation of the exchange format v.%1.'"), Результат.Версия);
	КонецЕсли;
