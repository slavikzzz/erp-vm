...
Процедура ИнициализироватьДанныеКонтроляИзменений(Запрос, ТекстыЗапроса, Документ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияТоварыКОформлениюДокументовИмпортаИзменение") Тогда
		...
		ТекстЗапроса =
			...
			|	) КАК ТаблицаОстатков
			//++ НЕ УТ
			|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаключениеДоговораАренды КАК ДокументЗаключенияДоговораАренды 
			|		ПО ТаблицаОстатков.ДокументПоступления = ДокументЗаключенияДоговораАренды.Ссылка
			//-- НЕ УТ
			|
			|ГДЕ
			|	(ТаблицаОстатков.КоличествоОстаток < 0 ИЛИ ТаблицаОстатков.КоличествоПоРНПТОстаток < 0)
			//++ НЕ УТ
			|	И ДокументЗаключенияДоговораАренды.Ссылка ЕСТЬ NULL
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			...
			|	ТаблицаОстатков.ВидЗапасов 									КАК ВидЗапасов,
			|	ТаблицаОстатков.ТипДокументаИмпорта 						КАК ТипДокументаИмпорта,
			|	ТаблицаОстатков.СуммаОборот 								КАК Сумма,
			|	0															КАК Количество,
			...
			|			Месяц,
			|			,
			|			(Организация, Поставщик, АналитикаУчетаНоменклатуры, ВидЗапасов, ДокументПоступления, ТипДокументаИмпорта) В
			|				(ВЫБРАТЬ
			|					Таблица.Организация,
			|					Таблица.Поставщик,
			|					Таблица.АналитикаУчетаНоменклатуры,
			...
			|				ИЗ
			|					ДвиженияТоварыКОформлениюДокументовИмпортаИзменение КАК Таблица)
			|			И ТипДокументаИмпорта = &ИдентификаторЗаявлениеОВвозеТоваров) КАК ТаблицаОстатков
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаключениеДоговораАренды КАК ДокументЗаключенияДоговораАренды
			|		ПО ТаблицаОстатков.ДокументПоступления = ДокументЗаключенияДоговораАренды.Ссылка
			|ГДЕ
			|	ТаблицаОстатков.СуммаОборот < 0
			//-- НЕ УТ
			|";
...
Процедура СообщитьОРезультатахКонтроляИзменений(РезультатыКонтроля, Документ, Отказ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияТоварыКОформлениюДокументовИмпортаИзменение") Тогда
		...
		ШаблонСообщенияБезПоступления = НСтр("ru = 'Номенклатура %1
			...
			|en = 'Item %1
			|The quantity of goods to register is not equal to the quantity in the receipt document by %2 %3'");
		ШаблонСообщенияСПоступлением = ШаблонСообщенияБезПоступления + " " + НСтр("ru = '(см. документ %4)';
																					|en = '(see the %4 document)'");
		...
		ШаблонСообщенияБезПоступленияАренда = НСтр("ru = 'Сумма услуг к оформлению не соответствует сумме в документе поступления на %1 %2';
													|en = 'Service amount to register is not equal to the amount in the receipt document by %1 %2'");
		ШаблонСообщенияСПоступлениемАренда = ШаблонСообщенияБезПоступленияАренда + " " + НСтр("ru = '(см. документ %3)';
																								|en = '(see the %3 document)'");
		
		Для каждого СтрокаОшибки Из РезультатыКонтроля.ОшибкиТоварыКОформлениюДокументовИмпорта Цикл
			
			//++ НЕ УТ
			Если ТипЗнч(СтрокаОшибки.ДокументПоступления) = Тип("ДокументСсылка.ЗаключениеДоговораАренды") Тогда 

				Если ЗначениеЗаполнено(СтрокаОшибки.ДокументПоступления) Тогда
					ТекстСообщения = СтрШаблон(ШаблонСообщенияСПоступлениемАренда,
						-СтрокаОшибки.Сумма, СтрокаОшибки.ЕдиницаИзмерения, СтрокаОшибки.ДокументПоступления);
				Иначе
					ТекстСообщения = СтрШаблон(ШаблонСообщенияБезПоступленияАренда,
						-СтрокаОшибки.Сумма, СтрокаОшибки.ЕдиницаИзмерения);
				КонецЕсли;
