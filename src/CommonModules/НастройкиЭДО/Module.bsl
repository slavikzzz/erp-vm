...
// Запрос настроек по участникам ЭДО. Содержит поля временной таблицы ОбъектУчета, Формат
// 
// Параметры:
...
// Возвращаемое значение:
//  см. ОбщегоНазначенияБЭД.НовоеОписаниеЗапроса
Функция ЗапросНастрокиОтправкиЭлектронныхДокументовПоВидамПоОписаниюОбъектовУчета(ОписанияОбъектовУчета, ИмяВременнойТаблицы = "") Экспорт
	ОписаниеЗапроса = ОбщегоНазначенияБЭД.НовоеОписаниеЗапроса();
	...
	ОписаниеЗапроса.СлужебныеПараметры.Вставить("ПустойДоговор", ПустойДоговор);
	...
	ОписаниеЗапроса.Текст =
		...
		|ПОМЕСТИТЬ ДанныеПараметровНастроек
		|ИЗ
		...
		|	НастройкиОтправкиЭлектронныхДокументовПоВидам.Договор = ОписанияОбъектовУчета.ДоговорКонтрагента КАК Приоритет
		|ПОМЕСТИТЬ НастройкиПоВидам
		|ИЗ
		|	ДанныеПараметровНастроек КАК ОписанияОбъектовУчета
		...
		|		ПО ОписанияОбъектовУчета.Организация = НастройкиОтправкиЭлектронныхДокументовПоВидам.Отправитель
		|		И ОписанияОбъектовУчета.Контрагент = НастройкиОтправкиЭлектронныхДокументовПоВидам.Получатель
		|		И ОписанияОбъектовУчета.ТипДокумента = НастройкиОтправкиЭлектронныхДокументовПоВидам.ВидДокумента.ТипДокумента
		|ГДЕ
		|	(ОписанияОбъектовУчета.ДоговорКонтрагента = НастройкиОтправкиЭлектронныхДокументовПоВидам.Договор
		|	ИЛИ НастройкиОтправкиЭлектронныхДокументовПоВидам.Договор В (&ПустойДоговор))
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ОбъектУчета
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	НастройкиПоВидам.ОбъектУчета КАК ОбъектУчета,
		|	МАКСИМУМ(НастройкиПоВидам.Приоритет) КАК Приоритет
		|ПОМЕСТИТЬ ПриоритетПоДоговорам
		|ИЗ
		|	НастройкиПоВидам КАК НастройкиПоВидам
		|СГРУППИРОВАТЬ ПО
		|	НастройкиПоВидам.ОбъектУчета
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ОбъектУчета
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ИмяВременнойТаблицы.ОбъектУчета КАК ОбъектУчета,
		|	ИмяВременнойТаблицы.Формат КАК Формат
		|ПОМЕСТИТЬ ИмяВременнойТаблицы
		|ИЗ
		|	НастройкиПоВидам КАК ИмяВременнойТаблицы
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПриоритетПоДоговорам КАК ПриоритетПоДоговорам
		|		ПО ИмяВременнойТаблицы.ОбъектУчета = ПриоритетПоДоговорам.ОбъектУчета
		|		И ИмяВременнойТаблицы.Приоритет = ПриоритетПоДоговорам.Приоритет
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ОбъектУчета";
...
Процедура ОбновитьФорматПоУмолчаниюВНастройках() Экспорт
	...
	Шаблон = НСтр("ru = 'Для вида ЭД ''%1'' указан одинаковый приоритет у %2 форматов';
					|en = '%2 formats have the same priority for ED kind ''%1'''");
	...
	Пока Выборка.Следующий() Цикл
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Обновление форматов в настройках отправки ЭДО';
														|en = 'Update formats in EDW dispatch settings'"), ТекстОшибки);
...
// Возвращает возможность завершения обмена без получения извещения
// 
// Параметры:
// 	ТипДокумента - ПеречислениеСсылка.ТипыДокументовЭДО
// Возвращаемое значение:
// 	Булево - признак возможности завершения обмена без получения извещений
Функция ОбменПоТипуДокументаМожноЗавершитьБезИзвещения(ТипДокумента) Экспорт 
	
	ТипДокументаМожноЗавершитьБезИзвещения = Новый Массив;
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.АктВыполненныхРабот);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.АктНаПередачуПрав);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.АктОРасхождениях);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.АктСверкиВзаиморасчетов);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.ДоговорныйДокумент);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.ЗаказТовара);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.ЗапросКоммерческихПредложений);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.КаталогТоваров);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.КоммерческоеПредложение);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.КорректировочныйСчетФактура);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.ОтветНаЗаказ);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.ОтчетОПродажахКомиссионногоТовара);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.ОтчетОСписанииКомиссионногоТовара);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.ПрайсЛист);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.СоглашениеОбИзмененииСтоимости);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.СчетНаОплату);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.СчетФактура);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.ТоварнаяНакладная);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.РеквизитыОрганизации);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.УПД);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.УКД);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.АктПриемкиСтроительныхРаботУслуг);

	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.СведенияОРеализацииКомиссионером);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.СведенияОЗакупкеКомиссионером);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.КорректировкаСведенийОРеализацииКомиссионером);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.КорректировкаСведенийОЗакупкеКомиссионером);

	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.ВозвратТоваровМеждуОрганизациями);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.ПередачаТоваровМеждуОрганизациями);
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.МашиночитаемаяДоверенность);
	
	ТипДокументаМожноЗавершитьБезИзвещения.Добавить(Перечисления.ТипыДокументовЭДО.Прочее);
	
	Возврат Не ТипДокументаМожноЗавершитьБезИзвещения.Найти(ТипДокумента) = Неопределено;
