...
Процедура ПриЗаполненииРеквизитовВладельцаВЗаявленииНаСертификат(Параметры) Экспорт
	...
	//++ НЕ УТ
	Параметры.МестоРождения     = ТекущийВладелец.МестоРождения;
	Параметры.СтраховойНомерПФР = ТекущийВладелец.СтраховойНомерПФР;
	
	Гражд = РегистрыСведений.ГражданствоФизическихЛиц.СрезПоследних(, Новый Структура("ФизическоеЛицо", Параметры.Сотрудник));
	Если Гражд.Количество() > 0 Тогда
		Параметры.Гражданство = Гражд[0].Страна;
	КонецЕсли;
	//-- НЕ УТ
