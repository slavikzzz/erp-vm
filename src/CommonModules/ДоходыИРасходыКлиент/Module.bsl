...
Процедура НачалоВыбораАналитикиРасходов(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	//++ НЕ УТ
	Если ТипЗнч(АналитикаРасходов) = Тип("СправочникСсылка.ПартииПроизводства") Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ЗначениеОтбораОрганизация = Новый Структура("Организация", Неопределено);
		ЗначениеОтбораДата = Новый Структура("Дата", Неопределено);
		ЗаполнитьЗначенияСвойств(ЗначениеОтбораОрганизация, Данные);
		ЗаполнитьЗначенияСвойств(ЗначениеОтбораДата, Данные);
		ВРеквизитахФормыЕстьОбъект = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "Объект");
		Если ВРеквизитахФормыЕстьОбъект И Не ЗначениеЗаполнено(ЗначениеОтбораОрганизация.Организация) Тогда
			ЗаполнитьЗначенияСвойств(ЗначениеОтбораОрганизация, Форма.Объект);
		КонецЕсли;
		
		Если ВРеквизитахФормыЕстьОбъект И Не ЗначениеЗаполнено(ЗначениеОтбораДата.Дата) Тогда
			ЗаполнитьЗначенияСвойств(ЗначениеОтбораДата, Форма.Объект);
		КонецЕсли;
		
		СтруктураОтбора = Новый Структура;
		СтруктураОтбора.Вставить("Организация", ЗначениеОтбораОрганизация.Организация);
		ПараметрыФормы = Новый Структура();
		
		Если ЗначениеЗаполнено(ЗначениеОтбораДата.Дата) Тогда
			ПараметрыФормы.Вставить("НачалоПериода",	НачалоМесяца(ЗначениеОтбораДата.Дата));
			ПараметрыФормы.Вставить("ОкончаниеПериода",	КонецМесяца(ЗначениеОтбораДата.Дата));
		КонецЕсли;

		ПараметрыФормы.Вставить("СтруктураОтбора",	СтруктураОтбора);
		ПараметрыФормы.Вставить("ОдиночныйВыбор",	Истина);
		ПараметрыФормы.Вставить("ОповещатьОВыборе",	Истина);
		ПараметрыФормы.Вставить("ИзменениеПериодаИОрганизации", Истина);
