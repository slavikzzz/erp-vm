...
Процедура РазместитьРезультатыАвтозаполнения(СтраницаРаздела, РассчитанныеПоказатели) Экспорт
...
Функция СчетаАнализовСчетов() Экспорт
	...
	СписокСчетов =
		"01, 02, 03, 04, 05, 07, 08, 10, 14, 15, 16, 19,
		|20, 23, 25, 26, 41, 43, 44, 45, 50, 51, 52, 55, 57, 58, 59,
		|60, 62, 63, 66, 67, 68, 69, 70, 71, 73, 75, 76, 79,
		|80, 81, 86, 90, 91";
...
// Возвращает сведения для заполнения ОСВ.
//
// Параметры:
...
//   ДеревоЗначений:
//     * КорневойСчет - ПланСчетовСсылка.Хозрасчетный
//     * Счет - ПланСчетовСсылка.Хозрасчетный
//     * СНД - Число - сальдо начальное по дебету счета;
//     * СНК - Число - сальдо начальное по кредиту счета;
//     * ДО - Число - дебетовый оборот по счету;
//     * КО - Число - кредитовый оборот по счету;
//     * СКД - Число - сальдо конечное по дебету счета;
//     * СКК - Число - сальдо конечное по кредиту счета.
//
Функция СведенияОСВЗаПериод_2017Кв3(Параметры) Экспорт
...
// Возвращает результат выполнения запроса по сведениям для заполнения анализа счета.
//
// Параметры:
//   Организация - СправочникСсылка.Организации
//   Счет - ПланСчетовСсылка.Хозрасчетный
...
//   РезультатЗапроса
//
Функция СведенияАнализаЗаПериод_2017Кв3(Параметры) Экспорт
	
	Запрос = ЗапросАнализСчета_5_11_01(Параметры);
		...
		Если ЗаполнятьОСВ_66 Тогда
			...
			Если СведенияОСВ.Количество() <> 0 Тогда
				Для Каждого СтрокаОСВ из СведенияОСВ Цикл
					...
					СтрокаМСЧ1.Данные.Вставить("П00007М100014", СтрокаОСВ.ДоговорСрокДействия);
					СтрокаМСЧ1.Данные.Вставить("П00007М100015", СтрокаОСВ.Продукт);
					СтрокаМСЧ1.Данные.Вставить("П00007М100016", СтрокаОСВ.Сумма);
					СтрокаМСЧ1.Данные.Вставить("П00007М100017", СтрокаОСВ.Валюта);
					СтрокаМСЧ1.Данные.Вставить("П00007М100018", СтрокаОСВ.Процент);
					...
					ГрафикПлатежей = ЗаполнениеОтчетностиВБанкиПереопределяемый.ГрафикПлатежейКредитовДепозитов(СтрокаОСВ, ДатаКонцаПериодаОтчета);
					Для каждого СтрокаИсточник Из ГрафикПлатежей Цикл
						
						СтрокаМСЧ2 = МСЧ2.Строки.Добавить();
						
						СтрокаМСЧ2.Данные = Новый Структура;
						СтрокаМСЧ2.Данные.Вставить("П00007М200001", СтрокаИсточник.ДатаПлатежа);
						СтрокаМСЧ2.Данные.Вставить("П00007М200002", СтрокаИсточник.СуммаПлатежа);
		...
		Если ЗаполнятьОСВ_67 Тогда
			...
			Если СведенияОСВ.Количество() <> 0 Тогда
				Для Каждого СтрокаОСВ из СведенияОСВ Цикл
					...
					СтрокаМСЧ1.Данные.Вставить("П00008М100014", СтрокаОСВ.ДоговорСрокДействия);
					СтрокаМСЧ1.Данные.Вставить("П00008М100015", СтрокаОСВ.Продукт);
					СтрокаМСЧ1.Данные.Вставить("П00008М100016", СтрокаОСВ.Сумма);
					СтрокаМСЧ1.Данные.Вставить("П00008М100017", СтрокаОСВ.Валюта);
					СтрокаМСЧ1.Данные.Вставить("П00008М100018", СтрокаОСВ.Процент);
					...
					Для каждого СтрокаИсточник Из ГрафикПлатежей Цикл
						...
						СтрокаМСЧ2.Данные = Новый Структура;
						СтрокаМСЧ2.Данные.Вставить("П00008М200001", СтрокаИсточник.ДатаПлатежа);
						СтрокаМСЧ2.Данные.Вставить("П00008М200002", СтрокаИсточник.СуммаПлатежа);
...
#Область ЗаполнениеОтчетностиВБанк
...
Процедура Заполнить2017Кв3_Баланс_ЗаПериод(ПараметрыЗаполнения, СтраницаРаздела)
	...
	РазместитьРезультатыАвтозаполнения(СтраницаРаздела, РассчитанныеПоказатели);
...
Процедура Заполнить2017Кв3_ЗадолженностьПоКредитамЗаймам_ЗаПериод(ПараметрыЗаполнения, СтраницаРаздела)
	...
	Если СведенияОСВ.Количество() <> 0 Тогда
		Для Каждого СтрокаОСВ из СведенияОСВ Цикл
			...
			СтрокаМСЧ1.Данные[ПрефиксМСЧ + "М100014"] = СтрокаОСВ.ДоговорСрокДействия;
			СтрокаМСЧ1.Данные[ПрефиксМСЧ + "М100015"] = СтрокаОСВ.Продукт;
			СтрокаМСЧ1.Данные[ПрефиксМСЧ + "М100016"] = СтрокаОСВ.Сумма;
			СтрокаМСЧ1.Данные[ПрефиксМСЧ + "М100017"] = СтрокаОСВ.Валюта;
			СтрокаМСЧ1.Данные[ПрефиксМСЧ + "М100018"] = СтрокаОСВ.Процент;
			...
			ГрафикПлатежей = ЗаполнениеОтчетностиВБанкиПереопределяемый.ГрафикПлатежейКредитовДепозитов(СтрокаОСВ, КонецПериода);
			Для каждого СтрокаИсточник Из ГрафикПлатежей Цикл
				
				СтрокаМСЧ2 = МСЧ2.Строки.Добавить();
				
				СтрокаМСЧ2.Данные = Новый Структура;
				СтрокаМСЧ2.Данные.Вставить(ПрефиксМСЧ + "М200001", СтрокаИсточник.ДатаПлатежа);
				СтрокаМСЧ2.Данные.Вставить(ПрефиксМСЧ + "М200004", СтрокаИсточник.СуммаПлатежа);
				СтрокаМСЧ2.Данные.Вставить(ПрефиксМСЧ + "М200005", СтрокаИсточник.ОсновнойДолг);
				СтрокаМСЧ2.Данные.Вставить(ПрефиксМСЧ + "М200002", СтрокаИсточник.СуммаПлатежаРегл);
				СтрокаМСЧ2.Данные.Вставить(ПрефиксМСЧ + "М200003", СтрокаИсточник.ОсновнойДолгРегл);
		...
		ЗаполнениеОтчетностиВБанкиПереопределяемый.ЗаполнитьРасшифровку_ЗадолженностьПоКредитамЗаймам(ТаблицаРасшифровки, ПараметрыЗаполнения, СведенияОСВ);
...
Процедура Заполнить2017Кв3_ОСВ_ЗаПериод(ЗаполняемаяСтруктура, ПараметрыЗаполнения)
	...
	СведенияОСВ = СведенияОСВЗаПериод_2017Кв3(ПараметрыЗаполнения);
	
	ИтогиОСВ = ИнициализироватьСуммовыеПоказателиСчета();
	
	Для Каждого СтрокаКорневогоСчета Из СведенияОСВ.Строки Цикл
		Секция_Строка = Макет.ПолучитьОбласть("ОСВ_Строка");
		
		ИтогиПоСчету = ИнициализироватьСуммовыеПоказателиСчета();
		...
		Если СтрокаКорневогоСчета.Забалансовый Тогда
		...
		Если ЗначениеЗаполнено(СчетОтчета)
		   И СчетОтмеченВСписке(СчетОтчета, ПараметрыОтчета[ИмяРаздела + "_Счета"]) Тогда
			Если ДействуетСВерсииФормата("5.11.01", ПараметрыОтчета.ВерсияФормата) Тогда
				ПериодыЗаполнения = ПараметрыОтчета[ИмяРаздела + "_ПериодыЗаполнения"];
				Для Каждого ПериодЗаполнения Из ПериодыЗаполнения Цикл
					ПараметрыЗаполнения = Новый Структура;
					ПараметрыЗаполнения.Вставить("Организация", ПараметрыОтчета.Организация);
					ПараметрыЗаполнения.Вставить("Счет", СчетОтчета);
					ПараметрыЗаполнения.Вставить("НачалоПериода", ПериодЗаполнения.НачалоПериода);
					ПараметрыЗаполнения.Вставить("КонецПериода", ПериодЗаполнения.КонецПериода);
					
					ПараметрыЗаполнения.Вставить("ПрефиксСегментов", ПериодЗаполнения.ПрефиксСегментов);
					ПараметрыЗаполнения.Вставить("РегламентированныйОтчет", ПараметрыОтчета.РегламентированныйОтчет);
					
					ПараметрыОтправкиСчета = ОтчетностьВБанкиСлужебный.ПараметрыОтправкиСчета(СчетОтчета);
...
Процедура Заполнить2017Кв3_ОСВПоСчету_ЗаПериод_5_11_01(ПараметрыЗаполнения)
	...
	Счет = ПараметрыЗаполнения.Счет;
	РегламентированныйОтчет = ПараметрыЗаполнения.РегламентированныйОтчет;
	ПрефиксСегментов = ПараметрыЗаполнения.ПрефиксСегментов;
	ОчиститьДанныеОтчетаВРегистреДополнительныхФайлов(РегламентированныйОтчет, ПрефиксСегментов, Счет);
	
	Запрос = ЗапросОСВПоСчету_5_11_01(ПараметрыЗаполнения);
	
	ИтогиОСВ = ИнициализироватьСуммовыеПоказателиСчета();
	
	ПрефиксВидаФайла = ПрефиксСегментов + "."
	                 + Счет.УникальныйИдентификатор() + ".";
	ВеличинаОтсечки = 3000;
	...
	ПараметрыЗаписи.Вставить("Данные", ИтогиОСВ);
	ПараметрыЗаписи.Вставить("ВидДополнительногоФайла", "ОСВПоСчетуИтоги" + "." + Счет.УникальныйИдентификатор() + ".0");
	ПараметрыЗаписи.Вставить("ИмяФайла", ПрефиксСегментов + Счет.Код);
	ПараметрыЗаписи.Вставить("КоличествоЗаписей", 1);
	ЗаписатьДанныеВРегистрДополнительныхФайлов(ПараметрыЗаписи);
...
Процедура Заполнить2017Кв3_АнализСчета_ЗаПериод(ПараметрыЗаполнения)
	...
	ИтогиАнализаСчета = ИнициализироватьСуммовыеПоказателиСчета();
...
Процедура Заполнить2017Кв3_АнализСчета_ЗаПериод_5_11_01(ПараметрыЗаполнения)
	...
	ИтогиПоОтчету = ИнициализироватьСуммовыеПоказателиСчета();
...
Функция Заполнить2017Кв3_Операции_НовыеСводныеДанные()
	...
	СводныеДанные.Колонки.Добавить("СВИФТБИК");
	СводныеДанные.Колонки.Добавить("КодСтраны");
...
Процедура Заполнить2017Кв3_Операции_СводныеСведенияПоСубсчету(СтрокиПоСубсчетам, ВыборкаПоСубсчетам, Запрос)
	...
	Иначе
		...
		Запрос.Текст =
			...
			|	БанковскиеСчета.СВИФТБИК КАК СВИФТБИК,
			|	БанковскиеСчета.КодСтраны КАК КодСтраны,
...
Процедура Заполнить2017Кв3_Операции_ДетальныеСведения(ПараметрыОтчета, СчетВерхнегоУровня)
	...
	ТаблицаОпераций.Колонки.Добавить("СчетКонтрагентаСВИФТБИК", Новый ОписаниеТипов("Строка"));
	ТаблицаОпераций.Колонки.Добавить("СчетКонтрагентаКодСтраны", Новый ОписаниеТипов("Строка"));
	
	ТаблицаОпераций.Колонки.Добавить("ПП_СтатусПлательщика_101", Новый ОписаниеТипов("Строка"));
	ТаблицаОпераций.Колонки.Добавить("ПП_КБК_104", Новый ОписаниеТипов("Строка"));
	ТаблицаОпераций.Колонки.Добавить("ПП_ОКАТО_105", Новый ОписаниеТипов("Строка"));
	ТаблицаОпераций.Колонки.Добавить("ПП_Основание_106", Новый ОписаниеТипов("Строка"));
	ТаблицаОпераций.Колонки.Добавить("ПП_Период_107", Новый ОписаниеТипов("Строка"));
	ТаблицаОпераций.Колонки.Добавить("ПП_Номер_108", Новый ОписаниеТипов("Строка"));
	ТаблицаОпераций.Колонки.Добавить("ПП_Дата_109", Новый ОписаниеТипов("Строка"));
	...
	Пока ВыборкаПоБанковскимСчетам.Следующий() Цикл
		...
		Пока ВыборкаПоОперациям.Следующий() Цикл
			...
			ИмяФайла = НСтр("ru = 'Сегмент %1 перечня проводок по счету %2';
							|en = 'Segment %1 of the list of postings by account %2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
			ИмяФайла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ИмяФайла, СтрокаЧГ0(НомерСегмента), БанковскийСчет);
...
Функция ЗапросОСВПоСчету(ПараметрыЗаполнения)
	...
	ПараметрыСчета = ОтчетностьВБанкиСлужебный.ПараметрыОтправкиСчета(Счет);
...
Функция ЗапросОСВПоСчету_5_11_01(ПараметрыЗаполнения)
	...
	РазвернутоеСальдо = ПараметрыЗаполнения.РазвернутоеСальдо;
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	ХозрасчетныйОстаткиИОбороты.СуммаКонечныйОстатокДт КАК СКД,
	               |	ХозрасчетныйОстаткиИОбороты.СуммаКонечныйОстатокКт КАК СКК,
	               |	ХозрасчетныйОстаткиИОбороты.КоличествоНачальныйОстатокДт КАК КНОД,
	               |	ХозрасчетныйОстаткиИОбороты.КоличествоНачальныйОстатокКт КАК КНОК,
	               |	ХозрасчетныйОстаткиИОбороты.КоличествоОборотДт КАК КДО,
	               |	ХозрасчетныйОстаткиИОбороты.КоличествоОборотКт КАК ККО,
	               |	ХозрасчетныйОстаткиИОбороты.КоличествоКонечныйОстатокДт КАК ККОД,
	               |	ХозрасчетныйОстаткиИОбороты.КоличествоКонечныйОстатокКт КАК ККОК,
	               |	NULL КАК Субконто1ИНН,
	               |	NULL КАК Субконто1КПП,
	               |	NULL КАК Субконто2ИНН,
	               |	NULL КАК Субконто2КПП
	...
	Если РазвернутоеСальдо Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Остаток", "РазвернутыйОстаток");
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыПоСубсчетам(ОтправляемыйСчет, ПараметрыСчета) Экспорт
...
Функция СводныеСведенияПоБанковскимОперациям2017Кв3(Параметры)
	...
	СтрокаСчетаВерхнегоУровня.Представление = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетБанковскихОпераций, "Код");
...
Функция ДетальныеСведенияПоБанковскимОперациям2017Кв3(Параметры)
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ВТ_ДанныеПП.СтатусПлательщика_101, """") КАК ПП_СтатусПлательщика_101,
		|	ЕСТЬNULL(ВТ_ДанныеПП.КБК_104, """") КАК ПП_КБК_104,
		|	ЕСТЬNULL(ВТ_ДанныеПП.ОКАТО_105, """") КАК ПП_ОКАТО_105,
		|	ЕСТЬNULL(ВТ_ДанныеПП.Основание_106, """") КАК ПП_Основание_106,
		|	ЕСТЬNULL(ВТ_ДанныеПП.Период_107, """") КАК ПП_Период_107,
		|	ЕСТЬNULL(ВТ_ДанныеПП.Номер_108, """") КАК ПП_Номер_108,
		|	ЕСТЬNULL(ВТ_ДанныеПП.Дата_109, """") КАК ПП_Дата_109,
		|	ЕСТЬNULL(ВТ_БанковскиеСчета.СВИФТБИК, """") КАК СчетКонтрагентаСВИФТБИК,
		|	ЕСТЬNULL(ВТ_БанковскиеСчета.КодСтраны, """") КАК СчетКонтрагентаКодСтраны
		|ИЗ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДанныеПППереопределяемый КАК ВТ_ДанныеПП
		|		ПО ВТ_ОперацииПоСчету.Документ = ВТ_ДанныеПП.Списание
		|
		|УПОРЯДОЧИТЬ ПО
		|	ДатаОперации,
		|	Документ,
...
Функция ОСВ_ПоКредитамЗаймам(ПараметрыОСВ)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	ЕСТЬNULL(ДоговорыКредитовДепозитов.Продукт, """") КАК Продукт,
	               |	ЕСТЬNULL(ДоговорыКредитовДепозитов.СуммаТраншей, 0) КАК Сумма,
	               |	ЕСТЬNULL(ДоговорыКредитовДепозитов.Процент, 0) КАК Процент,
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДанныеДоговоровПереопределяемый КАК ВТ_ДанныеДоговоров
	               |		ПО ХозрасчетныйОстаткиИОбороты.Субконто2 = ВТ_ДанныеДоговоров.Ссылка
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДоговорыКредитовДепозитовПереопределяемый КАК ДоговорыКредитовДепозитов
	               |		ПО ХозрасчетныйОстаткиИОбороты.Субконто2 = ДоговорыКредитовДепозитов.Договор";
...
Функция ИнициализироватьСуммовыеПоказателиСчета()
	
	СтруктураПоказателей = Новый Структура;
	СтруктураПоказателей.Вставить("СНД", 0);
	СтруктураПоказателей.Вставить("СНК", 0);
	СтруктураПоказателей.Вставить("ДО", 0);
	СтруктураПоказателей.Вставить("КО", 0);
	СтруктураПоказателей.Вставить("СКД", 0);
	СтруктураПоказателей.Вставить("СКК", 0);
