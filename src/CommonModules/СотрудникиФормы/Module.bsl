...
//		* ДанныеФизическихЛиц - Массив из Структура - данные найденных физических лиц:
...
//
Функция НовыйРезультатПоискаПохожихФизическихЛиц() Экспорт
	
	РезультатПоиска = Новый Структура;
	РезультатПоиска.Вставить("Фамилия", "");
	РезультатПоиска.Вставить("Имя", "");
	РезультатПоиска.Вставить("Отчество", "");
	РезультатПоиска.Вставить("ФизическоеЛицоУникально", Истина);
	РезультатПоиска.Вставить("ДанныеФизическихЛицДоступны", Ложь);
	РезультатПоиска.Вставить("ДанныеФизическихЛиц", Новый Массив);
	РезультатПоиска.Вставить("ВозможнаПроверкаПоИНН", Ложь);
	РезультатПоиска.Вставить("ВозможнаПроверкаПоСНИЛС", Ложь);
	РезультатПоиска.Вставить("ДоступнаРольСохранениеДанныхЗадвоенныхФизическихЛиц", Истина);
	
	Возврат РезультатПоиска;
	
КонецФункции

// Подобрать список физ лиц.
// 
// Параметры:
//  ПараметрыПоиска - см. РезультатПоискаФизическихЛиц.ПараметрыПоиска
// 
// Возвращаемое значение:
//  см. РезультатПоискаФизическихЛиц
Функция ПохожиеФизическиеЛица(ПараметрыПоиска) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПоискФизическихЛиц") Тогда
		...
		ПараметрыПоиска = МодульПоискФизическихЛиц.ПараметрыПоискаПохожихЛюдей(ПараметрыПоиска);
		ПараметрыПоиска.ИскатьСредиКандидатов = Ложь;
		РезультатПоиска = МодульПоискФизическихЛиц.ПохожиеЛюди(ПараметрыПоиска);
		Возврат РезультатПоиска.РезультатПоискаФизическихЛиц;
	КонецЕсли;
	
	Возврат РезультатПоискаФизическихЛиц(ПараметрыПоиска);
...
//		
// Возвращаемое значение:
//  см. НовыйРезультатПоискаПохожихФизическихЛиц
Функция РезультатПоискаФизическихЛиц(ПараметрыПоиска) Экспорт
	...
	Результат = НовыйРезультатПоискаПохожихФизическихЛиц();
	ЗаполнитьЗначенияСвойств(Результат, ПараметрыПоиска, "Фамилия, Имя, Отчество");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.КадровыйУчетРасширенная") Тогда
		...
		МодульКадровыйУчетФормыРасширенный.ПриПодбореСпискаФизЛиц(Результат);
	КонецЕсли;
	
	Если ПараметрыПоиска.ОбязательноЗаполненноеФИО Тогда
		Наименование = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ФИОИзЧастей(ПараметрыПоиска);
	...
	ТаблицаПохожих = ТаблицаПохожихФизическихЛиц(ПараметрыПоиска); 
	...
	Результат.ФизическоеЛицоУникально = Ложь;
	...
	Результат.ДанныеФизическихЛицДоступны = МассивДоступных.Количество() = МассивНайденных.Количество();
	...
	Если ПараметрыПоиска.ИскатьВАрхиве
		И ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.АрхивФизическихЛиц") Тогда
		МодульАрхивФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("АрхивФизическихЛиц");
		МодульАрхивФизическихЛиц.ДополнитьДанныеФизическихЛиц(ДанныеФизическихЛиц, МассивНайденных, Результат);
	КонецЕсли;
	...
	Результат.ДанныеФизическихЛиц = ОбщегоНазначения.ТаблицаЗначенийВМассив(ДанныеФизическихЛиц);
	
	Если Результат.ДанныеФизическихЛицДоступны Тогда
		Возврат Результат;
	КонецЕсли;
...
Процедура СотрудникиПередЗаписьюНаСервере(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	Если Форма.Параметры.Ключ.Пустая() Тогда
		...
		Если Форма.ФизическоеЛицоЗаблокировано Или Форма.ФизическоеЛицо.Ссылка.Пустая() Тогда
			Если Форма.ФизическоеЛицоСсылка.Пустая() Тогда
				Форма.ФизическоеЛицоСсылка = Справочники.ФизическиеЛица.ПолучитьСсылку();
			КонецЕсли;
			Если ТекущийОбъект.ФизическоеЛицо.Пустая() Тогда
				Форма.Сотрудник.ФизическоеЛицо = Форма.ФизическоеЛицоСсылка;
				ТекущийОбъект.ФизическоеЛицо = Форма.ФизическоеЛицоСсылка;
			КонецЕсли;
			ТекущийОбъект.ДополнительныеСвойства.Вставить("ГруппаДоступа", Форма.ФизическоеЛицо.ГруппаДоступа);
		КонецЕсли;
	...
	Если Не РедактированиеПериодическихСведенийКлиентСервер.ЗаполненыЗначенияПоУмолчаниюПоСтруктуре(Форма, "ДокументыФизическихЛиц", СтруктураВедущихОбъектов) Тогда
		...
		Если ЗначениеЗаполнено(Форма.ДокументыФизическихЛиц.ДатаВыдачи) Тогда
			...
			Если ЗначениеЗаполнено(Форма.ДокументыФизическихЛиц.СрокДействия) Тогда
				...
				ЗарплатаКадры.ПроверитьКорректностьДаты(Форма.ФизическоеЛицоСсылка, Форма.ДокументыФизическихЛиц.СрокДействия, "ДокументыФизическихЛиц.СрокДействия",
					Отказ, НСтр("ru = 'Срок действия документа';
								|en = 'Document expiration date'"), Форма.ДокументыФизическихЛиц.ДатаВыдачи, НСтр("ru = 'даты его выдачи';
																												|en = 'dates of its issue'"));
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru = 'Документ: не заполнено поле ""Дата выдачи"".';
									|en = 'Document: the ""Date of issue"" field is not filled in.'");
...
Процедура ЗапомнитьФамилиюИмяОтчествоФизическогоЛицаОбъектаФормы(Форма, ФизическоеЛицоОбъект) Экспорт
	...
	ЗаполнитьЗначенияСвойств(ФизическоеЛицоОбъект, Форма.ФИОФизическихЛиц, "Фамилия,Имя,Отчество,Инициалы");
...
Процедура ЗаписатьФизическоеЛицоСотрудника(Форма) Экспорт
	...
	Если Форма.ФизическоеЛицоЗаблокировано ИЛИ Форма.ФизическоеЛицо.Ссылка.Пустая() Тогда
		...
		НовоеФизическоеЛицо = Форма.ФизическоеЛицо.Ссылка.Пустая();
		...
		Если НовоеФизическоеЛицо И НЕ ПравоНаДобавлениеИзменениеФизическихЛиц Тогда
			ВызватьИсключение НСТР("ru = 'Нет прав на запись нового физического лица. Сотрудник не записан.';
									|en = 'Insufficient rights to save a new individual. The employee was not saved.'");
		КонецЕсли;
	
		ФизическоеЛицоОбъектФормы = Форма.РеквизитФормыВЗначение("ФизическоеЛицо"); 
		Если НовоеФизическоеЛицо Тогда
			ФизическоеЛицоОбъект = Форма.ФизическоеЛицоСсылка.ПолучитьОбъект();
			ЗаполнитьЗначенияСвойств(ФизическоеЛицоОбъект, ФизическоеЛицоОбъектФормы, , "Ссылка,Код,Владелец,ВерсияДанных");
			Для Каждого СтрокаКонтактнаяИнформация Из ФизическоеЛицоОбъектФормы.КонтактнаяИнформация Цикл
				ЗаполнитьЗначенияСвойств(ФизическоеЛицоОбъект.КонтактнаяИнформация.Добавить(), СтрокаКонтактнаяИнформация);
			КонецЦикла;
			Для Каждого СтрокаДополнительныеРеквизиты Из ФизическоеЛицоОбъектФормы.ДополнительныеРеквизиты Цикл
				ЗаполнитьЗначенияСвойств(ФизическоеЛицоОбъект.ДополнительныеРеквизиты.Добавить(), СтрокаДополнительныеРеквизиты);
			КонецЦикла;
		Иначе
			ФизическоеЛицоОбъект = ФизическоеЛицоОбъектФормы;
		КонецЕсли;
...
Функция ПодобратьСписокУточненийТекущегоФизическогоЛица(ФизическоеЛицоСсылка, Знач Фамилия, Знач Имя, Знач Отчество,
	...
	ЧастиФИО.Фамилия		= СокрЛП(Фамилия);
	ЧастиФИО.Имя			= СокрЛП(Имя);
	ЧастиФИО.Отчество	= СокрЛП(Отчество);
	
	Наименование = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ФИОИзЧастей(ЧастиФИО);
	...
	ПохожиеФизическиеЛица = ТаблицаПохожихФизическихЛиц(ПараметрыПоиска);
...
Функция ТаблицаПохожихФизическихЛиц(ПараметрыПоиска)
	
	Запрос = Новый Запрос;
	
	ФИО = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ФИОИзЧастей(ПараметрыПоиска);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПоискФизическихЛиц") Тогда
		МодульПоискФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("ПоискФизическихЛиц");
		МодульПоискФизическихЛиц.УдалитьСтрокиСНеполнымСовпадением(ТаблицаПохожих, ПараметрыПоиска);
	КонецЕсли;
...
Функция ТекстСообщенияОКонфликтеВидаЗанятости(ВидЗанятости, Организация, КонфликтныйСотрудникОрганизация = Неопределено, КонфликтныйСотрудникВидЗанятости = Неопределено, КонфликтныйСотрудникТабельныйНомер = "", КонфликтныйСотрудникДолжность = Неопределено) Экспорт
	...
	Если ВидЗанятости = Перечисления.ВидыЗанятости.ОсновноеМестоРаботы Тогда
		ТекстСообщения =  НСтр("ru = 'Нельзя оформить сотрудника по основному месту работы, так как он уже работает в %1 %2, Таб. номер: %3, %4.';
								|en = 'Primary employment is not allowed for the employee because he or she is already employed in %1 %2, Tab. number: %3 %4.'");
	ИначеЕсли ВидЗанятости = Перечисления.ВидыЗанятости.Совместительство Тогда
		ТекстСообщения =  НСтр("ru = 'Нельзя оформить сотрудника на внешнее совместительство, так как он уже работает в %1 %2, Таб. номер: %3, %4.';
								|en = 'External secondary employment is not allowed for the employee because he or she is already employed in %1 %2, Tab. number: %3, %4.'");
	ИначеЕсли ВидЗанятости = Перечисления.ВидыЗанятости.ВнутреннееСовместительство И ЗначениеЗаполнено(КонфликтныйСотрудникВидЗанятости) Тогда
		ТекстСообщения = НСтр("ru = 'Нельзя оформить сотрудника на внутреннее совместительство, так как он оформлен по основному виду занятости в %1.';
								|en = 'Internal secondary employment is not allowed for the employee as they have already employed in %1.'");
	ИначеЕсли ВидЗанятости = Перечисления.ВидыЗанятости.ВнутреннееСовместительство Тогда
		ТекстСообщения = НСтр("ru = 'Нельзя оформить сотрудника на внутреннее совместительство, так как он не работает в %1.';
								|en = 'Internal secondary employment is not allowed for the employee because he or she is not employed in %1.'");
	КонецЕсли;
	...
	Если КонфликтныйСотрудникВидЗанятости = Перечисления.ВидыЗанятости.ОсновноеМестоРаботы Тогда
		ПредставлениеВидаЗанятости = НСтр("ru = 'по основному месту работы';
											|en = 'at primary place of employment'");
	Иначе
		ПредставлениеВидаЗанятости = НСтр("ru = 'как внешний совместитель';
											|en = 'as an external secondary job employee'");
	КонецЕсли;
...
Функция ПолучитьСведенияОВычетахНДФЛ(ФизическоеЛицо) Экспорт
	...
	Если Пользователи.РолиДоступны("ЧтениеНалоговИВзносов,ДобавлениеИзменениеНалоговИВзносов") Тогда
		...
		Запрос.Текст =
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	МАКСИМУМ(ЕжегодныеСтандартныеВычеты.ПрименятьВычетыС) КАК Период,
			|	ЕжегодныеСтандартныеВычеты.Сотрудник КАК ФизическоеЛицо,
			|	ЕжегодныеСтандартныеВычеты.Организация.ГоловнаяОрганизация КАК ГоловнаяОрганизация
			|ПОМЕСТИТЬ ВТПоследниеПериодыЕжегодныхВычетов
			|ИЗ
			|	Документ.ЕжегодныеСтандартныеВычеты КАК ЕжегодныеСтандартныеВычеты
			|ГДЕ
			|	ЕжегодныеСтандартныеВычеты.Сотрудник = &ФизическоеЛицо
			|	И ЕжегодныеСтандартныеВычеты.Проведен
			|
			|СГРУППИРОВАТЬ ПО
			|	ЕжегодныеСтандартныеВычеты.Сотрудник,
			|	ЕжегодныеСтандартныеВычеты.Организация.ГоловнаяОрганизация";
		...
		Запрос.Текст =
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	МАКСИМУМ(ЕжегодныеСтандартныеВычеты.Ссылка) КАК Регистратор
			|ИЗ
			|	ВТПоследниеПериодыЕжегодныхВычетов КАК ПоследниеПериодыЕжегодныхВычетов
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЕжегодныеСтандартныеВычеты КАК ЕжегодныеСтандартныеВычеты
			|		ПО ПоследниеПериодыЕжегодныхВычетов.ГоловнаяОрганизация = ЕжегодныеСтандартныеВычеты.Организация.ГоловнаяОрганизация
			|			И ПоследниеПериодыЕжегодныхВычетов.ФизическоеЛицо = ЕжегодныеСтандартныеВычеты.Сотрудник
			|			И ПоследниеПериодыЕжегодныхВычетов.Период = ЕжегодныеСтандартныеВычеты.ПрименятьВычетыС
			|
			|СГРУППИРОВАТЬ ПО
			|	ПоследниеПериодыЕжегодныхВычетов.ФизическоеЛицо,
			|	ПоследниеПериодыЕжегодныхВычетов.ГоловнаяОрганизация";
		
		МассивРезультатов = Запрос.ВыполнитьПакет();
		Для ИндексРезультата = 0 По 4 Цикл
			...
			Если ИндексРезультата > 0 Тогда
				...
				ИначеЕсли ИндексРезультата = 4 Тогда
					
					ИменаРеквизитовРегистраторов = ИменаРеквизитовРегистраторов + ",ПрименятьВычетыС,НалоговыйПериод";
	...
	Если Форма.ИмеютсяАктуальныеСотрудники  Тогда
		...
		ИнфоНадписьМестаРаботы = СтрШаблон(
			НСтр("ru = '%1 является нашим сотрудником. Ниже приведены места работы. Для перехода к полной информации о месте работы используйте соответствующую ссылку. Для заполнения информации о новом месте работы сотрудника используйте ссылку ""Новое место работы""';
				|en = '%1 is our employee. Places of employment are specified below. To see full information about a place of employment, click the corresponding reference. To fill in information about a new place of employment, click ""New place of employment""'"),
	...
	ИначеЕсли Форма.ИмеютсяАктуальныеСотрудникиНеОтображаемые Тогда
		...
		ИнфоНадписьМестаРаботы = СтрШаблон(
			НСтр("ru = '%1 является нашим сотрудником. Для заполнения информации о месте работы, должности и окладе сотрудника используйте ссылку ""Новое место работы""';
				|en = '%1 is our employee. To fill in the information about place of employment, position, and base salary, click ""New place of employment""  '"),
			Форма.ФизическоеЛицоСсылка);
	...
	Иначе
		...
		ИнфоНадписьМестаРаботы = СтрШаблон(
			НСтр("ru = '%1 не является нашим сотрудником. Для заполнения информации о месте работы, должности и окладе сотрудника используйте ссылку ""Новое место работы""';
				|en = '%1 is not our employee. To fill in the information about place of employment, position, and base salary, click ""New place of employment""     '"),
			Форма.ФизическоеЛицоСсылка);
...
//   * ОбъектВзаимоотношений - СправочникСсылка - объект, для которого определяется история взаимоотношений.
//     Например, физическое лицо или личные данные кандидата.
//   * Ссылка - СправочникСсылка - сотрудник или кандидат объекта взаимоотношений.
//   * ФизическоеЛицо - СправочникСсылка.ФизическиеЛица -
//   * Примечание - Строка -
//   * Роль - Число -
//   * ГруппаРоли - Число -
//   * ДатаАктуальности - Дата -
Функция ТаблицаИсторииВзаимоотношений() Экспорт

	Таблица = Новый ТаблицаЗначений;
	
	Таблица.Колонки.Добавить("ОбъектВзаимоотношений");
