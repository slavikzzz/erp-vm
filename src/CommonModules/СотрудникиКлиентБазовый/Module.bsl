	...
	ИначеЕсли ИмяСобытия = "ОтредактированаИстория" И (Форма.ФизическоеЛицоСсылка = Источник ИЛИ Форма.СотрудникСсылка = Источник) Тогда
		Если Параметр.ИмяРегистра = "ФИОФизическихЛиц" Тогда
			Если Форма[Параметр.ИмяРегистра + "НаборЗаписейПрочитан"] Тогда
				...
				НаименованиеПоМенеджеруЗаписи = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ФИОИзЧастей(Форма.ФИОФизическихЛиц);
	...
	Если Форма.ДоступенПросмотрДанныхФизическихЛиц И НоваяЗапись = Ложь Тогда
		...
		ТекстВопроса = СтрШаблон(
				НСтр("ru = 'При редактировании изменено полное имя %1. 
				            ...
				            |en = 'When editing, the %1 full name was changed.
				            |If only previous data is corrected (it was incorrect), click ""Error is corrected"".
				            |If the employee''s last name or name changed, click ""Employee changed name""'"), 
				ПредставлениеСущности);
		ТекстКнопкиДа = НСтр("ru = 'Сотрудник изменил имя';
							|en = 'The employee changed name'");
