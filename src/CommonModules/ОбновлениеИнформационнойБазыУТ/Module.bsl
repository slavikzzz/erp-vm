			...
			Если ДополнительныеПараметры.ЗаписыватьВОднойТранзакции Тогда
				...
				Исключение
					...
					ТекстСообщения = НСтр("ru = 'Не удалось зафиксировать транзакцию записи движения в регистр %ИмяРегистра% по причине: %Причина%';
											|en = 'Cannot commit a transaction for saving record to register %ИмяРегистра%. Reason:%Причина%'");
				...
				Если Элемент.Ключ <> ДругойЭлемент.Ключ Тогда
					...
					Если СтрНайти(ДругойЭлемент.Значение, Элемент.Ключ) <> 0 Тогда
						ВызватьИсключение НСтр("ru = 'Адаптированный текст запроса. Вложенные временные таблицы не поддерживаются.';
												|en = 'Adapted query text. Nested temporary tables are not supported.'");
					КонецЕсли;
...
Функция РегистраторыДляПерепроведения(РезультатАдаптацииЗапроса,
	...
	Иначе
		ТекстИсключения = НСтр("ru = 'Функция пока не поддерживает работу с регистрами типа %ТипРегистра%.';
								|en = 'Function does not support operations with registers of type %ТипРегистра%.'");
...
Процедура СообщитьЧтоРаботаСФормойВременноОграничена() Экспорт
	...
	ТекстСообщения = НСтр("ru = 'Работа с формой временно ограничена, так как выполняется переход на новую версию программы. Это плановый процесс, он не мешает работе в информационной базе и скоро завершится. Если у вас возникли сложности - обратитесь к администратору или в службу поддержки.';
							|en = 'Operations with the form are temporarily restricted as migration to a new application version is not completed. This is a planned process. It does not interfere with the work in the infobase and will soon be over. If you have any difficulties, contact administrator or technical support.'");
...
Функция СформироватьИЗаписатьДвижения(Ссылка, ПолноеИмяРегистра, МенеджерРегистра, ДополнительныеПараметры, ИмяПоляРегистратор = "Регистратор", РеквизитыПроведен = Неопределено) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось перезаписать движения в регистр ""%ИмяРегистра%"" по документу %Ссылка% по причине: %Причина%';
								|en = 'Cannot overwrite records to register ""%ИмяРегистра%"" using document %Ссылка%. Reason: %Причина%'");
		...
		Если НаборЗаписей <> Неопределено Тогда
			ОбновлениеИнформационнойБазы.ЗарегистрироватьПроблемуСДанными(Ссылка,
				СтрШаблон(НСтр("ru = 'Не удалось перезаписать движения в регистр ""%1"" по документу %2';
								|en = 'Cannot overwrite records to the ""%1"" register by the %2 document'"),
			...
			Иначе
				...
				Иначе
					ТекстИсключения = НСтр("ru = 'В запросе выборки данных документа %ПолноеИмяДокумента% для формирования движений по регистру %ПолноеИмяРегистра% присутствует поле, которого нет в регистре.';
											|en = 'Request for selecting data of the %ПолноеИмяДокумента% document to generate records of the %ПолноеИмяРегистра% register has a field missing in the register.'");
...
Процедура УстановитьСлужебныеКонстанты() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Не удалось определить служебный идентификатор конфигурации.';
								|en = 'Cannot determine configuration service ID.'");
	КонецЕсли;
...
#Область СоздатьЭлементыСтавкиНДСДляМежценовойРазницы
	...
	Обработчик.Комментарий = НСтр("ru = 'Создает новые элементы справочника ""Ставки НДС"" для типа налогообложения НДС ""Облагается НДС с межценовой разницы"".
		...
		|en = 'Creates new ""VAT rates"" catalog items for the ""Subject to VAT from price difference"" VAT taxation type.
		|Adds the ""Space activity"" VAT taxation type to the 0% rate.
		|Adds the ""Subject to VAT in EAEU country (not the Russian Federation)"" VAT taxation type to the ""Excluding VAT"" rate.
		|Deletes duplicates for the ""Excluding VAT"" rate in the ""VAT taxation type"" table.'");
...
#Область СоздатьЭлементыСтавкиНДСДляЛьготногоНалогообложенияНДСнаУСН
	...
	Обработчик.Комментарий = НСтр("ru = 'Создает новые элементы справочника ""Ставки НДС"" для типа налогообложения НДС ""Льготное налогообложение НДС на УСН"".
		...
		|Удаляет дубли в табличной части ""Типы налогообложения НДС"" для ставки ""Без НДС"".';
		|en = 'Creates new items of the ""VAT rates"" catalog for the ""Preferential VAT taxation on STS"" VAT taxation type.
		|Adds the ""Preferential VAT taxation on STS"" taxation type to the ""Excluding VAT"" rate.
		|Removes duplicates in the ""VAT taxation types"" table for the ""Excluding VAT"" rate.'");
...
#Область УстановитьКонстантуВариантУпорядочиванияПотребностейПоЗаказамДляОбеспечения
	...
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает значение константы ""Вариант упорядочивания потребностей по заказам для обеспечения"".';
									|en = 'Sets the value of the ""Option of ordering demands by orders for supply"" constant.'");
...
#Область УстановитьКонстантуИспользоватьОтветственноеХранениеСобственнымиОрганизациями
	...
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает значение константы ""Использовать ответственное хранение собственными организациями"".';
									|en = 'Sets the value of the ""Use VMI by own companies"" constant.'");
...
#Область УстановитьКонстантуИспользоватьУправлениеПеремещениемОбособленныхТоваров
	...
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает значение константы ""Использовать управление перемещением обособленных товаров"".';
									|en = 'Sets the ""Use management of assigned goods transfer"" constant value.'");
	...
	Читаемые.Добавить(Метаданные.Константы.ИспользоватьПроизводствоИзДавальческогоСырья.ПолноеИмя());
...
#Область ПеренестиСведенияВНаборДополнительныеОтчетыИОбработки
	...
	Обработчик.Комментарий = НСтр("ru = 'Переносит дополнительные сведения в новый набор доп. реквизитов и сведений справочника ""Дополнительные отчеты и обработки"".';
									|en = 'Transfers additional information records to a new set of additional attributes and information records of the ""Additional reports and data processors"" catalog.'");
...
#Область ВключитьХранениеИсторииЮрАдресОрганизации
	...
	Обработчик.Комментарий = НСтр("ru = 'Включает хранение истории изменения юридического адреса организации"".';
									|en = 'Includes storage of the change history of the company''s legal address"".'");
...
#Область ОбновлениеИнформационнойБазыУТ_ДополнитьПользовательскиеПрофилиРольюЧтениеЛогистическихПараметров
	...
	Обработчик.Комментарий = НСтр("ru = 'Дополняет пользовательские профили групп доступа новой ролью ""Чтение логистических параметров"" .';
									|en = 'Adds the new ""Read logistic parameters"" role to user access group profiles.'");
...
#Область ВключитьИспользованиеИнтеграцииС1САналитикой
	...
	Обработчик.Комментарий = НСтр("ru = 'Включает использование интеграции с 1С:Аналитикой, если подлючение было настроено ранее';
									|en = 'Enables the use of integration with 1C:Analytics if connection was set up earlier'");
	...
	Пока ОбъектыДляОбработки.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось обработать %ИмяОбъекта%: %Ссылка% по причине: %Причина%';
									|en = 'Cannot process %ИмяОбъекта%: %Ссылка%. Reason: %Причина%'");
...
Процедура СообщитьОНеудачнойБлокировке(Ссылка) Экспорт
	ТекстСообщения = НСтр("ru = 'Не удалось заблокировать объект: %Объект% по причине: %Причина%';
							|en = 'Cannot lock the object: %Объект%. Reason: %Причина%'");
...
Процедура СоздатьЭлементыСтавкиНДСДляМежценовойРазницы(Параметры) Экспорт 
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось создать ставки НДС для межценовой разницы: %Причина%';
								|en = 'Cannot create VAT rates for price difference: %Причина%'",
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось создать ставки НДС для космической деятельности: %Причина%';
								|en = 'Cannot create VAT rates for space activity: %Причина%'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось обработать данные табличной части ""Типы налогообложения НДС"" ставки ""Без НДС"": %Причина%';
								|en = 'Cannot process data in the ""VAT taxation types"" table of the ""Excluding VAT"" rate: %Причина%'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура СоздатьЭлементыСтавкиНДСДляЛьготногоНалогообложенияНДСнаУСН(Параметры) Экспорт 
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось создать льготные ставки НДС при УСН: %Причина%';
								|en = 'Cannot create preferential VAT rates under STS: %Причина%'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось обработать данные табличной части ""Типы налогообложения НДС"": %Причина%';
								|en = 'Cannot process the data of the ""VAT taxation types"" table: %Причина%'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если Не Результат[0].Пустой()
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось включить использование актов о расхождениях после отгрузки или приемки: %Причина%';
									|en = 'Cannot enable non-conformance certificates following shipment or acceptance: %Причина%'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
