...
// Проверяет значение КПП на соответствие требованиям, с кодом причины состоящим только из цифр.
// 
// Параметры:
//  КПП - Строка
// 
...
Функция ПредставлениеДокументаРеглОтч(Док) Экспорт
	...
	ИначеЕсли СтрНачинаетсяС(Представление, "Реестр обученных по охране труда лиц") Тогда
		Представление = "Реестр обученных по охране труда лиц от "
		+ ПредставлениеПериода(НачалоДня(Док.ДатаОкончания), КонецДня(Док.ДатаОкончания), "ФП=Истина");
		
	ИначеЕсли СтрНачинаетсяС(Представление, "Декларация по налогу на сверхприбыль") Тогда
		Представление = "Декларация по налогу на сверхприбыль в "
		+ ПредставлениеПериода(НачалоДня(Док.ДатаНачала), КонецДня(Док.ДатаОкончания), "ФП=Истина");
		...
		Если СтруктураРеквизитовФормы.мСчетчикСтраниц <> Истина Тогда
			...
			Для Каждого Эл Из СтруктураРеквизитовФормы.мПечатныеФормы Цикл
				Если НЕ УдалосьПроставитьНомерСтраницы(Эл, НомерЛиста) Тогда
					...
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось проставить номер страницы для %1';
																									|en = 'Cannot specify the page number for %1'"), Эл.Представление);
...
Процедура СообщитьОбОшибке(Знач ТекстСообщения, Отказ, Заголовок = "", Знач Статус = Неопределено, ВызыватьИсключение = Истина) Экспорт
	...
	#Если Сервер Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Регламентированная отчетность';
										|en = 'Local reporting'"), УровеньЖурналаРегистрации.Ошибка, , , Заголовок + Символы.ПС + ТекстСообщения);
...
Процедура ЗаполнитьСведенияОбОрганизацииДляОтчетаСтатистики(Форма, ВПрограммеИзмененаДатаПодписи = Ложь, ИмяПоля = "ПолеТабличногоДокументаФормаОтчета", СхемаНаимПолноеКраткое = Ложь) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ОбособленноеПодразделение") Тогда
		ОбособленноеПодразделение = Форма.ОбособленноеПодразделение;
	Иначе
		ОбособленноеПодразделение = Неопределено;
	КонецЕсли;
	
	Если Форма[ИмяПоля].Области.Найти("РегистрацияРеспондента") <> Неопределено Тогда
		
		РегСведения = Новый Структура("ОКПО, АдрПочт_JSON, ОбособленноеПодразделение",
			ОКПО, АдрПочт_JSON, ОбособленноеПодразделение);
		...
		ОКПО = РегСведения.ОКПО;
		АдрПочт_JSON = РегСведения.АдрПочт_JSON;
		ОбособленноеПодразделение = РегСведения.ОбособленноеПодразделение;
		
		Форма.РегистрацияРеспондента = РегистрацияРеспондента;
		Форма[ИмяПоля].Области.РегистрацияРеспондента.Значение = РегистрацияРеспондента;
...
Процедура ЗаполнитьСведенияОбОрганизацииДляОтчетаСтатистикиЗП(Форма, ВПрограммеИзмененаДатаПодписи = Ложь) Экспорт
	...
	Если Форма.ПолеТабличногоДокументаФормаОтчета.Области.Найти("РегистрацияРеспондента") <> Неопределено Тогда
		...
		Форма.РегистрацияРеспондента = РегистрацияРеспондента;
		Форма.ПолеТабличногоДокументаФормаОтчета.Области.РегистрацияРеспондента.Значение = РегистрацияРеспондента;
	...
	Если (НЕ ЗначениеЗаполнено(Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргНазв.Значение)) ИЛИ (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) Тогда
		Если ЗначениеЗаполнено(ОбособленноеПодразделение) Тогда
			...
			Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргНазв.Значение = СокрЛП(ОбособленноеПодразделение) + ?(ПустаяСтрока(НаимЮЛПол), "", " (" + НаимЮЛПол + ")");
		Иначе
			// Устанавливаем в "ОргНазв" значение по схеме:
			...
			Если Константы.ИспользоватьСервисФормированияМЧБсPDF417.Получить() Тогда
				...
				Если НЕ КомандаПоказатьСДвухмернымШтрихкодомPDF417 = Неопределено Тогда
					...
					= НСтр("ru = 'При сдаче отчета в бумажном виде, сохраните его на компьютер в виде PDF файла для последующей печати. ';
							|en = 'When submitting the report in a paper form, save it to your computer as a PDF file so that you can print it later.'")
					+ НСтр("ru = 'Откройте этот файл и распечатайте его в 2-х экземплярах. На каждом экземпляре ';
							|en = 'Open this file and print it in two copies. On each copy'")
					+ НСтр("ru = 'укажите дату, подпишите отчет и отнесите в свою налоговую инспекцию';
							|en = 'specify the date, sign the report and take it to the tax inspection'");
...
Функция СравнитьКраткиеВерсииОтчетов(Версия1, Версия2) Экспорт
	...
	ЧислоСоставляющихВерсии2 = ЧастиВерсии2.Количество();
	...
	// удаляем лишние составляющие, уравнивая число составляющих первой и второй версий
	...
	ИначеЕсли ЧислоСоставляющихВерсии2 > ЧислоСоставляющихВерсии1 Тогда
		Для Инд = ЧислоСоставляющихВерсии1 + 1 По ЧислоСоставляющихВерсии2 Цикл
			ЧастиВерсии2.Удалить(0);
		КонецЦикла;
	КонецЕсли;
	
	// сравниваем по каждому составляющему
	Для Инд = 0 По Мин(ЧислоСоставляющихВерсии1, ЧислоСоставляющихВерсии2) - 1 Цикл
		ЧастьВерсии1 = ЧастиВерсии1[Инд];
		ЧастьВерсии2 = ЧастиВерсии2[Инд];
...
Функция ОперацииСРегламентированнымОтчетом(ИмяОтчета, ИмяФормы) Экспорт
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетПрибыль.ФормаОтчета2022Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетПрибыль.ФормаОтчета2025Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетНДС.ФормаОтчета2023Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетНДС.ФормаОтчета2025Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетУСН.ФормаОтчета2023Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетУСН.ФормаОтчета2024Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетРасчетПоСтраховымВзносам.ФормаОтчета2023Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетРасчетПоСтраховымВзносам.ФормаОтчета2023Кв3"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетРасчетПоСтраховымВзносам.ФормаОтчета2025Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет6_НДФЛ.ФормаОтчета2023Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет6_НДФЛ.ФормаОтчета2024Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет6_НДФЛ.ФормаОтчета2024Кв2"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет6НДФЛ.ФормаОтчета2017Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетДвижениеСредствПоСчетуВБанкеЗаПределамиРФ.ФормаОтчета2023Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетДвижениеСредствПоСчетуВБанкеЗаПределамиРФ.ФормаОтчета2024Кв2"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ.ФормаОтчета2024Кв2"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетБухОтчетность.ФормаОтчета2019Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетБухОтчетность.ФормаОтчета2025Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет3НДФЛ.ФормаОтчета2024кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет3НДФЛ.ФормаОтчета2025кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетИмущество.ФормаОтчета2022Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетИмущество.ФормаОтчета2024Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
...
Функция СтатусыОбъектовРеглОтчетностиПриРучномВводе(ОбъектРеглОтчетности = Неопределено, ВидКонтролирующегоОргана = Неопределено) Экспорт
	...
	Если ВидКонтролирующегоОргана = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.Минобороны") ИЛИ
		...
		Параметры.ИсточникОтчета = "РегламентированныйОтчетИсполнениеКонтрактовГОЗ" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетЦенаПродукцииГОЗ" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетКассовыйПланГОЗ" Тогда
	...
	ИначеЕсли ТипЗнч(ОбъектРеглОтчетности) = Тип("ФормаКлиентскогоПриложения")
		И (СтрНачинаетсяС(ОбъектРеглОтчетности.ИмяФормы, "Справочник.МашиночитаемыеДоверенностиРаспределенныйРеестр")
			...
			ИЛИ СтрНачинаетсяС(ОбъектРеглОтчетности.ИмяФормы,
		...
		ИЛИ ТипЗнч(ОбъектРеглОтчетности) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиФСС")
		ИЛИ ДокументооборотСКОКлиентСервер.ПодсистемаЦПРРМЧДСуществует()
			И ТипЗнч(ОбъектРеглОтчетности) = Тип("СправочникСсылка." + ИмяТипаСправочникаМашиночитаемыеДоверенности) Тогда
		...
		Если ТипЗнч(ОбъектРеглОтчетности) = Тип("ФормаКлиентскогоПриложения")
			...
			Если ТипЗнч(ОбъектРеглОтчетности) = Тип("ФормаКлиентскогоПриложения")
				И (СтрНачинаетсяС(ОбъектРеглОтчетности.ИмяФормы, "Справочник.МашиночитаемыеДоверенностиФНС")
					ИЛИ СтрНачинаетсяС(ОбъектРеглОтчетности.ИмяФормы,
						"Справочник." + ИмяТипаСправочникаМашиночитаемыеДоверенности + "."))
				ИЛИ ТипЗнч(ОбъектРеглОтчетности) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиФНС")
				ИЛИ ДокументооборотСКОКлиентСервер.ПодсистемаЦПРРМЧДСуществует()
				...
				Статусы.Добавить("Частично отозвано");
			КонецЕсли;
		КонецЕсли;
	...
	Для Инд = 0 ПО Статусы.Количество() - 1 Цикл
		Статусы[Инд] = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																					|en = '%1'"), Статусы[Инд]);
	КонецЦикла;
...
Процедура ДополнитьОпциональныеПараметрыОтчета(Форма, Параметры) Экспорт
	...
	Если ИспользуютсяРегистрацииВОрганеСтатистики() Тогда
		Если Форма.РегистрацияРеспондента <> Неопределено Тогда
			Параметры.Вставить("РегистрацияРеспондента", Форма.РегистрацияРеспондента);
		КонецЕсли;
	КонецЕсли;
...
#Область РегламентированныйОтчетПрибыль
...
Функция СуммаПоказателяПоВеткам(Форма, Ветки, ИмяПоказателя, Условия = Неопределено) Экспорт
	...
	Для каждого Ветка Из Ветки Цикл
		Если Условия = Неопределено ИЛИ ВыполненыУсловияЗаполнения(Ветка.Данные, Условия) Тогда
			Результат = Результат + ОкруглитьЧислоПоФормату(Форма, Ветка.Данные[ИмяПоказателя]);
		КонецЕсли;
	КонецЦикла;
...
Функция СтрЧГ0(ФорматируемоеЧисло, ДлинаСтроки = 0) Экспорт
	...
	ПриведенноеЗначение = ?(ФорматируемоеЧисло = Неопределено, 0, ФорматируемоеЧисло);
	
	Если ДлинаСтроки <> 0 Тогда
		Возврат Формат(ПриведенноеЗначение, СтрШаблон("ЧЦ=%1; ЧН=; ЧВН=; ЧГ=0;", ДлинаСтроки));
	Иначе
		Возврат Формат(ПриведенноеЗначение, "ЧН=; ЧГ=0");
	КонецЕсли;
...
Функция Список(СписокРазделенныйЗапятыми) Экспорт
	...
	ИсходныйСписок = СтрРазделить(СписокРазделенныйЗапятыми, ",", Ложь);
	
	ОчищенныйСписок = Новый Массив;
	Для Каждого ИсходноеЗначение Из ИсходныйСписок Цикл
		ОчищенныйСписок.Добавить(СокрЛП(ИсходноеЗначение));
	КонецЦикла;
	
	Возврат ОчищенныйСписок;
...
Функция СокращенияОбъектовАдресацииАдресаРФ() Экспорт
	...
	Результат.Вставить(НСтр("ru = 'квартира';
							|en = 'apartment'"), НСтр("ru = 'кв.';
													|en = 'apt.'"));
	Результат.Вставить(НСтр("ru = 'офис';
							|en = 'office'"), НСтр("ru = 'оф.';
												|en = 'of.'"));
	Результат.Вставить(НСтр("ru = 'бокс';
							|en = 'box'"), НСтр("ru = 'бокс';
												|en = 'box'"));
	Результат.Вставить(НСтр("ru = 'помещение';
							|en = 'wareroom'"), НСтр("ru = 'пом.';
														|en = 'wareroom'"));
	Результат.Вставить(НСтр("ru = 'комната';
							|en = 'room'"), НСтр("ru = 'ком.';
													|en = 'room'"));
	Результат.Вставить(НСтр("ru = 'этаж';
							|en = 'floor'"), НСтр("ru = 'этаж';
												|en = 'floor'"));
	Результат.Вставить(НСтр("ru = 'а/я';
							|en = 'P.O. box'"), НСтр("ru = 'а/я';
												|en = 'P.O. box'"));
	Результат.Вставить(НСтр("ru = 'п/о';
							|en = 'PO'"), НСтр("ru = 'п/о';
												|en = 'PO'"));
	Результат.Вставить(НСтр("ru = 'в/ч';
							|en = 'military unit'"), НСтр("ru = 'в/ч';
												|en = 'military unit'"));
...
Функция ПолучитьАдресСтраницыСШаблонамиРосстата() Экспорт
	...
	Возврат "https://rosstat.gov.ru/monitoring";
...
Функция ТекущаяТаблица(Форма, ТолькоРазделыСТаблицами = Ложь) Экспорт
	...
	ИначеЕсли ИмяТекущейТаблицы = "Настройки" Тогда //Для вкладки "Настройки" текущей таблицей назначаем - "Отчеты"
		
		ИмяТекущейТаблицы = "Отчеты";
...
Функция ПредставлениеПериодаРасширенное(ДатаНачала, ДатаОкончания, ДополнительныеПараметры) Экспорт
	
	Если НЕ (ТипЗнч(ДатаНачала) = Тип("Дата") И ТипЗнч(ДатаОкончания) = Тип("Дата") И ДатаНачала <= ДатаОкончания) Тогда
		
		Возврат Неопределено;
		
	КонецЕсли;
	
	ФорматнаяСтрока = ?(ДополнительныеПараметры.Свойство("ФорматнаяСтрока") И ТипЗнч(ДополнительныеПараметры[
		"ФорматнаяСтрока"]) = Тип("Строка"), ДополнительныеПараметры["ФорматнаяСтрока"], "");
	
	Если ВРег(СтрЗаменить(ФорматнаяСтрока, " ", "")) = "ФП=ИСТИНА" И Год(ДатаНачала) = Год(ДатаОкончания)
		И ((Месяц(ДатаНачала) = 1 И Месяц(ДатаОкончания) = 6) ИЛИ (Месяц(ДатаНачала) = 7 И Месяц(ДатаОкончания) = 12))
		И ДатаНачала = НачалоКвартала(ДатаНачала) И ДатаОкончания = КонецКвартала(ДатаОкончания) Тогда
		
		Возврат СтрШаблон(НСтр("ru = '%1 полугодие %2 г.';
								|en = '%1 half-year of %2'"), Месяц(ДатаОкончания) / 6, Формат(ДатаОкончания, "ДФ=гггг"));
		
	Иначе
		
		Возврат ПредставлениеПериода(НачалоДня(ДатаНачала), КонецДня(ДатаОкончания), ФорматнаяСтрока);
