...
Процедура ВойтиВОбластьДанных(Знач ОбластьДанных) Экспорт
	...
	Если ПараметрыСеанса.ОбластьДанныхИспользование Тогда
		ВызватьИсключение НСтр("ru = 'Перед входом в новую область данных, нужно выйти из предыдущей';
								|en = 'Before signing in to a new data area, sign out of the previous one'");
	КонецЕсли;
...
Процедура ОбработатьИнформациюОбОшибкеWebСервиса(Знач ИнформацияОбОшибке, Знач ИмяПодсистемы = "", Знач ИмяВебСервиса = "", Знач ИмяОперации = "") Экспорт
	...
	ИмяСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1.Ошибка вызова операции web-сервиса';
			|en = '%1.Error calling the web service operation'", ОбщегоНазначения.КодОсновногоЯзыка()),
		ИмяПодсистемы);
	
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка при вызове операции %1 веб-сервиса %2: %3';
			|en = 'An error occurred when calling operation %1 of web service %2: %3'", ОбщегоНазначения.КодОсновногоЯзыка()),
		ИмяОперации,
		ИмяВебСервиса,
...
Функция СобытиеЖурналаРегистрацииКопированиеОбластиДанных() Экспорт
	...
	Возврат НСтр("ru = 'Работа в модели сервиса.Копирование области данных';
				|en = 'SaaS.Copy data area'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииБлокировкаОбластиДанных() Экспорт
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Работа в модели сервиса.Блокировка области данных';
				|en = 'SaaS.Lock data area'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПодготовкаОбластиДанных() Экспорт
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Работа в модели сервиса.Подготовка области данных';
				|en = 'SaaS.Prepare data area'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПолучениеФайлаИзХранилища() Экспорт
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Работа в модели сервиса.Получение файла из хранилища';
				|en = 'SaaS.Get file from storage'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииДобавлениеФайлаОбменЧерезФС() Экспорт
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Работа в модели сервиса.Добавление файла.Обмен через ФС';
				|en = 'SaaS.Add file.Exchange using file system'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииДобавлениеФайлаОбменНеЧерезФС() Экспорт
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Работа в модели сервиса.Добавление файла.Обмен не через ФС';
				|en = 'SaaS.Add file.Exchange not using file system'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУдалениеВременногоФайла() Экспорт
	...
	// @skip-check module-nstr-camelcase - ошибка проверки
	Возврат НСтр("ru = 'Работа в модели сервиса.Удаление временного файла';
				|en = 'SaaS.Delete temporary file'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Если КонфликтующиеСеансы.Количество() > 0 Тогда
			...
			Для Каждого КонфликтующийСеанс Из КонфликтующиеСеансы Цикл
				...
				ТекстСеансы = ТекстСеансы + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1 (сеанс - %2)';
						|en = '%1 (session %2)'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ПолучитьФайлИзХранилища(Знач ИдентификаторФайла, Знач ПараметрыПодключения, 
		...
		Попытка
			...
			Если СвойстваФайла.Существует() Тогда
				...
				ЗаписатьВЖурналСобытиеФайловогоХранилища(
					НСтр("ru = 'Извлечение';
						|en = 'Extract'", ОбщегоНазначения.КодОсновногоЯзыка()),
					ИдентификаторФайла,
	...
	Исключение
		...
		ФайлОбъединен = Ложь;
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Выполнение операции объединения файлов';
										|en = 'Merging files'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ФайлОбъединен Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Выполнение операции чтения zip файла';
											|en = 'Reading zip file'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ФайлВременный.Существует() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Выполнение операции удаления временного файла';
											|en = 'Deleting temporary file'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ОбменЧерезФС Тогда
		...
		Если ЦелевойФайл.Существует() Тогда
			...
			Если СвойстваФайла.ПолноеИмя = ЦелевойФайл.ПолноеИмя Тогда
				...
				ЗаписатьВЖурналСобытиеФайловогоХранилища(
					НСтр("ru = 'Помещение';
						|en = 'Put'", ОбщегоНазначения.КодОсновногоЯзыка()),
					Результат,
...
Процедура ОчиститьДанныеИнформационнойБазы(УдалятьРасширения, УдалятьТекущегоПользователя = Истина) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение  НСтр("ru = 'В информационной базе включено разделение, допустимо использование метода РаботаВМоделиСервиса.ОчиститьДанныеОбласти';
								|en = 'Separation is enabled in the infobase. You can use the SaaSOperations.ClearAreaData method'");	
	КонецЕсли;
	...
	Если ОписаниеБТС = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В конфигурацию не внедрена библиотека ""1С:Библиотека технологии сервиса"".
                  ...
                  |en = 'The 1C:Service Technology Library was not implemented in the configuration.
                  |The configuration cannot be used in SaaS without this library.
                  |
                  |To use this configuration, implement
                  |the 1C:Service Technology Library library in SaaS of the version not earlier than %1.'", Метаданные.ОсновнойЯзык.КодЯзыка),
			ТребуемаяВерсияБТС());
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБТС, ТребуемаяВерсияБТС()) < 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для использования конфигурации в модели сервиса с текущей версией БСП требуется
                      ...
                      |
                      |Current version: %1, version not earlier than %2 is required.'", Метаданные.ОсновнойЯзык.КодЯзыка),
...
Процедура ВызватьИсключениеОтсутствуетРасширениеБТС() Экспорт
	...
	Если ЗначениеЗаполнено(ВерсияРасширения) Тогда
		...
		|is not suitable for Cloud Technology Library version %2'"), ВерсияРасширения,
...
Процедура ЗаписатьВЖурналСобытиеФайловогоХранилища(Знач Событие,
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Хранилище файлов';
			|en = 'File storage'", ОбщегоНазначения.КодОсновногоЯзыка()) + "." + Событие,
...
Процедура КонтрольНеразделенныхДанныхПриЗаписи(Знач Источник)
	...
	Если РазделениеВключено() И ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ПредставлениеИсключения = НСтр("ru = 'Нарушение прав доступа.';
										|en = 'Access violation.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если ВызыватьИсключение Тогда
		...
		Если Не ПустаяСтрока(Результат.ТекстИсключения) Тогда
			...
			ВызватьИсключение НСтр("ru = 'Обнаружены ошибки в структуре метаданных конфигурации:';
									|en = 'The following errors are found in the applied solution metadata structure:'") 
...
Процедура УдалитьДанныеПриложенияВФоне(ОбластьДанных)
	...
	Задание = РасширенияКонфигурации.ВыполнитьФоновоеЗаданиеБезРасширений(
		...
		СтрШаблон(НСтр("ru = 'Очистка данных приложения %1';
						|en = 'Clear the %1 application data'"), ОбластьДанныхСтрокой));
