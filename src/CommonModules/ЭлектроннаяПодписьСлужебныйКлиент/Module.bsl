...
Функция РекомендацииДляТокена() Экспорт
	...
	Возврат НСтр("ru = 'Убедитесь, что токен правильно вставлен в USB-порт компьютера и на нем горит световой индикатор, обновите список токенов и повторите действие.
		...
		|en = 'Make sure the token is correctly inserted into the USB port of the computer and the indicator light is on, update the list of tokens, and try again.
		|If the token is not found after updating the token list, try another USB port or another computer.'");
	...
	Иначе
		ОписаниеПрограммы.РезультатПроверкиНаКлиенте =
			ЭлектроннаяПодписьСлужебныйКлиентСервер.ТекстОшибкиПоискаПрограммы(
				НСтр("ru = 'Не установлена на компьютере.';
					|en = 'It is not installed on the computer.'"), Результат);
	КонецЕсли;
...
Процедура СохранитьПодпись(АдресПодписи, ИмяФайлаПодписи = "") Экспорт
	...
	Фильтр = Фильтр + "|" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Все файлы (%1)|%1';
																						|en = 'All files (%1)|%1'"), ПолучитьМаскуВсеФайлы());
		...
		Если СвойстваПодписи.Сертификат = Неопределено Тогда
			ВыполнитьОбработкуОповещения(Контекст.Оповещение, РезультатПроверкиПодписи(
				НСтр("ru = 'Сертификат не существует в данных подписи.';
					|en = 'The certificate does not exist in signature data.'"), Контекст.РезультатПроверки));
			Возврат;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ДополнительныеПараметры.Предупреждение) И Не Контекст.ДляПроверкиПодписи
			...
			ПоказатьОповещениеПользователя(
					НСтр("ru = 'Необходима замена сертификата';
						|en = 'You need to reissue the certificate'"), ДействиеПриНажатии, ДополнительныеПараметры.Сертификат,
		...
		Если Не Результат.УстановкаВыполнена Тогда
			...
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Обновление списков отзыва';
					|en = 'Digital signature.Update revocation list'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
		...
		Если Не РезультатПроверкиУдостоверяющегоЦентраСертификата.Действует Или ЗначениеЗаполнено(
			...
			Если ЗначениеЗаполнено(РезультатПроверкиУдостоверяющегоЦентраСертификата.Предупреждение.ТекстОшибки)
				...
				Если Не ПользовательскиеНастройкиСертификата.Оповещен И СсылкаНаСертификат <> Неопределено Тогда
					...
					ПоказатьОповещениеПользователя(
					НСтр("ru = 'Необходима замена сертификата';
						|en = 'You need to reissue the certificate'"), ДействиеПриНажатии, СсылкаНаСертификат,
...
Асинх Процедура СоздатьМенеджерКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	Если Не Подключено Тогда
		ТекстОбщейОшибки = НСтр("ru = 'Установите в браузере расширение для работы с 1С:Предприятием.';
								|en = 'Установите в браузере расширение для работы с 1С:Предприятием.'");
		СвойстваОшибки = ЭлектроннаяПодписьСлужебныйКлиентСервер.НовыеСвойстваОшибки();
...
Функция ЗаголовокОшибкиОперации(Операция, ПоказатьОшибку)
	...
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		ЗаголовокОшибки = НСтр("ru = 'Не удалось проверить подпись по причине:';
								|en = 'Cannot check the signature due to:'");
	...
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		ЗаголовокОшибки = НСтр("ru = 'Не удалось проверить сертификат по причине:';
								|en = 'Cannot check the certificate due to:'");
	...
	ИначеЕсли Операция <> "" Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в функции %1.
			           ...
			           |Parameter ""Operation"" has invalid value: %2.'"), "МенеджерКриптографии", Операция);
	Иначе
	...
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось подключить внешнюю компоненту %1.';
				|en = 'Couldn''t attach add-in ""%1"".'"), ОписаниеКомпоненты.ИмяОбъекта);
	...
	Если Форма = Неопределено Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Файл сертификата должен быть в формате DER X.509.';
				|en = 'Файл сертификата должен быть в формате DER X.509.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не Форма.Открыта() Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Сертификат уже добавлен.';
				|en = 'Certificate is already added.'"), Кнопки);
	КонецЕсли;
...
Процедура ДобавитьСертификатыТолькоДляШифрованияИзФайлаПослеПомещенияФайлов(ПомещенныеФайлы, Контекст)
	...
	Если Форма = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'В выбранном каталоге отсутствуют сертификаты для добавления.';
										|en = 'В выбранном каталоге отсутствуют сертификаты для добавления.'"));
		Возврат;
	КонецЕсли;
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'Не удалось записать файл по причине:';
				|en = 'Cannot save the file due to:'") + Символы.ПС + Результат.ОписаниеОшибки);
		
		ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru = 'Не удалось сохранить подписи вместе с файлом';
				|en = 'Couldn''t save signatures with the file'"), "", Ошибка, Новый Структура);
		Возврат;
			...
			Если Контекст.Операция <> "Подписание" Тогда
				Ошибка = Новый Структура("ОписаниеОшибки",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Внешняя компонента %1 может использоваться только для подписания.';
							|en = 'External add-in ""%1"" is intended for signing only.'"), "ExtraCryptoAPI"));
				ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
				Возврат;
			КонецЕсли;
			...
			Если СвойстваПодписи.Успех = Ложь Тогда
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1.
						 |Результат подписи: %2';
						 |en = '%1.
						 |Signature result: %2'"), СвойстваПодписи.ТекстОшибки, ДанныеПодписи);
				ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", ОписаниеОшибки);
				ОшибкаНаКлиенте.Вставить("Инструкция", Истина);
				ВыполнитьНаСторонеПослеЦикла(ОшибкаНаКлиенте, Контекст);
				Возврат;
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'При записи подписи возникла ошибка:';
				|en = 'An error occurred when saving the signature:'") + Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Не Контекст.ИгнорироватьСрокДействияСертификата
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Истек срок действия сертификата подписи: %1';
				|en = 'The signature certificate expired:%1'"),
...
Процедура УсовершенствоватьНаСторонеПослеОшибки(Ошибка, Контекст)
	...
	Если Не Контекст.ПрерыватьОбработкуМассиваПриОшибке Тогда
		...
		+ ?(Ошибка.НаКлиенте, НСтр("ru = 'На компьютере:';
									|en = 'On the computer:'"), НСтр("ru = 'На сервере:';
																|en = 'On the server:'")) + " "
		+ Ошибка.Текст;
	Иначе
		ЭлементДанных.Ошибка = ЭлементДанных.Ошибка + ?(ЭлементДанных.Ошибка <> "", Символы.ПС, "")
	...
	Если Контекст.ЕстьОшибки Или Не Контекст.ОперацияНачалась Тогда
		...
		Иначе
			...
			Для Каждого ЭлементДанных Из Контекст.ЭлементыДанных Цикл
				...
				КонецЕсли;
			КонецЦикла;
			НачалоОшибки = ?(Контекст.НаСторонеКлиента, НСтр("ru = 'На компьютере:';
															|en = 'On the computer:'"), НСтр("ru = 'На сервере:';
																							|en = 'On the server:'"));
	...
	Иначе
		...
		Если ПустаяСтрока(Результат.ОписаниеОшибки) Тогда 
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Операция невозможна. Требуется установка компоненты %1.';
		...
		Иначе
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Операция невозможна. %1';
						|en = 'Operation is not allowed. %1'"), Результат.ОписаниеОшибки));
	...
	Если ОписаниеПрограммы = Неопределено Тогда
		Если Не ПустаяСтрока(ОписаниеОшибки) Тогда
			ЗавершитьОперациюСОшибкой(Контекст, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось определить тип криптопровайдера %1. %2';
					|en = 'Cannot define a type of cryptographic service provider %1. %2'"), ИнформацияМодуляКриптографии.Имя, ОписаниеОшибки));
		Иначе
			ЗавершитьОперациюСОшибкой(Контекст, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось определить тип криптопровайдера %1';
					|en = 'Cannot define a type of cryptographic service provider %1'"), ИнформацияМодуляКриптографии.Имя));
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ПослеУстановкиСвойстваЗапретитьПользовательскийИнтерфейс(Контекст) Экспорт
	...
	Если Контекст.ТипПодписи = "XMLDSig" Тогда
		...
		Иначе
			ЗавершитьОперациюСОшибкой(Контекст, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не установлен режим работы компоненты %1.';
	...
	Иначе
		...
		ЗавершитьОперациюСОшибкой(Контекст, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не установлен тип подписи компоненты %1.';
...
Процедура НачатьПодписание_ПослеВыгрузкиСертификатаКриптографии(ДвоичныеДанныеСертификата, Контекст) Экспорт
	...
	ПодключитьКомпоненту(Контекст, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Для подписания XML необходима установка компоненты %1';
...
Процедура НачатьПодписаниеCMS_ПослеВыгрузкиСертификатаКриптографии(ДвоичныеДанныеСертификата, Контекст) Экспорт
	...
	ПодключитьКомпоненту(Контекст, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Для подписания CMS необходима установка компоненты %1';
		...
		Если ЭлектроннаяПодписьКлиент.ОбщиеНастройки().ДоступнаУсовершенствованнаяПодпись И ЗначениеЗаполнено(Контекст.ОписаниеДанных.ТипПодписи) Тогда
			Если Контекст.ОписаниеДанных.ТипПодписи <> ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.БазоваяCAdESBES")
				...
				Ошибка = Новый Структура("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Недоступно подписание типом подписи %1 с помощью встроенного криптопровайдера.';
						|en = 'Cannot sign with signature type %1 using the built-in cryptographic service provider.'"), Контекст.ОписаниеДанных.ТипПодписи));
				ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
				Возврат;
			КонецЕсли;
		КонецЕсли;
	...
	Если Не РезультатПоиска.Выполнено Тогда
		Ошибка = Новый Структура("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось найти сертификат в сервисе по причине:
			           |%1';
			           |en = 'Cannot find a certificate in the service due to:
...
Функция НачалоОписанияТекстаОшибки(ИмяМетода)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Метод %1 не выполнен по причине:';
			|en = 'The %1 method is not executed due to:'"), ИмяМетода); 
...
Процедура ПодписатьОблачнаяПодпись(Оповещение, Контекст, Данные, ПараметрыОперации, ТипРезультата = Неопределено)
	...
	Если ТипЗнч(Данные) = Тип("Структура") 
		...
		Иначе
			Ошибка = Новый Структура("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недоступно подписание типом подписи %1 с помощью учетной записи DSS.';
					|en = 'Cannot sign with signature type %1 using DSS account.'"), ПараметрыCMS.ТипПодписи));
			ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ЭлектроннаяПодписьКлиент.ОбщиеНастройки().ДоступнаУсовершенствованнаяПодпись И ЗначениеЗаполнено(Контекст.ОписаниеДанных.ТипПодписи) Тогда
		...
		Иначе
			Ошибка = Новый Структура("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недоступно подписание типом подписи %1 с помощью учетной записи DSS.';
					|en = 'Cannot sign with signature type %1 using DSS account.'"), Контекст.ОписаниеДанных.ТипПодписи));
			ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
			Возврат;
		КонецЕсли;
	Иначе
...
Функция ТекстОшибкиОблачнаяПодпись(Операция, РезультатВызова = Неопределено, ЭтоОшибкаВыполнения = Ложь, ЭтоВстроенныйКриптопровайдер = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ИсходныйТекст) Тогда
		...
		Если ЭтоОшибкаВыполнения И ЗначениеЗаполнено(АдресСервера) Тогда
			ТекстОшибки = ТекстОшибки + " " + НСтр("ru = 'на сервере';
													|en = 'on server'") + " " + СокрЛП(АдресСервера);
		КонецЕсли;
		
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		Если ЭтоОшибкаВыполнения Тогда
			ТекстОшибки = НСтр("ru = 'Не удалось проверить подпись.';
								|en = 'Cannot check the signature.'");
		Иначе
	...
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		Если ЭтоОшибкаВыполнения Тогда
			ТекстОшибки = НСтр("ru = 'Не удалось проверить сертификат.';
								|en = 'Cannot check the certificate.'");
		Иначе
	...
	ИначеЕсли Операция = "ПоискСертификата" Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось найти сертификат (возможно удален).';
							|en = 'Cannot find the certificate. It might have been deleted.'");
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неизвестная операция ""%1"".';
				|en = 'Unknown operation ""%1"".'"), Операция);
	КонецЕсли;
	...
	Если ЭтоВстроенныйКриптопровайдер Тогда
		...
		Если Не ОбщегоНазначенияКлиент.РазделениеВключено() Тогда
			ТекстРекомендации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отключите константу ""%1"".';
					|en = 'Disable the ""%1"" constant.'"), НСтр("ru = 'Использовать электронную подпись в модели сервиса';
		...
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Встроенный криптопровайдер: %1 %2';
				|en = 'Built-in cryptographic service provider: %1 %2'"), ТекстОшибки, ТекстРекомендации);
	Иначе
		Возврат НСтр("ru = 'Сервис DSS';
					|en = 'DSS service'") + ": " + ТекстОшибки;
	КонецЕсли;
...
Процедура СформироватьТехническуюИнформацию(Причина,
	...
	СопроводительныйТекст = СокрЛ(СокрП(Причина) + Символы.ПС + Символы.ПС)
		+ НСтр("ru = 'Информация о компьютере:';
				|en = 'Computer information:'") + Символы.ПС + Символы.ПС
...
Асинх Процедура ДиагностическаяИнформацияОПрограммахОпределенныхАвтоматически(Оповещение)
	...
	Заголовок = Символы.ПС + Токены + Символы.ПС + Символы.ПС + НСтр("ru = 'Автоматически определенные приложения электронной подписи и шифрования на компьютере:';
																	|en = 'Digital signing and encryption apps are auto-determined on your computer:'") + Символы.ПС;
...
Процедура ДиагностическаяИнформацияОПрограммах(Оповещение)
	...
	Если ИспользуемыеПрограммы.Количество() = 0 Тогда
		...
		Возврат;
	КонецЕсли;
	Заголовок = Символы.ПС + НСтр("ru = 'Программы на клиенте из справочника:';
									|en = 'Applications on the client from the catalog:'") + Символы.ПС;
...
Процедура ПолучитьВерсиюКомпоненты(Оповещение)
	...
	ПараметрыПодключения = ОбщегоНазначенияКлиент.ПараметрыПодключенияКомпоненты();
	ПараметрыПодключения.ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Для получения версии требуется установка компоненты %1';
	...
	Если Результат.Подключено Тогда
		...
		Исключение
			ВерсияКомпоненты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось определить версию компоненты. %1';
					|en = 'Cannot determine the add-in version. %1'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	Иначе
		Если ПустаяСтрока(Результат.ОписаниеОшибки) Тогда 
			// Пользователь отказался от установки.
			ВерсияКомпоненты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Компонента %1 не установлена.';
					|en = 'Add-in ""%1"" is not installed.'"), "ExtraCryptoAPI");
		Иначе 
			// Установка не удалась, описание ошибки в Результат.ОписаниеОшибки.
			ВерсияКомпоненты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Компонента %1 не установлена (%2).';
					|en = 'Add-in ""%1"" is not installed (%2).'"), "ExtraCryptoAPI", Результат.ОписаниеОшибки);
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьПослеПроверкиРасширенияИКомпоненты(Оповещение, Параметры = Неопределено)
	...
	Если Контекст.ТекстПояснения = Неопределено Тогда
		Контекст.ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для выполнения требуется установка компоненты %1';
...
#Если Не МобильноеПриложениеКлиент И Не МобильныйКлиент Тогда
	...
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись.Диагностика ошибок';
				|en = 'Digital signature.Error troubleshooting'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
...
Асинх Процедура УдалитьСертификатИзЛокальногоХранилищаПродолжение(МенеджерКриптографии, ДанныеСертификата) Экспорт
	...
	Если Ждать ХранилищеЛокальногоКомпьютера.НайтиПоОтпечаткуАсинх(СертификатКриптографии.Отпечаток) = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Сертификат не найден в личном хранилище компьютера. Выполните проверку заново.';
										|en = 'Certificate not found in personal storage on computer. Try again.'"));
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		ПараметрыОшибки.Вставить("ЗаголовокПредупреждения", НСтр("ru = 'Удаление сертификата из локального хранилища';
																|en = 'Uninstall certificate from local storage'"));
	...
	ПоказатьПредупреждение(
		...
		НСтр("ru = 'Сертификат удален из личного хранилища компьютера.';
			|en = 'The certificate is deleted from the personal computer store.'"));
...
Асинх Процедура УстановитьСписокОтзываСертификатаПослеПодключенияКомпоненты(ОбъектКомпоненты, Контекст) Экспорт
	...
	Параметры.НаименованиеОперации = НСтр("ru = 'Загрузка списка отзыва сертификата';
											|en = 'Import certificate revocation list'");
...
Процедура ПродолжитьУстановкуСпискаОтзываПослеСкачивания(РезультатВыполнения, Контекст) Экспорт
	...
	Если РезультатВыполнения = Неопределено Тогда
		ОбработатьОшибкуУстановкиСпискаОтзыва(НСтр("ru = 'Загрузка списка отзыва не была завершена штатно';
													|en = 'Abnormal end of certificate revocation list import'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Результат.ДанныеФайла) Тогда
		ОбработатьОшибкуУстановкиСпискаОтзыва(
			НСтр("ru = 'Не удалось загрузить список отзыва. Подробнее см. в журнале регистрации.';
				|en = 'Cannot import the revocation list. For more information, see the event log.'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Если СвойстваСпискаОтзыва = Неопределено Тогда
		ОбработатьОшибкуУстановкиСпискаОтзыва(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Файл %1 не является списком отзыва.';
				|en = 'This file is not a revocation list: %1.'"), Результат.АдресФайла), Результат, Истина);
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СвойстваСпискаОтзыва.ДатаОкончания) И СвойстваСпискаОтзыва.ДатаОкончания < ОбщегоНазначенияКлиент.ДатаУниверсальная() Тогда
		ОбработатьОшибкуУстановкиСпискаОтзыва(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Список отзыва %1 просрочен. Обратитесь в удостоверяющий центр, выдавший сертификат.';
				|en = 'The certificate revocation list %1 is expired. Contact the certificate issuer.'"), Результат.АдресФайла), Результат, Истина);
		Возврат;
	КонецЕсли;
...
Асинх Процедура УстановитьСписокОтзываПослеСозданияВременногоКаталога(ИмяВременногоКаталога, Контекст) Экспорт
	...
	Если Не ЗначениеЗаполнено(ИмяВременногоКаталога) Тогда
		ОбработатьОшибкуУстановкиСпискаОтзыва(
			НСтр("ru = 'Не удалось создать временный каталог для загрузки списка отзыва.';
				|en = 'Cannot create a temporary directory to import a revocation list.'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось записать список отзыва в файл ""%1"": %2. Проверьте наличие свободного места, права доступа и настройки антивируса.';
				|en = 'Не удалось записать список отзыва в файл ""%1"": %2. Проверьте наличие свободного места, права доступа и настройки антивируса.'"),
	...
	Иначе
		Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось записать список отзыва в файл ""%1"". Проверьте наличие свободного места, права доступа и настройки антивируса.';
				|en = 'Не удалось записать список отзыва в файл ""%1"". Проверьте наличие свободного места, права доступа и настройки антивируса.'"),
	...
	Если РезультатУстановки.Значение <> Истина Тогда
		...
		Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось установить список отзыва в хранилище ""%1"" из файла ""%2"" (%3 байт) по причине:
				 |%4';
				 |en = 'Не удалось установить список отзыва в хранилище ""%1"" из файла ""%2"" (%3 байт) по причине:
	...
	Если ЗначениеЗаполнено(АдресРесурса) Тогда
		Если ОшибкаИздателя Тогда
			...
			Ошибка = Ошибка + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Адрес списка отзыва, указанный в сертификате: %1';
					|en = 'Revocation list''s address in certificate: %1'"), СтрСоединить(АдресРесурса, Символы.ПС));
		...
		Иначе
			...
			Ошибка = Ошибка + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|%1';
				|en = 'Manually import and set the revocation list from:
...
Асинх Процедура УстановитьКорневойСертификатПослеПодключенияКомпоненты(ОбъектКомпоненты, ПараметрыУстановки) Экспорт
	...
	Если Результат.Сертификаты.Количество() = 0 Тогда
		ОбработатьОшибкуУстановкиСертификата(НСтр("ru = 'Цепочка сертификатов не содержит сертификатов.';
													|en = 'The certificate chain does not contain any certificates.'"), ПараметрыУстановки);
		Возврат;
	КонецЕсли;
...
Асинх Процедура УстановитьСертификатПослеВыбораВариантаУстановки(ПараметрыУстановкиСертификата, ОбъектКомпоненты = Неопределено) Экспорт
	...
	Результат.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Сертификат успешно установлен в %1.';
				|en = 'The certificate is installed in %1.'"), ПредставлениеХранилища);
...
Функция ТекстОшибкиНеУстановленаКомпонента()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не установлена компонента %1';
...
Процедура УстановитьКриптоПро(Форма, ОповещениеОЗавершении, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
			...
			|en = 'You can automatically install %1 only on Windows.
			|To install it manually, download the distribution package from %2.'"),
...
Процедура УстановитьCryptoProCSPПослеВводаРегистрационныхДанных(РегистрационныеДанные, ВходящийКонтекст) Экспорт
	...
	Иначе
		ОбработатьОшибкуУстановкиПрограммыКриптографии(НСтр("ru = 'Пользователь прервал операцию';
															|en = 'The user interrupted the transaction'"), ВходящийКонтекст);
	КонецЕсли;
	...
	Иначе
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Установка %1 возможна только на операционных системах семейства Windows.';
				|en = 'You can install %1 only on Windows operating systems.'"),
...
Процедура УстановитьКриптопровайдерПослеПолученияДистрибутива(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	Если ДлительнаяОперация = Неопределено Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Вызов API сервиса выдачи дистрибутивов %1 не был завершен штатно.';
				|en = 'The %1 API distribution package service was not called correctly.'"), ВходящийКонтекст.ИмяПрограммы);
...
Процедура ПослеСохраненияФайловДистрибутива(ПолученныеФайлы, ВходящийКонтекст) Экспорт
	...
	Если ПолученныеФайлы = Неопределено Тогда
		ОбработатьОшибкуУстановкиПрограммыКриптографии(НСтр("ru = 'Не удалось сохранить на компьютер файлы дистрибутива';
															|en = 'Cannot save the distribution package files to the computer'"),
			ВходящийКонтекст);
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьОшибкуУстановкиПрограммыКриптографии(Ошибка, ВходящийКонтекст)
	...
	Иначе
		...
		ПоказатьПредупреждение(Неопределено, Ошибка,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Установка %1';
				|en = 'Install %1'"), ВходящийКонтекст.ИмяПрограммы));
...
Процедура ПослеПроверкиОтправкиВПоддержку(Результат, Контекст) Экспорт
	...
	Если Результат <> Неопределено И ЗначениеЗаполнено(Результат.КодОшибки) Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Отправка сообщения в техподдержку';
					|en = 'Digital signature.Send request to technical support'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
...
Функция ТекстОбращенияВТехническуюПоддержку() Экспорт
	Возврат НСтр("ru = '<Опишите возникшую проблему, приложите скриншоты ошибки и версии приложения электронной подписи и шифрования (криптопровайдера).>';
				|en = '<Опишите возникшую проблему, приложите скриншоты ошибки и версии приложения электронной подписи и шифрования (криптопровайдера).>'");
КонецФункции
...
Асинх Функция СвойстваПодписиЧтениеМенеджеромКриптографии(Подпись, МенеджерКриптографии, ПрочитатьСертификаты) Экспорт
	...
	Исключение
		...
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При чтении данных подписи: %1';
						|en = 'When reading the signature data: %1'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(
					ИнформацияОбОшибке()));
		Возврат Результат;
	КонецПопытки;
...
Асинх Функция СвойстваПодписиИзДвоичныхДанных(Подпись, ПрочитатьСертификаты)
	...
	Исключение
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось прочитать свойства подписи: %1';
				|en = 'Cannot read the signature properties: %1'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(
			ИнформацияОбОшибке()));
		Результат.Успех = Ложь;
		Возврат Результат;
	КонецПопытки;
...
#Иначе
		СтрокаПодключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Подключение к серверам меток времени на компьютере <%1>.';
					|en = 'Accessing timestamp servers on computer <%1>.'"), ИмяКомпьютера());
...
Процедура ОбработатьОшибкуРасшифровки(Ошибка, Контекст)
	...
	Если Не ПустаяСтрока(ИнформацияАлгоритмаШифрования.Идентификатор) Тогда
		Ошибка.ОписаниеОшибки = СокрЛ(ТекстОшибки + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'OID алгоритма: %1';
				|en = 'Algorithm OID: %1'"), ИнформацияАлгоритмаШифрования.Идентификатор));
	КонецЕсли;
