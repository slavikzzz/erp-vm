		...
		Если ВидДокумента = "Реестр" Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = '%1 документов отмечен как принятый в ПФР. Редактирование запрещено';
							|en = '%1 documents marked as accepted to PF. Editing is prohibited  '"), 
						ВидДокумента);
		Иначе
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ДатаПостановленияАДВ2022() Экспорт
	
	Возврат '20230101';
	
КонецФункции

Функция ДатаПостановленияАДВ2023() Экспорт
	
	Возврат '20231215';
	
КонецФункции

Функция ДатаПостановленияЕФС1_2023() Экспорт
	
	Возврат '20240101';
	
КонецФункции

Функция ДатаВФорматеПФР(Дата) Экспорт 
...
Функция ПредставлениеОтчетногоПериода(Период, Кратко = Ложь) Экспорт
	...
	ИначеЕсли Период >= '20100101' Тогда
		Если Месяц(Период) <= 6  Тогда
			Если Кратко Тогда
				Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '1 полугод. %1 г.';
																					|en = '1 half-year %1'"), Формат(Период, "ДФ=ггг"));
			Иначе	
				Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '1 полугодие %1 г.';
																					|en = '1st half year of %1'"), Формат(Период, "ДФ=ггг"));
			КонецЕсли;	
		Иначе
			Если Кратко Тогда
				Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '2 полугод. %1 г.';
																					|en = '2 half-year %1'"), Формат(Период, "ДФ=ггг"));
			Иначе	
				Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '2 полугодие %1 г.';
																					|en = '2 half-year %1'"), Формат(Период, "ДФ=ггг"));
			КонецЕсли;	
		КонецЕсли;
...
Функция ОписаниеОтчетногоПериода(Начало, Окончание, Код) Экспорт
	...
	Если Начало = Дата(Год(Начало), 7, 1) И КонецДня(Окончание) = КонецГода(Начало) Тогда
		ОписаниеПериода.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '2 полугодие %1 г.';
																									|en = '2 half-year %1'"), Формат(Начало, "ДФ=ггг"));
