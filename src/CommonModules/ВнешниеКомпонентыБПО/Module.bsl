		...
		Если Не МакетСуществует(ПолноеИмяМакета) Тогда 
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"" на сервере
				           ...
				           |from %2.
				           |Reason:
				           |Attachment on the server not from the template is not allowed'"),
				Идентификатор,
				ПолноеИмяМакета);
		КонецЕсли;
			...
			Если ПодключаемыйМодуль = Неопределено Тогда
				...
				ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Не удалось создать объект внешней компоненты ""%1"", подключенной на сервере
					           ...
					           |по причине:
					           |%3';
					           |en = 'Cannot create an object for the ""%1"" add-in attached on the server
					           |from the ""%2"" template.
					           |Reason:
					           |%3'"),
					Идентификатор,
				...
				ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
					НСтр("ru = 'Подключение внешней компоненты на сервере';
						|en = 'Attaching add-in on the server'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()),
					ТекстОшибки);
			КонецЕсли;
		...
		Иначе
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"" на сервере
				           ...
				           |from the ""%2""
				           | template. Reason:
				           |The platform returned False when attaching an add-in.'"),
				Идентификатор,
				Местоположение);
			ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
				НСтр("ru = 'Подключение внешней компоненты на сервере';
