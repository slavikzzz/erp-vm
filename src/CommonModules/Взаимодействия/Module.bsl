...
Процедура СоздатьИОтправитьСообщениеSMS(Сообщение) Экспорт
	...
	СообщениеSMS.Комментарий = НСтр("ru = 'Создано и отправлено по шаблону';
									|en = 'Created from template and sent'") + " - " + Сообщение.ДополнительныеПараметры.Наименование;
...
Процедура ПриПоискеЗаменыСсылок(ПарыЗамен, НеобработанныеЗначенияОригиналов) Экспорт
	...
	Для каждого НеобработанныйДубль Из НеобработанныеЗначенияОригиналов Цикл
		...
		Исключение
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Поиск и замена ссылок';
											|en = 'Find and replace references'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция СписокДоступныхПоисков(ППДВключен, Параметры, ЭлементыФормы, ДляАдреснойКниги) Экспорт
	...
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "ПоEmail", НСтр("ru = 'По email';
																						|en = 'In email address'"), Адрес);
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "ПоДомену", НСтр("ru = 'По доменному имени';
																							|en = 'In domain name'"), ДоменныйАдрес);
	
	Если Не ДляАдреснойКниги И (Параметры.Свойство("ТолькоEmail") И Не Параметры.ТолькоEmail) Тогда
		ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "ПоТелефону", НСтр("ru = 'По телефону';
																								|en = 'In phone number'"), Адрес);
	КонецЕсли;
	...
	Если ППДВключен Тогда
		ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "ПоСтроке",
			НСтр("ru = 'По строке';
				|en = 'In description'"), ВариантыПоискаПоСтроке);
	КонецЕсли;
	
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы.ВариантыПоиска, "НачинаетсяС", НСтр("ru = 'Начинается с';
																							|en = 'Begins with'"), Представление);
...
Процедура УстановитьЗаголовокФормыЭлектронногоПисьма(Форма) Экспорт
	...
	Если НЕ ОбъектПисьмо.Ссылка.Пустая() Тогда
		...
		ЗаголовокФормы = ?(ПустаяСтрока(ОбъектПисьмо.Тема), НСтр("ru = 'Письмо без темы (%1)';
																|en = 'No-subject email (%1)'"), ОбъектПисьмо.Тема + " (%1)");
		Форма.Заголовок  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ЗаголовокФормы,
			?(ТипЗнч(ОбъектПисьмо.Ссылка) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее"), НСтр("ru = 'Входящее';
																									|en = 'Incoming'"), НСтр("ru = 'Исходящее';
																														|en = 'Outgoing'")));
...
Функция СформироватьЭлементДанныхШапкиПисьма(ЭлементРодитель, ШапкаПисьма, ТолькоПоПредставлениюОтправителя = Ложь)
	...
	Тема = ?(ПустаяСтрока(ШапкаПисьма.Тема), НСтр("ru = '<Без Темы>';
													|en = '<No Subject>'"), ШапкаПисьма.Тема);
...
Процедура ДополнитьТелоПисьмаПодваломВложения(ДокументHTML, Вложения) Экспорт
	...
	СтрокаКоличествоВложений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Вложений: %1.';
																							|en = '%1 attachments.'"), Вложения.Количество());
	ДобавитьТекстовыйУзел(ЭлементШрифт, СтрокаКоличествоВложений, Истина, Истина);
	
	Для Каждого Вложение Из Вложения Цикл 
		
		ПредставлениеВложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2).';
																							|en = '%1 (%2).'"), Вложение.ИмяФайла, Вложение.РазмерПредставление);
...
Функция ОтправкаSMSПоДокументу(Документ) Экспорт
	...
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не выполнены настройки отправки SMS.';
													|en = 'SMS settings not configured.'"),, "Объект");
	...
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
		ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), 
			...
			НСтр("ru = 'Не выполнены настройки отправки SMS.';
				|en = 'SMS settings not configured.'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат;
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ШаблонСообщенияОбОшибке = НСтр("ru = 'Не удалось применить правило обработки писем ""%1"" для почты ""%2"" по причине: 
			                                ...
			                                |%3
			                                |Correct the mailbox rule.'", ОбщегоНазначения.КодОсновногоЯзыка());
