...
Функция ДанныеДляРасшифровкиДопРасходовМеждуПартиямиИТоварами(Период, МассивОрганизаций, Регистратор, РасчетСебестоимостиВыполнен) Экспорт
	...
	СостояниеРасчетаСебестоимости = РасчетСебестоимостиПрикладныеАлгоритмы.ТекущееСостояниеРасчета(
		Период, МассивОрганизаций, Перечисления.ОперацииЗакрытияМесяца.РасчетПартийИСебестоимости);
...
Функция ПодготовитьБазуПоПрямымЗатрат(Параметры) Экспорт
	...
	ПараметрыЗапуска = Новый Структура(
		...
		Параметры.Период, Параметры.МассивОрганизаций, Истина);
	...
	ПараметрыРасчета.Вставить("Регистратор", Параметры.ДокументРаспределения);
	...
	//++ НЕ УТКА
	ПараметрыПолученияПартий = ЗатратыСервер.ОписаниеПараметровПолученияАктивныхПартийБезСпецификаций();
	...
	ЗатратыСервер.СформироватьАктивныеПартииБезСпецификаций(ПараметрыПолученияПартий, ПараметрыРасчета.МенеджерВременныхТаблиц);
	//-- НЕ УТКА
...
Процедура РасчетТранзитовОВЗ(ПараметрыРасчета) Экспорт
	...
	Запрос.Текст = РасчетСебестоимостиЛокализация.ТекстЗапросаНастроекНДД("", "РасчетСебестоимостиПостатейныеЗатраты_РасчетТранзитовОВЗ", ПараметрыРасчета);
	РасчетСебестоимостиПрикладныеАлгоритмы.ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос, Истина, 
		"НастройкиПризнанияРасходовНДД,");
	...
	|	СУММА(ВЫБОР
	|		КОГДА НастройкиПризнанияРасходовНДДпоПодразделению.ВариантПризнанияРасходов ЕСТЬ NULL
	...
	|	ПО НастройкиПризнанияРасходовНДДпоПодразделению.ОбъектРаздельногоУчетаНДД = ДД.Подразделение
	|		И НастройкиПризнанияРасходовНДДпоПодразделению.Организация = ДД.Организация
	...
	|	ПО НастройкиПризнанияРасходовНДДпоНаправлениюДеятельности.ОбъектРаздельногоУчетаНДД = ДД.НаправлениеДеятельности
	|		И НастройкиПризнанияРасходовНДДпоНаправлениюДеятельности.Организация = ДД.Организация
	...
	|	ДД.СтатьяРасходов КАК СтатьяРасходов,
	|	ДД.АналитикаРасходов КАК АналитикаРасходов,
	|	ДД.КорОрганизация КАК КорОрганизация,
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.УничтожитьВременныеТаблицы(ПараметрыРасчета,
		...
		|ВтРасходыКТранзитуМеждуОВЗРегл,ВтИсходныеДанныеУзловТранзитаРасходовМеждуОВЗРегл,ЗначенияПоказателейОВЗ,
		|НастройкиПризнанияРасходовНДДпоПодразделению,НастройкиПризнанияРасходовНДДпоНаправлениюДеятельности
		|");
...
Процедура РаспределитьРасходыБудущихПериодов(ПараметрыРасчета) Экспорт
	...
	|		КОГДА ДД.ВариантУказанияСуммыРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСуммыРБП.ОпределяетсяАвтоматически)
	|		И ДД.НДД
	|		И Статья.ВариантРаспределенияРасходовНУ В
...
Функция ТекстДвижениеРасходОтклоненийВСтоимостиСебестоимостьТоваров()
	Возврат 
		...
		|		КОНЕЦ КАК КорПартия,
		|		ВЫБОР
		|			КОГДА ДД.ЭтоВыбытие ТОГДА ЕСТЬNULL(ДопРасходыБезКоличества.КорАналитикаУчетаПартий, ДД.КорАналитикаУчетаПартий)
		...
		|		ЕСТЬNULL(ДопРасходыБезКоличества.КорАналитикаУчетаПартий, ДД.АналитикаУчетаПартий) КАК КорАналитикаУчетаПартий,
		...
		|		ДД.КорНаправлениеДеятельности,
		|		ЕСТЬNULL(ДопРасходыБезКоличества.КорАналитикаУчетаПартий, ДД.АналитикаУчетаПартий),
...
Функция ТекстДвижениеПриходОтклоненийВСтоимостиПартииПрочихРасходов()
	...
	Возврат "
		...
		|		ДД.АналитикаУчетаПартий,
		|		ДД.КорНаправлениеДеятельности КАК НаправлениеДеятельности,
		|		ДД.ВидДеятельностиНДСВыбытия КАК ВидДеятельностиНДС,
		|
		|		ДД.ХозяйственнаяОперация,
		|		ДД.ИдентификаторФинЗаписи,
		|		ДД.НастройкаХозяйственнойОперации,
...
Функция ТекстЗапросаДляРаспределенияДополнительныхРасходов(ПараметрыРасчета)
	...
	ТекстыПодготовкиДанных.Добавить(ТекстКоэффициентыПриведенияБазыРаспределенияДополнительныхРасходов());
	ТекстыПодготовкиДанных.Добавить(ТекстПервичныеПриемникиПриведенная());
...
Функция ШаблонТекстаПервичныеПриемникиПоЗаказам()
	...
	Шаблон = 
		...
		|
		|ГДЕ
		// Условия выборки движений должны соответствовать запросу ВТДвиженияСебестоимостиПоДокументам_1
...
Функция ШаблонТекстаПервичныеПриемникиПоСправочникам()
	...
	Шаблон = 
		...
		|	ИсточникБазы.СтоимостьРеглОстаток          КАК СтоимостьНУ
		|ИЗ
		|	Регистраторы КАК ДД
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ГруппыОбъектовДополнительныхРасходов КАК Группа
...
Функция ДополнительныеТаблицыДляПриемниковПоДокументам()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ИсточникБазы.Период КАК Период,
	|	ИсточникБазы.Регистратор КАК Регистратор,
	|	ИсточникБазы.ИдентификаторСтроки,
	|	ИсточникБазы.ИдентификаторФинЗаписи
	|
	|ПОМЕСТИТЬ ВтСписанияНаРасходы
	|ИЗ
	...
	|	И ИсточникБазы.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	И ИсточникБазы.Период <= &КонецПериода
	|	И ИсточникБазы.Количество <> 0
	|	И ИсточникБазы.ОперацияУчетаСебестоимости = ЗНАЧЕНИЕ(Перечисление.ОперацииУчетаСебестоимости.СписаниеНаРасходыАктивы)
	|	И ИсточникБазы.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеНаРасходыФиксированнаяСтоимость)
	|ИНДЕКСИРОВАТЬ ПО
	|	Период,
	|	Регистратор
	|;
	...
	|
	|ВЫБРАТЬ
	|	""ВТДвиженияСебестоимостиПоДокументам_1"" КАК ЗапросИсточник,
	...
	|	ИсточникБазы.СтоимостьНДД КАК СтоимостьНДД,
	|	ИсточникБазы.СтоимостьРегл + ИсточникБазы.ДопРасходыРегл КАК СтоимостьНУ
	|
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСвязиДокументовАналитикиРасходов КАК Отбор
	|		ПО ИсточникБазы.Регистратор = Отбор.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтСписанияНаРасходы КАК СписанияНаРасходы
	|		ПО СписанияНаРасходы.Период = ИсточникБазы.Период
	|		И СписанияНаРасходы.Регистратор = ИсточникБазы.Регистратор
	|		И СписанияНаРасходы.ИдентификаторСтроки = ИсточникБазы.ИдентификаторСтроки
	|		И СписанияНаРасходы.ИдентификаторФинЗаписи = ИсточникБазы.ИдентификаторФинЗаписи
	|ГДЕ
	|	ИсточникБазы.Организация В (&МассивОрганизаций)
	|	И ИсточникБазы.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И ИсточникБазы.Период < &НачалоПериода
	// Условия выборки движений должны соответствовать запросу ШаблонТекстаПервичныеПриемникиПоЗаказам_1
	|	И (ИсточникБазы.Количество <> 0
	|		ИЛИ ИсточникБазы.ТипЗаписи В (
	|			ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Партия),
	|			ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Потребление),
	...
	//++ НЕ УТ
	|
	// По дкументам выпуска продукции в базу распределения нужно включать только 
	|	И (НЕ ТИПЗНАЧЕНИЯ(ИсточникБазы.Регистратор) В (
	|		ТИП(Документ.ПроизводствоБезЗаказа)
	//++ НЕ УТКА
	|		, ТИП(Документ.ЭтапПроизводства2_2)
	...
	|		ИЛИ ИсточникБазы.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Партия)
	|			И НЕ ТИПЗНАЧЕНИЯ(ИсточникБазы.Партия) = ТИП(Справочник.ПартииПроизводства)
	|		ИЛИ ИсточникБазы.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Дополнение)
	|			И НЕ СписанияНаРасходы.Регистратор ЕСТЬ NULL
	|	)
	//-- НЕ УТ
	...
	// Из проверки исключаем исходные документы регистраторы движений. Это могут быть документы перемещения товаров.
	|		И ИсточникБазы.Регистратор <> Отбор.ИсходныйДокумент
	|	)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	""ВТДвиженияСебестоимостиПоДокументам_2"" КАК ЗапросИсточник,
	|	ИсточникБазы.Регистратор,
	|	Отбор.ИсходныйДокумент КАК ИсходныйДокумент,
...
Функция ТекстКоэффициентыПриведенияБазыРаспределенияДополнительныхРасходов()
	...
	ТекстЗапроса = 
		...
		|	ВЫБОР
		|		КОГДА СУММА(ДД.СтоимостьНУ) > 1000000000
		|	 		ТОГДА 1000000
		|		КОГДА СУММА(ДД.СтоимостьНУ) > 100000000
		|	 		ТОГДА 100000
		|		КОГДА СУММА(ДД.СтоимостьНУ) > 10000000
		|	 		ТОГДА 10000
		|		КОГДА СУММА(ДД.СтоимостьНУ) > 1000000
		|	 		ТОГДА 1000
		|		КОГДА СУММА(ДД.СтоимостьНУ) > 100000
		|	 		ТОГДА 100
		|		КОГДА СУММА(ДД.СтоимостьНУ) > 10000
		|	 		ТОГДА 10
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК КоэффициентНУ,
		|	ВЫБОР
		|		КОГДА СУММА(ДД.СтоимостьНДД) > 1000000000
		|	 		ТОГДА 1000000
		|		КОГДА СУММА(ДД.СтоимостьНДД) > 100000000
		|	 		ТОГДА 100000
		|		КОГДА СУММА(ДД.СтоимостьНДД) > 10000000
		|	 		ТОГДА 10000
		|		КОГДА СУММА(ДД.СтоимостьНДД) > 1000000
		|	 		ТОГДА 1000
		|		КОГДА СУММА(ДД.СтоимостьНДД) > 100000
		|	 		ТОГДА 100
		|		КОГДА СУММА(ДД.СтоимостьНДД) > 10000
		|	 		ТОГДА 10
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК КоэффициентНДД
		|ПОМЕСТИТЬ КоэффициентыПриведенияБазыРаспределения
		|ИЗ
		|	ПервичныеПриемники КАК ДД
		|
		|СГРУППИРОВАТЬ ПО
		|	ДД.Регистратор
		|";
...
Функция ТекстПервичныеПриемникиПриведенная()
	...
	|	ВЫРАЗИТЬ(Источник.Стоимость / КоэффициентыПриведения.Коэффициент КАК ЧИСЛО (23,10)) КАК Стоимость,
	|	ВЫРАЗИТЬ(Источник.СтоимостьБезНДС / КоэффициентыПриведения.КоэффициентБезНДС КАК ЧИСЛО (23,10)) КАК СтоимостьБезНДС,
	|	ВЫРАЗИТЬ(Источник.СтоимостьРегл / КоэффициентыПриведения.КоэффициентРегл КАК ЧИСЛО (23,10)) КАК СтоимостьРегл,
	|	ВЫРАЗИТЬ(Источник.СтоимостьУпр / КоэффициентыПриведения.КоэффициентУпр КАК ЧИСЛО (23,10)) КАК СтоимостьУпр,
	|	ВЫРАЗИТЬ(Источник.СтоимостьНДД / КоэффициентыПриведения.КоэффициентНДД КАК ЧИСЛО (23,10)) КАК СтоимостьНДД,
	|	ВЫРАЗИТЬ(Источник.СтоимостьНУ / КоэффициентыПриведения.КоэффициентНУ КАК ЧИСЛО (23,10)) КАК СтоимостьНУ
	|
	|ПОМЕСТИТЬ ПервичныеПриемникиПриведенная
	|ИЗ
	|	ПервичныеПриемники КАК Источник
	|	ЛЕВОЕ СОЕДИНЕНИЕ КоэффициентыПриведенияБазыРаспределения КАК КоэффициентыПриведения
	|		ПО Источник.Регистратор = КоэффициентыПриведения.Регистратор 
	|";
...
Функция КорректировкаПриемниковПоРегистраторам()
	...
	ТекстЗапроса =
		...
		|	ИЗ
		|		ПервичныеПриемникиПриведенная КАК Источник
		|	ГДЕ
...
Функция ПоказателиБазыРаспределенияПриемниковПрошлыхПериодов()
	...
	ТекстЗапросаВТОстаткиСебестоимостиПредварительная = "
		...
		|		И НЕ РазделУчета В (&ЗабалансовыеРазделыУчета)
		// Доп расходы и отклонения не распределяются на остатки товаров у партнера, т.к. при распределении доп расходов
		// на такие остатки в проводкн по бух учетту нельзя заполнить контрагента и договор (такой информации нет в остатках).
	...
	ТекстЗапросаВТФИФОСписанияИПрочиеВыбытия = "
		...
		|ПОМЕСТИТЬ ПартииПроизводстваДляРаспределения
		|ИЗ
		...
		// Если в качестве аналитики расходов указана партия производства, по которой нет остатков и оборотов в текущем периоде,
		// то списание нужно делать на предопределенную статью "Производственные расходы прошлых периодов",
		// т.к. по указанной аналитике распределение расходов не будет выполнено из-за отсутствия базы распределения.
		// Аналитику расходов в этом случае оставляем пустой.
		|		КОГДА ТИПЗНАЧЕНИЯ(Т.АналитикаРасходов) = ТИП(Справочник.ПартииПроизводства)
		|		  И Т.АналитикаРасходов <> ЗНАЧЕНИЕ(Справочник.ПартииПроизводства.ПустаяСсылка)
		|		  И ПартииПроизводства.ПартияПроизводства ЕСТЬ NULL
		...
		|		И ОтборНоменклатура.Характеристика = ОтборПартии.Характеристика
		//++ НЕ УТ
		|	ЛЕВОЕ СОЕДИНЕНИЕ ПартииПроизводстваДляРаспределения КАК ПартииПроизводства
		|		ПО Т.АналитикаРасходов = ПартииПроизводства.ПартияПроизводства
		//-- НЕ УТ
		|ГДЕ
		|	&РаспределениеДопРасходовПоВыбывшимТоварам
		|	И Т.Период < &НачалоПериода
	...
	ТекстЗапросаПоСредней =
		...
		|		)
		|	И ДД.КорАналитикаУчетаНоменклатуры.ТипМестаХранения <> ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.Партнер)
		|	И ДД.АналитикаУчетаНоменклатуры.Номенклатура = ДД.КорАналитикаУчетаНоменклатуры.Номенклатура
		|	И ДД.АналитикаУчетаНоменклатуры.Характеристика = ДД.КорАналитикаУчетаНоменклатуры.Характеристика
		|;
		|////////////////////////////////////////////////////////////////////////////////
		// При методе оценки стоимости "Средняя за месяц" нужно проверить наличие движений по изменению вида деятельности НДС.
		// При распределении на выбытия товаров в прошлом периоде нужно учитывать наличие изменений вида деятельности НДС.
		// Если изменений вида деятельности не было, доп. расходы должны распределяться только на исходный вид деятельности НДС. 
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДД.Организация КАК Организация,
		|	ДД.ВидЗапасов КАК ВидЗапасов,
		|	ДД.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры
		|
		|ПОМЕСТИТЬ ВТЕстьИзмененияВидаДеятельностиНДС
		|ИЗ
		|	РегистрНакопления.СебестоимостьТоваров КАК ДД
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПриемникиПоТоварамПрошлыхПериодов КАК Отбор
		|		ПО Отбор.МетодОценкиСтоимостиТоваров = ЗНАЧЕНИЕ(Перечисление.МетодыОценкиСтоимостиТоваров.СредняяЗаМесяц)
	...
	ТекстЗапросаПоказателиБазыРаспределенияПриемниковПрошлыхПериодов = "
		...
		|		ПО Источник.Регистратор = Реквизиты.Ссылка
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТЕстьИзмененияВидаДеятельностиНДС КАК ИзменениеВидаДеятельностиНДС
		|		ПО Источник.Организация = ИзменениеВидаДеятельностиНДС.Организация
		|			И Источник.ВидЗапасов = ИзменениеВидаДеятельностиНДС.ВидЗапасов
		|			И Источник.АналитикаУчетаНоменклатуры = ИзменениеВидаДеятельностиНДС.АналитикаУчетаНоменклатуры
		|
		|ГДЕ
		|	Источник.МетодОценкиСтоимостиТоваров = ЗНАЧЕНИЕ(Перечисление.МетодыОценкиСтоимостиТоваров.СредняяЗаМесяц)
		// Вид деятельности должен быть тот же, за исключением случаев изменения вида деятельности НДС.
		|	И (ВозможныеАналитики.ВидДеятельностиНДС = ВыбытиеДетально.ВидДеятельностиНДС
		|		ИЛИ НЕ ИзменениеВидаДеятельностиНДС.АналитикаУчетаНоменклатуры ЕСТЬ NULL)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Регистратор
		|";
...
Функция ТекстДолиДополнительныхРасходов()
	...
	ТекстЗапроса =
		...
		|	ИсточникБазы.СуммаБезНДС КАК ДоляСтоимостиБезНДС,
		|	ИсточникБазы.СуммаРегл КАК ДоляСтоимостиРегл,
		|	ИсточникБазы.СуммаРегл - ИсточникБазы.ПостояннаяРазница - ИсточникБазы.ВременнаяРазница КАК ДоляСтоимостиНУ,
		|	ИсточникБазы.СуммаНДД КАК ДоляСтоимостиНДД,
		|	ИсточникБазы.СуммаУпр КАК ДоляСтоимостиУпр
		|ИЗ
		|	Регистраторы КАК ДД
		|
...
Функция ТекстЗапросаДляРаспределенияДолейПроизводственныхРасходов(ПараметрыРасчета) Экспорт
	...
	ТекстЗапроса = ""
		...
		+ ТекстРаспределениеМеждуПартиямиПредварительная()
		// выборка данных
		+ ТекстОписаниеДанныхДляРаспределенияДолейПроизводственныхРасходов()
...
Процедура ПодготовитьПроизводственныеРасходыКРаспределению(ПараметрыРасчета)
	...
	// Запомним состав временных таблиц.
	СуществующиеВТ = РасчетСебестоимостиПрикладныеАлгоритмы.ПолучитьПереченьСуществующихВременныхТаблиц(ПараметрыРасчета);
	...
	Запрос.Текст = 
		...
		|		КОГДА НЕ НастройкиРаспределенияСтатейНДДПоОВЗ.ВариантРаспределенияРасходовНДД ЕСТЬ NULL
		|			ТОГДА НастройкиРаспределенияСтатейНДДПоОВЗ.ВариантРаспределенияРасходовНДД
		|		КОГДА НЕ НастройкиРаспределенияСтатейНДД.ВариантРаспределенияРасходовНДД ЕСТЬ NULL
		|			ТОГДА НастройкиРаспределенияСтатейНДД.ВариантРаспределенияРасходовНДД
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.ПустаяСсылка) КОНЕЦ) КАК ВариантРаспределенияРасходовНДД,
		|	ДД.Показатель,
		...
		|		ПО ДД.СтатьяРасходов = НастройкиРаспределенияСтатейНДД.СтатьяРасходов
		|			И &ДД_ОбъектРаздельногоУчетаНДД = НастройкиРаспределенияСтатейНДД.ОбъектРаздельногоУчетаНДД
		|			И ДД.Организация = НастройкиРаспределенияСтатейНДД.Организация
		|		ЛЕВОЕ СОЕДИНЕНИЕ НастройкиРаспределенияСтатейНДДПоОВЗ КАК НастройкиРаспределенияСтатейНДДПоОВЗ
		|		ПО ДД.АналитикаРасходов = НастройкиРаспределенияСтатейНДДПоОВЗ.ОВЗ
		|ГДЕ
		...
		|	ДД.АналитикаРасходов,
		|	ДД.Показатель,
		|	ДД.НаправлениеРаспределения КАК НаправлениеРаспределения,
		|	ДД.НаправлениеРаспределения = ЗНАЧЕНИЕ(Перечисление.НаправлениеРаспределенияПоПодразделениям.Нижестоящие) КАК РаспределениеНаНижестоящееПодразделение,
		...
		|	Регистраторы.ВариантРаспределенияРасходовНДД
		...
		|	ДД.ВариантРаспределенияРасходовНДД
	...
	РасчетСебестоимостиЛокализация.ПодставитьВЗапросеТипОбъектаРаздельногоУчетаНДД(Запрос.Текст, "ДД_ОбъектРаздельногоУчетаНДД");
	
	РасчетСебестоимостиПрикладныеАлгоритмы.ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос,
		,,,НСтр("ru = 'Инициализация служебных таблиц для распределения затрат.';
				|en = 'Initializing internal tables for cost allocation.'"));
		
	ПодготовитьДанныеПоНаправлениямРаспределенияПоПодразделениям(Запрос, ПараметрыРасчета);
	
	// Уничтожим вспомогательные временные таблицы.
	НовыеВТ = "Регистраторы,
	|РаспределитьПоПодразделениям,РаспределитьНаДругиеСтатьи,РаспределитьПоЭтапам,ОставитьВНЗП
	|,НастройкиРаспределенияСтатейНДД,НастройкиРаспределенияСтатейНДДПоОВЗ
	|,ВТПодразделенияСНаправлениемРаспределения
	|";
...
// Создает временную таблицу ВТПодразделенияСНаправлениемРаспределения,
// в которой подразделения заполнены по структуре подчиненности,  согласно установленным направлениям распределения.
// Параметры:
//	Запрос - Запрос - Запрос с существующей в МВТ ВТ РаспределитьПоПодразделениям
//	ПараметрыРасчета - Структура - параметры расчета себестоимости
Процедура ПодготовитьДанныеПоНаправлениямРаспределенияПоПодразделениям(Запрос, ПараметрыРасчета)
	
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 0
	|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КАК ПодразделениеГруппы,
	|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КАК Подразделение,
	|	ЗНАЧЕНИЕ(Перечисление.НаправлениеРаспределенияПоПодразделениям.ПустаяСсылка) КАК НаправлениеРаспределения
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РПП.НаправлениеРаспределения КАК НаправлениеРаспределения,
	|	РПП.Подразделение КАК Подразделение
	|ИЗ
	|	РаспределитьПоПодразделениям КАК РПП
	|ГДЕ
	|	РПП.РаспределениеНаНижестоящееПодразделение";
	
	РезультатЗапросаПакет = Запрос.ВыполнитьПакет();
	ПодразделенияСНаправлениемРаспределения = РезультатЗапросаПакет[0].Выгрузить();
	ПервичныеДанныеПоПодразделениям = РезультатЗапросаПакет[1].Выбрать();
	
	НаправлениеРаспределенияТекПодразделение = Перечисления.НаправлениеРаспределенияПоПодразделениям.Текущее;
	Пока ПервичныеДанныеПоПодразделениям.Следующий() Цикл
		Если ПервичныеДанныеПоПодразделениям.НаправлениеРаспределения = НаправлениеРаспределенияТекПодразделение Тогда
			НоваяСтрока = ПодразделенияСНаправлениемРаспределения.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ПервичныеДанныеПоПодразделениям);
			Продолжить;
		КонецЕсли;
		
		ПодходящиеПодразделения = Документы.РаспределениеПрочихЗатрат.ПодразделенияПоНаправлению(
			ПервичныеДанныеПоПодразделениям.Подразделение,
			ПервичныеДанныеПоПодразделениям.НаправлениеРаспределения
		);
			
		Для Каждого ПодходящиееПодразделение Из ПодходящиеПодразделения Цикл
			НоваяСтрока = ПодразделенияСНаправлениемРаспределения.Добавить();
			НоваяСтрока.ПодразделениеГруппы = ПервичныеДанныеПоПодразделениям.Подразделение;
			НоваяСтрока.НаправлениеРаспределения = ПервичныеДанныеПоПодразделениям.НаправлениеРаспределения;
			НоваяСтрока.Подразделение = ПодходящиееПодразделение;	
		КонецЦикла;
	КонецЦикла;
	
	РасчетСебестоимостиПрикладныеАлгоритмы.ПоместитьТаблицуЗначенийВоВременнуюТаблицу(
		ПараметрыРасчета, 
		"ВТПодразделенияСНаправлениемРаспределения", ПодразделенияСНаправлениемРаспределения, 
		"%1ПодразделениеГруппы, %1Подразделение, %1НаправлениеРаспределения",
		"ПодразделениеГруппы, Подразделение, НаправлениеРаспределения"
	);
...
Функция ТекстДанныеПоФактическимМатериальнымЗатратам(ПараметрыРасчета = Неопределено)
	...
	ТекстЗапроса = 
		...
		|		КОНЕЦ)) КАК ЧИСЛО(31, 10))                           КАК СтоимостьНУ,
		//-- Локализация
		...
		|		КОНЕЦ)) КАК ЧИСЛО(31, 10))                           КАК СтоимостьНДД,
		|	СУММА(ДД.Количество)                                     КАК Количество,
		...
		|		ПО ПартииПроизводства.ПартияПроизводства = ДД.КорПартия
		...
		//++ Локализация
		|			ИЛИ (ДД.СтоимостьРегл + ДД.ДопРасходыРегл - ДД.ПостояннаяРазница - ДД.ВременнаяРазница) > 0
		//-- Локализация
		|			ИЛИ ДД.СтоимостьНДД > 0
		|			)
		|
...
Функция ТекстДанныеПоПредварительнымМатериальнымЗатратам(ПараметрыРасчета)
	...
	ТекстЗапроса = 
		...
		|	СРЕДНЕЕ(ДД.СтоимостьРегл + ДД.ДопРасходыРегл - ДД.ПостояннаяРазница - ДД.ВременнаяРазница) КАК СтоимостьНУ,
		//-- Локализация
		|	СРЕДНЕЕ(ДД.СтоимостьУпр)              КАК СтоимостьУпр,
		|	СРЕДНЕЕ(ДД.ДопРасходыУпр)             КАК ДопРасходыУпр,
		|	СРЕДНЕЕ(ДД.СтоимостьНДД)              КАК СтоимостьНДД
...
Функция ШаблонТекстаБазыРаспределенияПоПартиям(ПоказательБазыРаспределения)
	...
	Шаблон = 
		...
		//++ Локализация
		|	ВЫРАЗИТЬ(СУММА(&ЗначениеНУ) КАК ЧИСЛО(31,10))		КАК СтоимостьНУ,
		//-- Локализация
		|	ВЫРАЗИТЬ(СУММА(&ЗначениеНДД) КАК ЧИСЛО(31,10))		КАК СтоимостьНДД
		|ПОМЕСТИТЬ ТаблицаДанных
		|ИЗ
	...
	Шаблон = СтрЗаменить(Шаблон,
		"&ЗначениеНУ",
		?(ПараметрыБазы.Свойство("ЗначениеНУ"), ПараметрыБазы.ЗначениеНУ, ПараметрыБазы.ЗначениеСНДС));
	//-- Локализация
	Шаблон = СтрЗаменить(Шаблон,
		"&ЗначениеНДД",
		?(ПараметрыБазы.Свойство("ЗначениеНДД"), ПараметрыБазы.ЗначениеНДД, ПараметрыБазы.ЗначениеСНДС));
	Шаблон = СтрЗаменить(Шаблон, 
		"&УсловиеЗапроса", 
...
Функция ТекстБазаРаспределенияДолейПоЭтапам(ПараметрыРасчета)
	...
	ГотовыеТекстыЗапросов = Новый Массив;
	ГотовыеТекстыЗапросов.Добавить(РасчетСебестоимостиЛокализация.ТекстЗапросаНастроекНДД("", "РасчетСебестоимостиПостатейныеЗатраты_ТекстБазаРаспределенияДолейПоЭтапам"));
	ГотовыеТекстыЗапросов.Добавить(ТекстПомещенияГруппыОбъектовВоВременнуюТаблицу(ИмяВременнойТаблицыГруппыОбъектов));
	ГотовыеТекстыЗапросов.Добавить(ТекстЗапросаОписаниеДанных);
	ДополнитьТекстамиПоОписаниюГрупп(ГотовыеТекстыЗапросов, ОписаниеГрупп, ИмяВременнойТаблицыГруппыОбъектов);
	...
	ШаблонТекстаЗапроса = 
		...
		|		ПО Т.ПартияПроизводства.Организация = НастройкиПризнанияРасходовНДД.Организация
		|		И &Т_ОбъектРаздельногоУчетаНДД = НастройкиПризнанияРасходовНДД.ОбъектРаздельногоУчетаНДД
		|		И НастройкиПризнанияРасходовНДД.РасходыВключаютсяВСтоимость
		|";
		
	РасчетСебестоимостиЛокализация.ПодставитьВЗапросеТипОбъектаРаздельногоУчетаНДД(ШаблонТекстаЗапроса, "Т_ОбъектРаздельногоУчетаНДД");
	...
	ТекстРаспределяемыеВручную = 
		...
		//-- Локализация
		|	
		// По НДД необходимо проверить настройки признания и обнулить базу, если по настройкам признания расходы не включаются в стоимость
		|	ВЫБОР
		|		КОГДА НастройкиПризнанияРасходовНДД.ВариантПризнанияРасходов ЕСТЬ НЕ NULL
		...
		|			И Данные.ПартияПроизводства = Аналитики.ПартияПроизводства
		|	ЛЕВОЕ СОЕДИНЕНИЕ НастройкиПризнанияРасходовНДД КАК НастройкиПризнанияРасходовНДД
		|		ПО Аналитики.Организация = НастройкиПризнанияРасходовНДД.Организация
		|		И &ТаблицаИсточник_ОбъектРаздельногоУчетаНДД = НастройкиПризнанияРасходовНДД.ОбъектРаздельногоУчетаНДД
		|		И НастройкиПризнанияРасходовНДД.РасходыВключаютсяВСтоимость
		|";
	
	РасчетСебестоимостиЛокализация.ПодставитьВЗапросеТипОбъектаРаздельногоУчетаНДД(ТекстРаспределяемыеВручную, "", "Данные", "Аналитики");
...
Функция ТекстБазаПоЭтапамСводно()
	...
	ТекстЗапроса = 
		...
		//++ Локализация
		|	ВЫРАЗИТЬ(СУММА(ДД.СтоимостьНУ) / МАКСИМУМ(КоэффициентыПриведения.КоэффициентНУ) КАК ЧИСЛО (23,10)) КАК СтоимостьНУ,
		//-- Локализация
		|	ВЫРАЗИТЬ(СУММА(ДД.СтоимостьНДД) / МАКСИМУМ(КоэффициентыПриведения.КоэффициентНДД) КАК ЧИСЛО (23,10)) КАК СтоимостьНДД
...
Функция ТекстИтогиБазыПоЭтапам()
	...
	|	СУММА(ДД.СтоимостьУпр) КАК СтоимостьУпр,
	//++ Локализация
	|	СУММА(ДД.СтоимостьНУ) КАК СтоимостьНУ,
	//-- Локализация
	|	СУММА(ДД.СтоимостьНДД) КАК СтоимостьНДД
	...
	//++ Локализация
	|	ИЛИ СУММА(ДД.СтоимостьНУ) > 0
	//-- Локализация
	|	ИЛИ СУММА(ДД.СтоимостьНДД) > 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ШаблонТекстаБазыРаспределенияПоПодразделениям(ПоказательБазыРаспределения)
	...
	Шаблон = СтрЗаменить(Шаблон,
		"&ЗначениеНУ",
		СтрЗаменить(?(ПараметрыБазы.Свойство("ЗначениеНУ"),
			ПараметрыБазы.ЗначениеНУ,
			ПараметрыБазы.ЗначениеСНДС), "БазаРаспределенияПоПартиям", "БазаРаспределенияПоПодразделениям"));
	//-- Локализация
	Шаблон = СтрЗаменить(Шаблон,
		"&ЗначениеНДД",
		СтрЗаменить(?(ПараметрыБазы.Свойство("ЗначениеНДД"),
			ПараметрыБазы.ЗначениеНДД,
			ПараметрыБазы.ЗначениеСНДС), "БазаРаспределенияПоПартиям", "БазаРаспределенияПоПодразделениям"));
	Шаблон = СтрЗаменить(Шаблон, 
		"&УсловиеЗапроса", 
		?(ПараметрыБазы.Свойство("УсловиеЗапроса"), ПараметрыБазы.УсловиеЗапроса, "ИСТИНА"));
...
Функция ПараметрыБазыРаспределения(ПоказательБазыРаспределения)
	...
	Если ПоказательБазыРаспределения = "МатериальныеЗатраты" Тогда
		...
		Параметры.Вставить("ЗначениеУпр", "ВЫБОР ДД.БазаРаспределенияПоПартиям
											...
											|КОНЕЦ");
		//++ Локализация
		Параметры.Вставить("ЗначениеНУ", "ВЫБОР ДД.БазаРаспределенияПоПартиям
											|	КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.СуммаМатериальныхЗатрат)
											|		ТОГДА ИсточникБазы.СтоимостьНУ
											|	КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.СуммаМатериальныхИТрудозатрат)
											|		ТОГДА ИсточникБазы.СтоимостьНУ
											|	КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ВесУказанныхМатериалов)
											|		ТОГДА ИсточникБазы.Вес
											...
											|КОНЕЦ");
		//-- Локализация
		Параметры.Вставить("ЗначениеНДД", "ВЫБОР ДД.БазаРаспределенияПоПартиям
											|	КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.СуммаМатериальныхЗатрат)
											|		ТОГДА ИсточникБазы.СтоимостьНДД
											|	КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.СуммаМатериальныхИТрудозатрат)
											|		ТОГДА ИсточникБазы.СтоимостьНДД
											|	КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ВесУказанныхМатериалов)
											|		ТОГДА ИсточникБазы.Вес
...
Функция ТекстБазаРаспределенияДолейПоПодразделениям(ПараметрыРасчета)
	...
	ТекстРаспределяемыеВручную = 
		...
		|	Данные.ДоляСтоимости 		КАК СтоимостьНУ,
		//-- Локализация
		|	Данные.ДоляСтоимости 		КАК СтоимостьНДД
		|ИЗ
		|	РаспределитьПоПодразделениям КАК ДД
	...
	ТекстПоПоказателям = 
		...
		//++ Локализация
		|	Данные.ЗначениеПоказателя	КАК СтоимостьНУ,
		//-- Локализация
		|	Данные.ЗначениеПоказателя	КАК СтоимостьНДД
		|ИЗ
		|	РаспределитьПоПодразделениям КАК ДД
		|		ЛЕВОЕ СОЕДИНЕНИЕ ГруппыОбъектовПоНаправлениюРаспределения КАК ГруппаНР
		|		ПО ДД.Регистратор = ГруппаНР.Объект
		...
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПодразделенияСНаправлениемРаспределения КАК ПНР
		|		ПО ДД.РаспределениеНаНижестоящееПодразделение
		|			И ДД.НаправлениеРаспределения = ПНР.НаправлениеРаспределения
		|			И Данные.Подразделение = ПНР.Подразделение
		|			И ДД.Подразделение = ПНР.ПодразделениеГруппы
		|
		|ГДЕ
		...
		|	И (НЕ ДД.РаспределениеНаНижестоящееПодразделение ИЛИ НЕ ПНР.Подразделение ЕСТЬ NULL)";
...
Функция ТекстБазаПоПодразделениямСводно()
	...
	ТекстЗапроса =
		...
		|			И ИтогиПоЭтапам.НаправлениеДеятельности = НЕОПРЕДЕЛЕНО
		|
		|СГРУППИРОВАТЬ ПО
		|	ДД.Регистратор
		|
		...
		//++ Локализация
		|	СУММА(ДД.СтоимостьНУ) / МАКСИМУМ(Коэффициенты.КоэффициентНУ)			КАК СтоимостьНУ,
		//-- Локализация
		|	СУММА(ДД.СтоимостьНДД) / МАКСИМУМ(Коэффициенты.КоэффициентНДД)			КАК СтоимостьНДД
...
Функция ТекстРаспределениеМеждуПартиямиПредварительная()

	Возврат "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПоПартиям.Дата,
	|	ПоПартиям.Регистратор КАК Регистратор,
	|	ПоПартиям.Организация,
	|	ПоПартиям.Подразделение,
	|	ПоПартиям.НаправлениеДеятельности,
	|	ПоПартиям.СтатьяРасходов,
	|	ПоПартиям.АналитикаРасходов,
	|	ПоПартиям.ВариантРаспределенияРасходовУпр,
	|	ПоПартиям.ВариантРаспределенияРасходовРегл,
	|	ПоПартиям.ВариантРаспределенияРасходовНУ,
	|	ПоПартиям.ВариантРаспределенияРасходовНДД,
	|	Регистраторы.УправленческийУчет,
	|	Регистраторы.РегламентированныйУчет,
	|	Регистраторы.НалоговыйУчет,
	|	Регистраторы.НДД,
	|	Регистраторы.ПартииУказаныВручную,
	|	Регистраторы.НаЛюбыеНаправлениеДеятельности,
	|	ВЫБОР
	|		КОГДА ПоПартиям.НаправлениеДеятельности = ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|		  ИЛИ Регистраторы.ПартииУказаныВручную
	|		  ИЛИ Регистраторы.НаЛюбыеНаправлениеДеятельности
	|		ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК БезУчетаНаправления
	|ПОМЕСТИТЬ РаспределитьПоЭтапамСРегистраторами
	|ИЗ
	|	РаспределитьПоЭтапам КАК ПоПартиям
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Регистраторы КАК Регистраторы
	|		ПО	Регистраторы.Организация = ПоПартиям.Организация
	|		И	Регистраторы.Регистратор = ПоПартиям.Регистратор
	...
	|	ПоПартиям.ВариантРаспределенияРасходовНУ,
	|	ПоПартиям.ВариантРаспределенияРасходовНДД,
	|	ПоПартиям.УправленческийУчет,
	|	ПоПартиям.РегламентированныйУчет,
	|	ПоПартиям.НалоговыйУчет,
	|	ПоПартиям.НДД,
	|	ПоПартиям.ПартииУказаныВручную,
	|	ПоПартиям.НаЛюбыеНаправлениеДеятельности,
	|
	|	БазаПоЭтапам.ГруппаПродукции,
	|	БазаПоЭтапам.ПравилоОтнесенияНаВыпуск,
	|	БазаПоЭтапам.Подразделение КАК КорПодразделение,
	|	БазаПоЭтапам.НаправлениеДеятельности КАК КорНаправлениеДеятельности,
	|	БазаПоЭтапам.ПартияПроизводства,
	|	БазаПоЭтапам.ЗаказНаПроизводство,
	|	БазаПоЭтапам.КодСтрокиПродукция,
	|	БазаПоЭтапам.СтатьяКалькуляции,
	|
	|	ВЫБОР
			// партии указываются вручную, итог базы используется без учета направлений деятельности (НД)
	|	  КОГДА ПоПартиям.ПартииУказаныВручную
			// в документе указано "На партии любых направлений деятельности", итог базы используется без учета НД
	|		ИЛИ НЕ ПоПартиям.НаправлениеДеятельности = ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) 
	|			И ПоПартиям.НаЛюбыеНаправлениеДеятельности 
			// у распределяемого расхода пустое НД, итог базы используется без учета НД
	|		ИЛИ ПоПартиям.НаправлениеДеятельности = ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) 
	|			ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК БезУчетаНаправления,
	|
	|	СУММА(БазаПоЭтапам.Стоимость) КАК Стоимость,
	|	СУММА(БазаПоЭтапам.СтоимостьБезНДС) КАК СтоимостьБезНДС,
	|	СУММА(БазаПоЭтапам.СтоимостьРегл) КАК СтоимостьРегл,
	|	СУММА(БазаПоЭтапам.СтоимостьУпр) КАК СтоимостьУпр
	|ПОМЕСТИТЬ РаспределитьПоЭтапамСБазой
	|ИЗ
	|	РаспределитьПоЭтапамСРегистраторами КАК ПоПартиям
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ БазаПоЭтапам КАК БазаПоЭтапам
	|		ПО	ПоПартиям.Регистратор = БазаПоЭтапам.Регистратор
	|			И (ПоПартиям.БезУчетаНаправления
	|				ИЛИ ПоПартиям.НаправлениеДеятельности = БазаПоЭтапам.НаправлениеДеятельности)
	|СГРУППИРОВАТЬ ПО
	|	ПоПартиям.Дата,
	|	ПоПартиям.Регистратор,
	|	ПоПартиям.Организация,
	|	ПоПартиям.Подразделение,
	|	ПоПартиям.НаправлениеДеятельности,
	|	ПоПартиям.СтатьяРасходов,
	...
	|	ПоПартиям.НаЛюбыеНаправлениеДеятельности,
	|
	|	ПоПартиям.ГруппаПродукции,
	|	ПоПартиям.ПравилоОтнесенияНаВыпуск,
	|	ПоПартиям.КорПодразделение КАК КорПодразделение,
	|	ПоПартиям.КорНаправлениеДеятельности,
	|	ПоПартиям.ПартияПроизводства,
	|	ПоПартиям.ЗаказНаПроизводство,
	|	ПоПартиям.КодСтрокиПродукция,
	|	ПоПартиям.СтатьяКалькуляции,
	|
	|	ПоПартиям.Стоимость,
	|	ПоПартиям.СтоимостьБезНДС,
	|	ПоПартиям.СтоимостьРегл,
	|	ПоПартиям.СтоимостьУпр,
	|
	|	СУММА(ВЫРАЗИТЬ(ВЫБОР 
	|		КОГДА ИтогиБазыПоЭтапамПодразделений.Стоимость = 0
	|			ТОГДА 0
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ(ПоПартиям.Стоимость КАК ЧИСЛО(28, 10)) / ВЫРАЗИТЬ(ИтогиБазыПоЭтапамПодразделений.Стоимость КАК ЧИСЛО(28, 10))
	|	КОНЕЦ КАК ЧИСЛО (28, 10))) КАК ДоляСтоимости,
	|	СУММА(ВЫРАЗИТЬ(ВЫБОР 
	|		КОГДА ИтогиБазыПоЭтапамПодразделений.СтоимостьБезНДС = 0
	|			ТОГДА 0
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ(ПоПартиям.СтоимостьБезНДС КАК ЧИСЛО(28, 10)) / ВЫРАЗИТЬ(ИтогиБазыПоЭтапамПодразделений.СтоимостьБезНДС КАК ЧИСЛО(28, 10))
	|	КОНЕЦ КАК ЧИСЛО (28, 10))) КАК ДоляСтоимостиБезНДС,
	|	СУММА(ВЫРАЗИТЬ(ВЫБОР 
	|		КОГДА ИтогиБазыПоЭтапамПодразделений.СтоимостьРегл = 0
	|			ТОГДА 0
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ(ПоПартиям.СтоимостьРегл КАК ЧИСЛО(28, 10)) / ВЫРАЗИТЬ(ИтогиБазыПоЭтапамПодразделений.СтоимостьРегл КАК ЧИСЛО(28, 10))
	|	КОНЕЦ КАК ЧИСЛО (28, 10))) КАК ДоляСтоимостиРегл,
	|	СУММА(ВЫРАЗИТЬ(ВЫБОР 
	|		КОГДА ИтогиБазыПоЭтапамПодразделений.СтоимостьУпр = 0
	|			ТОГДА 0
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ(ПоПартиям.СтоимостьУпр КАК ЧИСЛО(28, 10)) / ВЫРАЗИТЬ(ИтогиБазыПоЭтапамПодразделений.СтоимостьУпр КАК ЧИСЛО(28, 10))
	|	КОНЕЦ КАК ЧИСЛО (28, 10))) КАК ДоляСтоимостиУпр
	|ПОМЕСТИТЬ РаспределитьПоЭтапамСБазойПодразделений
	|ИЗ
	|	РаспределитьПоЭтапамСБазой КАК ПоПартиям
	...
	|			И ПоПартиям.КорПодразделение = ИтогиБазыПоЭтапамПодразделений.Подразделение
	|			И (
	|				ПоПартиям.БезУчетаНаправления И ИтогиБазыПоЭтапамПодразделений.НаправлениеДеятельности = НЕОПРЕДЕЛЕНО 
					// у распределяемого расхода непустое НД, итог базы используется с учетом НД
	|				ИЛИ НЕ ПоПартиям.БезУчетаНаправления 
	|					И ПоПартиям.НаправлениеДеятельности = ИтогиБазыПоЭтапамПодразделений.НаправлениеДеятельности)
	|
	|СГРУППИРОВАТЬ ПО
	|	ПоПартиям.Дата,
	|	ПоПартиям.Регистратор,
...
Функция ТекстНераспределяемыеДоли() // использует ОставитьВНЗП, ИтогиБазыПоЭтапамВсехПодразделений
	...
	|	КОНЕЦ																	КАК ДоляСтоимостиРегл,
	|	ВЫБОР 
	|		КОГДА НЕ ДД.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
	|			ИЛИ НЕ Регистраторы.НалоговыйУчет
	|			ТОГДА 0
	|		ИНАЧЕ 
	|			ВЫРАЗИТЬ(ДД.ДоляСтоимости 
	|				* ЕСТЬNULL(ИтогиБазыПоПодразделениям.СтоимостьРегл, 
	...
	|	КОНЕЦ																	КАК ДоляСтоимостиУпр,
	// Доля стоимости для НДД рассчитывается по базе распределения регламентированного учета.
	|	ВЫБОР 
	|		КОГДА НЕ ДД.ВариантРаспределенияРасходовНДД = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
	|			ИЛИ НЕ Регистраторы.НДД
	|			ТОГДА 0
	// Если по настройкам признания расходы по НДД не включается в стоимость, то базу обнуляем.
	|		КОГДА НастройкиПризнанияРасходовНДДпоПодразделениям.ВариантПризнанияРасходов ЕСТЬ NULL
	|				И НастройкиПризнанияРасходовНДДпоНаправлениямДеятельности.ВариантПризнанияРасходов ЕСТЬ NULL
	|			ТОГДА 0
	|		ИНАЧЕ 
	|			ВЫРАЗИТЬ(ДД.ДоляСтоимости 
	|				* ЕСТЬNULL(ИтогиБазыПоПодразделениям.СтоимостьРегл, 
	...
	|		ПО ДД.Подразделение = НастройкиПризнанияРасходовНДДпоПодразделениям.ОбъектРаздельногоУчетаНДД
	|			И ДД.Организация = НастройкиПризнанияРасходовНДДпоПодразделениям.Организация
	|			И НастройкиПризнанияРасходовНДДпоПодразделениям.РасходыВключаютсяВСтоимость
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ НастройкиПризнанияРасходовНДД КАК НастройкиПризнанияРасходовНДДпоНаправлениямДеятельности
	|		ПО ДД.НаправлениеДеятельности = НастройкиПризнанияРасходовНДДпоНаправлениямДеятельности.ОбъектРаздельногоУчетаНДД
	|			И ДД.Организация = НастройкиПризнанияРасходовНДДпоНаправлениямДеятельности.Организация
	|			И НастройкиПризнанияРасходовНДДпоНаправлениямДеятельности.РасходыВключаютсяВСтоимость
	|";
...
Функция ТекстРаспределениеМеждуПартиями()
	...
	|	ПоПартиям.ГруппаПродукции                                            	КАК ГруппаПродукции,
	|	ПоПартиям.ПравилоОтнесенияНаВыпуск                                   	КАК ПравилоОтнесенияНаВыпуск,
	|	ВЫРАЗИТЬ(ВЫБОР 
	...
	|			ИЛИ НЕ ПоПартиям.УправленческийУчет
	|			ТОГДА 0
	|		КОГДА ПоПартиям.ДоляСтоимости = 0
	...
	|				И ПоПартиям.РегламентированныйУчет)
	|			ТОГДА 0
	|		КОГДА ПоПартиям.ДоляСтоимостиРегл = 0
	|			ИЛИ ИтогиБазыПоПодразделениям.СтоимостьРегл = 0
	|			ТОГДА 0
	|		КОГДА ПоПодразделениям.Регистратор ЕСТЬ NULL
	|			ТОГДА ПоПартиям.СтоимостьРегл
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ(ЕСТЬNULL(БазаПоПодразделениям.СтоимостьРегл, 0) КАК ЧИСЛО(28, 10)) 
	|			* ПоПартиям.ДоляСтоимостиРегл
	|	КОНЕЦ КАК ЧИСЛО(28, 10)) КАК ДоляСтоимостиРегл,
	|	ВЫРАЗИТЬ(ВЫБОР 
	|		КОГДА НЕ (ПоПартиям.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
	|				И ПоПартиям.НалоговыйУчет)
	|			ТОГДА 0
	|		КОГДА ПоПартиям.СтоимостьРегл = 0
	|			ИЛИ ИтогиБазыПоПодразделениям.СтоимостьРегл = 0
	|			ТОГДА 0
	|		КОГДА ПоПодразделениям.Регистратор ЕСТЬ NULL
	|			ТОГДА ПоПартиям.СтоимостьРегл
	...
	|			ВЫРАЗИТЬ(ЕСТЬNULL(БазаПоПодразделениям.СтоимостьРегл, 0) КАК ЧИСЛО(28, 10)) 
	|			* ПоПартиям.ДоляСтоимостиРегл
	|	КОНЕЦ КАК ЧИСЛО(28, 10)) КАК ДоляСтоимостиНУ,
	|	ВЫРАЗИТЬ(ВЫБОР 
	|		КОГДА НЕ ПоПартиям.ВариантРаспределенияРасходовУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
	|			ИЛИ НЕ ПоПартиям.УправленческийУчет
	|			ТОГДА 0
	|		КОГДА ПоПартиям.ДоляСтоимостиУпр = 0
	|			ИЛИ ИтогиБазыПоПодразделениям.СтоимостьУпр = 0
	|			ТОГДА 0
	|		КОГДА ПоПодразделениям.Регистратор ЕСТЬ NULL
	|			ТОГДА ПоПартиям.СтоимостьУпр
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ(ЕСТЬNULL(БазаПоПодразделениям.СтоимостьУпр, 0) КАК ЧИСЛО(28, 10)) 
	|			* ПоПартиям.ДоляСтоимостиУпр
	|	КОНЕЦ КАК ЧИСЛО(28, 10)) КАК ДоляСтоимостиУпр,
	|	ВЫРАЗИТЬ(
	// Доля стоимости для НДД рассчитывается по базе распределения регламентированного учета.
	|	ВЫБОР 
	|		КОГДА НЕ (
	|			ПоПартиям.ВариантРаспределенияРасходовНДД = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
	|				И ПоПартиям.НДД
	|			)
	|			ТОГДА 0
	// Если по настройкам признания расходы по НДД не включается в стоимость, то базу обнуляем.
	...
	|			ВЫРАЗИТЬ(ЕСТЬNULL(БазаПоПодразделениям.СтоимостьРегл, 0) КАК ЧИСЛО(28, 10)) 
	|			* ПоПартиям.ДоляСтоимостиРегл
	|	КОНЕЦ КАК ЧИСЛО(28, 10)) КАК ДоляСтоимостиНДД,
	|
	|	НЕОПРЕДЕЛЕНО                                                            КАК КорСтатьяРасходов,
	|	НЕОПРЕДЕЛЕНО                                                            КАК КорАналитикаРасходов,
	|	НЕОПРЕДЕЛЕНО                                                            КАК КорСтатьяАктивовПассивов,
	|	НЕОПРЕДЕЛЕНО                                                            КАК КорАналитикаАктивовПассивов,
	|	ПоПартиям.КорПодразделение                                              КАК КорПодразделение,
	|	ПоПартиям.КорНаправлениеДеятельности                                    КАК КорНаправлениеДеятельности,
	|	ПоПартиям.ПартияПроизводства                                         	КАК ПартияПроизводства,
	|	ПоПартиям.ЗаказНаПроизводство                                        	КАК ЗаказНаПроизводство,
	|	ПоПартиям.КодСтрокиПродукция                                         	КАК КодСтрокиПродукция,
	|	ВЫБОР
	...
	|	НЕОПРЕДЕЛЕНО                                                            КАК ИдентификаторСтроки
	|ИЗ
	|	РаспределитьПоЭтапамСБазойПодразделений КАК ПоПартиям
	...
	|		ПО ПоПартиям.Регистратор = БазаПоПодразделениям.Регистратор
	|			И ПоПартиям.КорПодразделение = БазаПоПодразделениям.КорПодразделение
	|	ЛЕВОЕ СОЕДИНЕНИЕ ИтогиБазыПоПодразделениям КАК ИтогиБазыПоПодразделениям
	|		ПО ИтогиБазыПоПодразделениям.Регистратор = ПоПодразделениям.Регистратор
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ НастройкиПризнанияРасходовНДД КАК НастройкиПризнанияРасходовНДДпоПодразделениям
	|		ПО ПоПартиям.КорПодразделение = НастройкиПризнанияРасходовНДДпоПодразделениям.ОбъектРаздельногоУчетаНДД
	|			И ПоПартиям.Организация = НастройкиПризнанияРасходовНДДпоПодразделениям.Организация
	|			И НастройкиПризнанияРасходовНДДпоПодразделениям.РасходыВключаютсяВСтоимость
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ НастройкиПризнанияРасходовНДД КАК НастройкиПризнанияРасходовНДДпоНаправлениямДеятельности
	|		ПО ПоПартиям.КорНаправлениеДеятельности = НастройкиПризнанияРасходовНДДпоНаправлениямДеятельности.ОбъектРаздельногоУчетаНДД
	|			И ПоПартиям.Организация = НастройкиПризнанияРасходовНДДпоНаправлениямДеятельности.Организация
	|			И НастройкиПризнанияРасходовНДДпоНаправлениямДеятельности.РасходыВключаютсяВСтоимость
	|";
...
Функция ТекстЗапросаБазыРаспределенияПрямыхРасходовНаПроизводство() Экспорт
	...
	//++ Локализация
	|	Т.НормативнаяСтоимость КАК СтоимостьНУ,
	//-- Локализация
	|	Т.НормативнаяСтоимость КАК СтоимостьНДД
	|ИЗ ВтГруппыРаспределения КАК ДД
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеПоТрудозатратам КАК Т
	|ПО 
	|	ДД.ПартияПроизводства = Т.ПартияПроизводства
	...
	//++ Локализация
	|	Т.Количество КАК СтоимостьНУ,
	//-- Локализация
	|	Т.Количество КАК СтоимостьНДД
	|ИЗ ВтГруппыРаспределения КАК ДД
	...
	//++ Локализация
	|	Т.Количество + Т.КоличествоПлан КАК СтоимостьНУ,
	//-- Локализация
	|	Т.Количество + Т.КоличествоПлан КАК СтоимостьНДД
	|ИЗ ВтГруппыРаспределения КАК ДД
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеПоПродукции КАК Т
	|ПО 
	|	ДД.ПартияПроизводства = Т.ПартияПроизводства
	...
	//++ Локализация
	|	Т.Вес КАК СтоимостьНУ,
	//-- Локализация
	|	Т.Вес КАК СтоимостьНДД
	|ИЗ ВтГруппыРаспределения КАК ДД
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеПоПродукции КАК Т
	|ПО 
	|	ДД.ПартияПроизводства = Т.ПартияПроизводства
	...
	//++ Локализация
	|	Т.Вес + Т.ВесПлан КАК СтоимостьНУ,
	//-- Локализация
	|	Т.Вес + Т.ВесПлан КАК СтоимостьНДД
	|ИЗ ВтГруппыРаспределения КАК ДД
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеПоПродукции КАК Т
	|ПО 
	|	ДД.ПартияПроизводства = Т.ПартияПроизводства
	...
	//++ Локализация
	|	Т.Объем КАК СтоимостьНУ,
	//-- Локализация
	|	Т.Объем КАК СтоимостьНДД
	|ИЗ ВтГруппыРаспределения КАК ДД
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеПоПродукции КАК Т
	|ПО 
	|	ДД.ПартияПроизводства = Т.ПартияПроизводства
	...
	//++ Локализация
	|	Т.Объем + Т.ОбъемПлан КАК СтоимостьНУ,
	//-- Локализация
	|	Т.Объем + Т.ОбъемПлан КАК СтоимостьНДД
	|ИЗ ВтГруппыРаспределения КАК ДД
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеПоПродукции КАК Т
	|ПО 
	|	ДД.ПартияПроизводства = Т.ПартияПроизводства
	...
	|	Т.Стоимость КАК Стоимость,
	|	Т.Стоимость КАК СтоимостьБезНДС,
	|	Т.Стоимость КАК СтоимостьРегл,
	|	Т.Стоимость КАК СтоимостьУпр,
	//++ Локализация
	|	Т.Стоимость КАК СтоимостьНУ,
	//-- Локализация
	|	Т.Стоимость КАК СтоимостьНДД
	|ИЗ ВтГруппыРаспределения КАК ДД
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеПоПродукции КАК Т
	...
	|	Т.ГруппаПродукции,
	|	Т.Продукция,
	|	Т.Стоимость + Т.СтоимостьПлан КАК Стоимость,
	|	Т.Стоимость + Т.СтоимостьПлан КАК СтоимостьБезНДС,
	|	Т.Стоимость + Т.СтоимостьПлан КАК СтоимостьРегл,
	|	Т.Стоимость + Т.СтоимостьПлан КАК СтоимостьУпр,
	//++ Локализация
	|	Т.Стоимость + Т.СтоимостьПлан КАК СтоимостьНУ,
	//-- Локализация
	|	Т.Стоимость + Т.СтоимостьПлан КАК СтоимостьНДД
	|ИЗ ВтГруппыРаспределения КАК ДД
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеПоПродукции КАК Т
	|ПО 
	|	ДД.ПартияПроизводства = Т.ПартияПроизводства
	...
	//++ Локализация
	|	ВЫРАЗИТЬ(СУММА(ДД.СтоимостьНУ) / МАКСИМУМ(КоэффициентыПриведения.КоэффициентНУ) КАК ЧИСЛО (23,10)) КАК БазаНУ,
	//-- Локализация
	|	ВЫРАЗИТЬ(СУММА(ДД.СтоимостьНДД) / МАКСИМУМ(КоэффициентыПриведения.КоэффициентНДД) КАК ЧИСЛО (23,10)) КАК БазаНДД
	|ПОМЕСТИТЬ ВтБазыПрямыхРасходов
	|ИЗ ВтБазыПрямыхРасходовДетально КАК ДД
	|	ЛЕВОЕ СОЕДИНЕНИЕ КоэффициентыПриведенияБазыРаспределения КАК КоэффициентыПриведения
	|		ПО ДД.Организация = КоэффициентыПриведения.Организация
	|		И ДД.БазаРаспределенияПоПартиям = КоэффициентыПриведения.БазаРаспределенияПоПартиям
...
Функция ТекстЗапросаДляРаспределенияПрямыхРасходовНаПроизводство(ПараметрыРасчета) Экспорт
	...
	ТекстЗапроса = ""
		...
		|	КОНЕЦ КАК ДоляСтоимостиРегл,
		|	ВЫБОР 
		|		КОГДА НЕ ДД.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьПроизводства)
		|			ТОГДА 0
		|		ИНАЧЕ
		|			1
		|	КОНЕЦ КАК ДоляСтоимостиНУ,
		...
		|	КОНЕЦ КАК ДоляСтоимостиУпр,
		|	ВЫБОР 
		|		КОГДА НЕ ДД.ВариантРаспределенияРасходовНДД = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьПроизводства)
		|			ТОГДА 0
		|		ИНАЧЕ
		|			1
		|	КОНЕЦ КАК ДоляСтоимостиНДД,
		...
		|			И ДД.АналитикаРасходов ССЫЛКА Справочник.ПартииПроизводства
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтОтборПартииПроизводства КАК Отбор
		|			ПО Партии.Ссылка = Отбор.ПартияПроизводства
		//++ НЕ УТКА
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства.Организации КАК ПартииПроизводстваОрганизации
		|		ПО Партии.ВнутренняяПереработка
		|		И Партии.Ссылка = ПартииПроизводстваОрганизации.Ссылка
		//-- НЕ УТКА
		|ГДЕ
		|	ВЫБОР
		//++ НЕ УТКА
		|		КОГДА ПартииПроизводстваОрганизации.Организация ЕСТЬ НЕ NULL
		|			ТОГДА ПартииПроизводстваОрганизации.Организация
		//-- НЕ УТКА
		|		КОГДА ИСТИНА
		|			ТОГДА Партии.Организация
		|	КОНЕЦ = ДД.Организация
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|			ТОГДА 0
		|		ИНАЧЕ
		|			ВЫРАЗИТЬ(ЕСТЬNULL(Базы.БазаНДД, 0) КАК ЧИСЛО(28, 10))
		|	КОНЕЦ КАК ДоляСтоимостиНДД,
		|	НЕОПРЕДЕЛЕНО                                                            КАК КорСтатьяРасходов,
		|	НЕОПРЕДЕЛЕНО                                                            КАК КорАналитикаРасходов,
		|	НЕОПРЕДЕЛЕНО                                                            КАК КорСтатьяАктивовПассивов,
		...
		|			ПО	ДД.Организация = Базы.Организация
		|				И ДД.БазаРаспределенияПоПартиям = Базы.БазаРаспределенияПоПартиям
		|				И ДД.АналитикаРасходов = Базы.ГруппаРаспределения
		|				И ДД.Подразделение = Базы.Подразделение
		|				И НЕ ДД.АналитикаРасходов ССЫЛКА Справочник.ПартииПроизводства
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК Партии
...
Процедура ПодготовитьПрямыеРасходыКРаспределению(ПараметрыРасчета)
	...
	Запрос.Текст = 
		...
		|		ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.ПустаяСсылка)
		|	КОНЕЦ КАК ВариантРаспределенияРасходовНУ,
		|	ВЫБОР КОГДА ДД.НДД ТОГДА 
		|		ЕСТЬNULL(НастройкиРаспределенияСтатейНДД.ВариантРаспределенияРасходовНДД, ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.ПустаяСсылка))
		|	ИНАЧЕ
		|		ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.ПустаяСсылка)
		|	КОНЕЦ КАК ВариантРаспределенияРасходовНДД,
		|	ДД.СтатьяКалькуляции,
		|	ВЫБОР 
		|		КОГДА ДД.БазаРаспределенияПоПартиям = ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.СуммаМатериальныхИТрудозатрат)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ПравилаОтнесенияНаВыпуск.ПоМатериальнымИТрудозатратам)
		...
		//++ НЕ УТКА
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства.Организации КАК ПартииПроизводстваОрганизации
		|	ПО Т.ВнутренняяПереработка
		|	И Т.Ссылка = ПартииПроизводстваОрганизации.Ссылка
		//-- НЕ УТКА
		|ГДЕ
		|	ВЫБОР
		//++ НЕ УТКА
		...
		|			И ТИПЗНАЧЕНИЯ(ДД.АналитикаРасходов) = ТИП(Справочник.Номенклатура)
		//++ НЕ УТКА
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства.Организации КАК ПартииПроизводстваОрганизации
		|	ПО Т.ВнутренняяПереработка
		|	И Т.Ссылка = ПартииПроизводстваОрганизации.Ссылка
		...
		|			И ТИПЗНАЧЕНИЯ(ДД.АналитикаРасходов) = ТИП(Справочник.ГруппыАналитическогоУчетаНоменклатуры)
		//++ НЕ УТКА
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства.Организации КАК ПартииПроизводстваОрганизации
		|		ПО Т.ВнутренняяПереработка
		|			И Т.Ссылка = ПартииПроизводстваОрганизации.Ссылка
		...
		|	ДД.АналитикаРасходов,
		|	ДД.Подразделение КАК Подразделение,
		|	ЕСТЬNULL(ПартииПроизводстваОрганизации.Организация, Т.Организация) КАК Организация,
		|	Т.Ссылка
		|ИЗ ПрямыеРасходыКРаспределению КАК ДД
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК Т
		|		ПО Т.Документ = ДД.АналитикаРасходов
		|			И ТИПЗНАЧЕНИЯ(ДД.АналитикаРасходов) = ТИП(Документ.ЗаказНаПроизводство2_2)
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства.Организации КАК ПартииПроизводстваОрганизации
		|		ПО Т.ВнутренняяПереработка
		|			И Т.Ссылка = ПартииПроизводстваОрганизации.Ссылка
		|ГДЕ
		|	ЕСТЬNULL(ПартииПроизводстваОрганизации.Организация, Т.Организация) = ДД.Организация
		//-- НЕ УТКА
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ПартияПроизводства
	...
	// Уничтожим вспомогательные временные таблицы.
	НовыеВТ = "Регистраторы,
	|ПрямыеРасходыКРаспределению,ВтГруппыРаспределения,ВтОтборПартииПроизводства,
	|НастройкиРаспределенияСтатейНДД,НастройкиРаспределенияСтатейНДДПоОВЗ
	|";
	ДопНовыеВТ = "";
	Если ДляРасшифровки Тогда
		ДопНовыеВТ = "ДанныеПоМатериальнымЗатратам,ДанныеПоТрудозатратам,ДанныеПоПродукции";
	КонецЕсли;
	
	РасчетСебестоимостиПрикладныеАлгоритмы.УничтожитьВременныеТаблицы(ПараметрыРасчета, 
		РасчетСебестоимостиПрикладныеАлгоритмы.ПолучитьПереченьСуществующихВременныхТаблиц(ПараметрыРасчета,
			РасчетСебестоимостиУниверсальныеАлгоритмы.СоединитьСтроки(СуществующиеВТ, НовыеВТ, ДопНовыеВТ)));
...
Функция ТекстЗапросаДляПрочихРасходовНЗП(ПараметрыРасчета = Неопределено) Экспорт
	...
	ТекстФормированияВременныхТаблиц = ""
		...
		+ ТекстПартииДляОтнесенияРасходов(ПараметрыРасчета)
...
Функция ТекстОписаниеДанныхДляПрочихРасходовНЗП() Экспорт
	...
	|	ДД.ПоказательОтнесенияНаПартиюРегл										КАК ПоказательОтнесенияНаВыпускНУ,
	|	ДД.ПоказательОтнесенияНаПартиюУпр										КАК ПоказательОтнесенияНаВыпускУпр,
	|	0																		КАК ПоказательОтнесенияНаВыпускНДД,
	...
	|	ДД.ПоказательОтнесенияНаПартиюРегл										КАК ПоказательОтнесенияНаПартиюНУ,
...
Функция ТекстПартииДляОтнесенияРасходов(ПараметрыРасчета = Неопределено)
	
	ТекстЗапросаВтПартииДляОтнесенияРасходов = "
		|ВЫБРАТЬ
		|	ДД.Организация                                 КАК Организация,
		|	ДД.Подразделение                               КАК Подразделение,
		|	ДД.ПартияПроизводства                          КАК ПартияПроизводства,
		...
		//++ Локализация
		|			И Доли.ДоляСтоимостиНУ = 0
		//-- Локализация
		|			И Доли.ДоляСтоимостиНДД = 0
		|			)
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|		ИЛИ ЕСТЬNULL(ДД.СтоимостьНДДОстаток,0) <> 0
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ
		...
		ТекстЗапросаИдентификаторыПартий = "
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДД.ПартияПроизводства                          КАК ПартияПроизводства,
		|	ДД.ЗаказНаПроизводство                         КАК ЗаказНаПроизводство,
		|	ДД.КодСтрокиПродукция                          КАК КодСтрокиПродукция,
		...
		ТекстЗапросаНастройкиПризнанияРасходовНДДПоСтатьям = РасчетСебестоимостиЛокализация.ТекстЗапросаНастроекНДД("втПартииДляОтнесенияРасходов", 
			"РасчетСебестоимостиПостатейныеЗатраты_ТекстПартииДляОтнесенияРасходов",
			ПараметрыРасчета);
		
		ТекстЗапросаПартииДляОтнесенияРасходов = "
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДД.Организация                                 КАК Организация,
		|	ДД.Подразделение                               КАК Подразделение,
		|	ДД.ПартияПроизводства                          КАК ПартияПроизводства,
		...
		|		ЕСТЬNULL(НастройкиРаспределенияСтатейНДД.ВариантРаспределенияРасходовНДД, ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.ПустаяСсылка))) КАК ВариантРаспределенияРасходовНДД,
		|	ЕСТЬNULL(НастройкиПризнанияРасходовНДД.РасходыВключаютсяВстоимость, ЛОЖЬ) КАК РасходыНДДВключаютсяВстоимость,
		|	ДД.АналитикаРасходов                           КАК АналитикаРасходов,
		|	ДД.ГруппаПродукции                             КАК ГруппаПродукции,
		|	ДД.ПравилоОтнесенияНаВыпуск                    КАК ПравилоОтнесенияНаВыпуск,
		|	ДД.ДоляСтоимостиОстаток                        КАК ДоляСтоимостиОстаток,
		...
		|	ЛЕВОЕ СОЕДИНЕНИЕ НастройкиПризнанияРасходовНДД КАК НастройкиПризнанияРасходовНДД
		|		ПО &ДД_ОбъектРаздельногоУчетаНДД = НастройкиПризнанияРасходовНДД.ОбъектРаздельногоУчетаНДД
		|		И ДД.Организация = НастройкиПризнанияРасходовНДД.Организация
		|		И НастройкиПризнанияРасходовНДД.РасходыВключаютсяВСтоимость
		|;
		...
		ТекстЗапросаПартииДляОтнесенияРасходовСводно = "
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДД.Организация                                 КАК Организация,
		|	ДД.Подразделение                               КАК Подразделение,
		|	ДД.ПартияПроизводства                          КАК ПартияПроизводства,
	...
	ТекстыЗапроса.Добавить(ТекстЗапросаВтПартииДляОтнесенияРасходов);
	ТекстыЗапроса.Добавить(ТекстЗапросаИдентификаторыПартий);
	ТекстыЗапроса.Добавить(ТекстЗапросаНастройкиПризнанияРасходовНДДПоСтатьям);
	ТекстыЗапроса.Добавить(ТекстЗапросаПартииДляОтнесенияРасходов);
	ТекстыЗапроса.Добавить(ТекстЗапросаПартииДляОтнесенияРасходовСводно);
	
	ТекстЗапроса = СтрСоединить(ТекстыЗапроса, ОбщегоНазначенияУТ.РазделительЗапросовВПакете());
	
	РасчетСебестоимостиЛокализация.ПодставитьВЗапросеТипОбъектаРаздельногоУчетаНДД(ТекстЗапроса, "ДД_ОбъектРаздельногоУчетаНДД");
...
Функция ТекстМатериальныеЗатратыНЗП()
	...
	ТекстЗапроса =
		...
		|		+ ЕСТЬNULL(Цены.СтоимостьЗабалансовая, 0)) КАК ЧИСЛО(31, 10)))		КАК Стоимость,
		|	СУММА(ВЫРАЗИТЬ(ДД.Количество *
		|		(ЕСТЬNULL(Цены.СтоимостьБезНДС, 0)
		|		+ ЕСТЬNULL(Цены.СтоимостьДопРасходыБезНДС, 0)
		|		+ ЕСТЬNULL(Цены.СтоимостьЗабалансовая, 0)) КАК ЧИСЛО(31, 10)))		КАК СтоимостьБезНДС,
		|	СУММА(ВЫРАЗИТЬ(ДД.Количество *
		|		(ЕСТЬNULL(Цены.СтоимостьРегл, 0)
		|		+ ЕСТЬNULL(Цены.ДопРасходыРегл, 0)
		|		+ ЕСТЬNULL(Цены.СтоимостьЗабалансоваяРегл, 0)) КАК ЧИСЛО(31, 10)))	КАК СтоимостьРегл,
		|	СУММА(ВЫРАЗИТЬ(ДД.Количество *
		|		(ЕСТЬNULL(Цены.СтоимостьУпр, 0)
		|		+ ЕСТЬNULL(Цены.ДопРасходыУпр, 0)
		|		+ ЕСТЬNULL(Цены.СтоимостьЗабалансовая, 0)) КАК ЧИСЛО(31, 10)))		КАК СтоимостьУпр,
		//++ Локализация
		|	СУММА(ДД.Количество *
		|		(ЕСТЬNULL(Цены.СтоимостьРегл, 0)
		|		+ ЕСТЬNULL(Цены.ДопРасходыРегл, 0)
...
Функция ТекстМатериальныеЗатратыИТрудозатратыНЗП()
	...
	ТекстЗапроса = 
		...
		//++ Локализация
		|	ВЫРАЗИТЬ(СУММА(ДД.СтоимостьНУ) КАК ЧИСЛО(23,10))        КАК СтоимостьНУ,
		//-- Локализация
		|	ВЫРАЗИТЬ(СУММА(ДД.СтоимостьНДД) КАК ЧИСЛО(23,10))       КАК СтоимостьНДД
		...
		|		ДД.СтоимостьУпр                          КАК СтоимостьУпр,
		//++ Локализация
		|		ДД.СтоимостьНУ                           КАК СтоимостьНУ,
		//-- Локализация
		|		ДД.СтоимостьНДД                          КАК СтоимостьНДД
...
Функция ТекстВыпущеннаяПродукция()
	...
	ТекстЗапроса =
		...
		|	НЕОПРЕДЕЛЕНО					КАК ЗаказНаПроизводство,
		|	0								КАК КодСтрокиПродукция,
		|	КлючиАналитикиУчетаНоменклатуры.Ссылка											КАК КорАналитикаУчетаНоменклатуры,
		|	ЕСТЬNULL(СпрВидыЗапасов.Ссылка, ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка))	КАК КорВидЗапасов,
		|	ВЫБОР
		|		КОГДА СпрВидыЗапасов.Организация В (&ОрганизацииСФИФОСкользящая)
		...
		|		КОГДА СпрВидыЗапасов.ТипЗапасов В
		|				(ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца),
		|				ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца))
		...
		|			И СпрВидыЗапасов.Организация В (&МассивОрганизаций)
		|
		...
		|		ПО КлючиАналитикиУчетаНоменклатуры.Ссылка = ВЫБОР
		...
		|			И СпрВидыЗапасов.Организация = ПартииМатериаловВНЗП.Организация
		|
		|ГДЕ
		|	Реквизиты.Дата МЕЖДУ &НачалоПериода И &КонецПериода
		|	И НЕ КлючиАналитикиУчетаНоменклатуры.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ВЫБОР
		|		КОГДА СпрОрганизации.Ссылка В (&ОрганизацииСФИФОСкользящая)
		|			ТОГДА АналитикиУчетаПартий.КлючАналитики
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаПартий.ПустаяСсылка)
		|	КОНЕЦ                                                   КАК КорАналитикаУчетаПартий,
		...
		|			ТОГДА СпрОрганизации.Ссылка = ДД.ОрганизацияДавалец
		//-- НЕ УТКА
		|		КОГДА ИСТИНА
		|			ТОГДА СпрОрганизации.Ссылка = Реквизиты.Организация
		|		КОНЕЦ
		...
		|		И ПартииМатериаловВНЗП.ПартияМатериала = ДД.Ссылка
		|		И ПартииМатериаловВНЗП.АналитикаУчетаПартий = АналитикиУчетаПартий.КлючАналитики
		|		И СпрОрганизации.Ссылка = ПартииМатериаловВНЗП.Организация
		|
		|ГДЕ
		|	Реквизиты.Дата МЕЖДУ &НачалоПериода И &КонецПериода
		|	И Реквизиты.Проведен
	...
	ТекстЗапроса = 
		...
		"ВЫБРАТЬ
		|	""ТекстВыпущеннаяПродукция_6""			КАК ЗапросИсточник,
		...
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК СпрОрганизации
		|	ПО ВЫБОР КОГДА Реквизиты.ВнутренняяПереработка
		|			ТОГДА СпрОрганизации.Ссылка = Реквизиты.ОрганизацияДавалец
		|			ИНАЧЕ СпрОрганизации.Ссылка = Реквизиты.Организация
		|		КОНЕЦ
		|		И СпрОрганизации.Ссылка В (&МассивОрганизаций)
		|
		...
		|	ПО ДД.ВидЗапасов = СпрВидыЗапасов.Ссылка
		|		И СпрВидыЗапасов.Организация = СпрОрганизации.Ссылка
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаДокументаБезНазначения
		|	ПО АналитикаДокументаБезНазначения.Номенклатура = ДД.АналитикаУчетаНоменклатуры.Номенклатура
		|		И АналитикаДокументаБезНазначения.Характеристика = ДД.АналитикаУчетаНоменклатуры.Характеристика
		...
		|		И СпрОрганизации.Ссылка В (&ОрганизацииСФИФОСкользящая)
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ПартииМатериаловВНЗП КАК ПартииМатериаловВНЗП
		...
		|		И ПартииМатериаловВНЗП.Организация = СпрОрганизации.Ссылка
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитикиУчетаНоменклатуры
		|	ПО КлючиАналитикиУчетаНоменклатуры.Ссылка = ВЫБОР
		|				КОГДА &УчитыватьСебестоимостьТоваровПоНазначениям
		...
		|ВЫБРАТЬ
		|	""ТекстВыпущеннаяПродукция_7""                  КАК ЗапросИсточник,
		|	Реквизиты.Ссылка								КАК РегистраторРасхода,
		|	ДД.ДатаПроизводства								КАК ДатаПроизводства,
		|	Реквизиты.ПартияПроизводства					КАК ПартияПроизводства,
		|	ДД.Подразделение								КАК Подразделение,
		...
		|ВЫБРАТЬ
		|	""ТекстВыпущеннаяПродукция_8""                          КАК ЗапросИсточник,
		...
		|				ТОГДА ДД.Количество
		|			ИНАЧЕ ДД.ДоляСтоимости
		|		КОНЕЦ / ДД.Количество) КАК ЧИСЛО(23,10))  КАК Стоимость,
		|	ВЫРАЗИТЬ(СУММА(
		|		ДД.Количество *
		|		ВЫБОР
		|			КОГДА ДД.ДоляСтоимости = 0
		|				ТОГДА ДД.Количество
		|			ИНАЧЕ ДД.ДоляСтоимости
		|		КОНЕЦ / ДД.Количество) КАК ЧИСЛО(23,10))	КАК ПолнаяСтоимость,
		|	0,
		|	РеквизитыОтчета.Организация						КАК Организация
		|ИЗ
		|	Документ.ОтчетПереработчика2_5.ВидыЗапасовИзделия КАК ДД
		...
		|
		|ВЫБРАТЬ
		|	""ТекстВыпущеннаяПродукция_11""	КАК ЗапросИсточник,
		|	РеквизитыОтчета.Ссылка			КАК РегистраторРасхода,
		|	РеквизитыОтчета.Дата			КАК ДатаПроизводства,
		|	ДД.ПартияПроизводства			КАК ПартияПроизводства,
		|	РеквизитыОтчета.Подразделение	КАК Подразделение,
		...
		|	РеквизитыОтчета.Организация							КАК Организация
		|ИЗ
		|	Документ.ОтчетДавальцуМеждуОрганизациями.Товары КАК ДД
		|
		...
		|			И Т.ИдентификаторСтроки = ДД.ИдентификаторСтроки
		|			И Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		...
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаДокументаБезНазначения
		|		ПО АналитикаДокументаБезНазначения.Номенклатура = Аналитика.Номенклатура
		|			И АналитикаДокументаБезНазначения.Характеристика = Аналитика.Характеристика
		|			И АналитикаДокументаБезНазначения.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|			И АналитикаДокументаБезНазначения.СтатьяКалькуляции = Аналитика.СтатьяКалькуляции
		|			И АналитикаДокументаБезНазначения.МестоХранения = Аналитика.МестоХранения
		|			И АналитикаДокументаБезНазначения.Серия = Аналитика.Серия
		|
		// Исключаем документы по которым введена корректировка или сторнирование в текущем периоде
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК Реестр
		|		ПО Реестр.СторнируемыйДокумент = ДД.Ссылка
		|			И Реестр.СторноИсправление
...
Функция ТекстПрочиеРасходыНЗПИнициализация()
	...
	ТекстЗапроса =
		...
		//++ Локализация
		|	СУММА(ДД.ПоказательОтнесенияНаВыпускНУ),
		//-- Локализация
		
		|	СУММА(ДД.ПоказательОтнесенияНаВыпускНДД)
		...
		//++ Локализация
		|		СУММА(Затраты.СтоимостьНУ) КАК ПоказательОтнесенияНаВыпускНУ,
		//-- Локализация
		|		СУММА(Затраты.СтоимостьНДД) КАК ПоказательОтнесенияНаВыпускНДД
		|	ИЗ
		|		ПартииДляОтнесенияРасходовСводно КАК ДД
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ МатериальныеЗатратыПроизводства КАК Затраты
		|			ПО (Затраты.Организация = ДД.Организация)
		|				И (Затраты.Подразделение = ДД.Подразделение)
		...
		//++ Локализация
		|		СУММА(Затраты.СтоимостьНУ),
		//-- Локализация
		|		СУММА(Затраты.СтоимостьНДД)
		|	ИЗ
		|		ПартииДляОтнесенияРасходовСводно КАК ДД
		...
		|	
		|	ВЫБРАТЬ
		|		ДолиПроизводственныхРасходов.Организация,
		...
		|	
		|	ВЫБРАТЬ
		|		ПрямыеПроизводственныеРасходы.Организация,
		...
		|	
		|	ВЫБРАТЬ
		|		ПрочиеРасходыНезавершенногоПроизводстваОстатки.Организация,
...
Функция ТекстОстаткиБазРаспределения()
	...
	ТекстЗапроса =
		...
		//++ Локализация
		|	ВЫРАЗИТЬ(СУММА(ДД.Количество * (ЕСТЬNULL(Цены.СтоимостьРегл, 0) + ЕСТЬNULL(Цены.ДопРасходыРегл, 0) + ЕСТЬNULL(Цены.СтоимостьЗабалансоваяРегл, 0 - ЕСТЬNULL(Цены.ПостояннаяРазница, 0) - ЕСТЬNULL(Цены.ВременнаяРазница, 0)))) КАК ЧИСЛО(23, 10)) КАК ОстатокНУ,
		//-- Локализация
		|	ВЫРАЗИТЬ(СУММА(ДД.Количество * (ЕСТЬNULL(Цены.СтоимостьНДД, 0))) КАК ЧИСЛО(23, 10)) КАК ОстатокНДД
		...
		|		ПО ДД.Организация = Партии.Организация
		|		И ДД.Партия = Партии.ПартияПроизводства
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСтоимостьПартийТоваров КАК Цены
		|		ПО (Цены.Организация = ДД.Организация)
		|			И (Цены.Партия = ДД.Партия)
		|			И (Цены.АналитикаУчетаПартий = ДД.АналитикаУчетаПартий)
		...
		//++ Локализация
		|	СУММА(ВЫБОР
		|			КОГДА ЕСТЬNULL(Остатки.СтоимостьРегл, 0) - ЕСТЬNULL(Остатки.ПостояннаяРазница, 0) - ЕСТЬNULL( Остатки.ВременнаяРазница, 0) = 0
		|				ТОГДА ЕСТЬNULL(Остатки.НормативнаяСтоимость, 0)
		|			ИНАЧЕ Остатки.СтоимостьРегл - Остатки.ПостояннаяРазница - Остатки.ВременнаяРазница
		|		КОНЕЦ),
		//-- Локализация
		|	СУММА(ВЫБОР
		|			КОГДА ЕСТЬNULL(Остатки.СтоимостьНДД, 0) = 0
		|				ТОГДА ЕСТЬNULL(Остатки.НормативнаяСтоимость, 0)
		|			ИНАЧЕ Остатки.Стоимость
		|		КОНЕЦ)
		|ИЗ
...
Функция ТекстКоэффициентыПриведения()
	...
	ТекстЗапроса =
		...
		//++ Локализация
		|	ВЫБОР
		|		КОГДА СУММА(ДД.ПоказательОтнесенияНаВыпускНУ) > 1000000000
		|			ТОГДА 1000000
		|		КОГДА СУММА(ДД.ПоказательОтнесенияНаВыпускНУ) > 100000000
		|			ТОГДА 100000
		|		КОГДА СУММА(ДД.ПоказательОтнесенияНаВыпускНУ) > 10000000
		|			ТОГДА 10000
		|		КОГДА СУММА(ДД.ПоказательОтнесенияНаВыпускНУ) > 1000000
		|			ТОГДА 1000
		|		КОГДА СУММА(ДД.ПоказательОтнесенияНаВыпускНУ) > 100000
		|			ТОГДА 100
		|		КОГДА СУММА(ДД.ПоказательОтнесенияНаВыпускНУ) > 10000
		|			ТОГДА 10
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК КоэффициентНУ,
		//-- Локализация
		|	ВЫБОР
		|		КОГДА СУММА(ДД.ПоказательОтнесенияНаВыпускНДД) > 1000000000
		|			ТОГДА 1000000
		|		КОГДА СУММА(ДД.ПоказательОтнесенияНаВыпускНДД) > 100000000
		|			ТОГДА 100000
		|		КОГДА СУММА(ДД.ПоказательОтнесенияНаВыпускНДД) > 10000000
		|			ТОГДА 10000
		|		КОГДА СУММА(ДД.ПоказательОтнесенияНаВыпускНДД) > 1000000
		|			ТОГДА 1000
		|		КОГДА СУММА(ДД.ПоказательОтнесенияНаВыпускНДД) > 100000
		|			ТОГДА 100
		|		КОГДА СУММА(ДД.ПоказательОтнесенияНаВыпускНДД) > 10000
		|			ТОГДА 10
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК КоэффициентНДД
		...
		|ИЗ
		|	(ВЫБРАТЬ
		|		ЗатратыПродукции.Организация КАК Организация,
		...
		//++ Локализация
		|		ЗатратыПродукции.ПоказательОтнесенияНаВыпускНУ КАК ПоказательОтнесенияНаВыпускНУ,
		//-- Локализация
		|		ЗатратыПродукции.ПоказательОтнесенияНаВыпускНДД КАК ПоказательОтнесенияНаВыпускНДД
		|	ИЗ
		...
		|	
		|	ВЫБРАТЬ
		|		ОстаткиБазРаспределения.Организация,
		...
		//++ Локализация
		|		ОстаткиБазРаспределения.ОстатокНУ,
		//-- Локализация
		|		ОстаткиБазРаспределения.ОстатокНДД
		|	ИЗ
...
Функция ТекстПриходыПрочихРасходовНЗП()
	Возврат "
		...
		|	0													КАК ПоказательОтнесенияНаВыпускНУ,
		...
		|	ДД.ДоляСтоимостиНУ									КАК ПоказательОтнесенияНаПартиюНУ,
		|	ДД.ДоляСтоимостиУпр									КАК ПоказательОтнесенияНаПартиюУпр,
		|	ВЫБОР
		|		КОГДА ПартииСводно.РасходыНДДВключаютсяВстоимость
		|			ТОГДА ДД.ДоляСтоимостиНДД
		|		ИНАЧЕ 0
		|	КОНЕЦ												КАК ПоказательОтнесенияНаПартиюНДД,
...
Функция ТекстРасходыПрочихРасходовНЗППоМатериалам() // использует ПартииДляОтнесенияРасходов, МатериальныеЗатратыПроизводства, ОстаткиПриходыРасходы, РаботыДляДавальца
	...
	//++ Устарело_Переработка24
	|		КОГДА РаботыДляДавальца.КорАналитикаУчетаНоменклатуры ЕСТЬ НЕ NULL
	|			ТОГДА РаботыДляДавальца.КорАналитикаУчетаНоменклатуры
	//-- Устарело_Переработка24
	...
	|	0                                                   КАК ВременнаяРазница,
	|	0													КАК СтоимостьНДД,
	// Показатель отнесения на выпуск рассчитываем для статей расходов любым вариантом распределения,
	// т.к. на выпуск нужно распределить все суммы, ранее распределенные на партии производства.
	|	ВЫРАЗИТЬ(Материальные.Стоимость / ЕСТЬNULL(Коэффициенты.Коэффициент, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпуск,
	|	ВЫРАЗИТЬ(Материальные.СтоимостьБезНДС / ЕСТЬNULL(Коэффициенты.КоэффициентБезНДС, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускБезНДС,
	|	ВЫРАЗИТЬ(Материальные.СтоимостьРегл / ЕСТЬNULL(Коэффициенты.КоэффициентРегл, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускРегл,
	|	ВЫРАЗИТЬ(Материальные.СтоимостьРегл / ЕСТЬNULL(Коэффициенты.КоэффициентРегл, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускНУ,
	|	ВЫРАЗИТЬ(Материальные.СтоимостьУпр / ЕСТЬNULL(Коэффициенты.КоэффициентУпр, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускУпр,
	// Распределение НДД выполняется по базе регламентированного учета.
	|	ВЫБОР
	|		КОГДА НЕ ДД.РасходыНДДВключаютсяВСтоимость
	|			ТОГДА 0
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ(Материальные.СтоимостьРегл / ЕСТЬNULL(Коэффициенты.КоэффициентРегл, 1) КАК ЧИСЛО (23, 10))
	...
	//++ Устарело_Переработка24
	|		КОГДА РаботыДляДавальца.КорРазделУчета ЕСТЬ НЕ NULL
	|			ТОГДА РаботыДляДавальца.КорРазделУчета
	//-- Устарело_Переработка24
	...
	//++ Устарело_Переработка24
	|		КОГДА РаботыДляДавальца.КорВидЗапасов ЕСТЬ НЕ NULL
	|			ТОГДА РаботыДляДавальца.КорВидЗапасов
	//-- Устарело_Переработка24
	...
	|		И Материальные.Подразделение = ДД.Подразделение
...
Функция ТекстРасходыПрочихРасходовНЗППоТрудозатратам() // использует ПартииДляОтнесенияРасходов, ТрудозатратыПроизводства, ОстаткиПриходыРасходы, РаботыДляДавальца
	...
	// Показатель отнесения на выпуск рассчитываем для статей расходов любым вариантом распределения,
	// т.к. на выпуск нужно распределить все суммы, ранее распределенные на партии производства.
	|	ВЫРАЗИТЬ(Трудозатраты.Стоимость / ЕСТЬNULL(Коэффициенты.Коэффициент, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпуск,
	|	ВЫРАЗИТЬ(Трудозатраты.СтоимостьБезНДС / ЕСТЬNULL(Коэффициенты.КоэффициентБезНДС, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускБезНДС,
	|	ВЫРАЗИТЬ(Трудозатраты.СтоимостьРегл / ЕСТЬNULL(Коэффициенты.КоэффициентРегл, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускРегл,
	|	ВЫРАЗИТЬ(Трудозатраты.СтоимостьРегл / ЕСТЬNULL(Коэффициенты.КоэффициентРегл, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускНУ,
	|	ВЫРАЗИТЬ(Трудозатраты.СтоимостьУпр / ЕСТЬNULL(Коэффициенты.КоэффициентУпр, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускУпр,
	// Распределение НДД выполняется по базе регламентированного учета.
	|	ВЫБОР 
	|		КОГДА НЕ ДД.РасходыНДДВключаютсяВСтоимость
	|			ТОГДА 0
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ(Трудозатраты.СтоимостьРегл / ЕСТЬNULL(Коэффициенты.КоэффициентРегл, 1) КАК ЧИСЛО (23, 10))
	|	КОНЕЦ КАК ПоказательОтнесенияНаВыпускНДД,
	|	0                                                   КАК ПоказательОтнесенияНаПартию,
	|	0                                                   КАК ПоказательОтнесенияНаПартиюБезНДС,
	|	0                                                   КАК ПоказательОтнесенияНаПартиюРегл,
	...
	|		ПО Трудозатраты.Организация = ДД.Организация
	|		И Трудозатраты.Подразделение = ДД.Подразделение
...
Функция ТекстРасходыПрочихРасходовНЗППоМатериальнымИТрудозатратам() // использует ПартииДляОтнесенияРасходов, МатериальныеИТрудозатратыПроизводства, ОстаткиПриходыРасходы, РаботыДляДавальца
	...
	// Показатель отнесения на выпуск рассчитываем для статей расходов любым вариантом распределения,
	// т.к. на выпуск нужно распределить все суммы, ранее распределенные на партии производства.
	|	ВЫРАЗИТЬ(Затраты.Стоимость / ЕСТЬNULL(Коэффициенты.Коэффициент, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпуск,
	|	ВЫРАЗИТЬ(Затраты.СтоимостьБезНДС / ЕСТЬNULL(Коэффициенты.КоэффициентБезНДС, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускБезНДС,
	|	ВЫРАЗИТЬ(Затраты.СтоимостьРегл / ЕСТЬNULL(Коэффициенты.КоэффициентРегл, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускРегл,
	|	ВЫРАЗИТЬ(Затраты.СтоимостьРегл / ЕСТЬNULL(Коэффициенты.КоэффициентРегл, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускНУ,
	|	ВЫРАЗИТЬ(Затраты.СтоимостьУпр / ЕСТЬNULL(Коэффициенты.КоэффициентУпр, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускУпр,
	// Распределение НДД выполняется по базе регламентированного учета.
	|	ВЫБОР 
	|		КОГДА НЕ ДД.РасходыНДДВключаютсяВСтоимость
	|			ТОГДА 0
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ(Затраты.СтоимостьРегл / ЕСТЬNULL(Коэффициенты.КоэффициентРегл, 1) КАК ЧИСЛО (23, 10))
	|	КОНЕЦ КАК ПоказательОтнесенияНаВыпускНДД,
	|	0                                                   КАК ПоказательОтнесенияНаПартию,
	|	0                                                   КАК ПоказательОтнесенияНаПартиюБезНДС,
	|	0                                                   КАК ПоказательОтнесенияНаПартиюРегл,
	...
	// расход на каждую аналитику выпуска
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ МатериальныеИТрудозатратыПроизводства КАК Затраты
	|		ПО ДД.Организация = Затраты.Организация
	|		И Затраты.Подразделение = ДД.Подразделение
	|		И Затраты.ПартияПроизводства = ДД.ПартияПроизводства
	|		И Затраты.ЗаказНаПроизводство = ДД.ЗаказНаПроизводство
	|		И Затраты.КодСтрокиПродукция = ДД.КодСтрокиПродукция
...
Функция ТекстРасходыПрочихРасходовНЗППоПродукции() // использует ПартииДляОтнесенияРасходов, ВыпущеннаяПродукция, РаботыДляДавальца
	Возврат "
			...
			//++ Устарело_Переработка24
			|		КОГДА РаботыДляДавальца.АналитикаУчетаПартий ЕСТЬ НЕ NULL
			...
			// Показатель отнесения на выпуск рассчитываем для статей расходов любым вариантом распределения,
			// т.к. на выпуск нужно распределить все суммы, ранее распределенные на партии производства.
			|	ВЫРАЗИТЬ(Продукция.Стоимость / ЕСТЬNULL(Коэффициенты.Коэффициент, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпуск,
			|	ВЫРАЗИТЬ(Продукция.Стоимость / ЕСТЬNULL(Коэффициенты.Коэффициент, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускБезНДС,
			|	ВЫРАЗИТЬ(Продукция.Стоимость / ЕСТЬNULL(Коэффициенты.Коэффициент, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускРегл,
			|	ВЫРАЗИТЬ(Продукция.Стоимость / ЕСТЬNULL(Коэффициенты.Коэффициент, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускНУ,
			|	ВЫРАЗИТЬ(Продукция.Стоимость / ЕСТЬNULL(Коэффициенты.Коэффициент, 1) КАК ЧИСЛО (23, 10)) КАК ПоказательОтнесенияНаВыпускУпр,
			// Распределение НДД выполняется по базе регламентированного учета.
			|	ВЫБОР 
			|		КОГДА НЕ ДД.РасходыНДДВключаютсяВСтоимость
			|			ТОГДА 0
			|		ИНАЧЕ ВЫРАЗИТЬ(Продукция.Стоимость / ЕСТЬNULL(Коэффициенты.Коэффициент, 1) КАК ЧИСЛО (23, 10))
			|	КОНЕЦ												КАК ПоказательОтнесенияНаВыпускНДД,
			|	0                                                   КАК ПоказательОтнесенияНаПартию,
			|	0                                                   КАК ПоказательОтнесенияНаПартиюБезНДС,
			|	0                                                   КАК ПоказательОтнесенияНаПартиюРегл,
...
Функция ТекстЗапросаБазаРаспределенияРасходовНаПродажи()
	...
	//++ НЕ УТКА
	|	ИЛИ ПрочиеРасходы.АналитикаРасходов ССЫЛКА Документ.ОтчетДавальцу2_5
	|	ИЛИ ПрочиеРасходы.АналитикаРасходов ССЫЛКА Документ.ОтчетДавальцуМеждуОрганизациями
	//-- НЕ УТКА
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|		ПО Заказ.Ссылка = ПрочиеРасходы.АналитикаРасходов
	|		И Заказ.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовДавальцев.НеСогласован)
	|ГДЕ
	|	ПрочиеРасходы.Организация В (&МассивОрганизаций)
	//-- НЕ УТКА
	|
	...
	|	ВтОтборПродажПоЗаказу КАК Отбор
	|ГДЕ
	|	Отбор.АналитикаРасходов ССЫЛКА Документ.ОтчетДавальцуМеждуОрганизациями
	|	И ВЫРАЗИТЬ(Отбор.АналитикаРасходов КАК Документ.ОтчетДавальцуМеждуОрганизациями).Дата < &НачалоПериода
	//-- НЕ УТКА
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	АналитикаРасходов,
	...
	//++ НЕ УТКА
	|		ИЛИ Отбор.АналитикаРасходов = ЗНАЧЕНИЕ(Документ.ЗаказДавальца2_5.ПустаяСсылка)
	|		ИЛИ Отбор.АналитикаРасходов = ЗНАЧЕНИЕ(Документ.ОтчетДавальцу2_5.ПустаяСсылка)
	|		ИЛИ Отбор.АналитикаРасходов = ЗНАЧЕНИЕ(Документ.ОтчетДавальцуМеждуОрганизациями.ПустаяСсылка)
	//-- НЕ УТКА
...
Функция ТекстЗапросаБазаРаспределенияРасходовНаЗаказы()
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДД.Организация КАК Организация,
	|	ВЫРАЗИТЬ(ДД.АналитикаРасходов КАК Документ.ЗаказДавальца2_5) КАК АналитикаРасходов,
	...
	|ГДЕ
	|	ДД.Организация В (&МассивОрганизаций)
	|	И ДД.АналитикаРасходов ССЫЛКА Документ.ЗаказДавальца2_5
	|	И ВтОтборПродажПоЗаказу.АналитикаРасходов ЕСТЬ NULL
	//-- НЕ УТКА
	|ИНДЕКСИРОВАТЬ ПО
	|	АналитикаРасходов,
