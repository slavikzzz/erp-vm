...
Функция ПолучитьОтпечаткиСертификатовВСервисе() Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Получение отпечатков облачных сертификатов';
				|en = 'Receive cloud certificate thumbprints'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ТекстОшибкиНеправомерногоИспользованияОблачногоСертификата() Экспорт
	...
	СообщениеОбОшибке = НСтр("ru = 'Выбранный сертификат связан с недопустимым средством ""облачной"" электронной подписи.
				...
				|en = 'The selected certificate is related to invalid cloud digital signature facility.
				|In accordance with Order of the Ministry of Finance of the Russian Federation No. 174N dated 11/10/2015, electronic documents must be signed with 
				|strong qualified digital signature 
				|using digital signature facilities with confirmation (certificate) of compliance with the requirements established under Federal Law No. 63-FZ dated 04/06/2011.
				|Select a certificate related to another digital signature facility.'");
	
	Возврат СообщениеОбОшибке;
	
КонецФункции

// См. КриптографияБЭД.ПроверитьСертификат
Функция ПроверитьСертификат(МенеджерКриптографии, Сертификат, КонтекстДиагностики, Подпись = Неопределено) Экспорт

	РежимыПроверки = КриптографияБЭДСлужебныйКлиентСервер.РежимыПроверкиСертификата();
	ПараметрыПроверки = Новый Структура;
	ПараметрыПроверки.Вставить("ИгнорироватьПроверкуВСпискеОтозванныхСертификатов", Ложь);
	ПараметрыПроверки.Вставить("ВыполнятьПроверкуУдостоверяющегоЦентра", РежимыПроверки.НеПроверятьСертификат);
	ОшибкаПроверкиСертификата = "";
	РезультатПроверкиСредствамиКриптографии = ЭлектроннаяПодпись.ПроверитьСертификат(МенеджерКриптографии, Сертификат,
		ОшибкаПроверкиСертификата, , ПараметрыПроверки);
	Если ЗначениеЗаполнено(ОшибкаПроверкиСертификата) Тогда
		РезультатПроверкиСредствамиКриптографии = ОшибкаПроверкиСертификата;
	КонецЕсли;

	КонтекстПроверки = КриптографияБЭДКлиентСервер.НовыйКонтекстПроверкиКвалифицированногоСертификата(
		Сертификат.Выгрузить(), РезультатПроверкиСредствамиКриптографии, Подпись);
	РезультатПроверки = КриптографияБЭДКлиентСервер.ПроверитьКвалифицировнныйСертификат(Сертификат, КонтекстПроверки);
	Если Не РезультатПроверки.Успех И КонтекстДиагностики <> Неопределено Тогда
		Ошибка = КриптографияБЭДКлиентСервер.СформироватьОшибкуПоПротоколуПроверкиКвалификации(Сертификат,
			РезультатПроверки.Протокол);
		ОбработкаНеисправностейБЭД.ДобавитьОшибку(
			КонтекстДиагностики, Ошибка, ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().Криптография);
	КонецЕсли;
	...
	Исключение
		ИмяСобытия = ?(НастройкиВызова.ЭтоЭлектроннаяПодписьВМоделиСервиса,
			НСтр("ru = 'Электронная подпись в модели сервиса.Сервис криптографии.Чтение PKCS7';
				|en = 'Digital signature in SaaS mode.Cryptography service.Reading PKCS7'", ОбщегоНазначения.КодОсновногоЯзыка()),
			НСтр("ru = 'Сервис криптографии.Чтение PKCS7';
				|en = 'Cryptography service.Reading PKCS7'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(
			ИмяСобытия,
			УровеньЖурналаРегистрации.Ошибка,,,
			...
			Исключение
				ИмяСобытия = ?(НастройкиВызова.ЭтоЭлектроннаяПодписьВМоделиСервиса,
					НСтр("ru = 'Электронная подпись в модели сервиса.Сервис криптографии.Чтение сертификата';
						|en = 'Digital signature in SaaS mode.Cryptography service.Reading certificate'", ОбщегоНазначения.КодОсновногоЯзыка()),
					НСтр("ru = 'Сервис криптографии.Чтение сертификата';
						|en = 'Cryptography service.Reading certificate'", ОбщегоНазначения.КодОсновногоЯзыка()));
				ЗаписьЖурналаРегистрации(
					ИмяСобытия,
					УровеньЖурналаРегистрации.Ошибка,,,
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПолучитьДистрибутивCryptoProCSP(ПараметрыЗагрузки, АдресРезультата, АдресДополнительногоРезультата) Экспорт
	...
	ВидОперации = НСтр("ru = 'Получение серийного номера CryptoPRO CSP';
						|en = 'Receiving CryptoPRO CSP serial number'");
	...
	Если РезультатВызова.Успех Тогда
		...
		Иначе
			Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации,
				...
				|en = 'Error when receiving CryptoPro CSP serial number, see event log for details'"));
	...
	Иначе
		...
		КраткоеПредставлениеОшибки = НСтр("ru = 'Сервис получения серийных номеров для CryptoPRO CSP временно недоступен. Повторите попытку позже.';
											|en = 'The service of getting serial numbers for CryptoPRO CSP is temporary unavailable. Try again later.'");
		КонтекстДиагностики.ЗаголовокОперации = НСтр("ru = 'При получении серийного номера CryptoPRO CSP';
													|en = 'When receiving CryptoPRO CSP serial number'");
