...
Процедура ПолучитьВыписку(Оповещение, БанковскийСчет, ДатаНачала, Знач ДатаОкончания) Экспорт
	...
	ОбработатьПолучениеВыписки(Оповещение, ПараметрыОбмена, ДатаНачала, ДатаОкончания);
...
//  ДатаОкончания - Дата - окончание периода запроса.
//
Процедура ПолучитьВыпискуПоНастройкеОбмена(Оповещение, НастройкаОбмена, ДатаНачала, Знач ДатаОкончания) Экспорт
	...
	ПараметрыОбмена = ОбменСБанкамиСлужебныйВызовСервера.ПараметрыОбменаПоНастройкеОбмена(НастройкаОбмена);
...
Процедура ПриНажатииНаГиперссылку(ДокументСсылка, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли РеквизитыЭД.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияОбменСБанками.ТребуетсяПодтверждение") Тогда
		Если РеквизитыЭД.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.АсинхронныйОбмен") Тогда
			...
			МассивОбъектов.Добавить(РеквизитыЭД.СообщениеОбмена);
			ЗначенияРеквизитовОбъектов = ОбменСБанкамиСлужебныйВызовСервера.ЗначенияРеквизитовОбъектов(МассивОбъектов, "ВидЭД");
			ЗначенияРеквизитов = ЗначенияРеквизитовОбъектов.Получить(РеквизитыЭД.СообщениеОбмена);
			ВидЭД = ЗначенияРеквизитов["ВидЭД"];
			ДоступноПодписаниеПоSMS = ОбменСБанкамиСлужебныйВызовСервера.ДоступноПодписаниеПоSMS(ВидЭД, РеквизитыЭД.НастройкаОбмена);
			Если ДоступноПодписаниеПоSMS Тогда
				Если ЕстьПравоОбработкиЭлектронныхДокументов(РеквизитыЭД.ЕстьПравоПравоОбработкиЭД) Тогда
					СформироватьПодписатьОтправитьЭД(ДокументСсылка);
				КонецЕсли;
			Иначе
...
Процедура ОбработатьПолучениеВыписки(Оповещение, ПараметрыОбмена, ДатаНачала, ДатаОкончания)
	...
	ЗапросВыпискиТребуетЭП = РеквизитыНастройкиОбмена.НастройкиПодписанияПоВидамЭД[
								ПредопределенноеЗначение("Перечисление.ВидыЭДОбменСБанками.ЗапросВыписки")] = 
								ПредопределенноеЗначение("Перечисление.СпособыПодтвержденияДокументовОбменСБанками.ЭлектроннаяПодпись");
	...
	ИначеЕсли РеквизитыНастройкиОбмена.АутентификацияПоСертификату ИЛИ ЗапросВыпискиТребуетЭП Тогда
		Оповещение = Новый ОписаниеОповещения(
...
Процедура ПолучитьВыпискуСбербанк(Оповещение, НастройкаОбмена, РеквизитыНастройкиОбмена, Параметры)
	...
	Если ГотовыеВыписки.Количество() Тогда
		...
		ТекстВопроса = НСтр("ru = 'В базе уже есть выписки банка за указанный период.
								...
								|en = 'Bank statements for the specified period already exist in the base.
								|Import statements from the base or receive new ones from the bank?'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(Истина, НСтр("ru = 'Загрузить из базы';
									|en = 'Import from database'"));
		Кнопки.Добавить(Ложь, НСтр("ru = 'Получить из банка';
									|en = 'Receive from bank'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		Заголовок = НСтр("ru = 'Выбор способа получения выписки';
						|en = 'Select statement receipt method'");
...
Функция ЕстьПравоОбработкиЭлектронныхДокументов(ЕстьПравоПравоОбработкиЭД)
	...
	Если НЕ ЕстьПравоПравоОбработкиЭД Тогда
		ТекстСообщения = НСтр("ru = 'Недостаточно прав для обработки электронных документов.
									|Обратитесь к администратору.';
									|en = 'Not enough rights to process electronic documents.
