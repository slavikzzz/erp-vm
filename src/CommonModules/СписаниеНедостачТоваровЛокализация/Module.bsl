...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	ТекстыОтражения.Добавить(ПроизводствоБезЗаказаЛокализация.ТекстСписаниеТоваровНаСтатьиАктивовПассивов()); // Списание на прочие активы собственных товаров (Дт ХХ.ХХ :: Кт 20, 41.01, 76.06)
	ТекстыОтражения.Добавить(ТекстВключениеИсключениеНДСВСтоимостьРасходовАктивовПассивов()); // Включение/исключение НДС в стоимость товара на складе - получателе (Дт 08, 25, 26, 44 :: Кт 19.03) @НДСПредъявленный
	...
	ТекстыОтражения.Добавить(ТекстПеремещениеТоваровЗаБалансом()); // Списание товаров, принятых на ответственное хранение (Дт 002, 003, 004.01 :: Кт 002, 003, 004.01)
...
Функция ТекстВключениеИсключениеНДСВСтоимостьРасходовАктивовПассивов()
	
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Включение/исключение НДС в стоимость товара на складе - получателе (Дт 08, 25, 26, 44 :: Кт 19.03) @НДСПредъявленный
	|
	...
	|			И Статьи.ВидЦенностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ПрочиеРаботыИУслуги)
	...
	|
	|	ЕСТЬNULL(НастройкиСчетовУчета.Субконто1, Операция.СтатьяРасходов) КАК СубконтоДт1,
	|	ЕСТЬNULL(НастройкиСчетовУчета.Субконто2, Операция.АналитикаРасходов) КАК СубконтоДт2,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Статьи.ВариантРаспределенияРасходовРегл, ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.ПустаяСсылка))
	...
	|		ИНАЧЕ ЕСТЬNULL(НастройкиСчетовУчета.Субконто3, ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.Прочее))
	...
	|	ВЫБОР
	|		КОГДА Статьи.Ссылка ЕСТЬ NULL
	|		ТОГДА ""Включение/исключение НДС в стоимость актива""
	|		ИНАЧЕ ""Включение/исключение НДС в стоимость товара""
	|	КОНЕЦ КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеНедостачТоваров КАК Операция
	...
	|	ПО Статьи.Ссылка = Операция.СтатьяРасходов
	|
	...
	|	ПО Операция.НастройкаСчетовУчета = НастройкиСчетовУчета.Ссылка
...
Функция ТекстПеремещениеТоваровЗаБалансом()
	...
	|		ТОГДА ""Списание принятых на хранение товаров""
