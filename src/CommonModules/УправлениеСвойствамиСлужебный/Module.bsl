		...
		Для каждого Свойство Из Свойства Цикл
			Если Не Свойство.ЭтоДополнительноеСведение Тогда
				...
				Если ТипЗнч(Значения) = Тип("Массив") И Значения.Количество() > 0 Тогда
					СтрокаИнфоПроКолонки.Примечание = СтрокаИнфоПроКолонки.Примечание  + Символы.ПС + НСтр("ru = 'Варианты значений:';
																											|en = 'Available values:'") + Символы.ПС;
	...
	Для каждого НеобработанныйДубль Из НеобработанныеЗначенияОригиналов Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Поиск и замена ссылок';
											|en = 'Reference search and replacement'", ОбщегоНазначения.КодОсновногоЯзыка()),
