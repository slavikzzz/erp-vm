...
Функция УстановитьСоединениеССерверомМеханизмаОнлайнСервисовРО(ОписаниеОшибки = Неопределено, Таймаут = 120) Экспорт
	...
	Исключение 
		...
		Попытка
			...
			// переключаемся на защищенный протокол.
			Адрес = "https://" + URLСервера;
			ОписаниеОшибки = "";
			Соединение = ДокументооборотСКО.УстановитьСоединениеССерверомИнтернета(Адрес, ОписаниеОшибки, Таймаут);
			Если Соединение = Неопределено Тогда
				ЗаписатьСобытиеВЖурналОнлайнСервисы(УровеньЖурналаРегистрации.Ошибка, ОписаниеОшибки);
			КонецЕсли;
			Возврат Соединение;
		...
		Исключение
			...
			Шаблон = НСтр("ru = 'Ошибка установки соединения с сервером %1:';
							|en = 'Ошибка установки соединения с сервером %1:'") + Символы.ПС + "%2";
			КраткоеПредставлениеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
			ОписаниеОшибки = СтрШаблон(Шаблон,
				Соединение.Сервер, КраткоеПредставлениеОшибки);
			ПодробноеОписание = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
			ПодробноеОписание = СтрШаблон(Шаблон,
				Соединение.Сервер, ПодробноеОписание);
			ЗаписатьСобытиеВЖурналОнлайнСервисы(УровеньЖурналаРегистрации.Ошибка, ПодробноеОписание);
...
Функция ПолучитьРесурсССервера(Соединение, ИмяРесурсаНаСервере, ОписаниеОшибки = Неопределено) Экспорт
	...
	Попытка
		Ответ = Соединение.Получить(ПутьККаталогуРесурсов + ИмяРесурсаНаСервере, ИмяВременногоФайла);
	Исключение
		
		Если Соединение.ЗащищенноеСоединение = Неопределено Тогда
			Соединение = ОбработатьОшибкуИСоздатьЗащищенноеСоединение(Соединение, ПутьККаталогуРесурсов, ИмяРесурсаНаСервере);
			Если Соединение = Неопределено Тогда
				Возврат Неопределено;
			КонецЕсли;
			Возврат ПолучитьРесурсССервера(Соединение, ИмяРесурсаНаСервере, ОписаниеОшибки);
		КонецЕсли;
		
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		Шаблон = НСтр("ru = 'Ошибка получения ресурса %1 с сервера %2:';
						|en = 'Ошибка получения ресурса %1 с сервера %2:'") + Символы.ПС + "%3";
		КраткоеПредставлениеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
		ОписаниеОшибки = СтрШаблон(Шаблон,
			ПутьККаталогуРесурсов + ИмяРесурсаНаСервере, Соединение.Сервер, КраткоеПредставлениеОшибки);
		ПодробноеОписание = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		ПодробноеОписание = СтрШаблон(Шаблон,
			ПутьККаталогуРесурсов + ИмяРесурсаНаСервере, Соединение.Сервер, ПодробноеОписание);
		ЗаписатьСобытиеВЖурналОнлайнСервисы(УровеньЖурналаРегистрации.Ошибка, ПодробноеОписание);
	...
	Если НЕ Ответ.КодСостояния = 200 Тогда
		...
		Шаблон = НСтр("ru = 'Ошибка получения ресурса %1 с сервера %2, код состояния %3.';
						|en = 'Ошибка получения ресурса %1 с сервера %2, код состояния %3.'");
		ОписаниеОшибки = СтрШаблон(Шаблон,
			ПутьККаталогуРесурсов + ИмяРесурсаНаСервере, Соединение.Сервер, Ответ.КодСостояния);
		ДвоичныеДанные = Новый ДвоичныеДанные(ИмяВременногоФайла);
		ТекстОтвета = ПолучитьСтрокуИзДвоичныхДанных(ДвоичныеДанные);
		Если НЕ ПустаяСтрока(ТекстОтвета) Тогда
			ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + НСтр("ru = 'Текст ответа:';
																|en = 'Текст ответа:'") + Символы.ПС + ТекстОтвета;
		КонецЕсли;
		ЗаписатьСобытиеВЖурналОнлайнСервисы(УровеньЖурналаРегистрации.Ошибка, ОписаниеОшибки);
		Возврат Неопределено;
...
Функция ОбработатьОшибкуИСоздатьЗащищенноеСоединение(Знач Соединение, ПутьККаталогуРесурсов, ИмяРесурсаНаСервере)
	
	Сервер = Соединение.Сервер;
	Шаблон = НСтр("ru = 'Не удалось получить данные ресурса %1 с сервера %2 по протоколу http, переключение на https.';
					|en = 'Не удалось получить данные ресурса %1 с сервера %2 по протоколу http, переключение на https.'");
	ОписаниеОшибкиЖурнал = СтрШаблон(Шаблон,
				ПутьККаталогуРесурсов + ИмяРесурсаНаСервере, Сервер);
	ЗаписатьСобытиеВЖурналОнлайнСервисы(УровеньЖурналаРегистрации.Информация, ОписаниеОшибкиЖурнал);
	
	// переключаемся на защищенный протокол.
	Адрес = "https://" + Соединение.Сервер;
	ОписаниеОшибки = "";
	Соединение = ДокументооборотСКО.УстановитьСоединениеССерверомИнтернета(Адрес, ОписаниеОшибки, Соединение.Таймаут);
	Если Соединение = Неопределено Тогда
		Шаблон = НСтр("ru = 'Ошибка получения ресурса %1 с сервера %2:';
						|en = 'Ошибка получения ресурса %1 с сервера %2:'") + Символы.ПС + "%3";
		ОписаниеОшибки = СтрШаблон(Шаблон,
					ПутьККаталогуРесурсов + ИмяРесурсаНаСервере, Сервер, ОписаниеОшибки);
		ЗаписатьСобытиеВЖурналОнлайнСервисы(УровеньЖурналаРегистрации.Ошибка, ОписаниеОшибки);
		Возврат Неопределено;
	КонецЕсли;
...
Функция ЗапуститьФоновоеЗаданиеАктуализироватьСлужебнуюИнформациюМеханизмаОнлайнСервисовРО(УникальныйИдентификаторФормы, МожноЗагружатьВнешниеФормыОтчетов = Ложь) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Производится обновление служебной информации из Интернет...';
															|en = 'Производится обновление служебной информации из Интернет...'");
...
#КонецОбласти 
	...
	Если НЕ ЗначениеЗаполнено(РезультатРазбораКлючевогоФайла) Тогда
		ОписаниеОшибки = "Некорректный формат ключевого файла или ошибка загрузки" + ?(ЗначениеЗаполнено(ОписаниеОшибки), ": " + ОписаниеОшибки, ".");
		Если УведомлятьОбОшибках Тогда
			ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки);
		КонецЕсли;
...
//	ФлагСчитатьНаступившимИлиДатаИлиКлючФлага
//									- Булево - считать событие наступившим, если оно не найдено или произошла ошибка
//											   получения данных.
//									- Дата   - дата, начиная с которой считать событие наступивщим, если не найдено/ошибка.
//									- Строка - ключ хранилища общих настроек, при значении Истина событие считается наступившим.
// Возвращаемое значение:
// 	Булево - Истина, если событие наступило.
Функция СобытиеНаступило(Знач ИмяСобытия, Знач ФлагСчитатьНаступившимИлиДатаИлиКлючФлага = Ложь) Экспорт
	
	Если ТипЗнч(ФлагСчитатьНаступившимИлиДатаИлиКлючФлага) = Тип("Дата") Тогда
		ФлагСчитатьНаступившимИлиДатаИлиКлючФлага = ТекущаяДатаСеанса() >= ФлагСчитатьНаступившимИлиДатаИлиКлючФлага;
		
	ИначеЕсли ТипЗнч(ФлагСчитатьНаступившимИлиДатаИлиКлючФлага) = Тип("Строка") Тогда
		Если ХранилищеОбщихНастроек.Загрузить(ФлагСчитатьНаступившимИлиДатаИлиКлючФлага) = Истина Тогда
			Возврат Истина;
		Иначе
			ФлагСчитатьНаступившимИлиДатаИлиКлючФлага = Ложь;
		КонецЕсли;
	КонецЕсли;
	
	Если ТипЗнч(ИмяСобытия) = Тип("Массив") ИЛИ ТипЗнч(ИмяСобытия) = Тип("ФиксированныйМассив") Тогда
		МассивИменСобытий = ИмяСобытия;
		Если ФлагСчитатьНаступившимИлиДатаИлиКлючФлага Тогда
			Для каждого ЭлементМассива Из МассивИменСобытий Цикл
				Если НЕ СобытиеНаступило(ЭлементМассива, ФлагСчитатьНаступившимИлиДатаИлиКлючФлага) Тогда
					Возврат Ложь;
				КонецЕсли;
			КонецЦикла;
			Возврат Истина;
		...
		Иначе
			Для каждого ЭлементМассива Из МассивИменСобытий Цикл
				Если СобытиеНаступило(ЭлементМассива, ФлагСчитатьНаступившимИлиДатаИлиКлючФлага) Тогда
					Возврат Истина;
				КонецЕсли;
			КонецЦикла;
			Возврат Ложь;
	...
	Возврат СведенияОСобытии.СобытиеНаступило ИЛИ (ФлагСчитатьНаступившимИлиДатаИлиКлючФлага
...
Функция СобытиеНаступилоРасширенная(Знач ИмяСобытия, Знач Актуализировать = Ложь) Экспорт
	...
	Результат.Вставить("ДатаНачала", ПустаяДата);
	Результат.Вставить("ДатаОкончания", ПустаяДата);
...
Процедура ОбработатьИЗаписатьВнешнийОтчетВСправочникРО(ПараметрыВнешнейФормы, ПутьКФайлу) Экспорт
	...
	ФайлАрхива = ПутьКФайлу;
	Если ТипЗнч(ПутьКФайлу) = Тип("ДвоичныеДанные") Тогда
		ФайлАрхива = ПутьКФайлу.ОткрытьПотокДляЧтения();
	КонецЕсли;
	
	Попытка
		
		Архиватор = Новый ЧтениеZipФайла(ФайлАрхива);
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Не удалось распаковать архив с внешней формой отчета.';
							|en = 'Не удалось распаковать архив с внешней формой отчета.'");
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка внешних регламентированных отчетов';
										|en = 'Загрузка внешних регламентированных отчетов'"), УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
	...
	Если СвойстваМанифеста["ИДКонфигурации"] <> СвойстваКонфигурации["ИДКонфигурации"] Тогда
		ТекстОшибки = НСтр("ru = 'Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору текущей конфигурации';
							|en = 'Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору текущей конфигурации'");
	...
	Если СвойстваМанифеста["ВерсияКонфигурации"] <> СвойстваКонфигурации["ВерсияКонфигурации"] Тогда
		ТекстОшибки = НСтр("ru = 'Не пройдена проверка на соответствие версии в отчете версии текущей конфигурации.';
							|en = 'Не пройдена проверка на соответствие версии в отчете версии текущей конфигурации.'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыВнешнейФормы.Отчет) Тогда
		ПараметрыВнешнейФормы.Отчет = СвойстваМанифеста["ИсточникОтчета"];
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыВнешнейФормы.ОтчетСсылка) Тогда
		ПараметрыВнешнихФорм = Новый Массив;
		ПараметрыВнешнихФорм.Добавить(ПараметрыВнешнейФормы);
		
		НазначитьЭлементыСправочникаРОВнешнимФормамРО(ПараметрыВнешнихФорм);
	КонецЕсли;
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Получение объекта отчета по ссылке.
			|Описание:
			|""%1""';
			|en = 'Получение объекта отчета по ссылке.
			|Описание:
	...
	Если НЕ ВнешнийОтчетИспользовать Тогда 
		...
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Внешний отчет ""%1"" вер. ""%2"" отключен.';
				|en = 'Внешний отчет ""%1"" вер. ""%2"" отключен.'"), СвойстваМанифеста["Имя"], СвойстваМанифеста["КраткаяВерсияВнешнегоОтчета"]);
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		...
		Если РезультатСравнениеВерсийХранимогоИЗагружаемогоОтчетов  = -1 Тогда // если загружаемый отчет старее хранимого
			ТекстОшибки = НСтр("ru = 'Не пройдена проверка на сравнение версий манифеста выбранного и хранимого отчетов';
								|en = 'Не пройдена проверка на сравнение версий манифеста выбранного и хранимого отчетов'");
	...
	Если СвойстваКонфигурации["ИДКонфигурации"] <> СвойстваОтчета["ИДКонфигурации"] Тогда
		ТекстОшибки = НСтр("ru = 'Не пройдена проверка на соответствие идентификатора конфигурации идентификатору конфигурации в текущем подключаемом отчете';
							|en = 'Не пройдена проверка на соответствие идентификатора конфигурации идентификатору конфигурации в текущем подключаемом отчете'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		Если СвойстваОтчета["ВерсияКонфигурации"] <> СвойстваКонфигурации["ВерсияКонфигурации"] Тогда
			ТекстОшибки = НСтр("ru = 'Не пройдена проверка на соответствие версий текущего и подключаемого отчетов';
								|en = 'Не пройдена проверка на соответствие версий текущего и подключаемого отчетов'");
			ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ВРег(СвойстваОтчета["ИсточникОтчета"]) <> ВРег(СвойстваКонфигурации["ИсточникОтчета"]) Тогда
		ТекстОшибки = НСтр("ru = 'Не пройдена проверка на соответствие имен метаданных текущего и подключаемого отчетов';
							|en = 'Не пройдена проверка на соответствие имен метаданных текущего и подключаемого отчетов'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		...
		Если РезультатСравнениеВерсийХранимогоИЗагружаемогоОтчетов  = -1 Тогда // если загружаемый отчет старее хранимого
			ТекстОшибки = НСтр("ru = 'Не пройдена проверка на сравнение версий выбранного и хранимого отчетов';
								|en = 'Не пройдена проверка на сравнение версий выбранного и хранимого отчетов'");
			ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		ТекстИнформации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Внешний отчет вер. ""%1"" успешно загружен.';
				|en = 'Внешний отчет вер. ""%1"" успешно загружен.'"), СвойстваОтчета["КраткаяВерсияВнешнегоОтчета"]);
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Запись объекта отчета с новыми данными.
			|Описание:
			|""%1""';
			|en = 'Запись объекта отчета с новыми данными.
			|Описание:
			|""%1""'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
...
Функция ПрочитатьМанифест(КаталогРаспаковки)
	...
	Если Чтение.ТипУзла <> ТипУзлаXML.НачалоЭлемента ИЛИ Чтение.Имя <> "Info" Тогда
		ВызватьИсключение(НСтр("ru = 'Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента Info.';
								|en = 'Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента Info.'"));
	КонецЕсли;
	
	Если НЕ Чтение.Прочитать() Тогда
		ВызватьИсключение(НСтр("ru = 'Ошибка чтения XML. Обнаружено завершение файла.';
								|en = 'Ошибка чтения XML. Обнаружено завершение файла.'"));
	КонецЕсли;
...
Функция ФайлОтчетаИзАрхива(КаталогРаспаковки)
	...
	Если НайденныеФайлы.Количество() = 0 Тогда
		ВызватьИсключение(НСтр("ru = 'Ошибка получения отчета. Архив не содержит файл с расширением ""*.erf"".';
								|en = 'Ошибка получения отчета. Архив не содержит файл с расширением ""*.erf"".'"));
	Иначе
...
Процедура ЗаписатьВЖурналРегистрации(Знач Уровень, ТекстОшибки) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка внешних регламентированных отчетов';
									|en = 'Загрузка внешних регламентированных отчетов'"), УровеньВажности,,, ТекстОшибки);
...
Процедура ЗаписатьСобытиеВЖурналОнлайнСервисы(Уровень, ТекстСобытия, Данные = Неопределено)
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Онлайн сервисы регламентированной отчетности';
				|en = 'Онлайн сервисы регламентированной отчетности'");
