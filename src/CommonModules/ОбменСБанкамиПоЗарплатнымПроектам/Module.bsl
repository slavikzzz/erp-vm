...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Если Не (ПравоДоступа("Чтение", Метаданные.Справочники.ЗарплатныеПроекты) 
		И (ИсточникиПоЗарплатнымПроектам(Источники) Или ДополнительныеИсточникиДляВыгрузкиФайлов(Источники))) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВыгрузитьФайлыОбменаСБанкомПоВедомости(СтруктураПараметровДляФормированияФайла) Экспорт
		...
		Если ЗначениеЗаполнено(Выборка.Состояние) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По документу %1 уже получено подтверждение о зачислении зарплаты';
					|en = 'Payroll crediting confirmation for the %1 document has already been received'"),
				Выборка.Ведомость);
		...
		Если Не Выборка.Проведен Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Документ %1 не проведен.';
						|en = 'Document %1 is not posted.'"), Выборка.Ведомость);
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, Выборка.Ведомость,,, Отказ);
		КонецЕсли;
		Если ЗарплатныйПроект = Неопределено Тогда
			...
			Если ЗначениеЗаполнено(Выборка.ЗарплатныйПроект) Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'По зарплатному проекту %1 не используется обмен электронными документами с банком.';
						|en = 'Exchange of electronic documents with bank is not used for payroll card program %1.'"),
					Выборка.ЗарплатныйПроект);
			Иначе
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В документе %1 не выбран зарплатный проект.';
						|en = 'Payroll card program is not selected in document %1.'"), Выборка.Ведомость);
			КонецЕсли;
	...
	Если СтруктураПараметровДляФормированияФайла.ПлатежныйДокумент <> Неопределено Тогда
		Если НЕ СтруктураПараметровДляФормированияФайла.ПлатежныйДокумент.Проведен Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Документ %1 не проведен.';
						|en = 'Document %1 is not posted.'"), СтруктураПараметровДляФормированияФайла.ПлатежныйДокумент);
		...
		Если Не ВДокументахОдинЗарплатныйПроект Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В документе %1 присутствуют ведомости с разными зарплатными проектами. Файл по документу не сформирован.';
						|en = 'There are paysheets with different payroll card programs in document %1. The document file is not generated.'"),
...
Процедура ГруппаПодтверждениеИзБанкаДополнитьФорму(Форма) Экспорт
	...
	Если СписокДокументовПодтверждений.Количество() > 0 Тогда
		...
		Форма.ПодтверждениеИнфоНадпись = НСтр("ru = 'По документу получены подтверждения из банка, редактирование невозможно';
												|en = 'Bank confirmation has already been received for the document, therefore it cannot be modified.'");
	...
	Иначе
		...
		Форма.ПодтверждениеИнфоНадпись = НСтр("ru = 'По документу не получены подтверждения из банка';
												|en = 'No bank confirmations for this document'");
...
Процедура ПриПолученииXMLФайла(АдресДанныхФайла, ИмяФайла, ОбъектВладелец, ДанныеОповещения) Экспорт
	...
	Для Каждого СозданныйДокумент Из СозданныеДокументы Цикл
		...
		Если СозданныйДокумент = Неопределено Или СозданныйДокумент.Ключ = Неопределено Тогда
	...
	Если ПриЗагрузкеБылиОшибки Тогда 
		ВидОперации = НСтр("ru = 'Загрузка подтверждения из банка';
							|en = 'Import bank confirmation'");
		ПодробныйТекстОшибки = НСтр("ru = 'Не удалось создать документ подтверждения';
									|en = 'Cannot create the confirmation document'");
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, ТекстСообщения, "ОбменСБанками");
	Иначе
...
Процедура ПриЗаписиПлатежногоДокументаПеречисленияЗарплаты(Источник, Отказ) Экспорт
	...
	ДанныеПлатежногоДокумента = Новый Структура("Дата,Номер");
	
	ЗаполнитьДанныеПлатежногоДокумента(ДанныеПлатежногоДокумента, Источник);
	
	НоваяЗаписьНабора.Номер = ДанныеПлатежногоДокумента.Номер;
	НоваяЗаписьНабора.Дата = ДанныеПлатежногоДокумента.Дата;
...
Процедура ПроверитьЗаполнениеПлатежногоДокумента(ПлатежныйДокумент, МассивВедомостей, МассивОшибок) Экспорт
	...
	Если Не ДоступностьПлатежногоДокумента(ПлатежныйДокумент) Тогда
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'По документу %1 приняты файлы подтверждения зачисления зарплаты. Изменение документа невозможно.';
				|en = 'Files which confirm salary crediting were received against document %1. You cannot change the document.'"), 
			ПлатежныйДокумент);
	...
	Если Результат.Количество() > 1 Тогда
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'В документе %1 должны присутствовать ведомости только из одной организации и зарплатного проекта.';
				|en = 'The %1 document should contain paysheets only from one company and payroll card program.'"), 
			ПлатежныйДокумент);
		МассивОшибок.Добавить(Новый Структура("ТекстСообщения, Ведомость", ТекстОшибки, Неопределено));
	КонецЕсли;
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Ведомость %1 не проведена.';
									|en = 'Paysheet %1 is not posted.'"), Выборка.Ведомость);
		МассивОшибок.Добавить(Новый Структура("ТекстСообщения, Ведомость", ТекстОшибки, Выборка.Ведомость));
	КонецЦикла;
...
Процедура ПредупредитьЗарплатныйПроектИспользуется(ЗарплатныйПроект) Экспорт
	...
	ОбменСБанкамиПоЗарплатнымПроектамВнутренний.ПредупредитьЗарплатныйПроектИспользуется(ЗарплатныйПроект);
		...
		Если Выборка.ПодтверждениеПолучено Тогда
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'По документу %1 получено подтверждение из банка. Изменение документа невозможно.';
					|en = 'Bank confirmation has already been received for document %1, therefore it cannot be modified.'"),
...
Функция ТипыДокументовВыгружаемыхВБанк()
	...
	Типы.Добавить(Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыВБанк"));
	Типы.Добавить(Тип("ДокументСсылка.ВедомостьПрочихДоходовВБанк"));
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Начальная установка настройки Максимальная длина имени держателя карты.';
									|en = 'Initial setting of Maximum card holder name length.'");
	...
	Если Отказ Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
			...
			НСтр("ru = 'По документам получены подтверждения из банка. Изменение документов невозможно.';
				|en = 'Bank confirmation has already been received for the documents, therefore they cannot be modified.'"));
		Возврат;
	КонецЕсли;
						...
						ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
							...
							СтрШаблон(
								НСтр("ru = 'У сотрудника %1 неверный номер лицевого счета, он должен содержать 20 цифр - %2.';
									|en = 'Incorrect account number of employee %1, it should contain 20 digits - %2.'"), 
							...
							НСтр("ru = 'В строке %1 неверный номер лицевого счета, он должен содержать 20 цифр.';
								|en = 'Invalid personal account number in line %1; it should contain 20 digits.'"));
					КонецЕсли;
				КонецЕсли;
...
Функция РезультатВыгрузкиФайловОбменаСБанком(Форма, МассивДокументов) Экспорт
	...
	НаименованиеЗадания = НСтр("ru = 'Выгрузка файлов для обмена с банком';
								|en = 'Export files for bank exchange'");
	...
	Для Каждого КорневоеСвойствоXDTO Из Пакет.КорневыеСвойства Цикл
		...
		Для Каждого ЗагружаемоеСвойство Из ОбъектXDTO.Свойства() Цикл
			...
			Если ЗначениеЗаполнено(СтруктураДанныхДляЗаполненияДокумента.ЗарплатныйПроект) Тогда
				ФорматФайла = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураДанныхДляЗаполненияДокумента.ЗарплатныйПроект, "ФорматФайла");
			Иначе
				ФорматФайла = ПредопределенноеЗначение("Перечисление.ФорматыФайловОбменаПоЗарплатномуПроекту.Версия36");
			КонецЕсли;
			...
			Если ЗначениеОбязательноДляЗаполнения И ЗагружаемоеСвойствоОбъектаXDTO = Неопределено Тогда
				ТекстОшибки = СтрШаблон(
					НСтр("ru = 'В файле подтверждения не указано обязательное значение поля %1.';
						|en = 'Required value of the %1 field is not specified in the confirmation file.'"),
...
Функция ПредставлениеУдостоверенияЛичностиИзЭДXML(УдостоверениеЛичности)
	...
	Если ЗначениеЗаполнено(УдостоверениеЛичности.Серия) Тогда
		Представление.Добавить(СтрШаблон(НСтр("ru = 'серия: %1';
												|en = 'batch: %1'"), УдостоверениеЛичности.Серия))
	КонецЕсли;
	Если ЗначениеЗаполнено(УдостоверениеЛичности.Номер) Тогда
		Представление.Добавить(СтрШаблон(НСтр("ru = '№ %1';
												|en = 'No. %1'"), УдостоверениеЛичности.Номер))
	КонецЕсли;
	Если ЗначениеЗаполнено(УдостоверениеЛичности.ДатаВыдачи) Тогда
		Представление.Добавить(СтрШаблон(НСтр("ru = 'выдан: %1 года';
												|en = 'issued: %1'"), Формат(УдостоверениеЛичности.ДатаВыдачи, "ДЛФ=ДД")))
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(УдостоверениеЛичности.КодПодразделения) 
		...
		Представление.Добавить(СтрШаблон(НСтр("ru = '№ подр. %1';
												|en = 'Dept. No.%1'"), УдостоверениеЛичности.КодПодразделения))
	КонецЕсли;	
...
Функция ДополнительныеИсточникиДляВыгрузкиФайлов(Источники)
	Возврат ОбменСБанкамиПоЗарплатнымПроектамВнутренний.ДополнительныеИсточникиДляВыгрузкиФайлов(Источники);
КонецФункции

Процедура ЗаполнитьДанныеПлатежногоДокумента(ДанныеПлатежногоДокумента, ПлатежныйДокумент)
	
	СтандартнаяОбработка = Истина;
	ОбменСБанкамиПоЗарплатнымПроектамПереопределяемый.ЗаполнитьДанныеПлатежногоДокумента(
		ДанныеПлатежногоДокумента, ПлатежныйДокумент, СтандартнаяОбработка);
	
	Если Не СтандартнаяОбработка Тогда
		Возврат;
	КонецЕсли;

	ДанныеПлатежногоДокумента.Дата  = ПлатежныйДокумент.Дата;
	ДанныеПлатежногоДокумента.Номер = ПлатежныйДокумент.Номер;
