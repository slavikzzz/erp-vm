	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		Если РаботаВМоделиСервиса.СеансЗапущенБезРазделителей() Тогда
			...
			ТекстСообщения = НСтр("ru = 'Вызов ОбработкаНовостейУТ.ЗаполнитьОтборыНовостей() в модели сервиса в неразделённом сеансе не поддерживается.';
									|en = 'You cannot call ОбработкаНовостейУТ.ЗаполнитьОтборыНовостей() in SaaS in the undivided session.'");
			// Запись в журнал регистрации
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Новости.Переопределяемый.Обновление ИБ.ЗаполнитьОтборыНовостей';
					|en = 'Новости.Переопределяемый.Обновление ИБ.ЗаполнитьОтборыНовостей'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), // ИмяСобытия
...
Функция ТаблицаПодсистемИРолейДляПросмотра(ПроверятьРольТекущегоПользователя)
	...
	НоваяСтрока.МетаданныеРоли = Метаданные.Роли.ПодсистемаМеждународныйФинансовыйУчет;
	//-- НЕ УТКА
	...
	НоваяСтрока.МетаданныеРоли = Метаданные.Роли.ПодсистемаПроизводство;
	
	НоваяСтрока = ТаблицаПодсистем.Добавить();
	НоваяСтрока.Подсистема = Метаданные.Подсистемы.РегламентированныйУчет.Имя;
	НоваяСтрока.МетаданныеРоли = Метаданные.Роли.ПодсистемаРегламентированныйУчет;
	//-- НЕ УТ
