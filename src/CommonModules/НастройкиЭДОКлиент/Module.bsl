...
Процедура ОжидатьЗавершенияВыполненияДействийПередИзменениемИспользованияУтверждения(ДлительнаяОперация, Оповещение) Экспорт
	...
	ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Выполняется изменение опции ""Отправлять входящие документы на утверждение"".';
											|en = 'Changing the ""Send incoming documents for approval"" option.'");
...
Процедура ЗапроситьПодтверждениеОтключенияОжиданияОтветнойПодписи(Оповещение) Экспорт
	...
	ТекстВопроса = НСтр("ru = 'Отказ от ответной подписи разрешается, если:
		...
		|en = 'Waiver of the response signature is permitted if: 
		|- the document is made for provision of services for which RF CC does not require certification;
		|- the agreement on electronic document exchange stipulates absence of the response signature for this kind of the document.
		|
		| If the document does not fit into the listed criteria, waiver of waiting for the response signature will be considered as invalid.
		|
		|Do you really want to waive waiting for the response signature?'");
...
// Возвращаемое значение:
//  - Неопределено
//  - См. ОтражениеВУчетеЭДОКлиент.НовыеПараметрыФормыНастройкиОтраженияВУчете
Функция НовыеПараметрыФормыНастройкиОтраженияВУчете() Экспорт
	Если ИнтеграцияЭДОКлиент.ЕстьПодсистемаОтражениеВУчетеЭДО() Тогда
		Возврат ИнтеграцияЭДОКлиент.ОбщийМодульОтражениеВУчетеЭДОКлиент().НовыеПараметрыФормыНастройкиОтраженияВУчете();
	Иначе
...
Процедура ОткрытьНастройкуОтраженияВУчете(ПараметрыФормы, ПараметрыОткрытияФормы = Неопределено) Экспорт
	Если ИнтеграцияЭДОКлиент.ЕстьПодсистемаОтражениеВУчетеЭДО() Тогда
		ИнтеграцияЭДОКлиент.ОбщийМодульОтражениеВУчетеЭДОКлиент().ОткрытьНастройкуОтраженияВУчете(
...
Процедура ОткрытьНастройкуОтраженияВУчетеПоКлючу(КлючНастроек) Экспорт
	...
	ПараметрыФормы = ОбщийМодульОтражениеВУчетеЭДОКлиент.НовыеПараметрыФормыНастройкиОтраженияВУчете();
	...
	ОбщийМодульОтражениеВУчетеЭДОКлиент.ОткрытьНастройкуОтраженияВУчете(ПараметрыФормы);
...
Процедура ОткрытьНастройкиОтраженияВУчете(Контрагент) Экспорт
	...
	Если ИнтеграцияЭДОКлиент.ЕстьПодсистемаОтражениеВУчетеЭДО() Тогда
		ИнтеграцияЭДОКлиент.ОбщийМодульОтражениеВУчетеЭДОКлиент().ОткрытьНастройкиОтраженияВУчете(Контрагент);
	КонецЕсли;
