...
Процедура ПослеВыгрузкиОбъекта(Контейнер, МенеджерВыгрузкиОбъекта, Сериализатор, Объект, Артефакты) Экспорт
	...
	Если Не ОбъектНайденВКэше Тогда
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Объект метаданных %1 не может быть обработчиком %2,
                  ...
                  | as it is missing in the cache of separated objects.
                  |If the cache was not updated after editing the metadata structure, update 
                  | the cache of separated objects by calling the 
                  |%3 method'", Метаданные.ОсновнойЯзык.КодЯзыка),
			ОбъектМетаданных.ПолноеИмя(),
			"ВыгрузкаСовместноРазделенныхДанных.ПередВыгрузкойОбъекта",
...
Процедура ПередЗагрузкойОбъекта(Контейнер, Объект, Артефакты, Отказ) Экспорт
	...
	Если ОбщегоНазначенияБТС.ЭтоНаборЗаписей(Объект.Метаданные()) Тогда
		Если ОбщегоНазначенияБТС.ЭтоНезависимыйНаборЗаписей(Объект.Метаданные()) Тогда
			Объект.Отбор.ОбластьДанныхВспомогательныеДанные.Использование = Ложь;
		КонецЕсли;
	...
	Иначе
		...
		Объект.ОбластьДанныхВспомогательныеДанные = 0;
