...
Процедура РаспределитьВсеРасчетыСКлиентами(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено) Экспорт
	...
	Комментарий = НСтр("ru = 'Выполняется распределение расчетов с клиентами по %ОкончаниеПериода%';
						|en = 'Allocating customer AR/AP till %ОкончаниеПериода%'", КодЯзыка);
	Комментарий = СтрЗаменить(Комментарий, "%ОкончаниеПериода%", Формат(ОкончаниеПериодаРасчета, ФорматПериода));
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Взаиморасчеты. Начало распределение расчетов';
			|en = 'AR/AP accounting. Start payment allocation'", КодЯзыка), УровеньИнформация, , ОкончаниеПериодаРасчета, Комментарий);
	...
	Пока НачалоПериода < ОкончаниеПериодаРасчета Цикл
		...
		Если НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
			...
			Исключение
				ОтменитьТранзакцию();
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Не удалось заблокировать регистр заданий';
						|en = 'Cannot lock the job register'", КодЯзыка), УровеньЖурналаРегистрации.Ошибка);
				ВызватьИсключение;
			КонецПопытки;
		Иначе
			ДанныеКРасчетуСКлиентамиЗаМесяц = ДанныеКРасчетуСКлиентамиЗаМесяц(НачалоПериода, ОкончаниеПериода, АналитикиРасчета, ИспользуемыеМетаданные, НомерЗаданияДоРасчета);
		...
		//++ НЕ УТ
		РеглУчетСервер.ОбновитьЗаданияКЗакрытиюМесяцаПриВыполненииРеглОперации(ПервыйПериод, СписокОрганизаций, Ложь, КоличествоПовторяющихсяДанных);
		ОценкаПроизводительности.ЗафиксироватьЗамерДлительнойОперации(ОписаниеЗамера, КоличествоПовторяющихсяДанных, "ОбновитьЗаданияКЗакрытиюМесяцаПриВыполненииРеглОперации");
		КоличествоПовторяющихсяДанных = 0;
		//-- НЕ УТ
		...
		Если НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
			...
			Исключение
				ОтменитьТранзакцию();
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Не удалось установить границы расчетов';
						|en = 'Cannot set calculation limits'", КодЯзыка), УровеньЖурналаРегистрации.Ошибка);
				ВызватьИсключение;
			КонецПопытки
		Иначе
			УстановитьГраницыРасчетов(НачалоПериода, ОкончаниеПериода, ИспользуемыеМетаданные.ИмяРегистраЗаданий, АналитикиРасчета, ДанныеКРасчетуСКлиентамиЗаМесяц.ВременныеТаблицы, НомерЗаданияДоРасчета, КоличествоПовторяющихсяДанных);
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Взаиморасчеты. Распределены расчеты с клиентами за период';
				|en = 'AR/AP accounting. Customer AR/AP are allocated for the period'", КодЯзыка), УровеньИнформация, , Формат(НачалоПериода, НСтр("ru = 'ДФ=''yyyy.MM''';
																																				|en = 'DF=''yyyy.MM'''")));
	...
	Комментарий = НСтр("ru = 'Распределены расчеты с клиентами с %НачалоПериода% по %ОкончаниеПериода%';
						|en = 'Customer AR/AP are allocated from %НачалоПериода% to %ОкончаниеПериода%'", КодЯзыка);
	Комментарий = СтрЗаменить(Комментарий, "%НачалоПериода%", Формат(ПервыйПериод, ФорматПериода));
	Комментарий = СтрЗаменить(Комментарий, "%ОкончаниеПериода%", Формат(ОкончаниеПериодаРасчета, ФорматПериода));
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Взаиморасчеты. Окончание распределения расчетов';
			|en = 'AR/AP accounting. End payment allocation'", КодЯзыка), УровеньИнформация, , ОкончаниеПериодаРасчета, Комментарий);
КонецПроцедуры
...
Процедура РаспределитьВсеРасчетыСПоставщиками(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено) Экспорт
	...
	Комментарий = НСтр("ru = 'Выполняется распределение расчетов с поставщиками по %ОкончаниеПериода%';
						|en = 'Allocating vendor AR/AP till %ОкончаниеПериода%'", КодЯзыка);
	Комментарий = СтрЗаменить(Комментарий, "%ОкончаниеПериода%", Формат(ОкончаниеПериодаРасчета, ФорматПериода));
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Взаиморасчеты. Начало распределение расчетов';
			|en = 'AR/AP accounting. Start payment allocation'", КодЯзыка), УровеньИнформация, , ОкончаниеПериодаРасчета, Комментарий);
	...
	Пока НачалоПериода < ОкончаниеПериодаРасчета Цикл
		...
		//++ НЕ УТ
		РеглУчетСервер.ОбновитьЗаданияКЗакрытиюМесяцаПриВыполненииРеглОперации(ПервыйПериод, СписокОрганизаций,, КоличествоПовторяющихсяДанных);
		ОценкаПроизводительности.ЗафиксироватьЗамерДлительнойОперации(ОписаниеЗамера, КоличествоПовторяющихсяДанных, "ОбновитьЗаданияКЗакрытиюМесяцаПриВыполненииРеглОперации");
		КоличествоПовторяющихсяДанных = 0;
		//-- НЕ УТ
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Взаиморасчеты. Распределены расчеты с поставщиками за период';
				|en = 'AR/AP accounting. Vendor AR/AP are allocated for the period'", КодЯзыка), УровеньИнформация, , Формат(НачалоПериода, НСтр("ru = 'ДФ=''yyyy.MM''';
																																					|en = 'DF=''yyyy.MM'''")));
...
Процедура РассчитатьРасчетыСКлиентами(НачалоПериода, ОкончаниеПериода, ИсходныеДанные, КоличествоЗаписей = 0)
	...
	//++ НЕ УТ
	РеглУчетСервер.ЗарегистрироватьДокументыРасчетовСПартнерамиКОтражениюВРеглУчете(РасчетныеРасчетыСКлиентами);
	//-- НЕ УТ
...
Процедура РассчитатьРасчетыСПоставщиками(НачалоПериода, ОкончаниеПериода, ИсходныеДанные, КоличествоЗаписей = 0)
	...
	//++ НЕ УТ
	РеглУчетСервер.ЗарегистрироватьДокументыРасчетовСПартнерамиКОтражениюВРеглУчете(РасчетныеРасчетыСПоставщиками);
	//-- НЕ УТ
...
Процедура ЗаписатьРасчетныеПартии(МенеджерРегистра, РасчетныеПартии, Регистраторы)
	...
	ИмяТаблицыИзменений = "_ТаблицаИзмененийРасчетыПоДокументам";
	...
	ВыборкаЗаданий = НачислениеСписаниеРезервовПоСомнительнымДолгамСервер.ВыборкаЗаданийКЗакрытиюМесяцаПриИзмененииРасчетов(Запрос, ИмяТаблицыИзменений);
	РасчетСебестоимостиПрикладныеАлгоритмы.УничтожитьВременныеТаблицы(Запрос, ИмяТаблицыИзменений);
	
	РегистрыСведений.ЗаданияКЗакрытиюМесяца.СоздатьЗаписиРегистраПоДаннымВыборки(ВыборкаЗаданий);
...
Процедура СформироватьДвиженияПоУправленческомуУчету(Контекст, ДокументыРасчетов)
	...
	//++ НЕ УТКА
	МеждународныйУчетПроведениеСервер.ЗарегистрироватьДокументыРасчетовСПартнерамиКОтражениюВМеждународномУчете(ДокументыРасчетов);
	//-- НЕ УТКА
	Если Контекст = "РасчетыСКлиентами" Тогда
...
Процедура РассчитатьПартииПоЦепочкамИзВыборки(ОписаниеДвижений, РезультатЗапроса, РасчетныеПартии, ЦепочкиДвижений, Регистраторы,  Тест = Ложь, КоличествоЗаписей = 0)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если РасчетныеПартии.Количество() > 1000
			...
			КонецЕсли;
			//++ НЕ УТ
			РеглУчетСервер.ЗарегистрироватьДокументыРасчетовСПартнерамиКОтражениюВРеглУчете(РасчетныеПартии);
			//-- НЕ УТ
...
Функция СформироватьЗаданияКРасчетуНалогов(МенеджерРегистра, Расчеты, Регистраторы)
	...
	Если МенеджерРегистра = РегистрыНакопления.РасчетыСПоставщикамиПоДокументам Тогда
		...
		// ВАЖНО!
		// При нескольких вызовах процедуры не должно быть ситуации, когда по одному регистратору движения записываются
		// несколько раз. Если это не будет соблюдаться, то задания к расчету транспортного налога будут формироваться неверно.
		ВнеоборотныеАктивы.СформироватьЗаданияПриЗаписиДвиженийРаспределенияВзаиморасчетов(Запрос.МенеджерВременныхТаблиц);
		//-- НЕ УТ
	...
	Если ДатыЗапретаИзменения.НайденЗапретИзмененияДанных(ШаблонЗапретаДанных) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось запустить распределение взаиморасчетов за %1 из-за установленной даты запрета.
				           ...
				           |en = 'Cannot start AR/AP accounting allocation for %1 due to the set period-end closing date.
				           |Set the period-end closing date earlier than %1 or contact the user
				           |who has the right to edit documents of the ""Financial accounting"" and ""Local accounting"" subsystems starting from %1'"),
				Формат(Период,НСтр("ru = 'ДФ=''MMMM yyyy''';
									|en = 'DF=''MMMM yyyy'''")));
				
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Взаиморасчеты';
										|en = 'AR/AP accounting'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , ТекстСообщения);
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось очистить регистр: %1 у документа: %2 по причине: %3';
					|en = 'Cannot clear register: %1 in document: %2. Reason: %3'"),
				ИмяРегистра,
				Элемент.Ключ,
