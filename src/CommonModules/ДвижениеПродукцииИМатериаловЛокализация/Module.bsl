...
Функция ПолучитьЗапросПолученияДанныхДляПечатнойФормыМ11()
	...
	Запрос.Текст =
		...
		|	&ТекстПоляНДИГКОтправителя КАК НДИГКОтправителя,
		|	ВЫБОР
		|		КОГДА ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзПроизводства)
		|			ТОГДА &ТекстПоляНДИГКПолучателя
		|		ИНАЧЕ &ТекстПоляНДИГКОтправителя
		|	КОНЕЦ КАК НДИГКПолучателя
		|ИЗ
		|	Товары КАК Товары
		...
		|		ПО Товары.НазначениеОтправителя = ИсходныеНазначения.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НаправленияДеятельности КАК НДОтправитель
...
Функция ТекстЗапросаТаблицаОтражениеДокументовВРеглУчете(Запрос, ТекстыЗапроса, Регистры)
	...
	|	
	|	ВЫБРАТЬ
	|		ТаблицаВидовЗапасов.ВидЗапасов.Организация КАК Организация
	|	ИЗ
	|		Документ.ДвижениеПродукцииИМатериалов.ВидыЗапасов КАК ТаблицаВидовЗапасов
	|	ГДЕ
	|		ТаблицаВидовЗапасов.Ссылка = &Ссылка
	|		И ТаблицаВидовЗапасов.Ссылка.ВнутренняяПереработка
	//-- НЕ УТКА
	|	) КАК Организации
	|";
