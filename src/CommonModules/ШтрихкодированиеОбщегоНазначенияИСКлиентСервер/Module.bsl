...
// * РабочееМесто - Неопределено - Рабочее место при сканировании
...
Функция БазовыеПараметрыСканирования() Экспорт
	...
	БазовыеПараметры.Вставить("РабочееМесто",                                    Неопределено);
...
Процедура ЗаполнитьБазовыеПараметрыСканирования(ПараметрыСканирования, Контекст) Экспорт
	...
	ПараметрыСканирования.РабочееМесто = ОбщегоНазначенияИСКлиентСервер.РабочееМестоПользователя();
...
//  ТребуетсяПередачаВКодировке1251 - Булево - Истина, если требуется конвертация в кодировку windows-1251.
// Возвращаемое значение:
//  Строка - штрихкод закодированный по алгоритму Base64.
Функция ШтрихкодВBase64(Знач Штрихкод, ТребуетсяПередачаВКодировке1251 = Ложь) Экспорт
	
	Если ТребуетсяПередачаВКодировке1251 Тогда
		ДвоичныеДанные  = ПолучитьДвоичныеДанныеИзСтроки(Штрихкод, "windows-1251");
	Иначе
