...
Процедура УстановитьРасширенныйЗаголовокПриложения(ПриЗапуске = Ложь) Экспорт
	...
	Иначе
		...
		ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, НСтр("ru = 'Не установлены разделители';
																											|en = 'Separators are not set'"), ПараметрыКлиента.ПодробнаяИнформация);
	КонецЕсли;
	
	Если Не ОбщегоНазначенияКлиент.РазделениеВключено()
	   И ПараметрыКлиента.Свойство("РаботаСВнешнимиРесурсамиЗаблокирована") Тогда
		ЗаголовокПриложения = "[" + НСтр("ru = 'КОПИЯ';
										|en = 'COPY'") + "]" + " " + ЗаголовокПриложения;
	КонецЕсли;
...
Функция ИнформацияДляПоддержки() Экспорт
	...
	Текст = НСтр("ru = '[НазваниеПрограммы], [ВерсияПрограммы];
	                   ...
	                   |en = '[НазваниеПрограммы], [ВерсияПрограммы]
	                   |1C:Enterprise: [ВерсияПлатформы] [РазрядностьПлатформы]
	                   |Standard Subsystem Library: [ВерсияБСП]
	                   |App: [ПрограммаПросмотра]
	                   |OS: [ОперационнаяСистема]
	                   |RAM: [ОперативнаяПамять]
	                   |COM connector: [ИмяCOMСоединителя]
	                   |Basic configuration: [ЭтоБазоваяВерсияКонфигурации]
	                   |Full-access user: [ЭтоПолноправныйПользователь]
	                   |Sandbox: [ЭтоУчебнаяПлатформа]
	                   |Configuration modified: [КонфигурацияИзменена]'") + Символы.ПС;
	...
	Если Не ЗапускПрограммыЗавершен() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Возникла непредвиденная ситуация при запуске приложения.
			           ...
			           |%3';
			           |en = 'Возникла непредвиденная ситуация при запуске приложения.
			           |
			           |Техническая информация:
			           |Недопустимый вызов %1 при запуске приложения. Сначала должна быть завершена процедура %2.
			           |Предположительно, один из обработчиков события не вызвал оповещение для продолжения.
...
Процедура ОповеститьОНехваткеПамяти() Экспорт
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Рекомендуется увеличить объем памяти до %1 Гб.';
			|en = 'Consider increasing RAM size to %1 GB.'"), РекомендуемыйОбъем);
...
Процедура ОповещениеТекущегоПользователяОПредстоящемПерезапуске(СекундДоПерезапуска) Экспорт
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Перезапуск приложения в %1';
																					|en = 'Application restart at %1'"), ВремяПерезапуска);
