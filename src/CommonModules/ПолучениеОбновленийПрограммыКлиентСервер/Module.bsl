...
Функция Это64РазрядноеПриложение() Экспорт
	...
	Возврат СистИнфо.ТипПлатформы = ТипПлатформы.Windows_x86_64
		...
		Или ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистИнфо.ВерсияПриложения, "8.3.22.0") >= 0
			И СистИнфо.ТипПлатформы = ТипПлатформы["Linux_ARM64"];
...
// Возвращает каталог для хранения файлов исправлений для операционной системы клиента.
//
// Параметры:
...
//  Строка
//
Функция КаталогДляРаботыСИсправлениями(ЭтоWindowsКлиент)
	
	Разделитель     = ПолучитьРазделительПути();
	ПроверитьДоступ = Ложь;
	
	Если ЭтоWindowsКлиент Тогда
		ПутьКаталога = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(СистемныйКаталог(28));
	Иначе
		ПутьКаталога    = КаталогВременныхФайлов(); // АПК:495 Автоматическое удаление временного каталога недопустимо.
		ПроверитьДоступ = Истина;
	КонецЕсли;
	ПутьКаталога = ПутьКаталога + СтрЗаменить("1C|1Cv8ConfigUpdate|Patches|", "|", Разделитель);
	...
	Попытка
		...
		Если Не ОписательФайла.Существует() Тогда
			СоздатьКаталог(ПутьКаталога);
		ИначеЕсли ПроверитьДоступ Тогда
			
			ИмяФайла = ПутьКаталога + "test.txt";
	...
	Исключение
		ПутьКаталога = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(
...
Функция СистемныйКаталог(Идентификатор)
	...
	Иначе
		// АПК:495-выкл Автоматическое удаление временного каталога недопустимо.
...
Функция НовыйКонтекстПолученияИУстановкиОбновлений(Параметры) Экспорт
	...
	Результат.Вставить("ИндексТекущегоОбновления" , 0);
	Результат.Вставить("ИндексТекущегоИсправления", 0);
	Результат.Вставить("ОбъемПолученныхФайлов"    , 0);
	...
	КонецЕсли;
	Результат.Вставить("ПутьФайлаПротокола", "");
	...
	Если Результат.ОбновленияКонфигурации.Количество() > 0 Тогда
		...
		Для Каждого ТекОбновление Из Результат.ОбновленияКонфигурации Цикл
			...
			ТекОбновление.Вставить("КаталогДистрибутива", Результат.КаталогШаблонов + ТекОбновление.ПодкаталогШаблонов);
	...
	Иначе
		...
		Результат.Вставить("ВременныйКаталогОбновленийКонфигурации", "");
		Результат.Вставить("КаталогИндексаФайлов"                  , "");
		Результат.Вставить("КаталогШаблонов"                       , "");
		
	КонецЕсли;
	
	Если Параметры.УстановитьИсправления Тогда
		Результат.Вставить("КаталогДляРаботыСИсправлениями",
			КаталогДляРаботыСИсправлениями(Параметры.ЭтоWindowsКлиент));
		...
		ТекИсправление.ИмяПолученногоФайла =
			Результат.КаталогДляРаботыСИсправлениями + ТекИсправление.Имя
				+ СтрЗаменить(
					...
					"_p_")
				+ ".zip";
		ТекИсправление.Получено = ФайлСуществует(ТекИсправление.ИмяПолученногоФайла, Ложь, ТекИсправление.Размер);
	КонецЦикла;
		...
		Если ТекФайл.ПолучитьВремяИзменения() < ДатаАктуальности Тогда
			...
			Исключение
				...
				Если ПустаяСтрока(ОписаниеОшибкиУдаления) Тогда
					ОписаниеОшибкиУдаления = ОбработкаОшибок.ПодробноеПредставлениеОшибки(
						ИнформацияОбОшибке());
				КонецЕсли;
			КонецПопытки;
	...
	Иначе
		...
		Попытка
			...
			Если ТекСтрока = Неопределено
				Или ТекСтрока <> Обновление.ПолноеИмяCFUФайлаВКаталогеДистрибутивов Тогда
				Возврат Ложь;
			КонецЕсли;
			
			ТекСтрока = ЧтениеТекста.ПрочитатьСтроку();
			Если ТекСтрока = Неопределено
				Или ТекСтрока <> Обновление.КонтрольнаяСумма Тогда
				Возврат Ложь;
			КонецЕсли;
			...
			ТекСтрока = ЧтениеТекста.ПрочитатьСтроку();
			Если ТекСтрока = Неопределено
				Или ТекСтрока <> Строка(ОписательФайлаОбновления.Размер()) Тогда
				Возврат Ложь;
			КонецЕсли;
			
			ТекСтрока = ЧтениеТекста.ПрочитатьСтроку();
			Если ТекСтрока = Неопределено
				Или ТекСтрока <> Строка(ОписательФайлаОбновления.ПолучитьУниверсальноеВремяИзменения()) Тогда
				Возврат Ложь;
			КонецЕсли;
...
Функция ИмяСобытияЖурналаРегистрации(КодОсновногоЯзыка) Экспорт
	...
	Возврат НСтр("ru = 'Получение обновлений программы';
				|en = 'Receive application updates'", КодОсновногоЯзыка);
...
//  Строка
//
Функция КлючНастройкиДатаИнформированияОВключенииЗагрузкиИсправлений() Экспорт
	
	Возврат "ДатаИнформированияОВключенииЗагрузкиИсправлений";
