	...
	ИначеЕсли РезультатЗадания.Статус = "Отменено" Тогда
		...
		МониторПортала1СИТС.ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru = 'Не удалось проверить изменения информации Монитора Портала 1С:ИТС.
				...
				|Job was canceled by administrator.'"));
...
Функция НачатьПроверкуНаличияПредупреждений() Экспорт
	...
	РезультатЗадания = ДлительныеОперации.ВыполнитьФункцию(
		ПараметрыВыполненияВФоне,
		"МониторПортала1СИТС.ПроверитьНаличиеУведомлений");
	...
	ИначеЕсли РезультатЗадания.Статус = "Отменено" Тогда
		...
		МониторПортала1СИТС.ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("ru = 'Не удалось проверить предупреждения Монитора Портала 1С:ИТС.
				|Задание отменено администратором.';
				|en = 'Cannot check warnings of 1C:ITS Portal Dashboard.
				|The job is canceled by the administrator.'"));
		Возврат "НетИзменений";
