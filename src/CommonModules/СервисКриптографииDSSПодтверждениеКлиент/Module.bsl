...
Процедура ОбработатьРезультатЗапроса(ОповещениеСледующее,
	...
	Если ДанныеПодтверждения.ЭтапЦикла = "Ошибка" Тогда
		ПредставлениеОшибки = НСтр("ru = 'Не выполнен этап:';
									|en = 'The stage is not completed:'") + " " + ТекущийЭтап + Символы.ПС
...
Функция РазницаФормат(ВсегоСекунд, КороткийФормат = Ложь) Экспорт
	...
	Иначе	
		Если Часов = 1 Тогда
			Результат	= Результат + Формат(Часов, "ЧЦ=2; ЧН=00;") + " " + НСтр("ru = 'час';
																					|en = 'hour'", КодЯзыка) + " ";
		...
		Если Минут > 0 И Часов > 0 Тогда
			Результат	= Результат + Формат(Минут, "ЧЦ=2; ЧН=00; ЧВН=") + " " + НСтр("ru = 'мин.';
																						|en = 'min.'", КодЯзыка) + " ";
		Иначе	
			Результат	= Результат + Формат(Минут, "ЧЦ=2; ЧН=00") + " " + НСтр("ru = 'мин.';
																				|en = 'min.'", КодЯзыка) + " ";
		КонецЕсли;
		
		Если Минут > 0 Тогда
			Результат = Результат + Формат(Секунд, "ЧЦ=2; ЧН=00; ЧВН=") + " " + НСтр("ru = 'сек.';
																					|en = 'sec.'", КодЯзыка);
		Иначе
			Результат = Результат + Формат(Секунд, "ЧЦ=2; ЧН=00") + " " + НСтр("ru = 'сек.';
																				|en = 'sec.'", КодЯзыка);
		КонецЕсли;	
		
		Результат = НСтр("ru = 'Осталось';
						|en = 'Left'", КодЯзыка) + " " + Результат;
	КонецЕсли;
...
Функция ПроверкаПередВыполнениемОперации(КонтекстФормы, ЗначениеПароля) Экспорт
	...
	Если ЭтоОперацияОблачнойПодписи(КонтекстФормы) Тогда
		...
		Если Подтверждать И НЕ ВыбранСпособ Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("ru = 'Не указан способ подтверждения';
															|en = 'Confirmation method not specified'", КодЯзыка);
		КонецЕсли;
		
		Если ТребуетПинКод И Авторизован Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("ru = 'Не указан пин-код к закрытому ключу сертификата';
															|en = 'PIN to certificate private key not specified'", КодЯзыка);
		КонецЕсли;
		
		Если НЕ Авторизован 
			И НЕ СертификатАвторизации 
			И НЕ ЗначениеЗаполнено(КонтекстФормы[РеестрИмен.ПодтверждениеДанныеСертификата]) Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("ru = 'Необходимо загрузить сертификат авторизации.';
...
Функция СодержаниеПредставленияТелефона(НастройкиПользователя, КодЯзыка)
	...
	Если ЗначениеЗаполнено(НастройкиПользователя.Телефон) Тогда
		Результат = НСтр("ru = 'Одноразовый пароль на';
						|en = 'One-time password to'", КодЯзыка) + " "
								+ СервисКриптографииDSSКлиентСервер.ПредставлениеТелефона(НастройкиПользователя.Телефон);
	Иначе
		Результат = НСтр("ru = 'Одноразовый пароль на мобильный телефон';
						|en = 'One-time password to cell phone'", КодЯзыка);
	КонецЕсли;
...
Функция СодержаниеПредставленияПочты(НастройкиПользователя, КодЯзыка)
	...
	Если ЗначениеЗаполнено(НастройкиПользователя.ЭлектроннаяПочта) Тогда
		Результат = НСтр("ru = 'Одноразовый пароль на электронную почту';
						|en = 'One-time password to email'", КодЯзыка) + " "
...
Процедура ВыбратьСертификатАвторизацииПослеПолучения(РезультатВызова, КонтекстФормы) Экспорт
	...
	Если РезультатВызова.Выполнено Тогда
		...
		КонтекстФормы[РеестрИмен.ПодтверждениеПредставлениеСертификата] = НСтр("ru = 'Сертификат загружен';
																				|en = 'Certificate imported'", СервисКриптографииDSSСлужебныйКлиент.КодЯзыка());
	КонецЕсли;
...
Процедура ВыполнитьНачальнуюОперациюСервисаПослеАвторизации(РезультатВызова, ВходящийКонтекст) Экспорт
	...
	Если РезультатВызова.Выполнено Тогда
		...
		Если ПакетнаяОперация Тогда
			...
			ПредставлениеДанных.Вставить("Представление", НСтр("ru = 'Список документов';
																|en = 'Document list'", ДанныеПодтверждения.КодЯзыка));
...
Функция ПолучитьПредставлениеСпособа(ВторичнаяАвторизация, ДанныеПодтверждения, НастройкиПользователя)
	...
	ИначеЕсли ТекущийВариант = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ОфлайнПодтверждение") Тогда
		Результат = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = 'Офлайн-подтверждение через приложение';
				|en = 'Offline confirmation via application'", КодЯзыка));
		
	ИначеЕсли ТекущийВариант = ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_DSSSDK") Тогда
		ТекстСообщения = НСтр("ru = 'Мобильное приложение';
								|en = 'Mobile application'", КодЯзыка) 
...
Процедура СформироватьПодсказкуСпособовВыбора(КонтекстФормы, ИмяЭлемента = "ПодтверждениеСпособПодтверждения")
	...
	Если СписокВыбора.Количество() = 0 Тогда
		ОбщаяСтрока = НСтр("ru = 'Пользователю не назначен ни один способ подтверждения.';
							|en = 'No confirmation methods are assigned to the user.'", КодЯзыка);
		
	ИначеЕсли НЕ ЗначениеЗаполнено(ДанныеПодтверждения.ВторичнаяАвторизация) Тогда
		ОбщаяСтрока = НСтр("ru = 'Для продолжения укажите способ подтверждения';
							|en = 'To continue, specify the confirmation method'", КодЯзыка);

	ИначеЕсли ЗначениеЗаполнено(НастройкиПользователя.Ссылка) Тогда
		СпособОтображения = ОтображениеПодсказки.Кнопка;
		ПерваяЧасть = НСтр("ru = 'Можно изменить настройки подтверждения в';
							|en = 'You can change the confirmation settings in'", КодЯзыка);
		ВтораяЧасть = НСтр("ru = 'карточке';
							|en = 'card'", КодЯзыка);
		ТретьяЧасть = НСтр("ru = 'учетной записи';
							|en = 'account'", КодЯзыка);
...
Процедура УправлениеЭлементамиДляПодписания(ЭлементыФормы, НеНачалось, НуженПинКод, ПроверенПинКод = Ложь, СтандартныйВид = Ложь)
	...
	УстановитьСвойствоЭлемента(ЭлементыФормы, "Пароль", "Заголовок", ?(НуженПинКод И НЕ СтандартныйВид, НСтр("ru = 'Пин-код';
																											|en = 'PIN code'", КодЯзыка), НСтр("ru = 'Пароль';
																																				|en = 'Password'", КодЯзыка)));
	УстановитьСвойствоЭлемента(ЭлементыФормы, "ЗапомнитьПароль", "Заголовок", 
								?(НуженПинКод И НЕ СтандартныйВид, 
								НСтр("ru = 'Запомнить пин-код';
									|en = 'Remember PIN code'", КодЯзыка),
...
Процедура УправлениеЭлементамиДляРасшифрования(ЭлементыФормы, НеНачалось, НуженПинКод, ПроверенПинКод = Ложь, СтандартныйВид = Ложь)
	...
	УстановитьСвойствоЭлемента(ЭлементыФормы, "Пароль", "Заголовок", ?(НуженПинКод И НЕ СтандартныйВид, НСтр("ru = 'Пин-код';
																											|en = 'PIN code'", КодЯзыка), НСтр("ru = 'Пароль';
																																				|en = 'Password'", КодЯзыка)));
	УстановитьСвойствоЭлемента(ЭлементыФормы, "ЗапомнитьПароль", "Заголовок", ?(НуженПинКод И НЕ СтандартныйВид, НСтр("ru = 'Запомнить пин-код';
																														|en = 'Remember PIN code'", КодЯзыка), НСтр("ru = 'Запомнить пароль';
																																									|en = 'Remember password'", КодЯзыка)));
...
Процедура УправлениеЭлементамиОбщее(КонтекстФормы, ОткудаВызов)
	...
	Если ОтдельнаяАвторизация Тогда
		...
		УстановитьСвойствоЭлемента(ЭлементыФормы, "ПодтверждениеАвторизацияВеб", "Заголовок", 
				?(НЕ ЕстьАвторизация, НСтр("ru = 'авторизоваться';
											|en = 'authorize'", КодЯзыка), НСтр("ru = 'авторизован';
																					|en = 'authorized'", КодЯзыка)), ИменаЭлементов);
	КонецЕсли;
	
	Если ВариантВидимости = 1 Тогда
		УстановитьСвойствоЭлемента(ЭлементыФормы, "ПодтверждениеКодПодтверждения", "Заголовок", НСтр("ru = 'Одноразовый пароль';
																									|en = 'One-time password'", КодЯзыка), ИменаЭлементов);
	ИначеЕсли ВариантВидимости = 2 Тогда
		УстановитьСвойствоЭлемента(ЭлементыФормы, "ПодтверждениеКодПодтверждения", "Заголовок", НСтр("ru = 'Код';
																									|en = 'Code'", КодЯзыка), ИменаЭлементов);
	КонецЕсли;
