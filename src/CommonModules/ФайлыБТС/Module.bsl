...
Функция НовыйФайлВременногоХранилища(Знач Префикс, Знач Расширение, МинутХранения) Экспорт
	...
	Если Не ЗначениеЗаполнено(МинутХранения) Тогда
		ВызватьИсключение НСтр("ru = 'Не указан срок хранения нового файла временного хранилища';
								|en = 'The retention period of the new file of the temporary storage is not specified'");
	КонецЕсли;
	...
	Исключение
		...
		Если ИмяСобытияЖР = Неопределено Тогда
			ИмяСобытияЖР = НСтр("ru = 'Удаление файла';
								|en = 'Deleting file'", ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
...
Процедура ОтправитьОчереднуюПорцию(ИмяФайлаНаСервере, ДанныеФайла, Начало) Экспорт
	...
	РежимОткрытия = ?(Начало = 0, РежимОткрытияФайла.Создать, РежимОткрытияФайла.Дописать); // РежимОткрытияФайла
	...
	Если РазмерЗаписанногоФрагмента <> Начало Тогда
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Размер файла %1 не соответствует ожидаемому %2';
				|en = 'Size of file %1 does not match the expected size %2'"),
			РазмерЗаписанногоФрагмента,
			Начало);
