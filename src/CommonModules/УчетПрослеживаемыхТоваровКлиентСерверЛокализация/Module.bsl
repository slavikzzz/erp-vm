	...
	Если Кэш = Неопределено Тогда
		...
		#Иначе
			...
			ТекстИсключения = НСтр("ru = 'Попытка получения коэффициента по РНПТ на клиенте.';
									|en = 'Attempting to receive ratio by GBRN on the client.'");
	...
	Если НоваяСтрока <> Неопределено Тогда
		...
		Если ДополнительныеПараметры = Неопределено
			...
			ТекстОшибки = НСтр("ru = 'Ошибка в методе %1, входящий параметр %2 задан некорректно.';
								|en = 'Error in method %1, input parameter %2 is invalid.'");
...
Процедура СформироватьВедомость(ПараметрыФормы) Экспорт
	...
	//++ НЕ УТ
	#Если Клиент Тогда
		ОткрытьФорму("Отчет.ВедомостьПрослеживаемыхТоваровВСоставеОС.Форма",ПараметрыФормы);
	#КонецЕсли
	//-- НЕ УТ
...
Процедура ОткрытьФормуПодбора(ПараметрыПодбора, Форма) Экспорт
	...
	#Если Клиент Тогда
		ОткрытьФорму(
		"ОбщаяФорма.ПодборПрослеживаемыхТоваровВСоставеОС",
		ПараметрыПодбора, 
		Форма);
	#КонецЕсли
	//-- НЕ УТ
...
Функция ИмяФормыПодбораПоОстаткамПрослеживаемхТоваровВОС() Экспорт
	...
	//++ НЕ УТ
		ИмяФормыПодбора = "ОбщаяФорма.ПодборПрослеживаемыхТоваровВСоставеОС";
	//-- НЕ УТ
