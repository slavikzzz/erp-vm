			...
			Иначе
				...
				Иначе
					...
					Если СтрокаЗачета.ПорядокЗачета >= ГлобальныеПеременные.ПорядокФакт Тогда
						...
						//Перенос между филиалами
						НовСтр_Сторно.АналитикаУчетаПоПартнерамПриемник = СтрокаОплаты.АналитикаУчетаПоПартнерамПриемник;
						НовСтр_Сторно.ОбъектРасчетовПриемник            = СтрокаОплаты.ОбъектРасчетовПриемник;
						НовСтр_Сторно.ВалютаПриемник                    = СтрокаОплаты.ВалютаПриемник;
						НовСтр_Сторно.СуммаПриемник                     = СтрокаОплаты.СуммаПриемник;
...
Процедура ЗаполнитьОперативныеВзаиморасчетыПоТаблице(Параметры, ДополнительныеСвойства = Неопределено) Экспорт
	...
	Если ДополнительныеСвойства <> Неопределено
		И (ТипЗнч(Регистратор) = Тип("ДокументСсылка.ЗаказКлиента")
			...
			ИЛИ ТипЗнч(Регистратор) = Тип("ДокументСсылка.ЗаказДавальца")
			//-- Устарело_Переработка24
			ИЛИ ТипЗнч(Регистратор) = Тип("ДокументСсылка.ЗаказДавальца2_5")
			//-- НЕ УТКА
			
			//++ НЕ УТ

			//++ Устарело_Переработка24
			ИЛИ ТипЗнч(Регистратор) = Тип("ДокументСсылка.ЗаказПереработчику")
			//-- Устарело_Переработка24
			ИЛИ ТипЗнч(Регистратор) = Тип("ДокументСсылка.ЗаказПереработчику2_5")
			//-- НЕ УТ
...
Процедура ВыполнитьПереоценкуНаДату(МассивОрганизаций, Период, Параметры) Экспорт
	...
	|		ТОГДА ВЫБОР КОГДА СУММА(ОстаткиСКурсом.ДолгРеглОстаток) <> 0
	...
	|			КОНЕЦ
	|		ИНАЧЕ ВЫБОР КОГДА СУММА(ОстаткиСКурсом.ДолгРеглОстаток) <> 0
	...
	Для Каждого СтрокаПереоценки Из ОбъектыКПереоценке Цикл
		...
		Для Каждого ЗаписьНУ Из СтрокиДетальныхЗаписейНУ Цикл
			...
			ПереоценкаПо67ФЗ = (НЕ ЗаписьНУ.ПереоцениватьТоварыУслугиКОтчетуКомитенту
...
Функция ТаблицаСвободныхРегистраторовРасчета(Параметры)
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СписокПолныхРегистраторовПоТаблицам,
			СписокПолныхРегистраторовРасчетыПоСрокам);
...
Функция ПолучитьОтгрузкиПоставки(Запрос, ГлобальныеПеременные, ЭтоРасчетыСКлиентами = Ложь)
	...
	|	РасчетыОтгрузкиПоставки.ОбъектРасчетовПриемник            КАК ОбъектРасчетовПриемник,
	|	РасчетыОтгрузкиПоставки.ВалютаПриемник            КАК ВалютаПриемник,
	|	РасчетыОтгрузкиПоставки.ПоДаннымОбъектаРасчетовИсточника            КАК ПоДаннымОбъектаРасчетовИсточника,
	|	РасчетыОтгрузкиПоставки.СуммаПриемник                  КАК СуммаПриемник,
	|	РасчетыОтгрузкиПоставки.Сторно                         КАК Сторно,
	|	
	|	
...
Процедура ДобавитьПриход(ГлобальныеПеременные, ТаблицаДвиженийРасчетов, СтрокаРасчетов, ТипИсточника, ТаблицаЗачетовОплат, Индекс)
	...
	СуммаПриемник = СтрокаРасчетов.СуммаПриемник;
	Для Каждого СтрокаЗачета Из СтрокиЗачетов Цикл
		...
		СуммаСписанияПриемник = СуммаСписания(0,СуммаПриемник, Сумма, СтрокаЗачета.Сумма);
		Если СтрокаРасчетов.ПорядокОперации >= ГлобальныеПеременные.ПорядокФакт Тогда
			...
			НовСтр.СуммаПриемник                     = СуммаСписанияПриемник;
			НовСтр.ПоДаннымОбъектаРасчетовИсточника  = СтрокаРасчетов.ПоДаннымОбъектаРасчетовИсточника;
			СтрокаРасчетов.СуммаПриемник             = СтрокаРасчетов.СуммаПриемник-СуммаСписанияПриемник;
			...
			СуммаПриемник = СуммаПриемник - НовСтр.СуммаПриемник;
...
Процедура УдалитьКурсовыеРазницыЗаПериод(МассивОрганизаций, НачалоПериода, КонецПериода, Параметры)
	...
	ВсегоКОбработке = Выборка.Количество();
	ВсегоОбработано = 0;
	Пока Выборка.Следующий() Цикл
