...
Процедура ПрочитатьТекущиеДанныеСотрудника(Форма) Экспорт
	...
	Если ТаблицаКадровыхДанных.Количество() > 0 Тогда
		...
		Для Каждого ОписаниеКадровыхДанных Из СоответствиеКадровыхДанных Цикл
			...
			Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, ОписаниеКадровыхДанных.Ключ) Тогда
				Продолжить;
			КонецЕсли;
	...
	Если НЕ Форма.ИспользоватьКадровыйУчет И ФОИспользоватьНачислениеЗарплаты Тогда
		...
		Если ЗначениеЗаполнено(Форма.ТекущаяОрганизация) Тогда
			...
			Если ЗначениеЗаполнено(Форма.ДатаУвольнения) Тогда
				...
				Если Форма.ДатаУвольнения <= Форма.ДатаПриема Тогда
					...
					ТекстСообщения = НСтр("ru = 'Дата увольнения не может быть меньше или равна дате приема на работу';
											|en = 'Termination of employment date cannot be less than or equal to the hiring date  '");
			...
			Если ЗначениеЗаполнено(Форма.ДатаПриема) Тогда
				ЗарплатаКадры.ПроверитьКорректностьДаты(Форма.СотрудникСсылка, Форма.ДатаПриема, "ДатаПриема", Отказ, НСтр("ru = 'Дата приема';
																															|en = 'Hiring date'"), '19800101');
			КонецЕсли;
...
Процедура ФизическиеЛицаПриЧтенииНаСервере(Форма, ТекущийОбъект) Экспорт
	...
	Форма.ДоступноДобавлениеИзменениеДанныхФизическихЛиц = Пользователи.РолиДоступны(
	...
	Если Форма.ДоступенПросмотрДанныхФизическихЛиц Тогда
		РедактированиеПериодическихСведений.ПрочитатьЗаписьДляРедактированияВФорме(Форма, "ГражданствоФизическихЛиц",
			Форма.ФизическоеЛицоСсылка);
	...
	УстановитьОтображениеГруппыСведенийОСогласииНаОбработкуПДн(Форма);
...
Процедура УстановитьОтображениеГруппыСведенийОСогласииНаОбработкуПДн(Форма)
	
	УстановитьПривилегированныйРежим(Истина);
	СотрудникиФизическихЛиц = КадровыйУчет.ОсновныеСотрудникиФизическихЛиц(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Форма.ФизическоеЛицоСсылка), Ложь, , ТекущаяДатаСеанса());
	УстановитьПривилегированныйРежим(Ложь);
	ЗащитаПерсональныхДанныхЗарплатаКадры.ФормаСубъектаУстановитьОтображениеНадписей(Форма,
		Форма.ФизическоеЛицоСсылка, Форма.Элементы.Найти("ГруппаСведенияОСогласииНаОбработкуПДн"),
		ЗначениеЗаполнено(СотрудникиФизическихЛиц));
...
Функция ПоясняющаяНадписьКМестуРаботыСотрудника(ДанныеСотрудника, РеквизитыОрганизации, ДатаСведений) Экспорт
	...
	Если ДанныеСотрудника.СотрудникАктуален Тогда
		СтрокаПериодРаботы = СтрШаблон(НСтр("ru = 'Работает с %1 года';
											|en = 'Works since %1'"), Формат(ДанныеСотрудника.ДатаПриема, "ДЛФ=D"));
	Иначе
		...
		ИначеЕсли ЗначениеЗаполнено(ДанныеСотрудника.ДатаПриема) Тогда 
			СтрокаПериодРаботы = СтрШаблон(
				НСтр("ru = 'Дата планируемого приема: %1';
					|en = 'Planned acceptance date: %1'"), 
				Формат(ДанныеСотрудника.ДатаПриема, "ДЛФ=D")); 		
		Иначе	
			СтрокаПериодРаботы = НСтр("ru = 'Не указана дата приема на работу';
										|en = 'Hiring date is not specified'");
		КонецЕсли;				
	КонецЕсли;	
...
Процедура ЛичныеДанныеФизическихЛицОбработкаПроверкиЗаполненияВФорме(Форма, ФизическоеЛицоСсылка, Отказ) Экспорт
	...
	Если Форма.ДоступенПросмотрДанныхФизическихЛиц Тогда
		...
		Если СотрудникиВызовСервера.ДоступенВыборСтраныВыдачиДокумента(Форма.ДокументыФизическихЛиц.ВидДокумента)
			...
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Не указана страна выдачи документа.';
					|en = 'Document issue country is not specified.'"), , "ДокументыФизическихЛиц.СтранаВыдачи", , Отказ);
		КонецЕсли;
...
Процедура СохранитьДанныеДополнительнойФормы(Форма, ИмяФормы, Отказ, ТекущийОбъект) Экспорт
	...
	Если ИмяФормы = "Справочник.Сотрудники.Форма.ЛичныеДанные" Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "ДокументыФизическихЛицНаборЗаписейПрочитан") = Истина Тогда
			
			Запрос = Новый Запрос;
			Запрос.УстановитьПараметр("Физлицо", Форма.ФизическоеЛицоСсылка);
			Запрос.Текст =
				"ВЫБРАТЬ
				|	ДокументыФизическихЛиц.*
				|ИЗ
				...
				|	И НЕ ДокументыФизическихЛиц.ЯвляетсяДокументомУдостоверяющимЛичность";
			Выборка = Запрос.Выполнить().Выбрать();
			Пока Выборка.Следующий() Цикл
				ЗаполнитьЗначенияСвойств(
					Форма["ДокументыФизическихЛицНаборЗаписей"].Вставить(0),
					Выборка);
			КонецЦикла;
	...
	Иначе
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ВоинскийУчет") Тогда
			...
			МодульВоинскийУчет.СохранитьДанныеДополнительнойФормы(Форма, ИмяФормы, Отказ, ТекущийОбъект);
		КонецЕсли;
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.СемейныеПоложенияФизическихЛиц") Тогда
			...
			МодульСемейныеПоложенияФизическихЛиц.СохранитьДанныеДополнительнойФормы(Форма, ИмяФормы, Отказ, ТекущийОбъект);
		КонецЕсли;
...
Функция ОписаниеМенюВводаНаОсновании(ПараметрыПостроения) Экспорт
	...
	ПунктыМеню.Добавить("");
	ПунктыМеню.Добавить("Документы.СведенияОДоходахСотрудникаДляСоцВыплат");
	...
	ОписаниеМеню = СотрудникиФормы.РазделМеню(НСтр("ru = 'Оформить документ';
													|en = 'Create document'"), ПунктыМеню, ИсключаемыеПунктыМеню);
...
//  Сотрудники - см. КадровыйУчет.СотрудникиОрганизации
Процедура ДополнитьИсториюВзаимоотношенийФизическихЛицСотрудникамиДГПХ(История, ФизическиеЛица, Сотрудники) Экспорт
...
Функция ПримечаниеДГПХ(СтрокаОтношений)
	Возврат НСтр("ru = 'Сотрудник по договору гражданско-правового характера';
				|en = 'Employee under a civil contract'");
КонецФункции
