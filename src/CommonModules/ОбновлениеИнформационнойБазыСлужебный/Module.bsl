...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий =
		НСтр("ru = 'Очищает устаревшие данные, чтобы переход на следующую версию при реструктуризации был без ошибки ""Записи регистра стали неуникальными"".';
			|en = 'Clears obsolete data to avoid the ""Register records are no longer unique"" error during the update when restructuring.'");
...
Процедура ВыполнитьДействияПослеОбновленияИнформационнойБазы(ПараметрыОбновления, ДополнительныеПараметры)
	...
	Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Обновление информационной базы на версию ""%1"" выполнено успешно.';
			|en = 'The infobase was updated to version %1.'"), ВерсияМетаданных);
		ЗаписатьИнформацию(Сообщение);
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Обновление информационной базы';
				|en = 'Infobase update'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПротокол() Экспорт
	...
	Возврат СобытиеЖурналаРегистрации() + "." + НСтр("ru = 'Протокол выполнения';
													|en = 'Execution log'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПроверитьСвойстваОтложенногоОбработчика(Знач Обработчик, Знач РежимВыполненияОтложенныхОбработчиков, Знач ИмяБиблиотеки, ТекстОшибок)
	...
	Если РежимВыполненияОтложенныхОбработчиков = "Последовательно"
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'The ""Multithreaded"" property enabled in the deffered ""%1"" handler is invalid
			|as it is related to the ""%2"" subsystem with the sequence execution of deffered handlers.'"),
			Обработчик.ИмяОбработчика,
			ИмяБиблиотеки);
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	...
	Иначе
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Очистка устаревших данных выполняется
			           ...
			           |en = 'The last deferred update handler
			           |clears obsolete data.
			           |View the results: %1
			           |
			           |The scheduled job is used only in SaaS
			           |to clear obsolete shared data.'"),
...
Процедура ОчиститьУстаревшиеДанныеПолностью(Параметры) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru = 'Не удалось выполнить очистку устаревших данных.';
							|en = 'Cannot clear obsolete data.'")
		+ Символы.ПС + Символы.ПС;
	
	Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедура %1 не может вызываться в неразделенном режиме.';
				|en = 'You cannot call the %1 procedure in shared mode.'"),
	...
	Попытка
		...
		ИначеЕсли Результат.Статус = "Выполняется" Тогда
			ТекстОшибки = НСтр("ru = 'Система не стала дожидаться завершения задания';
								|en = 'The application did not wait for the job to be completed'");
		КонецЕсли;
...
Процедура ДобавитьОписаниеПолейВПланОчистки(Строки, ОписаниеПолей)
	...
	Для Каждого ОписаниеПоля Из ОписаниеПолей Цикл
		Строки.Добавить("		" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Измерение ""%1"" содержит значения следующих типов:';
				|en = 'The ""%1"" dimension contains values of the following types:'"), ОписаниеПоля.Ключ));
		Для Каждого ОписаниеТипа Из ОписаниеПоля.Значение Цикл
			Если ТипЗнч(ОписаниеТипа.Значение) = Тип("Массив") Тогда
				Строки.Добавить("			" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для типа ""%1"" проверяются только следующие значения:';
						|en = 'For the ""%1"" type, only the following values are checked:'"), Строка(ОписаниеТипа.Ключ)) + "
		...
		Если ОбъектМетаданных = Неопределено Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Объект метаданных ""%1"" не существует.';
					|en = 'The ""%1"" metadata object does not exist.'"), ПолноеИмя);
			ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
		ИначеЕсли ИмяПоля = Неопределено И Не СтрНачинаетсяС(ОбъектМетаданных.Имя, "Удалить") Тогда
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Имя объекта метаданных ""%1"" должно начинаться с ""%2"".';
					|en = 'The ""%1"" metadata object name must begin with ""%2"".'"), ПолноеИмя, "Удалить");
			ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
			...
			Если ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(ОбъектМетаданных) Или ЭтоПеречисление Тогда
				...
				ИначеЕсли ЭтоПеречисление Тогда
					...
					Если МетаданныеЗначения = Неопределено Тогда
						ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Значение ""%1"" не существует.';
								|en = 'The ""%1"" value does not exist.'"), ОписаниеОбъекта.Ключ);
						ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
					ИначеЕсли Не СтрНачинаетсяС(МетаданныеЗначения.Имя, "Удалить") Тогда
						ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Имя значения ""%1"" должно начинаться с ""%2"".';
								|en = 'The ""%1"" value name must begin with ""%2"".'"), ОписаниеОбъекта.Ключ, "Удалить");
						ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
					КонецЕсли;
				...
				Иначе // Точка маршрута бизнес-процесс.
					...
					Если ТочкаМаршрута = Неопределено
					 Или ВРег(ЧастиПоля[0]) <> ВРег("ТочкаМаршрута") Тогда
						ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Значение ""%1"" не является существующей точкой маршрута.';
								|en = 'The ""%1"" value is not an existing route point.'"), ОписаниеОбъекта.Ключ);
						ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
					ИначеЕсли Не СтрНачинаетсяС(ТочкаМаршрута.Имя, "Удалить") Тогда
						ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Имя точки маршрута ""%1"" должно начинаться с ""%2"".';
								|en = 'The ""%1"" route point name must begin with ""%2"".'"), ОписаниеОбъекта.Ключ, "Удалить");
						ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
					КонецЕсли;
					ТипТочекМаршрута = ТипЗнч(ПредопределенноеЗначение(ПолноеИмя + ".ТочкаМаршрута.ПустаяСсылка"));
...
Процедура ПроверитьВложеннуюТранзакцию(ТранзакцияАктивнаНаНачалоВыполнения, НазваниеОбработчика)
	...
	ИмяСобытия = СобытиеЖурналаРегистрации() + "." + НСтр("ru = 'Выполнение обработчиков';
															|en = 'Execute handlers'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПередЗапускомПроцедурыОбработкиДанных(КонтекстОбработчика,
	...
	Попытка
		...
		СообщениеОЗапущенномОбработчике = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выполняется процедура обновления ""%1"".';
																										|en = 'Executing update procedure %1.'"), ИмяОбработчика);
