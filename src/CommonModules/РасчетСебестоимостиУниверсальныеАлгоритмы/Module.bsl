...
// * ПравилаРаспределения - Массив из Структура-
// * ПоляСортировкиБазы - Строка -
...
Функция ИнициализироватьПараметрыРаспределенияМетодомУменьшаемогоОстатка(
	...
	// Описание полей сортировки базы
	ПараметрыРаспределения.Вставить("ПоляСортировкиБазы",	"");
...
//	ПоляСортировкиБазы - Строка - перечень полей (через запятую) таблицы-базы, по которой будет отсортирована база для задания порядка списания из базы.
//
Процедура ИнициализироватьПрочиеПоляРаспределенияМетодомУменьшаемогоОстатка(ПараметрыРаспределения,
			ПрочиеПоляИсточника = "", ПрочиеПоляБазы = "", НеВключатьПоляИсточника = "", НеВключатьПоляБазы = "", ПоляСортировкиБазы = "") Экспорт
	
	ПараметрыРаспределения.Вставить("ПоляСортировкиБазы",	ПоляСортировкиБазы);
	...
	СтруктураПоля   = Новый Структура(СоединитьСтроки(ПрочиеПоляБазы, ПоляСортировкиБазы));
...
Процедура РаспределитьМетодомУменьшаемогоОстатка(ПараметрыРасчета, ПараметрыРаспределения) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыРаспределения.Шаблоны.ПоляБазыИменованные) Тогда
		...
		Колонки = Запрос.Выполнить().Выгрузить().Колонки;
		СтруктураПоляСортировки = Новый Структура(ПараметрыРаспределения.ПоляСортировкиБазы);
		...
		Для Каждого ТекущаяКолонка Из Колонки Цикл
			...
			Если НЕ СтруктураПоляСортировки.Свойство(ТекущаяКолонка.Имя) Тогда
				ДополнитьСтроку(
					ПоляБазыДляУпорядочивания,
					ТекущаяКолонка.Имя + ?(ЭтоОписаниеТиповДокументов(ТипЗначенияКолонки), ".МоментВремени", ""));
			КонецЕсли;
	...
	ПараметрыНумерации = РасчетСебестоимостиПрикладныеАлгоритмы.СформироватьПараметрыНумерацииСтрокВременнойТаблицы(
		"",
		"",
		СоединитьСтроки(ПараметрыРаспределения.ПоляСвязи, ПараметрыРаспределения.ПоляСортировкиБазы, ПоляБазыДляУпорядочивания), // упорядочивание
	...
	НомерПравила = 0;
	УпорядочитьПоКлючу = ЗначениеЗаполнено(ПараметрыРаспределения.ПоляСортировкиБазы);
...
Функция ВыполнитьЗапросСЗамером(Запрос, ИмяСобытияЖР) Экспорт
	...
	ПараметрыРасчета.РезультатВыполненияЗапроса = 
		НСтр("ru = 'Общее время';
			|en = 'Total time'") + ": " + РасчетСебестоимостиПротоколРасчета.ПредставлениеВремени(ВремяВыполнения) + Символы.ПС
		+ ПараметрыРасчета.РезультатВыполненияЗапроса;
...
Процедура РассчитатьПолеТаблицыНарастающимИтогом(ПараметрыРасчета, Запрос, ИмяВТИсходной, ИмяВТРезультата, ПолеКлючаЗаписи, ПолеБазиса, УпорядочитьПоКлючу = Ложь)
	...
	Если УпорядочитьПоКлючу Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПолеБазиса УБЫВ", ПолеКлючаЗаписи);
	КонецЕсли;
