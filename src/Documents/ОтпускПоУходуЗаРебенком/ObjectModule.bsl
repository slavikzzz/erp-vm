...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ЗарплатаКадры.ПроверитьКорректностьДаты(Ссылка, ДатаНачала, "Объект.ДатаНачала", Отказ, НСтр("ru = 'Начало отпуска';
																								|en = 'Leave start'"), , , Ложь);
	
	Если ВыплачиватьПособиеДоПолутораЛет Тогда
		ЗарплатаКадры.ПроверитьКорректностьДаты(Ссылка, ДатаОкончанияПособияДоПолутораЛет, "Объект.ДатаОкончанияПособияДоПолутораЛет", Отказ, НСтр("ru = 'Окончание пособия до 1.5 лет';
																																					|en = 'End of child care allowance for children under 1.5 years old'"), , , Ложь);
	КонецЕсли;
	
	Если ВыплачиватьПособиеДоТрехЛет Тогда
		ЗарплатаКадры.ПроверитьКорректностьДаты(Ссылка, ДатаОкончанияПособияДоПолутораЛет, "Объект.ДатаОкончанияПособияДоТрехЛет", Отказ, НСтр("ru = 'Окончание пособия до 3-х лет';
																																				|en = 'End of child care allowance for children under 3 years old'"), , , Ложь);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеРеквизитовШапки(КонтейнерОшибок)
	...
	Если Не ЗначениеЗаполнено(ДатаНачала) Тогда
		ТекстСообщения = НСтр("ru = 'Не заполнена дата начала отпуска по уходу за ребенком.';
								|en = 'Child care leave start date is not entered.'");
	...
	Если ЗначениеЗаполнено(ДатаНачала) И ЗначениеЗаполнено(ДатаОкончания) И ДатаОкончания < ДатаНачала Тогда
		ТекстСообщения = НСтр("ru = 'Дата окончания отпуска не должна быть меньше даты начала.';
								|en = 'Leave end date cannot be less than the start date.'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(КонтейнерОшибок, "Объект.ДатаОкончания", ТекстСообщения, "");
	КонецЕсли; 
	...
	Если ВыплачиватьПособиеДоПолутораЛет Тогда
		Если КоличествоДетей = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Не указано количество детей, за которыми осуществляется уход.';
									|en = 'Number of children for whom care is provided is not specified.'");
...
Процедура ПроверитьЗаполнениеПлановыхНачисленийОтпуска(КонтейнерОшибок)
	...
	Если ВыплачиватьПособиеДоПолутораЛет
		...
		ТекстСообщения = НСтр("ru = 'Необходимо указать вид оплаты пособия по уходу до полутора лет.';
								|en = 'Enter payment kind for child care allowance for children under one and a half years old.'");
	...
	Если ВыплачиватьПособиеДоТрехЛет
		...
		ТекстСообщения = НСтр("ru = 'Необходимо указать вид оплаты пособия по уходу до трех лет.';
								|en = 'Enter payment kind for child care allowance for children under three years old.'");
...
Процедура ПроверитьДанныеОДетях(Отказ)
	...
	Если ДанныеОДетях.Количество() <> КоличествоДетей Тогда
		Текст = НСтр("ru = 'Количество детей на вкладке ""Пилотный проект ФСС"" (%1) не совпадает с количеством детей на вкладке ""Пособия"" (%2).';
					|en = 'The number of children on the ""SSF pilot project"" tab (%1) does not match the number of children on the ""Allowances"" tab (%2).'");
	...
	Если Не Результат.Пустой() Тогда
		...
		Для Каждого СтрокаТаблицы Из Таблица Цикл
			Текст = НСтр("ru = 'На вкладке ""Пилотный проект ФСС"" в строках %1 и %2 указана одинаковая очередность.';
						|en = 'The same order is specified on the ""SSF pilot project"" tab in lines %1 and %2.'");
	...
	Если Найденные.Количество() <> КоличествоПервыхДетей Тогда
		ЗаголовокФлажка = ?(КоличествоДетей = 1, НСтр("ru = 'Это первый ребенок';
														|en = 'This is the first child'"), НСтр("ru = 'Среди детей есть первый ребенок';
																						|en = 'There is the first child among children'"));
		Если КоличествоПервыхДетей = 0 Тогда
			...
			Текст = НСтр("ru = 'На вкладке ""Пилотный проект ФСС"" указана очередность ""Первый ребенок"", хотя флажок ""%2"" на вкладке ""Пособия"" отключен.';
						|en = 'The ""First child"" order is specified on the ""SSF pilot project"" tab, although the ""%2"" check box is cleared on the ""Allowances"" tab.'");
		...
		Иначе
			Текст = НСтр("ru = 'На вкладке ""Пилотный проект ФСС"" нет детей с очередностью ""Первый ребенок"", хотя флажок ""%1"" на вкладке ""Пособия"" включен.';
						|en = 'No children with the ""First child"" order on the ""SSF pilot project"" tab, although the ""%1"" check box is selected on the ""Allowances"" tab.'");
