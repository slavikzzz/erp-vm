...
Процедура ДанныеОДетяхПередУдалением(Элемент, Отказ)
	...
	Если ЗначениеЗаполнено(Элементы.ДанныеОДетях.ТекущиеДанные.Заявление) Тогда
		...
		ТекстВопроса = НСтр("ru = 'Существует заявление на выплату пособия, введенное на основании данной строки. Пометить его на удаление?';
							|en = 'The allowance payment request based on this line exists. Mark it for deletion?'");
...
Функция ПредставлениеОчередностиРебенка(Очередность)
	...
	ИначеЕсли Очередность = 9 Тогда
		Возврат НСтр("ru = 'Девятый ребенок';
					|en = 'The ninth child'");
	Иначе
		Возврат СтрШаблон(НСтр("ru = '%1й ребенок';
								|en = 'The %1th child'"), Формат(Очередность, "ЧГ="));
	КонецЕсли;
КонецФункции
...
Процедура ДополнитьФорму(ОтложенноеИзменение = Неопределено)
	...
	Если ОтложенноеИзменение = Неопределено Тогда // Выполняем процедуры, не нуждающиеся в механизме отложенного создания.	
		...
		// Создание элементов.
		ЗарплатаКадрыРасширенный.МногофункциональныеДокументыДобавитьЭлементыФормы(ЭтаФорма, НСтр("ru = 'Документ утвержден';
																									|en = 'Document is confirmed.'"), , "НачисленияУтверждены");
	...
	Если НЕ РасчетПоПравилам2010ГодаДоступен  Тогда
		Если Объект.РасчетПоПравилам2010Года И Сообщать Тогда			
			ТекстСообщения = НСтр("ru = 'Порядок расчета по правилам 2010 года не может быть применен';
									|en = 'Calculation based on 2010 rules cannot be applied'");
...
Функция КомментарийКДокументу(Объект)
	...
	ИначеЕсли Объект.ВыплачиватьПособиеДоПолутораЛет Тогда
		...
		Текст = НСтр(
			...
			|en = 'Monthly child care allowance is accrued in the amount of %1% of the average earnings until the child is one and a half years old.
			|Minimum benefits amount is determined for %2.'");
		Если Объект.КоличествоПервыхДетей = 0 Тогда
			Если Объект.КоличествоДетей = 1 Тогда
				ПредставлениеДетей = НСтр("ru = 'одного ребенка, рожденного (усыновленного) вторым или последующим';
											|en = 'one child born (adopted) as the second or the following one'");
			Иначе
				ПредставлениеДетей = НСтр("ru = '%1 детей, среди которых нет первого ребенка';
											|en = '%1 children among whom there is no first child'");
		...
		ИначеЕсли Объект.КоличествоПервыхДетей = 1 Тогда
			Если Объект.КоличествоДетей = 1 Тогда
				ПредставлениеДетей = НСтр("ru = 'одного ребенка, рожденного (усыновленного) первым';
											|en = 'one child born (adopted) as the first one'");
			Иначе
				ПредставлениеДетей = НСтр("ru = '%1 детей, среди которых есть ребенок, рожденный (усыновленный) первым';
											|en = '%1 children among whom there is the first born (adopted) child'");
				ПредставлениеДетей = СтрШаблон(ПредставлениеДетей, Объект.КоличествоДетей);
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		Текст = НСтр("ru = 'Ежемесячное пособие по уходу за ребенком до достижения ребенком полутора лет не начисляется.';
					|en = 'Monthly child care allowance is not accrued until the child is one and a half years old.'");
	КонецЕсли;
	Возврат Текст;
КонецФункции
...
Функция ЕстьКонфликтыПоРегиструПрименениеПлановыхНачислений()
	...
	ШаблонСообщения = НСтр("ru = 'Данные о применении плановых начислений на дату %1 уже введены документом %2';
							|en = 'Data on application of scheduled accruals on date %1 is already entered by document %2'");
...
Процедура СоздатьИсходящееСообщениеОСтраховомСлучаеФСС(Команда)
	...
	Если Не ЗначениеЗаполнено(СтрокаТаблицы.СообщениеВФСС)
		...
		СообщенияБЗККлиентСервер.СообщитьВФорме(НСтр("ru = 'Заполните ребенка';
													|en = 'Fill the child information'"), ПутьКРеквизиту, "Объект");
		Возврат;
	КонецЕсли;
