...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидПродукции = Перечисления.ВидыПродукцииИС.ИгрыИИгрушкиДляДетей
		И (Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.ИгрыИИгрушкиДляДетейСтандартныйКМ
			...
			Или Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.ИгрыИИгрушкиДляДетейНаборСтандартныйКМ)
		Или ВидПродукции = Перечисления.ВидыПродукцииИС.ПиротехническиеИзделияИСредстваПожарнойБезопасности
		И (Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.ПиротехническиеИзделияИСредстваПожарнойБезопасностиСтандартныйКМ
			Или Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.ПиротехническиеИзделияИСредстваПожарнойБезопасностиГрупповаяУпаковкаСтКМ
			Или Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.ПиротехническиеИзделияИСредстваПожарнойБезопасностиНаборСтандартныйКМ)
		Или ВидПродукции = Перечисления.ВидыПродукцииИС.СладостиИКондитерскиеИзделия
		И (Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.СладостиИКондитерскиеИзделияСтандартныйКМ
			Или Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.СладостиИКондитерскиеИзделияГрупповаяУпаковкаСтандартныйКМ
			Или Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.СладостиИКондитерскиеИзделияНаборСтандартныйКМ) Тогда
...
Процедура ОбновитьПоясняющийТекст(Форма)
	...
	ШаблонСообщения = НСтр("ru = 'Серийных номеров указано: %1 из %2';
							|en = 'Серийных номеров указано: %1 из %2'");
	...
	Для Каждого Строка Из СерийныеНомера Цикл
		...
		Если КоличествоСимволов < КоличествоСимволовСерии Тогда
			...
			ШаблонОшибки = НСтр("ru = 'Длина серийного должна содержать знаков: %1';
								|en = 'Длина серийного должна содержать знаков: %1'");
		...
		ИначеЕсли Не РазборКодаМаркировкиИССлужебныйКлиентСервер.КодСоответствуетАлфавиту(Строка.СерийныйНомер, АлфавитСерийногоНомера) Тогда
			...
			ШаблонОшибки = НСтр("ru = 'Серийный номер не соответствует алфавиту: %1';
								|en = 'Серийный номер не соответствует алфавиту: %1'");
		...
		Если НайденоВСтроке <> Неопределено Тогда
			...
			ШаблонОшибки = НСтр("ru = 'Серийный номер %1 уже присутствует в строке %2';
								|en = 'Серийный номер %1 уже присутствует в строке %2'");
