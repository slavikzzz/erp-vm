	...
	Если Не ЗавершениеРаботы
		...
		Если ЕстьРедактируемыеФайлы Тогда
			...
			ТекстВопроса = НСтр("ru = 'Данные были изменены, Записать?';
								|en = 'Data was changed. Save the changes?'");
			ДополнительныеПараметры = Новый Структура;
...
Процедура ОтправитьПереслатьВыполнить(Команда)
	...
	Если СписокПолучателей.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Укажите хотя бы одного получателя письма.';
				|en = 'Specify at least one email recipient.'"),, "СписокПолучателей");
		Возврат;
	...
	ИначеЕсли (СписокПолучателей.Количество() = 1 И ПустаяСтрока(СписокПолучателей[0].Адрес)) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Укажите хотя бы одного получателя письма.';
				|en = 'Specify at least one email recipient.'"),, "СписокПолучателей[0].Представление");
		Возврат;
...
Функция СформироватьИсходящееОбычныйТекст(ВыборкаДанныеВходящегоПисьма, ТекущийОбъект)
	...
	СтрокаШапка = СтрокаШапка + Символы.ПС+ НСтр("ru = 'От';
												|en = 'From'") + ": "+ ВыборкаДанныеВходящегоПисьма.ОтправительПредставление
	...
	СтрокаШапка = СтрокаШапка + Символы.ПС+ НСтр("ru = 'Тема';
												|en = 'Subject'") + ": " + ВыборкаДанныеВходящегоПисьма.Тема;
...
Процедура ОтправитьВыполнить()
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Выбранная почта недоступна для отправки писем.';
				|en = 'The selected account cannot be used to send mail.'"),, "ОтправительПредставление", "Объект");
		Возврат;
	КонецЕсли;
