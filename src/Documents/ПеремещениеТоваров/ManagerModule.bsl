...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат ПеремещениеТоваровЛокализация.ТекстОтраженияВРеглУчете();
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	Возврат ПеремещениеТоваровЛокализация.ТекстЗапросаВТОтраженияВРеглУчете();
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Если ЗначениеЗаполнено(СтруктураПараметров.СкладОтправитель) Тогда
		ШаблонСтроки = "ru = 'Перемещение с ""%1""';
	...
	Если ЗначениеЗаполнено(СтруктураПараметров.СкладПолучатель) Тогда
		ШаблонСтроки = "ru = 'Перемещение на ""%1""';
...
Процедура ТекстЗапросаВтВидыЗапасов(Запрос, ТекстыЗапроса)
	...
	//++ НЕ УТ
	|			И (ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка) = АналитикаНоменклатурыПолучатель.СтатьяКалькуляции)
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|			И (ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка) = АналитикаПолучательБезНазначения.СтатьяКалькуляции)
	//-- НЕ УТ
...
Функция КомплектПечатныхФорм() Экспорт
	...
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "НакладнаяНаПеремещение", НСтр("ru = 'Накладная на перемещение';
																																|en = 'Goods transfer'"),                     0);
	...
	Если ТаблицаВидовЦен.Количество() = 1 Тогда
		...
		Иначе
			ТекстСообщения = НСтр("ru = 'У пользователя нет доступа к складу-получателю. Печать этикеток невозможна.';
									|en = 'The user does not have access to the destination warehouse. Labels cannot be printed.'");
	...
	Если ТаблицаВидовЦен.Количество() = 1 Тогда
		...
		Иначе
			ТекстСообщения = НСтр("ru = 'У пользователя нет доступа к складу-получателю. Печать ценников невозможна.';
									|en = 'The user does not have access to the destination warehouse. Price tags cannot be printed.'");
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление  = НСтр("ru = 'Перемещения товаров к оформлению';
								|en = 'Transfers of goods to register'");
