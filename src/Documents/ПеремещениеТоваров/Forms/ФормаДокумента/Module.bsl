	...
	ОткрытьФорму(
		...
		Новый Структура("СписокДокументов, Заголовок",
			...
			НСтр("ru = 'Заказы на перемещение (%КоличествоДокументов%)';
				|en = 'Transfer orders (%КоличествоДокументов%)'")
		),
...
Процедура ПеремещениеПоЗаказуПриИзменении(Элемент)
	...
	Если Объект.ПеремещениеПоЗаказам И ЭтоРаспоряжениеНакладная Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Перемещение со склада-отправителя уже начато. Установка признака невозможна.';
									|en = 'Transfer from the origin warehouse already started. Cannot set the flag.'");
...
Процедура ПодобратьТовары(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.СкладОтправитель) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Поле ""Склад"" не заполнено';
																|en = '""Warehouse"" is required'"), Объект.Ссылка, "Объект.СкладОтправитель");
		Возврат;
...
Процедура СоставНабора(Команда)
	...
	ИначеЕсли ТекущаяСтрока.КодСтроки <> 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Открытие состава набора возможно только для строк сверх заказа.';
									|en = 'Opening of the set content is possible only for lines above the order.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПеремещениеПоЗаказуУстановленоЗаказВыбран(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если Объект.Товары.Количество() = 0 Тогда
			ТекстВопроса = СтрЗаменить(НСтр("ru = 'Список Товары будет заполнен остатками неоформленных товаров по документу %ЗаказНаПеремещение%. Продолжить?';
											|en = 'The Item list will be populated with remaining unregistered goods under the %ЗаказНаПеремещение% document. Continue?'"), "%ЗаказНаПеремещение%", Результат);
		Иначе
			ТекстВопроса = СтрЗаменить(НСтр("ru = 'Строки в списке Товары будут привязаны к строкам документа %ЗаказНаПеремещение%. Строки, которые невозможно привязать к строкам заказа, будут удалены.  Продолжить?';
											|en = 'Lines in the Item list will be linked to lines of the %ЗаказНаПеремещение% document. Lines which cannot be linked to order lines will be deleted. Continue?'"), "%ЗаказНаПеремещение%", Результат);
		КонецЕсли;
...
Процедура ОповеститьОПривязкеСтрокКЗаказам(ЗаказСсылка = Неопределено)
	...
	Если ЗаказСсылка <> Неопределено Тогда
		...
		ШаблонТекста = НСтр("ru = 'Строки в списке ""Товары"" привязаны к строкам документа %Заказ%.
			...
			|Lines which cannot be linked to order lines are deleted.'");
		...
		Иначе
			...
			ИначеЕсли ИспользоватьНесколькоОрганизаций И ИспользоватьПодразделения Тогда
				Если ЗначениеЗаполнено(Объект.Подразделение) Тогда
					ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по заказам на перемещение
					                        ...
					                        |en = 'No unregistered goods against orders of transfer
					                        | from warehouse ""%1"" to warehouse ""%2"" in business unit ""%3"" of company ""%4""'");
				Иначе
					ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по заказам на перемещение
					                        |со склада ""%1"" на склад ""%2"" без указания подразделения в организации ""%4""';
					                        |en = 'No unregistered goods against orders of goods transfer
					                        | from warehouse ""%1"" to warehouse ""%2"" without specifying a business unit in company ""%4""'");
				КонецЕсли;
			...
			ИначеЕсли ИспользоватьНесколькоОрганизаций Тогда
				ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по заказам на перемещение
				                        ...
				                        |en = 'No unregistered goods against orders of goods transfer
				                        | from warehouse ""%1"" to warehouse ""%2"" in company ""%3""'");
				ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекста,
			...
			ИначеЕсли ИспользоватьПодразделения Тогда
				Если ЗначениеЗаполнено(Объект.Подразделение) Тогда
					ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по заказам на перемещение
					                        ...
					                        |en = 'No unregistered goods against orders of goods transfer
					                        | from warehouse ""%1"" to warehouse ""%2"" in business unit ""%3""'");
				Иначе
					ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по заказам на перемещение
					                        |со склада ""%1"" на склад ""%2"" без указания подразделения';
					                        |en = 'No unregistered goods against orders of transfer
					                        | from warehouse ""%1"" to warehouse ""%2"" without specifying a business unit'");
				КонецЕсли;
				ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекста,
...
Функция ОписаниеКолонокМакетаДляЗагрузкиТоваров();
	...
	Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("Серия", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(150)), НСтр("ru = 'Серия';
																																					|en = 'Batch'"), 15);
	КолонкиМакета.Добавить(Колонка);
