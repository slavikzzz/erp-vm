...
// Тексты запросов по ОВЗ используются только в том случае, когда включена ФО "ИспользоватьУчетПоОбъектамВозникновенияЗатрат"
	...
	|			ВЫБОР КОГДА СтатьяНУ.Ссылка ЕСТЬ NULL ТОГДА 0 ИНАЧЕ 1 КОНЕЦ *
	|			ВЫБОР КОГДА ДД.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) ТОГДА 1 ИНАЧЕ -1 КОНЕЦ *
	|			(ДД.ПостояннаяРазница + ДД.ВременнаяРазница-ДД.СуммаРегл) +
	//-- Локализация
	...
	//++ Локализация
	|				ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьяНУ
	|				ПО СтатьяНУ.Ссылка = ДД.СтатьяРасходов
	|					И СтатьяНУ.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
	//-- Локализация
	...
	|				ИЛИ СтатьяРегл.Ссылка ЕСТЬ НЕ NULL
				...
				//++ Локализация
	|				ИЛИ СтатьяНУ.Ссылка ЕСТЬ НЕ NULL
				//-- Локализация
	...
	//++ Локализация
	|			ВЫБОР КОГДА СтатьяРегл.Ссылка ЕСТЬ NULL ТОГДА 0 ИНАЧЕ 1 КОНЕЦ *
	|			(Расходы.СуммаРегл - Расходы.ПостояннаяРазница) +
	|			ВЫБОР КОГДА СтатьяНУ.Ссылка ЕСТЬ NULL ТОГДА 0 ИНАЧЕ 1 КОНЕЦ *
	|			Расходы.ПостояннаяРазница + Расходы.ВременнаяРазница-Расходы.СуммаРегл +
	//-- Локализация
	...
	//++ Локализация
	|				ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьяНУ
	|				ПО СтатьяНУ.Ссылка = Расходы.СтатьяРасходов
	|					И СтатьяНУ.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
	...
	//++ Локализация
	|			СтатьяНУ.Ссылка ЕСТЬ НЕ NULL И РаспределениеНУ.Ссылка ЕСТЬ NULL ИЛИ
	//-- Локализация
	...
	//++ Локализация
	|				ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьяНУ
	|				ПО СтатьяНУ.Ссылка = Себестоимость.СтатьяРасходовСписания
	|					И СтатьяНУ.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
	//-- Локализация
	...
	|				ПО
	|					РаспределениеНУ.Организация = ЕСТЬNULL(ОрганизацияПолучатель.Ссылка, Себестоимость.Организация)
	|					И РаспределениеНУ.Подразделение = Себестоимость.Подразделение
	|					И РаспределениеНУ.СтатьяРасходов = СтатьяНУ.Ссылка
	|					И РаспределениеНУ.АналитикаРасходов = Себестоимость.АналитикаРасходов
	|					И РаспределениеНУ.НаправлениеДеятельности = ЕСТЬNULL(Назначения.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка))
	|					И РаспределениеНУ.НалоговыйУчет
	|					И РаспределениеНУ.Проведен
	|					И РаспределениеНУ.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	//-- Локализация
	...
	|				ИЛИ СтатьяРегл.Ссылка ЕСТЬ НЕ NULL И РаспределениеРегл.Ссылка ЕСТЬ NULL
				...
				//++ Локализация
	|				ИЛИ СтатьяНУ.Ссылка ЕСТЬ НЕ NULL И РаспределениеНУ.Ссылка ЕСТЬ NULL
				//-- Локализация
	...
	ТекстСписаниеРасходовНаРБП = "
	|		ВЫБРАТЬ
	|			//РежимОтладки ""ТекстСписаниеРасходовНаРБП"",
	|			ДД.Организация,
	...
	|				ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьяУпр
	|				ПО СтатьяУпр.Ссылка = СписаниеРасходов.СтатьяРасходов
	|					И СтатьяУпр.ВариантРаспределенияРасходовУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
	|				ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьяРегл
	|				ПО СтатьяРегл.Ссылка = СписаниеРасходов.СтатьяРасходов
	|					И СтатьяРегл.ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
	
	//++ Локализация
	|				ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьяНУ
	|				ПО СтатьяНУ.Ссылка = СписаниеРасходов.СтатьяРасходов
	|					И СтатьяНУ.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
	//-- Локализация
	
	|				ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК РаспределениеУпр
	|				ПО
	|					РаспределениеУпр.Организация = ДД.Организация
	|					И РаспределениеУпр.Подразделение = СписаниеРасходов.Подразделение
	|					И РаспределениеУпр.СтатьяРасходов = СтатьяУпр.Ссылка
	|					И РаспределениеУпр.АналитикаРасходов = СписаниеРасходов.АналитикаРасходов
	|					И РаспределениеУпр.НаправлениеДеятельности = СписаниеРасходов.НаправлениеДеятельности
	|					И РаспределениеУпр.УправленческийУчет
	|					И РаспределениеУпр.Проведен
	|					И РаспределениеУпр.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|				ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК РаспределениеРегл
	|				ПО
	|					РаспределениеРегл.Организация = ДД.Организация
	|					И РаспределениеРегл.Подразделение = СписаниеРасходов.Подразделение
	|					И РаспределениеРегл.СтатьяРасходов = СтатьяРегл.Ссылка
	|					И РаспределениеРегл.АналитикаРасходов = СписаниеРасходов.АналитикаРасходов
	|					И РаспределениеРегл.НаправлениеДеятельности = СписаниеРасходов.НаправлениеДеятельности
	|					И РаспределениеРегл.РегламентированныйУчет
	|					И РаспределениеРегл.Проведен
	|					И РаспределениеРегл.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	//++ Локализация
	|				ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК РаспределениеНУ
	|				ПО
	|					РаспределениеНУ.Организация = ДД.Организация
	|					И РаспределениеНУ.Подразделение = СписаниеРасходов.Подразделение
	|					И РаспределениеНУ.СтатьяРасходов = СтатьяНУ.Ссылка
	|					И РаспределениеНУ.АналитикаРасходов = СписаниеРасходов.АналитикаРасходов
	|					И РаспределениеНУ.НаправлениеДеятельности = СписаниеРасходов.НаправлениеДеятельности
	|					И РаспределениеНУ.НалоговыйУчет
	|					И РаспределениеНУ.Проведен
	|					И РаспределениеНУ.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	//-- Локализация
	...
	|				ИЗ РегистрСведений.ЗаданияКРасчетуСебестоимости КАК Задания
	|				ГДЕ
	|					Задания.Месяц <= ДД.Дата
	|					И Задания.Организация = ДД.Организация
	|				)
	|			И (
	|				СтатьяУпр.Ссылка ЕСТЬ НЕ NULL И РаспределениеУпр.Ссылка ЕСТЬ NULL
	|				ИЛИ СтатьяРегл.Ссылка ЕСТЬ НЕ NULL И РаспределениеРегл.Ссылка ЕСТЬ NULL
	...
	|				ТОГДА 1 ИНАЧЕ 0
	|			КОНЕЦ *
	|			(Остатки.СуммаРеглОстаток - Остатки.ПостояннаяРазницаОстаток) +
	|
	// налоговая ВР
	|			ВЫБОР КОГДА 
	...
	|				ТОГДА 1 ИНАЧЕ 0
	|			КОНЕЦ *
	|			(Остатки.ПостояннаяРазницаОстаток + Остатки.ВременнаяРазницаОстаток - Остатки.СуммаРеглОстаток) +
	//-- Локализация
	...
	//++ Локализация
	|						ИЛИ СтатьяРасходов.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
	//-- Локализация
	...
	ТекстПриходыРБПОВЗ = "
	|ВЫБРАТЬ
	|			//РежимОтладки ""ТекстПриходыРБПОВЗ"",
	...
	|			0,
	// без сумм
	|			ОВЗУпр.Ссылка ЕСТЬ НЕ NULL И РаспределениеУпр.Ссылка ЕСТЬ NULL КАК БезСуммУпр,
	|			ОВЗРегл.Ссылка ЕСТЬ НЕ NULL И РаспределениеРегл.Ссылка ЕСТЬ NULL КАК БезСуммРегл,
	
	//++ Локализация
	|			ОВЗНУ.Ссылка ЕСТЬ НЕ NULL И РаспределениеНУ.Ссылка ЕСТЬ NULL ИЛИ
	//-- Локализация
	...
	|				ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОбъектыВозникновенияЗатрат КАК ОВЗУпр
	|				ПО	(ОВЗУпр.Ссылка = Приходы.АналитикаРасходов)
	|					И ОВЗУпр.ВариантРаспределенияРасходовУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
	|				ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК РаспределениеУпр
	|				ПО НАЧАЛОПЕРИОДА(РаспределениеУпр.Дата, МЕСЯЦ) = НАЧАЛОПЕРИОДА(Приходы.Период, МЕСЯЦ)
	|					И РаспределениеУпр.Проведен
	|					И РаспределениеУпр.УправленческийУчет
	|					И РаспределениеУпр.ВариантУказанияСуммыУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСуммыРБП.ОпределяетсяАвтоматически)
	|					И РаспределениеУпр.Организация = Приходы.Организация
	|					И РаспределениеУпр.Подразделение = Приходы.Подразделение
	|					И РаспределениеУпр.АналитикаРасходов = ОВЗУпр.Ссылка
	|				ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОбъектыВозникновенияЗатрат КАК ОВЗРегл
	|				ПО	(ОВЗРегл.Ссылка = Приходы.АналитикаРасходов)
	|					И ОВЗРегл.ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
	|				ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК РаспределениеРегл
	|				ПО НАЧАЛОПЕРИОДА(РаспределениеРегл.Дата, МЕСЯЦ) = НАЧАЛОПЕРИОДА(Приходы.Период, МЕСЯЦ)
	|					И РаспределениеРегл.Проведен
	|					И РаспределениеРегл.РегламентированныйУчет
	|					И РаспределениеРегл.ВариантУказанияСуммыРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСуммыРБП.ОпределяетсяАвтоматически)
	|					И РаспределениеРегл.Организация = Приходы.Организация
	|					И РаспределениеРегл.Подразделение = Приходы.Подразделение
	|					И РаспределениеРегл.АналитикаРасходов = ОВЗРегл.Ссылка
	//++ Локализация
	|				ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОбъектыВозникновенияЗатрат КАК ОВЗНУ
	|				ПО	(ОВЗНУ.Ссылка = Приходы.АналитикаРасходов)
	|					И ОВЗНУ.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
	|				ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК РаспределениеНУ
	|				ПО НАЧАЛОПЕРИОДА(РаспределениеНУ.Дата, МЕСЯЦ) = НАЧАЛОПЕРИОДА(Приходы.Период, МЕСЯЦ)
	|					И РаспределениеНУ.Проведен
	|					И РаспределениеНУ.НалоговыйУчет
	|					И РаспределениеНУ.ВариантУказанияСуммыРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСуммыРБП.ОпределяетсяАвтоматически)
	|					И РаспределениеНУ.Организация = Приходы.Организация
	|					И РаспределениеНУ.Подразделение = Приходы.Подразделение
	|					И РаспределениеНУ.АналитикаРасходов = ОВЗНУ.Ссылка
	//-- Локализация
	|		ГДЕ
	...
	|			И (
	|				ОВЗУпр.Ссылка ЕСТЬ НЕ NULL И РаспределениеУпр.Ссылка ЕСТЬ NULL
	|				ИЛИ ОВЗРегл.Ссылка ЕСТЬ НЕ NULL И РаспределениеРегл.Ссылка ЕСТЬ NULL
	
	//++ Локализация
	|				ИЛИ ОВЗНУ.Ссылка ЕСТЬ НЕ NULL И РаспределениеНУ.Ссылка ЕСТЬ NULL
	//-- Локализация
	|			)
	...
	ТекстСписаниеМатериаловРаботНаРБПОВЗ = "
	|		ВЫБРАТЬ
	|			//РежимОтладки ""ТекстСписаниеМатериаловРаботНаРБПОВЗ"",
	...
	|				ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОбъектыВозникновенияЗатрат КАК ОВЗУпр
	|				ПО	(ОВЗУпр.Ссылка = Себестоимость.АналитикаРасходов)
	|					И ОВЗУпр.ВариантРаспределенияРасходовУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
	|				ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК РаспределениеУпр
	|				ПО НАЧАЛОПЕРИОДА(РаспределениеУпр.Дата, МЕСЯЦ) = НАЧАЛОПЕРИОДА(Себестоимость.Период, МЕСЯЦ)
	|					И РаспределениеУпр.Проведен
	|					И РаспределениеУпр.УправленческийУчет
	|					И РаспределениеУпр.ВариантУказанияСуммыУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСуммыРБП.ОпределяетсяАвтоматически)
	|					И РаспределениеУпр.Организация = Себестоимость.Организация
	|					И РаспределениеУпр.Подразделение = Себестоимость.Подразделение
	|					И РаспределениеУпр.АналитикаРасходов = ОВЗУпр.Ссылка
	|				ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОбъектыВозникновенияЗатрат КАК ОВЗРегл
	|				ПО	(ОВЗРегл.Ссылка = Себестоимость.АналитикаРасходов)
	|					И ОВЗРегл.ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
	|				ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК РаспределениеРегл
	|				ПО НАЧАЛОПЕРИОДА(РаспределениеРегл.Дата, МЕСЯЦ) = НАЧАЛОПЕРИОДА(Себестоимость.Период, МЕСЯЦ)
	|					И РаспределениеРегл.Проведен
	|					И РаспределениеРегл.РегламентированныйУчет
	|					И РаспределениеРегл.ВариантУказанияСуммыРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСуммыРБП.ОпределяетсяАвтоматически)
	|					И РаспределениеРегл.Организация = Себестоимость.Организация
	|					И РаспределениеРегл.Подразделение = Себестоимость.Подразделение
	|					И РаспределениеРегл.АналитикаРасходов = ОВЗРегл.Ссылка
	//++ Локализация
	|				ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОбъектыВозникновенияЗатрат КАК ОВЗНУ
	|				ПО	(ОВЗНУ.Ссылка = Себестоимость.АналитикаРасходов)
	|					И ОВЗНУ.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
	|				ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК РаспределениеНУ
	|				ПО НАЧАЛОПЕРИОДА(РаспределениеНУ.Дата, МЕСЯЦ) = НАЧАЛОПЕРИОДА(Себестоимость.Период, МЕСЯЦ)
	|					И РаспределениеНУ.Проведен
	|					И РаспределениеНУ.НалоговыйУчет
	|					И РаспределениеНУ.ВариантУказанияСуммыРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСуммыРБП.ОпределяетсяАвтоматически)
	|					И РаспределениеНУ.Организация = Себестоимость.Организация
	|					И РаспределениеНУ.Подразделение = Себестоимость.Подразделение
	|					И РаспределениеНУ.АналитикаРасходов = ОВЗНУ.Ссылка
	//-- Локализация
	...
	|		ВЫБРАТЬ РАЗЛИЧНЫЕ
	|			//РежимОтладки ""ТекстРБПизТранзитныхОВЗПостоянныеПоказателиУпр"",
	...
	|				ИЗ РегистрСведений.ЗаданияКРасчетуСебестоимости КАК Задания
	|				ГДЕ
	|					Задания.Месяц <= РаспределениеПрочихИзОВЗ.Дата
	|					И Задания.Организация = РаспределениеПрочихИзОВЗ.Организация
	|				)
	|";
	ТекстРБПизТранзитныхОВЗПостоянныеПоказателиРегл = "
	|		ВЫБРАТЬ РАЗЛИЧНЫЕ
	|			//РежимОтладки ""ТекстРБПизТранзитныхОВЗПостоянныеПоказателиРегл"",
	|			ОбъектыВозникновенияЗатрат.Организация,
	|			ОбъектыВозникновенияЗатрат.Подразделение,
	|			ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка),
	|			ОбъектыВозникновенияЗатрат.Ссылка,
	...
	|		ВЫБРАТЬ РАЗЛИЧНЫЕ
	|			//РежимОтладки ""ТекстРБПизТранзитныхОВЗПостоянныеПоказателиНУ"",
	|			ОбъектыВозникновенияЗатрат.Организация,
	|			ОбъектыВозникновенияЗатрат.Подразделение,
	|			ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка),
	|			ОбъектыВозникновенияЗатрат.Ссылка,
	...
	|		ВЫБРАТЬ РАЗЛИЧНЫЕ
	|			//РежимОтладки ""ТекстРБПизТранзитныхОВЗПеременныеПоказателиУпр"",
	|			ОбъектыВозникновенияЗатрат.Организация,
	|			ОбъектыВозникновенияЗатрат.Подразделение,
	|			ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка),
	|			ОбъектыВозникновенияЗатрат.Ссылка,
	...
	|		ВЫБРАТЬ РАЗЛИЧНЫЕ
	|			//РежимОтладки ""ТекстРБПизТранзитныхОВЗПеременныеПоказателиРегл"",
	|			ОбъектыВозникновенияЗатрат.Организация,
	|			ОбъектыВозникновенияЗатрат.Подразделение,
	|			ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка),
	|			ОбъектыВозникновенияЗатрат.Ссылка,
	...
	|		ВЫБРАТЬ РАЗЛИЧНЫЕ
	|			//РежимОтладки ""ТекстРБПизТранзитныхОВЗПеременныеПоказателиНУ"",
	|			ОбъектыВозникновенияЗатрат.Организация,
	|			ОбъектыВозникновенияЗатрат.Подразделение,
	|			ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка),
	|			ОбъектыВозникновенияЗатрат.Ссылка,
	...
	//++ НЕ УТ
	ТекстыЗапроса.Добавить(ТекстСписаниеРасходовНаРБП);
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетПоОбъектамВозникновенияЗатрат") Тогда
		ТекстыЗапроса.Добавить(ТекстПриходыРБПОВЗ);
		ТекстыЗапроса.Добавить(ТекстСписаниеМатериаловРаботНаРБПОВЗ);
...
Функция ТекстЗапросаРасходыКРаспределению(Первые = Ложь) Экспорт
	...
	ТекстЗапроса = 
		...
		|		ИЛИ Данные.БезСуммНУ) И РаспределениеНУ.Ссылка ЕСТЬ NULL
		|	ИЛИ (Данные.СуммаНДДОстаток + Данные.СуммаНДДПриход - Данные.СуммаНДДРасход <> 0 
		|		ИЛИ Данные.БезСуммНДД) И РаспределениеНДД.Ссылка ЕСТЬ NULL
		//-- НЕ УТ
		|";
