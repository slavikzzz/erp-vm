...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.РасходыКРаспределениюГруппаКРаспределениюУпр.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'К распределению в упр. учете (%1)';
			|en = 'For allocation in manag. accounting (%1)'"),
...
Процедура СоздатьРаспределениеРасходов(Команда)
	...
	Если Строка <> Неопределено Тогда
		...
		Если СостояниеРаспределения = Неопределено Тогда
			...
			|en = 'Invalid data is entered for the selected line. 
			|More than one allocation document may have been entered in the current period.'");
		...
		//++ НЕ УТ
		Если СостояниеРаспределения.НалоговыйУчет
			И НЕ ЗначениеЗаполнено(СостояниеРаспределения.ДокументНУ) 
		Тогда
			НаборРазрезовУчета.Вставить("НалоговыйУчет", Истина);
			Если ЗначениеЗаполнено(СостояниеРаспределения.ПравилоРаспределенияРасходовНУ) Тогда
				НаборПравил.Вставить(СостояниеРаспределения.ПравилоРаспределенияРасходовНУ);
			КонецЕсли;
		Иначе
			НаборРазрезовУчета.Вставить("НалоговыйУчет", Ложь);
		КонецЕсли;
		//-- НЕ УТ
...
Функция ПолучитьСостояниеРаспределенияРБП(Параметры)
	...
	//++ НЕ УТ
	|НалоговыйУчет,ПравилоРаспределенияРасходовНУ,ДокументНУ,
	//-- НЕ УТ
	...
	Если ЗначениеЗаполнено(Параметры.СтатьяРасходов) Тогда
		ТекстЗапроса = "ВЫБРАТЬ
			|	
			|	СтатьиРасходов.ВариантРаспределенияРасходовУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов) КАК УправленческийУчет,
			|	ЕСТЬNULL(НастройкиПоОрганизацииИПодразделению.НастройкаУпр, ЕСТЬNULL(НастройкиПоОрганизации.НастройкаУпр, СтатьиРасходов.ПравилоРаспределенияРасходовУпр)) КАК ПравилоРаспределенияРасходовУпр,
			|	ЕСТЬNULL(ДокументыУУ.Ссылка, ЗНАЧЕНИЕ(Документ.РаспределениеРасходовБудущихПериодов.ПустаяСсылка)) КАК ДокументУпр,
			|
			|	НЕ &ИспользоватьРеглУчет ИЛИ СтатьиРасходов.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов) КАК НалоговыйУчет,
			|	ЕСТЬNULL(НастройкиПоОрганизацииИПодразделению.НастройкаНУ, ЕСТЬNULL(НастройкиПоОрганизации.НастройкаНУ, СтатьиРасходов.ПравилоРаспределенияРасходовНУ)) КАК ПравилоРаспределенияРасходовНУ,
			|	ЕСТЬNULL(ДокументыНУ.Ссылка, ЗНАЧЕНИЕ(Документ.РаспределениеРасходовБудущихПериодов.ПустаяСсылка)) КАК ДокументНУ,
			|	
			|	СтатьиРасходов.ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов) КАК РегламентированныйУчет,
			|	ЕСТЬNULL(НастройкиПоОрганизацииИПодразделению.НастройкаРегл, ЕСТЬNULL(НастройкиПоОрганизации.НастройкаРегл, СтатьиРасходов.ПравилоРаспределенияРасходовРегл)) КАК ПравилоРаспределенияРасходовРегл,
			|	ЕСТЬNULL(ДокументыРУ.Ссылка, ЗНАЧЕНИЕ(Документ.РаспределениеРасходовБудущихПериодов.ПустаяСсылка)) КАК ДокументРегл
			|
			|ИЗ
			|	ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиРасходов
			...
			|	ПО
			|		СтатьиРасходов.Ссылка = НастройкиПоОрганизации.СтатьяРасходов
			...
			|	ПО
			|		СтатьиРасходов.Ссылка = НастройкиПоОрганизацииИПодразделению.СтатьяРасходов
			|		И НастройкиПоОрганизацииИПодразделению.Организация = &Организация
			|		И НастройкиПоОрганизацииИПодразделению.Подразделение = &Подразделение
			|	
			|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК ДокументыУУ
			|	ПО
			|		ДокументыУУ.СтатьяРасходов = СтатьиРасходов.Ссылка
			...
			|		И ДокументыУУ.Подразделение = &Подразделение
			|		И ДокументыУУ.АналитикаРасходов = &АналитикаРасходов
			|		И ДокументыУУ.НаправлениеДеятельности = &НаправлениеДеятельности
			|		И ДокументыУУ.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
			|		И ДокументыУУ.УправленческийУчет
			|		И ДокументыУУ.Проведен
			|		И ДокументыУУ.ВариантУказанияСуммыУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСуммыРБП.ОпределяетсяАвтоматически)
			|	
			|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК ДокументыНУ
			|	ПО
			|		ДокументыНУ.СтатьяРасходов = СтатьиРасходов.Ссылка
			|		И ДокументыНУ.Организация = &Организация
			|		И ДокументыНУ.Подразделение = &Подразделение
			|		И ДокументыНУ.АналитикаРасходов = &АналитикаРасходов
			|		И ДокументыНУ.НаправлениеДеятельности = &НаправлениеДеятельности
			|		И ДокументыНУ.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
			|		И ДокументыНУ.НалоговыйУчет
			|		И ДокументыНУ.Проведен
			|		И ДокументыНУ.ВариантУказанияСуммыРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСуммыРБП.ОпределяетсяАвтоматически)
			|	
			|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК ДокументыРУ
			|	ПО
			|		ДокументыРУ.СтатьяРасходов = СтатьиРасходов.Ссылка
			...
			|		И ДокументыРУ.Подразделение = &Подразделение
			|		И ДокументыРУ.АналитикаРасходов = &АналитикаРасходов
			|		И ДокументыРУ.НаправлениеДеятельности = &НаправлениеДеятельности
			|		И ДокументыРУ.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
			|		И ДокументыРУ.РегламентированныйУчет
			|		И ДокументыРУ.Проведен
			|		И ДокументыРУ.ВариантУказанияСуммыРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияСуммыРБП.ОпределяетсяАвтоматически)
	...
	Иначе
		...
		ТекстЗапроса = "ВЫБРАТЬ
			...
			|
			|	НЕ &ИспользоватьРеглУчет ИЛИ СтатьиРасходов.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов) КАК НалоговыйУчет,
			|	СтатьиРасходов.ПравилоРаспределенияРасходовНУ КАК ПравилоРаспределенияРасходовНУ,
			|	ЕСТЬNULL(ДокументыНУ.Ссылка, ЗНАЧЕНИЕ(Документ.РаспределениеРасходовБудущихПериодов.ПустаяСсылка)) КАК ДокументНУ,
			|	
			|	СтатьиРасходов.ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов) КАК РегламентированныйУчет,
			|	СтатьиРасходов.ПравилоРаспределенияРасходовРегл КАК ПравилоРаспределенияРасходовРегл,
			...
			|
			|ИЗ
			|	Справочник.ОбъектыВозникновенияЗатрат КАК СтатьиРасходов
			|	
			|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК ДокументыУУ
			|	ПО
			|		ДокументыУУ.АналитикаРасходов = СтатьиРасходов.Ссылка
			|		И ДокументыУУ.Организация = &Организация
			|		И ДокументыУУ.Подразделение = &Подразделение
			|		И ДокументыУУ.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
			|		И ДокументыУУ.УправленческийУчет
			...
			|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК ДокументыНУ
			|	ПО
			|		ДокументыНУ.АналитикаРасходов = СтатьиРасходов.Ссылка
			|		И ДокументыНУ.Организация = &Организация
			|		И ДокументыНУ.Подразделение = &Подразделение
			|		И ДокументыНУ.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
			|		И ДокументыНУ.НалоговыйУчет
			...
			|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеРасходовБудущихПериодов КАК ДокументыРУ
			|	ПО
			|		ДокументыРУ.АналитикаРасходов = СтатьиРасходов.Ссылка
			|		И ДокументыРУ.Организация = &Организация
			|		И ДокументыРУ.Подразделение = &Подразделение
			|		И ДокументыРУ.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
			|		И ДокументыРУ.РегламентированныйУчет
			...
			|	
			|ГДЕ
			|	СтатьиРасходов.Ссылка = &АналитикаРасходов";
	КонецЕсли;
	//-- НЕ УТ
...
Процедура УстановитьУсловноеОформление()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетПоОбъектамВозникновенияЗатрат") Тогда
		...
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.РасходыКРаспределениюСтатьяРасходов.Имя);
		
		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("РасходыКРаспределению.СтатьяРасходов");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = ПланыВидовХарактеристик.СтатьиРасходов.ПустаяСсылка();
	
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст",  НСтр("ru = '<В рамках ОВЗ>';
																		|en = '<Within cost center>'"));
	...
	ЗаголовокГруппыРегл = 
		?(ИспользоватьРеглУчет, 
			НСтр("ru = 'К распределению в регл. учете';
				|en = 'For allocation in local accounting'"),
			НСтр("ru = 'К распределению в бух. учете';
				|en = 'For allocation in accounting'")
		);
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	//++ НЕ УТ
	ВедетсяУчетПостоянныхИВременныхРазниц = ИспользоватьРеглУчет И ПолучитьФункциональнуюОпцию("ВедетсяУчетПостоянныхИВременныхРазниц");
	//-- НЕ УТ
