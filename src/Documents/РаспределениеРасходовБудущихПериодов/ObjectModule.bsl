...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		//++ НЕ УТ
		СоставНастроекУчета = СоставНастроекУчета + ",ВариантРаспределенияРасходовНУ";
		//-- НЕ УТ
		...
		Если НЕ ДанныеЗаполнения.Свойство("ПравилоРБП") Тогда
			...
			Запрос.Текст = 
				...
				//++ НЕ УТ
				|		КОГДА СтатьиРасходов.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
				|			ТОГДА ЕСТЬNULL(СтатьиРасходов.ПравилоРаспределенияРасходовНУ.НачалоПериода, """")
				//-- НЕ УТ
				|		ИНАЧЕ """"
				...
				//++ НЕ УТ
				|		КОГДА СтатьиРасходов.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
				|			ТОГДА ЕСТЬNULL(СтатьиРасходов.ПравилоРаспределенияРасходовНУ.БазаРаспределенияРБП, НЕОПРЕДЕЛЕНО)
				//-- НЕ УТ
				|		ИНАЧЕ НЕОПРЕДЕЛЕНО
				...
				//++ НЕ УТ
				|		КОГДА СтатьиРасходов.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
				|			ТОГДА ЕСТЬNULL(СтатьиРасходов.ПравилоРаспределенияРасходовНУ.КоличествоМесяцев, 0)
				//-- НЕ УТ
				|		ИНАЧЕ 0
				|	КОНЕЦ КАК КоличествоМесяцев,
				|	СтатьиРасходов.ВариантРаспределенияРасходовРегл КАК ВариантРаспределенияРасходовРегл,
				//++ НЕ УТ
				|	СтатьиРасходов.ВариантРаспределенияРасходовНУ КАК ВариантРаспределенияРасходовНУ,
				//-- НЕ УТ
			...
			Если НЕ ЗначениеЗаполнено(СтатьяРасходов) Тогда
				...
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПланВидовХарактеристик.СтатьиРасходов", "Справочник.ОбъектыВозникновенияЗатрат");
				Запрос.УстановитьПараметр("Ссылка", АналитикаРасходов);
		...
		Иначе
			...
			//++ НЕ УТ
			Если НЕ ЗначениеЗаполнено(СтатьяРасходов) И ТипЗнч(АналитикаРасходов) = Тип("СправочникСсылка.ОбъектыВозникновенияЗатрат") Тогда
				
				НастройкиУчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(АналитикаРасходов, СоставНастроекУчета);
		...
		Если НастройкиУчета <> Неопределено Тогда
			...
			КонецЕсли;
			//++ НЕ УТ
			Если Не ДанныеЗаполнения.Свойство("НалоговыйУчет") Тогда
				ДанныеЗаполнения.Вставить("НалоговыйУчет", 
					Не ПолучитьФункциональнуюОпцию("ИспользоватьРеглУчет") ИЛИ НастройкиУчета.ВариантРаспределенияРасходовНУ = ВариантРаспределенияРБП);
			КонецЕсли;
			//-- НЕ УТ
		...
		//++ НЕ УТ
		Если ДанныеЗаполнения.Свойство("НалоговыйУчет") И НЕ ДанныеЗаполнения.НалоговыйУчет Тогда
			СуммаДокументаВР = СуммаДокументаРегл;
			СуммаДокументаРегл = 0;
			СуммаДокументаПР = 0;
		КонецЕсли;
		//-- НЕ УТ
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если СуммаДокумента <> РаспределениеРасходов.Итог("Сумма")
	 ...
	 Тогда
		Текст = НСтр("ru = 'Сумма по строкам в табличной части должна равняться соответствующим суммам документа';
					|en = 'Amount of rows in the table should be equal to the corresponding document amount'");
	...
	Если АвтораспределениеУпр ИЛИ АвтораспределениеРегл Тогда
		...
		ШаблонСообщения = НСтр(
			...
			|en = 'Line %1: for the item with the ""%2"" allocation option, the allocation date cannot be related to the current period when the deferred expense amount is determined automatically'");
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если Выборка.Следующий() Тогда
		ТекстСообщения = НСтр("ru = 'Документ с такими настройками уже проведен в данном месяце';
								|en = 'The document with these settings has already been posted this month'");
