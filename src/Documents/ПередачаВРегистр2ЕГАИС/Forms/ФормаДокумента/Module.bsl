...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Документ не используется с 15.05.2023г.';
				|en = 'Документ не используется с 15.05.2023г.'"),,,,Отказ);
		Возврат;
	КонецЕсли;
...
Процедура СтатусЕГАИСПредставлениеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если (Не ЗначениеЗаполнено(Объект.Ссылка)) Или (Не Объект.Проведен) Тогда
		...
		ТекстВопроса = НСтр("ru = '""Передача в регистр №2 ЕГАИС"" не проведен. Провести?';
							|en = '""Передача в регистр №2 ЕГАИС"" не проведен. Провести?'");
	...
	ИначеЕсли Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru = '""Передача в регистр №2 ЕГАИС"" был изменен. Провести?';
							|en = '""Передача в регистр №2 ЕГАИС"" был изменен. Провести?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
