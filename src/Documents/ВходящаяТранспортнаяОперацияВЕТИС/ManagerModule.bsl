			...
			Для каждого СтрокаМаршрута Из ДокументОбъект.МаршрутВозврата Цикл
				...
				Иначе
					ТекстОшибки = НСтр("ru = 'В документе %1 указана строка №%2 табличной части ""Маршрут возврата"", которая не была передана в сервис. Данные ответа сервиса не актуальны.';
										|en = 'В документе %1 указана строка №%2 табличной части ""Маршрут возврата"", которая не была передана в сервис. Данные ответа сервиса не актуальны.'");
			...
			Пока ВыборкаПоТЧТовары.Следующий() Цикл
				...
				Иначе
					ТекстОшибки = НСтр("ru = 'В документе %1 указан вид продукции ""%2"", который не был передан в сервис. Данные ответа сервиса не актуальны.';
										|en = 'В документе %1 указан вид продукции ""%2"", который не был передан в сервис. Данные ответа сервиса не актуальны.'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
						ДанныеДокумента.Документ,
...
// 
// Возвращаемое значение:
//  см. РегистрыСведений.СтатусыДокументовВЕТИС.РассчитатьСтатусыКПередаче
...
// 
// Возвращаемое значение:
//  см. РегистрыСведений.СтатусыДокументовВЕТИС.РассчитатьСтатусы
...
// 
// Возвращаемое значение:
//  Соответствие из КлючИЗначение - Операции допустимых действий:
//  * Ключ - ПеречислениеСсылка.ДальнейшиеДействияПоВзаимодействиюВЕТИС
//  * Значение - ПеречислениеСсылка.ВидыОперацийВЕТИС
...
//
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.СтатусыОбработкиВходящихТранспортныхОперацийВЕТИС - Статусы ошибок.
//
...
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.СтатусыОбработкиВходящихТранспортныхОперацийВЕТИС - Конечные статусы.
...
// 
// Параметры:
//  ФлагНаличияПроверкиРегионализации - Булево - Флаг наличия проверки регионализации
//  ФлагНаличияНесоответствия - Булево - Флаг наличия несоответствия
//  ФлагУказанияНомера - Булево - Флаг указания номера
//  ФлагУказанияНомераВСД - Булево - Флаг указания номера ВСД
// 
// Возвращаемое значение:
...
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.ДальнейшиеДействияПоВзаимодействиюВЕТИС - Все требующие ожидания
...
//  ПараметрыПередачи - Структура - Параметры передачи
//  ДополнительныеПараметры - Произвольный, Неопределено - Дополнительные параметры
// 
// Возвращаемое значение:
//   Массив из см. ИнтеграцияВЕТИСКлиентСервер.СтруктураСообщенияXML - сообщения xml
...
// Статус и дальнейшие дейстия возможности перемещения.
// 
// Параметры:
//  ДанныеДокумента - Структура - Данные документа:
// * НомерТранспортногоСредстваНеУказан - Булево - 
...
// 
// Возвращаемое значение:
//  Структура - Статус и дальнейшие дейстия возможности перемещения:
// * Статус - ПеречислениеСсылка.СтатусыОбработкиВходящихТранспортныхОперацийВЕТИС - 
// * ДальнейшиеДействия - Массив из ПеречислениеСсылка.ДальнейшиеДействияПоВзаимодействиюВЕТИС - 
...
// Действия вместо передачи данных.
// 
// Параметры:
//  ДальнейшиеДействия - Массив из ПеречислениеСсылка.ДальнейшиеДействияПоВзаимодействиюВЕТИС
...
// Обработать результат запроса на внесение номеров транспортных средств в пунктах перегрузки.
// 
// Параметры:
//  ДанныеДокумента - Структура - Данные документа
//  ХозяйствующийСубъект - СправочникСсылка.ХозяйствующиеСубъектыВЕТИС - Хозяйствующий субъект
...
// 
// Параметры:
//  СообщенияXML - Массив из см. ИнтеграцияВЕТИСКлиентСервер.СтруктураСообщенияXML - список сообщений для дальнейшей обработки.
//  ДокументСсылка - ДокументСсылка - Документ ссылка
//  ДанныеДокумента - Структура - передает параметры о данных документа из вызывающей функции.
//  НастройкаОбмена - Произвольный - Настройка обмена
	...
	Для Каждого ДанныеВСД Из ЗагруженныеВСД Цикл
		...
		Если Не СтрокаТЧНайдена Тогда
			...
			ТекстОшибки = НСтр("ru = 'При заполнении возвратного ВСД по документу %1 не удалось найти позицию документа по продукции %2 (количество %3 %4; срок годности %5)';
								|en = 'При заполнении возвратного ВСД по документу %1 не удалось найти позицию документа по продукции %2 (количество %3 %4; срок годности %5)'");
...
Функция ЗапросПроверкиВозможностиПеревозкиВРамкахРегионализацииXML(ДокументСсылка, ПараметрыПередачи, ДополнительныеПараметры)
	...
	Пока Товары.Следующий() Цикл
		...
		ТипДоставкиXDTO = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ДокументXDTO, "cargoType");
		ТипДоставкиXDTO.guid = Товары.ВидПродукцииИдентификатор;
		
		ДокументXDTO.cargoType.Добавить(ТипДоставкиXDTO);
	...
	Пока Маршруты.Следующий() Цикл
		...
		ТочкаМаршрутаXDTO = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ДокументXDTO.shipmentRoute, "routePoint");
		ТочкаМаршрутаXDTO.sqnId = Маршруты.НомерСтроки;
		
		Если ЗначениеЗаполнено(Маршруты.Предприятие) Тогда
			ТочкаМаршрутаXDTO.enterprise = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ТочкаМаршрутаXDTO, "enterprise");
			ТочкаМаршрутаXDTO.enterprise.guid = Маршруты.ПредприятиеИдентификатор;
		Иначе
			ТочкаМаршрутаXDTO.location = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(ТочкаМаршрутаXDTO, "location");
			...
			Если ДанныеАдреса = Неопределено Тогда
				ТекстОшибки = НСтр("ru = 'В строке №%1 табличной части ""Маршрут возврата"" не заполнены данные адреса. Перевыберите адрес из классификатора';
									|en = 'В строке №%1 табличной части ""Маршрут возврата"" не заполнены данные адреса. Перевыберите адрес из классификатора'");
				...
				Возврат СообщенияXML;
			Иначе
				ИнтеграцияВЕТИС.УстановитьАдрес(ТочкаМаршрутаXDTO.location, ДанныеАдреса);
			КонецЕсли;
		КонецЕсли;
		
		ДокументXDTO.shipmentRoute.routePoint.Добавить(ТочкаМаршрутаXDTO);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'Возвратные ВСД (сжатое представление с расширенной информацией)';
										|en = 'Возвратные ВСД (сжатое представление с расширенной информацией)'");
	...
	КомандаПечати.Представление = НСтр("ru = 'Возвратные ВСД (сжатое представление)';
										|en = 'Возвратные ВСД (сжатое представление)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
...
//  МассивОбъектов  - ДокументСсылка.ВходящаяТранспортнаяОперацияВЕТИС, Массив из ДокументСсылка.ВходящаяТранспортнаяОперацияВЕТИС - ссылка или массив ссылок на объекты которые нужно распечатать.
// 
// Возвращаемое значение:
// Структура - результат пакетного запроса, номера таблиц с данными для печати, вариант вывода расхождений:
//  * РезультатЗапроса - Массив из РезультатЗапроса
//  * Шапка - Число
//  * Товары - Число
//  * Упаковки - Число
//  * Партии - Число
