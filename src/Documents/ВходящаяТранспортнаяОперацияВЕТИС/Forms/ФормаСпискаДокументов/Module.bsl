	...
	Иначе 
		...
		ИначеЕсли НЕ ПолучитьВСДПоОтбору(Элементы.СписокТТНКОформлению.ТекущиеДанные, МассивВСД) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр( "ru = 'Нет данных для заполнения';
																|en = 'Нет данных для заполнения'"),,"СписокТТНКОформлению");
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура СписокТТНКОформлениюВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	ПоказатьПредупреждение(,НСтр("ru = 'Открытие ветеринарно-сопроводительных документов возможно в режиме просмотра ""по ВСД""';
								|en = 'Открытие ветеринарно-сопроводительных документов возможно в режиме просмотра ""по ВСД""'"),30);
КонецПроцедуры
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл 
		Если МассивВСД.Найти(ВыделеннаяСтрока) = Неопределено Тогда 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Различаются ключевые реквизиты ВСД. Выделенные строки должны быть включены в различные входящие транспортные операции';
					|en = 'Различаются ключевые реквизиты ВСД. Выделенные строки должны быть включены в различные входящие транспортные операции'"),,"СписокКОформлению");
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
