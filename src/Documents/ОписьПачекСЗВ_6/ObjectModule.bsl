...
Процедура ПроверитьДанныеДокумента(Отказ = Ложь) Экспорт
	...
	ТекстОшибки = НСтр("ru = 'Форма АДВ-6-2 подается за периоды до 2014 г.';
						|en = 'ADV-6-2 form is submitted for the periods before 2014.'");
	...
	Пока ВыборкаДокументовДляПроверки.Следующий() Цикл
		...
		Если ВыборкаДокументовДляПроверки.ОтчетныйПериод <> ОтчетныйПериод Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отчетный период документа-пачки отличается от указанного.';
																							|en = 'Accounting period of bundle document differs from the specified one.'"), ВыборкаДокументовДляПроверки.НомерСтроки);
		...
		Если ВыборкаДокументовДляПроверки.Организация <> Организация Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Документ-пачка оформлен на другую организацию.';
																							|en = 'Document bundle is registered with another company.'"), ВыборкаДокументовДляПроверки.НомерСтроки);
		...
		Если ВыборкаДокументовДляПроверки.НомерПачки = НомерПачки Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Документ-пачка имеет такой же номер как и документ описи, рекомендуется изменить номер пачки в документе описи.';
																							|en = 'Document bundle has the same number as the list document, it is recommended that you change the bundle number in the list document. '"), ВыборкаДокументовДляПроверки.НомерСтроки);
		...
		Если ВыборкаДокументовДляПроверки.НомерСтрокиДубль <> Null Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Документ-пачка указан дважды.';
																							|en = 'Document bundle is listed twice.'"), ВыборкаДокументовДляПроверки.НомерСтроки);
