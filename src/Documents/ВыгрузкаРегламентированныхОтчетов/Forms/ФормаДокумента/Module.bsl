...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВерсииФорматовВыгрузки.Вставить("ПеречислениеВерсииФорматовВыгрузкиВерсияМинтруда", Перечисления.ВерсииФорматовВыгрузки.ВерсияМинтруда);
...
Процедура ОрганизацияОчистка(Элемент, СтандартнаяОбработка)
	...
	ИзменениеПараметровОтбораАсинх(Новый ОписаниеОповещения("ОрганизацияОчисткаЗавершение", ЭтотОбъект));
КонецПроцедуры
...
Процедура ОрганизацияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИзменениеПараметровОтбораАсинх(Новый ОписаниеОповещения("ОрганизацияОбработкаВыбораЗавершение", ЭтотОбъект, ДополнительныеПараметры));
КонецПроцедуры
...
Процедура КодНалоговогоОрганаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОтключитьОбработчикОжидания("КодНалоговогоОрганаВыборОжидание");
	ПодключитьОбработчикОжидания("КодНалоговогоОрганаВыборОжидание", 0.1, Истина);
КонецПроцедуры

&НаКлиенте
Процедура КодНалоговогоОрганаВыборОжидание()
	ДополнительныеПараметры = Новый Структура("Элемент", Элементы.КодНалоговогоОргана);
	ИзменениеПараметровОтбораАсинх(Новый ОписаниеОповещения("КодНалоговогоОрганаНачалоВыбораЗавершение", ЭтотОбъект, ДополнительныеПараметры));
КонецПроцедуры

&НаКлиенте
Процедура ГодПриИзменении(Элемент)
	Если Год <> Год(Объект.ПериодПо) Тогда
		ИзменениеПараметровОтбораАсинх(Новый ОписаниеОповещения("ГодПриИзмененииЗавершение", ЭтотОбъект));
	КонецЕсли;
КонецПроцедуры
...
Процедура ПолеВыбораПериодПриИзменении(Элемент)
	...
	Если Месяц <> Месяц(Объект.ПериодПо) Тогда
		...
		ИзменениеПараметровОтбораАсинх(Новый ОписаниеОповещения("ПолеВыбораПериодПриИзмененииЗавершение", ЭтотОбъект, ДополнительныеПараметры));
	Иначе
		ПриИзмененииМесяца();
	КонецЕсли;
КонецПроцедуры
...
Процедура КодНалоговогоОрганаОчистка(Элемент, СтандартнаяОбработка)
	ИзменениеПараметровОтбораАсинх(Новый ОписаниеОповещения("КодНалоговогоОрганаОчисткаЗавершение", ЭтотОбъект));
КонецПроцедуры
...
Процедура ОснованияПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элементы.Основания.ТекущийЭлемент.Имя = "ОснованияПометка"
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'При изменении списка документов будут очищены тексты выгрузки.%1Продолжить?';
																					|en = 'При изменении списка документов будут очищены тексты выгрузки.%1Продолжить?'"), Символы.ПС);
...
Процедура Заполнить(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Не указана организация.';
									|en = 'Не указана организация.'"));
		Возврат;
	КонецЕсли;
	
	Если Объект.Выгрузки.Количество() <> 0 Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект);
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Тексты выгрузки будут очищены.%1Продолжить?';
																					|en = 'Тексты выгрузки будут очищены.%1Продолжить?'"), Символы.ПС);
...
Процедура ПоказатьТекстВыгрузки(Команда)
	...
	Если НепривязанныеТекстыВыгрузки.Количество() = 0 И Элементы.Основания.ТекущиеДанные.НомерСтрокиТекстаВыгрузки = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Для выбранного отчета текст выгрузки не сформирован!';
									|en = 'Для выбранного отчета текст выгрузки не сформирован!'"));
		Возврат;
	КонецЕсли;
...
Процедура СформироватьТекстыВыгрузки(Команда)
	...
	Если ОшибкаВыгрузки <> Истина Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Формирование текстов выгрузки успешно завершено.';
									|en = 'Формирование текстов выгрузки успешно завершено.'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура СформироватьИЗаписать(ДокументыОснования, ТекстВыгрузки = Неопределено, ПечатаемыеЛисты = Неопределено, ФайлДляСохранения = Неопределено, ДляЦелейФормированияМЧБ = Ложь, ПроверкаБлокировкиФормы = Ложь, КаталогСохранения = Неопределено, СообщенияОбОшибках = Неопределено, ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно = Истина, ВыгружатьСОшибками = Ложь, ФайлВыгрузкиВоВременномХранилище = Неопределено, УникальныйИДФормы = Неопределено, ВыполняемоеОповещение = Неопределено, СтруктураДополнительныхПараметров = Неопределено) Экспорт
	...
	Исключение
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																		|en = '%1'"), РегламентированнаяОтчетностьКлиент.СформироватьТекстСообщения(ОписаниеОшибки())));
	...
	Если ФайлДляСохранения = Неопределено Тогда
		...
		Если СтруктураДополнительныхПараметров <> Неопределено И СтруктураДополнительныхПараметров.Свойство("ТекстСообщенияОВыгрузкеП1Приложения3", ТекстСообщенияОВыгрузкеП1Приложения3) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='" + СтруктураДополнительныхПараметров.ТекстСообщенияОВыгрузкеП1Приложения3 + "'"));
		КонецЕсли;
	Иначе
		Если Объект.Выгрузки.Количество() = 0 Тогда
			Возврат;
			...
			#Если ВебКлиент Тогда
				...
				Если АдресФайлаВыгрузки <> Неопределено Тогда
					...
					Если ВыполняемоеОповещение <> Неопределено И ДляЦелейФормированияМЧБ Тогда
						НачатьПолучениеФайловССервера(ВыполняемоеОповещение, МассивПередаваемыхФайлов);
					КонецЕсли;
					Возврат;
				КонецЕсли;
		...
		Если НЕ ФайлВыгрузкиЗаписан Тогда
			...
			КодировкаФайлаВыгрузки = ?(Объект.Выгрузки.Получить(0).Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсия400
				...
				ИЛИ Объект.Выгрузки.Получить(0).Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияМинтруда
...
Процедура УстановитьСнятьФлажки(Пометка)
	...
	Если Объект.Выгрузки.Количество() <> 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("УстановитьСнятьФлажкиЗавершение", ЭтотОбъект, Новый Структура("Пометка", Пометка)), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'При изменении списка документов будут очищены тексты выгрузки.%1Продолжить?';
																																																	|en = 'При изменении списка документов будут очищены тексты выгрузки.%1Продолжить?'"), Символы.ПС), РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура СформироватьТекстыВыгрузкиКлиент(ПечатаемыеЛисты = Неопределено, ДляЦелейФормированияМЧБ = Ложь, ПроверкаБлокировкиФормы = Ложь, ДляОтправкиВФНС = Ложь, ПервыйДок = Неопределено, СообщенияОбОшибках = Неопределено, ВыгружатьСОшибками = Ложь, СтруктураДополнительныхПараметров = Неопределено)
	...
	ОтборМинтруда = ВерсииИОтчеты.НайтиСтроки(Новый Структура("Версия", ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияМинтруда));
		...
		Для Каждого Стр Из ОтборФСС Цикл
			...
			Попытка
				...
				Если ВладелецФормы = Неопределено
				 ИЛИ НЕ ТипЗнч(ВладелецФормы) = Тип("ФормаКлиентскогоПриложения") Тогда
					ПолныйПутьКФорме = РегламентированнаяОтчетностьВызовСервера.ПолныйПутьКФорме(Стр.ИсточникОтчета, ИмяТекФормы);
					ТекФорма = ПолучитьФорму(ПолныйПутьКФорме, ПараметрыФормы, , ?(ЗначениеЗаполнено(Стр.Док), Стр.Док, Новый УникальныйИдентификатор));
				Иначе
					ТекФорма = ВладелецФормы;
				КонецЕсли;
			...
			Исключение
				...
				Если СообщенияОбОшибках = Неопределено Тогда
					ОбщегоНазначенияКлиент.СообщитьПользователю(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось выгрузить %1.';
																					|en = 'Не удалось выгрузить %1.'"), РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Стр)));
	...
	//выгрузка в формате Минтруда
	Если ОтборМинтруда.Количество() <> 0 И (НЕ ВыгрузкаПрервана) Тогда
		
		Текст = Новый ТекстовыйДокумент;//текст, объединяющий все выгруженные документы в формате Минтруда
		ТекТекст = Новый ТекстовыйДокумент;//текст выгрузки текущего документа
		
		//последовательно, для каждого документа из списка, инициализируем "контекст"
		//(создаем окружение) и вызываем экспортную процедуру, формирующую текст выгрузки
		Для Каждого Стр Из ОтборМинтруда Цикл
			НомерОтчета = НомерОтчета + 1;
			СообщитьОбОбработкеОтчета(Стр, НомерОтчета, ВерсииИОтчеты.Количество());
				...
				Если ТипЗнч(ТекстВыгрузкиВXML) = Тип("Массив") Тогда
					Для Каждого ЭлементПакетаВыгрузки Из ТекстВыгрузкиВXML Цикл
						Если ТипЗнч(ЭлементПакетаВыгрузки) = Тип("Структура") Тогда
							...
							НовСтр.Тип      = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияМинтруда;
	...
	КонецЕсли;
	//конец выгрузки в формате Минтруда
	
	//выгрузка в формате ЕФС СФР
	Если ОтборЕФСПФР.Количество() <> 0 И (НЕ ВыгрузкаПрервана) Тогда
		
		Текст = Новый ТекстовыйДокумент;//текст, объединяющий все выгруженные документы в формате ЕФС СФР
		ТекТекст = Новый ТекстовыйДокумент;//текст выгрузки текущего документа
	...
	КонецЕсли;
	//конец выгрузки в формате ЕФС СФР
	...
	Если ОтборПФР.Количество() <> 0 И (НЕ ВыгрузкаПрервана) Тогда
		...
		Для Каждого Стр Из ОтборПФР Цикл
			...
			Исключение
				...
				Если СообщенияОбОшибках = Неопределено Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось выгрузить %1.';
																									|en = 'Не удалось выгрузить %1.'"), РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Стр));
					Сообщение.Сообщить();
					ОтобразитьФормуНавигацииПоОшибкам();
				Иначе
	...
	Если Отбор300.Количество() <> 0 И (НЕ ВыгрузкаПрервана) Тогда	
		...
		Для Каждого Стр Из Отбор300 Цикл
			...
			Если НЕ РегламентированнаяОтчетностьВызовСервера.ВывестиСведенияСлужебнойЧастиИОбОтправителе30(Текст, Объект.Организация, КПП, СокрЛП(Объект.КодНалоговогоОргана)) Тогда 
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не удалось сформировать общую информацию для файла выгрузки формата 3.00.';
																|en = 'Не удалось сформировать общую информацию для файла выгрузки формата 3.00.'"));
				ОшибкаВыгрузки = Истина;
				Возврат;
			КонецЕсли;
			...
			Если Стр.Отчет = "Статистика: Форма П-1" И Год(Стр.ДатаНачала) <= 2016 Тогда
				...
				Если СтруктураДополнительныхПараметров = Неопределено Тогда
					...
					Иначе
						...
						ТекФормаП1Пр3 = ПолучитьФорму(ПолныйПутьКФорме, ПараметрыФормы, , ?(ЗначениеЗаполнено(Стр.Док), Стр.Док, Новый УникальныйИдентификатор));
					КонецЕсли;	
				...
				Если ТекФормаП1Пр3 = Неопределено Тогда
					...
					Если НазваниеИсточникОтчета <> "Без приложения" Тогда
						ТекстСообщения = НСтр("ru = 'Не найден сохраненный отчет ""Статистика: Форма П-1 Приложение 3"" для выгрузки в файл XML - соответствующий раздел будет пустым.';
												|en = 'Не найден сохраненный отчет ""Статистика: Форма П-1 Приложение 3"" для выгрузки в файл XML - соответствующий раздел будет пустым.'");
				...
				Иначе
					...
					Исключение
						...
						Если СообщенияОбОшибках = Неопределено Тогда
							ОбщегоНазначенияКлиент.СообщитьПользователю(
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось выгрузить %1.';
																							|en = 'Не удалось выгрузить %1.'"), РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(СтрДокП1Пр3)));
							ОтобразитьФормуНавигацииПоОшибкам();
						Иначе
							ЗаполнитьСообщенияОбОшибках(СообщенияОбОшибках);
						КонецЕсли;
						...
						Если ТипЗнч(ЭлементПакетаВыгрузки) = Тип("Структура") Тогда
							...
							Иначе
								НовСтр.Текст = НСтр("ru = 'Отчет ФСН был сохранен в каталог в бинарном формате';
													|en = 'Отчет ФСН был сохранен в каталог в бинарном формате'");
							КонецЕсли;
...
Процедура ОбработатьСобытие1СОтчетностиНаСервере(ПервыйДок)
	...
	РегламентированнаяОтчетность.ОбработатьСобытие1СОтчетности(НСтр("ru = 'Регламентированный отчет. Выгрузка';
																	|en = 'Регламентированный отчет. Выгрузка'"), ПервыйДок);
...
Процедура СформироватьТекстыВыгрузкиСервер(ПервыйДок, ДляОтправкиВФНС, КПП, ОшибкаВыгрузки)
	...
	Если КоличествоДокументовДляВыгрузки = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не выбраны документы.';
													|en = 'Не выбраны документы.'"));
		ОшибкаВыгрузки = Истина;
		Возврат;
	КонецЕсли;
	...
	Для Каждого Стр Из Основания Цикл
		Если Стр.Организация <> Основания.Получить(0).Организация Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'У выбранных отчетов различаются реквизиты ""Организация"". Пакетная выгрузка невозможна.';
														|en = 'У выбранных отчетов различаются реквизиты ""Организация"". Пакетная выгрузка невозможна.'"));
			ОшибкаВыгрузки = Истина;
			Возврат;
		КонецЕсли;
		
		Если КонецДня(Стр.ДатаОкончания) <> КонецДня(Основания.Получить(0).ДатаОкончания) Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'У выбранных отчетов различаются даты окончания периодов составления. Пакетная выгрузка невозможна.';
														|en = 'У выбранных отчетов различаются даты окончания периодов составления. Пакетная выгрузка невозможна.'"));
			ОшибкаВыгрузки = Истина;
			Возврат;
		КонецЕсли;
	КонецЦикла;
	...
	Если НалоговыеОрганы.Количество() > 0 Тогда
		...
		Если НЕ НалоговыеОрганы.Количество() = НайденныеСтроки.Количество() Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'У выбранных отчетов различаются коды ИФНС, в которую они должны представляться. Процесс прерван.';
														|en = 'У выбранных отчетов различаются коды ИФНС, в которую они должны представляться. Процесс прерван.'"));
			ОшибкаВыгрузки = Истина;
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОтчетыВыгружаемыеТолькоИзФормы.Количество() > 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Выгрузка следующих отчетов возможна только из формы отчета:';
													|en = 'Выгрузка следующих отчетов возможна только из формы отчета:'"));
	...
	Если ЕстьНеопределенныеФорматы Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Для следующих отчетов выгрузка не предусмотрена:';
													|en = 'Для следующих отчетов выгрузка не предусмотрена:'"));
		...
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Продолжение невозможно. Формирование текстов выгрузки прервано.';
													|en = 'Продолжение невозможно. Формирование текстов выгрузки прервано.'"));
		ОшибкаВыгрузки = Истина;
		Возврат;
	КонецЕсли;
	...
	ОтчетыМинтруда = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыМинтруда();
	...
	Для Каждого Стр Из ВерсииИОтчеты Цикл
		...
		ИначеЕсли ОтчетыМинобороны.НайтиПоЗначению(ИсточникОтчета) <> Неопределено Тогда
			...
			КонецЕсли;
			Продолжить;
		ИначеЕсли ОтчетыМинтруда.НайтиПоЗначению(ИсточникОтчета) <> Неопределено Тогда
			Продолжить;
	...
	Если ДляОтправкиВФНС И ОтчетыНеотправляемыеВФНС.Количество() > 0 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Следующие отчеты не предназначены для отправки в ФНС по каналам связи:';
								|en = 'Следующие отчеты не предназначены для отправки в ФНС по каналам связи:'");
		Сообщение.Сообщить();
	...
	Если ОтчетыСохраненныеСНевернымОкруглением.Количество() <> 0 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Следующие отчеты сохранены с единицей измерения и/или точностью, не соответствующими требованиям формата выгрузки:';
								|en = 'Следующие отчеты сохранены с единицей измерения и/или точностью, не соответствующими требованиям формата выгрузки:'");
		Сообщение.Сообщить();
		...
		Сообщение.Текст = НСтр("ru = 'Сохраните отчеты с нужной единицей измерения и перезапустите процесс формирования.';
								|en = 'Сохраните отчеты с нужной единицей измерения и перезапустите процесс формирования.'");
		Сообщение.Сообщить();
...
Функция ВывестиОписаниеФормыОтчетности(Текст, Организация, КодНалоговогоОргана)
	...
	Если НЕ РегламентированнаяОтчетностьВызовСервера.ВывестиСведенияСлужебнойЧастиИОбОтправителе30(Текст, Организация, , СокрЛП(КодНалоговогоОргана), "2.01") Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось сформировать общую информацию для файла выгрузки формата 2.01.';
								|en = 'Не удалось сформировать общую информацию для файла выгрузки формата 2.01.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ВывестиСведенияИнформационнойЧасти201ИСведенияОбОрганизации(Текст, Организация) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось выгрузить сведения об организации для файла выгрузки формата 2.01.';
								|en = 'Не удалось выгрузить сведения об организации для файла выгрузки формата 2.01.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ ВывестиСведенияОРуководствеОрганизации201(Текст, Организация) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось выгрузить сведения о руководстве организации для файла выгрузки формата 2.01.';
								|en = 'Не удалось выгрузить сведения о руководстве организации для файла выгрузки формата 2.01.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
...
Функция СформироватьИмяФайла(Док = Неопределено, ВерсияФормата = Неопределено)
	...
	Если ВерсияФормата = Неопределено Тогда
		...
		Если КППОтчета <> Неопределено Тогда
			...
			ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Организация,
				"ИНН,КрупнейшийНалогоплательщик,КодНалоговогоОрганаПолучателя");
			
			ИННОтчета = ЗначенияРеквизитов.ИНН;
			
			КодИФНСПосредника = ?(ЗначенияРеквизитов.КрупнейшийНалогоплательщик,
				ЗначенияРеквизитов.КодНалоговогоОрганаПолучателя, "");
	...
	ИначеЕсли ВерсияФормата <> Перечисления.ВерсииФорматовВыгрузки.Версия302алко Тогда
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Организация, "ИНН");
		
		Возврат "A"
				+ Сред(Док.ИсточникОтчета, 38, 1)
				+ Сред(СокрЛП(ЗначенияРеквизитов.ИНН), 5, 5)
				+ Прав(Формат(Док.ДатаОкончания, "ДФ=y; ДП=0"), 1);
				
	Иначе
		ПризнакПериодаОтчета = ?(Док.Периодичность = Перечисления.Периодичность.Квартал, 2, 1);
		
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Док.Организация, "ИНН,КПП");
		ЗначенияРеквизитовИНН = ЗначенияРеквизитов.ИНН;
		ЗначенияРеквизитовКПП = ЗначенияРеквизитов.КПП;
		
		Возврат "A"
				+ Сред(Док.ИсточникОтчета, 38, 1)
				+ СокрЛП(ЗначенияРеквизитовИНН)
				+ ЗначенияРеквизитовКПП
...
Процедура СохранитьТексты(НЕЗаписыватьДокумент,
	...
	Если Объект.Выгрузки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Нет текстов выгрузки для сохранения!';
									|en = 'Нет текстов выгрузки для сохранения!'"));
		ОшибкаВыгрузки = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ЧислоВыгружаемыхОтчетов = 0 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Нет информации для сохранения!';
								|en = 'Нет информации для сохранения!'");
		Сообщение.Сообщить();
	...
	Для Каждого Стр Из СписокВыбора Цикл
		...
		Если НЕ ПустаяСтрока(ТекСтр.ИмяФайла) Тогда
			...
			Если ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсия400
				...
				ИЛИ ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияМинтруда
								...
								Если ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно Тогда
										...
										ИначеЕсли ВыполняетсяСохранениеБолееОдногоФайлаВыгрузкиВБраузереCHROME Тогда
											...
											ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗакрытияВопроса", ЭтотОбъект, ДополнительныеПараметры),
															СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																НСтр("ru = 'Сохранить файл ""%1""?';
																	|en = 'Сохранить файл ""%1""?'"), ИмяКонечногоФайла),
    														РежимДиалогаВопрос.ДаНет);
											
										Иначе
											
											НачатьПолучениеФайлаССервера(АдресФайлаВыгрузки, ИмяКонечногоФайла);
									...
									Иначе
										...
										Иначе
											...
											НачатьПолучениеФайлаССервера(ПоместитьФайлВыгрузкиВоВременноеХранилищеНаСервере(
														 Текст, ТекСтр.ИмяФайла, КодировкаТекстаВыгрузки), ИмяКонечногоФайла);
								...
								Если МассивДополнительныхФайлов.Количество() > 0 Тогда
											...
											Для ИндМассива = 0 По МассивДополнительныхФайлов.Количество() - 1 Цикл
												...
												ИначеЕсли ВыполняетсяСохранениеБолееОдногоФайлаВыгрузкиВБраузереCHROME Тогда
													...
													ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗакрытияВопроса", ЭтотОбъект, ДополнительныеПараметры),
																	СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																		НСтр("ru = 'Сохранить файл ""%1""?';
																			|en = 'Сохранить файл ""%1""?'"), ИмяТекДопФайла),
    																РежимДиалогаВопрос.ДаНет);
													
												Иначе
													
													НачатьПолучениеФайлаССервера(АдресДанныхТекДопФайла, ИмяТекДопФайла);
									...
									#Иначе
										...
										Если МассивПередаваемыхФайлов.Количество() > 0 Тогда
											...
											НачатьПолучениеФайловССервера(ОписаниеОповещения, МассивПередаваемыхФайлов);
								...
								Если ТекСтр.Тип <> ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияФСГС Тогда
									...
									Если СписокДополнительныхФайлов.Количество() > 0 Тогда
										...
										Сообщение.Текст = НСтр("ru = 'К файлу выгрузки бухгалтерской отчетности приложите:';
																|en = 'К файлу выгрузки бухгалтерской отчетности приложите:'");
										Сообщение.Сообщить();
										...
										Для Каждого ДополнительныйФайл Из СписокДополнительныхФайлов Цикл
											...
											НСтр("ru = '%1';
												|en = '%1'"), ДополнительныйФайл.Представление);
											Сообщение.Сообщить();
											
											Сообщение = Новый СообщениеПользователю;
											Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru = '%1';
												|en = '%1'"), ДополнительныйФайл.Значение);
											Сообщение.Сообщить();
				...
				Исключение
					...
					Если ФайлВыгрузкиВоВременномХранилище = Неопределено Тогда
						...
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось записать файл ""%1"". Возможно, нет прав для записи в выбранный каталог, нет доступа к файлу, т. к. файл открыт в другой программе, недостаточно места на диске.';
																										|en = 'Не удалось записать файл ""%1"". Возможно, нет прав для записи в выбранный каталог, нет доступа к файлу, т. к. файл открыт в другой программе, недостаточно места на диске.'"), ИмяКонечногоФайла);
						...
						#Если ВебКлиент Тогда
							Если НЕ ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно Тогда
								Сообщение.Текст = Сообщение.Текст + НСтр("ru = ' Также, возможно, не подключено расширение для работы с файлами.';
																		|en = ' Также, возможно, не подключено расширение для работы с файлами.'");
							КонецЕсли;
						#КонецЕсли
						
					Иначе
						
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось поместить файл выгрузки ""%1"" во временное хранилище!';
																										|en = 'Не удалось поместить файл выгрузки ""%1"" во временное хранилище!'"), ИмяКонечногоФайла);
							...
							#Если ВебКлиент Тогда
								...
								Если ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно Тогда
									...
									Иначе	
										...
										НачатьПолучениеФайлаССервера(ПоместитьФайлВыгрузкиВоВременноеХранилищеНаСервере(Текст, КороткоеИмяФайла, КодировкаТекстаВыгрузки), ИмяКонечногоФайла);
		...
		Если БылаОшибкаЗаписи = Истина Тогда
			Если НЕ РежимВызоваЭкспортируемогоМетодаФормы Тогда
				...
				Сообщение.Текст = НСтр("ru = 'В процессе записи отчетов произошла ошибка!';
										|en = 'В процессе записи отчетов произошла ошибка!'");
				Сообщение.Сообщить();
					...
					#Иначе
						...
						Если МассивПередаваемыхФайловКомплекта.Количество() > 0 Тогда
							...
							Иначе
								...
								|en = 'Выполняется сохранение файлов комплекта.
								|Пожалуйста, подождите.'");
								...
								НачатьПолучениеФайловССервера(ОписаниеОповещения, МассивПередаваемыхФайловКомплекта);
...
Процедура Подключаемый_ПолучитьФайл()
	...
	НачатьПолучениеФайлаССервера(МассивПередаваемыхФайлов[0].Хранение, МассивПередаваемыхФайлов[0].Имя);
...
Процедура СохранитьТекстыПослеПоказаДиалогаВыбораКаталога(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ВыбранныеФайлы = Неопределено Тогда
		...
		НачатьПолучениеФайловССервера(ОписаниеОповещения, ДополнительныеПараметры.МассивПередаваемыхФайловКомплекта);
					...
					Попытка
						...
						Если СтруктураПараметров.Свойство("ТипФайлаВыгрузки", ТипФайлаВыгрузки) И ТипФайлаВыгрузки = "ОтчетФСНБР" Тогда
							ТекСтрока.Текст = НСтр("ru = 'Отчет ФСН был сохранен в каталог в бинарном формате';
													|en = 'Отчет ФСН был сохранен в каталог в бинарном формате'");
						ИначеЕсли СтруктураПараметров.Свойство("ТипФайлаВыгрузки", ТипФайлаВыгрузки)
								И СтрНачинаетсяС(ТипФайлаВыгрузки, "НДС")
								И СтрНайти(ТипФайлаВыгрузки, "Разделы1_7") = 0
								И СтрЧислоВхождений(ТипФайлаВыгрузки, "Раздел") = 1 Тогда
							ТекСтрока.Текст = НСтр("ru = 'Расширенные разделы декларации с 8-го по 12-й сохраняются только в каталог';
													|en = 'Расширенные разделы декларации с 8-го по 12-й сохраняются только в каталог'");
						Иначе
...
Функция ОпределитьИмяФайлаВыгрузкиФССПоТекстуВыгрузки(ТекстВыгрузки)
	...
	Если ДеревоXML = Неопределено Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось определить имя файла выгрузки ФСС по тексту выгрузки.';
								|en = 'Не удалось определить имя файла выгрузки ФСС по тексту выгрузки.'");
...
Процедура ПолучитьДополнительныеФайлыБухгалтерскойОтчетности(ТекстОсновногоФайла, ПутьВыгрузки, МассивДополнительныхФайлов, СписокДополнительныхФайлов, ИменаЗаписанныхФайловВыгрузки)
	...
	Если ЗначениеЗаполнено(ОтчетОснование) Тогда
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОтчетОснование, "Ссылка");
		...
		Если ЭлементыПояснительнаяЗаписка.Количество() > 0 Тогда
			...
			Если АтрибутУзла.Указан Тогда
				...
				ЗаписьРегистраСведений.РегламентированныйОтчет = ЗначенияРеквизитов.Ссылка;
...
Функция ВывестиСведенияИнформационнойЧасти201ИСведенияОбОрганизации(Текст, Орг)
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Орг, "ИНН,КПП");
		
	ОргИНН = СокрЛП(ЗначенияРеквизитов.ИНН);
	ОргКПП = ЗначенияРеквизитов.КПП;
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Основания.Получить(0).Ссылка, "ДатаНачала");
	ЗначенияРеквизитовДатаНачала = ЗначенияРеквизитов.ДатаНачала;
	
	СчДок = РегламентированнаяОтчетность.ПолучитьСчетчикВыгруженныхДокументов(Орг, ЗначенияРеквизитовДатаНачала) + 1;
	
	ИдДок = ИДОтпр + Формат(Год(ЗначенияРеквизитовДатаНачала), "ЧГ=") + Формат(СчДок, "ЧЦ=8; ЧН=; ЧВН=; ЧГ=");
	...
	Если Св.ТипНП = 1 Тогда//российская организация
		...
		Если НЕ ЗначениеЗаполнено(НаимЮЛПол) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Для организации не задано полное наименование!';
									|en = 'Для организации не задано полное наименование!'");
		...
		Если НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ИННЮЛ, Ложь) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Для организации неверно задан ИНН!';
									|en = 'Для организации неверно задан ИНН!'");
		...
		Если НЕ ПустаяСтрока(ОГРН) И НЕ РегламентированнаяОтчетность.ОГРНСоответствуетТребованиям(ОГРН) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Для организации неверно задан ОГРН!';
									|en = 'Для организации неверно задан ОГРН!'");
	...
	ИначеЕсли Св.ТипНП = 2 Тогда//обособленное подразделение
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Орг, "ГоловнаяОрганизация");
		ЗначенияРеквизитовГоловнаяОрганизация = ЗначенияРеквизитов.ГоловнаяОрганизация;
		
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЗначенияРеквизитовГоловнаяОрганизация, "НаименованиеПолное,КПП");
		ЗначенияРеквизитовНаименованиеПолное = ЗначенияРеквизитов.НаименованиеПолное;
		ЗначенияРеквизитовКПП = ЗначенияРеквизитов.КПП;
		
		НаимЮЛПол = СокрЛП(ЗначенияРеквизитовНаименованиеПолное);
		НаимОППол = СокрЛП(Св.НаимОППол);
		ИННЮЛ = СокрЛП(Св.ИННЮЛ);
		КППЮЛ = СокрЛП(ЗначенияРеквизитовКПП);
		КППОП = СокрЛП(Св.КППОП);
		ОГРН = СокрЛП(Св.ОГРН);
		ПризАдр = 1;
		АдрОП = СокрЛП(Св.АдрЮр);
		СвГоловОрг = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(ЗначенияРеквизитовГоловнаяОрганизация);
		...
		Если НЕ ЗначениеЗаполнено(НаимЮЛПол) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Для головной организации не задано полное наименование!';
									|en = 'Для головной организации не задано полное наименование!'");
		...
		Если НЕ ЗначениеЗаполнено(НаимОППол) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Для обособленного подразделения не задано полное наименование!';
									|en = 'Для обособленного подразделения не задано полное наименование!'");
	...
	ИначеЕсли Св.ТипНП = 3 Тогда//ИП
		...
		Если НЕ ЗначениеЗаполнено(Св.ФИО) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Для индивидуального предпринимателя не задано ФИО!';
									|en = 'Для индивидуального предпринимателя не задано ФИО!'");
		...
		Если НЕ РегламентированнаяОтчетность.ИННСоответствуетТребованиямБезТипаНаСервере(ИННФЛ) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Для индивидуального предпринимателя неверно задан ИНН!';
									|en = 'Для индивидуального предпринимателя неверно задан ИНН!'");
	...
	ИначеЕсли Св.ТипНП = 5 Тогда//иностранная организация
		...
		Если НЕ ЗначениеЗаполнено(НаимОИОПол) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Для отделения иностранной организации не задано полное наименование!';
									|en = 'Для отделения иностранной организации не задано полное наименование!'");
		...
		Если НЕ РегламентированнаяОтчетность.ИННСоответствуетТребованиямБезТипаНаСервере(ИННИО) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Для иностранной организации неверно задан ИНН!';
									|en = 'Для иностранной организации неверно задан ИНН!'");
...
Функция ПроверитьИОбработатьАдрЮр(Сведения)
	...
	Если Сведения.АдрЮр = "643,,,,,,,,," Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось определить юридический адрес организации!';
								|en = 'Не удалось определить юридический адрес организации!'");
	...
	Иначе
		Если СтрЧислоВхождений(Сведения.АдрЮр, ",") <> 9 Тогда
			...
			Сообщение.Текст = НСтр("ru = 'В информационной базе указан некорректный юридический адрес организации (число запятых в адресе должно быть равно девяти)!';
									|en = 'В информационной базе указан некорректный юридический адрес организации (число запятых в адресе должно быть равно девяти)!'");
		...
		Иначе
			...
			Если ПустаяСтрока(СтрАдрес.Индекс) Тогда
				...
				Сообщение.Текст = НСтр("ru = 'В юридическом адресе организации, хранящемся в информационной базе, не указан индекс!';
										|en = 'В юридическом адресе организации, хранящемся в информационной базе, не указан индекс!'");
			...
			Если ПустаяСтрока(СтрАдрес.Регион) Тогда
				...
				Сообщение.Текст = НСтр("ru = 'В юридическом адресе организации, хранящемся в информационной базе, не указан регион!';
										|en = 'В юридическом адресе организации, хранящемся в информационной базе, не указан регион!'");
			...
			Иначе
				...
				Если НЕ ЗначениеЗаполнено(КодРегиона) Тогда
					...
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось определить код региона по его названию, выделенному из юридического адреса организации.%1Проверьте правильность указания названия региона в юридическом адресе!';
																									|en = 'Не удалось определить код региона по его названию, выделенному из юридического адреса организации.%1Проверьте правильность указания названия региона в юридическом адресе!'"), Символы.ПС);
...
Функция ПроверитьИОбработатьАдрГолОрг(Сведения)
	...
	Если Сведения.АдрЮр = "643,,,,,,,,," Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось определить юридический адрес головной организации!';
								|en = 'Не удалось определить юридический адрес головной организации!'");
	...
	Иначе
		Если СтрЧислоВхождений(Сведения.АдрЮр, ",") <> 9 Тогда
			...
			Сообщение.Текст = НСтр("ru = 'В информационной базе указан некорректный юридический адрес головной организации (число запятых в адресе должно быть равно девяти)!';
									|en = 'В информационной базе указан некорректный юридический адрес головной организации (число запятых в адресе должно быть равно девяти)!'");
		...
		Иначе
			...
			Если ПустаяСтрока(СтрАдрес.Индекс) Тогда
				...
				Сообщение.Текст = НСтр("ru = 'В юридическом адресе головной организации, хранящемся в информационной базе, не указан индекс!';
										|en = 'В юридическом адресе головной организации, хранящемся в информационной базе, не указан индекс!'");
			...
			Если ПустаяСтрока(СтрАдрес.Регион) Тогда
				...
				Сообщение.Текст = НСтр("ru = 'В юридическом адресе головной организации, хранящемся в информационной базе, не указан регион!';
										|en = 'В юридическом адресе головной организации, хранящемся в информационной базе, не указан регион!'");
			...
			Иначе
				...
				Если НЕ ЗначениеЗаполнено(КодРегиона) Тогда
					...
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось определить код региона по его названию, выделенному из юридического адреса головной организации.%1Проверьте правильность указания названия региона в юридическом адресе головной организации!';
																									|en = 'Не удалось определить код региона по его названию, выделенному из юридического адреса головной организации.%1Проверьте правильность указания названия региона в юридическом адресе головной организации!'"), Символы.ПС);
...
Функция ВывестиСведенияОРуководствеОрганизации201(Текст, Орг)
	...
	Если ПустаяСтрока(Св.ФИОРук) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось определить ФИО руководителя организации!';
								|en = 'Не удалось определить ФИО руководителя организации!'");
	...
	И (Св.АдрМЖРукРф = "643,,,,,,,,,") Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось определить ИНН руководителя организации!';
								|en = 'Не удалось определить ИНН руководителя организации!'");
	...
	ИначеЕсли ЗначениеЗаполнено(Св.ИННРук) Тогда
		Если НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(Св.ИННРук, Истина) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'ИНН руководителя организации не соответствует требованиям!';
									|en = 'ИНН руководителя организации не соответствует требованиям!'");
	...
	Иначе
		Если НЕ ЗначениеЗаполнено(Св.КодУдЛичнРук) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не удалось определить код вида документа, удостоверяющего личность руководителя организации!';
									|en = 'Не удалось определить код вида документа, удостоверяющего личность руководителя организации!'");
		...
		Если НЕ ЗначениеЗаполнено(Св.СерияИНомерУдЛичнРук) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не удалось определить серию и номер документа, удостоверяющего личность руководителя организации!';
									|en = 'Не удалось определить серию и номер документа, удостоверяющего личность руководителя организации!'");
		...
		Если НЕ ЗначениеЗаполнено(Св.ДатаУдЛичнРук) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не удалось определить дату выдачи удостоверения личности руководителя организации!';
									|en = 'Не удалось определить дату выдачи удостоверения личности руководителя организации!'");
		...
		Если Св.АдрМЖРукРФ = "643,,,,,,,,," Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не удалось определить адрес места жительства руководителя организации!';
									|en = 'Не удалось определить адрес места жительства руководителя организации!'");
		...
		Иначе	
			Если СтрЧислоВхождений(Св.АдрМЖРукРФ, ",") <> 9 Тогда
				...
				Сообщение.Текст = НСтр("ru = 'В информационной базе указан некорректный адрес места жительства руководителя организации (число запятых в адресе должно быть равно девяти)!';
										|en = 'В информационной базе указан некорректный адрес места жительства руководителя организации (число запятых в адресе должно быть равно девяти)!'");
			...
			Иначе
				...
				Если ПустаяСтрока(СтрАдресРук.Индекс) Тогда
					...
					Сообщение.Текст = НСтр("ru = 'Не указан индекс в адресе места жительства руководителя организации!';
											|en = 'Не указан индекс в адресе места жительства руководителя организации!'");
				...
				Если ПустаяСтрока(СтрАдресРук.Регион) Тогда
					...
					Сообщение.Текст = НСтр("ru = 'Не указан регион в адресе места жительства руководителя организации!';
											|en = 'Не указан регион в адресе места жительства руководителя организации!'");
				...
				Иначе
					...
					Если Не ЗначениеЗаполнено(КодРегионаРук) Тогда
						...
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось определить код региона по его названию, выделенному из адреса места жительства руководителя организации.%1Проверьте правильность указания названия региона в адресе места жительства руководителя!';
																										|en = 'Не удалось определить код региона по его названию, выделенному из адреса места жительства руководителя организации.%1Проверьте правильность указания названия региона в адресе места жительства руководителя!'"), Символы.ПС);
	...
	Если ПустаяСтрока(Св.ФИОБух) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось определить ФИО главного бухгалтера организации!';
								|en = 'Не удалось определить ФИО главного бухгалтера организации!'");
	...
	И Св.АдрМЖБухРф = "643,,,,,,,,," Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не удалось определить ИНН главного бухгалтера организации!';
								|en = 'Не удалось определить ИНН главного бухгалтера организации!'");
	...
	ИначеЕсли ЗначениеЗаполнено(Св.ИННБух) Тогда
		Если НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(Св.ИННБух, Истина) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'ИНН главного бухгалтера организации не соответствует требованиям!';
									|en = 'ИНН главного бухгалтера организации не соответствует требованиям!'");
	...
	Иначе
		Если НЕ ЗначениеЗаполнено(Св.КодУдЛичнБух) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не удалось определить код вида документа, удостоверяющего личность главного бухгалтера организации!';
									|en = 'Не удалось определить код вида документа, удостоверяющего личность главного бухгалтера организации!'");
		...
		Если НЕ ЗначениеЗаполнено(Св.СерияИНомерУдЛичнБух) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не удалось определить серию и номер документа, удостоверяющего личность главного бухгалтера организации!';
									|en = 'Не удалось определить серию и номер документа, удостоверяющего личность главного бухгалтера организации!'");
		...
		Если НЕ ЗначениеЗаполнено(Св.ДатаУдЛичнБух) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не удалось определить дату выдачи удостоверения личности главного бухгалтера организации!';
									|en = 'Не удалось определить дату выдачи удостоверения личности главного бухгалтера организации!'");
		...
		Если Св.АдрМЖБухРФ = "643,,,,,,,,," Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не удалось определить адрес места жительства главного бухгалтера организации!';
									|en = 'Не удалось определить адрес места жительства главного бухгалтера организации!'");
		...
		Иначе	
			Если СтрЧислоВхождений(Св.АдрМЖБухРФ, ",") <> 9 Тогда
				...
				Сообщение.Текст = НСтр("ru = 'В информационной базе указан некорректный адрес места жительства главного бухгалтера организации (число запятых в адресе должно быть равно девяти)!';
										|en = 'В информационной базе указан некорректный адрес места жительства главного бухгалтера организации (число запятых в адресе должно быть равно девяти)!'");
			...
			Иначе
				...
				Если ПустаяСтрока(СтрАдресБух.Индекс) Тогда
					...
					Сообщение.Текст = НСтр("ru = 'Не указан индекс в адресе места жительства главного бухгалтера организации!';
											|en = 'Не указан индекс в адресе места жительства главного бухгалтера организации!'");
				...
				Если ПустаяСтрока(СтрАдресБух.Регион) Тогда
					...
					Сообщение.Текст = НСтр("ru = 'Не указан регион в адресе места жительства главного бухгалтера организации!';
											|en = 'Не указан регион в адресе места жительства главного бухгалтера организации!'");
				...
				Иначе
					...
					Если Не ЗначениеЗаполнено(КодРегионаБух) Тогда
						...
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось определить код региона по его названию, выделенному из адреса места жительства главного бухгалтера организации.%1Проверьте правильность указания названия региона в адресе места жительства главного бухгалтера!';
																										|en = 'Не удалось определить код региона по его названию, выделенному из адреса места жительства главного бухгалтера организации.%1Проверьте правильность указания названия региона в адресе места жительства главного бухгалтера!'"), Символы.ПС);
...
// Проверить возможность выгрузки регламентированных отчетов.
// 
// Параметры:
//   ДокументыОснования - СписокЗначений из ДокументСсылка.РегламентированныйОтчет
//   СообщенияОбОшибках - Произвольный
//
&НаКлиенте
Процедура Проверить(ДокументыОснования, СообщенияОбОшибках = Неопределено) Экспорт
	...
	РеквизитыДокумента = Новый Структура;
	РеквизитыДокумента.Вставить("КодНалоговогоОргана", "");
	РеквизитыДокумента.Вставить("ПериодПо", '0001-01-01');
	РеквизитыДокумента.Вставить("Организация", ПредопределенноеЗначение("Справочник.Организации.ПустаяСсылка"));
...
Процедура ПроверитьОтчеты(СообщенияОбОшибках = Неопределено)
	...
	ОтчетыСФоновойПроверкойВыгрузки = Новый Массив;
	...
	//выгрузка в формате Минтруда
	Если ОтборМинтруда.Количество() <> 0 Тогда
	...
	Отбор500 = ВерсииИОтчеты.НайтиСтроки(Новый Структура("Версия", ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсия500));
	Для Каждого Стр Из Отбор500 Цикл
		МетодПроверкиВыгрузкиВФоне = МетодПроверкиВыгрузкиВФоне(Стр);
		Если МетодПроверкиВыгрузкиВФоне <> Неопределено Тогда
			ОтчетыСФоновойПроверкойВыгрузки.Добавить(МетодПроверкиВыгрузкиВФоне);
			Продолжить;
		КонецЕсли;
		...
		Если ВладелецФормы = Неопределено
			...
			ТекФорма = ПолучитьФорму(ПолныйПутьКФорме, ПараметрыФормы, ,
				?(ЗначениеЗаполнено(Стр.Док), Стр.Док, Новый УникальныйИдентификатор));
		Иначе
			ТекФорма = ВладелецФормы;
		КонецЕсли;
	...
	// Дополнительные параметры для проброса значений контекста родительской процедуры через фоновое задание.
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("СообщенияОбОшибках", СообщенияОбОшибках);
	ДополнительныеПараметры.Вставить("ПодавитьВыводСтаройФормыНавигацииПоОшибкам", ПодавитьВыводСтаройФормыНавигацииПоОшибкам);
	
	Если ОтчетыСФоновойПроверкойВыгрузки.Количество() <> 0 Тогда
		РезультатЗапускаФоновогоЗадания = РезультатЗапускаПроверкиВыгрузкиВФоне(ОтчетыСФоновойПроверкойВыгрузки,
			ДополнительныеПараметры);
		
		Если РезультатЗапускаФоновогоЗадания <> Неопределено
		   И РезультатЗапускаФоновогоЗадания.Статус = "Выполняется" Тогда
			ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбработатьЗавершениеПроверкиВыгрузкиВФоне", ЭтотОбъект);
			ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
			ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Проверяется возможность выгрузки регламентированного отчета';
													|en = 'Проверяется возможность выгрузки регламентированного отчета'");
			ДлительныеОперацииКлиент.ОжидатьЗавершение(РезультатЗапускаФоновогоЗадания, ОповещениеОЗавершении, ПараметрыОжидания);
			
		Иначе
			ОбработатьЗавершениеПроверкиВыгрузкиВФоне(РезультатЗапускаФоновогоЗадания, ДополнительныеПараметры);
			
		КонецЕсли;
		
	Иначе
		ЗавершитьПроверкуОтчетов(СообщенияОбОшибках, ПодавитьВыводСтаройФормыНавигацииПоОшибкам);
...
&НаКлиенте
Процедура ЗавершитьПроверкуОтчетов(СообщенияОбОшибках, ПодавитьВыводСтаройФормыНавигацииПоОшибкам)
...
Процедура ПроверитьОтчетыСервер(ОшибкаВыгрузки)
	...
	Если ЕстьНеопределенныеФорматы Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Для следующих отчетов выгрузка и проверка не предусмотрены:';
								|en = 'Для следующих отчетов выгрузка и проверка не предусмотрены:'");
		Сообщение.Сообщить();
	...
	Если ОтчетыСохраненныеСНевернымОкруглением.Количество() <> 0 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Сохраните отчеты с нужной единицей измерения и запустите проверку повторно.';
								|en = 'Сохраните отчеты с нужной единицей измерения и запустите проверку повторно.'");
		Сообщение.Сообщить();
...
// Возвращает описание метода проверки возможности выгрузки регламентированного отчета в фоне.
// 
// Параметры:
//  ОснованиеПроверки - ДанныеФормыЭлементКоллекции
// 
// Возвращаемое значение:
//   - Неопределено
//   - Структура - Описание метода проверки выгрузки:
//       * МетодПроверки - Строка
//       * РегламентированныйОтчет - ДокументСсылка.РегламентированныйОтчет
// 
&НаКлиенте
Функция МетодПроверкиВыгрузкиВФоне(ОснованиеПроверки)
	
	ИсточникОтчета = ОснованиеПроверки.ИсточникОтчета;
	ВыбраннаяФорма = ОснованиеПроверки.ВыбраннаяФорма;
	
	МетодПроверки = "";
	
	Если ИсточникОтчета = "РегламентированныйОтчетНеСуществует" Тогда
		Если ВыбраннаяФорма = "ФормаОтчетаНеСуществует" Тогда
			МетодПроверки = "Отчеты.РегламентированныйОтчетНеСуществует.ПроверитьВыгрузку";
		КонецЕсли;
	КонецЕсли;
	
	ОписаниеМетодаПроверки = Неопределено;
	Если ЗначениеЗаполнено(МетодПроверки) Тогда
		ОписаниеМетодаПроверки = Новый Структура;
		ОписаниеМетодаПроверки.Вставить("МетодПроверки", МетодПроверки);
		ОписаниеМетодаПроверки.Вставить("РегламентированныйОтчет", ОснованиеПроверки.Док);
	КонецЕсли;
	
	Возврат ОписаниеМетодаПроверки;
	
КонецФункции

&НаСервере
Функция РезультатЗапускаПроверкиВыгрузкиВФоне(ОтчетыСФоновойПроверкойВыгрузки, ДополнительныеПараметры)
	
	НаименованиеЗадания = НСтр("ru = 'Проверка возможности выгрузки регламентированных отчетов';
								|en = 'Проверка возможности выгрузки регламентированных отчетов'");
	...
	РезультатЗапуска = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполненияВФоне,
		"РегламентированнаяОтчетность.ПроверитьВозможностьВыгрузкиВФоне",
		ОтчетыСФоновойПроверкойВыгрузки, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ОбработатьЗавершениеПроверкиВыгрузкиВФоне(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		ВозвращенныеЗначения = ЗагрузитьРезультатыПроверкиВФоне(Результат);
		// Проброшенные переменные.
		СообщенияОбОшибках = ВозвращенныеЗначения.СообщенияОбОшибках;
		ПодавитьВыводСтаройФормыНавигацииПоОшибкам = ВозвращенныеЗначения.ПодавитьВыводСтаройФормыНавигацииПоОшибкам;
...
&НаСервере
Функция ЗагрузитьРезультатыПроверкиВФоне(Результат)
	
	ВозвращенныеЗначения = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
	
	// Результаты проверки.
	ОшибкаВыгрузки = ОшибкаВыгрузки ИЛИ ВозвращенныеЗначения.ОшибкаВыгрузки;
	ВсеОшибки = ВозвращенныеЗначения.ОписаниеОшибок;
	
	// Загрузка сообщений об ошибках из фоновых заданий в форму.
	Для Каждого СтрСообщения Из ВсеОшибки Цикл
		НовСтр = ТаблицаСообщений.Добавить();
		ЗаполняемыеСвойства = "ОтчетДок, Отчет, Раздел, Страница, Строка, СтрокаПП, ИмяЯчейки, Графа, Описание";
		ЗаполнитьЗначенияСвойств(НовСтр, СтрСообщения, ЗаполняемыеСвойства);
	КонецЦикла;
	
	// Удаление серверной части транспортного контейнера.
	ВозвращенныеЗначения.Удалить("ОшибкаВыгрузки");
	ВозвращенныеЗначения.Удалить("ОписаниеОшибок");
	
	Возврат ВозвращенныеЗначения;
...
Процедура ПоказатьТекстВыгрузкиДляВыбранногоЭлемента(Выгрузка)
	...
	ИначеЕсли Выгрузка.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияПФР
		ИЛИ Выгрузка.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияЕФСПФР
		ИЛИ Выгрузка.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияМинтруда Тогда
...
Функция ПолучитьСписокВыбора()
	...
	|	Документ.РегламентированныйОтчет КАК РегламентированныйОтчет
	|ГДЕ
	|	СОКРЛП(РегламентированныйОтчет.КодНалоговогоОргана) <> """" И
	|	РегламентированныйОтчет.ДатаОкончания >= &ДатаНачала И
	|	РегламентированныйОтчет.ДатаОкончания <= &ДатаОкончания");
	
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		Запрос.Текст = Запрос.Текст + " И РегламентированныйОтчет.Организация = &Организация";
		Запрос.УстановитьПараметр("Организация", Объект.Организация);
	КонецЕсли;
	Запрос.УстановитьПараметр("ДатаНачала", НачалоДня(Объект.ПериодПо));
	Запрос.УстановитьПараметр("ДатаОкончания", КонецДня(Объект.ПериодПо));
...
&НаКлиенте
Асинх Процедура ИзменениеПараметровОтбораАсинх(ВыполняемоеОповещение) 
	...
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'При изменении реквизита будут очищены список документов и тексты выгрузок.%1Продолжить?';
																					|en = 'При изменении реквизита будут очищены список документов и тексты выгрузок.%1Продолжить?'"), Символы.ПС);
		...
		Если ОтветНаВопрос = КодВозвратаДиалога.Да Тогда
			...
			Объект.Выгрузки.Очистить();
		КонецЕсли;
		ВыполнитьОбработкуОповещения(ВыполняемоеОповещение, ОтветНаВопрос = КодВозвратаДиалога.Да);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Асинх Процедура КодНалоговогоОрганаНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат Тогда
		Элемент = ДополнительныеПараметры.Элемент;
		СписокВыбора = ПолучитьСписокВыбора();
		Элемент.СписокВыбора.ЗагрузитьЗначения(СписокВыбора.ВыгрузитьЗначения());
		
		ВыбранныйЭлемент = Ждать ВыбратьИзСпискаАсинх(СписокВыбора, Элемент);
		Если ВыбранныйЭлемент <> Неопределено Тогда
...
Процедура ГодПриИзмененииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат Тогда
		Объект.ПериодПо = КонецМесяца(Дата(?(ЗначениеЗаполнено(Год), Год, 0001), Месяц(Объект.ПериодПо), 1));
	Иначе
		Год = Год(Объект.ПериодПо);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриИзмененииМесяца()
	Объект.ПериодПо = КонецМесяца(Дата(?(ЗначениеЗаполнено(Год), Год, 0001), ?(ЗначениеЗаполнено(Месяц), Месяц, 1), 1));
КонецПроцедуры
...
Процедура ПослеЗакрытияВопроса(Результат, Параметры) Экспорт
	...
	НачатьПолучениеФайлаССервера(Параметры.АдресФайлаВыгрузки, Параметры.ИмяКонечногоФайла);
КонецПроцедуры
