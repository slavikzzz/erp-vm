...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ВыкупПринятыхНаХранениеТоваров") Тогда
		ЗаполнитьНаОснованииВыкупаТоваровДавальца(ДанныеЗаполнения);
	//-- НЕ УТКА
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПолучитьФункциональнуюОпцию("ЗапретитьПоступлениеТоваровБезНомеровГТД")
		//++ НЕ УТ
		И Не ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыкупТоваровПереработчиком
		//-- НЕ УТ
		И Истина Тогда
...
#Область ЗаполнениеПоВыкупуТоваровДавальца

Процедура ЗаполнитьНаОснованииВыкупаТоваровДавальца(СтруктураЗаполнения)
	
	ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыкупТоваровПереработчиком;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Основание", СтруктураЗаполнения);
	Запрос.Текст = ТекстЗапросаЗаполнитьНаОснованииВыкупаТоваровДавальца();
	...
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(НоменклатураСервер.ПараметрыУказанияСерий(ЭтотОбъект, Документы.ВыкупТоваровХранителем));
	НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(ЭтотОбъект, ПараметрыУказанияСерий);
	
КонецПроцедуры

Функция ТекстЗапросаЗаполнитьНаОснованииВыкупаТоваровДавальца()
	...
	|			ТОГДА ВыкупПринятыхНаХранениеТоваров.НаправлениеДеятельности
	...
	|			ТОГДА ВыкупПринятыхНаХранениеТоваров.ВалютаВзаиморасчетов
	|		ИНАЧЕ СпрДоговоры.ВалютаВзаиморасчетов
	|	КОНЕЦ                                 КАК ВалютаВзаиморасчетов,
	|	ВыкупПринятыхНаХранениеТоваров.Сделка КАК Сделка,
	|	ВыкупПринятыхНаХранениеТоваров.НалогообложениеНДС КАК НалогообложениеНДС,
	|	ВыкупПринятыхНаХранениеТоваров.ЦенаВключаетНДС    КАК ЦенаВключаетНДС,
	|	ВыкупПринятыхНаХранениеТоваров.Ссылка КАК ДокументОснование
	|ИЗ
	...
	|	ПО Давалец.Ссылка = ВыкупПринятыхНаХранениеТоваров.Контрагент
	|	
	...
	|	ПО Переработчик.Ссылка = ВыкупПринятыхНаХранениеТоваров.Организация
	|	
	...
	|	ПО ДоговорДавальца.Ссылка = ВыкупПринятыхНаХранениеТоваров.Договор
	|
	...
	|
	|ГДЕ
	|	ВыкупПринятыхНаХранениеТоваров.Ссылка = &Основание
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|ИЗ
	|	Документ.ВыкупПринятыхНаХранениеТоваров.Товары КАК ТаблицаТоваров
	|ГДЕ
