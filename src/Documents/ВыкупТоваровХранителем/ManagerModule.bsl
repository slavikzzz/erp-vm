...
#Область СозданиеНаОснованииВыкупаТоваровДавальца

// Добавляет команду создания документа "Выкуп товаров у хранителя" на основании "Выкупа товаров давальца".
//
// Параметры:
//  КомандыСозданияНаОсновании - см. СозданиеНаОснованииПереопределяемый.ПередДобавлениемКомандСозданияНаОсновании.КомандыСозданияНаОсновании
//
...
Функция ДобавитьКомандуСоздатьНаОснованииВыкупаТоваровДавальца(КомандыСозданияНаОсновании) Экспорт
	
	Если ПравоДоступа("Добавление", Метаданные.Документы.ВыкупТоваровХранителем) Тогда
		
		КомандаСоздатьНаОсновании                     = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Менеджер            = Метаданные.Документы.ВыкупТоваровХранителем.ПолноеИмя();
		КомандаСоздатьНаОсновании.Представление       = НСтр("ru = 'Выкуп товаров переработчиком';
															|en = 'Redemption of goods by subcontractor'");
		КомандаСоздатьНаОсновании.РежимЗаписи         = "Проводить";
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	Если Данные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыкупТоваровПереработчиком Тогда
		ПредставлениеОбъекта = НСтр("ru = 'Выкуп товаров переработчиком';
									|en = 'Redemption of goods by subcontractor'");
	Иначе
	//-- НЕ УТ
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	//++ НЕ УТ
	|		КОГДА НЕ ТаблицаВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)
	|			И &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыкупТоваровПереработчиком)
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "НакладнаяВыкупПереработчиком";
	КомандаПечати.Представление = НСтр("ru = 'Выкуп товаров переработчиком';
										|en = 'Sales invoice — External subcontracting'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	УправлениеПечатью.ДобавитьУсловиеВидимостиКоманды(КомандаПечати,
													  "ХозяйственнаяОперация",
													  Перечисления.ХозяйственныеОперации.ВыкупТоваровПереработчиком,
													  ВидСравнения.Равно);
	//-- НЕ УТ
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Накладная") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru = 'Выкуп товаров';
				|en = 'Redemption of goods'"),
			СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати));

	//++ НЕ УТ
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "НакладнаяВыкупПереработчиком") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"НакладнаяВыкупПереработчиком",
			НСтр("ru = 'Выкуп товаров';
				|en = 'Redemption of goods'"),
			СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати));
	//-- НЕ УТ
...
Функция ОбработчикДействий(ХозяйственнаяОперация = Неопределено) Экспорт
	...
	//++ НЕ УТ
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыкупТоваровПереработчиком Тогда
		ОбработчикДействий = ВыкупТоваровПереработчиком;
	Иначе
	//-- НЕ УТ
...
Функция ЗаполнитьПредставлениеДокументаВЗапросе(ПолеХозяйственнаяОперация = "ХозяйственнаяОперация")
	...
	//++ НЕ УТ
	|ВЫБОР
	|	КОГДА &ПолеХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыкупТоваровПереработчиком)
	|		ТОГДА ""&ВыкупТоваровПереработчиком""
	|	ИНАЧЕ
	//-- НЕ УТ 
	...
	//++ НЕ УТ
	Обработчик =Документы.ВыкупТоваровХранителем.ОбработчикДействий(Перечисления.ХозяйственныеОперации.ВыкупТоваровПереработчиком);
	Представление = СтрЗаменить(Представление, "&ВыкупТоваровПереработчиком", Обработчик.ПредставлениеОбъекта());
	//-- НЕ УТ
