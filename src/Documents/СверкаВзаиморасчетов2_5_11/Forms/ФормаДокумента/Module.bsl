...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ Локализация
	ОтобразитьНаименованиеИсходящихДокументов();
	//-- Локализация
...
&НаКлиенте
Процедура НаименованиеИсходящихДокументовПриИзменении(Элемент)
	КлючевойРеквизитПриИзменении("НаименованиеИсходящихДокументов");
КонецПроцедуры
...
Процедура ИтоговыеЗаписиПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если СверкаПоДоговорам Тогда
		ШаблонТекстаСообщения = НСтр("ru = 'Запись с такими значениями ключевых полей ""Тип расчетов"", ""Партнер"" и ""Договор"" уже существует (строка № %1). Значения ключевых полей должны отличаться.';
									|en = 'A record with such values of the ""Payment type"", ""Partner"", and ""Contract"" key fields already exists (line %1). Key field values must differ.'");
	Иначе
		ШаблонТекстаСообщения = НСтр("ru = 'Запись с такими значениями ключевых полей ""Тип расчетов"", ""Партнер"", ""Договор"" и ""Объект расчетов"" уже существует (строка № %1). Значения ключевых полей должны отличаться.';
									|en = 'A record with such values of the ""Payment type"", ""Partner"", ""Contract"", and ""AR/AP object"" key fields already exists (line %1). Key field values must differ.'");
	КонецЕсли;
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		|Продолжить?';
		|en = 'Key field values were changed.
		|Subordinate detailed records (%1) will be deleted.
		|Continue?'"),
...
Процедура ДетальныеЗаписиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ОтборРасчеты = -1 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Добавление строк возможно только при установленном отборе по итоговой записи.';
								|en = 'You can only add rows if the filter by summary record is set.'");
...
Процедура ОбъединитьСтрокиИтоговыеЗаписи(Команда)
	...
	ТекстСообщенияОбОшибке = НСтр("ru = 'Для объединения необходимо выделить две несопоставленные строки: с данными только одной стороны и с данными только второй стороны.';
									|en = 'To merge lines, select two unmapped lines: a line with one party''s data and a line with the other party''s data.'");
...
Процедура РазъединитьСтрокуИтоговыеЗаписи(Команда)
	...
	Если ВыделенныеСтроки.Количество() <> 1
		...
		ТекстСообщенияОбОшибке = НСтр("ru = 'Для разделения необходимо выделить одну сопоставленную строку.';
										|en = 'To split lines, select one mapped line.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщенияОбОшибке,, "Объект.ИтоговыеЗаписи", "Объект");
		Возврат;
	КонецЕсли;
...
Процедура КлючевойРеквизитПриИзмененииСервер(ДополнительныеПараметры)
	...
	Если Не РезультатЗаписи Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось отменить проведение документа';
													|en = 'Cannot cancel document posting'"));
...
Процедура ПоказатьИтоговыеЗаписиПриИзмененииКлиент(ПересчетИтогов = Истина)
	...
	Иначе // Детальные записи
		...
		Если ИтоговаяЗапись = Неопределено Тогда
			ПоказатьИтоговыеЗаписи = Истина;
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Переход к детальным записям невозможен, т.к. не выбрана итоговая запись.';
															|en = 'You cannot navigate to detailed records as you did not select a summary record.'"), , "ПоказатьИтоговыеЗаписи");
			Возврат;
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Поле ""Контрагент"" не заполнено';
				|en = 'The ""Counterparty"" field is required'"),
			,
			"Контрагент",
			"Объект",
			Отказ);
...
Процедура УстановитьУсловноеОформление()
	...
	УстановитьПроверкуЗаполненияАналитики("ДетальныеЗаписи", "НомерДокумента");
	УстановитьПроверкуЗаполненияАналитики("ДетальныеЗаписи", "ДатаДокумента");
	УстановитьПроверкуЗаполненияАналитики("ДетальныеЗаписи", "НаименованиеДокумента");
...
&НаСервере
Процедура УстановитьПроверкуЗаполненияАналитики(ТипЗаписей, ИмяПоля, ПоДаннымКонтрагента = Ложь);
...
Процедура УстановитьВидимостьПартнера()
	...
	Элементы.Партнер.Видимость = Объект.РазбиватьПоПартнерам;
...
Функция ПараметрыФормыРасширенногоОтбора()
	...
	Если ОтборДоговорыБезОборотов = Неопределено Тогда
		...
		ОтборДоговорыБезОборотов.Представление = НСтр("ru = 'Включать договора без оборотов в периоде';
														|en = 'Include contracts without cash transactions during the given period'");
	КонецЕсли;
...
Процедура ИнициализироватьПредыдущиеЗначения()
	...
	ПредыдущиеЗначения.Вставить("НаименованиеИсходящихДокументов");
...
Процедура УстановитьСтатусСверки(ИмяСтатуса)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'У документа будет установлен статус ""%1"". Продолжить?';
			|en = 'The document status will be set to ""%1"". Continue?'"),
		Статус);
...
Процедура УстановитьПоясняющуюНадписьСтатуса()
	...
	Если Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыСверокВзаиморасчетов2_5_11.НаСверке") Тогда
		...
		|en = 'Reconcile and set the ""<a href = ""%1"">Reconciled without discrepancies</a>"", ""<a href = ""%2"">Reconciled with discrepancies</a>"", or ""<a href = ""%3"">Rejected</a>"" status'"),
	...
	ИначеЕсли Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыСверокВзаиморасчетов2_5_11.Создана") Тогда
		...
		|en = 'Send the reconciliation statement to the counterparty and set the ""<a href = ""%1"">On reconciliation</a>"" status'"),
		"НаСверке");
	КонецЕсли;
...
Процедура КлючевойРеквизитПриИзмененииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.НужноПерезаполнитьТЧ Тогда
		ТекстСообщения = НСтр("ru = 'Табличная часть ""Взаиморасчеты"" заполнена.';
								|en = 'The ""AR/AP accounting"" table is filled.'");
...
Функция ПараметрыПриИзмененииКлючевыхРеквизитов(ИмяРеквизита)
	...
	ЗначениеОбязательно = ЗначениеЗаполнено(ЗначениеРеквизита)
		И (ЗначениеЗаполнено(Объект.ИтоговыеЗаписи) Или ЗначениеЗаполнено(Объект.ДетальныеЗаписи))
		ИЛИ ИмяРеквизита = "НаименованиеИсходящихДокументов";

	НужноПерезаполнитьТЧ = ЗначениеОбязательно;
	НужноРаспровести = НужноПерезаполнитьТЧ И Объект.Проведен И ЗначениеЗаполнено(Объект.Ссылка);
	ТекстВопроса = "";
	Если НужноРаспровести И НужноПерезаполнитьТЧ Тогда
		ТекстВопроса = НСтр("ru = 'Заполнение табличной части ""Взаиморасчеты"" возможно только в непроведенном документе.
			...
			|en = 'You can fill the ""AR/AP accounting"" table only in unposted documents.
			|The table will be refilled. Do you want to refill it and unpost the document?'");
	ИначеЕсли НужноРаспровести И Не НужноПерезаполнитьТЧ Тогда
		ТекстВопроса = НСтр("ru = 'Заполнение табличной части ""Взаиморасчеты"" возможно только в непроведенном документе. Отменить проведение?';
							|en = 'You can fill the ""AR/AP accounting"" table only in unposted documents. Do you want to unpost the document?'");
	ИначеЕсли Не НужноРаспровести И НужноПерезаполнитьТЧ Тогда
		ТекстВопроса = НСтр("ru = 'Табличная часть ""Взаиморасчеты"" будет перезаполнена, продолжить?';
							|en = 'The ""AR/AP accounting"" table will be refilled. Continue?'");
	КонецЕсли;
...
//++ Локализация
&НаСервере
Процедура ОтобразитьНаименованиеИсходящихДокументов()
	
	Элементы.НаименованиеИсходящихДокументов.Видимость = Истина;
