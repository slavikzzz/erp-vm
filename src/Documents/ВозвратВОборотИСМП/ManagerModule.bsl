...
Функция СтатусПослеПодготовкиКПередачеДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийИСМП.ВозвратВОборотПриРозничнойРеализации
		...
		Или Операция = Перечисления.ВидыОперацийИСМП.ВозвратВОборотТовараВыведенногоИзОборотаВЦеляхНеСвязанныхСРеализацией
		Или Операция = Перечисления.ВидыОперацийИСМП.ВозвратТовараВОборотПоСделкеСГосТайной
...
Процедура ВозвращатьВОборотЛогистическуюУпаковкуЗапрещено(РезультатОбработки)
	...
	РезультатОбработки.ТекстОшибки = НСтр("ru = 'Невозможно вернуть в оборот логистическую упаковку';
											|en = 'Невозможно вернуть в оборот логистическую упаковку'");
КонецПроцедуры
