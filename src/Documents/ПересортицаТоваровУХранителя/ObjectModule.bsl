		   ...
		   И СтрокаТЧТовары.Назначение     = СтрокаТЧТовары.НазначениеОприходование Тогда
			...
			ТекстОшибки = СтрШаблон(
							НСтр("ru = 'Списываемый товар совпадает с приходуемым в строке %1 списка ""Товары""';
								|en = 'Written-off goods and received goods in the %1 line of the ""Goods"" list are the same'"),
							СтрокаТЧТовары.НомерСтроки);
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПересортицаТоваров") Тогда
		ЗаполнитьНаОснованииПересортицыТоваровДавальца(ДанныеЗаполнения);
	//-- НЕ УТКА
...
#Область ЗаполнениеПоПересортицеТоваровДавальца

Процедура ЗаполнитьНаОснованииПересортицыТоваровДавальца(СтруктураЗаполнения)
	
	ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПересортицаТоваровУПереработчика;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Основание", СтруктураЗаполнения);
	Запрос.Текст = ТекстЗапросаЗаполнитьНаОснованииПересортицыТоваровДавальца();
	...
	Если РеквизитыШапки.Следующий() Тогда
		...
		ШаблонОшибки = НСтр("ru = 'Ввод на основании пересортицы товаров невозможен. В видах запасов присутствуют разные ""%1"".';
							|en = 'Cannot generate documents based on the stock reclass. Inventory owner attributes contain different %1.'");
		
		Если РеквизитыШапки.ОшибкаКонтрагент Тогда
			ОшибкиЗаполнения.Добавить(НСтр("ru = 'Контрагенты';
											|en = 'Counterparties'"));
		КонецЕсли;
		
		Если РеквизитыШапки.ОшибкаДоговор Тогда
			ОшибкиЗаполнения.Добавить(НСтр("ru = 'Договоры';
											|en = 'Contracts'"));
		КонецЕсли;
	...
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(НоменклатураСервер.ПараметрыУказанияСерий(ЭтотОбъект, Документы.ПересортицаТоваровУХранителя));
	НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(ЭтотОбъект, ПараметрыУказанияСерий);
	
КонецПроцедуры

Функция ТекстЗапросаЗаполнитьНаОснованииПересортицыТоваровДавальца()
	
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТаблицаТоваров.ВидЗапасов.Контрагент                     КАК Давалец,
	|	ТаблицаТоваров.ВидЗапасов.Договор                        КАК ДоговорДавальца,
	...
	|	ТаблицаТоваров.АналитикаУчетаНоменклатурыОприходование.Номенклатура   КАК НоменклатураОприходование,
	|	ТаблицаТоваров.АналитикаУчетаНоменклатурыОприходование.Характеристика КАК ХарактеристикаОприходование,
	|	ТаблицаТоваров.АналитикаУчетаНоменклатурыОприходование.Серия          КАК СерияОприходование,
	|	ТаблицаТоваров.НомерГТД                КАК НомерГТД,
	|	СУММА(ТаблицаТоваров.Количество)       КАК Количество,
	|	СУММА(ТаблицаТоваров.КоличествоПоРНПТ) КАК КоличествоПоРНПТ
	|ПОМЕСТИТЬ ТоварыПересортицы
	|ИЗ
	|	Документ.ПересортицаТоваров.ВидыЗапасов КАК ТаблицаТоваров
	|ГДЕ
	|	ТаблицаТоваров.Ссылка = &Основание
	|	И ТаблицаТоваров.ВидЗапасов.ТипЗапасов В(
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТоварыПересортицы.Давалец     КАК Давалец,
	|	ТоварыПересортицы.Давалец.ИНН КАК ИНН,
	|	ТоварыПересортицы.Давалец.КПП КАК КПП
	|ПОМЕСТИТЬ Давальцы
	|ИЗ
	|	ТоварыПересортицы КАК ТоварыПересортицы
	|
	|ГДЕ
	|	НЕ ТоварыПересортицы.Давалец.ПометкаУдаления
	|
	|СГРУППИРОВАТЬ ПО
	|	ТоварыПересортицы.Давалец,
	|	ТоварыПересортицы.Давалец.ИНН,
	|	ТоварыПересортицы.Давалец.КПП
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТоварыПересортицы.ДоговорДавальца       КАК ДоговорДавальца,
	|	ТоварыПересортицы.ДоговорДавальца.Номер КАК Номер,
	|	ТоварыПересортицы.ДоговорДавальца.Дата  КАК Дата
	|ПОМЕСТИТЬ ДоговорыДавальца
	|ИЗ
	|	ТоварыПересортицы КАК ТоварыПересортицы
	|
	|ГДЕ
	|	НЕ ТоварыПересортицы.ДоговорДавальца.ПометкаУдаления
	|	И ТоварыПересортицы.ДоговорДавальца.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыДоговоровКонтрагентов.Действует)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТоварыПересортицы.ДоговорДавальца,
	|	ТоварыПересортицы.ДоговорДавальца.Номер,
	|	ТоварыПересортицы.ДоговорДавальца.Дата
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	МИНИМУМ(СпрОрганизации.Ссылка)     КАК Организация,
	|	МИНИМУМ(СпрКонтрагенты.Ссылка)     КАК Контрагент,
	|	МИНИМУМ(СпрКонтрагенты.Партнер)    КАК Партнер,
	|	МИНИМУМ(СпрДоговоры.Ссылка)        КАК Договор,
	|	МИНИМУМ(ВЫБОР
	|		КОГДА СпрДоговоры.НаправлениеДеятельности ЕСТЬ NULL
	|			ТОГДА ПересортицаТоваров.НаправлениеДеятельности
	|		ИНАЧЕ СпрДоговоры.НаправлениеДеятельности
	|	КОНЕЦ)                             КАК НаправлениеДеятельности,
	|	МИНИМУМ(ПересортицаТоваров.Валюта) КАК Валюта,
	|	МИНИМУМ(ПересортицаТоваров.ПриходоватьТоварыПоСебестоимостиСписания) КАК ПриходоватьТоварыПоСебестоимостиСписания,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Давальцы.Давалец) > 1                 КАК ОшибкаКонтрагент,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДоговорыДавальца.ДоговорДавальца) > 1 КАК ОшибкаДоговор,
	|	МИНИМУМ(ПересортицаТоваров.Ссылка) КАК Основание
	|ИЗ
	|	Документ.ПересортицаТоваров КАК ПересортицаТоваров
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Давальцы КАК Давальцы
	|	ПО ИСТИНА
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации КАК СпрОрганизации
	|	ПО СпрОрганизации.ИНН = Давальцы.ИНН
	|	И СпрОрганизации.КПП = Давальцы.КПП
	|	И НЕ СпрОрганизации.ПометкаУдаления
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации КАК Переработчик
	|	ПО Переработчик.Ссылка = ПересортицаТоваров.Организация
	|	
	...
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ ДоговорыДавальца КАК ДоговорыДавальца
	|	ПО ИСТИНА
	|
	...
	|	И СпрДоговоры.Номер = ДоговорыДавальца.Номер
	|	И СпрДоговоры.Дата = ДоговорыДавальца.Дата
	|	И СпрДоговоры.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыДоговоровКонтрагентов.Действует)
	|	И НЕ СпрДоговоры.ПометкаУдаления
	|
	|ГДЕ
	|	ПересортицаТоваров.Ссылка = &Основание
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	ТоварыПересортицы.НомерГТД                    КАК НомерГТД,
	|	ТоварыПересортицы.Количество                  КАК Количество,
	|	ТоварыПересортицы.КоличествоПоРНПТ            КАК КоличествоПоРНПТ
	|ИЗ
	|	ТоварыПересортицы КАК ТоварыПересортицы
	|";
			...
			Если Не ПриходоватьТоварыПоСебестоимостиСписания Тогда
				...
				Если Не ЗначениеЗаполнено(НоваяСтрока.СуммаЗабаланс)
					...
					И (ТекущийТипЗапасов = Перечисления.ТипыЗапасов.ТоварНаХраненииСПравомПродажи
						//++ НЕ УТКА
						Или ТекущийТипЗапасов = Перечисления.ТипыЗапасов.МатериалДавальца
						Или ТекущийТипЗапасов = Перечисления.ТипыЗапасов.ПродукцияДавальца
						//-- НЕ УТКА
						Или Ложь) Тогда
					...
					ТекстОшибки = НСтр("ru = 'В строке %1 для товара ""%2"" необходимо заполнить забалансовую цену.';
										|en = 'In line %1, fill the off-balance price for the ""%2"" item.'");
				...
				Если ЗначениеЗаполнено(НоваяСтрока.Сумма)
					...
					ТекстОшибки = НСтр("ru = 'В строке %1 для товара ""%2"" необходимо заполнить только забалансовую цену.';
										|en = 'In line %1, fill only the off-balance price for the ""%2"" item.'");
				...
				//++ НЕ УТКА
				Если ТекущийТипЗапасов = Перечисления.ТипыЗапасов.ПолуфабрикатДавальца
					И Не ЗначениеЗаполнено(НоваяСтрока.Сумма)
					И Не ЗначениеЗаполнено(НоваяСтрока.СуммаЗабаланс) Тогда
					
					Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", СтрокаТоваров.НомерСтроки, "НомерСтроки");
					ТекстОшибки = НСтр("ru = 'В строке %1 для товара ""%2"" необходимо заполнить балансовую или забалансовую цену.';
										|en = 'In line %1, fill the book value or off-balance price for the ""%2"" item.'");
				...
				Если Не ЗначениеЗаполнено(НоваяСтрока.Сумма)
					...
					//++ НЕ УТКА
					И Не ТекущийТипЗапасов = Перечисления.ТипыЗапасов.МатериалДавальца
					И Не ТекущийТипЗапасов = Перечисления.ТипыЗапасов.ПолуфабрикатДавальца
					И Не ТекущийТипЗапасов = Перечисления.ТипыЗапасов.ПродукцияДавальца
					//-- НЕ УТКА
					...
					ТекстОшибки = НСтр("ru = 'В строке %1 для товара ""%2"" необходимо заполнить балансовую цену.';
										|en = 'In line %1, fill the book value for the ""%2"" item.'");
				...
				Если ЗначениеЗаполнено(НоваяСтрока.СуммаЗабаланс)
					...
					ТекстОшибки = НСтр("ru = 'В строке %1 для товара ""%2"" необходимо заполнить только балансовую цену.';
										|en = 'In line %1, fill only the book value for the ""%2"" item.'");
...
Процедура ПроверитьЗаполнениеНазначений(Отказ)
	...
	|				ЕСТЬNULL(ВЫРАЗИТЬ(Товары.НазначениеОприходование КАК Справочник.Назначения).Партнер, ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка))
	|		ИЛИ НЕ ВЫРАЗИТЬ(Товары.Назначение КАК Справочник.Назначения).Договор =
	|				ЕСТЬNULL(ВЫРАЗИТЬ(Товары.НазначениеОприходование КАК Справочник.Назначения).Договор, ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)))
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки
	|";
	...
	Запрос.УстановитьПараметр(
		...
		НСтр("ru = 'В строке %1 списка ""Товары"" приходуемый товар должен обосабливаться под назначение давальца, в котором партнер/договор не отличается от исходного.';
			|en = 'In line %1 of the ""Goods"" list, the stock surplus must be assigned for the provider assignment, in which the partner/contract is different from the initial one.'"));
