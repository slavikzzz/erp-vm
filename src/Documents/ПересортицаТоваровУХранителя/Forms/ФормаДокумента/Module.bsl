...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыНастройки = Документы.ПересортицаТоваровУХранителя.ПараметрыНастройкиСчетовУчета();
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//++ НЕ УТКА
	НазначенияСервер.ЗаполнитьТипНазначенияВТаблице(Объект.Товары);
	//-- НЕ УТКА
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	//++ НЕ УТКА
	СтруктураДействий.Вставить("ЗаполнитьТипНазначения",                   ПараметрыЗаполненияПоТипуНазначения());
	//-- НЕ УТКА
...
Процедура ОткрытьПодбор(Команда)
	...
	//++ НЕ УТ
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПересортицаТоваровУПереработчика") Тогда
...
Процедура ПодобратьПереданныеТовары(Команда)
	...
	ПараметрыФормы.Вставить(
		"СвернутьНомераГТД",
		Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПересортицаТоваровУПереработчика"));
	//-- НЕ УТ
...
Процедура ПартнерПриИзмененииСервер()
	...
	Если НаправлениеДеятельностиСтарое <> Объект.НаправлениеДеятельности Тогда
		...
		НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(Объект, ПараметрыУказанияСерий);
		//++ НЕ УТКА
		НазначенияСервер.ЗаполнитьТипНазначенияВТаблице(Объект.Товары,, ПараметрыЗаполненияПоТипуНазначения());
		//-- НЕ УТКА
	КонецЕсли;
...
Процедура ОбработкаВыбораПодборПереданныхТоваровНаСервере(ВыбранноеЗначение)
	...
	НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(Объект, ПараметрыУказанияСерий);
	//++ НЕ УТКА
	НазначенияСервер.ЗаполнитьТипНазначенияВТаблице(Объект.Товары, НовыеСтроки, ПараметрыЗаполненияПоТипуНазначения());
	//-- НЕ УТКА
...
Функция ЗаполнитьЦеныПоСебестоимостиСервер()
	...
	Иначе 
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось заполнить цены по себестоимости в документе %1: на %2 не произведен расчет себестоимости.';
										|en = 'Cannot fill in prices by cost in document %1: cost is not calculated for %2.'"), Объект.Ссылка, Формат(НачалоМесяца(Объект.Дата),"ДЛФ=DD"));
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<приходуется по себестоимости>';
																|en = '<recorded as received at cost>'"));
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыНазначениеОприходование.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект.Товары.ТипНазначения");
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Перечисления.ТипыНазначений.Давальческое2_5;

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект.Товары.НазначениеОприходование");
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект.Товары.НоменклатураОприходование");
...
//++ НЕ УТКА
&НаКлиентеНаСервереБезКонтекста
Функция ПараметрыЗаполненияПоТипуНазначения()
	
	Параметры = НазначенияКлиентСервер.ПараметрыЗаполнения();
	Параметры.ИмяЗаполняемогоПоля = "НазначениеОприходование";
	Параметры.ИмяПоляТЧ           = "Назначение";
	Параметры.ТипНазначения       = ПредопределенноеЗначение("Перечисление.ТипыНазначений.Давальческое2_5");
