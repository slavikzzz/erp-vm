...
Процедура ХозяйственнаяОперацияПриИзменении(Элемент)
	...
	Операции.Добавить(Объект.ХозяйственнаяОперация);
	Операции.Добавить(ХозяйственнаяОперацияДоИзменения);
		
	Если Объект.ХозяйственнаяОперация <> ХозяйственнаяОперацияДоИзменения
		И (Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СписаниеТоваровДавальцаЗаСчетДавальца")
				И Не ХозяйственнаяОперацияДоИзменения = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СписаниеТоваровДавальцаНаРасходы")
			Или Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СписаниеТоваровДавальцаНаРасходы")
				И Не ХозяйственнаяОперацияДоИзменения = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СписаниеТоваровДавальцаЗаСчетДавальца")) Тогда
...
Процедура ДоговорПриИзмененииСервер()
	...
	//++ НЕ УТКА
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваровДавальцаЗаСчетДавальца
		Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваровДавальцаНаРасходы Тогда
		...
		НайденныеСтроки = Объект.Товары.НайтиСтроки(Новый Структура("Назначение", ПустоеНазначение));
		Если Объект.Товары.Количество() > НайденныеСтроки.Количество() Тогда
			
			Для каждого СтрокаТЧТовары Из Объект.Товары Цикл
				
				Если Не ЗначениеЗаполнено(СтрокаТЧТовары.Назначение) Тогда
					Продолжить;
				КонецЕсли;
				
				СтрокаТЧТовары.Назначение = ПустоеНазначение;
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	ПересчитыватьЦены =
		...
		//++ НЕ УТКА
		Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваровДавальцаНаРасходы
		//-- НЕ УТКА
		Или Ложь;
...
Функция ХозяйственнаяОперацияДоговора(ХозяйственнаяОперация)
	...
	Если ХозяйственнаяОперация =
			ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СписаниеТоваровДавальцаЗаСчетДавальца")
		Или ХозяйственнаяОперация =
			ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СписаниеТоваровДавальцаНаРасходы") Тогда
...
Процедура НастроитьФорму()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваровДавальцаЗаСчетДавальца
		...
		ПредставлениеОбъекта = ДавальческаяСхемаКлиентСервер.СинонимДокументаОтчетОСписанииТоваровСХранения();
		...
		Элементы.ТоварыПодобратьТоварыКСписаниюСХранения.Заголовок = НСтр("ru = 'Подобрать списанные товары давальца';
																			|en = 'Pick adjusted goods of the provider'");
	...
	Иначе
		...
		Элементы.ТоварыПодобратьТоварыКСписаниюСХранения.Заголовок = НСтр("ru = 'Подобрать товары к списанию, принятых на ответственное';
																			|en = 'Pick available VMI to adjust'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Списание товаров, хранящихся на складе';
																|en = 'Write off goods stored in the warehouse'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Списание товаров, переданных в производство';
																|en = 'Write off goods transferred to production'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Списание товаров, переданных на хранение';
																|en = 'Write-off goods transferred for storage'"));
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья2_5") Тогда
		ХозяйственнаяОперацияСписокВыбора.Добавить(Перечисления.ХозяйственныеОперации.СписаниеТоваровДавальцаЗаСчетДавальца);
		ХозяйственнаяОперацияСписокВыбора.Добавить(Перечисления.ХозяйственныеОперации.СписаниеТоваровДавальцаНаРасходы);
	КонецЕсли;
...
Процедура УстановитьВидимостьСоглашений()
	...
	Если ЗначениеЗаполнено(Объект.Партнер) Тогда
		...
		Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваровДавальцаЗаСчетДавальца
			Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваровДавальцаНаРасходы Тогда
			УстановитьЗначениеСвойства = Ложь;
		КонецЕсли;
		//-- НЕ УТКА
...
Процедура УстановитьСвязиПараметровВыбораДоговора()
	...
	ПараметрыВыбораДоговор.Добавить(
		Новый ПараметрВыбора(
			"Отбор.ПорядокОформленияСписанияНедостачТоваровПринятыхНаХранение",
			Перечисления.ПорядокОформленияСписанияТоваровПринятыхНаХранение.ОформлятьСписание));
			
	ПараметрыВыбораДоговор.Добавить(
		Новый ПараметрВыбора(
			"Отбор.ХозяйственнаяОперация", ХозяйственнаяОперацияДоговора(Объект.ХозяйственнаяОперация)));
	//-- НЕ УТКА
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваровДавальцаЗаСчетДавальца
		...
		ПараметрыВыбораДоговор.Добавить(
			Новый ПараметрВыбора("Отбор.ТипДоговора", Перечисления.ТипыДоговоров.СДавальцем2_5));
	
	Иначе
	
		ТипыДоговора = Новый Массив;
		ТипыДоговора.Добавить(Перечисления.ТипыДоговоров.СПоклажедателем);
		ТипыДоговора.Добавить(Перечисления.ТипыДоговоров.СКомитентомНаЗакупку);
		
		ПараметрыВыбораДоговор.Добавить(Новый ПараметрВыбора("Отбор.ТипДоговора", ТипыДоговора));
	//-- НЕ УТКА
...
Процедура НастроитьВидимостьЭлементовДоговора()
	...
	//++ НЕ УТКА
	Операции.Добавить(Перечисления.ХозяйственныеОперации.СписаниеТоваровДавальцаНаРасходы);
	//-- НЕ УТКА
...
Процедура ПодборТоваровКСписаниюСХранения()
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Поле ""Организация"" не заполнено';
																|en = 'The ""Company"" field is required'"), Объект.Ссылка,
			"Объект.Организация", "", Отказ);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.Договор) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Поле ""Договор"" не заполнено';
	...
	Если Не Отказ Тогда
		...
		Иначе
			...
			ТипыЗапасов.Добавить(ПредопределенноеЗначение("Перечисление.ТипыЗапасов.МатериалДавальца"));
			...
			ПараметрыФормы.Вставить("Заголовок",  НСтр("ru = 'Подбор товаров давальца';
														|en = 'Pick provider goods'"));
		КонецЕсли;
		//-- НЕ УТКА
