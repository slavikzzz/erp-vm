...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Справочники.КассыККМ.НастроенаКассаККМДляРМК() Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для текущего рабочего места не указана Касса ККМ. Пробитие чека недоступно.';
								|en = 'The cash register is not specified for the current workplace. Cannot issue a sales slip.'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(Объект.ВидЦены)
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Вид цены, используемый для продажи в розницу, должен включать в цене НДС.';
														|en = 'The price type used for retail sales must include VAT in price.'"));
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если НоменклатураКлиент.ЭтоУказаниеСерий(ИсточникВыбора) Тогда
		...
		ТекущаяСтрока = Объект.Товары.НайтиПоИдентификатору(ВыбранноеЗначение.ИдентификаторТекущейСтроки);
		ОбработатьСтрокуПриИзмененииСерии(ТекущаяСтрока);
	...
	Если Источник = "ПодключаемоеОборудование"
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект,"КодМаркировкиДляУточнения") 
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Есть код маркировки для уточнения. Автоматическое добавление новых позиций невозможно.';
															|en = 'There is a marking code for specification. You cannot automatically add new items.'"));
		...
		Если (НЕ ПараметрыКассыККМ.ИспользоватьБезПодключенияОборудования И Не ЗначениеЗаполнено(ПараметрыКассыККМ.ИдентификаторУстройства))
			...
			ПоказатьПредупреждение(
				...
				НСтр("ru = 'Изменено рабочее место подключаемого оборудования. Для данного рабочего места не настроено оборудование.';
					|en = 'Peripherals workplace has been changed. Equipment is not configured for this workplace.'"));
			Возврат;
...
Процедура ТоварыНоменклатураПриИзмененииСервер(ИдентификаторТекущейСтроки)
	...
	Если КонтролироватьАссортимент Тогда
		...
		СтруктураПроверкиАссортимента.ТекстСообщения = НСтр("ru = 'Товар %1 не включен в ассортимент магазина или запрещен к продаже.';
															|en = 'Product %1 is not included in the store''s product range or prohibited for sale.'");
...
Процедура ТоварыСерияПриИзменении(Элемент)
	...
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	ОбработатьСтрокуПриИзмененииСерии(ТекущиеДанные);
	...
	Если МенеджерОборудованияКлиент.ПодключаемоеОборудование().Свойство("ПоследнийСлипЧек", СтрокаСлипЧека)
		...
		ТекстСообщения = 
			НСтр("ru = 'Слип-чек отсутствует.
			         ...
			         |en = 'Sales slip is missing.
			         |Probably, the acquiring transaction has not been performed for this session yet.'");
...
Процедура ВыполнитьРасчет(ДополнительныеПараметры)
	...
	Если ПараметрыПримененияСкидок <> Неопределено
		...
		ТесктВопроса = НСтр("ru = 'Предварительно необходимо рассчитать скидки (наценки). Выполнить расчет?';
							|en = 'It is required to calculate discounts (markups) beforehand. Calculate?'");
	...
	Если (ИспользоватьАвтоматическиеСкидкиВПродажах
		...
		Если Не Результат.РасчетВыполненУспешно Тогда
			...
			ДополнительныеПараметрыПопыткаЗаписи.ТекстСообщения             = НСтр("ru = 'При выполнении операции резервирования не удалось записать документ.';
																					|en = 'Cannot save the document during the reservation.'");
...
Процедура ПробитьЧек()
	...
	ВыполнитьДействиеПослеЗаписи(
		...
		НСтр("ru = 'Перед выполнением операции пробития чека не удалось записать документ.';
			|en = 'Cannot save the document before issuing the receipt.'"));
...
&НаКлиенте
Процедура ОбработатьСтрокуПриИзмененииСерии(ТекущаяСтрока)
...
Процедура ВывестиИнформациюНаДисплейПокупателя()
	...
	Если ВывестиКОплатеНаДисплейПокупателя Тогда
		...
		ДПТекст1 = РозничныеПродажиКлиент.ПодготовитьСтрокуКВыводуНаДисплейПокупателя(
			НСтр("ru = 'К оплате:';
				|en = 'Amount due:'"), ИнформацияОбОплате.СуммаКОплате);
		ДПТекст2 = "";
...
Процедура ЗарезервироватьНаКлиенте(РезультатОтложенДо, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли РезультатОтложенДо < Объект.Дата Тогда
		ТекстПредупреждения = НСтр("ru = 'Дата резерва должна быть не меньше даты чека %ДатаЧека%';
									|en = 'Reserve date should be at least the date of receipt %ДатаЧека%'");
			...
			Для Каждого СтрокаТЧ Из Объект.Товары Цикл
				Если СтрокаТЧ.Сумма = 0 И СтрокаТЧ.Цена * СтрокаТЧ.КоличествоУпаковок > 0  Тогда
					ОбщегоНазначения.СообщитьПользователю(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не корректно настроены скидки (наценки). После расчета скидок (наценок) сумма в строке %1 равна нулю.';
																					|en = 'Discounts (markups) are set up incorrectly. When discounts (markups) are calculated, the amount in line %1 is zero.'"), СтрокаТЧ.НомерСтроки), Объект.Ссылка, "Объект.Товары");
					Результат = Ложь;
				КонецЕсли;
			КонецЦикла;
...
Процедура ПересчитатьДокументНаКлиенте()
	...
	Элементы.ПоискПоШтрихкоду.Доступность = Истина;
	Элементы.СчитатьКартуЛояльности.Доступность = Истина;
	Элементы.ОткрытьПодбор.Доступность = Истина;
	Элементы.ОтменитьОплатыПлатежнымиКартами.Доступность = Истина;
	Элементы.ФормаБыстрыеТовары.Доступность = Истина;
	Элементы.БыстрыеТовары.Доступность = Истина;
	Элементы.РассчитатьСкидки.Доступность = Истина;
	
	Элементы.ГруппаНижняяКоманднаяПанельКонтекстныеКоманды.Доступность = Истина;

	Элементы.КонтекстноеМенюОбластьКорзиныТоварыДобавить.Доступность    = Истина;
	Элементы.КонтекстноеМенюОбластьКорзиныТоварыСкопировать.Доступность = Истина;
	...
	ИнформацияОбОплате = ИнформацияОбОплате(ЭтотОбъект);
	УправлениеДоступностьюКнопокОплаты(ЭтотОбъект, ИнформацияОбОплате);
	
	ПересчитатьДокументНаКлиентеЛокализация(ИнформацияОбОплате);
	ОбновитьДанныеИнформационнойПанели(ЭтотОбъект, Истина = ТолькоПросмотр);
...
Процедура ДобавитьОплатуКартой(ИзмененныеДанныеЗаписаны, ПараметрыЗавершения) Экспорт
	...
	ВыполнитьДействиеПослеЗаписи(
		...
		НСтр("ru = 'Перед оплатой платежной картой не удалось записать документ.';
			|en = 'Cannot save the document before paying by payment card.'"));
...
Процедура ДобавитьОплатуКартойЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ИзмененныеДанныеЗаписаны Тогда
		...
		ДополнительныеПараметрыЗавершение.ТекстСообщения                 = НСтр("ru = 'После проведения оплаты платежной картой не удалось сохранить документ.';
																				|en = 'Cannot save the document after payment by payment card.'");
...
Процедура ДобавитьОплатуКартойЗаписьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.ВыполненаОперацияНаУстройстве И Не Результат.ИзмененныеДанныеЗаписаны Тогда
		...
		ПараметрыФормы.Вставить("ТекстСообщения",
			...
			|The payment is made but it was not recorded in the application.'"));
...
Функция ЗаписатьНаСервере(ТребуетсяПовторнаяПопыткаЗаписи = Неопределено)
	...
	Исключение
		...
		Шаблон = НСтр("ru = 'Не удалось записать чек ККМ по причине: %1';
						|en = 'Cannot save the receipt. Reason: %1'");
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'РМК';
				|en = 'Point of sale workplace'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОбновитьЗаголовокФормы()
	...
	Заголовок = НСтр("ru = 'Продажа (Кассир: %Кассир%, Продавец: %Продавец%)';
					|en = 'Sale (Cashier: %Кассир%, Sales associate: %Продавец%)'");
...
Процедура ПередЗакрытиемЧека(Отказ, ОписаниеОповещения = Неопределено)
	...
	Если Не ТолькоПросмотр И (Модифицированность ИЛИ ЗначениеЗаполнено(Объект.Ссылка)) Тогда
		...
		ТекстВопроса = НСтр("ru = 'Данные чека ККМ были изменены.';
							|en = 'Cash register receipt data has changed.'");
		...
		Иначе
			Если ПраваДоступа.Отложить Тогда
				Кнопки.Добавить(1, НСтр("ru = 'Отложить без резервирования';
										|en = 'Put on hold without reservation'"));
			КонецЕсли;
	...
	ПередЗакрытиемЧекаЛокализация(Отказ, Кнопки);
	...
	Если Не РезультатВыполнения.Результат Тогда
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'При печати слип-чека возникла ошибка:
			           |""%1"".';
			           |en = 'An error occurred when printing a sales slip:
			           |""%1"".'"),
...
Процедура НастроитьРМКЛокализация(ДоступныеВидыОплаты, ЭлементыНаПерегруппировку)
	...
	Если СуммаСБПВыполнена > 0 Тогда
		Элементы.СуммаСБП.Заголовок = НСтр("ru = 'СБП оплачено';
											|en = 'Faster Payments System paid'");
	...
	ИначеЕсли СуммаСБПОжидание > 0 Тогда
		Элементы.СуммаСБП.Заголовок = НСтр("ru = 'СБП ожидается';
											|en = 'Faster Payments System amount due'");
...
Процедура УправлениеДоступностьюКнопокОплатыЛокализация(Форма, ИнформацияОбОплате)
	...
	ОплатаЭСФССНачатаЧекНеПробит = (Форма.Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.ТоварЗарезервирован") 
										И ИнформацияОбОплате.ЭСФСС <> 0);

	Элементы = Форма.Элементы;
	Элементы.ОплатитьСБП.Доступность = Не (ОплатаКартойНачатаЧекНеПробит Или ОплатаЭСФССНачатаЧекНеПробит);
	...
	Если ИнформацияОбОплате.СБП > 0 ИЛИ ИнформацияОбОплате.СБПОжидание > 0 ИЛИ ОплатаЭСФССНачатаЧекНеПробит Тогда
		...
		Если ОплатаЭСФССНачатаЧекНеПробит Тогда
			Элементы.ОплатитьЭСФСС.Доступность = Истина;
			Элементы.СмешаннаяОплата.Доступность = Ложь;
		КонецЕсли;	
	КонецЕсли;
	//-- Локализация
...
&НаКлиенте
Процедура ПересчитатьДокументНаКлиентеЛокализация(ИнформацияОбОплате)
	//++ Локализация
	
	ОплатаЭСФССНачатаЧекНеПробит = (Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.ТоварЗарезервирован") 
										И ИнформацияОбОплате.ЭСФСС <> 0);
										
	Если ОплатаЭСФССНачатаЧекНеПробит Тогда		

		Элементы.ПоискПоШтрихкоду.Доступность = Ложь;
		Элементы.СчитатьКартуЛояльности.Доступность = Ложь;
		Элементы.ОткрытьПодбор.Доступность = Ложь;
		Элементы.ОтменитьОплатыПлатежнымиКартами.Доступность = Ложь;
		Элементы.ФормаБыстрыеТовары.Доступность = Ложь;
		Элементы.БыстрыеТовары.Доступность = Ложь;
		Элементы.РассчитатьСкидки.Доступность = Ложь;
		
		Элементы.ГруппаНижняяКоманднаяПанельКонтекстныеКоманды.Доступность = Ложь;
		
		Элементы.Товары.ИзменятьСоставСтрок    = Ложь;
		
		Элементы.КонтекстноеМенюОбластьКорзиныТоварыДобавить.Доступность    = Ложь;
		Элементы.КонтекстноеМенюОбластьКорзиныТоварыСкопировать.Доступность = Ложь;
		...
		Элементы.ТоварыШтрихкод.ТолькоПросмотр           = Истина;
		Элементы.ТоварыХарактеристика.ТолькоПросмотр     = Истина;
		Элементы.ТоварыПродавец.ТолькоПросмотр           = Истина;
	КонецЕсли;
...
Процедура ПриИзмененииНоменклатурыХарактеристикиКоличестваЛокализация()
	...
	ВедетсяУчетЕГАИС = ОбщегоНазначенияЕГАИСКлиентСерверПовтИсп.ИспользуетсяМаркируемаяПродукция();
	ВедетсяУчетИСМП  = ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции();
	Если Не (ВедетсяУчетЕГАИС Или ВедетсяУчетИСМП) Тогда
		Возврат;
	ИначеЕсли ЭтотОбъект.ПараметрыИнтеграцииГосИС["ВедетсяУчетВидовПродукцииСЧастичнымВыбытиемИС"] <> Истина Тогда
		Возврат;
	КонецЕсли;
...
Процедура ТоварыПриОкончанииРедактированияЛокализация(
	...
	Если ТекущиеДанные.ЕстьВскрытыеПотребительскиеУпаковкиИС Тогда
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ВскрытиеУпаковкиПоНоменклатуре.Количество() > 1 Тогда
			ТекстСообщения = НСтр("ru = 'Вскрыто несколько кег. Для увеличения количества продажи используйте функционал
						...
						|en = 'Multiple kegs opened. To specify more sales, use
						|the quick goods feature (""Quick goods"" button) or the pick form (""Pick"" button).
						|To specify less sales, use the pick and check form (""Excise stamps"" button).'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
...
Функция ДополнительноеОписаниеТовараДляКеги(НомерКрана, КодМаркировки, УпаковкаЧастичногоВыбытия, Остаток)
	...
	Текст = СтрШаблон(НСтр("ru = 'Кег %1: остаток %2 %3;';
							|en = 'Kegs %1: balance %2 %3;'"), ДанныеПоКеге, Окр(Остаток, 1), УпаковкаЧастичногоВыбытия);
...
Функция ДополнительноеОписаниеТовараДляИныхВидовПродукцииИС(
	...
	Текст = СтрШаблон(НСтр("ru = '%1: остаток %2 %3;';
							|en = '%1: balance %2 %3;'"), КодМаркировки, Окр(Остаток, 0), УпаковкаЧастичногоВыбытия);
	Если ВскрытаОднаУпаковка Тогда
		Текст = СтрШаблон(НСтр("ru = 'остаток %1 %2;';
								|en = 'balance %1 %2;'"), Окр(Остаток, 0), УпаковкаЧастичногоВыбытия);
	КонецЕсли;
...
Процедура ОткрытьФормуОплатыЭСФСС(ИзмененныеДанныеЗаписаны, ДоступныеВидыОплаты) Экспорт
	...
	ДанныеОплатыЭСФСС = ПолучитьДанныеОплатыЭСФСС(Объект.ОплатаПлатежнымиКартами);
	Если Не ДанныеОплатыЭСФСС = Неопределено Тогда
		ПараметрыОткрытияФормы.Вставить("ИдентификаторКорзины", ДанныеОплатыЭСФСС.ИдентификаторКорзины);
		ПараметрыОткрытияФормы.Вставить("СсылочныйНомер", ДанныеОплатыЭСФСС.СсылочныйНомер);
	КонецЕсли;
...
Процедура ОбработатьРезультатОплатыЭСФСС(Знач РезультатВыполнения = Неопределено, Знач ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РезультатВыполнения.Результат
		...
		И ЗначениеЗаполнено(ПолучитьИдентификаторКорзиныОплатыЭСФСС(Объект.ОплатаПлатежнымиКартами)) Тогда
...
Процедура ОтправитьПодтверждениеНСПКНаКлиенте(ТоварыФССКОплате)
	...
	Если ПараметрыОперации = Неопределено Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Отсутствуют данные о фискализации чека: %1';
										|en = 'Receipt fiscalization data is missing: %1'"), СокрЛП(Объект.Ссылка));
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
	Иначе
...
Процедура ПередатьДанныеФискальногоЧекаНСПКЗавершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт 
	...
	Если Не РезультатПередачи.Результат Тогда
		ТекстСообщения = СтрШаблон(
			...
			|%3';
			|en = 'An error occurred when sending the %1 transaction confirmation to the server (code %2):
...
Процедура УстановитьДоступностьКнопкиОплатитьЭСФСС(Значение)
	...
	Элементы.ОплатитьЭСФСС.Доступность = Ложь;
	Если Значение И СуммаСБПВыполнена = 0 И СуммаСБПОжидание = 0 Тогда
		
		Если Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.ТоварЗарезервирован") И ЭтоОплатаЭСФСС Тогда

			Элементы.ОплатитьЭСФСС.Доступность = (Не ПолучитьИдентификаторКорзиныОплатыЭСФСС(Объект.ОплатаПлатежнымиКартами) = Неопределено);

		ИначеЕсли (Объект.Статус <> ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.ТоварЗарезервирован")
												Или СуммаИтогоОплачено = 0) Тогда
			Элементы.ОплатитьЭСФСС.Доступность = Истина;
		КонецЕсли;
...
// Возвращает только те номенклатурные позиции из переданного массива, где заполнен реквизит код ТРУ. 
// Параметры:
//  ТоварыДляЧтения - Массив из СправочникСсылка.Номенклатура -
//  
// Возвращаемое значение:
	...
	Если РезультатВыполнения.ЧекОтложен Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Чек отложен. Необходимо дождаться получение статуса оплаты СБП.';
				|en = 'The receipt is put on hold. Wait for the Faster Payments System payment status.'"));
	КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'ИдентификаторыОперацииОплаты';
				|en = 'ИдентификаторыОперацииОплаты'",
...
&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьДанныеОплатыЭСФСС(ТаблицаОплатПлатежнымиКартами)
	
	Результат = Неопределено;
	ОплатыЭСФСС = ТаблицаОплатПлатежнымиКартами.НайтиСтроки(Новый Структура("ВидОплаты", ПредопределенноеЗначение("Перечисление.ТипыПлатежнойСистемыККТ.СертификатНСПК")));
	Если ОплатыЭСФСС.Количество() Тогда
		Результат = Новый Структура;
		Результат.Вставить("ИдентификаторКорзины", ?(ПустаяСтрока(ОплатыЭСФСС[0].ИдентификаторКорзины), Неопределено, ОплатыЭСФСС[0].ИдентификаторКорзины));
		Результат.Вставить("СсылочныйНомер", ?(ПустаяСтрока(ОплатыЭСФСС[0].СсылочныйНомер), Неопределено, ОплатыЭСФСС[0].СсылочныйНомер));
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьИдентификаторКорзиныОплатыЭСФСС(ТаблицаОплатПлатежнымиКартами)
	
	Результат = Неопределено;
	ДанныеОплатыЭСФСС = ПолучитьДанныеОплатыЭСФСС(ТаблицаОплатПлатежнымиКартами);
	Если Не ДанныеОплатыЭСФСС = Неопределено Тогда
		Результат = ДанныеОплатыЭСФСС.ИдентификаторКорзины;
	КонецЕсли;
	Возврат Результат;
...
&НаКлиенте
Процедура ПередЗакрытиемЧекаЛокализация(Отказ, Кнопки)
