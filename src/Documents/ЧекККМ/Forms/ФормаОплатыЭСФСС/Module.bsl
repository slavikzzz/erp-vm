...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоОплатаЭСФСС Тогда
		// Оплата электронным сертификатом прошла, необходимо только пробить чек и отправить подтверждение в НСПК
		Если ЗначениеЗаполнено(ИдентификаторКорзины) И Не ЭтоВозврат Тогда
			ПредварительноеОдобрениеПолучено = Истина;
 			ОплатаЭСФССПрошлаЧекНеПробит = Истина;
		КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ПринудительноЗакрытьФорму ИЛИ ЗавершениеРаботы ИЛИ ОплатаЭСФССПрошлаЧекНеПробит Тогда
		Возврат;
	КонецЕсли;
	...
	Если ПредварительноеОдобрениеПолучено Тогда
		...
		Если ЭтоВозврат Тогда
			...
			ТекстВопроса = НСтр("ru = 'Перед закрытием формы возврата оплаты требуется отменить проверку.';
								|en = 'Cancel the check before closing the refund form.'");
			ПоказатьВопрос(
		...
		Иначе
			...
			ТекстВопроса = НСтр("ru = 'Перед закрытием формы оплаты требуется отменить проверку.';
								|en = 'Cancel the check before closing the payment form.'");
			ПоказатьВопрос(
				Новый ОписаниеОповещения("ПередЗакрытиемВопросЗавершение", ЭтотОбъект),
				ТекстВопроса, Кнопки);
...
Процедура КомандаПробитьЧек(Команда)
	...
	Если ЭтоОплатаЭСФСС Тогда
		...
		Если ПредварительноеОдобрениеПолучено Тогда
			...
			ИначеЕсли ОплатаЭСФССПрошлаЧекНеПробит Тогда
				СледующаяОперация = "ПробитьЧек";
			КонецЕсли;
		Иначе
...
Процедура ПолучениеСоставаКорзиныНСПКЗавершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт 
	...
	Иначе
		ОписаниеОшибки =
			СтрШаблон(НСтр("ru = 'Ошибка: (%1) %2';
							|en = 'Error: (%1) %2'"), РезультатВыполнения.КодРезультата, РезультатВыполнения.ОписаниеОшибки); 
	...
	Если Не ЗначениеЗаполнено(АдресМагазина) Тогда
			ЧастиФорматированнойСтроки.Добавить(
				СтроковыеФункции.ФорматированнаяСтрока(НСтр("ru = 'Необходимо указать адрес <a href=""%1"">склада</a>.';
															|en = 'Specify <a href=""%1""warehouse</a> address.'"),
		...
		ЧастиФорматированнойСтроки.Добавить(
			СтроковыеФункции.ФорматированнаяСтрока(
				НСтр("ru = 'В доступные виды оплаты эквайрингового терминала необходимо добавить вид оплаты Оплата ЭС (ФЗ-491)';
					|en = 'Add the ""Digital certificate payment (FZ-491)"" payment method to available payment methods of the POS terminal'")));
			ЧастиФорматированнойСтроки.Добавить(Символы.ПС);
	КонецЕсли;
	...
	Если Форма.ПредварительноеОдобрениеПолучено Тогда
		Если Форма.ЭтоВозврат Тогда
			...
			ТекстПодсказки = НСтр("ru = 'Возврат электронного сертификата должен производиться только на карту, к которой приписываются электронные сертификаты покупателя.';
									|en = 'Return the digital certificate only to the card, to which digital customer certificates are assigned.'");
		ИначеЕсли Форма.ОплатаЭСФССПрошлаЧекНеПробит Тогда
			ЗаголовокОплаты = НСтр("ru = 'Пробить чек';
									|en = 'Пробить чек'");
			ТекстПодсказки = НСтр("ru = 'Оплата электронным сертификатом уже прошла, необходимо пробить чек.';
									|en = 'Оплата электронным сертификатом уже прошла, необходимо пробить чек.'");
		Иначе
			...
			ТекстПодсказки = НСтр("ru = 'Доплата к электронному сертификату может производиться собственными средствами с той же карты,""
				|""а также наличными.';
				|en = 'You can pay extra to the digital certificate by your own card ""
				|""and in cash.'");
		КонецЕсли;
	Иначе
		...
		Если Форма.ЭтоВозврат Тогда
			ТекстПодсказки = НСтр("ru = 'Перед возвратом необходимо проверить доступную сумму к возврату ЭС (ФЗ-491).';
									|en = 'Before the return, check an available amount for the digital certificate return (FZ-491).'");
		Иначе
			ТекстПодсказки = НСтр("ru = 'Перед оплатой необходимо проверить доступную сумму к оплате ЭС (ФЗ-491).';
									|en = 'Before the payment, check the available amount for the digital certificate payment (FZ-491).'");
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьДоступностьИзмененияОплатыНаличными(Форма)
	ПолеПолученоНаличнымиДоступно = Форма.ПредварительноеОдобрениеПолучено И Форма.КОплате > Форма.ОплаченоЭСФСС И Не Форма.ОплатаЭСФССПрошлаЧекНеПробит;
...
Процедура РассчитатьИтоги(Форма, ИнформацияОбОплате = Неопределено)
	...
	ИначеЕсли Форма.ЭтоОплатаЭСФСС
		...
		Если Форма.ОплатаЭСФССПрошлаЧекНеПробит Тогда
			Форма.Элементы.ОплатаПодарочныйСертификат.Доступность = Ложь;
			Форма.Элементы.ОплатаБонусныеБаллы.Доступность = Ложь;
		КонецЕсли;
	...
	Если ЭтоОплатаЭСФСС Тогда
		...
		Если ЭтоВозврат Тогда
			Заголовок = НСтр("ru = 'Возврат оплаты ЭС (ФЗ-491)';
							|en = 'Return the digital certificate payment (FZ-491)'");
		Иначе
			Заголовок = НСтр("ru = 'Оплата ЭС (ФЗ-491)';
							|en = 'Digital certificate payment (FZ-491)'");
		КонецЕсли;
