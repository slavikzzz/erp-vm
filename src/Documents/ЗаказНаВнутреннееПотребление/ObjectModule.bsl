...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСогласованиеЗаказовНаВнутреннееПотребление")
		...
		ТекстОшибки = НСтр("ru = 'Нет прав на проведение Заказа на внутреннее потребление без согласования';
							|en = 'You do not have the rights to post Inventory consumption order without approval'");
	...
	//++ НЕ УТ
	ТМЦВЭксплуатацииСервер.ПроверитьЗаказНаПередачуВЭксплуатацию(ЭтотОбъект, МассивНепроверяемыхРеквизитов, Отказ);
	//-- НЕ УТ
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаВПроизводство И НЕ ПометкаУдаления Тогда
		ТекстСообщения = НСтр("ru = 'Запись документа запрещена, т.к. операция ""Передача в производство"" больше не используется.
								...
								|Material order is registered in form ""Production — Receipt and return of materials"".'");
	...
	//++ НЕ УТ
	ТМЦВЭксплуатацииСервер.ПередЗаписьюЗаказаНаПередачуВЭксплуатацию(ЭтотОбъект, Отказ);
	//-- НЕ УТ
