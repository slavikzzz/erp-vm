...
Процедура ПередВыполнениемДействийВСпискеЗаказов(Заказы, КодДействия, СообщенияПользователю) Экспорт
	...
	Текст = НСтр("ru = 'Невозможно выполнить действие отгрузить для %1: заказ находится в статусе ""На согласованиии"".';
				|en = 'Cannot ship %1: the order has the ""Pending approval"" status.'");
...
//
Процедура РассчитатьСостояние(Знач ЗаказНаВнутреннееПотребление, СостояниеРасчетов) Экспорт
	...
	Если ЗначениеЗаполнено(ЗаказНаВнутреннееПотребление) И ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыВнутреннихПотреблений") Тогда
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Если ЗначениеЗаполнено(Реквизиты.Сделка) Тогда
		ШаблонСтроки = "ru = 'По сделке ""%1""';
						|en = 'By the ""%1"" sales opportunity'"; // @НСтр
