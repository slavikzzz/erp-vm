	...
	Если ВыделенныеСтрокиПродукции.Количество() = 0 Тогда 
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Нет данных для заполнения';
				|en = 'Нет данных для заполнения'"),,
			НСтр("ru = 'Нет выделенных строк в табличной части ""Товары""';
				|en = 'Нет выделенных строк в табличной части ""Товары""'"));
		Возврат;
	КонецЕсли;
		...
		ИначеЕсли НизкокачественнаяПродукция <> Объект.Товары.НайтиПоИдентификатору(Стр).НизкокачественнаяПродукция Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ru = 'Заполнение невозможно';
					|en = 'Заполнение невозможно'"),,
				НСтр("ru = 'Нельзя заполнить цели для товаров с разным качеством';
					|en = 'Нельзя заполнить цели для товаров с разным качеством'"));
...
Функция ОписаниеОтбораЦелейВЕТИС(ГруппаСтрок = Ложь)
	...
	Возврат СтрШаблон(
		НСтр("ru = 'Наложен отбор по продукции: %1';
			|en = 'Наложен отбор по продукции: %1'"),
		Продукция);
			...
			Если ПустаяСтрока(ПредставлениеПериода) Тогда
				Строка["Надпись"+ВидПериода] =
					Новый ФорматированнаяСтрока(НСтр("ru = 'указать';
													|en = 'указать'"),,Форма.ЦветГиперссылкиОсобыйТекст,,ВидПериода);
			Иначе
				Строка["Надпись"+ВидПериода] =
	...
	ИначеЕсли УпаковкиСтроки.Количество() > 1 
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Автоматический пересчет упаковок не в базовой фасовке недоступен';
				|en = 'Автоматический пересчет упаковок не в базовой фасовке недоступен'"),,
	...
	Если КоличествоУпаковок <> Цел(КоличествоУпаковок) Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Отгружаемое количество не соответствует фасовке';
				|en = 'Отгружаемое количество не соответствует фасовке'"),
			Объект.Ссылка,
			"Объект.Товары[" + ИдентификаторСтрокиТовары + "].КоличествоВЕТИС");
	КонецЕсли;
		...
		Если ЕстьДополнительнаяСтрока Тогда
			...
			Иначе
				...
				ПараметрПредметаИсчисленияМаршрута = НСтр("ru = '|пункту маршрута,|пунктам маршрута,|пунктам маршрута,м,,,,,0';
															|en = '|пункту маршрута,|пунктам маршрута,|пунктам маршрута,м,,,,,0'");
...
Процедура ВыполнитьКомандуЗаполнитьЗаписиСкладскогоЖурнала()
	...
	Если Не ЗначениеЗаполнено(Объект.ГрузоотправительХозяйствующийСубъект) Тогда
		ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Хозяйствующий субъект (грузоотправитель)""';
								|en = 'Не заполнено поле ""Хозяйствующий субъект (грузоотправитель)""'");
	...
	Если Не ЗначениеЗаполнено(Объект.ГрузоотправительПредприятие) Тогда
		ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Предприятие (грузоотправитель)""';
								|en = 'Не заполнено поле ""Предприятие (грузоотправитель)""'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			ТекстСообщения,
			Объект.Ссылка, 
			"Объект.ГрузоотправительПредприятие",,
		...
		Если ДополнительныеПараметры.Свойство("ТоварыЗаписьСкладскогоЖурнала")
			...
			Если ЗначениеЗаполнено(ИзменяемаяСтрока.ЗаписьСкладскогоЖурнала)
				...
				ТекстСообщения = НСтр( "ru = 'Остатка продукции по записи складского журнала %1 не достаточно для оформления ВСД.';
										|en = 'Остатка продукции по записи складского журнала %1 не достаточно для оформления ВСД.'");
...
Процедура ЗаполнитьРеквизитыПоУмолчаниюНовойСтрокиТовары(СтрокаТабличнойЧасти, БлагополучиеМестности, Копирование = Ложь)
	...
	СтрокаТабличнойЧасти.БлагополучиеМестности               = НСтр("ru = 'Местность благополучна по заразным болезням животных';
																	|en = 'Местность благополучна по заразным болезням животных'");
	...
	Если Инициализация Или СтруктураПолей.Свойство("Номенклатура") Тогда
		...
		Если СопоставленоКоличество > 1 Тогда
			ТекущаяСтрока.СопоставлениеНоменклатура   = СтрШаблон(НСтр("ru = '<Несколько позиций (%1)>';
																		|en = '<Несколько позиций (%1)>'"), СопоставленоКоличество);
...
Процедура ВыполнитьКонтрольЗаписейСкладскихЖурналов(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ru = 'В табличной части ""Товары"" будут очищены подобранные записи складского журнала. Продолжить?';
						|en = 'В табличной части ""Товары"" будут очищены подобранные записи складского журнала. Продолжить?'");
