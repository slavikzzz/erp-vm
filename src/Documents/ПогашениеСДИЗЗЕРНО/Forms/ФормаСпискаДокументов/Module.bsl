...
Процедура СписокПоДокументамКОформлениюВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	ПоказатьПредупреждение(,НСтр("ru = 'Открытие СДИЗ возможно в режиме просмотра ""по СДИЗ""';
								|en = 'Открытие СДИЗ возможно в режиме просмотра ""по СДИЗ""'"),30);
КонецПроцедуры
...
Процедура Оформить(Команда)
	...
	Если ТТНПродукция = 0 Тогда //продукция
		...
		КонецЕсли;
	Иначе 
		Если НЕ ОбщегоНазначенияИСКлиент.ВыборСтрокиСпискаКорректен(Элементы.СписокПоДокументамКОформлению,Истина) Тогда
			Возврат;
		ИначеЕсли НЕ ПолучитьСДИЗПоОтбору(Элементы.СписокПоДокументамКОформлению.ТекущиеДанные, МассивСДИЗ) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр( "ru = 'Нет данных для заполнения';
																|en = 'Нет данных для заполнения'"),,"СписокПоДокументамКОформлению");
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура КорректировкаДанных(Команда)
	...
	Если Не ОбщегоНазначенияИСКлиент.ВыборСтрокиСпискаКорректен(Элементы.Список) Тогда
		Возврат;
	КонецЕсли;
...
Функция ВозможностьВводаПогашенияСДИЗ(Знач ВыделенныеСтроки)
	...
	РеквизитыСДИЗов = ОбщегоНазначенияИСВызовСервера.ЗначенияРеквизитовОбъектов(ВыделенныеСтроки, СтрСоединить(ТребуемыеРеквизиты, ","));
		...
		Для Каждого КлючИЗначение Из ДанныеОтбора Цикл
			Если РеквизитыСДИЗ[КлючИЗначение.Ключ] <> КлючИЗначение.Значение Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					НСтр("ru = 'Различаются ключевые реквизиты СДИЗ. Выделенные строки должны быть включены в различные документы гашения';
						|en = 'Различаются ключевые реквизиты СДИЗ. Выделенные строки должны быть включены в различные документы гашения'"),,"СписокКОформлению");
				Возврат Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
