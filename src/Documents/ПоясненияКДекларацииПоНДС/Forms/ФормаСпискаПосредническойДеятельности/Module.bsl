...
Процедура ЗаполнитьКонтрагентов()
	...
	ТаблицаИННКПП = ПосредническаяДеятельность.Выгрузить(, "ПродавецИНН");
	ТаблицаИННКПП.Колонки["ПродавецИНН"].Имя = "ИНН";
	ТаблицаИННКПП.Колонки.Добавить("КПП", ОбщегоНазначения.ОписаниеТипаСтрока(9)); 
	
	Для каждого Строка Из ТаблицаИННКПП Цикл
		Если СтрНайти(Строка.ИНН, "/") Тогда
			Части = СтрРазделить(Строка.ИНН, "/");
			Строка.ИНН = Части[0];
			Строка.КПП = Части[1];
		КонецЕсли;
	КонецЦикла;
	
	ТаблицаКонтрагентов = Документы.ПоясненияКДекларацииПоНДС.КонтрагентыПоСпискуИННиКПП(ТаблицаИННКПП);
	
	Для Каждого Элемент Из ПосредническаяДеятельность Цикл
		
		Если СтрНайти(Элемент.ПродавецИНН, "/") Тогда
			Части = СтрРазделить(Элемент.ПродавецИНН, "/");
			ИНН = Части[0];
			КПП = Части[1];
			Сведения = Документы.ПоясненияКДекларацииПоНДС.СведенияОКонтрагентеПоТаблицеКонтрагентов(ИНН, ТаблицаКонтрагентов, КПП);
		Иначе
			Сведения = Документы.ПоясненияКДекларацииПоНДС.СведенияОКонтрагентеПоТаблицеКонтрагентов(Элемент.ПродавецИНН, ТаблицаКонтрагентов, КПП);
		КонецЕсли;
...
Процедура УстановитьСведенияОКонтрагентеРеквизита(ДанныеСтроки, Сведения)
	...
	Если ЗначениеЗаполнено(Сведения.КПП) Тогда
		ДанныеСтроки.ПродавецИНН = Сведения.ИНН + "/" + Сведения.КПП;
	Иначе
...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Для Каждого Элемент Из ПосредническаяДеятельность Цикл
		Если ЗначениеЗаполнено(Элемент.ПродавецИНН) 
			И НЕ (СтрДлина(Элемент.ПродавецИНН) = 10 ИЛИ СтрДлина(Элемент.ПродавецИНН) = 12 ИЛИ СтрДлина(Элемент.ПродавецИНН) = 20) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Корректность", НСтр("ru = 'ИНН продавца';
																											|en = 'ИНН продавца'"), , , НСтр("ru = 'Проверьте ИНН/КПП продавца';
																																				|en = 'Проверьте ИНН/КПП продавца'"));
