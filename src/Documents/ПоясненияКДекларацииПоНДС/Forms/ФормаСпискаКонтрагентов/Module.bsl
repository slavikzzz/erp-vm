...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Контрагент Из Параметры.Контрагенты Цикл
		...
		НоваяСтрока.ИНН = Контрагент.Значение.ИНН;
		НоваяСтрока.КПП = Контрагент.Значение.КПП;
		НоваяСтрока.ПредставлениеКонтрагента = Контрагент.Представление;
		НоваяСтрока.Контрагенты = Контрагент.Значение.Контрагенты;
		НоваяСтрока.КоличествоКонтрагентов = Контрагент.Значение.Контрагенты.Количество();
...
&НаКлиенте
Процедура КонтрагентыПредставлениеКонтрагентаНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
...
&НаКлиенте
Процедура КонтрагентыПредставлениеКонтрагентаПриИзменении(Элемент)
	...
	Для Каждого Контрагент Из Контрагенты Цикл
		...
		ИначеЕсли НЕ (СтрДлина(Контрагент.ИНН) = 10 ИЛИ СтрДлина(Контрагент.ИНН) = 12) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Корректность", НСтр("ru = 'ИНН';
																											|en = 'ИНН'"), , , НСтр("ru = 'Длина ИНН должна быть 10 или 12 символов';
...
Процедура ЗаполнитьКонтрагентов()
	...
	ТаблицаИНН = Контрагенты.Выгрузить(, "ИНН, КПП");
	
	ТаблицаКонтрагентов = Документы.ПоясненияКДекларацииПоНДС.КонтрагентыПоСпискуИННиКПП(ТаблицаИНН);
	
	Для Каждого Контрагент Из Контрагенты Цикл
		
		Сведения = Документы.ПоясненияКДекларацииПоНДС.СведенияОКонтрагентеПоТаблицеКонтрагентов(Контрагент.ИНН, ТаблицаКонтрагентов, Контрагент.КПП);
...
Процедура УстановитьВозможностьВыбораКонтрагентов()
	...
	Если НЕ КонтрагентыДоступны Тогда
		Элементы.КонтрагентыПредставлениеКонтрагента.КнопкаВыбора = Ложь;
	КонецЕсли;
