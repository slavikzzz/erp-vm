...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если НЕ ДополнительныеСвойства.Свойство("ВыполненРасчет") Тогда
		...
		Попытка
			...
			//++ Локализация
			Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереоценкаФинансовыхИнструментов
				ИЛИ ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереоценкаРасчетовПоАренде Тогда
				Документы.РасчетКурсовыхРазниц.ЗаполнитьДвиженияНУФинансовыхИнструментов(Организация, Дата, ХозяйственнаяОперация, , Ссылка);
			КонецЕсли;
			//-- Локализация
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Если ЭтоРасчеты И ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") И ЕстьДвиженияПоСрокам() Тогда
		...
		|по организации ""%2""';
		|en = 'To cancel posting ""%1"" you need to
		|clear from posting all documents generating currency balances
		|by company ""%2""'");
...
Процедура ПроверитьДублиДокументовТекущегоПериода(Отказ)
	...
	Если Не Результат.Пустой() Тогда
		ТекстОшибки = НСтр("ru = 'Для организации ""%1"" в текущем месяце %2 уже существует аналогичный документ ""%3"".
							...
							|Mark for deletion all unposted documents of exchange difference calculation.'");
...
Процедура ОчиститьНаборыЗаписейДвижений(ЭтотОбъект)
	...
	//++ НЕ УТ
	НаборыДвижений.Добавить("ИнвестицииВАренду");
	//-- НЕ УТ
