...
Процедура ТоварыЗаказКлиентаПриИзменении(Элемент)
	...
	Если Объект.ТипОснованияАктаОРасхождении =
					ПредопределенноеЗначение("Перечисление.ТипыОснованияАктаОРасхождении.ПередачаДавальцу") Тогда
		
		ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
		
		Если Не ЗначениеЗаполнено(ТекущиеДанные.ЗаказКлиента) И ЗначениеЗаполнено(ТекущиеДанные.Назначение) Тогда
			
			ТекущиеДанные.Назначение            = Неопределено;
			ТекущиеДанные.НазначениеОтправителя = Неопределено;
			
		ИначеЕсли ЗначениеЗаполнено(ТекущиеДанные.ЗаказКлиента) Тогда
			
			ТекущееНазначение =
				ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(ТекущиеДанные.ЗаказКлиента, "Назначение");
			
			ТекущиеДанные.Назначение            = ТекущееНазначение;
			ТекущиеДанные.НазначениеОтправителя = ТекущееНазначение;
...
Процедура ТоварыНазначениеПриИзменении(Элемент)
	...
	Если Объект.ТипОснованияАктаОРасхождении =
					ПредопределенноеЗначение("Перечисление.ТипыОснованияАктаОРасхождении.ПередачаДавальцу")
	 Или Объект.ТипОснованияАктаОРасхождении =
					ПредопределенноеЗначение("Перечисление.ТипыОснованияАктаОРасхождении.ВозвратДавальцу") Тогда
	...
	ИначеЕсли Результат.Значение = "ФактическоеКоличество" И ТекущиеДанные.КоличествоУпаковокПоДокументу <> 0
		...
		Если Дельта = 0 Тогда 
			ТекстСообщения = НСтр("ru = 'Фактическое количество можно разбивать в строках с излишками.';
									|en = 'You can split the actual quantity in lines with surpluses.'");
			ПоказатьПредупреждение(, ТекстСообщения);
			Возврат;
		КонецЕсли;
		ПараметрыРазбиения = РаботаСТабличнымиЧастямиКлиент.ПараметрыРазбиенияСтроки();
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<заполнено по основанию>';
																|en = '<filled in using base document>'"));
	...
	ОтборЭлемента.ПравоеЗначение.Добавить(Перечисления.ТипыОснованияАктаОРасхождении.ПередачаДавальцу);
	ОтборЭлемента.ПравоеЗначение.Добавить(Перечисления.ТипыОснованияАктаОРасхождении.ВозвратДавальцу);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ТипАкта = ?(РасхожденияКлиентСервер.ТипОснованияПередачаТоваровХранителю(Объект.ТипОснованияАктаОРасхождении)
				...
				"Возврат")));
	//++ НЕ УТКА
	Если Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПередачаДавальцу Тогда
		ТипАкта = "ПередачаДавальцу";
	ИначеЕсли Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратДавальцу Тогда
		ТипАкта = "ВозвратДавальцу";
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура УправлениеРеквизитамиФормыВЗависимостиОтТипаОснования()
	...
	Если Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратПоставщику Тогда
		...
		Элементы.ТоварыКомментарийКлиентаПодвал.Заголовок = НСтр("ru = 'Комментарии поставщика по расхождениям в строке';
																|en = 'Vendor''s comments to the line discrepancies'");
	...
	ИначеЕсли Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ОтгрузкаТоваровСХранения Тогда
		...
		Элементы.ТоварыКомментарийКлиентаПодвал.Заголовок = НСтр("ru = 'Комментарии поклажедателя по расхождениям в строке';
																|en = 'Vendor comments on discrepancies in the line'");
		...
		Элементы.ТоварыЗаполнитьПоОснованиям.Заголовок = НСтр("ru = 'По отгрузке с ответственного хранения';
																|en = 'By VMI pick-up request from vendor'");
		Элементы.ТоварыВидЦены.Видимость = Ложь;
	...
	ИначеЕсли Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПередачаДавальцу
		...
		Элементы.ТоварыГруппаСумма.Видимость     = Ложь;
		Элементы.ТоварыСтавкаНДС.Видимость       = Ложь;
		Элементы.ТоварыГруппаНДС.Видимость       = Ложь;
		Элементы.ТоварыГруппаСуммаСНДС.Видимость = Ложь;
		Элементы.ТоварыПодразделение.Видимость   = Ложь;
		Элементы.Соглашение.Видимость            = Ложь;
		
		Элементы.Партнер.Заголовок                        = НСтр("ru = 'Давалец';
																|en = 'Material provider'");
		Элементы.ТоварыКомментарийКлиентаПодвал.Заголовок = НСтр("ru = 'Комментарии давальца по расхождениям в строке';
																|en = 'Provider comments on line discrepancies'");
		Элементы.ТоварыЕстьКомментарийКлиента.Заголовок   = НСтр("ru = 'давальца';
																|en = 'provider'");
		Элементы.НомерПоДаннымКлиента.Заголовок           = НСтр("ru = 'Номер акта 
																		|по данным давальца';
																		|en = 'Report number 
																		|by provider data'");
		Элементы.НаименованиеВходящегоДокумента.Заголовок = НСтр("ru = 'Наименование 
																		|по данным давальца';
																		|en = 'Description 
																		|by provider data'");
		...
		Если Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПередачаДавальцу Тогда
			...
			СписокВыбораХозяйственныеОперации.Добавить(Перечисления.ХозяйственныеОперации.ПередачаДавальцу2_5);
			
			Элементы.ТоварыЗаполнитьПоОснованиям.Заголовок = НСтр("ru = 'По передаче давальцу';
																	|en = 'By transfer to provider'");
			Элементы.ТоварыДокументОснование.Заголовок     =
				ДавальческаяСхемаКлиентСервер.СинонимДокументаОтгрузкаТоваровСХраненияПередачаДавальцу();
			Элементы.ПоРеализациям.Заголовок               = НСтр("ru = 'По передаче давальцу';
																	|en = 'By transfer to provider'");
		...
		ИначеЕсли Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратДавальцу Тогда
			...
			СписокВыбораХозяйственныеОперации.Добавить(Перечисления.ХозяйственныеОперации.ВозвратДавальцу2_5);
			
			Элементы.ТоварыЗаполнитьПоОснованиям.Заголовок = НСтр("ru = 'По возврату сырья давальцу';
																	|en = 'By return of raw materials to provider'");
			Элементы.ТоварыДокументОснование.Заголовок     =
				ДавальческаяСхемаКлиентСервер.СинонимДокументаОтгрузкаТоваровСХраненияВозвратДавальцу();
			Элементы.ПоРеализациям.Заголовок               = НСтр("ru = 'По возврату сырья давальцу';
																	|en = 'By return of raw materials to provider'");
	...
	Если НоменклатураКлиент.ДляУказанияСерийНуженСерверныйВызов(ЭтаФорма,ПараметрыУказанияСерий,Текст, ТекущиеДанные) Тогда
		...
		Если ТекущиеДанные.ЗаполненоПоРеализации
			...
			ПоказатьПредупреждение(,НСтр("ru = 'Серия в строке из документа-основания не редактируется, добавьте новую строку.';
										|en = 'Batch cannot be edited in a line of the base document, add a new line.'"));
			Возврат;
		КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовПоОперацииСервер()
	...
	ЦеныБезНДС = РасхожденияКлиентСервер.ТипОснованияПередачаТоваровХранителю(Объект.ТипОснованияАктаОРасхождении)
		...
		Или Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратДавальцу
		//-- НЕ УТКА
		Или РасхожденияКлиентСервер.ТипОснованияПередачаНаКомиссию(Объект.ТипОснованияАктаОРасхождении);
...
Процедура СформироватьЗависимыеОтТипаОснованияЗаголовки()
	...
	ИначеЕсли Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратПоставщику Тогда
		Заголовок = НСтр("ru = 'Акт о расхождениях после возврата товаров поставщику (создание)';
						|en = 'Discrepancy report following return of goods to vendor (create)'");
	...
	ИначеЕсли Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ОтгрузкаТоваровСХранения Тогда
		Заголовок = НСтр("ru = 'Акт о расхождениях после отгрузки товаров с хранения (создание)';
						|en = 'VMI pick-up discrepancy report (create)'");
		Элементы.Договор.Подсказка = НСтр("ru = 'Договор, в рамках которого оформлена отгрузка';
											|en = 'Contract under which the shipment is documented'");
	ИначеЕсли Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПередачаНаКомиссию Тогда
		Заголовок = НСтр("ru = 'Акт о расхождениях после передачи на комиссию (создание)';
						|en = 'Discrepancy report after consignment sales (create)'");
	...
	//++ НЕ УТКА
	ИначеЕсли Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПередачаДавальцу Тогда
		
		Заголовок = СтрШаблон(
						НСтр("ru = '%1 (создание)';
							|en = '%1 (create)'"),
						ДавальческаяСхемаКлиентСервер.СинонимДокументаАктОРасхожденияхПередачаДавальцу());
	...
	ИначеЕсли Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратДавальцу Тогда
		...
		Заголовок = СтрШаблон(
						НСтр("ru = '%1 (создание)';
							|en = '%1 (create)'"),
						ДавальческаяСхемаКлиентСервер.СинонимДокументаАктОРасхожденияхВозвратСырьяДавальцу());
...
Функция ПолучитьХозяйственнуюОперациюДоговора(ХозяйственнаяОперация)
	...
	//++ НЕ УТКА
	ИначеЕсли ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5")
		  Или ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратДавальцу2_5") Тогда
		
		Возврат ДавальческаяСхемаКлиентСервер.ХозяйственнаяОперацияДоговора();
