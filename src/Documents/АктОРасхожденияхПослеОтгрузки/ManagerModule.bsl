...
Функция МакетФормыВыбораНазначений() Экспорт
	...
	ОписаниеКолонок = Справочники.Назначения.ДобавитьОписаниеКолонок(
		МакетФормы, "ОбеспечениеЗаказов", Истина, "Объект.Товары.Назначение");
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если РасхожденияКлиентСервер.ТипОснованияРеализацияТоваровУслуг(Данные.ТипОснованияАктаОРасхождении) Тогда
		ШаблонПредставления = НСтр("ru = 'Акт о расхождениях после реализации %1 от %2';
									|en = 'Sales invoice dispute %1 dated %2'");
	ИначеЕсли РасхожденияКлиентСервер.ТипОснованияВозвратПоставщику(Данные.ТипОснованияАктаОРасхождении) Тогда
		ШаблонПредставления = НСтр("ru = 'Акт о расхождениях после возврата поставщику %1 от %2';
									|en = 'Discrepancy report following the return of goods to a vendor %1 dated %2'");
	ИначеЕсли Данные.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПередачаТоваровХранителю  Тогда
		ШаблонПредставления = НСтр("ru = 'Акт о расхождениях после передачи товаров хранителю %1 от %2';
									|en = 'Discrepancy report following VMI fill-up %1 dated %2'");
	//++ НЕ УТ
	ИначеЕсли Данные.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПередачаТоваровПереработчику Тогда
		ШаблонПредставления = НСтр("ru = 'Акт о расхождениях после передачи сырья переработчику %1 от %2';
									|en = 'Discrepancy report following customer stock fill-up %1 dated %2'");
	//-- НЕ УТ
	ИначеЕсли РасхожденияКлиентСервер.ТипОснованияПередачаНаКомиссию(Данные.ТипОснованияАктаОРасхождении) Тогда
		ШаблонПредставления = НСтр("ru = 'Акт о расхождениях после передачи товаров на комиссию %1 от %2';
									|en = 'Discrepancy report following consignment sales %1 dated %2'");
	//++ НЕ УТКА
	ИначеЕсли Данные.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПередачаДавальцу Тогда
		ШаблонПредставления = НСтр("ru = 'Акт о расхождениях после передачи давальцу %1 от %2';
									|en = 'Discrepancy report following the transfer of goods to a provider %1 dated %2'"); 
	ИначеЕсли Данные.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратДавальцу Тогда
		ШаблонПредставления = НСтр("ru = 'Акт о расхождениях после возврата сырья давальцу %1 от %2';
									|en = 'Discrepancy report following the return of raw materials to a provider %1 dated %2'");
	//-- НЕ УТКА
	Иначе
		ШаблонПредставления = НСтр("ru = 'Акт о расхождениях после отгрузки товаров с хранения %1 от %2';
									|en = 'VMI pick-up discrepancy report %1 dated %2'");
	КонецЕсли;
...
Процедура ЗапланироватьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	//++ НЕ УТ
	|		КОГДА ТИПЗНАЧЕНИЯ(ТоварыДокумента.ЗаказКлиента) = ТИП(Документ.ЗаказПереработчику2_5)
	|			И НЕ ТоварыДокумента.ЗаказКлиента = ЗНАЧЕНИЕ(Документ.ЗаказПереработчику2_5.ПустаяСсылка)
	|			ТОГДА ТоварыДокумента.ЗаказКлиента
	//-- НЕ УТ
...
Функция СкладскаяОперацияПоТипуОснования(ТипОснованияАктаОРасхождении)
	...
	ИначеЕсли РасхожденияКлиентСервер.ТипОснованияВозвратПоставщику(ТипОснованияАктаОРасхождении)
		//++ НЕ УТКА
		Или ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПередачаДавальцу
		Или ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратДавальцу
		//-- НЕ УТКА
