...
Процедура СформироватьЗаданияКФормированиюДвиженийПоНДС(ТаблицаИзменений) Экспорт
	...
	|	СчетФактура.ДокументОснование   КАК СчетФактура,
	|	СчетФактура.Организация         КАК Организация,
	|	ПоступлениеУслугПоАренде.Дата   КАК Месяц
	|ИЗ 
	...
	|		ВтРасчеты КАК Расчеты
	|	ПО
	|		Расчеты.РасчетныйДокумент = ПоступлениеУслугПоАренде.Ссылка
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.ПоступлениеУслугПоАренде.Начисления КАК ТаблицаНачисления 
	|	ПО
	|		ПоступлениеУслугПоАренде.Ссылка = ТаблицаНачисления.Ссылка
	| 
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		Документ.СчетФактураПолученныйАванс КАК СчетФактура 
	|	ПО 
	|		ТаблицаНачисления.РасчетныйДокумент = СчетФактура.ДокументОснование 
	| 
	|ГДЕ 
	|	СчетФактура.Проведен
	|	
	...
	|	СчетФактура.ДокументОснование   КАК СчетФактура,
	|	СчетФактура.Организация         КАК Организация,
	|	ВыкупАрендованныхОС.Дата        КАК Месяц
	|ИЗ 
	|	Документ.ВыкупАрендованныхОС КАК ВыкупАрендованныхОС
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		ВтРасчеты КАК Расчеты
	|	ПО
	|		Расчеты.РасчетныйДокумент = ВыкупАрендованныхОС.Ссылка
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.ВыкупАрендованныхОС.ЗачетАвансов КАК ТаблицаЗачетаАвансов 
	|	ПО
	|		ВыкупАрендованныхОС.Ссылка = ТаблицаЗачетаАвансов.Ссылка
	| 
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		Документ.СчетФактураПолученныйАванс КАК СчетФактура 
	|	ПО 
	|		ТаблицаЗачетаАвансов.РасчетныйДокумент = СчетФактура.ДокументОснование 
	| 
	|ГДЕ 
	|	СчетФактура.Проведен
	//-- Локализация
...
Функция СформироватьГиперссылкуСмТакжеВРаботе(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru = 'Список счетов-фактур на аванс';
							|en = 'List of tax invoices for advance'");
...
Функция ПолучитьВыданныеАвансы(Организация, НачалоПериода, КонецПериода, РасчетныйДокумент = Неопределено, ВРазрезеСтавокНДС = Ложь, ВРазрезеОР = Ложь, ИсключитьИУДА = Ложь) Экспорт
	...
	Если ТипЗнч(Организация) = Тип("Массив") И Организация.Количество() = 1 Тогда
		...
		ПрименяетсяЛьготноеНалогообложениеНДС = УчетНДСЛокализация.ПрименяетсяЛьготноеНалогообложениеНДС(Организация[0], НачалоПериода);
	ИначеЕсли ТипЗнч(Организация) = Тип("СправочникСсылка.Организации") Тогда
		...
		ПрименяетсяЛьготноеНалогообложениеНДС = УчетНДСЛокализация.ПрименяетсяЛьготноеНалогообложениеНДС(Организация, НачалоПериода);
	Иначе
	...
	Пока ВыборкаАванс.Следующий() Цикл
		...
		Если ПрименяетсяЛьготноеНалогообложениеНДС И НЕ ОблагаетсяНДСУПокупателя Тогда
			Продолжить;
		КонецЕсли;
...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры)
	...
	//++ НЕ УТ
	СчетФактураПолученныйАвансЛокализация.ДополнитьТекстЗапросаРеестрДокументов(Запрос, ТекстПроведения, Регистры);
	//-- НЕ УТ
