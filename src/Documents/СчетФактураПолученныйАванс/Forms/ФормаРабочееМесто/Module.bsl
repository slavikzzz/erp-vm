...
Процедура ВвестиСчетФактуруПолученныйАванс()
	...
	Если ТекущиеДанные.СчетФактура.Пустая() Тогда
		...
		Если Не ЗначениеЗаполнено(Организация) И ИспользоватьНесколькоОрганизаций Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выберите организацию';
																	|en = 'Select company'"), , "Организация");
			Возврат;
		КонецЕсли;
...
Процедура ОбновитьДанныеФормы()
	...
	Если ЗначениеЗаполнено(ВалютаРеглУчета) Тогда 
		Элементы.ТаблицаВыданныеАвансыСуммаАвансаРегл.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сумма аванса (%1)';
																																|en = 'Prepayment amount (%1)'"), ВалютаРеглУчета);
	Иначе
		Элементы.ТаблицаВыданныеАвансыСуммаАвансаРегл.Заголовок = НСтр("ru = 'Сумма аванса (регл.)';
																		|en = 'Prepayment amount (local)'");
	КонецЕсли;
