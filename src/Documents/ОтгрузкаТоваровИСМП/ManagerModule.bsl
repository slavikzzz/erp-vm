...
Функция ОтгрузкаТоваровJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	Если ДоступныеВидыПродукцииИС.Найти(Шапка.ВидПродукции) = Неопределено Тогда
		...
		СообщениеJSON.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Вид продукции ""%1"" не поддерживается для отгрузки товаров.';
						|en = 'Вид продукции ""%1"" не поддерживается для отгрузки товаров.'"),
					Шапка.ВидПродукции);
		СообщениеJSON.ТребуетсяПодписание = Ложь;
		СообщенияJSON.Добавить(СообщениеJSON);
		Возврат СообщенияJSON;
	...
	Если ВидыОперацийПоВидуПродукции = Неопределено
		...
		СообщениеJSON.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Вид операции ""%1"" не поддерживается для отгрузки товаров.';
						|en = 'Вид операции ""%1"" не поддерживается для отгрузки товаров.'"),
					Шапка.Операция);
		СообщениеJSON.ТребуетсяПодписание = Ложь;
		СообщенияJSON.Добавить(СообщениеJSON);
		Возврат СообщенияJSON;
	...
	Иначе
		ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
			СообщениеJSON,
			СтрШаблон(
				НСтр("ru = 'Неизвестный тип операции Уведомление об отгрузке маркированных товаров ИС МП: %1';
					|en = 'Неизвестный тип операции Уведомление об отгрузке маркированных товаров ИС МП: %1'"), Шапка.Операция));
	КонецЕсли;
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(РеквизитыКонтрагента.ИНН) Тогда
			ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
				СообщениеJSON,
				СтрШаблон(
					НСтр("ru = 'Не заполнено поле ""ИНН"" для грузополучателя %1';
						|en = 'Не заполнено поле ""ИНН"" для грузополучателя %1'"), Шапка.Контрагент));
		КонецЕсли;
				...
				ТекстыОшибокПоСертификатам.Добавить(
					СтрШаблон(
						НСтр("ru = 'Для всех строк %1 необходимо или указать данные сертификации или не указывать.';
							|en = 'Для всех строк %1 необходимо или указать данные сертификации или не указывать.'"),
				...
				ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
					...
					СтрШаблон(
						НСтр("ru = 'Передача кодов с видом упаковки ""%1"" не поддерживается.';
							|en = 'Передача кодов с видом упаковки ""%1"" не поддерживается.'"),
				...
				Если НайденныеСтроки.Количество() = 1 Тогда
					...
					Если СтрокаТЧТовары.Количество = 0 Тогда
						ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
							СообщениеJSON,
							СтрШаблон(
								НСтр("ru = 'Не заполнено поле ""Количество"" для номенклатуры %1.';
									|en = 'Не заполнено поле ""Количество"" для номенклатуры %1.'"), ПараметрыОтбора.Номенклатура));
					Иначе
									...
									ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
										...
										СтрШаблон(
											НСтр("ru = 'Не указан ИНН (или аналог) импортера %1 для номенклатуры %2';
												|en = 'Не указан ИНН (или аналог) импортера %1 для номенклатуры %2'"),
									...
									ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
										...
										СтрШаблон(
											НСтр("ru = 'Не указан ИНН (или аналог) производителя %1 для номенклатуры %2';
												|en = 'Не указан ИНН (или аналог) производителя %1 для номенклатуры %2'"),
...
Функция ДоступныеВидыОперацийПоВидуПродукцииИС() Экспорт
	...
	ВидыОперацийПоВидуПродукцииИС.Вставить(Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность2025, ОтгрузкаЕАЭССПризнаниемКИ);
