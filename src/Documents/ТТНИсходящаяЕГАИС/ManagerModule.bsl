...
// 
// Возвращаемое значение:
//  Структура - См. РегистрыСведений.СтатусыДокументовЕГАИС.ВозвращаемоеЗначениеДальнейшиеДействияСтатус
//  Булево - Истина, если требуется восстановить статус.
Функция СтатусПослеПолученияДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли Операция = Перечисления.ВидыДокументовЕГАИС.УведомлениеОРегистрацииДвиженияТТН Тогда
		...
		// получение информации от УТМ в неправильном поряждке
		Если ТекущееСостояние = Неопределено
			Или ТекущееСостояние.Статус = Перечисления.СтатусыОбработкиТТНИсходящейЕГАИС.ОбрабатываетсяЕГАИС Тогда
			ПараметрыОбновления = РегистрыСведений.СтатусыДокументовЕГАИС.РассчитатьСтатусыПриПолученииКвитанции(
				ДокументСсылка,
				"УведомлениеОРегистрацииДвижения",
				Перечисления.СтатусыОбработкиСообщенийЕГАИС.ПринятИзЕГАИС,
				СтатусыБазовыйПроцесс,
				ВыполнитьРасчетТекущегоСостояния);
		Иначе
			ПараметрыОбновления = Истина;
		КонецЕсли;
...
//  ДокументСсылка - ДокументСсылка.ТТНИсходящаяЕГАИС - Документ, для которого требуется обновить статус.
//  Операция - ПеречислениеСсылка.ВидыДокументовЕГАИС - Операция обмена с ЕГАИС.
//  ДополнительныеПараметры - Булево, Неопределено, Структура - со свойствами:
//   * СтатусОбработки - ПеречислениеСсылка.СтатусыОбработкиСообщенийЕГАИС - Статус обработки сообщения.
//   * ОперацияКвитанции - ПеречислениеСсылка.ВидыДокументовЕГАИС - Операция, на которую получена квитанция.
// 
// Возвращаемое значение:
...
Функция ОбновитьСтатусПослеПолученияДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ПараметрыОбновления = Истина Тогда
		
		ОбменДаннымиЕГАИСВызовСервера.ВосстановитьСтатусДокументаПоДаннымПротоколаОбмена(ДокументСсылка);
		НовыйСтатусПослеОбновления = РегистрыСведений.СтатусыДокументовЕГАИС.ТекущееСостояние(ДокументСсылка);
...
//
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.СтатусыОбработкиТТНИсходящейЕГАИС - Статусы ошибок.
//
...
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.СтатусыОбработкиТТНИсходящейЕГАИС - Конечные статусы.
...
Функция ТТНИсходящаяЕГАИСXML(ДокументСсылка)
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Если СтрокаТЧ.ПроверятьОбъемДАЛ
			...
			ОбменДаннымиЕГАИСКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Для номенклатуры в строке %1 не установлен объем в декалитрах.';
...
//
// Параметры:
// 	ПараметрыОкончанияПроверки - Структура - (См. Обработки.ПроверкаИПодборАлкогольнойПродукцииЕГАИС.ЗафиксироватьРезультатПроверкиИПодбора)
...
Процедура ПостроитьBoxInfo(СтрокаДереваРодитель, ИспользованныеУзлыДереваУпаковок, Boxtree)
	...
	Если КоробкиБезИерархии.Количество() > 0 Тогда
		Элемент = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(Boxtree, "bl");
		Для Каждого СтрокаДерева Из КоробкиБезИерархии Цикл
			Элемент.boxnum.Добавить(СтрокаДерева.Штрихкод);
		КонецЦикла;
		Boxtree.bl.Добавить(Элемент);
	КонецЕсли;
	...
	Если КоробкиСИерархией.Количество() > 0 Тогда
		Для Каждого СтрокаДерева Из КоробкиСИерархией Цикл
			Элемент = РаботаСXMLИС.ОбъектXDTOПоИмениТипа(Boxtree, "bl");
			Элемент.boxnum.Добавить(СтрокаДерева.Штрихкод);
			ПостроитьBoxInfo(СтрокаДерева, ИспользованныеУзлыДереваУпаковок, Элемент);
			Boxtree.bl.Добавить(Элемент);
		КонецЦикла;
	КонецЕсли;
