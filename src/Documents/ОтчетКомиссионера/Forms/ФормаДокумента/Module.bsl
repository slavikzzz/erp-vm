	...
	Если ЗначениеЗаполнено(Объект.Договор) Тогда
		...
		Если (Не ВидЦеныКомиссионера.ЦенаВключаетНДС = Неопределено 
			...
			ТекстСообщения = НСтр("ru = 'Признак ""Цена включает НДС"" различается в виде цен договора и документа';
									|en = 'The ""Price includes VAT"" checkbox has different statuses in the price type in the contract and in the document'");
...
Процедура ПересчитатьОстаткиПоДаннымУчетаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(
		...
		НСтр("ru = 'Остаток по данным учета в документе заполнен.';
			|en = 'Balance by accounting data is filled in the document.'"),
		БиблиотекаКартинок.Информация32);
