    ...
    Если РезультатВыбора <> Неопределено И РезультатВыбора.Соглашение <> Объект.Соглашение Тогда
        ...
        Если Объект.Товары.Количество() > 0 Тогда
            ТекстВопроса = НСтр("ru = 'Соглашение было изменено, ""Товары"" будут очищены, Продолжить?';
								|en = 'The agreement has been changed, ""Goods"" will be cleared. Continue?'");
            Ответ = Неопределено;
...
Процедура УстановитьЗаголовокДокумента(ЭтоНовыйОбъект)
	...
	Если ЭтоНовыйОбъект Тогда
		...
		Заголовок = НСтр("ru = 'Отчет комиссионера (создание)';
						|en = 'Consignment issue notification (Create)'");
		
	Иначе
		
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отчет комиссионера %1 от %2';
																				|en = 'Consignment issue notification %1 dated %2'"), Объект.Номер, Объект.Дата);
