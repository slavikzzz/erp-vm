	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктСверкиВзаимныхРасчетовСДаннымиКонтрагента") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм,
			...
			НСтр("ru = 'Акт сверки взаимных расчетов (с задолженностью по данными контрагента)';
				|en = 'AR/AP reconciliation statement (with debt amount according to the counterparty data)'"),
			МакетНаПечать);
...
Функция ВывестиДетальныеЗаписи(
	...
	Валюта = СтрШаблон(НСтр("ru = 'в валюте';
							|en = 'in currency'", ОбщегоНазначения.КодОсновногоЯзыка()) + " %1", ДанныеПоОбъектуГруппировки.Валюта);
	...
	Если НастройкиПечати.ДетализацияВзаиморасчетов = "ОбъектРасчетов"
		...
		ШаблонЗаголовка = ?(ШаблонЗаголовка = Неопределено, "", ШаблонЗаголовка + " ") + НСтр("ru = 'по объекту расчетов';
																								|en = 'by AR/AP object'", ОбщегоНазначения.КодОсновногоЯзыка()) + " %1 ";
	ИначеЕсли НастройкиПечати.ДетализацияВзаиморасчетов = "Договор" И НЕ ЭтоФинансовыеИнструменты
		...
		ШаблонЗаголовка = ?(ШаблонЗаголовка = Неопределено, "", ШаблонЗаголовка + " ") + НСтр("ru = 'по договору';
																								|en = 'by contract'", ОбщегоНазначения.КодОсновногоЯзыка()) + " %1 ";
	КонецЕсли;
	...
	Если ДанныеПоОбъектуГруппировки.ЕстьОбороты Тогда
		Для Каждого ДетальныеЗаписи Из ДанныеПоОбъектуГруппировки.Строки Цикл
			...
			Если ДетальныеЗаписи.ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком
				...
				//++ НЕ УТ
				ИЛИ ДетальныеЗаписи.ТипСуммы = Перечисления.ТипыПлатежейПоАренде.АрендныеОбязательства
				ИЛИ ДетальныеЗаписи.ТипСуммы = Перечисления.ТипыПлатежейПоАренде.УслугаПоАренде
				ИЛИ ДетальныеЗаписи.ТипСуммы = Перечисления.ТипыПлатежейПоАренде.УслугаПоАрендеАванс
				ИЛИ ДетальныеЗаписи.ТипСуммы = Перечисления.ТипыПлатежейПоАренде.ВыкупнаяСтоимость
				//-- НЕ УТ
			...
			ИначеЕсли ДетальныеЗаписи.ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом
				//++ НЕ УТ
				ИЛИ ДетальныеЗаписи.ТипСуммы = Перечисления.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж
				//-- НЕ УТ
...
Функция ЗапросДанныеНаПечать(МассивОбъектов, НастройкиПечати) Экспорт
	...
	//++ НЕ УТ
	ЗапросДанныеДокумента.УстановитьПараметр("АрендныеОбязательства", Перечисления.ТипыПлатежейПоАренде.АрендныеОбязательства);
	ЗапросДанныеДокумента.УстановитьПараметр("ОбеспечительныйПлатеж", Перечисления.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж);
	ЗапросДанныеДокумента.УстановитьПараметр("УслугаПоАренде", Перечисления.ТипыПлатежейПоАренде.УслугаПоАренде);
	ЗапросДанныеДокумента.УстановитьПараметр("ВыкупнаяСтоимость", Перечисления.ТипыПлатежейПоАренде.ВыкупнаяСтоимость);
	//-- НЕ УТ
...
// 
Функция ОписаниеРасчетногоДокумента(СтрокаРасчетов) Экспорт
	
	ДокументСсылка = СтрокаРасчетов.РасчетныйДокумент;
	Номер = СтрокаРасчетов.НомерДокумента;
	Дата = СтрокаРасчетов.ДатаДокумента;
	...
	Если ТипЗнч(ДокументСсылка) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
		//++ НЕ УТ
		ИЛИ ТипЗнч(ДокументСсылка) = Тип("СправочникСсылка.ДоговорыАренды")
		//-- НЕ УТ
	...
	ИначеЕсли ДокументСсылка <> Неопределено И НЕ ДокументСсылка.Пустая() Тогда
		...
		Если ЗначениеЗаполнено(СтрокаРасчетов.НаименованиеПервичногоДокумента) Тогда //заполнено только у входящих
			ПредставлениеДокумента = СтрокаРасчетов.НаименованиеПервичногоДокумента;
		Иначе
...
Функция ДанныеДляПечати(Данные, НаименованиеИсходящихДокументов)
	...
	ПредставленияДокументов = Документы.СверкаВзаиморасчетов2_5_11.ПредставленияДокументов();
	ВходящиеНакладные = Документы.СверкаВзаиморасчетов2_5_11.ВходящиеНакладные();
	ИсходящиеНакладные = Документы.СверкаВзаиморасчетов2_5_11.ИсходящиеНакладные();
	...
	ИначеЕсли Данные.РасчетныйДокумент <> Неопределено И НЕ Данные.РасчетныйДокумент.Пустая() Тогда
		...
		ИначеЕсли ИсходящиеНакладные.Свойство(ИмяДокумента) Тогда
			...
			Иначе
				Результат.НаименованиеДокумента = НСтр("ru = 'УПД';
														|en = 'УПД'");
			КонецЕсли;
		//-- Локализация
		Иначе
...
Процедура СвернутьРасчетыПоГруппировкам(ДанныеДокумента, ТаблицаВзаиморасчетов)
	...
	Для Каждого СтрокаРасчетов Из ТаблицаВзаиморасчетов Цикл
		...
		Если СтрокаРасчетов.НачальноеСальдо = 0 И ДанныеСумм.ЕстьЗаполненные Тогда
			...
			НоваяСтрока.ОписаниеДокумента = ОписаниеРасчетногоДокумента(СтрокаРасчетов);
			//++ Локализация
			...
			Если СФДокумента.Количество() > 0 Тогда
				Для Каждого СФ Из СФДокумента Цикл
					...
					КонецЕсли;
				КонецЦикла;
				ШаблонДокумента = НСтр("ru = '%1 №%2 от %3';
										|en = '%1 №%2 от %3'");
				ОписаниеСФ = СтрШаблон("(" + ШаблонДокумента + ")",
										НСтр("ru = 'Счет-фактура';
											|en = 'Счет-фактура'"), СокрЛП(НомерНаПечать), Формат(СФ.Дата, "ДЛФ=D"));
				Если НЕ СтрНачинаетсяС(СокрЛП(ВРег(ДанныеДляПечати.НаименованиеДокумента)), НСтр("ru = 'УПД';
																								|en = 'УПД'")) Тогда
					НоваяСтрока.ОписаниеДокумента = СтрШаблон(ШаблонДокумента,
						ДанныеДляПечати.НаименованиеДокумента, 
						?(ЗначениеЗаполнено(ДанныеДляПечати.НомерДокумента), ДанныеДляПечати.НомерДокумента, "_______"), 
						?(ЗначениеЗаполнено(ДанныеДляПечати.ДатаДокумента), Формат(ДанныеДляПечати.ДатаДокумента, "ДЛФ=D"), "  .  .    "))
						+ " " + ОписаниеСФ;
				Иначе
					НоваяСтрока.ОписаниеДокумента = СтрШаблон(ШаблонДокумента,
						ДанныеДляПечати.НаименованиеДокумента, СокрЛП(НомерНаПечать), Формат(СФ.Дата, "ДЛФ=D"));
				КонецЕсли;
			КонецЕсли;
			//-- Локализация
...
Функция ПолучитьВзаиморасчеты(КомпоновщикОтбор) Экспорт
	...
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикОтбор.Настройки, "УслугаПоАрендеАванс", Перечисления.ТипыПлатежейПоАренде.УслугаПоАрендеАванс,,Истина);
	...
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикОтбор.Настройки, "ВыкупнаяСтоимостьАванс", Перечисления.ТипыПлатежейПоАренде.ВыкупнаяСтоимостьАванс,,Истина);
	ТипыСуммАренды = Новый Массив;
	...
	ТипыСуммАренды.Добавить(Перечисления.ТипыПлатежейПоАренде.УслугаПоАрендеАванс);
...
// * Сумма - Неопределено -
// 
Функция ДанныеДокумента() Экспорт
	...
	Результат.Вставить("НаименованиеПервичногоДокумента", Неопределено);
	Результат.Вставить("Валюта", Неопределено);
	Результат.Вставить("Сумма", Неопределено);
...
Функция ДанныеРасчетногоДокумента(РасчетныеДокументы) Экспорт
	...
	|	РеестрДокументов.ДатаПервичногоДокумента КАК ДатаДокумента,
	|	РеестрДокументов.НомерПервичногоДокумента КАК НомерДокумента,
	...
	|	СуммыДокументов.Дата КАК ДатаДокумента,
	|	СуммыДокументов.Номер КАК НомерДокумента,
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеДокумента.РасчетныйДокумент = Выборка.Ссылка;
	КонецЦикла;
...
Процедура ИнициализироватьКомпоновщикНастроек(КомпоновщикОтбор, УникальныйИдентификаторФормы = Неопределено, НастройкиОтбора = Неопределено) Экспорт
	...
	Если ВнеоборотныеАктивы.ИспользуетсяУправлениеВНА() Тогда
		ДоступныеТипыРасчетов.Добавить(Перечисления.ТипыРасчетовСПартнерами.РасчетыСАрендодателем);
	КонецЕсли;
	//-- НЕ УТКА
...
Функция ТекстыЗапросовДоступныхТаблиц()
	...
	КонецЕсли;
	//++ НЕ УТ
	Если ПравоДоступа("Просмотр", Метаданные.Справочники.ДоговорыАренды) Тогда
		Результат.Вставить("#2", ТекстЗапросаДоговоровЛизинга()); //СправочникДоговорыАренды
	КонецЕсли;
	//-- НЕ УТ
...
КонецФункции
//++ НЕ УТ
Функция ТекстЗапросаДоговоровЛизинга()
	...
	|ВЫБРАТЬ
	|	&НачалоПериода КАК Период,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСАрендодателем) КАК ТипРасчетов,
	|	НЕОПРЕДЕЛЕНО КАК АналитикаУчетаПоПартнерам,
	|	Договоры.Организация КАК Организация,
	|	Договоры.Партнер КАК Партнер,
	|	Договоры.Контрагент КАК Контрагент,
	...
	|	0 КАК ВыкупнаяСтоимость,
	|	
	|	33 КАК ПорядокТипаПлатежа
	|ИЗ
	|	Справочник.ДоговорыАренды КАК Договоры
	|ГДЕ
	|	НЕ Договоры.ПометкаУдаления
	|	И &ДоговорыБезОборотов
	|{ГДЕ
	...
	|				ИЛИ Договоры.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерыДоговоровФинансовыхИнструментов.ЗаймВыданный)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСДебитором)
	|		КОГДА Договоры.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерыДоговоровФинансовыхИнструментов.Аренда)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСАрендодателем)
	|	КОНЕЦ КАК ТипРасчетов,
	|	ИСТИНА КАК ДоговорыБезОборотов}";
		...
		Если ДетальныеРасчеты = Неопределено Тогда 
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По документу %1 нет расчетов с партнерами. Печать акта сверки взаимных расчетов не требуется.';
					|en = 'There are no settlements with partners for document %1. There is no need to print reconciliation statement of mutual settlements.'"),
			...
			Если ТаблицаОстатков <> Неопределено Тогда
				...
				ТекстЗаголовкаПриложения = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(ДанныеДокумента,
					НСтр("ru = 'к акту сверки взаимных расчетов';
						|en = 'to mutual settlements reconciliation statement'"));
				...
				Для Каждого СтрокаЗадолженности Из ТаблицаЗадолженностиН Цикл
					...
					Иначе
						...
						Если ДанныеДокумента.РасшифровкаПоПартнерам Тогда
							ОбластьМакета.Параметры.Дебитор = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = '%1 перед %2';
									|en = '%1 before %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
								СтруктураПредставленийУчастников.ПредставлениеОрганизации,
	...
	Если ДанныеДокумента.РасшифровкаПоПартнерам Тогда 
		ОбластьМакетаГруппировка.Параметры.ПартнерВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1, в валюте: %2';
				|en = '%1, in currency: %2'", ОбщегоНазначения.КодОсновногоЯзыка()), СтрокаЗадолженности.Партнер, ВыборкаДетальныхЗаписей.Валюта);
	ИначеЕсли ДанныеДокумента.РасшифровкаПоДоговорам Тогда 
		ОбластьМакетаГруппировка.Параметры.ПартнерВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1, в валюте: %2';
				|en = '%1, in currency: %2'", ОбщегоНазначения.КодОсновногоЯзыка()), СтрокаЗадолженности.Договор, ВыборкаДетальныхЗаписей.Валюта);
	Иначе
		ОбластьМакетаГруппировка.Параметры.ПартнерВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'в валюте: %1';
				|en = 'in the currency: %1'", ОбщегоНазначения.КодОсновногоЯзыка()), ВыборкаДетальныхЗаписей.Валюта);
	КонецЕсли;
	...
	Пока ДетальныеЗаписи.Следующий() Цикл
		...
		Иначе
			...
			ОписаниеДокумента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 №%2 от %3';
					|en = '%1 No%2 dated %3'", ОбщегоНазначения.КодОсновногоЯзыка()),
				?(ПредставлениеДокумента <> Неопределено, ПредставлениеДокумента, ""),
				НомерНаПечать,
			...
			Если ЗначениеЗаполнено(ДетальныеЗаписи.ОрганизацияРеглУчет) Тогда
				...
				ОписаниеДокумента = НСтр("ru = 'Перенос задолженности';
										|en = 'Transfer debt'");
				ПредставлениеВладельцаДокумента = НСтр("ru = 'от';
														|en = 'dated'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + ПредставлениеОрганизации;
	...
	Для Каждого ВалютаВзаиморасчетов Из МассивВалют Цикл
		...
		Если МассивСтрокЗадолженности.Количество() > 0 Тогда
			Если Партнер <> Неопределено Тогда
				ОбластьМакетаГруппировка.Параметры.ПартнерВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1, в валюте: %2';
						|en = '%1, in currency: %2'", ОбщегоНазначения.КодОсновногоЯзыка()), ПредставлениеПартнера, ВалютаВзаиморасчетов);
			Иначе
				ОбластьМакетаГруппировка.Параметры.ПартнерВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'в валюте: %1';
						|en = 'in the currency: %1'", ОбщегоНазначения.КодОсновногоЯзыка()), ВалютаВзаиморасчетов);
			КонецЕсли;
			ТабличныйДокумент.Вывести(ОбластьМакетаГруппировка);
			ОбластьМакетаШапка.Параметры.Заполнить(СтруктураПредставленийУчастников);
			ТабличныйДокумент.Вывести(ОбластьМакетаШапка);
	...
	Иначе
		...
		ОписаниеДокумента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Предоплата от %1';
				|en = 'Prepayment from %1'", ОбщегоНазначения.КодОсновногоЯзыка()), ПредставлениеДебитора);
...
Функция УстаревшиеПредставленияДокументов()
	...
	ВидыДокументовКонтрагента.Вставить("АвансовыйОтчет",                        НСтр("ru = 'Приходный кассовый ордер';
																					|en = 'Приходный кассовый ордер'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("ВыкупВозвратнойТарыКлиентом",           НСтр("ru = 'Выкуп возвратной тары';
																					|en = 'Customer invoice — returnable packaging'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("ЗаказКлиента",                          НСтр("ru = 'Заказ';
																					|en = 'Order'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("ЗаявкаНаВозвратТоваровОтКлиента",       НСтр("ru = 'Заказ (возврат)';
																					|en = 'Order (return)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("ОперацияПоПлатежнойКарте",              НСтр("ru = 'Эквайринговая операция';
																					|en = 'Acquiring transaction'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ОтчетКомиссионера",                     НСтр("ru = 'Отчет комитенту';
																					|en = 'Report to consignor'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("ПриобретениеУслугПрочихАктивов",        НСтр("ru = 'Акт';
																					|en = 'Акт'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("РасходныйКассовыйОрдер",                НСтр("ru = 'Расходный кассовый ордер';
																					|en = 'Outgoing payment — Cash account'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("СписаниеБезналичныхДенежныхСредств",    НСтр("ru = 'Платежное поручение';
																					|en = 'Payment order'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("КорректировкаЗадолженности",            НСтр("ru = 'Списание задолженности';
																					|en = 'AR/AP write-off'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("СчетФактураВыданный",                   НСтр("ru = 'Счет-фактура';
																					|en = 'Tax invoice'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("ТаможеннаяДекларацияИмпорт",            НСтр("ru = 'Таможенная декларация';
																					|en = 'Customs declaration'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ НЕ УТ
	...
	ВидыДокументовКонтрагента.Вставить("ЗаказПереработчику",                    НСтр("ru = 'Заказ';
																					|en = 'Order'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ЗаказПереработчику2_5",                 НСтр("ru = 'Заказ';
																					|en = 'Order'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ОтчетПереработчика",                    НСтр("ru = 'Отчет переработчика';
																					|en = 'Отчет переработчика'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- Устарело_Переработка24
	...
	ВидыДокументовКонтрагента.Вставить("ОтчетДавальцу",                         НСтр("ru = 'Отчет переработчика';
																					|en = 'Отчет переработчика'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ЗаказДавальца2_5",							НСтр("ru = 'Заказ';
																							|en = 'Order'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ОтчетДавальцу2_5",							НСтр("ru = 'Отчет переработчика';
																							|en = 'Отчет переработчика'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТКА
...
Функция ПредставленияВладельцевДокументов()
	...
	ВидыДокументовКонтрагента.Вставить("АвансовыйОтчет",                        НСтр("ru = 'получатель';
																					|en = 'получатель'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("ОперацияПоПлатежнойКарте",              НСтр("ru = 'плательщик';
																					|en = 'payer'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("ЗаказПереработчику",                    НСтр("ru = 'от';
																					|en = 'dated'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ЗаказПереработчику2_5",                 НСтр("ru = 'от';
																					|en = 'dated'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ОтчетПереработчика",                    НСтр("ru = 'от';
																					|en = 'dated'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ОтчетПереработчика2_5",                 НСтр("ru = 'от';
																					|en = 'dated'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("ОтчетДавальцу",                         НСтр("ru = 'от';
																					|en = 'dated'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ЗаказДавальца2_5",							НСтр("ru = 'от';
																							|en = 'dated'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ОтчетДавальцу2_5",							НСтр("ru = 'от';
																							|en = 'dated'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТКА
...
Функция ИменаМетаданныхВходящихДокументов()
	...
	МассивДокументовКонтрагента.Добавить("ЗаказДавальца2_5");
	//-- Устарело_Переработка24
...
Функция ПредставлениеДетализацииВзаиморасчетов(Партнер, Договор, РасшифровкаПоПартнерам, РасшифровкаПоДоговорам, РасшифровкаПоЗаказам) Экспорт
	...
	Если ЗначениеЗаполнено(Партнер) Тогда
		Если РасшифровкаПоЗаказам Тогда
			ТекстДетализацииВзаиморасчетов = НСтр("ru = 'Расшифровка по объектам расчетов;';
													|en = 'Drill-down by settlement objects;'");
		КонецЕсли;
	...
	ИначеЕсли РасшифровкаПоПартнерам И РасшифровкаПоЗаказам Тогда
		ТекстДетализацииВзаиморасчетов = НСтр("ru = 'Расшифровка по партнерам и объектам расчетов';
												|en = 'Drill-down by partners and settlement objects'");
