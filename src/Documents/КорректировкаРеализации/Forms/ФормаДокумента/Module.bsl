...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТКА
	СформироватьЗаголовокИУстановитьВидимостьЭлементовПоОтчетуДавальца();
	//-- НЕ УТКА
	...
	//++ НЕ УТКА
	ЭтоКорректировкаОтчетаДавальцу =  Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5;
	Элементы.ГруппаСостояниеЭДО.Видимость = Элементы.ГруппаСостояниеЭДО.Видимость И Не ЭтоКорректировкаОтчетаДавальцу;
	Элементы.ПодменюЭДО.Видимость = Элементы.ПодменюЭДО.Видимость И Не ЭтоКорректировкаОтчетаДавальцу;
	//-- НЕ УТКА
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		...
		//++ НЕ УТКА
		ИЛИ ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.ОтчетДавальцу2_5")
		//-- НЕ УТКА
		Тогда
...
Процедура ПредставлениеДокументаОснованияНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		...
		//++ НЕ УТКА
		Иначе
			СписокТипов.Добавить(Тип("ДокументСсылка.ОтчетДавальцу2_5"));
		//-- НЕ УТКА
		КонецЕсли;
...
Процедура ПредставлениеДокументаОснованияНажатиеЗавершение(ВыбранноеЗначение, Параметры) Экспорт
	...
	//++ НЕ УТКА
	ИначеЕсли ВыбранноеЗначение.Значение = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
		ИмяФормыВыбора = "Документ.ОтчетДавальцу2_5.ФормаВыбора";
		СтруктураОтбор.Вставить("ХозяйственнаяОперация", ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5"));
	//-- НЕ УТКА
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.ВидЦены) И НЕ Объект.Согласован Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Для редактирования суммы очистите вид цены';
					|en = 'To edit the amount, clear the price type'"),
				Объект.Ссылка,
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.ВидЦены) И НЕ Объект.Согласован Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Для редактирования суммы с НДС очистите вид цены';
					|en = 'To edit the amount including VAT, clear the price type'"),
				Объект.Ссылка,
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Товары", ТекущаяСтрока.НомерСтроки, "ВидЦены"),);
		КонецЕсли;
...
Процедура ТоварыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Не ОтменаРедактирования Тогда
		...
		//++ НЕ УТКА
		ИначеЕсли ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
		
			Если ЗначениеЗаполнено(ТекущиеДанные.ЗаказКлиента) Тогда
				
				ТекущиеДанные.Назначение   =
					ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(ТекущиеДанные.ЗаказКлиента, "Назначение");
					
			ИначеЕсли ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
					
				ТекущиеДанные.Назначение   = ПолучитьНазначениеДоговора(Объект);
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	//++ НЕ УТКА
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5") Тогда
	...
	Если Элемент.ТекущийЭлемент <> Неопределено
		...
		Если СтрокаТаблицы.КоличествоУпаковок > 0 Тогда
			...
			Если Не СтрокаТаблицы.Сумма = 0
				//++ НЕ УТКА
				И Не Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5")
				//-- НЕ УТКА
				И Истина Тогда
				СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокРеализаций.ОтразитьНаПрочихДоходах"));
			КонецЕсли;
			...
			Если СтрокаТаблицы.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа")
				//++ НЕ УТКА
				Или Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5")
				//-- НЕ УТКА
		...
		ИначеЕсли СтрокаТаблицы.КоличествоУпаковок < 0 Тогда
			...
			Если 
				//++ НЕ УТКА
				Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5") Или
				//-- НЕ УТКА
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5");
	//-- НЕ УТКА
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<при корректировке товара/работы в минус по количеству без списания>';
																|en = '<when adjusting goods/works in minus quantity without write-off>'"));
...
Процедура НалогообложениеНДСПриИзмененииСервер()
	...
	//++ НЕ УТКА
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5 Тогда
		Возврат;
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура ЗаполнитьНалогообложениеНДСПродажи()
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		//++ НЕ УТКА
		ИначеЕсли ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
			ПараметрыЗаполнения = Документы.ОтчетДавальцу2_5.ПараметрыЗаполненияНалогообложенияНДС(Объект.ДокументОснование);
		//-- НЕ УТКА
		КонецЕсли;
...
Функция ПолучитьНазначениеДоговора(Знач Объект)
	
	Если ЗначениеЗаполнено(Объект.Договор) Тогда
		
		ШаблонНазначения   = Документы.ОтчетДавальцу2_5.ШаблонНазначения(Объект.ДокументОснование);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ИспользоватьСоглашенияСКлиентами =
		Истина
		//++ НЕ УТКА
		И НЕ Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5
		//-- НЕ УТКА
	...
	//++ НЕ УТКА
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5 Тогда
		ИспользоватьЗаказыВТабличнойЧасти = 
			ПолучитьФункциональнуюОпцию("ИспользоватьОтчетДавальцуПоНесколькимЗаказам2_5");
		Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
			СтавкаНДС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "СтавкаНДС");
		КонецЕсли;
	Иначе
	//-- НЕ УТКА
...
Процедура УстановитьПредставлениеДокументаОснования()
	...
	ИначеЕсли ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.РеализацияУслугПрочихАктивов") Тогда
		...
		Элементы.ПредставлениеДокументаОснования.ЦветТекста = ЦветаСтиля.ЦветОсобогоТекста;
	//++ НЕ УТКА
	ИначеЕсли ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ОтчетДавальцу2_5")
		Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5 Тогда
		ПредставлениеДокументаОснования = НСтр("ru = 'Отчет давальцу <не выбран>';
												|en = 'Consumption report — Subcontracting services delivered <is not selected>'");
		Элементы.ПредставлениеДокументаОснования.ЦветТекста = ЦветаСтиля.ЦветОсобогоТекста;
	//-- НЕ УТКА
	Иначе
...
Процедура ОбработкаПодбораТоваровИзЗаказа(АдресТоваровВХранилище)
	...
	//++ НЕ УТКА
	ЗаполняемПоОтчетуДавальца = ?(Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5, Истина, Ложь);
	СтавкаТоваровНДС = Справочники.СтавкиНДС.ПустаяСсылка();
	Если ЗаполняемПоОтчетуДавальца
		И ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		СтавкаТоваровНДС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "СтавкаНДС");
	КонецЕсли;
	//-- НЕ УТКА
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		//++ НЕ УТКА
		Если ЗаполняемПоОтчетуДавальца Тогда
			СтруктураПоискаТоваров.ЗаказКлиента = СтрокаТоваров.ЗаказДавальца;
		Иначе
		//-- НЕ УТКА
		...
		//++ НЕ УТКА
		Если ЗаполняемПоОтчетуДавальца Тогда
			СтрокаТЧТовары.СтавкаНДС = СтавкаТоваровНДС;
		КонецЕсли;
		//-- НЕ УТКА
...
Процедура УстановитьВидимостьПоОперации()
	...
	//++ НЕ УТКА
	ИспользоватьСоглашение = ?(Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5, Ложь, Истина);
	//-- НЕ УТКА
...
Процедура УстановитьВариантыОтражения(ИмяТЧ)
	...
	//++ НЕ УТКА
	КорректировкаДавальца = Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5");
	//-- НЕ УТКА
...
Процедура ЗаполнитьЗаказКлиента(ЭтаФорма, ТекущаяСтрока)
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		//++ НЕ УТКА
		Если ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
			ПолеЗаказа = "ЗаказДавальца";
		КонецЕсли;
		//-- НЕ УТКА
...
//++ НЕ УТКА
&НаСервере
Процедура СформироватьЗаголовокИУстановитьВидимостьЭлементовПоОтчетуДавальца()
	
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5 Тогда
		
		Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
			АвтоЗаголовок = Ложь;
			Заголовок = НСтр("ru = 'Корректировка отчета давальцу (создание)';
							|en = 'Adjust ""Consumption report — Subcontracting services delivered"" (create)'");
		Иначе
			Заголовок = "";
		...
		УдаляемыеЭлементыСпискаВыбора = Новый Массив();
		УдаляемыеЭлементыСпискаВыбора.Добавить(Перечисления.ХозяйственныеОперации.РеализацияПерепоставленногоТовара);
		УдаляемыеЭлементыСпискаВыбора.Добавить(Перечисления.ХозяйственныеОперации.ВозвратНедопоставленногоТовара);
		
		Для каждого УдаляемыйЭлемент Из УдаляемыеЭлементыСпискаВыбора Цикл
			
			ЭлементСпискаВыбра = Элементы.Операция.СписокВыбора.НайтиПоЗначению(УдаляемыйЭлемент);
			Если НЕ ЭлементСпискаВыбра = Неопределено Тогда
				Элементы.Операция.СписокВыбора.Удалить(ЭлементСпискаВыбра);
			КонецЕсли;
			
		КонецЦикла;
		
		Элементы.РасхожденияУстановитьВариантыОтражения.Видимость = Ложь;
		Элементы.ТекстНаПрочиеРасходы.Видимость = Ложь;
		Элементы.ТекстНаПрочиеДоходы.Видимость = Ложь;
...
Процедура ОткрытьПодборУслуг()
	...
	Для Каждого СтрокаТЧ Из Объект.Товары Цикл
		...
		Если СтрокаТЧ.КодСтроки <> 0 И ЗначениеЗаполнено(СтрокаТЧ.ЗаказКлиента) Тогда
			МассивКодовСтрок.Добавить(Новый Структура("КодСтроки, ЗаказДавальца", СтрокаТЧ.КодСтроки, СтрокаТЧ.ЗаказКлиента));
		КонецЕсли;
	...
	ПараметрыПодбора.Вставить("Ссылка",					Объект.ДокументОснование);
	ПараметрыПодбора.Вставить("МассивКодовСтрок",		МассивКодовСтрок);
	ПараметрыПодбора.Вставить("Партнер",				Объект.Партнер);
	ПараметрыПодбора.Вставить("Контрагент",				Объект.Контрагент);
	ПараметрыПодбора.Вставить("Договор",				Объект.Договор);
	...
	ПараметрыПодбора.Вставить("СтавкаНДС",              СтавкаНДС);
