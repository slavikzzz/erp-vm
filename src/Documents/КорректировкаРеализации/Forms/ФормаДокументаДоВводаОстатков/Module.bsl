...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		Если ЗначениеЗаполнено(Объект.Договор)
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "ТипДоговора") = Перечисления.ТипыДоговоров.СДавальцем2_5 Тогда
			КорректировкаДавальца = Истина;
		Иначе
		//-- НЕ УТКА
...
Процедура УстановитьВидимостьЭлементовПоОснованию()
	...
	Если КорректировкаДавальца Тогда
		...
		МассивИменЭлементов.Добавить("ТаблицаКорректировкиГруппаОтправитель");
		МассивИменЭлементов.Добавить("ТаблицаКорректировкиСпособОпределенияСебестоимостиСтрокой");
...
Процедура ДобавитьВСтруктуруДействияПриИзмененииКоличества(СтруктураДействий, Форма, Суффикс = "")
	...
	СтруктураПересчетаСуммы = ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(Форма);
	...
	Если ЗначениеЗаполнено(Объект.Договор)
		...
		Если ЭлементТипаКорректировки <> Неопределено Тогда
			ЭлементТипаКорректировки.Представление = НСтр("ru = 'Корректировка услуг по содержанию';
															|en = 'Service adjustment by content'");
		КонецЕсли;
...
Процедура УстановитьЗаголовокФормы()
	...
	ШаблонЗаголовка = НСтр("ru = 'Корректировка реализации до ввода остатков %1';
							|en = 'Sales adjustment before balance entry %1'");
