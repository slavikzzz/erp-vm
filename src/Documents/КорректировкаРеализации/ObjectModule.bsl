...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	//++ НЕ УТКА
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//++ НЕ УТКА
		Если ЗначениеЗаполнено(Договор)
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "ТипДоговора") = Перечисления.ТипыДоговоров.СДавальцем2_5 Тогда
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(ХозяйственнаяОперация)
		И ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("ДокументОснование") Тогда
			Если ТипЗнч(ДанныеЗаполнения.ДокументОснование) = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
				ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5;
			Иначе
	//-- НЕ УТКА
			ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту;
	//++ НЕ УТКА
	...
	Если РезультатЗапроса.Пустой() Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не требуется вводить корректировку реализации на основании документа %1.';
				|en = 'Generating sales adjustment from base document %1 is not required.'"),
			ДокументОснование);
	...
	Если Выборка.ЕстьОшибкиОперация Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для реализации с операцией ""%1"" не требуется вводить корректировку.';
				|en = 'No need to enter a sales adjustment for transaction ""%1"".'"),
...
Процедура ЗаполнитьПоПервичномуДокументу(Знач ДокументОснование, ДанныеЗаполнения)
	...
	Если РезультатЗапроса.Пустой() Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не требуется вводить корректировку реализации на основании документа %1.';
						|en = 'Generating sales adjustment note from base document %1 is not required.'"),
					ДокументОснование);
	...
	Если Выборка.ЕстьОшибкиОперация Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для первичного документа с типом ""%1"" не требуется вводить корректировку.';
						|en = 'Do not enter adjustment for the %1 source document type.'"),
					Выборка.ХозяйственнаяОперацияОснования);
...
Процедура ЗаполнитьПоОтчетуДавальцу(Знач ДокументОснование, ДанныеЗаполнения)
	
	МетаданныеДокументОснование = ДокументОснование.Метаданные();
	
	Если Не ПравоДоступа("Изменение", МетаданныеДокументОснование) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'У пользователя недостаточно прав на корректировку документа ""%1""';
							|en = 'User is not authorized to adjust document ""%1""'"),
						МетаданныеДокументОснование.Синоним);
	...
	|	ДанныеДокумента.Подразделение			КАК Подразделение,
	|	ДанныеДокумента.ЦенаВключаетНДС			КАК ЦенаВключаетНДС,
	|	ДанныеДокумента.ОтчетПоЗаказам			КАК ПродажаПоЗаказам,
	|	ДанныеДокумента.ФормаОплаты				КАК ФормаОплаты,
	|	ДанныеДокумента.ПорядокРасчетов			КАК ПорядокРасчетов,
	|	ДанныеДокумента.ГрафикОплаты			КАК ГрафикОплаты,
	|
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов(ВидыЗапасов, МассивВариантовОтражения, ЗаполнятьВидЗапасовОтгрузки = Ложь, ЗаполнятьСебестоимость = Ложь) Экспорт
	...
	//++ НЕ УТКА
	ЗаполнятьКорВидЗапасов =
		ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5
		И Не МассивВариантовОтражения.Найти(Перечисления.ВариантыОтраженияКорректировокРеализаций.УвеличитьРеализацию) = Неопределено;
		
	Если ЗаполнятьКорВидЗапасов Тогда
		
		РеквизитыДокумента = Новый Структура;
		РеквизитыДокумента.Вставить("Организация",                Организация);
		РеквизитыДокумента.Вставить("ВладелецТовара",             Партнер);
		РеквизитыДокумента.Вставить("Контрагент",                 Контрагент);
		РеквизитыДокумента.Вставить("Соглашение",                 Справочники.СоглашенияСПоставщиками.ПустаяСсылка());
		РеквизитыДокумента.Вставить("Договор",                    Договор);
		РеквизитыДокумента.Вставить("Валюта",                     Справочники.Валюты.ПустаяСсылка());
		РеквизитыДокумента.Вставить("НалогообложениеОрганизации", Перечисления.ТипыНалогообложенияНДС.ПустаяСсылка());
		РеквизитыДокумента.Вставить("ХозяйственнаяОперация",      Перечисления.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья2_5);
		РеквизитыДокумента.Вставить("ТипЗапасов",                 Перечисления.ТипыЗапасов.ПродукцияДавальца);
			
		КорВидЗапасов =
			Справочники.ВидыЗапасов.ВидЗапасовДокумента(
				Организация,
				Перечисления.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья2_5,
				РеквизитыДокумента);
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		//++ НЕ УТ
		СебестоимостьРеглТоваров	= СтрокаТоваров.СебестоимостьРегл;
		СебестоимостьПРТоваров		= СтрокаТоваров.СебестоимостьПР;
		СебестоимостьВРТоваров		= СтрокаТоваров.СебестоимостьВР;
		//-- НЕ УТ
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			Если ЗаполнятьСебестоимость
				...
				//++ НЕ УТ
				НоваяСтрока.СебестоимостьРегл	= ?(Количество = КоличествоТоваров,
														СебестоимостьРеглТоваров,
														Количество * СебестоимостьРеглТоваров / КоличествоТоваров);
				НоваяСтрока.СебестоимостьПР		= ?(Количество = КоличествоТоваров,
														СебестоимостьПРТоваров,
														Количество * СебестоимостьПРТоваров / КоличествоТоваров);
				НоваяСтрока.СебестоимостьВР		= ?(Количество = КоличествоТоваров,
														СебестоимостьВРТоваров,
														Количество * СебестоимостьВРТоваров / КоличествоТоваров);
				//-- НЕ УТ
			...
			Если ЗаполнятьСебестоимость
				...
				//++ НЕ УТ
				СебестоимостьРеглТоваров	= СебестоимостьРеглТоваров - НоваяСтрока.СебестоимостьРегл;
				СебестоимостьПРТоваров		= СебестоимостьПРТоваров - НоваяСтрока.СебестоимостьПР;
				СебестоимостьВРТоваров		= СебестоимостьВРТоваров - НоваяСтрока.СебестоимостьВР;
				//-- НЕ УТ
...
Процедура ЗаполнитьАналитикиУчетаНоменклатурыВТабличныхЧастяхТоваров(ТаблицыДокумента = Неопределено)
	...
	Если ТипЗнч(ДокументОснование) <> Тип("ДокументСсылка.РеализацияУслугПрочихАктивов")
		...
		//++ НЕ УТКА
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5 Тогда
			РеквизитыСклада = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Склад, "ЭтоГруппа, ВыборГруппы");
			Если Не (РеквизитыСклада.ЭтоГруппа
			...
			МестаУчета =
				РегистрыСведений.АналитикаУчетаНоменклатуры.МестаУчета(ДокументОснованиеХозяйственнаяОперация, Склад, Подразделение, Партнер);
		//-- НЕ УТКА
		Иначе
...
Процедура ПроверитьЗаполнениеТаблицыКорректировкаЗадолженности(Отказ)
	...
	ШаблонОшибки	= НСтр("ru = 'В строке ""%1"" сумма уменьшения больше суммы продажи';
							|en = 'In the ""%1"" line the decrease amount is greater than the sales amount'");
...
Процедура ПроверитьЗаполнениеСпособаОпределенияСебестоимости(ДатаОснования, ДатаДокумента, Отказ)
	...
	ШаблонОшибки	= НСтр("ru = 'В строке ""%1"" не указан способ расчета себестоимости.';
							|en = 'The cost calculation method is not specified in line %1.'");
	КлючДанных	= ОбщегоНазначенияУТ.КлючДанныхДляСообщенияПользователю(ЭтотОбъект);
...
Функция ВариантОтраженияПоСтроке(СтрокаРасхождений, ТипНоменклатуры, КорректировкаУслугПрочихАктивов, КорректировкаПрошлогоПериода, КорректировкаДавальца)
	...
	//++ НЕ УТКА	
	ИначеЕсли КорректировкаДавальца Тогда
...
Процедура ЗаполнитьРасхожденияПоРазнице()
	...
	//++ НЕ УТКА
	КорректировкаДавальца = ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5;
	//-- НЕ УТКА
