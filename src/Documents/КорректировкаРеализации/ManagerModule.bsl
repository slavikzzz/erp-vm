...
Функция ПараметрыЗаполненияНалогообложенияНДСПродажи(Объект) Экспорт
	...
	//++ НЕ УТКА
	Если Не ДанныеОбъекта.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5 Тогда
	//-- НЕ УТКА
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Данные.ТипКорректировки = Перечисления.ТипыКорректировки.КорректировкаДоВводаОстатковТоваровИУслуг
		...
		ПостфиксИмениДокумента = НСтр("ru = 'Корректировка реализации до ввода остатков';
										|en = 'Sales adjustment before balance entry'");
	//++ НЕ УТКА
	ИначеЕсли Данные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5 Тогда
		ПостфиксИмениДокумента = НСтр("ru = 'Корректировка отчета давальцу';
...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	ПереопределениеРасчетаПараметров.Вставить("ХозОперацияОснования",
		...
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиенту)
		//++ НЕ УТКА
		|		КОГДА ТИПЗНАЧЕНИЯ(ЕСТЬNULL(ДанныеДокумента.ДокументОснование, НЕОПРЕДЕЛЕНО)) = ТИП(Документ.ОтчетДавальцу2_5)
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.Текст =
		...
		//++ НЕ УТКА
		|		КОГДА ДанныеДокумента.ДокументОснование.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5)
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ОтчетДавальцу2_5)
		//-- НЕ УТКА
...
Функция ТекстЗапросаТаблицаТоварыОрганизаций(Запрос, ТекстыЗапроса, Регистры)
	...
	|ГДЕ
	|	НЕ ТаблицаВидыЗапасов.НаДоходыРасходы
	|	И НЕ ТаблицаВидыЗапасов.КорВидЗапасов = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	//-- НЕ УТКА
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	//++ НЕ УТКА
	|		ИЛИ &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5)
	//-- НЕ УТКА
	|	)
	...
	//++ НЕ УТКА
	|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцуКорректировкаПрошлогоПериода)
	//-- НЕ УТКА
	|		ИНАЧЕ
...
Процедура ИнициализироватьВтВидыЗапасовДокументаОснования(Запрос, ТекстыЗапроса)
	...
	|ИЗ
	|	Документ.ОтчетДавальцу2_5.ВидыЗапасов КАК Т
	|ГДЕ
	|	Т.Ссылка = &ДокументОснование
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	//++ НЕ УТКА
	|		КОГДА Товары.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5)
	...
	|	ВЫБОР
	//++ НЕ УТКА
	|		КОГДА ВидыЗапасов.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	//-- НЕ УТКА
	|		КОГДА ИСТИНА
	|			ТОГДА ВидыЗапасов.ВидЗапасов
...
Функция ТекстЗапросаВтВидыЗапасов() Экспорт
	...
	|	ДанныеОснования.МоментВремени КАК МоментВремени
	|ИЗ
	|	Документ.ОтчетДавальцу2_5 КАК ДанныеОснования
	|ГДЕ
	|	ДанныеОснования.Ссылка = &ОснованиеНеЗаписанногоДокумента	
	//-- НЕ УТКА
	|;
	...
	|	) КАК СуммаБезНДС,
	|	
	|	ВидыЗапасов.Ссылка.СтавкаНДС КАК СтавкаНДС,
	|	
	|	ЕСТЬNULL(
	|		СуммыРегл.СуммаНДСРегл,
	|		ВидыЗапасов.СуммаНДС
	|	) КАК СуммаНДС,
	|	
	|	ВидыЗапасов.ЗаказДавальца КАК ЗаказКлиента,
	|
	|	ВидыЗапасов.СтатьяРасходов КАК СтатьяРасходов,
	|	ВидыЗапасов.АналитикаРасходов КАК АналитикаРасходов,
	|	ВидыЗапасов.СписатьНаРасходы КАК СписатьНаРасходы,
	...
	|
	|ИЗ
	|	Документ.ОтчетДавальцу2_5.ВидыЗапасов КАК ВидыЗапасов
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтСвязанныеДокументы КАК СвязанныеДокументы
	|		ПО ВидыЗапасов.Ссылка = СвязанныеДокументы.Подчиненный
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютахУчета КАК СуммыРегл
	|		ПО ВидыЗапасов.Ссылка = СуммыРегл.Регистратор
...
Функция СформироватьПечатнуюФормуРеестрНомеровГТД(МассивОбъектов, ОбъектыПечати)
	...
	// МассивРезультатов[1] - ВтСвязанныеДокументыКорректировкаПоПервичнымДокументам
	// МассивРезультатов[2] - ВсеВидыЗапасов
	// МассивРезультатов[3] - ВтВидыЗапасов
	// МассивРезультатов[4] - УничтожитьВсеВидыЗапасов
	// МассивРезультатов[5] - ВтТаблицаДокументов.
	РезультатДанныеПечати 		= МассивРезультатов[6]; // РезультатЗапроса
	РезультатПоДокументам 		= МассивРезультатов[7]; // РезультатЗапроса
	РезультатПоДокументамБезГТД	= МассивРезультатов[8]; // РезультатЗапроса
...
Функция ПараметрыВыбораСтатейИАналитик(ПараметрыОпределенияДоступности) Экспорт
	...
	//++ НЕ УТКА
	КорректировкаОтчетаДавальца = ТипЗнч(ПараметрыОпределенияДоступности.ДокументОснование) = Тип("ДокументСсылка.ОтчетДавальцу2_5");
	//-- НЕ УТКА
...
Процедура ЗаполнитьИменаРеквизитовПоОснованию(ДокументОснование,
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
		...
		МассивРеквизитовОперации.Добавить("Расхождения.КодСтроки");
		МассивРеквизитовОперации.Добавить("Расхождения.Склад");
		МассивРеквизитовОперации.Добавить("Расхождения.СтатьяРасходов");
		МассивРеквизитовОперации.Добавить("Расхождения.АналитикаРасходов");
		МассивРеквизитовОперации.Добавить("Расхождения.АналитикаАктивовПассивов");
...
Процедура СформироватьВременнуюТаблицуИсходныхДанных(МенеджерВременныхТаблиц, ДокументОснование, ДокументСсылка = Неопределено) Экспорт
	...
	//++ НЕ УТКА
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
		Период = НачалоМесяца(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "Дата"));
...
Функция ТекстЗапросаПоИсходнымДаннымОтчетаДавальцу()
	...
	|	ТаблицаПродукция.Номенклатура.ТипНоменклатуры                КАК ТипНоменклатуры,
	|	ЗНАЧЕНИЕ(Справочник.НоменклатураКонтрагентов.ПустаяСсылка)   КАК НоменклатураПартнера,
	...
	|	ТаблицаПродукция.Ссылка.СтавкаНДС                            КАК СтавкаНДС,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиДоходов.ПустаяСсылка)  КАК СтатьяДоходов,
	|	НЕОПРЕДЕЛЕНО                                                 КАК АналитикаДоходов,
	|	ТаблицаПродукция.СуммаНДС                                    КАК СуммаНДС,
	|	ТаблицаПродукция.СуммаСНДС                                   КАК СуммаСНДС,
	|	ТаблицаПродукция.КодСтроки                                   КАК КодСтроки,
	|	ТаблицаПродукция.Склад                                       КАК Склад,
	|	ВЫБОР
	|		КОГДА НоменклатураДавальца.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|			ТОГДА ТаблицаПродукция.Ссылка.Подразделение
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|	КОНЕЦ                                                        КАК Подразделение,
	|	ТаблицаПродукция.ЗаказДавальца                               КАК ЗаказКлиента,
	|	ЗНАЧЕНИЕ(Справочник.КлассификаторТНВЭД.ПустаяСсылка)         КАК КодТНВЭД,
	|	ТаблицаПродукция.Цена                                        КАК Цена,
	|	ТаблицаПродукция.ВидЦены                                     КАК ВидЦены,
	|
	|	ТаблицаПродукция.СтатьяРасходов                              КАК СтатьяРасходов,
	|	ТаблицаПродукция.АналитикаРасходов                           КАК АналитикаРасходов,
	|	ТаблицаПродукция.АналитикаАктивовПассивов                    КАК АналитикаАктивовПассивов,
	|	ТаблицаПродукция.СписатьНаРасходы                            КАК СписатьНаРасходы
	|
	|ИЗ
	|	Документ.ОтчетДавальцу2_5.Продукция КАК ТаблицаПродукция
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ ДанныеПоследнейКорректировки КАК ДанныеПоследнейКорректировки
	|	ПО ИСТИНА
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК НоменклатураДавальца
	|	ПО НоменклатураДавальца.Ссылка = ТаблицаПродукция.Номенклатура
	|
	|ГДЕ
	|	ДанныеПоследнейКорректировки.Ссылка ЕСТЬ NULL 
	|	И ТаблицаПродукция.Ссылка = &ДокументОснование
	|";
...
Функция ХозяйственнаяОперацияДокументаОснования(ДокументОснование) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		...
		ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
			Операция = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5;
		//-- НЕ УТКА
		Иначе
...
Процедура СообщитьОшибкиЗаполнения(Выборка, Отказ)
	...
	ШаблонСообщенияОшибкаПересчетаКоличества				= НСтр("ru = 'Обнаружено нулевое %1 при пересчете в единицу хранения в строке %2 списка ""%3""';
																	|en = 'Zero %1 is detected when converting to stock UOM in line %2 of the %3 list'");
	...
	ШаблонСообщенияНеУказанСпособОпределенияСебестоимости	= НСтр("ru = 'Не указан способ определения себестоимости по строке %1 списка ""%2""';
																	|en = 'Cost defining method is not specified for line %1 of the %2 list'");
	...
	ШаблонСообщенияНеЗаполненаКолонкаСуммыВознаграждения	= НСтр("ru = 'Не заполнена колонка ""%1"" в строке %2 списка ""%3"". %4';
																	|en = 'Column %1 in line %2 of the %3 list is not filled. %4'");
	ШаблонСообщенияКоличествоГТДУвеличения                  = НСтр("ru = 'Для товаров с %1 необходимо указать данные %1 уже реализованных товаров, добавив строки с данными до и после корректировки.
			...
			|en = 'For items with %1, specify the %1 data of already sold items by adding rows with data before and after the adjustment.
			|If it did not change, the data before and after the adjustment must be the same.
			|Set the initial quantity to 0 in line %2 of the %3 list, and the new quantity to the amount the invoice is increased to.'");
		...
		Если Выборка.ТипПроверки = "НеУказанаСуммаВзаиморасчетов" Тогда
			ТекстСообщения = СтрШаблон(ШаблонСообщенияНеЗаполненаКолонкаСуммыВознаграждения,
										НСтр("ru = 'Сумма взаиморасчетов до корректировки.';
											|en = 'AR/AP amount before adjustment.'"),
										Выборка.НомерСтроки,
										ПредставлениеТЧ,
										НСтр("ru = 'Для автоматического расчета необходимо очистить сумму взаиморасчета после корректировки.';
											|en = 'To calculate the amount automatically, clear the AR/AP amount after adjustment.'"));
		...
		Если Выборка.ТипПроверки = "НеУказанаСуммаВзаиморасчетовПосле" Тогда
			ТекстСообщения = СтрШаблон(ШаблонСообщенияНеЗаполненаКолонкаСуммыВознаграждения,
										НСтр("ru = 'Сумма взаиморасчетов после корректировки.';
											|en = 'AR/AP amount after adjustment.'"),
										Выборка.НомерСтроки,
										ПредставлениеТЧ,
										НСтр("ru = 'Для автоматического расчета необходимо очистить сумму взаиморасчета до корректировки.';
											|en = 'To calculate the amount automatically, clear the AR/AP amount before adjustment.'"));
...
Процедура СообщитьОшибкиПоЦепочкеКорректировок(Выборка, Отказ)
	...
	ШаблонСообщенияРасходитсяЗначениеКоличество	= НСтр("ru = 'Предупреждение: в поле ""%1"" в строке %2 указано количество отличное от количества %3 после последней корректировки';
															|en = 'Warning: in line %2 of the %1 field, the quantity different from the quantity %3 after the last adjustment is specified'");
	ШаблонСообщенияРасходитсяЗначениеСумма		= НСтр("ru = 'Предупреждение: в поле ""%1"" в строке %2 указана сумма отличная от суммы %3 после последней корректировки';
														|en = 'Warning: in line %2 of the %1 field, the amount different from the amount %3 after the last adjustment is specified'");
	ШаблонСообщенияРасходитсяСтавкаНДС			= НСтр("ru = 'Предупреждение: ставка НДС в строке %1 отличается от ставки НДС %2 после последней корректировки';
														|en = 'Warning: VAT rate in line %1 is different from the VAT rate %2 after the last adjustment'");
...
Процедура ПроверитьУказаниеСуммыИСуммыНДС(Объект, МенеджерВременныхТаблиц, Отказ)
	...
	ШаблонСообщенияОшибкаУказанияСуммы		= НСтр("ru = 'Сумма НДС до корректировки %1 %2 при ставке ""%3"" больше общей суммы до корректировки по строке %4 %5 в строке %6 списка ""%7""';
													|en = 'VAT rate before adjustment %1 %2 at rate %3 is greater than the total amount before the adjustment for line %4 %5 in line %6 of the %7 list'");
	ШаблонСообщенияОшибкаУказанияСуммыПосле	= НСтр("ru = 'Сумма НДС после корректировки %1 %2 при ставке ""%3"" больше общей суммы после корректировки по строке %4 %5 в строке %6 списка ""%7""';
														|en = 'VAT rate after the adjustment %1 %2 at rate %3 is greater than the total amount after the adjustment for line %4 %5 in line %6 of the %7 list'");
