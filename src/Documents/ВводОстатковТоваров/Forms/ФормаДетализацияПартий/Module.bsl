...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковТоваровПереданныхНаКомиссию2_5
		...
		МассивНастроек.Добавить(Перечисления.ТипыПервичныхДокументов.ПриобретениеУПоставщика);
		МассивНастроек.Добавить(Перечисления.ТипыПервичныхДокументов.ВнутренняяНакладная);
		ПараметрВыбора = Новый ПараметрВыбора("Отбор.ТипПервичногоДокумента",Новый ФиксированныйМассив(МассивНастроек));
		МассивПараметров = Новый Массив;
		МассивПараметров.Добавить(ПараметрВыбора);
		Элементы.ДетализацияПартийДокументПоступления.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметров);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы И Модифицированность Тогда
		Отказ = Истина;
		ТекстПредупреждения = НСтр("ru = 'Данные не были перенесены в ввод остатков. После закрытия, сделанные изменения сохранены не будут.';
									|en = 'Data was not transferred to entering balance. Changes will not be saved after the form is closed.'");
		Возврат;
	КонецЕсли;
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru = 'Данные не перенесены в ввод остатков. Перенести?';
							|en = 'Data is not transferred to entering balance. Transfer?'");
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыДетализацияПартий
...
Процедура ПроверитьЗаполнениеТабличнойЧасти(Отказ)
	...
	Для ТекИндекс = 0 По ДетализацияПартий.Количество()-1 Цикл
		...
		Если НЕ ЗначениеЗаполнено(ДетализацияПартий[ТекИндекс].Контрагент) Тогда
			ТекстОшибки = НСтр("ru = 'Не заполнен партнер в строке %НомерСтроки%';
								|en = 'Partner is required in line %НомерСтроки%'");
		...
		Если НЕ ЗначениеЗаполнено(ДетализацияПартий[ТекИндекс].Контрагент) Тогда
			ТекстОшибки = НСтр("ru = 'Не заполнен контрагент в строке %НомерСтроки%';
								|en = 'Counterparty is required in line %НомерСтроки%'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки,"%НомерСтроки%",ТекИндекс+1);
			ОбщегоНазначения.СообщитьПользователю(
				ТекстОшибки,
				,
		...
		Если НЕ ЗначениеЗаполнено(ДетализацияПартий[ТекИндекс].ДокументПоступления) Тогда
			ТекстОшибки = НСтр("ru = 'Не заполнен документ поступления в строке %НомерСтроки%';
								|en = 'Receipt document is required in line %НомерСтроки%'");
			ТекстОшибки = СтрЗаменить( ТекстОшибки,"%НомерСтроки%",ТекИндекс+1);
			ОбщегоНазначения.СообщитьПользователю(
				ТекстОшибки ,
				,
		...
		Если НЕ ЗначениеЗаполнено(ДетализацияПартий[ТекИндекс].ВидЦенности) Тогда
			ТекстОшибки = НСтр("ru = 'Не заполнен Вид ценности в строке %НомерСтроки%';
								|en = 'Asset tax class is required in line %НомерСтроки%'");
			ТекстОшибки = СтрЗаменить( ТекстОшибки,"%НомерСтроки%",ТекИндекс+1);
			ОбщегоНазначения.СообщитьПользователю(
				ТекстОшибки ,
				,
		...
		Если НЕ ЗначениеЗаполнено(ДетализацияПартий[ТекИндекс].СтавкаНДС) Тогда
			ТекстОшибки = НСтр("ru = 'Не заполнен Ставка НДС в строке %НомерСтроки%';
								|en = 'VAT Rate is required in line %НомерСтроки%'");
			ТекстОшибки = СтрЗаменить( ТекстОшибки,"%НомерСтроки%",ТекИндекс+1);
			ОбщегоНазначения.СообщитьПользователю(
				ТекстОшибки ,
				,
		...
		Если ДетализацияПартий[ТекИндекс].СуммаРегл = 0
			...
			ТекстОшибки = НСтр("ru = 'Не заполнены суммовые показатели в строке %НомерСтроки%';
								|en = 'Amount indicators are required in line %НомерСтроки%'");
			ТекстОшибки = СтрЗаменить( ТекстОшибки,"%НомерСтроки%",ТекИндекс+1);
			ОбщегоНазначения.СообщитьПользователю(
				ТекстОшибки ,
				,
	...
	Если ИтогСуммаРегл > СуммаРегл Тогда
		ТекстОшибки = НСтр("ru = 'Сумма поступлений не может превышать стоимость товара';
							|en = 'Receipt amount cannot exceed goods cost'");
		ОбщегоНазначения.СообщитьПользователю(
			ТекстОшибки,
			,
			"ДетализацияПартий",
	...
	Если ИтогНДСРегл > НДСРегл Тогда
		ТекстОшибки = НСтр("ru = 'Сумма НДС поступлений не может превышать итоговую суммы НДС по товару';
							|en = 'The amount of VAT receipts cannot exceed the total VAT amount for goods'");
		ОбщегоНазначения.СообщитьПользователю(
			ТекстОшибки,
			,
			"ДетализацияПартий",
...
Процедура УстановитьТекстДанныеПартии()
	...
	Если НДСРегл <> 0 Тогда
		Шаблон = НСтр("ru = 'По партии ввода остатков будет отражена сумма (регл.) %1 НДС (регл.) %2';
						|en = 'Amount (local) %1 VAT (local) %2 will be recorded by balance input lot'");
	Иначе
		Шаблон = НСтр("ru = 'По партии ввода остатков будет отражена сумма (регл.) %1';
						|en = 'Amount (local) %1 will be recorded by balance input lot'");
	КонецЕсли;
