...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтрШаблон(НСтр("ru = 'Подбор материалов из заказа: %1';
								|en = 'Select materials from order: %1'"), Параметры.ЗаказПереработчику);
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		Если ЗаказНайден Тогда
			...
			Если КурсВалюты.Количество() = 1 Тогда
				ПараметрыТекущегоКурса = РаботаСКурсамиВалютУТ.ПолучитьКурсВалюты(
					РеквизитыЗаказов.Валюта,
					ТекущаяДатаСеанса(),
					Выборка.ВалютаРегламентированногоУчета);
				ПараметрыНовогоКурса = РаботаСКурсамиВалютУТ.ПолучитьКурсВалюты(
					ДанныеОтбора.Валюта,
					ТекущаяДатаСеанса(),
					Выборка.ВалютаРегламентированногоУчета);
				...
				СтрокаТаб.Сумма = РаботаСКурсамиВалютУТ.ПересчитатьПоКурсу(
					СтрокаТаб.Сумма,
					ПараметрыТекущегоКурса,
					ПараметрыНовогоКурса);
			КонецЕсли;
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, 
	...
	|				ТОГДА ЗаказыДвижения.Заказано * (-1)
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику.Услуги КАК ЗаказНаПереработку
	|	ПО ТаблицаОстатки.ЗаказПереработчику = ЗаказНаПереработку.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
