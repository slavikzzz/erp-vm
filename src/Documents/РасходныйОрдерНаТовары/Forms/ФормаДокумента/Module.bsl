	...
	Если ИспользоватьАдресноеХранение Тогда 
		...
		Если ТекущийСтатус = ПредопределенноеЗначение("Перечисление.СтатусыРасходныхОрдеров.КОтбору")
			...
			Если Модифицированность
				...
				ТекстВопроса = НСтр("ru = 'Перед изменением статуса документ будет проведен. Продолжить?';
									|en = 'Document will be posted before changing the status. Continue?'");
		...
		ИначеЕсли ТекущийСтатус = ПредопределенноеЗначение("Перечисление.СтатусыРасходныхОрдеров.Подготовлен")
			 ...
			 ТекстСообщения = НСтр("ru = 'Из статуса ""%Подготовлен%"" ордер можно перевести только в статус ""%КОтбору%"".';
									|en = 'You can switch the note status from ""%Подготовлен%"" to ""%КОтбору%"" only.'");
...
Процедура ТоварыПоРаспоряжениямРаспоряжениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Склад) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Перед выбором распоряжения нужно заполнить склад.';
																|en = 'Fill in a warehouse before selecting a reference.'"),
														Объект.Ссылка,
														"Склад");
		Отказ = Истина;												
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.Номенклатура) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Перед выбором распоряжения нужно заполнить номенклатуру.';
																|en = 'Fill in items before selecting a reference.'"),
														Объект.Ссылка,
														Поле);
		Отказ = Истина;												
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.Характеристика)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Перед выбором распоряжения нужно заполнить характеристику номенклатуры.';
																|en = 'Fill in item variant before selecting a reference.'"),
														Объект.Ссылка,
														Поле);
		Отказ = Истина;												
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.Серия)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Перед выбором распоряжения нужно заполнить серию номенклатуры.';
																|en = 'Fill in item batch before selecting a reference.'"),
														Объект.Ссылка,
														Поле);
		Отказ = Истина;												
	КонецЕсли;
	...
	Если ВыбранноеЗначение <> ПредопределенноеЗначение("Перечисление.ДействияСоСтрокамиОрдеровНаОтгрузку.Отгрузить") Тогда
		Если ТекущаяСтрока.ЭтоУпаковочныйЛист Тогда
			ТекстПредупреждения = НСтр("ru = 'Для строки упаковочного листа допустимо только действие %Отгрузить%. Для выбора другого действия сначала распакуйте товары.';
										|en = 'Only action %Отгрузить% is allowed for the packing list line. To select another action, unpack the goods first.'");	
		...
		ИначеЕсли ЗначениеЗаполнено(ТекущаяСтрока.УпаковочныйЛистРодитель) Тогда
			ТекстВопроса = НСтр("ru = 'При изменении действия со строкой на %Действие% товар будет исключен из упаковочного листа. Продолжить?';
								|en = 'Goods will be removed from the packing list while changing an action with line for %Действие%. Continue?'");
...
Процедура ЗаполнитьПоРаспоряжениям(Команда)
	...
	Если Объект.ОтгружаемыеТовары.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоРаспоряжениюЗавершение", ЭтотОбъект), НСтр("ru = 'Перед заполнением табличная часть будет очищена. Заполнить?';
...
Процедура РазбитьСправочноПоУпаковкам(Команда)
	Если Объект.ОтгружаемыеТовары.Количество() = 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("РазбитьСправочноПоУпаковкамПослеВопроса", ЭтотОбъект), НСтр("ru = 'Табличная часть не заполнена. Заполнить по распоряжению?';
																											|en = 'Table is not filled. Fill in by reference?'"), РежимДиалогаВопрос.ДаНет);
	...
	Иначе
		...
		|en = 'Table will be refilled while executing the operation.
		|Rows with zero quantities will be removed. Continue?'");
...
Процедура ЗаполнитьПоОтобраннымТоварам(Команда)
	...
	Если (Модифицированность Или Не Объект.Проведен) И Объект.ОтгружаемыеТовары.Количество() > 0 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Перед заполнением документ будет проведен, а табличная часть - очищена. Продолжить?';
							|en = 'The table will be cleared. The document will be posted. Do you want to continue?'");
...
Процедура ПеренестиВНовыйОрдер(Команда)
	...
	Если Элементы.ОтгружаемыеТовары.ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Выделите строки, которые нужно перенести в другой ордер.';
									|en = 'Select lines which should be transferred to another note.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОтобразитьИнформациюПоНеотгружаемымТоварам()
	...
	Если КоличествоНеОтгружать > 0 Тогда
		...
		ТекстНадписи = НСтр("ru = 'В зоне отгрузки необходимо оставить %КоличествоПозиций% товаров.';
							|en = '%КоличествоПозиций% of goods should be left in the shipping area.'");
