...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПредусмотренЗалогЗаТару И ВозвратТарыДавальцу Тогда
		ТекстОшибки = НСтр("ru = 'Залог не используется для тары, полученной от давальца.';
							|en = 'Deposit is not used for the package received from material provider.'");
...
Процедура ЗаполнитьДокументНаОснованииПоступлениеСырьяОтДавальца(Знач ДокументОснование)
	...
	ПараметрыЗаполнитьСтавкуНДС = ОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(ЭтотОбъект);
	ПараметрыЗаполнитьСтавкуНДС.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС;
