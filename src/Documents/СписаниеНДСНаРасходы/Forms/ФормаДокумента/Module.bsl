...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ПараметрыНастройкиСчетаУчета = Документы.СписаниеНДСНаРасходы.ПараметрыНастройкиСчетовУчета();
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	//++ НЕ УТ
	ПараметрыНастройкиСчетовУчета = Документы.СписаниеНДСНаРасходы.ПараметрыНастройкиСчетовУчета();
...
Процедура ПоказатьВопросОчисткаЦенности()
	...
	Если Объект.Ценности.Количество() > 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОчисткаЦенностиЗавершение", ЭтотОбъект),
				НСтр("ru = 'Список ""Ценности"" будет очищен. Продолжить?';
					|en = 'The Assets list will be cleared. Continue?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
		ЗаполнитьЦенностиСервер();
	КонецЕсли;
...
Процедура ПоказатьВопросОчисткаАвансыПолученные()
	...
	Если Объект.Ценности.Количество() > 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОчисткаАвансыПолученныеЗавершение", ЭтотОбъект),
				НСтр("ru = 'Список ""Авансы полученные"" будет очищен. Продолжить?';
					|en = 'The ""Prepayments from customer"" list will be cleared. Continue?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
		ЗаполнитьЦенностиСервер();
	КонецЕсли;
