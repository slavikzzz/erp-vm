...
#Область ПланыРазборкиКомплектующие
	...
	ДопПараметр.Вставить("Заголовок", 			НСтр("ru = 'План разборки (комплектующие)';
														|en = 'Reverse kitting plan (components)'"));
	ДопПараметр.Вставить("СоздаватьРеквизит", 	ИспользоватьПланыСборки);
	ДопПараметр.Вставить("УдалятьРеквизитыТаблицы", Истина);
...
#Область ФактыРазборкиКомплекты
	...
	ДопПараметр.Вставить("Заголовок", 			?(СоздаватьРеквизитСОтбором, НСтр("ru = 'Факт разборки (комплекты)*';
																					|en = 'Actual reverse kitting (kits)*'"), 
...
#Область ФактыРазборкиКомплектующие
	...
	ДопПараметр.Вставить("Заголовок", 			?(СоздаватьРеквизитСОтбором, НСтр("ru = 'Факт разборки (комплектующие)*';
																					|en = 'Actual reverse kitting (components)*'"), 
...
//
// Параметры:
//  ДокументСсылка - ДокументСсылка.ПланСборкиРазборки - Ссылка на документ.
...
Функция ТекстЗапросаТаблицаОборотыБюджетов(Запрос, ТекстыЗапроса, Регистры)
	...
	БюджетированиеСервер.УстановитьВЗапросеВыраженияЗаполненияАналитики(
		Запрос.Параметры.Ссылка, ТекстЗапроса, Запрос.Параметры.СтатьяБюджетов, Документы.ПланСборкиРазборки, "Аналитика");
...
Функция СформироватьПечатнуюФормуПлана(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ВыводитьШапку Тогда
			...
			Если ДанныеПечати.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СборкаТоваров Тогда
				ШаблонЗагловка = НСтр("ru = 'План сборки';
										|en = 'Kitting plan'", ОбщегоНазначения.КодОсновногоЯзыка());
			Иначе
				ШаблонЗагловка = НСтр("ru = 'План разборки';
										|en = 'Reverse kitting plan'", ОбщегоНазначения.КодОсновногоЯзыка());
			КонецЕсли; 
		...
		СтруктураДопПоля.Вставить("Заголовок", НСтр("ru = 'Комплектация';
													|en = 'Product kit'", ОбщегоНазначения.КодОсновногоЯзыка()));
