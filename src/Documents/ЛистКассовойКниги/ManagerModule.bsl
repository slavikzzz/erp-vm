...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'Титульный лист и последняя страница';
										|en = 'Cover page and the last page'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Обложка") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru = 'Титульный лист кассовой книги';
				|en = 'Cover page of the cash book'"),
...
Функция СформироватьПечатнуюФормуЛистаКассовойКниги(МассивОбъектов, ОбъектыПечати, ПараметрыПечати = Неопределено)
	...
	Пока ВыборкаПоДокументамКассоваяКнига.Следующий() Цикл
		...
		|		КОНЕЦ
		|	КОНЕЦ КАК Счет,
		|	КассовыеОрдера.Валюта КАК Валюта,
		|	ВЫБОР КОГДА КассовыеОрдера.Документ ССЫЛКА Документ.ПриходныйКассовыйОрдер ТОГДА
		|		0
		|	ИНАЧЕ 1
		|	КОНЕЦ КАК Признак
		|ИЗ
		...
		|	И НЕ КассовыеОрдера.Документ ССЫЛКА Документ.ИнвентаризацияНаличныхДенежныхСредств
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	КассовыеОрдера.Документ КАК Документ,
		|	Хозрасчетный.СчетКт.Код КАК Счет,
		|	КассовыеОрдера.Валюта КАК Валюта,
		|	0
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный КАК Хозрасчетный
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЛистКассовойКниги.КассовыеОрдера КАК КассовыеОрдера
		|		ПО (КассовыеОрдера.Документ = Хозрасчетный.Регистратор)
		|ГДЕ
		|	КассовыеОрдера.Ссылка = &ЛистКассовойКниги
		|	И КассовыеОрдера.Документ ССЫЛКА Документ.ИнвентаризацияНаличныхДенежныхСредств
		|	И КассовыеОрдера.Приход > 0
		|	И Хозрасчетный.СчетДт В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Касса))
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	КассовыеОрдера.Документ,
		|	Хозрасчетный.СчетДт.Код,
		|	КассовыеОрдера.Валюта,
		|	1
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный КАК Хозрасчетный
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЛистКассовойКниги.КассовыеОрдера КАК КассовыеОрдера
			...
			Если ВыборкаДокументовПоДням.День = ДатаЛиста Тогда
				...
				Пока ВыборкаНомеровСтрок.Следующий() Цикл
					...
					Пока ВыборкаДокументов.Следующий() Цикл
						...
						Если ВыборкаДокументов.Расход > 0 Тогда
							Признак = 1;
						КонецЕсли;
						
						СтруктураПоиска = Новый Структура("Документ, Валюта, Признак");
						СтруктураПоиска.Документ = ВыборкаДокументов.Документ;
						СтруктураПоиска.Валюта = ВыборкаДокументов.Валюта;
						СтруктураПоиска.Признак = Признак;
