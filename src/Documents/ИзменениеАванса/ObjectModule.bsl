...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ЗарплатаКадры.ПроверитьКорректностьМесяца(Ссылка, МесяцИзменения, "Объект.МесяцИзменения", Отказ, НСтр("ru = 'Месяц изменения';
	...
	Если МесяцИзменения > ПериодОкончания
		...
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Период окончания не может быть меньше периода начала изменения аванса';
													|en = 'End period cannot be less than advance change start period'"), ЭтотОбъект, "ПериодОкончания", ,Отказ);
