...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(Документ) = Тип("ДокументОбъект.ВозвратОСИзАренды") Тогда
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	Если ПравоДоступа("Добавление", Метаданные.Документы.ВозвратОСИзАренды) Тогда
		КомандаСоздатьНаОсновании = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Менеджер = Метаданные.Документы.ВозвратОСИзАренды.ПолноеИмя();
		КомандаСоздатьНаОсновании.Представление = ОбщегоНазначенияУТ.ПредставлениеОбъекта(Метаданные.Документы.ВозвратОСИзАренды);
...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	ПолноеИмяДокумента = "Документ.ВозвратОСИзАренды";
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка, ДопПараметры)
	...
	|ИЗ
	|	Документ.ВозвратОСИзАренды КАК ДанныеДокумента
	|ГДЕ
...
Процедура ПолучитьДанныеДокумента(Запрос)
	...
	|
	|ИЗ
	|	Документ.ВозвратОСИзАренды.ОС КАК ТаблицаДокумента
	|
	|ГДЕ
...
Функция ЗначенияПараметровПроведения(Реквизиты = Неопределено)
	...
	ЗначенияПараметровПроведения.Вставить("ИдентификаторМетаданных", ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Документ.ВозвратОСИзАренды"));
...
Процедура ТекстЗапросаТаблицаМестонахождениеОС(ТекстыЗапроса, Регистры)
	...
	|	втМестонахождениеОС.МОЛ КАК МОЛ,
	|	&ПодразделениеПолучатель КАК Местонахождение,
	|	втМестонахождениеОС.АдресМестонахождения КАК АдресМестонахождения
	|ИЗ
	|	Документ.ВозвратОСИзАренды.ОС КАК ТаблицаОС
...
Процедура ТекстЗапросаТаблицаПорядокУчетаОС(ТекстыЗапроса, Регистры)
	...
	|ИЗ
	|	ВтПорядокУчетаОС КАК ТаблицаОС";
...
Функция ТекстЗапросаТаблицаДокументыПоОС(Запрос, ТекстыЗапроса, Регистры)
	...
	|ИЗ
	|	Документ.ВозвратОСИзАренды КАК ДанныеДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратОСИзАренды.ОС КАК ТаблицаОС
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	ТекстыВременныхТаблиц.Добавить(УчетОСВызовСервера.ТекстЗапросаВТОтраженияВРеглУчетеНачисленнойАмортизации("ВозвратОСИзАренды"));
	ТекстыВременныхТаблиц.Добавить(УчетОСВызовСервера.ТекстЗапросаВТОтраженияВРеглУчетеОстаткиПоСчетам());
	ТекстыВременныхТаблиц.Добавить(ТекстЗапросаВТСчетаЗабалансовогоУчета());
...
Функция ТекстЗапросаВТСчетаЗабалансовогоУчета()
	...
	|	ЕСТЬNULL(СчетаОтражения.СчетЗабалансовогоУчета, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)) КАК СчетУчета
	...
	|						ИЗ
	|							Документ.ВозвратОСИзАренды.ОС КАК ТабличнаяЧасть
	|						ГДЕ
...
Функция ПеремещениеСчетУчета()
	...
	|ИЗ
	|	Документ.ВозвратОСИзАренды КАК Операция
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратОСИзАренды.ОС КАК Строки
...
Функция ЗабалансовыйУчетАрендованныхОС()
	...
	|				ИЗ
	|					Документ.ВозвратОСИзАренды.ОС КАК ТабличнаяЧастьОС
	|				ГДЕ
...
Функция ПолучитьДанныеДляПечатнойФормыОС1(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|ИЗ
	|	Документ.ВозвратОСИзАренды КАК Операция
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратОСИзАренды.ОС КАК ТабличнаяЧасть
	...
	|	ВЫБОР КОГДА втОбщиеСведенияОбъектовДокументов.КоличествоРазличныхМОЛ=1 ТОГДА втОбщиеСведенияОбъектовДокументов.МОЛ ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ КАК ПринялМОЛ,
	|	НЕОПРЕДЕЛЕНО КАК ДолжностьПринялМОЛ
	|ИЗ
	|	Документ.ВозвратОСИзАренды КАК Шапка
