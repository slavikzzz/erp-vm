...
Функция ТекстОтраженияВРеглУчете() Экспорт
	Возврат РаспределениеНДСЛокализация.ТекстОтраженияВРеглУчете();
КонецФункции
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	Возврат РаспределениеНДСЛокализация.ТекстЗапросаВТОтраженияВРеглУчете();
КонецФункции
...
Функция НеобходимоРаспределениеНДСПоТоварам(Организация, Знач Период) Экспорт
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Периодичность распределения НДС ""%1"" не поддерживается.';
										|en = 'Frequency of VAT allocation ""%1"" is not provided.'"), Строка(ПериодичностьРаспределенияНДС));
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл 
		...
		ТекстРегистрацииОшибки = СтрЗаменить(
			НСтр("ru = 'При %операция% по организации ""%1"" за период %2 были диагностированы ошибки';
				|en = 'Errors were found during %операция% for the ""%1"" company for period %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
			"%операция%",
			?(ИдетРасчетСебестоимости, НСтр("ru = 'распределении НДС';
											|en = 'VAT allocation'", ОбщегоНазначения.КодОсновногоЯзыка()), НСтр("ru = 'выполнении расчета';
																													|en = 'calculation'", ОбщегоНазначения.КодОсновногоЯзыка())));
...
Функция ПолучитьДанныеДокумента(ДокументСсылка) Экспорт
	...
	//++ НЕ УТ
	Реквизиты.Вставить("РаспределятьНДСВМесяцеОсуществленияКапВложений",
			?(НЕ ПараметрыУчетнойПолитикиНДС = Неопределено,
				ПараметрыУчетнойПолитикиНДС.РаспределятьНДСВМесяцеОсуществленияКапВложений,
				Ложь));
	
	ИспользоватьВНА2_4 = ВнеоборотныеАктивы.ИспользуетсяУправлениеВНА_2_4(Реквизиты.Период);
	Реквизиты.Вставить("ИспользоватьВНА2_4", ИспользоватьВНА2_4);
	//-- НЕ УТ
...
Функция ДанныеДляРаспределения(Реквизиты, ДопПараметры)
	...
	Запрос.УстановитьПараметр("РаспределятьНДСВМесяцеОсуществленияКапВложений",
		Реквизиты.РаспределятьНДСВМесяцеОсуществленияКапВложений);
	//-- НЕ УТ
	...
	//++ НЕ УТ
	РаспределениеНДСЛокализация.ИнициализироватьВтВнеоборотныеАктивы(Запрос.МенеджерВременныхТаблиц, Реквизиты);
	...
	|	И ТаблицаОстатков.НалогообложениеНДСПартии <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ВводОСВЭксплуатацию)
	|	И &РаспределениеЗаПолныйПериод
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	
	|	КОНЕЦПЕРИОДА(ВнеобАктивы.ДатаПринятияКУчету, МЕСЯЦ) КАК ПериодБазы,
	|	ЕСТЬNULL(ПартииНДСКРаспределениюКорректировка.ОснованиеКорректировки, НЕОПРЕДЕЛЕНО) КАК ОснованиеКорректировки,
	|	
	|	ВЫБОР 
	...
	|	НЕОПРЕДЕЛЕНО                                     КАК НовоеНалогообложениеНДС,
	|	ВЫБОР
	|		КОГДА ТаблицаОстатков.ВидЦенностиНДССтатьиРасходов = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ОС)
	|			ТОГДА ""ОС""
	|		ИНАЧЕ ""НМА""
	|	КОНЕЦ                                            КАК ВнеоборотныйАктив,
	|	
	...
	|	
	|ГДЕ
	|	ВнеобАктивы.Объект ЕСТЬ NULL
	|	И &РаспределятьНДСВМесяцеОсуществленияКапВложений
	|	И ТаблицаОстатков.НалогообложениеНДСПартии = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ВводОСВЭксплуатацию)
	|	
	|ОБЪЕДИНИТЬ ВСЕ 
	|
	...
	|		КОГДА ТаблицаОстатков.ВидЦенностиНДССтатьиРасходов = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ОС)
	|			ТОГДА ""ОС""
	|		КОГДА ТаблицаОстатков.ВидЦенностиНДССтатьиРасходов = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.НМА)
	|			ТОГДА ""НМА""
	|		ИНАЧЕ ""ОбъектСтроительства""
	|	КОНЕЦ                                            КАК ВнеоборотныйАктив,
	|	ЛОЖЬ                                             КАК ВключаетсяВРасходыНУПриПринятииКУчету,
	|	НЕОПРЕДЕЛЕНО                                     КАК ПодразделениеРасходовНУ,
	|	НЕОПРЕДЕЛЕНО                                     КАК СтатьяРасходовНУ,
	...
	|ГДЕ
	|	ТаблицаОстатков.НалогообложениеНДСПартии <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг)
	|	И ТаблицаОстатков.ВариантРаспределенияРасходовСтатьи = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	...
	|		ИЛИ НЕ &ИспользоватьРаздельныйУчетВНАПоНалогообложению
...
Функция СформироватьДвижения(Реквизиты, Регистры, ДопПараметры)
	...
	Если ВыполнятьРаспреление Тогда
		...
		//++ НЕ УТ
		Если Реквизиты.ИспользоватьВНА2_4 И Реквизиты.РаспределятьНДСВМесяцеОсуществленияКапВложений Тогда
			ТаблицыДляДвижений.Вставить("Таблица" + "ДокументыПоОС", ТаблицаДокументыПоОС(ДанныеДляРаспределения.ПартииНДСКРаспределению, Реквизиты));
			ТаблицыДляДвижений.Вставить("Таблица" + "ДокументыПоНМА", ТаблицаДокументыПоНМА(ДанныеДляРаспределения.ПартииНДСКРаспределению, Реквизиты));
		КонецЕсли;
		//-- НЕ УТ
			...
			Если Таблица = Неопределено Тогда
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Неопределено налогообложение по внеоборотному активу %1.';
						|en = 'Taxation by fixed asset %1 is undefined.'"), 
...
Функция ТаблицаПартииНДСКРаспределению(РезультатРаспределения, Реквизиты)
	...
	Если Реквизиты.ИспользоватьВНА2_4
		И Реквизиты.РаспределятьНДСВМесяцеОсуществленияКапВложений Тогда
			Запрос = Новый Запрос;
			Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ДокументыПоОС.ОсновноеСредство КАК ОсновноеСредство,
			|	ДокументыПоОС.Организация КАК Организация,
			|	ПринятиеКУчетуОС2_4.НалогообложениеНДС КАК НалогообложениеНДС
			|ПОМЕСТИТЬ ВНАРаспределениеНДСВПрошлыхПериодах
			|ИЗ
			|	РегистрСведений.ДокументыПоОС КАК ДокументыПоОС
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПринятиеКУчетуОС2_4 КАК ПринятиеКУчетуОС2_4
			|		ПО (ДокументыПоОС.Ссылка = ПринятиеКУчетуОС2_4.Ссылка)
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ДокументыПоОС КАК ДокументыПоОСПредыдущиеРаспределения
			|		ПО (ДокументыПоОС.Организация = ДокументыПоОСПредыдущиеРаспределения.Организация)
			|			И (ДокументыПоОС.ОсновноеСредство = ДокументыПоОСПредыдущиеРаспределения.ОсновноеСредство)
			|ГДЕ
			|	ДокументыПоОС.Дата МЕЖДУ &ПериодНачало И &ПериодОкончание
			|	И ДокументыПоОС.Организация = &Организация
			|	И ДокументыПоОСПредыдущиеРаспределения.ТипСсылки = &ТипСсылки
			|	И ДокументыПоОСПредыдущиеРаспределения.Дата < &ПериодНачало
			|	И ПринятиеКУчетуОС2_4.Проведен
			|	И ПринятиеКУчетуОС2_4.ОтражатьВБУ
			|	И ПринятиеКУчетуОС2_4.ВариантРаздельногоУчетаНДС = &ВариантРаздельногоУчетаНДС
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ДокументыПоНМА.НематериальныйАктив КАК ОсновноеСредство,
			|	ДокументыПоНМА.Организация КАК Организация,
			|	ПринятиеКУчетуНМА2_4.НалогообложениеНДС КАК НалогообложениеНДС
			|ИЗ
			|	РегистрСведений.ДокументыПоНМА КАК ДокументыПоНМА
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПринятиеКУчетуНМА2_4 КАК ПринятиеКУчетуНМА2_4
			|		ПО (ДокументыПоНМА.Ссылка = ПринятиеКУчетуНМА2_4.Ссылка)
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ДокументыПоНМА КАК ДокументыПоНМАПредыдущиеРаспределения
			|		ПО (ДокументыПоНМА.Организация = ДокументыПоНМАПредыдущиеРаспределения.Организация)
			|			И (ДокументыПоНМА.НематериальныйАктив = ДокументыПоНМАПредыдущиеРаспределения.НематериальныйАктив)
			|ГДЕ
			|	ДокументыПоНМА.Дата МЕЖДУ &ПериодНачало И &ПериодОкончание
			|	И ДокументыПоНМА.Организация = &Организация
			|	И ДокументыПоНМАПредыдущиеРаспределения.ТипСсылки = &ТипСсылки
			|	И ДокументыПоНМАПредыдущиеРаспределения.Дата < &ПериодНачало
			|	И ПринятиеКУчетуНМА2_4.Проведен
			|	И ПринятиеКУчетуНМА2_4.ОтражатьВРеглУчете
			|	И ПринятиеКУчетуНМА2_4.ВариантРаздельногоУчетаНДС = &ВариантРаздельногоУчетаНДС
			|ИНДЕКСИРОВАТЬ ПО
			|	Организация,
			|	ОсновноеСредство,
			|	НалогообложениеНДС
			...
			|	ПартииНДСКРаспределению.Поставщик КАК Поставщик,
			|	ПартииНДСКРаспределению.ДокументПоступленияРасходов КАК ДокументПоступленияРасходов,
			|	ПартииНДСКРаспределению.КорВидДеятельностиНДС КАК ВидДеятельностиНДС,
			|	ПартииНДСКРаспределению.ВидЦенности КАК ВидЦенности,
			|	ПартииНДСКРаспределению.СтавкаНДС КАК СтавкаНДС,
			|	ПартииНДСКРаспределению.СтоимостьРегл КАК СтоимостьРегл,
			|	ПартииНДСКРаспределению.НДСРегл КАК НДСРегл,
			|	ПартииНДСКРаспределению.НДСУпр КАК НДСУпр,
			|	ВНАРаспределениеНДСВПрошлыхПериодах.НалогообложениеНДС КАК КорВидДеятельностиНДС,
			|	ПартииНДСКРаспределению.СтатьяОтраженияРасходов КАК СтатьяОтраженияРасходов,
			|	ПартииНДСКРаспределению.АналитикаОтраженияРасходов КАК АналитикаОтраженияРасходов
			|ПОМЕСТИТЬ ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода
			|ИЗ
			|	РегистрНакопления.ПартииНДСКРаспределению КАК ПартииНДСКРаспределению
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВНАРаспределениеНДСВПрошлыхПериодах КАК ВНАРаспределениеНДСВПрошлыхПериодах
			|		ПО ПартииНДСКРаспределению.Организация = ВНАРаспределениеНДСВПрошлыхПериодах.Организация
			|			И ПартииНДСКРаспределению.АналитикаРасходов = ВНАРаспределениеНДСВПрошлыхПериодах.ОсновноеСредство
			|			И ПартииНДСКРаспределению.КорВидДеятельностиНДС <> ВНАРаспределениеНДСВПрошлыхПериодах.НалогообложениеНДС
			|			И ПартииНДСКРаспределению.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
			|			И ПартииНДСКРаспределению.Период < &ПериодНачало
			|			И ПартииНДСКРаспределению.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ВводОСВЭксплуатацию)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|	&ПериодБазы КАК ПериодБазы,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.Организация КАК Организация,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.Подразделение КАК Подразделение,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.НаправлениеДеятельности КАК НаправлениеДеятельности,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.СтатьяРасходов КАК СтатьяРасходов,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.АналитикаРасходов КАК АналитикаРасходов,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.АналитикаАктивовПассивов КАК АналитикаАктивовПассивов,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.Поставщик КАК Поставщик,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.ДокументПоступленияРасходов КАК ДокументПоступленияРасходов,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.ВидДеятельностиНДС КАК ВидДеятельностиНДС,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.ВидЦенности КАК ВидЦенности,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.СтавкаНДС КАК СтавкаНДС,
			|	-ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.СтоимостьРегл КАК СтоимостьРегл,
			|	-ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.НДСРегл КАК НДСРегл,
			|	-ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.НДСУпр КАК НДСУпр,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.СтатьяОтраженияРасходов КАК СтатьяОтраженияРасходов,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.АналитикаОтраженияРасходов КАК АналитикаОтраженияРасходов,
			|	НЕОПРЕДЕЛЕНО КАК КорВидДеятельностиНДС,
			|	НЕОПРЕДЕЛЕНО КАК ИдентификаторФинЗаписи,
			|	НЕОПРЕДЕЛЕНО КАК НастройкаХозяйственнойОперации
			|ИЗ
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода КАК ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			...
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.СтатьяОтраженияРасходов КАК СтатьяОтраженияРасходов,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.АналитикаОтраженияРасходов КАК АналитикаОтраженияРасходов,
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода.КорВидДеятельностиНДС КАК КорВидДеятельностиНДС,
			|	НЕОПРЕДЕЛЕНО КАК ИдентификаторФинЗаписи,
			|	НЕОПРЕДЕЛЕНО КАК НастройкаХозяйственнойОперации
			|ИЗ
			|	ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода КАК ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода";
				
			Запрос.УстановитьПараметр("ТипСсылки", ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Тип("ДокументСсылка.РаспределениеНДС")));
			Запрос.УстановитьПараметр("Организация",  Реквизиты.Организация);
			Запрос.УстановитьПараметр("Период",       Реквизиты.Период);
			Запрос.УстановитьПараметр("ПериодБазы",   Реквизиты.ПериодБазы);
			Запрос.УстановитьПараметр("ПериодНачало", НачалоМесяца(Реквизиты.ПериодОкончание));
			Запрос.УстановитьПараметр("ПериодОкончание", Реквизиты.ПериодОкончание);
			Запрос.УстановитьПараметр("ВариантРаздельногоУчетаНДС", Перечисления.ВариантыРаздельногоУчетаНДС.ИзДокумента);
			
			ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода = Запрос.Выполнить().Выгрузить();
			
			Для каждого Строка Из ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода Цикл
				Строка.ИдентификаторФинЗаписи = Новый УникальныйИдентификатор();
				Если ВидыДеятельностиНДСПринимаетсяКВозмещению.Найти(Строка.ВидДеятельностиНДС) <> Неопределено
					И ВидыДеятельностиНДСУчитываетсяВСтоимости.Найти(Строка.КорВидДеятельностиНДС) <> Неопределено Тогда
					Строка.НастройкаХозяйственнойОперации = Справочники.НастройкиХозяйственныхОпераций.СписаниеНДСПоПриобретеннымЦенностям;
			...
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ПартииНДСКРаспределениюКорректировкаРаспределенияПрошлогоПериода, ПартииНДСКРаспределению);
		КонецЕсли;
	//-- НЕ УТ
...
Функция ТаблицаПрочиеРасходы(РезультатРаспределения, Реквизиты, ПартииНДСКРаспределению)
	...
	Если Реквизиты.ИспользоватьВНА2_4
		...
		Для Каждого Строка Из ПартииНДСКРаспределению Цикл
			Если Строка.НДСРегл < 0 И Строка.КорВидДеятельностиНДС = Перечисления.ТипыНалогообложенияНДС.ПустаяСсылка() Тогда
				...
				НоваяСтрока.НастройкаХозяйственнойОперации = Справочники.НастройкиХозяйственныхОпераций.СписаниеНДСПоПриобретеннымЦенностям;
				Если Строка.ВидДеятельностиНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС Тогда
...
// Функция возвращает записи регистра сведений ДокументыПоОС,
// необходимых для последующем возможном перерасчете при принятии к учету ОС
//
// Возвращаемое значение:
//	ТаблицаЗначений - Записи регистра сведений ДокументыПоОС.
//
Функция ТаблицаДокументыПоОС(ПартииНДСКРаспределению, Реквизиты)
	
	ДокументыПоОС = РегистрыСведений.ДокументыПоОС.СоздатьНаборЗаписей().ВыгрузитьКолонки();

	Для каждого Строка Из ПартииНДСКРаспределению Цикл
		
		Если Строка.ВнеоборотныйАктив <> "ОС" Тогда
			Продолжить;
		КонецЕсли;
		
		НоваяСтрока = ДокументыПоОС.Добавить();
		НоваяСтрока.Дата					= Реквизиты.Период;
		НоваяСтрока.ТипСсылки				= ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Документ.РаспределениеНДС");
		НоваяСтрока.Организация				= Реквизиты.Организация;
		НоваяСтрока.Подразделение			= Строка.Подразделение;
		НоваяСтрока.ОсновноеСредство		= ?(ЗначениеЗаполнено(Строка.АналитикаРасходов), Строка.АналитикаРасходов, Строка.АналитикаАктивовПассивов);
		НоваяСтрока.Ссылка					= Реквизиты.Ссылка;
		НоваяСтрока.ХозяйственнаяОперация	= Перечисления.ХозяйственныеОперации.РаспределениеНДС;
		
	КонецЦикла;
	
	ДокументыПоОС.Свернуть("Дата, ТипСсылки, Организация, Подразделение, ОсновноеСредство, Ссылка, ХозяйственнаяОперация");
	
	Возврат ДокументыПоОС;
	
КонецФункции

// Функция возвращает записи регистра сведений ДокументыПоНМС,
// необходимых для последующем возможном перерасчете при принятии к учету НМА
//
// Возвращаемое значение:
//	ТаблицаЗначений - Записи регистра сведений ДокументыПоНМА.
//
Функция ТаблицаДокументыПоНМА(ПартииНДСКРаспределению, Реквизиты)
	
	ДокументыПоНМА = РегистрыСведений.ДокументыПоНМА.СоздатьНаборЗаписей().ВыгрузитьКолонки();

	Для каждого Строка Из ПартииНДСКРаспределению Цикл
		
		Если Строка.ВнеоборотныйАктив <> "НМА" Тогда
			Продолжить;
		КонецЕсли;
		
		НоваяСтрока = ДокументыПоНМА.Добавить();
		НоваяСтрока.Дата					= Реквизиты.Период;
		НоваяСтрока.ТипСсылки				= ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Документ.РаспределениеНДС");
		НоваяСтрока.Организация				= Реквизиты.Организация;
		НоваяСтрока.Подразделение			= Строка.Подразделение;
		НоваяСтрока.НематериальныйАктив		= ?(ЗначениеЗаполнено(Строка.АналитикаРасходов), Строка.АналитикаРасходов, Строка.АналитикаАктивовПассивов);
		НоваяСтрока.Ссылка					= Реквизиты.Ссылка;
		НоваяСтрока.ХозяйственнаяОперация	= Перечисления.ХозяйственныеОперации.РаспределениеНДС;
		
	КонецЦикла;
	
	ДокументыПоНМА.Свернуть("Дата, ТипСсылки, Организация, Подразделение, НематериальныйАктив, Ссылка, ХозяйственнаяОперация");
	
	Возврат ДокументыПоНМА;
	...
	Если ДанныеВыручки.ВыручкаПоНалогообложениямНДС = Неопределено Тогда
		...
		Иначе
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Не определен период получения базы НДС';
											|en = 'Period of getting VAT base is not defined.'"));
		КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось обработать документ: %Ссылка% по причине: %Причина%';
									|en = 'Cannot process the document: %Ссылка%. Reason: %Причина%'");
