...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		УстановитьПараметрыВыборочнойРегистрацииКОтражениюВРеглУчете(ЭтотОбъект);
	КонецЕсли;
	//-- НЕ УТ
...
Процедура ЗаполнитьПоПлатежномуДокументу(РеквизитыЗаполнения)
	...
	Если РеквизитыЗаполнения.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ОплатаПоставщику
		...
		ТекстОшибки = НСтр("ru = 'Не требуется вводить счет-фактуру на основании документа %Документ%';
							|en = 'It is not required to enter a tax invoice based on document %Документ%'");
			...
			Текст = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для документа %1 по организации %2 и договору %3 уже введен счет-фактура %4';
						|en = 'Tax invoice %4 is already entered for document %1 by company %2 under contract %3'"),
					ДокументОснование,
					Организация,
					Договор,
					Выборка.Ссылка);
...
Процедура УстановитьПараметрыВыборочнойРегистрацииКОтражениюВРеглУчете(Объект)
	...
	ОбщегоНазначенияУТ.ИзмененияДокумента(Объект);
