	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		ИначеЕсли НЕ ОтветПередЗаписью И Объект.СуммаДокумента <> СуммаЭтаповОплаты Тогда
			...
			ТекстВопроса = НСтр("ru = 'Сумма этапов графика оплаты не совпадает с суммой документа. Скорректировать сумму этапов оплаты?';
								|en = 'Amount of payment milestones is not corresponding to the document amount. Adjust payment milestones amount?'");
...
Процедура ЧастичнаяОплатаПриИзменении(Элемент)
	...
	Если Не Объект.ЧастичнаяОплата И ЗначениеЗаполнено(ОБъект.ДокументОснование) Тогда
		Если Объект.ЭтапыГрафикаОплаты.Количество() > 0 Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ЧастичнаяОплатаПриИзмененииЗавершение", ЭтотОбъект), НСтр("ru = 'Таблица этапов оплаты будет заполнена по основанию. Продолжить?';
																												|en = 'Payment milestone table will be filled in by basis. Continue?'"), РежимДиалогаВопрос.ДаНет);
            Возврат;
		КонецЕсли;
...
Процедура УправлениеЭлементамиФормы()
	...
	Если ТипЗнч(Объект.ДокументОснование) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
	 ...
	 //-- Устарело_Переработка24
	 Или ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ОтчетДавальцу2_5")
	 Или ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ЗаказДавальца2_5")
	 //-- НЕ УТКА
	 Тогда
