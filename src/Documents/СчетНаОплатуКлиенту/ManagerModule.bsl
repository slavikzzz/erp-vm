...
Функция ДобавитьКомандуСоздатьНаОснованииСчетаНаОплатуПредоплатаПоДоговору(КомандыСозданияНаОсновании) Экспорт
	Если ПравоДоступа("Добавление", Метаданные.Документы.СчетНаОплатуКлиенту) Тогда 
		...
		КомандаСоздатьНаОсновании.Представление = НСтр("ru = 'Счет на оплату (предоплата)';
														|en = 'Commercial invoice (prepayment)'");
		КомандаСоздатьНаОсновании.РежимЗаписи = "Проводить";
		КомандаСоздатьНаОсновании.ФункциональныеОпции = "ИспользоватьСчетаНаОплатуКлиентам";
...
Функция ПолучитьСуммуДокументаОснования(ДокументОснование) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		...
		ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
			...
			|";	
		//-- Устарело_Переработка24
		ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ЗаказДавальца2_5") Тогда
			ТекстЗапроса = "
			|ВЫБРАТЬ
			|	ДокументПродажи.СуммаДокумента КАК СуммаДокумента
			|ИЗ
			|	Документ.ЗаказДавальца2_5 КАК ДокументПродажи
			|ГДЕ
			|	ДокументПродажи.Ссылка = &Ссылка
			|";	
		//++ Устарело_Переработка24
		...
		ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
			...
			|	ДокументПродажи.СуммаДокумента КАК СуммаДокумента
			|ИЗ
			|	Документ.ОтчетДавальцу2_5 КАК ДокументПродажи
			|ГДЕ
			|	ДокументПродажи.Ссылка = &Ссылка
			|";
		//-- НЕ УТКА
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Запись счетов на оплату';
				|en = 'Save commercial invoices'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
