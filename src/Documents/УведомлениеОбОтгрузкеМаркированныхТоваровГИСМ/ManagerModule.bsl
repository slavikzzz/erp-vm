...
Функция ТекстЗапросаУведомленияОбОтгрузке() Экспорт
	...
	ИнтеграцияГИСМПереопределяемый.ТекстЗапросаУведомленияОбОтгрузке(ТекстЗапросаУведомленияОбОтгрузке);
	Возврат ТекстЗапросаУведомленияОбОтгрузке;
...
Функция ТекстЗапросаУведомленияОбОтгрузкеОформите() Экспорт
	...
	ИнтеграцияГИСМПереопределяемый.ТекстЗапросаУведомленияОбОтгрузкеОформите(ТекстЗапросаУведомленияОбОтгрузкеОформите);
	Возврат ТекстЗапросаУведомленияОбОтгрузкеОформите;
...
Функция УведомлениеОбОтгрузкеМаркированныхТоваровXML2_40(ДокументСсылка)
	...
	Если Шапка.ВидОборотаТовара <> Перечисления.ВидОборотаТовараГИСМ.ЭкспортВСтраныЕАЭС Тогда
		...
		Попытка
			УведомлениеОбОтгрузкеМаркированныхТоваров.sender_gln = РеквизитыОрганизации.GLN;
		Исключение
		...
		Исключение
			СообщениеXML.ТекстОшибки = НСтр("ru = 'Не заполнен номер документа-основания.';
											|en = 'Не заполнен номер документа-основания.'");
		КонецПопытки;
		...
		Исключение
			СообщениеXML.ТекстОшибки = НСтр("ru = 'Не заполнена дата документа-основания.';
											|en = 'Не заполнена дата документа-основания.'");
		КонецПопытки;
	...
	Иначе
		...
		Попытка
			УведомлениеОбЭкспорте.sender_gln = РеквизитыОрганизации.GLN;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибкиНеЗаполненGLNОрганизации(СообщениеXML, РеквизитыОрганизации.GLN, Шапка);
		КонецПопытки;
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Для контрагента %1 не заполнена страна регистрации.';
																						|en = 'Для контрагента %1 не заполнена страна регистрации.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder.org_name = Шапка.НаименованиеПолное;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Для контрагента %1 не заполнено полное наименование.';
																						|en = 'Для контрагента %1 не заполнено полное наименование.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder.org_number = Шапка.РегистрационныйНомер;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Для контрагента %1 не заполнен регистрационный номер.';
																						|en = 'Для контрагента %1 не заполнен регистрационный номер.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder.org_address = ЮридическийАдрес;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Для контрагента %1 не заполнен юридический адрес.';
																						|en = 'Для контрагента %1 не заполнен юридический адрес.'"), Шапка.Контрагент));
		КонецПопытки;
		...
		Исключение
			СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ru = 'В документе %1 не заполнен номер документа.';
														|en = 'В документе %1 не заполнен номер документа.'"), Шапка.Основание);
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.export_doc_date = Шапка.ДатаДокументаОснования;
		Исключение
			СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ru = 'В документе %1 не заполнена дата документа.';
														|en = 'В документе %1 не заполнена дата документа.'"), , Шапка.Основание);
		КонецПопытки;
...
Функция УведомлениеОбОтгрузкеМаркированныхТоваровXML2_41(ДокументСсылка)
	...
	Если Шапка.ВидОборотаТовара <> Перечисления.ВидОборотаТовараГИСМ.ЭкспортВСтраныЕАЭС
		...
		Если ИнтеграцияГИСМ.ДокументОснованиеВозвратПоставщику(Шапка.Основание)
			И РеквизитыКонтрагента.ЭтоРозничныйПокупатель
			...
			Попытка
				УведомлениеОбОтгрузке.sender_gln = РеквизитыОрганизации.GLN;
			Исключение
				ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибкиНеЗаполненGLNОрганизации(СообщениеXML, РеквизитыОрганизации.GLN, Шапка);
			КонецПопытки;
		...
		Иначе
			...
			Если Не Шапка.КонтрагентНеЗарегистрированВГИСМ Тогда
				...
				Если ЗначениеЗаполнено(РеквизитыОрганизации.КПП) Тогда
					Попытка
						УведомлениеОбОтгрузкеМаркированныхТоваров.sender_kpp = РеквизитыОрганизации.КПП;
					Исключение
			...
			Иначе
				...
				НаименованиеКонтрагента = РеквизитыКонтрагента.НаименованиеПолное;
				Если ПустаяСтрока(НаименованиеКонтрагента) Тогда
					НаименованиеКонтрагента = РеквизитыКонтрагента.Наименование;
				КонецЕсли;
				
				Попытка
					subject.organisation_name = НаименованиеКонтрагента;
				Исключение
				...
				Если РеквизитыКонтрагента.ЭтоФизическоеЛицо Тогда
					...
					Исключение
						СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ru = 'У контрагента %1 не правильно заполнено ИНН.';
																	|en = 'У контрагента %1 не правильно заполнено ИНН.'"), Шапка.Контрагент);
					КонецПопытки;
				...
				Иначе
					...
					Если ЗначениеЗаполнено(РеквизитыКонтрагента.КПП) Тогда
						...
						Исключение
							СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ru = 'У контрагента %1 не правильно заполнено КПП.';
																		|en = 'У контрагента %1 не правильно заполнено КПП.'"), Шапка.Контрагент);
						КонецПопытки;
					КонецЕсли;
	...
	ИначеЕсли Шапка.ВидОборотаТовара = Перечисления.ВидОборотаТовараГИСМ.ЭкспортВСтраныЕАЭС Тогда
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Для контрагента %1 указана неверно или не заполнена страна регистрации.';
																						|en = 'Для контрагента %1 указана неверно или не заполнена страна регистрации.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder_info.org_name = РеквизитыКонтрагента.НаименованиеПолное;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Для контрагента %1 указан неверно или не заполнено полное наименование.';
																						|en = 'Для контрагента %1 указан неверно или не заполнено полное наименование.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder_info.org_number = РеквизитыКонтрагента.РегистрационныйНомер;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Для контрагента %1 указан неверно или не заполнен регистрационный номер.';
																						|en = 'Для контрагента %1 указан неверно или не заполнен регистрационный номер.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder_info.org_address = РеквизитыКонтрагента.ЮридическийАдрес;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Для контрагента %1 указан неверно или не заполнен юридический адрес.';
																						|en = 'Для контрагента %1 указан неверно или не заполнен юридический адрес.'"), Шапка.Контрагент));
		КонецПопытки;
