...
Функция ПечатьБланкаПриемкиТоваров(МассивОбъектов, ОбъектыПечати)
	...
	СинонимДокумента = НСтр("ru = 'Бланк приемки товаров';
							|en = 'Goods receipt form'", ОбщегоНазначения.КодОсновногоЯзыка());
			...
			Если ВыборкаПоСтрокамТЧ.СтатусУказанияСерий <> 0 Тогда
				...
				Если ВыборкаПоСтрокамТЧ.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваров Тогда
					Если ВыборкаПоСтрокамТЧ.ИспользоватьНомерСерии
						...
						НадписьСерии = НСтр("ru = 'Серии (номер, годен до, количество):';
											|en = 'Batch (number, expiration date, quantity):'", ОбщегоНазначения.КодОсновногоЯзыка());
					ИначеЕсли ВыборкаПоСтрокамТЧ.ИспользоватьНомерСерии Тогда
						НадписьСерии = НСтр("ru = 'Серии (номер, количество):';
											|en = 'Batch (number, quantity):'", ОбщегоНазначения.КодОсновногоЯзыка());
					Иначе
						НадписьСерии = НСтр("ru = 'Серии (годен до, количество):';
											|en = 'Batch (expiration date, quantity):'", ОбщегоНазначения.КодОсновногоЯзыка());
					КонецЕсли;
				ИначеЕсли ВыборкаПоСтрокамТЧ.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ЭкземплярТовара Тогда
					НадписьСерии = НСтр("ru = 'Серийные номера:';
										|en = 'Serial numbers:'", ОбщегоНазначения.КодОсновногоЯзыка());	
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(ВыборкаПоСтрокамТЧ.Серия) Тогда
					ПредставлениеСерии = НСтр("ru = '%1, %2 %3';
												|en = '%1, %2 %3'", ОбщегоНазначения.КодОсновногоЯзыка());
			...
			Если СкладыСервер.ИспользоватьАдресноеХранение(Объект.Склад,Объект.Помещение,Объект.Дата)
				...
				Если ВыдаватьОшибки Тогда
					ТекстСообщения = НСтр("ru = 'На складе ""%Склад%"" ведется учет остатков в разрезе складских ячеек, поэтому серии нужно указывать в разрезе упаковок. На складе-отправителе учет в разрезе складских ячеек не ведется, поэтому серии учитываются без разреза упаковок. Заполнить серии по данным отгрузки нельзя.';
											|en = 'Remaining quantity is accounted by storage bins in warehouse ""%Склад%"", that is why batch should be specified by packaging units. Accounting by storage bins in the origin warehouse is not kept, that is why batches are not recorded by packaging units. You cannot fill in the batch by shipment data.'");
				...
				Если Не Запрос.Выполнить().Пустой() Тогда
					Если ВыдаватьОшибки Тогда
						ТекстСообщения = НСтр("ru = 'На складе ""%Склад%"" ведется учет остатков в разрезе складских ячеек, поэтому серии нужно указывать в разрезе упаковок. Отгрузка производилась из помещений склада-отправителя, на которых учет в разрезе складския ячеек не ведется, поэтому серии учитываются без разреза упаковок. Заполнить серии по данным отгрузки нельзя.';
												|en = 'Remaining quantity is accounted by storage bins in warehouse ""%Склад%"", that is why batch should be specified by packaging units. Shipment was performed from the origin warehouse warerooms where accounting by storage bins is not kept, that is why batches are not recorded by packaging units. You cannot fill in the batch by shipment data.'");
						ТекстСообщения = СтрЗаменить(ТекстСообщения,"%Склад%", СкладыСервер.ПолучитьПредставлениеСклада(Объект.Склад, Объект.Помещение)); 
						ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
					КонецЕсли;
					Возврат Ложь;	
		...
		Если СкладыСервер.ИспользоватьАдресноеХранение(Объект.Склад,Объект.Помещение,Объект.Дата)
			...
			Если ВыдаватьОшибки Тогда
				ТекстСообщения = НСтр("ru = 'На складе ""%Склад%"" ведется учет остатков в разрезе складских ячеек, поэтому серии нужно указывать в разрезе упаковок. В производстве серии заполняются без учета упаковок. Заполнить серии по данным выпуска продукции нельзя.';
										|en = 'Remaining quantity is accounted by storage bins in warehouse ""%Склад%"", that is why batch should be specified by packaging units. During manufacturing batches are filled in without recording packaging units. You cannot fill in the batch by product release data.'");
...
//++ НЕ УТ
	Если Ложь Тогда
	//++ Устарело_Производство21	
	ИначеЕсли ТипЗнч(Объект.Распоряжение) = Тип("ДокументСсылка.ВыпускПродукции") Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ВложенныйЗапрос.Серия КАК Серия,
		|	СУММА(ВложенныйЗапрос.Количество) КАК Количество,
	...
	ИначеЕсли ТипЗнч(Объект.Распоряжение) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
		...
		|	СУММА(ВложенныйЗапрос.Количество) > 0";
	//-- Устарело_Производство21
	ИначеЕсли ТипЗнч(Объект.Распоряжение) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ВложенныйЗапрос.Серия КАК Серия,
		|	СУММА(ВложенныйЗапрос.Количество) КАК Количество,
		...
		|	(ВЫБРАТЬ
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(ТаблицаСерииВыходныеИзделия.Серия, ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|				ТОГДА ТаблицаСерииВыходныеИзделия.Серия
		|			ИНАЧЕ ВыходныеИзделия.Серия
		|		КОНЕЦ КАК Серия,
		|		Максимум(ВЫБОР
		|				КОГДА ЕСТЬNULL(ТаблицаСерииВыходныеИзделия.Серия, ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|					ТОГДА ТаблицаСерииВыходныеИзделия.Количество
		|				ИНАЧЕ ВыходныеИзделия.Количество
		|			КОНЕЦ) КАК Количество,
		|		ВыходныеИзделия.Номенклатура КАК Номенклатура,
		|		ВыходныеИзделия.Характеристика КАК Характеристика,
		|		ВЫБОР
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ВыходныеИзделияСерии КАК ТаблицаСерииВыходныеИзделия
		|			ПО ВыходныеИзделия.Ссылка = ТаблицаСерииВыходныеИзделия.Ссылка
		|				И ВыходныеИзделия.Получатель = ТаблицаСерииВыходныеИзделия.Получатель
		|				И ВыходныеИзделия.Номенклатура = ТаблицаСерииВыходныеИзделия.Номенклатура
		|				И ВыходныеИзделия.Характеристика = ТаблицаСерииВыходныеИзделия.Характеристика
		|				И ВыходныеИзделия.Назначение = ТаблицаСерииВыходныеИзделия.Назначение
		|				И ВыходныеИзделия.Произведено = ТаблицаСерииВыходныеИзделия.Произведено
		|				И ВыходныеИзделия.ДатаПроизводства = ТаблицаСерииВыходныеИзделия.ДатаПроизводства
		|	ГДЕ
		|		ВыходныеИзделия.Ссылка = &Распоряжение
		|		И ВыходныеИзделия.Получатель = &Склад
		|		И ВыходныеИзделия.Ссылка.Проведен
		...
		|	ВЫБРАТЬ
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(ТаблицаСерииПобочныеИзделия.Серия, ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|				ТОГДА ТаблицаСерииПобочныеИзделия.Серия
		|			ИНАЧЕ ПобочныеИзделия.Серия
		|		КОНЕЦ,
		|		Максимум(ВЫБОР
		|				КОГДА ЕСТЬNULL(ТаблицаСерииПобочныеИзделия.Серия, ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|					ТОГДА ТаблицаСерииПобочныеИзделия.Количество
		|				ИНАЧЕ ПобочныеИзделия.Количество
		|			КОНЕЦ),
		|		ПобочныеИзделия.Номенклатура,
		|		ПобочныеИзделия.Характеристика,
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(ПобочныеИзделия.Назначение.ДвиженияПоСкладскимРегистрам, ЛОЖЬ)
		|				ТОГДА ПобочныеИзделия.Назначение
		|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|		КОНЕЦ,
		|		ПобочныеИзделия.Упаковка
		|	ИЗ
		|		Документ.ЭтапПроизводства2_2.ПобочныеИзделия КАК ПобочныеИзделия
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ПобочныеИзделияСерии КАК ТаблицаСерииПобочныеИзделия
		|			ПО ПобочныеИзделия.Ссылка = ТаблицаСерииПобочныеИзделия.Ссылка
		|				И ПобочныеИзделия.Получатель = ТаблицаСерииПобочныеИзделия.Получатель
		|				И ПобочныеИзделия.Номенклатура = ТаблицаСерииПобочныеИзделия.Номенклатура
		|				И ПобочныеИзделия.Характеристика = ТаблицаСерииПобочныеИзделия.Характеристика
		|				И ПобочныеИзделия.Назначение = ТаблицаСерииПобочныеИзделия.Назначение
		|				И ПобочныеИзделия.Произведено = ТаблицаСерииПобочныеИзделия.Произведено
		|				И ПобочныеИзделия.ДатаПроизводства = ТаблицаСерииПобочныеИзделия.ДатаПроизводства
		|				И ПобочныеИзделия.ВладелецИзделия = ТаблицаСерииПобочныеИзделия.ВладелецИзделия
		|	ГДЕ
		|		ПобочныеИзделия.Ссылка = &Распоряжение
		|		И ПобочныеИзделия.Получатель = &Склад
		...
		|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|		КОНЕЦ,
		|		ПобочныеИзделия.Номенклатура) КАК ВложенныйЗапрос
		|	
		|		СГРУППИРОВАТЬ ПО
		|		ВложенныйЗапрос.Серия,
		|		ВложенныйЗапрос.Номенклатура,
		...
		|	СУММА(ВложенныйЗапрос.Количество) > 0";
		//-- НЕ УТКА
	ИначеЕсли ТипЗнч(Объект.Распоряжение) = Тип("ДокументСсылка.ДвижениеПродукцииИМатериалов") Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ВложенныйЗапрос.Серия КАК Серия,
		|	СУММА(ВложенныйЗапрос.Количество) КАК Количество,
		...
		|		КОНЕЦ КАК Назначение
		|	ИЗ
		|		Документ.ДвижениеПродукцииИМатериалов.Товары КАК ВыходныеИзделия
		|	ГДЕ
		|		ВыходныеИзделия.Ссылка = &Распоряжение
		|		И ВыходныеИзделия.Ссылка.Получатель = &Склад
		|		И ВыходныеИзделия.Ссылка.Проведен
		|		И ВыходныеИзделия.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		...
		|	ГДЕ
		|		ТаблицаСерии.Ссылка = &Распоряжение
		|		И ТаблицаСерии.Ссылка.Получатель = &Склад
		|		И ТаблицаСерии.Ссылка.Проведен
		|		И ТаблицаСерии.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		...
		|ИМЕЮЩИЕ
		|	СУММА(ВложенныйЗапрос.Количество) > 0";
	ИначеЕсли ТипЗнч(Объект.Распоряжение) = Тип("ДокументСсылка.ПроизводствоБезЗаказа") Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ВложенныйЗапрос.Серия КАК Серия,
		|	СУММА(ВложенныйЗапрос.Количество) КАК Количество,
		...
		|	ГДЕ
		|		ТаблицаСерии.Ссылка = &Распоряжение
		|		И ТаблицаСерии.Получатель = &Склад
		|		И ТаблицаСерии.Ссылка.Проведен
		|		И ТаблицаСерии.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		...
		|		КОНЕЦ КАК Назначение
		|	ИЗ
		|		Документ.ПроизводствоБезЗаказа.ПобочныеИзделия КАК ВыходныеИзделия
		|	ГДЕ
		|		ВыходныеИзделия.Ссылка = &Распоряжение
		|		И ВыходныеИзделия.Получатель = &Склад
		|		И ВыходныеИзделия.Ссылка.Проведен
		...
		|ИМЕЮЩИЕ
		|	СУММА(ВложенныйЗапрос.Количество) > 0";
	ИначеЕсли ТипЗнч(Объект.Распоряжение) = Тип("ДокументСсылка.РаспределениеВозвратныхОтходов") Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ВложенныйЗапрос.Серия КАК Серия,
		|	СУММА(ВложенныйЗапрос.Количество) КАК Количество,
		...
		|		КОНЕЦ КАК Назначение
		|	ИЗ
		|		Документ.РаспределениеВозвратныхОтходов КАК ВыходныеИзделия
		|	ГДЕ
		|		ВыходныеИзделия.Ссылка = &Распоряжение
		|		И ВыходныеИзделия.Получатель = &Склад
		|		И ВыходныеИзделия.Ссылка.Проведен
		...
		|	СУММА(ВложенныйЗапрос.Количество) > 0";
	Иначе
		ТекстИсключения = НСтр("ru = 'Тип распоряжения не поддерживается.';
								|en = 'Reference type is not supported.'");
