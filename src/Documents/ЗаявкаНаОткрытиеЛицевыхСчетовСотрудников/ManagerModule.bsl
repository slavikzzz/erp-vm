...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'Список на открытие лицевых счетов';
										|en = 'List for personal account opening'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СписокНаОткрытиеЛицевыхСчетов") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "СписокНаОткрытиеЛицевыхСчетов", НСтр("ru = 'Список на открытие лицевых счетов';
																															|en = 'List for personal account opening'"), ТабличныйДокумент);
	КонецЕсли;
...
Процедура ВыгрузитьФайлыОбменаСБанком(СтруктураПараметровДляФормированияФайла) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Если Не Выборка.Проведен Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Документ %1 не проведен.';
										|en = 'Document %1 is not posted.'"), Выборка.Заявка);
			...
			Иначе
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'В документе %1 не выбран зарплатный проект.';
						|en = 'Payroll card program is not selected in document %1.'"), 
					Выборка.Заявка);
			КонецЕсли;
