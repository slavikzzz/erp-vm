...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Отбор.Свойство("Сотрудник") Тогда
		Если ТипЗнч(Параметры.Отбор.Сотрудник) = Тип("СправочникСсылка.Сотрудники")
			И ЗначениеЗаполнено(Параметры.Отбор.Сотрудник)
	...
	Если Параметры.Отбор.Свойство("ФизическоеЛицо") Тогда
		Если ЗначениеЗаполнено(Параметры.Отбор.ФизическоеЛицо) И Не ЗначениеЗаполнено(ФизическоеЛицо) Тогда
	...
	Иначе
		...
		Элементы.НадписьСообщенияОжидаемыеОтФСС.Заголовок = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
			НСтр("ru = ';Ожидается %1 ответ на запрос ФСС для расчета пособия;;Ожидается %1 ответа на запросы ФСС для расчета пособий;Ожидается %1 ответов на запросы ФСС для расчета пособий;';
				|en = ';Waiting for %1 response to the SSF request for allowance calculation;;Waiting for %1 responses to the SSF requests for allowance calculation;Waiting for %1 responses to the SSF requests for allowance calculation;'"),
