...
Функция ПроверитьЗавершениеРасчетов() Экспорт
	...
	|en = 'There is an outstanding payment due for this order: 
	|%СуммаКОплате% %Валюта%.
	|To close the order, pay the outstanding amount or cancel the outstanding items.'");
	...
	Если ТипДанныхЗаполнения = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("Основание") Тогда
			...
			ИначеЕсли ТипОснования = Тип("ДокументСсылка.ЗаказПереработчику2_5") Тогда
				...
				ЗаполнитьДокументНаОснованииЗаказаПереработчику2_5(ДанныеЗаполнения);
				//-- НЕ УТ
	...
	Если Не ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для номенклатуры в строке %1, закупаемой по агентской схеме, можно использовать только операцию ""Закупка у поставщика""';
					|en = 'Only the ""Purchase from vendor"" transaction can be used for the items in line %1 purchased on agency basis'"),
				НомерСтроки);
	...
	Если ВалютаДоговораНазначения <> Валюта Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для номенклатуры в строке %1, закупаемой по агентской схеме, валюта, указанная в договоре назначения, должна совпадать с валютой документа';
				|en = 'The currency specified in the assignment contract must match the document currency for the items in line %1 purchased on agency basis'"),
			НомерСтроки);
	...
	Если ОрганизацияДоговораНазначения <> Организация Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для номенклатуры в строке %1, закупаемой по агентской схеме, организация, указанная в договоре назначения, должна совпадать с организацией документа';
				|en = 'The company specified in the assignment contract must match the document company for the items in line %1 purchased on agency basis'"),
			НомерСтроки);
...
Процедура ЗаполнитьДокументНаОснованииЗаказаМатериаловВПроизводство(Знач ДанныеЗаполнения)
	
	ЗаказМатериалов = ДанныеЗаполнения.Основание;
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	Заказ.Приоритет КАК Приоритет,
		|	Заказ.Организация КАК Организация,
...
Процедура ЗаполнитьДокументНаОснованииЗаказаПереработчику(Знач ДанныеЗаполнения)
	
	ЗаказПереработчику = ДанныеЗаполнения.Основание;
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		...
		|	ВЫБОР
		|		КОГДА Таблица.НаправлениеДеятельности.УчетЗатрат
		|				ИЛИ Таблица.НаправлениеДеятельности.УчетРасчетовСПоставщиками
...
Процедура ЗаполнитьДокументНаОснованииЗаказаПереработчику2_5(Знач ДанныеЗаполнения)
	...
	Запрос = Новый Запрос(
		...
		|	Таблица.Организация КАК Организация,
		|	Таблица.Подразделение КАК Подразделение,
		|	Таблица.СкладМатериалов КАК СкладДокумента,
		|	ЕСТЬNULL(Таблица.СкладМатериалов.ЭтоГруппа, ЛОЖЬ) КАК ЭтоГруппа,
		|	ВЫБОР
		|		КОГДА Таблица.НаправлениеДеятельности.УчетЗатрат
		|				ИЛИ Таблица.НаправлениеДеятельности.УчетРасчетовСПоставщиками
		|			ТОГДА Таблица.НаправлениеДеятельности
...
Процедура ЗаполнитьДокументНаОснованииЭтапаПроизводства(Знач ДанныеЗаполнения)
	
	ЭтапПроизводства = ДанныеЗаполнения.Основание;
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	Таблица.Организация КАК Организация,
		|	Таблица.Подразделение КАК Подразделение,
		...
		|ГДЕ
		|	Таблица.Ссылка = &Заказ");
	Запрос.УстановитьПараметр("Заказ", ЭтапПроизводства);
	
	Реквизиты = Запрос.Выполнить().Выбрать();
	Реквизиты.Следующий();
	
	Организация = Реквизиты.Организация;
	ДокументОснование = ЭтапПроизводства;
	Подразделение = Реквизиты.Подразделение;
	НаправлениеДеятельности = Реквизиты.НаправлениеДеятельности;
		...
		Если Выборка.Следующий() Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Заказ поставщику оплачен. Нельзя устанавливать операцию %1';
					|en = 'Purchase order is paid. Cannot perform the ""%1"" operation'"),
				ХозяйственнаяОперация);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				Текст,
				ЭтотОбъект,
...
Процедура ЗаполнитьДокументНаОснованииИзмененияАссортимента(Знач ИзменениеАссортимента)
	...
	Если НЕ ОснованиеПроведен Тогда
		ТекстИсключения = Нстр("ru = 'Документ-основание %1 не проведен. Заполнение документа не возможно.';
								|en = 'Cannot populate the document. Post its base document first: %1.'");
