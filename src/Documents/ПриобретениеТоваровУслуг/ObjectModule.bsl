...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОтчетПереработчика2_5") Тогда
		ЗаполнитьДокументНаОснованииОтчетаПереработчика2_5(ДанныеЗаполнения);
	//-- НЕ УТ
	КонецЕсли;
	...
	Если Не ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика Тогда
		ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Для номенклатуры %1, закупаемой по агентской схеме, можно использовать только операцию ""Закупка у поставщика""';
					|en = 'Only the ""Purchase from vendor"" transaction can be used for the products %1 purchased by agency.'"),
				АдресОшибки);
	...
	Если ВалютаДоговораНазначения <> Валюта Тогда
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Для номенклатуры %1, закупаемой по агентской схеме, валюта, указанная в договоре назначения, должна совпадать с валютой документа';
				|en = 'The currency specified in the assignment contract must match with the currency of the document to the %1products purchased on agency basis.'"),
			АдресОшибки);
	...
	Если ОрганизацияДоговораНазначения <> Организация Тогда
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Для номенклатуры %1, закупаемой по агентской схеме, организация, указанная в договоре назначения, должна совпадать с организацией документа';
				|en = 'The company specified in the assignment contract must match with the company of the document for the %1 products purchased on the agency basis.'"),
			АдресОшибки);
...
//++ НЕ УТ
Процедура ЗаполнитьДокументНаОснованииОтчетаПереработчика2_5(ДанныеЗаполнения)
	...
	// Заполнение шапки.
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, Шапка, , "ХозяйственнаяОперация");
	
	Если Шапка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчикаВСтранахЕАЭС2_5 Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Поступление оплачено. Нельзя устанавливать операцию %1';
					|en = 'Receipt is paid. Cannot set transaction %1'"),
				ХозяйственнаяОперация);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				Текст,
				ЭтотОбъект,
