...
Процедура УстановитьУсловноеОформление()
	...
	СписокЗначений.Добавить(НСтр("ru = 'Договор купли-продажи';
								|en = 'Purchase and sale contract'"));
	СписокЗначений.Добавить(НСтр("ru = 'Договор комиссии';
								|en = 'Cross-company sales contract'"));
...
Процедура УстановитьВидимость()
	...
	//++ Локализация
	Элементы.РасчетыПоРеализацииОтложенныйДоходНУ.Видимость = Объект.ОтражатьВБУиНУ;
	ДатаДокумента = ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, ТекущаяДатаСеанса());
	Если ДатаДокумента >= РегистрыСведений.НастройкиУчетаНалогаНаПрибыль.НачалоПрименения67ФЗПоРасходам() Тогда
		Элементы.РасчетыПоРеализацииОтложенныйРасходНУ.Видимость = Объект.ОтражатьВБУиНУ;
	КонецЕсли;
	//-- Локализация
...
Процедура ЗаполнитьСпискиВыбораТипаРасчетов()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковРасчетовМеждуОрганизациямиПоРеализациям Тогда
		...
		СписокВыбора.Добавить(Документы.ОтчетПоКомиссииМеждуОрганизациями.ПустаяСсылка(), НСтр("ru = 'Отчет по комиссии между организациями';
																								|en = 'Commission sales report — Intercompany'"));
