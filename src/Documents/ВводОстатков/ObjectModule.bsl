...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Иначе
		...
		// Очистить не используемые реквизиты строк с инвентарным учетом в табличной части "ТМЦ в эксплуатации"
		СвойстваКатегорий = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ТМЦВЭксплуатации.ВыгрузитьКолонку("КатегорияЭксплуатации"), "ИнвентарныйУчет, СпособПогашенияСтоимостиБУ");
		СвойстваКатегорий.Вставить(
			Справочники.КатегорииЭксплуатации.ПустаяСсылка(),
			Новый Структура("ИнвентарныйУчет, СпособПогашенияСтоимостиБУ", Ложь, Перечисления.СпособыПогашенияСтоимостиТМЦ.ПоСроку));
			
		ЗначенияЗаполненияИнвентарныхСтрок = Новый Структура("Количество", 1);
		ЗначенияЗаполненияСписанныхСтрок = Новый Структура("Сумма, СуммаПР, СуммаВР", 0, 0, 0);
		
		Для Каждого Строка Из ТМЦВЭксплуатации Цикл
			...
			КонецЕсли;
			Если СвойстваКатегорий[Строка.КатегорияЭксплуатации].СпособПогашенияСтоимостиБУ = Перечисления.СпособыПогашенияСтоимостиТМЦ.ПриПередаче Тогда
				ЗаполнитьЗначенияСвойств(Строка, ЗначенияЗаполненияСписанныхСтрок);
			КонецЕсли;
		КонецЦикла;
		//-- НЕ УТ
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
				...
				Если Товары[ТекИндекс].ПредусмотренЗалогЗаТару Тогда
					Если НЕ ЗначениеЗаполнено(Товары[ТекИндекс].Цена) Тогда
						ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Цена"" в строке %НомерСтроки% табличной части ""Товары""';
											|en = 'Column ""Price"" in line %НомерСтроки% of table ""Goods"" is not filled in'");
					...
					Если НЕ ЗначениеЗаполнено(Товары[ТекИндекс].Сумма) Тогда
						ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Сумма"" в строке %НомерСтроки% табличной части ""Товары""';
											|en = 'Column ""Amount"" in line %НомерСтроки% of table ""Goods"" is not filled in'");
						ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", Товары.Получить(ТекИндекс).НомерСтроки);
						ОбщегоНазначения.СообщитьПользователю(
							ТекстОшибки,
							ЭтотОбъект,
					...
					Если НЕ ЗначениеЗаполнено(Товары[ТекИндекс].СуммаРегл) Тогда
						ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Сумма(регл)"" в строке %НомерСтроки% табличной части ""Товары""';
											|en = 'Column ""Amount (local)"" in line %НомерСтроки% of table ""Goods"" is not filled in'");
						ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", Товары.Получить(ТекИндекс).НомерСтроки);
						ОбщегоНазначения.СообщитьПользователю(
							ТекстОшибки,
							ЭтотОбъект,
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковТМЦВЭксплуатации Тогда
		...
		ПараметрыПроверки.ИмяТЧ = "ТМЦВЭксплуатации";
		ТМЦВЭксплуатацииСервер.ПроверитьУчетПоФизЛицам(ЭтотОбъект, МассивНепроверяемыхРеквизитов, Отказ);
		...
		Если НеобходимоЗаполнитьОбъектРасчетов Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не заполнена колонка ""Объект расчетов"" в строке %1 списка ""Расчеты с партнерами""';
						|en = '""AR/AP object"" in line %1 of list ""Partners AR/AP"" is not filled in'"),
		...
		Если НеобходимоЗаполнитьНомерРасчетногоДокумента Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не заполнена колонка ""Номер"" в строке %1 списка ""Расчеты с партнерами""';
							|en = '""Number"" in line %1 of list ""Partners AR/AP"" is not filled in'"),
						СтрокаТаблицы.НомерСтроки);
			ОбщегоНазначения.СообщитьПользователю(
				Текст,
				ЭтотОбъект,
		...
		Если НеобходимоЗаполнитьДатуРасчетногоДокумента Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не заполнена колонка ""Дата"" в строке %1 списка ""Расчеты с партнерами""';
							|en = '""Date"" in line %1 of list ""Partners AR/AP"" is not filled in'"),
						СтрокаТаблицы.НомерСтроки);
			ОбщегоНазначения.СообщитьПользователю(
				Текст,
				ЭтотОбъект,
		...
		Если ОтражатьВОперативномУчете
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не заполнена колонка ""Расчетный документ"" в строке %1 списка ""Расчеты с партнерами""';
							|en = '""AR/AP document"" in line %1 of list ""Partners AR/AP"" is not filled in'"),
						СтрокаТаблицы.НомерСтроки);
			ОбщегоНазначения.СообщитьПользователю(
				Текст,
				ЭтотОбъект,
			...
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.ДатаРасчетногоДокумента) Тогда
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не заполнена колонка ""Дата"" в строке %1 списка ""Расчеты между организациями""';
						|en = 'Column ""Date"" is not filled in in row %1 of list ""Settlements between companies""'"),
					СтрокаТаблицы.НомерСтроки);
				ОбщегоНазначения.СообщитьПользователю(
					Текст,
					ЭтотОбъект,
			...
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.НомерРасчетногоДокумента) Тогда
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не заполнена колонка ""Номер"" в строке %1 списка ""Расчеты между организациями""';
						|en = 'Column ""Number"" is not filled in in row %1 of list ""Settlements between companies""'"),
					СтрокаТаблицы.НомерСтроки);
				ОбщегоНазначения.СообщитьПользователю(
					Текст,
					ЭтотОбъект,
		...
		Если Не ЗначениеЗаполнено(СтрТабл.ОбъектРасчетов)
		 ...
		 И ЗначениеЗаполнено(СтрТабл.ДатаРасчетногоДокумента) Тогда
			...
			ТекстКомментария = НСтр("ru = 'Документ сгенерирован автоматически при вводе остатков %ТекущаяДата%';
									|en = 'Document is generated automatically when entering balance %ТекущаяДата%'");
			...
			Исключение
				ТекстСообщения = НСтр("ru = 'Ошибка создания расчетного документа: ""%ОписаниеОшибки%""';
										|en = 'Error creating accounting document: ""%ОписаниеОшибки%""'");
			...
			Если ТипЗнч(СтрокаТаблицы.ОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
				...
				НовыйОбъект.Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Элемент справочника сгенерирован автоматически при вводе остатков %1';
						|en = 'Catalog item has been generated automatically on entering balance %1'"),
			...
			Иначе
				...
				НовыйОбъект.Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Документ сгенерирован автоматически при вводе остатков %1';
						|en = 'Document is generated automatically when entering balance %1'"),
					Формат(ТекущаяДатаСеанса(),"ДЛФ=D"));
			КонецЕсли;
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка создания расчетного документа: ""%1""';
						|en = 'Error creating accounting document: ""%1""'"),
					ТекстОшибки);
				ОбщегоНазначения.СообщитьПользователю(
					ТекстСообщения,
					,
	...
	Если ЗначениеЗаполнено(Организация)
		...
		ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользоватьОбособленныеПодразделенияВыделенныеНаБаланс")
			...
			Текст = НСтр("ru = 'Организация-получатель не должна быть взаимосвязана с организацией-отправителем по организационной структуре.';
						|en = 'Receiving company should not be interrelated to the shipping company by the organizational structure.'");
			ОбщегоНазначения.СообщитьПользователю(
				Текст,
				ЭтотОбъект,
...
Процедура ПроверитьРазрядностьНомеровОбъектовРасчета(Отказ)
	...
	ШаблонСообщения = НСтр("ru = 'Длина номера объекта расчетов превышает допустимую длину (%Длина%) для выбранного объекта расчетов в строке %НомерСтроки%.';
							|en = 'Length of AR/AP object number exceeds the maximum length (%Длина%) for the selected AR/AP object in line %НомерСтроки%.'");
	ТабЧасть = "";
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковЗадолженностиКлиентов
		Или ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковЗадолженностиПоставщикам
		Или ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовКлиентов
...
Процедура ПроверитьЗаполнениеТМЦВЭксплуатации(НепроверяемыеРеквизиты, Отказ)
	...
	НепроверяемыеРеквизиты.Добавить("ТМЦВЭксплуатации.Сумма");
	НепроверяемыеРеквизиты.Добавить("ТМЦВЭксплуатации.СуммаПР");
	НепроверяемыеРеквизиты.Добавить("ТМЦВЭксплуатации.СуммаВР");
	...
	Запрос = Новый Запрос(
		...
		|	ВЫРАЗИТЬ(Таблица.Сумма КАК ЧИСЛО) КАК Сумма,
		|	ВЫРАЗИТЬ(Таблица.СуммаПР КАК ЧИСЛО) КАК СуммаПР,
		|	ВЫРАЗИТЬ(Таблица.СуммаВР КАК ЧИСЛО) КАК СуммаВР
		|ПОМЕСТИТЬ Таблица
		|ИЗ
		|	&Таблица КАК Таблица
		|;
		...
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(Таблица.КатегорияЭксплуатации.ИнвентарныйУчет, ЛОЖЬ)
		|				И Таблица.ИнвентарныйНомер В
		|					(ВЫБРАТЬ
		|						ТаблицаДублей.ИнвентарныйНомер
		|					ИЗ
		|						Таблица КАК ТаблицаДублей
		|					ГДЕ
		|						ТаблицаДублей.ИнвентарныйНомер = Таблица.ИнвентарныйНомер
		|						И ТаблицаДублей.НомерСтроки <> Таблица.НомерСтроки)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ЕстьОшибкиДубль
		|ИЗ
		|	Таблица КАК Таблица"
	);
	Запрос.УстановитьПараметр("Таблица", ТМЦВЭксплуатации.Выгрузить(, "НомерСтроки, КатегорияЭксплуатации, ИнвентарныйНомер, Сумма, СуммаПР, СуммаВР"));
	
	Результат = Запрос.Выполнить();
		
	ШаблонОшибкаДубль = НСтр("ru = 'Инвентарный номер ""%ИнвентарныйНомер%""
		|в строке %НомерСтроки% списка ""ТМЦ в эксплуатации"" повторяется в других строках документа';
		|en = 'Inventory number ""%ИнвентарныйНомер%""
		|in line %НомерСтроки% of list ""Inventory in operation"" is repeated in other lines of the document'");
	
	Выборка = Результат.Выбрать();
	Пока Выборка.Следующий() Цикл
		
		Если Выборка.ИнвентарныйУчет 
			И Выборка.ЕстьОшибкиДубль Тогда
			
			СтрокаТЧ = ТМЦВЭксплуатации.Получить(Выборка.НомерСтроки-1);
			...
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ТМЦВЭксплуатации", Выборка.НомерСтроки, "ИнвентарныйНомер");
