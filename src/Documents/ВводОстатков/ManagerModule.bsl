...
Функция ТекстОтраженияВРеглУчете() Экспорт
	Возврат ВводОстатковЛокализация.ВводОстатковТекстОтраженияВРеглУчете();
КонецФункции
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	Возврат ВводОстатковЛокализация.ВводОстатковТекстЗапросаВТОтраженияВРеглУчете();
КонецФункции
...
Функция ОстаткиПоТипуОперации(Дата, ХозяйственнаяОперация, Организации, ДополнительныйОтбор = Неопределено, ПараметрыЗаполненияОстатков = Неопределено) Экспорт
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковВКассах Тогда
		Возврат ОстаткиПоКассам(ДатаОстатков, Организации);
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковНаБанковскихСчетах Тогда
		Возврат ОстаткиПоБанковскимСчетам(ДатаОстатков, Организации);
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковЗадолженностиКлиентов
		...
		Если Не ЗначениеЗаполнено(ПараметрыЗаполненияОстатков) Или Не ПараметрыЗаполненияОстатков.Свойство("ВводОстатков22") Тогда
			ВводОстатков22 = Ложь;
		Иначе
			ВводОстатков22 = ПараметрыЗаполненияОстатков.ВводОстатков22;
		КонецЕсли;
		
		Возврат ОстаткиПоРасчетамСПартнерами(ДатаОстатков, ХозяйственнаяОперация, Организации, ВводОстатков22);
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковРасчетовМеждуОрганизациямиПоАвансам
		...
		Возврат ОстаткиПоРасчетамМеждуОрганизациями(ДатаОстатков, ХозяйственнаяОперация, Организации, ДополнительныйОтбор);
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковСобственныхТоваров
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковЗадолженностиПодотчетников
		...
		Возврат ОстаткиПоРасчетамСПодотчетниками(ДатаОстатков, ХозяйственнаяОперация, Организации, ДополнительныйОтбор);
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковПоДоговорамКредитовИДепозитов Тогда
		Возврат ОстаткиПоРасчетамПоДоговорамКредитовИДепозитов(ДатаОстатков, Организации);
...
Функция КлючевыеПоляРеглУчетаПоТипуОперации(ХозяйственнаяОперация) Экспорт
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковМатериаловПереданныхВПроизводство
		ИЛИ ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковЗадолженностиПодотчетников
		ИЛИ ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковПерерасходовПодотчетныхСредств
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВыбраннаяОперация) Тогда
		...
		//++ НЕ УТ
		СоответствиеТиповОперацийФормам.Вставить(ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковПрочихРасходовУСН"), "ФормаРасходыПриУСН");
		СоответствиеТиповОперацийФормам.Вставить(ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковРасходовУСНПоМатериалам"), "ФормаРасходыПриУСН");
		СоответствиеТиповОперацийФормам.Вставить(ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковРасходовУСНПоТоварам"), "ФормаРасходыПриУСН");
		//-- НЕ УТ
	...
	Если Параметры.Свойство("ЗначениеКопирования") 
		...
		Если ПараметрыКопирования.НовыйДокументВводаОстатковСтрокой = "Документ.ВводОстатков" Тогда
			ТекстИсключения = НСтр("ru = 'Копирование документов %ВыбраннаяОперация% при использовании партионного учета версии 2.1 запрещено';
									|en = 'Copying documents %ВыбраннаяОперация% when using batch accounting version 2.1 is not allowed'");
		КонецЕсли;
		...
		Если ПараметрыКопирования.ХозяйственныеОперацииДоступныеДляВвода.Найти(ВыбраннаяОперация) = Неопределено Тогда
			ТекстИсключения = НСтр("ru = 'Копирование документов %ВыбраннаяОперация% запрещено';
									|en = 'Copying documents %ВыбраннаяОперация% is not allowed'");
		КонецЕсли;
		...
		Если ИмяТЧ = "РасчетыСПартнерами" Тогда
			Если Выборка.Организация <> Объект.Организация Тогда
				...
				ТекстСообщения = НСтр("ru = 'В найденном документе ""%ТипДокумента%"" № %Номер% организация отличается от указанной во вводе остатков';
										|en = 'Company in found document ""%ТипДокумента%"" No. %Номер% differs from the one specified on entering balance'");
			...
			Если ЗначениеЗаполнено(ТекущиеДанные.Партнер)
				...
				ТекстСообщения = НСтр("ru = 'В найденном документе ""%ТипДокумента%"" № %Номер% партнер отличается от указанного во вводе остатков';
										|en = 'Partner in found document ""%ТипДокумента%"" No. %Номер% differs from the one specified on entering balance'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ТипДокумента%",ПредставлениеОбъекта);
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%Номер%",ТекущиеДанные.НомерРасчетногоДокумента);
			...
			Если ЗначениеЗаполнено(ТекущиеДанные.Контрагент)
				...
				ТекстСообщения = НСтр("ru = 'В найденном документе ""%ТипДокумента%"" № %Номер% контрагент отличается от указанного во вводе остатков';
										|en = 'Counterparty in found document ""%ТипДокумента%"" No. %Номер% differs from the one specified on entering balance'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ТипДокумента%",ПредставлениеОбъекта);
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%Номер%",ТекущиеДанные.НомерРасчетногоДокумента);
		...
		ИначеЕсли ИмяТЧ = "РасчетыСПодотчетниками" Тогда
			...
			Если ЗначениеЗаполнено(ТекущиеДанные.ПодотчетноеЛицо)
				...
				ТекстСообщения = НСтр("ru = 'В найденном документе ""%ТипДокумента%"" № %Номер% подотчетное лицо отличается от указанного во вводе остатков';
										|en = 'The advance holder specified in found document ""%ТипДокумента%"" No. %Номер% differs from the one specified on entering balance'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ТипДокумента%",ПредставлениеОбъекта);
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%Номер%",ТекущиеДанные.НомерРасчетногоДокумента);
			...
			Если ЗначениеЗаполнено(ТекущиеДанные.Валюта)
				...
				ТекстСообщения = НСтр("ru = 'В найденном документе ""%ТипДокумента%"" № %Номер% валюта отличается от указанной во вводе остатков';
										|en = 'Currency in found document ""%ТипДокумента%"" No. %Номер% differs from the one specified on entering balance'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ТипДокумента%",ПредставлениеОбъекта);
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%Номер%",ТекущиеДанные.НомерРасчетногоДокумента);
			...
			Если ТипЗнч(ТекущиеДанные.ОбъектРасчетов) <> Тип("ДокументСсылка.АвансовыйОтчет")
				...
				ТекстСообщения = НСтр("ru = 'В найденном документе ""%ТипДокумента%"" № %Номер% хозяйственная операция отличается от ""%ВыдачаДенежныхСредствПодотчетнику%""';
										|en = 'In the ""%ТипДокумента%"" document No. %Номер%, the business transaction is different from ""%ВыдачаДенежныхСредствПодотчетнику%""'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ТипДокумента%",ПредставлениеОбъекта);
		...
		ИначеЕсли ИмяТЧ = "РасчетыМеждуОрганизациями" Тогда
			...
			Если Выборка.Организация <> Объект.Организация Тогда
				...
				ТекстСообщения = НСтр("ru = 'В найденном документе ""%ТипДокумента%"" № %Номер% %РольОрганизации% отличается от указанной во вводе остатков';
										|en = '%РольОрганизации% in found document ""%ТипДокумента%"" No. %Номер% differs from the one specified on entering balance'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ТипДокумента%",ПредставлениеОбъекта);
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%Номер%",ТекущиеДанные.НомерРасчетногоДокумента);
					...
					Если Объект.УдалитьТипОперации = Перечисления.УдалитьТипыОперацийВводаОстатков.ОстаткиРасчетовМеждуОрганизациямиПоАвансам
						...
						Если НеобходимоСоздатьЭлемент Тогда
							...
							ДоговорМеждуОрганизациями.Наименование          = НСтр("ru = 'Договор создан автоматически при вводе остатков';
																					|en = 'The contract is created automatically upon entering balance'");
							...
							Исключение
								ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось записать элемент справочника Договоры между организациями';
																			|en = 'Cannot save the item of Contracts between companies catalog'"));
...
Функция ОстаткиПоКассам(ДатаОстатков, МассивОрганизаций)
	...
	Запрос.Текст ="ВЫБРАТЬ
		...
		|	СУММА(ДенежныеСредстваНаличныеОстатки.СуммаУпрОстаток) КАК СуммаУпр,
		|	""Кассы"" КАК ЗаполняемаяТабличнаяЧасть
		|ИЗ
...
Функция ОстаткиПоБанковскимСчетам(ДатаОстатков, МассивОрганизаций)
	...
	|	СУММА(ДенежныеСредстваБезналичныеОстатки.СуммаУпрОстаток) КАК СуммаУпр,
	|	""БанковскиеСчета"" КАК ЗаполняемаяТабличнаяЧасть
	|ИЗ
	|	РегистрНакопления.ДенежныеСредстваБезналичные.Остатки(&Дата, Организация В (&МассивОрганизаций)) КАК
	|		ДенежныеСредстваБезналичныеОстатки
	|СГРУППИРОВАТЬ ПО
	|	ДенежныеСредстваБезналичныеОстатки.Организация,
	|	ДенежныеСредстваБезналичныеОстатки.БанковскийСчет,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковНаБанковскихСчетах)
	|УПОРЯДОЧИТЬ ПО
	|	Организация";
...
Функция ОстаткиПоРасчетамСПартнерами(ДатаОстатков, ХозяйственнаяОперация, МассивОрганизаций, ВводОстатков22)
	...
	Если Не ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
			...
			|			ТОГДА ДанныеАналитики.Контрагент ССЫЛКА Справочник.Контрагенты
			|		ИНАЧЕ ИСТИНА
			|	КОНЕЦ
			|ИНДЕКСИРОВАТЬ ПО
			|	КлючАналитики
			...
			|	ДанныеПоОстаткам.ЗаказКлиента КАК ОбъектРасчетов,
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ
			...
			|		КОГДА НЕ &ВводОстатков22
			|			ТОГДА ИСТИНА
			|		ИНАЧЕ ДанныеАналитики.Контрагент ССЫЛКА Справочник.Контрагенты
			|	КОНЕЦ
			|ИНДЕКСИРОВАТЬ ПО
			|	КлючАналитики
			|;
			...
			|	ДанныеПоОстаткам.ОбъектРасчетов          КАК ОбъектРасчетов,
			|	ДанныеПоОстаткам.РасчетныйДокумент       КАК ДокументРасчетов,
			|	ДанныеПоОстаткам.Валюта                  КАК ВалютаВзаиморасчетов,
			|	ВЫБОР
			|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковЗадолженностиКлиентов)
	...
	Запрос.Параметры.Вставить("Дата",                                      ДатаОстатков);
	Запрос.Параметры.Вставить("ВводОстатков22",                 ВводОстатков22);
...
Функция ОстаткиПоРасчетамСПодотчетниками(ДатаОстатков, ХозяйственнаяОперация, МассивОрганизаций, ДопОтбор)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
		|	""РасчетыСПодотчетниками"" КАК ЗаполняемаяТабличнаяЧасть,
		...
		|	&Коэффициент * ДенежныеСредстваУПодотчетныхЛицОстатки.СуммаУпрОстаток КАК СуммаУпр
		|ИЗ
		|	РегистрНакопления.ДенежныеСредстваУПодотчетныхЛиц.Остатки(&Дата, Организация В (&МассивОрганизаций) И &УсловиеПодразделения) КАК ДенежныеСредстваУПодотчетныхЛицОстатки
		|ГДЕ
	...
	Запрос.Параметры.Вставить("МассивОрганизаций",     МассивОрганизаций);
	Запрос.Параметры.Вставить("ХозяйственнаяОперация", ХозяйственнаяОперация);
	Запрос.Параметры.Вставить("Коэффициент",           ?(ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковПерерасходовПодотчетныхСредств, -1, 1));
	Запрос.Параметры.Вставить("Дата",                  ДатаОстатков);
...
Функция ОстаткиПоРасчетамМеждуОрганизациями(ДатаОстатков, ХозяйственнаяОперация, МассивОрганизаций, ДопОтбор)
	...
	Если Не ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	""РасчетыМеждуОрганизациями"" КАК ЗаполняемаяТабличнаяЧасть,
		|	ДанныеАналитики.Партнер,
		...
		|	ДанныеПоОстаткам.Валюта КАК ВалютаВзаиморасчетов,
		|	СУММА(ВЫБОР
		|			КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковРасчетовМеждуОрганизациямиПоРеализациям)
		|				ТОГДА ДанныеПоОстаткам.ДолгОстаток
		|			ИНАЧЕ -ДанныеПоОстаткам.ПредоплатаОстаток
		|		КОНЕЦ) КАК Сумма,
		|	СУММА(ВЫБОР
		...
		|		КОНЕЦ) КАК СуммаУпр
		|ИЗ
		|	РегистрНакопления.РасчетыСКлиентамиПоДокументам.Остатки(&Дата, АналитикаУчетаПоПартнерам В (ВЫБРАТЬ КлючАналитики ИЗ ВтКлючиАналитики)) КАК ДанныеПоОстаткам
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК ДанныеАналитики
		|		ПО ДанныеПоОстаткам.АналитикаУчетаПоПартнерам = ДанныеАналитики.КлючАналитики
		|ГДЕ
		|	ВЫБОР
	...
	Иначе
		...
		|	КОНЕЦ                                    КАК СуммаУпр
		|ИЗ
		|	РегистрНакопления.РасчетыСКлиентамиПоСрокам.Остатки(&Дата, АналитикаУчетаПоПартнерам В (ВЫБРАТЬ КлючАналитики ИЗ ВтКлючиАналитики)) КАК ДанныеПоОстаткам
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК ДанныеАналитики
		|		ПО ДанныеПоОстаткам.АналитикаУчетаПоПартнерам = ДанныеАналитики.КлючАналитики
		|ГДЕ
		|	ВЫБОР
	...
	Если ДопОтбор <> Неопределено И ДопОтбор.Свойство("ОрганизацияПолучатель") Тогда
		Запрос.УстановитьПараметр("ОрганизацияПолучатель", ДопОтбор.ОрганизацияПолучатель);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборПоКонтрагенту", "ДанныеАналитики.Контрагент = &ОрганизацияПолучатель");
	Иначе
...
Функция ОстаткиПоТоварам(ДатаОстатков, ХозяйственнаяОперация, МассивОрганизаций, ДопОтбор, ПараметрыЗаполненияОстатков)
	...
	Если ЗаполнятьПоСебестоимости Тогда
		...
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК НДСРегл,
		|	ЕСТЬNULL(СтавкиНДСНоменклатуры.СтавкаНДС, ЕСТЬNULL(СтавкиНДСНоменклатурыПустойСтраны.СтавкаНДС, ЕСТЬNULL(ОсновныеСтавкиНДС.СтавкаНДС,
		...
		|		ПО СебестоимостьТоваровОстатки.АналитикаУчетаНоменклатуры.Номенклатура = СтавкиНДСНоменклатуры.Номенклатура
		|			И СебестоимостьТоваровОстатки.Организация.СтранаРегистрации = СтавкиНДСНоменклатуры.Страна
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОсновныеСтавкиНДС.СрезПоследних(&Дата) КАК ОсновныеСтавкиНДС
		|		ПО СебестоимостьТоваровОстатки.Организация.СтранаРегистрации = ОсновныеСтавкиНДС.Страна
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтавкиНДСНоменклатуры.СрезПоследних(&Дата, Страна = ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)) КАК СтавкиНДСНоменклатурыПустойСтраны
		|		ПО СебестоимостьТоваровОстатки.АналитикаУчетаНоменклатуры.Номенклатура = СтавкиНДСНоменклатурыПустойСтраны.Номенклатура
		|ГДЕ
	...
	Иначе
		...
		|			КОНЕЦ)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтавкиНДСНоменклатуры.СрезПоследних(&Дата) КАК СтавкиНДСНоменклатуры
		|		ПО ТоварыОрганизацийОстатки.АналитикаУчетаНоменклатуры.Номенклатура = СтавкиНДСНоменклатуры.Номенклатура
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтавкиНДСНоменклатуры.СрезПоследних(&Дата, Страна = ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)) КАК СтавкиНДСНоменклатурыПустойСтраны
		|		ПО ТоварыОрганизацийОстатки.АналитикаУчетаНоменклатуры.Номенклатура = СтавкиНДСНоменклатурыПустойСтраны.Номенклатура
		|ГДЕ
...
Функция ОстаткиПоРасчетамПоДоговорамКредитовИДепозитов(ДатаОстатков, МассивОрганизаций)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	""РасчетыПоФинансовымИнструментам"" КАК ЗаполняемаяТабличнаяЧасть,
	|	ДанныеАналитики.Организация КАК Организация,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковПоДоговорамКредитовИДепозитов) КАК ХозяйственнаяОперация,
	|	ДанныеАналитики.Партнер,
	|	ДанныеАналитики.Контрагент,
	|	РасчетыПоФинансовымИнструментамОстатки.Договор,
	|	СУММА(ВЫБОР
	|			КОГДА РасчетыПоФинансовымИнструментамОстатки.Договор.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерыДоговоровФинансовыхИнструментов.КредитИлиЗайм)
	|				ТОГДА -1
	|			ИНАЧЕ 1
	|		КОНЕЦ * РасчетыПоФинансовымИнструментамОстатки.СуммаОстаток) КАК Сумма,
	|	СУММА(ВЫБОР
	|			КОГДА РасчетыПоФинансовымИнструментамОстатки.Договор.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерыДоговоровФинансовыхИнструментов.КредитИлиЗайм)
	|				ТОГДА -1
	|			ИНАЧЕ 1
	|		КОНЕЦ * РасчетыПоФинансовымИнструментамОстатки.СуммаРеглОстаток) КАК СуммаРегл,
	|	СУММА(ВЫБОР
	|			КОГДА РасчетыПоФинансовымИнструментамОстатки.Договор.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерыДоговоровФинансовыхИнструментов.КредитИлиЗайм)
	|				ТОГДА -1
	|			ИНАЧЕ 1
	|		КОНЕЦ * РасчетыПоФинансовымИнструментамОстатки.СуммаУпрОстаток) КАК СуммаУпр,
	|	РасчетыПоФинансовымИнструментамОстатки.ТипСуммы
	|ИЗ
	|	РегистрНакопления.РасчетыПоФинансовымИнструментам.Остатки(&Дата, ) КАК РасчетыПоФинансовымИнструментамОстатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК ДанныеАналитики
	|		ПО РасчетыПоФинансовымИнструментамОстатки.АналитикаУчетаПоПартнерам = ДанныеАналитики.КлючАналитики
...
Функция ОстаткиПоПрочимРасходам(ДатаОстатков, МассивОрганизаций, ДопОтбор)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
		...
		|	ВЫБОР КОГДА СУММА(ПрочиеРасходыОстатки.СуммаБезНДСОстаток) = 0 ТОГДА СУММА(ПрочиеРасходыОстатки.СуммаОстаток) ИНАЧЕ СУММА(ПрочиеРасходыОстатки.СуммаБезНДСОстаток) КОНЕЦ КАК СуммаБезНДС,
	...
	Если РезультатВыполненияЗапроса <> Неопределено Тогда
		...
		Результат.Колонки.Добавить("ВидДеятельностиНДС", Новый ОписаниеТипов("ПеречислениеСсылка.ТипыНалогообложенияНДС"));
		
		Для Каждого СтрокаТаблицы Из Результат Цикл
			ПараметрыУчетаПоОрганизации = УчетНДСУП.ПараметрыУчетаПоОрганизации(СтрокаТаблицы.Организация, ДатаОстатков);
			СтрокаТаблицы.ВидДеятельностиНДС = ПараметрыУчетаПоОрганизации.ОсновноеНалогообложениеНДСПродажи;
		КонецЦикла;
