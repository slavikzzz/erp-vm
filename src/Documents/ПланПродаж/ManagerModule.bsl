...
#Область ДоляДнейНаличияТовараНаОстатках
	...
	ДопПараметр.Вставить("Заголовок", 			НСтр("ru = 'Доля дней наличия товара на остатках';
														|en = 'Share of stock availability days'"));
...
#Область ФактыПродаж
	...
	ДопПараметр.Вставить("Тип", 				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
	ДопПараметр.Вставить("Путь", 				ПутьКРеквизиту);
	ДопПараметр.Вставить("Заголовок", 			?(СоздаватьРеквизитСОтбором, НСтр("ru = 'Факт продаж*';
																					|en = 'Actual sales*'"), НСтр("ru = 'Факт продаж';
																												|en = 'Actual sales'")));
	...
	ДопПараметр.Вставить("ЗаголовокОперанда",	?(СоздаватьРеквизитСОтбором, НСтр("ru = 'Факт продаж*';
																					|en = 'Actual sales*'"), НСтр("ru = 'Факт продаж';
																												|en = 'Actual sales'")));
...
Функция ДоступныеТипыАналитикБюджетирования(ДокументСсылка, ИмяТаблицы = "") Экспорт
	...
	ИначеЕсли ВыборкаПлан.ЗаполнятьСоглашение Тогда
		ДоступныеТипыАналитик.Вставить(Тип("СправочникСсылка.СоглашенияСКлиентами"), ИмяТаблицы + ".Ссылка.Соглашение"); 
	КонецЕсли;
	...
	ДоступныеТипыАналитик.Вставить(Тип("СправочникСсылка.Пользователи"), ИмяТаблицы + ".Ссылка.Менеджер");
	...
	Если ИмяТаблицы = "Товары" Тогда
		...
		ДоступныеТипыАналитик.Вставить(Тип("СправочникСсылка.НаправленияДеятельности"), "Товары.Назначение.НаправлениеДеятельности");
		ДоступныеТипыАналитик.Вставить(Тип("ДокументСсылка.ЗаказКлиента"), "Товары.Назначение.Заказ");
	...
	Иначе 
		...
		ДоступныеТипыАналитик.Вставить(Тип("СправочникСсылка.НаправленияДеятельности"), ИмяТаблицы + ".Ссылка.Назначение.НаправлениеДеятельности");
		ДоступныеТипыАналитик.Вставить(Тип("ДокументСсылка.ЗаказКлиента"), ИмяТаблицы + ".Ссылка.Назначение.Заказ");
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.УстановитьПараметр("ВалютаРегламентированногоУчета", ЗначениеНастроекПовтИсп.БазоваяВалютаПоУмолчанию());
...
Функция ТекстЗапросаТаблицаОборотыБюджетов(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтКонтрагентыПартнеров", ТекстыЗапроса) Тогда
	...
	|		ИЛИ НЕ ПланОплат.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыКонтроляОплатыКлиентом.КредитСдвиг)
	|			И НЕ ПланОплат.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыКонтроляОплатыКлиентом.КредитПослеОтгрузки))
	...
	|	И &Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.Отменен)
	|	И (ПланОплатКредит.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыКонтроляОплатыКлиентом.КредитСдвиг)
	|		ИЛИ ПланОплатКредит.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыКонтроляОплатыКлиентом.КредитПослеОтгрузки))
	...
	БюджетированиеСервер.УстановитьВЗапросеВыраженияЗаполненияАналитики(
		Запрос.Параметры.Ссылка, ТекстЗапроса, Запрос.Параметры.СтатьяБюджетов, Документы.ПланПродаж, "АналитикаТовары", "Товары");
		
	БюджетированиеСервер.УстановитьВЗапросеВыраженияЗаполненияАналитики(
		Запрос.Параметры.Ссылка, ТекстЗапроса, Запрос.Параметры.СтатьяБюджетовОплат, Документы.ПланПродаж, "АналитикаПланОплат", "ПланОплат");
	
	БюджетированиеСервер.УстановитьВЗапросеВыраженияЗаполненияАналитики(
		Запрос.Параметры.Ссылка, ТекстЗапроса, Запрос.Параметры.СтатьяБюджетовОплатКредит, Документы.ПланПродаж, "АналитикаПланОплатКредит", "ПланОплатКредит");
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПланПродаж") Тогда
		Если ОбщегоНазначенияУТКлиентСервер.АвторизованВнешнийПользователь() Тогда
			Если МассивОбъектов.Количество() = 1 Тогда
				ПараметрыВывода.ЗаголовокФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'План закупок %1 от %2';
																												|en = 'Purchase plan %1 dated %2'"), МассивОбъектов[0].Номер, МассивОбъектов[0].Дата);
			КонецЕсли;
...
Функция СформироватьПечатнуюФормуПлана(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ДанныеПечати.ЗаполнятьПартнераВТЧ Тогда
			СтруктураДопПоля = Новый Структура;
			СтруктураДопПоля.Вставить("Заголовок", НСтр("ru = 'Клиент';
														|en = 'Customer'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Если ДанныеПечати.ЗаполнятьСоглашениеВТЧ Тогда
			...
			СтруктураДопПоля.Вставить("Заголовок", НСтр("ru = 'Соглашение';
														|en = 'Terms of sales'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Если ВыводитьШапку Тогда
			...
			Если ПланироватьПоСумме Тогда
				...
				ИтоговаяСтрока = НСтр("ru = 'Всего наименований %КоличествоНаименований%, на сумму %СуммаДокумента%';
										|en = 'Total items %КоличествоНаименований% in the amount of %СуммаДокумента%'", ОбщегоНазначения.КодОсновногоЯзыка());
