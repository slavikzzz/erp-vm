...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
		...
		ИначеЕсли СуммаДокумента <> СуммаЭтаповОплаты Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Сумма отгрузок по документу не совпадает с суммой оплат. Необходимо обновить план оплаты.';
					|en = 'Shipment amount against the document does not match the payment amount. Update the payment plan.'"),
				ЭтотОбъект,
				"ПланОплаты",
				, 
				Отказ);
	...
	Если ПроверятьЗаполнениеПрогнозныхКурсовВалют И ВалютыПроверкиКурсов.Количество() <> 0 Тогда
		...
		Периоды = БюджетнаяОтчетностьВыводСервер.ПериодыБюджетногоОтчета(НачалоПериода, ОкончаниеПериода, Периодичность);
		...
		Если НеНайденныеКурсы.Количество() <> 0 Тогда
			...
			УстановитьПривилегированныйРежим(Истина);
			НаименованиеСценарияБюджетирования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СценарийБюджетирования, "Наименование");
			УстановитьПривилегированныйРежим(Ложь);
			...
			ТекстСообщения = 
				...
				|en = 'Sales forecast is recorded in budgeting.
				|Set forecasted exchange rates for %1 in the ""%2"" budgeting scenario for each period of %3 forecast.'");
			ТекстСообщения = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ТекстСообщения, 
					СтрСоединить(МассивВалют, ", "), 
					НаименованиеСценарияБюджетирования, ?(ПравоПросмотраСценариевБюджетирования, "", НСтр("ru = ', обратитесь к администратору';
	...
	Если Не КроссТаблица
		...
		Для Каждого СтрокаОшибки Из ТаблицаОшибок Цикл
			...
			ТекстСообщения = НСтр("ru = 'Для строк плана с номенклатурой %Номенклатура%%Характеристика%%Назначение%%Склад%%Партнер%%Соглашение% не запланировано количество ни в одном периоде планирования.';
									|en = 'Quantity is not planned in any planning period for lines of plan with the %Номенклатура%%Характеристика%%Назначение%%Склад%%Партнер%%Соглашение% items.'");
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Характеристика%", ?(ЗначениеЗаполнено(СтрокаОшибки.Характеристика), НСтр("ru = ', характеристикой';
																																	|en = ', variants'") + " " + СтрокаОшибки.Характеристика, ""));
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Партнер%", ?(ЗначениеЗаполнено(СтрокаОшибки.Партнер)
				И РеквизитыВидПлана.ЗаполнятьПартнераВТЧ, НСтр("ru = ', партнером';
																|en = ', partner'") + " " + СтрокаОшибки.Партнер, ""));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Соглашение%", ?(ЗначениеЗаполнено(СтрокаОшибки.Соглашение)
				И РеквизитыВидПлана.ЗаполнятьСоглашениеВТЧ, НСтр("ru = ', соглашением';
																|en = ', agreement'") + " " + СтрокаОшибки.Соглашение, ""));
