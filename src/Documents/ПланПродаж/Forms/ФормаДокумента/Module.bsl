...
Процедура УстановитьВидимость()
	...
	Элементы.СтатьяБюджетовОплатыДоОтгрузки.Видимость = Объект.ОтражаетсяВБюджетированииОплаты И ПравоПросмотраСтатейБюджетов;
	Элементы.СтатьяБюджетовОплатыПослеОтгрузки.Видимость = Объект.ОтражаетсяВБюджетированииОплатыКредит И ПравоПросмотраСтатейБюджетов;
	Элементы.СтатьяБюджетовОплатыДоОтгрузки.Заголовок = 
		?(Объект.ОтражаетсяВБюджетированииОплаты И ПравоПросмотраСтатейБюджетов И Не Объект.ОтражаетсяВБюджетированииОплатыКредит,
			НСтр("ru = 'Оплаты';
				|en = 'Payments'"),
...
Функция НадписьДопустимаяСуммаЗадолженности(СуммаЗадолженности, Валюта)
	...
	НадписьДопустимаяСуммаЗадолженности = НСтр("ru = 'Допустимая сумма задолженности не более: %СуммаЗадолженности% %Валюта%.';
												|en = 'Allowable debt amount no more than: %СуммаЗадолженности% %Валюта%.'");
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	КолонкиПоиска.Добавить(НСтр("ru = 'Уникальный идентификатор (Клиент)';
								|en = 'Unique ID (Customer)'"),"ИдентификаторПартнер");
	...
	Если ЗаполнятьПартнераВТЧ Тогда
		КолонкиПоиска.Добавить(НСтр("ru = 'Клиент';
									|en = 'Customer'"),"ЗагружаемыйПартнер");
	КонецЕсли; 
	Если ЗаполнятьСоглашениеВТЧ Тогда
		КолонкиПоиска.Добавить(НСтр("ru = 'Соглашение';
									|en = 'Terms of sales'"),"ЗагружаемоеСоглашение");
	КонецЕсли; 
	Если ЗаполнятьСкладВТЧ Тогда
	...
	Иначе
		...
		КолонкиПоиска.Добавить(НСтр("ru = 'Дата отгрузки';
									|en = 'Shipment date'"),"ДатаОтгрузки");
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	Представление = НСтр("ru = 'План продаж с учетом корректировки по факту продаж';
						|en = 'Sales forecast considering adjustment according to sales fact'");
	РаботаСФормулами.ДобавитьОператор(ГруппаОператоров, Идентификатор, КонструкцияДляВставки, Представление);
	...
	Представление = НСтр("ru = 'Факт продаж с учетом наличия товаров на остатках';
						|en = 'Actual sales considering goods availability in stock'");
	РаботаСФормулами.ДобавитьОператор(ГруппаОператоров, Идентификатор, КонструкцияДляВставки, Представление);
...
Функция ДополнительныеНастройки(СтруктураНастроек, ДополнительныеПоля = Неопределено)
	...
	Если НЕ ТипЗнч(СтруктураНастроек) = Тип("Структура") Тогда
		...
		СтруктураНастроек.Вставить("ФормулаПредставление",НСтр("ru = 'Увеличение планов продаж прошлых периодов на 15%';
																|en = 'Increase of sales forecasts of previous periods by 15%'"));
