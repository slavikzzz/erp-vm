...
Функция СформироватьЗапросПоЗаписямСтажаДляПечати(МассивОбъектов)
	...
	|	СведенияСЗВКСотрудники.ФактПроживанияДНРЛНРЗОХО КАК ФактПроживанияДНРЛНРЗОХО,
	|	СведенияСЗВКСотрудники.ФактПроживанияДНРЛНР КАК ФактПроживанияДНРЛНР,
	|	СведенияСЗВКСотрудники.ФактПроживанияЗОХО КАК ФактПроживанияЗОХО,
...
Функция СформироватьЗапросПоШапкеДляПечати(МассивСсылок)
	...
	|	ПачкаДокументовСЗВ_К.РегистрационныйНомерСФР КАК РегистрационныйНомерСФР,
...
Функция СформироватьВыходнойФайл(ВыборкаПоШапкеДокумента, ВыборкаПоЗаписямСтажа)
	...
	Пока ВыборкаПоЗаписямСтажа.СледующийПоЗначениюПоля("НомерСтрокиСотрудник") Цикл
		...
		Если ВыборкаПоЗаписямСтажа.ФактПроживанияДНРЛНРЗОХО Тогда
			НаборЗаписейСЗВК.ФактПроживанияДНРЛНРЗОХО.Значение = "1";
		КонецЕсли;
		
		Если ВыборкаПоЗаписямСтажа.ФактПроживанияДНРЛНР Тогда
			НаборЗаписейСЗВК.ФактПроживанияДНРЛНР.Значение = "1";
		КонецЕсли;
		
		Если ВыборкаПоЗаписямСтажа.ФактПроживанияЗОХО Тогда
			НаборЗаписейСЗВК.ФактПроживанияЗОХО.Значение = "1";
		КонецЕсли;
		
		ПерсонифицированныйУчет.УстановитьВыводВФайл(НаборЗаписейСЗВК.ФактПроживанияКрым, "Значение");
		ПерсонифицированныйУчет.УстановитьВыводВФайл(НаборЗаписейСЗВК.ФактПроживанияДНРЛНРЗОХО, "Значение");
		ПерсонифицированныйУчет.УстановитьВыводВФайл(НаборЗаписейСЗВК.ФактПроживанияДНРЛНР, "Значение");
		ПерсонифицированныйУчет.УстановитьВыводВФайл(НаборЗаписейСЗВК.ФактПроживанияЗОХО, "Значение");
...
Функция СформироватьПечатнуюФормуСЗВ_К(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаПоШапкеДокумента.Следующий() Цикл 
		...
		ИспользоватьФорму2023 = ДатаПостановленияАДВ2023 <= ВыборкаПоШапкеДокумента.Дата;
		
		Если ИспользоватьФорму2019 Или ИспользоватьФорму2023 Тогда
...
Процедура ВывестиПечатнуюФормуСЗВ_К(ТабличныйДокумент, ВыборкаПоШапкеДокумента, ВыборкаПоЗаписямСтажа, ТаблицаИтоговогоСтажа, НомерДокумента)
	...
	ИспользоватьФорму2022 = ДатаПостановленияАДВ2022 <= ВыборкаПоШапкеДокумента.Дата;
	ИспользоватьФорму2023 = ДатаПостановленияАДВ2023 <= ВыборкаПоШапкеДокумента.Дата;
	
	Если ИспользоватьФорму2023 Тогда 
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПачкаДокументовСЗВ_К.ПФ_MXL_ФормаСЗВ_К_2023");
	ИначеЕсли ИспользоватьФорму2022 Тогда 
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПачкаДокументовСЗВ_К.ПФ_MXL_ФормаСЗВ_К_2022");
	Иначе 
	...
	Если ВыборкаПоЗаписямСтажа.НайтиСледующий(СтруктураОтбора) Тогда  
		...
		Пока ВыборкаПоЗаписямСтажа.СледующийПоЗначениюПоля("НомерСтрокиСотрудник") Цикл
			...
			Если ИспользоватьФорму2023 Тогда
				ОбластьШапка.Параметры.ФактПроживанияДНРЛНРЗОХО = ?(ВыборкаПоЗаписямСтажа.ФактПроживанияДНРЛНРЗОХО, "Х", "");
				ОбластьШапка.Параметры.ФактПроживанияДНРЛНР = ?(ВыборкаПоЗаписямСтажа.ФактПроживанияДНРЛНР, "Х", "");
				ОбластьШапка.Параметры.ФактПроживанияЗОХО = ?(ВыборкаПоЗаписямСтажа.ФактПроживанияЗОХО, "Х", "");
			КонецЕсли;
			
			Если ИспользоватьФорму2023 И Не ЗначениеЗаполнено(СокрЛП(ВыборкаПоЗаписямСтажа.ТерриториальныеУсловияНа_31_12_2001Код)) Тогда 
				ОбластьШапка.Параметры.ТерриториальныеУсловия = "пустое";
			КонецЕсли;
			...
			Если ИспользоватьФорму2023 Тогда
				НомерСтрокиВариантСЗВК = ТабличныйДокумент.ВысотаТаблицы - 20;
			КонецЕсли;
...
Функция СформироватьПечатнуюФормуАДВ_6_1(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаДокументов.СледующийПоЗначениюПоля("Документ") Цикл
		...
		Если ИспользоватьФорму2023 Тогда 
			МакетОписи = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_ФормаАДВ_6_1_2023");
		ИначеЕсли ИспользоватьФорму2019 Тогда 
			МакетОписи = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_ФормаАДВ_6_1_2019");
		Иначе 
		...
		Если Не ИспользоватьФорму2023 Тогда 
