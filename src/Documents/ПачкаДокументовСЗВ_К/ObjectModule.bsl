...
Функция СформироватьЗапросДляПроверки()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПачкаДокументовСЗВ_КСотрудники.НомерСтроки КАК НомерСтроки,
	|	ПачкаДокументовСЗВ_КСотрудники.Фамилия КАК Фамилия,
	|	ПачкаДокументовСЗВ_КСотрудники.Имя КАК Имя,
	...
	|	ПачкаДокументовСЗВ_КСотрудники.ТерриториальныеУсловияНа31_12_2001 КАК ТерриториальныеУсловияНа31_12_2001,
	|	ПачкаДокументовСЗВ_КСотрудники.РайонныйКоэффициентНа31_12_2001 КАК РайонныйКоэффициентНа31_12_2001,
	|	ПачкаДокументовСЗВ_КСотрудники.ФактПроживанияВКрыму КАК ФактПроживанияВКрыму,
	|	ПачкаДокументовСЗВ_КСотрудники.ФактПроживанияДНРЛНРЗОХО КАК ФактПроживанияДНРЛНРЗОХО,
	|	ПачкаДокументовСЗВ_КСотрудники.ФактПроживанияДНРЛНР КАК ФактПроживанияДНРЛНР,
	|	ПачкаДокументовСЗВ_КСотрудники.ФактПроживанияЗОХО КАК ФактПроживанияЗОХО
	...
	|	СотрудникиДокумента.ФактПроживанияДНРЛНРЗОХО,
	|	СотрудникиДокумента.ФактПроживанияДНРЛНР,
	|	СотрудникиДокумента.ФактПроживанияЗОХО,
	...
	|	Сотрудники.ФактПроживанияДНРЛНРЗОХО,
	|	Сотрудники.ФактПроживанияДНРЛНР,
	|	Сотрудники.ФактПроживанияЗОХО,
...
Процедура ПроверкаДополнительнойЗаписи(Запись, ОсновнаяЗапись, НомерСтрокиСотрудник, НомерСтроки, Отказ) 
	...
	Если НЕ ЗначениеЗаполнено(ОсновнаяЗапись.ОсобыеУсловияТруда) 
		...
		| 		Особые условия труда , Выслуга лет, Территориальные условия, Основание исчисляемого стажа.';
		|en = 'additional entry can be entered only when populating at least one attribute of the main entry:
		| 		Special working conditions , Seniority, Territorial conditions, Estimated length of service basis.'"), НомерСтроки, Запись.СотрудникНаименование);
		СообщитьОбОшибкеСтажа(ТекстОшибки, Запись, Отказ);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Запись.ОснованиеИсчисляемогоСтажа) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'в дополнительной записи о стаже не может указываться основание исчисляемого стажа.';
																					|en = 'the basis of estimated length of service cannot be specified in the additional entry on the length of service.'"), НомерСтроки, Запись.СотрудникНаименование);
		СообщитьОбОшибкеСтажа(ТекстОшибки, Запись, Отказ);
	КонецЕсли;
	...
	Если (НЕ ЗаданаГруппаТерриториальныеУсловия) И (НЕ УказаныОсобыеУсловияТруда) И (НЕ УказаноОснованиеВыслугиЛет) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'в дополнительной записи должна быть задана либо территориальная льгота, либо особые условия труда, либо выслуга лет.';
																					|en = 'territorial benefit, or special working conditions, or seniority should be specified in the additional entry.'"), НомерСтроки, Запись.СотрудникНаименование);
		СообщитьОбОшибкеСтажа(ТекстОшибки, Запись, Отказ);
	КонецЕсли;
	...
	Если ЗаданаГруппаТерриториальныеУсловия Тогда
		Если  (НЕ ЗначениеЗаполнено(ОсновнаяЗапись.ТерриториальныеУсловия)) И(НЕ ЗначениеЗаполнено(ОсновнаяЗапись.РайонныйКоэффициент)) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'в дополнительной записи территориальная льгота может быть указана только при указании территориальной льготы в основной записи.';
																						|en = 'territorial benefit can be specified in an additional entry only if the territorial benefit is specified in the main entry.'"), НомерСтроки, Запись.СотрудникНаименование);
			СообщитьОбОшибкеСтажа(ТекстОшибки, Запись, Отказ);
		КонецЕсли;
	КонецЕсли;
	...
	Если УказаныОсобыеУсловияТруда Тогда
		...
		Если (УсловияТрудаЗаписи <> Справочники.ОсобыеУсловияТрудаДляСЗВКПФР.ЗП12А)
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'в дополнительной записи код условий труда должен принимать значение ЗП12А, ЗП12Б или ЗП12Ж.';
																						|en = 'code of working conditions should be ZP12A, ZP12B or ZP12G in the additional entry.'"), НомерСтроки, Запись.СотрудникНаименование);
			СообщитьОбОшибкеСтажа(ТекстОшибки, Запись, Отказ);
		КонецЕсли;
		...
		Если (УсловияТрудаЗаписи = Справочники.ОсобыеУсловияТрудаДляСЗВКПФР.ЗП12Ж) 
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'в дополнительной записи код условий труда ЗП12Ж может быть указан 
					|только в случае указания в основной записи кода исчисляемого стажа УИК104.';
					|en = 'working condition code ZP12G can be specified in the additional entry 
					| only if estimated length of service code UIC104 is specified in the main entry.'"), НомерСтроки, Запись.СотрудникНаименование);
			СообщитьОбОшибкеСтажа(ТекстОшибки, Запись, Отказ);
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ОсновнаяЗапись.ОснованиеВыслугиЛет) Тогда
		Если СтрНайти(",,ЗП81СМ,ЗП81ГД,ХИРУРСМ,ХИРУРГД,",","+КодВыслугиЛетЗаписи+",") = 0 Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'если в основной записи не указан код выслуги лет, то в дополнительной записи
					|основание выслуги лет должно быть пустым или указаны коды ЗП81СМ,ЗП81ГД,ХИРУРСМ или ХИРУРГД.';
					|en = 'if the seniority code is not specified in the main entry, leave the seniority basis blank or enter codes ЗП81СМ,ЗП81ГД,ХИРУРСМ or ХИРУРГД in the additional entry 
					|.'"), НомерСтроки, Запись.СотрудникНаименование);
			СообщитьОбОшибкеСтажа(ТекстОшибки, Запись, Отказ);
		КонецЕсли;
	Иначе//В основной записи код выслуги указан
		...
		Если ПроверочнаяСтрокаКодов = "" Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'при указании в основной записи кода выслуги лет %3
					|в дополнительной записи код выслуги указываться не может.';
					|en = 'when specifying seniority code %3
					| in the main entry, you cannot specify it in the additional entry.'"), НомерСтроки, Запись.СотрудникНаименование, КодВыслугиЛетОсновнойЗаписи);
			СообщитьОбОшибкеСтажа(ТекстОшибки, Запись, Отказ);
		Иначе
			...
			Если НЕ СтрНайти(ПроверочнаяСтрокаКодов,","+КодВыслугиЛетЗаписи+",") Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'при указании в основной записи кода выслуги лет %3
						|в дополнительной могут быть указаны только следующие коды выслуги: %4';
						|en = 'if you specify seniority code %3
						| in the main entry, you can specify only the following seniority codes in the additional entry: %4'"), НомерСтроки, Запись.СотрудникНаименование, КодВыслугиЛетОсновнойЗаписи, Сред(ПроверочнаяСтрокаКодов,2));
				СообщитьОбОшибкеСтажа(ТекстОшибки, Запись, Отказ);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
				...
				Пока Выборка.СледующийПоЗначениюПоля("НомерДополнительнойЗаписи") Цикл
					...
					Если Не ЗначениеЗаполнено(Выборка.НомерПравила)
						...
						СтрокаСообщений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'комбинация значений реквизитов:
								...
								|en = 'combination of attribute values:
								| Seniority basis = %3
								| Basis of estimated length of service = %4
								|Special working conditions = %5
								|Code of list item = %6
								|Not valid.'"), Выборка.СотрудникНаименование, НомерСтроки, Выборка.УказанныйКодОснованияВыслугиЛет, Выборка.УказанныйКодОснованияИсчисляемогоСтажа,
					...
					Пока Выборка.Следующий() Цикл
						...
						Для Сч = 0 По 2 Цикл
							...
							Если (ФорматПараметра = "_") Тогда
								Если (ЗначениеПараметра <> 0) Тогда//ПУСТОЕ
									МассивСообщений.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'параметр исчисляемого стажа № %3 должен быть пустым (=0).';
																															|en = 'estimated length of service parameter No. %3 should be empty (=0).'"), Выборка.СотрудникНаименование, НомерСтроки, ТекстПараметра)); 
								Иначе	
									ПараметрПрошелПроверку = ИСТИНА;
								КонецЕсли;
							ИначеЕсли (ФорматПараметра = "Г") Тогда
								Если (ЗначениеПараметра > 100) Тогда//ГОДЫ
									МассивСообщений.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'параметр исчисляемого стажа № %3 должен содержать значение не > 100 (лет).';
																															|en = 'estimated length of service parameter No. %3 should contain value not > 100 (years).'"), Выборка.СотрудникНаименование, НомерСтроки, ТекстПараметра)); 
								Иначе	
									ПараметрПрошелПроверку = ИСТИНА;
								КонецЕсли;
							ИначеЕсли (ФорматПараметра = "М") Тогда 
								Если (ЗначениеПараметра > 11) Тогда//МЕСЯЦЫ
									МассивСообщений.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'параметр исчисляемого стажа № %3  должен содержать значение не > 11 (месяцев).';
																															|en = 'estimated length of service parameter No. %3 should contain value not > 11 (months).'"), Выборка.СотрудникНаименование, НомерСтроки, ТекстПараметра));
								Иначе	
									ПараметрПрошелПроверку = ИСТИНА;
								КонецЕсли;
							ИначеЕсли (ФорматПараметра = "Д") Тогда 
								Если (ЗначениеПараметра > 30) Тогда//ДНИ
									МассивСообщений.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'параметр исчисляемого стажа № %3 должен содержать значение не > 30 (дней).';
																															|en = 'estimated length of service parameter No. %3 should contain value not > 30 (days).'"), Выборка.СотрудникНаименование, НомерСтроки, ТекстПараметра));
								Иначе	
									ПараметрПрошелПроверку = ИСТИНА;
								КонецЕсли;
							...
							ИначеЕсли (ФорматПараметра = "м") Тогда 
								Если (ЗначениеПараметра > 59) Тогда//МИНУТЫ
									МассивСообщений.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'параметр исчисляемого стажа № %3 должен содержать значение не > 59 (минут).';
																															|en = 'estimated length of service parameter No. %3 should contain value not > 59 (minutes).'"),  Выборка.СотрудникНаименование, НомерСтроки, ТекстПараметра));
								Иначе	
									ПараметрПрошелПроверку = ИСТИНА;
								КонецЕсли;
							КонецЕсли;
						...
						Если (СокрЛП(ФорматФОВ) <> "___")И(СтрокаЗначенияПараметров="000") Тогда//
							...
							МассивСообщений.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'в параметрах (Годы,Месяцы,Дни) должно быть указано отработанное время.';
																													|en = 'hours worked should be specified in the parameters (Years,Months,Days).'"), НомерСтроки, Выборка.СотрудникНаименование));
						КонецЕсли;
...
Процедура ПроверитьДанныеДокумента(Отказ = Ложь, НеПроверяемыеРеквизиты = Неопределено) Экспорт 
	...
	Если Сотрудники.Количество() > 200 Тогда
		ТекстОшибки = НСтр("ru = 'В документе должно быть не более 200 сотрудников.';
							|en = 'Document should contain no more than 200 employees.'");
		...
		Если ЗначениеЗаполнено(ВыборкаЗаписиСтажа.Сотрудник) Тогда 
			Если Не ВыборкаЗаписиСтажа.СотрудникРаботаетВОрганизации Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %2 не работает в организации %3.';
																							|en = 'Employee %2 does not work for company %3.'"), ВыборкаЗаписиСтажа.НомерСтрокиСотрудник, ВыборкаЗаписиСтажа.СотрудникНаименование, ДанныеОрганизации.Наименование);
			...
			Если ВыборкаЗаписиСтажа.КонфликтующаяСтрока <> Null Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Информация о сотруднике %2 была введена в документе ранее.';
																							|en = 'Information on employee %2 has already been entered into the document.'"), ВыборкаЗаписиСтажа.НомерСтрокиСотрудник, ВыборкаЗаписиСтажа.СотрудникНаименование);
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаЗаписиСтажа.НомерСтрокиСотрудник - 1, "ЧН=0; ЧГ=0") + "].Сотрудник",,Отказ);
				Продолжить;
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(ВыборкаЗаписиСтажа.НомерСтрокиСтаж) Тогда
				...
				Пока ВыборкаЗаписиСтажа.СледующийПоЗначениюПоля("НомерСтрокиСтаж") Цикл
					...
					Если (СменилисьОрганизацияВидДеятельности)И(НЕ СменилсяНомерПериодаДеятельности) Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'в рамках одного периода деятельности организация и вид деятельности меняться не должны.';
																									|en = 'within one activity period a company and an activity category should not change.'"), НомерСтроки, Запись.СотрудникНаименование);
					...
					Если Не ЗначениеЗаполнено(Запись.ДатаНачалаПериода) И Не ЗначениеЗаполнено(Запись.ДатаОкончанияПериода) Тогда
						...
						Иначе
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'не указаны даты начала и окончания периода.';
																										|en = 'dates of period start and end are not specified.'"), НомерСтроки, Запись.СотрудникНаименование);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
					ИначеЕсли Не ЗначениеЗаполнено(Запись.ДатаНачалаПериода) Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'при указании даты окончания периода необходимо указывать дату начала периода.';
																									|en = 'if you specify period end date, you should specify period start date.'"), НомерСтроки, Запись.СотрудникНаименование);
						СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
					ИначеЕсли Не ЗначениеЗаполнено(Запись.ДатаОкончанияПериода) Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'при указании даты начала периода необходимо указывать дату окончания периода.';
																									|en = 'if you specify period start date, you should specify period end date.'"), НомерСтроки, Запись.СотрудникНаименование);
						СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
					Иначе//Обе даты заполнены
						
						// Дата окончания периода не должна быть позднее 31.12.2001
						ДатаОкончанияМакс = '20011231';
						Если Запись.ФактПроживанияВКрыму Тогда 
							ДатаОкончанияМакс = '20151231';
						ИначеЕсли Запись.ФактПроживанияДНРЛНРЗОХО Или Запись.ФактПроживанияДНРЛНР Или Запись.ФактПроживанияЗОХО Тогда 
							ДатаОкончанияМакс = '20221231';
						КонецЕсли;
						Если Запись.ДатаОкончанияПериода > ДатаОкончанияМакс Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'дата окончания периода должна быть не позднее %1';
																										|en = 'period end date must be not later than %1'"), Формат(ДатаОкончанияМакс, "ДЛФ=ДД"));
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
						
						// Начало периода не должно быть позже окончания периода 
						Если Запись.ДатаНачалаПериода > Запись.ДатаОкончанияПериода Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'дата начала периода не должна превышать дату окончания периода.';
																										|en = 'period start date cannot be greater than end date.'"), НомерСтроки, Запись.СотрудникНаименование);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
						
						// Начало периода не должно быть слишком старой
						Если Запись.ДатаНачалаПериода < Дата('19000101') Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'дата начала периода слишком старая.';
																										|en = 'period start date is too old.'"), НомерСтроки, Запись.СотрудникНаименование);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
						...
						Если ГОД(Запись.ДатаНачалаПериода) <> ГОД (Запись.ДатаОкончанияПериода) Тогда
							...
							Если (Запись.ОснованиеИсчисляемогоСтажа = Справочники.ОснованияИсчисляемогоСтраховогоСтажа.ВОДОЛАЗ) 
								...
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для оснований исчисляемого стажа ВОДОЛАЗ и СЕЗОН даты начала и окончания периода должны находиться в пределах одного календарного года.';
																											|en = 'for basis of the calculated DIVER and SEASON length of service start date and end date of the period must be within one calendar year.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
							
							Если НЕ ((Запись.ПервыйПараметрВыслугиЛет = 0) И (Запись.ВторойПараметрВыслугиЛет  = 0) И (Запись.ТретийПараметрВыслугиЛет  = 0)) Тогда 
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'при указании параметров 1-3 выслуги лет даты начала и окончания периода должны находиться в пределах одного календарного года.';
																											|en = 'if you specify parameters 1-3 of seniority, period start date and period end date should be within the limits of one calendar year.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						КонецЕсли;
					...
					Если ЭтоОсновнаяЗапись Тогда
						...
						Если ТекущаяОсновнаяЗапись.Организация <> Неопределено Тогда
							Если Запись.ДатаНачалаПериода <= ТекущаяОсновнаяЗапись.ДатаОкончанияПериода Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'дата начала периода должна быть больше даты окончания предыдущего периода.';
																											|en = 'period start date must be greater than end date of the previous period. '"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						КонецЕсли;
					...
					Если СменилсяНомерПериодаДеятельности Тогда
						Если (Запись.НомерПериодаТрудовойДеятельности <> ОжидаемыйНомерПериода)  Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'неверный номер периода трудовой деятельности. Ожидается номер: %3.';
																										|en = 'incorrect number of the labor activity period. Awaiting number: %3.'"), НомерСтроки, Запись.СотрудникНаименование, ОжидаемыйНомерПериода);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
					...
					Если (ЭтоОсновнаяЗапись) Тогда
						...
						Если (Запись.НомерОсновнойЗаписи <> ОжидаемыйНомерОсновнойЗаписи)  Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'неверный номер основной записи. Ожидается номер: %3.';
																										|en = 'incorrect main entry number. Awaiting number: %3.'"), НомерСтроки, Запись.СотрудникНаименование, ОжидаемыйНомерОсновнойЗаписи);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
					...
					Иначе
						...
						Если (Запись.НомерДополнительнойЗаписи <> ОжидаемыйНомерДополнительнойЗаписи)  Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'неверный номер дополнительной записи. Ожидается номер: %3.';
																										|en = 'incorrect additional entry number. Awaiting number: %3.'"), НомерСтроки, Запись.СотрудникНаименование, ОжидаемыйНомерДополнительнойЗаписи);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
					КонецЕсли;
					...
					ВидДеятельностиРабота = Новый Массив;
					ВидДеятельностиРабота.Добавить(Справочники.ВидыОбщественноПолезнойДеятельностиСЗВК.РАБОТА);
					ВидДеятельностиРабота.Добавить(Справочники.ВидыОбщественноПолезнойДеятельностиСЗВК.ТДКРЫМ);
					ВидДеятельностиРабота.Добавить(Справочники.ВидыОбщественноПолезнойДеятельностиСЗВК.ИПКРЫМ);
					ВидДеятельностиРабота.Добавить(Справочники.ВидыОбщественноПолезнойДеятельностиСЗВК.ДОГКРЫМ);
					ВидДеятельностиРабота.Добавить(Справочники.ВидыОбщественноПолезнойДеятельностиСЗВК.ТДДЛ);
					ВидДеятельностиРабота.Добавить(Справочники.ВидыОбщественноПолезнойДеятельностиСЗВК.ТДУЗХ);
					ВидДеятельностиРабота.Добавить(Справочники.ВидыОбщественноПолезнойДеятельностиСЗВК.ДОГИП);
					ВидДеятельностиРабота.Добавить(Справочники.ВидыОбщественноПолезнойДеятельностиСЗВК.ТДДЛКОМИС);
					ВидДеятельностиРабота.Добавить(Справочники.ВидыОбщественноПолезнойДеятельностиСЗВК.ТДУЗХКОМИС);
					ВидДеятельностиРабота.Добавить(Справочники.ВидыОбщественноПолезнойДеятельностиСЗВК.ДОГИПКОМИС);
					ВидДеятельностиРабота.Добавить(Справочники.ВидыОбщественноПолезнойДеятельностиСЗВК.РАБОТАКОМИС);
					
					Если ВидДеятельностиРабота.Найти(Запись.ВидДеятельности) = Неопределено Тогда
						Если ЗаданаГруппаТерриториальныеУсловия ИЛИ ЗаданаГруппаУсловияТруда ИЛИ ЗаданаГруппаИсчисляемыйСтаж ИЛИ ЗаданаГруппаФОВ
							...
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для записей с видом деятельности, отличным от РАБОТА, должен быть указан только период.';
																										|en = 'specify only period for entries with activity category which is not WORK.'"), НомерСтроки, Запись.СотрудникНаименование);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
						
						Если НЕ ЭтоОсновнаяЗапись Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'дополнительные записи для видов деятельности, отличных от РАБОТА, недопустимы.';
																										|en = 'additional entries for activity categories different from WORK are prohibited.'"), НомерСтроки, Запись.СотрудникНаименование);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
					КонецЕсли;
					...
					Если ЗаданаГруппаТерриториальныеУсловия Тогда
						...
						Если СписокТерриториальныеУсловияЗаписи.НайтиПоЗначению(ПредставлениеЗначения) <> Неопределено Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'указанные в дополнительной записи территориальные условия %3 не должны повторяться в рамках одной основной записи.';
																										|en = 'site conditions %3 specified in the additional entry shall not be repeated within one main entry.'"), НомерСтроки, Запись.СотрудникНаименование, ПредставлениеЗначения);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						Иначе
					...
					Если УказаныОсобыеУсловияТруда Тогда
						...
						Если СписокУсловияТрудаЗаписи.НайтиПоЗначению(ПредставлениеЗначения) <> Неопределено Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'указанный в дополнительной записи код условий труда %3 не должен повторяться в рамках одной основной записи.';
																										|en = 'working conditions code %3 specified in the additional entry shall not be repeated within one main entry.'"), НомерСтроки, Запись.СотрудникНаименование, ПредставлениеЗначения);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						Иначе
					...
					Если УказаноОснованиеВыслугиЛет Тогда
						...
						Если СписокВыслугаЛетЗаписи.НайтиПоЗначению(ПредставлениеЗначения) <> Неопределено Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'указанный в дополнительной записи код выслуги лет %3 не должен повторяться в рамках одной основной записи.';
																										|en = 'seniority code %3 specified in the additional entry shall not be repeated within one main entry.'"), НомерСтроки, Запись.СотрудникНаименование, ПредставлениеЗначения);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						Иначе
					...
					Если УказаныТерриториальныеУсловия Тогда
						Если Запись.ТерриториальныеУсловия = Справочники.ТерриториальныеУсловияПФР.РКС
							...
							Или Запись.ТерриториальныеУсловия = Справочники.ТерриториальныеУсловияПФР.РКСР
							Или Запись.ТерриториальныеУсловия = Справочники.ТерриториальныеУсловияПФР.МКСРКСР
							Или Запись.ТерриториальныеУсловия = Справочники.ТерриториальныеУсловияПФР.МКСР Тогда
							
							Если НЕ УказанРК Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для территориальных условий РКС, МКС, РКСР, МКС-РКСР, МКСР и РКСМ должен быть указан районный коэффициент.';
																											|en = 'regional factor must be specified for site conditions of regions of the Far North and equated localities.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						Иначе
							Если УказанРК Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для территориальных условий, отличных от РКС, МКС, РКСР, МКС-РКСР, МКСР и РКСМ, районный коэффициент указываться не должен.';
																											|en = 'regional factor must not be specified for site conditions which differ from the regions of the Far North and equated localities.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
					...
					Если (УказанРК) И НЕ(Запись.РайонныйКоэффициент >= 1.01 И Запись.РайонныйКоэффициент <= 2.00) Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'районный коэффициент должен быть в диапазоне от 1.01 до 2.00.';
																									|en = 'regional factor must be in the range from 1.01 to 2.00.'"), НомерСтроки, Запись.СотрудникНаименование);
						СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
					КонецЕсли;
					...
					Если УказанКодПозицииСписка Тогда 
						Если НЕ УказаныОсобыеУсловияТруда Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'код позиции списка не может указываться без указания кода особых условий труда.';
																										|en = 'list position code cannot be specified without specifying the code of special working conditions.'"), НомерСтроки, Запись.СотрудникНаименование);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
						...
						Если (ДлинаКПС=8)ИЛИ(ДлинаКПС=14) Тогда
							...
							Иначе	
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'неверный код позиции списка вредных профессий.';
																											|en = 'incorrect code of unhealthy occupation list position.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						...
						Иначе	
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'длина кода позиции списка должна быть равна либо 5, либо 8, либо 14 символам.';
																										|en = 'the length of list position code should be equal to 5, or 8, or 14 characters.'"), НомерСтроки, Запись.СотрудникНаименование);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
					КонецЕсли;
					...
					Если УказаныОсобыеУсловияТруда Тогда
						...
						Если Запись.ОсобыеУсловияТруда = Справочники.ОсобыеУсловияТрудаДляСЗВКПФР.ЗП12А Тогда
							Если (НазваниеСписка <> "Список№1") Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для особых условий труда с кодом %3 должен быть указан код позиции Списка №1.';
																											|en = 'specify a position code of List No. 1 for special working conditions with code %3.'"), НомерСтроки, Запись.СотрудникНаименование, КодОсобыхУсловийТруда);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						...
						ИначеЕсли Запись.ОсобыеУсловияТруда = Справочники.ОсобыеУсловияТрудаДляСЗВКПФР.ЗП12Б Тогда
							...
							Если (НЕ УказанКодПозицииСписка) Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для особых условий труда с кодом %3 должен быть указан код позиции Списка №1 или Списка №2.';
																											|en = 'specify position code of List No. 1 or List No. 2 for special working conditions with code %3.'"), НомерСтроки, Запись.СотрудникНаименование, КодОсобыхУсловийТруда);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
							
							Если (НазваниеСписка = "Список№1") Тогда
								Если (ВРЕГ(Первые8СимволовКПС) <> "1010300Б")И(ВРЕГ(Первые8СимволовКПС) <> "1010100Г")И(ВРЕГ(Первые8СимволовКПС) <> "1010100Д")  Тогда
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для особых условий труда с кодом %3 указан неверный код позиции списка.';
																												|en = 'invalid code of the list position is specified for special working conditions with code %3.'"), НомерСтроки, Запись.СотрудникНаименование, КодОсобыхУсловийТруда);
									СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
								КонецЕсли;
						...
						ИначеЕсли Запись.ОсобыеУсловияТруда = Справочники.ОсобыеУсловияТрудаДляСЗВКПФР.ЗП12Г Тогда
							Если НЕ ((НЕ УказанКодПозицииСписка)ИЛИ(НазваниеСписка="ПрочиеСписки")) Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для особых условий труда с кодом %3 код позиции списка должен быть либо пустым либо принадлежать списку по текстильной промышленности.';
																											|en = 'code of the list position should either be empty or belong to textile industry list for special working conditions with code %3.'"), НомерСтроки, Запись.СотрудникНаименование, КодОсобыхУсловийТруда);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
							
						ИначеЕсли Запись.ОсобыеУсловияТруда = Справочники.ОсобыеУсловияТрудаДляСЗВКПФР.ЗП12Д Тогда
							Если НЕ ((НЕ УказанКодПозицииСписка)ИЛИ(НазваниеСписка="ПрочиеСписки")) Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для особых условий труда с кодом %3 код позиции списка должен быть либо пустым либо принадлежать списку железнодорожников.';
																											|en = 'code of the list position should either be empty or belong to the list of railway personnel for special working conditions with code %3.'"), НомерСтроки, Запись.СотрудникНаименование, КодОсобыхУсловийТруда);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
							
						ИначеЕсли Запись.ОсобыеУсловияТруда = Справочники.ОсобыеУсловияТрудаДляСЗВКПФР.ЗП12Ж Тогда
							Если НЕ ((НЕ УказанКодПозицииСписка)ИЛИ(НазваниеСписка="ПрочиеСписки")) Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для особых условий труда с кодом %3 код позиции списка должен быть либо пустым либо принадлежать списку лесозаготовителей.';
																											|en = 'code of the list position should either be empty or belong to the logger list for special working conditions with code %3.'"), НомерСтроки, Запись.СотрудникНаименование, КодОсобыхУсловийТруда);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
							
						ИначеЕсли Запись.ОсобыеУсловияТруда = Справочники.ОсобыеУсловияТрудаДляСЗВКПФР.ЗП12М Тогда
							Если НЕ ((НЕ УказанКодПозицииСписка)ИЛИ(НазваниеСписка="ПрочиеСписки")) Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для особых условий труда с кодом %3 код позиции списка должен быть либо пустым либо принадлежать списку по работе с осужденными.';
																											|en = 'code of the list position should either be empty or belong to the work with convinced persons list for special working conditions with code %3.'"), НомерСтроки, Запись.СотрудникНаименование, КодОсобыхУсловийТруда);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
							
						ИначеЕсли Запись.ОсобыеУсловияТруда = Справочники.ОсобыеУсловияТрудаДляСЗВКПФР.ЗП12О Тогда
							Если НЕ ((НЕ УказанКодПозицииСписка)ИЛИ(НазваниеСписка="ПрочиеСписки")) Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для особых условий труда с кодом %3 код позиции списка должен быть либо пустым либо принадлежать списку должностей противопожарной службы.';
																											|en = 'code of the list position should either be empty or belong to the list of fire service positions for special working conditions with code %3.'"), НомерСтроки, Запись.СотрудникНаименование, КодОсобыхУсловийТруда);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						...
						Иначе
							...
							Если УказанКодПозицииСписка Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для особых условий труда с кодом %3 код позиции списка указываться не должен.';
																											|en = 'do not specify code of the list position for special working conditions with code %3.'"), НомерСтроки, Запись.СотрудникНаименование, КодОсобыхУсловийТруда);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						КонецЕсли;
					...
					Если УказаноОснованиеВыслугиЛет Тогда
						...
						Если (Запись.ОснованиеВыслугиЛет = Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ЗП81СМ) 
							...
							Если ТекущаяОсновнаяЗапись.ДатаОкончанияПериода < Дата('19991101') Тогда
								...
								Если ПараметрыУказаны Тогда
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %3 до 01.11.1999 года параметры 1-3 указываться не должны.';
																												|en = 'parameters 1-3 should not be specified for seniority basis %3 before 11/01/1999.'"), НомерСтроки, Запись.СотрудникНаименование, КодОснования);
									СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
								КонецЕсли;
								
							Иначе//после 1 ноября 1999 г
								Если ПерсонифицированныйУчет.ДоляСтавкиУказанаНеверно(Запись.ТретийПараметрВыслугиЛет) Тогда
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %3 после 01.11.1999 третий параметр должен содержать долю ставки (число от 0.1 до 2)';
																												|en = 'the third parameter should contain rate share (number from 0.1 to 2) for seniority basis %3 after 11/01/1999'"), НомерСтроки, Запись.СотрудникНаименование, КодОснования);
									СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
								ИначеЕсли НЕ УказанТолькоТретийПараметр Тогда
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %3 после 01.11.1999 должен указываться только третий параметр.';
																												|en = 'specify only the third parameter for seniority basis %3 after 11/01/1999.'"), НомерСтроки, Запись.СотрудникНаименование, КодОснования);
									СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
								КонецЕсли;
							КонецЕсли;
						...
						ИначеЕсли Запись.ОснованиеВыслугиЛет = Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ЗП80ПД Тогда
							...
							Если ТекущаяОсновнаяЗапись.ДатаОкончанияПериода < Дата('20000901') Тогда
								// до 1 сентября 2000г
								Если  ПараметрыУказаны Тогда
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %3  до 01.09.2000 года параметры 1-3 указываться не должны.';
																												|en = 'parameters 1-3 should not be specified for seniority basis %3 before 09/01/2000.'"), НомерСтроки, Запись.СотрудникНаименование, КодОснования);
									СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
								КонецЕсли;
							Иначе//после 1 сентября 2000г
								Если УказанПараметр2 Тогда
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %3 после 01.09.2000 второй параметр указываться не должен.';
																												|en = 'The second parameter should not be specified for seniority basis %3 after 09/01/2000.'"), НомерСтроки, Запись.СотрудникНаименование, КодОснования);
									СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
								ИначеЕсли ПерсонифицированныйУчет.ДоляСтавкиУказанаНеверно(Запись.ТретийПараметрВыслугиЛет) Тогда
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %3 после 01.09.2000 третий параметр должен содержать долю ставки (число от 0.1 до 2).';
																												|en = 'the third parameter should contain rate share (number from 0.1 to 2) for seniority basis %3 after 09/01/2000.'"), НомерСтроки, Запись.СотрудникНаименование, КодОснования);
									СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
								КонецЕсли;
							КонецЕсли;
						...
						ИначеЕсли Запись.ОснованиеВыслугиЛет = Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ЗП80РК Тогда
							...
							Если ТекущаяОсновнаяЗапись.ДатаОкончанияПериода < Дата('20000901') Тогда
								// до 1 сентября 2000г
								Если  ПараметрыУказаны Тогда
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %3 до 01.09.2000 параметры 1-3 указываться не должны.';
																												|en = 'parameters 1-3 should not be specified for seniority basis %3 before 09/01/2000.'"), НомерСтроки, Запись.СотрудникНаименование, КодОснования);
									СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
								КонецЕсли;
							Иначе//после 1 сентября 2000г
								Если (УказанПараметр2)ИЛИ(УказанПараметр3) Тогда
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %3 после 01.09.2000 может быть указан только первый параметр.';
																												|en = 'Only the first parameter can be specified for seniority basis %3 after 09/01/2000.'"), НомерСтроки, Запись.СотрудникНаименование, КодОснования);
									СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
								КонецЕсли;
							КонецЕсли;
						...
						ИначеЕсли (Запись.ОснованиеВыслугиЛет = Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.САМОЛЕТ) 
							...
							Если  УказанПараметр3 Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %3 параметр 3 указываться не должен';
																											|en = 'parameter 3 should not be specified for seniority basis %3'"), НомерСтроки, Запись.СотрудникНаименование, КодОснования);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
							
							Если  (НЕ УказанПараметр1) И (НЕ УказанПараметр2) Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %3 параметры 1 и 2 должны содержать отработанное время (часы и минуты).';
																											|en = 'parameters 1 and 2 should contain hours worked (hours and minutes) for seniority basis %3.'"), НомерСтроки, Запись.СотрудникНаименование, КодОснования);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						...
						ИначеЕсли (Запись.ОснованиеВыслугиЛет = Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.РЕАКТИВН)
							...
							Если (УказанПараметр2) ИЛИ (УказанПараметр3) Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %3 параметры 2 и 3 указываться не должны.';
																											|en = 'parameters 2 and 3 should not be specified for seniority basis %3.'"), НомерСтроки, Запись.СотрудникНаименование, КодОснования);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						...
						Иначе
							Если  ПараметрыУказаны Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %3 параметры выслуги лет 1,2,3 указываться не должны.';
																											|en = 'seniority parameters 1,2,3 should not be specified for seniority basis %3.'"), НомерСтроки, Запись.СотрудникНаименование, КодОснования);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						КонецЕсли;
					Иначе
						Если ПараметрыУказаны Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %3 Параметры выслуги лет 1,2,3 без указания основания выслуги лет указываться не должны.';
																										|en = 'Seniority parameters 1,2,3 without specified basis should not be specified for seniority basis %3.'"), НомерСтроки, Запись.СотрудникНаименование, КодОснования);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
					КонецЕсли;
					...
					Если УказанТретийПараметрИсчисляемогоСтажа Тогда
						Если (Запись.ТретийПараметрИсчисляемогоСтажа <> Справочники.ПараметрыИсчисляемогоСтраховогоСтажа.ДЕТИ) И (Запись.ТретийПараметрИсчисляемогоСтажа <> Справочники.ПараметрыИсчисляемогоСтраховогоСтажа.ДЕКРЕТ) Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'третий параметр исчисляемого стажа должен содержать значение ДЕТИ или ДЕКРЕТ.';
																										|en = 'the third parameter of the estimated length of service should contain value CHILDREN or MATERNITY.'"), НомерСтроки, Запись.СотрудникНаименование);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						ИначеЕсли (Запись.ТретийПараметрИсчисляемогоСтажа = Справочники.ПараметрыИсчисляемогоСтраховогоСтажа.ДЕТИ) Тогда
							Если (УказаноОснованиеИсчисляемогоСтажа) ИЛИ (ЗаданаГруппаФОВ) ИЛИ (УказаноОснованиеВыслугиЛет)  Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'при указании третьего параметра исчисляемого стажа следующие группы реквизитов должны быть пустыми:
										| Исчисляемый стаж: основание, Фактически отработанное время, Выслуга лет.';
										|en = 'if you specify the third parameter of the estimated length of service, the following attribute groups must be empty:
										| Estimated length of service: basis, Actual period worked, Seniority.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
							Если ((ЗаданаГруппаУсловияТруда)ИЛИ(ЗаданаГруппаТерриториальныеУсловия)) Тогда
								...
								|""Условия труда, Территориальные условия"" указываться не должны.';
								|en = 'if you specify the CHILDREN value in the third parameter of estimated length of service, you cannot specify the ""Working conditions, Site conditions"" attribute groups
								|.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						КонецЕсли;
					...
					Если Запись.ОснованиеИсчисляемогоСтажа = Справочники.ОснованияИсчисляемогоСтраховогоСтажа.СЕЗОН Тогда
						...
						Иначе
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'основание исчисляемого стажа СЕЗОН в течение календарного года может указываться не более одного раза.';
																										|en = 'basis of the SEASON length of service calculated during a calendar year can be specified only once.'"), НомерСтроки, Запись.СотрудникНаименование);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
					КонецЕсли;
					...
					Если ЗаданаГруппаФОВ Тогда
						Если (НЕ УказаныОсобыеУсловияТруда) И (НЕ УказаноОснованиеИсчисляемогоСтажа)
							...
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'параметры 0,1,2 исчисляемого стажа должны указываться только при заполнении других групп реквизитов
									|(Территориальные условия, Условия труда, Исчисляемый стаж: основание).';
									|en = 'parameters 0,1,2 of the estimated length of service must be specified only when filling other attribute groups
									| (Site conditions, Working conditions, Estimated length of service: basis).'"), НомерСтроки, Запись.СотрудникНаименование);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
						...
						Если Запись.ОснованиеИсчисляемогоСтажа <> Справочники.ОснованияИсчисляемогоСтраховогоСтажа.ВОДОЛАЗ Тогда
							...
							Если Запись.ПервыйПараметрИсчисляемогоСтажа > 11 Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'первый параметр исчисляемого стажа (Месяцы) не должен быть больше 11.';
																											|en = 'first parameter of estimated length of service (Months) should not be more than 11.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
							Если Запись.ВторойПараметрИсчисляемогоСтажа > 30 Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'второй параметр исчисляемого стажа (Дни) не должен быть больше 30.';
																											|en = 'the second parameter of the estimated length of service (Days) cannot be more than 30.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
							...
							Если (ФОВДляСравнения > КалендарноеВремя) Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'указанное фактически отработанное время превышает календарное время.';
																											|en = 'the specified actual hours worked exceed the calendar time.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						КонецЕсли;
		...
		Пока ВыборкаСведений.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(Запись.ВидСтажа) Тогда
				...
				Если (Запись.Лет + Запись.Месяцев + Запись.Дней) = 0 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'значения реквизитов ""Лет, Месяцев, Дней"" не должны одновременно равняться нулю.';
																								|en = 'the ""Years, Months, Days"" attribute values should not be zero at the same time. '"), НомерСтроки, Запись.СотрудникНаименование);
				...
				Если Запись.ВидСтажа = Перечисления.ВидыТрудовогоСтажа.ОбщийТрудовойСтаж Тогда
					...
					Если КоличествоЗаписейОбОбщемТрудовомСтаже > 1 Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'записей об общем стаже должно быть не больше одной.';
																									|en = 'there must be only one entry on total length of service'"), НомерСтроки, Запись.СотрудникНаименование);
						СообщитьОбОшибкеСтажаИтог(ТекстОшибки, ВыборкаСведений, НомерСтроки, Отказ);
					КонецЕсли;
				...
				Иначе
					...
					Если Запись.КодСтажа = Неопределено Тогда
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'не указан код стажа.';
																									|en = 'length of service code is not specified.'"), НомерСтроки, Запись.СотрудникНаименование);     
						СообщитьОбОшибкеСтажаИтог(ТекстОшибки, ВыборкаСведений, НомерСтроки, Отказ);
					...
					Иначе//Код стажа заполнен
						...
						Если СписокПроверенныхКодовСтажа.НайтиПоЗначению(КлючПоискаДублей) <> Неопределено Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'указанный вид стажа ""%3"" и код стажа ""%4"" не должны повторяться.';
																										|en = 'the specified service kind ""%3"" and service code ""%4"" cannot be duplicated.'"), НомерСтроки, Запись.СотрудникНаименование, Запись.ВидСтажа, Запись.КодСтажаКод);
							СообщитьОбОшибкеСтажаИтог(ТекстОшибки, ВыборкаСведений, НомерСтроки, Отказ);
						Иначе	
						...
						Если Запись.ВидСтажа = Перечисления.ВидыТрудовогоСтажа.ТерриториальныеУсловия Тогда	
							...
							Если ТипЗнч(Запись.КодСтажа)<>Тип("СправочникСсылка.ТерриториальныеУсловияПФР") Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'указанный код территориального стажа не соответствует классификатору.';
																											|en = 'the specified territorial service code does not correspond with the classifier.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажаИтог(ТекстОшибки, ВыборкаСведений, НомерСтроки, Отказ);
							КонецЕсли;
							
							КоличествоЗаписейТЕРР = КоличествоЗаписейТЕРР +1;
							Если КоличествоЗаписейТЕРР > 10 Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'количество записей с расчетом стажа по территориальным условиям должно быть не более 10.';
																											|en = 'number of entries with calculation of length of service by territorial working conditions cannot be more than 10.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажаИтог(ТекстОшибки, ВыборкаСведений, НомерСтроки, Отказ);
							КонецЕсли;
						...
						ИначеЕсли Запись.ВидСтажа = Перечисления.ВидыТрудовогоСтажа.ОсобыеУсловияТруда Тогда	
							...
							Если ТипЗнч(Запись.КодСтажа)<>Тип("СправочникСсылка.ОсобыеУсловияТрудаДляСЗВКПФР") Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'указанный код условий труда не соответствует классификатору.';
																											|en = 'the specified working conditions code does not correspond with the one from the classifier.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажаИтог(ТекстОшибки, ВыборкаСведений, НомерСтроки, Отказ);
							КонецЕсли;
							
							КоличествоЗаписейОСОБ = КоличествоЗаписейОСОБ +1;
							Если КоличествоЗаписейОСОБ > 10 Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'количество записей с расчетом стажа по особым условиям труда должно быть не более 10.';
																											|en = 'number of entries with calculation of length of service by special working conditions cannot be more than 10.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажаИтог(ТекстОшибки, ВыборкаСведений, НомерСтроки, Отказ);
							КонецЕсли;
						...
						ИначеЕсли Запись.ВидСтажа = Перечисления.ВидыТрудовогоСтажа.ВыслугаЛет Тогда	
							...
							Если ТипЗнч(Запись.КодСтажа)<>Тип("СправочникСсылка.ОснованияДосрочногоНазначенияПенсииДляСЗВК") Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'указанный код выслуги лет не соответствует классификатору.';
																											|en = 'the specified seniority code does not correspond to the classifier.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажаИтог(ТекстОшибки, ВыборкаСведений, НомерСтроки, Отказ);
							КонецЕсли;
							
							КоличествоЗаписейВЫСЛ = КоличествоЗаписейВЫСЛ +1;
							Если КоличествоЗаписейВЫСЛ > 10 Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'количество записей с расчетом стажа по выслуге лет должно быть не более 10.';
																											|en = 'number of entries with calculation of length of service by seniority cannot be more than 10.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажаИтог(ТекстОшибки, ВыборкаСведений, НомерСтроки, Отказ);
							КонецЕсли;
						...
						Если МассивНайденныхСтрокСтажа.Количество() = 0 Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'указанный код стажа ""%3"" не указан ни в одном из периодов трудовой деятельности.';
																										|en = 'the specified service code ""%3"" is not indicated in any of the labor activity periods.'"), НомерСтроки, Запись.СотрудникНаименование, Запись.КодСтажа);
							СообщитьОбОшибкеСтажаИтог(ТекстОшибки, ВыборкаСведений, НомерСтроки, Отказ);
						КонецЕсли;
...
Процедура СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписейСтажа, Отказ) Экспорт
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %1, записи о стаже, № основной записи %2 %3: %4';
																					|en = 'Employee %1, entries about length of service, No. of the main entry %2 %3: %4'"), ВыборкаЗаписейСтажа.СотрудникНаименование, ВыборкаЗаписейСтажа.НомерОсновнойЗаписи, ?(ЗначениеЗаполнено(ВыборкаЗаписейСтажа.НомерДополнительнойЗаписи), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '№ дополнительной записи %1';
																																																																																											|en = 'Additional entry %1 No.'"), ВыборкаЗаписейСтажа.НомерДополнительнойЗаписи), ""), ТекстОшибки);
...
Процедура СообщитьОбОшибкеСтажаИтог(ТекстОшибки, ВыборкаЗаписейСтажа, НомерСтроки, Отказ) Экспорт
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %1, в строке %2 ""Сведения для оценки пенсионных прав"": %3';
																					|en = 'Employee %1, in line %2 ""Information for evaluation of pension rights"": %3'"), ВыборкаЗаписейСтажа.СотрудникНаименование, НомерСтроки, ТекстОшибки );
	ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаЗаписейСтажа.НомерСтрокиСотрудник - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", ,Отказ);
