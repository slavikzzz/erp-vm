...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТребуетсяМенюКомандЗаполнения Тогда 
		...
		ОписаниеКоманды = Новый Структура("Имя,Заголовок,Представление", "ЗаполнитьВсеПозиции", НСтр("ru = 'Заполнить все позиции';
																									|en = 'Fill in all positions'"), НСтр("ru = 'Заполнено всеми позициями';
																																			|en = 'Filled in with all positions'"));
		КомандыЗаполнения.Вставить(0, ОписаниеКоманды);
		ОписаниеКоманды = Новый Структура("Имя,Заголовок,Представление", "ЗаполнитьОстальныеПозиции", НСтр("ru = 'Заполнить остальные позиции';
																											|en = 'Fill in remaining positions'"), НСтр("ru = 'Заполнено остальными позициями';
																																						|en = 'Filled in with remaining positions'"));
...
Процедура ПоказателиПоказательОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.ИндексируемыеПоказатели.НайтиСтроки(Новый Структура("Показатель", ВыбранноеЗначение)).Количество() > 0 
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Такой показатель уже выбран';
														|en = 'Such indicator has already been selected'"));
...
Процедура ДобавитьЭлементыПоказателя(СтрокаПоказателя, ГруппаПоказателиПозиций)
	...
	Если ФОИспользоватьВилкуСтавокВШтатномРасписании Тогда
		ГруппаЗначенияПоказателя.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1(мин) / %1(макс)';
																											|en = '%1(min) / %1(max)'"), СтрокаПоказателя.Заголовок);
...
Процедура ОбновитьЗаголовокГруппыФОТ(Форма, ГруппаФОТ)
	...
	Если ФОИспользоватьВилкуСтавокВШтатномРасписании Тогда		
		ГруппаФОТ.Заголовок = НСтр("ru = 'ФОТ(мин) / ФОТ(макс)';
									|en = 'Salary budget(min)/ salary budget(max)'");
	Иначе
				...
				Если НЕ ТекущиеЗначенияПоказателей = Неопределено Тогда
					...
					Если ПоказательИспользуется Тогда
						...
						Если ФОИспользоватьВилкуСтавокВШтатномРасписании Тогда
							...
							ПредставлениеТекущегоМин = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'было: %1';
							...
							ПредставлениеТекущегоМакс = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'было: %1';
