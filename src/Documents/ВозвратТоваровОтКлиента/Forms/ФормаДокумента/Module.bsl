	...
	Если ЗначениеЗаполнено(Объект.ЧекККМ) Тогда
		Если СменаЧекаККМОткрыта(Объект.ЧекККМ) Тогда
			...
			ВызватьИсключение НСтр("ru = 'Кассовая смена открыта! Ввод на основании чека документа ""Возврат товаров от клиента"" при открытой кассовой смене запрещен.';
									|en = 'POS shift is opened. Input on the basis of receipt of document ""Sales return"" is prohibited when the POS shift is opened.'");
		КонецЕсли;
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Перезаполнить возвращаемые товары по выбранному чеку ККМ?';
				|en = 'Refill goods pending return by the selected receipt?'"),
			ВариантыОтветов);
		Возврат;
	...
	ОткрытьФорму(
		...
		Новый Структура("СписокДокументов, Заголовок",
			...
			НСтр("ru = 'Реализации клиентам (%КоличествоДокументов%)';
				|en = 'Sales to the customers (%КоличествоДокументов%)'")
		),
	...
	Если ВозвратТоваровЧерезКомиссионера
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Конечный клиент"" не заполнено';
				|en = 'The ""End customer"" field is not filled in'"),
	...
	Если Не ИспользоватьПартнеровКакКонтрагентов 
		...
		Если НЕ ЗначениеЗаполнено(Объект.КлиентКонтрагент) Тогда
			...
			НСтр("ru = 'Поле ""Контрагент конечного клиента"" не заполнено';
				|en = '""End customer counterparty"" is not filled in'"),
...
Процедура ЗаполнитьРеализацииИЦены(Команда)
	...
	Если Объект["Товары"].Количество() = 0 Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'В документе не заполнен список Товары. Документы продажи и цены не могут быть заполнены.';
									|en = 'The ""Goods"" list in the document is not filled in. Cannot fill in sales documents and prices.'");
...
Процедура РассчитатьСуммуКомпенсации(Форма)
	...
	Форма.УменьшенДолгСтрокой = НСтр("ru = 'Уменьшен долг клиента:';
									|en = 'Customer receivable is reduced:'") + " " + Формат(Форма.РаспределеннаяСуммаРасшифровки, "ЧДЦ=2; ЧН=0,00")
		+ " " + Форма.Объект.Валюта;
