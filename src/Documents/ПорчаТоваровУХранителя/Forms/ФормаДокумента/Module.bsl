...
Процедура ВидЦеныПриИзменении(Элемент)
	...
	ТекстВопроса = СтрШаблон(НСтр("ru = 'Заполнить цены по виду цен ""%1""?';
									|en = 'Fill in prices by price type ""%1""?'"), Объект.ВидЦены);
...
Процедура ТоварыНоменклатураОприходованиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Выберите строку, для которой нужно изменить качество';
													|en = 'Select the line for which quality should be changed'"));
		Возврат;
	КонецЕсли;
	...
	Если Результат.Пустой() Тогда
		...
		ТекстСообщения =
			СтрШаблон(НСтр("ru = 'Не было произведено ни одного расчета себестоимости товаров по организации ""%1"".';
							|en = 'No inventory costs were calculated for company ""%1"".'"),
			Объект.Организация);
...
Функция ПараметрыЗаполненияПоТипуНазначения()
	...
	Параметры.ИмяЗаполняемогоПоля = "ПодНазначение";
	Параметры.ЗначениеПоля        = Истина;
