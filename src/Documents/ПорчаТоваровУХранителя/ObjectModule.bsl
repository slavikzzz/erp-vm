		   ...
		   И СтрокаТЧТовары.Номенклатура = СтрокаТЧТовары.НоменклатураОприходование Тогда
			...
			ТекстОшибки = СтрШаблон(
							НСтр("ru = 'Нельзя выбирать товар исходного качества. Выберите товар другого качества в строке %1 списка ""Товары""';
								|en = 'Cannot select goods of the original quality. Select goods of other quality in the %1 line of the ""Goods"" list'"),
							СтрокаТЧТовары.НомерСтроки);
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПорчаТоваров") Тогда
		ЗаполнитьНаОснованииПорчиТоваровДавальца(ДанныеЗаполнения);
	//-- НЕ УТКА
...
#Область ЗаполнениеПоПорчеТоваровДавальца

Процедура ЗаполнитьНаОснованииПорчиТоваровДавальца(СтруктураЗаполнения)
	
	ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПорчаТоваровУПереработчика;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Основание", СтруктураЗаполнения);
	Запрос.Текст = ТекстЗапросаЗаполнитьНаОснованииПорчиТоваровДавальца();
	...
	Если РеквизитыШапки.Следующий() Тогда
		...
		ШаблонОшибки = НСтр("ru = 'Ввод на основании порчи товаров невозможен. В видах запасов присутствуют разные ""%1"".';
							|en = 'Cannot generate documents based on the stock quality change. Inventory owner attributes contain different %1.'");
	...
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(НоменклатураСервер.ПараметрыУказанияСерий(ЭтотОбъект, Документы.ПорчаТоваровУХранителя));
	НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(ЭтотОбъект, ПараметрыУказанияСерий);
	
КонецПроцедуры

Функция ТекстЗапросаЗаполнитьНаОснованииПорчиТоваровДавальца()
	...
	|ПОМЕСТИТЬ ТоварыПорчи
	|ИЗ
	|	Документ.ПорчаТоваров.ВидыЗапасов КАК ТаблицаТоваров
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТоварыПорчи.Давалец     КАК Давалец,
	|	ТоварыПорчи.Давалец.ИНН КАК ИНН,
	|	ТоварыПорчи.Давалец.КПП КАК КПП
	|ПОМЕСТИТЬ Давальцы
	|ИЗ
	|	ТоварыПорчи КАК ТоварыПорчи
	|
	|ГДЕ
	|	НЕ ТоварыПорчи.Давалец.ПометкаУдаления
	|
	|СГРУППИРОВАТЬ ПО
	|	ТоварыПорчи.Давалец,
	|	ТоварыПорчи.Давалец.ИНН,
	|	ТоварыПорчи.Давалец.КПП
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТоварыПорчи.ДоговорДавальца       КАК ДоговорДавальца,
	|	ТоварыПорчи.ДоговорДавальца.Номер КАК Номер,
	|	ТоварыПорчи.ДоговорДавальца.Дата  КАК Дата
	|ПОМЕСТИТЬ ДоговорыДавальца
	|ИЗ
	|	ТоварыПорчи КАК ТоварыПорчи
	|
	|ГДЕ
	|	НЕ ТоварыПорчи.ДоговорДавальца.ПометкаУдаления
	|	И ТоварыПорчи.ДоговорДавальца.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыДоговоровКонтрагентов.Действует)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТоварыПорчи.ДоговорДавальца,
	|	ТоварыПорчи.ДоговорДавальца.Номер,
	|	ТоварыПорчи.ДоговорДавальца.Дата
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	МИНИМУМ(СпрДоговоры.НаправлениеДеятельности) КАК НаправлениеДеятельности,
	|	МИНИМУМ(ПорчаТоваров.Валюта)       КАК Валюта,
	|	МИНИМУМ(ПорчаТоваров.ПриходоватьТоварыПоСебестоимостиСписания) КАК ПриходоватьТоварыПоСебестоимостиСписания,
	|	МИНИМУМ(ПорчаТоваров.ПричинаПорчи) КАК ПричинаПорчи,
	...
	|	МИНИМУМ(ПорчаТоваров.Ссылка)       КАК Основание
	|ИЗ
	...
	|	ПО Переработчик.Ссылка = ПорчаТоваров.Организация
	|	
	...
	|
	|ГДЕ
	|	ПорчаТоваров.Ссылка = &Основание
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТоварыПорчи.Номенклатура                КАК Номенклатура,
	|	ТоварыПорчи.Характеристика              КАК Характеристика,
	|	ТоварыПорчи.Серия                       КАК Серия,
	|	ТоварыПорчи.НоменклатураОприходование   КАК НоменклатураОприходование,
	|	ТоварыПорчи.ХарактеристикаОприходование КАК ХарактеристикаОприходование,
	|	ТоварыПорчи.СерияОприходование          КАК СерияОприходование,
	|	ТоварыПорчи.НомерГТД                    КАК НомерГТД,
	|	ТоварыПорчи.Количество                  КАК Количество,
	|	ТоварыПорчи.КоличествоПоРНПТ            КАК КоличествоПоРНПТ
	|ИЗ
	|	ТоварыПорчи КАК ТоварыПорчи
	|";
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения =
			НСтр("ru = 'Невозможно отразить порчу, т.к. у некачественной и исходной номенклатуры не равны ставки НДС';
				|en = 'Cannot record damage as spoiled and source products have different VAT rates'");
