...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИзменитьОформлениеЕслиНеИспользоватьРегНомерСФР(ЭтотОбъект);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Завершение отправки"
		И Тип(Параметр) = Тип("Структура")
		И Параметр.Свойство("Ссылка")
		И Параметр.Ссылка = Объект.Ссылка Тогда
		
		КонтекстЭДОКлиент.ОтправитьУПУППослеОбмена();
	...
	Если Очищать Тогда
		Для каждого РеквизитЗаявления Из Метаданные.Документы.ЗаявленияПоЭлДокументооборотуСПФР.Реквизиты Цикл
			Если РеквизитЗаявления.Имя <> "Организация"
				...
				И РеквизитЗаявления.Имя <> "Сертификат"
				И РеквизитЗаявления.Имя <> "ФайлВыгрузки"
				И РеквизитЗаявления.Имя <> "ЭлектроннаяПодпись" Тогда
...
Процедура ОпределитьКодОрганаПФР()
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		КонтекстЭДОСервер.ОпределитьОрганПФРОрганизации(Объект.Организация, КодОрганаПФР,, РегНомерСФР, ИспользоватьРегНомерСФР);
		
		Элементы.ПолучательПредставление.Видимость = 
			НЕ ЗначениеЗаполнено(РегНомерСФР)
			ИЛИ НЕ ЗначениеЗаполнено(Объект.Получатель);
		
	Иначе
		КодОрганаПФР = "";
		Элементы.ПолучательПредставление.Видимость = Ложь;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Объект.Организация) И НЕ ЗначениеЗаполнено(Объект.Получатель) Тогда
			...
			Если ЗначениеЗаполнено(Форма.УчетнаяЗапись) Тогда
				...
				Если Оператор = ПредопределенноеЗначение("Перечисление.СпецоператорыСвязи.КалугаАстрал") Тогда
					Подстрока2 = НСтр("ru = 'Для подключения направления ';
										|en = 'Для подключения направления '");
					Подстрока3 = Новый ФорматированнаяСтрока(НСтр("ru = 'отправьте';
																	|en = 'отправьте'"),,,,"отправьте");
					Подстрока4 = НСтр("ru = ' заявление на подключение направления 1С-Отчетности';
										|en = ' заявление на подключение направления 1С-Отчетности'");
		...
		Иначе
			...
			Форма.ПолучательПредставление = НСтр("ru = 'Укажите код органа СФР (бывш. ПФР) в карточке организации';
												|en = 'Укажите код органа СФР (бывш. ПФР) в карточке организации'");
		КонецЕсли;
...
Процедура ИзменитьОформлениеСертификата(Форма)
	...
	Элемент.РасширеннаяПодсказка.Заголовок  = 
        ...
        |en = 'Сертификат заполняется автоматически из учетной записи организации.
        |Сертификат необходимо отправить в СФР (бывш. ПФР) не позднее одного дня с момента его получения и не позднее 7 дней до даты истечения предыдущего сертификата.'");
	...
	Иначе
		...
		Если Форма.СертификатПросрочен Тогда
			...
			ТекстОшибки = НСтр("ru = 'Срок действия сертификата истек %1';
								|en = 'Срок действия сертификата истек %1'");
...
Процедура ИзменитьОформлениеКнопок(Форма)
	...
	СтатусОтправлено = ЗначениеЗаполнено(Форма.СтатусОтправки)
		И Форма.СтатусОтправки <> ПредопределенноеЗначение("Перечисление.СтатусыОтправки.ВКонверте");
	Элементы.ФормаКомандаОтправить.Видимость = НЕ ЗапретитьИзменение И НЕ СтатусОтправлено
		И НЕ Форма.ЭтоОткрытиеИзМастера;
...
Процедура УстановитьЗаголовокФормы(Форма)
	...
	Иначе
		Заголовок = НСтр("ru = 'Заявление по эл. документообороту с СФР (бывш. ПФР)';
						|en = 'Заявление по эл. документообороту с СФР (бывш. ПФР)'");
	КонецЕсли;
...
&НаСервере
Процедура ИзменитьОформлениеЕслиНеИспользоватьРегНомерСФР(Форма)
	
	Если НЕ ИспользоватьРегНомерСФР Тогда
		Элементы.РегНомерПФР.ПодсказкаВвода = НСтр("ru = '999-999-999999';
													|en = '999-999-999999'");
		Элементы.ОператорРегНомерПФР.ПодсказкаВвода = НСтр("ru = '999-999-999999';
															|en = '999-999-999999'");
	КонецЕсли;
