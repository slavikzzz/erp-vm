...
Процедура ЗаполнитьИменаРеквизитовПоОснованию(ДокументОснование, МассивВсехРеквизитов, МассивРеквизитовОперации, ТипКорректировки = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетПереработчика2_5") Тогда
		...
		МассивРеквизитовОперации.Добавить("Товары.СписатьНаРасходы");
		МассивРеквизитовОперации.Добавить("Товары.НаправлениеДеятельности");
		МассивРеквизитовОперации.Добавить("Товары.НомерГруппыЗатрат");
		МассивРеквизитовОперации.Добавить("Товары.СтатьяКалькуляции");
		...
		МассивРеквизитовОперации.Добавить("Расхождения.СписатьНаРасходы");
		МассивРеквизитовОперации.Добавить("Расхождения.НаправлениеДеятельности");
		МассивРеквизитовОперации.Добавить("Расхождения.НомерГруппыЗатрат");
		МассивРеквизитовОперации.Добавить("Расхождения.СтатьяКалькуляции");
...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат КорректировкаПриобретенияЛокализация.ТекстОтраженияВРеглУчете();
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	Возврат КорректировкаПриобретенияЛокализация.ТекстЗапросаВТОтраженияВРеглУчете();
...
//
// Параметры:
//  ПараметрФормированияШаблоновНазначения - Строка - параметр заполнения макета назначений.
//
//  Возвращаемое значение:
//  См. Справочники.Назначения.МакетФормыВыбораНазначений
//
Функция МакетФормыВыбораНазначенийПоПараметру(ПараметрФормированияШаблоновНазначения = "") Экспорт
	...
	Если ПараметрФормированияШаблоновНазначения = "КорректировкаДоВоодаОстатков" Тогда
		ШаблонНазначения.УсловиеИспользования = "Объект.ТипКорректировки = ЗНАЧЕНИЕ(Перечисление.ТипыКорректировки.КорректировкаДоВводаОстатковТоваровИУслуг)";
		// Потребности в товарах на складе.
...
// 
// Параметры:
// 		ПараметрыДляДоступности - Структура:
//		* ХозяйственнаяОперация - ПеречислениеСсылка.ХозяйственныеОперации - Хозяйственная операция документа
//		* ДокументОснование - ДокументСсылка - документ основание.
// 	
// Возвращаемое значение:
// 	Структура - Параметры настройки счетов учета (См. НастройкаСчетовУчетаСервер.ПараметрыНастройкиСчетаУчетаОперации).
//
Функция ПараметрыНастройкиСчетовУчета(ПараметрыДляДоступности) Экспорт
	
	МассивПараметров = Новый Массив();
	
	ТипКорректировки = Неопределено;
	ПараметрыДляДоступности.Свойство("ТипКорректировки", ТипКорректировки);
	
	КорректировкаПриобретенияУслугПрочихАктивов = (ТипЗнч(ПараметрыДляДоступности.ДокументОснование) = Тип("ДокументСсылка.ПриобретениеУслугПрочихАктивов"))
		Или ТипКорректировки = Перечисления.ТипыКорректировки.КорректировкаДоВводаОстатковУслугИПрочихАктивов;
	
	#Область СчетУчетаАктивов	
	ПараметрыНастройки = НастройкаСчетовУчетаСервер.ПараметрыНастройки();
	ПараметрыНастройки.ПутьКДанным = "Объект";
	ПараметрыНастройки.НастройкаСчетовУчета = "НастройкаСчетовУчетаАктивов";
	ПараметрыНастройки.Представление = "ТекстНастроитьОтражениеВРеглУчетеАктивов";
	...
	ПараметрыНастройки.АналитикаАктивовПассивов = "АналитикаАктивов";

	ПараметрыНастройки.ЭлементыФормы.Добавить("ТекстНастроитьОтражениеВРеглУчетеАктивов");
	ПараметрыНастройки.ЭлементыФормы.Добавить("ТекстНастроитьОтражениеВРеглУчетеАктивовКорректировкаЗадолженности");
	
	МассивПараметров.Добавить(ПараметрыНастройки);
	#КонецОбласти
	
	#Область СчетУчетаПассивов	
	ПараметрыНастройки = НастройкаСчетовУчетаСервер.ПараметрыНастройки();
	ПараметрыНастройки.ПутьКДанным = "Объект";
	ПараметрыНастройки.НастройкаСчетовУчета = "НастройкаСчетовУчетаПассивов";
	ПараметрыНастройки.Представление = "ТекстНастроитьОтражениеВРеглУчетеПассивов";
	...
	ПараметрыНастройки.Организация = "Объект.Организация";
	ПараметрыНастройки.АналитикаАктивовПассивов = "АналитикаПассивов";

	ПараметрыНастройки.ЭлементыФормы.Добавить("ТекстНастроитьОтражениеВРеглУчетеПассивов");
	ПараметрыНастройки.ЭлементыФормы.Добавить("ТекстНастроитьОтражениеВРеглУчетеПассивовКорректировкаЗадолженности");
	
	МассивПараметров.Добавить(ПараметрыНастройки);
	#КонецОбласти

	#Область ТоварыСчетУчета	
	ПараметрыНастройки = НастройкаСчетовУчетаСервер.ПараметрыНастройки();
	...
	ПараметрыНастройки.ДоступностьПоОперации = КорректировкаПриобретенияУслугПрочихАктивов;
	ПараметрыНастройки.СкрыватьНедоступныйСчетУчетаВСтроках = Истина;
	
	ПараметрыНастройки.Организация = "Объект.Организация";

	ПараметрыНастройки.ЭлементыФормы.Добавить("ТоварыСчетРеглУчетаАктивовПассивов");

	МассивПараметров.Добавить(ПараметрыНастройки);
	#КонецОбласти

	#Область РасхожденияСчетУчета	
	ПараметрыНастройки = НастройкаСчетовУчетаСервер.ПараметрыНастройки();
	ПараметрыНастройки.ПутьКДанным = "Объект.Расхождения";
	ПараметрыНастройки.Представление = "ПредставлениеОтраженияВРеглУчете";
	ПараметрыНастройки.ТипСтатьи     = "ТипСтатьи";
	...
	ПараметрыНастройки.ЭлементыФормы.Добавить("РасхожденияСчетРеглУчетаАктивовПассивов");
	МассивПараметров.Добавить(ПараметрыНастройки);
	#КонецОбласти
...
// Возвращает параметры настройки счетов учета в документе корректировка приобретения до ввода остатков.
// 
// Параметры:
// 		ПараметрыДляДоступности - Структура:
//		* ХозяйственнаяОперация - ПеречислениеСсылка.ХозяйственныеОперации - Хозяйственная операция документа
...
// 	Структура - Параметры настройки счетов учета (См. НастройкаСчетовУчетаСервер.ПараметрыНастройкиСчетаУчетаОперации).
//
Функция ПараметрыНастройкиСчетовУчетаКорректировкаДоВводаОстатков(ПараметрыДляДоступности) Экспорт
	
	МассивПараметров = Новый Массив();
	
	КорректировкаПриобретенияУслугПрочихАктивов = ПараметрыДляДоступности.ТипКорректировки = Перечисления.ТипыКорректировки.КорректировкаДоВводаОстатковУслугИПрочихАктивов;
	...
	#Область ТаблицаКорректировкиСчетУчета	
	ПараметрыНастройки = НастройкаСчетовУчетаСервер.ПараметрыНастройки();
	ПараметрыНастройки.ПутьКДанным = "ТаблицаКорректировки";
	ПараметрыНастройки.Представление = "ПредставлениеОтраженияВРеглУчете";
	ПараметрыНастройки.ТипСтатьи     = "ТипСтатьи";
	...
	ПараметрыНастройки.ЭлементыФормы.Добавить("ТаблицаКорректировкиСчетРеглУчетаАктивовПассивов");
	МассивПараметров.Добавить(ПараметрыНастройки);
	#КонецОбласти
...
Функция ПараметрыЗаполненияНалогообложенияНДСЗакупки(Объект) Экспорт
	...
	//++ НЕ УТ
	Если Не ДанныеОбъекта.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КорректировкаОтчетаПереработчика Тогда
	//-- НЕ УТ
...
Процедура ПроверитьЗаполнениеРеквизитов(Форма, ПроверяемыеРеквизиты, Отказ) Экспорт
	...
	ТекстСообщения = НСтр("ru = 'Не указан получатель товаров/работ/услуг в строке %1 списка ""%2""';
							|en = 'Recipient of goods/works/services is not specified in line %1 of the %2 list'");
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Данные.ТипКорректировки = Перечисления.ТипыКорректировки.КорректировкаДоВводаОстатковТоваровИУслуг
		...
		ПостфиксИмениДокумента = НСтр("ru = 'Корректировка приобретения до ввода остатков';
										|en = 'Purchase adjustment before balance entry'");
	//++ НЕ УТ
	ИначеЕсли Данные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КорректировкаОтчетаПереработчика Тогда
		ПостфиксИмениДокумента = НСтр("ru = 'Корректировка отчета переработчика';
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	//++ НЕ УТ
	|	ДанныеДокумента.НастройкаСчетовУчетаАктивов  КАК НастройкаСчетовУчетаАктивов,
	|	ДанныеДокумента.НастройкаСчетовУчетаПассивов КАК НастройкаСчетовУчетаПассивов,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	Если ТипЗнч(Реквизиты.ДокументОснование) = Тип("ДокументСсылка.ОтчетПереработчика2_5") Тогда
		РеквизитыДокументаОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Реквизиты.ДокументОснование, "ХозяйственнаяОперация, ПоЗаказам, ГруппировкаЗатрат");
		Запрос.УстановитьПараметр("ПроизводствоУПереработчикаВСтранахЕАЭС", РеквизитыДокументаОснования.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчикаВСтранахЕАЭС2_5);
		Запрос.УстановитьПараметр("ПоЗаказам", РеквизитыДокументаОснования.ПоЗаказам);
		Запрос.УстановитьПараметр("ГруппировкаЗатрат", РеквизитыДокументаОснования.ГруппировкаЗатрат);
	КонецЕсли;
	//-- НЕ УТ
...
Функция ТекстЗапросаВтТаблицаРасхожденияТоваров(Запрос, ТекстыЗапроса) Экспорт
	...
	|
	//++ НЕ УТ
	|	ТаблицаРасхождения.СтатьяКалькуляции КАК СтатьяКалькуляции,
	//++ НЕ УТКА
	|	ТаблицаРасхождения.ЭтапПроизводства КАК ЭтапПроизводства,
	//-- НЕ УТКА
	|
	//-- НЕ УТ
	|
	...
	//++ НЕ УТ
	|	ТаблицаРасхождения.НастройкаСчетовУчета КАК НастройкаСчетовУчета,
	//-- НЕ УТ
	|	
	|	ВЫБОР 
...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса)
	...
	//++ НЕ УТ
	|
	// При корректировке отчета переработчика прошлого периода сумму корректировки относим на статью ""Выбытия товаров в прошлых периодах"".
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	&Период											КАК Период,
	...
	|	ТаблицаРасхождения.Номенклатура					КАК АналитикаРасходов,
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтТаблицаРасхожденияТоваров КАК ТаблицаРасхождения
	|	ПО ИСТИНА
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтПартииПроизводства КАК ПартииПроизводства
	|	ПО ПартииПроизводства.ПартияВыпуска = &ДокументОснование
	|	И ПартииПроизводства.НомерГруппыЗатрат = ТаблицаРасхождения.НомерГруппыЗатрат
	|
	|ГДЕ
	...
	|	И ТаблицаДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаОтчетаПереработчика)
	|	И НАЧАЛОПЕРИОДА(&ПериодПоступления, МЕСЯЦ) < НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|	И ТаблицаРасхождения.НастройкаХозяйственнойОперации <> ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ВключениеНДСВСтоимость)
	//-- НЕ УТ
	|";
...
Функция ТекстЗапросаТаблицаВтИсходныеДвиженияПоПрочимАктивамПассивам(Запрос, ТекстыЗапроса)
	...
	//++ НЕ УТ
	|	&НастройкаСчетовУчетаАктивов КАК НастройкаСчетовУчета,
	//-- НЕ УТ
	|	ВЫБОР
	...
	//++ НЕ УТ
	|	&НастройкаСчетовУчетаПассивов КАК НастройкаСчетовУчета,
	//-- НЕ УТ
	|	ВЫБОР
	|		КОГДА ТаблицаРасхождения.СуммаСНДСУпр < 0
	...
	|	ВЫБОР 
	|		КОГДА КорректировкаЗадолженности.НаПрочиеАктивы
	|			ТОГДА &НастройкаСчетовУчетаАктивов
	|		ИНАЧЕ &НастройкаСчетовУчетаПассивов
...
Функция РаспределенныеВидыЗапасов(ОсновнойЗапрос)
	...
	ТекстЗапроса =
	// шаблон распределенных сумм, ВГраница - 3
	"ВЫБРАТЬ ПЕРВЫЕ 0
	...
	|	Расхождения.НомерГруппыЗатрат             КАК НомерГруппыЗатрат,
	|	Расхождения.ВидЗапасов                    КАК ВидЗапасов,
	|	Расхождения.НомерГТД                      КАК НомерГТД,
	|	Расхождения.Сумма                         КАК Сумма
	|ИЗ
	...
	|;
	|
	// услуги переработки к распределению, ВГраница
	|ВЫБРАТЬ
	|	Расхождения.НомерГруппыЗатрат           КАК НомерГруппыЗатрат,
	|	Расхождения.АналитикаУчетаНоменклатуры  КАК АналитикаУчетаНоменклатуры,
	|	Расхождения.ВидЗапасов                  КАК ВидЗапасов,
	...
	|	ЕСТЬNULL(Суммы.СуммаБезНДС, 0)          КАК Сумма
	|ИЗ
	|	Документ.КорректировкаПриобретения.Расхождения КАК Расхождения
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтСуммыДокументовВВалютахУчета КАК Суммы
	|		ПО Расхождения.Ссылка = Суммы.Ссылка
	...
	ПоЗаказам = Запрос.Параметры.ПоЗаказам;
	ГруппировкаЗатрат = Запрос.Параметры.ГруппировкаЗатрат;
	...
	Для каждого Партия Из ТаблицаПартий Цикл
		...
		Для ИндексСтроки = 0 По СтрокКРаспределению - 1 Цикл
			...
			Сумма      = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(РаспределяемаяСтрока.Сумма, МассивКоэффициентов, 3);
			...
			Для ИндексПродукции = 0 По СтрокПродукции - 1 Цикл
				...
				НоваяСтрока.Сумма       = ?(ЗначениеЗаполнено(Сумма), Сумма[ИндексПродукции], 0);				
			КонецЦикла;
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|			ТОГДА ЛОЖЬ
	//++ НЕ УТ
	|		КОГДА НЕ ПартииПроизводства.НаправлениеДеятельности ЕСТЬ NULL
	|			ТОГДА ИСТИНА
	//-- НЕ УТ
	|		ИНАЧЕ ЛОЖЬ
	...
	//++ НЕ УТ
	|		КОГДА НЕ ПартииПроизводства.НаправлениеДеятельности ЕСТЬ NULL
	|			ТОГДА ПартииПроизводства.НаправлениеДеятельности
	//-- НЕ УТ
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	//++ НЕ УТ
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВтПартииПроизводства КАК ПартииПроизводства
	|		ПО ПартииПроизводства.ПартияВыпуска = &ДокументОснование
	|		И ПартииПроизводства.НомерГруппыЗатрат = ТаблицаРасхождения.НомерГруппыЗатрат
	//-- НЕ УТ
	|ГДЕ
...
Процедура ОтразитьВвозТоваровИзТаможенногоСоюза(Запрос, ТекстыЗапроса, Регистры)
	...
	Если ПроведениеДокументов.ТребуетсяТаблицаДляДвижений("ТоварыКОформлениюДокументовИмпорта", Регистры) Тогда 
		...
		//++ НЕ УТ
		Если Запрос.Параметры.ХозОперацияОснования = Перечисления.ХозяйственныеОперации.КорректировкаОтчетаПереработчика
			И Запрос.Параметры.ПроизводствоУПереработчикаВСтранахЕАЭС Тогда 
			...
			|ИЗ
			|	ВтРаспределенныеВидыЗапасов КАК ТоварыЗаявления";
...
Функция СформироватьДополнительныеТаблицыОперацийУчетаЗатратИСебестоимости(Документ, Запрос = Неопределено, ТекстыЗапроса = Неопределено) Экспорт
	...
	Если Запрос <> Неопределено Тогда
		...
		//++ НЕ УТ
		Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса(ДополнительныеТаблицы[3], ТекстыЗапроса) Тогда
			ТекстЗапросаВтПартииПроизводства(Запрос, ТекстыЗапроса);
		КонецЕсли;
		//-- НЕ УТ
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|	ВЫБОР
	//++ НЕ УТ
	|		КОГДА ТаблицаДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаОтчетаПереработчика)
	...
	//++ НЕ УТ	
	#Область ВнешнееПоступление_Корректировка_Работа_ОтчетПереработчика
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтПартииПроизводства КАК ПартииПроизводства
	|	ПО ПартииПроизводства.ПартияВыпуска = ТаблицаДокумента.ДокументОснование
	|	И ПартииПроизводства.НомерГруппыЗатрат = ТаблицаВидыЗапасов.НомерГруппыЗатрат
	|
	|ГДЕ
	|	ТаблицаДокумента.Ссылка В (&Ссылка)
	|	И ТаблицаВидыЗапасов.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
...
Функция ТекстЗапросаВтПартииПроизводства(Запрос, ТекстыЗапроса)
	...
	ТекстЗапроса = Документы.ОтчетПереработчика2_5.ТекстЗапросаВтПартииПроизводства(Запрос);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Ссылка", "&ДокументОснование");
...
Процедура СформироватьВременнуюТаблицуИсходныхДанных(МенеджерВременныхТаблиц, ДокументОснование, ДокументСсылка = Неопределено) Экспорт
	...
	//++ НЕ УТ
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетПереработчика2_5") Тогда
		ТекстЗапросаПоОснованию = ТекстЗапросаПоИсходнымДаннымОтчетаПереработчика();
	//-- НЕ УТ
	КонецЕсли;
...
Функция ТекстЗапросаПоИсходнымДаннымОтчетаПереработчика()
	...
	|	ОтчетПереработчика2_5.ЗаказПереработчику                     КАК ЗаказПоставщику,
	|	ТаблицаУслуги.КодСтроки                                      КАК КодСтроки,
	|	ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)                     КАК Склад,
	|	ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)                  КАК НомерГТД,
	|	ЗНАЧЕНИЕ(Справочник.КлассификаторТНВЭД.ПустаяСсылка)         КАК КодТНВЭД,
	|	ЗНАЧЕНИЕ(Справочник.СделкиСКлиентами.ПустаяСсылка)           КАК Сделка,
	|	ОтчетПереработчика2_5.Подразделение                          КАК Подразделение,
	|	ТаблицаУслуги.НомерГруппыЗатрат                              КАК НомерГруппыЗатрат,
	|	ТаблицаУслуги.СтатьяКалькуляции                              КАК СтатьяКалькуляции,
	//++ НЕ УТКА
	...
	|	НЕОПРЕДЕЛЕНО                                                 КАК АналитикаРасходов,
	|	ОтчетПереработчика2_5.НаправлениеДеятельности                КАК НаправлениеДеятельности,
	|	ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка) КАК НастройкаСчетовУчета,
	|	НЕОПРЕДЕЛЕНО                                                 КАК АналитикаАктивовПассивов,
	|	НЕОПРЕДЕЛЕНО                                                 КАК АналитикаДоходов
	|ИЗ
	...
	|	ПО ОтчетПереработчика2_5.Ссылка = ТаблицаУслуги.Ссылка
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК АналитикаНоменклатуры
	|	ПО АналитикаНоменклатуры.Ссылка = ТаблицаУслуги.АналитикаУчетаНоменклатуры
	|
	|ГДЕ
	|	ДанныеПоследнейКорректировки.Ссылка ЕСТЬ NULL 
...
Процедура СообщитьОшибкиЗаполнения(Выборка, Отказ)
	...
	ШаблонСообщенияНеСоответствуетПерыдущейКорректировке= НСтр("ru = 'Поле ""%1"" не соответствует аналогичному полю предыдущей корректировки';
																|en = 'The %1 field does not match the respective field of the previous adjustment'");
		...
		Если Выборка.ТипПроверки = "НеУказаноКоличествоУпаковок" Тогда
			ТекстСообщения = СтрШаблон(ШаблонСообщенияНеЗаполненаКолонка,
										НСтр("ru = 'Количество упаковок до корректировки';
											|en = 'Packaging unit quantity before adjustment'"),
										Выборка.НомерСтроки,
										ПредставлениеТЧ);
		...
		Если Выборка.ТипПроверки = "НеУказаноКоличествоУпаковокПосле" Тогда
			ТекстСообщения = СтрШаблон(ШаблонСообщенияНеЗаполненаКолонка,
										НСтр("ru = 'Количество упаковок после корректировки';
											|en = 'Packaging unit quantity after adjustment'"),
										Выборка.НомерСтроки,
										ПредставлениеТЧ);
		...
		Если Выборка.ТипПроверки = "НеУказаноКоличествоПоРНПТ" Тогда
			ТекстСообщения = СтрШаблон(ШаблонСообщенияНеЗаполненаКолонка,
										НСтр("ru = 'Количество по РНПТ до корректировки';
											|en = 'Quantity by RNPT before adjustment'"),
										Выборка.НомерСтроки,
										ПредставлениеТЧ);
		...
		Если Выборка.ТипПроверки = "НеУказаноКоличествоПоРНПТПосле" Тогда
			ТекстСообщения = СтрШаблон(ШаблонСообщенияНеЗаполненаКолонка,
										НСтр("ru = 'Количество по РНПТ после корректировки';
											|en = 'Quantity by RNPT after adjustment'"),
										Выборка.НомерСтроки,
										ПредставлениеТЧ);
...
Процедура СообщитьОшибкиПоЦепочкеКорректировок(Выборка, Отказ)
	...
	ШаблонСообщенияРасходитсяЗначениеКоличество	= НСтр("ru = 'В поле ""%1"" в строке %2 указано количество отличное от количества %3 после последней корректировки';
															|en = 'In line %2 of the %1 field, the quantity different from the quantity %3 after the last adjustment is specified'");
	ШаблонСообщенияРасходитсяЗначениеСумма		= НСтр("ru = 'В поле ""%1"" в строке %2 указана сумма отличная от суммы %3 после последней корректировки';
														|en = 'In line %2 of the %1 field, the amount different from the amount %3 after the last adjustment is specified'");
	ШаблонСообщенияРасходитсяСтавкаНДС			= НСтр("ru = 'Ставка НДС в строке %1 отличается от ставки НДС %2 после последней корректировки';
														|en = 'VAT rate in line %1 is different from the VAT rate %2 after the last adjustment'");
...
Процедура ПроверитьУказаниеСуммыИСуммыНДС(Объект, МенеджерВременныхТаблиц, Отказ)
	...
	ШаблонСообщенияОшибкаУказанияСуммы		= НСтр("ru = 'Сумма НДС до корректировки %1 %2 при ставке ""%3"" больше общей суммы до корректировки по строке %4 %5 в строке %6 списка ""%7""';
													|en = 'VAT amount before adjustment %1 %2 at rate %3 is greater than the total amount before the adjustment for line %4 %5 in line %6 of the %7 list'");
	ШаблонСообщенияОшибкаУказанияСуммыПосле	= НСтр("ru = 'Сумма НДС после корректировки %1 %2 при ставке ""%3"" больше общей суммы после корректировки по строке %4 %5 в строке %6 списка ""%7""';
														|en = 'VAT amount after the adjustment %1 %2 at rate %3 is greater than the total amount after the adjustment for line %4 %5 in line %6 of the %7 list'");
