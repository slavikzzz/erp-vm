...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//++ НЕ УТ
	СформироватьЗаголовокПоКорректировкеОтчетаПереработчику();
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	//++ НЕ УТ
		ИЛИ ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.ОтчетПереработчика2_5")
	//-- НЕ УТ
		Тогда
...
Процедура ПредставлениеДокументаОснованияНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ТипОснования = Тип("ДокументСсылка.ОтчетПереработчика2_5")Тогда
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И НЕ Копирование Тогда
		...
		//++ НЕ УТ
		Если КорректировкаОтчетаПереработчика Тогда
			ТекущаяСтрока.Подразделение = ПодразделениеОснования;
			ТекущаяСтрока.НаправлениеДеятельности = НаправлениеДеятельностиОснования;
		КонецЕсли;
		//-- НЕ УТ
...
Процедура ТоварыПриОкончанииРедактированияНаСервере(ТекущийИдентификатор, КэшированныеЗначения)
	...
	//++ НЕ УТ
	УстановитьНомерГруппыЗатратПоУмолчанию(ТекущийИдентификатор);
	//-- НЕ УТ
...
Процедура ТоварыГруппаЗатратОткрытие(Элемент, СтандартнаяОбработка)
	...
	//++ НЕ УТКА
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	Если ГруппировкаЗатратОснования = ПредопределенноеЗначение("Перечисление.ГруппировкиЗатратВЗаказеПереработчику.ПоЭтапамПроизводства")
...
Процедура ТоварыГруппаЗатратНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ГруппировкаЗатрат",         ГруппировкаЗатратОснования);
	ПараметрыФормы.Вставить("НомерГруппыЗатрат",         Элементы.Товары.ТекущиеДанные.НомерГруппыЗатрат);
	ПараметрыФормы.Вставить("АдресХранилищаГруппЗатрат", ПоместитьДанныеГруппЗатратВХранилище());
	
	ПараметрыФормы.Вставить(
		"ПоляГруппыЗатрат",
		ПереработкаНаСторонеКлиентСервер.ПереченьПолейГруппыЗатратОтчетаПереработчика(ГруппировкаЗатратОснования));
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ТаблицаГруппаЗатратНачалоВыбораЗавершение", ЭтотОбъект);
	...
	Если Не (
	 ...
	 Или НастройкиВариантовОтражения.ЕстьУвеличениеСуммы
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Нет строк, в которых доступно изменение варианта отражения.';
													|en = 'No lines in which posting options can be changed.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПодобратьТовары(Команда)
	...
	//++ НЕ УТ
	ИначеЕсли КорректировкаОтчетаПереработчика Тогда
		ПараметрыФормы.Вставить("ОтборПоТипуНоменклатуры", НоменклатураКлиентСервер.ОтборПоРаботе(Ложь));
		ПараметрыФормы.Вставить("Подразделение", ПодразделениеОснования);
	//-- НЕ УТ
	Иначе
...
Процедура РаспределитьОбщуюСуммуКорректировкиЗадолженностиНаДокументы(Команда)
	...
	ЗаголовокФормыВвода = НСтр("ru = 'Введите общую сумму корректировки';
								|en = 'Enter total adjustment amount'");
...
Процедура УстановитьУсловноеОформлениеДляСтатейИАналитик()
	...
	// видимость, если корректируется отчет переработчика
	Элемент = УсловноеОформление.Элементы.Добавить();
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыАналитикаАктивовПассивов.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыАналитикаДоходов.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыАналитикаРасходов.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыСчетРеглУчетаРасходов.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыСчетРеглУчетаАктивовПассивов.Имя);
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.РасхожденияСчетРеглУчетаРасходов.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.РасхожденияСчетРеглУчетаАктивовПассивов.Имя);
...
Процедура УстановитьУсловноеОформление()
	...
	// Отключение доступности подразделения, направления деятельности
	Элемент = УсловноеОформление.Элементы.Добавить();
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыНаправлениеДеятельности.Имя);
	...
	// Отключение обязательность заполнения стоимостей
	Элемент = УсловноеОформление.Элементы.Добавить();
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Для ГТД по РНПТ или комплектам по РНПТ';
																|en = 'For CCD by RNPT or kits by RNPT'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
...
Процедура ЗаполнитьРасхожденияСервер()
	...
	//++ НЕ УТ
	Если КорректировкаОтчетаПереработчика Тогда
		Для Каждого ТекущиеДанные Из Объект.Расхождения Цикл
			
			СтруктураПоиска = Новый Структура("НомерГруппыЗатрат", ТекущиеДанные.НомерГруппыЗатрат);
			
			НайденныеСтроки = ГруппыЗатрат.НайтиСтроки(СтруктураПоиска);
			Если Не НайденныеСтроки.Количество() = 0 Тогда
				ТекущиеДанные.ГруппаЗатрат = НайденныеСтроки[0].ГруппаЗатрат;
			КонецЕсли;
...
Процедура ЗаполнитьСлужебныеРеквизитыПрослеживаемостиВСтрокеТоваров(ТекущиеДанные, ЭтоРасхождение = Ложь)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		//++ НЕ УТ
		Если ТипЗнч(ТекущиеДанные.АналитикаРасходов) = Тип("СправочникСсылка.ОбъектыЭксплуатации") Тогда
			
			УказанНомерГТД = ЗначениеЗаполнено(ТекущиеДанные.НомерГТД);
			ТекущиеДанные.ВедетсяУчетПоГТД = ?(ЭтоРасхождение, УказанНомерГТД, Истина);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	//++ НЕ УТ
	КорректировкаОтчетаПереработчика = (ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ОтчетПереработчика2_5"));
	
	Если КорректировкаОтчетаПереработчика Тогда
		
		РеквизитыДокументаОснования      = РеквизитыДокументаОснования(Объект.ДокументОснование);
		ПодразделениеОснования           = РеквизитыДокументаОснования.Подразделение;
		НаправлениеДеятельностиОснования = РеквизитыДокументаОснования.НаправлениеДеятельности;
		ГруппировкаЗатратОснования       = РеквизитыДокументаОснования.ГруппировкаЗатрат;
...
Процедура УстановитьПредставлениеДокументаОснования()
	...
	ИначеЕсли ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ПриобретениеУслугПрочихАктивов") Тогда
		...
		Элементы.ПредставлениеДокументаОснования.ЦветТекста = ЦветаСтиля.ЦветОсобогоТекста;
	//++ НЕ УТ
	ИначеЕсли ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ОтчетПереработчика2_5") Тогда
		ПредставлениеДокументаОснования = НСтр("ru = 'Отчет переработчика <не выбран>';
												|en = 'Subcontracting services notification received is <not selected>'");
		Элементы.ПредставлениеДокументаОснования.ЦветТекста = ЦветаСтиля.ЦветОсобогоТекста;
	//-- НЕ УТ
	Иначе
...
Функция РеквизитыДокументаОснования(ДокументОснование)
	...
	//++ НЕ УТ
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетПереработчика2_5") Тогда
		РеквизитыОснования.Вставить("Подразделение");
		РеквизитыОснования.Вставить("НаправлениеДеятельности");
		РеквизитыОснования.Вставить("ГруппировкаЗатрат");
	КонецЕсли;
	//-- НЕ УТ
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		УстановитьНомерГруппыЗатратПоУмолчанию(ТекущаяСтрока.ПолучитьИдентификатор());
		//-- НЕ УТ
...
//++ НЕ УТ
&НаСервере
Процедура СформироватьЗаголовокПоКорректировкеОтчетаПереработчику()
	
	Если КорректировкаОтчетаПереработчика Тогда
		
		Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
			АвтоЗаголовок = Ложь;
			Заголовок = НСтр("ru = 'Корректировка отчета переработчика (создание)';
							|en = 'Adjust subcontracting services notification received (create)'");
		Иначе
			Заголовок = "";
...
Процедура ОбновитьПредставлениеЭлементыИсправляемогоДокумента()
	...
	Если НЕ ЗначениеЗаполнено(ПредставлениеИсправляемогоДокумента) Тогда
		ПредставлениеИсправляемогоДокумента = Нстр("ru = 'Выберите исправляемый документ';
													|en = 'Select the document to correct'");
	КонецЕсли;	
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Если Не КорректировкаПриобретенияУслугПрочихАктивов Тогда
		...
		//++ НЕ УТ
		ИначеЕсли КорректировкаОтчетаПереработчика Тогда
			ОтборПоТипамНоменклатуры = НоменклатураКлиентСервер.ОтборПоРаботе(Ложь);
		//-- НЕ УТ
		Иначе
			ОтборПоТипамНоменклатуры = НоменклатураКлиентСервер.ОтборПоТоваруМногооборотнойТареУслугеРаботе(Ложь);
		КонецЕсли;
...
&НаСервере
Процедура УстановитьНомерГруппыЗатратПоУмолчанию(ТекущийИдентификатор)
...
&НаСервере
Функция ПоместитьДанныеГруппЗатратВХранилище()
...
&НаКлиенте
Процедура ТаблицаГруппаЗатратНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт

	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаГруппаЗатратНачалоВыбораЗавершениеНаСервере(Результат, Элементы.Товары.ВыделенныеСтроки);
	
КонецПроцедуры

&НаСервере
Процедура ТаблицаГруппаЗатратНачалоВыбораЗавершениеНаСервере(Результат, Знач ВыделенныеСтроки)
...
Процедура ОбновитьГруппыЗатрат()
	...
	Если Не КорректировкаОтчетаПереработчика
	...
	РеквизитыДокумента =
		ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			Объект.ДокументОснование,
			"ГруппировкаЗатрат, ПоЗаказам, ЗаказПереработчику, МаксимальныйНомерГруппыЗатрат, ВыходныеИзделия");
	
	Документы.ОтчетПереработчика2_5.ОбновитьГруппыЗатрат(РеквизитыДокумента, ГруппыЗатрат, КоличествоГруппЗатрат);
...
Процедура ОбновитьПредставлениеГруппЗатрат()
	...
	Если Не КорректировкаОтчетаПереработчика Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ТекГруппа Из ГруппыЗатрат Цикл
		...
		Строки = Объект.Товары.НайтиСтроки(СтруктураОтбора);
		Для Каждого ТекСтрока Из Строки Цикл
			ТекСтрока.ГруппаЗатрат = ТекГруппа.ГруппаЗатрат;
		КонецЦикла;
		
		Строки = Объект.Расхождения.НайтиСтроки(СтруктураОтбора);
		Для Каждого ТекСтрока Из Строки Цикл
			ТекСтрока.ГруппаЗатрат = ТекГруппа.ГруппаЗатрат;
		КонецЦикла;
	...
	Если КорректировкаОтчетаПереработчика Тогда
		...
		МассивПараметров.Добавить(
			Новый ПараметрВыбора(
				...
				Новый ФиксированныйМассив(НоменклатураКлиентСервер.ОтборПоРаботе(Ложь))));
...
Процедура УстановитьПараметрыВыбораНомераГТДИКодаТНВЭД()
	...
	Если КорректировкаПриобретенияУслугПрочихАктивов Тогда
		...
		МассивПараметровКодаТНВЭД.Добавить(
			Новый ПараметрВыбора(
				"Отбор.ПрослеживаемыйТовар",
				Истина));
...
Функция ХозяйственнаяОперацияДокументаОснования(ДокументОснование)
	...
	ИначеЕсли ЗначениеЗаполнено(ДокументОснование)
		И ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетПереработчика2_5") Тогда
		Операция = Перечисления.ХозяйственныеОперации.КорректировкаОтчетаПереработчика;
	//-- НЕ УТ
	КонецЕсли;
...
Процедура УстановитьХозяйственнуюОперациюДокумента(ДокументОснование)
	...
	//++ НЕ УТ
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетПереработчика2_5") Тогда
		ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КорректировкаОтчетаПереработчика;
	КонецЕсли;
	//-- НЕ УТ
