...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Справочники.Назначения.ФормаДокументаПриСозданииНаСервере(ЭтотОбъект, "КорректировкаДоВоодаОстатков");
	
	//++ НЕ УТ
	ПараметрыДляВыбора = Новый Структура;
	ПараметрыДляВыбора.Вставить("ХозяйственнаяОперация", Объект.ВидКорректировки);
	ПараметрыДляВыбора.Вставить("ТипКорректировки", Объект.ТипКорректировки);
	ПараметрыНастройки = Документы.КорректировкаПриобретения.ПараметрыНастройкиСчетовУчетаКорректировкаДоВводаОстатков(ПараметрыДляВыбора);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//++ НЕ УТ
	НастройкаСчетовУчетаКлиент.ПередЗаписьюНаКлиенте(ЭтотОбъект);
	//-- НЕ УТ
...
Процедура СтатьяРасходовПриИзмененииСервер()
	...
	//++ НЕ УТ
	ПараметрыДляДоступности = Новый Структура("ХозяйственнаяОперация, ТипКорректировки", Объект.ВидКорректировки, Объект.ТипКорректировки);
	ПараметрыНастройки = Документы.КорректировкаПриобретения.ПараметрыНастройкиСчетовУчетаКорректировкаДоВводаОстатков(ПараметрыДляДоступности);
...
Процедура ТипКорректировкиПриИзменении(Элемент)
	...
	Если Объект.ТипКорректировки <> ТипКорректировкиСтарый
		...
		ТекстВопроса = НСтр("ru = 'Очистить таблицу с данными по корректировке?';
							|en = 'Do you want to clear the table with adjustment data?'");
...
Процедура ЗаполнитьНоменклатуруПоНоменклатуреПартнера(Команда)
	...
	Если Не ОбщегоНазначенияУТКлиент.ПроверитьНаличиеСтрокВСписке(ТаблицаКорректировки, НСтр("ru = 'Данные корректировки';
																							|en = 'Adjustment data'")) Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ИзменитьДокументОснование(Команда)
...
Процедура УстановитьУсловноеОформлениеДляСтатейИАналитик()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<для товаров и работ без назначения>';
																|en = '<for goods and works without assignment>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
...
Процедура ДоговорПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено(Объект.Договор)
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "ТипДоговора") = Перечисления.ТипыДоговоров.СПереработчиком2_5 Тогда
		КорректировкаПереработчика = Истина;
	Иначе
	//-- НЕ УТ
...
Процедура ЗаполнитьОграниченияПриСменеДоговора()
	...
	Если ЗначениеЗаполнено(Объект.Договор)
		...
		СпсисокТиповНоменклатуры.Очистить();
		СпсисокТиповНоменклатуры.Добавить(Перечисления.ТипыНоменклатуры.Услуга);
		...
		ЭлементТипаКорректировки = Элементы.ТипКорректировки.СписокВыбора.НайтиПоЗначению(Перечисления.ТипыКорректировки.КорректировкаДоВводаОстатковТоваровИУслуг);
		Если ЭлементТипаКорректировки <> Неопределено Тогда
			ЭлементТипаКорректировки.Представление = НСтр("ru = 'Корректировка услуг по номенклатуре';
															|en = 'Service adjustment by items'");
		КонецЕсли;
		
		ЭлементТипаКорректировки = Элементы.ТипКорректировки.СписокВыбора.НайтиПоЗначению(Перечисления.ТипыКорректировки.КорректировкаДоВводаОстатковУслугИПрочихАктивов);
		Если ЭлементТипаКорректировки <> Неопределено Тогда
			ЭлементТипаКорректировки.Представление = НСтр("ru = 'Корректировка услуг по содержанию';
															|en = 'Service adjustment by scope'");
		КонецЕсли;
	...
	Иначе
		...
		ЭлементТипаКорректировки = Элементы.ТипКорректировки.СписокВыбора.НайтиПоЗначению(Перечисления.ТипыКорректировки.КорректировкаДоВводаОстатковТоваровИУслуг);
		Если ЭлементТипаКорректировки <> Неопределено Тогда
			ЭлементТипаКорректировки.Представление = НСтр("ru = 'Корректировка по товарам и услугам';
															|en = 'Adjustment by goods and services'");
		КонецЕсли;
		
		ЭлементТипаКорректировки = Элементы.ТипКорректировки.СписокВыбора.НайтиПоЗначению(Перечисления.ТипыКорректировки.КорректировкаДоВводаОстатковУслугИПрочихАктивов);
		Если ЭлементТипаКорректировки <> Неопределено Тогда
			ЭлементТипаКорректировки.Представление = НСтр("ru = 'Корректировка по услугам и прочим активам';
															|en = 'Adjustment by services and other assets'");
		КонецЕсли;
...
//++ НЕ УТ
&НаСервере
Процедура УдалитьСтрокиНеУслуг()
	
	МассивСтрокКУдалению = Новый Массив();
	
	Для каждого СтрокаКорректировки Из ТаблицаКорректировки Цикл
		Если СтрокаКорректировки.ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Услуга Тогда
			МассивСтрокКУдалению.Добавить(СтрокаКорректировки);
		КонецЕсли;
	КонецЦикла;
	
	Для каждого СтрокаКУдалению Из МассивСтрокКУдалению Цикл
		ТаблицаКорректировки.Удалить(СтрокаКУдалению);
	КонецЦикла;
...
Процедура УстановитьПредставлениеДокументаОснования()
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		Элементы.ИзменитьДокументОснование.Видимость = Истина;
	Иначе
		...
		Элементы.ИзменитьДокументОснование.Видимость = Ложь;
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовПоОснованию()
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТаблицаКорректировкиУпаковка", "Видимость",
												Элементы.ТаблицаКорректировкиНоменклатура.Видимость);
	//++ НЕ УТ
	Если КорректировкаПереработчика Тогда
		
		МассивИменЭлементов = Новый Массив;
		МассивИменЭлементов.Добавить("ТаблицаКорректировкиХарактеристика");
		МассивИменЭлементов.Добавить("ТаблицаКорректировкиГруппаКолонокСерия");
		МассивИменЭлементов.Добавить("ТаблицаКорректировкиНомерГТД");
		МассивИменЭлементов.Добавить("ТаблицаКорректировкиНомерГТДСтранаПроисхождения");
		МассивИменЭлементов.Добавить("ТаблицаКорректировкиГруппаКоличествоПоРНПТ");
		МассивИменЭлементов.Добавить("Соглашение");
		МассивИменЭлементов.Добавить("Склад");
		
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(Элементы, МассивИменЭлементов, "Видимость", Ложь);
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаДоговор", "Видимость", Истина);
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Договор", "Видимость", Истина);
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Договор", "Доступность", Истина);
...
Процедура ОбновитьВыборСтатейСчетовУчетаСервер()
	...
	//++ НЕ УТ
	ПараметрыВыбораСчетов = Документы.КорректировкаПриобретения.ПараметрыНастройкиСчетовУчетаКорректировкаДоВводаОстатков(ПараметрыДляВыбора);
...
Процедура УстановитьЗаголовокФормы()
	...
	ШаблонЗаголовка = НСтр("ru = 'Корректировка приобретения до ввода остатков %1';
							|en = 'Purchase adjustment before balance entry %1'");
...
Процедура ОбновитьСлужебныеПоляТаблицыЗначений()
	...
	Если ТаблицаКорректировки.Количество() = 0 Тогда
		ТекстЗаголовка = НСтр("ru = 'Данные корректировки';
								|en = 'Correction data'");
	Иначе
		ТекстЗаголовка = СтрШаблон(НСтр("ru = 'Данные корректировки (%1)';
										|en = 'Adjustment data (%1)'"), ТаблицаКорректировки.Количество());
	КонецЕсли;
