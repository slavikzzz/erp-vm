...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе
		...
		Иначе
			//++ НЕ УТ
			Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КорректировкаОтчетаПереработчика Тогда
				МассивНепроверяемыхРеквизитов.Добавить("Товары.Цена");
				МассивНепроверяемыхРеквизитов.Добавить("Товары.Сумма");
			КонецЕсли;
			//-- НЕ УТ
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Если Не ПравоДоступа("Изменение", МетаданныеДокументОснование) Тогда
		ТекстОшибки = НСтр("ru = 'У пользователя недостаточно прав на корректировку документа ""%1"".';
							|en = 'User is not authorized to adjust document ""%1"".'");
...
Процедура ЗаполнитьПоПервичномуДокументу(Знач ДокументОснование, ДанныеЗаполнения)
	...
	Если Выборка.ТипДокумента <> Перечисления.ТипыПервичныхДокументов.ПриобретениеУПоставщика Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не требуется вводить корректировку поступления на основании документа %1, с типом документа %2.';
				|en = 'Do not enter receipt adjustment based on the %1 document with the %2 document type.'"),
			ДокументОснование,
	...
	Если Выборка.ТипДоговора <> Перечисления.ТипыДоговоров.СПоставщиком
		//++ НЕ УТ
		И Выборка.ТипДоговора <> Перечисления.ТипыДоговоров.СПереработчиком2_5
		//-- НЕ УТ
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не требуется вводить корректировку поступления на основании документа %1, с типом договора %2.';
				|en = 'Do not enter the receipt adjustment based on the %1 document with the %2 contract type.'"),
			ДокументОснование,
			Выборка.ТипДоговора);
		ВызватьИсключение Текст;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Выборка.НалогообложениеНДС) Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не требуется вводить корректировку поступления на основании документа %1, с не заполненным налогообложением.';
				|en = 'Do not enter the receipt adjustment based on the %1 document with empty taxation.'"),
			ДокументОснование,
			Выборка.ТипДоговора);
		ВызватьИсключение Текст;
	КонецЕсли;
	...
	Если Не ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не требуется вводить корректировку поступления на основании документа %1, при выключенной функциональной опции ""Новая архитектура взаиморасчетов"".';
				|en = 'Do not enter receipt adjustment based on the %1 document with the disabled ""New AR/AP configuration"" functional option.'"),
			ДокументОснование);
		ВызватьИсключение Текст;
	КонецЕсли;
...
Процедура ЗаполнитьПоОтчетуПереработчика(Знач ДокументОснование, ДанныеЗаполнения)
	...
	|	ДанныеДокумента.ЦенаВключаетНДС          КАК ЦенаВключаетНДС,
	|	ДанныеДокумента.ПорядокРасчетов          КАК ПорядокРасчетов,
	|	ДанныеДокумента.ПоЗаказам                КАК ПоступлениеПоЗаказам,
	|	НЕ ДанныеДокумента.Проведен              КАК ЕстьОшибкиПроведен,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаОтчетаПереработчика) КАК ХозяйственнаяОперацияОснования,
	|	
	|	ЛОЖЬ                                     КАК ЕстьОшибкиОперация,
	|	ЛОЖЬ                                     КАК ЕстьОшибкиНалогообложениеНДС,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаОтчетаПереработчика) КАК ХозяйственнаяОперация
	|	
	|ИЗ
	...
	Если РезультатЗапроса.Пустой() Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не требуется вводить корректировку на основании документа %1.';
				|en = 'Generation of adjustment from the %1 document is not required.'"),
			ДокументОснование);
		ВызватьИсключение Текст;
	КонецЕсли;
	...
	ЗаполнитьЗначенияСвойств(ДанныеЗаполнения, Выборка);
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, Выборка, "Организация, Валюта, ВалютаВзаиморасчетов, ДокументОснование, ЦенаВключаетНДС, НалогообложениеНДС, ХозяйственнаяОперация");
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПриобретениеУслугПрочихАктивов")
		//++ НЕ УТ
		ИЛИ ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетПереработчика2_5")
		//-- НЕ УТ
		Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьАналитикиУчетаНоменклатурыВТабличныхЧастяхТоваров(ТаблицыДокумента = Неопределено)
	...
	Если ЗначениеЗаполнено(ДокументОснование)
		...
		//++ НЕ УТ
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КорректировкаОтчетаПереработчика Тогда
			ОперацияПодбораАналитики = ОперацияОснования;
		//-- НЕ УТ
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеТаблицыКорректировкаЗадолженности(Отказ)
	...
	ШаблонОшибки = НСтр("ru = 'В строке ""%1"" сумма уменьшения больше суммы приобретения';
						|en = 'In the ""%1"" line the decrease amount is greater than the purchase amount'");
...
Процедура ПроверитьЗаполненностиСуммыВзаиморасчетов(Отказ)
	...
	ШаблонОшибки = НСтр("ru = 'В строке ""%1"" неверно указана сумма взаиморасчетов ""после"", т.к. сумма взаиморасчетов ""до"" стала нулевой.';
						|en = 'In line %1, the ""after"" AR/AP amount is specified incorrectly as the ""before"" AR/AP amount is now zero.'");
	КлючДанных = ОбщегоНазначенияУТ.КлючДанныхДляСообщенияПользователю(ЭтотОбъект);
...
Процедура ПроверитьЦепочкуИсправленийПередЗаписью(Отказ, РежимЗаписи)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение И ЗначениеЗаполнено(ИсправляемыйДокумент)
		...
		ТекстОшибки = НСтр("ru = 'Исправляемый документ не проведен или сторнирован.
				...
				|en = 'The document to correct is not posted or reversed.
...
Процедура ЗаполнитьРасхождения(ИспользуетсяДокументПоступлениеТоваров) Экспорт
	...
	//++ НЕ УТ
	|	НовыеДанные.СтатьяКалькуляции КАК СтатьяКалькуляции,
	//++ НЕ УТКА
	|	НовыеДанные.ЭтапПроизводства КАК ЭтапПроизводства,
	//-- НЕ УТКА
	...
	//++ НЕ УТ
	|	НовыеДанныеИзТаблицы.СтатьяКалькуляции КАК СтатьяКалькуляции,
	//++ НЕ УТКА
	|	НовыеДанныеИзТаблицы.ЭтапПроизводства КАК ЭтапПроизводства,
	//-- НЕ УТКА
	...
	//++ НЕ УТ
	|	МАКСИМУМ(НовыеДанныеИзТаблицы.НастройкаСчетовУчета) КАК НастройкаСчетовУчета,
	|	ЕСТЬNULL(НовыеДанныеИзТаблицы.НастройкаСчетовУчета.ХешСумма, """") КАК ХешСуммаНастройкиСчетов,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	НовыеДанныеИзТаблицы.СтатьяКалькуляции,
	//++ НЕ УТКА
	|	НовыеДанныеИзТаблицы.ЭтапПроизводства,
	//-- НЕ УТКА
	...
	|	НовыеДанныеИзТаблицы.НаправлениеДеятельности,
	//++ НЕ УТ
	|	ЕСТЬNULL(НовыеДанныеИзТаблицы.НастройкаСчетовУчета.ХешСумма, """"),
	//-- НЕ УТ
	|	НовыеДанныеИзТаблицы.НомерГТД,
	|	НовыеДанныеИзТаблицы.КодТНВЭД,
	|	НовыеДанныеИзТаблицы.СтавкаНДС,
	...
	//++ НЕ УТ
	|		КОГДА ИсходныеДанные.ЗаказПоставщику ССЫЛКА Документ.ЗаказПереработчику2_5
	|			ТОГДА ВЫРАЗИТЬ(ИсходныеДанные.ЗаказПоставщику КАК Документ.ЗаказПереработчику2_5)
	//-- НЕ УТ
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	//++ НЕ УТ
	|	ВЫРАЗИТЬ(ИсходныеДанные.СтатьяКалькуляции КАК Справочник.СтатьиКалькуляции) КАК СтатьяКалькуляции,
	//++ НЕ УТКА
	|	ВЫРАЗИТЬ(ИсходныеДанные.ЭтапПроизводства КАК Документ.ЭтапПроизводства2_2) КАК ЭтапПроизводства,
	//-- НЕ УТКА
	...
	//++ НЕ УТ
	|	МАКСИМУМ(ИсходныеДанные.НастройкаСчетовУчета) КАК НастройкаСчетовУчета,
	|	ЕСТЬNULL(ИсходныеДанные.НастройкаСчетовУчета.ХешСумма, """") КАК ХешСуммаНастройкиСчетов,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	ИсходныеДанные.СтатьяКалькуляции,
	//++ НЕ УТКА
	|	ИсходныеДанные.ЭтапПроизводства,
	//-- НЕ УТКА
	...
	//++ НЕ УТ
	|	ЕСТЬNULL(ИсходныеДанные.НастройкаСчетовУчета.ХешСумма, """"),
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|			И НовыеДанные.СтатьяКалькуляции = ИсходныеДанные.СтатьяКалькуляции
	//++ НЕ УТКА
	|			И НовыеДанные.ЭтапПроизводства = ИсходныеДанные.ЭтапПроизводства
	//-- НЕ УТКА
	...
	|	ИсходныеДанные.НомерГруппыЗатрат,
	//++ НЕ УТ
	|	ИсходныеДанные.СтатьяКалькуляции КАК СтатьяКалькуляции,
	//++ НЕ УТКА
	|	ИсходныеДанные.ЭтапПроизводства,
	//-- НЕ УТКА
	...
	//++ НЕ УТ
	|			И ИсходныеДанные.СтатьяКалькуляции = НовыеДанные.СтатьяКалькуляции
	//++ НЕ УТКА
	|			И ИсходныеДанные.ЭтапПроизводства = НовыеДанные.ЭтапПроизводства
	//-- НЕ УТКА
	...
	//++ НЕ УТ
	|	ДанныеРасхождений.СтатьяКалькуляции,
	//++ НЕ УТКА
	|	ДанныеРасхождений.ЭтапПроизводства,
	//-- НЕ УТКА
	...
	//++ НЕ УТ
	|	УчтенныеРасхождения.СтатьяКалькуляции КАК СтатьяКалькуляции,
	//++ НЕ УТКА
	|	УчтенныеРасхождения.ЭтапПроизводства КАК ЭтапПроизводства,
	//-- НЕ УТКА
	...
	|	ИсходныеДанные.СтатьяКалькуляции КАК СтатьяКалькуляции,
	//++ НЕ УТКА
	|	ИсходныеДанные.ЭтапПроизводства КАК ЭтапПроизводства,
	//-- НЕ УТКА
	...
	//++ НЕ УТ
	|	ДанныеРасхождений.СтатьяКалькуляции КАК СтатьяКалькуляции,
	//++ НЕ УТКА
	|	ДанныеРасхождений.ЭтапПроизводства,
	//-- НЕ УТКА
