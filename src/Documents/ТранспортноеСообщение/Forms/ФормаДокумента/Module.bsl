...
&НаКлиенте
Процедура НадписьРекомендацииОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки,
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "Отчет" Тогда
		СтандартнаяОбработка = Ложь;
...
Процедура КомандаКонтейнер(Команда)
	...
	Если Перемещен > 0 Тогда
		ОписаниеОповещения  = Новый ОписаниеОповещения("КомандаКонтейнерПродолжение", ЭтотОбъект);
			...
			Если ФайлыОшибкиОбработкиПакета.Количество() > 0 Тогда
				...
				Если ЗначениеЗаполнено(ИмяФайлаОшибки) Тогда
					...
					Если ДеревоXML <> Неопределено Тогда
						...
						ПоказатьРекомендацииПоОшибкеПриНеобходимости();
...
&НаСервере
Процедура ПоказатьРекомендацииПоОшибкеПриНеобходимости()
	
	ЭтоОшибкаРасшифровкиФайла = Объект.Тип = Перечисления.ТипыТранспортныхСообщений.УведомлениеОбОшибкеПФР
		И ЗначениеЗаполнено(Объект.ЦиклОбмена) И Объект.ДатаТранспорта >= '20240824'
		И СтрНайти(ВРег(ОписаниеОшибки), "CRYPTO_KEY_NOT_EXIST") <> 0;
	Если НЕ ЭтоОшибкаРасшифровкиФайла Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Реком.по ошиб.расшиф.ЭДОК",
		Истина) Тогда
		
		Возврат;
	КонецЕсли;
	
	СвойстваЦиклаОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ЦиклОбмена,
		"Предмет, ФорматДокументооборота");
	Если СвойстваЦиклаОбмена.ФорматДокументооборота <> Перечисления.ФорматыДокументооборотаСПФР.ЭДОК Тогда
		Возврат;
	КонецЕсли;
	...
	ПротоколыЦиклаОбмена = КонтекстЭДОСервер.ПолучитьСообщенияЦиклаОбмена(Объект.ЦиклОбмена, ТипыСообщений);
	Если ПротоколыЦиклаОбмена.Количество() <> 0 Тогда
		Возврат;
	КонецЕсли;
	
	ТекстТехническиеРаботыНаСервереСФР =
		НСтр("ru = 'По причине внеплановых технических работ на сервере СФР отчет не был принят.';
			|en = 'По причине внеплановых технических работ на сервере СФР отчет не был принят.'");
	
	СсылкаНаОбъект = СвойстваЦиклаОбмена.Предмет;
	НазваниеОбъектаВИменительномПадеже = ДлительнаяОтправкаКлиентСервер.НазваниеОбъектаВИменительномПадеже(
		СсылкаНаОбъект);
	СсылкаОбъектаВИменительномПадеже = Новый ФорматированнаяСтрока(НазваниеОбъектаВИменительномПадеже,,,, "Отчет");
	ТекстПовторнаяОтправкаОтчета = Новый ФорматированнаяСтрока(
		НСтр("ru = 'Необходимо отправить';
			|en = 'Необходимо отправить'") + " ",
		СсылкаОбъектаВИменительномПадеже,
		" " + НСтр("ru = 'повторно.';
					|en = 'повторно.'"));
	
	РежимАвтонастройки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.УчетнаяЗапись, "РежимАвтонастройки");
	
	Если РежимАвтонастройки = Перечисления.РежимыАвтонастройкиУчетнойЗаписиНалогоплательщика.Отключена
		ИЛИ НЕ ЗначениеЗаполнено(РежимАвтонастройки) Тогда
		...
		Элементы.НадписьРекомендации.Заголовок = Новый ФорматированнаяСтрока(
			ТекстТехническиеРаботыНаСервереСФР + "
			|" + НСтр("ru = 'Необходимо отправить';
						|en = 'Необходимо отправить'") + " ",
			СсылкаОбъектаВИменительномПадеже,
	...
	Элементы.ГруппаРекомендации.Видимость = Истина;
...
Процедура ОформитьСтрокуСодержимого(ДанныеСтроки)
	...
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПодтверждениеЗагрузкиЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Подтверждение загрузки документа';
									|en = 'Подтверждение загрузки документа'");
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОшибкеЗагрузкиЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Уведомление об ошибке загрузки документа';
									|en = 'Уведомление об ошибке загрузки документа'");
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.КвитанцияОПриемеВОбработкуОтчетаЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Квитанция о приеме в обработку';
									|en = 'Квитанция о приеме в обработку'");
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОтказеОбработкиОтчетаЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Уведомление об отказе от приема в обработку';
									|en = 'Уведомление об отказе от приема в обработку'");
	...
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.КвитанцияОРезультатахОбработкиОтчетаЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Квитанция о результатах обработки';
									|en = 'Квитанция о результатах обработки'");
	...
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПротоколПриложениеЦБ Тогда
		ПредставлениеЯчейки = НСтр("ru = 'Приложение к протоколу ошибок';
									|en = 'Приложение к протоколу ошибок'");
	...
	ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ЭлектронноЦифроваяПодпись Тогда
		...
		ИначеЕсли Объект.ЦиклОбмена.Вид = Перечисления.ВидыЦикловОбмена.ЦиклОбменаСЦБ Тогда
			ПредставлениеЯчейки = НСтр("ru = 'Электронная подпись представителя ЦБ';
										|en = 'Электронная подпись представителя ЦБ'");
...
Процедура КомандаРасшифроватьТСЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		КонтекстЭДОКлиент.РасшифроватьТранспортноеСообщение(Объект, ОписаниеОповещения, , ПараметрыОперации);
	КонецЕсли;
...
Процедура ПолучитьТранспортныйКонтейнерНаСервере()
	...
	Для Каждого ПолученныйКонтейнер Из ПолученныеКонтейнеры Цикл
		...
		НовыйКонтейнер.Перемещен = ПолученныйКонтейнер.Перемещен;
	КонецЦикла;
	...
	Если КонтекстЭДОСервер = Неопределено Тогда 
		...
		Если КонтекстЭДОСервер = Неопределено Тогда 
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Вложения транспортного сообщения недоступны.%1';
																								|en = 'Вложения транспортного сообщения недоступны.%1'"),
...
&НаСервере
Процедура УдалитьСодержимое()
	
	КонтекстЭДОСервер = ДокументооборотСКО.ПолучитьОбработкуЭДО();
	КонтекстЭДОСервер.УдалитьСодержимоеТранспортногоКонтейнера(Объект.Ссылка);
	
КонецПроцедуры

&НаКлиенте
Процедура Перерасшифровать(Команда)
	
	УдалитьСодержимое();
	КомандаРасшифроватьТСЗавершение(КодВозвратаДиалога.Да, Неопределено); // асинх
