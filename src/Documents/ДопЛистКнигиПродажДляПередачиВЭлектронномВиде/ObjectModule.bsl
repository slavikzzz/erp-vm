...
Функция ВыгрузитьДокумент(УникальныйИдентификатор = Неопределено) Экспорт
	...
	Если НалоговыйПериод >= '2025-01-01' Тогда
		Результат = УчетНДСФормированиеОтчетности.ЭлектронноеПредставлениеДопЛистовКнигиПродаж_507(ЭтотОбъект);
		
	ИначеЕсли НалоговыйПериод >= '2021-07-01' Тогда
		Результат = УчетНДСФормированиеОтчетности.ЭлектронноеПредставлениеДопЛистовКнигиПродаж_506(ЭтотОбъект);
		
	ИначеЕсли НалоговыйПериод >= '2019-01-01' Тогда
		Результат = УчетНДСФормированиеОтчетности.ЭлектронноеПредставлениеДопЛистовКнигиПродаж_504(ЭтотОбъект);
		
	ИначеЕсли НалоговыйПериод >= '2017-10-01' Тогда
		Результат = УчетНДСФормированиеОтчетности.ЭлектронноеПредставлениеДопЛистовКнигиПродаж_503(ЭтотОбъект);
		
	ИначеЕсли НалоговыйПериод >= '2014-10-01' Тогда
		Результат = УчетНДСФормированиеОтчетности.ЭлектронноеПредставлениеДопЛистовКнигиПродаж_502(ЭтотОбъект);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	НайденныеДокументы	= Документы.ДопЛистКнигиПродажДляПередачиВЭлектронномВиде.НайтиДокументыЗаНалоговыйПериод(Организация, НалоговыйПериод, Дата, ?(НалоговыйПериод < '20141001',0,2));
		...
		Для каждого Документ Из НайденныеДокументы Цикл
			...
			Если НЕ Документ = Ссылка Тогда
				ТекстСообщения	= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Уже имеется оформленный дополнительный лист книги продаж за %1 по периоду %2';
						|en = 'There is already created additional sales ledger sheet for %1 for period %2'"), 
					Формат(Дата, "ДЛФ=ДД"), 
	...
	Если ДополнительныеСвойства.Свойство("АдресДанныхДляПередачи") Тогда 
		...
		Если РазделыОтчета.Количество() = 0 Тогда 
			ТекстСообщения	= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Нет данных для заполнения дополнительного листа книги продаж за период %1';
						|en = 'Data is missing for filling in additional sales ledger sheet for period %1'"), 
							ПредставлениеПериода(НачалоКвартала(НалоговыйПериод), КонецКвартала(НалоговыйПериод), "ФП = Истина"));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Документ, , , Отказ);
			Возврат;
		КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НачалоДня(Дата) <= КонецКвартала(НалоговыйПериод)
		...
		ТекстСообщения	= НСтр("ru = 'Дата документа должна быть позже конца налогового периода';
								|en = 'Document date must be later than the fiscal period end '");
...
Функция ПолучитьСледующийНомерДополнительногоЛиста()
	...
	НайденныеДокументы	= Документы.ДопЛистКнигиПродажДляПередачиВЭлектронномВиде.НайтиДокументыЗаНалоговыйПериод(Организация, НалоговыйПериод, Дата,  -1);
	Если НЕ НайденныеДокументы = Неопределено Тогда
...
Процедура ЗаполнитьДаннымиИтого(Параметры, ДеревоВыгрузки)
	...
	УзелСвДопЛКнПрод = ПолучитьПодчиненныйЭлемент(УзелДокумент, "СвДопЛКнПрод");
	
	УзелВсего = ПолучитьПодчиненныйЭлемент(УзелСвДопЛКнПрод, "Итого");
	УстановитьЗначениеЭлемента(УзелВсего, "СтТовУчНалВсего", ВсегоПродажДо);
	
	ОбразецУзлаВтЧисле = ПолучитьПодчиненныйЭлемент(УзелВсего, "ВтЧисле");
...
Процедура ЗаполнитьДаннымиВсего(Параметры, ДеревоВыгрузки)
	...
	УзелВсего = ПолучитьПодчиненныйЭлемент(УзелСвДопЛКнПрод, "Всего");
	УстановитьЗначениеЭлемента(УзелВсего, "СтТовУчНалВсего", ВсегоПродаж);
...
Процедура ЗаполнитьДаннымиТабличнойЧасти(Параметры, ДеревоВыгрузки)
	...
	ОбразецУзелСвПродаж = ПолучитьПодчиненныйЭлемент(УзелДокумент, "СвПродаж");
	Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
		НовыйУзелСвПродаж = ПродублированныйУзелОбразец(ОбразецУзелСвПродаж);
		
		ДатаНомерСчетаФактуры = СтрокаТабличнойЧасти.ДатаНомер;
		РеквизитыСчетаФактуры = РасщепленнаяСтрока(ДатаНомерСчетаФактуры, ";");
		УстановитьЗначениеЭлемента(НовыйУзелСвПродаж, "ДатаСчФ", СокрЛП(РеквизитыСчетаФактуры[0]));
		УстановитьЗначениеЭлемента(НовыйУзелСвПродаж, "НомерСчФ", СокрЛП(РеквизитыСчетаФактуры[1]));
		...
		УстановитьЗначениеЭлемента(НовыйУзелСвПродаж, "НомИспрСчФ", СокрЛП(РеквизитыИсправленияСчетаФактуры[0]));
		УстановитьЗначениеЭлемента(НовыйУзелСвПродаж, "ДатаИспрСчФ", СокрЛП(РеквизитыИсправленияСчетаФактуры[1]));
		...
		УстановитьЗначениеЭлемента(НовыйУзелСвПродаж, "НомерКСчФ", СокрЛП(РеквизитыКорректировочногоСчетаФактуры[0]));
		УстановитьЗначениеЭлемента(НовыйУзелСвПродаж, "ДатаКСчФ", СокрЛП(РеквизитыКорректировочногоСчетаФактуры[1]));
		...
		УстановитьЗначениеЭлемента(НовыйУзелСвПродаж, "НомИспрКСчФ", СокрЛП(РеквизитыИсправленияКорректировочногоСчетаФактуры[0]));
		УстановитьЗначениеЭлемента(НовыйУзелСвПродаж, "ДатаИспрКСчФ", СокрЛП(РеквизитыИсправленияКорректировочногоСчетаФактуры[1]));
		
		УстановитьЗначениеЭлемента(НовыйУзелСвПродаж, "НаимПок", СтрокаТабличнойЧасти.Покупатель);
		ИНН = СокрЛП(СтрокаТабличнойЧасти.ПокупательИНН);
		Если СтрДлина(ИНН) = 12 Тогда
			УстановитьЗначениеЭлемента(НовыйУзелСвПродаж, "ИННФЛ", ИНН);
		Иначе
			УстановитьЗначениеЭлемента(НовыйУзелСвПродаж, "ИННЮЛ", ИНН);
			УстановитьЗначениеЭлемента(НовыйУзелСвПродаж, "КПП", СтрокаТабличнойЧасти.ПокупательКПП);
		КонецЕсли;
		
		УстановитьЗначениеЭлемента(НовыйУзелСвПродаж, "СтТовУчНалВсего", СтрокаТабличнойЧасти.ВсегоПродаж);
		
		ОбразецУзлаДатаОплСчФПрод = ПолучитьПодчиненныйЭлемент(НовыйУзелСвПродаж, "ДатаОплСчФПрод");
		...
		ОбразецУзлаВтЧисле = ПолучитьПодчиненныйЭлемент(НовыйУзелСвПродаж, "ВтЧисле");
		УдалятьОбразецУзлаВтЧисле = Ложь;
		// НДС 18%.
		Если СтрокаТабличнойЧасти.СуммаБезНДС18 <> 0 ИЛИ СтрокаТабличнойЧасти.НДС18 <> 0 Тогда
			НовыйУзелВтЧисле = ПродублированныйУзелОбразец(ОбразецУзлаВтЧисле);
			ЗаполнитьДаннымиПоСтавкеНДС(НовыйУзелВтЧисле, "18", СтрокаТабличнойЧасти.СуммаБезНДС18, СтрокаТабличнойЧасти.НДС18);
			УдалятьОбразецУзлаВтЧисле = Истина;
		КонецЕсли;
		// НДС 10%.
		Если СтрокаТабличнойЧасти.СуммаБезНДС10 <> 0 ИЛИ СтрокаТабличнойЧасти.НДС10 <> 0 Тогда
			НовыйУзелВтЧисле = ПродублированныйУзелОбразец(ОбразецУзлаВтЧисле);
			ЗаполнитьДаннымиПоСтавкеНДС(НовыйУзелВтЧисле, "10", СтрокаТабличнойЧасти.СуммаБезНДС10, СтрокаТабличнойЧасти.НДС10);
			УдалятьОбразецУзлаВтЧисле = Истина;
		КонецЕсли;
		// НДС 0%.
		Если СтрокаТабличнойЧасти.НДС0 <> 0 Тогда
			НовыйУзелВтЧисле = ПродублированныйУзелОбразец(ОбразецУзлаВтЧисле);
			ЗаполнитьДаннымиПоСтавкеНДС(НовыйУзелВтЧисле, "0", СтрокаТабличнойЧасти.НДС0);
			УдалятьОбразецУзлаВтЧисле = Истина;
		КонецЕсли;
		// НДС 20%.
		Если СтрокаТабличнойЧасти.СуммаБезНДС20 <> 0 ИЛИ СтрокаТабличнойЧасти.НДС20 <> 0 Тогда
			НовыйУзелВтЧисле = ПродублированныйУзелОбразец(ОбразецУзлаВтЧисле);
			ЗаполнитьДаннымиПоСтавкеНДС(НовыйУзелВтЧисле, "20", СтрокаТабличнойЧасти.СуммаБезНДС20, СтрокаТабличнойЧасти.НДС20);
			УдалятьОбразецУзлаВтЧисле = Истина;
		КонецЕсли;
		// НДС Без НДС.
		Если СтрокаТабличнойЧасти.СуммаСовсемБезНДС <> 0 Тогда
			НовыйУзелВтЧисле = ПродублированныйУзелОбразец(ОбразецУзлаВтЧисле);
			ЗаполнитьДаннымиПоСтавкеНДС(НовыйУзелВтЧисле, "без НДС", СтрокаТабличнойЧасти.СуммаСовсемБезНДС); // Не локализуется
			УдалятьОбразецУзлаВтЧисле = Истина;
		КонецЕсли;
		Если УдалятьОбразецУзлаВтЧисле Тогда
			УдалитьУзел(ОбразецУзлаВтЧисле);
	...
	КонецЦикла;
	УдалитьУзел(ОбразецУзелСвПродаж);
