...
Функция ПодтверждениеПоступленияТоваровПоЗаявкеНаВыпускКиЗXML(ДокументСсылка)
	...
	Пока ВыборкаПоДокументамПоступления.Следующий() Цикл
		...
		Иначе
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru = 'Не заполнен номер ГИСМ документа %1.';
																						|en = 'Не заполнен номер ГИСМ документа %1.'"), ДокументСсылка));
		КонецЕсли;
		...
		Исключение
			СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ru = 'В документе %1 не заполнен номер входящего документа.';
														|en = 'В документе %1 не заполнен номер входящего документа.'"), ВыборкаПоДокументамПоступления.ДокументПоступления);
		КонецПопытки;
		
		Попытка
			ПодтверждениеПоступления.commit_doc_date = ВыборкаПоДокументамПоступления.ДатаВходящегоДокумента;
		Исключение
			СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ru = 'В документе %1 не заполнена дата входящего документа.';
														|en = 'В документе %1 не заполнена дата входящего документа.'"), , ВыборкаПоДокументамПоступления.ДокументПоступления);
		КонецПопытки;
...
Функция ЗаявкаНаВыпускКиЗXML2_40(ДокументСсылка)
	...
	Исключение
		СообщениеXML.ТекстОшибки = НСтр("ru = 'Не заполнен номер договора с эмитентом.';
										|en = 'Не заполнен номер договора с эмитентом.'");
	КонецПопытки;
	...
	Исключение
		СообщениеXML.ТекстОшибки = НСтр("ru = 'Не заполнена дата договора с эмитентом.';
										|en = 'Не заполнена дата договора с эмитентом.'");
	КонецПопытки;
