...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОписаниеНДС = ?(Объект.ВариантУказанияЦеныТовара, НСтр("ru = 'с НДС';
															|en = 'VAT inclusive'"), НСтр("ru = 'без НДС';
																				|en = 'Excluding VAT'"));
	Элементы.ТоварыМаксимальнаяЦена.Заголовок = СтрШаблон(НСтр("ru = 'Максимальная цена (%1) %2';
																|en = 'Maximum price (%1) %2'"), Объект.Валюта, ОписаниеНДС);
	...
	Если ДействиеДекорации = 0 Тогда
		...
		Иначе
			...
			ПоказатьВопрос(Описание, НСтр("ru = 'Для продолжения необходимо записать документ.
                                   |Продолжить?';
                                   |en = 'The document must be saved to continue.
