...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ПараметрыНастройки = Документы.КорректировкаЗадолженности.ПараметрыНастройкиСчетовУчета(Объект.ХозяйственнаяОперация);
...
Процедура ПереключательРасходыПриИзменении(Элемент)
	...
	Если РасходыСписком Тогда
		Если Объект.Задолженность.Количество() > 1 Тогда
			...
			|Продолжить?';
			|en = 'You can adjust AR/AP of the item list only for one AR/AP object.
			|AR/AP table will be reduced to one record.
			|Continue?'");
	...
	Иначе
		Если Объект.ДоходыРасходыАктивыПассивы.Количество() > 1 Тогда
			...
			|Продолжить?';
			|en = 'Item table will be reduced to one record.
			|Continue?'");
...
Процедура ПереключательЗадолженностьПриИзменении(Элемент)
	...
	Если ЗадолженностьСписком Тогда
		Если Объект.ДоходыРасходыАктивыПассивы.Количество() > 1 Тогда
			...
			|Продолжить?';
			|en = 'You can adjust AR/AP of the AR/AP object list only for one item.
			|Item table will be reduced to one record.
			|Continue?'");
			ДополнительныеПараметры.Вставить("ЗадолженностьМногиеКОдному");
			ПоказатьВопрос(ОбработчикОтвета, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Иначе
	...
	Иначе
		Если Объект.Задолженность.Количество() > 1 Тогда
			...
			|Продолжить?';
			|en = 'AR/AP table will be reduced to one record.
			|Continue?'");
			ДополнительныеПараметры.Вставить("ЗадолженностьОдинКоМногим");
			ПоказатьВопрос(ОбработчикОтвета, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Иначе
