...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ДоходыРасходыАктивыПассивы.Количество() > 1
		...
		Текст = НСтр("ru = 'Итоговая сумма по статьям не совпадает с суммой корректируемой задолженности.';
					|en = 'Total items amount does not match the AR/AP amount to adjust.'");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ПараметрыНастройки = Документы.КорректировкаЗадолженности.ПараметрыНастройкиСчетовУчета(ХозяйственнаяОперация);
