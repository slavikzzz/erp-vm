...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	Если Параметры.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5 Тогда
		Заголовок = НСтр("ru = 'Заказы переработчикам';
...
Процедура ЗаполнитьСписокРаспоряжений()
	...
	//++ НЕ УТ
	|		КОГДА ТИПЗНАЧЕНИЯ(РаспоряженияКОформлению.Распоряжение) = ТИП(Документ.ЗаказПереработчику2_5)
	|			ТОГДА ВЫРАЗИТЬ(РаспоряженияКОформлению.Распоряжение КАК Документ.ЗаказПереработчику2_5).Приоритет
	//-- НЕ УТ
	|	КОНЕЦ                                             КАК Приоритет,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(РаспоряженияКОформлению.Распоряжение) = ТИП(Документ.ЗаказКлиента)
	...
	//++ НЕ УТ
	|		КОГДА ТИПЗНАЧЕНИЯ(РаспоряженияКОформлению.Распоряжение) = ТИП(Документ.ЗаказПереработчику2_5)
	|			И ВЫРАЗИТЬ(РаспоряженияКОформлению.Распоряжение КАК Документ.ЗаказПереработчику2_5).Приоритет В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					Приоритеты.Ссылка КАК Приоритет
	|				ИЗ
	|					Справочник.Приоритеты КАК Приоритеты
