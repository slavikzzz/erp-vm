...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	НастройкиОпераций.Вставить(
		...
		ПолучитьФункциональнуюОпцию("ИспользоватьАктыРасхожденийПослеОтгрузкиПоПередачамПереработчику2_5"));
	//-- НЕ УТ
	...
	//++ НЕ УТ
	Команда = Документы.ПоступлениеТоваровОтХранителя.ДобавитьКомандуСоздатьНаОснованииВозвратОтПереработчика(КомандыСозданияНаОсновании);
	Если Команда <> Неопределено Тогда
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(
			Команда,
			"ХозяйственнаяОперация",
	...
	//++ НЕ УТКА
	Документы.ПриемкаТоваровНаХранение.ДобавитьКомандуСоздатьНаОснованииПередачиПереработчику(КомандыСозданияНаОсновании);
	//-- НЕ УТКА
...
// Добавляет команду создания документа "Передача товаров хранителю" на основании Заказа переработчику.
//
// Параметры:
//  КомандыСозданияНаОсновании - см. СозданиеНаОснованииПереопределяемый.ПередДобавлениемКомандСозданияНаОсновании.КомандыСозданияНаОсновании
//
// Возвращаемое значение:
//  СтрокаТаблицыЗначений из см. СозданиеНаОснованииПереопределяемый.ПередДобавлениемКомандСозданияНаОсновании.КомандыСозданияНаОсновании, Неопределено -
//
Функция ДобавитьКомандуСоздатьНаОснованииЗаказаПереработчику(КомандыСозданияНаОсновании) Экспорт
	
	Если ПравоДоступа("Добавление", Метаданные.Документы.ПередачаТоваровХранителю) Тогда
		
		ИмяОбработчика = "ПереработкаНаСторонеКлиент.ПередачаТоваровХранителюСоздатьНаОснованииЗаказаПереработчику";
		
		КомандаСоздатьНаОсновании = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Обработчик          = ИмяОбработчика;
		КомандаСоздатьНаОсновании.Идентификатор       = "СоздатьНаОснованииЗаказаПереработчику";
		КомандаСоздатьНаОсновании.Представление       = НСтр("ru = 'Передача сырья переработчику';
...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат ПередачаТоваровХранителюЛокализация.ТекстОтраженияВРеглУчете();
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	Возврат ПередачаТоваровХранителюЛокализация.ТекстЗапросаВТОтраженияВРеглУчете();
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ИначеЕсли Данные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5 Тогда
		ПредставлениеОбъекта = НСтр("ru = 'Передача сырья переработчику';
									|en = 'Goods issue — Subcontracting services received'");
	//-- НЕ УТ
	Иначе
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'Передача товаров комиссионеру';
										|en = 'Customer stock fill-up documents'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	КомандаПечати.Порядок = 11;
	УправлениеПечатью.ДобавитьУсловиеВидимостиКоманды(КомандаПечати,
													  "ХозяйственнаяОперация",
	...
	Если НЕ ЭтоПартнер
		...
		КомандаПечати.Представление                  = НСтр("ru = 'Этикетки поставки на склады Ozon';
															|en = 'Distribution package labels to Ozon warehouses'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		...
		Пока ДанныеПечати.Следующий() Цикл
			...
			Пока ВыборкаПоСкладам.Следующий() Цикл
				...
				ПредставлениеРаспоряжения = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(
					РеквизитыДокумента,
					НСтр("ru = 'Передача товаров хранителю';
						|en = 'VMI fill-up'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		СтрокаЗаголовка = ?(ДанныеПечати.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию,
			НСтр("ru = 'Передача товаров комиссионеру';
				|en = 'Customer stock fill-up documents'", ОбщегоНазначения.КодОсновногоЯзыка()),
			НСтр("ru = 'Передача товаров хранителю';
				|en = 'VMI fill-up'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, 
	...
	|					ИЛИ Номенклатура.ОсобенностьУчета В (&ОсобенностиУчетаНоменклатуры))) КАК ЗаказыОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		И ЗаказыДвижения.Склад В(
	|			ВЫБРАТЬ
	|				СкладыПередачи.Ссылка
	|			ИЗ
	|				СкладыПередачи КАК СкладыПередачи)
	|		И (&НеОтбиратьПоОсобенностямУчета
	|				ИЛИ ЗаказыДвижения.Номенклатура.ОсобенностьУчета В (&ОсобенностиУчетаНоменклатуры))
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|
	|ВЫБРАТЬ
	|	ЗаказТовары.Ссылка.ЗаказПереработчику.Сделка   КАК Сделка,
	|	ЗаказТовары.НомерСтроки                        КАК НомерСтроки,
	|	ТаблицаОстатки.ЗаказКлиента                    КАК ЗаказКлиента,
	|	ТаблицаОстатки.КодСтроки                       КАК КодСтроки,
	|	ТаблицаОстатки.Склад                           КАК Склад,
	...
	|	ТаблицаОстатки.Номенклатура.КодТНВЭД           КАК КодТНВЭД,
	|	ТаблицаОстатки.Характеристика                  КАК Характеристика,
	|	&ЗаказКлиентаНазначениеЭтап                    КАК Назначение,
	|	ВЫБОР
	|		КОГДА ЗаказТовары.Обособленно
	|			ТОГДА &ЗаказКлиентаНазначениеОтправителя
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	...
	|	ЕСТЬNULL(НакладнаяТовары.Упаковка, ЗаказТовары.Упаковка)     КАК Упаковка,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(НакладнаяТовары.Упаковка, ЗаказТовары.Упаковка) =
	...
	|		/ ЕСТЬNULL(ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, &ТекстЗапросаКоэффициентУпаковки2), 1) КАК КоличествоУпаковок,
	|	ЗаказТовары.Количество                         КАК КоличествоВЗаказе,
	|	ЕСТЬNULL(НакладнаяТовары.Количество, 0)        КАК КоличествоВНакладной,
	|	ЗаказТовары.Ссылка.ЗаказПереработчику.ВидЦены  КАК ВидЦены,
	|	0                                              КАК Цена,
	|	0                                              КАК Сумма,
	|	ТаблицаОстатки.Сумма                           КАК СуммаОстаток,
	|	ЗаказТовары.ДатаОтгрузки                       КАК ДатаОтгрузки,
	|	0                                              КАК СрокПоставки,
	|	ЗаказТовары.ВариантОбеспечения                 КАК ВариантОбеспечения,
	|	ВЫБОР
	...
	|	ТаблицаОстатки КАК ТаблицаОстатки
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ОбеспечениеМатериаламиИРаботами КАК ЗаказТовары
	|	ПО ТаблицаОстатки.КодСтроки = ЗаказТовары.КодСтроки
	|	И ТаблицаОстатки.ЗаказКлиента = ЗаказТовары.Ссылка.ЗаказПереработчику
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтТоварыПередачиПоЗаказу КАК НакладнаяТовары
	|	ПО ТаблицаОстатки.КодСтроки = НакладнаяТовары.КодСтроки
	|	И ТаблицаОстатки.ЗаказКлиента = НакладнаяТовары.ЗаказКлиента
	...
	|		ЗаказыОстатки.Характеристика     КАК Характеристика,
	|		ЗаказыОстатки.Серия              КАК Серия,
	|		-ЗаказыОстатки.КОформлениюРасход КАК Количество,
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	ИспользоватьПроизводствоНаСтороне =
		...
		И ПравоДоступа("Чтение", Метаданные.Документы.ЗаказПереработчику2_5);
	//-- НЕ УТ
	...
	//++ НЕ УТ
			И Не ИспользоватьПроизводствоНаСтороне
	//-- НЕ УТ
	...
	//-- НЕ УТ
		...
		|ИЗ
		|	(ВЫБРАТЬ
		|		ТаблицаДанных.ЗаказКлиента КАК Ссылка,
		...
		|	ИЗ
		|		РегистрНакопления.ЗаказыКлиентов.Остатки(,
		|			ТИПЗНАЧЕНИЯ(ЗаказКлиента) В (&ТипыРаспоряжений)
		|			) КАК ТаблицаДанных
		|		
		|	ГДЕ
		|		ТаблицаДанных.КОформлениюОстаток > 0
		|		И ИСТИНА В
		|			(ВЫБРАТЬ ПЕРВЫЕ 1
		|				ИСТИНА
		|			ИЗ
		...
		|			ГДЕ
		|				РеестрДокументов.Ссылка = ТаблицаДанных.ЗаказКлиента
		...
		ПодставитьТекстИспользоватьРасширенныеВозможностиЗаказаКлиента(Запрос.Текст, "РеестрДокументов", 
				"&ИспользоватьРасширенныеВозможностиЗаказов", "ТаблицаДанных");
		...
		Дело.Представление  = НСтр("ru = 'Распоряжения на оформление в работе';
									|en = 'References for registration in progress'");
	...
	Если ИспользоватьПроизводствоНаСтороне Тогда
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("СтруктураБыстрогоОтбора", ПараметрыОтбора);
		ПараметрыФормы.Вставить("ИмяТекущейСтраницы",      "СтраницаПередачиСырья");
		ПараметрыФормы.Вставить("КлючНазначенияФормы",     "ТекущиеДела");
		
		Дело = ТекущиеДела.Добавить();
		Дело.Идентификатор  = "ПередачаПереработчикуКОформлению2_5";
		Дело.ЕстьДела       = Результат > 0;
		Дело.Представление  = НСтр("ru = 'Передачи переработчику к оформлению';
									|en = 'Transfers to subcontractor for registration'");
...
Функция ТекстЗапросаОстаткиТоваровКОформлению(ИмяВременнойТаблицы, ОтборПоИзмерениям = Неопределено) Экспорт
	...
	ТекстЗапроса =
		РегистрыНакопления.ЗаказыКлиентов.ТекстЗапросаОстатки(
			ИмяВременнойТаблицы,
			ОтборПоИзмерениям,
			"КОформлению > 0");
...
Процедура РедактироватьТекстЗапросаПоОстаткамЗаказов(ТекстЗапроса) Экспорт
	...
	//++ НЕ УТКА
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗаказКлиентаНазначениеЭтап",            "ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)");
	//-- НЕ УТКА
...
Функция ЗаголовокФормыСозданияАкта() Экспорт
	...
	Возврат НСтр("ru = 'Акт о расхождениях после передачи товаров хранителю (создание)';
				|en = 'Discrepancy report following the VMI fill-up (create)'");
...
Функция ЗаголовокФормыАкта() Экспорт
	...
	Возврат НСтр("ru = 'Акт о расхождениях после передачи товаров хранителю';
				|en = 'Discrepancy report following the VMI fill-up'");
...
Процедура ПодставитьТекстИспользоватьРасширенныеВозможностиЗаказаКлиента(
		ТекстЗапроса, ИмяТаблицы, Параметр = "&ИспользоватьРасширенныеВозможностиЗаказаКлиента", ИмяТаблицыТовары = "") Экспорт
	
	//++ НЕ УТ
	ХозяйственнаяОперацияДокумента = Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5;
	ХозяйственнаяОперацияДоговора  = ПереработкаНаСтороне.ХозяйственнаяОперацияДоговора();
	
	ИмяОперацииДокумента = ОбщегоНазначения.ИмяЗначенияПеречисления(ХозяйственнаяОперацияДокумента);
	ИмяОперацииДоговора  = ОбщегоНазначения.ИмяЗначенияПеречисления(ХозяйственнаяОперацияДоговора);
		
	НовыйТекст = "(%1.ХозяйственнаяОперация В (
				  |					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.%2),
				  |					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.%3))
				  |ИЛИ %4)";
				  
	НовыйТекст = СтрШаблон(НовыйТекст, ИмяТаблицы, ИмяОперацииДокумента, ИмяОперацииДоговора, Параметр);
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, Параметр, НовыйТекст);
	//-- НЕ УТ
	
	Если Не ИмяТаблицыТовары = "" Тогда
		НовыйТекст = "НЕ ЕСТЬNULL(%1.ЗаказКлиента.ЭтоЗаказКакСчет, НЕ %2)"; //@Query-part
		НовыйТекст = СтрШаблон(НовыйТекст, ИмяТаблицыТовары, Параметр);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, Параметр, НовыйТекст);
	КонецЕсли;
...
Процедура ПривязатьСтрокиПередачиКЗаказам(Форма, Объект, ПараметрыУказанияСерий, ОтобратьПоЗаказу) Экспорт
	...
	//++ НЕ УТКА
	|		КОГДА ЗаказПереработчику.Ссылка ЕСТЬ НЕ NULL
	|			И ЗаказПереработчикуТовары.Назначение.ТипНазначения = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое2_5)
	|			ТОГДА ЗаказПереработчикуТовары.Назначение
	|		КОГДА ЭтапТовары.Ссылка ЕСТЬ НЕ NULL
	|			И ЭтапТовары.Назначение.ТипНазначения = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое2_5)
	|			ТОГДА ЭтапТовары.Назначение
	|		КОГДА ЭтапТовары.Ссылка ЕСТЬ НЕ NULL
	|			И ВЫРАЗИТЬ(ЭтапТовары.Ссылка.ЗаказПереработчику КАК Документ.ЗаказПереработчику2_5).ПередачаПереработчикуПоНазначениюЗаказа
	|			ТОГДА ВЫРАЗИТЬ(ЭтапТовары.Ссылка.ЗаказПереработчику КАК Документ.ЗаказПереработчику2_5).НазначениеПередачи
	//-- НЕ УТКА
	|		КОГДА ЗаказПереработчику.Ссылка ЕСТЬ НЕ NULL
	|			И ЗаказПереработчику.ПередачаПереработчикуПоНазначениюЗаказа
	|			ТОГДА ЗаказПереработчику.НазначениеПередачи
	//-- НЕ УТ
	|		ИНАЧЕ Значение(Справочник.Назначения.ПустаяСсылка)
	|	КОНЕЦ                                КАК Назначение,
	|	ВЫБОР
	...
	|			ТОГДА ЗаказКлиентаТовары.Ссылка.Назначение
	//++ НЕ УТ
	|		КОГДА ЗаказПереработчикуТовары.Ссылка ЕСТЬ НЕ NULL
	|			И ЗаказПереработчикуТовары.Обособленно
	|			ТОГДА ЗаказПереработчикуТовары.Назначение
	//++ НЕ УТКА
	|		КОГДА ЭтапТовары.Ссылка ЕСТЬ НЕ NULL
	|			И ЭтапТовары.Обособленно
	|			ТОГДА ЭтапТовары.Назначение
	//-- НЕ УТКА
	...
	//++ НЕ УТ
	|		КОГДА ЗаказПереработчику.Ссылка ЕСТЬ НЕ NULL
	|			ТОГДА ЗаказПереработчику.ВидЦены
	//-- НЕ УТ
	|	КОНЕЦ                                КАК ВидЦены,
	|	ВЫБОР
	|		КОГДА ЗаказКлиентаТовары.Ссылка ЕСТЬ НЕ NULL
	...
	//++ НЕ УТ
	|		КОГДА ЗаказПереработчикуТовары.Ссылка ЕСТЬ НЕ NULL
	|			ТОГДА ЗаказПереработчикуТовары.Цена
	//-- НЕ УТ
	|	КОНЕЦ                                КАК Цена,
	|	ТоварыКОформлению.Склад              КАК Склад,
	|	ВЫБОР
	...
	//++ НЕ УТ
	|		КОГДА ЗаказПереработчикуТовары.Ссылка ЕСТЬ НЕ NULL
	|			ТОГДА ЗаказПереработчикуТовары.СрокПоставки
	//-- НЕ УТ
	...
	|	
	//++ НЕ УТ
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику2_5 КАК ЗаказПереработчику
	|	ПО ЗаказПереработчику.Ссылка = ТоварыКОформлению.Распоряжение
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику2_5.ОбеспечениеМатериаламиИРаботами КАК ЗаказПереработчикуТовары
	|	ПО ЗаказПереработчикуТовары.Ссылка = ТоварыКОформлению.Распоряжение
	|	И ЗаказПереработчикуТовары.КодСтроки = ТоварыКОформлению.КодСтроки
	|	
	//++ НЕ УТКА
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ОбеспечениеМатериаламиИРаботами КАК ЭтапТовары
	|	ПО ЭтапТовары.Ссылка.ЗаказПереработчику = ТоварыКОформлению.Распоряжение
	|	И ЭтапТовары.КодСтроки = ТоварыКОформлению.КодСтроки
	//-- НЕ УТКА
	...
	//++ НЕ УТ
	|		КОГДА ЗаказПереработчикуТовары.Ссылка ЕСТЬ НЕ NULL
	|			ТОГДА ЗаказПереработчикуТовары.ДатаОтгрузки
	//++ НЕ УТКА
	|		КОГДА ЭтапТовары.Ссылка ЕСТЬ НЕ NULL
	|			ТОГДА ЭтапТовары.ДатаОтгрузки
	//-- НЕ УТКА
...
Функция ОбработчикДействий(ХозяйственнаяОперация = Неопределено) Экспорт
	...
	//++ НЕ УТ
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5 Тогда
		Обработчик = ПередачаПереработчику;
	//-- НЕ УТ
	Иначе
...
Функция ОбработчикДействийПоТипуЗаказа(ТипЗаказа) Экспорт
	...
	//++ НЕ УТ
	Если ТипЗаказа = ПередачаПереработчику.ТипЗаказа() Тогда
...
Функция ОбработчикДействийПоТипуОснованияАкта(ТипОснования) Экспорт
	...
	//++ НЕ УТ
	ИначеЕсли ТипОснования = ПередачаПереработчику.ТипОснованияАкта() Тогда
		ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5;
	//-- НЕ УТ
	Иначе
		ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи;
...
Функция ПолучитьДоступныеОперацииПоДокументу() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне2_5")
		И (ДоступныеХозяйственныеОперацииДокумента = Неопределено
			Или ДоступныеХозяйственныеОперацииДокумента.Найти(Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5) <> Неопределено) Тогда
		СпсиокДоступныхОпераций.Добавить(
			Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5);
	КонецЕсли;
	//-- НЕ УТ
