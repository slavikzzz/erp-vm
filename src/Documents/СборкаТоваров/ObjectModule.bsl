	...
	Если ВидыЗапасовДляСозданияСоставныхНомеровГТД.Количество() > 0 Тогда
		...
		ИначеЕсли ПрослеживаемыйКомплект
			...
			ТекстСообщения = НСтр("ru = 'При использовании прослеживаемого по РНПТ комплекта, требуется указать прослеживаемые комплектующие.
								...
								|en = 'When using a kit traceable by RNPT, specify traceable components.
								|Make sure that stock for traceable components has CCD numbers of the RNPT type.'");
		...
		ИначеЕсли Не ПрослеживаемыйКомплект
			...
			ТекстСообщения = НСтр("ru = 'При использовании прослеживаемой по РНПТ комплектующей, требуется указать прослеживаемый комплект.';
									|en = 'When using a component traceable by RNPT, specify the traceable kit.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Неопределено, "", "", Отказ)
		Иначе
