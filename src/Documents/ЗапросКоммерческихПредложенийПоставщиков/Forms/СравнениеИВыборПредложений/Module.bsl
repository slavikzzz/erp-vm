...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстЗаголовок = СтрШаблон(НСтр("ru = '. Требуется %1 %2. Цены пересчитаны в валюту %3 и %4';
									|en = '. Required %1 %2. Prices are recalculated in the currency %3 and %4'"),
		Количество,
		ЕдиницаИзмерения,
		Валюта,
		?(ЦенаВключаетНДС, НСтр("ru = 'включают НДС';
								|en = 'include VAT'") , НСтр("ru = 'не включают НДС';
																|en = 'do not include VAT'")));
	...
	СтрокаИтого = СтрШаблон(НСтр("ru = '%1 %2';
								|en = '%1 %2'"), Количество, ЕдиницаИзмерения);
...
Процедура ВыбратьПредложения(Команда)
	...
	Если СнятСРассмотрения Тогда
		...
		ПоказатьВопрос(Описание, НСтр("ru = 'Выбраны предложения снятые с рассмотрения. Продолжить?';
										|en = 'Quotations stopped being reviewed are selected. Continue?'"), 
...
Процедура ОтметитьКРассмотрению(Установить)
	...
	Если НеСохраняемыеИсточники.Количество() > 0 Тогда
		...
		Шаблон = НСтр("ru = 'Для источника ''%1'' операция не поддерживается';
						|en = 'The transaction is not supported for the source ''%1'''");
...
Функция ВыполнитьПоискДоступныхПредложенийВФоне(Знач ПараметрыПроцедуры, Знач АнализируемыеИсточникиПредложений, ИдентификаторЗадания)
	...
	НаименованиеФоновогоЗадания = СтрШаблон(НСтр("ru = 'Поиск доступных предложений по документу %1';
												|en = 'Search for available quotations in the document %1'"), ПараметрыПроцедуры.ДокументИсточник);
...
Процедура Подключаемый_ОбновитьАнализируемыеИсточники(Элемент)
	...
	Если Модифицированность Тогда 
		...
		ПоказатьВопрос(Описание, НСтр("ru = 'Для выполнения этого действия нужно записать внесенные изменения.
                                       |Продолжить?';
                                       |en = 'You must save changes to complete this action.
...
Функция ВыполнитьСохранениеИзмененийВДокументахВФоне(Знач Источники, Знач ПараметрыПроцедуры)
	...
	НаименованиеФоновогоЗадания = СтрШаблон(НСтр("ru = 'Сохранение изменений сделанных при выборе предложений в документе %1';
												|en = 'Save changes made during selection of quotations in the document %1'"), ПараметрыПроцедуры.ДокументИсточник);
...
Функция ВыполнитьПоискТорговыхПредложенийВФоне(Знач ПараметрыПроцедуры)
	...
	НаименованиеЗадания   = НСтр("ru = 'Получение доступных товаров из 1С:Торговая площадка';
								|en = 'Get available products from 1C:Trading Site'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(Описание, НСтр("ru = 'Данные были изменены. Сохранить изменения?';
