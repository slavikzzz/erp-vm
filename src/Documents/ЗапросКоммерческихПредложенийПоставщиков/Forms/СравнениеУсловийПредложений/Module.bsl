...
Процедура ДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.Текст = НСтр("ru = 'Скрыть';
								|en = 'Hide'") Тогда
		Скрыть = Истина;
	ИначеЕсли Область.Текст = НСтр("ru = 'К заказу';
									|en = 'To order'") Тогда
		Скрыть = Ложь;
	Иначе 
		Возврат;
			...
			Иначе
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					НСтр("ru = 'Для документа не доступен выбор предложения';
						|en = 'Selection of the quotation is not available for the document'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция НачатьПолучениеТаблицыДанныхПредложений(ПараметрыПроцедуры)
	...
	НаименованиеФоновогоЗадания = СтрШаблон(НСтр("ru = 'Сравнение предложений по документу %1';
												|en = 'Comparison of quotations in the document %1'"), ПараметрыПроцедуры.ДокументИсточник);
...
Процедура СформироватьТаблицуСравнения()
	...
	Для Каждого СтрокаИсточник Из СписокПредложений Цикл 
		...
		Если Не СтрокаИсточник.Использовать
			...
			СтрокаУсловияПредложение.Параметры.Охват           = СтрШаблон(НСтр("ru = '%1 (%2%%)';
																				|en = '%1 (%2%%)'"), СтрокаИсточник.СтруктураУсловий.Охват, Формат(ОхватПроценты, "ЧДЦ=0"));;
