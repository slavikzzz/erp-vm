	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Если НЕ ВыборкаПоТабличнымЧастям.НайтиСледующий(Новый Структура("Ссылка",ВыборкаПоДокументам.Ссылка)) Тогда
			ТекстСообщения = НСтр("ru = 'Невозможно напечатать ""%Документ%"", т.к. не заполнена табличная часть.';
									|en = 'Cannot print ""%Документ%"" because table is not filled in.'");
		...
		Если ВыборкаПоДокументам.Вид = Перечисления.ВидыУпаковочныхЛистов.Входящий Тогда
			ТекстЗаголовка = ТекстЗаголовка  + " " + НСтр("ru = '(входящий)';
															|en = '(incoming)'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ИначеЕсли ВыборкаПоДокументам.Вид = Перечисления.ВидыУпаковочныхЛистов.Исходящий Тогда
			ТекстЗаголовка = ТекстЗаголовка + " " + НСтр("ru = '(исходящий)';
														|en = '(outbound)'", ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Если ВыборкаПоДокументам.Вид = Перечисления.ВидыУпаковочныхЛистов.Входящий Тогда
			...
			ТекстЗаголовка = НСтр("ru = 'Упаковочный лист (входящий)';
									|en = 'Packing list (incoming)'", ОбщегоНазначения.КодОсновногоЯзыка());
			ВыводитьСкладИУпаковщика = Ложь;
			
		ИначеЕсли ВыборкаПоДокументам.Вид = Перечисления.ВидыУпаковочныхЛистов.Исходящий Тогда
			
			ТекстЗаголовка = НСтр("ru = 'Упаковочный лист (исходящий)';
									|en = 'Packing list (outbound)'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура РекурсивноВывестиИерархиюУпаковочныхЛистов(ТабличныйДокумент, ОбластьСтрокаТаблицыУпаковочныеЛисты, УпаковочныеЛистыДокумента, УпаковочныйЛистРодитель, Уровень = 0) Экспорт
	...
	Для Каждого Строка Из НайденныеСтроки Цикл
		...
		ПредставлениеУпаковочногоЛиста = НСтр("ru = '%Отступ% %УпаковочныйЛист% от %Дата%';
												|en = '%Отступ% %УпаковочныйЛист% from %Дата%'", ОбщегоНазначения.КодОсновногоЯзыка());
