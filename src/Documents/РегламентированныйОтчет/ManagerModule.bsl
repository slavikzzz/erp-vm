...
Функция ПолучитьБлокируемыеРеквизитыОбъекта() Экспорт
	...
	БлокируемыеРеквизиты.Добавить("ФормаЗаполнитьДаннымиОтчетов;ФормаЗаполнитьДаннымиОтчетов");
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	НаименованиеОтчета = Данные.НаименованиеОтчета + " за " + Данные.ПредставлениеПериода
		+ " (Вид: " + ?(ЗначениеЗаполнено(Данные.ПредставлениеВида), Данные.ПредставлениеВида, "П")
		+ ". Организация: " + Данные.Организация + ")";
	
	Представление = СтрШаблон(НСтр("ru = '%1';
									|en = '%1'"), НаименованиеОтчета);
...
// Меняет названия регламентированных отчетов, сохраненных в предыдущих версиях библиотеки.
//
// Параметры:
...
Процедура ИзменитьНаименованиеРеглОтчета(СтароеНаименование, НовоеНаименование)
	...
	Запрос.УстановитьПараметр("НаименованиеОтчета", СтароеНаименование);
	Результат = Запрос.Выполнить().Выгрузить();
	
	Для Каждого СтрТаблЗнач Из Результат Цикл
		Док = СтрТаблЗнач.Ссылка.ПолучитьОбъект();
		Док.НаименованиеОтчета = НовоеНаименование;
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(Док);
	КонецЦикла;
