...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ИначеЕсли ЭтотОбъект.ИсточникОтчета = "РегламентированныйОтчетДвижениеСредствНерезидентаЗаПределамиРФ" Тогда
	...
	ЭтоВызовИзОбработчикаОбновления = Ложь;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбновлениеВерсииИБ") Тогда
		Если ОбновлениеИнформационнойБазы.ЭтоВызовИзОбработчикаОбновления() Тогда
			ЭтоВызовИзОбработчикаОбновления = Истина;
			Если НЕ ОбщегоНазначения.СсылкаСуществует(ЭтотОбъект.Организация) Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ДокументыВыгрузки.Количество() > 0 Тогда
		...
		Попытка
			...
			Для Каждого ДокВыгрузки Из ДокументыВыгрузки Цикл
				...
				Если ЭтоВызовИзОбработчикаОбновления
					И НЕ ОбщегоНазначения.СсылкаСуществует(ДокВыгрузки.Ссылка) Тогда
					Продолжить;
				КонецЕсли;
