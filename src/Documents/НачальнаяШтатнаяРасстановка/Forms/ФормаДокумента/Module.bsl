...
Процедура СотрудникиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Сотрудник) Тогда
		...
		ИначеЕсли Поле.Имя = "СотрудникиПредставлениеПраваНаОтпуск" Тогда
			...
			ЕжегодныеОтпуска = ЕжегодныеОтпускаСотрудника(ТекущиеДанные.Сотрудник);
			Если ЕжегодныеОтпуска.Количество() > 0 Тогда
				ПараметрыОткрытия.Вставить("ДатаОстатков", ЕжегодныеОтпуска[0].ДатаОстатка);
			Иначе	
			...
			ПараметрыОткрытия.Вставить("ЕжегодныеОтпуска", ЕжегодныеОтпуска);
...
Процедура ОбновитьПоШтатномуРасписанию(Команда)
	...
	Если Объект.Сотрудники.Количество() > 0 Тогда
		...
		ТекстВопроса = НСтр("ru = 'При обновлении данных по настройкам штатного расписания, внесенные в ручную
			...
			|en = 'When updating data by the headcount settings, manually edited changes
			|will be lost. Update by the headcount?'");
