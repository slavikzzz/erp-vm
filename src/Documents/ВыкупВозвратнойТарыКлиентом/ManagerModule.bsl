...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса)
	...
	// В управленческом учете сумма НДС в расходах не отражается, т.к. для этой суммы нет корреспонденции в упр. балансе.
	// В регл учете и в упр учете организаций сумма НДС отражается в расходах и корреспондирует с регистром "Движения по НДС". 
	|	0 КАК СуммаСНДС,
	|	0 КАК СуммаБезНДС,
	|	СУММА(ВЫРАЗИТЬ(ТаблицаТовары.СуммаНДС * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(31,2))) КАК СуммаБезНДСУпр,
	|	СУММА(ВЫРАЗИТЬ(ТаблицаТовары.СуммаНДС * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2))) КАК СуммаСНДСРегл,
	|	СУММА(ВЫРАЗИТЬ(ТаблицаТовары.СуммаНДС * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2))) КАК СуммаБезНДСРегл,
...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат ВыкупВозвратнойТарыКлиентомЛокализация.ТекстОтраженияВРеглУчете();
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	Возврат ВыкупВозвратнойТарыКлиентомЛокализация.ТекстЗапросаВТОтраженияВРеглУчете();
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Накладная") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "Накладная", НСтр("ru = 'Выкуп возвратной тары';
																										|en = 'Customer invoice — returnable packaging'"), СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати));
	КонецЕсли;
