...
Процедура ОснованиеПлатежаОбработкаВыбораСервер(ВыбранноеЗначение)
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту Тогда
		...
		Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
			...
			ЗаполнитьЗначенияСвойств(Объект, ВыбранноеЗначение, "Партнер, Контрагент, ОбъектРасчетов, Валюта, ВалютаКомиссии, СуммаНДС20, СуммаНДС10, СуммаНДС7, СуммаНДС5");
			ЗначенияРеквизитов = ЗначенияРеквизитовНастройки(Объект.НастройкаЯндексКассы);
			Объект.СтатьяДвиженияДенежныхСредств = ЗначенияРеквизитов.СтатьяДвиженияДенежныхСредствВозвраты;
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Элементы.СуммаНДС20.Заголовок = СтрШаблон(НСтр("ru = 'Сумма НДС (%1)';
													|en = 'VAT amount (%1)'"), Строка(УчетНДСУП.СтавкаНДСПоУмолчанию(Объект.Организация, Объект.Дата)));
...
Процедура УстановитьЗаголовокГруппыСуммНДС()
	...
	Если Объект.СуммаНДС20 <> 0 Тогда
		СписокНДС.Добавить(
			СтрокаСуммыНДС(Строка(УчетНДСУП.СтавкаНДСПоУмолчанию(Объект.Организация, Объект.Дата)),
			Объект.СуммаНДС20,
			Объект.Валюта));
	КонецЕсли;
	...
	Если СписокНДС.Количество() = 0 Тогда
		СписокНДС.Добавить(СтрШаблон(НСтр("ru = '0 %1';
											|en = '0 %1'"), Объект.Валюта));
	КонецЕсли;
	
	Элементы.ГруппаСуммыНДС.Заголовок =
		СтрШаблон(НСтр("ru = 'Сумма НДС: %1';
						|en = 'Сумма НДС: %1'"), СтрСоединить(СписокНДС, ","));
...
Функция СтрокаСуммыНДС(СтавкаНДС, СуммаНДС, Валюта)
	...
	Возврат СтрШаблон(НСтр("ru = '%1 %2 (%3)';
							|en = '%1 %2 (%3)'"), СуммаНДС, Валюта, СтавкаНДС);
