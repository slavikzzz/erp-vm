...
Функция ЕстьКодыМаркировкиДляБлокировки(Объект)
	...
	Если Объект.ВидПродукции <> Перечисления.ВидыПродукцииИС.ВетеринарныеПрепараты Тогда
...
Функция ПеремаркировкаJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	|	ЕСТЬNULL(Товары.НовыйКодМаркировки.ТипУпаковки, Неопределено) КАК ТипУпаковки,
	...
	Для Каждого СтрокаТЧТовары Из Товары Цикл
		...
		// Сертификация
		Если Шапка.ВидПродукции <> Перечисления.ВидыПродукцииИС.ВетеринарныеПрепараты
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧТовары.ТипУпаковки)
			Или СтрокаТЧТовары.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар Тогда
