...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Статусы.Количество() = 0 Тогда
		// Доступен выбор документов во всех статусах
		Статусы.Добавить(Перечисления.СтатусыСменныхЗаданий.Формируется);
		Статусы.Добавить(Перечисления.СтатусыСменныхЗаданий.Сформировано);
		Статусы.Добавить(Перечисления.СтатусыСменныхЗаданий.Закрыто);
	КонецЕсли;
...
Процедура ВидимыеСменыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.МожноВыбирать Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВидимыеСмены.СменноеЗадание");
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВидимыеСмены.МожноВыбирать");
...
Процедура ИнициализироватьСмены()
	...
	Начало = ДобавитьМесяц(ТекущаяДата, -1);
	Окончание = ДобавитьМесяц(ТекущаяДата, 1);
	КоличествоВыбранныхСмен = 0;
	...
	Если ВыбранныеРанееСмены.Количество() > 0 Тогда
		...
		Запрос = Новый Запрос(
			"ВЫБРАТЬ
			|	МИНИМУМ(НАЧАЛОПЕРИОДА(СменноеЗадание.Дата, ДЕНЬ)) КАК ДатаМин,
			|	МАКСИМУМ(КОНЕЦПЕРИОДА(СменноеЗадание.Дата, ДЕНЬ)) КАК ДатаМакс,
			|	КОЛИЧЕСТВО(СменноеЗадание.Ссылка)                 КАК Количество
			|ИЗ
			|	Документ.СменноеЗадание КАК СменноеЗадание
			|ГДЕ
			|	СменноеЗадание.Подразделение = &Подразделение
			|	И СменноеЗадание.Ссылка В(&ВыбранныеРанееСмены)
			|	И СменноеЗадание.Статус В(&Статусы)
			|	И СменноеЗадание.Проведен
			|
			|ИМЕЮЩИЕ
			|	КОЛИЧЕСТВО(СменноеЗадание.Ссылка) > 0");
		Запрос.УстановитьПараметр("Подразделение", Подразделение);
		Запрос.УстановитьПараметр("ВыбранныеРанееСмены", ВыбранныеРанееСмены.ВыгрузитьЗначения());
		Запрос.УстановитьПараметр("Статусы", Статусы.ВыгрузитьЗначения());
		Выборка = Запрос.Выполнить().Выбрать();
		
		Если Выборка.Следующий() Тогда
			
			Начало = Мин(Начало, Выборка.ДатаМин);
			Окончание = Макс(Окончание, Выборка.ДатаМакс);
			КоличествоВыбранныхСмен = Выборка.Количество;
			
		КонецЕсли;
		
	КонецЕсли;
	
	ПрочитатьСмены(Начало, Окончание, Ложь);
...
Функция ВыбратьСменыЗаПериод(Начало, Окончание)
	...
	Запрос = Новый Запрос(
		...
		|		И СменноеЗадание.Статус В (&Статусы)       КАК Выбрана,
		|	СменноеЗадание.Статус В (&Статусы)             КАК МожноВыбирать
		|ПОМЕСТИТЬ ВТЗадания
		|ИЗ
		|	Документ.СменноеЗадание КАК СменноеЗадание
		|ГДЕ
		...
		|	ЕСТЬNULL(ВТЗадания.МожноВыбирать, ИСТИНА)                      КАК МожноВыбирать
		|ИЗ
		|	ВТСмены КАК ВТСмены
...
Процедура ДобавитьСтрокуВВидимыеСмены(Форма, Владелец, ДобавлятьВНачало)
	...
	ЗаполнитьЗначенияСвойств(НоваяСтрока, Владелец, "СменноеЗадание, СтатусЗадания, Выбрана, МожноВыбирать");
...
Процедура НастроитьЭлементыФормы()
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.ГруппаУчасток.Видимость                       = Ложь;
		Элементы.ПерейтиВНачалоСписка.Видимость                = Ложь;
		Элементы.ЗаполнятьИсполнителейИзПрошлойСмены.Видимость = Ложь;
		
		Элементы.ГруппаСмены.Группировка     = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
		Элементы.ГруппаВверхВниз.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
		
		Элементы.Вверх.РастягиватьПоГоризонтали = Истина;
		Элементы.Вниз.РастягиватьПоГоризонтали  = Истина;
		
	КонецЕсли;
	
	Элементы.Выбрана.Видимость                 = МножественныйВыбор;
	Элементы.ОтменитьВыбор.Видимость           = МножественныйВыбор;
	Элементы.ПодсказкаВыбраноЗаданий.Видимость = МножественныйВыбор;
