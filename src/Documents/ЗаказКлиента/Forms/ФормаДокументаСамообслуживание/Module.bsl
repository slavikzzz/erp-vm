...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Заказ %1 от %2';
																			|en = 'Order %1 from %2'"), Объект.Номер, Объект.Дата);
...
Процедура ОтменитьВыделенныеСтроки(Команда)
	...
	Если Не ПродажиКлиент.НеобходимоЗаполнениеПричиныОтменыВыделенныхСтрок(Объект.Товары, НСтр("ru = 'Товары';
																								|en = 'Goods'"), Элементы.Товары.ВыделенныеСтроки) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьПодбор(Команда)
	...
	ПараметрыФормы.Вставить("Заголовок",                                     НСтр("ru = 'Подбор товаров в';
																					|en = 'Pick goods in'") + " " + Объект.Ссылка);
...
Процедура СоглашениеПриИзмененииСервер(РезультатВыбора)
	...
	СтруктураДействийСоСкидками.Вставить("ЗаполнитьСтавкуНДС", ОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект, Истина));
...
Процедура УправлениеЭлементамиЗаписи()
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаПровести", "Заголовок",
	                                                               ?(Объект.Проведен, НСтр("ru = 'Записать';
																							|en = 'Save'"), НСтр("ru = 'На согласование';
																													|en = 'For approval'")));
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаПровестиИЗакрыть", "Заголовок",
	                                                               ?(Объект.Проведен, НСтр("ru = 'Записать и закрыть';
																							|en = 'Save and close'"), НСтр("ru = 'На согласование и закрыть';
																																|en = 'For approval and close'")));
