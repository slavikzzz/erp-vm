...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзмененыРеквизитыИсполнительногоДокумента" И Источник = ЭтотОбъект Тогда
		
		ПараметрыРеквизитовИсполнительногоДокумента = Новый Структура("ДатаВыдачи, НомерИсполнительногоДокумента, ОрганВыдавшийИсполнительныйДокумент");
		ЗаполнитьЗначенияСвойств(ПараметрыРеквизитовИсполнительногоДокумента, Параметр);
		
		ЗаполнитьРеквизитыИсполнительногоДокумента(ПараметрыРеквизитовИсполнительногоДокумента);
	...
	Если ИспользоватьПлатежногоАгента Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ПлатежныйАгент) Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Поле «Платежный агент» не заполнено.';
					|en = 'The ""Paying agent"" field is not filled in.'"), , "Объект.ПлатежныйАгент", , Отказ);
		КонецЕсли;
			
		Если Не ЗначениеЗаполнено(Объект.ТарифПлатежногоАгента) Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Поле «Тариф платежного агента» не заполнено.';
					|en = 'The ""Paying agent tariff"" field is not filled in.'"), , "Объект.ТарифПлатежногоАгента", , Отказ);
		КонецЕсли;
...
&НаКлиенте
Процедура РеквизитыИсполнительногоДокументаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ДатаВыдачи",							Объект.ДатаВыдачи);
	ПараметрыФормы.Вставить("НомерИсполнительногоДокумента",		Объект.НомерИсполнительногоДокумента);
	ПараметрыФормы.Вставить("ОрганВыдавшийИсполнительныйДокумент",	Объект.ОрганВыдавшийИсполнительныйДокумент);
	
	ОткрытьФорму("Документ.ИсполнительныйЛист.Форма.ВводРеквизитов", ПараметрыФормы, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	...
	Иначе
		...
		Если ПустаяСтрока(Форма.Объект.РеквизитыИсполнительногоДокумента) Тогда
			ЗаголовокГруппы = ЗаголовокГруппы + "; " + НСтр("ru = 'Не заданы реквизиты документа (номер, кем и когда выдан)';
															|en = 'Document details are not specified (number, issued by whom and when)'");
		Иначе
...
&НаКлиенте
Процедура ЗаполнитьРеквизитыИсполнительногоДокумента(ПараметрыРеквизитов)
	
	Объект.ДатаВыдачи = ПараметрыРеквизитов.ДатаВыдачи;
	Объект.НомерИсполнительногоДокумента = ПараметрыРеквизитов.НомерИсполнительногоДокумента;
	Объект.ОрганВыдавшийИсполнительныйДокумент = ПараметрыРеквизитов.ОрганВыдавшийИсполнительныйДокумент;
	
	Объект.РеквизитыИсполнительногоДокумента = "" + ПараметрыРеквизитов.НомерИсполнительногоДокумента
											+" выдан " + Формат(ПараметрыРеквизитов.ДатаВыдачи, "ДЛФ=DD")
											+" " + ПараметрыРеквизитов.ОрганВыдавшийИсполнительныйДокумент;
