...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	КонецЕсли;
	ПроверитьЗаполнениеНоменклатурыТабличнойЧастиТовары(МассивНепроверяемыхРеквизитов, Отказ);
...
Процедура ПроверитьЗаполнениеТабличнойЧастиТовары(Отказ)
	...
	ШаблонСообщения = НСтр("ru = 'Не заполнены производители для продукции ""%1"" в строке %2 списка ""%3""';
							|en = 'Не заполнены производители для продукции ""%1"" в строке %2 списка ""%3""'");
	...
	Если ИнтеграцияВЕТИСВызовСервера.ИспользуетсяОграничениеВыбораПродукцииТретьегоУровня(Дата) Тогда
		...
		Для Каждого НомерСтроки Из НомераСтрокСПродукциейТретьегоУровня Цикл
			...
			Если (ТекущаяСтрока.Операция = Перечисления.ОперацииИнвентаризацииПродукцииВЕТИС.Добавление
				...
				|en = 'В строке №%1 требуется указание четвертого уровня продукции в соответствии с письмом РСХН
				|""О переходе на четвертый уровень справочника подконтрольных товаров с 01 февраля 2019""'");
	...
	ШаблонСообщения = НСтр("ru = 'В строке %1 списка ""%2"" обнаружен дубль записи складского журнала из строки %3';
							|en = 'В строке %1 списка ""%2"" обнаружен дубль записи складского журнала из строки %3'");
...
Процедура ПроверитьЗаполнениеНоменклатурыТабличнойЧастиТовары(МассивНепроверяемыхРеквизитов, Отказ)
	...
	// производители при добавлении
	ШаблонСообщения = НСтр("ru = 'Не заполнена номенклатура в строке %1 списка ""%2""';
							|en = 'Не заполнена номенклатура в строке %1 списка ""%2""'");
	...
	Для каждого ТекущаяСтрока Из Товары Цикл
		...
		Если ТекущаяСтрока.Операция <> Перечисления.ОперацииИнвентаризацииПродукцииВЕТИС.Удаление
			...
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(ИмяТЧ, ТекущаяСтрока.НомерСтроки, "Номенклатура");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ТекущаяСтрока.НомерСтроки, ПредставлениеТЧ);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Поле,, Отказ);
