...
Функция ТекстЗапросаОснованияКОформлениюЗаявленийОВвозеТоваров()
	...
	Для Каждого ИмяТипа Из МассивТипов Цикл
		...
		//++ НЕ УТ
		ИначеЕсли ИмяТипа = "ОтчетПереработчика2_5"
			Или ИмяТипа = "ЗаключениеДоговораАренды" Тогда 
			ТекстЗапросаДляТипа = СтрЗаменить(ТекстЗапросаДляТипа, "&ПолеСклад", "НЕОПРЕДЕЛЕНО");
		//-- НЕ УТ
		КонецЕсли;
...
Процедура ОбработкаВыбораЗначения()
	...
	Для каждого ИндексСтроки Из ТаблицаДокументов.ВыделенныеСтроки Цикл
		...
		Если СтрокаТаблицыВыбора.ДокументУжеВыбран Тогда 
			ТекстПредупреждения = СтрЗаменить(НСтр("ru = 'Документ %Документ% уже выбран';
													|en = 'Document %Документ% is already selected'"), "%Документ%", Строка(СтрокаТаблицыВыбора.Ссылка));
