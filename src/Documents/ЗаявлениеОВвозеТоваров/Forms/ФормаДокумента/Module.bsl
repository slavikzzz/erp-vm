...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СтруктураРеквизитовВыгрузки = Неопределено Тогда
		...
		Если СтруктураРеквизитовВыгрузки = Неопределено Тогда
			СтруктураРеквизитовЗаявления = РеквизитыЗаявления();
			СтруктураРеквизитовВыгрузки = ЗаявлениеОВвозеЛокализация.ОбновитьДанныеРеквизитовВыгрузки(СтруктураРеквизитовЗаявления);
		КонецЕсли;
	КонецЕсли;
	
	ЗаявлениеОВвозеЛокализация.ОбновитьСтруктуруРеквизитовВыгрузки(СтруктураРеквизитовВыгрузки);
	ЗаявлениеОВвозеЛокализация.ЗаполнитьОтчетностьВФТС(ЭтотОбъект);
	// -- НЕ УТ
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	// ++ НЕ УТ
	ЗаявлениеОВвозеКлиентЛокализация.ОбработкаОповещения(ЭтотОбъект, ИмяСобытия, Параметр, Источник);
	// -- НЕ УТ
...
Процедура ДоговорОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.ДоговорыКонтрагентов") 
		//++ НЕ УТ
		ИЛИ ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.ДоговорыАренды")
		//-- НЕ УТ
		Тогда
...
Процедура ДатаПриИзменении(Элемент)
	...
	Если НеобходимоПерезаполнитьТоварыПриИзмененииДаты() Тогда 
		...
		ВариантыОтветов.Добавить(Истина, НСтр("ru = 'Перезаполнить';
												|en = 'Refill'", КодОсновногоЯзыка));
		ВариантыОтветов.Добавить(Ложь, НСтр("ru = 'Не перезаполнять';
											|en = 'Do not refill'", КодОсновногоЯзыка));
	
		Оповещение = Новый ОписаниеОповещения("ДатаПриИзмененииЗавершение", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Перезаполнить товары?';
										|en = 'Do you want to refill the goods?'", КодОсновногоЯзыка), ВариантыОтветов);
		Возврат;
...
Процедура ДобавитьСтроки(Команда)
	...
	Если СтруктураДокументыПартий.ДобавлятьСтрокиВручную Тогда 
		ПоказатьВводЧисла(Новый ОписаниеОповещения("ДобавитьСтрокуДляУслугПереработки", ЭтотОбъект, Новый Структура("ТекущийДокумент, НомерТекущейСтроки", ?(ТекущаяСтрока = Неопределено, Неопределено, ТекущаяСтрока.ДокументПоступления), ?(ТекущаяСтрока = Неопределено, Неопределено, ТекущаяСтрока.НомерСтроки))), 0, НСтр("ru = 'Укажите стоимость';
																																																																																|en = 'Specify the cost'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()));
	Иначе
...
Процедура ВыгрузитьЗаявлениеОВвозе(Команда)
	...
	// ++ НЕ УТ
	ЗаявлениеОВвозеКлиентЛокализация.ВыгрузитьЗаявлениеОВвозе(ЭтотОбъект);
	// -- НЕ УТ
...
Процедура СоздатьСтатформуВФТСНажатие(Элемент)
	...
	ЗаявлениеОВвозеКлиентЛокализация.СоздатьСтатформуВФТС(ЭтотОбъект);
...
Процедура ОбработкаВыбораДобавитьСтрокиИзДокументаПартии(МассивДокументов)
	...
	Для каждого ЭлементМассива Из МассивДокументов Цикл
		...
		//++ НЕ УТ
		Если ТипЗнч(ЭлементМассива.Ссылка) = Тип("ДокументСсылка.ЗаключениеДоговораАренды") Тогда
			ДатаОстатков = ?(ЗначениеЗаполнено(Объект.Дата), КонецДня(Объект.Дата), КонецДня(ТекущаяДатаСеанса()));
		КонецЕсли;
		//-- НЕ УТ
...
Процедура ПроверитьВыгрузку(Команда)
	...
	// ++ НЕ УТ
	ЗаявлениеОВвозеКлиентЛокализация.ПроверитьВыгрузку(ЭтотОбъект);
	// -- НЕ УТ
...
Функция РеквизитыЗаявления()
	...
	СтруктураРеквизитов.Вставить("СтруктураРеквизитовВыгрузки", СтруктураРеквизитовВыгрузки);
...
Процедура ОбновитьОтправку(Команда)
	...
	// ++ НЕ УТ
	ЗаявлениеОВвозеКлиентЛокализация.ОбновитьОтправку(ЭтаФорма);
	// -- НЕ УТ
...
Процедура ГиперссылкаПротоколНажатие(Элемент)
	...
	// ++ НЕ УТ
	ЗаявлениеОВвозеКлиентЛокализация.ГиперссылкаПротоколНажатие(ЭтаФорма);
	// -- НЕ УТ
...
Процедура ОтправитьНеотправленноеИзвещение(Команда)
	...
	// ++ НЕ УТ
	ЗаявлениеОВвозеКлиентЛокализация.ОтправитьНеотправленноеИзвещение(ЭтаФорма);
	// -- НЕ УТ
...
Процедура ЭтапыОтправкиНажатие(Элемент)
	...
	// ++ НЕ УТ
	ЗаявлениеОВвозеКлиентЛокализация.ЭтапыОтправкиНажатие(ЭтаФорма);
	// -- НЕ УТ
...
Процедура КритическиеОшибкиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	// ++ НЕ УТ
	ЗаявлениеОВвозеКлиентЛокализация.КритическиеОшибкиОбработкаНавигационнойСсылки(ЭтаФорма, СтандартнаяОбработка);
	// -- НЕ УТ
...
Функция НеобходимоПерезаполнитьТоварыПриИзмененииДаты()
	...
	Для Каждого СтрокаТовары Из Объект.Товары Цикл 
		Если ТипЗнч(СтрокаТовары.ДокументПоступления) = Тип("ДокументСсылка.ЗаключениеДоговораАренды") Тогда
