...
Функция ТаблицаОстатковТоваровКОформлениюЗаявленийОВвозеТоваров(ДокументПоступления, ЗаполнениеНаОсновании = Ложь, ПериодОформления = Неопределено) Экспорт
	...
	//++ НЕ УТ
	Если ТипЗнч(ДокументПоступления) = Тип("СправочникСсылка.ДоговорыАренды") Тогда
		Запрос.УстановитьПараметр("ДокументПоступления", Справочники.ДоговорыАренды.ДокументЗаключенияДоговораАренды(ДокументПоступления));
		Запрос.УстановитьПараметр("ДоговорАренды", ДокументПоступления);
	ИначеЕсли ТипЗнч(ДокументПоступления) = Тип("ДокументСсылка.ЗаключениеДоговораАренды") Тогда
		Запрос.УстановитьПараметр("ДокументПоступления", ДокументПоступления);
		Запрос.УстановитьПараметр("ДоговорАренды", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументПоступления, "Договор"));
	КонецЕсли;
	//-- НЕ УТ
	...
	//++ НЕ УТ
	ЭтоЛизинг = ТипЗнч(ДокументПоступления) = Тип("СправочникСсылка.ДоговорыАренды")
					Или ТипЗнч(ДокументПоступления) = Тип("ДокументСсылка.ЗаключениеДоговораАренды");
	//-- НЕ УТ
	...
	//++ НЕ УТ
	Если ЭтоЛизинг Тогда 
		ТаблицаТоваров = РаспределитьСуммыДляУслугАренды(Запрос);
	КонецЕсли;
	//-- НЕ УТ
...
Функция ТипыДокументовОснованийКОформлению() Экспорт
	...
	//++ НЕ УТ
	МассивТипов.Добавить("ОтчетПереработчика2_5");
	МассивТипов.Добавить("ЗаключениеДоговораАренды");
	//-- НЕ УТ
...
Функция ВыгрузитьЗаявлениеОВвозеТоваров(УникальныйИдентификатор, Объект, ТолькоПроверка = Ложь) Экспорт
	...
	Возврат ЗаявлениеОВвозеЛокализация.ВыгрузитьЗаявлениеОВвозеТоваров(УникальныйИдентификатор, Объект, ТолькоПроверка);
...
Функция ВыгрузитьУчетПеремещенияТоваров(УникальныйИдентификатор, Объект, ТолькоПроверка = Ложь) Экспорт
	...
	Возврат ЗаявлениеОВвозеЛокализация.ВыгрузитьУчетПеремещенияТоваров(УникальныйИдентификатор, Объект, ТолькоПроверка);
...
Функция ТекстЗапросаТаблицаТоварыКОформлениюДокументовИмпорта(Запрос, ТекстыЗапроса, Регистры)
	...
	|	0,
	|	0,
	|	СУММА(ТаблицаТовары.ФактурнаяСтоимость),
	|	NULL,
	|	ЛОЖЬ,
	|	МАКСИМУМ(ТаблицаТовары.ИдентификаторСтроки)
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров.Товары КАК ТаблицаТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаключениеДоговораАренды КАК ДокументЗаключенияДоговораАренды
	|		ПО ТаблицаТовары.ДокументПоступления = ДокументЗаключенияДоговораАренды.Ссылка
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаВтТаблицаАналитикУчетаПартий(Запрос, ТекстыЗапроса)
	...
	|	ДанныеДокументаПоступления.Партнер					КАК Поставщик,
	|	ДанныеДокументаПоступления.Контрагент				КАК Контрагент,
	|	МИНИМУМ(ЕСТЬNULL(СтавкиНДСНоменклатуры.СтавкаНДС, ЕСТЬNULL(ОсновныеСтавкиНДС.СтавкаНДС,
	...
	|		ПО ДанныеДокумента.Ссылка = ТаблицаДокумента.Ссылка
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика2_5.ВыходныеИзделия КАК ТаблицаДокументаПоступления
	|		ПО ТаблицаДокумента.ДокументПоступления = ТаблицаДокументаПоступления.Ссылка
	|		И ТаблицаДокумента.Номенклатура = ТаблицаДокументаПоступления.Номенклатура
	|		И ТаблицаДокумента.Характеристика = ТаблицаДокументаПоступления.Характеристика
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика2_5 КАК ДанныеДокументаПоступления
	|		ПО ТаблицаДокумента.ДокументПоступления = ДанныеДокументаПоступления.Ссылка
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СпрНоменклатура
	|		ПО СпрНоменклатура.Ссылка = ТаблицаДокумента.Номенклатура
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ГруппыФинансовогоУчетаНоменклатуры КАК ГФУ
	...
	|	ПО
	|		ТаблицаДокумента.Номенклатура = СтавкиНДСНоменклатуры.Номенклатура
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЕСТЬNULL(СтавкиНДСНоменклатуры.СтавкаНДС, ЕСТЬNULL(ОсновныеСтавкиНДС.СтавкаНДС,
	|		ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)))) = 1
	//-- НЕ УТ
	|";
...
Функция ТекстЗапросаТоварыКОформлениюЗаявленийОВвозе(ЗаполнениеНаОсновании = Ложь)
	...
	|
	|ВЫБРАТЬ
	|	ОтчетПереработчика2_5.Ссылка,
	|	ОтчетПереработчика2_5.Контрагент,
	|	ОтчетПереработчика2_5.Договор,
	|	ОтчетПереработчика2_5.Валюта,
	|	ОтчетПереработчика2_5.ЗакупкаПодДеятельность,
	|	ОтчетПереработчика2_5.НомерВходящегоДокумента,
	|	ОтчетПереработчика2_5.ДатаВходящегоДокумента
	|ИЗ
	|	Документ.ОтчетПереработчика2_5 КАК ОтчетПереработчика2_5
	|ГДЕ
	|	ОтчетПереработчика2_5.Ссылка = &ДокументПоступления
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ЗаключениеДоговораАренды.Договор,
	|	ЗаключениеДоговораАренды.Валюта,
	|	ЗаключениеДоговораАренды.ЗакупкаПодДеятельность,
	|	ЗаключениеДоговораАренды.НомерВходящегоДокумента,
	|	ЗаключениеДоговораАренды.ДатаВходящегоДокумента
	|ИЗ
	|	Документ.ЗаключениеДоговораАренды КАК ЗаключениеДоговораАренды
	|ГДЕ
	|	ЗаключениеДоговораАренды.Ссылка = &ДокументПоступления
	//-- НЕ УТ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|ВЫБРАТЬ
	|	ОтчетПереработчика2_5.Ссылка,
	|	ОтчетПереработчикаВидыЗапасов.НомерСтроки,
	|	ОтчетПереработчикаВидыЗапасов.АналитикаУчетаНоменклатуры.Номенклатура,
	|	NULL,
	|	ОтчетПереработчикаВидыЗапасов.АналитикаУчетаНоменклатуры.Характеристика,
	|	ОтчетПереработчикаВидыЗапасов.АналитикаУчетаНоменклатуры.Серия,
	|	ВЫБОР
	|		КОГДА
	|			ОтчетПереработчикаВидыЗапасов.АналитикаУчетаНоменклатуры.ТипМестаХранения = ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.Склад)
	|			ТОГДА ОтчетПереработчикаВидыЗапасов.АналитикаУчетаНоменклатуры.МестоХранения
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|	КОНЕЦ,
	|	ЕСТЬNULL(ОтчетПереработчикаВидыЗапасов.АналитикаУчетаНоменклатуры.Назначение.ВидДеятельностиНДС,
	|		ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)),
	|	ОтчетПереработчика2_5.ЗакупкаПодДеятельность,
	|	ОтчетПереработчикаВидыЗапасов.НомерГТД.СтранаПроисхождения,
	|	ОтчетПереработчикаВидыЗапасов.НомерГТД,
	|	ОтчетПереработчикаВидыЗапасов.АналитикаУчетаНоменклатуры,
	|	ОтчетПереработчикаВидыЗапасов.ВидЗапасов,
	|	"""",
	|	""""
	|ИЗ
	|	Документ.ОтчетПереработчика2_5.ВидыЗапасовИзделия КАК ОтчетПереработчикаВидыЗапасов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика2_5 КАК ОтчетПереработчика2_5
	|		ПО ОтчетПереработчикаВидыЗапасов.Ссылка = ОтчетПереработчика2_5.Ссылка
	|ГДЕ
	|	ОтчетПереработчика2_5.Ссылка = &ДокументПоступления
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ЗаключениеДоговораАренды.Ссылка,
	|	ЗаключениеДоговораАрендыОС.НомерСтроки,
	|	NULL,
	|	NULL,
	|	NULL,
	|	NULL,
	...
	|	ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка),
	|	ЗаключениеДоговораАренды.ЗакупкаПодДеятельность,
	|	ЗаключениеДоговораАренды.Контрагент.СтранаРегистрации,
	|	НЕОПРЕДЕЛЕНО,
	|	NULL,
	|	NULL,
	|	"""",
	|	ПРЕДСТАВЛЕНИЕ(ЗаключениеДоговораАрендыОС.ОсновноеСредство)
	|ИЗ
	|	Документ.ЗаключениеДоговораАренды.ОС КАК ЗаключениеДоговораАрендыОС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаключениеДоговораАренды КАК ЗаключениеДоговораАренды
	|		ПО ЗаключениеДоговораАрендыОС.Ссылка = ЗаключениеДоговораАренды.Ссылка
	|ГДЕ
	|	ЗаключениеДоговораАренды.Ссылка = &ДокументПоступления
	|	И ЗаключениеДоговораАренды.Проведен
	//-- НЕ УТ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	РегистрНакопления.ТоварыКОформлениюДокументовИмпорта КАК ТоварыКОформлению
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаключениеДоговораАренды КАК ЗаключениеДоговораАренды
	|		ПО ТоварыКОформлению.ДокументПоступления = ЗаключениеДоговораАренды.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОстаткиКОформлениюНоваяМеханика КАК ОстаткиКОформлениюНоваяМеханика
	|		ПО ТоварыКОформлению.АналитикаУчетаНоменклатуры = ОстаткиКОформлениюНоваяМеханика.АналитикаУчетаНоменклатуры
	|			И ТоварыКОформлению.ВидЗапасов = ОстаткиКОформлениюНоваяМеханика.ВидЗапасов
	|			И ТоварыКОформлению.ДокументПоступления = ОстаткиКОформлениюНоваяМеханика.ДокументПоступления
	...
	|ГДЕ
	|	ТоварыКОформлению.Активность
	|	И ТоварыКОформлению.Период = &ПериодОформления
	|	И ТоварыКОформлению.ДокументПоступления = &ДокументПоступления
	|	И ТоварыКОформлению.ТипДокументаИмпорта = &ТипЗаявлениеОВвозе
	|	И ТИПЗНАЧЕНИЯ(ТоварыКОформлению.Регистратор) <> ТИП(Документ.ЗаявлениеОВвозеТоваров)
	|	И ДокументыСторно.Ссылка ЕСТЬ NULL
...
Функция РаспределитьСуммыДляУслугАренды(ЗапросАренды)
	
	ЗапросАренды.Текст = "ВЫБРАТЬ
		|	УсловияДоговоровАрендыСрезПоследних.АктуальныеУсловияДоговора КАК АктуальныеУсловияДоговора
		|ПОМЕСТИТЬ ВтДокументУстановкиУсловийДоговораАренды
		|ИЗ
		|	РегистрСведений.УсловияДоговоровАренды.СрезПоследних(&ПериодОформления, Договор = &ДоговорАренды) КАК УсловияДоговоровАрендыСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЗаключениеДоговораАрендыОС.ОсновноеСредство КАК ОсновноеСредство,
		|	ЗаключениеДоговораАрендыОС.Стоимость КАК СтоимостьОС,
		|	ОперацияЗаключенияДоговораАренды.СтоимостьПредметовАренды КАК СтоимостьПредметовАренды
		|ПОМЕСТИТЬ ВтСтоимостиОС
		|ИЗ
		|	Документ.ЗаключениеДоговораАренды.ОС КАК ЗаключениеДоговораАрендыОС
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаключениеДоговораАренды КАК ОперацияЗаключенияДоговораАренды
		|		ПО ЗаключениеДоговораАрендыОС.Ссылка = ОперацияЗаключенияДоговораАренды.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДокументУстановкиУсловийДоговораАренды КАК ВтДокументУстановкиУсловийДоговораАренды
		|		ПО ЗаключениеДоговораАрендыОС.Ссылка = ВтДокументУстановкиУсловийДоговораАренды.АктуальныеУсловияДоговора
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ИзменениеУсловийДоговораАрендыОС.ОсновноеСредство,
		|	ИзменениеУсловийДоговораАрендыОС.Стоимость,
		|	ОперацияИзмененияДоговораАренды.СтоимостьПредметовАренды
		|ИЗ
		|	Документ.ИзменениеУсловийДоговораАренды.ОС КАК ИзменениеУсловийДоговораАрендыОС
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ИзменениеУсловийДоговораАренды КАК ОперацияИзмененияДоговораАренды
		|		ПО ИзменениеУсловийДоговораАрендыОС.Ссылка = ОперацияИзмененияДоговораАренды.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДокументУстановкиУсловийДоговораАренды КАК ВтДокументУстановкиУсловийДоговораАренды
		|		ПО ИзменениеУсловийДоговораАрендыОС.Ссылка = ВтДокументУстановкиУсловийДоговораАренды.АктуальныеУсловияДоговора
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТоварыЗаявления.ДокументПоступления КАК ДокументПоступления,
		|	ТоварыЗаявления.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|	ТоварыЗаявления.Номенклатура КАК Номенклатура,
		|	ТоварыЗаявления.ВидЗапасов КАК ВидЗапасов,
		|	ТоварыЗаявления.КодТНВЭД КАК КодТНВЭД,
		|	ТоварыЗаявления.Характеристика КАК Характеристика,
		|	ТоварыЗаявления.Серия КАК Серия,
		|	ТоварыЗаявления.Склад КАК Склад,
		|	ТоварыЗаявления.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		|	1 КАК Количество,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ВтСтоимостиОС.СтоимостьПредметовАренды, 0) = 0
		|			ТОГДА 0
		|		ИНАЧЕ ТоварыЗаявления.ФактурнаяСтоимость * ВтСтоимостиОС.СтоимостьОС / ВтСтоимостиОС.СтоимостьПредметовАренды
		|	КОНЕЦ КАК ФактурнаяСтоимость,
		|	0 КАК НалоговаяБазаНДС,
		|	ТоварыЗаявления.СтавкаНДС КАК СтавкаНДС,
		|	0 КАК СуммаНДС,
		|	0 КАК СтатСтоимостьДолларыСША,
		|	ТоварыЗаявления.ВесНеттоКг КАК ВесНеттоКг,
		|	1 КАК КоличествоПоТНВЭД,
		|	0 КАК НалоговаяБазаАкцизы,
		|	ТоварыЗаявления.СтранаПроисхождения КАК СтранаПроисхождения,
		|	ТоварыЗаявления.НомерТД КАК НомерТД,
		|	ТоварыЗаявления.Валюта КАК Валюта,
		|	ТоварыЗаявления.СерияНомерТСД КАК СерияНомерТСД,
		|	ТоварыЗаявления.ДатаТСД КАК ДатаТСД,
		|	ТоварыЗаявления.Договор КАК Договор,
		|	ТоварыЗаявления.ЗакупкаПодДеятельность КАК ЗакупкаПодДеятельность,
		|	ТоварыЗаявления.НоменклатураПартнера КАК НоменклатураПартнера,
		|	ТоварыЗаявления.ИдентификаторСтроки КАК ИдентификаторСтроки,
		|	ТоварыЗаявления.ДатаПринятияНаУчет КАК ДатаПринятияНаУчет,
		|	ТоварыЗаявления.ВидТранспорта КАК ВидТранспорта,
		|	ЕСТЬNULL(ПРЕДСТАВЛЕНИЕ(ВтСтоимостиОС.ОсновноеСредство), """") КАК Содержание
		|ИЗ
		|	ВтТоварыЗаявления КАК ТоварыЗаявления
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтСтоимостиОС КАК ВтСтоимостиОС
		|		ПО (ИСТИНА)";
	
	Возврат ЗапросАренды.Выполнить().Выгрузить();
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	//++ НЕ УТ
	#Область ВнешнееПоступление_ПродукцияПереработчика
	...
	|	ТаблицаДокумента.Ссылка 				КАК Ссылка,
	|	ТаблицаВидыЗапасов.ДокументПоступления	КАК ДокументИсточник,
	|	ОтчетПереработчика2_5.Дата				КАК ПериодДокументаИсточника,
	|	НЕОПРЕДЕЛЕНО 							КАК КорректируемыйДокумент,
	|
	// Поля учета номенклатуры
	|	ТаблицаДокумента.Организация 										  КАК Организация,
	...
	|
	// Поля аналитики финансового учета
	|	ОтчетПереработчика2_5.Сделка			КАК Сделка,
	|	ОтчетПереработчика2_5.Подразделение 	КАК Подразделение,
	|	ОтчетПереработчика2_5.Менеджер 			КАК Менеджер,
	|
	// Количественные и суммовые показатели
	|	0 										КАК Количество,
	|	НЕОПРЕДЕЛЕНО 							КАК ИдентификаторСтроки,
	...
	|
	// Прочие поля
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС) ХозяйственнаяОперация,
	|	ТаблицаВидыЗапасов.ИдентификаторСтроки   						 КАК ИдентификаторФинЗаписи,
	|	&НастройкаХозяйственнойОперации_ВключениеНДСВСтоимость			 КАК НастройкаХозяйственнойОперации
	|
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров.Товары КАК ТаблицаВидыЗапасов
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика2_5.ВидыЗапасовИзделия КАК ТоварыПоступления
	|		ПО ТоварыПоступления.Ссылка = ТаблицаВидыЗапасов.ДокументПоступления
	|		И ТоварыПоступления.АналитикаУчетаНоменклатуры = ТаблицаВидыЗапасов.АналитикаУчетаНоменклатуры
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявлениеОВвозеТоваров КАК ТаблицаДокумента
	|		ПО ТаблицаВидыЗапасов.Ссылка = ТаблицаДокумента.Ссылка
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика2_5 КАК ОтчетПереработчика2_5
	|		ПО ТаблицаВидыЗапасов.ДокументПоступления = ОтчетПереработчика2_5.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтТаблицаАналитикУчетаПартий КАК ТаблицаАналитикУчетаПартийПоступления
	|		ПО ТаблицаАналитикУчетаПартийПоступления.НомерСтроки 	   = ТаблицаВидыЗапасов.НомерСтроки
	|		 И ТаблицаАналитикУчетаПартийПоступления.ИмяТабличнойЧасти = ""ТоварыПоступления""
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтТаблицаАналитикУчетаПартий КАК ТаблицаАналитикУчетаПартий
