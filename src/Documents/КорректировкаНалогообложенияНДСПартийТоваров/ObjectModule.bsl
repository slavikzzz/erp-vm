	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'За %1 квартал по организации ""%2"" уже проведена корректировка налогообложения НДС. Проведение еще одной корректировки невозможно.';
				|en = 'VAT taxation is already adjusted for the %1 quarter by the ""%2"" company. Cannot adjust again.'"),
			Формат(Дата, "ДФ=к"),
			Организация);
	...
	Если Не КонецМесяца(Дата) = КонецКвартала(Дата) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Документ может быть проведен только в последнем месяце отчетного периода по НДС (квартала), укажите дату документа в промежутке с %1 по %2';
				|en = 'Document can be posted only in the last month of the VAT accounting period (quarter), specify the document date between %1 and %2'"),
			Формат(ДатаС,  "ДЛФ=DD"),
...
Процедура ЗаполнитьВидЗапасовПолучателя()
	...
	|		И ЕСТЬNULL(ВидыЗапасов.Договор.ТипДоговора, ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СДавальцем2_5)
	...
	|	КОГДА ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца) ТОГДА
