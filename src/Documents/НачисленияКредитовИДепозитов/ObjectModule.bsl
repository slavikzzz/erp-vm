	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ДатаНеВПериоде Тогда
			Текст = СтрШаблон(НСтр("ru = 'Дата начисления %1 в строке %2 не входит в заданный период начисления.';
									|en = 'Accrual date %1 in line %2 does not belong to the specified accrual period.'"),
		...
		ИначеЕсли Выборка.НеСовпадаютОрганизации Тогда
			Текст = СтрШаблон(НСтр("ru = 'Организация договора %1 в строке %2 не совпадает с организацией %3, выбранной в документе.';
									|en = 'Contract company %1 in line %2 does not match the %3 company selected in the document.'"),
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	НастройкаСчетовУчетаСервер.ПередЗаписью(ЭтотОбъект,
		Документы.НачисленияКредитовИДепозитов.ПараметрыНастройкиСчетовУчета(ХозяйственнаяОперация));
	//-- НЕ УТ
