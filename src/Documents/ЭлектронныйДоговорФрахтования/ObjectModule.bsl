...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЗначениеЗаполнено(ВидДокумента) = Ложь Тогда
		ВидДокумента = ЭлектронныеДокументыЭДО.ВидДокументаПоТипу(Перечисления.ТипыДокументовЭДО.ЭДФ);
	КонецЕсли;
	...
	Если РольУчастника = 3 Тогда
		Если ТипЗнч(СсылкаТитулФрахтователяСоставитель) = Тип("СправочникСсылка.Организации") Тогда
			Организация = СсылкаТитулФрахтователяСоставитель;
		Иначе
			Организация = СсылкаТитулФрахтователяФрахтователь;
		КонецЕсли;
	ИначеЕсли РольУчастника = 1 Тогда
		Если ТипЗнч(СсылкаТитулФрахтовщикаСоставитель) = Тип("СправочникСсылка.Организации") Тогда
			Организация = СсылкаТитулФрахтовщикаСоставитель;
		Иначе
...
Процедура ПриЗаписи(Отказ)
	...
	СтруктураДляРеестра.НомерЭПД = Номер;
	СтруктураДляРеестра.ДатаЭПД = Дата;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ОбменСГИСЭПДПереопределяемый.ОбработкаЗаполненияЭДФ(ЭтотОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Если ОбъектКопирования.ЭтоВходящий = Истина Тогда
		ВызватьИсключение НСтр("ru = 'Нельзя копировать входящий документ';
								|en = 'You cannot copy an incoming document'");
	КонецЕсли;
	
	ЭтотОбъект.УИДМинтранс = Неопределено;
	
	ПрефиксыТитулов = ОбменСГИСЭПДКлиентСервер.ПрефиксыТитуловДокумента("Документ.ЭлектронныйДоговорФрахтования");
	...
	ЭтотОбъект.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭДФ_Титул1");
