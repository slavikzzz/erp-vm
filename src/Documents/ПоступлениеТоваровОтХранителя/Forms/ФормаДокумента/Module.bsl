...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//++ НЕ УТ
	Обработчик.ЗаполнитьПризнакЦеховаяКладоваяВТаблице(Объект, Ложь);
	//-- НЕ УТ
...
Процедура НадписьЗаголовокЗаказыНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Объект.ХозяйственнаяОперация =
			ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5") Тогда
		ТекстЗаголовка    = НСтр("ru = 'Заказы переработчикам (%КоличествоДокументов%)';
...
Процедура ХозяйственнаяОперацияПриИзменении(Элемент)
	...
	//++ НЕ УТ
	Если (ХозяйственнаяОперацияДоИзменения = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5")
				Или ХозяйственнаяОперацияДоИзменения = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратОтПереработчика2_5"))
			И Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратОтХранителя")
		Или ХозяйственнаяОперацияДоИзменения = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратОтХранителя")
			И (Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5")
				Или Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратОтПереработчика2_5")) Тогда
...
Процедура ДатаПриИзменении(Элемент)
	...
	//++ НЕ УТ
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5") Тогда
		Возврат;
	КонецЕсли;
	//-- НЕ УТ
...
Процедура ВалютаПриИзменении(Элемент)
	...
	ПересчитатьСуммы =
		...
		//++ НЕ УТ
		И Не Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5")
		//-- НЕ УТ
		И Истина;
...
Процедура ОткрытьПодбор(Команда)
	...
	//++ НЕ УТ
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5")
		Или Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратОтПереработчика2_5") Тогда
...
Процедура ПодобратьПереданныеТовары(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Партнер) Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Поле ""%1"" не заполнено';
									|en = 'Field %1 is required'"), Элементы.Партнер.Заголовок);
	...
	ПараметрыФормы.Вставить(
		"СвернутьНомераГТД",
		Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5")
		Или Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратОтПереработчика2_5"));
	//-- НЕ УТ
...
Процедура ПерезаполнитьПоПриемке(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ТекстВопроса       = НСтр("ru = 'Для заполнения по приемке необходимо предварительно записать документ. Выполнить запись документа и продолжить?';
									|en = 'To fill in by receipt, you should save the document first. Save the document and continue?'");
...
Процедура ЗагрузитьИзВнешнегоФайла(Команда)
	...
	//++ НЕ УТ
	ПараметрыЗагрузки.ЗагружатьЦены =
		Не Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5");
	//-- НЕ УТ
		...
		Иначе
			Если Объект.Товары.Количество() = 0 Тогда
				ТекстВопроса = НСтр("ru = 'Список ""Товары"" будет перезаполнен остатками неоформленных товаров по заказам. Продолжить?';
									|en = 'The ""Goods"" list will be refilled with remaining unregistered order goods. Continue?'");
			Иначе
				ТекстВопроса = НСтр("ru = 'Строки в списке ""Товары"" будут привязаны к строкам заказов. Продолжить?';
									|en = 'The ""Goods"" list lines will be linked to the order lines. Continue?'");
			КонецЕсли;
		КонецЕсли;
			...
			Если ИспользоватьПоступлениеПоНесколькимЗаказам Тогда
				ТекстВопроса =
					НСтр("ru = 'Строки в списке ""Товары"" перестанут быть связанными со строками заказов. Продолжить?';
						|en = 'The ""Goods"" list lines will be unlinked from the order lines. Continue?'");
			Иначе
				ТекстВопроса =
...
Процедура ДоговорПриИзмененииСервер(ОчиститьСтроки, ОтвязатьСтроки)
	...
	РеквизитыДоговора = Новый Структура("Валюта, Подразделение", "ВалютаВзаиморасчетов");
...
Процедура СкладПриИзмененииСервер(ОчиститьСтроки, ОтвязатьСтроки)
	...
	//++ НЕ УТ
	Обработчик = Документы.ПоступлениеТоваровОтХранителя.ОбработчикДействий(Объект.ХозяйственнаяОперация);
	Обработчик.ЗаполнитьПризнакЦеховаяКладоваяВТаблице(Объект, Обработчик.НеобходимПересчетКоличествоЕдиниц());
	//-- НЕ УТ
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	//++ НЕ УТ
	Если Не Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5") Тогда
	//-- НЕ УТ
...
Процедура ОбработкаВыбораПодборПереданныхТоваров(ВыбранноеЗначение, ЦеныРассчитаны)
	...
	СтруктураДействий = Новый Структура; 
	Обработчик.ДобавитьДействиеЗаполнитьПризнакЦеховаяКладовая(СтруктураДействий, Объект.Склад);
	
	Если Обработчик.НеобходимПересчетКоличествоЕдиниц() Тогда
		Обработчик.ДобавитьДействиеПересчитатьКоличествоЕдиниц(СтруктураДействий);
	КонецЕсли;
	
	ОбработкаТабличнойЧастиСервер.ОбработатьТЧ(МассивСтрок, СтруктураДействий, КэшированныеЗначения);
	//-- НЕ УТ
...
Процедура ЗаполнитьНомераГТДПоПереданнымТоварамНаСервере()
	...
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(ДействияСоСтрокой, Объект, КомиссионерНеВедетУчетПоРНПТ, Ложь);
...
Процедура УстановитьУсловноеОформление()
	...
	НоменклатураСервер.УстановитьУсловноеОформлениеНомераГТД(ЭтотОбъект, "ТоварыНомерГТДСтранаПроисхождения");
	...
	ОперацииПереработчика.Добавить(Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5);
	ОперацииПереработчика.Добавить(Перечисления.ХозяйственныеОперации.ВозвратОтПереработчика2_5);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	//++ НЕ УТ
	ПересчитатьКоличествоЕдиниц = Обработчик.НеобходимПересчетКоличествоЕдиниц() И Не ЗначениеЗаполнено(Объект.Ссылка);
	Обработчик.ЗаполнитьПризнакЦеховаяКладоваяВТаблице(Объект, ПересчитатьКоличествоЕдиниц);
	//-- НЕ УТ
...
Процедура РассчитатьИтоговыеПоказатели(Форма)
	...
	//++ НЕ УТ
	Если Форма.Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5")Тогда
		Возврат;
	КонецЕсли;
	//-- НЕ УТ
...
Процедура ДобавитьДействиеПересчитатьКоличествоЕдиниц(ХозяйственнаяОперация, СтруктураДействий, Склад = Неопределено)
	...
	Обработчик.ДобавитьДействиеПересчитатьКоличествоЕдиниц(СтруктураДействий);
	//++ НЕ УТ
	Обработчик.ДобавитьДействиеЗаполнитьПризнакЦеховаяКладовая(СтруктураДействий, Склад);
	//-- НЕ УТ
...
Процедура УстановитьСвойстваЭлементов()
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НаправлениеДеятельности", "ТолькоПросмотр", Объект.ПоРаспоряжениям);
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище)
	...
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(
		СтруктураДействийИзмененныеСтроки, Объект, КомиссионерНеВедетУчетПоРНПТ, Ложь);
...
Функция ПолучитьПараметрыОтбораВидаЦен()
	...
	//++ НЕ УТ
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5
		Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтПереработчика2_5 Тогда
		ПараметрыОтбораВидаЦен.Вставить("ИспользоватьПриВыпускеПродукции", Истина);
	Иначе
	//-- НЕ УТ
...
Функция ОтборПоТоваруМногооборотнойТаре(ХозяйственнаяОперация)
	...
	//++ НЕ УТ
	Если ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5") Тогда
		ВключатьНаборы = Ложь;
	КонецЕсли;
	//-- НЕ УТ
