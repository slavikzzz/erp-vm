...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если (ПолучитьФункциональнуюОпцию("ЗапретитьПоступлениеТоваровБезНомеровГТД")
		...
		//++ НЕ УТ
		И Не ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5
		И Не ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтПереработчика2_5
		//-- НЕ УТ
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		//++ НЕ УТ
		Если ОперацияРаспоряжения = Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5
		 Или ОперацияРаспоряжения = Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчика2_5
		 Или ОперацияРаспоряжения = Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчикаВСтранахЕАЭС2_5 Тогда
			ДанныеЗаполнения.РеквизитыШапки.Свойство("ХозяйственнаяОперация", ОперацияРаспоряжения);
		КонецЕсли;
		//-- НЕ УТ
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОтгрузкаТоваровСХранения") Тогда
		ЗаполнитьПоОтгрузкеТоваровСХранения(ДанныеЗаполнения);
	//-- НЕ УТКА
	...
	Если ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента")
		...
		Если КомиссионныеПродажи25 = Ложь Тогда
			ТекстОшибки = НСтр("ru = 'Невозможно оформить документ ""%1"" на основании распоряжения %2 с договором комиссии версии %3. Для оформления возврата воспользуйтесь документом ""%4""';
								|en = 'Cannot register the ""%1"" document based on the %2 reference with the cross-company sales contract version %3. To register the return, use the ""%4"" document'");
...
Процедура ЗаполнитьПоАктуОРасхожденияхПослеОтгрузки(СтруктураЗаполнения)
	...
	//++ НЕ УТ
	|		КОГДА ПередачаТоваров.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПереработчику2_5)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтПереработчика2_5)
	//-- НЕ УТ
...
#Область ЗаполнениеПоОтгрузкеТоваровСХранения

Процедура ЗаполнитьПоОтгрузкеТоваровСХранения(СтруктураЗаполнения)
	
	РеквизитыОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтруктураЗаполнения, "ХозяйственнаяОперация, ПоЗаказу");
	
	Если РеквизитыОснования.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаДавальцу2_5 Тогда
		ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5;
	Иначе
		ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтПереработчика2_5;
	КонецЕсли;
	...
	Если РеквизитыОснования.ПоЗаказу
		И ПолучитьФункциональнуюОпцию("ИспользоватьЗаказыПереработчикам2_5") Тогда
		ПоРаспоряжениям  = Истина;
		Запрос.Текст = ТекстЗапросаЗаполнитьНаОснованииПередачиДавальцуПоЗаказам();
	Иначе
		Запрос.Текст = ТекстЗапросаЗаполнитьНаОснованииПередачиДавальцуБезЗаказа();
	КонецЕсли;
	...
	Если РеквизитыШапки.Следующий() Тогда
		...
		ШаблонОшибки = НСтр("ru = 'Ввод на основании передачи давальцу невозможен. В заказах переработчику указаны разные ""%1"".';
							|en = 'Cannot generate documents based on transfers to provider. Different %1 are specified in subcontracting purchase orders.'");
		
		Если РеквизитыШапки.ОшибкаНаправлениеДеятельности Тогда
			ОшибкиЗаполнения.Добавить(НСтр("ru = 'Направления деятельности';
											|en = 'Line of business'"));
		КонецЕсли;
		
		Если РеквизитыШапки.ОшибкаСклад Тогда
			ОшибкиЗаполнения.Добавить(НСтр("ru = 'Склады';
											|en = 'Warehouses'"));
		КонецЕсли;
		
		Если РеквизитыШапки.ОшибкаПодразделение Тогда
			ОшибкиЗаполнения.Добавить(НСтр("ru = 'Подразделения';
											|en = 'Business units'"));
		КонецЕсли;
		
		Если РеквизитыШапки.ОшибкаВалюта Тогда
			ОшибкиЗаполнения.Добавить(НСтр("ru = 'Валюты';
											|en = 'Currencies'"));
		КонецЕсли;
		
		Если РеквизитыШапки.ОшибкаЦенаВключаетНДС Тогда
			ОшибкиЗаполнения.Добавить(НСтр("ru = 'Включения НДС в цену';
											|en = 'VAT is included in the price'"));
		КонецЕсли;
		
		Если РеквизитыШапки.ОшибкаСделка Тогда
			ОшибкиЗаполнения.Добавить(НСтр("ru = 'Сделки';
											|en = 'Sales opportunities'"));
		КонецЕсли;
		
		Если Не ОшибкиЗаполнения.Количество() = 0 Тогда
			ТекстОшибки = СтрШаблон(ШаблонОшибки, СтрСоединить(ОшибкиЗаполнения, ", "));
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	...
	Если ПоРаспоряжениям Тогда
		...
		СкладыЗаказов = ТаблицаЗаказы.ВыгрузитьКолонку("Склад");
		СкладыЗаказов = ОбщегоНазначенияКлиентСервер.СвернутьМассив(СкладыЗаказов);
		
		Если СкладыЗаказов.Количество() > 1 Тогда
			ВызватьИсключение НСтр("ru = 'В заказах переработчикам указано несколько складов, ввод на основании передачи давальцу невозможен. Необходимо оформить поступление по каждому складу отдельно.';
									|en = 'Subcontracting purchase orders contain several warehouses. Cannot generate documents based on transfers to provider. Register the receipt for each warehouse separately.'");
		КонецЕсли;
		
		ЗаполнитьЗаказПереработчикуВТоварахДавальца(ТаблицаТовары, ТаблицаЗаказы);
		
		ЗаказыПереработчику = ТаблицаЗаказы.ВыгрузитьКолонку("ЗаказПереработчику");
		ЗаказыПереработчику = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ЗаказыПереработчику);
		
		Если ЗаказыПереработчику.Количество() = 1 Тогда
			ТаблицаТовары.ЗаполнитьЗначения(ЗаказыПереработчику[0], "Распоряжение");
		КонецЕсли;
	...
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(НоменклатураСервер.ПараметрыУказанияСерий(ЭтотОбъект, Документы.ПоступлениеТоваровОтХранителя));
	НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(ЭтотОбъект, ПараметрыУказанияСерий);
	
КонецПроцедуры

Функция ТекстЗапросаЗаполнитьНаОснованииПередачиДавальцуПоЗаказам()
	
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЗаказыДавальца.Ссылка КАК ЗаказДавальца
	|ПОМЕСТИТЬ ЗаказыДавальца
	|ИЗ
	|	Документ.ОтгрузкаТоваровСХранения.Товары КАК ТаблицаТоваров
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца2_5 КАК ЗаказыДавальцаОтгрузки
	|	ПО ЗаказыДавальцаОтгрузки.Ссылка = ТаблицаТоваров.ЗаказКлиента
	|	И НЕ ЗаказыДавальцаОтгрузки.Основание = ЗНАЧЕНИЕ(Документ.ЗаказПереработчику2_5.ПустаяСсылка)
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца2_5 КАК ЗаказыДавальца
	|	ПО ЗаказыДавальца.Основание = ЗаказыДавальцаОтгрузки.Основание
	|	
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ДанныеЗаказа.Основание                      КАК ЗаказПереработчику,
	|	ЗаказыПереработчику.Дата                    КАК Дата,
	...
	|	ВЫБОР
	|		КОГДА ЗаказыПереработчику.СкладПродукции.ЭтоГруппа
	|			И ЗаказыПереработчику.СкладПродукции.ВыборГруппы =
	|				ЗНАЧЕНИЕ(Перечисление.ВыборГруппыСкладов.РазрешитьВЗаказах)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|		ИНАЧЕ ЗаказыПереработчику.СкладПродукции
	|	КОНЕЦ                                       КАК Склад,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеЗаказа.Основание,
	|	ЗаказыПереработчику.Дата,
	...
	|	ВЫБОР
	|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыПереработчику.НаправлениеДеятельности) = 1
	|			ТОГДА МИНИМУМ(ЗаказыПереработчику.НаправлениеДеятельности)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|	КОНЕЦ                                    КАК НаправлениеДеятельности,
	|	ВЫБОР
	|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыПереработчику.Склад) = 1
	|			ТОГДА МИНИМУМ(ЗаказыПереработчику.Склад)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|	КОНЕЦ                                    КАК Склад,
	|	ВЫБОР
	|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыПереработчику.Подразделение) = 1
	|			ТОГДА МИНИМУМ(ЗаказыПереработчику.Подразделение)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|	КОНЕЦ                                    КАК Подразделение,
	|	ВЫБОР
	|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыПереработчику.Валюта) = 1
	|			ТОГДА МИНИМУМ(ЗаказыПереработчику.Валюта)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	|	КОНЕЦ                                    КАК Валюта,
	|	ВЫБОР
	|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыПереработчику.ЦенаВключаетНДС) = 1
	|			ТОГДА МИНИМУМ(ЗаказыПереработчику.ЦенаВключаетНДС)
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ                                    КАК ЦенаВключаетНДС,
	|	ВЫБОР
	|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыПереработчику.Сделка) = 1
	|			ТОГДА МИНИМУМ(ЗаказыПереработчику.Сделка)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СделкиСКлиентами.ПустаяСсылка)
	|	КОНЕЦ                                    КАК Сделка,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыПереработчику.НаправлениеДеятельности) > 1 КАК ОшибкаНаправлениеДеятельности,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыПереработчику.Склад) > 1                   КАК ОшибкаСклад,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыПереработчику.Подразделение) > 1           КАК ОшибкаПодразделение,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыПереработчику.Валюта) > 1                  КАК ОшибкаВалюта,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыПереработчику.ЦенаВключаетНДС) > 1         КАК ОшибкаЦенаВключаетНДС,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказыПереработчику.Сделка) > 1                  КАК ОшибкаСделка,
	...
	|	ЗНАЧЕНИЕ(Документ.ЗаказПереработчику2_5.ПустаяСсылка) КАК Распоряжение,
	|	0                                      КАК КодСтроки,
	...
	|		И ТаблицаТоваров.Распоряжение В
	|			(ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА ВыходныеИзделияЗаказа.Ссылка ЕСТЬ НЕ NULL
	|			ТОГДА ВыходныеИзделияЗаказа.Назначение
	|		КОГДА ПобочныеИзделияЗаказа.Ссылка ЕСТЬ НЕ NULL
	|			ТОГДА ПобочныеИзделияЗаказа.Назначение
	|		КОГДА ВыходныеИзделияЭтапа.Ссылка ЕСТЬ НЕ NULL
	|			ТОГДА ВыходныеИзделияЭтапа.Назначение
	|		ИНАЧЕ ПобочныеИзделияЭтапа.Назначение
	|	КОНЕЦ                              КАК Назначение,
	|	ОстаткиТоваров.КодСтроки           КАК КодСтроки,
	|	ОстаткиТоваров.Склад               КАК Склад,
	|	ОстаткиТоваров.КПоступлениюОстаток КАК Количество
	|ИЗ
	...
	|	) КАК ОстаткиТоваров
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику2_5.ВыходныеИзделия КАК ВыходныеИзделияЗаказа
	|	ПО ВыходныеИзделияЗаказа.Ссылка = ОстаткиТоваров.ЗаказПоставщику
	|	И ВыходныеИзделияЗаказа.КодСтроки = ОстаткиТоваров.КодСтроки
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику2_5.ПобочныеИзделия КАК ПобочныеИзделияЗаказа
	|	ПО ПобочныеИзделияЗаказа.Ссылка = ОстаткиТоваров.ЗаказПоставщику
	|	И ПобочныеИзделияЗаказа.КодСтроки = ОстаткиТоваров.КодСтроки
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2 КАК ДанныеЭтапа
	|	ПО ДанныеЭтапа.ЗаказПереработчику = ОстаткиТоваров.ЗаказПоставщику
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ВыходныеИзделия КАК ВыходныеИзделияЭтапа
	|	ПО ВыходныеИзделияЭтапа.Ссылка = ДанныеЭтапа.Ссылка
	|	И ВыходныеИзделияЭтапа.КодСтроки = ОстаткиТоваров.КодСтроки
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ПобочныеИзделия КАК ПобочныеИзделияЭтапа
	|	ПО ПобочныеИзделияЭтапа.Ссылка = ДанныеЭтапа.Ссылка
	|	И ПобочныеИзделияЭтапа.КодСтроки = ОстаткиТоваров.КодСтроки
	|	
...
Функция ТекстЗапросаЗаполнитьНаОснованииПередачиДавальцуБезЗаказа()
	...
	|			ТОГДА ОтгрузкаТоваровСХранения.НаправлениеДеятельности
	...
	|	ОтгрузкаТоваровСХранения.Номер  КАК НомерВходящегоДокумента,
	|	ОтгрузкаТоваровСХранения.Дата   КАК ДатаВходящегоДокумента,
	|	ЛОЖЬ                            КАК ОшибкаНаправлениеДеятельности,
	|	ЛОЖЬ                            КАК ОшибкаСклад,
	|	ЛОЖЬ                            КАК ОшибкаПодразделение,
	|	ЛОЖЬ                            КАК ОшибкаВалюта,
	|	ЛОЖЬ                            КАК ОшибкаЦенаВключаетНДС,
	|	ЛОЖЬ                            КАК ОшибкаСделка,
	|	ОтгрузкаТоваровСХранения.Ссылка КАК ДокументОснование
	|ИЗ
	|	Документ.ОтгрузкаТоваровСХранения КАК ОтгрузкаТоваровСХранения
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Давалец
	|	ПО Давалец.Ссылка = ОтгрузкаТоваровСХранения.Контрагент
	|	
	...
	|	ПО Переработчик.Ссылка = ОтгрузкаТоваровСХранения.Организация
	|	
	...
	|	ПО ДоговорДавальца.Ссылка = ОтгрузкаТоваровСХранения.Договор
	|
	...
	|	ТаблицаТоваров.АналитикаУчетаНоменклатуры.Серия          КАК Серия,
	|	ВЫБОР
	|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5)
	...
	|	СУММА(ВЫБОР
	|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5)
	|			ТОГДА ТаблицаТоваров.КоличествоПоРНПТ
	|		ИНАЧЕ 0
	|	КОНЕЦ)                                                   КАК КоличествоПоРНПТ
	|ИЗ
	|	Документ.ОтгрузкаТоваровСХранения.ВидыЗапасов КАК ТаблицаТоваров
...
Процедура ЗаполнитьЗаказПереработчикуВТоварахДавальца(ТаблицаТовары, ТаблицаЗаказы)
	...
	Для Каждого СтрокаЗаказаПереработчику Из ТаблицаЗаказы Цикл
		...
		Для Каждого ТекущиеДанные Из ТаблицаТовары.НайтиСтроки(Отбор) Цикл
			...
			НоваяСтрока.Распоряжение = СтрокаЗаказаПереработчику.ЗаказПереработчику;
			НоваяСтрока.КодСтроки    = СтрокаЗаказаПереработчику.КодСтроки;
			НоваяСтрока.Склад        = СтрокаЗаказаПереработчику.Склад;
			НоваяСтрока.Назначение   = СтрокаЗаказаПереработчику.Назначение;
			НоваяСтрока.Количество   = Количество;
			
			ТекущиеДанные.Количество = ТекущиеДанные.Количество - Количество;
			
			КоличествоЗаказаПереработчику = КоличествоЗаказаПереработчику - Количество;
...
Процедура ЗаполнитьДополнительныеКолонкиВидовЗапасов()
	...
	//++ НЕ УТ
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5 Тогда
		
		ТипыЗапасов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ВидыЗапасов.ВыгрузитьКолонку("ВидЗапасов"), "ТипЗапасов");
		СтруктурыВидовЗапасов = Новый Соответствие;
		
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	Товары.АналитикаУчетаНоменклатурыТоварыУПартнеров КАК АналитикаУчетаНоменклатурыТоварыУПартнеров
		|ПОМЕСТИТЬ ВтТовары
		|ИЗ
		|	&Товары КАК Товары
		|;
		...
		|ВЫБРАТЬ
		|	Товары.АналитикаУчетаНоменклатурыТоварыУПартнеров КАК АналитикаУчетаНоменклатурыТоварыУПартнеров,
		|	Товары.АналитикаУчетаНоменклатурыТоварыУПартнеров.Номенклатура.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчета
		|ИЗ
		|	ВтТовары КАК Товары
		|СГРУППИРОВАТЬ ПО
		|	Товары.АналитикаУчетаНоменклатурыТоварыУПартнеров
		|";
		
		Запрос.УстановитьПараметр("Товары", Товары.Выгрузить(, "АналитикаУчетаНоменклатурыТоварыУПартнеров"));
		
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			
			СтруктураВидаЗапасов = Справочники.ВидыЗапасов.ПараметрыОтбораВидовЗапасов();
			ЗаполнитьЗначенияСвойств(СтруктураВидаЗапасов, Выборка);
			
			СтруктурыВидовЗапасов.Вставить(
				Выборка.АналитикаУчетаНоменклатурыТоварыУПартнеров,
				СтруктураВидаЗапасов);
	...
	Для Каждого СтрокаТоваров Из Товары Цикл
		...
		Для Каждого СтрокаЗапасов Из НайденныеСтроки Цикл
			...
			//++ НЕ УТ
			Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5
				//++ НЕ УТКА
				И Не ТипыЗапасов[НоваяСтрока.ВидЗапасов] = Перечисления.ТипыЗапасов.ПолуфабрикатДавальца
				И Не ТипыЗапасов[НоваяСтрока.ВидЗапасов] = Перечисления.ТипыЗапасов.ПродукцияДавальца
				//-- НЕ УТКА
				И Не ТипыЗапасов[НоваяСтрока.ВидЗапасов] = Перечисления.ТипыЗапасов.ТоварНаХраненииСПравомПродажи Тогда
				НоваяСтрока.КорВидЗапасов =
					Справочники.ВидыЗапасов.ВидЗапасовДокумента(
						Организация,
						ХозяйственнаяОперация,
						СтруктурыВидовЗапасов.Получить(НоваяСтрока.АналитикаУчетаНоменклатурыТоварыУПартнеров));
			Иначе
			//-- НЕ УТ
...
Процедура ПроверитьЗаполнениеНомеровГТДВВидахЗапасов(Отказ)
	...
	Если Не УчетПрослеживаемыхТоваровЛокализация.ЭтоПрослеживаемыйДокумент(Товары, Дата)
		...
		Или ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5
			И ПраваПользователяПовтИсп.ОтключитьКонтрольПередачиПродукцииПереработчиком()
		//-- НЕ УТ
		Тогда
		Возврат;
	КонецЕсли;
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		ШаблонСообщения = НСтр("ru = 'Не удалось подобрать номер ГТД для товара ""%1""!';
								|en = 'Cannot pick CCD number for the ""%1"" goods.'");
...
Процедура ЗаполнитьЦеныПоУсловиямПродаж()
	...
	//++ НЕ УТ
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5
		Или ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтПереработчика2_5 Тогда
		Возврат;
	КонецЕсли;
	//-- НЕ УТ
