...
#Область ФактыВнутреннихПотреблений
	...
	ДопПараметр.Вставить("Заголовок", 			?(СоздаватьРеквизитСОтбором,
												НСтр("ru = 'Факт внутренних потреблений*';
													|en = 'Actual internal consumption*'"),
												НСтр("ru = 'Факт внутренних потреблений';
													|en = 'Actual internal consumption'")));
...
Функция ДоступныеТипыАналитикБюджетирования(ДокументСсылка, ИмяТаблицы = "") Экспорт
	...
	|ИЗ
	|	Документ.ПланВнутреннихПотреблений.ДополнительныеРеквизиты КАК ДополнительныеРеквизиты
	|ГДЕ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если НЕ ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Значение) Тогда
			Продолжить;
		КонецЕсли;
		ДоступныеТипыАналитик.Вставить(ВыборкаДетальныеЗаписи.Тип, "&ДополнительныйРеквизит"
			+ Формат(ВыборкаДетальныеЗаписи.НомерСтроки, "ЧГ=0"));
	КонецЦикла;
	...
	Если ИмяТаблицы = "Товары" Тогда
		...
		ДоступныеТипыАналитик.Вставить(Тип("СправочникСсылка.ВидыНоменклатуры"), 
			"Товары.Номенклатура.ВидНоменклатуры");
		ДоступныеТипыАналитик.Вставить(Тип("ПеречислениеСсылка.ТипыНоменклатуры"), 
		...
		ДоступныеТипыАналитик.Вставить(Тип("СправочникСсылка.ТоварныеКатегории"), 
		...
		ДоступныеТипыАналитик.Вставить(Тип("СправочникСсылка.ГруппыАналитическогоУчетаНоменклатуры"), 
			"Товары.Номенклатура.ГруппаАналитическогоУчета");
		ДоступныеТипыАналитик.Вставить(Тип("СправочникСсылка.НаправленияДеятельности"), 
	...
	Иначе
		...
		ДоступныеТипыАналитик.Вставить(Тип("СправочникСсылка.НаправленияДеятельности"), ИмяТаблицы 
			+ ".Ссылка.Назначение.НаправлениеДеятельности");
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если НЕ ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Значение) Тогда
			Продолжить;
		КонецЕсли;
		Запрос.УстановитьПараметр("ДополнительныйРеквизит"+Формат(ВыборкаДетальныеЗаписи.НомерСтроки, "ЧГ=0"), 
...
Функция ТекстЗапросаТаблицаОборотыБюджетов(Запрос, ТекстыЗапроса, Регистры)
	...
	|ИЗ
	|	Документ.ПланВнутреннихПотреблений.Товары КАК Товары
	|ГДЕ
	...
	БюджетированиеСервер.УстановитьВЗапросеВыраженияЗаполненияАналитики(
		Запрос.Параметры.Ссылка, ТекстЗапроса, Запрос.Параметры.СтатьяБюджетов, 
		Документы.ПланВнутреннихПотреблений, "АналитикаТовары", "Товары");
...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	Иначе
		ТекстИсключения = НСтр("ru = 'В документе %ПолноеИмяДокумента% 
			...
			|en = 'In document %ПолноеИмяДокумента%, 
			| adaptation of request for generating records of register %ИмяРегистра% is not implemented.'");
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ВыводитьШапку Тогда
			...
			ТекстЗаголовка = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(ДанныеПечати, 
				НСтр("ru = 'План внутренних потреблений';
					|en = 'Inventory consumption plan'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление  = НСтр("ru = 'Планы внутренних потреблений';
								|en = 'Inventory consumption plans'");
