...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	НастройкаСчетовУчетаСервер.ПередЗаписью(ЭтотОбъект,
		Документы.ПоступлениеБезналичныхДенежныхСредств.ПараметрыНастройкиСчетовУчета(ХозяйственнаяОперация, ТипНалога));
	//-- НЕ УТ
	...
	Если РезультатЗапроса.Пустой() Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не требуется вводить поступление безналичных денежных средств на основании документа %1';
				|en = 'It is not required to enter ""Incoming payment — Bank account"" based on document %1'"),
			ДокументОснование);
...
Процедура ЗаполнитьПоЗаказуДавальца(Знач ДокументОснование, ДанныеЗаполнения, СуммаКОплате = 0)
...
Процедура ЗаполнитьПоОтчетуДавальцу(Знач ДокументОснование, ДанныеЗаполнения, СуммаКОплате = 0)
	...
	Если Выборка.Следующий() Тогда
		...
		Иначе
			...
			|	И ТаблицаПродукции.Ссылка.ОтчетПоЗаказам
			//++ Устарело_Переработка24
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
...
Процедура ЗаполнитьПоОтчетуДавальцуМеждуОрганизациями(Знач ДокументОснование, ДанныеЗаполнения, СуммаКОплате = 0)
	...
	|	ДанныеДокумента.БанковскийСчетДавальца КАК БанковскийСчетПолучатель,
	|	ДанныеДокумента.БанковскийСчетПереработчика КАК БанковскийСчет,
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.БанковскийСчетПереработчика <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)
	|			ТОГДА ДанныеДокумента.БанковскийСчетПереработчика.ВалютаДенежныхСредств
	|		ИНАЧЕ ДанныеДокумента.Валюта
	|	КОНЕЦ КАК Валюта,
		...
		Если Не (Выборка.ТипДоговора = Перечисления.ТипыДоговоров.СПокупателем
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не требуется вводить документ поступления безналичных денежных средств на основании договора с типом взаимоотношений ""%1""';
					|en = 'It is not required to enter the ""Incoming payment — Bank account"" document based on a contract with the ""%1"" relation type'"),
				Выборка.ТипДоговора);
			ВызватьИсключение Текст;
		КонецЕсли;
		...
		Если Выборка.УдержатьВознаграждение Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не требуется вводить поступление безналичных денежных средств на основании документа %1, поскольку в нем удержано вознаграждение';
					|en = 'It is not required to enter ""Incoming payment — Bank account"" based on document %1 as remuneration is deducted in it'"),
				ОтчетКомитенту);
