...
Процедура ПередЗаписьюПредложитьПодобратьМногооборотнуюТаруЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ВозможныКоллизииСМобильнымПриложением(Объект.Ссылка) Тогда
		...
		ТекстВопроса = НСтр("ru = 'Запись задания, которое было ранее подготовлено или отработано,%ПереводСтроки%может привести к коллизиям при обмене с мобильным приложением.%ПереводСтроки%Продолжить?';
							|en = 'Saving a job that was previously prepared or processed%ПереводСтроки%can cause collisions when exchanging data with mobile application.%ПереводСтроки%Continue?'");
...
Процедура РазбитьСтроку(Команда)
	...
	ПараметрыРазбиенияСтроки.Заголовок = НСтр("ru = 'Введите количество (план) в новой строке.';
												|en = 'Enter quantity (plan) in a new line.'");
