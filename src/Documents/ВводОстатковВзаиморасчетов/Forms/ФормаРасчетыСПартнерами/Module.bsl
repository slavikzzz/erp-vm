...
Процедура УстановитьУсловноеОформление()
	...
	ХозОперации.Добавить(Перечисления.ХозяйственныеОперации.ВводОстатковЗадолженностиКлиентов);
	ХозОперации.Добавить(Перечисления.ХозяйственныеОперации.ВводОстатковЗадолженностиПоставщикам);
...
Процедура УстановитьВидимость()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковЗадолженностиКлиентов
		...
		Элементы.РасчетыСПартнерамиОтложенныйДоходНУ.Видимость = Объект.ОтражатьВБУиНУ;
		ДатаДокумента = ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, ТекущаяДатаСеанса());
		Если ДатаДокумента >= РегистрыСведений.НастройкиУчетаНалогаНаПрибыль.НачалоПрименения67ФЗПоРасходам() Тогда
			Элементы.РасчетыСПартнерамиОтложенныйРасходНУ.Видимость = Объект.ОтражатьВБУиНУ;
		КонецЕсли;
	КонецЕсли;
	//-- Локализация
...
Процедура УстановитьПараметрыВыбораРасчетногоДокумента(ИмяЭлементаРасчетныйДокумент, СтрокаТаблицы, СозданиеНового = Ложь)
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковЗадолженностиКлиентов") Тогда
		//++ НЕ УТКА
		Если ТипЗнч(СтрокаТаблицы.ОбъектРасчетов) <> Тип("ДокументСсылка.ЗаказДавальца2_5")
			Или Не ИспользоватьПроизводствоИзДавальческогоСырья2_5 Тогда
		//-- НЕ УТКА
		МассивТиповПервичныхДокументов.Добавить(ПредопределенноеЗначение("Перечисление.ТипыПервичныхДокументов.РеализацияКлиенту"));
		//++ НЕ УТКА
		КонецЕсли;
		Если ТипЗнч(СтрокаТаблицы.ОбъектРасчетов) <> Тип("ДокументСсылка.ЗаказКлиента")
			И ТипЗнч(СтрокаТаблицы.ОбъектРасчетов) <> Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента")
			И ИспользоватьПроизводствоИзДавальческогоСырья2_5 Тогда
			МассивТиповПервичныхДокументов.Добавить(ПредопределенноеЗначение("Перечисление.ТипыПервичныхДокументов.ОтчетДавальцу"));
		КонецЕсли;
		//-- НЕ УТКА
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковЗадолженностиПоставщикам") Тогда
		//++ НЕ УТ
		Если ТипЗнч(СтрокаТаблицы.ОбъектРасчетов) <> Тип("ДокументСсылка.ЗаказПереработчику2_5")
			Или Не ИспользоватьПроизводствоНаСтороне2_5 Тогда
		//-- НЕ УТ
		...
		//++ НЕ УТ
		КонецЕсли;
		Если ТипЗнч(СтрокаТаблицы.ОбъектРасчетов) <> Тип("ДокументСсылка.ЗаказПоставщику")
			И ИспользоватьПроизводствоНаСтороне2_5 Тогда
			МассивТиповПервичныхДокументов.Добавить(ПредопределенноеЗначение("Перечисление.ТипыПервичныхДокументов.ОтчетПереработчика"));
		КонецЕсли;
		//-- НЕ УТ
		МассивПараметровРасчетногоДокумента.Добавить(Новый ПараметрВыбора("Отбор.ТипПервичногоДокумента", Новый ФиксированныйМассив(МассивТиповПервичныхДокументов)));
	...
	Если ЗначениеЗаполнено(РасчетныйДокумент) 
		...
		Если ТекущиеДанные <> Неопределено
			...
			ТекстПредупреждения = НСтр("ru = '""%1"" - валюта взаиморасчетов выбираемого расчетного документа отличается от валюты взаиморасчетов объекта расчетов';
										|en = '""%1"": the AR/AP currency of the selected AR/AP document differs from the AR/AP currency of the AR/AP object'");
		...
		ИначеЕсли РеквизитыПервичногоДокумента.Дата > КонецДня(Объект.Дата) Тогда
			...
			ТекстПредупреждения = НСтр("ru = '""%1"" - дата выбираемого документа не может быть больше даты ввода начальных остатков';
										|en = '""%1"": the selected document date cannot be later than the date of opening balance entry'");
...
Функция ОбъектСПорядкомРасчетов(Ссылка)
	...
	Возврат ТипЗнч(Ссылка) <> Тип("ДокументСсылка.АвансовыйОтчет") 
		...
		//++ Локализация
		И ТипЗнч(Ссылка) <> Тип("ДокументСсылка.ОтражениеЗарплатыВФинансовомУчете2_5")
		//-- Локализация
		
		И ТипЗнч(Ссылка) <> Тип("СправочникСсылка.ДоговорыАренды")
		//-- НЕ УТ
...
Процедура ЗаполнитьПоОстаткамНаСервере()
	...
	КлючевыеПоля = Документы.ВводОстатковВзаиморасчетов.КлючевыеПоляРеглУчетаПоТипуОперации(Объект.ХозяйственнаяОперация);
	...
	Остатки = Документы.ВводОстатковВзаиморасчетов.ОстаткиПоТипуОперации(Объект.Дата, Объект.ХозяйственнаяОперация, Объект.Организация, Отбор);
