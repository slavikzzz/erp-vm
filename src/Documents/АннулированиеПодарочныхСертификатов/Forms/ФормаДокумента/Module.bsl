...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Если Объект.Организация <> ОрганизацияСтароеЗначение 
		...
		ПоказатьВопрос(
				...
				НСтр("ru = 'При изменении организации документа таблица ""Подарочные сертификаты"" будет очищена. Продолжить?';
					|en = 'When you change the document company, the Gift cards table will be cleared. Continue?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура Заполнить(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Организация) Тогда
		ТекстСообщения = НСтр("ru = 'Перед заполнением необходимо выбрать организацию.';
								|en = 'Before filling, select a company.'");
	...
	Если Результат = Истина Тогда
		Если Объект.ПодарочныеСертификаты.Количество() Тогда
			ПоказатьВопрос(
					...
					НСтр("ru = 'Перед заполнением таблица ""Подарочные сертификаты"" будет очищена. Продолжить?';
						|en = 'Before filling, the Gift cards table will be cleared. Continue?'"), РежимДиалогаВопрос.ДаНет);
