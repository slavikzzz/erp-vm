...
Процедура ЗаполнитьТабличнуюЧастьТовары(ДокументОснование = Неопределено) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ДокументОснование) Тогда
		...
		ТекстСообщения = НСтр("ru = 'В документе ""%ДокументОснование%"" отсутствуют товары, по которым необходимо оформить порчу.';
								|en = 'Goods for which damage registration is required are absent in document ""%ДокументОснование%"".'");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//++ НЕ УТКА
	Назначения = Товары.ВыгрузитьКолонку("Назначение");
	ТипыНазначений = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(Назначения, "ТипНазначения");
	//-- НЕ УТКА
	...
	Для Каждого СтрТабл Из Товары Цикл
		...
		Если ЗначениеЗаполнено(СтрТабл.Номенклатура)
			...
			ТекстОшибки = НСтр("ru = 'Нельзя выбирать товар исходного качества. Выберите товар другого качества';
								|en = 'You cannot select goods of the original quality. Select goods with another quality'");
		...
		//++ НЕ УТКА
		Если ТипыНазначений.Получить(СтрТабл.Назначение) = Перечисления.ТипыНазначений.Давальческое2_5
			И Не СтрТабл.ПодНазначение Тогда
			
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", СтрТабл.НомерСтроки, "ПодНазначение");
			ТекстОшибки = НСтр("ru = 'При порче товара, обособленного под назначение давальца, приход товара должен быть ""Под назначение""';
								|en = 'If an item assigned for the provider assignment has a stock quality change, the goods receipt must be ""For assignment""'");
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки + " " + АдресОшибки,ЭтотОбъект,Поле,,Отказ);
