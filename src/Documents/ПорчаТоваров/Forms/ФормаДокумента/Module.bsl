...
Процедура ТоварыНоменклатураОприходованиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Выберите строку, для которой Вы хотите изменить качество';
													|en = 'Select a line you want to change quality for'"));
		Возврат;
	КонецЕсли;
...
Процедура Заполнить(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект), НСтр("ru = 'Табличная часть будет очищена и заполнена всеми товарами, по которым нужно оформить порчу. Продолжить?';
																						|en = 'The table will be refilled with damaged items. Do you want to continue?'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
	КонецЕсли;
	...
	Пока ПредварительныеДанные.Следующий() Цикл
		Если ПредварительныеДанные.Количество = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Не было произведено ни одного расчета себестоимости товаров по организации ""%Организация%"".';
									|en = 'No inventory costs were calculated for company ""%Организация%"".'");
