	...
	Если Не ТребуетсяПереоценка И Не ПериодВДиапазонеПересчета Тогда
		...
		ИначеЕсли НетПроцентовКСписанию Тогда
			...
			ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
				...
				НСтр("ru = 'Нет процентных расходов дисконтирования к списанию.';
					|en = 'No discounting interest expenses to write off.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Иначе
		...
		Если ПериодВДиапазонеПересчета Тогда
			...
			НачалоРасчета = ЗакрытиеМесяцаСервер.НачалоРасчета(
				...
				Неопределено,
				ПараметрыРасчета.МассивОрганизаций);
			ТекстОперации = НСтр("ru = 'Формирование движений по процентным расходам дисконтирования начиная с периода %1';
								|en = 'Generate register records by discounting interest expenses starting from %1'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Если ТребуетсяПереоценка Тогда
			...
			Если ПериодВДиапазонеПересчета Тогда
				ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
					ПараметрыОбработчика,
					НСтр("ru = 'Расчет курсовых разниц по процентным расходам дисконтирования.';
						|en = 'Calculate exchange differences for discounting interest expenses.'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура Оформление_РасчетПроцентныхРасходовДисконтирования(ПараметрыОбработчика) Экспорт
	...
	ПараметрыОбработчика.ДанныеЭтапа.Наименование = НСтр("ru = 'Расчет процентных расходов дисконтирования';
														|en = 'Calculate discounting interest expenses'");
...
Процедура РасчетПроцентныхРасходовДисконтированияРезервов(Запрос, ДокументСсылка)
	...
	|	ТаблицаРезервов.СуммаДисконтированияУпр - ТаблицаРезервов.СуммаУпр * (1 - 1 / (ВЫРАЗИТЬ(POW(1 + ТаблицаСтавок.СтавкаДисконтирования / 100, РАЗНОСТЬДАТ(ВЫБОР
	...
	|					ИНАЧЕ &КонецПериода
	|				КОНЕЦ, ТаблицаСроков.ДатаИсполнения, ДЕНЬ) / 365) КАК ЧИСЛО(23, 10)))) КАК СуммаДисконтированияУпр,
	|	ТаблицаРезервов.СуммаДисконтированияРегл - ТаблицаРезервов.СуммаРегл * (1 - 1 / (ВЫРАЗИТЬ(POW(1 + ТаблицаСтавок.СтавкаДисконтирования / 100, РАЗНОСТЬДАТ(ВЫБОР
	|					КОГДА НАЧАЛОПЕРИОДА(&КонецПериода, МЕСЯЦ) >= НАЧАЛОПЕРИОДА(ТаблицаСроков.ДатаИсполнения, МЕСЯЦ)
	|						ТОГДА ТаблицаСроков.ДатаИсполнения
	|					ИНАЧЕ &КонецПериода
	|				КОНЕЦ, ТаблицаСроков.ДатаИсполнения, ДЕНЬ) / 365) КАК ЧИСЛО(23, 10)))) КАК СуммаДисконтированияРегл,
