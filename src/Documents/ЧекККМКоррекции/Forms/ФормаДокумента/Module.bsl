...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.Основание) = Тип("ДокументСсылка.ЧекККМКоррекции")
		...
		ТекстСообщения = НСтр("ru = 'Копирование документа ""Чек ККМ коррекции"" запрещено!';
								|en = 'Copying of the ""Cash register adjustment voucher"" document is not allowed!'");
...
Процедура ТоварыПередНачаломИзменения(Элемент, Отказ)
	...
	Если ИзменяетсяНоменклатура(Элемент.ТекущийЭлемент.Имя)
		...
		ТекстСообщения = НСтр("ru = 'Корректировка маркированной продукции запрещена. По данной номенклатуре можно изменять только ставку НДС.';
								|en = 'Adjusting entry of marked products is not permitted. For this item, only the VAT rate can be changed.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	Если НЕ РазрешеноИзменятьНоменклатуру(ТекущаяСтрока.Номенклатура) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Корректировка алкогольной и табачной продукции запрещена.';
								|en = 'Adjustment of alcohol containing and tobacco products is prohibited.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	Иначе	
...
Процедура ОбновитьЗаголовокСтраницыРасхождения()
	...
	Если РасхожденияКоличествоСтрок > 0 Тогда
		пЗаголовок = СтрШаблон(НСтр("ru = 'Расхождения (%1)';
									|en = 'Discrepancies (%1)'"), РасхожденияКоличествоСтрок);
	Иначе
...
Функция РазрешеноИзменятьНоменклатуру(Номенклатура)
	...
	Если ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.АлкогольнаяПродукция
		...
		Или ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ЛегкаяПромышленность2025
		...
		Или ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.КормаДляЖивотныхВлажныеБезВЕТИС
		...
		Или ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.МедицинскиеИзделия
		Или ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.МедицинскиеИзделия20
		Или ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПолимерныеТрубы
		Или ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.СладостиИКондитерскиеИзделия
		Или ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПечатныеПлаты
		Или ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ДезинфицирующиеСредства
		Или ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.БАДы2025 Тогда
	...
	Если Результат.ВыполненаОперацияНаУстройстве Тогда
		...
		Если Не Результат.ИзмененныеДанныеЗаписаны Тогда
			...
			ПараметрыФормы.Вставить("ТекстСообщения",
				...
				|en = 'ATTENTION! An exception occurred:
				| Cash register adjustment voucher is issued but not recorded in the system.'"));
