...
Процедура ЗаполнитьПоказатели(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Начисление) Тогда 
		ТекстСообщения = НСтр("ru = 'Не указан вид дохода.';
								|en = 'Income kind is not specified.'");
...
Функция ОписаниеДокумента(Форма)
	...
	Описание.ОбязательныеПоля.Добавить(РасчетЗарплатыРасширенныйКлиентСервер.ОписаниеОбязательногоПоляДокумента(НСтр("ru = 'Начисление';
																													|en = 'Accrual'"), "Объект.Начисление"));
	Описание.ОбязательныеПоля.Добавить(РасчетЗарплатыРасширенныйКлиентСервер.ОписаниеОбязательногоПоляДокумента(НСтр("ru = 'Дата получения дохода';
																													|en = 'Date of income receipt'"), "Объект.ДатаПолученияДохода"));
