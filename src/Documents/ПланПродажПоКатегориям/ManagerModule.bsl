...
#Область ПланыПродажПоКатегориямПоСкоростиПродаж
	...
	ДопПараметр.Вставить("Заголовок", 			НСтр("ru = 'План продаж по категориям (по скорости продаж)';
														|en = 'Sales forecast by categories (by sales speed)'"));
...
Функция ДоступныеТипыАналитикБюджетирования(ДокументСсылка, ИмяТаблицы = "") Экспорт
	...
	ДоступныеТипыАналитик.Вставить(Тип("СправочникСсылка.ТоварныеКатегории"), "ТоварныеКатегории.ТоварнаяКатегория");
	ДоступныеТипыАналитик.Вставить(Тип("СправочникСсылка.НаправленияДеятельности"), "ТоварныеКатегории.Назначение.НаправлениеДеятельности");
	ДоступныеТипыАналитик.Вставить(Тип("ДокументСсылка.ЗаказКлиента"), "ТоварныеКатегории.Назначение.Заказ");
...
Функция ТекстЗапросаТаблицаОборотыБюджетов(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И &Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.Отменен)
	|	И НЕ ТоварныеКатегории.Отменено
	|	И НЕ ТоварныеКатегории.Замещен
	|
	|СГРУППИРОВАТЬ ПО
	|	&Аналитика1,
	|	&Аналитика2,
	...
	БюджетированиеСервер.УстановитьВЗапросеВыраженияЗаполненияАналитики(
		...
		ТекстЗапроса,
		Запрос.Параметры.СтатьяБюджетов, Документы.ПланПродажПоКатегориям, "Аналитика");
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ВыводитьШапку Тогда
			...
			ТекстЗаголовка = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(
				ДанныеПечати,
				НСтр("ru = 'План продаж по категориям';
					|en = 'Sales forecast by categories'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Если ДанныеПечати.ЗаполнятьРейтингПродаж Тогда
			...
			СтруктураДопПоля.Вставить("Заголовок", НСтр("ru = 'Рейтинг продаж';
														|en = 'Sales rank'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление  = НСтр("ru = 'Планы продаж по категориям';
								|en = 'Sales forecasts by categories'");
