	...
	Если НЕ Объект.КроссТаблица Тогда
		...
		Если Отказ Тогда
			...
			ТекстВопроса = НСтр("ru = 'В строках списка ""Товарные категории"" указаны даты за границами планирования - строки будут удалены. Удалить строки?';
								|en = 'Dates outside the planning fences are specified in lines of the ""Product categories"" list. Lines will be deleted. Delete the lines?'");
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	КолонкиПоиска.Добавить(НСтр("ru = 'Уникальный идентификатор (Товарная категория)';
								|en = 'UUID (Product category)'"),"ИдентификаторТоварнаяКатегория");
	КолонкиПоиска.Добавить(НСтр("ru = 'Уникальный идентификатор (Рейтинг продаж)';
								|en = 'UUID (Sales rating)'"),"ИдентификаторРейтингПродаж");
	...
	КолонкиПоиска.Добавить(НСтр("ru = 'Номенклатура';
								|en = 'Items'"),"ЗагружаемаяТоварнаяКатегория");
	...
	Если ИспользоватьРасчетПоСкоростиПродаж Тогда
		КолонкиПоиска.Добавить(НСтр("ru = 'Рейтинг продаж';
									|en = 'Sales rank'"),"ЗагружаемыйРейтингПродаж");
	КонецЕсли; 
	Если ЗаполнятьСкладВТЧ Тогда
...
Процедура ПоказатьФормуЗагрузкиИзФайла(ДополнительныеПараметры)
	...
	ПараметрыЗагрузки.Заголовок               = НСтр("ru = 'Загрузка таблицы ""Товарные категории"" из файла';
													|en = 'Import the Product categories table from the file'");
	...
	Если Объект.КроссТаблица Тогда
		...
		Если Отказ Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'В строках списка ""Товарные категории"" указаны даты за границами планирования - строки были удалены.';
														|en = 'Dates outside the planning fences are specified in lines of the Product categories list. The lines were removed.'"));
		КонецЕсли; 
