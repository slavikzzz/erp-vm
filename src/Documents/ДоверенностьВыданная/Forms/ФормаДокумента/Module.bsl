	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Выбрано некорректное основание: %Основание%.';
									|en = 'Incorrect base is selected: %Основание%.'");
...
Процедура ЗаполнитьНаличныеДС(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		Сообщение = НСтр("ru = 'Укажите основание для заполнения по расчетам.';
						|en = 'Specify the base document for population by settlements.'");
	...
	Если НЕ Объект.СуммаДокумента = 0 Тогда
		...
		|будет перезаполнена по расчетам по основанию %1. Продолжить?';
		|en = 'The amount to receive under the authorization letter will be overwritten according to settlements by base document %1.
		|Continue?'"), СокрЛП(Объект.ДокументОснование));
...
Процедура ЗаполнитьНаличныеДСФрагмент()
	...
	Если Объект.СуммаДокумента = 0 Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Отсутствует задолженность по расчетам по основанию %1';
										|en = 'No debt for settlements by base document %1'"),
...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Список товаров к получению по доверенности будет перезаполнен по основанию. Продолжить?';
							|en = 'The list of goods to receive under the authorization letter will be repopulated using the ""Input on basis"" method. Continue?'");
		...
		ЗаголовокОкнаВопроса = НСтр("ru = 'Заполнение товаров к получению';
									|en = 'Filling in the goods to be received'");
...
Процедура РассчитатьСостояние(Команда)
	...
	Если Объект.Ссылка.Пустая() Или Модифицированность Тогда
		...
		|en = 'Document is not saved yet.
		|Status can be calculated only after data is saved.
		|Data will be saved.'");
...
Функция ТипыОснований(ТипДоверенности)
	...
	Если ТипДоверенности = ПредопределенноеЗначение("Перечисление.ТипыВыданныхДоверенностей.НаПолучениеТоваров")
		...
		ТипыОснований.Добавить(Тип("ДокументСсылка.ЗаказДавальца"));
		//-- Устарело_Переработка24
		ТипыОснований.Добавить(Тип("ДокументСсылка.ЗаказДавальца2_5"));
		//-- НЕ УТКА

		//++ Устарело_Переработка24
		ТипыОснований.Добавить(Тип("ДокументСсылка.ЗаказПереработчику"));
		//-- Устарело_Переработка24
		ТипыОснований.Добавить(Тип("ДокументСсылка.ЗаказПереработчику2_5"));
		//-- НЕ УТ
	...
	ИначеЕсли Объект.ТипДоверенности = ПредопределенноеЗначение("Перечисление.ТипыВыданныхДоверенностей.НаПолучениеДС") Тогда
		...
		//++ Устарело_Переработка24
		ТипыОснований.Добавить(Тип("ДокументСсылка.ЗаказДавальца"));
		ТипыОснований.Добавить(Тип("ДокументСсылка.ОтчетДавальцу"));
		//-- Устарело_Переработка24
		ТипыОснований.Добавить(Тип("ДокументСсылка.ЗаказДавальца2_5"));
		ТипыОснований.Добавить(Тип("ДокументСсылка.ОтчетДавальцу2_5"));
		//-- НЕ УТКА
...
Процедура ЗаполнитьПоЗаказуДавальца2_5(Знач ДокументОснование)
	...
	|ПОМЕСТИТЬ ДанныеПоТоварам
	|ИЗ
	|	Документ.ЗаказДавальца2_5.Материалы КАК ЗаказТовары
	|ГДЕ
	|	ЗаказТовары.Ссылка = &ДокументОснование
	...
	|			И КодСтроки В
	|					(ВЫБРАТЬ
	|						ДанныеПоТоварам.КодСтроки КАК КодСтроки
	|					ИЗ
	|						ДанныеПоТоварам КАК ДанныеПоТоварам)) КАК ЗаказыПоставщикам
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеПоТоварам КАК ЗаказТовары
	|		ПО ЗаказыПоставщикам.КодСтроки = ЗаказТовары.КодСтроки
	|ГДЕ
	|	ЗаказыПоставщикам.КОформлениюОстаток > 0";
