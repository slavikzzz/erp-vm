...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	//-- Устарело_Переработка24
	ЧтениеЗаказПереработчику2_5 = ПравоДоступа("Чтение", Метаданные.Документы.ЗаказПереработчику2_5)
	...
	//-- Устарело_Переработка24
	ЧтениеЗаказДавальца2_5 = ПравоДоступа("Чтение", Метаданные.Документы.ЗаказДавальца2_5)
							 И ПравоДоступа("Просмотр", Метаданные.Документы.ЗаказДавальца2_5);
	ЧтениеОтчетДавальцу2_5 = ПравоДоступа("Чтение", Метаданные.Документы.ОтчетДавальцу2_5)
							 И ПравоДоступа("Просмотр", Метаданные.Документы.ОтчетДавальцу2_5);
	//-- НЕ УТКА
	...
	Если ДоступностьРаспоряженийТовары Тогда
		...
		КонецЕсли;
		//-- Устарело_Переработка24
		Если ЧтениеЗаказПереработчику2_5 Тогда
			ТипыДокументов.Добавить(Тип("ДокументСсылка.ЗаказПереработчику2_5"));
		КонецЕсли;
		//-- НЕ УТ
	...
	Если ДоступностьРаспоряженийДС Тогда
		...
		//++ Устарело_Переработка24
		Если ЧтениеОтчетДавальцу Тогда
			ТипыДокументов.Добавить(Тип("ДокументСсылка.ОтчетДавальцу"));
		КонецЕсли;
		Если ЧтениеЗаказДавальца Тогда
			ТипыДокументов.Добавить(Тип("ДокументСсылка.ЗаказДавальца"));
		КонецЕсли;
		//-- Устарело_Переработка24
		Если ЧтениеОтчетДавальцу2_5 Тогда
			ТипыДокументов.Добавить(Тип("ДокументСсылка.ОтчетДавальцу2_5"));
		КонецЕсли;
		Если ЧтениеЗаказДавальца2_5 Тогда
			ТипыДокументов.Добавить(Тип("ДокументСсылка.ЗаказДавальца2_5"));
		КонецЕсли;
		//-- НЕ УТКА
	...
	Если ДоступностьРаспоряженийТовары Тогда
		...
		ПараметрыФормы.Вставить("СостояниеРаспоряжения", НСтр("ru = 'Требующие создания доверенности';
																|en = 'Authorization letter is required'"));
		...
		ДелоРодитель.Представление  = НСтр("ru = 'Доверенности на получение ТМЦ';
											|en = 'Authorization letters for inventory receipt'");
		...
		Дело.Представление  = НСтр("ru = 'Основания для оформления (получение товаров)';
									|en = 'Bases for issuing (goods receipt)'");
	...
	Если ДоступностьРаспоряженийДС Тогда
		...
		ДелоРодительПродажи.Представление  = НСтр("ru = 'Доверенности на получение наличных ДС';
													|en = 'Authorization letters for receiving cash'");
		...
		Дело.Представление  = НСтр("ru = 'Основания для оформления (получение наличных ДС)';
									|en = 'Bases for issuing (receiving cash)'");
	...
	ДелоРодительФинансы.Представление  = НСтр("ru = 'Выданные доверенности';
												|en = 'Issued authorization letters'");
	...
	Дело.Представление  = НСтр("ru = 'Доверенности, истекающие сегодня';
								|en = 'Authorization letters expiring today'");
	...
	Дело.Представление  = НСтр("ru = 'Просроченные доверенности';
								|en = 'Overdue authorization letters'");
...
Функция ТекстЗапросаРаспоряженийНаТовары(НетДоверенности = Неопределено) Экспорт
	...
	//-- Устарело_Переработка24
	|			ТИПЗНАЧЕНИЯ(ПлановоеПоступление.Распоряжение) = ТИП(Документ.ЗаказПереработчику2_5) ИЛИ
	//-- НЕ УТ
	|				ЛОЖЬ
	...
	//-- Устарело_Переработка24
	|			ИЛИ ТИПЗНАЧЕНИЯ(ПлановоеПоступление.Распоряжение) = ТИП(Документ.ЗаказПереработчику2_5)
	//-- НЕ УТ
	...
	//-- Устарело_Переработка24
	|			ИЛИ ТИПЗНАЧЕНИЯ(ПлановоеПоступление.Распоряжение) = ТИП(Документ.ЗаказДавальца2_5)
	//-- НЕ УТКА
	...
	Если ТекстЗапросаПлановоеПоступление.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'У пользователя недостаточно прав для исполнения операции над базой данных';
								|en = 'User has insufficient rights to perform operations with the database'");
	КонецЕсли;
...
Функция ТекстЗапросаРаспоряженийНаДС() Экспорт
	...
	//++ Устарело_Переработка24
	ЧтениеЗаказДавальца = ПравоДоступа("Просмотр", Метаданные.Документы.ЗаказДавальца);
	ЧтениеОтчетДавальцу = ПравоДоступа("Просмотр", Метаданные.Документы.ОтчетДавальцу);
	//-- Устарело_Переработка24
	ЧтениеЗаказДавальца2_5 = ПравоДоступа("Просмотр", Метаданные.Документы.ЗаказДавальца2_5);
	ЧтениеОтчетДавальцу2_5 = ПравоДоступа("Просмотр", Метаданные.Документы.ОтчетДавальцу2_5);
	//-- НЕ УТКА
	...
	Если ЧтениеРасчетыСКлиентами Тогда
		...
		// Заказ давальца.
		Если ЧтениеЗаказДавальца Тогда
			ТекстЗапроса = СтрЗаменить(ТекстШаблона, "ЗаказКлиента", Метаданные.Документы.ЗаказДавальца.Имя);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПоступлениеОплатыОтКлиентаПредставление", ТекстХОПредставление);
			ТекстыЗапросов.Добавить(ТекстЗапроса);
		КонецЕсли;
		// Отчет давальцу.
		Если ЧтениеОтчетДавальцу Тогда
			ТекстЗапроса = СтрЗаменить(ТекстШаблона, "ЗаказКлиента", Метаданные.Документы.ОтчетДавальцу.Имя);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПоступлениеОплатыОтКлиентаПредставление", ТекстХОПредставление);
			ТекстыЗапросов.Добавить(ТекстЗапроса);
		КонецЕсли;
		//-- Устарело_Переработка24
		...
		// Заказ давальца (2.5)
		Если ЧтениеЗаказДавальца2_5 Тогда
			ТекстЗапроса = СтрЗаменить(ТекстШаблона, "ЗаказКлиента", Метаданные.Документы.ЗаказДавальца2_5.Имя);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПоступлениеОплатыОтКлиентаПредставление", ТекстХОПредставление);
			ТекстыЗапросов.Добавить(ТекстЗапроса);
		КонецЕсли;
		...
		// Отчет давальцу (2.5)
		Если ЧтениеОтчетДавальцу2_5 Тогда
			ТекстЗапроса = СтрЗаменить(ТекстШаблона, "ЗаказКлиента", Метаданные.Документы.ОтчетДавальцу2_5.Имя);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПоступлениеОплатыОтКлиентаПредставление", ТекстХОПредставление);
			ТекстыЗапросов.Добавить(ТекстЗапроса);
		КонецЕсли;
		//-- НЕ УТКА
...
Процедура РассчитатьСтатусДоверенностей(ПараметрыМетода, АдресХранилища) Экспорт
	...
	ШаблонОшибкиПомеченНаУдаление = НСтр("ru = 'Документ %1 помечен на удаление. Невозможно рассчитать статус';
										|en = 'The %1 document is marked for deletion. Cannot calculate status'");
