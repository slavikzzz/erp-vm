...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ИспользоватьСтатусыАвансовыхОтчетов
		...
		Если Не ПраваПользователяПовтИсп.УтверждениеАвансовыхОтчетов() Тогда
			ТекстОшибки = НСтр("ru = 'У вас отсутствует право утверждения авансовых отчетов.';
								|en = 'You are not authorized to confirm expense reports.'");
	...
	Для ТекИндекс = 0 По ПрочиеРасходы.Количество()-1 Цикл
		...
		АдресОшибки = СтрШаблон(НСтр("ru = 'в строке %1 списка ""Расходы""';
									|en = 'in the %1 line of the ""Expenses"" list'"), ПрочиеРасходы[ТекИндекс].НомерСтроки);
		...
		Для каждого Реквизит Из МассивРеквизитовРучнойПроверки Цикл
			Если Не ЗначениеЗаполнено(ПрочиеРасходы[ТекИндекс][Реквизит])
				...
				ТекстОшибки = СтрШаблон(НСтр("ru = 'Не заполнена колонка ""%1""';
											|en = 'Column ""%1"" is not filled in'"), ТабличнаяЧасть.Реквизиты[Реквизит]);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ТекстОшибки + " " + АдресОшибки,
					ЭтотОбъект,
	...
	Для каждого СтрокаКонвертации Из КонвертацияВалюты Цикл
		Если СтрокаКонвертации.Валюта = СтрокаКонвертации.ВалютаКонвертации Тогда
			...
			АдресОшибки = СтрШаблон(НСтр("ru = 'в строке %1 списка ""Конвертация валюты""';
										|en = 'in the %1 line of the ""Currency conversion"" list'"), СтрокаКонвертации.НомерСтроки);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	НастройкаСчетовУчетаСервер.ПередЗаписью(ЭтотОбъект,
		Документы.АвансовыйОтчет.ПараметрыНастройкиСчетовУчета(Статус));
	//-- НЕ УТ
	...
	Если РезультатЗапроса.Пустой() Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не требуется вводить авансовый отчет на основании документа %1';
				|en = 'It is not required to enter an expense report based on document %1'"),
			ДокументОснование);
...
Процедура ПроверитьДублиУчетаНДСПервичныхДокументов(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Для документа №%1 от %2 указано одновременно несколько вариантов учета НДС';
									|en = 'Several VAT accounting options are specified at the same time for the document No.%1 from %2'"),
		...
		ОбщегоНазначения.СообщитьПользователю(
			СтрШаблон(
				НСтр("ru = 'Уже существует %1 по подотчетному лицу %2 организации %3 и подразделению %4 на дату %5.';
					|en = 'There is already %1 for advance holder %2 of the %3 company and the %4 business unit as of %5.'"),
				Выборка.Ссылка,
				ПодотчетноеЛицо,
				Организация,
				Подразделение,
