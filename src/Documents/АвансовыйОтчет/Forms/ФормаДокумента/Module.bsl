...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ПараметрыНастройки = Документы.АвансовыйОтчет.ПараметрыНастройкиСчетовУчета(Объект.Статус);
	...
	Если Не Объект.ПрочиеРасходы.Количество()
		...
		Если Объект.Мультивалютный Тогда
			Текст = НСтр("ru = 'Сумма израсходованных денежных средств равна 0.
				...
				|en = 'Spent cash amount is 0.
				|Fill in data on the ""Purchases"", the ""Expenses"", the ""Payment to vendors"" or ""Currency conversion"" tab'");
		Иначе
			Текст = НСтр("ru = 'Сумма израсходованных денежных средств равна 0.
				|Необходимо заполнить данные на закладке ""Закупки"", ""Расходы "" или ""Оплата поставщикам""';
				|en = 'Spent cash amount is 0.
				|Fill in data on the ""Purchases"", the ""Expenses"" or the ""Payment to vendors"" tab'");
		КонецЕсли;
...
Процедура ДатаПриИзменении(Элемент)
	...
	Если ЗадаватьВопрос Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Очистить суммы взаиморасчетов?';
										|en = 'Clear AR/AP amounts?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
	КонецЕсли;
...
Процедура ВалютаПриИзменении(Элемент)
	...
	Если СписокДокументовЗакупки.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ru = 'Список документов закупки будет очищен, продолжить ?';
							|en = 'Purchase document list will be cleared. Continue?'");
...
Процедура СписокДокументовВопросЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Если ТекущаяВалюта <> Объект.Валюта
			...
			ТекстВопроса = СтрШаблон(НСтр("ru = 'Пересчитать суммы в документе в валюту %1 ?';
											|en = 'Convert amounts in the document into currency %1?'"), Объект.Валюта);
...
Процедура ВалютаПриИзмененииКлиентЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Текст = СтрШаблон(НСтр("ru = 'Суммы в документе пересчитаны в валюту %1';
								|en = 'Document amounts are converted to currency %1'"), Объект.Валюта);
		ПоказатьОповещениеПользователя(НСтр("ru = 'Суммы пересчитаны';
											|en = 'Amounts are recalculated'"),, Текст, БиблиотекаКартинок.Информация32);
...
Процедура ПрочиеРасходыСпособУчетаНДСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.ЭлектронныйБилет) Тогда
		...
		Элемент.СписокВыбора.Добавить(2, НСтр("ru = 'Принять к вычету по БСО (или иному документу)';
												|en = 'Accept for deduction by accountable forms (or by another document)'"));
	Иначе
		Элемент.СписокВыбора.Добавить(0, НСтр("ru = 'Списать в расходы';
												|en = 'Expense as'"));
		Элемент.СписокВыбора.Добавить(1, НСтр("ru = 'Принять к вычету по СФ';
												|en = 'Accept for deduction'"));
		Элемент.СписокВыбора.Добавить(2, НСтр("ru = 'Принять к вычету по БСО (или иному документу)';
												|en = 'Accept for deduction by accountable forms (or by another document)'"));
	КонецЕсли;
...
Процедура ЗакупкиСоздатьДокументВопрос(ДополнительныеПараметры)
	...
	Если Объект.Ссылка.Пустая() Тогда
		ТекстВопроса = НСтр("ru = 'Авансовый отчет еще не записан.
			...
			|en = 'The expense report is not recorded yet.
			|You can create the purchase document only after recording the expense report.
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ЗаполнитьСуммуПолучено();
	РассчитатьСуммуДокумента(ЭтотОбъект);
...
Процедура РассчитатьСуммуДокумента(Форма)
	...
	ЦелиВыдачи = Новый Соответствие;
	ЦельВыдачи     = ПредопределенноеЗначение("Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка");
	
	Для каждого СтрокаТЧ Из Форма.ТаблицаИтоговПоВалютам Цикл
		
		Если Форма.КонтролироватьВыдачуПодОтчетВРазрезеЦелей Тогда
			ЦельВыдачи = СтрокаТЧ.ЦельВыдачи;
			ЦелиВыдачи.Вставить(ЦельВыдачи);
		КонецЕсли;
		
		Если СтрокаТЧ.Получено <> 0 Тогда
			
			ДобавитьСуммуВИтогиПоВалютам(ИтогиПоВалютам,
			                             СтрокаТЧ.Валюта,
			                             ЦельВыдачи,
			                             СтрокаТЧ.Получено,
			                             "Получено");
		...
		ДобавитьСуммуВИтогиПоВалютам(ИтогиПоВалютам,
		                             СтрокаТЧ.Валюта,
		                             ЦельВыдачи,
		                             СтрокаТЧ.НачальныйОстаток,
		                             "НачальныйОстаток");
		
		ДобавитьСуммуВИтогиПоВалютам(ИтогиПоВалютам,
		                             СтрокаТЧ.Валюта,
		                             ЦельВыдачи,
		                             СтрокаТЧ.НачальныйПерерасход,
		                             "НачальныйПерерасход");
	...
	Для каждого СтрокаТЧ Из Объект.ПрочиеРасходы Цикл
		...
		Если Форма.КонтролироватьВыдачуПодОтчетВРазрезеЦелей Тогда
			ЦельВыдачи = СтрокаТЧ.СтатьяДвиженияДенежныхСредств;
			ЦелиВыдачи.Вставить(ЦельВыдачи);
		КонецЕсли;
		...
		Если СтрокаТЧ.Отменено Тогда
			...
			ДобавитьСуммуВИтогиПоВалютам(ИтогиПоВалютам,
			                             ?(Форма.Объект.Мультивалютный,СтрокаТЧ.Валюта,Форма.Объект.Валюта),
			                             ЦельВыдачи,
			                             СтрокаТЧ.Сумма + ?(Форма.Объект.ЦенаВключаетНДС,0,СтрокаТЧ.СуммаНДС),
			                             "Отклонено");
		
		Иначе
			
			ДобавитьСуммуВИтогиПоВалютам(ИтогиПоВалютам,
			                             ?(Форма.Объект.Мультивалютный,
			                             СтрокаТЧ.Валюта,Форма.Объект.Валюта),
			                             ЦельВыдачи,
			                             СтрокаТЧ.Сумма + ?(Форма.Объект.ЦенаВключаетНДС,0,СтрокаТЧ.СуммаНДС),
			                             "Израсходовано");
	...
	Для каждого СтрокаТЧ Из Объект.КонвертацияВалюты Цикл
		...
		ДобавитьСуммуВИтогиПоВалютам(ИтогиПоВалютам,
		                             СтрокаТЧ.ВалютаКонвертации,
		                             ЦельВыдачи,
		                             СтрокаТЧ.СуммаКонвертации,
		                             "Конвертировано");
		
	КонецЦикла;
	
	Форма.ВыводитьЦелиВыдачи = ЦелиВыдачи.Количество() > 1;
	УдаляемыеСтрокиВалют = Новый Массив;
	
	Для каждого КлючИЗначениеВалюта Из ИтогиПоВалютам Цикл
		
		УдаляемыеСтроки = Новый Массив;
		
		Для Каждого КлючИЗначение Из КлючИЗначениеВалюта.Значение Цикл
		...
		Для каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
			КлючИЗначениеВалюта.Значение.Удалить(УдаляемаяСтрока);
		КонецЦикла;
		
		Если Не КлючИЗначениеВалюта.Значение.Количество() Тогда
			УдаляемыеСтрокиВалют.Добавить(КлючИЗначениеВалюта.Ключ);
		КонецЕсли;
		
	КонецЦикла;
	
	Для каждого УдаляемаяСтрока Из УдаляемыеСтрокиВалют Цикл
	...
	Для Каждого КлючИЗначениеВалюта Из ИтогиПоВалютам Цикл
		...
		Для Каждого КлючИЗначение Из КлючИЗначениеВалюта.Значение Цикл
			...
			НоваяСтрока.Валюта     = КлючИЗначениеВалюта.Ключ;
			НоваяСтрока.ЦельВыдачи = КлючИЗначение.Ключ;
		
			Если Форма.ВыводитьЦелиВыдачи Тогда
				КлючИЗначение.Значение.НачальныйОстаток = КлючИЗначение.Значение.НачальныйОстаток - КлючИЗначение.Значение.НачальныйПерерасход;
				КлючИЗначение.Значение.НачальныйПерерасход = 0;
				КлючИЗначение.Значение.Остаток = КлючИЗначение.Значение.Остаток - КлючИЗначение.Значение.Перерасход;
				КлючИЗначение.Значение.Перерасход = 0;
			КонецЕсли;
	...
	Форма.МультивалютныйПодвал = ИтогиПоВалютам.Количество() > 1
	                             Или Объект.Мультивалютный
	                             Или Форма.ВыводитьЦелиВыдачи;
	...
	Иначе
		...
		Форма.Элементы.ТаблицаИтоговПоВалютамНачальныйПерерасход.Видимость = Не Форма.ВыводитьЦелиВыдачи;
		Форма.Элементы.ТаблицаИтоговПоВалютамПерерасход.Видимость          = Не Форма.ВыводитьЦелиВыдачи;
		Форма.Элементы.ТаблицаИтоговПоВалютамЦельВыдачи.Видимость          = Форма.ВыводитьЦелиВыдачи;
		Форма.Элементы.ТаблицаИтоговПоВалютамНачальныйОстаток.Заголовок = ?(Форма.ВыводитьЦелиВыдачи,
		                                                                НСтр("ru = 'Начальный остаток / перерасход';
																			|en = 'Начальный остаток / перерасход'"),
		                                                                НСтр("ru = 'Начальный остаток';
																			|en = 'Opening balance'"));
		Форма.Элементы.ТаблицаИтоговПоВалютамОстаток.Заголовок          = ?(Форма.ВыводитьЦелиВыдачи,
		                                                                НСтр("ru = 'Остаток / перерасход';
																			|en = 'Остаток / перерасход'"),
		                                                                НСтр("ru = 'Остаток';
																			|en = 'Balance'"));
...
Процедура ЗаполнитьСуммуПолучено()
	...
	МассивИсключаемыхКолонок.Добавить("ЦельВыдачи");
	...
	Пока ВыборкаПолучено.Следующий() Цикл
		...
		СтрокаИтогПоВалюте.ЦельВыдачи = ВыборкаПолучено.ЦельВыдачи;
	КонецЦикла;
	
	Таблица.Свернуть("Валюта, ЦельВыдачи, Курс, Растяжка",
	                 "НачальныйОстаток, НачальныйПерерасход, Получено, Возвращено, Израсходовано, Перерасход, Отклонено, Остаток, Конвертировано, СуммаЗакупок");
...
Процедура ОбновитьДокументыЗакупки()
	...
	|	Данные.СтатьяДвиженияДенежныхСредств КАК ЦельВыдачи
	|ИЗ 
	|	&ДокументыЗакупки КАК Данные
	|СГРУППИРОВАТЬ ПО
	|	Данные.Валюта,
	|	Данные.СтатьяДвиженияДенежныхСредств";
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ДокументыЗакупки", СтрШаблон("(%1)", ДокументыЗакупки.ТекстЗапроса));
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаИтогПоВалюте.ЦельВыдачи   = Выборка.ЦельВыдачи;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьСуммуВИтогиПоВалютам(ИтогиПоВалютам, Валюта, ЦельВыдачи, Сумма, Колонка)
	
	ИтогПоВалюте = ИтогиПоВалютам.Получить(Валюта);
	Если ИтогПоВалюте = Неопределено Тогда
		ИтогПовалюте = ИнициализироватьИтогПоВалюте(ЦельВыдачи, Сумма, Колонка);
		ИтогиПоВалютам.Вставить(Валюта, ИтогПовалюте);
	КонецЕсли;

	ИтогПоЦели = ИтогПоВалюте.Получить(ЦельВыдачи);
	Если ИтогПоЦели = Неопределено Тогда
		ИтогПоЦели = ИнициализироватьИтогПоцели();
		ИтогПоВалюте.Вставить(ЦельВыдачи, ИтогПоЦели);
	КонецЕсли;
	
	ИсходнаяСумма = 0;
	ИтогПоЦели.Свойство(Колонка, ИсходнаяСумма);
	ИтогПоЦели.Вставить(Колонка, ИсходнаяСумма + Сумма);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ИнициализироватьИтогПоВалюте(ЦельВыдачи, Сумма, Колонка)
	
	ИтогПоВалюте = Новый Соответствие;
...
&НаКлиентеНаСервереБезКонтекста
Функция ИнициализироватьИтогПоцели()

	ИтогПоЦели = Новый Структура;
	ИтогПоЦели.Вставить("НачальныйОстаток", 0);
	ИтогПоЦели.Вставить("НачальныйПерерасход", 0);
	ИтогПоЦели.Вставить("Получено", 0);
	ИтогПоЦели.Вставить("Возвращено", 0);
	ИтогПоЦели.Вставить("Израсходовано", 0);
	ИтогПоЦели.Вставить("Конвертировано", 0);
	ИтогПоЦели.Вставить("Отклонено", 0);
	ИтогПоЦели.Вставить("Остаток", 0);
	ИтогПоЦели.Вставить("Перерасход", 0);
	ИтогПоЦели.Вставить("СуммаЗакупок", 0);
		...
		Если ЗначениеЗаполнено(Объект.Подразделение) Тогда
			Текст = СтрШаблон(
				НСтр("ru = 'У подотчетного лица отсутствуют выданные авансы, за которые он не отчитался перед организацией %1 в подразделении %2 в валюте %3';
					|en = 'The advance holder has no advances received for which he or she has not reported to the %1 company in the %2 business unit in %3 currency'"),
		...
		Иначе
			Текст = СтрШаблон(
				НСтр("ru = 'У подотчетного лица отсутствуют выданные авансы, за которые он не отчитался перед организацией %1 в валюте %2 без указания подразделения';
					|en = 'The advance holder has no advances received for which he or she has not reported to the %1 company in %2 currency without specifying a business unit'"),
	...
	Если Объект.КонвертацияВалюты.Количество() = 0 Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'У подотчетного лица отсутствуют денежные средства от организации %1';
				|en = 'Advance holder has no %1 company cash'"),
			Объект.Организация);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
	КонецЕсли;
...
Функция СтрокаМультивалютныеСуммы(ИтогиПоВалютам)
	...
	Для Каждого КлючИЗначениеВалюты Из ИтогиПоВалютам Цикл
		
		ИтогоОстаток = 0;
		ИтогоПерерасход = 0;
		Валюта = Строка(КлючИЗначениеВалюты.Ключ);
		
		Для Каждого КлючИЗначение Из КлючИЗначениеВалюты.Значение Цикл
			
			СтрокаИтогов = КлючИЗначение.Значение;
			Если СтрокаИтогов.Остаток <> 0 Тогда
	
				Если СтрокаИтогов.Остаток < 0 Тогда
					ИтогоПерерасход = ИтогоПерерасход + -1 * СтрокаИтогов.Остаток;
				Иначе
					ИтогоОстаток = ИтогоОстаток + СтрокаИтогов.Остаток;
				КонецЕсли;
	
			КонецЕсли;
	
			Если СтрокаИтогов.Перерасход <> 0 Тогда
				ИтогоПерерасход = ИтогоПерерасход + СтрокаИтогов.Перерасход;
			КонецЕсли;
			
		КонецЦикла;
	
		Если ИтогоОстаток <> 0 Тогда
			СтрокаОстаток.Добавить(Строка(ИтогоОстаток) + " " + Валюта + " ");
		КонецЕсли;
		Если ИтогоПерерасход <> 0 Тогда
			СтрокаПерерасход.Добавить(Строка(ИтогоПерерасход) + " " + Валюта + " ");
		КонецЕсли;
...
Процедура Подключаемый_ПриСозданииНаСервереЛокализация(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ВидимостьСозданияПоступленияДД = ПравоДоступа("Добавление", Метаданные.Документы.ПоступлениеДенежныхДокументов);
	Элементы.СписокДокументовЗакупкиЗакупкиСоздатьПДД.Видимость = ВидимостьСозданияПоступленияДД;
	
	Если ВидимостьСозданияПоступленияДД Тогда
		КнопокСоздания = КнопокСоздания + 1;
	КонецЕсли;
	
	Если КнопокСоздания = 1 И ВидимостьСозданияПоступленияДД Тогда
		
		ЭлементСоздания = Элементы.СписокДокументовЗакупкиЗакупкиСоздатьПДД;
		ЭлементСоздания.Заголовок = НСтр("ru = 'Создать';
										|en = 'Create'");
		Элементы.Переместить(ЭлементСоздания, Элементы.ДокументыЗакупки.КоманднаяПанель, Элементы.СписокДокументовЗакупкиГруппаСоздать);
		
	ИначеЕсли КнопокСоздания > 1 Тогда
		
		Элементы.СписокДокументовЗакупкиЗакупкиСоздатьПТиУ.Заголовок = НСтр("ru = 'Приобретение товаров и услуг';
																			|en = 'Vendor invoice'");
		Элементы.СписокДокументовЗакупкиЗакупкиСоздатьЭлектронныйБилет.Заголовок = НСтр("ru = 'Электронный билет';
																						|en = 'Electronic ticket'");
	КонецЕсли;
	
	Если КнопокСоздания > 0 Тогда
		Элементы.СписокДокументовЗакупкиЗакупкиПодобрать.Видимость = Истина;
		Элементы.СписокДокументовЗакупкиЗакупкиУдалить.Видимость = Истина;
	КонецЕсли;
	//-- НЕ УТ
...
Процедура ПриЧтенииСозданииНаСервереЛокализация()
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ДанныеДокументаПереопределяемый.НомерВходящегоДокумента = """" ТОГДА
	|			ДанныеДокумента.НомерВходящегоДокумента
	|		ИНАЧЕ
	|			ДанныеДокументаПереопределяемый.НомерВходящегоДокумента
	|	КОНЕЦ КАК НомерВходящегоДокумента,
	|	ВЫБОР
	|		КОГДА ДанныеДокументаПереопределяемый.ДатаВходящегоДокумента = ДАТАВРЕМЯ(1,1,1) ТОГДА
	|			ДанныеДокумента.ДатаВходящегоДокумента
	|		ИНАЧЕ
	|			ДанныеДокументаПереопределяемый.ДатаВходящегоДокумента
	|	КОНЕЦ КАК ДатаВходящегоДокумента,
	|	ВЫБОР
	|		КОГДА ДанныеДокументаПереопределяемый.НаименованиеВходящегоДокумента = """" ТОГДА
	|			ДанныеДокумента.НаименованиеВходящегоДокумента
	|		ИНАЧЕ
	|			ДанныеДокументаПереопределяемый.НаименованиеВходящегоДокумента
	|	КОНЕЦ КАК НаименованиеВходящегоДокумента,
	|	ДанныеДокумента.Валюта                           КАК Валюта,
	|	ДанныеДокумента.СтатьяДвиженияДенежныхСредств    КАК СтатьяДвиженияДенежныхСредств,
	|	ДанныеДокумента.Ссылка                                  КАК ДокументЗакупки,
	|	ДанныеДокумента.Контрагент                       КАК Контрагент,
	|	СУММА(ДанныеДокументаПереопределяемый.СуммаСНДС)                        КАК Сумма
	|ИЗ
	|	Документ.ПриобретениеТоваровУслуг.Товары КАК ДанныеДокументаПереопределяемый
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриобретениеТоваровУслуг КАК ДанныеДокумента
	|		ПО ДанныеДокументаПереопределяемый.Ссылка = ДанныеДокумента.Ссылка
	|ГДЕ
	...
	|	""""                                                    КАК НаименованиеВходящегоДокумента,
	|	ДанныеДокумента.Валюта                                  КАК Валюта,
	...
	|ИЗ
	|	Документ.ПоступлениеДенежныхДокументов.ДенежныеДокументы КАК ДанныеДокумента
	|	
	|ГДЕ
	|	ДанныеДокумента.Ссылка В (&СписокДокументовЗакупки)
	...
	|
	|ВЫБРАТЬ
	|	ДанныеДокумента.НомерБилета                             КАК НомерВходящегоДокумента,
	|	ДанныеДокумента.Дата                                    КАК ДатаВходящегоДокумента,
	|	ПРЕДСТАВЛЕНИЕ(ДанныеДокумента.ТипУслуги)                КАК НаименованиеВходящегоДокумента,
	|	ДанныеДокумента.Валюта                                  КАК Валюта,
	|	ДанныеДокумента.СтатьяДвиженияДенежныхСредств           КАК СтатьяДвиженияДенежныхСредств,
	|	ДанныеДокумента.Ссылка                                  КАК ДокументЗакупки,
	...
	ОбщегоНазначения.УстановитьСвойстваДинамическогоСписка(Элементы.ДокументыЗакупки, СвойстваСписка);
	//-- НЕ УТ
