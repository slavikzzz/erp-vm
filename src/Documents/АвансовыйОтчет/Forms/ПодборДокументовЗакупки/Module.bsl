...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока.ПредставлениеДокументаЗакупки = СтрШаблон(" %1 " + НСтр("ru = 'от %2';
																			|en = 'dated %2'"), Выборка.Номер, Формат(Выборка.Дата, "ДЛФ=D"));
	КонецЦикла;
...
Функция ВыборкаДокументовЗакупкиЛокализация(Параметры)
	...
	|	ПО ДокументыЗакупки.Ссылка = ДанныеДокумента.Ссылка
	|ГДЕ
	|	ДанныеДокумента.Валюта = &Валюта ИЛИ &Мультивалютный
	//-- НЕ УТ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	ДанныеДокумента.АвансовыйОтчет = &СсылкаНаДокумент
	|	И &СсылкаЗаполнена
	|	И (ДанныеДокумента.Валюта = &Валюта ИЛИ &Мультивалютный)
	//-- НЕ УТ
	|";
