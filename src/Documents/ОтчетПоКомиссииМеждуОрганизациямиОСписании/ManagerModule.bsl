...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат ОтчетПоКомиссииМеждуОрганизациямиОСписанииЛокализация.ТекстОтраженияВРеглУчете();
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	Возврат ОтчетПоКомиссииМеждуОрганизациямиОСписанииЛокализация.ТекстЗапросаВТОтраженияВРеглУчете();
...
Функция ПолучитьДанныеДляПечатнойФормыОтчетПоКомиссииОСписании(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Заголовок 					= НСтр("ru = 'Отчет комитенту о списании';
										|en = 'Vendor consignment stock adjustment'", ОбщегоНазначения.КодОсновногоЯзыка());
	МассивРезультатов 			= Запрос.ВыполнитьПакет();
	РезультатПоШапке			= МассивРезультатов[0];
	РезультатПоТабличнойЧасти 	= МассивРезультатов[1];
	СтруктураДанныхДляПечати 	= Новый Структура(
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	ИнформацияПоКомитенту = НСтр("ru = 'Отчет от ""%Комитент%""';
								|en = 'Report from ""%Комитент%""'", ОбщегоНазначения.КодОсновногоЯзыка());
	ИнформацияПоКомитенту = СтрЗаменить(ИнформацияПоКомитенту, "%Комитент%", Реквизиты.Комитент);
	
	ИнформацияПоКомиссионеру = НСтр("ru = 'Отчет перед ""%Комиссионер%""';
									|en = 'Report for ""%Комиссионер%""'", ОбщегоНазначения.КодОсновногоЯзыка());
