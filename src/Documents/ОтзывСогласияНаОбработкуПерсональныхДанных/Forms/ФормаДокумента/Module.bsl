...
Процедура УстановитьСведенияДействующегоСогласия()
	...
	Если ЗначениеЗаполнено(Объект.Субъект) И Объект.Субъект.ЭтоГруппа Тогда
		ВызватьИсключение НСтр("ru = 'Для отзыва согласия на обработку персональных данных выберите элемент, а не группу.';
								|en = 'To withdraw the consent to personal data processing, select an item, not a group.'");
	КонецЕсли;	
	...
	Если Согласие = Неопределено Тогда
		...
		Элементы.СведенияТекст.Заголовок = НСтр("ru = 'Не обнаружено действующего согласия на обработку персональных данных субъекта.';
												|en = 'Valid consent to personal data processing of the subject is not found.'");
		Возврат;
	КонецЕсли;
