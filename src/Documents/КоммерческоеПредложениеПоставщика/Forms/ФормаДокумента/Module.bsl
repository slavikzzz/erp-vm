...
Процедура ОткрытьДопИнформациюПоставщика()
	...
	ПараметрыФормы.Вставить("Заголовок",         НСтр("ru = 'Дополнительная информация поставщика';
														|en = 'Additional supplier''s information'"));
...
Процедура СформироватьНадписьПрисоединенныеФайлы()
	...
	ПредставлениеПрисоединенныхФайлов = НСтр("ru = 'Файлы, полученные от поставщика';
											|en = 'Files received from the vendor'");
...
Процедура СформироватьИнформационнуюСтрокуПоОхватуШапка(СтрокиОхваченныеПолностью, СтрокиОхваченныеЧастично, КоличествоРассматриваемыхСтрок)
	...
	Если КоличествоСтрокВЗапросе > КоличествоСтрокЗапросаСПредложениями Тогда
		...
		ТекстПодстроки   = НСтр("ru = 'охватывает не все позиции запроса';
								|en = 'does not include all query items'");
	...
	Если СтрокиОхваченныеЧастично > 0 Тогда
		...
		ТекстПодстроки   = НСтр("ru = 'потребности закрываются не полностью';
								|en = 'needs are not fully covered'");
	...
	Если Объект.СрокДействия < ДатаОкончанияРассмотренияЗапроса Тогда
		...
		МассивСтрок.Добавить(НСтр("ru = 'действия предложения меньше запрошенного';
									|en = 'of quotation validity is less than requested'"));
...
Процедура СформироватьИнформационнуюСтрокуПоОхватуПодвал(СтрокиОхваченныеПолностью, СтрокиОхваченныеЧастично, КоличествоРассматриваемыхСтрок)
	...
	Если КоличествоСтрокВЗапросе > КоличествоСтрокЗапросаСПредложениями Тогда
		...
		ТекстЦифрыОхвата = СтрШаблон(НСтр("ru = '%1 из %2 (%3 %4)';
											|en = '%1 from %2 (%3 %4)'"), КоличествоСтрокЗапросаСПредложениями, КоличествоСтрокВЗапросе, ПроцентОхвата, "%");
		
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Охват';
																|en = 'Scope'"), Новый Шрифт(,,Истина)));
		МассивСтрок.Добавить(" ");
	...
	Иначе
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Все позиции запроса охвачены';
																|en = 'All query items are included'"), Новый Шрифт(,,Истина)));
	КонецЕсли;
	...
	Иначе
		...
		Если СтрокиОхваченныеПолностью > 0 Тогда
			ТекстПодстроки = СтрШаблон(НСтр("ru = 'полностью - %1';
											|en = 'fully - %1'"), СтрокиОхваченныеПолностью);
		...
		Если СтрокиОхваченныеЧастично > 0 Тогда
			Если СтрокиОхваченныеПолностью > 0 Тогда
				МассивСтрок.Добавить(", ");
			КонецЕсли;
			ТекстПодстроки = СтрШаблон(НСтр("ru = 'частично - %1';
											|en = 'partially - %1'"), СтрокиОхваченныеЧастично);
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ТекстПодстроки, Новый Шрифт(,,Истина)));
		КонецЕсли;
	...
	Иначе
		...
		МассивСтрок.Добавить(НСтр("ru = 'Отмечено';
									|en = 'Marked'"));
		МассивСтрок.Добавить(" ");
		ТекстПодстроки = СтрШаблон(НСтр("ru = 'к рассмотрению - %1 из %2';
										|en = 'for review - %1 of %2'"), КоличествоРассматриваемыхСтрок, КоличествоСтрокЗапросаСПредложениями);
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ТекстПодстроки, Новый Шрифт(,,Истина)));
	...
	Иначе
		...
		Если СнятьСРассмотрения Тогда
			...
			ТекстЗаголовка  = НСтр("ru = 'Снятие с рассмотрения.';
									|en = 'Stopping being reviewed.'");
			ТекстОповещения = СтрШаблон(НСтр("ru = 'Снято с рассмотрения позиций: %1 из %2.';
											|en = 'Items stopped being reviewed: %1 of %2.'"),КоличествоИзмененныхСтрок, ВсегоСтрок);
			
		Иначе
			
			ТекстЗаголовка  = НСтр("ru = 'Принятие к рассмотрению';
									|en = 'Taking to review'");
			ТекстОповещения = СтрШаблон(НСтр("ru = 'Принято к рассмотрению позиций: %1 из %2.';
											|en = 'Items taken to review: %1 of %2.'"),КоличествоИзмененныхСтрок, ВсегоСтрок);
