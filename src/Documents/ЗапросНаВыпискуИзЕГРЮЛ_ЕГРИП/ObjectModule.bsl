	...
	ИначеЕсли ДлинаСтрокиИНН_ОГРН = 10 ИЛИ ДлинаСтрокиИНН_ОГРН = 12 Тогда
		...
		Если Не РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(ПараметрЗапроса, ?(ДлинаСтрокиИНН_ОГРН = 10, Истина, Ложь), ТекстСообщения) Тогда
			ТекстСообщения = ТекстСообщения + " " + НСтр("ru = 'Данный ИНН не существует.';
														|en = 'Данный ИНН не существует.'");
			ОбщегоНазначения.СообщитьПользователю(
				ТекстСообщения,, "ПредставлениеЗапроса", "", Отказ);
		КонецЕсли;
	Иначе
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Укажите верные параметры поиска ИНН или ОГРН.';
				|en = 'Укажите верные параметры поиска ИНН или ОГРН.'"),, "ПредставлениеЗапроса", "", Отказ);
	КонецЕсли;
