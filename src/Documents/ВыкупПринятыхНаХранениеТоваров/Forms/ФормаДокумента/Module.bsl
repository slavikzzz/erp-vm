...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЕстьПродукцияВЕТИС
		...
		|en = 'Different storage locations are specified for an item of the ""Products under VETIS control"" type.
		|For an item of this type, the same storage location must be specified in all rows of table ""Goods"".'");
...
Процедура ПриИзмененииПартнераСервер()
	...
	Если Договор <> Объект.Договор Тогда
		...
		//++ НЕ УТКА
		ОчиститьНазначениеВТаблицеТовары();
		ЗаполнитьНазначениеПоУмолчанию();
		//-- НЕ УТКА
	КонецЕсли;
...
Процедура ДоговорПриИзмененииСервер()
	...
	//++ НЕ УТКА
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыкупТоваровДавальца Тогда
		
		Если ЗначениеЗаполнено(Объект.Договор) Тогда
			ЗаполнитьЗначенияСвойств(
				Объект,
				ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Договор, "Подразделение"));
		КонецЕсли;
...
Процедура ПриИзмененииХозяйственнойОперацииСервер()
	...
	Если Операции.Найти(Перечисления.ХозяйственныеОперации.ВыкупТоваровДавальца) <> Неопределено
...
Процедура НастроитьФорму()
	...
	//++ НЕ УТКА
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыкупТоваровДавальца Тогда
		
		АвтоЗаголовок = Ложь;
		
		ПредставлениеОбъекта = ДавальческаяСхемаКлиентСервер.СинонимДокументаВыкупПринятыхНаХранениеТоваров();
		...
		Элементы.ТоварыПодобратьТоварыПринятыеНаОтветственноеХранение.Заголовок =
			НСтр("ru = 'Подобрать товары давальца';
...
Процедура НастроитьПараметрыВыбораЭлементов()
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыкупТоваровДавальца Тогда
		...
		// Группа фин. учета расчетов
		ПараметрыВыбораГФУ.Добавить(Новый ПараметрВыбора("Отбор.РасчетыСДавальцами", Истина));
...
Функция ХозяйственнаяОперацияДоговора(ХозяйственнаяОперацияДокумента)
	...
	//++ НЕ УТКА
	ИначеЕсли ХозяйственнаяОперацияДокумента =
						ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыкупТоваровДавальца") Тогда
		
		ХозОперация = ДавальческаяСхемаКлиентСервер.ХозяйственнаяОперацияДоговора();
...
&НаСервере
Процедура ЗаполнитьНазначениеПоУмолчанию()
	
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыкупТоваровДавальца Тогда
		
		НазначениеПоУмолчанию = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "Назначение");
		
	ИначеЕсли ЗначениеЗаполнено(НазначениеПоУмолчанию) Тогда
...
&НаСервере
Процедура ОчиститьНазначениеВТаблицеТовары()
	
	НайденныеСтроки = Объект.Товары.НайтиСтроки(Новый Структура("Назначение", Справочники.Назначения.ПустаяСсылка()));
	Если Объект.Товары.Количество() > НайденныеСтроки.Количество() Тогда
		
		Для каждого СтрокаТаблицы Из Объект.Товары Цикл
			
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.Назначение) Тогда
				Продолжить;
			КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Выкуп товаров, хранящихся на складе';
																|en = 'Purchase invoice — VMI-in-stock'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Выкуп товаров, переданных в производство';
																|en = 'Purchase invoice — VMI-in-WIP'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Выкуп товаров, переданных на хранение';
																|en = 'Purchase invoice — VMI-in-chain'"));
	...
	// Установка условного оформления для элемента 'ТоварыНазначение' табличной части 'Товары'
	Элемент = УсловноеОформление.Элементы.Добавить();
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", Новый ПолеКомпоновкиДанных("НазначениеПоУмолчанию"));
	//-- НЕ УТКА
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья2_5") Тогда
		ХозяйственнаяОперацияСписокВыбора.Добавить(Перечисления.ХозяйственныеОперации.ВыкупТоваровДавальца);
	КонецЕсли;
	//-- НЕ УТКА
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОказаниеАгентскихУслугПриЗакупке") Тогда
		ХозяйственнаяОперацияСписокВыбора.Добавить(Перечисления.ХозяйственныеОперации.ПоставкаПодПринципала, НСтр("ru = 'Выкуп товаров принципала';
																													|en = 'Redemption of principal''s goods'"));
	КонецЕсли;
