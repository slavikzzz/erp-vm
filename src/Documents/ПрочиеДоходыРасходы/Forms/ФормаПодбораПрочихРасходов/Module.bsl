...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВедетсяУчетПостоянныхИВременныхРазниц =
		НастройкиНалоговУчетныхПолитикЛокализация.ВедетсяУчетПостоянныхИВременныхРазниц(Параметры.Организация, ДатаДокумента);
	Элементы.ТаблицаРасходовПостояннаяРазница.Видимость = ВедетсяУчетПостоянныхИВременныхРазниц;
	Элементы.ТаблицаРасходовВременнаяРазница.Видимость  = ВедетсяУчетПостоянныхИВременныхРазниц;
	//-- НЕ УТ
	...
	Элементы.ТаблицаРасходовСумма.Заголовок =
		СтрШаблон(НСтр("ru = 'Упр. учет с НДС (%1)';
						|en = 'Manag. accounting including VAT (%1)'"), ВалютаУправленческогоУчета);
	Элементы.ТаблицаРасходовСуммаБезНДС.Заголовок =
		СтрШаблон(НСтр("ru = 'Упр. учет без НДС (%1)';
						|en = 'Management accounting excluding VAT (%1)'"), ВалютаУправленческогоУчета);
	Если ВестиУправленческийУчетОрганизаций Тогда
		Элементы.ТаблицаРасходовСуммаУпр.Заголовок =
			СтрШаблон(НСтр("ru = 'Международный фин. учет (%1)';
							|en = 'Financial accounting (%1)'"), ВалютаУправленческогоУчета);
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗавершениеРаботы
		...
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Некоторые строки были выбраны. Добавить их в документ?';
										|en = 'You have selected several lines. Add them to the document?'"), Режим);
