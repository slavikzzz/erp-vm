...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ПараметрыНастройки = Документы.ПрочиеДоходыРасходы.ПараметрыНастройкиСчетовУчета(Объект.ХозяйственнаяОперация);
...
Процедура РасходыПредставлениеОтраженияВРеглУчетеБезРазбиенияНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыОповещения = Новый Структура("ИмяТЧ", "ПрочиеРасходы");
	ОписаниеОповещения = Новый ОписаниеОповещения("ПредставлениеОтраженияВРеглУчетеБезРазбиенияПослеНажатия", ЭтотОбъект, ПараметрыОповещения);
	
	НастройкаСчетовУчетаКлиент.ПриНажатии(ЭтотОбъект, Элемент, СтандартнаяОбработка, ОписаниеОповещения);
...
Процедура ДоходыПредставлениеОтраженияВРеглУчетеНажатие(Элемент, СтандартнаяОбработка)
	//++ НЕ УТ
	НастройкаСчетовУчетаКлиент.ПриНажатии(ЭтаФорма, Элемент, СтандартнаяОбработка);
	//-- НЕ УТ
	Возврат; // В УТ обработчик пустой
КонецПроцедуры
...
Процедура ДоходыПредставлениеОтраженияВРеглУчетеБезРазбиенияНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыОповещения = Новый Структура("ИмяТЧ", "ПрочиеДоходы");
	ОписаниеОповещения = Новый ОписаниеОповещения("ПредставлениеОтраженияВРеглУчетеБезРазбиенияПослеНажатия", ЭтотОбъект, ПараметрыОповещения);
...
Процедура ОтобразитьСуммыПРИВР(Команда)
	...
	УправлениеФормойСервер();
	Элементы.ФормаОтобразитьСуммыПРИВР.Пометка = ВыводитьСуммыРазницДляПлательщикаНалогаНаПрибыль;
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ПрочиеРасходыКорСтатьяРасходов.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПрочиеРасходы.КорСтатьяРасходов");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ХозяйственнаяОперация");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Перечисления.ХозяйственныеОперации.ПередачаПрочихРасходовМеждуФилиалами;
	
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст",					НСтр("ru = '<Совпадает со статьей расходов>';
																					|en = '<Matches the expense item>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста",			ЦветаСтиля.ЦветТекстаПодсказкиВвода);
			...
			Если ПереключательРасшифровкиДоходыКорЧасть = 0 Тогда
				...
				Если ИсходнаяСтрока <> Неопределено Тогда
					ЗаполнитьЗначенияСвойств(СтрокаТаблицы, ИсходнаяСтрока,
						...
						//++ НЕ УТ
						НастройкаСчетовУчетаКлиент.ПриКопированииСтроки(ЭтотОбъект, Элементы[ИмяТЧ]);
						//-- НЕ УТ
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура УправлениеФормойСервер()
	...
	ПоляРазниц.Вставить(Элементы.ПрочиеДоходыВременнаяРазница,   "ВР");
	ПоляРазниц.Вставить(Элементы.ДоходыВременнаяРазница,         "ВР");
	ПоляРазниц.Вставить(Элементы.ПрочиеРасходыВременнаяРазница,  "ВР");
	ПоляРазниц.Вставить(Элементы.РасходыВременнаяРазница,        "ВР");
	ПоляРазниц.Вставить(Элементы.ПрочиеДоходыПостояннаяРазница,  "ПР");
	ПоляРазниц.Вставить(Элементы.ДоходыПостояннаяРазница,        "ПР");
	ПоляРазниц.Вставить(Элементы.ПрочиеРасходыПостояннаяРазница, "ПР");
	ПоляРазниц.Вставить(Элементы.РасходыПостояннаяРазница,       "ПР");
	РеглУчетСервер.НастроитьВидимостьПолейНалоговогоУчетаИРазниц(
		ПоляРазниц, Объект.Дата, Объект.Организация, "", ВыводитьСуммыРазницДляПлательщикаНалогаНаПрибыль);
	//-- НЕ УТ
	...
	РасходыСуммаЗаголовокПолный = СтрШаблон(НСтр("ru = 'Суммы в упр. учете (%1):';
												|en = 'Amounts in manag. accounting (%1):'"), ВалютаУправленческогоУчета);
	Элементы.РасходыСумма.Заголовок = ?(ИспользоватьУчетНДС, НСтр("ru = 'С НДС';
																	|en = 'Including VAT'"), РасходыСуммаЗаголовокПолный);
	
	Элементы.ДекорацияРасходыСуммыУпр.Заголовок =
		СтрШаблон(НСтр("ru = 'Суммы расходов в управленческим учете (%1):';
						|en = 'Expense amounts in management accounting (%1):'"), ВалютаУпр);
	Элементы.ДекорацияРасходыСуммыУпр.Видимость = ИспользоватьУчетНДС;
	
	Элементы.ДекорацияРасходыСуммыРегл.Заголовок =
		СтрШаблон(НСтр("ru = 'Суммы расходов в регламентированном учете (%1):';
						|en = 'Expense amounts in local accounting (%1):'"), ВалютаРегламентированногоУчета);
		
	ПрочиеРасходыСуммаШаблон = ?(ИспользоватьУчетНДС, НСтр("ru = 'Упр. учет с НДС (%1)';
															|en = 'Management accounting including VAT (%1)'"), НСтр("ru = 'Упр. учет (%1)';
																								|en = 'Management accounting (%1)'"));
	...
	Элементы.ПрочиеРасходыСуммаБезНДС.Заголовок =
		СтрШаблон(НСтр("ru = 'Упр. учет без НДС (%1)';
						|en = 'Management accounting excluding VAT (%1)'"), ВалютаУпр);
	Если ВестиУправленческийУчетОрганизаций Тогда
		Элементы.ПрочиеРасходыСуммаУпр.Заголовок =
			СтрШаблон(НСтр("ru = 'Международный фин. учет (%1)';
							|en = 'Financial accounting (%1)'"), ВалютаУпр);
	КонецЕсли;
	Элементы.ПрочиеРасходыСуммаРегл.Заголовок =
		СтрШаблон(НСтр("ru = 'Бухгалтерский учет (%1):';
						|en = 'Accounting (%1):'"), ВалютаРегламентированногоУчета);
	Элементы.ПрочиеРасходыСуммаНУ.Заголовок =
		СтрШаблон(НСтр("ru = 'Налоговый учет (%1):';
						|en = 'Tax accounting (%1):'"), ВалютаРегламентированногоУчета);
	Элементы.ПрочиеРасходыПостояннаяРазница.Заголовок =
		СтрШаблон(НСтр("ru = 'Постоянная разница (%1):';
						|en = 'Permanent difference (%1):'"), ВалютаРегламентированногоУчета);
	Элементы.ПрочиеРасходыВременнаяРазница.Заголовок =
		СтрШаблон(НСтр("ru = 'Временная разница (%1):';
						|en = 'Temporary difference (%1):'"), ВалютаРегламентированногоУчета);
	ВедетсяУчетПоНДД = ВедетсяУчетПоНДД(Объект.Дата, Объект.Организация);
	Элементы.ПрочиеРасходыСуммаНДД.Заголовок =
		СтрШаблон(НСтр("ru = 'НДД (%1):';
						|en = 'AIT (%1):'"), ВалютаРегламентированногоУчета);
	...
	Элементы.ДекорацияДоходыСуммыУпр.Заголовок =
		СтрШаблон(НСтр("ru = 'Суммы доходов в управленческим учете (%1):';
						|en = 'Income amounts in management accounting (%1):'"), ВалютаУпр);
	Элементы.ДекорацияДоходыСуммыРегл.Заголовок =
		СтрШаблон(НСтр("ru = 'Суммы доходов в регламентированном учете (%1):';
						|en = 'Income amounts in local accounting (%1):'"), ВалютаРегламентированногоУчета);
		
	Элементы.ПрочиеДоходыСумма.Заголовок =
		СтрШаблон(НСтр("ru = 'Упр. учет с НДС (%1)';
						|en = 'Management accounting including VAT (%1)'"), ВалютаУпр);
...
Процедура УправлениеЭлементамиФормыНаКлиенте()
	...
	Если ЭтоРасходы Тогда
		...
		Элементы.ПереключательРасшифровкиРасходыКорЧасть.Заголовок =
			?(РеклассификацияРасходов, НСтр("ru = 'Кор. расходы';
											|en = 'Cor. expenses'"), НСтр("ru = 'Активы/пассивы';
		...
		Иначе
			//++ НЕ УТ
			УправлениеЭлементамиНастройкиСчетовУчета();
			//-- НЕ УТ
		КонецЕсли;
	...
	Иначе
		...
		Элементы.ПереключательРасшифровкиДоходыКорЧасть.Заголовок =
			?(РеклассификацияДоходов, НСтр("ru = 'Кор. доходы';
											|en = 'Cor. income'"), НСтр("ru = 'Активы/пассивы';
																		|en = 'Assets/liabilities'"));
...
&НаСервере
Процедура УправлениеЭлементамиНастройкиСчетовУчета()
...
&НаКлиенте
Процедура ПредставлениеОтраженияВРеглУчетеБезРазбиенияПослеНажатия(Результат, ДополнительныеПараметры) Экспорт
	
	ИмяТЧ = ДополнительныеПараметры.ИмяТЧ;
	СтрокаСНастройкой = Объект[ИмяТЧ][0];
	
	Для Каждого СтрокаТЧ Из Объект[ИмяТЧ] Цикл
		Если СтрокаТЧ <> СтрокаСНастройкой Тогда
			СтрокаТЧ.НастройкаСчетовУчета = СтрокаСНастройкой.НастройкаСчетовУчета;
			СтрокаТЧ.ПредставлениеОтраженияВРеглУчете = СтрокаСНастройкой.ПредставлениеОтраженияВРеглУчете;
			НастройкаСчетовУчетаКлиент.ПриКопированииСтроки(ЭтотОбъект, Элементы[ИмяТЧ], СтрокаТЧ);
		КонецЕсли;
	КонецЦикла;
...
Процедура НастроитьВидимостьКомандыОтобразитьСуммыПРИВР()
	...
	Элементы.ФормаОтобразитьСуммыПРИВР.Видимость = ВывестиКомандуОтобразитьСуммыПРИВР;    
	...
	Если ПереключательРасшифровкиРасходы = 0
	 ...
	 И НЕ РасходыВСтрокахОдинаковые()Тогда
		ТекстСообщения = НСтр("ru = 'Переключение в режим без разбиения невозможно, если в строках табличной части указаны разные расходы!';
								|en = 'Cannot switch to no-split mode if different expenses are specified in the table rows.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ЗначениеПереключателя = 1;
	КонецЕсли;
	...
	Если ПереключательРасшифровкиРасходы = 1
	 ...
	 И НЕ КорЧастьВСтрокахОдинаковая("ПрочиеРасходы") Тогда
		ТекстСообщения = НСтр("ru = 'Переключение в режим без разбиения невозможно, если в строках табличной части указаны разные активы/пассивы!';
								|en = 'Cannot switch to no-split mode if different assets/liabilities are specified in the table rows.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ЗначениеПереключателя = 1;
	КонецЕсли;
	...
	Если ПереключательРасшифровкиРасходы = 1
	 ...
	 И НЕ КорРасходыВСтрокахОдинаковые() Тогда
		ТекстСообщения = НСтр("ru = 'Переключение в режим без разбиения невозможно, если в строках табличной части указаны разные кор. расходы!';
								|en = 'Cannot switch to no-split mode if different cor. expenses are specified in the table rows.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ЗначениеПереключателя = 1;
	КонецЕсли;
...
Функция КорЧастьВСтрокахОдинаковая(ИмяТЧ)
	...
	//++ НЕ УТ
	Таблица = Объект[ИмяТЧ].Выгрузить(, "НастройкаСчетовУчета");
	НастройкиСчетовУчета = Новый Массив();
	
	Для Каждого СтрокаТаблицы Из Таблица Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицы.НастройкаСчетовУчета) Тогда
			НастройкиСчетовУчета.Добавить(СтрокаТаблицы.НастройкаСчетовУчета);
		КонецЕсли;
	КонецЦикла;
	
	Если НастройкиСчетовУчета.Количество() > 0 Тогда
		Таблица.Колонки.Добавить("ХешСуммаНастройки", ОбщегоНазначения.ОписаниеТипаСтрока(50));
		
		НастройкаСчетовУчетаСервер.ЗаписатьНастройкиСчетовУчетаФормы(ЭтотОбъект);
		ХешСуммыНастроек = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(НастройкиСчетовУчета, "ХешСумма");
		
		Для Каждого СтрокаТаблицы Из Таблица Цикл
			СтрокаТаблицы.ХешСуммаНастройки = ХешСуммыНастроек[СтрокаТаблицы.НастройкаСчетовУчета];
		КонецЦикла;
		
		Таблица.Свернуть("ХешСуммаНастройки");
		Возврат Таблица.Количество() < 2;
	КонецЕсли;
	//-- НЕ УТ
	...
	Если ПереключательРасшифровкиДоходы = 0
	 ...
	 И НЕ ДоходыВСтрокахОдинаковые()Тогда
		ТекстСообщения = НСтр("ru = 'Переключение в режим без разбиения невозможно, если в строках табличной части указаны разные доходы!';
								|en = 'Cannot switch to no-split mode if different income is specified in the table rows.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ЗначениеПереключателя = 1;
	КонецЕсли;
	...
	Если ПереключательРасшифровкиДоходы = 1
	 ...
	 И НЕ КорДоходыВСтрокахОдинаковые() Тогда
		ТекстСообщения = НСтр("ru = 'Переключение в режим без разбиения невозможно, если в строках табличной части указаны разные кор. доходы!';
								|en = 'Cannot switch to no-split mode if different cor. income is specified in the table rows.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ЗначениеПереключателя = 1;
	КонецЕсли;
