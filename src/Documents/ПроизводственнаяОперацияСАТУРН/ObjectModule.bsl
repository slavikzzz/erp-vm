...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ШаблонСообщения = НСтр("ru = 'Не заполнена колонка ""%1"" в строке %2 списка ""Продукция""';
							|en = 'Не заполнена колонка ""%1"" в строке %2 списка ""Продукция""'");
	
	Для Каждого СтрокаТовары Из Товары Цикл
		Если Не ЗначениеЗаполнено(СтрокаТовары.НомерПартии) Тогда
			ТекстСообщения = СтрШаблон(ШаблонСообщения, НСтр("ru = 'Данные партии';
															|en = 'Данные партии'"), СтрокаТовары.НомерСтроки);
