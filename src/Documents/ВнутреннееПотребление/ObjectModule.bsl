...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//++ НЕ УТ
	ТМЦВЭксплуатацииСервер.ПроверитьПередачуВЭксплуатацию(ЭтотОбъект, МассивНепроверяемыхРеквизитов, Отказ);
	//-- НЕ УТ
	...
	Для ТекИндекс = 0 По Товары.Количество()-1 Цикл
		...
		Если ПотреблениеПоЗаказам
			...
			ТекстОшибки = НСтр("ru = 'Не заполнено поле ""Заказ"" в строке %НомерСтроки% списка ""Товары""';
								|en = '""Order"" in line %НомерСтроки% of the Item list is not filled in'");
	...
	Если ПотреблениеПоЗаказам И Не ПраваПользователяПовтИсп.ВнутреннееПотреблениеСверхЗаказа() Тогда
		...
		ШаблонОшибки = НСтр("ru = 'Нет доступа для оформления внутреннего потребления сверхзаказа (строка %1 списка ""Товары"")';
							|en = 'No access for registration of the extra order internal consumption (the %1 line of the ""Goods"" list)'");
	...
	//++ НЕ УТ
	ПроверитьСтатьиИАналитики = (ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПередачаВЭксплуатацию);
	//-- НЕ УТ
...
Процедура ЗаполнитьНаОснованииПоступления(ДанныеЗаполнения)
	...
	ИначеЕсли МассивСкладов.Количество() > 0 Тогда
		ТекстОшибки = НСтр("ru = 'Ввод на основании поступления на разные склады не поддерживается.';
							|en = 'Generation of documents from receipts to different warehouses is not supported.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Процедура ЗаполнитьСтатьюРасходовВидовЗапасов()
	...
	Для Каждого СтрокаТоваров Из Товары Цикл
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			НоваяСтрока.НастройкаСчетовУчета = СтрокаТоваров.НастройкаСчетовУчета;
			//-- НЕ УТ
...
Процедура ЗаполнитьВидЗапасовПолучателя()
	...
	|		И ВидыЗапасов.Договор.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СДавальцем2_5) ТОГДА
	...
	|		И ВидыЗапасов.Договор.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СДавальцем2_5)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья2_5)
	//++ Устарело_Переработка24
	|	КОГДА ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.МатериалДавальца)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеОтДавальца)
	|	КОГДА ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПолуфабрикатДавальца)
