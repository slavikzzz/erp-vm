...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	ИсправлениеДокументов.ДобавитьКомандыСозданияНаОсновании(
		...
		Метаданные.Документы.ВнутреннееПотребление, 
	...
	Документы.РегистрацияДефекта.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	//-- НЕ УТКА
...
Функция ДанныеЗаполненияНакладной(МассивСсылок, СвойстваЗаказов = Неопределено) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказНаРемонт) Тогда
		...
		Текст =
			...
			|	Заказ.Организация КАК Организация,
			|	Заказ.Подразделение КАК Подразделение,
			|	ВЫБОР КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Материалы.Склад) = 1 ТОГДА
			|		МАКСИМУМ(Материалы.Склад)
			|	ИНАЧЕ
			...
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаРемонт.МатериалыИРаботы КАК Материалы
			|		ПО Заказ.Ссылка = Материалы.Ссылка
			|ГДЕ
			|	Заказ.Ссылка = &Заказ
			|
			|СГРУППИРОВАТЬ ПО
...
Функция ДанныеДляПроверкиВводаНаОсновании(Распоряжения, ХозяйственнаяОперация = Неопределено) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказНаРемонт) Тогда
		...
		Текст =
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ЗаказНаРемонт.Ссылка КАК Основание,
			|	ЗаказНаРемонт.Статус КАК Статус,
			|	ЗаказНаРемонтМатериалыИРаботы.Склад КАК Склад,
			|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию) КАК ХозяйственнаяОперация
			|ИЗ
			|	Документ.ЗаказНаРемонт КАК ЗаказНаРемонт
			|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаРемонт.МатериалыИРаботы КАК ЗаказНаРемонтМатериалыИРаботы
			|		ПО ЗаказНаРемонт.Ссылка = ЗаказНаРемонтМатериалыИРаботы.Ссылка
			|			И НЕ ЗаказНаРемонтМатериалыИРаботы.Отменено
			|ГДЕ
			|	ЗаказНаРемонт.Ссылка В(&Распоряжения)";
		Тексты.Добавить(Текст);
...
Процедура ЗаполнитьСерииПоОтгрузке(Объект) Экспорт
	...
	//++ НЕ УТКА
	РаспоряжениеПоУмолчанию = Неопределено;
	//-- НЕ УТКА
...
Функция ПроверкаПередСменойСтатуса(ВыборкаПроверки, НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если ВыборкаПроверки.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПередачаВЭксплуатацию Тогда
		...
		ТекстОшибки = НСтр("ru = 'У документа %1 статус ""%2"" не установлен, т.к. статусы не используются для операции, выбранной в документе';
							|en = '""%2"" status of the %1 document is not set as statuses are not used for the operation selected in the document'");
...
Процедура ЗапланироватьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	//++ НЕ УТКА
	|																ЗНАЧЕНИЕ(Документ.ЗаказНаРемонт.ПустаяСсылка),
	//-- НЕ УТКА
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление  = НСтр("ru = 'Внутренние потребления к оформлению';
								|en = 'Inventory consumptions to register'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Изменение", Метаданные.Документы.ВнутреннееПотребление) Тогда
		...
		КомандаПечати.Представление = ?(ПолучитьФункциональнуюОпцию("БазоваяВерсия"),НСтр("ru = 'Списание товаров на хознужды';
																							|en = 'Write off goods to business needs'"),НСтр("ru = 'Внутреннее потребление';
																																		|en = 'Inventory consumption'"));
...
Функция ПечатьВнутреннееПотребление(МассивОбъектов, ОбъектыПечати)
	...
	Если ПолучитьФункциональнуюОпцию("БазоваяВерсия") Тогда
		СинонимДокумента = НСтр("ru = 'Списание товаров и работ на хознужды';
								|en = 'Write off goods and works for business needs'");
	Иначе
