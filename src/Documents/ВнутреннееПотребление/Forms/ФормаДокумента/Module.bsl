...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыНастройки = Документы.ВнутреннееПотребление.ПараметрыНастройкиСчетовУчета(Объект.ХозяйственнаяОперация);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПередачаВЭксплуатацию") Тогда
		...
		ОповеститьОбИзменении(Тип("СправочникСсылка.ПартииТМЦВЭксплуатации"));
	КонецЕсли;
	//-- НЕ УТ
...
Процедура СкладПриИзменении(Элемент)
	...
	Если Склад <> Объект.Склад Тогда
		...
		Если Объект.ПотреблениеПоЗаказам И Объект.Товары.Количество() > 0 Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("СкладПриИзмененииЗавершение", ЭтотОбъект), НСтр("ru = 'Список ""Товары"" будет очищен. Продолжить?';
																									|en = 'The Item list will be cleared. Continue?'"), РежимДиалогаВопрос.ДаНет);
			Возврат;
		КонецЕсли;
	...
	ОткрытьФорму(
		...
		Новый Структура("СписокДокументов, Заголовок",
			...
			НСтр("ru = 'Заказы на внутреннее потребление (списание) (%КоличествоДокументов%)';
				|en = 'Inventory consumption orders (write-off) (%КоличествоДокументов%)'")), ЭтаФорма,,,, Неопределено, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПотреблениеПоЗаказуПриИзменении(Элемент)
	...
	Если Объект.ПотреблениеПоЗаказам И ЭтоРаспоряжениеНакладная Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Потребление со склада уже начато. Установка признака невозможна.';
									|en = 'Consumption from the warehouse already started. Cannot set the flag.'");
		...
		Если Объект.Товары.Количество() > 0 Тогда
			Если ИспользоватьВнутреннееПотреблениеПоНесколькимЗаказам Тогда
				ТекстОповещения = НСтр("ru = 'Строки в списке ""Товары"" отвязаны от строк заказов.';
										|en = 'Lines in Item list are unlinked from order lines.'");
			Иначе
				ТекстОповещения = НСтр("ru = 'Строки в списке ""Товары"" отвязаны от строк заказа.';
										|en = 'Lines in Item list are unlinked from order lines.'");
			КонецЕсли;
			ПоказатьОповещениеПользователя(НСтр("ru = 'Обработка списка ""Товары""';
												|en = 'The Item list processing'"),, ТекстОповещения);
		КонецЕсли;
		ОбновитьЗависимыеРеквизитыФормыСерверПоЗаказу();
		УстановитьВидимостьКомандПоЗаказу();
...
Процедура СоздатьГрупповоеОС(Команда)
	...
	Если СписокСтрок.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Необходимо выбрать строки с учетом в виде группового ОС';
										|en = 'Select lines with accounting as group fixed assets'"));
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = НСтр("ru = 'Создать групповое ОС и заполнить аналитику в выбранных строках?';
						|en = 'Create group fixed assets and fill the analytics in the selected lines?'");
	ОписаниеОповещения = Новый ОписаниеОповещения("СоздатьГрупповоеОСЗавершение", ЭтотОбъект);
...
Процедура РазбитьСтрокуЗавершение(НоваяСтрока, ДополнительныеПараметры) Экспорт
	...
	Если НоваяСтрока <> Неопределено Тогда
		...
		//++ НЕ УТ
		НастройкаСчетовУчетаКлиент.ПриКопированииСтроки(ЭтотОбъект, Элементы.Товары, НоваяСтрока);
		//-- НЕ УТ
...
Процедура ОповеститьОПривязкеСтрокКЗаказам(ЗаказСсылка = Неопределено)
	...
	Если ЗаказСсылка <> Неопределено Тогда
		...
		ШаблонТекста = НСтр("ru = 'Строки в списке ""Товары"" привязаны к строкам документа %Заказ%.';
							|en = 'Lines in Item list are linked to lines of document %Заказ%.'");
	...
	Иначе
		...
		ТекстОповещения = НСтр("ru = 'Строки в списке ""Товары"" привязаны к строкам неоформленных заказов.';
								|en = 'Lines in Item list are linked to ungenerated order lines.'");
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		Если ЗаказСсылка <> Неопределено Тогда
			...
			ШаблонТекста = НСтр("ru = 'Список ""Товары"" заполнен остатками неоформленных товаров по документу %Заказ%.';
								|en = 'The Item list is filled in with remaining unregistered goods under the %Заказ% document.'");
			ТекстОповещения = СтрЗаменить(ШаблонТекста, "%Заказ%", ЗаказСсылка);
			
		Иначе
			
			ТекстОповещения = НСтр("ru = 'Список ""Товары"" заполнен остатками неоформленных товаров по заказам.';
									|en = 'The Item list is filled in with remaining unregistered goods against orders.'");
	...
	Иначе
		...
		Если ЗаказСсылка <> Неопределено Тогда
			...
			ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по документу %Заказ%.';
								|en = 'No unregistered goods for document %Заказ%.'");
			ТекстОповещения = СтрЗаменить(ШаблонТекста, "%Заказ%", ЗаказСсылка);
		...
		Иначе
			...
			ИначеЕсли ИспользоватьНесколькоОрганизаций И ИспользоватьПодразделения И ЗначениеЗаполнено(Объект.Подразделение) Тогда
				ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по заказам на внутреннее потребление
				                        ...
				                        |en = 'No unregistered goods of inventory consumption orders
				                        |from warehouse ""%1"" in business unit ""%2"" in company ""%3""'");
			...
			ИначеЕсли ИспользоватьПодразделения И ЗначениеЗаполнено(Объект.Подразделение) Тогда
				ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по заказам на внутреннее потребление
				                        ...
				                        |en = 'No unregistered goods of inventory consumption orders
				                        |from warehouse ""%1"" in business unit ""%2""'");
				ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекста,
...
Функция ПривязатьСтрокиКЗаказамНаВнутреннееПотребление(ОтобратьПоЗаказу = Ложь)
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказНаРемонт) Тогда
		...
		Текст =
			...
			|	ВЫБОР КОГДА НЕ Ремонты.Ссылка.ОбъектЭксплуатации ЕСТЬ NULL ТОГДА
			|			Ремонты.Ссылка.ОбъектЭксплуатации
			|		ИНАЧЕ
			|			НЕОПРЕДЕЛЕНО
			...
			|	
			|	ВЫБОР КОГДА МатериалыИРаботы.Обособленно ТОГДА
			|				
			|				ТаблицаЗаказы.ЗаказНаВнутреннееПотребление.Назначение
			|				
			|			ИНАЧЕ
			...
			|	ЗаказыНаВнутреннееПотреблениеБезАналитик КАК ТаблицаЗаказы
			|		
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаРемонт.МатериалыИРаботы КАК МатериалыИРаботы
			|		ПО МатериалыИРаботы.Ссылка = ТаблицаЗаказы.ЗаказНаВнутреннееПотребление
			|			И МатериалыИРаботы.КодСтроки = ТаблицаЗаказы.КодСтроки
			|		
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаРемонт.Ремонты КАК Ремонты
			|		ПО Ремонты.Ссылка = МатериалыИРаботы.Ссылка
			|			И Ремонты.КодРемонта = МатериалыИРаботы.КодРемонта";
		Тексты.Добавить(Текст);
...
Процедура ОповеститьОПроведенииДокумента(ПараметрыЗаписи)
	...
	Оповестить("Запись_ВнутреннееПотребление", ПараметрыОповещения, Объект.Ссылка);
		...
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Списание товаров на хознужды %1 от %2';
					|en = 'Goods write-off to business needs %1 from %2'"),
				Объект.Номер,
				Объект.Дата);
		Иначе
			Заголовок = НСтр("ru = 'Списание товаров на хознужды (создание)';
							|en = 'Goods write-off to business needs (generation)'");
		КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<создается автоматически, если не заполнено>';
																					|en = '<created automatically if not filled>'"));
...
Функция ВыводитьСообщениеПоИспользованиюСтатусов()
	...
	Если ПараметрыУказанияСерий.ИспользоватьСерииНоменклатуры
		И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаВЭксплуатацию 
		И НЕ ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыВнутреннихПотреблений") Тогда
		
		УстановитьПривилегированныйРежим(Истина);
		
		ВозможенУчетСерийВЭксплуатации = ТМЦВЭксплуатацииПовтИсп.ВозможенУчетСерийВЭксплуатации();
		
		ПолитикиУчетаСерийСкладОтправитель = СкладыСервер.ИспользованиеСерийНаСкладеПоПолитикамУчета(Объект.Склад);
		ИспользоватьОрдернуюСхемуОтправитель = СкладыСервер.ИспользоватьОрдернуюСхемуПриОтгрузке(Объект.Склад, Объект.Дата);
		ПараметрыСерийСкладаОтправитель = СкладыСервер.ИспользованиеСерийНаСкладе(Объект.Склад, Ложь);
		...
		Если ИспользоватьОрдернуюСхемуОтправитель Тогда
			...
			Если НЕ ПолитикиУчетаСерийСкладОтправитель.УказыватьПриПланированииОтгрузки 
				И ВозможенУчетСерийВЭксплуатации Тогда
