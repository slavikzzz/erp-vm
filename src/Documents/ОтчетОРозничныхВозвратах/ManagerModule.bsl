...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат ОтчетОРозничныхВозвратахЛокализация.ТекстОтраженияВРеглУчете();
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	Возврат ОтчетОРозничныхВозвратахЛокализация.ТекстЗапросаВТОтраженияВРеглУчете();
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ИЗ
	|		Документ.ОтчетОРозничныхВозвратах.Товары КАК ТабличнаяЧасть
	|	ГДЕ
...
Функция ПолучитьДанныеДляПечатнойФормыТоварныйЧек(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Запрос.УстановитьПараметр("ПредставлениеДокумента", НСтр("ru = 'Отчет о розничных возвратах';
															|en = 'Retail return report'", ОбщегоНазначения.КодОсновногоЯзыка()));
	Запрос.УстановитьПараметр("ПредставлениеВОшибке",   НСтр("ru = 'отчета о розничных возвратах';
															|en = 'retail return report'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	СтруктураДанныхДляПечати = Новый Структура(
		...
		НСтр("ru = 'Отчет о розничных возвратах';
			|en = 'Retail return report'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ШаблонКомментарияОбработчика = НСтр("ru = 'Проверка/исправление значения реквизита %1 в табличной части %2 документа %3. Значение 9 будет заменено на 10, если серии корректно указаны в отдельной табличной части.';
										|en = 'Check/correct the value of the %1 attribute in the %2 table of the %3 document. The value 9 will be replaced with 10 if batches are correctly specified in a separate table.'");
