...
Процедура ПоРезультатамИнвентаризацииПриИзменении(Элемент)
	...
	Если Объект.Товары.Количество() > 0 И Объект.ПоРезультатамИнвентаризации Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ПоРезультатамИнвентаризацииПриИзмененииЗавершение", ЭтотОбъект), 
			НСтр("ru = 'Данные о количестве могут быть потеряны, продолжить?';
				|en = 'Quantity data may be lost, continue?'"), РежимДиалогаВопрос.ДаНет);
	    Возврат;
	Иначе
	...
	Если ЗначениеЗаполнено(Объект.КассоваяСмена) Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Отчет перезаполнен';
				|en = 'Report is refilled'"),
			,
			НСтр("ru = 'Отчет о розничных возвратах перезаполнен по данным кассовой смены';
				|en = 'Retail return report is refilled by the POS shift data'"),
...
Процедура ОткрытьПодбор(Команда)
	...
	ПараметрЗаголовок = НСтр("ru = 'Подбор товаров в %Документ%';
							|en = 'Pick goods in %Документ%'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПараметрЗаголовок = СтрЗаменить(ПараметрЗаголовок, "%Документ%", Объект.Ссылка);
	Иначе
		ПараметрЗаголовок = СтрЗаменить(ПараметрЗаголовок, "%Документ%", НСтр("ru = 'отчет о розничных возвратах';
																				|en = 'retail return report'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	КонецЕсли;
