...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		ВызватьИсключение НСтр("ru = 'Извещения ФСС загружаются в автоматическом режиме';
								|en = 'SSF notifications are imported automatically'");
	КонецЕсли;
...
Процедура ОбновитьЭлементыФормы()
	...
	Элементы.ГруппаВходящийДокумент.Доступность = ЗначениеЗаполнено(Объект.ВходящийФайл);
		...
		Если Объект.ПодтверждениеПолученоСФР Тогда
			...
			Элементы.НадписьПодтверждениеОтправленоИПолучено.Заголовок = СтрШаблон(
				НСтр("ru = 'Подтверждение получения отправлено %1
					...
					|and received by the Social Insurance Fund of Russia.'"),
		...
		Иначе
			...
			Элементы.НадписьПодтверждениеОтправленоНоНеПолучено.Заголовок = СтрШаблон(
				НСтр("ru = 'Подтверждение получения отправлено %1,
					...
					|en = 'Receipt confirmation was sent on %1.
					|However, there is no information that the Social Insurance Fund of Russia received it.'"),
				Формат(Объект.ДатаОтправкиПодтверждения, "ДЛФ=D"));
		КонецЕсли;
	Иначе
