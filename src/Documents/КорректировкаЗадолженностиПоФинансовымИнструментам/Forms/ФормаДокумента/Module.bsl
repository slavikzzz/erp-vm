...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ПараметрыНастройки = Документы.КорректировкаЗадолженностиПоФинансовымИнструментам.ПараметрыНастройкиСчетовУчета(Объект.ХозяйственнаяОперация);
...
Процедура ЗадолженностьТипСуммыПриИзменении(Элемент)
	...
	//++ НЕ УТ
	СтрокаТабличнойЧасти = Элементы.Задолженность.ТекущиеДанные;
	Если СтрокаТабличнойЧасти <> Неопределено
		И СтрокаТабличнойЧасти.ТипСуммы <> ПредопределенноеЗначение("Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж")
		И Объект.ХарактерДоговора <> ПредопределенноеЗначение("Перечисление.ХарактерыДоговоровФинансовыхИнструментов.Аренда") Тогда
		СтрокаТабличнойЧасти.РасчетныйДокумент = Неопределено;
	КонецЕсли;
	НастройкаРасчетногоДокумента();
	//-- НЕ УТ
	Возврат;
...
Процедура ЗадолженностьРасчетныйДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыДанных.Вставить("Договор", Объект.Договор);
	ПараметрыДанных.Вставить("Контрагент", Объект.Контрагент);
	ПараметрыДанных.Вставить("Организация", Объект.Организация);
	ПараметрыДанных.Вставить("ТипПлатежаПоАренде", СтрокаТабличнойЧасти.ТипСуммы);
	
	ОткрытьФорму("Справочник.ДоговорыАренды.Форма.ВыборРасчетногоДокумента", ПараметрыДанных, Элемент);
	//-- НЕ УТ
...
Процедура ЗадолженностьРасчетныйДокументОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		СтрокаТабличнойЧасти = Элементы.Задолженность.ТекущиеДанные;
		СтрокаТабличнойЧасти.РасчетныйДокумент = ВыбранноеЗначение.РасчетныйДокумент;
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	Если ВнеоборотныеАктивы.ИспользуетсяУправлениеВНА() Тогда
		СписокВыбораИнструмента.Добавить(Перечисления.ХарактерыДоговоровФинансовыхИнструментов.Аренда);
	КонецЕсли;
	//-- НЕ УТ
...
Процедура УправлениеЭлементамиФормы()
	...
	//++ НЕ УТ
	ИначеЕсли Объект.ХарактерДоговора = Перечисления.ХарактерыДоговоровФинансовыхИнструментов.Аренда Тогда
		Элементы.Договор.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.ДоговорыАренды");
		Элементы.ЗадолженностьТипСуммы.ОграничениеТипа = Новый ОписаниеТипов("ПеречислениеСсылка.ТипыПлатежейПоАренде");
	//-- НЕ УТ
	КонецЕсли;
	...
	Элементы.ЗадолженностьСумма.Заголовок = НСтр("ru = 'Сумма';
												|en = 'Amount'") + " (" + Строка(Объект.ВалютаВзаиморасчетов) + ")";
	
	//++ НЕ УТ
	ПараметрыВыбораРасчетногоДокумента = Новый Массив;
	ПараметрыВыбораРасчетногоДокумента.Добавить(Новый ПараметрВыбора("Отбор.РасшифровкаПлатежа_ДоговорАренды", Объект.Договор));
	ПараметрыВыбораРасчетногоДокумента.Добавить(Новый ПараметрВыбора("Отбор.РасшифровкаПлатежа_ТипПлатежаПоАренде", Перечисления.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж));
	ПараметрыВыбораРасчетногоДокумента.Добавить(Новый ПараметрВыбора("Отбор.Контрагент", Объект.Контрагент));
	МассивПараметровВыбораРасчетногоДокумента = Новый ФиксированныйМассив(ПараметрыВыбораРасчетногоДокумента);
	Элементы.ЗадолженностьРасчетныйДокумент.ПараметрыВыбора = МассивПараметровВыбораРасчетногоДокумента;
	//-- НЕ УТ
...
Процедура ОчиститьТабличнуюЧасть()
	...
	Если Объект.Задолженность.Количество() Тогда
		ТекстВопроса = НСтр("ru = 'Суммы корректировки задолженности могут стать неактуальными.
			...
			|en = 'AR/AP adjustment amounts can become irrelevant.
			|Clear debt list?'");
		ПоказатьВопрос(
...
Процедура ЗаполнитьЗадолженностьПоОстаткамСервер()
	...
	//++ НЕ УТ
	|	ВЫБОР
	|		КОГДА Расчеты.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж)
...
Процедура ЗаполнитьРеквизитыПоСтатистике()
	...
	//++ НЕ УТ
	|	ДанныеДокумента.НастройкаСчетовУчета,
	//-- НЕ УТ
...
//++ НЕ УТ
&НаКлиенте
Процедура НастройкаРасчетногоДокумента()
	...
	Если СтрокаТабличнойЧасти.ТипСуммы = ПредопределенноеЗначение("Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж")
			ИЛИ СтрокаТабличнойЧасти.ТипСуммы = ПредопределенноеЗначение("Перечисление.ТипыПлатежейПоАренде.УслугаПоАрендеАванс") Тогда
		Элементы.ЗадолженностьРасчетныйДокумент.ТолькоПросмотр = Ложь;
	Иначе
		Элементы.ЗадолженностьРасчетныйДокумент.ТолькоПросмотр = Истина;
		СтрокаТабличнойЧасти.РасчетныйДокумент = Неопределено;
	КонецЕсли;
