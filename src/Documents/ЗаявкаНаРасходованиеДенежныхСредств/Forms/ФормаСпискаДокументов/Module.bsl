...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	Элементы.СписокСоздатьВыдачуЗаймовСотруднику.Видимость = Истина;
	Элементы.СписокСоздатьОплатуАрендодателю.Видимость = Истина;
	//-- НЕ УТ
...
Процедура УстановитьСтатус(Статус, ИмяСтатуса)
	...
	ТекстВопроса = СтрШаблон(НСтр("ru = 'У выделенных в списке заявок будет установлен статус ""%1"". Продолжить?';
									|en = 'Status ""%1"" will be set for the requests selected in the list. Continue?'"), ИмяСтатуса);
...
Процедура ЗакрытьЗаявки(Команда)
	...
	Если КоличествоОбработанных > 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Закрыто %1 из %2 выделенных заявок.';
								|en = 'Closed %1 out of %2 selected requests.'");
	...
	Иначе
		ПоказатьОповещениеПользователя(НСтр("ru = 'Выбранные заявки не нуждаются в закрытии.';
											|en = 'The selected requests do not need to be closed.'"));
	КонецЕсли;
...
Процедура УстановитьВидимостьСтатусов()
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ДоступностьЗакрытияЗаявки = Ложь;
	Иначе
		
		ДоступностьЗакрытияЗаявки = Истина;
		...
		Для Каждого СтрокаСписка Из ВыделенныеСтроки Цикл
			...
			Если ДанныеЗаявки.Закрыта Тогда
				ДоступностьЗакрытияЗаявки = Ложь;
			Иначе
				ВидимостьЗакрытьЗаявки = 
					ДанныеЗаявки.Проведен
					И Не ДанныеЗаявки.ЗаявкаОплачена
					И ДанныеЗаявки.Статус <> ПредопределенноеЗначение("Перечисление.СтатусыЗаявокНаРасходованиеДенежныхСредств.Отклонена");
			КонецЕсли;
	...
	Если Не ДоступностьЗакрытияЗаявки Тогда
...
Процедура Подключаемый_ПриСозданииНаСервереЛокализация(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	Элементы.СписокСоздатьВыдачуЗаймовСотруднику.Видимость = Не ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплаты") Или
		ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплаты") И ПолучитьФункциональнуюОпцию("ИспользоватьЗаймыСотрудникам");
	//-- НЕ УТ
