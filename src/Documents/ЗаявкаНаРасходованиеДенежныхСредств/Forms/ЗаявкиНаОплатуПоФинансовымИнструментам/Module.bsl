...
Процедура ОбластиПланированияОтборНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Перечисление.ХозяйственныеОперации.Форма.ФормаВыбораОперации",
		Новый Структура("СписокОпераций, Заголовок", ОбластиПланированияОтбор, НСтр("ru = 'Финансовые инструменты';
																					|en = 'Financial instruments'")), Элемент);
...
Процедура УстановитьОграничениеТипаДоговора()
	...
	//++ НЕ УТ
	ДоговорКредитов = Ложь;
	ДоговорАренды = Ложь;
	
	Для каждого Область Из ОбластиПланированияОтбор Цикл
		Если Область.Пометка Тогда
			Если Область.Значение = Перечисления.ОбластиПланированияПлатежей.Аренда Тогда
				ДоговорАренды = Истина;
			Иначе
				ДоговорКредитов = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	//-- НЕ УТ
	...
	//++ НЕ УТ
	Если ДоговорКредитов И ДоговорАренды
		Или Не ДоговорКредитов И Не ДоговорАренды Тогда
		Элементы.ДоговорОтбор.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.ДоговорыКредитовИДепозитов, СправочникСсылка.ДоговорыАренды");
	ИначеЕсли ДоговорКредитов Тогда
	//-- НЕ УТ
	...
	ИначеЕсли ДоговорАренды Тогда
		Элементы.ДоговорОтбор.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.ДоговорыАренды");
	КонецЕсли;
	//-- НЕ УТ
