...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтчетДавальцуМеждуОрганизациями") Тогда
		ЗаполнитьПоОтчетуДавальцуМеждуОрганизациями(ДанныеЗаполнения, ДанныеЗаполнения);
	//-- НЕ УТКА
...
Процедура ЗаполнитьПоОтчетуДавальцуМеждуОрганизациями(Знач ДокументОснование, ДанныеЗаполнения)
	...
	|
	|	ДанныеДокумента.СуммаДокумента      КАК СуммаДокумента,
	|	РеквизитыСторно.Ссылка ЕСТЬ НЕ NULL КАК ВведеноСторно,
	|	&Ссылка КАК ДокументОснование
	|ИЗ
	|	Документ.ОтчетДавальцуМеждуОрганизациями КАК ДанныеДокумента
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.Сторно КАК РеквизитыСторно
	|	ПО РеквизитыСторно.СторнируемыйДокумент = ДанныеДокумента.Ссылка
	|	И РеквизитыСторно.Проведен
	|
	|ГДЕ
	|	ДанныеДокумента.Ссылка = &Ссылка
	...
	Иначе
		...
		Если Выборка.Следующий() Тогда
			...
			Если Выборка.ВведеноСторно Тогда
				Текст =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'По документу оформлено сторно. Ввод заявки на расходования денежных средств невозможен.';
							|en = 'Storno is registered for the document. Cannot generate a payment request.'"),
						ДокументОснование);
				ВызватьИсключение Текст;
			КонецЕсли;
...
Процедура ЗаполнитьПоПодарочномуСертификату(Знач ДокументОснование, ДанныеЗаполнения)
	...
	Если ДанныеПодарочногоСертификата.Номинал <= 0 Тогда
		Текст = НСтр("ru = 'Данная операция поддерживается только для подарочных сертификатов с ненулевым номиналом.';
					|en = 'This transaction is supported only for gift cards with a non-zero nominal value.'");
		ВызватьИсключение Текст;
	КонецЕсли;
