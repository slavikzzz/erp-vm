...
Процедура ОбновитьНадписьИсправляемыйСчетФактура()
	...
	Элементы.РеквизитыИсправляемыйСФНадпись.Заголовок = ?(Объект.Корректировочный,
		НСтр("ru = 'Корр. счет-фактура';
			|en = 'Corr. tax invoice'"), НСтр("ru = 'Счет-фактура';
												|en = 'Tax invoice'"));
...
Процедура ОбновитьНадписьКорректируемыеСчетаФактуры(Форма)
	...
	Форма.Элементы.РеквизитыКорректируемыеСФНадпись.Заголовок = ?(КоличествоСчетовФактур > 1,
		НСтр("ru = 'К счетам-фактурам';
			|en = 'To tax invoices'"), НСтр("ru = 'К счету-фактуре';
												|en = 'To tax invoice'"));
...
Процедура ОткрытьИсходныеСчетаФактуры()
	...
	СтруктураПараметров.Вставить("Заголовок", НСтр("ru = 'Корректировка к счетам-фактурам (%КоличествоДокументов%)';
													|en = 'Adjustment of tax invoices (%КоличествоДокументов%)'"));
...
Процедура ПослеЗаполненияТаблицыТоваров(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ТекстСообщения = НСтр("ru = 'Произошла ошибка обновления таблицы товаров:';
								|en = 'The error is occurred while updating the goods table:'") + Символы.ПС + Результат.КраткоеПредставлениеОшибки;
...
Процедура УправлениеЭлементамиФормы()
	...
	Элементы.Номер.Заголовок = ?(Объект.Корректировочный,
		НСтр("ru = 'Корр. счет-фактура №';
			|en = 'Corr. tax invoice No.'"), НСтр("ru = 'Счет-фактура №';
													|en = 'Tax invoice No.'"));
...
Процедура ОбновитьИнформациюПоПокупателям()
	...
	ИначеЕсли УникальныеПокупатели.Количество() = 0 Тогда
		МассивСтрок.Добавить(
			Новый ФорматированнаяСтрока(НСтр("ru = 'Выбрать клиента';
											|en = 'Select customer'"), , WebЦвета.Кирпичный, , "ВыбратьПокупателя"));
		...
		Если Не Выставлен Тогда
			МассивСтрок.Добавить(
				Новый ФорматированнаяСтрока(НСтр("ru = '(Изменить)';
												|en = '(Change)'"), , ЦветГиперссылки, , "ОткрытьФормуПокупатели"));
		Иначе
			МассивСтрок.Добавить(
				Новый ФорматированнаяСтрока(НСтр("ru = '(Посмотреть)';
												|en = '(View)'"), , ЦветГиперссылки, , "ОткрытьФормуПокупатели"));
		КонецЕсли;
	Иначе
		ТекстПокупателей = ОбщегоНазначенияУТКлиентСервер.СклонениеСлова(
				УникальныеПокупатели.Количество(),
				НСтр("ru = 'клиент';
					|en = 'customer'"), НСтр("ru = 'клиента';
											|en = 'of customer'"), НСтр("ru = 'клиентов';
																	|en = 'customers'"), НСтр("ru = 'м';
																							|en = 'm'"));
		Текст = СтрШаблон(НСтр("ru = 'Выбрано %1 %2';
								|en = 'Selected %1 %2'"), УникальныеПокупатели.Количество(), ТекстПокупателей);
		МассивСтрок.Добавить(
