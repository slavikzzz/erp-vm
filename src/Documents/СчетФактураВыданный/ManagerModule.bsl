...
Функция ИменаРеквизитовДляЗаполненияПараметровУказанияСерий() Экспорт
	ИменаРеквизитов = "Корректировочный,Склад,Дата,Перевыставленный,Исправление";
...
Функция ДобавитьКомандуСоздатьНаОснованииИсправленныйСчетФактура(КомандыСозданияНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.СчетФактураВыданный) Тогда
		...
		КомандаСоздатьНаОсновании.Представление = НСтр("ru = 'Счет-фактура выданный';
														|en = 'Issued tax invoice'", ОбщегоНазначения.КодОсновногоЯзыка());
		КомандаСоздатьНаОсновании.РежимЗаписи = "Проводить";
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		Документ.СчетФактураВыданный.Товары КАК Строки
	|	ПО 
	...
	|		И УчетнаяПолитикаОрганизаций.ДатаОтражения = НАЧАЛОПЕРИОДА(Операция.Дата, ДЕНЬ)
	|		И УчетнаяПолитикаОрганизаций.РаздельныйУчетПоНалогуНаПрибыль
	|
	|ГДЕ
	|	Операция.НДССМежценовойРазницы
	|";
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Если ЗначениеЗаполнено(Реквизиты.НомерИсправления) Тогда
		Если Реквизиты.Исправление Тогда
			ШаблонСтроки = "ru = 'Исправление %1 от %2';
							|en = 'Correction %1 dated %2'"; // @НСтр
		КонецЕсли;
		Если Реквизиты.Корректировочный Тогда
			ШаблонСтроки = "ru = 'Исправление %1 от %2 (корректировочный)';
							|en = 'Correction %1 dated %2 (corrective)'"; // @НСтр
		КонецЕсли;
...
Функция ТекстНДССРеализацииРаботыУслуги()
	...
	|		КОГДА Строки.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ Строки.СтавкаНДС.ПеречислениеСтавкаНДС
	|	КОНЕЦ КАК СубконтоДт2,
	|	ВЫБОР
	|		КОГДА Строки.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА Строки.СтавкаНДС.ПеречислениеСтавкаНДС
...
Функция ТекстВыручкаОтРеализацииРаботУслуг()
	...
	|		КОГДА Строки.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА Строки.СтавкаНДС.ПеречислениеСтавкаНДС
	|		ИНАЧЕ ЕСТЬNULL(ТаблицаНалогообложенияПрибыли.ВариантНалогообложенияПрибыли, Строки.Номенклатура)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		НоваяСтрока.ПредставлениеВыставленКомиссионеру = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Выставлен комиссионеру %1, ИНН/КПП: %2, по счетам-фактурам %3';
				|en = 'Issued to commission agent %1, TIN/KPP: %2, of tax invoices %3'"),
...
Процедура СформироватьВременнуюТаблицуТоваровДляПечати(МенеджерВременныхТаблиц, МассивОснований, ПараметрыПечати) Экспорт
	...
	//++ НЕ УТКА
	ЕстьОтчетыДавальцу =  ОтчетДавальцу2_5Локализация.ЕстьОтчетыДавальцу(МассивОснований);
	//-- НЕ УТКА
	...
	Для Каждого ТипОснования Из ТипыОснований Цикл
		...
		//++ НЕ УТКА
		ИначеЕсли ЕстьОтчетыДавальцу Тогда
		...
		Если ИмяОбъекта = "АктВыполненныхРабот"
				...
				Или ИмяОбъекта = "ОтчетДавальцу"
				//-- Устарело_Переработка24
				Или ИмяОбъекта = "ОтчетДавальцу2_5"
...
Функция ТекстЗапросаДанныхШапкиДляПечатиСчетаФактуры(ПараметрыПечати)
	...
	|		ДанныеПодразделенийДляКПП.Подразделение
	|	ИНАЧЕ
	|		НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ПодразделениеДляКПП,
	|
	|	ВЫБОР КОГДА НЕ ДанныеПодразделенийДляКПП.РегистрацияВНалоговомОргане.Ссылка ЕСТЬ NULL ТОГДА
	...
	|	ДанныеОснований.ПодразделениеДляКПП                   КАК ПодразделениеДляКПП
	|ИЗ
	|	СчетаФактурыОснованияПервые КАК СчетаФактурыОснования
	|
	...
	Если ПечататьСчетаФактурыПолученные Тогда
		...
		|	ДанныеКомиссионера.ПредставлениеВыставленКомиссионеру КАК ПредставлениеВыставленКомиссионеру,
		|	НЕОПРЕДЕЛЕНО                                          КАК ПодразделениеДляКПП
		|
		|ИЗ
		|	СчетаФактурыПолученныеОснованияПервые КАК СчетаФактурыОснования
		|
	...
	Если ПараметрыПечати.Свойство("ДополнитьДаннымиУПД") И ПараметрыПечати.ДополнитьДаннымиУПД Тогда
		...
		|	ДанныеОснований.АдресДоставкиЗначенияПолей  КАК АдресДоставкиЗначенияПолей,
		|	ДанныеОснований.АдресДоставкиЗначение       КАК АдресДоставкиЗначение,
