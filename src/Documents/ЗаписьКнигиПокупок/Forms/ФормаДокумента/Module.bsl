...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ПараметрыНастройки = Документы.ЗаписьКнигиПокупок.ПараметрыНастройкиСчетовУчета();
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ТекстСообщенияДатаПолучения = НСтр("ru = 'Поле ""Дата получения"" не заполнено';
										|en = 'The ""Date received"" field is not filled in'");
...
Процедура ВидЦенностиПриИзмененииСервер(ИдентификаторСтроки)
	...
	НастройкаСчетовУчетаСервер.ОбновитьПредставлениеНастройки(ЭтотОбъект, "Объект.Ценности", ИдентификаторСтроки);
	//-- НЕ УТ
	Возврат;
...
Функция СвязиПараметрыОтбора(ТипЗначения)
	...
	ИначеЕсли ТипЗначения = Тип("ДокументСсылка.ВозвратТоваровМеждуОрганизациями") Тогда
		СвязиПараметрыОтбора.Вставить("ОрганизацияПолучатель", "Организация");
	//++ НЕ УТКА
	ИначеЕсли ТипЗначения = Тип("ДокументСсылка.ОтчетДавальцуМеждуОрганизациями") Тогда
		СвязиПараметрыОтбора.Вставить("Давалец", "Организация");
	//-- НЕ УТКА
	Иначе
