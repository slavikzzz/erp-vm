...
Процедура ДобавитьДанныеПоСотрудникам(СотрудникКДобавлению)
	...
	ПолучитьБухучетНачислений(СотрудникиКЗаполнениюНачислений);
	
КонецПроцедуры 

&НаСервере
Процедура ПолучитьБухучетНачислений(СотрудникиКЗаполнениюНачислений)
	
	СотрудникиПериоды = Объект.Сотрудники.Выгрузить(СотрудникиКЗаполнениюНачислений);
	СотрудникиПериоды.Колонки.ВремяРегистрации.Имя = "Период";
	...
	Для каждого СтрокаСотрудника Из СотрудникиПериоды Цикл
		Отбор.ИдентификаторСтрокиСотрудника = СтрокаСотрудника.ИдентификаторСтрокиСотрудника;
		НачисленияСотрудника = Объект.Начисления.НайтиСтроки(Отбор);
...
Процедура УдалитьДанныеПоСотрудникам(ИдентификаторыСтрокСотрудников, УдалятьСтрокиСотрудников = Истина)
	...
	Для каждого ИдентификаторСтрокиСотрудника Из ИдентификаторыСтрок Цикл
		...
		СтрокиКУдалению = Объект.БухучетНачислений.НайтиСтроки(СтруктураПоиска);
		Для каждого СтрокаКУдалению Из СтрокиКУдалению Цикл
			Объект.БухучетНачислений.Удалить(СтрокаКУдалению);
		КонецЦикла;
...
Функция АдресВХранилищеНачисленийИУдержаний(ИдентификаторСтроки)
	...
	ПараметрыОткрытия.РедактироватьБухучетНачислений = ПолучитьФункциональнуюОпцию("РегистрироватьБухучетПлановыхНачислений");
...
Процедура ДополнитьСтруктуруНачислениямиИПоказателями(ПараметрыОткрытия, ТекущиеДанные)
	...
	ПоказателиНачислений = Новый Массив();
		...
		Если ИнфоОВидеРасчета.Рассчитывается Тогда
			...
			Для каждого ОписаниеПоказателя Из ИнфоОВидеРасчета.Показатели Цикл
				...
				Если ОписаниеПоказателя.ЗапрашиватьПриВводе Тогда
					...
					ПоказателиНачислений.Добавить(СтруктураПоказателя.Показатель);
	...
	Для Каждого ДанныеПокзаателя Из ТаблицаПоказателей Цикл
		Если ПоказателиНачислений.Найти(ДанныеПокзаателя.Показатель) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		СтруктураПоказателя = Новый Структура("Показатель,ИдентификаторСтрокиВидаРасчета,Значение,ТекущееЗначение");
		СтруктураПоказателя.Показатель = ДанныеПокзаателя.Показатель;
		СтруктураПоказателя.ИдентификаторСтрокиВидаРасчета = 0;
		СтруктураПоказателя.Значение = ДанныеПокзаателя.Значение;
		ДанныеПоказателя = ТекущиеЗначенияПоказателей.НайтиСтроки(Новый Структура("Показатель", ДанныеПокзаателя.Показатель));
		Если ДанныеПоказателя.Количество() > 0 Тогда
			СтруктураПоказателя.ТекущееЗначение = ДанныеПоказателя[0].Значение;
		КонецЕсли;
		МассивПоказателей.Добавить(СтруктураПоказателя);
	...
	ДобавитьБухучетНачислений(ИзмеренияДаты, МассивНачислений, ТекущиеДанные.ИдентификаторСтрокиСотрудника);
...
&НаСервере
Процедура ДобавитьБухучетНачислений(ИзмеренияДаты, МассивНачислений, ИдентификаторСтрокиСотрудника)

	Если Не ЗначениеЗаполнено(МассивНачислений) Тогда
		Возврат;
	КонецЕсли;
	
	СотрудникиДаты = ИзмеренияДаты.Скопировать(,"Сотрудник,Период");
