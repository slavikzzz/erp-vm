...
Процедура ЗаполнитьДанныеСотрудников(Команда)
	...
	Если Объект.Сотрудники.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Данные о сотрудниках будут перезаполнены, продолжить?';
										|en = 'Data on employees will be repopulated, continue?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура ПредупреждениеОНедопустимомОтчетномПериоде()
	...
	Текст = НСтр("ru = 'Форма СПВ-2 предоставляется, начиная с 2014 г. За предыдущие периоды предоставляется форма СПВ-1.';
				|en = 'SPV-2 form is provided since 2014. SPV-1 form is provided for the previous periods.'");
