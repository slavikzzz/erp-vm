			...
			ПерсонифицированныйУчет.ПроверитьДанныеЗастрахованногоЛица(
				...
				Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаОкончанияПериодаСтажа) Тогда 
					...
					Если ВыборкаПоСтрокамДокумента.ДатаСоставления < ВыборкаПоСтрокамДокумента.ДатаОкончанияПериодаСтажа Тогда
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %1: дата окончания последнего периода стажа должна быть меньше, чем дата составления.';
																									|en = 'Employee %1: end date of the last service period should be less than the date created.'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
				...
				ИначеЕсли ТипСведенийСЗВ <> Перечисления.ТипыСведенийСЗВ.ОТМЕНЯЮЩАЯ Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %1: не заполнены данные о стаже.';
																								|en = 'Employee %1: information about service length is not filled in.'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
