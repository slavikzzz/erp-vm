...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ЗаполнятьПоОрдеру 
		...
		//++ НЕ УТКА
		ПривязатьСтрокиКЗаказу =
			ЗначениеЗаполнено(Параметры.Ключ)
			И Объект.ПоступлениеПоЗаказам
			И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5;
		//-- НЕ УТКА
	...
	//++ НЕ УТКА
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//++ НЕ УТКА
	ЗаполнитьПризнакЦеховаяКладоваяВТаблице(Ложь);
		...
		Если НеобходимВопросОбОчисткеСклада() Тогда
			...
			Если СкладГруппа Тогда
				ТекстВопроса = НСтр("ru = 'Строки по заказу, в которых склад не входит в группу складов ""%Склад%"" будут удалены из списка ""Товары"". Продолжить?';
									|en = 'The order lines containing a warehouse that does not belong to the ""%Склад%"" group will be deleted from the Item list. Continue?'");
			Иначе
				ТекстВопроса = НСтр("ru = 'Строки по заказу, в которых склад отличается от ""%Склад%"" будут удалены из списка ""Товары"". Продолжить?';
									|en = 'The order lines containing a warehouse other than ""%Склад%"" will be deleted from the Item list. Continue?'");
			КонецЕсли;
...
Процедура ПоступлениеПоЗаказамПриИзмененииПослеВыбора(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Массив") И Результат.Количество() > 1 Тогда
		ТекстВопроса = НСтр("ru = 'Список Товары будет перезаполнен по остаткам неоформленных товаров выбранных заказов. Продолжить?';
							|en = 'The Goods list will be refilled with the remaining unregistered goods of selected orders. Continue?'");
	ИначеЕсли ТипЗнч(Результат) = Тип("Массив") И Результат.Количество() = 0 Тогда
		ТекстВопроса = НСтр("ru = 'Список Товары будет отвязан от заказов. Продолжить?';
							|en = 'The Goods list will be unlinked from orders. Continue?'");
	Иначе
		ТекстВопроса = НСтр("ru = 'Список Товары будет заполнен остатками неоформленных товаров по документу %ЗаказПоставщику%. Продолжить?';
							|en = 'The Goods list will be filled with the remaining unregistered goods by the %ЗаказПоставщику% document. Continue?'");
...
Процедура ХозяйственнаяОперацияПриИзменении(Элемент)
	...
	ВидЦеныПоставщикаПриИзменении(Элементы.ВидЦеныПоставщика);
...
Процедура ТоварыНоменклатураПартнераПриИзменении(Элемент)
	...
	Если Объект.ХозяйственнаяОперация =
			ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтДавальца2_5") Тогда
...
Процедура ЗаполнитьЦеныПоПоступлениям(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Партнер) Тогда
		...
		Если Объект.ХозяйственнаяОперация =
				ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтДавальца2_5") Тогда
			ТекстИсключение = НСтр("ru = 'Для заполнения цен по последним поступлениям необходимо указать давальца';
									|en = 'To fill the prices for the latest receipts, specify the provider'");
		Иначе
		//-- НЕ УТКА
		
			ТекстИсключение = НСтр("ru = 'Для заполнения цен по последним поступлениям необходимо указать поставщика';
									|en = 'To fill in the prices for the latest receipts, specify the vendor'");
	...
	Если ЦеныЗаполнены Тогда
		...
		ПоказатьОповещениеПользователя(
			...
			НСтр("ru = 'В строках документа заполнены цены';
				|en = 'Prices are populated in the document lines'"),
	...
	Иначе
		ПоказатьОповещениеПользователя(
			...
			НСтр("ru = 'Ни в одной строке цена не заполнена';
				|en = 'No lines have prices filled in'"),
			БиблиотекаКартинок.Информация32);
	КонецЕсли;
...
Процедура ИзменитьКачествоНоменклатуры(Команда)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.КодСтроки) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Для строки по заказу запрещено изменять качество. Воспользуйтесь командой ""Отвязать строки от заказа"".';
													|en = 'Cannot change quantity for the order line. Use the Unlink lines from the order command.'"));
		Возврат;
	КонецЕсли;
		...
		Если Модифицированность
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ЗакрытьЗаказЗавершение", ЭтотОбъект),
							НСтр("ru = 'Документ не проведен. Провести документ?';
								|en = 'The document is not posted. Post the document?'"),
							РежимДиалогаВопрос.ДаНет);
...
Процедура ПриИзмененииСкладаСервер()
	...
	//++ НЕ УТКА
	ЗаполнитьПризнакЦеховаяКладоваяВТаблице(Истина);
	//-- НЕ УТКА
	...
	Если Объект.ПоступлениеПоЗаказам Тогда
		...
		Если НЕ ИспользоватьПоступлениеПоНесколькимЗаказам 
			...
			Если ТипЗнч(Объект.ЗаказПоставщику) = Тип("ДокументСсылка.ЗаказДавальца2_5") Тогда
				СоглашениеЗаказа = Справочники.СоглашенияСПоставщиками.ПустаяСсылка();
			Иначе
			//-- НЕ УТКА
			...
			//++ НЕ УТКА
			Если ТипЗнч(Объект.ЗаказПоставщику) = Тип("ДокументСсылка.ЗаказДавальца2_5") Тогда
				Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5;
			КонецЕсли;
			//-- НЕ УТКА
...
Процедура ПриИзмененииХозяйственнойОперацииСервер()
	...
	Объект.ПоступлениеПоЗаказам = Объект.ПоступлениеПоЗаказам И ИспользоватьЗаказы(ЭтотОбъект);
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
		...
		ПараметрыПересчетаКоличестваЕдиниц = ПроизводствоКлиентСервер.ПараметрыПересчетаКоличестваЕдиниц();
...
Процедура ЗаполнитьЦеныНоменклатурыЗаказов()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца2_5.Материалы КАК ТабличнаяЧасть
	|		ПО ТаблицаДокументов.Ссылка = ТабличнаяЧасть.Ссылка
	|ГДЕ
	|	ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	...
	Если ЗаполнитьЦены Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ВидЦеныПоставщика) Тогда
			ТекстПредупреждения = НСтр("ru = 'В документе не указан вид цены поставщика. Цены не могут быть заполнены';
										|en = 'Vendor price type is not specified in the document. Prices cannot be filled in'");
...
Процедура ПослеУказанияЗаказаВСтроке(СтрокаТаблицы)
	...
	//++ НЕ УТКА
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтДавальца2_5")
		И ЗначениеЗаполнено(СтрокаТаблицы.ЗаказПоставщику)
		И Не СтрокаТаблицы.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.МногооборотнаяТара") Тогда
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранениеВПроцессеЗакупки") Тогда
		ХозяйственнаяОперацияСписокВыбора.Добавить(Перечисления.ХозяйственныеОперации.ПриемНаХранениеСПравомПродажи);
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья2_5") Тогда
		
		ХозяйственнаяОперацияСписокВыбора.Добавить(Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5);
		
		ВариантОбособленияМатериаловПриПриемеВПереработку =
			Константы.ВариантОбособленияМатериаловПриПриемеВПереработку.Получить();
	...
	ЗаполнитьПризнакЦеховаяКладоваяВТаблице(ПересчитатьКоличествоЕдиниц);
	...
	Если Объект.ПоступлениеПоЗаказам И
		...
		Если ЗначениеЗаполнено(Объект.ЗаказПоставщику) Тогда
			НадписьОтклонениеЦен = НСтр("ru = 'Цены отличаются от заказа';
										|en = 'Prices differ from the order'");
		Иначе
			НадписьОтклонениеЦен = НСтр("ru = 'Цены отличаются от заказов';
										|en = 'Prices differ from the orders'");
		КонецЕсли;
		...
		|en = 'To set the filter by the lines with price variance, 
		|use command ""Show price variances"" in command group ""Prices""'");
	Иначе
...
Процедура ЗаполнитьРеквизитыИзДоговора()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5
...
Функция ЗаполнитьСкладВВыделенныхСтрокахНаСервере(Знач МассивВыделенныхСтрок, Склад)
	...
	ЗаполнитьПризнакЦеховаяКладоваяВТаблице(, Строки);
	//-- НЕ УТКА
...
Функция ПривязатьСтрокиПоступленияКЗаказамПоставщикам(ОтобратьПоЗаказу, ЗаполнитьЦеныПоЗаказу = Ложь)
	...
	Иначе
		ТекстЗапросаОстатки =
			...
			|		ПО ЗаказыОстатки.ЗаказПоставщику = ЗаказДавальца2_5.Ссылка
			|
			|	ГДЕ
			|		ЗаказДавальца2_5.Партнер                 = &Партнер
			|		И ЗаказДавальца2_5.Контрагент            = &Контрагент
			|		И ЗаказДавальца2_5.Договор               = &Договор
			|		И ЗаказДавальца2_5.Организация           = &Организация
			|		И ЗаказДавальца2_5.ВариантПриемкиТоваров = &ВариантПриемкиТоваров
			|		И ЗаказДавальца2_5.ХозяйственнаяОперация = &ХозяйственнаяОперация
			//-- НЕ УТКА
			|
			|	ОБЪЕДИНИТЬ ВСЕ
			|
			...
			|		РегистрНакопления.ЗаказыПоставщикам КАК ЗаказыДвижения
			|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца2_5 КАК ЗаказДавальца2_5
			|			ПО ЗаказыДвижения.ЗаказПоставщику = ЗаказДавальца2_5.Ссылка
			|
			|	ГДЕ
			|		ЗаказыДвижения.Регистратор = &Регистратор
			|		И ЗаказыДвижения.Активность
			...
			|				Товары КАК Товары
			|			ГДЕ
			|				Товары.Номенклатура     = ЗаказыДвижения.Номенклатура
			|				И Товары.Характеристика = ЗаказыДвижения.Характеристика)
			|		И ИСТИНА В
			|			(ВЫБРАТЬ ПЕРВЫЕ 1
			|				ИСТИНА
			|			ИЗ
			|				Склады КАК Склады
			|			ГДЕ
			|				Склады.Склад = ЗаказыДвижения.Склад)
	...
	ТекстЗапросаЗаказы =
		...
		//++ НЕ УТКА
		|ГДЕ
		|	НЕ &ЭтоПоступлениеОтДавальца2_5
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЕСТЬNULL(ЗаказДавальца2_5Материалы.Ссылка, НЕОПРЕДЕЛЕНО)              КАК Ссылка,
		|	ЕСТЬNULL(ЗаказДавальца2_5Материалы.НомерСтроки, 0)                    КАК НомерСтроки,
		|	ЕСТЬNULL(ЗаказДавальца2_5Материалы.ДатаПоступления, ДАТАВРЕМЯ(1,1,1)) КАК ДатаПоступления,
		|	ЕСТЬNULL(ЗаказДавальца2_5Материалы.Цена, 0)                           КАК Цена,
		|	ЗаказыПоставщикам.Сделка                                              КАК Сделка,
		|	ЗаказыПоставщикам.ЗаказПоставщику                                     КАК ЗаказПоставщику,
		|	ЗаказыПоставщикам.Номенклатура                                        КАК Номенклатура,
		|	ЗаказыПоставщикам.Характеристика                                      КАК Характеристика,
		...
		|		ПО ЗаказыПоставщикам.Номенклатура   = ДублиТоваров.Номенклатура
		|		 И ЗаказыПоставщикам.Характеристика = ДублиТоваров.Характеристика
		|		 И ЗаказыПоставщикам.Склад          = ДублиТоваров.Склад
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца2_5.Материалы КАК ЗаказДавальца2_5Материалы
		|		ПО ЗаказыПоставщикам.ЗаказПоставщику = ЗаказДавальца2_5Материалы.Ссылка 
		|		 И ЗаказыПоставщикам.КодСтроки       = ЗаказДавальца2_5Материалы.КодСтроки
		|ГДЕ
		|	&ЭтоПоступлениеОтДавальца2_5
		//-- НЕ УТКА
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка,
		|	НомерСтроки,
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
		ХозяйственнаяОперация = ДавальческаяСхемаКлиентСервер.ХозяйственнаяОперацияДоговора();
		Запрос.УстановитьПараметр("ЭтоПоступлениеОтДавальца2_5", Истина);
	Иначе
	//-- НЕ УТКА
		ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПриемНаХранениеСПравомПродажи;
	//++ НЕ УТКА
		Запрос.УстановитьПараметр("ЭтоПоступлениеОтДавальца2_5", Ложь);
	КонецЕсли;
	//-- НЕ УТКА
	...
	//++ НЕ УТКА
	Если ЗаполнитьЦеныПоЗаказу Тогда
		
		ПоляКодСтроки 			= ПоляКодСтроки + ", Цена";
		ПоляКодСтрокиКоличество = ПоляКодСтрокиКоличество + ", Цена";
			...
			Для Каждого ТекСтрока Из НайденныеСтроки Цикл
				...
				Если КоличествоВСтроке > НераспределенноеКоличество Тогда
					...
					УчетПрослеживаемыхТоваровКлиентСерверЛокализация.ДополнитьОписаниеНастроекПересчетаРеквизитовТабличнойЧасти(
						...
						Истина);
				//++ НЕ УТКА
				ИначеЕсли ЗаполнитьЦеныПоЗаказу Тогда
					СтруктураДействийПересчета.Вставить("ПересчитатьСумму");
				//-- НЕ УТКА
				КонецЕсли;
...
Процедура УстановитьДоступностьСоглашений()
	...
	//++ НЕ УТКА
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Соглашение", "Видимость", Ложь);
	Иначе
	//-- НЕ УТКА
...
Процедура НастроитьФорму()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
		...
		Элементы.ТоварыЗаказПоставщику.Заголовок = НСтр("ru = 'Заказ давальца';
		...
		Элементы.ТоварыНазначение.ПропускатьПриВводе =
			Объект.ПоступлениеПоЗаказам
			Или ВариантОбособленияМатериаловПриПриемеВПереработку =
					Перечисления.ВариантыОбособленияПриПриемеВПереработку.ДоговорСДавальцем;
	...
	Иначе
		...
		Элементы.Партнер.Заголовок               = НСтр("ru = 'Поставщик';
														|en = 'Vendor'");
		Элементы.ТоварыЗаказПоставщику.Заголовок = НСтр("ru = 'Заказ поставщику';
														|en = 'Purchase order'");
		...
		Элементы.ТоварыНазначение.ПропускатьПриВводе          = Неопределено;
...
Процедура НастроитьПараметрыВыбораЭлементов()
	...
	Иначе
		...
		МассивПараметровВыбора.Добавить(
			Новый ПараметрВыбора(
					"Отбор.ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ПриемНаХранениеСПравомПродажи));
...
&НаСервере
Процедура ЗаполнитьПризнакЦеховаяКладоваяВТаблице(ПересчитатьКоличествоЕдиниц = Неопределено, Строки = Неопределено)
	
	ПроизводствоСервер.ЗаполнитьПризнакЦеховаяКладоваяВТаблице(Объект.Товары,,, Строки);
	
	Если ПересчитатьКоличествоЕдиниц = Истина
		Или ПересчитатьКоличествоЕдиниц = Неопределено
			И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
...
Функция ЗаполнитьПоПриемкеСервер(ЕстьПринятыеТовары, ПривязатьСтрокиКЗаказу = Ложь)
	...
	//++ НЕ УТКА
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
		СтруктураДействийСИзмененнымиСтроками.Вставить(
			"ПересчитатьКоличествоЕдиниц", ПараметрыПересчетаКоличестваЕдиниц);
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура УстановитьЗаголовокЗаполнитьПоЗаказамОрдерам()
	...
	Иначе
		ЗаголовокЗаполнить = НСтр("ru = 'Подобрать товары по заказам/ордерам';
									|en = 'Add items from Purchase order/Goods receipt'");
	КонецЕсли;
...
Функция ВидимостьОтвязатьОтЗаказа()
	...
	ПризнакВидимость =
		ПризнакВидимость
		И Объект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5
		Или ИспользоватьЗаказыДавальцев2_5 
			И Объект.ПоступлениеПоЗаказам
			И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5;
	//-- НЕ УТКА
