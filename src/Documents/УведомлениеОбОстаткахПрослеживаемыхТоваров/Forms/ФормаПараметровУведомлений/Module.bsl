		...
		Если НЕ ЗначениеЗаполнено(ИННПродавца) Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В строке %1 продавца %2 не заполнен ИНН';
					|en = 'TIN is not filled in line %1 of seller %2'"),
		...
		Если СписокПродавцов.НайтиПоЗначению(СтрокаПродавцы.Продавец)<> Неопределено Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В строке %1 повторно указан продавец %2.';
					|en = 'Seller %2 is specified repeatedly in line %1.'"),
				Индекс + 1, СтрокаПродавцы.Продавец);
			ОбщегоНазначения.СообщитьПользователю(
				Текст,
				,
	...
	Если ПризнакУведомления = 3 
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Не указан продавец';
				|en = 'The seller is not specified'"),
			,
			,
			,
			Отказ);
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.ФормаОткрытаИзКорректировочногоУведомления Тогда
		...
		Элементы.ОписаниеДополнительныхРеквизитов.Заголовок = 
			...
			|en = 'The attributes are filled in if previous goods owners were individuals, self-employment tax payers,
			|or state authorities, or a corrective notification is submitted for a reorganized company'");
	...
	Иначе
		...
		Элементы.ОписаниеДополнительныхРеквизитов.Заголовок = 
			...
			|en = 'Fill in attributes if previous goods owners were individuals, self-employment tax payers,
