...
Процедура ЗагрузитьДанныеТаблиц(ПараметрыОтбора = Неопределено)
	...
	Элементы.ГруппаОстатки.Заголовок 	 = НСтр("ru = 'По остаткам';
												|en = 'By remaining quantity'") + " (" + СокрЛП(ТаблицаОстатки.Количество()) + ")";
	Элементы.ГруппаПересортица.Заголовок = НСтр("ru = 'По исправлению остатков';
												|en = 'By balance correction'") + " (" + СокрЛП(ТаблицаПересортица.Количество()) + ")";
	Элементы.ГруппаВозвраты.Заголовок 	 = НСтр("ru = 'По возвратам и прочим поступлениям';
													|en = 'By returns and other receipts'") + " (" + СокрЛП(ТаблицаВозвраты.Количество()) + ")";
		...
		Если НомерГруппы <> СтрокаДанных.НомерГруппы Тогда
			...
			Если ЗначениеЗаполнено(ТекущийДокумент.НаименованиеПервичногоДокумента) Тогда
				ТекущийДокумент.Комментарий = ТекущийДокумент.НаименованиеПервичногоДокумента
					...
					+ " " + НСтр("ru = 'от';
								|en = 'from'") + " " + Формат(ТекущийДокумент.ДатаПервичногоДокумента, "ДЛФ=D");
			КонецЕсли;
