...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("Просмотр", Метаданные.ЖурналыДокументов.Взаимодействия) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'У вас нет прав на просмотр отчета по созданным документам рассылки.';
																|en = 'You are not authorized to view the report on the created mailout documents.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(ДанныеРассылки.Основание) = Тип("ДокументСсылка.НазначениеОпросов") И ЗначениеЗаполнено(ДанныеРассылки.Основание) Тогда
		...
		Если ДанныеОснования.ТипРеспондентов = Справочники.Пользователи.ПустаяСсылка() Тогда
			ВызватьИсключение НСтр("ru = 'В ""Назначении опросов"" указан некорректный тип респондентов. Для пользователей не может использоваться рассылка клиентам.';
									|en = 'There is wrong type of respondents indicated in the ""Purpose of survey"". For users, mailout to customers cannot be used.'");
		КонецЕсли
	Иначе
...
Функция ТекстЛегенды(ДанныеРассылки)
	...
	Если ДанныеРассылки.Статус = Перечисления.СтатусыРассылокКлиентам.Черновик Тогда
		...
		ТекстЛегенды = НСтр("ru = 'Рассылка находится в статусе ""Черновик"". В отчет выведены возможные адресаты рассылки и их контактная информация.
		                           ...
		                           |en = 'The mailout is in the ""Draft"" status. Available mailout recipients and their contact details are displayed in the report.
		                           |If contact details are not specified or specified incorrectly, mailout to these recipients
		                           |will not be sent. You can change the required contact details by double-clicking a mailout recipient.'");
	...
	ИначеЕсли ДанныеРассылки.Статус = Перечисления.СтатусыРассылокКлиентам.КОтправке Тогда
		...
		ТекстЛегенды = НСтр("ru = 'Рассылка находится в статусе ""К отправке"" и запланирована %1
		                          ...
		                          |en = 'Mailout has the ""For dispatch"" status and is scheduled for %1 ""
		                          |""The report contains available recipients and their contact details.
		                          |If contact details are not specified or specified incorrectly, mailout to these recipients
		                          |will not be performed. You still can change the contact details of the required kind by double-clicking a mailout recipient.'");
		
		Если ДанныеРассылки.ДатаАктуальности = Дата(1, 1, 1) Тогда
			ТекстПериод = НСтр("ru = 'на';
								|en = 'to'") + " " + Формат(ДанныеРассылки.ДатаРассылки,"ДЛФ=DD");
		Иначе
			...
			Если ДанныеРассылки.ДатаАктуальности < ТекущаяДатаСеанса() Тогда
				ТекстЛегенды = НСтр("ru = 'Рассылка находится в статусе ""К отправке"" и была запланирована %1
				                  ...
				                  |В отчет выведены возможные адресаты рассылки и их контактная информация.';
				                  |en = 'The mailout has the ""For dispatch"" status, and was scheduled for %1
				                  |This date has expired and the mailout will not be performed.
				                  |Available mailout recipients and their contact details are displayed in the report.'");
			КонецЕсли;
	...
	ИначеЕсли ДанныеРассылки.Статус = Перечисления.СтатусыРассылокКлиентам.Обрабатывается Тогда
		...
		ТекстЛегенды = НСтр("ru = 'Рассылка находится в статусе ""Обрабатывается"". В данный момент создаются сообщения адресатам рассылки.
		                           ...
		                           |en = 'The mailout has the ""In progress"" status. Messages to mailout recipients are being created.
		                           |Available mailout recipients and their contact details are displayed in the report. If contact details are not specified
		                           |or specified incorrectly, messages to these recipients will not be created.'");
	...
	ИначеЕсли ДанныеРассылки.Статус = Перечисления.СтатусыРассылокКлиентам.Выполнена Тогда
		...
		ТекстЛегенды = НСтр("ru = 'Рассылка находится в статусе ""Выполнена"". Все возможные сообщения созданы.
		                           ...
		                           |en = 'The mailout has the ""Completed"" status. All messages are created.
		                           |Messages were not created if contact details were not specified or were specified incorrectly.
		                           |The report may not contain the created messages if the message retention period has expired.'");
