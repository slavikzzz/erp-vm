...
Процедура СоздатьДокументыЗаПериод(Период, Организация = Неопределено, Отказ = Ложь) Экспорт
	...
	ПроверитьБлокировкуВходящихДанныхПриОбновленииИБ(НСтр("ru = 'Распределение доходов и расходов по направлениям деятельности';
															|en = 'Allocation of income and expenses by lines of business'"));
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При проведении документа ""%1"" были обнаружены ошибки:
						|%2';
						|en = 'Errors occurred when posting the ""%1"" document:
						|%2'", ОбщегоНазначения.КодОсновногоЯзыка()),
					СокрЛП(ДокументРаспределения),
					РасчетСебестоимостиПротоколРасчета.ПредставлениеМногострочногоТекста(ТекстОшибки, " - "));
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			ЗакрытиеМесяцаСервер.ЗарегистрироватьПроблемуВыполненияРасчета(
				...
				НСтр("ru = 'При распределение доходов и расходов по организации ""%1"" за период %2 были диагностированы ошибки';
					|en = 'Errors were found when allocating income and expenses of the ""%1"" company for period %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
				,
				ТекстОшибки,
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Нет оборотов по регистрам прочих доходов за период %1';
					|en = 'No turnovers for registers of income for period %1'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Распределение доходов по направлениям деятельности за период %1 завершилось с ошибкой:
					|%2';
					|en = 'Income allocation by lines of business for the period %1 ended with the error:
					|%2'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОписаниеПроверок_РаспределениеДоходовПоНаправлениямДеятельности(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Не распределены прочие доходы на финансовый результат';
			|en = 'Income for the financial result is not allocated'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Все прочие доходы должны быть распределены на финансовый результат.';
			|en = 'All income must be allocated for the financial result.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаОстатковПрочихДоходов(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("СтатьяДоходов", 			НСтр("ru = 'Статья доходов';
															|en = 'Income item'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("АналитикаДоходов", 		НСтр("ru = 'Аналитика доходов';
															|en = 'Income dimension'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'По организации ""%1"" на конец периода %2 есть остатки по регистру прочих доходов.';
			|en = 'There is balance by income register by the ""%1"" company as of the end of the period %2.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		,
...
Функция ПредставлениеРаннихДокументов() Экспорт
	...
	Возврат НСтр("ru = 'Распределение доходов и расходов';
				|en = 'Allocation of income and expenses'");
...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат РаспределениеДоходовИРасходовПоНаправлениямДеятельностиЛокализация.ТекстОтраженияВРеглУчете();
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	Возврат РаспределениеДоходовИРасходовПоНаправлениямДеятельностиЛокализация.ТекстЗапросаВТОтраженияВРеглУчете();
			 ...
			 И ФормироватьФинансовыйРезультат Тогда
				...
				Если МассивСтрокТаблицы[0].Доходы <> 0
				 ...
				 ИЛИ МассивСтрокТаблицы[0].ДоходыУпр <> 0 Тогда
					...
					Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Отсутствует база для распределения доходов по способу распределения ""%1"". Укажите другой способ распределения или удалите строку доходов из документа.';
							|en = 'No base for income allocation according to the ""%1"" allocation method. Specify another allocation method or delete the income line from the document.'", КодЯзыка),
					...
					Если ВыводитьСообщенияВЖурналРегистрации Тогда
						ЗаписьЖурналаРегистрации(
							НСтр("ru = 'Ошибка при проведении документа';
								|en = 'An error occurred while posting the document'", КодЯзыка), 
				...
				Если МассивСтрокТаблицы[0].Расходы <> 0
				 ...
				 ИЛИ МассивСтрокТаблицы[0].ВременнаяРазница <> 0 Тогда
					...
					Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Отсутствует база для распределения расходов по способу распределения ""%1"". Укажите другой способ распределения или удалите строку расходов из документа.';
							|en = 'No base for expense allocation according to the ""%1"" allocation method. Specify another allocation method or delete the expense line from the document.'", КодЯзыка),
						ВыборкаПоСпособуРаспределения.СпособРаспределения);
