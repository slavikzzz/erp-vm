...
Функция ПредставлениеСчетаФактуры(Номер, Дата, Проведен) Экспорт
	...
	МассивПодстрок.Добавить(СтрШаблон(НСтр("ru = 'Счет-фактура № %1 от %2 г.';
											|en = 'Tax invoice No.%1 dated %2'"), СокрЛП(Номер), Формат(Дата, "ДЛФ=D")));
...
//
// Возвращаемое значение:
//	РеквизитыСчетаФактуры -Структура - Реквизиты счета-фактуры
//
...
//	* Сумма документа - Число - сумма счета-фактуры в валюте
//
...
#Область НачислениеНДСНалоговогоАгента       // (Дт 76.НА :: Кт 68.52)
	...
	|
	|	ЗаписиКнигиПродаж.НДС КАК Сумма,
	|	ЗаписиКнигиПродаж.НДСУпр КАК СуммаУУ,
	|
	...
	|	ПО
	|		ЗаписиКнигиПродаж.Регистратор = Операция.Ссылка
	|		И НЕ ЗаписиКнигиПродаж.РегламентнаяОперация
	|		И ЗаписиКнигиПродаж.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.НДСНачисленКУплате)
	|		И ЗаписиКнигиПродаж.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ТоварыНалоговыйАгент)
	|";
#КонецОбласти

#Область ЗакупкаНДСНалоговогоАгента         // (Дт 19.03 :: Кт 76.НА)
	ЗакупкаНДСНалоговогоАгента = "
	|ВЫБРАТЬ //// НДС при приобретении по рег. НДСПредъявленный (Дт 19.03 :: Кт 76.НА) @НДСПредъявленный
	|
	|	Операция.Ссылка КАК Ссылка,
...
Функция ТекстЗапросаТаблицаОтражениеДокументовВРеглУчете(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&Организация                            КАК Организация,
	|	НАЧАЛОПЕРИОДА(&ДатаСоставления, ДЕНЬ)   КАК ДатаОтражения
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	|	&Организация                            КАК Организация,
	|	НАЧАЛОПЕРИОДА(ВЫБОР
	|		КОГДА &ДатаПереходаПраваСобственности > &Период
...
Функция СформироватьГиперссылкуКОформлению(Параметры) Экспорт
	...
	Иначе
		ТекстГиперссылки = НСтр("ru = 'Счета-фактуры полученные (налоговый агент)';
								|en = 'Received tax invoices (tax agent)'");
	КонецЕсли;	
