...
Процедура РасшифровкаСуммыНалоговаяБазаПриИзменении(Элемент)
	...
	ТекущаяСтрока.Сумма = ТекущаяСтрока.НалоговаяБаза - ТекущаяСтрока.СуммаНДС;
...
Процедура РасшифровкаСуммыСтавкаНДСПриИзменении(Элемент)
	...
	ТекущаяСтрока = Элементы.ДокументыОснования.ТекущиеДанные;
	СтавкаНДСПроцентом = УчетНДСУПКлиентСервер.ЗначениеСтавкиНДС(ТекущаяСтрока.СтавкаНДС);
	ТекущаяСтрока.СуммаНДС = ТекущаяСтрока.Сумма * СтавкаНДСПроцентом / 100;   
	ТекущаяСтрока.НалоговаяБаза = ТекущаяСтрока.Сумма + ТекущаяСтрока.СуммаНДС;
	...
	Иначе
		...
		Если КоличествоДокументов = 1 Тогда
			...
			Если РазрешеноИзменение Тогда
				...
				МассивСтрок.Добавить(Новый ФорматированнаяСтрока(
					НСтр("ru = '(Добавить)';
						|en = '(Add)'"),  , ЦветаСтиля.ГиперссылкаЦвет, , "ИзменитьДокументыОснования"));
			КонецЕсли;
		Иначе
	...
	Если ТоварПолучен И Объект.ДокументыОснования.Количество() И Не ЗначениеЗаполнено(Объект.ДатаПереходаПраваСобственности) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""переход права"" не заполнено';
				|en = 'The ""right transfer"" field is required'"),
			,
