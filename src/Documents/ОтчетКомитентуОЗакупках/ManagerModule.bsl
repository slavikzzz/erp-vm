	...
	Если Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС Тогда
		...
		Пока СтрокаКорректировки.Следующий() Цикл
			ТекстСообщения = НСтр("ru = 'В одном отчете не может быть документа приобретения и исправительной корректировки по нему. Необходимо сделать раздельные отчеты.';
									|en = 'One report cannot have a purchase document and a corrective adjustment for it. Create separate reports.'");
...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат ОтчетКомитентуОЗакупкахЛокализация.ТекстОтраженияВРеглУчете();
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	Возврат ОтчетКомитентуОЗакупкахЛокализация.ТекстЗапросаВТОтраженияВРеглУчете();
...
Функция ТекстЗапросаТаблицаЗаказыКлиентов(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	&Период КАК Период,
	|	ТаблицаТовары.Назначение.Заказ КАК ЗаказКлиента,
	|	ТаблицаТовары.Номенклатура КАК Номенклатура,
	|	ТаблицаТовары.Характеристика КАК Характеристика,
	|	ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка) КАК Серия,
	|	0 КАК КодСтроки,
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Изменение", Метаданные.Документы.ОтчетКомитентуОЗакупках) Тогда
		...
		КомандаПечати.Представление = НСтр("ru = 'Акт об оказании услуг агентской закупки';
											|en = 'Certificate of delivered agent purchase services'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	КонецЕсли;
...
Функция КомплектПечатныхФорм() Экспорт
	...
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ОтчетКомитентуОЗакупках", НСтр("ru = 'Отчет комитенту (принципалу) о закупках';
																																	|en = 'Purchase notification'"), 2);
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		УстановитьПараметр(ЗаголовокТаблицы, "ТекстЗаголовка",
			ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(ДанныеПечати, НСтр("ru = 'Акт об оказании услуг';
																							|en = 'Certificate of rendered services'", ОбщегоНазначения.КодОсновногоЯзыка())));
		...
		ТекстОСуммеПрописью = НСтр("ru = 'Сумма комиссионного вознаграждения составила';
									|en = 'Commission equals'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
			+ ?(ДанныеПечати.Сумма < 0, НСтр("ru = 'минус';
											|en = 'minus'", ОбщегоНазначения.КодОсновногоЯзыка()) + " ", "")
			+ РаботаСКурсамиВалютУТ.СформироватьСуммуПрописью(ДанныеПечати.Сумма, ДанныеПечати.Валюта) + ","
			+ Символы.ПС
			+ НСтр("ru = 'в том числе НДС (';
					|en = 'including VAT ('", ОбщегоНазначения.КодОсновногоЯзыка())	+ ДанныеПечати.СтавкаНДС + ") "
...
Функция ПолучитьДанныеДляПечатнойФормыОтчетПоЗакупкам(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Заголовок = НСтр("ru = 'Отчет комитенту о закупках';
					|en = 'Purchase notification'", ОбщегоНазначения.КодОсновногоЯзыка());
