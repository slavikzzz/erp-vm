			...
			Если Объект.Товары.Количество() > 0 Тогда
				...
				ПоказатьВопрос(
						...
						НСтр("ru = 'Перезаполнить таблицу товаров по документу приобретения?';
							|en = 'Do you want to refill the goods table by the purchase document?'"),
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОтчетПоПриобретению
		...
		ТекстСообщения = НСтр("ru = 'Не указан документ приобретения. Необходимо указать документ или отказаться от разбиения.';
								|en = 'Purchase document is not specified. Specify the document or do not split.'");
...
Процедура УстановитьПредставлениеДокументаОснования()
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.Договор)
		...
		ПредставлениеПриобретения = Новый ФорматированнаяСтрока(НСтр("ru = '<не выбран>';
																	|en = '<Not selected>'"),,ЦветаСтиля.ЦветОсобогоТекста,,"ВыбратьДокумент");
		УказанДокументПриобретения = Истина;
	Иначе
		Объект.ДокументПриобретения = Неопределено;
	КонецЕсли;
...
Процедура ОбновитьТекстСчетаФактурыВыданные()
	...
	Если Объект.СпособРасчетаВознаграждения = ПредопределенноеЗначение("Перечисление.СпособыРасчетаКомиссионногоВознаграждения.НеРассчитывается") Тогда
		ТекстСчетФактура = НСтр("ru = 'Оформление счета-фактуры не требуется (Комиссионное вознаграждение)';
								|en = 'Tax invoice registration is not required (Commission)'");
...
Процедура ОбновитьТекстСчетаФактурыПеревыставленные()
	...
	Если Объект.Проведен Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Если КоличествоВыданы = 0 И КоличествоПеревыставить = 0 Тогда
				МассивГиперСсылок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Перевыставление счетов-фактуры комитенту (принципалу) не требуется.';
																			|en = 'Re-issuance of the tax invoices to the consignor (principal) is not required.'"), , ЦветаСтиля.ЦветТекстаФормы,, ));
			ИначеЕсли КоличествоВыданы = 0 И КоличествоПеревыставить = 1 Тогда
				...
				ТекстКПеревыставлению = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Требует переоформления %1 комитенту (принципалу)';
																									|en = 'Requires to reregister %1 to consignor'"), ТекстСчетФактура);
			...
			ИначеЕсли КоличествоВыданы > 1 И КоличествоПеревыставить = 0 Тогда
				ТекстВыданы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Перевыставлены (%1) счета-фактуры комитенту (принципалу)';
																							|en = 'Tax invoices are reissued (%1) to consignor'"), КоличествоВыданы);
			...
			ИначеЕсли КоличествоВыданы = 0 И КоличествоПеревыставить > 1 Тогда
				ТекстКПеревыставлению = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Требуют переоформления/не оформлены (%1) счета-фактуры комитенту (принципалу)';
																									|en = 'Tax invoices to consignor require reregistration/are not registered (%1)'"), КоличествоПеревыставить);
				МассивГиперСсылок.Добавить(Новый ФорматированнаяСтрока(ТекстКПеревыставлению, , ЦветаСтиля.ГиперссылкаЦвет,, ?(ДоступКСчетуФактуреВыданному, 
					"ОткрытьРабочееМестоДляПеревыставленияСчетовФактурКОформлению", "НетДОступаКПервыставлениюСчетовФактур")));
			Иначе
				ТекстКПеревыставлению = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Требуют переоформления/не оформлены (%1) счета-фактуры комитенту (принципалу)';
																									|en = 'Tax invoices to consignor require reregistration/are not registered (%1)'"), КоличествоПеревыставить);
				ТекстВыданы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Перевыставлены (%1)';
																							|en = 'Reissued (%1)'"), КоличествоВыданы);
				МассивГиперСсылок.Добавить(Новый ФорматированнаяСтрока(ТекстКПеревыставлению, , ЦветаСтиля.ГиперссылкаЦвет,, ?(ДоступКСчетуФактуреВыданному, 
					"ОткрытьРабочееМестоДляПеревыставленияСчетовФактурКОформлению", "НетДОступаКПервыставлениюСчетовФактур")));
	...
	Иначе
		МассивГиперСсылок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Для перевыставления счетов-фактур комитенту (принципалу) необходимо провести документ.';
																	|en = 'To reissue tax invoices to the consignor, post the document.'"), , ЦветаСтиля.ЦветТекстаФормы,, ));
	КонецЕсли;	
	//-- Локализация
...
Процедура ТекстСчетаФактурыПеревыставленныеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "НетДОступаКПервыставлениюСчетовФактур" Тогда
		...
		ТекстОшибки = Нстр("ru = 'Нет прав на доступ к счетам-фактурам выданным';
							|en = 'You are unauthorized to access the tax invoices issued'");
