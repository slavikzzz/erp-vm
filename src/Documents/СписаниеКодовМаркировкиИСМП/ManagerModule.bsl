			...
			ИначеЕсли Не ЗначениеЗаполнено(СтрокаДанныхОтчета.ПолныйКодМаркировки) Тогда
				...
				ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
					...
					СтрШаблон(НСтр("ru = 'Не найден полный код маркировки для кода %1.';
									|en = 'Не найден полный код маркировки для кода %1.'"), СтрокаДанныхОтчета.КодМаркировки));
			Иначе
		...
		Для Каждого СтрокаИсточникаДанных Из ИсточникДанных Цикл
			...
			Если НайденныеСтроки.Количество() = 1 Тогда
				...
				Если Не ЭтоМех Тогда
					...
					КодМаркировкиДляПередачиИСМП = ОбменДаннымиИСМП.КодМаркировкиДляПередачиИСМП(РезультатРазбора, ПараметрыНормализацииПрочее);
				КонецЕсли;
