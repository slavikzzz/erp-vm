			...
			Попытка
				...
				Иначе
					...
					ДокументОбъект.Комментарий  = НСтр("ru = 'Документ создан автоматически регламентным заданием загрузки остатков партий.';
														|en = 'Документ создан автоматически регламентным заданием загрузки остатков партий.'");
				КонецЕсли;
...
Функция СообщениеЗагрузкиПартий(ДокументСсылка, ДополнительныеПараметры)
	...
	Для Каждого ЗапрашиваемыйСтатус Из ЗапрашиваемыеСтатусы Цикл
		...
		СообщениеXML = СообщенияЗагрузкиПартий(
			НовыеПараметрыФормирования, ДополнительныеПараметры.ПараметрыОбмена, АбстрактноеСообщениеXML);
...
Функция СообщенияЗагрузкиПартий(ПараметрыФормирования, ПараметрыОбмена, КешОшибок = Неопределено)
	...
	Если ЗначениеЗаполнено(ПараметрыФормирования.ЗапрашиваемыйСтатус) Тогда
		...
		ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСвойствоXDTO(ЗапросПартийXDTO, "status", СтатусПартииЗерна, КешОшибок);
		ДанныеОписания.Добавить(СтрШаблон(НСтр("ru = '(статус: %1)';
												|en = '(статус: %1)'"), ПараметрыФормирования.ЗапрашиваемыйСтатус));
	...
	Если ПараметрыФормирования.РеквизитыВладельцаПартии <> Неопределено Тогда
		ВладелецПартииXDTO = ИнтеграцияЗЕРНОСлужебный.ОрганизацияКонтрагентXDTO(
			...
			КешОшибок,
			ПараметрыОбмена,
			СтрШаблон(НСтр("ru = 'Владелец партии %1';
							|en = 'Владелец партии %1'"), Строка(ПараметрыФормирования.ВладелецПартии)));
		Если ВладелецПартииXDTO <> Неопределено Тогда
			ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСвойствоXDTO(ЗапросПартийXDTO, "Owner", ВладелецПартииXDTO, КешОшибок);
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыФормирования.РеквизитыЭлеватора <> Неопределено Тогда
		ЭлеваторXDTO = ИнтеграцияЗЕРНОСлужебный.ОрганизацияКонтрагентXDTO(
			...
			КешОшибок,
			ПараметрыОбмена,
			СтрШаблон(НСтр("ru = 'Элеватор %1';
							|en = 'Элеватор %1'"), Строка(ПараметрыФормирования.Элеватор)));
		Если ЭлеваторXDTO <> Неопределено Тогда
			ИнтеграцияЗЕРНОСлужебный.ЗаполнитьСвойствоXDTO(ЗапросПартийXDTO, "Repository", ЭлеваторXDTO, КешОшибок);
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрыФормирования.НомерПартии) Тогда
		...
		ДанныеОписания.Добавить(СтрШаблон(НСтр("ru = '№ %1';
												|en = '№ %1'"), ПараметрыФормирования.НомерПартии));
	КонецЕсли;
