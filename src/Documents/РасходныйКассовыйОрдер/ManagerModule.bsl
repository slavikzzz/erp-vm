...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат РасходныйКассовыйОрдерЛокализация.ТекстОтраженияВРеглУчете();
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	Возврат РасходныйКассовыйОрдерЛокализация.ТекстЗапросаВТОтраженияВРеглУчете();
...
Процедура ЗаполнитьНастройкиПолейФормы(Настройки) Экспорт
	...
	Финансы.НовыйОтбор(Элемент.Условие, "ХозяйственнаяОперация", Операции.ВыдачаДенежныхСредствВДругуюКассу);
...
Функция ЗначенияПараметровПроведения(Реквизиты = Неопределено)
	...
	Если Реквизиты <> Неопределено Тогда
		...
		Значения.Вставить("Подразделение", ПодразделениеДокумента(Реквизиты));
...
Функция ТекстЗапросаВременнаяТаблицаРасшифровкаПлатежа(Запрос, ТекстыЗапроса, ДокументОбъект = Неопределено) Экспорт
	...
	//++ НЕ УТ
	|	ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка),
	//-- НЕ УТ
...
Функция ТекстЗапросаТаблицаДвиженияДенежныеСредстваДоходыРасходы(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаАрендодателю Тогда
		...
		|	ТаблицаРасшифровкаПлатежа.НаправлениеДеятельности,
		|	ТаблицаРасшифровкаПлатежа.ДоговорАренды,
		|	ТаблицаРасшифровкаПлатежа.ДоговорАренды.Подразделение,
		|	ТаблицаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств,
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Из реквизита ""(не используется) Кассир"" переносит физическое лицо в новый реквизит ""Кассир"".';
									|en = 'Transfers a person from the ""(not used) Cashier"" attribute to the new ""Cashier"" attribute.'" );
...
Функция ПодразделениеДокумента(ДанныеДокумента)

	Если ДанныеДокумента.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствВДругуюКассу Тогда
		Подразделение = ДенежныеСредстваСервер.ИнициаторВыдачиНаличных(
			ДанныеДокумента.РаспоряжениеНаПеремещениеДенежныхСредств, ДанныеДокумента.КассаПолучатель);
	Иначе
