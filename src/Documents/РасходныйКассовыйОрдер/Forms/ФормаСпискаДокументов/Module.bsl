...
Процедура УправлениеЭлементамиФормы()
	...
	//++ НЕ УТ
	Элементы.СоздатьОплатуАрендодателю.Видимость                = ИспользоватьАренду;
	//-- НЕ УТ
...
Процедура Подключаемый_ПриСозданииНаСервереЛокализация(Отказ, СтандартнаяОбработка)
	...
	Если ИспользоватьНачислениеЗарплаты Тогда
		...
		СвойстваСписка.ТекстЗапроса = ТекстЗапросаВыплатаЗарплаты();
		ОбщегоНазначения.УстановитьСвойстваДинамическогоСписка(Элементы.ВыплатаЗарплаты, СвойстваСписка);
	КонецЕсли;
	//-- НЕ УТ
...
Функция ТекстЗапросаВыплатаЗарплаты()
	...
	|			И Зарплата.ФизическоеЛицо = ОплатаВедомостей.ФизическоеЛицо
	|ГДЕ
	|	ОплатаВедомостей.Ведомость ЕСТЬ NULL 
	|	И Зарплата.Ссылка.Проведен
	|	И (Зарплата.ФизическоеЛицо = &Работник
	...
	|ВЫБРАТЬ
	|	Выплаты.Ссылка,
	|	Выплаты.Ссылка.Организация,
	|	ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка),
	|	Неопределено,
	|	СУММА(Выплаты.КВыплате),
	|	ЗНАЧЕНИЕ(Документ.ЗаявкаНаРасходованиеДенежныхСредств.ПустаяСсылка)
	|ИЗ
	|	Документ.ВедомостьПрочихДоходовВКассу.Выплаты КАК Выплаты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОплатаВедомостейНаВыплатуЗарплаты КАК ОплатаВедомостей
	|		ПО Выплаты.Ссылка = ОплатаВедомостей.Ведомость
	|			И Выплаты.ФизическоеЛицо = ОплатаВедомостей.ФизическоеЛицо
	|ГДЕ
	|	ОплатаВедомостей.Ведомость ЕСТЬ NULL 
	|	И Выплаты.Ссылка.Проведен
	|	И (Выплаты.ФизическоеЛицо = &Работник
	|			ИЛИ &ВыплатаПоВедомости)
	|
	|СГРУППИРОВАТЬ ПО
	|	Выплаты.Ссылка,
...
Процедура УстановитьПараметрыДинамическихСписковЛокализация()
	...
	Если Константы.ИспользоватьНачислениеЗарплаты.Получить() Тогда
		...
		ВыплатаЗарплаты.Параметры.УстановитьЗначениеПараметра(
			"ВыплатаПоВедомости", ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыЧерезКассу);
	КонецЕсли;
	//-- НЕ УТ
	Возврат;
...
Процедура УправлениеЭлементамиФормыЛокализация()
	...
	//++ НЕ УТ
	Элементы.СоздатьВыдачуЗаймаСотруднику.Видимость = Не ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплаты") Или 
