...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыНастройки = Документы.РасходныйКассовыйОрдер.ПараметрыНастройкиСчетовУчета(Объект.ХозяйственнаяОперация);
...
Процедура ХозяйственнаяОперацияПриИзмененииКлиент(Оповещение)
	...
	Если Объект.ВыплатаЗаработнойПлаты.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ru = 'Список работников будет очищен. Продолжить?';
							|en = 'Employee list will be cleared. Continue?'");
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	//++ НЕ УТ
	ПараметрыНастройкиСчетовУчета = Документы.РасходныйКассовыйОрдер.ПараметрыНастройкиСчетовУчета(Объект.ХозяйственнаяОперация);
...
Процедура ВедомостьНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.ЗаявкаНаРасходованиеДенежныхСредств) Тогда
		СтруктураПараметров.Вставить("ЗаявкаНаОплату", Объект.ЗаявкаНаРасходованиеДенежныхСредств);
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ведомость) И ИспользоватьНачислениеЗарплаты Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Ведомость"" не заполнено';
				|en = 'The ""Paysheet"" field is not filled in'"),,
			"Ведомость",
			"Объект",
			Отказ);
	КонецЕсли;
...
Процедура РасшифровкаПлатежаДоговорАрендыПриИзменении(Элемент)
	...
	//++ НЕ УТ
	РасшифровкаПлатежаДоговорАрендыПриИзмененииНаСервере();
...
&НаСервере
Процедура РасшифровкаПлатежаДоговорАрендыПриИзмененииНаСервере()
	...
	ТекущаяСтрока.ТипПлатежаПоАренде = Справочники.ДоговорыАренды.ТипыПлатежейПоДоговору(ТекущаяСтрока.ДоговорАренды)[0];
...
Процедура Подключаемый_ОбработкаВыбораЛокализация(РезультатВыбора, ИсточникВыбора)
	...
	//++ НЕ УТ
	Если ИсточникВыбора.ИмяФормы = "Документ.РасходныйКассовыйОрдер.Форма.ФормаВыплатаЗаработнойПлаты" Тогда
...
Процедура ХозяйственнаяОперацияПриИзмененииСерверЛокализация()
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыРаботнику") Тогда
		ЗаполнитьСуммыНевыплаченнойЗарплатыНаСервере();
	КонецЕсли;
...
Процедура ОбновитьОтображениеСтрокаФизическиеЛица()
	...
	СтрокаФизическиеЛица = НСтр("ru = 'Список работников';
								|en = 'Employee list'") + " (" + Объект.ВыплатаЗаработнойПлаты.Количество() + ")";
...
&НаСервере
Процедура ЗаполнитьСуммыНевыплаченнойЗарплатыНаСервере()
	...
	СтруктураПараметров.Вставить("МассивВедомостей",
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.Ведомость));
	
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыРаботнику Тогда
	...
	|ГДЕ
	|	НЕ ДанныеВедомости.Депонирована ЕСТЬ NULL
	|		ИЛИ ДанныеВедомости.СуммаОплаты = 0
	|
	|СГРУППИРОВАТЬ ПО
	|	ФизическоеЛицо";
	
	ТаблицаВыплаты = Запрос.Выполнить().Выгрузить();
	Объект.СуммаДокумента = ТаблицаВыплаты.Итог("Сумма");
	
	Если НЕ Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыРаботнику Тогда
		Объект.ВыплатаЗаработнойПлаты.Загрузить(ТаблицаВыплаты);
	КонецЕсли;
