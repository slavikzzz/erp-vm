		...
		Если ИспользоватьАктыВыполненныхРаботПоНесколькимЗаказам Тогда
			...
			Если Объект.Услуги.Количество() = 0 Тогда
				ТекстВопроса = НСтр("ru = 'Список Услуги будет перезаполнен остатками неоказанных услуг по заказам. Продолжить?';
									|en = 'List Services will be refilled with non-provided services by orders. Continue?'");
			Иначе
				ТекстВопроса = НСтр("ru = 'Строки в списке Услуги будут привязаны к строкам заказов. Строки, которые невозможно привязать к строкам заказа, будут удалены. Продолжить?';
									|en = 'Lines in the Services list will be linked to order lines. Lines that cannot be linked to order lines will be deleted. Continue?'");
			КонецЕсли;
		...
		Если Объект.Услуги.Количество() > 0 Тогда
			...
			Если ИспользоватьАктыВыполненныхРаботПоНесколькимЗаказам Тогда
				ТекстВопроса = НСтр("ru = 'Строки в списке Услуги перестанут быть связанными со строками заказов. Продолжить?';
									|en = 'Lines in the Services list will not be associated with order lines. Continue?'");
			Иначе
...
Процедура АктВыполненныхРаботПоЗаказамПриИзмененииПослеВыбора(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Объект.Услуги.Количество() = 0 Тогда
		ТекстВопроса = СтрЗаменить(НСтр(
			...
			|en = 'List Services will be refilled with non-generated services by document %ЗаказКлиента%. Continue?'"),
	...
	Иначе
		ТекстВопроса = СтрЗаменить(НСтр(
			...
			|en = 'Lines in list Services will be linked to lines of document %ЗаказКлиента%. Lines that cannot be linked will be deleted. Continue?'"),
			"%ЗаказКлиента%", ЗаказКлиента);
	КонецЕсли;
...
Процедура ЗаполнитьЦеныПоСоглашению(Команда)
	...
	Если ЦеныПредприятияЗаполнениеКлиент.НеобходимоЗаполнениеЦенПоСоглашению(Объект, "Услуги", НСтр("ru = 'Услуги';
																									|en = 'Services'")) Тогда
...
Процедура ЗаполнитьЦеныВыделенныхСтрокПоВидуЦен(Команда)
	...
	Если ЦеныПредприятияЗаполнениеКлиент.НеобходимоЗаполнениеЦенПоВидуЦен(Объект, "Услуги", НСтр("ru = 'Услуги';
																								|en = 'Services'")) Тогда
...
Процедура НазначитьРучнуюСкидку(Команда)
	...
	Если Не СкидкиНаценкиЗаполнениеКлиент.ВозможноНазначениеРучнойСкидкиНаценки(Объект, "Услуги", НСтр("ru = 'Услуги';
																										|en = 'Services'")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОтменитьРучныеСкидки(Команда)
	...
	Если Не СкидкиНаценкиЗаполнениеКлиент.ВозможнаОтменаРучныхСкидокНаценок(Объект, "Услуги", НСтр("ru = 'Услуги';
																									|en = 'Services'")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоЗаказу(Команда)
	...
	Если Объект.Услуги.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоЗаказуЗавершение", ЭтотОбъект), НСтр("ru = 'Таблица ""Услуги"" будет перезаполнена. Продолжить?';
																								|en = 'The ""Services"" table will be refilled. Continue?'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
...
Функция ПривязатьСтрокиАктовВыполненныхРаботКЗаказамКлиентов(ОтобратьПоЗаказу)
	...
	|		ЗаказыОстатки.ЗаказКлиента.ПорядокРасчетов КАК ПорядокРасчетов,
	...
	|		ЗаказыДвижения.ЗаказКлиента.ПорядокРасчетов,
