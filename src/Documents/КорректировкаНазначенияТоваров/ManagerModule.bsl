...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если Константы.ВыводитьДопКолонкиНаправлениеДеятельностиИГК.Получить() Тогда
		...
		КомандаПечати.МенеджерПечати = "Документ.КорректировкаНазначенияТоваров";
		КомандаПечати.Идентификатор = "КорректировкаНазначенияТоваров";
		КомандаПечати.Представление = НСтр("ru = 'Корректировка назначения товаров';
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировкаНазначенияТоваров") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, 
		                                                     "КорректировкаНазначенияТоваров",
		                                                     НСтр("ru = 'Корректировка назначения товаров';
																	|en = 'Inventory assignment adjustment'"), 
		                                                     ПечатнаяФормаКорректировкаНазначенияТоваров(МассивОбъектов, ОбъектыПечати, ПараметрыПечати));
	КонецЕсли;
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерий(ПараметрыУказанияСерий) Экспорт
	...
	// Вызов из заполнения коррекировки назначения для заказа переработчику по этапам
	|		КОГДА Товары.ИсходноеНазначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
...
Функция ТаблицаСнятияРезерва(Назначение, Товары, Регистратор) Экспорт
	...
	Ключ = "ИсходноеНазначение,Номенклатура,Характеристика,Склад";
	Строки = ТаблицаТоваров.Скопировать(, Ключ);
	ТоварыОрганизаций = ДавальческиеТоварыОрганизацийПоОтбору(Строки, Регистратор);
	ТоварыОрганизаций.Индексы.Добавить(Ключ);
	СократитьСкорректироватьНазначенияДавальческихМатериаловНаНазначениеДавальцаВЦелом(ТаблицаТоваров, ТоварыОрганизаций);
...
Функция ТаблицаСнятияРезерваПоМногимНазначениям(Организация, Товары, Регистратор, Назначения) Экспорт
	...
	ТоварыОрганизаций = ДавальческиеТоварыОрганизацийПоОтбору(Строки, Регистратор);
	ТоварыОрганизаций.Индексы.Добавить(Ключ);
	СократитьНазначенияДавальческихМатериаловПриСнятииРезерва(ТаблицаТоваров, ТоварыОрганизаций);
...
Функция ТаблицаРезервирования(Назначение, Источники, ТолькоСкладОтгрузки, Организация, Товары, Регистратор) Экспорт
	...
	Если Выборка.ТипНазначения = Перечисления.ТипыНазначений.Собственное Тогда
		
		Ключ = "ИсходноеНазначение,Номенклатура,Характеристика,Склад";
		Строки = ТаблицаТоваров.Скопировать(, Ключ);
		ТоварыОрганизаций = ДавальческиеТоварыОрганизацийПоОтбору(Строки, Регистратор);
		ТоварыОрганизаций.Индексы.Добавить(Ключ);
		СократитьНазначенияДавальческихМатериаловПриРезервировании(ТаблицаТоваров, ТоварыОрганизаций);
...
Функция ТекстЗапросаИсточникДанных(ВТалицеОтбораЕстьНазначение)
	...
	ТекстЗапроса =
		...
		|		0 КАК Запас,
		|		0 КАК Резерв,
		|		ТаблицаОстатков.Требуется КАК КЗаказу,
		|		ТаблицаОстатков.Требуется КАК Потребность
		|	ИЗ
		|		РегистрСведений.СтруктураЗаказа КАК ТаблицаОстатков
		|	ГДЕ
		...
		|				ИЗ
		|					ОтборТоваров КАК Отбор))
		|			И ТаблицаОстатков.ВидСтроки = ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Потребление)
		|			И ТаблицаОстатков.КлючСвязиПеремещение = &ПустойКлючСвязи
		|	
		//-- НЕ УТКА
		|) КАК Набор
		|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаСвободныеОстатки()
	...
	ТекстЗапроса =
		...
		|	
		|	ВЫБРАТЬ
		|		СтруктураЗаказаДвижения.Номенклатура КАК Номенклатура,
		|		СтруктураЗаказаДвижения.Характеристика КАК Характеристика,
		|		СтруктураЗаказаДвижения.Склад КАК Склад,
		|		СтруктураЗаказаДвижения.Требуется КАК Потребность,
		|		0 КАК Свободно,
		|		0 КАК Запас,
		|		0 КАК Резерв
		|	ИЗ
		|		РегистрСведений.СтруктураЗаказа КАК СтруктураЗаказаДвижения
		|	ГДЕ
		|		&ОстатокНаСкладе
		|			И (СтруктураЗаказаДвижения.Номенклатура, СтруктураЗаказаДвижения.Характеристика) В(
		|				ВЫБРАТЬ
		|					ОтборТоваров.Номенклатура   КАК Номенклатура,
		|					ОтборТоваров.Характеристика КАК Характеристика
		|				ИЗ
		|					ОтборТоваровИсточникаДанных КАК ОтборТоваров)
		|			И СтруктураЗаказаДвижения.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|			И СтруктураЗаказаДвижения.ВидСтроки = ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Потребление)
		|			И СтруктураЗаказаДвижения.КлючСвязиПеремещение = &ПустойКлючСвязи
		|	
		//-- НЕ УТКА
		|) КАК Набор
		|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаОбеспечениеЗаказов()
	...
	ТекстЗапроса =
		...
		|	
		|	ВЫБРАТЬ
		|		СтруктураЗаказаДвижения.Назначение КАК Назначение,
		|		СтруктураЗаказаДвижения.Номенклатура КАК Номенклатура,
		|		СтруктураЗаказаДвижения.Характеристика КАК Характеристика,
		|		СтруктураЗаказаДвижения.Склад КАК Склад,
		|		СтруктураЗаказаДвижения.Требуется КАК Потребность,
		...
		|		РегистрСведений.СтруктураЗаказа КАК СтруктураЗаказаДвижения
		|	ГДЕ
		|		СтруктураЗаказаДвижения.Назначение В(
		|				ВЫБРАТЬ
		|					Отбор.Назначение КАК Назначение
		|				ИЗ
		|					ОтборНазначенийИсточников КАК Отбор)
...
Процедура СократитьСкорректироватьНазначенияДавальческихМатериаловНаНазначениеДавальцаВЦелом(ТаблицаТовары, ТоварыОрганизаций)
	
	ПараметрыОтбора = Новый Структура("ИсходноеНазначение,Номенклатура,Характеристика,Склад");
	ВсегоСтрок = ТаблицаТовары.Количество();
	Для Счетчик = 1 По ВсегоСтрок Цикл
		
		Индекс = ВсегоСтрок - Счетчик;
		ТекСтрока = ТаблицаТовары[Индекс];
		ЗаполнитьЗначенияСвойств(ПараметрыОтбора, ТекСтрока);
		НайденныеСтроки = ТоварыОрганизаций.НайтиСтроки(ПараметрыОтбора);
		
		Если НайденныеСтроки.Количество() > 0 Тогда
			
			Количество = Макс(0, НайденныеСтроки[0].Количество);
			НазначениеДавальца = НайденныеСтроки[0].НазначениеДавальца;
			Резерв = ТекСтрока.Резерв;
			Потребность = ТекСтрока.Потребность;
			Если Количество < ТекСтрока.Запас Тогда
				
				Если НазначениеДавальца <> ТекСтрока.ИсходноеНазначение Тогда // давальческий вид запасов
					НоваяСтрока = ТаблицаТовары.Вставить(0);
					ВсегоСтрок = ВсегоСтрок + 1;
					ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока);
					НоваяСтрока.Запас = ТекСтрока.Запас - Количество;
					НоваяСтрока.Резерв = Мин(НоваяСтрока.Запас, НоваяСтрока.Резерв);
					Резерв = Резерв - НоваяСтрока.Резерв;
					НоваяСтрока.Потребность = Мин(НоваяСтрока.Запас, НоваяСтрока.Потребность);
					Потребность = Потребность - НоваяСтрока.Потребность;
					НоваяСтрока.НовоеНазначение = НазначениеДавальца;
				КонецЕсли;
				ТекСтрока.Запас = Количество; // собственный вид запасов
				ТекСтрока.Резерв = Резерв;
				ТекСтрока.Потребность = Потребность;
				Если ТекСтрока.Запас = 0 Тогда
					ТаблицаТовары.Удалить(ТекСтрока);
				КонецЕсли;
...
Процедура СократитьНазначенияДавальческихМатериаловПриСнятииРезерва(ТаблицаТовары, ТоварыОрганизаций)
	...
	Для Счетчик = 1 По ВсегоСтрок Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			...
			Если НазначениеДавальца = ТекСтрока.ИсходноеНазначение Тогда
				ТаблицаТовары.Удалить(ТекСтрока); // снятие резерва с давальческого назначения нужно выполнять через операцию "Снятие резерва"
			Иначе
				// Снимаем назначение на назначение давальца для возврата давальцу или переиспользования.
				ТекСтрока.НовоеНазначение = НазначениеДавальца;
			КонецЕсли;
...
Процедура СократитьНазначенияДавальческихМатериаловПриРезервировании(ТаблицаТовары, ТоварыОрганизаций)
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			...
			Если Количество < ТекСтрока.Запас Тогда
				...
				Если Количество = 0 Тогда
					ТаблицаТовары.Удалить(ТекСтрока);
				Иначе
					ТекСтрока.Резерв = Макс(0, ТекСтрока.Резерв - (ТекСтрока.Запас - Мин(Количество, ТекСтрока.Запас)));
					ТекСтрока.Запас = Мин(Количество, ТекСтрока.Запас);
				КонецЕсли;
...
Функция ДавальческиеТоварыОрганизацийПоОтбору(ТаблицаОтбора, КорректировкаНазначения)
	...
	Запрос.Текст = 
		...
		|ИНДЕКСИРОВАТЬ ПО
		|	ИсходноеНазначение, Номенклатура, Характеристика, Склад
		|;
		|
		|/////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		И ДавальческоеНазначение.Заказ = ТаблицаОтбора.ИсходноеНазначение.Заказ
		|		И ДавальческоеНазначение.НаправлениеДеятельности = ТаблицаОтбора.ИсходноеНазначение.НаправлениеДеятельности
		|		И ДавальческоеНазначение.ТипНазначения = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое2_5)
		|
		...
		|ГДЕ
		|	ТаблицаОтбора.ИсходноеНазначение.ТипНазначения В(
		|		ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.ДавальческоеМатериалыПодЭтап22),
		...
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	КлючАналитики, ИсходноеНазначение, Номенклатура, Характеристика, Склад
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ПОМЕСТИТЬ Ключи
		|ИЗ
		|	ТаблицаДавальческихСтрок КАК Таблица
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка
		|;
		|
		...
		|	Набор.ВидЗапасов КАК ВидЗапасов,
		...
		|				АналитикаУчетаНоменклатуры В(
		|					ВЫБРАТЬ
		|						Таблица.Ссылка КАК АналитикаУчетаНоменклатуры
		|					ИЗ
		|						Ключи КАК Таблица)) КАК ТоварыОрганизацийОстатки
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		ТаблицаСторно.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|		ТаблицаСторно.Организация КАК Организация,
		|		ТаблицаСторно.ВидЗапасов КАК ВидЗапасов,
		...
		|			ПО Ключи.Ссылка = ТаблицаСторно.АналитикаУчетаНоменклатуры
		|	ГДЕ
		...
		|ПОМЕСТИТЬ ОстаткиМожноСнятьОбособление
		|ИЗ
		|	ОстаткиТоваровОрганизаций КАК Остатки
		|ГДЕ
		|	НЕ Остатки.ВидЗапасов.ТипЗапасов В(
		...
		|	Таблица.Характеристика                 КАК Характеристика,
		|	Таблица.Склад                          КАК Склад,
		|	МАКСИМУМ(Таблица.НазначениеДавальца)   КАК НазначениеДавальца,
		|	СУММА(ЕСТЬNULL(Остатки.Количество, 0)) КАК Количество
		|ИЗ
		|	ТаблицаДавальческихСтрок КАК Таблица
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ОстаткиМожноСнятьОбособление КАК Остатки
		|		ПО Остатки.АналитикаУчетаНоменклатуры = Таблица.КлючАналитики
		|СГРУППИРОВАТЬ ПО
...
Процедура ДобавитьВременнуюТаблицуСортировкаСкладов(Запрос, ИмяИсточникДанных, ИмяТаблицаЗаполненияКорректировки)
	...
	Текст =
		...
		|		И Таблица.ВидСтроки = ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Потребление) 
		|		И Таблица.КлючСвязиПеремещение = &ПустойКлючСвязи
		|	
		//-- НЕ УТКА
		|ИНДЕКСИРОВАТЬ ПО
...
//++ НЕ УТКА
		Если ПолучитьФункциональнуюОпцию("ДинамическаяСтруктураЗаказовНаПроизводство") Тогда
			...
			|ГДЕ
			|	СтруктураЗаказа.Назначение В(&Назначение)
			|		И СтруктураЗаказа.Склад ССЫЛКА Справочник.Склады
...
//++ НЕ УТКА
		Если ПолучитьФункциональнуюОпцию("ДинамическаяСтруктураЗаказовНаПроизводство") Тогда
			...
			|	СтруктураЗаказа.Назначение В(&Назначение)
			|		И СтруктураЗаказа.Склад ССЫЛКА Справочник.Склады
			|		И (СтруктураЗаказа.Номенклатура, СтруктураЗаказа.Характеристика) В (
			|			ВЫБРАТЬ
			|				Товары.Номенклатура КАК Номенклатура,
			|				Товары.Характеристика КАК Характеристика
			|			ИЗ
...
Функция ПечатнаяФормаКорректировкаНазначенияТоваров(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КОРРЕКТИРОВКА_НАЗНАЧЕНИЯ";
	
	ДанныеДляПечати = ДанныеДляПечати(МассивОбъектов, ПараметрыПечати);
	
	ЗаполнитьТабличныйДокументКорректировкаНазначенияТоваров(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати);
					
	Возврат ТабличныйДокумент;
	
КонецФункции

Процедура ЗаполнитьТабличныйДокументКорректировкаНазначенияТоваров(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.КорректировкаНазначенияТоваров.ПФ_MXL_КорректировкаНазначенияТоваров");
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если НЕ ВыборкаПоДокументам.НайтиСледующий(СтруктураПоиска) Тогда
			...
			НСтр("ru = 'В документе %1 отсутствуют товары. Печать не требуется';
				|en = 'Goods are missing in the %1 document. Printing is not required'"),
			ДанныеПечати.Ссылка);
		...
		ВыборкаПоНДИГКОтправителя = ВыборкаПоДокументам.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		КоличествоРазныхОтправителей = ВыборкаПоНДИГКОтправителя.Количество();
		
		ПервыйЗаголовокТаблицы = Истина;
		...
		Пока ВыборкаПоНДИГКОтправителя.Следующий() Цикл
			...
			КоличествоРазныхПолучателей = ВыборкаПоНДИГКПолучателя.Количество();
			
			НДИГКСовпадают = Ложь;
			Если КоличествоРазныхОтправителей = 1 И КоличествоРазныхПолучателей = 1 Тогда
				НДИГКСовпадают = Истина;
			КонецЕсли;
			
			Если ВыводитьНДИГК Тогда
				Если НДИГКСовпадают Тогда
					ЗаголовокТаблицы = Макет.ПолучитьОбласть("ЗаголовокТаблицы");
					ШапкаТаблицы = Макет.ПолучитьОбласть("ШапкаТаблицы");
					ОбластьМакета  = Макет.ПолучитьОбласть("Строка"); 
				Иначе
					ЗаголовокТаблицы = Макет.ПолучитьОбласть("ЗаголовокТаблицыНДИГК");
					ОбластьМакета  = Макет.ПолучитьОбласть("СтрокаНДИГК");
				КонецЕсли;
			Иначе
				ЗаголовокТаблицы = Макет.ПолучитьОбласть("ЗаголовокТаблицы");
				ОбластьМакета  = Макет.ПолучитьОбласть("Строка");
			...
			Пока ВыборкаПоНДИГКПолучателя.Следующий() Цикл
				...
				Если ВыводитьНДИГК И НДИГКСовпадают Тогда
					ШапкаТаблицы.Параметры.Заполнить(ВыборкаПоНДИГКПолучателя);
					ТабличныйДокумент.Вывести(ШапкаТаблицы);
				КонецЕсли;

				Если ПервыйЗаголовокТаблицы Тогда
					ТабличныйДокумент.Вывести(ЗаголовокТаблицы);
				КонецЕсли;

				ПервыйЗаголовокТаблицы = Ложь;

				СтрокаТовары = ВыборкаПоНДИГКПолучателя.Выбрать();
...
Процедура ЗаполнитьРеквизитыШапки(ДанныеПечати, Макет, ТабличныйДокумент)
	...
	УстановитьПараметр(ОбластьМакета, "ПредставлениеОрганизации", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации));
...
Процедура ЗаполнитьРеквизитыСтрокиТовара(СтрокаТовары, ОбластьМакета, НомерСтроки, ВыводитьКодНоменклатуры = Истина, ВыводитьСерии = Ложь)
...
Функция ДанныеДляПечати(МассивОбъектов, ПараметрыПечати)
	...
	|		ПО ТаблицаТовары.ИсходноеНазначение = ИсходныеНазначения.Ссылка
	...
	|		ПО ТаблицаТовары.НовоеНазначение = НовыеНазначения.Ссылка
