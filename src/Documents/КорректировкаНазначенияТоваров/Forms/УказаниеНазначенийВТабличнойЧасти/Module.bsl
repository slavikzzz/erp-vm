...
Процедура ПеренестиВДокумент(Команда)
	...
	Если Подобрано <> Параметры.Подобрать Тогда
		Шаблон = НСтр("ru = 'Подобранное количество (%1 %3) не совпадает с количеством, которое необходимо подобрать (%2 %3). Команда не может быть выполнена.';
						|en = 'The picked quantity (%1 %3) does not match the quantity to be picked (%2 %3). Cannot execute the command.'");
		Текст = СтрШаблон(
			Шаблон,
...
Процедура ЗаполнитьТаблицуПриСозданииФормы()
	...
	ТекстЗапроса =
		...
		|		КОГДА НазначениеПолучатель.ТипНазначения = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое21)
		|			ТОГДА
		|				Таблица.Назначение.ТипНазначения = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое21)
		|						И ВЫРАЗИТЬ(НазначениеПолучатель.Заказ КАК Документ.ЗаказДавальца).Партнер = ВЫРАЗИТЬ(Таблица.Назначение.Заказ КАК Документ.ЗаказДавальца).Партнер
		|						И ВЫРАЗИТЬ(НазначениеПолучатель.Заказ КАК Документ.ЗаказДавальца).Договор = ВЫРАЗИТЬ(Таблица.Назначение.Заказ КАК Документ.ЗаказДавальца).Договор
...
Процедура УстановитьЗаголовкиПриСозданииФормы()
	...
	Если ЗначениеЗаполнено(Параметры.Характеристика) Тогда
		Заголовок = НСтр("ru = 'Указание назначений для товара: %1,%3 на складе: %2';
						|en = 'Specify assignments for goods: %1,%3 in the warehouse: %2'");
	...
	Иначе
		Заголовок = НСтр("ru = 'Указание назначений для товара: %1 на складе: %2';
						|en = 'Specify assignments for goods: %1 in the warehouse: %2'");
