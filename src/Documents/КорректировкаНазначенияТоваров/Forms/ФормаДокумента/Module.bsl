		...
		Если Объект.Товары.Количество() > 0 Тогда
			...
			ПоказатьВопрос(
				...
				НСтр("ru = 'Таблица товаров будет очищена. Продолжить?';
					|en = 'The Goods tab will be cleared. Do you want to continue?'"),
...
Процедура ВидОперацииОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.ВидОперации <> ВыбранноеЗначение Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ВидОперацииОбработкаВыбораЗавершение", ЭтотОбъект, ВыбранноеЗначение), НСтр("ru = 'Таблица товаров будет очищена. Продолжить?';
																															|en = 'The Goods tab will be cleared. Do you want to continue?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура ПодобратьТовары(Команда)
	...
	ПараметрЗаголовок = НСтр("ru = 'Подбор товаров в документ корректировки назначения';
							|en = 'Pick goods to the assignment adjustment document'");
		...
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкиНазначения.РезервироватьИКорректировать")) Тогда
			...
			Если Не ЗначениеЗаполнено(Объект.Назначение) Тогда
				...
				ТекстРезервированиеСнятие = ?(Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкиНазначения.СнятьРезерв")
					, НСтр("ru = 'Снятие резерва невозможно.';
							|en = 'Unable to remove from reserve.'"), НСтр("ru = 'Резервирование невозможно.';
																		|en = 'Assignment is impossible.'"));
				
				ТекстНазначениеЗаказ = ?(МогутБытьНазначенияБезЗаказа, 
					НСтр("ru = 'Необходимо выбрать назначение.';
						|en = 'Select the assignment.'"), НСтр("ru = 'Необходимо выбрать заказ.';
																		|en = 'Select the order.'"));
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкиНазначения.СнятьРезерв") Тогда
		...
		Если Не ЕстьТоварыКСнятиюРезерва() Тогда
			...
			ТекстПредупреждения = НСтр("ru = 'По данному назначению нет в наличии обособленных товаров. Команда не может быть выполнена.';
										|en = 'Assigned goods of this assignment are out of stock. Cannot execute the command.'");
		...
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкиНазначения.РезервироватьИКорректировать") Тогда
			...
			Если Не ЕстьТоварыКОбособленномуОбеспечению() Тогда
				...
				ТекстПредупреждения = НСтр("ru = 'По данному назначению нет потребностей. Необходимо добавить строки в документ вручную.';
											|en = 'No demands for this assignment. Add lines to the document manually.'");
				ПоказатьПредупреждение(, ТекстПредупреждения);
...
Процедура УказатьНазначения(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекстПредупреждения = НСтр("ru = 'Для указания назначений необходимо выбрать строку табличной части ""Товары""';
									|en = 'To specify assignments, select the ""Goods"" table row'");
				...
				Если ИспользоватьНесколькоСкладов Тогда
					Если ТекущиеДанные.ХарактеристикиИспользуются Тогда
						ТекстПредупреждения = НСтр("ru = 'Для указания назначений необходимо выбрать номенклатуру, характеристику, склад, количество в строке табличной части ""Товары"" и указать назначение в документе';
													|en = 'To specify assignments, select items, a variant, a warehouse, and quantity in the ""Goods"" table row and specify the assignment in the document'");
					Иначе
						ТекстПредупреждения = НСтр("ru = 'Для указания назначений необходимо выбрать номенклатуру, склад, количество в строке табличной части ""Товары"" и указать назначение в документе';
													|en = 'To specify assignments, select items, a warehouse, and quantity in the ""Goods"" table row and specify the assignment in the document'");
					КонецЕсли;
				Иначе
					Если ТекущиеДанные.ХарактеристикиИспользуются Тогда
						ТекстПредупреждения = НСтр("ru = 'Для указания назначений необходимо выбрать номенклатуру, характеристику, количество в строке табличной части ""Товары"" и указать назначение в документе';
													|en = 'To specify assignments, select items, a variant, and quantity in the ""Goods"" table row and specify the assignment in the document'");
					Иначе
						ТекстПредупреждения = НСтр("ru = 'Для указания назначений необходимо выбрать номенклатуру, количество в строке табличной части ""Товары"" и указать назначение в документе';
													|en = 'To specify assignments, select items, and quantity in the ""Goods"" table row and specify the assignment in the document'");
					КонецЕсли;
				КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.Номенклатура) Тогда
		ТекстПредупреждения = НСтр("ru = 'Для указания назначений необходимо выбрать номенклатуру в строке табличной части ""Товары""';
									|en = 'To specify assignments, select items in the ""Goods"" table row'");
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийКорректировкиНазначения.СнятьРезервПоМногимНазначениям Тогда
			ТекущаяСтрока.ИсходноеНазначение = Неопределено;
			ТекущаяСтрока.ИсходныйЗаказ = Неопределено;
		ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийКорректировкиНазначения.ВстречнаяКорректировка Тогда
			ТекущаяСтрока.ИсходноеНазначение = Неопределено;
			ТекущаяСтрока.ИсходныйЗаказ = Неопределено;
		КонецЕсли;
...
Процедура УстановитьВидимостьДоступность()
	...
	ЗаголовокЗаполнитьПоВидуОперации.Вставить(ВидыОпераций.Резервировать,                  НСтр("ru = 'Подобрать из свободных остатков';
																								|en = 'Pick from available inventory'"));
...
Процедура УстановитьПараметрыВыбораНазначения()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья") Тогда
		МассивТипов.Добавить(Перечисления.ТипыНазначений.Давальческое21);
		МассивТипов.Добавить(Перечисления.ТипыНазначений.ДавальческоеМатериалы22);
		МассивТипов.Добавить(Перечисления.ТипыНазначений.ДавальческоеМатериалыПодЭтап22);
	КонецЕсли;
	//-- Устарело_Переработка24
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья2_5") Тогда
		МассивТипов.Добавить(Перечисления.ТипыНазначений.ДавальческоеМатериалыПодЭтап22);
		МассивТипов.Добавить(Перечисления.ТипыНазначений.Давальческое2_5);
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура ОткрытьСписокНазначений()
	...
	ЗаголовокФормыПросмотра = ?(МогутБытьНазначенияБезЗаказа, 
		НСтр("ru = 'Назначения (%КоличествоДокументов%)';
			|en = 'Assignments (%КоличествоДокументов%)'"),
...
Процедура ОбновитьИнформациюПоЗаказам()
	...
	ШаблонВсегоНазначений = ?(МогутБытьНазначенияБезЗаказа, НСтр("ru = 'Всего назначений: %1';
																|en = 'Total assignments: %1'"), НСтр("ru = 'Всего заказов: %1';
																										|en = 'Total orders: %1'"));
