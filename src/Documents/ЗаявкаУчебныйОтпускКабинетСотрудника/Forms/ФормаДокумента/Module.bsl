...
Процедура ВыбратьВидОтпуска()
	...
	ВидыОтпусков = ВидыУчебныхОтпусков();
	
	Оповещение = Новый ОписаниеОповещения("ВыбратьВидОтпускаЗавершение", ЭтотОбъект);
	ПараметрыОткрытия = Новый Структура("ВидыОтпусков,КомпенсацияОтпуска", ВидыОтпусков, Ложь);
...
Функция ДанныеОтпусков(ВидОтпуска, Сотрудник)
	...
	ДанныеОтпуска.ДатаНачала 	= Объект.ДатаНачала;
	ДанныеОтпуска.ДатаОкончания = Объект.ДатаОкончания;
	...
	ДанныеОтпуска.КоличествоДней = УчетРабочегоВремениРасширенный.ДлительностьИнтервала(
		Сотрудник, Объект.ДатаНачала, Объект.ДатаОкончания, ПоКалендарнымДням, ВидОтпускаРеквизиты.ОтпускЯвляетсяЕжегодным);
