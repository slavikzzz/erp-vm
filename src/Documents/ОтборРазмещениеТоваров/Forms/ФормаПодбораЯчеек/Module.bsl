...
Процедура ЗаполнитьСписокЯчеекПоОтбору(Команда)
	Если Ячейки.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьСписокЯчеекПоОтборуЗавершение", ЭтотОбъект), НСтр("ru = 'Перед заполнением список ячеек будет очищен. Продолжить?';
																											|en = 'Before starting filling in, we need to clear up the storage bin list. Continue?'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
	КонецЕсли;
...
Процедура ОбработатьШтрихкоды(Данные)
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'Складская ячейка со штрихкодом %Штрихкод% не найдена';
								|en = 'Storage bin with barcode %Штрихкод% is not found'");
...
Функция АдресЯчеекВХранилище(УникальныйИдентификаторФормы)
	...
	Если ТаблицаЯчеек.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Не указано ни одной ячейки';
								|en = 'No storage bin is specified'");
