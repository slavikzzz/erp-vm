...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
		ТекстОшибкиЗаказНеЗаполнен =
			НСтр("ru = 'Не заполнено поле ""Заказ давальца"" в строке %НомерСтроки% списка ""Товары""';
	...
	//-- НЕ УТКА
		ТекстОшибкиЗаказНеЗаполнен =
			НСтр("ru = 'Не заполнено поле ""Заказ поставщика"" в строке %НомерСтроки% списка ""Товары""';
				|en = '""Purchase order"" is not populated in line %НомерСтроки% of the Item list'");
	//++ НЕ УТКА
	КонецЕсли;
	//-- НЕ УТКА
	...
	Для ТекИндекс = 0 По Товары.Количество()-1 Цикл
		...
		Если Статус <> Перечисления.СтатусыАктаОРасхождениях.НеСогласовано 
			...
			ТекстОшибки = НСтр("ru = 'Не заполнено поле ""Как отработать расхождение"" в строке %НомерСтроки% списка ""Товары""';
								|en = 'Discrepancy treatment in line %НомерСтроки% is required'");
			ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекущаяСтрока.НомерСтроки);
	...
	//++ НЕ УТКА
	Если ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПоступлениеОтДавальца Тогда
		
		МассивСтрок = Товары.НайтиСтроки(Новый Структура("Назначение", Справочники.Назначения.ПустаяСсылка()));
		Для каждого СтрокаТаблицы Из МассивСтрок Цикл
			
			ТекстОшибки = СтрШаблон(
							НСтр("ru = 'Не заполнено поле ""Назначение"" в строке %1 списка ""Товары""';
								|en = 'The ""Assignment"" field in line %1 of the ""Goods"" list is not filled'"),
							СтрокаТаблицы.НомерСтроки);
			
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", СтрокаТаблицы.НомерСтроки, "Назначение");
	...
	Если Не ЗначениеЗаполнено(Партнер) Тогда
		...
		ИначеЕсли ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПоступлениеОтДавальца Тогда
	...
	Иначе
		...
		Если МассивДвухходовыхОпераций.Найти(ХозяйственнаяОперация) = Неопределено Тогда
			...
			ТекстОшибки = НСтр("ru = 'Не указан получатель работ (услуг) в строке %1 списка Товары';
								|en = 'Party receiving works (services) in line %1 of the Item list is not specified.'");
	...
	//++ НЕ УТ
	ИначеЕсли ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратТоваровОтПереработчика Тогда
		ОписаниеМетаданных = РасхожденияСервер.ОписаниеМетаданныхПроверкиВозможностиВнесенияИзлишкаВНакладную();
		ОписаниеМетаданных.ИмяПоляНакладнойЗаказ = "Распоряжение";
		ОписаниеМетаданных.ИмяПоляРегистраЗаказ  = "ДокументПоступления";
		ОписаниеМетаданных.ИмяПоляАктаЗаказ = "ЗаказПоставщику";
	...
	ИначеЕсли ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПоступлениеТоваровОтПереработчика Тогда
		...
		ОписаниеМетаданных.ИмяПоляКОформлению = "КПоступлению";
...
Процедура ЗаполнитьПоОтбору(СтруктураОтбора)
	...
	Если СтруктураОтбора.Свойство("ТипОснованияАктаОРасхождении") Тогда
		...
		//++ НЕ УТКА
		Если ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПоступлениеОтДавальца
		   И Не ЗначениеЗаполнено(ХозяйственнаяОперация) Тогда
...
Процедура ЗаполнитьНаОсновании(Основание, НеПроверятьВозможностьВвода = Ложь, ТипОснования = Неопределено)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПриемкаТоваровНаХранение")
		...
		ХозяйственнаяОперацияОснование =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущееОснование, "ХозяйственнаяОперация");
		
		Если ХозяйственнаяОперацияОснование = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
			ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПоступлениеОтДавальца;
		Иначе
		//-- НЕ УТКА
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг")
		...
		Если РеквизитыОснования.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаЧерезПодотчетноеЛицо Тогда
			ТекстОшибки = НСтр("ru = 'Создание документа""Акт о расхождениях после поступления"" при закупке через подотчетное лицо не предусмотрено.';
								|en = 'Creation of ""Goods return dispute"" document when purchasing through an advance holder is not provided.'");
		...
		ИначеЕсли РеквизитыОснования.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаРеглУчет Тогда
			ТекстОшибки = НСтр("ru = 'Создание документа ""Акт о расхождениях после поступления"" при закупке по регл. учету не предусмотрено.';
								|en = 'Creation of ""Goods return dispute"" document when purchasing by local accounting is not provided.'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Процедура ЗаполнитьУсловияЗакупокПоУмолчанию() Экспорт
	...
	Если ЗначениеЗаполнено (Партнер) Тогда
		...
		//++ НЕ УТКА
		МассивОперацийОтбора.Добавить(Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5);
		//-- НЕ УТКА
...
Функция ТипОснованияАктаОРасхожденииПоХозОперации(ХозОперация)
	...
	//++ НЕ УТ
	ИначеЕсли ХозОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5 Тогда
		Возврат Перечисления.ТипыОснованияАктаОРасхождении.ПоступлениеТоваровОтПереработчика;
	ИначеЕсли ХозОперация = Перечисления.ХозяйственныеОперации.ВозвратОтПереработчика2_5 Тогда
		Возврат Перечисления.ТипыОснованияАктаОРасхождении.ВозвратТоваровОтПереработчика;
	//-- НЕ УТ

	//++ НЕ УТКА
	ИначеЕсли ХозОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтДавальца2_5 Тогда
		Возврат Перечисления.ТипыОснованияАктаОРасхождении.ПоступлениеОтДавальца;
	//-- НЕ УТКА
	Иначе
