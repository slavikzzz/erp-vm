	...
	Пока ВыборкаИтоги.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось обработать документ: %Объект% по причине: %Причина%';
									|en = 'Не удалось обработать документ: %Объект% по причине: %Причина%'");
...
Функция УведомлениеОбИмпортеМаркированныхТоваровСXML(ДокументСсылка)
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		ПредставлениеНоменклатуры = ОбщегоНазначенияИС.ПредставлениеНоменклатуры(СтрокаТЧ.Номенклатура, СтрокаТЧ.Характеристика);
		...
		Иначе
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
				СообщениеXML,
				СтрШаблон(НСтр("ru = 'Не заполнены номера КиЗ для номенклатуры %1 в строке %2.';
								|en = 'Не заполнены номера КиЗ для номенклатуры %1 в строке %2.'"), ПредставлениеНоменклатуры, СтрокаТЧ.НомерСтроки));
		КонецЕсли;
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
				СообщениеXML,
				СтрШаблон(НСтр("ru = 'Не заполнен Код ТНВЭД для номенклатуры %1 в строке %2.';
								|en = 'Не заполнен Код ТНВЭД для номенклатуры %1 в строке %2.'"), ПредставлениеНоменклатуры, СтрокаТЧ.НомерСтроки));
		КонецПопытки;
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.КодВидМеха) Тогда
			...
			Исключение
				ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
					СообщениеXML,
					СтрШаблон(НСтр("ru = 'Не корректно заполнен Вид меха для номенклатуры %1 в строке %2.';
									|en = 'Не корректно заполнен Вид меха для номенклатуры %1 в строке %2.'"), ПредставлениеНоменклатуры, СтрокаТЧ.НомерСтроки));
			КонецПопытки;
		КонецЕсли;
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
				СообщениеXML,
				СтрШаблон(НСтр("ru = 'Не заполнено Описание товара для номенклатуры %1 в строке %2.';
								|en = 'Не заполнено Описание товара для номенклатуры %1 в строке %2.'"), ПредставлениеНоменклатуры, СтрокаТЧ.НомерСтроки));
		КонецПопытки;
