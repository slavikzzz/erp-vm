	...
	Если РезультатЗапроса.Пустой() Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не требуется вводить эквайринговую операцию на основании документа %1';
				|en = 'Generating an acquiring transaction from base document %1 is not required'"),
			ДокументОснование);
...
Процедура ЗаполнитьПоПодарочномуСертификату(Знач ДокументОснование, ДанныеЗаполнения)
	...
	Если РазрешенныеСтатусы.Найти(СтатусПодарочногоСертификата) = Неопределено Тогда
		Текст = НСтр("ru = 'Данная операция поддерживается для подарочных сетификатов в статусе: ""Не активирован"", ""Активирован"", ""Частично погашен"".';
					|en = 'This transaction is supported for gift cards with the following status: Not activated, Activated, or Used partially.'");
		ВызватьИсключение Текст;
	КонецЕсли;
...
Процедура ЗаполнитьПоОтчетуДавальцу2_5(Знач ДокументОснование, ДанныеЗаполнения)
