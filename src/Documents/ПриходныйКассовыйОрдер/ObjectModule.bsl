...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ДокОснование) Тогда
		...
		ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтчетДавальцу") Тогда
			ЗаполнитьПоОтчетуДавальцу(ДокОснование, ДанныеЗаполнения, СуммаКОплате);
		ИначеЕсли ТипОснования = Тип("ДокументСсылка.ЗаказДавальца") Тогда
			ЗаполнитьПоЗаказуДавальца(ДокОснование, ДанныеЗаполнения, СуммаКОплате);
		//-- Устарело_Переработка24
		ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
			ЗаполнитьПоОтчетуДавальцу(ДокОснование, ДанныеЗаполнения, СуммаКОплате);
		ИначеЕсли ТипОснования = Тип("ДокументСсылка.ЗаказДавальца2_5") Тогда
			ЗаполнитьПоЗаказуДавальца(ДокОснование, ДанныеЗаполнения, СуммаКОплате);
		ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтчетДавальцуМеждуОрганизациями") Тогда
			ЗаполнитьПоОтчетуДавальцуМеждуОрганизациями(ДокОснование, ДанныеЗаполнения, СуммаКОплате);
		//-- НЕ УТКА
		...
		Если Не ЗначениеЗаполнено(ПодотчетноеЛицо) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Поле ""Сотрудник"" не заполнено.';
					|en = '""Employee"" is required.'"), , "ПодотчетноеЛицо", "Объект", Отказ);
			МассивНепроверяемыхРеквизитов.Добавить("ПодотчетноеЛицо");
		КонецЕсли;
	КонецЕсли;
	
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствОтАрендодателя Тогда
		МассивНепроверяемыхРеквизитов.Добавить("РасшифровкаПлатежа.РасчетныйДокументПоАренде");
		Для каждого СтрокаРасшифровки Из РасшифровкаПлатежа Цикл
			Если (СтрокаРасшифровки.ТипПлатежаПоАренде = Перечисления.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж
				Или СтрокаРасшифровки.ТипПлатежаПоАренде = Перечисления.ТипыПлатежейПоАренде.ВыкупнаяСтоимостьАванс)
				И Не ЗначениеЗаполнено(СтрокаРасшифровки.РасчетныйДокументПоАренде) Тогда
				
				ТекстОшибки = НСтр("ru = 'Поле ""Расчетный документ"" не заполнено.';
									|en = 'The ""AR/AP document"" field is not filled in.'");
				
				Если РасшифровкаБезРазбиения Тогда
					ИмяПоля = "Элементы.РасшифровкаПлатежа.ТекущиеДанные.РасчетныйДокументПоАренде";
					КлючДанных = Неопределено;
				Иначе
					ИмяПоля = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
						"РасшифровкаПлатежа", СтрокаРасшифровки.НомерСтроки, "РасчетныйДокументПоАренде");
					КлючДанных = ЭтотОбъект;
				КонецЕсли;
				
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, КлючДанных, ИмяПоля,, Отказ);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	//-- НЕ УТ
	...
	Если Исправление Тогда
		...
		Если СуммаДокумента <> СуммаИсправляемогоДокумента Тогда
			Текст = НСтр("ru = 'При исправлении сумма документа не может быть изменена.';
						|en = 'When corrected, document amount cannot be changed.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				Текст,
				ЭтотОбъект,
				"СуммаДокумента",,
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	НастройкаСчетовУчетаСервер.ПередЗаписью(ЭтотОбъект,
		Документы.ПриходныйКассовыйОрдер.ПараметрыНастройкиСчетовУчета(ХозяйственнаяОперация));
	//-- НЕ УТ
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента
			...
			//++ НЕ УТ
			ИЛИ ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствОтАрендодателя
			//-- НЕ УТ
			Тогда
	...
	Если РезультатЗапроса.Пустой() Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не требуется вводить приходный кассовый ордер на основании документа %1';
				|en = 'It is not required to enter an ""Incoming payment — Cash account"" based on document %1'"),
			ДокументОснование);
...
Процедура ЗаполнитьПоРасходномуКассовомуОрдеру(Знач ДокументОснование, ДанныеЗаполнения)
	...
	Если РезультатЗапроса.Пустой() Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'You do not have to enter ""Incoming payment — Cash account"" based on document %1 
...
Процедура ЗаполнитьПоЗаказуДавальца(Знач ЗаказДавальца, ДанныеЗаполнения, СуммаКОплате = 0)
	...
	Если Выборка.Следующий() Тогда
		...
		ПараметрыЗаполненияРасшифровки = ВзаиморасчетыСервер.ПараметрыЗаполненияРасшифровкиПлатежаПоЗаказу();
		ПараметрыЗаполненияРасшифровки.ЗаказКлиента     = ЗаказДавальца;
		ПараметрыЗаполненияРасшифровки.Договор          = Выборка.Договор;
		ПараметрыЗаполненияРасшифровки.ВалютаДокумента  = ДанныеЗаполнения.Валюта;
		ПараметрыЗаполненияРасшифровки.Партнер          = Выборка.Партнер;
		ПараметрыЗаполненияРасшифровки.ОснованиеПлатежа = ЗаказДавальца;
...
Процедура ЗаполнитьПоОтчетуДавальцу(Знач ОтчетДавальцу, ДанныеЗаполнения, СуммаКОплате = 0)
	...
	|		КОГДА ДанныеДокумента.Касса <> ЗНАЧЕНИЕ(Справочник.Кассы.ПустаяСсылка)
	|			ТОГДА ДанныеДокумента.Касса.Владелец
	|		КОГДА ДанныеДокумента.БанковскийСчетОрганизации <>
	...
	Если Выборка.Следующий() Тогда
		...
		ПараметрыЗаполненияРасшифровки.ВалютаДокумента  = ДанныеЗаполнения.Валюта;
		ПараметрыЗаполненияРасшифровки.Партнер          = Выборка.Партнер;
		ПараметрыЗаполненияРасшифровки.ОснованиеПлатежа = ОтчетДавальцу;
...
Процедура ЗаполнитьПоОтчетуДавальцуМеждуОрганизациями(Знач ДокументОснование, ДанныеЗаполнения, СуммаКОплате = 0)
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ
	|	ДанныеДокумента.Организация КАК Организация,
	|	ДанныеДокумента.Давалец КАК Партнер,
	|	ДанныеДокумента.Давалец КАК Контрагент,
	...
	|ГДЕ
	|	ДанныеДокумента.Ссылка = &Ссылка
	|	И ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцуМеждуОрганизациями)
	|");
...
Процедура ЗаполнитьПоДоговоруКонтрагента(Знач Договор, ДанныеЗаполнения, СуммаКОплате = 0)
	...
	Если Выборка.Следующий() Тогда
		...
		Если Не (Выборка.ТипДоговора = Перечисления.ТипыДоговоров.СПокупателем
			...
			//-- Устарело_Переработка24
			//++ НЕ УТ
			Или Выборка.ТипДоговора = Перечисления.ТипыДоговоров.ПередачаВАренду
			Или Выборка.ТипДоговора = Перечисления.ТипыДоговоров.Субаренда
			//-- НЕ УТ
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не требуется вводить приходный кассовый ордер на основании договора с типом взаимоотношений ""%1""';
					|en = 'You do not have to enter ""Incoming payment — Cash account"" based on the contract with the ""%1"" interaction type'"),
				Выборка.ТипДоговора);
			ВызватьИсключение Текст;
		КонецЕсли;
		...
		Если Выборка.УдержатьВознаграждение Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не требуется вводить приходный кассовый ордер на основании документа %1, поскольку в нем удержано вознаграждение';
					|en = 'You do not have to enter ""Incoming payment — Cash account"" based on document %1 as remuneration is deducted in it'"),
				ОтчетКомитенту);
			ВызватьИсключение Текст;
		КонецЕсли;
...
Процедура ЗаполнитьПоПодарочномуСертификату(Знач ДокументОснование, ДанныеЗаполнения, СуммаКОплате = 0)
	...
	Если Не ДанныеПодарочногоСертификата.УчетПодарочныхСертификатов2_5 Тогда
		Текст = НСтр("ru = 'Данная операция поддерживается только для видов подарочных сертификатов, использующих учет подарочных сертификатов 2.5.';
					|en = 'This transaction is supported only for gift card kinds that use gift card accounting 2.5.'");
		ВызватьИсключение Текст;
	КонецЕсли;
	Если ДанныеПодарочногоСертификата.СтатусАктивации Тогда
		Текст = НСтр("ru = 'Данная операция поддерживается только для не активированных подарочных сертификатов.';
					|en = 'This transaction is supported only for unactivated gift cards.'");
		ВызватьИсключение Текст;
	КонецЕсли;
	Если ДанныеПодарочногоСертификата.Номинал <= 0 Тогда
		Текст = НСтр("ru = 'Данная операция поддерживается только для подарочных сертификатов с ненулевым номиналом.';
					|en = 'This transaction is supported only for gift cards with a non-zero amount.'");
		ВызватьИсключение Текст;
	КонецЕсли;
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзБанка
		...
		Если Организация <> РеквизитыСчета.Организация Тогда
			Текст = НСтр("ru = 'Организация счета не соответствует организации документа';
						|en = 'Account company does not match the document company'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				Текст,
				ЭтотОбъект,
				"БанковскийСчет",
		...
		Если Валюта <> РеквизитыСчета.Валюта Тогда
			Текст = НСтр("ru = 'Валюта счета не соответствует валюте документа';
						|en = 'Account currency does not correspond to the document currency'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				Текст,
				ЭтотОбъект,
				"БанковскийСчет",
	...
	Для Каждого СтрокаРасшифровки Из РасшифровкаПлатежа Цикл
		Если ЗначениеЗаполнено(СтрокаРасшифровки.ДатаПогашения)
			...
			ТекстОшибки = НСтр("ru = 'Дата погашения не может быть раньше даты платежа.';
								|en = 'The repayment date cannot be earlier than the payment date.'");
