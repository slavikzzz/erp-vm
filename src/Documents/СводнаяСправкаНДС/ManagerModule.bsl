...
Процедура ОбработкаУстановкиДатыНачалаПримененияСводныхСправок(Параметры, АдресХранилища) Экспорт
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось очистить движения по регистру НДС продажи по причине: %1';
										|en = 'Cannot clear movements by the VAT sales register for the reason: %1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			
		ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Установка даты начала применения сводных справок.';
					|en = 'Set up the start date of consolidated certificate application.'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Процедура СформироватьВТПериодыПродаж(Запрос, ПараметрыРасчета)
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиУчетаНДСПриУСН.СрезПоследних(&КонецПериода, Организация В
	|			(ВЫБРАТЬ
	|				ГоловныеОрганизации.Организация
	|			ИЗ
