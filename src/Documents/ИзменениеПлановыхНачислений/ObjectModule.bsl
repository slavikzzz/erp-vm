...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользоватьИндексациюЗаработка")
		И УчитыватьПриИндексацииЗаработка Тогда
...
Функция ПолучитьДанныеДляПроведения()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		...
		Модуль = ОбщегоНазначения.ОбщийМодуль("РасчетДенежногоДовольствия");
		КоэффициентыИндексацииДенежногоДовольствия = Модуль.ТаблицаИндексацииВоеннослужащих(ДанныеДляПроведения.КоэффициентыИндексации);
...
Функция ДанныеДляПроведенияБухучетНачислений()
	...
	Если ПолучитьФункциональнуюОпцию("РегистрироватьБухучетПлановыхНачислений") Тогда
		...
		|	Сотрудники.Ссылка.ДатаИзменения КАК Период,
		|	ВЫБОР
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ИзменениеПлановыхНачислений.НачисленияСотрудников КАК Начисления
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ИзменениеПлановыхНачислений.БухучетНачислений КАК БухучетНачислений
...
Процедура ОчиститьДанныеПоСотруднику(Сотрудник, ДатаИзменения)
	...
	Для Каждого СтрокаДляУдаления Из СтрокиДляУдаления Цикл
		...
		СтрокиДляУдаленияБухучетНачислений = БухучетНачислений.НайтиСтроки(Новый Структура("ИдентификаторСтрокиСотрудника", СтрокаДляУдаления.ИдентификаторСтрокиСотрудника));
		Для Каждого СтрокаДляУдаленияПоказатели Из СтрокиДляУдаленияПоказатели Цикл
			БухучетНачислений.Удалить(СтрокаДляУдаленияПоказатели);
		КонецЦикла;
