...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияИСМП.ЭтоРасширеннаяВерсияГосИС() Тогда
		...
		Если МодульИнтеграцияИСМП.ЗапрещеноИспользованиеОбъектаВИСМП(Объект.Ссылка) Тогда
			Отказ = Истина;
			Возврат;
		КонецЕсли;
...
Процедура АдресПодключенияСтрокойПриИзменении(Элемент)
	...
	Объект.АдресПодключения = ИнтеграцияИСВызовСервера.ЗначенияПолейКонтактнойИнформации(
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	РеквизитыОрганизации = ОбщегоНазначенияИС.ИННКППGLNОрганизации(Объект.Организация, Объект.Подразделение);
...
Функция ЗначенияРеквизитовШтрихкодаУпаковки(КодМаркировки)
	
	Если ОбщегоНазначенияИСМП.ЭтоРасширеннаяВерсияГосИС() Тогда
		Результат = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(КодМаркировки, "Номенклатура, Характеристика, Серия");
		Результат.Вставить("ВидПродукции", ОбщегоНазначенияИС.ВидПродукцииПоНоменклатуре(Результат.Номенклатура));
	Иначе
