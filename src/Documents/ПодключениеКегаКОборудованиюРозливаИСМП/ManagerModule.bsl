...
Процедура ПриИзмененииСтатусаДокумента(ДокументСсылка, ПредыдущийСтатус, НовыйСтатус, ПараметрыОбновленияСтатуса) Экспорт
	...
	Если ОбщегоНазначенияИСМП.ЭтоРасширеннаяВерсияГосИС() Тогда
		...
		МодульИнтеграцияИСМППереопределяемый.ПриИзмененииСтатусаДокумента(ДокументСсылка, ПредыдущийСтатус, НовыйСтатус, ПараметрыОбновленияСтатуса);
		...
		Если КонечныеСтатусы().Найти(НовыйСтатус)<>Неопределено
			...
			МодульРасчетСтатусовОформленияИСМП.РассчитатьСтатусыОформленияДокументов(ДокументСсылка);
		КонецЕсли;
...
Процедура ПриЗаполненииОграниченияДоступа(Списки) Экспорт
	...
	УправлениеДоступомИСПереопределяемый.ПриЗаполненииОграниченияДоступа(
		Метаданные.Документы.ПодключениеКегаКОборудованиюРозливаИСМП, Списки);
...
Функция ИменаРеквизитовДляЗаполненияПараметровУказанияСерий() Экспорт
	...
	Возврат МодульИнтеграцияИС.ИменаРеквизитовДляЗаполненияПараметровУказанияСерий(Метаданные.Документы.ПодключениеКегаКОборудованиюРозливаИСМП);
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	МодульИнтеграцияИС = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияИС");
	Возврат МодульИнтеграцияИС.ПараметрыУказанияСерий(Метаданные.Документы.ПодключениеКегаКОборудованиюРозливаИСМП, Объект);
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерий(ПараметрыУказанияСерий) Экспорт
	...
	МодульИнтеграцияИС = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияИС");
	Возврат МодульИнтеграцияИС.ТекстЗапросаЗаполненияСтатусовУказанияСерий(Метаданные.Документы.ПодключениеКегаКОборудованиюРозливаИСМП, ПараметрыУказанияСерий);
...
Функция ПодключениеКОборудованиюРозливаJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	РеквизитыОрганизации = ОбщегоНазначенияИС.ИННКППGLNОрганизации(Шапка.Организация, Шапка.Подразделение);
		...
		ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
			...
			СтрШаблон(
				НСтр("ru = 'Не найден идентификатор ФИАС по адресу:
						   |%1';
						   |en = 'Не найден идентификатор ФИАС по адресу:
						   |%1'"), Шапка.АдресПодключенияСтрокой));
	КонецЕсли;
	
	КодМаркировкиСтрокой = Строка(Шапка.КодМаркировки);
	
	ШтрихкодированиеИСКлиентСерверПереопределяемый.ПриВосстановленииЭкранированногоКодаМаркировки(КодМаркировкиСтрокой);
	...
	Если СтрокаКодаМаркировки.Свойство("ВидУпаковки")
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Код маркировки %1 не является кодом потребительской упаковки';
				|en = 'Код маркировки %1 не является кодом потребительской упаковки'"),
			КодМаркировкиСтрокой);
		ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
			СообщениеJSON,
			ТекстОшибки);
	...
	СообщениеJSON.ПараметрыЗапроса.Вставить("КодМаркировкиСтрокой",          КодМаркировкиСтрокой);
...
Функция ДанныеЗаполненияПоСтатистике(Организация, ПоТекущемуПользователю = Истина) Экспорт
	...
	Если ДанныеПоследнихДокументов.Количество() = 0 И Не БезУчетаОтветственного Тогда
		...
		Возврат ДанныеЗаполненияПоСтатистике(Организация, Ложь);
		
	ИначеЕсли ДанныеПоследнихДокументов.Количество() Тогда
		
		ОбщегоНазначенияИСМП.ЗаполнитьПоСтатистике(
			"Подразделение, Склад",
