	...
	Если Объект.ДатаВступленияВСилу < ДатаПоследнегоИзменения Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Новая дата %1 меньше даты предыдущего изменения %2, документ будет перезаполнен.
				...
				|en = 'New date %1 is less than the date of previous change %2, the document will be refilled.
				|Change the effectiveness date of changes?'"),
...
Процедура ЗаполнитьЗначенияДействующихТарифов(ОчиститьЗначения = Ложь)
	...
	Для Каждого ДанныеТарифов Из ДанныеТарифнойСетки.ЗначенияТарифов Цикл 
		...
		СтрокаТаблицы.ТекущийРазрядныйКоэффициентПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'было: %1';
																																|en = 'previous value: %1'"), Формат(ДанныеТарифов.РазрядныйКоэффициент, "ЧДЦ=2"));
		СтрокаТаблицы.ТекущийТарифПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'было: %1';
																												|en = 'previous value: %1'"), Формат(ДанныеТарифов.Тариф, "ЧДЦ=2"));
	...
	Если УтверждениеНовойТарифнойСетки Тогда
		...
		Исключение
			...
			ВызватьИсключение НСтр("ru = 'Не удалось записать';
									|en = 'Cannot save'") + " " + ИнфоОшибка.Описание;
	...
	Иначе
		Если ДополнительныеПараметры.СписокОрганизаций.Количество() = 0 Тогда
			ТекстСообщения =  НСтр("ru = 'Измененные тарифы не используются в штатном расписании.';
									|en = 'Changed tariffs are not used in headcount.'");
