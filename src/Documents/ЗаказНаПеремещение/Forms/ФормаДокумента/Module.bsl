...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	Если КонтролироватьАссортимент Тогда
		...
		СтруктураПроверкиАссортимента.ТекстСообщения = НСтр("ru = 'Товар %1 не включен в ассортимент магазина-получателя. Заказывать его не рекомендуется.';
															|en = 'Product %1 is not included in the receiving store''s product range. It is not recommended that you order it.'");
...
Процедура ПодобратьТовары(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.СкладОтправитель) Тогда
		...
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Поле ""Отправитель"" не заполнено';
																|en = '""Sender"" is not filled in'"), Объект.Ссылка, "Объект.СкладОтправитель",, Отказ);
...
Процедура УправлениеЭлементамиФормы()
	...
	Элементы.Организация.Заголовок = ?(ЭтоПередачаТоваровМеждуОбособленнымиПодразделениями,
		НСтр("ru = 'Организация-отправитель';
			|en = 'Shipping company'"), НСтр("ru = 'Организация';
													|en = 'Company'"));
