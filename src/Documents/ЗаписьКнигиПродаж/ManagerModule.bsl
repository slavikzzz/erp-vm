...
Процедура ЗаполнитьРасхождения(Объект) Экспорт
	...
	//++ НЕ УТ
	|	,
	|	ТаблицаЦенности.НастройкаСчетовУчета КАК НастройкаСчетовУчета
	//-- НЕ УТ
	...
	|
	|ВЫБРАТЬ
	|	ДанныеДляРасхождения.ХешСуммаНастройкиСчетов КАК ХешСуммаНастройкиСчетов,
	|	МАКСИМУМ(ДанныеДляРасхождения.НастройкаСчетовУчета) КАК НастройкаСчетовУчета
	|ИЗ
	|	ДанныеДляРасхождения КАК ДанныеДляРасхождения
	|СГРУППИРОВАТЬ ПО
	|	ДанныеДляРасхождения.ХешСуммаНастройкиСчетов
	|;
	//-- НЕ УТ
	|
	|ВЫБРАТЬ
	...
	//++ НЕ УТ
	ВыборкаНастройкиСчетов = Результаты[Результаты.ВГраница() - 1].Выбрать();
	//-- НЕ УТ
	...
	Пока ВыборкаРасхождения.Следующий() Цикл
		...
		//++ НЕ УТ
		Если ЗначениеЗаполнено(ВыборкаРасхождения.ХешСуммаНастройкиСчетов) Тогда
			ВыборкаНастройкиСчетов.Сбросить();
			Если ВыборкаНастройкиСчетов.НайтиСледующий(ВыборкаРасхождения.ХешСуммаНастройкиСчетов, "ХешСуммаНастройкиСчетов") Тогда
				НоваяСтрока.НастройкаСчетовУчета = НастройкаСчетовУчетаСервер.КопияНастройкиСчетовУчета(ВыборкаНастройкиСчетов.НастройкаСчетовУчета);
			КонецЕсли;
		КонецЕсли;
		//-- НЕ УТ
	КонецЦикла;
...
#Область НачислениеИВосстановлениеНДС
	
	ТекстНачислениеИВосстановлениеНДС = "
	...
	|	ВЫБОР 
	|		КОГДА Строки.НастройкаСчетовУчета.Субконто2 = НЕОПРЕДЕЛЕНО ИЛИ Строки.НастройкаСчетовУчета.Субконто2 = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.ПустаяСсылка)
	|			ТОГДА Строки.СтавкаНДС
	|		ИНАЧЕ Строки.НастройкаСчетовУчета.Субконто2
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Начисление/восстановление НДС"" КАК Содержание
	|
	|ИЗ 
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.ЗаписьКнигиПродаж КАК Операция
	|	ПО
	|		ДокументыКОтражению.Ссылка = Операция.Ссылка
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		Документ.ЗаписьКнигиПродаж.Ценности КАК Строки
	|	ПО 
...
#Область ИсправлениеПрочегоНачисленияНДС
	...
	ТекстИсправлениеПрочегоНачисленияНДС = "
	|ВЫБРАТЬ //// Начисление НДС (Дт <90.3, 91.2> :: Кт <68.2>)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Исправление начисления НДС"" КАК Содержание
	|
	|ИЗ 
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		Документ.ЗаписьКнигиПродаж.Расхождения КАК Строки
	|	ПО 
	|		Операция.Ссылка = Строки.Ссылка
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	...
	|		И КурсВалютыУпрУчета.Дата = НАЧАЛОПЕРИОДА(Операция.Дата, День)
	|ГДЕ
	|	Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ИсправлениеПрочегоНачисленияНДС)
	|";
	
#КонецОбласти

	ТекстыОтражения = Новый Массив;
	ТекстыОтражения.Добавить(ТекстНачислениеИВосстановлениеНДС);
	ТекстыОтражения.Добавить(ТекстИсправлениеПрочегоНачисленияНДС);
...
// 	
// Возвращаемое значение:
// 	Массив - Массив параметров настройки счетов учета (См. НастройкаСчетовУчетаСервер.ПараметрыНастройки).
//
Функция ПараметрыНастройкиСчетовУчета(ХозяйственнаяОперация) Экспорт
	...
	ПараметрыНастройки.ДоступностьПоОперации =
		(ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ИсправлениеПрочегоНачисленияНДС);
	
	ПараметрыНастройки.Организация = "Объект.Организация";

	ПараметрыНастройки.ЭлементыФормы.Добавить("РасхожденияСчетРеглУчета");
	МассивПараметров.Добавить(ПараметрыНастройки);
	#КонецОбласти
...
Функция ТекстЗапросаТаблицаОперацииСПрослеживаемымиТоварами(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЗНАЧЕНИЕ(Перечисление.ПорядокОтраженияВОтчетностиПоПрослеживаемости.КнигаПродаж) КАК ОтражениеВОтчетности
	|ИЗ
