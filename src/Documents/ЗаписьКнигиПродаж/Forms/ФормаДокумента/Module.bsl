...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ПараметрыНастройки = Документы.ЗаписьКнигиПродаж.ПараметрыНастройкиСчетовУчета(Объект.ХозяйственнаяОперация);
	...
	Если Не ЗначениеЗаполнено(ДокументОбъект.ИсправляемыйДокумент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Исправляемый документ"" не заполнено.';
				|en = 'The ""Corrected document"" field is not filled in.'"),
			,
