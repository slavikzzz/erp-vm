...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("Ключ") Или Параметры.Ключ = Документы.СведенияОДоходахСотрудникаДляСоцВыплат.ПустаяСсылка() Тогда
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.св.о доход.соц.выпл.") Тогда
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	БлокировкаИзмененияОбъектов.ПриЧтенииНаСервереФормыОбъекта(ЭтотОбъект, ТекущийОбъект,
		ПравоДоступа("Изменение", Метаданные.Документы.СведенияОДоходахСотрудникаДляСоцВыплат));
	// Конец БлокировкаИзмененияОбъектов
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ДокументОбъект.ПроверитьЗаполнение() Тогда
		...
		ТекстСообщения = НСтр("ru = 'Не удалось провести документ.
								...
								|Errors were detected when checking population.'");
...
Процедура ПроверитьСЗапросомДальнейшегоДействия(ОповещениеЗавершения = Неопределено)
	...
	Если Отказ Тогда
		...
		ТекстВопроса = НСтр("ru = 'В документе обнаружены ошибки.
							...
							|en = 'Errors were found in the document.
...
Процедура ПроверитьДанныеДокумента(Отказ)
	...
	ДокументОбъект = РеквизитФормыВЗначение("Объект", Тип("ДокументОбъект.СведенияОДоходахСотрудникаДляСоцВыплат"));
...
Процедура УстановитьСвойстваЭлементовПоИсточникуДанныхСправки()
	...
	Если ВнешниеДанные Тогда 
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ru = 'Сведения о доходах физического лица, направляемые в целях назначения социальных выплат';
...
Процедура ПринятьИзменения()
	...
	Если Модифицированность Тогда
		...
		Оповестить("ИзмененыДанныеСведенияОДоходахСотрудникаДляСоцВыплат", АдресДанныхСправки, ВладелецФормы);
		Модифицированность = Ложь;
...
Функция ПоместитьДанныеСправкиВХранилище()
	...
	ПараметрыДанныхСправки = СправкиНДФЛФормы.ПараметрыПомещенияДанныхСведенияОДоходахСотрудникаДляСоцВыплатВХранилище();
	...
	Возврат СправкиНДФЛФормы.ПоместитьДанныеСведенияОДоходахСотрудникаДляСоцВыплатВХранилище(ЭтотОбъект, ПараметрыДанныхСправки, ОшибкиДанныхСправки, НоваяСтрока);
...
Процедура УстановитьВидимостьАдресов(Форма)
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "ГруппаАдреса", "Видимость", Ложь);
КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьПолейСправки2022(Форма)
	ПоказыватьПоля = Истина;
...
Функция ОписаниеЭлементовСИндикациейОшибок()
	...
	ОписаниеЭлемента = ЗарплатаКадрыОтображениеОшибок.ОписаниеЭлементаСИндикациейОшибокВРеквизитеШапки();
	ОписаниеЭлемента.ИмяЭлемента = "КППОтправки";
	ОписаниеЭлемента.ОтображатьНепривязанныеОшибки = Истина;
	ОписаниеЭлемента.ОтображатьГиперссылку = Истина;
	ОписаниеЭлемента.ПутьКДаннымФормыСодержащимОшибку = "Объект.КППОтправки";
	ЗарплатаКадрыОтображениеОшибок.ДобавитьОписаниеЭлементаСИндикациейОшибокВРеквизитеШапки(КоллекцияОписанийЭлементов, ОписаниеЭлемента);
