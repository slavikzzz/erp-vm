...
Процедура ЗапроситьСостояниеЭД(Команда)
	...
	Если ПрограммаБанка <> ПредопределенноеЗначение("Перечисление.ПрограммыБанка.СбербанкОнлайн")
		...
		ТекстСообщения = НСтр("ru = 'Банк не поддерживает данную функциональность';
								|en = 'The bank does not support this functionality'");
...
Процедура ОтозватьЭД(Команда)
	...
	ЗаголовокОкна = НСтр("ru = 'Укажите причину отзыва электронного документа';
						|en = 'Specify a reason for revocation of the electronic document'");
...
Процедура ЗаполнитьТаблицуЭП()
	...
	Пока Выборка.Следующий() Цикл
		Если Не ЗначениеЗаполнено(Выборка.КомуВыданСертификат) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьСтатусПодписи(НоваяСтрока, ТекСтрока)
	...
	Если ЗначениеЗаполнено(ТекСтрока.ДатаПроверкиПодписи) Тогда
		НоваяСтрока.Представление = ?(ТекСтрока.ПодписьВерна, НСтр("ru = 'Верна';
																	|en = 'Correct'"), НСтр("ru = 'Неверна';
																						|en = 'Incorrect'"))
...
Процедура ПослеПолученияСтатусаДокументаFintech(РезультатЗадания, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗадания.Статус = "Ошибка" Тогда
		ВидОперации = НСтр("ru = 'Отправка запроса статуса платежа через сервис Fintech';
							|en = 'Sending a payment status request via Fintech service'");
		ОбработкаНеисправностейБЭДВызовСервера.ОбработатьОшибку(ВидОперации, 
	...
	Иначе // выполнено
		...
		Иначе
			...
			ТекстЗаголовка = НСтр("ru = 'Получен статус документа';
									|en = 'Document status received'");
			Шаблон = НСтр("ru = 'Текущий статус документа: %1';
							|en = 'Current document status: %1'");
...
Функция ЗапускЗаданияПолученияСтатусаДокументаFintech(Знач ПараметрыПолучения, Знач УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение статуса документа через сервис Fintech integration';
															|en = 'Retrieving document status via Fintech integration service'");
...
Процедура СформироватьЗапросСостоянияЭД(
	...
	Если ТребуетсяПодпись Тогда
		...
		Если ТаблицаСертификатов.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Не найден подходящий сертификат подписи для документа Запрос состояния электронного документа.
										...
										|en = 'Failed to find a suitable certificate for the ""Electronic document state request"" document. 
										|Check settings of exchange via 1C:DirectBank.'");
		...
		Если МассивСертификатов.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ru = 'На компьютере не установлен ни один сертификат, указанный в настройке обмена.
										...
										|en = 'No certificate specified in the exchange settings is installed on the computer. 
										|Install certificates or contact the administrator.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если РеквизитыНастройкиОбмена.Недействительна Тогда
		ШаблонСообщения = НСтр("ru = 'Настройка обмена с банком %1 недействительна';
								|en = 'Setting of exchange with the %1 bank is invalid'");
