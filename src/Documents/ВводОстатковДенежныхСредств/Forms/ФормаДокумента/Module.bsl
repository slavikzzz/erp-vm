	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковВКассах")
		...
		Если Не ЗначениеЗаполнено(ЕдинственнаяКасса) Тогда
			...
			ПодробноеПредставлениеОшибки =
				...
				|en = 'One cash account is used in the infobase,
				|but its properties are not set in the ""Master data"" - ""Settings and catalogs"" - ""Cash account settings"" section.'");
	...
	Если (Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВводОстатковВАвтономныхКассахККМКОформлениюОтчетовОРозничныхПродажах")
		...
		Если Не ЗначениеЗаполнено(ЕдинственнаяКассаККМ) Тогда
			...
			ПодробноеПредставлениеОшибки =
				...
				|en = 'One cash register is used in the infobase,
				|but its properties are not set in the ""Master data"" - ""Settings and catalogs"" - ""Cash register settings"" section.'");
...
Процедура ЗаполнитьПоОстаткам(Команда)
	...
	//++ НЕ УТ
	Если Объект.БанковскиеСчета.Количество() > 0
		Или Объект.Кассы.Количество() > 0 Тогда
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковНаБанковскихСчетах  Тогда
		...
		Если Не ЗначениеЗаполнено(БанковскийСчет) Тогда
			...
			Если Запрос.Выполнить().Выбрать().Количество() = 2
				...
				|en = 'Failed to fill in the ""Bank account"" field. Several bank accounts of organizations have been entered in the infobase, 
				|but the functional option ""Use multiple bank accounts"" has not been enabled. Enable the ""Use multiple bank accounts"" functional option.'");
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковВКассах Тогда
		...
		Если Не ЗначениеЗаполнено(Касса) Тогда
			...
			Если Запрос.Выполнить().Выбрать().Количество() = 2
				...
				|en = 'Cannot fill in the ""Cash account"" field. Several cash accounts have been entered in the infobase, but at the same time, 
				|the functional option ""Use multiple cash accounts"" is not enabled. Enable the ""Use multiple cash accounts"" functional option.'");
			ИначеЕсли Запрос.Выполнить().Выбрать().Количество()  = 0 Тогда
				ВызватьИсключение НСтр("ru = 'Не удалось заполнить поле ""Касса"". Возможно, в информационной базе не введено ни одной кассы.';
										|en = 'Cannot fill in field ""Cash account"". Possibly there are no cash accounts entered into the infobase.'");
			КонецЕсли;
	...
	ИначеЕсли (ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковВАвтономныхКассахККМКОформлениюОтчетовОРозничныхПродажах
		...
		Если Не ЗначениеЗаполнено(КассаККМ) Тогда
			...
			Если Запрос.Выполнить().Выбрать().Количество() = 2 
				...
				|en = 'Cannot fill in the ""Cash register"" field. There are several cash registers entered into the infobase.
				|Enable the ""Use several cash registers"" functional option.'");
			ИначеЕсли Запрос.Выполнить().Выбрать().Количество()  = 0 Тогда
				ВызватьИсключение НСтр("ru = 'Не удалось заполнить поле ""Касса ККМ"". Возможно, в информационной базе не введено ни одной кассы ККМ';
										|en = 'Cannot fill in field ""Cash register"". Possibly, there are no cash registers entered into the infobase.'");
			КонецЕсли;
...
Процедура ЗаполнитьПоОстаткамНаСервере()
	...
	Остатки = Документы.ВводОстатковДенежныхСредств.ОстаткиПоТипуОперации(Объект.Дата, Объект.ХозяйственнаяОперация, Объект.Организация);
