...
Функция ПроверкаЗаполненияДляПодтвержденияИмпорта()
	...
	Если Не ЗначениеЗаполнено(Объект.ДатаПолучения) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Укажите дату поступления на склад.';
														|en = 'Укажите дату поступления на склад.'"),, "Объект.ДатаПолучения",, Отказ);
		
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.МестоХранения) Тогда
		
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Укажите склад-получатель.';
														|en = 'Укажите склад-получатель.'"),, "Объект.МестоХранения",, Отказ);
