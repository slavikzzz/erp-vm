...
Процедура ДоговорПриИзменении(Элемент)
	...
	Если РеализацияЧерезКомиссионера
			И ЗначениеЗаполнено(ВидЦеныДоговора().ВидЦены) Тогда
...
Процедура ДоговорПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт 
	...
	Если НЕ РезультатВопроса = КодВозвратаДиалога.Отмена И РезультатВопроса Тогда
		...
		ВидЦеныДоговора = ВидЦеныДоговора();
		...
		Если ЦеныПредприятияЗаполнениеКлиент.НеобходимоЗаполнениеЦенПоВиду(Объект, ВидЦеныДоговора) Тогда
			...
			ТекстОшибки = "";
			ЦеныРассчитаны = ЗаполнитьЦеныПоСоглашениюСервер(Истина, Ложь, ТекстОшибки);
...
&НаСервере
Функция ЗаполнитьЦеныПоСоглашениюНаСервере(ТекстОшибки = "")
...
Функция ВозможноПробитиеЧекаПоДокументу()
	...
	Возврат РеализацияТоваровУслугЛокализация.РазрешеноПробитиеФискальныхЧековПоДокументу(ЭтотОбъект);
