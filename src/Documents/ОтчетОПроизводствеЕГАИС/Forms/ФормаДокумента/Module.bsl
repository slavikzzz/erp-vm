...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентСерверПереопределяемый.УстановитьПараметрыВыбораНоменклатуры(
		ЭтотОбъект, ИнтеграцияЕГАИСИСМП.ДопустимыеВидыПродукцииИмпортаПроизводства(Истина), "СырьеНоменклатура");
...
Процедура ДекорацияОтсканироватьПроверитьАлкогольнуюПродукциюНажатие(Элемент)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ТекстВопроса = НСтр("ru = 'Сканирование алкогольной продукции возможно только в записанном ""Отчете о производстве ЕГАИС"". Записать?';
							|en = 'Сканирование алкогольной продукции возможно только в записанном ""Отчете о производстве ЕГАИС"". Записать?'");
	...
	ИначеЕсли Модифицированность Тогда
		...
		ТекстВопроса = СтрШаблон(НСтр("ru = '""Отчет о производстве ЕГАИС"" был изменен. %1?';
										|en = '""Отчет о производстве ЕГАИС"" был изменен. %1?'"), ТекстПровестиЗаписать);
		ТребуетсяВопрос = Истина;
...
Процедура ОтчетПроизводственнойЛинииПриИзменении(Элемент)
	...
	Если Отчет И (Действия.ОчиститьТовары Или Действия.ОчиститьШтрихкодыУпаковок) Тогда
		ТекстВопроса = НСтр(
			...
			|en = 'При установке признака Отчет прозводственной линии табличные части ""Товары""
			|и ""Штрихкоды упаковок"" будут очищены. Продолжить?'");
	...
	ИначеЕсли Не Отчет И Действия.ОчиститьДанныеОтчетаПроизводственнойЛинии Тогда
		ТекстВопроса = НСтр(
			...
			|en = 'При снятии признака Отчет прозводственной линии табличная часть ""Коды маркировки"" будет очищена. Продолжить?'");
			Кнопки = РежимДиалогаВопрос.ДаНет; 
		ПоказатьВопрос(ОписаниеОповещенияОЗавершении, ТекстВопроса, Кнопки);
		Возврат;
	КонецЕсли;
...
Процедура СырьеНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентПереопределяемый.ПриНачалеВыбораНоменклатуры(
		ЭтотОбъект,
		ИнтеграцияЕГАИСИСМПКлиент.ДопустимыеВидыПродукцииИмпортаПроизводства(Истина),
		СтандартнаяОбработка,
...
Процедура ПерезаполнитьПоОснованию(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Табличная часть будет перезаполнена. Продолжить?';
							|en = 'Табличная часть будет перезаполнена. Продолжить?'");
...
Процедура УправлениеДоступностьюЭлементовФормы(Форма)
	...
	ОбщегоНазначенияИСКлиентСервер.УстановитьДоступностьЭлементовФормы(Форма, ДоступныеДляЧерновика, ЭтоЧерновик);
	...
	Если Объект.СтатусПроверкиИПодбора = Перечисления.СтатусыПроверкиИПодбораИС.Выполняется Тогда
		Если ПравоИзменения Тогда
			ТекстНадписи = НСтр("ru = 'Продолжить подбор и проверку сырья';
								|en = 'Продолжить подбор и проверку сырья'");
		Иначе
			ТекстНадписи = НСтр("ru = 'Промежуточные результаты подбора сырья';
								|en = 'Промежуточные результаты подбора сырья'");
		КонецЕсли;
	ИначеЕсли (СтатусЕГАИС = Перечисления.СтатусыОбработкиОтчетаОПроизводствеЕГАИС.Черновик
		...
		Если ПравоИзменения Тогда
			ТекстНадписи = НСтр("ru = 'Подобрать и проверить сырье';
								|en = 'Подобрать и проверить сырье'");
...
Процедура ПрименитьУсловноеОформлениеСправка2()
	...
	ОбщегоНазначенияИС.ПрименитьУсловноеОформлениеКПолю(ЭтотОбъект, "ШапкаСправка2");
