...
Процедура ВидЦеныПриИзменении(Элемент)
	Если ЗначениеЗаполнено(Объект.ВидЦены) И Объект.Товары.Количество()>0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ВидЦеныПриИзмененииЗавершение", ЭтотОбъект), НСтр("ru = 'Перезаполнить цены в табличной части документа?';
																									|en = 'Refill prices in the table of the document?'"),РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
КонецПроцедуры
	...
	ОткрытьФорму(
		...
		Новый Структура("СписокДокументов, Заголовок",
			...
			НСтр("ru = 'Передачи товаров между организациями (%КоличествоДокументов%)';
				|en = 'Customer invoices — Intercompany (%КоличествоДокументов%)'")),
...
Процедура ЗаполнитьСебестоимость(Команда)
	...
	Если Элементы.Товары.ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Для выполнения команды требуется выбрать строки табличной части.';
								|en = 'Select rows in the table to execute the command.'");
...
Процедура ОбновитьИнформациюПоПередачам()
	...
	ИначеЕсли СписокПередач.Количество() > 1 Тогда
		...
		НСтр("ru = 'Всего документов: %КоличествоРеализаций%';
			|en = 'Total documents: %КоличествоРеализаций%'"), "%КоличествоРеализаций%",
