...
#Область ФактыЗакупок
	...
	ДопПараметр.Вставить("Тип", 				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
	ДопПараметр.Вставить("Путь", 				ПутьКРеквизиту);
	ДопПараметр.Вставить("Заголовок", 			?(СоздаватьРеквизитСОтбором, НСтр("ru = 'Факт закупок*';
																					|en = 'Actual purchases*'"), НСтр("ru = 'Факт закупок';
																												|en = 'Actual purchases'")));
	...
	ДопПараметр.Вставить("ЗаголовокОперанда", 	?(СоздаватьРеквизитСОтбором, НСтр("ru = 'Факт закупок*';
																					|en = 'Actual purchases*'"), НСтр("ru = 'Факт закупок';
																												|en = 'Actual purchases'")));
...
Функция ДоступныеТипыАналитикБюджетирования(ДокументСсылка, ИмяТаблицы = "") Экспорт
	...
	ИначеЕсли ВыборкаПлан.ЗаполнятьСоглашение Тогда
		ДоступныеТипыАналитик.Вставить(Тип("СправочникСсылка.СоглашенияСПоставщиками"), ИмяТаблицы + ".Ссылка.Соглашение"); 
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаОборотыБюджетов(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИЛИ НЕ ПланОплат.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыКонтроляОплатыПоставщику.КредитСдвиг)
	|				И НЕ ПланОплат.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыКонтроляОплатыПоставщику.КредитПослеПоступления))
	|
	|СГРУППИРОВАТЬ ПО
	|	&АналитикаПланОплат1,
	|	&АналитикаПланОплат2,
	...
	|	И (ПланОплатКредит.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыКонтроляОплатыПоставщику.КредитСдвиг)
	|		ИЛИ ПланОплатКредит.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыКонтроляОплатыПоставщику.КредитПослеПоступления))
	|
	|СГРУППИРОВАТЬ ПО
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ДанныеПечати.ЗаполнятьПартнераВТЧ Тогда
			...
			СтруктураДопПоля.Вставить("Заголовок", НСтр("ru = 'Поставщик';
														|en = 'Vendor'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Если ДанныеПечати.ЗаполнятьСоглашениеВТЧ Тогда
			...
			СтруктураДопПоля.Вставить("Заголовок", НСтр("ru = 'Соглашение';
														|en = 'Agreement'", ОбщегоНазначения.КодОсновногоЯзыка()));
