	...
	Если Объект.ЗаполнятьПланОплат Тогда
		...
		Если Объект.СуммаДокумента = 0 И Объект.ПланОплаты.Количество() > 0 Тогда
			...
			Если КоличествоЗапланированныхСтрок = 0 И Объект.ПланОплаты.Количество() > 0 Тогда
				...
				НСтр("ru = 'В плане все строки отменены. Таблица плана оплаты будет очищена';
					|en = 'All lines have been canceled. The Payment plan tab will be cleared.'"),
				НСтр("ru = 'В плане все строки отменены. Очистить таблицу плана оплаты?';
					|en = 'All plan lines are canceled. Clear the payment plan table?'"));
				
			Иначе
				
				ТекстВопроса = ?(Объект.Проведен,
				НСтр("ru = 'Сумма запланированных строк нулевая. Таблица плана оплаты будет очищена';
					|en = 'The lines total is 0. The Payment plan tab will be cleared.'"),
				НСтр("ru = 'Сумма запланированных строк нулевая. Очистить таблицу плана оплаты?';
					|en = 'Sum of the planned lines equals to 0. Clear the payment plan table?'"));
			...
			Если Объект.ПланОплаты.Количество() = 0 Тогда
				...
				Если ГрафикСоглашенияЗаполнен Тогда
					ТекстВопроса = НСтр("ru = 'Отсутствуют этапы плана оплаты. 
						...
						|en = 'No payment plan steps.
						|Fill in the steps according to the agreement?'");
				ИначеЕсли ГрафикЗаполнен Тогда
					ТекстВопроса = НСтр("ru = 'Отсутствуют этапы плана оплаты. 
						|Заполнить этапы в соответствии с графиком?';
						|en = 'No payment plan steps.
						|Fill in the steps according to the schedule?'");
				Иначе
					ТекстВопроса = НСтр("ru = 'Отсутствуют этапы плана оплаты. 
			...
			Иначе
				...
				Если ГрафикСоглашенияЗаполнен Тогда
					ТекстВопроса = НСтр("ru = 'Сумма этапов плана оплаты не совпадает с суммой запланированных строк. 
						...
						|en = 'Sum of the payment plan steps does not match the sum of planned lines.
						|Refill the steps in accordance with the agreement?'");
				ИначеЕсли ГрафикЗаполнен Тогда
					ТекстВопроса = НСтр("ru = 'Сумма этапов плана оплаты не совпадает с суммой запланированных строк. 
						|Перезаполнить этапы в соответствии с графиком?';
						|en = 'Sum of the payment plan steps does not match the sum of planned lines.
						|Refill the steps in accordance with the schedule?'");
				Иначе
					ТекстВопроса = НСтр("ru = 'Сумма этапов плана оплаты не совпадает с суммой запланированных строк. 
						|Перезаполнить этапы, добавив один этап ""%ВариантОплаты%"" на период с оплатой 100%?';
						|en = 'Sum of the payment plan steps does not match the sum of planned lines.
						|Refill the steps by adding the ""%ВариантОплаты%"" step for the period with 100% payment?'");
					ТекстВопроса = СтрЗаменить(ТекстВопроса, "%ВариантОплаты%", ПредопределенноеЗначение("Перечисление.ВариантыКонтроляОплатыПоставщику.ПредоплатаДоПоступления"));
				КонецЕсли;
		...
		ИначеЕсли Объект.Замещающий Тогда
			...
			Если ЗначениеЗаполнено(ТекстПериод) Тогда
				...
				|Перезаполнить этапы в соответствии с графиком?';
				|en = 'Sum of the payment plan steps does not match the sum of planned lines for periods: %ТекстПериод%.
				|Refill the steps in accordance with the schedule?'");
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ШаблонСообщения1 = НСтр("ru = 'Для номенклатуры %НоменклатураХарактеристика% 
		...
		|en = 'Cannot use plans with an empty field value and plans with a populated field value simultaneously for items %НоменклатураХарактеристика%
		| in one period and business unit:'") + " ";
	...
	ИначеЕсли ИмяСобытия = "Запись_КорректировкаПлановыхПотребностей" Тогда
		...
		Если Параметр.Свойство("План")
			...
			ТекстВопроса = НСтр("ru = 'Потребность в товарах изменилась, перезаполнить табличную часть?';
								|en = 'Demand for goods has changed, refill the table?'");
...
Процедура ЗаполнитьЭтапыГрафикаОплаты(Команда)
	...
	Если Объект.СуммаДокумента = 0 Тогда
		...
		Если Объект.ПланОплаты.Количество() = 0 Тогда
			...
			ПоказатьПредупреждение(,НСтр("ru = 'Сумма запланированных строк нулевая. Заполнение плана оплаты не требуется.';
										|en = 'Sum of the planned lines equals to 0. It is not required to fill in the payment plan.'"));
			Возврат;
		...
		ПоказатьОповещениеПользователя(
			...
			НСтр("ru = 'Сумма запланированных строк нулевая. Таблица этапов оплаты очищена';
				|en = 'Sum of planned lines equals to 0. Payment milestone table is cleared'"),
	...
	Если Объект.СуммаДокумента = СуммаОплатыПоЭтапам Тогда
		...
		ТекстВопроса = НСтр("ru = 'Сумма запланированных строк совпадает с суммой этапов оплаты';
							|en = 'Sum of planned lines matches the payment milestone sum'") + Символы.ПС +
	...
	ИначеЕсли Объект.ПланОплаты.Количество() > 0 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Таблица этапов оплаты заполнена';
							|en = 'Payment milestones table is filled in'")+ Символы.ПС +
							НСтр("ru = 'Перезаполнить этапы оплаты %ИсточникЗаполнения%?';
								|en = 'Refill steps of payment %ИсточникЗаполнения%?'");
...
Процедура ЗаполнитьСкладПоЗначению(Команда)
	...
	Если СкладыКлиент.ПроверитьВозможностьЗаполненияСкладовВТабличнойЧасти(Объект, ТаблицаТовары, НСтр("ru = 'Товары';
	...
	Если НЕ Объект.КроссТаблица Тогда
		...
		Если Отказ Тогда
			...
			ТекстВопроса = НСтр("ru = 'В строках списка ""Товары"" указаны даты за границами планирования - строки будут удалены. Удалить строки?';
								|en = 'Dates outside the planning fences are specified in lines of the Item list. Lines will be deleted. Delete the lines?'");
...
Процедура ПодобратьАналоги(Команда)
	...
	//++ НЕ УТКА
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Документ.ПланЗакупок.Форма.ФормаДокумента.Команда.ПодобратьАналоги");
...
Процедура СохранитьНовыйПериодСервер()
	...
	Если Объект.ПланироватьПоСумме Тогда
		НадписьПериодичностьВалюта = НСтр("ru = 'Периодичность: %Периодичность%, валюта: %Валюта%';
											|en = 'Time fence: %Периодичность%, currency: %Валюта%'");
	КонецЕсли; 
...
Процедура УстановитьВидимость()
	...
	Элементы.ТоварыПоПериодамПодобратьАналоги.Видимость = ПравоЧтенияАналогов;
	//-- НЕ УТКА
...
Функция НадписьОплата(Календарь, КоличествоЭтаповОплаты)
	...
	НадписьОплата = НСтр("ru = 'Этапов оплаты: %КоличествоЭтаповОплаты%, %Календарь%';
						|en = 'Payment milestones: %КоличествоЭтаповОплаты%, %Календарь%'");
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	КолонкиПоиска.Добавить(НСтр("ru = 'Уникальный идентификатор (Поставщик)';
								|en = 'Unique ID (Vendor)'"),"ИдентификаторПартнер");
	КолонкиПоиска.Добавить(НСтр("ru = 'Уникальный идентификатор (Соглашение)';
								|en = 'Unique ID (Agreement)'"),"ИдентификаторСоглашение");
	КолонкиПоиска.Добавить(НСтр("ru = 'Уникальный идентификатор (Склад)';
								|en = 'Unique ID (Warehouse)'"),"ИдентификаторСклад");
	...
	Если ЗаполнятьПартнераВТЧ Тогда
		КолонкиПоиска.Добавить(НСтр("ru = 'Поставщик';
									|en = 'Vendor'"),"ЗагружаемыйПартнер");
	КонецЕсли; 
	Если ЗаполнятьСоглашениеВТЧ Тогда
		КолонкиПоиска.Добавить(НСтр("ru = 'Соглашение';
									|en = 'Agreement'"),"ЗагружаемоеСоглашение");
	КонецЕсли; 
	Если ЗаполнятьСкладВТЧ Тогда
		КолонкиПоиска.Добавить(НСтр("ru = 'Склад';
									|en = 'Warehouse'"),"ЗагружаемыйСклад");
	КонецЕсли; 
	...
	Иначе
		...
		КолонкиПоиска.Добавить(НСтр("ru = 'Дата поступления';
									|en = 'Date of receipt'"),"ДатаПоступления");
		КолонкиПоиска.Добавить(НСтр("ru = 'Дата заказа';
									|en = 'Order date'"),"ДатаЗаказа");
		КолонкиПоиска.Добавить(НСтр("ru = 'Количество';
									|en = 'Quantity'"),"КоличествоУпаковок");
		КолонкиПоиска.Добавить(НСтр("ru = 'Ед. изм.';
									|en = 'Unit'"),"ЗагружаемаяУпаковка");
		Если Объект.ПланироватьПоСумме Тогда
			КолонкиПоиска.Добавить(НСтр("ru = 'Цена';
										|en = 'Price'"),"Цена");
			КолонкиПоиска.Добавить(НСтр("ru = 'Сумма';
										|en = 'Amount'"),"Сумма");
		КонецЕсли; 
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	Представление = НСтр("ru = 'Закупки прошедшего периода с учетом остатка';
						|en = 'Purchases of the previous period including remaining quantity'");
	РаботаСФормулами.ДобавитьОператор(ГруппаОператоров, Идентификатор, КонструкцияДляВставки, Представление);
	...
	Представление = НСтр("ru = 'Увеличение продаж прошлых периодов на 15%';
						|en = 'Previous period sales increase by 15%'");
	РаботаСФормулами.ДобавитьОператор(ГруппаОператоров, Идентификатор, КонструкцияДляВставки, Представление);
...
Функция ДополнительныеНастройки(СтруктураНастроек, ДополнительныеПоля = Неопределено)
	...
	Если НЕ ТипЗнч(СтруктураНастроек) = Тип("Структура") Тогда
		...
		СтруктураНастроек.Вставить("ФормулаПредставление",НСтр("ru = 'Увеличение планов закупок прошлых периодов на 15%';
																|en = 'Increase purchase plans of previous periods by 15%'"));
...
Процедура ТоварыПоПериодамПриАктивизацииЯчейкиНаКлиенте()
	...
	ИтогоПоСтроке = НСтр("ru = 'Всего по строке: %СуммаПоСтроке% %Валюта%.';
						|en = 'Line total: %СуммаПоСтроке% %Валюта%.'");
			...
			Если Элемент.ТекущийЭлемент.Имя = "ТоварыПоПериодам" + "Цена_"+Период.ИмяКолонки 
				...
				Если Объект.ПланироватьПоСумме Тогда
					...
					ИтогоПоКолонке = НСтр("ru = 'Всего за период %ПериодЗаголовок%: %СуммаПоКолонке% %Валюта%';
											|en = 'Total for the period %ПериодЗаголовок%: %СуммаПоКолонке% %Валюта%'");
...
Процедура ЗаменитьНаАналоги(АдресВХранилище)
	...
	ДополнительныеПоляЗаполнения = "Назначение, ДатаПоступления";
	
	Результат = АналогиМатериалов.ВыполнитьЗаменуНаАналоги(
		?(Объект.КроссТаблица, ТоварыПоПериодам, Объект.Товары), АдресВХранилище, ДополнительныеПоляЗаполнения);
...
Функция ПараметрыВыбораАналогов(ИмяТаблицы, Режим)
	...
	ТипИсточника = Новый ОписаниеТипов("ДокументСсылка.ПланЗакупок");
	...
	СтруктураПолей.ДополнительныеПоля.Вставить("Ссылка", Новый ОписаниеТипов("ДокументСсылка.ПланЗакупок"));
...
Процедура СформироватьКорректировкуПлановыхПотребностей(АдресВХранилище, ИндексыНовыхСтрок)
	...
	СтрокаЗамены = Объект.Товары.Получить(ИндексыНовыхСтрок[0]);
	ПараметрыКорректировки.НазначениеКорректировки = СтрокаЗамены.Назначение;
	ПараметрыКорректировки.Период = СтрокаЗамены.ДатаПоступления;
...
Процедура ПоказатьФормуЗагрузкиИзФайла(ДополнительныеПараметры)
	...
	ПараметрыЗагрузки.Заголовок               = НСтр("ru = 'Загрузка таблицы ""Товары"" из файла';
													|en = 'Import the Goods table from the file'");
	...
	Если Объект.КроссТаблица Тогда
		...
		Если Отказ Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'В строках списка ""Товары"" указаны даты за границами планирования - строки были удалены.';
														|en = 'Dates outside the planning fences are specified in lines of the Goods list. The lines were removed.'"));
		КонецЕсли;
