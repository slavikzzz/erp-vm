...
Процедура УстановитьДоступностьКомандУстановкиСтатуса()
	...
	ПравоДоступаУтверждениеПлановЗакупок = Пользователи.РолиДоступны("УтверждениеПлановЗакупок");
	
	Элементы.ГруппаУстановитьСтатус.Видимость = ПравоДоступаИзменениеПланЗакупок;
	Элементы.СписокУстановитьСтатусУтвержден.Видимость = ПравоДоступаУтверждениеПлановЗакупок;
...
Процедура УстановитьСтатус(Статус, ТексСтатуса)
	...
	ТекстВопроса = НСтр("ru = 'У выделенных в списке планов будет установлен статус ""%Статус%"". По принятым в работу планам могут быть оформлены документы. Продолжить?';
						|en = 'Plans selected in the list will have status ""%Статус%"". Documents can be registered according to the plans accepted into operation. Continue?'");
