...
Процедура ЗаполнитьПолученныеАвансыДляСФ(ОтборАвансов, ПолученныеАвансы, РасчетныеДокументыБезСчетаФактуры = Неопределено) Экспорт
	...
	Если ТипЗнч(Организация) = Тип("Массив") И Организация.Количество() = 1 Тогда
		...
		НалогообложениеНДСПоУмолчанию = УчетНДСУП.ПараметрыУчетаПоОрганизации(Организация[0], КонецПериода).ОсновноеНалогообложениеНДСПродажи;
	ИначеЕсли ТипЗнч(Организация) = Тип("СправочникСсылка.Организации") Тогда
		...
		НалогообложениеНДСПоУмолчанию = УчетНДСУП.ПараметрыУчетаПоОрганизации(Организация, КонецПериода).ОсновноеНалогообложениеНДСПродажи;
	Иначе
	...
	НалогообложениеНДСПоУмолчанию = ?(
		ЗначениеЗаполнено(НалогообложениеНДСПоУмолчанию),
		НалогообложениеНДСПоУмолчанию,
	...
	Пока ВыборкаАванс.Следующий() Цикл
		...
		Для каждого Ставка Из МассивСтавок Цикл
			...
			Если НЕ ЗначениеЗаполнено(НоваяСтрока.НалогообложениеНДС) Тогда 
				НоваяСтрока.НалогообложениеНДС = НалогообложениеНДСПоУмолчанию;
			КонецЕсли;
...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры)
	...
	//++ НЕ УТ
	СчетФактураВыданныйАвансЛокализация.ДополнитьТекстЗапросаРеестрДокументов(Запрос, ТекстПроведения, Регистры);
	//-- НЕ УТ
