...
Процедура НастроитьСписок(ВидИнцидента = Неопределено, ОтключениеОтбора = Ложь)
	...
	Если ОграничениеВыбораВидаИнцидентов Тогда
		...
		ТекстСообщения = (НСтр(
			...
			|en = 'If an incident is already under review, changes to an incident kind might lead to issues. On the screen, you can see incident kinds that you can select without causing issues.'"));

		Если Пользователи.РолиДоступны("РедактированиеРеквизитовОбъектов") Тогда
			ИмяНавигационнойСсылки = "ОтключитьОтбор";
			ТекстСообщения = СтрШаблон(НСтр(
				"ru = 'Если инцидент уже в работе, то произвольное изменение вида инцидента может привести к проблемам. Показаны виды инцидентов, которые можно выбрать без привнесения проблем. <a href=""%1"">Показать все виды инцидентов.</a>';
				|en = 'If an incident is already under review, changes to an incident kind might lead to issues. You can see the incident kinds that you can select without causing issues. <a href=""%1"">Show all incident kinds.</a>'"), ИмяНавигационнойСсылки);
		КонецЕсли;
