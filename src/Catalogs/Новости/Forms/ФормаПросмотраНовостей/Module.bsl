...
Процедура ОбработкаПолученияСпискаНавигационныхСсылок(СписокНавигационныхСсылок, КлючПоУмолчанию)
	...
	СписокНавигационныхСсылок.Добавить("All"      , НСтр("ru = 'Новости: Все новости';
														|en = 'News: all news'")                  , Ложь);
	СписокНавигационныхСсылок.Добавить("Marked"   , НСтр("ru = 'Новости: Помеченные новости';
														|en = 'News: Marked news'")           , Ложь);
	СписокНавигационныхСсылок.Добавить("Urgent"   , НСтр("ru = 'Новости: Важные и очень важные новости';
														|en = 'News: Important and urgent news'"), Ложь);
	СписокНавигационныхСсылок.Добавить("ReadLater", НСтр("ru = 'Новости: Прочитать позже';
														|en = 'News: Read later'")              , Ложь);
	СписокНавигационныхСсылок.Добавить("Settings" , НСтр("ru = 'Новости: Окно настроек';
														|en = 'News: Settings window'")                , Ложь);
...
Процедура КомандаПоиск(Команда)
	...
	Для каждого ТекущаяПапка Из ЭтотОбъект.Папки.ПолучитьЭлементы() Цикл
		...
		Если ТекущаяПапка.ВариантОтбора = 2 Тогда // Результаты поиска
			...
			Иначе
				ТекущаяПапка.Представление = НСтр("ru = 'Результаты поиска';
													|en = 'Search results'") + " (" + СокрЛП(ЭтотОбъект.СтрокаПоиска) + ")";
			КонецЕсли;
			...
			Если Команда.Имя = "КомандаПрочитатьПозжеУдалитьИзСписка" Тогда // Мы в папке "Прочитать позже" - сбросить признак.
				...
				Если МассивНовостей.Количество() = 1 Тогда
					ТекстОповещения = НСтр("ru = 'Новость удалена из списка [Прочитать позже]';
											|en = 'News is deleted from list [Read later]'");
				Иначе // > 1
					ТекстОповещения = НСтр("ru = 'Новости удалены из списка [Прочитать позже]';
											|en = 'News are deleted from list [Read later]'");
				КонецЕсли;
			...
			Иначе // Мы в любой папке, кроме "Прочитать позже" - включить в список "Прочитать позже".
				...
				Если МассивНовостей.Количество() = 1 Тогда
					ТекстОповещения = НСтр("ru = 'Новость добавлена в список [Прочитать позже]';
											|en = 'News is added to list [Read later]'");
				Иначе // > 1
					ТекстОповещения = НСтр("ru = 'Новости добавлены в список [Прочитать позже]';
											|en = 'News are added to list [Read later]'");
				КонецЕсли;
				ПоказатьОповещениеПользователя(
					ТекстОповещения,
					,
...
Процедура ОбновитьИнформационныеСтрокиКлиент()
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Последнее обновление списка новостей было
			...
			|%2
			|%3';
			|en = 'News list was last updated 
			|%1
			|The next news list update will be
			|%2
			|%3'"),
		...
		Пока Выборка.Следующий() Цикл
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1, последняя новость от %2, последняя загрузка %3';
					|en = '%1, the latest news dated %2, the latest download %3'"),
...
Процедура УправлениеФормой(Знач Форма)
	...
	ТекстПодсказки = ТекстПодсказки + Символы.ПС + Символы.ПС
		+ НСтр("ru = 'Доступно использование подстановочных знаков:
			...
			|en = 'You can use wildcards:
			|% - any character sequence:
			|_ (underscore) - any character;
			|[a-z] - characters from a to z;
			|[^a-z] - characters excluding ones from a to z;
			|\ - a special character transforming a wildcard into a simple search character.'");
...
Процедура ОбновитьСписокПапокОтбора()
	...
	НоваяСтрока = КорневойУровень.Добавить();
		...
		НоваяСтрока.Наименование           = НСтр("ru = 'Все важные и очень важные новости';
													|en = 'All important and urgent news'");
		НоваяСтрока.КоличествоНепрочтенных = 0; //// ... Всегда = 0, т.к. расчет количества непрочтенных для этой ситуации будет ресурсоемким
		НоваяСтрока.ВыделеннаяСтрока       = Ложь;
...
Процедура ИзменитьПодпискуНаКлиенте(Знач ЛентаНовостей, СостояниеПодписки, СохранятьВБазе)
	...
	Если ПапкаВсеПодписанные = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru = 'Не найдена папка со списком всех лент новостей';
								|en = 'Folder with a list of all news feeds is not found'");
		...
		Если ТекущиеДанные.ОбязательныйКанал = Истина Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Лента новостей %1 является обязательной.
					...
					|en = 'The %1 news feed is required.
