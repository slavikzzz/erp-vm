...
Процедура ПередЗаписью(Отказ)
	...
	ДополнительныеСвойстваЭталон = Новый Структура;
		ДополнительныеСвойстваЭталон.Вставить("ЭтоЗагрузкаНовостей", Ложь);
	...
	ИначеЕсли (СокрЛП(ВРег(УИННовости)) = ВРег("313f4928-a71c-4cef-96ee-a4b6106c1275"))
		...
		ТекстСообщения = НСтр("ru = 'Перед записью новости про системную ошибку у нее было установлено свойство СкрыватьВОбщемСпискеНовостей, чтобы она не отображалась пользователю.
			...
			|en = 'Before a news about system error was written, property СкрыватьВОбщемСпискеНовостей was assigned for it not to be displayed to a user.
			|News data:
			|Payment ID: %УИННовости%
			|News text: %ТекстНовости%'");
		...
		ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Изменение данных';
				|en = 'ISL:News.Data change'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Изменение данных. Новость. Перед записью';
				|en = 'News. Change data. News. Before writing'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
	...
	Если (ДополнительныеСвойстваЭталон.ЭтоЗагрузкаНовостей = Истина) Тогда
	...
	Иначе
		...
		Если ВыполнениеРазрешено = Ложь Тогда
			...
			ТекстСообщения = НСтр("ru = 'В разделенном режиме разрешено редактировать новости только в неразделенном сеансе.';
									|en = 'In the split mode, you can edit news only in an undivided session.'");
				...
				Иначе
					Если ЭтоРедакторНовостей = Истина Тогда
						...
						Если ЛентаНовостей.ЗагруженоССервера = Истина Тогда
							...
							ТекстСообщения = НСтр("ru = 'Запрещено записывать новость в ленту новостей, загруженную с сервера.';
													|en = 'It is prohibited to record news in a news feed imported from the server.'");
							ВызватьИсключение ТекстСообщения;
						ИначеЕсли ЛентаНовостей.ЛокальнаяЛентаНовостей = Ложь Тогда
							...
							ТекстСообщения = НСтр("ru = 'Запрещено записывать новость в ленту новостей, которая не является локальной.';
													|en = 'It is prohibited to record news in a news feed that is not local.'");
							ВызватьИсключение ТекстСообщения;
						Иначе
							Запрос = Новый Запрос;
							Запрос.Текст = "
							...
							Если РезультатЗапроса.Пустой() Тогда
								...
								ТекстСообщения = НСтр("ru = 'У текущего пользователя (%1)
									...
									|en = 'The current user (%1)
									|is not authorized to record news in a news feed%2.'");
					...
					Иначе
						...
						ТекстСообщения = НСтр("ru = 'Нет прав для записи новости.
							...
							|en = 'No rights to record news.
							|Recording is available:
							| - for the system administrator role (without restrictions);
							| - for the news editor role (with restrictions).'");
						ВызватьИсключение ТекстСообщения;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	ДополнительныеСвойстваЭталон = Новый Структура;
		ДополнительныеСвойстваЭталон.Вставить("ЭтоНовый"                                                                  , Ложь);
		ДополнительныеСвойстваЭталон.Вставить("ТребуетсяПересчетОтборов"                                                  , Ложь);
		ДополнительныеСвойстваЭталон.Вставить("СброситьПользовательскиеСвойстваПрочтенностиНовости"                       , Ложь);
		ДополнительныеСвойстваЭталон.Вставить("ВажностьДоЗаписи"                                                          , Важность);
		ДополнительныеСвойстваЭталон.Вставить("РегистрыСведений_КатегорииНовостейИнтервалыВерсийНеобработанные_Записи"    , Новый Массив);
		ДополнительныеСвойстваЭталон.Вставить("РегистрыСведений_КатегорииНовостейИнтервалыВерсийНеобработанные_Записывать", Ложь);
		ДополнительныеСвойстваЭталон.Вставить("РегистрыСведений_КатегорииНовостейПростыеНеобработанные_Записи"            , Новый Массив);
		ДополнительныеСвойстваЭталон.Вставить("РегистрыСведений_КатегорииНовостейПростыеНеобработанные_Записывать"        , Ложь);
	ЗаполнитьЗначенияСвойств(ДополнительныеСвойстваЭталон, ДополнительныеСвойства);

	// Если увеличили важность новости с "Обычная" до "Важная" или до "Очень важная", то установить признак "ОповещениеВключено".
	Если ДополнительныеСвойстваЭталон.ЭтоНовый <> Истина Тогда
		Если ( // Изменилась важность новости - новость стала Важная или Очень важная.
					(ДополнительныеСвойстваЭталон.ВажностьДоЗаписи <> Важность)
					И (ДополнительныеСвойстваЭталон.ВажностьДоЗаписи = 0)
					И (Важность > 0))
				// Если новость загружается заново (перепубликация, отзыв новости и т.п.),
				//  то сбросить все пользовательские свойства прочтенности новости - прочтенность, оповещение и т.п.
				ИЛИ (ДополнительныеСвойстваЭталон.СброситьПользовательскиеСвойстваПрочтенностиНовости = Истина) Тогда

			СброситьСостояниеНовости = Истина;
			ИнтеграцияПодсистемБИП.ПереопределитьСбросСостоянияНовостиПриЗаписи(ЭтотОбъект, СброситьСостояниеНовости);
			...
			Если СброситьСостояниеНовости <> Ложь Тогда
				ОбработкаНовостейСлужебный.СбросСостоянияНовостиДляВсехПользователей(Ссылка);
			КонецЕсли;
	...
	Запись = РегистрыСведений.СвойстваНовостей.СоздатьМенеджерЗаписи();
		...
		Запись.ТребуетсяПересчетОтборов      = (ДополнительныеСвойстваЭталон.ТребуетсяПересчетОтборов = Истина);
	Запись.Записать(Истина);

	// При загрузке новости регистры сведений "КатегорииНовостейИнтервалыВерсийНеобработанные" и "КатегорииНовостейПростыеНеобработанные"
	//  теперь не записываются по одной записи, а сохраняются в памяти объекта (ДополнительныеСвойства) и записываются через набор записей однократно.
	Записывать = ДополнительныеСвойстваЭталон.РегистрыСведений_КатегорииНовостейИнтервалыВерсийНеобработанные_Записывать;
	Записи     = ДополнительныеСвойстваЭталон.РегистрыСведений_КатегорииНовостейИнтервалыВерсийНеобработанные_Записи;
	Если Записывать = Истина Тогда
		...
		НаборЗаписей.Отбор["ЛентаНовостей"].Установить(ЛентаНовостей, Истина);
		НаборЗаписей.Отбор["УИННовости"].Установить(УИННовости, Истина);
		Для Каждого ТекущаяЗапись Из Записи Цикл
			НоваяЗапись = НаборЗаписей.Добавить();
	...
	Записывать = ДополнительныеСвойстваЭталон.РегистрыСведений_КатегорииНовостейПростыеНеобработанные_Записывать;
	Записи     = ДополнительныеСвойстваЭталон.РегистрыСведений_КатегорииНовостейПростыеНеобработанные_Записи;
	Если Записывать = Истина Тогда
