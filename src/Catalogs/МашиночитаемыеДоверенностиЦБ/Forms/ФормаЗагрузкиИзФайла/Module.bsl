...
Процедура СохранитьИзменения(Команда)
	...
	РезультатЗагрузки = ДокументооборотСКОВызовСервера.ЗагрузитьМЧДЦБ(
		XMLФайл.Адрес,
...
Процедура ОбработкаНавигационнойСсылкиПослеВыбораФайла(Результат, ИмяЭлемента) Экспорт
	...
	Если ЭтоXmlФайл Тогда
		Если НЕ СтрНайти(Врег(ОписаниеФайла.Имя), "DOVER_CBR") Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Имя файла машиночитаемой доверенности Банка России должно начинаться на DOVER_CBR';
					|en = 'Имя файла машиночитаемой доверенности Банка России должно начинаться на DOVER_CBR'"));
			Возврат;
		КонецЕсли;
