...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Ссылка") Тогда
		...
		Если ТипЗнч(Параметры.Ссылка) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
			...
			//++ НЕ УТКА
			ИначеЕсли Параметры.ТипДоговора = Перечисления.ТипыДоговоров.СДавальцем2_5
				//++ Устарело_Переработка24
				Или Параметры.ТипДоговора = Перечисления.ТипыДоговоров.СДавальцем
				//-- Устарело_Переработка24
				Или Ложь Тогда
				
				СтрокаОтбора = СтрокаОтбора + " И ((НаправленияДеятельности.УчетЗатрат = ЛОЖЬ И НаправленияДеятельности.УчетДоходов = ИСТИНА)
					|ИЛИ (НаправленияДеятельности.УчетЗатрат = ИСТИНА И НаправленияДеятельности.УчетДоходов = ИСТИНА))";
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 50
		...
		|ГДЕ
		|	НаправленияДеятельности.Наименование ПОДОБНО &СтрокаВвода" + СтрокаОтбора;
