...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Иначе
		...
		Если МакетИзменен Тогда
			Отказ = Истина;
			ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект), НСтр("ru = 'Закрыть без сохранения?';
																									|en = 'Close without saving?'"), РежимДиалогаВопрос.ДаНет);
		КонецЕсли;
	...
	Если ВыбранноеЗначение<>ШагСетки Тогда
		...
		Если УпраздняемыеПоля.Количество()>0 Тогда
			...
			СтрокаВопрос = НСтр("ru = 'Поля:%Поля%имеют слишком маленькие размеры.%ПереносСтроки%Продолжить изменение шага сетки, удалив указанные поля?';
								|en = 'Fields:%Поля% are too small.%ПереносСтроки%Continue changing the grid step by deleting the specified fields?'");
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	#Если ВебКлиент Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("НачатьЗагрузкуИзФайла", ЭтотОбъект);
	...
	#Иначе
		...
		НачатьЗагрузкуИзФайла(Истина, Неопределено);
	#КонецЕсли
...
Процедура СохранитьВФайл(Команда)
	...
	#Если ВебКлиент Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("НачатьСохранениеВФайл", ЭтотОбъект);
		ПроверитьДоступностьРасширенияРаботыСФайлами(ОписаниеОповещения, Истина);
	#Иначе
		// В тонком и толстом клиентах расширение подключено всегда.
		НачатьСохранениеВФайл(Истина, Неопределено);
	#КонецЕсли
КонецПроцедуры
...
Процедура ОчиститьМакет(Команда)
	...
	Если Поля.Количество()>0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОчиститьМакетЗавершение", ЭтотОбъект), НСтр("ru = 'Очистить макет?';
																							|en = 'Clear template?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
		ОчисткаМакета();
		МакетИзменен = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьПоДаннымМакетаЗавершение(ДополнительныеПараметры) Экспорт
	...
	ТекстСообщения = "";
	СодержимоеXMLФайла = Неопределено; // Структура
	...
	Если ПолучитьОписаниеМакетаНаСервере(Источник, СообщениеОбОшибке, СодержимоеXMLФайла) Тогда
		...
		Для Каждого ТекПоле Из СодержимоеXMLФайла.Поля Цикл
			...
			ИначеЕсли ТекПоле.Тип="Barcode" Тогда
				ПредставлениеПоля = СтрШаблон(НСтр("ru = 'Штрихкод %1';
													|en = 'Barcode %1'"), Строка(ТекПоле.ТипШтрихкода));
...
Процедура ВыборФайла()
	...
	ДОФ.Фильтр = НСтр("ru = 'Файл шаблона';
						|en = 'Template file'") + "(*.xml)|*.xml|" + НСтр("ru = 'Файл шаблона';
																		|en = 'Template file'") + "(*.mxl)|*.mxl"; 
	...
	Если ПомещенныеФайлы.Количество() > 0 Тогда
		...
		Если XMLОписаниеМакета = "" Тогда
			ТекстСообщения = НСтр("ru = 'Не удалось конвертировать макет.';
									|en = 'Cannot convert the template.'");
			ОбщегоНазначенияБПОКлиент.СообщитьПользователю(ТекстСообщения);
		Иначе
			ЗаполнитьПоДаннымМакета();
		КонецЕсли;
...
Процедура СохранитьОписаниеПоля(ОписаниеПоля, РедактируемоеПоле=Неопределено)
	...
	Если ОписаниеПоля.Тип<>"UserData" Тогда
		...
		ИначеЕсли ОписаниеПоля.Тип = "Barcode" Тогда
			ПредставлениеПоля						= СтрШаблон(НСтр("ru = 'Штрихкод %1';
																		|en = 'Barcode %1'"), Строка(ОписаниеПоля.ТипШтрихкода));
...
Процедура СохранитьОформлениеПоля(ОписаниеПоля, ОткрытиеМакета=Ложь, ТабличныйДокумент=Неопределено)
	...
	#Иначе
		...
		ИначеЕсли ОписаниеПоля.Тип = "Barcode" Тогда		
			...
			Область.Текст = СтрШаблон(НСтр("ru = 'Штрихкод %1';
											|en = 'Barcode %1'"), Строка(ОписаниеПоля.ТипШтрихкода));
			Область.РазмещениеТекста=ТипРазмещенияТекстаТабличногоДокумента.Переносить;
...
Процедура ПредложитьПользователюПродолжитьРедактирование(ОписаниеПоля, РедактируемоеПоле)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ПредложитьПользователюПродолжитьРедактированиеЗавершение", ЭтотОбъект, ПередаваемыеПараметры),
					НСтр("ru = 'Запись невозможна. Текущее значение реквизита ""Имя поля"" уже используется. Продолжить редактирование?';
						|en = 'Cannot save. The current value of the ""Field name"" attribute is already used. Continue editing?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОпроситьПользователяОбУдаленииПересекаемыхПолей(ПередаваемыеПараметры)
	...
	Если ПередаваемыеПараметры.Свойство("РедактируемоеПоле") Тогда
		ТекстВопроса = НСтр("ru = 'Новые границы редактируемого поля пересекаются с полями:%СписокПолей%Отменить внесенные изменения?';
							|en = 'New borders of the edited field overlap fields:%СписокПолей% Cancel the changes?'");
	Иначе
		ТекстВопроса = НСтр("ru = 'Создаваемое поле пересекается с уже существующими полями:%СписокПолей%Отменить внесенные изменения?';
							|en = 'Created field overlaps with the existing fields:%СписокПолей%Cancel the changes made?'");
	КонецЕсли;
...
&НаКлиенте
Процедура НачатьЗагрузкуИзФайла(Результат, ДополнительныеПараметры) Экспорт
	
	Если Поля.Количество()>0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗагрузитьИзФайлаЗавершение", ЭтотОбъект), НСтр("ru = 'Загрузить макет этикетки из файла?';
																								|en = 'Загрузить макет этикетки из файла?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
		ВыборФайла();
	КонецЕсли;
...
&НаКлиенте
Процедура НачатьСохранениеВФайл(Результат, ДополнительныеПараметры) Экспорт
	
	// АПК: 1348-выкл не используется модуль ФайловаяСистемаКлиент из БСП
	ДОФ = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
	ДОФ.ПолноеИмяФайла = ИмяФайла;
	ДОФ.Фильтр = НСтр("ru = 'XML-файл';
						|en = 'XML-файл'") + "(*.xml)|*.xml"; 
	ДОФ.МножественныйВыбор = Ложь;
	ДОФ.Заголовок = НСтр("ru = 'Сохранение файла-макета';
						|en = 'Сохранение файла-макета'");
...
Процедура НачатьПодключениеРасширенияРаботыСФайламиЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Не Подключено И ДополнительныеПараметры.ПредлагатьУстановку Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для продолжении работы необходимо установить расширение для веб-клиента ""1С:Предприятие"". Установить?';
								|en = 'Для продолжении работы необходимо установить расширение для веб-клиента ""1С:Предприятие"". Установить?'");
		ПоказатьВопрос(Оповещение, ТекстСообщения, РежимДиалогаВопрос.ДаНет); 
	КонецЕсли;
	
	Если Подключено И ДополнительныеПараметры.ОповещениеПриЗавершении <> Неопределено Тогда
