	...
	ИначеЕсли ПолучитьОписаниеМакетаНаСервере(XMLОписаниеМакета, СообщениеОбОшибке, СодержимоеXMLФайла) Тогда
		...
		Для Каждого ТекПоле Из СодержимоеXMLФайла.Поля Цикл
			...
			ИначеЕсли ТекПоле.Тип="Barcode" Тогда
				НовоеПоле.Представление = СтрШаблон(НСтр("ru = 'Штрихкод %1';
			...
			Если ТекПоле.Тип<>"UserData" Тогда
				СохранитьОформлениеПоля(НовоеПоле, Истина, Макет);
			КонецЕсли;
...
Процедура ВыборФайлаСКД()
	...
	ДОФ.Заголовок = НСтр("ru = 'Выберите файл макета СКД';
						|en = 'Select the DCS template file'");
