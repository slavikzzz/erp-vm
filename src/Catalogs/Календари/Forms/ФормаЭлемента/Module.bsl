...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ШаблонЗаполнения.НайтиСтроки(Новый Структура("ДеньВключенВГрафик", Истина)).Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Не отмечены дни, включаемые в график работы';
				|en = 'Please select the days to include in the work schedule.'"), , "Объект.ШаблонЗаполнения", , Отказ);
	КонецЕсли;
...
Процедура НомерТекущегоГодаПриИзменении(Элемент)
	...
	Если МодифицированностьРезультата Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Записать измененные данные за %1 год?';
																						|en = 'Do you want to save the changes for year %1?'"), Формат(НомерПредыдущегоГода, "ЧГ=0"));
...
Функция ПредставлениеВремениСуток(Знач ВремяСуток, ДвенадцатиЧасовойФорматВремени)
	...
	ФорматВремени = ?(ДвенадцатиЧасовойФорматВремени,
		НСтр("ru = 'ДФ=''чч:мм вв''';
			|en = 'DF=''hh:mm tt'''"), НСтр("ru = 'ДФ=ЧЧ:мм; ДП=';
											|en = 'DF=hh:mm tt; DE='"));
