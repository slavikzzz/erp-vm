	...
	Если СвойстваФайлов <> Неопределено Тогда
		...
		Если КонтекстЭДОСервер = Неопределено Тогда 
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Электронное представление не записано:%1.';
																							|en = 'Электронное представление не записано:%1.'"),
...
Процедура ПолучательПриИзменении(Элемент)
	...
	Если Элементы.Получатель.Видимость Тогда
		КодПолучателяПФР = ?(ТипЗнч(Объект.Получатель) = Тип("СправочникСсылка.ОрганыПФР"),
			ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЗначениеРеквизитаОбъекта(Объект.Получатель, "Код"),
			"");
		
		Если СокрЛП(КодПолучателяПФР) = "ЕЦП" Тогда
			Объект.Получатель = Неопределено;
			ПоказатьПредупреждение(, НСтр("ru = 'Получатель не поддерживается';
											|en = 'Получатель не поддерживается'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПолучателяИзСвойствОрганизации()
	...
	ИначеЕсли ТипПолучателя = Перечисления.ТипыКонтролирующихОрганов.ПФР Тогда
		...
		ИначеЕсли СвойстваФайлов.Количество() = 1 Тогда
			ОсновнойФайл = СвойстваФайлов[0];
		Иначе
			ОсновнойФайл = ПолучитьОсновнойФайл();
		КонецЕсли;
		ИмяОсновногоФайла = ?(ОсновнойФайл = Неопределено, "", ОсновнойФайл.ИмяФайла);
		РегНомерСФР = Неопределено;
		ИспользоватьОрганЕЦП = (КонтекстЭДОСервер().ФорматДокументооборотаСПФР(Объект.ВидОтчета).ФорматДокументооборота =
			Перечисления.ФорматыДокументооборотаСПФР.ЭДОК
			И ДокументооборотСКОКлиентСервер.ИспользоватьРегистрационныйНомерСФР(Объект.ВидОтчета, ИмяОсновногоФайла));
		ОрганПФР = КонтекстЭДОСервер().ОпределитьОрганПФРОрганизации(
			Организация,,,
			РегНомерСФР,
			ИспользоватьОрганЕЦП);
		Если ЗначениеЗаполнено(ОрганПФР) Тогда
			Объект.Получатель = ОрганПФР;
		ИначеЕсли НЕ ИспользоватьОрганЕЦП И ТипЗнч(Объект.Получатель) = Тип("СправочникСсылка.ОрганыПФР")
			И СокрЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Получатель, "Код")) = "ЕЦП" Тогда
			Объект.Получатель = Неопределено;
		КонецЕсли;
	ИначеЕсли ТипПолучателя = Перечисления.ТипыКонтролирующихОрганов.ФСС Тогда
		...
		Если ЗначениеЗаполнено(СведенияОбОрганизации) И ЗначениеЗаполнено(СведенияОбОрганизации.КодПодчиненностиФСС) Тогда
			Объект.Получатель = Лев(СокрЛП(СведенияОбОрганизации.КодПодчиненностиФСС), 5);
		КонецЕсли;
	...
	Если ВыбранныеФайлы.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Среди выбранных файлов не удалось обнаружить ни одного файла с расширением ""xml"", ""txt"", ""xlsm"", ""xls"", ""pdf"", ""docx"" или ""doc""!';
				|en = 'Среди выбранных файлов не удалось обнаружить ни одного файла с расширением ""xml"", ""txt"", ""xlsm"", ""xls"", ""pdf"", ""docx"" или ""doc""!'"));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПривестиПолучателяКТипу(Форма)
	...
	Если ЗначениеЗаполнено(ТипПолучателя) Тогда
		...
		ИначеЕсли ТипПолучателя = ТипФСС Тогда
			МассивТипов.Добавить(Тип("Строка"));
			ПараметрыСтроки = Новый КвалификаторыСтроки(5);
			ДопустимыйТип = Новый ОписаниеТипов(МассивТипов, , ПараметрыСтроки);
		ИначеЕсли ТипПолучателя = ТипФСГС Тогда
			МассивТипов.Добавить(Тип("СправочникСсылка.ОрганыФСГС"));
			ДопустимыйТип = Новый ОписаниеТипов(МассивТипов);
...
Функция ПроверитьФайлыВыгрузкиУведомленияОКонтролируемыхСделках(ФайлыОтчетов)
	...
	Для каждого ФайлОтчета Из ФайлыОтчетов Цикл
		...
		Если РазмерФайла(ВремФайл) > МаксимальныйРазмер Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Размер файла %2 превышает максимально допустимый (%1 Мб). Загрузка файлов будет прервана';
																							|en = 'Размер файла %2 превышает максимально допустимый (%1 Мб). Загрузка файлов будет прервана'"), 
				...
				Если ФайлXML.Имя = "ВерсФорм" Тогда
					...
					Если НоваяСтрокаТаблицы.ВерсФорм <> "5.02" И ФайлыОтчетов.Количество() > 1 Тогда
						...
						ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Файлы с версией, отличной от 5.02, не могут быть загружены комплектом';
																	|en = 'Файлы с версией, отличной от 5.02, не могут быть загружены комплектом'"));
						Возврат Ложь;
	...
	Если ТаблицаПроверкиИдФайлИсх.Количество() > 1 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'В файлах различаются значения в полях ""ИдФайлИсх"", ""ИдФайлПерв"". Загрузка файлов будет прервана';
													|en = 'В файлах различаются значения в полях ""ИдФайлИсх"", ""ИдФайлПерв"". Загрузка файлов будет прервана'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если КоличествоИзТэгаКолФайл <> КоличествоВыбранныхФайлов Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'В описании к файлам число файлов комплекта не соответствует числу выбранных файлов. Загрузка файлов будет прервана';
													|en = 'В описании к файлам число файлов комплекта не соответствует числу выбранных файлов. Загрузка файлов будет прервана'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ТаблицаПроверкиКолФайл.Количество() > 1 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'В файлах различается значение в поле ""КолФайл"". Загрузка файлов будет прервана';
													|en = 'В файлах различается значение в поле ""КолФайл"". Загрузка файлов будет прервана'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Для сч = 0 По КоличествоФайлов - 1 Цикл
		...
		Если НомерТекущегоФайла <> сч + 1 Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не все файлы комплекта выбраны. Загрузка файлов будет прервана';
														|en = 'Не все файлы комплекта выбраны. Загрузка файлов будет прервана'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ОпределитьТипФайлаПФР(ПолноеИмяФайла)
	...
	Если ВРЕГ(Лев(КороткоеИмяФайла, 4)) = "ПФР_" ИЛИ ВРЕГ(Лев(КороткоеИмяФайла, 4)) = "СФР_" Тогда
...
Функция ПроанализироватьФайлВыгрузкиФСРАР(ОсновнойФайл)
	...
	Если ВидОтправляемогоДокумента = Неопределено Тогда
		РегламентированнаяОтчетностьКлиентСервер.СообщитьОбОшибке(НСтр("ru = 'Не удалось определить тип файла выгрузки!';
																		|en = 'Не удалось определить тип файла выгрузки!'"), Ложь);
		Возврат Ложь;
	КонецЕсли;
...
Функция ПроанализироватьФайлВыгрузкиРПН(ОсновнойФайл)
	...
	Иначе
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось определить тип файла выгрузки!';
													|en = 'Не удалось определить тип файла выгрузки!'"));
		Возврат Ложь;
	КонецЕсли;
	ПараметрыИмпорта.Вставить("ВидОтчета", ВидОтправляемогоДокумента);
				...
				ИначеЕсли ИННВПервомУзел <> ИННВТекущемУзле Тогда
					...
					ОбщегоНазначения.СообщитьПользователю(
						НСтр("ru = 'Отправка файлов, содержащих отчетность по нескольким организациям, не поддерживается';
							|en = 'Отправка файлов, содержащих отчетность по нескольким организациям, не поддерживается'"));
					Возврат Ложь;
				КонецЕсли;
			КонецЕсли;
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Файл ""%1"" не является файлом выгрузки в формате XML.';
					|en = 'Файл ""%1"" не является файлом выгрузки в формате XML.'"), КороткоеИмяФайла);
...
Функция ПроанализироватьФайлВыгрузкиОтчет4ФСС(ПолноеИмяФайла)
	...
	Если КодПодчиненности <> Неопределено Тогда
		ПараметрыИмпорта.Вставить("Получатель", Лев(КодПодчиненности,5));
	КонецЕсли;
...
Функция ПроанализироватьФайлВыгрузкиЦБ(ПолноеИмяФайла)
	...
	Если Расширение <> "xml" Тогда
		...
		ИначеЕсли СтрНайти(КороткоеИмяФайла, "1-СТР") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1СТР") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "1-STR") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1STR") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "СТР-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "СТР1") > 0
			ИЛИ СтрНайти(КороткоеИмяФайла, "STR-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "STR1") > 0 Тогда
			Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.Форма1СТР;
...
Функция ОпределитьВидОтчетаПФР(ДеревоXML, ВидКомплектовОтправляемыхДокументов = Неопределено, КороткоеИмяФайла = "")
	...
	ИначеЕсли СвойстваУзла.Получить("ЕФС-1") <> Неопределено ИЛИ СвойстваУзла.Получить("ЕФС_1") <> Неопределено
		И СтрНайти(ВРег(КороткоеИмяФайла), "_УППО_") = 0 Тогда
		...
		ИначеЕсли СвойстваУзла.Получить("ЕФС_1") <> Неопределено
			И УзелПачкаВходящихДокументов.ЕФС_1.Свойства().Получить("СЗВ") <> Неопределено
			И УзелПачкаВходящихДокументов.ЕФС_1.СЗВ.Свойства().Получить("ДНП") <> Неопределено Тогда
			Возврат Справочники.ВидыОтправляемыхДокументов.ОписьОДВ_1;
		ИначеЕсли СвойстваУзла.Получить("ЕФС_1") <> Неопределено
			И УзелПачкаВходящихДокументов.ЕФС_1.Свойства().Получить("СЗВ") <> Неопределено
			И УзелПачкаВходящихДокументов.ЕФС_1.СЗВ.Свойства().Получить("ДСВ") <> Неопределено Тогда
			Возврат Справочники.ВидыОтправляемыхДокументов.РеестрДСВ3;
		ИначеЕсли СвойстваУзла.Получить("ЕФС_1") <> Неопределено
			И УзелПачкаВходящихДокументов.ЕФС_1.Свойства().Получить("СЗВ") <> Неопределено
			И УзелПачкаВходящихДокументов.ЕФС_1.СЗВ.Свойства().Получить("ЗЛ") <> Неопределено
			И ОбщегоНазначенияЭДКОКлиентСервер.ПервыйОбъект(
				УзелПачкаВходящихДокументов.ЕФС_1.СЗВ.ЗЛ).Свойства().Получить("СЗВ_ТД") <> Неопределено Тогда
			Возврат Справочники.ВидыОтправляемыхДокументов.СведенияОТрудовойДеятельностиРаботниковСЗВ_ТД;
		ИначеЕсли СвойстваУзла.Получить("ЕФС_1") <> Неопределено
			И УзелПачкаВходящихДокументов.ЕФС_1.Свойства().Получить("СЗВ") <> Неопределено
			И УзелПачкаВходящихДокументов.ЕФС_1.СЗВ.Свойства().Получить("ЗЛ") <> Неопределено
			И ОбщегоНазначенияЭДКОКлиентСервер.ПервыйОбъект(
				УзелПачкаВходящихДокументов.ЕФС_1.СЗВ.ЗЛ).Свойства().Получить("СЗВ_СТАЖ") <> Неопределено
			И ОбщегоНазначенияЭДКОКлиентСервер.ПервыйОбъект(ОбщегоНазначенияЭДКОКлиентСервер.ПервыйОбъект(
				УзелПачкаВходящихДокументов.ЕФС_1.СЗВ.ЗЛ).СЗВ_СТАЖ).Свойства().Получить("КоррОтмПериод") <> Неопределено Тогда
			Возврат Справочники.ВидыОтправляемыхДокументов.ДанныеОКорректировкеСведенийЗастрахованныхЛицСЗВ_КОРР;
		ИначеЕсли СвойстваУзла.Получить("ЕФС_1") <> Неопределено
			И УзелПачкаВходящихДокументов.ЕФС_1.Свойства().Получить("СЗВ") <> Неопределено
			И УзелПачкаВходящихДокументов.ЕФС_1.СЗВ.Свойства().Получить("ЗЛ") <> Неопределено
			И ОбщегоНазначенияЭДКОКлиентСервер.ПервыйОбъект(
				УзелПачкаВходящихДокументов.ЕФС_1.СЗВ.ЗЛ).Свойства().Получить("СЗВ_СТАЖ") <> Неопределено Тогда
			Возврат Справочники.ВидыОтправляемыхДокументов.СведенияОСтраховомСтажеЗастрахованныхЛицСЗВ_СТАЖ;
		Иначе
...
Функция ОпределитьОрганизациюПФР(КороткоеИмяФайла, ДеревоXML)
	...
	Если ЗначениеЗаполнено(РегистрационныйНомерПФР) Тогда
		...
		Если Метаданные.Справочники.Организации.Реквизиты.Найти("РегистрационныйНомерСФР") <> Неопределено Тогда
			Запрос = Новый Запрос(
				"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
				|	Организации.Ссылка
				|ИЗ
				|	Справочник.Организации КАК Организации
				|ГДЕ
				|	Организации.РегистрационныйНомерСФР = &РегистрационныйНомерСФР");
			Запрос.УстановитьПараметр("РегистрационныйНомерСФР", РегистрационныйНомерПФР);
			Выборка = Запрос.Выполнить().Выбрать();
			Если Выборка.Следующий()
				И КонтекстЭДОСервер().СформироватьПараметрыОтправкиВПФР(Выборка.Ссылка).ИспользоватьРегНомерСФР Тогда
...
Функция ЭтоФайлВыгрузкиПФР(ОсновнойФайл)
	...
	ИЛИ ВРЕГ(Лев(КороткоеИмяФайла, СтрДлина(ПрефиксИмениФайлаЕЦП))) = ПрефиксИмениФайлаЕЦП)
...
Функция ПолучитьСоответствиеПериодОтчПериодичности()
	...
	СоответствиеПериодОтчПериодичности.Вставить("0A", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("0B", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("0C", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("0D", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("1A", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("1B", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("1C", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("1D", Перечисления.Периодичность.Месяц);
	...
	КонецЕсли;
	СоответствиеПериодОтчПериодичности.Вставить("2A", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("2B", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("2C", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("2D", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("3A", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("3B", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("3C", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("3D", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("4A", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("4B", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("4C", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("4D", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("5A", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("5B", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("5C", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("5D", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("6A", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("6B", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("6C", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("6D", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("7A", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("7B", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("7C", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("7D", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("8A", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("8B", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("8C", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("8D", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("9A", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("9B", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("9C", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("9D", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("AC", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("AD", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("AE", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("BC", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("BD", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("BE", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("BF", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("CC", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("CD", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("CE", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("DC", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("DD", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("DE", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("DF", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("ED", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("EE", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("EF", Перечисления.Периодичность.Месяц);
	...
	СоответствиеПериодОтчПериодичности.Вставить("FD", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("FE", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("FF", Перечисления.Периодичность.Месяц);
...
Функция ПолучитьСоответствиеПериодОтчМесяцуОкончания()
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("0A", 10);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("0B", 10);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("0C", 10);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("0D", 10);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("1A", 1);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("1B", 1);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("1C", 1);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("1D", 1);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("2A", 2);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("2B", 2);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("2C", 2);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("2D", 2);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("3A", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("3B", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("3C", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("3D", 3);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("4A", 4);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("4B", 4);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("4C", 4);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("4D", 4);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("5A", 5);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("5B", 5);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("5C", 5);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("5D", 5);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("6A", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("6B", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("6C", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("6D", 6);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("7A", 7);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("7B", 7);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("7C", 7);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("7D", 7);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("8A", 8);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("8B", 8);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("8C", 8);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("8D", 8);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("9A", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("9B", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("9C", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("9D", 9);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("AC", 1);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("AD", 2);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("AE", 3);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("BC", 4);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("BD", 5);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("BE", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("BF", 12);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("CC", 7);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("CD", 8);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("CE", 9);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("DC", 10);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("DD", 11);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("DE", 12);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("DF", 11);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("ED", 11);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("EE", 12);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("EF", 11);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("FD", 12);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("FE", 11);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("FF", 12);
				...
				Если ОпределитьТипФайлаНДС(ВыбранныйФайл.Имя) = Неопределено
					...
					ТекстСообщения = СтрШаблон(
						НСтр("ru = 'Файл ""%1"" не может быть отправлен в составе реестра НДС.';
							|en = 'Файл ""%1"" не может быть отправлен в составе реестра НДС.'"),
						ВыбранныйФайл.Имя);
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
				...
				Если НЕ КонтекстЭДОСервер().ЭтоФайлВыгрузкиРеестраАкцизов(ВыбранныйФайл.Имя) Тогда
					...
					ТекстСообщения = СтрШаблон(
						НСтр("ru = 'Файл ""%1"" не может быть отправлен в составе реестра по акцизам.';
							|en = 'Файл ""%1"" не может быть отправлен в составе реестра по акцизам.'"),
						ВыбранныйФайл.Имя);
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
...
Процедура УправлениеФормой(Форма)
		...
		Пока ИндексФайла >= 1 Цикл
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Файл ""%1"" не может быть отправлен в составе комплекта.';
					|en = 'Файл ""%1"" не может быть отправлен в составе комплекта.'"),
				СвойстваФайла.ИмяФайла);
			#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			#Иначе
	...
	Элементы.Получатель.Видимость =
		...
		И (Объект.ТипПолучателя <> ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.ПФР")
			ИЛИ Форма.ИспользоватьОрганЕЦП = Ложь ИЛИ НЕ ЗначениеЗаполнено(Форма.РегНомерСФР)
