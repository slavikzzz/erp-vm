...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Партнер) Тогда
		...
		Если Выборка.Следующий() Тогда
			Если Не Выборка.Клиент Тогда
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Партнер ""%1"" не является клиентом. Регистрация карт лояльности возможна только для партнеров, являющихся клиентами.';
												|en = 'Partner ""%1"" is not a customer. Loyalty cards can be registered only for partners who are customers.'"), Партнер);
			КонецЕсли;
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Партнер) И ЗначениеЗаполнено(ЗаменяемаяКарта) Тогда
		...
		Если ДанныеКарты.Статус = Перечисления.СтатусыКартЛояльности.Аннулирована Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Карта лояльности %1 уже аннулирована.';
											|en = 'Loyalty card %1 is already canceled.'"), ЗаменяемаяКарта);
		КонецЕсли;
	...
	Иначе
		...
		Заголовок = НСтр("ru = 'Помощник регистрации карты лояльности';
						|en = 'Loyalty card registration wizard'");
	КонецЕсли;
...
Процедура Готово(Команда)
	...
	Если НоваяКартаПерсонизирована И ЗначениеЗаполнено(Партнер) Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Зарегистрирована карта лояльности %1 для клиента %2.';
										|en = 'Loyalty card %1 is registered for customer %2.'"), НоваяКарта, Партнер);
	Иначе
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Зарегистрирована карта лояльности %1.';
										|en = 'Loyalty card %1 is registered.'"), НоваяКарта);
	КонецЕсли;
	...
	ИначеЕсли Страница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаУточнениеВторогоКодаКартыЛояльности Тогда
		Если ТипПервогоКодаНовойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.МагнитныйКод") Тогда
			...
			Текст = НСтр("ru = 'Карта лояльности ""%1"" (Магнитный код: %2) является картой смешанного типа (имеет магнитный и штриховой коды).
			                   ...
			                   |en = 'Loyalty card ""%1"" (Magnetic code: %2) is a mixed kind of card (it has a magnetic code and a barcode).
			                   |Read it with a barcode scanner or specify the card barcode manually.'");
		Иначе
			...
			Текст = НСтр("ru = 'Карта лояльности ""%1"" (Штрихкод: %2) является картой смешанного типа (имеет магнитный и штриховой коды).
			                   ...
			                   |en = 'Loyalty card ""%1"" (Barcode: %2) is a mixed kind of card (it has a magnetic code and a barcode).
			                   |Read it with a barcode scanner or specify the magnetic code manually.'");
		КонецЕсли;
	...
	ИначеЕсли Страница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаВводИнформацииОВладельцеКарты Тогда
		Если ТипНовойКарты = ПредопределенноеЗначение("Перечисление.ТипыКарт.Смешанная") Тогда
			Текст = НСтр("ru = 'Карта лояльности ""%1"" (%2: %3, %4: %5) является персонализированной.
			                   ...
			                   |en = 'Loyalty card ""%1"" (%2: %3, %4: %5) is personalized.
			                   |To finish the registration, specify the information about the card holder.'");
		...
		Иначе
			Текст = НСтр("ru = 'Карта лояльности ""%1"" (%2: %3) является персонализированной.
			                   |Для завершения регистрации уточните информацию о владельце карты.';
			                   |en = 'Loyalty card ""%1"" (%2: %3) is personalized.
			                   |To finish the registration, specify the information about the card holder.'");
			НадписьВводИнформацииОВладельцеКарты = СтрШаблон(Текст, ВидНовойКарты, ТипПервогоКодаНовойКарты, ПервыйКодНовойКарты);
		КонецЕсли;
		Если ЗначениеЗаполнено(ЗаменяемаяКарта) Тогда
			НадписьВводИнформацииОВладельцеКарты = СтрШаблон(НСтр("ru = 'Заменяемая карта лояльности ""%1"" будет аннулирована.';
																	|en = 'Loyalty card ""%1"" being replaced will be canceled.'"), ЗаменяемаяКарта)
...
Функция ОбработатьПервыйКодНовойКартыНаСервере(Данные, ТипКода, Предобработка)
	...
	Для Каждого СтрокаТЧ Из КартыЛояльности.ЗарегистрированныеКартыЛояльности Цикл
		...
		НоваяСтрока.Наименование = Строка(СтрокаТЧ.Ссылка) + ?(ЗначениеЗаполнено(СтрокаТЧ.Партнер) И ЗначениеЗаполнено(СтрокаТЧ.Ссылка), " " + СтрШаблон(НСтр("ru = 'Партнер: %1';
																																								|en = 'Partner: %1'"), Строка(СтрокаТЧ.Партнер)), "");
	...
	Если НайденныеКартыЛояльностиСчитываниеНовойКартыЛояльности.Количество() = 0 Тогда
		...
		Если ТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Штрихкод ""%1"" не соответствует виду карт лояльности ""%2""';
												|en = 'Barcode ""%1"" does not match loyalty card kind ""%2""'"), ПервыйКодНовойКарты, ТребуемыйВидНовойКарты);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Штрихкод ""%1"" не соответствует ни одному из видов карт лояльности.';
												|en = 'Barcode ""%1"" matches no loyalty card kinds.'"), ПервыйКодНовойКарты);
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Магнитный код ""%1"" не соответствует виду карт лояльности ""%2""';
												|en = 'Magnetic code ""%1"" does not correspond to loyalty card kind ""%2""'"), ПервыйКодНовойКарты, ТребуемыйВидНовойКарты);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Магнитный код ""%1"" не соответствует ни одному из видов карт лояльности.';
												|en = 'Magnetic code ""%1"" does not correspond to any loyalty card kinds.'"), ПервыйКодНовойКарты);
			КонецЕсли;
		КонецЕсли;
...
Функция ОбработатьВторойКодНовойКартыНаСервере(Данные, Предобработка)
	...
	Если Не Результат Тогда
		...
		Если ТипВторогоКодаНовойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Штрихкод ""%1"" не соответствует виду карт лояльности ""%2""';
												|en = 'Barcode ""%1"" does not match loyalty card kind ""%2""'"), ВторойКодНовойКарты, ТребуемыйВидНовойКарты);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Штрихкод ""%1"" не соответствует ни одному из видов карт лояльности.';
												|en = 'Barcode ""%1"" matches no loyalty card kinds.'"), ВторойКодНовойКарты);
			КонецЕсли;
			Поле = "ШтрихкодНовойКарты";
		Иначе
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Магнитный код ""%1"" не соответствует виду карт лояльности ""%2""';
												|en = 'Magnetic code ""%1"" does not correspond to loyalty card kind ""%2""'"), ВторойКодНовойКарты, ТребуемыйВидНовойКарты);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Магнитный код ""%1"" не соответствует ни одному из видов карт лояльности.';
												|en = 'Magnetic code ""%1"" does not correspond to any loyalty card kinds.'"), ВторойКодНовойКарты);
			КонецЕсли;
			Поле = "МагнитныйКодНовойКарты";
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Ссылка) И ТекущиеДанные.Ссылка <> НоваяКарта Тогда // Карта уже зарегистрирована.
		...
		Если ТипПервогоКодаНовойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			ТекстСообщения = НСтр("ru = 'Карта ""%1"" со штрихкодом %2 уже зарегистрирована.';
									|en = 'Card ""%1"" with barcode %2 is already registered.'");
		Иначе
			ТекстСообщения = НСтр("ru = 'Карта ""%1"" с магнитным кодом %2 уже зарегистрирована.';
									|en = 'Card ""%1"" with magnetic code %2 is already registered.'");
		КонецЕсли;
...
Функция ОбработатьПервыйКодЗаменяемойКартыНаСервере(Данные, ТипКода, Предобработка)
	...
	Если НайденныеКартыЛояльностиСчитываниеЗаменяемойКартыЛояльности.Количество() > 0 Тогда
		...
		Если ТипКодаЗаменяемойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Штрихкод ""%1"" не соответствует виду карт лояльности ""%2""';
												|en = 'Barcode ""%1"" does not match loyalty card kind ""%2""'"), КодЗаменяемойКарты, ТребуемыйВидНовойКарты);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Штрихкод ""%1"" не соответствует ни одному из видов карт лояльности.';
												|en = 'Barcode ""%1"" matches no loyalty card kinds.'"), КодЗаменяемойКарты);
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Магнитный код ""%1"" не соответствует виду карт лояльности ""%2""';
												|en = 'Magnetic code ""%1"" does not correspond to loyalty card kind ""%2""'"), КодЗаменяемойКарты, ТребуемыйВидНовойКарты);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Магнитный код ""%1"" не соответствует ни одному из видов карт лояльности.';
												|en = 'Magnetic code ""%1"" does not correspond to any loyalty card kinds.'"), КодЗаменяемойКарты);
			КонецЕсли;
		КонецЕсли;
	...
	Иначе // Карта не зарегистрирована.
		...
		Если ТипКодаЗаменяемойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			ТекстСообщения = НСтр("ru = 'Карта ""%1"" со штрихкодом %2 еще не зарегистрирована и не может быть заменена.';
									|en = 'Card ""%1"" with barcode %2 is not registered yet and cannot be replaced.'");
		Иначе
			ТекстСообщения = НСтр("ru = 'Карта ""%1"" с магнитным кодом %2 еще не зарегистрирована и не может быть заменена.';
									|en = 'Card ""%1"" with magnetic code %2 is not registered yet and cannot be replaced.'");
		КонецЕсли;
