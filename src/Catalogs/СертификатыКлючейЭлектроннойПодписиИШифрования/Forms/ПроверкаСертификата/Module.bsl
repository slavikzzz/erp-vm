...
Процедура ВыгрузитьТехническуюИнформацию(ВыгрузитьАрхив)
	...
	ТемаСообщения = "";
	ТекстОшибок = ТекстОшибок + НСтр("ru = 'Результат проверки на клиенте';
									|en = 'Check result on client'") + ":" + Символы.ПС;
	...
	ТекстОшибок = ТекстОшибок + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Результат проверки на сервере ""%1"":';
			|en = 'Check result on server ""%1"":'"), ИмяСервера) + Символы.ПС;
	...
	Иначе
		ЭлектроннаяПодписьСлужебныйКлиент.СформироватьТехническуюИнформацию(
			ТекстОшибок, Новый Структура("Тема, Сообщение",
				?(ПустаяСтрока(ТемаСообщения), НСтр("ru = 'Проблема при проверке сертификата электронной подписи';
													|en = 'Issue occurred checking the signature certificate'"), ТемаСообщения),
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификатаОблачнойПодписи(РезультатВызова, Контекст) Экспорт
	...
	ИначеЕсли Не ЗначениеЗаполнено(РезультатВызова.ДанныеСертификата) Тогда
		ОписаниеОшибки = НСтр("ru = 'Сертификат отсутствует в сервисе (возможно удален).';
								|en = 'The certificate does not exist in the service. Probably, it has been deleted.'");
	КонецЕсли;
...
Функция СформироватьЗаголовокПодробно(ОписаниеОшибки)
	...
	Строка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '<a href = %1>Поиск решения...</a>';
			|en = '<a href = %1>Finding solution…</a>'"), НавигационнаяСсылка);
...
Функция СтандартныйЗаголовокЭлемента(ИмяЭлемента, ТипПодписи)
	...
	ИначеЕсли ИмяЭлемента = "НаличиеСертификатаНадпись" Тогда
		Возврат НСтр("ru = 'Наличие сертификата в личном списке';
					|en = 'Existence of certificate in the private list'");
	...
	ИначеЕсли ИмяЭлемента = "ПодписаниеНадпись" Тогда
		...
		Если ЗначениеЗаполнено(ТипПодписи) Тогда
			...
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Подписание данных типом подписи - %1';
					|en = 'Signing data with signature type: %1'"), ТипПодписиСтрока);
	...
	Для Каждого Проверка Из Проверки Цикл
		...
		Если СоставПроверок = Неопределено Тогда
			ТекстОшибок = ТекстОшибок + Проверка + ": "
				+ НСтр("ru = 'Не выполнялась, так как не требуется (не настроено)';
						|en = 'Not performed as it is not necessary (not configured)'") + Символы.ПС;
		Иначе
