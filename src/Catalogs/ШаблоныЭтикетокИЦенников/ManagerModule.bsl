...
//  Структура - Параметры печати
//
Функция ПолучитьПараметрыДляПечатиОбразцаЭтикеткиОбъектаЭксплуатации(ДляЧего, ТипКода, УникальныйИдентификатор) Экспорт
	
	ДанныеПечати = Новый ТаблицаЗначений;
	ДанныеПечати.Колонки.Добавить("ОбъектЭксплуатации", Новый ОписаниеТипов("СправочникСсылка.ОбъектыЭксплуатации"));
	ДанныеПечати.Колонки.Добавить("Штрихкод",           Новый ОписаниеТипов("Строка"));
	
	НоваяСтрока = ДанныеПечати.Добавить();
	НоваяСтрока.ОбъектЭксплуатации = ДляЧего;
	НоваяСтрока.Штрихкод = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДляЧего, "ИнвентарныйНомер");
...
// Получить параметры для печати образца этикетки узла объекта эксплуатации
//
// Параметры:
//  ДляЧего - СправочникСсылка.УзлыОбъектовЭксплуатации - Назначение
//  ТипКода	- Число - Тип кода
//  УникальныйИдентификатор	- УникальныйИдентификатор - Уникальный идентификатор.
// 
// Возвращаемое значение:
//  Структура - Параметры печати
//
Функция ПолучитьПараметрыДляПечатиОбразцаЭтикеткиУзлаОбъектаЭксплуатации(ДляЧего, ТипКода, УникальныйИдентификатор) Экспорт
	
	ДанныеПечати = Новый ТаблицаЗначений;
	ДанныеПечати.Колонки.Добавить("Узел", Новый ОписаниеТипов("СправочникСсылка.УзлыОбъектовЭксплуатации"));
	ДанныеПечати.Колонки.Добавить("Штрихкод", Новый ОписаниеТипов("Строка"));
	
	НоваяСтрока = ДанныеПечати.Добавить();
	НоваяСтрока.Узел = ДляЧего;
	НоваяСтрока.Штрихкод = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДляЧего, "Штрихкод");
