...
Процедура ОСВыбрать(Команда)
	...
	"Справочник.ЕдиницыГенерирующиеДенежныеСредства.Форма.ФормаИсключенияИзЕГДС.ДобавитьВВыбранное");
...
Процедура ДобавитьВыбранныеОС(ОСВыделенныеСтроки)
	...
	Для Каждого СтрокаОС Из ОСВыделенныеСтроки Цикл
		...
		ФильтрПоискаОС.Вставить("ОсновноеСредство", ТекДанныеОС.ОсновноеСредство);

		Если КорзинаОС.НайтиСтроки(ФильтрПоискаОС).Количество() = 0 Тогда
			СтрокаВыбранныхОС = КорзинаОС.Добавить();
			СтрокаВыбранныхОС.ОсновноеСредство = ТекДанныеОС.ОсновноеСредство;
		КонецЕсли;
...
Процедура ДобавитьВыбранныеНМА(НМАВыделенныеСтроки)
	...
	Для Каждого СтрокаНМА Из НМАВыделенныеСтроки Цикл
		...
		ФильтрПоискаНМА.Вставить("НематериальныйАктив", ТекДанныеНМА.НематериальныйАктив);

		Если КорзинаНМА.НайтиСтроки(ФильтрПоискаНМА).Количество() = 0 Тогда
			СтрокаВыбранныхНМА = КорзинаНМА.Добавить();
			СтрокаВыбранныхНМА.НематериальныйАктив = ТекДанныеНМА.НематериальныйАктив;
		КонецЕсли;
...
&НаКлиенте
Процедура ОСИсключитьИзЕГДС(Команда)
...
&НаКлиенте
Процедура НМАИсключитьИзЕГДС(Команда)
...
Процедура ВопросОПривязкеКЕГДС(ВыбранныеАктивы)
	...
	Если ВыбранныеАктивы.Количество() = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Объекты для исключения из ЕГДС не выбраны.';
													|en = 'Objects to be excluded from the cash-generating unit are not selected.'"));
		Возврат;
	КонецЕсли;



	ТекстВопроса = НСтр("ru = 'Исключить выбранные объекты из %1?';
						|en = 'Exclude selected objects from %1?'");
...
Процедура ПродолжитьПривязкуВНА(РезультатВопроса, ПараметрыПривязки) Экспорт
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени(
	"Справочник.ЕдиницыГенерирующиеДенежныеСредства.Форма.ФормаИсключенияИзЕГДС.СозданиеДокументов");
...
Процедура ИзменитьПривязку(ПараметрыПривязки)
	...
	ЗначенияЗаполнения.Вставить("ЕГДС", ПредопределенноеЗначение("Справочник.ЕдиницыГенерирующиеДенежныеСредства.ПустаяСсылка"));
...
Процедура УстановитьПараметрыДинамическихСписков()
	...
	ОС.Параметры.УстановитьЗначениеПараметра("ЕГДС", ЕГДС);

	НМА.Параметры.УстановитьЗначениеПараметра("Организация", Организация);
	НМА.Параметры.УстановитьЗначениеПараметра("ЕГДС", ЕГДС);
