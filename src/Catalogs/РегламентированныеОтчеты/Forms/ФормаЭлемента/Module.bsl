...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПустаяСтрока(Объект.ИсточникОтчета) Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Укажите наименование объекта.';
									|en = 'Укажите наименование объекта.'"));
	...
	Если Объект.ВнешнийОтчетИспользовать И НЕ ВнешнийОтчетЗарегистрирован Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Внешний отчет отсутствует. Загрузите внешний отчет или установите признак ""Использовать"" в положение ""Объект"".';
									|en = 'Внешний отчет отсутствует. Загрузите внешний отчет или установите признак ""Использовать"" в положение ""Объект"".'"));
		Модифицированность = Истина;
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ФайлВнешнегоОтчета) Тогда
		...
		Если ТипОтчета = 0 Тогда
			...
			Если НЕ НеВыводитьСообщения Тогда
				...
				НСтр("ru = 'Не найден отчет ""%1"".';
					|en = 'Не найден отчет ""%1"".'"), ФайлВнешнегоОтчета);
	...
	Иначе
		...
		Если НЕ НеВыводитьСообщения Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не выбран файл внешнего отчета.';
									|en = 'Не выбран файл внешнего отчета.'");
...
&НаКлиенте
Асинх Процедура ВыбратьВнешнийОтчетПродолжение()
	...
	ФайлОтчета = Ждать Диалог.ВыбратьАсинх();
	Если ТипЗнч(ФайлОтчета) = Тип("Массив") Тогда 
		ПомещенныйФайл = Ждать ПоместитьФайлНаСерверАсинх(,,, ФайлОтчета[0], УникальныйИдентификатор);
		Если ТипЗнч(ПомещенныйФайл) = Тип("ОписаниеПомещенногоФайла") И Не ПомещенныйФайл.ПомещениеФайлаОтменено Тогда 
			АдресФайлаВоВременномХранилище = ПомещенныйФайл.Адрес;
			ПолноеИмяЗагруженногоВнешнегоОтчета = ФайлОтчета[0];
...
&НаКлиенте
Асинх Процедура ДекорацияВыгрузитьНажатие(Элемент)
	...
	Если ЗначениеЗаполнено(КаталогВыгрузки) Тогда
		Ждать ПолучитьФайлССервераАсинх(АдресДанныхВыгружаемогоФайла, КаталогВыгрузки + ПолноеИмяВыгружаемогоФайла);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПолеИсточникОтчетаФайлОчистка(Элемент, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ru = 'Удалить хранящийся в информационной базе отчет?';
						|en = 'Удалить хранящийся в информационной базе отчет?'");
