...
Процедура ПроверкаИспользования(ОбъектыРасширений = Ложь) Экспорт
	...
	Если СтандартныеПодсистемыПовтИсп.ОтключитьИдентификаторыОбъектовМетаданных() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Справочник ""%1"" не используется.';
				|en = 'The ""%1"" catalog is not used.'"), НазваниеСправочника(ОбъектыРасширений));
	   ...
	   И ЗначениеЗаполнено(ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени("Константа.ГлавныйУзел").Получить()) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Справочник ""%1"" не может использоваться
					   ...
					   |en = 'Cannot use the ""%1"" catalog in an infobase whose
					   |master node disconnection was not confirmed.
					   |
					   |To reconnect with the master node, run 1C:Enterprise and click ""Restore"".
					   |Alternatively, set the master node programmatically in the ""Master node"" constant.
					   |
					   |To confirm the disconnection,
					   |run 1C:Enterprise and click ""Disconnect"".
					   |Alternatively, programmatically clear the value of the ""Master node"" constant.'"),
...
Функция КлючРоли(ОбъектМетаданныхРоль, ВызыватьИсключение = Истина)
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось получить ключ роли %1';
			|en = 'Cannot receive a key of role %1'"), ОбъектМетаданныхРоль.Имя);
			...
			Если НомерОшибки = 3 И КоличествоОшибок > 5 Тогда
				...
				ОписаниеОшибки = "... " + СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
					НСтр("ru = ';И еще %1 ошибка;;И еще %1 ошибки;И еще %1 ошибок;И еще %1 ошибки';
						|en = ';and %1 more errors;;and %1 more errors;and %1 more errors;and %1 more errors'"),
					...
					ИначеЕсли ОтключенныеРасширения.Количество() > 0 И Не ОтключенныеРасширения[0].Активно
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Расширение ""%1"" установлено после запуска сеанса, поэтому не подключено.
							           |Перезапустите сеанс.';
							           |en = 'The ""%1"" extension is installed, but the application requires a restart.
				...
				ИначеЕсли РезультатПроверки.ОбъектМетаданныхУдаленного <> Неопределено
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Идентификатор объекта метаданных ""%1"" помечен на удаление.
						           ...
						           |Связанные с ним данные и настройки более недоступны.';
						           |en = 'The ID of the ""%1"" metadata object is marked for deletion.
