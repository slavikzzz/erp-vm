...
Процедура ВывестиИнформациюОУспешномРезультатеОбмена()
	...
	Строки.Добавить(
		Новый ФорматированнаяСтрока(
			СтрШаблон(НСтр("ru = 'На запрос о %1 ПАТ %2 получен ответ.';
							|en = 'На запрос о %1 ПАТ %2 получен ответ.'"), СтрокаТипОперации("Получение"), Наименование)));

	Строки.Добавить(" ");
	Строки.Добавить(
		Новый ФорматированнаяСтрока(
			СтрШаблон(НСтр("ru = '%1 ПАТ';
							|en = '%1 ПАТ'"), СтрокаТипОперации("Завершено"))));
	Строки.Добавить(" ");
		...
		Иначе
			...
			ИначеЕсли Не Статус = Перечисления.СтатусыОбъектовСАТУРН.Черновик Тогда
				...
				ШаблонИнформации = НСтр("ru = 'Редактирование ПАТ возможно только в статусе Черновик.
					...
					|en = 'Редактирование ПАТ возможно только в статусе Черновик.
					|ПАТ переведен в статус %1, редактирование невозможно.'");
