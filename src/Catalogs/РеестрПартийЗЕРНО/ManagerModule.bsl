...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Данные.ТребуетсяЗагрузка Тогда
		Представление = СтрШаблон(НСтр("ru = '(не загружено) %1';
										|en = '(не загружено) %1'"), Представление);
	КонецЕсли;
...
Функция СообщениеЗагрузкиПартииПоНомеру(НомерПартии, ДополнительныеПараметры, ПараметрыОбмена) Экспорт
	...
	СообщениеXML.ДополнительноеОписание    = СтрШаблон(НСтр("ru = '№ партии: %1';
															|en = '№ партии: %1'"), НомерПартии);
	...
	Иначе
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Внутренняя ошибка. Не определен вид продукции при загрузке партии %1';
				|en = 'Внутренняя ошибка. Не определен вид продукции при загрузке партии %1'"),
			НомерПартии);
...
Функция СообщенияЗагрузкиПартий(ПараметрыФормирования, ПараметрыОбмена = Неопределено)
	...
	Если ЗначениеЗаполнено(ПараметрыФормирования.ИдентификаторПартии) Тогда
		...
		ДанныеОписания.Добавить(СтрШаблон(НСтр("ru = 'id: %1';
												|en = 'id: %1'"), ПараметрыФормирования.ИдентификаторПартии));
	КонецЕсли;
	Если ЗначениеЗаполнено(ПараметрыФормирования.СтатусПартииЗерна) Тогда
		...
		ДанныеОписания.Добавить(СтрШаблон(НСтр("ru = '(статус: %1)';
												|en = '(статус: %1)'"), ПараметрыФормирования.СтатусПартииЗерна));
	КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(СправочникСсылка) Тогда
			...
			Исключение
				...
				ОбщегоНазначенияИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
					ТекстОшибкиПодробно,
					НСтр("ru = 'Работа с реестром партий';
						|en = 'Работа с реестром партий'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Ошибка записи соответствия партий:
					       |%1';
					       |en = 'Ошибка записи соответствия партий:
					       |%1'"),
