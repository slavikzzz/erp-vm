...
Процедура ЗаписатьНаКлиенте(ЗакрытьПослеЗаписи, ОповещениеЗавершения = Неопределено) Экспорт 
	...
	ТекстКнопкиДа = НСтр("ru = 'Изменились сведения о виде дохода';
						|en = 'Income kind information records were changed'");
	ТекстВопроса = СтрШаблон(
		НСтр("ru = 'Изменены сведения о соответствии вида дохода оплате труда.
		           |Если прежние сведения были ошибочными, нажмите ""%1"".';
		           |en = 'Details of correspondence between income kind and payroll were changed.
		           |Click ""%1"" if the previous details were incorrect.'"), 
		ТекстКнопкиДа);
