	...
	Для каждого ТекущаяСтрока Из ТаблицаПроверки Цикл
		Если ТекущаяСтрока.КоличествоСтрок > 1 Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Категория [%КатегорияНовостей%] повторяется несколько раз.
				...
				|en = 'Category [%КатегорияНовостей%] is repeated several times.
				|Each category can be entered only once.'");
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЗагруженоССервера = Истина Тогда
		...
		Если (ПометкаУдаления = Истина)
				И (НЕ Ссылка.Пустая())
				И (ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "ПометкаУдаления") = Ложь) Тогда // = Неопределено (для нового объекта, который еще не записан)
			ДополнительныеСвойстваЭталон = Новый Структура;
				ДополнительныеСвойстваЭталон.Вставить("УдалениеЛентыНовостейЗагруженнойССервера", Ложь);
			ЗаполнитьЗначенияСвойств(ДополнительныеСвойстваЭталон, ДополнительныеСвойства);
			Если ДополнительныеСвойстваЭталон.УдалениеЛентыНовостейЗагруженнойССервера = Истина Тогда
			...
			Иначе
				ТекстСообщения = НСтр("ru = 'Нельзя помечать на удаление ленты новостей, загруженные автоматически с сервера новостей.';
										|en = 'Cannot mark news feeds for deletion that were downloaded from the news server automatically.'");
				Сообщение = Новый СообщениеПользователю();
	...
	Если (ЗагруженоССервера = Истина)
		...
		Если (ЧастотаОбновления = 0) Тогда
			ТекстСообщения = НСтр("ru = 'Для обязательной ленты новостей нельзя устанавливать частоту обновления [Обновлять только вручную].';
									|en = 'Update frequency cannot be set for mandatory news feed [Обновлять только вручную].'");
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
			Сообщение.УстановитьДанные(Ссылка);
			Сообщение.Сообщить();
		...
		Если (ВидимостьПоУмолчанию = Ложь) И (ИсключенияВидимости.Количество() = 0) Тогда
			ТекстСообщения = НСтр("ru = 'Для обязательной ленты новостей нельзя устанавливать видимость по умолчанию в [Скрыта от всех] и не указывать список пользователей, которые будут видеть эту ленту новостей.';
									|en = 'For a mandatory news feed, you cannot set default visibility to [Скрыта от всех] and leave its users list blank.'");
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
			Сообщение.УстановитьДанные(Ссылка);
			Сообщение.Сообщить();
