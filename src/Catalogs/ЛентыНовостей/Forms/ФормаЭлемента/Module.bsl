...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
			...
			Если Запись.Выбран() Тогда
				...
				Если ЭтотОбъект.РазделениеВключено Тогда
					ТекстЗаголовка = НСтр("ru = 'Лента новостей программно скрыта от всех пользователей этой области данных';
											|en = 'News feed is hidden from all users of this data area from 1C:Enterprise script'");
				Иначе
					ТекстЗаголовка = НСтр("ru = 'Лента новостей программно скрыта от всех пользователей';
											|en = 'News feed is hidden from all users from 1C:Enterprise script'");
				КонецЕсли;
						...
						ТекстЗаголовка = ТекстЗаголовка
							...
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'по причине: %1 (разделение включено).';
									|en = 'due to: %1 (splitting is enabled).'"),
								ПричинаОтключения);
					Иначе
						ТекстЗаголовка = ТекстЗаголовка
							+ " "
			...
			Для Каждого ТекущийРедактор Из МассивРедакторовЛентыНовостей Цикл
				...
				НоваяСтрока.ПользовательПредставление = СтрЗаменить(
					...
					НСтр("ru = '(полные права, может редактировать новости во всех лентах новостей)';
						|en = '(full rights, can edit news in all news feeds)'"));
			КонецЦикла;
		...
		Иначе
			Элементы.ГруппаФайлНовостей.Заголовок
				= НСтр("ru = 'Получение файла новостей (эти поля перезаполняются автоматически при изменениях на сервере 1С)';
						|en = 'Receive news file (these fields are refilled automatically if there are changes on 1C server)'");
		КонецЕсли;
	...
	Элементы.ЧастотаОбновления.СписокВыбора.Добавить(3, НСтр("ru = 'Очень часто (каждые 15 минут)';
															|en = 'Very often (every 15 minutes)'"));
...
Процедура ИмяФайлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ВРег(Объект.Протокол) = ВРег("file") Тогда
		...
		Диалог.Заголовок = НСтр("ru = 'Выберите файл новостей в формате Atom или RSS';
								|en = 'Select a news file in Atom or RSS format'");
...
Процедура ДоступныеКатегорииНовостейПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Нельзя добавлять категории для ленты новостей, загруженной с сервера.';
								|en = 'Cannot add categories for news feed downloaded from server.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ДоступныеКатегорииНовостейПередУдалением(Элемент, Отказ)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Нельзя удалять категории для ленты новостей, загруженной с сервера.';
								|en = 'Cannot remove categories for news feed downloaded from server.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура КомандаПодборКатегорий(Команда)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Нельзя изменять категории для ленты новостей, загруженной с сервера.';
								|en = 'Cannot change categories for news feed downloaded from server.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьИнформационныеСтроки()
	...
	Если ЭтотОбъект.РазделениеВключено Тогда
		...
		Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок   = НСтр("ru = 'без исключений. В модели сервиса нельзя настроить видимость ленты новостей по пользователям.';
																	|en = 'without exceptions. You cannot set up news feed visibility by users in SaaS.'");
		...
		Если Объект.ИсключенияВидимости.Количество() = 0 Тогда
			...
			Иначе
				Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок = НСтр("ru = 'без исключений. Никто не сможет выбрать эту ленту новостей для получения новостей.';
																		|en = 'without exceptions. Nobody can select this news feed to receive news.'");
	...
	Если Объект.Ссылка.Пустая() Тогда
		Элементы.ДекорацияСтатистика.Заголовок = НСтр("ru = 'Необходимо вначале записать ленту новостей, чтобы получить по ней статистику.';
														|en = 'Save the news feed to receive its statistics.'");
	Иначе
			...
			Иначе
				ТекстСтатистики = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Всего новостей в этой ленте новостей: %1';
						|en = 'Total number of news in this news feed: %1'"),
				...
				Если КоличествоНовостейПомеченныхНаУдаление = 0 Тогда
					ТекстСтатистики = ТекстСтатистики + " " + НСтр("ru = 'и среди них нет ни одной помеченной на удаление';
																	|en = 'and no marked for deletion among them'")
						+ Символы.ПС;
				ИначеЕсли КоличествоНовостейПомеченныхНаУдаление = КоличествоНовостейВсего Тогда // Все помечены на удаление
					ТекстСтатистики = ТекстСтатистики + ", " + НСтр("ru = 'все они помечены на удаление (а значит не видны в списке новостей)';
																	|en = 'all of them are marked for deletion (and they are not visible in the news list)'")
						+ Символы.ПС;
				Иначе
					ТекстСтатистики = ТекстСтатистики
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = ', из них помечено на удаление %1';
								|en = ', of them marked for deletion %1'")
								+ Символы.ПС,
			...
			Иначе
				...
				Иначе
					ТекстСтатистики = ТекстСтатистики + ", "
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'из них помечено на удаление %1';
								|en = 'among them marked for deletion %1'")
								+ Символы.ПС,
							КоличествоНовостейПомеченныхНаУдаление);
				КонецЕсли;
			КонецЕсли;
			Если ДатаПоследнейУспешнойЗагрузки = '00010101' Тогда
				Если КоличествоНеудачПриЗагрузкеНовостей = 0 Тогда
					ТекстСтатистики = ТекстСтатистики + НСтр("ru = 'Новости по этой ленте новостей еще не загружались';
															|en = 'News by this news feed has not been imported yet'");
				Иначе
					ТекстСтатистики = ТекстСтатистики
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Новости по этой ленте новостей еще ни разу не были успешно загружены (было %1 неудачных попыток загрузки новостей)';
								|en = 'News of this news feed has never been imported successfully (there were %1 unsuccessful attempts to import news)'"),
					...
					ТекстСтатистики = ТекстСтатистики
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени)';
								|en = 'Last time news of this news feed was successfully imported %1 (local time)'"),
					...
					ТекстСтатистики = ТекстСтатистики
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени), после этого было %2 неудачных попыток загрузки новостей.';
								|en = 'Last time news of this news feed was successfully imported %1 (local time), after that there were %2 unsuccessful attempts to import news.'"),
							Формат(МестноеВремя(ДатаПоследнейУспешнойЗагрузки), "ДЛФ=DT"),
							КоличествоНеудачПриЗагрузкеНовостей);
				КонецЕсли;
			КонецЕсли;
