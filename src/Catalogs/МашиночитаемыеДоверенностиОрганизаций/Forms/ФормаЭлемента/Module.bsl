...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНоваяЗапись Тогда
		...
		Объект.ПолномочияОграничены = Не МашиночитаемыеДоверенности.ЭтоПолныеПолномочия(Объект.Полномочия);
		ПолномочияОграничены = Объект.ПолномочияОграничены;
	...
	Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		Правило = РегистрыСведений.ПравилаПроверкиПолномочийПоМЧД.ПравилоПроверки(Параметры.ЗначениеКопирования).Правило;
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не Объект.ДоверительЮЛ_ИностраннаяОрганизация
		...
		ТекстСообщения = НСтр("ru = 'Не заполнена должность ФЛ доверителя.';
								|en = 'The authorizing party of the individual is not filled.'");
	...
	КонецЕсли;
 	...
 	Если НЕ ЭтоФизЛицо Тогда
	 	...
	 	Если ПустаяСтрока(Объект.ЛицоБезДовФЛ_ОснованиеПолномочий) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнено основание полномочий физлица доверителя.';
									|en = 'Grounds for authorities of the authorizing individual are not filled.'");
	 	...
	 	Если ПустаяСтрока(Объект.ДоверительЮЛ_КПП) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнен КПП организации-доверителя.';
									|en = 'Authorizing company KPP is not filled.'");
	 	...
	 	Если ПустаяСтрока(Объект.ДоверительЮЛ_ОГРН) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнен ОГРН организации-доверителя.';
									|en = 'Authorizing company OGRN is not filled.'");
	 	...
	 	Если ПустаяСтрока(Объект.ДоверительЮЛ_Адр) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнен адрес организации-доверителя.';
									|en = 'Authorizing company address is not filled.'");
	 	...
	 	Если ПустаяСтрока(Объект.ЛицоБезДовФЛ_ИНН) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнен ИНН физлица доверителя.';
									|en = 'Authorizing individual TIN is not filled.'");
	 	...
	 	Если НЕ ЗначениеЗаполнено(Объект.ЛицоБезДовФЛ_Гражданство) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнено гражданство физлица доверителя.';
									|en = 'Authorizing individual citizenship is not filled.'");
	 	...
	 	Если НЕ ЗначениеЗаполнено(Объект.ЛицоБезДовФЛ_ДатаРождения) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнена дата рождения физлица доверителя.';
									|en = 'Authorizing individual birth date is not filled.'");
 	...
 	Если НЕ ПустаяСтрока(Объект.ДоверительФЛ_ИНН) Тогда
 		...
 		Если ПустаяСтрока(Объект.ДоверительФЛ_ОГРН) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнен ОГРН доверителя.';
									|en = 'Authorizing party OGRN is not filled.'");
 		...
 		Если ПустаяСтрока(Объект.ДоверительФЛ_СНИЛС) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнен СНИЛС доверителя.';
									|en = 'Authorizing party SNILS is not filled.'");
 		...
 		Если ПустаяСтрока(Объект.ДоверительФЛ_Гражданство) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнено гражданство доверителя.';
									|en = 'Authorizing party citizenship is not filled.'");
 		...
 		Если НЕ ЗначениеЗаполнено(Объект.ДоверительФЛ_ДатаРождения) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнена дата рождения доверителя.';
									|en = 'Authorizing party birth date is not filled.'");
 	...
 	Если НеУказанДоверитель Тогда
		...
		ТекстСообщения = НСтр("ru = 'Не указан доверитель.';
								|en = 'Authorizing party is not specified.'");
	...
	Если Не Объект.Подписана И Объект.Полномочия.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Не указаны полномочия.';
								|en = 'Authorities are not specified.'");
	...
	Если Не Объект.Подписана И Объект.Представители.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Необходимо указать хотя бы одного представителя.';
								|en = 'Specify at least one representative.'");
		...
		Для Каждого СтрокаПолномочий Из Объект.Полномочия Цикл
			Если Не ЗначениеЗаполнено(СтрокаПолномочий.ПредставлениеПолномочия) Тогда 
				...
				ТекстОшибки = НСтр("ru = 'В строке %1 не указаны полномочия.';
									|en = 'Authorities are not specified in line %1.'"); 
	...
	Если Не Объект.Подписана И Объект.Полномочия.Количество() > 0 Тогда
		...
		Для Каждого СтрокаПолномочий Из Объект.Полномочия Цикл 
			...
			Если МашиночитаемыеДоверенности.ЭтоТекстПолныхПолномочий(СтрокаПолномочий.ПредставлениеПолномочия) Тогда
		...
		Если НайденныеСтрокиДублей.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ru = 'Дублируется текст полномочий в строках %1.';
								|en = 'Authority text is duplicated in lines %1.'");
		...
		Если НайденныеСтрокиПолныхПолномочий.Количество() > 1 Тогда
			...
			ТекстОшибки = НСтр("ru = 'Полные полномочия дублируются в строках %1.';
								|en = 'Full authorities are duplicated in lines %1.'");
...
Процедура ВариантЗаполненияПолномочийПриИзменении(Элемент)
	...
	Если Объект.ВозможноПередоверие И Объект.ВариантЗаполненияПолномочий <>
		ПредопределенноеЗначение("Перечисление.ВариантыЗаполненияПолномочийМЧД.Классификатор") Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Подбор полномочий для доверенностей с возможностью передоверия возможен только из классификатора.';
									|en = 'You can select authorities for authorization letters with a sub-authorization option only from the classifier.'");
	...
	Если ЗначениеЗаполнено(ПредыдущийВариантЗаполненияПолномочий)
		...
		ПоказатьВопрос(Оповещение,
			НСтр("ru = 'Табличная часть полномочий будет очищена. Продолжить?';
				|en = 'The authority table will be cleared. Continue?'"),
...
Процедура ПолномочияПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, Параметр)
	...
	Если Объект.ВариантЗаполненияПолномочий = ПредопределенноеЗначение(
		"Перечисление.ВариантыЗаполненияПолномочийМЧД.Классификатор") Тогда
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Не указан вариант заполнения полномочий.';
								|en = 'An authority filling option is not specified.'"); 
...
Процедура ВыгрузитьЗаявлениеНаОтмену(Команда)
	...
	Если ЗначенияРеквизитов.ФайлЗаявленияНаОтзыв = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Заявление на отмену не отправлялось';
										|en = 'Application for cancellation is not sent'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьПолномочия(ТекущийОбъект)
	...
	ТекущийОбъект.ПолномочияОграничены =
		НЕ МашиночитаемыеДоверенности.ЭтоПолныеПолномочия(ТекущийОбъект.Полномочия);
...
Процедура УстановитьЗаголовокФормы(ЭтоНоваяДоверенность = Ложь)
	...
	Если ЭтоНоваяДоверенность Тогда
		Заголовок = НСтр("ru = 'Машиночитаемая доверенность (создание)';
						|en = 'Machine-readable authorization letter (create)'");
	Иначе
...
Функция ПолучитьПредставлениеУдостоверение(СтрокаУдостоверения)
	...
	Если ЗначениеЗаполнено(СтрокаУдостоверения.ВидДок) Тогда
		...
		Если Представление = Неопределено Тогда
			...
			Сообщение.Текст = НСтр("ru = 'У физического лица указан недопустимый для доверенности тип документа, удостоверяющего личность.';
									|en = 'The person has an invalid identity document type for an authorization letter.'");
			Сообщение.Сообщить();
			Возврат "";
	...
	Если ЗначениеЗаполнено(СтрокаУдостоверения.ДатаДок) Тогда
		Представление =
			Представление + " " + НСтр("ru = 'выдан';
										|en = 'issued'") + " " + Формат(СтрокаУдостоверения.ДатаДок, "ДЛФ=ДД");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаУдостоверения.КодВыдДок) Тогда
		Представление = Представление + ", " + НСтр("ru = 'код подразделения';
													|en = 'department code'") + ":" + СтрокаУдостоверения.КодВыдДок;
	КонецЕсли;
...
Процедура ПолучитьСсылкиНаСтороныДоверенности()
	...
	Если РеквизитДоверительФизЛицо = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'По данным реквизитам не удалось найти физическое лицо в базе данных.';
								|en = 'Cannot find a person by the details in the database.'");
...
Функция ЗаполнитьПредставлениеУчастникаМЧД(ОписаниеУчастникаМЧД)
	...
	ШаблонПредставленияФЛ = 
		НСтр("ru = 'Дата рождения:%1	ИНН:%2	СНИЛС:%3';
			|en = 'Date of birth:%1	TIN:%2	SNILS:%3'");
...
Процедура ОбработатьИзменениеПолномочий()
	...
	Объект.ПолномочияОграничены = НЕ ЭтоПолныеПолномочия(Объект.Полномочия);
	ОтобразитьПредупреждениеПолномочий();
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ЭтоПолныеПолномочия(Знач Полномочия)
	Возврат МашиночитаемыеДоверенности.ЭтоПолныеПолномочия(Полномочия);	
КонецФункции
