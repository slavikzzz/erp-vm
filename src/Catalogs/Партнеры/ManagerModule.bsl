...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Команда = Документы.ЗаказДавальца.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	Если Команда <> Неопределено Тогда
		Команда.РежимЗаписи = "";
	КонецЕсли;
	//-- Устарело_Переработка24
	
	Команда = Документы.ЗаказДавальца2_5.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	Если Команда <> Неопределено Тогда
		Команда.РежимЗаписи = "";
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	Элемент.Наименование = НСтр("ru = 'Наше предприятие';
								|en = 'Our enterprise'", КодОсновногоЯзыка);
	Элемент.НаименованиеПолное = НСтр("ru = 'Наше предприятие';
										|en = 'Our enterprise'", КодОсновногоЯзыка);
	...
	Элемент.Наименование = НСтр("ru = 'Неизвестный партнер';
								|en = 'Unknown partner'", КодОсновногоЯзыка);
	Элемент.НаименованиеПолное = НСтр("ru = 'Неизвестный партнер';
										|en = 'Unknown partner'", КодОсновногоЯзыка);
	Элемент.ЮрФизЛицо = Перечисления.КомпанияЧастноеЛицо.Компания;
	Элемент.ДатаРегистрации = Дата("19800101000000");
	...
	Элемент.Наименование = НСтр("ru = 'Розничный покупатель';
								|en = 'Retail customer'", КодОсновногоЯзыка);
	Элемент.НаименованиеПолное = НСтр("ru = 'Розничный покупатель';
										|en = 'Retail customer'", КодОсновногоЯзыка);
...
Функция УстановитьИПЛ(Ссылка, ВидЦен) Экспорт
	...
	Если ЗначениеЗаполнено(СправочникОбъект.ИндивидуальныйВидЦены) Тогда
		...
		ШаблонТекстаСообщения = НСтр("ru = 'Не произведена установка индивидуального виды цен для партнера ""%ОбъектНаименование%"" по причине: %Причина%';
									|en = 'The individual price types have not been set for the partner ""%ОбъектНаименование%"". Reason: %Причина%'");
			...
			Если ОбъектСопоставленияЗаполнен Тогда
				...
				Если ПартнерОбъект = Неопределено Тогда
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Контрагент с наименованием %1 был сопоставлен, но больше не существует.';
							|en = 'The counterparty with the name %1 has been matched but no longer exists.'"), СтрокаТаблицы.РабочееНаименование);
