...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Контрагент.Пустая() Тогда
		...
		Элементы.ПоКонтрагенту.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'По контрагенту ""%1""';
																										|en = 'By counterparty ""%1""'"), Параметры.Контрагент);
	КонецЕсли;
...
&НаКлиенте
Процедура ПереключитьРежимПоиска(Команда)
	
	ИспользоватьРасширенныйПоиск = Не ИспользоватьРасширенныйПоиск;
	Если Не ИспользоватьРасширенныйПоиск Тогда
		СтрокаПоиска = "";
		ВыполнитьПоиск(Неопределено); 
	Иначе
		Если ЕстьТекстПлатформенногоПоиска() Тогда
			ИспользоватьРасширенныйПоиск = Не ИспользоватьРасширенныйПоиск;
			ПоказатьПредупреждение(, НСтр("ru = 'Перед изменением режима поиска необходимо очистить строку поиска';
											|en = 'Before you change the search mode, clear the search string'"));
			ТекущийЭлемент = Элементы.СписокСтрокаПоиска;
			Возврат;
		КонецЕсли;
	КонецЕсли; 
	
	ПриИзмененииРежимаИспользованияПолнотекстовогоПоиска(Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьТекущийЭлементЭлементПоиска(Команда)
...
Процедура ОбновитьИндексПолнотекстовогоПоиска()
	...
	Состояние(НСтр("ru = 'Идет обновление индекса полнотекстового поиска ...';
					|en = 'Updating full-text search index...'"));
	...
	Состояние(НСтр("ru = 'Обновление индекса полнотекстового поиска завершено...';
					|en = 'Full-text search index update completed.'"));
...
&НаСервере
Процедура ПриИзмененииРежимаИспользованияПолнотекстовогоПоиска(СохранятьНастройки = Ложь)
	
	ПартнерыИКонтрагенты.ПриИзмененииРежимаИспользованияПолнотекстовогоПоиска(ЭтотОбъект, СохранятьНастройки);
	
КонецПроцедуры

&НаСервере
Функция ЕстьТекстПлатформенногоПоиска()
	
	Возврат ПартнерыИКонтрагенты.ЕстьТекстПлатформенногоПоиска(Элементы.Список);
