...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИспользуютсяТолькоПартнеры Тогда
		...
		Элементы.УказатьБанковскийСчетКонтрагента.Заголовок                 = НСтр("ru = 'Указать данные банковского счета.';
																					|en = 'Specify bank account details.'");
		...
		Элементы.ДействиеПоРезультатамНайдено.СписокВыбора.Получить(0).Представление = НСтр("ru = 'Не регистрировать нового контрагента, использовать найденного.';
																							|en = 'Do not register a new counterparty, use a found one.'");
		Элементы.ДействиеПоРезультатамНайдено.СписокВыбора.Получить(1).Представление = НСтр("ru = 'Продолжить регистрацию нового контрагента.';
																							|en = 'Continue registering the new counterparty.'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗавершениеРаботы И Не ВыполняетсяЗакрытие Тогда
		...
		Если НеобходимостьВопроса Тогда
			Отказ = Истина;
			ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект), НСтр("ru = 'Введенная информация будет потеряна. Все равно закрыть?';
																									|en = 'The entered data will be lost. Close anyway?'"),РежимДиалогаВопрос.ДаНет);
		КонецЕсли;
...
Процедура БизнесРегионПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(БизнесРегион) Тогда
			...
			Если ЗначениеЗаполнено(ОсновнойМенеджерБизнесРегиона) И ОсновнойМенеджерБизнесРегиона <> ОсновнойМенеджер Тогда
				...
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для выбранного бизнес-региона основным менеджером является %1. Установить его основным менеджером партнера?';
																							|en = 'For selected business region %1 is the main manager. Set him or her as the main manager of the partner?'"),ОсновнойМенеджерБизнесРегиона);
...
Процедура ЗаполнитьИтоговуюИнформацию()
	...
	Если ДействиеПоРезультатамНайдено <> 0 Тогда
		...
		ИтоговаяИнформация = СтрЗаменить(ИтоговаяИнформация,"#TitlePartnerName#",
			?(ИспользуютсяТолькоПартнеры, НСтр("ru = 'Сокращенное юр. наименование';
												|en = 'Abbreviated legal name'"), НСтр("ru = 'Публичное наименование';
																							|en = 'Public name'")));
...
Функция ПолучитьСтрокуТипОтношений()
	...
	СтрокаТипОтношений = ?(Клиент,НСтр("ru = 'Клиент';
										|en = 'Customer'"),"") + ?(Поставщик, НСтр("ru = ', Поставщик';
																				|en = ', Vendor'"),"") + ?(Перевозчик, НСтр("ru = ', Перевозчик';
																																|en = ', Carrier'"),"") + ?(Конкурент, НСтр("ru = ', Конкурент';
																																												|en = ', Competitor'"),"") + ?(ПрочиеОтношения, НСтр("ru = ', Прочие отношения';
																																																									|en = ', Other relations'"),"");
...
Процедура УправлениеЭлементамиБанковскийСчет(Форма, ЭтоГоловнойКонтрагент=Ложь)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы,
		...
		?(Форма["ИностранныйБанк" + Постфикс], НСтр("ru = 'Код';
													|en = 'Code'"), НСтр("ru = 'БИК';
																		|en = 'BIC'")));
			...
			Иначе
				...
				Если ТипИнформации = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
					УзелИнформации.Реквизит = НСтр("ru = 'Информация о контактном лице';
													|en = 'Information about contact person'");
...
Процедура ПерейтиНаСтраницуДополнительныхСведений()
	...
	Если НаОснованииДанныхРозничногоПокупателя Тогда
		ЗаголовокДекорации = НСтр("ru = 'Укажите прочие доступные сведения о розничном покупателе %1:';
									|en = 'Specify other available details on retail customer %1:'");
	ИначеЕсли ИспользуютсяТолькоПартнеры
		...
		ЗаголовокДекорации = НСтр("ru = 'Укажите прочие доступные сведения о контрагенте %1:';
									|en = 'Specify other available details about counterparty %1:'");
	Иначе
		ЗаголовокДекорации = НСтр("ru = 'Укажите прочие доступные сведения о партнере %1:';
									|en = 'Specify other available details about partner %1:'");
	КонецЕсли;
...
Процедура ПерейтиНаФинальнуюСтраницу()
	...
	Если НаОснованииДанныхРозничногоПокупателя Тогда
		ЗаголовокСтраницы = НСтр("ru = 'Проверьте введенные сведения, и нажмите кнопку ""Создать""  для регистрации нового розничного покупателя.';
								|en = 'Verify the information that you entered, and then click Create to register a new retail customer.'");
		ЗаголовокДействия = НСтр("ru = 'Открыть форму вновь созданного розничного покупателя для указания дополнительной информации.';
								|en = 'Open the newly created retail customer form to specify additional information.'");
...
Процедура ПерейтиНаСтраницуДополнительныхРеквизитов()
	...
	ЗаголовокДекорации = СтрШаблон(НСтр("ru = 'Укажите дополнительные реквизиты %1:';
										|en = 'Specify additional attributes %1:'"),
	                               НаименованиеПартнераДляЗаголовка());
...
Функция ДополнительныеСведенияЗаполненыНеправильно()
	...
	Если ДействиеПоРезультатамНайдено <> 0 Тогда
		...
		Если Не Клиент И НЕ Поставщик И НЕ ПрочиеОтношения И НЕ Конкурент Тогда
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо определить хотя бы один тип отношений';
																		|en = 'You must define at least one type of relations'"),ИспользуютсяТолькоПартнеры, "Клиент");
			Отказ = Истина;
		КонецЕсли;
		...
		Если ИспользуютсяГруппыДоступаПартнеров
		 И ГруппаДоступа = ПредопределенноеЗначение("Справочник.ГруппыДоступаПартнеров.ПустаяСсылка") Тогда
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать группу доступа';
																		|en = 'Specify access group'"),ИспользуютсяТолькоПартнеры, "ГруппаДоступа");
			Отказ = Истина;
		КонецЕсли;
		
		Если ПустаяСтрока(Название) Тогда
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать рабочее наименование';
																		|en = 'Specify working title'"),ИспользуютсяТолькоПартнеры, "Название");
			Отказ = Истина;
		КонецЕсли;
	...
	Если ЭтоКомпания = 0 Тогда
		...
		Если УказыватьЮридическиеРеквизиты 
			...
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать юридический адрес';
																		|en = 'Specify legal address'"),ИспользуютсяТолькоПартнеры, "ЮридическийАдрес");
			Отказ = Истина;
		...
		Если ((УказыватьЮридическиеРеквизиты И ФактАдресКонтрагентаОбязателен)
			...
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать фактический адрес';
																		|en = 'Specify business address'"),ИспользуютсяТолькоПартнеры, "ФактическийАдрес");
			Отказ = Истина;
	...
	Иначе
		...
		Если ТребуетсяУказаниеАдресаЧастногоЛица(ЭтотОбъект) Тогда
			...
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать адрес проживания';
																		|en = 'Specify residential address'"), ИспользуютсяТолькоПартнеры, "ЮридическийАдрес");
			Отказ = Истина;
...
Функция СведенияГоловногоКонтрагентаЗаполненыНеправильно()
	...
	Если ВыбратьГоловногоКонтрагента = 1 Тогда
		...
		Если ПустаяСтрока(ПолноеНаименованиеГоловногоКонтрагента) Тогда
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Не указано сокращенное юр. наименование';
																		|en = 'Abbreviated company name is not specified'"), ИспользуютсяТолькоПартнеры, "ПолноеНаименованиеГоловногоКонтрагента");
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
...
Функция РасчетныйСчетЗаполненНеправильно()
	...
	Если УказатьБанковскийСчетКонтрагента Тогда
		...
		Если ПустаяСтрока(НомерСчета) Тогда
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать номер банковского счета';
																		|en = 'You must specify bank account number'"), ИспользуютсяТолькоПартнеры, "НомерСчета");
			Отказ = Истина;
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ВалютаДенежныхСредств) Тогда
			...
			ТекстСообщения = НСтр("ru = 'В системе не указана ""Базовая валюта по умолчанию"". Для настройки необходимо перейти в раздел ""НСИ и администрирование""-""Предприятие""-""Валюты"".';
									|en = '""Default base currency"" is not specified in the system. To set it up, go to """"Master data and settings"" - ""Enterprise"" - ""Currencies"".'");
			...
			ТекстСообщения = НСтр("ru = 'В системе не указана ""Валюта регламентированного учета"". Для настройки необходимо перейти в раздел ""НСИ и администрирование""-""Предприятие""-""Валюты"".';
									|en = '""Local accounting currency"" is not specified in the system. To set it up, go to """"Master data and settings"" - ""Enterprise"" - ""Currencies"".'");
			//-- Локализация
		...
		Если ПустаяСтрока(БИКБанка) И Не ИностранныйБанк Тогда
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать БИК банка';
																		|en = 'Specify BIC'"), ИспользуютсяТолькоПартнеры, "БИКБанка");
			Отказ = Истина;
		КонецЕсли;
		...
		Если ИспользуетсяБанкДляРасчетов Тогда
			...
			Если ПустаяСтрока(БИКБанкаДляРасчетов)
				...
				ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать БИК банка для расчетов';
																			|en = 'Specify BIC of the bank for payments'"), ИспользуютсяТолькоПартнеры, "БИКБанкаДляРасчетов");
				Отказ = Истина;
			КонецЕсли;
...
Функция РасчетныйСчетГоловногоКонтрагентаЗаполненНеправильно()
	...
	Если УказатьБанковскийСчетГоловногоКонтрагента Тогда
		...
		Если ПустаяСтрока(НомерСчетаГоловнойКонтрагент) Тогда
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать номер банковского счета';
																		|en = 'You must specify bank account number'"), ИспользуютсяТолькоПартнеры, "НомерСчетаГоловнойКонтрагент");
			Отказ = Истина;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ВалютаДенежныхСредствГоловнойКонтрагент) Тогда
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать валюту денежных средств';
																		|en = 'You must specify currency'"), ИспользуютсяТолькоПартнеры, "ВалютаДенежныхСредствГоловнойКонтрагент");
			Отказ = Истина;
		КонецЕсли;
		
		Если ПустаяСтрока(БИКБанкаГоловнойКонтрагент) И Не ИностранныйБанкГоловнойКонтрагент Тогда
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать БИК банка';
																		|en = 'Specify BIC'"), ИспользуютсяТолькоПартнеры, "БИКБанкаГоловнойКонтрагент");
			Отказ = Истина;
		КонецЕсли;
		...
		Если ИспользуетсяБанкДляРасчетовГоловнойКонтрагент Тогда
			...
			Если ПустаяСтрока(БИКБанкаДляРасчетовГоловнойКонтрагент)
				...
				ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать БИК банка для расчетов';
																			|en = 'Specify BIC of the settlements bank'"), ИспользуютсяТолькоПартнеры, "БИКБанкаДляРасчетовГоловнойКонтрагент");
				Отказ = Истина;
			КонецЕсли;
...
Процедура ПриСозданииНовогоРозничногоПокупателя()
	...
	Заголовок = НСтр("ru = 'Помощник регистрации нового розничного покупателя';
					|en = 'New retail customer registration wizard'");
	Элементы.ДействиеПоРезультатамНайдено.СписокВыбора[0].Представление = НСтр("ru = 'Не регистрировать нового розничного покупателя, использовать найденного';
																				|en = 'Do not register a new retail customer, use a found one'");
	Элементы.ДействиеПоРезультатамНайдено.СписокВыбора[1].Представление = НСтр("ru = 'Продолжить регистрацию розничного покупателя и подчинить найденному';
																				|en = 'Continue retail customer registration and make it subordinate to the found one'");
	Элементы.ДействиеПоРезультатамНайдено.СписокВыбора[2].Представление = НСтр("ru = 'Продолжить регистрацию нового розничного покупателя';
																				|en = 'Continue registering the new retail customer'");
	...
	Если Не ВидДокумента.Пустая() Тогда
		...
		ПредставлениеДокументаФизЛица = ""
			...
			+ ?(ЗначениеЗаполнено(СерияДокумента),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = ', серия: %1';
																			|en = ', batch: %1'"), СерияДокумента), "")
			...
			+ ?(ЗначениеЗаполнено(ДатаВыдачи),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = ', выдан: %1 года';
																			|en = ', issued: %1'"), Формат(ДатаВыдачи,"ДЛФ=DD")), "")
			+ ?(ЗначениеЗаполнено(СрокДействия),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = ', действует до: %1 года';
																			|en = ', valid till: %1'"), Формат(СрокДействия,"ДЛФ=DD")), "")
		...
		ПредставлениеДокументаФизЛица = ПредставлениеДокументаФизЛица +  ?(ЗначениеЗаполнено(КодПодразделения) И ВидДокумента = Справочники.ВидыДокументовФизическихЛиц.ПаспортРФ,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = ', № подр. %1';
																			|en = ', dept. No.%1'"), КодПодразделения), "")
		//-- Локализация
			...
			Если ТекущиеДанные.Различие Тогда
				...
				Если ИспользуютсяТолькоПартнеры Тогда
					ТекстВопроса = НСтр("ru = 'Выбранный контрагент имеет существенное различие с введенными данными. Продолжить?';
										|en = 'Selected counterparty has a significant difference with the data entered. Continue?'");
				Иначе
					ТекстВопроса = НСтр("ru = 'Выбранный партнер имеет существенное различие с введенными данными. Продолжить?';
										|en = 'Selected partner has a significant difference with data entered. Continue?'");
				КонецЕсли;
...
Процедура УстановитьЗаголовки()
	...
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	                     ЭтотОбъект, "", НСтр("ru = 'Помощник регистрации нового контрагента';
												|en = 'New counterparty registration wizard'"), ИспользуютсяТолькоПартнеры);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	                     ЭтотОбъект, "ДекорацияПервичнаяИнформация", 
	                     НСтр("ru = 'Укажите доступную вам на данный момент информацию о контрагенте';
								|en = 'Specify available details about counterparty'"), ИспользуютсяТолькоПартнеры);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	                     ЭтотОбъект, "УказатьДанныеКонтактногоЛица", 
	                     НСтр("ru = 'Указать данные контактного лица (будет создано контактное лицо контрагента)';
								|en = 'Specify contact person details (counterparty''s contact person will be created)'"), ИспользуютсяТолькоПартнеры);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	                     ЭтотОбъект, "ФактическийАдрес", 
	                     НСтр("ru = 'Фактический адрес контрагента';
								|en = 'Business address of the counterparty'"), ИспользуютсяТолькоПартнеры);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	                     ЭтотОбъект, "ЮридическийАдрес", 
	                     НСтр("ru = 'Юридический адрес контрагента';
								|en = 'Counterparty legal address'"), ИспользуютсяТолькоПартнеры);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	                     ЭтотОбъект, "ДекорацияНайдено", 
	                     НСтр("ru = 'Найдены контрагенты с совпадающими данными, примите решение о дальнейших действиях.';
								|en = 'Counterparties with matching data are found, decide on further actions.'"), ИспользуютсяТолькоПартнеры);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	                     ЭтотОбъект, "ТаблицаНайдено", 
	                     НСтр("ru = 'Найденные контрагенты';
								|en = 'Found counterparties'"), ИспользуютсяТолькоПартнеры);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	                     ЭтотОбъект, "ДеревоСравненияПартнеров", 
	                     НСтр("ru = 'Сравнение контрагентов';
								|en = 'Compare counterparties'"), ИспользуютсяТолькоПартнеры);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	                     ЭтотОбъект, "ДеревоСравненияПартнеровЗначениеНайденного", 
	                     НСтр("ru = 'Найденный контрагент';
								|en = 'Found counterparty'"), ИспользуютсяТолькоПартнеры);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	                     ЭтотОбъект, "ДеревоСравненияПартнеровЗначениеТекущего", 
	                     НСтр("ru = 'Создаваемый контрагент';
								|en = 'Counterparty to create'"), ИспользуютсяТолькоПартнеры);
...
Процедура СоздатьПартнера(СписокСозданных, ТребуетсяПереходНаСтраницуСовпадений)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Создание нового партнера.';
										|en = 'Create a new partner.'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПроверитьНомерСчетаРФ(Постфикс)
	...
	Если Не ЭтотОбъект["ИностранныйБанк" + Постфикс]
		...
		Элементы["ОшибкаВНомереСчета" + Постфикс].Заголовок = НСтр("ru = 'Номер не соответствует формату банковского счета РФ';
																	|en = 'The number does not match the format of the bank account of the Russian Federation'");
		Элементы["ГруппаОшибкаВНомереСчета" + Постфикс].Видимость = Истина;
