...
Процедура ЗаполнитьДанныеСтрокиЦепочкиПередоверия(ФайлМЧД, СтрокаДерева)
	...
	СтрокаДерева.СрокДействия = СтрШаблон(НСтр("ru = '%1';
												|en = '%1'"), СрокДействия);
	СтрокаДерева.Представление = СтрШаблон(НСтр("ru = '%1';
												|en = '%1'"), Представление);
...
Функция ЗаписатьСтатусИзРеестраФНС(Результат)
	...
	Если Результат.Статус = "Ошибка" Тогда
		Операция = НСтр("ru = 'Обновление статуса МЧД из реестра ФНС.';
						|en = 'Update the status of the machine-readable authorization letter from the FTS registry.'");
		...
		ТекстОшибкиДляПользователя = НСтр("ru = 'Ошибка при обновлении статуса МЧД из реестра ФНС.';
											|en = 'An error occurred when updating the status of the machine-readable authorization letter from the FTS registry.'") + Символы.ПС
	...
	Если Не ЗначениеЗаполнено(Настройки.ДатаОбновленияСтатуса) Тогда
		ТекстСообщения = НСтр("ru = 'Не удалось получить текущий статус МЧД из распределенного реестра ФНС. Повторите попытку позже.';
								|en = 'Не удалось получить текущий статус МЧД из распределенного реестра ФНС. Повторите попытку позже.'");
...
Процедура ПроверкаПодписиМЧДЗавершение(РезультатПроверки, Параметры) Экспорт
	...
	Иначе
		...
		ТекстСообщения = НСтр(
			...
			|en = 'Cannot return. Cryptographic authentication of this authorization letter failed. 
			|The signature is incorrect or the authorization letter was changed after signing. 
			|Create or import a new authorization letter'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
...
Процедура ВводДатыОтзываЗавершение(Результат, Параметры) Экспорт
	...
	ШаблонПредставленияДоверенности = НСтр("ru = 'Доверенность № %1 от %2 была отозвана с %3';
											|en = 'Доверенность № %1 от %2 была отозвана с %3'");
	...
	Если ТекущаяДата < Объект.ДатаПрекращения Тогда
		ШаблонПредставленияДоверенности = НСтр("ru = 'Доверенность № %1 от %2 будет отозвана с %3';
												|en = 'Доверенность № %1 от %2 будет отозвана с %3'");
	КонецЕсли;
	...
	ШаблонТекста = 
		...
		НСтр("ru = 'Данная доверенность не была зарегистрирована в распределенном реестре ФНС.';
			|en = 'Данная доверенность не была зарегистрирована в распределенном реестре ФНС.'"),
		НСтр("ru = 'Поэтому контрагенты не узнают об отзыве этой доверенности.';
			|en = 'Поэтому контрагенты не узнают об отзыве этой доверенности.'"),
		НСтр("ru = 'Можно отправить контрагентам сообщение следующего содержания:';
			|en = 'Можно отправить контрагентам сообщение следующего содержания:'"),
		ТекстОтзываДляКопирования);
	...
	ПараметрыВопросаПользователю.Заголовок =  НСтр("ru = 'Сообщите контрагентам об отзыве МЧД';
													|en = 'Сообщите контрагентам об отзыве МЧД'");
