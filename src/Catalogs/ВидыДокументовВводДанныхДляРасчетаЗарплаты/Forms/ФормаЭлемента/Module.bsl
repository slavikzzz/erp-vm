...
Процедура СпособПримененияЗначенийПоказателейПриИзменении(Элемент)
	...
	Если Не ДоступностьВводаОперативныхДанных И (ВыбранныеВидыВремени.Количество() > 0 Или Объект.ВидыРабот.Количество() > 0) Тогда
		...
		ТекстВопроса = НСтр("ru = 'При вводе постоянных показателей ввод оперативных данных (о времени или сдельных работах) невозможен. Выбор видов времени и/или видов работ будет отменен.
			|Продолжить?';
			|en = 'Cannot enter operational data (on time and piecework) while entering permanent indicators. Selection of time kinds and/or work kinds will be canceled.
			|Continue?'");
	...
	Если Объект.ВидыРабот.Количество() > 0 Или ВыбранныеВидыВремени.Количество() > 0 Или КоличествоВыбранныхПоказателей(ЭтаФорма) > 0 Тогда
		Если ВыбранныеВидыВремени.Количество() + КоличествоВыбранныхПоказателей(ЭтаФорма) = 0 Тогда
			ТекстВопроса = НСтр("ru = 'Список выбранных видов работ будет очищен.
				|Продолжить?';
				|en = 'List of the selected work kinds will be cleared.
				|Continue?'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Такой способ ввода не предусматривает совместный ввод выполненных работ
				...
				|Продолжить?';
				|en = 'Such input method does not allow you to enter performed works
				|and values of indicators and/or working hours data together.
				|Selection of indicators, time kinds will be cleared.
				|Continue?'");
		КонецЕсли;
...
Процедура НастроитьЭлементыВводаИспользуемыхВидовВремени()
	...
	|			ИЛИ ВидыВремени.ОсновноеВремя = &МеждувахтовыйОтдых
	|				И (ВидыВремени.Предопределенный = ЛОЖЬ
	|					ИЛИ ВидыВремени.Ссылка = &МеждувахтовыйОтдых)
	|			ИЛИ ВидыВремени.ОсновноеВремя = &ДниВПутиВахта
	|				И (ВидыВремени.Предопределенный = ЛОЖЬ
	|					ИЛИ ВидыВремени.Ссылка = &ДниВПутиВахта))
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("МеждувахтовыйОтдых", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.МеждувахтовыйОтдых"));
	Запрос.УстановитьПараметр("ДниВПутиВахта", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ДниВПутиВахта"));
