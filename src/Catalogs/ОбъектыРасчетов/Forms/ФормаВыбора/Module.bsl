...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПравоДоступа("Чтение", Метаданные.Справочники.ДоговорыКонтрагентов)
		...
		Текст = НСтр("ru = 'Недостаточно прав для открытия формы выбора объекта расчетов.';
					|en = 'Insufficient rights to open the choice form of an AR/AP object.'");
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
				...
				НСтр("ru = 'Скрывать служебные';
					|en = 'Hide internal AR/AP objects'"),
				Истина,
	...
	Если Параметры.ВыборОснованияПлатежа Тогда
		Заголовок = СтрШаблон(НСтр("ru = 'Выбор основания платежа %1';
									|en = '%1 payment purpose selection'"),
			?(ЭтоРасчетыСКлиентами,НСтр("ru = 'клиента';
										|en = 'Customer'"),НСтр("ru = 'поставщика';
																|en = 'Vendor'")));
	ИначеЕсли БезОтбораПоТипуРасчетов Тогда
		Заголовок = НСтр("ru = 'Выбор объекта расчетов';
						|en = 'Select AR/AP object'");
	Иначе
		Заголовок = СтрШаблон(НСтр("ru = 'Выбор объекта расчетов с %1';
									|en = 'Select %1 AR/AP object'"),
			?(ЭтоРасчетыСКлиентами,НСтр("ru = 'клиентом';
										|en = 'customer'"),НСтр("ru = 'поставщиком';
																|en = 'vendor'")));
	КонецЕсли;
	...
	Если Параметры.Свойство("ПодборДебиторскойЗадолженности") Тогда
		Элементы.ОбъектыРасчетовСуммаОстаток.Заголовок = ?(Параметры.ПодборДебиторскойЗадолженности, НСтр("ru = 'Дебиторская задолженность';
																											|en = 'Accounts receivable'"), НСтр("ru = 'Кредиторская задолженность';
																																					|en = 'Accounts payable'"));
	...
	Если СтрокаТаблицы <> Неопределено Тогда
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "ГруппировкаСтроки") Тогда
			ПоказатьПредупреждение(,
							...
							|To expand groups, use ""Ctrl"" and the up/down arrows
							|or ""+"" and ""-"" keys on the additional keyboard.'"));
		Иначе
