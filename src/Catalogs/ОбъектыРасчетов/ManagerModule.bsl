...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	МассивКомментариев.Добавить(НСтр("ru = 'Заполнение реквизита ""Операция с самозанятым"" в справочнике ""Объекты расчетов""';
									|en = 'Fills the ""Operation with self-employed person"" attribute in the ""AR/AP objects"" catalog.'"));
	//-- Локализация
	МассивКомментариев.Добавить(НСтр("ru = 'Заполнение реквизита ""Контролировать остатки авансов клиентов""';
									|en = 'Fill the ""Control customer prepayment balances"" attribute'"));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст = 
		...
		|					КОГДА Справочник.Объект Ссылка Документ.ЗаказДавальца
		|						ТОГДА ВЫРАЗИТЬ(Справочник.Объект КАК Документ.ЗаказДавальца).ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
		//-- Устарело_Переработка24
		|					КОГДА Справочник.Объект Ссылка Документ.ЗаказДавальца2_5
		|						ТОГДА ВЫРАЗИТЬ(Справочник.Объект КАК Документ.ЗаказДавальца2_5).ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
		//-- НЕ УТКА
		
		//++ НЕ УТ
	
		//++ Устарело_Переработка24
		|					КОГДА Справочник.Объект Ссылка Документ.ЗаказПереработчику
		|						ТОГДА ВЫРАЗИТЬ(Справочник.Объект КАК Документ.ЗаказПереработчику).ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
		//-- Устарело_Переработка24
		|					КОГДА Справочник.Объект Ссылка Документ.ЗаказПереработчику2_5
		|						ТОГДА ВЫРАЗИТЬ(Справочник.Объект КАК Документ.ЗаказПереработчику2_5).ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
		//-- НЕ УТ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось заполнить значение реквизита ""Операция с самозанятым"" справочнике ""Объекты расчетов""';
								|en = 'Cannot fill the ""Operation with self-employed person"" attribute value in the ""AR/AP objects"" catalog'"));
	...
	ШаблонСообщения = НСтр("ru = 'Обработана порция объектов расчетов: %1';
							|en = 'A batch of AR/AP objects is processed: %1'");
