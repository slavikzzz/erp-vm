...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Элементы.ГруппаСоздать.Видимость Тогда
		...
		Элементы.ФормаСоздатьУпаковку.Видимость = Ложь;
		Если ТипЗнч(Номенклатура) = Тип("СправочникСсылка.Номенклатура") и ЗначениеЗаполнено(Номенклатура) Тогда
			ЗначениеРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "ИспользоватьУпаковки,НаборУпаковок");
			Элементы.ФормаСоздатьУпаковку.Видимость = ЗначениеРеквизитов.ИспользоватьУпаковки
		КонецЕсли;
		Если Не (ДобавлятьЕдиницыИзмерения
			...
			Элементы.СоздатьУпаковку.Видимость = ЗначениеЗаполнено(Номенклатура);
	...
	Если ТипЗнч(ВыбраннаяСтрокаДерева.Ссылка) = Тип("СправочникСсылка.УпаковкиЕдиницыИзмерения") Тогда 
		Если Не ВыбраннаяСтрокаДерева.ДопустимыйКоэффициент Тогда
			ТекстПредупреждения = 
				...
				|en = 'Cannot convert [ЕдИзм1] to [ЕдИзм2] for [Номенклатура].
				|The conversion ratio is too big.'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Представление = НСтр("ru = 'Изменение цвета текста единиц измерения с недопустимым коэффициентом';
								|en = 'Text color for units with unsupported conversion ratio'");
