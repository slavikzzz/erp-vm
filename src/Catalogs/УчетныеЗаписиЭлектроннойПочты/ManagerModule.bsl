...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление учетных записей';
										|en = 'Email account update'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	Элемент.ИмяПользователя                  = НСтр("ru = '1С:Предприятие';
													|en = '1C:Enterprise'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПроверитьПодключениеКСерверуВходящейПочты(Профиль, Протокол)
	...
	Если Протокол = ПротоколИнтернетПочты.POP3 Тогда
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1:%2%3 (%4)" + Символы.ПС + "%5",
			...
			?(ПустаяСтрока(ТекстОшибки), НСтр("ru = 'OK';
												|en = 'OK'"), ТекстОшибки));
	Иначе
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1:%2%3 (%4)" + Символы.ПС + "%5",
...
Функция ПроверитьНастройкиПрофилей(ПрофильИсходящейПочты, ПрофильВходящейПочты, Знач АдресЭлектроннойПочты) Экспорт
	...
	ТехническиеПодробности.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Адрес почты: %1';
			|en = 'Email address: %1'"), АдресЭлектроннойПочты));
	...
	Если ЗначениеЗаполнено(ОшибкиПодключения) Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка настроек почты';
										|en = 'Validating email account settings'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если ЗначениеЗаполнено(Профиль.АдресСервераSMTP) Тогда
		...
		Для Каждого ИмяСвойства Из СтрРазделить(СписокСвойствSMTP, ",", Ложь) Цикл
			Настройки.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1=""%2""';
					|en = '%1=""%2""'"), ИмяСвойства, Профиль[ИмяСвойства]));
		КонецЦикла;
...
Функция ИнформацияОПрограмме()
	...
	Результат = Новый Массив;
	Результат.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Операционная система: %1';
																					|en = 'Operating system: %1'"), СистемнаяИнформация.ВерсияОС));
...
Функция СобытиеПроверкаПодключенияКПочтовомуСерверу()
	Возврат НСтр("ru = 'Проверка подключения к почтовому серверу';
				|en = 'Mail server connection test'", ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
