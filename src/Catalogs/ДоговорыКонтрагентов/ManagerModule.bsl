...
Функция ХозяйственнаяОперация(ТипДоговора, ВариантОформленияЗакупок) Экспорт
	...
	//-- Устарело_Переработка24
	ИначеЕсли ТипДоговора = Типы.СПереработчиком2_5 Тогда
		Возврат Операции.ПроизводствоУПереработчика2_5;
	ИначеЕсли ТипДоговора = Типы.СПереработчиком2_5_ЕАЭС Тогда
		Возврат Операции.ПроизводствоУПереработчикаВСтранахЕАЭС2_5;
	//-- НЕ УТ

	//++ НЕ УТКА
	
	//++ Устарело_Переработка24
	ИначеЕсли ТипДоговора = Типы.СДавальцем Тогда
		Возврат Операции.ПроизводствоИзДавальческогоСырья;
	//-- Устарело_Переработка24
	ИначеЕсли ТипДоговора = Типы.СДавальцем2_5 Тогда
		Возврат Операции.ПроизводствоИзДавальческогоСырья2_5;
	//-- НЕ УТКА

	//++ НЕ УТ
	ИначеЕсли ТипДоговора = Типы.ПередачаВАренду Или ТипДоговора = Типы.Субаренда Тогда
		Возврат Операции.РеализацияУслугПоАренде;
	//-- НЕ УТ
	КонецЕсли;
...
Функция ТипДоговора(ХозяйственнаяОперация) Экспорт
	...
	//-- Устарело_Переработка24
	ИначеЕсли ХозяйственнаяОперация = Операции.ПроизводствоУПереработчика2_5 Тогда
		Возврат Типы.СПереработчиком2_5;
	ИначеЕсли ХозяйственнаяОперация = Операции.ПроизводствоУПереработчикаВСтранахЕАЭС2_5 Тогда
		Возврат Типы.СПереработчиком2_5_ЕАЭС;
	//-- НЕ УТ

	//++ НЕ УТКА

	//++ Устарело_Переработка24
	ИначеЕсли ХозяйственнаяОперация = Операции.ПроизводствоИзДавальческогоСырья Тогда
		Возврат Типы.СДавальцем;
	//-- Устарело_Переработка24
	ИначеЕсли ХозяйственнаяОперация = Операции.ПроизводствоИзДавальческогоСырья2_5 Тогда
		Возврат Типы.СДавальцем2_5;
	//-- НЕ УТКА
	КонецЕсли;
...
Функция ПараметрыЗаполненияНалогообложенияНДСПродажи(Объект) Экспорт
	...
	//++ НЕ УТКА
	ИначеЕсли Объект.ТипДоговора = Перечисления.ТипыДоговоров.СДавальцем2_5
		//++ Устарело_Переработка24
	...
	//++ НЕ УТ
	ИначеЕсли Объект.ТипДоговора = Перечисления.ТипыДоговоров.ПередачаВАренду
		Или Объект.ТипДоговора = Перечисления.ТипыДоговоров.Субаренда Тогда
...
Функция ПараметрыЗаполненияНалогообложенияНДСЗакупки(Объект) Экспорт
	...
	ИначеЕсли Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПереработчиком Тогда
		...
		ПараметрыЗаполнения.ПриобретениеРабот = Истина;
	//-- Устарело_Переработка24
	ИначеЕсли Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПереработчиком2_5 Тогда
		
		ПараметрыЗаполнения.ПриобретениеРабот        = Истина;
		ПараметрыЗаполнения.ПриобретениеНаСтатьи     = Истина;
		ПараметрыЗаполнения.ПоступлениеИзПереработки = Истина;
		
	ИначеЕсли Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПереработчиком2_5_ЕАЭС Тогда
...
Функция ПараметрыВзаиморасчеты(ДанныеЗаполнения = Неопределено) Экспорт
	...
	Если ТипДоговора = Перечисления.ТипыДоговоров.СПокупателем 
		...
		ИЛИ ТипДоговора = Перечисления.ТипыДоговоров.Субаренда
		//-- НЕ УТ
...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Отбор") Тогда
		ОтборПараметров = Параметры.Отбор;
		Если ОтборПараметров.Свойство("ХозяйственнаяОперация")
			И ОтборПараметров.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчика2_5 Тогда
			ПроверятьСоглашение = Ложь;
		КонецЕсли;
	КонецЕсли;
	//-- НЕ УТ
		...
		Для Каждого ТекущийОтбор Из Параметры.Отбор Цикл
			...
			ИначеЕсли ТекущийОтбор.Ключ = "ХозяйственнаяОперация" Тогда
				...
				Если Параметры.Свойство("ПоказыватьДоговорыРаздельнойЗакупки")
					...
					//++ НЕ УТ
					Если Параметры.Свойство("ПоказыватьПроизводствоУПереработчика") Тогда
					
						Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне2_5") Тогда
							Если ТекущийОтбор.Значение = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика Тогда 
								ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Запрос.Параметры.ХозяйственныеОперацииРаздельнойЗакупки, 
											ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчика2_5));
							ИначеЕсли ТекущийОтбор.Значение = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС Тогда 
								ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Запрос.Параметры.ХозяйственныеОперацииРаздельнойЗакупки, 
											ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчикаВСтранахЕАЭС2_5));
							КонецЕсли;
						КонецЕсли;
...
Функция ШаблонНазначения(Объект) Экспорт
	...
	//-- Устарело_Переработка24
	ИначеЕсли Объект.ТипДоговора = Перечисления.ТипыДоговоров.СДавальцем2_5 Тогда
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	ТекстОписания = НСтр("ru = 'Сбрасывает флаг %1 для всех договоров кроме договоров с комитентом';
						|en = 'Clears the %1 checkbox for all contracts except for contracts with consignors'");
