...
Процедура Заполнить(Команда)
	...
	Если Объект.Филиалы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОбработчикОтвета, НСтр("ru = 'Список филиалов будет очищен. Продолжить?';
												|en = 'List of branch offices will be cleared. Continue?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
		ЗаполнитьНаСервере();
	КонецЕсли;
