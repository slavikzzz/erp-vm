...
Процедура ТипДоговораНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	МассивТипов.Добавить(ПредопределенноеЗначение("Перечисление.ТипыДоговоров.СПереработчиком"));
	//-- Устарело_Переработка24

	//++ НЕ УТКА

	//++ Устарело_Переработка24
	МассивТипов.Добавить(ПредопределенноеЗначение("Перечисление.ТипыДоговоров.СДавальцем"));
	//-- Устарело_Переработка24

	//-- НЕ УТКА
	
	ДоступныеТипыДоговоров = Неопределено;
	Если МассивТипов.Найти(Объект.ТипДоговора) <> Неопределено Тогда
		ДоступныеТипыДоговоров = ДоступныеТипыДоговоров(Объект.ТипДоговора, СтандартнаяОбработка)
	КонецЕсли;
	
	Если ДоступныеТипыДоговоров <> Неопределено Тогда
		ДанныеВыбора = ДоступныеТипыДоговоров;
	КонецЕсли;
	//-- НЕ УТ
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(Объект.Дата) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Для ввода графика укажите дату договора.';
				|en = 'To enter the schedule, specify the contract date.'"));
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Договор еще не записан. Перед созданием договора с комиссионером необходимо записать данный договор с клиентом.';
				|en = 'The contract is not saved yet. Before creating a contract with a consignee, save this contract with a customer.'"));
		СтандартнаяОбработка = Ложь;
...
Процедура ТипДоговораПриИзмененииСервер()
		...
		И (Не РеквизитыПартнера.Клиент
		   ...
		   ИЛИ Не РеквизитыПартнера.Поставщик
			И (ОперацииЗакупки.Найти(Объект.ХозяйственнаяОперация) <> Неопределено
				...
				Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчикаВСтранахЕАЭС2_5
				//-- НЕ УТ
	...
	Объект.ПереоцениватьТоварыУслугиКОтчетуКомитенту = 
			Объект.ТипДоговора = Перечисления.ТипыДоговоров.СКомитентом
		...
		ИначеЕсли ФлагиПартнера.Клиент <> Истина 
			...
			Если ЭтоНовый Тогда
				ТекстСообщения = НСтр("ru = 'Создание договора с клиентом возможно для партнеров с признаком ""Клиент""';
										|en = 'You can create a customer contract with partners that have the ""Customer"" flag'");
			Иначе
		...
		ИначеЕсли ФлагиПартнера.Поставщик <> Истина
			...
			Если ЭтоНовый Тогда
				ТекстСообщения = НСтр("ru = 'Создание договора с поставщиком возможно для партнеров с признаком ""Поставщик""';
										|en = 'You can create a contract with vendor only with partners that have the ""Vendor"" flag'");
			Иначе
		...
		Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПриемНаХранениеСПравомПродажи Тогда
			СписокВыбора.Добавить(Перечисления.ПорядокРасчетов.ПоЗаказам,
									НСтр("ru = 'По документам выкупа и списания товаров с хранения';
										|en = 'By purchase invoices — VMI and VMI adjustments'"));
		Иначе
			СписокВыбора.Добавить(Перечисления.ПорядокРасчетов.ПоЗаказам,
									НСтр("ru = 'По документам выкупа товаров хранителем';
										|en = 'According to ""Sales invoice — VMI sold"" documents'"));
		КонецЕсли;
...
Процедура НастроитьПараметрыВыбораСтатьиДвиженияДенежныхСредств()
	...
	//-- Устарело_Переработка24
	ТипыДоговоровПокупатель.Добавить(Перечисления.ТипыДоговоров.СДавальцем2_5);
	//-- НЕ УТКА
	...
	ТипыДоговоровПоставщик.Добавить(Перечисления.ТипыДоговоров.СПереработчиком2_5_ЕАЭС);
	//-- НЕ УТ
...
Процедура УправлениеЭлементамиГрафикИсполнения()
	...
	ТипыДоговоров.Добавить(Перечисления.ТипыДоговоров.СПереработчиком2_5_ЕАЭС);
	//-- НЕ УТ
...
Процедура УстановитьВидимостьПорядокСписанияПринятыхТоваров(Форма)
	...
	Форма.Элементы.ПорядокОформленияСписанияНедостачТоваровПринятыхНаХранение.Видимость = ?(
		Форма.Объект.ТипДоговора = ПредопределенноеЗначение("Перечисление.ТипыДоговоров.СПоклажедателем")
			//++ НЕ УТКА
			Или Форма.Объект.ТипДоговора = ПредопределенноеЗначение("Перечисление.ТипыДоговоров.СДавальцем2_5")
			//-- НЕ УТКА
...
Процедура УстановитьВидимостьНоваяСхемаКомиссии()
	...
	Элементы.ГруппаФинансовогоУчета.Заголовок = ?(ЭтоНоваяКомиссия, НСтр("ru = 'Группа фин. учета расчетов с комиссионером';
																		|en = 'Group of financial AR/AP accounting with a consignee'"), "");
...
Процедура УстановитьЗаголовкиРеквизитовПриемкиТоваров()
	...
	ЗаголовокПриемкиТоваров			= ?(ДоступноОформлениеРаздельнойЗакупки,
										...
										НСтр("ru = 'Оформление поступлений и приемки на ордерный склад';
											|en = 'Register goods receipts at the warehouse with warehouse management enabled'"));
	ЗаголовокОформлениеПоступления	= ?(ДоступноОформлениеРаздельнойЗакупки,
										...
										НСтр("ru = 'Оформление ордера доступно';
											|en = 'Goods receipt registration is available'"));
	...
	ПодсказкаПриемкаТоваров			= ?(ДоступноОформлениеРаздельнойЗакупки,
										...
										НСтр("ru = 'Приемку товаров на ордерном складе можно сгруппировать по договору,
												   |либо разделить по документам.';
												   |en = 'You can group the receipt of goods at warehouse (when warehouse management is enabled) by a contract
												   |or split it by documents.'"));
...
//++ НЕ УТ
&НаСервереБезКонтекста
Функция ДоступныеТипыДоговоров(ТипДоговораДоИзменения, СтандартнаяОбработка)
	
	ПараметрыВыбора = Новый Структура;
	ДоступныеТипы = Перечисления.ТипыДоговоров.ПолучитьДанныеВыбора(ПараметрыВыбора);
	Если ДоступныеТипы.НайтиПоЗначению(ТипДоговораДоИзменения) = Неопределено Тогда
		СтандартнаяОбработка = Ложь;
		ДоступныеТипы.Добавить(ТипДоговораДоИзменения);
		ДоступныеТипы.СортироватьПоЗначению();
	Иначе
		ДоступныеТипы = Неопределено;
	КонецЕсли;
