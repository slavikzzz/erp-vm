...
Процедура ПеренестиИсточникВПриемник(Источник, Приемник, ПриемникВладелец)
	...
	Если ЗначениеЗаполнено(ПриемникВладелец)
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Невозможно переместить элемент %1:
							|	договор подключения к платежной системе у группы не соответствует перемещаемому элементу';
							|en = 'Cannot transfer the %1 item:
							|	the payment system contract of the group does not match the item to transfer'"), Строка(Источник.Наименование));
