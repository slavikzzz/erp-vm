...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ДанныеЗаполнения) И ДанныеЗаполнения.Свойство("ВидОрганизации") Тогда
		...
		Иначе ВызватьИсключение Нстр("ru = 'Создание организации невозможно. На форму переданы неверные параметры.
											...
											|en = 'Cannot create company. Invalid parameters were passed to the form.
	...
	Если ЭтоНовый() И Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоВалют") Тогда
		...
		Если Не ЗначениеЗаполнено(ВалютаРегламентированногоУчета) Тогда
			...
			|en = 'Cannot fill in the ""Local accounting currency"" field. Probably no currencies are registered in the infobase
			|To create currencies, go to ""Master data and settings"" – ""Enterprise"" – ""Currencies"".'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЭтоНовый()
		...
		ТекстСообщения = НСтр("ru = 'Организация не может быть обособленным подразделением, потому что она уже выбрана в качестве головной для других организаций.';
								|en = 'Company cannot be a branch office as it has already been selected as the parent company for other companies.'");
	...
	Если Не ЭтоНовый() Тогда
		ДействующиеПараметры = НастройкиНалоговУчетныхПолитик.ДействующиеПараметрыНалоговУчетныхПолитикНаДату("НастройкиСистемыНалогообложения", Ссылка,,Ложь);
		Если ЗначениеЗаполнено(ДействующиеПараметры) Тогда
			Если ЮрФизЛицо = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель
				И ДействующиеПараметры.СистемаНалогообложения =  Перечисления.СистемыНалогообложения.Общая Тогда
				
				ТекстСообщения = НСтр("ru = 'Для индивидуального предпринимателя доступна только упрощенная система налогообложения.';
										|en = 'For an individual entrepreneur simplified taxation system is available only.'");
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект,,, Отказ);
...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") Тогда
		...
		Если Не Запрос.Выполнить().Пустой() Тогда
			...
			Иначе
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Базовая версия может вести учет только по одной организации';
															|en = 'Base version can keep records only for one company'"),,,, Отказ);
			КонецЕсли;
		КонецЕсли;
	...
	Если Статус = Перечисления.СтатусыОрганизаций.НеДействует Тогда
		...
		Если Не Запрос.Выполнить().Пустой() Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Статус ""Не действует"" не может быть установлен для головной организации с действущими обособленными подразделениями';
														|en = 'Cannot set the ""Inactive"" status for a parent company with operating branch offices'"),,,, Отказ);
		КонецЕсли;
		Если ГоловнаяОрганизация <> Ссылка 
			...
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Выбрана головная организация со статусом ""Не действует"". Должна быть выбрана действующая головная организация';
														|en = 'A parent company with the ""Inactive"" status is selected. Select an operating parent company'"),,,, Отказ);
		КонецЕсли;
	КонецЕсли;
