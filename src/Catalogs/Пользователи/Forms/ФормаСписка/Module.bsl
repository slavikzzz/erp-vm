...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			...
			НСтр("ru = 'Подготовленные ответственным за список';
				|en = 'Users are submitted for authorization'"), Ложь,
	...
	Если Не ПустаяСтрока(РезультатОбработки) Тогда
		...
		ПоказатьПредупреждение(,
			...
			НСтр("ru = 'Не удалось заблокировать пользователей';
				|en = 'Cannot lock users'"));
	Иначе
		Состояние(НСтр("ru = 'Блокировка недействительных пользователей
			|выполнена.';
			|en = 'Invalid users are
			|locked.'"),,,БиблиотекаКартинок.Успешно32);
	КонецЕсли;
КонецПроцедуры
...
Функция ЗаблокироватьНедействительныхПользователейВСистемеВзаимодействийНаСервере()
	...
	Для каждого НедействительныйПользователь Из НедействительныеПользователи Цикл
		...
		Если РезультатБлокировки <> Неопределено Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обсуждения.Блокировка недействительных пользователей';
											|en = 'Conversations.Lock invalid users'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если СообщениеПользователю.ЕстьОшибки = Ложь Тогда
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Перемещение пользователей';
				|en = 'Move users'"), , СообщениеПользователю.Сообщение, БиблиотекаКартинок.ДиалогИнформация);
	Иначе
