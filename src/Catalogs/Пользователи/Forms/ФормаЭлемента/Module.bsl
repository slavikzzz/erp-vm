	...
	Если Не Элементы.Роли.ТолькоПросмотр Тогда
		...
		Для Каждого Элемент Из ЭлементыДерева Цикл
			...
			Если Элемент.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Роль ""%1"" недоступна для пользователей.';
																				|en = 'Role ""%1"" is unavailable to users.'"), Элемент.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Элемент),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Роль ""%1"" в строке %2 недоступна для пользователей.';
																				|en = 'Role ""%1"" in line %2 is unavailable to users.'"), Элемент.Синоним, "%1"));
			КонецЕсли;
		КонецЦикла;
