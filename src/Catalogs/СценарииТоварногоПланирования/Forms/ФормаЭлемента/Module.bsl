...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПолучитьФункциональнуюОпцию("УправлениеПредприятием") Тогда
		...
		Элементы.ИспользоватьДляПланированияМатериалов.Заголовок = НСтр("ru = 'Расчет потребностей в материалах и трудовых ресурсах';
																		|en = 'Calculation of demand for materials and human resources'");
		...
		Элементы.Календарь.Подсказка = НСтр("ru = 'Календарь работы, используемый для расчета дат запуска продукции, а так же сроков потребностей в материалах и трудовых ресурсах';
											|en = 'Work calendar used to calculate product launch dates as well as dates of demand for materials and human resources'");
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("СпособРасчетаПотребностейВМатериалах")
		...
		ИначеЕсли Объект.СпособРасчетаПотребностейВМатериалах = ПредопределенноеЗначение("Перечисление.СпособыРасчетаМатериалов.ВероятноеПотребление") Тогда
			ТекстПодсказки = НСтр("ru = 'Основные и альтернативные материалы планируются с поправкой на вероятность применения.';
									|en = 'Main and alternate materials are planned depending on use probability.'");
		ИначеЕсли Объект.СпособРасчетаПотребностейВМатериалах = ПредопределенноеЗначение("Перечисление.СпособыРасчетаМатериалов.МаксимальноеПотребление") Тогда
			ТекстПодсказки = НСтр("ru = 'Основные материалы планируются без поправки на вероятность применения, альтернативные с поправкой на вероятность.';
									|en = 'Main materials are planned regardless of use probability, and alternate materials are planned depending on use probability.'");
		ИначеЕсли Объект.СпособРасчетаПотребностейВМатериалах = ПредопределенноеЗначение("Перечисление.СпособыРасчетаМатериалов.МинимальноеПотребление") Тогда
			ТекстПодсказки = НСтр("ru = 'Основные материалы планируются без поправки на вероятность применения, альтернативные материалы не планируются.';
									|en = 'Main materials are planned regardless of use probability, and alternate materials are not planned.'");
		КонецЕсли;
...
Функция ПроверитьЗаписатьСценарий()
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ТекстВопроса = НСтр("ru = 'Для настройки вида плана необходимо записать сценарий. Записать?';
							|en = 'You need to record the scenario to configure the plan profile. Continue?'");
