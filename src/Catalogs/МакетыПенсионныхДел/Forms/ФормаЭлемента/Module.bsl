		...
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.Документ) Тогда
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'Заполните вид документа в строке %1';
								|en = 'Заполните вид документа в строке %1'"), ТаблицаДокументов.Индекс(СтрокаТаблицы) + 1),,
...
Функция ПроверитьЗаполнениеПоляОтправитель()
	...
	Если ЗначениеЗаполнено(Объект.Отправитель) И Не ЗначениеЗаполнено(УчетнаяЗаписьОтправителя(Объект.Отправитель)) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Организация не подключена к 1С-Отчетности';
								|en = 'Организация не подключена к 1С-Отчетности'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.Отправитель");
		ПолеЗаполнено = Ложь;	
	КонецЕсли;
...
Процедура ОткрытьФайлыДляПросмотра(Элемент, ВыбраннаяСтрока = Неопределено)
	...
	Если ПроверитьЗаполнениеПоляОтправитель() И ПроверитьЗаполнениеПоляПолучатель() Тогда
		...
		ОперацииСФайламиЭДКОКлиент.ДобавитьФайлыСПредпросмотром(
			...
			СтрШаблон(НСтр("ru = 'Выберите файлы документа <%1> ';
							|en = 'Выберите файлы документа <%1> '"), Документ), 
			ПараметрыФункции);
