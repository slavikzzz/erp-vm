	...
	Если ЗначениеЗаполнено(СрокПоставки) И ЗначениеЗаполнено(СрокДоставки) Тогда
		...
		Если СрокДоставки > СрокПоставки Тогда
			...
			ТекстОшибки = НСтр("ru = 'Срок доставки не может быть больше срока покупки';
								|en = 'Delivery time cannot be longer than the purchase period'");
...
Процедура ПроверитьАгентскиеУслуги(Отказ) Экспорт
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Для данной хозяйственной операции необходимо указание организации';
																|en = 'For this business transaction, company name is required'"),ЭтотОбъект,"Организация",,Отказ);
	КонецЕсли;
	...
	ШаблонСообщения = НСтр("ru = 'Обнаружено существующее соглашение %Наименование%%ДляУслуги%';
							|en = 'The existing %Наименование% %ДляУслуги% agreement is detected'");
