		...
		Если Выборка.Следующий() Тогда
			...
			Если СоответствуетОрганизации Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Данная организация уже сопоставлена другому хозяйствующему субъекту с идентификатором %1.';
												|en = 'Данная организация уже сопоставлена другому хозяйствующему субъекту с идентификатором %1.'"),
			...
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Данный контрагент уже сопоставлен другому хозяйствующему субъекту (%1).';
												|en = 'Данный контрагент уже сопоставлен другому хозяйствующему субъекту (%1).'"),
	...
	Если Предприятия.Количество() > 1 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ЭтоТорговыйОбъект = Истина Тогда
				...
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Торговый объект %1 сопоставлен более чем одному предприятию.';
												|en = 'Торговый объект %1 сопоставлен более чем одному предприятию.'"), Выборка.Объект);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,
				                                      ЭтотОбъект,
			...
			ИначеЕсли Выборка.ЭтоТорговыйОбъект = Ложь Тогда
				...
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Производственный объект %1 сопоставлен более чем одному предприятию.';
												|en = 'Производственный объект %1 сопоставлен более чем одному предприятию.'"), Выборка.Объект);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,
				                                      ЭтотОбъект,
			...
			Иначе
				...
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Предприятию %1, указанному для множественного сопоставления, ничего не сопоставлено.';
												|en = 'Предприятию %1, указанному для множественного сопоставления, ничего не сопоставлено.'"), Выборка.Объект);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,
				                                      ЭтотОбъект,
