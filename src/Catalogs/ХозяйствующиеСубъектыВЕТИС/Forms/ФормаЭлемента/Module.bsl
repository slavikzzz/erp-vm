...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = ОбменДаннымиИСКлиентСервер.ИмяСобытияИзмененоСостояние(ИнтеграцияВЕТИСКлиентСервер.ИмяПодсистемы())
		...
		Если ЕстьПравоСозданияКонтрагента Тогда
			ПоказатьВопрос(
				...
				НСтр("ru = 'Контрагент с указанными ИНН и КПП не найден. Создать нового?';
					|en = 'Контрагент с указанными ИНН и КПП не найден. Создать нового?'"),
				РежимДиалогаВопрос.ДаНет);
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Контрагент с указанными ИНН и КПП не найден.';
					|en = 'Контрагент с указанными ИНН и КПП не найден.'"),,
				"ИНН",
				"Объект");
		КонецЕсли;
	КонецЕсли;
...
Процедура СоздатьПредприятие(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru = 'Создание нового предприятия возможно только для записанного хозяйствующего субъекта. Записать?';
							|en = 'Создание нового предприятия возможно только для записанного хозяйствующего субъекта. Записать?'");
...
Процедура ИзменитьПредприятие(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Спозиционируйтесь на предприятии, которое необходимо изменить';
								|en = 'Спозиционируйтесь на предприятии, которое необходимо изменить'");
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ТекстВопроса = НСтр("ru = 'Редактирование предприятия возможно только для записанного хозяйствующего субъекта. Записать?';
							|en = 'Редактирование предприятия возможно только для записанного хозяйствующего субъекта. Записать?'");
...
Процедура СинхронизироватьСписокПредприятий(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru = 'Синхронизация списка предприятий возможна только для записанного хозяйствующего субъекта. Записать?';
							|en = 'Синхронизация списка предприятий возможна только для записанного хозяйствующего субъекта. Записать?'");
...
Процедура РазорватьСвязь(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Спозиционируйтесь на предприятии, с которым необходимо разорвать связь';
								|en = 'Спозиционируйтесь на предприятии, с которым необходимо разорвать связь'");
		ПоказатьПредупреждение(, ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьGLN(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Спозиционируйтесь на предприятии, которому необходимо изменить GLN';
								|en = 'Спозиционируйтесь на предприятии, которому необходимо изменить GLN'");
		ПоказатьПредупреждение(, ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура Подобрать(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru = 'Подбор предприятий возможен только для записанного хозяйствующего субъекта. Записать?';
							|en = 'Подбор предприятий возможен только для записанного хозяйствующего субъекта. Записать?'");
...
Процедура ЗагрузитьВсе(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru = 'Загрузка списка предприятий возможна только для записанного хозяйствующего субъекта. Записать?';
							|en = 'Загрузка списка предприятий возможна только для записанного хозяйствующего субъекта. Записать?'");
...
Процедура СкопироватьПользователя(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Спозиционируйтесь на пользователе, права которого необходимо скопировать';
								|en = 'Спозиционируйтесь на пользователе, права которого необходимо скопировать'");
		ПоказатьПредупреждение(, ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьПраваДоступа(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Спозиционируйтесь на пользователе, права которого необходимо изменить';
								|en = 'Спозиционируйтесь на пользователе, права которого необходимо изменить'");
		ПоказатьПредупреждение(, ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура УдалитьПользователя(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Спозиционируйтесь на пользователе, с которым необходимо разорвать связь';
								|en = 'Спозиционируйтесь на пользователе, с которым необходимо разорвать связь'");
		ПоказатьПредупреждение(, ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьСинхронизациюПредприятий(ЗагрузитьНовые)
	...
	Исключение
		...
		ОписаниеИсключения = НСтр( "ru = 'Синхронизация списка предприятий не выполнена по причине:';
									|en = 'Синхронизация списка предприятий не выполнена по причине:'");
