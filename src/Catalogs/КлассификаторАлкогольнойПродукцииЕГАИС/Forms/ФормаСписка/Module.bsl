...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияЕГАИС.ЭтоРасширеннаяВерсияГосИС() Тогда
		МодульОбщегоНазначенияИС = ОбщегоНазначения.ОбщийМодуль("ОбщегоНазначенияИС");
		МодульОбщегоНазначенияИС.НастроитьПодключаемоеОборудование(ЭтотОбъект, ПоддерживаемыеТипыПодключаемогоОборудования);
	КонецЕсли;
...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.Количество > 1 Тогда
			...
			Иначе
				ЗначениеПоля = СтрШаблон(НСтр("ru = '%1 ( + еще %2...)';
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'По коду акцизной марки %1 не удалось определить код алкогольной продукции';
						|en = 'По коду акцизной марки %1 не удалось определить код алкогольной продукции'"),
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'Марке по штрихкоду %1 не сопоставлена алкогольная продукция';
						|en = 'Марке по штрихкоду %1 не сопоставлена алкогольная продукция'"),
					ДанныеШтрихкода.Штрихкод));
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'По штрихкоду %1 не удалось найти справку 2';
						|en = 'По штрихкоду %1 не удалось найти справку 2'"),
					ДанныеШтрихкода.Штрихкод));
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'По штрихкоду %1 не удалось найти алкогольную продукцию';
						|en = 'По штрихкоду %1 не удалось найти алкогольную продукцию'"),
					ДанныеШтрихкода.Штрихкод));
		КонецЕсли;
