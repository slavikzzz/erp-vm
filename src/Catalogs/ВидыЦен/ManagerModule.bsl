...
//	СправочникСсылка.ВидыЦен - Найденный вид цен.
//
Функция ВидЦеныПлановойСтоимостиТМЦ(ВидЦены = Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(ВидЦены) Тогда
		Возврат ВидЦены;
	КонецЕсли;
	
	// выбор единственного значения вида цены или значения из константы
	Результат = ВидЦеныПоУмолчанию(Константы.ВидЦеныПлановойСтоимостиМатериаловРабот.Получить());
	
	// Попытка выбрать единственный вид цены с признаком ИспользоватьПриВыпускеПродукции.
...
Процедура УстановитьВидЦеныВОбъектах(
	...
	Если НЕ РеквизитыВидаЦены.Назначение = Перечисления.НазначенияВидовЦен.Общий Тогда
		СообщениеПользователю = 
			НСтр("ru = 'Вид цены ""%1"" недоступен для установки/очистки, поскольку является индивидуальным.';
				|en = 'Price type ""%1"" is not available for setting/cleaning since it is individual.'");
		СообщениеПользователю = 
		...
		Иначе
			СообщениеПользователю = 
				НСтр("ru = 'Для объектов типа ""%1"" недоступен для установки/очистки вид цен, отличный от розничного.';
					|en = 'The price type other than retail is not available for setting/cleaning for objects type ""%1"".'");
			СообщениеПользователю = 
		...
		Иначе
			СообщениеПользователю = 
				НСтр("ru = 'Для объектов типа ""%1"" недоступен для установки/очистки вид цен, отличный от учетного.';
					|en = 'The price type other than accounting is not available for setting/cleaning for objects type ""%1"".'");
			СообщениеПользователю = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СообщениеПользователю,ТипОбъектаУстановки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеПользователю);
			Возврат;
	...
	Иначе
		...
		СообщениеПользователю = 
			НСтр("ru = 'Для объектов типа ""%1"" не определен порядок работы с видами цен';
				|en = 'The procedure for working with price types is not defined for objects type ""%1""'");
		СообщениеПользователю = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СообщениеПользователю,ТипОбъектаУстановки);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеПользователю);
	...
	Если НельзяОбновить.Количество() > 0 ИЛИ ОшибокЗаписи.Количество() > 0 Тогда
		...
		Если НельзяОбновить.Количество() > 0 Тогда
			...
			|%3';
			|en = 'Cannot change price type ""%1"" for %2 item(s):
			|%3'");
		...
		Если ОшибокЗаписи.Количество() > 0 Тогда
			...
			|%2';
			|en = 'errors when updating %1 elements:
			|%2'");
			СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Если НовыйОбъект.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.РассчитыватьПоФормуламОтДругихВидовЦен Тогда
				...
				Если Отказ Тогда
					...
					РезультатСоздания.ОписаниеОшибки = НСтр("ru = 'В виде цен указана некорректная влияющая цена. Проверьте формулы или вид цен основание.';
															|en = 'An incorrect influencing price is specified in the price type. Check the formulas or basic price type.'");
...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ВыводитьПроизвольныйВидЦен", ЗначениеПараметра) И ЗначениеПараметра = Истина Тогда
		...
		Если Параметры.Отбор.Свойство("ИспользоватьПриВыпускеПродукции") Тогда
			ДополнительныеУсловия.Добавить("ВидыЦен.ИспользоватьПриВыпускеПродукции = &ИспользоватьПриВыпускеПродукции");
		...
		Если Параметры.Отбор.Свойство("ИспользоватьПриПередачеПродукцииДавальцу") Тогда
			ДополнительныеУсловия.Добавить("ВидыЦен.ИспользоватьПриПередачеПродукцииДавальцу = &ИспользоватьПриПередачеПродукцииДавальцу");
