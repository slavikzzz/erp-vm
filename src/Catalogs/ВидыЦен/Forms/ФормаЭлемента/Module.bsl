...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБ
		...
		Если Не ЗначениеЗаполнено(Объект.СхемаКомпоновкиДанных) Тогда
			Если ХранилищеСхемыКомпоновкиДанных <> Неопределено Тогда
				...
				КонецЕсли;
			Иначе
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Необходимо настроить схему компоновки данных для заполнения цен.';
															|en = 'Configure data composition schema for price population.'"), Объект.Ссылка,,,Отказ);
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ТекущийОбъект.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.РассчитыватьПоФормуламОтДругихВидовЦен Тогда
		...
		Если ИспользоватьЦеновыеГруппы Тогда
			...
			Если Отказ Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'В виде цен указан некорректный влияющий вид цены. Проверьте формулы или базовый вид цены.';
															|en = 'An incorrect influencing price type is specified in the price type. Check the formulas or basic price type.'"), Объект.Ссылка,,,Отказ);
			КонецЕсли;
	...
	Если ИспользоватьЦеновыеГруппы 
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекстОшибки = НСтр("ru = 'Ценовая группа ""%ЦеноваяГруппа%"" повторяется';
								|en = 'Price group ""%ЦеноваяГруппа%"" is repeated'");
		...
		Если ИспользоватьЦеновыеГруппы Тогда
			...
			Для Каждого Формула Из Объект.ЦеновыеГруппы Цикл
				...
				ТекстОшибки = НСтр("ru = 'В формуле для ценовой группы ""%ЦеноваяГруппа%"" обнаружены ошибки';
									|en = 'Errors are detected in a formula for price group ""%ЦеноваяГруппа%""'");
	...
	ИначеЕсли Объект.СпособЗаданияЦены = ПредопределенноеЗначение("Перечисление.СпособыЗаданияЦен.НаценкаНаДругойВидЦен") Тогда
		...
		Если Объект.Наценка < -100 Тогда
			ТекстОшибки = НСтр("ru = 'В скидке для обнаружена ошибка - скидка не может быть больше 100%';
								|en = 'An error is detected in the discount. The discount cannot be more than 100%'");
		...
		Если ИспользоватьЦеновыеГруппы Тогда
			Для Каждого ЦеноваяГруппа Из Объект.ЦеновыеГруппы Цикл
				...
				ТекстОшибки = НСтр("ru = 'В скидке для ценовой группы ""%ЦеноваяГруппа%"" обнаружены ошибки - скидка не может быть больше 100%';
									|en = 'Errors are detected in the discount for the ""%ЦеноваяГруппа%"" price group. The discount cannot be more than 100%'");
...
Процедура ВладелецИндивидуальногоВидаЦенПредставлениеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ИзменитьНазначениеВидаЦен" Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
			...
			|Записать элемент?';
			|en = 'The data has not been written yet.
			|Selecting the owner of the individual price list is only possible after the element has been saved.
			|Save the element?'");
...
Процедура РедактироватьСхемуКомпоновкиДанныхКлиент(РедактированиеНастроекОтбораПоНоменклатуре = Ложь)
	...
	ЗаголовокФормыНастройкиСхемыКомпоновкиДанных = НСтр("ru = 'Настройка схемы компоновки данных для вида цены ""%1""';
														|en = 'Setting of data composition schema for price type ""%1""'");
...
Процедура ПрименитьИзмененияКСхемеКомпоновкиДанных(АдресСхемыКомпоновкиДанныхВХранилище, АдресНастроекКомпоновкиДанных, АдресСхемыКомпоновкиДанныхКонтрольВХранилище = Неопределено)
	...
	Иначе
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Цены';
											|en = 'Prices'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Иначе
			...
			Иначе
				...
				ЦеновойДиапазон = НСтр("ru = 'От %НачалоДиапазона% до %ОкончаниеДиапазона%';
										|en = 'From %НачалоДиапазона% to %ОкончаниеДиапазона%'");
				ЦеновойДиапазон = СтрЗаменить(ЦеновойДиапазон, "%НачалоДиапазона%",    Форма.Объект.ПравилаОкругленияЦены[Индекс].НижняяГраницаДиапазонаЦен);
...
Процедура ОбновитьПредставлениеГлубиныПродаж()
	Если ГлубинаАнализаДанных = 0 Тогда
		ГлубинаАнализаДанныхПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'без ограничений';
																			|en = 'without restrictions'"),,,,"Изменить");
	Иначе
		ГлубинаАнализаДанныхПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'использовать данные за последние дней';
																			|en = 'use data for the last days'"),,,,"Изменить");
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьПредставлениеУточненияЗначенияСкидкиНаценки()
	...
	Если Объект.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.РассчитыватьПоФормуламОтДругихВидовЦен Тогда
		Если Объект.ЦеновыеГруппы.Количество() = 0 Тогда
			УточнениеПоЦеновымГруппамПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'Уточнить формулы по ценовым группам';
																						|en = 'Specify formulas by price groups'"),,,,"Изменить");
		Иначе
			УточнениеПоЦеновымГруппамПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'Есть уточнения формул по ценовым группам';
																						|en = 'There are specifications of formulas by price groups'"),,,,"Изменить");
		КонецЕсли;
	Иначе	
		Если Объект.ЦеновыеГруппы.Количество() = 0 Тогда
			УточнениеПоЦеновымГруппамПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'Уточнить наценки по ценовым группам';
																						|en = 'Specify markups by price groups'"),,,,"Изменить");
		Иначе
			УточнениеПоЦеновымГруппамПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'Есть уточнения наценок по ценовым группам';
																						|en = 'There are specifications of markups by price groups'"),,,,"Изменить");
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьПредставлениеУточненияЗначенияВалюты()
	...
	Если Объект.Валюты.Количество() = 0 Тогда
		УточнениеВалютПоЦеновымГруппамПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'Уточнить валюты по ценовым группам';
																						|en = 'Specify currencies by price groups'"),,,,"Изменить");
	Иначе
		УточнениеВалютПоЦеновымГруппамПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'Есть уточнения валют по ценовым группам';
																						|en = 'There are specifications of currencies by price groups'"),,,,"Изменить");
	КонецЕсли;
...
Процедура ОбновитьПредставлениеУточнениеПороговСрабатывания()
	...
	Если Объект.ПорогиСрабатывания.Количество() = 0 Тогда
		УточнениеПороговСрабатыванияПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'Уточнить пороги срабатывания по ценовым группам';
																					|en = 'Specify trigger thresholds by price groups'"),,,,"Изменить");
	Иначе
		УточнениеПороговСрабатыванияПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'Есть уточнения порогов срабатывания по ценовым группам';
																					|en = 'There are trigger threshold specifications by price groups'"),,,,"Изменить");
	КонецЕсли;
...
Процедура ОбновитьПредставлениеУточнениеНастроекОкругления()
	...
	Если Объект.ПравилаОкругленияЦены.Количество() = 0 Тогда
		УточнениеНастроекОкругленияПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'Настроить округление цен по диапазонам';
																					|en = 'Configure price rounding by ranges'"),,,,"Изменить");
	Иначе
			...
			Для Каждого Правило Из Объект.ПравилаОкругленияЦены Цикл
				...
				Если Правило.ТочностьОкругления = 0 Тогда
					СтрокаДляПодстановки = НСтр("ru = 'не округлять';
												|en = 'do not round off'");
				Иначе
					СтрокаДляПодстановки = НСтр("ru = 'с точностью';
												|en = 'to'") + " " + Формат(Правило.ТочностьОкругления, ФорматнаяСтрока);
				КонецЕсли;
				
				МассивПравил.Добавить(СтрШаблон(НСтр("ru = '%1 %2';
													|en = '%1 %2'"), Правило.ЦеновойДиапазон, СтрокаДляПодстановки));
		...
		Иначе
			...
			РасшифровкаЦеновыхДиапазонов = НСтр("ru = 'Настроить округление цен по диапазонам';
												|en = 'Configure price rounding by ranges'");
...
Процедура ОбновитьПредставлениеОтбораНоменклатуры()
	...
	Иначе
		КомпоновщикНастроекОтборПоНоменклатуреПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'всю номенклатуру';
																								|en = 'All items'"),,,,"Изменить");
...
Процедура ОбновитьПредставлениеКомпоновкиДанных()
	...
	Если КомпоновщикНастроек.Настройки.Отбор.Элементы.Количество() > 0
		...
		КомпоновщикНастроекПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'Дополнительно ограничивать отбор по:';
																			|en = 'Additionally limit filter by:'")
		                           + " " + Строка(КомпоновщикНастроек.Настройки.Отбор),,,,"Изменить");		
	Иначе
		КомпоновщикНастроекПредставление = Новый ФорматированнаяСтрока(НСтр("ru = 'Уточнить дополнительные настройки';
																			|en = 'Specify additional settings'"),,,,"Изменить");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВладелецИндивидуальногоВидаЦен) Тогда
		...
		Если ЗначениеЗаполнено(ВладелецИндивидуальногоВидаЦен) Тогда
			МассивТекстов.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'изменить';
																	|en = 'change'"),,,,"ИзменитьНазначениеВидаЦен"));
		Иначе
			МассивТекстов.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'выбрать';
																	|en = 'select'"),,,,"ИзменитьНазначениеВидаЦен"));
		КонецЕсли;
		МассивТекстов.Добавить(")");
