...
Процедура ВыполнитьПроверку(Команда)
	...
	Если Не Объект.Использование Тогда
		...
		ПоказатьВопрос(ОповещениеОЗавершении, НСтр("ru = 'Проверка отключена. Все равно выполнить?';
													|en = 'Checkup is disabled. Check anyway?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
	КонецЕсли;	
...
Функция ВыполнитьПроверкуНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Выполнение проверки ведения учета ""%1""';
			|en = 'Checking accounting ""%1""'"), Объект.Наименование);
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ОтдельноеРегламентноеЗадание = Неопределено Тогда
			Элементы.ПредставлениеИндивидуальногоРасписания.Заголовок = 
				Новый ФорматированнаяСтрока(НСтр("ru = 'Настроить расписание';
												|en = 'Configure schedule'"), , , , ПоместитьВоВременноеХранилище("ДобавитьЗадание", УникальныйИдентификатор));
		Иначе
			Элементы.ПредставлениеИндивидуальногоРасписания.Заголовок = 
...
Процедура УстановитьПутьДоПроцедурыОбработчика(Форма, МетаданныеТекущейПроверки)
	Форма.ПутьДоПроцедурыОбработчика = ?(МетаданныеТекущейПроверки = Неопределено, НСтр("ru = 'Не задан обработчик';
																						|en = 'Handler is not defined'"), МетаданныеТекущейПроверки.ОбработчикПроверки);
КонецПроцедуры
...
Процедура УстановитьСтандартныеНастройкиНаСервере()
	...
	Если МетаданныеТекущейПроверки = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Проверка ведения учета с идентификатором ""%1"" не существует.';
				|en = 'The accounting check with ID ""%1"" does not exist.'"), Объект.Идентификатор);
	КонецЕсли;
