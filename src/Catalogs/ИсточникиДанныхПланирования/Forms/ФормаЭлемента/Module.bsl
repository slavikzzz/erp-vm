		...
		Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
			...
			Исключение
				ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В источнике заполнения данных ""%1"" имеются ошибки в настройках. Установлены настройки по умолчанию.';
						|en = 'There are errors in the settings in the ""%1"" data filling source. The default settings are set.'"), Параметры.ЗначениеКопирования.Ссылка));
			КонецПопытки;
		Иначе
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В источнике заполнения данных ""%1"" имеются ошибки в настройках. Установлены настройки по умолчанию.';
				|en = 'There are errors in the settings in the ""%1"" data filling source. The default settings are set.'"), ТекущийОбъект.Ссылка));
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Компоновка настроек источника данных планирования';
										|en = 'Composition of planning data source settings'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура Редактировать(Команда)
	...
	ЗаголовокФормыНастройкиСхемыКомпоновкиДанных = НСтр("ru = 'Настройки источника данных планирования ""%ИмяИсточникаДанных%""';
														|en = 'Settings of the ""%ИмяИсточникаДанных%"" planning data source'");
...
Процедура ИмяШаблонаСКДПриИзменении(Элемент)
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Ошибка загрузки шаблона. Выберите другой шаблон.';
									|en = 'An error occurred while importing the template. Select another template.'");
  		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = НСтр("ru = 'Текущие настройки будут потеряны. Продолжить?';
						|en = 'Current settings will be lost. Continue?'");
...
Процедура ПредупредитьОВыборкеДанных(РекомендуемаяВыборкаПоПериоду = Неопределено)
	...
	ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Схема компоновки данных, возможно, не поддерживает выборку данных ""%1"".';
										|en = 'Data composition schema may not support selection of the ""%1"" data.'"),
	...
	ТекстПредупреждения = ТекстПредупреждения
							...
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Рекомендуется использовать: %1.';
										|en = 'It is recommended that you use: %1.'"), 
									РекомендуемыеВыборкиПоПериоду);
