...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если КоличествоТоваров > 0 Тогда
		Если Товары[КоличествоТоваров-1].Код - 99999 > ТекущийОбъект.МаксимальныйКод И ТекущийОбъект.МаксимальныйКод <> 0 Тогда
			...
			НСтр("ru = 'В табличной части ""Товары"" слишком много товаров. Измените максимальное количество товаров или уменьшите количество товаров к выгрузке при помощи отбора.';
				|en = 'The ""Goods"" table contains too many goods. Change the maximum quantity of goods or reduce quantity of goods to be exported using the filter.'"),,"Товары",,Отказ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ОтменаРедактирования Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			ОписаниеОшибки = НСтр("ru = 'Такой код уже назначен для номенклатуры %Номенклатура%';
									|en = 'Such code is already assigned for items %Номенклатура%'");
			ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Номенклатура%", """" + НайденнаяСтрока.Номенклатура + """"
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.Характеристика), " " + НСтр("ru = 'с характеристикой';
																				|en = 'with variant'") + " """ + НайденнаяСтрока.Характеристика + """", "")
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.Упаковка), " " + НСтр("ru = 'в упаковке';
																		|en = 'in packaging unit'") + " """ + НайденнаяСтрока.Упаковка + """", ""))+НСтр("ru = '. Обменять их местами?';
																																					|en = '. Counterchange them?'");
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Номенклатура) И ТекущиеДанные.Используется Тогда
			...
			ПоказатьВопрос(
				...
				НСтр("ru = 'Удаляемой номенклатуре, соответствующей заданному отбору после удаления будет заново назначены коды товаров. Продолжить?';
					|en = 'After deletion goods codes will be reassigned to the deleted products, which satisfy the specified filter. Continue?'"), 
	...
	Если Товары.Количество() > 0 Тогда
		...
		Иначе
			ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Нет данных для удаления';
														|en = 'There is no data for deletion'"));
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Нет данных для удаления';
													|en = 'There is no data for deletion'"));
...
//-- Локализация
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<штрихкод будет сгенерирован при выгрузке>';
																|en = '<barcode will be generated during exporting>'"));
...
Процедура ПрименитьИзмененияНаСервере(ТекущийОбъект)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Редактирование правила обмена с ККМ Offline';
										|en = 'Edit rules of exchange with offline cash register'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			СтрШаблон(НСтр("ru = 'Во время применения изменений произошла ошибка:
			                     |%1';
			                     |en = 'An error occurred when applying the changes:
...
Процедура УдалитьСвободныеКодыТоваровНаСервере()
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление свободных кодов ККМ Offline';
										|en = 'Delete available codes of offline cash register'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			СтрШаблон(НСтр("ru = 'Во время удаления свободных кодов ККМ Offline произошла ошибка:
			                     |%1';
			                     |en = 'An error occurred when deleting available codes of offline cash register:
			                     |%1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
