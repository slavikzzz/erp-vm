		...
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.Пользователь) Тогда
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Заполните поле Пользователь у подписанта %1';
					|en = 'Fill the User field for the %1 signer'"),
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//++ НЕ УТ
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСерверПереопределяемый.ПроверятьУникальностьРегистрацииВНалоговомОргане(ПроверятьУникальность);
	
	Если ПроверятьУникальность Тогда
		СуществующаяЗапись = РегистрацияВИФНС();
		Если ЗначениеЗаполнено(СуществующаяЗапись) И СуществующаяЗапись <> Объект.Ссылка Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Для данной организации уже существует запись с указанными кодом налогового органа';
											|en = 'A record with the specified tax authority code already exists for this company'")
...
Процедура ЗаполнитьРеквизитыНалоговогоОрганаПоКоду(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Код) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Поле ""Код"" не заполнено';
										|en = 'The ""Code"" field is not filled'"));
...
Функция ПредставлениеПодписанта()
	...
	Если ЗначениеЗаполнено(Объект.Подписанты) Тогда
		Представление = СтрШаблон(НСтр("ru = 'Несколько подписантов (%1)';
										|en = 'Multiple signers (%1)'"), Объект.Подписанты.Количество());
	...
	ИначеЕсли ТипЗнч(Объект.Представитель) = Тип("СправочникСсылка.Контрагенты")
		...
		Представление = СтрШаблон(НСтр("ru = 'Представитель: %1 (%2)';
										|en = 'Representative: %1 (%2)'"), Объект.УполномоченноеЛицо, Объект.Представитель);
	Иначе
		Представление = СтрШаблон(НСтр("ru = 'Представитель: %1';
										|en = 'Representative: %1'"), Объект.Представитель);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		Если ОписаниеОшибки = "НеУказаныПараметрыАутентификации" Тогда
			...
			ТекстВопроса = НСтр("ru = 'Для автоматического заполнения реквизитов контрагентов
				...
				|Подключиться сейчас?';
				|en = 'To fill counterparty bank details automatically,
