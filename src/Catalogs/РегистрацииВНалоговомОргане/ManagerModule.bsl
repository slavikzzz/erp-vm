...
// Создает новую регистрацию в налоговом органе по КПП и коду налогового органа, в случае ее отсутсвия.
// Возвращает ссылку на "Регистрация в налоговом органе".
// Параметры:
//	Организация - СправочникСсылка.Организации - структурная единица.
//	КПП - Строка - КПП обособленного подразделения.
//	КодНалоговогоОргана - Строка - код налоговой.
//	ПараметрыСоздания - Структура - параметры создания новой регистрации
//
// Возвращаемое значение:
//	СправочникСсылка.РегистрацииВНалоговомОргане - регистрация в налоговом органе.
//
Функция ПроверитьСоздатьРегистрациюВНалоговомОрганеПоКодуНО(Организация, КПП, КодНалоговогоОргана, ПараметрыСоздания) Экспорт
	Если Организация.Пустая() Тогда
		Возврат Справочники.РегистрацииВНалоговомОргане.ПустаяСсылка();
	КонецЕсли;
	Регистрация = РегистрацияВНалоговомОрганеПоКодуНО(Организация, КПП, КодНалоговогоОргана);
	Если Регистрация.Пустая() Тогда       
		РегистрацияОбъект = Справочники.РегистрацииВНалоговомОргане.СоздатьЭлемент();
		ДатаПостановкиНаУчет = Неопределено;
		Ссылка = Неопределено;
		ПараметрыСоздания.Свойство("ДатаПостановкиНаУчет", ДатаПостановкиНаУчет);
		ПараметрыСоздания.Свойство("Ссылка", Ссылка);     
		
		РегистрацияОбъект.Код			   = КодНалоговогоОргана;
		...
		РегистрацияОбъект.ДатаПостановкиНаУчет  = ДатаПостановкиНаУчет;
		Если Не Ссылка = Неопределено Тогда 
			РегистрацияОбъект.УстановитьСсылкуНового(Ссылка);
		КонецЕсли;
