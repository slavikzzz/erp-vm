...
&НаСервере
Процедура СкрытьОтветственныхБРО()
	Организация = ОбщегоНазначенияБЗК.ЗначениеСвойства(Параметры, "ОрганизацияДляСкрытияОтветственныхБРО");
	...
	Массив = Новый Массив;
	Физлицо1 = ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервераПереопределяемый.Руководитель(Организация);
	Физлицо2 = ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервераПереопределяемый.ГлБухгалтер(Организация);
	Если ЗначениеЗаполнено(Физлицо1) Тогда
		Массив.Добавить(Физлицо1);
	КонецЕсли;
	Если ЗначениеЗаполнено(Физлицо2) И Физлицо1 <> Физлицо2 Тогда
		Массив.Добавить(Физлицо2);
	КонецЕсли;
	Если Массив.Количество() > 0 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			...
			НСтр("ru = 'Не показывать руководителя и главного бухгалтера';
				|en = 'Do not show the supervisor and the chief accountant'"),
			Истина,
			РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Обычный,
			"7c014b39-10d8-11ee-8117-4cedfb43b11a");
	КонецЕсли;
КонецПроцедуры
