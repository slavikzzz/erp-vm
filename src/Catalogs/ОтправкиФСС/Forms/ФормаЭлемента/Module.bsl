...
Процедура ВыгрузитьПодтверждениеДоставкиВСФР(Команда)
	...
	ВАрхиве = ОбъектВАрхиве(Объект.Ссылка, "КвитанцияДоставкиВСФР");
	Если ВАрхиве Тогда 
	...
	Адрес = ПолучитьАдресФайлаПодтвержденияДоставкиВСФР(Объект.Ссылка, УникальныйИдентификатор);
...
Процедура ВыгрузитьПакетКвитанции(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ru = 'Зашифрованная_квитанция';
							|en = 'Зашифрованная_квитанция'") + "_" + Объект.ИдентификаторОтправкиНаСервере + ".xml");
...
Процедура ВыгрузитьПротокол(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ru = 'Протокол';
							|en = 'Протокол'") + "_" + Объект.ИдентификаторОтправкиНаСервере + ".html");
	...
	Если Объект.СтатусОтправки = Перечисления.СтатусыОтправки.Отправлен Тогда
		Если ПротоколЗаполнен(Объект.Ссылка) Тогда
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ru = 'Промежуточный протокол обработки получен';
															|en = 'Промежуточный протокол обработки получен'");
	...
	Иначе
		...
		ИначеЕсли Объект.СтатусОтправки = Перечисления.СтатусыОтправки.ПринятЕстьОшибки Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок = ?(ЭтоРеестрСтимулирующихВыплатМедицинскимИСоциальнымРаботникам,
				НСтр("ru = 'Протокол о сдаче отчета с ошибками получен';
					|en = 'Протокол о сдаче отчета с ошибками получен'"),
				НСтр("ru = 'Квитанция о сдаче отчета получена, протокол ошибок получен';
					|en = 'Квитанция о сдаче отчета получена, протокол ошибок получен'"));
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСиний;
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		ИначеЕсли Объект.СтатусОтправки = Перечисления.СтатусыОтправки.Сдан Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок =
				?(ЭтоДанныеЭЛН ИЛИ ЭтоПодтверждениеВидаДеятельности ИЛИ ЭтоРеестрСтимулирующихВыплатМедицинскимИСоциальнымРаботникам,
				НСтр("ru = 'Протокол о сдаче отчета получен';
					|en = 'Протокол о сдаче отчета получен'"), НСтр("ru = 'Квитанция о сдаче отчета получена';
																	|en = 'Квитанция о сдаче отчета получена'"));
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСиний;
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьМеню()
	...
	КнопкаВыгрузитьПодтверждениеДоставкиВСФР 	= Элементы.Найти("ФормаВыгрузитьПодтверждениеДоставкиВСФР");
	...
	Если Объект.СтатусОтправки = Перечисления.СтатусыОтправки.Отправлен Тогда
		КнопкаВыгрузитьПодтверждениеДоставкиВСФР.Видимость 	= Объект.ДоставленоВСФР;
...
Функция ПолучитьАдресФайлаПодписанногоПакета(ОтправкаСсылка, Знач УникальныйИдентификаторФормы)
	...
	ДанныеПодписанногоПакета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОтправкаСсылка, "ПодписанныйПакет").Получить();
...
Функция ПолучитьАдресФайлаПакета(ОтправкаСсылка, Знач УникальныйИдентификаторФормы)
	...
	ДанныеЗашифрованногоПакета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ОтправкаСсылка, "ЗашифрованныйПакет").Получить();
...
Функция ПолучитьАдресФайлаПротокола(ОтправкаСсылка, Знач УникальныйИдентификаторФормы)
	...
	СтрПротокол = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОтправкаСсылка, "Протокол").Получить();
...
Функция ПолучитьАдресФайлаКвитанции(ОтправкаСсылка, Знач УникальныйИдентификаторФормы)
	...
	ДанныеКвитанции = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОтправкаСсылка, "Квитанция").Получить();
...
Функция ПолучитьАдресФайлаПакетаКвитанции(ОтправкаСсылка, Знач УникальныйИдентификаторФормы)
	...
	ДанныеПакетаКвитанции = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОтправкаСсылка, "ПакетКвитанции").Получить();
...
&НаСервереБезКонтекста
Функция ПолучитьАдресФайлаПодтвержденияДоставкиВСФР(ОтправкаСсылка, Знач УникальныйИдентификаторФормы)
	
	ДанныеКвитанции = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОтправкаСсылка, "КвитанцияДоставкиВСФР").Получить();
	Возврат ПоместитьВоВременноеХранилище(ДанныеКвитанции, УникальныйИдентификаторФормы);
...
Функция ПротоколЗаполнен(ОтправкаСсылка)
	...
	СтрПротокол = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОтправкаСсылка, "Протокол").Получить();
	Возврат ЗначениеЗаполнено(СтрПротокол);
