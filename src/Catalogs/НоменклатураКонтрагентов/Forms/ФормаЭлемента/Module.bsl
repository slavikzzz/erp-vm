...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не Объект.Недействителен И Не Объект.ПометкаУдаления
		...
		Текст =  НСтр("ru = 'Необходимо заполнить наименование единицы измерения и упаковки.';
						|en = 'Fill in unit of measure and packaging unit name.'");
...
Процедура УстановитьСвойстваПереопределяемыхЭлементовФормы()
	...
	Заголовок = СтрШаблон(НСтр("ru = '%1: %2';
								|en = '%1: %2'"), МетаданныеСопоставления.НоменклатураКонтрагентаПредставлениеОбъекта, ПредставлениеЗаголовка);
...
Процедура ИзменитьНадписьВладельца()
	...
	Если ЗначениеЗаполнено(Объект.ВладелецНоменклатуры) Тогда
		Элементы.НадписьВладельца.Заголовок = Строка(Объект.ВладелецНоменклатуры);
	Иначе
		Элементы.НадписьВладельца.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = '<указать владельца>';
																				|en = '<specify owner>'"), , ЦветаСтиля.ВажныеДанныеЦветБЭД, , );
	КонецЕсли;
...
Процедура ЗаполнитьПоляНаОснованииПараметров(ПараметрыФормы)
	...
	Если ЗначениеЗаполнено(ВладелецНоменклатуры) Тогда
		...
		Объект.Владелец             = ВладелецНоменклатуры;
	КонецЕсли;
	
	НоменклатураИБ = Новый Структура;
	НоменклатураИБ.Вставить("Номенклатура"  , Неопределено);
	НоменклатураИБ.Вставить("Характеристика", Неопределено);
	НоменклатураИБ.Вставить("Упаковка"      , Неопределено);
