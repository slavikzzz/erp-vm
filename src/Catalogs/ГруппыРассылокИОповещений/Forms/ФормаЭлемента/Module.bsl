...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если УдалятьСозданныеСообщения И Объект.СрокХраненияСообщений = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Не указан срок хранения сообщений.';
								|en = 'Message storage period is not specified.'");
...
Функция ТекстПоясненияТипРассылки(Принудительная)
	...
	Если НЕ Принудительная Тогда
		ТекстПояснения = НСтр("ru = 'Рассылки и оповещения будут отправляться согласно настройкам подписок. Виды контактной информации,
		                             ...
		                             |en = 'Mailouts and notifications are sent according to subscription settings. Contact details kinds
		                             |specified in this form are used as the default values and can be 
		                             |changed upon registering subscriptions.'");
	Иначе
		ТекстПояснения = НСтр("ru = 'Рассылки и оповещения будут отправляться принудительно. Настройка подписок невозможна. В данной форме
		                             ...
		                             |en = 'Mailouts and notifications are sent regardless of subscription settings. You cannot configure subscriptions. In this form,
		                             |you need to configure message recipients and contact details kinds according to
		                             |which email addresses and recipient phone numbers are determined.'");
	КонецЕсли;
...
Процедура ЗаполнитьДоступныеУчетныеЗаписи()
	...
	Иначе
		...
		Элементы.ДекорацияОшибкаУчетнаяЗапись.Подсказка = НСтр("ru = 'У вас нет прав на чтение учетных записей электронной почты, обратитесь к администратору.';
																|en = 'You are not authorized to read email accounts. Please contact the administrator.'");
	КонецЕсли;
...
Процедура УстановитьЗаголовки()
	...
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	            ЭтотОбъект, "ОтправлятьПартнеру", НСтр("ru = 'на контактную информацию контрагента';
														|en = 'for the counterparty contact details'"), ИспользоватьПартнеровКакКонтрагентов);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	            ЭтотОбъект, "ВидКонтактнойИнформацииПартнераДляПисем", НСтр("ru = 'для контрагента';
																			|en = 'for counterparty'"), ИспользоватьПартнеровКакКонтрагентов);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	            ЭтотОбъект, "ВидКонтактнойИнформацииПартнераДляSMS", НСтр("ru = 'для контрагента';
																			|en = 'for counterparty'"), ИспользоватьПартнеровКакКонтрагентов);
