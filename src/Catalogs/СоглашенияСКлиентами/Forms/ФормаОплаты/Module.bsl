	...
	Если Объект.ДоступноВнешнимПользователям И Объект.ЭтапыГрафикаОплаты.Количество() = 0 Тогда
		...
		Если ИспользоватьУпрощеннуюСхемуОплаты Тогда
			ТекстОшибки = НСтр("ru = 'Соглашение доступно внешним пользователям. Необходимо заполнить хотя бы один из этапов';
								|en = 'Agreement is available for external users. Populate at least one stage'");
		...
		Иначе
			ТекстОшибки = НСтр("ru = 'Соглашение доступно внешним пользователям. Необходимо заполнить список ""Этапы графика оплаты""';
								|en = 'The agreement is available for external users. Payment milestones are required.'");
		...
		Если Не ЗначениеЗаполнено(ТекСтрока.ВариантОплаты) Тогда
			...
			Если НЕ ИспользоватьУпрощеннуюСхемуОплаты Тогда
				ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Вариант контроля""';
									|en = 'The ""Control option"" column is not filled in'");
			...
			Если Не ЗначениеЗаполнено(ТекСтрока.ПроцентПлатежа)
				...
				Если ИспользоватьУпрощеннуюСхемуОплаты Тогда
					ТекстОшибки = НСтр("ru = 'Для этапа должен быть указан процент платежа или процент залога за тару';
										|en = 'Percentage of payment and packaging deposit should be specified for the step'");
					ОбщегоНазначения.СообщитьПользователю(
						ТекстОшибки,
						,
			...
			Если Не ЗначениеЗаполнено(ТекСтрока.ПроцентПлатежа) Тогда
				...
				Если ИспользоватьУпрощеннуюСхемуОплаты Тогда
					ТекстОшибки = НСтр("ru = 'Не заполнен процент платежа';
										|en = 'Payment percentage is not filled in'");
					ОбщегоНазначения.СообщитьПользователю(
						ТекстОшибки,
						,
						?(ТекСтрока.ВариантОплаты = Перечисления.ВариантыКонтроляОплатыКлиентом.КредитСдвиг,"ПроцентПлатежаКредит","ПроцентПлатежаПредоплата"),
	...
	Если КоличествоЭтапов > 0 Тогда
		...
		Если Объект.ТребуетсяЗалогЗаТару
			...
			ТекстОшибки = НСтр("ru = 'Процент залога за тару по всем этапам должен быть равен 100%';
								|en = 'Percentage of the packaging deposit at all stages should be equal to 100%'");
			Если ИспользоватьУпрощеннуюСхемуОплаты Тогда
			...
			Для Каждого ЭтапГрафикаОплаты Из Объект.ЭтапыГрафикаОплаты Цикл
				...
				Если ЭтапГрафикаОплаты.ВариантОплаты <> Перечисления.ВариантыКонтроляОплатыКлиентом.КредитСдвиг
					...
					ТекстОшибки = НСтр("ru = 'Вариант оплаты ""%ВариантОплаты%"" нельзя использовать';
										|en = 'Cannot use: %ВариантОплаты%'") + Символы.ПС;
					ТекстОшибки = ТекстОшибки + НСтр("ru = 'при установленной хозяйственной операции ""%ХозяйственнаяОперация%""';
													|en = 'with a set ""%ХозяйственнаяОперация%"" business transaction'");
