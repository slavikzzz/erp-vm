		...
		Если ДиалогВыбораФайла.Выбрать() Тогда
					...
					Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
						Если ЧтениеXML.Имя <> "MobileApplicationVersionData" Тогда
							ТекстСообщения = НСтр("ru = 'Ошибка чтения данных из файла';
													|en = 'Error reading data from file'");
			...
			Если СтруктураПриложения.Версия <> Неопределено Тогда
				...
				ТекстВопроса = НСтр("ru = 'Загружаемая версия уже существует (%ИмяПриложения%, версия %НомерВерсии% ). Заменить?';
									|en = 'The version being imported already exists (%ИмяПриложения%, version %НомерВерсии% ). Do you want to overwrite it?'");
