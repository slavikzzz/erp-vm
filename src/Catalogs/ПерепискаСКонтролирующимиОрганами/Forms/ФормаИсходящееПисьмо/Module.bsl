...
&НаКлиенте
Функция ОтветНаТребованиеОтправляютКакПисьмо() Экспорт
	
	Результат = Объект.Тип = ПредопределенноеЗначение("Перечисление.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаСФНС")
		И СтрНайти(ВРег(Объект.Наименование), ВРег(НСтр("ru = 'ответ';
														|en = 'ответ'")))
		И СтрНайти(ВРег(Объект.Наименование), ВРег(НСтр("ru = 'Требован';
														|en = 'Требован'")));
...
&НаКлиенте
Процедура КомандаОтправить_ПослеОтказаОтСозданияПакета(Результат, ВходящийКонтекст) Экспорт
...
Процедура ДобавитьВложенияПослеВопросаИменаПересекаютсяПродолжитьСЗаменой(МассивФайлов, МассивНеподходящихФайлов, БылаЗамена = Ложь) Экспорт
	...
	Если МассивНеподходящихФайлов.Количество() > 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Среди выбранных файлов присутствовали такие, имена которых превышали 100 символов.
							...
							|en = 'Среди выбранных файлов присутствовали такие, имена которых превышали 100 символов.
							|Формат не допускает такой длины имени файлов вложений, поэтому следующие файлы не были добавлены:'");
	...
	Если БылаЗамена Тогда
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Есть недопустимые символы.';
				|en = 'Есть недопустимые символы.'"), , 
			НСтр("ru = 'Произведена замена недопустимых символов в имени файла.';
				|en = 'Произведена замена недопустимых символов в имени файла.'"), , 
...
Функция КонтекстЭДОСервер(ТекстСоощения = "")
	...
	Если КонтекстЭДОСервер = Неопределено Тогда 
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось получить текст вложения с именем
                                                                                                      |%1.';
                                                                                                      |en = 'Не удалось получить текст вложения с именем
                                                                                                      |%1.'"), ТекстСообщения);
...
&НаКлиенте
Процедура ПредложитьСоздатьОтветНаТребование()
	
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"КомандаОтправить_ПослеОтказаОтСозданияПакета", 
		ЭтотОбъект);
		
	Ошибка = ДокументооборотСКОКлиентСервер.ОшибкаНаОтветНаТребованиеФНСПисьмом();
		
	ДлительнаяОтправкаКлиент.ПоказатьОшибкуПроверкиПисьмаФНС(
		Объект.Ссылка,
...
Процедура КомандаОтправитьПроверкаТребованийКФайлам()
	...
	Если АнализПисем.Допустимо
		...
		Если ОтветНаТребованиеОтправляютКакПисьмо() И НЕ ИгнорироватьОшибку Тогда 
			ПредложитьСоздатьОтветНаТребование();
		Иначе
			НепостредственноОтправить();
		КонецЕсли;
...
&НаКлиенте
Процедура НепостредственноОтправить()
...
Процедура РазделениеПисем(АнализПисем = Неопределено)
	...
	Если НЕ АнализПисем.Допустимо Тогда
		...
		Ошибка = ДокументооборотСКОКлиентСервер.ОшибкаНаРазмерБольше5Мб(Размер);
	...
	Если ОтветНаТребованиеОтправляютКакПисьмо() И НЕ ИгнорироватьОшибку Тогда 
		ПредложитьСоздатьОтветНаТребование();
	Иначе
		РазбитьНаНесколькоПисем(АнализПисем);
	КонецЕсли;
		
КонецПроцедуры

&НаКлиенте
Процедура РазбитьНаНесколькоПисем(АнализПисем)
...
Процедура Подключаемый_РазделениеПисемОжидание()
	...
	Если ЗначениеЗаполнено(ПараметрыЦиклаРазделения.Ошибка) Тогда
		...
		ТекстОписания = НСтр("ru = 'Возникла ошибка при разбиении писем.';
						...
						+ СтрШаблон(НСтр("ru = 'Было создано %1 писем.';
										|en = 'Было создано %1 писем.'"), ПараметрыЦиклаРазделения.ТекущийСчетчик - 2) + Символы.ПС
						+ ТекстОписания;
		ФормаОперации = ФормаОжидания();
	...
	Если НЕ АнализПисем.Допустимо Тогда
		Если АнализПисем.КоличествоФайлов = 1 Тогда
			ТекстОписания = НСтр("ru = 'Выбранный файл превышает максимальный размер: %1 Мб.';
								|en = 'Выбранный файл превышает максимальный размер: %1 Мб.'");
		Иначе
			ТекстОписания = НСтр("ru = 'Один из файлов превышает максимальный размер: %1 Мб.';
								|en = 'Один из файлов превышает максимальный размер: %1 Мб.'");
		КонецЕсли;
	...
	ИначеЕсли НЕ АнализПисем.КоличествоПисем = 1 Тогда
		Если АнализПисем.ПревышениеКоличества > 0 Тогда
			ТекстОписания = СтрШаблон(НСтр("ru = 'Количество вложений превышено на %1. Разрешено: %2.';
											|en = 'Количество вложений превышено на %1. Разрешено: %2.'"), 
							АнализПисем.ПревышениеКоличества, АнализПисем.МаксКоличество);
		Иначе
		...
		ТекстОписания = ТекстОписания + " " + НСтр("ru = '<a href = ""Разделить"">Разбить на несколько писем</a>';
													|en = '<a href = ""Разделить"">Разбить на несколько писем</a>'");
...
Процедура ЗапросСправкиОбОткрытыхСчетах(Команда)
	...
	ЗаполнитьПоШаблону(НСтр("ru = 'Запрос справки об открытых счетах';
							|en = 'Запрос справки об открытых счетах'"));
...
Процедура ЗаполнитьПоШаблону(Шаблон)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Для заполнения по шаблону необходимо указать организацию';
														|en = 'Для заполнения по шаблону необходимо указать организацию'"),,"Отправитель");
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Текст = НСтр("ru = 'Письмо по данному шаблону должно направляться в ФНС. Сменить получателя?';
					|en = 'Письмо по данному шаблону должно направляться в ФНС. Сменить получателя?'");
