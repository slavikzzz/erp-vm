...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Тип = Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаССФР Тогда
		Содержание = ПолучитьСодержимоеРассылки(Объект.Идентификатор);
		
		Элементы.Отправитель.Видимость			= Ложь;
		
		КодСФР = ПолучитьКодПодчиненностиСФР(Объект.Организация);
		Шаблон									= НСтр("ru = 'СФР %1';
														|en = 'СФР %1'");
		ОтправительТекстом						= СтрШаблон(Шаблон, КодСФР);
		...
		Если Не Результат.Выполнено Тогда
			ПоказатьПредупреждение(,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось скачать вложение
                                                                              |%1';
                                                                              |en = 'Не удалось скачать вложение
                                                                              |%1'"), ИмяФайла));
		Иначе
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронный документооборот с контролирующими органами.Анализ текста письма рассылки';
				|en = 'Электронный документооборот с контролирующими органами.Анализ текста письма рассылки'", 
...
Процедура УправлениеФормой()
	...
	Если Объект.Тип = Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаССФР Тогда
		Элементы.ОтветитьПисьмом.Видимость		= Ложь;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПолучитьСодержимоеРассылки(Идентификатор)
	
	Возврат Справочники.ПерепискаСКонтролирующимиОрганами.ПолучитьСодержимоеРассылки(Идентификатор);
	
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьКодПодчиненностиСФР(Организация)
	
	КодПодчиненностиСФР = ЭлектронныйДокументооборотСФСС.ПолучитьКодОрганаСФР(Организация);
	Возврат КодПодчиненностиСФР;
