...
Функция РазделитьПисьма(ПараметрыОперации, АдресХранилища = "") Экспорт
	...
	Попытка
		...
		Если ИсходноеПисьмо Тогда
			Если СтрНайти(РеквизитыПисьма.Наименование, "(" + НСтр("ru = 'Часть';
																	|en = 'Часть'"),) = 0 Тогда
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru = 'Электронный документооборот с контролирующими органами. Разбиение писем: ';
							|en = 'Электронный документооборот с контролирующими органами. Разбиение писем: '", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПечатнаяФормаОбращенияФНС_05_04_Подвал(Письмо, ТабДокумент, Бланк) Экспорт
	...
	Доверенность = РегистрацияВНОСервер.ДанныеРегистрации(ЭлРегистрацияВИФНС).Доверенность;
		
	ПодписаноПредставителем =
		ЗначениеЗаполнено(ЭлРегистрацияВИФНС) 
		И ЗначениеЗаполнено(Доверенность);
		
	Если ПодписаноПредставителем Тогда
...
Функция XDTOПисьма(Письмо)
	...
	Если Сообщения.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось получить содержание письма';
								|en = 'Не удалось получить содержание письма'");
	КонецЕсли;
...
Процедура ЗаполнитьНаименованиеИКодКонтролирующегоОргана(Письмо, Бланк, ОбластьНаименований)
	...
	РеквизитыПереписки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Письмо, "Тип", Истина);
	
	Если НЕ РеквизитыПереписки.Тип = Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаССФР Тогда
	...
	Если РеквизитыПереписки.Тип = Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаССФР Тогда
		СвойстваПереписки.КодКонтролирующегоОргана = ЭлектронныйДокументооборотСФСС.ПолучитьКодОрганаСФР(СвойстваПереписки.Организация);
		НаименованиеОргана = "СФР";
	КонецЕсли;
	...
	Если (ТипЗнч(Орган) = Тип("СправочникСсылка.НалоговыеОрганы")
		...
		И СтрНайти(НаименованиеОргана, СвойстваПереписки.КодКонтролирующегоОргана) = 0
		ИЛИ РеквизитыПереписки.Тип = Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаССФР Тогда
...
Процедура ЗаполнитьКонтактыОрганизации(Письмо, ТабДокумент, Бланк, Область)
	...
	Если ЗначениеЗаполнено(СведенияОбОрганизации.ИННЮЛ) Тогда
		...
		НадписьИННиКПП = НСтр("ru = 'ИНН ';
								|en = 'ИНН '") + СведенияОбОрганизации.ИННЮЛ;
		Если ЗначениеЗаполнено(СведенияОбОрганизации.КППЮЛ) Тогда 
			 НадписьИННиКПП = НадписьИННиКПП + НСтр("ru = ' КПП ';
													|en = ' КПП '") + СведенияОбОрганизации.КППЮЛ;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(СведенияОбОрганизации.АдрЮР_XML) Тогда
		Адрес = ОбработкаЗаявленийАбонента.ПредставлениеАдресаИзДанныхОрганизации(СведенияОбОрганизации.АдрЮР_XML);
	Иначе
		Адрес = ОбработкаЗаявленийАбонента.ПредставлениеАдресаИзДанныхОрганизации(СведенияОбОрганизации.АдрМЖ_XML);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СведенияОбОрганизации.ТелОрганизации)
		...
		Область.Параметры["Телефон" + Постфикс] = НСтр("ru = 'тел: ';
														|en = 'тел: '") + СведенияОбОрганизации.ТелОрганизации;
...
Процедура ЗаполнитьЗаголовокИТекст(Письмо, ТабДокумент, Бланк)
	...
	РеквизитыПисьма = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Письмо, "Тип, Идентификатор", Истина);
	
	ОбластьЗаголовок = Бланк.ПолучитьОбласть("Заголовок");
	Если РеквизитыПисьма.Тип = Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаССФР Тогда
		ОбластьЗаголовок.Параметры.Заголовок = Лев(Письмо.Наименование, 50) + " [...]";
	Иначе
	...
	ВвестиТекстПисьма(Письмо, ТабДокумент, Бланк, РеквизитыПисьма);

КонецПроцедуры

Процедура ВвестиТекстПисьма(Письмо, ТабДокумент, Бланк, РеквизитыПисьма = Неопределено)
	
	Содержание = Письмо.Содержание;
	Если НЕ РеквизитыПисьма = Неопределено Тогда
		Если РеквизитыПисьма.Тип = Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаССФР Тогда
			Содержание = ПолучитьСодержимоеРассылки(РеквизитыПисьма.Идентификатор);
		КонецЕсли;
	КонецЕсли;
	
	Для НомерСтроки = 1 По СтрЧислоСтрок(СокрЛП(Содержание)) Цикл
		ОбластьТекстПисьма = Бланк.ПолучитьОбласть("ТекстПисьма");
		ОбластьТекстПисьма.Параметры.ТекстПисьма = СтрПолучитьСтроку(СокрЛП(Содержание), НомерСтроки);
...
Функция ПолучитьСодержимоеРассылки(Идентификатор) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СодержимоеСообщенийПереписки.Содержание КАК Содержание
	|ИЗ
	|	РегистрСведений.СодержимоеРассылокКонтролирующихОрганов КАК СодержимоеСообщенийПереписки
	|ГДЕ
	|	СодержимоеСообщенийПереписки.Идентификатор = &Идентификатор
	|	И СодержимоеСообщенийПереписки.Тип = &Тип";
	
	Запрос.УстановитьПараметр("Идентификатор", Идентификатор);
	Запрос.УстановитьПараметр("Тип", Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаССФР);
	
	Результат = Запрос.Выполнить();
	Если Результат.Пустой() Тогда
		Возврат "Не удалось получить содержимое рассылки";
	Иначе
		Выгрузка = Результат.Выгрузить();
		
		Возврат Выгрузка[0].Содержание.Получить();
	КонецЕсли;
