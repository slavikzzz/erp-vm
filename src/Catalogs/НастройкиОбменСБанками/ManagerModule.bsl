...
Процедура ЗаполнитьВерсиюФормата(Параметры) Экспорт
	...
	Исключение
		...
		Операция = НСтр("ru = '1С:ДиректБанк: Заполнение версии формата.';
						|en = '1C:DirectBank: Filling in format version.'");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось обработать справочник: %Регистратор% по причине: %Причина%';
									|en = 'Cannot process the catalog: %Регистратор% due to: %Причина%'");
	...
	Если ОбработанныхОбъектов = 0 И ПроблемныхОбъектов <> 0 Тогда
		ШаблонСообщения = НСтр("ru = 'Не удалось обработать некоторые настройки обмена с банками (пропущены): %1';
								|en = 'It was not possible to process some exchange settings with banks (skipped): %1'");
	...
	Иначе
		...
		ШаблонСообщения = НСтр("ru = 'Обработана очередная порция настроек обмена с банками: %1';
								|en = 'The next portion of exchange settings with banks was processed: %1'");
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В процедуре Справочники.НастройкиОбменСБанками.ПереЗаполнитьНастройкиОбменСБанками() не удалось обработать элемент по причине:
					|%1';
					|en = 'The Справочники.НастройкиОбменСБанками.ПереЗаполнитьНастройкиОбменСБанками() procedure cannot process the item due to:
					|%1'"),
	...
	Если ПроблемныхОбъектов > 0 Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуре Справочники.НастройкиОбменСБанками.ПереЗаполнитьНастройкиОбменСБанками() не удалось обработать часть элементов. В %1 из %2 возникли ошибки';
				|en = 'The Справочники.НастройкиОбменСБанками.ПереЗаполнитьНастройкиОбменСБанками() procedure cannot process some items. Errors occurred in %1 out of %2'"),
				ПроблемныхОбъектов,
				ОбъектовОбработано);
		ВызватьИсключение ТекстСообщения;
	...
	Иначе
		...
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура Справочники.НастройкиОбменСБанками.ПереЗаполнитьНастройкиОбменСБанками() обработала очередную порцию элементов: %1 элементов';
					|en = 'The Справочники.НастройкиОбменСБанками.ПереЗаполнитьНастройкиОбменСБанками() procedure processed the next batch of items: %1 items'"), ОбъектовОбработано));
...
Функция ПолучитьНастройкиОбмена(ПараметрыПолученияНастроек)
	...
	ВидОперации = НСтр("ru = 'Получение настроек с сервера банка';
						|en = 'Receiving settings from the bank server'");
	...
	Если Не Результат.Статус Тогда
		Если ЗначениеЗаполнено(Результат.КодСостояния) Тогда
			Шаблон = НСтр("ru = 'Ошибка загрузки настроек с сервера банка.
								|Код ошибки: %1.
								|%2';
								|en = 'An error occurred when importing settings from the bank server.
								|Error code: %1.
								|%2'");
	...
	Исключение
		Если Не ПараметрыПолученияНастроек.ПробнаяОперация Тогда
			...
			ТекстСообщения = НСтр("ru = 'Произошла ошибка при получении настроек из банка';
									|en = 'An error occurred when receiving settings from bank'");
