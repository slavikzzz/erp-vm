...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(Справка1) И НЕ ПустаяСтрока(НомерСправки1) Тогда
		Если ВРег(СокрЛП(НомерСправки1)) <> ВРег(СокрЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Справка1, "РегистрационныйНомер"))) Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Номер справки 1 не совпадает с выбранной справкой.';
														|en = 'Номер справки 1 не совпадает с выбранной справкой.'"),,,, Отказ);
		КонецЕсли;
	КонецЕсли;
		...
		Если МассивСтрок.Количество() > 1 Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Обнаружено дублирование диапазонов.';
					|en = 'Обнаружено дублирование диапазонов.'"),
				Ссылка,
		...
		Если НачальныйНомер > КонечныйНомер Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Начальный номер диапазона не может быть больше конечного.';
					|en = 'Начальный номер диапазона не может быть больше конечного.'"),
				Ссылка,
	...
	Если ТаблицаДиапазонов.Количество() > 1 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Диапазоны номеров акцизных марок должны принадлежать одному виду алкогольной продукции.';
													|en = 'Диапазоны номеров акцизных марок должны принадлежать одному виду алкогольной продукции.'"),,,, Отказ);
	КонецЕсли;
