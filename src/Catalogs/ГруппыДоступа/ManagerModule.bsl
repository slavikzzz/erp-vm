	...
	Попытка
		...
		Иначе
			ГруппаДоступаПоНаименованию = ГруппаДоступаПоНаименованию(
				НСтр("ru = 'Администраторы';
					|en = 'Administrators'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	Элемент.Наименование = НСтр("ru = 'Администраторы';
								|en = 'Administrators'", ОбщегоНазначения.КодОсновногоЯзыка());
	   ...
	   И (Не ОбщегоНазначения.ИнформационнаяБазаФайловая()
		...
		НаименованиеЗадания =
			НСтр("ru = 'Обновление вспомогательных данных групп доступа';
				|en = 'Updating auxiliary data of access groups'",
				ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Завершено Тогда
			...
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Фоновое задание ""%1"" не завершилось.';
						|en = 'Background job ""%1"" did not complete.'"), НаименованиеЗадания);
			КонецЕсли;
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		...
		Если ТипЗнч(Результат) <> Тип("Структура") Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Фоновое задание ""%1"" не вернуло результат.';
					|en = 'Background job ""%1"" did not return the result.'"), НаименованиеЗадания);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	Иначе
