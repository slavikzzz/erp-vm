	...
	Для Каждого ТекущаяСтрока Из ДеревоПользователейСтроки Цикл
		...
		Если Не ЗначениеЗаполнено(Участник) Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				УточнитьСообщение(НСтр("ru = 'Пользователь не выбран.';
										|en = 'The user is not selected.'"), Участник),
				"ПользователиГруппы",
				НомерСтроки,
				УточнитьСообщение(НСтр("ru = 'Пользователь в строке %1 не выбран.';
										|en = 'The user is not selected in line #%1.'"), Участник));
		КонецЕсли;
		Если ЗначениеЗаполнено(СрокДействия) И СрокДействия <= НачалоДня Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				"ПользователиГруппы[%1].Пользователь",
				УточнитьСообщение(НСтр("ru = 'Срок участия %2 должен быть до завтра или более.';
										|en = 'Membership of %2 should expire tomorrow or later.'"), Срок),
				"ПользователиГруппы",
				НомерСтроки,
				УточнитьСообщение(НСтр("ru = 'Срок участия %2 в строке %1 должен быть до завтра или более.';
										|en = 'Membership of %2 (line %1) should expire tomorrow or later.'"), Срок));
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Участник) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОбновитьНазначение()
	...
	Элементы.Пользователи.Подсказка = НСтр("ru = 'Допустимые участники:';
											|en = 'Allowed members:'") + " " + НазначениеПредставление;
