...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоверительЮЛ_РоссийскаяОрганизация = (СтруктураДанных = Неопределено
		ИЛИ СтруктураДанных.ДоверительЮЛ_РоссийскаяОрганизация ИЛИ ЭтоКорневаяДоверенность = Ложь);
...
Процедура УправлениеФормой(Форма)
	...
	ЭтоВерсия5_02 = (Форма.ВерсияФормата = "5.02");
...
Функция СохранениеВозможно()
	...
	ЭтоВерсия5_02 = (ВерсияФормата = "5.02");
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ДоверительФЛ_КодВыдДок) И СтрДлина(СокрЛП(ДоверительФЛ_КодВыдДок)) <> 7 Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Длина кода подразделения не 7 символов.';
															|en = 'Длина кода подразделения не 7 символов.'"),,
				"ДоверительФЛ_Удостоверение",, Отказ);
		КонецЕсли;
