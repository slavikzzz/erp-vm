...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоНовоеЗаполнение 		= Ложь;
	
	Если Параметры.Ключ.Пустая() Тогда
		Объект.ВерсияФормата = ?(ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило(
			"Отпр.МЧД ФНС РР вер.5.02"), "5.02", "5.01");
		
		Если НЕ ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
			ЭтоНовоеЗаполнение 			= Истина;
		...
		Объект.СведенияОбИнформационнойСистеме 				= "https://m4d.nalog.gov.ru/";
	...
	ОтправкаРазрешена = (Объект.ВерсияФормата = "5.02"
		ИЛИ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.МЧД ФНС РР вер.5.01", Истина));
...
Процедура НомерДоверенностиПриИзменении(Элемент)
	...
	ЭтоВерсия5_02 = (Объект.ВерсияФормата = "5.02");
...
Процедура ПредставительЯвляетсяСотрудникомПриИзменении(Элемент)
	...
	Если НЕ ОбновитьПредставителей(ЭтаФорма, Истина, Истина) И НЕ Доверитель_ЮридическоеЛицо
		И НЕ Представитель_ЮридическоеЛицо И НЕ Объект.ПредставительЯвляетсяСотрудником
		И ТипЗнч(Объект.Представитель) = Тип("Строка") Тогда
...
Процедура Отправить(Команда)
	...
	ИначеЕсли НЕ ЭтоВерсия5_02 И ЗначениеЗаполнено(Объект.НомерДоверенности)
		И СтрДлина(СокрЛП(Объект.НомерДоверенности)) <> 36 Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Номер доверенности не является уникальным идентификатором из 36 символов, возможен отказ
					   ...
					   |en = 'Номер доверенности не является уникальным идентификатором из 36 символов, возможен отказ
					   |налогового органа при сдаче отчетности по машиночитаемой доверенности, продолжить отправку?'"),
			РежимДиалогаВопрос.ДаНет,,
			КодВозвратаДиалога.Нет);
...
Процедура Выгрузить(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ВыгрузитьПослеПодтвержденияВыгрузкиПодписи",
		ЭтотОбъект, ДополнительныеПараметры);
	Если ПодписьЗагружена ИЛИ МассивФайлов <> Неопределено И МассивФайлов.Количество() >= 2 Тогда
		НесколькоПодписей = МассивФайлов <> Неопределено И МассивФайлов.Количество() >= 3;
		ПоказатьВопрос(
			ОписаниеОповещения,
			?(НесколькоПодписей,
				НСтр("ru = 'Будут выгружены файлы машиночитаемой доверенности и подписей доверителей, продолжить?';
					|en = 'Будут выгружены файлы машиночитаемой доверенности и подписей доверителей, продолжить?'"),
...
Процедура Инициализация()
	...
	Если ИспользуетсяЗагрузкаПодписи И ЗначениеЗаполнено(Объект.Ссылка)
		И ЗначениеЗаполнено(Объект.ИмяФайлаВыгрузки) Тогда
...
Функция ПрорисоватьСтатус(ПрочитатьФорму = Ложь)
	...
	ИначеЕсли Объект.Статус =
		...
		Элементы.НаименованиеЭтапа.Заголовок = НСтр("ru = 'Ожидание подтверждения в ФНС';
													|en = 'Ожидание подтверждения в ФНС'");
		Элементы.КомментарийЭтапа.Заголовок = "";
		Элементы.БлокСостоянияОтправки.ЦветФона = ЦветаСтиля.ЦветФонаТекущейОтправки; // желтый
	...
	ИначеЕсли Объект.Статус =
		...
		Элементы.НаименованиеЭтапа.Заголовок = НСтр("ru = 'Отказ в регистрации в ФНС';
													|en = 'Отказ в регистрации в ФНС'");
		Элементы.КомментарийЭтапа.Заголовок = ?(ЗначениеЗаполнено(Объект.ТекстОшибки), Объект.ТекстОшибки,
			НСтр("ru = 'Исправьте ошибки и отправьте доверенность еще раз';
				|en = 'Исправьте ошибки и отправьте доверенность еще раз'"));
		Элементы.БлокСостоянияОтправки.ЦветФона = ЦветаСтиля.ЦветФонаОшибкиОтправки; // красный
	...
	ИначеЕсли Объект.Статус =
		...
		Элементы.КомментарийЭтапа.Заголовок = ?(ЗначениеЗаполнено(Объект.ТекстОшибки), Объект.ТекстОшибки,
			НСтр("ru = 'Проверьте реквизиты доверенности';
				|en = 'Проверьте реквизиты доверенности'"));
		Элементы.БлокСостоянияОтправки.ЦветФона = ЦветаСтиля.ЦветФонаОшибкиОтправки; // красный
	...
	ИначеЕсли Объект.Статус =
		...
		Элементы.НаименованиеЭтапа.Заголовок = НСтр("ru = 'Зарегистрировано, срок действия не наступил';
													|en = 'Зарегистрировано, срок действия не наступил'");
		Элементы.КомментарийЭтапа.Заголовок = "";
		Элементы.БлокСостоянияОтправки.ЦветФона = ЦветаСтиля.ЦветФонаОшибкиОтправки; // красный
	...
	ИначеЕсли Объект.Статус =
		...
		Элементы.НаименованиеЭтапа.Заголовок = НСтр("ru = 'Отправлено заявление на отзыв';
													|en = 'Отправлено заявление на отзыв'");
		Элементы.КомментарийЭтапа.Заголовок = "";
		Элементы.БлокСостоянияОтправки.ЦветФона = ЦветаСтиля.ЦветФонаТекущейОтправки; // желтый
	...
	ИначеЕсли Объект.Статус =
		...
		Элементы.НаименованиеЭтапа.Заголовок = НСтр("ru = 'Ожидание подтверждения отзыва в ФНС';
													|en = 'Ожидание подтверждения отзыва в ФНС'");
		Элементы.КомментарийЭтапа.Заголовок = "";
		Элементы.БлокСостоянияОтправки.ЦветФона = ЦветаСтиля.ЦветФонаТекущейОтправки; // желтый
	...
	ИначеЕсли Объект.Статус =
		...
		Элементы.НаименованиеЭтапа.Заголовок = НСтр("ru = 'Отказ в регистрации отзыва в ФНС';
													|en = 'Отказ в регистрации отзыва в ФНС'");
		Элементы.КомментарийЭтапа.Заголовок = "";
		Элементы.БлокСостоянияОтправки.ЦветФона = ЦветаСтиля.ЦветФонаОшибкиОтправки; // красный
	...
	ИначеЕсли Объект.Статус =
		...
		Элементы.НаименованиеЭтапа.Заголовок = НСтр("ru = 'Зарегистрировано, ошибка отзыва';
													|en = 'Зарегистрировано, ошибка отзыва'")
	...
	Элементы.Отправить.Видимость = НЕ УведомлениеОтправлено И ОтправкаРазрешена;
	...
	Элементы.Отозвать.Заголовок = ?(Объект.Статус =
		...
		НСтр("ru = 'Показать заявление на отзыв';
			|en = 'Показать заявление на отзыв'"), НСтр("ru = 'Отозвать';
														|en = 'Отозвать'"));
...
&НаСервере
Процедура ПрорисоватьКнопкиСопоставления()
	
	ПроверяемыеМЧДФНС = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.Ссылка);
	
	СопоставлениеДоступно = НЕ Параметры.Ключ.Пустая()
		И (Объект.Статус = Перечисления.СтатусыМашиночитаемойДоверенностиКО.ВРаботе
		ИЛИ Объект.Статус = Перечисления.СтатусыМашиночитаемойДоверенностиКО.Зарегистрировано
		ИЛИ Объект.Статус = Перечисления.СтатусыМашиночитаемойДоверенностиКО.ОтправленоЗаявлениеНаОтзыв
		ИЛИ Объект.Статус = Перечисления.СтатусыМашиночитаемойДоверенностиКО.ОшибкаОтзыва
		ИЛИ Объект.Статус = Перечисления.СтатусыМашиночитаемойДоверенностиКО.ДатаНачалаДействияНеНаступила
		ИЛИ Объект.Статус = Перечисления.СтатусыМашиночитаемойДоверенностиКО.Подписано
		ИЛИ Объект.Статус = Перечисления.СтатусыМашиночитаемойДоверенностиКО.ОжиданиеПодтвержденияОтзыва
		ИЛИ Объект.Статус = Перечисления.СтатусыМашиночитаемойДоверенностиКО.ОтказВРегистрацииОтзыва);
	...
	НедействительныеДоверенности = Неопределено;
...
Процедура УправлениеФормой(Форма)
	...
	ЭтоВерсия5_02 = (Форма.Объект.ВерсияФормата = "5.02");
...
Процедура ВвестиРеквизитыДоверителяПослеВвода(
	...
	Если Элемент = Элементы.ДекорацияРеквизитыДоверяющегоРодительскойДоверенности
		...
		Если СтрокиДоверенностей.Количество() = 0 Тогда
			СтрокаДоверенности = Объект.СтрокиДоверенностей.Добавить();
...
Процедура ПодставитьРеквизитыПредставителя()
	...
	ИначеЕсли НЕ ПодписьЗагружена И ТипЗнч(Объект.Представитель) = Тип("СправочникСсылка.Контрагенты")
		...
		Если СвойстваКонтрагента.ЭтоЮридическоеЛицо = Истина
			ИЛИ СвойстваКонтрагента.ЭтоЮридическоеЛицо = Ложь Тогда
	...
	Если ДополнительныеПараметры.ОчиститьПредставителей Тогда
		...
		Если ДополнительныеПараметры.ДобавитьПредставителя Тогда
			...
			Если ПредставительЗаполнен Тогда
				...
				СтрокаПредставителя.ПредставительЯвляетсяСотрудником = Объект.ПредставительЯвляетсяСотрудником;
...
Функция ОбновитьПредставителей(
	...
	Результат = Форма.Доверитель_ЮридическоеЛицо И НЕ Форма.Представитель_ЮридическоеЛицо
		И Форма.Объект.ПредставительЯвляетсяСотрудником;
	...
	Если ЭтоВерсия5_02 И НЕ ОбновитьТолькоПредставителяВРеквизитахДоверенности Тогда
		Для каждого ПредставительСтрока Из Форма.Объект.Представители Цикл
			Если ПредставительСтрока.ПредставительЯвляетсяСотрудником Тогда
				...
				ВставитьПредставителяВТаблицу(Форма, ПредставительСтрока);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура НомерРодительскойДоверенностиНачалоВыбораПослеВыбора(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(РеквизитыРодительскойДоверенности.НомерКорневойДоверенности) Тогда
		...
		Если РеквизитыРодительскойДоверенности.РеквизитыКорневойДоверенности <> Неопределено Тогда
			ПодставитьРеквизитыКорневойДоверенности(РеквизитыРодительскойДоверенности.РеквизитыКорневойДоверенности);
		КонецЕсли;
	...
	Иначе
		...
		ПодставитьРеквизитыКорневойДоверенности(РеквизитыРодительскойДоверенности);
	КонецЕсли;
...
Процедура НомерКорневойДоверенностиНачалоВыбораПослеВыбора(Результат, ДополнительныеПараметры) Экспорт
	...
	Объект.КорневаяДоверенность = Результат;
	Объект.НомерКорневойДоверенности = РеквизитыКорневойДоверенности.НомерДоверенности;
...
Функция ЗначенияРеквизитовМЧД(СсылкаНаОбъект, ПрочитатьКорневуюМЧД = Ложь)
	...
	ИменаРеквизитов = "ВерсияФормата, НомерДоверенности, РегистрационныйНомерДоверенности, ПередовериеВозможно, "
		...
		+ "ДоверительФЛ_ОГРН, ДоверительФЛ_СНИЛС, ДоверительФЛ_Гражданство, ДоверительФЛ_ДатаРождения, "
...
Процедура ОтправитьПослеОтправки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено
		И (Результат.Свойство("СтатусТранзакции") И ЗначениеЗаполнено(Результат.СтатусТранзакции)
		ИЛИ Результат.Свойство("ИмяФайлаВыгрузки") И ЗначениеЗаполнено(Результат.ИмяФайлаВыгрузки)) Тогда
...
Процедура ОбновитьОтправкуПослеОбновления(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено И
		(Результат.Свойство("СтатусДоверенности") И ЗначениеЗаполнено(Результат.СтатусДоверенности)
		ИЛИ Результат.Свойство("ОтправкаСсылка") И ЗначениеЗаполнено(Результат.ОтправкаСсылка)) Тогда
...
Процедура ВыгрузитьПослеПодтвержденияВыгрузкиПодписи(Результат, ДополнительныеПараметры) Экспорт
	...
	РезультатВыгрузки = ДополнительныеПараметры.МассивФайлов;
