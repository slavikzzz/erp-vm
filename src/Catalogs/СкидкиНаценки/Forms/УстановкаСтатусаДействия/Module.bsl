...
Процедура Применить(Команда)
	...
	Если Не ЗначениеЗаполнено(Статус) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не заполнено поле ""Статус""';
				|en = '""Status"" is required'"), ,"Статус",,Отказ);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ДатаНачала) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не заполнено поле ""Дата начала""';
				|en = '""Start date"" is required'"), ,"ДатаНачала",,Отказ);
	КонецЕсли;
	...
	Если МаксимальнаяДатаОпределена Тогда
		...
		Если Не ЗначениеЗаполнено(ДатаОкончания)
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Не заполнено поле ""Дата окончания""';
					|en = '""End date"" is required'"), ,"ДатаОкончания",,Отказ);
		КонецЕсли;
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Дата окончания действия не может быть позже %1';
						|en = 'The action expiration date cannot be later than %1'"),
