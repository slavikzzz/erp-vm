...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ВариантОтбораНоменклатуры = ПредопределенноеЗначение("Перечисление.ВариантыОтбораНоменклатурыДляРасчетаСкидокНаценок.ОтборКомпоновкиДанных")
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не установлен отбор номенклатуры';
				|en = 'Item filter is not set'"),,"КомпоновщикНастроекОтборПоНоменклатуреПредставление",, Отказ);
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("СпособПредоставленияПриИзмененииЗавершение", ЭтотОбъект), 
			СтрШаблон(
				НСтр("ru = 'Добавить условие ""Карта лояльности ""%1"" не зарегистрирована?""';
					|en = 'Add condition ""Is loyalty card ""%1"" not registered?""'"),
...
Процедура ИнформацияОДействииСкидокОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "Изменить" Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Для изменения статуса действия скидка (наценка) должна быть записана.';
													|en = 'Discount (markup) must be saved to change the action status.'"), Кнопки,,,НСтр("ru = 'Скидка (наценка) не записана';
																																				|en = 'Discount (markup) is not saved'"));
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Для изменения статуса действия скидка (наценка) должна быть записана.';
				|en = 'Discount (markup) must be saved to change the action status.'"),
...
Функция АвтоНаименование(Форма)
	...
	Если Объект.СпособПредоставления = ПредопределенноеЗначение("Перечисление.СпособыПредоставленияСкидокНаценок.Процент") Тогда
		...
		Иначе
			СтрокаНаименования = СтрШаблон(НСтр("ru = '%1%3, бонусная программа %2';
												|en = '%1%3, bonus program %2'"), Объект.ЗначениеСкидкиНаценки, Объект.БонуснаяПрограммаЛояльности, "%");
		КонецЕсли;
	...
	ИначеЕсли Объект.СпособПредоставления = ПредопределенноеЗначение("Перечисление.СпособыПредоставленияСкидокНаценок.ВидЦены") Тогда
		...
		СтрокаНаименования = СтрШаблон(НСтр("ru = 'Вид цены: %1';
											|en = 'Price type: %1'"), Объект.ВидЦены);
	...
	ИначеЕсли Объект.СпособПредоставления = ПредопределенноеЗначение("Перечисление.СпособыПредоставленияСкидокНаценок.Сообщение") Тогда
		...
		СтрокаНаименования = СтрШаблон(НСтр("ru = 'Сообщение: %1';
											|en = 'Message: %1'"), Объект.ТекстСообщения);
	...
	ИначеЕсли Объект.СпособПредоставления = ПредопределенноеЗначение("Перечисление.СпособыПредоставленияСкидокНаценок.КартаЛояльности") Тогда
		...
		СтрокаНаименования = СтрШаблон(НСтр("ru = 'Карта лояльности: %1';
											|en = 'Loyalty card: %1'"), Объект.ВидКартыЛояльности);
		...
		Если ЗначениеЗаполнено(Объект.СегментПодарков) Тогда
			СтрокаНаименования = СтрШаблон(
				НСтр("ru = '%1 ед. из сегмента %2 в подарок';
					|en = '%1 units as a gift from segment %2'"),
				Объект.ЗначениеСкидкиНаценки,
				Объект.СегментПодарков);
		Иначе
			СтрокаНаименования = СтрШаблон(
				НСтр("ru = '%1 ед. из документа в подарок';
					|en = '%1 units as a gift from the document'"),
				Объект.ЗначениеСкидкиНаценки);
		КонецЕсли;
	...
	ИначеЕсли Объект.СпособПредоставления = ПредопределенноеЗначение("Перечисление.СпособыПредоставленияСкидокНаценок.ОкруглениеСуммы") Тогда
		...
		СтрокаНаименования = СтрШаблон(
			НСтр("ru = 'Округление суммы документа, точность %1';
				|en = 'Document amount rounding, precision %1'"),
	...
	Если Объект.Управляемая Тогда
		СтрокаНаименования = НСтр("ru = 'Назначается вручную:';
									|en = 'Assigned manually:'") + " "+ СтрокаНаименования;
	КонецЕсли;
	
	Если Объект.СпособПримененияСкидки = ПредопределенноеЗначение("Перечисление.СпособыПримененияСкидокНаценок.НачислитьБонусныеБаллы") Тогда
		СтрокаНаименования = НСтр("ru = 'Бонус:';
									|en = 'Bonus:'") + " "+ СтрокаНаименования;
	КонецЕсли;
	...
	Если Объект.ЦеновыеГруппы.Количество() > 0 Тогда
		СтрокаНаименования = СтрокаНаименования + " " +НСтр("ru = '(с уточнением по ценовым группам)';
															|en = '(specification by price groups)'");
		Форма.Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
	КонецЕсли;
	...
	Иначе
		...
		НовоеУсловиеОбъект.Наименование = СтрШаблон(
			НСтр("ru = 'Карта лояльности ""%1"" не зарегистрирована';
				|en = 'Loyalty card ""%1"" is not registered'"),
			ВидКартыЛояльности);
...
Процедура ОткрытьУсловиеПредоставления()
	...
	Если ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Не выбрано условие предоставления';
													|en = 'Provision condition is not selected'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗначениеСкидкиНаценкиКоличествоОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	Если СтрНайти(Текст, "-") > 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Для типа скидки ""Скидка количеством"", поле ""Бесплатно"" не может быть меньше 0';
																|en = 'For the discount type ""Discount by amount"" ""Free"" field cannot be less than 0'"),,,,);	
	КонецЕсли;  
КонецПроцедуры
...
Процедура ЗначениеСкидкиНаценкиПодарокОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	Если СтрНайти(Текст, "-") > 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Для типа скидки ""Подарок"", поле ""Стоимость"" не может быть меньше 0';
																|en = 'For the discount type ""Gift"" ""Price"" field cannot be less than 0'"),,,,);	
	КонецЕсли;
