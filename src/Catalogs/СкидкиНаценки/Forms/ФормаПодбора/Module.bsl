...
Процедура ПеренестиВДокумент(Команда)
	...
	Если СкидкиНаценки.Количество() = 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПеренестиВДокументЗавершение", ЭтотОбъект), НСтр("ru = 'Не подобрано ни одной скидки (наценки). Закрыть?';
																									|en = 'No discount (markup) is selected. Close?'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
	КонецЕсли;
