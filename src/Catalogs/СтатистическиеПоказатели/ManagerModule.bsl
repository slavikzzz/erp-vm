...
// Служит для удаления старых неиспользуемых форм статистики.
// Предполагается, что актуальные формы будут заполнены (восстановлены) последующим вызовом процедуры ЗаполнитьПоставляемымиПравилами()
//
Процедура ПометитьНаУдалениеВсеФормыСтатистики() Экспорт

	Если ПланыОбмена.ГлавныйУзел() <> Неопределено Тогда
		// Поставляемые элементы не следует помечать в подчиненных узлах РИБ - 
		// они будут помечены при обмене.
 		Возврат;
	КонецЕсли;
	
	ВыборкаФормСтатистики = Справочники.ФормыСтатистики.Выбрать();
	
	Пока ВыборкаФормСтатистики.Следующий() Цикл
	
		Если ВыборкаФормСтатистики.ПометкаУдаления Тогда
			Продолжить;
		КонецЕсли;
		
		Попытка
			ФормаСтатистики = ВыборкаФормСтатистики.ПолучитьОбъект();
			ФормаСтатистики.УстановитьПометкуУдаления(Истина); // будут также помечены на удаление поля форм статистики
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Обновление поставляемых данных статистики';
					|en = 'Updating supplied statistics data'", Метаданные.ОсновнойЯзык.КодЯзыка),
				УровеньЖурналаРегистрации.Ошибка,, ВыборкаФормСтатистики.Ссылка, 
...
Функция ИмяСобытияВЖурналеРегистрации()
	...
	Возврат НСтр("ru = 'Правила заполнения форм статистики';
				|en = 'Statistics forms filling rules'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПравилаЗаполненияФормСтатистики(Знач ДанныеДляЗаполненияXML = Неопределено)
	...
	ЧтениеИзМакета = (ДанныеДляЗаполненияXML = Неопределено);
	Если ЧтениеИзМакета Тогда // поставляемые данные содержатся в макете
		
		ИсточникДанных = ПолучитьИмяВременногоФайла("txt");
		Справочники.СтатистическиеПоказатели.ПолучитьМакет("ОбъектыНаблюдения").Записать(ИсточникДанных);
		
	Иначе
		
		ИсточникДанных = ДанныеДляЗаполненияXML.ОбъектыНаблюдения;
		
	КонецЕсли;

	КонтейнерОбъектов = Контейнер(ИсточникДанных, ЧтениеИзМакета);
	
	Если КонтейнерОбъектов = Неопределено Тогда
		// Чтение макета не удалось, возвращаем пустую структуру данных.
		Если ЧтениеИзМакета Тогда
			УдалитьФайлы(ИсточникДанных);
		КонецЕсли;
	...
	Если ЧтениеИзМакета Тогда // поставляемые данные содержатся в макете
		...
		Справочники.СтатистическиеПоказатели.ПолучитьМакет("ФормыСтатистики").Записать(ИсточникДанных);
		
	Иначе
		
		ИсточникДанных = ДанныеДляЗаполненияXML.ФормыСтатистики;
		
	КонецЕсли;

	КонтейнерФорм = Контейнер(ИсточникДанных, ЧтениеИзМакета);
	Если ЧтениеИзМакета Тогда
		УдалитьФайлы(ИсточникДанных);
	КонецЕсли;
...
Процедура ЗагрузитьОбъектыНаблюдения(ПравилаЗаполнения)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ОбъектыСтатистическогоНаблюдения.Код КАК Код,
	|	ОбъектыСтатистическогоНаблюдения.Ссылка КАК Ссылка,
	|	ОбъектыСтатистическогоНаблюдения.ПометкаУдаления КАК ПометкаУдаления,
	|	ОбъектыСтатистическогоНаблюдения.ДатаИзменения КАК ДатаИзменения,
	|	ИСТИНА КАК Удалить
	|ИЗ
	...
	Для Каждого ПоставляемыйОбъектНаблюдения Из ПравилаЗаполнения Цикл
		...
		ОписаниеЗагруженногоРанее = ЗагруженныйРанееЭлемент(
		                            ПоставляемыйОбъектНаблюдения,
		                            ЗагруженныеРанееОбъектыНаблюдения,
		                            ПоискЗагруженногоРанееОбъектаНаблюдения);
		...
		Иначе
			...
			Если Не ЗагружатьЭлементПравил(ПоставляемыйОбъектНаблюдения, ОписаниеЗагруженногоРанее) Тогда
				...
				// Снимем флаг удаления у подчиненных показателей
				ПоискЗагруженныхРанееПоказателейОбъекта = Новый Структура("ОбъектНаблюдения", ОписаниеЗагруженногоРанее.Ссылка);
				Для Каждого ЗагруженныйРанееПоказатель Из ЗагруженныеРанееПоказатели.НайтиСтроки(ПоискЗагруженныхРанееПоказателейОбъекта) Цикл
					ЗагруженныйРанееПоказатель.Удалить = Ложь;
				КонецЦикла;
				Продолжить;
			КонецЕсли;			
		...
		Для Каждого ПоставляемыйПоказатель Из ПоставляемыйОбъектНаблюдения.СтатистическиеПоказатели Цикл
			...
			ОписаниеЗагруженногоРанее = ЗагруженныйРанееЭлемент(
			                            ПоставляемыйПоказатель,
										ЗагруженныеРанееПоказатели,
										ПоискЗагруженногоРанееПоказателя);
	...
	ПометитьНаУдалениеУстаревшиеЭлементы(ЗагруженныеРанееОбъектыНаблюдения);
	
КонецПроцедуры

Процедура ЗагрузитьФормыСтатистики(ПравилаЗаполнения)

    // Сначала у всех элементов ставим флаг Удалить.
	// Ниже у элементов, которые остались в актуальных поставляемых данных,
	// флаг снимем.
	...
	Для Каждого ПоставляемаяФорма Из ПравилаЗаполнения Цикл
		...
		Иначе
			...
			Если Не ЗагружатьЭлементПравил(ПоставляемаяФорма, ОписаниеЗагруженногоРанее) Тогда
			    // Снимем флаг удаления у подчиненных полей
				ПоискЗагруженныхРанееПолейФормы = Новый Структура("ФормаСтатистики", ОписаниеЗагруженногоРанее.Ссылка);
				Для Каждого ЗагруженноеРанееПолеФормы Из ЗагруженныеРанееПоляФорм.НайтиСтроки(ПоискЗагруженныхРанееПолейФормы) Цикл
					ЗагруженноеРанееПолеФормы.Удалить = Ложь;
				КонецЦикла;
				Продолжить;
			КонецЕсли;
			Объект = ОписаниеЗагруженногоРанее.Ссылка.ПолучитьОбъект();
		...
		Объект.Владелец      = Справочники.РегламентированныеОтчеты.НайтиПоРеквизиту("ИсточникОтчета",
                        	   ПоставляемаяФорма.РегламентированныйОтчет);
		...
		Для Каждого ПоставляемоеПоле Из ПоставляемаяФорма.ГруппыПолей Цикл
			...
			ОписаниеЗагруженногоРанее = ЗагруженныйРанееЭлемент(ПоставляемоеПоле,
                                    	ЗагруженныеРанееПоляФорм,
										ПоискЗагруженногоРанееПоляФормы);
		...
		// Загрузим поля формы.
		ПоискЗагруженногоРанееПоляФормы.ЭтоГруппа = Ложь;
		Для Каждого ПоставляемоеПоле Из ПоставляемаяФорма.Поля Цикл
			...
			ОписаниеЗагруженногоРанее = ЗагруженныйРанееЭлемент(ПоставляемоеПоле.СтатистическийПоказатель,
                            			СтатистическиеПоказатели,
										ПоискСтатистическогоПоказателя);
			Если ОписаниеЗагруженногоРанее <> Неопределено Тогда
	...
	// Пометим элементы, которые отсутствуют в акутальных данных.
	ПометитьНаУдалениеУстаревшиеЭлементы(ЗагруженныеРанееПоляФорм);
	ПометитьНаУдалениеУстаревшиеЭлементы(ЗагруженныеРанееФормыСтатистики);
...
Функция Контейнер(ДанныеСтрокой, ЧтениеИзМакета)
	
	Чтение = Новый ЧтениеXML;
	Если ЧтениеИзМакета Тогда
		Чтение.ОткрытьФайл(ДанныеСтрокой);
	Иначе
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось прочитать поставляемые данные.';
								|en = 'Cannot read the supplied data.'");
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Контейнер.ВерсияФормата, "3.0.2.0") >= 0 Тогда
		ТекстСообщения = НСтр("ru = 'Неподдерживаемая версия поставляемых данных %1';
								|en = 'Unsupported version of the %1 supplied data'");
