...
Функция ПроверкаПройдена()
	...
	Для каждого СтрокаДокумента Из ДокументыИсточники Цикл
		...
		Если СтрокаДокумента.Пометка Тогда
			...
			Если НетВложений(СтрокаДокумента) Тогда
				...
				ТекстОшибки = НСтр("ru = 'У документа ""';
									|en = 'У документа ""'") + Строка(СтрокаДокумента.Документ) + НСтр("ru = '"" отсутствуют вложения';
																										|en = '"" отсутствуют вложения'");
	...
	Если ВыбраноДокументов = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Выберите хотя бы один документ';
													|en = 'Выберите хотя бы один документ'"));
		ЕстьОшибка = Истина;
	КонецЕсли;
	...
	Для каждого ДокументИсточник Из ДокументыИсточники Цикл
		...
		Если ЗначениеЗаполнено(ДокументИсточник.Сканы) Тогда
			Для каждого Скан Из ДокументИсточник.Сканы Цикл
				...
				Если Скан.Значение.Свойство("АдресФайлаПодписи") Тогда
					Справочники.СканированныеДокументыДляПередачиВЭлектронномВиде.ДобавитьПодписьКПрисоединенномуФайлу(
						ПрисоединенныйФайлСсылка, 
						Скан.Значение.ИмяФайлаПодписи, 
						Скан.Значение.АдресФайлаПодписи);
				КонецЕсли;
...
Процедура ИзменитьОформлениеВложений()
	...
	Для каждого СтрокаДокумента Из ДокументыИсточники Цикл
		...
		Если СтрокаДокумента.ЕстьЭлектронныйДокумент Тогда
			СтрокаДокумента.ПредставлениеВложений = НСтр("ru = 'Документ 1С-ЭДО';
														|en = 'Документ 1С-ЭДО'");
		Иначе
