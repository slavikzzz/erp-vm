...
Процедура ПредставлениеАдресаПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ДанныеРазбораАдреса.ТекстОшибки) Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиент.СообщитьПользователю(ДанныеРазбораАдреса.ТекстОшибки,, ДанныеРазбораАдреса.Поле);
	КонецЕсли;
...
Процедура ОбработатьПараметрыПоиска()
	...
	Если ЗначениеЗаполнено(ИдентификаторХС) Тогда
		...
		Строки = Новый Массив;
		Строки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Только предприятия ХС';
														|en = 'Только предприятия ХС'"), Новый Шрифт("Аrial", 9, Истина), Новый Цвет(0, 150, 70)));
...
Процедура ПроверитьКорректностьЗаполненияРеквизитовПоиска(Отказ)
	...
	Если Не ПустаяСтрока(Идентификатор)
		...
		ТекстОшибки = НСтр("ru = 'Неправильно указан идентификатор';
							|en = 'Неправильно указан идентификатор'");
	...
	Если Не ПустаяСтрока(GLN)
		...
		ТекстОшибки = НСтр("ru = 'Необходимо указать 13 символов';
							|en = 'Необходимо указать 13 символов'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки, ,"GLN",, Отказ);
...
//   * Текст        - Строка - введенное представление адреса (входящий),
//   * Вид          - СправочникСсылка.ВидыКонтактнойИнформации - вид контактной информации адреса (входящий),
//   * Страна       - СправочникСсылка.СтраныМира - страна адреса (входящий),
//   * Адрес        - Произвольный - см соответствующую процедуру ЗначенияПолейКонтактнойИнформацииСервер (исходящий)
//   * ДанныеАдреса - Неопределено,Структура - структура с полями адреса (исходящий),
//   * ТекстОшибки  - Строка - описание ошибки получения адреса (исходящий),
//   * Поле         - Строка - поле в котором будет выведено сообщение об ошибке (исходящий).
//
&НаКлиенте
...
//
// Параметры:
//  ДанныеАдреса - (См. ДанныеДляРазбораАдреса)
		...
		Иначе
			...
			ДанныеАдреса.ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Не удалось прочитать данные адреса. Возможно не корректно введены данные. Повторите ввод. Причина ошибки: %1';
					|en = 'Не удалось прочитать данные адреса. Возможно не корректно введены данные. Повторите ввод. Причина ошибки: %1'"),
				ОписаниеОшибки());
		КонецЕсли;
	КонецПопытки;
