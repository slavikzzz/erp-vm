...
Процедура Загрузить(Команда)
	...
	Если ЗначениеЗаполнено(Предприятие) Тогда
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Выполнена загрузка предприятия %1.';
										|en = 'Выполнена загрузка предприятия %1.'"), Предприятие);
...
Процедура ОпределитьНаличиеВИБ()
	...
	Если Выборка.Следующий() Тогда
		...
		Строки.Добавить(НСтр("ru = 'Предприятие уже загружено';
							|en = 'Предприятие уже загружено'"));
		Строки.Добавить(" (");
		Строки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'открыть';
														|en = 'открыть'"),, ЦветаСтиля.ЦветГиперссылкиГосИС,, "ОткрытьПредприятие"));
		Строки.Добавить(").");
