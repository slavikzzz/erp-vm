...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ХозяйствующийСубъект") И ЗначениеЗаполнено(Параметры.ХозяйствующийСубъект) Тогда
		...
		Если ЗначениеЗаполнено(Параметры.СтрокаПоиска) Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ВведенноеНаименование", 
				"(ХозяйствующиеСубъектыВЕТИСПредприятия.Предприятие.Наименование ПОДОБНО (&Наименование)
				|ИЛИ ХозяйствующиеСубъектыВЕТИСПредприятия.Предприятие.Идентификатор ПОДОБНО (&Идентификатор))");
			Запрос.УстановитьПараметр("Наименование",  "%"+Параметры.СтрокаПоиска+"%");
			Запрос.УстановитьПараметр("Идентификатор", Параметры.СтрокаПоиска + "%");
		КонецЕсли;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ШаблонОшибкиОбработкиПредприятия =
		НСтр("ru = 'Не удалось обработать предприятие ВетИС ""%1"" по причине:
		           |%2';
		           |en = 'Не удалось обработать предприятие ВетИС ""%1"" по причине:
		           |%2'");
...
//   Производитель - Неопределено, СправочникСсылка.ПредприятияВЕТИС - изменившееся значение производителя
//
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.Номер > 1 Тогда
			...
			Если СтрДлина(Номер) > 75 Тогда
				Номер = СтрШаблон(НСтр("ru = '%1 (+ еще %2)';
										|en = '%1 (+ еще %2)'"), НомераПредприятия, Выборка.Номер - 1);
			КонецЕсли;
