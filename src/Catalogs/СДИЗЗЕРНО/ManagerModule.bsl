...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Данные.ТребуетсяЗагрузка Тогда
		СтрШаблон(НСтр("ru = '%1 (не загружено)';
						|en = '%1 (не загружено)'"), Данные.Идентификатор);
	Иначе
...
Процедура ПриЗаполненииОграниченияДоступа(Списки) Экспорт
	...
	УправлениеДоступомИСПереопределяемый.ПриЗаполненииОграниченияДоступа(
		Метаданные.Справочники.СДИЗЗЕРНО, Списки);
...
Процедура ОбработкаЗагрузкиПолученныхДанных(ЭлементОчереди, ПараметрыОбмена, ПолученныеДанные, ИзмененныеОбъекты, ПрочитаноВерсиейAPI) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийЗЕРНО.ЗапросПогашенийСДИЗ Тогда
		...
		РегистрыСведений.ИсторияПогашенийСДИЗЗЕРНО.ОбработкаЗагрузкиПолученныхДанных(
...
Функция СообщениеЗагрузкиСДИЗ(ПараметрыФормирования, ПараметрыОбмена)
	...
	Если ЗначениеЗаполнено(ПараметрыФормирования.Статус) Тогда
		...
		ДанныеОписания.Добавить(СтрШаблон(НСтр("ru = 'статус: %1';
												|en = 'статус: %1'"), ПараметрыЗапроса.Статус));
	КонецЕсли;
	Если ЗначениеЗаполнено(ПараметрыФормирования.ВидСДИЗ) Тогда
		...
		ДанныеОписания.Добавить(СтрШаблон(НСтр("ru = 'вид СДИЗ: %1';
												|en = 'вид СДИЗ: %1'"), ПараметрыФормирования.ВидСДИЗ));
	КонецЕсли;
	Если ЗначениеЗаполнено(ПараметрыФормирования.ИдентификаторСДИЗ) Тогда
		...
		ДанныеОписания.Добавить(СтрШаблон(НСтр("ru = 'id: %1';
												|en = 'id: %1'"), ПараметрыФормирования.ИдентификаторСДИЗ));
	КонецЕсли;
	Если ЗначениеЗаполнено(ПараметрыФормирования.НомерПартии) Тогда
		...
		ДанныеОписания.Добавить(СтрШаблон(НСтр("ru = 'номер партии: %1';
												|en = 'номер партии: %1'"), ПараметрыФормирования.НомерПартии));
	КонецЕсли;
	Если ЗначениеЗаполнено(ПараметрыФормирования.ОКПД2) Тогда
		...
		ДанныеОписания.Добавить(СтрШаблон(НСтр("ru = 'ОКПД2: %1';
												|en = 'ОКПД2: %1'"), ПараметрыФормирования.ОКПД2));
	КонецЕсли;
	Если ЗначениеЗаполнено(ПараметрыФормирования.НомерСДИЗ) Тогда
		...
		ДанныеОписания.Добавить(СтрШаблон(НСтр("ru = '№ %1';
												|en = '№ %1'"), ПараметрыФормирования.НомерСДИЗ));
	КонецЕсли;
...
Функция СообщениеЗагрузкиПогашенийСДИЗ(ПараметрыОбработкиДокумента, ПараметрыОбмена)
	...
	ДанныеОписания = Новый Массив();
	ДанныеОписания.Добавить(СтрШаблон(НСтр("ru = '№ %1';
											|en = '№ %1'"), ПараметрыЗапроса.НомерСДИЗ));
...
Функция СообщениеАннулированияСДИЗ(ПараметрыОбработкиДокумента, ПараметрыОбмена)
	...
	СообщениеXML.ДополнительноеОписание = СтрШаблон(НСтр("ru = '№ %1';
														|en = '№ %1'"), РеквизитыСДИЗ.Идентификатор);
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		ИнтеграцияЗЕРНОКлиентСервер.ДобавитьТекстОшибки(
			СообщениеXML,
			СтрШаблон(НСтр("ru = 'Не удалось определить организацию по %1';
							|en = 'Не удалось определить организацию по %1'"), РеквизитыСДИЗ.УполномоченноеЛицо));
	КонецЕсли;
	СообщениеXML.ТекстСообщенияЗапросXML = ИнтеграцияЗЕРНОСлужебный.ОбъектXDTOВXML(
		ТелоXDTO, ПространствоИмен, ИмяСвойстваЗапроса,
		ПараметрыОбмена);
	...
	Иначе
		...
		Исключение
			...
			ОбщегоНазначенияИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
				ТекстОшибкиПодробно,
				НСтр("ru = 'Работа со справочником СДИЗ';
					|en = 'Работа со справочником СДИЗ'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ЗагрузитьСДИЗ(ЭлементДанных, Организация, Подразделение, ВидПродукции, ПараметрыОбмена, ПрочитаноВерсиейAPI) Экспорт
	...
	Если ИнтеграцияЗЕРНОСлужебный.ЭтоВерсияВключая_1_0_8(ПрочитаноВерсиейAPI)
		И ЗначениеЗаполнено(ЭлементДанных[ИмяПоляКорректируемыйСДИЗ]) Тогда
	...
	ЗаполнитьСДИЗПоВидуОперации(
		СуществующийОбъект, ЭлементДанных, Организация, Подразделение, ВидПродукции, ПараметрыОбмена, ПрочитаноВерсиейAPI);
...
Процедура ЗаполнитьСДИЗПоВидуОперации(СуществующийОбъект, ЭлементДанных, Организация, Подразделение, ВидПродукции, ПараметрыОбмена, ПрочитаноВерсиейAPI)
	...
	Иначе
		...
		ЗаполнитьСведенияПриПеревозке(
			СуществующийОбъект, ДанныеСДИЗ.Shipping, Организация, Подразделение, ПараметрыОбмена, ВидПродукции, ПрочитаноВерсиейAPI);
...
Процедура ЗаполнитьСведенияПриПеревозке(СуществующийОбъект, Сведения, Организация, Подразделение, ПараметрыОбмена, ВидПродукции, ПрочитаноВерсиейAPI)
	...
	Если ИнтеграцияЗЕРНОСлужебный.ЭтоВерсияВключая_1_0_8(ПрочитаноВерсиейAPI) Тогда
