...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Данные.ТребуетсяЗагрузка Тогда
		Представление = СтрШаблон(НСтр("ru = '<Требуется загрузка> %1';
										|en = '<Требуется загрузка> %1'"), Данные.Идентификатор);
	Иначе
...
Функция СообщениеЗагрузки(ПараметрыФормирования, ПараметрыОбмена)
	...
	Если ПараметрыЗапроса <> Неопределено Тогда
		...
		Если ПараметрыЗапроса.Свойство("Идентификатор") Тогда
			...
			ДанныеОписания.Добавить(СтрШаблон(НСтр("ru = 'идентификатор: %1';
													|en = 'идентификатор: %1'"), ПараметрыЗапроса.Идентификатор));
		КонецЕсли;
		...
		Исключение
			...
			ОбщегоНазначенияИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
				...
				НСтр("ru = 'Акт отбора проб';
					|en = 'Акт отбора проб'", ОбщегоНазначения.КодОсновногоЯзыка()));
