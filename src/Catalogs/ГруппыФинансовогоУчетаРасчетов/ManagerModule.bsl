...
Процедура ПреобразоватьОтборПараметровВыбора(Отбор) Экспорт
	...
	Если Отбор.Свойство("ХозяйственнаяОперация") И ЗначениеЗаполнено(Отбор.ХозяйственнаяОперация) Тогда
		...
		//++ НЕ УТКА
		СоответствиеХОиРасчетов.Вставить(Операции.ОтчетДавальцуМеждуОрганизациями, ?(ГФУПолучателя, "РасчетыСПоставщиками", "РасчетыСКлиентами"));
		//-- НЕ УТКА
...
Процедура ЗаполнитьСоответствиеРеквизитовНастройкиСчетовУчета(СоответствиеИмен) Экспорт
	...
	СоответствиеИмен.РасчетыСПереработчиками = "РасчетыСПереработчиками";
	СоответствиеИмен.РасчетыСДавальцами = "РасчетыСДавальцами";
	СоответствиеИмен.РасчетыСКредиторами = "РасчетыСКредиторами";
	СоответствиеИмен.РасчетыСДебиторами = "РасчетыСДебиторами";
	СоответствиеИмен.РасчетыСАрендодателями = "РасчетыСАрендодателями";
	СоответствиеИмен.РасчетыВВалюте = "РасчетыВВалюте";
	СоответствиеИмен.РасчетыПоБронированию = "РасчетыПоБронированию";
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет признак Расчеты в валюте регламентированного учета.';
									|en = 'Fills the ""AR/AP accounting in local accounting currency"" checkbox.'");
	...
	Если Запрос.Выполнить().Пустой() Тогда
		...
		Попытка
			...
			СправочникОбъект.Наименование = НСтр("ru = 'Расчеты с перевозчиками по электронным билетам';
												|en = 'AR/AP accounting with carriers by electronic tickets'");
			...
			//++ НЕ УТ
			Запись = РегистрыСведений.ПорядокОтраженияНаСчетахУчета.СоздатьМенеджерЗаписи();
			Запись.ВидСчета = Перечисления.ВидыСчетовРеглУчета.РасчетыПоБронированию;
			Запись.СчетУчета = ПланыСчетов.Хозрасчетный.РасчетыПоБронированию;
			Запись.АналитикаУчета = СправочникОбъект.Ссылка;
			Запись.Записать();
			//-- НЕ УТ
