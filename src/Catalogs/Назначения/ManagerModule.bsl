...
//-- НЕ УТ
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Источник данных ""%1"" не предусмотрен.';
							|en = 'The ""%1"" data source is not provided.'");
...
//  ПараметрФормированияШаблонаНазначений - Строка - параметр формирования шаблона назначений.
//
Процедура ФормаДокументаПриСозданииНаСервере(ФормаОбъекта, ПараметрФормированияШаблонаНазначений = "") Экспорт
	...
	Если ПараметрФормированияШаблонаНазначений = "" Тогда
		МакетФормыВыбораНазначений = МенеджерОбъекта.МакетФормыВыбораНазначений();
	Иначе
		МакетФормыВыбораНазначений = МенеджерОбъекта.МакетФормыВыбораНазначенийПоПараметру(ПараметрФормированияШаблонаНазначений);
	КонецЕсли;
...
Функция СвойстваНазначений(Назначения) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		//++ НЕ УТКА
		Значение.Вставить("ТипНазначения", Выборка.ТипНазначения);
		
		Если Выборка.ТипНазначения = Перечисления.ТипыНазначений.Давальческое2_5 Тогда
			Значение.ЭтоНазначениеДавальца2_5 = Истина;
		//++ Устарело_Переработка24
		ИначеЕсли Выборка.ТипНазначения = Перечисления.ТипыНазначений.Давальческое21
			Или Выборка.ТипНазначения = Перечисления.ТипыНазначений.ДавальческоеМатериалы22
			Или Выборка.ТипНазначения = Перечисления.ТипыНазначений.ДавальческоеПродукция22
			Или Выборка.ТипНазначения = Перечисления.ТипыНазначений.ДавальческоеМатериалыПодЭтап22 Тогда
			Значение.ЭтоНазначениеДавальца = Истина;
		//-- Устарело_Переработка24
		ИначеЕсли Выборка.ТипНазначения = Перечисления.ТипыНазначений.ДавальческоеМатериалыПодЭтап22 Тогда
			Значение.ЭтоНазначениеДавальца2_5 = Истина;
		КонецЕсли;
		
		//++ Устарело_Переработка24
		Если ТипЗнч(Выборка.Заказ) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
			Значение.ЭтоДавальческаяПродукция = Истина;
		КонецЕсли;
		//-- Устарело_Переработка24
...
//++ НЕ УТКА
	...
	// Потребность в материалах и полуфабрикатах по графику производства.
	Если ПолучитьФункциональнуюОпцию("ДинамическаяСтруктураЗаказовНаПроизводство") Тогда
		ТекстЗапроса =
			...
			|	СУММА(Движения.Требуется) КАК ПрефиксПотребность,
...
//-- НЕ УТКА
	...
	ПредставлениеПотребность     = СтрШаблон(НСтр("ru = 'Потребность на складе ""%1""';
													|en = 'Demand in warehouse ""%1""'"),   ПараметрПредставления);
	ПредставлениеВНаличии        = СтрШаблон(НСтр("ru = 'В наличии на складе ""%1""';
													|en = 'In stock in the ""%1"" warehouse'"),     ПараметрПредставления);
	ПредставлениеАрхивныеОстатки = СтрШаблон(НСтр("ru = 'Заказано на складе ""%1""';
													|en = 'Ordered at warehouse ""%1""'"),      ПараметрПредставления);
...
//-- НЕ УТКА
	...
	ПредставлениеПотребность     = СтрШаблон(НСтр("ru = 'Потребность в подразделении ""%1""';
													|en = 'Demand in business unit ""%1""'"),   ПараметрПредставления);
	ПредставлениеВНаличии        = СтрШаблон(НСтр("ru = 'В наличии в подразделении ""%1""';
													|en = 'Available at business unit ""%1""'"),     ПараметрПредставления);
	ПредставлениеАрхивныеОстатки = СтрШаблон(НСтр("ru = 'Заказано в подразделение ""%1""';
													|en = 'Ordered to business unit ""%1""'"),      ПараметрПредставления);
...
Функция ДобавитьОписаниеДанныеРеализации(МакетФормы, Префикс)
	...
	Колонки = Новый СписокЗначений();
	Колонки.Добавить("Реализовано", НСтр("ru = 'Реализовано';
										|en = 'Sold'"), Истина);
...
//-- НЕ УТКА
	...
	Колонки = Новый СписокЗначений();
	Колонки.Добавить("ВНаличии",    НСтр("ru = 'В наличии на всех складах';
										|en = 'In stock in all warehouses'"), Ложь);
	Колонки.Добавить("Потребность", НСтр("ru = 'Потребность на всех складах';
										|en = 'Demand in all warehouses'"), Ложь);
	Колонки.Добавить("Архивное",    НСтр("ru = 'Заказано на всех складах';
										|en = 'Ordered at all warehouses'"), Ложь);
...
//++ НЕ УТКА
	...
	// Потребность в работах по графику производства.
	Если ПолучитьФункциональнуюОпцию("ДинамическаяСтруктураЗаказовНаПроизводство") Тогда
		ТекстЗапроса =
...
Функция НайтиЭлементФормы(Форма, ПутьКДанным)
	...
	ВызватьИсключение СтрШаблон(НСтр("ru = 'Не найден элемент формы с путем к данным: %1';
									|en = 'No form item is found with data path:%1'"), ПутьКДанным);
...
Функция ДобавитьОписаниеМатериалыИРаботыВПроизводстве(МакетФормы, ДобавитьСторно, Префикс)
	...
	Колонки = Новый СписокЗначений();
	Колонки.Добавить("Количество", НСтр("ru = 'Количество в производстве';
										|en = 'Quantity in production'"), Ложь);
...
Функция ДобавитьОписаниеТоварыПереданныеХранителям(МакетФормы, ДобавитьСторно, Префикс)
	...
	Колонки = Новый СписокЗначений();
	Колонки.Добавить("Передано", НСтр("ru = 'Передано';
										|en = 'Handed over'"), Ложь);
...
Функция ТипОбъектаНазначенияПоДокументу(Документ)
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказМатериаловВПроизводство") Тогда
		ЗначениеПеречисления = Перечисления.ТипыОбъектовНазначений.ЗаказМатериаловВПроизводство;
	//++ Устарело_Переработка24
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказПереработчику") Тогда
		ЗначениеПеречисления = Перечисления.ТипыОбъектовНазначений.ЗаказПереработчику;
	//-- Устарело_Переработка24
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказПереработчику2_5") Тогда
		ЗначениеПеречисления = Перечисления.ТипыОбъектовНазначений.ЗаказПереработчику2_5;
	//-- НЕ УТ

	//++ НЕ УТКА

	//++ Устарело_Переработка24
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
		ЗначениеПеречисления = Перечисления.ТипыОбъектовНазначений.ЗаказДавальца;
	//-- Устарело_Переработка24
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказДавальца2_5") Тогда
		ЗначениеПеречисления = Перечисления.ТипыОбъектовНазначений.ЗаказДавальца2_5;
	//++ Устарело_Производство21
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
		ЗначениеПеречисления = Перечисления.ТипыОбъектовНазначений.ЗаказНаПроизводство;
	//-- Устарело_Производство21
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказНаРемонт") Тогда
		ЗначениеПеречисления = Перечисления.ТипыОбъектовНазначений.ЗаказНаРемонт;
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		ЗначениеПеречисления = Перечисления.ТипыОбъектовНазначений.ЭтапПроизводства2_2;
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ЗаказНаПроизводство2_2") Тогда
		ЗначениеПеречисления = Перечисления.ТипыОбъектовНазначений.ЗаказНаПроизводство2_2;
	//-- НЕ УТКА
	КонецЕсли;
...
Функция СообщениеНетНазначенияВБазе(ШаблонНазначения)
	...
	ИначеЕсли ЗначениеЗаполнено(ШаблонНазначения.Договор) Тогда
		...
		Сообщение = СтрШаблон(НСтр("ru = 'В информационной базе не обнаружено назначение для договора: ""%1""';
									|en = 'Assignment for contract ""%1"" was not found in the infobase'"),
	...
	ИначеЕсли ЗначениеЗаполнено(ШаблонНазначения.Партнер) Тогда
		...
		Сообщение = СтрШаблон(НСтр("ru = 'В информационной базе не обнаружено назначение для партнера: ""%1""';
									|en = 'Assignment for partner ""%1"" was not found in the infobase'"),
...
Функция ДанныеОбновленыНаНовуюВерсиюПрограммы(МетаданныеИОтбор) Экспорт
	...
	Если ДанныеОбновлены И МетаданныеИОтбор.Метаданные = Метаданные.Справочники.НаправленияДеятельности Тогда
		ЗависимыеДанныеОбновлены = 
			...
			//++ НЕ УТКА
			Или Не ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки(Неопределено, Метаданные.Документы.ЗаказНаРемонт)
			//-- НЕ УТКА
