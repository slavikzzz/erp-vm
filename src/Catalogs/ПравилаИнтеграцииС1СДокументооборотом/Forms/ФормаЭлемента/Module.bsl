...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.УсловиеПрименимостиПриВыгрузке.Пометка = Не ПустаяСтрока(Объект.УсловиеПрименимостиПриВыгрузке);
	Элементы.УсловиеПрименимостиПриЗагрузке.Пометка = Не ПустаяСтрока(Объект.УсловиеПрименимостиПриЗагрузке);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(ПредставлениеОбъектаДО) Тогда
		...
		Сообщение.Текст = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("ru = 'Объект 1С:Документооборота';
																					|en = '1C:Document Management object'"));
...
Процедура СообщитьОбОшибке(Строка)
	...
	Сообщение.Текст = СтрШаблон(
		НСтр("ru = 'Не установлено правило для обязательного %1 ""%2""';
			|en = 'Rule for required %1 ""%2"" not configured'"),
		?(Строка.ЭтоТаблица, НСтр("ru = 'списка';
									|en = 'list'"), НСтр("ru = 'реквизита';
														|en = 'attribute'")),
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Справочники.ПравилаИнтеграцииС1СДокументооборотом.ПроверитьПравило(ТекущийОбъект);
...
Процедура ПравилаЗаполненияРеквизитовДОПередУдалением(Элемент, Отказ)
	...
	Если Объект.ШаблонЗапрещаетИзменение И ТекущиеДанные.ЗаполненВШаблоне Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выбранный шаблон запрещает изменение реквизитов.';
										|en = 'The template you selected does not permit editing attributes.'"));
		Возврат;
	КонецЕсли;
	...
	Если ТекущиеДанные.ИмяРеквизитаОбъектаДО = "documentType" И ТекущиеДанные.ЗаполненВШаблоне Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Вид документа указан в шаблоне и не может быть изменен.';
										|en = 'Document kind is specified in the template and cannot be changed.'"));
		Возврат;
	КонецЕсли;
	
	ПравилаЗаполненияРеквизитовДОПередУдалениемПродолжение(ТекущиеДанные);
...
&НаКлиенте
Асинх Процедура ЗаполнитьПоУмолчаниюПриВыгрузке(Знач Команда)
...
&НаКлиенте
Асинх Процедура ЗаполнитьПоУмолчаниюПриЗагрузке(Знач Команда)
...
&НаКлиенте
Асинх Процедура ПравилаЗаполненияРеквизитовДОПередУдалениемПродолжение(Знач ТекущиеДанные)
	
	Ответ = Ждать ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПоказатьВопросДаНетАсинх(
		НСтр("ru = 'Очистить правило заполнения?';
			|en = 'Clear the autofill rule?'"),
...
Асинх Процедура ПравилаЗаполненияРеквизитовИСПередУдалениемПродолжение(Знач ТекущиеДанные)
	...
	Ответ = Ждать ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПоказатьВопросДаНетАсинх(
		НСтр("ru = 'Очистить правило заполнения реквизита?';
			|en = 'Clear the attribute autofill rule?'"),
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПараметрыВопросаДаНет(КодВозвратаДиалога.Да));
...
Процедура УстановитьДоступность()
	...
	Элементы.ЗаполнитьПоУмолчанию.Доступность = ЗначениеЗаполнено(Объект.ТипОбъектаДО);
	Элементы.Шаблон.Доступность = ЗначениеЗаполнено(Объект.ТипОбъектаДО) И (Объект.ТипОбъектаДО <> "DMCorrespondent");
	Элементы.УсловиеПрименимостиПриВыгрузке.Доступность = ЭтоПолноправныйПользователь;
	Элементы.УсловиеПрименимостиПриЗагрузке.Доступность = ЭтоПолноправныйПользователь;
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ВидДокумента, "templateRequired")
			...
			Сообщение.Текст = СтрШаблон(
				НСтр("ru = 'В настройках вида документа ""%1"" установлен запрет на создание документов не по шаблону,
							...
							|en = 'The prohibition on creating documents not based on the template is set in the settings for the document type ""%1"",
							|but the document template is not selected.'"),
