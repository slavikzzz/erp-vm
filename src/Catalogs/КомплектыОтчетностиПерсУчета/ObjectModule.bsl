...
Процедура ПроверитьУникальностьНомеровПачек(Ошибки, Отказ)
	...
	ТекстОшибки = НСтр("ru = 'Документ с таким же номером уже входит в состав комплекта.';
						|en = 'Document with the same number is already included in the set.'");
...
Процедура ПроверитьСоставКомплекта(Ошибки, ДанныеРСВ = Неопределено, Отказ = Ложь)
	...
	Если ДанныеРСВ = Неопределено
		...
		ТекстОшибки = НСтр("ru = 'При наличии Описи сведений по форме АДВ-6-2, наличие формы РСВ-1 обязательно за тот же отчетный период.';
							|en = 'If there is the List of information according to form ADV-6-2, form RSV-1 must be present for the same accounting period.'");
	...
	Если ДанныеРСВ <> Неопределено
		...
		ТекстОшибки = НСтр("ru = 'При наличии Расчета с ненулевыми начислениями страховых взносов за 3 месяца отчетного периода (строка  114 Расчета) наличие Описи сведений АДВ-6-2  за тот же отчетный период обязательно.';
							|en = 'If there is Calculation with non-zero accruals of insurance contributions for 3 months of the accounting period (line 114 of the Calculation), the List of information ADV-6-2 must be present for the same accounting period.'");
	...
	Если Не ЕстьФормыСЗВ6 И Не ОписьПачекСЗВ.Пустая() Тогда
		...
		ИмяФорм = ?(ОтчетныйПериод >= '20130101', НСтр("ru = 'СЗВ-6-4';
														|en = 'SZV-6-4'"), НСтр("ru = 'СЗВ-6-1(2)';
																				|en = 'SZV-6-1(2)'"));
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'При наличии Описи сведений по форме АДВ-6-2, наличие исходных форм %1 обязательно за тот же отчетный период.';
							|en = 'If there is the List of information according to form ADV-6-2, the %1 forms must be present for the same accounting period.'"),
						ИмяФорм);
...
Процедура УстановитьПредставлениеСоставаКомплекта() Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(РСВ_1) Тогда
			СтрокаФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, РСВ-1';
																						|en = '%1, RSV-1'"), СтрокаФормы);
		КонецЕсли;	
		
		Если ЗначениеЗаполнено(ОписьПачекСЗВ) Тогда
			СтрокаФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, АДВ-6-2';
																						|en = '%1, ADV-6-2'"), СтрокаФормы);
		КонецЕсли;	
		
		Если КомплектСодержитДокументы(Тип("ДокументСсылка.ПачкаДокументовСЗВ_6_1")) Тогда
			СтрокаФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, СЗВ-6-1';
																						|en = '%1, SZV-6-1'"), СтрокаФормы);
		КонецЕсли;	
		
		Если КомплектСодержитДокументы(Тип("ДокументСсылка.РеестрСЗВ_6_2")) Тогда
			СтрокаФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, СЗВ-6-2';
																						|en = '%1, SZV-6-2'"), СтрокаФормы);
		КонецЕсли;	
		
		Если КомплектСодержитДокументы(Тип("ДокументСсылка.ПачкаДокументовСЗВ_6_4")) Тогда
			СтрокаФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, СЗВ-6-4';
																						|en = '%1, SZV-6-4'"), СтрокаФормы);
		КонецЕсли;	
		
		Если СписокПачекСЗВ6_3.Количество() > 0 Тогда
			СтрокаФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, СЗВ-6-3';
																						|en = '%1, SZV-6-3'"), СтрокаФормы);
		КонецЕсли;	
		
		СтрокаФормы = НСтр("ru = 'Комплект форм';
							|en = 'Set of forms'") + " " + Сред(СтрокаФормы, 3);
...
Функция ПредставлениеКорректируемыхПериодов()
	...
	Если КорректируемыеПериоды.Количество() > 0 Тогда 			
			...
			Если ГодПредыдущегоПериода <> Год(ОписаниеКорректируемогоПериода.Период) Тогда
				Если ГодПредыдущегоПериода = 2010 Тогда
					СтрокаКоррПериоды = СтрокаКоррПериоды + НСтр("ru = ' полугодие 2010 г.,';
																|en = ' half-year of 2010,'");
				Иначе 	
					СтрокаКоррПериоды = СтрокаКоррПериоды + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = ' кв. %1 г.,';
																														|en = ' quart. %1, '"), Формат(ГодПредыдущегоПериода, "ЧГ="));
				КонецЕсли;
		...
		СтрокаКоррПериоды = НСтр("ru = 'Коррект.:';
								|en = 'Adjust.:'") + " " + Сред(СтрокаКоррПериоды, 3, ДлинаСтроки - 2);
	КонецЕсли;	
...
Процедура ПроверитьДанныеРСВЗа2012(ДанныеРСВ, Ошибки, Отказ = Ложь)
	...
	Если НЕ ДанныеРСВ.ФормаРеглОтчетаДействуетВТекущемОтчетномПериоде Тогда
		ТекстСообщения = НСтр("ru = 'Форма РСВ-1 не актуальна в текущем периоде';
								|en = 'Form RSV-1 is not up to date in the current period'");
	...
	Если ДанныеРСВ.КоличествоЗастрахованных = 0 
		...
		ТекстСообщения = НСтр("ru = 'Количество застрахованных лиц должно быть больше нуля.';
								|en = 'Number of insured persons must be greater than zero.'");
	...
	Если СредняяЧисленность = 0 
		...
		ТекстСообщения = НСтр("ru = 'Среднесписочная численность должна быть больше нуля.';
								|en = 'Average number must be greater than zero.'");
	...
	Для каждого ЛистДанных Из Разделы Цикл
		...
		Если ПоказателиРазделов.П000200024003 <> ПоказателиРазделов.П000200020103 + ПоказателиРазделов.П000200020303 - ПоказателиРазделов.П000200021103 - ПоказателиРазделов.П000200021303 - ПоказателиРазделов.П000200022103 - ПоказателиРазделов.П000200022303 - ПоказателиРазделов.П000200023103 - ПоказателиРазделов.П000200023303 Тогда
			ТекстСообщения = НСтр("ru = 'Значение показателя в гр.3 строки 240 должно быть равно: строка 201 плюс строка 203, минус строка 211, минус строка 213, минус строка 221, минус строка 223, минус строка 231, минус строка 233 по гр.3.';
									|en = 'Indicator value in col. 3 of line 240 should be equal to: line 201, plus line 203, minus line 211, minus line 213, minus line 221, minus line 223, minus line 231, minus line 233 of col. 3.'");
		...
		Если ПоказателиРазделов.П000200024004 <> ПоказателиРазделов.П000200020104 + ПоказателиРазделов.П000200020304 - ПоказателиРазделов.П000200021104 - ПоказателиРазделов.П000200021304 - ПоказателиРазделов.П000200022104 - ПоказателиРазделов.П000200022304 - ПоказателиРазделов.П000200023104 - ПоказателиРазделов.П000200023304 Тогда
			ТекстСообщения = НСтр("ru = 'Значение показателя в гр.4 строки 240 должно быть равно: строка 201 плюс строка 204, минус строка 211, минус строка 213, минус строка 221, минус строка 223, минус строка 231, минус строка 233 по гр.4.';
									|en = 'Indicator value in col. 3 of line 240 should be equal to: line 201 plus line 204, minus line 211, minus line 213, minus line 221, minus line 223, minus line 231, minus line 233 of col. 4.'");
		...
		Если ПоказателиРазделов.П000200024005 <> ПоказателиРазделов.П000200020105 + ПоказателиРазделов.П000200020305 - ПоказателиРазделов.П000200021105 - ПоказателиРазделов.П000200021305 - ПоказателиРазделов.П000200022105 - ПоказателиРазделов.П000200022305 - ПоказателиРазделов.П000200023105 - ПоказателиРазделов.П000200023305 Тогда
			ТекстСообщения = НСтр("ru = 'Значение показателя в гр.5 строки 240 должно быть равно: строка 201 плюс строка 205, минус строка 211, минус строка 213, минус строка 221, минус строка 223, минус строка 231, минус строка 233 по гр.5.';
									|en = 'Indicator value in col. 5 of line 240 should be equal to: line 201 plus line 205, minus line 211, minus line 213, minus line 221, minus line 223, minus line 231, minus line 233 of col. 5.'");
		...
		Если ПоказателиРазделов.П000200024006 <> ПоказателиРазделов.П000200020106 + ПоказателиРазделов.П000200020306 - ПоказателиРазделов.П000200021106 - ПоказателиРазделов.П000200021306 - ПоказателиРазделов.П000200022106 - ПоказателиРазделов.П000200022306 - ПоказателиРазделов.П000200023106 - ПоказателиРазделов.П000200023306 Тогда
			ТекстСообщения = НСтр("ru = 'Значение показателя в гр.6 строки 240 должно быть равно: строка 201 плюс строка 206, минус строка 211, минус строка 213, минус строка 221, минус строка 223, минус строка 231, минус строка 233 по гр.6.';
									|en = 'Indicator value in col. 6 of line 240 should be equal to: line 201 plus line 206, minus line 211, minus line 213, minus line 221, minus line 223, minus line 231, minus line 233 of col. 6.'");
		...
		Если ПоказателиРазделов.П000200024103 <> ПоказателиРазделов.П000200020203 - ПоказателиРазделов.П000200021203 - ПоказателиРазделов.П000200022203 - ПоказателиРазделов.П000200023203 Тогда
			ТекстСообщения = НСтр("ru = 'Значение показателя в гр.3 строки 241 должно быть равно: строка 202 минус строка 212, минус строка 222, минус строка 232 по гр.3.';
									|en = 'Indicator value in col. 3 of line 241 should be equal to: line 202 minus line 212, minus line 222, minus line 232 of col. 3.'");
		...
		Если ПоказателиРазделов.П000200024104 <> ПоказателиРазделов.П000200020204 - ПоказателиРазделов.П000200021204 - ПоказателиРазделов.П000200022204 - ПоказателиРазделов.П000200023204 Тогда
			ТекстСообщения = НСтр("ru = 'Значение показателя в гр.4 строки 241 должно быть равно: строка 202 минус строка 212, минус строка 222, минус строка 232 по гр.4.';
									|en = 'Indicator value in col. 3 of line 241 should be equal to: line 202 minus line 212, minus line 222, minus line 232 of col. 4.'");
		...
		Если ПоказателиРазделов.П000200024105 <> ПоказателиРазделов.П000200020205 - ПоказателиРазделов.П000200021205 - ПоказателиРазделов.П000200022205 - ПоказателиРазделов.П000200023205 Тогда
			ТекстСообщения = НСтр("ru = 'Значение показателя в гр.5 строки 241 должно быть равно: строка 202 минус строка 212, минус строка 222, минус строка 232 по гр.5.';
									|en = 'Indicator value in col. 5 of line 240 should be equal to: line 202 minus line 212, minus line 222, minus line 232 of col. 5.'");
		...
		Если ПоказателиРазделов.П000200024106 <> ПоказателиРазделов.П000200020206 - ПоказателиРазделов.П000200021206 - ПоказателиРазделов.П000200022206 - ПоказателиРазделов.П000200023206 Тогда
			ТекстСообщения = НСтр("ru = 'Значение показателя в гр.6 строки 241 должно быть равно: строка 202 минус строка 212, минус строка 222, минус строка 232 по гр.6.';
									|en = 'Indicator value in col. 6 of line 241 should be equal to: line 202 minus line 212, minus line 222, minus line 232 of col. 6.'");
		...
		Если СтрокаТарифа <> Неопределено Тогда
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.3 строки 250 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																								|en = 'Indicator value in col. 3 of line 250 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"), Формат(ПоказателиРазделов.П000200025003,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.4 строки 250 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																								|en = 'Indicator value in col. 4 of line 250 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"),  Формат(ПоказателиРазделов.П000200025004,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.5 строки 250 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																								|en = 'Indicator value in col. 5 of line 250 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"),  Формат(ПоказателиРазделов.П000200025005,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.6 строки 250 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																								|en = 'Indicator value in col. 6 of line 250 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"),  Формат(ПоказателиРазделов.П000200025006,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.3 строки 251 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																								|en = 'Indicator value in col. 3 of line 251 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"), Формат(ПоказателиРазделов.П000200025103,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.4 строки 251 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																								|en = 'Indicator value in col. 4 of line 251 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"), Формат(ПоказателиРазделов.П000200025104,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.5 строки 251 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																								|en = 'Indicator value in col. 5 of line 251 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"), Формат(ПоказателиРазделов.П000200025105,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.6 строки 251 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																								|en = 'Indicator value in col. 6 of line 251 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"), Формат(ПоказателиРазделов.П000200025106,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
		...
		Если Разность > ДопустимоеРасхождение Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.3 строки 252 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																							|en = 'Indicator value in col. 3 of line 252 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"), Формат(ПоказателиРазделов.П000200025203,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
		...
		Если Разность > ДопустимоеРасхождение Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.4 строки 252 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																							|en = 'Indicator value in col. 4 of line 252 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"), Формат(ПоказателиРазделов.П000200025204,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
		...
		Если Разность > ДопустимоеРасхождение Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.5 строки 252 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																							|en = 'Indicator value in col. 5 of line 252 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"), Формат(ПоказателиРазделов.П000200025205,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
		...
		Если Разность > ДопустимоеРасхождение Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.6 строки 252 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																							|en = 'Indicator value in col. 6 of line 252 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"), Формат(ПоказателиРазделов.П000200025206,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
		...
		Если ПоказателиРазделов.П000200023103 + ПоказателиРазделов.П000200023203 + ПоказателиРазделов.П000200023303 > 0 И ПоказателиРазделов.П000200026003 = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Если сумма значений гр.3 строк 231, 232, 233 больше 0, то и значение в гр.3 строки 260 раздела 2 должно быть больше 0.';
									|en = 'If value sum of col. 3 in lines 231, 232, 233 is more than 0, value in col. 3 in line 260 section 2 should be more than 0.'");
		...
		Если ПоказателиРазделов.П000200023104 + ПоказателиРазделов.П000200023204 + ПоказателиРазделов.П000200023304 > 0 И ПоказателиРазделов.П000200026004 = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Если сумма значений гр.4 строк 231, 232, 233 больше 0, то и значение в гр.4 строки 260 раздела 2 должно быть больше 0.';
									|en = 'If value sum of col. 4 in lines 231, 232, 233 is more than 0, value in col. 4 in line 260 section 2 should be more than 0.'");
		...
		Если ПоказателиРазделов.П000200023105 + ПоказателиРазделов.П000200023205 + ПоказателиРазделов.П000200023305 > 0 И ПоказателиРазделов.П000200026005 = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Если сумма значений гр.5 строк 231, 232, 233 больше 0, то и значение в гр.5 строки 260 раздела 2 должно быть больше 0.';
									|en = 'If value sum of col. 5 in lines 231, 232, 233 is more than 0, value in col. 5 in line 260 section 2 should be more than 0.'");
		...
		Если ПоказателиРазделов.П000200023106 + ПоказателиРазделов.П000200023206 + ПоказателиРазделов.П000200023306 > 0 И ПоказателиРазделов.П000200026006 = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Если сумма значений гр.6 строк 231, 232, 233 больше 0, то и значение в гр.6 строки 260 раздела 2 должно быть больше 0.';
									|en = 'If value sum of col. 6 in lines 231, 232, 233 is more than 0, value in col. 6 in line 260 section 2 should be more than 0.'");
		...
		Если ПоказателиРазделов.П000200027503 <> ПоказателиРазделов.П000200027103 - ПоказателиРазделов.П000200027203 - ПоказателиРазделов.П000200027303 - ПоказателиРазделов.П000200027403 Тогда
			ТекстСообщения = НСтр("ru = 'Значение показателя в гр.3 строки 275 должно быть равно: строка 271 минус строка 272, минус строка 273, минус строка 274 по гр.3.';
									|en = 'Indicator value in col. 3 of line 275 should be equal to: line 271 minus line 272, minus line 273, minus line 274 of col. 3.'");
		...
		Если ПоказателиРазделов.П000200027504 <> ПоказателиРазделов.П000200027104 - ПоказателиРазделов.П000200027204 - ПоказателиРазделов.П000200027304 - ПоказателиРазделов.П000200027404 Тогда
			ТекстСообщения = НСтр("ru = 'Значение показателя в гр.4 строки 275 должно быть равно: строка 271 минус строка 272, минус строка 273, минус строка 274 по гр.4.';
									|en = 'Indicator value in col. 4 of line 275 should be equal to: line 271 minus line 272, minus line 273, minus line 274 of col. 4.'");
		...
		Если ПоказателиРазделов.П000200027505 <> ПоказателиРазделов.П000200027105 - ПоказателиРазделов.П000200027205 - ПоказателиРазделов.П000200027305 - ПоказателиРазделов.П000200027405 Тогда
			ТекстСообщения = НСтр("ru = 'Значение показателя в гр.5 строки 275 должно быть равно: строка 271 минус строка 272, минус строка 273, минус строка 274 по гр.5.';
									|en = 'Indicator value in col. 5 of line 275 should be equal to: line 271 minus line 272, minus line 273, minus line 274 of col. 5.'");
		...
		Если ПоказателиРазделов.П000200027506 <> ПоказателиРазделов.П000200027106 - ПоказателиРазделов.П000200027206 - ПоказателиРазделов.П000200027306 - ПоказателиРазделов.П000200027406 Тогда
			ТекстСообщения = НСтр("ru = 'Значение показателя в гр.6 строки 275 должно быть равно: строка 271 минус строка 272, минус строка 273, минус строка 274 по гр.6.';
									|en = 'Indicator value in col. 6 of line 275 should be equal to: line 271 minus line 272, minus line 273, minus line 274 of col. 6.'");
		...
		Если СтрокаТарифа <> Неопределено Тогда
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.3 строки 276 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																								|en = 'Indicator value in col. 3 of line 276 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"), Формат(ПоказателиРазделов.П000200027603,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.4 строки 276 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																								|en = 'Indicator value in col. 4 of line 276 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"), Формат(ПоказателиРазделов.П000200027604,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.5 строки 276 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																								|en = 'Indicator value in col. 5 of line 276 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"), Формат(ПоказателиРазделов.П000200027605,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение показателя в гр.6 строки 276 (""%1"") и расчетное значение, определенное исходя из облагаемой базы и тарифа (""%2""), различаются более чем на ""%3"" руб.';
																								|en = 'Indicator value in col. 6 of line 276 (""%1"") and calculated value determined based on the taxable base and rate (""%2"") differ more than by ""%3"" rub.'"), Формат(ПоказателиРазделов.П000200027606,"ЧДЦ=2; ЧН=0.00"), Формат(РасчетноеЗначение,"ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);  
...
Процедура ПроверитьСоответствиеДанныхКомплектаЗаГод(ДанныеРСВ, Ошибки, Отказ)
	...
	Если ДанныеРСВ <> Неопределено И ДанныеРСВ.ФормаРеглОтчетаДействуетВТекущемОтчетномПериоде И ДанныеРСВ.ОтчетныйПериодРСВ1 = ОтчетныйПериод Тогда
		...
		Если ОбъектОбложенияПоФормамСЗВ63 < ДанныеРСВ.УменьшенныйОбъектОбложенияРСВ1 Тогда
			ТекстСообщения = НСтр("ru = 'Общая сумма заработка, указанная в формах СЗВ-6-3 с категориями «НР», «ВЖНР» и «ВПНР», меньше облагаемых взносами в ПФР сумм, указанных в разделе 2 Расчета РСВ-1 с кодами тарифа 01, 52 и 53.';
									|en = 'Total amount of earnings specified in forms SZV-6-3 with categories ""NR"", ""VGNR"", ""VPNR"" is less than the amount subject to PF contributions specified in section 2 of RSV-1 calculation with rate codes 01, 52 and 53. '"); 
		...
		Если ОблагаемаяБазаПоФормамСЗВ63 <> ДанныеРСВ.ОблагаемаяБазаРСВ1 Тогда
			ТекстСообщения = НСтр("ru = 'Общая сумма облагаемой базы, указанная в формах СЗВ-6-3 с категориями «НР», «ВЖНР» и «ВПНР», и сумма показателей колонки ""Всего с начала расчетного периода"" по строкам 240 и 241 раздела 2 Расчета РСВ-1 с кодами тарифа 01, 52 и 53 не совпадают.';
									|en = 'Total amount of taxable base specified in forms SZV-6-3 with categories ""NR"", ""VGNR"", ""VPNR"" and the amount of the ""Total since payroll period start"" column indicators of lines 240 and 241 in section 2 of RSV-1 calculation with rate codes 01, 52 and 53 do not match.'");
			РазделыИтоговСОшибкой = Новый Массив;
		...
		Если ОбъектОбложенияПоФормамСЗВ63Льготный < ДанныеРСВ.УменьшенныйОбъектОбложенияРСВ1Льготный Тогда
			ТекстСообщения = НСтр("ru = 'Общая сумма заработка, указанная в формах СЗВ-6-3 по всем категориям, кроме «НР», «ВЖНР» и «ВПНР», меньше облагаемых взносами в ПФР сумм, указанных в разделе 2 Расчета РСВ-1 по всем кодам тарифа, кроме 01, 52 и 53.';
									|en = 'Total amount of earnings specified in forms SZV-6-3 for all categories other than ""NR"", ""VGNR"", ""VPNR"" is less than the amount subject to PF contributions specified in section 2 of RSV-1 calculation for all rate codes other than 01, 52 and 53. '"); 
			РазделыИтоговСОшибкой = Новый Массив;
		...
		Если ОблагаемаяБазаПоФормамСЗВ63Льготная <> ДанныеРСВ.ОблагаемаяБазаРСВ1Льготная Тогда
			ТекстСообщения = НСтр("ru = 'Общая сумма облагаемой базы, указанная в формах СЗВ-6-3 по всем категориям, кроме «НР», «ВЖНР» и «ВПНР», и сумма показателей колонки ""Всего с начала расчетного периода"" по строкам 240 и 241 раздела 2 Расчета РСВ-1 по всем кодам тарифа, кроме 01, 52 и 53, не совпадают.';
									|en = 'Total amount of taxable base specified in forms SZV-6-3 for all categories other than ""NR"", ""VGNR"", ""VPNR"" and the amount of the ""Total since payroll period start"" column indicators of lines 240 and 241 in section 2 of RSV-1 calculation for all rate codes other than 01, 52 and 53 do not match.'");
			РазделыИтоговСОшибкой = Новый Массив;
		...
		Пока ВыборкаСведенияОЗаработкеСЗВ63ПоФизЛицам.Следующий() Цикл
			...
			Если ПроверяемыйПоказатель = 0 И ВыборкаСведенияОЗаработкеСЗВ63ПоФизЛицам.Заработок <> ВыборкаСведенияОЗаработкеСЗВ63ПоФизЛицам.ОблагаетсяВзносами Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для застрахованного %1, указанного в строке %2  пачки СЗВ-6-3 № %3: общая сумма заработка и общая сумма облагаемой базы, должны быть равны';
																								|en = 'For insured person %1 specified in line %2 of SZV-6-3 bundle No. %3: total salary amount and total amount of the taxable base, must be equal to '"), 
		...
		Если ОбъектОбложенияПоФормамСЗВ63 <> ДанныеРСВ.ОбъектОбложенияРСВ1 Тогда 
			ТекстСообщения =   НСтр("ru = 'Сумма показателей строк «201», «202», «203» формы РСВ-1 должна быть равна сумме значений итоговых строк в графе «Сумма выплат и иных вознаграждений, начисленных в пользу физического лица» по всем формам СЗВ-6-3.
                                                             ...
                                                             |en = 'Sum of indicators of lines 201, 202, 203 of RSV-1 form should be equal to the sum of values of total lines of column ""Amount of payments and other remunerations accrued to the individual"" for all SZV-6-3 forms. 
                                                             |It is possible not to meet the condition if there are payments to wage earners for whom SZV-6-3 forms are not submitted: payments to military personnel temporary staying in RF set forth in subcl. 14 and 15 of part 2 art. 9 of Federal Law dated July 24, 2009 No. 212-FZ; payments to prosecutors, investigators, judges set forth in cl. 1 of part 3 art. 9 of the above mentioned Federal Law, including from amounts exceeding the limit base for insurance contribution accrual. In other cases failure to meet the condition is a mistake and should be corrected.'");
	...
	Пока ВыборкаПоСЗВ63.Следующий() Цикл
		Если ВыборкаПоСЗВ63.ТипСведенийСЗВ = Перечисления.ТипыСведенийСЗВ.ИСХОДНАЯ И ВыборкаПоСЗВ63.ОтчетныйПериод <> Год(ОтчетныйПериод) Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Исходная пачка СЗВ-6-3 № %1 содержит данные за другой отчетный период.';
																							|en = 'Original bundle SZV-6-3 No. %1 contains the data for another accounting period.  '")
	...
	Если ВыборкаСверкиКоличества.КоличествоФормСЗВ_6_3 < ВыборкаСверкиКоличества.КоличествоЗастрахованныхСЗВ_6_12 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Количество исходных форм СЗВ-6-3 меньше количества исходных форм СЗВ-6-1(2), содержащих ненулевые начисленные взносы.';
								|en = 'The number of original SZV-6-3 forms is less than the number of original SZV-6-1(2) forms containing non-zero accrued contributions.'");
	...
	Пока ВыборкаСверкиЗастрахованных.Следующий() Цикл
		...
		НачалоСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для застрахованного %1, указанного в строке %2 пачки СЗВ-6-1(2) № %3:';
																						|en = 'For insured person %1 specified in line %2 of SZV-6-1(2) bundle No. %3:'")
		...
		Если ВыборкаСверкиЗастрахованных.ОтсутствуетФормаСЗВ63 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 отсутствуют формы СЗВ-6-3.';
																							|en = '%1 SZV-6-4 forms are missing.'"), НачалоСообщения);	
		...
		Иначе 
			...
			НачалоСообщенияНесовпадения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 в пачке СЗВ-6-3 № %2 не совпадает';
																										|en = '%1 does not match in bundle SZV-6-3 No. %2'"), НачалоСообщения, ВыборкаСверкиЗастрахованных.НомерПачкиСЗВ_6_3); 
			Если ВыборкаСверкиЗастрахованных.НеСовпадаетСНИЛС Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 СНИЛС.';
																								|en = '%1 SNILS.'"), НачалоСообщенияНесовпадения);
			...
			Если ВыборкаСверкиЗастрахованных.НеСовпадаетФамилия Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 фамилия.';
																								|en = '%1 last name.'"), НачалоСообщенияНесовпадения);
			...
			Если ВыборкаСверкиЗастрахованных.НеСовпадаетИмя Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '% 1имя.';
																								|en = '% 1name.'"), НачалоСообщенияНесовпадения);
			...
			Если ВыборкаСверкиЗастрахованных.НеСовпадаетОтчество Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 отчество.';
																								|en = '%1 patronymic.'"), НачалоСообщенияНесовпадения);
			...
			Если СтруктураПоиска.Ссылка <> ВыборкаСверкиЗастрахованных.СЗВ61Ссылка Или СтруктураПоиска.НомерСтрокиСЗВ6_12 <> ВыборкаСверкиЗастрахованных.НомерСтрокиСЗВ6_12 Тогда
				...
				Если ВыборкаСверкиВзносов.НайтиСледующий(СтруктураПоиска) Тогда
					Если ВыборкаСверкиВзносов.РасхождениеВоВзносах > 0.03 Тогда						
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 сумма начисленных взносов, указанная в форме СЗВ-6-1,2 (%2), отличается от суммы взносов, исчисленной по всем формам СЗВ-6-3 (%3), более чем на 3 копейки - величины, допустимой правилами проверки сведений персонифицированного учета.';
																										|en = '%1 accrued contribution amount specified in form SZV-6-1,2 (%2) differs from contribution amount of all forms SZV-6-3 (%3) by more than 3 copecks, that is the value allowed by the rules of personal record-keeping information check.'"), НачалоСообщения, Формат(ВыборкаСверкиВзносов.НачисленоВзносов,"ЧДЦ=2; ЧН=0; ЧГ=0"), Формат(ВыборкаСверкиВзносов.ВзносовКНачислению,"ЧДЦ=2; ЧН=0.00; ЧГ=0"));
	...
	Пока ВыборкаСверкиВзносовЗаГод.Следующий() Цикл
		Если ВыборкаСверкиВзносовЗаГод.РасхождениеВоВзносах > 0.03 Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для застрахованного %1: сумма начисленных взносов, указанная в формах СЗВ-6-1,2 текущего года по категории ЗЛ %2 (%3), отличается от суммы взносов, исчисленной по всем формам СЗВ-6-3 этой же категории (%4), более чем на 3 копейки - величины, допустимой правилами проверки сведений персонифицированного учета.';
																							|en = 'For insured person %1: the accrued contribution amount specified in forms SZV-6-1,2 of the current year for insured person category %2 (%3) differs from the contribution amount of all forms SZV-6-3 of the same category (%4) by more than 3 copecks, that is the value allowed by the rules of personal record-keeping information check.'"), 
...
Процедура ПроверитьСоответствиеДанныхПачекДаннымРСВЗа2012(ДанныеРСВ, Ошибки, Отказ = Ложь)
	...
	Если ДанныеРСВ <> Неопределено И ДанныеРСВ.ФормаРеглОтчетаДействуетВТекущемОтчетномПериоде И ДанныеРСВ.ОтчетныйПериодРСВ1 <> ОтчетныйПериод Тогда
		ТекстСообщения = НСтр("ru = 'Период Расчета РСВ-1 не соответствует отчетному периоду Описи АДВ-6-2';
								|en = 'RSV-1 calculation period does not correspond to the accounting period of ADV-6-2 list'");
	...
	ИначеЕсли ДанныеРСВ <> Неопределено И ДанныеРСВ.ФормаРеглОтчетаДействуетВТекущемОтчетномПериоде Тогда	
		...
		Если Разность > ДопустимоеРасхождение Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Разница суммы начисленных взносов на страховую часть пенсии в Расчете РСВ-1 (%1) и Описи АДВ-6-2 (%2) больше ""%3 руб. - величины, допустимой правилами проверки сведений персонифицированного учета';
																							|en = 'Difference between the amount of contributions charged for insurance part of pension in RSV-1 calculation (%1) and ADV-6-2 list (%2) is more than ""%3 rub. - value allowed by the rules of checking personal record-keeping information'"), Формат(ДанныеРСВ.НачисленоРСВ1НаСтраховую, "ЧДЦ=2; ЧН=0.00"), Формат(ДанныеАДВ62.НачисленоСтраховая - ДанныеАДВ62.НачисленоЧленамЭкипажейСудовНаСтраховую, "ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);
		...
		Если Разность > ДопустимоеРасхождение Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Разница суммы начисленных взносов на накопительную часть пенсии в Расчете РСВ-1 (%1) и Описи АДВ-6-2 (%2) больше ""%3 руб. - величины, допустимой правилами проверки сведений персонифицированного учета';
																							|en = 'Difference between the amount of contributions charged for funded component of pension in RSV-1 calculation (%1) and ADV-6-2 list (%2) is more than %3 rub. - value allowed by the rules of checking personal record-keeping information'"), Формат(ДанныеРСВ.НачисленоРСВ1НаНакопительную, "ЧДЦ=2; ЧН=0.00"), Формат(ДанныеАДВ62.НачисленоНакопительная - ДанныеАДВ62.НачисленоЧленамЭкипажейСудовНаНакопительную, "ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);
		...
		Если ДанныеРСВ.НачисленоРСВ1НаСтраховуюПоКатегориям[Перечисления.КатегорииЗастрахованныхЛицДляПФР.НР] <> Неопределено Тогда
			Если ДанныеРСВ.УплаченоРСВ1НаСтраховую + 1 < ДанныеАДВ62.УплаченоСтраховая Тогда
				ТекстСообщения = НСтр("ru = 'Суммы уплаченных взносов на страховую часть пенсии в Описи АДВ-6-2 превысила сумму уплаченных взносов на страховую часть в Расчете РСВ-1.';
										|en = 'The amount of contributions paid to the insurance part of pension in ADV-6-2 sheet exceeds the amount of contributions paid to the insurance part in RSV-1 Calculation.     '");
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Разница суммы уплаченных взносов на страховую часть пенсии в Расчете РСВ-1 (%2) и Описи АДВ-6-2 (%3) больше %1 руб. - величины, допустимой правилами проверки сведений персонифицированного учета';
										|en = 'Difference between the amount of contributions charged for insurance part of pension in RSV-1 calculation (%2) and ADV-6-2 list (%3) is more than %1 rub. - value allowed by the rules of checking personal record-keeping information'"),  
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Разница суммы уплаченных взносов на накопительную часть пенсии в Расчете РСВ-1 (%2) и Описи АДВ-6-2 (%3) больше %1 руб. - величины, допустимой правилами проверки сведений персонифицированного учета';
										|en = 'Difference between the amount of contributions charged for funded component of pension in RSV-1 calculation (%2) and ADV-6-2 list (%3) is more than %1 rub. - value allowed by the rules of checking personal record-keeping information'"),  
									ДопустимоеРасхождение,
		...
		Если ЗначениеЗаполнено(СтрокаКатегорий) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отсутствуют документы СЗВ-6-1,2 по категориям %1, соответствующим указанным в Расчете РСВ-1 кодам тарифа.';
																							|en = 'No SZV-6-1,2 documents of the %1 categories which correspond to ones specified in RSV-1 calculation to the rate codes.'"),  СтрокаКатегорий);
		...
		Для каждого Элемент Из ДанныеАДВ62.НачисленоСтраховаяПоКатегориям Цикл
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Разница суммы начисленных взносов на страховую часть пенсии в Расчете РСВ-1 (%1) и документах СЗВ-6-1,2 по категориям %2 (%3) больше %4 руб. - величины, допустимой правилами проверки сведений персонифицированного учета';
																								|en = 'Difference between the amount of contributions charged for insurance part of pension in RSV-1 calculation (%1) and SZV-6-1,2 documents by categories %2 (%3) is more than %4 rub. - value allowed by the rules of checking personal record-keeping information'"), Формат(ДанныеРСВ.НачисленоРСВ1НаСтраховуюПоКатегориям[Элемент.Ключ], "ЧДЦ=2; ЧН=0.00"), ДочерниеКатегории(Элемент.Ключ, СоответствиеКатегорий), Формат(Элемент.Значение, "ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);   
			...
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Разница суммы начисленных взносов на накопительную часть пенсии в Расчете РСВ-1 (%1) и документах СЗВ-6-1,2 по категориям %2 (%3) больше %4 руб. - величины, допустимой правилами проверки сведений персонифицированного учета';
																								|en = 'Difference between the amount of contributions charged for funded component of pension in RSV-1 calculation (%1) and SZV-6-1,2 documents by categories %2 (%3) is more than %4 rub. - value allowed by the rules of checking personal record-keeping information'"), Формат(ДанныеРСВ.НачисленоРСВ1НаНакопительнуюПоКатегориям[Элемент.Ключ], "ЧДЦ=2; ЧН=0.00"), ДочерниеКатегории(Элемент.Ключ, СоответствиеКатегорий), Формат(ДанныеАДВ62.НачисленоНакопительнаяПоКатегориям[Элемент.Ключ], "ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);   
		...
		КонецЦикла;
		Если ЗначениеЗаполнено(СтрокаКатегорий) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В Расчете РСВ-1 отсутствует Раздел 2 с кодом тарифа, соответствующим категориям %1.';
																							|en = 'No Section 2 with tariff code corresponding to the %1 categories in RSV-1 calculation.'"), СтрокаКатегорий);   
	...
	ИначеЕсли ДанныеРСВ <> Неопределено И ДанныеРСВ.ФормаРеглОтчетаДействуетВТекущемОтчетномПериоде Тогда	
		...
		Если ДанныеАДВ62.Заработок + 1 < ДанныеРСВ.УменьшенныйОбъектОбложенияРСВ1 Тогда
			ТекстСообщения = НСтр("ru = 'Общая сумма заработка, указанная в формах СЗВ-6-4 с категориями «НР», «ВЖНР» и «ВПНР», меньше облагаемых взносами в ПФР сумм, указанных в разделе 2 Расчета РСВ-1 с кодами тарифа 01, 52 и 53.';
									|en = 'Total amount of earnings specified in forms SZV-6-4 with categories ""NR"", ""VGNR"", ""VPNR"" is less than the amount subject to PF contributions specified in section 2 of RSV-1 calculation with rate codes 01, 52 and 53.'"); 
			РазделыИтоговСОшибкой = Новый Массив;
		...
		Если Разность > ДопустимоеРасхождение Тогда
			...
			ТекстСообщения = НСтр("ru = 'Общая сумма облагаемой базы, указанная в формах СЗВ-6-4 с категориями «НР», «ВЖНР» и «ВПНР» и сумма показателей колонок ""1, 2 и 3 месяц"" по строкам 240, 241, 231, 232 и 233 раздела 2 Расчета РСВ-1 с кодами тарифа 01, 52 и 53 не совпадают.';
									|en = 'Total amount of taxable base specified in forms SZV-6-4 with categories ""NR"", ""VGNR"", ""VPNR"" and the amount of the ""1, 2 and 3 month"" column indicators of lines 240 241, 231, 232 and 233 in section 2 of RSV-1 calculation with rate codes 01, 52 and 53 do not match.'");
			РазделыИтоговСОшибкой = Новый Массив;
		...
		Если ДанныеАДВ62.ЗаработокПоЛьготнымКатегориям + 1 < ДанныеРСВ.УменьшенныйОбъектОбложенияРСВ1Льготный Тогда
			ТекстСообщения = НСтр("ru = 'Общая сумма заработка, указанная в формах СЗВ-6-4 по всем категориям, кроме «НР», «ВЖНР» и «ВПНР», меньше облагаемых взносами в ПФР сумм, указанных в разделе 2 Расчета РСВ-1 по всем кодам тарифа, кроме 01, 52 и 53.';
									|en = 'Total amount of earnings specified in forms SZV-6-4 for all categories other than ""NR"", ""VGNR"", ""VPNR"" is less than the amount subject to PF contributions specified in section 2 of RSV-1 calculation for all rate codes other than 01, 52 and 53.'"); 
			РазделыИтоговСОшибкой = Новый Массив;
		...
		Если Разность > ДопустимоеРасхождение Тогда
			ТекстСообщения = НСтр("ru = 'Общая сумма облагаемой базы, указанная в формах СЗВ-6-4 по всем категориям, кроме «НР», «ВЖНР» и «ВПНР», и сумма показателей колонок ""1, 2 и 3 месяц"" по строкам 240 и 241 раздела 2 Расчета РСВ-1 по всем кодам тарифа, кроме 01, 52 и 53, не совпадают.';
									|en = 'Total amount of taxable base specified in forms SZV-6-4 for all categories other than ""NR"", ""VGNR"", ""VPNR"" and the amount of the ""1, 2 and 3 month"" column indicators of lines 240 and 241 in section 2 of RSV-1 calculation for all rate codes other than 01, 52 and 53 do not match.'");
			РазделыИтоговСОшибкой = Новый Массив;
			...
			Если ПроверяемыйПоказатель = 0 И ВыборкаСведенияОЗаработкеСЗВ64ПоФизЛицам.Заработок <> ВыборкаСведенияОЗаработкеСЗВ64ПоФизЛицам.ОблагаетсяВзносами Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Для застрахованного %1, указанного в строке %2  пачки СЗВ-6-4 № %3: общая сумма заработка и общая сумма облагаемой базы, должны быть равны';
										|en = 'For insured person %1 specified in line %2 of SZV-6-4 bundle No. %3: total salary amount and total amount of the taxable base, must be equal to'"), 
		...
		Если ДанныеАДВ62.Заработок + ДанныеАДВ62.ЗаработокПоЛьготнымКатегориям <> ДанныеРСВ.ОбъектОбложенияРСВ1 + ДанныеРСВ.ОбъектОбложенияРСВ1Льготный Тогда 
			ТекстСообщения =  НСтр("ru = 'Сумма показателей строк «201», «202», «203» формы РСВ-1 должна быть равна сумме значений итоговых строк в графе «Сумма выплат и иных вознаграждений, начисленных в пользу физического лица» по всем формам СЗВ-6-4.
									...
									|en = 'Sum of indicators of lines 201, 202, 203 of RSV-1 form should be equal to the sum of values of total lines of column ""Amount of payments and other remunerations accrued to the individual"" for all SZV-6-4 forms. 
									|It is possible not to meet the condition if there are payments to wage earners for whom SZV-6-4 forms are not submitted: payments to military personnel set forth in subcl. 14 of part 2 art. 9 of Federal Law dated July 24, 2009 No. 212-FZ; payments to prosecutors, investigators, judges set forth in cl. 1 of part 3 art. 9 of the above mentioned Federal Law, including from amounts exceeding the limit base for insurance contribution accrual. In other cases failure to meet the condition is a mistake and should be corrected.'");
		...
		Если ДанныеАДВ62.ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботах <> ДанныеРСВ.ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботах Тогда
			ТекстСообщения =  НСтр("ru = 'Общая сумма облагаемой базы взносов по дополнительному тарифу за занятых на вредных и подземных работах (подпункт 1 пункта 1 статьи 27 Федерального закона «О трудовых пенсиях в Российской Федерации»), указанная в формах СЗВ-6-4, и сумма показателей колонок ""1, 2 и 3 месяц"" по строке 283 раздела 2.2 Расчета РСВ-1 не совпадают.';
									|en = 'Total amount of taxable base of additional rate contributions for people employed at underground and harmful works (subclause 1 clause 1 of article 27 of the Federal Law ""On Labor Pensions in the Russian Federation"") specified in forms SZV-6-4 and the amount of indicators in the ""1,2 and 3 month"" columns of line 283, section 2.2 of RSV-1 calculation do not match.'");
		...
		Если ДанныеАДВ62.ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботах <> ДанныеРСВ.ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботах Тогда
			ТекстСообщения =  НСтр("ru = 'Общая сумма облагаемой базы взносов по дополнительному тарифу за занятых на тяжелых работах (подпункты 2-18 пункта 1 статьи 27 Федерального закона «О трудовых пенсиях в Российской Федерации»), указанная в формах СЗВ-6-4 , и сумма показателей колонок ""1, 2 и 3 месяц"" по строке 293 раздела 2.3 Расчета РСВ-1 не совпадают.';
									|en = 'Total amount of taxable base of additional rate contributions for people involved in arduous works (subclauses 2-18, clause 1 of article 27 of the Federal Law ""On Labor Pensions in the Russian Federation"") specified in forms SZV-6-4 and the amount of indicators in the ""1,2 and 3 month"" columns of line 293, section 2.3 of RSV-1 calculation do not match.'");
		...
		КонецЦикла;
		Если ЗначениеЗаполнено(СтрокаКатегорий) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отсутствуют документы СЗВ-6-4 по категориям %1, соответствующим указанным в Расчете РСВ-1 кодам тарифа.';
																							|en = 'No SZV-6-4 documents of the %1 categories which correspond to ones specified in RSV-1 calculation to the rate codes.'"),  СтрокаКатегорий);
		...
		Для каждого Элемент Из ДанныеАДВ62.НачисленоСтраховаяПоКатегориям Цикл
			...
			Разность = Макс(ДанныеРСВ.НачисленоРСВ1НаСтраховуюПоКатегориям[Элемент.Ключ] - Элемент.Значение, Элемент.Значение - ДанныеРСВ.НачисленоРСВ1НаСтраховуюПоКатегориям[Элемент.Ключ]);
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Разница суммы начисленных взносов на страховую часть пенсии в Расчете РСВ-1 (%1) и документах СЗВ-6-4 по категориям %2 (%3) больше %4 руб. - величины, допустимой правилами проверки сведений персонифицированного учета';
																								|en = 'Difference between the amount of contributions charged for insurance part of pension in RSV-1 calculation (%1) and SZV-6-4 documents by categories %2 (%3) is more than %4 rub. - value allowed by the rules of checking personal record-keeping information'"), Формат(ДанныеРСВ.НачисленоРСВ1НаСтраховуюПоКатегориям[Элемент.Ключ], "ЧДЦ=2; ЧН=0.00"), ДочерниеКатегории(Элемент.Ключ, СоответствиеКатегорий), Формат(Элемент.Значение, "ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);   
				
				ПерсонифицированныйУчетКлиентСервер.ДобавитьОшибкуСверкиВзносовРСВ1СОписьюПоКатегории(Ошибки, Элемент.Ключ, ТекстСообщения, ДанныеАДВ62, ДанныеРСВ, "НачисленоСтраховая", Отказ);
			КонецЕсли;
			Разность = Макс(ДанныеРСВ.НачисленоРСВ1НаНакопительнуюПоКатегориям[Элемент.Ключ] - ДанныеАДВ62.НачисленоНакопительнаяПоКатегориям[Элемент.Ключ], ДанныеАДВ62.НачисленоНакопительнаяПоКатегориям[Элемент.Ключ] - ДанныеРСВ.НачисленоРСВ1НаНакопительнуюПоКатегориям[Элемент.Ключ]);
			Если Разность > ДопустимоеРасхождение Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Разница суммы начисленных взносов на накопительную часть пенсии в Расчете РСВ-1 (%1) и документах СЗВ-6-4 по категориям %2 (%3) больше %4 руб. - величины, допустимой правилами проверки сведений персонифицированного учета';
																								|en = 'Difference between the amount of contributions charged for funded component of pension in RSV-1 calculation (%1) and SZV-6-4 documents by categories %2 (%3) is more than %4 rub. - value allowed by the rules of checking personal record-keeping information'"), Формат(ДанныеРСВ.НачисленоРСВ1НаНакопительнуюПоКатегориям[Элемент.Ключ], "ЧДЦ=2; ЧН=0.00"), ДочерниеКатегории(Элемент.Ключ, СоответствиеКатегорий), Формат(ДанныеАДВ62.НачисленоНакопительнаяПоКатегориям[Элемент.Ключ], "ЧДЦ=2; ЧН=0.00"), ДопустимоеРасхождение);   
	...
	Если ДанныеРСВ.КоличествоЗастрахованных <> ДанныеРазделов6.КоличествоЗастрахованныхЛиц Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Количество застрахованных лиц, указанное на титульном листе Расчета РСВ-1 (%1) не совпадает с количеством лиц, на которых подаются сведения перс.учета (%2).';
							|en = 'Number of insured persons specified on the cover page of the RSV-1 Calculation (%1) does not match the number of persons for whom personal record-keeping information is submitted (%2).'"),
	...
	Если ДанныеРСВ.НачисленоРСВ1НаСтраховую <> ДанныеРазделов6.НачисленоВзносов Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Сумма начисленных взносов на ОПС в разделе 1 Расчета РСВ-1 (%1) не совпадает с данными перс.учета (%2).';
							|en = 'Amount of accrued contributions for MPI in section 1 of RSV-1 calculation (%1) does not match the personal record-keeping data (%2).'"),
	...
	Если ДанныеРазделов6.ЗаработокЗа1Месяц < ДанныеРСВ.ОбъектОбложенияРСВ1За1Месяц Тогда 
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Общая сумма выплат и иных вознаграждений, указанная за первый месяц отчетного периода в разделах 2.1 Расчета РСВ-1, уменьшенная на суммы, не подлежащие обложению, (%1) не может быть больше общей суммы выплат и иных вознаграждений по данным перс.учета (%2)';
							|en = 'Total amount of payments and remunerations specified for the first month of the accounting period in section 2.1 of RSV-1 calculation decreased by amount not subject to taxation (%1) cannot be more than the total amount of payments and other remunerations according to personal record-keeping data (%2).'"),
	...
	Если ДанныеРазделов6.ОблагаетсяВзносамиДоПредельнойВеличиныЗа1Месяц <> ДанныеРСВ.ОблагаемаяБазаРСВ1За1Месяц Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Общая сумма облагаемой базы, не превышающей предельной величины, указанная за первый месяц отчетного периода в разделах 2.1 Расчета РСВ-1 (%1) не совпадает с данными перс.учета (%2)';
								|en = 'Total amount of taxable base which does not exceed the limit value specified for the first month of the accounting period in sections 2.1 of RSV-1 calculation (%1) does not match personal record-keeping data (%2)'"),
	...
	Если ДанныеРазделов6.ОблагаетсяВзносамиСвышеПредельнойВеличиныЗа1Месяц <> ДанныеРСВ.БазаСверхПредельнойРСВ1За1Месяц Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Общая сумма облагаемой базы, превышающей предельную величину, указанная за первый месяц отчетного периода в разделах 2.1 Расчета РСВ-1 (%1) не совпадает с данными перс.учета (%2)';
								|en = 'Total amount of taxable base which exceeds the limit value specified for the first month of the accounting period in sections 2.1 of RSV-1 calculation (%1) does not match personal record-keeping data (%2)'"),
	...
	Если ДанныеРазделов6.ЗаработокЗа2Месяц < ДанныеРСВ.ОбъектОбложенияРСВ1За2Месяц Тогда 
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Общая сумма выплат и иных вознаграждений, указанная за второй месяц отчетного периода в разделах 2.1 Расчета РСВ-1, уменьшенная на суммы, не подлежащие обложению, (%1) не может быть больше общей суммы выплат и иных вознаграждений по данным перс.учета (%2)';
							|en = 'Total amount of payments and remunerations specified for the second month of the accounting period in section 2.1 of RSV-1 calculation decreased by the amount not subject to taxation (%1) cannot be more than the total amount of payments and other remunerations according to personal record-keeping data (%2).'"),
	...
	Если ДанныеРазделов6.ОблагаетсяВзносамиДоПредельнойВеличиныЗа2Месяц <> ДанныеРСВ.ОблагаемаяБазаРСВ1За2Месяц Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Общая сумма облагаемой базы, не превышающей предельной величины, указанная за второй месяц отчетного периода в разделах 2.1 Расчета РСВ-1 (%1) не совпадает с данными перс.учета (%2)';
								|en = 'Total amount of taxable base which does not exceed the limit value specified for the second month of the accounting period in sections 2.1 of RSV-1 calculation (%1) does not match personal record-keeping data (%2)'"),
	...
	Если ДанныеРазделов6.ОблагаетсяВзносамиСвышеПредельнойВеличиныЗа2Месяц <> ДанныеРСВ.БазаСверхПредельнойРСВ1За2Месяц Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Общая сумма облагаемой базы, превышающей предельную величину, указанная за второй месяц отчетного периода в разделах 2.1 Расчета РСВ-1 (%1) не совпадает с данными перс.учета (%2)';
								|en = 'Total amount of taxable base which exceeds the limit value specified for the second month of the accounting period in sections 2.1 of RSV-1 calculation (%1) does not match personal record-keeping data (%2)'"),
	...
	Если ДанныеРазделов6.ЗаработокЗа3Месяц < ДанныеРСВ.ОбъектОбложенияРСВ1За3Месяц Тогда 
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Общая сумма выплат и иных вознаграждений, указанная за третий месяц отчетного периода в разделах 2.1 Расчета РСВ-1, уменьшенная на суммы, не подлежащие обложению, (%1) не может быть больше общей суммы выплат и иных вознаграждений по данным перс.учета (%2)';
							|en = 'Total amount of payments and remunerations specified for the third month of the accounting period in section 2.1 of RSV-1 calculation decreased by amount not subject to taxation (%1) cannot be more than the total amount of payments and other remunerations according to personal record-keeping data (%2).'"),
	...
	Если ДанныеРазделов6.ОблагаетсяВзносамиДоПредельнойВеличиныЗа3Месяц <> ДанныеРСВ.ОблагаемаяБазаРСВ1За3Месяц Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Общая сумма облагаемой базы, не превышающей предельной величины, указанная за третий месяц отчетного периода в разделах 2.1 Расчета РСВ-1 (%1) не совпадает с данными перс.учета (%2)';
								|en = 'Total amount of taxable base which does not exceed the limit value specified for the third month of the accounting period in sections 2.1 of RSV-1 calculation (%1) does not match personal record-keeping data (%2)'"),
	...
	Если ДанныеРазделов6.ОблагаетсяВзносамиСвышеПредельнойВеличиныЗа3Месяц <> ДанныеРСВ.БазаСверхПредельнойРСВ1За3Месяц Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Общая сумма облагаемой базы, превышающей предельную величину, указанная за третий месяц отчетного периода в разделах 2.1 Расчета РСВ-1 (%1) не совпадает с данными перс.учета (%2)';
								|en = 'Total amount of taxable base which exceeds the limit value specified for the third month of the accounting period in sections 2.1 of RSV-1 calculation (%1) does not match personal record-keeping data (%2)'"),
		...
		Если СтрокаРСВ1[ИмяПоляСтрокиТаблицы] <> СтрокаПерсучета[ИмяПоляСтрокиТаблицы] Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Общая сумма базы для начисления взносов, указанная за %1 месяц отчетного периода в разделе 2.2 Расчета РСВ-1 (%2) не совпадает с данными перс.учета (%3).';
									|en = 'Total amount of contribution accrual base specified for %1 month of the accounting period in section 2.2 of RSV-1 calculation (%2) does not match personal record-keeping data (%3). '"),
								ОписаниеМесяца,
		...
		Если СтрокаРСВ1[ИмяПоляСтрокиТаблицы] <> СтрокаПерсучета[ИмяПоляСтрокиТаблицы] Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Общая сумма базы для начисления взносов, указанная за %1 месяц отчетного периода в разделе 2.3 Расчета РСВ-1 (%2) не совпадает с данными перс.учета (%3).';
									|en = 'Total amount of contribution accrual base specified for %1 month of the accounting period in section 2.3 of RSV-1 calculation (%2) does not match personal record-keeping data (%3). '"),
								ОписаниеМесяца,
								Формат(СтрокаРСВ1[ИмяПоляСтрокиТаблицы], "ЧДЦ=2; ЧН=0"),
								Формат(СтрокаПерсучета[ИмяПоляСтрокиТаблицы], "ЧДЦ=2; ЧН=0"));
			...
			Если СтрокаРСВ1[ИмяПоляСтрокиТаблицы] <> СтрокаПерсучета[ИмяПоляСтрокиТаблицы] Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Общая сумма базы для начисления взносов, указанная за %1 месяц отчетного периода в разделе 2.4 с кодом основания «1» по %2 условиям труда Расчета РСВ-1 (%3) не совпадает с данными перс.учета (%4).';
										|en = 'Total amount of contribution accrual base specified for %1 month of the accounting period in section 2.4 with the ""1"" basis code of the %2 working conditions of RSV-1 calculation (%3) does not match personal record-keeping data (%4). '"),
									ОписаниеМесяца,
			...
			Если СтрокаРСВ1[ИмяПоляСтрокиТаблицы] <> СтрокаПерсучета[ИмяПоляСтрокиТаблицы] Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Общая сумма базы для начисления взносов, указанная за %1 месяц отчетного периода в разделе 2.4 с кодом основания «2» по %2 условиям труда Расчета РСВ-1 (%3) не совпадает с данными перс.учета (%4).';
										|en = 'Total amount of contribution accrual base specified for %1 month of the accounting period in section 2.4 with the ""2"" basis code of the %2 working conditions of RSV-1 calculation (%3) does not match personal record-keeping data (%4).'"),
									ОписаниеМесяца,
									ОписаниеУсловийТруда,
									Формат(СтрокаРСВ1[ИмяПоляСтрокиТаблицы], "ЧДЦ=2; ЧН=0"),
									Формат(СтрокаПерсучета[ИмяПоляСтрокиТаблицы], "ЧДЦ=2; ЧН=0"));
	...
	Если ЗначениеЗаполнено(СтрокаКатегорий) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Отсутствуют Разделы 6 по категориям %1, соответствующим указанным в Расчете РСВ-1 кодам тарифа.';
							|en = 'No Sections 6 of the %1 categories which correspond to ones specified in RSV-1 calculation to the rate codes.'"),
						СтрокаКатегорий);
	...
	Если ЗначениеЗаполнено(СтрокаКатегорий) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'В Расчете РСВ-1 отсутствует Раздел 2 с кодом тарифа, соответствующим категориям %1.';
							|en = 'No Section 2 with tariff code corresponding to the %1 categories in RSV-1 calculation.'"),
						СтрокаКатегорий);
...
Процедура ПроверитьСоответствиеИсходныхИКорректируемыхСведений(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		ШаблонОшибки = НСтр("ru = 'Сотрудник %1: при наличии корректирующих сведений, данные по сотруднику обязательно должны присутствовать в пачках исходных сведений Раздела 6.';
							|en = 'Employee %1: if there is adjusting information, data on the employee must be in the original information bundles of Section 6. '");
