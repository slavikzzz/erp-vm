...
Процедура ДобавлениеРеквизитов(Сообщение, ДобавляемыеРеквизиты)
	...
	Реквизит = Новый РеквизитФормы("Сообщение" + Идентификатор, Новый ОписаниеТипов("СправочникСсылка.ВЕТИСПрисоединенныеФайлы"), "", НСтр("ru = 'Сообщение';
																																			|en = 'Сообщение'"));
...
Процедура ДобавлениеЭлементов(СтрокаТЧ, РеквизитыСообщений)
	...
	Страница = ДобавитьСтраницу("Страница",СтрокаТЧ.Заголовок,Элементы.Страницы,Идентификатор);
...
Функция АнализСообщенийПередачиДанных(Сообщение, Операция = Неопределено)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВЕТИСПрисоединенныеФайлы КАК ФайлОснование
	|		ПО ФайлОснование.Документ = ВЕТИСПрисоединенныеФайлы.Документ
	|ГДЕ
	...
	|
	|	ЕСТЬNULL(ОтветыНаПередачуДанных.Ссылка, &ПустаяСсылка)            КАК ОтветНаЗапрос,
	|	ЕСТЬNULL(ОтветыНаПередачуДанных.Операция, &ПустаяОперация)        КАК ОтветНаЗапросОперация,
	|	ЕСТЬNULL(ОтветыНаПередачуДанных.ДатаСоздания, ДатаВремя(1,1,1))   КАК ОтветНаЗапросДатаСоздания,
	|	ЕСТЬNULL(ОтветыНаПередачуДанных.ТипСообщения, Неопределено)       КАК ОтветНаЗапросТипСообщения,
	|	ЕСТЬNULL(ОтветыНаПередачуДанных.СтатусОбработки, Неопределено)    КАК ОтветНаЗапросСтатусОбработки,
	|
	|	ЕСТЬNULL(ПолученныеРезультатыЗапросовВЕТИС.Ссылка, &ПустаяСсылка)            КАК ПолученРезультатЗапросаВЕТИС,
	|	ЕСТЬNULL(ПолученныеРезультатыЗапросовВЕТИС.Операция, &ПустаяОперация)        КАК ПолученРезультатЗапросаВЕТИСОперация,
	|	ЕСТЬNULL(ПолученныеРезультатыЗапросовВЕТИС.ДатаСоздания, ДатаВремя(1,1,1))   КАК ПолученРезультатЗапросаВЕТИСДатаСоздания,
	|	ЕСТЬNULL(ПолученныеРезультатыЗапросовВЕТИС.ТипСообщения, Неопределено)       КАК ПолученРезультатЗапросаВЕТИСТипСообщения,
	...
	|	ЕСТЬNULL(ОтветыНаЗапросыВЕТИС.Операция, &ПустаяОперация)       КАК ОтветНаЗапросВЕТИСОперация,
	|	ЕСТЬNULL(ОтветыНаЗапросыВЕТИС.ДатаСоздания, ДатаВремя(1,1,1))  КАК ОтветНаЗапросВЕТИСДатаСоздания,
	|	ЕСТЬNULL(ОтветыНаЗапросыВЕТИС.ТипСообщения, Неопределено)      КАК ОтветНаЗапросВЕТИСТипСообщения,
	...
	|		ПО Сообщения.Ссылка = ПолученныеРезультатыЗапросовВЕТИС.СообщениеОснование
	|		И (ПолученныеРезультатыЗапросовВЕТИС.ТипСообщения = ЗНАЧЕНИЕ(Перечисление.ТипыЗапросовИС.Входящий))
	...
	|		ПО Сообщения.Ссылка = ОтветыНаЗапросыВЕТИС.СообщениеОснование
	|		И (ОтветыНаЗапросыВЕТИС.ТипСообщения = ЗНАЧЕНИЕ(Перечисление.ТипыЗапросовИС.Входящий))
	...
	|	ИЛИ ЕСТЬNULL(ОтветыНаПередачуДанных.Ссылка,   &ПустаяСсылка) = &ФайлОснование
	|	ИЛИ ЕСТЬNULL(ПолученныеРезультатыЗапросовВЕТИС.Ссылка,   &ПустаяСсылка) = &ФайлОснование
	|	ИЛИ ЕСТЬNULL(ОтветыНаЗапросыВЕТИС.Ссылка,  &ПустаяСсылка)  = &ФайлОснование)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ПустаяОперация", Перечисления.ВидыОперацийВЕТИС.ПустаяСсылка());
...
Функция ПоследовательностьСообщенийПередачиДанных(Данные)
	...
	Колонка.Вставить("Имя",          "ПолученРезультатЗапросаВЕТИС");
	Колонки.Добавить(Колонка);
	
	Колонка = Новый Структура;
	Колонка.Вставить("Приоритет",    4);
	Колонка.Вставить("Имя",          "ОтветНаЗапросВЕТИС");
	Колонки.Добавить(Колонка);
	...
	Для Каждого КлючИЗначение Из ИнтеграцияВЕТИС.КатегорииОпераций().ПередачаДанных Цикл
		ОперацииПередачиДанных.Добавить(КлючИЗначение.Ключ);
	КонецЦикла;
	
	ОтветНаЗапросВЕТИС = Перечисления.ВидыОперацийВЕТИС.ОтветыНаЗапросыВЕТИС();
	ПолученРезультатЗапросаВЕТИС = Перечисления.ВидыОперацийВЕТИС.ПолученныеРезультатыЗапросовВЕТИС();
	...
	Для Каждого СтрокаТЧ Из Данные Цикл
		Для Каждого Колонка Из Колонки Цикл
			Если ПоследовательностьСообщений.Найти(СтрокаТЧ[Колонка.Имя]) = Неопределено Тогда
				...
				Если ОтветНаЗапросВЕТИС.Найти(Операция) <> Неопределено Тогда
					НоваяСтрока.Заголовок = НСтр("ru = 'Квитанция ВетИС';
												|en = 'Квитанция ВетИС'");
					НоваяСтрока.Подсказка = Операция;
				ИначеЕсли ПолученРезультатЗапросаВЕТИС.Найти(Операция) <> Неопределено Тогда
					НоваяСтрока.Заголовок = НСтр("ru = 'Квитанция ВетИС';
												|en = 'Квитанция ВетИС'");
					НоваяСтрока.Подсказка = Операция;
				ИначеЕсли НоваяСтрока.ТипСообщения = Перечисления.ТипыЗапросовИС.Входящий
					...
					НоваяСтрока.Заголовок = НСтр("ru = 'Входящий запрос';
												|en = 'Входящий запрос'");
					НоваяСтрока.Подсказка = Операция;
				Иначе
...
Функция ТекстыСообщения(Сообщение)
	...
	ТекстСообщенияXML = ПротоколОбменаИС.ТекстСообщенияXMLИзПротокола(Сообщение);
	
	Попытка
		ФорматированныйТекстСообщенияXML = РаботаСXMLИС.ФорматироватьXMLСПараметрами(ТекстСообщенияXML, РаботаСXMLИС.ПараметрыФорматированияXML(Истина, "  "));
	Исключение
