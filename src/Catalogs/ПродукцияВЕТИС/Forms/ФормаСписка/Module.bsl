...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.Количество = 1 Тогда
			СтрокаСписка.Данные["Сопоставлено"] = ОбщегоНазначенияИС.ПредставлениеНоменклатуры(
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.Производитель > 1 Тогда
			...
			Если СтрДлина(ПредставлениеПроизводителей) > 75 Тогда
				ПредставлениеПроизводителей = СтрШаблон(НСтр("ru = '%1 (+ еще %2)';
															|en = '%1 (+ еще %2)'"), СтрокаСписка.Данные["Производитель"], Выборка.Производитель - 1);
			КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.Маркировка > 1 Тогда
			...
			Если СтрДлина(Маркировка) > 75 Тогда
				Маркировка = СтрШаблон(НСтр("ru = '%1 (+ еще %2)';
											|en = '%1 (+ еще %2)'"), СтрокаСписка.Данные["Маркировка"], Выборка.Маркировка - 1);
			КонецЕсли;
...
Процедура НастроитьВидимостьТаблицыПоиска(Форма)
	...
	ИначеЕсли Форма.ОтборыИзменились Тогда
		...
		Форма.Элементы.ПоискНеНастроен.ОтображениеСостояния.Текст = НСтр("ru = 'Параметры поиска были изменены. Выполните поиск.';
																		|en = 'Параметры поиска были изменены. Выполните поиск.'");
		...
		Иначе
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Производитель продукции не найден. Фильтр не изменен.';
					|en = 'Производитель продукции не найден. Фильтр не изменен.'"),,
				"МаркировкаОтбор");
		КонецЕсли;
