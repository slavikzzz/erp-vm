...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если ЗначениеЗаполнено(GTIN) И СтрДлина(GTIN) < 8 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Длина GTIN не может быть менее 8 символов';
													|en = 'Длина GTIN не может быть менее 8 символов'"),,
			"GTIN",
			,
			Отказ);
	КонецЕсли;
...
Процедура СформироватьТекстОжиданиеРезультатаОбменаВЕТИС()
	...
	Строки.Добавить(
		Новый ФорматированнаяСтрока(
			СтрШаблон(НСтр("ru = 'на %1 продукции %2 передан в ВетИС.';
							|en = 'на %1 продукции %2 передан в ВетИС.'"), СтрокаТипОперации("Ожидание"), Наименование)));
...
Процедура ВывестиИнформациюОУспешномРезультатеОбмена(Изменения)
	...
	Строки.Добавить(
		Новый ФорматированнаяСтрока(
			СтрШаблон(НСтр("ru = 'о %1 продукции %2 получен';
							|en = 'о %1 продукции %2 получен'"), СтрокаТипОперации("Получение"), Наименование)));
	...
	Строки.Добавить(
		Новый ФорматированнаяСтрока(
			СтрШаблон(НСтр("ru = '%1 продукция';
							|en = '%1 продукция'"), СтрокаТипОперации("Завершено"))));
	Строки.Добавить(" ");
...
Процедура УправлениеЭлементамиФормы(Форма)
	...
	Если Форма.РежимИзменения Тогда
		...
		Форма.Заголовок = НСтр("ru = 'Изменение продукции';
								|en = 'Изменение продукции'");
		
	Иначе
		
		Форма.Заголовок = НСтр("ru = 'Создание продукции';
								|en = 'Создание продукции'");
