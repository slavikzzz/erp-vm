...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоверительЮЛ_РоссийскаяОрганизация = (СтруктураДанных = Неопределено
		ИЛИ СтруктураДанных.ДоверительЮЛ_РоссийскаяОрганизация ИЛИ ЭтоПередоверие);
...
Процедура УправлениеФормой(Форма)
	...
	Форма.Элементы.ДекорацияДоверительФЛ_ИНННомЕРНРазделитель.Видимость = НЕ Форма.Доверитель_ЮридическоеЛицо
		И ЭтоВерсия5_03;
	Форма.Элементы.ДекорацияДоверительФЛ_НомЕРН.Видимость = НЕ Форма.Доверитель_ЮридическоеЛицо И ЭтоВерсия5_03;
	...
	Форма.Элементы.ДекорацияДоверительФЛ_ОГРНСНИЛСРазделитель.Видимость = НЕ Форма.Доверитель_ЮридическоеЛицо
		И ЭтоВерсия5_02;
	Форма.Элементы.ДекорацияДоверительФЛ_СНИЛС.Видимость = НЕ Форма.Доверитель_ЮридическоеЛицо И ЭтоВерсия5_02;
	Форма.Элементы.ДоверительФЛ_СНИЛС.Видимость = НЕ Форма.Доверитель_ЮридическоеЛицо И ЭтоВерсия5_02;
	...
	Форма.Элементы.ДоверительФЛ_ДатаРождения.АвтоОтметкаНезаполненного = НЕ Форма.Доверитель_ЮридическоеЛицо
		И НЕ ЭтоВерсия5_03;
	Форма.Элементы.ДоверительФЛ_ДатаРождения.АвтоВыборНезаполненного = НЕ Форма.Доверитель_ЮридическоеЛицо
		И НЕ ЭтоВерсия5_03;
	...
	Форма.Элементы.ДоверительФЛ_Гражданство.Видимость = Форма.Доверитель_ЮридическоеЛицо
		И НЕ Форма.ДоверительЮЛ_РоссийскаяОрганизация ИЛИ НЕ Форма.Доверитель_ЮридическоеЛицо И ЭтоВерсия5_02;
...
Функция СохранениеВозможно()
	...
	Иначе
		...
		Если ЭтоВерсия5_02 И НЕ ЗначениеЗаполнено(ДоверительФЛ_СНИЛС) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не задан СНИЛС.';
															|en = 'Не задан СНИЛС.'"),,
				"ДоверительФЛ_СНИЛС",, Отказ);
		ИначеЕсли ЭтоВерсия5_02 И СтрДлина(ДоверительФЛ_СНИЛС) <> 14 Тогда
