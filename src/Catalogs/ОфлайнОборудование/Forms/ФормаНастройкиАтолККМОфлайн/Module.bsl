...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Иначе
		ОчиститьСообщения();
		ОбщегоНазначенияБПОКлиент.СообщитьПользователю(НСтр("ru = 'При проверке были обнаружены следующие ошибки:';
															|en = 'The following errors were detected during the check:'")+ Символы.ПС + ТекстОшибкиОбщий);
	КонецЕсли;
...
Процедура ЗаполнитьНалоги()
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, Налоги,
		"НомерНалога0, НомерНалога5, НомерНалога7, НомерНалога10, НомерНалога18, НомерНалога20, НомерНалогаБезНДС");
