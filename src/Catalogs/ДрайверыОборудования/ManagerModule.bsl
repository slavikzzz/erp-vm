...
Процедура ПриНачальномЗаполненииЭлементов(ДрайверыОборудования) Экспорт
	...
	Попытка  
		...
		Для Каждого ДрайверОборудования Из ДрайверыОборудования Цикл
			...
			Если ТипЗнч(ПараметрыДрайвера.ТипОборудования) = Тип("Строка") Тогда     
				ПараметрыДрайвера.ТипОборудования = МенеджерОборудования.ТипОборудованияПоСтроке(ПараметрыДрайвера.ТипОборудования);
			КонецЕсли;
	...
	Исключение
		...
		ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Заполнение предопределенных элементов драйвера';
				|en = 'Fill predefined driver items'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()));
	КонецПопытки;
...
Функция ДанныеДрайвераОборудования(ИдентификаторДрайвера) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
		ДанныеДрайвера.Вставить("Несертифицированное"  , Несертифицированное);
...
Процедура ЗарегистрироватьНесертифицированныеДрайверы(ДрайверыОборудования) Экспорт
	
	// ++ НеМобильноеПриложение
	Если Не ОбщегоНазначенияБПО.ИспользуетсяНесертифицированноеОборудование() Тогда
		Возврат 
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	ДрайверыОборудования.СетевойРежим КАК СетевойРежим,
		|	ДрайверыОборудования.СерверныйРежим КАК СерверныйРежим,
		|	ДрайверыОборудования.СнятСПоддержки КАК СнятСПоддержки,
		|	ДрайверыОборудования.ЛокальныйРежим КАК ЛокальныйРежим
		|ИЗ
		|	Справочник.ДрайверыОборудования КАК ДрайверыОборудования
		|ГДЕ
		|	ДрайверыОборудования.ТипОборудования В(&ТипыПрочегоОборудования)";
	
	ТипыОборудования =  МенеджерОборудования.ДоступныеТипыНесертифицированногоОборудования();
	Запрос.УстановитьПараметр("ТипыПрочегоОборудования", ТипыОборудования);
	

	Результат = Запрос.Выполнить();
	ТаблицаДрайверов = Результат.Выгрузить();
	
	Для Каждого Драйвер Из ДрайверыОборудования Цикл
		
		Если Не РеквизитыДрайвераЗаполнены(Драйвер) Тогда
			#Если НЕ МобильноеПриложениеСервер Тогда
				ТекстСообщения = НСтр("ru = 'Не корректно заполнены реквизиты драйвера ""%1"".
				|Для регистрации должны быть заполнены: Тип оборудования, Наименование, Идентификатор объекта, Имя макета.';
				|en = 'Не корректно заполнены реквизиты драйвера ""%1"".
				|Для регистрации должны быть заполнены: Тип оборудования, Наименование, Идентификатор объекта, Имя макета.'");
				ТекстСообщения = СтрШаблон(ТекстСообщения, ДрайверыОборудования.Наименование);
				МодульОборудованиеНесертифицированноеКлиентСервер = ОбщегоНазначенияБПО.ОбщийМодуль("ОборудованиеНесертифицированноеКлиентСервер");
				ЗаписьЖурналаРегистрации(МодульОборудованиеНесертифицированноеКлиентСервер.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Ошибка,,, ТекстСообщения);
			#КонецЕсли
			Продолжить;
		КонецЕсли;
		
		Строка = ТаблицаДрайверов.Найти(Драйвер.ИдентификаторОбъекта, "ИдентификаторОбъекта");
		Если Строка = Неопределено Тогда
			ДрайверОбъект = Справочники.ДрайверыОборудования.СоздатьЭлемент();
			ЗаполнитьЗначенияСвойств(ДрайверОбъект, Драйвер);
			ДрайверОбъект.СпособПодключения = Перечисления.СпособПодключенияДрайвера.ИзМакета;
			ДрайверОбъект.Записать();
		Иначе
			Если ОбъектыИдентичны(Строка, Драйвер, Результат.Колонки, "Ссылка") Тогда
				Продолжить;
			КонецЕсли;
			ДрайверОбъект = Строка.Ссылка.ПолучитьОбъект();
			ЗаполнитьЗначенияСвойств(ДрайверОбъект, Драйвер);
			ДрайверОбъект.Записать();
		КонецЕсли
	КонецЦикла;
...
Функция ОбъектыИдентичны(Объект1, Объект2, Колонки, Исключения = "")
	
	МассивИсключений = СтрРазделить(Исключения, ",", Ложь);
	Для Каждого Поле Из Колонки Цикл
		ИмяПоля = СокрЛП(Поле.Имя);
		Если МассивИсключений.Найти(ИмяПоля)<>Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если Объект1[ИмяПоля] <> Объект2[ИмяПоля] Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция РеквизитыДрайвераЗаполнены(Драйвер)
	
	Возврат ЗначениеЗаполнено(Драйвер.ТипОборудования) И
		ЗначениеЗаполнено(Драйвер.Наименование) И
		ЗначениеЗаполнено(Драйвер.ИдентификаторОбъекта) И
		ЗначениеЗаполнено(Драйвер.ИмяМакетаДрайвера);
