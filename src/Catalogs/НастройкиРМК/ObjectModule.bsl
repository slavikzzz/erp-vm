...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(РабочееМесто) Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для рабочего места %1 уже существует настройка РМК %2';
																						|en = '%2 Point of sale workplace setting already exists for workplace %1'"), РабочееМесто, Выборка.Ссылка);
	...
	Для Каждого СтрокаТЧ Из КассыККМ Цикл
		Если Не СтрокаТЧ.ИспользоватьБезПодключенияОборудования
			...
			ТекстОшибки = НСтр("ru = 'Не заполнено поле ""Оборудование""';
								|en = '""Equipment"" is required'");
			АдресОшибки = НСтр("ru = 'в строке %НомерСтроки% списка ""Кассы ККМ""';
								|en = 'in line %НомерСтроки% of the ""Cash registers"" list'");
	...
	Для Каждого СтрокаТЧ Из ЭквайринговыеТерминалы Цикл
		Если Не СтрокаТЧ.ИспользоватьБезПодключенияОборудования
			...
			АдресОшибки = НСтр("ru = 'в строке %НомерСтроки% списка ""Эквайринговые терминалы""';
								|en = 'in line %НомерСтроки% of the ""POS terminals"" list'");
			АдресОшибки = СтрЗаменить(АдресОшибки, "%НомерСтроки%", СтрокаТЧ.НомерСтроки);
	...
	Для Каждого СтрокаТЧ Из ПлатежныеСистемы Цикл
		Если Не ЗначениеЗаполнено(СтрокаТЧ.ДоговорПодключенияКПлатежнойСистеме) Тогда
			...
			АдресОшибки = НСтр("ru = 'в строке %НомерСтроки% списка ""Платежные системы""';
								|en = 'in line %НомерСтроки% of the ""Payment systems"" list'");
			АдресОшибки = СтрЗаменить(АдресОшибки, "%НомерСтроки%", СтрокаТЧ.НомерСтроки);
