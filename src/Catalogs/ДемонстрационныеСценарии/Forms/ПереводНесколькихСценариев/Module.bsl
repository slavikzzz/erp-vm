...
Процедура ОбработатьПереданныеПараметры(Параметры, Отказ)
	...
	ТекстСообщенияНетДляПеревода = НСтр("ru = 'Не передано ни одного объекта для перевода';
										|en = 'No object is passed for translation'");
...
Функция РезультатПроверкиЗаполнения()
	...
	Если Не ЗначениеЗаполнено(ИсходныйЯзык) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не указан язык, с которого будет выполняться перевод';
														|en = 'A source language is not specified'"),,"ИсходныйЯзык");
		ЗаполненоКорректно = Ложь;
	КонецЕсли; 
	
	Если Не ЗначениеЗаполнено(ЦелевойЯзык) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не указан язык, на который будет выполняться перевод';
														|en = 'A target language is not specified'"),,"ЦелевойЯзык");
		ЗаполненоКорректно = Ложь;
	КонецЕсли;
	...
	Если ЗаполненоКорректно 
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Исходный и целевой языки одинаковые';
														|en = 'The source and target languages are the same'"),,"ЦелевойЯзык");
		ЗаполненоКорректно = Ложь
	...
	Если УстанавливатьСтатус 
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не указан устанавливаемый статус';
														|en = 'The status to set is not specified'"),,"УстанавливаемыйСтатус");
		ЗаполненоКорректно = Ложь;
