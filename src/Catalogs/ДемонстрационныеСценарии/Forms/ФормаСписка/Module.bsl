...
Процедура ПеревестиВыделенныеЭлементы(Команда)
	...
	Иначе
		...
		ПоказатьПредупреждение(,НСтр("ru = 'Для выполнения перевода необходимо выбрать один или несколько сценариев';
									|en = 'To translate, select one or several scenarios'"), 20);
...
Процедура ПереместитьЭлемент(Ссылка, Родитель, Вверх = Истина)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Перемещение не выполнено. Не удалось заблокировать сценарий';
								|en = 'The item is not transferred. Cannot lock the scenario'");
...
Процедура ОбработатьЗавершениеПереводаПоНастройкам(Результат, ДополнительныеПараметры)
	...
	Если Результат.Переведено = 0 Тогда
		...
		ТекстПояснения = НСтр("ru = 'Ни один сценарий не переведен';
								|en = 'No scenario is translated'");
		
	Иначе
		
		ТекстПояснения = СтрШаблон(НСтр("ru = 'Переведено сценариев %1 из %2';
										|en = 'Translated %1 out of %2 scenarios'"), Результат.Переведено, Результат.ВсегоОбъектов);
