	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
		...
		Для Каждого СтрокаПартнер Из ПартнерыИКонтактныеЛица Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаПартнер.Партнер) Тогда
				...
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Не заполнена колонка ""Контрагент"" в строке %1 табличной части ""Контрагенты и контактные лица""';
												|en = 'Column ""Counterparty"" in line %1 of the ""Counterparties and contact persons"" table is not filled in'"), СтрокаПартнер.НомерСтроки);
