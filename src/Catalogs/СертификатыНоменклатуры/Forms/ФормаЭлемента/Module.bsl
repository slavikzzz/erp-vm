	...
	Если Не ТекущийОбъект.ЭтоНовый() Тогда
		...
		Если СтатусДействует <> СтатусРосаккредитацииДействует
			...
			ТекстСообщения = НСтр("ru = 'Статус сертификата не соответствует статусу по данным Росаккредитации.';
									|en = 'The certificate status does not correspond to the status according to the RusAccreditation data.'");
...
Процедура ОбновитьПредставление(ИзменятьПредставление = Ложь)
	...
	Если Объект.Бессрочный Тогда
		ПредставлениеДаты = " " + НСтр("ru = 'от %ДатаНачалаСрокаДействия%';
										|en = 'from %ДатаНачалаСрокаДействия%'");
	Иначе	
	    ПредставлениеДаты = " " + НСтр("ru = 'с %ДатаНачалаСрокаДействия% по %ДатаОкончанияСрокаДействия%';
										|en = 'from %ДатаНачалаСрокаДействия% to %ДатаОкончанияСрокаДействия%'");
	КонецЕсли;	
	...
	Если ЗначениеЗаполнено(Объект.ОрганВыдавшийДокумент) Тогда
		ПредставлениеВыдан = НСтр("ru = '. Выдан: %ОрганВыдавшийДокумент%';
									|en = '. Issued by: %ОрганВыдавшийДокумент%'");
	...
	Если ТекущаяОбластьДействия.ВариантОбластиДействия = "СписокТоваров" Тогда
		...
		Если ОбластьДействия.Количество() > 1 Тогда
			ТекстВопроса = НСтр("ru = 'При смене варианта области действия список будет очищен. Продолжить?';
								|en = 'The list will be cleared when changing the scope. Continue?'");
...
Процедура ОбновитьСчетчикИзображений()
	...
	ТекстДекорации = НСтр("ru = '%ТекущееИзображение% из %ВсегоИзображений%';
							|en = '%ТекущееИзображение% from %ВсегоИзображений%'");
...
Процедура УстановитьТекстРосаккредитацияДетальнаяИнформация(ДетальнаяИнформация = Неопределено)
	...
	ШаблонТехническиеРегламенты = НСтр("ru = '<b>Технические регламенты:</b> %1';
										|en = '<b>Technical regulations:</b> %1'");
	ШаблонКодыТНВЭД = НСтр("ru = '<b>Коды ТН ВЭД:</b> %1';
							|en = '<b>FEACN codes:</b> %1'");
	ШаблонШтрихкоды = НСтр("ru = '<b>Штрихкоды (GTIN):</b> %1';
							|en = '<b>Barcodes (GTIN):</b> %1'");
	ШаблонСтрокаСОтступом = НСтр("ru = '%1';
								|en = '%1'") + Символы.ПС;
...
Функция ЗаполнитьПоДаннымРосаккредитацииНаСервере()
	...
	ШаблонКонкретныйТоварОтличаетсяНоменклатура = НСтр("ru = 'Подобрана номенклатура %1, которая отличается от уже выбранной %2. Переключить область действия на ""список товаров"" и заполнить?';
														|en = 'The %1 item selected differs from the already selected item (%2). Do you want to switch the scope to ""goods list"" and fill it in?'");
	ШаблонКонкретныйТоварОтличаетсяХарактеристика = НСтр("ru = 'Подобрана характеристика %1, которая отличается от уже выбранной %2. Переключить область действия на ""список товаров"" и заполнить?';
														|en = 'The %1 variant selected differs from the already selected variant (%2). Do you want to switch the scope to ""goods list"" and fill it in?'");
	ШаблонВидНоменклатурыОтличаетсяВидНоменклатуры = НСтр("ru = 'Подобран вид номенклатуры %1, который отличается от выбранного %2. Переключить область действия на ""список товаров"" и заполнить?';
															|en = 'The %1 item kind selected differs from the already selected item kind (%2). Do you want to switch the scope to ""goods list"" and fill it in?'");
	ШаблонВыбраноНесколькоЗаписей = НСтр("ru = 'Подобрано несколько записей (%1 шт.). Переключить область действия на ""список товаров"" и заполнить?';
										|en = 'Several records are selected (%1 pcs.). Do you want to switch the scope to ""goods list"" and fill it in?'");
...
Процедура ОбновитьТекстыРосаккредитации()
	...
	ШаблонСтатусДействителен = НСтр("ru = '<span style=""color: ЦветТекстаУспех; font: ВажнаяНадписьШрифт"">%1</span> (актуально на: %2)';
									|en = '<span style=""color: ЦветТекстаУспех; font: ВажнаяНадписьШрифт"">%1</span> (is relevant as of: %2)'");
	ШаблонСтатусНеДействителен = НСтр("ru = '<span style=""color: ЦветШрифтаОшибки; font: ВажнаяНадписьШрифт"">%1</span> (актуально на: %2)';
										|en = '<span style=""color: ЦветШрифтаОшибки; font: ВажнаяНадписьШрифт"">%1</span> (is relevant as of: %2)'");
	ШаблонСтатусНетИнформации = НСтр("ru = '<span style=""font: ВажнаяНадписьШрифт"">Нет информации</span>';
									|en = '<span style=""font: ВажнаяНадписьШрифт"">No information</span>'");
