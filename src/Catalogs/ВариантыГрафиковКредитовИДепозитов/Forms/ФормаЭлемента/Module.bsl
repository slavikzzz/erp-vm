	...
	Если ЗначениеЗаполнено(СуммаЛимита) Тогда
		...
		Если СуммаТраншей > СуммаЛимита Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'Сумма траншей превосходит заданный лимит в %1 %2 по договору!';
								|en = 'Total amount of tranches exceeds the limit specified in the contract (%1 %2).'"), СуммаЛимита, ВалютаДоговора),
				,,, Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура ТаблицаГрафикаПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если УказаннаяСумма = 0 И (НЕ НоваяСтрока ИЛИ НЕ ОтменаРедактирования) Тогда
		Текст = НСтр("ru = 'В строке должна быть указана хотя бы одна из сумм!';
					|en = 'The line must contain at least one of the amounts.'");
		Если Не НоваяСтрока И ОтменаРедактирования Тогда
			...
			|en = 'Lines with zero amounts cannot be saved.
	...
	Иначе
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗагрузитьГрафикПослеВопроса", ЭтотОбъект, Новый Структура("ТипГрафика", ТипГрафика)),
			НСтр("ru = 'Перед загрузкой необходимо записать объект.
				...
				|en = 'You need to save the object before importing.
...
Процедура НастроитьФормуПоДоговору()
	...
	Элементы.ГруппаТранши.Заголовок       = СтрШаблон(НСтр("ru = 'Всего транши (%1)';
															|en = 'Total tranches (%1)'"), СтрокаВалюта);
	Элементы.ГруппаОплаты.Заголовок       = СтрШаблон(НСтр("ru = 'Всего оплата (%1)';
															|en = 'Total payment (%1)'"), СтрокаВалюта);
	Элементы.ГруппаНачисления.Заголовок   = СтрШаблон(НСтр("ru = 'Всего начисления (%1)';
															|en = 'Total accruals (%1)'"), СтрокаВалюта);
	Элементы.ТраншиСумма.Заголовок        = СтрШаблон(НСтр("ru = 'Сумма (%1)';
															|en = 'Amount (%1)'"), СтрокаВалюта);
	Элементы.ОплатыСумма.Заголовок        = СтрШаблон(НСтр("ru = 'Сумма основного долга (%1)';
															|en = 'Principal debt (%1)'"), СтрокаВалюта);
	Элементы.ОплатыПроценты.Заголовок     = СтрШаблон(НСтр("ru = 'Сумма процентов (%1)';
															|en = 'Interest amount (%1)'"), СтрокаВалюта);
	Элементы.ОплатыКомиссия.Заголовок     = СтрШаблон(НСтр("ru = 'Сумма комиссии (%1)';
															|en = 'Fee amount (%1)'"), СтрокаВалюта);
	Элементы.НачисленияПроценты.Заголовок = СтрШаблон(НСтр("ru = 'Сумма процентов (%1)';
															|en = 'Interest amount (%1)'"), СтрокаВалюта);
	Элементы.НачисленияКомиссия.Заголовок = СтрШаблон(НСтр("ru = 'Сумма комиссии (%1)';
															|en = 'Fee amount (%1)'"), СтрокаВалюта);
...
Функция НаборГрафика(ТипГрафика, Ссылка)
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Не определен регистр сведений для хранения данных графика ""%1""';
										|en = 'No information register defined for storing scheduled ""%1"" data'"), ТипГрафика);
	КонецЕсли;
...
Функция СуммыГрафиковСходятся()
	...
	Если Не СуммыДолгаРавны Тогда
		Текст  = НСтр("ru = 'Различаются итоговые суммы основного долга на закладках ""Транши"" и ""Оплаты"".';
						|en = 'Total principal debt amounts specified on the Tranches and Payments tabs do not match.'");
	...
	Если Не ПроцентыРавны Тогда
		Текст  = НСтр("ru = 'Различаются итоговые суммы процентов на закладках ""Оплаты"" и ""Начисления"".';
						|en = 'Total interest amounts specified on the Payments and Accruals tabs do not match.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
	КонецЕсли;
	...
	Если ЕстьКомиссия И Не КомиссияРавна Тогда
		Текст  = НСтр("ru = 'Различаются итоговые суммы комиссии на закладках ""Оплаты"" и ""Начисления"".';
						|en = 'Total fee amounts specified on the Payments and Accruals tabs do not match.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
	КонецЕсли;
...
Процедура ЗаписатьГрафик(ЗакрыватьПослеЗаписи = Ложь)
	...
	Если Не СуммыГрафиковСходятся() Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОбработкаОтветаНаВопросИгнорировать", ЭтотОбъект, ЗакрыватьПослеЗаписи),
			...
			|Игнорировать?';
			|en = 'Total amounts specified in the current schedule option do not match.
			|Ignore?'"),
			РежимДиалогаВопрос.ДаНет);
	Иначе
