...
Процедура ПередЗаписью(Отказ)
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Владелец, "Статус") = Перечисления.СтатусыДоговоровКонтрагентов.Закрыт Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Изменения графика по закрытому договору запрещены!';
				|en = 'Changing schedule for a closed contract is not allowed.'"),,,, Отказ);
	КонецЕсли;
			...
			Исключение
				ТекстСообщения = СтрШаблон(
					НСтр("ru = '%1 находится в процессе редактирования пользователем или системой и не может быть изменен.
						...
						|en = '%1 is being edited by a user or application and cannot be changed.
						|To set statuses, release other contract schedule options.'"),
