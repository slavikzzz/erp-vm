...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("СтруктураОтбора") Тогда
		МассивНесопоставленныхКлючей = ИнтеграцияЗЕРНО.НеСопоставленныеКлючиАдресов(Параметры.СтруктураОтбора);
	ИначеЕсли Параметры.Свойство("НесопоставленныеКлючи") Тогда
		МассивНесопоставленныхКлючей = Параметры.НесопоставленныеКлючи;
	КонецЕсли;
	
	Если МассивНесопоставленныхКлючей.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Отсутствуют данные для сопоставления';
				|en = 'Отсутствуют данные для сопоставления'"),,,,
			Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ВРег(Лев(ИмяСобытия, 6)) = "ЗАПИСЬ" Тогда
		...
		Если ИмяСобытия = "Запись_КлючиАдресовЗЕРНО" Тогда
...
Процедура ПолеСоставногоТипаПриИзменении(Элемент)	
	...
	ОбработкаСопоставленияДляКлюча(ТекущиеДанные.СкладКонтрагент);
...
Процедура ОформитьСтрокуПодсказкиУстановкиСоответствияКлючейАдресов(ИдентификаторСтрокиТаблицы)
	...
	СтрокаТаблицы.СкладКонтрагентСтрокой = СтрокаПредставления; 
	
	Если МассивОператоровАдреса.Количество() = 1 Тогда
		СтрокаТаблицы.СкладКонтрагент    = МассивОператоровАдреса[0];
	КонецЕсли;
...
Функция СформироватьОператоровАдресаПоКлючу(КлючАдреса)
	...
	МассивВозврата = Новый Массив; // Массив из ОпределяемыйТип.СкладКонтрагентЗЕРНО -
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	КлючиАдресовЗЕРНООператорыАдреса.СкладКонтрагент
		|ИЗ
		|	Справочник.КлючиАдресовЗЕРНО.ОператорыАдреса КАК КлючиАдресовЗЕРНООператорыАдреса
		|ГДЕ
		|	КлючиАдресовЗЕРНООператорыАдреса.Ссылка = &Ссылка";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		МассивВозврата.Добавить(ВыборкаДетальныеЗаписи.СкладКонтрагент);
	КонецЦикла;
...
Процедура ОбработкаСопоставленияДляКлюча(ВыбранноеЗначениеСопоставления)
	...
	Если ЗначениеЗаполнено(РезультатВыполнения) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Операция не может быть выполнена по причине:';
										|en = 'Операция не может быть выполнена по причине:'") + Символы.ПС + РезультатВыполнения);
		
		ТекущиеДанные.СкладКонтрагент        = Неопределено;
		ТекущиеДанные.СкладКонтрагентСтрокой = "";
...
Функция ОбновитьСопоставлениеКлючаАдреса(КлючАдреса, ЗначениеСопоставления)
	...
	Если ЗначениеЗаполнено(ЗначениеСопоставления) Тогда
		...
		НоваяСтрокаСкладаКонтрагента                 = ОбъектКлючАдреса.ОператорыАдреса.Добавить();
		НоваяСтрокаСкладаКонтрагента.СкладКонтрагент = ЗначениеСопоставления;
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТаблицаСопоставлений.СкладКонтрагентСтрокой");
