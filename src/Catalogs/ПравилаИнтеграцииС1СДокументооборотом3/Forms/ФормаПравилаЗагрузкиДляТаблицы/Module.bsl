...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(СокращенноеНаименованиеКонфигурации) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Правило заполнения таблицы %1';
				|en = 'Table filling rule %1'"), СокращенноеНаименованиеКонфигурации);
	КонецЕсли;
	...
	Если МестоВыполненияВыражения = Перечисления.МестаВыполненияВыраженийНаВстроенномЯзыке.НаСторонеДО Тогда
		Элементы.ВычисляемоеВыражение.Подсказка =
			НСтр("ru = 'Выражение будет выполняться в базе 1С:Документооборот';
				|en = 'The expression will be executed in 1C:Document Management'");
	Иначе
		Элементы.ВычисляемоеВыражение.Подсказка =
			СтрШаблон(НСтр("ru = 'Выражение будет выполняться в базе %1';
							|en = 'The expression will be executed in %1'"), СокращенноеНаименованиеКонфигурации);
	КонецЕсли;
...
Процедура ВариантПриИзменении(Элемент)
	...
	Обновлять = Обновлять И (Вариант <> НеЗаполнять);
...
Процедура УстановитьДоступность()
	...
	Элементы.Обновлять.Доступность = (Вариант <> НеЗаполнять);
