...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступенОтборЗначенийДопРеквизитов =
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("3.0.15.41");
...
Процедура КлючевыеРеквизитыДОЗначениеРеквизитаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыВыбораЗначенияДО = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПараметрыВыбораЗначенияДО();
	ПараметрыВыбораЗначенияДО.Оповещение =
		Новый ОписаниеОповещения("ВыбратьЗначениеРеквизитаДОЗавершениеВводаЗначения", ЭтотОбъект);
	ПараметрыВыбораЗначенияДО.ТекстРедактирования = Элемент.ТекстРедактирования;
	ПараметрыВыбораЗначенияДО.Представление = ТекущиеДанные.Представление;
	ПараметрыВыбораЗначенияДО.ЗначениеРеквизита = ТекущиеДанные.ЗначениеРеквизита;
	ПараметрыВыбораЗначенияДО.ЗначениеРеквизитаID = ТекущиеДанные.ЗначениеРеквизитаID;
	ПараметрыВыбораЗначенияДО.ЗначениеРеквизитаТип = ТекущиеДанные.ЗначениеРеквизитаТип;
	...
	Если ТекущиеДанные.ЭтоДополнительныйРеквизитДО
		...
		ПараметрыВыбораЗначенияДО.Отбор = Новый Структура("additionalPropertyID", ТекущиеДанные.Имя);
	КонецЕсли;
		...
		Если Родитель.Вариант = ВариантВыражение() Тогда
			ПоказатьПредупреждение(,
				НСтр("ru = 'Таблица заполняется результатом выражения на встроенном языке,
					...
					|en = 'The table is populated with the result of the expression in 1C:Enterprise language,
					| so the rules for the individual details are not required.'"));
			Возврат;
		КонецЕсли;
		Если Родитель.Вариант = ВариантПустой() Тогда
			ПоказатьПредупреждение(,
				НСтр("ru = 'Вначале нужно выбрать вариант заполнения таблицы.';
					|en = 'First select a table filling option.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПравилаЗаполненияРеквизитовИСПередУдалением(Элемент, Отказ)
	...
	ПравилаЗаполненияРеквизитовИСПередУдалениемПродолжение(ТекущиеДанные);
...
&НаКлиенте
Асинх Процедура ПометитьНаУдаление(Знач Команда)
	...
	Если Модифицированность Тогда
		Если Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ru = 'Для снятия пометки на удаление необходимо записать внесенные изменения';
								|en = 'To clear a deletion mark, save the changes'");
			ПараметрыВопросаДаНет.ТекстКнопкиДа = НСтр("ru = 'Записать и снять пометку';
														|en = 'Save and clear check box'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Для пометки на удаление необходимо записать внесенные изменения';
								|en = 'To set a deletion mark, save the changes'");
			ПараметрыВопросаДаНет.ТекстКнопкиДа = НСтр("ru = 'Записать и пометить';
														|en = 'Save and mark'");
		КонецЕсли;
	...
	Иначе
		...
		Иначе
			ТекстВопроса = СтрШаблон(
				...
				Строка(Объект.Ссылка));
		КонецЕсли;
		ПараметрыВопросаДаНет.ТекстКнопкиДа = НСтр("ru = 'Да';
													|en = 'Yes'");
		ПараметрыВопросаДаНет.ТекстКнопкиНет = НСтр("ru = 'Нет';
													|en = 'No'");
	КонецЕсли;
	...
	Если НужноЗадатьВопрос Тогда
		Ответ = Ждать ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПоказатьВопросДаНетАсинх(
			НСтр("ru = 'Правила будут заполнены по умолчанию. Заполнить?';
				|en = 'Rules will be populated by default. Populate?'"),
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПараметрыВопросаДаНет(КодВозвратаДиалога.Нет));
		Если Ответ = КодВозвратаДиалога.Да Тогда
...
&НаКлиенте
Асинх Процедура ПравилаЗаполненияРеквизитовИСПередУдалениемПродолжение(Знач ТекущиеДанные)
	
	Ответ = Ждать ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПоказатьВопросДаНетАсинх(
		НСтр("ru = 'Очистить правило заполнения реквизита?';
			|en = 'Clear the attribute filling rule?'"),
...
&НаКлиенте
Асинх Процедура ПриИзмененииОбъектаДО(Знач СтрокаПодходящиеПравилаЗагрузкиВДО)
	...
	Если ЗначениеЗаполнено(Объект.ТипОбъектаДО) И ЗначениеЗаполнено(Объект.ПредставлениеОбъектаДО) Тогда
		ТекстВопроса = СтрШаблон(
			НСтр("ru = 'Установить заполнение реквизитов из объекта ""%1""?';
				|en = 'Do you want to set attribute filling from the %1 object?'"),
			Объект.ПредставлениеОбъектаДО);
		Ответ = Ждать ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПоказатьВопросДаНетАсинх(
			ТекстВопроса,
			ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПараметрыВопросаДаНет(КодВозвратаДиалога.Да));
		Если Ответ = КодВозвратаДиалога.Нет Тогда
			ЗаполнятьИзОбъектаДО = Ложь;
		КонецЕсли;
	Иначе
		ЗаполнятьИзОбъектаДО = Ложь;
	КонецЕсли;
	
	ЗаполнитьПоУмолчаниюНаСервере(, ЗаполнятьИзОбъектаДО);
...
&НаКлиенте
Асинх Процедура ПриПодключении()
	...
	Если ОбработатьФормуСогласноВерсииСервиса() Тогда
		...
		Если Не ВопросЗадан
			...
			Ответ = Ждать ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПоказатьВопросДаНетАсинх(
				НСтр("ru = 'Заполнить правила интеграции по умолчанию?';
					|en = 'Do you want to populate integration rules by default?'"),
				ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПараметрыВопросаДаНет(КодВозвратаДиалога.Нет));
			Если Ответ = КодВозвратаДиалога.Да Тогда
				ЗаполнитьПоУмолчаниюНаСервере();
			КонецЕсли;
	...
	Для Каждого Строка Из Реквизиты Цикл
		...
		Если НоваяСтрока.Вариант = ВариантРеквизитТаблицы() Тогда
			НоваяСтрока.Пояснение = НСтр("ru = 'Заполняется по правилам для отдельных реквизитов таблицы';
										|en = 'Filled in according to the rules of separate table attributes'");
		КонецЕсли;
...
Процедура ЗаполнитьПоУмолчаниюНаСервере(НачальноеЗаполнение = Ложь, ЗаполнятьИзОбъектаДО = Истина)
		...
		Для Каждого РеквизитДО Из КлючевыеРеквизитыДО Цикл
			...
			Для Каждого РеквизитИС Из КлючевыеРеквизитыИС Цикл
				...
				Если РеквизитДО.Представление = РеквизитИС.Представление Тогда
					...
					ПравилоЗаполнения.ЭтоДополнительныйРеквизитДО = РеквизитДО.ЭтоДополнительныйРеквизитДО;
					ПравилаЗаполнения.Вставить(РеквизитИС.Имя, ПравилоЗаполнения);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	...
	Для Каждого СтрокаПравил Из ДеревоПравил.Строки Цикл
		...
		Если ЗначениеЗаполнено(ЗначениеЗаполнения) Тогда
			...
			СтрокаПравил.ЭтоДополнительныйРеквизитДО = ЗначениеЗаполнения.ЭтоДополнительныйРеквизитДО;
			...
			Если ЗначениеЗаполнения.ЭтоДополнительныйРеквизитДО Тогда
				СтрокаПравил.ИмяРеквизитаОбъектаДО = "";
				СтрокаПравил.ДополнительныйРеквизитДОID = ЗначениеЗаполнения.Имя;
				СтрокаПравил.ДополнительныйРеквизитДОТип = "DMAdditionalProperty";
			Иначе
				СтрокаПравил.ИмяРеквизитаОбъектаДО = ЗначениеЗаполнения.Имя;
				СтрокаПравил.ДополнительныйРеквизитДОID = "";
				СтрокаПравил.ДополнительныйРеквизитДОТип = "";
			КонецЕсли;
			...
			Исключение
				ОбщегоНазначения.СообщитьПользователю(СтрШаблон(
					НСтр("ru = 'Не удалось получить реквизиты объекта 1С:Документооборот по причине:
						|%1';
						|en = 'Cannot get attributes of the 1C:Document Management object. Reason:
						|%1'"),
