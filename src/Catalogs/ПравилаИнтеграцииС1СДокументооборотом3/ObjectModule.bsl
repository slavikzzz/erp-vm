...
Функция ПредставлениеКлючевогоРеквизита(КлючевойРеквизит)
	...
	Иначе
		...
		Если Представление = "" Тогда
			Представление = СтрШаблон(НСтр("ru = '%1: пустое значение';
											|en = '%1: empty value'"), КлючевойРеквизит.Представление);
		КонецЕсли;
...
Процедура ПроверитьВозможностьЗаписиПравила()
	...
	Если ЭтоНовый() Тогда
		ВызватьИсключение НСтр("ru = 'В правилах интеграции с 1С:Документооборотом запрещено изменять выражения на встроенном языке.
			...
			|en = 'You cannot change expressions in 1C:Enterprise language in the rules of integration with 1C:Document Management.
