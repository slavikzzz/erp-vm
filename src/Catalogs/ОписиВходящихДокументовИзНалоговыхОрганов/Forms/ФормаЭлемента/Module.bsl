...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		ТекстПредупреждения = НСтр("ru = 'Копирование описей входящих документов запрещено!';
									|en = 'Копирование описей входящих документов запрещено!'");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ПрорисоватьПанелиСтатусаИПриема()
	...
	Иначе
		...
		Элементы.КомандаОтказатьВПриеме.Видимость = НЕ ТребованияФНСВызовСервера.ПодтверждениеОтправилОператор(Объект.Ссылка);
	КонецЕсли;
