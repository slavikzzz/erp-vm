...
Процедура ОтменитьДоверенность(Команда)
	...
	ОписаниеДанных.Вставить("Операция", НСтр("ru = 'Отмена доверенности';
											|en = 'Cancel the letter of authority'"));
	...
	Если Результат.Успех Тогда
		...
		Если ЗначениеЗаполнено(РезультатПроверки.Ошибка) Тогда
			...
			КонтекстОшибки.ЗаголовокПредупреждения = НСтр("ru = 'Подпись не подходит для отмены';
															|en = 'The signature is not suitable for cancellation'");
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Заявление отправлено';
											|en = 'Application is sent'"), ПолучитьНавигационнуюСсылку(Доверенность), 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отмена доверенности %1.';
																		|en = 'Cancel letter of authority %1.'"), НомерДоверенности));
...
Функция ПроверитьСертификатДоверителя(Знач Доверенность, Знач Сертификат)
	...
	Если РезультатПроверки <> Истина Тогда
		ПараметрыОшибкиМЧДРР = МашиночитаемыеДоверенностиФНССлужебный.ПараметрыОшибкиМЧДРР(НСтр("ru = 'Подпись не подходит для отмены';
																								|en = 'The signature is not suitable for cancellation'"), РезультатПроверки);
