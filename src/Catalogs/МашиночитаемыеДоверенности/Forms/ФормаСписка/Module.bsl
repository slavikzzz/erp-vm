...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка);
	...
	Заголовок = НСтр("ru = 'Машиночитаемые доверенности';
					|en = 'Machine-readable letters of authority'");
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	Если ЗначениеЗаполнено(РезультатАвторизации.Ошибка) Тогда
		...
		ТекстОшибки = НСтр("ru = 'Нет возможности проверить доверенность в распределенном реестре, доверенность будет загружена без признака ""Зарегистрирована в реестре ФНС""';
							|en = 'Couldn''t verify the letter of authority against the distributed registry. It will be marked as unregistered in the FTS registry.'");
...
Процедура ОбновитьСтатусИзРеестраФНС(Команда)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
		НСтр("ru = ';Запрашивается статус для %1 документа;;Запрашивается статус для %1 документов;Запрашивается статус для %1 документов;|Запрашивается статус для %1 документов';
			|en = ';Requesting status for %1 document;;;;Requesting statuses for %1 documents%1%1'"), 
		Контекст.ВыделенныеСтроки.Количество(),, "ЧДЦ=0");
	
	ПоказатьОповещениеПользователя(
		НСтр("ru = 'Обновление статусов из Реестра ФНС';
			|en = 'Updating statuses from FTS distributed ledger'"),
		,
		ТекстСообщения,
...
Процедура ПослеОтветаНаВопросОЗагрузкеДоверенности(Результат, Ошибка) Экспорт
	...
	ПараметрыЗагрузки.Диалог.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Файлы доверенностей (%1)|%1';
			|en = 'Files of letters of authority (%1)|%1'"), "*.xml;*.p7s;*.sig;*.zip");
...
Процедура УстановитьОтбор()
	...
	Если ОтборПоСостоянию = "СИстекающимСрокомДействия" Тогда
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (с истекающим сроком)';
																				|en = '%1 (expiring)'"), Заголовок);
	ИначеЕсли ОтборПоСостоянию = "ТребуютВнимания" Тогда
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (требуют внимания)';
																				|en = '%1 (requiring attention)'"), Заголовок);
	ИначеЕсли ОтборПоСостоянию = "ОжидаютПодписания" Тогда
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (ожидают подписания)';
																				|en = '%1 (pending signing)'"), Заголовок);
	КонецЕсли;
...
Процедура ПослеПолученияСтатуса(Результат, Контекст) Экспорт
	...
	Если СОшибками = 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Успешно загружен статус для %1 доверенностей';
																						|en = 'Status for %1 letters of authority is imported'"), СостоянияДоверенностей.Количество());
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Успешно: %1, с ошибками: %2';
																						|en = 'Success: %1, with errors: %2'"), СостоянияДоверенностей.Количество() - СОшибками, СОшибками);
	КонецЕсли;
...
Процедура УстановитьПредставлениеПолномочий()
	Если ЗначениеЗаполнено(Полномочия) Тогда
		...
		КонецЕсли;         
		ПолномочияПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отбор по полномочиям (%1)';
																								|en = 'Filter by powers (%1)'"), КоличествоПолномочий);
	Иначе
		...
		Если СтрДлина(ПараметрыПоиска.СтрокаПоиска) > 2 Тогда
			СообщениеФорматированное = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				НСтр("ru = '<span style=""color: %1; font: %2"">%3</span> нет в списках';
					|en = '<span style=""color: %1; font: %2"">%3</span> нет в списках'"),
...
Функция ШаблонФрагментаСтрокиПоиска(СтрокаПоиска)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '<span style=""color: %1; font: %2"">%3</span>';
			|en = '<span style=""color: %1; font: %2"">%3</span>'"), "РезультатУспехЦвет", "ВажнаяНадписьШрифт",
		СтрокаПоиска);
...
Функция ТекстОшибкиФизическоеЛицо()
	Возврат НСтр("ru = '<нет связи пользователя с физическим лицом>';
				|en = '<there is no connection between the user and the person>'");
КонецФункции
