...
Процедура ВыбранныеПолномочияТаблицаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ЗначениеЗаполнено(Параметры.ПолномочияРодительскойДоверенности) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Нельзя добавлять полномочия у передоверия вручную.';
					|en = 'Sub-delegation powers cannot be added manually.'"), , , , Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ОграниченияПолномочийПередУдалением(Элемент, Отказ)
	...
	Если ЗначениеЗаполнено(Параметры.ПолномочияРодительскойДоверенности) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Нельзя удалять ограничения у передоверия.';
					|en = 'Sub-delegation powers cannot be deleted.'"), , , , Отказ);
...
Асинх Процедура СохранитьНастройкуПолномочий(Команда)
	...
	Наименование = Ждать ВвестиСтрокуАсинх("", НСтр("ru = 'Введите имя настройки';
													|en = 'Enter a setting name'"), 150);
...
Процедура ОбновитьКлассификаторыПолномочий(Команда)
	...
	ПоказатьПредупреждение(, НСтр("ru = 'Обновление классификаторов полномочий выполнено.';
									|en = 'The power classifiers are updated.'"));
...
Функция СохранениеВозможно(ВыбранныеПолномочия, ВыбранныеОграничения)
	...
	Если ВидПолномочий = "0" Тогда
		...
		Для Каждого Строка Из ВыбранныеПолномочияТаблица Цикл
			...
			Если Не ЗначениеЗаполнено(Строка.Код) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнен код полномочия.';
																|en = 'Power code is not specified.'"), ,
			...
			Если Не ЗначениеЗаполнено(Строка.Наименование) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено наименование полномочия.';
																|en = 'Power description is not specified.'"), ,
			...
			Для Каждого Ограничение Из Найдено Цикл
				...
				Если Не ЗначениеЗаполнено(Ограничение.Код) Тогда
					ОбщегоНазначенияКлиент.СообщитьПользователю(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Не заполнен код ограничения у полномочия %1 %2.';
								|en = 'Limitation code for power %1 %2 is not specified.'"), Строка.Код, Строка.Наименование), ,
				...
				Если Не ЗначениеЗаполнено(Ограничение.НаименованиеЗначения) Тогда
					ОбщегоНазначенияКлиент.СообщитьПользователю(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Не заполнено значение ограничения у полномочия %1 %2.';
								|en = 'Limitation value for power %1 %2 is not specified.'"), Строка.Код, Строка.Наименование), ,
		...
		Если Не РежимКлассификатора И ВыбранныеПолномочия.Количество() = 0 Тогда
			...
			ВидимостьПоказатьВыбранные(ЭтотОбъект);
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Не выбрано ни одно полномочие.';
					|en = 'No power is selected.'"), , "Полномочия", , Отказ);
		КонецЕсли;
	Иначе
		Если ПустаяСтрока(ТекстПолномочий) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Не заполнен текст полномочий.';
					|en = 'Power text is not specified.'"), , "ТекстПолномочий", , Отказ);
		ИначеЕсли СтрДлина(ТекстПолномочий) > 10000 Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Текст полномочий превышает 10000 символов.';
					|en = 'Power text exceeds 10,000 characters.'"), , "ТекстПолномочий", , Отказ);
		КонецЕсли;
	КонецЕсли;
