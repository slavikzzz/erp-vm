...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ru = 'Машиночитаемая доверенность';
											|en = 'Electronic letter of authority'");
	...
	Если ДляНалоговыхОрганов Тогда
		...
		Если Доверенность.ИдФайлНО = Неопределено Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Доверенность %1 не предназначена для представления в налоговые органы.';
					|en = 'Letter of authority %1 cannot be submitted to tax authorities.'"), ИдентификаторФайла);
		Иначе
...
Функция ПечатьДоверенности(Доверенности, ОбъектыПечати, КодЯзыка)
	...
	Для Каждого Ссылка Из Доверенности Цикл
		...
		Если ФайлДоверенности.Пустая() Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Печатная форма недоступна, потому что не заполнены все обязательные реквизиты доверенности.';
														|en = 'The print form is unavailable as some required details of the letter of authority are not filled in.'"), Ссылка);
			Продолжить;
		КонецЕсли;
...
Функция РеквизитыФизическогоЛица(СведенияОФизическомЛице)
	...
	Если СведенияОДокументеУдостоверяющемЛичность <> Неопределено Тогда
		...
		РеквизитыДокумента.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'выдан %1';
				|en = 'issued on %1'"),  Формат(Дата(ДатаВыдачиДокумента), "ДЛФ=D;")));
		...
		Если ЗначениеЗаполнено(ДатаИстеченияСрокаДействияДокумента) Тогда
			РеквизитыДокумента.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'действителен до %1';
					|en = 'valid until %1'"), Формат(Дата(ДатаИстеченияСрокаДействияДокумента), "ДЛФ=D;")));
		КонецЕсли;
...
Функция РеквизитыИндивидуальногоПредпринимателя(СведенияОбИндивидуальномПредпринимателе)
	...
	НаименованиеПолное = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'ИП %1';
																						|en = 'Individual entrepreneur %1'"), Параметры.ФИО);
...
Функция ЗагрузитьДоверенностиВИнформационнуюБазу(ЗагруженныеФайлы, Замещать = Ложь) Экспорт
	...
	Если ФайлыДоверенностей.Количество() = 0 Тогда
		ТекстыОшибок.Добавить(НСтр("ru = 'Не удалось загрузить доверенность - неверный формат.
			...
			|en = 'The format of this letter of authority is not supported.
			|This list supports only the ""003"" format.'"));
	КонецЕсли;
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось прочитать файл %1 по причине:
					|%2';
					|en = 'Couldn''t read the file ""%1"" due to:
					|%2'"),
				ИмяФайла,
	...
	Если КоличествоПропущенных > 1 Тогда
		Если КоличествоПрочитанных > 0 Тогда
			ТекстОшибки = НСтр("ru = 'Не удалось прочитать некоторые файлы доверенностей.
				...
				|en = 'Couldn''t read some LOA files.
				|They might be corrupted or their formats are unsupported.'");
		Иначе
			ТекстОшибки = НСтр("ru = 'Не удалось прочитать файлы доверенностей.
				|Возможно, файлы повреждены или имеют другой формат.';
				|en = 'Couldn''t read the LOA files.
				|They might be corrupted or their formats are unsupported.'");
		КонецЕсли;
	ИначеЕсли КоличествоПропущенных = 1 Тогда
		Если КоличествоПрочитанных > 0 Тогда 
			ТекстОшибки = НСтр("ru = 'Не удалось прочитать один из файлов доверенностей.
				...
				|en = 'Couldn''t read a LOA file.
				|It might be corrupted or its format is unsupported.'");
		Иначе
			ТекстОшибки = НСтр("ru = 'Не удалось прочитать файл доверенности.
				|Возможно, файл поврежден или имеет другой формат.';
				|en = 'Couldn''t read the LOA file.
				|It might be corrupted or its format is unsupported.'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1
				...
				|en = '%1
				|This list supports LOA in the ""003"" format.
...
Функция АвтоНаименование(Объект) Экспорт
	...
	Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1 от %2 %3';
			|en = '%1 dated %2 %3'"), НаименованиеПредставитель, НаименованиеДоверитель,
	...
	ИначеЕсли ТипПолномочия = "1" Тогда
		Для Каждого СведенияОМашиночитаемомПолномочии Из СведенияОМашиночитаемыхПолномочиях Цикл
			...
			ОписаниеПолномочия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = '%1 %2';
				|en = '%1 %2'"), КодПолномочия, НаименованиеПолномочия);
...
Функция ДоверителиПервоначальнойДоверенностиПоФайлу(ФайлДоверенности) Экспорт
	...
	Если ФайлДоверенности.Пустая() Тогда
		Возврат НСтр(
				...
				|en = 'The original principal data is unavailable as no authorization letter file is imported.'");
	КонецЕсли;
	...
	Если ДвоичныеДанные = Неопределено И ДанныеФайла.ПометкаУдаления Тогда
		Возврат НСтр(
			...
			|en = 'The original principal data is unavailable as the authorization letter file is marked for deletion.'");
	КонецЕсли;
	ДоверенностьXML = ПолучитьСтрокуИзДвоичныхДанных(ДвоичныеДанные);
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось прочитать доверенность по причине:
				|%1';
				|en = 'Couldn''t read the LOA. Reason:
				|%1'"),
