...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СтруктураДанных <> Неопределено И СтруктураДанных.Свойство("ТолькоПросмотрФормы")
		И СтруктураДанных.ТолькоПросмотрФормы Тогда
		
		Элементы.Доверитель_ЮридическоеЛицо.ТолькоПросмотр 			= Истина;
		...
		Элементы.ДоверительФЛ_НомЕРН.ТолькоПросмотр 				= Истина;
	...
	ВерсияФормата = ?(СтруктураДанных <> Неопределено И СтруктураДанных.Свойство("ВерсияФормата"),
		СтруктураДанных.ВерсияФормата, "");
	ЭтоПередоверие = СтруктураДанных <> Неопределено И СтруктураДанных.Свойство("ЭтоПередоверие")
		И СтруктураДанных.ЭтоПередоверие;
	ЭтоКорневаяДоверенность = ?(СтруктураДанных <> Неопределено И СтруктураДанных.Свойство("ЭтоКорневаяДоверенность"),
		СтруктураДанных.ЭтоКорневаяДоверенность, Неопределено);
	...
	Если СтруктураДанных <> Неопределено Тогда
		...
		ДоверительФЛ_НомЕРН 					= СтруктураДанных.ДоверительФЛ_НомЕРН;
...
&НаКлиенте
Процедура Доверитель_ЮридическоеЛицоПриИзменении(Элемент)
	УправлениеФормой(ЭтаФорма);
КонецПроцедуры
...
Процедура Сохранить(Команда)
	...
	СтруктураДанных.Вставить("ДоверительЮЛ_РоссийскаяОрганизация", 	НЕ Доверитель_ЮридическоеЛицо
																	ИЛИ ДоверительЮЛ_РоссийскаяОрганизация);
	СтруктураДанных.Вставить("ДоверительЮЛ_НаимОрг", 				?(Доверитель_ЮридическоеЛицо, ДоверительЮЛ_НаимОрг, ""));
	СтруктураДанных.Вставить("ДоверительЮЛ_ИНН", 					?(Доверитель_ЮридическоеЛицо, ДоверительЮЛ_ИНН, ""));
	СтруктураДанных.Вставить("ДоверительЮЛ_КПП", 					?(Доверитель_ЮридическоеЛицо, ДоверительЮЛ_КПП, ""));
	СтруктураДанных.Вставить("ДоверительЮЛ_ОГРН", 					?(Доверитель_ЮридическоеЛицо, ДоверительЮЛ_ОГРН, ""));
	СтруктураДанных.Вставить("ДоверительЮЛ_СтрРег", 				?(Доверитель_ЮридическоеЛицо, ДоверительЮЛ_СтрРег,
																	ПредопределенноеЗначение("Справочник.СтраныМира.ПустаяСсылка")));
	СтруктураДанных.Вставить("ДоверительЮЛ_НаимРегОрг", 			?(Доверитель_ЮридическоеЛицо, ДоверительЮЛ_НаимРегОрг, ""));
	СтруктураДанных.Вставить("ДоверительЮЛ_РегНомер", 				?(Доверитель_ЮридическоеЛицо, ДоверительЮЛ_РегНомер, ""));
	СтруктураДанных.Вставить("ДоверительЮЛ_КодНПРег", 				?(Доверитель_ЮридическоеЛицо, ДоверительЮЛ_КодНПРег, ""));
	СтруктураДанных.Вставить("ДоверительЮЛ_Адр", 					?(Доверитель_ЮридическоеЛицо, ДоверительЮЛ_Адр, ""));
	СтруктураДанных.Вставить("ДоверительЮЛ_АдрВЛатТранскрипции", 	?(Доверитель_ЮридическоеЛицо,
																	ДоверительЮЛ_АдрВЛатТранскрипции, Ложь));
	СтруктураДанных.Вставить("ДоверительЮЛ_АдрXML", 				?(Доверитель_ЮридическоеЛицо, ДоверительЮЛ_АдрXML, ""));
	...
	СтруктураДанных.Вставить("ДоверительФЛ_НомЕРН", 				ДоверительФЛ_НомЕРН);
...
Процедура УправлениеФормой(Форма)
	...
	ЭтоВерсия5_02 = ЗначениеЗаполнено(Форма.ВерсияФормата)
		И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Форма.ВерсияФормата + ".0.0", "5.02.0.0") >= 0;
	...
	Форма.Элементы.ГруппаПравая.Видимость = НЕ Форма.Доверитель_ЮридическоеЛицо
		ИЛИ НЕ Форма.ДоверительЮЛ_РоссийскаяОрганизация И НЕ Форма.ЭтоПередоверие;
	Форма.Элементы.ГруппаПравая.Заголовок = ?(Форма.Доверитель_ЮридическоеЛицо
		...
		НСтр("ru = 'Индивидуальный предприниматель или физическое лицо';
			|en = 'Индивидуальный предприниматель или физическое лицо'"));
	
	Форма.Элементы.ДоверительЮЛ_РоссийскаяОрганизация.Видимость = НЕ Форма.ЭтоПередоверие
		ИЛИ Форма.ЭтоКорневаяДоверенность = Истина;
	...
	Форма.Элементы.ДоверительЮЛ_ИНН.АвтоОтметкаНезаполненного = Форма.ЭтоКорневаяДоверенность <> Истина
		ИЛИ Форма.ДоверительЮЛ_РоссийскаяОрганизация;
	Форма.Элементы.ДоверительЮЛ_ИНН.АвтоВыборНезаполненного = Форма.ЭтоКорневаяДоверенность <> Истина
		ИЛИ Форма.ДоверительЮЛ_РоссийскаяОрганизация;
	Форма.Элементы.ДоверительЮЛ_КПП.АвтоОтметкаНезаполненного = Форма.ЭтоКорневаяДоверенность <> Истина
		ИЛИ Форма.ДоверительЮЛ_РоссийскаяОрганизация;
	Форма.Элементы.ДоверительЮЛ_КПП.АвтоВыборНезаполненного = Форма.ЭтоКорневаяДоверенность <> Истина
		ИЛИ Форма.ДоверительЮЛ_РоссийскаяОрганизация;
	Форма.Элементы.ДоверительЮЛ_Адр.АвтоОтметкаНезаполненного = НЕ Форма.ЭтоПередоверие;
	Форма.Элементы.ДоверительЮЛ_Адр.АвтоВыборНезаполненного = НЕ Форма.ЭтоПередоверие;
	...
	Форма.Элементы.ДоверительЮЛ_Адр.Видимость = НЕ Форма.ЭтоПередоверие;
	Форма.Элементы.ДоверительЮЛ_АдрВЛатТранскрипции.Видимость = НЕ Форма.ДоверительЮЛ_РоссийскаяОрганизация
		И НЕ Форма.ЭтоПередоверие;
	Форма.Элементы.ДоверительЮЛ_ОГРН.АвтоОтметкаНезаполненного = НЕ Форма.ЭтоПередоверие;
	Форма.Элементы.ДоверительЮЛ_ОГРН.АвтоВыборНезаполненного = НЕ Форма.ЭтоПередоверие;
	Форма.Элементы.ДоверительЮЛ_ОГРН.Видимость = Форма.ДоверительЮЛ_РоссийскаяОрганизация
		ИЛИ Форма.ЭтоПередоверие И Форма.ЭтоКорневаяДоверенность <> Истина;
	...
	Форма.Элементы.ДекорацияДоверительФЛ_СНИЛС.Видимость = НЕ Форма.Доверитель_ЮридическоеЛицо;
	Форма.Элементы.ДоверительФЛ_СНИЛС.Видимость = НЕ Форма.Доверитель_ЮридическоеЛицо;
	Форма.Элементы.ДекорацияДоверительФЛ_ИНННомЕРНРазделитель.Видимость = НЕ Форма.Доверитель_ЮридическоеЛицо
		И ЭтоВерсия5_03;
	Форма.Элементы.ДекорацияДоверительФЛ_ИНННомЕРНДопРазделитель.Видимость = НЕ Форма.Доверитель_ЮридическоеЛицо
		И ЭтоВерсия5_03;
	Форма.Элементы.ДекорацияДоверительФЛ_НомЕРН.Видимость = НЕ Форма.Доверитель_ЮридическоеЛицо
		И ЭтоВерсия5_03;
	Форма.Элементы.ДоверительФЛ_НомЕРН.Видимость = НЕ Форма.Доверитель_ЮридическоеЛицо И ЭтоВерсия5_03;
	...
	Форма.Элементы.ДоверительФЛ_Пол.Видимость = Форма.Доверитель_ЮридическоеЛицо
		И НЕ Форма.ДоверительЮЛ_РоссийскаяОрганизация
		И НЕ Форма.ЭтоПередоверие;
	Форма.Элементы.ДоверительФЛ_МестоРожд.Видимость = НЕ Форма.ЭтоПередоверие;
	Форма.Элементы.ДоверительФЛ_ИНН.АвтоОтметкаНезаполненного = НЕ Форма.Доверитель_ЮридическоеЛицо И НЕ ЭтоВерсия5_02;
	Форма.Элементы.ДоверительФЛ_ИНН.АвтоВыборНезаполненного = НЕ Форма.Доверитель_ЮридическоеЛицо И НЕ ЭтоВерсия5_02;
...
Функция СохранениеВозможно()
	...
	Если Доверитель_ЮридическоеЛицо Тогда
		...
		Если НЕ ЗначениеЗаполнено(ДоверительЮЛ_ИНН) И (ЭтоКорневаяДоверенность <> Истина
			ИЛИ ДоверительЮЛ_РоссийскаяОрганизация) Тогда
			
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не задан ИНН организации.';
															|en = 'Не задан ИНН организации.'"),, "ДоверительЮЛ_ИНН",,
				Отказ);
			
		ИначеЕсли ЗначениеЗаполнено(ДоверительЮЛ_ИНН)
			И НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ДоверительЮЛ_ИНН, Ложь) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ДоверительЮЛ_КПП) И (ЭтоКорневаяДоверенность <> Истина
			ИЛИ ДоверительЮЛ_РоссийскаяОрганизация) Тогда
			
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не задан КПП организации.';
															|en = 'Не задан КПП организации.'"),, "ДоверительЮЛ_КПП",,
				Отказ);
			
		ИначеЕсли ЗначениеЗаполнено(ДоверительЮЛ_КПП) И СтрДлина(ДоверительЮЛ_КПП) <> 9 Тогда
		...
		Если НЕ ЗначениеЗаполнено(ДоверительЮЛ_Адр) И НЕ ЭтоПередоверие Тогда
		...
		Если ДоверительЮЛ_РоссийскаяОрганизация И (НЕ ЭтоПередоверие ИЛИ ЭтоКорневаяДоверенность = Истина)
			И НЕ ЗначениеЗаполнено(ДоверительЮЛ_ОГРН) Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(ДоверительЮЛ_ОГРН) И СтрДлина(ДоверительЮЛ_ОГРН) <> 13 Тогда
		...
		Если НЕ ДоверительЮЛ_РоссийскаяОрганизация И НЕ ЭтоПередоверие Тогда
			...
			Если НЕ ЗначениеЗаполнено(ДоверительФЛ_Пол) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не задан пол руководителя обособленного подразделения.';
																|en = 'Не задан пол руководителя обособленного подразделения.'"),,
	...
	Иначе
		...
		Если НЕ ЭтоВерсия5_02 И НЕ ЗначениеЗаполнено(ДоверительФЛ_ИНН) Тогда
		...
		Если ЭтоВерсия5_03 И ЗначениеЗаполнено(ДоверительФЛ_НомЕРН) И СтрДлина(СокрЛП(ДоверительФЛ_НомЕРН)) <> 15 Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Указан некорректный номер ЕРН.';
															|en = 'Указан некорректный номер ЕРН.'"),,
				"ДоверительФЛ_НомЕРН",, Отказ);
		КонецЕсли;
