...
Функция СохранениеВозможно()
	...
	Если НЕ ОтсутствуетКонкретнаяДатаОкончания И НЕ ЗначениеЗаполнено(ДокументДатаОкончания) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не задана дата окончания действия.';
														|en = 'Не задана дата окончания действия.'"),, "ДатаОкончания",,
			Отказ);
	КонецЕсли;
	
	Если ОтсутствуетКонкретнаяДатаОкончания И НЕ ЗначениеЗаполнено(ДокументСрокДействия) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не задан срок действия.';
														|en = 'Не задан срок действия.'"),, "ДокументСрокДействия",,
			Отказ);
	КонецЕсли;
