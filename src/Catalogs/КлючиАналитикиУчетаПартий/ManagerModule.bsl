...
Функция ТекстЗапросаВтТаблицаАналитикУчетаПартий(ТекстВыборкаПоляАналитик, ЗапросОбщий, ТекстыЗапроса = Неопределено) Экспорт
	...
	КонецЕсли;
	Если ЗапросОбщий.Параметры.Свойство("УчитыватьСебестоимостьТоваровПоНазначениям") Тогда
		ЗапросАналитикиПартий.УстановитьПараметр("УчитыватьСебестоимостьТоваровПоНазначениям", ЗапросОбщий.Параметры.УчитыватьСебестоимостьТоваровПоНазначениям);
	КонецЕсли;
...
Функция НаименованиеКлючаАналитики(СтруктураРеквизитовКлюча) Экспорт
	...
	Если ЗначениеЗаполнено(СтруктураРеквизитовКлюча.НалогообложениеНДС) Тогда
		Наименование = Наименование + НСтр("ru = 'Налогообложение НДС';
											|en = 'VAT taxation'", КодОсновногоЯзыка) + ": " + СтруктураРеквизитовКлюча.НалогообложениеНДС + "; ";
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтруктураРеквизитовКлюча.СтавкаНДС) Тогда
		...
		Наименование = Наименование + НСтр("ru = 'Ставка НДС';
											|en = 'VAT rate'", КодОсновногоЯзыка) + ": " + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураРеквизитовКлюча.СтавкаНДС, "Наименование") + "; ";
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СтруктураРеквизитовКлюча.ВидЦенности) Тогда
		Наименование = Наименование + НСтр("ru = 'Вид ценности';
											|en = 'Asset tax class'", КодОсновногоЯзыка) + ": " + СтруктураРеквизитовКлюча.ВидЦенности + "; ";
	КонецЕсли;  	
	
	Если ЗначениеЗаполнено(СтруктураРеквизитовКлюча.КодСтроки) Тогда
		Наименование = Наименование + НСтр("ru = 'Код строки (продукция)';
											|en = 'Line code (manufactured products)'", КодОсновногоЯзыка) + ": " + СтруктураРеквизитовКлюча.КодСтроки;
	КонецЕсли;
