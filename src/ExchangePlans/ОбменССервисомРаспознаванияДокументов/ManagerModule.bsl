...
Процедура СоздатьУзлыСервисаРаспознаванияДокументов() Экспорт
	...
	Если Не ЭтотУзел.Пустая() Тогда
		...
		Объект.Код = "000";
		Объект.Наименование = НСтр("ru = 'Узел этой информационной базы';
									|en = 'This infobase node'", ОбщегоНазначения.КодОсновногоЯзыка());
		Объект.Записать();
	КонецЕсли;
	...
	Если УзелСервисаРаспознаванияДокументов().Пустая() Тогда
		...
		Объект.Код = "001";
		Объект.Наименование = НСтр("ru = 'Узел сервиса распознавания документов';
									|en = 'Document recognition service node'", ОбщегоНазначения.КодОсновногоЯзыка());
		Объект.Записать();
	КонецЕсли;
