		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Не Выборка.ПометкаУдаления Тогда
					...
					Исключение
						ИмяСобытияЖурналРегистрации = НСтр("ru = 'Получение данных в РИБ.';
															|en = 'Receiving data in DIB.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
						Сообщение = НСтр("ru = 'При попытке пометить на удаление дубли элементов справочника СтроковыеКонтактыВзаимодействия произошла ошибка.
											|%1';
											|en = 'An error occurred when trying to mark duplicates of the СтроковыеКонтактыВзаимодействия catalog items for deletion.
											|%1'");
