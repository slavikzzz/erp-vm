...
Функция ПолучитьОписаниеСчета(Счет) 
			...
			Если УровеньСчета > 0 Тогда
				ТекстСпискаСубсчетов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'К счету открыты следующие субсчета %1-го уровня:';
						|en = 'The following subaccounts of the %1 level were opened for the account:'"),
					УровеньСчета + 1);
			Иначе
				ТекстСпискаСубсчетов = НСтр("ru = 'К счету открыты следующие субсчета:';
											|en = 'The following subaccounts were opened for the account:'");
			КонецЕсли;
	...
	Если НЕ Счет.Родитель.Пустая() Тогда
		РеквизитыРодителя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Счет.Родитель, "Код, Наименование");
		ТекстОписаниеРодителя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Описание счета %1 ""%2""';
				|en = 'Description of account %1 ""%2""'"),
			РеквизитыРодителя.Код,
			РеквизитыРодителя.Наименование);
