...
Функция СформироватьГиперссылкиУправленияОтборами(ПолучатьКоличество, ПараметрыОтборов = Неопределено) Экспорт
	...
	ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'К отправке (%1)';
			|en = 'To send (%1)'"), ОбщаяСтатистика.ОжидаютОтправки);
	...
	ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ожидают получения РНПТ (%1)';
			|en = 'Pending RNPT receipt (%1)'"), ОбщаяСтатистика.ОжидаютПолучения);
	ДобавитьГиперссылкуВСписок(МассивСтрок, ТекстГиперссылки, "ОжидаютПолучения");
	//-- НЕ УТ
	...
	ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Архив (%1)';
			|en = 'Archive (%1)'"), ОбщаяСтатистика.Завершены);
...
Функция СтатистикаОтправкиУведомлений(ТипыУведомлений, Организации = Неопределено) Экспорт
	...
	|		ПО Уведомление.Ссылка = СтатусыОтправки.Объект
	//-- НЕ УТ
	|ГДЕ
	...
	Пока Выборка.СледующийПоЗначениюПоля("ТипУведомлений") Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			//++ НЕ УТ
			ИначеЕсли Выборка.Статус = Перечисления.СтатусыОтправки.Отправлен
				ИЛИ Выборка.Статус = Перечисления.СтатусыОтправки.Доставлен Тогда
				СтатусыУведомлений.ОжидаютПолучения = СтатусыУведомлений.ОжидаютПолучения + Выборка.Количество;
			ИначеЕсли Выборка.Статус = Перечисления.СтатусыОтправки.ПустаяСсылка() Тогда
...
Функция СформироватьГиперссылкиКОформлению(ПолучатьКоличество, Организация = Неопределено) Экспорт
	...
	ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Уведомления о ввозе и перемещении (вывозе) (%1)';
			|en = 'Import and transfer (export) notifications (%1)'"),
