...
Процедура ОтобразитьТекущееСообщение()
	...
	Если ЗначениеЗаполнено(СтрокаТЧ.ВидКарты) Тогда
		Если ЗначениеЗаполнено(КартаЛояльности) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Заменить карту лояльности ""%1"" на карту ""%2""';
																							|en = 'Replace loyalty card ""%1"" with card ""%2""'"), ВидЗаменяемойКартыЛояльности, СтрокаТЧ.ВидКарты);
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выдать карту лояльности ""%1""';
																							|en = 'Issue loyalty card ""%1""'"), СтрокаТЧ.ВидКарты);
		КонецЕсли;
	Иначе
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сообщение %1 из %2';
																			|en = 'Message %1 of %2'"), НомерСообщения, ВсегоСообщений);
