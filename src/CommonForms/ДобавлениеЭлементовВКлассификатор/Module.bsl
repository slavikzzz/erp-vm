...
Функция ПолучитьВерсииСписковОтчета(ПолноеИмяОбъекта)
	...
	Для Каждого ТекМакет Из ОбъектМетаданные.Макеты Цикл
		...
		//++ Локализация
		//++ НЕ УТ
		ИначеЕсли ОбъектМетаданные.Имя = "РегламентированныйОтчетСтатистикаФормаП1" И ИмяМакета >= "Списки2024Кв1" Тогда
			
			// Начиная с версии 2024Кв1 структура макета сильно изменилась
			// и он более не подходит для подбора элементов классификатора
			Продолжить;
		...
		ИначеЕсли Лев(ИмяМакета, 20) = "ДанныеКлассификатора" Тогда
			...
			СписокВерсий.Добавить(ИмяМакета, Лев(СтрЗаменить(ИмяМакета, "ДанныеКлассификатора", НСтр("ru = 'Данные классификатора';
																									|en = 'Classifier data'")), 25));
