...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Настройки отчета ""%1""';
				|en = '%1 report settings'"), ВариантНаименование);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЦентрМониторинга") Тогда 
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 режим';
																					|en = '%1 mode'"), ТекущиеРежим.Представление);
	...
	Если Не РезультатВыбораСформирован Тогда
		...
		ОбщегоНазначенияКлиент.ПоказатьПодтверждениеЗакрытияФормы(Оповещение, Отказ, ЗавершениеРаботы,
			НСтр("ru = 'Настройки изменены. Сохранить изменения?';
				|en = 'Settings are changed. Save the changes?'"));
	КонецЕсли;
...
Процедура ОформлениеВыбор(Элемент, ИдентификаторСтроки, Поле, СтандартнаяОбработка)
	...
	Если Строка.ЭтоПараметрВывода Тогда 
		Если Строка(Строка.Идентификатор) = "TITLE"
			...
			ПоказатьВводСтроки(Обработчик, Строка.Значение, НСтр("ru = 'Заголовок для печати';
																|en = 'Printing header'"),, Истина);
		КонецЕсли;
...
Процедура ОформлениеИзменитьЭлемент(ИдентификаторСтроки = Неопределено, Строка = Неопределено)
	...
	ПараметрыФормы.Вставить("Заголовок", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Элемент условного оформления отчета ""%1""';
			|en = 'Conditional report appearance item ""%1""'"), ВариантНаименование));
...
Функция ДоступныеРежимыОтображения(ПоказыватьРежимыФлажков)
	...
	ДоступныеРежимыОтображения.Добавить("ПоказыватьВШапкеОтчета", НСтр("ru = 'В шапке отчета';
																		|en = 'In report header'"), , БиблиотекаКартинок.БыстрыйДоступ);
	
	Если ПоказыватьРежимыФлажков Тогда
		ДоступныеРежимыОтображения.Добавить("ПоказыватьТолькоФлажокВШапкеОтчета", НСтр("ru = 'Только флажок в шапке отчета';
																						|en = 'Check box in report header'"), , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;
	
	ДоступныеРежимыОтображения.Добавить("ПоказыватьВНастройкахОтчета", НСтр("ru = 'В настройках отчета';
																			|en = 'In report settings'"), , БиблиотекаКартинок.Реквизит);
	
	Если ПоказыватьРежимыФлажков Тогда
		ДоступныеРежимыОтображения.Добавить("ПоказыватьТолькоФлажокВНастройкахОтчета", НСтр("ru = 'Только флажок в настройках отчета';
																							|en = 'Check box in report settings'"), , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;
	
	ДоступныеРежимыОтображения.Добавить("НеПоказывать", НСтр("ru = 'Не показывать';
															|en = 'Do not show'"), , БиблиотекаКартинок.СерыйКрест);
