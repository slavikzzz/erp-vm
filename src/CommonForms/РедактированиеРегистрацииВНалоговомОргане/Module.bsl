...
Процедура НаименованиеПриИзменении(Элемент)
	...
	Если ПустаяСтрока(РегистрацияВНалоговомОргане.НаименованиеИФНС) Тогда
		...
		НаименованиеИФНС = СтрЗаменить(НаименованиеИФНС,	НСтр("ru = 'МИФНС';
																|en = 'IIFTS'"),	НСтр("ru = 'Межрайонная инспекция федеральной налоговой службы';
																						|en = 'Interdistrict Inspectorate of the Federal Tax Service'"));
		НаименованиеИФНС = СтрЗаменить(НаименованиеИФНС,	НСтр("ru = 'ИФНС';
																|en = 'IFTS'"),	НСтр("ru = 'Инспекция федеральной налоговой службы';
																						|en = 'Inspectorate of the Federal Tax Service'"));
		НаименованиеИФНС = СтрЗаменить(НаименованиеИФНС,	НСтр("ru = 'ФНС';
																|en = 'FTS'"),	НСтр("ru = 'Федеральная налоговая служба';
																						|en = 'Federal Tax Service'"));
...
Процедура ПрочитатьРегистрациюВНалоговомОргане()
	...
	СведенияОПредставителях = РегистрацияВНОСервер.СведенияОПодписантахПоРегистрации(РегистрацияВНалоговомОргане);
	ОписаниеПредставителейПодписанияОтчетности = РегистрацияВНОСервер.ОписаниеПредставителей(СведенияОПредставителях);
...
Процедура ЗаписатьРегистрациюВНалоговомОргане(ОповещениеЗакрытия = Неопределено)
	...
	ТекстКнопкиДа = НСтр("ru = 'Изменились данные о регистрации в налоговом органе';
						|en = 'Data about registration with the tax authority was changed'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При редактировании изменили данные о регистрации. 
					...
					|en = 'When editing, registration data was changed.
					|If the previous record is corrected (it was incorrect), click ""Error is corrected"".
					|If the registration data changed from %1, click ""%2""'"), 
...
&НаКлиенте
Процедура НачатьВыборПодписантов()
	Обработчик = Новый ОписаниеОповещения("Подключаемый_ПослеВыбораПодписантов", ЭтотОбъект);
	АдресСведенийОПодписантах = АдресСведенийОПодписантах();
	РегистрацияВНОКлиент.ОткрытьФормуРедактированияПредставителей(ЭтотОбъект, АдресСведенийОПодписантах, Обработчик);
КонецПроцедуры

&НаСервере
Функция АдресСведенийОПодписантах()
	Сведения = РегистрацияВНОСервер.СведенияОПодписантахПоРегистрации(РегистрацияВНалоговомОргане);
	Возврат ПоместитьВоВременноеХранилище(Сведения, УникальныйИдентификатор);
КонецФункции

&НаКлиенте
Процедура Подключаемый_ПослеВыбораПодписантов(АдресСведенийОПодписантах, ДополнительныеПараметры) Экспорт
	Если Не ЗначениеЗаполнено(АдресСведенийОПодписантах)
		Или Не ЭтоАдресВременногоХранилища(АдресСведенийОПодписантах) Тогда
		Возврат;
	КонецЕсли;
	Подключаемый_ПослеВыбораПредставителейНаСервере(АдресСведенийОПодписантах);
КонецПроцедуры

&НаСервере
Процедура Подключаемый_ПослеВыбораПредставителейНаСервере(АдресСведенийОПодписантах)
	СведенияОПодписантах = ПолучитьИзВременногоХранилища(АдресСведенийОПодписантах);
	РегистрацияВНОСервер.ЗаполнитьСведенияОПодписантахВРегистрации(РегистрацияВНалоговомОргане, СведенияОПодписантах);
	ОписаниеПредставителейПодписанияОтчетности = РегистрацияВНОСервер.ОписаниеПредставителей(СведенияОПодписантах);
