...
Процедура ЗагрузитьРасширениеПослеПодтверждения(Ответ, Контекст) Экспорт
	...
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("ru = 'Расширение конфигурации';
		...
		Если СинонимыРасширений.СДанными.Количество() = 1 Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Среди отключаемых расширений есть расширение, хранящее данные в приложении, которые станут недоступными.
				           ...
				           | - %1';
				           |en = 'One of the selected extensions stores its data in the application. This data will become unavailable.
		...
		Иначе
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Среди отключаемых расширений есть расширения, хранящие данные в приложении, которые станут недоступными.
				           ...
				           | - %1';
				           |en = 'Some selected extensions store their data in the application. This data will become unavailable.
				           |The related application data might become unchangeable.
				           |
...
Процедура УстановитьЗаголовокЭлементаКомментарий()
	...
	Элементы.СписокРасширенийКомментарий.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Комментарий для %1';
			|en = 'Comment for %1'"), Элементы.СписокРасширений.ТекущиеДанные.Синоним);
...
Процедура ОткрытьФормуЗавершенияРаботыПользователей()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЗавершениеРаботыПользователей") Тогда
		...
		ПараметрыФормы.Заголовок = ?(КоличествоКЗагрузке = 1,
			НСтр("ru = 'Не удалось добавить расширение';
				|en = 'Couldn''t add extension'"), НСтр("ru = 'Не удалось добавить расширения';
		...
		ПараметрыФормы.ТекстСообщенияОбОшибке = ?(КоличествоКЗагрузке = 1, 
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось добавить расширения (%1), т.к. с приложением работают пользователи.';
				|en = 'Couldn''t add extensions (%1): active users detected'"), КоличествоКЗагрузке));
		ПараметрыФормы.ТекстСообщенияОбОшибкеЗавершенияРаботы = ?(КоличествоКЗагрузке = 1, 
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно выполнить добавление расширений (%1), т.к. не удалось завершить работу пользователей:';
				|en = 'Couldn''t add extensions (%1). Failed to terminate the user sessions:'"), КоличествоКЗагрузке));
