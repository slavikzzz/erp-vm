...
Процедура ОКЗавершение(Результат, Контекст = Неопределено) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'Не удалось записать подпись по причине:';
				|en = 'Cannot save the signature due to:'") + Символы.ПС + Результат.ОписаниеОшибки);
			
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru = 'Не удалось добавить электронную подпись из файла';
				|en = 'Cannot add a digital signature from the file'"), "", Ошибка, Новый Структура);
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьФайлы(МножественныйВыбор = Ложь)
	...
	ПараметрыЗагрузки.Диалог.Фильтр = ПараметрыЗагрузки.Диалог.Фильтр + "|" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Все файлы (%1)|%1';
	...
	Если Сертификаты.Количество() = 0 Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'В файле подписи нет ни одного сертификата.';
				|en = 'Signature file does not include any certificates.'"));
