...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Данные о стаже: %1';
																			|en = 'Information on length of service: %1'"), Сотрудник);
	...
	Если ТипЗаписи = 1 Тогда
		...
		ИначеЕсли ДатаНачалаПериода > ДатаОкончанияПериода Тогда
			ТекстСообщения = НСтр("ru = 'Дата начала не может быть больше даты окончания периода.';
									|en = 'Start date cannot be greater than the period end date.'");
			Отказ = Истина;
		КонецЕсли;
	Иначе
		...
		ИначеЕсли Не ЗначениеЗаполнено(НомерДополнительнойЗаписи) Тогда	
			ТекстСообщения = НСтр("ru = 'Не заполнен номер дополнительной записи.';
									|en = 'Number of additional entry is required.'");
			Отказ = Истина;
		КонецЕсли;	
	КонецЕсли;	
