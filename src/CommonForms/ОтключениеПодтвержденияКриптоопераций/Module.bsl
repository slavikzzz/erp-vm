...
Процедура ПриОткрытии(Отказ)
	...
	Если ДлительнаяОперация <> Неопределено Тогда
		ПоказатьСообщение(НСтр("ru = 'Получение настроек';
								|en = 'Getting settings'"), Истина);
...
Процедура ПовторитьКодНажатие(Элемент, СтандартнаяОбработка)
	...
	ПоказатьСообщение(НСтр("ru = 'Выполняется отправка пароля в SMS-сообщении на номер';
							|en = 'Sending SMS message with password to the number'"), Истина);
...
Процедура ОбратныйОтсчетКодТелефона()
	...
	ИначеЕсли ОтключитьОбратныйОтсчет <> 99 Тогда
		Если Таймер > 0 Тогда
			ПовторитьКод = СтрШаблон(НСтр("ru = 'Запросить пароль повторно можно будет через %1 сек.';
		...
		ИначеЕсли ТаймерОбший > 0 Тогда
			СтатусКода = СтрШаблон(НСтр("ru = 'Истекает через %1:%2';
										|en = 'Expires in %1:%2'"), Цел(ТаймерОбший / 60), Формат(ТаймерОбший % 60, "ЧЦ=2; ЧВН="));
		Иначе
...
Процедура ПолучитьВременныйПарольПослеВыполнения(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		ПоказатьСообщение(НСтр("ru = 'Пароль отправлен на номер:';
								|en = 'Password is sent to the number:'") + " " + ПредставлениеТелефона(Получатель));

		ЗапуститьОбратныйОтсчетКодТелефона(Таймер, 600);
		УправлениеФормой(ЭтотОбъект);
		
	Иначе
		ПоказатьСообщение(НСтр("ru = 'Сервис отправки SMS-сообщений временно недоступен.';
								|en = 'SMS message sending service is temporarily unavailable.'"), , Истина);
...
Процедура ПроверитьКодТелефона(ТекущийКод)
	...
	ИначеЕсли ЗначениеЗаполнено(ТекущийКод) И СтрДлина(ТекущийКод) = 6 Тогда
		...
		ПоказатьСообщение(НСтр("ru = 'Выполняется проверка пароля';
								|en = 'Checking the password'"), Истина);
