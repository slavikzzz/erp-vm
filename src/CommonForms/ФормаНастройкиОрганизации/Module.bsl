...
Процедура Подключаемый_ВыполнитьКомандуСоздатьШаблон(Команда)
	...
	ПредставлениеДокумента  = СокрЛП(СтрЗаменить(СтрЗаменить(Кнопка.Заголовок, НСтр("ru = 'Шаблон для';
																					|en = 'Template type'"), ""), """", ""));
	НаименованиеШаблона     = СтрШаблон(НСтр("ru = 'Счет на оплату через Яндекс.Кассу (%1)';
											|en = 'Proforma invoice via Yandex.Checkout (%1)'"), ПредставлениеДокумента);
			...
			Исключение
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Полнотекстовый поиск';
						|en = 'Full-text search'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьНесколькоКасс" ИЛИ РеквизитПутьКДанным = "" Тогда
		...
		Элементы.Кассы.Заголовок = ?(ЗначениеКонстанты,НСтр("ru = 'Кассы предприятия';
															|en = 'Cash accounts'"),НСтр("ru = 'Настройка кассы';
																							|en = 'Cash account settings'"));;
	КонецЕсли;
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ПредставлениеВходящихДокументов" ИЛИ РеквизитПутьКДанным = "" Тогда
		...
		Шаблон = НСтр("ru = 'Способ отображения представления входящих документов в отчетах, списках и печатных формах.
							|Пример: %1';
							|en = 'Method of displaying the incoming document presentation in reports, lists, and print forms.
							|Example:%1'");
		...
		Если НаборКонстант.ПредставлениеВходящихДокументов = Перечисления.СпособыПредставленияВходящихДокументов.Внешнее Тогда
			Элементы.ПредставлениеВходящихДокументов.Подсказка = СтрШаблон(Шаблон, НСтр("ru = 'вх. УПД 553 от 27.04.2021';
																						|en = 'inc. UTD 553 from 04/27/2021'"));
		ИначеЕсли НаборКонстант.ПредставлениеВходящихДокументов = Перечисления.СпособыПредставленияВходящихДокументов.Полное Тогда
			Элементы.ПредставлениеВходящихДокументов.Подсказка = СтрШаблон(Шаблон,
				НСтр("ru = 'Приобретение товаров и услуг ТД00-000044 от 28.04.2021 13:33:22 (вх. УПД 553 от 27.04.2021)';
					|en = 'Vendor invoice TD00-000044 from 04/28/2021 01:33:22 PM (inc. UTD 553 from 04/27/2021)'"));
		Иначе
			Элементы.ПредставлениеВходящихДокументов.Подсказка = СтрШаблон(Шаблон,
				НСтр("ru = 'Приобретение товаров и услуг ТД00-000044 от 28.04.2021 13:33:22';
					|en = 'Vendor invoice TD00-000044 from 04/28/2021 01:33:22 PM'"));
		КонецЕсли;
	КонецЕсли;
