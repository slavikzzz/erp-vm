...
Функция ПолучитьТекстИнструкции(Представление = Неопределено, АдресПечатнойФормы = Неопределено)
	...
	Если Представление = Неопределено Тогда	
		ТекстИнструкции = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстИнструкции, НСтр("ru = 'Документы сформированы и подготовлены к печати';
																										|en = 'The documents are generated and prepared for printing'"), НСтр("ru = 'Можно открыть документ для просмотра или отправить его на печать.';
																																										|en = 'You can open the document to view or send it to print.'"));
	Иначе
		ТекстИнструкции = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстИнструкции, НСтр("ru = 'Документ сформирован и подготовлен к печати';
																										|en = 'The document is generated and prepared for printing'"), "<a href=""ОткрытьЕдинственнуюПечатнуюФорму"">"+Представление+"</a>");
	КонецЕсли;
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("ОбщаяФорма.СохранениеПечатнойФормы") Тогда
		...
		Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение <> КодВозвратаДиалога.Отмена Тогда
			...
			ИначеЕсли ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
				...
				ПоказатьОповещениеПользователя(НСтр("ru = 'Сохранение завершено';
													|en = 'Saved'"), , , БиблиотекаКартинок.ДиалогИнформация);
			КонецЕсли;
		КонецЕсли;
...
Функция ПоместитьФайлыВАрхив(ПечатныеФормыДокументов, ПереданныеНастройки)
	...
	Для Каждого СтруктураФайла Из ПечатныеФормыДокументов Цикл
		...
		Если СоответствиеДляАрхивов[ОбъектПечати] = Неопределено Тогда
			...
			Представление = ?(ОбъектПечати = Неопределено, НСтр("ru = 'Документы';
																|en = 'Documents'"), ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(Строка(ОбъектПечати)));
...
#Если Не ВебКлиент Тогда
	Если ЗначениеЗаполнено(ИмяКаталога) Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Сохранение успешно завершено';
											|en = 'Saved successfully.'"), ОписаниеОповещения,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'в папку %1';
																		|en = 'Folder: %1'"), ИмяКаталога), БиблиотекаКартинок.ДиалогИнформация);
	КонецЕсли;
#КонецЕсли
