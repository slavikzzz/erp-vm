...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ТипЗнч(Источник) <> Тип("ФормаКлиентскогоПриложения") И ТребуетсяПозиционирование Тогда
	...
	Если ДополнительныеПараметры.ВыводитьОшибку Тогда
		Если Результат.Статус = "Ошибка" Тогда
			ВидОперации = НСтр("ru = 'Обновление статусов отчетов.';
								|en = 'Обновление статусов отчетов.'");
...
Функция ЗаданиеПоОбновлениюСтатусовНаСервере(Знач ИдентификаторФормы, Знач ПараметрыКлиента)
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = Нстр("ru = 'Получение статусов отчетов для банков.';
																|en = 'Получение статусов отчетов для банков.'");
...
Процедура ПослеФоновогоОбновленияСтатусовОтчетов(Результат, ДополнительныеПараметры) Экспорт
	...
	// Функция вызывается как обработчиком ожидания, так и при обновлении статусов отчетов пользователем.
	// ДополнительныеПараметры.ВыводитьОшибку = Истина при запуске обновления статусов пользователем.
	// Результат = Неопределено при отмене фонового обновления статусов из формы ожидания длительной операции.
