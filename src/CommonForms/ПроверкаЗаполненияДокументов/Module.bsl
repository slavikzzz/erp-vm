	...
	Если Параметры.Свойство("ВыгруженаТолькоНеМаркируемаяПродукция", ВыгруженаТолькоНеМаркируемаяПродукция) Тогда
		Если ВыгруженаТолькоНеМаркируемаяПродукция Тогда
			Заголовок = НСтр("ru = 'Проверка количества товаров в документе (немаркируемая продукция)';
							|en = 'Goods quantity check in the document (non-marked products)'");
		КонецЕсли;
	КонецЕсли;
		...
		Иначе
			...
			ПоказатьВопрос(
				...
				НСтр("ru = 'Завершить проверку без переноса расхождений в документ?';
					|en = 'Finish checking without discrepancy transfer to the document?'"),
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Номенклатура) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не заполнено поле ""Номенклатура"" в строке %1 списка ""Товары""';
																							|en = '""Items"" in line %1 of the ""Goods"" list is required'"), НомерСтрокиДляПользователя);
		...
		Если ИспользоватьХарактеристикиНоменклатуры И Не ЗначениеЗаполнено(СтрокаТЧ.Характеристика) И СтрокаТЧ.ХарактеристикиИспользуются Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не заполнено поле ""Характеристика"" в строке %1 списка ""Товары""';
																							|en = '""Variant"" in line %1 of the ""Goods"" list is required'"), НомерСтрокиДляПользователя);
		...
		Если Выборка.ОшибкаПоТипуНоменклатуры Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Тип номенклатуры %1 в строке %2 списка ""Товары"" отличается от допустимых типов: %3';
					|en = 'Type of item %1 in line %2 of the Item list differs from available types: %3'"),
		...
		Если Выборка.ОшибкаПоМаркировке Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Номенклатура %1 в строке %2 списка ""Товары"" не может быть маркируемой.';
					|en = '%1 item in line %2 of the ""Goods"" list cannot be the marked one.'"),
	...
	Если ВыгруженаТолькоНеМаркируемаяПродукция Тогда
		Если НоменклатураЛокализация.ЭтоМаркируемаяПродукция(Номенклатура) Тогда
			...
			НСтр("ru = 'Номенклатура %1 маркируемая. Можно выбрать только немаркируемую продукцию';
				|en = '%1 item is the marked one. You can select only non-marked products'"),
			Номенклатура);
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	Для Каждого СтрокаТЧ Из НеудаляемыеСтроки Цикл
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Строку %1 удалить нельзя, т.к. она присутствует в исходном документе.';
																						|en = 'Line %1 cannot be deleted as it is in the original document.'"), Товары.Индекс(СтрокаТЧ) + 1);
...
Процедура ЗавершитьПроверку(Команда)
	...
	Если    МассивСтрокНовыеТовары.Количество() > 0
		...
		ТекстВопроса = НСтр("ru = 'Обнаружены расхождения между количеством товаров в документе и результатом проверки. 
									...
									|en = 'Discrepancies are found between the quantity of goods in the document and the check result. 
									|Do you want to transfer the actual data to the document?'");
