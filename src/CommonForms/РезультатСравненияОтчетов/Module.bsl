...
Процедура ПриОткрытии(Отказ)
	...
	Если РазделыОтчетаРазличия.ПолучитьЭлементы().Количество() = 0 Тогда
		...
		ПоказатьПредупреждение( , НСтр("ru = 'Данные сравниваемых отчетов не содержат различий';
										|en = 'Данные сравниваемых отчетов не содержат различий'"));
...
Процедура РазделыОтчетаПриАктивизацииСтроки(Элемент)
	Если Элемент.ТекущиеДанные = Неопределено ИЛИ Элемент.ТекущаяСтрока = ТекущаяСтрокаРазделовОтчета Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура РазделыОтчетаПриАктивизацииСтрокиЗавершение()
	Элемент = Элементы.РазделыОтчета;
	ВывестиРезультатВТабДокНаСервере(НазваниеРазделаСокр(Элемент.ТекущиеДанные), Элемент.ТекущиеДанные.КодСтраницы);
КонецПроцедуры
	   ...
	   И Элемент.ТекущаяОбласть.Текст = "-" Тогда
		ПоказатьПредупреждение( ,
			НСтр("ru = 'Текущая страница отсутствует в соответствующем разделе сравниваемого отчета.
					 ...
					 |en = 'Текущая страница отсутствует в соответствующем разделе сравниваемого отчета.
					 |Переход на нее невозможен.'"));
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Асинх Процедура ПоказатьДиалогВыбораФайла()
	...
	Исключение
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не удалось записать файл на диск. Возможно, диск защищен от записи или недостаточно места на диске.';
														|en = 'Не удалось записать файл на диск. Возможно, диск защищен от записи или недостаточно места на диске.'"));
	КонецПопытки;
КонецПроцедуры
...
Процедура ПоказатьОтменитьТолькоРазличияНаСервере(ИмяРаздела, КодСтраницы)
	...
	Если НЕ ПоказатьТолькоРазличия Тогда
		Команды.ПоказатьОтменитьТолькоРазличия.Подсказка = НСтр("ru = 'Показать только разделы и строки, содержащие различия';
																|en = 'Показать только разделы и строки, содержащие различия'");
		КопироватьДанныеФормы(РазделыОтчетаВсе, РазделыОтчета);
	Иначе
		Команды.ПоказатьОтменитьТолькоРазличия.Подсказка = НСтр("ru = 'Показать все разделы и строки';
																|en = 'Показать все разделы и строки'");
		КопироватьДанныеФормы(РазделыОтчетаРазличия, РазделыОтчета);
