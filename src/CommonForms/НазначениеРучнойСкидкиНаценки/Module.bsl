...
Процедура ОК(Команда)
	...
	Если ИспользоватьОграниченияРучныхСкидок Тогда
		...
		Если ВариантПредоставления = 1 Тогда
			Если СуммаРучнойСкидкиНаценки > МаксимальнаяСуммаСкидкиДляКонтроля Тогда
				ТекстСообщения = НСтр("ru = 'Сумма скидки превышает максимальное возможное значение:';
										|en = 'Discount amount exceeds the maximum possible value:'") + " " + МаксимальнаяСуммаСкидкиДляКонтроля + " " + Валюта;
			КонецЕсли;
		КонецЕсли;
		
		Если ВариантПредоставления = 2 Тогда
			Если СуммаРучнойСкидкиНаценки > МаксимальнаяСуммаНаценкиДляКонтроля Тогда
				ТекстСообщения = НСтр("ru = 'Сумма наценки превышает максимальное возможное значение:';
										|en = 'Markup amount exceeds the maximum allowed value:'") + " " + МаксимальнаяСуммаНаценкиДляКонтроля + " " + Валюта;
			КонецЕсли;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекстСообщения) Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ОКЗавершение", ЭтотОбъект), ТекстСообщения + ". " + НСтр("ru = 'Продолжить?';
																												|en = 'Continue?'"), РежимДиалогаВопрос.ОКОтмена);
            Возврат;
		КонецЕсли;
...
Процедура СуммаДокументаСУчетомСкидкиПодготовитьСписокВыбора()
	...
	Пока ТочностьОкругления <= МаксимальноеТочностьОкругления Цикл
		...
		Если Варианты.Найти(Значение) = Неопределено Тогда
			...
			СоответствиеТочностей.Вставить(Значение, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '(+окр.до %1)';
																													|en = '(+round. to %1)'"), ТочностьОкругления));
		КонецЕсли;
