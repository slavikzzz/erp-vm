		...
		Если КоличествоПомеченныхНаУдаление = 0 Тогда 
			...
			Сценарий.Оповещение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Пометка удаления установлена (%1)';
					|en = 'Deletion mark set (%1)'"), КоличествоВыбранныхЭлементов);
		...
		ИначеЕсли КоличествоПомеченныхНаУдаление = КоличествоВыбранныхЭлементов Тогда 
			...
			Сценарий.Оповещение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Пометка удаления снята (%1)';
					|en = 'Deletion mark removed (%1)'"), КоличествоВыбранныхЭлементов);
...
Функция ИзменитьПометкуУдаленияЭлементов(ВыбранныеЭлементы, Пометка)
	...
	Для Каждого Элемент Из ВыбранныеЭлементы Цикл 
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Связанные документы.Изменение пометки удаления';
											|en = 'Related documents.Change deletion mark'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ОбработанныеДокументы(ВыбранныеДокументы, Режим, Ошибки)
	...
	Пока Индекс >= 0 Цикл 
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Связанные документы.Проведение документа';
											|en = 'Related documents.Post documents'", ОбщегоНазначения.КодОсновногоЯзыка()),
