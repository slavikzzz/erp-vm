...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	ИначеЕсли Настройки["ОрганизацияЕГАИС"] = Неопределено Тогда
		...
		НастройкиОбменаЕГАИС = ОбменДаннымиЕГАИСВызовСервера.НастройкиОбменаЕГАИС();
...
&НаКлиенте
Процедура ТекстОжиданиеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	ТекстОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка);
	
КонецПроцедуры

&НаКлиенте
Процедура ТекстРезультатОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ТекстОшибкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
		...
		Если Не ЗначениеЗаполнено(ОрганизацияЕГАИС) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Не заполнено поле ""Организация ЕГАИС""';
					|en = 'Не заполнено поле ""Организация ЕГАИС""'"),,
				"ОрганизацияЕГАИС",, Отказ);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ЗначениеПараметра) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'Не заполнено поле ""%1""';
								|en = 'Не заполнено поле ""%1""'"), ПредставлениеПараметра),,
	...
	Иначе
		...
		Для Каждого СтрокаТаблицы Из ИсходящиеЗапросы Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.ЗначениеПараметра) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					СтрШаблон(НСтр("ru = 'Не заполнено поле ""%1""';
									|en = 'Не заполнено поле ""%1""'"), СтрокаТаблицы.ПредставлениеПараметра),,
...
Функция ПараметрыЗапроса()
	...
	ПараметрыЗапроса.Вставить("Операция");
	ПараметрыЗапроса.Вставить("ИмяПараметра");
	ПараметрыЗапроса.Вставить("ЗначениеПараметра");
	ПараметрыЗапроса.Вставить("ПредставлениеПараметра");
...
Функция ПодготовитьМассивЗапросовКПередаче(МассивЗапросов)
	...
	Для Каждого ДанныеЗапроса Из МассивЗапросов Цикл
		...
		Если ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросАлкогольнойПродукции")
			ИЛИ ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросДанныхОрганизации") Тогда
			
			ПодготовленныеДанные = ОбменДаннымиЕГАИСВызовСервера.ПодготовитьЗапросДанныхКлассификатораКПередаче(
				ДанныеЗапроса.ОрганизацияЕГАИС,
				ДанныеЗапроса.Операция,
				ДанныеЗапроса.ИмяПараметра,
				СокрЛП(ДанныеЗапроса.ЗначениеПараметра));
				
		ИначеЕсли ОбщегоНазначенияЕГАИС.ЭтоРасширеннаяВерсияГосИС() Тогда
			...
			Если ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросСправки1")
				...
				ПодготовленныеДанные = МодульИнтеграцияЕГАИСВызовСервера.ПодготовитьЗапросДанныхСправкиКПередаче(
					ДанныеЗапроса.ОрганизацияЕГАИС,
					ДанныеЗапроса.Операция,
					СокрЛП(ДанныеЗапроса.ЗначениеПараметра));
			...
			ИначеЕсли ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросТТН") Тогда
				...
				МодульИнтеграцияЕГАИСВызовСервера = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияЕГАИСВызовСервера");
				ПодготовленныеДанные = МодульИнтеграцияЕГАИСВызовСервера.ПодготовитьЗапросДанныхТТНКПередаче(ДанныеЗапроса.ОрганизацияЕГАИС, СокрЛП(ДанныеЗапроса.ЗначениеПараметра));
			...
			ИначеЕсли ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросПроверкиАкцизныхМарокНаОстаткахОрганизации") Тогда
				...
				МодульИнтеграцияЕГАИСВызовСервера = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияЕГАИСВызовСервера");
				ПодготовленныеДанные = МодульИнтеграцияЕГАИСВызовСервера.ПодготовитьЗапросДанныхТТНКПередаче(ДанныеЗапроса.ОрганизацияЕГАИС, АкцизныеМарки);
...
Процедура ПодготовитьЗапросКПередаче(ОповещениеПриЗавершении)
	...
	Если Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросАлкогольнойПродукции")
		ИЛИ Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросДанныхОрганизации") Тогда
		
		ОчиститьСообщения();
	
		ДанныеДляВыполненияОбменаНаКлиенте = ОбменДаннымиЕГАИСВызовСервера.ПодготовитьЗапросДанныхКлассификатораКПередаче(
			ОрганизацияЕГАИС, Операция, ИмяПараметра, СокрЛП(ЗначениеПараметра), УникальныйИдентификатор);
		ДанныеДляВыполненияОбменаНаКлиенте.Вставить("ВОсновнойФорме");
		ОбменДаннымиЕГАИССлужебныйКлиент.ОбработатьОчередьПередачиДанных(
			ОповещениеПриЗавершении, ДанныеДляВыполненияОбменаНаКлиенте);
	
	ИначеЕсли ОбщегоНазначенияЕГАИСКлиент.ЭтоРасширеннаяВерсияГосИС() Тогда
		...
		Если Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросСправки1")
			...
			МодульИнтеграцияЕГАИСКлиент.ПодготовитьЗапросДанныхСправкиКПередаче(
		...
		ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросТТН") Тогда
			...
			МодульИнтеграцияЕГАИСКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияЕГАИСКлиент");
			МодульИнтеграцияЕГАИСКлиент.ПодготовитьЗапросДанныхТТНКПередаче(
				ОповещениеПриЗавершении,
				ОрганизацияЕГАИС,
				СокрЛП(ЗначениеПараметра),
				УникальныйИдентификатор);
		...
		ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросПроверкиАкцизныхМарокНаОстаткахОрганизации") Тогда
			...
			МодульИнтеграцияЕГАИСКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияЕГАИСКлиент");
			МодульИнтеграцияЕГАИСКлиент.ПодготовитьЗапросПроверкиАкцизныхМарокНаОстаткахОрганизацииКПередаче(
				ОповещениеПриЗавершении,
				ОрганизацияЕГАИС,
				АкцизныеМарки,
				УникальныйИдентификатор);
...
Процедура ВыполнитьОбмен()
	...
	ОбменДаннымиЕГАИСКлиент.ВыполнитьОбмен(МассивОрганизацийЕГАИС, ОповещениеПриЗавершении, Истина);
		...
		Если ОбработанныеИзменения.Количество() = 0 Тогда
			...
			Для Каждого Изменение Из Изменения Цикл
				Если ЗначениеЗаполнено(Изменение.ТекстОшибки) Тогда
					ОбщегоНазначенияКлиент.СообщитьПользователю(Изменение.ТекстОшибки);
				КонецЕсли;
			КонецЦикла;
			Возврат;
		Иначе
...
Функция НайтиОбработанныеСообщения(Сообщения)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВходящееСообщениеОперация = Сообщения[Выборка.ИсходящееСообщение].ОжидаемаяОперация Тогда
			...
			ТекстСообщенияXML = ПротоколОбменаИС.ТекстСообщенияXMLИзПротокола(Выборка.ВходящееСообщение);
			...
			Если Выборка.ВходящееСообщениеОперация = Перечисления.ВидыДокументовЕГАИС.ОтветНаЗапросПроверкиАкцизныхМарокНаОстаткахОрганизации Тогда
				...
				Объект = МодульИнтеграцияЕГАИС.ПрочитатьОтветНаЗапросПроверкиАкцизныхМарокНаОстаткахОрганизации(
					ТекстСообщенияXML, Выборка.ОрганизацияЕГАИС);
				
			Иначе
				
				ВходящиеДанные = ОбменДаннымиЕГАИСКлиентСервер.СтруктураЗагрузкиВходящегоДокумента(
				...
				РезультатОбработки = ОбменДаннымиЕГАИСВызовСервера.ОбработатьВходящиеДокументы(ДокументыКЗагрузке).Изменения;
		...
		Строки.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru = 'Получение ответов от сервера может занять продолжительное время.
				           ...
				           |выполнения операции в фоновом режиме.';
				           |en = 'Получение ответов от сервера может занять продолжительное время.
				           |Дождитесь ответов или закройте окно для продолжения
				           |выполнения операции в фоновом режиме.'")));
	...
	Иначе
		Если ЕстьОшибкиОбработкиОтветов() Тогда
			Строки.Добавить(НСтр("ru = 'Обмен данными завершен с ошибками.';
								|en = 'Обмен данными завершен с ошибками.'"));
		Иначе
			Строки.Добавить(НСтр("ru = 'Обмен данными успешно завершен.';
								|en = 'Обмен данными успешно завершен.'"));
		КонецЕсли;
...
Функция ТекстОбработанногоЗапроса(ДанныеЗапроса, Объект, ТекстОшибки = "", ИндексТаблицыЗапросов = "")
	...
	Если Объект <> Неопределено Тогда
		...
		Если ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросСправки1") Тогда
			...
			Строки.Добавить(
				Новый ФорматированнаяСтрока(
					СтрШаблон(НСтр("ru = 'справки 1 по номеру %1 получен';
									|en = 'справки 1 по номеру %1 получен'"), СокрЛП(ДанныеЗапроса.ЗначениеПараметра))));
		...
		ИначеЕсли ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросСправки2") Тогда
			...
			Строки.Добавить(
				Новый ФорматированнаяСтрока(
					СтрШаблон(НСтр("ru = 'справки 2 по номеру %1 получен';
									|en = 'справки 2 по номеру %1 получен'"), СокрЛП(ДанныеЗапроса.ЗначениеПараметра))));
		...
		ИначеЕсли ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросАлкогольнойПродукции") Тогда
			...
			Если ВРег(ДанныеЗапроса.ИмяПараметра) = "ИНН" Тогда
				Строки.Добавить(
					Новый ФорматированнаяСтрока(
						СтрШаблон(НСтр("ru = 'алкогольной продукции по ИНН производителя/импортера %1 получен';
										|en = 'алкогольной продукции по ИНН производителя/импортера %1 получен'"), СокрЛП(ДанныеЗапроса.ЗначениеПараметра))));
			ИначеЕсли ВРег(ДанныеЗапроса.ИмяПараметра) = "КОД" Тогда
				Строки.Добавить(
					Новый ФорматированнаяСтрока(
						СтрШаблон(НСтр("ru = 'алкогольной продукции по коду %1 получен';
										|en = 'алкогольной продукции по коду %1 получен'"), СокрЛП(ДанныеЗапроса.ЗначениеПараметра))));
			КонецЕсли;
		...
		ИначеЕсли ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросДанныхОрганизации") Тогда
			...
			Если ВРег(ДанныеЗапроса.ИмяПараметра) = "ИНН" Тогда
				Строки.Добавить(
					Новый ФорматированнаяСтрока(
						СтрШаблон(НСтр("ru = 'организации ЕГАИС по ИНН %1 получен';
										|en = 'организации ЕГАИС по ИНН %1 получен'"), СокрЛП(ДанныеЗапроса.ЗначениеПараметра))));
			ИначеЕсли ВРег(ДанныеЗапроса.ИмяПараметра) = "СИО" Тогда
				Строки.Добавить(
					Новый ФорматированнаяСтрока(
						СтрШаблон(НСтр("ru = 'организации ЕГАИС по коду ФСРАР %1 получен';
										|en = 'организации ЕГАИС по коду ФСРАР %1 получен'"), СокрЛП(ДанныеЗапроса.ЗначениеПараметра))));
			КонецЕсли;
				...
				Строки.Добавить(
					Новый ФорматированнаяСтрока(
						НСтр("ru = 'Загружена организация ЕГАИС';
							|en = 'Загружена организация ЕГАИС'")));
				Строки.Добавить(" ");
				Строки.Добавить(Новый ФорматированнаяСтрока(ПредставлениеРезультата,, ЦветГиперссылки,, "ОткрытьРезультат" + ИндексТаблицыЗапросов));
				Строки.Добавить(".");
			Иначе
				Строки.Добавить(
					Новый ФорматированнаяСтрока(
						СтрШаблон(НСтр("ru = 'Загружено организаций ЕГАИС: %1';
										|en = 'Загружено организаций ЕГАИС: %1'"), МассивОрганизацийЕГАИС.Количество())));
				Строки.Добавить(".");
				Строки.Добавить(" ");
				Строки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Показать';
																|en = 'Показать'"),, ЦветГиперссылки,, "ОткрытьРезультат" + ИндексТаблицыЗапросов));
		...
		ИначеЕсли ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросТТН") Тогда
			...
			Строки.Добавить(
				Новый ФорматированнаяСтрока(
					СтрШаблон(НСтр("ru = 'ТТН ЕГАИС (входящая) по идентификатору %1 получен';
									|en = 'ТТН ЕГАИС (входящая) по идентификатору %1 получен'"), СокрЛП(ДанныеЗапроса.ЗначениеПараметра))));
		...
		ИначеЕсли ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросПроверкиАкцизныхМарокНаОстаткахОрганизации") Тогда
			...
			Строки.Добавить(
				Новый ФорматированнаяСтрока(
					СтрШаблон(НСтр("ru = 'проверки кода акцизных марок %1 получен';
									|en = 'проверки кода акцизных марок %1 получен'"), СокрЛП(ДанныеЗапроса.ЗначениеПараметра))));
			...
			Если Объект.ОтсутствующиеАкцизныеМарки <> Неопределено И Объект.ОтсутствующиеАкцизныеМарки.Количество() > 0 Тогда
				Строки.Добавить(Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ru = 'На остатках организации не найдено %1 акцизных марок';
																			|en = 'На остатках организации не найдено %1 акцизных марок'"), Объект.ОтсутствующиеАкцизныеМарки.Количество())));
				Строки.Добавить(".");
				Строки.Добавить(" ");
				Строки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Показать';
																|en = 'Показать'"),, ЦветГиперссылки,, "ОткрытьРезультат" + ИндексТаблицыЗапросов));
	...
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, "ОткрытьРезультат") Тогда
		...
		Если ОперацияОбъекта = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросПроверкиАкцизныхМарокНаОстаткахОрганизации") Тогда
			...
			ТекстовыйДокумент.Показать(НСтр("ru = 'Отсутствующие акцизные марки';
											|en = 'Отсутствующие акцизные марки'"));
