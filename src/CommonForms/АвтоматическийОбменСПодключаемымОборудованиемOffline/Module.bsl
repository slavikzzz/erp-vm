...
Процедура Начать(Команда)
	...
	Если Не ЗначениеЗаполнено(ПериодичностьОбмена) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не задана периодичность обмена с оборудованием';
																|en = 'Frequency of exchange with equipment is not specified'"),,"ПериодичностьОбмена");
		Возврат;
	КонецЕсли;
	
	Если Не ЕстьОборудованиеДляОбмена() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбрано оборудование для обмена';
																|en = 'Equipment for exchange is not selected'"),,"Оборудование");
		Возврат;
	КонецЕсли;
	...
	Состояние = НСтр("ru = 'Выполняется обмен с подключенным оборудованием...';
					|en = 'Performing data exchange with peripherals...'");
	...
	Если ОбменВыполняется Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект), 
			НСтр("ru = 'После закрытия обмен с оборудованием выполняться не будет. Закрыть и остановить обмен?';
				|en = 'Exchange with the equipment will not be executed after closing. Close and stop the exchange?'"), 
