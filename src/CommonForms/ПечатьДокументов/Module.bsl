...
Процедура ЗавершениеПодписанияСохранитьВПапку(Результат, Контекст) Экспорт
	...
	ПоказатьОповещениеПользователя(, , НСтр("ru = 'Подписание и сохранение завершено';
											|en = 'Signed and saved'"), БиблиотекаКартинок.ДиалогИнформация);
КонецПроцедуры
...
Процедура ЗавершениеПодписанияФайлов(Результат, Контекст) Экспорт	
	...
	ПоказатьОповещениеПользователя(, , НСтр("ru = 'Сохранение и подписание завершено';
											|en = 'Saved and signed'"), БиблиотекаКартинок.ДиалогИнформация);
...
Процедура УстановитьИмяПринтераВПодсказкеКнопкиПечать()
	...
	Если Не ПустаяСтрока(ИмяПринтера) И Не СтрНайти(Подсказка.Заголовок, ИмяПринтера) Тогда
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Напечатать на принтере (%1)';
				|en = 'Printer (%1)'"), ИмяПринтера);
	КонецЕсли;
