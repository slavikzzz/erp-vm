		...
		Если ЗначениеЗаполнено(Идентификатор) Тогда
			...
			Если ИсходящиеЗапросы.НайтиСтроки(СтруктураПоиска).Количество() Тогда
				Если ТипЗапроса = "Партии" Тогда
					ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Партия %1 уже добавлена';
																				|en = 'Партия %1 уже добавлена'"), Идентификатор));
				Иначе
					ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(НСтр("ru = 'СДИЗ %1 уже добавлен';
																				|en = 'СДИЗ %1 уже добавлен'"), Идентификатор));
				КонецЕсли;
			Иначе
		...
		Иначе
			...
			Если ТипЗапроса = "Партии" Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					СтрШаблон(
						НСтр("ru = 'Из штрихкода %1 не удалось получить номер партии';
							|en = 'Из штрихкода %1 не удалось получить номер партии'"), Штрихкод));
			Иначе
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					СтрШаблон(
						НСтр("ru = 'Из штрихкода %1 не удалось получить номер СДИЗ';
							|en = 'Из штрихкода %1 не удалось получить номер СДИЗ'"), Штрихкод));
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(КонецПериода)
			...
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Дата конца периода загрузки должна быть больше даты начала.';
					|en = 'Дата конца периода загрузки должна быть больше даты начала.'"),,
				"КонецПериода",,
				Отказ);
		КонецЕсли;
		...
		ТекстРезультатаВыполненияОбмена = Новый ФорматированнаяСтрока(
			СтрШаблон(
				НСтр( "ru = 'Результат обмена данным с ошибками (%1)';
						|en = 'Результат обмена данным с ошибками (%1)'"),
				КоличествоОшибок),, ЦветПроблема,, "ОткрытьРезультат");
	Иначе
		ТекстРезультатаВыполненияОбмена = Новый ФорматированнаяСтрока(
			НСтр( "ru = 'Результат обмена данным';
					|en = 'Результат обмена данным'"),,,, "ОткрытьРезультат");
	КонецЕсли;
