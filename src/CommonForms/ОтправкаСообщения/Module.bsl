	...
	ИначеЕсли ТипЗнч(Параметры.Отправитель) = Тип("СписокЗначений") Тогда
		...
		Если НаборУчетныхЗаписей.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Не указаны учетные записи для отправки сообщения, обратитесь к администратору системы.';
									|en = 'Accounts for sending mail are not specified. Please contact your system administrator.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,,, Отказ);
			Возврат;
		КонецЕсли;
	...
	Если СообщениеОтправлено Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Сообщение отправлено:';
			...
			?(ПустаяСтрока(ТемаПисьма), НСтр("ru = '<Без темы>';
											|en = '<No subject>'"), ТемаПисьма), БиблиотекаКартинок.ДиалогИнформация);
		
		Если ЕстьОшибочныеПолучатели Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Письмо отправлено не всем получателям.';
											|en = 'The email is not sent to some recipients.'"));
		Иначе
			Закрыть();
		КонецЕсли;
	КонецЕсли;
...
Функция ПоляЗаполненыКорректно()
	...
	Если ПочтовыеАдресаПолучателей.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Заполните получателя письма';
				|en = 'Enter the email recipient'"), , "ПочтовыеАдресаПолучателей");
		Результат = Ложь;
	КонецЕсли;
	Для каждого ПолучательПочты Из ПочтовыеАдресаПолучателей Цикл
		...
		Если ПустаяСтрока(Адрес) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Заполните получателя письма';
					|en = 'Enter the email recipient'"),, "ПочтовыеАдресаПолучателей[" + Формат(ПочтовыеАдресаПолучателей.Индекс(ПолучательПочты), "ЧГ=0") + "].Представление");
			Результат = Ложь;
		ИначеЕсли Не ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(Адрес, Ложь) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Неверный адрес электронной почты';
					|en = 'Invalid email address.'"),, "ПочтовыеАдресаПолучателей[" + Формат(ПочтовыеАдресаПолучателей.Индекс(ПолучательПочты), "ЧГ=0") + "].Представление");
			Результат = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьТабличныйДокументПоДвоичнымДанным(Знач ДвоичныеДанные)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Получение табличного документа';
										|en = 'Get spreadsheet document'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , 
...
Функция ПолучитьПриведенныйПочтовыйАдресВФормате(Текст)
	...
	Если Адреса.Количество() > 1 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Можно указывать только один адрес для ответа.';
				|en = 'Please specify a single reply-to address.'"), , "АдресОтвета");
		Возврат Текст;
	КонецЕсли;
