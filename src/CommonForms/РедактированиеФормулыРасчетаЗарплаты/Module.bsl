...
Процедура ПроверитьФормулу(Команда)
	...
	Если Результаты <> Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Проверка формулы завершена успешно.';
										|en = 'Formula is successfully checked.'"));
	КонецЕсли;
...
Функция РезультатыРедактирования(Формула, ВидРасчета, НедоступныеПоказатели, ДополнятьРезультаты = Истина)
	...
	Иначе
		...
		Если ДополнятьРезультаты Тогда
			...
			Результаты.Вставить("ЕстьПоказательСтоимостьДняЧасаСУчетомРазовыхНачислений", ПоказателиФормулы.Найти(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.СтоимостьДняЧасаСУчетомРазовыхНачислений")) <> Неопределено);
		КонецЕсли;
		Возврат Результаты;
	КонецЕсли;
