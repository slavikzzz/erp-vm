...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИсточникОтчета = Новый Массив;
	ИсточникОтчета.Добавить(СохраненныйДок.ИсточникОтчета);
	ИсточникОтчета.Добавить(?(СтрНачинаетсяС(СохраненныйДок.ИсточникОтчета, "EF_"), Сред(СохраненныйДок.ИсточникОтчета, 4), "EF_" + СохраненныйДок.ИсточникОтчета));
...
Процедура ПриОткрытии(Отказ)
	...
	Если СписокПустой = Истина Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Нет доступных отчетов для сравнения в соответствующем периоде.';
										|en = 'Нет доступных отчетов для сравнения в соответствующем периоде.'"));
	КонецЕсли;
