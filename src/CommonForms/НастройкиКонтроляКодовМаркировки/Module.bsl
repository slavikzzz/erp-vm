...
Процедура ПриОткрытии(Отказ)
	...
	Если ВладелецФормы = Неопределено Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Открытие формы без владельца не предусмотренно';
														|en = 'Открытие формы без владельца не предусмотренно'"));
		Возврат;
	КонецЕсли;
	...
	Если ВариантОтображения = ВариантПоВидамПродукции() Тогда
		...
		Для Каждого ЭлементВидОперации Из ДопустимыеВидыОпераций Цикл
			...
			ИначеЕсли ЭлементВидОперации.Значение = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.ПеремещениеМеждуМОД")
				И ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ВидыПродукцииВнутреннегоПеремещенияМеждуМОД().Найти(ТекущиеДанные.Настройка) = Неопределено Тогда
				Продолжить;
	...
	Иначе
		...
		Для Каждого ЭлементВидПродукции Из УчитываемыеВидыМаркируемойПродукции Цикл
			...
			ИначеЕсли ТекущиеДанные.Настройка = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.ПеремещениеМеждуМОД")
				И ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ВидыПродукцииВнутреннегоПеремещенияМеждуМОД().Найти(ВидПродукции) = Неопределено Тогда
				Продолжить;
...
Процедура НастроитьФорму()
	...
	Если ГруппаНастроек.РежимИсключения Тогда
		Заголовок = СтрШаблон(НСтр("ru = '%1 (исключения)';
									|en = '%1 (исключения)'"), ЗначениеСинонима);
	Иначе
	...
	Если Вариант = ВариантПоВидамПродукции() Тогда
		...
		Если Исключения Тогда
			Возврат НСтр("ru = '<Все операции, кроме розничной продажи>';
						|en = '<Все операции, кроме розничной продажи>'");
		Иначе
	...
	Иначе
		...
		Если Исключения Тогда
			Возврат НСтр("ru = '<Все товарные группы, кроме требующих обязательной проверки>';
						|en = '<Все товарные группы, кроме требующих обязательной проверки>'");
		Иначе
			Возврат НСтр("ru = '<Все товарные группы>';
						|en = '<Все товарные группы>'");
		КонецЕсли;
