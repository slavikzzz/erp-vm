...
Функция ЗапуститьЗавершениеСеансовНаСервере(Знач НомераСеансов, Знач Пароль, Знач СообщениеОтПользователя)
	...
	Если ЗначениеЗаполнено(СообщениеОтПользователя) Тогда
		ЧастиТекстаСообщенияПользователям.Добавить(СтрШаблон(НСтр("ru = 'Сообщение от пользователя: %1';
																	|en = 'Message from a user: %1'"), СообщениеОтПользователя));
	КонецЕсли;	
...
Процедура ПослеЗавершенияСеансов(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ЗаписатьОшибку(
			НСтр("ru = 'Ошибка завершения сеанса';
				|en = 'An error occurred when closing the session'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
			ПредставлениеОшибки);
...
Процедура ЗаписатьОшибку(ИмяСобытия, ТекстОшибки)
	...
	ПолноеИмяСобытия = НСтр("ru = 'ЗавершениеСеансовВМоделиСервиса';
							|en = 'SessionsTerminationSaaS'", ОбщегоНазначения.КодОсновногоЯзыка()) + "." + ИмяСобытия;
