...
Процедура ОтправитьЗаявление(Команда)
	...
	СписокКоманд.Добавить(КомандаОтправитьЗаявление(), НСтр("ru = 'Отправить заявление';
															|en = 'Send application.'"), Истина);
	...
	ОткрытьФорму("ОбщаяФорма.ПодтверждениеСменыТелефона", ПараметрыФормы, ЭтотОбъект, , , , ОповещениеСледующее);
...
Процедура Подключаемый_ОбработчикОбратногоОтсчета()
	...
	Если Таймер >= 0 Тогда
		НадписьОбратногоОтсчета = СтрШаблон(НСтр("ru = 'Запросить код повторно можно будет через %1 сек.';
												|en = 'You can request the code again in %1 sec.'"), Таймер);
		ПодключитьОбработчикОжидания("Подключаемый_ОбработчикОбратногоОтсчета", 1, Истина);		
	Иначе
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.Информация.Заголовок = СтрШаблон(НСтр("ru = 'Заявление принято к рассмотрению.
                                                    ...
                                                    |We''ll send you a text with the status to %1'"), Форма.НовыйТелефон);
