...
Процедура СохранитьПротоколВФайл(Команда)
	...
	ИмяФайла = СтрШаблон(
		НСтр("ru = 'Лог запросов из протокола по документу %1.txt';
			|en = 'Лог запросов из протокола по документу %1.txt'"),
		Документ);
	...
	СохраняемыйДокумент.ДобавитьСтроку(
		СтрШаблон(
			НСтр("ru = 'Лог запросов из протокола по документу: %1';
				|en = 'Лог запросов из протокола по документу: %1'"),
			Документ));
	...
	Попытка
		Для Каждого ЭлементДанных Из ПротоколОбмена Цикл
			...
			Если Лев(НРег(СокрЛП(ЭлементДанных.ЗапросТело)), 5) = "<?xml" Тогда
				...
				КонецЕсли;
			Иначе
				ОбъектJSON = ОбменДаннымиИСМПКлиентСервер.ТекстJSONВОбъект(ЭлементДанных.ЗапросТело, Истина);
				Если ОбъектJSON <> Неопределено Тогда
					СтрокаТЧ.ЗапросТело = ОбменДаннымиИСМПКлиентСервер.ОбъектВТекстJSON(ОбъектJSON);
				Иначе
					СтрокаТЧ.ЗапросТело = ЭлементДанных.ЗапросТело;
				КонецЕсли;
			КонецЕсли;
			
			ОбъектJSON = ОбменДаннымиИСМПКлиентСервер.ТекстJSONВОбъект(ЭлементДанных.ОтветТело, Истина);
			Если ОбъектJSON <> Неопределено Тогда
				СтрокаТЧ.ОтветТело = ОбменДаннымиИСМПКлиентСервер.ОбъектВТекстJSON(ОбъектJSON);
			Иначе
