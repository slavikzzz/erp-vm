...
Функция ДанныеДляРасчетаСреднего()
	...
	ДанныеДляРасчета.Вставить("СреднийЗаработокСН", СреднийЗаработокСН);
	ДанныеДляРасчета.Вставить("СреднийЗаработокРК", СреднийЗаработокРК);
	ДанныеДляРасчета.Вставить("СреднийЗаработокБольничныйЗаСчетРаботодателя", СреднийЗаработокБольничныйЗаСчетРаботодателя);
	ДанныеДляРасчета.Вставить("СреднийЗаработокБольничныйЗаСчетРаботодателяСН", СреднийЗаработокБольничныйЗаСчетРаботодателяСН);
	ДанныеДляРасчета.Вставить("СреднийЗаработокБольничныйЗаСчетРаботодателяРК", СреднийЗаработокБольничныйЗаСчетРаботодателяРК);
...
Процедура РассчитатьСреднийЗаработок()
	...
	ПараметрыРасчета.ДанныеНачислений = РезультатРедактированияНачислений(ЭтотОбъект);
	ПараметрыРасчета.ДанныеСтрахователей = ДанныеСтрахователей(ЭтотОбъект);
	ПараметрыРасчета.ДанныеВремени = РезультатРедактированияВремени(ЭтотОбъект);
	ПараметрыРасчета.РасчетныеГоды = ГодыРасчета(ЭтотОбъект);
	ПараметрыРасчета.ПериодРасчетаСреднегоЗаработкаНачало = НачалоПериодаРасчета;
	ПараметрыРасчета.ПериодРасчетаСреднегоЗаработкаОкончание = ОкончаниеПериодаРасчета;
	
	Результат = УчетПособийСоциальногоСтрахования.СреднийЗаработокФСС(ПараметрыРасчета);
	СреднийЗаработокИтог   = Результат.Общий;
	СреднийЗаработокСН = Результат.СН;
	СреднийЗаработокРК = Результат.РК;
	
	Если РассчитыватьСреднийЗаработокДляБольничногоЗаСчетРаботодателя Тогда 
		ПараметрыРасчета.ДанныеНачислений = РезультатРедактированияНачислений(ЭтотОбъект, , Истина);
		Результат = УчетПособийСоциальногоСтрахования.СреднийЗаработокФСС(ПараметрыРасчета);
		СреднийЗаработокБольничныйЗаСчетРаботодателя   = Результат.Общий;
		СреднийЗаработокБольничныйЗаСчетРаботодателяСН = Результат.СН;
		СреднийЗаработокБольничныйЗаСчетРаботодателяРК = Результат.РК;
	КонецЕсли;
...
Процедура СоздатьСтроку(Идентификатор, ЗаголовокСтроки, Точность = 2, МаксимальноеЗначение = 0, Год = Неопределено)
	...
	Если СтрНайти(Идентификатор, "Начисления") > 0 Тогда
...
Процедура СоздатьСтроки(ДобавляемыеГоды = Неопределено)
	...
	Если ПорядокРасчета = Перечисления.ПорядокРасчетаСреднегоЗаработкаФСС.Постановление2010 Тогда
		...
		СоздатьСтроку("Начисления", НСтр("ru = 'Начисления';
										|en = 'Accruals'"));
		СоздатьСтроку("ОтработаноДнейКалендарных", НСтр("ru = 'Отработано дней (календ.)';
														|en = 'Days worked (calend.)'"), 0, 31);
	ИначеЕсли ПорядокРасчета = Перечисления.ПорядокРасчетаСреднегоЗаработкаФСС.Постановление2011 Тогда
		...
		Для Каждого ДобавляемыйГод Из ДобавляемыеГоды Цикл
			...
			Если ИспользоватьДниБолезниУходаЗаДетьми Тогда
				СоздатьСтроку("ДнейБолезниУходаЗаДетьми" + Формат(ДобавляемыйГод, "ЧГ="), НСтр("ru = 'Дней болезни, ухода за детьми';
																								|en = 'Sick days, child care days'"), 0, 31, ДобавляемыйГод);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура УстановитьЗаголовокФормы()
	...
	Элементы.ДанныеУчетаСреднегоЗаработкаГруппа.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Данные %1';
			|en = 'Data %1'"), ЗарплатаКадры.ГоловнаяОрганизация(Организация));
...
Процедура УстановитьЗаголовокФлажкаПереноситьВДругойПорядокРасчета()
	...
	ЗаголовокФлажка = НСтр("ru = 'Применять эти данные и при расчете больничных, пособий по уходу за ребенком';
							|en = 'Apply this data while calculating sick leaves, child care allowances'")  + " ";
	Для Каждого ДругойПорядок Из ДругиеПорядкиРасчета Цикл
		...
		ИначеЕсли ДругойПорядок = Перечисления.ПорядокРасчетаСреднегоЗаработкаФСС.Постановление2011 Тогда
			ЗаголовокФлажка = ЗаголовокФлажка + НСтр("ru = 'по правилам, действующим с 1 января 2011 года,';
													|en = 'according to the rules valid from January 1, 2011,'")  + " ";
		КонецЕсли;
	КонецЦикла;
...
Процедура ДополнитьПредположительно()
	...
	Если МесяцыКорректировки.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Нет месяцев, за которые можно было бы дополнить таблицу по данным ФОТ.';
								|en = 'No months for which the table could be supplemented by salary budget data.'");
	...
	Если РезультатЗапроса.Пустой() Тогда
		ТекстСообщения = НСтр("ru = 'Не удалось дополнить таблицу: не обнаружено данных ФОТ.';
								|en = 'Cannot supplement the table: salary budget data is not found.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ВводПериодовОтсутствияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекстВопроса = НСтр("ru = 'Перезаполнить дни болезни, ухода за детьми с учетом введенных периодов отсутствия?';
						|en = 'Refill sick days, child care days considering entered absence periods?'");
