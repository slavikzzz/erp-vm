...
Процедура ОтразитьВКадровомУчете(Команда)
	...
	Если Модифицированность Тогда 
		...
		ТекстВопроса = НСтр("ru = 'Для продолжения необходимо записать позиции.';
							|en = 'Save positions to continue.'") + Символы.ПС + НСтр("ru = 'Продолжить?';
																										|en = 'Continue?'"); 
	...
	Для каждого СтрокаПозиции Из Позиции Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|Maybe, the positions are being edited by another user'"),
		...
		Исключение
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось сохранить изменения в позиции';
														|en = 'Cannot save changes to the position'") + " """ + ПозицияОбъект.Наименование + """",,,,Отказ);
		КонецПопытки;
...
Процедура ДанныеПоказателейВРеквизитФормы(СтрокиПозиций = Неопределено)
	...
	СведенияОПоказателях = ПлановыеНачисленияСотрудников.СведенияОВсехПоказателях();
	
	Для Каждого СтрокаПозиции Из КоллекцияПозиций Цикл
		УправлениеШтатнымРасписаниемФормы.ЗаполнитьНачисленияПоказателиСтрокиПозиции(ЭтаФорма, СтрокаПозиции, Начисления, Показатели, СведенияОПоказателях);
	КонецЦикла;
