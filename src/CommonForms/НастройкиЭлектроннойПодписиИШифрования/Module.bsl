...
Процедура ТехническаяИнформация(Команда)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.СформироватьТехническуюИнформацию(НСтр("ru = 'Настройки электронной подписи и шифрования';
																			|en = 'Digital signature and encryption settings'"));
...
Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклПродолжение(Менеджер, Контекст) Экспорт
	...
	Если Ошибка.НеУказанПуть Тогда
		...
		ОбновитьЗначение(ОписаниеПрограммы.Установлена, "");
	Иначе
		ТекстОшибки = НСтр("ru = 'Не установлена на компьютере.';
							|en = 'It is not installed on the computer.'") + " " + Ошибка.Описание;
	...
	Если Не Результат.ПроверкаВыполнена Тогда
		...
		Элементы.ДекорацияПроверкаУстановкиКриптопровайдера.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<a href = ""%1"">Нажмите здесь</a>, чтобы увидеть все приложения для работы с электронной подписью, установленные на компьютере.';
				|en = '<a href = ""%1"">Click here</a> to view all installed digital signing applications.'"),
			"ПроверитьУстановкуПрограммКриптографии");
	// Локализация
	ИначеЕсли Результат.ВозможенКонфликт Тогда
		...
		Элементы.ДекорацияПроверкаУстановкиКриптопровайдера.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = 'На вашем компьютере установлено несколько приложений с алгоритмами ГОСТ: %1.
				...
				|en = 'The following apps that use GOST algorithms are installed on your computer: %1.
				|They might cause conflicts and system malfunction. In this case, uninstall all of them, reboot, and install only one.'"),
		...
		Если Не ЕстьПроверяемыеПрограммы Тогда
			...
			Элементы.ДекорацияПроверкаУстановкиКриптопровайдера.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				НСтр("ru = 'Если планируется работа с усиленной квалифицированной электронной подписью, то на компьютере должно быть установлено сертифицированное приложение (средство криптографической защиты информации).
					|<a href = ""%1"">Установить.</a>';
					|en = 'If you expect to use advanced qualified digital signatures, install a certified data encryption tool.
					|<a href = ""%1"">Click to install</a>.'"), "ПроверитьУстановкуПрограммКриптографии");
		КонецЕсли;
		// Конец Локализация
	КонецЕсли;
...
Процедура УстановитьПодключитьСообщитьВерсиюКомпоненты(ПредложитьЗагрузить)
	...
	ПараметрыПодключения.ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Требуется установка компоненты %1';
...
Процедура СообщитьВерсиюКомпонентыПослеПодключения(Результат, Параметры, ДополнительныеПараметры) Экспорт
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Установлена компонента %1 версии %2.';
			|en = 'Add-in %1 version is %2.'"), "ExtraCryptoAPI", Результат);
