...
Процедура УстановитьЗаголовокКомандеГод(Форма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"КомандаГод",
		"Заголовок",
		Формат(Форма.ВыбираемыйПериод, НСтр("ru = 'ДФ=''гггг''';
											|en = 'DF=''yyyy'''")));
...
Процедура УстановитьНевыбранныйЦветКнопок()
	...
	Для НомерПоПорядку = 1 По ЧислоКнопок Цикл
		Если РежимВыбораПериода = "ГОД" Тогда
			ЭлементКнопка = Элементы[ПрефиксКоманды]; // КнопкаФормы - 
		Иначе
...
Процедура ОбновитьОтображение()
	...
	Если РежимВыбораПериода = "МЕСЯЦ" Тогда
		...
		КнопкаМесяца = Элементы["КомандаМесяц" + Формат(Месяц(ВыбираемыйПериод), "ЧЦ=2; ЧВН=")]; // КнопкаФормы - 
		...
		ПериодСтрокой = Формат(ВыбираемыйПериод, НСтр("ru = 'ДФ=''ММММ гггг''';
														|en = 'DF=''MMMM yyyy'''"))
		
	ИначеЕсли РежимВыбораПериода = "КВАРТАЛ" Тогда
		
		КварталМесяца = (Месяц(ВыбираемыйПериод) - 1) / 3 + 1;
		КнопкаКвартала = Элементы["КомандаКвартал" + Формат(КварталМесяца, "ЧЦ=1")]; // КнопкаФормы - 
		...
		ПериодСтрокой = Формат(КварталМесяца, "ЧЦ=1") + " " + НСтр("ru = 'квартал';
																	|en = 'quarter'") + " " + Формат(ВыбираемыйПериод,"ДФ=гггг");
		
	ИначеЕсли РежимВыбораПериода = "ПОЛУГОДИЕ" Тогда
		
		Если Месяц(ВыбираемыйПериод) = 1 Тогда
			Элементы.КомандаПолугодие1.ЦветФона = ЦветФонаКнопкиВыбранногоПериода;
			ПериодСтрокой = НСтр("ru = '1 полугодие';
								|en = 'Half-year I'") + " " + Формат(ВыбираемыйПериод,"ДФ=гггг");
		Иначе
			Элементы.КомандаПолугодие2.ЦветФона = ЦветФонаКнопкиВыбранногоПериода;
			ПериодСтрокой = НСтр("ru = '2 полугодие';
								|en = 'Half-year II'") + " " + Формат(ВыбираемыйПериод,"ДФ=гггг");
		КонецЕсли;
		
	Иначе
		Элементы.КомандаГод.ЦветФона = ЦветФонаКнопкиВыбранногоПериода;
		ПериодСтрокой = Формат(ВыбираемыйПериод, НСтр("ru = 'ДФ=''гггг''';
														|en = 'DF=''yyyy'''"))
	КонецЕсли;
