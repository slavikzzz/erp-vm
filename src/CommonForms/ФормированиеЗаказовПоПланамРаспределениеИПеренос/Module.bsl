...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Периодичность = Перечисления.Периодичность.Полугодие Тогда
		Элементы.ВариантПереноса.СписокВыбора[0].Представление = НСтр("ru = 'На день полугодия';
																		|en = 'On the day of the half-year'");
	...
	ИначеЕсли Периодичность = Перечисления.Периодичность.Декада Тогда
		Элементы.ВариантПереноса.СписокВыбора[0].Представление = НСтр("ru = 'На день в декаде';
																		|en = 'On the day in ten-day period'");
...
Процедура ОбновитьПодсказкуПереноса()
	...
	Если ТипПланирования = "ПланированиеПроизводстваСборки" Тогда
		ШаблонПодсказки = НСтр("ru = 'Например, дата выпуска %ТекущаяДатаПоступления% будет перенесена на %НоваяДатаПоступления%.';
								|en = 'For example, release date on %ТекущаяДатаПоступления% will be changed to %НоваяДатаПоступления%.'");
	ИначеЕсли ТипПланирования = "ПланированиеПроизводстваРазборки" Тогда
		ШаблонПодсказки = НСтр("ru = 'Например, дата запуска %ТекущаяДатаПоступления% будет перенесена на %НоваяДатаПоступления%.';
								|en = 'For example, start date %ТекущаяДатаПоступления% will be changed to %НоваяДатаПоступления%.'");
	ИначеЕсли ТипПланирования = "ПланированиеСборкиРазборки" Тогда
		ШаблонПодсказки = НСтр("ru = 'Например, дата сборки (разборки) %ТекущаяДатаПоступления% будет перенесена на %НоваяДатаПоступления%.';
								|en = 'For example, kitting date on %ТекущаяДатаПоступления% will be changed to %НоваяДатаПоступления%.'");
	ИначеЕсли ТипПланирования = "ПланированиеПередачиМатериаловВПроизводство" Тогда
		ШаблонПодсказки = НСтр("ru = 'Например, дата отгрузки материалов %ТекущаяДатаПоступления% будет перенесена на %НоваяДатаПоступления%.';
								|en = 'For example, material shipment date on %ТекущаяДатаПоступления% will be changed to %НоваяДатаПоступления%.'");
	Иначе
		ШаблонПодсказки = НСтр("ru = 'Например, дата поступления %ТекущаяДатаПоступления% будет перенесена на %НоваяДатаПоступления%.';
								|en = 'For example, receipt date on %ТекущаяДатаПоступления% will be changed to %НоваяДатаПоступления%.'");
	КонецЕсли;
	...
	Если ВариантРаспределенияДат = "ПоНеделям" Тогда
		...
		Если ТипПланирования = "ПланированиеПроизводстваСборки" Тогда
			ШаблонПодсказки = НСтр("ru = 'Например, выпуск %ТекущаяДатаПоступления% распределен на %КоличествоПериодов% периода(ов), в зависимости от конкретного месяца. Выпуск будет в %ДеньНедели%.';
									|en = 'For example, release on %ТекущаяДатаПоступления% is allocated to %КоличествоПериодов% periods depending on a month. The release will be on %ДеньНедели%.'");
		ИначеЕсли ТипПланирования = "ПланированиеПроизводстваРазборки" Тогда
			ШаблонПодсказки = НСтр("ru = 'Например, запуск %ТекущаяДатаПоступления% распределен на %КоличествоПериодов% периода(ов), в зависимости от конкретного месяца. Запуск будет в %ДеньНедели%.';
									|en = 'For example, launch %ТекущаяДатаПоступления% is allocated to %КоличествоПериодов% period(s) depending on a specific month. Launch will be on %ДеньНедели%.'");
		ИначеЕсли ТипПланирования = "ПланированиеСборкиРазборки" Тогда
			ШаблонПодсказки = НСтр("ru = 'Например, сборка(разборка) %ТекущаяДатаПоступления% распределена на %КоличествоПериодов% периода(ов), в зависимости от конкретного месяца. Сборка (разборка) будет в %ДеньНедели%.';
									|en = 'For example, kitting on %ТекущаяДатаПоступления% is allocated to %КоличествоПериодов% periods depending on a month. Kitting will be on %ДеньНедели%.'");
		ИначеЕсли ТипПланирования = "ПланированиеПередачиМатериаловВПроизводство" Тогда
			ШаблонПодсказки = НСтр("ru = 'Например, отгрузка материалов %ТекущаяДатаПоступления% распределена на %КоличествоПериодов% периода(ов), в зависимости от конкретного месяца. Сборка (разборка) будет в %ДеньНедели%.';
									|en = 'For example, material shipment on %ТекущаяДатаПоступления% is allocated to %КоличествоПериодов% periods depending on a month. Kitting will be on %ДеньНедели%.'");
		Иначе
			ШаблонПодсказки = НСтр("ru = 'Например, поступление %ТекущаяДатаПоступления% распределено на %КоличествоПериодов% периода(ов), в зависимости от конкретного месяца. Поступления будут в %ДеньНедели%.';
									|en = 'For example, the receipt on %ТекущаяДатаПоступления% is allocated to %КоличествоПериодов% periods depending on a month. Receipts will be on %ДеньНедели%.'");
		КонецЕсли;
	...
	ИначеЕсли ВариантРаспределенияДат = "ПоДекадам" Тогда
		...
		Если ТипПланирования = "ПланированиеПроизводстваСборки" Тогда
			ШаблонПодсказки = НСтр("ru = 'Например, выпуск %ТекущаяДатаПоступления% распределен на 3 декады. Даты выпуска установлены на 1, 11 и 21 число месяца.';
									|en = 'For example, release on %ТекущаяДатаПоступления% is allocated to 3 ten-day periods. The release dates are set to the 1st, 11th and 21st days of the month.'");
		ИначеЕсли ТипПланирования = "ПланированиеПроизводстваРазборки" Тогда
			ШаблонПодсказки = НСтр("ru = 'Например, запуск %ТекущаяДатаПоступления% распределен на 3 декады. Даты запуска установлены на 1, 11 и 21 число месяца.';
									|en = 'For example, launch on %ТекущаяДатаПоступления% is allocated to 3 ten-day periods. Launch dates are set to the 1st, 11th, and 21st day of the month.'");
		ИначеЕсли ТипПланирования = "ПланированиеСборкиРазборки" Тогда
			ШаблонПодсказки = НСтр("ru = 'Например, сборка (разборка) %ТекущаяДатаПоступления% распределена на 3 декады. Даты сборок (разборок) установлены на 1, 11 и 21 число месяца.';
									|en = 'For example, kitting on %ТекущаяДатаПоступления% is allocated to 3 ten-day periods. Kitting dates are set to the 1st, 11th and 21st days of the month.'");
		ИначеЕсли ТипПланирования = "ПланированиеПередачиМатериаловВПроизводство" Тогда
			ШаблонПодсказки = НСтр("ru = 'Например, отгрузка материалов %ТекущаяДатаПоступления% распределена на 3 декады. Даты сборок (разборок) установлены на 1, 11 и 21 число месяца.';
									|en = 'For example, material shipment on %ТекущаяДатаПоступления% is allocated to 3 ten-day periods. Kitting dates are set to the 1st, 11th and 21st days of the month.'");
		Иначе
			ШаблонПодсказки = НСтр("ru = 'Например, поступление %ТекущаяДатаПоступления% распределено на 3 декады. Даты поступления установлены на 1, 11 и 21 число месяца.';
									|en = 'For example, the receipt on %ТекущаяДатаПоступления% is allocated to 3 ten-day periods. The receipt dates are set to the 1st, 11th and 21st days of the month.'");
		КонецЕсли;
	...
	ИначеЕсли ВариантРаспределенияДат = "ПоПериодам" Тогда
		...
		Если ТипПланирования = "ПланированиеПроизводстваСборки" Тогда
			ШаблонПодсказки = НСтр("ru = 'Например, выпуск %ТекущаяДатаПоступления% распределен на %КоличествоПериодов% периода(ов). Даты выпуска установлены первый день каждого периода.';
									|en = 'For example, release on %ТекущаяДатаПоступления% is allocated to %КоличествоПериодов% periods. The release dates are set to the first day of each period.'");
		ИначеЕсли ТипПланирования = "ПланированиеПроизводстваРазборки" Тогда
			ШаблонПодсказки = НСтр("ru = 'Например, запуск %ТекущаяДатаПоступления% распределен на %КоличествоПериодов% периода(ов). Даты запуска установлены первый день каждого периода.';
									|en = 'For example, launch %ТекущаяДатаПоступления% is allocated to %КоличествоПериодов% period(s). Launch dates are set to the first day of each period.'");
		ИначеЕсли ТипПланирования = "ПланированиеСборкиРазборки" Тогда
			ШаблонПодсказки = НСтр("ru = 'Например, сборка (разборка) %ТекущаяДатаПоступления% распределена на %КоличествоПериодов% периода(ов). Даты сборок (разборок) установлены первый день каждого периода.';
									|en = 'For example, kitting on %ТекущаяДатаПоступления% is allocated to %КоличествоПериодов% periods. Kitting dates are set to the first day of each period.'");
		ИначеЕсли ТипПланирования = "ПланированиеПередачиМатериаловВПроизводство" Тогда
			ШаблонПодсказки = НСтр("ru = 'Например, отгрузка материалов %ТекущаяДатаПоступления% распределена на %КоличествоПериодов% периода(ов). Даты сборок (разборок) установлены первый день каждого периода.';
									|en = 'For example, material shipment on %ТекущаяДатаПоступления% is allocated to %КоличествоПериодов% periods depending on a month. Kitting dates are set to the first day of each period.'");
		Иначе
			ШаблонПодсказки = НСтр("ru = 'Например, поступление %ТекущаяДатаПоступления% распределено на %КоличествоПериодов% периода(ов). Даты поступления установлены первый день каждого периода.';
									|en = 'For example, the receipt on %ТекущаяДатаПоступления% is allocated to %КоличествоПериодов% periods. The receipt dates are set to the first day of each period.'");
		КонецЕсли;
		СтрокаКоличествоПериодов = Формат(КоличествоПериодов, "ЧДЦ=; ЧГ=0") ;
		ШаблонПодсказки = СтрЗаменить(ШаблонПодсказки, "%КоличествоПериодов%", СтрокаКоличествоПериодов);
