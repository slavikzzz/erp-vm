...
Процедура ЗагрузитьИзФайла(Команда)
	...
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("ru = 'Офисный документ';
...
Процедура ПосмотретьПечатнуюФормуПродолжение(Результат, Контекст) Экспорт
	...
	Если АдресОбразца <> Неопределено Тогда
		...
		ПараметрыОткрытияМакета.ИмяФайлаДляОткрытия = НСтр("ru = 'Образец';
															|en = 'Preview'")+" "+ СтрЗаменить(ТекущаяДата, ":", "_")+".docx";
		ОткрытьМакет(ПараметрыОткрытияМакета);
	КонецЕсли;
КонецПроцедуры
...
Процедура СохранитьВФайлПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	ПараметрыСохранения.Диалог.Фильтр = НСтр("ru = 'Офисный документ';
											|en = 'Office document'") + " (*.docx)|*.docx";
...
Процедура ПереключитьЯзыкПослеЗагрузкиФайлаПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Модифицированность Тогда
		ОписаниеЯзыка = ?(ЗначениеЗаполнено(ТекущийЯзык), " (" + ТекущийЯзык + ")", "");
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сохранить изменения в %1%2?';
																					|en = 'Do you want to save the changes to %1%2?'"), ИмяДокумента, ОписаниеЯзыка);
...
Процедура ПредупредитьОБлокировкеИОткрытьПриложение()
	...
	ПоказатьПредупреждение(ОписаниеОповещения, НСтр("ru = 'Завершите работу с файлом в другом приложении.';
													|en = 'Complete the operation with the file in another application.'"), , НСтр("ru = 'Файл открыт в другом приложении';
