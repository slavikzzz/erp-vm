...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Статус) Тогда
		Элементы.ДекорацияСтатус.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Статус на %1: %2';
					|en = 'Status as of %1: %2'"), ДатаПроверкиПодписи, Статус);
	Иначе
		Элементы.ДекорацияСтатус.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Статус на %1: %2';
					|en = 'Status as of %1: %2'"), ДатаПроверкиПодписи, РезультатПроверкиКраткий);
	КонецЕсли;
		...
		Для Каждого Сертификат Из СертификатыДляПроверкиПодписи Цикл
			...
			Если Сертификат.ЭтоСертификатПодписи Тогда
				Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Сертификат подписанта: %1';
						|en = 'Signer''s certificate: %1'"), Сертификат.КомуВыдан);
			Иначе
				Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Сертификат для проверки подписи: %1';
						|en = 'Certificate for signature verification: %1'"), Сертификат.КомуВыдан);
			КонецЕсли;
