	...
	Элементы.НадписьСообщенияВТехПоддержку.Заголовок =
		СтроковыеФункции.ФорматированнаяСтрока(
			ИнтернетПоддержкаПользователей.ПодставитьДомен(
				НСтр("ru = 'При возникновении проблем отправьте письмо по адресу <a href=""SendMail"">webits-info@1c.ru</a>';
					|en = 'If you encounter any issues, send an email to <a href=""SendMail"">webits-info@1c.ru</a>'"),
...
Процедура НадписьСообщенияВТехПоддержкуОбработкаНавигационнойСсылки(
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "SendMail" Тогда
		...
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СообщенияВСлужбуТехническойПоддержки") Тогда
			...
			ДанныеСообщения.Сообщение  = Тело;
...
Процедура УстановитьНастройкуЗапускатьПриСтарте(ЗначениеНастройки)
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
		МодульМониторПортала1СИТС.ИдентификаторОбщихНастроек(),
		МодульМониторПортала1СИТС.КлючНастройкиВсегдаПоказыватьПриСтартеПрограммы(),
		ЗначениеНастройки);
