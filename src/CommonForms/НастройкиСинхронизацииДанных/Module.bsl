	...
	ИначеЕсли ОписаниеСтрокиПриложения.ЗапускОбменаИзКорреспондента
		...
		ТексПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Запуск синхронизации с ""%1"" из этой программы не поддерживается.
				 ...
				 |Please open %1 and start the synchronization from there.'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
...
Процедура ВыполнитьКомандуСПредварительнойПроверкой(КомандаСтрокой)
	...
	Если КомандаСтрокой <> "УдалитьНастройкуСинхронизации"
		...
		ТекстСообщения = НСтр("ru = 'Синхронизация недоступна для использования';
								|en = 'Synchronization is unavailable to use'");
...
Процедура ЗавершитьНастройкуСинхронизацииВДиалоге(ОписаниеСтрокиПриложения, ДанныеДляЗавершенияНастройки)
	...
	Если ОписаниеСтрокиПриложения.ЭтоНачальнаяВыгрузка Тогда
		...
		ТекстЧасть1 = НСтр("ru = 'Перед выгрузкой данных для сопоставления завершите настройку синхронизации.';
							|en = 'Before exporting data for mapping, finish the synchronization setup.'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		
	Иначе
		
		ТекстЧасть1 = НСтр("ru = 'Перед запуском синхронизации данных завершите ее настройку.';
							|en = 'Before starting data synchronization, finish the synchronization setup.'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		
	КонецЕсли;
	
	ТекстВопроса = ТекстЧасть1 + Символы.ПС + НСтр("ru = 'Открыть форму помощника настройки?';
													|en = 'Open the setup wizard form?'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура ДиалогПользователяСНесовместимымиПравилами(ОписаниеСтрокиПриложения, ОписаниеОшибки)
	...
	Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
		...
		ПараметрыФормы.Заголовок = НСтр("ru = 'Синхронизация данных не может быть выполнена';
										|en = 'Cannot perform data synchronization'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		
	Иначе
		
		ПараметрыФормы.Заголовок = НСтр("ru = 'Синхронизация данных может быть выполнена некорректно';
										|en = 'Data synchronization might be performed incorrectly'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура СкрытьПредупреждениеЗацикливанияОтПользователя()
	...
	ТекстПредупреждения = НСтр("ru = 'Функциональность контроля циклически настроенных синхронизаций расположена в каждой форме настроек синхронизации в меню Еще (см. команду ""Предупреждение цикличности настроек обменов"")';
								|en = 'Функциональность контроля циклически настроенных синхронизаций расположена в каждой форме настроек синхронизации в меню Еще (см. команду ""Предупреждение цикличности настроек обменов"")'",
		ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		
	ТекстЗаголовка = НСтр("ru = 'Скрытие предупреждения о цикличности настроек';
							|en = 'Скрытие предупреждения о цикличности настроек'",
		ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура УстановитьОтображениеЭлементовФормы()
	...
	Если ОтключенныеСценарии.Количество() = 1 Тогда
		...
		Шаблон = НСтр("ru = 'Сценарий <a href = %1>""%2""</a> был отключен из-за ошибок синхронизации. Включите сценарий после устранения проблем.';
						|en = 'The <a href = %1>""%2""</a> scenario was disabled due to synchronization errors. Enable the scenario after troubleshooting.'");
	...
	ИначеЕсли ОтключенныеСценарии.Количество() > 1 Тогда
		...
		Шаблон = НСтр("ru = 'Обнаружены <a href = ""%1"">сценарии</a>, которые были отключены из-за ошибок выполнения. Включите сценарий после устранения проблем.';
						|en = '<a href = ""%1"">Some scenarios</a> have been disabled due to runtime errors. Enable scenarios after you fix the issues.'");
