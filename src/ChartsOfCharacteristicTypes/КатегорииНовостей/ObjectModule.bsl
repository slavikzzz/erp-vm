...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// Это необходимо для того, чтобы никакая бизнес-логика объекта не выполнялась
	//  при записи объекта через механизм обмена данными, поскольку она уже была выполнена
	//  для объекта в том узле, где он был создан.
	...
	Если ТипЗначения.Типы().Количество() > 1 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Составные типы не поддерживаются. Выберите только один тип значения.';
								|en = 'Composite types are not supported. Select only one value type.'");
		Сообщение.Сообщить();
	КонецЕсли;
	...
	Если ТипЗначения.Типы().Количество() = 0 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Пустые типы не поддерживаются. Выберите один любой тип значения.';
								|en = 'Empty types are not supported. Select a value type.'");
		Сообщение.Сообщить();
	КонецЕсли;
	...
	Если ЗагруженоССервера = Истина Тогда
		...
		Если (ПометкаУдаления = Истина)
			...
			ТекстСообщения = НСтр("ru = 'Нельзя помечать на удаление категории, загруженные автоматически с сервера новостей.';
									|en = 'Cannot mark categories for deletion that were downloaded from the news server automatically.'");
			Сообщение = Новый СообщениеПользователю();
