...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "БуферОбмена: изменился состав сохраненных" Тогда
		...
		Если ИсточникЭталон.КлючОбъекта = ЭтотОбъект.КлючОбъекта Тогда
			ОбновитьСписокНастроекСервер();
		КонецЕсли;
	КонецЕсли;
				...
				Если НЕ ПустаяСтрока(ЭтотОбъект.КлючНастройки) Тогда
					Если ЭтотОбъект.СписокКлючейНастроек.НайтиПоЗначению(ЭтотОбъект.КлючНастройки) = Неопределено Тогда
						...
						ПоказатьВопрос(ОписаниеОповещенияОЗакрытии, ТекстВопроса, СписокКнопок, 0, "ЗаписатьИЗакрыть", НСтр("ru = 'Текущее значение буфера обмена не сохранено';
																															|en = 'The current clipboard value is not saved'"));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура КомандаВыгрузитьСписокБуферовОбмена(Команда)
	...
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение); // АПК:1348 Форма не вызывается в веб-клиенте.
	Диалог.Заголовок = НСтр("ru = 'Выберите файл, куда записать список буферов обмена';
							|en = 'Select a file to save the clipboard list to'");
	Диалог.МножественныйВыбор = Ложь;
	Диалог.Расширение = "xml";
	Диалог.Фильтр = "Файл xml (*.xml)|*.xml";

	ДополнительныеПараметры = Новый Структура;
	ОписаниеОповещенияПослеВыбора = Новый ОписаниеОповещения("ПослеВыгрузкиСпискаБуферовОбмена", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура КомандаЗагрузитьСписокБуферовОбмена(Команда)
	...
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие); // АПК:1348 Форма не вызывается в веб-клиенте.
	Диалог.Заголовок = НСтр("ru = 'Выберите файл с сохраненными ранее буферами обмена';
							|en = 'Select the file with previously saved clipboards'");
	Диалог.МножественныйВыбор = Ложь;
	Диалог.Расширение = "xml";
	...
	ДополнительныеПараметры = Новый Структура;
	ОписаниеОповещенияПослеВыбора = Новый ОписаниеОповещения("ПослеЗагрузкиСпискаБуферовОбмена", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура КомандаУдалитьНаСервере(Знач ЛокальныйКлючОбъекта, Знач ЛокальныйКлючНастройки, Знач ЛокальноеИмяПользователя)

	ХранилищаНастроек.БуферыОбменаНовостей.Удалить(
		ЛокальныйКлючОбъекта,
		ЛокальныйКлючНастройки,
		ЛокальноеИмяПользователя);
	...
	Если ЭтотОбъект.ТекущийБуферОбмена.Количество() <= 0 Тогда
		ЭтотОбъект.ТекущийБуферОбмена_Представление =
			НСтр("ru = 'Пустой буфер обмена.
				...
				|en = 'Empty clipboard.
				|Cannot save an empty clipboard.
				|At first copy data lines to the clipboard.'");
...
&НаСервере
Процедура КомандаВыгрузитьСписокБуферовОбменаНаСервере(Знач ИмяФайла)
		...
		ЗаписьХМЛ.ЗаписатьКомментарий(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Выгрузка данных буферов обмена для %1, %2';
					|en = 'Clipboard data export for %1, %2'"),
				ЭтотОбъект.КлючОбъекта,
...
&НаСервере
Процедура КомандаЗагрузитьСписокБуферовОбменаНаСервере(Знач ИмяФайла)
...
&НаКлиенте
Процедура ПослеВыгрузкиСпискаБуферовОбмена(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ТипЗнч(ВыбранныеФайлы) = ТипМассив)
		...
		КомандаВыгрузитьСписокБуферовОбменаНаСервере(ИмяФайла);
...
&НаКлиенте
Процедура ПослеЗагрузкиСпискаБуферовОбмена(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт

	ТипМассив = Тип("Массив");

	Если (ТипЗнч(ВыбранныеФайлы) = ТипМассив)
			И (ВыбранныеФайлы.Количество() > 0) Тогда
		КомандаЗагрузитьСписокБуферовОбменаНаСервере(ВыбранныеФайлы[0]);
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Данные загружены.';
				|en = 'Data is not imported yet.'"),
			,
			,
