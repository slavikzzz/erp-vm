...
&НаКлиенте
Перем КартинкиСпособовОповещений;

&НаКлиенте
Перем СпособыОповещений;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Поэтому используется чтение напрямую, а не ОбработкаНовостейСлужебный.ПолучитьНастройкиПоказаНовостейНастроенныеАдминистратором.
	...
	Если ТипЗнч(НастройкиПоказаНовостейНастроенныеАдминистратором) = ТипСтруктура Тогда
		НастройкиПоказаНовостейНастроенныеАдминистраторомЭталон = Новый Структура;
			НастройкиПоказаНовостейНастроенныеАдминистраторомЭталон.Вставить("СписокНовостей_ИнтервалАвтообновления", 360);
		ЗаполнитьЗначенияСвойств(НастройкиПоказаНовостейНастроенныеАдминистраторомЭталон, НастройкиПоказаНовостейНастроенныеАдминистратором);
		ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления = НастройкиПоказаНовостейНастроенныеАдминистраторомЭталон.СписокНовостей_ИнтервалАвтообновления;
		Если (ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления < 1)
				ИЛИ (ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления > 999999) Тогда
			ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления = 360;
		КонецЕсли;
	КонецЕсли;
	Если ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления % (60*24*7) = 0 Тогда // 1 раз в неделю
		ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ru = 'Список новостей обновляется еженедельно';
																				|en = 'News list is updated weekly'");
	ИначеЕсли ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления % (60*24) = 0 Тогда // 1 раз в день
		ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ru = 'Список новостей обновляется ежедневно';
																				|en = 'News list is daily updated'");
	ИначеЕсли ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления % 60 = 0 Тогда // Кратно часу
		Если ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления = 60 Тогда
			ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ru = 'Список новостей обновляется ежечасно';
																					|en = 'News list is updated every hour'");
		Иначе
			ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ru = 'Список новостей обновляется каждые несколько часов';
																					|en = 'News list is updated every several hours'");
		КонецЕсли;
	Иначе
		ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления_Представление =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Список новостей обновляется каждые %1 минут';
					|en = 'News list is updated every %1 minutes'"),
				ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления);
	КонецЕсли;
	Элементы.СписокНовостей_ИнтервалАвтообновления_Представление.Подсказка = ""
		+ ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления + " " + НСтр("ru = 'минут';
																		|en = 'minutes'");
	...
	// Здесь возможно получение настроек для другого (не текущего) пользователя.
	ЛентыНовостейВсеНастройки = ХранилищаНастроек.НастройкиНовостей.Загрузить(
		"ЛентыНовостейВсеНастройки", // КлючОбъекта
		ЭтотОбъект.КлючНастроек, // КлючНастроек
		,
		ИмяПользователяИБ); // ИмяПользователя
	...
	Для каждого ТекущаяЛентаНовостей Из ТаблицаВидимостиЛентНовостей Цикл
		Если ТекущаяЛентаНовостей.Видимость = Истина Тогда
			...
			Для Каждого ТекущаяНастройка Из ЛентыНовостейВсеНастройки Цикл
				Если ТекущаяНастройка.ЛентаНовостей = ТекущаяЛентаНовостей.ЛентаНовостей Тогда
					НайденнаяНастройка = ТекущаяНастройка;
					Прервать;
				КонецЕсли;
			КонецЦикла;

			// Пометка - искать в Настройках.
			...
			Если (НайденнаяНастройка <> Неопределено) Тогда
				Пометка = (НайденнаяНастройка.ПодпискаПринудительно = "Подписан")
						ИЛИ (НайденнаяНастройка.ПодпискаСамостоятельно = "Подписан");
				// {При добавлении способов оповещения исправлять здесь}.
				Если НЕ Пометка Тогда
					СпособОповещения = Перечисления.СпособыОповещенияПользователяОНовостях.Отключено;
				Иначе
					СпособОповещения = НайденнаяНастройка.СпособОповещения;
				КонецЕсли;
			КонецЕсли;
			...
			НоваяСтрока = ЭтотОбъект.ЛентыНовостей.Добавить();
				...
				// НоваяСтрока.ТаблицаКатегорийДляНастройкиОтборов - Заполнится потом.
				НоваяСтрока.ОбязательныйКанал            = ТекущаяЛентаНовостей.ОбязательныйКанал;
				НоваяСтрока.СпособОповещения             = СпособОповещения;
				НоваяСтрока.СпособОповещенияИконка       = ПолучитьКартинкуСпособаОповещения(
					НоваяСтрока.СпособОповещения,
					НоваяСтрока.ОбязательныйКанал);
		КонецЕсли;
	КонецЦикла;
	ЭтотОбъект.ЛентыНовостей.Сортировать("ЛентаНовостей");

	// Дозаполнить способом оповещения.
	// Здесь возможно получение настроек для другого (не текущего) пользователя.
	ЛентыНовостейВсеНастройки = ХранилищаНастроек.НастройкиНовостей.Загрузить(
		"ЛентыНовостейВсеНастройки", // КлючОбъекта
	...
	Для Каждого ТекущаяЛентаНовостей Из ЭтотОбъект.ЛентыНовостей Цикл
		Для Каждого ТекущаяНастройкаЛентыНовостей Из ЛентыНовостейВсеНастройки Цикл
			Если ТекущаяЛентаНовостей.ЛентаНовостей = ТекущаяНастройкаЛентыНовостей.ЛентаНовостей Тогда
				Если НЕ ТекущаяЛентаНовостей.ОбязательныйКанал Тогда // Запрещено изменять способ оповещения для обязательного канала.
					ТекущаяЛентаНовостей.СпособОповещения = ТекущаяЛентаНовостей.СпособОповещения;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	...
	// {При добавлении способов оповещения исправлять здесь}.
	КартинкиСпособовОповещений = Новый Соответствие;
		КартинкиСпособовОповещений.Вставить(
			Истина,
			БиблиотекаКартинок.НовостиОповещенияВключеныВсегда);
		КартинкиСпособовОповещений.Вставить(
			Перечисления.СпособыОповещенияПользователяОНовостях.ПоУмолчанию,
			БиблиотекаКартинок.НовостиОповещенияВключены);
		КартинкиСпособовОповещений.Вставить(
			Перечисления.СпособыОповещенияПользователяОНовостях.СистемаВзаимодействия,
			БиблиотекаКартинок.НовостиОповещенияСистемаВзаимодействия);
		КартинкиСпособовОповещений.Вставить(
			Перечисления.СпособыОповещенияПользователяОНовостях.Отключено,
			БиблиотекаКартинок.НовостиОповещенияОтключены);

	СпособыОповещений = Новый Соответствие;
		СпособыОповещений.Вставить("ПустаяСсылка", Перечисления.СпособыОповещенияПользователяОНовостях.ПустаяСсылка());
		СпособыОповещений.Вставить("ПоУмолчанию" , Перечисления.СпособыОповещенияПользователяОНовостях.ПоУмолчанию);
		СпособыОповещений.Вставить("Отключено"   , Перечисления.СпособыОповещенияПользователяОНовостях.Отключено);

	Если Параметры.Страница = "СтраницаЛентыНовостей" Тогда
		Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаЛентыНовостей;
		Для Каждого ТекущаяСтрока Из ЛентыНовостей Цикл
			Если ТекущаяСтрока.ЛентаНовостей = Параметры.ЛентаНовостей Тогда
				Элементы.ЛентыНовостей.ТекущаяСтрока = ТекущаяСтрока.ПолучитьИдентификатор();
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	ДанныеДляПередачиНаКлиента = Новый Структура;
		ДанныеДляПередачиНаКлиента.Вставить("КартинкиСпособовОповещений"    , КартинкиСпособовОповещений);
		ДанныеДляПередачиНаКлиента.Вставить("СпособыОповещений"             , СпособыОповещений);
		ДанныеДляПередачиНаКлиента.Вставить("СпособыОповещений_ПустаяСсылка", Перечисления.СпособыОповещенияПользователяОНовостях.ПустаяСсылка());
...
Процедура ПриОткрытии(Отказ)
	...
	КартинкиСпособовОповещений     = ДанныеДляПередачиНаКлиента.КартинкиСпособовОповещений;
	СпособыОповещений              = ДанныеДляПередачиНаКлиента.СпособыОповещений;
	СпособыОповещений_ПустаяСсылка = ДанныеДляПередачиНаКлиента.СпособыОповещений_ПустаяСсылка;
	ДанныеДляПередачиНаКлиента = Неопределено;

	Элементы.ЛентыНовостейСпособОповещения.СписокВыбора.Очистить();
	Для Каждого ТекущийСпособОповещения Из СпособыОповещений Цикл
		Если ТекущийСпособОповещения.Значение <> СпособыОповещений_ПустаяСсылка Тогда
			Элементы.ЛентыНовостейСпособОповещения.СписокВыбора.Добавить(ТекущийСпособОповещения.Значение);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЛентыНовостейПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные.Пометка
			И ТекущиеДанные.КоличествоДоступныхОтборов > 0 Тогда
	...
	Если НЕ ТекущиеДанные.Пометка Тогда // Отключили ленту новостей = отключили по ней оповещения.
		ТекущиеДанные.СпособОповещения = СпособыОповещений.Получить("Отключено");
		ТекущиеДанные.СпособОповещенияИконка = ПолучитьКартинкуСпособаОповещенияНаКлиенте(
			ТекущиеДанные.СпособОповещения,
			ТекущиеДанные.ОбязательныйКанал);
	КонецЕсли;
...
&НаКлиенте
Процедура ЛентыНовостейСпособОповещенияПриИзменении(Элемент)
	...
	Если ТекущиеДанные.СпособОповещения.Пустая() Тогда
		ТекущиеДанные.СпособОповещения = СпособыОповещений.Получить("ПоУмолчанию");
	КонецЕсли;
...
Процедура КомандаОК(Команда)
	...
	Отказ = Ложь;
	ИнтеграцияПодсистемБИПКлиент.ПослеЗаписиПользовательскихНастроек(Результат, Отказ);
...
Процедура КомандаНастроитьОтборы(Команда)
	...
	Если ТипЗнч(ПараметрыФормы) = ТипСтруктура Тогда
		ПараметрыФормыЭталон = Новый Структура;
			ПараметрыФормыЭталон.Вставить("Пометка"                   , Ложь);
			ПараметрыФормыЭталон.Вставить("КоличествоДоступныхОтборов", 0);
		ЗаполнитьЗначенияСвойств(ПараметрыФормыЭталон, ПараметрыФормы);
		Если ПараметрыФормыЭталон.Пометка = Истина Тогда
			Если ПараметрыФормыЭталон.КоличествоДоступныхОтборов > 0 Тогда
		...
		Иначе
			ПоказатьПредупреждение(
				...
				НСтр("ru = 'Вначале включите получение новостей по этой ленте.';
					|en = 'Enable news receipt from this new feed first.'"),
				0,
...
Функция ПодготовитьСтруктуруДляВозвратаПоОКИСохранитьНастройки()
	...
	// - подписка (в отличие от "список отключенных лент новостей", здесь будет полный список лент новостей кроме обязательных,
	//     чтобы обновить список в основной форме);
	// - способы оповещений.
	...
	лкТаблицаОтборовПоЛентамНовостей = ХранилищаНастроек.НастройкиНовостей.ПровестиВалидациюНастроек(
		Неопределено, // Никакие настройки не передаются. Будет создана пустая таблица с колонками по-умолчанию.
		"ОтборыПоЛентамНовостейПользовательские",
		ЭтотОбъект.КлючНастроек); // КлючНастроек

	СпособыОповещений = Новый Соответствие;
	СостояниеПодписки = Новый Соответствие;
	...
	Для каждого ТекущаяЛентаНовостей Из ЭтотОбъект.ЛентыНовостей Цикл
		...
		Если НЕ ТекущаяЛентаНовостей.ОбязательныйКанал Тогда
			// Для обязательных каналов не имеет смысла запоминать состояние способа оповещения и подписки, т.к. он всегда "ПоУмолчанию".
			СостояниеПодписки.Вставить(ТекущаяЛентаНовостей.ЛентаНовостей, ТекущаяЛентаНовостей.Пометка);
			СпособыОповещений.Вставить(ТекущаяЛентаНовостей.ЛентаНовостей, ТекущаяЛентаНовостей.СпособОповещения);
			НастройкиОповещения = Новый Структура;
				НастройкиОповещения.Вставить("КодЛентыНовостей", ""); // Здесь это поле необязательно для заполнения.
				НастройкиОповещения.Вставить("ЛентаНовостей"   , ТекущаяЛентаНовостей.ЛентаНовостей);
				НастройкиОповещения.Вставить("СпособОповещения", ТекущаяЛентаНовостей.СпособОповещения);
	...
	Результат = Новый Структура;
		Результат.Вставить("ОтборыПоЛентамНовостейПользовательские", лкОтборыПоЛентамНовостейПользовательские);
		Результат.Вставить("ОтключенныеЛентыНовостей"              , МассивОтключенныхЛентНовостей);
		Результат.Вставить("НастройкиПоказаНовостей"               , СтруктураНастроек);
		Результат.Вставить("СпособыОповещений"                     , СпособыОповещений);
		Результат.Вставить("СостояниеПодписки"                     , СостояниеПодписки);
	...
	Если ЭтотОбъект.БылиИзмененияОтборов = Истина Тогда
		...
		ОбработкаНовостейСлужебный.ОптимизироватьОтборыПоНовостям(Неопределено);

		// Рассчитать заново регистр "РассчитанныеОтборыПоНовостям_Пользовательские" для отборов
		//  по новостям, настроенных пользователем.
		ОбработкаНовостейСлужебный.ПересчитатьОтборыПоНовостям_Пользовательские(ЭтотОбъект.ТекущийПользователь, Неопределено); // По текущему пользователю
	КонецЕсли;
...
Процедура ЗагрузитьПараметрыИзФормыНастройкиОтборовПоВыбраннойЛентеНовостей(Знач СтруктураНастроек)
	...
	СтруктураНастроекЭталон = Новый Структура;
		СтруктураНастроекЭталон.Вставить("ЛентаНовостей"                                       , Неопределено);
		СтруктураНастроекЭталон.Вставить("АдресХранилищаТаблицыЗначенийПользовательскихОтборов", Неопределено);
	...
	Если ТипЗнч(СтруктураНастроек) = ТипСтруктура Тогда
		Если СтруктураНастроекЭталон.АдресХранилищаТаблицыЗначенийПользовательскихОтборов <> Неопределено Тогда
			ТаблицаЗначенийОтборов = ПолучитьИзВременногоХранилища(СтруктураНастроекЭталон.АдресХранилищаТаблицыЗначенийПользовательскихОтборов);
		Иначе
		...
		НайденныеСтроки = ЭтотОбъект.ОтборыПоЛентамНовостейПользовательские.НайтиСтроки(
			Новый Структура("ЛентаНовостей", СтруктураНастроекЭталон.ЛентаНовостей));
		...
		Для каждого ТекущаяСтрока Из ТаблицаЗначенийОтборов Цикл
			...
			НоваяСтрока.ЛентаНовостей         = СтруктураНастроекЭталон.ЛентаНовостей;
		...
		Если (ТекущаяСтрока.ЛентаНовостей = лкЛентаНовостей) ИЛИ (лкЛентаНовостей = Неопределено) Тогда
			...
			Если (ТекущаяСтрока.КоличествоВсехОтборов = 0) Тогда
				...
				ПредставлениеВсехОтборовПодробно = НСтр("ru = 'По этой ленте отборы не настраиваются';
														|en = 'Filters are not set for this feed'");
			Иначе
				ПредставлениеВсехОтборовКратко               = "";
				...
				ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно
					...
					+ ?(ПустаяСтрока(ПредставлениеПользовательскихОтборов),
							НСтр("ru = 'Нет отборов, разрешенных для настройки пользователем';
								|en = 'No filters allowed for user setting'")
								+ Символы.ПС,
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Отборы, настроенные пользователем (можно изменить):
									|%1';
									|en = 'Filters set up by user (can be edited):
									|%1'")
									+ Символы.ПС,
								ПредставлениеПользовательскихОтборов));
					...
					+ ?(ПустаяСтрока(ПредставлениеАдминистративныхОтборов),
							...
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Отборы, настроенные администратором (нельзя изменить):
									|%1';
									|en = 'Filters set up by administrator (cannot be edited):
									|%1'")
									+ Символы.ПС,
								ПредставлениеАдминистративныхОтборов));
					...
					+ ?(ПустаяСтрока(ПредставлениеАвтоматическиЗаполняемыхОтборов),
							...
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Автоматически заполняемые отборы (нельзя изменить):
									|%1';
									|en = 'Automatically filled flters (cannot change):
									|%1'")
									+ Символы.ПС,
								ПредставлениеАвтоматическиЗаполняемыхОтборов));
				...
				ПредставлениеВсехОтборовПодробно = 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'По этой ленте нельзя отключить получение новостей (так настроено администратором)
							|%1';
							|en = 'For this feed, you cannot disable news receipt (the administrator configured it)
							|%1'")
							+ Символы.ПС,
						ПредставлениеВсехОтборовПодробно);
			КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	// 1. Только просмотр: Нельзя отключить новости или менять способ оповещения в ленте новостей, помеченной как "Обязательная".
		...
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
			ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ЛентыНовостейСпособОповещения.Имя);
	...
	// 4. Только просмотр: Нельзя изменить способ оповещения в отключенной ленте новостей.
		Элемент = УсловноеОформление.Элементы.Добавить();
...
// Возвращает картинку, соответствующую способу оповещения ленты новостей.
//
// Параметры:
//  СпособОповещения  - ПеречислениеСсылка.СпособыОповещенияПользователяОНовостях;
//  ОбязательныйКанал - Булево - признак обязательного канала.
//
// Возвращаемое значение:
//   Картинка - картинка, соответствующая способу оповещения.
//
&НаСервереБезКонтекста
Функция ПолучитьКартинкуСпособаОповещения(СпособОповещения, ОбязательныйКанал)

	Если ОбязательныйКанал = Истина Тогда
		Результат = БиблиотекаКартинок.НовостиОповещенияВключеныВсегда;
	Иначе
		Результат = БиблиотекаКартинок.НовостиОповещенияВключены;
		// {При добавлении способов оповещения исправлять здесь}.
		Если СпособОповещения = Перечисления.СпособыОповещенияПользователяОНовостях.Отключено Тогда
			Результат = БиблиотекаКартинок.НовостиОповещенияОтключены;
		ИначеЕсли СпособОповещения = Перечисления.СпособыОповещенияПользователяОНовостях.СистемаВзаимодействия Тогда
			Результат = БиблиотекаКартинок.НовостиОповещенияСистемаВзаимодействия;
		КонецЕсли;
	КонецЕсли;
...
//
&НаКлиенте
Функция ПолучитьКартинкуСпособаОповещенияНаКлиенте(СпособОповещения, ОбязательныйКанал)

	Если ОбязательныйКанал = Истина Тогда
		Результат = КартинкиСпособовОповещений.Получить(Истина);
	Иначе
		Результат = КартинкиСпособовОповещений.Получить(СпособОповещения);
	КонецЕсли;
