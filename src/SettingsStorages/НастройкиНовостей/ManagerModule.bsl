...
Процедура ОбработкаСохранения(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек, ИмяПользователяИБ)
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось записать константу НастройкиНовостей.
						...
						|Подробности:
						|%5';
						|en = 'Cannot save the НастройкиНовостей constant.
						|Writing might be in process in a background job or in another session.
						|КлючОбъекта: %1
						|Settings:
						|%2
						|Session number: %3
				...
				ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru = 'БИП:Новости.Хранилище настроек новостей';
						|en = 'ISL:News.News setting storage'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
					НСтр("ru = 'Новости. Хранилище настроек новостей. ОбработкаСохранения. Ошибка';
						|en = 'News. News setting storage. ОбработкаСохранения. Error'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
	...
	ИначеЕсли КлючОбъекта = "ОтборыПоЛентамНовостейНастроенныеАдминистратором" Тогда
		...
		Иначе
			ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
				...
				, // ОбъектМетаданных
				, // Данные
				НСтр("ru = 'Ошибка при проверке набора записей регистра ОтборыПоЛентамНовостейОбщие (ключ объекта ОтборыПоЛентамНовостейНастроенныеАдминистратором) - возможно, не заполнены какие-то поля.';
					|en = 'An error occurred while checking a record set of register ОтборыПоЛентамНовостейОбщие (object key ОтборыПоЛентамНовостейНастроенныеАдминистратором) - some fields may not be filled in.'"), // Комментарий
				ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		КонецЕсли;
		...
		Если ЗаписыватьДанные = Истина Тогда
			...
			Иначе
				ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
					...
					, // ОбъектМетаданных
					, // Данные
					НСтр("ru = 'Ошибка при проверке набора записей регистра ОтборыПоЛентамНовостейДляОбластиДанных (ключ объекта ОтборыПоЛентамНовостейДляОбластиДанных) - возможно, не заполнены какие-то поля.';
						|en = 'An error occurred while checking a record set of the ОтборыПоЛентамНовостейДляОбластиДанных register (object key ОтборыПоЛентамНовостейДляОбластиДанных). Some fields might have been left blank.'"), // Комментарий
					ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли КлючОбъекта = "ОтборыПоЛентамНовостейПользовательские" Тогда
		...
		Иначе
			ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
				...
				, // ОбъектМетаданных
				, // Данные
				НСтр("ru = 'Ошибка при проверке набора записей регистра ОтборыПоЛентамНовостейПользовательские (ключ объекта ОтборыПоЛентамНовостейПользовательские) - возможно, не заполнены какие-то поля.';
					|en = 'An error occurred while checking a record set of register ОтборыПоЛентамНовостейПользовательские (object key ОтборыПоЛентамНовостейПользовательские) - some fields may not be filled in.'"), // Комментарий
				ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		КонецЕсли;
	...
	ИначеЕсли КлючОбъекта = "ОтключенныеЛентыНовостей" Тогда
		...
		Иначе
			ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
				...
				, // ОбъектМетаданных
				, // Данные
				НСтр("ru = 'Ошибка при проверке набора записей регистра ОтключенныеЛентыНовостей (ключ объекта ОтключенныеЛентыНовостей) - возможно, не заполнены какие-то поля.';
					|en = 'An error occurred while checking a record set of register ОтключенныеЛентыНовостей (object key ОтключенныеЛентыНовостей) - some fields may not be filled in.'"), // Комментарий
				ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		КонецЕсли;
	...
	ИначеЕсли КлючОбъекта = "ДоступныеЛентыНовостей" Тогда
		...
		ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
			...
			, // ОбъектМетаданных
			, // Данные
			НСтр("ru = 'Ошибка при записи настроек новостей. Настройки для ключа ДоступныеЛентыНовостей не должны записываться. Записывать можно только ОтключенныеЛентыНовостей.';
				|en = 'An error occurred while saving news settings. Settings for key ДоступныеЛентыНовостей should not be saved . You can save only ОтключенныеЛентыНовостей.'"), // Комментарий
			ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		...
		ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при записи настроек новостей. Настройки для ключа [%1] не должны записываться через ХранилищеНастроек.НастройкиНовостей.';
					|en = 'An error occurred while saving news settings. Settings for the [%1] key should not be saved using ХранилищеНастроек.НастройкиНовостей.'"),
	...
	Если (НЕ ОбработкаНовостейПовтИсп.ЕстьРолиЧтенияНовостей())
		...
		ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Хранилище настроек новостей';
				|en = 'ISL:News.News setting storage'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Новости. Хранилище настроек новостей. %1. Нарушение прав доступа';
					|en = 'News. News setting storage.%1. Access right violation'", ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Попытка вызова ХранилищаНастроек.НастройкиНовостей.%1, когда недоступна роль ЧтениеНовостей или ПолныеПрава и не включен привилегированный режим.';
					|en = 'Attempt to call ХранилищаНастроек.НастройкиНовостей.%1, when the role ЧтениеНовостей or ПолныеПрава is unavailable, and and privileged mode is not enabled.'",
