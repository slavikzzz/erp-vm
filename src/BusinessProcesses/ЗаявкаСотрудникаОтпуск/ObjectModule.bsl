...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ДанныеЗаполнения <> Неопределено Тогда
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("ФизическоеЛицо") Тогда
			Если ТипЗаявкиКабинетСотрудника = Перечисления.ТипыЗаявокКабинетСотрудника.ЗаявлениеНаКомпенсациюОтпуска Тогда
				Наименование = СтрШаблон(НСтр("ru = 'Заявление на компенсацию дней отпуска %1';
												|en = 'Application for leave days compensation %1'"), Строка(ДанныеЗаполнения.ФизическоеЛицо));
			Иначе
				Если ЗаСвойСчет Тогда
					Наименование = СтрШаблон(НСтр("ru = 'Заявление на отпуск без сохранения оплаты %1';
													|en = 'Application for unpaid leave %1'"), Строка(ДанныеЗаполнения.ФизическоеЛицо));
				ИначеЕсли УчебныйОтпуск Тогда
					Наименование = СтрШаблон(НСтр("ru = 'Заявление на учебный отпуск %1';
													|en = 'Application for study leave %1'"), Строка(ДанныеЗаполнения.ФизическоеЛицо));
				Иначе
					Наименование = СтрШаблон(НСтр("ru = 'Заявление на оплачиваемый отпуск %1';
													|en = 'Application for paid leave %1'"), Строка(ДанныеЗаполнения.ФизическоеЛицо));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ДоступноВыполнение() Экспорт
	ДоступноВыполнение = БизнесПроцессыЗаявокСотрудников.ДоступноВыполнение(ЭтотОбъект,  Отпуска.Выгрузить());
