	...
	Если РеквизитыСделки.Статус = Перечисления.СтатусыСделок.Проиграна Тогда
		...
		Если НЕ Результат Тогда
			...
			НСтр("ru = 'Для выполнения задачи необходимо определить причину проигрыша сделки';
				|en = 'To complete the task, determine the sales opportunity loss reason'"));
		КонецЕсли;
	ИначеЕсли РеквизитыСделки.Статус = Перечисления.СтатусыСделок.ВРаботе Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Для выполнения задачи сделка должна находится в статусе выиграна или проиграна.';
				|en = 'To complete the task, set sales opportunity status to Won or Lost.'"));
		Результат = Ложь;
	Иначе
		Результат = Истина;
	КонецЕсли;
	...
	Если Результат Тогда
		...
		Если НЕ Запрос.Выполнить().Пустой() Тогда
			...
			НСтр("ru = 'Для выполнения задачи необходимо корректно заполнить результаты удовлетворения первичного спроса.';
				|en = 'To execute the task, correctly fill in the results of primary demand fulfillment.'"));
			Результат = Ложь;
		Иначе
			Результат = Истина;
		КонецЕсли;
...
Процедура ПервичныйКонтактПриВыполнении(ТочкаМаршрутаБизнесПроцесса, Задача, Отказ)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПочтовыйКлиент") Тогда
		...
		НСтр("ru = 'Задача не может считаться выполненной, так как нет зарегистрированных взаимодействий.';
			|en = 'Cannot complete the task because no interactions are registered.'");
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр(
				...
				|en = 'Cannot complete the task because no agreement with customer is specified.'"));
	КонецЕсли;
...
Процедура ПодготовкаПредложенияПриВыполнении(ТочкаМаршрута, Задача, Отказ)
	...
	ТекстСообщения =
		НСтр("ru = 'Задача не может считаться выполненной, так как нет действующего коммерческого предложения по сделке.';
			|en = 'The task cannot be considered completed since there is no valid sales quotation for the sales opportunity.'");
	...
	Если Не Результат Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Задача не может считаться выполненной, так как не определен контрагент партнера.';
				|en = 'Cannot complete the task because no partner counterparty is specified.'"));
	Иначе
		ТекстЗапроса =
			"ВЫБРАТЬ ПЕРВЫЕ 1
			|	ЗаказКлиента.Ссылка
		...
		ТекстСообщения =
			НСтр("ru = 'Задача не может считаться выполненной, так как нет заказа клиента.';
				|en = 'Cannot complete the task because no sales order is specified.'");
		Результат = ДокументСоздан(ТекстЗапроса, Предмет, ТекстСообщения);
	КонецЕсли;
	...
	Если Не Результат Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр(
				...
				|en = 'Cannot complete the task because the sales opportunity has no open liabilities.'"));
	КонецЕсли;
...
Процедура КонтрольВыполненияОбязательствПриВыполнении(ТочкаМаршрутаБизнесПроцесса, Задача, Отказ)
	...
	Если НЕ Результат Тогда
		...
		НСтр("ru = 'Задача не может быть выполнена, так как есть не выполненные обязательства по сделке';
			|en = 'Cannot complete the task because the sales opportunity has unfulfilled liabilities'"));
	КонецЕсли;
