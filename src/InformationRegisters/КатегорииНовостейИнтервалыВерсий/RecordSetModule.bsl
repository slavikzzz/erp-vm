	...
	Для каждого ТекущаяЗапись Из ЭтотОбъект Цикл
		...
		Если ТекущаяЗапись.КатегорияНовостей = М.КатегорияВерсияПродукта()
			...
			ТекстСообщения = НСтр("ru = 'Формат значения версии ОТ (%ВерсияОТ%) должен быть 99.99.999.9999 %ПодробныйТекстОшибки%.';
									|en = 'Format of FROM version value (%ВерсияОТ%) must be 99.99.999.9999 %ПодробныйТекстОшибки%.'");
			...
			ТекстСообщения = НСтр("ru = 'Формат значения версии ДО (%ВерсияДО%) должен быть 99.99.999.9999 %ПодробныйТекстОшибки%.';
									|en = 'Format of BEFORE version value (%ВерсияДО%) must be 99.99.999.9999 %ПодробныйТекстОшибки%.'");
...
Процедура ПроверитьВерсию(ВерсияСтрокой, ТекстСообщения, Отказ)
	...
	ВсеЦифры = ОбработкаНовостейСлужебный.ВсеЦифры();
					...
					Сообщение.Текст = СтрЗаменить(
						...
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '(первое число версии %1 должно быть длиной 2 символа)';
								|en = '(the first number of the %1 version should contain 2 characters)'"),
							ВерсияСтрокой));
							...
							Сообщение.Текст = СтрЗаменить(
								...
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = '(первое число версии %1 должно содержать только цифры, а найден символ %2)';
										|en = '(the first number of the %1 version should contain only digits but the %2 character is found)'"),
									ВерсияСтрокой,
									ТекущийСимвол));
							Сообщение.УстановитьДанные(ЭтотОбъект);
							Сообщение.Сообщить();
					...
					Сообщение.Текст = СтрЗаменить(
						...
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '(второе число версии %1 должно быть длиной 2 символа)';
								|en = '(the second number of the %1 version should contain only 2 characters)'"),
							ВерсияСтрокой));
					Сообщение.УстановитьДанные(ЭтотОбъект);
					Сообщение.Сообщить();
					Отказ = Истина;
							...
							Сообщение.Текст = СтрЗаменить(
								...
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = '(второе число версии %1 должно содержать только цифры, а найден символ %2)';
										|en = '(the second number of the %1 version should contain only digits but the %2 character is found)'"),
									ВерсияСтрокой,
									ТекущийСимвол));
							Сообщение.УстановитьДанные(ЭтотОбъект);
							Сообщение.Сообщить();
					...
					Сообщение.Текст = СтрЗаменить(
						...
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '(третье число версии %1 должно быть длиной 3 символа)';
								|en = '(the third number of the %1 version should contain 3 characters)'"),
							ВерсияСтрокой));
					Сообщение.УстановитьДанные(ЭтотОбъект);
					Сообщение.Сообщить();
					Отказ = Истина;
							...
							Сообщение.Текст = СтрЗаменить(
								...
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = '(третье число версии %1 должно содержать только цифры, а найден символ %2)';
										|en = '(the third number of the %1 version should contain only digits but the %2 character is found)'"),
									ВерсияСтрокой,
									ТекущийСимвол));
							Сообщение.УстановитьДанные(ЭтотОбъект);
							Сообщение.Сообщить();
					...
					Сообщение.Текст = СтрЗаменить(
						...
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '(четвертое число версии %1 должно быть длиной 4 символа)';
								|en = '(the fourth number of the %1 version should contain 4 characters)'"),
							ВерсияСтрокой));
					Сообщение.УстановитьДанные(ЭтотОбъект);
					Сообщение.Сообщить();
					Отказ = Истина;
							...
							Сообщение.Текст = СтрЗаменить(
								...
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = '(четвертое число версии %1 должно содержать только цифры, а найден символ %2)';
										|en = '(the fourth number of the %1 version should contain only digits but the %2 character is found)'"),
									ВерсияСтрокой,
									ТекущийСимвол));
							Сообщение.УстановитьДанные(ЭтотОбъект);
							Сообщение.Сообщить();
		...
		Сообщение.Текст = СтрЗаменить(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '(должно быть 4 числа версии, а не %1 как в %2)';
					|en = '(there should be 4 numbers of the version, not %1 like in %2)'"),
