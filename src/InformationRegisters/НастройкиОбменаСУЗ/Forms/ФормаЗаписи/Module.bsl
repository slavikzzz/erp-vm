			...
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'Настройка обмена для организации %1 и производственного объекта %2 уже существует';
						|en = 'Настройка обмена для организации %1 и производственного объекта %2 уже существует'"),
					Запись.Организация,
	...
	Если Не ЗначениеЗаполнено(Запись.Токен)
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Необходимо заполнить идентификатор соединения или токен';
				|en = 'Необходимо заполнить идентификатор соединения или токен'"),,
			"Запись.ИдентификаторСоединения",,
			Отказ);
	КонецЕсли;
		...
		Если ИспользуетсяПроизводственныйОбъект
			...
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Необходимо заполнить производственный объект';
					|en = 'Необходимо заполнить производственный объект'"),,
				"Запись.ПроизводственныйОбъект",,
				Отказ);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Запись.ПроизводственныйОбъектАдресСтрокой) Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Необходимо заполнить адрес производственного объекта';
					|en = 'Необходимо заполнить адрес производственного объекта'"),,
...
Процедура ПолучитьИдентификаторСоединенияЧерезИнтернетПоддержку(Команда)
	...
	Если Не ЗначениеЗаполнено(Запись.ПроизводственныйОбъектАдресСтрокой) Тогда
		ТекстСообщения = НСтр("ru = 'Поле ""Адрес объекта"" не заполнено';
								|en = 'Поле ""Адрес объекта"" не заполнено'");
...
Функция ПроверитьПодключениеСУЗНаКлиенте()
	...
	Возврат Новый Структура("ТекстОшибки, ТребуетсяОбновлениеКлючаСессии", НСтр("ru = 'Недоступно';
																				|en = 'Недоступно'"), Ложь);
...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Если Не ЗначениеЗаполнено(Запись.СтанцияУправленияЗаказами) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не заполнено поле ""СУЗ""';
				|en = 'Не заполнено поле ""СУЗ""'"),, "Запись.СтанцияУправленияЗаказами",, ЕстьОшибки);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Запись.Адрес) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не заполнено поле ""Адрес""';
				|en = 'Не заполнено поле ""Адрес""'"),, "Запись.Адрес",, ЕстьОшибки);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Запись.Порт) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не заполнено поле ""Порт""';
				|en = 'Не заполнено поле ""Порт""'"),, "Запись.Порт",, ЕстьОшибки);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Запись.Таймаут) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не заполнено поле ""Таймаут""';
				|en = 'Не заполнено поле ""Таймаут""'"),, "Запись.Таймаут",, ЕстьОшибки);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Запись.Организация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не заполнено поле ""Организация""';
				|en = 'Не заполнено поле ""Организация""'"),, "Запись.Организация",, ЕстьОшибки);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Запись.Токен)
		И Не ЗначениеЗаполнено(Запись.ИдентификаторСоединения) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не заполнено поле ""Идентификатор соединения""';
				|en = 'Не заполнено поле ""Идентификатор соединения""'"),, "Запись.ИдентификаторСоединения",, ЕстьОшибки);
	КонецЕсли;
...
Процедура ПриИзмененииТекстаАдресаПроизводственногоОбъекта(Текст)
	...
	Запись.ПроизводственныйОбъектАдрес = ИнтеграцияИСВызовСервера.ЗначенияПолейКонтактнойИнформации(
	...
	Если КоличествоВидовПродукции > 0
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Для подключения к системе ""Честный знак"" необходимо указать идентификатор соединения.';
				|en = 'Для подключения к системе ""Честный знак"" необходимо указать идентификатор соединения.'"),,
		...
		Если Не ЗначениеЗаполнено(Запись.ПроизводственныйОбъектАдресСтрокой) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Учет по видам продукции, использующим код ФИАС отключен.
					...
					|en = 'Учет по видам продукции, использующим код ФИАС отключен.
					|Для поиска по адресному классификатору, поле ""Адрес объекта"" должно быть заполнено.'"),,
				"ПроизводственныйОбъектАдресСтрокой", "Запись.ПроизводственныйОбъектАдресСтрокой");
		Иначе
			ПоказатьВопрос(
	...
	Если ИдентификаторАдреса = "" Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Адрес, указанный в поле ""Адрес объекта"", не найден в адресном классификаторе';
				|en = 'Адрес, указанный в поле ""Адрес объекта"", не найден в адресном классификаторе'"),,
			"ПроизводственныйОбъектАдресСтрокой", "Запись.ПроизводственныйОбъектАдресСтрокой");
	Иначе
	...
	ИначеЕсли СписокМОД.Количество() = 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'В системе ""Честный знак"" по виду продукции ""%1"" места осуществления деятельности не найдены.';
				|en = 'В системе ""Честный знак"" по виду продукции ""%1"" места осуществления деятельности не найдены.'"),
		...
		Если АдресЗаполнен Тогда
			СписокМОД.Добавить("ПоискПоАдресномуКлассифкатору",
				НСтр("ru = '<искать в адресном классификаторе>';
					|en = '<искать в адресном классификаторе>'"));
		КонецЕсли;
		ПоказатьВыборИзСписка(
			Новый ОписаниеОповещения(
