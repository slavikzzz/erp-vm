	...
	Если Не ЗавершениеРаботы И ДанныеИзменены Тогда
		...
		Если Отказ Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'В заданных настройках обнаружены возможные ошибки.';
													|en = 'Possible errors in specified settings were found.'"),
...
Процедура УстановитьУсловноеОформление()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами") Тогда
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Соглашение об изменении стоимости (отправитель)';
																	|en = 'Cost change agreement (sender)'"));
		
	КонецЕсли;
	
	ЕстьПравоРедактирования = ПравоДоступа("Редактирование", Метаданные.РегистрыСведений.ПодписываемыеВидыЭД);
	Элементы.ВидыПодписываемыхЭДВыделитьВсе.Видимость = ЕстьПравоРедактирования;
	Элементы.ВидыПодписываемыхЭДСнятьВсе.Видимость = ЕстьПравоРедактирования;
