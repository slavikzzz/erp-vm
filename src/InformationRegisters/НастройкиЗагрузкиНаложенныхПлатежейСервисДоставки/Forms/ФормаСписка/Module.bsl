...
Процедура ДобавитьДоговорЭквайрингаСервер(ДоговорЭквайринга, Отказ)
	...
	Если ДоговорЭквайринга <> Справочники.ДоговорыЭквайринга.ПустаяСсылка() Тогда
		...
		Если ПараметрыДоговора.ИспользуютсяЭквайринговыеТерминалы Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ru = 'В договоре необходимо указать, что платежи проводятся через Интернет-эквайринг.';
									|en = 'In the contract, specify that payments are made via Internet acquiring.'", ОбщегоНазначения.КодОсновногоЯзыка());
		Иначе
			
			Отказ = Ложь;
			Если ПараметрыДоговора.Организация = Справочники.Организации.ПустаяСсылка() Тогда
				ТекстСообщения = НСтр("ru = 'В договоре необходимо указать организацию.';
										|en = 'Specify the company in the contract.'", ОбщегоНазначения.КодОсновногоЯзыка());
				Отказ = Истина;
			КонецЕсли;
			
			Если ПараметрыДоговора.Контрагент = Справочники.Контрагенты.ПустаяСсылка() Тогда
				ТекстСообщения = ТекстСообщения + Символы.ПС
					+ НСтр("ru = 'В договоре необходимо указать контрагента (перевозчика).';
							|en = 'Specify the counterparty (carrier) in the contract.'", ОбщегоНазначения.КодОсновногоЯзыка());
				Отказ = Истина;
			КонецЕсли;
		...
		Если Отказ Тогда
			ТекстОшибки = НСтр("ru = 'Выбран некорректный договор подключения.';
								|en = 'Incorrect payment system contract is specified.'", ОбщегоНазначения.КодОсновногоЯзыка());
