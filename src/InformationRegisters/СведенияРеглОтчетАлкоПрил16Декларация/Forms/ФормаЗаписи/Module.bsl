...
Процедура ПриОткрытии(Отказ)
	...
	Заголовок = "Сведения о фармацевтической субстанции спирта этилового (этанола)";
...
Процедура ДоступностьПолейНаСервере()
	...
	Элементы.П000010000316.ТолькоПросмотр = НЕ ФлажокОтклАвтоРасчет;
	Элементы.П000010000312.ТолькоПросмотр = НЕ ФлажокОтклАвтоРасчет;
	Элементы.П000010000323.ТолькоПросмотр = НЕ ФлажокОтклАвтоРасчет;
	
	Элементы.П000010000306.ПропускатьПриВводе = НЕ ФлажокОтклАвтоРасчет;
	Элементы.П000010000316.ПропускатьПриВводе = НЕ ФлажокОтклАвтоРасчет;
	Элементы.П000010000312.ПропускатьПриВводе = НЕ ФлажокОтклАвтоРасчет;
	Элементы.П000010000323.ПропускатьПриВводе = НЕ ФлажокОтклАвтоРасчет;
...
Процедура ФормированиеЗаголовковСвернутогоОтображения()
	...
	Элементы.Остатки.ЗаголовокСвернутогоОтображения = "Остатки в декалитрах: 
					...
					| на конец периода " + Формат(Запись.П000010000323, "ЧЦ = 15; ЧДЦ = 2; ЧН=; ЧВН;");
	
	// Поступление.
	Элементы.Приход.ЗаголовокСвернутогоОтображения = "Приход: 
					| всего " + Формат(Запись.П000010000306 + Запись.П000010000305, "ЧЦ = 15; ЧДЦ = 2; ЧН=; ЧВН;") + " декалитров, 
					| в том числе получено извне " + Формат(Запись.П000010000306, "ЧЦ = 15; ЧДЦ = 2; ЧН=; ЧВН;") + " декалитров.";  
	
	// Расход.
	ОбщийРасход = Запись.П000010000319 + Запись.П000010000320 + Запись.П000010000321 + Запись.П000010000322 + Запись.П000010000312;
	Элементы.Расход.ЗаголовокСвернутогоОтображения = "Расход: 
					| всего " + Формат(ОбщийРасход, "ЧЦ = 15; ЧДЦ = 2; ЧН=; ЧВН;") + " декалитров, 
					| в том числе отгружено извне " + Формат(Запись.П000010000312, "ЧЦ = 15; ЧДЦ = 2; ЧН=; ЧВН;") + " декалитров.";
