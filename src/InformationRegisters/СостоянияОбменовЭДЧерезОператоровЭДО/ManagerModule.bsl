...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось обработать состояния обменов электронных документов по причине:';
									|en = 'Cannot process states of electronic document exchanges for the reason:'") + Символы.ПС 
	...
	Если ОбработанныхОбъектов = 0 И ПроблемныхОбъектов <> 0 Тогда
		ШаблонСообщения = НСтр("ru = 'Не удалось обработать некоторые состояния обменов электронных документов (пропущены): %1';
								|en = 'Cannot process some states of electronic document exchanges (skipped): %1'");
		ТекстСообщения = СтрШаблон(ШаблонСообщения, ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ШаблонСообщения = НСтр("ru = 'Обработана очередная порция состояний обменов электронных документов: %1';
								|en = 'The next part of electronic document exchange states is processed: %1'");
