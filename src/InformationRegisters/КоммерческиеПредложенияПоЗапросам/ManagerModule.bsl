...
Процедура СопоставитьПоступившееПредложение(Знач ЗапросКоммерческихПредложений, Знач КоммерческоеПредложениеПоставщика) Экспорт
	...
	Исключение
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Сопоставление поступившего предложения с запросом КП';
														|en = 'Mapping of the received quotation with the CP query'"), ПодробноеПредставлениеОшибки(Информация));
	КонецПопытки;
