...
Процедура РамкаНевидна(Команда)
	...
	ОшибкаВидеозаписи = НСтр("ru = 'Ошибка запуска видеозаписи через плагин';
							|en = 'Error starting video recording via the plugin'");
	ПроверкаОтключитьЗапись();
...
Функция Текст_Плагин()
	...
	ЗаголовокСодержимого = НСтр("ru = '<b>В процессе проверки возникла ошибка:</b>';
								|en = '<b>An error occurred when checking:</b>'");
	ТекстОписания = "";
	
	Если НЕ ЕстьПлагин Тогда
		ЗаголовокСодержимого = НСтр("ru = '<b>Не удалось подключиться к плагину ГИС НР</b>';
									|en = '<b>Cannot connect to the Independent Registrar State Information System plugin</b>'");
		...
		Если ОбщегоНазначения.ЭтоMacOSКлиент() 
			...
			|en = '
			| - The plugin is for Windows and Linux.'");
		КонецЕсли;
		...
		|en = '
		| - The installed plugin version is 6.4.2.0 or later. You can download the latest version <a href = ""Скачать"">here</a>.
		| - The plugin is running and displayed in the operating system notification area.
		| - The TCP port number of the plugin service is specified correctly.'");
	...
	ИначеЕсли ЗначениеЗаполнено(ТекстОшибки) И ЗначениеСостояниеПлагина("Ошибка", Ложь) Тогда
		ЗаголовокСодержимого = НСтр("ru = '<b>В плагине ГИС НР возникла ошибка</b>';
									|en = '<b>An error occurred in the plugin of Independent Registrar State Information System</b>'");
		СодержимоеОшибки = НСтр("ru = 'Убедитесь, что:
		...
		|en = 'You can:
		| - Check the settings of the plugin of Independent Registrar State Information System in the user workplace.
		| - Restart the plugin from the operating system.
		| - Install plugin version 6.4.2.0 or later. You can download the latest version <a href = ""Скачать"">here</a>.'")
	...
	ИначеЕсли ЗначениеСостояниеПлагина("ВидеозаписьДоступна", Ложь) = Ложь Тогда
		СодержимоеОшибки = НСтр("ru = 'Видеозапись уже запущена в плагине';
								|en = 'The video recording has already started in the plugin'");
	...
	ИначеЕсли ЗначениеСостояниеПлагина("СертификатВыбран", Ложь) = Ложь Тогда
		СодержимоеОшибки = СодержимоеОшибки + Символы.ПС
				+ НСтр("ru = 'В настройках плагина не указан сертификат. 
					...
					|en = 'The certificate is not specified in the settings. 
					|<a href = ""Сертификат"">Specify</a> a digital signature certificate for the plugin.'");
	...
	ИначеЕсли ЗначениеСостояниеПлагина("ОшибкаЭП", Ложь) = Истина Тогда
		СодержимоеОшибки = СодержимоеОшибки + Символы.ПС 
				+ НСтр("ru = 'Обнаружены проблемы формирования электронной подписи.
					...
					|en = 'There are digital signature generation issues.
					|Check or <a href = ""Сертификат"">change</a> the digital signature certificate of the plugin.'");
