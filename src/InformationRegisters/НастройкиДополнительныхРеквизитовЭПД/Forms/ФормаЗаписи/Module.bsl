...
Процедура ПоказатьПредупреждениеОРеквизите(Элемент, Узел)
	...
	Если ОбменСГИСЭПДКлиентСервер.ИспользуютсяДополнительныеРеквизитыЭПД() = 0 Тогда
		Элемент.Заголовок = НСтр("ru = 'Функционал дополнительных реквизитов не используется и будет задействован в следующих релизах';
								|en = 'Additional attribute features are not used and will be used in the next releases'");	
	...
	И СтрНачинаетсяС(Узел, "//Card") Тогда
		Элемент.Заголовок = НСтр("ru = 'Добавление реквизита на этом уровне не предусмотрено форматом, передаваться по ЭДО он не будет.
			...
			|en = 'Adding an attribute is not supported for this format. It cannot be sent via EDI.
			|You will be able to pass such attributes in next releases'");
		Элемент.Видимость = Истина;
	Иначе
