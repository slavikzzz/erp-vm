...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.КонтролироватьСвободныеОстатки Тогда
		Заколовок = НСтр("ru = 'Номенклатура без контроля свободного остатка (Склад: %1)';
						|en = 'Item without available inventory control (Warehouse: %1)'");
	Иначе
		Заколовок = НСтр("ru = 'Номенклатура с контролем свободного остатка (Склад: %1)';
						|en = 'Item with available inventory control (Warehouse: %1)'");
	КонецЕсли;
...
Процедура СформироватьНадпись(Строка)
	...
	ИначеЕсли Строка.ТипЗаписи = 2 И Строка.КоличествоИсключений = 1 Тогда
		Строка.Характеристика =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для %1 характеристики';
																		|en = 'for the %1 variant'"), Строка.КоличествоИсключений);
	Иначе
		Строка.Характеристика =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для %1 характеристик';
																		|en = 'for %1 variants'"), Строка.КоличествоИсключений);
	КонецЕсли;
