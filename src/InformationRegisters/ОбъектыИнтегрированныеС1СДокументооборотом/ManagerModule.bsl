...
Процедура ДобавитьСвязь(ID, Тип, ОбъектИС, НаличиеПрисоединенныхФайлов = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ОбъектИС) Тогда
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Установка связи объекта ДО (%1, %2) с несуществующим объектом ИС.';
				|en = 'Link the Document Management object (%1, %2) with a non-existing information system object.'"), ID, Тип);
	КонецЕсли;
