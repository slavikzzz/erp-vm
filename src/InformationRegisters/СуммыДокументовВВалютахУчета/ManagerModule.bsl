...
Процедура ОбновитьДвиженияДокументов(РегистраторыСумм) Экспорт
	...
	//++ НЕ УТ
	СоставОбновления.Вставить("РезервыПоСомнительнымДолгам", РегистрыНакопления.РезервыПоСомнительнымДолгам.СоздатьНаборЗаписей());
	СоставОбновления.Вставить("ИнвестицииВАренду", РегистрыНакопления.ИнвестицииВАренду.СоздатьНаборЗаписей());
	//-- НЕ УТ
	...
	СоставОбновления.Вставить("ОперацииСПрослеживаемымиТоварамиРасширенный", РегистрыСведений.ОперацииСПрослеживаемымиТоварамиРасширенный.СоздатьНаборЗаписей());
	...
	ИсправленияДокументов = ИсправлениеДокументов.ИсправленияДокументовПоТипуИсправления(Регистраторы, 
		Метаданные.Документы.КорректировкаПриобретения.ПолноеИмя()); 
		
	Для каждого ИсправлениеПриобретения Из ИсправленияДокументов Цикл
		СоставОбновленияПрослеживаемости = Новый Структура();
		СоставОбновленияПрослеживаемости.Вставить("ОперацииСПрослеживаемымиТоварамиРасширенный", РегистрыСведений.ОперацииСПрослеживаемымиТоварамиРасширенный.СоздатьНаборЗаписей());
		ОбновитьДвиженияДокумента(ИсправлениеПриобретения, СоставОбновленияПрослеживаемости);	
	КонецЦикла;	
	...
	//++ НЕ УТ
	СоставОбновления.Удалить("РезервыПоСомнительнымДолгам");
	//-- НЕ УТ
	...
	СоставОбновления.Удалить("ОперацииСПрослеживаемымиТоварамиРасширенный");
...
Процедура ОбновитьДвиженияДокумента(Ссылка, СоставОбновления)
	...
	Попытка
		...
		Для Каждого ОписаниеОбновления Из СоставОбновления Цикл
			Если Таблицы.Свойство("Таблица" + ОписаниеОбновления.Ключ, Таблица) И ЗначениеЗаполнено(Таблица) Тогда
				...
				Если ОписаниеОбновления.Ключ = "ОперацииСПрослеживаемымиТоварамиРасширенный" 
					И УчетПрослеживаемыхТоваровЛокализация.НеОбновлятьДвиженияРасширенногоРегистра(Таблицы) Тогда
					Продолжить;
				КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось записать движения %Обновление% по причине %Причина%';
								|en = 'Cannot record register records %Обновление%. Reason: %Причина%'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Актуализирует движения документов ""Поступление товаров от хранителей"" и ""Прочее оприходование товаров"".';
									|en = 'Updates register records of the ""VMI return"" and ""Other receipt of goods"" documents.'");
