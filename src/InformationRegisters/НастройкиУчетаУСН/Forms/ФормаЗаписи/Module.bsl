...
Процедура СтавкаНалогаУСНПриИзменении(Элемент)
	...
	Если Запись.СтавкаНалогаУСН > ПределСтавки Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'The selected rate cannot be applied. 
			|The maximum allowable tax rate for the selected taxation object is %1 %.'"),
			ПределСтавки);
...
Процедура ОбъектНалогообложенияУСНПриИзменении(Элемент)
	...
	//++ НЕ УТ
	Запись.РаздельныйУчетТоваров = ПрименяетсяУСНДоходыМинусРасходы;
	//-- НЕ УТ
...
Процедура СкопироватьЗаписьРегистра(ЗначениеКопирования)
	...
	//++ НЕ УТ
	Запись.СтавкаНалогаУСН = УчетУСНПСНКлиентСервер.НалоговаяСтавкаПоУмолчанию(Запись.ПрименяетсяУСНДоходыМинусРасходы);
	//-- НЕ УТ
	УстановитьВидимостьДоступностьЭлементов(ЭтотОбъект);
	Копирование = Истина;
...
Процедура УстановитьВидимостьДоступностьЭлементов(Форма)
	...
	//++ НЕ УТ
	Форма.Элементы.РаздельныйУчетТоваров.Видимость = Истина;
	Форма.Элементы.РаздельныйУчетТоваров.Доступность = Не Форма.ПрименяетсяУСНДоходыМинусРасходы;
	Форма.Элементы.ГруппаПредупреждениеДоходыРасходы.Видимость = Форма.ПрименяетсяУСНДоходыМинусРасходы И Форма.ПартионныйУчетВключен
		И Не Форма.МетодОценкиСтоимостиТоваров = ПредопределенноеЗначение("Перечисление.МетодыОценкиСтоимостиТоваров.ФИФОСкользящаяОценка");
	Форма.Элементы.ИспользуетсяТрудНаемныхРаботников.Видимость = Истина;
	СтавкаПоУмолчанию = УчетУСНПСНКлиентСервер.НалоговаяСтавкаПоУмолчанию(Форма.ПрименяетсяУСНДоходыМинусРасходы);
	ПрименяетсяПовышеннаяСтавка = Форма.Запись.СтавкаНалогаУСН > СтавкаПоУмолчанию;
	Форма.Элементы.СтавкаНалогаУСН.ТолькоПросмотр = ПрименяетсяПовышеннаяСтавка;
	Форма.Элементы.ГруппаЗначениеСтавкиНалогаУСН.РасширеннаяПодсказка.Заголовок =
		УчетУСНПСНКлиентСервер.УточнениеПримененияСтавкиНалогаУСН(ПрименяетсяПовышеннаяСтавка, 
			?(ЗначениеЗаполнено(Форма.ДатаСледующейЗаписи), Форма.ДатаСледующейЗаписи, НачалоДня(ТекущаяДата())));
	//-- НЕ УТ
