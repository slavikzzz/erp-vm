...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Элементы.ПользователиПолноеПредставление.Заголовок = 
			...
			НСтр("ru = 'Пользователь, группа пользователей';
				|en = 'User, user group'"), НСтр("ru = 'Пользователь';
																	|en = 'User'"));
		Элементы.ПользователиКомментарий.Подсказка =
			...
			Если ЕстьПравоРедактированияЗапретаЗагрузкиДанных Тогда
				ПереходКДругимДатамЗапрета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Также можно настроить <a href=""%1"">даты запрета загрузки данных</a> из других приложений.';
						|en = 'You can also set up <a href=""%1"">restriction dates for importing data</a> from applications.'"),
...
Процедура УстановкаДатыЗапретаПриИзменении(Элемент)
	...
	Если ТекущаяУстановкаДатыЗапрета = "ПоПользователям" И ВыбранноеЗначение = "ДляВсехПользователей" Тогда
		...
		Если ЕстьНедоступныеОбъектыПоПользователям Тогда
			...
			ВызватьИсключение(НСтр("ru = 'Недостаточно прав для изменения дат запрета.';
									|en = 'You are not authorized to edit period-end closing dates.'"), КатегорияОшибки.НарушениеПравДоступа);
		КонецЕсли;
		...
		Иначе
			Если ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить даты запрета для ""%1""?';
						|en = 'Do you want to turn off period-end closing dates for ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить даты запрета для группы пользователей ""%1""?';
						|en = 'Do you want to turn off period-end closing dates for ""%1"" user group?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить даты запрета для внешнего пользователя ""%1""?';
						|en = 'Do you want to turn off period-end closing dates for external user ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ГруппыВнешнихПользователей") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить даты запрета для группы внешних пользователей ""%1""?';
						|en = 'Do you want to turn off period-end closing dates for external user group ""%1""?'"), ТекущиеДанные.Пользователь);
			Иначе
			...
			Если ТекущиеДанные.ПолучитьЭлементы().Количество() > 0 Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить все настроенные даты запрета для раздела ""%1"" и его объектов?';
						|en = 'Do you want to turn off all period-end closing dates for section ""%1"" and its objects?'"), ТекущиеДанные.Раздел);
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отключить дату запрета для раздела ""%1""?';
						|en = 'Do you want to turn off the period-end closing date for section ""%1""?'"), ТекущиеДанные.Раздел);
			КонецЕсли;
		Иначе
			ТекстВопроса = НСтр("ru = 'Отключить общую дату запрета для всех разделов?';
								|en = 'Do you want to turn off the single-date restriction setting for all sections?'");
		КонецЕсли;
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отключить дату запрета для объекта ""%1""?';
				|en = 'Do you want to turn off the period-end closing date for object ""%1""?'"), ТекущиеДанные.Объект);
	КонецЕсли;
	...
	Если Не ЗначениеНайдено Тогда
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '""%1"" уже есть в списке объектов';
				|en = '""%1"" is already in the object list'"), ВыбранноеЗначение));
		Возврат;
	КонецЕсли;
		...
		Если Не ЗаменитьОбъектВЗаписиПользователяНаСервере(
			...
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '""%1"" уже есть в списке объектов.
					...
					|Refresh the form (F5).'"), ВыбранноеЗначение));
			Возврат;
		Иначе
			ОбновитьЗначенияПрочитанныхСвойств(
...
Процедура СпособУказанияДатыЗапретаОбработкаВыбораОбработчикОжидания()
	...
	Если ЗначениеЗаполнено(ТекстВопроса) Тогда
		...
		Если ЕстьНедоступныеОбъекты Тогда
			ВызватьИсключение(НСтр("ru = 'Недостаточно прав для изменения дат запрета.';
									|en = 'You are not authorized to change period-end closing dates.'"), КатегорияОшибки.НарушениеПравДоступа);
		КонецЕсли;
...
Процедура ПользователиПолноеПредставлениеОбработкаВыбораОбработчикОжидания()
	...
	Иначе
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '""%1"" уже есть в списке пользователей.';
				|en = '""%1"" is already in the user list.'"), ВыбранноеЗначение));
	КонецЕсли;
			...
			Форма.Элементы.ТекущийПользовательПредставление.Заголовок =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Настройка для ""%1"":';
						|en = 'Setting for ""%1"":'"), НайденныеСтроки[0].Представление);
		КонецЕсли;
	КонецЕсли;
	...
	Если ВыборИПодборВнешнихПользователей Тогда
		Если Не СправочникВнешниеПользователиДоступен Тогда
			ВызватьИсключение(НСтр("ru = 'Недостаточно прав для выбора внешних пользователей.';
									|en = 'Insufficient rights to select external users.'"), КатегорияОшибки.НарушениеПравДоступа);
		КонецЕсли;
...
Процедура РедактироватьДатуЗапретаВФормеЗавершение(ВыделенныеСтроки) Экспорт
	...
	Иначе
		ПараметрыФормы.Объект = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Выбранные строки (%1)';
				|en = 'Selected lines (%1)'"), ВыделенныеСтроки.Количество());
	КонецЕсли;
...
Функция НетДатыЗапретаПредставление(Данные)
	...
	Если Данные.ЭтоРаздел И Не Данные.Раздел.Пустая() Тогда
		Возврат НСтр("ru = 'Общая дата для всех разделов';
					|en = 'Single date for all sections'");
	ИначеЕсли Не Данные.ЭтоРаздел Тогда	
		...
		ИначеЕсли Не ЗначениеЗаполнено(ДанныеРаздела.ДатаЗапрета) Тогда
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Общая дата для раздела ""%1""';
					|en = 'Single date for section %1'"), ДанныеРаздела.Представление);
		Иначе	
...
Функция ТекстСообщенияВВыбранномРазделеДатыЗапретаДляОбъектовНеУстанавливаются(Раздел)
	...
	Возврат ?(Раздел <> РазделПустаяСсылка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В разделе ""%1"" не предусмотрена установка дат запрета для отдельных объектов.';
				|en = 'Period-end closing date setting is not available for separate objects in the ""%1"" section.'"), Раздел),
		НСтр("ru = 'Для установки дат запрета по отдельным объектам выберите один из разделов ниже и нажмите ""Подобрать"".';
			|en = 'To set a period-end closing date for some objects, choose one of the sections below, and then click Select.'"));
