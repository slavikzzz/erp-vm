...
Процедура ДобавитьПоддержаниеЗапасаГруппировкой(КлючиЗаписи, КлючГруппировки) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Недопустимый тип ключа записи группировки!';
								|en = 'Invalid type of grouping record key.'");
	КонецЕсли;
	...
	Для каждого Ключ Из КлючиЗаписиХарактеристик Цикл
		...
		Иначе
			ВызватьИсключение НСтр("ru = 'Недопустимый тип ключа записи характеристики!';
									|en = 'Invalid type of variant record key.'");;
		КонецЕсли;
	КонецЦикла;
...
Функция ЗаполнитьПараметрПоддержанияЗапаса(КлючиЗаписи, ИмяПараметра, Значение) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Некорректно задано имя параметра!';
								|en = 'Parameter name is specified incorrectly.'");
	КонецЕсли;
...
Процедура ЗадатьУпаковкуТовара(Номенклатура, Характеристика, Склад, УпаковкаЗаказа) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Задание упаковки заказа для товара на складе';
										|en = 'Job for packaging the order for goods in the warehouse'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура РассчитатьСтатистикуПотребленияТоваров(МенеджерВременныхТаблиц) Экспорт
	...
	Пока Истина Цикл
			...
			Для каждого ЗаписьГруппировки Из Группировки Цикл
				...
				Если НайденныеСтроки.Количество() = 0 Тогда
					ВызватьИсключение НСтр("ru = 'Несоответствие в транзакции заблокированных и изменяемых группировок';
											|en = 'Mismatch in the transaction of locked and changed groupings'");
				КонецЕсли;
			КонецЦикла;
		...
		Исключение
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Расчет статистики потребления товаров';
											|en = 'Goods consumption statistics calculation'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение;
		КонецПопытки;
...
Функция Записать(ТаблицаТоваров, ТаблицаГруппировок, ПараметрыДляЗаписи, ОбнулятьРанееЗаданныйМинМакс,
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Запись в регистр ""Товарные ограничения""';
										|en = 'Record to the ""Goods limitations"" register'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
...
Процедура Удалить(ТаблицаТоваров, ТаблицаГруппировок, УдалятьДетальные = Ложь)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление записей в регистре ""Товарные ограничения""';
										|en = 'Delete records in the ""Goods limitations"" register'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
