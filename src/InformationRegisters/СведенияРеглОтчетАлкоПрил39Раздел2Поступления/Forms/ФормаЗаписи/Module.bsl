...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ФормированиеПредставленияПродукцииНаСервере(Запись.П000020000301, Запись.П000020000302, Запись.П000020000303Адрес);
...
Процедура ПолеПриИзменении(Элемент)
	...
	Если Элемент.Имя = "П000020000307" ИЛИ Элемент.Имя = "П000020000308" Тогда
...
Процедура ФормированиеЗаголовковСвернутогоОтображения()
	...
	Если ЗначениеЗаполнено(Запись.П000020000307) 
	    ...
	    Элементы.ГруппаПоставщика.ЗаголовокСвернутогоОтображения = "Поставщик: " + 
			...
			+ ?(ЗначениеЗаполнено(Запись.П000020000308),", КПП " + Запись.П000020000308, 
											?(СтрДлина(СокрЛП(Запись.П000020000307)) = 10,", КПП не заполнено", "") );
	...
	// Доступ к КПП только если введен 10 значный ИНН.
	Если СтрДлина(СокрЛП(Запись.П000020000307)) = 10 Тогда
...
Процедура ВводАдресаНаКлиенте()
	...
	РегламентированнаяОтчетностьАЛКОКлиент.ВызватьФормуВводаАдресаКонтрагента(
					Запись.П000020000303АдресXML, ЗаголовокФормыВвода, Оповещение, ВидКонтактнойИнформации);
...
Процедура ВводАдресаЗавершениеНаКлиенте(Результат, Параметры) Экспорт
	...
	РегламентированнаяОтчетностьАЛКОКлиент.ВводАдресаКонтрагентаЗавершениеНаКлиенте(Результат, 
				Запись.П000020000303Адрес, Запись.П000020000303АдресXML, Модифицированность);
...
Процедура СформироватьСпискиВыбораНаСервере()
	...
	Элементы.П000020000309.СписокВыбора.ЗагрузитьЗначения(МассивВидовДеятельностиПоЛицензии);
...
Процедура ТаблицаПоставщиковВыборНаСервере(СоответствиеТекущиеДанные)
	...
	Запись.П000020000309 = СокрЛП(ВидДеятельности);
