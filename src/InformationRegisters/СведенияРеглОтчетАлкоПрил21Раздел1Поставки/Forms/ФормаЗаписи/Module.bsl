...
Процедура ПриОткрытии(Отказ)
	...
	Заголовок = "Поставка алкогольной продукции";
	...
	Если НЕ ВладелецФормы.ТекущийЭлемент = Неопределено Тогда
		...
		Если ИмяАктивногоПоля = "П000010000307" Тогда
...
Процедура ПолеПриИзменении(Элемент)
	...
	Если Элемент.Имя = "П000010000308" ИЛИ Элемент.Имя = "П000010000309" Тогда
...
Процедура ФормированиеЗаголовковСвернутогоОтображения()
	...
	Если ЗначениеЗаполнено(Запись.П000010000308) 
		или ЗначениеЗаполнено(Запись.П000010000309)
		или ЗначениеЗаполнено(Запись.П000010000306)
		Тогда
		
	    Элементы.ГруппаПолучателя.ЗаголовокСвернутогоОтображения = "Получатель: " + 
			?(ЗначениеЗаполнено(Запись.П000010000306),Запись.П000010000306, "наименование не заполнено") 
			+ ?(ЗначениеЗаполнено(Запись.П000010000308),", ИНН " + Запись.П000010000308, ", ИНН не заполнено")
			+ ?(ЗначениеЗаполнено(Запись.П000010000309),", КПП " + Запись.П000010000309, 
											?(СтрДлина(СокрЛП(Запись.П000010000308)) = 10,", КПП не заполнено", "") );
	...
	// Доступ к КПП только если введен 10 значный ИНН.
	Если СтрДлина(СокрЛП(Запись.П000010000308)) = 10 Тогда
		
	    Элементы.П000010000309.ТолькоПросмотр = Ложь;
		Элементы.П000010000309.ПропускатьПриВводе = Ложь;
		
	Иначе
		
	    Элементы.П000010000309.ТолькоПросмотр = Истина;
		Элементы.П000010000309.ПропускатьПриВводе = Истина;
		Если НЕ СокрЛП(Запись.П000010000309) = "" Тогда
...
Процедура ФормированиеПредставленияПродукцииНаСервере(
	...
	Если АдресПолучателя = Неопределено Тогда
	    АдресПолучателя = Запись.П000010000307;	
	КонецЕсли;
...
Процедура СформироватьСпискиВыбораНаСервере()
	...
	Элементы.П000010000310.СписокВыбора.ЗагрузитьЗначения(МассивВидовДеятельностиПоЛицензии);
...
Процедура УправлениеВидимостью(ПоказатьВыборПроизводителей = Ложь, ПоказатьВыборПолучателей = Ложь)
	...
	ИначеЕсли ПоказатьВыборПолучателей Тогда
		...
		Если ВставитьКодПродукцииПолучатель Тогда	
			ДинСписокПолучателя.Параметры.УстановитьЗначениеПараметра("КодПродукции", Запись.П000010000302);			
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Запись.Получатель) Тогда
			...
			Если НЕ ДинСписокПолучателя.ОсновнаяТаблица = "Справочник.Контрагенты" Тогда
				...
				Если ВставитьКодПродукцииПолучатель Тогда	
					ЗапросВидаЛицензии.Параметры.Вставить("КодПродукции", Запись.П000010000302);					
				КонецЕсли;
				...
				Если КолСтрокРезультата > 0 Тогда
					...
					Если КолСтрокРезультата > 1 Тогда
						...
						Для Инд = 0 По КолСтрокРезультата - 1 Цикл
							...
							Если (СокрЛП(ТекСерияНомер) = СокрЛП(Запись.П00001СерияНомер))
								И (ТекДатаОкончания = Запись.П00001ДатаОкончания) Тогда
...
Процедура ТаблицаПолучателейВыборНаСервере(СоответствиеТекущиеДанные)
	...
	Если НЕ (Запись.П000010000308 = СокрЛП(ИНН)
		И Запись.П000010000309 = СокрЛП(КПП)
		И Запись.П000010000306 = СокрЛП(НаименованиеПолное)) Тогда
	
		Запись.П000010000307 = "";
		Запись.П000010000307XML = "";
		
		Запись.П000010000308 = СокрЛП(ИНН);
		Запись.П000010000309 = СокрЛП(КПП);	
		Запись.П000010000306 = СокрЛП(НаименованиеПолное);
		
		Запись.П000010000310 = СокрЛП(ВидДеятельности);
		
		Запись.П00001ДатаОкончания = ДатаОкончания;
		Запись.П00001ДатаВыдачи = ДатаВыдачи;
		Запись.П00001СерияНомер = СокрЛП(СерияНомер);
		Запись.П00001КемВыдана = СокрЛП(КемВыдана);	
