	...
	Иначе
		...
		Если ДоступнаЗагрузкаАдресныхСведенийИзИнтернет Тогда
			...
			Если КоличествоОбновлений > 0 Тогда
				...
				ИнформацияОбОбновление = НСтр("ru = 'Доступно обновление от';
												|en = 'Available update from'") + " " + ДатаОбновления;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Текст = НСтр("ru = 'Прервать загрузку адресного классификатора?';
				|en = 'Cancel address classifier import?'");
...
Процедура АдресЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	АдресныйКлассификаторКлиент.ВыбратьКаталог(ЭтотОбъект, "АдресЗагрузки", 
		НСтр("ru = 'Папка с файлами адресного классификатора';
			|en = 'Folder with address classifier files'"),
...
Процедура Загрузить(Команда)
	...
	Если КодИсточникаЗагрузки = "КАТАЛОГ" Тогда
		Текст = НСтр("ru = 'Для загрузки адресного классификатора из папки установите расширение для работы с 1С:Предприятием.';
					|en = 'To import the address classifier from the folder, install 1C:Enterprise extension.'");
	...
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю( НСтр("ru = 'Не указан вариант загрузки классификатора.';
															|en = 'Classifier import option is not specified.'") );
...
Процедура ПослеПроверкиДоступностиОбновленияКлиент(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Если Даты.ДатаОбновления > Даты.ДатаСамойСтаройЗагрузки Тогда
			...
			ИнформацияОбОбновление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Доступно обновление от %1';
																									|en = 'An update issued on %1 is available.'"), ДатаОбновленияСтрокой);
...
Функция ЗапуститьПроверкуДоступностиОбновления(УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Проверка обновления для адресного классификатора';
															|en = 'Check update for address classifier'");
...
Процедура ОбновитьИнтерфейсПоКоличествуЗагружаемых()
	...
	// Страница загрузки
	ТекстОписанияЗагрузки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загружаются данные по выбранным регионам (%1)';
																						|en = 'Importing data on selected states (%1)'"), ВыбраноРегионовДляЗагрузки);
...
Процедура ЗагрузитьКлассификаторССайтаАутентификация(Знач КодыРегионов)
	...
	ТекстСостоянияЗагрузки = НСтр("ru = 'Загрузка файлов с Портала 1С:ИТС...';
									|en = 'Downloading files from 1C:ITS Portal…'");
	...
	Если Статус = "Ошибка" Тогда
		Если СтрНайти(Результат.КраткоеПредставлениеОшибки, "404 Not Found") > 0 ИЛИ СтрНайти(Результат.КраткоеПредставлениеОшибки, "401 Unauthorized") > 0 Тогда
			ТекстОшибки = НСтр("ru = 'Не удается загрузить адресные сведения. Возможные причины:
				...
				|en = 'Cannot load address details. Possible reasons:
				| • Incorrect or no login and password entered.
				| • No Internet connection.
				| • The website is on maintenance, try again later.
				| • Firewall or another middleware (antivirus software, etc.) locks attempts to connect to the Internet.
				| • Internet connection is performed via proxy server, but its parameters are not specified in the application.
				|
				|Technical information:'") + Символы.ПС + СтрПолучитьСтроку(Результат.КраткоеПредставлениеОшибки, 1);
		Иначе
			ТекстОшибки = НСтр("ru = 'Вероятно в данный момент на сайте ведутся технические работы. Попробуйте повторить загрузку позднее.
				|Техническая информация:';
				|en = 'There might be some routine maintenance on the website. Try again later.
				| Technical information:'") + Символы.ПС + Результат.КраткоеПредставлениеОшибки;
		КонецЕсли;
...
Процедура ЗапуститьФоновуюЗагрузкуИзКаталогаКлиента(Знач ПараметрыЗагрузки)
	...
	ТекстСостоянияЗагрузки = НСтр("ru = 'Передача файла на сервер приложения...';
									|en = 'Sending file to application server…'");
	...
	Если Статус = "Выполнено" Тогда
		...
		Если ИменаНезагруженныхРегионов.Количество() = 1 Тогда
			...
			ШаблонТекстаОшибки = НСтр("ru = 'Не удается загрузить адресные сведения из папки для региона ""%1"".
				...
				|en = 'Cannot import address details from the folder for the ""%1"" region.
		...
		ИначеЕсли ИменаНезагруженныхРегионов.Количество() > 1 Тогда
			...
			Если ИменаНезагруженныхРегионов.Количество() = 2 Тогда
				...
				ШаблонТекстаОшибки = НСтр("ru = 'Не удается загрузить адресные сведения из папки для регионов ""%1"" и ""%2"".
					|Подробнее см. в журнале регистрации.';
					|en = 'Cannot import address details from the folder for the ""%1"" and ""%2"" regions.
					|For more information, see the event log.'");
			...
			Иначе
				...
				ШаблонТекстаОшибки = НСтр("ru = 'Не удается загрузить адресные сведения из папки для регионов ""%1"", ""%2"" и др.
					|Подробнее см. в журнале регистрации.';
					|en = 'Cannot import address details from the folder for the ""%1"", ""%2"", and other regions.
					|For more information, see the event log.'");
	...
	Если Статус = "Ошибка" Тогда
		...
		Если ПустаяСтрока(ТекстОшибки) Тогда
			ТекстОшибки = НСтр("ru = 'Не удается загрузить адресные сведения из папки.
				...
				|
				|Техническая информация:';
				|en = 'Couldn''t import address details from the folder.
				|Download the files from http://its.1c.ru/download/fias2 (in Russian) and import them manually.
				|
				|Technical details:'") + Символы.ПС + Результат.КраткоеПредставлениеОшибки;
		КонецЕсли;
...
Процедура ЗавершениеЗагрузки()
	...
	ТекстОписанияЗагрузки = НСтр("ru = 'Адресный классификатор успешно загружен.';
								|en = 'Address classifier imported.'");
