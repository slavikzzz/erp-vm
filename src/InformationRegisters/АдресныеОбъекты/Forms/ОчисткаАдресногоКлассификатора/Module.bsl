...
Процедура ПриОткрытии(Отказ)
	...
	Если Не АдресныйКлассификаторСодержитЗагруженныеСведения Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Адресные сведения для очистки отсутствуют';
										|en = 'No address information to clear'"));
	Иначе
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Текст = НСтр("ru = 'Прервать очистку адресного классификатора?';
				|en = 'Cancel clearing the address classifier?'");
...
Процедура ОбновитьИнтерфейсПоКоличествуОчищаемых()
	...
	// Страница загрузки
	ТекстОписанияОчистки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Очищаются данные выбранных регионов (%1)';
																						|en = 'Clearing data for selected states (%1)'"), ВыбраноРегионовДляОчистки);
...
Процедура ОчиститьКлассификатор()
	...
	ТекстСостоянияОчистки = НСтр("ru = 'Очистка адресного классификатора ...';
								|en = 'Clearing the address classifier …'");
...
Процедура ПослеФоновойОчистки(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекстОписанияОчистки = НСтр("ru = 'Адресный классификатор успешно очищен.';
								|en = 'Address classifier cleared successfully.'");
