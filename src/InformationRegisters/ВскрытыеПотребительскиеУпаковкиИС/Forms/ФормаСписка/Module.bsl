...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИмяПодсистемы = "ИСМП" Тогда
		...
		МодульОбщегоНазначенияИСМП.ЗаполнитьСписокВыбораДальнейшееДействие(
		...
		Если Организации.Количество() <> 1 Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Для сканирования в форме списка необходимо выбрать одну организацию.';
					|en = 'Для сканирования в форме списка необходимо выбрать одну организацию.'"),,
...
Процедура ОформленоОрганизацииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбщегоНазначенияИСМПКлиент.ОткрытьФормуВыбораОрганизаций(ЭтотОбъект, "Оформлено",, ВыборОрганизацииЗавершение);
...
Процедура ВыполнитьОбмен(Команда)
	...
	ОбменДаннымиИСМПКлиент.ВыполнитьОбмен(ЭтотОбъект, ОбменДаннымиИСМПКлиент.ОрганизацииДляОбмена(ЭтотОбъект));
...
Процедура АрхивироватьДокументы(Команда)
	...
	ОбщегоНазначенияИСКлиент.АрхивироватьДокументы(ЭтотОбъект, ДокументыКАрхивированию, ОбщегоНазначенияИСМПКлиент);
...
Процедура ЗаполнитьПараметрыПриСоздании()
	...
	Заголовок      = ОбщегоНазначенияИСПовтИсп.ПредставленияВскрытыхПотребительскихУпаковок(ИмяПодсистемы).ЗаголовокСписка;
...
Процедура УстановитьТекстЗапросаДинамическогоСписка()
	...
	Если ИмяПодсистемы = "ИСМП" Или Не ОбщегоНазначенияИС.ЭтоРасширеннаяВерсияГосИС() Тогда
		Список.ТекстЗапроса = "
		|	ВЫБРАТЬ
		...
		|	ЛЕВОЕ СОЕДИНЕНИЕ &ПутьКИсточникуДанныхШтрихкода КАК ШтрихкодыУпаковок
	...
	Иначе
		...
		|	ПО ОписаниеНоменклатурыИС.Номенклатура = &ИмяПоляНоменклатураШтрихкода
...
Процедура НастроитьЭлементыФормыПриСоздании()
	...
	Элементы.Характеристика.Видимость = ОбщегоНазначенияИС.ХарактеристикиИспользуются();
	Элементы.Серия.Видимость          = ОбщегоНазначенияИС.СерииИспользуются();
	
	Элементы.ПередатьДанные.Видимость                           = ИспользуетсяПередачаДанных И ИспользуетсяИнтерактивнаяПередачаДанных;
	Элементы.СписокКонтекстноеМенюПередатьДанные.Видимость      = ИспользуетсяПередачаДанных И ИспользуетсяИнтерактивнаяПередачаДанных;
	Элементы.СписокВыполнитьОбмен.Видимость                     = ИспользуетсяПередачаДанных И ИспользуетсяИнтерактивнаяПередачаДанных;
	...
	Элементы.СписокПроверкаКодовМаркировки.Видимость     = ОбщегоНазначенияИС.ЭтоРасширеннаяВерсияГосИС();
...
Процедура УстановитьУсловноеОформление()
	...
	Если ЭтоИСМП Тогда
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Передайте отчет о подключении';
																	|en = 'Передайте отчет о подключении'"));
...
Процедура ДействиеКомандыОтключить()
	...
	ПоказатьВопрос(
		...
		СтрШаблон(НСтр("ru = 'Отключить коды маркировки по выделенными строкам (%1)?';
						|en = 'Отключить коды маркировки по выделенными строкам (%1)?'"), КоличествоВыделенныхСтрок),
		КнопкиДиалога,,
		"Отключить");
...
Процедура УстановитьБыстрыйОтборСервер()
	...
	Если СтруктураБыстрогоОтбора <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Организации) Тогда
			ОбщегоНазначенияИСМПКлиентСервер.УстновитьОтборПоОрганизацииИПустомуЗначению(ЭтотОбъект);
		...
		Если ОбщегоНазначенияИС.ЭтоРасширеннаяВерсияГосИС() Тогда
			...
			МодульИнтеграцияИС.ОтборПоЗначениюСпискаПриСозданииНаСервере(Список, "Ответственный", Ответственный, СтруктураБыстрогоОтбора);
		КонецЕсли;
...
Процедура Подключаемый_ОткрытьФормуУточненияДанных()
	...
	Если ОбщегоНазначенияИСКлиент.ЭтоРасширеннаяВерсияГосИС() Тогда
...
Процедура ОбработатьШтрихкодКодаМаркировки(РезультатВыбораДанныеШтрихкода, ДополнительныеПараметры)
	...
	Если ОбщегоНазначенияИСКлиент.ЭтоРасширеннаяВерсияГосИС() Тогда
		КодМаркировки = РезультатВыбораДанныеШтрихкода.ШтрихкодУпаковки;
	Иначе
		КодМаркировки = РезультатВыбораДанныеШтрихкода.ПолныйКодМаркировки;
	...
	ДанныеДляОбработки = ДанныеДляОбработкиКодаМаркировки(КодМаркировки);
	...
	Если ПравоИзменения Или ДанныеДляОбработки.КлючЗаписи <> Неопределено Тогда
		ОткрытьФормуВскрытияПотребительскойУпаковки(
			ДанныеДляОбработки.КодМаркировки,
			РезультатВыбораДанныеШтрихкода.Номенклатура,
			РезультатВыбораДанныеШтрихкода.Характеристика,
			РезультатВыбораДанныеШтрихкода.Серия);
	ИначеЕсли Не ПравоИзменения И ДанныеДляОбработки.КлючЗаписи = Неопределено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Отсутсвуют права доступа для вскрытия потребительской упаковки. Обратитесь к администратору.';
				|en = 'Отсутсвуют права доступа для вскрытия потребительской упаковки. Обратитесь к администратору.'"));
	КонецЕсли;
...
&НаКлиенте
Процедура ОткрытьФормуВскрытияПотребительскойУпаковки(КодМаркировки = Неопределено, Номенклатура = Неопределено, Характеристика = Неопределено, Серия = Неопределено)
	...
	ПараметрыОткрытия.Организация         = ПерваяВыбраннаяОрганизация(ЭтотОбъект);
	...
	Если Не ОбщегоНазначенияИСКлиент.ЭтоРасширеннаяВерсияГосИС() Тогда
		ПараметрыОткрытия.Номенклатура   = Номенклатура;
		ПараметрыОткрытия.Характеристика = Характеристика;
		ПараметрыОткрытия.Серия          = Серия;
	КонецЕсли;
