...
Функция МестаУчета(Операция, Склад, Подразделение, Партнер, Договор = Неопределено) Экспорт
	...
	ИспользоватьДоговор = 
		...
		//++ НЕ УТ
		Или (Операция = Перечисления.ХозяйственныеОперации.ВводОстатковМатериаловПереданныхПереработчикам2_5)
		Или (Операция = Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчика2_5)
		Или (Операция = Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчикаВСтранахЕАЭС2_5)
		Или (Операция = Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5)
		Или (Операция = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5)
		Или (Операция = Перечисления.ХозяйственныеОперации.ВозвратОтПереработчика2_5)
		Или (Операция = Перечисления.ХозяйственныеОперации.СписаниеТоваровУПереработчика)
		Или (Операция = Перечисления.ХозяйственныеОперации.ВыкупТоваровПереработчиком)
		Или (Операция = Перечисления.ХозяйственныеОперации.ПересортицаТоваровУПереработчика)
		Или (Операция = Перечисления.ХозяйственныеОперации.ПорчаТоваровУПереработчика)
		//++ НЕ УТКА
		Или Операция = Перечисления.ХозяйственныеОперации.ВыкупТоваровДавальца
		Или Операция = Перечисления.ХозяйственныеОперации.СписаниеТоваровДавальцаЗаСчетДавальца
		Или Операция = Перечисления.ХозяйственныеОперации.СписаниеТоваровДавальцаНаРасходы
		//-- НЕ УТКА
...
Функция ПолучитьНаборЗаписей(ПараметрыАналитики)
	...
	Если Не ЗначениеЗаполнено(СтруктураАналитики.Номенклатура) Тогда
		Шаблон1 = НСтр("ru = 'Ошибка при получении набора регистра сведений %Парам1%: не указана номенклатура.';
						|en = 'An error occurred while retrieving a set of data from information register %Парам1%: no products are specified.'");
		...
		Если ТипыНоменклатурыБезКонтроляМестаХранения.Найти(РеквизитыНоменклатуры.ТипНоменклатуры) = Неопределено Тогда
			...
			Если Не ЗначениеЗаполнено(СтруктураАналитики.МестоХранения) Тогда
				Шаблон1 = НСтр("ru = 'Ошибка при получении набора регистра сведений %Парам1%: указана номенклатура с типом, отличным от ""Услуга"" или ""Работа"", а место хранения не заполнено.';
								|en = 'An error occurred while retrieving a set of data from information register %Парам1%: items with type other than “Service” or “Work” are found but the storage location is not specified.'");
				Шаблон1 = СтрЗаменить(Шаблон1, "%Парам1%", "АналитикаУчетаНоменклатуры");
		...
		Если РеквизитыНоменклатуры.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.МногооборотнаяТара Тогда 
			...
			Если ЗначениеЗаполнено(СтруктураАналитики.Назначение)
				...
				Шаблон1 = НСтр("ru = 'Ошибка при получении набора регистра сведений %Парам1%: указана номенклатура с типом ""Многооборотная тара"", и указано назначение.';
								|en = 'An error occurred while retrieving a set of data from information register %Парам1%: items with the ""Reusable packaging"" type are specified but intended use is indicated.'");
				Шаблон1 = СтрЗаменить(Шаблон1, "%Парам1%", "АналитикаУчетаНоменклатуры");
		...
		Если Не ЗначениеЗаполнено(РеквизитыНоменклатуры.ИспользованиеХарактеристик)
			...
			Если ЗначениеЗаполнено(СтруктураАналитики.Характеристика) Тогда
				Шаблон1 = НСтр("ru = 'Ошибка при получении набора регистра сведений %Парам1%: для номенклатуры не настроено ведение характеристики, но характеристика указана.';
								|en = 'Ошибка при получении набора регистра сведений %Парам1%: для номенклатуры не настроено ведение характеристики, но характеристика указана.'");
				Шаблон1 = СтрЗаменить(Шаблон1, "%Парам1%", "АналитикаУчетаНоменклатуры");
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(СтруктураАналитики.Характеристика) Тогда
				Шаблон1 = НСтр("ru = 'Ошибка при получении набора регистра сведений %Парам1%: для номенклатуры настроено ведение характеристики, но характеристика не указана.';
								|en = 'Ошибка при получении набора регистра сведений %Парам1%: для номенклатуры настроено ведение характеристики, но характеристика не указана.'");
				Шаблон1 = СтрЗаменить(Шаблон1, "%Парам1%", "АналитикаУчетаНоменклатуры");
	...
	Если ЗначениеЗаполнено(СтруктураАналитики.Серия) Тогда
		...
		Если ТипЗнч(СтруктураАналитики.МестоХранения) <> Тип("СправочникСсылка.Склады")
			...
			Шаблон1 = НСтр("ru = 'Ошибка при получении набора регистра сведений %Парам1%: серия указана, но место хранения не того типа.';
							|en = 'An error occurred while retrieving a set of data from information register %Парам1%: batch is specified but the storage location is of a wrong type.'");
			Шаблон1 = СтрЗаменить(Шаблон1, "%Парам1%", "АналитикаУчетаНоменклатуры");
		...
		Если РеквизитыНоменклатуры.ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Товар Тогда
			Шаблон1 = НСтр("ru = 'Ошибка при получении набора регистра сведений %Парам1%: серия указана, но номенклатура не с типом ""Товар"".';
							|en = 'An error occurred while retrieving a set of data from information register %Парам1% batch is specified but the item type is not ""Goods"".'");
			Шаблон1 = СтрЗаменить(Шаблон1, "%Парам1%", "АналитикаУчетаНоменклатуры");
	...
	Если ЗначениеЗаполнено(СтруктураАналитики.СтатьяКалькуляции) Тогда
		Если ТипЗнч(СтруктураАналитики.МестоХранения) <> Тип("СправочникСсылка.СтруктураПредприятия")
			...
			Шаблон1 = НСтр("ru = 'Ошибка при получении набора регистра сведений %Парам1%: %Парам2% указана, но место хранения не того типа.';
							|en = 'An error occurred while retrieving a set of data from information register %Парам1%: %Парам2% is specified but the storage location is of a wrong type.'");
			Шаблон1 = СтрЗаменить(Шаблон1, "%Парам1%", "АналитикаУчетаНоменклатуры");
...
Функция ТекстЗначенияКлючейАналитикиВКоллекции(ИменаПолей)
	...
	Если ЗначениеЗаполнено(ИменаПолей.СтатьяКалькуляции) Тогда
		...
		Иначе
			СтрокаЗамены = """""";
			//++ НЕ УТ
			СтрокаЗамены = "Коллекция." + ИменаПолей.СтатьяКалькуляции;
			//-- НЕ УТ
	...
	Иначе
		СтрокаЗамены = """""";
		//++ НЕ УТ
		СтрокаЗамены = "ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)";
		//-- НЕ УТ
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолеСтатьяКалькуляции", СтрокаЗамены);
	КонецЕсли;
