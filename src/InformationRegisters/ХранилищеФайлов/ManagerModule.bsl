...
Процедура ПеренестиДанные(СообщатьПрогресс = Ложь, АдресРезультата = Неопределено) Экспорт
	...
	ШаблонПрогресса = НСтр("ru = 'Обработано %1 (%2 Мб) файлов из %3 (%4 Мб)';
							|en = '%1 (%2 MB) files out of %3 (%4MB) are processed'");
	ВсегоЗаписей = 0;
	ВсегоРазмерМб = 0;
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ИначеЕсли ТипЗнч(ДвоичныеДанные) <> Тип("ДвоичныеДанные") Тогда
				...
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Ошибка дедупликации файла.';
												|en = 'Files.File deduplication error.'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|%2';
				|en = 'Cannot process binary data of the %1 file. Reason:
				|%2'"), 
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать некоторые двоичные данные файла (пропущены): %1';
				|en = 'Cannot process (skipped) some binary data of the file: %1'"), 
			ПроблемныхОбъектов);
		...
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Обработана очередная порция двоичных данных файлов: %1';
					|en = 'Another batch of binary data of files is processed: %1'"),
				ОбъектовОбработано));
	КонецЕсли;
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать некоторые файлы (пропущены): %1';
				|en = 'Cannot process (skipped) some files: %1'"), 
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
