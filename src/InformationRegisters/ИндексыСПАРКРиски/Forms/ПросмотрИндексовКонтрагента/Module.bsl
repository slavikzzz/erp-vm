...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ru = '1СПАРК Риски:';
					|en = '1CPARK Risks:'") + " " + Строка(ИндексыСПАРКРиски.Контрагент);
	...
	КлючСохраненияПоложенияОкна = Строка(Элементы.ДекорацияСтатусИСобытие.Видимость)
		+ Строка(Элементы.ГруппаСводныйИндикатор.Видимость)
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "СПАРКРиски:ИзмененоСостояниеФоновогоЗадания" И Параметр.Статус = "Выполнено" Тогда
		ОтобразитьРезультатыФоновогоЗадания(Параметр.АдресХранилищаФоновогоЗадания);
	КонецЕсли;
...
&НаСервере
Процедура ОтобразитьРезультатыФоновогоЗадания(Знач АдресХранилища)
	...
	Если СтрокаРезультат <> Неопределено Тогда
		...
		ИндексыСПАРКРиски.Вставить(
			"СостояниеВыводаДанных",
			ПредопределенноеЗначение("Перечисление.СостоянияВыводаИндексовСПАРКРиски.ПолученоИзФоновогоЗадания"));
		ИндексыСПАРКРиски.Вставить(
			"СостояниеЗагрузкиДанных",
			ПредопределенноеЗначение("Перечисление.СостоянияЗагрузкиИндексовСПАРКРиски.ПустаяСсылка"));
...
Процедура ОтобразитьЗначенияИндексов(РезультатИндексыКонтрагента)
	...
	Элементы.ДекорацияСтатусИСобытие.ЦветТекста = ЦветаСтилей.ЦветОсобогоТекста;
	Элементы.ГруппаСводныйИндикатор.Видимость   = Ложь;
	...
	Если Не ЗначениеЗаполнено(ВидОшибки) Тогда
		...
		Если (РезультатИндексыКонтрагента.СводныйИндикатор >= 1)
			...
			Элементы.ДекорацияСводныйИндикаторЗначение.Заголовок = Новый ФорматированнаяСтрока(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1';
						|en = '%1'"),
	...
	Иначе
		...
		Элементы.ДекорацияОписаниеОшибки.Заголовок =
			СтроковыеФункции.ФорматированнаяСтрока(
				...
				|
				|<a href=""SPARK:About"">Подробнее о сервисе</a>';
				|en = 'Counterparty reliability evaluation.
				|
				|<a href=""SPARK:About"">More about the service</a>'"));
