...
Процедура ДобавитьПакетОшибкуДанныхВОчередь(ОфлайнОборудование, ИдентификаторПередачи, ТекстОшибки) Экспорт
	...
	Исключение
		...
		ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Добавление пакета ошибки данных в очередь';
				|en = 'Add a data error package to the queue'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()), 
...
Процедура ДобавитьПакетДанныхВОчередь(ОфлайнОборудование, ИдентификаторПередачи, ТекстСообщения, ПорядковыйНомер = 1, ПакетовВсего = 1) Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Добавление пакета данных в очередь.';
				|en = 'Add a data package to the queue.'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()), 
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		ВызватьИсключение(
			СтрШаблон(
				НСтр("ru = 'Не удалось выполнить отправку данных. Очередь сообщений обмена уже содержит сообщение с номером %1.';
					|en = 'Cannot send data. The exchange message queue already contains a message with number %1.'"),
...
Процедура ОчиститьОчередьСообщений(ОфлайнОборудование) Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Очистка очереди сообщений.';
				|en = 'Clear the message queue.'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()), 
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура СброситьФлагОтправкиВОчередиОбмена(ОфлайнОборудование, ИдентификаторПередачи)
	...
	Исключение
		...
		ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Сброс флага отправки в очереди обмена';
				|en = 'Reset the sending check box in the exchange queue'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()), 
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
