...
Функция СведенияОБлокировкеОтИзменений(Организации, Период = Неопределено, ОбновлениеИБ = Ложь) Экспорт
	...
	Если Результат.ИспользоватьМеханизмБлокировкиОтИзменений Тогда
		...
		// Сформируем временную таблицу организаций НА УСН
		Запрос.УстановитьПараметр("МаксимальныйПериод",  КонецГода(КонецГода(ТекущаяДатаСеанса()) + 1));
		Запрос.УстановитьПараметр("ОперацииУСН",  Перечисления.ОперацииЗакрытияМесяца.ОперацииУСН());
		
		Запрос.Текст =
		"ВЫБРАТЬ
		|	Т.Ссылка КАК Организация,
		|	ЕСТЬNULL(УчетныеПолитики.Период, ДАТАВРЕМЯ(1, 1, 1)) КАК Период,
		|	ВЫБОР
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиСистемыНалогообложения КАК УчетныеПолитики
		|		ПО Т.Ссылка = УчетныеПолитики.Организация
		|ГДЕ
		|	Т.Ссылка В(&Организации)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация
		|;
		...
		|	НАЧАЛОПЕРИОДА(МИНИМУМ(Т.НачалоПериода), МЕСЯЦ) КАК НачалоПериода,
		|	КОНЕЦПЕРИОДА(Т.КонецПериода, МЕСЯЦ) КАК КонецПериода
		|ПОМЕСТИТЬ ВТОрганизацииУСН
		|ИЗ
		|	(ВЫБРАТЬ
		|		Организации.Организация КАК Организация,
		|		Организации.Период КАК НачалоПериода,
		|		МИНИМУМ(ЕСТЬNULL(ДОБАВИТЬКДАТЕ(ПериодыПолитик.Период, СЕКУНДА, -1), &МаксимальныйПериод)) КАК КонецПериода
		|	ИЗ
		|		ВТУчетныеПолитики КАК Организации
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВТУчетныеПолитики КАК ПериодыПолитик
		|			ПО Организации.Организация = ПериодыПолитик.Организация
		|				И Организации.Период < ПериодыПолитик.Период
		|				И (ПериодыПолитик.ОСН)
		|	ГДЕ
		|		НЕ Организации.ОСН
		|	
		|	СГРУППИРОВАТЬ ПО
		|		Организации.Организация,
		|		Организации.Период) КАК Т
		|
		|СГРУППИРОВАТЬ ПО
		|	Т.Организация,
		|	Т.КонецПериода
		...
		Для Каждого КлючИЗначение Из РегистрыЗаданий Цикл
			...
			Если МетаРегистр = Метаданные.РегистрыСведений.ЗаданияКЗакрытиюМесяца Тогда
				...
				ТекстСоединение =
					"ЛЕВОЕ СОЕДИНЕНИЕ ВТОрганизацииУСН КАК ОрганизацииУСН
					|		ПО Т.Организация = ОрганизацииУСН.Организация
					|		И Т.Месяц МЕЖДУ ОрганизацииУСН.НачалоПериода И ОрганизацииУСН.КонецПериода";
				ДополнительныеУсловия = "НЕ Т.Операция В (&НеиспользуемыеОперацииЗакрытияМесяца)
					// Организация на УСН - доступны все операции
					|	И (НЕ ОрганизацииУСН.Организация ЕСТЬ NULL
					// Организация на ОСН - игнорируем операции для УСН
					|		ИЛИ НЕ Т.Операция В (&ОперацииУСН))
					|";
				//-- НЕ УТ
			...
			//++ Локализация
			ИначеЕсли МетаРегистр = Метаданные.РегистрыСведений.ОтражениеДокументовВРеглУчете Тогда

				ДополнительныеУсловия = "Т.Статус В (
			...
			//++ НЕ УТКА
			ИначеЕсли МетаРегистр = Метаданные.РегистрыСведений.ОтражениеДокументовВМеждународномУчете Тогда
				...
				ДополнительныеУсловия = "Т.Статус В (
					...
					|		ЗНАЧЕНИЕ(Перечисление.СтатусыОтраженияВМеждународномУчете.ОжидаетсяОтражениеВРеглУчете),
	...
	Если ПериодЗаблокирован Тогда
		 ...
		 ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		 	НСтр("ru = 'Невозможен расчет заблокированного период %1 по организациям %2';
					|en = 'Cannot calculate the locked period %1 by the %2 companies'"),
