			...
			Если ТипХраненияФайла = Неопределено Тогда
				ПлохиеДанные[Строка.Владелец] = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В регистре Сведения о файлах содержится ссылка на несуществующий файл ""%1"".';
						|en = 'The ""File properties"" information register references a non-existent file: ""%1"".'"),
					Строка.Файл);
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать некоторые сведения о файлах (пропущены): %1';
				|en = 'Couldn''t process (skipped) some information record about files: %1'"), 
			ПроблемныхОбъектов);
		...
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Обработана очередная порция сведений о файлах: %1';
					|en = 'Yet another batch of information records about files is processed: %1'"),
				ОбъектовОбработано));
	КонецЕсли;
