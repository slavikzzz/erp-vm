...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстЗапроса = РегламентированнаяОтчетностьАЛКО.ТекстЗапросаВыбораКонтрагентаАЛКО(
														ОсновнаяТаблица, НужныЛицензии, ТипАлкоголя);
...
&НаКлиенте
Процедура П000010000306ПриИзменении(Элемент)
	ТаблицаПолучателейВыборНаСервере(Неопределено, Запись.П000010000306, "");
КонецПроцедуры
...
Процедура ОбработкаМодифицированности(НачальноеЗначениеПолей, СтруктураМодифицированности)
	...
	Иначе
		...
		ОбъектОтчета(ИмяФормыВладельца).ОбработкаЗаписи(ИмяРегистра, Запись, , ДатаПериодаОтчета, ЕстьСтранаРегистрации);		
...
Процедура ТаблицаПолучателейВыборНаСервере(НаименованиеПолное, ИНН, КПП)
	...
	ОбщаяСтрокаДоВыбора = СокрЛП(Запись.П000010000305) + СокрЛП(Запись.П000010000306) + СокрЛП(Запись.П000010000307);
	...
	Если ОбщаяСтрокаДоВыбора <> ОбщаяСтрокаПослеВыбора Тогда
		...
		Если НЕ СокрЛП(Запись.П000010000305) = СокрЛП(НаименованиеПолное) Тогда
			
				Запись.П000010000305 = СокрЛП(НаименованиеПолное);
				Запись.П0000100003051 = "";	
