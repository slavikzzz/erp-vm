...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Элементы.СписокДоверенность.Заголовок = НСтр("ru = 'Номер доверенности';
													|en = 'Номер доверенности'");
		Элементы.СписокДоверенность.КнопкаОчистки = Ложь;
	КонецЕсли;
	МассивТипов.Добавить(Тип("Строка"));
	Элементы.Доверенность.ОграничениеТипа = Новый ОписаниеТипов(МассивТипов);
	Элементы.СписокДоверенность.ОграничениеТипа = Новый ОписаниеТипов(МассивТипов);
	...
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.ДобавитьОрганизациюВЗаголовок(
		...
		НСтр("ru = 'Настройки отправки отчетности в Федеральную таможенную службу по пользователям';
			|en = 'Настройки отправки отчетности в Федеральную таможенную службу по пользователям'"));
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле <> Неопределено И (Поле.Имя = "СписокСертификатАбонентаПредставление"
		ИЛИ Поле.Имя = "СписокДоверенность" ИЛИ Поле.Имя = "СписокЛогин" ИЛИ Поле.Имя = "СписокПароль") Тогда
...
Процедура ОбновитьДоступностьЭлементов()
	...
	Элементы.Логин.Видимость 							= (ИспользоватьНесколько <> 1);
	Элементы.Пароль.Видимость 							= (ИспользоватьНесколько <> 1);
...
Процедура ОбновитьНастройкиОбменаПользователейПослеПоискаСертификатов(Результат, ДополнительныеПараметры) Экспорт
	...
	Для НомерОбхода = 1 По 2 Цикл
		...
		Для каждого НастройкаОбменаПользователей Из НастройкиОбменаПользователей Цикл
			...
			СтрокаСписка.Логин 								= НастройкаОбменаПользователей.Логин;
			СтрокаСписка.Пароль 							= НастройкаОбменаПользователей.Пароль;
...
Процедура ЗаполнитьНастройкиОбменаПользователей()
	...
	Запрос.Текст =
		...
		|	НастройкиОбменаФТС.Доверенность КАК Доверенность,
		...
		|ГДЕ
		|	НастройкиОбменаФТС.Организация = &Организация
		|	И НастройкиОбменаФТС.Пользователь = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
	...
	Для каждого СтрокаТаблицы Из ТаблицаНастроек Цикл
		...
		Иначе
			...
			СтрокаНастроек.Логин 						= СтрокаТаблицы.Логин;
			СтрокаНастроек.Пароль 						= СтрокаТаблицы.Пароль;
...
Процедура СохранитьНаСервере()
	...
	РегистрыСведений.НастройкиОбменаФТС.СохранитьСписокПользователей(
		ОрганизацияСсылка,
		Список,
