...
//
// Параметры:
//  ПараметрыОбновленияСтатуса - Структура - см. функцию ОбменДаннымиИСМПКлиентСервер.ПараметрыОбновленияСтатуса().
// 
// Возвращаемое значение:
...
Функция РассчитатьСтатусы(ДокументСсылка, СтатусОбработки, Статусы) Экспорт
	...
	ИначеЕсли СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаОбрабатывается
		...
		Или СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийИСМП.ЗаявкаОжидаетРегистрациюУчастникаГИСМТ
		Или ЭтоСтатусОбработкиСУЗПринятОбрабатывается(СтатусОбработки) Тогда
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Неизвестный статус обработки: %1';
										|en = 'Неизвестный статус обработки: %1'"), СтатусОбработки);
...
Функция РассчитатьСтатусыКПередаче(ДокументСсылка, НовыйСтатус) Экспорт
	...
	Если ОбщегоНазначенияИСМП.ЭтоРасширеннаяВерсияГосИС()
		И ПолучитьФункциональнуюОпцию("ИспользоватьАвтоматическуюОтправкуПолучениеДанныхИСМП") Тогда
...
Функция Архивировать(ДокументыКАрхивированию) Экспорт
		...
		Если Не ПравоДоступа("Изменение", МетаданныеДокумента) Тогда
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Недостаточно прав доступа для очистки дальнейших действий по документу %1';
					|en = 'Недостаточно прав доступа для очистки дальнейших действий по документу %1'"),
				ДокументСсылка);
		КонецЕсли;
	...
	СписокЗапросов.Добавить(
		"ВЫБРАТЬ
		|	РазрешениеНаОтгрузкуИСМП.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.РазрешениеНаОтгрузкуИСМП КАК РазрешениеНаОтгрузкуИСМП
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыДокументовИСМП КАК Статусы
		|	ПО Статусы.Документ = РазрешениеНаОтгрузкуИСМП.Ссылка
		|	И Статусы.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОбработкиРазрешенийНаОтгрузкуИСМП.ОтменитеРазрешениеОшибка)
		|ГДЕ
		|	РазрешениеНаОтгрузкуИСМП.Ссылка В (&Документы)",
		"РазрешенияНаОтгрузкуКПересчетуСтатусовОформления");
	...
	//@skip-warning
	РазрешенияКПересчетуСтатусов = РезультатЗапроса["РазрешенияНаОтгрузкуКПересчетуСтатусовОформления"].Выгрузить().ВыгрузитьКолонку("Ссылка");
	...
	Для Каждого ДокументСсылка Из ДокументыКАрхивированию Цикл
		...
		ОтменититьРазрешениеНаОтгрузку = (РазрешенияКПересчетуСтатусов.Найти(ДокументСсылка) <> Неопределено);
		
		НачатьТранзакцию();
		
		Попытка
			
			Если ОтменититьРазрешениеНаОтгрузку Тогда
				ЗначенияЗаписи.Статус = Перечисления.СтатусыОбработкиРазрешенийНаОтгрузкуИСМП.РазрешениеОтменено;
			КонецЕсли;
			...
			Если ОтменититьРазрешениеНаОтгрузку Тогда
				МодульРасчетСтатусовОформленияИСМП = ОбщегоНазначения.ОбщийМодуль("РасчетСтатусовОформленияИСМП");
				МодульРасчетСтатусовОформленияИСМП.РассчитатьСтатусыОформленияДокументов(
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'При архивировании документа %1 возникла ошибка:
				           |Текст ошибки: %2';
				           |en = 'При архивировании документа %1 возникла ошибка:
				           |Текст ошибки: %2'"),
				ДокументСсылка,
...
Функция ЭтоСтатусОбработкиСУЗПринятОбрабатывается(СтатусОбработки)
	
	Если ОбщегоНазначенияИСМП.ЭтоРасширеннаяВерсияГосИС() Тогда
		Возврат (СтатусОбработки = Перечисления["СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП"]["СУЗПринятКОбработке"]
