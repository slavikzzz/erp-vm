	...
	Для Каждого Запись Из ЭтотОбъект Цикл
		Если Запись.Валюта = Запись.БазоваяВалюта
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					|en = 'The same value %1 is selected in the currency and base currency fields.
					|Specify ""1"" in the Rate (Numerator) and Rate (Denominator) fields'"),
					Запись.Валюта);
	...
	Исключение
		...
		Если ОшибкаРасчетаКурсаПоФормуле[Валюта] = Неопределено Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Расчет курса валюты ""%1"" по формуле ""%2"" за период ""%3""не выполнен:';
																						|en = 'Cannot calculate the ""%1"" exchange rate using formula ""%2"" for the ""%3"" period:'",
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка курсов валют ЕЦБ';
