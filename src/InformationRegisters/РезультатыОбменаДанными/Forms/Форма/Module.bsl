...
Процедура ПринятьВерсиюНепринятые(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Принять версию, несмотря на запрет загрузки?';
											|en = 'Do you want to accept the version even though import is restricted?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура ОбновитьЗаголовкиСтраниц()
	...
	УстановитьЗаголовокСтраницы(Элементы.СтраницаНепроведенныеДокументы, НСтр("ru = 'Непроведенные документы';
																				|en = 'Unposted documents'"), КоличествоНепроведенныхДокументов());
	УстановитьЗаголовокСтраницы(Элементы.СтраницаНезаполненныеРеквизиты, НСтр("ru = 'Незаполненные реквизиты';
																				|en = 'Blank attributes'"), КоличествоНезаполненныхРеквизитов());
	УстановитьЗаголовокСтраницы(Элементы.СтраницаОшибкиXDTO,             НСтр("ru = 'Ошибки проверки конвертируемых объектов';
																				|en = 'An error occurred when checking the objects being converted'"), КоличествоОшибокXDTO());	
	
	Если ИспользуетсяВерсионирование Тогда
		УстановитьЗаголовокСтраницы(Элементы.СтраницаКоллизии, НСтр("ru = 'Конфликты';
																	|en = 'Conflicts'"), КоличествоКоллизий());
		УстановитьЗаголовокСтраницы(Элементы.СтраницаНепринятыеПоДатеЗапрета, НСтр("ru = 'Непринятые по дате запрета';
																					|en = 'Items rejected due to restriction date'"), КоличествоНепринятых());
	КонецЕсли;
