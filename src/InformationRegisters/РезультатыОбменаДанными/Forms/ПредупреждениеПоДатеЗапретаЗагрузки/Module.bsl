...
Процедура ПринятьВерсиюНепринятые(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Принять версию, несмотря на запрет загрузки?';
						|en = 'Do you want to accept the version even though import is restricted?'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура ПоказатьОтличия(Элемент)
	...
	Если ВерсияЭтойПрограммы = 0
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Для сравнения должно быть две версии объекта.';
														|en = 'There must be two object versions for comparison.'"), ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		Возврат;
...
Процедура ПринятьОтклонитьВерсиюНаСервере(СообщениеОбОшибке)
	...
	Исключение
		...
		ШаблонТекста			= НСтр("ru = 'Не удалось принять версию объекта ""%1"" по причине:%2 %3.';
										|en = 'Cannot accept the object version ""%1"" due to:%2 %3.'", ОбщегоНазначения.КодОсновногоЯзыка());
