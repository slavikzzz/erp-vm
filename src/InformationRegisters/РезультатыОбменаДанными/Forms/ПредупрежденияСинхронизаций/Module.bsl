...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Список.Параметры.УстановитьЗначениеПараметра("НепроведенныйДокумент", НСтр("ru = 'Непроведенный документ';
																				|en = 'Unposted document'", ОбщегоНазначения.КодОсновногоЯзыка()));
	Список.Параметры.УстановитьЗначениеПараметра("ПустыеРеквизиты", НСтр("ru = 'Пустые реквизиты';
																		|en = 'Empty attributes'", ОбщегоНазначения.КодОсновногоЯзыка()));
	Список.Параметры.УстановитьЗначениеПараметра("ПроверкаПередОтправкой", НСтр("ru = 'Проверка перед отправкой';
																				|en = 'Check before sending'", ОбщегоНазначения.КодОсновногоЯзыка()));
	Список.Параметры.УстановитьЗначениеПараметра("ТехническаяОшибкаОтправка", НСтр("ru = 'Тех. ошибка (отправка)';
																					|en = 'Technical error (sending)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	Список.Параметры.УстановитьЗначениеПараметра("ТехническаяОшибкаПолучение", НСтр("ru = 'Тех. ошибка (получение)';
																					|en = 'Technical error (receipt)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	Если ПодсистемаВерсионированияСуществует Тогда
		
		Список.Параметры.УстановитьЗначениеПараметра("НепринятыйСуществующий", НСтр("ru = 'Непринятый, существующий';
																					|en = 'Rejected, existing'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Список.Параметры.УстановитьЗначениеПараметра("НепринятыйНовый", НСтр("ru = 'Непринятый, новый';
																			|en = 'Rejected, new'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Список.Параметры.УстановитьЗначениеПараметра("ПринятыеДанныеКоллизии", НСтр("ru = 'Приняты данные коллизии';
																					|en = 'Received conflict data'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Список.Параметры.УстановитьЗначениеПараметра("НепринятыеДанныеКоллизии", НСтр("ru = 'Неприняты данные коллизии';
																						|en = 'Rejected conflict data'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ЖурналРегистрации(Команда)
	...
	УровниПредупреждений.Добавить(НСтр("ru = 'Ошибка';
										|en = 'Error'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()));
	УровниПредупреждений.Добавить(НСтр("ru = 'Предупреждение';
										|en = 'Warning'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()));
...
Процедура ЖурналРегистрацииВУказанноеВремя(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ДанныеТекущейСтроки.ДатаВозникновения) Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'В текущей строке не указано время и дата возникновения предупреждения';
									|en = 'The current line does not contain the time and warning date specified'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Функция ТекущиеДанныеСпискаПредупреждений()
	...
	Если ДанныеТекущейСтроки = Неопределено
		...
		ТекстИсключения = НСтр("ru = 'Для текущей строки выполнение команды не предусмотрено';
								|en = 'No command execution is provided for the current line'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура ОткрытьОбъектТекущейСтроки(ДанныеТекущейСтроки)
	...
	Если НЕ ЗначениеЗаполнено(ДанныеТекущейСтроки.ПроблемныйОбъект) Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Предупреждение в текущей строке не привязано к объекту информационной базы';
									|en = 'Warning in the current line is not linked to an infobase object'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
		Возврат;
...
Процедура ПредставлениеОтбораПоПериоду()
	...
	Если НЕ ЗначениеЗаполнено(КэшЗначений.ОтборПоДатеВозникновения) Тогда
		...
		ПредставлениеОтбораПоПериоду = НСтр("ru = 'Отбор не установлен';
											|en = 'Filter not set'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура ПредставлениеОтбораУзловОбмена()
	...
	Если КэшЗначений.ОтборУзловПланаОбмена = Неопределено
		...
		ПредставлениеОтбораСинхронизаций = НСтр("ru = 'Отбор не установлен';
												|en = 'Filter not set'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
	...
	Иначе
		...
		ШаблонТекста = НСтр("ru = '%1%2 (и еще %3 шт.)';
							|en = '%1%2 (and %3 more pcs)'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура ПредставлениеОтбораПоТипамПредупреждений()
	...
	Если КэшЗначений.ОтборТипыПредупреждений = Неопределено
		...
		ПредставлениеОтбораТиповПредупреждений = НСтр("ru = 'Отбор не установлен';
														|en = 'Filter not set'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
	...
	ИначеЕсли КэшЗначений.ОтборТипыПредупреждений.Количество() > 8 Тогда
		...
		ПредставлениеОтбораТиповПредупреждений = НСтр("ru = 'Все типы предупреждений';
														|en = 'All warning types'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура ОтборПоДатеВозникновения()
	...
	ПредставлениеГруппыОтбора = НСтр("ru = 'Отбор по дате возникновения';
									|en = 'Filter by issue date'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	ПараметрыЭлементаОтбора.Вставить("ПредставлениеОтбора", НСтр("ru = 'Отбор даты с';
																|en = 'Filter date from'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыЭлементаОтбора.ПредставлениеОтбора = НСтр("ru = 'Отбор даты по';
														|en = 'Filter date to'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОтборПоУзламИнформационнойБазы()
	...
	ПараметрыЭлементаОтбора.Вставить("ПредставлениеОтбора", НСтр("ru = 'Отбор синхронизаций';
																|en = 'Synchronization filter'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ПараметрыЭлементаОтбора.Вставить("Использование", Использование);
...
Процедура ОтборПоТипамПредупреждений()
	...
	ПараметрыЭлементаОтбора.Вставить("ПредставлениеОтбора", НСтр("ru = 'Отбор по типу предупреждения';
																|en = 'Filter by warning type'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ПараметрыЭлементаОтбора.Вставить("Использование", Использование);
...
Процедура ОтборСкрытыеПредупреждения()
	...
	ПараметрыЭлементаОтбора.Вставить("ПредставлениеОтбора", НСтр("ru = 'Отбор скрытых предупреждений';
																|en = 'Filter hidden warnings'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ПараметрыЭлементаОтбора.Вставить("Использование", Использование);
...
Процедура ПровестиДокументыПоВыделеннымСтрокам(ВыделенныеСтроки)
	...
	Если ДанныеВыделенныхСтрок.Количество() = 0 Тогда
		...
		СообщениеОбОшибке = НСтр("ru = 'В выделенных строках нет предупреждений с типом <Непроведенный документ>';
								|en = 'There are no warnings with the <Unposted document> type in the selected lines'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура КоллизииПоВыделеннымСтрокам(ВыделенныеСтроки)
	...
	Если ДанныеВыделенныхСтрок.Количество() = 0 Тогда
		...
		СообщениеОбОшибке = НСтр("ru = 'В выделенных строках нет предупреждений с типами <Неприняты данные коллизии>, <Приняты данные коллизии>';
								|en = 'There are no warnings with the <Rejected conflict data> and <Received conflict data> types in the selected lines'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		ПоказатьПредупреждение(Неопределено, СообщениеОбОшибке);
		Возврат
...
Процедура ЗапретЗагрузкиПоВыделеннымСтрокам(ВыделенныеСтроки)
	...
	Если ДанныеВыделенныхСтрок.Количество() = 0 Тогда
		...
		СообщениеОбОшибке = НСтр("ru = 'В выделенных строках нет предупреждений с типами <Непринятый, существующий>, <Непринятый, новый>,
			...
			|resulted from an attempt to import data to a closed period.'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		ПоказатьПредупреждение(Неопределено, СообщениеОбОшибке);
		Возврат
...
Процедура ГрупповоеИзменениеРеквизитовПоВыделеннымСтрокам(ВыделенныеСтроки)
	...
	Если ДанныеВыделенныхСтрок.Количество() = 0 Тогда
		...
		СообщениеОбОшибке = НСтр("ru = 'В выделенных строках нет предупреждений с типами <Пустые реквизиты>.';
								|en = 'There are no warnings with the <Empty attributes> types in the selected lines.'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		ПоказатьПредупреждение(Неопределено, СообщениеОбОшибке);
		Возврат
