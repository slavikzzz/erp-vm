...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("ДанныеВыделенныхСтрок") Тогда
		...
		ВызватьИсключение НСтр("ru = 'Самостоятельное использование формы не предусмотрено.';
								|en = 'The form cannot be used independently.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПринятьВерсию(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Принять версии в отмеченных строках, несмотря на запрет загрузки?';
						|en = 'Accept versions in the selected lines even though import is restricted?'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура ПринятьВерсиюЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если ПараметрыПересмотра.КоличествоПересмотренных <> ПараметрыПересмотра.КоличествоВсего Тогда
		...
		ШаблонСообщения = НСтр("ru = 'Отмечено строк: %1. Изменено объектов: %2.';
								|en = 'Lines selected: %1. Objects changed: %2.'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура ПересмотретьРезультатыНаСервере(ПараметрыПересмотра)
	...
	ШаблонТекстаОшибки = НСтр("ru = 'Не удалось изменить версию объекта по причине:%1%2';
								|en = 'Cannot change the object version due to:%1%2'", ОбщегоНазначения.КодОсновногоЯзыка());
