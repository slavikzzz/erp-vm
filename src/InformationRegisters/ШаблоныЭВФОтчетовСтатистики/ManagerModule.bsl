...
Процедура ЗагрузитьРекомендуемыеЭлектронныеПредставления(Параметры = Неопределено) Экспорт
	...
	Исключение
		СтрОш = НСтр("ru = 'Не удалось получить шаблоны для обновления';
					|en = 'Не удалось получить шаблоны для обновления'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Исключение
		СтрОш = НСтр("ru = 'Не удалось распаковать шаблоны для обновления';
					|en = 'Не удалось распаковать шаблоны для обновления'", ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(СтрОш, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
...
Процедура ДобавитьШаблоныИзКонфигурации(АдресВременногоХранилища, КоличествоНезагруженныхШаблонов)
	...
	Для Каждого ОбъектФайл Из ОбъектыФайл Цикл
		...
		Если ОбъектФайл.Существует() Тогда
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось загрузить XML-шаблон ""%1"":%2';
	...
	Попытка
		УдалитьФайлы(ВремКаталог);
	Исключение
		СтрОш = НСтр("ru = 'Не удалось удалить временные файлы';
					|en = 'Не удалось удалить временные файлы'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Для Каждого ШаблонЭВФ Из ШаблоныЭВФ Цикл
		...
		Если НЕ ДобавитьРеквизитыИзФайлаШаблона(ШаблонЭВФ) Тогда
			Если Не ТихийРежим Тогда 
				ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не распознан формат XML-шаблона ""%1""';
		...
		Если СтрНайти(ШаблонЭВФ.ВерсияФормата, "1.3") = 0 Тогда
			Если Не ТихийРежим Тогда 
				ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Версия формата XML-шаблона ""%1"" не соответствует версии ""1.3""';
	 ...
	 ИЛИ СвойстваШаблонов.Количество() = 0 Тогда
		Если Не ТихийРежим Тогда 
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Нет данных для обновления шаблонов.';
														|en = 'Нет данных для обновления шаблонов.'"));
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если WSПрокси = Неопределено Тогда
		Если Не ТихийРежим Тогда 
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось подключиться к веб-сервису Росстата.';
														|en = 'Не удалось подключиться к веб-сервису Росстата.'"));
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если WSОперация <> Неопределено Тогда
		...
		Для НомерШаблона = 1 По КоличествоОбновляемыхШаблонов Цикл
					...
					Если СчетчикОщибокОпераций > 3 Тогда // превышено количество ошибок обращения к сервису
						...
						ЗаписьЖурналаРегистрации(
							НСтр("ru = 'Обновление шаблонов Росстата. Ошибка вызова операции веб-сервиса (GetTemplateBody).';
								|en = 'Обновление шаблонов Росстата. Ошибка вызова операции веб-сервиса (GetTemplateBody).'",
						...
						Если Не ТихийРежим Тогда 
							ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Ошибка доступа к веб-сервису при получении данных шаблона Росстата: %1 ""%2"".';
									|en = 'Ошибка доступа к веб-сервису при получении данных шаблона Росстата: %1 ""%2"".'"),
						...
						ОбщегоНазначения.СообщитьПользователю(
							НСтр("ru = 'Превышено количество ошибок при запросах к веб-сервису. Операция обновления шаблонов прервана.';
								|en = 'Превышено количество ошибок при запросах к веб-сервису. Операция обновления шаблонов прервана.'"));
			...
			Если НЕ (ТипЗнч(Ответ) = Тип("ОбъектXDTO")
				...
				Если Не ТихийРежим Тогда 
					ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Формат ответа веб-сервиса не позволяет получить данные шаблона Росстата:|%1 ""%2"".';
							|en = 'Формат ответа веб-сервиса не позволяет получить данные шаблона Росстата:|%1 ""%2"".'"), СвойстваШаблона.Код, СвойстваШаблона.ИмяФормы));
				КонецЕсли;
				КоличествоНезагруженныхШаблонов = КоличествоНезагруженныхШаблонов + 1;
				Продолжить;
			КонецЕсли;
			...
			Исключение
				Если Не ТихийРежим Тогда 
					ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка при распаковке данных шаблона Росстата:|%1 ""%2"".';
							|en = 'Ошибка при распаковке данных шаблона Росстата:|%1 ""%2"".'"), СвойстваШаблона.Код, СвойстваШаблона.ИмяФормы));
				КонецЕсли;
				КоличествоНезагруженныхШаблонов = КоличествоНезагруженныхШаблонов + 1;
				Продолжить;
			КонецПопытки;
			...
			Если ДанныеШаблона = Неопределено Тогда
				Если Не ТихийРежим Тогда 
					ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Архив не содержит необходимый файл шаблона:|%1 ""%2"".';
							|en = 'Архив не содержит необходимый файл шаблона:|%1 ""%2"".'"), СвойстваШаблона.Код, СвойстваШаблона.ИмяФормы));
				КонецЕсли;
				КоличествоНезагруженныхШаблонов = КоличествоНезагруженныхШаблонов + 1;
				Продолжить;
			КонецЕсли;
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 из %2';
					|en = '%1 из %2'"), Формат(НомерШаблона, "ЧГ="), Формат(КоличествоОбновляемыхШаблонов, "ЧГ="));
	...
	Иначе
		Если Не ТихийРежим Тогда 
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Веб-сервис не поддерживает операцию получения содержимого шаблонов Росстата.';
														|en = 'Веб-сервис не поддерживает операцию получения содержимого шаблонов Росстата.'"));
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Функция СписокШаблоновСервисаВебСбора(СвойстваМодуля, ИспользоватьЦентральныйСервисСбора = Ложь, ПолучатьВсеВерсии = Ложь, ТихийРежим = Ложь) Экспорт
	Перем АдресСервера;
	...
	Если WSОперация <> Неопределено Тогда
		...
		Попытка
			...
			Если НЕ (ТипЗнч(Ответ) = Тип("ОбъектXDTO")
				...
				Если Не ТихийРежим Тогда 
					ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Формат ответа веб-сервиса не позволяет получить номер актуальной версии программного модуля Росстата.';
																|en = 'Формат ответа веб-сервиса не позволяет получить номер актуальной версии программного модуля Росстата.'"))
				КонецЕсли;
			Иначе
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			КодОсновногоЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление шаблонов Росстата. Ошибка вызова операции веб-сервиса (GetActualVersion).';
											|en = 'Обновление шаблонов Росстата. Ошибка вызова операции веб-сервиса (GetActualVersion).'", КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				
			Если Не ТихийРежим Тогда 
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Ошибка при получении номера версии программного модуля Росстата.';
															|en = 'Ошибка при получении номера версии программного модуля Росстата.'"));
			КонецЕсли;
		КонецПопытки;
	ИначеЕсли Не ТихийРежим Тогда 
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Веб-сервис не поддерживает операцию получения номера актуальной версии программного модуля Росстата.';
													|en = 'Веб-сервис не поддерживает операцию получения номера актуальной версии программного модуля Росстата.'"));
	КонецЕсли;
	...
	Если WSОперация <> Неопределено Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			КодОсновногоЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление шаблонов Росстата. Ошибка вызова операции веб-сервиса (GetTemplatesInfo).';
											|en = 'Обновление шаблонов Росстата. Ошибка вызова операции веб-сервиса (GetTemplatesInfo).'", КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				
			Если Не ТихийРежим Тогда 
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Ошибка доступа к веб-сервису при получении списка актуальных шаблонов Росстата.
					|Проверьте правильность ввода адреса ON-line модуля ТОГС.';
					|en = 'Ошибка доступа к веб-сервису при получении списка актуальных шаблонов Росстата.
					|Проверьте правильность ввода адреса ON-line модуля ТОГС.'"));
			КонецЕсли;
			Возврат Неопределено;
		КонецПопытки;
		...
		Если НЕ (ТипЗнч(Ответ) = Тип("ОбъектXDTO")
			...
			Если Не ТихийРежим Тогда 
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Формат ответа веб-сервиса не позволяет получить список актуальных шаблонов Росстата.';
															|en = 'Формат ответа веб-сервиса не позволяет получить список актуальных шаблонов Росстата.'"));
			КонецЕсли;
			Возврат Неопределено;
		КонецЕсли;
	Иначе
		Если Не ТихийРежим Тогда 
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Веб-сервис не поддерживает операцию получения списка актуальных шаблонов Росстата.';
														|en = 'Веб-сервис не поддерживает операцию получения списка актуальных шаблонов Росстата.'"));
		КонецЕсли;
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПроксиСервисаВебСбора(АдресСервера, Таймаут = 120, ИспользоватьЦентральныйСервисСбора = Ложь) Экспорт 
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление шаблонов Росстата. Ошибка создания клиентского прокси для вызова веб-сервиса';
										|en = 'Обновление шаблонов Росстата. Ошибка создания клиентского прокси для вызова веб-сервиса'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ДвоичныеДанныеФайлаИзАрхива(ДвоичныеДанныеФайлаАрхива) Экспорт 
	...
	Для Каждого ОбъектФайл Из ОбъектыФайл Цикл
		...
		Если ОбъектФайл.Существует() Тогда
			...
			Исключение
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось получить данные шаблона из файла ""%1"":%2';
																								|en = 'Не удалось получить данные шаблона из файла ""%1"":%2'"), ОбъектФайл.Имя, Символы.ПС + ОписаниеОшибки());
				Сообщение.Сообщить();
			КонецПопытки;
	...
	Попытка
		УдалитьФайлы(ВремКаталог);
	Исключение
		СообщениеОбОшибке = НСтр("ru = 'Не удалось удалить временные файлы';
								|en = 'Не удалось удалить временные файлы'", ОбщегоНазначения.КодОсновногоЯзыка());
