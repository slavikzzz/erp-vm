...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(ТаблицаСвойствШаблонов) <> Тип("ТаблицаЗначений") Тогда 
		ОбщегоНазначения.СообщитьПользователю("Сервис временно недоступен. Попробуйте позднее или загрузите шаблоны вручную с сайта Росстата");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьМодульПодготовкиОтчетовНажатие(Элемент)
	...
	Исключение
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Не удалось перейти по ссылке!';
										|en = 'Не удалось перейти по ссылке!'"));
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСвойстваШаблонов
...
Процедура Обновить(Команда)
	...
	Иначе
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Не отмечены шаблоны для обновления или добавления';
										|en = 'Не отмечены шаблоны для обновления или добавления'"));
