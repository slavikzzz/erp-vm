...
Процедура КаталогОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ФайловаяСистемаКлиент.ВыбратьКаталог(Оповещение, НСтр("ru = 'Выберите каталог для обмена';
															|en = 'Select a directory for exchange'"), Запись.КаталогОбмена);
...
Функция СформироватьНаименование(Форма)
	...
	Возврат СтрШаблон(НСтр("ru = '%1, %2';
							|en = '%1, %2'"), Форма.Запись.Организация, Форма.Запись.СпособОбменаЭД);
