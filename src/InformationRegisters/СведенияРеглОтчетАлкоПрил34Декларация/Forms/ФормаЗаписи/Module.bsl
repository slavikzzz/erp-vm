...
Процедура ПриОткрытии(Отказ)
	...
	Заголовок = "Объем винограда, использованного для производства винодельческой продукции";
	...
	Если НЕ ВладелецФормы.ТекущийЭлемент = Неопределено Тогда
		...
		Если ИмяАктивногоПоля = "П000010000304" Тогда
		    ИмяАктивногоПоля = "ТаблицаПоставщиковНаименование";			
		КонецЕсли;
		
		Если ИмяАктивногоПоля = "П000010000305" Тогда
		    ИмяАктивногоПоля = "ТаблицаПоставщиковИНН";			
		КонецЕсли;
		
		Если ИмяАктивногоПоля = "П000010000306" Тогда
		    ИмяАктивногоПоля = "ТаблицаПоставщиковКПП";			
		КонецЕсли;
...
&НаКлиенте
Процедура ТаблицаПоставщиковПриИзменении(Элемент)
	
	СохранитьТаблицу("ТаблицаПоставщиков");		
...
Процедура ПодготовкаНаСервере()
	...
	ЗагрузитьТаблицу("ТаблицаПоставщиков");
...
&НаСервере
Процедура ЗагрузитьТаблицу(ИмяТаблицы)
	
	Если ЗначениеЗаполнено(Запись[ИмяТаблицы])
		И СтрНачинаетсяС(Запись[ИмяТаблицы], ПрефиксХранения) Тогда
	
		
		ДлинаПрефикса = СтрДлина(ПрефиксХранения);
		
		СтрокаВнутрСтруктурыТаблицы = Сред(Запись[ИмяТаблицы], ДлинаПрефикса + 1);
		
		Таб = ЗначениеИзСтрокиВнутр(СтрокаВнутрСтруктурыТаблицы);
		Если ТипЗнч(Таб) = Тип("ТаблицаЗначений") Тогда		
			ЗначениеВДанныеФормы(Таб, ЭтаФорма[ИмяТаблицы]);		
		КонецЕсли;
...
&НаСервере
Процедура СохранитьТаблицу(ИмяТаблицы)
	
	Таб = ДанныеФормыВЗначение(ЭтаФорма[ИмяТаблицы],  Тип("ТаблицаЗначений"));
	...
	Для каждого СтрокаТЗ Из Таб Цикл
		...
		Для каждого Колонка Из Колонки Цикл
			...
			Если ТипЗнч(СтрокаТЗ[Колонка.Имя]) = Тип("Строка") Тогда
			
				СтрокаТЗ[Колонка.Имя] = СокрЛП(СтрокаТЗ[Колонка.Имя]);
	...
	СтрокаВнутрСтруктурыТаблицы = ЗначениеВСтрокуВнутр(Таб);
	
	Запись[ИмяТаблицы] = ПрефиксХранения + СтрокаВнутрСтруктурыТаблицы;
...
Процедура ДоступностьПолейНаСервере()
	...
	Элементы.П000010000308.ТолькоПросмотр = НЕ ФлажокОтклАвтоРасчет;
	Элементы.П000010000308.ПропускатьПриВводе = НЕ ФлажокОтклАвтоРасчет;
	
	// По схеме выгрузки возможно или заполнение поля Закуплено и таблицы Поставщиков,
	// или - заполнение поля Собрано.
	Элементы.П000010000307.ТолькоПросмотр = (Запись.П000010000303 > 0);
	Элементы.П000010000307.ПропускатьПриВводе = (Запись.П000010000303 > 0);
	Элементы.ТаблицаПоставщиков.ТолькоПросмотр = (Запись.П000010000303 > 0);
	Элементы.ТаблицаПоставщиков.ПропускатьПриВводе = (Запись.П000010000303 > 0);
	
	Если Запись.П000010000303 > 0 Тогда	
		ТаблицаПоставщиков.Очистить();	
	КонецЕсли;
	
	Элементы.П000010000303.ТолькоПросмотр = (Запись.П000010000307 > 0) ИЛИ (ТаблицаПоставщиков.Количество() > 0);
	Элементы.П000010000303.ПропускатьПриВводе = (Запись.П000010000307 > 0) ИЛИ (ТаблицаПоставщиков.Количество() > 0);
