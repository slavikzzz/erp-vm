...
Процедура ОчиститьРегистрЗаПрошлыеМесяцы(ДатаОчистки) Экспорт
	...
	Исключение
		...
		ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Очистка платежных операций';
				|en = 'Clear payment transactions'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()));
	КонецПопытки
	...
	Если ЗначениеЗаполнено(ИдентификаторЗапросаСтрока) Тогда
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Ошибка записи в журнал логирования платежных операций: %1';
					|en = 'An error occurred when logging the payment transactions: %1'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()), 
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
				НСтр("ru = 'Платежные операции';
					|en = 'Payment transactions'",  ОбщегоНазначенияБПО.КодОсновногоЯзыка()),
					ТекстСообщения);
		КонецПопытки;
	КонецЕсли;
