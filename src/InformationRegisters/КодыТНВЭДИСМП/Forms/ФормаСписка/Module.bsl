...
Процедура ВыбратьИзКлассификатора(Команда)
	...
	Если Не ОбщегоНазначенияИСКлиент.ВыборСтрокиСпискаКорректен(Элементы.КодыТНВЭДОнлайн, Истина) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьИзСправочника(Команда)
	...
	Если Не ОбщегоНазначенияИСКлиент.ВыборСтрокиСпискаКорректен(Элементы.Список, Истина) Тогда
		Возврат;
	КонецЕсли;
...
Функция ЗагрузитьКодыТНВЭД()
	...
	КешПоВидуПродукции = Новый Соответствие;
	Для Каждого СтрокаКодаТНВЭД Из РезультатПроверки.КодыТНВЭД Цикл
		
		Если КодТНВЭДСоответствуетВидуПродукции(СтрокаКодаТНВЭД.КодТНВЭД, КешПоВидуПродукции) Тогда
...
&НаСервере
Функция КодТНВЭДСоответствуетВидуПродукции(КодТНВЭД, КешПоВидуПродукции)
	
	ВидыПродукции = РегистрыСведений.КодыТНВЭДИСМП.ВидПродукцииПоТНВЭДОКПД2(
		ВидПродукции, КодТНВЭД,, КешПоВидуПродукции);
	Возврат ВидыПродукции.Найти(ВидПродукции) <> Неопределено;
