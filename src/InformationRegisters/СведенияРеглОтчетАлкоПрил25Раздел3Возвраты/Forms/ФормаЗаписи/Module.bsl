...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ФормированиеПредставленияПродукцииНаСервере(Запись.П000030000301, Запись.П000030000302, Запись.П000030000303Адрес);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстЗапроса = РегламентированнаяОтчетностьАЛКО.ТекстЗапросаВыбораКонтрагентаАЛКО(
									ОсновнаяТаблица, Истина, "НеПиво", , СтруктураПараметров);
...
Процедура ПолеПриИзменении(Элемент)
	...
	Если Элемент.Имя = "П000030000307" ИЛИ Элемент.Имя = "П000030000308" Тогда
...
Процедура ФормированиеЗаголовковСвернутогоОтображения()
	...
	Если ЗначениеЗаполнено(Запись.П000030000303) 
		или ЗначениеЗаполнено(Запись.П000030000304)
		или ЗначениеЗаполнено(Запись.П000030000305)
		Тогда
	    Элементы.ГруппаПроизводительИмпортер.ЗаголовокСвернутогоОтображения = "Производитель или импортер: " + 
			?(ЗначениеЗаполнено(Запись.П000030000303),Запись.П000030000303, "наименование не заполнено") 
			+ ?(ЗначениеЗаполнено(Запись.П000030000304),", ИНН " + Запись.П000030000304, ", ИНН не заполнено")
			+ ?(ЗначениеЗаполнено(Запись.П000030000305),", КПП " + Запись.П000030000305, 
											?(СтрДлина(СокрЛП(Запись.П000030000304)) = 10,", КПП не заполнено", "") );
	Иначе	
	...
	Если ЗначениеЗаполнено(Запись.П000030000307) 
		или ЗначениеЗаполнено(Запись.П000030000308)
		или ЗначениеЗаполнено(Запись.П000030000306)
		Тогда
		
	    Элементы.ГруппаПоставщика.ЗаголовокСвернутогоОтображения = "Поставщик: " + 
			?(ЗначениеЗаполнено(Запись.П000030000306),Запись.П000030000306, "наименование не заполнено") 
			+ ?(ЗначениеЗаполнено(Запись.П000030000307),", ИНН " + Запись.П000030000307, ", ИНН не заполнено")
			+ ?(ЗначениеЗаполнено(Запись.П000030000308),", КПП " + Запись.П000030000308, 
											?(СтрДлина(СокрЛП(Запись.П000030000307)) = 10,", КПП не заполнено", "") );
	...
	// Доступ к КПП только если введен 10 значный ИНН.
	Если СтрДлина(СокрЛП(Запись.П000030000307)) = 10 Тогда
		
	    Элементы.П000030000308.ТолькоПросмотр = Ложь;
		Элементы.П000030000308.ПропускатьПриВводе = Ложь;
		
	Иначе
		
	    Элементы.П000030000308.ТолькоПросмотр = Истина;
		Элементы.П000030000308.ПропускатьПриВводе = Истина;
		Если НЕ СокрЛП(Запись.П000030000308) = "" Тогда
		    Запись.П000030000308 = "";
			Модифицированность = Истина;		
		КонецЕсли; 
...
Процедура ВводАдресаНаКлиенте()
	...
	РегламентированнаяОтчетностьАЛКОКлиент.ВызватьФормуВводаАдресаКонтрагента(
					Запись.П000030000303АдресXML, ЗаголовокФормыВвода, Оповещение, ВидКонтактнойИнформации);
...
Процедура ВводАдресаЗавершениеНаКлиенте(Результат, Параметры) Экспорт
	...
	РегламентированнаяОтчетностьАЛКОКлиент.ВводАдресаКонтрагентаЗавершениеНаКлиенте(Результат, 
				Запись.П000030000303Адрес, Запись.П000030000303АдресXML, Модифицированность);
...
Процедура СформироватьСпискиВыбораНаСервере()
	...
	Элементы.П000030000309.СписокВыбора.ЗагрузитьЗначения(МассивВидовДеятельностиПоЛицензии);
	...
	ИначеЕсли ПоказатьВыборПоставщиков Тогда
		...
		Если ЗначениеЗаполнено(Запись.Поставщик) Тогда
			...
			Если НЕ ДинСписокПоставщика.ОсновнаяТаблица = "Справочник.Контрагенты" Тогда
				...
				ТекстЗапроса = РегламентированнаяОтчетностьАЛКО.ТекстЗапросаВыбораКонтрагентаАЛКО(
										ОсновнаяТаблица, Истина, "НеПиво", Истина, СтруктураПараметров);
...
Процедура ТаблицаПоставщиковВыборНаСервере(СоответствиеТекущиеДанные)
	...
	Запись.П000030000307 = СокрЛП(ИНН);
	Запись.П000030000308 = СокрЛП(КПП);	
	Запись.П000030000306 = СокрЛП(НаименованиеПолное);
	
	Запись.П000030000309 = СокрЛП(ВидДеятельности);
