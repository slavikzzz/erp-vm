...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	//++ НЕ УТ
	Если Запись.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Общая
		И ЗначениеЗаполнено(Запись.Организация) Тогда
		
		ЮрФизЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.Организация,
			"ЮрФизЛицо");
		
		Если ЮрФизЛицо = ПредопределенноеЗначение("Перечисление.ЮрФизЛицо.ИндивидуальныйПредприниматель") Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , "Запись.СистемаНалогообложения", , Отказ);
		КонецЕсли;
	КонецЕсли;
	//-- НЕ УТ
	Возврат
...
Процедура ДатаИзмененияПриИзменении(Элемент)
	ТекстВопроса = НСтр("ru = 'Создать новую настройку на %1? Иначе будет изменен период существующей.';
						|en = 'Create a new customization on the %1? Otherwise, the period of the current will be changed.'");
...
Процедура УстановитьВидимостьЭлементов(Форма)
	...
	//++ НЕ УТ
	Элементы.ПрименяетсяПСН.Видимость = ПрименяетсяУСН И ИП;
	//-- НЕ УТ
