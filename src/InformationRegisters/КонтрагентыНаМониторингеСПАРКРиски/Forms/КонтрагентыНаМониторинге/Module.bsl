...
Процедура СнятьСМониторинга(Команда)
	...
	ПоказатьВопрос(
		...
		НСтр("ru = 'Снять выбранных контрагентов с мониторинга?';
			|en = 'Stop monitoring selected counterparties?'"),
	...
	Если КодВозврата = КодВозвратаДиалога.Да Тогда
		...
		Если УдаленоЗаписей = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Среди выбранных контрагентов нет поставленных вручную.';
											|en = 'There are no manually marked counterparties among selected ones.'"));
		КонецЕсли;
	КонецЕсли;
...
Функция СнятьСМониторингаНаСервере()
	...
	КоличествоИзмененных = СПАРКРискиМониторингСобытий.ОтключитьМониторингКонтрагентов(
		КонтрагентыДляСнятияСМониторинга,
		Истина);
