...
Процедура УстановитьВидимостьЭлементов()
	...
	ТекстПодсказки = НСтр("ru = 'Отражает результат отправки события на сервер ГИС НР.';
							|en = 'Records the result of sending an event to the server of Independent Registrar State Information System.'") + Символы.ПС
			...
			|en = 'The ""Overdue"" status means that the accounting system failed to send the
			|%1 event details when the event happened
			|as stated in the regulation of Independent Registrar State Information System.'");
	...
	Если Запись.СтатусОтправкиДанных = Перечисления.СтатусыОтправкиДанныхВГИСНР.Просрочено Тогда
		ТекстПояснения = НСтр("ru = 'Просрочена отправка на';
								|en = 'Sending is overdue:'") + " " + Формат(Цел(РазницаДат / 60), "ЧЦ=10; ЧГ=0")
					+ " " + НСтр("ru = 'мин';
								|en = 'min'") + ".";
	ИначеЕсли Запись.СтатусОтправкиДанных = Перечисления.СтатусыОтправкиДанныхВГИСНР.ОшибкаОтправки Тогда
		ТекстПояснения = НСтр("ru = 'Проверьте доступность сервиса по адресу';
								|en = 'Follow the address to check if the service is available'") + ": "
