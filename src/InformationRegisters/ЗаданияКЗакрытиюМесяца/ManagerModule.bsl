...
// 
// Параметры:
//  Операция - ПеречислениеСсылка.ОперацииЗакрытияМесяца - Операция
// 
// Возвращаемое значение:
...
Функция ВходящиеДанныеМеханизма(Операция = Неопределено) Экспорт
	...
	Если Операция = Перечисления.ОперацииЗакрытияМесяца.ЗакрытиеРасходовОтВыбытияОС
		ИЛИ Операция = Неопределено Тогда
