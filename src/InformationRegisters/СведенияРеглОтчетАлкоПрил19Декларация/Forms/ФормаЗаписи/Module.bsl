...
Процедура ПриОткрытии(Отказ)
	...
	Заголовок = "Производство и оборот алкогольной продукции";
	...
	Если НЕ ВладелецФормы.ТекущийЭлемент = Неопределено Тогда
		...
		Если ИмяАктивногоПоля = "П000010000310Сумма" Тогда
		    ИмяАктивногоПоля = "ТаблицаПоРФКодРФ";			
		КонецЕсли;
		
		Если ИмяАктивногоПоля = "П000010000311Сумма" Тогда
		    ИмяАктивногоПоля = "ТаблицаПоРФРозница";			
		КонецЕсли;
		
		Если ИмяАктивногоПоля = "П000010000312Сумма" Тогда
		    ИмяАктивногоПоля = "ТаблицаПоРФОпт";			
		КонецЕсли;
		
		Если ИмяАктивногоПоля = "П000010000313Сумма" Тогда
		    ИмяАктивногоПоля = "ТаблицаПоРФДругим";			
		КонецЕсли;
...
&НаКлиенте
Процедура ПредставлениеНажатие(Элемент, СтандартнаяОбработка) Экспорт
...
&НаКлиенте
Процедура ТаблицаПоРФПриИзменении(Элемент)
	
	СохранитьТаблицуПоРФ();		
	ОбработкаПослеИзменения();
	Расчет();
...
&НаСервере
Процедура ЗагрузитьТаблицуПоРФ()
	...
	Если ЗначениеЗаполнено(Запись.ТаблицаПоРФ)
		...
		Если ТипЗнч(Таб) = Тип("ТаблицаЗначений") Тогда		
			ЗначениеВДанныеФормы(Таб, ТаблицаПоРФ);		
		КонецЕсли;
...
&НаСервере
Процедура СохранитьТаблицуПоРФ()
	
	ТабПоРФ = ДанныеФормыВЗначение(ТаблицаПоРФ,  Тип("ТаблицаЗначений"));
	
	СтрокаВнутрСтруктурыТаблицы = ЗначениеВСтрокуВнутр(ТабПоРФ);
	
	Запись.ТаблицаПоРФ = ПрефиксХранения + СтрокаВнутрСтруктурыТаблицы;
...
Процедура ФормированиеЗаголовковСвернутогоОтображения()
	...
	// Остатки.
	Элементы.Остатки.ЗаголовокСвернутогоОтображения = "Остатки в декалитрах: 
					| на начало периода " + Формат(Запись.П000010000304, "ЧЦ = 15; ЧДЦ = 2; ЧН=; ЧВН;") + "
					| на конец периода " + Формат(Запись.П000010000320, "ЧЦ = 15; ЧДЦ = 2; ЧН=; ЧВН;");
...
Процедура СформироватьСпискиВыбораНаСервере()
	...
	// Список выбора для кодов субъектов РФ.
	Для каждого СтрокаТаблицы Из КоллекцияСписковВыбора["КодыСубъектовРФ"] Цикл
	
		Элементы.ТаблицаПоРФКодРФ.СписокВыбора.Добавить(СтрокаТаблицы.Код, СтрокаТаблицы.Код + " - " + СтрокаТаблицы.Название);
	
	КонецЦикла; 
	
	Элементы.ТаблицаПоРФКодРФ.СписокВыбора.СортироватьПоЗначению();
