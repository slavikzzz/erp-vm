	...
	Для каждого СтрокаОбъектОплаты Из ТаблицаОбъектовОплаты Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось записать график платежей по: %1 по причине: %2';
									|en = 'Cannot save a payment schedule for: %1. Reason: %2'");
...
Процедура РассчитатьГрафикПлатежейПоФинансовымИнструментам(ОбъектыОплаты, ДопПараметр = Неопределено, Очередь = Неопределено) Экспорт
	...
	Для каждого ОбъектОплаты Из ОбъектыОплаты Цикл
		...
		//++ НЕ УТ
		ИначеЕсли ТипЗнч(ОбъектОплаты) = Тип("СправочникСсылка.ДоговорыАренды") Тогда
			ДоговорыАренды.Добавить(ОбъектОплаты);
		//-- НЕ УТ
		КонецЕсли;
	КонецЦикла;
	...
	//++ НЕ УТ
	Если ДоговорыАренды.Количество() Тогда
		ИдентификаторОбъектаМетаданных = ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Справочник.ДоговорыАренды");
		ТекстЗапроса = ТекстЗапросаГрафикПлатежейПоДоговорамАренды();
		ЗаписатьГрафикПлатежейПоФинансовымИнструментам(
			ДоговорыАренды, 
			ТекстЗапроса, 
			ИдентификаторОбъектаМетаданных, 
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Изменение операции ""Оплата самозанятому"" на операцию ""Оплата поставщику"" в регистре сведений ""График платежей""';
									|en = 'Change the ""Payment to self-employed person"" transaction to the ""Payment to vendor"" transaction in the ""Payment schedule"" information register'");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось обработать записи регистра сведений ""График платежей"".';
								|en = 'Cannot process the ""Payment schedule"" information register records.'"));
	...
	Иначе
		...
		ШаблонСообщения = НСтр("ru = 'Обработана порция объектов оплаты по регистру ""График платежей"": %1';
								|en = 'A batch of payment objects of the Payment schedule register is processed: %1'");
	...
	Для каждого СтрокаТаблицы Из ТаблицаОбъектовОплаты Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось записать график платежей по: %Ссылка% по причине: %Причина%';
									|en = 'Cannot save a payment schedule for: %Ссылка%. Reason: %Причина%'");
...
Функция ТекстЗапросаГрафикПлатежейПоДоговорамАренды()
	...
	|	ДанныеСправочника.Дата                              КАК Дата,
	|	ЗНАЧЕНИЕ(Перечисление.ФормыОплаты.Безналичная)      КАК ФормаОплаты,
	|	ЗНАЧЕНИЕ(Перечисление.ОбластиПланированияПлатежей.Аренда) КАК ОбластьПланирования
	|ПОМЕСТИТЬ Договоры
	|ИЗ
	|	Справочник.ДоговорыАренды КАК ДанныеСправочника
	|ГДЕ
	|	ДанныеСправочника.Ссылка В (&ОбъектыОплаты)
	|ИНДЕКСИРОВАТЬ ПО
	|	ОбъектОплаты
	...
	|	Расчеты.Валюта КАК Валюта,
	|	ВЫБОР
	|		КОГДА Расчеты.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.УслугаПоАрендеАванс)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.УслугаПоАренде)
	|		ИНАЧЕ
	|			Расчеты.ТипСуммы
	|	КОНЕЦ КАК ТипСуммы,
	|	СУММА(Расчеты.СуммаПриход - Расчеты.ОплачиваетсяПриход + Расчеты.ОплачиваетсяРасход) КАК Сумма
	|ПОМЕСТИТЬ СписаниеСредств
	|ИЗ
	|	РегистрНакопления.РасчетыПоФинансовымИнструментам.Обороты(,,,
	|		Договор В (&ОбъектыОплаты)
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(Расчеты.СуммаПриход - Расчеты.ОплачиваетсяПриход + Расчеты.ОплачиваетсяРасход) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	РегистрСведений.УсловияДоговоровАренды.СрезПоследних(, Договор В (ВЫБРАТЬ Договоры.ОбъектОплаты ИЗ Договоры)) КАК УсловияДоговоровАренды
	|
	|ГДЕ
	|	НЕ Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияДоговоровКонтрагентов.Закрыт)
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ // Оплата услуг по аренде
	|	Договоры.ОбъектОплаты КАК ОбъектОплаты,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыДвиженияДенежныхСредств.Списание) КАК ПоступлениеСписание,
	...
	|	График.Дата КАК ДатаПлатежа,
	|	ВЫБОР
	|		КОГДА График.УслугаПоАрендеИтог - График.УслугаПоАренде - ЕСТЬNULL(СписаниеСредств.Сумма, 0) < 0 ТОГДА
	|			График.УслугаПоАрендеИтог - ЕСТЬNULL(СписаниеСредств.Сумма, 0)
	|		ИНАЧЕ
	|			График.УслугаПоАренде
	|	КОНЕЦ КАК Сумма
	|
	|ИЗ
	|	РегистрСведений.ГрафикОплатУслугПоАренде КАК График
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Договоры
	|		ПО Договоры.ОбъектОплаты = График.Договор
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ СписаниеСредств
	|		ПО СписаниеСредств.Договор = Договоры.ОбъектОплаты
	|			И СписаниеСредств.Валюта = Договоры.Валюта
	|			И СписаниеСредств.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.УслугаПоАренде)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВводОстатковВзаиморасчетовПоАренде КАК ВводОстатковВзаиморасчетовПоАренде
	|		ПО График.Регистратор = ВводОстатковВзаиморасчетовПоАренде.Ссылка
	|	
	|ГДЕ
	|	График.УслугаПоАренде > 0
	|	И График.УслугаПоАрендеИтог - ЕСТЬNULL(СписаниеСредств.Сумма, 0) > 0
	|	И ((График.АктуальныеУсловияДоговора, График.Договор) В 
	|			(ВЫБРАТЬ 
	...
	|
	|		ИЛИ График.АктуальныеУсловияДоговора В (&ДопПараметр)
	|			И НЕ ИСТИНА В (ВЫБРАТЬ ИСТИНА ИЗ втУсловияДоговоровАренды КАК Т))
	|	И График.Дата >= ЕСТЬNULL(ВводОстатковВзаиморасчетовПоАренде.Дата, ДАТАВРЕМЯ(1,1,1))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	График.Дата КАК ДатаПлатежа,
	|	ВЫБОР
	|		КОГДА График.ОбеспечительныйПлатежИтог - График.ОбеспечительныйПлатеж - ЕСТЬNULL(СписаниеСредств.Сумма, 0) < 0 ТОГДА
	|			График.ОбеспечительныйПлатежИтог - ЕСТЬNULL(СписаниеСредств.Сумма, 0)
	|		ИНАЧЕ
	|			График.ОбеспечительныйПлатеж
	|	КОНЕЦ КАК Сумма
	|
	|ИЗ
	|	РегистрСведений.ГрафикОплатУслугПоАренде КАК График
	...
	|		ПО СписаниеСредств.Договор = Договоры.ОбъектОплаты
	|			И СписаниеСредств.Валюта = Договоры.Валюта
	|			И СписаниеСредств.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВводОстатковВзаиморасчетовПоАренде КАК ВводОстатковВзаиморасчетовПоАренде
	|		ПО График.Регистратор = ВводОстатковВзаиморасчетовПоАренде.Ссылка
	|	
	|ГДЕ
	|	График.ОбеспечительныйПлатеж > 0
	|	И График.ОбеспечительныйПлатежИтог - ЕСТЬNULL(СписаниеСредств.Сумма, 0) > 0
	|	И ((График.АктуальныеУсловияДоговора, График.Договор) В 
	|			(ВЫБРАТЬ 
	|				Т.АктуальныеУсловияДоговора, 
	|				Т.Договор 
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ // Выкуп предметов аренды
	|	Договоры.ОбъектОплаты КАК ОбъектОплаты,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыДвиженияДенежныхСредств.Списание) КАК ПоступлениеСписание,
	...
	|	График.Дата КАК ДатаПлатежа,
	|	ВЫБОР
	|		КОГДА График.ВыкупнаяСтоимостьИтог - График.ВыкупнаяСтоимость - ЕСТЬNULL(СписаниеСредств.Сумма, 0) < 0 ТОГДА
	|			График.ВыкупнаяСтоимостьИтог - ЕСТЬNULL(СписаниеСредств.Сумма, 0)
	|		ИНАЧЕ
	|			График.ВыкупнаяСтоимость
	|	КОНЕЦ КАК Сумма
	|
	|ИЗ
	|	РегистрСведений.ГрафикОплатУслугПоАренде КАК График
	...
	|		ПО СписаниеСредств.Договор = Договоры.ОбъектОплаты
	|			И СписаниеСредств.Валюта = Договоры.Валюта
	|			И СписаниеСредств.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ВыкупнаяСтоимость)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВводОстатковВзаиморасчетовПоАренде КАК ВводОстатковВзаиморасчетовПоАренде
	|		ПО График.Регистратор = ВводОстатковВзаиморасчетовПоАренде.Ссылка
	|	
	|ГДЕ
	|	График.ВыкупнаяСтоимость > 0
	|	И График.ВыкупнаяСтоимостьИтог - ЕСТЬNULL(СписаниеСредств.Сумма, 0) > 0
	|	И ((График.АктуальныеУсловияДоговора, График.Договор) В 
	|			(ВЫБРАТЬ 
	|				Т.АктуальныеУсловияДоговора, 
	|				Т.Договор 
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ // Авансирование выкупа предметов аренды
	|	Договоры.ОбъектОплаты КАК ОбъектОплаты,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыДвиженияДенежныхСредств.Списание) КАК ПоступлениеСписание,
	...
	|	График.Дата КАК ДатаПлатежа,
	|	ВЫБОР
	|		КОГДА График.ВыкупнаяСтоимостьАвансИтог - График.ВыкупнаяСтоимостьАванс - ЕСТЬNULL(СписаниеСредств.Сумма, 0) < 0 ТОГДА
	|			График.ВыкупнаяСтоимостьАвансИтог - ЕСТЬNULL(СписаниеСредств.Сумма, 0)
	|		ИНАЧЕ
	|			График.ВыкупнаяСтоимостьАванс
	|	КОНЕЦ КАК Сумма
	|
	|ИЗ
	|	РегистрСведений.ГрафикОплатУслугПоАренде КАК График
	...
	|		ПО СписаниеСредств.Договор = Договоры.ОбъектОплаты
	|			И СписаниеСредств.Валюта = Договоры.Валюта
	|			И СписаниеСредств.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ВыкупнаяСтоимостьАванс)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВводОстатковВзаиморасчетовПоАренде КАК ВводОстатковВзаиморасчетовПоАренде
	|		ПО График.Регистратор = ВводОстатковВзаиморасчетовПоАренде.Ссылка
	|	
	|ГДЕ
	|	График.ВыкупнаяСтоимостьАванс > 0
	|	И График.ВыкупнаяСтоимостьАвансИтог - ЕСТЬNULL(СписаниеСредств.Сумма, 0) > 0
	|	И ((График.АктуальныеУсловияДоговора, График.Договор) В 
	|			(ВЫБРАТЬ 
	|				Т.АктуальныеУсловияДоговора, 
	|				Т.Договор 
