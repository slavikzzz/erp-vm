...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Попытка
		...
		Если РезультатЗапроса.Пустой() Тогда
			ДатаПоследнегоИзменения = СинхронизацияЭДО.ДатаПоследнегоИзмененияДанныхСервисаНастроек(
				Перечисления.ТипыСинхронизацииСервисаНастроекЭДО.СвязьТиповИФорматовЭД);
	...
	Исключение
		ОтменитьТранзакцию();
		ТекстСообщения = НСтр("ru = 'Не удалось заполнить кеши связей видов и форматов электронных документов по причине:';
								|en = 'Cannot fill caches of links between electronic document kinds and formats due to:'") + Символы.ПС 
