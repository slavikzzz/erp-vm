...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("Пользователь", Пользователь) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр(
			...
			|en = 'The report snapshot list is available only from a report form or report panel.'"), , , , Отказ);
		Возврат;
	КонецЕсли;
...
#КонецЕсли
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Обновление снимков завершено';
										|en = 'Snapshots updated'"), , ТекстОповещения);
