...
Функция ПолучитьОписаниеПоВерсии(СсылкаНаОбъект, НомерВерсии)
	...
	Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '№ %1 / (%2) / %3';
																			|en = '#%1 / (%2) / %3'"), НомерВерсии.Представление, 
...
Процедура ПрогрессУдалениеПредупрежденийСинхронизации(Знач ТекущийШаг, Максимум, ИтераторВыборки = 0)
	...
	Если ИтераторВыборки = 0 Тогда
		...
		Шаблон = НСтр("ru = 'Выполнено итераций %1 из %2';
						|en = '%1 out of %2 iterations completed'",  ОбщегоНазначения.КодОсновногоЯзыка());
		ТекстПрогресса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ТекущийШаг, Максимум);
		
	Иначе
		
		Шаблон = НСтр("ru = 'Выполнено итераций %1 из %2 (%3)';
						|en = '%1 out of %2 iterations completed (%3)'",  ОбщегоНазначения.КодОсновногоЯзыка());
