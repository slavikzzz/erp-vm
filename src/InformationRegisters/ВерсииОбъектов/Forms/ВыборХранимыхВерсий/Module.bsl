...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НетВерсий.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Предыдущие версии отсутствуют: ""%1"".';
																								|en = 'Earlier versions are not available: %1.'"), Строка(Ссылка));
	...
	Реквизиты = НСтр("ru = 'Все';
					|en = 'All'");
	Заголовок = НСтр("ru = 'История изменений:';
					|en = 'Change history:'") + " " + Ссылка;
...
Процедура ВыполнитьПереходНаВыбраннуюВерсию(ОтменятьПроведение = Ложь)
	...
	Иначе //Результат = "ВосстановлениеВыполнено"
		...
		ПоказатьОповещениеПользователя(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Восстановлена версия № %1.';
																		|en = 'Version #%1 is restored.'"), ПредставлениеНомераВерсии),
