			...
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'Настройка обмена для организации с кодом ФСРАР %1 уже существует';
						|en = 'Настройка обмена для организации с кодом ФСРАР %1 уже существует'"),
		...
		ПоказатьВопрос(
			...
			СтрШаблон(
				НСтр("ru = 'Перед выполнением обмена данными с ЕГАИС рекомендуется
				         ...
				         |en = 'Перед выполнением обмена данными с ЕГАИС рекомендуется
				         |запросить данные организации с кодом ФСРАР %1.'"),
				Запись.ИдентификаторФСРАР),
			Кнопки,,
		...
		Если Модифицированность Тогда
			...
			ПоказатьВопрос(
				...
				НСтр("ru = 'Перед запросом данных организации ЕГАИС
				         ...
				         |en = 'Перед запросом данных организации ЕГАИС
				         |по коду ФСРАР текущая настройка обмена должна быть записана.'"),
				Кнопки,,
				КодВозвратаДиалога.Да);
		...
		МассивСтрок.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru = 'Организация ЕГАИС не найдена по коду ФСРАР
				            ...
				            |en = 'Организация ЕГАИС не найдена по коду ФСРАР
				            |в классификаторе организаций.'"),,
				Форма.ЦветаСтиляКлиент.ЦветТекстаПроблемаГосИС));
	...
	Если Форма.ОбменНаСервере = 0 Тогда
		Форма.Подсказка = Новый ФорматированнаяСтрока(
			НСтр("ru = 'Подключение к УТМ будет осуществляться с компьютера пользователя';
				|en = 'Подключение к УТМ будет осуществляться с компьютера пользователя'"));
	ИначеЕсли Форма.ОбменНаСервере = 1 Тогда
		Форма.Подсказка = Новый ФорматированнаяСтрока(
			НСтр("ru = 'Подключение к УТМ будет осуществляться с компьютера, на котором установлен сервер 1С:Предприятия';
				|en = 'Подключение к УТМ будет осуществляться с компьютера, на котором установлен сервер 1С:Предприятия'"));
	КонецЕсли;
...
&НаКлиенте
Асинх Процедура ПолучениеКодаФСРАР_Завершение(Результат, ДополнительныеПараметры) Экспорт
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			СтрШаблон(
				НСтр("ru = 'Внутренняя ошибка получения кода ФСРАР. Обратитесь к администратору.
				           |%1.';
				           |en = 'Внутренняя ошибка получения кода ФСРАР. Обратитесь к администратору.
				           |%1.'"),
	...
	Если ЗаданияОбмена.Количество() <> 1 Тогда
		ЗаписьЖурналаРегистрации(
			...
			НСтр("ru = 'Не найдено регламентное задание обработки ответов ЕГАИС';
				|en = 'Не найдено регламентное задание обработки ответов ЕГАИС'"));
...
Функция ЗаписатьНастройкуОбмена(ЗаписыватьНастройкуОбмена = Истина)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'ЕГАИС';
				|en = 'ЕГАИС'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
...
&НаКлиенте
Асинх Функция КодФСРАРИзУТМНаКлиенте(НастройкаОбмена)
	...
	#Если ВебКлиент Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнфо.ВерсияПриложения, "8.3.21.0") < 0 Тогда
			...
			Результат.Вставить("КодФСРАРУТМ","");
			Результат.Вставить("ТекстОшибки", НСтр("ru = 'Обмен с ЕГАИС из вебклиента недоступен для текущей версии платформы. Используйте тонкий клиент.';
													|en = 'Обмен с ЕГАИС из вебклиента недоступен для текущей версии платформы. Используйте тонкий клиент.'"));
			Возврат Результат;
		КонецЕсли;
		
		РезультатПолученияКодаФСРАР = Ждать ОбменДаннымиЕГАИСВебКлиент.КодФСРАРv3(НастройкаОбмена);
		Если РезультатПолученияКодаФСРАР.ПроверитьУТМv4 Тогда
			РезультатПолученияКодаФСРАР = Ждать ОбменДаннымиЕГАИСВебКлиент.КодФСРАРv4(НастройкаОбмена);
		КонецЕсли;
...
&НаКлиенте
Асинх Процедура ПослеПроверкиПодключенияКУТМ(РезультатПроверки, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если ТипЗнч(РезультатПроверки) = Тип("Обещание") Тогда
		РезультатПроверки = Ждать РезультатПроверки;
	КонецЕсли;
	...
	Если ПустаяСтрока(Запись.АдресУТМ) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не указан IP-адрес (или доменное имя) компьютера с установленной службой УТМ.';
				|en = 'Не указан IP-адрес (или доменное имя) компьютера с установленной службой УТМ.'"),,
			"АдресУТМ",
			"Запись");
		Возврат;
	КонецЕсли;
	...
	Если Запись.ПортУТМ = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не указан порт доступа к УТМ.';
				|en = 'Не указан порт доступа к УТМ.'"),,
			"ПортУТМ",
			"Запись");
		Возврат;
	КонецЕсли;
