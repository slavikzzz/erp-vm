...
Процедура ПолеПриИзменении(Элемент)
	...
	Если Элемент.Имя = "П000010000306" ИЛИ Элемент.Имя = "П000010000307" Тогда
...
&НаКлиенте
Процедура АдресНажатие(Элемент, СтандартнаяОбработка) Экспорт
...
Процедура ФормированиеЗаголовковСвернутогоОтображения()
	...
	Если ЗначениеЗаполнено(Запись.П000010000304)
	    ...
	    Элементы.ГруппаПолучателя.ЗаголовокСвернутогоОтображения = "Получатель: " + 
			...
			+ ?(ЗначениеЗаполнено(Запись.П000010000307),", КПП " + Запись.П000010000307, 
											?(СтрДлина(СокрЛП(Запись.П000010000306)) = 10,", КПП не заполнено", "") )
	...
	Если СтрДлина(СокрЛП(Запись.П000010000306)) = 10 Тогда
...
Процедура ВводАдресаНаКлиенте()
	...
	РегламентированнаяОтчетностьАЛКОКлиент.ВызватьФормуВводаАдресаКонтрагента(
					Запись.П000010000305XML, ЗаголовокФормыВвода, Оповещение, ВидКонтактнойИнформации);
...
Процедура ВводАдресаЗавершениеНаКлиенте(Результат, Параметры) Экспорт
	...
	РегламентированнаяОтчетностьАЛКОКлиент.ВводАдресаКонтрагентаЗавершениеНаКлиенте(Результат, 
				Запись.П000010000305, Запись.П000010000305XML, Модифицированность);
...
Процедура ТаблицаПолучателейВыборНаСервере(НаименованиеПолное, ИНН, КПП)
	...
	Если НЕ (Запись.П000010000306 = ИНН
			И Запись.П000010000307 = КПП
			И Запись.П000010000304 = НаименованиеПолное) Тогда
	
		Запись.П000010000305 	= "";
		Запись.П000010000305XML = "";
