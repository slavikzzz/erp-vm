...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ИЛИ (НастройкиОтправкиЭлектронныхДокументовПоВидам.ТребуетсяИзвещениеОПолучении)";
	...
	Для Каждого СтрокаДанных Из ВыбранныеДанные Цикл
		...
		Попытка
			...
			Если НаборЗаписей.Количество() Тогда
				...
				ОбработатьДанные_ЗаполнитьТребуетсяИзвещениеОПолучении(НаборЗаписей, ТипыДокументовПоВиду, Записать);
...
Процедура УдалитьНастройкиОтправкиЭДО(Параметры, АдресРезультата) Экспорт
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось удалить настройки отправки по:
                                      |Организация: %1,
                                      |Контрагент: %2,
                                      |Договор контрагента: %3.';
                                      |en = 'Cannot remove dispatch settings by:
                                      |Company: %1,
                                      |Counterparty: %2,
                                      |Counterparty contract: %3.'"), Организация, Контрагент, ДоговорКонтрагента);
		
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Удаление настроек отправки ЭДО';
														|en = 'Removal of EDI dispatch settings'"), ОбработкаОшибок.ПодробноеПредставлениеОшибки(Информация), ТекстОшибки);
		Результат = Ложь;
...
Процедура УдалитьНастройкиИнтеркампани(Параметры, АдресРезультата) Экспорт
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось удалить настройки интеркампани по:
                                      |Отправитель: %1,
                                      |Получатель: %2.';
                                      |en = 'Cannot remove intercompany settings by:
                                      |Sender: %1,
                                      |Recipient: %2.'"), Отправитель, Получатель);
		
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Удаление настроек интеркампани';
														|en = 'Removal of intercompany settings'"), ОбработкаОшибок.ПодробноеПредставлениеОшибки(Информация), ТекстОшибки);
		Результат = Ложь;
...
Процедура ОбработатьДанные_ЗаполнитьТребуетсяИзвещениеОПолучении(НаборЗаписей, ТипыДокументовПоВиду, Записать)

	Для Каждого Запись Из НаборЗаписей Цикл
		Если Запись.ТребуетсяИзвещениеОПолучении 
			И РегламентыЭДО.ЭтоОбменЧерезОператора(Запись.СпособОбменаЭД) Тогда
			
			ТипДокумента = ТипыДокументовПоВиду[Запись.ВидДокумента];
			
			Если НастройкиЭДО.ОбменПоТипуДокументаМожноЗавершитьБезИзвещения(ТипДокумента) Тогда
				Запись.ТребуетсяИзвещениеОПолучении = Ложь;
				Записать = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
