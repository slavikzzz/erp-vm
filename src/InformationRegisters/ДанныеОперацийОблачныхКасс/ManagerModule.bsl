	...
	Если ДанныеОперации.СтатусОперации = ОблачныеКассыКлиентСервер.СтатусОперацииВыполнена()
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Запрещено обновление статуса чека с незаполненными терминальными данными.
				|Документ операции: %1
				|Тип данных: %2';
				|en = 'You cannot update a reciept status with unfilled terminal data.
	...
	Попытка
		...
		Если Набор.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ru = 'Не найдена информация о данных операции с Облачной кассы.
				...
				|en = 'Information about the Cloud cash account transaction data is not found.
				|You cannot update the receipt issue information.'",
				ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
	...
	Исключение
		...
		ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка записи информации о пробитии чека.
				|%1';
				|en = 'An error occurred when recording the receipt issue information.
				|%1'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Попытка
		...
		Если Набор.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ru = 'Не найдена информация о данных операции с Облачной кассы.
				...
				|en = 'Information about the Cloud cash account transaction data is not found.
				|You cannot increase the number of attempts to execute the deferred transaction.'",
				ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
	...
	Исключение
		...
		ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка увеличения количества попыток выполнить отложенную операцию Облачной кассы.
				|%1';
				|en = 'An error occurred when increasing the number of attempts to execute the deferred transaction of the Cloud cash account.
				|%1'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Исключение
		...
		ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка изменения признака обработки данных операции Облачной кассой.
				|%1';
				|en = 'An error occurred when changing the flag of processing the Cloud cash account transaction data.
				|%1'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
