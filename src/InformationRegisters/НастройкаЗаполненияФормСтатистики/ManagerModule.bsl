	...
	Если Детализировать Тогда
		...
		Если ЗначениеЗаполнено(ПредставлениеКлассификатора) Тогда
			ШаблонТекста = НСтр("ru = '[ОбъектНаблюдения] - по отдельным [ПредставлениеКлассификатора]';
								|en = '[ОбъектНаблюдения] - by separate [ПредставлениеКлассификатора]'");
		Иначе
			ШаблонТекста = НСтр("ru = '[ОбъектНаблюдения] - детально';
								|en = '[ОбъектНаблюдения] - detailed'");
		КонецЕсли;
...
// Описание формы, которую нужно открыть для настройки поля формы статистики.
//
// Параметры:
...
//
// Возвращаемое значение:
//   Структура    - Описание формы, содержит свойства:
//                  * ИмяФормы - Строка.
...
Функция ОписаниеФормыНастройкиПоля(ИмяОтчета, ИмяРедакцииОтчета, ИмяПоля, Организация) Экспорт
	...
	Если Выборка.Количество() > 1 Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(НастраиваемыеСтрокиОтчета, НомераСтрокОтчета, Истина);
