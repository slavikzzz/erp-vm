...
Процедура ОчиститьРегистрЗаПрошлыеМесяцы(ДатаОчистки) Экспорт
	...
	Исключение
		...
		ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Очистка операций проверки кода маркировки.';
				|en = 'Clear marking code check operations.'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()));
	КонецПопытки
	...
	Если ЗначениеЗаполнено(ИдентификаторСессии) Тогда
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Ошибка записи в журнал логирования операций проверки кода маркировки: %1';
					|en = 'An error occurred when logging marking code check operations: %1'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()),
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
				НСтр("ru = 'Операции проверки кода маркировки.';
					|en = 'Marking code check operations.'",  ОбщегоНазначенияБПО.КодОсновногоЯзыка()),
					ТекстСообщения);
		КонецПопытки;
	КонецЕсли;
...
Процедура ОбновитьИдентификаторФискальнойОперацииРезультатаПроверкиКодаМаркировки(ИдентификаторСессии, ИдентификаторФискальнойОперации) Экспорт
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Ошибка записи в журнал логирования операций проверки кода маркировки: %1';
				|en = 'Ошибка записи в журнал логирования операций проверки кода маркировки: %1'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Операции проверки кода маркировки.';
				|en = 'Операции проверки кода маркировки.'",  ОбщегоНазначенияБПО.КодОсновногоЯзыка()),
				ТекстСообщения);
	КонецПопытки;
