			...
			ПоказатьПредупреждение(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В процессе регистрации изменений произошла ошибка: %1';
						|en = 'An error occurred when registering changes: %1'"),
					ОписаниеОшибки));
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Не выбраны строки для регистрации изменений';
													|en = 'Rows for change registration are not selected'"));
	КонецЕсли;
			...
			ПоказатьПредупреждение(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В процессе удаления регистрации изменений произошла ошибка: %1';
						|en = 'An error occurred when deleting the change registration: %1'"),
					ОписаниеОшибки)); 
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Не выбраны строки для удаления регистрации изменений';
													|en = 'Rows for change registration deletion are not selected'"));
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<Для товара требуется указание серий при отгрузке в розницу с данного склада>';
																|en = '<You need to specify batch of the goods on shipment to retail from this warehouse>'"));
...
Процедура ОбменВыполнен(ВозвращаемоеЗначение, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") 
			...
			ТекстСообщения = ?(ВозвращаемоеЗначение.Результат, НСтр("ru = 'Данные успешно загружены';
																	|en = 'Data imported successfully'"), НСтр("ru = 'При загрузке данных произошла ошибка';
																											|en = 'An error occurred when importing data'"));
		Иначе
			ТекстСообщения = ?(ВозвращаемоеЗначение.Результат, НСтр("ru = 'Данные успешно выгружены';
																	|en = 'Data is exported successfully'"), НСтр("ru = 'При выгрузке данных произошла ошибка';
