	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЭтикеткаКодМаркировкиИСМП") Тогда
		...
		Если ТипЗнч(ТабличныйДокумент) = Тип("Массив") Тогда
			...
			ПрефиксИмениМакета = СтрШаблон("%1_%2", НСтр("ru = 'ЭтикеткаИСМП';
														|en = 'ЭтикеткаИСМП'"), Новый УникальныйИдентификатор);
			НомерМакета = 0;
		...
		Иначе
			УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
				...
				НСтр("ru = 'Этикетка (обувь, одежда, табак...)';
					|en = 'Этикетка (обувь, одежда, табак...)'"),
				ТабличныйДокумент);
		КонецЕсли;
...
Процедура ЗарезервироватьКодыПодЗаказ(Параметры, АдресРезультата) Экспорт
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Произошла ошибка при резервировании кодов маркировки.';
							|en = 'Произошла ошибка при резервировании кодов маркировки.'");
		...
		Если КоличествоОшибок > 0 Тогда
			...
			Если КоличествоОшибок = КоличествоОбъектовПечати Тогда
				ВызватьИсключение НСтр("ru = 'При формировании печатной формы возникли ошибки.
					...
					|en = 'При формировании печатной формы возникли ошибки.
					|Не удалось обработать объекты для печати.'");
			Иначе
				ОбщегоНазначения.СообщитьПользователю(СтрШаблон(
					НСтр("ru = 'При формировании печатной формы возникли ошибки. Обработано объектов для печати %1 из %2';
						|en = 'При формировании печатной формы возникли ошибки. Обработано объектов для печати %1 из %2'"),
			...
			ЗаписьЖурналаРегистрации(
				...
				СтрШаблон(
					НСтр("ru = 'Ошибка при очистке пула кодов маркировки по причине: %1';
						|en = 'Ошибка при очистке пула кодов маркировки по причине: %1'"),
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Произошла ошибка сбрасывании признака печати по документу %1:
				       |%2';
				       |en = 'Произошла ошибка сбрасывании признака печати по документу %1:
				       |%2'"),
			ЗаказНаЭмиссию,
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Произошла ошибка сохранения кода маркировки %1 в пул:
				       |%2';
				       |en = 'Произошла ошибка сохранения кода маркировки %1 в пул:
				       |%2'"),
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось обработать регистр сведений: %ИмяРегистра% по причине: %Причина%';
									|en = 'Не удалось обработать регистр сведений: %ИмяРегистра% по причине: %Причина%'");
...
Процедура РаспечататьЭтикеткиИЗафиксироватьДатуПечатиДляОбъектовПечати(ДанныеПечати, СтруктураОбъектовПечати,
	...
	Если Не ЗначениеЗаполнено(Документ)
		...
		ВызватьИсключение НСтр("ru = 'Ошибка печати этикеток: Не указан документ для печати';
								|en = 'Ошибка печати этикеток: Не указан документ для печати'");
	КонецЕсли;
	...
	Попытка
		...
		Если ОбщаяТаблицаПечати.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ru = 'Ошибка печати этикеток: Ошибка режима печати (нет данных для печати)';
									|en = 'Ошибка печати этикеток: Ошибка режима печати (нет данных для печати)'");
		КонецЕсли;
			...
			Для Каждого Рисунок Из Область.Рисунки Цикл
				...
				Иначе
					ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'При генерации картинки штрихкода произошла ошибка';
																|en = 'При генерации картинки штрихкода произошла ошибка'"));
				КонецЕсли;
...
Функция ТаблицаДляПечатиНеРаспечатанныеКодыПоДокументу(ТаблицаНоменклатуры, СтруктураОтбораДокументы, РезервироватьСвободныеКоды, МаркировкаОстатков)
		...
		Иначе
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Ошибка печати кода для %1, требуется кодов: %2, доступно %3';
					|en = 'Ошибка печати кода для %1, требуется кодов: %2, доступно %3'"),
	...
	Если Отказ Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка печати кодов маркировки';
								|en = 'Ошибка печати кодов маркировки'");
	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из НедоступныеКоды Цикл
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Для %1: код маркировки уже распечатан.';
				|en = 'Для %1: код маркировки уже распечатан.'"),
			ОбщегоНазначенияИС.ПредставлениеНоменклатуры(СтрокаТЧ.Номенклатура, СтрокаТЧ.Характеристика));
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,,, Отказ);
	КонецЦикла;
		...
		Если Зарезервировано <> СтрокаТаблицы.Количество Тогда
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Не удалось зарезервировать код маркировки для:
					       ...
					       |en = 'Не удалось зарезервировать код маркировки для:
					       | ""%1""
					       |по причине:
					       |Не найдено свободного кода.'"),
...
Процедура СформироватьТаблицуМаркированныхТоваровОснований(Запрос, ИмяВременнойТаблицыСКолонкойДокументОснование)
	...
	|			ИЛИ Товары.Ссылка.ВидПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.ЛегкаяПромышленность2025)
	...
	|			ИЛИ Товары.Ссылка.ВидПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.МедицинскиеИзделия20)
	...
	|			ИЛИ Товары.Ссылка.ВидПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.ДезинфицирующиеСредства)
	...
	|			ИЛИ Товары.Ссылка.ВидПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.БАДы2025)
...
Функция НастройкиУдаленияНеиспользованныхКодовМаркировки()
	...
	Строка = ТаблицаНастроек.Добавить();
	Строка.ВидПродукции = Перечисления.ВидыПродукцииИС.ДезинфицирующиеСредства;
	Строка.ДатаЭмиссии  = СрокХранения;
	...
	Строка = ТаблицаНастроек.Добавить();
	Строка.ВидПродукции = Перечисления.ВидыПродукцииИС.БАДы2025;
	Строка.ДатаЭмиссии  = СрокХранения;
	...
	Строка = ТаблицаНастроек.Добавить();
	Строка.ВидПродукции = Перечисления.ВидыПродукцииИС.МедицинскиеИзделия20;
	Строка.ДатаЭмиссии  = СрокХранения;
	...
	Строка = ТаблицаНастроек.Добавить();
	Строка.ВидПродукции = Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность2025;
	Строка.ДатаЭмиссии  = СрокХранения;
...
Функция ДополнитьШаблоныНовыхТоварныхГруппВЗапросе()
	...
	|ИЛИ Товары.Ссылка.ВидПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.КормаДляЖивотныхВлажныеПодконтрольныеВЕТИС)";
	...
	|ИЛИ Товары.Ссылка.ВидПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.КормаДляЖивотныхВлажныеБезВЕТИС)";
	...
	|ИЛИ Товары.Ссылка.ВидПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.ТелефоныИНоутбуки)
	|ИЛИ Товары.Ссылка.ВидПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.ПечатныеПлаты)";
	...
	Результат = Результат + "
	|КОГДА Товары.Ссылка.ВидПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.ПолимерныеТрубы)";
	Если ОбщегоНазначенияИСМПКлиентСервер.ПолимерныеТрубыИспользуетСтандартныйКМ() Тогда
		Результат = Результат + "
		|	ТОГДА ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.ПолимерныеТрубыСтандартныйКМ)";
	Иначе
		Результат = Результат + "
	...
	Результат = Результат + "
	|КОГДА Товары.Ссылка.ВидПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.СладостиИКондитерскиеИзделия)";
	Если ОбщегоНазначенияИСМПКлиентСервер.СладостиИКондитерскиеИзделияИспользуетСтандартныйКМ() Тогда
		Результат = Результат + "
		|	ТОГДА ЗНАЧЕНИЕ(Перечисление.ШаблоныКодовМаркировкиСУЗ.СладостиИКондитерскиеИзделияСтандартныйКМ)";
	Иначе
		Результат = Результат + "
