	...
	Для Каждого СтрокаТЧ Из ДокументыОплаты Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Номер) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'В строке %1 не заполнен номер документа перечисления налога';
								|en = 'Tax transfer document number is not filled in line %1'"), НомерСтроки),
				,
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Дата) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'В строке %1 не заполнена дата документа перечисления налога';
								|en = 'Tax transfer document date is not filled in line %1.'"), НомерСтроки),
				,
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Сумма) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'В строке %1 не заполнена сумма НДС';
								|en = 'VAT amount is not filled in line %1.'"), НомерСтроки),
				,
		...
		Если ЭтоНалоговыйАгент И Не ЗначениеЗаполнено(СтрокаТЧ.ДатаПеречисленияНалога) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'В строке %1 не заполнена дата перечисления налога';
								|en = 'Tax transfer date is not filled in line %1.'"), НомерСтроки),
				,
...
Процедура УстановитьТекстПодсказки(Форма)
	...
	ТекстСообщенияОплаченоПолностью = НСтр("ru = 'Всего перечислено в бюджет [Оплачено] руб.
		...
		|en = 'Total amount transferred to the budget: [Оплачено] rub.
		|This matches VAT amount according to the document.'");
	ТекстСообщенияОсталосьОплатить = НСтр("ru = 'Всего перечислено в бюджет [Оплачено] руб.
		|Осталось перечислить [Осталось] руб.';
		|en = 'Total amount transferred to the budget: [Оплачено] rub.
		|Left to transfer: [Осталось] rub.'");
