	...
	Если НапоминанияПользователяКлиентСервер.ЭтоНавигационнаяСсылкаСообщения(Объект.Идентификатор) Тогда
		...
		Если НачалоДня(Сообщение.Дата) = НачалоДня(ТекущаяДатаСеанса()) Тогда
			ДатаТекстом = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'сегодня %1';
																						|en = 'today %1'"), Формат(Сообщение.Дата, НСтр("ru = 'ДФ=HH:mm';
																																		|en = 'DF=HH:mm'")));
		Иначе
...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.СпособУстановкиВремениНапоминания = ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ОтносительноТекущегоВремени") Тогда
		СпособУстановкиВремениНапоминания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'через %1';
				|en = 'in %1'"), НапоминанияПользователяКлиент.ПредставлениеВремени(Объект.ИнтервалВремениНапоминания));
	Иначе
...
Процедура СпособУстановкиВремениНапоминанияПриИзменении(Элемент)
	...
	Если ИнтервалВремени > 0 Тогда
		...
		СпособУстановкиВремениНапоминания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'через %1';
				|en = 'in %1'"), ИнтервалВремениСтрокой);
	Иначе
		Если Элементы.СпособУстановкиВремениНапоминания.СписокВыбора.НайтиПоЗначению(СпособУстановкиВремениНапоминания) = Неопределено Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Интервал времени не определен.';
															|en = 'Please specify the time interval.'"), , "СпособУстановкиВремениНапоминания");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииИнтервалаВремени(Элемент)
	...
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Интервал времени не определен';
														|en = 'Please specify the time interval.'"), , "ИнтервалВремениСтрокой");
	КонецЕсли;
КонецПроцедуры
...
Процедура Удалить(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Прекратить напоминание?';
											|en = 'Dismiss the reminder?'"), КнопкиДиалога);
