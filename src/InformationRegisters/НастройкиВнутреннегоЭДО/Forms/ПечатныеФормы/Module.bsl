	...
	Если Модифицированность Тогда
		...
		Если ЗавершениеРаботы Тогда
			ТекстПредупреждения = НСтр("ru = 'Настройки печатных форм внутреннего электронного документооборота не сохранены.
										...
										|en = 'The Electronic Data Interchange print form settings have not been saved. 
										|Quit the application?'")
		Иначе
			Описание = Новый ОписаниеОповещения("ПередЗакрытиемОкнаФормы", ЭтотОбъект);
			ПоказатьВопрос(Описание, НСтр("ru = 'Настройки печатных форм внутреннего электронного документооборота изменены. Сохранить изменения?';
											|en = 'The Electronic Data Interchange print forms settings have been changed. Save changes?'"), РежимДиалогаВопрос.ДаНетОтмена);
		КонецЕсли;
	...
	Если РасширенныйРежим Тогда
		...
		Если РазличныеЗначенияМаршрутов.Количество() > 1 Тогда
			ПоказатьВопрос(Оповещение, НСтр("ru = 'При отключении расширенного режима для всех печатных форм будет применяться один маршрут, продолжить?';
											|en = 'If you turn off the advanced mode, the same route will be applied to all print forms. Continue?'"),
