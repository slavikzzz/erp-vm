	...
	Если ТабличнаяЧасть <> "" Тогда
		...
		Для Каждого Сотрудник Из МассивСотрудников Цикл
			...
			Для Каждого ТекСтрока Из СписокПечатныхФорм Цикл
				...
				НоваяСтрока.Представление = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = '[Представление] [ФИО]';
																										|en = '[Представление] [ФИО]'"), Значения);
...
Процедура УстановитьСсылкуНаОригинал() 
	...
	Если ЗначениеЗаполнено(НомерНаПечать) И ЗначениеЗаполнено(СведенияОДокументе.Дата) Тогда
		СсылкаНаДокумент = СтрШаблон(НСтр("ru = '%1 № %2 от %3';
											|en = '%1 #%2, %3'"), ТипДокумента, НомерНаПечать,
	...
	ИначеЕсли ЗначениеЗаполнено(НомерНаПечать) И Не ЗначениеЗаполнено(СведенияОДокументе.Дата) Тогда 
		СсылкаНаДокумент = СтрШаблон(НСтр("ru = '%1 № %2';
											|en = '%1 #%2'"), ТипДокумента, НомерНаПечать);
	Иначе 
		СсылкаНаДокумент = СтрШаблон(НСтр("ru = '%1 от %2';
											|en = '%1, %2'"), ТипДокумента, Формат(СведенияОДокументе.Дата, НСтр("ru = 'ДЛФ=DD';
																												|en = 'DLF=DD'")));
	КонецЕсли;
