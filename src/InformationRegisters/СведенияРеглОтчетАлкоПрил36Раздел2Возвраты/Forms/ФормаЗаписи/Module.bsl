...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнициализацияДинСпискаПолучателя();
...
Процедура ПриОткрытии(Отказ)
	...
	Заголовок = "Возврат от получателя алкогольной продукции";
	...
	Если НЕ ВладелецФормы.ТекущийЭлемент = Неопределено Тогда
		...
		Если ИмяАктивногоПоля = "П000020000301" ИЛИ ИмяАктивногоПоля = "П000020000302" Тогда
		    ИмяАктивногоПоля = "Представление";			
		КонецЕсли; 
		
		Если ИмяАктивногоПоля = "П000020000303Адрес" Тогда
		    ИмяАктивногоПоля = "ПредставлениеАдресаПроизводителя";			
		КонецЕсли;
		
		Если ИмяАктивногоПоля = "П000020000307" Тогда
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Иначе
		// Обычное закрытие.
	    Если Элементы.ГруппаВыборПроизводителя.Видимость или Элементы.ГруппаВыборПолучателя.Видимость Тогда
...
&НаКлиенте
Процедура П000020000304ПриИзменении(Элемент)
	ТаблицаПроизводителейВыборНаСервере(Неопределено, Запись.П000020000304, "");
КонецПроцедуры

&НаКлиенте
Процедура П000020000308ПриИзменении(Элемент)
	
	СоответствиеТекущиеДанные = Новый Соответствие;
	СоответствиеТекущиеДанные.Вставить("НаименованиеПолное", Неопределено);
	СоответствиеТекущиеДанные.Вставить("ИНН", Запись.П000020000308);
	СоответствиеТекущиеДанные.Вставить("КПП", "");
		
	СоответствиеТекущиеДанные.Вставить("ДатаОкончания", Неопределено);
	СоответствиеТекущиеДанные.Вставить("ДатаВыдачи", Неопределено);
	СоответствиеТекущиеДанные.Вставить("СерияНомер", "");
	СоответствиеТекущиеДанные.Вставить("КемВыдана", "");
	СоответствиеТекущиеДанные.Вставить("ВидДеятельности", "");
		
	ТаблицаПолучателейВыборНаСервере(СоответствиеТекущиеДанные);
...
Процедура ПолеПриИзменении(Элемент)
	...
	Если Элемент.Имя = "П000020000304" ИЛИ Элемент.Имя = "П000020000305" Тогда
	
		ОчищаемыйКонтрагент = "ПроизводительИмпортер";	
	
	КонецЕсли; 
	
	Если Элемент.Имя = "П000020000308" ИЛИ Элемент.Имя = "П000020000309" Тогда
...
Процедура АдресНажатие(Элемент = Неопределено, СтандартнаяОбработка = Ложь) Экспорт
	...
	Если Элемент.Имя = "ПредставлениеАдресаПроизводителя" Тогда
	    ИмяПоля = "П000020000303Адрес";
	ИначеЕсли Элемент.Имя = "ПредставлениеАдресаПолучателя" Тогда	
	    ИмяПоля = "П000020000307";
	КонецЕсли;
...
&НаКлиенте
Процедура ПолучательБезЛицензииПриИзменении(Элемент)
	ИнициализацияДинСпискаПолучателя()
КонецПроцедуры
...
Процедура ТаблицаПолучателейВыбор(Элемент, ВыбраннаяСтрока = Неопределено, Поле = Неопределено, СтандартнаяОбработка = Истина)
	...
	Если Элемент.ТекущиеДанные.Свойство("ДатаОкончания") Тогда
	
		СоответствиеТекущиеДанные.Вставить("ДатаОкончания", Элемент.ТекущиеДанные["ДатаОкончания"]);
		СоответствиеТекущиеДанные.Вставить("ДатаВыдачи", Элемент.ТекущиеДанные["ДатаВыдачи"]);
		СоответствиеТекущиеДанные.Вставить("СерияНомер", Элемент.ТекущиеДанные["СерияНомер"]);
		СоответствиеТекущиеДанные.Вставить("КемВыдана", Элемент.ТекущиеДанные["КемВыдана"]);
		СоответствиеТекущиеДанные.Вставить("ВидДеятельности", Элемент.ТекущиеДанные["ВидДеятельности"]);	
		
	Иначе
		
		СоответствиеТекущиеДанные.Вставить("ДатаОкончания", РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа(Тип("Дата")));
		СоответствиеТекущиеДанные.Вставить("ДатаВыдачи", РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа(Тип("Дата")));
		СоответствиеТекущиеДанные.Вставить("СерияНомер", "");
		СоответствиеТекущиеДанные.Вставить("КемВыдана", "");
		СоответствиеТекущиеДанные.Вставить("ВидДеятельности", "");
...
Процедура ФормированиеЗаголовковСвернутогоОтображения()
	...
	Если ЗначениеЗаполнено(Запись.П000020000303) 
	    ...
	    Элементы.ГруппаПроизводительИмпортер.ЗаголовокСвернутогоОтображения = "Производитель или импортер: " + 
			...
			+ ?(ЗначениеЗаполнено(Запись.П000020000305),", КПП " + Запись.П000020000305, 
											?(СтрДлина(СокрЛП(Запись.П000020000304)) = 10,", КПП не заполнено", "") );
	Иначе	
	...
	Если СтрДлина(СокрЛП(Запись.П000020000304)) = 10 Тогда
	...
	Если ЗначениеЗаполнено(Запись.П000020000308) 
		или ЗначениеЗаполнено(Запись.П000020000309)
	    ...
	    Элементы.ГруппаПолучателя.ЗаголовокСвернутогоОтображения = "Получатель: " + 
			...
			+ ?(ЗначениеЗаполнено(Запись.П000020000308),", ИНН " + Запись.П000020000308, ", ИНН не заполнено")
			+ ?(ЗначениеЗаполнено(Запись.П000020000309),", КПП " + Запись.П000020000309, 
											?(СтрДлина(СокрЛП(Запись.П000020000308)) = 10,", КПП не заполнено", "") );
	...
	// Доступ к КПП только если введен 10 значный ИНН.
	Если СтрДлина(СокрЛП(Запись.П000020000308)) = 10 Тогда
		
	    Элементы.П000020000309.ТолькоПросмотр = Ложь;
		Элементы.П000020000309.ПропускатьПриВводе = Ложь;
		
	Иначе
		
	    Элементы.П000020000309.ТолькоПросмотр = Истина;
		Элементы.П000020000309.ПропускатьПриВводе = Истина;
		Если НЕ СокрЛП(Запись.П000020000309) = "" Тогда
		    Запись.П000020000309 = "";
			Модифицированность = Истина;		
		КонецЕсли; 
...
Процедура ФормированиеПредставленияПродукцииНаСервере(
						...
						АдресПроизводителя = Неопределено, АдресПолучателя = Неопределено)
	...
	Если АдресПроизводителя = Неопределено Тогда
	    АдресПроизводителя = Запись.П000020000303Адрес;	
	КонецЕсли;
	...
	Если АдресПолучателя = Неопределено Тогда
	    АдресПолучателя = Запись.П000020000307;	
	КонецЕсли;
...
Процедура СформироватьСпискиВыбораНаСервере()
	...
	Элементы.П000020000310.СписокВыбора.ЗагрузитьЗначения(МассивВидовДеятельностиПоЛицензии);
	
КонецПроцедуры


&НаСервере
Процедура УправлениеВидимостью(ПоказатьВыборПроизводителей = Ложь, ПоказатьВыборПолучателей = Ложь)
	...
	ИначеЕсли ПоказатьВыборПолучателей Тогда
		...
		Если ВставитьКодПродукцииПолучатель Тогда	
			ДинСписокПолучателя.Параметры.УстановитьЗначениеПараметра("КодПродукции", Запись.П000020000302);			
		КонецЕсли;
		
		Если СтрНайти(ДинСписокПолучателя.ТекстЗапроса, "&ДатаКонца") > 0 Тогда
			
			ДинСписокПолучателя.Параметры.УстановитьЗначениеПараметра("ДатаНачала", ДатаНачалаОтчета);
			ДинСписокПолучателя.Параметры.УстановитьЗначениеПараметра("ДатаКонца", ДатаПериодаОтчета);
		...
		Если ЗначениеЗаполнено(Запись.Получатель) Тогда
			...
			Если НЕ ДинСписокПолучателя.ОсновнаяТаблица = "Справочник.Контрагенты" Тогда
								
				ОсновнаяТаблица = "";
				
				СтруктураПараметров = Новый Структура;
				СтруктураПараметров.Вставить("КодПродукции", Ложь);
				СтруктураПараметров.Вставить("ВставитьКодПродукции", ВставитьКодПродукцииПолучатель);
				...
				ТекстЗапроса = РегламентированнаяОтчетностьАЛКО.ТекстЗапросаВыбораКонтрагентаАЛКО(
					ОсновнаяТаблица, ?(ПолучательБезЛицензии, Ложь, Истина), , Истина, СтруктураПараметров);
				ЗапросВидаЛицензии = Новый Запрос;
				ЗапросВидаЛицензии.Текст = ТекстЗапроса;
				
				ЗапросВидаЛицензии.Параметры.Вставить("ИскомыйКонтрагентСсылка", Запись.Получатель);
				
				Если ВставитьКодПродукцииПолучатель Тогда	
					ЗапросВидаЛицензии.Параметры.Вставить("КодПродукции", Запись.П000020000302);					
				КонецЕсли;
				...
				СтрокаРегистрСведенийЛицензииПолучателейАлкогольнойПродукции = ДинСписокПолучателя.ОсновнаяТаблица;
				
				ИзмерениеПолучатель = "";
				
				Если СтрНайти(ВРег(СтрокаРегистрСведенийЛицензииПолучателейАлкогольнойПродукции), 
								  ВРег("РегистрСведений.")) > 0 Тогда
					ИзмерениеПолучатель 	= "Поставщик";
					
				ИначеЕсли СтрНайти(ВРег(СтрокаРегистрСведенийЛицензииПолучателейАлкогольнойПродукции), 
								  ВРег("Справочник.")) > 0 Тогда
					ИзмерениеПолучатель 	= "Владелец";
				...
				Если КолСтрокРезультата > 0 Тогда
					...
					Если ИзмерениеПолучатель = "Владелец" Тогда
					...
					Если КолСтрокРезультата > 1 Тогда
						...
						Для Инд = 0 По КолСтрокРезультата - 1 Цикл
							...
							Если (СокрЛП(ТекСерияНомер) = СокрЛП(Запись.П00002СерияНомер))
								И (ТекДатаОкончания = Запись.П00002ДатаОкончания) Тогда
				...
				Если (НЕ ВидЛицензии = Неопределено) 
					...
					Если ИзмерениеПолучатель = "Поставщик" Тогда
								  
						Отбор = Новый Структура;
						Отбор.Вставить("Поставщик", Запись.Получатель);
						Отбор.Вставить("КодВидаПродукции", КодВидаПродукции);
						Отбор.Вставить("ВидЛицензии", ВидЛицензии);
						Отбор.Вставить("Период", ДатаНачала);
											
						ИмяРегистраСведенийЛицензииПолучателейАлкогольнойПродукции = 
							СтрЗаменить(ВРег(СтрокаРегистрСведенийЛицензииПолучателейАлкогольнойПродукции), 
								  		ВРег("РегистрСведений."), "");
						РегистрСведенийЛицензий = РегистрыСведений[ИмяРегистраСведенийЛицензииПолучателейАлкогольнойПродукции];
					...
					Элементы.ТаблицаПолучателей.ТекущаяСтрока = КлючЗаписи;
...
Процедура ТаблицаПроизводителейВыборНаСервере(Производитель, ПроизводительИНН, ПроизводительКПП)
	...
	Если НЕ (Запись.П000020000304 = СокрЛП(ПроизводительИНН)
		И Запись.П000020000305 = СокрЛП(ПроизводительКПП)
		И Запись.П000020000303 = СокрЛП(Производитель)) Тогда
	
		Запись.П000020000303Адрес = "";
		Запись.П000020000303АдресXML = "";
...
&НаСервере
Процедура ТаблицаПолучателейВыборНаСервере(СоответствиеТекущиеДанные)
	...
	ДатаВыдачи = СоответствиеТекущиеДанные.Получить("ДатаВыдачи");
	КемВыдана = СоответствиеТекущиеДанные.Получить("КемВыдана");
		
	Если НЕ (Запись.П000020000308 = СокрЛП(ИНН)
		И Запись.П000020000309 = СокрЛП(КПП)
		И Запись.П000020000306 = СокрЛП(НаименованиеПолное)) Тогда
	
		Запись.П000020000307 = "";
		Запись.П000020000307XML = "";
				
		Запись.П000020000308 = СокрЛП(ИНН);
		Запись.П000020000309 = СокрЛП(КПП);	
		Запись.П000020000306 = СокрЛП(НаименованиеПолное);
		
		Запись.П000020000310 = СокрЛП(ВидДеятельности);
		
		Запись.П00002ДатаОкончания = ДатаОкончания;
		Запись.П00002ДатаВыдачи = ДатаВыдачи;
		Запись.П00002СерияНомер = СокрЛП(СерияНомер);
		Запись.П00002КемВыдана = СокрЛП(КемВыдана);
...
&НаСервере
Процедура ИнициализацияДинСпискаПолучателя()

	ВставитьКодПродукцииПолучатель = Ложь;
	ОсновнаяТаблица = "";
	...
	ТекстЗапроса = РегламентированнаяОтчетностьАЛКО.ТекстЗапросаВыбораКонтрагентаАЛКО(
									ОсновнаяТаблица, ?(ПолучательБезЛицензии, Ложь, Истина), , , СтруктураПараметров);
									
	СтруктураПараметров.Свойство("ВставитьКодПродукции", ВставитьКодПродукцииПолучатель);
	...
	Если СтрНайти(ТекстЗапроса, "&ДатаКонца") > 0 Тогда
		...
		ДинСписокПолучателя.Параметры.УстановитьЗначениеПараметра("ДатаНачала", '1000.01.01 00:00.00');
		ДинСписокПолучателя.Параметры.УстановитьЗначениеПараметра("ДатаКонца", '2099.12.31 23:59.59');
