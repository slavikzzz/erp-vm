	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не удалось сформировать задание к расчету стоимости ОС и НМА за %1 в организации %2 по причине: %3';
				|en = 'Cannot generate a job to calculate fixed assets and intangible assets value for %1 in company %2. Reason: %3'"),
			Выборка.Месяц,
			Выборка.Организация,
			ТекстОшибки);
...
Процедура СформироватьЗаданияНаСледующийПериод(НачалоСледующегоМесяца, СписокОрганизаций, МенеджерВременныхТаблиц, ИмяТаблицыОбъектов)
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ЕСТЬNULL(МИНИМУМ(ДанныеКРасчету.Период), ДАТАВРЕМЯ(1, 1, 1)) КАК Месяц,
	...
	Выборка = Результат.Выбрать();
	Пока Выборка.Следующий() Цикл
		НоваяЗапись = РегистрыСведений.ЗаданияКРасчетуСтоимостиВНА.СоздатьМенеджерЗаписи();
