	...
	Пока ОбновляемыеДанные.Следующий() Цикл
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru = 'Не удалось обработать запись по торговому предложению %1 по причине: %2%3';
									|en = 'Cannot process the record for the %1 selling proposition due to: %2%3'");
...
Процедура ИзменитьКратностьУпаковкиЗаписи(ПрайсЛист, ДанныеЗаписи, Отказ = Ложь) Экспорт
	...
	Исключение
		...
		ВидОперации = 
			НСтр("ru = 'Изменение кратности упаковки торгового предложения';
				|en = 'Change selling proposition packaging multiplicity'", ОбщегоНазначения.КодОсновногоЯзыка());
