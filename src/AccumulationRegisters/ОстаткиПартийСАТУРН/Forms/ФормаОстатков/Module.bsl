		...
		Если ЗначениеЗаполнено(Строка.Значение.Данные.ОрганизацияСАТУРН) Тогда
			...
			Если Оформление <> Неопределено Тогда
				...
				Если Не ЗначениеЗаполнено(Строка.Значение.Данные.КонтрагентОрганизация) Тогда
					Оформление.УстановитьЗначениеПараметра(
						"Текст", СтрШаблон(НСтр("ru = '%1 <не сопоставлен>';
												|en = '%1 <не сопоставлен>'"), Строка.Значение.Данные.ОрганизацияСАТУРН));
					Оформление.УстановитьЗначениеПараметра(
						"ЦветТекста", ЦветТребуетВнимания);
				Иначе
					Оформление.УстановитьЗначениеПараметра(
						"Текст",  СтрШаблон(НСтр("ru = '%1 (%2)';
												|en = '%1 (%2)'"), Строка.Значение.Данные.ОрганизацияСАТУРН, Строка.Значение.Данные.КонтрагентОрганизация));
				КонецЕсли;
		...
		Если ЗначениеЗаполнено(Строка.Значение.Данные.МестоХранения) Тогда
			...
			Если Оформление <> Неопределено Тогда
				...
				ИначеЕсли Не ЗначениеЗаполнено(Данные) Тогда
					Оформление.УстановитьЗначениеПараметра(
						"Текст", СтрШаблон(НСтр("ru = '%1 <не сопоставлен>';
												|en = '%1 <не сопоставлен>'"), Строка.Значение.Данные.МестоХранения));
					Оформление.УстановитьЗначениеПараметра(
						"ЦветТекста", ЦветТребуетВнимания);
				Иначе
					Оформление.УстановитьЗначениеПараметра(
						"Текст",  СтрШаблон(НСтр("ru = '%1 (%2)';
												|en = '%1 (%2)'"), Строка.Значение.Данные.МестоХранения, Данные));
				КонецЕсли;
...
Процедура СписокВыборЗначения(Элемент, Значение, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	Если ОбщегоНазначенияИСКлиент.ВыборСтрокиСпискаКорректен(Элементы.Список, Истина) Тогда
		...
		Если УпаковкиИспользуются И ЗначениеЗаполнено(ТипИзмеряемойВеличины) Тогда
			Если Не ЗначениеЗаполнено(КоэффициентПересчетаВУпаковкуКилограммы) 
				И ТекущиеДанные.ТипИзмеряемойВеличины = ПредопределенноеЗначение("Перечисление.ТипыИзмеряемыхВеличинСАТУРН.Вес") Тогда
					ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Недоступно использование партий в килограммах: нет пересчета из единицы хранения номенклатуры';
																	|en = 'Недоступно использование партий в килограммах: нет пересчета из единицы хранения номенклатуры'"),,"Список");
					Возврат;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(КоэффициентПересчетаВУпаковкуЛитры) 
				И ТекущиеДанные.ТипИзмеряемойВеличины = ПредопределенноеЗначение("Перечисление.ТипыИзмеряемыхВеличинСАТУРН.Объем") Тогда
					ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Недоступно использование партий в литрах: нет пересчета из единицы хранения номенклатуры';
																	|en = 'Недоступно использование партий в литрах: нет пересчета из единицы хранения номенклатуры'"),,"Список");
					Возврат;
			КонецЕсли;
...
Процедура НастроитьСписокДляВыбора()
	...
	Если ЗначениеЗаполнено(Параметры.Номенклатура) Тогда
		...
		Элементы.СписокДоступныйОстатокУпаковок.Заголовок = СтрШаблон(
			НСтр("ru = 'Доступный остаток, %1 %2';
				|en = 'Доступный остаток, %1 %2'"), Параметры.Номенклатура, Параметры.Упаковка);
...
Процедура УстановитьБыстрыйОтборСервер()
	...
	ИначеЕсли Параметры.Отбор.Свойство("ОрганизацияСАТУРН") Тогда
		...
		ОрганизацииСАТУРНПредставление = СтрШаблон(НСтр("ru = '%1 (%2)';
														|en = '%1 (%2)'"), Строка(ОрганизацияСАТУРН), Строка(МестоХранения));
...
Процедура ПредложитьВыбратьТипПартии()
	...
	СписокВыбора.Добавить(Ключ, СтрШаблон(НСтр("ru = 'Партия в килограммах (%1 %2)';
												|en = 'Партия в килограммах (%1 %2)'"), КоличествоВес, НаименованиеУпаковки));
	...
	СписокВыбора.Добавить(Ключ, СтрШаблон(НСтр("ru = 'Партия в литрах (%1 %2)';
												|en = 'Партия в литрах (%1 %2)'"), КоличествоОбъем, НаименованиеУпаковки));
	СписокВыбора.Добавить(КодВозвратаДиалога.Отмена);
	Оповещение = Новый ОписаниеОповещения("ПриВыбореТипаПартии", ЭтотОбъект);
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Укажите тип единицы измерения выбранной партии';
									|en = 'Укажите тип единицы измерения выбранной партии'"), СписокВыбора);
