		...
		Для Каждого СтрокаОрганизация Из ТаблицаОрганизаций Цикл
			...
			Если ТаблицаДвижений.Количество() > 0 Тогда
				...
				КорректировкаРегистров.Комментарий = НСтр("ru = 'Создан автоматически при зачете встречных резервов товаров организаций.';
															|en = 'Created automatically while setting off counter reserves of company goods.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Возникла ошибка зачете встречных резервов: %1';
				|en = 'An error occurred when offsetting counter reserves: %1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
		);
		
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Зачет встречных резервов товаров организаций';
										|en = 'Set off counter reserves of company goods'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Если Не ВыборкаРегистратора.Следующий() Тогда
			...
			Если ЕстьЗаписиСторно() Тогда
				ТекстИсключения = НСтр("ru = 'Не удалось зачесть все сторно записи резервов товаров организаций. Подробнее - см. Журанал регистрации';
										|en = 'Cannot set off all storno entries of company goods reserves. For more information, see the event log'");
...
Функция СвернутьДвиженияПоРезервамТоваровОрганизацийПоТекущемуРегистраторуСторно(ТекущийРегистраторСторно)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось свернуть резервы товаров организаций по регистратору сторно %ТекущийРегистраторСторно% по причине ""%ПричинаОшибки%"".';
								|en = 'Cannot roll up company goods reserves by storno recorder %ТекущийРегистраторСторно%. Reason: %ПричинаОшибки%.'");
...
Процедура ЗаписатьОшибкуСверткиВЖР(ТекстСообщения)
	...
	ИмяСобытия = НСтр("ru = 'Свертка движений по резервам товаров организаций';
						|en = 'Roll up records by company goods reserves'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Попытка
		...
		Если ТаблицаНовыеРезервы.Количество() > 0 Тогда
			...
			КорректировкаРегистров.Комментарий = НСтр("ru = 'Создан автоматически при распределении резервов товаров организаций по периодам.';
														|en = 'Created automatically while allocating company goods reserves by periods.'",
		...
		Если ЛишниеРезервы.Количество() > 0 Тогда
			...
			Для Каждого СтрТабл Из ЛишниеРезервы Цикл
				...
				Если НераспределенныйЛишнийРезерв <> 0 Тогда
					ТекстИсключения = НСтр("ru = 'Ошибка при распределении лишних резервов. Обратитесь к разработчикам.';
											|en = 'An error occurred while allocating excessive reserves. Contact the developers.'");
					ВызватьИсключение ТекстИсключения;
				КонецЕсли;
			...
			Если ТаблицаДвижений.Количество() > 0 Тогда
				...
				КорректировкаРегистров.Комментарий = НСтр("ru = 'Создан автоматически при сторнированнии лишнего резерва.';
															|en = 'Created automatically while reversing an excessive reserve.'",
				ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Возникла ошибка при переносе резервов товаров организаций за период %1: %2';
				|en = 'An error occurred when transferring company goods reserves for the period %1: %2'"),
...
Функция ИмяСобытияПереносаРезервов() Экспорт
	Возврат НСтр("ru = 'Перенос резервов товаров организаций';
				|en = 'Transfer company goods reserves'", ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
...
Процедура ДоформироватьРезервыДляПокрытияЛишнихСторно()
	...
	Попытка
		...
		КорректировкаРегистров.Комментарий = НСтр("ru = 'Создан автоматически при удалении лишних сторно-резервов.';
													|en = 'Created automatically while deleting excess reserve storno.'",
			ОбщегоНазначения.КодОсновногоЯзыка()
		);
