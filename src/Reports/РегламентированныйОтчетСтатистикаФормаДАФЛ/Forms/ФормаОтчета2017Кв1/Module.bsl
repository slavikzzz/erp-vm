...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "22/09/2017";
	...
	СтруктураРеквизитовФормы.мИдГруппы2 = "П0000001015";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ПолеТабличногоДокументаФормаОтчета.Области["П000000101502_1"].Значение = "12";
	ПолеТабличногоДокументаФормаОтчета.Области["П000000101502_2"].Значение = "13";
	ПолеТабличногоДокументаФормаОтчета.Области["П000000101502_3"].Значение = "14";
...
Процедура УдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, ТекущаяСтраница = Неопределено) Экспорт
	...
	Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000000101502_1"].Значение = "12";
	Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000000101502_2"].Значение = "13";
	Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000000101502_3"].Значение = "14";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 4 Цикл 
		Области["П00000200010"+Гр].Значение = Области["П00000200020"+Гр].Значение + Области["П00000200030"+Гр].Значение 
											+ Области["П00000200080"+Гр].Значение + Области["П00000200100"+Гр].Значение
											+ Области["П00000200110"+Гр].Значение;
	КонецЦикла;
	
	// Гр. 1 = сумме граф с 2 по 6.
	// (Несмотря на отсутствие прямых указаний в приказе, это очевидный факт.)
	Для НомСтр = 1 По Форма.П0001020001.Количество() Цикл
		Области["П000102000103_" + Формат(НомСтр, "ЧГ=")].Значение
			...
			+ Области["П000102000108_" + Формат(НомСтр, "ЧГ=")].Значение;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ИменаЭлементовУправленияСтроками.Вставить("ДобавлениеСтроки2", "УдалитьСтроку" + СтруктураРеквизитовФормы.мИдГруппы2 + "02");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000102000102", 250, , "Выбор территории",
		КоллекцияСписковВыбора["СправочникТерриторий"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000000101501", 250, , "Выбор ОКВЭД2",
		КоллекцияСписковВыбора["ОКВЭД"]);
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(КолонкаПоиска, Формат("" "ЧГ=")));
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Имя = СтруктураРеквизитовФормы.мИдГруппы2 Тогда 
		ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы2 + "01" + Суффикс].Значение = ?(ЗначениеЗаполнено(ВыборКод), СокрЛП(ВыборКод), "");
	Иначе 
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2017Кв1");
...
Процедура ДополнительныеПоказателиДАФЛ(ТекСтруктура)
	...
	ТЗ.Колонки.Добавить("П000000101501", Новый ОписаниеТипов("Строка"));
	ТЗ.Колонки.Добавить("П000000101503", Новый ОписаниеТипов("Число"));
	ТЗ.Колонки.Добавить("П000000101504", Новый ОписаниеТипов("Число"));
	
	ВсеКоды = "ABCDEFGHIJKLMNOPQRSU";
	Пока ЗначениеЗаполнено(ВсеКоды) Цикл
		НовСтр = ТЗ.Добавить();
		НовСтр.П000000101501 = Лев(ВсеКоды, 1);
		ВсеКоды = Сред(ВсеКоды, 2);
	КонецЦикла;
	
	Для Каждого Стр Из П0000001015 Цикл 
		НовСтр = ТЗ.Добавить();
		ЗаполнитьЗначенияСвойств(НовСтр, Стр);
	КонецЦикла;
	
	ТЗ.Свернуть("П000000101501", "П000000101503,П000000101504");
	Для Каждого Стр Из ТЗ Цикл 
		Если ЗначениеЗаполнено(Стр.П000000101501) Тогда
			ТекСтруктура.Вставить("ПКод" + Стр.П000000101501 + "03", Стр.П000000101503);
			ТекСтруктура.Вставить("ПКод" + Стр.П000000101501 + "04", Стр.П000000101504);
		КонецЕсли;
	КонецЦикла;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Для Инд = 3 По 8 Цикл
		ПФ = "СуммГр" + Инд;
		ПараметрыВыгрузки.Вставить(ПФ, 0);
		Итер = 0;
		Пока Истина Цикл 
			Итер = Итер + 1;
			Если Не ТабДокумент.Свойство("П000102000101_" + Итер) Тогда 
				Прервать;
			КонецЕсли;
			ПараметрыВыгрузки[ПФ] = ПараметрыВыгрузки[ПФ] + ТабДокумент["П00010200010" + Инд + "_" + Итер]
		КонецЦикла;
	КонецЦикла;
...
Функция ПолучитьПараметрыИнициализацииУниверсальногоОтчета()
	Возврат УниверсальныйОтчетСтатистики.ПолучитьПараметрыИнициализацииУниверсальногоОтчета(ЭтаФорма, "АтрибВыгрузкиXML2017Кв1", "ШаблонРосстата2015Кв1", "СхемаВыгрузкиXML2017Кв1", Истина, Ложь);
КонецФункции
