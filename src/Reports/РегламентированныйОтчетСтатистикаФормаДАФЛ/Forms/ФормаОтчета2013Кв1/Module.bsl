...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "10/12/2013";
		...
		ИначеЕсли ТекГруппа = "П000006" И Сред(Область.Имя, 12, 2) = "02" Тогда
			...
			Если ОтмеченоФакторов = 3 И Область.Значение = "" Тогда
				...
				Сообщение.Текст = НСтр("ru = 'В разделе 6 можно отметить не более трех факторов.';
										|en = 'В разделе 6 можно отметить не более трех факторов.'");
				Сообщение.Сообщить();
				Возврат;
			КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для НомСтр = 1 По СтруктураГруппы.Количество() Цикл
		мФормаОтчета.Области["П000102000103_" + Формат(НомСтр, "ЧГ=")].Значение
			= мФормаОтчета.Области["П000102000104_" + Формат(НомСтр, "ЧГ=")].Значение
			+ мФормаОтчета.Области["П000102000105_" + Формат(НомСтр, "ЧГ=")].Значение
			+ мФормаОтчета.Области["П000102000106_" + Формат(НомСтр, "ЧГ=")].Значение
			+ мФормаОтчета.Области["П000102000107_" + Формат(НомСтр, "ЧГ=")].Значение;
	КонецЦикла;
	
	// Раздел 2 (считаем по данным из "Справочно").
	
	// Строка 1 гр.1 раздела 2  = сумме свободных строк по гр.1 раздела «Справочно».
	// Строка 2 гр.1 раздела 2  = сумме свободных строк по гр.2 раздела «Справочно».
	// Строка 3 гр.1 раздела 2  = сумме свободных строк по гр.3 раздела «Справочно».
	// Строка 5 гр.1 раздела 2  = сумме свободных строк по гр.4 раздела «Справочно».
	// Строка 10 гр.1 раздела 2 = сумме свободных строк по гр.5 раздела «Справочно».
	...
	Для НомСтр = 1 По СтруктураГруппы.Количество() Цикл
		ТекСуммаГр1 = ТекСуммаГр1 + мФормаОтчета.Области["П000102000103_" + Формат(НомСтр, "ЧГ=")].Значение;
		ТекСуммаГр2 = ТекСуммаГр2 + мФормаОтчета.Области["П000102000104_" + Формат(НомСтр, "ЧГ=")].Значение;
		ТекСуммаГр3 = ТекСуммаГр3 + мФормаОтчета.Области["П000102000105_" + Формат(НомСтр, "ЧГ=")].Значение;
		ТекСуммаГр4 = ТекСуммаГр4 + мФормаОтчета.Области["П000102000106_" + Формат(НомСтр, "ЧГ=")].Значение;
		ТекСуммаГр5 = ТекСуммаГр5 + мФормаОтчета.Области["П000102000107_" + Формат(НомСтр, "ЧГ=")].Значение;
	КонецЦикла;
	
	мФормаОтчета.Области["П000002000103"].Значение = ТекСуммаГр1;
	мФормаОтчета.Области["П000002000203"].Значение = ТекСуммаГр2;
	мФормаОтчета.Области["П000002000303"].Значение = ТекСуммаГр3;
	мФормаОтчета.Области["П000002000503"].Значение = ТекСуммаГр4;
	мФормаОтчета.Области["П000002001003"].Значение = ТекСуммаГр5;
	...
	Для НомСтр = 2 По 9 Цикл
		ТекСумма = ТекСумма + мФормаОтчета.Области["П000007" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01"].Значение
	КонецЦикла;
	мФормаОтчета.Области["П000007000101"].Значение = ТекСумма;
	
	мФормаОтчета.Области["П000007001001"].Значение
		= мФормаОтчета.Области["П000007001101"].Значение
		+ мФормаОтчета.Области["П000007001201"].Значение;
	
	ТекСумма = 0;
	Для НомСтр = 15 По 22 Цикл
		ТекСумма = ТекСумма + мФормаОтчета.Области["П000007" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01"].Значение
	КонецЦикла;
	мФормаОтчета.Области["П000007001401"].Значение = ТекСумма;
	
	ТекСумма = 0;
	Для НомСтр = 24 По 32 Цикл
		ТекСумма = ТекСумма + мФормаОтчета.Области["П000007" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01"].Значение
	КонецЦикла;
	мФормаОтчета.Области["П000007002301"].Значение = ТекСумма;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
    ...
    ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "02" + Суффикс].Значение = ?(ЗначениеЗаполнено(ВыборКод), Число(СокрЛП(ВыборКод)), 0);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// КОНТРОЛЬ показателей формы (из текста "Указаний ФСН по заполнению Формы ДАФЛ").
	
	// Строка 1 раздела 2 = (стр. 2 + стр. 3 + стр. 5 + стр. 10) раздела 2.
	Если Не (ПараметрыВыгрузки["ПР00002000103"] = ПараметрыВыгрузки["ПР00002000203"] + ПараметрыВыгрузки["ПР00002000303"]
		+ ПараметрыВыгрузки["ПР00002000503"] + ПараметрыВыгрузки["ПР00002001003"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в разделе 2 по гр. 1 значение в стр. 1 должно быть равно сумме значений в строках: 2, 3, 5, 10!",
			ИмяПоляТаблДокумента, "П000002000103");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Строка 1 раздела 2 = сумме строк 1 - 10 раздела 3.
	// (Проверяем, согл. шаблону: Строка 1 раздела 2 = сумме строк 1 - 9 и 11 - 15 раздела 3.)
	Если Не (ПараметрыВыгрузки["ПР00002000103"] = ПараметрыВыгрузки["ПР00003000103"] + ПараметрыВыгрузки["ПР00003000203"]
		+ ПараметрыВыгрузки["ПР00003000303"] + ПараметрыВыгрузки["ПР00003000403"] + ПараметрыВыгрузки["ПР00003000503"]
		+ ПараметрыВыгрузки["ПР00003000603"] + ПараметрыВыгрузки["ПР00003000703"] + ПараметрыВыгрузки["ПР00003000803"]
		+ ПараметрыВыгрузки["ПР00003000903"] + ПараметрыВыгрузки["ПР00003001103"] + ПараметрыВыгрузки["ПР00003001203"]
		+ ПараметрыВыгрузки["ПР00003001303"] + ПараметрыВыгрузки["ПР00003001403"] + ПараметрыВыгрузки["ПР00003001503"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: значение в стр. 1 раздела 2 должно быть равно сумме значений в строках раздела 3: 1 - 9, 11 - 15!",
			ИмяПоляТаблДокумента, "П000002000103");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Строка 3 раздела 2 = сумме строк 11 - 21 раздела 2.
	// (Проверяем, согл. шаблону: Строка 3 раздела 2 = сумме строк 11 - 20 и 22 - 27 раздела 2.)
	Если Не (ПараметрыВыгрузки["ПР00002000303"] = ПараметрыВыгрузки["ПР00002001103"] + ПараметрыВыгрузки["ПР00002001203"]
		+ ПараметрыВыгрузки["ПР00002001303"] + ПараметрыВыгрузки["ПР00002001403"] + ПараметрыВыгрузки["ПР00002001503"]
		+ ПараметрыВыгрузки["ПР00002001603"] + ПараметрыВыгрузки["ПР00002001703"] + ПараметрыВыгрузки["ПР00002001803"]
		+ ПараметрыВыгрузки["ПР00002001903"] + ПараметрыВыгрузки["ПР00002002003"] + ПараметрыВыгрузки["ПР00002002203"]
		+ ПараметрыВыгрузки["ПР00002002303"] + ПараметрыВыгрузки["ПР00002002403"] + ПараметрыВыгрузки["ПР00002002503"]
		+ ПараметрыВыгрузки["ПР00002002603"] + ПараметрыВыгрузки["ПР00002002703"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в разделе 2 значение в стр. 3 должно быть равно сумме значений в строках: 11 - 20, 22 - 27!",
			ИмяПоляТаблДокумента, "П000002000303");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Строка 3 раздела 2 >= строке 4 раздела 2.
	Если Не (ПараметрыВыгрузки["ПР00002000303"] >= ПараметрыВыгрузки["ПР00002000403"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в разделе 2 значение в стр. 3 должно быть больше или равно значению в стр 4!",
			ИмяПоляТаблДокумента, "П000002000303");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Строка 5 раздела 2 >= сумме строк 6 - 9 раздела 2.
	Если Не (ПараметрыВыгрузки["ПР00002000503"] >= ПараметрыВыгрузки["ПР00002000603"] + ПараметрыВыгрузки["ПР00002000703"]
		+ ПараметрыВыгрузки["ПР00002000803"] + ПараметрыВыгрузки["ПР00002000903"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в разделе 2 значение в стр. 5 должно быть больше или равно сумме значений в строках: 6 - 9!",
			ИмяПоляТаблДокумента, "П000002000503");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если строка 1 раздела 7  > 0, то строки 10, 13 и 23 раздела 7 > 0;
	Если ПараметрыВыгрузки["ПР00007000101"] > 0 Тогда
		Если Не (ПараметрыВыгрузки["ПР00007001001"] > 0) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе 7 значение в стр. 1 больше нуля, то значение в стр. 10 должно быть больше нуля!",
				ИмяПоляТаблДокумента, "П000007001001");
			СтатусПроверки = Ложь;
		КонецЕсли;
		Если Не (ПараметрыВыгрузки["ПР00007001301"] > 0) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе 7 значение в стр. 1 больше нуля, то значение в стр. 13 должно быть больше нуля!",
				ИмяПоляТаблДокумента, "П000007001301");
			СтатусПроверки = Ложь;
		КонецЕсли;
		Если Не (ПараметрыВыгрузки["ПР00007002301"] > 0) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе 7 значение в стр. 1 больше нуля, то значение в стр. 23 должно быть больше нуля!",
				ИмяПоляТаблДокумента, "П000007002301");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	// Строка 13 раздела 7 > строки 14 раздела 7;
	Если Не (ПараметрыВыгрузки["ПР00007001301"] >= ПараметрыВыгрузки["ПР00007001401"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в разделе 7 значение в стр. 13 должно быть больше или равно значению в стр 14!",
			ИмяПоляТаблДокумента, "П000007001301");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если строка 11 раздела 7  > 0, то строки 15 и 27 раздела 7 > 0;
	Если ПараметрыВыгрузки["ПР00007001101"] > 0 Тогда
		Если Не (ПараметрыВыгрузки["ПР00007001501"] > 0) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе 7 значение в стр. 11 больше нуля, то значение в стр. 15 должно быть больше нуля!",
				ИмяПоляТаблДокумента, "П000007001501");
			СтатусПроверки = Ложь;
		КонецЕсли;
		Если Не (ПараметрыВыгрузки["ПР00007002701"] > 0) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе 7 значение в стр. 11 больше нуля, то значение в стр. 27 должно быть больше нуля!",
				ИмяПоляТаблДокумента, "П000007002701");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	// Строка 27 раздела 7 ≥ строке 15 раздела 7.
	Если Не (ПараметрыВыгрузки["ПР00007002701"] >= ПараметрыВыгрузки["ПР00007001501"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в разделе 7 значение в стр. 27 должно быть больше или равно значению в стр 15!",
			ИмяПоляТаблДокумента, "П000007002701");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// Дополнительные КС (из шаблона для выгрузки).
	
	// Если в разд. 2 стр. 1 > 0, то в разд. 1 стр. 1 > 0.
	Если ПараметрыВыгрузки["ПР00002000103"] > 0 Тогда
		Если Не (ПараметрыВыгрузки["ПР00001000000"] > 0) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе 2 значение в стр. 1 больше нуля, то значение в разделе 1 должно быть больше нуля!",
				ИмяПоляТаблДокумента, "П000001000000");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	// В разд. 2 стр. 5 = сумме стр. с 6 по 9.
	Если Не (ПараметрыВыгрузки["ПР00002000503"] = ПараметрыВыгрузки["ПР00002000603"] + ПараметрыВыгрузки["ПР00002000703"]
		+ ПараметрыВыгрузки["ПР00002000803"] + ПараметрыВыгрузки["ПР00002000903"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в разделе 2 значение в стр. 5 должно быть равно сумме значений в строках: 6 - 9!",
			ИмяПоляТаблДокумента, "П000002000503");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	// Для ПОСЛЕДУЮЩЕГО (ПЕРЕД ВЫГРУЗКОЙ) контроля показателей формы (Указания ФСН по заполнению Формы ДАФЛ).
	// Раздел 1.
	ПараметрыВыгрузки.Вставить("ПР00001000000", ТабДокумент["П000001000000"]);
	// Раздел 2.
	НомСтр = 1;
	...
	// Раздел 3.
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03") Цикл
		ПараметрыВыгрузки.Вставить("ПР00003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03",
			ТабДокумент["П000003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]);
		НомСтр = НомСтр + 1;
	КонецЦикла;
	// Раздел 7.
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000007" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01") Цикл
		ПараметрыВыгрузки.Вставить("ПР00007" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01",
			ТабДокумент["П000007" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01"]);
		НомСтр = НомСтр + 1;
	КонецЦикла;
