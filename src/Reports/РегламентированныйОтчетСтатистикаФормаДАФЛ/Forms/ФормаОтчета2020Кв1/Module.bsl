...
Процедура УстановитьЗначокВГруппеЯчеек(Область, НачальнаяСтрокаВГруппе, КонечнаяСтрокаВГруппе, НачальнаяКолонкаВГруппе, КонечнаяКолонкаВГруппе, МножественныйВыбор = Ложь)
	...
	НовЗначениеОбласти = ?(Область.Значение = "", "Х", "");
...
Процедура ПеренумероватьСтроки(ПолеТабличногоДокументаФормаОтчета)
	...
	Пока Истина Цикл 
		...
		Если ПолеТабличногоДокументаФормаОтчета.Области.Найти("П000010101202_" + Инд) = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		ПолеТабличногоДокументаФормаОтчета.Области["П000010101202_" + Инд].Значение = "12";
	КонецЦикла;
КонецПроцедуры
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Для Каждого Эл Из ТабДокумент Цикл
		...
		Если Эл.Ключ = "П000010100001" Или ИмяПарам = "П00004" Или ИмяПарам = "П00005" Или ИмяПарам = "П00006" Тогда
			ТабДокумент.Вставить(Эл.Ключ, ?(Эл.Значение = "Х", 1, 0));
		КонецЕсли;
	КонецЦикла;
