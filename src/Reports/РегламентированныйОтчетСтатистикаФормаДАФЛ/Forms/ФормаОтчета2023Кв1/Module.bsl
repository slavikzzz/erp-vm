...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли РегламентированнаяОтчетностьКлиент.ВыбраннаяОбластьРегламентированногоОтчетаНеВерна(ЭтаФорма, Область, СтандартнаяОбработка) Тогда
		Возврат;
	ИначеЕсли Область.Имя = "П000010100101" Тогда
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	//Строка 05 раздела 4 = сумме данных строк 06 – 13 раздела 4
	ТекСумма = 0;
	Для Стр = 6 По 13 Цикл
		НомСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
		ТекСумма = ТекСумма + Области["П00400100" + НомСтр + "01"].Значение;
	КонецЦикла;
	Области["П004001000501"].Значение = ТекСумма;
	
	//Строка 14 раздела 4 = сумме строк 15 – 23, 26 – 28 раздела 4
	ТекСумма = 0;
	Для Стр = 15 По 28 Цикл
		Если Стр = 24 Или Стр = 25 Тогда
			Продолжить;
		КонецЕсли;
		ТекСумма = ТекСумма + Области["П00400100" + Стр + "01"].Значение;
	КонецЦикла;
	Области["П004001001401"].Значение = ТекСумма;
	
	//Строка 23 раздел 4 = сумме подстрок 24 – 25 раздела 4
	ТекСумма = 0;
	Для Стр = 24 По 25 Цикл
		ТекСумма = ТекСумма + Области["П00400100" + Стр + "01"].Значение;
	КонецЦикла;
	Области["П004001002301"].Значение = ТекСумма;
	
	//Строка 30 раздел 4 = сумме строк 31 – 32 раздел 4
	ТекСумма = 0;
	Для Стр = 31 По 32 Цикл
		ТекСумма = ТекСумма + Области["П00400100" + Стр + "01"].Значение;
	КонецЦикла;
	Области["П004001003001"].Значение = ТекСумма;
	
	//Строка 33 раздел 5 = сумме строк 34 – 44 раздела 5
	ТекСумма = 0;
	Для Стр = 34 По 44 Цикл
		ТекСумма = ТекСумма + Области["П00500100" + Стр + "01"].Значение;
	КонецЦикла;
	Области["П005001003301"].Значение = ТекСумма;
	
	//Строка 45 раздела 5 = сумме строк 46 – 49 раздела 5
	ТекСумма = 0;
	Для Стр = 46 По 49 Цикл
		ТекСумма = ТекСумма + Области["П00500100" + Стр + "01"].Значение;
	КонецЦикла;
	Области["П005001004501"].Значение = ТекСумма;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П004001000501"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П004001000501"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П004001001401"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П004001001401"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П004001002301"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П004001002301"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П004001003001"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П004001003001"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П005001003301"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П005001003301"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П005001004501"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П005001004501"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("ИНН", СокрЛП(ТабДокумент.ОргКодЗнач));
