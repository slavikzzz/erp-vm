...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0000101012";
	СтруктураРеквизитовФормы.мИдГруппы2 = "П0000102001";
	СтруктураРеквизитовФормы.мИдГруппы3 = "П0000301009";
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	Для НомерГрафы = 1 По Форма.мСтруктураКолвоКолонокРазделов[ИмяГруппы] Цикл
		...
		Если СтруктураГруппы[0].Свойство(ИмяКолонки) Тогда
			...
			Если НомерГрафы = 1 Тогда
				НомерПервойКолонки = ?(ИмяГруппы = "П0000101012", 3, 2);
	...
	ПеренумероватьСтроки(ТекТабличноеПоле);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Область.Имя = "П000010100001" Тогда
			СтандартнаяОбработка = Ложь;
			УстановитьЗначокВГруппеЯчеек(Область, 0, 0, 1, 1);
			Модифицированность = Истина;
		ИначеЕсли ТекГруппа = "П00004" Тогда
			СтандартнаяОбработка = Ложь;
			НомерСтроки = Число(Сред(Область.Имя, 10, 2));
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 1, 4);
			Модифицированность = Истина;
		ИначеЕсли ТекГруппа = "П00005" Тогда
			СтандартнаяОбработка = Ложь;
			НомерСтроки = Число(Сред(Область.Имя, 10, 2));
			Если СтрНайти("123", Прав(Область.Имя, 1)) > 0 Тогда
				УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 1, 3);
			ИначеЕсли СтрНайти("456", Прав(Область.Имя, 1)) > 0 Тогда
		...
		ИначеЕсли ТекГруппа = "П00006" Тогда
			СтандартнаяОбработка = Ложь;
			УстановитьЗначокВГруппеЯчеек(Область, 1, 10, 1, 1, Истина);
			Модифицированность = Истина;
		Иначе
			...
			ПараметрВводаПоля =
				РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, Лев(Область.Имя, 13));
...
Процедура УстановитьЗначокВГруппеЯчеек(Область, НачальнаяСтрокаВГруппе, КонечнаяСтрокаВГруппе, НачальнаяКолонкаВГруппе, КонечнаяКолонкаВГруппе, МножественныйВыбор = Ложь)
	...
	Если Лев(Область.Имя, 6) = "П00006" И НовЗначениеОбласти <> "" Тогда
		КолВТаб = 0;
		МассОбластей = Новый Массив;
		Для Н=1 По 10 Цикл
			ОтмеченнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области["П00006010" + Формат(Н, "ЧЦ=2; ЧВН=") + "01"];
			Если ЗначениеЗаполнено(ОтмеченнаяОбласть.Значение) Тогда
				МассОбластей.Добавить(ОтмеченнаяОбласть);
				КолВТаб = КолВТаб + 1;
			КонецЕсли;
		КонецЦикла;
		Если МассОбластей.Количество() = 3 Тогда
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 4 Цикл
		Области["П00001010010" + Гр].Значение = Области["П00001010020" + Гр].Значение
			+ Области["П00001010030" + Гр].Значение + Области["П00001010080" + Гр].Значение
			+ Области["П00001010100" + Гр].Значение + Области["П00001010110" + Гр].Значение;
	КонецЦикла;
	
	// Гр. 1 = сумме граф с 2 по 6.
	// (Несмотря на отсутствие прямых указаний в приказе, это очевидный факт.)
	Для НомСтр = 1 По Форма.П0000102001.Количество() Цикл
		Области["П000010200103_" + Формат(НомСтр, "ЧГ=")].Значение
			= Области["П000010200104_" + Формат(НомСтр, "ЧГ=")].Значение
			+ Области["П000010200105_" + Формат(НомСтр, "ЧГ=")].Значение
			+ Области["П000010200106_" + Формат(НомСтр, "ЧГ=")].Значение
			+ Области["П000010200107_" + Формат(НомСтр, "ЧГ=")].Значение
			+ Области["П000010200108_" + Формат(НомСтр, "ЧГ=")].Значение;
	КонецЦикла;
	
	ТекСумма = 0;
	Для НомСтр = 2 По 9 Цикл
		ТекСумма = ТекСумма + Области["П000070100" + НомСтр + "01"].Значение;
	КонецЦикла;
	Области["П000070100101"].Значение = ТекСумма;
	
	Области["П000070101001"].Значение
		= Области["П000070101101"].Значение
		+ Области["П000070101201"].Значение;
	
	ТекСумма = 0;
	Для НомСтр = 15 По 22 Цикл
		ТекСумма = ТекСумма + Области["П00007010" + НомСтр + "01"].Значение
	КонецЦикла;
	Области["П000070101401"].Значение = ТекСумма;
	
	ТекСумма = 0;
	Для НомСтр = 24 По 32 Цикл
		ТекСумма = ТекСумма + Области["П00007010" + НомСтр + "01"].Значение
	КонецЦикла;
	Области["П000070102301"].Значение = ТекСумма;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 3 По 4 Цикл
		мФормаОтчета.Области["П00001010010" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00001010010" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для НомСтр = 1 По ЭтотОбъект.П0000102001.Количество() Цикл
		мФормаОтчета.Области["П000010200103_" + НомСтр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П000010200103_" + НомСтр].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	мФормаОтчета.Области["П000070100101"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000070100101"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000070101001"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000070101001"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000070101401"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000070101401"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000070102301"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000070102301"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Процедура ПеренумероватьСтроки(ПолеТабличногоДокументаФормаОтчета)
	...
	Пока Истина Цикл 
		Инд = Инд + 1;
		Если ПолеТабличногоДокументаФормаОтчета.Области.Найти("П000010101202_" + Инд) = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		ПолеТабличногоДокументаФормаОтчета.Области["П000010101202_" + Инд].Значение = Инд + 11;
	КонецЦикла;
КонецПроцедуры
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма ДАФЛ");
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть2_1"));
	ПечатнаяФорма.УдалитьОбласть(ПечатнаяФорма.Области.НепечатаемаяОбласть, ТипСмещенияТабличногоДокумента.ПоВертикали);
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010200101",
		250, , "Выбор территории", КоллекцияСписковВыбора["СправочникТерриторий"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010200102",
		250, , "Выбор территории", КоллекцияСписковВыбора["СправочникТерриторий"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010101201",
		250, , "Выбор ОКВЭД2", КоллекцияСписковВыбора["ОКВЭД"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000030100901",
		250, , "Выбор источника инвестиций", КоллекцияСписковВыбора["ИсточникиИнвестиций"]);
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Лев(Область.Имя, 8) = "П0000101" Тогда
		ИсходноеЗначение = Область.Значение;
	ИначеЕсли Лев(Область.Имя, 8) = "П0000102" Тогда
		ИсходноеЗначение = ПолеТабличногоДокументаФормаОтчета.Области["П000010200102" + Суффикс].Значение;
	ИначеЕсли Лев(Область.Имя, 8) = "П0000301" Тогда
		ИсходноеЗначение = ПолеТабличногоДокументаФормаОтчета.Области["П000030100902" + Суффикс].Значение;
	КонецЕсли;
	...
	ДополнительныеПараметры = Новый Структура("Суффикс, Имя", Суффикс, Лев(Область.Имя, 11));
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Имя = СтруктураРеквизитовФормы.мИдГруппы1 Тогда 
		ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "01" + Суффикс].Значение
			= ?(ЗначениеЗаполнено(ВыборКод), СокрЛП(ВыборКод), "");
	ИначеЕсли ДополнительныеПараметры.Имя = СтруктураРеквизитовФормы.мИдГруппы2 Тогда
		ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы2 + "01" + Суффикс].Значение
			= ?(ЗначениеЗаполнено(ВыборКод), СокрЛП(ВыборНазвание), "");
		ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы2 + "02" + Суффикс].Значение
			= ?(ЗначениеЗаполнено(ВыборКод), СокрЛП(ВыборКод), "");
	ИначеЕсли ДополнительныеПараметры.Имя = СтруктураРеквизитовФормы.мИдГруппы3 Тогда
		ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы3 + "01" + Суффикс].Значение
			= ?(ЗначениеЗаполнено(ВыборКод), СокрЛП(ВыборНазвание), "");
		ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы3 + "02" + Суффикс].Значение
			= ?(ЗначениеЗаполнено(ВыборКод), СокрЛП(ВыборКод), "");
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Для Каждого Эл Из ТабДокумент Цикл
		ИмяПарам = Лев(Эл.Ключ, 6);
		Если Эл.Ключ = "П000010100001" Или ИмяПарам = "П00004" Или ИмяПарам = "П00005" Или ИмяПарам = "П00006" Тогда
			ТабДокумент.Вставить(Эл.Ключ, ?(Эл.Значение = "1", 1, 0));
		КонецЕсли;
	КонецЦикла;
