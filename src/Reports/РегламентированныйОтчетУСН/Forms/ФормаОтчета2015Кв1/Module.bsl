		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
			...
			Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел3" И ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Форма.ТабличныйДокумент.Области["П000030000107_" + НомСтроки].Значение
					= Макс(Окр_П000030000103 - Окр_П000030000104 - Окр_П000030000106, 0);
...
Процедура ОткорректироватьСуммыСтраховыхВзносов(ИмяРаздела, ИмяПоказателя)
	...
	Иначе
		...
		Для НомСтр = 0 По 3 Цикл
			СуммаНалога = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
				ЭтотОбъект, СтруктураДанныхРаздел21["П002120013" + НомСтр + "03"]);
			СуммаВзносов = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
				ЭтотОбъект, СтруктураДанныхРаздел21["П002120014" + НомСтр + "03"]);
			МаксимальнаяСуммаНалога = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
				ЭтотОбъект, СуммаНалога * Коэффициент);
			Если СуммаВзносов > МаксимальнаяСуммаНалога Тогда
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
		...
		Если НЕ ПустаяСтрока(Область.Значение) И НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(Область.Значение) Тогда
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД!%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра)';
					|en = 'На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД!%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра)'"), Символы.ПС, Символы.ПС);
			Сообщение.Сообщить();
	...
	Если Лев(ИмяПоказателя, 10) = "П002110012" Тогда
		Если ЗначПоказателя > 6 Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Значение ставки налога не может быть больше 6%';
									|en = 'Значение ставки налога не может быть больше 6%'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
	...
	Если Лев(ИмяПоказателя, 10) = "П000220026" Тогда
		Если ЗначПоказателя > 15 Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Значение ставки налога не может быть больше 15%';
									|en = 'Значение ставки налога не может быть больше 15%'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
	...
	Если ИмяПоказателя = "П002110010203" Тогда
		...
		Если НЕ (ЗначПоказателя = "1" ИЛИ ЗначПоказателя = "2") Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В разделе 2.1.1 задано некорректное значение признака налогоплательщика!%1Допустимы значения 1 или 2';
					|en = 'В разделе 2.1.1 задано некорректное значение признака налогоплательщика!%1Допустимы значения 1 или 2'"), Символы.ПС);
			Сообщение.Сообщить();
	...
	Если ИмяПоказателя = "П002110010203"
		...
		Для НомСтр = 0 По 3 Цикл
			СуммаНалога = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
				ЭтотОбъект, ТабличныйДокумент.Области["П002110013" + НомСтр + "03"].Значение);
			СуммаВзносов = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
				ЭтотОбъект, ТабличныйДокумент.Области["П002110014" + НомСтр + "03"].Значение);
			МаксимальнаяСуммаНалога = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
				ЭтотОбъект, СуммаНалога * Коэффициент);
			Если СуммаВзносов > МаксимальнаяСуммаНалога Тогда
				МассивНомСтр211.Добавить(НомСтр);
		...
		Для НомСтр = 0 По 3 Цикл
			СуммаНалога = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
				ЭтотОбъект, ТабличныйДокумент.Области["П002120013" + НомСтр + "03"].Значение);
			СуммаВзносов = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
				ЭтотОбъект, ТабличныйДокумент.Области["П002120014" + НомСтр + "03"].Значение);
			МаксимальнаяСуммаНалога = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
				ЭтотОбъект, СуммаНалога * Коэффициент);
			Если СуммаВзносов > МаксимальнаяСуммаНалога Тогда
				МассивНомСтр212.Добавить(НомСтр);
...
Процедура ВопросСуммаУплаченныхНалоговУменьшаетЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если ИмяРаздела = "Раздел21" Тогда
		Если МассивНомСтр211.Количество() > 1 ИЛИ МассивНомСтр212.Количество() > 1 Тогда
			// Передается пустая строка вместо имени текущего показателя
			// для пересчета всех зависимых показателей в разделах 2.1 и 1.1.
...
Процедура ПроверитьЗаполнение_Раздел21(СтатусПроверки)
	...
	Окр_П002120016003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
		ЭтотОбъект, ДанныеРаздела.П002120016003);
	Окр_П002110013003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
		ЭтотОбъект, ДанныеРаздела.П002110013003);
	Окр_П002110014003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
		ЭтотОбъект, ДанныеРаздела.П002110014003);
	...
	Окр_П002120016103 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
		ЭтотОбъект, ДанныеРаздела.П002120016103);
	Окр_П002110013103 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
		ЭтотОбъект, ДанныеРаздела.П002110013103);
	Окр_П002110014103 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
		ЭтотОбъект, ДанныеРаздела.П002110014103);
	...
	Окр_П002120016203 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
		ЭтотОбъект, ДанныеРаздела.П002120016203);
	Окр_П002110013203 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
		ЭтотОбъект, ДанныеРаздела.П002110013203);
	Окр_П002110014203 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
		ЭтотОбъект, ДанныеРаздела.П002110014203);
	...
	Окр_П002120016303 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
		ЭтотОбъект, ДанныеРаздела.П002120016303);
	Окр_П002110013303 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
		ЭтотОбъект, ДанныеРаздела.П002110013303);
	Окр_П002110014303 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
		ЭтотОбъект, ДанныеРаздела.П002110014303);
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Если ПрименяетсяУСН(ЭтотОбъект) Тогда
		ВидОбъектаНалогообложения = КодВидаОбъектаНалогообложения(ЭтотОбъект);
		Если ВидОбъектаНалогообложения = 1 Тогда // (Доходы)
			// Раздел 1.1
			//
			...
			// Раздел 2.1.2
			//
			Если НЕ Раздел2_1_2_Пуст(мДанныеОтчета.ПолеТабличногоДокументаРаздел21) Тогда
				ПроверитьКС_1_31_1(СтруктураРезультата);
				ПроверитьКС_1_31_2(СтруктураРезультата);
				ПроверитьКС_1_31_3(СтруктураРезультата);
				ПроверитьКС_1_31_4(СтруктураРезультата);
				ПроверитьКС_1_31_5(СтруктураРезультата);
				ПроверитьКС_1_31_6(СтруктураРезультата);
				ПроверитьКС_1_31_7(СтруктураРезультата);
				ПроверитьКС_1_31_8(СтруктураРезультата);
				ПроверитьКС_1_31_9а(СтруктураРезультата);
				ПроверитьКС_1_31_9б(СтруктураРезультата);
				ПроверитьКС_1_31_10а(СтруктураРезультата);
				ПроверитьКС_1_31_10б(СтруктураРезультата);
				ПроверитьКС_1_31_11а(СтруктураРезультата);
				ПроверитьКС_1_31_11б(СтруктураРезультата);
				ПроверитьКС_1_31_12а(СтруктураРезультата);
				ПроверитьКС_1_31_12б(СтруктураРезультата);
			КонецЕсли;
		ИначеЕсли ВидОбъектаНалогообложения = 2 Тогда // (Доходы - расходы)
			// Раздел 1.2
			//
	...
	Иначе
		...
		ПроверитьКС_1_45(СтруктураРезультата);
	КонецЕсли;
	ПроверитьКС_1_46(СтруктураРезультата); // Для всех
...
&НаСервере
Функция ПроверитьКС_1_0(СтруктураРезультата)
	...
	ТекстФормулы   = "Проверка раздела 1.1 (КС 1.0)";
	ОписаниеКС     = "Сумма авансового платежа к уплате по строке 020 Раздела 1.1 должна равна (строка 130 - строка 140) Раздела 2.1.1 - строка 160 Раздела 2.1.2, если результат такого вычитания больше или равен 0";
	Нарушение      = "пп. 3, 3.1 и 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уплате по сроку не позднее 25-го апреля отчетного года";
	
	Р1_1_Стр020   = ПоказательПоСтроке("Р1_1_Стр020");
	Р2_1_1_Стр130 = ПоказательПоСтроке("Р2_1_1_Стр130");
	Р2_1_1_Стр140 = ПоказательПоСтроке("Р2_1_1_Стр140");
	Р2_1_2_Стр160 = ПоказательПоСтроке("Р2_1_2_Стр160");
	
	Раздел11 = мДанныеОтчета.ПолеТабличногоДокументаРаздел11;
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр020);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр130);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр140);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр160);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р1_1_Стр020,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр130, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр140, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр160, "Раздел21");
	
	КС = ПоказателиКС(Р1_1_Стр020.Значение, 0);
	
	ЗначениеУсловия = Р2_1_1_Стр130.Значение - Р2_1_1_Стр140.Значение - Р2_1_2_Стр160.Значение;
	ЗначениеУсловияСтр = ЧислоСтрЧГ0(ЗначениеУсловия);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " >= 0 и " + КС.ЛеваяЧастьСтр + " = " + Р2_1_1_Стр130.ЗначениеСтр + " - "
	+ Р2_1_1_Стр140.ЗначениеСтр + " - " + Р2_1_2_Стр160.ЗначениеСтр + " Итог [" + Число(ЗначениеУсловияСтр) + "] >= 0";
	
	УсловиеВыполнено = ЗначениеУсловия >= 0;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть >= 0 И КС.ЛеваяЧасть = ЗначениеУсловия) Тогда
		ЭтоОшибка = Истина;
...
&НаСервере
Функция ПроверитьКС_1_1(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.1 (КС 1.1)";
	ОписаниеКС     = "Сумма авансового платежа к уплате по строке 020 Раздела 1.1 должна быть равна 0, если (строка 130 - строка 140) Раздела 2.1.1 равно значению строки 160 Раздела 2.1.2";
	Нарушение      = "пп. 3, 3.1 и 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уплате по сроку не позднее 25-го апреля отчетного года";
	...
	ЗначениеУсловия = Р2_1_1_Стр130.Значение - Р2_1_1_Стр140.Значение;
	ЗначениеУсловияСтр = ЧислоСтрЧГ0(ЗначениеУсловия);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = 0, когда " + Р2_1_1_Стр130.ЗначениеСтр + " - " + Р2_1_1_Стр140.ЗначениеСтр
	+ " Итог [" + Число(ЗначениеУсловияСтр) + "] = " + Р2_1_2_Стр160.ЗначениеСтр;
	
	УсловиеВыполнено = ЗначениеУсловия = Р2_1_2_Стр160.Значение;
...
&НаСервере
Функция ПроверитьКС_1_2(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.1 (КС 1.2)";
	ОписаниеКС     = "Сумма авансового платежа к уплате по строке 040 Раздела 1.1 должна быть равна (строка 131 - строка 141) Раздела 2.1.1 - строка 161 Раздела 2.1.2 - строка 020 Раздела 1.1, если (строка 131 - строка 141) Раздела 2.1.1 - строка 161 Раздела 2.1.2 - строка 020 Раздела 1.1 >= 0";
	Нарушение      = "пп. 3, 3.1 и 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уплате по сроку не позднее 25-го июля отчетного года";
	
	Р1_1_Стр020   = ПоказательПоСтроке("Р1_1_Стр020");
	Р1_1_Стр040   = ПоказательПоСтроке("Р1_1_Стр040");
	Р2_1_1_Стр131 = ПоказательПоСтроке("Р2_1_1_Стр131");
	Р2_1_1_Стр141 = ПоказательПоСтроке("Р2_1_1_Стр141");
	Р2_1_2_Стр161 = ПоказательПоСтроке("Р2_1_2_Стр161");
	
	Раздел11 = мДанныеОтчета.ПолеТабличногоДокументаРаздел11;
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр020);
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр040);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр131);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр141);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр161);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р1_1_Стр020,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р1_1_Стр040,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр131, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр141, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр161, "Раздел21");
	
	ЗначениеУсловия = Р2_1_1_Стр131.Значение - Р2_1_1_Стр141.Значение - Р2_1_2_Стр161.Значение - Р1_1_Стр020.Значение;
	ЗначениеУсловияСтр = ЧислоСтрЧГ0(ЗначениеУсловия);
	ЗначениеУсловияРасшифровка = Р2_1_1_Стр131.ЗначениеСтр + " - " + Р2_1_1_Стр141.ЗначениеСтр
	+ " - " + Р2_1_2_Стр161.ЗначениеСтр + " - " + Р1_1_Стр020.ЗначениеСтр;
	
	КС = ПоказателиКС(Р1_1_Стр040.Значение, ЗначениеУсловия);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + ЗначениеУсловияРасшифровка + " Итог [" + КС.ПраваяЧастьСтр + "], когда "
	+ ЗначениеУсловияРасшифровка + " Итог [" + КС.ПраваяЧастьСтр + "] >= 0";
...
&НаСервере
Функция ПроверитьКС_1_3(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.1 (КС 1.3)";
	ОписаниеКС     = "Сумма авансового платежа к уплате по строке 040 Раздела 1.1 должна быть равна 0, если (строка 131 - строка 141) Раздела 2.1.1 - строка 161 Раздела 2.1.2 - строка 020 Раздела 1.1 <= 0";
	Нарушение      = "пп. 3, 3.1 и 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уплате по сроку не позднее 25-го июля отчетного года";
	...
	КС = ПоказателиКС(Р1_1_Стр040.Значение, 0);
	
	ЗначениеУсловия = Р2_1_1_Стр131.Значение - Р2_1_1_Стр141.Значение - Р2_1_2_Стр161.Значение - Р1_1_Стр020.Значение;
	ЗначениеУсловияСтр = ЧислоСтрЧГ0(ЗначениеУсловия);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = 0, когда " + Р2_1_1_Стр131.ЗначениеСтр + " - " + Р2_1_1_Стр141.ЗначениеСтр
	+ " - " + Р2_1_2_Стр161.ЗначениеСтр + " - " + Р1_1_Стр020.ЗначениеСтр + " Итог [" + Число(ЗначениеУсловияСтр) + "] <= 0";
...
&НаСервере
Функция ПроверитьКС_1_4(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.1 (КС 1.4)";
	ОписаниеКС     = "Сумма авансового платежа к уменьшению по строке 050 Раздела 1.1 должна быть равна (строка 020 Раздела 1.1 - строка 131 Раздела 2.1.1 + строка 141 Раздела 2.1.1 + строка 161 Раздела 2.1.2), если (строка 131 Раздела 2.1.1 - строка 141 Раздела 2.1.1 - строка 161 Раздела 2.1.2 - строка 020 Раздела 1.1) < 0";
	Нарушение      = "пп. 3, 3.1, 5 и 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уплате по сроку не позднее 25-го июля отчетного года";
	
	Р1_1_Стр020   = ПоказательПоСтроке("Р1_1_Стр020");
	Р1_1_Стр050   = ПоказательПоСтроке("Р1_1_Стр050");
	Р2_1_1_Стр131 = ПоказательПоСтроке("Р2_1_1_Стр131");
	Р2_1_1_Стр141 = ПоказательПоСтроке("Р2_1_1_Стр141");
	Р2_1_2_Стр161 = ПоказательПоСтроке("Р2_1_2_Стр161");
	...
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр020);
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр050);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр131);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр141);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр161);
	...
	ВнестиПоказательВРасшифровку(Р1_1_Стр020,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р1_1_Стр050,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр131, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр141, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр161, "Раздел21");
	
	КС = ПоказателиКС(Р1_1_Стр050.Значение, Р1_1_Стр020.Значение - Р2_1_1_Стр131.Значение + Р2_1_1_Стр141.Значение
	+ Р2_1_2_Стр161.Значение);
	
	ЗначениеУсловия = Р2_1_1_Стр131.Значение - Р2_1_1_Стр141.Значение - Р2_1_2_Стр161.Значение - Р1_1_Стр020.Значение;
	ЗначениеУсловияСтр = ЧислоСтрЧГ0(ЗначениеУсловия);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р1_1_Стр020.ЗначениеСтр + " - " + Р2_1_1_Стр131.ЗначениеСтр
	+ " + " + Р2_1_1_Стр141.ЗначениеСтр + " + " + Р2_1_2_Стр161.ЗначениеСтр + " Итог [" + КС.ПраваяЧастьСтр
	+ "], когда " + Р2_1_1_Стр131.ЗначениеСтр + " - " + Р2_1_1_Стр141.ЗначениеСтр + " - " + Р2_1_2_Стр161.ЗначениеСтр
	+ " - " + Р1_1_Стр020.ЗначениеСтр + " Итог [" + Число(ЗначениеУсловияСтр) + "] < 0";
...
&НаСервере
Функция ПроверитьКС_1_5(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.1 (КС 1.5)";
	ОписаниеКС     = "Сумма авансового платежа  к уплате по строке 070 Раздела 1.1 должна быть равна (строка 132 Раздела 2.1.1 - строка 142 Раздела 2.1.1 - строка 162 Раздела 2.1.2 - строка 020 Раздела 1.1 - строка 040 Раздела 1.1 + строка 050 Раздела 1.1), если (строка 132 Раздела 2.1.1 - строка 142 Раздела 2.1.1 - строка 162 Раздела 2.1.2 - строка 020 Раздела 1.1 - строка 040 Раздела 1.1 + строка 050 Раздела 1.1) >= 0";
	Нарушение      = "пп. 3, 3.1, 5 и 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уплате по сроку не позднее 25-го октября отчетного года";
	
	Р1_1_Стр020   = ПоказательПоСтроке("Р1_1_Стр020");
	Р1_1_Стр040   = ПоказательПоСтроке("Р1_1_Стр040");
	Р1_1_Стр050   = ПоказательПоСтроке("Р1_1_Стр050");
	Р1_1_Стр070   = ПоказательПоСтроке("Р1_1_Стр070");
	Р2_1_1_Стр132 = ПоказательПоСтроке("Р2_1_1_Стр132");
	Р2_1_1_Стр142 = ПоказательПоСтроке("Р2_1_1_Стр142");
	Р2_1_2_Стр162 = ПоказательПоСтроке("Р2_1_2_Стр162");
	...
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр040);
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр050);
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр070);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр132);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр142);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр162);
	...
	ВнестиПоказательВРасшифровку(Р1_1_Стр040,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р1_1_Стр050,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р1_1_Стр070,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр132, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр142, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр162, "Раздел21");
	
	ЗначениеУсловия = Р2_1_1_Стр132.Значение - Р2_1_1_Стр142.Значение - Р2_1_2_Стр162.Значение - Р1_1_Стр020.Значение
	- Р1_1_Стр040.Значение + Р1_1_Стр050.Значение;
	ЗначениеУсловияСтр = ЧислоСтрЧГ0(ЗначениеУсловия);
	ЗначениеУсловияРасшифровка = Р2_1_1_Стр132.ЗначениеСтр + " - " + Р2_1_1_Стр142.ЗначениеСтр
	+ " - " + Р2_1_2_Стр162.ЗначениеСтр + " - " + Р1_1_Стр020.ЗначениеСтр + " - " + Р1_1_Стр040.ЗначениеСтр
	+ " + " + Р1_1_Стр050.ЗначениеСтр;
	
	КС = ПоказателиКС(Р1_1_Стр070.Значение, ЗначениеУсловия);
...
&НаСервере
Функция ПроверитьКС_1_6(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.1 (КС 1.6)";
	ОписаниеКС     = "Сумма авансового платежа к уплате по строке 070 Раздела 1.1 должна быть равна нулю, если (строка 132 Раздела 2.1.1 - строка 142 Раздела 2.1.1 - строка 162 Раздела - строка 020 Раздела 1.1 - строка 040 Раздела 1.1 + строка 050 Раздела 1.1) <= 0";
	Нарушение      = "пп. 3, 3.1, 5 и 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уплате по сроку не позднее 25-го октября отчетного года";
	...
	КС = ПоказателиКС(Р1_1_Стр070.Значение, 0);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = 0, когда " + ЗначениеУсловияРасшифровка + " Итог [" + Число(ЗначениеУсловияСтр) + "] <= 0";
...
&НаСервере
Функция ПроверитьКС_1_7(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.1 (КС 1.7)";
	ОписаниеКС     = "Сумма авансового платежа к уменьшению по строке 080 Раздела 1.1 должна быть равна (строка 020 Раздела 1.1 + строка 040 Раздела 1.1 - строка 050 Раздела 1.1 - строка 132 Раздела 2.1.1 + строка 142 Раздела 2.1.1 + строка 162 Раздела 2.1.2), если (строка 132 Раздела 2.1.1 - строка 142 Раздела 2.1.1 - строка 162 Раздела 2.1.2 - строка 020 Раздела 1.1 - строка 040 Раздела 1.1 + строка 050 Раздела 1.1) меньше нуля";
	Нарушение      = "пп. 3, 3.1, 5 и 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уменьшению по сроку не позднее 25-го октября отчетного год";
	
	Р1_1_Стр020   = ПоказательПоСтроке("Р1_1_Стр020");
	Р1_1_Стр040   = ПоказательПоСтроке("Р1_1_Стр040");
	Р1_1_Стр050   = ПоказательПоСтроке("Р1_1_Стр050");
	Р1_1_Стр080   = ПоказательПоСтроке("Р1_1_Стр080");
	Р2_1_1_Стр132 = ПоказательПоСтроке("Р2_1_1_Стр132");
	Р2_1_1_Стр142 = ПоказательПоСтроке("Р2_1_1_Стр142");
	Р2_1_2_Стр162 = ПоказательПоСтроке("Р2_1_2_Стр162");
	...
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр040);
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр050);
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр080);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр132);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр142);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр162);
	...
	ВнестиПоказательВРасшифровку(Р1_1_Стр040,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р1_1_Стр050,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р1_1_Стр080,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр132, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр142, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр162, "Раздел21");
	
	КС = ПоказателиКС(Р1_1_Стр080.Значение, Р1_1_Стр020.Значение + Р1_1_Стр040.Значение - Р1_1_Стр050.Значение
		- Р2_1_1_Стр132.Значение + Р2_1_1_Стр142.Значение + Р2_1_2_Стр162.Значение);
	
	ЗначениеУсловия = Р2_1_1_Стр132.Значение - Р2_1_1_Стр142.Значение - Р2_1_2_Стр162.Значение - Р1_1_Стр020.Значение
		- Р1_1_Стр040.Значение + Р1_1_Стр050.Значение;
	ЗначениеУсловияСтр = ЧислоСтрЧГ0(ЗначениеУсловия);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р1_1_Стр020.ЗначениеСтр + " + " + Р1_1_Стр040.ЗначениеСтр
	+ " - " + Р1_1_Стр050.ЗначениеСтр + " - " + Р2_1_1_Стр132.ЗначениеСтр + " + " + Р2_1_1_Стр142.ЗначениеСтр
	+ " + " + Р2_1_2_Стр162.ЗначениеСтр + " Итог [" + КС.ПраваяЧастьСтр + "], когда " + Р2_1_1_Стр132.ЗначениеСтр
	+ " - " + Р2_1_1_Стр142.ЗначениеСтр + " - " + Р2_1_2_Стр162.ЗначениеСтр + " - " + Р1_1_Стр020.ЗначениеСтр
	+ " - " + Р1_1_Стр040.ЗначениеСтр + " + " + Р1_1_Стр050.ЗначениеСтр + " Итог [" + Число(ЗначениеУсловияСтр) + "] < 0";
...
&НаСервере
Функция ПроверитьКС_1_8(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.1 (КС 1.8)";
	ОписаниеКС     = "Сумма налога, подлежащая доплате за налоговый период (календарный год) по строке 100 Раздела 1.1 должна быть равна (строка 133 Раздел 2.1.1 - строка 143 Раздела 2.1.1 - строка 163 Раздела 2.1.2 - строка 020 раздела 1.1 - строка 040 раздела 1.1 + строка 050 Раздела 1.1 - строка 070 Раздела 1.1 + строка 080 раздела 1.1), если (строка 133 Раздел 2.1.1 - строка 143 Раздела 2.1.1 - строка 163 Раздела 2.1.2 - строка 020 раздела 1.1 - строка 040 раздела 1.1 + строка 050 Раздела 1.1 - строка 070 Раздела 1.1 + строка 080 раздела 1.1) >= 0";
	Нарушение      = "пп. 3.1, 5 и 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы налога, подлежащей доплате за налоговый период";
	...
	Р1_1_Стр070   = ПоказательПоСтроке("Р1_1_Стр070");
	Р1_1_Стр080   = ПоказательПоСтроке("Р1_1_Стр080");
	Р1_1_Стр100   = ПоказательПоСтроке("Р1_1_Стр100");
	Р2_1_1_Стр133 = ПоказательПоСтроке("Р2_1_1_Стр133");
	Р2_1_1_Стр143 = ПоказательПоСтроке("Р2_1_1_Стр143");
	Р2_1_2_Стр163 = ПоказательПоСтроке("Р2_1_2_Стр163");
	...
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр070);
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр080);
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр100);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр133);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр143);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр163);
	...
	ВнестиПоказательВРасшифровку(Р1_1_Стр070,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р1_1_Стр080,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р1_1_Стр100,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр133, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр143, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр163, "Раздел21");
	
	ЗначениеУсловия = Р2_1_1_Стр133.Значение - Р2_1_1_Стр143.Значение - Р2_1_2_Стр163.Значение
	- Р1_1_Стр020.Значение - Р1_1_Стр040.Значение + Р1_1_Стр050.Значение - Р1_1_Стр070.Значение + Р1_1_Стр080.Значение;
	ЗначениеУсловияСтр = ЧислоСтрЧГ0(ЗначениеУсловия);
	ЗначениеУсловияРасшифровка = Р2_1_1_Стр133.ЗначениеСтр + " - " + Р2_1_1_Стр143.ЗначениеСтр
	+ " - " + Р2_1_2_Стр163.ЗначениеСтр + " - " + Р1_1_Стр020.ЗначениеСтр + " - " + Р1_1_Стр040.ЗначениеСтр
	+ " + " + Р1_1_Стр050.ЗначениеСтр + " - " + Р1_1_Стр070.ЗначениеСтр + " + " + Р1_1_Стр080.ЗначениеСтр;
	
	КС = ПоказателиКС(Р1_1_Стр100.Значение, ЗначениеУсловия);
...
&НаСервере
Функция ПроверитьКС_1_9(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.1 (КС 1.9)";
	ОписаниеКС     = "Сумма налога, подлежащая доплате за налоговый период (календарный год) по строке 100 Раздела 1.1 должна быть равна 0, если (строка 133 Раздел 2.1.1 - строка 143 Раздела 2.1.1 - строка 163 Раздела 2.1.2 - строка 020 раздела 1.1 - строка 040 раздела 1.1 + строка 050 Раздела 1.1 - строка 070 Раздела 1.1 + строка 080 раздела 1.1) <= 0";
	Нарушение      = "пп. 3.1, 5 и 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы налога, подлежащей доплате за налоговый период";
	...
	КС = ПоказателиКС(Р1_1_Стр100.Значение, 0);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = 0, когда " + ЗначениеУсловияРасшифровка
	+ " Итог [" + Число(ЗначениеУсловияСтр) + "] <= 0";
...
&НаСервере
Функция ПроверитьКС_1_10(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.1 (КС 1.10)";
	ОписаниеКС     = "Сумма налога к уменьшению за налоговый период по строке 110 Раздела 1.1 должна быть равна строка 020 Раздела 1.1 + строка 040 Раздела 1.1 - строка 050 Раздела 1.1 + строка 070 Раздела 1.1 - строка 080 Раздела 1.1 - строка 133 раздела 2.1.1 + строка 143 раздела 2.1.1 + строка 163 Раздела 2.1.2, если строка 133 раздела 2.1.1 - строка 143 раздела 2.1.1 - строка 163 Раздела 2.1.2  - строка 020 Раздела 1.1 - строка 040 Раздела 1.1 + строка 050 Раздела 1.1 - строка 070 Раздела 1.1 + строка 080 Раздела 1.1 < 0";
	Нарушение      = "пп. 3.1, 5 и 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы налога к уменьшению за налоговый период";
	...
	Р1_1_Стр070   = ПоказательПоСтроке("Р1_1_Стр070");
	Р1_1_Стр080   = ПоказательПоСтроке("Р1_1_Стр080");
	Р1_1_Стр110   = ПоказательПоСтроке("Р1_1_Стр110");
	Р2_1_1_Стр133 = ПоказательПоСтроке("Р2_1_1_Стр133");
	Р2_1_1_Стр143 = ПоказательПоСтроке("Р2_1_1_Стр143");
	Р2_1_2_Стр163 = ПоказательПоСтроке("Р2_1_2_Стр163");
	...
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр070);
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр080);
	ВставитьЗначениеПоказателя(Раздел11, Р1_1_Стр110);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр133);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр143);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр163);
	...
	ВнестиПоказательВРасшифровку(Р1_1_Стр070,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р1_1_Стр080,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р1_1_Стр110,   "Раздел11");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр133, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр143, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр163, "Раздел21");
	...
	КС = ПоказателиКС(Р1_1_Стр110.Значение, Р1_1_Стр020.Значение + Р1_1_Стр040.Значение - Р1_1_Стр050.Значение
	+ Р1_1_Стр070.Значение - Р1_1_Стр080.Значение - Р2_1_1_Стр133.Значение + Р2_1_1_Стр143.Значение
	+ Р2_1_2_Стр163.Значение);
	
	КСПравРасшифровка = Р1_1_Стр020.ЗначениеСтр + " + " + Р1_1_Стр040.ЗначениеСтр + " - " + Р1_1_Стр050.ЗначениеСтр
	+ " + " + Р1_1_Стр070.ЗначениеСтр + " - " + Р1_1_Стр080.ЗначениеСтр + " - " + Р2_1_1_Стр133.ЗначениеСтр
	+ " + " + Р2_1_1_Стр143.ЗначениеСтр + " + " + Р2_1_2_Стр163.ЗначениеСтр;
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + КСПравРасшифровка + " Итог [" + КС.ПраваяЧастьСтр + "], когда "
	+ ЗначениеУсловияРасшифровка + " Итог [" + Число(ЗначениеУсловияСтр) + "] < 0";
...
&НаСервере
Функция ПроверитьКС_1_11(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.2 (КС 1.11)";
	ОписаниеКС     = "Сумма авансового платежа к уплате по строке 020 Раздела 1.2 должна быть равна значению в строке 270 Раздела 2.2, если строка 270 Раздела 2.2 >= 0";
	Нарушение      = "п. 4 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уплате по сроку не позднее 25-го апреля отчетного года";
	
	Р1_2_Стр020 = ПоказательПоСтроке("Р1_2_Стр020");
	Р2_2_Стр270 = ПоказательПоСтроке("Р2_2_Стр270");
	
	Раздел12 = мДанныеОтчета.ПолеТабличногоДокументаРаздел12;
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр020);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр270);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р1_2_Стр020, "Раздел12");
	ВнестиПоказательВРасшифровку(Р2_2_Стр270, "Раздел22");
	
	КС = ПоказателиКС(Р1_2_Стр020.Значение, Р2_2_Стр270.Значение);
	
	ЗначениеУсловия = Р2_2_Стр270.Значение;
	ЗначениеУсловияСтр = ЧислоСтрЧГ0(ЗначениеУсловия);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + КС.ПраваяЧастьСтр + ", если " + ЗначениеУсловия + " >= 0";
...
&НаСервере
Функция ПроверитьКС_1_12(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.2 (КС 1.12)";
	ОписаниеКС     = "Сумма авансового платежа к уплате по строке 020 Раздела 1.2 должна быть равна нулю, если строка 210 Раздела 2.2 = строка 220 Раздела 2";
	Нарушение      = "п. 4 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уплате по сроку не позднее 25-го апреля отчетного года";
	
	Р1_2_Стр020 = ПоказательПоСтроке("Р1_2_Стр020");
	Р2_2_Стр210 = ПоказательПоСтроке("Р2_2_Стр210");
	Р2_2_Стр220 = ПоказательПоСтроке("Р2_2_Стр220");
	
	Раздел12 = мДанныеОтчета.ПолеТабличногоДокументаРаздел12;
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр020);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр210);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр220);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р1_2_Стр020, "Раздел12");
	ВнестиПоказательВРасшифровку(Р2_2_Стр210, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр220, "Раздел22");
	
	КС = ПоказателиКС(Р1_2_Стр020.Значение, 0);
	
	ЗначениеУсловияСлева = Р2_2_Стр210.Значение;
	ЗначениеУсловияСлеваСтр = ЧислоСтрЧГ0(ЗначениеУсловияСлева);
	
	ЗначениеУсловияСправа = Р2_2_Стр220.Значение;
	ЗначениеУсловияСправаСтр = ЧислоСтрЧГ0(ЗначениеУсловияСправа);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = 0, если " + ЗначениеУсловияСлеваСтр + " = " + ЗначениеУсловияСправаСтр;
	
	УсловиеВыполнено = ЗначениеУсловияСлева = ЗначениеУсловияСправа;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_13(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.2 (КС 1.13)";
	ОписаниеКС     = "Сумма авансового платежа к уплате по строке 040 Раздела 1.2 должна быть равна разнице значений строк 271 Раздела 2.2 и 020 Раздела 1.2, если (строка 271 Раздела 2.2 - строка 020 Раздела 1.2) => 0";
	Нарушение      = "пп. 4 и 5 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уплате по сроку не позднее 25-го июля отчетного года";
	
	Р1_2_Стр020 = ПоказательПоСтроке("Р1_2_Стр020");
	Р1_2_Стр040 = ПоказательПоСтроке("Р1_2_Стр040");
	Р2_2_Стр271 = ПоказательПоСтроке("Р2_2_Стр271");
	
	Раздел12 = мДанныеОтчета.ПолеТабличногоДокументаРаздел12;
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр020);
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр040);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр271);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р1_2_Стр020, "Раздел12");
	ВнестиПоказательВРасшифровку(Р1_2_Стр040, "Раздел12");
	ВнестиПоказательВРасшифровку(Р2_2_Стр271, "Раздел22");
	
	ЗначениеУсловияСлева = Р2_2_Стр271.Значение - Р1_2_Стр020.Значение;
	ЗначениеУсловияСлеваСтр = ЧислоСтрЧГ0(ЗначениеУсловияСлева);
	ЗначениеУсловияСлеваРасшифровка = Р2_2_Стр271.ЗначениеСтр + " - " + Р1_2_Стр020.ЗначениеСтр;
	
	КС = ПоказателиКС(Р1_2_Стр040.Значение, ЗначениеУсловияСлева);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + ЗначениеУсловияСлеваРасшифровка + " Итог [" + Число(ЗначениеУсловияСлеваСтр)
	+ "], если " + ЗначениеУсловияСлеваРасшифровка + " Итог [" + Число(ЗначениеУсловияСлеваСтр) + "] >= 0";
	
	УсловиеВыполнено = ЗначениеУсловияСлева >= 0;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_14(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.2 (КС 1.14)";
	ОписаниеКС     = "Сумма авансового платежа к уплате по строке 040 Раздела 1.2 должна быть равна нулю, если (строка 271 Раздела 2.2 - строка 020 Раздела 1.2) <= 0";
	Нарушение      = "пп. 4 и 5 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уплате по сроку не позднее 25-го июля отчетного года";
	...
	КС = ПоказателиКС(Р1_2_Стр040.Значение, 0);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = 0, если " + ЗначениеУсловияСлеваРасшифровка
	+ " Итог [" + Число(ЗначениеУсловияСлеваСтр) + "] <= 0";
...
&НаСервере
Функция ПроверитьКС_1_15(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.2 (КС 1.15)";
	ОписаниеКС     = " Сумма авансового платежа к уменьшению по строке 050 Раздела 1.2 должна быть равна разнице значений строк 020 Раздела 1.2 и 271 Раздела 2.2, если (строка 271 Раздела 2.2 - строка 020 Раздела 1.2) < 0";
	Нарушение      = "пп. 4 и 5 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уменьшению по сроку не позднее 25-июля отчетного года";
	
	Р1_2_Стр020 = ПоказательПоСтроке("Р1_2_Стр020");
	Р1_2_Стр050 = ПоказательПоСтроке("Р1_2_Стр050");
	Р2_2_Стр271 = ПоказательПоСтроке("Р2_2_Стр271");
	...
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр020);
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр050);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр271);
	...
	ВнестиПоказательВРасшифровку(Р1_2_Стр020, "Раздел12");
	ВнестиПоказательВРасшифровку(Р1_2_Стр050, "Раздел12");
	ВнестиПоказательВРасшифровку(Р2_2_Стр271, "Раздел22");
	...
	КС = ПоказателиКС(Р1_2_Стр050.Значение, Р1_2_Стр020.Значение - Р2_2_Стр271.Значение);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р1_2_Стр020.ЗначениеСтр + " - " + Р2_2_Стр271.ЗначениеСтр + " Итог ["
	+ КС.ПраваяЧастьСтр + "], если " + ЗначениеУсловияСлеваРасшифровка + " Итог [" + Число(ЗначениеУсловияСлеваСтр) + "] < 0";
...
&НаСервере
Функция ПроверитьКС_1_16(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.2 (КС 1.16)";
	ОписаниеКС     = "Сумма авансового платежа к уплате по строке 070 Раздела 1.2 должна быть равна (строка 272 Раздела 2.2 - строка 020 Раздела 1.2 + строка 050 Раздела 1.2), если (строка 272 Раздела 2.2 - строка 020 Раздела 1.2 + строка 050 Раздела 1.2) => 0";
	Нарушение      = "пп. 4 и 5 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уплате по сроку не позднее 25-го октября отчетного года";
	...
	Р1_2_Стр040 = ПоказательПоСтроке("Р1_2_Стр040");
	Р1_2_Стр050 = ПоказательПоСтроке("Р1_2_Стр050");
	Р1_2_Стр070 = ПоказательПоСтроке("Р1_2_Стр070");
	Р2_2_Стр272 = ПоказательПоСтроке("Р2_2_Стр272");
	...
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр040);
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр050);
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр070);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр272);
	...
	ВнестиПоказательВРасшифровку(Р1_2_Стр040, "Раздел12");
	ВнестиПоказательВРасшифровку(Р1_2_Стр050, "Раздел12");
	ВнестиПоказательВРасшифровку(Р1_2_Стр070, "Раздел12");
	ВнестиПоказательВРасшифровку(Р2_2_Стр272, "Раздел22");
	
	ЗначениеУсловияСлева = Р2_2_Стр272.Значение - Р1_2_Стр020.Значение - Р1_2_Стр040.Значение + Р1_2_Стр050.Значение;
	ЗначениеУсловияСлеваСтр = ЧислоСтрЧГ0(ЗначениеУсловияСлева);
	ЗначениеУсловияСлеваРасшифровка = Р2_2_Стр272.ЗначениеСтр + " - " + Р1_2_Стр020.ЗначениеСтр
	+ " - " + Р1_2_Стр040.ЗначениеСтр + " + " + Р1_2_Стр050.ЗначениеСтр;
	
	КС = ПоказателиКС(Р1_2_Стр070.Значение, ЗначениеУсловияСлева);
...
&НаСервере
Функция ПроверитьКС_1_17(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.2 (КС 1.17)";
	ОписаниеКС     = "Сумма авансового платежа к уплате по строке 070 Раздела 1.2 должна быть равна нулю, если (строка 272 Раздела 2.2 - строка 020 Раздела 1.2 - строка 040 Раздела 1.2 + строка 050 Раздела 1.2) <= 0";
	Нарушение      = "пп. 4 и 5 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уплате по сроку не позднее 25-го июля отчетного года";
	...
	КС = ПоказателиКС(Р1_2_Стр070.Значение, 0);
...
&НаСервере
Функция ПроверитьКС_1_18(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.2 (КС 1.18)";
	ОписаниеКС     = "Сумма авансового платежа к уменьшению по строке 080 Раздела 1.2 должна быть равна (строка 020 Раздела 1.2 + строка 040 раздел 1.2 - строка 050 раздел 1.2 - строка 272 Раздела 272 Раздела 2.2), если (строка 272 Раздела 2.2 - строка 020 Раздела 1.2 - строка 040 раздела 1.2 - строка 050 раздела 1.2) < 0";
	Нарушение      = "пп. 4 и 5 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу к уменьшению по сроку не позднее 25-го октября отчетного года";
	
	Р1_2_Стр020 = ПоказательПоСтроке("Р1_2_Стр020");
	Р1_2_Стр040 = ПоказательПоСтроке("Р1_2_Стр040");
	Р1_2_Стр050 = ПоказательПоСтроке("Р1_2_Стр050");
	Р1_2_Стр080 = ПоказательПоСтроке("Р1_2_Стр080");
	Р2_2_Стр272 = ПоказательПоСтроке("Р2_2_Стр272");
	...
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр040);
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр050);
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр080);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр272);
	...
	ВнестиПоказательВРасшифровку(Р1_2_Стр040, "Раздел12");
	ВнестиПоказательВРасшифровку(Р1_2_Стр050, "Раздел12");
	ВнестиПоказательВРасшифровку(Р1_2_Стр080, "Раздел12");
	ВнестиПоказательВРасшифровку(Р2_2_Стр272, "Раздел22");
	...
	КС = ПоказателиКС(Р1_2_Стр080.Значение, Р1_2_Стр020.Значение + Р1_2_Стр040.Значение - Р1_2_Стр050.Значение
	- Р2_2_Стр272.Значение);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р1_2_Стр020.ЗначениеСтр + " + " + Р1_2_Стр040.ЗначениеСтр
	+ " - " + Р1_2_Стр050.ЗначениеСтр + " - " + Р2_2_Стр272.ЗначениеСтр + " Итог [" + КС.ПраваяЧастьСтр + "], если "
	+ ЗначениеУсловияСлеваРасшифровка + " Итог [" + Число(ЗначениеУсловияСлеваСтр) + "] < 0";
...
&НаСервере
Функция ПроверитьКС_1_19(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.2 (КС 1.19)";
	ОписаниеКС     = "Сумма налога, подлежащая уплате за налоговый период (строка 100 Раздел 1.2) должна быть равна строке 273 Раздела 2.2 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2, если  результат вычитания строка 273 Раздела 2.2 - (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 больше либо равен 0 и значение строка 273 Раздела 2.2 больше или равно значению строки 280 Раздела 2.2";
	Нарушение      = "п. 5 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы налога, подлежащей доплате за налоговый период";
	...
	Р1_2_Стр070 = ПоказательПоСтроке("Р1_2_Стр070");
	Р1_2_Стр080 = ПоказательПоСтроке("Р1_2_Стр080");
	Р1_2_Стр100 = ПоказательПоСтроке("Р1_2_Стр100");
	Р2_2_Стр273 = ПоказательПоСтроке("Р2_2_Стр273");
	Р2_2_Стр280 = ПоказательПоСтроке("Р2_2_Стр280");
	...
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр070);
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр080);
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр100);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр273);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр280);
	...
	ВнестиПоказательВРасшифровку(Р1_2_Стр070, "Раздел12");
	ВнестиПоказательВРасшифровку(Р1_2_Стр080, "Раздел12");
	ВнестиПоказательВРасшифровку(Р1_2_Стр100, "Раздел12");
	ВнестиПоказательВРасшифровку(Р2_2_Стр273, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр280, "Раздел22");
	
	ЗначениеУсловияСлева = Р2_2_Стр273.Значение - Р1_2_Стр020.Значение - Р1_2_Стр040.Значение + Р1_2_Стр050.Значение
	- Р1_2_Стр070.Значение + Р1_2_Стр080.Значение;
	ЗначениеУсловияСлеваСтр = ЧислоСтрЧГ0(ЗначениеУсловияСлева);
	ЗначениеУсловияСлеваРасшифровка = Р2_2_Стр273.ЗначениеСтр + " - " + Р1_2_Стр020.ЗначениеСтр
	+ " - " + Р1_2_Стр040.ЗначениеСтр + " + " + Р1_2_Стр050.ЗначениеСтр + " - " + Р1_2_Стр070.ЗначениеСтр
	+ " + " + Р1_2_Стр080.ЗначениеСтр;
	
	КС = ПоказателиКС(Р1_2_Стр100.Значение, ЗначениеУсловияСлева);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + ЗначениеУсловияСлеваРасшифровка + " Итог [" + КС.ПраваяЧастьСтр
		+ "], если " + ЗначениеУсловияСлеваРасшифровка + " Итог [" + Число(ЗначениеУсловияСлеваСтр) + "] >= 0 и "
		+ Р2_2_Стр273.ЗначениеСтр + " >= " + Р2_2_Стр280.ЗначениеСтр;
	
	УсловиеВыполнено = ЗначениеУсловияСлева >= 0 И Р2_2_Стр273.Значение >= Р2_2_Стр280.Значение;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_20(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.2 (КС 1.20)";
	ОписаниеКС     = "Сумма налога, подлежащая уплате за налоговый период (строка 100 Раздела 1.2) должна быть равна 0, если результат от вычитания (строка 020 + строка 040 - строка 050 + строка 070 - строка 080) Раздела 1.2 из строки 273 Раздела 2.2 < = 0";
	Нарушение      = "п. 5 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы налога, подлежащей доплате за налоговый период";
	...
	КС = ПоказателиКС(Р1_2_Стр100.Значение, 0);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = 0, если "
		+ ЗначениеУсловияСлеваРасшифровка + " Итог [" + Число(ЗначениеУсловияСлеваСтр) + "] < = 0";
...
&НаСервере
Функция ПроверитьКС_1_21а(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 1.2 (КС 1.21а)";
	...
	Р1_2_Стр070 = ПоказательПоСтроке("Р1_2_Стр070");
	Р1_2_Стр080 = ПоказательПоСтроке("Р1_2_Стр080");
	Р1_2_Стр110 = ПоказательПоСтроке("Р1_2_Стр110");
	Р2_2_Стр273 = ПоказательПоСтроке("Р2_2_Стр273");
	Р2_2_Стр280 = ПоказательПоСтроке("Р2_2_Стр280");
	...
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр070);
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр080);
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр110);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр273);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр280);
	...
	ВнестиПоказательВРасшифровку(Р1_2_Стр070, "Раздел12");
	ВнестиПоказательВРасшифровку(Р1_2_Стр080, "Раздел12");
	ВнестиПоказательВРасшифровку(Р1_2_Стр110, "Раздел12");
	ВнестиПоказательВРасшифровку(Р2_2_Стр273, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр280, "Раздел22");
	
	ЗначениеУсловияСлева = Р1_2_Стр020.Значение + Р1_2_Стр040.Значение - Р1_2_Стр050.Значение
	+ Р1_2_Стр070.Значение - Р1_2_Стр080.Значение - Р2_2_Стр273.Значение;
	ЗначениеУсловияСлеваСтр = ЧислоСтрЧГ0(ЗначениеУсловияСлева);
	ЗначениеУсловияСлеваРасшифровка = Р1_2_Стр020.ЗначениеСтр + " + " + Р1_2_Стр040.ЗначениеСтр
	+ " - " + Р1_2_Стр050.ЗначениеСтр + " + " + Р1_2_Стр070.ЗначениеСтр + " - " + Р1_2_Стр080.ЗначениеСтр
	+ " - " + Р2_2_Стр273.ЗначениеСтр;
	
	ЗначениеУсловияСправа = Р2_2_Стр273.Значение - (Р1_2_Стр020.Значение + Р1_2_Стр040.Значение - Р1_2_Стр050.Значение
	+ Р1_2_Стр070.Значение - Р1_2_Стр080.Значение);
	ЗначениеУсловияСправаСтр = ЧислоСтрЧГ0(ЗначениеУсловияСправа);
	ЗначениеУсловияСправаРасшифровка = Р2_2_Стр273.ЗначениеСтр + " - (" + Р1_2_Стр020.ЗначениеСтр + " + "
	+ Р1_2_Стр040.ЗначениеСтр + " - " + Р1_2_Стр050.ЗначениеСтр + " + " + Р1_2_Стр070.ЗначениеСтр + " - "
	+ Р1_2_Стр080.ЗначениеСтр + ")";
	
	КС = ПоказателиКС(Р1_2_Стр110.Значение, ЗначениеУсловияСлева);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + ЗначениеУсловияСлеваРасшифровка + " Итог [" + КС.ПраваяЧастьСтр
	+ "], если " + Р2_2_Стр273.ЗначениеСтр + " >= " + Р2_2_Стр280.ЗначениеСтр + " и "
	+ ЗначениеУсловияСправаРасшифровка + " Итог [" + Число(ЗначениеУсловияСправаСтр) + "] < 0";
	
	УсловиеВыполнено = Р2_2_Стр273.Значение >= Р2_2_Стр280.Значение И ЗначениеУсловияСправа < 0;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_21б(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы  = "Проверка раздела 1.2 (КС 1.21б)";
	...
	Нарушение      = "п.4 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно исчислена сумма налога, подлежащая уплате за налоговый период";
	...
	КС = ПоказателиКС(Р1_2_Стр110.Значение, (Р1_2_Стр020.Значение + Р1_2_Стр040.ЗначениеСтр - Р1_2_Стр050.Значение
	+ Р1_2_Стр070.Значение - Р1_2_Стр080.Значение) - Р2_2_Стр280.Значение);
	
	ЗначениеУсловия = (Р1_2_Стр020.Значение + Р1_2_Стр040.Значение - Р1_2_Стр050.Значение + Р1_2_Стр070.Значение
	- Р1_2_Стр080.Значение);
	ЗначениеУсловияСтр = ЧислоСтрЧГ0(ЗначениеУсловия);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = (" + Р1_2_Стр020.ЗначениеСтр + " + " + Р1_2_Стр040.ЗначениеСтр
	+ " - " + Р1_2_Стр050.ЗначениеСтр + " + " + Р1_2_Стр070.ЗначениеСтр + " - " + Р1_2_Стр080.ЗначениеСтр
	+ ") - " + Р2_2_Стр280.ЗначениеСтр + " Итог [" + КС.ПраваяЧастьСтр + "], когда " + Р2_2_Стр273.ЗначениеСтр
	+ " < " + Р2_2_Стр280.ЗначениеСтр + " и " + Р1_2_Стр020.ЗначениеСтр + " + " + Р1_2_Стр040.ЗначениеСтр
	+ " - " + Р1_2_Стр050.ЗначениеСтр + " + " + Р1_2_Стр070.ЗначениеСтр + " - " + Р1_2_Стр080.ЗначениеСтр
	+ " Итог [" + Число(ЗначениеУсловияСтр) + "] > " + Р2_2_Стр280.ЗначениеСтр;
	
	УсловиеВыполнено = (Р2_2_Стр273.Значение < Р2_2_Стр280.Значение) И (ЗначениеУсловия > Р2_2_Стр280.Значение);
	Если УсловиеВыполнено И НЕ КС.ЛеваяЧасть = КС.ПраваяЧасть Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
Функция ПроверитьКС_1_22(СтруктураРезультата)
	...
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр120);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр273);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр280);
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр020);
	ВставитьЗначениеПоказателя(Раздел12, Р1_2_Стр040);
	...
	ВнестиПоказательВРасшифровку(Р1_2_Стр120, "Раздел12");
	...
	КС = ПоказателиКС(Р1_2_Стр120.Значение, Р2_2_Стр280.Значение - (Р1_2_Стр020.Значение + Р1_2_Стр040.Значение
	- Р1_2_Стр050.Значение + Р1_2_Стр070.Значение - Р1_2_Стр080.Значение));
	...
	ЗначениеУсловияПодробноСтр = Р1_2_Стр020.ЗначениеСтр + " + " + Р1_2_Стр040.ЗначениеСтр + " - "
	+ Р1_2_Стр050.ЗначениеСтр + " + " + Р1_2_Стр070.ЗначениеСтр + " - " + Р1_2_Стр080.ЗначениеСтр;
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р2_2_Стр280.ЗначениеСтр + " - ("+ ЗначениеУсловияПодробноСтр + ") Итог ["
	+ Число(ЗначениеУсловияСтр) + "]" + ", когда " + Р2_2_Стр280.ЗначениеСтр + " > " + Р2_2_Стр273.ЗначениеСтр + " и "
	+ Р2_2_Стр280.ЗначениеСтр + " > (" + ЗначениеУсловияПодробноСтр + ") Итог [" + Число(ЗначениеУсловияСтр) + "]";
	
	УсловиеВыполнено = ((Р2_2_Стр280.Значение > Р2_2_Стр273.Значение) И (Р2_2_Стр280.Значение > ЗначениеУсловия));
...
&НаСервере
Функция ПроверитьКС_1_23(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.1 (КС 1.23)";
	ОписаниеКС     = "Признак налогоплательщика в строке 102 Раздела 2.1.1 должен быть равен ""1"" или ""2""";
	Нарушение      = "";
	ОписаниеОшибки = "Неправильно заполнен признак налогоплательщик";
	
	Р2_1_1_Стр102 = ПоказательПоСтроке("Р2_1_1_Стр102");
	
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр102);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр102, "Раздел21");
	
	Расшифровка = Р2_1_1_Стр102.ЗначениеСтр + " = (1 или 2)";
	
	Если НЕ КодНайден(Р2_1_1_Стр102.ЗначениеСтр, "1,2") Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
		ЭтоОшибка = Ложь;
...
&НаСервере
Функция ПроверитьКС_1_24(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.1 (КС 1.24)";
	ОписаниеКС     = "Сумма исчисленного налога за первый квартал (строка 130 Раздела 2.1.1) должна быть равна произведению значений строк 110 и 120 Раздела 2.1.1, деленому на 100";
	Нарушение       = "ст. 346.20, п. 3 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу за первый квартал";
	
	Р2_1_1_Стр110 = ПоказательПоСтроке("Р2_1_1_Стр110");
	Р2_1_1_Стр120 = ПоказательПоСтроке("Р2_1_1_Стр120");
	Р2_1_1_Стр130 = ПоказательПоСтроке("Р2_1_1_Стр130");
	
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр110);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр120);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр130);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр110, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр120, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр130, "Раздел21");
	
	КС = ПоказателиКС(Р2_1_1_Стр130.Значение, Р2_1_1_Стр110.Значение * Р2_1_1_Стр120.Значение / 100);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р2_1_1_Стр110.ЗначениеСтр + " * " + Р2_1_1_Стр120.ЗначениеСтр
...
&НаСервере
Функция ПроверитьКС_1_25(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.1 (КС 1.25)";
	ОписаниеКС     = "Сумма исчисленного налога  за полугодие (строка 131 Раздела 2.1.1 должна быть равна произведению значений строк 111 и 121 Раздела 2.1.1, деленому на 100";
	Нарушение       = "ст. 346.20, п. 3 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу за полугодие";
	
	Р2_1_1_Стр111 = ПоказательПоСтроке("Р2_1_1_Стр111");
	Р2_1_1_Стр121 = ПоказательПоСтроке("Р2_1_1_Стр121");
	Р2_1_1_Стр131 = ПоказательПоСтроке("Р2_1_1_Стр131");
	
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр111);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр121);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр131);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр111, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр121, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр131, "Раздел21");
	
	КС = ПоказателиКС(Р2_1_1_Стр131.Значение, Р2_1_1_Стр111.Значение * Р2_1_1_Стр121.Значение / 100);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р2_1_1_Стр111.ЗначениеСтр + " * " + Р2_1_1_Стр121.ЗначениеСтр
	+ " / 100 Итог [" + КС.ПраваяЧастьСтр + "]";
...
&НаСервере
Функция ПроверитьКС_1_26(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.1 (КС 1.26)";
	ОписаниеКС     = "Сумма исчисленного налога за девять месяцев (строка 132 Раздела 2.1.1 должна быть равна произведению значений строк 112 и 122 Раздела 2.1.1, деленому на 100";
	Нарушение       = "ст. 346.20, п. 3 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу за девять месяцев";
	
	Р2_1_1_Стр112 = ПоказательПоСтроке("Р2_1_1_Стр112");
	Р2_1_1_Стр122 = ПоказательПоСтроке("Р2_1_1_Стр122");
	Р2_1_1_Стр132 = ПоказательПоСтроке("Р2_1_1_Стр132");
	
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр112);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр122);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр132);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр112, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр122, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр132, "Раздел21");
	
	КС = ПоказателиКС(Р2_1_1_Стр132.Значение, Р2_1_1_Стр112.Значение * Р2_1_1_Стр122.Значение / 100);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р2_1_1_Стр112.ЗначениеСтр + " * " + Р2_1_1_Стр122.ЗначениеСтр
	+ " / 100 Итог [" + КС.ПраваяЧастьСтр + "]";
...
&НаСервере
Функция ПроверитьКС_1_27(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.1 (КС 1.27)";
	ОписаниеКС     = "Сумма исчисленного налога за налоговый период (строка 133 Раздела 2.1.1 должна быть равна произведению значений строк 113 и 123 Раздела 2.1.1, деленому на 100";
	Нарушение       = "пп. 1 и 5 ст. 346.18, ст. 346.20 и п. 1 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы авансового платежа по налогу за налоговый период";
	
	Р2_1_1_Стр113 = ПоказательПоСтроке("Р2_1_1_Стр113");
	Р2_1_1_Стр123 = ПоказательПоСтроке("Р2_1_1_Стр123");
	Р2_1_1_Стр133 = ПоказательПоСтроке("Р2_1_1_Стр133");
	
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр113);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр123);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр133);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр113, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр123, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр133, "Раздел21");
	
	КС = ПоказателиКС(Р2_1_1_Стр133.Значение, Р2_1_1_Стр113.Значение * Р2_1_1_Стр123.Значение / 100);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р2_1_1_Стр113.ЗначениеСтр + " * " + Р2_1_1_Стр123.ЗначениеСтр
	+ " / 100 Итог [" + КС.ПраваяЧастьСтр + "]";
...
&НаСервере
Функция ПроверитьКС_1_28(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.1 (КС 1.28)";
	Нарушение      = "п. 3.1 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма страховых взносов, уменьшающая сумму исчисленного за первый квартал авансового платежа по налогу";
	...
	Если Р2_1_1_Стр102.ЗначениеСтр = "1" Тогда
		КС = ПоказателиКС(Р2_1_1_Стр140.Значение, Р2_1_1_Стр130.Значение / 2);
		ОписаниеКС = "Сумма страховых взносов за первый квартал по строке 140 должна быть меньше или равна значению строки 130 Раздела 2.1.1 деленому на 2, если строка 102 Раздела 2.1.1 = ""1""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + Р2_1_1_Стр130.ЗначениеСтр + " / 2 Итог [" + КС.ПраваяЧастьСтр + "]";
	Иначе // Р2_1_1_Стр102.ЗначениеСтр = "2"
		КС = ПоказателиКС(Р2_1_1_Стр140.Значение, Р2_1_1_Стр130.Значение);
		ОписаниеКС = "Сумма страховых взносов за первый квартал по строке 140 должна быть меньше или равна значению строки 130 Раздела 2.1.1, если строка 102 Раздела 2.1.1 = ""2""";
...
&НаСервере
Функция ПроверитьКС_1_29(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.1 (КС 1.29)";
	Нарушение      = "п. 3.1 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма страховых взносов, уменьшающая сумму исчисленного за полугодие авансового платежа по налогу";
	...
	Если Р2_1_1_Стр102.ЗначениеСтр = "1" Тогда
		КС = ПоказателиКС(Р2_1_1_Стр141.Значение, Р2_1_1_Стр131.Значение / 2);
		ОписаниеКС = "Сумма страховых взносов за полугодие по строке 141 должна быть меньше или равна значению строки 131 Раздела 2.1.1 деленому на 2, если строка 102 Раздела 2.1.1 = ""1""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + Р2_1_1_Стр131.ЗначениеСтр + " / 2 Итог [" + КС.ПраваяЧастьСтр + "]";
	Иначе // Р2_1_1_Стр102.ЗначениеСтр = "2"
		КС = ПоказателиКС(Р2_1_1_Стр141.Значение, Р2_1_1_Стр131.Значение);
		ОписаниеКС = "Сумма страховых взносов за полугодие по строке 141 должна быть меньше или равна значению строки 131 Раздела 2.1.1, если строка 102 Раздела 2.1.1 = ""2""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + КС.ПраваяЧастьСтр;
	КонецЕсли;
...
&НаСервере
Функция ПроверитьКС_1_30(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.1 (КС 1.30)";
	Нарушение      = "п. 3.1 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма страховых взносов, уменьшающая сумму исчисленного за девять месяцев авансового платежа по налогу";
	...
	Если Р2_1_1_Стр102.ЗначениеСтр = "1" Тогда
		КС = ПоказателиКС(Р2_1_1_Стр142.Значение, Р2_1_1_Стр132.Значение / 2);
		ОписаниеКС = "Сумма страховых взносов за девять месяцев по строке 142 должна быть меньше или равна значению строки 132 Раздела 2.1.1 деленому на 2, если строка 102 Раздела 2.1.1 = ""1""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + Р2_1_1_Стр132.ЗначениеСтр + " / 2 Итог [" + КС.ПраваяЧастьСтр + "]";
	Иначе // Р2_1_1_Стр102.ЗначениеСтр = "2"
		КС = ПоказателиКС(Р2_1_1_Стр142.Значение, Р2_1_1_Стр132.Значение);
		ОписаниеКС = "Сумма страховых взносов за девять месяцев по строке 142 должна быть меньше или равна значению строки 132 Раздела 2.1.1, если строка 102 Раздела 2.1.1 = ""2""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + КС.ПраваяЧастьСтр;
	КонецЕсли;
...
&НаСервере
Функция ПроверитьКС_1_31(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.1 (КС 1.31)";
	Нарушение      = "п. 3.1 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма страховых взносов, уменьшающая сумму исчисленного за налоговый период по налогу";
	...
	Если Р2_1_1_Стр102.ЗначениеСтр = "1" Тогда
		КС = ПоказателиКС(Р2_1_1_Стр143.Значение, Р2_1_1_Стр133.Значение / 2);
		ОписаниеКС = "Сумма страховых взносов за налоговый период по строке 143 должна быть меньше или равна значению строки 133 Раздела 2.1.1 деленому на 2, если строка 102 Раздела 2.1.1 = ""1""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + Р2_1_1_Стр133.ЗначениеСтр + " / 2 Итог [" + КС.ПраваяЧастьСтр + "]";
	Иначе // Р2_1_1_Стр102.ЗначениеСтр = "2"
		КС = ПоказателиКС(Р2_1_1_Стр143.Значение, Р2_1_1_Стр133.Значение);
		ОписаниеКС = "Сумма страховых взносов за налоговый период по строке 143 должна быть меньше или равна значению строки 133 Раздела 2.1.1, если строка 102 Раздела 2.1.1 = ""2""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + КС.ПраваяЧастьСтр;
	КонецЕсли;
...
&НаСервере
Функция ПроверитьКС_1_31_1(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.1)";
	ОписаниеКС     = "Сумма исчисленного налога за первый квартал (строка 130 Раздела 2.1.2 должна быть равна произведению значений строки 110 Раздела 2.1.2 и 120 Раздела 2.1.1, деленому на 100";
	Нарушение      = "ст. 346.20, п. 3 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неверное исчисление суммы авансового платежа по налогу за первый квартал";
	
	Р2_1_1_Стр120 = ПоказательПоСтроке("Р2_1_1_Стр120");
	Р2_1_2_Стр110 = ПоказательПоСтроке("Р2_1_2_Стр110");
	Р2_1_2_Стр130 = ПоказательПоСтроке("Р2_1_2_Стр130");
	
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр120);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр110);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр130);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр120, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр110, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр130, "Раздел21");
	
	КС = ПоказателиКС(Р2_1_2_Стр130.Значение, Р2_1_2_Стр110.Значение * Р2_1_1_Стр120.Значение / 100);
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р2_1_2_Стр110.ЗначениеСтр + " * " + Р2_1_1_Стр120.ЗначениеСтр
	+ " / 100 Итог [" + КС.ПраваяЧастьСтр + "]";
...
&НаСервере
Функция ПроверитьКС_1_31_2(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.2)";
	ОписаниеКС     = "Сумма исчисленного налога за полугодие (строка 131 Раздела 2.1.2 должна быть равна произведению значений строки 111 Раздела 2.1.2 и 121 Раздела 2.1.1, деленому на 100";
	Нарушение      = "ст. 346.20, п. 3 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неверное исчисление суммы авансового платежа по налогу за полугодие";
	
	Р2_1_1_Стр121 = ПоказательПоСтроке("Р2_1_1_Стр121");
	Р2_1_2_Стр111 = ПоказательПоСтроке("Р2_1_2_Стр111");
	Р2_1_2_Стр131 = ПоказательПоСтроке("Р2_1_2_Стр131");
	
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр121);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр111);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр131);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр121, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр111, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр131, "Раздел21");
	
	КС = ПоказателиКС(Р2_1_2_Стр131.Значение, Р2_1_2_Стр111.Значение * Р2_1_1_Стр121.Значение / 100);
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р2_1_2_Стр111.ЗначениеСтр + " * " + Р2_1_1_Стр121.ЗначениеСтр
	+ " / 100 Итог [" + КС.ПраваяЧастьСтр + "]";
...
&НаСервере
Функция ПроверитьКС_1_31_3(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.3)";
	ОписаниеКС     = "Сумма исчисленного налога за девять месяцев (строка 132 Раздела 2.1.2 должна быть равна произведению значений строки 112 Раздела 2.1.2 и 122 Раздела 2.1.1, деленому на 100";
	Нарушение      = "ст. 346.20, п. 3 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неверное исчисление суммы авансового платежа по налогу за девять месяцев";
	
	Р2_1_1_Стр122 = ПоказательПоСтроке("Р2_1_1_Стр122");
	Р2_1_2_Стр112 = ПоказательПоСтроке("Р2_1_2_Стр112");
	Р2_1_2_Стр132 = ПоказательПоСтроке("Р2_1_2_Стр132");
	
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр122);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр112);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр132);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр122, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр112, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр132, "Раздел21");
	
	КС = ПоказателиКС(Р2_1_2_Стр132.Значение, Р2_1_2_Стр112.Значение * Р2_1_1_Стр122.Значение / 100);
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р2_1_2_Стр112.ЗначениеСтр + " * " + Р2_1_1_Стр122.ЗначениеСтр
	+ " / 100 Итог [" + КС.ПраваяЧастьСтр + "]";
...
&НаСервере
Функция ПроверитьКС_1_31_4(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.4)";
	ОписаниеКС     = "Сумма исчисленного налога за налоговый период (строка 133 Раздела 2.1.2 должна быть равна произведению значений строки 113 Раздела 2.1.2 и 123 Раздела 2.1.1, деленому на 100";
	Нарушение      = "ст. 346.20, п. 3 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неверное исчисление суммы налога за налоговый период";
	
	Р2_1_1_Стр123 = ПоказательПоСтроке("Р2_1_1_Стр123");
	Р2_1_2_Стр113 = ПоказательПоСтроке("Р2_1_2_Стр113");
	Р2_1_2_Стр133 = ПоказательПоСтроке("Р2_1_2_Стр133");
	
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр123);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр113);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр133);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр123, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр113, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр133, "Раздел21");
	
	КС = ПоказателиКС(Р2_1_2_Стр133.Значение, Р2_1_2_Стр113.Значение * Р2_1_1_Стр123.Значение / 100);
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р2_1_2_Стр113.ЗначениеСтр + " * " + Р2_1_1_Стр123.ЗначениеСтр
	+ " / 100 Итог [" + КС.ПраваяЧастьСтр + "]";
...
&НаСервере
Функция ПроверитьКС_1_31_5(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.5)";
	ОписаниеКС     = "";
	Нарушение      = "п. 3.1 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма страховых взносов, уменьшающая сумму исчисленного за первый квартал авансового платежа по налогу";
	...
	Р2_1_1_Стр102 = ПоказательПоСтроке("Р2_1_1_Стр102");
	Р2_1_2_Стр130 = ПоказательПоСтроке("Р2_1_2_Стр130");
	Р2_1_2_Стр140 = ПоказательПоСтроке("Р2_1_2_Стр140");
	
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр102);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр130);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр140);
	...
	Если Р2_1_1_Стр102.ЗначениеСтр = "1" Тогда
		КС = ПоказателиКС(Р2_1_2_Стр140.Значение, Р2_1_2_Стр130.Значение / 2);
		ОписаниеКС = "Сумма страховых взносов за первый квартал по строке 140 должна быть меньше или равна значению строки 130 Раздела 2.1.2 деленому на 2, если строка 102 Раздела 2.1.1 = ""1""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + Р2_1_2_Стр130.ЗначениеСтр + " / 2 Итог [" + КС.ПраваяЧастьСтр + "]";
	ИначеЕсли Р2_1_1_Стр102.ЗначениеСтр = "2" Тогда
		КС = ПоказателиКС(Р2_1_2_Стр140.Значение, Р2_1_2_Стр130.Значение);
		ОписаниеКС = "Сумма страховых взносов за первый квартал по строке 140 должна быть меньше или равна значению строки 130 Раздела 2.1.2, если строка 102 Раздела 2.1.1 = ""2""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + КС.ПраваяЧастьСтр;
	Иначе
		ПроверяемКС = Ложь;
	КонецЕсли;
	...
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр102, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр130, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр140, "Раздел21");
	
	Если ПроверяемКС Тогда
...
&НаСервере
Функция ПроверитьКС_1_31_6(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.6)";
	ОписаниеКС     = "";
	Нарушение      = "п. 3.1 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма страховых взносов, уменьшающая сумму исчисленного за полугодие авансового платежа по налогу";
	...
	Р2_1_1_Стр102 = ПоказательПоСтроке("Р2_1_1_Стр102");
	Р2_1_2_Стр131 = ПоказательПоСтроке("Р2_1_2_Стр131");
	Р2_1_2_Стр141 = ПоказательПоСтроке("Р2_1_2_Стр141");
	
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр102);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр131);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр141);
	...
	Если Р2_1_1_Стр102.ЗначениеСтр = "1" Тогда
		КС = ПоказателиКС(Р2_1_2_Стр141.Значение, Р2_1_2_Стр131.Значение / 2);
		ОписаниеКС = "Сумма страховых взносов за полугодие по строке 141 должна быть меньше или равна значению строки 131 Раздела 2.1.2 деленому на 2, если строка 102 Раздела 2.1.1 = ""1""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + Р2_1_2_Стр131.ЗначениеСтр + " / 2 Итог [" + КС.ПраваяЧастьСтр + "]";
	ИначеЕсли Р2_1_1_Стр102.ЗначениеСтр = "2" Тогда
		КС = ПоказателиКС(Р2_1_2_Стр141.Значение, Р2_1_2_Стр131.Значение);
		ОписаниеКС = "Сумма страховых взносов за полугодие по строке 141 должна быть меньше или равна значению строки 131 Раздела 2.1.2, если строка 102 Раздела 2.1.1 = ""2""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + КС.ПраваяЧастьСтр;
	Иначе
		ПроверяемКС = Ложь;
	КонецЕсли;
	...
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр102, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр131, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр141, "Раздел21");
...
&НаСервере
Функция ПроверитьКС_1_31_7(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.7)";
	ОписаниеКС     = "";
	Нарушение      = "п. 3.1 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма страховых взносов, уменьшающая сумму исчисленного за девять месяцев авансового платежа по налогу";
	...
	Р2_1_1_Стр102 = ПоказательПоСтроке("Р2_1_1_Стр102");
	Р2_1_2_Стр132 = ПоказательПоСтроке("Р2_1_2_Стр132");
	Р2_1_2_Стр142 = ПоказательПоСтроке("Р2_1_2_Стр142");
	
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр102);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр132);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр142);
	...
	Если Р2_1_1_Стр102.ЗначениеСтр = "1" Тогда
		КС = ПоказателиКС(Р2_1_2_Стр142.Значение, Р2_1_2_Стр132.Значение / 2);
		ОписаниеКС = "Сумма страховых взносов за девять месяцев по строке 142 должна быть меньше или равна значению строки 132 Раздела 2.1.2 деленому на 2, если строка 102 Раздела 2.1.1 = ""1""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + Р2_1_2_Стр132.ЗначениеСтр + " / 2 Итог [" + КС.ПраваяЧастьСтр + "]";
	ИначеЕсли Р2_1_1_Стр102.ЗначениеСтр = "2" Тогда
		КС = ПоказателиКС(Р2_1_2_Стр142.Значение, Р2_1_2_Стр132.Значение);
		ОписаниеКС = "Сумма страховых взносов за девять месяцев по строке 142 должна быть меньше или равна значению строки 132 Раздела 2.1.2, если строка 102 Раздела 2.1.1 = ""2""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + КС.ПраваяЧастьСтр;
	Иначе
		ПроверяемКС = Ложь;
	КонецЕсли;
	...
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр102, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр132, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр142, "Раздел21");
...
&НаСервере
Функция ПроверитьКС_1_31_8(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.8)";
	ОписаниеКС     = "";
	Нарушение      = "п. 3.1 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма страховых взносов, уменьшающая сумму исчисленного за налоговый период авансового платежа по налогу";
	
	Р2_1_1_Стр102 = ПоказательПоСтроке("Р2_1_1_Стр102");
	Р2_1_2_Стр133 = ПоказательПоСтроке("Р2_1_2_Стр133");
	Р2_1_2_Стр143 = ПоказательПоСтроке("Р2_1_2_Стр143");
	
	Раздел21 = мДанныеОтчета.ПолеТабличногоДокументаРаздел21;
	
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_1_Стр102);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр133);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр143);
	...
	Если Р2_1_1_Стр102.ЗначениеСтр = "1" Тогда
		КС = ПоказателиКС(Р2_1_2_Стр143.Значение, Р2_1_2_Стр133.Значение / 2);
		ОписаниеКС = "Сумма страховых взносов за налоговый период по строке 143 должна быть меньше или равна значению строки 133 Раздела 2.1.2 деленому на 2, если строка 102 Раздела 2.1.1 = ""1""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + Р2_1_2_Стр133.ЗначениеСтр + " / 2 Итог [" + КС.ПраваяЧастьСтр + "]";
	ИначеЕсли Р2_1_1_Стр102.ЗначениеСтр = "2" Тогда
		КС = ПоказателиКС(Р2_1_2_Стр143.Значение, Р2_1_2_Стр133.Значение);
		ОписаниеКС = "Сумма страховых взносов за налоговый период по строке 143 должна быть меньше или равна значению строки 133 Раздела 2.1.2, если строка 102 Раздела 2.1.1 = ""2""";
		Расшифровка = КС.ЛеваяЧастьСтр + " <= " + КС.ПраваяЧастьСтр;
	Иначе
		ПроверяемКС = Ложь;
	КонецЕсли;
	...
	ВнестиПоказательВРасшифровку(Р2_1_1_Стр102, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр133, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр143, "Раздел21");
...
&НаСервере
Функция ПроверитьКС_1_31_9а(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.9а)";
	ОписаниеКС     = "Сумма уплаченного торгового сбора за первый квартал по строке 160 Раздела 2.1.2 должна быть равна разнице значений по строкам 130 и 140 раздела 2.1.2, если (строка 130 - строка 140) Раздела 2.1.2 < строка 150 Раздела 2.1.2 и строка 160 Раздела 2.1.2 <= (строка 130 - строка 140) Раздела 2.1.1";
	Нарушение      = "п. 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма уплаченного сбора, уменьшающая сумму исчисиленного за первый квартал авансового платежа по налогу";
	
	Р2_1_1_Стр130 = ПоказательПоСтроке("Р2_1_1_Стр130");
	Р2_1_1_Стр140 = ПоказательПоСтроке("Р2_1_1_Стр140");
	Р2_1_2_Стр130 = ПоказательПоСтроке("Р2_1_2_Стр130");
	Р2_1_2_Стр140 = ПоказательПоСтроке("Р2_1_2_Стр140");
	Р2_1_2_Стр150 = ПоказательПоСтроке("Р2_1_2_Стр150");
	Р2_1_2_Стр160 = ПоказательПоСтроке("Р2_1_2_Стр160");
	...
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр130);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр140);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр150);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр160);
	...
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр130, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр140, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр150, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр160, "Раздел21");
	
	Условие1Слева = Р2_1_2_Стр130.Значение - Р2_1_2_Стр140.Значение;
	Условие1СлеваСтр = ЧислоСтрЧГ0(Условие1Слева);
	Условие1СлеваРасшифровка = Р2_1_2_Стр130.ЗначениеСтр + " - " + Р2_1_2_Стр140.ЗначениеСтр
	+ " Итог [" + Число(Условие1СлеваСтр) + "]";
	
	Условие1Справа = Р2_1_2_Стр150.Значение;
	Условие1СправаСтр = ЧислоСтрЧГ0(Условие1Справа);
	
	Условие2Слева = Р2_1_2_Стр160.Значение;
	Условие2СлеваСтр = ЧислоСтрЧГ0(Условие2Слева);
	
	Условие2Справа = Р2_1_1_Стр130.Значение - Р2_1_1_Стр140.Значение;
	Условие2СправаСтр = ЧислоСтрЧГ0(Условие2Справа);
	Условие2СправаРасшифровка = Р2_1_1_Стр130.ЗначениеСтр + " - " + Р2_1_1_Стр140.ЗначениеСтр
	+ " Итог [" + Число(Условие2СправаСтр) + "]";
	
	КС = ПоказателиКС(Р2_1_2_Стр160.Значение, Условие1Слева);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Условие1СлеваРасшифровка
	+ ", если " + Условие1СлеваРасшифровка + " < " + Условие1СправаСтр
	+ " и " + Условие2СлеваСтр + " <= " + Условие2СправаРасшифровка;
	
	УсловиеВыполнено = Условие1Слева <Условие1Справа И Условие2Слева <= Условие2Справа;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_31_9б(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.9б)";
	ОписаниеКС     = "Сумма уплаченного торгового сбора за первый квартал по строке 160 Раздела 2.1.2 должна быть равна сумме торгового сбора, уплаченного в течение первого квартала по строке 150 Раздела 2.1.2, если (строка 130 - строка 140) Раздела 2.1.2 >= строка 150 Раздела 2.1.2 и строка 160 Раздела 2.1.2 <= (строка 130 - строка 140) Раздела 2.1.1";
	Нарушение      = "п. 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма уплаченного сбора, уменьшающая сумму исчисиленного за первый квартал авансового платежа по налогу";
	...
	КС = ПоказателиКС(Р2_1_2_Стр160.Значение, Условие1Справа);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Условие1СправаСтр
	+ ", если " + Условие1СлеваРасшифровка + " >= " + Условие1СправаСтр
	+ " и " + Условие2СлеваСтр + " <= " + Условие2СправаРасшифровка;
...
&НаСервере
Функция ПроверитьКС_1_31_10а(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.10а)";
	ОписаниеКС     = "Сумма уплаченного торгового сбора за полугодие по строке 161 Раздела 2.1.2 должна быть равна разнице значений по строкам 131 и 141 раздела 2.1.2, если (строка 131 - строка 141) Раздела 2.1.2 < строка 151 Раздела 2.1.2 и строка 161 Раздела 2.1.2 <= (строка 131 - строка 141) Раздела 2.1.1";
	Нарушение      = "п. 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма уплаченного сбора, уменьшающая сумму исчисиленного за полугодие авансового платежа по налогу";
	
	Р2_1_1_Стр131 = ПоказательПоСтроке("Р2_1_1_Стр131");
	Р2_1_1_Стр141 = ПоказательПоСтроке("Р2_1_1_Стр141");
	Р2_1_2_Стр131 = ПоказательПоСтроке("Р2_1_2_Стр131");
	Р2_1_2_Стр141 = ПоказательПоСтроке("Р2_1_2_Стр141");
	Р2_1_2_Стр151 = ПоказательПоСтроке("Р2_1_2_Стр151");
	Р2_1_2_Стр161 = ПоказательПоСтроке("Р2_1_2_Стр161");
	...
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр131);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр141);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр151);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр161);
	...
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр131, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр141, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр151, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр161, "Раздел21");
	
	Условие1Слева = Р2_1_2_Стр131.Значение - Р2_1_2_Стр141.Значение;
	Условие1СлеваСтр = ЧислоСтрЧГ0(Условие1Слева);
	Условие1СлеваРасшифровка = Р2_1_2_Стр131.ЗначениеСтр + " - " + Р2_1_2_Стр141.ЗначениеСтр
	+ " Итог [" + Число(Условие1СлеваСтр) + "]";
	
	Условие1Справа = Р2_1_2_Стр151.Значение;
	Условие1СправаСтр = ЧислоСтрЧГ0(Условие1Справа);
	
	Условие2Слева = Р2_1_2_Стр161.Значение;
	Условие2СлеваСтр = ЧислоСтрЧГ0(Условие2Слева);
	
	Условие2Справа = Р2_1_1_Стр131.Значение - Р2_1_1_Стр141.Значение;
	Условие2СправаСтр = ЧислоСтрЧГ0(Условие2Справа);
	Условие2СправаРасшифровка = Р2_1_1_Стр131.ЗначениеСтр + " - " + Р2_1_1_Стр141.ЗначениеСтр
	+ " Итог [" + Число(Условие2СправаСтр) + "]";
	
	КС = ПоказателиКС(Р2_1_2_Стр161.Значение, Условие1Слева);
...
&НаСервере
Функция ПроверитьКС_1_31_10б(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.10б)";
	ОписаниеКС     = "Сумма уплаченного торгового сбора за полугодие по строке 161 Раздела 2.1.2 должна быть равна сумме торгового сбора, уплаченного в течение полугодия по строке 151 Раздела 2.1.2, если (строка 131 - строка 141) Раздела 2.1.2 >= строка 151 Раздела 2.1.2 и строка 161 Раздела 2.1.2 <= (строка 131 - строка 141) Раздела 2.1.1";
	Нарушение      = "п. 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма уплаченного сбора, уменьшающая сумму исчисиленного за полугодие авансового платежа по налогу";
	...
	КС = ПоказателиКС(Р2_1_2_Стр161.Значение, Условие1Справа);
...
&НаСервере
Функция ПроверитьКС_1_31_11а(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.11а)";
	ОписаниеКС     = "Сумма уплаченного торгового сбора за девять месяцев по строке 162 Раздела 2.1.2 должна быть равна разнице значений по строкам 132 и 142 раздела 2.1.2, если (строка 132 - строка 142) Раздела 2.1.2 < строка 152 Раздела 2.1.2 и строка 162 Раздела 2.1.2 <= (строка 132 - строка 142) Раздела 2.1.1";
	Нарушение      = "п. 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма уплаченного сбора, уменьшающая сумму исчисиленного за девять месяцев авансового платежа по налогу";
	
	Р2_1_1_Стр132 = ПоказательПоСтроке("Р2_1_1_Стр132");
	Р2_1_1_Стр142 = ПоказательПоСтроке("Р2_1_1_Стр142");
	Р2_1_2_Стр132 = ПоказательПоСтроке("Р2_1_2_Стр132");
	Р2_1_2_Стр142 = ПоказательПоСтроке("Р2_1_2_Стр142");
	Р2_1_2_Стр152 = ПоказательПоСтроке("Р2_1_2_Стр152");
	Р2_1_2_Стр162 = ПоказательПоСтроке("Р2_1_2_Стр162");
	...
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр132);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр142);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр152);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр162);
	...
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр132, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр142, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр152, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр162, "Раздел21");
	
	Условие1Слева = Р2_1_2_Стр132.Значение - Р2_1_2_Стр142.Значение;
	Условие1СлеваСтр = ЧислоСтрЧГ0(Условие1Слева);
	Условие1СлеваРасшифровка = Р2_1_2_Стр132.ЗначениеСтр + " - " + Р2_1_2_Стр142.ЗначениеСтр
	+ " Итог [" + Число(Условие1СлеваСтр) + "]";
	
	Условие1Справа = Р2_1_2_Стр152.Значение;
	Условие1СправаСтр = ЧислоСтрЧГ0(Условие1Справа);
	
	Условие2Слева = Р2_1_2_Стр162.Значение;
	Условие2СлеваСтр = ЧислоСтрЧГ0(Условие2Слева);
	
	Условие2Справа = Р2_1_1_Стр132.Значение - Р2_1_1_Стр142.Значение;
	Условие2СправаСтр = ЧислоСтрЧГ0(Условие2Справа);
	Условие2СправаРасшифровка = Р2_1_1_Стр132.ЗначениеСтр + " - " + Р2_1_1_Стр142.ЗначениеСтр
	+ " Итог [" + Число(Условие2СправаСтр) + "]";
	
	КС = ПоказателиКС(Р2_1_2_Стр162.Значение, Условие1Слева);
...
&НаСервере
Функция ПроверитьКС_1_31_11б(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.11б)";
	ОписаниеКС     = "Сумма уплаченного торгового сбора за девять месяцев по строке 162 Раздела 2.1.2 должна быть равна сумме торгового сбора, уплаченного в течение девяти месяцев по строке 152 Раздела 2.1.2, если (строка 132 - строка 142) Раздела 2.1.2 >= строка 152 Раздела 2.1.2 и строка 162 Раздела 2.1.2 <= (строка 132 - строка 142) Раздела 2.1.1";
	Нарушение      = "п. 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма уплаченного сбора, уменьшающая сумму исчисиленного за девять месяцев авансового платежа по налогу";
	...
	КС = ПоказателиКС(Р2_1_2_Стр162.Значение, Условие1Справа);
...
&НаСервере
Функция ПроверитьКС_1_31_12а(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.12а)";
	ОписаниеКС     = "Сумма уплаченного торгового сбора за налоговый период по строке 163 Раздела 2.1.2 должна быть равна разнице значений по строкам 133 и 143 раздела 2.1.2, если (строка 133 - строка 143) Раздела 2.1.2 < строка 153 Раздела 2.1.2 и строка 163 Раздела 2.1.2 <= (строка 133 - строка 143) Раздела 2.1.1";
	Нарушение      = "п. 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма уплаченного сбора, уменьшающая сумму исчисиленного за налоговый период авансового платежа по налогу";
	
	Р2_1_1_Стр133 = ПоказательПоСтроке("Р2_1_1_Стр133");
	Р2_1_1_Стр143 = ПоказательПоСтроке("Р2_1_1_Стр143");
	Р2_1_2_Стр133 = ПоказательПоСтроке("Р2_1_2_Стр133");
	Р2_1_2_Стр143 = ПоказательПоСтроке("Р2_1_2_Стр143");
	Р2_1_2_Стр153 = ПоказательПоСтроке("Р2_1_2_Стр153");
	Р2_1_2_Стр163 = ПоказательПоСтроке("Р2_1_2_Стр163");
	...
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр133);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр143);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр153);
	ВставитьЗначениеПоказателя(Раздел21, Р2_1_2_Стр163);
	...
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр133, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр143, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр153, "Раздел21");
	ВнестиПоказательВРасшифровку(Р2_1_2_Стр163, "Раздел21");
	
	Условие1Слева = Р2_1_2_Стр133.Значение - Р2_1_2_Стр143.Значение;
	Условие1СлеваСтр = ЧислоСтрЧГ0(Условие1Слева);
	Условие1СлеваРасшифровка = Р2_1_2_Стр133.ЗначениеСтр + " - " + Р2_1_2_Стр143.ЗначениеСтр
	+ " Итог [" + Число(Условие1СлеваСтр) + "]";
	
	Условие1Справа = Р2_1_2_Стр153.Значение;
	Условие1СправаСтр = ЧислоСтрЧГ0(Условие1Справа);
	
	Условие2Слева = Р2_1_2_Стр163.Значение;
	Условие2СлеваСтр = ЧислоСтрЧГ0(Условие2Слева);
	
	Условие2Справа = Р2_1_1_Стр133.Значение - Р2_1_1_Стр143.Значение;
	Условие2СправаСтр = ЧислоСтрЧГ0(Условие2Справа);
	Условие2СправаРасшифровка = Р2_1_1_Стр133.ЗначениеСтр + " - " + Р2_1_1_Стр143.ЗначениеСтр
	+ " Итог [" + Число(Условие2СправаСтр) + "]";
	
	КС = ПоказателиКС(Р2_1_2_Стр163.Значение, Условие1Слева);
...
&НаСервере
Функция ПроверитьКС_1_31_12б(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.1.2 (КС 1.31.12б)";
	ОписаниеКС     = "Сумма уплаченного торгового сбора за налоговый период по строке 163 Раздела 2.1.2 должна быть равна сумме торгового сбора, уплаченного в течение налогового периода по строке 153 Раздела 2.1.2, если (строка 133 - строка 143) Раздела 2.1.2 >= строка 153 Раздела 2.1.2 и строка 163 Раздела 2.1.2 <= (строка 133 - строка 143) Раздела 2.1.1";
	Нарушение      = "п. 8 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильно указана сумма уплаченного сбора, уменьшающая сумму исчисиленного за налоговый период авансового платежа по налогу";
	...
	КС = ПоказателиКС(Р2_1_2_Стр163.Значение, Условие1Справа);
...
&НаСервере
Функция ПроверитьКС_1_32(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.32)";
	ОписаниеКС     = "Налоговая база для исчисления налога за первый квартал (строка 240 Раздел 2.2) должна быть равна результату вычитания значения строки 220 Раздела 2.2 из величины по строке 210 Раздела 2.2, если результат этого вычитания больше 0";
	Нарушение      = "п. 2 ст. 346.18 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление налоговой базы за первый квартал";
	
	Р2_2_Стр210 = ПоказательПоСтроке("Р2_2_Стр210");
	Р2_2_Стр220 = ПоказательПоСтроке("Р2_2_Стр220");
	Р2_2_Стр240 = ПоказательПоСтроке("Р2_2_Стр240");
	
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр210);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр220);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр240);
	...
	ВнестиПоказательВРасшифровку(Р2_2_Стр210, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр220, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр240, "Раздел22");
	
	Условие1Слева = Р2_2_Стр210.Значение - Р2_2_Стр220.Значение;
	Условие1СлеваСтр = ЧислоСтрЧГ0(Условие1Слева);
	Условие1СлеваРасшифровка = Р2_2_Стр210.ЗначениеСтр + " - " + Р2_2_Стр220.ЗначениеСтр + " Итог ["
		+ Число(Условие1СлеваСтр) + "]";
	
	КС = ПоказателиКС(Р2_2_Стр240.Значение, Условие1Слева);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + Условие1СлеваРасшифровка + ", если " + Условие1СлеваРасшифровка + " > 0";
	
	УсловиеВыполнено = Условие1Слева > 0;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_33(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.33)";
	ОписаниеКС     = "Налоговая база для исчисления налога за полугодие (строка 241 Раздел 2.2) должна быть равна результату вычитания значения строки 221 Раздела 2.2 из величины по строке 211 Раздела 2.2, если результат этого вычитания больше 0";
	Нарушение      = "п. 2 ст. 346.18 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление налоговой базы за полугодие";
	
	Р2_2_Стр211 = ПоказательПоСтроке("Р2_2_Стр211");
	Р2_2_Стр221 = ПоказательПоСтроке("Р2_2_Стр221");
	Р2_2_Стр241 = ПоказательПоСтроке("Р2_2_Стр241");
	
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр211);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр221);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр241);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р2_2_Стр211, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр221, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр241, "Раздел22");
	
	Условие1Слева = Р2_2_Стр211.Значение - Р2_2_Стр221.Значение;
	Условие1СлеваСтр = ЧислоСтрЧГ0(Условие1Слева);
	Условие1СлеваРасшифровка = Р2_2_Стр211.ЗначениеСтр + " - " + Р2_2_Стр221.ЗначениеСтр + " Итог ["
		+ Число(Условие1СлеваСтр) + "]";
	
	КС = ПоказателиКС(Р2_2_Стр241.Значение, Условие1Слева);
...
&НаСервере
Функция ПроверитьКС_1_34(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.34)";
	ОписаниеКС     = "Налоговая база для исчисления налога за девять месяцев (строка 242 Раздел 2.2) должна быть равна результату вычитания значения строки 222 Раздела 2.2 из величины по строке 212 Раздела 2.2, если результат этого вычитания больше 0";
	Нарушение      = "п. 2 ст. 346.18 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление налоговой базы за девять месяцев";
	
	Р2_2_Стр212 = ПоказательПоСтроке("Р2_2_Стр212");
	Р2_2_Стр222 = ПоказательПоСтроке("Р2_2_Стр222");
	Р2_2_Стр242 = ПоказательПоСтроке("Р2_2_Стр242");
	
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр212);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр222);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр242);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р2_2_Стр212, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр222, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр242, "Раздел22");
	
	Условие1Слева = Р2_2_Стр212.Значение - Р2_2_Стр222.Значение;
	Условие1СлеваСтр = ЧислоСтрЧГ0(Условие1Слева);
	Условие1СлеваРасшифровка = Р2_2_Стр212.ЗначениеСтр + " - " + Р2_2_Стр222.ЗначениеСтр + " Итог ["
		+ Число(Условие1СлеваСтр) + "]";
	
	КС = ПоказателиКС(Р2_2_Стр242.Значение, Условие1Слева);
...
&НаСервере
Функция ПроверитьКС_1_35(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.35)";
	ОписаниеКС     = "Налоговая база для исчисления налога за налоговый период (строка 243 Раздел 2.2) должна быть равна результату вычитания значения строк 223 и 230 Раздела 2.2 из величины по строке 213 Раздела 2.2, если результат этого вычитания больше 0";
	Нарушение      = "п. 2 ст. 346.18 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление налоговой базы за налоговый период";
	
	Р2_2_Стр213 = ПоказательПоСтроке("Р2_2_Стр213");
	Р2_2_Стр223 = ПоказательПоСтроке("Р2_2_Стр223");
	Р2_2_Стр230 = ПоказательПоСтроке("Р2_2_Стр230");
	Р2_2_Стр243 = ПоказательПоСтроке("Р2_2_Стр243");
	
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр213);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр223);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр230);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр243);
	
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
	
	ВнестиПоказательВРасшифровку(Р2_2_Стр213, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр223, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр230, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр243, "Раздел22");
	
	Условие1Слева = Р2_2_Стр213.Значение - Р2_2_Стр223.Значение - Р2_2_Стр230.Значение;
	Условие1СлеваСтр = ЧислоСтрЧГ0(Условие1Слева);
	Условие1СлеваРасшифровка = Р2_2_Стр213.ЗначениеСтр + " - " + Р2_2_Стр223.ЗначениеСтр
	+ " - " + Р2_2_Стр230.ЗначениеСтр + " Итог [" + Число(Условие1СлеваСтр) + "]";
	
	КС = ПоказателиКС(Р2_2_Стр243.Значение, Условие1Слева);
...
&НаСервере
Функция ПроверитьКС_1_36(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.36)";
	ОписаниеКС     = "Сумма убытка, полученного за первый квартал (строка 250 Раздел 2.2) должна быть равна разнице значений по строкам 220 и 210 Раздела 2.2, если значение строки 210 Раздела 2.2 меньше значения по строке 220 Раздела 2.2";
	Нарушение      = "п. 7 ст. 346.18 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы полученного убытка за первый квартал";
	
	Р2_2_Стр210 = ПоказательПоСтроке("Р2_2_Стр210");
	Р2_2_Стр220 = ПоказательПоСтроке("Р2_2_Стр220");
	Р2_2_Стр250 = ПоказательПоСтроке("Р2_2_Стр250");
	
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр210);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр220);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр250);
	...
	ВнестиПоказательВРасшифровку(Р2_2_Стр210, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр220, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр250, "Раздел22");
	
	КС = ПоказателиКС(Р2_2_Стр250.Значение, Р2_2_Стр220.Значение - Р2_2_Стр210.Значение);
	
	КСПравРасшифровка = Р2_2_Стр220.ЗначениеСтр + " - " + Р2_2_Стр210.ЗначениеСтр
	+ " Итог [" + КС.ПраваяЧастьСтр + "]";
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + КСПравРасшифровка
	+ ", если " + Р2_2_Стр210.ЗначениеСтр + " < " + Р2_2_Стр220.ЗначениеСтр;
	
	УсловиеВыполнено = Р2_2_Стр210.Значение < Р2_2_Стр220.Значение;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_37(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.37)";
	ОписаниеКС     = "Сумма убытка, полученного за полугодие (строка 251 Раздел 2.2) должна быть равна разнице значений по строкам 221 и 211 Раздела 2.2, если значение строки 211 Раздела 2.2 меньше значения по строке 221 Раздела 2.2";
	Нарушение      = "п. 7 ст. 346.18 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы полученного убытка за полугодие";
	
	Р2_2_Стр211 = ПоказательПоСтроке("Р2_2_Стр211");
	Р2_2_Стр221 = ПоказательПоСтроке("Р2_2_Стр221");
	Р2_2_Стр251 = ПоказательПоСтроке("Р2_2_Стр251");
	
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр211);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр221);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр251);
	...
	ВнестиПоказательВРасшифровку(Р2_2_Стр211, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр221, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр251, "Раздел22");
	
	КС = ПоказателиКС(Р2_2_Стр251.Значение, Р2_2_Стр221.Значение - Р2_2_Стр211.Значение);
	
	КСПравРасшифровка = Р2_2_Стр221.ЗначениеСтр + " - " + Р2_2_Стр211.ЗначениеСтр
	+ " Итог [" + КС.ПраваяЧастьСтр + "]";
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + КСПравРасшифровка
	+ ", если " + Р2_2_Стр211.ЗначениеСтр + " < " + Р2_2_Стр221.ЗначениеСтр;
	
	УсловиеВыполнено = Р2_2_Стр211.Значение < Р2_2_Стр221.Значение;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_38(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.38)";
	ОписаниеКС     = "Сумма убытка, полученного за девять месяцев (строка 252 Раздел 2.2) должна быть равна разнице значений по строкам 222 и 212 Раздела 2.2, если значение строки 212 Раздела 2.2 меньше значения по строке 222 Раздела 2.2";
	Нарушение      = "п. 7 ст. 346.18 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы полученного убытка за девять месяцев";
	
	Р2_2_Стр212 = ПоказательПоСтроке("Р2_2_Стр212");
	Р2_2_Стр222 = ПоказательПоСтроке("Р2_2_Стр222");
	Р2_2_Стр252 = ПоказательПоСтроке("Р2_2_Стр252");
	
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр212);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр222);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр252);
	...
	ВнестиПоказательВРасшифровку(Р2_2_Стр212, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр222, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр252, "Раздел22");
	
	КС = ПоказателиКС(Р2_2_Стр252.Значение, Р2_2_Стр222.Значение - Р2_2_Стр212.Значение);
	
	КСПравРасшифровка = Р2_2_Стр222.ЗначениеСтр + " - " + Р2_2_Стр212.ЗначениеСтр
	+ " Итог [" + КС.ПраваяЧастьСтр + "]";
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + КСПравРасшифровка
	+ ", если " + Р2_2_Стр212.ЗначениеСтр + " < " + Р2_2_Стр222.ЗначениеСтр;
	
	УсловиеВыполнено = Р2_2_Стр212.Значение < Р2_2_Стр222.Значение;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_39(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.39)";
	ОписаниеКС     = "Сумма убытка, полученного за истекший налоговый период (строка 253 Раздел 2.2) должна быть равна разнице значений по строкам 223 и 213 Раздела 2.2, если значение строки 213 Раздела 2.2 меньше значения по строке 223 Раздела 2.2";
	Нарушение      = "п. 7 ст. 346.18 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы полученного убытка за истекший налоговый период";
	
	Р2_2_Стр213 = ПоказательПоСтроке("Р2_2_Стр213");
	Р2_2_Стр223 = ПоказательПоСтроке("Р2_2_Стр223");
	Р2_2_Стр253 = ПоказательПоСтроке("Р2_2_Стр253");
	
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр213);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр223);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр253);
	...
	ВнестиПоказательВРасшифровку(Р2_2_Стр213, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр223, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр253, "Раздел22");
	
	КС = ПоказателиКС(Р2_2_Стр253.Значение, Р2_2_Стр223.Значение - Р2_2_Стр213.Значение);
	
	КСПравРасшифровка = Р2_2_Стр223.ЗначениеСтр + " - " + Р2_2_Стр213.ЗначениеСтр
	+ " Итог [" + КС.ПраваяЧастьСтр + "]";
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + КСПравРасшифровка
	+ ", если " + Р2_2_Стр213.ЗначениеСтр + " < " + Р2_2_Стр223.ЗначениеСтр;
	
	УсловиеВыполнено = Р2_2_Стр213.Значение < Р2_2_Стр223.Значение;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_40(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.40)";
	ОписаниеКС     = "Сумма исчисленного налога за первый квартал (строка 270 Раздела 2.2) должна быть равна произведению налоговой базы (строка 240 Раздел 2.2) и налоговой ставки (строка 260 Раздел 2.2) деленой на 100, если строка 240 Раздел 2.2 > 0";
	Нарушение      = "п. 4 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы налога за первый квартал";
	
	Р2_2_Стр240 = ПоказательПоСтроке("Р2_2_Стр240");
	Р2_2_Стр260 = ПоказательПоСтроке("Р2_2_Стр260");
	Р2_2_Стр270 = ПоказательПоСтроке("Р2_2_Стр270");
	
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр240);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр260);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр270);
	...
	ВнестиПоказательВРасшифровку(Р2_2_Стр240, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр260, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр270, "Раздел22");
	
	КС = ПоказателиКС(Р2_2_Стр270.Значение, Р2_2_Стр240.Значение * Р2_2_Стр260.Значение / 100);
	
	КСПравРасшифровка = Р2_2_Стр240.ЗначениеСтр + " * " + Р2_2_Стр260.ЗначениеСтр
	+ " / 100 Итог [" + КС.ПраваяЧастьСтр + "]";
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + КСПравРасшифровка + ", если " + Р2_2_Стр240.ЗначениеСтр + " > 0 ";
	
	УсловиеВыполнено = Р2_2_Стр240.Значение > 0;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_41(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.41)";
	ОписаниеКС     = "Сумма исчисленного налога за полугодие (строка 271 Раздела 2.2) должна быть равна произведению налоговой базы (строка 241 Раздел 2.2) и налоговой ставки (строка 261 Раздел 2.2) деленой на 100, если строка 241 Раздел 2.2 > 0";
	Нарушение      = "п. 4 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы налога за полугодие";
	
	Р2_2_Стр241 = ПоказательПоСтроке("Р2_2_Стр241");
	Р2_2_Стр261 = ПоказательПоСтроке("Р2_2_Стр261");
	Р2_2_Стр271 = ПоказательПоСтроке("Р2_2_Стр271");
	
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр241);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр261);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр271);
	...
	ВнестиПоказательВРасшифровку(Р2_2_Стр241, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр261, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр271, "Раздел22");
	
	КС = ПоказателиКС(Р2_2_Стр271.Значение, Р2_2_Стр241.Значение * Р2_2_Стр261.Значение / 100);
	
	КСПравРасшифровка = Р2_2_Стр241.ЗначениеСтр + " * " + Р2_2_Стр261.ЗначениеСтр
	+ " / 100 Итог [" + КС.ПраваяЧастьСтр + "]";
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + КСПравРасшифровка + ", если " + Р2_2_Стр241.ЗначениеСтр + " > 0 ";
	
	УсловиеВыполнено = Р2_2_Стр241.Значение > 0;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_42(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.42)";
	ОписаниеКС     = "Сумма исчисленного налога за девять месяцев (строка 272 Раздела 2.2) должна быть равна произведению налоговой базы (строка 242 Раздел 2.2) и налоговой ставки (строка 262 Раздел 2.2) деленой на 100, если строка 242 Раздел 2.2 > 0";
	Нарушение      = "п. 4 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы налога за девять месяцев";
	
	Р2_2_Стр242 = ПоказательПоСтроке("Р2_2_Стр242");
	Р2_2_Стр262 = ПоказательПоСтроке("Р2_2_Стр262");
	Р2_2_Стр272 = ПоказательПоСтроке("Р2_2_Стр272");
	
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр242);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр262);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр272);
	...
	ВнестиПоказательВРасшифровку(Р2_2_Стр242, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр262, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр272, "Раздел22");
	
	КС = ПоказателиКС(Р2_2_Стр272.Значение, Р2_2_Стр242.Значение * Р2_2_Стр262.Значение / 100);
	
	КСПравРасшифровка = Р2_2_Стр242.ЗначениеСтр + " * " + Р2_2_Стр262.ЗначениеСтр
	+ " / 100 Итог [" + КС.ПраваяЧастьСтр + "]";
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + КСПравРасшифровка + ", если " + Р2_2_Стр242.ЗначениеСтр + " > 0 ";
	
	УсловиеВыполнено = Р2_2_Стр242.Значение > 0;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_43(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.43)";
	ОписаниеКС     = "Сумма исчисленного налога за налоговый период (строка 273 Раздела 2.2) должна быть равна произведению налоговой базы (строка 243 Раздел 2.2) и налоговой ставки (строка 263 Раздел 2.2) деленой на 100, если строка 243 Раздел 2.2 > 0";
	Нарушение      = "п. 1 ст. 346.21 НК РФ";
	ОписаниеОшибки = "Неправильное исчисление суммы налога за налоговый период";
	
	Р2_2_Стр243 = ПоказательПоСтроке("Р2_2_Стр243");
	Р2_2_Стр263 = ПоказательПоСтроке("Р2_2_Стр263");
	Р2_2_Стр273 = ПоказательПоСтроке("Р2_2_Стр273");
	
	Раздел22 = мДанныеОтчета.ПолеТабличногоДокументаРаздел22;
	
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр243);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр263);
	ВставитьЗначениеПоказателя(Раздел22, Р2_2_Стр273);
	...
	ВнестиПоказательВРасшифровку(Р2_2_Стр243, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр263, "Раздел22");
	ВнестиПоказательВРасшифровку(Р2_2_Стр273, "Раздел22");
	
	КС = ПоказателиКС(Р2_2_Стр273.Значение, Р2_2_Стр243.Значение * Р2_2_Стр263.Значение / 100);
	
	КСПравРасшифровка = Р2_2_Стр243.ЗначениеСтр + " * " + Р2_2_Стр263.ЗначениеСтр
	+ " / 100 Итог [" + КС.ПраваяЧастьСтр + "]";
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = " + КСПравРасшифровка + ", если " + Р2_2_Стр243.ЗначениеСтр + " > 0 ";
	
	УсловиеВыполнено = Р2_2_Стр243.Значение > 0;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_44(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.44)";
	ОписаниеКС     = "Сумма исчисленного минимального налога за налоговый период (строка 280 Раздел 2.2) должна быть равна 1% от значения по строке 213 Раздела 2.2";
	Нарушение      = "п. 6 ст. 346.18 НК РФ";
	ОписаниеОшибки = "Не исчислена сумма минимального налога за налоговый период";
	...
	КС = ПоказателиКС(Р2_2_Стр280.Значение, Р2_2_Стр213.Значение / 100);
	
	Процент1Стр = ЧислоСтрЧГ0(Р2_2_Стр213.Значение / 100);
	
	КСПравРасшифровка = Р2_2_Стр280.ЗначениеСтр + " / 100 Итог [" + КС.ПраваяЧастьСтр + "]";
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = 1% от " + Р2_2_Стр213.ЗначениеСтр + " Итог [" + Процент1Стр + "]";
...
&НаСервере
Функция ПроверитьКС_1_45(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 2.2 (КС 1.45)";
	ОписаниеКС     = "Сумма исчисленного минимального налога за налоговый период (строка 280 Раздел 2.2) должна быть равна 0, если строка 213 Раздела 2.2 = 0";
	Нарушение      = "п. 6 ст. 346.18 НК РФ";
	ОписаниеОшибки = "Неисчисление суммы минимального налога за налоговый период";
	...
	КС = ПоказателиКС(Р2_2_Стр280.Значение, 0);
	
	Расшифровка = КС.ЛеваяЧастьСтр + " = 0 , если " + Р2_2_Стр213.ЗначениеСтр + " = 0";
	
	УсловиеВыполнено = Р2_2_Стр213.Значение = 0;
	Если УсловиеВыполнено И НЕ (КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
	Иначе
...
&НаСервере
Функция ПроверитьКС_1_46(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы   = "Проверка раздела 3 (КС 1.46)";
	ОписаниеКС     = "Сумма средств, использованных не по назначению или не использованных в установленный срок (графа 7 Раздела 3) должна быть равна разнице граф 3, 4, 6 Раздела 3 (построчно)";
	Нарушение      = "Арифметическая ошибка";
	ОписаниеОшибки = "Неправильное определение суммы средств использованных не по назначению или не использованных по назначению в установленный срок";
	
	Р3_Гр3 = ПоказательПоСтроке("Р3_Гр3");
	Р3_Гр4 = ПоказательПоСтроке("Р3_Гр4");
	Р3_Гр6 = ПоказательПоСтроке("Р3_Гр6");
	Р3_Гр7 = ПоказательПоСтроке("Р3_Гр7");
	
	Раздел3 = мДанныеОтчета.ПолеТабличногоДокументаРаздел3;
	КолвоСтрокВМногострочнойЧасти = РегламентированнаяОтчетностьКлиентСервер.ПоказательВходитВМногострочнуюЧасть(ЭтотОбъект, "П000030000101_");
	
	Для Инд = 1 По КолвоСтрокВМногострочнойЧасти Цикл
		
		ВставитьЗначениеМногострочногоПоказателя(Раздел3, Р3_Гр3, Инд);
		ВставитьЗначениеМногострочногоПоказателя(Раздел3, Р3_Гр4, Инд);
		ВставитьЗначениеМногострочногоПоказателя(Раздел3, Р3_Гр6, Инд);
		ВставитьЗначениеМногострочногоПоказателя(Раздел3, Р3_Гр7, Инд);
		
		СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
		ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
		
		ВнестиПоказательВРасшифровку(Р3_Гр3, "Раздел3");
		ВнестиПоказательВРасшифровку(Р3_Гр4, "Раздел3");
		ВнестиПоказательВРасшифровку(Р3_Гр6, "Раздел3");
		ВнестиПоказательВРасшифровку(Р3_Гр7, "Раздел3");
		
		КС = ПоказателиКС(Р3_Гр7.Значение, Р3_Гр3.Значение - Р3_Гр4.Значение - Р3_Гр6.Значение);
		
		КСПравРасшифровка = Р3_Гр3.ЗначениеСтр + " - " + Р3_Гр4.ЗначениеСтр + " - " + Р3_Гр6.ЗначениеСтр
		+ " Итог [" + КС.ПраваяЧастьСтр + "]";
...
Функция Раздел2_1_2_Пуст(Раздел21)
	
	РазделПуст = Истина;
	
	Для Каждого ЭлСтрук Из Раздел21 Цикл
		Если СтрНайти(ЭлСтрук.Ключ, "П00212") > 0 Тогда
			Если Число(ЭлСтрук.Значение) > 0 Тогда
				Возврат НЕ РазделПуст;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	// Раздел 1.1
	//
	Если УсловныйКодПоказателя = "Р1_1_Стр020" Тогда
		Результат.Вставить("Имя", "П000110002003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 020 Раздел 1.1';
												|en = 'Строка 020 Раздел 1.1'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р1_1_Стр040" Тогда
		Результат.Вставить("Имя", "П000110004003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 040 Раздел 1.1';
												|en = 'Строка 040 Раздел 1.1'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р1_1_Стр050" Тогда
		Результат.Вставить("Имя", "П000110005003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 050 Раздел 1.1';
												|en = 'Строка 050 Раздел 1.1'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р1_1_Стр070" Тогда
		Результат.Вставить("Имя", "П000110007003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 070 Раздел 1.1';
												|en = 'Строка 070 Раздел 1.1'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р1_1_Стр080" Тогда
		Результат.Вставить("Имя", "П000110008003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 080 Раздел 1.1';
												|en = 'Строка 080 Раздел 1.1'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р1_1_Стр100" Тогда
		Результат.Вставить("Имя", "П000110010003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 100 Раздел 1.1';
												|en = 'Строка 100 Раздел 1.1'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р1_1_Стр110" Тогда
		Результат.Вставить("Имя", "П000110011003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 110 Раздел 1.1';
												|en = 'Строка 110 Раздел 1.1'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	// Раздел 1.2
	//
	ИначеЕсли УсловныйКодПоказателя = "Р1_2_Стр020" Тогда
		Результат.Вставить("Имя", "П000120002003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 020 Раздел 1.2';
												|en = 'Строка 020 Раздел 1.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р1_2_Стр040" Тогда
		Результат.Вставить("Имя", "П000120004003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 040 Раздел 1.2';
												|en = 'Строка 040 Раздел 1.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р1_2_Стр050" Тогда
		Результат.Вставить("Имя", "П000120005003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 050 Раздел 1.2';
												|en = 'Строка 050 Раздел 1.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р1_2_Стр070" Тогда
		Результат.Вставить("Имя", "П000120007003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 070 Раздел 1.2';
												|en = 'Строка 070 Раздел 1.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р1_2_Стр080" Тогда
		Результат.Вставить("Имя", "П000120008003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 080 Раздел 1.2';
												|en = 'Строка 080 Раздел 1.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р1_2_Стр100" Тогда
		Результат.Вставить("Имя", "П000120010003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 100 Раздел 1.2';
												|en = 'Строка 100 Раздел 1.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р1_2_Стр110" Тогда
		Результат.Вставить("Имя", "П000120011003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 110 Раздел 1.2';
												|en = 'Строка 110 Раздел 1.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р1_2_Стр120" Тогда
		Результат.Вставить("Имя", "П000120012003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 120 Раздел 1.2';
												|en = 'Строка 120 Раздел 1.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	// Раздел 2.1.1
	//
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр102" Тогда
		Результат.Вставить("Имя", "П002110010203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 102 Раздел 2.1.1.';
												|en = 'Строка 102 Раздел 2.1.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр110" Тогда
		Результат.Вставить("Имя", "П002110011003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 110 Раздел 2.1.1.';
												|en = 'Строка 110 Раздел 2.1.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр111" Тогда
		Результат.Вставить("Имя", "П002110011103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 111 Раздел 2.1.1.';
												|en = 'Строка 111 Раздел 2.1.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр112" Тогда
		Результат.Вставить("Имя", "П002110011203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 112 Раздел 2.1.1.';
												|en = 'Строка 112 Раздел 2.1.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр113" Тогда
		Результат.Вставить("Имя", "П002110011303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 113 Раздел 2.1.1.';
												|en = 'Строка 113 Раздел 2.1.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр120" Тогда
		Результат.Вставить("Имя", "П002110012003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 120 Раздел 2.1.1.';
												|en = 'Строка 120 Раздел 2.1.1.'"));
		//Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр121" Тогда
		Результат.Вставить("Имя", "П002110012103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 121 Раздел 2.1.1.';
												|en = 'Строка 121 Раздел 2.1.1.'"));
		//Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр122" Тогда
		Результат.Вставить("Имя", "П002110012203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 122 Раздел 2.1.1.';
												|en = 'Строка 122 Раздел 2.1.1.'"));
		//Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр123" Тогда
		Результат.Вставить("Имя", "П002110012303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 123 Раздел 2.1.1.';
												|en = 'Строка 123 Раздел 2.1.1.'"));
		//Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр130" Тогда
		Результат.Вставить("Имя", "П002110013003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 130 Раздел 2.1.1.';
												|en = 'Строка 130 Раздел 2.1.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр131" Тогда
		Результат.Вставить("Имя", "П002110013103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 131 Раздел 2.1.1.';
												|en = 'Строка 131 Раздел 2.1.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр132" Тогда
		Результат.Вставить("Имя", "П002110013203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 132 Раздел 2.1.1.';
												|en = 'Строка 132 Раздел 2.1.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр133" Тогда
		Результат.Вставить("Имя", "П002110013303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 133 Раздел 2.1.1.';
												|en = 'Строка 133 Раздел 2.1.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр140" Тогда
		Результат.Вставить("Имя", "П002110014003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 140 Раздел 2.1.1.';
												|en = 'Строка 140 Раздел 2.1.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр141" Тогда
		Результат.Вставить("Имя", "П002110014103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 141 Раздел 2.1.1.';
												|en = 'Строка 141 Раздел 2.1.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр142" Тогда
		Результат.Вставить("Имя", "П002110014203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 142 Раздел 2.1.1.';
												|en = 'Строка 142 Раздел 2.1.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_1_Стр143" Тогда
		Результат.Вставить("Имя", "П002110014303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 143 Раздел 2.1.1.';
												|en = 'Строка 143 Раздел 2.1.1.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	// Раздел 2.1.2
	//
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр110" Тогда
		Результат.Вставить("Имя", "П002120011003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 110 Раздел 2.1.2.';
												|en = 'Строка 110 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр111" Тогда
		Результат.Вставить("Имя", "П002120011103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 111 Раздел 2.1.2.';
												|en = 'Строка 111 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр112" Тогда
		Результат.Вставить("Имя", "П002120011203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 112 Раздел 2.1.2.';
												|en = 'Строка 112 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр113" Тогда
		Результат.Вставить("Имя", "П002120011303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 113 Раздел 2.1.2.';
												|en = 'Строка 113 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр130" Тогда
		Результат.Вставить("Имя", "П002120013003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 130 Раздел 2.1.2.';
												|en = 'Строка 130 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр131" Тогда
		Результат.Вставить("Имя", "П002120013103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 131 Раздел 2.1.2.';
												|en = 'Строка 131 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр132" Тогда
		Результат.Вставить("Имя", "П002120013203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 132 Раздел 2.1.2.';
												|en = 'Строка 132 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр133" Тогда
		Результат.Вставить("Имя", "П002120013303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 133 Раздел 2.1.2.';
												|en = 'Строка 133 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр140" Тогда
		Результат.Вставить("Имя", "П002120014003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 140 Раздел 2.1.2.';
												|en = 'Строка 140 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр141" Тогда
		Результат.Вставить("Имя", "П002120014103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 141 Раздел 2.1.2.';
												|en = 'Строка 141 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр142" Тогда
		Результат.Вставить("Имя", "П002120014203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 142 Раздел 2.1.2.';
												|en = 'Строка 142 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр143" Тогда
		Результат.Вставить("Имя", "П002120014303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 143 Раздел 2.1.2.';
												|en = 'Строка 143 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр150" Тогда
		Результат.Вставить("Имя", "П002120015003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 150 Раздел 2.1.2.';
												|en = 'Строка 150 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр151" Тогда
		Результат.Вставить("Имя", "П002120015103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 151 Раздел 2.1.2.';
												|en = 'Строка 151 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр152" Тогда
		Результат.Вставить("Имя", "П002120015203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 152 Раздел 2.1.2.';
												|en = 'Строка 152 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр153" Тогда
		Результат.Вставить("Имя", "П002120015303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 153 Раздел 2.1.2.';
												|en = 'Строка 153 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр160" Тогда
		Результат.Вставить("Имя", "П002120016003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 160 Раздел 2.1.2.';
												|en = 'Строка 160 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр161" Тогда
		Результат.Вставить("Имя", "П002120016103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 161 Раздел 2.1.2.';
												|en = 'Строка 161 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр162" Тогда
		Результат.Вставить("Имя", "П002120016203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 162 Раздел 2.1.2.';
												|en = 'Строка 162 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_1_2_Стр163" Тогда
		Результат.Вставить("Имя", "П002120016303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 163 Раздел 2.1.2.';
												|en = 'Строка 163 Раздел 2.1.2.'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	// Раздел 2.2
	//
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр210" Тогда
		Результат.Вставить("Имя", "П000220021003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 210 Раздел 2.2';
												|en = 'Строка 210 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр211" Тогда
		Результат.Вставить("Имя", "П000220021103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 211 Раздел 2.2';
												|en = 'Строка 211 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр212" Тогда
		Результат.Вставить("Имя", "П000220021203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 212 Раздел 2.2';
												|en = 'Строка 212 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр213" Тогда
		Результат.Вставить("Имя", "П000220021303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 213 Раздел 2.2';
												|en = 'Строка 213 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр220" Тогда
		Результат.Вставить("Имя", "П000220022003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 220 Раздел 2.2';
												|en = 'Строка 220 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр221" Тогда
		Результат.Вставить("Имя", "П000220022103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 221 Раздел 2.2';
												|en = 'Строка 221 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр222" Тогда
		Результат.Вставить("Имя", "П000220022203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 222 Раздел 2.2';
												|en = 'Строка 222 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр223" Тогда
		Результат.Вставить("Имя", "П000220022303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 223 Раздел 2.2';
												|en = 'Строка 223 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр230" Тогда
		Результат.Вставить("Имя", "П000220023003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 230 Раздел 2.2';
												|en = 'Строка 230 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр240" Тогда
		Результат.Вставить("Имя", "П000220024003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 240 Раздел 2.2';
												|en = 'Строка 240 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр241" Тогда
		Результат.Вставить("Имя", "П000220024103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 241 Раздел 2.2';
												|en = 'Строка 241 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр242" Тогда
		Результат.Вставить("Имя", "П000220024203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 242 Раздел 2.2';
												|en = 'Строка 242 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр243" Тогда
		Результат.Вставить("Имя", "П000220024303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 243 Раздел 2.2';
												|en = 'Строка 243 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр250" Тогда
		Результат.Вставить("Имя", "П000220025003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 250 Раздел 2.2';
												|en = 'Строка 250 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр251" Тогда
		Результат.Вставить("Имя", "П000220025103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 251 Раздел 2.2';
												|en = 'Строка 251 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр252" Тогда
		Результат.Вставить("Имя", "П000220025203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 252 Раздел 2.2';
												|en = 'Строка 252 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр253" Тогда
		Результат.Вставить("Имя", "П000220025303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 253 Раздел 2.2';
												|en = 'Строка 253 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр260" Тогда
		Результат.Вставить("Имя", "П000220026003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 260 Раздел 2.2';
												|en = 'Строка 260 Раздел 2.2'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр261" Тогда
		Результат.Вставить("Имя", "П000220026103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 261 Раздел 2.2';
												|en = 'Строка 261 Раздел 2.2'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр262" Тогда
		Результат.Вставить("Имя", "П000220026203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 262 Раздел 2.2';
												|en = 'Строка 262 Раздел 2.2'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр263" Тогда
		Результат.Вставить("Имя", "П000220026303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 263 Раздел 2.2';
												|en = 'Строка 263 Раздел 2.2'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр270" Тогда
		Результат.Вставить("Имя", "П000220027003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 270 Раздел 2.2';
												|en = 'Строка 270 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр271" Тогда
		Результат.Вставить("Имя", "П000220027103");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 271 Раздел 2.2';
												|en = 'Строка 271 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр272" Тогда
		Результат.Вставить("Имя", "П000220027203");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 272 Раздел 2.2';
												|en = 'Строка 272 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр273" Тогда
		Результат.Вставить("Имя", "П000220027303");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 273 Раздел 2.2';
												|en = 'Строка 273 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2_2_Стр280" Тогда
		Результат.Вставить("Имя", "П000220028003");
		Результат.Вставить("Представление", НСтр("ru = 'Строка 280 Раздел 2.2';
												|en = 'Строка 280 Раздел 2.2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	// Раздел 3
	//
	ИначеЕсли УсловныйКодПоказателя = "Р3_Гр3" Тогда
		Результат.Вставить("Имя", "П000030000103_%1");
		Результат.Вставить("Представление", НСтр("ru = 'Графа 3 Раздел 3';
												|en = 'Графа 3 Раздел 3'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р3_Гр4" Тогда
		Результат.Вставить("Имя", "П000030000104_%1");
		Результат.Вставить("Представление", НСтр("ru = 'Графа 4 Раздел 3';
												|en = 'Графа 4 Раздел 3'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р3_Гр6" Тогда
		Результат.Вставить("Имя", "П000030000106_%1");
		Результат.Вставить("Представление", НСтр("ru = 'Графа 6 Раздел 3';
												|en = 'Графа 6 Раздел 3'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р3_Гр7" Тогда
		Результат.Вставить("Имя", "П000030000107_%1");
		Результат.Вставить("Представление", НСтр("ru = 'Графа 7 Раздел 3';
												|en = 'Графа 7 Раздел 3'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
...
Процедура ВставитьЗначениеПоказателя(ИсточникДанных, Показатель)
	...
	Если Показатель.Свойство("ИзменяемаяРазмерность") И Показатель.ИзменяемаяРазмерность = Истина Тогда
		ЗначениеПоказателя = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, ЗначениеПоказателя);
	КонецЕсли;
...
Процедура ДобавитьЭлементВСтруктуруОшибок(СтруктураОшибок, ТекстФормулы, ОписаниеКС, ОбщееЧислоОшибок, ОбщееЧислоБезОшибок, Расшифровка,
	...
	СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																																								|en = '%1'"), Описание));
	СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																																								|en = '%1'"), Нарушение));
	СтруктураОшибок.Вставить("Расшифровка" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																																								|en = '%1'"), Расшифровка));
	СтруктураОшибок.Вставить("ЭтоОшибка" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), ЭтоОшибка);
	СтруктураОшибок.Вставить("ТекстовоеПредставлениеФормулы" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																																													|en = '%1'"), ТекстовоеПредставлениеФормулы));
	СтруктураОшибок.Вставить("Комментарий" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), Коммент);
...
Функция ЧислоСтрЧГ0(ПреобразуемоеЧисло)
	...
	Возврат Формат(ПреобразуемоеЧисло, "ЧН=; ЧГ=");
...
Функция ПоказателиКС(ЛеваяЧасть, ПраваяЧасть, РазмерЛев = Истина, РазмерПрав = Истина)
	...
	Если РазмерЛев Тогда 
		ЛеваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, ЛеваяЧасть);
	КонецЕсли;
	
	Если РазмерПрав Тогда 
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, ПраваяЧасть);
	КонецЕсли;
