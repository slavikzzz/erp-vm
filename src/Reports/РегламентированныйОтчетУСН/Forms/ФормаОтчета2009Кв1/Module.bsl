...
Процедура ЗаполнитьСведенияОбОрганизации(ИзмененаДатаПодписи = Ложь)
	...
	ОбъектНалогообложенияУСН = ПолучитьОбъектНалогообложенияУСН(ЭтотОбъект);
	...
	ИначеЕсли ОбъектНалогообложенияУСН = 2 Тогда
		...
		Раздел2.Вставить("П000020020103", ПолучитьСтавкуНалогаУСН(ЭтотОбъект));
	...
	ИначеЕсли ОбъектНалогообложенияУСН = 0 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Для налогоплательщика не выбран специальный налоговый режим.';
								|en = 'Для налогоплательщика не выбран специальный налоговый режим.'");
				
		Сообщение.Сообщить();
		
	КонецЕсли;
		
	ИзменитьФорматПоказателей(ЭтотОбъект, СтруктураРеквизитовФормы.НаимТекущегоРаздела);
...
Процедура ЗаполнитьСведенияОПодписантах() Экспорт
	...
	КодНО = РегламентированнаяОтчетностьКлиентСервер.ВернутьЗначениеПараметраСЛиста(ЭтотОбъект, "Титульный", "КодИМНС");
	КПП   = РегламентированнаяОтчетностьКлиентСервер.ВернутьЗначениеПараметраСЛиста(ЭтотОбъект, "Титульный", "КПП1");
...
Процедура ИзменитьФорматПоказателейНаСервере(ИмяРаздела)
	...
	ИзменитьФорматПоказателей(ЭтотОбъект, ИмяРаздела);
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если ИмяПоказателя = "П000020028003" Тогда
		...
		Если (НЕ СтруктураРеквизитовФормы.ИП ИЛИ (СтруктураРеквизитовФормы.ИП И СтруктураРеквизитовФормы.ИПИспользуетТрудНаемныхРаботников))
		   И РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, ЗначПоказателя) > РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, П000020026003) * 0.5  Тогда
			...
			ТекстВопроса = НСтр("ru = 'Сумма уплаченных за налоговый (отчетный) период страховых
					...
					|en = 'Сумма уплаченных за налоговый (отчетный) период страховых
					|взносов на обязательное пенсионное страхование, а также
					|выплаченных работникам в течение налогового (отчетного)
					|периода из средств налогоплательщика пособий по временной
					|нетрудоспособности, уменьшает сумму исчисленного налога
...
Процедура ВопросСуммаУплаченныхНалоговУменьшаетЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если  РезультатВопроса = КодВозвратаДиалога.Да Тогда
		Область.Значение = Цел(РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, П000020026003) * 0.5);
	КонецЕсли;
	Если ЗначПоказателя = "1" Тогда
		П000020026003 = ТабличныйДокумент.Области.П000020026003.Значение;
	ИначеЕсли ЗначПоказателя = "2" Тогда		
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокументаЗавершение(ЗначПоказателя, ИмяПоказателя, ИмяРаздела, Область)
	...
	СвойстваПоказателя = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, ИмяПоказателя);
	...
	Если СвойстваПоказателя <> Неопределено Тогда
		...
		Для каждого ЗависимЗнач Из ТЗЗависимЗнач Цикл
			СвойстваЗависимогоПоказателя = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, ЗависимЗнач.ОбластьПоказателяИмя);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ИННОрг) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан ИНН!", "Титульный", "ИНН1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ПериодОтч) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На титульном листе не задан код отчетного периода!", "Титульный", "Период1");
		СтатусПроверки = Ложь;
	ИначеЕсли ВозможныеКодыПериода.Найти(ПараметрыВыгрузки.ПериодОтч) = Неопределено Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На титульном листе задан некорректный код отчетного периода!", "Титульный", "Период1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если ПустаяСтрока(ПараметрыВыгрузки.НаимОрг) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указан налогоплательщик!", "Титульный", "НаимОрг");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ПоМесту) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На титульном листе не задан код места нахождения (учета)!", "Титульный", "ПоМесту");
		СтатусПроверки = Ложь;
	ИначеЕсли ВозможныеКодыПоМесту.Найти(ПараметрыВыгрузки.ПоМесту) = Неопределено Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На титульном листе задан некорректный код места нахождения (учета)!", "Титульный", "ПоМесту");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ОбНал <> "1" И ОбНал <> "2" Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "В разделе 1 не" + ?(НЕ ЗначениеЗаполнено(ОбНал), "", "корректно") + " задан код объекта налогообложения!", "Раздел1", "П000010000103");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ДлинаОКАТОРаздел1 <> 11 И ДлинаОКАТОРаздел1 <> 8 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "В разделе 1 не" + ?(ДлинаОКАТОРаздел1 = 0, "", "корректно") + " задан код по ОКТМО(ОКАТО)!", "Раздел1", "П000010001003");
		СтатусПроверки = Ложь;
	ИначеЕсли НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ОКАТОРаздел1) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "В разделе 1 некорректно задан код по ОКТМО(ОКАТО)!", "Раздел1", "П000010001003");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ДлинаКБКРаздел1 <> 20 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "В разделе 1 не" + ?(ДлинаКБКРаздел1= 0, "", "корректно") + " задан код бюджетной классификации!", "Раздел1", "П000010002003");
		СтатусПроверки = Ложь;
	ИначеЕсли НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КБКРаздел1) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "В разделе 1 некорректно задан код бюджетной классификации!", "Раздел1", "П000010002003");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ОбНал = "2" И ЗначениеСуммыЗаполнено(Раздел1.П000010009003) Тогда
		...
		Если ДлинаКБКМинРаздел1 <> 20 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "В разделе 1 не" + ?(ДлинаКБКМинРаздел1= 0, "", "корректно") + " задан код бюджетной классификации по строке 80!", "Раздел1", "П000010008003");
			СтатусПроверки = Ложь;
		ИначеЕсли НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КБКМинРаздел1) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "В разделе 1 некорректно задан код бюджетной классификации по строке 80!", "Раздел1", "П000010008003");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверкаКС_1_0(СтруктураРезультата)
	...
	ЛеваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела1[ИмяР1Ст060]);
	...
	ЗначениеР2Ст260 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст260]);
	...
	ЗначениеР2Ст280 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст280]);
	...
	ЗначениеР1Ст050 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела1[ИмяР1Ст050]);
...
Процедура ПроверкаКС_1_2(СтруктураРезультата)
	...
	ЗначениеР2Ст270 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст270]);
...
Процедура ПроверкаКС_1_4(СтруктураРезультата)
	...
	ЛеваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела1[ИмяР1Ст070]);
	ЛеваяЧастьСтр = Формат(ЛеваяЧасть, "ЧРД=.;ЧН=0");
...
Процедура ПроверкаКС_1_9(СтруктураРезультата)
	...
	ЛеваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела1[ИмяР1Ст090]);
	ЛеваяЧастьСтр = Формат(ЛеваяЧасть, "ЧРД=.;ЧН=0");
...
Процедура ПроверкаКС_1_10(СтруктураРезультата)
	...
	ЛеваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст201]);
	ЛеваяЧастьСтр = Формат(ЛеваяЧасть, "ЧРД=.;ЧН=0");
...
Процедура ПроверкаКС_1_12(СтруктураРезультата)
	...
	ЛеваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст220]);
	ЛеваяЧастьСтр = Формат(ЛеваяЧасть, "ЧРД=.;ЧН=0");
...
Процедура ПроверкаКС_1_13(СтруктураРезультата)
	...
	ЛеваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст230]);
	ЛеваяЧастьСтр = Формат(ЛеваяЧасть, "ЧРД=.;ЧН=0");
...
Процедура ПроверкаКС_1_14(СтруктураРезультата)
	...
	ЛеваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст240]);
	ЛеваяЧастьСтр = Формат(ЛеваяЧасть, "ЧРД=.;ЧН=0");
	...
	ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст210]);
	ПраваяЧастьСтр = Формат(ПраваяЧасть, "ЧРД=.;ЧН=0");
...
Процедура ПроверкаКС_1_15(СтруктураРезультата)
	...
	ИмяР2Ст210 = "П000020021003";
	ЗначениеР2Ст210 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст210]);
	...
	ИмяР2Ст220 = "П000020022003";
	ЗначениеР2Ст220 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст220]);
	...
	ИмяР2Ст230 = "П000020023003";
	ЗначениеР2Ст230 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст230]);
...
Процедура ПроверкаКС_1_16(СтруктураРезультата)
	...
	ЛеваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст250]);
	ЛеваяЧастьСтр = Формат(ЛеваяЧасть, "ЧРД=.;ЧН=0");
...
Процедура ПроверкаКС_1_18(СтруктураРезультата)
	...
	// Зарегистрируем левую часть соотношения.
	ИмяР2Ст260 = "П000020026003";
	ЛеваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст260]);
	ЛеваяЧастьСтр = Формат(ЛеваяЧасть, "ЧРД=.;ЧН=0");
	НаименованиеР2Ст260 = "Сумма исчисленного налога за налоговый период";
	...
	// Зарегистрируем правую часть соотношения.
	ИмяР2Ст240 = "П000020024003";
	ЗначениеР2Ст240 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст240]);
	...
	ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, ЗначениеР2Ст240 * ЗначениеР2Ст201 / 100);
	ПраваяЧастьСтр = Формат(ПраваяЧасть, "ЧРД=.;ЧН=0");
...
Процедура ПроверкаКС_1_19(СтруктураРезультата)
	...
	// Зарегистрируем левую часть соотношения.
	ИмяР2Ст270 = "П000020027003";
	ЛеваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2[ИмяР2Ст270]);
	ЛеваяЧастьСтр = Формат(ЛеваяЧасть, "ЧРД=.;ЧН=0");
	НаименованиеР2Ст270 = "Сумма исчисленного минимального налога за налоговый период (ставка налога 1%)";
...
Процедура ПроверкаКС_1_20(СтруктураРезультата)
	...
	ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, ЗначениеР2Ст210 / 100);
	ПраваяЧастьСтр = Формат(ПраваяЧасть, "ЧРД=.;ЧН=0");
...
Процедура ПроверкаКС_4_3_3(СтруктураРезультата)
	...
	ЗначениеР2Стр230 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, СтруктураРаздела2.П000020023003);
	...
	П130 = РегламентированнаяОтчетностьПереопределяемый.СуммаФактическогоУменьшенияНалоговойБазы(СтруктураРеквизитовФормы.мСохраненныйДок.Организация,
		СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, П130);
	ПраваяЧастьСтр = Формат(ПраваяЧасть, "ЧРД=.;ЧН=0");
