...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "07/03/2018";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы["ИмяМакетаСоставаПоказателей"] = "СоставПоказателей2018Кв1";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		Иначе
			ПолеТабличногоДокументаФормаОтчета.Области["П017403"].Значение = 1;
			ПолеТабличногоДокументаФормаОтчета.Области["П017404"].Значение = 2;
		КонецЕсли;
	...
	Если СтруктураРеквизитовФормы.ОтображатьКнопкуЗаполнить Тогда
		...
		Если НЕ РегламентированнаяОтчетностьПереопределяемый.РеализованаНастройкаАвтозаполняемыхПоказателейРеглОтчета(
			ИДОтчета, ИДРедакцииОтчета) Тогда
...
Процедура ОчиститьТабличноеПоле(Форма)
	...
	ВыбТабличноеПоле.Области["П017403"].Значение = 1;
	ВыбТабличноеПоле.Области["П017404"].Значение = 2;
КонецПроцедуры
...
Процедура Расчет(ПолеТабличногоДокументаФормаОтчета) 
	...
	Области["П012703"].Значение = Области["П010103"].Значение + Области["П010503"].Значение
		+ Области["П010603"].Значение + Области["П011503"].Значение
		+ Области["П011603"].Значение + Области["П011803"].Значение
		+ Области["П011903"].Значение + Области["П012003"].Значение
		+ Области["П012103"].Значение + Области["П012203"].Значение
		+ Области["П012303"].Значение + Области["П012403"].Значение
		+ Области["П012503"].Значение + Области["П012603"].Значение;
		
	Области["П013403"].Значение = Области["П013503"].Значение + Области["П013603"].Значение;
	
	Области["П013803"].Значение = Области["П013903"].Значение + Области["П014003"].Значение;
	
	Области["П012803"].Значение = Области["П012903"].Значение + Области["П013403"].Значение
		+ Области["П013703"].Значение + Области["П013803"].Значение
		+ Области["П014103"].Значение + Области["П014703"].Значение
		+ Области["П014803"].Значение + Области["П014903"].Значение
		+ Области["П015003"].Значение + Области["П015103"].Значение;
		
	Области["П017103"].Значение = Области["П012803"].Значение + Области["П016403"].Значение
		+ Области["П016603"].Значение + Области["П016803"].Значение
		+ Области["П016903"].Значение + Области["П017003"].Значение;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.Имя = "П017404" Или Область.Имя = "П017403" Тогда
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("Стр7403", ?(ЗначениеЗаполнено(ТабДокумент.П017403) И ТабДокумент.П017403 < 0, 1, 0));
	ПараметрыВыгрузки.Вставить("Стр7404", ?(ЗначениеЗаполнено(ТабДокумент.П017403) И ТабДокумент.П017404 < 0, 1, 0));
...
Функция ЗаполнитьАвтоНаСервере()
	...
	ПараметрыОтчета.Вставить("мДатаКонцаПериодаОтчета",
							 СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	ПараметрыОтчета.Вставить("ДатаНачалаПериодаОтчета",
							 НачалоКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
	ПараметрыОтчета.Вставить("ДатаКонцаПериодаОтчета",
							 СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	ПараметрыОтчета.Вставить("Периодичность",
							 СтруктураРеквизитовФормы.мПериодичность);
	ПараметрыОтчета.Вставить("ОбособленноеПодразделение",
	...
	РезультатВыполнения = ДлительныеОперации.ВыполнитьВФоне(
		"РегламентированнаяОтчетностьПереопределяемый.ЗаполнитьОтчетВФоне", Новый Структура(
		"ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета, Контейнер", ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета, Контейнер),
	...
	Иначе
		...
		Для Каждого Строка Из ТаблицаВариантыЗаполнения Цикл
			...
			КолвоСтрокВМногострочнойЧасти =
				РегламентированнаяОтчетностьКлиентСервер.ПоказательВходитВМногострочнуюЧасть(Форма,
				ИмяПоказателя);
			...
			Если КолвоСтрокВМногострочнойЧасти > 0 Тогда
				...
				ОписанияБлоков =
					РегламентированнаяОтчетностьКлиентСервер.ПолучитьСвойствоРаздела(Форма,
