...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КолКолонокИдГруппы11 = 11;
	КолКолонокИдГруппы12 = 11;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010000104", "П000010000105;П000010000106;П000010000107");
	
	// Многострочная часть 2
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010000302", "П000010000301;П000010000303");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010000304", "П000010000305;П000010000306;П000010000307");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010000109", 3, , "Выбор вида продукции", КоллекцияСписковВыбора["ВидыПродукции"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010000108", 3, , "Выбор вида продукции", КоллекцияСписковВыбора["ВидыПродукции"]);
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Декларация" Тогда
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Для Инд = 1 По КоличествоСтрок Цикл
					...
					Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000010000104, ОбластьИмя) > 0 Тогда   

						Графа06 = Форма.ТабличныйДокумент.Области["П0000100001" + "05" + ПостфиксСтроки].Значение;
						Графа07 = Форма.ТабличныйДокумент.Области["П0000100001" + "06" + ПостфиксСтроки].Значение;
						Графа08 = Форма.ТабличныйДокумент.Области["П0000100001" + "07" + ПостфиксСтроки].Значение;						
						
						ЗначениеГрафы05 = Графа06 + Графа07 + Графа08;
						
						Форма.ТабличныйДокумент.Области["П0000100001" + "04" + ПостфиксСтроки].Значение = ЗначениеГрафы05;
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Для Инд = 1 По КоличествоСтрок Цикл
					...
					Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000010000304, ОбластьИмя) > 0 Тогда   

						Графа06 = Форма.ТабличныйДокумент.Области["П0000100003" + "05" + ПостфиксСтроки].Значение;
						Графа07 = Форма.ТабличныйДокумент.Области["П0000100003" + "06" + ПостфиксСтроки].Значение;
						Графа08 = Форма.ТабличныйДокумент.Области["П0000100003" + "07" + ПостфиксСтроки].Значение;						
						
						ЗначениеГрафы05 = Графа06 + Графа07 + Графа08;
						
						Форма.ТабличныйДокумент.Области["П0000100003" + "04" + ПостфиксСтроки].Значение = ЗначениеГрафы05;
	...
	Для Каждого Страница Из ТаблицаРаздела Цикл
		...
		Если ТаблицаБлока <> Неопределено Тогда
			...
			Для Каждого ТекСтрока из ТаблицаБлока Цикл
				...
				// Графа 05
				Графа06 = ТекСтрока["П0000100003" + "05"];
				Графа07 = ТекСтрока["П0000100003" + "06"];
				Графа08 = ТекСтрока["П0000100003" + "07"];						
				
				ЗначениеГрафы05 = Графа06 + Графа07 + Графа08;
				
				ТекСтрока["П0000100003" + "04"] = ЗначениеГрафы05;
		...
		Если ТаблицаБлока <> Неопределено Тогда
			...
			Для Каждого ТекСтрока из ТаблицаБлока Цикл
				...
				// Графа 05
				Графа06 = ТекСтрока["П0000100001" + "05"];
				Графа07 = ТекСтрока["П0000100001" + "06"];
				Графа08 = ТекСтрока["П0000100001" + "07"];						
				
				ЗначениеГрафы05 = Графа06 + Графа07 + Графа08;
				
				ТекСтрока["П0000100001" + "04"] = ЗначениеГрафы05;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если СтрНайти(КодПоказателя, "П000010000109") = 1 ИЛИ СтрНайти(КодПоказателя, "П000010000309") = 1 Тогда
		КолонкаПоиска = "Название";
	КонецЕсли;
	
	Если СтрНайти(КодПоказателя, "П000010000108") = 1 ИЛИ СтрНайти(КодПоказателя, "П000010000308") = 1 Тогда
		КолонкаПоиска = "Код";
	КонецЕсли;
...
Процедура ЗаполнитьСвязанныеПоказатели(ИмяОбласти, РезультатВыбора)
	...
	Если СтрНайти(ИмяОбласти, "П000010000109") = 1 Тогда
		ИмяСвязаннойОбласти = "П000010000108" + Сред(ИмяОбласти, 14);
	...
	ИначеЕсли СтрНайти(ИмяОбласти, "П000010000108") = 1 Тогда
		ИмяСвязаннойОбласти = "П000010000109" + Сред(ИмяОбласти, 14);
		СвязаннаяОбласть = ТабличныйДокумент.Области[ИмяСвязаннойОбласти];
		СвязаннаяОбласть.Значение = ?(ПустаяСтрока(РезультатВыбора.Код), "", РезультатВыбора.Название);
		
	ИначеЕсли СтрНайти(ИмяОбласти, "П000010000309") = 1 Тогда
		ИмяСвязаннойОбласти = "П000010000308" + Сред(ИмяОбласти, 14);
		СвязаннаяОбласть = ТабличныйДокумент.Области[ИмяСвязаннойОбласти];
		СвязаннаяОбласть.Значение = РезультатВыбора.Код;
		Если ПустаяСтрока(РезультатВыбора.Код) Тогда
			ТабличныйДокумент.Области[ИмяОбласти].Значение = "";
	...
	ИначеЕсли СтрНайти(ИмяОбласти, "П000010000308") = 1 Тогда
		ИмяСвязаннойОбласти = "П000010000309" + Сред(ИмяОбласти, 14);
		СвязаннаяОбласть = ТабличныйДокумент.Области[ИмяСвязаннойОбласти];
		СвязаннаяОбласть.Значение = ?(ПустаяСтрока(РезультатВыбора.Код), "", РезультатВыбора.Название);
