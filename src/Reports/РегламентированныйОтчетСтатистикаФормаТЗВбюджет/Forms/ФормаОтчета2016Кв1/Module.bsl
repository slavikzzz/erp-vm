...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "21/09/2016";
	СтруктураРеквизитовФормы.мКодОтчета   = "1601371";
	...
	СтруктураРеквизитовФормы.мИдГруппы8 = "П1600000000";
...
Процедура СформироватьСпискиВыбора()
	...
	Таблицы.Добавить("pril_tri_1600");
	Для Каждого Элт Из Таблицы Цикл 
		...
		РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, Элт,   512, , "Выбор кода строки " + Сред(Элт, СтрДлина("pril_tri__")),   ТаблСписка);
	КонецЦикла;
КонецПроцедуры
...
Процедура ПеренумероватьСтроки(Форма)
	...
	Области.П160000000002_1.Значение = "1600";
КонецПроцедуры
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Дес = 1 По 6 Цикл 
		...
		Области["П0000" + Дес + "0004"].Значение = Области["П0000" + Дес + "5004"].Значение + Сумм;
		Итого2000 = Итого2000 + Области["П0000" + Дес + "0004"].Значение;
	КонецЦикла;
	
	Области["П000070004"].Значение = Области["П000070104"].Значение + Области["П000070404"].Значение + Области["П000075004"].Значение;
	Итого2000 = Итого2000 + Области["П000070004"].Значение;
	
	Сумм = 0;
	Для Стр = 1 По 74 Цикл 
		Обл = Области.Найти("П00008" + Формат(Стр, "ЧЦ=2; ЧВН=") + "04");
		Если Обл <> Неопределено Тогда 
			Сумм = Сумм + Обл.Значение;
		КонецЕсли;
	КонецЦикла;
	
	Области["П000080004"].Значение = Области["П000150004"].Значение + Сумм;
	Области["П000200004"].Значение = Итого2000 + Области["П000080004"].Значение;
	...
	ИначеЕсли Не СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		...
		ИначеЕсли СтрНачинаетсяС(ОблИмя, "П066000000001_") Или СтрНачинаетсяС(ОблИмя, "П066000000003_") Тогда 
			...
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				...
				ВвестиПоказатель(Элемент, ОбластьДляВвода, СмежнаяОбласть, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ПараметрВводаПоля.ТаблицаЗначений, СтандартнаяОбработка);
			КонецЕсли;
		ИначеЕсли СтрНачинаетсяС(ОблИмя, "П160000000001_") Или СтрНачинаетсяС(ОблИмя, "П160000000003_") Тогда 
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "pril_tri_1600");
			
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				СмежнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(ОблИмя, "П160000000003_", "П160000000001_")];
				ОбластьДляВвода = ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(ОблИмя, "П160000000001_", "П160000000003_")];
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОблНачало"));
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОблЗаголовок"));
	
	ОблЗаголовок = ТабДок.ПолучитьОбласть("ОблЗаголовок");
	
	Низ = ТабДок.Область("Подвал").Верх;
	ТекСтр = ТабДок.Область("ОблЗаголовок").Низ;
	
	Пока ТекСтр < Низ - 1 Цикл 
		ТекСтр = ТекСтр + 1;
		Если СтрНачинаетсяС(ТабДок.Область(ТекСтр, , ТекСтр).Имя, "ДобавлениеСтроки") Тогда 
			Продолжить;
		КонецЕсли;
		
		Кусок = ТекСтр;
		Пока Кусок < Низ Цикл
			Если СтрНачинаетсяС(ТабДок.Область(Кусок, , Кусок).Имя, "ДобавлениеСтроки") Тогда 
				Прервать;
			КонецЕсли;
			
			Если ТабДок.Область(Кусок, 7, Кусок, 7).ГраницаСнизу.ТипЛинии = ТипЛинииЯчейкиТабличногоДокумента.Сплошная Тогда 
				Прервать;
			КонецЕсли;
		...
		Обл = ТабДок.ПолучитьОбласть(ТекСтр, , Кусок);
		Если Не ПечатнаяФорма.ПроверитьВывод(Обл) Тогда 
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма ТЗВ-бюджет");
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОблЗаголовок"));
		КонецЕсли;
		...
		ПечатнаяФорма.Вывести(Обл);
	КонецЦикла;
	
	Обл = ТабДок.ПолучитьОбласть("Подвал");
	Если Не ПечатнаяФорма.ПроверитьВывод(Обл) Тогда 
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма ТЗВ-бюджет");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОблЗаголовок"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.Аддр) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не указан почтовый адрес!", ИмяПоляТаблДокумента, "ОргАдрес");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("Аддр", РегламентированнаяОтчетностьКлиентСервер.ПредставлениеАдресаВФормате9Запятых(СтруктураРеквизитовФормы.мАдресвФорматеДляВыгрузки));
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Для Инд = 1 По 8 Цикл 
		Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок" + Инд,
					РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтаФорма, СтруктураРеквизитовФормы["мИдГруппы" + Инд], Ложь));
	КонецЦикла;
...
Функция РазделТекущейОбласти(Форма, ТекущаяОбласть)
	...
	Если СтрНачинаетсяС(ИмяТекущейОбласти, "ДобавитьСтроку") Тогда 
		Возврат Форма.СтруктураРеквизитовФормы["мИдГруппы" + Прав(ИмяТекущейОбласти, 1)];
	КонецЕсли;
