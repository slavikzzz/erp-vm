...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	//Для совместимости состава показателей(ни одного варианта заполнения отличного от нуля) формы 5-З с общими механизмами.
	мСтруктураВариантыЗаполнения.Вставить("ПолеТабличногоДокументаФормаОтчета", "ПолеТабличногоДокументаФормаОтчета");
...
Процедура ПоказатьПериод()
	...
	СтруктураРеквизитовФормы.СтрПериодОтчета
		= Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ=к")
		+ " квартал " + Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг'") + " г.";
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКВЭД) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			"Не задан код ОКВЭД2 организации!", ИмяПоляТаблДокумента, "ОргКодОКПО");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьДаннымиОтчетовНаСервере(ТекстОписанияПроблемы)
	ЗаполнениеСтатистическойОтчетности.ЗаполнитьДаннымиПредидущегоПериода(
		ЭтотОбъект, "МакетПереносаДанных2016Кв1", ТекстОписанияПроблемы);
КонецПроцедуры
