...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ПрекращениеСтраховыеВзносы;
...
&НаСервере
Функция СнилсВСтрокуФормы(СНИЛС)
	Если СтрДлина(СНИЛС) = 11 И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СНИЛС) Тогда 
		Возврат Лев(СНИЛС, 3) + "-" + Сред(СНИЛС, 4, 3) + "-" + Сред(СНИЛС, 7, 3) + "-" + Прав(СНИЛС, 2);
	ИначеЕсли СтрДлина(СНИЛС) = 14 
		И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Лев(СНИЛС, 3))
		И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Сред(СНИЛС, 5, 3))
		И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Сред(СНИЛС, 9, 3))
		И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Прав(СНИЛС, 2)) Тогда 
		
		Возврат Лев(СНИЛС, 3) + "-" + Сред(СНИЛС, 5, 3) + "-" + Сред(СНИЛС, 9, 3) + "-" + Прав(СНИЛС, 2);
	Иначе
		Возврат "";
	КонецЕсли;
КонецФункции
...
Процедура ЗаполнитьНачальныеДанные() Экспорт
	...
	СтрокаСведений = "ИННФЛ,ФИО,ТелДом,ФамилияИП,ИмяИП,ОтчествоИП,СНИЛС";
	...
	ДанныеУведомленияТитульный.Вставить("СНИЛС", СнилсВСтрокуФормы(СведенияОбОрганизации.СНИЛС));
