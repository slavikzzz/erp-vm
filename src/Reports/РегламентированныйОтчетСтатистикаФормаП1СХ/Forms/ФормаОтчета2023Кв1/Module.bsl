...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0000401860";
	СтруктураРеквизитовФормы.мИдГруппы2 = "П0000401870";
	СтруктураРеквизитовФормы.мИдГруппы3 = "П0000401880";
...
Процедура ПоказатьПериод()
	...
	СтрПериод2 = Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг'");
	СтрПериод3 = Формат(КонецГода(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) + 1, "ДФ = 'гггг'");
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета2.Значение = СтрПериод2;
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета3.Значение = СтрПериод3;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 Тогда
		УдалитьСтроку("");
	Иначе
		НомСтр = Сред(Область.Имя, 9, 2);
		НомКол = Сред(Область.Имя, 13, 1);
		Если Лев(Область.Имя, 6) = "П00004" И (НомКол = "1" Или НомКол = "4") Тогда
			Если НомСтр = "86" Или НомСтр = "87" Или НомСтр = "88" Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
					ЭтотОбъект, "СправочникТерриторииВывозаРФ");
			ИначеЕсли НомСтр = "89" Или НомСтр = "90" Или НомСтр = "91" Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
					ЭтотОбъект, "СправочникТерриторииВывозаВнеРФ");
			КонецЕсли;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	ИсходноеЗначение = ПолеТабличногоДокументаФормаОтчета.Области[СмежнаяОбласть].Значение;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Сред(Область.Имя, 13, 1) = "1" Тогда
		Область.Значение = РезультатНазвание;
		СмежнаяОбласть = СтрЗаменить(Область.Имя, Лев(Область.Имя, 13), Лев(Область.Имя, 11) + "04");
		ПолеТабличногоДокументаФормаОтчета.Области[СмежнаяОбласть].Значение = РезультатКод;
	Иначе
		Область.Значение = РезультатКод;
		СмежнаяОбласть = СтрЗаменить(Область.Имя, Лев(Область.Имя, 13), Лев(Область.Имя, 11) + "01");
		ПолеТабличногоДокументаФормаОтчета.Области[СмежнаяОбласть].Значение = РезультатНазвание;
	КонецЕсли;
...
Процедура ПолеТабличногоДокументаФормаОтчетаПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	ДополнительныеПараметры = Новый Структура("Область, Элемент, Форма, ВосстанавливатьЭталонФормата", Область, Элемент, ЭтотОбъект);
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 5 По 7 Цикл
		...
		СуммаПоСтр = 0;
		Для Стр = 16 По 20 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101015" + Графа].Значение = СуммаПоСтр;
	КонецЦикла;
	
	Для Стр = 86 По 88 Цикл
		СуммаПоСтр = 0;
		ИндЧ = 0;
		Пока Истина Цикл
			ИндЧ = ИндЧ + 1;
			Инд = Формат(ИндЧ, "ЧГ=");
			Обл = Области.Найти("П0000401" + Стр + "005_" + Инд);
			Если Обл = Неопределено Тогда
				Прервать;
			КонецЕсли;
			СуммаПоСтр = СуммаПоСтр + Обл.Значение;
		КонецЦикла;
		Области["П00004010" + Стр + "05"].Значение = СуммаПоСтр;
	КонецЦикла;
	
	Области["П000040108905"].Значение = Области["П000040189105"].Значение + Области["П000040189205"].Значение;
	Области["П000040109005"].Значение = Области["П000040190105"].Значение + Области["П000040190205"].Значение;
	Области["П000040109105"].Значение = Области["П000040191105"].Значение + Области["П000040191205"].Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 5 По 7 Цикл
		мФормаОтчета.Области["П00001010150" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00001010150" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Стр = 86 По 91 Цикл
		мФормаОтчета.Области["П00004010" + Стр + "05"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00004010" + Стр + "05"].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	П0000401860.Очистить();
	П0000401870.Очистить();
	П0000401880.Очистить();
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-1 (СХ)");
	...
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть2_2_3"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-1 (СХ)");
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть4_4")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-1 (СХ)");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ПустаяСтрока"));
	КонецЕсли;
	...
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть4_4.Низ + 1;
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть4_5")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-1 (СХ)");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ПустаяСтрока"));
	КонецЕсли;
	
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть4_5"));
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть4_5.Низ + 1;
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверки.Добавить(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть4_6"));
		Если ПечатнаяФорма.ПроверитьВывод(ТекущаяОбластьМногострочнойЧасти) Тогда
			ПечатнаяФорма.Вывести(ТекущаяОбластьМногострочнойЧасти);
		Иначе
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-1 (СХ)");
	...
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть4_6"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-1 (СХ)");
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть4_7"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-1 (СХ)");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникТерриторииВывозаРФ",
		512, , "Выбор территории вывоза", КоллекцияСписковВыбора["ТерриторииВывозаРФ"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникТерриторииВывозаВнеРФ",
		512, , "Выбор территории вывоза", КоллекцияСписковВыбора["ТерриторииВывозаВнеРФ"]);
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Для Стр = 89 По 91 Цикл 
		Для Стр2 = 1 По 2 Цикл 
			Для Гр = 4 По 5 Цикл 
				ПараметрыВыгрузки.Вставить("ПД000401" + Стр + "00" + Гр + "_" + Стр2, 
				ПолеТабличногоДокументаФормаОтчета.Области["П0000401" + Стр + Стр2 + "0" + Гр].Значение);
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
