...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "30/08/2016";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	СтруктураДанныхРаздел1_2.П000003000003 = СокрЛП(Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ=гггг"));
	СтруктураДанныхРаздел1_2.П000003000004 = СокрЛП(Формат(КонецГода(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) + 1, "ДФ=гггг"));
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = "Раздел IV(1,2)";
	СтрокаУровня1.ОриентацияСтраницы       = "Ландшафт";
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = 1;
	СтрокаУровня1.ВыгрузитьСтраницу        = 2;
	...
	СтрокаУровня1.Представление            = "Раздел IV(3)";
	СтрокаУровня1.ОриентацияСтраницы       = "Ландшафт";
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = 1;
	СтрокаУровня1.ВыгрузитьСтраницу        = 2;
...
Процедура ОчиститьТабличноеПоле(Форма, ВыбТабличноеПолеИмя) Экспорт
	...
	Если ВыбТабличноеПолеИмя = "Раздел3_5" Тогда
		Форма.УдалитьНеобходимоеКоличествоДопСтрокНаСервере(ВыбТабличноеПоле, ВыбТабличноеПолеИмя, Форма.СтруктураРеквизитовФормы.мИдГруппы1);
		Форма.УдалитьНеобходимоеКоличествоДопСтрокНаСервере(ВыбТабличноеПоле, ВыбТабличноеПолеИмя, Форма.СтруктураРеквизитовФормы.мИдГруппы2);
		Форма["ТабличныйДокумент"] = ВыбТабличноеПоле;
	ИначеЕсли ВыбТабличноеПолеИмя = "Раздел5" Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	Строка8  = Новый ОписаниеТипов(Массив, , Новый КвалификаторыСтроки(11, ДопустимаяДлина.Переменная));
...
#КонецЕсли
	...
	Если (Неопределено <> Форма.ТабличныйДокумент.Области.Найти("ОргКодОКПО"))
		...
		Если ЗначениеЗаполнено(Форма.ОбособленноеПодразделение) Тогда
			Форма.ТабличныйДокумент.Области.ОргНазв.Значение = СокрЛП(Форма.ОбособленноеПодразделение) + ?(ПустаяСтрока(НаимЮЛПол), "", " (" + НаимЮЛПол + ")");
		Иначе
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	Если ИмяТекРаздела = "Раздел3_5" Или ИмяТекРаздела = "Раздел5" Тогда
		...
		Иначе
			...
			СтруктураГруппы = ЭтаФорма[СтруктураГруппы];
			ВывестиРазделВТабличныйДокументНаСервере(СтруктураРеквизитовФормы.мИдГруппы1, СтруктураГруппы);
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Раздел1_2", "ФормаОтчета2019Кв1_Раздел1_2");
	СоответствиеРазделовМакетам.Вставить("Раздел3",   "ФормаОтчета2019Кв1_Раздел3");
	СоответствиеРазделовМакетам.Вставить("Раздел3_5", "ФормаОтчета2019Кв1_Раздел3_5");
	СоответствиеРазделовМакетам.Вставить("Раздел5",   "ФормаОтчета2019Кв1_Раздел5");
	СоответствиеРазделовМакетам.Вставить("Подписи",   "ФормаОтчета2019Кв1_Подписи");
	...
	Если Парам1.ИмяСтраницы = "Раздел3_5" Тогда
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	РегламентированнаяОтчетностьКлиент.ВыборОтчетаПредидущегоПериода(ЭтаФорма, Область, СтандартнаяОбработка, "ТабличныйДокумент");
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел3_5" Тогда
		...
		Если СтрНайти(Область.Имя, "П000005008601_") > 0 ИЛИ СтрНайти(Область.Имя, "П000005008604_") > 0 ИЛИ СтрНайти(Область.Имя, "П000005008605_") > 0 
			Или СтрНайти(Область.Имя, "П000005008701_") > 0 ИЛИ СтрНайти(Область.Имя, "П000005008704_") > 0 ИЛИ СтрНайти(Область.Имя, "П000005008705_") > 0 Тогда
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел5" Тогда
		...
		Если СтрНайти(Область.Имя, "П000005008801_") > 0 ИЛИ СтрНайти(Область.Имя, "П000005008804_") > 0 ИЛИ СтрНайти(Область.Имя, "П000005008805_") > 0 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "Коды_ОКАТО_ОКСМ");
			//создание динамического списка выбора
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
...
Функция СформироватьПараметрыВыбораОтчетаПредыдущегоПериода() Экспорт 
	...
	Парам.Вставить("ИсточникОтчета", "РегламентированныйОтчетСтатистикаФормаП1СХ");
	Парам.Вставить("ВидОтправляемогоДокумента", ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.СтатистикаФормаП1СХ"));
