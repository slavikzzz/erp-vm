...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы1, 8);
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Для Каждого Раздел Из мСвойстваРазделовДекларации.ПолучитьЭлементы() Цикл
		//Восстановление данных отчета 1-ИП (мес) по приказу 13.10.2021 № 704
		//после обновления на новые редакции конфигураций включающие БРО
		Если Раздел.ИмяСтраницы = "Подписи" И Не ПоказателиОтчета.Свойство("ПолеТабличногоДокументаПодписи") Тогда
			Если РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ЭтотОбъект, "СтруктураДанныхПодписи")
			   И ТипЗнч(СтруктураДанныхПодписи) = Тип("Структура") Тогда
				ПоказателиОтчетаРаздел1 = ПоказателиОтчета.ПолеТабличногоДокументаРаздел1;
				СтруктураДанныхПодписи.Вставить("ОргИсполнитель", ПоказателиОтчетаРаздел1.ОргИсполнитель);
				СтруктураДанныхПодписи.Вставить("ОргДолжностьИсп", ПоказателиОтчетаРаздел1.ОргДолжностьИсп);
				СтруктураДанныхПодписи.Вставить("ОргТелефонИсп", ПоказателиОтчетаРаздел1.ОргТелефонИсп);
				СтруктураДанныхПодписи.Вставить("ОргАдресЭлектроннойПочты", ПоказателиОтчетаРаздел1.ОргАдресЭлектроннойПочты);
				СтруктураДанныхПодписи.Вставить("ДатаСоставленияОтчета", ПоказателиОтчетаРаздел1.ДатаСоставленияОтчета);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "1000013346",
		250, , "Выбор продукции");
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_okpd2_ei_50_2025",
		250, , "Выбор единицы измерения");
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Раздел1",		"ФормаОтчета2022Кв1_Раздел1");
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Сред(Область.Имя, 12, 2) = "01" Или  Сред(Область.Имя, 12, 2) = "03" Тогда
		
		ИмяСправочника = "1000013346";
		
		ОбластьКод = ТабличныйДокумент.Области.Найти(СтрЗаменить(Область.Имя, Сред(Область.Имя, 1, 11) + "01_", Сред(Область.Имя, 1, 11) + "03_"));
		ОбластьНаим  = ТабличныйДокумент.Области.Найти(СтрЗаменить(Область.Имя, Сред(Область.Имя, 1, 11) + "03_", Сред(Область.Имя, 1, 11) + "01_"));
		
		НомСтр = Сред(Область.Имя, СтрНайти(Область.Имя, "_") + 1);
		
		МассивОчистки = Новый Массив;
		МассивОчистки.Добавить(ТабличныйДокумент.Области[Сред(Область.Имя, 1, 11) + "04_" + НомСтр]);
		МассивОчистки.Добавить(ТабличныйДокумент.Области[Сред(Область.Имя, 1, 11) + "06_" + НомСтр]);
	...
	ИначеЕсли Сред(Область.Имя, 12, 2) = "04" Тогда
		...
		ОбластьКод = ТабличныйДокумент.Области.Найти(СтрЗаменить(Область.Имя, Сред(Область.Имя, 1, 11) + "04_", Сред(Область.Имя, 1, 11) + "06_"));
		ОбластьНаим  = ТабличныйДокумент.Области.Найти(СтрЗаменить(Область.Имя, Сред(Область.Имя, 1, 11) + "06_", Сред(Область.Имя, 1, 11) + "04_"));
		
		НомСтр = Сред(Область.Имя, СтрНайти(Область.Имя, "_") + 1);
		КодТовараОКПД2 = СокрЛП(Строка(ТабличныйДокумент.Области[Сред(Область.Имя, 1, 11) + "03_" + НомСтр].Значение));
		
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, ИмяСправочника);
		ВвестиПоказатель(Элемент, ОбластьКод, ОбластьНаим, ИмяСправочника, КодТовараОКПД2, ПараметрВводаПоля.ТекстПриВыборе, , ,СтандартнаяОбработка);
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Пока Истина Цикл 
		Инд = Инд + 1;
		О2 = ""; О3 = "";
		Если Не ПараметрыВыгрузки.Свойство("П001001000104_" + Инд, О2)
			Или Не ПараметрыВыгрузки.Свойство("П001001000106_" + Инд, О3) Тогда 
			
			Прервать;
		КонецЕсли;
		
		Если О2 = "-" И О3 = "0" Тогда 
			ПараметрыВыгрузки.Вставить("ЧНП001001000105_" + Инд, "ЧН='-'");
			Прервать;
		КонецЕсли;
	КонецЦикла;
