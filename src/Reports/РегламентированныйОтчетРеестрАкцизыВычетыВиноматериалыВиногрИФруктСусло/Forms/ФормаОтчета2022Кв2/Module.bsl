...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	ИменаГраф.Добавить("П1200027");
	ИменаГраф.Добавить("П1200028");
	ИменаГраф.Добавить("П1200029");
	ИменаГраф.Добавить("П1200030");
	ИменаПодчиненныхГрупп = Новый Массив;
	...
	Если ИмяРаздела = "Реестр" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя)
				...
				Пока ТДО.Найти("П1000004_" + Формат(Ном, "ЧГ=")) <> Неопределено Цикл
					НомПодч = 1;
					Пока ТДО.Найти("П1200030_" + Формат(Ном, "ЧГ=") + "_" + Формат(НомПодч, "ЧГ=")) <> Неопределено Цикл
						СуммаГрафа30 = СуммаГрафа30 + ТДО.Найти("П1200030_" + Формат(Ном, "ЧГ=") + "_" + Формат(НомПодч, "ЧГ=")).Значение;
						НомПодч = НомПодч + 1;
					КонецЦикла;
					Ном = Ном + 1;
				КонецЦикла;
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если Ветка = АктивнаяВетка Тогда
				...
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					...
					Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
						...
						+ Формат(ДанныеМнЧ_П10000.Индекс(Ветка_П10000) + 1, "ЧГ=") + "_"
						+ Формат(ДанныеМнЧ_П12000.Индекс(Ветка_П12000) + 1, "ЧГ=")].Значение);
						Ветка_П12000.Данные.Вставить("П1200030", Форма.ТабличныйДокумент.Области["П1200030_"
						+ Формат(ДанныеМнЧ_П10000.Индекс(Ветка_П10000) + 1, "ЧГ=") + "_"
						+ Формат(ДанныеМнЧ_П12000.Индекс(Ветка_П12000) + 1, "ЧГ=")].Значение);
					КонецЦикла;
				КонецЦикла;
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
					...
					СуммаГрафа30 = СуммаГрафа30 + Ветка_П12000.Данные["П1200030"];
				КонецЦикла;
			КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("ИтогоСуммаГрафа30", "П1200030");
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					...
					СтатусПроверки = Ложь;
				Иначе
					ВозмКодыГрафа18_Строка = "251, 252, 253, 254, 255, 256";
					Если СтрРазделить(ВозмКодыГрафа18_Строка, ", ", Ложь).Найти(ЗначПок) = Неопределено Тогда
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомерСтрокиП10000_Строка
						+ " некорректно задан код вида подакцизного товара по графе 18. Возможны значения: " + ВозмКодыГрафа18_Строка,
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200027"]) Тогда
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомерСтрокиП10000_Строка
					+ " не задано наименование документа по графе 27", "Реестр", "П1200027" + ПостфиксП12000, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ЗначПок = Ветка_П12000.Данные["П1200029"];
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомерСтрокиП10000_Строка
					+ " не задана дата документа по графе 29", "Реестр", "П1200029" + ПостфиксП12000, НомерСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(ЗначПок) Тогда
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомерСтрокиП10000_Строка
					+ " задана дата документа по графе 29, выходящая за пределы периода с 01.01.1900 по 31.12.2099",
					"Реестр", "П1200029" + ПостфиксП12000, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	Для каждого Эл Из РрАк201_19_45.Строки Цикл
		ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
	КонецЦикла;
	
	РрАк201_19_1237 = ПолучитьПодчиненныйЭлемент(РеестрАк201_19, "РрАк201.19.1237");
	Для каждого Эл Из РрАк201_19_1237.Строки Цикл
		ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
	КонецЦикла;
	
	// Узел "РрАк201.19" имеется только в схеме выгрузки "Сведений". Если он не найден,
	// значит, сейчас формируются данные "Реестра" (для выгрузки первого из 2-х файлов).
	//
	РрАк201_19 = ПолучитьПодчиненныйЭлемент(РеестрАк201_19, "РрАк201.19");
	Если РрАк201_19 = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРрАк201_19 = СкопироватьУзел(РеестрАк201_19, РрАк201_19, РеестрАк201_19.Строки.Количество() - 1);
		
		Для каждого Эл Из ЭкзРрАк201_19.Строки Цикл
		...
		РрАк201_19Гр4_30 = ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_19, "РрАк201.19Гр4-30");
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзРрАк201_19Гр4_30 = СкопироватьУзел(ЭкзРрАк201_19, РрАк201_19Гр4_30, ЭкзРрАк201_19.Строки.Количество() - 1);
				
				РрАк201_19_45Пок = ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_19Гр4_30, "РрАк201.19.45Пок");
				
				ДокПдтвВыч4 = ПолучитьПодчиненныйЭлемент(РрАк201_19_45Пок, "ДокПдтвВыч4");
				Для каждого Эл Из ДокПдтвВыч4.Строки Цикл
					ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
				КонецЦикла;
				
				ДокПдтвВыч5 = ПолучитьПодчиненныйЭлемент(РрАк201_19_45Пок, "ДокПдтвВыч5");
				...
				КонрагПокОрг = ПолучитьПодчиненныйЭлемент(ДокПдтвВыч5, "КонрагПокОрг");
				КонрагПокИП  = ПолучитьПодчиненныйЭлемент(ДокПдтвВыч5, "КонрагПокИП");
				...
				ДокПдтвВыч = ПолучитьПодчиненныйЭлемент(ДокПдтвВыч5, "ДокПдтвВыч");
				
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					
					ЭкзДокПдтвВыч = СкопироватьУзел(ДокПдтвВыч5, ДокПдтвВыч, ДокПдтвВыч5.Строки.Количество() - 1);
				...
				РрАк201_19_1237Пр = ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_19Гр4_30, "РрАк201.19.1237Пр");
				
				ДанныеМнЧ_П12000 = Ветка_П10000.ДанныеМногострочныхЧастей.П12000.Строки;
				Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
					
					ЭкзРрАк201_19_1237Пр = СкопироватьУзел(
					ЭкзРрАк201_19Гр4_30, РрАк201_19_1237Пр, ЭкзРрАк201_19Гр4_30.Строки.Количество() - 1);
					
					КонрагПрОрг = ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_19_1237Пр, "КонрагПрОрг");
					КонрагПрИП  = ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_19_1237Пр, "КонрагПрИП");
					...
					КодПТ = ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_19_1237Пр, "КодПТ");
					ВывестиПоказательВXML(КодПТ, Ветка_П12000.Данные[КодПТ.Ключ]);
					
					ДокПдтвВыч12 = ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_19_1237Пр, "ДокПдтвВыч12");
					Для каждого Эл Из ДокПдтвВыч12.Строки Цикл
						ВывестиПоказательВXML(Эл, Ветка_П12000.Данные[Эл.Ключ]);
					КонецЦикла;
					
					ДокПдтвВыч7 = ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_19_1237Пр, "ДокПдтвВыч7");
					Для каждого Эл Из ДокПдтвВыч7.Строки Цикл
						ВывестиПоказательВXML(Эл, Ветка_П12000.Данные[Эл.Ключ]);
					КонецЦикла;
					
					ДокПдтвВыч3 = ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_19_1237Пр, "ДокПдтвВыч3");
					Для каждого Эл Из ДокПдтвВыч3.Строки Цикл
						ВывестиПоказательВXML(Эл, Ветка_П12000.Данные[Эл.Ключ]);
					КонецЦикла;
					
				КонецЦикла;
				
				РегламентированнаяОтчетность.УдалитьУзел(РрАк201_19_1237Пр);
				
			КонецЦикла;
			
		КонецЦикла;
		
		РегламентированнаяОтчетность.УдалитьУзел(РрАк201_19Гр4_30);
		
	КонецЦикла;
	
	РегламентированнаяОтчетность.УдалитьУзел(РрАк201_19);
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Для ИндПодчСтроки = 0 По КолПодчСтрок - 1 Цикл
					...
					Если НЕ ПечФорма.ПроверитьВывод(МассивПроверки) Тогда
						...
						Обл = ПечФорма.Область(ПечФорма.ВысотаТаблицы, 2, ПечФорма.ВысотаТаблицы, 31);
						Если Обл <> Неопределено Тогда
							Обл.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная);
						КонецЕсли;
					...
					Иначе
						...
						Если Строки12.Количество() > 1 Тогда
							Обл = ПечФорма.Область(ПечФорма.ВысотаТаблицы, 16, ПечФорма.ВысотаТаблицы, 31);
							Если Обл <> Неопределено Тогда
								Обл.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная);
							КонецЕсли;
						КонецЕсли;
				...
				Обл = ПечФорма.Область(ПечФорма.ВысотаТаблицы, 5, ПечФорма.ВысотаТаблицы, 31);
				Если Обл <> Неопределено Тогда
					Обл.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная);
				КонецЕсли;
