...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПараметрыОтчета = ПараметрыОтчета(НастройкиОсновнойСхемы);
	
	Если ВРег(ПараметрыОтчета.ОценкаТМЦ) = ВРег("ПоВидуЦен")
		И НЕ ЗначениеЗаполнено(ПараметрыОтчета.ВидЦены) Тогда
		
		ТекстСообщения = НСтр("ru = 'Для оценки стоимости необходимо указать вид цены в настройках отчета.';
								|en = 'To valuate cost, specify the price kind in the report settings.'");
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ТекстЗапроса = СтрЗаменить(
		...
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаВесУпаковки(
			"ТМЦВЭксплуатации.Номенклатура.ЕдиницаИзмерения",
...
Процедура ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Параметры.Свойство("ПараметрКоманды") Тогда
		...
		Если Параметры.Свойство("ОписаниеКоманды")
			...
			Если Параметры.ОписаниеКоманды.ДополнительныеПараметры.ИмяКоманды = "ОстаткиТМЦВЭксплуатации" Тогда
...
Функция ПараметризуемыеЗаголовкиПолей()
	...
	ПереопределениеЗаголовков.Вставить("ИтогоВЕдиницахХранения", НСтр("ru = 'Итого в единицах хранения';
																		|en = 'Total in stock UOM'"));
	ПереопределениеЗаголовков.Вставить("ИтогоВЕдиницахДляОтчетов", НСтр("ru = 'Итого в единицах для отчетов';
																		|en = 'Total in report UOM'"));

	СтруктураЗаголовков = КомпоновкаДанныхСервер.ЗаголовкиПолейЕдиницИзмерений(КомпоновщикНастроек, ПереопределениеЗаголовков);
...
Процедура УстановитьПараметрыОтчета()
	...
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ТекущаяДата", ТекущаяДатаСеанса());
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ВалютаУпр", Константы.ВалютаУправленческогоУчета.Получить());
	Ценообразование.УстановитьВариантЦенообразованияВСКД(КомпоновщикНастроек);
	
КонецПроцедуры

Функция ПараметрыОтчета(НастройкиОсновнойСхемы)

	ЗначенияОтбораДанных = Новый Структура;
	ЗначенияОтбораДанных.Вставить("ОценкаТМЦ", ОтчетыУТСервер.ЗначениеПараметра(НастройкиОсновнойСхемы, "ОценкаТМЦ"));
	ЗначенияОтбораДанных.Вставить("ВидЦены", ОтчетыУТСервер.ЗначениеПараметра(НастройкиОсновнойСхемы, "ВидЦены"));
