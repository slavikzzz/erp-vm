...
Процедура ПоказатьПериод()
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодВЗаголовкеТаблицы.Значение = "Фактические запасы
		|на 1 число " + Сред(Формат(Дата1ЧислоСледМесяца, "ДФ=""д ММММ"""), 3);
		
	Если Не (Месяц(Дата1ЧислоСледМесяца) = 4 Или Месяц(Дата1ЧислоСледМесяца) = 5 Или Месяц(Дата1ЧислоСледМесяца) = 6) Тогда
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателей.Добавить(ВыбТабПоле.Области["ПериодВЗаголовкеТаблицы"].Значение,   "ПериодВЗаголовкеТаблицы");
