...
Процедура ПоказатьПериод()
	...
	Дата1ЧислоСледМесяца = СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета + 1;
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета.Значение =
		"по состоянию на 1 число " + Сред(Формат(Дата1ЧислоСледМесяца, "ДФ=""д ММММ гггг 'г.'"""), 3);
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодВЗаголовкеТаблицы.Значение = "Фактические запасы
		|на 1 число " + Сред(Формат(Дата1ЧислоСледМесяца, "ДФ=""д ММММ"""), 3) + ", т";
		
	Если Не (Месяц(Дата1ЧислоСледМесяца) = 5 Или Месяц(Дата1ЧислоСледМесяца) = 6) Тогда 
		ОбластьПредупреждения = ПолеТабличногоДокументаФормаОтчета.Области.Найти("ОбластьПредупреждение");
		Если ОбластьПредупреждения <> Неопределено Тогда
			ПолеТабличногоДокументаФормаОтчета.УдалитьОбласть(
				ПолеТабличногоДокументаФормаОтчета.Области.ОбластьПредупреждение, ТипСмещенияТабличногоДокумента.ПоВертикали);
		КонецЕсли;
	КонецЕсли;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если Не СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация Тогда
		...
		Если СписокСохранения.Свойство("ОбособленноеПодразделение", ВерсияФормы) Тогда
			ОбособленноеПодразделение = ВерсияФормы;
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли Область.Имя = "ОргКодЗнач1" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "ОргКодЗнач1_" + СокрЛП(ПолеТабличногоДокументаФормаОтчета.Области["ОргКодЗнач"].Значение));
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателейНеПодлежащихОчистке.Добавить(ВыбТабличноеПоле.Области["ПериодВЗаголовкеТаблицы"].Значение,  "ПериодВЗаголовкеТаблицы");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Стр = 1 По 2 Цикл 
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-запасы");
	КонецЦикла;
	...
	Для Каждого КЗ Из СоответствиеБольшеЛибоРавно Цикл
		Для Каждого Элт Из КЗ.Значение Цикл 
			Для Гр = 4 По 6 Цикл
				...
				РегламентированнаяОтчетность.ДобавитьЭлементВСтруктуруОшибок(СтруктураОшибок, ТекстФормулы,
					"", ОбщееЧислоОшибок, ОбщееЧислоБезОшибок, Расшифровка, Нарушение, Описание, ЭтоОшибка);
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
...
Функция СформироватьПараметрыВыбораОтчетаПредыдущегоПериода() Экспорт 
	...
	Парам.Вставить("ИсточникОтчета", "РегламентированныйОтчетСтатистикаФорма4Запасы");
	Парам.Вставить("ВидОтправляемогоДокумента", ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.СтатистикаФорма4Запасы"));
