...
Процедура ПоказатьПериод(Форма)
	...
	Если ГодПериода >= 2017 Тогда
		...
		Форма.НадписьКтоСдаетОтчет = НСтр("ru = 'Лица, производящие выплаты и иные вознаграждения физическим лицам: организации, индивидуальные предприниматели, физические лица, не являющиеся индивидуальными предпринимателями (п.1 ст.419 НК РФ).';
											|en = 'Лица, производящие выплаты и иные вознаграждения физическим лицам: организации, индивидуальные предприниматели, физические лица, не являющиеся индивидуальными предпринимателями (п.1 ст.419 НК РФ).'");
...
Процедура ОткрытьФормуОтчета(Команда)
	...
	Если Год(мДатаКонцаПериодаОтчета) = 2023 Тогда
		ПараметрыФормы.Вставить("ФормаРекомендованаПисьмомОт20230926", СтрНайти(ПолеРедакцияФормы, "БС-4-11/12322@") > 0);
	Иначе
		ПараметрыФормы.Вставить("ФормаРекомендованаПисьмомОт20230926", Ложь);
	КонецЕсли;
