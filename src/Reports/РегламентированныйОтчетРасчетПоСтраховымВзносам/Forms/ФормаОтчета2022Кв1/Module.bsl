...
Процедура ЗагрузитьПодготовленныеДанные() Экспорт
	...
	Если СтруктураДанных.Свойство("Контейнер", Контейнер)
		...
		ОбъектОтчета(ИмяФормы).ОчиститьНезаполняемыеПоказатели_ДляКомпанийИТ(ДеревоРазд, СтруктураРеквизитовФормы);
...
Процедура Проверить(Команда)
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) > 2022 Тогда
		...
		|en = 'Проверка контрольных соотношений за отчетные периоды 2023 и более 
		| поздних календарных годов для данной формы не предусмотрена'"));
