	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2021 Тогда
		...
		ИначеЕсли ИмяПоказателя = "П000120009001" ИЛИ ИмяПоказателя = "П000120009003"
			...
			Если Признак = "2" Тогда
				...
				Графа = Сред(ИмяПоказателя, 13);
				ОбщегоНазначенияКлиент.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
					"ru='С отчетного периода 1 квартал 2021 года в строке 090 приложения 2 к разделу 1 признак ""2"" не может быть "
					+ "указан (письмо ФНС России от 29.01.2021 № БС-4-11/1020@). Значение в графе %1 строки 090 очищено'"), Графа));
				Мофицированность = Истина;
		...
		ИначеЕсли ИмяРаздела = "Раздел1Прил3" И ЗначениеЗаполнено(ЗначПоказателя) Тогда
			...
			Область.Очистить();
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр(
				"ru='С отчетного периода 1 квартал 2021 года приложение 3 к разделу 1 не подлежит заполнению "
		...
		ИначеЕсли ИмяРаздела = "Раздел1Прил4" И ЗначениеЗаполнено(ЗначПоказателя) Тогда
			...
			Область.Очистить();
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр(
				"ru='С отчетного периода 1 квартал 2021 года приложение 4 к разделу 1 не подлежит заполнению "
				+ "(письмо ФНС России от 29.01.2021 № БС-4-11/1020@). Значение очищено'"));
			Мофицированность = Истина;
