...
Процедура ДоступностьРазделов()
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <> 12 Тогда
		Для НомСтр = 1 По 6 Цикл
			ПолеТабличногоДокументаФормаОтчета.Области["П000140100101_" + НомСтр].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области["П000140100101_" + НомСтр].Защита = Истина;
			ПолеТабличногоДокументаФормаОтчета.Области["П000140100101_" + НомСтр].Значение = "";
			ПолеТабличногоДокументаФормаОтчета.Области["П000140100101_" + НомСтр].Формат = "";
		КонецЦикла;
	КонецЕсли;
