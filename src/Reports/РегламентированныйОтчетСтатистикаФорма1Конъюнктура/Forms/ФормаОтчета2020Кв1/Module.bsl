...
Процедура ДоступностьРазделов()
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6
		...
		Для НомСтр = 1 По 5 Цикл
			Для НомКол = 1 По 8 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П000060000" + НомСтр + "0" + НомКол].ЦветФона = Новый Цвет();
				ПолеТабличногоДокументаФормаОтчета.Области["П000060000" + НомСтр + "0" + НомКол].Защита = Истина;
			КонецЦикла;
		КонецЦикла;

		Для НомСтр = 1 По 7 Цикл
			ПолеТабличногоДокументаФормаОтчета.Области["П000080000" + НомСтр + "01"].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области["П000080000" + НомСтр + "01"].Защита = Истина;
		КонецЦикла;				
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <> 12 Тогда
		Для НомСтр = 1 По 5 Цикл
			ПолеТабличногоДокументаФормаОтчета.Области["П000140000" + НомСтр + "02"].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области["П000140000" + НомСтр + "02"].Защита = Истина;
		КонецЦикла;	
	КонецЕсли;	
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли ТекГруппа = "П00002" Тогда
		Если Область.Имя <> "П000020000107" Тогда
	...
	ИначеЕсли ТекГруппа = "П00006" Тогда
		Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 3 Тогда
			...
			Модифицированность = Истина;
			НомерСтроки = Число(Сред(Область.Имя, 10, 2));
			НомерКолонки = Прав(Область.Имя, 1);
			Если СтрНайти("1234", НомерКолонки) > 0 Тогда
				УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 1, 4);
			ИначеЕсли СтрНайти("5678", НомерКолонки) > 0 Тогда
				УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 5, 8, Истина);
			КонецЕсли;	
		КонецЕсли;		
	ИначеЕсли ТекГруппа = "П00007" Тогда
		...
		Модифицированность = Истина;
		НомерСтроки = Число(Сред(Область.Имя, 10, 2));
		УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 1, 2);
	ИначеЕсли ТекГруппа = "П00008" Тогда
		Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 3 Тогда
			СтандартнаяОбработка = Ложь;
			Модифицированность = Истина;
			УстановитьЗначокВГруппеЯчеек(Область, 1, 7, 1, 1, Истина);	
		КонецЕсли;		
	ИначеЕсли ТекГруппа = "П00009" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		УстановитьЗначокВГруппеЯчеек(Область, 1, 12, 1, 1, Истина);
	ИначеЕсли ТекГруппа = "П00010" Тогда
		СтандартнаяОбработка = Ложь;		
		Модифицированность = Истина;
		НомерКолонки = Число(Прав(Область.Имя, 1));
		УстановитьЗначокВГруппеЯчеек(Область, 1, 3, НомерКолонки, НомерКолонки);
	ИначеЕсли ТекГруппа = "П00011" Или ТекГруппа = "П00012" Или ТекГруппа = "П00013" Тогда
		СтандартнаяОбработка = Ложь;		
		Модифицированность = Истина;
		НомерСтроки = Число(Сред(Область.Имя, 10, 2));
		Если ТекГруппа = "П00013" Тогда			
	...
	Для Строка = НачальнаяСтрокаВГруппе По КонечнаяСтрокаВГруппе Цикл
		...
		Для Колонка = НачальнаяКолонкаВГруппе По КонечнаяКолонкаВГруппе Цикл
			...
			ИначеЕсли Не МножественныйВыбор Тогда
				Если Область.Имя = "П000020000107" Тогда
					мФормаОтчета.Области[ТекЯчейка].Значение = "";
				Иначе
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабПоле.Очистить();
	ВыбТабПоле.Вывести(ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаОтчета2020Кв1_ФормаОтчета"));
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-Конъюнктура");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1-Конъюнктура");
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтотОбъект, "СхемаВыгрузкиXML2020Кв1");
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревоДляЗагрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтотОбъект, "СхемаВыгрузкиXML2020Кв1");
...
Функция ПроверитьПоказатели() Экспорт
	...
	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтотОбъект, "СхемаВыгрузкиXML2020Кв1");
...
Функция УниверсальнаяВыгрузкаXMLПроверкаДанныхНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	РегламентированнаяОтчетность.ИзменитьВерсиюФорматаСПроверкойСовместимостиШаблонов(
		ЭтотОбъект, "АтрибВыгрузкиXML2020Кв1", "ШаблонРосстата2020Кв1", ПараметрыВыгрузки);
	ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки);
	Если НЕ РегламентированнаяОтчетность.ПроверитьДанные(ЭтотОбъект, ДеревоВыгрузки) Тогда
		КоллекцияСписковВыбора = "";
		Возврат Ложь;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ДопАтрибуты = РегламентированнаяОтчетность.СформироватьСтруктуруДопАтрибутов(ИмяФормы, "АтрибВыгрузкиXML2020Кв1");
