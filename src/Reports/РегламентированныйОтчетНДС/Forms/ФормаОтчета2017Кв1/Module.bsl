...
Процедура УдалитьСтроку(Команда)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Удалить выбранную строку?';
...
Процедура ПроверкаКС_1_2(СтруктураРезультата)
	...
	Если ЛеваяЧасть > ПраваяЧасть Тогда
		...
		СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), НСтр("ru = 'НК РФ ст. 161, 171, 172';
																											|en = 'НК РФ ст. 161, 171, 172'"));
		
	Иначе
		СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
			НСтр("ru = 'Нарушено соотношение показателей. Возможна арифметическая ошибка в декларации.';
				|en = 'Нарушено соотношение показателей. Возможна арифметическая ошибка в декларации.'"));
...
Процедура ПроверкаКС_1_3(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка суммы налога к уплате в бюджет (КС 1.3)';
						|en = 'Проверка суммы налога к уплате в бюджет (КС 1.3)'");
...
Процедура ПроверкаКС_1_4(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка суммы налога к уплате в бюджет (КС 1.4)';
						|en = 'Проверка суммы налога к уплате в бюджет (КС 1.4)'");
	...
	Если ЛеваяЧасть > ПраваяЧасть Тогда 
		СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), 
			НСтр("ru = 'Занижение суммы налога вследствие неполного отражения налоговой базы по операциям, указанным в строках 010-090.';
				|en = 'Занижение суммы налога вследствие неполного отражения налоговой базы по операциям, указанным в строках 010-090.'"));
		СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
			НСтр("ru = 'НК РФ ст. 146, 154, 155, 156, 158, 159, 162';
				|en = 'НК РФ ст. 146, 154, 155, 156, 158, 159, 162'"));
	...
	Если ЛеваяЧасть > ПраваяЧасть Тогда 
		СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
			НСтр("ru = 'Необоснованные налоговые вычеты';
				|en = 'Необоснованные налоговые вычеты'"));
	...
	Если ЛеваяЧасть < ПраваяЧасть Тогда 
		СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
			НСтр("ru = 'Занижение суммы НДС, исчисленной к уплате в бюджет по Разделу 3';
				|en = 'Занижение суммы НДС, исчисленной к уплате в бюджет по Разделу 3'"));
...
Процедура ПроверкаКС_1_11(СтруктураРезультата)
	...
	СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		НСтр("ru = 'Налоговые вычеты не обоснованны, либо налоговая база занижена, так как суммы отработанных авансов не включены в реализацию.';
			|en = 'Налоговые вычеты не обоснованны, либо налоговая база занижена, так как суммы отработанных авансов не включены в реализацию.'"));
	СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		НСтр("ru = 'НК РФ ст.171, п.8, НК РФ ст.172, п.6, 172, либо НК РФ ст.146, п.1';
			|en = 'НК РФ ст.171, п.8, НК РФ ст.172, п.6, 172, либо НК РФ ст.146, п.1'"));
	СтруктураОшибок.Вставить("Расшифровка" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																	|en = '%1'"), Расшифровка));
	СтруктураОшибок.Вставить("НомерФормулыРасшифровки" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), СчетчикФормулДляРасшифровок);
...
Процедура ПроверкаКС_1_12(СтруктураРезультата)
	...
	СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		НСтр("ru = 'Необоснованное применение налоговых вычетов при выполнении СМР для собственного потребления.';
			|en = 'Необоснованное применение налоговых вычетов при выполнении СМР для собственного потребления.'"));
	СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		НСтр("ru = 'НК РФ ст.171, п.6, ст.172, п.5';
			|en = 'НК РФ ст.171, п.6, ст.172, п.5'"));
	СтруктураОшибок.Вставить("Расшифровка" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																	|en = '%1'"), Расшифровка));
	СтруктураОшибок.Вставить("НомерФормулыРасшифровки" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), СчетчикФормулДляРасшифровок);
	...
	Если ЛеваяЧасть > ПраваяЧасть Тогда 
		СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
			НСтр("ru = 'Завышение суммы НДС, подлежащей вычету.';
				|en = 'Завышение суммы НДС, подлежащей вычету.'"));
		СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
			НСтр("ru = 'НК РФ ст.171, 172. Положение, разд. II п.1-п.3; Протокол, ст.2, ст.4.';
				|en = 'НК РФ ст.171, 172. Положение, разд. II п.1-п.3; Протокол, ст.2, ст.4.'"));
	...
	Иначе //Если (ЛеваяЧасть < ПраваяЧасть) Тогда
		СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
			НСтр("ru = 'Занижение НБ по НДС вследствие неполного отражения указанных гр.2 операций';
				|en = 'Занижение НБ по НДС вследствие неполного отражения указанных гр.2 операций'"));
		СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
			НСтр("ru = 'НК РФ ст.153, 164, 165, 167. Положение разд II п.1-п.3; Протокол, ст.2, ст.4';
				|en = 'НК РФ ст.153, 164, 165, 167. Положение разд II п.1-п.3; Протокол, ст.2, ст.4'"));
	...
	Иначе
		СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
			НСтр("ru = 'Занижена сумма НДС, подлежащая уплате в бюджет за отчетный период.';
				|en = 'Занижена сумма НДС, подлежащая уплате в бюджет за отчетный период.'"));
		СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), НСтр("ru = 'НК РФ ст.173.';
																											|en = 'НК РФ ст.173.'"));
	...
	Иначе
		СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
			НСтр("ru = 'Завышение суммы НДС, подлежащей возмещению за отчетный период.';
				|en = 'Завышение суммы НДС, подлежащей возмещению за отчетный период.'"));
		СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), НСтр("ru = 'НК РФ ст.173.';
																											|en = 'НК РФ ст.173.'"));
...
Процедура ПроверкаКС_1_22(СтруктураРезультата)
	...
	Если ЛеваяЧасть > ПраваяЧасть Тогда 
		...
		СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), НСтр("ru = 'НК РФ ст.171, 172, п.2 Раздела IV Приложения 4 к Постановлению № 1137.';
																											|en = 'НК РФ ст.171, 172, п.2 Раздела IV Приложения 4 к Постановлению № 1137.'"));
...
Процедура ПроверкаКС_1_23_1(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка приложения 1 к разделу 9 (КС 1.23.1)';
						|en = 'Проверка приложения 1 к разделу 9 (КС 1.23.1)'");
	...
	Если ЛеваяЧасть > ПраваяЧасть Тогда 
		СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
			НСтр("ru = 'Занижение суммы НДС, подлежащей уплате в бюджет.';
				|en = 'Занижение суммы НДС, подлежащей уплате в бюджет.'"));
		СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), НСтр("ru = 'НК РФ ст.173.';
																											|en = 'НК РФ ст.173.'"));
...
Процедура ПроверкаКС_1_23_2(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка приложения 1 к разделу 9 (КС 1.23.2)';
						|en = 'Проверка приложения 1 к разделу 9 (КС 1.23.2)'");
...
Процедура ПроверкаКС_1_23_3(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка приложения 1 разделу 9 (КС 1.23.3)';
						|en = 'Проверка приложения 1 разделу 9 (КС 1.23.3)'");
...
Процедура ПроверкаКС_1_23_4(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка приложения 1 к разделу 9 (КС 1.23.4)';
						|en = 'Проверка приложения 1 к разделу 9 (КС 1.23.4)'");
...
Процедура ПроверкаКС_1_23_5(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка приложения 1 к разделу 9 (КС 1.23.5)';
						|en = 'Проверка приложения 1 к разделу 9 (КС 1.23.5)'");
...
Процедура ПроверкаКС_1_23_6(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка приложения 1 к разделу 9 (КС 1.23.6)';
						|en = 'Проверка приложения 1 к разделу 9 (КС 1.23.6)'");
...
Процедура ПроверкаКС_1_24(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка раздела 12 (КС 1.24)';
						|en = 'Проверка раздела 12 (КС 1.24)'");
...
Процедура ПроверкаКС_1_25(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка суммы НДС, подлежащей возмещению за отчетный период (КС 1.25)';
						|en = 'Проверка суммы НДС, подлежащей возмещению за отчетный период (КС 1.25)'");
...
Процедура ПроверкаКС_1_27(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка НДС, исчисленного к уплате в бюджет (КС 1.27)';
						|en = 'Проверка НДС, исчисленного к уплате в бюджет (КС 1.27)'");
	...
	СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		НСтр("ru = 'Занижение суммы НДС, исчисленного к уплате в бюджет.';
			|en = 'Занижение суммы НДС, исчисленного к уплате в бюджет.'"));
	СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), НСтр("ru = 'НК РФ ст.153, 161, 164, 165, 166, 167, 173.';
																										|en = 'НК РФ ст.153, 161, 164, 165, 166, 167, 173.'"));
	...
	СтруктураОшибок.Вставить("Комментарий" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		НСтр("ru = 'Возможна погрешность в результате округления показателей.';
			|en = 'Возможна погрешность в результате округления показателей.'"));
...
Процедура ПроверкаКС_1_28(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка суммы НДС, подлежащей вычету (КС 1.28)';
						|en = 'Проверка суммы НДС, подлежащей вычету (КС 1.28)'");
...
Процедура ПроверкаКС_1_29(СтруктураРезультата)
	...
	СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		НСтр("ru = 'Завышение (занижение) суммы НДС, подлежащей вычету.';
			|en = 'Завышение (занижение) суммы НДС, подлежащей вычету.'"));
	СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), НСтр("ru = 'НК РФ ст. 171, 172.';
																										|en = 'НК РФ ст. 171, 172.'"));
...
Процедура ПроверкаКС_1_30(СтруктураРезультата)
	...
	СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		НСтр("ru = 'Завышение (занижение) суммы НДС, подлежащей вычету в связи с ввозом товаров на таможенную территорию ТС.';
			|en = 'Завышение (занижение) суммы НДС, подлежащей вычету в связи с ввозом товаров на таможенную территорию ТС.'"));
	СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), НСтр("ru = 'НК РФ п.1 ст.153, ст.171, 172, Протокол р.III.';
																										|en = 'НК РФ п.1 ст.153, ст.171, 172, Протокол р.III.'"));
...
Процедура ПроверкаКС_1_32(СтруктураРезультата)
	...
	СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		НСтр("ru = 'Возможно завышение суммы НДС, подлежащей вычету.';
			|en = 'Возможно завышение суммы НДС, подлежащей вычету.'"));
	СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		НСтр("ru = 'НК РФ ст.171, 172, п.7 Раздела II Приложения 4 к Постановлению № 1137.';
			|en = 'НК РФ ст.171, 172, п.7 Раздела II Приложения 4 к Постановлению № 1137.'"));
...
Процедура ПроверкаКС_1_34(СтруктураРезультата)
	...
	СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		НСтр("ru = 'Возможно занижение суммы НДС, исчисленного к уплате в бюджет.';
			|en = 'Возможно занижение суммы НДС, исчисленного к уплате в бюджет.'"));
	СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), НСтр("ru = 'НК РФ ст.153, 173, п.8 Раздела II  Приложения 5 к Постановлению № 1137.';
																										|en = 'НК РФ ст.153, 173, п.8 Раздела II  Приложения 5 к Постановлению № 1137.'"));
...
Процедура ПроверкаКС_1_35(СтруктураРезультата)
	...
	СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), НСтр("ru = 'НК РФ ст.153, 173, п.3 Раздела IV  Приложения 5 к Постановлению № 1137.';
																										|en = 'НК РФ ст.153, 173, п.3 Раздела IV  Приложения 5 к Постановлению № 1137.'"));
...
Процедура ПроверкаКС_1_36(СтруктураРезультата)
	...
	СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="),
		НСтр("ru = 'Возможно занижение исчисленной суммы НДС, вследствие неполного отражения НБ либо неверное применение ставки по НДС.';
			|en = 'Возможно занижение исчисленной суммы НДС, вследствие неполного отражения НБ либо неверное применение ставки по НДС.'"));
	СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ="), НСтр("ru = 'НК РФ ст.164, 165, 167, 173.';
																										|en = 'НК РФ ст.164, 165, 167, 173.'"));
	...
	ИначеЕсли РезультатПроверкиВыгрузки.КоличествоОбнаруженныхОшибок = 0
		...
		Если РезультатПроверкиКонтрагентов.ОшибкаПодключенияКСервису Тогда
			...
			Иначе
				...
				|en = 'Нет ошибок, препятствующих отправке декларации. Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису.'"));
				Элементы.РезультатПроверки.ЦветФона = Новый Цвет(255, 255, 192);
			КонецЕсли;
		...
		Если РезультатПроверкиКонтрагентов.ОшибкаПодключенияКСервису Тогда
			...
			Иначе
				ТекстРезультатаПроверки.Добавить(НСтр("ru = '. Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису.';
														|en = '. Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису.'"));
			КонецЕсли;
