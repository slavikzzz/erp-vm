...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоВидовЦен") Тогда
		...
		Если НастройкаВидЦены.Использование И Не ЗначениеЗаполнено(НастройкаВидЦены.Значение) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнено обязательное поле ""Вид цены"".';
																	|en = 'Required ""Price type"" is not filled in.'"), , , , Отказ);
		КонецЕсли;
	Иначе
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'В системе не настроены параметры прайс-листа.';
																	|en = 'Price list parameters are not set in the application.'"), , , , Отказ);
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НастройкаПериодЗначение) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнено обязательное поле ""Период"".';
																|en = 'Required ""Period"" is not filled in.'"), , , , Отказ);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(НастройкаПериодичность.Значение) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнено обязательное поле ""Периодичность"".';
																|en = 'Required ""Frequency"" is not filled in.'"), , , , Отказ);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(НастройкаПериодЗначение) 
		...
		Если НЕ НастройкаПериодаЗаданаКорректно(НастройкаПериодЗначение.ДатаНачала,
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Периодичность расчета должна быть короче периода отчета.';
																	|en = 'Periodicity of calculation should be less than the report period.'"), , , , Отказ);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
