...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Или СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация Тогда
		...
		Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
			...
			Если ТаблицаСтраницыРаздела1.Количество() = 1
				И ТипЗнч(ТаблицаСтраницыРаздела1[0].Данные) = Тип("СписокЗначений")
				И ТаблицаСтраницыРаздела1[0].Данные.Количество() = 1 Тогда
				
				ДанныеСтр1Разд1 = ТаблицаСтраницыРаздела1[0].Данные[0].Значение;
				...
				Если ТипЗнч(ДанныеСтр1Разд1) = Тип("Структура") Тогда
					...
					Если СведенияОбОрганизации.Свойство("ОКТМО") Тогда
						...
						ДанныеСтр1Разд1.Вставить("ОКАТО", СокрЛП(СведенияОбОрганизации.ОКТМО));
...
Процедура ФормироватьСтруктуруСвойствСтраниц() Экспорт
	...
	СтрокаУровня1.ИмяПредставления                     = "Раздел1";
	...
	Для Каждого СтраницаРаздела1 Из ТаблицаСтраницыРаздела1 Цикл
		...
		Если СтраницаРаздела1.Данные[0].Значение.Свойство("ОКАТО1") Тогда
			...
			СтраницаРаздела1.Данные[0].Значение.Вставить("ОКАТО",
											 СтрЗаменить(СокрЛП(СтраницаРаздела1.Данные[0].Значение.ОКАТО1
		...
		Для ПорядковыйНомер = 1 По 10 Цикл
			...
			Если СтраницаРаздела1.Данные[0].Значение.Свойство("Дата" + ПорядковыйНомер + "_1") Тогда
				...
				СтраницаРаздела1.Данные[0].Значение["Дата" + ПорядковыйНомер] = ?(ЗначениеЗаполнено(СтраницаРаздела1.Данные[0].Значение["Дата" + ПорядковыйНомер]), Дата(СтраницаРаздела1.Данные[0].Значение["Дата" + ПорядковыйНомер]), '00010101');
		...
		Если СтраницаРаздела1.Данные[0].Значение.Свойство("КолЛицензия1") Тогда
			...
			СтраницаРаздела1.Данные[0].Значение.Вставить("КолЛицензия",
											 СокрЛП(СтраницаРаздела1.Данные[0].Значение.КолЛицензия1
											  	  ...
											  	  + СтраницаРаздела1.Данные[0].Значение.КолЛицензия4
												  + СтраницаРаздела1.Данные[0].Значение.КолЛицензия5));
			Для Ном = 1 По 5 Цикл
	    ...
	    Если ТипДанныхПоказателя = "Е" Тогда
			...
			Иначе
				...
				Иначе	
					...
					Если НЕ СтруктураГруппы = Неопределено Тогда
...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	СтруктураПараметров = Новый Структура("Организация, ДатаЗначения, СведенияОбОрганизации",
	СтруктураРеквизитовФормы.Организация, ДатаПодписи, Результат);
	РегламентированнаяОтчетность.ЗаменитьСведенияОбОрганизацииСведениямиОКрупнейшемНалогоплательщике(
	СтруктураПараметров);
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Титульный", "ПечатнаяФормаОтчета2013кв4_Титульный");
	СоответствиеРазделовМакетам.Вставить("Раздел1",   "ПечатнаяФормаОтчета2013кв4_Раздел1");
	СоответствиеРазделовМакетам.Вставить("Раздел2",   "ПечатнаяФормаОтчета2013кв4_Раздел2");
	...
	Для Каждого Область Из ИсходныйМакет.Области Цикл
		...
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник И Область.СодержитЗначение Тогда
			...
			ИначеЕсли ПараметрыПечати.ИмяСтраницы = "Раздел1" Тогда
				...
				ИначеЕсли СтрЧислоВхождений(Область.Имя, "ОКАТО") > 0 
					ИЛИ СтрЧислоВхождений(Область.Имя, "КолЛицензия") > 0 Тогда
						...
						Если СтрЧислоВхождений(Область.Имя, "ОКАТО") Тогда
							стрОкато = РегламентированнаяОтчетностьКлиентСервер.ДополнитьСтроку(стрОкато, 11, "-", 1);
						КонецЕсли;
				...
				ИначеЕсли СтрЧислоВхождений(Область.Имя, "Сумма") > 0 Тогда
					...
					Если ЗначениеЗаполнено(Область.Значение) Тогда
						...
						стрСумма = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Область.Значение);
						стрСумма = Формат(стрСумма, "ЧГ=");
						...
						Для Ном = 1 По СтрДлина(стрСумма) Цикл
							...
							Если МакетПечатнойФормы.Области.Найти(Область.Имя + "_" + НомСтр) = Неопределено Тогда
								Прервать;
							КонецЕсли;
							
							МакетПечатнойФормы.Области[Область.Имя + "_" + НомСтр].Значение = Сред(стрСумма, Ном, 1);
				...
				ИначеЕсли СтрЧислоВхождений(Область.Имя, "Дата") > 0 Тогда
					...
					Если ЗначениеЗаполнено(Область.Значение) Тогда
						...
						Для Ном = 1 По СтрДлина(Лев(СтрЗаменить(Область.Значение, ".", ""), 8)) Цикл
							...
							МакетПечатнойФормы.Области[Область.Имя + "_" + Формат(Ном, "ЧГ=")].Значение = Сред(СтрЗаменить(Область.Значение, ".", ""), Ном, 1);
			...
			ИначеЕсли ПараметрыПечати.ИмяСтраницы = "Раздел2" Тогда
				...
				Если СтрНачинаетсяС(Область.Имя, "П000020001008") ИЛИ СтрНачинаетсяС(Область.Имя, "П000020001009") Тогда
					
					МакетПечатнойФормы.Области[Область.Имя].Значение
					= РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Область.Значение);
...
Процедура ВыгрузитьТитульныйЛистИСведенияОФизЛицах(Текст)
	...
	ОргКПП = ПоказателиТитульный.КПП;
	ОргИНН = ПоказателиТитульный.ИНН;
	КатегорияНП = ?(НЕ РегламентированнаяОтчетностьВызовСервера.ЭтоЮридическоеЛицо(Орг), 2, 1);
