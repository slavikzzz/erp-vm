...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Титульный" Тогда
		...
		Если ЗначениеЗаполнено(Форма.СтруктураРеквизитовФормы.СведенияВыгрузкиДекл["ИмяФайлаВыгрузкиДекл"]) Тогда
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ЗаполненаМногострочнаяЧасть(ДанныеМнЧ) Тогда
		...
		КонецЦикла;
	Иначе
		СообщитьВТС("Не заполнены сведения об учедителях", "Раздел1", "П00001М200001_1", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
