	...
	ИначеЕсли Область.Имя = "ОргПочтАдрес" Тогда
		...
		Если ТаблицаВыбора.Количество() = 1 Тогда
			ПоказатьПредупреждение(,НСтр("ru = 'В контактной информации юридический и почтовый адрес не указаны.';
										|en = 'В контактной информации юридический и почтовый адрес не указаны.'"));
			Возврат;
		КонецЕсли;
...
Процедура СообщитьОНевозможностиВыгрузки()
	...
	РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
		НСтр("ru = 'Выгрузка бухгалтерской отчетности в формате версии 5.01 в настоящее время не применяется';
			|en = 'Выгрузка бухгалтерской отчетности в формате версии 5.01 в настоящее время не применяется'"),
		"Баланс");
...
Процедура ВставитьДополнительнуюСтроку()
	...
	Если ИмяГруппы = Неопределено Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не выбрана добавляемая строка многострочного раздела.';
													|en = 'Не выбрана добавляемая строка многострочного раздела.'"));
		Возврат;
	КонецЕсли;
