...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТабличныйДокумент.Вывести(
		ОбъектОтчета(ИмяФормы).ПолучитьМакет(
	...
	СтруктураРеквизитовФормы.ЗаголовокПредставление = Метаданные.Отчеты[Сред(Лев(ИмяФормы, СтрНайти(
		ИмяФормы, ".Форма.") - 1), 7)].ОсновнаяФорма.Синоним;
...
Процедура ЗагрузитьПоказателиИзМакета(РазделВМакете)
	...
	МакетСоставаПоказателей = ОбъектОтчета(ИмяФормы).ПолучитьМакет("Списки2014Кв1");
	...
	Если НЕ СвязьУстановлена Тогда
		...
		Если ТаблицаСтраницРазделаМ.Количество() = 0 Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Ошибка установки активной страницы раздела ФЛ.';
									|en = 'Ошибка установки активной страницы раздела ФЛ.'");
			Сообщение.Сообщить();
	...
	Если ВыбЭлемент = Неопределено Тогда
		Если НЕ ЗначениеЗаполнено(ТекОбласть.Значение) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Не выбран налоговый (отчетный) период';
									|en = 'Не выбран налоговый (отчетный) период'");
			Сообщение.Сообщить();
		КонецЕсли;
	Иначе
			...
			Если (НЕ ПустаяСтрока(Представитель.ИННУпПред)) И (ПустаяСтрока(ТекПредставительИНН)) Тогда
				...
				Для Инд = 0 По ВсегоЭкземпляров - 1 Цикл
					Если ЭтаФорма[мСтруктураМногостраничныхРазделов["ФизЛица"]][Инд].Данные[0].Значение.ФизЛицоФИО = ТекПредставительФИО Тогда
						Если ВсегоЭкземпляров = 1 Тогда
							// Лист единственный в разделе.
						...
						Иначе
							// Лист не единственный в разделе.
	...
	ИначеЕсли ИмяРаздела = "Раздел2" Тогда
		...
		ИначеЕсли ИмяПоказателя = "ИННРО1" Тогда
			...
			Если НЕ РегламентированнаяОтчетностьКлиент.ИННСоответствуетТребованиямНаКлиенте(ЭтаФорма, Область.Значение, Ложь) Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Указанный ИНН %1 - не соответствует требованиям!';
																								|en = 'Указанный ИНН %1 - не соответствует требованиям!'"), Область.Значение);
				Сообщение.Сообщить();
		...
		ИначеЕсли ИмяПоказателя = "КППРО1" Тогда
			...
			Если НЕ РегламентированнаяОтчетностьКлиентСервер.КППСоответствуетТребованиям(Область.Значение) Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Указанный КПП %1 - не соответствует требованиям!';
																								|en = 'Указанный КПП %1 - не соответствует требованиям!'"), Область.Значение);
				Сообщение.Сообщить();
...
Функция ПроверитьТитульныйЛистИСведенияОФизЛицах(БратьИзФормы = Истина, ВыводитьСообщения = Истина, ЧтоПроверять = "12", ПечатаемыеЛисты = Неопределено)
	...
	КодИМНС = СокрЛП(ПоказателиТитульный.КодИМНС);
	Если НЕ ЗначениеЗаполнено(КодИМНС) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не задан код налогового органа!", "Титульный", "КодИМНС");
		СтатусПроверки = 1;
	ИначеЕсли СтрДлина(КодИМНС) <> 4 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Задан код налогового органа, не состоящий из 4 цифр!", "Титульный", "КодИМНС");
		СтатусПроверки = 1;
	КонецЕсли;
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	ПроверкаКС_4_1_1_к(СтруктураРезультата);
	ПроверкаКС_4_1_2_к(СтруктураРезультата);
	ПроверкаКС_4_1_3_к(СтруктураРезультата);
	ПроверкаКС_4_1_4_к(СтруктураРезультата);
	ПроверкаКС_4_1_5_к(СтруктураРезультата);
	ПроверкаКС_4_1_6_к(СтруктураРезультата);
	ПроверкаКС_4_2_1_к(СтруктураРезультата);
	ПроверкаКС_4_2_2_к(СтруктураРезультата);
	ПроверкаКС_4_2_3_к(СтруктураРезультата);
	ПроверкаКС_4_2_4_к(СтруктураРезультата);
...
Функция ПроверкаРаздел5(СтруктураРезультата)
	...
	Для Каждого СтраницаР5 Из Раздел5 Цикл
		...
		РегламентированнаяОтчетность.ЗарегистрироватьПоказательДляРасшифровки(СписокРасшифровкиПоказателей, "П000050035004",
			...
			")", Р5Ст350, "", СтруктураРеквизитовФормы.мСохраненныйДок, "Раздел5", Неопределено, НомерСтраницыСтр, СчетчикФормулДляРасшифровок);
	...
	Для Каждого Страница1 Из Раздел5 Цикл
		...
		ПраваяЧасть = Окр(Страница1.Данные.П000050001004) - Окр(Страница1.Данные.П000050016004) - Окр(Страница1.Данные.П000050026004) - Окр(Страница1.Данные.П000050027004) - Окр(Страница1.Данные.П000050028004) - Окр(Страница1.Данные.П000050030004);
		ПраваяЧасть = Макс(ПраваяЧасть, 0);
		
		ЭтоОшибка = Не Окр(Страница1.Данные.П000050034004) = ПраваяЧасть;
		
		Расшифровка = "" + Окр(Страница1.Данные.П000050034004) + " = " + Окр(Страница1.Данные.П000050001004) + " - " + Окр(Страница1.Данные.П000050016004) + " - " + Окр(Страница1.Данные.П000050026004) + " - " + Окр(Страница1.Данные.П000050027004) + " - " + Окр(Страница1.Данные.П000050028004) + " - " + Окр(Страница1.Данные.П000050030004) + " Итог [" + ПраваяЧасть + "]";
		
		Если УсловиеВыполнено И ЭтоОшибка Тогда
			ОписаниеНарушения = ?(Окр(Страница1.Данные.П000050034004) > ПраваяЧасть, "Завышена налоговая база по налогу на прибыль, подлежащая уплате в федеральный бюджет", "Занижена налоговая база по налогу на прибыль, подлежащая уплате в федеральный бюджет");
			Норма = "ст. 45 НК РФ";
			ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
		Иначе
			ОписаниеНарушения = "";
	...
	Для Каждого Страница1 Из Раздел5 Цикл
		...
		ПраваяЧасть = Окр(Страница1.Данные.П000050001004) - Окр(Страница1.Данные.П000050016004) - Окр(Страница1.Данные.П000050026004) - Окр(Страница1.Данные.П000050027004) - Окр(Страница1.Данные.П000050028004) - Окр(Страница1.Данные.П000050030004) - Окр(Страница1.Данные.П000050032004);
		ПраваяЧасть = Макс(ПраваяЧасть, 0);
		
		ЭтоОшибка = Не Окр(Страница1.Данные.П000050035004) = ПраваяЧасть;
		
		Расшифровка = "" + Окр(Страница1.Данные.П000050035004) + " = " + Окр(Страница1.Данные.П000050001004) + " - " + Окр(Страница1.Данные.П000050016004) + " - " + Окр(Страница1.Данные.П000050026004) + " - " + Окр(Страница1.Данные.П000050027004) + " - " + Окр(Страница1.Данные.П000050028004) + " - " + Окр(Страница1.Данные.П000050030004) + " - " + Окр(Страница1.Данные.П000050032004) + " Итог [" + ПраваяЧасть + "]";
		
		Если УсловиеВыполнено И ЭтоОшибка Тогда
			ОписаниеНарушения = ?(Окр(Страница1.Данные.П000050035004) > ПраваяЧасть, "Завышена налоговая база по налогу на прибыль, подлежащая уплате в бюджет субъекта РФ", "Занижена налоговая база по налогу на прибыль, подлежащая уплате в бюджет субъекта РФ");
			Норма = "ст. 45 НК РФ";
			ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
		Иначе
			ОписаниеНарушения = "";
	...
	Для Каждого Страница1 Из Раздел5 Цикл
		...
		ПраваяЧасть = Окр(Страница1.Данные.П000050001004) - Окр(Страница1.Данные.П000050016004) - Окр(Страница1.Данные.П000050026004) - Окр(Страница1.Данные.П000050027004) - Окр(Страница1.Данные.П000050028004) - Окр(Страница1.Данные.П000050030004) - Окр(Страница1.Данные.П000050033004);
		ПраваяЧасть = Макс(ПраваяЧасть, 0);
		
		ЭтоОшибка = Не Окр(Страница1.Данные.П000050036004) = ПраваяЧасть;
		
		Расшифровка = "" + Окр(Страница1.Данные.П000050036004) + " = " + Окр(Страница1.Данные.П000050001004) + " - " + Окр(Страница1.Данные.П000050016004) + " - " + Окр(Страница1.Данные.П000050026004) + " - " + Окр(Страница1.Данные.П000050027004) + " - " + Окр(Страница1.Данные.П000050028004) + " - " + Окр(Страница1.Данные.П000050030004) + " - " + Окр(Страница1.Данные.П000050033004) + " Итог [" + ПраваяЧасть + "]";
		
		Если УсловиеВыполнено И ЭтоОшибка Тогда
			ОписаниеНарушения = ?(Окр(Страница1.Данные.П000050036004) > ПраваяЧасть, "Завышена налоговая база по налогу на прибыль, подлежащая уплате в местный бюджет", "Занижена налоговая база по налогу на прибыль, подлежащая уплате в местный бюджет");
			Норма = "ст. 45 НК РФ";
			ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
		Иначе
			ОписаниеНарушения = "";
...
Процедура СформироватьДанныеОтчета()
	...
	Раздел11 = ДанныеОтчета.ПоказателиОтчета.ПолеТабличногоДокументаРаздел1_1;
	Раздел11.Вставить("Страница", "1");
	
	Раздел12 = ДанныеОтчета.ПоказателиОтчета.ПолеТабличногоДокументаРаздел1_2;
	Раздел12.Вставить("Страница", "1");
	
	Раздел13 = ДанныеОтчета.ПоказателиОтчета.ПолеТабличногоДокументаРаздел1_3;
	Раздел13.Вставить("Страница", "1");
	
	ДанныеКС.Вставить("Раздел11", Раздел11);
	ДанныеКС.Вставить("Раздел12", Раздел12);
	ДанныеКС.Вставить("Раздел13", Раздел13);
...
Процедура СформироватьСлужебныеДанные()
	...
	МассивИсключений.Вставить("Р11С031", "Строка 030 Раздела 1.1, федеральный бюджет");
	МассивИсключений.Вставить("Р11С041", "Строка 040 Раздела 1.1, федеральный бюджет");
	МассивИсключений.Вставить("Р11С032", "Строка 030 Раздела 1.1, бюджет субъекта РФ");
	МассивИсключений.Вставить("Р11С042", "Строка 040 Раздела 1.1, бюджет субъекта РФ");
	МассивИсключений.Вставить("Р11С033", "Строка 030 Раздела 1.1, местный бюджет");
	МассивИсключений.Вставить("Р11С043", "Строка 040 Раздела 1.1, местный бюджет");
...
&НаСервере
Процедура ПроверкаКС_4_1_1_к(СтруктураРезультата)
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.1.1.к';
													|en = '4.1.1.к'"));
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма налога в федеральный бюджет в стр.030 Раздела 1.1 >= 0';
														|en = 'Сумма налога в федеральный бюджет в стр.030 Раздела 1.1 >= 0'"));
	...
	ИменаПоказателей.Вставить("Р11С031"		, "СуммаНалога1");
	...
	Настройки.ЭтоОшибка = Показатели.Р11С031.Значение < 0;
	Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, 
		Показатели.Р11С031.Значение);
	ДополнитьСтруктуруОшибок(Настройки, СтруктураРезультата);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_4_1_2_к(СтруктураРезультата)
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.1.2.к';
													|en = '4.1.2.к'"));
	Заголовки.Вставить("Комментарий"		, НСтр("ru = 'Критическая проверка';
													|en = 'Критическая проверка'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 1.1';
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@';
													|en = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма налога в федеральный бюджет в стр.040 Раздела 1.1 >= 0';
														|en = 'Сумма налога в федеральный бюджет в стр.040 Раздела 1.1 >= 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Возможно ошибочная сумма налога';
														|en = 'Возможно ошибочная сумма налога'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р11С041"		, "СуммаНалУм1");
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	ЗарегистрироватьФормулу(Настройки);
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Раздел11);
	
	Настройки.ЭтоОшибка = Показатели.Р11С041.Значение < 0;
	Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, 
		Показатели.Р11С041.Значение);
	ДополнитьСтруктуруОшибок(Настройки, СтруктураРезультата);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_4_1_3_к(СтруктураРезультата)
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.1.3.к';
													|en = '4.1.3.к'"));
	Заголовки.Вставить("Комментарий"		, НСтр("ru = 'Критическая проверка';
													|en = 'Критическая проверка'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 1.1';
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@';
													|en = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма налога в бюджет субъекта РФ в стр.030 Раздела 1.1 >= 0';
														|en = 'Сумма налога в бюджет субъекта РФ в стр.030 Раздела 1.1 >= 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Возможно ошибочная сумма налога';
														|en = 'Возможно ошибочная сумма налога'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р11С032"		, "СуммаНалога2");
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	ЗарегистрироватьФормулу(Настройки);
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Раздел11);
	
	Настройки.ЭтоОшибка = Показатели.Р11С032.Значение < 0;
	Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, 
		Показатели.Р11С032.Значение);
	ДополнитьСтруктуруОшибок(Настройки, СтруктураРезультата);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_4_1_4_к(СтруктураРезультата)
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.1.4.к';
													|en = '4.1.4.к'"));
	Заголовки.Вставить("Комментарий"		, НСтр("ru = 'Критическая проверка';
													|en = 'Критическая проверка'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 1.1';
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@';
													|en = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма налога в бюджет субъекта РФ в стр.040 Раздела 1.1 >= 0';
														|en = 'Сумма налога в бюджет субъекта РФ в стр.040 Раздела 1.1 >= 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Возможно ошибочная сумма налога';
														|en = 'Возможно ошибочная сумма налога'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р11С042"		, "СуммаНалУм2");
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	ЗарегистрироватьФормулу(Настройки);
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Раздел11);
	
	Настройки.ЭтоОшибка = Показатели.Р11С042.Значение < 0;
	Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, 
		Показатели.Р11С042.Значение);
	ДополнитьСтруктуруОшибок(Настройки, СтруктураРезультата);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_4_1_5_к(СтруктураРезультата)
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.1.5.к';
													|en = '4.1.5.к'"));
	Заголовки.Вставить("Комментарий"		, НСтр("ru = 'Критическая проверка';
													|en = 'Критическая проверка'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 1.1';
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@';
													|en = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма налога в местный бюджет в стр.030 Раздела 1.1 >= 0';
														|en = 'Сумма налога в местный бюджет в стр.030 Раздела 1.1 >= 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Возможно ошибочная сумма налога';
														|en = 'Возможно ошибочная сумма налога'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р11С033"		, "СуммаНалога3");
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	ЗарегистрироватьФормулу(Настройки);
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Раздел11);
	
	Настройки.ЭтоОшибка = Показатели.Р11С033.Значение < 0;
	Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, 
		Показатели.Р11С033.Значение);
	ДополнитьСтруктуруОшибок(Настройки, СтруктураРезультата);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_4_1_6_к(СтруктураРезультата)
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.1.6.к';
													|en = '4.1.6.к'"));
	Заголовки.Вставить("Комментарий"		, НСтр("ru = 'Критическая проверка';
													|en = 'Критическая проверка'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 1.1';
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@';
													|en = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма налога в местный бюджет в стр.040 Раздела 1.1 >= 0';
														|en = 'Сумма налога в местный бюджет в стр.040 Раздела 1.1 >= 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Возможно ошибочная сумма налога';
														|en = 'Возможно ошибочная сумма налога'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р11С043"		, "СуммаНалУм3");
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	ЗарегистрироватьФормулу(Настройки);
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Раздел11);
	
	Настройки.ЭтоОшибка = Показатели.Р11С043.Значение < 0;
	Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, 
		Показатели.Р11С043.Значение);
	ДополнитьСтруктуруОшибок(Настройки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверкаКС_4_2_1_к(СтруктураРезультата)
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.2.1.к';
													|en = '4.2.1.к'"));
	Заголовки.Вставить("Комментарий"		, НСтр("ru = 'Критическая проверка';
													|en = 'Критическая проверка'"));
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@';
													|en = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма налога в федеральный бюджет в стр.030 Раздела 1.2 >= 0';
														|en = 'Сумма налога в федеральный бюджет в стр.030 Раздела 1.2 >= 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Возможно ошибочная сумма налога';
														|en = 'Возможно ошибочная сумма налога'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р12С030"		, "СуммаДоплФед");
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	ЗарегистрироватьФормулу(Настройки);
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Раздел12);
	
	Настройки.ЭтоОшибка = Показатели.Р12С030.Значение < 0;
	Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, 
		Показатели.Р12С030.Значение);
	ДополнитьСтруктуруОшибок(Настройки, СтруктураРезультата);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_4_2_2_к(СтруктураРезультата)
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.2.2.к';
													|en = '4.2.2.к'"));
	Заголовки.Вставить("Комментарий"		, НСтр("ru = 'Критическая проверка';
													|en = 'Критическая проверка'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 1.2';
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@';
													|en = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма налога в федеральный бюджет в стр.040 Раздела 1.2 >= 0';
														|en = 'Сумма налога в федеральный бюджет в стр.040 Раздела 1.2 >= 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Возможно ошибочная сумма налога';
														|en = 'Возможно ошибочная сумма налога'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р12С040"		, "СуммаУменФед");
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	ЗарегистрироватьФормулу(Настройки);
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Раздел12);
	
	Настройки.ЭтоОшибка = Показатели.Р12С040.Значение < 0;
	Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, 
		Показатели.Р12С040.Значение);
	ДополнитьСтруктуруОшибок(Настройки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверкаКС_4_2_3_к(СтруктураРезультата)
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.2.3.к';
													|en = '4.2.3.к'"));
	Заголовки.Вставить("Комментарий"		, НСтр("ru = 'Критическая проверка';
													|en = 'Критическая проверка'"));
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@';
													|en = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма налога в федеральный бюджет в стр.030 Раздела 1.3 >= 0';
														|en = 'Сумма налога в федеральный бюджет в стр.030 Раздела 1.3 >= 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Возможно ошибочная сумма налога';
														|en = 'Возможно ошибочная сумма налога'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р13С030"		, "Сумма2ДоплФед");
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	ЗарегистрироватьФормулу(Настройки);
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Раздел13);
	
	Настройки.ЭтоОшибка = Показатели.Р13С030.Значение < 0;
	Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, 
		Показатели.Р13С030.Значение);
	ДополнитьСтруктуруОшибок(Настройки, СтруктураРезультата);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_4_2_4_к(СтруктураРезультата)
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.2.4.к';
													|en = '4.2.4.к'"));
	Заголовки.Вставить("Комментарий"		, НСтр("ru = 'Критическая проверка';
													|en = 'Критическая проверка'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 1.3';
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@';
													|en = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма налога в федеральный бюджет в стр.040 Раздела 1.3 >= 0';
														|en = 'Сумма налога в федеральный бюджет в стр.040 Раздела 1.3 >= 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Возможно ошибочная сумма налога';
														|en = 'Возможно ошибочная сумма налога'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р13С040"		, "Сумма2УменФед");
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	ЗарегистрироватьФормулу(Настройки);
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Раздел13);
	
	Настройки.ЭтоОшибка = Показатели.Р13С040.Значение < 0;
	Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, 
		Показатели.Р13С040.Значение);
	ДополнитьСтруктуруОшибок(Настройки, СтруктураРезультата);
