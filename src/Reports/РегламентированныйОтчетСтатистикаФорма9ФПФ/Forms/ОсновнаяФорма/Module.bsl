...
Процедура ПоказатьПериод(Форма)
	...
	Иначе
		...
		Если НачалоДня(Форма.мДатаНачалаПериодаОтчета) >= '20220101' Тогда
			Форма.ОписаниеНормативДок = "Форма утратила силу в соответствии с приказом Росстата от 10.09.2021 № 558.";
		Иначе
			Форма.ОписаниеНормативДок = "Отсутствует в программе.";
		КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(мДатаНачалаПериодаОтчета) И НЕ ЗначениеЗаполнено(мДатаКонцаПериодаОтчета) Тогда
		мДатаКонцаПериодаОтчета  = Макс(КонецКвартала(ДобавитьМесяц(ТекущаяДатаСеанса(), -3)), '20140331235959');
