...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы110 Тогда
		Подстрока = "рег. номер при ввозе";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный рег. номер при ввозе?';
							|en = 'Удалить выбранный рег. номер при ввозе?'");
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя)
				...
				Если ЗначениеЗаполнено(КодОперации) Тогда
					Ном = 1;
					Пока Форма.ТабличныйДокумент.Области.Найти("П1000004_" + Формат(Ном, "ЧГ=")) <> Неопределено Цикл
						СуммаНалБаз = СуммаНалБаз
							+ Форма.ТабличныйДокумент.Области["П1000004_" + Формат(Ном, "ЧГ=")].Значение;
						Ном = Ном + 1;
					КонецЦикла;
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если Ветка = АктивнаяВетка Тогда
				...
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					Ветка_П10000.Данные.Вставить("П1000004", Форма.ТабличныйДокумент.Области["П1000004_"
			...
			Если ЗначениеЗаполнено(КодОперации) Тогда
				ДанныеМнЧ_П10000 = Ветка.ДанныеМногострочныхЧастей.П10000.Строки;
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					СуммаНалБаз = СуммаНалБаз + Ветка_П10000.Данные["П1000004"];
				КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("НалБазаИтого", "КодОперации;П1000004");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрДТ_22");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRTD165-22"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRTD165-22.2"));
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				ДлинаРегНомТД = СтрДлина(СокрЛП(Ветка_П11000.Данные["П1100002"]));
				Если НЕ (ДлинаРегНомТД = 23 ИЛИ (ДлинаРегНомТД >= 25 И ДлинаРегНомТД <= 29)) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не" + ?(ДлинаРегНомТД = 0,
					"", "верно") + " задан регистрационный номер декларации на товары (при ввозе)."
			...
			Если НЕ (ДлинаРегНомТД = 23 ИЛИ (ДлинаРегНомТД >= 25 И ДлинаРегНомТД <= 29)) Тогда
				...
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не" + ?(ДлинаРегНомТД = 0,
				"", "верно") + " задан регистрационный номер декларации на товары (при реэкспорте)."
				+ " Возможное количество символов в номере: 23 или от 25 до 29", "Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200006"]) Тогда
					...
					СтатусПроверки = Ложь;
				ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П12000.Данные["П1200006"]) Тогда
					ИмяПок = "П1200006" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрДТ_22 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрДТ_22");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрДТ_22 = СкопироватьУзел(УзелРодитель.Родитель, РеестрДТ_22);
		Для каждого Эл Из ЭкзРеестрДТ_22.Строки Цикл
		...
		СведДТ = ПолучитьПодчиненныйЭлемент(ЭкзРеестрДТ_22, "СведДТ");
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведДТ = СкопироватьУзел(ЭкзРеестрДТ_22, СведДТ, ЭкзРеестрДТ_22.Строки.Количество() - 1);
				...
				// 11000.
				СведДТВ = ПолучитьПодчиненныйЭлемент(ЭкзСведДТ, "СведДТВ");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					
					ЭкзСведДТВ = СкопироватьУзел(ЭкзСведДТ, СведДТВ, ЭкзСведДТ.Строки.Количество() - 1);
					Для каждого Эл Из ЭкзСведДТВ.Строки Цикл
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_ДекларацииНаТоварыВвозРеэкспорт");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_ДекларацииНаТоварыВвозРеэкспорт_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
	...
	Если ВыбранныеСтраницы.Найти("Реестр") <> Неопределено Тогда
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				// Особая обработка для графы 2.
				МасТекЗнач = Новый Массив;
				Для каждого Строка11 Из Строки11 Цикл
					Если ЗначениеЗаполнено(Строка11.Данные.П1100002) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100002));
					КонецЕсли;
				КонецЦикла;
				СтрокаДокумент.Параметры.П1100002 = СтрСоединить(МасТекЗнач, "; ");
