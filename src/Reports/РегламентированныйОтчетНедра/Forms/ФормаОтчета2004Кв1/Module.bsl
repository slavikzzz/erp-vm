...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	ДанныеКС.Вставить("СчетчикФормулДляРасшифровок"	, 0);
	ДанныеКС.Вставить("КонтрольныеСоотношенияАдресФормул");
	ДанныеКС.Вставить("КонтрольныеСоотношенияАдресПоказателей");
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Расчет регулярных платежей за пользование недрами");
	СтруктураРезультата.Вставить("НаименованиеФормы"	, "2004Кв1");
...
Процедура СформироватьДанныеОтчета()
	...
	ИсходнаяСтруктура = СтруктураРеквизитовФормы.мСохраненныйДок.ДанныеОтчета.Получить();
	
	Раздел1 = ИсходнаяСтруктура.ДанныеМногостраничныхРазделов.Раздел1;
	...
	Для каждого СтраницаРаздела Из Раздел1 Цикл
		...
		ДанныеСтраницы.Вставить("Страница", XMLСтрока(Раздел1.Индекс(СтраницаРаздела) + 1));
		ДанныеСтраницы.Вставить("КолвоСтрок", КоличествоСтрок(ДанныеСтраницы, "П000010003000_")); 
		
		ДанныеРаздела1.Добавить(ДанныеСтраницы);
	
	КонецЦикла; 
	
	ДанныеКС.Вставить("Раздел1", ДанныеРаздела1);
	ДанныеКС.Вставить("КолвоСтраницРаздела1", ДанныеРаздела1.Количество());
...
Процедура РасшифроватьПоказатель(Показатель)
	...
	НоваяСтрока.Номер					= ДанныеКС.СчетчикФормулДляРасшифровок;
...
Процедура ЗарегистрироватьФормулу(Знач Настройки);
	...
	ДанныеКС.СчетчикФормулДляРасшифровок	= ДанныеКС.СчетчикФормулДляРасшифровок + 1;
	НовСтр									= СписокРасшифровкиФормулы.Добавить();
	НовСтр.ТекстовоеПредставлениеФормулы	= Настройки.ОписаниеКС;
	НовСтр.Номер							= ДанныеКС.СчетчикФормулДляРасшифровок;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.НаименованиеОтчета      = Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7);
...
Функция ВерсияФормата(ДатаПериода)
	
	Если ДатаПериода >= '2022-01-01' Тогда
		
		Возврат "3.00004";
		
	ИначеЕсли ДатаПериода >= '2013-10-01' Тогда
...
Процедура ЗагрузитьПоказателиИзМакета(РазделВМакете)
	...
	МакетСоставаПоказателей = ОбъектОтчета(ИмяФормы).ПолучитьМакет("Списки2004Кв1");
...
Процедура ВосстановитьСохраненныеДанные()
	...
	ОбработатьРеквизитыЛицензийНаПравоПользованияНедрами();
...
&НаСервере
Процедура ОбработатьРеквизитыЛицензийНаПравоПользованияНедрами()
	
	ВерсияФормата = ВерсияФормата(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	
	Если ВерсияФормата = "3.00004" Тогда
		
		Для каждого СтраницаРаздела Из ТаблицаСтраницыРаздел21 Цикл
			ДанныеСтраницы = СтраницаРаздела.Данные[0].Значение;
			Если ДанныеСтраницы.Свойство("П000210007000") Тогда
				СерНомВидЛиц = СокрЛП(ДанныеСтраницы["П000210007000"]);
				Если СтрДлина(СерНомВидЛиц) = 11 Тогда
					// Реквизиты лицензии - строка из 11 символов - соответствуют версии "3.00004".
					Прервать;
				ИначеЕсли СтрДлина(СерНомВидЛиц) = 10 Тогда
					// Реквизиты лицензии - строка из 10 символов - не соответствуют версии "3.00004", необходима конвертация.
					ДанныеСтраницы.Вставить("П000210007000",
					Лев(СерНомВидЛиц, 3) + "0" + Сред(СерНомВидЛиц, 4));// конвертация: добавление символа "0" к номеру слева
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		
		Для каждого СтраницаРаздела Из ТаблицаСтраницыРаздел22 Цикл
			ДанныеСтраницы = СтраницаРаздела.Данные[0].Значение;
			Если ДанныеСтраницы.Свойство("П000220005000") Тогда
				СерНомВидЛиц = СокрЛП(ДанныеСтраницы["П000220005000"]);
				Если СтрДлина(СерНомВидЛиц) = 11 Тогда
					// Реквизиты лицензии - строка из 11 символов - соответствуют версии "3.00004".
					Прервать;
				ИначеЕсли СтрДлина(СерНомВидЛиц) = 10 Тогда
					// Реквизиты лицензии - строка из 10 символов - не соответствуют версии "3.00004", необходима конвертация.
					ДанныеСтраницы.Вставить("П000220005000",
					Лев(СерНомВидЛиц, 3) + "0" + Сред(СерНомВидЛиц, 4));// конвертация: добавление символа "0" к номеру слева
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗаполнитьСведенияОбОрганизацииНаСервере(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	ОргСведения = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(СтруктураРеквизитовФормы.Организация, ДатаПодписи, СтрокаСведений);
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		СтруктураРеквизитовФормы.мТелОрганизации = ОргСведения.ТелОрганизации;
		...
		СтруктураРеквизитовФормы.мОргОГРН = ?(ТипНП = 3, "", ОргСведения.ОГРН);
		Раздел.Области.ОГРН.Значение = СтруктураРеквизитовФормы.мОргОГРН;
		
		ИспользуетсяОКТМО = (СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2013-10-01');
		КодТерриториальногоОбразования = ?(ИспользуетсяОКТМО, ОргСведения.ОКТМО, ОргСведения.ОКАТО);
				
		СтруктураРеквизитовФормы.мОКАТО = СокрЛП(КодТерриториальногоОбразования);
		
		СтруктураРеквизитовФормы.мОргКодИМНС = ОргСведения.КодНО;
...
Процедура СформироватьРаздел1()
	...
	ИначеЕсли НЕ ЕстьДанныеДляЗаполненияРаздела1 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'В Разделе 2.1 и в Разделе 2.2 не обнаружены данные для заполнения Раздела 1!';
								|en = 'В Разделе 2.1 и в Разделе 2.2 не обнаружены данные для заполнения Раздела 1!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		СтрПерваяСтраница = ТаблицаСтраницРаздел1.Добавить();
...
Процедура ОчиститьТекущуюСтраницу(Команда)
	...
	Если Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "ФизЛица" Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Очистка раздела со сведениями о физ. лицах запрещена. Эти сведения необходимы для корректной выгрузки данных отчета.';
								|en = 'Очистка раздела со сведениями о физ. лицах запрещена. Эти сведения необходимы для корректной выгрузки данных отчета.'");
		Сообщение.Сообщить();
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = Неопределено;
	
	Результат = РегламентированнаяОтчетностьСравнение.СравнениеОтчетов(
		ЭтотОбъект, Новый Структура, ДанныеДляСравнения, ДеревоДляСравнения);
		...
		Если НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ОргИНН, Истина) Тогда
			Если ВыводитьСообщения Тогда
				...
				Сообщение.Текст = НСтр("ru = 'Неверно задан ИНН индивидуального предпринимателя!';
										|en = 'Неверно задан ИНН индивидуального предпринимателя!'");
				Сообщение.Сообщить();
			КонецЕсли;
			СтатусПроверки = 1;
		КонецЕсли;
...
Процедура ВыгрузитьТитульныйЛистИСведенияОФизЛицах(Текст)
	...
	Текст.ДобавитьСтроку("ВерФОтч:" + ВерсияФормата);
	
	Если ВерсияФормата = "3.00004" Тогда
		
		Текст.ДобавитьСтроку("ПериодВерОтч:01.01.2022,");
		
	ИначеЕсли ВерсияФормата = "3.00003" Тогда
		
		Текст.ДобавитьСтроку("ПериодВерОтч:01.10.2013,31.12.2021");
...
Процедура ОбработатьРезультирующийТекстВыгрузки(Текст)
	...
	Для Сч = 1 По КолСтрок Цикл
		...
		Если Тек13Симв = "П000210007000" ИЛИ Тек13Симв = "П000220005000" Тогда
			...
			СерНомВидЛиц = Сред(ТекСтрока, 15);
			...
			Если ВерсияФормата = "3.00004" Тогда
				...
				Если СтрДлина(СерНомВидЛиц) = 11 Тогда
					...
					// Реквизиты лицензии - строка из 11 символов - соответствуют версии "3.00004".
					СерЛиц = Сред(ТекСтрока, 15, 3);
					НомЛиц = Сред(ТекСтрока, 18, 6);
					ВидЛиц = Сред(ТекСтрока, 24, 2);
					
				Иначе
					
					// Реквизиты лицензии - строка из 10 символов - не соответствуют версии "3.00004", необходима конвертация.
					СерЛиц = Сред(ТекСтрока, 15, 3);
					НомЛиц = "0" + Сред(ТекСтрока, 18, 5);// конвертация: добавление символа "0" к номеру слева
					ВидЛиц = Сред(ТекСтрока, 23, 2);
					
				КонецЕсли;
				
				Текст.ЗаменитьСтроку(ТекИнд, Тек13Симв + ":" + СерЛиц + "," + НомЛиц + "," + ВидЛиц);
				
			Иначе// "3.00003", "3.00002"
				
				// Реквизиты лицензии - строка из 10 символов - соответствуют версиям "3.00003", "3.00002".
				СерЛиц = Сред(ТекСтрока, 15, 3);
				НомЛиц = Сред(ТекСтрока, 18, 5);
				ВидЛиц = Сред(ТекСтрока, 23, 2);
...
Процедура ИнициализацияСоставаПоказателей()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ИмяФормы).ПолучитьМакет("СоставПоказателей2004Кв1");
	ОписаниеТиповСтрока100 = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(100));
	Для Инд = 0 По МакетСоставаПоказателей.Области.Количество() - 1 Цикл
		ТекОбласть    = МакетСоставаПоказателей.Области[Инд];
		ИмяСтраницы   = ТекОбласть.Имя;
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если (СтрЧислоВхождений(Область.Имя, "П000010002000") > 0
		...
		И ЗначениеЗаполнено(Область.Значение) И НЕ (ВерсияФормата = "3.00004" ИЛИ ВерсияФормата = "3.00003") Тогда
...
Процедура ПолеТабличногоДокументаРаздел21Выбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли (Область.Имя = "П000210007000") И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
		...
		ПараметрыФормы.Вставить("ВерсияФормата", ВерсияФормата(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
		
		Если СтрНачинаетсяС(ИмяФормы, "Внешний") Тогда
			ФормаВводаЛицензии
			= ПолучитьФорму("ВнешнийОтчет.РегламентированныйОтчетНедра.Форма.ФормаВводаЛицензии", ПараметрыФормы);
		Иначе
			ФормаВводаЛицензии
			= ПолучитьФорму("Отчет.РегламентированныйОтчетНедра.Форма.ФормаВводаЛицензии", ПараметрыФормы);
		КонецЕсли;
