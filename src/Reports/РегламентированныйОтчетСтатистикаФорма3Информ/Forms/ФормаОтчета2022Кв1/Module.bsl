...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы4, 3);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		ИначеЕсли Область.Имя = "П003001030104" Или Область.Имя = "П003001030204" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникУдовлетворенностьСкоростью");
		ИначеЕсли Область.Имя = "П005001043103" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникУдовлетворенностьКачествомУслуг");
		ИначеЕсли Лев(Область.Имя, 7) = "П008001" Или Лев(Область.Имя, 7) = "П010001" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникЦели");
		ИначеЕсли Область.Имя = "П009001071004" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникАнализСиламиСотрудников");
		ИначеЕсли Лев(Область.Имя, 7) = "П011001" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникПредоставлениеСервиса");
		ИначеЕсли Область.Имя = "П014001120703" Или Область.Имя = "П014001120704" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
		...
		ИначеЕсли Лев(Область.Имя, 7) = "П025001" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникВоздействиеЦифровыхТехнологий");
		КонецЕсли;
...
Функция ПолеДаНет(ИмяОбласти)
	...
	ОбластиДаНет = "П001001;П004001;П006001;П007001;П012001;П013001;П015001;П017001;П020001;П021001;П026001";
	МассивОбластейДаНет = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ОбластиДаНет, ";", Истина);
	
	ОбластиНомераСтрок = "1201;1202;1203;1204;1205;1206";
	...
	Если МассивОбластейДаНет.Найти(Лев(ИмяОбласти, 7)) <> Неопределено
	 ...
	 Или (Лев(ИмяОбласти, 6) = "П00013" И Сред(ИмяОбласти, 13, 1) = "4")
	 Или ИмяОбласти = "П009001070903" Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Стр = 204 По 207 Цикл
		СуммаПоСтр = СуммаПоСтр + Области["П0020010" + Стр + "03"].Значение;
	КонецЦикла;
	Области["П002001020103"].Значение = СуммаПоСтр;
	
	Области["П019001161403"].Значение
		= Области["П019001161503"].Значение
		+ Области["П019001161603"].Значение
		+ Области["П019001161703"].Значение
		+ Области["П019001161803"].Значение;
	
	Области["П022001180403"].Значение
		= Области["П022001180503"].Значение
		+ Области["П022001180903"].Значение
		+ Области["П022001181103"].Значение
		+ Области["П022001181203"].Значение
		+ Области["П022001181403"].Значение
		+ Области["П022001181503"].Значение;
	
	Области["П022001181603"].Значение
		= Области["П022001181703"].Значение
		+ Области["П022001181803"].Значение
		+ Области["П022001182203"].Значение
		+ Области["П022001182303"].Значение;
	
	Области["П022001180103"].Значение
		= Области["П022001180403"].Значение
		+ Области["П022001181603"].Значение;
	
	Области["П023001190303"].Значение
		= Области["П023001190403"].Значение
		+ Области["П023001190503"].Значение;
	
	Области["П023001190103"].Значение
		= Области["П023001190203"].Значение
		+ Области["П023001190303"].Значение
		+ Области["П023001190603"].Значение
		+ Области["П023001190703"].Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	ЯчейкиАвторасчета = "П002001020103;П019001161403;П022001180103;П022001180403;"
		+ "П022001181603;П023001190103;П023001190303;П000200200104;П000200200304";
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Если НЕ ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть12_4")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 3-информ");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
	
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть12_4"));
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть12_4.Низ + 1;
	...
	Если НЕ ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть12_5")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 3-информ");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть12_5"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 3-информ");
	...
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть13_3.Низ + 1;
	...
	Если НЕ ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть13_4")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 3-информ");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникВоздействиеЦифровыхТехнологий",
		512, , "Выбор кода", КоллекцияСписковВыбора["ВоздействиеЦифровыхТехнологий"]);
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Пока Истина Цикл
		Инд = Инд + 1;
		Обл = ПолеТабличногоДокументаФормаОтчета.Области.Найти("П000210210203_" + Инд);
		Если Обл = Неопределено Тогда
			Прервать;
		КонецЕсли;
		Если ЗначениеЗаполнено(Обл.Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД00210210203_" + Инд, Обл.Значение);
		КонецЕсли;
	КонецЦикла;
