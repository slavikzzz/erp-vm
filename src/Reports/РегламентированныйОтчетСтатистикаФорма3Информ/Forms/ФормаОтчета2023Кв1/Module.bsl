...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		ИначеЕсли Область.Имя = "П005001040803" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
		...
		ИначеЕсли Область.Имя = "П012001090703" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникПредоставлениеСервиса");
		ИначеЕсли Область.Имя = "П015001120703" Или Область.Имя = "П015001120704" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникЗаказыПоИнтернету");
		ИначеЕсли Лев(Область.Имя, 7) = "П026001" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникДаНетЗатрудняюсь");
...
Функция ПолеДаНет(ИмяОбласти)
	...
	ОбластиДаНет = "П001001;П004001;П006001;П007001;П010001;П011001;П013001;П014001;П016001;П018001;П021001;П022001";
	МассивОбластейДаНет = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ОбластиДаНет, ";", Истина);
	
	ОбластиНомераСтрок = "0709;1201;1202;1203;1204;1205;1206";
	...
	Если МассивОбластейДаНет.Найти(Лев(ИмяОбласти, 7)) <> Неопределено
	 ...
	 Или (Лев(ИмяОбласти, 6) = "П00013" И Сред(ИмяОбласти, 13, 1) = "4") Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 4 Цикл
		...
		СуммаПоСтр = 0;
		ФлагФорматНуля = Ложь;
		Для Стр = 205 По 208 Цикл
			ОбластьИмя ="П0020010" + Стр + Графа;
			СуммаПоСтр = СуммаПоСтр + Области[ОбластьИмя].Значение;
			Если Области[ОбластьИмя].Формат = "ЧН=0" Тогда
				ФлагФорматНуля = Истина;
			КонецЕсли;
		КонецЦикла;
		Области["П0020010201" + Графа].Значение = СуммаПоСтр;
		Области["П0020010201" + Графа].Формат = ?(ФлагФорматНуля, "ЧН=0", "");
	КонецЦикла;
	
	СуммаПоСтр = 0;
	ФлагФорматНуля = Ложь;
	Для НомСтр = 613 По 616 Цикл
		ОбластьИмя = "П0200011" + НомСтр + "03";
		СуммаПоСтр = СуммаПоСтр + Области[ОбластьИмя].Значение;
		Если Области[ОбластьИмя].Формат = "ЧН=0" Тогда
			ФлагФорматНуля = Истина;
		КонецЕсли;
	КонецЦикла;
	Области["П020001161203"].Значение = СуммаПоСтр;
	Области["П020001161203"].Формат = ?(ФлагФорматНуля, "ЧН=0", "");
	
	Области["П023001180403"].Значение
		= Области["П023001180503"].Значение
		+ Области["П023001180903"].Значение
		+ Области["П023001181103"].Значение
		+ Области["П023001181203"].Значение
		+ Области["П023001181403"].Значение
		+ Области["П023001181503"].Значение;
	
	Области["П023001181603"].Значение
		= Области["П023001181703"].Значение
		+ Области["П023001181803"].Значение
		+ Области["П023001182203"].Значение
		+ Области["П023001182303"].Значение;
	
	Области["П023001180103"].Значение
		= Области["П023001180403"].Значение
		+ Области["П023001181603"].Значение;
	
	Области["П024001190303"].Значение
		= Области["П024001190403"].Значение
		+ Области["П024001190503"].Значение;
	
	Области["П024001190103"].Значение
		= Области["П024001190203"].Значение
		+ Области["П024001190303"].Значение
		+ Области["П024001190603"].Значение
		+ Области["П024001190703"].Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	ЯчейкиАвторасчета = "П002001020103;П002001020104;П020001161203;П023001180103;П023001180403;"
		+ "П023001181603;П024001190103;П024001190303;П000200200104;П000200200304";
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	ТекущийЭлемент = Элементы[ИмяПоляТабличногоДокумента];
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть11_2.Низ + 1;
	НизМногострочнойЧасти = ТабличныйДокумент.Области.ДобавлениеСтроки1.Верх - 1;
	ОбластьОсновнаяЧасть11_3 = ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть11_3");
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверки.Добавить(ОбластьОсновнаяЧасть11_3);
	...
	КонецЦикла;
	ПечатнаяФорма.Вывести(ОбластьОсновнаяЧасть11_3);
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 3-информ");
	...
	Если НЕ ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть15_4")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 3-информ");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ПустаяСтрока"));
	КонецЕсли;
	...
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть15_4.Низ + 1;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникДаНетЗатрудняюсь",
		512, , "Выбор кода", КоллекцияСписковВыбора["ДаНетЗатрудняюсь"]);
