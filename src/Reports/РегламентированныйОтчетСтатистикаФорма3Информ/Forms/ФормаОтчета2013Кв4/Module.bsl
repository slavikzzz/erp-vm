...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "25/09/2013";
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Строки_434_435",        "П101010403;П101010703;П101010803;П101010903;П101011003;П101011103;П101011203");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Строки_436_443_446_452","П101011203");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Строка_901_902",        "П208080103;П208080203;П208080303;П208080403;П208080503;П208080603;П208080703;П208080803;П208080903;П208081003");
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Если ИмяТекущейСтраницыПанели = "Раздел1" Тогда
		...
		// Установка цвета и доступности строки 115 для ввода.
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строки_115, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Форма.ТабличныйДокумент.Области.П101010703.Значение = 1
			Или Форма.ТабличныйДокумент.Области.П101010803.Значение = 1
			Или Форма.ТабличныйДокумент.Области.П101011003.Значение = 1;
			УстановитьЦветИДоступностьЯчейки(Форма, "П101011503", УсловиеДоступа);
		КонецЕсли;
		
		// Установка цвета и доступности строк 118-119 для ввода.
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строки_118_119, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Форма.ТабличныйДокумент.Области.П101010703.Значение = 1
			Или Форма.ТабличныйДокумент.Области.П101010803.Значение = 1
			Или Форма.ТабличныйДокумент.Области.П101011003.Значение = 1;
			Для КодСтроки = 118 По 119 Цикл
				ИмяЯчейки = "П1010" + Строка(КодСтроки) + "03";
				УстановитьЦветИДоступностьЯчейки(Форма, ИмяЯчейки, УсловиеДоступа);
			КонецЦикла;
		КонецЕсли;
		
		// Установка цвета и доступности строк 301-306 для ввода.
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строки_301_306, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Форма.ТабличныйДокумент.Области.П101010103.Значение = 1;
			Для КодСтроки = 301 По 306 Цикл
				ИмяЯчейки = "П1030" + Строка(КодСтроки) + "03";
				УстановитьЦветИДоступностьЯчейки(Форма, ИмяЯчейки, УсловиеДоступа);
			КонецЦикла;
		КонецЕсли;
		
		// Установка цвета и доступности строк 307-313 для ввода.
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строки_307_313, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Форма.ТабличныйДокумент.Области.П101010703.Значение = 1;
			Для КодСтроки = 307 По 313 Цикл
				ИмяЯчейки = "П1030" + Строка(КодСтроки) + "03";
				УстановитьЦветИДоступностьЯчейки(Форма, ИмяЯчейки, УсловиеДоступа);
			КонецЦикла;
		КонецЕсли;
		
		// Установка цвета и доступности строк 314-328 для ввода.
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строки_314_328, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Форма.ТабличныйДокумент.Области.П101010103.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101010303.Значение = 1;
			Для КодСтроки = 314 По 328 Цикл
				ИмяЯчейки = "П1030" + Строка(КодСтроки) + "03";
				УстановитьЦветИДоступностьЯчейки(Форма, ИмяЯчейки, УсловиеДоступа);
			КонецЦикла;
		КонецЕсли;
		
		// Установка цвета и доступности строк 329-340 для ввода.
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строки_329_340, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Форма.ТабличныйДокумент.Области.П101010103.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101010303.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101010703.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101010803.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101010903.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101011003.Значение = 1;
			Для КодСтроки = 329 По 340 Цикл
				ИмяЯчейки = "П1030" + Строка(КодСтроки) + "03";
				УстановитьЦветИДоступностьЯчейки(Форма, ИмяЯчейки, УсловиеДоступа);
			КонецЦикла;
		КонецЕсли;
		
		// Установка цвета и доступности строк 401-432 для ввода.
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строки_401_432, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Форма.ТабличныйДокумент.Области.П101010703.Значение = 1;
			Для КодСтроки = 401 По 432 Цикл
				ИмяЯчейки = "П1040" + Строка(КодСтроки) + "03";
				УстановитьЦветИДоступностьЯчейки(Форма, ИмяЯчейки, УсловиеДоступа);
			КонецЦикла;
		КонецЕсли;
		
		// Установка цвета и доступности строки 433 для ввода.
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строка_433, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Ложь;
			Для КодСтроки = 419 По 432 Цикл
				ИмяЯчейки = "П1040" + Строка(КодСтроки) + "03";
				Если Форма.ТабличныйДокумент.Область(ИмяЯчейки).Значение = 1 Тогда
					УсловиеДоступа = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			УстановитьЦветИДоступностьЯчейки(Форма, "П104043303", УсловиеДоступа);
		КонецЕсли;
		
		// Установка цвета и доступности строк 434-435 для ввода.
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строки_434_435, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Форма.ТабличныйДокумент.Области.П101010403.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101010703.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101010803.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101010903.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101011003.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101011103.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101011203.Значение = 1;
			Для КодСтроки = 434 По 435 Цикл
				ИмяЯчейки = "П1040" + Строка(КодСтроки) + "03";
				УстановитьЦветИДоступностьЯчейки(Форма, ИмяЯчейки, УсловиеДоступа);
			КонецЦикла;
		КонецЕсли;
		
		// Установка цвета и доступности строк 436-443 и 446-449 для ввода.
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строки_436_443_446_452, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Форма.ТабличныйДокумент.Области.П101011203.Значение = 1;
			Для КодСтроки = 436 По 443 Цикл
				ИмяЯчейки = "П1040" + Строка(КодСтроки) + "03";
				УстановитьЦветИДоступностьЯчейки(Форма, ИмяЯчейки, УсловиеДоступа);
			КонецЦикла;
			Для КодСтроки = 446 По 452 Цикл
				ИмяЯчейки = "П1040" + Строка(КодСтроки) + "03";
				УстановитьЦветИДоступностьЯчейки(Форма, ИмяЯчейки, УсловиеДоступа);
			КонецЦикла;
		КонецЕсли;
		
		Форма.ТабличныйДокумент.Области.П105050103.Значение = Форма.ТабличныйДокумент.Области.П105050203.Значение
		+ Форма.ТабличныйДокумент.Области.П105050303.Значение
		+ Форма.ТабличныйДокумент.Области.П105050403.Значение
		+ Форма.ТабличныйДокумент.Области.П105050603.Значение
		+ Форма.ТабличныйДокумент.Области.П105050703.Значение
		+ Форма.ТабличныйДокумент.Области.П105050803.Значение;
		
		Форма.ТабличныйДокумент.Области.П106060204.Значение = Форма.ТабличныйДокумент.Области.П106060304.Значение
		+ Форма.ТабличныйДокумент.Области.П106060404.Значение
		+ Форма.ТабличныйДокумент.Области.П106060504.Значение
		+ Форма.ТабличныйДокумент.Области.П106060604.Значение
		+ Форма.ТабличныйДокумент.Области.П106060704.Значение
		+ Форма.ТабличныйДокумент.Области.П106060804.Значение
		+ Форма.ТабличныйДокумент.Области.П106060904.Значение
		+ Форма.ТабличныйДокумент.Области.П106061004.Значение
		+ Форма.ТабличныйДокумент.Области.П106061104.Значение;
		
	ИначеЕсли ИмяТекущейСтраницыПанели = "Раздел2" Тогда
		
		Форма.ТабличныйДокумент.Области.П208080703.Значение = Форма.ТабличныйДокумент.Области.П208080803.Значение
		+ Форма.ТабличныйДокумент.Области.П208080903.Значение
		+ Форма.ТабличныйДокумент.Области.П208081003.Значение;
		
		// Установка цвета и доступности строки 901-902 для ввода.
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строка_901_902, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Ложь;
			Для КодСтроки = 801 По 810 Цикл
				ИмяЯчейки = "П2080" + Строка(КодСтроки) + "03";
				Если ЗначениеЗаполнено(Форма.ТабличныйДокумент.Область(ИмяЯчейки).Значение) Тогда
					УсловиеДоступа = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Для КодСтроки = 901 По 902 Цикл
				ИмяЯчейки = "П2090" + Строка(КодСтроки) + "03";
				УстановитьЦветИДоступностьЯчейки(Форма, ИмяЯчейки, УсловиеДоступа);
			КонецЦикла;
		КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьЦветИДоступностьЯчейки(Форма, ИмяОбласти, УсловиеДоступности = Истина)
	
	Если УсловиеДоступности Тогда
		Форма.ТабличныйДокумент.Область(ИмяОбласти).ЦветФона = Форма.СтруктураРеквизитовФормы.мЦветЖелтый;
		Форма.ТабличныйДокумент.Область(ИмяОбласти).Защита = Ложь;
	Иначе
		Форма.ТабличныйДокумент.Область(ИмяОбласти).Значение = 0;
		Форма.ТабличныйДокумент.Область(ИмяОбласти).ЦветФона = Форма.СтруктураРеквизитовФормы.мЦветБелый;
		Форма.ТабличныйДокумент.Область(ИмяОбласти).Защита = Истина;
	КонецЕсли;
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Титульный", "ФормаОтчета2013Кв4_Титульный");
	СоответствиеРазделовМакетам.Вставить("Раздел1",   "ФормаОтчета2013Кв4_Раздел1");
	СоответствиеРазделовМакетам.Вставить("Раздел2",   "ФормаОтчета2013Кв4_Раздел2");
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1" Тогда
		...
		Если (Часть1Имени = "П101" И Часть2Имени >= "101" И Часть2Имени <= "115" И Часть3Имени = "03") // номера строк 101-115
			ИЛИ (Часть1Имени = "П103" И Часть2Имени >= "307" И Часть2Имени <= "312" И Часть3Имени = "03") // номера строк 307-312
			ИЛИ (Часть1Имени = "П103" И Часть2Имени >= "314" И Часть2Имени <= "340" И Часть3Имени = "03") // номера строк 314-340
			ИЛИ (Часть1Имени = "П104" И Часть2Имени >= "401" И Часть2Имени <= "432" И Часть3Имени = "03") // номера строк 401-432
			ИЛИ (Часть1Имени = "П104" И Часть2Имени >= "434" И Часть2Имени <= "443" И Часть3Имени = "03") // номера строк 434-443
		...
		ИначеЕсли (Часть1Имени = "П101" И Часть2Имени >= "118" И Часть2Имени <= "119" И Часть3Имени = "03") Тогда // номера строк 118-119
		...
		ИначеЕсли Часть1Имени = "П102" И Часть2Имени >= "201" И Часть2Имени <= "205" И Часть3Имени = "03" Тогда // номера строк 201-205
			
			СтандартнаяОбработка = Ложь;
			
			Список = Новый СписокЗначений;
			Список.Добавить(0,"Пустое значение");
			Список.Добавить(1,"1 - незначительная или малосущественная");
			Список.Добавить(2,"2 - значительная");
			Список.Добавить(3,"3 - основная или решающая");
		...
		ИначеЕсли ОбластьИмя = "П103031303" Тогда
			...
			Список.Добавить(4,"4 - 2.0-10.0 Мбит/сек");
			Список.Добавить(5,"5 - 10.1-30.0 Мбит/сек");
			Список.Добавить(6,"6 - 30.1-100.0 Мбит/сек");
			Список.Добавить(7,"7 - выше 100 Мбит/сек");
		...
		ИначеЕсли ОбластьИмя = "П104043303" Тогда
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревоДляЗагрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2013Кв4");
...
Функция ПроверитьПоказатели() Экспорт
	...
	РегламентированнаяОтчетность.СформироватьСтруктуруПоказателей(ЭтаФорма);
	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2013Кв4");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если НЕ РегламентированнаяОтчетность.ПроверитьРазделыНаНаличиеИнформации(ЭтаФорма, ДеревоВыгрузки, "ПолеТабличногоДокументаРаздел1") Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнен раздел I!", "Раздел1", "");
		СтатусПроверки = Ложь;
		Возврат СтатусПроверки;
	КонецЕсли;
	
	// КОНТРОЛЬ показателей формы (Указания по заполнению Формы 3-информ).
	
	// Раздел I.
	
	// Подраздел 1.
	// Стр. 101 - 114 гр. 3 должны быть оценены одним из кодов «1» или «2» (т.е. стр. 101-114 гр. 3 = «1» или «2»).
	Для НомСтр = 101 По 114 Цикл
		Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П101" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в разделе I значение в стр. " + Формат(НомСтр, "ЧГ=") + " по гр. 3 должно быть равно 1 или 2!",
			"Раздел1", "П101" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Стр. 115 гр. = «1» или «2», при хотя бы одной из стр. 107, 108, 110 гр.3 = 1.
	Если (ПараметрыВыгрузки["П101010703_Ч"] = 1 Или ПараметрыВыгрузки["П101010803_Ч"] = 1 Или ПараметрыВыгрузки["П101011003_Ч"] = 1)
		И Не ЗначениеЗаполнено(ПараметрыВыгрузки["П101011503_Ч"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если в разделе I значение хотя бы в одной из стр. 107, 108, 110 по гр.3 равно 1, то значение в стр. 115 по гр. 3 должно быть равно 1 или 2!",
		"Раздел1", "П101011503");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если стр. 102 гр. 3 = 1, то стр. 101 гр. 3 = 1.
	Если ПараметрыВыгрузки["П101010203_Ч"] = 1 И Не ПараметрыВыгрузки["П101010103_Ч"] = 1 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если в разделе I значение в стр. 102 по гр.3 равно 1, то значение в стр. 101 по гр. 3 должно быть равно 1!",
		"Раздел1", "П101010103");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если стр. 105 гр. 3 = 1, то стр. 104 гр.3 = 1.
	Если ПараметрыВыгрузки["П101010503_Ч"] = 1 И Не ПараметрыВыгрузки["П101010403_Ч"] = 1 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если в разделе I значение в стр. 105 по гр.3 равно 1, то значение в стр. 104 по гр. 3 должно быть равно 1!",
		"Раздел1", "П101010403");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если одновременно стр. 101, 103 гр.3 = 2, то ни в одной из стр. 104 - 111,114-115 не может быть указан код 1.
	Если ПараметрыВыгрузки["П101010103_Ч"] = 2 И ПараметрыВыгрузки["П101010303_Ч"] = 2 Тогда
		Для НомСтр = 104 По 115 Цикл
			Если НомСтр = 112 Или НомСтр = 113 Тогда
				Продолжить;
			КонецЕсли;
			Если ПараметрыВыгрузки["П101" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"] = 1 Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе I значения в стр. 101, 103 по гр. 3 одновременно равны 2, то в стр. " + Формат(НомСтр, "ЧГ=") + " по гр. 3 значение не должно быть равно 1!",
				"Раздел1", "П101" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	// Если стр. 111 гр.3 = 1, то одна из строк 104, 107, 108, 109 гр. 3 = 1.
	Если ПараметрыВыгрузки["П101011103_Ч"] = 1 И Не (ПараметрыВыгрузки["П101010403_Ч"] = 1
		Или ПараметрыВыгрузки["П101010403_Ч"] = 1
		Или ПараметрыВыгрузки["П101010403_Ч"] = 1
		Или ПараметрыВыгрузки["П101010403_Ч"] = 1) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если в разделе I значение в стр. 111 по гр. 3 равно 1, то в одной из строк 104, 107, 108, 109 по гр. 3 значение должно быть равно 1!",
		"Раздел1", "П101011103");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	// Справка 1.
	// Стр. 116 >= стр. 117.
	Если Не (ПараметрыВыгрузки["П101011603_Ч"] >= ПараметрыВыгрузки["П101011703_Ч"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: в разделе I значение в стр. 116 должно быть больше или равно значению в стр. 117!",
		"Раздел1", "П101011603");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// Справка 2.
	// Стр. 118-119 = «1» или «2», или «3», или «4», или «5», или «6» при хотя бы в одной из стр. 107, 108, 110 гр. 3 = 1.
	Если (ПараметрыВыгрузки["П101010703_Ч"] = 1 Или ПараметрыВыгрузки["П101010803_Ч"] = 1 Или ПараметрыВыгрузки["П101011003_Ч"] = 1) Тогда
		Для НомСтр = 118 По 119 Цикл
			Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П101" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе I хотя бы в одной из стр. 107, 108, 110 по гр.3 значение равно 1, то значение в стр. " + Формат(НомСтр, "ЧГ=") + " должно быть равно 1, 2, 3, 4, 5 или 6!",
				"Раздел1", "П101" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	// Если стр. 119 оценена одним из кодов 1,2,3,4,5,6, то стр. 116 > 0 справки 1.
	Если ЗначениеЗаполнено(ПараметрыВыгрузки["П101011903_Ч"]) И Не ПараметрыВыгрузки["П101011603_Ч"] > 0 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если в разделе I значение в стр. 119 равно 1, 2, 3, 4, 5 или 6, то в стр. 116 значение должно быть больше 0!",
		"Раздел1", "П101011603");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// Подраздел 2.
	// Стр. 201-205 гр.3 оцениваются одним из кодов «1» или «2», или «3», или «4».
	Для НомСтр = 201 По 205 Цикл
		Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П102" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в разделе I значение в стр. " + Формат(НомСтр, "ЧГ=") + " по гр. 3 должно быть равно 1, 2, 3 или 4!",
			"Раздел1", "П102" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	
	// Подраздел 3.
	// Стр. 301-306 гр.3 заполняются при стр. 101 гр. 3 = 1.
	Если ПараметрыВыгрузки["П101010103_Ч"] = 1 Тогда
		Для НомСтр = 301 По 306 Цикл
			Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе I значение в стр. 101 по гр. 3 равно 1, то должна быть заполнена стр. " + Формат(НомСтр, "ЧГ=") + " по гр. 3!",
				"Раздел1", "П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	// Стр. 301 гр. 3 >= стр. 302 гр.3;
	// стр. 301 гр. 3 >= стр. 303 гр.3;
	// стр. 301 гр. 3 >= стр. 304 гр.3;
	// стр. 301 гр. 3 >= стр. 305 гр.3;
	// стр. 301 гр. 3 >= стр. 306 гр.3.
	Для НомСтр = 302 По 306 Цикл
		Если Не (ПараметрыВыгрузки["П103030103_Ч"] >= ПараметрыВыгрузки["П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в разделе I значение в стр. 301 по гр. 3 должно быть больше или равно значению в стр. " + Формат(НомСтр, "ЧГ=") + " по гр. 3!",
			"Раздел1", "П103030103");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Стр. 304 гр. 3 >= стр. 305 гр.3.
	Если Не (ПараметрыВыгрузки["П103030403_Ч"] >= ПараметрыВыгрузки["П103030503_Ч"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: в разделе I значение в стр. 304 по гр. 3 должно быть больше или равно значению в стр. 305 по гр. 3!",
		"Раздел1", "П103030403");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если стр. 303 гр. 3 > 0, то стр. 104 гр. 3 = 1.
	Если ПараметрыВыгрузки["П103030303_Ч"] > 0 И Не (ПараметрыВыгрузки["П101010403_Ч"] = 1) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если в разделе I значение в стр. 303 по гр. 3 больше 0, то значение в стр. 104 по гр. 3 должно быть равно 1!",
		"Раздел1", "П101010403");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если стр. 304 гр. 3 > 0, то хотя бы одна из стр. 107-110 гр. 3 = 1.
	Если ПараметрыВыгрузки["П103030403_Ч"] > 0 И Не (ПараметрыВыгрузки["П101010703_Ч"] = 1
		Или ПараметрыВыгрузки["П101010803_Ч"] = 1
		Или ПараметрыВыгрузки["П101010903_Ч"] = 1
		Или ПараметрыВыгрузки["П101011003_Ч"] = 1) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если в разделе I значение в стр. 304 по гр. 3 больше 0, то хотя бы в одной из стр. 107-110 значение должно быть равно 1!",
		"Раздел1", "П103030403");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если стр. 305 гр. 3 > 0, то стр. 107 гр. 3 = 1.
	Если ПараметрыВыгрузки["П103030503_Ч"] > 0 И Не (ПараметрыВыгрузки["П101010703_Ч"] = 1) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если в разделе I значение в стр. 305 по гр. 3 больше 0, то значение в стр. 107 по гр. 3 должно быть равно 1!",
		"Раздел1", "П103030503");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если стр. 306 гр. 3 > 0, то стр. 102 гр. 3 = 1.
	Если ПараметрыВыгрузки["П103030603_Ч"] > 0 И Не (ПараметрыВыгрузки["П101010203_Ч"] = 1) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если в разделе I значение в стр. 306 по гр. 3 больше 0, то значение в стр. 102 по гр. 3 должно быть равно 1!",
		"Раздел1", "П103030603");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// Справка 3.
	// Стр. 307-312 гр.3 = «1» или «2», при стр. 107 гр.3 = 1.
	Если ПараметрыВыгрузки["П101010703_Ч"] = 1 Тогда
		Для НомСтр = 307 По 312 Цикл
			Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе I значение в стр. 107 по гр. 3 равно 1, то значение в стр. " + Формат(НомСтр, "ЧГ=") + " по гр. 3 должно быть равно 1 или 2!",
				"Раздел1", "П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	// Если стр. 107 гр.3 = 2, то стр. 307-313 не заполняются.
	Если ПараметрыВыгрузки["П101010703_Ч"] = 2 Тогда
		Для НомСтр = 307 По 313 Цикл
			Если ЗначениеЗаполнено(ПараметрыВыгрузки["П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе I значение в стр. 107 по гр. 3 равно 2, то значение в стр. " + Формат(НомСтр, "ЧГ=") + " по гр. 3 не заполняется!",
				"Раздел1", "П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	// Если стр. 312 гр.3 = 1, то стр. 311 гр. 3 = 1.
	Если ПараметрыВыгрузки["П103031203_Ч"] = 1 И Не (ПараметрыВыгрузки["П103031103_Ч"] = 1) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если в разделе I значение в стр. 312 по гр. 3 равно 1, то значение в стр. 311 по гр. 3 должно быть равно 1!",
		"Раздел1", "П103031103");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если стр. 311 гр.3 = 2, то стр. 312 гр.3 = 2.
	Если ПараметрыВыгрузки["П103031103_Ч"] = 2 И Не (ПараметрыВыгрузки["П103031203_Ч"] = 2) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если в разделе I значение в стр. 311 по гр. 3 равно 2, то значение в стр. 312 по гр. 3 должно быть равно 2!",
		"Раздел1", "П103031203");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если стр. 107 гр.3 = 1, то стр. 313 гр.3 должна быть заполнена одним из значений (кодов) 1,2,3,4,5,6,7.
	Если ПараметрыВыгрузки["П101010703_Ч"] = 1 И Не ЗначениеЗаполнено(ПараметрыВыгрузки["П103031303_Ч"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если в разделе I значение в стр. 107 по гр. 3 равно 1, то значение в стр. 313 по гр. 3 должно быть равно одному из значений: 1, 2, 3, 4, 5, 6, 7!",
		"Раздел1", "П103031303");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если стр. 313 оценена одним из кодов 1, 2, 3, 4, 5, 6, 7, то стр. 107 гр.3 = 1.
	Если ЗначениеЗаполнено(ПараметрыВыгрузки["П103031303_Ч"]) И Не ПараметрыВыгрузки["П101010703_Ч"] = 1 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если в разделе I стр. 313 по гр. 3 заполнена одним из значений: 1, 2, 3, 4, 5, 6, 7, то значение в стр. 107 по гр. 3 должно быть равно 1!",
		"Раздел1", "П101010703");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если хотя бы одна из стр. 307-312 гр.3 = 1, то в стр. 313 гр.3 должен быть указан один из кодов 1, 2, 3, 4, 5, 6, 7.
	Для НомСтр = 307 По 312 Цикл
		Если ПараметрыВыгрузки["П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"] = 1 Тогда
			Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П103031303_Ч"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе I значение хотя бы одной из стр. 307-312 по гр. 3 равно 1, то значение в стр. 313 по гр. 3 должно быть равно одному из значений: 1, 2, 3, 4, 5, 6, 7!",
				"Раздел1", "П103031303");
				СтатусПроверки = Ложь;
			КонецЕсли;
			Прервать;
	...
	// Справка 4.
	// Стр. 314-328 гр.3 = «1» или «2», при стр. 101 и /или 103 гр.3 = 1.
	Если ПараметрыВыгрузки["П101010103_Ч"] = 1 Или ПараметрыВыгрузки["П101010303_Ч"] = 1 Тогда
		Для НомСтр = 314 По 328 Цикл
			Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе I значение в стр. 101 и/или 103 по гр. 3 равно 1, то значение в стр. " + Формат(НомСтр, "ЧГ=") + " по гр. 3 должно быть равно 1 или 2!",
				"Раздел1", "П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЦикла;
	...
	// Справка 5.
	// Стр. 329-340 гр.3 = «1» или «2», при хотя бы одной из стр. 101, 103, 107-110 гр.3 = 1.
	Если ПараметрыВыгрузки["П101010103_Ч"] = 1 Или ПараметрыВыгрузки["П101010303_Ч"] = 1
		Или ПараметрыВыгрузки["П101010703_Ч"] = 1 Или ПараметрыВыгрузки["П101010803_Ч"] = 1
		Или ПараметрыВыгрузки["П101010903_Ч"] = 1 Или ПараметрыВыгрузки["П101011003_Ч"] = 1 Тогда
		Для НомСтр = 329 По 340 Цикл
			Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе I значение хотя бы одной из стр. 101, 103, 107-110 по гр. 3 равно 1, то значение в стр. " + Формат(НомСтр, "ЧГ=") + " по гр. 3 должно быть равно 1 или 2!",
				"Раздел1", "П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЦикла;
	...
	// Справка 6.
	// Стр. 341< = стр. 305 гр.3.
	Если Не (ПараметрыВыгрузки["П103034103_Ч"] <= ПараметрыВыгрузки["П103030503_Ч"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: в разделе I значение в стр. 341 по гр. 3 должно быть меньше или равно значению в стр. 305 по гр. 3!",
		"Раздел1", "П103034103");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	// Справка 7.
	// Стр. 342 >= 0.
	//  Реализация: установлен флажок "Неотрицательное" в свойствах ячейки с показателем "П103034203".
	
	// Подраздел 4.
	// Стр. 401 - 432 гр.3 = «1» или «2», при стр. 107 гр.3 = 1.
	Если ПараметрыВыгрузки["П101010703_Ч"] = 1 Тогда
		Для НомСтр = 401 По 432 Цикл
			Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П104" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе I значение в стр. 107 по гр. 3 равно 1, то значение в стр. " + Формат(НомСтр, "ЧГ=") + " по гр. 3 должно быть равно 1 или 2!",
				"Раздел1", "П104" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	// Стр. 433 гр.3 = 1 или 2, или 3, или 4, при хотя бы одной из стр. 419 - 432 гр. 3 = 1.
	Для НомСтр = 419 По 432 Цикл
		Если ПараметрыВыгрузки["П104" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"] = 1 Тогда
			Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П104043303_Ч"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе I значение хотя бы одной из стр. 419-432 по гр. 3 равно 1, то значение в стр. 433 по гр. 3 должно быть равно одному из значений: 1, 2, 3, 4!",
				"Раздел1", "П104043303");
				СтатусПроверки = Ложь;
			КонецЕсли;
			Прервать;
	...
	// Справка 8.
	// Стр. 434 - 435 = «1» или «2», при хотя бы  одной из стр. 104, 107-112 гр. 3 = 1.
	Если ПараметрыВыгрузки["П101010103_Ч"] = 1 Или ПараметрыВыгрузки["П101010303_Ч"] = 1
		Или ПараметрыВыгрузки["П101010703_Ч"] = 1 Или ПараметрыВыгрузки["П101010803_Ч"] = 1
		Или ПараметрыВыгрузки["П101010903_Ч"] = 1 Или ПараметрыВыгрузки["П101011003_Ч"] = 1
		Или ПараметрыВыгрузки["П101011103_Ч"] = 1 Или ПараметрыВыгрузки["П101011203_Ч"] = 1 Тогда
		Для НомСтр = 434 По 435 Цикл
			Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П104" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если в разделе I значение хотя бы одной из стр. 104, 107-112 по гр. 3 равно 1, то значение в стр. " + Формат(НомСтр, "ЧГ=") + " по гр. 3 должно быть равно 1 или 2!",
				"Раздел1", "П104" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЦикла;
	...
	// Справка 9.
	// Стр. 436 - 443 гр.3 = «1» или «2», при стр. 112 гр.3 = 1 у организаций, относящихся к коду 75.11 по ОКВЭД.
	//  Не реализовано: данное указание содержится непосредственно в заголовке Справки 9.
	
	// Справка 10.
	// Стр. 444 - 445 гр. 3, 4 заполняют организации, относящиеся к коду 75.11 по ОКВЭД.
	//  Не реализовано: данное указание содержится непосредственно в заголовке Справки 10.
	// Стр. 444 гр. 3, 4 >= стр. 445 гр. 3,4.
	Если Не (ПараметрыВыгрузки["П104044403_Ч"] >= ПараметрыВыгрузки["П104044503_Ч"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: в разделе I значение в стр. 444 по гр. 3 должно быть больше или равно значению в стр. 445 по гр. 3!",
		"Раздел1", "П104044403");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Не (ПараметрыВыгрузки["П104044404_Ч"] >= ПараметрыВыгрузки["П104044504_Ч"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: в разделе I значение в стр. 444 по гр. 4 должно быть больше или равно значению в стр. 445 по гр. 4!",
		"Раздел1", "П104044404");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// Справка 11.
	// Стр. 446 - 452 гр.3 = «1» или «2», при стр. 112 гр.3 = 1 у организаций,
	// относящихся к следующим разделам по ОКВЭД: C,D,E,F,G,H,I,K.
	//  Не реализовано: данное указание содержится непосредственно в заголовке Справки 11.
	
	// Подраздел 5.
	// Стр. 504 гр.3 > = стр. 505 гр.3.
	Если Не (ПараметрыВыгрузки["П105050403_Ч"] >= ПараметрыВыгрузки["П105050503_Ч"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: в разделе I значение в стр. 504 по гр. 3 должно быть больше или равно значению в стр. 505 по гр. 3!",
		"Раздел1", "П105050403");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// Подраздел 6.
	// Стр. 601 гр.4 > = стр. 602 гр.4.
	Если Не (ПараметрыВыгрузки["П106060104_Ч"] >= ПараметрыВыгрузки["П106060204_Ч"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: в разделе I значение в стр. 601 по гр. 4 должно быть больше или равно значению в стр. 602 по гр. 4!",
		"Раздел1", "П106060104");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Каждая из стр. 612 - 615 гр.4 <= стр. 601 гр.4.
	Для НомСтр = 612 По 615 Цикл
		Если Не (ПараметрыВыгрузки["П106" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04_Ч"] <= ПараметрыВыгрузки["П106060104_Ч"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в разделе I значение в стр. " + Формат(НомСтр, "ЧГ=") + " по гр. 4 должно быть меньше или равно значению в стр. 601 по гр. 4!",
			"Раздел1", "П106" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	
	// Подраздел 7.
	// Стр. 701 -702 гр. 3, 4 = «1» или «2»;
	// стр. 703 гр. 3 = «1» или «2».
	Для НомСтр = 701 По 703 Цикл
		Для НомКол = 3 По 4 Цикл
			Если НомСтр = 703 И НомКол = 4 Тогда
				Продолжить;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П107" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе I значение в стр. " + Формат(НомСтр, "ЧГ=") + " по гр. " + Формат(НомКол, "ЧГ=") + " должно быть равно 1 или 2!",
				"Раздел1", "П107" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	...
	// Стр. 801 - 810 гр. 3 заполняют организации, относящиеся или входящие в коды по ОКВЭД:
	// 30, 31.3, 32.1, 32.2, 32.3, 33.2, 33.3, 51.43.2, 51.84, 51.86, 51.87.5, 64.2, 71.33, 72.
	//  Не реализовано: данное указание содержится непосредственно в заголовке Раздела II.
	
	// Подраздел 8.
	// Стр. 801 гр. 3 < = стр. 117.
	Если Не (ПараметрыВыгрузки["П208080103_Ч"] <= ПараметрыВыгрузки["П101011703_Ч"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: значение в стр. 801 по гр. 3 (раздел II) должно быть меньше или равно значению в стр. 117 по гр. 3 (раздел I)!",
		"Раздел2", "П208080103");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Стр. 801 гр. 3 >=  сумме стр. 802-805 гр. 3.
	Если Не (ПараметрыВыгрузки["П208080103_Ч"] >= ПараметрыВыгрузки["П208080203_Ч"]
		+ ПараметрыВыгрузки["П208080303_Ч"]
		+ ПараметрыВыгрузки["П208080403_Ч"]
		+ ПараметрыВыгрузки["П208080503_Ч"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: в разделе II значение в стр. 801 по гр. 3 должно быть больше или равно сумме значений в стр. 802-805 по гр. 3!",
		"Раздел2", "П208080103");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Стр. 805 гр. 3 >= стр. 806 гр. 3.
	Если Не (ПараметрыВыгрузки["П208080503_Ч"] >= ПараметрыВыгрузки["П208080603_Ч"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: в разделе II значение в стр. 805 по гр. 3 должно быть больше или равно значению в стр. 806 по гр. 3!",
		"Раздел2", "П208080503");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// Подраздел 9.
	// Стр. 901 - 902 гр. 3 заполняют организации, относящиеся или входящие в коды по ОКВЭД:
	// 30, 31.3, 32.1, 32.2, 32.3, 33.2, 33.3, 51.43.2, 51.84, 51.86, 51.87.5, 64.2, 71.33, 72,
	// у которых хотя бы одна из стр.801 - 810 гр.3 > 0.
	//  Не реализовано: данное указание содержится непосредственно в заголовке Раздела II.
	
	// Стр. 901 гр. 3 >= стр. 902 гр. 3.
	Если Не (ПараметрыВыгрузки["П209090103_Ч"] >= ПараметрыВыгрузки["П209090203_Ч"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: в разделе II значение в стр. 901 по гр. 3 должно быть больше или равно значению в стр. 902 по гр. 3!",
		"Раздел2", "П209090103");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	ДопАтрибуты = РегламентированнаяОтчетность.СформироватьСтруктуруДопАтрибутов(ЭтаФорма.ИмяФормы, "АтрибВыгрузкиXML2013Кв4");
	...
	// Для ПОСЛЕДУЮЩЕГО (ПЕРЕД ВЫГРУЗКОЙ) контроля показателей формы (Указания по заполнению Формы 3-информ).
	// По разделу I.
	Для НомСтр = 101 По 119 Цикл
		ПараметрыВыгрузки.Вставить("П101" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч",
		ТабДокРаздел1["П101" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]);
	КонецЦикла;
	Для НомСтр = 201 По 205 Цикл
		ПараметрыВыгрузки.Вставить("П102" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч",
		ТабДокРаздел1["П102" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]);
	КонецЦикла;
	Для НомСтр = 301 По 342 Цикл
		ПараметрыВыгрузки.Вставить("П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч",
		ТабДокРаздел1["П103" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]);
	КонецЦикла;
	Для НомСтр = 401 По 452 Цикл
		ПараметрыВыгрузки.Вставить("П104" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч",
		ТабДокРаздел1["П104" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]);
	КонецЦикла;
	ПараметрыВыгрузки.Вставить("П104044404_Ч", ТабДокРаздел1["П104044404"]);
	ПараметрыВыгрузки.Вставить("П104044504_Ч", ТабДокРаздел1["П104044504"]);
	Для НомСтр = 501 По 508 Цикл
		ПараметрыВыгрузки.Вставить("П105" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч",
		ТабДокРаздел1["П105" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]);
	КонецЦикла;
	Для НомСтр = 601 По 615 Цикл
		ПараметрыВыгрузки.Вставить("П106" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04_Ч",
		ТабДокРаздел1["П106" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"]);
	КонецЦикла;
	Для НомСтр = 701 По 703 Цикл
		ПараметрыВыгрузки.Вставить("П107" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч",
		ТабДокРаздел1["П107" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]);
	КонецЦикла;
	ПараметрыВыгрузки.Вставить("П107070104_Ч", ТабДокРаздел1["П107070104"]);
	ПараметрыВыгрузки.Вставить("П107070204_Ч", ТабДокРаздел1["П107070204"]);
	// По разделу II.
	Для НомСтр = 801 По 810 Цикл
		ПараметрыВыгрузки.Вставить("П208" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч",
		ТабДокРаздел2["П208" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]);
	КонецЦикла;
	Для НомСтр = 901 По 903 Цикл
		ПараметрыВыгрузки.Вставить("П209" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч",
		ТабДокРаздел2["П209" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]);
	КонецЦикла;
