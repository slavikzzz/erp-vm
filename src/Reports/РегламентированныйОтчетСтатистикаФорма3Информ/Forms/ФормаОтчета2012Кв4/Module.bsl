...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Строки_401_431","П101010703");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Строка_431","П104041903;П104042003;П104042103;П104042203;П104042303;П104042403;П104042503;П104042603;П104042703;П104042803;П104042903;П104043003; П101010703");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Строки_432_433","П101010103;П101010303;П101010703;П101010803;П101010903;П101011003;П101011103;П101011203");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Строки_434_435","П101010703;П101010803;П101011003");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Строки_436_443_446_449","П101011203");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Строка_901_905","П208080103;П208080203;П208080303;П208080403;П208080503;П208080603;П208080703;П208080803;П208080903;П208081003");
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Если ИмяТекущейСтраницыПанели = "Раздел1" Тогда
		...
		КонецЕсли;
		Если Форма.ТабличныйДокумент.Области.П103034103.Значение > Форма.ТабличныйДокумент.Области.П103030503.Значение Тогда
			Форма.ТабличныйДокумент.Области.П103034103.Значение = Форма.ТабличныйДокумент.Области.П103030503.Значение;
		КонецЕсли;
		...
		// Установка цвета и доступности строк 401-431 для ввода 
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строки_401_431, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Форма.ТабличныйДокумент.Области.П101010703.Значение = 1;
			Для КодСтроки = 401 По 431 Цикл
		...
		// Установка цвета и доступности строк 432-433 для ввода 
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строки_432_433, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Форма.ТабличныйДокумент.Области.П101010103.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101010303.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101010703.Значение = 1
			ИЛИ Форма.ТабличныйДокумент.Области.П101010803.Значение = 1
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строки_436_443_446_449, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Форма.ТабличныйДокумент.Области.П101011203.Значение = 1;
			Для КодСтроки = 436 По 443 Цикл
				ИмяЯчейки = "П1040" + Строка(КодСтроки) + "03";
				УстановитьЦветИДоступностьЯчейки(Форма, ИмяЯчейки, УсловиеДоступа);
			КонецЦикла;
			Для КодСтроки = 446 По 449 Цикл
				ИмяЯчейки = "П1040" + Строка(КодСтроки) + "03";
				УстановитьЦветИДоступностьЯчейки(Форма, ИмяЯчейки, УсловиеДоступа);
			КонецЦикла;
		КонецЕсли;
	...
	ИначеЕсли ИмяТекущейСтраницыПанели = "Раздел2" Тогда
		...
		// Установка цвета и доступности строки 901-905 для ввода 
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрНайти(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.Строка_901_905, ОбластьИмя) > 0 Тогда
			УсловиеДоступа = Ложь;
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Титульный", "ФормаОтчета2012Кв4_Титульный");
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1" Тогда
		...
		Если (Часть1Имени = "П101" И Часть2Имени >= "101" И Часть2Имени <= "114" И Часть3Имени = "03") //Номера строк 101-114
			...
			ИЛИ (Часть1Имени = "П104" И Часть2Имени >= "432" И Часть2Имени <= "433" И Часть3Имени = "03") //Номера строк 432-433
			ИЛИ (Часть1Имени = "П104" И Часть2Имени >= "436" И Часть2Имени <= "443" И Часть3Имени = "03") //Номера строк 436-443
			ИЛИ (Часть1Имени = "П104" И Часть2Имени >= "446" И Часть2Имени <= "449" И Часть3Имени = "03") //Номера строк 446-449
		...
		ИначеЕсли (Часть1Имени = "П104" И Часть2Имени >= "434" И Часть2Имени <= "435" И Часть3Имени = "03") Тогда //Номера строк 434-435
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревоДляЗагрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2012Кв4");
...
Функция ПроверитьПоказатели() Экспорт
	...
	РегламентированнаяОтчетность.СформироватьСтруктуруПоказателей(ЭтаФорма);
	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2012Кв4");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКПО) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не задан код ОКПО организации!", "Титульный", "ОргКодОКПО");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ОргНазв) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не задано наименование организации!", "Титульный", "ОргНазв");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ОргИсполнитель) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не указаны Ф.И.О. должностного лица, ответственного за предоставление статистической информации!", "Раздел2", "ОргИсполнитель");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ОргДолжностьИсп) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не указана должность лица, ответственного за предоставление статистической информации!", "Раздел2", "ОргДолжностьИсп");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ОргТелефонИсп) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не указан телефон должностного лица, ответственного за предоставление статистической информации!", "Раздел2", "ОргТелефонИсп");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	ДопАтрибуты = РегламентированнаяОтчетность.СформироватьСтруктуруДопАтрибутов(ЭтаФорма.ИмяФормы, "АтрибВыгрузкиXML2012Кв4");
