	...
	Для Каждого Строка Из ИнформацияОСертификатах Цикл
		...
		Исключение
			Строка.Предупреждение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось прочитать данные сертификата: %1';
					|en = 'Cannot read the certificate properties: (%1)'"), ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			Продолжить;
		КонецПопытки;
