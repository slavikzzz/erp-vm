...
Процедура СформироватьСпискиВыбора()
	...
	НовСтрока = ТаблСписка.Добавить();
	НовСтрока.Код = "0";
	НовСтрока.Название = "Метка не установлена";
	
	НовСтрока = ТаблСписка.Добавить();
	НовСтрока.Код = "1";
	НовСтрока.Название = "Метка установлена";
	
	КоллекцияСписковВыбора.Вставить("Метка", ТаблСписка);
	
	СвойстваПоказателей.Очистить();
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникМетка",
	512, , "Выбор метки", КоллекцияСписковВыбора["Метка"]);
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	Для Инд = 1 По 32 Цикл 
		ВыбТабПоле.Области["П00001010" + Формат(Инд, "ЧЦ=2; ЧВН=") + "02"].Значение = Формат(Инд, "ЧЦ=2; ЧВН=");
	КонецЦикла;
...
Процедура ЗаполнитьДаннымиОтчетовНаСервере(ТекстОписанияПроблемы)
	ЗаполнениеСтатистическойОтчетности.ЗаполнитьДаннымиПредидущегоПериода(ЭтаФорма, "МакетПереносаДанных2022Кв1", ТекстОписанияПроблемы);
КонецПроцедуры
