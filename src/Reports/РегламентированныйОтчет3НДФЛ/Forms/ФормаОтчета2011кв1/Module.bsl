...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Списки2011кв1");
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2012 Тогда
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета, УдалениеСтраницы = Ложь, ИндексУдаляемойСтраницы = Неопределено, ЗаголовокРаздела = Ложь)
	...
	Если ИмяТекРаздела = "ЛистД"
		...
		// Активная страница будет удалена, а активность перемещена на предыдущую стрницу, которая уже выведена
...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		СтрокаСведений = "ИННФЛ, НаимЮЛПол, КодНО, НаимНО, ТипНП, ТелДом, ФИО, 
						...
						|ГородМЖ, НПунктМЖ, УлицаМЖ, ДомМЖ, КорпусМЖ, КвартираМЖ,ИндивидуальныйПредпринимательИНН,ФИОРук,ОКАТО";
	...
	Если Этотобъект.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Титульный" Тогда
		...
		ТабличныйДокумент.Области.ОКАТО.Значение = ЗнОКАТО;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если СписокСохранения.ВерсияФормы = "01/01/2011" Тогда
...
Процедура Расчет(Форма, ИмяРаздела = "", ОбластьИмя = "", АвторасчетПослеАвтозаполнения = Ложь)
	...
	ИначеЕсли ИмяРаздела = "Раздел2" Тогда
		...
		Иначе
			...
			Если ОбновитьТабличныйДокумент Тогда
				...
				мРаздел2.Области.П000020002003.Значение = мЛистГ1.П000010116004 + мЛистГ2.П000010207001;
			...
			Иначе
				...
				мРаздел2.П000020002003 = мЛистГ1.П000010116004 + мЛистГ2.П000010207001;
	...
	ИначеЕсли ИмяРаздела = "Раздел3" Тогда
		...
		Если ОбновитьТабличныйДокумент Тогда
			...
			мРаздел3.Области.П000030002003.Значение = Окр( мЛистГ2.П000010207001, 2);
		...
		Иначе
			...
			мРаздел3.П000030002003 = Окр( мЛистГ2.П000010207001, 2);
			...
			мРаздел3.П000030005103 = Окр( мЛистГ3.П000027207001, 2);
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Лев(ИмяЯчейки,13) = "П000080001001") Тогда //Лист Б Код ОКСМ
		...
		ДополнительныеПараметры = Новый Структура("ИмяЯчейки, Область, СтандартнаяОбработка, Элемент", ИмяЯчейки, Область, СтандартнаяОбработка, Элемент);
...
Процедура ОбработкаВыбораИННФизЛица(Область)	
	...
	ДополнительныеПараметры = Новый Структура("ИНН, Область", ИНН, Область);
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СтруктураXML507");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если НЕ ПроверитьРазделыНаНаличиеИнформации(ДеревоВыгрузки, "Раздел1, Раздел2, Раздел3, Раздел4, Раздел5, Раздел6") Тогда
		...
		ТекстСообщений = НСтр("ru = 'Должен быть заполнен, как минимум, один из следующих разделов: ""Раздел 1"", ""Раздел 2"", ""Раздел 3"", ""Раздел 4"", ""Раздел 5"" или ""Раздел 6"".';
								|en = 'Должен быть заполнен, как минимум, один из следующих разделов: ""Раздел 1"", ""Раздел 2"", ""Раздел 3"", ""Раздел 4"", ""Раздел 5"" или ""Раздел 6"".'");
	...
	Для Каждого Стр Из ДанныеЛистА Цикл
		...
		Если НЕ СтраницаПуста(ДанныеСтраницы, "ЛистА", ДеревоВыгрузки) Тогда
			...
			Пока ДанныеСтраницы.Свойство("П000070001001_" + Формат(НомСтроки, "ЧГ=")) Цикл
				...
				Если ДлинаИНН <> 12 И  ДлинаИНН <> 10  Тогда
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Титульный",	 		"МЧБ_2011_01_Титульный");
	СоответствиеРазделовМакетам.Вставить("Титульный002", 		"МЧБ_2011_02_Титул_002");
	
	СоответствиеРазделовМакетам.Вставить("Раздел1",	 			"МЧБ_2011_03_Раздел_1");
	СоответствиеРазделовМакетам.Вставить("Раздел2",	 			"МЧБ_2011_04_Раздел_2");
	СоответствиеРазделовМакетам.Вставить("Раздел3",	 			"МЧБ_2011_05_Раздел_3");
	СоответствиеРазделовМакетам.Вставить("Раздел4",	 			"МЧБ_2011_06_Раздел_4");
	СоответствиеРазделовМакетам.Вставить("Раздел5",	 			"МЧБ_2011_07_Раздел_5");
	СоответствиеРазделовМакетам.Вставить("Раздел6",	 			"МЧБ_2011_08_Раздел_6");
	
	СоответствиеРазделовМакетам.Вставить("ЛистА",		 		"МЧБ_2011_09_Лист_А");
	СоответствиеРазделовМакетам.Вставить("ЛистБ",		 		"МЧБ_2011_10_Лист_Б");
	СоответствиеРазделовМакетам.Вставить("ЛистВ",	 			"МЧБ_2011_11_Лист_В");
	СоответствиеРазделовМакетам.Вставить("ЛистГ1",	 			"МЧБ_2011_12_Лист_Г1");
	СоответствиеРазделовМакетам.Вставить("ЛистГ1Продолжение",	"МЧБ_2011_27_Лист_Г1_Продолжение");
	СоответствиеРазделовМакетам.Вставить("ЛистГ2",	 			"МЧБ_2011_13_Лист_Г2");
	СоответствиеРазделовМакетам.Вставить("ЛистГ3",	 			"МЧБ_2011_28_Лист_Г3");
	СоответствиеРазделовМакетам.Вставить("ЛистД",	 			"МЧБ_2011_14_Лист_Д");
	СоответствиеРазделовМакетам.Вставить("ЛистЕ",		 		"МЧБ_2011_15_Лист_Е");
	СоответствиеРазделовМакетам.Вставить("ЛистЖ1",		 		"МЧБ_2011_16_Лист_Ж1");
	СоответствиеРазделовМакетам.Вставить("ЛистЖ2",	 	 		"МЧБ_2011_17_Лист_Ж2");
	СоответствиеРазделовМакетам.Вставить("ЛистЖ3",		 		"МЧБ_2011_18_Лист_Ж3");
	СоответствиеРазделовМакетам.Вставить("ЛистЗ",		 		"МЧБ_2011_21_Лист_З");
	СоответствиеРазделовМакетам.Вставить("ЛистЗПродолжение",	"МЧБ_2011_22_Лист_З_Продолжение");
	СоответствиеРазделовМакетам.Вставить("ЛистИ",		 		"МЧБ_2011_24_Лист_И");
...
Процедура ПроверкаРаздела6(СтруктураРезультата)
	...
	СтруктураОшибок = СтруктураРезультата.СтруктураОшибок;
		...
		ТекстФормулы = "строка 050 раздела 6 = строка 100 раздела 1 + строка 080 раздела 2 + строка 070 раздела 3 + строка 060 раздела 4 + строка 050 раздела 5, если возврат по одному ОКАТО и одному КБК";
		Нарушение = "ст. 23 НК РФ";
		Описание = "если строка 050 раздела 6 >,< строка 100 раздела 1 + строка 080 раздела 2 + строка 070 раздела 3 + строка 060 раздела 4 + строка 050 раздела 5, и если возврат по одному ОКАТО и одному КБК, то завышена/занижена сумма налога к возврату";
		...
		ТекстФормулы = "если возврат производится по ОКАТО муниципального образования по месту жительства НП и (или) по ОКАТО по месту нахождения налоговых агентов и сумма строк 050 всех листов раздела 6 = строка 100 раздела 1 + строка 080 раздела 2 + строка 070 раздела 3 + строка 060 раздела 4 + строка 050 раздела 5";
		Нарушение = "ст. 23 НК РФ";
		Описание = "если возврат производится по ОКАТО муниципального образования по месту жительства НП и (или) по ОКАТО по месту нахождения налоговых агентов и сумма строк 050 всех листов раздела 6 >,< строка 100 раздела 1 + строка 080 раздела 2 + строка 070 раздела 3 + строка 060 раздела 4 + строка 050 раздела 5, то завышена/занижена сумма налога к возврату";
			...
			Для НомСтр = 1 По КоличествоСтраницЛистаА Цикл
				...
				Если СтавкаНалога = ТекСтавкаНалога Тогда
					...
					Пока  ТаблицаБлока.Свойство("П000070001005_" + СчСтрок) Цикл
						...
						СтрокаПраваяЧасть = СтрокаПраваяЧасть + ?(ЗнакРавно," = ", " + ") + Строка(Окр(Значение,0));
						ЗнакРавно = Ложь;
						СчСтрок = СчСтрок + 1;	
					КонецЦикла;
				...
				ПраваяЧасть = ПраваяЧасть + Окр(СуммаСтрок, 0);
...
Процедура ОбработкаВыбораИННФизЛицаЗавершение(Строка, ДополнительныеПараметры) Экспорт
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СокрЛП(ИНН)) Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'В ИНН должны быть указаны только цифры!';
									|en = 'В ИНН должны быть указаны только цифры!'"),, Заголовок );
		Возврат;
	КонецЕсли; 
