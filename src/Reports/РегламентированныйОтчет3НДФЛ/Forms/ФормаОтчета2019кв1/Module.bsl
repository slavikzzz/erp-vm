...
Процедура ПроверитьСтраницуПриложения7(ДанныеСтраницы, НомерСтраницы)
	...
	Если ПустаяСтрока(ДанныеСтраницы.П000700001030) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			СтрШаблон(НСтр("ru = 'На странице %1 Приложения 7 не указан код номера объекта (30)';
							|en = 'На странице %1 Приложения 7 не указан код номера объекта (30)'"), НомерСтраницы),
			"Приложение7",
			"П000700001030",
			НомерСтраницы);
	ИначеЕсли НомерОбъектаНедвижимостиОтсутствует(ДанныеСтраницы.П000700001030) Тогда
		...
		Если ПустаяСтрока(ДанныеСтраницы.П000700001032) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'На странице %1 Приложения 7 не указаны сведения о местонахождении объекта (32)';
								|en = 'На странице %1 Приложения 7 не указаны сведения о местонахождении объекта (32)'"), НомерСтраницы),
				"Приложение7",
				"П000700001032",
				НомерСтраницы);
		КонецЕсли;
	Иначе
		Если ПустаяСтрока(ДанныеСтраницы.П000700001031) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'На странице %1 Приложения 7 не указан номер объекта (31)';
								|en = 'На странице %1 Приложения 7 не указан номер объекта (31)'"), НомерСтраницы),
				"Приложение7",
				"П000700001031",
				НомерСтраницы);
		КонецЕсли;
...
Процедура ПроверитьПриложение5(ДанныеСтраницы)
	...
	Если ДанныеСтраницы.П000900001090 <> 0 Тогда
		...
		Если ДанныеСтраницы.П000900001090 > Окр(СуммаОблагаемогоДоходаПоСтавке13 * 0.25, 2) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'Сумма вычета в виде пожертвований превышает 25%% от общей суммы дохода %1';
								|en = 'Сумма вычета в виде пожертвований превышает 25%% от общей суммы дохода %1'"), СуммаОблагаемогоДоходаПоСтавке13),
				"Приложение5",
				"П000900001090");
		КонецЕсли;
	КонецЕсли;
