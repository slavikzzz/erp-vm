...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.мДатаКонцаПериодаОтчета = Мин(КонецГода(ТекущаяДатаСеанса()), Параметры.мДатаКонцаПериодаОтчета);
	Параметры.мДатаНачалаПериодаОтчета = Мин(НачалоГода(ТекущаяДатаСеанса()), Параметры.мДатаНачалаПериодаОтчета);
	Параметры.мВыбраннаяФорма = "ФормаОтчета2016кв1";
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2016Кв1"; // Наименование макета с описанием состава показателей
	СтруктураРеквизитовФормы.мВерсияФормы = "14/06/2016"; // Версия формы
	...
	СтруктураРеквизитовФормы.ВариантыФорматовВыгрузки.Добавить(СтруктураРеквизитовФормы.мСоставПоказателей, "Формат 5.10", Истина);
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОтчетОбъект().ПолучитьМакет("Списки2016Кв1");
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000400001031", 0, , "Выбор вида дохода", КоллекцияСписковВыбора["КодыВидовДоходовЛистаБ"]);
...
#Область Пересчет_ЛистД2
		...
		// Пункт 4
		ПоказателиЛиста.П000800001170 = ПоказателиЛиста.П000800001020 + ПоказателиЛиста.П000800001040
							+ ПоказателиЛиста.П000800001060 + ПоказателиЛиста.П000800001080 
							+ ПоказателиЛиста.П000800001100 + ПоказателиЛиста.П000800001120
							+ ПоказателиЛиста.П000800001140 + ПоказателиЛиста.П000800001160;
		
		
		Если Форма.ТабличныйДокумент.Области.Найти("П000800001170") <> Неопределено Тогда
			
			// Пункт 1.3
			Форма.ТабличныйДокумент.Области["П000800001170"].Значение = ПоказателиЛиста.П000800001170;
...
Процедура ЗаполнитьСводныеТаблицы(Форма)
	...
	Для Каждого Лист Из Форма.ТаблицаСтраницыЛистА Цикл
		...
		Для Каждого СтрокаЛиста Из Лист.ДанныеДопСтрок[0].Значение Цикл
			...
			Если СтрокаЛиста.П000300001020 = "08" Тогда
				// с 2015-го года доходы в виде дивидендов и в виде сумм прибыли контролируемых иностранных компаний должны считаться отдельно
	...
	Для Каждого Лист Из Форма.ТаблицаСтраницыЛистБ Цикл
		...
		Для Каждого СтрокаЛиста Из Лист.ДанныеДопСтрок[0].Значение Цикл
			...
			Если Строка(СтрокаЛиста.П000400001031) = "1" Тогда
				// с 2015-го года доходы в виде дивидендов и в виде сумм прибыли контролируемых иностранных компаний должны считаться отдельно
				СтрокаДохода.ВидДохода = Дивиденды();
			Иначе
				СтрокаДохода.ВидДохода = ОбычныйДоход();
	...
	// - пункте 4 листа Д2
	Если Форма.СтруктураДанныхЛистД2.П000800001170 <> 0 Тогда
		СтрокаВычета                         = Форма.СводВычетов.Добавить();
		СтрокаВычета.ВидВычета               = "ЛистД2.170";
		СтрокаВычета.Ставка                  = 13;
		СтрокаВычета.СуммаВычетаДляРаздела2  = Форма.СтруктураДанныхЛистД2.П000800001170;
		СтрокаВычета.СуммаВычетаДляД1        = СтрокаВычета.СуммаВычетаДляРаздела2;
	КонецЕсли; 
...
#Область Пересчет_Раздел2
		...
		ОчиститьЗначенияСтраницы(Форма, ЛистСтраницы.Данные[0].Значение, "Раздел2", Ложь);
	...
	Если ЗначениеЗаполнено(Ставка) Тогда
		...
		Если СтрокиДоходов.Количество() > 0 Тогда
			...
			Для Каждого СтрокаДохода Из СтрокиДоходов Цикл
				...
				Если СтрокаДохода.ВидДохода = Дивиденды() Тогда
			...
			Для Каждого СтрокаВычета Из СтрокиВычетов Цикл
				...
				Если СтрокаВычета.ВидВычета = "ЛистГ.200" ИЛИ СтрокаВычета.ВидВычета = "ЛистГ.220" Тогда
				...
				ИначеЕсли СтрокаВычета.ВидВычета = "ЛистВ.120" ИЛИ СтрокаВычета.ВидВычета = "ЛистД1.180"
					...
					ИЛИ СтрокаВычета.ВидВычета = "ЛистД1.220" ИЛИ СтрокаВычета.ВидВычета = "ЛистД2.170"
					ИЛИ СтрокаВычета.ВидВычета = "ЛистЕ1.190" ИЛИ СтрокаВычета.ВидВычета = "ЛистЕ2.110"
					ИЛИ СтрокаВычета.ВидВычета = "ЛистЖ.150" Тогда
		...
		ДанныеСтраницы.П000200001121 = Макс(0, ДанныеСтраницы.П000200001070 
					- ДанныеСтраницы.П000200001080- ДанныеСтраницы.П000200001090
					- ДанныеСтраницы.П000200001100 - ДанныеСтраницы.П000200001110 
					- ДанныеСтраницы.П000200001120);
					
		// 13.3. Сумма налога, подлежащая уплате (доплате) в бюджет
		ДанныеСтраницы.П000200001130 = Макс(0, ДанныеСтраницы.П000200001121 - ДанныеСтраницы.П000200001122 - ДанныеСтраницы.П000200001123);
...
Процедура ПодготовитьДанныеРаздела2(Форма)
	...
	Для Каждого СтрокаСтавки Из Форма.мСтавкиНалога Цикл
		...
		СтрокиДохода = Форма.СводДоходов.НайтиСтроки(Новый Структура("Ставка", Число(СтрокаСтавки.Код)));
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = ОтчетОбъект().ПолучитьМакет("СтруктураXML510");
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Титульный",	 		"МЧБ_2016Кв1_01_Титульный");
	СоответствиеРазделовМакетам.Вставить("Раздел1",	 			"МЧБ_2016Кв1_02_Раздел1");
	СоответствиеРазделовМакетам.Вставить("Раздел2",	 			"МЧБ_2016Кв1_03_Раздел2");
	СоответствиеРазделовМакетам.Вставить("ЛистА",		 		"МЧБ_2016Кв1_04_ЛистА");
	СоответствиеРазделовМакетам.Вставить("ЛистБ",		 		"МЧБ_2016Кв1_05_ЛистБ");
	СоответствиеРазделовМакетам.Вставить("ЛистВ",	 			"МЧБ_2016Кв1_06_ЛистВ");
	СоответствиеРазделовМакетам.Вставить("ЛистГ",	 			"МЧБ_2016Кв1_07_ЛистГ");
	СоответствиеРазделовМакетам.Вставить("ЛистД1",	 			"МЧБ_2016Кв1_08_ЛистД1");
	СоответствиеРазделовМакетам.Вставить("ЛистД2",	 			"МЧБ_2016Кв1_09_ЛистД2");
	СоответствиеРазделовМакетам.Вставить("ЛистЕ1",		 		"МЧБ_2016Кв1_10_ЛистЕ1");
	СоответствиеРазделовМакетам.Вставить("ЛистЕ2",		 		"МЧБ_2016Кв1_11_ЛистЕ2");
	СоответствиеРазделовМакетам.Вставить("ЛистЖ",		 		"МЧБ_2016Кв1_12_ЛистЖ");
	СоответствиеРазделовМакетам.Вставить("ЛистЗ",		 		"МЧБ_2016Кв1_13_ЛистЗ");
	СоответствиеРазделовМакетам.Вставить("ЛистЗПродолжение",	"МЧБ_2016Кв1_14_ЛистЗПродолжение");
	СоответствиеРазделовМакетам.Вставить("ЛистЗПродолжение2",	"МЧБ_2016Кв1_15_ЛистЗПродолжение2");
	СоответствиеРазделовМакетам.Вставить("ЛистЗПродолжение3",	"МЧБ_2016Кв1_17_ЛистЗПродолжение3");
	СоответствиеРазделовМакетам.Вставить("ЛистЗПродолжение4",	"МЧБ_2016Кв1_17_ЛистЗПродолжение4");
	СоответствиеРазделовМакетам.Вставить("ЛистИ",		 		"МЧБ_2016Кв1_18_ЛистИ");
	СоответствиеРазделовМакетам.Вставить("ЛистИПродолжение",	"МЧБ_2016Кв1_19_ЛистИПродолжение");
