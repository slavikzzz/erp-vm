...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.мВыбраннаяФорма = Отчеты.РегламентированныйОтчет3НДФЛ.НормироватьИдентификаторФормы(Параметры.мВыбраннаяФорма);
	...
	СтруктураДанныхПриложение3Расчет = Новый Структура;
	...
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.мИдРаздел1Приложение, КолИсхСтрокИдРаздел1Приложение);
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли ИмяЯчейки = "П002100001010" Тогда // Приложение к Разделу 1: КБК налога к зачету
			
		ОбработкаВыбораКБКПереплаты(Элемент, Область, СтандартнаяОбработка);
			
	ИначеЕсли ИмяЯчейки = "П002100001020" Тогда // Приложение к Разделу 1: ОКТМО налога к зачету
			
		ОбработкаВыбораОКТМОПереплаты(Элемент, Область, СтандартнаяОбработка);
					
	ИначеЕсли ИмяЯчейки = "П002100001140" Тогда // Приложение к Разделу 1: Haимeнование банка
		...
		ПоказатьВводСтроки(
			ОписаниеОповещенияВыбораВПолеТабличногоДокумента("ОткрытьФормуВводаСтрокиЗавершение", Элемент, Область),
			Область.Текст,
			НСтр("ru = 'Haимeнование банка';
				|en = 'Haимeнование банка'"),
			,
			Истина);
	
	ИначеЕсли ИмяЯчейки = "П002100001110" Тогда // Приложение к Разделу 1: КБК налога к возврату
			
		ОбработкаВыбораКБКПереплаты(Элемент, Область, СтандартнаяОбработка);
			
	ИначеЕсли ИмяЯчейки = "П002100001120" Тогда // Приложение к Разделу 1: ОКТМО налога к возврату
...
Процедура РасчетПослеИзмененияСодержания(ИмяПоказателя, ИмяРаздела, Область)
	...
	// Приложение 5, Сумма, перечисленная в виде пожертвований (090)
	// В ситуации, когда перечисления делаются в учреждения культуры, тогда предел может быть другим
	Если ИмяПоказателя = "П000900001090" Тогда
		
		ТекстСообщения = СообщениеПроверкиСуммыПожертвований(ТабличныйДокумент.Области[ИмяПоказателя].Значение,
	...
	Если НЕ ИмяРаздела = "Титульный" Тогда
		...
		Если ИмяРаздела = "Раздел1Приложение"
			И ТребуетсяРасчетРаздел1Приложение(ИмяПоказателя, Область.Значение) Тогда
			РасчетРаздел1Приложение();
		КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ТребуетсяРасчетРаздел1Приложение(ИмяПоказателя, ВыбранноеЗначение)
	
	Если ИмяПоказателя = "П002100001010"
		Или ИмяПоказателя = "П002100001110" Тогда
		// КБК
		Возврат Истина;
		
	ИначеЕсли ИмяПоказателя = "П002100001020"
		Или ИмяПоказателя = "П002100001120" Тогда
		// ОКТМО
		Возврат ЗначениеЗаполнено(ВыбранноеЗначение);
...
Функция ОпределитьТабличноеПолеПоИдМногострочногоРазделаНаКлиенте(ИдГруппы) Экспорт
	...
	ИдГрупп.Вставить("мИдРаздел1Приложение", СтруктураРеквизитовФормы.мИдРаздел1Приложение);
...
Функция ОпределитьТабличноеПолеПоИдМногострочногоРаздела(ИдГруппы, ИдГрупп) Экспорт
	...
	ИначеЕсли ИдГруппы = ИдГрупп.мИдРаздел1Приложение Тогда
		ТекТабличноеПоле = "Раздел1Приложение";
...
Функция ОпределитьИдМногострочногоРазделаПоАктивнойСтранице(ИмяЛиста = Неопределено, ТекущаяСтраница, ИдГрупп)
	...
	ИначеЕсли ИмяТекущейСтраницыПанели = "Раздел1Приложение" Тогда
		ИмяГруппы = ИдГрупп.мИдРаздел1Приложение;
...
&НаКлиенте
Процедура ОбработкаВыбораОКТМОПереплаты(Элемент, Область, СтандартнаяОбработка)
	
	ИмяЯчейки = Область.Имя;
	ПараметрВводаПоля = ПараметрыВыбораОКТМОПереплаты(ИмяЯчейки);
...
&НаКлиенте
Процедура ОбработкаВыбораКБКПереплаты(Элемент, Область, СтандартнаяОбработка)
	
	ИмяЯчейки = Область.Имя;
	ПараметрВводаПоля = ПараметрыВыбораКБКПереплаты(ИмяЯчейки);
...
&НаСервере
Функция ПараметрыВыбораОКТМОПереплаты(ИмяЯчейки)
	
	ПереплатаНДФЛ = ПереплатаНДФЛ();
	ПереплатаНДФЛ.Свернуть("ОКТМО, КБК");
	
	Если ЗначениеЗаполнено(ПереплатаНДФЛ) Тогда
		ТЗВыбора = Новый ТаблицаЗначений;
		ТЗВыбора.Колонки.Добавить("Код", ОбщегоНазначения.ОписаниеТипаСтрока(11));
		ТЗВыбора.Колонки.Добавить("Название", ОбщегоНазначения.ОписаниеТипаСтрока(20));
		НоваяСтрока = ТЗВыбора.Добавить();
		НоваяСтрока.Код = "";
		НоваяСтрока.Название = НСтр("ru = 'Пустое значение';
									|en = 'Пустое значение'");
		Для Каждого СтрокаВыбора Из ПереплатаНДФЛ() Цикл
			Если ТЗВыбора.Найти(СтрокаВыбора.ОКТМО, "Код") = Неопределено Тогда
				НоваяСтрока = ТЗВыбора.Добавить();
				НоваяСтрока.Код = СтрокаВыбора.ОКТМО;
				НоваяСтрока.Название = СтрокаВыбора.КБК;
			КонецЕсли;
		КонецЦикла;
		Результат = Новый Структура;
		Результат.Вставить("АдресТаблицыЗначений", ПоместитьВоВременноеХранилище(ТЗВыбора));
		Результат.Вставить("Показатель", Лев(ИмяЯчейки, 13));
		Результат.Вставить("ТекстПриВыборе", НСтр("ru = 'ОКТМО';
													|en = 'ОКТМО'"));
		Возврат Результат;
...
&НаСервере
Функция ПараметрыВыбораКБКПереплаты(ИмяЯчейки)
	
	ПараметрВводаПоля = СвойствоПоказателя(ИмяЯчейки, АдресСвойствПоказателей);
	СписокВыбораКБК = ПолучитьИзВременногоХранилища(ПараметрВводаПоля.АдресТаблицыЗначений);
	
	ПереплатаНДФЛ = ПереплатаНДФЛ();
	Если ЗначениеЗаполнено(ПереплатаНДФЛ) Тогда
		СтрокиДляВыбора = Новый Массив;
		Для Каждого СтрокаВыбора Из СписокВыбораКБК Цикл
			Если Не ЗначениеЗаполнено(СтрокаВыбора.Код)
				Или ПереплатаНДФЛ.Найти(СтрокаВыбора.Код, "КБК") <> Неопределено Тогда
				СтрокиДляВыбора.Добавить(СтрокаВыбора);
			КонецЕсли;
		КонецЦикла;
		Если СтрокиДляВыбора.Количество() > 1 Тогда
			СписокВыбораКБК = СписокВыбораКБК.Скопировать(СтрокиДляВыбора);
		КонецЕсли;
			
	КонецЕсли;
	
	ПараметрВводаПоля.Вставить("АдресТаблицыЗначений", ПоместитьВоВременноеХранилище(СписокВыбораКБК));
	
	Возврат ПараметрВводаПоля;
	
КонецФункции

&НаСервере
Функция ПереплатаНДФЛ()
	
	ПереплатаНДФЛ = Новый ТаблицаЗначений;
	ПереплатаНДФЛ.Колонки.Добавить("КБК", ОбщегоНазначения.ОписаниеТипаСтрока(20));
	ПереплатаНДФЛ.Колонки.Добавить("ОКТМО", ОбщегоНазначения.ОписаниеТипаСтрока(11));
	ПереплатаНДФЛ.Колонки.Добавить("Сумма", ОбщегоНазначения.ОписаниеТипаЧисло(15, 0));
	...
	Для Каждого СтраницаРаздела Из ТаблицаСтраницРаздела Цикл
		...
		Если СтраницаРаздела.ДанныеДопСтрок <> Неопределено Тогда
			Для Каждого ДопСтрока Из СтраницаРаздела.ДанныеДопСтрок Цикл
				
				ПризнакиУплатыВозврата = Отчеты.РегламентированныйОтчет3НДФЛ.ПризнакиУплатыВозврата(
					СтруктураРеквизитовФормы.мВыбраннаяФорма);
				ПризнакВозвратаНалога = ПризнакиУплатыВозврата.Возврат;
					
				СтрокаЛиста = ДопСтрока.Значение[0];
				Если СтрокаЛиста.П000100001010 <> ПризнакВозвратаНалога Тогда
					Продолжить;
				КонецЕсли;
				
				Если Не ЗначениеЗаполнено(СтрокаЛиста.П000100001020) И Не ЗначениеЗаполнено(СтрокаЛиста.П000100001030) Тогда
					Продолжить;
				КонецЕсли;
				
				НоваяСтрока = ПереплатаНДФЛ.Добавить();
				НоваяСтрока.КБК   = СтрокаЛиста.П000100001020;
				НоваяСтрока.ОКТМО = СтрокаЛиста.П000100001030;
				НоваяСтрока.Сумма = СтрокаЛиста.П000100001050;
				
			КонецЦикла;
		КонецЕсли;
		
		Если СтраницаРаздела.Данные <> Неопределено Тогда
			
			ДанныеСтраницы = СтраницаРаздела.Данные[0].Значение;
			Если Не ЗначениеЗаполнено(ДанныеСтраницы.П000100001060) И Не ЗначениеЗаполнено(ДанныеСтраницы.П000100001150) Тогда
				Продолжить;
			КонецЕсли;
			
			НоваяСтрока = ПереплатаНДФЛ.Добавить();
			НоваяСтрока.КБК   = ДанныеСтраницы.П000100001060;
			НоваяСтрока.ОКТМО = ДанныеСтраницы.П000100001070;
			НоваяСтрока.Сумма = ДанныеСтраницы.П000100001170;
			
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат ПереплатаНДФЛ;
...
Процедура ОбработкаВыбораИФНСЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено Тогда
		ТабличныйДокумент.Области[ДополнительныеПараметры.ИмяЯчейки].Значение = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(РезультатВыбора.КодНО, 4);
...
Процедура СформироватьСтруктуруПоказателей()
	...
	Для Каждого СтраницаРаздела1 Из мДанныеОтчета.Раздел1 Цикл
		...
		Если ЗначениеЗаполнено(ЗначенияПоказателей["П000100001100"]) 
			Или ЗначениеЗаполнено(ЗначенияПоказателей["П000100001110"]) Тогда
			
			ЗначенияПоказателей.П000100001100 = ЗначенияПоказателей.П000100001100 - ЗначенияПоказателей.П000100001110;
		КонецЕсли;
			
		Если ЗначениеЗаполнено(ЗначенияПоказателей["П000100001130"]) 
			Или ЗначениеЗаполнено(ЗначенияПоказателей["П000100001140"]) Тогда
			
			ЗначенияПоказателей.П000100001130 = ЗначенияПоказателей.П000100001130 - ЗначенияПоказателей.П000100001140;
		КонецЕсли;
			
		Если ЗначениеЗаполнено(ЗначенияПоказателей["П000100001160"]) 
			Или ЗначениеЗаполнено(ЗначенияПоказателей["П000100001170"]) Тогда
			
			ЗначенияПоказателей.П000100001160 = ЗначенияПоказателей.П000100001160 - ЗначенияПоказателей.П000100001170;
		КонецЕсли;
...
Функция ПроверитьРазделыНаНаличиеИнформации(ДеревоВыгрузки, парамРазделы)
	...
	Для Каждого Раздел Из Разделы Цикл
		Если Отчеты.РегламентированныйОтчет3НДФЛ.РазделПуст(мДанныеОтчета, ДеревоВыгрузки, Раздел, СтруктураРеквизитовФормы.мВыбраннаяФорма) Тогда
			РазделыПусты = Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура РассчитатьИтогиЛиста(Форма, ИмяЛиста = Неопределено)
	...
	ИначеЕсли ИмяЛиста = "Приложение4" Тогда
		...
		Если Форма.ТабличныйДокумент.Области.Найти("П000600001130") <> Неопределено Тогда
			Форма.ТабличныйДокумент.Области["П000600001130"].Значение = ПоказателиЛиста.П000600001130;
		КонецЕсли; 
	...
	ИначеЕсли ИмяЛиста = "Раздел1Приложение" Тогда
		
		ПоказательОтчетГодЧисло = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Форма.СтруктураДанныхТитульный, "ОтчетГод", 0);
		
		ПоказательПериодКод = СтрШаблон("ГД.00.%1", Формат(ПоказательОтчетГодЧисло, "ЧН='""""'; ЧГ=0;"));
		
		Для Каждого Лист Из Форма.ТаблицаСтраницыРаздел1Приложение Цикл
			...
			СтрокаЛиста = Лист.Данные[0].Значение;
			МассивРассчитываемыхЗначений = Новый Массив;
			МассивРассчитываемыхЗначений.Добавить("П002100001030");
			
			КБКзачет = СокрЛП(СтрокаЛиста.П002100001010);
			Если ЗначениеЗаполнено(КБКзачет) Тогда
				СтрокаЛиста.П002100001030 = ПоказательПериодКод;
			Иначе
				СтрокаЛиста.П002100001030 = "";
			КонецЕсли;
			
			МассивРассчитываемыхЗначений.Добавить("П002100001130");
			КБКвозврат = СокрЛП(СтрокаЛиста.П002100001110);
			Если ЗначениеЗаполнено(КБКвозврат) Тогда
				СтрокаЛиста.П002100001130 = ПоказательПериодКод;
			Иначе
				СтрокаЛиста.П002100001130 = "";
			КонецЕсли;
			
			МассивРассчитываемыхЗначений.Добавить("ЗаявВозвратФамилия");
			МассивРассчитываемыхЗначений.Добавить("ЗаявВозвратИмя");
			МассивРассчитываемыхЗначений.Добавить("ЗаявВозвратОтчество");
			Если ЗначениеЗаполнено(КБКвозврат) Тогда
				РеквизитыТитульногоЛиста = Форма.СтруктураДанныхТитульный;
				СтрокаЛиста.ЗаявВозвратФамилия = РеквизитыТитульногоЛиста.Фамилия;
				СтрокаЛиста.ЗаявВозвратИмя = РеквизитыТитульногоЛиста.Имя;
				СтрокаЛиста.ЗаявВозвратОтчество = РеквизитыТитульногоЛиста.Отчество;
			Иначе
				СтрокаЛиста.ЗаявВозвратФамилия = "";
				СтрокаЛиста.ЗаявВозвратИмя = "";
				СтрокаЛиста.ЗаявВозвратОтчество = "";
			КонецЕсли;
			
			Если Лист.АктивнаяСтраница Тогда
				Для Каждого РасссчитанноеЗначение Из МассивРассчитываемыхЗначений Цикл
					Область = Форма.ТабличныйДокумент.Области.Найти(РасссчитанноеЗначение);
					Если Область <> Неопределено Тогда
						Область.Значение = СтрокаЛиста[РасссчитанноеЗначение];
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Процедура ЗаполнитьСводныеТаблицы(Форма)
	...
	КодыВидовДоходовЗаПределамиРФ = КодыВидовДоходовЗаПределамиРФ(Форма.СтруктураРеквизитовФормы.мВыбраннаяФорма);
	КодыБК = КБК(Форма.СтруктураРеквизитовФормы.мВыбраннаяФорма);
	СтавкаПоУмолчанию = НалоговыеСтавки(Форма.СтруктураРеквизитовФормы.мВыбраннаяФорма).ПоУмолчанию;
	...
	Для Каждого Лист Из Форма.ТаблицаСтраницыПриложение2 Цикл
		...
		Если Строка(ПоказателиЛиста.П000400001031) = КодыВидовДоходовЗаПределамиРФ.Дивиденды Тогда
			СтрокаДохода.КБК       = КодыБК.ДоходыПриПолученииКоторыхНеУдержанНалог;
			СтрокаДохода.ВидДохода = ВидыДоходов.Дивиденды;
		ИначеЕсли Строка(ПоказателиЛиста.П000400001031) = КодыВидовДоходовЗаПределамиРФ.ПрибыльКИК Тогда
			СтрокаДохода.КБК       = КодыБК.ДоходыСПрибылиКИК;
		...
		Если ПоказателиЛиста.П000400001071 <> 0 Тогда
			...
			Если Строка(ПоказателиЛиста.П000400001031) = КодыВидовДоходовЗаПределамиРФ.ПрибыльКИК Тогда
		...
		Если ПоказателиЛиста.П000400001072 <> 0 Тогда
			...
			СтрокаВычета                         = Форма.СводВычетов.Добавить();
			СтрокаВычета.ВидВычета               = "СуммаРубЦенныеБумагиКИК";
			СтрокаВычета.Ставка                  = СтавкаНалога;
		...
		Если ПоказателиЛиста.П000400001073 <> 0 Тогда
			
			СтрокаВычета                         = Форма.СводВычетов.Добавить();
			СтрокаВычета.ВидВычета               = "СуммаРубДивидендовКИК";
			СтрокаВычета.Ставка                  = СтавкаНалога;
			СтрокаВычета.СуммаВычетаДляРаздела2  = ПоказателиЛиста.П000400001073;
			Если Строка(ПоказателиЛиста.П000400001031) = КодыВидовДоходовЗаПределамиРФ.ПрибыльКИК Тогда
				СтрокаВычета.ВидДохода = ВидыДоходов.ПрибыльКИК;
			Иначе
				СтрокаВычета.ВидДохода = ВидыДоходов.Иное;
	...
	Для Каждого Лист Из Форма.ТаблицаСтраницыПриложение3 Цикл
		...
		Если ЗначениеЗаполнено(СтрокаЛиста.П000500001110) Тогда // Код источника дохода
			...
			СтрокаВычета.Ставка = СтавкаПоУмолчанию;
	...
	Если СтраницаИтоговПриложения3 <> 0 Тогда
		...
		Если СтрокаЛиста.П000500001050 <> 0 Тогда
			...
			СтрокаДохода.ОКТМО =  Форма.СтруктураРеквизитовФормы.ОКТМО;
			СтрокаДохода.Ставка = СтавкаПоУмолчанию;
	...
	// Приложение 4
	Если Форма.СтруктураДанныхПриложение4.П000600001130 <> 0 Тогда
		СтрокаВычета                         = Форма.СводВычетов.Добавить();
		СтрокаВычета.ВидВычета               = "СуммаДоходовНеПодлежащаяНалогообложению";
		СтрокаВычета.Ставка                  = СтавкаПоУмолчанию;
		СтрокаВычета.СуммаВычетаДляРаздела2  = Форма.СтруктураДанныхПриложение4.П000600001130;
		СтрокаВычета.СуммаВычетаДляД1        = СтрокаВычета.СуммаВычетаДляРаздела2;
		СтрокаВычета.ВидДохода               = ВидыДоходов.Иное;
	КонецЕсли;
	
	// Общая сумма стандартных налоговых вычетов, предоставленных в отчетном налоговом периоде налоговыми агентами
	Если Форма.СтруктураДанныхПриложение5.П000900001070 <> 0 Тогда
		СтрокаВычета                         = Форма.СводВычетов.Добавить();
		СтрокаВычета.ВидВычета               = "ОбщаяСуммаСтандартныхВычетовПредоставленныхНалоговымиАгентами";
		СтрокаВычета.Ставка                  = СтавкаПоУмолчанию;
		СтрокаВычета.СуммаВычетаДляРаздела2  = Форма.СтруктураДанныхПриложение5.П000900001070;
		СтрокаВычета.СуммаВычетаДляД1        = СтрокаВычета.СуммаВычетаДляРаздела2;
		СтрокаВычета.ВидДохода               = ВидыДоходов.Иное;
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяРаздела = "", ИмяОбласти = "")
	...
	ИначеЕсли ИмяРаздела = "Приложение5" Тогда
		...
		ИначеЕсли ИмяОбласти = "П000900001070" Тогда
			...
			// Приказ ФНС России от 03.10.2018 N ММВ-7-11/569@ (ред. от 07.10.2019)
		...
		ИначеЕсли ИмяОбласти = "П000900001181" Тогда
			...
			// Приказ ФНС России от 03.10.2018 N ММВ-7-11/569@ (ред. от 07.10.2019)
			ПоказателиСуммы = Новый Массив;
			ПоказателиСуммы.Добавить("П000900001100");
			ПоказателиСуммы.Добавить("П000900001110");
	...
	ИначеЕсли ИмяРаздела = "Приложение3Расчет" Тогда
		СтавкаНалога = (НалоговыеСтавки(Форма.СтруктураРеквизитовФормы.мВыбраннаяФорма).ПоУмолчанию) / 100;
		Если ИмяОбласти = "П002000001010"
			Или ИмяОбласти = "П002000001020"
			Или ИмяОбласти = "П002000001030" Тогда
			НалоговаяБазаПервыйКвартал = Макс(Форма.ТабличныйДокумент.Области["П002000001010"].Значение
				- Форма.ТабличныйДокумент.Области["П002000001020"].Значение
				- Форма.ТабличныйДокумент.Области["П002000001030"].Значение, 0);
			Форма.ТабличныйДокумент.Области["П002000001040"].Значение = НалоговаяБазаПервыйКвартал;
			Форма.ТабличныйДокумент.Области["П002000001050"].Значение = НалоговаяБазаПервыйКвартал * СтавкаНалога;
			СохранитьЗначениеПоказателяВСтруктуре(Форма, ИмяРаздела, "П002000001040", НалоговаяБазаПервыйКвартал);
			СохранитьЗначениеПоказателяВСтруктуре(Форма, ИмяРаздела, "П002000001050", Окр(НалоговаяБазаПервыйКвартал * СтавкаНалога, 0));
		ИначеЕсли ИмяОбласти = "П002000001011"
			Или ИмяОбласти = "П002000001021"
			Или ИмяОбласти = "П002000001031" Тогда
			НалоговаяБазаПолугодие = Макс(Форма.ТабличныйДокумент.Области["П002000001011"].Значение
				- Форма.ТабличныйДокумент.Области["П002000001021"].Значение
				- Форма.ТабличныйДокумент.Области["П002000001031"].Значение, 0);
			Форма.ТабличныйДокумент.Области["П002000001041"].Значение = НалоговаяБазаПолугодие;
			Форма.ТабличныйДокумент.Области["П002000001051"].Значение = НалоговаяБазаПолугодие * СтавкаНалога;
			СохранитьЗначениеПоказателяВСтруктуре(Форма, ИмяРаздела, "П002000001041", НалоговаяБазаПолугодие);
			СохранитьЗначениеПоказателяВСтруктуре(Форма, ИмяРаздела, "П002000001051", Окр(НалоговаяБазаПолугодие * СтавкаНалога, 0));
		
		ИначеЕсли ИмяОбласти = "П002000001012"
			Или ИмяОбласти = "П002000001022"
			Или ИмяОбласти = "П002000001032" Тогда
			НалоговаяБазаДевятьМесяцев = Макс(Форма.ТабличныйДокумент.Области["П002000001012"].Значение
				- Форма.ТабличныйДокумент.Области["П002000001022"].Значение
				- Форма.ТабличныйДокумент.Области["П002000001032"].Значение, 0);
			Форма.ТабличныйДокумент.Области["П002000001042"].Значение = НалоговаяБазаДевятьМесяцев;
			Форма.ТабличныйДокумент.Области["П002000001052"].Значение = НалоговаяБазаДевятьМесяцев * СтавкаНалога;
			СохранитьЗначениеПоказателяВСтруктуре(Форма, ИмяРаздела, "П002000001042", НалоговаяБазаДевятьМесяцев);
			СохранитьЗначениеПоказателяВСтруктуре(Форма, ИмяРаздела, "П002000001052", Окр(НалоговаяБазаДевятьМесяцев * СтавкаНалога, 0));
		КонецЕсли;
...
&НаСервере
Процедура РасчетРаздел1Приложение()
	
	ПереплатаНДФЛ = ПереплатаНДФЛ();
	Если ПереплатаНДФЛ.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого СтраницаРаздела Из ТаблицаСтраницыРаздел1Приложение Цикл
		...
		// Заявление о зачете суммы излишне уплаченного налога на доходы физических лиц
		
		КБКзачет = СокрЛП(ДанныеСтраницы.П002100001010);
		Если ЗначениеЗаполнено(КБКзачет) Тогда
			// ОКТМО
			ОКТМОзачет = ДанныеСтраницы.П002100001020;
			Если Не ЗначениеЗаполнено(ОКТМОзачет) Тогда
				НайденныеСтроки =  ПереплатаНДФЛ.НайтиСтроки(Новый Структура("КБК", КБКзачет));
				Если НайденныеСтроки.Количество() = 1 Тогда
					ОКТМОзачет = НайденныеСтроки[0].ОКТМО;
					ДанныеСтраницы.П002100001020 = ОКТМОзачет;
				КонецЕсли;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ОКТМОзачет) Тогда
				Продолжить;
			...
			// Сумма
			Если СтраницаРаздела.ДанныеДопСтрок <> Неопределено
				И СтраницаРаздела.ДанныеДопСтрок.Количество() = 1 Тогда
				Если Не ЗначениеЗаполнено(ДанныеСтраницы.П002100001050_1) Тогда
					НайденныеСтроки = ПереплатаНДФЛ.НайтиСтроки(Новый Структура("КБК, ОКТМО", КБКзачет, ОКТМОзачет));
					Если НайденныеСтроки.Количество() = 1 Тогда
						ДанныеСтраницы.П002100001050_1 = НайденныеСтроки[0].Сумма;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		КБКвозврат = СокрЛП(ДанныеСтраницы.П002100001110);
		Если ЗначениеЗаполнено(КБКвозврат) Тогда
			// ОКТМО
			ОКТМОвозврат = ДанныеСтраницы.П002100001120;
			Если Не ЗначениеЗаполнено(ОКТМОвозврат) Тогда
				НайденныеСтроки =  ПереплатаНДФЛ.НайтиСтроки(Новый Структура("КБК", КБКвозврат));
				Если НайденныеСтроки.Количество() = 1 Тогда
					ОКТМОвозврат = НайденныеСтроки[0].ОКТМО;
					ДанныеСтраницы.П002100001120 = ОКТМОвозврат;
				КонецЕсли;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ОКТМОвозврат) Тогда
				Продолжить;
			...
			// Сумма
			Если Не ЗначениеЗаполнено(ДанныеСтраницы.П002100001100) Тогда
				НайденныеСтроки = ПереплатаНДФЛ.НайтиСтроки(Новый Структура("КБК, ОКТМО", КБКвозврат, ОКТМОвозврат));
				Если НайденныеСтроки.Количество() = 1 Тогда
					ДанныеСтраницы.П002100001100 = НайденныеСтроки[0].Сумма;
				КонецЕсли;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ДанныеСтраницы.П002100001140)
				И Не ЗначениеЗаполнено(ДанныеСтраницы.П002100001150)
				И Не ЗначениеЗаполнено(ДанныеСтраницы.П002100001170) Тогда
				
				РеквизитыОсновногоБанковскогоСчета = РеквизитыОсновногоБанковскогоСчета(
					СтруктураРеквизитовФормы.Организация, ДатаПодписи);
				Если РеквизитыОсновногоБанковскогоСчета <> Неопределено Тогда
					ДанныеСтраницы.П002100001140 = РеквизитыОсновногоБанковскогоСчета.БанкСчетНаимБанка;
					ДанныеСтраницы.П002100001150 = РеквизитыОсновногоБанковскогоСчета.БанкСчетБИКБанка;
					ДанныеСтраницы.П002100001160 = РеквизитыОсновногоБанковскогоСчета.БанкСчетВид;
					ДанныеСтраницы.П002100001170 = РеквизитыОсновногоБанковскогоСчета.БанкСчетНомер;
				КонецЕсли;
		...
		Если СтраницаРаздела.АктивнаяСтраница Тогда
			Для Каждого КлючИЗначение Из ДанныеСтраницы Цикл
				...
				Область = ТабличныйДокумент.Области.Найти(КлючИЗначение.Ключ);
				Если Область <> Неопределено Тогда
					Область.Значение = КлючИЗначение.Значение;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция РеквизитыОсновногоБанковскогоСчета(Организация, ДатаПодписи)
	
	Если ЗначениеЗаполнено(Организация) Тогда
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
			Организация, ДатаПодписи, "БанкСчетНомер, БанкСчетНаимБанка, БанкСчетБИКБанка");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	
	БанкСчетВид = Отчеты.РегламентированныйОтчет3НДФЛ.ВидБанковскогоСчета(СведенияОбОрганизации.БанкСчетНомер);
	СведенияОбОрганизации.Вставить("БанкСчетВид", БанкСчетВид);
			...
			Если СтрокиДоходов.Количество() > 0 Тогда
				...
				Для Каждого СтрокаВычета Из СтрокиВычетов Цикл
					...
					Если СтрокаВычета.ВидВычета = "СуммаРубСтоимостиИмуществаПриЛиквидацииКИК"
						...
						Или СтрокаВычета.ВидВычета = "СуммаРубЦенныеБумагиКИК"
					...
					ИначеЕсли СтрокаВычета.ВидВычета = "ОбщаяСуммаВычетаИП"
						...
						Или СтрокаВычета.ВидВычета = "ОбщаяСуммаСтандартныхВычетовПредоставленныхНалоговымиАгентами"
...
&НаСервереБезКонтекста
Функция КодыВидовДоходовЗаПределамиРФ(Знач ВыбраннаяФорма)
	
	Возврат Отчеты.РегламентированныйОтчет3НДФЛ.КодыВидовДоходовЗаПределамиРФ(ВыбраннаяФорма);
...
Процедура СобратьДанныеВРаздел1Сервер()
	...
	КодыБК = КБК(СтруктураРеквизитовФормы.мВыбраннаяФорма);
	КБКИП = КодыБК.ДоходыОтПредпринимательскойДеятельности;
	
	// Очистим результат прошлого распределения.
	Для Каждого СтрокаДохода Из СводДоходов Цикл
		СтрокаДохода.РезультатРасчетов = 0;
	КонецЦикла;
	
	// Уменьшаем полученные доходы на сумму вычетов и расходов, уменьшающих налог.
	// Дополнительно, учитываем платежи по патентам и торговому сбору, на которые тоже можно уменьшить налог.
	// Важно распределить по доходам именно вычеты и расходы, а не результат Раздела 2, как было в предыдущих формах отчета.
	...
	ПорядокРаспределенияПоКБК = Новый Массив;
	ПорядокРаспределенияПоКБК.Добавить(КБКИП);
	ПорядокРаспределенияПоКБК.Добавить(КодыБК.ДоходыПриПолученииКоторыхНеУдержанНалог);
	ПорядокРаспределенияПоКБК.Добавить(КодыБК.ДоходыПолученныеОтНалоговыхАгентов);
	ПорядокРаспределенияПоКБК.Добавить(КодыБК.ФиксированныеПлатежиИностранныхГраждан);
	ПорядокРаспределенияПоКБК.Добавить(КодыБК.ДоходыСПрибылиКИК);
	
	ОтборДоходов = Новый Структура("ВидДохода, Ставка, КБК");
	ИтогиРаздела2 = ИтогиРаздела2();
	
	ОбщаяСуммаВычетаИП = СводВычетов.Итог("СуммаВычетаДляРаздела2")
		- СуммаВычета("СуммаДоходовНеПодлежащаяНалогообложению");
	
	Для Индекс = 0 По ПорядокРаспределенияПоКБК.ВГраница() Цикл
		
		Для Каждого ДанныеРаздела2 Из ИтогиРаздела2 Цикл
			
			Если ДанныеРаздела2.ОстатокКУплате = 0 И ДанныеРаздела2.ОстатокКВозмещению = 0 Тогда
				Продолжить;
			КонецЕсли;
			
			ЗаполнитьЗначенияСвойств(ОтборДоходов, ДанныеРаздела2);
			ОтборДоходов.КБК = ПорядокРаспределенияПоКБК[Индекс];
			
			НайденныеСтроки = СводДоходов.НайтиСтроки(ОтборДоходов);
			
			Для Каждого СтрокаДохода Из НайденныеСтроки Цикл
				
				// Уменьшаем сумму дохода на налоговые вычеты
				Если СтрокаДохода.КБК = КБКИП Тогда
					СуммаВычета = Макс(0, Мин(ДанныеРаздела2.НалоговыйВычет, ОбщаяСуммаВычетаИП));
				Иначе
					СуммаВычета = Макс(0, ДанныеРаздела2.НалоговыйВычет);
				КонецЕсли;
				
				МожноУменьшитьВычет = Макс(0, Мин(СтрокаДохода.СуммаДохода, СуммаВычета));
				НалоговаяБаза = СтрокаДохода.СуммаДохода - МожноУменьшитьВычет;
				ДанныеРаздела2.НалоговыйВычет = ДанныеРаздела2.НалоговыйВычет - МожноУменьшитьВычет;
				
				// Уменьшаем на сумму расходов
				МожноУменьшитьРасходы = Макс(0, Мин(НалоговаяБаза, ДанныеРаздела2.РасходыУменьшающиеНалог));
				НалоговаяБаза = НалоговаяБаза - МожноУменьшитьРасходы;
				ДанныеРаздела2.РасходыУменьшающиеНалог = ДанныеРаздела2.РасходыУменьшающиеНалог - МожноУменьшитьРасходы;
				
				// Считаем налог
				НалогИсчисленный = НалоговаяБаза * СтрокаДохода.Ставка / 100;
				
				Если СтрокаДохода.КБК = КБКИП Тогда
					НалогУплачен = 0;
				Иначе
					НалогУплачен = СтрокаДохода.УплаченоНалога
						+ СтрокаДохода.УдержаноНалога
						+ СтрокаДохода.НалогПринимаемыйКУчету
						+ ДанныеРаздела2.АвансыПоПатенту; // возмещается только то, что уплачено или удержано
					ДанныеРаздела2.АвансыПоПатенту = 0;
				КонецЕсли;
				
				СтрокаДохода.РезультатРасчетов = НалогИсчисленный - НалогУплачен;
				...
				Если СтрокаДохода.РезультатРасчетов > 0 Тогда
					...
					Если СтрокаДохода.КБК = КБКИП
						И (СтрокаДохода.РезультатРасчетов - СтруктураДанныхПриложение3Расчет.П002000001052) > 0 Тогда
						
						МожноУменьшитьПатентИП = Мин(СтрокаДохода.РезультатРасчетов, ДанныеРаздела2.ПлатежиПоПатентуИП);
						
						СтрокаДохода.РезультатРасчетов = СтрокаДохода.РезультатРасчетов - МожноУменьшитьПатентИП;
						ДанныеРаздела2.ПлатежиПоПатентуИП = ДанныеРаздела2.ПлатежиПоПатентуИП - МожноУменьшитьПатентИП;
						
						МожноУменьшитьТорговыйСбор = Мин(СтрокаДохода.РезультатРасчетов, ДанныеРаздела2.ТорговыйСбор);
						СтрокаДохода.РезультатРасчетов = СтрокаДохода.РезультатРасчетов - МожноУменьшитьТорговыйСбор;
						ДанныеРаздела2.ТорговыйСбор = ДанныеРаздела2.ТорговыйСбор - МожноУменьшитьТорговыйСбор;
	...
	// КБК доходов, полученных от налоговых агентов, и КБК доходов, с которых не удержан налог, можно тоже свернуть.
	// Но в дальнешем при заполнении раздела 1 выполняется условие - 
	// для возврата налога используется КБК ДоходыПолученныеОтНалоговыхАгентов
	// для уплаты налога подставляется КБК ДоходыПриПолученииКоторыхНеУдержанНалог
	Для Каждого СтрокаДохода Из СводДоходов Цикл
		Если СтрокаДохода.КБК = КодыБК.ДоходыПолученныеОтНалоговыхАгентов Тогда
			СтрокаДохода.КБК = КодыБК.ДоходыПриПолученииКоторыхНеУдержанНалог;
		КонецЕсли;
	КонецЦикла;
	
	ИтогоНачисленоНалогаПоПредпринимательскойДеятельности = 0;
	// Уменьшаем на сумму доходов, не подлежащую налогообложению
	СуммаДоходовНеПодлежащаяНалогообложению = СуммаВычета("СуммаДоходовНеПодлежащаяНалогообложению");
	Для Каждого СтрокаРасчета Из СводДоходов Цикл
		
		Если СуммаДоходовНеПодлежащаяНалогообложению > 0 Тогда
			СуммаКУменьшению = Мин(СтрокаРасчета.РезультатРасчетов, СуммаДоходовНеПодлежащаяНалогообложению * СтрокаРасчета.Ставка / 100);
			СтрокаРасчета.РезультатРасчетов = Окр(СтрокаРасчета.РезультатРасчетов - СуммаКУменьшению, 0);
			СуммаДоходовНеПодлежащаяНалогообложению = СуммаДоходовНеПодлежащаяНалогообложению
				- СуммаКУменьшению * 100 / СтрокаРасчета.Ставка;
		Иначе
			СтрокаРасчета.РезультатРасчетов = Окр(СтрокаРасчета.РезультатРасчетов, 0);
		КонецЕсли;
		
		Если СтрокаРасчета.КБК = КБКИП Тогда
			ИтогоНачисленоНалогаПоПредпринимательскойДеятельности =
				ИтогоНачисленоНалогаПоПредпринимательскойДеятельности + СтрокаРасчета.РезультатРасчетов;
		КонецЕсли;
	...
	ИсчисленоАвансов = СтруктураДанныхПриложение3Расчет.П002000001052;
	ИтогоСуммаНалога = СводДоходов.Итог("РезультатРасчетов");
	
	Для Каждого СтрокаРасчета Из ТаблицаРасчетов Цикл
		
		Если СтрокаРасчета.КБК = КБКИП Тогда
			// Доходы по предпринимательской деятельности указываются в п.2.
			Продолжить;
		КонецЕсли;
		...
		// Распределяем сумму исчисленного аванса
		Если ИтогоСуммаНалога <= ИсчисленоАвансов Тогда
			
			Если СтрокаРасчета.РезультатРасчетов > 0 Тогда
				СуммаРаспределить = Мин(СтрокаРасчета.РезультатРасчетов, ИсчисленоАвансов);
				СуммаКУплате = СтрокаРасчета.РезультатРасчетов - СуммаРаспределить;
				ИсчисленоАвансов = Макс(ИсчисленоАвансов - СуммаРаспределить, 0);
			ИначеЕсли СтрокаРасчета.РезультатРасчетов < 0 И ИтогоНачисленоНалогаПоПредпринимательскойДеятельности = 0 Тогда
				// Если налог за год необходимо возместить, но уже уплачен аванс, то добавляем всю сумму исчисленного аванса
				СуммаКУплате = СтрокаРасчета.РезультатРасчетов - ИсчисленоАвансов;
				ИсчисленоАвансов = 0;
			ИначеЕсли ИтогоНачисленоНалогаПоПредпринимательскойДеятельности = 0 Тогда
				// Если налог за год платить не надо, но аванс перечислен, то это сумма к возмещению
				СуммаКУплате = - ИсчисленоАвансов;
			КонецЕсли;
			
		Иначе
			СуммаКУплате = СтрокаРасчета.РезультатРасчетов;
		КонецЕсли;
		
		Если СуммаКУплате > 0 Тогда
			
			ДанныеСтраницыРасчета.П000100001010 = ПризнакиУплатыВозврата.Уплата;
			ДанныеСтраницыРасчета.П000100001040 = СуммаКУплате;
			ДанныеСтраницыРасчета.П000100001050 = 0;
			
		ИначеЕсли СуммаКУплате < 0 Тогда
			
			Если СтрокаРасчета.КБК = КодыБК.ДоходыПриПолученииКоторыхНеУдержанНалог Тогда
				// Можно вернуть только тот налог, что был уплачен
				ДанныеСтраницыРасчета.П000100001020 = КодыБК.ДоходыПолученныеОтНалоговыхАгентов;
			КонецЕсли;
			
			ДанныеСтраницыРасчета.П000100001010 = ПризнакиУплатыВозврата.Возврат;
			ДанныеСтраницыРасчета.П000100001040 = 0;
			ДанныеСтраницыРасчета.П000100001050 = -СуммаКУплате;
	...
	Если ТаблицаСтраницыРаздел1.Количество() = 0 Тогда
		...
		ЗаполнятьПункт2 = Истина;
	Иначе
		ТаблицаСтраницыРаздел1[0].АктивнаяСтраница  = Истина;
		ЗаполнятьПункт2 = ИтогоНачисленоНалогаПоПредпринимательскойДеятельности <> 0;
	КонецЕсли;
	
	Если ЗаполнятьПункт2 Тогда
		// Строки 060 - 170
		ПоследняяСтраницаРаздела1 = ТаблицаСтраницыРаздел1[ТаблицаСтраницыРаздел1.Количество()-1];
		ЗначенияПоказателей = ПоследняяСтраницаРаздела1.Данные[0].Значение;
		// Заполним КБК
		ЗначенияПоказателей.Вставить(
			"П000100001060",
			КБК(СтруктураРеквизитовФормы.мВыбраннаяФорма).ДоходыОтПредпринимательскойДеятельности);
		
		// 1 квартал
		ЗначенияПоказателей.Вставить("П000100001070", ОКТМОНалогоплательщика);
		ЗначенияПоказателей.Вставить("П000100001080", СтруктураДанныхПриложение3Расчет.П002000001050);
		
		// Полугодие
		ЗначенияПоказателей.Вставить("П000100001090", ОКТМОНалогоплательщика);
		АвансИсчисленный = СтруктураДанныхПриложение3Расчет.П002000001051
			- СтруктураДанныхПриложение3Расчет.П002000001050;
		Если АвансИсчисленный > 0 Тогда
			ЗначенияПоказателей.Вставить("П000100001100", АвансИсчисленный);
			ЗначенияПоказателей.Вставить("П000100001110", 0);
		Иначе
			ЗначенияПоказателей.Вставить("П000100001100", 0);
			ЗначенияПоказателей.Вставить("П000100001110", -АвансИсчисленный);
		КонецЕсли;
		
		// 9 Месяцев
		ЗначенияПоказателей.Вставить("П000100001120", ОКТМОНалогоплательщика);
		АвансИсчисленный = СтруктураДанныхПриложение3Расчет.П002000001052
			- СтруктураДанныхПриложение3Расчет.П002000001051;
		Если АвансИсчисленный > 0 Тогда
			ЗначенияПоказателей.Вставить("П000100001130", АвансИсчисленный);
			ЗначенияПоказателей.Вставить("П000100001140", 0);
		Иначе
			ЗначенияПоказателей.Вставить("П000100001130", 0);
			ЗначенияПоказателей.Вставить("П000100001140", -АвансИсчисленный);
		КонецЕсли;
		
		// Год
		ЗначенияПоказателей.Вставить("П000100001150", ОКТМОНалогоплательщика);
		НалогЗаГод = ИтогоНачисленоНалогаПоПредпринимательскойДеятельности - ИсчисленоАвансов;
		
		Если НалогЗаГод > 0 Тогда
			ЗначенияПоказателей.Вставить("П000100001160", НалогЗаГод);
			ЗначенияПоказателей.Вставить("П000100001170", 0);
		Иначе
			ЗначенияПоказателей.Вставить("П000100001160", 0);
			ЗначенияПоказателей.Вставить("П000100001170", -НалогЗаГод);
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Функция ИтогиРаздела2()
	
	ТипЧисло = ОбщегоНазначения.ОписаниеТипаЧисло(15, 2);
	СводДанныхРаздела2 = Новый ТаблицаЗначений;
	СводДанныхРаздела2.Колонки.Добавить("ВидДохода", ОбщегоНазначения.ОписаниеТипаСтрока(0));
	СводДанныхРаздела2.Колонки.Добавить("Ставка", ТипЧисло);
	
	ПоказателиСуммы = Новый Массив;
	ПоказателиСуммы.Добавить("НалоговыйВычет");
	ПоказателиСуммы.Добавить("РасходыУменьшающиеНалог");
	ПоказателиСуммы.Добавить("ТорговыйСбор");
	ПоказателиСуммы.Добавить("АвансыПоПатенту");
	ПоказателиСуммы.Добавить("ПлатежиПоПатентуИП");
	ПоказателиСуммы.Добавить("ОстатокКУплате");
	ПоказателиСуммы.Добавить("ОстатокКВозмещению");
	ПоказателиСуммы.Добавить("УплаченоАвансов");
	
	Для Каждого ТекущийПоказатель Из ПоказателиСуммы Цикл
		СводДанныхРаздела2.Колонки.Добавить(ТекущийПоказатель, ТипЧисло);
	КонецЦикла;
	...
	Для Каждого СтраницаРаздела Из ТаблицаСтраницыРаздел2 Цикл
		...
		СтрокаДанных = СводДанныхРаздела2.Добавить();
		СтрокаДанных.ВидДохода = ДанныеСтраницы.П000200001002;
		СтрокаДанных.Ставка = ДанныеСтраницы.П000200001001;
		
		СтрокаДанных.НалоговыйВычет = ДанныеСтраницы.П000200001040;
		СтрокаДанных.РасходыУменьшающиеНалог = ДанныеСтраницы.П000200001050;
		СтрокаДанных.ТорговыйСбор = ДанныеСтраницы.П000200001100;
		СтрокаДанных.АвансыПоПатенту = ДанныеСтраницы.П000200001120;
		СтрокаДанных.ПлатежиПоПатентуИП = ДанныеСтраницы.П000200001140;
		СтрокаДанных.ОстатокКУплате = ДанныеСтраницы.П000200001150;
		СтрокаДанных.ОстатокКВозмещению = ДанныеСтраницы.П000200001160;
		СтрокаДанных.УплаченоАвансов = ДанныеСтраницы.П000200001110;
		
	КонецЦикла;
	
	СводДанныхРаздела2.Свернуть("ВидДохода, Ставка", СтрСоединить(ПоказателиСуммы, ","));
	
	Возврат СводДанныхРаздела2;
	
КонецФункции

&НаСервере
Функция СуммаВычета(ВидВычета)
	
	Отбор = Новый Структура;
	Отбор.Вставить("ВидВычета", ВидВычета);
	
	СтрокиОтбора = СводВычетов.НайтиСтроки(Отбор);
	
	Результат = 0;
	Для Каждого ТекущаяСтрока Из СтрокиОтбора Цикл
		Результат = Результат + ТекущаяСтрока.СуммаВычетаДляРаздела2;
	КонецЦикла;
...
Функция ВыгрузкаВозможна(ДеревоВыгрузки)
	...
	ПоказателиРаздела = Отчеты.РегламентированныйОтчет3НДФЛ.ПолучитьПоказателиРаздела(
		ДеревоВыгрузки, "Раздел1Приложение", СтруктураРеквизитовФормы.мВыбраннаяФорма);
	
	Для Индекс = 0 По мДанныеОтчета.Раздел1Приложение.Количество() - 1 Цикл
		ДанныеСтраницы = мДанныеОтчета.Раздел1Приложение[Индекс].Данные[0].Значение;
		Если Не Отчеты.РегламентированныйОтчет3НДФЛ.СтраницаПуста(ДанныеСтраницы, ПоказателиРаздела) Тогда
			ПроверитьСтраницуПриложенияКРазделу1(ДанныеСтраницы, Индекс + 1);
		КонецЕсли;
	КонецЦикла;
	...
	ПоказателиРаздела = Отчеты.РегламентированныйОтчет3НДФЛ.ПолучитьПоказателиРаздела(
		ДеревоВыгрузки, "Приложение5", СтруктураРеквизитовФормы.мВыбраннаяФорма);
	
	Если Не Отчеты.РегламентированныйОтчет3НДФЛ.СтраницаПуста(мДанныеОтчета.Приложение5, ПоказателиРаздела) Тогда
...
&НаСервереБезКонтекста
Функция ПоказателиРаздела1ИсключаяАвансы()
	
	Показатели = Новый Соответствие;
	Показатели.Вставить("ПризнакУплаты", "П000100001010_%1");
...
&НаСервереБезКонтекста
Функция ПоказателиРаздела1ПоАвансам()
	
	МассивПоказателейПоАвансам = Новый Массив;
	МассивПоказателейПоАвансам.Добавить("П000100001060");
	МассивПоказателейПоАвансам.Добавить("П000100001070");
	МассивПоказателейПоАвансам.Добавить("П000100001080");
	МассивПоказателейПоАвансам.Добавить("П000100001090");
	МассивПоказателейПоАвансам.Добавить("П000100001100");
	МассивПоказателейПоАвансам.Добавить("П000100001110");
	МассивПоказателейПоАвансам.Добавить("П000100001120");
	МассивПоказателейПоАвансам.Добавить("П000100001130");
	МассивПоказателейПоАвансам.Добавить("П000100001140");
	МассивПоказателейПоАвансам.Добавить("П000100001150");
	МассивПоказателейПоАвансам.Добавить("П000100001160");
	МассивПоказателейПоАвансам.Добавить("П000100001170");
	
	Возврат МассивПоказателейПоАвансам;
...
Процедура ПроверитьСтраницуРаздела1(ДанныеСтраницы, НомерСтраницы)
	...
	ПоказателиРаздела1ИсключаяАвансы = ПоказателиРаздела1ИсключаяАвансы();
	ЗначенияПоказателей = Новый Структура;
	НомерСтроки = 1;
	
	Пока ДанныеСтраницы.Свойство(СтрШаблон("П000100001010_%1", НомерСтроки)) Цикл
		
		Для Каждого Показатель Из ПоказателиРаздела1ИсключаяАвансы Цикл
		...
		// Код ОКТМО
		ИмяЯчейкиОКТМО = СтрШаблон(ПоказателиРаздела1ИсключаяАвансы["КодОТКМО"], НомерСтроки);
	...
	ЕстьДанныеПоАвансам = Ложь;
	МассивПоказателейПоАвансам = ПоказателиРаздела1ПоАвансам();
	Для Каждого ИмяЯчейки Из МассивПоказателейПоАвансам Цикл
		Если ЗначениеЗаполнено(ДанныеСтраницы[ИмяЯчейки]) Тогда
			ЕстьДанныеПоАвансам = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ЕстьДанныеПоАвансам Тогда
		ЯчейкиОКТМО = Новый Массив;
		ЯчейкиОКТМО.Добавить("П000100001070");
		ЯчейкиОКТМО.Добавить("П000100001090");
		ЯчейкиОКТМО.Добавить("П000100001120");
		ЯчейкиОКТМО.Добавить("П000100001150");
		Для Каждого ИмяЯчейки Из ЯчейкиОКТМО Цикл
			// Код ОКТМО
			Если ПустаяСтрока(ДанныеСтраницы[ИмяЯчейки]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
					СтрШаблон(НСтр("ru = 'На странице %1 Раздела 1 не указан код ОКТМО!';
									|en = 'На странице %1 Раздела 1 не указан код ОКТМО!'"), НомерСтраницы),
					"Раздел1",
					ИмяЯчейки,
					НомерСтраницы);
			Иначе
				ДлинаОКТМО = СтрДлина(СокрЛП(ДанныеСтраницы[ИмяЯчейки]));
				Если ДлинаОКТМО <> 8 И ДлинаОКТМО <> 11 Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
...
&НаСервере
Процедура ПроверитьСтраницуПриложенияКРазделу1(ДанныеСтраницы, НомерСтраницы)
	
	ПоказательОтчетГодЧисло = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураДанныхТитульный, "ОтчетГод", 0);
	ПоказательПериодКод = СтрШаблон("ГД.00.%1", Формат(ПоказательОтчетГодЧисло, "ЧН='""""'; ЧГ=0;"));
	
	МассивОКТМОРаздела1 = Новый Массив;
	Для Индекс = 0 По мДанныеОтчета.Раздел1.Количество() - 1 Цикл
		ДанныеРаздела1 = мДанныеОтчета.Раздел1[Индекс].Данные[0].Значение;
		НомерСтроки = 1;
		ЗначениеОКТМО = Неопределено;
		Пока ДанныеРаздела1.Свойство(СтрШаблон("П000100001030_%1", НомерСтроки), ЗначениеОКТМО) Цикл
			Если Не ПустаяСтрока(ЗначениеОКТМО) Тогда
				МассивОКТМОРаздела1.Добавить(СокрЛП(ЗначениеОКТМО));
			КонецЕсли;
			НомерСтроки = НомерСтроки + 1;
		КонецЦикла;
		
		Если ДанныеРаздела1.Свойство("П000100001070") И ЗначениеЗаполнено(ДанныеРаздела1.П000100001070) Тогда
			МассивОКТМОРаздела1.Добавить(ДанныеРаздела1.П000100001070);
		КонецЕсли;
	КонецЦикла;
	
	// Проверка заявления о зачете налога	
	// Код периода
	ЯчейкаКодПериода = "П002100001030";
	Если ЗначениеЗаполнено(ДанныеСтраницы[ЯчейкаКодПериода]) 
		И ДанныеСтраницы[ЯчейкаКодПериода] <> ПоказательПериодКод Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 код периода не соответствует периоду декларации!';
							|en = 'На странице %1 Приложения к Разделу 1 код периода не соответствует периоду декларации!'"), НомерСтраницы),
				"Раздел1Приложение",
				ЯчейкаКодПериода,
				НомерСтраницы);
	КонецЕсли;
	...
	КБКЗачет = ДанныеСтраницы["П002100001010"];
	Если ЗначениеЗаполнено(КБКЗачет) Тогда 
		
		// Номер заявления
		Если Не ЗначениеЗаполнено(ДанныеСтраницы["П002100001005"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 не указан номер заявления!';
								|en = 'На странице %1 Приложения к Разделу 1 не указан номер заявления!'"), НомерСтраницы),
				"Раздел1Приложение",
				"П002100001005",
				НомерСтраницы);
		КонецЕсли;
		...
		// ОКТМО
		ЯчейкаОКТМО = "П002100001020";
		Если ПустаяСтрока(ДанныеСтраницы[ЯчейкаОКТМО]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
					СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 не указан код ОКТМО!';
									|en = 'На странице %1 Приложения к Разделу 1 не указан код ОКТМО!'"), НомерСтраницы),
					"Раздел1Приложение",
					ЯчейкаОКТМО,
					НомерСтраницы);
		Иначе
			ДлинаОКТМО = СтрДлина(СокрЛП(ДанныеСтраницы[ЯчейкаОКТМО]));
			Если ДлинаОКТМО <> 8 И ДлинаОКТМО <> 11 Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
					СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 указан код ОКТМО некорректной длины!';
									|en = 'На странице %1 Приложения к Разделу 1 указан код ОКТМО некорректной длины!'"), НомерСтраницы),
					"Раздел1Приложение",
					ЯчейкаОКТМО,
					НомерСтраницы);
			КонецЕсли;
			Если МассивОКТМОРаздела1.Найти(СокрЛП(ДанныеСтраницы[ЯчейкаОКТМО])) = Неопределено Тогда
				 РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
					СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 код ОКТМО не соответствует ни одному из кодов ОКТМО, указанных в строках 030 Раздела 1!';
									|en = 'На странице %1 Приложения к Разделу 1 код ОКТМО не соответствует ни одному из кодов ОКТМО, указанных в строках 030 Раздела 1!'"), НомерСтраницы),
					"Раздел1Приложение",
					ЯчейкаОКТМО,
					НомерСтраницы);
			КонецЕсли;
	...
	// Проверка заявления о возврате налога	
	// Код периода
	ЯчейкаКодПериода = "П002100001130";
	Если ЗначениеЗаполнено(ДанныеСтраницы[ЯчейкаКодПериода]) 
		И ДанныеСтраницы[ЯчейкаКодПериода] <> ПоказательПериодКод Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 код периода не соответствует периоду декларации!';
	...
	КБКВозврат = ДанныеСтраницы["П002100001110"];
	Если ЗначениеЗаполнено(КБКВозврат) Тогда
		
		// Номер заявления
		Если Не ЗначениеЗаполнено(ДанныеСтраницы["П002100001095"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 не указан номер заявления!';
								|en = 'На странице %1 Приложения к Разделу 1 не указан номер заявления!'"), НомерСтраницы),
				"Раздел1Приложение",
		...
		ЯчейкаФамилия = "ЗаявВозвратФамилия";
		Если ЗначениеЗаполнено(ДанныеСтраницы[ЯчейкаФамилия])
			И ДанныеСтраницы[ЯчейкаФамилия] <> СтруктураДанныхТитульный.Фамилия Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 фамилия получателя не соответствует фамилии, указанной на Титульном листе!';
								|en = 'На странице %1 Приложения к Разделу 1 фамилия получателя не соответствует фамилии, указанной на Титульном листе!'"), НомерСтраницы),
				"Раздел1Приложение",
				ЯчейкаФамилия,
				НомерСтраницы);
		КонецЕсли;
		
		ЯчейкаИмя = "ЗаявВозвратИмя";
		Если ЗначениеЗаполнено(ДанныеСтраницы[ЯчейкаИмя])
			И ДанныеСтраницы[ЯчейкаИмя] <> СтруктураДанныхТитульный.Имя Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 имя получателя не соответствует имени, указанному на Титульном листе!';
								|en = 'На странице %1 Приложения к Разделу 1 имя получателя не соответствует имени, указанному на Титульном листе!'"), НомерСтраницы),
				"Раздел1Приложение",
				ЯчейкаИмя,
				НомерСтраницы);
		КонецЕсли;
		
		ЯчейкаОтчество = "ЗаявВозвратОтчество";
		Если ЗначениеЗаполнено(ДанныеСтраницы[ЯчейкаОтчество]) 
			И ДанныеСтраницы[ЯчейкаОтчество] <> СтруктураДанныхТитульный.Отчество Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 отчество получателя не соответствует отчеству, указанному на Титульном листе!';
								|en = 'На странице %1 Приложения к Разделу 1 отчество получателя не соответствует отчеству, указанному на Титульном листе!'"), НомерСтраницы),
				"Раздел1Приложение",
				ЯчейкаОтчество,
				НомерСтраницы);
		КонецЕсли;
...
Процедура ПроверитьПриложение5(ДанныеСтраницы)
	...
	Если ДанныеСтраницы.П000900001090 <> 0 Тогда
		...
		ТекстСообщения = СообщениеПроверкиСуммыПожертвований(ДанныеСтраницы.П000900001090,
			СтруктураРеквизитовФормы.мВыбраннаяФорма);
		
		Если ЗначениеЗаполнено(ТекстСообщения) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, ТекстСообщения, "Приложение5", "П000900001090");
		КонецЕсли;
...
&НаСервере
Функция СообщениеПроверкиСуммыПожертвований(ТекущееЗначение, ВыбраннаяФорма, УчитыватьМинимальныйПроцент = Ложь)
	
	ТекстСообщения = "";
	СтавкаПоУмолчанию = Отчеты.РегламентированныйОтчет3НДФЛ.НалоговыеСтавки(ВыбраннаяФорма).ПоУмолчанию;
	
	Если УчитыватьМинимальныйПроцент Тогда
		ПределВычета = Отчеты.РегламентированныйОтчет3НДФЛ.ПределыВычетов(ВыбраннаяФорма).ПожертвованияПроцентДохода;
	Иначе
		ПределВычета = Отчеты.РегламентированныйОтчет3НДФЛ.ПределыВычетов(ВыбраннаяФорма).ПожертвованияФондыПроцентДохода;
	КонецЕсли;
	
	СуммаОблагаемогоДоходаПоСтавке13 = СуммаОблагаемогоДоходаПоСтавке(ЭтотОбъект, СтавкаПоУмолчанию);
	Если ТекущееЗначение > Окр(СуммаОблагаемогоДоходаПоСтавке13 * ПределВычета, 2) Тогда
		
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Сумма вычета в виде пожертвований превышает %1%% от общей суммы дохода %2';
										|en = 'Сумма вычета в виде пожертвований превышает %1%% от общей суммы дохода %2'"),
			ПределВычета * 100, СуммаОблагаемогоДоходаПоСтавке13);
		
		Если УчитыватьМинимальныйПроцент Тогда
			
			ПроцентПожертвованияВФонды = Отчеты.РегламентированныйОтчет3НДФЛ.ПределыВычетов(
				ВыбраннаяФорма).ПожертвованияФондыПроцентДохода;
			
			ТекстСообщения = СтрШаблон(НСтр("ru = '%1.
				|Сумма вычета может быть увеличена до %2%% законом субъекта РФ.';
				|en = '%1.
				|Сумма вычета может быть увеличена до %2%% законом субъекта РФ.'"),
				ТекстСообщения, ПроцентПожертвованияВФонды * 100);
...
Процедура ЗаполнитьДаннымиУзелРаздела(Родитель, Узел)
	...
	Если Узел.Код = "РасчАвансПлат" Тогда
		ЗаполнитьДаннымиУзелРаздела_РасчАвансПлат(Родитель, Узел);
		Возврат;
	КонецЕсли;
	...
	Если Отчеты.РегламентированныйОтчет3НДФЛ.СтраницаПуста(мДанныеОтчета[Узел.Раздел], ПоказателиРаздела, Узел.Многострочность) Тогда
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьДаннымиУзелРаздела_РасчАвансПлат(Родитель, Узел)
	
	ПоказателиРаздела = Отчеты.РегламентированныйОтчет3НДФЛ.ПолучитьПоказателиРаздела(
		Родитель.Владелец(), Узел.Раздел, СтруктураРеквизитовФормы.мВыбраннаяФорма);
	
	Если Отчеты.РегламентированныйОтчет3НДФЛ.СтраницаПуста(
	...
	ПоказателиПриложения3Расчет = ЗначенияПоказателейПриложения3Расчет(
		мДанныеОтчета[Узел.Раздел], АдресКлючейПоказателей);
	
	Если ПоказателиПриложения3Расчет.ДоходыИП1Квартал <> 0
		Или ПоказателиПриложения3Расчет.ПрофВычетИП1Квартал <> 0 Тогда
		
		УказатьОбязательностьВыгрузки("НалБазаИсч", "ЗаКв", ЭкземплярУзла);
		УказатьОбязательностьВыгрузки("СумИсчАванс", "ЗаКв", ЭкземплярУзла);
	КонецЕсли;
	
	Если ПоказателиПриложения3Расчет.ДоходыИППолугодие <> 0
		Или ПоказателиПриложения3Расчет.ПрофВычетИППолугодие <> 0 Тогда
		
		УказатьОбязательностьВыгрузки("НалБазаИсч", "ЗаПг", ЭкземплярУзла);
		УказатьОбязательностьВыгрузки("СумИсчАванс", "ЗаПг", ЭкземплярУзла);
	КонецЕсли;
	
	Если ПоказателиПриложения3Расчет.ДоходыИП9Месяцев <> 0
		Или ПоказателиПриложения3Расчет.ПрофВычетИП9Месяцев <> 0 Тогда
		
		УказатьОбязательностьВыгрузки("НалБазаИсч", "За9м", ЭкземплярУзла);
		УказатьОбязательностьВыгрузки("СумИсчАванс", "За9м", ЭкземплярУзла);
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура УказатьОбязательностьВыгрузки(ИмяЭлемента, ИмяСвойства, ЭкземплярУзла);
	
	СтрокаЭлемента = ЭкземплярУзла.Строки.НайтиСтроки(Новый Структура("Код", ИмяЭлемента));
	Если СтрокаЭлемента.Количество() <> 0 Тогда
		ОбязательноеСвойство = СтрокаЭлемента[0].Строки.НайтиСтроки(Новый Структура("Код", ИмяСвойства));
		Если ОбязательноеСвойство.Количество() <> 0 Тогда
			ОбязательноеСвойство[0].Обязательность = "О";
		КонецЕсли;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЗначенияПоказателейПриложения3Расчет(ДанныеПриложения, АдресКлючейПоказателей)
	
	ИдентификаторыПоказателей = Новый Массив;
	ИдентификаторыПоказателей.Добавить("ДоходыИП1Квартал");
	ИдентификаторыПоказателей.Добавить("ДоходыИППолугодие");
	ИдентификаторыПоказателей.Добавить("ДоходыИП9Месяцев");
	ИдентификаторыПоказателей.Добавить("ПрофВычетИП1Квартал");
	ИдентификаторыПоказателей.Добавить("ПрофВычетИППолугодие");
	ИдентификаторыПоказателей.Добавить("ПрофВычетИП9Месяцев");
	...
	Если ЭтоАдресВременногоХранилища(АдресКлючейПоказателей) Тогда
		
		КлючиПоказателей = ПолучитьИзВременногоХранилища(АдресКлючейПоказателей);
		
		ПоказателиПриложения = КлючиПоказателей.НайтиСтроки(Новый Структура("ИмяСтраницы", "Приложение3Расчет"));
		
		Для Каждого ТекущийПараметр Из ПоказателиПриложения Цикл
			
			Если ИдентификаторыПоказателей.Найти(ТекущийПараметр.Ключ) <> Неопределено Тогда
				Результат.Вставить(ТекущийПараметр.Ключ, ДанныеПриложения[ТекущийПараметр.ИмяПоля]);
			КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ДанныеРазделов.Вставить("Приложение3Расчет", СтруктураДанныхПриложение3Расчет);
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Контейнер.Вставить("Приложение3Расчет", СтруктураДанныхПриложение3Расчет);
