...
Процедура ОбработкаВыбораИФНСЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено Тогда
		КодНО = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(РезультатВыбора.КодНО, 4);
		ТабличныйДокумент.Области["КодИМНС"].Значение = КодНО;
		СохранитьЗначениеПоказателяВСтруктуре(ЭтотОбъект, "Титульный", "КодИМНС", КодНО);
...
Процедура СобратьДанныеВРаздел1Сервер()
	...
	ОтборДоходов = Новый Структура("ВидДохода, Ставка");
	...
	Для Каждого СтраницаРаздела Из ТаблицаСтраницыРаздел2 Цикл
		...
		Если ДанныеСтраницы.П000200001001 = 0
			Или ДанныеСтраницы.П000200001010 + ДанныеСтраницы.П000200001051 = 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		// Находим строки доходов по указанной ставке
		ОтборДоходов.ВидДохода = ДанныеСтраницы.П000200001002;
		ОтборДоходов.Ставка = ДанныеСтраницы.П000200001001;
		
		СтрокиДоходов = СводДоходов.НайтиСтроки(ОтборДоходов);
		Для Каждого СтрокаДохода Из СтрокиДоходов Цикл
			...
			Если ОстатокКУплате > 0 Тогда
				...
				МожноРаспределить = СтрокаДохода.НачисленоНалога - СтрокаДохода.УдержаноНалога - СтрокаДохода.РезультатРасчетов;
				СтрокаДохода.РезультатРасчетов = СтрокаДохода.РезультатРасчетов + Макс(0, Мин(ОстатокКУплате, МожноРаспределить));
			...
			Если ОстатокКВозмещению > 0 Тогда
				...
				// Налог к возмещению пишем со знаком минус для получения итоговой суммы при сворачивании по КБК и ОКТМО
				СтрокаДохода.РезультатРасчетов =
					СтрокаДохода.РезультатРасчетов - Мин(ОстатокКВозмещению, НалогУплачен);
...
Процедура ПроверитьСтраницуРаздела1(ДанныеСтраницы, НомерСтраницы)
	...
	НомерСтроки = 1;
	ШаблонЯчейкиКодОТКМО = "П000100001030_%1";
	
	ИмяЯчейки = СтрШаблон(ШаблонЯчейкиКодОТКМО, НомерСтроки);
	Пока ДанныеСтраницы.Свойство(ИмяЯчейки) Цикл
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого ОписаниеРаздела Из мСтруктураМногостраничныхРазделов Цикл
		ДанныеРазделов.Вставить(ОписаниеРаздела.Ключ, ЭтаФорма[ОписаниеРаздела.Значение]);
	КонецЦикла;
