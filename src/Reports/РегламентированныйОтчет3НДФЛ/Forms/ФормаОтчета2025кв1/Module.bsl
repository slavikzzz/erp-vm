...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Иначе
		...
		ПреобразоватьЗагруженныйРаздел1();
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ТекстВыбора, АдресТаблицыВыбора, СтандартнаяОбработка = Ложь, КолонкаПоиска = "Код") Экспорт
	...
	Если ПоказательВыводитсяВНесколькихЯчейках(Область.Имя) Тогда
		
		ПозицияРазделителя = СтрНайти(Область.Имя, РазделительМногосрочнойЯчейки());
		КодПоказателя = Область.Имя;
		ИндексОбласти = Прав(Область.Имя, СтрДлина(Область.Имя) - ПозицияРазделителя);
		ИсходноеЗначение = Область.Значение;
		
	ИначеЕсли Область.Имя = КодПоказателя
...
Функция ПоказательВыводитсяВНесколькихЯчейках(ИмяОбласти)
	
	Результат = Ложь;
	
	Если СтрНайти(ИмяОбласти, РазделительМногосрочнойЯчейки()) = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	
	МногосрочныеЯчейки = Новый Массив;
	МногосрочныеЯчейки.Добавить("П001200001051");
	МногосрочныеЯчейки.Добавить("П001200001052");
	
	Для Каждого ТекущаяЯчейка Из МногосрочныеЯчейки Цикл
		Если СтрНайти(ИмяОбласти, ТекущаяЯчейка) > 0 Тогда
			Результат = Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция РазделительМногосрочнойЯчейки()
...
Процедура РассчитатьИтогиЛиста(Форма, ИмяЛиста = Неопределено)
	...
	ИначеЕсли ЭтоРазделПриложения5(ИмяЛиста) Тогда
		...
		ПоказателиЛистаПродолжение.П000900001180 = Мин(ПределСоциальногоВычета,
			...
			+ ПоказателиЛистаПродолжение.П000900001161
...
Функция СводВычетов()
	...
	ДолгосрочныеСбереженияНПФ = СтруктураДанныхПриложение5Продолжение.П000900001240;
	Если ДолгосрочныеСбереженияНПФ <> 0 Тогда
		СтрокаВычета = СводВычетов.Добавить();
		СтрокаВычета.ВидВычета = ВидыВычетов.ДолгосрочныеСбереженияНПФ;
		СтрокаВычета.СуммаНалоговыхВычетов = ДолгосрочныеСбереженияНПФ;
		СтрокаВычета.ВидДохода = ""; // Подлежит распределению
	КонецЕсли;
	
	ДолгосрочныеСбереженияИИС = СтруктураДанныхПриложение5Продолжение.П000900001250;
	Если ДолгосрочныеСбереженияИИС <> 0 Тогда
		СтрокаВычета = СводВычетов.Добавить();
		СтрокаВычета.ВидВычета = ВидыВычетов.ДолгосрочныеСбереженияИИС;
		СтрокаВычета.СуммаНалоговыхВычетов = ДолгосрочныеСбереженияИИС;
		СтрокаВычета.ВидДохода = ""; // Подлежит распределению
	КонецЕсли;
	...
	Для Каждого СтраницаЛиста Из ТаблицаСтраницыПриложение8 Цикл
		...
		Для Каждого ДанныеЛиста Из СтраницаЛиста.Данные Цикл
			
			СтрокаЛиста = ДанныеЛиста.Значение;
			...
			ЦенныеБумагиСуммаУбытка = СуммаМногострочногоПоказателя("П001200001052", СтрокаЛиста);
...
Процедура Расчет(Форма, ИмяРаздела = "", ИмяОбласти = "")
	...
	ИначеЕсли ЭтоРазделПриложения5(ИмяРаздела) Тогда
		...
		ИначеЕсли ИмяОбласти = "П000900001025" Тогда // Сумма стандартного налогового вычета в соответствии с пп.2.1 п.1 ст.218 НК РФ
			МаксимальноеЗначение = ПределВычета("СтандартныйВычетЗаСдачуНормГТО", Форма.СтруктураРеквизитовФормы.мВыбраннаяФорма);
		...
		ИначеЕсли ИмяОбласти = "П000900001240" Тогда// Сумма вычета на долгосрочные сбережения по договорам, заключенным с НПФ
			МаксимальноеЗначение = ПределВычета("ДолгосрочныеСбереженияНПФ", Форма.СтруктураРеквизитовФормы.мВыбраннаяФорма);
		ИначеЕсли ИмяОбласти = "П000900001250" Тогда// Сумма вычета на долгосрочные сбережения по ИИС
			МаксимальноеЗначение = ПределВычета("ДолгосрочныеСбереженияИИС", Форма.СтруктураРеквизитовФормы.мВыбраннаяФорма);
		...
		ИначеЕсли ИмяОбласти = "П000900001181" Тогда
			...
			СуммаПриложение5 = Форма.СтруктураДанныхПриложение5.П000900001100
				+ Форма.СтруктураДанныхПриложение5.П000900001105
				+ Форма.СтруктураДанныхПриложение5.П000900001110
				+ Форма.СтруктураДанныхПриложение5.П000900001130
				+ Форма.СтруктураДанныхПриложение5.П000900001135;
			...
			ПоказателиСуммы.Добавить("П000900001161");
			ПоказателиСуммы.Добавить("П000900001170");
			ПоказателиСуммы.Добавить("П000900001171");
...
&НаСервере
Процедура ПреобразоватьЗагруженныйРаздел1()
	
	// Преобразуем загруженные данные из хранилища для Раздела 1
	// при загрузке из файла данные не делятся по строкам - на странице есть ограничение - 2 строки на страницу
	МаксимальноеКоличествоСтрок = МаксимальноеКоличествоСтрокНаСтранице(ЭтотОбъект, "Раздел1");
	Для Каждого ТекущаяСтрока Из ХранилищеП000100001 Цикл
		
		Если ТекущаяСтрока.Таблица.Количество() <= МаксимальноеКоличествоСтрок Тогда
			Продолжить;
		КонецЕсли;
		
		НоваяСтрока = ХранилищеП000100001.Добавить();
		Пока ТекущаяСтрока.Таблица.Количество() > МаксимальноеКоличествоСтрок Цикл
			УдаляемаяСтрока = ТекущаяСтрока.Таблица[ТекущаяСтрока.Таблица.Количество() - 1];
			ЗаполнитьЗначенияСвойств(НоваяСтрока.Таблица.Добавить(), УдаляемаяСтрока);
			ТекущаяСтрока.Таблица.Удалить(УдаляемаяСтрока);
		КонецЦикла;
		
	КонецЦикла;
	
	КолонкиРаздела1 = П000100001.Выгрузить().Колонки;
	Для Каждого ТекущийЛист Из ТаблицаСтраницыРаздел1 Цикл
		
		Если ТипЗнч(ТекущийЛист.Данные) <> Тип("СписокЗначений") Тогда
			Продолжить;
		КонецЕсли;
		
		Для Каждого ТекущееЗначение Из ТекущийЛист.Данные Цикл
			
			ИсходноеЗначение = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ТекущееЗначение.Значение);
			РазделяемоеЗначение = Новый Структура;
			МинимальныйИндекс = 0;
			МаксимальныйИндекс = 0;
			
			Для Каждого ТекущаяЯчейка Из ТекущееЗначение.Значение Цикл
				
				ПозицияРазделителя = СтрНайти(ТекущаяЯчейка.Ключ, "_");
				Если ПозицияРазделителя = 0 Тогда
					Продолжить;
				КонецЕсли;
				
				ИндексСтроки = Число(Сред(ТекущаяЯчейка.Ключ, ПозицияРазделителя + 1));
				Если ИндексСтроки > МаксимальноеКоличествоСтрок Тогда
					РазделяемоеЗначение.Вставить(ТекущаяЯчейка.Ключ, ТекущаяЯчейка.Значение);
					ИсходноеЗначение.Удалить(ТекущаяЯчейка.Ключ);
					Если МинимальныйИндекс = 0 Или МинимальныйИндекс > ИндексСтроки Тогда
						МинимальныйИндекс = ИндексСтроки;
					КонецЕсли;
					МаксимальныйИндекс = Макс(МаксимальныйИндекс, ИндексСтроки);
				КонецЕсли;
				
			КонецЦикла;
			
			ТекущееЗначение.Значение = ИсходноеЗначение;
			НачальныйИндекс = 1;
			
			Пока НачальныйИндекс * МаксимальноеКоличествоСтрок <= МаксимальныйИндекс Цикл
				
				НовыеДанные = Новый Структура;
				Для Каждого ТекущаяЯчейка Из ИсходноеЗначение Цикл
					МассивТипов = Новый Массив;
					МассивТипов.Добавить(ТипЗнч(ТекущаяЯчейка.Значение));
					ОписаниеТипа = Новый ОписаниеТипов(МассивТипов);
					НовыеДанные.Вставить(ТекущаяЯчейка.Ключ, ОписаниеТипа.ПривестиЗначение());
				КонецЦикла;
				
				НоваяСтраница = ТаблицаСтраницыРаздел1.Добавить();
				НоваяСтраница.Представление = СтрШаблон(Нстр("ru = 'Лист N %1';
															|en = 'Лист N %1'"), ТаблицаСтраницыРаздел1.Количество());
				
				Для Каждого ТекущаяКолонка Из КолонкиРаздела1 Цикл
					
					ИмяЯчейки = СтрШаблон("%1_%2", ТекущаяКолонка.Имя, НачальныйИндекс * МаксимальноеКоличествоСтрок + 1);
					Если РазделяемоеЗначение.Свойство(ИмяЯчейки) Тогда
						НовыеДанные.Вставить(СтрШаблон("%1_%2", ТекущаяКолонка.Имя, НачальныйИндекс), РазделяемоеЗначение[ИмяЯчейки]);
					КонецЕсли;
					
				КонецЦикла;
				
				НоваяСтраница.Данные.Добавить(НовыеДанные);
...
&НаСервере
Функция СуммаМногострочногоПоказателя(ИмяПоказателя, Данные)
	
	Результат = 0;
	
	Для Каждого ТекущиеДанные Из Данные Цикл
		Если СтрНачинаетсяС(ТекущиеДанные.Ключ, ИмяПоказателя) И ЗначениеЗаполнено(ТекущиеДанные.Значение) Тогда
			Результат = Результат + ТекущиеДанные.Значение;
		КонецЕсли;
	КонецЦикла;
