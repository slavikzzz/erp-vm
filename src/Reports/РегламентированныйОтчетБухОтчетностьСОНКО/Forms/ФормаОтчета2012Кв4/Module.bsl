...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.КоличествоСтрокИнформационногоМакета = СтрЧислоСтрок(ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаОтчета2012Кв4_Изменения").ПолучитьТекст());
...
Процедура Расшифровать(Команда)
	...
	РегламентированнаяОтчетностьКлиент.ОткрытьРасшифровкуОтчета(Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7), Сред(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") + 7), ИДИменПоказателей, ПараметрыОтчета)
...
Функция РазделыОтчетаСДанными()
	...
	Возврат СтрРазделить("Баланс,ОФР,ОЦИС", ",", Ложь);
...
Процедура Расчет_Баланс(Форма, ОбластьИмя)
	...
	Для Графа = 4 По 6 Цикл
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
		 ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета["П00001000050" + Графа], ОбластьИмя) > 0 Тогда
			...
			УстановитьЗначениеПоказателя(РазмещениеДанных, "П00001000050" + Графа, РасчетСтроки1600);
			
		КонецЕсли;
		
		// Пассив
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
		 ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета["П00001000110" + Графа], ОбластьИмя) > 0 Тогда
			РасчетСтроки1700 = ПоказательОтчета(Форма, РазмещениеДанных, "П00001000060" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00001000070" + Графа)
			...
			УстановитьЗначениеПоказателя(РазмещениеДанных, "П00001000110" + Графа, РасчетСтроки1700);
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
		 ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета["П00010064000" + Графа], ОбластьИмя) > 0 Тогда
			РасчетСтроки6400 = ПоказательОтчета(Форма, РазмещениеДанных, "П00010061000" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010062100" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010062400" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010062500" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010063100" + Графа)
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если СохраненныеДанные.Свойство("ВерсияФормы", ВерсияФормы) Тогда
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	Если СписокДополнительныхФайлов.Количество() > 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'К файлу выгрузки бухгалтерской отчетности приложите:';
													|en = 'К файлу выгрузки бухгалтерской отчетности приложите:'"));
		Для Каждого ДополнительныйФайл Из СписокДополнительныхФайлов Цикл
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон("%1%2%3",
				ДополнительныйФайл.Представление, Символы.ПС, ДополнительныйФайл.Значение));
		КонецЦикла;
	КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСписковВыбора = ОбъектОтчета(ИмяФормы).ПолучитьМакет("СпискиВыбора2012Кв4");
...
Процедура ОбновитьСведенияОбОрганизацииВРазделахНаСервере()
	...
	ЗаполнитьСведенияОбОрганизации(ЭтотОбъект, , СведенияОбОрганизации);
...
Процедура УстановитьФорматВывода(Форма, НаимРаздела)
	...
	Если Форма.СтруктураРеквизитовФормы.ЕдиницаИзмерения = Форма.ПорядкиОкругленияОтчетности.ПеречислениеПорядкиОкругленияОтчетностиОкр1 Тогда
		...
		Форма.СтруктураРеквизитовФормы.СтрокаФорматаПредставления = "ЧЦ = 15; ЧДЦ = " + Форма.СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения
			+ "; ЧРД=,; ЧН=-; ЧО=0; ЧС = 0";
...
Процедура ПроверитьТочность()
	...
	Если СтруктураРеквизитовФормы.ЕдиницаИзмерения = ПорядкиОкругленияОтчетности.ПеречислениеПорядкиОкругленияОтчетностиОкр1 Тогда
		Если СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения > 2 Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Баланс: при выводе сумм в рублях точность не может превышать 2 знака.';
	...
	ИначеЕсли СтруктураРеквизитовФормы.ЕдиницаИзмерения = ПорядкиОкругленияОтчетности.ПеречислениеПорядкиОкругленияОтчетностиОкр1000 Тогда
		Если СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения > 3 Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Баланс: при выводе сумм в тысячах рублей точность не может превышать 3 знака.';
	...
	ИначеЕсли СтруктураРеквизитовФормы.ЕдиницаИзмерения = ПорядкиОкругленияОтчетности.ПеречислениеПорядкиОкругленияОтчетностиОкр1000000 Тогда
		Если СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения > 6 Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Баланс: при выводе сумм в миллионах рублей точность не может превышать 6 знаков.';
...
Процедура ПроверитьКодСтрокиОФР(СтатусПроверки, ИмяПоказателя, НаименованиеСтроки)
	...
	Если ЗначениеЗаполнено(КодСтроки) ИЛИ ЗначениеЗаполнено(Графа03) ИЛИ ЗначениеЗаполнено(Графа04) Тогда
		Если ПустаяСтрока(КодСтроки) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указан код строки """ + НаименованиеСтроки + """ отчета о финансовых результатах", "ОФР", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(КодСтроки) <> 4 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Неверно указан код строки """ + НаименованиеСтроки + """ отчета о финансовых результатах", "ОФР", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьКодСтрокиОЦИС(СтатусПроверки, ИмяПоказателя, НаименованиеСтроки)
	...
	Если ЗначениеЗаполнено(КодСтроки) ИЛИ ЗначениеЗаполнено(Графа03) ИЛИ ЗначениеЗаполнено(Графа04) Тогда
		Если ПустаяСтрока(КодСтроки) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указан код строки """ + НаименованиеСтроки + """ отчета о целевом использовании средств", "ОЦИС", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(КодСтроки) <> 4 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Неверно указан код строки """ + НаименованиеСтроки + """ отчета о целевом использовании средств", "ОЦИС", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьКодСтрокиБаланс(СтатусПроверки, ИмяПоказателя, НаименованиеСтроки)
	...
	Если ЗначениеЗаполнено(КодСтроки) ИЛИ ЗначениеЗаполнено(Графа04) ИЛИ ЗначениеЗаполнено(Графа05) ИЛИ ЗначениеЗаполнено(Графа06) Тогда
		Если ПустаяСтрока(КодСтроки) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указан код строки """ + НаименованиеСтроки + """ бухгалтерского баланса", "Баланс", ИмяПоказателя + "03");
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(КодСтроки) <> 4 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Неверно указан код строки """ + НаименованиеСтроки + """ бухгалтерского баланса", "Баланс", ИмяПоказателя + "03");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
