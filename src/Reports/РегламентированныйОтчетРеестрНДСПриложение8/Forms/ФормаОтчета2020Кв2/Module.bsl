...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Если РазделЗаполнен(ДанныеРеестра.Строки, Ложь) Тогда
		...
		ПроверитьКС_8_1_1(СтруктураРезультата);
		ПроверитьКС_8_1_2(СтруктураРезультата);
		ПроверитьКС_8_1_3(СтруктураРезультата);
		ПроверитьКС_8_1_4(СтруктураРезультата);
		ПроверитьКС_8_2_1(СтруктураРезультата);
		ПроверитьКС_8_2_2(СтруктураРезультата);
		ПроверитьКС_8_2_3(СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_8_1_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 8.1.1';
												|en = 'Проверка КС 8.1.1'"));
...
&НаСервере
Процедура ПроверитьКС_8_1_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 8.1.2';
												|en = 'Проверка КС 8.1.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_8_1_3(СтруктураРезультата)
	
	Настройки	= ОбработатьНастройки(,, СтруктураРезультата);
	Настройки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 8.1.3';
												|en = 'Проверка КС 8.1.3'"));
	Настройки.Вставить("Нарушение"		,НСтр("ru = 'Ошибка заполнения';
												|en = 'Ошибка заполнения'"));
	Настройки.Вставить("ОписаниеКС"		,
		НСтр("ru = 'Должен быть указан код страны погрузки (графа 18 Реестра) или разгрузки ';
			|en = 'Должен быть указан код страны погрузки (графа 18 Реестра) или разгрузки '") +
		НСтр("ru = '(графа 19 Реестра) в зависимости от кода направления транспортного средства, ';
			|en = '(графа 19 Реестра) в зависимости от кода направления транспортного средства, '") +
		НСтр("ru = 'при этом код страны должен быть отличен от (RU - Российская Федерация)';
			|en = 'при этом код страны должен быть отличен от (RU - Российская Федерация)'"));
	Настройки.Вставить("ШаблонОшибки"	,
		НСтр("ru = 'Не указан код страны %1 транспортного средства или указано недопустимое ';
			|en = 'Не указан код страны %1 транспортного средства или указано недопустимое '") +
		НСтр("ru = 'значение (RU) при указанном коде направления транспортного средства (%2)';
			|en = 'значение (RU) при указанном коде направления транспортного средства (%2)'"));
	Настройки.Вставить("ШаблонРасшифровки"	,
		НСтр("ru = 'Указанный код направления: (""%1""), код страны %2: (""%3"")';
			|en = 'Указанный код направления: (""%1""), код страны %2: (""%3"")'"));
	...
	Для каждого СтраницаРаздела Из ДанныеРеестра.Строки Цикл
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл
			...
			Настройки.Вставить("ДопСведенияПараметры", Новый Структура("НомерСтраницы, НомерСтроки", НомерСтраницы, 
				НомерРасчетный));
			Показатели		= ЗаполнитьПоказатели("КодНаправления, СтранаПогрузки, СтранаРазгрузки", 
				ДанныеСтраницы[НомерСтроки - 1].Данные, НомерСтроки);
			
			Если ЗначениеЗаполнено(Показатели.КодНаправления.Значение) И (Показатели.КодНаправления.Значение = ВРег("A")
				Или Показатели.КодНаправления.Значение = ВРег("D")) Тогда
				
				ЗарегистрироватьФормулу(Настройки);
				Страна	= ?(Показатели.КодНаправления.Значение = ВРег("A"), Показатели.СтранаПогрузки, Показатели.СтранаРазгрузки);
				РасшифроватьПоказатели(Показатели, Настройки, "Реестр", НомерСтраницы);
				Настройки.ОписаниеОшибки = ?(Показатели.КодНаправления.Значение = ВРег("A"), 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонОшибки, "погрузки", "A - прибытие ТС"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонОшибки, "разгрузки", "D - убытие ТС"));
				Настройки.Расшифровка = ?(Показатели.КодНаправления.Значение = ВРег("A"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
						"A - прибытие ТС", "погрузки",  Страна.Значение),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
						"D - убытие ТС", "разгрузки", Страна.Значение));
				Настройки.ЭтоОшибка	= Не ЗначениеЗаполнено(Страна.Значение) Или Страна.Значение = ВРег("RU");;
...
&НаСервере
Процедура ПроверитьКС_8_1_4(СтруктураРезультата)
	
	Заголовки		= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		,НСтр("ru = 'Проверка КС 8.1.4';
													|en = 'Проверка КС 8.1.4'"));
	Заголовки.Вставить("ОписаниеКС"			,
		НСтр("ru = 'Указанный код страны погрузки (графа 18 Реестра) должен соответствовать справочнику ОКСМ';
			|en = 'Указанный код страны погрузки (графа 18 Реестра) должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	,НСтр("ru = 'Указанный код: (""%1"")';
													|en = 'Указанный код: (""%1"")'"));
	Заголовки.Вставить("Показатель"			,НСтр("ru = 'СтранаПогрузки';
													|en = 'СтранаПогрузки'"));
	...
	Заголовки.Вставить("ТекстФормулы"		,НСтр("ru = 'Проверка КС 8.1.4';
													|en = 'Проверка КС 8.1.4'"));
	Заголовки.Вставить("ОписаниеКС"			,
		НСтр("ru = 'Указанный код страны разгрузки (графа 19 Реестра) должен соответствовать справочнику ОКСМ';
			|en = 'Указанный код страны разгрузки (графа 19 Реестра) должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	,НСтр("ru = 'Указанный код: (""%1"")';
													|en = 'Указанный код: (""%1"")'"));
	Заголовки.Вставить("Показатель"			,НСтр("ru = 'СтранаРазгрузки';
													|en = 'СтранаРазгрузки'"));
	Заголовки.Вставить("ПроверятьПустые"	,Ложь);
	Настройки	= ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_8_2_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 8.2.1';
												|en = 'Проверка КС 8.2.1'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_8_2_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 8.2.2';
												|en = 'Проверка КС 8.2.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_8_2_3(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 8.2.3';
												|en = 'Проверка КС 8.2.3'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	ИначеЕсли УсловныйКодПоказателя = "НалоговаяБаза" Тогда
		Результат.Вставить("Имя", "П1000020%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Сумма налоговой базы';
												|en = 'Сумма налоговой базы'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "КодНаправления" Тогда
		Результат.Вставить("Имя", "П1000004%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Код направления ТС';
												|en = 'Код направления ТС'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "СтранаПогрузки" Тогда
		Результат.Вставить("Имя", "П1000018%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Страна места погрузки';
												|en = 'Страна места погрузки'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "СтранаРазгрузки" Тогда
		Результат.Вставить("Имя", "П1000019%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Страна места разгрузки';
												|en = 'Страна места разгрузки'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
