...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр р.НДС прилож.08 21к2") Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1000004", 1, ,
		"Выбор кода направления транспортного средства", КоллекцияСписковВыбора["КодыНаправленияТС"]);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 7) = "П100009" Тогда
		
		СтандартнаяОбработка = Ложь;
		Область.Значение = ?(ЗначениеЗаполнено(Область.Значение), "", "V");
		Модифицированность = Истина;
		
	ИначеЕсли Лев(Область.Имя, 8) = "П1000018"
		ИЛИ Лев(Область.Имя, 8) = "П1000019" Тогда
	...
	ИначеЕсли Лев(Область.Имя, 8) = "П1000009"
		ИЛИ Лев(Область.Имя, 8) = "П1000012"
		ИЛИ Лев(Область.Имя, 8) = "П1000015" Тогда
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы120 Тогда
		Подстрока = "автомобиль";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный автомобиль?';
							|en = 'Удалить выбранный автомобиль?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы121 Тогда
		Подстрока = "прицеп";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный прицеп?';
							|en = 'Удалить выбранный прицеп?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяТекРаздела, ДопПараметры = Неопределено)
	...
	ИначеЕсли ИмяТекРаздела = "Реестр" Тогда
		...
		Обл = Форма.ТабличныйДокумент.Области.Найти("П1000004_" + ИндексСтр);
		Пока Обл <> Неопределено Цикл
			
			КодНаправления = СокрЛП(Обл.Значение);
			
			Обл_СтранПогр  = Форма.ТабличныйДокумент.Области["П1000018_" + ИндексСтр];
			Обл_СтранРазгр = Форма.ТабличныйДокумент.Области["П1000019_" + ИндексСтр];
			
			Если КодНаправления = "A" Тогда
				РазрешитьВводДанныхВОбласть(Форма.мСтруктураЦветовРаскраски, Обл_СтранПогр);
				ЗапретитьВводДанныхВОбласть(Форма.мСтруктураЦветовРаскраски, Обл_СтранРазгр);
			ИначеЕсли КодНаправления = "D" Тогда
				РазрешитьВводДанныхВОбласть(Форма.мСтруктураЦветовРаскраски, Обл_СтранРазгр);
				ЗапретитьВводДанныхВОбласть(Форма.мСтруктураЦветовРаскраски, Обл_СтранПогр);
			Иначе
				РазрешитьВводДанныхВОбласть(Форма.мСтруктураЦветовРаскраски, Обл_СтранПогр);
				РазрешитьВводДанныхВОбласть(Форма.мСтруктураЦветовРаскраски, Обл_СтранРазгр);
			КонецЕсли;
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя)
				...
				Если ЗначениеЗаполнено(КодОперации) Тогда
					Ном = 1;
					Пока Форма.ТабличныйДокумент.Области.Найти("П1000020_" + Формат(Ном, "ЧГ=")) <> Неопределено Цикл
						СуммаНалБаз = СуммаНалБаз
							+ Форма.ТабличныйДокумент.Области["П1000020_" + Формат(Ном, "ЧГ=")].Значение;
						Ном = Ном + 1;
					КонецЦикла;
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если Ветка = АктивнаяВетка Тогда
				...
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					Ветка_П10000.Данные.Вставить("П1000020", Форма.ТабличныйДокумент.Области["П1000020_"
			...
			Если ЗначениеЗаполнено(КодОперации) Тогда
				ДанныеМнЧ_П10000 = Ветка.ДанныеМногострочныхЧастей.П10000.Строки;
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					СуммаНалБаз = СуммаНалБаз + Ветка_П10000.Данные["П1000020"];
				КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("НалБазаИтого", "КодОперации;П1000020");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрТРСД_8");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRTRINNDS"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRTRINNDS.2"));
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000003"]) Тогда
				...
				+ НомПП + " не задана дата транспортного, товаросопроводительного и (или) иного документа",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			КодНапрТС = СокрЛП(Ветка_П10000.Данные["П1000004"]);
			Если НЕ (КодНапрТС = "A" ИЛИ КодНапрТС = "D") Тогда
				ИмяПок = "П1000004" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не" + ?(ЗначениеЗаполнено(КодНапрТС), "верно", "")
				+ " задан код направления прибытия (убытия) транспортного средства с товаром",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000005"]) Тогда
				ДлинаНомТмжНакл = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000005"]));
				Если НЕ (ДлинаНомТмжНакл = 23 ИЛИ (ДлинаНомТмжНакл >= 25 И ДлинаНомТмжНакл <= 29)) Тогда
					ИмяПок = "П1000005" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " неверно задан номер таможенной накладной (при ввозе). Возможное количество символов в номере: 23 или от 25 до 29",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
			Если КодНапрТС = "A" Тогда
				ДлинаСтранПогр = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000018"]));
				Если ДлинаСтранПогр <> 2 Тогда
					ИмяПок = "П1000018" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не"
					+ ?(ДлинаСтранПогр = 0, "", "верно") + " задана страна места погрузки груза",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			ИначеЕсли КодНапрТС = "D" Тогда
				ДлинаСтранРазгр = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000019"]));
				Если ДлинаСтранРазгр <> 2 Тогда
					ИмяПок = "П1000019" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не"
					+ ?(ДлинаСтранРазгр = 0, "", "верно") + " задана страна места разгрузки груза",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				ДлинаРегНомТД = СтрДлина(СокрЛП(Ветка_П11000.Данные["П1100006"]));
				Если ДлинаРегНомТД > 0
					И НЕ (ДлинаРегНомТД = 23 ИЛИ (ДлинаРегНомТД >= 25 И ДлинаРегНомТД <= 29)) Тогда
					ИмяПок = "П1100006" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " неверно задан регистрационный номер таможенной декларации (при вывозе)",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			// ГрузОтпр.
			Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000091"]) Тогда // СведФЛ
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000007"]) Тогда
					ДлинаИННФЛ = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000007"]));
					Если ДлинаИННФЛ <> 12 Тогда
						ИмяПок = "П1000007" + "_" + НомерСтрокиП10000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " неверно задан ИНН отправителя", "Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
				ФИО = РегламентированнаяОтчетность.РазложитьФИО(Ветка_П10000.Данные["П1000008"]);
				Если НЕ ЗначениеЗаполнено(ФИО.Фамилия) Тогда
					ИмяПок = "П1000008" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана фамилия отправителя", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(ФИО.Имя) Тогда
					ИмяПок = "П1000008" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано имя отправителя", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			Иначе // СведОрг
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000007"]) Тогда
					ДлинаИНН = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000007"]));
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000008"]) Тогда
					ИмяПок = "П1000008" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование отправителя", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000009"]) Тогда
				ИмяПок = "П1000009" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " не задан адрес отправителя", "Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			// ГрузПолуч.
			Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000092"]) Тогда // СведФЛ
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000010"]) Тогда
					ДлинаИННФЛ = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000010"]));
					Если ДлинаИННФЛ <> 12 Тогда
						ИмяПок = "П1000010" + "_" + НомерСтрокиП10000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " неверно задан ИНН получателя", "Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
				ФИО = РегламентированнаяОтчетность.РазложитьФИО(Ветка_П10000.Данные["П1000011"]);
				Если НЕ ЗначениеЗаполнено(ФИО.Фамилия) Тогда
					ИмяПок = "П1000011" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана фамилия получателя", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(ФИО.Имя) Тогда
					ИмяПок = "П1000011" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано имя получателя", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			Иначе // СведОрг
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000010"]) Тогда
					ДлинаИНН = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000010"]));
					Если ДлинаИНН <> 10 Тогда
						ИмяПок = "П1000010" + "_" + НомерСтрокиП10000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " неверно задан ИНН получателя", "Реестр", ИмяПок, НомерСтраницы);
					...
					КонецЕсли;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000011"]) Тогда
					ИмяПок = "П1000011" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование получателя", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000012"]) Тогда
				ИмяПок = "П1000012" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " не задан адрес получателя", "Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			// Перевозчик.
			Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000013"])
				ИЛИ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000014"])
				ИЛИ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000015"]) Тогда
				
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000093"]) Тогда // СведФЛ
					Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000013"]) Тогда
						ДлинаИННФЛ = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000013"]));
						Если ДлинаИННФЛ <> 12 Тогда
							ИмяПок = "П1000013" + "_" + НомерСтрокиП10000_Строка;
							СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
							+ " неверно задан ИНН перевозчика", "Реестр", ИмяПок, НомерСтраницы);
							СтатусПроверки = Ложь;
						КонецЕсли;
					КонецЕсли;
					ФИО = РегламентированнаяОтчетность.РазложитьФИО(Ветка_П10000.Данные["П1000014"]);
					Если НЕ ЗначениеЗаполнено(ФИО.Фамилия) Тогда
						ИмяПок = "П1000014" + "_" + НомерСтрокиП10000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " не задана фамилия перевозчика", "Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					Если НЕ ЗначениеЗаполнено(ФИО.Имя) Тогда
						ИмяПок = "П1000014" + "_" + НомерСтрокиП10000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " не задано имя перевозчика", "Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				Иначе // СведОрг
					Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000013"]) Тогда
						ДлинаИНН = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000013"]));
						Если ДлинаИНН <> 10 Тогда
							ИмяПок = "П1000013" + "_" + НомерСтрокиП10000_Строка;
							СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
							+ " неверно задан ИНН перевозчика", "Реестр", ИмяПок, НомерСтраницы);
						...
						КонецЕсли;
					КонецЕсли;
					Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000014"]) Тогда
						ИмяПок = "П1000014" + "_" + НомерСтрокиП10000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " не задано наименование перевозчика", "Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000015"]) Тогда
					ИмяПок = "П1000015" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан адрес перевозчика", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200016"]) Тогда
					ИмяПок = "П1200016" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан регистрационный номер автомобиля", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрТРСД_8 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрТРСД_8");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрТРСД_8 = СкопироватьУзел(УзелРодитель.Родитель, РеестрТРСД_8);
		Для каждого Эл Из ЭкзРеестрТРСД_8.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		// Узел "СведТРСД" имеется только в схеме выгрузки "Сведений".
		// Если он не найден, значит, сейчас формируются данные "Реестра"
		// (для выгрузки первого из 2-х файлов).
		СведТРСД = ПолучитьПодчиненныйЭлемент(ЭкзРеестрТРСД_8, "СведТРСД");
		Если СведТРСД = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведТРСД = СкопироватьУзел(ЭкзРеестрТРСД_8, СведТРСД, ЭкзРеестрТРСД_8.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСведТРСД.Строки Цикл
					Если Эл.Строки.Количество() = 0 И Эл.Код <> "РегНомТД" Тогда
						Если Эл.Код = "НомТРСДок" Тогда
							ТекЗнач = СокрЛП(Ветка_П10000.Данные[Эл.Ключ]);
							Заполн_ДатаТРСДок = ЗначениеЗаполнено(Ветка_П10000.Данные["П1000003"]);
							ВывестиПоказательВXML(Эл, ?(НЕ ЗначениеЗаполнено(ТекЗнач) И Заполн_ДатаТРСДок, "б/н", ТекЗнач));
						ИначеЕсли Эл.Код = "СтранРазгр" Тогда
							Если Ветка_П10000.Данные["П1000004"] = "D" Тогда
								ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
							КонецЕсли;
						ИначеЕсли Эл.Код = "СтранПогр" Тогда
							Если Ветка_П10000.Данные["П1000004"] = "A" Тогда
								ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
							КонецЕсли;
						Иначе
							ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
				...
				// 11000.
				РегНомТД = ПолучитьПодчиненныйЭлемент(ЭкзСведТРСД, "РегНомТД");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					ЭкзРегНомТД = СкопироватьУзел(ЭкзСведТРСД, РегНомТД);
					ВывестиПоказательВXML(ЭкзРегНомТД, Ветка_П11000.Данные[ЭкзРегНомТД.Ключ]);
				КонецЦикла;
				
				// 10000 (продолжение).
				// Отправитель.
				ГрузОтпр = ПолучитьПодчиненныйЭлемент(ЭкзСведТРСД, "ГрузОтпр");
				Для каждого Эл Из ГрузОтпр.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
						ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
					КонецЕсли;
				КонецЦикла;
				Парам = Новый Структура("Данные,ИмяОблПризнакИП,ИмяОблНаим",
					Ветка_П10000.Данные, "П1000091", "П1000008");
				ДобавитьУзелСведенийОбОрганизацииИлиИП(ГрузОтпр, Парам);
				
				// Получатель.
				ГрузПолуч = ПолучитьПодчиненныйЭлемент(ЭкзСведТРСД, "ГрузПолуч");
				Для каждого Эл Из ГрузПолуч.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
						ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
					КонецЕсли;
				КонецЦикла;
				Парам = Новый Структура("Данные,ИмяОблПризнакИП,ИмяОблНаим",
					Ветка_П10000.Данные, "П1000092", "П1000011");
				ДобавитьУзелСведенийОбОрганизацииИлиИП(ГрузПолуч, Парам);
				
				// Перевозчик.
				Перевозчик = ПолучитьПодчиненныйЭлемент(ЭкзСведТРСД, "Перевозчик");
				Для каждого Эл Из Перевозчик.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
						ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
					КонецЕсли;
				КонецЦикла;
				Парам = Новый Структура("Данные,ИмяОблПризнакИП,ИмяОблНаим",
					Ветка_П10000.Данные, "П1000093", "П1000014");
				ДобавитьУзелСведенийОбОрганизацииИлиИП(Перевозчик, Парам);
				
				// 12000.
				СведТС = ПолучитьПодчиненныйЭлемент(ЭкзСведТРСД, "СведТС");
				ДанныеМнЧ_П12000 = Ветка_П10000.ДанныеМногострочныхЧастей.П12000.Строки;
				Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
					
					ЭкзСведТС = СкопироватьУзел(ЭкзСведТРСД, СведТС, ЭкзСведТРСД.Строки.Количество() - 1);
					Для каждого Эл Из ЭкзСведТС.Строки Цикл
						Если Эл.Строки.Количество() = 0
							И Эл.Код <> "РегНомПриц" Тогда
							ВывестиПоказательВXML(Эл, Ветка_П12000.Данные[Эл.Ключ]);
						КонецЕсли;
					КонецЦикла;
					
					// 12100.
					РегНомПриц = ПолучитьПодчиненныйЭлемент(ЭкзСведТС, "РегНомПриц");
					ДанныеМнЧ_П12100 = Ветка_П12000.ДанныеМногострочныхЧастей.П12100.Строки;
					Для каждого Ветка_П12100 Из ДанныеМнЧ_П12100 Цикл
						
						ЭкзРегНомПриц = СкопироватьУзел(ЭкзСведТС, РегНомПриц, ЭкзСведТС.Строки.Количество() - 1);
						ВывестиПоказательВXML(ЭкзРегНомПриц, Ветка_П12100.Данные[ЭкзРегНомПриц.Ключ]);
...
Процедура ДобавитьУзелСведенийОбОрганизацииИлиИП(Узел, Знач ДопПараметры)
	...
	СведОрг = ПолучитьПодчиненныйЭлемент(Узел, "СведОрг");
	СведФЛ  = ПолучитьПодчиненныйЭлемент(Узел, "СведФЛ");
	
	ЭтоИП = ЗначениеЗаполнено(ДопПараметры.Данные[ДопПараметры.ИмяОблПризнакИП]);
	Если ЭтоИП Тогда
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_8");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_8_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Для каждого Строка11 Из Строки11 Цикл
					Если ЗначениеЗаполнено(Строка11.Данные.П1100006) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100006));
					КонецЕсли;
				КонецЦикла;
				СтрокаДокумент.Параметры.П1100006 = СтрСоединить(МасТекЗнач, "; ");
				...
				Для каждого Строка12 Из Строки12 Цикл
					...
					Если Строки12.Индекс(Строка12) > 0 Тогда
						Для ИндПарам = 0 По СтрокаДокумент.Параметры.Количество() - 1 Цикл
							СтрокаДокумент.Параметры[ИндПарам] = Неопределено;
						КонецЦикла;
					КонецЕсли;
					
					СтрокаДокумент.Параметры.Заполнить(Строка12.Данные);
					
					// П12100.
					//
					Строки121 = Строка12.ДанныеМногострочныхЧастей.П12100.Строки;
					
					// Особая обработка для графы 17.
					МасТекЗнач = Новый Массив;
					Для каждого Строка121 Из Строки121 Цикл
						Если ЗначениеЗаполнено(Строка121.Данные.П1210017) Тогда
							МасТекЗнач.Добавить(СокрЛП(Строка121.Данные.П1210017));
						КонецЕсли;
					КонецЦикла;
					СтрокаДокумент.Параметры.П1210017 = СтрСоединить(МасТекЗнач, "; ");
				...
				Если ЗначениеЗаполнено(ОблОбъедСтрока1_Верх) И ЗначениеЗаполнено(ОблОбъедСтрока1_Низ)
					...
					Для НомерКолонки = 2 По 22 Цикл
						Если НомерКолонки = 17 ИЛИ НомерКолонки = 18 Тогда
							Продолжить;
						КонецЕсли;
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Реестр НДС Приложение 8");
...
#Область ОбщиеДанные
	...
	ДанныеКС.Вставить("ГрафаНалоговойБазы"		, "20");
...
&НаСервере
Процедура ПроверкаКС_8_1_1()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '8.1.1';
													|en = '8.1.1'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 8';
														|en = 'Реестр 8'"));
...
&НаСервере
Процедура ПроверкаКС_8_1_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '8.1.2';
													|en = '8.1.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 8';
														|en = 'Реестр 8'"));
...
&НаСервере
Процедура ПроверкаКС_8_1_3()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '8.1.3';
													|en = '8.1.3'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 8';
														|en = 'Реестр 8'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'В зависимости от кода направления транспортного средства, должен быть указан код страны ';
			|en = 'В зависимости от кода направления транспортного средства, должен быть указан код страны '") +
		НСтр("ru = 'страны погрузки в гр.18 Реестра, или код страны разгрузки в гр.19 Реестра, отличный от ';
			|en = 'страны погрузки в гр.18 Реестра, или код страны разгрузки в гр.19 Реестра, отличный от '") +
		НСтр("ru = 'значения RU - кода Российской Федерации';
			|en = 'значения RU - кода Российской Федерации'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Код страны%1 %2 указан';
													|en = 'Код страны%1 %2 указан'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не указан (указан ошибочный) код страны погрузки / разгрузки';
														|en = 'Не указан (указан ошибочный) код страны погрузки / разгрузки'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001", "П1000004_");
	ИменаПоказателей.Вставить("Р01Г002", "П1000018_");
	ИменаПоказателей.Вставить("Р01Г003", "П1000019_");
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			...
			Показатели.Р01Г003.Дополнение = Дополнение;
			ЗаполнитьИРасшифроватьПоказатели(Показатели, СтраницаРеестра, Счетчик);
			
			УсловиеПрибытия = Показатели.Р01Г001.Значение = ВРег("A") И (НЕ ЗначениеЗаполнено(Показатели.Р01Г002.Значение) 
				ИЛИ Показатели.Р01Г002.Значение = ВРег("RU"));
			УсловиеУбытия = Показатели.Р01Г001.Значение = ВРег("D") И (НЕ ЗначениеЗаполнено(Показатели.Р01Г003.Значение) 
				ИЛИ Показатели.Р01Г003.Значение = ВРег("RU"));
			Если ЗначениеЗаполнено(Показатели.Р01Г001.Значение) И Показатели.Р01Г001.Значение = ВРег("A") Тогда
				ОписаниеСтраны = " погрузки";
				ЗначениеПоказателя = Показатели.Р01Г002.ЗначениеСтр;
			ИначеЕсли ЗначениеЗаполнено(Показатели.Р01Г001.Значение) И Показатели.Р01Г001.Значение = ВРег("D") Тогда
				ОписаниеСтраны = " разгрузки";
				ЗначениеПоказателя = Показатели.Р01Г003.ЗначениеСтр;
			Иначе
				ОписаниеСтраны = "";
				ЗначениеПоказателя = "(...)";
			КонецЕсли;
			
			Настройки.ЭтоОшибка = УсловиеПрибытия ИЛИ УсловиеУбытия;
			Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, ОписаниеСтраны, ЗначениеПоказателя);
...
&НаСервере
Процедура ПроверкаКС_8_1_4а()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '8.1.4.а';
													|en = '8.1.4.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 8';
														|en = 'Реестр 8'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны погрузки в гр.18 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны погрузки в гр.18 Реестра должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Код страны %1 соответствует справочнику ОКСМ';
													|en = 'Код страны %1 соответствует справочнику ОКСМ'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный код страны';
														|en = 'Ошибочный код страны'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1, Строка %2';
													|en = 'Страница %1, Строка %2'"));
	Настройки = ОбработатьНастройки(Заголовки);
	...
	Настройки.Вставить("ИмяПоказателя", "П1000018_");
	ПроверкаКС_8_1_4_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_8_1_4б()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '8.1.4.б';
													|en = '8.1.4.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 8';
														|en = 'Реестр 8'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны разгрузки в гр.19 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны разгрузки в гр.19 Реестра должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Код страны %1 соответствует справочнику ОКСМ';
													|en = 'Код страны %1 соответствует справочнику ОКСМ'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный код страны';
														|en = 'Ошибочный код страны'"));
	...
	Настройки.Вставить("ИмяПоказателя", "П1000019_");
	ПроверкаКС_8_1_4_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_8_1_4_код(Настройки)
	
	Перем Показатели;
	
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001", Настройки.ИмяПоказателя);
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			...
			Настройки.ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р01Г001.Значение)
				И УправлениеКонтактнойИнформацией.ДанныеКлассификатораСтранМираПоКоду(Показатели.Р01Г001.Значение, 
				"Альфа2") = Неопределено;
			Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, Показатели.Р01Г001.ЗначениеСтр);
			Настройки.ДопСведения = Дополнение;
			ДополнитьСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверкаКС_8_2_1а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '8.2.1.а';
													|en = '8.2.1.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 8';
														|en = 'Реестр 8'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_8_2_1б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '8.2.1.б';
													|en = '8.2.1.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 8';
														|en = 'Реестр 8'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
