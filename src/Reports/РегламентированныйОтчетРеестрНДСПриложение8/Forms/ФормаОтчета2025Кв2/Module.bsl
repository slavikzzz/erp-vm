...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ТабличныйДокумент.Вывести(ОбъектОтчета(ИмяФормы).ПолучитьМакет(Сред(ИмяФормы, СтрНайти(ИмяФормы, "ФормаОтчета")) + "_" + СтруктураРеквизитовФормы.НаимТекущегоРаздела));
		...
		Если мСтруктураМногоуровневыхРазделов.Свойство(Раздел.ИмяСтраницы) Тогда
			...
			Если ДеревоДанныхРазделУровень0.Строки.Количество() = 1 Тогда
				Если ДеревоДанныхРазделУровень0.Строки[0].Данные.Количество() = 0 Тогда
					...
					УстановитьНомерПервойСтроки(ЭтотОбъект);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "ВыгрузкаДеклНДС" Тогда
		...
		ФормаВыбораОтчета = РегламентированнаяОтчетностьКлиент.ПолучитьОбщуюФормуПоИмени(
			"ФормаВыбораВыгрузкиОтчетов", Новый Структура("ДокументыВыгрузки", ВыгрузкиДекларацииНДС), ЭтотОбъект);
...
Процедура ВставитьДополнительнуюСтроку(Знач Копирование = Ложь)
	...
	Если ПревышеноДопустимоеКоличествоСтрокТабличногоДокумента(
		...
		ОбщегоНазначения.СообщитьПользователю(НСтр(
			"ru = 'Превышено допустимое количество строк табличного документа. Необходимо добавить страницу и продолжить ввод данных.';
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревьяДляСравнения.Вставить("ДеревоСравненияРеестра",  ИзвлечьСтруктуруXML("СхемаВыгрузки505_1"));
	ДеревьяДляСравнения.Вставить("ДеревоСравненияСведений", ИзвлечьСтруктуруXML("СхемаВыгрузки505_2"));
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревьяДляЗагрузки.Вставить("ДеревоЗагрузкиРеестра",  ИзвлечьСтруктуруXML("СхемаВыгрузки505_1"));
	ДеревьяДляЗагрузки.Вставить("ДеревоЗагрузкиСведений", ИзвлечьСтруктуруXML("СхемаВыгрузки505_2"));
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки, ПроверятьВозможностьВыгрузки)
	...
	ДеревоВыгрузкиРеестра  = ИзвлечьСтруктуруXML("СхемаВыгрузки505_1");
	ДеревоВыгрузкиСведений = ИзвлечьСтруктуруXML("СхемаВыгрузки505_2");
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если КодНапрТС = "A" Тогда
				// Возможно отсутствие накладной при ввозе из ЕАЭС. По этой причине удалена указанная
				// в формате проверка на обязательность заполнения номера таможенной накладной при ввозе.
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000005"])
					И НЕ (ДлинаНомТмжНакл = 23 ИЛИ (ДлинаНомТмжНакл >= 25 И ДлинаНомТмжНакл <= 29)) Тогда
					...
					СтатусПроверки = Ложь;
				КонецЕсли;
			ИначеЕсли КодНапрТС = "D" И ЗначениеЗаполнено(Ветка_П10000.Данные["П1000005"]) Тогда
				ИмяПок = "П1000005" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не должен быть задан номер таможенной "
				+ "накладной (при ввозе), если код направления прибытия (убытия) транспортного средства с товаром равен D",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если КодНапрТС = "D" Тогда
					Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100006"]) Тогда
						...
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " должен быть задан регистрационный "
						+ "номер таможенной декларации (при вывозе), если код направления прибытия (убытия) транспортного средства "
						+ "с товаром равен D", "Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					ИначеЕсли НЕ (ДлинаРегНомТД = 23 ИЛИ (ДлинаРегНомТД >= 25 И ДлинаРегНомТД <= 29)) Тогда
						ИмяПок = "П1100006" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " неверно задан номер таможенной "
						+ "накладной (при ввозе). Возможное количество символов в номере: 23 или от 25 до 29",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				ИначеЕсли КодНапрТС = "A" И ЗначениеЗаполнено(Ветка_П11000.Данные["П1100006"]) Тогда
					ИмяПок = "П1100006" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не должен быть задан регистрационный "
					+ "номер таможенной декларации (при вывозе), если код направления прибытия (убытия) транспортного средства "
					+ "с товаром равен A", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
...
Процедура ВывестиНаПечатьОтчет(СтруктураПараметров, ВыбранныеСтраницы, СтрПоля = Неопределено)
	...
	Если ВыбранныеСтраницы.Найти("Титульный") <> Неопределено Тогда
		...
		ИмяМакета = "ПечатныйБланк2025Кв2_Титульный";
	...
	Если ВыбранныеСтраницы.Найти("Реестр") <> Неопределено Тогда
		...
		ИмяМакета = "ПечатныйБланк2025Кв2_Реестр";
...
Процедура Проверить(Команда)
	...
	|от 15.11.2024 № ЕД-7-15/1035@ будет
