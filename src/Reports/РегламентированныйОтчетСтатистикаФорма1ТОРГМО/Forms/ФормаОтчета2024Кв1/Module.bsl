...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0614026";
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "1000014489",
		512, , "Выбор типа муниципального образования");
...
Процедура ПоказатьПериод()
	...
	СтрПериод = "по состоянию на конец " + Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = к")
		+ " квартала " + Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ='гггг ""г.""'");
	
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета.Значение = СтрПериод;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "ОргКодОКТМО" Тогда
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 1 По 2 Цикл
		Области["П00100100010" + Гр].Значение
			= Области["П00100100030" + Гр].Значение
			+ Области["П00100100050" + Гр].Значение
			+ Области["П00100100070" + Гр].Значение
			+ Области["П00100100090" + Гр].Значение
			+ Области["П00100100110" + Гр].Значение
			+ Области["П00100100130" + Гр].Значение
			+ Области["П00100100150" + Гр].Значение;
	КонецЦикла;
	
	Для Гр = 1 По 2 Цикл
		Области["П00100100020" + Гр].Значение
			= Области["П00100100040" + Гр].Значение
			+ Области["П00100100060" + Гр].Значение
			+ Области["П00100100080" + Гр].Значение
			+ Области["П00100100100" + Гр].Значение
			+ Области["П00100100120" + Гр].Значение
			+ Области["П00100100140" + Гр].Значение
			+ Области["П00100100160" + Гр].Значение;
	КонецЦикла;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателей.Добавить(ВыбТабличноеПоле.Области["КодТипаОбразования"].Значение,               "КодТипаОбразования");
	СписокПоказателей.Добавить(ВыбТабличноеПоле.Области["ОргКодОКТМО"].Значение,              "ОргКодОКТМО");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-ТОРГ (МО)");
...
Функция СформироватьСтруктуруПараметров() Экспорт
	...
	ПараметрыВыгрузки.Вставить("КодТипаОбразования", СокрЛП(ТабДокумент.КодТипаОбразования));
