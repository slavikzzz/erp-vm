...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
		...
		МодульОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
			ЭтаФорма,
			"БП.Отчет.РО_ПрослеживаемыеТоварыОстатки",
			ИмяФормы(ИмяФормы),
			,
			НСтр("ru = 'Новости: Уведомление об остатках товаров, подлежащих прослеживаемости';
				|en = 'News: Notification of balance of goods to be traced'"),
			Ложь,
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "ИННПродавца" Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ПараметрыФормы = Новый Структура("ИНН", Область.Значение);
		
		ФормаЗаполнения = ПолучитьФорму(
			"Отчет.РегламентированныйОтчетПрослеживаемыеТоварыОстатки.Форма.ФормаЗаполненияИНН", ПараметрыФормы, ЭтаФорма);
		ФормаЗаполнения.ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения(
			"ОткрытьФормуЗаполненияИННЗавершение", ЭтотОбъект);
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	ИначеЕсли ИмяПоказателя = "П000010000801" Тогда
		
		ЗначПок = СтрЗаменить(ЗначПоказателя, " ", "");
		Пока Прав(ЗначПок, 1) = "." Цикл
			ЗначПок = Лев(ЗначПок, СтрДлина(ЗначПок) - 1);
		КонецЦикла;
		Если КодПоОКПД2СоответствуетТребованиям(ЗначПок) Тогда
			Область.Значение = ЗначПок;
		Иначе
			Область.Значение = "";
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр(
			"ru = 'Код товара по ОКПД2 должен соответствовать одному из шаблонов: ХХ, ХХ.Х, ХХ.ХХ, ХХ.ХХ.Х, ХХ.ХХ.ХХ, ХХ.ХХ.ХХ.ХХХ (X - любая цифра). Значение очищено';
			|en = 'Goods code by RCPEA2 must match one of the templates: ХХ, ХХ.Х, ХХ.ХХ, ХХ.ХХ.Х, ХХ.ХХ.ХХ, ХХ.ХХ.ХХ.ХХХ (X is any number). The value is cleared'");
			Сообщение.Сообщить();
		КонецЕсли;
...
&НаКлиенте
Процедура ОткрытьФормуЗаполненияИННЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> Неопределено Тогда
		
		Обл = ТабличныйДокумент.Области.Найти("ИННПродавца");
		Если Обл <> Неопределено Тогда
			Обл.Значение = Результат;
		КонецЕсли;
...
&НаКлиенте
Функция КодПоОКПД2СоответствуетТребованиям(Знач ОКПД2)
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Титульный" Тогда
		...
		ОблИННПродавца = Форма.ТабличныйДокумент.Области.ИННПродавца;
		ПризнакУведомления = СокрЛП(Форма.ТабличныйДокумент.Области.ПризнакУведомления.Значение);
		Если ПризнакУведомления = "1" ИЛИ ПризнакУведомления = "2" Тогда
			РазрешитьВводДанныхВОбласть(Форма, ОблИННПродавца);
		Иначе
			ЗапретитьВводДанныхВОбласть(Форма, ОблИННПродавца);
		КонецЕсли;
...
Процедура УстановитьЧисловойФорматОбласти(ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел1" Тогда
		ТабличныйДокумент.Области["П000010000901"].ТипЗначения = СтруктураРеквизитовФормы.ТипКоличествоПрослеживаемогоТовара;
	КонецЕсли;
