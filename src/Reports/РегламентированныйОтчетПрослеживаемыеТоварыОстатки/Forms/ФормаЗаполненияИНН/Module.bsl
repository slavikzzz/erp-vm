...
Процедура ИННПриИзменении(Элемент)
	...
	Если ТекИННДлина <> 0 И ТекИННДлина <> 10 И ТекИННДлина <> 12 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'ИНН должен состоять из 10 или 12 цифр. Значение очищено';
								|en = 'TIN must contain 10 or 12 digits. The value is cleared'");
		Сообщение.Сообщить();
	...
	ИначеЕсли ТаблицаИНН.НайтиСтроки(Новый Структура("ИНН", ТекИНН)).Количество() > 1 Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтрШаблон(НСтр("ru = 'ИНН %1 уже введен. Значение очищено';
										|en = 'TIN %1 is already entered. The value is cleared'"), СокрЛП(ТекИНН));
		Сообщение.Сообщить();
