		...
		Иначе
			...
			РегламентированнаяОтчетность.ДобавитьОшибкуФормированияРеглОтчета(ПараметрыЗаписи,
				...
				НСтр("ru = 'Неверный тип входящего параметра (%1) при автоматическом формировании формы %2';
					|en = 'Incorrect incoming parameter type (%1) upon automatic form %2 generation'"),
...
Функция ПечатныйБланк(ПараметрыПечати) Экспорт
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Уведомление об остатках");
...
Процедура ВывестиНаПечатьЛист(ИмяФормы, СтруктураПараметров, СтруктураРеквизитовФормы, ДанныеРазделов)
	...
	Если СтруктураРеквизитовФормы.Свойство("СтруктураКонтейнераДанных") Тогда
		ПечФорма.Параметры.Заполнить(СтруктураРеквизитовФормы.СтруктураКонтейнераДанных.СодержательнаяЧасть);
	Иначе
		ПечФорма.Параметры.Заполнить(ДеревоДанныхРаздела.Строки[0].Данные);
	КонецЕсли;
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.Вывести(ПечФорма);
	СтрПоля = "ПолеСверху, ПолеСправа, ПолеСнизу, ПолеСлева";
	ЗаполнитьЗначенияСвойств(ТабличныйДокумент, ПечФорма, СтрПоля, "Макет");
	
	РегламентированнаяОтчетность.ВывестиНаПечатьЛистОбщая("", СтруктураПараметров,
