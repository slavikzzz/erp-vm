...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.ИдМСЧ_Сведения = "П0000100010";
	
	// Количество строк многострочных разделов по "бумажной" форме отчета
	// (минимальное количество строк, которое должно присутствовать всегда).
	КолИсхСтрокИдМСЧ_Сведения = 1;
	
	// Количество колонок многострочных разделов по "бумажной" форме отчета.
	КолКолонокИдМСЧ_Сведения = 5;
	...
	мСтруктураМногострочныхРазделов.Вставить(СтруктураРеквизитовФормы.ИдМСЧ_Сведения, СтруктураРеквизитовФормы.ИдМСЧ_Сведения);
	
	мСтруктураИменаКолонокРазделов = Новый Структура;
	мСтруктураИменаКолонокРазделов.Вставить(СтруктураРеквизитовФормы.ИдМСЧ_Сведения, Новый СписокЗначений);
	...
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.ИдМСЧ_Сведения, КолИсхСтрокИдМСЧ_Сведения);
	
	мСтруктураКолвоКолонокРазделов = Новый Структура;
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.ИдМСЧ_Сведения, КолКолонокИдМСЧ_Сведения);
	...
	СтруктураРеквизитовФормы.мКодОтчета = 1111056;
	...
	СтруктураРеквизитовФормы.КНД = "1111056";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("ИдМСЧ_Сведения");
...
Процедура ФормироватьСтруктуруСвойствСтраниц() Экспорт
	...
	НовСтр.ИдГруппы = СтруктураРеквизитовФормы.ИдМСЧ_Сведения;
...
Функция ОпределитьТабличноеПолеПоИдМногострочногоРазделаНаСервере(ИдГруппы) Экспорт
	...
	ИдГрупп.Вставить("ИдМСЧ_Сведения", СтруктураРеквизитовФормы.ИдМСЧ_Сведения);
...
Функция ОпределитьТабличноеПолеПоИдМногострочногоРаздела(ИдГруппы, ИдГрупп)
	...
	Если ИдГруппы = ИдГрупп.ИдМСЧ_Сведения Тогда
...
Функция ОпределитьИдМногострочногоРазделаПоАктивнойСтранице(ИмяЛиста = Неопределено, ТекущаяСтраница = Неопределено, ИдГрупп)
	...
	Если ИмяТекущейСтраницыПанели = "Сведения" Тогда
		ИмяГруппы = ИдГрупп.ИдМСЧ_Сведения;
	КонецЕсли;
...
Функция ПолучитьСмысловуюЧастьИмениЯчейки(ИмяЯчейки, Определение = Истина)
	...
	Для Инд = 1 По СтрДлина(ИмяЯчейки) Цикл
		...
		// Последний разделитель. Не пользуемся тут функцией Найти,
		// т.к. разделителей может быть много в одной ячейку, пример ИНН_1_1.
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	Если ИдГруппы <> Неопределено Тогда
		...
		Если Парам1.ИмяСтраницы = "Сведения" Тогда
			...
			Если РезультатПоиска = Неопределено Тогда
				...
				Если ТекАдресЗначенияОбласти.ИндексЗнакоместа <> 0 Тогда
					...
					Если НЕ ЗначениеВыведено
						И ПараметрыПечати.ИмяСтраницы = "Сведения" Тогда
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Для Каждого Раздел Из мСвойстваРазделовДекларации.ПолучитьЭлементы() Цикл	
		...
		ВосстановитьЗначенияМногострочнойЧастиРазделаСведенияСтарогоОбразца();
...
&НаСервере
Процедура ВосстановитьЗначенияМногострочнойЧастиРазделаСведенияСтарогоОбразца()
	
	Перем ТекЗначение;
	
	Сведения = ЭтаФорма["СтруктураДанныхСведения"];
	
	НомСтрМНЧ = 1;
	
	Пока Сведения.Свойство("П000010001000_" + Формат(НомСтрМНЧ, "ЧГ="), ТекЗначение) Цикл
		
		ЭлемКоллекции = ЭтаФорма.П0000100010.Получить(НомСтрМНЧ - 1);
		
		ЭлемКоллекции["П000010001000"] = ТекЗначение;
		
		Сведения.Свойство("П000010001001_" + Формат(НомСтрМНЧ, "ЧГ="), ТекЗначение);
		ЭлемКоллекции["П000010001001"] = ТекЗначение;
		
		Сведения.Свойство("П000010001002_" + Формат(НомСтрМНЧ, "ЧГ="), ТекЗначение);
		ЭлемКоллекции["П000010001002"] = ТекЗначение;
		
		Сведения.Свойство("П000010001003_" + Формат(НомСтрМНЧ, "ЧГ="), ТекЗначение);
		ЭлемКоллекции["П000010001003"] = ТекЗначение;
		
		Сведения.Свойство("П000010001004_" + Формат(НомСтрМНЧ, "ЧГ="), ТекЗначение);
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ТабличныйДокумент.Области.ДатаЦен.Значение = Формат(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, "ДФ=dd.MM.yyyy");
...
Процедура ФормироватьСтруктуруСтраницОтчета() Экспорт
	...
	СтрокаУровня1 = мДеревоСтраницОтчета.ПолучитьЭлементы().Добавить();
	СтрокаУровня1.ИмяСтраницы              = "Сведения";
	СтрокаУровня1.Представление            = "Сведения"+Символы.ПС+"о розн. ценах";
	СтрокаУровня1.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = 1;
	СтрокаУровня1.ВыгрузитьСтраницу        = 2;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010001001", 3, , "Выбор кода вида табачного изделия",
		КоллекцияСписковВыбора["КодыВидовТабачныхИзделий"]);
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ДатаНачЦен", СокрЛП(Титульный.ДатаЦен));
	...
	ПараметрыВыгрузки.Вставить("ПрефиксИмениФайлаВыгрузки", "ON_UVCENTAB");
...
Процедура ПроверитьЗаполнение_Сведения(Параметры, СтатусПроверки)
	...
	ДанныеРаздела = мДанныеОтчета.Сведения;
	
	РазделЗаполнен = Ложь;
	
	Инд = 1;
	Пока ДанныеРаздела.Свойство("П000010001000_" + Формат(Инд, "ЧГ=")) Цикл
		
		НомерСтроки = Формат(Инд, "ЧГ=");
		
		СтрокаЗаполнена = ЗначениеЗаполнено(ДанныеРаздела["П000010001000_" + НомерСтроки])
					  ИЛИ ЗначениеЗаполнено(ДанныеРаздела["П000010001001_" + НомерСтроки])
					  ИЛИ ЗначениеЗаполнено(ДанныеРаздела["П000010001002_" + НомерСтроки]);
		
		Если СтрокаЗаполнена Тогда
			РазделЗаполнен = Истина;
			
			Наименование = СокрЛП(ДанныеРаздела["П000010001000_" + НомерСтроки]);
			Если НЕ ЗначениеЗаполнено(Наименование) Тогда
				СообщитьВТС("В строке " + НомерСтроки
					+ " раздела Сведения не указана марка (наименование) табачного изделия", "Сведения", "П000010001000_" + НомерСтроки);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			КодВида = СокрЛП(ДанныеРаздела["П000010001001_" + НомерСтроки]);
			Если НЕ ЗначениеЗаполнено(КодВида) Тогда
				СообщитьВТС("В строке " + НомерСтроки
					+ " раздела Сведения не указан признак табачного изделия", "Сведения", "П000010001001_" + НомерСтроки);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ ЗначениеСодержитсяВСписке(КодВида, "1,2,3") Тогда
				СообщитьВТС("В строке " + НомерСтроки
					+ " раздела Сведения неверно указан признак табачного изделия", "Сведения", "П000010001001_" + НомерСтроки);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Количество = ДанныеРаздела["П000010001002_" + НомерСтроки];
			Если Количество <=0 Тогда
				СообщитьВТС("В строке " + НомерСтроки
					+ " раздела Сведения не указано количество сигарет (папирос)", "Сведения", "П000010001002_" + НомерСтроки);
				СтатусПроверки = Ложь;
			КонецЕсли;
	...
	Если НЕ РазделЗаполнен Тогда
		СообщитьВТС("Раздел Сведения не заполнен", "Сведения", "П000010001000_1");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура СохранитьДанныеРаздела(НаимРаздела, ОчищатьРазделЕслиЕстьМногострочность, ИзменятьАктивнуюСтраницу = Истина)
	...
	Если НаимРаздела = "Сведения" Тогда
...
Функция ОпределитьИдМногострочногоРазделаПоТабличномуПолю(Форма, ИмяТекущейСтраницы) Экспорт
	...
	Если ИмяТекущейСтраницы = "Сведения" Тогда
		ИмяГруппы = Форма.СтруктураРеквизитовФормы.ИдМСЧ_Сведения;
	КонецЕсли;
...
Процедура УдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, ТекущаяСтраница) Экспорт
	...
	Если СтруктураГруппы.Количество() > ИсходноеКоличествоСтрокГруппы Тогда
		...
		Если НомУдаляемойСтроки = СтруктураГруппы.Количество() Тогда
			...
			НомерНижнЯчейкиОбласти = НомерНижнЯчейкиОбласти - 1;
		КонецЕсли;
...
Процедура ТабличныйДокументПриАктивизации(Элемент)
	...
	Если ЗначениеЗаполнено(ИмяГруппы) Тогда 
		ТестовоеИмя = ИмяГруппы + "00_" + Строка(мСтруктураИсхКолвоСтрокРазделов[ИмяГруппы] + 1);
...
Функция РазделТекущейОбласти(Форма, ТекущаяОбласть, ДлИмениОбласти = 13) Экспорт
	...
	Если ИмяТекущейОбласти = "ДобавитьСтроку1" Тогда
		Возврат Форма.СтруктураРеквизитовФормы.ИдМСЧ_Сведения;
