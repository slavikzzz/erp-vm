...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(мДатаНачалаПериодаОтчета) И НЕ ЗначениеЗаполнено(мДатаКонцаПериодаОтчета) Тогда
		...
		НачалоСледующегоМесяца = НачалоМесяца(ДобавитьМесяц(ДатаСоздания, 1));
		
		ДнейДоСледующегоМесяца = (НачалоСледующегоМесяца - ДатаСоздания) / (60 * 60 * 24);
		
		СмещениеПериода = ?(ДнейДоСледующегоМесяца >= 10, 1, 2);
		
		мДатаНачалаПериодаОтчета = НачалоМесяца(ДобавитьМесяц(ДатаСоздания, СмещениеПериода));
		мДатаКонцаПериодаОтчета  = КонецМесяца(мДатаНачалаПериодаОтчета);
...
Процедура ОткрытьФормуОтчета(Команда)
	Если мСкопированаФорма <> Неопределено Тогда
		Если мВыбраннаяФорма <> мСкопированаФорма Тогда
			СписокФормПоддерживающихКопирование = "ФормаОтчета2022Кв1";
