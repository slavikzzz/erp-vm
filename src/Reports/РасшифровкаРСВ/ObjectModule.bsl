...
Процедура СформироватьОтчетРасшифровку(Параметры, ДокументРезультат) Экспорт
	...
	ВыделятьМеждународныеДоговоры = Ложь;
	Если Параметры.Свойство("ВыделятьМеждународныеДоговоры") Тогда
		ВыделятьМеждународныеДоговоры = Параметры.ВыделятьМеждународныеДоговоры;
	КонецЕсли;
	...
	ИсточникРасшифровки.РасчетПоказателейРСВ(Параметры.ИмяРасчета, ДатаНачалаНП, ДатаКонцаНП, Организация, ВнешниеНаборыДанных, Истина, ?(Параметры.ДатаПодписи > ДатаКонцаНП,Параметры.ДатаПодписи, ДатаКонцаНП), , ВыделятьМеждународныеДоговоры);
	...
	КонецЕсли;
	// с 2023 года
	Если Параметры.Свойство("Раздел1Подр1КодТарифа") И ЗначениеЗаполнено(Параметры.Раздел1Подр1КодТарифа) 
		Или Параметры.Свойство("Раздел1Подр4КодТарифа") И ЗначениеЗаполнено(Параметры.Раздел1Подр4КодТарифа) Тогда
		ЗначениеОтбора = Неопределено;
		КодТарифа = "";
		Если Параметры.Свойство("Раздел1Подр1КодТарифа") И ЗначениеЗаполнено(Параметры.Раздел1Подр1КодТарифа) Тогда
			КодТарифа = Параметры.Раздел1Подр1КодТарифа;
		ИначеЕсли Параметры.Свойство("Раздел1Подр4КодТарифа") И ЗначениеЗаполнено(Параметры.Раздел1Подр4КодТарифа)	Тогда
			КодТарифа = Параметры.Раздел1Подр4КодТарифа;
		КонецЕсли;
		Если КодТарифа = "01" Тогда
			ЗначениеОтбора = Справочники.ВидыТарифовСтраховыхВзносов.ОбщийНалоговыйРежим;
		Иначе
			Запрос = Новый Запрос;
			Запрос.Текст = 
			...
			|ГДЕ
			|	ВидыТарифовСтраховыхВзносов.КодФНС = &КодФНС";
			Запрос.УстановитьПараметр("КодФНС", КодТарифа);
			Выборка = Запрос.Выполнить().Выбрать();
			Если Выборка.Следующий() Тогда
				ЗначениеОтбора = Выборка.Ссылка	
			КонецЕсли;
