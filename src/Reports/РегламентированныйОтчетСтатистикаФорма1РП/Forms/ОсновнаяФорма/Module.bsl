...
Процедура ПоказатьПериод(Форма)
	...
	Иначе
		...
		Если НачалоДня(Форма.мДатаКонцаПериодаОтчета) >= '20150701' Тогда
			Форма.ОписаниеНормативДок = "Форма утратила силу в соответствии с приказом Росстата от 22.07.2015 № 336.";
		Иначе
...
Процедура ИзменитьПериод(Форма, Шаг)
	...
	Если (Форма.мДатаКонцаПериодаОтчета >= Дата("20121231235959") И Шаг > 0) Или (Форма.мДатаКонцаПериодаОтчета >= Дата("20130331235959") И Шаг < 0) Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(мДатаНачалаПериодаОтчета) И НЕ ЗначениеЗаполнено(мДатаКонцаПериодаОтчета) Тогда
		Если Год(ТекущаяДатаСеанса()) >= 2013 Тогда
			мДатаКонцаПериодаОтчета  = КонецКвартала(ДобавитьМесяц(КонецМесяца(ТекущаяДатаСеанса()), -3));
