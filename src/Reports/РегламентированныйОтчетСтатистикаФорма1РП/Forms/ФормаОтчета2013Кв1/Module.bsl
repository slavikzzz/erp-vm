...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "08/09/2012";
	// Код отчета, равен коду по ОКУД
	СтруктураРеквизитовФормы.мКодОтчета = "0608025";
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если ВерсияФормы = "07/09/2012" Тогда
		СтруктураРеквизитовФормы.ЕдиницаИзмерения         = ПорядкиОкругленияОтчетности.ПеречислениеПорядкиОкругленияОтчетностиОкр1;
		СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения = 0;
	КонецЕсли;
...
Процедура Расчет(Форма) Экспорт
	...
	мФормаОтчета.Области.П01000603.Значение = мФормаОтчета.Области.П01000703.Значение + мФормаОтчета.Области.П01000803.Значение + мФормаОтчета.Области.П01000903.Значение + мФормаОтчета.Области.П01001003.Значение + мФормаОтчета.Области.П01001103.Значение + мФормаОтчета.Области.П01001203.Значение;
	мФормаОтчета.Области.П01000604.Значение = мФормаОтчета.Области.П01000704.Значение + мФормаОтчета.Области.П01000804.Значение + мФормаОтчета.Области.П01000904.Значение + мФормаОтчета.Области.П01001004.Значение + мФормаОтчета.Области.П01001104.Значение + мФормаОтчета.Области.П01001204.Значение;
	мФормаОтчета.Области.П01000605.Значение = мФормаОтчета.Области.П01000705.Значение + мФормаОтчета.Области.П01000805.Значение + мФормаОтчета.Области.П01000905.Значение + мФормаОтчета.Области.П01001005.Значение + мФормаОтчета.Области.П01001105.Значение + мФормаОтчета.Области.П01001205.Значение;
	мФормаОтчета.Области.П01000606.Значение = мФормаОтчета.Области.П01000706.Значение + мФормаОтчета.Области.П01000806.Значение + мФормаОтчета.Области.П01000906.Значение + мФормаОтчета.Области.П01001006.Значение + мФормаОтчета.Области.П01001106.Значение + мФормаОтчета.Области.П01001206.Значение;
	
	мФормаОтчета.Области.П01000303.Значение = мФормаОтчета.Области.П01000403.Значение + мФормаОтчета.Области.П01000503.Значение;
	мФормаОтчета.Области.П01000304.Значение = мФормаОтчета.Области.П01000404.Значение + мФормаОтчета.Области.П01000504.Значение;
	мФормаОтчета.Области.П01000305.Значение = мФормаОтчета.Области.П01000405.Значение + мФормаОтчета.Области.П01000505.Значение;
	мФормаОтчета.Области.П01000306.Значение = мФормаОтчета.Области.П01000406.Значение + мФормаОтчета.Области.П01000506.Значение;
	
	мФормаОтчета.Области.П01000203.Значение = мФормаОтчета.Области.П01000303.Значение + мФормаОтчета.Области.П01000603.Значение;
	мФормаОтчета.Области.П01000204.Значение = мФормаОтчета.Области.П01000304.Значение + мФормаОтчета.Области.П01000604.Значение;
	мФормаОтчета.Области.П01000205.Значение = мФормаОтчета.Области.П01000305.Значение + мФормаОтчета.Области.П01000605.Значение;
	мФормаОтчета.Области.П01000206.Значение = мФормаОтчета.Области.П01000306.Значение + мФормаОтчета.Области.П01000606.Значение;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого Обл Из ОбластиСтроки Цикл
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-РП (срочная)");
	КонецЦикла;
...
Функция СформироватьСтруктуруДопАтрибутов()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("АтрибВыгрузкиXML2012Кв1");
	ИмяОбласти = "АтрПериодКвартал";
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 27.07.2012 № 423";
	
	// 1.
	// Гр. 3 >= гр. 4 по всем строкам.
	
	Знак = ">=";
	
	Для НомерСтроки = 1 По 12 Цикл
		
		ТекстФормулы = "стр. " + НомерСтроки + " гр. 3 " + Знак + " стр. " + НомерСтроки + " гр. 4";
		Описание     = "стр. " + НомерСтроки + " гр. 3 " + "<"  + " стр. " + НомерСтроки + " гр. 4";;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П01" + Формат(НомерСтроки, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П01" + Формат(НомерСтроки, "ЧЦ=4; ЧВН=; ЧГ=") + "04"]);
	...
	// 2.
	// Гр. 5 >= гр. 6 по всем строкам.
	
	Знак = ">=";
	
	Для НомерСтроки = 1 По 12 Цикл
		
		ТекстФормулы = "стр. " + НомерСтроки + " гр. 5 " + Знак + " стр. " + НомерСтроки + " гр. 6";
		Описание     = "стр. " + НомерСтроки + " гр. 5 " + "<"  + " стр. " + НомерСтроки + " гр. 6";
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П01" + Формат(НомерСтроки, "ЧЦ=4; ЧВН=; ЧГ=") + "05"]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П01" + Формат(НомерСтроки, "ЧЦ=4; ЧВН=; ЧГ=") + "06"]);
	...
	// 3.
	// Строка 01 >= стр. 02 по всем графам.
	
	Знак = ">=";
	
	Для НомерКолонки = 3 По 6 Цикл
		
		ТекстФормулы = "стр. 01 гр. " + НомерКолонки + " " + Знак + " стр. 02 гр. " + НомерКолонки;
		Описание     = "стр. 01 гр. " + НомерКолонки + " " + "<"  + " стр. 02 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П010001" + Формат(НомерКолонки, "ЧЦ=2; ЧВН=; ЧГ=")]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П010002" + Формат(НомерКолонки, "ЧЦ=2; ЧВН=; ЧГ=")]);
