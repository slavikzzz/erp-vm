...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ОплатаПенсионныхВзносов;
...
Процедура ПредставлениеУведомленияПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	ИначеЕсли Область.Имя = "Фамилия" Или Область.Имя = "Имя" Или Область.Имя = "Отчество" Тогда
		УведомлениеОСпецрежимахНалогообложенияКлиентСервер.ИмяСтраницыВДеревеЖурналаСправок(
			ДеревоСтраниц, ЭтотОбъект["УИДТекущаяСтраница"], ЭтотОбъект["ДанныеМногостраничныхРазделов"]["СведУплПенсВзн"]);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗагрузитьСправкуИзXML(ДополнительныеПараметры)
	УведомлениеОСпецрежимахНалогообложения.ЗагрузитьСправкуИзXMLЗаполнениеДанных(ЭтотОбъект, ДополнительныеПараметры, "СведУплПенсВзн");
	УведомлениеОСпецрежимахНалогообложенияКлиентСервер.ИмяСтраницыВДеревеЖурналаСправок(
		ДеревоСтраниц, ЭтотОбъект["УИДТекущаяСтраница"], ЭтотОбъект["ДанныеМногостраничныхРазделов"]["СведУплПенсВзн"])
КонецПроцедуры
...
Процедура СформироватьДеревоСтраниц() Экспорт
	...
	СтрРег.ИндексКартинки = 1;
	СтрРег.ИмяМакета = "СведУплПенсВзн_2023";
	СтрРег.Многостраничность = Истина;
	СтрРег.Многострочность = Ложь;
	СтрРег.УИД = Новый УникальныйИдентификатор;
	СтрРег.ИДНаименования = "СведУплПенсВзн";
КонецПроцедуры
...
Функция СформироватьВыгрузкуИПолучитьДанные() Экспорт 
	...
	СтруктураВыгрузки.Вставить("ИмяФайла", "1151157_5.01000_04.tif");
