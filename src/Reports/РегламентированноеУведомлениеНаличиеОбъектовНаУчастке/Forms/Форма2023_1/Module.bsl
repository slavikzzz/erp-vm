...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.НаличиеОбъектовНаУчастке;
...
Процедура ЗаполнитьНачальныеДанные() Экспорт
	...
	Иначе
		СтрокаСведений = "ИННФЛ,ФИО,ТелДом,КодУдЛичн,СерияУдЛичн,НомерУдЛичн,ДатаУдЛичн,ОрганВыданУдЛичн,ДатаРожд,МестоРожд";
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(Объект.Организация, Объект.ДатаПодписи, СтрокаСведений);
		...
		ДанныеУведомленияТитульный.Вставить("СерНомДок", СокрЛП(СведенияОбОрганизации.СерияУдЛичн + " " + СведенияОбОрганизации.НомерУдЛичн));
	КонецЕсли;
...
Процедура СформироватьДеревоСтраниц() Экспорт
	...
	СтрРег.Наименование = "Сведения о" + символы.ПС + "земельных участках";
...
Процедура ПодобратьОС()
	...
	ПараметрыОтчета.Вставить("ТекущееИДНаименования", ЭтотОбъект["ТекущееИДНаименования"]);
	...
	РегламентированнаяОтчетностьКлиентПереопределяемый.ПередЗаполнениемОтчета(
		Объект.ИмяОтчета, ПараметрыОтчета, ЭтотОбъект, ОписаниеОповещения, СтандартнаяОбработка);
КонецПроцедуры
...
Процедура ПредставлениеУведомленияВыбор(Элемент, Область, СтандартнаяОбработка)
	Если Область.Имя = "ПодобратьОС" И Область.Гиперссылка = Истина Тогда
		ПодобратьОС();
		СтандартнаяОбработка = Ложь;
		Возврат;
	КонецЕсли;
