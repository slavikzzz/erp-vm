...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "16/10/2015";
	// Код отчета, равен коду по ОКУД
	СтруктураРеквизитовФормы.мКодОтчета   = "0613035";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	РегламентированнаяОтчетность.ОбновитьШаблоныЧерезВебСервисНаСервере(ЭтотОбъект, "АтрибВыгрузкиXML2014Кв1");
...
Процедура ВосстановитьСохраненныеДанные()
	...
	СписокСохранения.Свойство("ВТ1", ВТ1);
	СписокСохранения.Свойство("ВТ2", ВТ2);
	СписокСохранения.Свойство("ВТ3", ВТ3);
	СписокСохранения.Свойство("ВТ4", ВТ4);
	СписокСохранения.Свойство("ВТ5", ВТ5);
	СписокСохранения.Свойство("ВТ6", ВТ6);
	Для Инд = 1 По 6 Цикл 
		СписокСохранения.Свойство("ВТ"+Инд, ЭтаФорма["ВТ"+Инд]);
	КонецЦикла;
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант = Неопределено, ВидДокументаНомерКорректировкиИзменен = Неопределено, ПоказателиОтчета = Неопределено) Экспорт
	...
	Для Инд = 1 По 6 Цикл 
		СписокСохранения.Вставить("ВТ"+Инд, ЭтаФорма["ВТ"+Инд]);
	КонецЦикла;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьШрифты(ЭтаФорма)
	ШрифтСтарый = ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области.Найти("ВТ1").Шрифт;
	Для Инд = 1 По 6 Цикл 
		ШрифтНовый = Новый Шрифт(ШрифтСтарый.Имя, ШрифтСтарый.Размер, ШрифтСтарый.Полужирный, ШрифтСтарый.Наклонный, ШрифтСтарый.Подчеркивание, ЭтаФорма["ВТ"+Инд], ШрифтСтарый.Масштаб);
		ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области.Найти("ВТ"+Инд).Шрифт = ШрифтНовый;
	КонецЦикла;
КонецПроцедуры
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 2) = "ВТ" Тогда 
		Зачеркивание = Не ЭтаФорма[Область.Имя];
		ЭтаФорма[Область.Имя] = Зачеркивание;
		ШрифтСтарый = Область.Шрифт;
		ШрифтНовый = Новый Шрифт(ШрифтСтарый.Имя, ШрифтСтарый.Размер, ШрифтСтарый.Полужирный, ШрифтСтарый.Наклонный, ШрифтСтарый.Подчеркивание, Зачеркивание, ШрифтСтарый.Масштаб);
		Область.Шрифт = ШрифтНовый;
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
	ИначеЕсли Лев(Область.Имя, 4) = "П00_" Тогда 
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "s_vidtr");
		...
		КонецЕсли;
	ИначеЕсли Лев(Область.Имя, 5) = "П001_" Или Лев(Область.Имя, 5) = "П002_" Тогда 
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "s_oksm");
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Инд = 1 По 10 Цикл 
		Суффикс = "_"+Инд;
		Области["П20"+Суффикс].Значение = Области["П21"+Суффикс].Значение+Области["П22"+Суффикс].Значение+Области["П23"+Суффикс].Значение+Области["П24"+Суффикс].Значение;
		Области["П16"+Суффикс].Значение = Области["П17"+Суффикс].Значение+Области["П18"+Суффикс].Значение+Области["П19"+Суффикс].Значение;
		Области["П11"+Суффикс].Значение = Области["П12"+Суффикс].Значение+Области["П13"+Суффикс].Значение+Области["П14"+Суффикс].Значение+Области["П15"+Суффикс].Значение;
		Области["П07"+Суффикс].Значение = Области["П08"+Суффикс].Значение+Области["П09"+Суффикс].Значение+Области["П10"+Суффикс].Значение;
		Области["П02"+Суффикс].Значение = Области["П03"+Суффикс].Значение+Области["П04"+Суффикс].Значение+Области["П05"+Суффикс].Значение+Области["П06"+Суффикс].Значение;
		Области["П01"+Суффикс].Значение = Области["П02"+Суффикс].Значение+Области["П07"+Суффикс].Значение;
	КонецЦикла;
КонецПроцедуры
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателейНеПодлежащихОчистке.Добавить(ВыбТабличноеПоле.Области["ВыбратьОтчетПрошлогоПериода"].Значение,"ВыбратьОтчетПрошлогоПериода");

	ВыбТабличноеПоле.Очистить();
	ВыбТабличноеПоле.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(
		"ФормаОтчета2014Кв1_ФормаОтчета"));
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 8-ВЭС (транспортные услуги)");
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть2а"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть2б"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 8-ВЭС (транспортные услуги)");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_oksm", 512, , "Выбор страны", КоллекцияСписковВыбора["s_oksm"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_vidtr", 512, , "Выбор кода транспорта", КоллекцияСписковВыбора["s_vidtr"]);
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	СмежнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя,"П001_","П002_"));
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Обл2 = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя,"П001_","П002_"));
	Обл1 = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя,"П002_","П001_"));
	Обл1.Значение = РезультатВыбора.Название;
	Обл2.Значение = РезультатВыбора.Код;
...
Функция УниверсальнаяВыгрузкаXMLПроверкаДанныхНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	КонецЕсли;
	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2014Кв1");
	РегламентированнаяОтчетность.ИзменитьВерсиюФорматаСПроверкойСовместимостиШаблонов(ЭтотОбъект, "АтрибВыгрузкиXML2014Кв1", "ШаблонРосстата2014Кв1", ПараметрыВыгрузки);
	ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки);
	Если НЕ РегламентированнаяОтчетность.ПроверитьДанные(ЭтаФорма, ДеревоВыгрузки) Тогда
		КоллекцияСписковВыбора = "";
		Возврат Ложь;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Для Каждого Обл Из ПолеТабличногоДокументаФормаОтчета.Области Цикл 
		Если Обл.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник И Обл.СодержитЗначение = Истина И СтрНайти(Обл.Имя, "_") <> 0 Тогда
			ПараметрыВыгрузки.Вставить(Обл.Имя, Обл.Значение);
		КонецЕсли;
	КонецЦикла;
...
Функция ПроверитьКонтрольныеСоотношенияИзШаблонаВФоне()
	...
	ПараметрыПроверкиКонтрольныхСоотношений.Вставить("ШаблонРосстата", "ШаблонРосстата2014Кв1");
	ПараметрыПроверкиКонтрольныхСоотношений.Вставить("Атрибуты", "АтрибВыгрузкиXML2014Кв1");
	ПараметрыПроверкиКонтрольныхСоотношений.Вставить("СхемаВыгрузки", "СхемаВыгрузкиXML2014Кв1");
...
Функция СформироватьПараметрыВыбораОтчетаПредыдущегоПериода() Экспорт 
	...
	Парам.Вставить("ИсточникОтчета", "РегламентированныйОтчетСтатистикаФорма8ВЭСТрусл");
	Парам.Вставить("ВидОтправляемогоДокумента", ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.СтатистикаФорма8ВЭСТрусл"));
