...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "22/01/2014";
	// Код отчета, равен коду по ОКУД.
	СтруктураРеквизитовФормы.мКодОтчета = "0608005";
...
Процедура ВосстановитьСохраненныеДанные()
	...
	// Восстанавливаем значение флага отключения выбора из списка.
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Область.Имя = "П000001000101" Или Область.Имя = "П000001000102"
		 Или Область.Имя = "П000001000201" Или Область.Имя = "П000001000202" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "Программы");
		ИначеЕсли СтрНайти(Область.Имя, "_") <> 0 Тогда
		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			...
			Если Сред(Область.Имя, 12, 3) = "03_" Или Сред(Область.Имя, 12, 3) = "04_" Тогда
				// Фильтр единиц измерения по коду индикатора,
				// указанного в колонке 01 многострочной части.
				ОтборПоКодуИндикатора = Новый Структура;
				Если СвойстваПоказателей[4].ТаблицаЗначений.НайтиСтроки(
					Новый Структура("КодИндикатора", ПолеТабличногоДокументаФормаОтчета.Области["П000001000101_" + Сред(Область.Имя, 15)].Значение)).Количество() > 0 Тогда
					ОтборПоКодуИндикатора.Вставить("КодИндикатора", СвойстваПоказателей[4].ТаблицаЗначений.НайтиСтроки(
						Новый Структура("КодИндикатора", ПолеТабличногоДокументаФормаОтчета.Области["П000001000101_" + Сред(Область.Имя, 15)].Значение))[0].КодИндикатора);
					ФильтрМассив = ТЗВыбора.НайтиСтроки(ОтборПоКодуИндикатора);
					ТЗВыбора.Очистить();
				...
				Иначе
					ТЗВыбора.Очистить();
				КонецЕсли;
				ПерваяСтрокаТЗВыбора          = ТЗВыбора.Вставить(0);
				ПерваяСтрокаТЗВыбора.Код      = "";
				ПерваяСтрокаТЗВыбора.Название = "Пустое значение";
			КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-ФП (индикаторы)");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1ФПинд");
...
Процедура СформироватьСпискиВыбора()
	...
	Для Каждого Область Из МакетСоставаПоказателей.Области Цикл
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
			...
			Если Область.Имя = "СпрЕдИзм" Тогда
				ТаблСписка.Колонки.Добавить("КодИндикатора",,, МакетСоставаПоказателей.Область(ВерхОбласти, 3, ВерхОбласти, 3).ШиринаКолонки);
			КонецЕсли;
			Для НомСтр = ВерхОбласти По НизОбласти Цикл
				...
				Если КодПоказателя <> "###" Тогда
					...
					Если Область.Имя = "СпрЕдИзм" Тогда
						НовСтрока.КодИндикатора = СокрП(МакетСоставаПоказателей.Область(НомСтр, 3).Текст);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "Программы", 250, , "Выбор программы (подпрограммы)",
		КоллекцияСписковВыбора["СпрПрограмм"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000101", 250, , "Выбор индикатора",
		КоллекцияСписковВыбора["СпрИндикаторов"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000102", 250, , "Выбор индикатора",
		КоллекцияСписковВыбора["СпрИндикаторов"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000103", 250, , "Выбор единицы измерения",
		КоллекцияСписковВыбора["СпрЕдИзм"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000104", 250, , "Выбор единицы измерения",
		КоллекцияСписковВыбора["СпрЕдИзм"]);
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Сред(Область.Имя, 12, 3) = "01_" Или Сред(Область.Имя, 12, 3) = "02_" Тогда
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(КолонкаПоиска,
			Формат(ЭтаФорма[ТекущийЭлемент.Имя].Области["П000001000101_" + Формат(Сред(Область.Имя, 15), "ЧГ=")].Значение, "ЧГ=")));
	ИначеЕсли Сред(Область.Имя, 12, 3) = "03_" Или Сред(Область.Имя, 12, 3) = "04_" Тогда
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(КолонкаПоиска,
			Формат(ЭтаФорма[ТекущийЭлемент.Имя].Области["П000001000104_" + Формат(Сред(Область.Имя, 15), "ЧГ=")].Значение, "ЧГ=")));
	Иначе
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(КолонкаПоиска,
			Формат(ЭтаФорма[ТекущийЭлемент.Имя].Области[Лев(Область.Имя, 11) + "02"].Значение, "ЧГ=")));
	КонецЕсли;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Сред(Область.Имя, 12, 3) = "01_" Или Сред(Область.Имя, 12, 3) = "02_" Тогда
		ЭтаФорма[ТекущийЭлемент.Имя].Области["П000001000101_"
			+ Формат(Сред(Область.Имя, 15), "ЧГ=")].Значение = ВыборКод;
		ЭтаФорма[ТекущийЭлемент.Имя].Области["П000001000102_"
			+ Формат(Сред(Область.Имя, 15), "ЧГ=")].Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборНазвание, "");
		// Очистка единиц измерения.
		ЭтаФорма[ТекущийЭлемент.Имя].Области["П000001000103_"
			+ Формат(Сред(Область.Имя, 15), "ЧГ=")].Значение = "";
		ЭтаФорма[ТекущийЭлемент.Имя].Области["П000001000104_"
			+ Формат(Сред(Область.Имя, 15), "ЧГ=")].Значение = "";
	ИначеЕсли Сред(Область.Имя, 12, 3) = "03_" Или Сред(Область.Имя, 12, 3) = "04_" Тогда
		ЭтаФорма[ТекущийЭлемент.Имя].Области["П000001000103_"
			+ Формат(Сред(Область.Имя, 15), "ЧГ=")].Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборНазвание, "");
		ЭтаФорма[ТекущийЭлемент.Имя].Области["П000001000104_"
			+ Формат(Сред(Область.Имя, 15), "ЧГ=")].Значение = ВыборКод;
	Иначе
		ЭтаФорма[ТекущийЭлемент.Имя].Области["П000001"
			+ Формат(Сред(Область.Имя, 8, 4), "ЧГ=") + "01"].Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборНазвание, "");
		ЭтаФорма[ТекущийЭлемент.Имя].Области["П000001"
			+ Формат(Сред(Область.Имя, 8, 4), "ЧГ=") + "02"].Значение = ВыборКод;;
	КонецЕсли;
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2013Кв1");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если Не ЗначениеЗаполнено(ПараметрыВыгрузки.Программа) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не указан код программы (подпрограммы) по бюджетной классификации!",
	...
	// КОНТРОЛЬ показателей формы (Указания ФСН по заполнению Формы 1-ФП (индикаторы)).
	// Данные графы 10 должны быть больше или равны данным графы 11.
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, "ПР00001000110_" + Формат(НомСтр, "ЧГ=")) Цикл
		Попытка
			Данные10Графы = ?(ЗначениеЗаполнено(ПараметрыВыгрузки["ПР00001000110_" + Формат(НомСтр, "ЧГ=")]),
				Число(ПараметрыВыгрузки["ПР00001000110_" + Формат(НомСтр, "ЧГ=")]),0);
			Данные11Графы = ?(ЗначениеЗаполнено(ПараметрыВыгрузки["ПР00001000111_" + Формат(НомСтр, "ЧГ=")]),
				Число(ПараметрыВыгрузки["ПР00001000111_" + Формат(НомСтр, "ЧГ=")]),0);
			Если Не (Данные10Графы >= Данные11Графы) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: в стр. с номером индикатора "
						+ ПараметрыВыгрузки["ПР00001000101_" + Формат(НомСтр, "ЧГ=")]
						+ " данные гр. 10 должны быть больше или равны данным гр. 11!",
...
Функция СформироватьСтруктуруПоказателей()
	...
	ДополнительныеПоказатели1ФПинд(мДанныеОтчета.ПолеТабличногоДокументаФормаОтчета, "П000001000101");
	
КонецФункции

&НаСервере
Процедура ДополнительныеПоказатели1ФПинд(ТекСтруктура, ТекПоказатель)
	
	Перем ТекКодЕдИзм, ТекКодИндикатора, ТекЗначение;
	
	НомСуффикса   = 1;
	НомСуффикса_  = 1;
	НомСуффикса__ = 1;
	Пока ТекСтруктура.Свойство(ТекПоказатель + "_" + Формат(НомСуффикса, "ЧГ="), ТекКодИндикатора) Цикл
		Если ЗначениеЗаполнено(ТекКодИндикатора) Тогда
			ТекСтруктура.Свойство(Лев(ТекПоказатель, 11) + "04_" + Формат(НомСуффикса, "ЧГ="), ТекКодЕдИзм);
			Если ЗначениеЗаполнено(ТекКодЕдИзм) Тогда
				
				// Заполняем раздел 1 по схеме выгрузки Росстата.
				ТекСтруктура.Вставить("ПГ00001000101_" + Формат(НомСуффикса_, "ЧГ="), ТекКодИндикатора);
				ТекСтруктура.Вставить("ПГ00001000104_" + Формат(НомСуффикса_, "ЧГ="), ТекКодЕдИзм);
				
				ТекСтруктура.Свойство(Лев(ТекПоказатель, 11) + "05_" + Формат(НомСуффикса, "ЧГ="), ТекЗначение);
				ТекСтруктура.Вставить("ПГ00001000105_" + Формат(НомСуффикса_, "ЧГ="), ТекЗначение);
				
				ТекСтруктура.Свойство(Лев(ТекПоказатель, 11) + "06_" + Формат(НомСуффикса, "ЧГ="), ТекЗначение);
				ТекСтруктура.Вставить("ПГ00001000106_" + Формат(НомСуффикса_, "ЧГ="), ТекЗначение);
				
				ТекСтруктура.Свойство(Лев(ТекПоказатель, 11) + "07_" + Формат(НомСуффикса, "ЧГ="), ТекЗначение);
				ТекСтруктура.Вставить("ПГ00001000107_" + Формат(НомСуффикса_, "ЧГ="), ТекЗначение);
				
				ТекСтруктура.Свойство(Лев(ТекПоказатель, 11) + "08_" + Формат(НомСуффикса, "ЧГ="), ТекЗначение);
				ТекСтруктура.Вставить("ПГ00001000108_" + Формат(НомСуффикса_, "ЧГ="), ТекЗначение);
				
				ТекСтруктура.Свойство(Лев(ТекПоказатель, 11) + "09_" + Формат(НомСуффикса, "ЧГ="), ТекЗначение);
				ТекСтруктура.Вставить("ПГ00001000109_" + Формат(НомСуффикса_, "ЧГ="), ТекЗначение);
				
				ТекСтруктура.Свойство(Лев(ТекПоказатель, 11) + "10_" + Формат(НомСуффикса, "ЧГ="), ТекЗначение);
				ТекСтруктура.Вставить("ПГ00001000110_" + Формат(НомСуффикса_, "ЧГ="), ТекЗначение);
				
				ТекСтруктура.Свойство(Лев(ТекПоказатель, 11) + "11_" + Формат(НомСуффикса, "ЧГ="), ТекЗначение);
				ТекСтруктура.Вставить("ПГ00001000111_" + Формат(НомСуффикса_, "ЧГ="), ТекЗначение);
				
				НомСуффикса_ = НомСуффикса_ + 1;
				
			Иначе
				
				// Заполняем раздел 2 по схеме выгрузки Росстата
				// (когда код единицы измерения равен нулю).
				ТекСтруктура.Вставить("ПД00001000101_" + Формат(НомСуффикса__, "ЧГ="), ТекКодИндикатора);
				ТекСтруктура.Вставить("ПД00001000102_" + Формат(НомСуффикса__, "ЧГ="), 1);
				
				НомСуффикса__ = НомСуффикса__ + 1;
...
Функция СформироватьСтруктуруПараметров()
	...
	ОтчПериод  = Месяц(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) / 3 + 12;
	...
	Если ЗначениеЗаполнено(ПараметрыВыгрузки["ПР00001000102"])
		И Не ЗначениеЗаполнено(ПараметрыВыгрузки["ПР00001000202"]) Тогда
		ПараметрыВыгрузки.Вставить("Программа", ПараметрыВыгрузки["ПР00001000102"]);
	ИначеЕсли Не ЗначениеЗаполнено(ПараметрыВыгрузки["ПР00001000102"])
		И ЗначениеЗаполнено(ПараметрыВыгрузки["ПР00001000202"]) Тогда
		ПараметрыВыгрузки.Вставить("Программа", ПараметрыВыгрузки["ПР00001000202"]);
	ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["ПР00001000102"])
		И ЗначениеЗаполнено(ПараметрыВыгрузки["ПР00001000202"]) Тогда
		ПараметрыВыгрузки.Вставить("Программа", ПараметрыВыгрузки["ПР00001000202"]);
	Иначе
		ПараметрыВыгрузки.Вставить("Программа", "");
	КонецЕсли;
