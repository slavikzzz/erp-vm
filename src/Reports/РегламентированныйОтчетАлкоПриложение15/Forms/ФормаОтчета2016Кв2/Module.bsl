...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КолКолонокИдГруппы11 = 16;
	КолКолонокИдГруппы12 = 16;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010000108", "П000010000109;П000010000110;П000010000111;П000010000112");
	
	// Многострочная часть 2
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010000302", "П000010000301;П000010000303");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010000308", "П000010000309;П000010000310;П000010000311;П000010000312");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоПБОЮЛ Тогда
		// в основной форме фильтр по ЮрЛицам - эта блокировка на всякий непредвиденный случай.		
		Отказ = Истина;		
		Возврат;
	...
	Если ИмяРаздела = "Декларация" Тогда
		...
		Если НЕ РазрешитьВвод Тогда
			...
			Пока Форма.ТабличныйДокумент.Области.Найти("П000010000101_" + Постфикс) <> Неопределено Цикл
				...
				УстановитьЦветФонаЯчейки(Форма, "П000010000108_" + Постфикс, 5, "Декларация");
				УстановитьЦветФонаЯчейки(Форма, "П000010000109_" + Постфикс, 5, "Декларация");
				УстановитьЦветФонаЯчейки(Форма, "П000010000110_" + Постфикс, 5, "Декларация");
				УстановитьЦветФонаЯчейки(Форма, "П000010000111_" + Постфикс, 5, "Декларация");
				УстановитьЦветФонаЯчейки(Форма, "П000010000112_" + Постфикс, 5, "Декларация");
				УстановитьЦветФонаЯчейки(Форма, "П000010000113_" + Постфикс, 5, "Декларация");
				УстановитьЦветФонаЯчейки(Форма, "П000010000114_" + Постфикс, 5, "Декларация");
				УстановитьЦветФонаЯчейки(Форма, "П000010000115_" + Постфикс, 5, "Декларация");
...
Процедура СформироватьСпискиВыбора()
	...
	ИмяМакета = "Списки2016Кв2";
	МакетСоставаПоказателей = ПолучитьМакетИзОбъекта(ИмяМакета);
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010000113", 3, , "Выбор вида продукции", КоллекцияСписковВыбора["ВидыПродукции"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010000114", 3, , "Выбор вида продукции", КоллекцияСписковВыбора["ВидыПродукции"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010000313", 3, , "Выбор вида продукции", КоллекцияСписковВыбора["ВидыПродукции"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010000314", 3, , "Выбор вида продукции", КоллекцияСписковВыбора["ВидыПродукции"]);
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Декларация" Тогда
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Для Инд = 1 По КоличествоСтрок Цикл
					...
					Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000010000108, ОбластьИмя) > 0 Тогда   

						Графа10 = Форма.ТабличныйДокумент.Области["П0000100001" + "09" + ПостфиксСтроки].Значение;
						Графа11 = Форма.ТабличныйДокумент.Области["П0000100001" + "10" + ПостфиксСтроки].Значение;
						Графа12 = Форма.ТабличныйДокумент.Области["П0000100001" + "11" + ПостфиксСтроки].Значение;
						Графа13 = Форма.ТабличныйДокумент.Области["П0000100001" + "12" + ПостфиксСтроки].Значение;
						
						ЗначениеГрафы09 = Графа10 + Графа11 + Графа12 + Графа13;
						
						Форма.ТабличныйДокумент.Области["П0000100001" + "08" + ПостфиксСтроки].Значение = ЗначениеГрафы09;
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Для Инд = 1 По КоличествоСтрок Цикл
					...
					Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000010000308, ОбластьИмя) > 0 Тогда   

						Графа10 = Форма.ТабличныйДокумент.Области["П0000100003" + "09" + ПостфиксСтроки].Значение;
						Графа11 = Форма.ТабличныйДокумент.Области["П0000100003" + "10" + ПостфиксСтроки].Значение;
						Графа12 = Форма.ТабличныйДокумент.Области["П0000100003" + "11" + ПостфиксСтроки].Значение;
						Графа13 = Форма.ТабличныйДокумент.Области["П0000100003" + "12" + ПостфиксСтроки].Значение;
						
						ЗначениеГрафы09 = Графа10 + Графа11 + Графа12 + Графа13;
						
						Форма.ТабличныйДокумент.Области["П0000100003" + "08" + ПостфиксСтроки].Значение = ЗначениеГрафы09;
	...
	Для Каждого Страница Из ТаблицаРаздела Цикл
		...
		Если ТаблицаБлока <> Неопределено Тогда
			...
			Для Каждого ТекСтрока из ТаблицаБлока Цикл
				...
				// Графа 09
				Графа10 = ТекСтрока["П0000100003" + "09"];
				Графа11 = ТекСтрока["П0000100003" + "10"];
				Графа12 = ТекСтрока["П0000100003" + "11"];						
				Графа13 = ТекСтрока["П0000100003" + "12"];
				
				ЗначениеГрафы09 = Графа10 + Графа11 + Графа12 + Графа13;
				
				ТекСтрока["П0000100003" + "08"] = ЗначениеГрафы09;
		...
		Если ТаблицаБлока <> Неопределено Тогда
			...
			Для Каждого ТекСтрока из ТаблицаБлока Цикл
				...
				// Графа 09
				Графа10 = ТекСтрока["П0000100001" + "09"];
				Графа11 = ТекСтрока["П0000100001" + "10"];
				Графа12 = ТекСтрока["П0000100001" + "11"];						
				Графа13 = ТекСтрока["П0000100001" + "12"];
				
				ЗначениеГрафы09 = Графа10 + Графа11 + Графа12 + Графа13;
				
				ТекСтрока["П0000100001" + "08"] = ЗначениеГрафы09;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если СтрНайти(КодПоказателя, "П000010000114") = 1 ИЛИ СтрНайти(КодПоказателя, "П000010000314") = 1 Тогда
		КолонкаПоиска = "Название";
	КонецЕсли;
	
	Если СтрНайти(КодПоказателя, "П000010000113") = 1 ИЛИ СтрНайти(КодПоказателя, "П000010000313") = 1 Тогда
		КолонкаПоиска = "Код";
	КонецЕсли;
...
Процедура ЗаполнитьСвязанныеПоказатели(ИмяОбласти, РезультатВыбора)
	...
	Если СтрНайти(ИмяОбласти, "П000010000114") = 1 Тогда
		ИмяСвязаннойОбласти = "П000010000113" + Сред(ИмяОбласти, 14);
	...
	ИначеЕсли СтрНайти(ИмяОбласти, "П000010000113") = 1 Тогда
		ИмяСвязаннойОбласти = "П000010000114" + Сред(ИмяОбласти, 14);
		СвязаннаяОбласть = ТабличныйДокумент.Области[ИмяСвязаннойОбласти];
		СвязаннаяОбласть.Значение = ?(ПустаяСтрока(РезультатВыбора.Код), "", РезультатВыбора.Название);
		
	ИначеЕсли СтрНайти(ИмяОбласти, "П000010000314") = 1 Тогда
		ИмяСвязаннойОбласти = "П000010000313" + Сред(ИмяОбласти, 14);
		СвязаннаяОбласть = ТабличныйДокумент.Области[ИмяСвязаннойОбласти];
		СвязаннаяОбласть.Значение = РезультатВыбора.Код;
		Если ПустаяСтрока(РезультатВыбора.Код) Тогда
			ТабличныйДокумент.Области[ИмяОбласти].Значение = "";
	...
	ИначеЕсли СтрНайти(ИмяОбласти, "П000010000313") = 1 Тогда
		ИмяСвязаннойОбласти = "П000010000314" + Сред(ИмяОбласти, 14);
		СвязаннаяОбласть = ТабличныйДокумент.Области[ИмяСвязаннойОбласти];
		СвязаннаяОбласть.Значение = ?(ПустаяСтрока(РезультатВыбора.Код), "", РезультатВыбора.Название);
...
Процедура Выгрузить(Команда)
	...
	ТекстПредупреждения = НСтр("ru = 'Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде редакции формы, утвержденной ПП РФ №411 от 13.05.2016, будет реализована в будущих версиях программы.';
								|en = 'Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде редакции формы, утвержденной ПП РФ №411 от 13.05.2016, будет реализована в будущих версиях программы.'");
...
Процедура ВывестиНаПечатьДекларация(СтруктураПараметровСтраницы, СоставленаНа)
	...
	Для Каждого Лист Из ЭтаФорма[мСтруктураМногостраничныхРазделов.Декларация] Цикл
		...
		Для Каждого СтрокаДанных Из ДанныеПоПодразделению Цикл
			...
			Если ОсталосьСтрок < 3 Тогда
			    // переходим на новый лист, если осталось место только на одну строку
				ПечатнаяФорма.ВывестиГоризонтальныйРазделительСтраниц();
				ОсталосьСтрок = ВысотаСтраницыВСтроках;
				// выводим заголовок таблицы
				ПечатнаяФорма.Вывести(Область_ЗаголовокТаблицы);			
