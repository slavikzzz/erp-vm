...
Функция ПодготовитьПараметрыОтчета()
	...
	// Для организаций на УСН и ИП отчет по БУ формируется в формате отчета по НУ
	Если УчетнаяПолитика.СистемаНалогообложения(Отчет.Организация,Отчет.НачалоПериода) = Перечисления.СистемыНалогообложения.Упрощенная
		Или Отчет.Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		ПараметрыОтчета.ПоказательБУ = Ложь;			
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	Форма.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Справка-расчет транспортных расходов %1 %2';
			|en = 'Detailed calculation of transportation expenses %1 %2'"),
		БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(
			?(Отчет.СНачалаГода,
				НачалоГода(Отчет.НачалоПериода),
				Отчет.НачалоПериода),
			Отчет.КонецПериода),
		ТекстОрганизация);
