...
Функция ТекстЗапроса(ПоказыватьПродажи)
	...
	|ИЗ
	|	РегистрНакопления.ВыручкаИСебестоимостьПродаж.Обороты(ДАТАВРЕМЯ(1,1,1) {(ДАТАВРЕМЯ(1,1,1))}, &КонецПериода {(&КонецПериода)}, Регистратор, 
	|		&ТекстУсловияОтбораАналитикаУчетаПоПартнерам
	|	) КАК ИсходныеПродажи
	|ГДЕ
