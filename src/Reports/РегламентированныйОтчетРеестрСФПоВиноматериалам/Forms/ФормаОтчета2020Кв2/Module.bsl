...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П1000001") Тогда
		...
		Сп.Добавить("295", "295 - виноматериалы");
		Сп.Добавить("296", "296 - виноградное сусло");
		Сп.Добавить("297", "297 - Фруктовое сусло");
...
Процедура ВывестиНаПечатьОтчет(СтруктураПараметров, ВыбранныеСтраницы, СтрПоля = Неопределено)
	...
	Виноматериалы    = ПечФорма.Получитьобласть("Виноматериалы");
	ВиноградноеСусло = ПечФорма.Получитьобласть("ВиноградноеСусло");
	ФруктовоеСусло   = ПечФорма.Получитьобласть("ФруктовоеСусло");
	ПодакцТоварПусто = ПечФорма.Получитьобласть("ПодакцТоварПусто");
	...
	Для каждого СтраницаРеестра Из ДеревоДанныхРаздела.Строки Цикл
		...
		Если СтраницаРеестраИндекс = 0 Тогда
			...
			ЕстьЗаполненныйПодакцТовар = Ложь;
			
			Если ЗначениеЗаполнено(СтруктураДанныхТитульный.СортВиноматериала)
				ИЛИ ЗначениеЗаполнено(СтруктураДанныхТитульный.ОбъемВиноматериала) Тогда
				Виноматериалы.Параметры.Заполнить(СтруктураДанныхТитульный);
				ПечФорма.Вывести(Виноматериалы);
				ЕстьЗаполненныйПодакцТовар = Истина;
			КонецЕсли;
			Если ЗначениеЗаполнено(СтруктураДанныхТитульный.СортВиноградногоСусла)
				ИЛИ ЗначениеЗаполнено(СтруктураДанныхТитульный.ОбъемВиноградногоСусла) Тогда
				ВиноградноеСусло.Параметры.Заполнить(СтруктураДанныхТитульный);
				ПечФорма.Вывести(ВиноградноеСусло);
				ЕстьЗаполненныйПодакцТовар = Истина;
			КонецЕсли;
			Если ЗначениеЗаполнено(СтруктураДанныхТитульный.СортФруктовогоСусла)
				ИЛИ ЗначениеЗаполнено(СтруктураДанныхТитульный.ОбъемФруктовогоСусла) Тогда
				ФруктовоеСусло.Параметры.Заполнить(СтруктураДанныхТитульный);
				ПечФорма.Вывести(ФруктовоеСусло);
				ЕстьЗаполненныйПодакцТовар = Истина;
			КонецЕсли;
			
			Если НЕ ЕстьЗаполненныйПодакцТовар Тогда
				ПечФорма.Вывести(ПодакцТоварПусто);
			КонецЕсли;
