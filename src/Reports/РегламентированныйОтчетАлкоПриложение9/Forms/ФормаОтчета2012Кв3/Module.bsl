...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КолКолонокИдГруппы1 = 17;
	КолКолонокИдГруппы2 = 17;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010000101", 2, , "Выбор вида перевозки", КоллекцияСписковВыбора["ВидыПеревозок"]);
	...
	Если ИмяТекущейСтраницыПанели = "Декларация" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000010000408, ОбластьИмя) > 0 Тогда
			...
			Для Инд = 1 По СтруктураГруппы.Количество() Цикл
				ПостфиксСтроки = "_" + Формат(Инд, "ЧГ=0");
				Для Каждого Графа Из МассивНомеровГраф Цикл
					ЗначениеГрафы = Форма.ТабличныйДокумент.Области["П0000100003" + Графа + ПостфиксСтроки].Значение;
					СуммыПоГрафам.Вставить(Графа, СуммыПоГрафам[Графа] + ЗначениеГрафы);
				КонецЦикла;
			КонецЦикла;
			
			Для Каждого Графа Из МассивНомеровГраф Цикл
				Форма.ТабличныйДокумент.Области["П0000100004" + Графа].Значение = СуммыПоГрафам[Графа];
			КонецЦикла;
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ИмяОбласти = "ЗаявлениеОКорректировкеПредставление" Тогда
		...
		Иначе
			...
			ПараметрыФормы.Вставить("НомерПриложения",          "9");
	...
	ЭтоАдресВМногострочнойЧасти = СтрНайти(ИмяОбласти, "П000010000106") = 1
	                          ИЛИ СтрНайти(ИмяОбласти, "П000010000306") = 1
	                          ИЛИ СтрНайти(ИмяОбласти, "П000010000113") = 1
	                          ИЛИ СтрНайти(ИмяОбласти, "П000010000313") = 1;
...
Процедура ВвестиАдресКонтрагента(ИмяОбласти, Элемент);
	...
	Если ДополнительныеДанныеСтраницы.Свойство(ИмяОбласти, СохраненныйАдресПеревозчика) Тогда
		ЗаполнитьЗначенияСвойств(Адрес, СохраненныйАдресПеревозчика);
	КонецЕсли;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если СтрНайти(КодПоказателя, "П000000000105") = 1 Тогда
...
&НаСервере
Функция ПроверитьЗаполнениеОП(СтатусПроверки, ЛистРаздела, НомерЛиста, ИмяРаздела, КПП_НеОбязателен = Ложь)
...
Процедура ПроверитьЗаполнениеДекларация(ПараметрыВыгрузки, СтатусПроверки)
	...
	Если КоличествоСтрокТаблицыПоОрганизации > ОбщееКоличествоСтрокПоОП Тогда
	    ТекстОписанияОшибки = "В разделе ""Декларация"" в верхней таблице ""Сведения по организации"" " 
...
Процедура ПроверитьЗаполнениеТаблицыДекларации(СтатусПроверки, ИдГруппы, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста, КоличЗаполненных)
	...
	Если ПрефиксИмени = "П0000100001" Тогда
		ИмяТаблицы = """Сведения по организации""";
	...
	Для Каждого СтрокаДекларации Из ДанныеДопСтрок Цикл
		...
		ИначеЕсли НЕ ЗначениеСодержитсяВСписке(ЗначениеГрафы, "АТ, ЖД, МП, РП, ВП, АВ") Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации вид перевозки должен быть в списке АТ, ЖД, МП, РП, ВП, АВ!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателя, НомерЛиста);
		КонецЕсли;
		...
		ЗначениеГрафы = СокрЛП(СтрокаДекларации[ИмяГрафы]);
		Если ПустаяСтрока(ЗначениеГрафы) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации не задан код вида перевозимой продукции!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателя, НомерЛиста);
		ИначеЕсли СтрДлина(ЗначениеГрафы) <> 3 Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации неверно задан код вида перевозимой продукции!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателя, НомерЛиста);
		КонецЕсли;
		...
		Если ПустаяСтрока(ЗначениеГрафы) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации не указано наименование организации-грузоотправителя!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателя, НомерЛиста);
		КонецЕсли;
		...
		ИмяГрафыАдреса = ПрефиксИмени + "06";
		ИмяПоказателяАдреса = ПрефиксИмени + "06_" + НомерСтрокиСтроковый;
		Адрес = СокрЛП(СтрокаДекларации[ИмяГрафыАдреса]);
		
		РоссийскийГрузоотправитель = Истина;
		АдресПолный = Неопределено;
		Если ДополнительныеДанные.Свойство(ИмяПоказателяАдреса, АдресПолный) Тогда
			
		    Если ЗначениеЗаполнено(АдресПолный.Страна) Тогда
			
				РоссийскийГрузоотправитель = (АдресПолный.Страна = "643");
		...
		РоссийскийКонтрагент = РоссийскийГрузоотправитель;
		МожетБытьПустойИНН = Истина;
		МожетБытьПустойКПП = Истина;
		РоссийскийТолькоЮЛ = Истина; РоссийскийТолькоФЛ = Ложь;
		...
		ПроверкаИННКППвСтрокахТаблиц(СтатусПроверки, ИмяРаздела, НомерЛиста, НомерСтрокиСтроковый, ИмяТаблицы, 
									...
									РоссийскийТолькоЮЛ, РоссийскийТолькоФЛ);
		//
		////////////////////////////////////////////////////////////////////////////////
		
		Если ПустаяСтрока(Адрес) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации не указан адрес организации-грузоотправителя!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяАдреса, НомерЛиста);
		Иначе
			Если АдресПолный <> Неопределено Тогда
				Если НЕ РоссийскийГрузоотправитель Тогда
					Если ПустаяСтрока(АдресПолный.Страна) Тогда
						ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации в адреса организации-грузоотправителя не задан код страны!";
						СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяАдреса, НомерЛиста);
					ИначеЕсли СтрДлина(СокрЛП(АдресПолный.Страна))<> 3 Тогда
						ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации в адреса организации-грузоотправителя неверно задан код страны!";
						СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяАдреса, НомерЛиста);
					КонецЕсли;
					
					Если ПустаяСтрока(АдресПолный.Адрес) Тогда
						ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации в адреса организации-грузоотправителя не задан адрес внутри страны!";
						СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяАдреса, НомерЛиста);
					КонецЕсли;
				Иначе
					Если ПустаяСтрока(АдресПолный.Регион) Тогда
						ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации в адреса организации-грузоотправителя не задан регион!";
						СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяАдреса, НомерЛиста);
					КонецЕсли;
				КонецЕсли;
			Иначе
				ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации нарушена внутренняя структура адреса организации-грузоотправителя, требуется повторный ввод адреса!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяАдреса, НомерЛиста);
			КонецЕсли;
		КонецЕсли;
		...
		ИмяГрафы = ПрефиксИмени + "08";
		ИмяПоказателя = ПрефиксИмени + "08_" + НомерСтрокиСтроковый;
		...
		ИмяГрафыИНН = ПрефиксИмени + "11";
		ИмяПоказателяИНН = ПрефиксИмени + "11_" + НомерСтрокиСтроковый;
		ИНН = СокрЛП(СтрокаДекларации[ИмяГрафыИНН]);
		СтрокаДекларации[ИмяГрафыИНН] = ИНН;
		
		ИмяГрафыКПП = ПрефиксИмени + "12";
		ИмяПоказателяКПП = ПрефиксИмени + "12_" + НомерСтрокиСтроковый;
		КПП = СокрЛП(СтрокаДекларации[ИмяГрафыКПП]);
		СтрокаДекларации[ИмяГрафыКПП] = КПП;
		
		ИмяГрафыАдреса = ПрефиксИмени + "13";
		ИмяПоказателяАдреса = ПрефиксИмени + "13_" + НомерСтрокиСтроковый;
		Адрес = СокрЛП(СтрокаДекларации[ИмяГрафыАдреса]);
		
		РоссийскийГрузополучатель = Истина;
		АдресПолный = Неопределено;
		Если ДополнительныеДанные.Свойство(ИмяПоказателяАдреса, АдресПолный) Тогда
			
		    Если ЗначениеЗаполнено(АдресПолный.Страна) Тогда
			
				РоссийскийГрузополучатель = (АдресПолный.Страна = "643");
		...
		РоссийскийКонтрагент = РоссийскийГрузополучатель;
		МожетБытьПустойИНН = Истина;
		МожетБытьПустойКПП = Истина;
		...
		Если ПустаяСтрока(Адрес) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации не указан адрес организации-грузополучателя!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяАдреса, НомерЛиста);
		Иначе
			Если АдресПолный <> Неопределено Тогда
				Если НЕ РоссийскийГрузополучатель Тогда
					Если ПустаяСтрока(АдресПолный.Страна) Тогда
						ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации в адреса организации-грузополучателя не задан код страны!";
						СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяАдреса, НомерЛиста);
					ИначеЕсли СтрДлина(СокрЛП(АдресПолный.Страна))<> 3 Тогда
						ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации в адреса организации-грузополучателя неверно задан код страны!";
						СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяАдреса, НомерЛиста);
					КонецЕсли;
					
					ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации в адреса организации-грузополучателя не задан адрес внутри страны!";
					Если ПустаяСтрока(АдресПолный.Адрес) Тогда
						СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяАдреса, НомерЛиста);
					КонецЕсли;
				Иначе
					Если ПустаяСтрока(АдресПолный.Регион) Тогда
						ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации в адреса организации-грузополучателя не задан регион!";
						СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяАдреса, НомерЛиста);
					КонецЕсли;
				КонецЕсли;
			Иначе
				ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации нарушена внутренняя структура адреса организации-грузополучателя, требуется повторный ввод адреса!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяАдреса, НомерЛиста);
			КонецЕсли;
		КонецЕсли;
		...
		ЗначениеГрафы = СтрокаДекларации[ИмяГрафы];
		Если НЕ ЗначениеЗаполнено(ЗначениеГрафы) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации не указана дата накладной!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателя, НомерЛиста);
		КонецЕсли;
		...
		СтрокаДекларации[ИмяГрафы] = ЗначениеГрафы;
		Если ПустаяСтрока(ЗначениеГрафы) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации не указан номер накладной!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателя, НомерЛиста);
		КонецЕсли;
		...
		ЗначениеГрафы = СтрокаДекларации[ИмяГрафы];
		Если НЕ ЗначениеЗаполнено(ЗначениеГрафы) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации не указана масса груза!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателя, НомерЛиста);
		КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, КорневойУзел, Параметры)
	...
	ТранспортныеСредства.Колонки.Добавить("П000000000008"); // Тип транспортного средства
	ТранспортныеСредства.Колонки.Добавить("П000000000009"); // Грузоподъемность транспортного средства в тоннах
	ТранспортныеСредства.Колонки.Добавить("П000000000010"); // Регистрационный номер транспортного средства
	...
	Для Каждого Страница Из мДанныеОтчета.Декларация Цикл
		...
		ДанныеВыгрузки.Вставить("ДанныеСтраницы", Страница.Данные[0].Значение);
		ДанныеВыгрузки.Вставить("ДанныеДопСтрок", Страница.ДанныеДопСтрок[0].Значение);
		ДанныеВыгрузки.Вставить("ДополнительныеДанные", Страница.ДополнительныеДанные[0].Значение);
...
Процедура ЗаполнитьСведенияПоСтранице(ДанныеВыгрузки, Узел_ОбъемПеревозки)
	...
	ТаблицаПеревозок = ДанныеДопСтрок[СтруктураРеквизитовФормы.мИдГруппы2].Выгрузить();
	
	ТаблицаПеревозок.Колонки.Добавить("ИдГрузоотправителя");
	ТаблицаПеревозок.Колонки.Добавить("ИдГрузополучателя");
	...
	Для Инд = 0 По ТаблицаПеревозок.Количество() - 1 Цикл
		...
		Если ТипЗнч(ДополнительныеДанныеСтраницы) = Тип("Структура") Тогда
			АдресГрузоотправителя = Неопределено;
			ДополнительныеДанныеСтраницы.Свойство("П000010000306" + Постфикс, АдресГрузоотправителя);
			ТаблицаПеревозок[Инд].ИдГрузоотправителя = ПолучитьИдентификаторГрузоотправителя(ДанныеВыгрузки, ТаблицаПеревозок[Инд], АдресГрузоотправителя);
			
			АдресГрузополучателя = Неопределено;
			ДополнительныеДанныеСтраницы.Свойство("П000010000313" + Постфикс, АдресГрузополучателя);
			ТаблицаПеревозок[Инд].ИдГрузополучателя = ПолучитьИдентификаторГрузополучателя(ДанныеВыгрузки, ТаблицаПеревозок[Инд], АдресГрузополучателя);
		КонецЕсли;
	...
	Для Каждого КодВидаПеревозки Из КодыВидовПеревозки Цикл
		...
		ПрототипУзла_ВидПродукции = ПолучитьПодчиненныйЭлемент(Узел_Перевозка, "ВидПродукции");
		
		НомерППВидаПродукции = 0;
		Для Каждого КодВидаПродукции Из ВидыПродукции Цикл
			Узел_ВидПродукции = НовыйУзелИзПрототипа(ПрототипУзла_ВидПродукции);
			
			НомерППВидаПродукции = НомерППВидаПродукции + 1;
			УстановитьЗначениеЭлемента(Узел_ВидПродукции, "ПN", НомерППКодаВидаПеревозки);
			УстановитьЗначениеЭлемента(Узел_ВидПродукции, "П000000000003", КодВидаПродукции);
			
			УсловияОтбора = Новый Структура;
			УсловияОтбора.Вставить("П000010000301", КодВидаПеревозки);
			УсловияОтбора.Вставить("П000010000302", КодВидаПродукции);
			
			СтрокиГрузоотправителей = ТаблицаПеревозок.НайтиСтроки(УсловияОтбора);
			Грузоотправители = ТаблицаПеревозок.Скопировать(СтрокиГрузоотправителей, "ИдГрузоотправителя");
			Грузоотправители.Свернуть("ИдГрузоотправителя");
			Грузоотправители = Грузоотправители.ВыгрузитьКолонку("ИдГрузоотправителя");
			
			ПрототипУзла_Грузоотправитель = ПолучитьПодчиненныйЭлемент(Узел_ВидПродукции, "Грузоотправитель");
			
			НомерППГрузоотправителя = 0;
			Для Каждого ИдГрузоотправителя Из Грузоотправители Цикл
				Узел_Грузоотправитель = НовыйУзелИзПрототипа(ПрототипУзла_Грузоотправитель);
				
				НомерППГрузоотправителя = НомерППГрузоотправителя + 1;
				УстановитьЗначениеЭлемента(Узел_Грузоотправитель, "ПN", НомерППГрузоотправителя);
				УстановитьЗначениеЭлемента(Узел_Грузоотправитель, "ИдГрузоотправителя", ИдГрузоотправителя);
				
				УсловияОтбора = Новый Структура;
				УсловияОтбора.Вставить("П000010000301", КодВидаПеревозки);
				УсловияОтбора.Вставить("П000010000302", КодВидаПродукции);
				УсловияОтбора.Вставить("ИдГрузоотправителя", ИдГрузоотправителя);
				
				СтрокиТранспортныхСредств = ТаблицаПеревозок.НайтиСтроки(УсловияОтбора);
				ТранспортныеСредства = ТаблицаПеревозок.Скопировать(СтрокиТранспортныхСредств, "ИдТС");
				ТранспортныеСредства.Свернуть("ИдТС");
				ТранспортныеСредства = ТранспортныеСредства.ВыгрузитьКолонку("ИдТС");
				
				ПрототипУзла_ТранспортноеСредство = ПолучитьПодчиненныйЭлемент(Узел_Грузоотправитель, "ТранспортноеСредство");
				
				НомерППТранспортогоСредства = 0;
				Для Каждого ИдТС Из ТранспортныеСредства Цикл
					Узел_ТранспортноеСредство = НовыйУзелИзПрототипа(ПрототипУзла_ТранспортноеСредство);
					
					НомерППТранспортогоСредства = НомерППТранспортогоСредства + 1;
					УстановитьЗначениеЭлемента(Узел_ТранспортноеСредство, "ПN", НомерППТранспортогоСредства);
					УстановитьЗначениеЭлемента(Узел_ТранспортноеСредство, "ИдТС", ИдТС);
					
					УсловияОтбора = Новый Структура;
					УсловияОтбора.Вставить("П000010000301", КодВидаПеревозки);
					УсловияОтбора.Вставить("П000010000302", КодВидаПродукции);
					УсловияОтбора.Вставить("ИдГрузоотправителя", ИдГрузоотправителя);
					УсловияОтбора.Вставить("ИдТС", ИдТС);
					
					СтрокиГрузополучателей = ТаблицаПеревозок.НайтиСтроки(УсловияОтбора);
					Грузополучатели = ТаблицаПеревозок.Скопировать(СтрокиГрузополучателей, "ИдГрузополучателя");
					Грузополучатели.Свернуть("ИдГрузополучателя");
					Грузополучатели = Грузополучатели.ВыгрузитьКолонку("ИдГрузополучателя");
					
					ПрототипУзла_Грузополучатель = ПолучитьПодчиненныйЭлемент(Узел_ТранспортноеСредство, "Грузополучатель");
					
					НомерППГрузополучателя = 0;
					Для Каждого ИдГрузополучателя Из Грузополучатели Цикл
						Узел_Грузополучатель = НовыйУзелИзПрототипа(ПрототипУзла_Грузополучатель);
						
						НомерППГрузополучателя = НомерППГрузополучателя + 1;
						УстановитьЗначениеЭлемента(Узел_Грузополучатель, "ПN", НомерППГрузополучателя);
						УстановитьЗначениеЭлемента(Узел_Грузополучатель, "ИдГрузополучателя", ИдГрузополучателя);
						...
						УсловияОтбора.Вставить("ИдГрузоотправителя", ИдГрузоотправителя);
						УсловияОтбора.Вставить("ИдТС", ИдТС);
						УсловияОтбора.Вставить("ИдГрузополучателя", ИдГрузополучателя);
						
						СтрокиСведений = ТаблицаПеревозок.НайтиСтроки(УсловияОтбора);
						
						ПрототипУзла_Свед = ПолучитьПодчиненныйЭлемент(Узел_Грузополучатель, "Свед");
						
						НомерППСведений = 0;
						Для Каждого СтрокаСведений Из СтрокиСведений Цикл
							Узел_Свед = НовыйУзелИзПрототипа(ПрототипУзла_Свед);
							
							НомерППСведений = НомерППСведений + 1;
							УстановитьЗначениеЭлемента(Узел_Свед, "ПN", НомерППСведений);
							УстановитьЗначениеЭлемента(Узел_Свед, "П000000000016", СтрокаСведений.П000010000314);
							УстановитьЗначениеЭлемента(Узел_Свед, "П000000000017", СтрокаСведений.П000010000315);
							УстановитьЗначениеЭлемента(Узел_Свед, "П000000000018", СтрокаСведений.П000010000316);
							
						КонецЦикла;
						РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_Свед);
						
					КонецЦикла;
					РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_Грузополучатель);
					
				КонецЦикла;
				РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_ТранспортноеСредство);
				
			КонецЦикла;
			РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_Грузоотправитель);
			
		КонецЦикла;
		РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_ВидПродукции);
		
	КонецЦикла;
	РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_Перевозка);
...
Функция ПолучитьИдентификаторТранспортногоСредства(ДанныеВыгрузки, СтрокаСведенийТранспортногоСредства)
	...
	СведенияУказаны = ЗначениеЗаполнено(СтрокаСведенийТранспортногоСредства.П000010000307)
	              ИЛИ ЗначениеЗаполнено(СтрокаСведенийТранспортногоСредства.П000010000308)
	              ИЛИ ЗначениеЗаполнено(СтрокаСведенийТранспортногоСредства.П000010000309);
	
	ТранспортныеСредства = ДанныеВыгрузки.ТранспортныеСредства;
	
	Если СведенияУказаны Тогда
		УсловияОтбора = Новый Структура;
		УсловияОтбора.Вставить("П000000000008", СтрокаСведенийТранспортногоСредства.П000010000307);
		УсловияОтбора.Вставить("П000000000009", СтрокаСведенийТранспортногоСредства.П000010000308);
		УсловияОтбора.Вставить("П000000000010", СтрокаСведенийТранспортногоСредства.П000010000309);
		...
		Иначе
			...
			НоваяСтрока.П000000000008 = СтрокаСведенийТранспортногоСредства.П000010000307;
			НоваяСтрока.П000000000009 = СтрокаСведенийТранспортногоСредства.П000010000308;
			НоваяСтрока.П000000000010 = СтрокаСведенийТранспортногоСредства.П000010000309;
...
&НаСервере
Функция ПолучитьИдентификаторГрузоотправителя(ДанныеВыгрузки, СтрокаСведений, АдресКонтрагента)
	
	Результат = 0;
	
	Грузоотправители = ДанныеВыгрузки.Контрагенты;
	
	ИНН = СокрЛП(СтрокаСведений.П000010000304);
	КПП = СокрЛП(СтрокаСведений.П000010000305);
	ЭтоРоссийскийКонтрагент = НЕ (АдресКонтрагента <> Неопределено И ЗначениеЗаполнено(АдресКонтрагента.Страна));
	
	УсловияОтбора = Новый Структура;
	
	УсловияОтбора.Вставить("Наименование", СтрокаСведений.П000010000303);
	...
	Если ДанныеЗаполнены Тогда
		...
		НайденныеСтроки = Грузоотправители.НайтиСтроки(УсловияОтбора);
		...
		Иначе
			...
			Идентификатор = Грузоотправители.Индекс(НоваяСтрока) + 1;
...
&НаСервере
Функция ПолучитьИдентификаторГрузополучателя(ДанныеВыгрузки, СтрокаСведений, АдресКонтрагента)
	
	Результат = 0;
	
	Грузополучатели = ДанныеВыгрузки.Контрагенты;
	
	ИНН = СокрЛП(СтрокаСведений.П000010000311);
	КПП = СокрЛП(СтрокаСведений.П000010000312);
	
	ЭтоРоссийскийКонтрагент = НЕ (АдресКонтрагента <> Неопределено И ЗначениеЗаполнено(АдресКонтрагента.Страна));
	
	УсловияОтбора = Новый Структура;
	
	УсловияОтбора.Вставить("Наименование", СтрокаСведений.П000010000310);
	...
	Если ДанныеЗаполнены Тогда
		...
		НайденныеСтроки = Грузополучатели.НайтиСтроки(УсловияОтбора);
		...
		Иначе
			...
			Идентификатор = Грузополучатели.Индекс(НоваяСтрока) + 1;
...
Процедура ЗаполнитьСправочникиДекларации(Узел_Справочники, ДанныеВыгрузки)
	...
	Для Каждого СтрокаКонтрагента Из ДанныеВыгрузки.Контрагенты Цикл
		...
		УстановитьЗначениеЭлемента(Узел_Контрагенты, "Наименование", СтрокаКонтрагента.Наименование);
		
		КодСтраны = СтрокаКонтрагента.КодСтраны;
		ЭтоКонтрагентРФ = НЕ ЗначениеЗаполнено(КодСтраны);
		
		Узел_КонтрагентРФ = ПолучитьПодчиненныйЭлемент(Узел_Контрагенты, "КонтрагентРФ");
		Узел_КонтрагентИностранный = ПолучитьПодчиненныйЭлемент(Узел_Контрагенты, "КонтрагентИностранный");
		
		Если ЭтоКонтрагентРФ Тогда
			УстановитьЗначениеЭлемента(Узел_КонтрагентРФ, "ИНН", СтрокаКонтрагента.ИНН);
			УстановитьЗначениеЭлемента(Узел_КонтрагентРФ, "КПП", СтрокаКонтрагента.КПП);
			
			Узел_АдресКонтрагента = ПолучитьПодчиненныйЭлемент(Узел_КонтрагентРФ, "АдресКонтрагента");
			УстановитьЗначениеЭлемента(Узел_АдресКонтрагента, "КодСтраны",  643);
			УстановитьЗначениеЭлемента(Узел_АдресКонтрагента, "Индекс",     СтрокаКонтрагента.Индекс);
			УстановитьЗначениеЭлемента(Узел_АдресКонтрагента, "КодРегион",  СтрокаКонтрагента.КодРегион);
			УстановитьЗначениеЭлемента(Узел_АдресКонтрагента, "Район",      СтрокаКонтрагента.Район);
			УстановитьЗначениеЭлемента(Узел_АдресКонтрагента, "Город",      СтрокаКонтрагента.Город);
			УстановитьЗначениеЭлемента(Узел_АдресКонтрагента, "НаселПункт", СтрокаКонтрагента.НаселПункт);
			УстановитьЗначениеЭлемента(Узел_АдресКонтрагента, "Улица",      СтрокаКонтрагента.Улица);
			УстановитьЗначениеЭлемента(Узел_АдресКонтрагента, "Дом",        СтрокаКонтрагента.Дом);
			УстановитьЗначениеЭлемента(Узел_АдресКонтрагента, "Корпус",     СтрокаКонтрагента.Корпус);
			УстановитьЗначениеЭлемента(Узел_АдресКонтрагента, "Литера",     СтрокаКонтрагента.Литера);
			УстановитьЗначениеЭлемента(Узел_АдресКонтрагента, "Кварт",      СтрокаКонтрагента.Кварт);
			
			РегламентированнаяОтчетность.УдалитьУзел(Узел_КонтрагентИностранный);
		Иначе
			РегламентированнаяОтчетность.УдалитьУзел(Узел_КонтрагентРФ);
			
			Если Число(Лев(СтруктураРеквизитовФормы.мВерсияФормата, 3)) >= 431 Тогда
				УстановитьЗначениеЭлемента(Узел_КонтрагентИностранный, "Номер", СтрокаКонтрагента.ИНН);
			КонецЕсли;
			
			Узел_АдресКонтрагента = ПолучитьПодчиненныйЭлемент(Узел_КонтрагентИностранный, "АдресКонтрагента");
			УстановитьЗначениеЭлемента(Узел_АдресКонтрагента, "КодСтраны", КодСтраны);
			УстановитьЗначениеЭлемента(Узел_АдресКонтрагента, "Адрес",     СтрокаКонтрагента.Адрес);
			
			Если Число(Лев(СтруктураРеквизитовФормы.мВерсияФормата, 3)) < 431 Тогда
				Если КодСтраны = "112" Тогда
					Узел_Беларусь = ПолучитьПодчиненныйЭлемент(Узел_КонтрагентИностранный, "Беларусь");
					УстановитьЗначениеЭлемента(Узел_Беларусь, "УчНомер", СтрокаКонтрагента.ИНН);
				ИначеЕсли КодСтраны = "398" Тогда
					Узел_Казахстан = ПолучитьПодчиненныйЭлемент(Узел_КонтрагентИностранный, "Казахстан");
					УстановитьЗначениеЭлемента(Узел_Казахстан, "РегНомер", СтрокаКонтрагента.ИНН);
				КонецЕсли;
			КонецЕсли; 				
	...
	Для Каждого СтрокаТранспортногоСредства Из ДанныеВыгрузки.ТранспортныеСредства Цикл
		...
		УстановитьЗначениеЭлемента(Узел_ТранспортныеСредства, "П000000000008", СтрокаТранспортногоСредства.П000000000008);
		УстановитьЗначениеЭлемента(Узел_ТранспортныеСредства, "П000000000009", СтрокаТранспортногоСредства.П000000000009);
		УстановитьЗначениеЭлемента(Узел_ТранспортныеСредства, "П000000000010", СтрокаТранспортногоСредства.П000000000010);
	КонецЦикла;
