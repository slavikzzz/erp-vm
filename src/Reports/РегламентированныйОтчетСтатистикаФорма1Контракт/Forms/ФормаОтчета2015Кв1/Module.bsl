...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0608031";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 4) = "П010" Тогда 
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 4 По 16 Цикл
		...
		Области["П02122" + ГрСтр].Значение = Области["П02123" + ГрСтр].Значение + Области["П02124" + ГрСтр].Значение
											+ Области["П02125" + ГрСтр].Значение + Области["П02126" + ГрСтр].Значение;
	КонецЦикла;
	
	Для Гр = 4 По 14 Цикл
		ГрСтр = Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=");
		Области["П02211" + ГрСтр].Значение = Области["П02212" + ГрСтр].Значение + Области["П02213" + ГрСтр].Значение
											+ Области["П02214" + ГрСтр].Значение;
	КонецЦикла;
	
	Для Гр = 4 По 16 Цикл
		ГрСтр = Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=");
		Области["П02322" + ГрСтр].Значение = Области["П02323" + ГрСтр].Значение + Области["П02324" + ГрСтр].Значение
											+ Области["П02325" + ГрСтр].Значение + Области["П02326" + ГрСтр].Значение;
	КонецЦикла;
	
	Для Разд = 2 По 4 Цикл 
		Для Часть = 1 По 3 Цикл 
			Для Стр = 1 По 40 Цикл
				ПрефиксОбласти = "П0"+Разд+Часть+Формат(Стр, "ЧЦ=2; ЧВН=; ЧГ=");
				ИтоговаяОбласть = Области.Найти(ПрефиксОбласти + "03");
				Если ИтоговаяОбласть = Неопределено Или Не ИтоговаяОбласть.Защита Тогда 
					Продолжить;
				КонецЕсли;
				
				Сумма = 0;
				Для Гр = 4 По 16 Цикл
					Обл = Области.Найти(ПрефиксОбласти + Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ="));
					Если Обл <> Неопределено Тогда 
						Сумма = Сумма + Обл.Значение;
					КонецЕсли;
				КонецЦикла;
				
				ИтоговаяОбласть.Значение = Сумма;
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Инд = 1 По 26 Цикл 
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-контракт");
	КонецЦикла;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Для Инд = 1 По 8 Цикл
		ПараметрыВыгрузки.Вставить("ПР010"+Инд, ?(ТабДокумент["П010"+Инд], "1", "0"));
	КонецЦикла;
	
	ПараметрыВыгрузки.Вставить("ПРЗ", ТабДокумент.ПРЗ);
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 12.11.2014 № 654";
	
	ТекстФормулы = "В разделе 1 хотя бы одна из граф 1-3 должна быть отмечена";
	Описание     = "В разделе 1 хотя бы одна из граф 1-3 должна быть отмечена";
	Расшифровка  = "";
	Если Показатели.П0101 Или Показатели.П0102 Или Показатели.П0103 Тогда
		ЭтоОшибка = Ложь;
	...
	СоответствиеБР["1"] = "П02101П02102";
	СоответствиеБР["2"] = "П02101П02103";
	СоответствиеБР["3"] = "П02103П02104";
	СоответствиеБР["4"] = "П02101П02105";
	СоответствиеБР["5"] = "П02101П02106";
	СоответствиеБР["6"] = "П02101П02107";
	СоответствиеБР["7"] = "П02107П02108";
	СоответствиеБР["8"] = "П02107П02109";
	СоответствиеБР["9"] = "П02110П02111";
	СоответствиеБР["10"] = "П02110П02112";
	СоответствиеБР["11"] = "П02110П02113";
	СоответствиеБР["12"] = "П02110П02114";
	СоответствиеБР["13"] = "П02110П02115";
	СоответствиеБР["14"] = "П02110П02116";
	СоответствиеБР["15"] = "П02110П02119";
	СоответствиеБР["16"] = "П02110П02120";
	СоответствиеБР["17"] = "П02116П02117";
	СоответствиеБР["18"] = "П02116П02118";
	СоответствиеБР["19"] = "П02122П02127";
	СоответствиеБР["20"] = "П02201П02202";
	СоответствиеБР["21"] = "П02201П02203";
	СоответствиеБР["22"] = "П02201П02204";
	СоответствиеБР["23"] = "П02201П02205";
	СоответствиеБР["24"] = "П02201П02206";
	СоответствиеБР["25"] = "П02201П02208";
	СоответствиеБР["26"] = "П02206П02207";
	СоответствиеБР["27"] = "П02208П02209";
	СоответствиеБР["28"] = "П02208П02210";
	СоответствиеБР["29"] = "П02201П02211";
	СоответствиеБР["30"] = "П02201П02215";
	СоответствиеБР["31"] = "П02201П02216";
	СоответствиеБР["32"] = "П02205П02217";
	СоответствиеБР["33"] = "П02216П02217";
	СоответствиеБР["34"] = "П02216П02218";
	СоответствиеБР["35"] = "П02216П02219";
	СоответствиеБР["36"] = "П02208П02219";
	СоответствиеБР["37"] = "П02219П02220";
	СоответствиеБР["38"] = "П02219П02221";
	СоответствиеБР["39"] = "П02301П02303";
	СоответствиеБР["40"] = "П02303П02304";
	СоответствиеБР["41"] = "П02301П02305";
	СоответствиеБР["42"] = "П02301П02306";
	СоответствиеБР["43"] = "П02301П02307";
	СоответствиеБР["44"] = "П02307П02308";
	СоответствиеБР["45"] = "П02309П02310";
	СоответствиеБР["46"] = "П02303П02310";
	СоответствиеБР["47"] = "П02309П02311";
	СоответствиеБР["48"] = "П02309П02312";
	СоответствиеБР["49"] = "П02309П02313";
	СоответствиеБР["50"] = "П02309П02315";
	СоответствиеБР["51"] = "П02313П02314";
	СоответствиеБР["52"] = "П02309П02316";
	СоответствиеБР["53"] = "П02316П02317";
	СоответствиеБР["54"] = "П02316П02318";
	СоответствиеБР["55"] = "П02309П02319";
	СоответствиеБР["56"] = "П02309П02320";
	СоответствиеБР["57"] = "П02322П02327";
	
	Для Каждого КЗ Из СоответствиеБР Цикл 
		Для Гр = 3 По 16 Цикл 
			ГрСтр = Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=");
			ЛевЧасть = Лев(Кз.Значение, 6) + ГрСтр;
			ПравЧасть = Прав(Кз.Значение, 6) + ГрСтр;
			
			Если Показатели.Свойство(ЛевЧасть) И Показатели.Свойство(ПравЧасть) Тогда 
				ТекстФормулы = "В разделе 2 по гр." + ГрСтр + " стр." + Сред(ЛевЧасть, 4, 3) + " >= стр." + Сред(ПравЧасть, 4, 3);
				Описание     = "В разделе 2 по гр." + ГрСтр + " стр." + Сред(ЛевЧасть, 4, 3) + " < стр." + Сред(ПравЧасть, 4, 3);
				ЛЧ           = Показатели[ЛевЧасть];
				ПЧ           = Показатели[ПравЧасть];
