		...
		Для каждого Ветка Из ДеревоРаздела.Строки Цикл
			...
			Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
				...
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					...
					СтатусПроверки = Ложь;
				ИначеЕсли ЗначПок <> 9.09 И ЗначПок <> 15.25 И ЗначПок <> 16.67 Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
					+ "расчетная налоговая ставка должна принимать одно из значений: 9,09; 15,25; 16,67",
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "НДС Услуги в электронной форме 2024");
...
Процедура ПолучитьДанныеСтраницы(СтруктураПриемник, 
	...
	Если НЕ СтруктураИсточник.Строки.Количество() = 0 Тогда
		Для каждого СтраницаРаздела Из СтруктураИсточник.Строки Цикл
			
			ИндексПриложения = СтруктураИсточник.Строки.Индекс(СтраницаРаздела);
			КлючПриложения = мСтруктураМногоуровневыхРазделов.Лист02.ИменаПодчиненныхРазделов[ИндексПриложения];
			СтруктураПриемник.Вставить("КолвоСтраниц" + КлючПриложения, СтраницаРаздела.Строки.Количество());
			МассивСтраниц = Новый Массив;
			
			Для каждого СтраницаПриложения Из СтраницаРаздела.Строки Цикл
		
				ДанныеСтраницы = Новый Структура;
				ДанныеСтраницы.Вставить("Страница", СтруктураПриемник.Страница + "," + (ИндексПриложения + 1) + "," +
					XMLСтрока(СтраницаРаздела.Строки.Индекс(СтраницаПриложения) + 1));
...
&НаСервере
Процедура ПроверкаКС_2_1_к()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 1';
														|en = 'Раздел 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '2.1.к';
													|en = '2.1.к'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма налога в стр.130 Раздела 1 >= 0';
													|en = 'Сумма налога в стр.130 Раздела 1 >= 0'"));
	...
	Заголовки.Вставить("Рекомендации"		, НСтр("ru = 'Укажите сумму налога в стр.130 Раздела 1 >= 0';
													|en = 'Укажите сумму налога в стр.130 Раздела 1 >= 0'"));
	...
	ИменаПоказателей.Вставить("Р01С001"	, "П000010013003");
