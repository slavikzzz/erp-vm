...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "01/10/2024";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("мИдГруппыР4М2");
	СтруктураРеквизитовФормы.Вставить("мИдГруппыР5М1");
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.НДС иностр.орг.25к1",
		'20250224') Тогда
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	СтруктураРеквизитовФормы.мИдГруппыР4М2 = "П00004М2";
	СтруктураРеквизитовФормы.мИдГруппыР5М1 = "П00005М1";
	...
	// Группа П00004М1.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П00004М145003");
	ИменаГраф.Добавить("П00004М146003");
	ИменаГраф.Добавить("П00004М147003");
	ИменаГраф.Добавить("П00004М148003");
	ИменаГраф.Добавить("П00004М149003");
	ИменаПодчиненныхГрупп = Новый Массив;
	ИменаПодчиненныхГрупп.Добавить(СтруктураРеквизитовФормы.мИдГруппыР4М2);
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппыР4М1, Новый Структура(
	"ИменаГраф,ИсхКолСтрок,УровеньПодчинения,ИменаПодчиненныхГрупп", ИменаГраф, 1, 0, ИменаПодчиненныхГрупп));
	
	// Группа П00004М2.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П00004М241003");
	ИменаГраф.Добавить("П00004М242003");
	ИменаГраф.Добавить("П00004М243003");
	ИменаГраф.Добавить("П00004М244003");
	ИменаПодчиненныхГрупп = Новый Массив;
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппыР4М2, Новый Структура(
	"ИменаГраф,ИсхКолСтрок,УровеньПодчинения,ИменаПодчиненныхГрупп", ИменаГраф, 1, 1, ИменаПодчиненныхГрупп));
	
	// Группа П00005М1.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П00005М152003");
	ИменаГраф.Добавить("П00005М153003");
	ИменаГраф.Добавить("П00005М154003");
	ИменаГраф.Добавить("П00005М155003");
	ИменаПодчиненныхГрупп = Новый Массив;
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппыР5М1, Новый Структура(
	"ИменаГраф,ИсхКолСтрок,УровеньПодчинения,ИменаПодчиненныхГрупп", ИменаГраф, 1, 0, ИменаПодчиненныхГрупп));
...
Процедура ИнициализироватьМногоуровневыеРазделы()
	...
	СтруктураРаздела.Вставить("Многостраничность");
	ИменаГруппМнЧ = Новый Массив;
	ИменаГруппМнЧ.Добавить(СтруктураРеквизитовФормы.мИдГруппыР5М1);
	СтруктураРаздела.Вставить("ИменаГруппМнЧ", ИменаГруппМнЧ);
...
Процедура СформироватьСпискиВыбора()
	...
	"Выбор признака иностранного налогоплательщика", КоллекцияСписковВыбора["ПризнакиИнНпл"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П00002М120503", 3, ,
	"Выбор кода вида оказанной услуги", КоллекцияСписковВыбора["КодыВидаУслуги"]);
	...
	//Раздел 4.
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000040040003", 1, ,
	"Выбор признака иностранного налогоплательщика", КоллекцияСписковВыбора["ПризнакиИнПрод"]);
	
	//Раздел 5.
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000050051003", 1, ,
	"Выбор признака иностранного налогоплательщика", КоллекцияСписковВыбора["ПризнакиИнПрод"]);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00002М222003") ИЛИ СтрНачинаетсяС(Область.Имя, "П00003М232003")
		ИЛИ СтрНачинаетсяС(Область.Имя, "П00004М242003") ИЛИ СтрНачинаетсяС(Область.Имя, "П00005М153003") Тогда
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00004М146003") И НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		
		СтандартнаяОбработка = Ложь;
		
		СписокСтавок = Новый СписокЗначений;
		СписокСтавок.Добавить(0,     "Пустое значение");
		СписокСтавок.Добавить(9.09,  "9,09");
		СписокСтавок.Добавить(16.67, "16,67");
...
Процедура ВыбратьЧислоИзСпискаЗавершение(ВыбЭлемент, ДополнительныйПараметр) Экспорт
	...
	Если ВыбЭлемент <> Неопределено Тогда
		Если ВыбЭлемент.Значение <> ДополнительныйПараметр.Значение Тогда
			...
			Если СтрНачинаетсяС(ДополнительныйПараметр.Имя, "П00002М125003")
				ИЛИ СтрНачинаетсяС(ДополнительныйПараметр.Имя, "П00004М146003") Тогда
		...
		Если ПредКодВалюты = "643" Тогда
			...
			ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00003М232003") Тогда
				...
				Если Обл <> Неопределено Тогда
					Обл.Очистить();
				КонецЕсли;
			ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00004М242003") Тогда
				Обл = ТабличныйДокумент.Области.Найти("П00004М243003_" + Постфикс);
				Если Обл <> Неопределено Тогда
					Обл.Очистить();
				КонецЕсли;
				Обл = ТабличныйДокумент.Области.Найти("П00004М244003_" + Постфикс);
				Если Обл <> Неопределено Тогда
					Обл.Очистить();
				КонецЕсли;
			ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00005М153003") Тогда
				Обл = ТабличныйДокумент.Области.Найти("П00005М154003_" + Постфикс);
				Если Обл <> Неопределено Тогда
					Обл.Очистить();
				КонецЕсли;
				Обл = ТабличныйДокумент.Области.Найти("П00005М155003_" + Постфикс);
				Если Обл <> Неопределено Тогда
					Обл.Очистить();
				КонецЕсли;
	...
	Для каждого ИмяПок Из АвтоРасчПоказатели Цикл
		Если СтрДлина(ИмяПок) = Форма.СтруктураРеквизитовФормы.ДлинаИмениГрафы И Сред(ИмяПок, 7, 1) = "М" Тогда
			...
			Пока Обл <> Неопределено Цикл
				...
				ИначеЕсли ИмяПок = "П00004М145003" Тогда
					
					// Особая обработка в строках 430, 440 раздела 4.
					РегламентированнаяОтчетностьКлиентСервер.УстановитьИндекс(ИндексПодч, ИндексПодчСтр);
					Обл420 = Форма.ТабличныйДокумент.Области.Найти("П00004М242003_" + ИндексСтр + "_" + ИндексПодчСтр);
					Пока Обл420 <> Неопределено Цикл
						Обл430 = Форма.ТабличныйДокумент.Области.Найти("П00004М243003_" + ИндексСтр + "_" + ИндексПодчСтр);
						Обл440 = Форма.ТабличныйДокумент.Области.Найти("П00004М244003_" + ИндексСтр + "_" + ИндексПодчСтр);
						Если НЕ Форма.СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет И СокрЛП(Обл420.Значение) = "643" Тогда
							Обл430.Защита = Истина;
							Обл430.ЦветФона = Форма.мСтруктураЦветовРаскраски["_4"];
							Обл440.Защита = Истина;
							Обл440.ЦветФона = Форма.мСтруктураЦветовРаскраски["_4"];
						Иначе
							Обл430.Защита = Ложь;
							Обл430.ЦветФона = Форма.мСтруктураЦветовРаскраски["_0"];
							Обл440.Защита = Ложь;
							Обл440.ЦветФона = Форма.мСтруктураЦветовРаскраски["_0"];
						КонецЕсли;
						РегламентированнаяОтчетностьКлиентСервер.УвеличитьИндекс(ИндексПодч, ИндексПодчСтр);
						Обл420 = Форма.ТабличныйДокумент.Области.Найти("П00004М242003_" + ИндексСтр + "_" + ИндексПодчСтр);
					КонецЦикла;
		...
		Иначе
			...
			Если ИмяРаздела = "Раздел5" Тогда
				...
				// Особая обработка в строках 540, 550 раздела 5.
				РегламентированнаяОтчетностьКлиентСервер.УстановитьИндекс(Индекс, ИндексСтр);
				Обл = Форма.ТабличныйДокумент.Области.Найти("П00005М153003_" + ИндексСтр);
				Пока Обл <> Неопределено Цикл
					
					Обл540 = Форма.ТабличныйДокумент.Области.Найти("П00005М154003_" + ИндексСтр);
					Обл550 = Форма.ТабличныйДокумент.Области.Найти("П00005М155003_" + ИндексСтр);
					Если НЕ Форма.СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет И СокрЛП(Обл.Значение) = "643" Тогда
						Обл540.Защита = Истина;
						Обл540.ЦветФона = Форма.мСтруктураЦветовРаскраски["_4"];
						Обл550.Защита = Истина;
						Обл550.ЦветФона = Форма.мСтруктураЦветовРаскраски["_4"];
					Иначе
						Обл540.Защита = Ложь;
						Обл540.ЦветФона = Форма.мСтруктураЦветовРаскраски["_0"];
						Обл550.Защита = Ложь;
						Обл550.ЦветФона = Форма.мСтруктураЦветовРаскраски["_0"];
					КонецЕсли;
...
Процедура ДобавитьСтраницу(Команда)
	...
	Если ИмяРаздела = "Раздел2" ИЛИ ИмяРаздела = "Раздел3"
		ИЛИ ИмяРаздела = "Раздел4" ИЛИ ИмяРаздела = "Раздел5" Тогда
...
Процедура ДобавитьСтраницуУровня0(ИмяРаздела, ТекстПредупреждения = Неопределено)
	...
	Если ДеревоРаздела.Строки.Количество() >= 2 Тогда
		...
		"ru = 'В разделе " + Прав(ИмяРаздела, 1) + " не допускается более двух страниц'");
		Возврат;
	КонецЕсли;
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Пока Обл245 <> Неопределено Цикл
					...
					Если Обл250 <> Неопределено И Обл260 <> Неопределено Тогда
						...
						Сумма270 = Сумма270 + Обл260.Значение;
					КонецЕсли;
				...
				Обл270 = Форма.ТабличныйДокумент.Области.Найти("П000020027003");
				Если Обл270 <> Неопределено Тогда
					Обл270.Значение = Сумма270;
				КонецЕсли;
	...
	ИначеЕсли ИмяРаздела = "Раздел4" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Сумма500 = 0;
				РегламентированнаяОтчетностьКлиентСервер.УстановитьИндекс(Индекс, ИндексСтр);
				Обл450 = Форма.ТабличныйДокумент.Области.Найти("П00004М145003_" + ИндексСтр);
				Пока Обл450 <> Неопределено Цикл
					
					Сумма450 = 0;
					РегламентированнаяОтчетностьКлиентСервер.УстановитьИндекс(ИндексПодч, ИндексПодчСтр);
					Обл440 = Форма.ТабличныйДокумент.Области.Найти("П00004М244003_" + ИндексСтр + "_" + ИндексПодчСтр);
					Пока Обл440 <> Неопределено Цикл
						Обл420 = Форма.ТабличныйДокумент.Области.Найти("П00004М242003_" + ИндексСтр + "_" + ИндексПодчСтр);
						Если Обл420 <> Неопределено И СокрЛП(Обл420.Значение) = "643" Тогда
							Обл430 = Форма.ТабличныйДокумент.Области.Найти("П00004М243003_" + ИндексСтр + "_" + ИндексПодчСтр);
							Если Обл430 <> Неопределено Тогда
								Обл430.Значение = 1;
							КонецЕсли;
							Обл410 = Форма.ТабличныйДокумент.Области.Найти("П00004М241003_" + ИндексСтр + "_" + ИндексПодчСтр);
							Если Обл410 <> Неопределено Тогда
								Обл440.Значение = Обл410.Значение;
							КонецЕсли;
						КонецЕсли;
						Сумма450 = Сумма450 + Обл440.Значение;
						РегламентированнаяОтчетностьКлиентСервер.УвеличитьИндекс(ИндексПодч, ИндексПодчСтр);
						Обл440 = Форма.ТабличныйДокумент.Области.Найти("П00004М244003_" + ИндексСтр + "_" + ИндексПодчСтр);
					КонецЦикла;
					Обл450.Значение = Сумма450;
					
					Обл460 = Форма.ТабличныйДокумент.Области.Найти("П00004М146003_" + ИндексСтр);
					Обл470 = Форма.ТабличныйДокумент.Области.Найти("П00004М147003_" + ИндексСтр);
					Если Обл460 <> Неопределено И Обл470 <> Неопределено Тогда
						Обл470.Значение = Окр(Сумма450 * Обл460.Значение / 100);
					КонецЕсли;
					
					Обл480 = Форма.ТабличныйДокумент.Области.Найти("П00004М148003_" + ИндексСтр);
					Обл490 = Форма.ТабличныйДокумент.Области.Найти("П00004М149003_" + ИндексСтр);
					Если Обл480 <> Неопределено И Обл490 <> Неопределено Тогда
						Обл490.Значение = Окр(Обл470.Значение - Обл480.Значение);
						Сумма500 = Сумма500 + Обл490.Значение;
					КонецЕсли;
				...
				Обл500 = Форма.ТабличныйДокумент.Области.Найти("П000040050003");
				Если Обл500 <> Неопределено Тогда
					Обл500.Значение = Сумма500;
				КонецЕсли;
	...
	ИначеЕсли ИмяРаздела = "Раздел5" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				РегламентированнаяОтчетностьКлиентСервер.УстановитьИндекс(Индекс, ИндексСтр);
				Обл550 = Форма.ТабличныйДокумент.Области.Найти("П00005М155003_" + ИндексСтр);
				Пока Обл550 <> Неопределено Цикл
					
					Обл530 = Форма.ТабличныйДокумент.Области.Найти("П00005М153003_" + ИндексСтр);
					Если Обл530 <> Неопределено И СокрЛП(Обл530.Значение) = "643" Тогда
						Обл540 = Форма.ТабличныйДокумент.Области.Найти("П00005М154003_" + ИндексСтр);
						Если Обл540 <> Неопределено Тогда
							Обл540.Значение = 1;
						КонецЕсли;
						Обл520 = Форма.ТабличныйДокумент.Области.Найти("П00005М152003_" + ИндексСтр);
						Если Обл520 <> Неопределено Тогда
							Обл550.Значение = Обл520.Значение;
						КонецЕсли;
					КонецЕсли;
					
					Сумма560 = Сумма560 + Обл550.Значение;
				...
				Обл560 = Форма.ТабличныйДокумент.Области.Найти("П000050056003");
				Если Обл560 <> Неопределено Тогда
					Обл560.Значение = Сумма560;
				КонецЕсли;
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел1" Тогда
		...
		ДеревоР = ПолучитьИзВременногоХранилища(
		Форма.СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхРаздел2"]);
		Для каждого ВеткаР Из ДеревоР.Строки Цикл
			Сумма120 = Сумма120 + ВеткаР.Данные["П000020027003"];
		КонецЦикла;
		
		ДеревоР = ПолучитьИзВременногоХранилища(
		Форма.СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхРаздел4"]);
		Для каждого ВеткаР Из ДеревоР.Строки Цикл
			Сумма120 = Сумма120 + ВеткаР.Данные["П000040050003"];
		КонецЦикла;
		
		ДеревоДанных = ПолучитьИзВременногоХранилища(
		Форма.СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанных" + ИмяГлРаздела]);
		
		ДеревоДанных.Строки[0].Данные.Вставить("П000010012003", Сумма120);
	...
	ИначеЕсли ИмяРаздела = "Раздел2" Тогда
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
				...
				Сумма270 = Сумма270 + СтрокаМнЧ.Данные["П00002М126003"];
				
			КонецЦикла;
			
			Ветка.Данные.Вставить("П000020027003", Сумма270);
	...
	ИначеЕсли ИмяРаздела = "Раздел4" Тогда
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			ДеревоМнЧ = Ветка.ДанныеМногострочныхЧастей[Форма.СтруктураРеквизитовФормы.мИдГруппыР4М1];
			Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
				
				Сумма450 = 0;
				
				ДеревоПодчМнЧ = СтрокаМнЧ.ДанныеМногострочныхЧастей[Форма.СтруктураРеквизитовФормы.мИдГруппыР4М2];
				Для каждого СтрокаПодчМнЧ Из ДеревоПодчМнЧ.Строки Цикл
					Если СокрЛП(СтрокаПодчМнЧ.Данные["П00004М242003"]) = "643" Тогда
						СтрокаПодчМнЧ.Данные.Вставить("П00004М243003", 1);
						СтрокаПодчМнЧ.Данные.Вставить("П00004М244003", СтрокаПодчМнЧ.Данные["П00004М241003"]);
					КонецЕсли;
					Сумма450 = Сумма450 + СтрокаПодчМнЧ.Данные["П00004М244003"];
				КонецЦикла;
				
				СтрокаМнЧ.Данные.Вставить("П00004М145003", Сумма450);
				
				СтрокаМнЧ.Данные.Вставить("П00004М147003", Окр(Сумма450 * СтрокаМнЧ.Данные["П00004М146003"] / 100));
				
				СтрокаМнЧ.Данные.Вставить("П00004М149003", Окр(СтрокаМнЧ.Данные["П00004М147003"] - СтрокаМнЧ.Данные["П00004М148003"]));
				
				Сумма500 = Сумма500 + СтрокаМнЧ.Данные["П00004М149003"];
				
			КонецЦикла;
			
			Ветка.Данные.Вставить("П000040050003", Сумма500);
	...
	ИначеЕсли ИмяРаздела = "Раздел5" Тогда
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			ДеревоМнЧ = Ветка.ДанныеМногострочныхЧастей[Форма.СтруктураРеквизитовФормы.мИдГруппыР5М1];
			Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
				Если СокрЛП(СтрокаМнЧ.Данные["П00005М153003"]) = "643" Тогда
					СтрокаМнЧ.Данные.Вставить("П00005М154003", 1);
					СтрокаМнЧ.Данные.Вставить("П00005М155003", СтрокаМнЧ.Данные["П00005М152003"]);
				КонецЕсли;
				Сумма560 = Сумма560 + СтрокаМнЧ.Данные["П00005М155003"];
			КонецЦикла;
			
			Ветка.Данные.Вставить("П000050056003", Сумма560);
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ.Вставить("П000050056003");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Раздел5", Структ);
	
	Структ = Новый Структура;
	Структ.Вставить("П00004М145003");
	Структ.Вставить("П00004М147003");
	Структ.Вставить("П00004М149003");
	Структ.Вставить("П000040050003");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПараметрыВыгрузки.ЭтоИП Тогда
		Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ИННОрг) Тогда
			СообщитьВТС("Не задан ИНН иностранного налогоплательщика", "Титульный", "ИНН");
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(ПараметрыВыгрузки.ИННОрг) <> 12 Тогда
			СообщитьВТС("Задан ИНН иностранного налогоплательщика, не состоящий из 12 цифр", "Титульный", "ИНН");
			СтатусПроверки = Ложь;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.Фамилия) Тогда
			СообщитьВТС("Не задана фамилия иностранного налогоплательщика", "Титульный", "НаимОрг");
			СтатусПроверки = Ложь;
		ИначеЕсли НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.Имя) Тогда
			СообщитьВТС("Не задано имя иностранного налогоплательщика", "Титульный", "НаимОрг");
			СтатусПроверки = Ложь;
		КонецЕсли;
	Иначе
		Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ИННОрг) Тогда
		...
		ИначеЕсли СтрДлина(ПараметрыВыгрузки.ИННОрг) <> 10 Тогда
			СообщитьВТС("Задан ИНН иностранного налогоплательщика, не состоящий из 10 цифр", "Титульный", "ИНН");
			СтатусПроверки = Ложь;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.КПП) Тогда
			СообщитьВТС("Не задан КПП иностранного налогоплательщика", "Титульный", "КПП");
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(ПараметрыВыгрузки.КПП) <> 9 Тогда
			СообщитьВТС("Задан КПП иностранного налогоплательщика, не состоящий из 9 символов", "Титульный", "КПП");
			СтатусПроверки = Ложь;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.НаимОрг) Тогда
			СообщитьВТС("Не задано наименование иностранного налогоплательщика", "Титульный", "НаимОрг");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыВыгрузки.ПрПодп = "2" И НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.НаимДокПред) Тогда
		СообщитьВТС(
		"Не задано наименование документа, подтверждающего полномочия представителя иностранного налогоплательщика",
		"Титульный", "ДокУпПред");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ЗаполненРаздел2 Тогда
		...
		ВозмПризнакиИнНпл  = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000020020003");
		...
		Для каждого Ветка Из ДеревоРаздела.Строки Цикл
			...
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ": не задан признак иностранного налогоплательщика",
				"Раздел2", "П000020020003", НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ВозмПризнакиИнНпл.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
				СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы
				+ ": признак иностранного налогоплательщика должен принимать одно из значений: "
				+ ВозмПризнакиИнНпл.СтрокаЗначений, "Раздел2", "П000020020003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
				...
				ИначеЕсли ЗначПок <> 15.25 И ЗначПок <> 16.67 Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
					+ "расчетная налоговая ставка должна принимать одно из значений: 15,25; 16,67",
	...
	Если ЗаполненРаздел3 Тогда
		...
		ВозмПризнакиИнНпл  = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000030030003");
		...
		Для каждого Ветка Из ДеревоРаздела.Строки Цикл
			...
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": не задан признак иностранного налогоплательщика",
				"Раздел3", "П000030030003", НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ВозмПризнакиИнНпл.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы
				+ ": признак иностранного налогоплательщика должен принимать одно из значений: "
				+ ВозмПризнакиИнНпл.СтрокаЗначений, "Раздел3", "П000030030003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
	...
	ЗаполненРаздел4 = ЗаполненРаздел(ДеревоРаздела, "Раздел4");
	
	Если ЗаполненРаздел4 Тогда
		
		ВозмПризнакиИнПрод = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000040040003");
		
		Для каждого Ветка Из ДеревоРаздела.Строки Цикл
			
			НомСтраницы = Формат(ДеревоРаздела.Строки.Индекс(Ветка) + 1, "ЧГ=");
			
			ЗначПок = СокрЛП(Ветка.Данные["П000040040003"]);
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("Раздел 4 -> Страница " + НомСтраницы + ": не задан признак иностранного налогоплательщика",
				"Раздел4", "П000040040003", НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ВозмПризнакиИнПрод.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
				СообщитьВТС("Раздел 4 -> Страница " + НомСтраницы
				+ ": признак иностранного налогоплательщика должен принимать одно из значений: "
				+ ВозмПризнакиИнПрод.СтрокаЗначений, "Раздел4", "П000040040003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
				...
				ИндСтрокиМнЧ = ДанныеМнЧ.Строки.Индекс(СтрокаМнЧ);
				НомСтроки = Формат(ИндСтрокиМнЧ + 1, "ЧГ=");
				Текст_СтрокаГруппыСтрок = ?(КолСтрокМнЧ = 1, "", "в строке " + НомСтроки + " группы строк 410-490 ");
				
				ЗначПок = СтрокаМнЧ.Данные["П00004М146003"];
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Раздел 4 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
					+ "не задана расчетная налоговая ставка", "Раздел4", "П00004М146003_" + НомСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли ЗначПок <> 9.09 И ЗначПок <> 16.67 Тогда
					СообщитьВТС("Раздел 4 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
					+ "расчетная налоговая ставка должна принимать одно из значений: 9,09; 16,67",
					"Раздел4", "П00004М146003_" + НомСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ДанныеПодчМнЧ = СтрокаМнЧ.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР4М2];
				КолСтрокПодчМнЧ = ДанныеПодчМнЧ.Строки.Количество();
				Для каждого СтрокаПодчМнЧ Из ДанныеПодчМнЧ.Строки Цикл
					
					ИндСтрокиПодчМнЧ = ДанныеПодчМнЧ.Строки.Индекс(СтрокаПодчМнЧ);
					НомПодчСтроки = Формат(ИндСтрокиПодчМнЧ + 1, "ЧГ=");
					Текст_СтрокаГруппыПодчСтрок = ?(КолСтрокПодчМнЧ = 1, "", "в строке " + НомПодчСтроки + " группы строк 410-440 ");
					
					ЗначПок = СокрЛП(СтрокаПодчМнЧ.Данные["П00004М242003"]);
					Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
						СообщитьВТС("Раздел 4 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок + "не задан код валюты "
						+ Текст_СтрокаГруппыПодчСтрок, "Раздел4", "П00004М242003_" + НомСтроки + "_" + НомПодчСтроки, НомСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
	...
	ЗаполненРаздел5 = ЗаполненРаздел(ДеревоРаздела, "Раздел5");
	
	Если ЗаполненРаздел5 Тогда
		
		ВозмПризнакиИнПрод = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000050051003");
		
		Для каждого Ветка Из ДеревоРаздела.Строки Цикл
			
			НомСтраницы = Формат(ДеревоРаздела.Строки.Индекс(Ветка) + 1, "ЧГ=");
			
			ЗначПок = СокрЛП(Ветка.Данные["П000050051003"]);
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("Раздел 5 -> Страница " + НомСтраницы + ": не задан признак иностранного налогоплательщика",
				"Раздел5", "П000050051003", НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ВозмПризнакиИнПрод.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
				СообщитьВТС("Раздел 5 -> Страница " + НомСтраницы
				+ ": признак иностранного налогоплательщика должен принимать одно из значений: "
				+ ВозмПризнакиИнПрод.СтрокаЗначений, "Раздел5", "П000050051003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ДанныеМнЧ = Ветка.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР5М1];
			КолСтрокМнЧ = ДанныеМнЧ.Строки.Количество();
			Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
				
				ИндСтрокиМнЧ = ДанныеМнЧ.Строки.Индекс(СтрокаМнЧ);
				НомСтроки = Формат(ИндСтрокиМнЧ + 1, "ЧГ=");
				Текст_СтрокаГруппыСтрок = ?(КолСтрокМнЧ = 1, "", "в строке " + НомСтроки + " группы строк 520-550 ");
				
				ЗначПок = СокрЛП(СтрокаМнЧ.Данные["П00005М153003"]);
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Раздел 5 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок + "не задан код валюты",
					"Раздел5", "П00005М153003_" + НомСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
	...
	//
	Если НЕ ЗаполненРаздел2 И НЕ ЗаполненРаздел3 И НЕ ЗаполненРаздел4 И НЕ ЗаполненРаздел5 Тогда
		
		СообщитьВТС("Должен быть заполнен хотя бы один из разделов 2-5", "Раздел2", "П000020020003", "1");
		СтатусПроверки = Ложь;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, ПараметрыВыгрузки)
	...
	// Раздел 2.
	//
	РасчНалУсл = ПолучитьПодчиненныйЭлемент(УзелРодитель, "РасчНалУсл");
	ДеревоРаздела = СохрСтруктура.ДанныеМногоуровневыхРазделов["Раздел2"];
	Для каждого Ветка Из ДеревоРаздела.Строки Цикл
		
		ЭкзРасчНалУсл = СкопироватьУзел(УзелРодитель, РасчНалУсл, УзелРодитель.Строки.Количество() - 1);
		
		Для каждого Эл Из ЭкзРасчНалУсл.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, Ветка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		РасчНалКод = ПолучитьПодчиненныйЭлемент(ЭкзРасчНалУсл, "РасчНалКод");
		ДанныеМнЧ = Ветка.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР2М1];
		Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
			
			ЭкзРасчНалКод = СкопироватьУзел(ЭкзРасчНалУсл, РасчНалКод, ЭкзРасчНалУсл.Строки.Количество() - 1);
	...
	// Раздел 4.
	//
	РасчНалТовар = ПолучитьПодчиненныйЭлемент(УзелРодитель, "РасчНалТовар");
	ДеревоРаздела = СохрСтруктура.ДанныеМногоуровневыхРазделов["Раздел4"];
	Для каждого Ветка Из ДеревоРаздела.Строки Цикл
		
		ЭкзРасчНалТовар = СкопироватьУзел(УзелРодитель, РасчНалТовар, УзелРодитель.Строки.Количество() - 1);
		
		Для каждого Эл Из ЭкзРасчНалТовар.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, Ветка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		РасчНалСтав = ПолучитьПодчиненныйЭлемент(ЭкзРасчНалТовар, "РасчНалСтав");
		ДанныеМнЧ = Ветка.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР4М1];
		Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
			
			ЭкзРасчНалСтав = СкопироватьУзел(ЭкзРасчНалТовар, РасчНалСтав, ЭкзРасчНалТовар.Строки.Количество() - 1);
			ЗаполнитьДаннымиУзел(СтрокаМнЧ.Данные, ЭкзРасчНалСтав);
			
			СтоимТоварВ = ПолучитьПодчиненныйЭлемент(ЭкзРасчНалСтав, "СтоимТоварВ");
			ДанныеПодчМнЧ = СтрокаМнЧ.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР4М2];
			Для каждого СтрокаПодчМнЧ Из ДанныеПодчМнЧ.Строки Цикл
				
				ЭкзСтоимТоварВ = СкопироватьУзел(ЭкзРасчНалСтав, СтоимТоварВ, ЭкзРасчНалСтав.Строки.Количество() - 1);
				ЗаполнитьДаннымиУзел(СтрокаПодчМнЧ.Данные, ЭкзСтоимТоварВ);
	...
	// Раздел 5.
	//
	ТоварНеНал = ПолучитьПодчиненныйЭлемент(УзелРодитель, "ТоварНеНал");
	ДеревоРаздела = СохрСтруктура.ДанныеМногоуровневыхРазделов["Раздел5"];
	Для каждого Ветка Из ДеревоРаздела.Строки Цикл
		
		ЭкзТоварНеНал = СкопироватьУзел(УзелРодитель, ТоварНеНал, УзелРодитель.Строки.Количество() - 1);
		
		Для каждого Эл Из ЭкзТоварНеНал.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, Ветка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		СтоимТоварВ = ПолучитьПодчиненныйЭлемент(ЭкзТоварНеНал, "СтоимТоварВ");
		ДанныеМнЧ = Ветка.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР5М1];
		Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
			
			ЭкзСтоимТоварВ = СкопироватьУзел(ЭкзТоварНеНал, СтоимТоварВ, ЭкзТоварНеНал.Строки.Количество() - 1);
			ЗаполнитьДаннымиУзел(СтрокаМнЧ.Данные, ЭкзСтоимТоварВ);
...
Процедура ВывестиНаПечатьДанныеМногоуровневогоРаздела(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	ИначеЕсли ИмяТекРаздела = "Раздел4" Тогда
		...
		ИмяГруппыМнЧР4М1 = СтруктураРеквизитовФормы.мИдГруппыР4М1;
		ИмяГруппыМнЧР4М2 = СтруктураРеквизитовФормы.мИдГруппыР4М2;
		...
		Для каждого Ветка Из ДеревоДанныхРаздела.Строки Цикл
			...
			ДанныеМнЧ1 = Ветка.ДанныеМногострочныхЧастей[ИмяГруппыМнЧР4М1];
			
			Для каждого СтрокаМнЧ1 Из ДанныеМнЧ1.Строки Цикл
				
				ДанныеМнЧ2 = СтрокаМнЧ1.ДанныеМногострочныхЧастей[ИмяГруппыМнЧР4М2];
				...
				// Подготовка многострочных данных для заполнения листов.
				МассивДанныхМнЧ2ДляПечати = Новый Массив;
				ИмяП00004М241003 = "П00004М241003";
				ИмяП00004М242003 = "П00004М242003";
				ИмяП00004М243003 = "П00004М243003";
				ИмяП00004М244003 = "П00004М244003";
				...
				Для НомЛиста = 1 По КолЛистовРаздела Цикл
					...
					Для ИндСтрокиМнЧ = 1 По СтрокМнЧ2НаЛисте Цикл
						...
						НовИмяП00004М241003 = ИмяП00004М241003 + _ИндСтрокиМнЧ;
						НовИмяП00004М242003 = ИмяП00004М242003 + _ИндСтрокиМнЧ;
						НовИмяП00004М243003 = ИмяП00004М243003 + _ИндСтрокиМнЧ;
						НовИмяП00004М244003 = ИмяП00004М244003 + _ИндСтрокиМнЧ;
						
						ДанныеМнЧ2ДляПечати.Вставить(НовИмяП00004М241003, 0);
						ДанныеМнЧ2ДляПечати.Вставить(НовИмяП00004М242003, "");
						ДанныеМнЧ2ДляПечати.Вставить(НовИмяП00004М243003, 0);
						ДанныеМнЧ2ДляПечати.Вставить(НовИмяП00004М244003, 0);
						
						Если КолСтрокМнЧ2 >= ИндСПриращением Тогда
							ДанныеМнЧ2ДляПечати.Вставить(НовИмяП00004М241003,
							ДанныеМнЧ2.Строки[ИндСПриращением - 1].Данные[ИмяП00004М241003]);
							ДанныеМнЧ2ДляПечати.Вставить(НовИмяП00004М242003,
							ДанныеМнЧ2.Строки[ИндСПриращением - 1].Данные[ИмяП00004М242003]);
							ДанныеМнЧ2ДляПечати.Вставить(НовИмяП00004М243003,
							ДанныеМнЧ2.Строки[ИндСПриращением - 1].Данные[ИмяП00004М243003]);
							ДанныеМнЧ2ДляПечати.Вставить(НовИмяП00004М244003,
							ДанныеМнЧ2.Строки[ИндСПриращением - 1].Данные[ИмяП00004М244003]);
						КонецЕсли;
					КонецЦикла;
					МассивДанныхМнЧ2ДляПечати.Добавить(ДанныеМнЧ2ДляПечати);
				...
				Для НомЛиста = 1 По КолЛистовРаздела Цикл
					...
					Для Ном = 1 По СтрокМнЧ2НаЛисте - 1 Цикл
						ИмяОбластиПреемника = "ДобавлениеСтроки" + ИмяГруппыМнЧР4М2 + "_1" + СтрокаСИндексомПоказателя(Ном);
						ВставитьДопСтроку(ИмяГруппыМнЧР4М2, ИмяОбластиПреемника);
					КонецЦикла;
	...
	ИначеЕсли ИмяТекРаздела = "Раздел5" Тогда
		...
		ИмяГруппыМнЧ = СтруктураРеквизитовФормы.мИдГруппыР5М1;
		СтрокМнЧНаЛисте = 3;
...
Процедура Проверить(Команда)
	...
	ПоказатьПредупреждение(, НСтр(
		...
		|от 21.10.2024 № ЕД-7-3/881@, будет реализована
...
Процедура ЗаполнитьСведенияОбОрганизацииНаСервере(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если ВПрограммеИзмененаДатаПодписи Тогда
		...
		Если СтруктураРеквизитовФормы.ИП Тогда
			...
			ТабличныйДокумент.Области.НаимОрг.Значение = СокрЛП(СведенияОбОрганизации.ФИО);
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	ИначеЕсли Ячейка.Раздел = "Раздел2" ИЛИ Ячейка.Раздел = "Раздел3"
		ИЛИ Ячейка.Раздел = "Раздел4" ИЛИ Ячейка.Раздел = "Раздел5" Тогда
...
Процедура СформироватьЗаголовокОтчета()
	...
	Заголовок = "Декларация по НДС на услуги иностранных организаций в электронной форме за "
