...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "17/10/2012";
...
Процедура ЗаполнитьЗначенияНомеровСтрок()
	...
	Для Ном = 29 По 30 Цикл
		НомСтр = Формат(Ном, "ЧЦ=2");
		КодПоказателя = "П03000" + НомСтр + "01";
		Если НЕ(СтруктураДанныхРаздел2_3.Свойство(КодПоказателя, ЗначениеПоказателя) И ЗначениеЗаполнено(ЗначениеПоказателя)) Тогда
			СтруктураДанныхРаздел2_3.Вставить(КодПоказателя, Ном);
		КонецЕсли;
	КонецЦикла;
...
Процедура УдалитьСтраницу(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, , РежимДиалогаВопрос.ДаНет, 60, КодВозвратаДиалога.Да);
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Раздел2_3", "ФормаОтчета2013Кв1_Раздел2_3");
	СоответствиеРазделовМакетам.Вставить("Раздел4_6", "ФормаОтчета2013Кв1_Раздел4_6");
	СоответствиеРазделовМакетам.Вставить("Раздел7",   "ФормаОтчета2013Кв1_Раздел7");
	СоответствиеРазделовМакетам.Вставить("Раздел8",   "ФормаОтчета2013Кв1_Раздел8");
	СоответствиеРазделовМакетам.Вставить("Раздел9",   "ФормаОтчета2013Кв1_Раздел9");
	СоответствиеРазделовМакетам.Вставить("Раздел10",  "ФормаОтчета2013Кв1_Раздел10");
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1" Тогда
		...
		ИначеЕсли СтрНайти("П010000401,П010000501,П010000601,П010000701,П010000801,П010000901,П010001001,П010001101,П010001201", Область.Имя) > 0 Тогда
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел2_3" Тогда
		...
		ИначеЕсли Область.Имя = "П030002901" Тогда
			...
			СтандартнаяОбработка = Ложь;
			ЭтаФорма[Элемент.Имя].Области[Область.Имя].Значение = -1 * ЭтаФорма[Элемент.Имя].Области[Область.Имя].Значение;
			Если ЭтаФорма[Элемент.Имя].Области[Область.Имя].Значение * ЭтаФорма[Элемент.Имя].Области["П030003001"].Значение >= 0 Тогда
				ЭтаФорма[Элемент.Имя].Области["П030003001"].Значение = -1 * ЭтаФорма[Элемент.Имя].Области["П030003001"].Значение;
			КонецЕсли;
			Модифицированность = Истина;
		...
		ИначеЕсли Область.Имя = "П030003001" Тогда
			...
			СтандартнаяОбработка = Ложь;
			ЭтаФорма[Элемент.Имя].Области[Область.Имя].Значение = -1 * ЭтаФорма[Элемент.Имя].Области[Область.Имя].Значение;
			Если ЭтаФорма[Элемент.Имя].Области[Область.Имя].Значение * ЭтаФорма[Элемент.Имя].Области["П030002901"].Значение >= 0 Тогда
				ЭтаФорма[Элемент.Имя].Области["П030002901"].Значение = -1 * ЭтаФорма[Элемент.Имя].Области["П030002901"].Значение;
			КонецЕсли;
			Модифицированность = Истина;
...
Функция СформироватьИмяФайла(ПараметрыВыгрузки, ДопАтрибуты)
	...
	ИмяФайлаXML = ИмяФайлаXML + ?(ПустаяСтрока(КодОКАТО), "", "_" + КодОКАТО) + ?(ПустаяСтрока(КодОКВЭД), "", "_" + КодОКВЭД) + ".xml";
