...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_tri",   512, , "Выбор кода",   КоллекцияСписковВыбора["s_tri"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_kod_tzv",   512, , "Выбор кода",   КоллекцияСписковВыбора["s_kod_tzv"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_kod_tzv_tri_1",   512, , "Выбор кода",   КоллекцияСписковВыбора["s_kod_tzv_tri_1"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_kod_tzv_tri_2",   512, , "Выбор кода",   КоллекцияСписковВыбора["s_kod_tzv_tri_2"]);
	
	pril_tri_8600 = Новый ТаблицаЗначений;
	pril_tri_8600.Колонки.Добавить("Код",Строка32,, МакетСоставаПоказателей.Область(ВерхОбласти, 1, ВерхОбласти, 1).ШиринаКолонки);
	pril_tri_8600.Колонки.Добавить("Название",Строка512,, МакетСоставаПоказателей.Область(ВерхОбласти, 2, ВерхОбласти, 2).ШиринаКолонки);
	pril_tri_8600.Колонки.Добавить("ДопИнфо",Строка512,, МакетСоставаПоказателей.Область(ВерхОбласти, 3, ВерхОбласти, 3).ШиринаКолонки);
	НоваяСтрокаТЗВыбора = pril_tri_8600.Добавить();
	НоваяСтрокаТЗВыбора.Код = "";
	НоваяСтрокаТЗВыбора.Название = "Пустое значение";
	Для Каждого Стр Из КоллекцияСписковВыбора["s_tri"] Цикл 
		Если КоллекцияСписковВыбора["pril_tri_8600"].НайтиСтроки(Новый Структура("Код", Стр.Код)).Количество() > 0 Тогда 
			НоваяСтрокаТЗВыбора = pril_tri_8600.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаТЗВыбора, Стр);
		КонецЕсли;
	КонецЦикла;
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "pril_tri_8600",   512, , "Выбор кода",   pril_tri_8600);
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Раздел2_3", "ФормаОтчета2016Кв1_Раздел2_3");
	СоответствиеРазделовМакетам.Вставить("Раздел4_6", "ФормаОтчета2016Кв1_Раздел4_6");
	СоответствиеРазделовМакетам.Вставить("Раздел7",   "ФормаОтчета2016Кв1_Раздел7");
	СоответствиеРазделовМакетам.Вставить("Раздел8",   "ФормаОтчета2016Кв1_Раздел8");
	СоответствиеРазделовМакетам.Вставить("Раздел9",   "ФормаОтчета2016Кв1_Раздел9");
	СоответствиеРазделовМакетам.Вставить("Раздел10",  "ФормаОтчета2016Кв1_Раздел10");
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "ПриложенияДляВыгрузки" Тогда
		...
		ИначеЕсли СтрНайти(Область.Имя, "П996001000004_") > 0 Или СтрНайти(Область.Имя, "П997001000003_") > 0 Тогда
			...
			Если Не ЗначениеЗаполнено(Отбор) Тогда 
				ОбщегоНазначенияКлиент.СообщитьПользователю("Необходимо заполнить строку 5999");
				Возврат;
			КонецЕсли;
