...
Процедура ИнициализацияПеременныхМодуля()
	...
	ИменаРазделовИТаблицСтр = "Раздел1,OWN;Раздел2,FOREIGN";
	...
	ИменаСправочниковИСсылки.Добавить("REGION_1", "N:O");          // Регионы2
...
Функция ВыполнитьПроверкуСтроки(Форма, ИмяРазделаИТаблицы,
	...
	Если ИмяРазделаИТаблицы[1] = "OWN" Тогда
		
		ЭлементыПолей = Новый Массив(6); // индексы полей начинаются с 6
		ЭлементыПолей.Добавить(ВопросыРазделаИЗначения["Total_OWN"]);
		ЭлементыПолей.Добавить(ВопросыРазделаИЗначения["Inc1Day_OWN"]);
		ЭлементыПолей.Добавить(ВопросыРазделаИЗначения["Inc25Days_OWN"]);
		ЭлементыПолей.Добавить(ВопросыРазделаИЗначения["Inc614Days_OWN"]);
		ЭлементыПолей.Добавить(ВопросыРазделаИЗначения["Inc15Days_OWN"]);
		
		Если  (ЭлементыПолей[6]  <> Неопределено И ЭлементыПолей[6].Результат = 0)
			И (ЭлементыПолей[7]  <> Неопределено И ЭлементыПолей[7].Результат = 0)
			И (ЭлементыПолей[8]  <> Неопределено И ЭлементыПолей[8].Результат = 0)
			И (ЭлементыПолей[9]  <> Неопределено И ЭлементыПолей[9].Результат = 0)
			И (ЭлементыПолей[10] <> Неопределено И ЭлементыПолей[10].Результат = 0) Тогда
			
			Если  ЭлементыПолей[7].Значение + ЭлементыПолей[8].Значение
				+ ЭлементыПолей[9].Значение + ЭлементыПолей[10].Значение <> ЭлементыПолей[6].Значение Тогда
				Описание = НСтр("ru='ОШИБКА: "
					+ "Итоговая сумма подключений не равна сумме значений подгрупп продолжительности подключений'");
				РезультатПроверки = РезультатПроверки + ?(РезультатПроверки < 2, 2, 0);
				
				ЗаписатьСообщениеОбОшибкеВСтроке(Форма, ПараметрыВыгрузки,
					ЭлементыПолей[6], Описание, ИмяРазделаИТаблицы[0], СтрНомСтроки, НомСтр);
			КонецЕсли;
			
			Для ИндПоля = 7 По 10 Цикл
				ЭлементПоля = ЭлементыПолей[ИндПоля];
				Если ЭлементПоля.Значение > ЭлементыПолей[6].Значение Тогда
					Описание = НСтр("ru='ОШИБКА: "
						+ "Значение поля должно быть меньше или равно количеству подключений ""ВСЕГО""'");
					РезультатПроверки = РезультатПроверки + ?(РезультатПроверки < 2, 2, 0);
		...
		ЗначенияВопросаМесяцКвартала = ВопросыРазделаИЗначения["Month_OWN"];
		...
		Если ЗначенияВопросаМесяцКвартала <> Неопределено И ЗначенияВопросаМесяцКвартала.Результат = 0 Тогда
			Если ОбщегоНазначенияКлиентСервер.ЭтоЧисло(ЗначенияВопросаМесяцКвартала.Значение) Тогда
				...
				Если НомерКварталаПоМесяцу <> ПараметрыВыгрузки.ОтчетКвартал Тогда
					Описание = НСтр("ru = 'ОШИБКА: Выбранный месяц не относится к указанному отчетному кварталу';
									|en = 'ОШИБКА: Выбранный месяц не относится к указанному отчетному кварталу'");
					РезультатПроверки = РезультатПроверки + ?(РезультатПроверки < 2, 2, 0);
			...
			Иначе
				Описание = НСтр("ru = 'ОШИБКА: Должен быть представлен как порядковый номер месяца в году';
								|en = 'ОШИБКА: Должен быть представлен как порядковый номер месяца в году'");
				РезультатПроверки = РезультатПроверки + ?(РезультатПроверки < 2, 2, 0);
	...
	ИначеЕсли ИмяРазделаИТаблицы[1] = "FOREIGN" Тогда
		
		ЭлементыПолей = Новый Массив(6); // индексы полей начинаются с 6
		ЭлементыПолей.Добавить(ВопросыРазделаИЗначения["Total_FOREIGN"]);
		ЭлементыПолей.Добавить(ВопросыРазделаИЗначения["Inc1Day_FOREIGN"]);
		ЭлементыПолей.Добавить(ВопросыРазделаИЗначения["Inc25Days_FOREIGN"]);
		ЭлементыПолей.Добавить(ВопросыРазделаИЗначения["Inc614Days_FOREIGN"]);
		ЭлементыПолей.Добавить(ВопросыРазделаИЗначения["Inc15Days_FOREIGN"]);
		...
		ЗначенияВопросаМесяцКвартала = ВопросыРазделаИЗначения["Month_FOREIGN"];
...
Процедура ВыполнитьПроверкуЗначения(РезультатПроверки, Описание,
	...
	ИначеЕсли ИмяПоказателя = "Razdel1" Тогда
		ТаблицаРаздела1 = ПараметрыВыгрузки.МетаданныеШаблона.ТаблицыРазделов["OWN"];
	...
	ИначеЕсли ИмяПоказателя = "Razdel2" Тогда
		ТаблицаРаздела2 = ПараметрыВыгрузки.МетаданныеШаблона.ТаблицыРазделов["FOREIGN"];
		Если ЗначениеОтвета = "00" Тогда
