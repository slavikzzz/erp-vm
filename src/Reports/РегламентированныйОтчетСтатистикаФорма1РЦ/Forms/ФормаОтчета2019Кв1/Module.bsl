...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0616005";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.Имя = "ОргАдрес" Тогда
		РегламентированнаяОтчетностьКлиент.ОбработкаАдресаВСтатистическомОтчетеJSON(
			ЭтотОбъект, Область, СтандартнаяОбработка);
	ИначеЕсли Лев(Область.Имя, 13) = "П000010000103" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "СправочникКодыВидаПродукции");
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ОбластьИмя = "П000010000103" Тогда
		ПолеТабличногоДокументаФормаОтчета.Области["П000020000103_" + Прав(Область.Имя, 1)].Значение = РезультатКод;
	КонецЕсли;	
...
Процедура ПолеТабличногоДокументаФормаОтчетаПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	ИначеЕсли Область.Имя = "НомерКорректировки" Тогда 
		...
		Возврат;
	ИначеЕсли Лев(Область.Имя, 13) = "П000010000103" Тогда
		ПолеТабличногоДокументаФормаОтчета.Области["П000020000103_" + Прав(Область.Имя, 1)].Значение = Область.Значение;
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Н = 1 По 6 Цикл
		...
		Области["П000020000203_" + Н].Значение = Области["П000020000303_" + Н].Значение
			+ Области["П000020000403_" + Н].Значение;

		Области["П000020000603_" + Н].Значение = Области["П000020000203_" + Н].Значение
			- Области["П000020000503_" + Н].Значение;

		Стр15 = 0;
		Для Стр = 6 По 14 Цикл
			Стр15 = Стр15 + Области["П00002000" + Формат(Стр, "ЧЦ=2; ЧВН=") + "03_" + Н].Значение;
		КонецЦикла;
		Области["П000020001503_" + Н].Значение = Стр15;

		Области["П000020001703_" + Н].Значение = Области["П000020001503_" + Н].Значение
			+ Области["П000020001603_" + Н].Значение;

		Области["П000020001903_" + Н].Значение = Области["П000020001703_" + Н].Значение
			+ Области["П000020001803_" + Н].Значение;

		Области["П000020002103_" + Н].Значение = Области["П000020001903_" + Н].Значение
			+ Области["П000020002003_" + Н].Значение;

		Области["П000020002403_" + Н].Значение = Области["П000020002103_" + Н].Значение
			+ Области["П000020002203_" + Н].Значение
			+ Области["П000020002303_" + Н].Значение;

		Области["П000020002603_" + Н].Значение = Области["П000020002403_" + Н].Значение
			+ Области["П000020002503_" + Н].Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Н = 1 по 6 Цикл
		...
		мФормаОтчета.Области["П000020000203_" + Н].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П000020000203_" + Н].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П000020000603_" + Н].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П000020000603_" + Н].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П000020001503_" + Н].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П000020001503_" + Н].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П000020001703_" + Н].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П000020001703_" + Н].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П000020001903_" + Н].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П000020001903_" + Н].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П000020002103_" + Н].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П000020002103_" + Н].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П000020002403_" + Н].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П000020002403_" + Н].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П000020002603_" + Н].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П000020002603_" + Н].Защита   = Не ОтклАвтоРасчетЗначение;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(
		ПечатнаяФорма, "Форма 1-РЦ ""Сведения о структуре отпускной цены на отдельные виды товаров""");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникКодыВидаПродукции",
	512, , "Выбор кода вида продукции", КоллекцияСписковВыбора["КодыВидаПродукции"]);
