...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("ПредупреждениеПослеАвтозаполнения");
	СтруктураРеквизитовФормы.Вставить("АдресВоВременномХранилищеПредупреждениеПослеАвтозаполнения");
...
Процедура СформироватьСпискиВыбора()
	...
	//Реестр.
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1000009", , ,
	"Выбор программы обучения", КоллекцияСписковВыбора["ПрограммыОбучения"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1000010", , ,
	"Выбор результата обучения", КоллекцияСписковВыбора["РезультатыОбучения"]);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "ТипРеестра1" Тогда
		...
		Иначе
			Область.Значение = "Х";
			ТабличныйДокумент.Области.ТипРеестра2.Значение = "";
		КонецЕсли;
		
		Модифицированность = Истина;
		
	ИначеЕсли Область.Имя = "ТипРеестра2" Тогда
		...
		Иначе
			Область.Значение = "Х";
			ТабличныйДокумент.Области.ТипРеестра1.Значение = "";
		КонецЕсли;
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяТекРаздела)
	...
	ИначеЕсли ИмяТекРаздела = "Реестр" Тогда
		...
		Форма.ТабличныйДокумент.Области["Секция_ПризнакИзмененияДанных"].Видимость = Ложь;
...
Процедура СохранитьДанныеРаздела(НаимРаздела)
	...
	Если мСтруктураМногоуровневыхРазделов.Свойство(НаимРаздела) Тогда
		...
		Для каждого Ветка Из ДеревоДанныхРаздела.Строки Цикл
			Ветка.Данные["ПризнакИзмененияДанных"] = 1;
		КонецЦикла;
...
Функция ДанныеТабличногоДокумента(НаимРаздела)
	...
	Если мСтруктураМногоуровневыхРазделов.Свойство(НаимРаздела) Тогда
		...
		СтруктураДанных.Вставить("ПризнакИзмененияДанных", ТабличныйДокумент.Область("ПризнакИзмененияДанных").Значение);
...
Процедура ВывестиДанныеВТабличныйДокумент(ИмяТекРаздела, НовыеНомераСтрокМногоуровнегоРаздела = Неопределено)
	...
	Если мСтруктураМногоуровневыхРазделов.Свойство(ИмяТекРаздела) Тогда
		...
		ТабличныйДокумент.Области["ПризнакИзмененияДанных"].Значение = 1;
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревьяДляСравнения.Вставить("ДеревоСравненияРеестра",  ИзвлечьСтруктуруXML("СхемаВыгрузки103"));
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревьяДляЗагрузки.Вставить("ДеревоЗагрузкиРеестра",  ИзвлечьСтруктуруXML("СхемаВыгрузки103"));
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	ДеревоВыгрузкиРеестра = ИзвлечьСтруктуруXML("СхемаВыгрузки103");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИННОрг",  СокрЛП(мДанныеОтчета.Титульный.ИНН));
	ПараметрыВыгрузки.Вставить("НаимОрг", СокрЛП(мДанныеОтчета.Титульный.НаимОрг));
	
	ПараметрыВыгрузки.Вставить("ТипРеестра1", СокрЛП(мДанныеОтчета.Титульный.ТипРеестра1));
	ПараметрыВыгрузки.Вставить("ТипРеестра2", СокрЛП(мДанныеОтчета.Титульный.ТипРеестра2));
	
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "RegistrySet");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "ROOTL"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	Если ПараметрыВыгрузки.ЭтоПБОЮЛ Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.НаимОрг) Тогда
			СообщитьВТС("Не задано наименование индивидуального предпринимателя", "Титульный", "НаимОрг");
			СтатусПроверки = Ложь;
		КонецЕсли;
	Иначе
		Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ИННОрг) Тогда
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ТипРеестра1) И НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ТипРеестра2) Тогда
		
		СообщитьВТС("Не задан тип реестра", "Титульный", "ТипРеестра1");
		СтатусПроверки = Ложь;
		
		Возврат СтатусПроверки;
		
	ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки.ТипРеестра1) И ЗначениеЗаполнено(ПараметрыВыгрузки.ТипРеестра2) Тогда
		
		СообщитьВТС("Должен быть задан один из возможных типов реестра", "Титульный", "ТипРеестра1");
		СтатусПроверки = Ложь;
	...
	ВозможныеКодыПрогрОбуч  = ВозможныеЗначенияИзСпискаВыбораПоказателя("П1000009");
	ВозможныеРезультатыОбуч = ВозможныеЗначенияИзСпискаВыбораПоказателя("П1000010");
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000002"]) Тогда
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не задана фамилия",
				"Реестр", "П1000002" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000003"]) Тогда
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не задано имя",
				"Реестр", "П1000003" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ТекстСообщения = "";
			СНИЛС = СокрЛП(Ветка_П10000.Данные["П1000005"]);
			Если НЕ ЗначениеЗаполнено(СНИЛС) Тогда
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не задан СНИЛС",
				"Реестр", "П1000005" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ СНИЛССоответствуетТребованиям(СНИЛС, ТекстСообщения) Тогда
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " неверно задан СНИЛС."
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000006"]) Тогда
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не задана профессия (должность)",
				"Реестр", "П1000006" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000007"]) Тогда
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не задана дата проверки",
				"Реестр", "П1000007" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(НачалоДня(Ветка_П10000.Данные["П1000007"]), , ТекДата) Тогда
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " задана дата проверки, выходящая за пределы периода с 01.01.1900 по " + Формат(ТекДата, "ДФ=дд.ММ.гггг"),
				"Реестр", "П1000007" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000008"]) Тогда
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не задан номер протокола",
				"Реестр", "П1000008" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ЗначПок = СокрЛП(Ветка_П10000.Данные["П1000009"]);
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не задана программа обучения",
				"Реестр", "П1000009" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ВозможныеКодыПрогрОбуч.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " задан некорректный код программы обучения. Возможны значения: " + ВозможныеКодыПрогрОбуч.СтрокаЗначений,
				"Реестр", "П1000009" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ЗначПок = СокрЛП(Ветка_П10000.Данные["П1000010"]);
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не задан результат обучения",
				"Реестр", "П1000010" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ВозможныеРезультатыОбуч.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " задан некорректный код результата обучения. Возможны значения: " + ВозможныеРезультатыОбуч.СтрокаЗначений,
				"Реестр", "П1000010" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ПараметрыВыгрузки.ТипРеестра1) Тогда
				
				ЗначПок = СокрЛП(Ветка_П10000.Данные["П1000011"]);
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан ИНН организации или индивидуального предпринимателя",
					"Реестр", "П1000011" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли (СтрДлина(ЗначПок) <> 10 И СтрДлина(ЗначПок) <> 12)
					ИЛИ НЕ СтроковыеФункцииКлиентсервер.ТолькоЦифрыВСтроке(ЗначПок) Тогда
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " задан ИНН организации или индивидуального предпринимателя, не состоящий из 10 или 12 цифр",
					"Реестр", "П1000011" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000012"]) Тогда
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование организации или индивидуального предпринимателя",
					"Реестр", "П1000012" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
			ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки.ТипРеестра2) Тогда
				
				ЗначПок = СокрЛП(Ветка_П10000.Данные["П1000011"]);
				Если ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не должен быть задан ИНН организации или индивидуального предпринимателя, если реестр составлен"
					+ " по результатам осуществленного работодателем обучения по охране труда своих работников",
					"Реестр", "П1000011" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000012"]) Тогда
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не должно быть задано наименование организации или индивидуального предпринимателя, если реестр составлен"
					+ " по результатам осуществленного работодателем обучения по охране труда своих работников",
					"Реестр", "П1000012" + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
...
&НаСервере
Функция СоответствиеНаименованийКодамПрограммОбучения()
	
	ТЗ = ДанныеФормыВЗначение(РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
	ЭтаФорма, "П1000009").ТаблицаЗначений, Тип("ТаблицаЗначений"));
	
	Соотв = Новый Соответствие;
	
	Для каждого СтрокаТЗ Из ТЗ Цикл
		Если ЗначениеЗаполнено(СтрокаТЗ.Код) Тогда
			Соотв.Вставить("_" + СокрЛП(СтрокаТЗ.Код), СтрокаТЗ.Название);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, ПараметрыВыгрузки)
	...
	ЗаписьРегистра = ПолучитьПодчиненныйЭлемент(УзелРодитель, "RegistryRecord");
	
	СоответствиеНаименованийКодамПрограммОбучения = СоответствиеНаименованийКодамПрограммОбучения();
	...
	Для каждого Ветка Из ДеревоДанных.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			ЭкзЗаписьРегистра = СкопироватьУзел(УзелРодитель, ЗаписьРегистра, УзелРодитель.Строки.Количество() - 1);
			
			Ветка_П10000_Данные = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(Ветка_П10000.Данные);
			
			Если НЕ ЗначениеЗаполнено(Ветка_П10000_Данные["П1000004"]) Тогда
				Ветка_П10000_Данные.Вставить("П1000004", "-");
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ПараметрыВыгрузки.ТипРеестра2) Тогда
				Ветка_П10000_Данные.Вставить("П1000011", ПараметрыВыгрузки.ИННОрг);
				Ветка_П10000_Данные.Вставить("П1000012", ПараметрыВыгрузки.НаимОрг);
			КонецЕсли;
			
			Ветка_П10000_Данные.Вставить("ИННОрг",  ПараметрыВыгрузки.ИННОрг);
			Ветка_П10000_Данные.Вставить("НаимОрг", ПараметрыВыгрузки.НаимОрг);
			
			НаимПрограммы = СоответствиеНаименованийКодамПрограммОбучения.Получить(
			"_" + СокрЛП(Ветка_П10000_Данные["П1000009"]));
			Ветка_П10000_Данные.Вставить("НаимПрограммы", ?(НаимПрограммы = Неопределено, "", НаимПрограммы));
			
			ЗаполнитьДаннымиУзел(Ветка_П10000_Данные, ЭкзЗаписьРегистра);
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "ОбученныеПоОхранеТрудаЛица");
...
Процедура ВывестиНаПечатьОтчет(СтруктураПараметров, ВыбранныеСтраницы, СтрПоля = Неопределено)
	...
	ИмяМакета = "ПечатныйБланк2023Кв1";
	...
	ПечФорма.Параметры.ЗаголовокРеестра = "РЕЕСТР ОБУЧЕННЫХ ПО ОХРАНЕ ТРУДА ЛИЦ от "
	+ ПредставлениеПериода(НачалоДня(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета),
	КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ФП = Истина");
...
Функция ЗаполнитьАвтоНаСервере()
	...
	СтруктураРеквизитовФормы.АдресВоВременномХранилищеПредупреждениеПослеАвтозаполнения = 
		ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
	...
	ПараметрыОтчета.Вставить("АдресВоВременномХранилищеПредупреждениеПослеАвтозаполнения",
	СтруктураРеквизитовФормы.АдресВоВременномХранилищеПредупреждениеПослеАвтозаполнения);
	...
	"РегламентированнаяОтчетность.Автозаполнение_ОбученныеПоОхранеТрудаЛица",
...
Процедура ВывестиДанныеПослеАвтозаполнения()
	...
	СтруктураРеквизитовФормы.Вставить("ПредупреждениеПослеАвтозаполнения",
	ПолучитьИзВременногоХранилища(СтруктураРеквизитовФормы.АдресВоВременномХранилищеПредупреждениеПослеАвтозаполнения));
	УдалитьИзВременногоХранилища(СтруктураРеквизитовФормы.АдресВоВременномХранилищеПредупреждениеПослеАвтозаполнения);
