...
Процедура ВывестиОтчетШтатноеРасписаниеНаПодпись(ДокументРезультат, ПараметрыОтчета, Данные)
	...
	Для Каждого СтрокаПодразделения Из Данные.Строки Цикл
		...
		Если ФОТДоступен Тогда
			Если ИспользоватьВилкуСтавокВШтатномРасписании Тогда
				СтрокаПодразделения.ПлановыйФОТМин = Окр(СтрокаПодразделения.ПлановыйФОТМин, 2);
				СтрокаПодразделения.ПлановыйФОТМакс = Окр(СтрокаПодразделения.ПлановыйФОТМакс, 2);
			Иначе
				СтрокаПодразделения.ПлановыйФОТ = Окр(СтрокаПодразделения.ПлановыйФОТ, 2);
			КонецЕсли;
		КонецЕсли;
		...
		Для каждого СтрокаДолжности Из СтрокаПодразделения.Строки Цикл
			...
			Если ФОТДоступен Тогда
				Если ИспользоватьВилкуСтавокВШтатномРасписании Тогда
					СтрокаДолжности.ПлановыйФОТМин = Окр(СтрокаДолжности.ПлановыйФОТМин, 2);
					СтрокаДолжности.ПлановыйФОТМакс = Окр(СтрокаДолжности.ПлановыйФОТМакс, 2);
				Иначе
					СтрокаДолжности.ПлановыйФОТ = Окр(СтрокаДолжности.ПлановыйФОТ, 2);
				КонецЕсли;
			КонецЕсли;
