...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "05/10/2017";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "П0101" Тогда 
		...
		КонецЕсли;
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П10") Тогда 
		НЗ = (Не Область.Значение);
		Для Инд = 1 По 9 Цикл 
			ПолеТабличногоДокументаФормаОтчета.Области["П10" + Инд].Значение = Ложь;
		КонецЦикла;
		Область.Значение = НЗ;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Для Инд = 101 По 109 Цикл 
		ПараметрыВыгрузки.Вставить("П00" + Инд, ?(ТабДокумент["П" + Инд] = Истина, 1, 0));
	КонецЦикла;
