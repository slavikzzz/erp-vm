...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "14/08/2018";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П11") Тогда 
		НЗ = (Не Область.Значение);
		Для Инд = 0 По 1 Цикл 
			ПолеТабличногоДокументаФормаОтчета.Области["П11" + Инд].Значение = Ложь;
		КонецЦикла;
		Область.Значение = НЗ;
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
