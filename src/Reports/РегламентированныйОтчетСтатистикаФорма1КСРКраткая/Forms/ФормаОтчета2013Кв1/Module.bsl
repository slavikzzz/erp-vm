...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = "0609401";
...
Процедура ПоказатьПериод()
	...
	СтрПериод = НРег("Январь - " + Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'ММММ гггг'") + " г.");
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Для Инд = 1 По 9 Цикл
		ТипЛинии = Неопределено;
		СписокСохранения.Свойство("ТипОрг"+Инд, ТипЛинии);
		Область = ПолеТабличногоДокументаФормаОтчета.Области["ТипОрг"+Инд];
		Область.ГраницаСнизу = Новый Линия(ТипЛинии);
	КонецЦикла;
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант = Неопределено, ВидДокументаНомерКорректировкиИзменен = Неопределено, ПоказателиОтчета = Неопределено) Экспорт
	...
	Для Инд = 1 По 9 Цикл
		Область = ПолеТабличногоДокументаФормаОтчета.Области["ТипОрг"+Инд];
		СписокСохранения.Вставить("ТипОрг"+Инд, Область.ГраницаСнизу.ТипЛинии);
	КонецЦикла;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНайти(Область.Имя, "ТипОрг") = 1 Тогда
		Если Область.ГраницаСнизу.ТипЛинии = ТипЛинииЯчейкиТабличногоДокумента.Сплошная Тогда
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Области.П0106.Значение = Области.П0107.Значение + Области.П0109.Значение + Области.П0110.Значение;
КонецПроцедуры
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Ном = 1 По 2 Цикл
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-КСР (краткая)");
	КонецЦикла;
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 10.08.2011 № 351";
	
	Зн07 = Показатели["П0107"];
	Зн08 = Показатели["П0108"];
	ТекстФормулы = "стр.07 >= стр.08";
	Описание = "стр.07 < стр.08";
	Расшифровка = Строка(Зн07)+" >= "+Строка(Зн08);
	Если Зн07 >= Зн08 Тогда
		ЭтоОшибка = Ложь;
	...
	Зн04 = Показатели["П0104"];
	Если Зн04 > 0 Тогда
		Зн02 = Показатели["П0102"];
		Зн03 = Показатели["П0103"];
		Зн05 = Показатели["П0105"];
		Зн11 = Показатели["П0111"];
		
		ТекстФормулы = "Если стр.04 > 0, то стр.02 > 0";
		Описание = "Если стр.04 > 0, но стр.02 = 0";
		Расшифровка = Строка(Зн04)+" >= 0, "+Строка(Зн02)+" = 0";
		Если Зн02 > 0 Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
		...
		ТекстФормулы = "Если стр.04 > 0, то стр.03 > 0";
		Описание = "Если стр.04 > 0, но стр.03 = 0";
		Расшифровка = Строка(Зн04)+" >= 0, "+Строка(Зн03)+" = 0";
		Если Зн03 > 0 Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
		...
		ТекстФормулы = "Если стр.04 > 0, то стр.05 > 0";
		Описание = "Если стр.04 > 0, но стр.05 = 0";
		Расшифровка = Строка(Зн04)+" >= 0, "+Строка(Зн05)+" = 0";
		Если Зн05 > 0 Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
		...
		ТекстФормулы = "Если стр.04 > 0, то стр.11 > 0";
		Описание = "Если стр.04 > 0, но стр.11 = 0";
		Расшифровка = Строка(Зн04)+" >= 0, "+Строка(Зн11)+" = 0";
		Если Зн11 >= 0 Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
	...
	Для Стр = 3 По 10 Цикл 
		Если Показатели["П01" + Формат(Стр, "ЧЦ=2; ЧВН=; ЧГ=;")] > 0 Тогда
			Зн3_11БН = Истина;
			Зн3_10БН = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Зн3_11БН = Зн3_11БН Или (Показатели["П0111"] > 0);
	
	Если Зн3_10БН И Показатели["П0102"] = 0 Тогда
		ТекстФормулы = "Если одна из строк 3-10 > 0, то стр.02 > 0";
		Описание = "Если одна из строк 3-10 > 0, но стр.02 = 0";
		Расшифровка = "";
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
		РегламентированнаяОтчетность.ДобавитьЭлементВСтруктуруОшибок(СтруктураОшибок, ТекстФормулы, "", ОбщееЧислоОшибок, ОбщееЧислоБезОшибок, Расшифровка, Нарушение, Описание, ЭтоОшибка);
	...
	Если Зн3_10БН И Показатели["П0111"] = 0 Тогда
		ТекстФормулы = "Если одна из строк 3-10 > 0, то стр.11 > 0";
		Описание = "Если одна из строк 3-10 > 0, но стр.11 = 0";
		Расшифровка = "";
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
		РегламентированнаяОтчетность.ДобавитьЭлементВСтруктуруОшибок(СтруктураОшибок, ТекстФормулы, "", ОбщееЧислоОшибок, ОбщееЧислоБезОшибок, Расшифровка, Нарушение, Описание, ЭтоОшибка);
	...
	Если (Не Зн3_10БН) И Зн3_11БН И Показатели["П0102"] = 0 Тогда
		ТекстФормулы = "Если одна из строк 3-11 > 0, то стр.02 > 0";
		Описание = "Если одна из строк 3-11 > 0, но стр.02 = 0";
		Расшифровка = "";
		ЭтоОшибка = Истина;
		ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
		РегламентированнаяОтчетность.ДобавитьЭлементВСтруктуруОшибок(СтруктураОшибок, ТекстФормулы, "", ОбщееЧислоОшибок, ОбщееЧислоБезОшибок, Расшифровка, Нарушение, Описание, ЭтоОшибка);
