...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "18/08/2017";
...
Процедура УдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, ТекущаяСтраница) Экспорт
	...
	УдалитьДополнительнуюСтрокуИзГруппы(Форма, СпециальноеДобавление, ИмяКолонкиНомера, ТекущаяСтраница, "П0000020001", НомУдаляемойСтроки, НомерСтрокиТаблицы);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли (Не (Область.Защита = Истина)) И СтрНайти(Область.Имя, "_") = 14 И СтруктураРеквизитовФормы.мАвтоВыборКодов = Ложь Тогда 
		Если СтрНайти(Область.Имя, "П000004000102") = 1 Или СтрНайти(Область.Имя, "П000004000103") = 1 Тогда 
		...
		ИначеЕсли СтрНайти(Область.Имя, "П000002000102") = 1 Или СтрНайти(Область.Имя, "П000002000103") = 1 Тогда 
			
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "Вещества");
			ОбластьДляВвода     = ПолеТабличногоДокументаФормаОтчета.Области["П000002000103" + Сред(Область.Имя, 14)];
			ОбластьНаименование = ПолеТабличногоДокументаФормаОтчета.Области["П000002000102" + Сред(Область.Имя, 14)];
			ВвестиПоказатель(ОбластьДляВвода, ПараметрВводаПоля.Показатель, ПараметрВводаПоля.ТекстПриВыборе,
							ПараметрВводаПоля.ТаблицаЗначений, СтандартнаяОбработка, ОбластьНаименование);
							
		ИначеЕсли СтрНайти(Область.Имя, "П000004000104") = 1 Тогда
		...
		ИначеЕсли СтрНайти(Область.Имя, "П000002000104") = 1 Тогда 
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "КлассОпасностиОтходов");
			ВвестиПоказатель(Область, ПараметрВводаПоля.Показатель, ПараметрВводаПоля.ТекстПриВыборе,
							ПараметрВводаПоля.ТаблицаЗначений, СтандартнаяОбработка);
		КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Пока Истина Цикл
		...
		Если Области.Найти("П000002000101_" + СтрСтр) = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		Области["П000004000112_" + СтрСтр].Значение = Области["П000002000105_" + СтрСтр].Значение + Области["П000002000106_" + СтрСтр].Значение
												+ Области["П000002000107_" + СтрСтр].Значение - Области["П000002000109_" + СтрСтр].Значение
												- Области["П000002000110_" + СтрСтр].Значение - Области["П000002000113_" + СтрСтр].Значение
												- Области["П000004000105_" + СтрСтр].Значение - Области["П000004000106_" + СтрСтр].Значение
												- Области["П000004000107_" + СтрСтр].Значение - Области["П000004000108_" + СтрСтр].Значение
												- Области["П000004000109_" + СтрСтр].Значение - Области["П000004000110_" + СтрСтр].Значение
												- Области["П000004000111_" + СтрСтр].Значение;
		Стр = Стр + 1;
	КонецЦикла;
		...
		Если Стр.Тип = "С" ИЛИ Стр.Тип = "C" Тогда // учтем оба варианта: кириллицу и латиницу
			Если Стр.Многострочность И Стр.Код = ПараметрыВыгрузки.ИмяКлючевогоУзлаСодержательнойЧасти Тогда
				...
				Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, "П000002000101_" + Формат(НомСтр, "ЧГ=")) Цикл
					...
					ПараметрыВыгрузки.Вставить("П000002000199_" + НомСтрСтр, ПараметрыВыгрузки["П000004000105_" + НомСтрСтр]+ПараметрыВыгрузки["П000004000106_" + НомСтрСтр]
																				+ПараметрыВыгрузки["П000004000107_" + НомСтрСтр]+ПараметрыВыгрузки["П000004000108_" + НомСтрСтр]
																				+ПараметрыВыгрузки["П000004000109_" + НомСтрСтр]);
					ПараметрыВыгрузки.Вставить("П000004000199_" + НомСтрСтр, ПараметрыВыгрузки["П000004000110_" + НомСтрСтр]+ПараметрыВыгрузки["П000004000111_" + НомСтрСтр]);
