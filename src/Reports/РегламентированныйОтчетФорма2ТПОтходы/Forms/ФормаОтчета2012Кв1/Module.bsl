...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "30/05/2014";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Стр = 1;
	Пока Истина Цикл
		Если Области.Найти("П000002000101_" + Стр) = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		Области["П000004000105_" + Стр].Значение = Области["П000004000106_" + Стр].Значение + Области["П000004000107_" + Стр].Значение
												+ Области["П000004000108_" + Стр].Значение + Области["П000004000109_" + Стр].Значение;
		Области["П000004000110_" + Стр].Значение = Области["П000004000111_" + Стр].Значение + Области["П000004000112_" + Стр].Значение;
		Области["П000004000113_" + Стр].Значение = Области["П000002000105_" + Стр].Значение + Области["П000002000106_" + Стр].Значение
												+ Области["П000002000107_" + Стр].Значение - Области["П000002000109_" + Стр].Значение
												- Области["П000002000110_" + Стр].Значение - Области["П000004000105_" + Стр].Значение
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабличноеПоле.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2012Кв1_ФормаОтчета"));
		...
		Если Стр.Тип = "С" ИЛИ Стр.Тип = "C" Тогда // учтем оба варианта: кириллицу и латиницу
			Если Стр.Многострочность И Стр.Код = ПараметрыВыгрузки.ИмяКлючевогоУзлаСодержательнойЧасти Тогда
				...
				Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, "П000002000101_" + Формат(НомСтр, "ЧГ=")) Цикл
					ЗначениеПоказателя = СтрЗаменить(СтрЗаменить(ПараметрыВыгрузки["П000002000103_" + Формат(НомСтр, "ЧГ=")], " ", ""), Символы.НПП, "");
					ПараметрыВыгрузки.Вставить("П000002000103_" + Формат(НомСтр, "ЧГ="), ЗначениеПоказателя);
