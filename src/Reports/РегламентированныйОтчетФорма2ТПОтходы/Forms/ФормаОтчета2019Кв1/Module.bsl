...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы2, 13);
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы3, 14);
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы4, 16);
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы5, 13);
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Пока Истина Цикл
		...
		Если Области.Найти("П000002000101_" + СтрСтр) = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		Области["П000004000113_" + СтрСтр].Значение = Области["П000002000105_" + СтрСтр].Значение + Области["П000002000106_" + СтрСтр].Значение
												+ Области["П000002000107_" + СтрСтр].Значение - Области["П000002000109_" + СтрСтр].Значение - Области["П000002000110_" + СтрСтр].Значение
												- Области["П000002000113_" + СтрСтр].Значение - Области["П000004000105_" + СтрСтр].Значение - Области["П000004000106_" + СтрСтр].Значение
												- Области["П000004000107_" + СтрСтр].Значение - Области["П000004000108_" + СтрСтр].Значение - Области["П000004000109_" + СтрСтр].Значение
												- Области["П000004000110_" + СтрСтр].Значение - Области["П000004000111_" + СтрСтр].Значение - Области["П000004000112_" + СтрСтр].Значение;
		Стр = Стр + 1;
	КонецЦикла;
	...
	Пока Истина Цикл
		...
		Если Области.Найти("П000005000101_" + СтрСтр) = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		Области["П000007000110_" + СтрСтр].Значение = Области["П000005000105_" + СтрСтр].Значение + Области["П000005000106_" + СтрСтр].Значение
												+ Области["П000005000107_" + СтрСтр].Значение - Области["П000005000111_" + СтрСтр].Значение - Области["П000006000105_" + СтрСтр].Значение
												- Области["П000006000108_" + СтрСтр].Значение - Области["П000006000111_" + СтрСтр].Значение - Области["П000006000114_" + СтрСтр].Значение
												- Области["П000007000105_" + СтрСтр].Значение - Области["П000007000108_" + СтрСтр].Значение;
		Стр = Стр + 1;
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗаполнитьДаннымиУзел(ПараметрыВыгрузки, Узел, НомерСтроки = Неопределено)
	...
	Для Каждого Стр из СтрокиУзла Цикл
		...
		Если Стр.Тип = "С" ИЛИ Стр.Тип = "C" Тогда // учтем оба варианта: кириллицу и латиницу
			Если Стр.Многострочность И СтрНачинаетсяС(Стр.Код, ПараметрыВыгрузки.ИмяКлючевогоУзлаСодержательнойЧасти + "1") Тогда
				НомСтр = 1;
				Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, "П000002000101_" + Формат(НомСтр, "ЧГ=")) Цикл
					...
					ЗначениеПоказателя = СтрЗаменить(СтрЗаменить(ПараметрыВыгрузки["П000002000103_" + НомСтрСтр], " ", ""), Символы.НПП, "");
					ПараметрыВыгрузки.Вставить("П000002000103_" + НомСтрСтр, ЗначениеПоказателя);
			...
			ИначеЕсли Стр.Многострочность И СтрНачинаетсяС(Стр.Код, ПараметрыВыгрузки.ИмяКлючевогоУзлаСодержательнойЧасти + "2") Тогда
				НомСтр = 1;
				Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, "П000005000103_" + Формат(НомСтр, "ЧГ=")) Цикл
					НомСтрСтр = Формат(НомСтр, "ЧГ=");
					ЗначениеПоказателя = Формат(ОТЧ.ПривестиЗначение(ПараметрыВыгрузки["П000005000104_" + НомСтрСтр]), "");
					ПараметрыВыгрузки.Вставить("П000005000104_" + НомСтрСтр, ЗначениеПоказателя);
					УзелСоответствующийСтроке = РегламентированнаяОтчетность.СкопироватьУзел(Узел, Стр);
					ЗаполнитьДаннымиУзел(ПараметрыВыгрузки, УзелСоответствующийСтроке, НомСтр);
					НомСтр = НомСтр + 1;
				КонецЦикла;
