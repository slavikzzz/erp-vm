...
Процедура ПоказатьПериод()
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета.Значение = "в " + Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)/3 + " квартале " + НРег(Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг'")) + " г.";
	СтруктураРеквизитовФормы.СтрПериодОтчета = "за " + Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)/3 + " квартал "
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 2) = "П0" Тогда
		ПравЧасть = Прав(Область.Имя, 2);
		Если ПравЧасть = "09" Или ПравЧасть = "10" Или ПравЧасть = "11" Тогда 
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "КодПричины");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКАТО) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не задан код ОКАТО организации!",
...
&НаСервере
Функция ПолучитьПодчиненныйУзелПоКоду(Узел, Код)
	
	Для Каждого Стр из Узел.Строки Цикл
		Если Стр.Код = Код Тогда
			Возврат Стр;
		КонецЕсли;
	КонецЦикла;
	Возврат Неопределено;
...
&НаСервере
Процедура УстановитьФлагиВСтроке(Строка)
	Для Каждого Стр Из Строка.Строки Цикл 
		Если Не ЗначениеЗаполнено(Стр.Ключ) Или Не ЗначениеЗаполнено(Стр.Значение) Тогда 
			Продолжить;
		КонецЕсли;
		
		ЗначениеПравильногоФормата = (Стр.Значение = "3" Или Стр.Значение = "4" Или Стр.Значение = "6" Или Стр.Значение = "10");
		Если ЗначениеПравильногоФормата Тогда 
			НомерКол = Прав(Стр.Ключ, 2);
			Если НомерКол = "09" Тогда
				Стр.Значение = "3" + ?(Стр.Значение = "10", "", "0") + Стр.Значение;
			КонецЕсли;
			Если НомерКол = "10" Тогда
				Стр.Значение = "4" + ?(Стр.Значение = "10", "", "0") + Стр.Значение;
			КонецЕсли;
			Если НомерКол = "11" Тогда
				Стр.Значение = "5" + ?(Стр.Значение = "10", "", "0") + Стр.Значение;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
&НаСервере
Процедура ДополнительнаяОбработка1РЖ(ДеревоВыгрузки)
	Узед_report = ПолучитьПодчиненныйУзелПоКоду(ДеревоВыгрузки, "report");
	Узед_sections = ПолучитьПодчиненныйУзелПоКоду(Узед_report, "sections");
	
	Для Каждого Узед_section Из Узед_sections.Строки Цикл
		Для Каждого Стр Из Узед_section.Строки Цикл 
			Если Стр.Код <> "row" Тогда 
				Продолжить;
			КонецЕсли;
			
			УстановитьФлагиВСтроке(Стр);
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	ДополнительнаяОбработка1РЖ(ДеревоВыгрузки);
	РегламентированнаяОтчетность.ОтсечьНезаполненныеНеобязательныеУзлыСтатистики(ДеревоВыгрузки);
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ОтчПериод  = "" + (12 + Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)/3);
...
Процедура СформироватьСпискиВыбора()
	...
	ТаблСписка.Колонки.Добавить("Код",      Строка32, , 32);
	ТаблСписка.Колонки.Добавить("Название", Строка32, , 32);
	...
	НовСтрока = ТаблСписка.Добавить();
	НовСтрока.Код = "3";
	НовСтрока.Название = "Сезонное изменение цен";
	
	НовСтрока = ТаблСписка.Добавить();
	НовСтрока.Код = "4";
	НовСтрока.Название = "Изменение спроса";
	
	НовСтрока = ТаблСписка.Добавить();
	НовСтрока.Код = "6";
	НовСтрока.Название = "Изменение курса валют";
	
	НовСтрока = ТаблСписка.Добавить();
	НовСтрока.Код = "10";
	НовСтрока.Название = "Другие причины изменения цены";
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "КодПричины", 512, , "Выбор признака", ТаблСписка);
