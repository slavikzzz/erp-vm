...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы4 = "П0000201001";
	СтруктураРеквизитовФормы.мИдГруппы5 = "П0000201002";
	СтруктураРеквизитовФормы.мИдГруппы6 = "П0000201003";
...
Процедура ПоказатьПериод()
	...
	СтруктураРеквизитовФормы.СтрПериодОтчета = "за "
		+ Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)/3 + " квартал "
		...
		Для МнгСтр = 1 По 3 Цикл
			...
			Пока Истина Цикл
				...
				Если Обл = Неопределено Тогда
					Прервать;
				КонецЕсли;
				Если ЗначениеЗаполнено(Области["П0000" + Раздел + "0100" + МнгСтр + "01_" + Инд].Значение) Тогда
					НПП = НПП + 1;
					Области[ИмяЯчейкиНомерСроки].Значение = НПП;
				Иначе
					Области[ИмяЯчейкиНомерСроки].Значение = "";
				КонецЕсли;
				Инд = Инд + 1;
			КонецЦикла;
		КонецЦикла;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	П0000101002.Очистить();
	П0000101003.Очистить();
	П0000201001.Очистить();
	П0000201002.Очистить();
	П0000201003.Очистить();
	РегламентированнаяОтчетность.НачальныеОперацииСМногострочнымиБлоками(ЭтотОбъект);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ТекущаяОбласть = ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть2_6");
