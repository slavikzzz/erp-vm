...
Процедура ИнициализироватьСписокВыбораОрганизаций(ИзмененПериод = Ложь)
	...
	Если ЭтоСлучайУстановкиФильтраПоЮридическимЛицам(мДатаКонцаПериодаОтчета) Тогда
	...
	Если Элементы.Организация.СписокВыбора.Количество() = 0 Тогда
		...
		Если НЕ ИзмененПериод И ЭтоСлучайУстановкиФильтраПоЮридическимЛицам(мДатаКонцаПериодаОтчета) Тогда
...
Функция ДоступныеОрганизацииОтсутствуютТекст()
	...
	Возврат НСтр(
		"ru = 'Декларацию по налогу на игорный бизнес представляют только организации (ст. 365 НК РФ).
		|В справочнике ""Организации"" сведения об организациях отсутствуют.';
		|en = 'Декларацию по налогу на игорный бизнес представляют только организации (ст. 365 НК РФ).
		|В справочнике ""Организации"" сведения об организациях отсутствуют.'");
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоСлучайУстановкиФильтраПоЮридическимЛицам(ДатаКонцаПериодаОтчета)
	
	Возврат ДатаКонцаПериодаОтчета > '20211201';
