...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь)
    ...
    Сведения = "ИННЮЛ, КППЮЛ, НаимЮЛПол, КодНО, НаимНО, ТипНП, ТелОрганизации, ФИО, ОКТМО";
...
Процедура ЗаполнитьСведенияОПодписантахИКодПоОКАТО(Ссылка)
	...
	ЗаполнитьОКТМО(ЭтаФорма, Ссылка.КодПоОКТМО);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураДанныхТитульный.Вставить("НомерКорректировки", Строка(СтруктураРеквизитовФормы.ВидДокумента * НомерКорректировки));
...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	// ОКТМО.
	ПоказательОКТМО = Показатели.Найти("П000100002003", "Имя");
					...
					Если РезультатПоиска <> Неопределено Тогда
						Если ТипЗнч(РезультатПоиска.Значение)=Тип("Число") Тогда
							...
							Если ИндексТочкиПоКорню.Свойство(ТекАдресЗначенияОбласти.Корень,ИндексТочкиЧислаНаМЧБ) = Ложь Тогда
								...
								Если (ИмяОбласти = "П000200008003_6"
									ИЛИ ИмяОбласти = "П000200011003_6"
									ИЛИ ИмяОбласти = "П000200012003_6")
									И РезультатПоиска.Значение < 100000 Тогда
									ПрисвоитьЗначениеОбласти(Область, РезультатПоиска.ЗначениеМЧБ, ТекАдресЗначенияОбласти.ИндексЗнакоместа, Ложь);
									ЗначениеВыведено = Истина;
								ИначеЕсли (ИмяОбласти = "П000200008003_5"
									ИЛИ ИмяОбласти = "П000200011003_5"
									ИЛИ ИмяОбласти = "П000200012003_5")
									И РезультатПоиска.Значение < 10000 Тогда
									ПрисвоитьЗначениеОбласти(Область, РезультатПоиска.ЗначениеМЧБ, ТекАдресЗначенияОбласти.ИндексЗнакоместа, Ложь);
									ЗначениеВыведено = Истина;
								Иначе
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	ДлинаОКТМО = СтрДлина(СокрЛП(ДанныеРаздела1.П000100002003));
	Если ДлинаОКТМО <> 11 И ДлинаОКТМО <> 8 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 1 не" + ?(ДлинаОКТМО = 0, "", "верно") + " задан код по ОКТМО!", "Раздел1", "П000100002003");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьОКТМО(Форма, Знач ОКТМО)
	...
	Форма.СтруктураДанныхРаздел1.Вставить("П000100002003", ОКТМО);
