...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ЗачетГоспошлины;
	...
	Если ЭтотОбъект["ЭтоЮЛ"] Тогда 
		...
		Элементы.ДеревоСтраниц.Видимость = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПредставлениеУведомленияВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "КБКЗачет" Или Область.Имя = "КБК" Тогда 
...
Процедура ЗаполнитьНачальныеДанные() Экспорт
	...
	УдЛичнФЛ = ЭтотОбъект["ДанныеУведомления"]["УдЛичнФЛ"];
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(СведенияОбОрганизации.ИННФЛ) Тогда 
			...
			УдЛичнФЛ.Вставить("КодВидДок", СведенияОбОрганизации.КодУдЛичн);
			УдЛичнФЛ.Вставить("СерНомДок", СведенияОбОрганизации.СерияУдЛичн + " " + СведенияОбОрганизации.НомерУдЛичн);
			УдЛичнФЛ.Вставить("ВыдДок", СведенияОбОрганизации.ОрганВыданУдЛичн);
			УдЛичнФЛ.Вставить("ДатаДок", СведенияОбОрганизации.ДатаУдЛичн);
		КонецЕсли;
	КонецЕсли;
...
Процедура СформироватьДеревоСтраниц() Экспорт
	...
	Стр001 = ДеревоСтраниц.ПолучитьЭлементы().Добавить();
	Стр001.Наименование = "Сведения о"+символы.ПС+"документе";
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "УдЛичнФЛ_2023";
	Стр001.Многостраничность = Ложь;
	Стр001.Многострочность = Ложь;
	Стр001.УИД = Новый УникальныйИдентификатор;
