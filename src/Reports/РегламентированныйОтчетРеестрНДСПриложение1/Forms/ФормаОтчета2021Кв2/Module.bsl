...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр р.НДС прилож.01 21к2") Тогда
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрТД_1");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRTDNDS"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRTDNDS.2"));
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрТД_1 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрТД_1");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрТД_1 = СкопироватьУзел(УзелРодитель.Родитель, РеестрТД_1);
		Для каждого Эл Из ЭкзРеестрТД_1.Строки Цикл
		...
		СведТД = ПолучитьПодчиненныйЭлемент(ЭкзРеестрТД_1, "СведТД");
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведТД = СкопироватьУзел(ЭкзРеестрТД_1, СведТД, ЭкзРеестрТД_1.Строки.Количество() - 1);
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_1");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_1_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
...
Функция ЗаполнитьАвтоНаСервере()
	...
	ПараметрыОтчета.Вставить("МаксКолСтрокНаСтранице",    1000);
	...
	РезультатЗапуска = ДлительныеОперации.ВыполнитьВФоне("РегламентированнаяОтчетность.Автозаполнение_РеестрПоНДС_Приложение1",
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Реестр НДС Приложение 1");
...
Процедура ПроверкаКС_1_1_1()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '1.1.1';
													|en = '1.1.1'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 1';
														|en = 'Реестр 1'"));
...
&НаСервере
Процедура ПроверкаКС_1_1_3а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '1.1.3.а';
													|en = '1.1.3.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 1';
														|en = 'Реестр 1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_1_1_3б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '1.1.3.б';
													|en = '1.1.3.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 1';
														|en = 'Реестр 1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_1_1_3в()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '1.1.3.в';
													|en = '1.1.3.в'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 1';
														|en = 'Реестр 1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_1_2_1а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '1.2.1.а';
													|en = '1.2.1.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 1';
														|en = 'Реестр 1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_1_2_1б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '1.2.1.б';
													|en = '1.2.1.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 1';
														|en = 'Реестр 1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_1_2_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '1.2.2';
													|en = '1.2.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 1';
														|en = 'Реестр 1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
