...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрДТ_1");
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если НЕ (ДлинаРегНомДТ = 23 ИЛИ (ДлинаРегНомДТ >= 25 И ДлинаРегНомДТ <= 29)) Тогда
				...
				+ НомПП + " не" + ?(ДлинаРегНомДТ = 0, "", "верно")
				+ " задан регистрационный номер декларации на товары (полной декларации на товары). Возможное количество символов в номере: 23 или от 25 до 29",
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100005"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не задана дата контракта/договора",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрДТ_1 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрДТ_1");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрДТ_1 = СкопироватьУзел(УзелРодитель.Родитель, РеестрДТ_1);
		Для каждого Эл Из ЭкзРеестрДТ_1.Строки Цикл
		...
		СведДТ = ПолучитьПодчиненныйЭлемент(ЭкзРеестрДТ_1, "СведДТ");
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведДТ = СкопироватьУзел(ЭкзРеестрДТ_1, СведДТ, ЭкзРеестрДТ_1.Строки.Количество() - 1);
				...
				// 11000.
				СведКонтрДог = ПолучитьПодчиненныйЭлемент(ЭкзСведДТ, "СведКонтрДог");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					
					ЭкзСведКонтрДог = СкопироватьУзел(ЭкзСведДТ, СведКонтрДог, ЭкзСведДТ.Строки.Количество() - 1);
...
Функция ЗаполнитьАвтоНаСервере()
	...
	ПараметрыОтчета.Вставить("МаксКолСтрокНаСтранице",    500);
