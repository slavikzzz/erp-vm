...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'В настройку отчета Т-54 внесены критичные изменения. Отчет не будет сформирован.';
								|en = 'Critical changes have been made to T-54 report setting. Cannot generate the report.'") + " " + КраткоеПредставлениеОшибки(Инфо);
	КонецПопытки;
		...
		Для НомерСтроки = 0 По МаксимальноеКоличествоСтрок - 1 Цикл
			...
			Если НастройкиПечатныхФорм.УдалятьПрефиксыОрганизацииИИБИзНомеровКадровыхПриказов Тогда
				...
				Если Не СтрНачинаетсяС(МакетСтроки.Параметры.КадровыйПриказНомер, НСтр("ru = 'Тр. дог. №';
																						|en = 'Employment contract No.'"))
					И Не СтрНачинаетсяС(МакетСтроки.Параметры.КадровыйПриказНомер, НСтр("ru = 'Контр. №';
																						|en = 'Contract No.'")) Тогда
...
Функция ДанныеНачислений(КлючВарианта, ПараметрыОтчетов)
	...
	ПараметрИсчисленныеНалоги = ОтчетАнализНачисленийИУдержаний.КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ИсчисленныеНалоги"));
				...
				Если ДанныеСотрудника <> Неопределено Тогда
					...
					Для Каждого СтрокаБезРеквизитов Из СтрокиСотрудника Цикл
						Если Не ЗначениеЗаполнено(ДанныеСотрудника.ПриказОПриемеВидДоговора)
							...
							СтрокаБезРеквизитов.НомерПриказа = СтрШаблон(НСтр("ru = 'Тр. дог. №%1';
																				|en = 'Employment contract No. %1'"), ДанныеСотрудника.ПриказОПриемеТрудовойДоговорНомер);
						Иначе
							СтрокаБезРеквизитов.НомерПриказа = СтрШаблон(НСтр("ru = 'Контр. №%1';
																				|en = 'Contract No. %1'"), ДанныеСотрудника.ПриказОПриемеТрудовойДоговорНомер);
						КонецЕсли;
...
Функция ПредставлениеДокументаУдержания(СтрокаДанных, СоответствиеПользовательскихПолей)
	...
	Если ЗначениеЗаполнено(ДокументУдержания) Тогда
		...
		Представление = КраткоеПредставлениеТипаДокумента(ДокументУдержания)
			...
			+ " " + НСтр("ru = 'от';
						|en = 'dated'") + " " + Формат(СвойстваРегистратора.Дата, "ДЛФ=D");
