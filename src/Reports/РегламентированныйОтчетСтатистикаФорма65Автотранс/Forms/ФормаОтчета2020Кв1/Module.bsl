...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Области["П000020101403"].Значение = Области["П000020101503"].Значение + Области["П000020101603"].Значение;
	Области["П000020101404"].Значение = Области["П000020101504"].Значение + Области["П000020101604"].Значение;
	
	Области["П000010100203"].Значение
		= Области["П000020101203"].Значение + Области["П000020101303"].Значение
		+ Области["П000020101403"].Значение + Области["П000020101703"].Значение
		+ Области["П000020101803"].Значение + Области["П000020101903"].Значение;
		
	Для Стр = 21 По 26 Цикл
		СуммаПоГрафам = 0;
		Для Гр = 4 По 7 Цикл
			СуммаПоГрафам = СуммаПоГрафам + Области["П00003010" + Стр + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00003010" + Стр + "03"].Значение = СуммаПоГрафам;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П000010100203"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000010100203"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000020101403"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000020101403"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000020101404"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000020101404"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	Для Стр = 21 По 26 Цикл
		мФормаОтчета.Области["П00003010" + Стр + "03"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00003010" + Стр + "03"].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
