...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "21/08/2015";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Области.П0110004.Значение = Области.П0110005.Значение + Области.П0110006.Значение + Области.П0110007.Значение;
	Области.П0110104.Значение = Области.П0110105.Значение + Области.П0110106.Значение + Области.П0110107.Значение;
	Области.П0110504.Значение = Области.П0110505.Значение + Области.П0110506.Значение + Области.П0110507.Значение;
	Области.П0110604.Значение = Области.П0110605.Значение + Области.П0110606.Значение + Области.П0110607.Значение;
	Области.П0110704.Значение = Области.П0110705.Значение + Области.П0110706.Значение + Области.П0110707.Значение;
	...
	Области.П0330005.Значение = Области.П0330006.Значение + Области.П0330007.Значение + Области.П0330008.Значение + Области.П0330009.Значение;
	Области.П0331005.Значение = Области.П0331006.Значение + Области.П0331007.Значение + Области.П0331008.Значение + Области.П0331009.Значение;
КонецПроцедуры
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 18.08.2015 № 378";
	...
	Для Гр = 3 По 9 Цикл 
		...
		ТекстФормулы = "по гр." + Гр + " стр.105 >= стр.106";
		Описание = "по гр." + Гр + " стр.105 < стр.106";
		Расшифровка = Строка(Зн105) + " >= " + Строка(Зн106);
		
		Если Зн105 >= Зн106 Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
			ЭтоОшибка = Истина;
	...
	Для Стр = 100 По 107 Цикл
		ЗнГр3 = ПолеТабличногоДокументаФормаОтчета.Области["П01"+Стр+"03"].Значение;
		ЗнГр4 = ПолеТабличногоДокументаФормаОтчета.Области["П01"+Стр+"04"].Значение;
		
		ТекстФормулы = "по стр." + Стр + " гр.3 >= гр.4";
		Описание = "по стр." + Стр + " гр.3 < гр.4";
		...
		Если Стр >= 102 И Стр <= 104 Тогда 
			Продолжить;
		КонецЕсли;
		
		ЗнГр8 = ПолеТабличногоДокументаФормаОтчета.Области["П01"+Стр+"08"].Значение;
		ЗнГр9 = ПолеТабличногоДокументаФормаОтчета.Области["П01"+Стр+"09"].Значение;
		
		ТекстФормулы = "по стр." + Стр + " гр.3 >= гр.8+гр.9";
		Описание = "по стр." + Стр + " гр.3 < гр.8+гр.9";
		Расшифровка = Строка(ЗнГр3) + " >= " + Строка(ЗнГр8) + "+" + Строка(ЗнГр9);
		
		Если ЗнГр3 >= ЗнГр8+ЗнГр9 Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
			ЭтоОшибка = Истина;
	...
	Для Стр = 20 По 21 Цикл 
		СтрСтр = "" + (Стр*10 + 20);
		ЗнГр8 = ПолеТабличногоДокументаФормаОтчета.Области["П02"+СтрСтр+"08"].Значение;
		ЗнГр9 = ПолеТабличногоДокументаФормаОтчета.Области["П02"+СтрСтр+"09"].Значение;
		
		ТекстФормулы = "по стр." + (Стр*10) + " гр.8 >= гр.9";
		Описание = "по стр." + (Стр*10) + " гр.8 < гр.9";
	...
	Для Гр = 5 По 13 Цикл 
		ГрСтр = Прав("0" + Гр, 2);
		Зн200 = ПолеТабличногоДокументаФормаОтчета.Области["П02220"+ГрСтр].Значение;
		Зн210 = ПолеТабличногоДокументаФормаОтчета.Области["П02230"+ГрСтр].Значение;
		
		ТекстФормулы = "стр.200 = 0 <=> стр.210=0 по гр." + Гр;
		Описание = "стр.200 = 0 <=> стр.210=0 по гр." + Гр;
		Расшифровка = Строка(Зн200) + "," + Строка(Зн210);
		
		Если (Зн200 = 0 И Зн210 = 0) Или (Зн200 <> 0 И Зн210 <> 0) Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
			ЭтоОшибка = Истина;
	...
	Зн600 = ПолеТабличногоДокументаФормаОтчета.Области["П0460005"].Значение;
	Зн610 = ПолеТабличногоДокументаФормаОтчета.Области["П0461005"].Значение;
	
	ТекстФормулы = "стр.600 >= стр.610";
	Описание = "стр.600 < стр.610";
	Расшифровка = Строка(Зн600) + " >= " + Строка(Зн610);
	
	Если Зн600 >= Зн610 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
	...
	Для Стр = 30 По 31 Цикл 
		СтрСтр = "" + (Стр*10);
		ЗнГр9 = ПолеТабличногоДокументаФормаОтчета.Области["П03"+СтрСтр+"09"].Значение;
		ЗнГр10 = ПолеТабличногоДокументаФормаОтчета.Области["П03"+СтрСтр+"10"].Значение;
		
		ТекстФормулы = "по стр." + СтрСтр + " гр.9 >= гр.10";
		Описание = "по стр." + СтрСтр + " гр.9 < гр.10";
		Расшифровка = Строка(ЗнГр9) + " >= " + Строка(ЗнГр10);
		
		Если ЗнГр9 >= ЗнГр10 Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
			ЭтоОшибка = Истина;
	...
	Для Стр = 3 По 5 Цикл 
		Для Гр = 5 По 11 Цикл 
			Если Стр >= 4 И Гр >= 6 Тогда 
				Продолжить;
			КонецЕсли;
			
			ГрСтр = Прав("0" + Гр, 2);
			Зн200 = ПолеТабличногоДокументаФормаОтчета.Области["П03"+Стр+"00"+ГрСтр].Значение;
			Зн210 = ПолеТабличногоДокументаФормаОтчета.Области["П03"+Стр+"10"+ГрСтр].Значение;
			
			ТекстФормулы = "стр."+Стр+"00 = 0 <=> стр."+Стр+"10=0 по гр." + Гр;
			Описание = "стр."+Стр+"00 = 0 <=> стр."+Стр+"10=0 по гр." + Гр;
			Расшифровка = Строка(Зн200) + "," + Строка(Зн210);
