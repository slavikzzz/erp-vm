...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "03/07/2013";
...
Процедура ПриОткрытии(Отказ)
	...
	// Дополнительно, для Формы № 65-автотранс.
	ПолеТабличногоДокументаФормаОтчета.Области.ГодОтчетаВЗаголовокРазделаVI.Значение  = "за январь-декабрь " + Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг'") + " года";
	ПолеТабличногоДокументаФормаОтчета.Области.ГодОтчетаВЗаголовокРазделаVII.Значение = "за январь-декабрь " + Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг'") + " года";
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПараметрыВыгрузки["ПД00003000103"] <> ПараметрыВыгрузки["ПД00102000405"] Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
	...
	Если ПараметрыВыгрузки["ПД00003000203"] <> ПараметрыВыгрузки["ПД00102000406"] Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
	...
	Если ПараметрыВыгрузки["ПД00003000303"] <> ПараметрыВыгрузки["ПД00102000408"] Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
	...
	Если ПараметрыВыгрузки["ПД00003000403"] <> ПараметрыВыгрузки["ПД00001000205"] Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
...
Функция СформироватьСтруктуруПараметров()
	...
	// Для ПОСЛЕДУЮЩЕГО (ПЕРЕД ВЫГРУЗКОЙ) контроля равенств
	// (согласно информации в графе 1 Раздела III).
	ПараметрыВыгрузки.Вставить("ПД00003000103", ТабДокумент["П000003000103"]);
	ПараметрыВыгрузки.Вставить("ПД00003000203", ТабДокумент["П000003000203"]);
	ПараметрыВыгрузки.Вставить("ПД00003000303", ТабДокумент["П000003000303"]);
	ПараметрыВыгрузки.Вставить("ПД00003000403", ТабДокумент["П000003000403"]);
	
	ПараметрыВыгрузки.Вставить("ПД00102000405", ТабДокумент["П000102000405"]);
	ПараметрыВыгрузки.Вставить("ПД00102000406", ТабДокумент["П000102000406"]);
	ПараметрыВыгрузки.Вставить("ПД00102000408", ТабДокумент["П000102000408"]);
	ПараметрыВыгрузки.Вставить("ПД00001000205", ТабДокумент["П000001000205"]);
