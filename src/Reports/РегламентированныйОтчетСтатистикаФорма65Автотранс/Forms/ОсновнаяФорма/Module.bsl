...
Процедура ИзменитьПериод(Форма, Шаг)
	...
	Если Шаг = 1 Тогда
		Если Форма.мДатаКонцаПериодаОтчета > Дата("20141010") Тогда 
			Сдвиг = 12;
		Иначе
			Сдвиг = 3;
		КонецЕсли;
	ИначеЕсли Шаг = -1 Тогда 
		Если НачалоДня(Форма.мДатаКонцаПериодаОтчета) > НачалоДня(Дата("20141231")) Тогда 
			Сдвиг = -12;
		Иначе
			Сдвиг = -3;
		КонецЕсли;
	...
	Форма.мДатаКонцаПериодаОтчета  = КонецКвартала(ДобавитьМесяц(Форма.мДатаКонцаПериодаОтчета, Сдвиг));
