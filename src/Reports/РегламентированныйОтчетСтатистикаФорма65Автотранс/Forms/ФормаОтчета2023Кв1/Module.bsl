...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Области["П001001000203"].Значение
		= Области["П002001001103"].Значение
		+ Области["П002001001203"].Значение
		+ Области["П002001001303"].Значение;
	
	//гр. 3 равна сумме гр. 4-7 по стр. 21, 22, 23, 24, 25, 26
	Для Стр = 21 По 26 Цикл
		СуммаПоГрафам = 0;
		Для Гр = 4 По 7 Цикл
			СуммаПоГрафам = СуммаПоГрафам + Области["П00300100" + Стр + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00300100" + Стр + "03"].Значение = СуммаПоГрафам;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П001001000203"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001000203"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	Для Стр = 21 По 26 Цикл
		мФормаОтчета.Области["П00300100" + Стр + "03"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00300100" + Стр + "03"].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
