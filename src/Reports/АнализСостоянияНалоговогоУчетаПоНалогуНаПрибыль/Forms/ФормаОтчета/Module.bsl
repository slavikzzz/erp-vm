...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьНадписьПоСтатусамРегламентныхОпераций(Истина);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗавершениеРаботы
	   ...
	   И НЕ ЗавершитьРаботуСОтчетом Тогда	
		...
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Завершить работу с отчетом?';
										|en = 'Close the report?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура НачалоПериодаПриИзменении(Элемент)
	...
	ОпределитьВедетсяУчетПостоянныхИВременныхРазниц();
	УстановитьНадписьПоСтатусамРегламентныхОпераций();
	ПодключитьОбработчикОжиданияСтатусРегламентныхЗаданий();
...
Процедура РезультатСхемыВыбор(Элемент)
	...
	Если Отчет.ИмяРасшифровки = "ТекстДанныеНеАктуальны" Тогда
		Если ВедетсяУчетПостоянныхИВременныхРазниц Тогда
			ПараметрыЗакрытияМесяца = Новый Структура("ПериодРегистрации, Организация", НачалоМесяца(ПериодНеактуальностиРеглЗаданий), Отчет.Организация);
...
Процедура СформироватьРасшифровку(УправляющаяСтруктура)
	...
	РезультатВыполнения = ОбновитьТаблицуНаСервере(УправляющаяСтруктура);
	Если РезультатВыполнения = Неопределено Тогда
		Возврат;
	ИначеЕсли РезультатВыполнения.ЗаданиеВыполнено Тогда
		ОбновитьСтраницы(ЭтаФорма, УправляющаяСтруктура);	
...
&НаСервере
Процедура ОпределитьВедетсяУчетПостоянныхИВременныхРазниц()
	
	ВедетсяУчетПостоянныхИВременныхРазниц =
			УчетнаяПолитика.ВедетсяУчетПостоянныхИВременныхРазниц(Отчет.Организация, Отчет.НачалоПериода);
...
&НаСервере
Функция ОбновитьТаблицуНаСервере(УправляющаяСтруктура)
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьСтраницы(ФормаКлиентскогоПриложения, УправляющаяСтруктура)
	
	Элементы = ФормаКлиентскогоПриложения.Элементы;
...
Процедура УстановитьЗаголовокФормы(Заголовок, ПредставлениеПериода, ПредставлениеОрганизации)
	...
	Заголовок = СправкиРасчетыКлиентСервер.ТекстЗаголовка(
		НСтр("ru = 'Анализ учета по налогу на прибыль';
			|en = 'Profit tax accounting analysis'"),
...
#Область СтатусРегламентныхЗаданий

&НаСервере
Процедура УстановитьНадписьПоСтатусамРегламентныхОпераций(ПриОткрытии = Ложь)
	
	Если РезультатСхемы.ЭлементыГрафическойСхемы.Найти("ТекстДанныеНеАктуальны") = Неопределено Тогда
		Возврат;
	КонецЕсли;

	НадписьДанныеНеАктуальны = РезультатСхемы.ЭлементыГрафическойСхемы.ТекстДанныеНеАктуальны;
	
	Если ПриОткрытии = Истина Тогда
		ОпределитьВедетсяУчетПостоянныхИВременныхРазниц();
	КонецЕсли;
	
	НадписьДанныеНеАктуальны.Наименование = НСтр("ru = 'Проверка выполнения регламентных операций закрытия месяца';
												|en = 'Check period-end operations of month-end closing'");
	НадписьДанныеНеАктуальны.Гиперссылка = Ложь;
	НадписьДанныеНеАктуальны.Картинка = БиблиотекаКартинок.ДлительнаяОперация16;
	...
	Если ЗначениеЗаполнено(Отчет.Организация) 
		...
		ПараметрыФормирования.Вставить("Организация", Отчет.Организация);
		ПараметрыФормирования.Вставить("НачалоПериода", Отчет.НачалоПериода);
		ПараметрыФормирования.Вставить("КонецПериода", Отчет.КонецПериода);
		...
		РезультатРасчета = ДлительныеОперации.ВыполнитьВФоне("Отчеты.АнализСостоянияНалоговогоУчетаПоНалогуНаПрибыль.ОпределитьСтатусРегламентныхЗаданийФоновоеЗадание",
																ПараметрыЗадания,
																ПараметрыВыполнения);
		...
		Если РезультатРасчета.Статус = "Выполнено" Тогда
			ОбновитьСтатусРегламентныхЗаданийЗавершение();
		КонецЕсли;
...
&НаСервере
Процедура ОбновитьСтатусРегламентныхЗаданийЗавершение()
	...
	СтатусРегламентныхЗаданий = ПолучитьИзВременногоХранилища(АдресХранилищаСтатусРегламентныхЗаданий);
	НадписьДанныеНеАктуальны.Картинка = БиблиотекаКартинок.Пустая;
	
	Если СтатусРегламентныхЗаданий.СостояниеФинансовыйРезультат = Перечисления.СостоянияОперацийЗакрытияМесяца.ВыполненоСОшибками
		Или СтатусРегламентныхЗаданий.СостояниеФинансовыйРезультат = Перечисления.СостоянияОперацийЗакрытияМесяца.НеВыполнено
		Или СтатусРегламентныхЗаданий.СостояниеФинансовыйРезультат = Перечисления.СостоянияОперацийЗакрытияМесяца.ОжидаетВыполненияПредыдущихОпераций Тогда
		
		ПериодНеактуальностиРеглЗаданий = СтатусРегламентныхЗаданий.Период;
		НадписьДанныеНеАктуальны.Наименование = 
		СтрШаблон(НСтр("ru = 'Данные отчета могут быть не актуальны, так как не выполнены регламентные операции за %1';
						|en = 'Report data may be irrelevant as the period-end operations for %1 were not executed'"), 
					Формат(СтатусРегламентныхЗаданий.Период, "ДФ='MMММ yyyy'"));
		НадписьДанныеНеАктуальны.Гиперссылка = Истина;
		
	Иначе
		
		ПериодНеактуальностиРеглЗаданий = Отчет.НачалоПериода;
		НадписьДанныеНеАктуальны.Наименование = НСтр("ru = ' '; en = ' '");
		НадписьДанныеНеАктуальны.Гиперссылка = Ложь;
