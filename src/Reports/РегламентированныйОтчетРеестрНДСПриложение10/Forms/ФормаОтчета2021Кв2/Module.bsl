...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр р.НДС прилож.10 21к2") Тогда
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	ИменаГраф.Добавить("П1100028");
	ИменаГраф.Добавить("П1100009");
	ИменаГраф.Добавить("П1100029");
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 8) = "П1100007"
		ИЛИ Лев(Область.Имя, 8) = "П1100028"
		ИЛИ Лев(Область.Имя, 8) = "П1100029" Тогда
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрМор_10");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRMORNDS"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRMORNDS.2"));
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				ВидТрДок = СокрЛП(Ветка_П11000.Данные["П1100002"]);
				
				ДлинаВидТрДок = СтрДлина(ВидТрДок);
				Если ДлинаВидТрДок <> 2 Тогда
					...
					+ " задан вид транспортного, товаросопроводительного и (или) иного документа",
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100004"]) Тогда
					...
					+ " не задана дата транспортного, товаросопроводительного и (или) иного документа",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100006"]) Тогда
					...
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если ЗначениеЗаполнено(Ветка_П11000.Данные["П1100007"]) Тогда
					ДлинаСтранРегСудн = СтрДлина(СокрЛП(Ветка_П11000.Данные["П1100007"]));
					Если ДлинаСтранРегСудн <> 2 Тогда
						ИмяПок = "П1100007" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " неверно задана страна регистрации судна (флаг)",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100008"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование порта погрузки", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ДлинаСтрПортПогруз = СтрДлина(СокрЛП(Ветка_П11000.Данные["П1100028"]));
				Если ДлинаСтрПортПогруз <> 2 Тогда
					ИмяПок = "П1100028" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " не" + ?(ДлинаСтрПортПогруз = 0, "", "верно")
					+ " задан код страны порта погрузки", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100009"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование порта разгрузки", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ДлинаСтрПортРазгруз = СтрДлина(СокрЛП(Ветка_П11000.Данные["П1100029"]));
				Если ДлинаСтрПортРазгруз <> 2 Тогда
					ИмяПок = "П1100029" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " не" + ?(ДлинаСтрПортРазгруз = 0, "", "верно")
					+ " задан код страны порта разгрузки", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ВыпУсловие = (ВидТрДок = "06");
				Если ВыпУсловие Тогда
					...
					КонецЕсли;
				КонецЕсли;
				ВыпУсловие = (Ветка.Данные["КодОперации"] = "1010433" ИЛИ Ветка.Данные["КодОперации"] = "1010482"
						   ИЛИ Ветка.Данные["КодОперации"] = "1010483" ИЛИ Ветка.Данные["КодОперации"] = "1010484")
						   И (СокрЛП(Ветка_П11000.Данные["П1100029"]) = "RU");
				Если ВыпУсловие Тогда
					Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100011"]) Тогда
						...
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " не задана дата отметки российского таможенного органа (при ввозе)",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
				...
				Для каждого Ветка_П11100 Из ДанныеМнЧ_П11100 Цикл
					...
					Если ЗначениеЗаполнено(Ветка_П11100.Данные["П1110005"]) Тогда
						ДлинаРегНомТД = СтрДлина(Ветка_П11100.Данные["П1110005"]);
						Если НЕ (ДлинаРегНомТД = 23 ИЛИ (ДлинаРегНомТД >= 25 И ДлинаРегНомТД <= 29)) Тогда
							...
							СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
							+ НомПП + " неверно задан регистрационный номер таможенной декларации (при вывозе)",
							"Реестр", ИмяПок, НомерСтраницы);
							СтатусПроверки = Ложь;
						КонецЕсли;
					КонецЕсли
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрМор_10 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрМор_10");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрМор_10 = СкопироватьУзел(УзелРодитель.Родитель, РеестрМор_10);
		Для каждого Эл Из ЭкзРеестрМор_10.Строки Цикл
		...
		СвОбОпер = ПолучитьПодчиненныйЭлемент(ЭкзРеестрМор_10, "СвОбОпер");
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСвОбОпер = СкопироватьУзел(ЭкзРеестрМор_10, СвОбОпер, ЭкзРеестрМор_10.Строки.Количество() - 1);
				...
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					...
					Для каждого Эл Из ЭкзСвТрДок.Строки Цикл
						Если Эл.Строки.Количество() = 0 И Эл.Код <> "РегНомТД" Тогда
							Если Эл.Код = "НомТрДок" Тогда
								ТекЗнач = СокрЛП(Ветка_П11000.Данные[Эл.Ключ]);
					...
					// 11100.
					РегНомТД = ПолучитьПодчиненныйЭлемент(ЭкзСвТрДок, "РегНомТД");
					ДанныеМнЧ_П11100 = Ветка_П11000.ДанныеМногострочныхЧастей.П11100.Строки;
					Для каждого Ветка_П11100 Из ДанныеМнЧ_П11100 Цикл
						ЭкзРегНомТД = СкопироватьУзел(ЭкзСвТрДок, РегНомТД, ЭкзСвТрДок.Строки.Количество() - 1);
						ВывестиПоказательВXML(ЭкзРегНомТД, Ветка_П11100.Данные[ЭкзРегНомТД.Ключ]);
					КонецЦикла;
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_10");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_10_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Для каждого Строка11 Из Строки11 Цикл
					...
					// Особая обработка для графы 8.
					МасТекЗнач = Новый Массив;
					Если ЗначениеЗаполнено(Строка11.Данные.П1100008) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100008));
					КонецЕсли;
					Если ЗначениеЗаполнено(Строка11.Данные.П1100028) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100028));
					КонецЕсли;
					СтрокаДокумент.Параметры.П1100008 = СтрСоединить(МасТекЗнач, "; ");
					
					// Особая обработка для графы 9.
					МасТекЗнач = Новый Массив;
					Если ЗначениеЗаполнено(Строка11.Данные.П1100009) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100009));
					КонецЕсли;
					Если ЗначениеЗаполнено(Строка11.Данные.П1100029) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100029));
					КонецЕсли;
					СтрокаДокумент.Параметры.П1100009 = СтрСоединить(МасТекЗнач, "; ");
					...
					Для каждого Строка111 Из Строки111 Цикл
						Если ЗначениеЗаполнено(Строка111.Данные.П1110005) Тогда
							МасТекЗнач.Добавить(СокрЛП(Строка111.Данные.П1110005));
						КонецЕсли;
					КонецЦикла;
					СтрокаДокумент.Параметры.П1110005 = СтрСоединить(МасТекЗнач, "; ");
				...
				Если ЗначениеЗаполнено(ОблОбъедСтрока1_Верх) И ЗначениеЗаполнено(ОблОбъедСтрока1_Низ)
					...
					ОблОбъед.Объединить();
					Для НомерКолонки = 13 По 14 Цикл
						ОблОбъед = ПечФорма1.Область(ОблОбъедСтрока1_Верх, НомерКолонки, ОблОбъедСтрока1_Низ, НомерКолонки);
						ОблОбъед.Объединить();
					КонецЦикла;
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Реестр НДС Приложение 10");
...
&НаСервере
Процедура ПроверкаКС_10_1_1()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '10.1.1';
													|en = '10.1.1'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 10';
														|en = 'Реестр 10'"));
...
&НаСервере
Процедура ПроверкаКС_10_1_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '10.1.2';
													|en = '10.1.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 10';
														|en = 'Реестр 10'"));
...
&НаСервере
Процедура ПроверкаКС_10_1_3()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '10.1.3';
													|en = '10.1.3'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 10';
														|en = 'Реестр 10'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны регистрации судна в гр.7 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны регистрации судна в гр.7 Реестра должен соответствовать справочнику ОКСМ'"));
	...
	ПроверкаКС_10_1_5_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_10_1_4()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '10.1.4';
													|en = '10.1.4'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 10';
														|en = 'Реестр 10'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны порта погрузки в гр.8 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны порта погрузки в гр.8 Реестра должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Код страны %1 соответствует справочнику ОКСМ';
													|en = 'Код страны %1 соответствует справочнику ОКСМ'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный код страны';
														|en = 'Ошибочный код страны'"));
	...
	Настройки.Вставить("ИмяПоказателя", "П1100028_");
	ПроверкаКС_10_1_5_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_10_1_5()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '10.1.5';
													|en = '10.1.5'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 10';
														|en = 'Реестр 10'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны порта разгрузки в гр.9 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны порта разгрузки в гр.9 Реестра должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Код страны %1 соответствует справочнику ОКСМ';
													|en = 'Код страны %1 соответствует справочнику ОКСМ'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный код страны';
														|en = 'Ошибочный код страны'"));
	...
	Настройки.Вставить("ИмяПоказателя", "П1100029_");
	ПроверкаКС_10_1_5_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_10_1_5_код(Настройки)
...
&НаСервере
Процедура ПроверкаКС_10_1_6()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '10.1.6';
													|en = '10.1.6'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 10';
														|en = 'Реестр 10'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Дата отметки таможенного органа в гр.10 Реестра должна быть заполнена, если произведен вывоз ';
			|en = 'Дата отметки таможенного органа в гр.10 Реестра должна быть заполнена, если произведен вывоз '") +
		НСтр("ru = 'товаров (значение порта погрузки в гр.8 Реестра = RU), и вывоз выполнен морским, речным ';
			|en = 'товаров (значение порта погрузки в гр.8 Реестра = RU), и вывоз выполнен морским, речным '") +
		НСтр("ru = 'транспортом (значение кода вида товаросопроводительного документа в гр.2 Реестра = 06)';
			|en = 'транспортом (значение кода вида товаросопроводительного документа в гр.2 Реестра = 06)'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001", "П1100010_");
	ИменаПоказателей.Вставить("Р01Г002", "П1100028_");
	ИменаПоказателей.Вставить("Р01Г003", "П1100002_");
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			Для Счетчик2 = 1 По СтраницаРеестра["КолвоСтрокП11000_" + XMLСтрока(Счетчик)]  Цикл
				...
				Настройки.ЭтоОшибка = Показатели.Р01Г003.Значение = "06" И ВРег(Показатели.Р01Г002.Значение) = "RU" 
...
&НаСервере
Процедура ПроверкаКС_10_1_7()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '10.1.7';
													|en = '10.1.7'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 10';
														|en = 'Реестр 10'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Дата отметки таможенного органа в гр.11 Реестра должна быть заполнена, если произведен ввоз ';
			|en = 'Дата отметки таможенного органа в гр.11 Реестра должна быть заполнена, если произведен ввоз '") +
		НСтр("ru = 'товаров (значение порта разгрузки в гр.9 Реестра = RU), и код операции = 1010433, 1010482, ';
			|en = 'товаров (значение порта разгрузки в гр.9 Реестра = RU), и код операции = 1010433, 1010482, '") +
		НСтр("ru = '1010483, 1010484';
			|en = '1010483, 1010484'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Дата отметки таможенного органа %1 заполнена';
													|en = 'Дата отметки таможенного органа %1 заполнена'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не заполнена дата отметки таможенного органа';
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001", "П1100011_");
	ИменаПоказателей.Вставить("Р01Г002", "П1100029_");
	ИменаПоказателей.Вставить("Р01С001", "КодОперации");
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		...
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			Для Счетчик2 = 1 По СтраницаРеестра["КолвоСтрокП11000_" + XMLСтрока(Счетчик)]  Цикл
				...
				Настройки.ЭтоОшибка = ВРег(Показатели.Р01Г002.Значение) = "RU" И ЗначениеЗаполнено(Показатели.Р01С001.Значение)
					И НЕ СтрНайти("1010433,1010482,1010483,1010484", Показатели.Р01С001.Значение) = 0 
					И НЕ ЗначениеЗаполнено(Показатели.Р01Г001.Значение);
				Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, Показатели.Р01Г001.ЗначениеСтр);
				Настройки.ДопСведения = Дополнение;
				ДополнитьСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверкаКС_10_2_1а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '10.2.1.а';
													|en = '10.2.1.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 10';
														|en = 'Реестр 10'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_10_2_1б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '10.2.1.б';
													|en = '10.2.1.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 10';
														|en = 'Реестр 10'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_10_2_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '10.2.2';
													|en = '10.2.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 10';
														|en = 'Реестр 10'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
