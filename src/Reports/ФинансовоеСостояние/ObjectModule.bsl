...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(ПараметрПериодичность.Значение) Тогда
		ВызватьИсключение НСтр("ru = 'Не заполнен параметр ""Периодичность""';
								|en = 'Parameter ""Frequency"" is not filled in'");
	КонецЕсли;
	...
	Если МакетКомпоновки.НаборыДанных.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Отчет не сформирован. Включите хотя бы одну группировку или таблицу.';
								|en = 'Report is not generated. Enable at least one grouping or table.'");
	КонецЕсли;
...
Функция ТекстЗапросаДенежныеСредства()
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСтатейУправленческогоБаланса.Актив) КАК ТипПоказателя,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ПрочиеАктивы) КАК Показатель,
	|	
	|	ДенежныеДокументы.Организация,
	|	ВЫБОР КОГДА &ВалютаОтчета = ДенежныеДокументы.Организация.ВалютаРегламентированногоУчета ТОГДА
	|		ДенежныеДокументы.Организация.ВалютаРегламентированногоУчета
	|	КОГДА &ВалютаОтчета = &ВалютаУправленческогоУчета ТОГДА
	|		&ВалютаУправленческогоУчета
	|	ИНАЧЕ
	|		ДенежныеДокументы.ДенежныйДокумент.Валюта
	|	КОНЕЦ КАК Валюта,
	|	ДенежныеДокументы.МОЛ КАК Партнер,
	|	ДенежныеДокументы.ДенежныйДокумент КАК Аналитика,
	|	ДенежныеДокументы.Подразделение КАК Подразделение,
	|	
	|	ДенежныеДокументы.ПериодСекунда КАК ПериодСекунда,
	...
	|	
	|	ВЫБОР КОГДА ДенежныеДокументы.Регистратор = НЕОПРЕДЕЛЕНО ТОГДА
	|		NULL
	|	ИНАЧЕ
	|		ДенежныеДокументы.Регистратор
	|	КОНЕЦ КАК Регистратор,
	|	
	|	ВЫБОР КОГДА &ВалютаОтчета = ДенежныеДокументы.Организация.ВалютаРегламентированногоУчета ТОГДА
	...
	|	ИНАЧЕ
	|		ДенежныеДокументы.СуммаНачальныйОстаток
	|	КОНЕЦ КАК СуммаНачальныйОстаток,
	|
	|	ВЫБОР КОГДА &ВалютаОтчета = ДенежныеДокументы.Организация.ВалютаРегламентированногоУчета ТОГДА
	...
	|	ИНАЧЕ
	|		ДенежныеДокументы.СуммаКонечныйОстаток
	|	КОНЕЦ КАК СуммаКонечныйОстаток
	|ИЗ
	|	РегистрНакопления.ДенежныеДокументы.ОстаткиИОбороты({&НачалоПериода}, {&КонецПериода}, Авто, Движения) КАК ДенежныеДокументы
	//-- НЕ УТ
...
Процедура УстановитьОбязательныеНастройки(ПользовательскиеНастройкиМодифицированы)
	...
	Если ПараметрСтатьиПрочихАктивовПассивов <> Неопределено Тогда
		...
		//++ НЕ УТ
		СтатьиПрочихАктивовПассивов.Добавить(ПланыВидовХарактеристик.СтатьиАктивовПассивов.ОсновныеСредства);
		СтатьиПрочихАктивовПассивов.Добавить(ПланыВидовХарактеристик.СтатьиАктивовПассивов.Лизинг);
		//-- НЕ УТ
