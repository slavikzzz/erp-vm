...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	//стр. 100 равна сумме стр. 101÷107 по гр. 3, 4
	Для Гр = 3 По 4 Цикл
		Сумм = 0;
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		Для Стр = 101 По 107 Цикл
			Сумм = Сумм + Области["П0010010" + Формат(Стр, "ЧЦ=3; ЧВН=") + Графа].Значение;
		КонецЦикла;
		Области["П0010010100" + Графа].Значение = Сумм;
	КонецЦикла;
	
	//гр. 3 равна сумме гр. 4÷6 по стр. 210, 211, 240, 241
	Области.П002001021003.Значение
		= Области.П002001021004.Значение
		+ Области.П002001021005.Значение
		+ Области.П002001021006.Значение;
	
	Области.П002001024003.Значение
		= Области.П002001024004.Значение
		+ Области.П002001024005.Значение
		+ Области.П002001024006.Значение;
	
	Области.П002001024103.Значение
		= Области.П002001024104.Значение
		+ Области.П002001024105.Значение
		+ Области.П002001024106.Значение;
	
	//стр. 211 равна сумме стр. 212÷ 218, 220÷226 по гр. 3, 6, 7
	Для Гр = 3 По 7 Цикл
		...
		Сумм = 0;
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		Для Стр = 212 По 226 Цикл
			Если Стр = 219 Тогда
				Продолжить;
			КонецЕсли;
			Сумм = Сумм + Области["П0020010" + Формат(Стр, "ЧЦ=3; ЧВН=") + Графа].Значение;
		КонецЦикла;
		Области["П0020010211" + Графа].Значение = Сумм;
	КонецЦикла;
	
	Области.П002001021103.Значение
		= Области.П002001021104.Значение
		+ Области.П002001021105.Значение
		+ Области.П002001021106.Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П001001010003"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001010003"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П001001010004"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001010004"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П002001021003"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П002001021003"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П002001021103"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П002001021103"].Защита   = Не ОтклАвтоРасчетЗначение;

	мФормаОтчета.Области["П002001021106"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П002001021106"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П002001021107"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П002001021107"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П002001024003"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П002001024003"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П002001024103"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П002001024103"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",   "Форма 1-ТР (автотранспорт)");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(мДанныеОтчета.ПолеТабличногоДокументаФормаОтчета.ОргАдрес) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
