...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	// Стр130 равна сумме строк с 131 по 134.
	Для НомКол = 3 По 5 Цикл
		ТекСумма = 0;
		Постфикс = "0" + Формат(НомКол, "ЧЦ=1; ЧВН=; ЧГ=");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Для НомКол = 3 По 5 Цикл
		...
		Знач2 = ПараметрыВыгрузки["П00000101090" + Окончание] +
				...
				ПараметрыВыгрузки["П00000101120" + Окончание];
				
		Если Знач1 <> Знач2 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 1 по графе " + НомКол + " значение в строке 100 должно быть равно сумме значений в строках 109-112!",
	...
	// По разделу 3.
	// По строкам 300 и 310.
	// Гр. 3  >= гр.4.
	// Гр. 4  >= гр.5.
	Для НомКол = 3 По 4 Цикл
		Для НомСтр = 30 По 31 Цикл
			Знач1 = ПараметрыВыгрузки["П0000030" + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + "00" + Формат(НомКол, "ЧЦ=1; ЧВН=; ЧГ=") + "_Ч"];
			Знач2 = ПараметрыВыгрузки["П0000030" + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + "00" + Формат(НомКол+1, "ЧЦ=1; ЧВН=; ЧГ=") + "_Ч"];
			Если Знач1 < Знач2 Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: в разделе 3 в строке " + 10*НомСтр +  " значение в графе " + НомКол + " должно быть больше либо равным значению в графе " + (НомКол+1) + "!",
