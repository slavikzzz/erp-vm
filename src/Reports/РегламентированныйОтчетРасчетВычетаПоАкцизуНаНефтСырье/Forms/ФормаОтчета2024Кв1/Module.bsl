...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	ИменаГраф.Добавить("П00011М102003");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РасчОтнГрКомп");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "NO_OTNPOKGRKOMP"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ЗаполненРаздел(ДеревоРаздела, "Раздел1") Тогда
		...
		ДанныеМнЧ = ДеревоРаздела.Строки[0].ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР11М1];
		Если ЗаполненаМногострочнаяЧасть(ДанныеМнЧ) Тогда
			Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
				...
				ЗначПок = СокрЛП(СтрокаМнЧ.Данные["П00011М102003"]);
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Строка " + НомСтроки + " подраздела 1.1: не задано полное наименование организации",
					"Раздел1", "П00011М102003_" + НомСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ЗначПок = СокрЛП(СтрокаМнЧ.Данные["П00011М103003"]);
				ДлинаЗначПок = СтрДлина(ЗначПок);
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Строка " + НомСтроки + " подраздела 1.1: не задан ИНН организации",
					"Раздел1", "П00011М103003_" + НомСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли ДлинаЗначПок <> 10 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ЗначПок) Тогда
					СообщитьВТС("Строка " + НомСтроки + " подраздела 1: задан ИНН организации, не состоящий из 10 цифр",
					"Раздел1", "П00011М103003_" + НомСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли Лев(ЗначПок, 2) = "00" Тогда
					СообщитьВТС("Строка " + НомСтроки + " подраздела 1: первые 2 цифры в ИНН организации не должны быть равны 00",
					"Раздел1", "П00011М103003_" + НомСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			КонецЦикла;
		Иначе
			СообщитьВТС("Не заполнен подраздел 1.1", "Раздел1", "П00011М102003_1", НомСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, ПараметрыВыгрузки)
	...
	Для каждого Эл Из УзелРодитель.Строки Цикл
		Если Эл.Строки.Количество() = 0 Тогда
			ВывестиПоказательВXML(Эл, Дан[Эл.Ключ]);
		КонецЕсли;
	КонецЦикла;
	
	СведЗначПокНП = ПолучитьПодчиненныйЭлемент(УзелРодитель, "СведЗначПокНП");
	
	ДанныеМнЧ = ДеревоРаздела.Строки[0].ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР11М1];
	Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
		ЭкзСведЗначПокНП = СкопироватьУзел(УзелРодитель, СведЗначПокНП, УзелРодитель.Строки.Количество() - 1);
		Для каждого Эл Из ЭкзСведЗначПокНП.Строки Цикл
			ВывестиПоказательВXML(Эл, СтрокаМнЧ.Данные[Эл.Ключ]);
		КонецЦикла;
	КонецЦикла;
	
	РегламентированнаяОтчетность.УдалитьУзел(СведЗначПокНП);
...
Функция ВыгрузкаОтчета(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РасчетВычетаПоАкцизуНаНефтСырье");
...
Процедура ВывестиНаПечатьДанныеМногоуровневогоРаздела(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	Если ИмяТекРаздела = "Раздел1" Тогда
		...
		ИмяГруппыМнЧ = СтруктураРеквизитовФормы.мИдГруппыР11М1;
		СтрокМнЧНаЛисте = 2;
