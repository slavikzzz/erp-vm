...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
		...
		МодульОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
			ЭтаФорма,
			"БП.Отчет.РО_ПрослеживаемыеТоварыПеремещение",
			ИмяФормы(ИмяФормы),
			,
			НСтр("ru = 'Новости: Уведомление о перемещении товаров, подлежащих прослеживаемости';
				|en = 'News: Notification of transfer of goods to be traced'"),
			Ложь,
...
Процедура ПолучитьСведенияОПоказателяхОтчета()
	...
	ПараметрыОтчета.Вставить("ДатаУведомления", СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
...
Процедура СформироватьСпискиВыбора()
	...
	//Титульный.
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ПризнакУведомления", 1, ,
	"Выбор признака уведомления", КоллекцияСписковВыбора["ПризнакиУведомления"]);
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П00001М100001", 1, ,
...
Процедура ПоказатьПериод()
	...
	ТабличныйДокумент.Области.ДатаУведомления.Значение = СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета;
...
Процедура СохранитьНаКлиентеСВопросом(Автосохранение, Вариант, ВыполняемоеОповещение, ОткрытьФормуВопросаПриИзмененииВидаДокументаНомераКорректировки, ПодобныйОтчетСуществует, КодИФНС, КПП)
	...
	Если ПодобныйОтчетСуществует Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отчет с видом %1 уже существует.
			...
			|en = 'Report with the %1 kind already exists.
			|Save the report with the same kind?'"), ?(Вариант = 0, """Первичный""", """Корректирующий/" + Вариант + """"));
...
Процедура СохранитьНаКлиентеСВопросомПродолжение(Автосохранение, Вариант, ВыполняемоеОповещение, ОткрытьФормуВопросаПриИзмененииВидаДокументаНомераКорректировки, КодИФНС, КПП)
	...
	Иначе
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сохраняется %1...';
																				|en = 'Saving %1...'"), Заголовок), , , БиблиотекаКартинок.Записать);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "П000010000600" ИЛИ СтрНачинаетсяС(Область.Имя, "П00001М400001") Тогда
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел1" Тогда
		...
		ПризнакФЛ = СокрЛП(Форма.ТабличныйДокумент.Области["П000010000100"].Значение);
		
		СекцияНаимОрг = Форма.ТабличныйДокумент.Области["СекцияНаимОрг"];
		СекцияФИО     = Форма.ТабличныйДокумент.Области["СекцияФИО"];
		
		ОблНаимОрг  = Форма.ТабличныйДокумент.Области["П000010000200"];
		ОблФамилия  = Форма.ТабличныйДокумент.Области["П000010000300"];
		ОблИмя      = Форма.ТабличныйДокумент.Области["П000010000400"];
		ОблОтчество = Форма.ТабличныйДокумент.Области["П000010000500"];
		
		Если ЗначениеЗаполнено(ПризнакФЛ) Тогда
			
			СекцияНаимОрг.Видимость = Ложь;
			СекцияФИО.Видимость     = Истина;
			
			ОблНаимОрг.Значение = "";
			
		Иначе
			
			СекцияНаимОрг.Видимость = Истина;
			СекцияФИО.Видимость     = Ложь;
			
			ОблФамилия.Значение  = "";
			ОблИмя.Значение      = "";
			ОблОтчество.Значение = "";
...
Процедура УстановитьЧисловойФорматОбласти(ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел1" Тогда
		...
		Для каждого Обл Из ТабличныйДокумент.Области Цикл
			Если СтрНачинаетсяС(Обл.Имя, "П00001М300007") Тогда
...
Процедура ДобавитьСтроку(Команда)
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1" Тогда
		...
		Если СтрНачинаетсяС(ИмяОбласти, СтруктураРеквизитовФормы.мИдГруппыР1М4) Тогда
			ИмяПервойГрафы = СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей[
			СтруктураРеквизитовФормы.мИдГруппыР1М4]["ИменаГраф"][0];
			Если ТабличныйДокумент.Области.Найти(ИмяПервойГрафы + "_3") <> Неопределено Тогда
				ТекстПредупреждения = НСтр("ru = 'Возможно не более трех кодов государств - членов ЕАЭС, через которые
				|осуществляется транзит товара, подлежащего прослеживаемости (ОКСМ)';
				|en = 'You can enter no more than three codes of the EAEU member states, through which
				|goods subject to traceability are transited (RNCC)'");
			КонецЕсли;
		КонецЕсли;
...
Процедура УдалитьСтроку(Команда)
	...
	Если НЕ СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей.Свойство(ИмяГруппы) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не выбрана удаляемая строка многострочного раздела';
								|en = 'Multiline section line to be deleted is not selected'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Если ИмяРаздела = "Раздел1" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Если СтрЧислоВхождений(СтруктЗависимыхЯчеек["СтоимостьИтого"], ОбластьИмяБезИнд) > 0 Тогда
					
					СтоимостьИтого = 0;
					Для каждого ТекОбл Из ТДО Цикл
						Если СтрНачинаетсяС(ТекОбл.Имя, "П00001М300008_") Тогда
							СтоимостьИтого = СтоимостьИтого + Окр(ТекОбл.Значение, 2);
						КонецЕсли;
					КонецЦикла;
					ТДО["СтоимостьИтого"].Значение = СтоимостьИтого;
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел1" Тогда
		...
		ИмяГруппыПодчМнЧ = Форма.СтруктураРеквизитовФормы.мИдГруппыР1М3;
		СтруктураГруппыПодчМнЧ = Форма.СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей[ИмяГруппыПодчМнЧ];
			...
			Если Ветка = АктивнаяВетка Тогда
				...
				Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
					...
					Для каждого СтрокаПодчМнЧ Из ДеревоПодчМнЧ.Строки Цикл
						ИндПодчСтроки = Формат(ДеревоПодчМнЧ.Строки.Индекс(СтрокаПодчМнЧ) + 1, "ЧГ=");
						Для каждого ИмяГрафы Из ИменаГрафПодчМнЧ Цикл
							СтрокаПодчМнЧ.Данные.Вставить(ИмяГрафы, ТДО[ИмяГрафы + "_" + ИндСтроки + "_" + ИндПодчСтроки].Значение);
						КонецЦикла;
					КонецЦикла;
				КонецЦикла;
			КонецЕсли;
			...
			Если Ветка = АктивнаяВетка Тогда
				...
				Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
					...
					Для каждого СтрокаПодчМнЧ Из ДеревоПодчМнЧ.Строки Цикл
						ИндПодчСтроки = Формат(ДеревоПодчМнЧ.Строки.Индекс(СтрокаПодчМнЧ) + 1, "ЧГ=");
						Для каждого ИмяГрафы Из ИменаГрафПодчМнЧ Цикл
							ТДО[ИмяГрафы + "_" + ИндСтроки + "_" + ИндПодчСтроки].Значение = СтрокаПодчМнЧ.Данные[ИмяГрафы];
						КонецЦикла;
					КонецЦикла;
				КонецЦикла;
			КонецЕсли;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ.Вставить("СтоимостьИтого", "П00001М300008");
...
Функция СравнениеОтчетов(ДанныеДляСравнения) Экспорт
	...
	Если Модифицированность Тогда
		...
		Сообщение.Текст = СтрШаблон(НСтр("ru = 'Отчет ""%1"" не сохранен.';
										|en = 'The ""%1"" report is not saved.'"), ЭтотОбъект.Заголовок);
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Если Открыта() Тогда
		Если СтруктураРеквизитовФормы.ЕдиницаИзмерения
			...
			Сообщение.Текст = НСтр("ru = 'Перед выгрузкой необходимо установить единицу измерения ""в рублях""';
									|en = 'Before exporting, set the ""in rubles"" unit of measure'");
			Сообщение.Сообщить();
			Возврат Ложь;
		КонецЕсли;
		...
		Если СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения <> 0 Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Перед выгрузкой необходимо установить точность ""0""';
									|en = 'Before exporting, set the ""0"" accuracy'");
			Сообщение.Сообщить();
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеАвторасчета <> СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
		Если СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет = Ложь Тогда
			...
			ТекстВопроса = НСтр("ru = 'Будет произведен перерасчет всех рассчитываемых показателей. Продолжить?';
								|en = 'All calculated indicators will be recalculated. Continue?'");
...
Процедура ОчиститьТекущийРаздел(Команда)
	...
	|en = 'Warning! The current section will be cleared.%1Continue?'"), Символы.ПС);
...
Процедура Печать(ВидПечати, НеИзФормыОтчета = Ложь) Экспорт
	...
	|en = '%1. Print form is being generated...'"), Заголовок), , , БиблиотекаКартинок.Печать);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Если НЕ РегламентированнаяОтчетность.ПринтерДоступен() Тогда
		...
		|en = 'Before generating print forms, select a printer and set it as the default one'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Если СтруктураРеквизитовФормы.ЕдиницаИзмерения
		...
		|en = 'You can print only if the set unit of measure is ""in rubles"" and the accuracy is ""0""'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗаполнитьАвтоНаСервере()
	...
	НаименованиеЗадания = НСтр("ru = 'Автоматическое заполнение по данным информационной базы';
								|en = 'Automatic filling by infobase data'");
...
Функция ПараметрыОжидания()
	...
	ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Отчет формируется';
											|en = 'The report is being generated'");
...
Процедура РазделыОтчетаПриАктивизацииСтроки(Элемент)
	...
	Если мСтруктураМногоуровневыхРазделов.Свойство(
		...
		Если мСтруктураМногоуровневыхРазделов[Элемент.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим].Свойство(
			...
			Элементы.ОчиститьТекущийРаздел.Доступность = Истина;
