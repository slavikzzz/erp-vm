...
Процедура ОбработатьРезультирующийТекстВыгрузки(Текст)
	...
	Для Сч = 1 По КолСтрок Цикл
		...
		Если (Тек13Симв = "П000210003000" ИЛИ Тек13Симв = "П000220003000" ИЛИ Тек13Симв = "П000230003000" ИЛИ Тек13Симв = "П000240003000") Тогда
...
Процедура СформироватьРаздел1НаСервере()
	...
	Для Каждого Стр из ТаблДанных Цикл
		...
		Иначе
			...
			Сообщение.Текст = НСтр("ru = 'Не заполнен код КБК или ОКАТО на странице Раздела 2!';
									|en = 'Не заполнен код КБК или ОКАТО на странице Раздела 2!'");
			Сообщение.Сообщить();
	...
	Для Каждого Стр из ТаблДанных Цикл
		...
		Иначе
			...
			Сообщение.Текст = НСтр("ru = 'Не заполнен код КБК или ОКАТО на странице Раздела 22!';
									|en = 'Не заполнен код КБК или ОКАТО на странице Раздела 22!'");
			Сообщение.Сообщить();
			УстановитьДоступностьКнопкиУдалитьСтраницу(ТаблицаСтраницыРаздела1);
			Возврат;
		КонецЕсли;
	...
	Для Каждого Стр из ТаблДанных Цикл
		...
		Иначе
			...
			Сообщение.Текст = НСтр("ru = 'Не заполнен код КБК или ОКАТО на странице Раздела 23!';
									|en = 'Не заполнен код КБК или ОКАТО на странице Раздела 23!'");
			Сообщение.Сообщить();
			УстановитьДоступностьКнопкиУдалитьСтраницу(ТаблицаСтраницыРаздела1);
			Возврат;
		КонецЕсли;
	...
	Для Каждого Стр из ТаблДанных Цикл
		...
		Иначе
			...
			Сообщение.Текст = НСтр("ru = 'Не заполнен код КБК или ОКАТО на странице Раздела 24!';
									|en = 'Не заполнен код КБК или ОКАТО на странице Раздела 24!'");
			Сообщение.Сообщить();
			УстановитьДоступностьКнопкиУдалитьСтраницу(ТаблицаСтраницыРаздела1);
			Возврат;
		КонецЕсли;
	...
	Если ДанныеДляЗаполнения.Количество() = 0 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Для Сбора данных Раздела 1 необходимо предварительно заполнить хотя бы один из разделов 2.1, 2.2, 2.3, 2.4 декларации.';
								|en = 'Для Сбора данных Раздела 1 необходимо предварительно заполнить хотя бы один из разделов 2.1, 2.2, 2.3, 2.4 декларации.'");
		Сообщение.Сообщить();
		УстановитьДоступностьКнопкиУдалитьСтраницу(ТаблицаСтраницыРаздела1);
		Возврат;
	КонецЕсли;
