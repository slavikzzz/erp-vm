...
Процедура ВопросПерерасчетВсехРассчитываемыхПоказателейЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ДопПараметры = Новый Структура(
		"НомерСтраницыРазделаОтчета, ТекущийРазделОтчетаСокрНаим, флЗначенияИдентичны, ЗначениеЕдИзм, ЗначениеЗпт",
		НомерСтраницыРазделаОтчета, ТекущийРазделОтчетаСокрНаим, Ложь, ЗначениеЕдИзм, ЗначениеЗпт);
	ПослеОткрытияФормыВыбораСтраниц(ДопПараметры);
	...
	Если ИмяТекРаздела = "Раздел2" Тогда
		...
		Для НомСтрокиВДереве = 1 По ДеревоДанныхРаздела.Строки.Количество() Цикл
			...
			Для НомСтрокиВВетке21_1 = 1 По Ветка21_1.Строки.Количество() Цикл
				...
				ТабличныйДокумент.Вывести(ОбъектОтчета(ИмяФормы).ПолучитьМакет(Сред(ИмяФормы, СтрНайти(ИмяФормы, "ФормаОтчета")) + "_Раздел2"));
				...
				ТабличныйДокумент.Вывести(ОбъектОтчета(ИмяФормы).ПолучитьМакет(Сред(ИмяФормы, СтрНайти(ИмяФормы, "ФормаОтчета")) + "_Раздел21"));
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		Если НЕ ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСкопированаФорма) Тогда
			...
			ДанныеДопСтрок = ТаблицаСтраницыРаздел1[0].ДанныеДопСтрок[0].Значение;
			Если ДанныеДопСтрок.Количество() = 1 Тогда
				Если СведенияОбОрганизации.Свойство("ОКТМО") Тогда
					ДанныеДопСтрок[0]["П000100002003"] = СокрЛП(СведенияОбОрганизации.ОКТМО);
				КонецЕсли;
			КонецЕсли;
			...
			Если ОтчетСкопированОтчетныйГодИзменился Тогда
					...
					Для каждого Строка21_2 Из Строка21_1.Строки Цикл
						...
						Если ЗначениеЗаполнено(ПредЗначСтавки) И (ЗначСтавки <> ПредЗначСтавки) Тогда
							...
							Если ЗначениеСтавкиОпределяетсяПоОтчетномуГоду Тогда
								...
								НСтр("ru = 'Раздел 2.1 -> Водный объект %1 -> Страница %2: в строке 110 изменено значение с %3 на %4.';
									|en = 'Раздел 2.1 -> Водный объект %1 -> Страница %2: в строке 110 изменено значение с %3 на %4.'"),
						...
						Если ЗначениеЗаполнено(ПредЗначСтавки) И (ЗначСтавки <> ПредЗначСтавки) Тогда
							...
							Если ЗначениеСтавкиОпределяетсяПоОтчетномуГоду Тогда
								...
								НСтр("ru = 'Раздел 2.1 -> Водный объект %1 -> Страница %2: в строке 120 изменено значение с %3 на %4.';
									|en = 'Раздел 2.1 -> Водный объект %1 -> Страница %2: в строке 120 изменено значение с %3 на %4.'"),
								НомВодОбъекта, НомСтраницы, ПредЗначСтавки, ЗначСтавки));
							КонецЕсли;
						КонецЕсли;
						...
						Если ЗначениеЗаполнено(ПредЗначКоэф) И (ЗначКоэф <> ПредЗначКоэф ИЛИ Строка080 = "1") Тогда
							...
							НСтр("ru = 'Раздел 2.1 -> Водный объект %1 -> Страница %2: в строке 130 изменено значение с %3 на %4.';
								|en = 'Раздел 2.1 -> Водный объект %1 -> Страница %2: в строке 130 изменено значение с %3 на %4.'"),
				...
				Для каждого Страница Из ТаблицаСтраницыРаздел22 Цикл
					...
					Если ЗначениеЗаполнено(ПредЗначКоэф) И (ЗначКоэф <> ПредЗначКоэф) Тогда
						...
						НСтр("ru = 'Раздел 2.2 -> Страница %1: в строке 120 изменено значение с %2 на %3.';
							|en = 'Раздел 2.2 -> Страница %1: в строке 120 изменено значение с %2 на %3.'"),
				...
				Если СообщенияОбИзменениях.Количество() > 0 Тогда
					...
					НСтр("ru = 'Установлены актуальные для %1 года значения:';
						|en = 'Установлены актуальные для %1 года значения:'"),
	...
	Если СообщенияОбИзменениях.Количество() > 0 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Произведен перерасчет всех рассчитываемых показателей.';
								|en = 'Произведен перерасчет всех рассчитываемых показателей.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ИмяФормы).ПолучитьМакет("СпискиВыбора2016Кв1");
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = ОбъектОтчета(ИмяФормы).ПолучитьМакет("СхемаВыгрузки501");
...
Процедура ДобавитьСтраницуВРазделе2НаСервере()
	...
	ТабличныйДокумент.Очистить();
	ТабличныйДокумент.Вывести(ОбъектОтчета(ИмяФормы).ПолучитьМакет(Сред(ИмяФормы, СтрНайти(ИмяФормы, "ФормаОтчета")) + "_" + "Раздел21"));
	ВеткаРаздел21.Данные = СобратьДанныеТекущегоТаблПоля(ЭтаФорма, "ТабличныйДокумент");
	ТабличныйДокумент.Очистить();
	ТабличныйДокумент.Вывести(ОбъектОтчета(ИмяФормы).ПолучитьМакет(Сред(ИмяФормы, СтрНайти(ИмяФормы, "ФормаОтчета")) + "_" + "Раздел2"));
	...
	Если Результат <> Неопределено Тогда
		...
		Иначе
			...
			Сообщение.Текст = НСтр("ru = 'Длина значения показателя ""Лицензия на водопользование"" не должна превышать 14 символов.';
									|en = 'Длина значения показателя ""Лицензия на водопользование"" не должна превышать 14 символов.'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	ПроверитьКС_12_1_к(СтруктураРезультата);
	ПроверитьКС_12_2_к(СтруктураРезультата);
...
Процедура ПроверитьКС_1_1(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код бюджетной классификации (строка 010 Раздела 1) должен соответствовать КБК в строке 010 раздела 2.1 или в строке 010 раздела 2.2';
							|en = 'Код бюджетной классификации (строка 010 Раздела 1) должен соответствовать КБК в строке 010 раздела 2.1 или в строке 010 раздела 2.2'");
	
	ОписаниеОшибки  = НСтр("ru = 'Для КБК, указанного в разделе 1 по которому уплачивается налог, не заполнен раздел 2';
							|en = 'Для КБК, указанного в разделе 1 по которому уплачивается налог, не заполнен раздел 2'");
...
Процедура ПроверитьКС_1_2(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код ОКТМО (строка 020 Раздела 1)  должен соответствовать ОКТМО в строке 020 раздела 2.1 или в строке 020 раздела 2.2';
							|en = 'Код ОКТМО (строка 020 Раздела 1)  должен соответствовать ОКТМО в строке 020 раздела 2.1 или в строке 020 раздела 2.2'");
	
	ОписаниеОшибки  = НСтр("ru = 'Для ОКТМО, указанного в разделе 1 по которому уплачивается налог, не заполнен раздел 2';
							|en = 'Для ОКТМО, указанного в разделе 1 по которому уплачивается налог, не заполнен раздел 2'");
...
Процедура ПроверитьКС_2_1(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.1 раздела 2 (КС 2.1)';
							|en = 'Проверка подраздела 2.1 раздела 2 (КС 2.1)'");
	ОписаниеКС      = НСтр("ru = 'Код бюджетной классификации в строке 010 подраздела 2.1 должен соответствовать КБК в строке 010 раздела 1 ';
							|en = 'Код бюджетной классификации в строке 010 подраздела 2.1 должен соответствовать КБК в строке 010 раздела 1 '");
	...
	ОписаниеОшибки  = НСтр("ru = 'КБК, указанный в подразделе 2.1 раздела 2, отсутствует в разделе 1. Либо неверно указан КБК в подразделе 2.1 раздела 2';
							|en = 'КБК, указанный в подразделе 2.1 раздела 2, отсутствует в разделе 1. Либо неверно указан КБК в подразделе 2.1 раздела 2'");
...
Процедура ПроверитьКС_2_2(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.1 раздела 2 (КС 2.2)';
							|en = 'Проверка подраздела 2.1 раздела 2 (КС 2.2)'");
	ОписаниеКС      = НСтр("ru = 'Код ОКТМО в строке 020 подраздела 2.1 должен соответствовать ОКТМО в строке 020 раздела 1';
							|en = 'Код ОКТМО в строке 020 подраздела 2.1 должен соответствовать ОКТМО в строке 020 раздела 1'");
	...
	ОписаниеОшибки  = НСтр("ru = 'ОКТМО, указанный в подразделе 2.1 раздела 2, отсутствует в разделе 1. Либо неверно указан ОКТМО в подразделе 2.1 раздела 2.';
							|en = 'ОКТМО, указанный в подразделе 2.1 раздела 2, отсутствует в разделе 1. Либо неверно указан ОКТМО в подразделе 2.1 раздела 2.'");
...
Процедура ПроверитьКС_2_3(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.1 раздела 2 (КС 2.3)';
							|en = 'Проверка подраздела 2.1 раздела 2 (КС 2.3)'");
	ОписаниеКС      = НСтр("ru = 'Код водопользования в строке 050 подраздела 2.1 раздела 2 должен иметь значение: ""11011"" - ""11015"" или ""11021"" - ""11024"" или ""11031"" - ""11036"" или ""11041"" - ""11043"" или ""11051"" - ""11054"" или ""11061"" - ""11063"" или ""11071"" - ""11076"" или ""11081"" - ""11084"" или  ""11091"" - ""11092"" или ""11101"" - ""11106"" или ""11111"" - ""11122"" или ""12011"" - ""12015"" или ""12021"" - ""12024"" или ""12031"" - ""12036"" или ""12041"" - ""12043"" или ""12051"" - ""12054"" или ""12061"" - ""12063"" или ""12071"" - ""12076"" или ""12081"" - ""12084"" или ""12091"" - ""12092"" или ""12101"" - ""12106"" или ""12111"" - ""12113"" или ""12121"" - ""12122"" или ""13010"" - ""13140"" или ""50091"" - ""50092""';
							|en = 'Код водопользования в строке 050 подраздела 2.1 раздела 2 должен иметь значение: ""11011"" - ""11015"" или ""11021"" - ""11024"" или ""11031"" - ""11036"" или ""11041"" - ""11043"" или ""11051"" - ""11054"" или ""11061"" - ""11063"" или ""11071"" - ""11076"" или ""11081"" - ""11084"" или  ""11091"" - ""11092"" или ""11101"" - ""11106"" или ""11111"" - ""11122"" или ""12011"" - ""12015"" или ""12021"" - ""12024"" или ""12031"" - ""12036"" или ""12041"" - ""12043"" или ""12051"" - ""12054"" или ""12061"" - ""12063"" или ""12071"" - ""12076"" или ""12081"" - ""12084"" или ""12091"" - ""12092"" или ""12101"" - ""12106"" или ""12111"" - ""12113"" или ""12121"" - ""12122"" или ""13010"" - ""13140"" или ""50091"" - ""50092""'");
	
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан код вида водопользования в подразделе 2.1 раздела 2';
							|en = 'Неверно указан код вида водопользования в подразделе 2.1 раздела 2'");
...
Процедура ПроверитьКС_2_4(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.1 раздела 2 (КС 2.4)';
							|en = 'Проверка подраздела 2.1 раздела 2 (КС 2.4)'");
	ОписаниеКС      = НСтр("ru = 'Общий объем воды в строке 060 подраздела 2.1 раздела 2 должен быть больше или равен объему воды в строке 070 подраздела 2.1 раздела 2 или больше или равен объему воды в строке 100 подраздела 2.1 раздела 2';
							|en = 'Общий объем воды в строке 060 подраздела 2.1 раздела 2 должен быть больше или равен объему воды в строке 070 подраздела 2.1 раздела 2 или больше или равен объему воды в строке 100 подраздела 2.1 раздела 2'");
	
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан объем забранной воды в подразделе 2.1 раздела 2';
							|en = 'Неверно указан объем забранной воды в подразделе 2.1 раздела 2'");
...
Процедура ПроверитьКС_2_5(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.1 раздела 2 (КС 2.5)';
							|en = 'Проверка подраздела 2.1 раздела 2 (КС 2.5)'");
	ОписаниеКС      = НСтр("ru = 'Общий объем воды в строке 060 подраздела 2.1 раздела 2 должен быть равен сумме значений строк 070 и 100 подраздела 2.1 раздела 2';
							|en = 'Общий объем воды в строке 060 подраздела 2.1 раздела 2 должен быть равен сумме значений строк 070 и 100 подраздела 2.1 раздела 2'");
...
Процедура ПроверитьКС_2_6(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.1 раздела 2 (КС 2.6)';
							|en = 'Проверка подраздела 2.1 раздела 2 (КС 2.6)'");
	ОписаниеКС      = НСтр("ru = 'Код цели забора воды в строке 080 подраздела 2.1 раздела 2 должен иметь значение равное ""1"", ""2"" или ""3""';
							|en = 'Код цели забора воды в строке 080 подраздела 2.1 раздела 2 должен иметь значение равное ""1"", ""2"" или ""3""'");
	
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан код цели забора воды в подразделе 2.1 раздела 2';
							|en = 'Неверно указан код цели забора воды в подразделе 2.1 раздела 2'");
...
Процедура ПроверитьКС_2_7(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.1 раздела 2 (КС 2.7)';
							|en = 'Проверка подраздела 2.1 раздела 2 (КС 2.7)'");
	ОписаниеКС      = НСтр("ru = 'Объем воды в строке 100 подраздела 2.1 раздела 2 должен быть равен сумме значений строк 101 и 102 подраздела 2.1 раздела 2';
							|en = 'Объем воды в строке 100 подраздела 2.1 раздела 2 должен быть равен сумме значений строк 101 и 102 подраздела 2.1 раздела 2'");
	...
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан объем воды, забранной из водного объекта, признаваемый объектом налогообложения в подразделе 2.1 раздела 2';
							|en = 'Неверно указан объем воды, забранной из водного объекта, признаваемый объектом налогообложения в подразделе 2.1 раздела 2'");
...
Процедура ПроверитьКС_2_8(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.1 раздела 2 (КС 2.8)';
							|en = 'Проверка подраздела 2.1 раздела 2 (КС 2.8)'");
	ОписаниеКС      = НСтр("ru = 'Коэффициент в строке 140 подраздела 2.1 раздела 2 должен иметь значение 1.1, если строка 140 заполнена';
							|en = 'Коэффициент в строке 140 подраздела 2.1 раздела 2 должен иметь значение 1.1, если строка 140 заполнена'");
	
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента в подразделе 2.1 раздела 2';
							|en = 'Допущена ошибка при определении коэффициента в подразделе 2.1 раздела 2'");
...
Процедура ПроверитьКС_2_9(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.1 раздела 2 (КС 2.9)';
							|en = 'Проверка подраздела 2.1 раздела 2 (КС 2.9)'");
	ОписаниеКС      = НСтр("ru = 'Коэффициент в строке 150 подраздела 2.1 раздела 2 должен иметь значение 10 или ""-""';
							|en = 'Коэффициент в строке 150 подраздела 2.1 раздела 2 должен иметь значение 10 или ""-""'");
...
Процедура ПроверитьКС_2_10(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.1 раздела 2 (КС 2.10)';
							|en = 'Проверка подраздела 2.1 раздела 2 (КС 2.10)'");
	ОписаниеКС      = НСтр("ru = 'Сумма налога, подлежащая уплате в строке 160 подраздела 2.1 раздела 2 должна быть равна (строка 101 х округленное значение (строки 110 х строка 130 х строка 140 х строка 150) + строка 102 х округленное значение (строки 120 х строка 130 х строка 140 х строка 150) подраздела 2.1 раздела 2)';
							|en = 'Сумма налога, подлежащая уплате в строке 160 подраздела 2.1 раздела 2 должна быть равна (строка 101 х округленное значение (строки 110 х строка 130 х строка 140 х строка 150) + строка 102 х округленное значение (строки 120 х строка 130 х строка 140 х строка 150) подраздела 2.1 раздела 2)'");
	
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении суммы налога в подразделе 2.1 раздела 2';
							|en = 'Допущена ошибка при определении суммы налога в подразделе 2.1 раздела 2'");
...
Процедура ПроверитьКС_2_11(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.2 раздела 2 (КС 2.11)';
							|en = 'Проверка подраздела 2.2 раздела 2 (КС 2.11)'");
	ОписаниеКС      = НСтр("ru = 'Код бюджетной классификации в строке 010 подраздела 2.2 раздела 2 должен соответствовать КБК в строке 010 раздела 1 ';
							|en = 'Код бюджетной классификации в строке 010 подраздела 2.2 раздела 2 должен соответствовать КБК в строке 010 раздела 1 '");
	...
	ОписаниеОшибки  = НСтр("ru = 'КБК, указанный в подразделе 2.2 раздела 2, отсутствует в разделе 1. Либо неверно указан КБК в подразделе 2.2 раздела 2';
							|en = 'КБК, указанный в подразделе 2.2 раздела 2, отсутствует в разделе 1. Либо неверно указан КБК в подразделе 2.2 раздела 2'");
...
Процедура ПроверитьКС_2_12(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.2 раздела 2 (КС 2.12)';
							|en = 'Проверка подраздела 2.2 раздела 2 (КС 2.12)'");
	ОписаниеКС      = НСтр("ru = 'Код ОКТМО в строке 020 подраздела 2.2 должен соответствовать ОКТМО в строке 020 раздела 1';
							|en = 'Код ОКТМО в строке 020 подраздела 2.2 должен соответствовать ОКТМО в строке 020 раздела 1'");
	...
	ОписаниеОшибки  = НСтр("ru = 'ОКТМО, указанный в подразделе 2.2 раздела 2, отсутствует в разделе 1. Либо неверно указан ОКТМО в подразделе 2.2 раздела 2';
							|en = 'ОКТМО, указанный в подразделе 2.2 раздела 2, отсутствует в разделе 1. Либо неверно указан ОКТМО в подразделе 2.2 раздела 2'");
...
Процедура ПроверитьКС_2_13(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.2 раздела 2 (КС 2.13)';
							|en = 'Проверка подраздела 2.2 раздела 2 (КС 2.13)'");
	ОписаниеКС      = НСтр("ru = 'Код водопользования в строке 060 подраздела 2.2 раздела 2 должен иметь значение: ""21010"" - ""21120"" или ""22010"" - ""22140"" или ""30100"" - ""32100"" или ""40100"" - ""41200"" или ""50091"" - ""50092""';
							|en = 'Код водопользования в строке 060 подраздела 2.2 раздела 2 должен иметь значение: ""21010"" - ""21120"" или ""22010"" - ""22140"" или ""30100"" - ""32100"" или ""40100"" - ""41200"" или ""50091"" - ""50092""'");
	
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан код вида водопользования в подразделе 2.2 раздела 2';
							|en = 'Неверно указан код вида водопользования в подразделе 2.2 раздела 2'");
...
Процедура ПроверитьКС_2_14(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.2 раздела 2 (КС 2.14)';
							|en = 'Проверка подраздела 2.2 раздела 2 (КС 2.14)'");
	ОписаниеКС      = НСтр("ru = 'Строки 080, 090, 100 подраздела 2.2 раздела 2 не заполняются, если код в строке 060 подраздела 2.2 раздела 2 равен ""21010"" - ""22140"" или ""50091"" - ""50092""';
							|en = 'Строки 080, 090, 100 подраздела 2.2 раздела 2 не заполняются, если код в строке 060 подраздела 2.2 раздела 2 равен ""21010"" - ""22140"" или ""50091"" - ""50092""'");
...
Процедура ПроверитьКС_2_15(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.2 раздела 2 (КС 2.15)';
							|en = 'Проверка подраздела 2.2 раздела 2 (КС 2.15)'");
	ОписаниеКС      = НСтр("ru = 'Строки 070, 090, 100 подраздела 2.2 раздела 2 не заполняются, если код в строке 060 подраздела 2.2 раздела 2 равен ""30100"" - ""32100"" или ""50091"" - ""50092""';
							|en = 'Строки 070, 090, 100 подраздела 2.2 раздела 2 не заполняются, если код в строке 060 подраздела 2.2 раздела 2 равен ""30100"" - ""32100"" или ""50091"" - ""50092""'");
...
Процедура ПроверитьКС_2_16(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.2 раздела 2 (КС 2.16)';
							|en = 'Проверка подраздела 2.2 раздела 2 (КС 2.16)'");
	ОписаниеКС      = НСтр("ru = 'Строки 070, 080 подраздела 2.2 раздела 2 не заполняются, если код в строке 060 подраздела 2.2 раздела 2 равен ""40100"" - ""41200"" или ""50091"" - ""50092""';
							|en = 'Строки 070, 080 подраздела 2.2 раздела 2 не заполняются, если код в строке 060 подраздела 2.2 раздела 2 равен ""40100"" - ""41200"" или ""50091"" - ""50092""'");
...
Процедура ПроверитьКС_2_17(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.2 раздела 2 (КС 2.17)';
							|en = 'Проверка подраздела 2.2 раздела 2 (КС 2.17)'");
	ОписаниеКС      = НСтр("ru = 'Площадь предоставленного водного пространства (строка 070 подраздела 2.2 раздела 2) должна иметь значение больше или равное нулю, если код в строке 060 подраздела 2.2 раздела 2 равен ""21010"" - ""22140""';
							|en = 'Площадь предоставленного водного пространства (строка 070 подраздела 2.2 раздела 2) должна иметь значение больше или равное нулю, если код в строке 060 подраздела 2.2 раздела 2 равен ""21010"" - ""22140""'");
...
Процедура ПроверитьКС_2_18(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.2 раздела 2 (КС 2.18)';
							|en = 'Проверка подраздела 2.2 раздела 2 (КС 2.18)'");
	ОписаниеКС      = НСтр("ru = 'Количество произведенной электроэнергии (строка 080 подраздела 2.2 раздела 2) должно иметь значение больше или равное нулю, если код в строке 060 подраздела 2.2 раздела 2 равен ""30100"" - ""32100""';
							|en = 'Количество произведенной электроэнергии (строка 080 подраздела 2.2 раздела 2) должно иметь значение больше или равное нулю, если код в строке 060 подраздела 2.2 раздела 2 равен ""30100"" - ""32100""'");
...
Процедура ПроверитьКС_2_19(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.2 раздела 2 (КС 2.19)';
							|en = 'Проверка подраздела 2.2 раздела 2 (КС 2.19)'");
	ОписаниеКС      = НСтр("ru = 'Объем древесины (строка 090 подраздела 2.2 раздела 2) должен иметь значение больше или равное нулю, если код в строке 060 подраздела 2.2 раздела 2 равен ""40100"" - ""41200""';
							|en = 'Объем древесины (строка 090 подраздела 2.2 раздела 2) должен иметь значение больше или равное нулю, если код в строке 060 подраздела 2.2 раздела 2 равен ""40100"" - ""41200""'");
...
Процедура ПроверитьКС_2_20(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.2 раздела 2 (КС 2.20)';
							|en = 'Проверка подраздела 2.2 раздела 2 (КС 2.20)'");
	ОписаниеКС      = НСтр("ru = 'Расстояние сплава (строка 100 подраздела 2.2 раздела 2) должно иметь значение больше или равное нулю, если код в строке 060 подраздела 2.2 раздела 2 равен ""40100"" - ""41200""';
							|en = 'Расстояние сплава (строка 100 подраздела 2.2 раздела 2) должно иметь значение больше или равное нулю, если код в строке 060 подраздела 2.2 раздела 2 равен ""40100"" - ""41200""'");
...
Процедура ПроверитьКС_2_21(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.2 раздела 2 (КС 2.21)';
							|en = 'Проверка подраздела 2.2 раздела 2 (КС 2.21)'");
	ОписаниеКС      = НСтр("ru = 'Сумма налога, подлежащая уплате в строке 130 подраздела 2.2 раздела 2 должна быть равна (строка 070 х округленное значение (строки 110 х строка 120) / 4, если код в строке 060 подраздела 2.2 раздела 2 равен ""21010"" - ""22140""';
							|en = 'Сумма налога, подлежащая уплате в строке 130 подраздела 2.2 раздела 2 должна быть равна (строка 070 х округленное значение (строки 110 х строка 120) / 4, если код в строке 060 подраздела 2.2 раздела 2 равен ""21010"" - ""22140""'");
	
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении суммы налога в подразделе 2.2 раздела 2';
							|en = 'Допущена ошибка при определении суммы налога в подразделе 2.2 раздела 2'");
...
Процедура ПроверитьКС_2_22(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.2 раздела 2 (КС 2.22)';
							|en = 'Проверка подраздела 2.2 раздела 2 (КС 2.22)'");
	ОписаниеКС      = НСтр("ru = 'Сумма налога, подлежащая уплате в строке 130 подраздела 2.2 раздела 2 должна быть равна (строка 080 х округленное значение (строки 110 х строка 120), если код в строке 060 подраздела 2.2 раздела 2 равен ""30100"" - ""32100""';
							|en = 'Сумма налога, подлежащая уплате в строке 130 подраздела 2.2 раздела 2 должна быть равна (строка 080 х округленное значение (строки 110 х строка 120), если код в строке 060 подраздела 2.2 раздела 2 равен ""30100"" - ""32100""'");
...
Процедура ПроверитьКС_2_23(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Проверка подраздела 2.2 раздела 2 (КС 2.23)';
							|en = 'Проверка подраздела 2.2 раздела 2 (КС 2.23)'");
	ОписаниеКС      = НСтр("ru = 'Сумма налога, подлежащая уплате в строке 130 подраздела 2.2 раздела 2 должна быть равна (строка 090 х строка 100 х округленное значение (строки 110 х строка 120) / 100, если код в строке 060 подраздела 2.2 раздела 2 равен ""40100"" - ""41200""';
							|en = 'Сумма налога, подлежащая уплате в строке 130 подраздела 2.2 раздела 2 должна быть равна (строка 090 х строка 100 х округленное значение (строки 110 х строка 120) / 100, если код в строке 060 подраздела 2.2 раздела 2 равен ""40100"" - ""41200""'");
...
#Область МеханизмПроверкиКонтрольныхСоотношений_Доп
...
Процедура СформироватьДанныеОтчета()
	...
	ДанныеОтчета = СохраненныеДанныеОтчета();
	
	Для каждого РазделОтчета Из ДанныеОтчета.ДанныеМногостраничныхРазделов Цикл
		ДанныеРаздела = Новый Массив;
		
		Для каждого ДанныеСтраницы Из РазделОтчета.Значение Цикл
		
			СтраницаРаздела = Новый Структура;
			
			Если ТипЗнч(ДанныеСтраницы.Данные) = Тип("Структура") Тогда
				СтраницаРаздела = ОбщегоНазначения.СкопироватьРекурсивно(ДанныеСтраницы.Данные);
			КонецЕсли;
			
			СтраницаРаздела.Вставить("Страница", "" + (РазделОтчета.Значение.Индекс(ДанныеСтраницы) + 1));
			Если СтраницаРаздела.Свойство("П000100003003_1") Тогда
				СтраницаРаздела.Вставить("КолвоСтрокР01", КоличествоСтрок(СтраницаРаздела, "П000100003003_"));
			КонецЕсли;
			
		    ДанныеРаздела.Добавить(СтраницаРаздела);
		
		КонецЦикла;
	
		ДанныеКС.Вставить(РазделОтчета.Ключ, ДанныеРаздела);

	КонецЦикла;
	
	Если ДанныеОтчета.Свойство("ДанныеМногоуровневыхРазделов") Тогда
	...
	МассивОКТМО_Р1 = Новый Массив;
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01С020", "П000100002003_");
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел1 Цикл
		Для Счетчик = 1 По СтраницаРаздела.КолвоСтрокР01 Цикл
	
			ЗаполнитьПоказатель(Показатели.Р01С020, СтраницаРаздела, Счетчик);
			Если МассивОКТМО_Р1.Найти(Показатели.Р01С020.Значение) = Неопределено Тогда 
				МассивОКТМО_Р1.Добавить(Показатели.Р01С020.Значение);
			КонецЕсли;		
		КонецЦикла;
	КонецЦикла;
	
	ДанныеКС.Вставить("МассивОКТМО_Р1", МассивОКТМО_Р1);
...
Процедура СформироватьСлужебныеДанные()
	...
	МассивИсключений = Новый Структура;
	
	ДанныеКС.Вставить("МассивИсключений", МассивИсключений);
...
Процедура СформироватьПоказатели(Показатели, ИменаПоказателей)
	...
	Для каждого Элемент Из ИменаПоказателей Цикл
		...
		Если ДанныеКС.МассивИсключений.Свойство(Элемент.Ключ) Тогда
			МассивПредставления.Добавить(ДанныеКС.МассивИсключений[Элемент.Ключ]);
		...
		Многострочный			= Сред(Элемент.Значение, 14, 1) = "_";
...
Процедура РасшифроватьПоказатель(Показатель)
	...
	НоваяСтрока.СсылкаНаДокументРО		= ?(ЗначениеЗаполнено(Показатель.Отчет), 
		Показатель.Отчет, СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка);
...
&НаСервере
Процедура ДополнитьСтруктуруОшибок(Настройки, СтруктураРезультата)
	
	Если Настройки.ЭтоОшибка Тогда
		СтруктураРезультата.Вставить("КоличествоНайденныхОшибок", СтруктураРезультата.КоличествоНайденныхОшибок + 1);
	Иначе
		СтруктураРезультата.Вставить("КоличествоНайденныхБезОшибок", СтруктураРезультата.КоличествоНайденныхБезОшибок + 1);
	КонецЕсли;
	
	Счетчик		= XMLСтрока(СтруктураРезультата.КоличествоНайденныхОшибок + СтруктураРезультата.КоличествоНайденныхБезОшибок);
	ОписаниеКС	= СформироватьОписаниеКС(Настройки, "ДопИнфо");
	
	СтруктураРезультата.СтруктураОшибок.Вставить("ТекстовоеПредставлениеФормулы" + Счетчик, ОписаниеКС);
	СтруктураРезультата.СтруктураОшибок.Вставить("Расшифровка"	+ Счетчик, Настройки.Расшифровка);
	СтруктураРезультата.СтруктураОшибок.Вставить("ЭтоОшибка"	+ Счетчик, Настройки.ЭтоОшибка);
	СтруктураРезультата.СтруктураОшибок.Вставить("Нарушение"	+ Счетчик, Настройки.Нарушение);
	СтруктураРезультата.СтруктураОшибок.Вставить("Описание"		+ Счетчик, Настройки.ОписаниеОшибки);
	СтруктураРезультата.СтруктураОшибок.Вставить("Комментарий"	+ Счетчик, Настройки.Комментарий);
...
Функция ОбработатьНастройки(Знач ИзменяемыеСвойства, Знач Настройки = Неопределено)
	...
	Настройки.ОписаниеКС = Настройки.Раздел + ". (КС " + Настройки.НомерКС + ")" + Символы.ПС + Настройки.ОписаниеКС;
...
&НаСервере
Процедура ПроверитьКС_12_1_к(СтруктураРезультата)
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '12.1.к';
													|en = '12.1.к'"));
	...
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Группа строк %1 страницы %2 Раздела 1';
													|en = 'Группа строк %1 страницы %2 Раздела 1'"));
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел1 Цикл
		Для Счетчик = 1 По СтраницаРаздела.КолвоСтрокР01 Цикл
			...
			Настройки.ДопСведения = СтрШаблон(Настройки.ШаблонДопСведений,
				Счетчик, СтраницаРаздела.Страница);
			ДополнитьСтруктуруОшибок(Настройки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_12_2_к(СтруктураРезультата)
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '12.2.к';
													|en = '12.2.к'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 1';
														|en = 'Раздел 1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@';
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма налога в стр.030 Раздела 1 = сумма стр.160 подр.2.1 Раздела 2 + сумма стр.130 подр.2.2 ';
			|en = 'Сумма налога в стр.030 Раздела 1 = сумма стр.160 подр.2.1 Раздела 2 + сумма стр.130 подр.2.2 '") + 
		НСтр("ru = 'Раздела 2 по соответствующему ОКТМО';
			|en = 'Раздела 2 по соответствующему ОКТМО'")); 
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 (%3 + %4), ОКТМО %5';
													|en = '%1 = %2 (%3 + %4), ОКТМО %5'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Возможно ошибочная сумма налога';
														|en = 'Возможно ошибочная сумма налога'"));
	...
	ИменаПоказателей.Вставить("Р01С020"		, "П000100002003_");
	ИменаПоказателей.Вставить("Р01С030"		, "П000100003003_");
	ИменаПоказателей.Вставить("Р02С020"		, "П000210002003");
	ИменаПоказателей.Вставить("Р21С160"		, "П000210016003");
	ИменаПоказателей.Вставить("Р22С020"		, "П000220002003");
	ИменаПоказателей.Вставить("Р22С130"		, "П000220013003");
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	
	Для каждого ОКТМО_Р1 Из ДанныеКС.МассивОКТМО_Р1 Цикл
	
		ЗарегистрироватьФормулу(Настройки);
		Показатели.Р01С030.Итог = 0;
		Показатели.Р21С160.Итог = 0;
		Показатели.Р22С130.Итог = 0;
		
		Для каждого СтраницаРаздела Из ДанныеКС.Раздел1 Цикл
			Для Счетчик = 1 По СтраницаРаздела.КолвоСтрокР01 Цикл
		
				ЗаполнитьПоказатель(Показатели.Р01С020, СтраницаРаздела, Счетчик);
				Если НЕ Показатели.Р01С020.Значение = ОКТМО_Р1 Тогда
					Продолжить;
				КонецЕсли;
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01С030, СтраницаРаздела, Счетчик);
		...
		Для каждого СтраницаРаздела Из ДанныеКС.Раздел2 Цикл 
			...
			Если НЕ Показатели.Р02С020.Значение = ОКТМО_Р1 Тогда
				Продолжить;
			КонецЕсли;
		...
		Для каждого СтраницаРаздела Из ДанныеКС.Раздел22 Цикл
			...
			ЗаполнитьПоказатель(Показатели.Р22С020, СтраницаРаздела);
			Если НЕ Показатели.Р22С020.Значение = ОКТМО_Р1 Тогда
				Продолжить;
			КонецЕсли;
			ЗаполнитьИРасшифроватьПоказатель(Показатели.Р22С130, СтраницаРаздела);
				
		КонецЦикла;
		
		РасчетнаяСумма = Показатели.Р21С160.Итог + Показатели.Р22С130.Итог;
		Настройки.ЭтоОшибка = НЕ Показатели.Р01С030.Итог = РасчетнаяСумма;
		Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, 
			Показатели.Р01С030.Итог, РасчетнаяСумма, Показатели.Р21С160.Итог, Показатели.Р22С130.Итог,
			ЗначСтр(ОКТМО_Р1));
		ДополнитьСтруктуруОшибок(Настройки, СтруктураРезультата);
