...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "10/07/2013";
...
Процедура ПоказатьПериод()
	...
	Попытка
		ТабличныйДокумент.Области.ПериодОтчета.Значение = "за январь - " + НРег(Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'ММММ гггг'")) + " г.";
	Исключение
	КонецПопытки;
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("РазделВывозимыеТовары",  "ФормаОтчета2013Кв1_РазделВывозимыеТовары");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	ИмяПоляТаблДокумента = "ПолеТабличногоДокументаРазделВывозимыеТовары";
	
	// Проверка заполнения отчета.
	НомСтрПар = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, "ВывозимыеТоварыСтраница" + Формат(НомСтрПар, "ЧГ=")) Цикл
		СтруктураТекСтраницы = ПараметрыВыгрузки["ВывозимыеТоварыСтраница" + Формат(НомСтрПар, "ЧГ=")];
		ТекСумма = 0;
		НомСтр = 1;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(СтруктураТекСтраницы, "ПР00001000103_" + Формат(НомСтр, "ЧГ=")) Цикл
			ТекСумма = ТекСумма + СтруктураТекСтраницы["ПР00001000103_" + Формат(НомСтр, "ЧГ=")];
			НомСтр = НомСтр + 1;
		КонецЦикла;
		ТекСумма = ТекСумма + СтруктураТекСтраницы["ПРПроданоВсего"];
		ТекСумма = ТекСумма + СтруктураТекСтраницы["ПРПроданоНаЭкспорт"];
		Если ТекСумма = 0 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе ""Вывозимые товары"" не заполнена страница " + НомСтрПар + "!", ИмяПоляТаблДокумента, "");
			СтатусПроверки = Ложь;
		КонецЕсли;
		НомСтрПар = НомСтрПар + 1;
	КонецЦикла;
