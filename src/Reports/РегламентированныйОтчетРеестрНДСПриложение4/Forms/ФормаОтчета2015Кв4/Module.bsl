...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузкиРеестра, ДеревоВыгрузкиСведений)
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			ИначеЕсли ПрТаможДекл = "1" ИЛИ ЗначениеЗаполнено(СокрЛП(Ветка_П10000.Данные["П1000013"])) Тогда
				
				ДлинаРегНомТД = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000013"]));
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100005"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " не задано наименование пункта назначения нефти и нефтепродуктов (за пределами территории Российской Федерации или на границе Российской Федерации).",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Если РазделЗаполнен(ДанныеРеестра.Строки, Ложь) Тогда
		...
		ПроверитьКС_4_1_1(СтруктураРезультата);
		ПроверитьКС_4_1_2(СтруктураРезультата);
		ПроверитьКС_4_1_3(СтруктураРезультата);
		ПроверитьКС_4_1_4(СтруктураРезультата);
		ПроверитьКС_4_2_1(СтруктураРезультата);
		ПроверитьКС_4_2_2(СтруктураРезультата);
		ПроверитьКС_4_2_3(СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_4_1_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 4.1.1';
												|en = 'Проверка КС 4.1.1'"));
...
&НаСервере
Процедура ПроверитьКС_4_1_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 4.1.2';
												|en = 'Проверка КС 4.1.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_4_1_3(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		,НСтр("ru = 'Проверка КС 4.1.3';
													|en = 'Проверка КС 4.1.3'"));
	Заголовки.Вставить("Нарушение"			,НСтр("ru = 'п. 1, п. 3.2, п. 3.3, п. 3.6, п. 4 ст. 165 НК РФ';
													|en = 'п. 1, п. 3.2, п. 3.3, п. 3.6, п. 4 ст. 165 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			,
		НСтр("ru = 'Если признак таможенного декларирования (графа 2 Реестра) = 1, то регистрационный номер ДТ должен отвечать следующим требованиям:
		|-должен состоять из 23 символов и иметь вид ""TTTTTTTT/DDMMGG/NNNNNNN"", / - косая черта вправо, разделитель показателей;
		|-DDMMGG – дата декларации на товары (день, месяц, год последние 2 цифры);
		|-GG - значение года оформления ДТ, должно быть не меньше значения текущего года уменьшенного на 4 и не больше текущего;
		|-NNNNNNN – порядковый номер декларации на товары для данного таможенного органа, должен быть больше 0';
		|en = 'Если признак таможенного декларирования (графа 2 Реестра) = 1, то регистрационный номер ДТ должен отвечать следующим требованиям:
		|-должен состоять из 23 символов и иметь вид ""TTTTTTTT/DDMMGG/NNNNNNN"", / - косая черта вправо, разделитель показателей;
		|-DDMMGG – дата декларации на товары (день, месяц, год последние 2 цифры);
		|-GG - значение года оформления ДТ, должно быть не меньше значения текущего года уменьшенного на 4 и не больше текущего;
		|-NNNNNNN – порядковый номер декларации на товары для данного таможенного органа, должен быть больше 0'"));
	Заголовки.Вставить("ПроверятьПустые"	,Истина);
	Заголовки.Вставить("Декларирование"		,Истина);
	Настройки	= ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	
	ПроверитьКС_СоответствиеНомераГТДФормату(СтруктураРезультата, Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверитьКС_4_1_4(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 4.1.4';
												|en = 'Проверка КС 4.1.4'"));
	Заголовки.Вставить("ОписаниеКС"		,НСтр("ru = 'Код вида ТСД должен соответствовать указанному коду вида транспортного средства:
		|-если Код вида транспортного средства = 30/31/32, то Вид транспортного, товаросопроводительного и (или) иного документа = 01/05/12;
		|-если Код вида транспортного средства = 10/80, то Вид транспортного, товаросопроводительного и (или) иного документа = 02/06/07;
		|-если Код вида транспортного средства = 20, то Вид транспортного, товаросопроводительного и (или) иного документа = 03;
		|-если Код вида транспортного средства = 71/80/99, то Вид транспортного, товаросопроводительного и (или) иного документа = 12';
		|en = 'Код вида ТСД должен соответствовать указанному коду вида транспортного средства:
		|-если Код вида транспортного средства = 30/31/32, то Вид транспортного, товаросопроводительного и (или) иного документа = 01/05/12;
		|-если Код вида транспортного средства = 10/80, то Вид транспортного, товаросопроводительного и (или) иного документа = 02/06/07;
		|-если Код вида транспортного средства = 20, то Вид транспортного, товаросопроводительного и (или) иного документа = 03;
		|-если Код вида транспортного средства = 71/80/99, то Вид транспортного, товаросопроводительного и (или) иного документа = 12'"));
	...
	ТаблицаПравил.Колонки.Добавить("ВидТранспорта");
	ТаблицаПравил.Колонки.Добавить("ВидДокумента");
	НовСтрока				= ТаблицаПравил.Добавить();
	НовСтрока.ВидТранспорта	= "10";
	НовСтрока.ВидДокумента	= "02/06/07";
	НовСтрока				= ТаблицаПравил.Добавить();
	НовСтрока.ВидТранспорта	= "20";
	НовСтрока.ВидДокумента	= "03";
	НовСтрока				= ТаблицаПравил.Добавить();
	НовСтрока.ВидТранспорта	= "30/31/32";
	НовСтрока.ВидДокумента	= "01/05/12";
	НовСтрока				= ТаблицаПравил.Добавить();
	НовСтрока.ВидТранспорта	= "71/99";
	НовСтрока.ВидДокумента	= "12";
	НовСтрока				= ТаблицаПравил.Добавить();
	НовСтрока.ВидТранспорта	= "80";
	НовСтрока.ВидДокумента	= "02/06/07/12";
	
	Настройки.Вставить("ТаблицаПравил", ТаблицаПравил);
	
	ПроверитьКС_СоответствиеВидаДокументаВидуТранспорта_3Уровня(СтруктураРезультата, Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверитьКС_4_2_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 4.2.1';
												|en = 'Проверка КС 4.2.1'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_4_2_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 4.2.2';
												|en = 'Проверка КС 4.2.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_4_2_3(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 4.2.3';
												|en = 'Проверка КС 4.2.3'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
Процедура ПроверитьКС_СоответствиеНалоговойБазыПострочнойСумме(СтруктураРезультата, Настройки)
	...
	Настройки.Вставить("ОписаниеКС"			,
		НСтр("ru = 'Итого налоговая база = ∑ Налоговая база по соответствующей операции по реализации ';
			|en = 'Итого налоговая база = ∑ Налоговая база по соответствующей операции по реализации '") +
		НСтр("ru = 'товаров (работ, услуг), обоснованность применения налоговой ставки 0 процентов по ';
			|en = 'товаров (работ, услуг), обоснованность применения налоговой ставки 0 процентов по '") +
		НСтр("ru = 'которой документально подтверждена';
			|en = 'которой документально подтверждена'"));
	Настройки.Вставить("ШаблонОшибки"		,
		НСтр("ru = 'Значение итоговой налоговой базы по реестру %1 %2 суммы значений налоговой базы по всем ';
			|en = 'Значение итоговой налоговой базы по реестру %1 %2 суммы значений налоговой базы по всем '") +
		НСтр("ru = 'строкам реестра сведений %3 на %4';
			|en = 'строкам реестра сведений %3 на %4'"));
...
&НаСервере
Процедура ПроверитьКС_СоответствиеНомераГТДФормату(СтруктураРезультата, Настройки)
	
	Настройки.Вставить("ШаблонОшибки"		,
		НСтр("ru = 'Сверка с данными ФТС России не производилась. Расчет налоговой базы по таможенной декларации ';
			|en = 'Сверка с данными ФТС России не производилась. Расчет налоговой базы по таможенной декларации '") +
		НСтр("ru = 'не производился. Номер таможенной декларации не соответствует формату: ';
			|en = 'не производился. Номер таможенной декларации не соответствует формату: '"));
	Настройки.Вставить("ШаблонОшибки2"		,НСтр("ru = 'недопустимый формат номера таможенной декларации';
													|en = 'недопустимый формат номера таможенной декларации'"));
	Настройки.Вставить("ШаблонОшибки3"		,НСтр("ru = 'указана недопустимая дата таможенной декларации';
													|en = 'указана недопустимая дата таможенной декларации'"));
	Настройки.Вставить("ШаблонОшибки4"		,
		НСтр("ru = 'указан недопустимый год таможенной декларации (только %1 - %2 гг.)';
			|en = 'указан недопустимый год таможенной декларации (только %1 - %2 гг.)'"));
	Настройки.Вставить("ШаблонОшибки5"		,НСтр("ru = 'порядковый номер таможенной декларации должен быть больше нуля';
													|en = 'порядковый номер таможенной декларации должен быть больше нуля'"));
	Настройки.Вставить("ШаблонРасшифровки"	,НСтр("ru = 'Указанный номер: (""%1"")';
													|en = 'Указанный номер: (""%1"")'"));
	КолвоСтрокРасч			= Новый Соответствие;
	...
	Для каждого СтраницаРаздела Из ДанныеРеестра.Строки Цикл
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл
			...
			Если Настройки.Декларирование Тогда
				Декларирование = ЗаполнитьПоказатели("ПризнакДекларирования", ДанныеСтраницы[НомерСтроки - 1].Данные, 
					НомерСтроки);
				Если Не Декларирование.ПризнакДекларирования.ЗначениеСтр = "1" Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			
			ПоказательГТД = ЗаполнитьПоказатели("НомерГТД", ДанныеСтраницы[НомерСтроки - 1].Данные, НомерСтроки);
			
			Если Не Настройки.ПроверятьПустые Тогда
				Если Не ЗначениеЗаполнено(ПоказательГТД.НомерГТД.Значение) Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			...
			Настройки.Вставить("ШаблонДопСведений"	,НСтр("ru = 'Страница %1, Строка %2';
															|en = 'Страница %1, Строка %2'"));
			Настройки.Вставить("ДопСведенияПараметры", Новый Структура("НомерСтраницы, НомерСтроки", 
				НомерСтраницы, НомерРасчетный));
			ЗарегистрироватьФормулу(Настройки);
			РасшифроватьПоказатели(ПоказательГТД, Настройки, "Реестр", НомерСтраницы);
			НомерГТД = ПоказательГТД.НомерГТД.Значение;
			Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				Настройки.ШаблонРасшифровки, НомерГТД);
			НомерГТДСоответствуетФормату = СтрДлина(СокрЛП(НомерГТД)) = 23
				И КодСимвола(Сред(СокрЛП(НомерГТД), 9, 1)) = 47 И КодСимвола(Сред(СокрЛП(НомерГТД), 16, 1)) = 47;
			Настройки.ЭтоОшибка		= Не НомерГТДСоответствуетФормату;
				
			Если Настройки.ЭтоОшибка Тогда
				Настройки.ОписаниеОшибки	= Настройки.ШаблонОшибки + Настройки.ШаблонОшибки2;
				ДобавитьЭлементВСтруктуруОшибок(Настройки);
				Продолжить;
			КонецЕсли;
			
			// проверка даты
			ДатаГТДИсходная	= Сред(СокрЛП(НомерГТД), 10, 6);
			ДатаГТД			= СтрокаВДату(ДатаГТДИсходная);
			Если Не ЗначениеЗаполнено(ДатаГТД) Тогда
				МассивОшибок.Добавить(Настройки.ШаблонОшибки3);
			КонецЕсли;
			
			// проверка года
			ГодГТД			= Сред(СокрЛП(НомерГТД), 14, 2);
			ГодТекущий		= СтрокаВЧисло((Формат(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, "ДФ=yy")));
			ГодКрайний		= ГодТекущий - 4;
			ШаблонОшибки4	= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонОшибки4, 
				ГодКрайний, ГодТекущий);
			ГодГТДЧисло 	= СтрокаВЧисло(ГодГТД);
			Если ГодГТДЧисло = Неопределено Или Не ТолькоЦифры(ГодГТД) Тогда
				МассивОшибок.Добавить(ШаблонОшибки4);
			Иначе
				Если ГодГТДЧисло < ГодКрайний ИЛИ ГодГТДЧисло > ГодТекущий Тогда
					МассивОшибок.Добавить(ШаблонОшибки4);
				КонецЕсли;
			КонецЕсли;
			
			// проверка порядкового номера
			ПорядкНомер		= Сред(СокрЛП(НомерГТД), 17, 7);
			ПорядкНомерЧисло= СтрокаВЧисло(ПорядкНомер);
			Если ПорядкНомерЧисло = Неопределено Или Не ТолькоЦифры(ПорядкНомер) Тогда
				МассивОшибок.Добавить(Настройки.ШаблонОшибки5);
			Иначе
				Если ПорядкНомерЧисло = 0 Тогда
					МассивОшибок.Добавить(Настройки.ШаблонОшибки5);
				КонецЕсли;
			КонецЕсли;
			
			Настройки.ЭтоОшибка = Не МассивОшибок.Количество() = 0;
			Настройки.ОписаниеОшибки = Настройки.ШаблонОшибки + СтрСоединить(МассивОшибок, ", ");
...
&НаСервере
Процедура ПроверитьКС_СоответствиеВидаДокументаВидуТранспорта_3Уровня(СтруктураРезультата, Настройки)
	
	Настройки.Вставить("ШаблонОшибки"		,
		НСтр("ru = 'Указанный код вида документа %1 не соответствует коду вида транспортного средства %2';
			|en = 'Указанный код вида документа %1 не соответствует коду вида транспортного средства %2'"));
	Настройки.Вставить("ШаблонРасшифровки"	,НСтр("ru = 'Указанный код вида документа: (""%1""), допустимый: (""%2"")';
													|en = 'Указанный код вида документа: (""%1""), допустимый: (""%2"")'"));
	КолвоСтрокРасч			= Новый Соответствие;
	НомерСтраницы			= 0;
	...
	Для каждого СтраницаРаздела Из ДанныеРеестра.Строки Цикл
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл
			...
			НомерРасчетный	= НомерРасчетный + 1;
			СписокСтрок2Уровень	= ДанныеСтраницы[НомерСтроки - 1].ДанныеМногострочныхЧастей.П12000.Строки;
			КоличествоСтрок2Уровень = СписокСтрок2Уровень.Количество();
			
			Для НомерСтроки2Уровень = 1 По КоличествоСтрок2Уровень Цикл
				
				Строка2Уровень = СписокСтрок2Уровень[НомерСтроки2Уровень - 1];
				ПоказательВТ = ЗаполнитьПоказатели("ВидТранспорта", Строка2Уровень.Данные, НомерСтроки, НомерСтроки2Уровень);
				Если Не ЗначениеЗаполнено(ПоказательВТ.ВидТранспорта.Значение) Тогда
					Продолжить;
				Иначе
					Для каждого СтрокаПравил Из Настройки.ТаблицаПравил Цикл
						Если СтрНайти(СтрокаПравил.ВидТранспорта, ПоказательВТ.ВидТранспорта.Значение) = 0 Тогда
							Продолжить;
						Иначе
							СписокСтрок3Уровень = Строка2Уровень.ДанныеМногострочныхЧастей.П12100.Строки;
							КоличествоСтрок3Уровень = СписокСтрок3Уровень.Количество();
							
							Для НомерСтроки3Уровень = 1 По КоличествоСтрок3Уровень Цикл;
								
								Строка3Уровень 	= СписокСтрок3Уровень[НомерСтроки3Уровень - 1];
								ПоказательВД = ЗаполнитьПоказатели("ВидДокумента", Строка3Уровень.Данные, НомерСтроки, НомерСтроки2Уровень, 
									НомерСтроки3Уровень);
								Настройки.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонОшибки, 
									ПоказательВД.ВидДокумента.Значение, ПоказательВТ.ВидТранспорта.Значение);
								Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
									ПоказательВД.ВидДокумента.Значение, СтрокаПравил.ВидДокумента);
								Настройки.ЭтоОшибка = СтрНайти(СтрокаПравил.ВидДокумента, ПоказательВД.ВидДокумента.Значение) = 0 
									Или Не ЗначениеЗаполнено(ПоказательВД.ВидДокумента.Значение);
								Настройки.Вставить("ШаблонДопСведений"	,НСтр("ru = 'Страница %1, Строка %2, Подстрока вида транспорта %3';
																				|en = 'Страница %1, Строка %2, Подстрока вида транспорта %3'"));
								Настройки.Вставить("ДопСведенияПараметры", Новый Структура("НомерСтраницы, НомерСтроки, НомерВТ", 
									НомерСтраницы, НомерРасчетный, НомерСтроки2Уровень));
								ЗарегистрироватьФормулу(Настройки);
								РасшифроватьПоказатели(ПоказательВТ, Настройки, "Реестр", НомерСтраницы);
								Настройки.Вставить("ШаблонДопСведений"	,
									НСтр("ru = 'Страница %1, Строка %2, Подстрока вида транспорта %3, Подстрока документа %4';
										|en = 'Страница %1, Строка %2, Подстрока вида транспорта %3, Подстрока документа %4'"));
								Настройки.Вставить("ДопСведенияПараметры", Новый Структура("НомерСтраницы, НомерСтроки, НомерВТ, НомерВД", 
									НомерСтраницы, НомерРасчетный, НомерСтроки2Уровень, НомерСтроки3Уровень));
								РасшифроватьПоказатели(ПоказательВД, Настройки, "Реестр", НомерСтраницы);
								ДобавитьЭлементВСтруктуруОшибок(Настройки);
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			...
			?(Настройки.ЭтоОшибка, НСтр("ru = 'строка 010 раздела 4 налоговой декларации по НДС';
										|en = 'строка 010 раздела 4 налоговой декларации по НДС'"), 
			Показатели.КодОперации.ЗначениеСтр));
...
Процедура ПроверитьКС_СверкаСДекларациейНДС_Сумма(СтруктураРезультата, Настройки);
	...
	Настройки.Вставить("ОписаниеКС"		,
		НСтр("ru = 'Итого налоговая база (в рублях) = строка 020 раздела 4 налоговой декларации НДС по ';
			|en = 'Итого налоговая база (в рублях) = строка 020 раздела 4 налоговой декларации НДС по '") +
		НСтр("ru = 'соответствующему коду операции';
			|en = 'соответствующему коду операции'"));
	Настройки.Вставить("ШаблонОшибки"		,
		НСтр("ru = 'Налоговая база из реестра %1 налоговой базы из налоговой декларации по НДС по коду операции %2 на %3';
			|en = 'Налоговая база из реестра %1 налоговой базы из налоговой декларации по НДС по коду операции %2 на %3'"));
	Настройки.Вставить("ШаблонРасшифровки"	,НСтр("ru = '%1 = %2';
													|en = '%1 = %2'"));
	Настройки.Вставить("ШаблонДопСведений"	,НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	ИначеЕсли УсловныйКодПоказателя = "БазаИтого" Тогда
		...
		Результат.Вставить("Представление", Нстр("ru = 'Итоговая сумма налоговой базы';
												|en = 'Итоговая сумма налоговой базы'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "НалоговаяБаза" Тогда
		Результат.Вставить("Имя", "П1000018%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Сумма налоговой базы';
												|en = 'Сумма налоговой базы'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "НомерГТД" Тогда
		Результат.Вставить("Имя", "П1000013%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Регистрационный номер ГТД';
												|en = 'Регистрационный номер ГТД'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "ПризнакДекларирования" Тогда
		Результат.Вставить("Имя", "П1000002%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Признак таможенного декларирования';
												|en = 'Признак таможенного декларирования'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "ВидТранспорта" Тогда
		Результат.Вставить("Имя", "П1200014%1%2%3%4");
		Результат.Вставить("Представление", Нстр("ru = 'Код вида транспортного средства';
												|en = 'Код вида транспортного средства'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "ВидДокумента" Тогда
		Результат.Вставить("Имя", "П1210015%1%2%3%4%5%6");
		Результат.Вставить("Представление", Нстр("ru = 'Код вида товаросопроводительного документа';
												|en = 'Код вида товаросопроводительного документа'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
...
Функция ТолькоЦифры(Знач Значение)
	...
	Возврат СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение);
