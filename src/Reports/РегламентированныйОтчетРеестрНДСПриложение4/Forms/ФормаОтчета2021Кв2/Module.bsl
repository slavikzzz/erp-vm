...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("мИдГруппы910");
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр р.НДС прилож.04 21к2") Тогда
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	СтруктураРеквизитовФормы.мИдГруппы910 = "П91000";
	...
	ИменаПодчиненныхГрупп.Добавить(СтруктураРеквизитовФормы.мИдГруппы910);
	...
	КолонкиСИзмНижнГраницей.Добавить(11);
	СтруктураМногострочныхЧастей.Вставить(
	...
	// Группа П91000.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П9100013");
	ИменаПодчиненныхГрупп = Новый Массив;
	СтруктураМногострочныхЧастей.Вставить(
		СтруктураРеквизитовФормы.мИдГруппы910, Новый Структура(
	...
	// Группа П12100.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П1210015");
	ИменаГраф.Добавить("П1210016");
	ИменаГраф.Добавить("П1210017");
	ИменаПодчиненныхГрупп = Новый Массив;
	СтруктураМногострочныхЧастей.Вставить(
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1000002", 1, ,
		"Выбор признака таможенного декларирования", КоллекцияСписковВыбора["ПризнакиТамДекл"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1200014", 2, ,
		"Выбор кода вида транспортного средства", КоллекцияСписковВыбора["КодыВидаТС"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1210015", 2, ,
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 8) = "П1000008" Тогда
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы110 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Удалить выбранное маршрутное поручение?';
							|en = 'Удалить выбранное маршрутное поручение?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы910 Тогда
		Подстрока = "таможенную декларацию";
		ТекстВопроса = НСтр("ru = 'Удалить выбранную таможенную декларацию?';
							|en = 'Удалить выбранную таможенную декларацию?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы120 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Удалить выбранный вид транспорта?';
							|en = 'Удалить выбранный вид транспорта?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы121 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Удалить выбранный транспортный документ?';
							|en = 'Удалить выбранный транспортный документ?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВставитьДополнительнуюСтроку(Знач Копирование = Ложь)
	...
	Если ПревышеноДопустимоеКоличествоСтрокТабличногоДокумента(
		...
		Сообщение.Текст = НСтр("ru = 'Превышено допустимое количество строк табличного документа. Необходимо добавить страницу и продолжить ввод данных.';
								|en = 'Превышено допустимое количество строк табличного документа. Необходимо добавить страницу и продолжить ввод данных.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя)
				...
				Если ЗначениеЗаполнено(КодОперации) Тогда
					Ном = 1;
					Пока Форма.ТабличныйДокумент.Области.Найти("П1000018_" + Формат(Ном, "ЧГ=")) <> Неопределено Цикл
						СуммаНалБаз = СуммаНалБаз
							+ Форма.ТабличныйДокумент.Области["П1000018_" + Формат(Ном, "ЧГ=")].Значение;
						Ном = Ном + 1;
					КонецЦикла;
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если Ветка = АктивнаяВетка Тогда
				...
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					Ветка_П10000.Данные.Вставить("П1000018", Форма.ТабличныйДокумент.Области["П1000018_"
			...
			Если ЗначениеЗаполнено(КодОперации) Тогда
				ДанныеМнЧ_П10000 = Ветка.ДанныеМногострочныхЧастей.П10000.Строки;
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					СуммаНалБаз = СуммаНалБаз + Ветка_П10000.Данные["П1000018"];
				КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("НалБазаИтого", "КодОперации;П1000018");
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревьяДляСравнения.Вставить("ДеревоСравненияРеестра",  ИзвлечьСтруктуруXML("СхемаВыгрузки503_1"));
	ДеревьяДляСравнения.Вставить("ДеревоСравненияСведений", ИзвлечьСтруктуруXML("СхемаВыгрузки503_2"));
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревьяДляЗагрузки.Вставить("ДеревоЗагрузкиРеестра",  ИзвлечьСтруктуруXML("СхемаВыгрузки503_1"));
	ДеревьяДляЗагрузки.Вставить("ДеревоЗагрузкиСведений", ИзвлечьСтруктуруXML("СхемаВыгрузки503_2"));
...
Функция УниверсальнаяВыгрузка(ТекстВыгрузки = Неопределено, ПроверятьБлокировки = Истина, ПроверятьВозможностьВыгрузки = Истина) Экспорт
	...
	Если ПроверятьВозможностьВыгрузки И СтруктураРеквизитовФормы.мПроверятьСоотношенияПриПечатиИВыгрузки = Истина Тогда 
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки, ПроверятьВозможностьВыгрузки)
	...
	ДеревоВыгрузкиРеестра  = ИзвлечьСтруктуруXML("СхемаВыгрузки503_1");
	ДеревоВыгрузкиСведений = ИзвлечьСтруктуруXML("СхемаВыгрузки503_2");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрТрНефт_4");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRNEFTNDS"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRNEFTNDS.2"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузкиРеестра, ДеревоВыгрузкиСведений)
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			ПрТаможДекл = СокрЛП(Ветка_П10000.Данные["П1000002"]);
			
			Если НЕ ЗначениеЗаполнено(ПрТаможДекл) Тогда
				
				ИмяПок = "П1000002" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не задан признак таможенного декларирования",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
				
			ИначеЕсли ПрТаможДекл = "2" Тогда
				
				Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000004"]) Тогда
					ИмяПок = "П1000004" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " не задана дата документа, подтверждающего факт оказания услуг",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000007"]) Тогда
					...
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000008"]) Тогда
			...
			Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
				...
				КодВидТС = СокрЛП(Ветка_П12000.Данные["П1200014"]);
				
				ДлинаКодВидТС = СтрДлина(КодВидТС);
				Если ДлинаКодВидТС <> 2 Тогда
					ИмяПок = "П1200014" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " не" + ?(ДлинаКодВидТС = 0, "", "верно")
					+ " задан код вида транспортного средства, которым нефть и нефтепродукты вывозились с территории Российской Федерации",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				// 12100.
				ДанныеМнЧ_П12100 = Ветка_П12000.ДанныеМногострочныхЧастей.П12100.Строки;
				ЕстьХотяБыОднаЗапись_П12100 = Ложь;
				Для каждого Ветка_П12100 Из ДанныеМнЧ_П12100 Цикл
					
					НомерСтрокиП12100 = ДанныеМнЧ_П12100.Индекс(Ветка_П12100) + 1;
					НомерСтрокиП12100_Строка = Формат(НомерСтрокиП12100, "ЧГ=");
					
					Если НЕ (ЗначениеЗаполнено(Ветка_П12100.Данные["П1210015"])
						ИЛИ ЗначениеЗаполнено(Ветка_П12100.Данные["П1210016"])
						ИЛИ ЗначениеЗаполнено(Ветка_П12100.Данные["П1210017"])) Тогда
						Продолжить;
					КонецЕсли;
					
					ЕстьХотяБыОднаЗапись_П12100 = Истина;
					
					ДлинаВидТрДок = СтрДлина(СокрЛП(Ветка_П12100.Данные["П1210015"]));
					Если ДлинаВидТрДок <> 2 Тогда
						ИмяПок = "П1210015" + "_" + НомерСтрокиП10000_Строка
						+ "_" + НомерСтрокиП12000_Строка + "_" + НомерСтрокиП12100_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомПП + " не" + ?(ДлинаВидТрДок = 0, "", "верно")
						+ " задан вид транспортного, товаросопроводительного и (или) иного документа, подтверждающего вывоз нефти и нефтепродуктов за пределы территории Российской Федерации или таможенный транзит",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					Если НЕ ЗначениеЗаполнено(Ветка_П12100.Данные["П1210017"]) Тогда
						ИмяПок = "П1210017" + "_" + НомерСтрокиП10000_Строка
						+ "_" + НомерСтрокиП12000_Строка + "_" + НомерСтрокиП12100_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомПП + " не задана дата транспортного, товаросопроводительного и (или) иного документа, подтверждающего вывоз нефти и нефтепродуктов за пределы территории Российской Федерации или таможенный транзит",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				...
				Если ЗначениеЗаполнено(КодВидТС) И КодВидТС <> "71" И НЕ ЕстьХотяБыОднаЗапись_П12100 Тогда
					ИмяПок = "П1210015" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка + "_1";
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " должны быть заданы сведения о транспортном, товаросопроводительном и (или) ином документе, подтверждающем вывоз нефти и нефтепродуктов за пределы территории Российской Федерации или таможенный транзит, если значение кода вида транспортного средства, которым нефть и нефтепродукты вывозились с территории Российской Федерации, неравно 71",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			ЕстьХотяБыОднаЗапись_П11000 = Ложь;
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если НЕ (ЗначениеЗаполнено(Ветка_П11000.Данные["П1100009"])
					ИЛИ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100010"])
					ИЛИ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100011"])
					ИЛИ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100012"])
					ИЛИ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100005"])) Тогда
					Продолжить;
				КонецЕсли;
				
				ЕстьХотяБыОднаЗапись_П11000 = Истина;
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100009"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " не задан номер маршрутного поручения (телеграммы)",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100010"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " не задана дата маршрутного поручения (телеграммы)",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100011"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " не задано наименование конечного пункта маршрута",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100005"]) Тогда
					ИмяПок = "П1100005" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " не задано наименование пункта назначения нефти и нефтепродуктов (за пределами "
					+ "территории Российской Федерации или на Государственной границе Российской Федерации)",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			Если ПрТаможДекл = "2" И НЕ ЕстьХотяБыОднаЗапись_П11000 Тогда
				ИмяПок = "П1100005" + "_" + НомерСтрокиП10000_Строка + "_1";
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " должны быть заданы сведения о маршрутном поручении, если значение признака таможенного декларирования равно 2",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			// 91000.
			ДанныеМнЧ_П91000 = Ветка_П10000.ДанныеМногострочныхЧастей.П91000.Строки;
			ЕстьХотяБыОднаЗапись_П91000 = Ложь;
			Для каждого Ветка_П91000 Из ДанныеМнЧ_П91000 Цикл
				
				НомерСтрокиП91000 = ДанныеМнЧ_П91000.Индекс(Ветка_П91000) + 1;
				НомерСтрокиП91000_Строка = Формат(НомерСтрокиП91000, "ЧГ=");
				
				Если НЕ ЗначениеЗаполнено(Ветка_П91000.Данные["П9100013"]) Тогда
					Продолжить;
				КонецЕсли;
				
				ЕстьХотяБыОднаЗапись_П91000 = Истина;
				
				ДлинаРегНомТД = СтрДлина(СокрЛП(Ветка_П91000.Данные["П9100013"]));
				Если НЕ (ДлинаРегНомТД = 23 ИЛИ (ДлинаРегНомТД >= 25 И ДлинаРегНомТД <= 29)) Тогда
					ИмяПок = "П9100013" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП91000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " не" + ?(ДлинаРегНомТД = 0, "", "верно")
					+ " задан регистрационный номер полной таможенной декларации. Возможное количество символов в номере: 23 или от 25 до 29",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			Если ПрТаможДекл = "1" И НЕ ЕстьХотяБыОднаЗапись_П91000 Тогда
				ИмяПок = "П9100013" + "_" + НомерСтрокиП10000_Строка + "_1";
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " должен быть задан задан регистрационный номер полной таможенной декларации, если значение признака таможенного декларирования равно 1",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрТрНефт_4 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрТрНефт_4");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрТрНефт_4 = СкопироватьУзел(УзелРодитель.Родитель, РеестрТрНефт_4);
		Для каждого Эл Из ЭкзРеестрТрНефт_4.Строки Цикл
		...
		СведДкПдтвУслуг = ПолучитьПодчиненныйЭлемент(ЭкзРеестрТрНефт_4, "СведДкПдтвУслуг");
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведДкПдтвУслуг = СкопироватьУзел(ЭкзРеестрТрНефт_4, СведДкПдтвУслуг, ЭкзРеестрТрНефт_4.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСведДкПдтвУслуг.Строки Цикл
					
					ПрТаможДекл = СокрЛП(Ветка_П10000.Данные["П1000002"]);
					
					Если Эл.Строки.Количество() = 0 И Эл.Код <> "РегНомТД" Тогда
						Если Эл.Код = "НомерДк" И ПрТаможДекл = "2" Тогда
				...
				// 91000.
				РегНомТД = ПолучитьПодчиненныйЭлемент(ЭкзСведДкПдтвУслуг, "РегНомТД");
				ДанныеМнЧ_П91000 = Ветка_П10000.ДанныеМногострочныхЧастей.П91000.Строки;
				Для каждого Ветка_П91000 Из ДанныеМнЧ_П91000 Цикл
					ЭкзРегНомТД = СкопироватьУзел(ЭкзСведДкПдтвУслуг, РегНомТД);
					ВывестиПоказательВXML(ЭкзРегНомТД, Ветка_П91000.Данные[ЭкзРегНомТД.Ключ]);
				КонецЦикла;
				
				// 12000.
				СвВидТрДок = ПолучитьПодчиненныйЭлемент(ЭкзСведДкПдтвУслуг, "СвВидТрДок");
				ДанныеМнЧ_П12000 = Ветка_П10000.ДанныеМногострочныхЧастей.П12000.Строки;
				Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
					
					ЭкзСвВидТрДок = СкопироватьУзел(ЭкзСведДкПдтвУслуг, СвВидТрДок, ЭкзСведДкПдтвУслуг.Строки.Количество() - 1);
					...
					// 12100.
					ТрДок = ПолучитьПодчиненныйЭлемент(ЭкзСвВидТрДок, "ТрДок");
					ДанныеМнЧ_П12100 = Ветка_П12000.ДанныеМногострочныхЧастей.П12100.Строки;
					Для каждого Ветка_П12100 Из ДанныеМнЧ_П12100 Цикл
						
						ЭкзТрДок = СкопироватьУзел(ЭкзСвВидТрДок, ТрДок, ЭкзСвВидТрДок.Строки.Количество() - 1);
						Для каждого Эл Из ЭкзТрДок.Строки Цикл
							Если Эл.Код = "НомТрДок" Тогда
								ТекЗнач = СокрЛП(Ветка_П12100.Данные[Эл.Ключ]);
								Заполн_ВидТрДок = ЗначениеЗаполнено(Ветка_П12100.Данные["П1210015"]);
								Заполн_ДатаТрДок = ЗначениеЗаполнено(Ветка_П12100.Данные["П1210017"]);
								ВывестиПоказательВXML(Эл,
								?(НЕ ЗначениеЗаполнено(ТекЗнач) И (Заполн_ВидТрДок ИЛИ Заполн_ДатаТрДок), "б/н", ТекЗнач));
							Иначе
								ВывестиПоказательВXML(Эл, Ветка_П12100.Данные[Эл.Ключ]);
							КонецЕсли;
						КонецЦикла;
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_4");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_4_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
...
Процедура ВывестиНаПечатьОтчет(СтруктураПараметров, ВыбранныеСтраницы, СтрПоля = Неопределено)
	...
	Если ВыбранныеСтраницы.Найти("Титульный") <> Неопределено Тогда
		...
		ИмяМакета = "ПечатныйБланк2021Кв2_Титульный";
	...
	Если ВыбранныеСтраницы.Найти("Реестр") <> Неопределено Тогда
		...
		ИмяМакета = "ПечатныйБланк2021Кв2_Реестр";
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				// П91000.
				//
				Строки91 = Строка1.ДанныеМногострочныхЧастей.П91000.Строки;
				
				// Особая обработка для графы 13.
				МасТекЗнач = Новый Массив;
				Для каждого Строка91 Из Строки91 Цикл
					Если ЗначениеЗаполнено(Строка91.Данные.П9100013) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка91.Данные.П9100013));
					КонецЕсли;
				КонецЦикла;
				СтрокаДокумент.Параметры.П9100013 = СтрСоединить(МасТекЗнач, "; ");
				
				// Особая обработка данных П11000, П12000, П12100.
				//
				Строки11 = Строка1.ДанныеМногострочныхЧастей.П11000.Строки;
				КолСтрок11 = Строки11.Количество();
				
				КолСтрок12_121 = 0;
				Строки12 = Строка1.ДанныеМногострочныхЧастей.П12000.Строки;
				Для каждого Строка12 Из Строки12 Цикл
					КолСтрок12_121 = КолСтрок12_121 + Строка12.ДанныеМногострочныхЧастей.П12100.Строки.Количество();
				КонецЦикла;
				
				МаксКолСтрок11_12_121 = Макс(КолСтрок11, КолСтрок12_121);
				
				ИменаГраф11
				= СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы110]["ИменаГраф"];
				ИменаГраф12
				= СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы120]["ИменаГраф"];
				ИменаГраф121
				= СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы121]["ИменаГраф"];
				
				МасДанныхДляВыводаСтрокВПечФорму = Новый Массив;
				
				// Подготовка данных П11000, П12000, П12100 перед выводом в печатную форму.
				Для Инд = 1 По МаксКолСтрок11_12_121 Цикл
					ДанныеДляВыводаСтрокиВПечФорму = Новый Структура;
					Если Инд <= КолСтрок11 Тогда
						Для каждого ИмяГрафы Из ИменаГраф11 Цикл
							ДанныеДляВыводаСтрокиВПечФорму.Вставить(ИмяГрафы, Строки11[Инд - 1].Данные[ИмяГрафы]);
						КонецЦикла;
					КонецЕсли;
					МасДанныхДляВыводаСтрокВПечФорму.Добавить(ДанныеДляВыводаСтрокиВПечФорму);
				КонецЦикла;
				Инд = 0;
				Для каждого Строка12 Из Строки12 Цикл
					Строки121 = Строка12.ДанныеМногострочныхЧастей.П12100.Строки;
					КолСтрок121 = Строки121.Количество();
					Для каждого Строка121 Из Строки121 Цикл
						ДанныеДляВыводаСтрокиВПечФорму = МасДанныхДляВыводаСтрокВПечФорму[Инд];
						Если Строки121.Индекс(Строка121) = 0 Тогда
							Для каждого ИмяГрафы Из ИменаГраф12 Цикл
								ДанныеДляВыводаСтрокиВПечФорму.Вставить(ИмяГрафы, Строка12.Данные[ИмяГрафы]);
							КонецЦикла;
						КонецЕсли;
						Для каждого ИмяГрафы Из ИменаГраф121 Цикл
							ДанныеДляВыводаСтрокиВПечФорму.Вставить(ИмяГрафы, Строка121.Данные[ИмяГрафы]);
						КонецЦикла;
						Если КолСтрок121 > 1 Тогда
							Если Строки121.Индекс(Строка121) = 0 Тогда
								ДанныеДляВыводаСтрокиВПечФорму.Вставить("ОблОбъедСтрока12_Верх");
							ИначеЕсли Строки121.Индекс(Строка121) = КолСтрок121 - 1 Тогда
								ДанныеДляВыводаСтрокиВПечФорму.Вставить("ОблОбъедСтрока12_Низ");
							КонецЕсли;
						КонецЕсли;
						Инд = Инд + 1;
					КонецЦикла;
				...
				// Вывод предварительно подготовленных данных П11000, П12000, П12100 в печатную форму.
				ОблОбъедСтрока12_Верх = Неопределено;
				ОблОбъедСтрока12_Низ  = Неопределено;
				Для Инд = 0 По МасДанныхДляВыводаСтрокВПечФорму.Количество() - 1 Цикл
					...
					СтрокаДокумент.Параметры.Заполнить(ДанныеДляВыводаСтрокиВПечФорму);
					...
					Если ДанныеДляВыводаСтрокиВПечФорму.Свойство("ОблОбъедСтрока12_Верх") Тогда
						ОблОбъедСтрока12_Верх = ПечФорма1СтрокаДокумент.Верх;
					ИначеЕсли ДанныеДляВыводаСтрокиВПечФорму.Свойство("ОблОбъедСтрока12_Низ") Тогда
						ОблОбъедСтрока12_Низ = ПечФорма1СтрокаДокумент.Низ;
					КонецЕсли;
					
					Если ЗначениеЗаполнено(ОблОбъедСтрока12_Верх) И ЗначениеЗаполнено(ОблОбъедСтрока12_Низ) Тогда
						НомерКолонки = 15;
						ОблОбъед = ПечФорма1.Область(ОблОбъедСтрока12_Верх, НомерКолонки, ОблОбъедСтрока12_Низ, НомерКолонки);
						ОблОбъед.Объединить();
						ОблОбъедСтрока12_Верх = Неопределено;
						ОблОбъедСтрока12_Низ  = Неопределено;
					КонецЕсли;
					...
					ИначеЕсли Инд = МасДанныхДляВыводаСтрокВПечФорму.Количество() - 1 Тогда
						ОблОбъедСтрока1_Низ = ПечФорма1СтрокаДокумент.Верх;
					КонецЕсли;
				...
				Если ОблОбъедСтрока1_Верх <> ОблОбъедСтрока1_Низ Тогда
					Для НомерКолонки = 2 По 20 Цикл
						Если НомерКолонки = 6
							ИЛИ (НомерКолонки >= 10 И НомерКолонки <= 13)
							ИЛИ (НомерКолонки >= 15 И НомерКолонки <= 18) Тогда
							Продолжить;
						КонецЕсли;
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Реестр НДС Приложение 4");
...
Процедура СформироватьДанныеОтчета(ОтчетСсылка, СтруктураОтчета)
	...
	ДанныеКС.Вставить("ДекларацияНДС", СтруктураДанных.ДекларацияНДС);
	...
	Для каждого Раздел Из СтруктураДанных.ДанныеМногоуровневыхРазделов Цикл
		...
		Для каждого СтраницаРаздела Из Раздел.Значение.Строки Цикл
			...
			ПолучитьДанныеСтраницы(ДанныеСтраницы, СтраницаРаздела, 
				Новый Структура("Раздел, Уровень", Раздел.Ключ, 1),
				...
				Если НЕ СтрокаМнЧасти.ДанныеМногострочныхЧастей = Неопределено Тогда
					Если НЕ СтрокаМнЧасти.ДанныеМногострочныхЧастей.Количество() = 0 Тогда
						ПолучитьДанныеСтраницы(СтруктураПриемник, СтрокаМнЧасти, ВходящийРаздел,
							Новый Структура("Уровень", ВходящаяМнЧасть.Уровень + "_" + XMLСтрока(НомерСтроки)));
					КонецЕсли;
				КонецЕсли;
...
#Область ОбщиеДанные
	...
	ДанныеКС.Вставить("ГрафаНалоговойБазы"		, "18");
	ДанныеКС.Вставить("ФормыДеклараций"			, "ФормаОтчета2019Кв1,ФормаОтчета2020Кв4");
	ДанныеКС.Вставить("КолвоСтрокРасшифровки"	, 1000);
	
	ПредставлениеРазделов = Новый Структура();
	
	ПредставлениеРазделов.Вставить("Р00", "Титульный лист");
	ПредставлениеРазделов.Вставить("Р01", "Реестра");
	ПредставлениеРазделов.Вставить("Р04", "Раздела 4 Налоговой Декларации по НДС");
	...
	РазделыПоказателей.Вставить("Р01", "Реестр");
	РазделыПоказателей.Вставить("Р04", "Раздел4");
	
	ДанныеКС.Вставить("РазделыПоказателей", РазделыПоказателей);
	
	ПроизвольныеОписанияПолей = Новый Структура();
	
	ПроизвольныеОписанияПолей.Вставить("КодОперации", "Код операции");
	ПроизвольныеОписанияПолей.Вставить("НалБазаИтого", "Налоговая база (итого)");
	
	ДанныеКС.Вставить("ПроизвольныеОписанияПолей", ПроизвольныеОписанияПолей);
	
	МногострочныеЧасти = "";
	ДанныеКС.Вставить("МногострочныеЧасти", МногострочныеЧасти);
	
	ПоляТитульногоЛиста = Новый Структура();
	ПоляТитульногоЛиста.Вставить("Р00С001", "Налоговая Декларация по НДС");
	...
	СоответствиеВидаТранспортаВидуДокумента = Новый Соответствие;
	СоответствиеВидаТранспортаВидуДокумента.Вставить("(...)", "(...)");
	СоответствиеВидаТранспортаВидуДокумента.Вставить("10"	, "02, 06, 07");
	СоответствиеВидаТранспортаВидуДокумента.Вставить("20"	, "03");
	СоответствиеВидаТранспортаВидуДокумента.Вставить("30"	, "01, 05, 12");
	СоответствиеВидаТранспортаВидуДокумента.Вставить("31"	, "01, 05, 12");
	СоответствиеВидаТранспортаВидуДокумента.Вставить("32"	, "01, 05, 12");
	СоответствиеВидаТранспортаВидуДокумента.Вставить("71"	, "12");
	СоответствиеВидаТранспортаВидуДокумента.Вставить("80"	, "02, 06, 07, 12");
	СоответствиеВидаТранспортаВидуДокумента.Вставить("99"	, "12");
	
	ДанныеКС.Вставить("СоответствиеВидаТранспортаВидуДокумента", СоответствиеВидаТранспортаВидуДокумента);
	
#КонецОбласти

#Область ДеревоРеестра

	ТипСтрока		= Новый ОписаниеТипов("Строка",,,, Новый КвалификаторыСтроки(7));
	ТипЧисло		= Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(16, 2));
	ИмяПоказателя 	= "П10000" + ДанныеКС.ГрафаНалоговойБазы + "_";

	ДеревоРеестра = Новый ДеревоЗначений;
	ДеревоРеестра.Колонки.Добавить("КодОперации"	, ТипСтрока);
	ДеревоРеестра.Колонки.Добавить("Страница"		, ТипСтрока);
	ДеревоРеестра.Колонки.Добавить("НомерСтроки"	, ТипСтрока);
	ДеревоРеестра.Колонки.Добавить("НалБазаИтого"	, ТипЧисло);
	ДеревоРеестра.Колонки.Добавить(ИмяПоказателя	, ТипЧисло);
	
	КолвоСтрок = ДанныеКС.Реестр.ВГраница();
	Для Счетчик = 0 По КолвоСтрок Цикл
		
		ДанныеСтраницы	= Новый Структура;
		ДанныеСтраницы.Вставить("КодОперации"	, ДанныеКС.Реестр[Счетчик].КодОперации);
		ДанныеСтраницы.Вставить("НалБазаИтого"	, ДанныеКС.Реестр[Счетчик].НалБазаИтого);
		ДанныеСтраницы.Вставить("Страница"		, ДанныеКС.Реестр[Счетчик].Страница);
		
		СтрокаРеестра = ДеревоРеестра.Строки.Найти(ДанныеСтраницы.КодОперации, "КодОперации");
		СтрокаРеестра = ?(СтрокаРеестра = Неопределено, ДеревоРеестра.Строки.Добавить(), СтрокаРеестра);
		ЗаполнитьЗначенияСвойств(СтрокаРеестра, ДанныеСтраницы);
		
		КолвоСтрок2 = ДанныеКС.Реестр[Счетчик].КолвоСтрокП10000;
		Для Счетчик2 = 1 По КолвоСтрок2 Цикл
			
			НоваяСтрока = СтрокаРеестра.Строки.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ДанныеСтраницы, "Страница");
			НоваяСтрока.НомерСтроки = Счетчик2;
			НоваяСтрока[ИмяПоказателя] = ДанныеКС.Реестр[Счетчик][ИмяПоказателя + XMLСтрока(Счетчик2)]; 
		
		КонецЦикла; 
	КонецЦикла;
	
	Для каждого СтрокаРеестра Из ДеревоРеестра.Строки Цикл
	
		НалоговаяБазаПострочная = 0;
		Для каждого НоваяСтрока Из СтрокаРеестра.Строки Цикл
			НалоговаяБазаПострочная = НалоговаяБазаПострочная + НоваяСтрока[ИмяПоказателя];
		КонецЦикла;
		СтрокаРеестра[ИмяПоказателя] = НалоговаяБазаПострочная;
	
	КонецЦикла;
	
	ДанныеКС.Вставить("ДеревоРеестра", ДеревоРеестра);
	
#КонецОбласти

#Область ДеревоДекларации

	ТипСтрока		= Новый ОписаниеТипов("Строка",,,, Новый КвалификаторыСтроки(7));
	ТипЧисло		= Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(16, 2));
	ИмяПоказателя1 	= "П0004М1001003_";
	ИмяПоказателя2 	= "П0004М1002003_";

	ДеревоДекларации = Новый ДеревоЗначений;
	ДеревоДекларации.Колонки.Добавить("КодОперации"	, ТипСтрока);
	ДеревоДекларации.Колонки.Добавить("Страница"	, ТипСтрока);
	ДеревоДекларации.Колонки.Добавить("НомерСтроки"	, ТипСтрока);
	ДеревоДекларации.Колонки.Добавить("НалБазаИтого", ТипЧисло);
	ДеревоДекларации.Колонки.Добавить(ИмяПоказателя1, ТипСтрока);
	ДеревоДекларации.Колонки.Добавить(ИмяПоказателя2, ТипЧисло);
	ДанныеКС.Вставить("ДанныеДекларацииПолучены"	, Ложь);
	
	Если ЗначениеЗаполнено(ДанныеКС.ДекларацияНДС) 
		И НЕ СтрНайти(ДанныеКС.ФормыДеклараций, ДанныеКС.ДекларацияНДС.ВыбраннаяФорма) = 0 Тогда
		
		ДанныеКС.Вставить("ДанныеДекларацииПолучены", Истина);
		ДанныеДекларации = РегламентированнаяОтчетность.ПолучитьДанныеСохрРегОтчета(ДанныеКС.ДекларацияНДС);
		Раздел4 = ДанныеДекларации.ДанныеРазделовРеглОтчета.Раздел4;
		КолвоСтрок = КоличествоСтрок(Раздел4, ИмяПоказателя1);
		
		Для Счетчик = 1 По КолвоСтрок Цикл
			
			СчетчикСтр 	= XMLСтрока(Счетчик);
			КодОперации = Раздел4[ИмяПоказателя1 + СчетчикСтр];
			НалБаза		= Раздел4[ИмяПоказателя2 + СчетчикСтр];
			
			СтрокаДекларации = ДеревоДекларации.Строки.Найти(КодОперации, "КодОперации");
			СтрокаДекларации = ?(СтрокаДекларации = Неопределено, ДеревоДекларации.Строки.Добавить(), СтрокаДекларации);
			СтрокаДекларации.КодОперации = КодОперации;
			
			НоваяСтрока = СтрокаДекларации.Строки.Добавить();
			НоваяСтрока.Страница = "1";
			НоваяСтрока.НомерСтроки = СчетчикСтр;
			НоваяСтрока[ИмяПоказателя1] = КодОперации; 
			НоваяСтрока[ИмяПоказателя2] = НалБаза; 
			
		КонецЦикла;
	
		Для каждого СтрокаДекларации Из ДеревоДекларации.Строки Цикл
		
			НалоговаяБазаПострочная = 0;
			Для каждого НоваяСтрока Из СтрокаДекларации.Строки Цикл
				НалоговаяБазаПострочная = НалоговаяБазаПострочная + НоваяСтрока[ИмяПоказателя2];
			КонецЦикла;
			СтрокаДекларации.НалБазаИтого = НалоговаяБазаПострочная;
		
		КонецЦикла;
	
	КонецЕсли; 
	
	ДанныеКС.Вставить("ДеревоДекларации", ДеревоДекларации);
...
Процедура СформироватьПоказатели(Показатели, ИменаПоказателей)
	...
	Для каждого Элемент Из ИменаПоказателей Цикл
		...
		ПодстрокаДополнения	= "";
		ПодстрокаСтроки		= ?(ИндексРаздела = "Р04", Сред(Элемент.Значение, 9, 3), "");
		ПроизвольноеОписание= "";
		ПодстрокаГрафы		= Сред(Элемент.Значение, 7,2);
		...
		Иначе
			Если ДанныеКС.ПроизвольныеОписанияПолей.Свойство(Элемент.Значение, ПроизвольноеОписание) Тогда
				МассивПредставления.Добавить(ПроизвольноеОписание);
		...
		Многострочный		= СтрЗаканчиваетсяНа(Элемент.Значение, "_");
		...
		ДлинаИмени			= ?(ИндексРаздела = "Р04", 14, 9);
		Отчет				= ?(ИндексРаздела = "Р04", ДанныеКС.ДекларацияНДС, Неопределено);
		...
		Показатели[Элемент.Ключ].Вставить("ДлинаИмени"		, ДлинаИмени);
		
		// переменные данные
		Показатели[Элемент.Ключ].Вставить("Отчет"			, Отчет);
...
Процедура ЗаполнитьПоказатель(Показатель, Источник, 
	...
	Если ТипЗнч(Источник) = Тип("Структура") Тогда
		...
		Если ЗначениеЗаполнено(НомерСтроки) И ЗначениеЗаполнено(НомерСтроки2) И ЗначениеЗаполнено(НомерСтроки3) Тогда
			Показатель.ИмяПоказателя = Лев(Показатель.ИмяПоказателя, Показатель.ДлинаИмени) + XMLСтрока(НомерСтроки) + "_" + 
				XMLСтрока(НомерСтроки2) + "_" + XMLСтрока(НомерСтроки3);
		ИначеЕсли ЗначениеЗаполнено(НомерСтроки) И ЗначениеЗаполнено(НомерСтроки2) Тогда
			Показатель.ИмяПоказателя = Лев(Показатель.ИмяПоказателя, Показатель.ДлинаИмени) + XMLСтрока(НомерСтроки) + "_" + 
				XMLСтрока(НомерСтроки2);
		ИначеЕсли ЗначениеЗаполнено(НомерСтроки) Тогда
			Показатель.ИмяПоказателя = Лев(Показатель.ИмяПоказателя, Показатель.ДлинаИмени) + XMLСтрока(НомерСтроки);
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("СтрокаДереваЗначений") Тогда
		...
		Если Показатель.Многострочный Тогда
			Показатель.ИмяПоказателя = Лев(Показатель.ИмяПоказателя, Показатель.ДлинаИмени)
		КонецЕсли;
...
Функция ОбработатьНастройки(Знач ИзменяемыеСвойства, Знач Настройки = Неопределено)
	...
	Если Настройки = Неопределено Тогда
		...
		Настройки.Вставить("ШаблонДополнения"		, (НСтр("ru=''")));
...
Функция РазделЗаполнен(Раздел)
	...
	Для каждого Элемент Из Раздел Цикл
		Если НЕ ДанныеКС.МассивПолейИсключений.Найти(Элемент.Ключ) = Неопределено
			...
			ИЛИ НЕ СтрНайти(Элемент.Ключ, "Страница") = 0
			ИЛИ НЕ СтрНайти(Элемент.Ключ, "П1000001_") = 0
			Тогда
			Продолжить;
		КонецЕсли; 
...
&НаСервере
Процедура ПроверкаКС_4_1_1()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.1.1';
													|en = '4.1.1'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 4';
														|en = 'Реестр 4'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Номер строки по порядку = номер предшествующей строки, увеличенный на 1';
			|en = 'Номер строки по порядку = номер предшествующей строки, увеличенный на 1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Нумерация строк не нарушена';
													|en = 'Нумерация строк не нарушена'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный номер строки';
														|en = 'Ошибочный номер строки'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	Заголовки.Вставить("ШаблонДополнения"	, НСтр("ru = 'Страница %1, Строка %2';
													|en = 'Страница %1, Строка %2'"));
	...
	ИменаПоказателей.Вставить("Р01Г001", "П1000001_");
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		
		ЗарегистрироватьФормулу(Настройки);
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			Показатели.Р01Г001.Дополнение = ПодставитьПараметрыВСтроку(Настройки.ШаблонДополнения, 
				СтраницаРеестра.Страница, XMLСтрока(Счетчик));
			ЗаполнитьИРасшифроватьПоказатели(Показатели, СтраницаРеестра, Счетчик);
		КонецЦикла;
	
		Настройки.ЭтоОшибка = Ложь;
		Настройки.Расшифровка = Настройки.ШаблонРасшифровки;
		Настройки.ДопСведения = ПодставитьПараметрыВСтроку(Настройки.ШаблонДопСведений, СтраницаРеестра.Страница);
...
&НаСервере
Процедура ПроверкаКС_4_1_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.1.2';
													|en = '4.1.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 4';
														|en = 'Реестр 4'"));
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Итого налоговая база = сумме налоговых баз по каждой строке Реестра по соответствующему коду операции';
			|en = 'Итого налоговая база = сумме налоговых баз по каждой строке Реестра по соответствующему коду операции'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 по коду операции %3';
													|en = '%1 = %2 по коду операции %3'"));
	...
	ИмяКолонкиНалоговойБазы = "П10000" + ДанныеКС.ГрафаНалоговойБазы + "_"; 
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01С001"		, "КодОперации");
	ИменаПоказателей.Вставить("Р01С002"		, "НалБазаИтого");
	ИменаПоказателей.Вставить("Р01Г001"		, ИмяКолонкиНалоговойБазы);
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	
	Для каждого СтрокаДерева Из ДанныеКС.ДеревоРеестра.Строки Цикл
			
		ЗарегистрироватьФормулу(Настройки);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01С001, СтрокаДерева);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01С002, СтрокаДерева);
		
		Счетчик = 1;
		КолвоСтрок = Мин(СтрокаДерева.Строки.Количество(), ДанныеКС.КолвоСтрокРасшифровки);
		Для каждого ПодстрокаДерева Из СтрокаДерева.Строки Цикл
			
			Показатели.Р01Г001.Дополнение = ПодставитьПараметрыВСтроку(Настройки.ШаблонДополнения, ПодстрокаДерева.Страница,
				ПодстрокаДерева.НомерСтроки);
			ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01Г001, ПодстрокаДерева, ПодстрокаДерева.НомерСтроки);
		...
		РасчетнаяСумма = Окр(СтрокаДерева[ИмяКолонкиНалоговойБазы]);
		Настройки.ЭтоОшибка = НЕ Показатели.Р01С002.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р01С002.Значение, 
			РасчетнаяСумма, Показатели.Р01С001.ЗначениеСтр);
		Настройки.ДопСведения = ПодставитьПараметрыВСтроку(Настройки.ШаблонДопСведений, Показатели.Р01С001.ЗначениеСтр);
		ДополнитьСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверкаКС_4_1_3а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.1.3.а';
													|en = '4.1.3.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 4';
														|en = 'Реестр 4'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Если признак таможенного декларирования в гр.2 Реестра = 1, то ';
			|en = 'Если признак таможенного декларирования в гр.2 Реестра = 1, то '") +
		НСтр("ru = 'регистрационный номер ГТД должен состоять из 23 знаков, из них 9-й и 16-й - косая черта ';
			|en = 'регистрационный номер ГТД должен состоять из 23 знаков, из них 9-й и 16-й - косая черта '") +
		НСтр("ru = 'вправо, и иметь следующий формат: TTTTTTTT/DDMMGG/NNNNNNN';
			|en = 'вправо, и иметь следующий формат: TTTTTTTT/DDMMGG/NNNNNNN'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Регистрационный номер ГТД %1 соответствует формату';
													|en = 'Регистрационный номер ГТД %1 соответствует формату'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный формат номера ГТД';
														|en = 'Ошибочный формат номера ГТД'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001"		, "П9100013_");
	ИменаПоказателей.Вставить("Р01Г002"		, "П1000002_");
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			ЗаполнитьПоказатель(Показатели.Р01Г002, СтраницаРеестра, Счетчик);
			Для Счетчик2 = 1 По СтраницаРеестра["КолвоСтрокП91000_" + XMLСтрока(Счетчик)]  Цикл
			
				ЗарегистрироватьФормулу(Настройки);
				Дополнение = ПодставитьПараметрыВСтроку(Настройки.ШаблонДопСведений, СтраницаРеестра.Страница,
					XMLСтрока(Счетчик) + "-" + XMLСтрока(Счетчик2));
				Дополнение2 = ПодставитьПараметрыВСтроку(Настройки.ШаблонДопСведений, СтраницаРеестра.Страница,
					XMLСтрока(Счетчик));
				Показатели.Р01Г002.Дополнение = Дополнение2;
				РасшифроватьПоказатель(Показатели.Р01Г002);
				Показатели.Р01Г001.Дополнение = Дополнение;
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01Г001, СтраницаРеестра, Счетчик, Счетчик2);
		
				Настройки.ЭтоОшибка = Показатели.Р01Г002.Значение = "1" И ЗначениеЗаполнено(Показатели.Р01Г001.Значение)
					И НЕ (СтрДлина(СокрЛП(Показатели.Р01Г001.Значение)) = 23 И 
					КодСимвола(Сред(СокрЛП(Показатели.Р01Г001.Значение), 9, 1)) = 47 И 
					КодСимвола(Сред(СокрЛП(Показатели.Р01Г001.Значение), 16, 1)) = 47);
				Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р01Г001.ЗначениеСтр);
				Настройки.ДопСведения = Дополнение;
				ДополнитьСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверкаКС_4_1_3б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.1.3.б';
													|en = '4.1.3.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 4';
														|en = 'Реестр 4'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Если признак таможенного декларирования в гр.2 Реестра = 1, то ';
			|en = 'Если признак таможенного декларирования в гр.2 Реестра = 1, то '") +
		НСтр("ru = 'регистрационный номер ГТД с 10 по 15 символы должен содержать дату в формате DDMMGG';
			|en = 'регистрационный номер ГТД с 10 по 15 символы должен содержать дату в формате DDMMGG'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Дата в регистрационном номере ГТД %1 соответствует формату';
													|en = 'Дата в регистрационном номере ГТД %1 соответствует формату'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочная дата в номере ГТД';
														|en = 'Ошибочная дата в номере ГТД'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1, Строка %2';
													|en = 'Страница %1, Строка %2'"));
	Настройки = ОбработатьНастройки(Заголовки);
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			...
			Для Счетчик2 = 1 По СтраницаРеестра["КолвоСтрокП91000_" + XMLСтрока(Счетчик)]  Цикл
				...
				ДатаГТДИсходная	= Сред(СокрЛП(Показатели.Р01Г001.Значение), 10, 6);
				ДатаГТД = Сред(ДатаГТДИсходная, 5) + Сред(ДатаГТДИсходная, 3, 2) + Лев(ДатаГТДИсходная, 2);
				ДатаГТД = "20" + ДатаГТД;
				ОписаниеТипа = Новый ОписаниеТипов("Дата");
				ДатаГТДИтог  = ОписаниеТипа.ПривестиЗначение(ДатаГТД);
		
				Настройки.ЭтоОшибка = Показатели.Р01Г002.Значение = "1" И ЗначениеЗаполнено(Показатели.Р01Г001.Значение)
					И НЕ ЗначениеЗаполнено(ДатаГТДИтог);
				Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
					?(ЗначениеЗаполнено(ДатаГТДИсходная), ДатаГТДИсходная, "(...)"));
				Настройки.ДопСведения = Дополнение;
				ДополнитьСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверкаКС_4_1_3в()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.1.3.в';
													|en = '4.1.3.в'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 4';
														|en = 'Реестр 4'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Если признак таможенного декларирования в гр.2 Реестра = 1, то ';
			|en = 'Если признак таможенного декларирования в гр.2 Реестра = 1, то '") +
		НСтр("ru = 'регистрационный номер ГТД с 14 по 15 символы должен содержать год оформления ГТД в формате GG, ';
			|en = 'регистрационный номер ГТД с 14 по 15 символы должен содержать год оформления ГТД в формате GG, '") +
		НСтр("ru = 'не менее текущего, уменьшенного на 4, и не больше текущего';
			|en = 'не менее текущего, уменьшенного на 4, и не больше текущего'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 <= %2 <= %3';
													|en = '%1 <= %2 <= %3'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный год оформления ГТД';
														|en = 'Ошибочный год оформления ГТД'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1, Строка %2';
													|en = 'Страница %1, Строка %2'"));
	Настройки = ОбработатьНастройки(Заголовки);
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			...
			Для Счетчик2 = 1 По СтраницаРеестра["КолвоСтрокП91000_" + XMLСтрока(Счетчик)]  Цикл
				...
				ГодГТД			= Сред(СокрЛП(Показатели.Р01Г001.Значение), 14, 2);
				ГодТекущий		= ДанныеКС.ГодОтчета - 2000;
				ГодКрайний		= ГодТекущий - 4;
				УсловиеПоГоду	= Ложь;
				ГодТолькоЦифры	= ЗначениеЗаполнено(ГодГТД) И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ГодГТД);
				Если ГодТолькоЦифры Тогда
					УсловиеПоГоду = ГодКрайний <= Число(ГодГТД) И Число(ГодГТД) <= ГодТекущий;
				КонецЕсли;
				
				Настройки.ЭтоОшибка = Показатели.Р01Г002.Значение = "1" И ЗначениеЗаполнено(Показатели.Р01Г001.Значение)
					И НЕ (ГодТолькоЦифры И УсловиеПоГоду);
				Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
					ГодКрайний, ?(ЗначениеЗаполнено(ГодГТД), ГодГТД, "(...)"), ГодТекущий);
				Настройки.ДопСведения = Дополнение;
				ДополнитьСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверкаКС_4_1_4()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.1.4';
													|en = '4.1.4'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 4';
														|en = 'Реестр 4'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'п.3.2 ст.165 НК РФ';
													|en = 'п.3.2 ст.165 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код вида транспортного средства должен соответствовать коду товаросопроводительного документа:
		|-если код вида ТС = 30, 31, 32, то код вида товаросопроводительного документа = 01, 05, 12; 
		|-если код вида ТС = 10, 80, то код вида товаросопроводительного документа = 02, 06, 07; 
		|-если код вида ТС = 20, то код вида товаросопроводительного документа = 03; 
		|-если код вида ТС = 71, 80, 99, то код вида товаросопроводительного документа = 12';
		|en = 'Код вида транспортного средства должен соответствовать коду товаросопроводительного документа:
		|-если код вида ТС = 30, 31, 32, то код вида товаросопроводительного документа = 01, 05, 12; 
		|-если код вида ТС = 10, 80, то код вида товаросопроводительного документа = 02, 06, 07; 
		|-если код вида ТС = 20, то код вида товаросопроводительного документа = 03; 
		|-если код вида ТС = 71, 80, 99, то код вида товаросопроводительного документа = 12'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2, когда %3 = %4';
													|en = '%1 = %2, когда %3 = %4'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный код вида товаросопроводительного документа';
														|en = 'Ошибочный код вида товаросопроводительного документа'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1, Строка %2';
													|en = 'Страница %1, Строка %2'"));
	Настройки = ОбработатьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001"		, "П1200014_");
	ИменаПоказателей.Вставить("Р01Г002"		, "П1210015_");
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			Для Счетчик2 = 1 По СтраницаРеестра["КолвоСтрокП12000_" + XMLСтрока(Счетчик)]  Цикл
				ЗаполнитьПоказатель(Показатели.Р01Г001, СтраницаРеестра, Счетчик, Счетчик2);
					Для Счетчик3 = 1 По СтраницаРеестра["КолвоСтрокП12100_" + XMLСтрока(Счетчик) + "_" + XMLСтрока(Счетчик2)]  Цикл
			
						ЗарегистрироватьФормулу(Настройки);
						Дополнение = ПодставитьПараметрыВСтроку(Настройки.ШаблонДопСведений, СтраницаРеестра.Страница,
							XMLСтрока(Счетчик) + "-" + XMLСтрока(Счетчик2) + "-" + XMLСтрока(Счетчик3));
						Дополнение2 = ПодставитьПараметрыВСтроку(Настройки.ШаблонДопСведений, СтраницаРеестра.Страница,
							XMLСтрока(Счетчик) + "-" + XMLСтрока(Счетчик2));
						Показатели.Р01Г001.Дополнение = Дополнение2;
						РасшифроватьПоказатель(Показатели.Р01Г001);
						Показатели.Р01Г002.Дополнение = Дополнение;
						ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01Г002, СтраницаРеестра, Счетчик, Счетчик2, Счетчик3);
						
						СтрокаЗначений = ДанныеКС.СоответствиеВидаТранспортаВидуДокумента.Получить(Показатели.Р01Г001.ЗначениеСтр);
						Настройки.ЭтоОшибка = НЕ СтрокаЗначений = Неопределено И СтрНайти(СтрокаЗначений, 
							Показатели.Р01Г002.ЗначениеСтр) = 0;
						Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р01Г002.ЗначениеСтр,
							СтрокаЗначений, Показатели.Р01Г001.ЗначениеСтр, Показатели.Р01Г001.ЗначениеСтр);
						Настройки.ДопСведения = Дополнение;
						ДополнитьСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверкаКС_4_2_1а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.2.1.а';
													|en = '4.2.1.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 4';
														|en = 'Реестр 4'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'На Титульном листе Реестра должна быть указана Налоговая Декларация по НДС ';
			|en = 'На Титульном листе Реестра должна быть указана Налоговая Декларация по НДС '"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Указанная декларация: %1';
													|en = 'Указанная декларация: %1'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не указана Налоговая Декларация по НДС';
														|en = 'Не указана Налоговая Декларация по НДС'"));
	Настройки = ОбработатьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р00С001"		, "ДекларацияНДС");
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		РеестрЗаполнен = РеестрЗаполнен ИЛИ СтраницаРеестра.РазделЗаполнен;
	КонецЦикла;
	
	Настройки.ЭтоОшибка = РеестрЗаполнен И НЕ ЗначениеЗаполнено(ДанныеКС.ДекларацияНДС);
	Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р00С001.ЗначениеСтр);
	ДополнитьСтруктуруОшибок(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_4_2_1б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.2.1.б';
													|en = '4.2.1.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 4';
														|en = 'Реестр 4'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код операции = стр.010 Раздела 4 Налоговой Декларации по НДС, указанной на Титульном  ';
			|en = 'Код операции = стр.010 Раздела 4 Налоговой Декларации по НДС, указанной на Титульном  '") +
		НСтр("ru = 'листе';
			|en = 'листе'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2';
													|en = '%1 = %2'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный код операции';
														|en = 'Ошибочный код операции'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Код операции %1';
													|en = 'Код операции %1'"));
	Заголовки.Вставить("ШаблонДополнения"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	Настройки = ОбработатьНастройки(Заголовки);
	...
	Настройки.Вставить("ШаблонДополнения2"	, НСтр("ru = 'Страница %1, Строка %2';
													|en = 'Страница %1, Строка %2'"));
	...
	Для каждого СтрокаДерева Из ДанныеКС.ДеревоРеестра.Строки Цикл
		...
		ЗарегистрироватьФормулу(Настройки);
		Показатели.Р01С001.Дополнение = ПодставитьПараметрыВСтроку(Настройки.ШаблонДополнения, СтрокаДерева.Страница);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01С001, СтрокаДерева);
		КодНайден = Ложь;
		Показатели.Р04С010.ЗначениеСтр = "(...)";
		
		СтрокаДекларации = ДанныеКС.ДеревоДекларации.Строки.Найти(Показатели.Р01С001.Значение, "КодОперации");
		Если ДанныеКС.ДанныеДекларацииПолучены И НЕ СтрокаДекларации = Неопределено Тогда
		
			Для каждого ПодстрокаДекларации Из СтрокаДекларации.Строки Цикл
				Если ПодстрокаДекларации.П0004М1001003_ = Показатели.Р01С001.Значение Тогда
					КодНайден = Истина;
					Показатели.Р04С010.Дополнение = ПодставитьПараметрыВСтроку(Настройки.ШаблонДополнения2, 
						ПодстрокаДекларации.Страница, ПодстрокаДекларации.НомерСтроки);
					ЗаполнитьИРасшифроватьПоказатель(Показатели.Р04С010, ПодстрокаДекларации, ПодстрокаДекларации.НомерСтроки);
				КонецЕсли; 
			КонецЦикла;
		КонецЕсли;
		
		Настройки.ЭтоОшибка = ДанныеКС.ДанныеДекларацииПолучены И НЕ КодНайден;
...
&НаСервере
Процедура ПроверкаКС_4_2_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4.2.2';
													|en = '4.2.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 4';
														|en = 'Реестр 4'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Итого налоговая база = стр.020 Раздела 4 Налоговой Декларации по НДС, указанной на Титульном  ';
			|en = 'Итого налоговая база = стр.020 Раздела 4 Налоговой Декларации по НДС, указанной на Титульном  '") +
		НСтр("ru = 'листе, по соответствующему коду операции';
			|en = 'листе, по соответствующему коду операции'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 по коду операции %3';
													|en = '%1 = %2 по коду операции %3'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочная сумма налоговой базы';
														|en = 'Ошибочная сумма налоговой базы'"));
	...
	ИменаПоказателей.Вставить("Р01С001"		, "КодОперации");
	ИменаПоказателей.Вставить("Р01С002"		, "НалБазаИтого");
	ИменаПоказателей.Вставить("Р04С020"		, "П0004М1002003_");
	...
	Для каждого СтрокаДерева Из ДанныеКС.ДеревоРеестра.Строки Цикл
		...
		ЗарегистрироватьФормулу(Настройки);
		Дополнение = ПодставитьПараметрыВСтроку(Настройки.ШаблонДополнения, СтрокаДерева.Страница);
		Показатели.Р01С001.Дополнение = Дополнение;
		Показатели.Р01С002.Дополнение = Дополнение;
		...
		Если ДанныеКС.ДанныеДекларацииПолучены И НЕ СтрокаДекларации = Неопределено Тогда
			...
			Для каждого ПодстрокаДекларации Из СтрокаДекларации.Строки Цикл
				Показатели.Р04С020.Дополнение = ПодставитьПараметрыВСтроку(Настройки.ШаблонДополнения2, 
					ПодстрокаДекларации.Страница, ПодстрокаДекларации.НомерСтроки);
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р04С020, ПодстрокаДекларации, ПодстрокаДекларации.НомерСтроки);
			КонецЦикла;
			РасчетнаяСумма = Окр(СтрокаДекларации.НалБазаИтого);
		КонецЕсли;
		
		Настройки.ЭтоОшибка = ДанныеКС.ДанныеДекларацииПолучены И НЕ Показатели.Р01С002.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р01С002.Значение, 
			РасчетнаяСумма, Показатели.Р01С001.ЗначениеСтр);
		Настройки.ДопСведения = ПодставитьПараметрыВСтроку(Настройки.ШаблонДопСведений, Показатели.Р01С001.ЗначениеСтр);
		ДополнитьСтруктуруОшибок(Настройки);
