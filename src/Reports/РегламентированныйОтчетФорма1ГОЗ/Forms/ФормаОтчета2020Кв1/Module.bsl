...
Процедура Печать(ВидПечати, НеИзФормыОтчета = Ложь) Экспорт
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1. Выполняются проверки перед началом печати...';
			|en = '%1. Выполняются проверки перед началом печати...'"), Заголовок), , , БиблиотекаКартинок.Печать);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Если СтруктураРеквизитовФормы.ЕдиницаИзмерения <> ПорядкиОкругленияОтчетности.ПеречислениеПорядкиОкругленияОтчетностиОкр1000
		...
		Сообщение.Текст = НСтр("ru = 'Печать возможна только при установленной единице измерения ""в тысячах рублей"" и точности ""0"".';
								|en = 'Печать возможна только при установленной единице измерения ""в тысячах рублей"" и точности ""0"".'");
		Сообщение.Сообщить();
...
Процедура ВывестиНаПечатьОтчет(СтруктураПараметров, ВыбранныеСтраницы, СтрПоля = Неопределено)
	...
	Если ВыбранныеСтраницы.Найти("Раздел1") <> Неопределено Тогда
		...
		ИмяМакета_Стр1 = "ПечатныйБланк2020Кв1_Раздел1";
		
		ПечФорма.Очистить();
		
		Макет_Стр1 = ОбъектОтчета(ИмяФормы).ПолучитьМакет(ИмяМакета_Стр1);
	...
	Если ВыбранныеСтраницы.Найти("Раздел2") <> Неопределено Тогда
		...
		ИмяМакета_Стр2 = "ПечатныйБланк2020Кв1_Раздел2";
		
		ПечФорма.Очистить();
		
		Макет_Стр2 = ОбъектОтчета(ИмяФормы).ПолучитьМакет(ИмяМакета_Стр2);
			...
			Если СтруктураРеквизитовФормы.ЕстьРазмещение Тогда
				...
				Для каждого СтраницаРаздела Из ДеревоДанныхРаздела.Строки Цикл
					...
					Если Найти(СтраницаРаздела.Данные.ТипСведений, "Размещение") > 0 Тогда
						...
						// Обработка для комбинированной ячейки
						ПечСтрока2.Параметры["П0000200007"] = СокрЛП(СтраницаРаздела.Данные["П000020009104"]) + "," + Символы.ПС + СокрЛП(СтраницаРаздела.Данные["П000020000704"]);
	...
	Если ВыбранныеСтраницы.Найти("Раздел3") <> Неопределено Тогда
		...
		ИмяМакета_Стр3 = "ПечатныйБланк2020Кв1_Раздел3";
		
		ПечФорма.Очистить();
		
		Макет_Стр3 = ОбъектОтчета(ИмяФормы).ПолучитьМакет(ИмяМакета_Стр3);
