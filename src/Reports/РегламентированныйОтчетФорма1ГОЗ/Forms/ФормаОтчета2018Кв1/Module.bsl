...
Процедура Печать(ВидПечати, НеИзФормыОтчета = Ложь) Экспорт
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1. Выполняются проверки перед началом печати...';
			|en = '%1. Выполняются проверки перед началом печати...'"), ЭтаФорма.Заголовок), , , БиблиотекаКартинок.Печать);
...
Функция ЗаполнитьАвтоНаСервере()
	...
	ПараметрыПроцедуры.Вставить("ИДОтчета", ИДОтчета(ЭтаФорма.ИмяФормы));
	ПараметрыПроцедуры.Вставить("ИДРедакцииОтчета", ИДРедакцииОтчета(ЭтаФорма.ИмяФормы));
