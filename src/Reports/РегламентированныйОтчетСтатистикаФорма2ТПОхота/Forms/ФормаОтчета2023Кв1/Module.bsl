...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0000201033";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 Тогда
		УдалитьСтроку("");
	ИначеЕсли Лев(Область.Имя, 13) = "П000020103301" Или Лев(Область.Имя, 13) = "П000020103302" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "СправочникКодыЖивотных");
		Если СтрНайти(Область.Имя, "П000020103301") = 1 Тогда
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Лев(Область.Имя, 13) = "П000020103301" Или Лев(Область.Имя, 13) = "П000020103302" Тогда
	
		РезультатКод = РезультатВыбора["Код"];
		РезультатНазвание = Строка(РезультатВыбора["Название"]);
		
		Если Лев(Область.Имя, 13) = "П000020103301" Тогда
			Область.Значение = ?(ЗначениеЗаполнено(РезультатКод), РезультатНазвание, "");
			ЭтотОбъект.ПолеТабличногоДокументаФормаОтчета.Области["П000020103302_" + Сред(Область.Имя, 15)].Значение =
				?(ЗначениеЗаполнено(РезультатКод), РезультатКод, "");
		ИначеЕсли Лев(Область.Имя, 13) = "П000020103302" Тогда
			Область.Значение = ?(ЗначениеЗаполнено(РезультатКод), РезультатКод, "");
			ЭтотОбъект.ПолеТабличногоДокументаФормаОтчета.Области["П000020103301_" + Сред(Область.Имя, 15)].Значение =
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	СуммаПоКол = 0;
	Для НомСтр = 1 По Форма.П0000201033.Количество() Цикл
		СуммаПоКол = СуммаПоКол + Области["П000020103305_" + НомСтр].Значение;
	КонецЦикла;
	Области["П000020103403"].Значение = СуммаПоКол;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П000020103403"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000020103403"].Защита   = Не ОтклАвтоРасчетЗначение;
