	...
	Если ИмяПоказателя = "П000260000600"
		...
		Если ДлинаКодСубРФ = 2 ИЛИ ДлинаКодСубРФ = 0 Тогда
			Если КодСубРФ = "00" И (ИмяПоказателя <> "П000П20000000") Тогда
				Область.Значение = "";
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru = 'Код субъекта РФ не должен быть равен ""00"". Значение очищено';
	...
	Если ИмяПоказателя = "КППРеорганизованнойОрганизации" ИЛИ ИмяПоказателя = "П000250000400"
		...
		Если ДлинаКПП = 9 ИЛИ ДлинаКПП = 0 Тогда
			Если ИмяПоказателя = "П000250000400" И ДлинаКПП = 9 И Сред(КПП, 5, 2) = "50" Тогда
				Область.Значение = "";
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='В Приложении 5 к Листу 02 не применяется КПП, присвоенный "
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Лист02" Тогда
		...
		Лист02_ПрНП_20 = ВеткаУдовлетворяющаяУсловиямПоиска(Листы02, Новый Структура("П000020000100", "20"));
								...
								Если ЕстьЛист05ИлиЛист06 Тогда
									...
									Для каждого Лист02_Поиск Из ДеревоДанных.Строки Цикл
										...
										Если Лист02_4_ПрНП_20 = Неопределено Тогда
											Лист02_4_ПрНП_20 = ВеткаУдовлетворяющаяУсловиямПоиска(
...
Процедура ПроверкаКС_1_41()
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = '""Признак налогоплательщика"" равен 01';
			|en = '""Признак налогоплательщика"" равен 01'"));
...
Процедура ПроверкаКС_1_42()
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = '""Признак налогоплательщика"" равен 02 или код по реквизиту ""По месту нахождения"" равен ';
			|en = '""Признак налогоплательщика"" равен 02 или код по реквизиту ""По месту нахождения"" равен '") +
...
Процедура ПроверкаКС_1_43()
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = '""Признак налогоплательщика"" равен 03 (резиденты Калининградской ОЭЗ)';
			|en = '""Признак налогоплательщика"" равен 03 (резиденты Калининградской ОЭЗ)'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 <= 2';
													|en = '%1 <= 2'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочная величина ставки налога';
														|en = 'Ошибочная величина ставки налога'"));
...
Процедура ПроверкаКС_1_116()
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		Для каждого Лист02_4 Из Лист02.Лист02_4 Цикл
			...
			Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, Показатели.П24С001.Значение, 
				Показатели.П24С002.Значение, Показатели.П24С003.Значение, Разница);
			ДополнитьСтруктуруОшибок(Настройки);
