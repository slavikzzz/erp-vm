...
Функция ПолучитьСрокПредставленияОтчетности(Форма)
	...
	ИначеЕсли ПоследнийМесяцПериода % 3 = 0 Тогда
		...
		ДатаСрокаПредставления = Дата(ГодПериода, ПоследнийМесяцПериода + 1, ?(ГодПериода >= 2023, 25, 28));
		СтрСрокПредставления = СтрСрокПредставления + Формат(ДатаСрокаПредставления,
...
Процедура ПоказатьПериод(Форма)
	...
	Форма.НадписьКтоСдаетОтчет = НСтр("ru = 'Только организации (п.1 ст.246 НК РФ).';
										|en = 'Только организации (п.1 ст.246 НК РФ).'");
...
Процедура ПолеВыбораПериодичностиПоказаПериодаПриИзменении(Элемент)
	...
	// Исключаем конфликт выбора варианта кодов периода в этом поле и настроек, используемых помощником
	ОтказИспользованияПомощника = Истина;
...
Процедура ОткрытьФормуОтчета(Команда)
	...
	Заполнить(ОтказИспользованияПомощника);
...
Процедура Заполнить(БезПомощника = Ложь)
	...
	Если Не БезПомощника Тогда
		ДанныеЗаполнениеВПомощнике = ЗаполнениеВПомощнике(Организация, мДатаКонцаПериодаОтчета);
		
		Если ДанныеЗаполнениеВПомощнике.ОтчетМожноЗаполнитьВПомощнике Тогда
			
			РегламентированнаяОтчетностьКлиентПереопределяемый.ОткрытьФормуПомощникаУплатыНалога(
				Организация, мДатаКонцаПериодаОтчета, ДанныеЗаполнениеВПомощнике.ПараметрыПравила);
			Закрыть();
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗаполнениеВПомощнике(Знач Организация, Знач ДатаКонцаПериода)
	
	Возврат ИнтерфейсыВзаимодействияБРО.ДанныеДляЗаполненияОтчетаВПомощнике(Организация, ДатаКонцаПериода);
...
Процедура ВыбратьФорму(Команда)
	...
	ДопТекстОписания = НСтр("ru = 'Декларацию по налогу на прибыль представляют организации (п.1 ст.246 НК РФ).';
							|en = 'Декларацию по налогу на прибыль представляют организации (п.1 ст.246 НК РФ).'");
...
Функция ДоступныеОрганизацииОтсутствуютТекст()
	...
	|en = 'Декларацию по налогу на прибыль представляют только организации (п.1 ст.246 НК РФ).
	|В справочнике ""Организации"" сведения об организациях отсутствуют.'");
...
Процедура ОтобразитьПоясненияКПериодуОтчета(Форма)
	...
	Если Форма.ГраницыПервогоНалоговогоПериода <> Неопределено Тогда
		...
		Если КонецГода(Форма.мДатаКонцаПериодаОтчета) < Форма.ГраницыПервогоНалоговогоПериода.Конец
			...
			|en = 'Декларацию по налогу на прибыль за %1 сдавать не нужно. Период с даты регистрации %2 по %3 включается в декларации за отчетные периоды %4.'");
		...
		ИначеЕсли КонецГода(Форма.мДатаКонцаПериодаОтчета) = Форма.ГраницыПервогоНалоговогоПериода.Конец Тогда
			...
			ШаблонСообщения = НСтр("ru = 'Период с даты регистрации %1 по %2 включается в декларацию по налогу на прибыль за %3.';
									|en = 'Период с даты регистрации %1 по %2 включается в декларацию по налогу на прибыль за %3.'");
