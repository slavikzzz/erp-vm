...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КолКолонокИдГруппы1 = 23;
	КолКолонокИдГруппы2 = 25;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010000224", "П000010000124");
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010000424", "П000010000324");
...
Процедура УстановитьДоступностьПолейДекларация(Форма)
	...
	Пока Форма.ТабличныйДокумент.Области.Найти("П000010000101_" + Постфикс) <> Неопределено Цикл
		...
		УстановитьЦветФонаЯчейки(Форма, "П000010000124_" + Постфикс, ВариантРучногоВвода, "Декларация");
	...
	УстановитьЦветФонаЯчейки(Форма, "П000010000224", ВариантАвторасчета, "Декларация");
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", Знач ОбластьИмя = "") Экспорт
	...
	Если ИмяТекущейСтраницыПанели = "Декларация" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000010000224, ОбластьИмя) > 0 Тогда
			...
			КоличествоСтрок = СтруктураГруппы.Количество();
			Форма.ТабличныйДокумент.Области.П000010000224.Значение = 0;
			Для Инд = 1 По КоличествоСтрок Цикл
				Постфикс = "_" + Формат(Инд, "ЧГ=0");
				Форма.ТабличныйДокумент.Области.П000010000224.Значение = Форма.ТабличныйДокумент.Области.П000010000224.Значение + Форма.ТабличныйДокумент.Области["П000010000124" + Постфикс].Значение;
			КонецЦикла;
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000010000424, ОбластьИмя) > 0 Тогда
			...
			КоличествоСтрок = СтруктураГруппы.Количество();
			Форма.ТабличныйДокумент.Области.П000010000424.Значение = 0;
			Для Инд = 1 По КоличествоСтрок Цикл
				Постфикс = "_" + Формат(Инд, "ЧГ=0");
				Форма.ТабличныйДокумент.Области.П000010000424.Значение = Форма.ТабличныйДокумент.Области.П000010000424.Значение + Форма.ТабличныйДокумент.Области["П000010000324" + Постфикс].Значение;
			КонецЦикла;
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ИмяОбласти = "ЗаявлениеОКорректировкеПредставление" Тогда
		...
		Иначе
			...
			ПараметрыФормы.Вставить("НомерПриложения",          "5");
		...
		Если ТипЗнч(Результат) = Тип("Структура") Тогда
			...
			Если Результат.ВведеноВСвободнойФорме 
				...
				Сообщение.Текст = НСтр("ru = 'Российские адреса нельзя вводить в свободной форме!';
										|en = 'Российские адреса нельзя вводить в свободной форме!'");
				Сообщение.Сообщить();
...
Процедура ПроверитьЗаполнениеТаблицыДекларации(СтатусПроверки, ИдГруппы, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста, КоличЗаполненных)
	...
	Для Каждого Дубликат Из ТаблицаДубликатов Цикл
		...
		Если НЕ ПустаяСтрока(АдресДубликата) Тогда
			ТекстОписанияОшибки = "В таблице "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " раздела ""Декларация"" повторяются сочетания кода вида продукции и реквизитов производителя/импортера в строках " + ПереченьСтрок + "!";
...
Функция СформироватьСтруктуруПоказателейИСтруктуруПараметров()
	...
	ИНН = ?(КПП = "000000000", СокрЛП(ПоказателиТитульный.ИНН1), ИНН);
...
Процедура ЗаполнитьСведенияПоСтранице(Страница, УзелСведений)
	...
	Для Каждого КодВидаПродукции Из КодыВидовПродукции Цикл
		...
		Для Каждого Производитель Из СтрокиПроизводителей Цикл
			...
			УстановитьЗначениеЭлемента(Узел_СведПроизвИмпорт, "П000000000025", Производитель.П000010000324);
...
Процедура ВывестиНаПечатьДекларация(СтруктураПараметровСтраницы)
	...
	Если ЭтаФорма[мСтруктураМногостраничныхРазделов.Декларация][0].ДанныеДопСтрок.Количество() > 0 Тогда
		...
		|П000010000122, П000010000123, П000010000124");
		Для Каждого СтрокаДанных Из ДанныеПоОрганизации Цикл
		...
		Для Каждого Лист Из ЭтаФорма[мСтруктураМногостраничныхРазделов.Декларация] Цикл
			...
			|П000010000321, П000010000322, П000010000323, П000010000324");
			Для Каждого СтрокаДанных Из ДанныеПоПодразделению Цикл
				Область_Строка = МакетПечати.ПолучитьОбласть("СтрокаИтого");
...
Функция ПолучитьМакетПечатнойФормы(ИмяРаздела)
	...
	Возврат ОбъектОтчета(ЭтаФорма.ИмяФормы)
	    .ПолучитьМакет("Печатная" + Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + ИмяРаздела);
