...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = 1152023;
	...
	СтруктураРеквизитовФормы.КНД = "1152023";
	СтруктураРеквизитовФормы.НаименованиеОтчетаДляВыгрузки = "ДЕКЛАРАЦИЯ ОБ ОБЪЕМАХ ОБОРОТА ЭТИЛОВОГО СПИРТА, АЛКОГОЛЬНОЙ И СПИРТОСОДЕРЖАЩЕЙ ПРОДУКЦИИ";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", Знач ОбластьИмя = "", НомерИзменяемойСтроки = Неопределено) Экспорт
	...
	Если ИмяТекущейСтраницыПанели = "Декларация" Тогда
		...
		Для Ном = НачальныйНомер По КонечныйНомер Цикл
			...
			ТекСтрокаТаблицы.П000000000006 = Форма.ТабличныйДокумент.Области["П000000000006" + НомСтр].Значение;
			
			_5яКолонка = Форма.ТабличныйДокумент.Области["П000000000006" + НомСтр].Значение
			+ Форма.ТабличныйДокумент.Области["П000000000010" + НомСтр].Значение;
			
			ТекСтрокаТаблицы.П000000000005 = _5яКолонка;
			Форма.ТабличныйДокумент.Области["П000000000005" + НомСтр].Значение = _5яКолонка;
			...
			ТекСтрокаТаблицы.П000000000012 = Форма.ТабличныйДокумент.Области["П000000000012" + НомСтр].Значение;
			
			_11яКолонка = Форма.ТабличныйДокумент.Области["П000000000012" + НомСтр].Значение
			+ Форма.ТабличныйДокумент.Области["П000000000016" + НомСтр].Значение
			+ Форма.ТабличныйДокумент.Области["П000000000017" + НомСтр].Значение;
			
			ТекСтрокаТаблицы.П000000000011 = _11яКолонка;
			Форма.ТабличныйДокумент.Области["П000000000011" + НомСтр].Значение = _11яКолонка;
			
			Форма.ТабличныйДокумент.Области["П000000000018" + НомСтр].Значение = Форма.ТабличныйДокумент.Области["П000000000004" + НомСтр].Значение
			...
			ТекСтрокаТаблицы.П000000000018 = Форма.ТабличныйДокумент.Области["П000000000018" + НомСтр].Значение;
		...
		Если Форма.ФлажокИтог Тогда
			...
			Форма.ТабличныйДокумент.Области.СуммаКолонка6.Значение = СтруктураГруппы.Итог("П000000000006");
			Форма.ТабличныйДокумент.Области.СуммаКолонка11.Значение = СтруктураГруппы.Итог("П000000000011");
			Форма.ТабличныйДокумент.Области.СуммаКолонка12.Значение = СтруктураГруппы.Итог("П000000000012");
			Форма.ТабличныйДокумент.Области.СуммаКолонка18.Значение = СтруктураГруппы.Итог("П000000000018");
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Декларация" Тогда
		...
		Если ((СтрНайти(Область.Имя, "П000000000003") > 0)
		   ...
		   И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьФормуВыбораЗначенияДекларацияЗавершение", ЭтотОбъект, ДополнительныеПараметры);
			ОткрытьФорму("Обработка.ОбщиеОбъектыРеглОтчетности.Форма.ФормаВыбораЗначенияИзТаблицы", ПараметрыФормы, ЭтаФорма,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы",				"ALK_1152023_4_01.xsd");
	ПараметрыВыгрузки.Вставить("ПрефиксИмениФайлаВыгрузки",	"A5");
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	//ОбъемОборот
	УзелОбъемОборотШаблон = ПолучитьПодчиненныйЭлемент(УзелРодитель, "ОбъемОборот");
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(РазделДекларация, "П000000000003_" + Формат(НомерСтроки, "ЧГ=")) Цикл
		...
		Если ЗначениеЗаполнено(РазделДекларация["П000000000003_" + Формат(НомерСтроки, "ЧГ=")]) Тогда
			...
			УзелОбъемОборот = СкопироватьУзел(УзелРодитель, УзелОбъемОборотШаблон);
			ВыгрузитьСтрокуМногострочнойЧасти(РазделДекларация, УзелОбъемОборот, НомерСтроки);
			ЗадатьЗначениеЭлемента(УзелОбъемОборот, "П000000000001", Формат(ЧислоВыгруженныхСтрок, "ЧГ="));
		КонецЕсли;
...
Процедура ДобавитьСтрокуИтогов()
	...
	Ячейка = "R" + ВставляемаяОбластьСтр + "C3:R" + ВставляемаяОбластьСтр + "C20";
	ТабличныйДокумент.Область(Ячейка).Обвести(ЛинияТип, ЛинияТип, ЛинияТип, ЛинияТип);

	Ячейка = "R" + ВставляемаяОбластьСтр + "C3:R" + ВставляемаяОбластьСтр + "C6";
	...
	СоздатьОбластьЧисло("19", "19", "СуммаКолонка17", ВставляемаяОбластьСтр);
	СоздатьОбластьЧисло("20", "20", "СуммаКолонка18", ВставляемаяОбластьСтр);
...
Процедура ПроставитьИтогиВсемКолонкам()
	...
	ПолучитьСуммуПоЗаданнойКолонке("П000000000017_", "СуммаКолонка17", ВсегоСтрок);
	ПолучитьСуммуПоЗаданнойКолонке("П000000000018_", "СуммаКолонка18", ВсегоСтрок);
