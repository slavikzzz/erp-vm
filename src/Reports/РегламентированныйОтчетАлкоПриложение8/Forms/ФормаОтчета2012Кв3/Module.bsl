...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КолКолонокИдГруппы1 = 26;
	КолКолонокИдГруппы2 = 26;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010000223", "П000010000123");
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010000423", "П000010000323");
...
Процедура УстановитьДоступностьПолейДекларация(Форма)
	...
	Пока Форма.ТабличныйДокумент.Области.Найти("П000010000101_" + Постфикс) <> Неопределено Цикл
		...
		УстановитьЦветФонаЯчейки(Форма, "П000010000181_" + Постфикс, ВариантРучногоВвода, "Декларация");
		УстановитьЦветФонаЯчейки(Форма, "П000010000182_" + Постфикс, ВариантРучногоВвода, "Декларация");
		...
		УстановитьЦветФонаЯчейки(Форма, "П000010000123_" + Постфикс, ВариантРучногоВвода, "Декларация");
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ЭтоАдресВМногострочнойЧасти Тогда
		СтандартнаяОбработка = Ложь;
		ВвестиАдресПеревозчика(ИмяОбласти, Элемент);
	КонецЕсли;
...
Процедура ОткрытьФормуКонтактнойИнформацииЗавершение(Результат, Параметры) Экспорт
	...
	ИначеЕсли Параметры.Свойство("АдресПеревозчика") Тогда
		ОбновитьАдресВТабличномДокументе(Результат, Параметры.АдресПеревозчика, Параметры.ИмяОбласти, Параметры.АктивнаяСтраница);
	КонецЕсли;
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Декларация" Тогда
		...
		Если Результат <> Неопределено Тогда
			...
			Если ТипЗнч(Результат) = Тип("Структура") Тогда
				...
				ТабличныйДокумент.Области[Область].Значение = Результат.Представление;
...
&НаКлиенте
Процедура ВвестиАдресПеревозчика(ИмяОбласти, Элемент);
	
	ИмяГруппы = ОпределитьИдМногострочногоРазделаПоАктивнойСтраницеНаКлиенте(СтруктураРеквизитовФормы.НаимТекущегоРаздела);

	СтраницыРазделаДекларация = Неопределено;
	
	Если Не мСтруктураМногостраничныхРазделов.Свойство("Декларация", СтраницыРазделаДекларация) Тогда
		Возврат;
	КонецЕсли;
	
	СтраницыРазделаДекларация = ЭтаФорма[СтраницыРазделаДекларация];
	
	АктивнаяСтраница = СтраницыРазделаДекларация.НайтиСтроки(Новый Структура("АктивнаяСтраница", Истина));
	...
	ОпределитьНазвания(АдресПеревозчика, НазваниеСтраны, НазваниеРегиона);

	ЗначенияПолей.Добавить(НазваниеСтраны,       			 "Страна");
	ЗначенияПолей.Добавить(АдресПеревозчика.Индекс,          "Индекс");
	ЗначенияПолей.Добавить(НазваниеРегиона,       			 "Регион");
	ЗначенияПолей.Добавить(АдресПеревозчика.Район,           "Район");
	ЗначенияПолей.Добавить(АдресПеревозчика.Город,           "Город");
	ЗначенияПолей.Добавить(АдресПеревозчика.НаселенныйПункт, "НаселенныйПункт");
	ЗначенияПолей.Добавить(АдресПеревозчика.Улица,           "Улица");
	ЗначенияПолей.Добавить(АдресПеревозчика.Дом,             "Дом");
	ЗначенияПолей.Добавить(АдресПеревозчика.Корпус,          "Корпус");
	ЗначенияПолей.Добавить(АдресПеревозчика.Квартира,        "Квартира");
	ЗначенияПолей.Добавить(АдресПеревозчика.ТипДома,         "ТипДома");
	ЗначенияПолей.Добавить(АдресПеревозчика.ТипКорпуса,      "ТипКорпуса");
	ЗначенияПолей.Добавить(АдресПеревозчика.ТипКвартиры,     "ТипКвартиры");
	
	Представление = "";
	
	ДополнитьПредставлениеАдреса(СокрЛП(ЗначениеПоКлючуСтуктуры("Индекс", АдресПеревозчика)),             ", ", Представление);
	ДополнитьПредставлениеАдреса(СокрЛП(НазваниеРегиона),            									  ", ", Представление);
	ДополнитьПредставлениеАдреса(СокрЛП(ЗначениеПоКлючуСтуктуры("Район", АдресПеревозчика)),              ", ", Представление);
	ДополнитьПредставлениеАдреса(СокрЛП(ЗначениеПоКлючуСтуктуры("Город", АдресПеревозчика)),              ", ", Представление);
	ДополнитьПредставлениеАдреса(СокрЛП(ЗначениеПоКлючуСтуктуры("НаселенныйПункт", АдресПеревозчика)),    ", ", Представление);
	ДополнитьПредставлениеАдреса(СокрЛП(ЗначениеПоКлючуСтуктуры("Улица", АдресПеревозчика)),              ", ", Представление);
	ДополнитьПредставлениеАдреса(СокрЛП(ЗначениеПоКлючуСтуктуры("Дом", АдресПеревозчика)),                ", " + ЗначениеПоКлючуСтуктуры("ТипДома", АдресПеревозчика)    + " № ", Представление);
	ДополнитьПредставлениеАдреса(СокрЛП(ЗначениеПоКлючуСтуктуры("Корпус", АдресПеревозчика)),             ", " + ЗначениеПоКлючуСтуктуры("ТипКорпуса ", АдресПеревозчика)+ " ",	Представление);
	ДополнитьПредставлениеАдреса(СокрЛП(ЗначениеПоКлючуСтуктуры("Квартира", АдресПеревозчика)),           ", " + ЗначениеПоКлючуСтуктуры("ТипКвартиры", АдресПеревозчика) + " ",	Представление);
	...
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("АдресПеревозчика", АдресПеревозчика);
...
Процедура ПроверитьЗаполнениеДекларация(ПараметрыВыгрузки, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		ПроверитьЗаполнениеТаблицыДекларации(СтатусПроверки, СтруктураРеквизитовФормы.мИдГруппы1, ДанныеДопСтрок, ДополнительныеДанныеЛиста, НомерЛиста);

		ЛистРаздела.ДанныеДопСтрок[0].Значение.Свойство(СтруктураРеквизитовФормы.мИдГруппы2, ДанныеДопСтрок);
		ПроверитьЗаполнениеТаблицыДекларации(СтатусПроверки, СтруктураРеквизитовФормы.мИдГруппы2, ДанныеДопСтрок, ДополнительныеДанныеЛиста, НомерЛиста);
	...
	Для Каждого СтрокаДекларации Из ДанныеДопСтрок Цикл
		...
		Иначе
			...
			ИначеЕсли ЗаполненыСведенияОбИП Тогда
				...
				Иначе
					ФИО = РегламентированнаяОтчетность.РазложитьФИО(ФИО);
			...
			Для Каждого СтрокаПеревозчика Из ТаблицаПеревозчиков Цикл	
				...
				Для Каждого СтрокаТранспортногоСредства Из ТаблицаТранспортныхСредств Цикл
					...
					Для Каждого СтрокаТранспортнойНакладной Из ТаблицаТранспортныхНакладных Цикл
						...
						КонецЦикла;
						РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_СведТТН);
						
					КонецЦикла;
					РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_СведТН);
					
				КонецЦикла;
				РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_ТранспортноеСредство);
				
			КонецЦикла;
			РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_СведПеревозчик);
			
		КонецЦикла;
		РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_Продукция);
...
Функция ПолучитьИдентификаторПеревозчика(ДанныеВыгрузки, СтрокаСведенийПеревозчика, АдресПеревозчика)
	...
	ФИО = РегламентированнаяОтчетность.РазложитьФИО(СтрокаСведенийПеревозчика.П000010000306);
	...
	Если АдресПеревозчика <> Неопределено Тогда
		УсловияОтбора.Вставить("КодСтраны",  СокрЛП(АдресПеревозчика.Страна));
...
Процедура ЗаполнитьСправочникиДекларации(Узел_Справочники, ДанныеВыгрузки)
	...
	КонецЦикла;
	РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_Перевозчики);
