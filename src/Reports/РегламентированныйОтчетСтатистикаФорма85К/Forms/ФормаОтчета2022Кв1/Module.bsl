...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.Имя = "ОргАдрес" Тогда
		РегламентированнаяОтчетностьКлиент.ОбработкаАдресаВСтатистическомОтчетеJSON(
			ЭтотОбъект, Область, СтандартнаяОбработка);
	Иначе
		Если Область.Имя = "П001001010503" Тогда
		...
		ИначеЕсли (Лев(Область.Имя, 4) = "П015" И Сред(Область.Имя, 13, 1) <> "1")
			  Или Лев(Область.Имя, 4) = "П016" Или Лев(Область.Имя, 4) = "П018" Или Лев(Область.Имя, 4) = "П019"
			  Или Лев(Область.Имя, 13) = "П020001201003" Или Лев(Область.Имя, 13) = "П020001201103" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникДаНет");
		ИначеЕсли Область.Имя = "П001001010103" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
		...
		ИначеЕсли Область.Имя = "П001001010403" Тогда
			...
			ИначеЕсли СтрНайти(Область.Имя, "П008001080201") = 1 Или СтрНайти(Область.Имя, "П008001080203") = 1 Тогда
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Области["П008001080202_1"].Значение = "802";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	//*****************************************************************3
	//Стр. 301 = Сумме Стр. 302,303 по всем графам 
	Для Гр = 3 По 6 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаСтр302_303 = 0;
		Для Стр = 302 По 303 Цикл
			СуммаСтр302_303 = СуммаСтр302_303 + Области["П0030010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0030010301" + Графа].Значение = СуммаСтр302_303;
	КонецЦикла;
	...
	//*****************************************************************5
	//Стр. 502 = Сумме Стр. 503,504,506,507,508,510,511,512 по всем 
	Для Гр = 4 По 8 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаСтр502 = 0;
	...
	//Стр.601 = Сумме Стр. 602, 613, 614, 618−621 по графам 3–8
	Для Гр = 4 По 8 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаСтр601 = 0;
	...
	//*****************************************************************7
	//Гр. 3 = Сумме Гр. 4-11 по всем строкам
	...
	//*****************************************************************8
	//Стр. 801 гр. 4 = сумме данных по стр. 802 гр. 4
	Инд = 0;
	СуммаГр4 = 0;
	Пока Истина Цикл
		Инд = Инд + 1;
		Если Области.Найти("П008001080204_" + Инд) = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		СуммаГр4 = СуммаГр4 + Области["П008001080204_" + Инд].Значение;
	КонецЦикла;
	Области["П008001080104"].Значение = СуммаГр4;
	
	//*****************************************************************9
	//Стр. 901 = Сумме стр. 902, 906, 919, 922 по всем графам
	Для Гр = 3 По 8 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
	...
	//*****************************************************************13
	//Стр. 1301 = Сумме Стр. 1302, 1306, 1319, 1322 по всем графам
	Для Гр = 3 По 13 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		
		Области["П0130011301" + Графа].Значение
			= Области["П0130011302" + Графа].Значение
			+ Области["П0130011306" + Графа].Значение
			+ Области["П0130011319" + Графа].Значение
			+ Области["П0130011322" + Графа].Значение;
	КонецЦикла;
	
	//*****************************************************************17
	//Гр. 3 = сумме гр. 5–8 по стр. 1701–1703
	Для Стр = 1701 По 1703 Цикл
		СуммаГр3 = 0;
		Для Гр = 5 По 8 Цикл
			Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
			СуммаГр3 = СуммаГр3 + Области["П017001" + Формат(Стр, "ЧГ=") + Графа].Значение;
		КонецЦикла;
		Области["П017001" + Формат(Стр, "ЧГ=") + "03"].Значение = СуммаГр3;
	КонецЦикла;
	
	
	//*****************************************************************21
	//Стр. 2101= сумме стр. 2102, 2106−2109 по графе 3 и 4
	Для Гр = 3 По 4 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		...
		Области["П0210012101" + Графа].Значение
			...
			+ Области["П0210012107" + Графа].Значение
			+ Области["П0210012108" + Графа].Значение
			+ Области["П0210012109" + Графа].Значение;
	КонецЦикла;
	
	
	//*****************************************************************22
	//Стр. 2201 = сумме стр. 2202–2205 по графе 3
	//Стр. 2201 = сумме стр. 2202–2205 по графе 4
	Для Гр = 3 По 4 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		
		Области["П0220012201" + Графа].Значение
			= Области["П0220012202" + Графа].Значение
			+ Области["П0220012203" + Графа].Значение
			+ Области["П0220012204" + Графа].Значение
			+ Области["П0220012205" + Графа].Значение;
	КонецЦикла;
	
	
	//*****************************************************************23
	//Гр. 5= сумме гр. 8–10
	Для Стр = 2302 По 2310 Цикл
		СуммаГр5 = 0;
		Для Гр = 8 По 10 Цикл
			Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
			СуммаГр5 = СуммаГр5 + Области["П023001" + Формат(Стр, "ЧГ=") + Графа].Значение;
		КонецЦикла;
		Области["П023001" + Формат(Стр, "ЧГ=") + "05"].Значение = СуммаГр5;
	КонецЦикла;
	
	//Гр. 7= сумме гр. 11–13
	Для Стр = 2302 По 2310 Цикл
		СуммаГр7 = 0;
		Для Гр = 11 По 13 Цикл
			СуммаГр7 = СуммаГр7 + Области["П023001" + Формат(Стр, "ЧГ=") + Гр].Значение;
		КонецЦикла;
		Области["П023001" + Формат(Стр, "ЧГ=") + "07"].Значение = СуммаГр7;
	КонецЦикла;
	
	//Стр. 2301= сумме стр. 2302, 2306, 2309, 2310 по графам 3 − 13
	Для Гр = 3 По 13 Цикл
		
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		
		Области["П0230012301" + Графа].Значение
			= Области["П0230012302" + Графа].Значение
			+ Области["П0230012306" + Графа].Значение
			+ Области["П0230012309" + Графа].Значение
			+ Области["П0230012310" + Графа].Значение;
	КонецЦикла;
	
	
	//*****************************************************************24
	//Стр. 2401 = Сумме Стр. 2403, 2412 по графе 3
	Области["П024001240103"].Значение
			= Области["П024001240303"].Значение
			+ Области["П024001241203"].Значение;
	
	
	//*****************************************************************25
	//Стр. 2501 = Сумме Стр. 2502–2504 по графе 3
	Области["П025001250103"].Значение
		= Области["П025001250203"].Значение
		+ Области["П025001250303"].Значение
		+ Области["П025001250403"].Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 3 По 6 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0030010301" + Графа + ";";
	КонецЦикла;
	...
	ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П008001080104" + ";";
	...
	Для Гр = 3 По 13 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0130011301" + Графа + ";";
	КонецЦикла;
	
	Для Стр = 1701 по 1703 Цикл
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П017001" + Формат(Стр, "ЧГ=") + "03" + ";";
	КонецЦикла;
	
	Для Гр = 3 По 4 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0210012101" + Графа + ";";
	КонецЦикла;
	
	Для Гр = 3 По 4 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0220012201" + Графа + ";";
	КонецЦикла;
	
	Для Стр = 2301 По 2310 Цикл
		Если Стр = 2301 Тогда
			Для Гр = 3 По 13 Цикл
				Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
				ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П023001" + Формат(Стр, "ЧГ=") + Графа + ";";
			КонецЦикла;
		Иначе
			ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П023001" + Формат(Стр, "ЧГ=") + "05" + ";";
			ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П023001" + Формат(Стр, "ЧГ=") + "07" + ";";
		КонецЕсли;
	КонецЦикла;
	
	ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П024001240103" + ";";
	
	ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П025001250103" + ";";
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть11_3.Низ + 1;
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверки.Добавить(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть11_4"));
	...
	МассивПроверки.Очистить();
	МассивПроверки.Добавить(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть11_5"));
	МассивПроверки.Добавить(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть11_6"));
	МассивПроверки.Добавить(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть11_7"));
	...
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть11_5"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть11_6"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть11_7"));
	
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть11_7.Низ + 1;
	...
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть12_3.Низ + 1;
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	МнгКолСтр = ЭтотОбъект["П0150011503"].Количество();
	Для Стр = 1 По 3 Цикл
		МнгКолСтр = МнгКолСтр + 1;
		...
		Для Гр = 1 По 10 Цикл
			...
			КонецЕсли;
			Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
			ЯчейкаИмя = "П015001150" + Стр + Графа;
			ЯчейкаВыгрузкаИмя = "П0150011503" + Графа + "_" + МнгКолСтр;
			Если Гр = 1 Тогда
				ПараметрыВыгрузки.Вставить(ЯчейкаВыгрузкаИмя, Стр);
			Иначе
				ПараметрыВыгрузки.Вставить(ЯчейкаВыгрузкаИмя, ПолеТабличногоДокументаФормаОтчета.Области[ЯчейкаИмя].Значение);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
