...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.Имя = "ОргАдрес" Тогда
		РегламентированнаяОтчетностьКлиент.ОбработкаАдресаВСтатистическомОтчетеJSON(
			ЭтотОбъект, Область, СтандартнаяОбработка);
	Иначе
		Если Область.Имя = "П013001013503" Или Область.Имя = "П013001013603" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникДаНет");
		ИначеЕсли Область.Имя = "П001001000103" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникТипОрганизации");
		ИначеЕсли Область.Имя = "П001001000203" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникТипМестности");
		ИначеЕсли Область.Имя = "П001001000303" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникКодСтатусаОрганизации");
		ИначеЕсли Область.Имя = "П001001000403" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникКодВидаБлагоустройства");
		ИначеЕсли Область.Имя = "П001001000503" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникКодРежимаФункционирования");
		ИначеЕсли Область.Имя = "П001001000603" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 8 Цикл
		...
		СуммаСтр7_11 = 0;
		Для Стр = 7 По 11 Цикл
			СуммаСтр7_11 = СуммаСтр7_11 + Области["П00200100" + Формат(Стр, "ЧЦ=2; ЧВН=") + Графа].Значение;
		КонецЦикла;
		Области["П0020010012" + Графа].Значение = СуммаСтр7_11;
	КонецЦикла;
	...
	Для Гр = 3 По 11 Цикл
		...
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаСтр15_22 = 0;
		Для Стр = 15 По 22 Цикл
			СуммаСтр15_22 = СуммаСтр15_22 + Области["П00300100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0030010014" + Графа].Значение = СуммаСтр15_22;
		
		Области["П0030010013" + Графа].Значение
			= Области["П0030010014" + Графа].Значение
			+ Области["П0030010023" + Графа].Значение
			+ Области["П0030010024" + Графа].Значение
			+ Области["П0030010027" + Графа].Значение
			+ ?(Гр = 4 Или Гр = 10, 0, Области["П0030010028" + Графа].Значение)
			+ Области["П0030010029" + Графа].Значение
			+ Области["П0030010030" + Графа].Значение;
		
	КонецЦикла;
	
	Для Стр = 36 По 42 Цикл
		СуммаГр4_11 = 0;
		Для Гр = 4 По 11 Цикл
			Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
			СуммаГр4_11 = СуммаГр4_11 + Области["П00400100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П00400100" + Стр + "03"].Значение = СуммаГр4_11;
	КонецЦикла;
	
	Для Гр = 3 По 5 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаСтр44_45 = 0;
		Для Стр = 44 По 45 Цикл
			СуммаСтр44_45 = СуммаСтр44_45 + Области["П00500100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0050010043" + Графа].Значение = СуммаСтр44_45;
	КонецЦикла;
	
	Для Гр = 3 По 9 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаСтр47_58 = 0;
		Для Стр = 47 По 58 Цикл
			СуммаСтр47_58 = СуммаСтр47_58 + Области["П00600100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0060010046" + Графа].Значение = СуммаСтр47_58;
	КонецЦикла;
	
	Для Гр = 3 По 12 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаСтр65_76 = 0;
		Для Стр = 65 По 76 Цикл
			СуммаСтр65_76 = СуммаСтр65_76 + Области["П00700100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0070010064" + Графа].Значение = СуммаСтр65_76;
	КонецЦикла;
	
	СуммаГр4_9 = 0;
	Для Гр = 4 По 9 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаГр4_9 = СуммаГр4_9 + Области["П0080010077" + Графа].Значение;
	КонецЦикла;
	Области["П008001007703"].Значение = СуммаГр4_9;
	
	СуммаГр11_16 = 0;
	Для Гр = 11 По 16 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаГр11_16 = СуммаГр11_16 + Области["П0080010077" + Графа].Значение;
	КонецЦикла;
	Области["П008001007710"].Значение = СуммаГр11_16;
	
	СуммаСтр83_104 = 0;
	Для Стр = 83 По 104 Цикл
		СуммаСтр83_104 = СуммаСтр83_104 + Области["П0100010" + Формат(Стр, "ЧЦ=3; ЧВН=") + "04"].Значение;
	КонецЦикла;
	Области["П010001008204"].Значение = СуммаСтр83_104;
	
	Области["П014001013703"].Значение = Области["П014001013903"].Значение + Области["П014001014803"].Значение;
	
	Области["П015001014903"].Значение
		= Области["П015001015003"].Значение
		+ Области["П015001015103"].Значение
		+ Области["П015001015203"].Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 3 По 8 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0020010012" + Графа + ";";
	КонецЦикла;
	
	Для Гр = 3 По 11 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		Для Стр = 13 По 14 Цикл
			ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П00300100" + Стр + Графа + ";";
		КонецЦикла;
	КонецЦикла;
	
	Для Стр = 36 по 42 Цикл
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П00400100" + Стр + "03" + ";";
	КонецЦикла;
	
	Для Гр = 3 По 5 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0050010043" + Графа + ";";
	КонецЦикла;
	
	Для Гр = 3 По 9 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0060010046" + Графа + ";";
	КонецЦикла;
	
	Для Гр = 3 По 12 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0070010064" + Графа + ";";
	КонецЦикла;
	
	ЯчейкиАвторасчета = ЯчейкиАвторасчета
		+ "П008001007703;П008001007710;П010001008204;П014001013703;П015001014903";
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникКодВидаБлагоустройства",
		512, , "Выбор кода вида благоустройства", КоллекцияСписковВыбора["КодВидаБлагоустройства"]);
