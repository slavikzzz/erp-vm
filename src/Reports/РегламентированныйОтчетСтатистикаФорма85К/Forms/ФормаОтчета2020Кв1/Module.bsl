...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0000601045";
...
Процедура ПоказатьПериод()
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета2.Значение = "(число полных лет на "
		+ Формат(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, "ДЛФ=ДД") + "):";
	
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета3.Значение = "Число полных лет по состоянию на "
		+ Формат(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, "ДФ='д ММММ гггг'") + " года";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		ИначеЕсли Область.Имя = "П000020101003" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникРежимРаботы");
		ИначеЕсли Область.Имя = "П000020101103" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникКапРемонт");
		ИначеЕсли Область.Имя = "П000020101203" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникПриостановкаДеятельности");
		ИначеЕсли Область.Имя = "П000020101303" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникМестность");
		ИначеЕсли Лев(Область.Имя, 13) = "П000060104503" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникЯзыки");
		КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 9 Цикл
		...
		Сумм17_24 = 0;
		Для Стр = 17 По 24 Цикл
			Сумм17_24 = Сумм17_24 + Области["П00003010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000301016" + Графа].Значение = Сумм17_24;
		
		Области["П0000301015" + Графа].Значение = Области["П0000301016" + Графа].Значение
			+ Области["П0000301025" + Графа].Значение
			+ Области["П0000301026" + Графа].Значение
			+ Области["П0000301029" + Графа].Значение
			+ ?(Гр = 4 Или Гр = 8, 0, Области["П0000301030" + Графа].Значение)
			+ Области["П0000301031" + Графа].Значение
			+ Области["П0000301032" + Графа].Значение;
		
	КонецЦикла;
	
	Для Стр = 38 По 41 Цикл
		СуммПоГр4_11 = 0;
		Для Гр = 4 По 11 Цикл
			Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
			СуммПоГр4_11 = СуммПоГр4_11 + Области["П00004010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П00004010" + Стр + "03"].Значение = СуммПоГр4_11;
	КонецЦикла;
	
	СуммаПоСтр = 0;
	Для Стр = 1 По Форма.П0000601045.Количество() Цикл
		СуммаПоСтр = СуммаПоСтр + Области["П000060104504_" + Формат(Стр, "ЧГ=")].Значение;
	КонецЦикла;
	Области["П000060104404"].Значение = СуммаПоСтр;
	
	Для Гр = 3 По 9 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		Сумм52_62 = 0;
		Для Стр = 52 По 62 Цикл
			Сумм52_62 = Сумм52_62 + Области["П00007010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000701051" + Графа].Значение = Сумм52_62;
	КонецЦикла;
	
	Для Гр = 3 По 12 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		Сумм66_76 = 0;
		Для Стр = 66 По 76 Цикл
			Сумм66_76 = Сумм66_76 + Области["П00008010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000801065" + Графа].Значение = Сумм66_76;
	КонецЦикла;
	
	Области["П000090107703"].Значение = Области["П000090107704"].Значение
		+ Области["П000090107705"].Значение
		+ Области["П000090107706"].Значение
		+ Области["П000090107707"].Значение
		+ Области["П000090107708"].Значение
		+ Области["П000090107709"].Значение;
		
	СуммаПоГр11_16 = 0;
	Для Гр = 11 По 16 Цикл
		СуммаПоГр11_16 = СуммаПоГр11_16 + Области["П0000901077" + Гр].Значение;
	КонецЦикла;
	Области["П000090107710"].Значение = СуммаПоГр11_16;
	
	Области["П000100107803"].Значение = Области["П000100107804"].Значение
		+ Области["П000100107805"].Значение
		+ Области["П000100107806"].Значение
		+ Области["П000100107807"].Значение;
		
	Области["П000100107903"].Значение = Области["П000100107904"].Значение
		+ Области["П000100107905"].Значение
		+ Области["П000100107906"].Значение
		+ Области["П000100107907"].Значение;
		
	Области["П000140110303"].Значение = Области["П000140110503"].Значение + Области["П000140111403"].Значение;
	
	Области["П000150111503"].Значение = Области["П000150111603"].Значение
		+ Области["П000150111703"].Значение
		+ Области["П000150111803"].Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 3 По 9 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		Для Стр = 15 По 16 Цикл
			ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П00003010" + Стр + Графа + ";";
		КонецЦикла;
	КонецЦикла;
	
	Для Стр = 38 По 41 Цикл
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П00004010" + Стр + "03;";
	КонецЦикла;
	
	ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П000060104404;";
	
	Для Гр = 3 По 9 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000701051" + Графа + ";";
	КонецЦикла;
	
	Для Гр = 3 По 12 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000801065" + Графа + ";";
	КонецЦикла;
	
	ЯчейкиАвторасчета = ЯчейкиАвторасчета
		+ "П000090107703;П000090107710;П000100107803;П000100107903;П000140110303;П000150111503";
...
Процедура ПеренумероватьСтроки(ПолеТабличногоДокументаФормаОтчета)
	...
	Для Инд = 1 По 6 Цикл
		ПолеТабличногоДокументаФормаОтчета.Области["П000060104502_" + Инд].Значение = Строка(44 + Инд);
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел3"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 85-К");
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел4_5"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел6_1"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел6_2"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел6_3"));
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.Раздел6_3.Низ + 1;
	...
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел8_9"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 85-К");
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел10_11"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 85-К");
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел12_13"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 85-К");
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел14"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 85-К");
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел15"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 85-К");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникРежимРаботы",
		512, , "Выбор кода режима работы", КоллекцияСписковВыбора["РежимРаботы"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникКапРемонт",
		512, , "Выбор", КоллекцияСписковВыбора["КапРемонт"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникПриостановкаДеятельности",
		512, , "Выбор", КоллекцияСписковВыбора["ПриостановкаДеятельности"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникМестность",
		512, , "Выбор типа местности", КоллекцияСписковВыбора["Местность"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникЯзыки",
		512, , "Выбор языка", КоллекцияСписковВыбора["Языки"]);
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ПолеДаНет(Область.Имя)
	 Или Область.Имя = "П000020101103"
	 Или Область.Имя = "П000020101203" Тогда
...
Функция ПолеДаНет(ИмяОбласти)
	...
	ТекГруппа = Лев(ИмяОбласти, 6);
	
	Если ТекГруппа = "П00001"
	 Или ТекГруппа = "П00011"
	 Или ИмяОбласти = "П000020101403" 
	 Или ИмяОбласти = "П000120108803"
	 Или ИмяОбласти = "П000120108903"
	 Или ИмяОбласти = "П000120109003"
	 Или ИмяОбласти = "П000130110003" Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
