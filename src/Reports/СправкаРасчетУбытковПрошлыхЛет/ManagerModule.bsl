...
Процедура ПослеВыводаРезультата(Контекст, Результат) Экспорт
	...
	ИгнорироватьОграничениеНаПереносУбытков = Ложь;
	Если ЗначениеЗаполнено(Контекст.ВариантНО) Тогда
		ИгнорироватьОграничениеНаПереносУбытков = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контекст.ВариантНО,
			"УменьшатьНалоговуюБазуНаВсюСуммуРанееПолученногоУбытка");
	КонецЕсли;
	
	Если Не ИгнорироватьОграничениеНаПереносУбытков Тогда
...
Процедура ДобавитьПримечаниеЛимитУменьшенияПрибыли(Результат, Контекст)
	...
	ТекстПримечания =
	НСтр("ru = '""В отчетные (налоговые) периоды с 1 января 2017 года по 31 декабря 2026 года налоговая база по налогу за текущий отчетный (налоговый) период ... не может быть уменьшена на сумму убытков, полученных в предыдущих налоговых периодах, более чем на 50 процентов"" (п. 2.1 ст. 283 НК РФ)';
		|en = '""In the reporting (fiscal) periods from January 1, 2017 to December 31, 2026, the tax base for the tax for the current reporting (fiscal) period ... cannot be reduced by more than 50 percent of the losses incurred in previous fiscal periods ""(item 2.1, art. 283 of the Tax Code of the Russian Federation)'");
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	// Выведем сноску с пояснением расчета суммы переносимого убытка прошлых лет
	// Выводится только в 2017-2020 годах (см. текст сноски в макете).
	ИгнорироватьОграничениеНаПереносУбытков = Ложь;
	Если ЗначениеЗаполнено(ПараметрыОтчета.ВариантНО) Тогда
		ИгнорироватьОграничениеНаПереносУбытков = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыОтчета.ВариантНО,
			"УменьшатьНалоговуюБазуНаВсюСуммуРанееПолученногоУбытка");
	КонецЕсли;
	
	Сноска2017_2020 = НайтиПоИмени(КомпоновщикНастроек.Настройки.Структура, "Сноска2017_2020");
	Сноска2017_2020.Использование = Не ИгнорироватьОграничениеНаПереносУбытков 
		И НалоговыйУчет.ПрименяетсяОграничениеНаПереносУбытковПрошлыхЛетПоДолеПрибыли(ПараметрыОтчета.КонецПериода);
