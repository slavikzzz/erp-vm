...
Функция ПолучитьИмяМакета(ДатаНачалаПериодаОтчета)
	Возврат ?(Год(ДатаНачалаПериодаОтчета) >= 2020, "ФормаОтчета2020Кв1_ФормаОтчета", "ФормаОтчета2019Кв3_ФормаОтчета");
КонецФункции
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "19/04/2019";
	СтруктураРеквизитовФормы.мКодОтчета   = "0612003";
	СтруктураРеквизитовФормы.мПечатныеФормы = Новый СписокЗначений;
	ПолеТабличногоДокументаФормаОтчета.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(ПолучитьИмяМакета(Параметры.мДатаНачалаПериодаОтчета)));
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда 
		ИмяМакета = "АтрибВыгрузкиXML2019Кв3_месяц";
	Иначе 
		ИмяМакета = "АтрибВыгрузкиXML2019Кв3_год";
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 10 Цикл
		...
		Для Стр = 4 По 10 Цикл 
			...
			Тот = Тот + Области["П01"+СтрСтр+Формат(Гр, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0101"+Формат(Гр, "ЧЦ=2; ЧВН=")].Значение = Тот;
	КонецЦикла;
	
	Для Гр = 3 По 4 Цикл 
		Области["П02160"+Гр].Значение = Области["П02170"+Гр].Значение + Области["П02180"+Гр].Значение + Области["П02190"+Гр].Значение + Области["П02200"+Гр].Значение;
	КонецЦикла;
	
	Для Гр = 3 По 6 Цикл
		Тот = 0;
		Для Стр = 22 По 28 Цикл 
			Тот = Тот + Области["П03"+Стр+"0"+Гр].Значение;
		КонецЦикла;
		Области["П03210"+Гр].Значение = Тот;
		Если Гр = 3 Или Гр = 5 Тогда
			Тот = 0;
			Для Стр = 33 По 36 Цикл 
				Тот = Тот + Области["П03"+Стр+"0"+Гр].Значение;
			КонецЦикла;
			Области["П03320"+Гр].Значение = Тот;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого Обл Из ПолеТабличногоДокументаФормаОтчета.Области Цикл
		Если Обл.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
			Если Обл.Имя = "ПолеНомерКорректировки"
				Или Обл.Имя = "ШапкаОбособленныеПодразделения" Тогда 
				Продолжить;
			КонецЕсли;
	...
	Для Каждого Обл Из ОбластиСтроки Цикл
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма ИЖС");
	КонецЦикла;
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2019Кв3");
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревоДляЗагрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2019Кв3");
...
Функция ПроверитьПоказатели() Экспорт
	...
	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2019Кв3");
...
Функция УниверсальнаяВыгрузкаXMLПроверкаДанныхНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда 
		ИмяМакета = "АтрибВыгрузкиXML2019Кв3_месяц";
		ИмяШаблона = "ШаблонРосстата2019Кв3_месяц";
	Иначе 
		ИмяМакета = "АтрибВыгрузкиXML2019Кв3_год";
		ИмяШаблона = "ШаблонРосстата2019Кв3_год";
	КонецЕсли;
...
Функция ПроверитьКонтрольныеСоотношенияИзШаблонаВФоне()
	...
	ПараметрыПроверкиКонтрольныхСоотношений.Вставить("СхемаВыгрузки", "СхемаВыгрузкиXML2019Кв3");
