...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "13/08/2018";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда 
		ИмяМакета = "АтрибВыгрузкиXML2019Кв1_месяц";
	Иначе 
		ИмяМакета = "АтрибВыгрузкиXML2019Кв1_год";
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 6 Цикл
		...
		Для Стр = 4 По 10 Цикл 
			...
			Тот = Тот + Области["П01"+СтрСтр+"0"+Гр].Значение;
		КонецЦикла;
		Области["П01010"+Гр].Значение = Тот;
	КонецЦикла;
...
Функция УниверсальнаяВыгрузкаXMLПроверкаДанныхНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда 
		ИмяМакета = "АтрибВыгрузкиXML2019Кв1_месяц";
		ИмяШаблона = "ШаблонРосстата2019Кв1_месяц";
	Иначе 
		ИмяМакета = "АтрибВыгрузкиXML2019Кв1_год";
		ИмяШаблона = "ШаблонРосстата2019Кв1_год";
	КонецЕсли;
...
Функция ПолучитьПараметрыИнициализацииУниверсальногоОтчета()
	...
	Иначе 
		...
		ИмяШаблона = "ШаблонРосстата2019Кв1_год";
	КонецЕсли;
	Возврат УниверсальныйОтчетСтатистики.ПолучитьПараметрыИнициализацииУниверсальногоОтчета(ЭтаФорма, ИмяМакета, ИмяШаблона, "СхемаВыгрузкиXML2019Кв1", Истина, Ложь);
КонецФункции
