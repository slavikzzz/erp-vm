...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "01/09/2015";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 6 Цикл 
		Области["П01010"+Гр].Значение = Области["П01030"+Гр].Значение + Области["П01020"+Гр].Значение;
	КонецЦикла;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда 
		ИмяМакета = "АтрибВыгрузкиXML2016Кв1_месяц";
	Иначе 
		ИмяМакета = "АтрибВыгрузкиXML2016Кв1_год";
	КонецЕсли;
...
Функция ПолучитьПараметрыИнициализацииУниверсальногоОтчета()
	Если Месяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда 
		ИмяМакета = "АтрибВыгрузкиXML2016Кв1_месяц";
		ИмяШаблона = "ШаблонРосстата2016Кв1_месяц";
	Иначе 
		ИмяМакета = "АтрибВыгрузкиXML2016Кв1_год";
		ИмяШаблона = "ШаблонРосстата2016Кв1_год";
	КонецЕсли;
