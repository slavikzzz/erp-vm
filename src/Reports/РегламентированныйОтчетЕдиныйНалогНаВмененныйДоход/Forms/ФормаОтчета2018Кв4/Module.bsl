...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мСтруктураМногострочныхРазделов.Вставить(СтруктураРеквизитовФормы.ИдМСЧ_Раздел4,   СтруктураРеквизитовФормы.ИдМСЧ_Раздел4);
	...
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.ИдМСЧ_Раздел4,   КолИсхСтрокМСЧ_Раздел4);
	...
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.ИдМСЧ_Раздел4,   КолКолонокИдМСЧ_Раздел4);
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		Иначе
			...
			Если НЕ СтруктураРеквизитовФормы.ИП Тогда
				ИзменитьНастройкиРазделаВДеревеРазделов("Раздел4", 0);
			КонецЕсли;
		КонецЕсли;
	...
	Если (НЕ ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСкопированаФорма)
	 ...
	 ИЛИ СтруктураРеквизитовФормы.СформироватьФормуОтчетаАвтоматически) Тогда
		...
		ЗаполнитьКодПоОКТМОПоУмолчаниюПриСозданииНовогоОтчета();
		...
		Если ТипЗнч(РегистрацияВНО) = Тип("СправочникСсылка.РегистрацииВНалоговомОргане") Тогда
			...
			Если НЕ РегистрацияВНО.Пустая() Тогда
				...
				ЗаполнитьКодПоОКТМОПоУмолчаниюПриСозданииНовогоОтчета(РегистрацияВНО);
	...
	Если НЕ БезОткрытияФормы Тогда
		...
		Элементы.ГруппаЕНВДОтменяется.Видимость = СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '20200930';
	КонецЕсли;
	...
	ИзменитьРамкиОбластейТабличногоДокумента(ЭтаФорма, СтруктураРеквизитовФормы.НаимТекущегоРаздела);
...
&НаСервере
Процедура ЗаполнитьКодПоОКТМОПоУмолчаниюПриСозданииНовогоОтчета(РегистрацияВНО = Неопределено)
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено
		...
		Если РегистрацияВНО = Неопределено Тогда
			
			ПараметрыОтбораРегистрацийВНО = Новый Структура("КодНО,КПП",
			ТабличныйДокумент.Области.КодИМНС.Значение, ТабличныйДокумент.Области.КПП1.Значение);
			...
			Если РегистрацииВНО.Количество() = 1 Тогда
				
				ОКТМО = СокрЛП(РегистрацииВНО[0].КодПоОКТМО);
				
			КонецЕсли;
			
		Иначе
			
			ОКТМО = СокрЛП(РегистрацияВНО.КодПоОКТМО);
			
		КонецЕсли;
		
		Если ОКТМО <> "" Тогда
			
			СтруктураДанныхРаздел1.Вставить("П000010001003_1", ОКТМО);
			П00001000[0].П000010001003 = ОКТМО;
			ТаблицаСтраницыРаздела2[0].Данные[0].Значение.Вставить("П000020003000", ОКТМО);
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000030005003", "П000030000503;П000030001003;П000030002003;П000030003003;П000030004003");
...
Процедура ВывестиРазделВТабличныйДокумент(Форма, ИмяГруппы, СтруктураГруппы)
		...
		Если ИмяГруппы = Форма.СтруктураРеквизитовФормы.ИдМСЧ_Раздел2_1
		 ...
		 // Обработка многострочной части калькулятора календарных дней.
			Для НомерНовойСтроки = ИсходноеКоличествоСтрокГруппы + 1 По ТекущееКоличествоСтрокГруппы Цикл
				...
				ОпорнаяОбласть = Форма.ТабличныйДокумент.Области[ИмяГиперссылки + "_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомерНовойСтроки - 1)];
				...
				ПромежуточныйДокумент.Области[ОпорнаяОбласть.Имя].Имя = ИмяГиперссылки + "_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомерНовойСтроки);
				...
				НоваяОбластьВводаДаты.Имя = ИмяГруппы + "01_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомерНовойСтроки);
	...
	Для Каждого СтрокаТаблицы Из СтруктураГруппы Цикл
		...
		Для НомерГрафы = 1 По Форма.мСтруктураКолвоКолонокРазделов[ИмяГруппы] Цикл
			...
			Если ИмяГруппы = Форма.СтруктураРеквизитовФормы.ИдМСЧ_Раздел1
				ИЛИ ИмяГруппы = Форма.СтруктураРеквизитовФормы.ИдМСЧ_Раздел4 Тогда
				
				// Отдельный алгоритм для разделов 1 и 4.
	...
	ИначеЕсли ИмяГруппы = Форма.СтруктураРеквизитовФормы.ИдМСЧ_Раздел4 Тогда
		ТаблицаДопСтрок = Форма["Хранилище" + Форма.СтруктураРеквизитовФормы.ИдМСЧ_Раздел4].Добавить().Таблица;
	КонецЕсли;
...
Процедура ДобавитьДополнительнуюСтрокуДаты(ИмяОбласти, МногострочнаяЧасть, ИдМСЧ)
	...
	ПромежуточныйДокумент = Новый ТабличныйДокумент;
	ПромежуточныйДокумент.Вывести(ОбластьКопирования);
	ПромежуточныйДокумент.Области[ИмяОбласти].Имя = ОсновноеИмяГиперссылки + "_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомерГиперссылки + 1);
	...
	НоваяОбластьВводаДаты.Имя = ИдМСЧ + "01_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(МногострочнаяЧасть.Количество());
	НоваяОбластьВводаДаты.Очистить();
	...
	Для НСтр = 1 По СтруктураГруппы.Количество() Цикл
		...
		Для НомерГрафы = 1 По мСтруктураКолвоКолонокРазделов[ИмяГруппы] Цикл
			...
			Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1"
				ИЛИ СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел4" Тогда
...
Процедура ОчиститьСписокДат(ИдМСЧ)
	...
	Если АктуальноеЧислоСтрок > ИсходноеЧислоСтрок Тогда
		ВерхняяОбласть = ТабличныйДокумент.Области[ИдМСЧ + "01_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(ИсходноеЧислоСтрок + 1)];
		НижняяОбласть  = ТабличныйДокумент.Области[ИдМСЧ + "01_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(АктуальноеЧислоСтрок)];
		...
		ОбластьГиперссылки = ТабличныйДокумент.Области[ИмяГиперссылки + "_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(АктуальноеЧислоСтрок)];
...
Процедура РассчитатьДниОсуществленияДеятельности()
	...
	Для Инд = 1 По ДатыПостановкиНаУчет.Количество() Цикл
		НомерСтроки = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Инд);
...
Процедура РассчитатьДниОсуществленияДеятельностиВТаблицеРаздела2()
	...
	Для каждого СтраницаРаздела2 Из ТаблСтраницРаздела2 Цикл
		...
		НомСтр = 1;
		Пока ДанныеСтраницыРаздела2.Свойство(СтруктураРеквизитовФормы.ИдМСЧ_Раздел2_1 + "01_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомСтр)) Цикл
			ЗначениеДаты = ДанныеСтраницыРаздела2[СтруктураРеквизитовФормы.ИдМСЧ_Раздел2_1 + "01_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомСтр)];
			Если ЗначениеЗаполнено(ЗначениеДаты)
		...
		НомСтр = 1;
		Пока ДанныеСтраницыРаздела2.Свойство(СтруктураРеквизитовФормы.ИдМСЧ_Раздел2_2 + "01_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомСтр)) Цикл
			ЗначениеДаты = ДанныеСтраницыРаздела2[СтруктураРеквизитовФормы.ИдМСЧ_Раздел2_2 + "01_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомСтр)];
			Если ЗначениеЗаполнено(ЗначениеДаты)
				И (НачалоДня(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) <= ЗначениеДаты
				И ЗначениеДаты <= КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)) Тогда
				СтрокаПроверки = ДатыСобытий.Добавить();
...
Процедура УдалитьДополнительнуюСтрокуДаты(ИмяОбласти, МногострочнаяЧасть, ИдМСЧ)
	...
	Для Инд = НомерГиперссылки + 1 По МногострочнаяЧасть.Количество() Цикл
		ОбластьГиперссылки = ТабличныйДокумент.Области[ОсновноеИмяГиперссылки + "_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Инд)];
		ОбластьГиперссылки.Имя = ОсновноеИмяГиперссылки + "_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Инд - 1);
	КонецЦикла;
		
	Для Инд = НомерГиперссылки + 2 По МногострочнаяЧасть.Количество() Цикл
		ОбластьВводаДаты = ТабличныйДокумент.Области[ИдМСЧ + "01_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Инд)];
		ОбластьВводаДаты.Имя = ИдМСЧ + "01_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Инд - 1);
	КонецЦикла;
...
Процедура ФормироватьСтруктуруСвойствСтраниц() Экспорт
	...
	НовСтр = СтрокаУровня1.МногострочностьВРазделе.Добавить();
	НовСтр.ИдГруппы = СтруктураРеквизитовФормы.ИдМСЧ_Раздел4;
	НовСтр.Область = "МногострочнаяЧасть";
...
Процедура ВставитьДополнительнуюСтроку(ТекущаяСтраница)
	...
	Пока Истина Цикл 
		Инд = Инд + 1;
		Обл = ТекТабличноеПоле.Области.Найти("УдалитьСтроку" + ИмяГруппы + "1003_" + Формат(Инд, "ЧГ="));
	...
	Если ТекущаяСтраница = "Раздел1" Тогда
		...
		Область = ТабличныйДокумент.Области.Найти("П000010001003_" + КолСтрокСтр1);
		
		Если Область <> Неопределено Тогда
			
			ИзменитьРамкуОбластиТабличногоДокумента(Область);
			
		КонецЕсли;
		
	ИначеЕсли ТекущаяСтраница = "Раздел4" Тогда
		
		ИзменитьРамкиОбластейТабличногоДокумента(ЭтаФорма, ТекущаяСтраница)
...
Процедура УдалитьДополнительнуюСтроку(СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, ТекущаяСтраница) Экспорт
	...
	Если СтруктураГруппы.Количество() > ИсходноеКоличествоСтрокГруппы Тогда
		ИмяПервойЯчейкиУдаляемойСтроки = ИмяГруппы + "1003_" + Формат(НомУдаляемойСтроки, "ЧГ=");
	...
	ИмяТекущейОбласти = ИмяГруппы + "1003_" + Формат(НомТекущейСтроки, "ЧГ=");
	...
	Если ТекущаяСтраница = "Раздел4" Тогда
		...
		РасчетНаСервере(ТекущаяСтраница);
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ИмяФормы).ПолучитьМакет("СпискиВыбора2018Кв4");
...
Функция ОпределитьИдМногострочногоРазделаПоАктивнойСтраницеНаСервере(ИмяЛиста = Неопределено) Экспорт
	...
	ИдГрупп.Вставить("ИдМСЧ_Раздел4",   СтруктураРеквизитовФормы.ИдМСЧ_Раздел4);
...
Функция ОпределитьИдМногострочногоРазделаПоАктивнойСтранице(ИмяЛиста = Неопределено, ТекущаяСтраница, ИдГрупп)
	...
	ИначеЕсли ИмяТекущейСтраницыПанели = "Раздел4" Тогда
		ИмяГруппы = ИдГрупп.ИдМСЧ_Раздел4;
	КонецЕсли;
...
Функция ОпределитьТабличноеПолеПоИдМногострочногоРаздела(ИдГруппы, ИдГрупп) Экспорт
	...
	ИначеЕсли ИдГруппы = ИдГрупп.ИдМСЧ_Раздел4 Тогда
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если СписокСохранения.Свойство( "ДанныеМногострочныхРазделов", ДанныеМногострочныхРазделов) Тогда
		...
		Для Каждого ЭлементСтруктуры Из ДанныеМногострочныхРазделов Цикл
			...
			Если СписокСохранения.ПоказателиОтчета.ПолеТабличногоДокументаТитульный.Свойство("ИНН1_1") Тогда
				
				// Случай перехода с 2.0 на 3.0.
				
				Если ЭлементСтруктуры.Ключ = "П0000100010" Тогда
				...
				ИначеЕсли ЭлементСтруктуры.Ключ = "П0000400010" Тогда
					
					НайденноеЗначение = "П00004000";
					ЗначениеВДанныеФормы(ЭлементСтруктуры.Значение, ЭтаФорма[НайденноеЗначение]);
					ПоказателиРаздела4 = ПоказателиОтчета.ПолеТабличногоДокументаРаздел4;
					Для НомСтроки = 0 По ЭтаФорма[НайденноеЗначение].Количество() - 1 Цикл
						НомСтроки_ = Формат(НомСтроки + 1, "ЧГ=0");
						Для НомПодстроки = 1 По 5 Цикл
							ЭтаФорма[НайденноеЗначение][НомСтроки]["П00004000" + НомПодстроки + "003"]
							= ПоказателиРаздела4["П00004000" + НомПодстроки + "003_" + НомСтроки_];
						КонецЦикла;
					КонецЦикла;
...
Процедура ЗаполнитьСведенияОбОрганизацииНаСервере(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		// Стандартное определение КПП и кода налогового органа.
		КПП = СокрЛП(СведенияОбОрганизации.КППЮЛ);
		КодНО = СведенияОбОрганизации.КодНО;
		
		// Переопределение КПП и кода налогового органа юридического лица по данным регистраций в налоговом
		// органе, содержащих в КПП на 5-6 позициях (код причины постановки на учет) цифры 35 или 77.
		Если НЕ СтруктураРеквизитовФормы.ИП Тогда
			
			МассивКодовППУ = Новый Массив;
			МассивКодовППУ.Добавить("35");
			МассивКодовППУ.Добавить("77");
			
			ПараметрыОтбораРегистрацийВНО = Новый Структура("КодыПричиныПостановкиНаУчет", МассивКодовППУ);
			...
			Если РегистрацииВНО.Количество() = 0 Тогда
				...
				КодНО = "";
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru = 'Не найдена регистрация организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77""';
										|en = 'Не найдена регистрация организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77""'");
				Сообщение.Сообщить();
				
			ИначеЕсли РегистрацииВНО.Количество() = 1 Тогда
				
				КПП = СокрЛП(РегистрацииВНО[0].КПП);
				КодНО = СокрЛП(РегистрацииВНО[0].Код);
				
			Иначе
				
				ПараметрыОтбораРегистрацийВНО = Новый Структура("КодНО,КодыПричиныПостановкиНаУчет", КодНО, МассивКодовППУ);
				...
				Иначе
					...
					КодНО = "";
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = НСтр("ru = 'Найдено более 1-й регистрации организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77"". Укажите код налогового органа на титульном листе.';
											|en = 'Найдено более 1-й регистрации организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77"". Укажите код налогового органа на титульном листе.'");
					Сообщение.Сообщить();
		...
		// Установка значений КПП и кода налогового органа на титульный лист.
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС, КПП, Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт
	...
	СтруктураРеквизитовФормы.мСохраненныйДок.ИсточникОтчета
		= Метаданные.Отчеты[ИмяОтчета(ИмяФормы)].Имя;
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если Лев(Область.Имя, 13) = "П000010001003" Тогда
		ИзменитьРамкуОбластиТабличногоДокумента(Область);
	Иначе
		ИзменитьРамкиОбластейТабличногоДокумента(ЭтаФорма, СтруктураРеквизитовФормы.НаимТекущегоРаздела);
	КонецЕсли;
...
Процедура ОбновитьАдресВТабличномДокументе(Результат, РоссийскийАдрес)
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		ИзменитьРамкуОбластиТабличногоДокумента(ТабличныйДокумент.Области.П000020002002);
...
Процедура ОбработкаВыбораИФНС()
	...
	Если НЕ СтруктураРеквизитовФормы.ИП Тогда
		...
		МассивКодовППУ.Добавить("35");
		МассивКодовППУ.Добавить("77");
		ЗначенияДляОтбора.Добавить(Новый Структура("КодыПричиныПостановкиНаУчет", МассивКодовППУ));
	КонецЕсли;
...
Процедура ОбработкаВыбораИФНСЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено Тогда
		...
		ИзменитьРамкуОбластиТабличногоДокумента(ТабличныйДокумент.Области.КодИМНС);
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "", ПриИзмененииСодержимогоОбласти = Ложь)
	...
	ИначеЕсли ИмяСтраницыПанели = "Раздел3" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000030005003, ОбластьИмя) > 0 Тогда
			...
			Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел3" Тогда
				...
				Строка040 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ТабличныйДокумент.Области.П000030004003.Значение);
				...
				КонецЕсли;
				Форма.ТабличныйДокумент.Области.П000030005003.Значение = ЗначениеСтроки050;
			...
			Иначе
				...
				Если Форма.СтруктураДанныхРаздел3.Количество() > 0 Тогда
					...
					Строка040 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.СтруктураДанныхРаздел3.П000030004003);
					...
					Иначе
						ЗначениеСтроки050 = Макс(0, Строка010 - Строка030 - Строка040);
					КонецЕсли;
					Форма.СтруктураДанныхРаздел3.П000030005003 = ЗначениеСтроки050;
	...
	ИначеЕсли ИмяСтраницыПанели = "Раздел4" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел4" Тогда
			...
			НомСтр = 1;
			Обл = Форма.ТабличныйДокумент.Области.П000040005003_1;
			Пока Обл <> Неопределено Цикл
				...
				НомСтр = НомСтр + 1;
				Обл = Форма.ТабличныйДокумент.Области.Найти("П000040005003_" + Формат(НомСтр, "ЧГ="));
			КонецЦикла;
			
			Форма.ТабличныйДокумент.Области.П000040015003.Значение = ОбщаяСумма;
			
			Если НЕ Форма.СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
				Форма.СтруктураДанныхРаздел3.П000030004003 = ОбщаяСумма;
			КонецЕсли;
			
		Иначе
			
			Для каждого СтрокаТЗ Из Форма.П00004000 Цикл
				ОбщаяСумма = ОбщаяСумма
				+ РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, СтрокаТЗ["П000040005003"]);
			КонецЦикла;
			
			Форма.СтруктураДанныхРаздел4.П000040015003 = ОбщаяСумма;
			
			Если НЕ Форма.СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
				Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел3" Тогда
					Форма.ТабличныйДокумент.Области.П000030004003.Значение = ОбщаяСумма;
				Иначе
					Форма.СтруктураДанныхРаздел3.П000030004003 = ОбщаяСумма;
				КонецЕсли;
			КонецЕсли;
...
Процедура РасчетВСтруктуре() Экспорт
	...
	Если СтруктураДанныхРаздел3.Количество() = 0 Тогда
		...
		СтруктураДанныхРаздел3.Вставить("П000030005003", 0);
	КонецЕсли;
	...
	// Расчет в разделе 4. Сбор данных в раздел 3 из раздела 4.
	ЗначП000030004003 = 0;
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел4" Тогда
		НомСтр = 1;
		Обл = ТабличныйДокумент.Области.П000040005003_1;
		Пока Обл <> Неопределено Цикл
			ЗначП000030004003 = ЗначП000030004003
			+ РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Обл.Значение);
			НомСтр = НомСтр + 1;
			Обл = ТабличныйДокумент.Области.Найти("П000040005003_" + Формат(НомСтр, "ЧГ="));
		КонецЦикла;
		ТабличныйДокумент.Области.П000040015003.Значение = ЗначП000030004003;
	Иначе
		Для каждого СтрокаТЗ Из П00004000 Цикл
			ЗначП000030004003 = ЗначП000030004003
			+ РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, СтрокаТЗ["П000040005003"]);
		КонецЦикла;
		СтруктураДанныхРаздел4.П000040015003 = ЗначП000030004003;
	КонецЕсли;
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел3" Тогда
		...
		ТабличныйДокумент.Области.П000030004003.Значение = ЗначП000030004003;
	Иначе
		СтруктураДанныхРаздел3.П000030001003 = ЗначП000030001003;
		СтруктураДанныхРаздел3.П000030004003 = ЗначП000030004003;
	КонецЕсли;
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел3" Тогда
		...
		СтруктураДанныхРаздел3.П000030005003 = ТабличныйДокумент.Области.П000030005003.Значение;
	КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ДанныеТаблПоле.Значение.П000020003000) Тогда
			...
			Если Форма.СтруктураРеквизитовФормы.СообщатьОНезаполненномКодеПоОКТМОВРазделе2 Тогда
				...
				Сообщение.Текст = НСтр("ru = 'Не заполнен код ОКТМО на странице Раздела 2.';
										|en = 'Не заполнен код ОКТМО на странице Раздела 2.'");
				Сообщение.Сообщить();
			КонецЕсли;
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если СтрНайти(Область.Имя, "П000010001003") > 0 ИЛИ Область.Имя = "П000020003000" Тогда
		...
		Если НЕ (ДлинаОКТМО = 0 ИЛИ ДлинаОКТМО = 8 ИЛИ ДлинаОКТМО = 11) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Код по ОКТМО должен состоять из 8 или 11 цифр.';
									|en = 'Код по ОКТМО должен состоять из 8 или 11 цифр.'");
			Сообщение.Сообщить();
	...
	Если Область.Имя = "П000020010503" Тогда
		...
		Если НЕ (Область.Значение >= 7.5 И Область.Значение <= 15) Тогда
			
			ПоказатьПредупреждение(,НСтр("ru = 'Ставка единого налога может быть установлена в пределах от 7.5 до 15 процентов.';
										|en = 'Ставка единого налога может быть установлена в пределах от 7.5 до 15 процентов.'"));
...
Функция ЗначениеК1(Форма)
	...
	Если Год(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2020 Тогда
		Возврат 2.005;
	ИначеЕсли Год(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2019 Тогда
		Возврат 1.915;
	Иначе
		Возврат 1.868;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ИзменитьРамкиОбластейТабличногоДокумента(Форма, ИмяСтраницыПанели = "")
	
	ПараметрыВыделяемыхОбластей = Новый Массив;
	
	Если ИмяСтраницыПанели = "Титульный" Тогда
		
		ПараметрыВыделяемыхОбластей.Добавить(Новый Структура("ИмяОбласти", "Период"));
		ПараметрыВыделяемыхОбластей.Добавить(Новый Структура("ИмяОбласти", "КодИМНС"));
		ПараметрыВыделяемыхОбластей.Добавить(Новый Структура("ИмяОбласти", "ПоМесту"));
		ПараметрыВыделяемыхОбластей.Добавить(Новый Структура("ИмяОбласти", "РеорганизацияИНН"));
		ПараметрыВыделяемыхОбластей.Добавить(Новый Структура("ИмяОбласти", "РеорганизацияКПП"));
		ПараметрыВыделяемыхОбластей.Добавить(Новый Структура("ИмяОбласти", "ДатаПодписи"));
		
		// Особое изменение рамки поля "Налогоплательщик".
		НезаполненнаяЛиния = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии);
		ДанныеОбластиНаимОрг = Новый Структура;
		ДанныеОбластиНаимОрг.Вставить("ИмяОбласти", "НаимОрг");
		ДанныеОбластиНаимОрг.Вставить("ГраницаСверхуАвто", НезаполненнаяЛиния);
		ДанныеОбластиНаимОрг.Вставить("ГраницаСлеваАвто", НезаполненнаяЛиния);
		ДанныеОбластиНаимОрг.Вставить("ГраницаСправаАвто", НезаполненнаяЛиния);
		ПараметрыВыделяемыхОбластей.Добавить(ДанныеОбластиНаимОрг);
		
	ИначеЕсли ИмяСтраницыПанели = "Раздел1" Тогда
		
		ПараметрыВыделяемыхОбластей.Добавить(Новый Структура("ИмяОбласти", "П000010001003_1"));
		
	ИначеЕсли ИмяСтраницыПанели = "Раздел2" Тогда
		
		ПараметрыВыделяемыхОбластей.Добавить(Новый Структура("ИмяОбласти", "П000020001000"));
		ПараметрыВыделяемыхОбластей.Добавить(Новый Структура("ИмяОбласти", "П000020002002"));
		ПараметрыВыделяемыхОбластей.Добавить(Новый Структура("ИмяОбласти", "П000020003000"));
		ПараметрыВыделяемыхОбластей.Добавить(Новый Структура("ИмяОбласти", "П000020004003"));
		ПараметрыВыделяемыхОбластей.Добавить(Новый Структура("ИмяОбласти", "П000020006003"));
		
	ИначеЕсли ИмяСтраницыПанели = "Раздел3" Тогда
		
		ПараметрыВыделяемыхОбластей.Добавить(Новый Структура("ИмяОбласти", "П000030000503"));
		
	КонецЕсли;
	
	РегламентированнаяОтчетностьКлиентСервер.ИзменитьРамкиОбластейТабличногоДокумента(
	Форма.ТабличныйДокумент, ПараметрыВыделяемыхОбластей);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ИзменитьРамкуОбластиТабличногоДокумента(Область)
	
	РегламентированнаяОтчетностьКлиентСервер.ИзменитьРамкуОбластиТабличногоДокумента(
	Новый Структура("Область", Область));
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяСтраницыПанели = "")
	...
	ИначеЕсли ИмяСтраницыПанели = "Раздел2" Тогда
		...
		Инд = 1;
		Пока Истина Цикл
			ИмяПоляДаты        = "П000020006201_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Инд);
			ИмяПоляГиперссылки = "П000020006301_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Инд);
		...
		Инд = 1;
		Пока Истина Цикл
			ИмяПоляДаты        = "П000020006501_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Инд);
			ИмяПоляГиперссылки = "П000020006601_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Инд);
	...
	ИначеЕсли ИмяСтраницыПанели = "Раздел3" Тогда
		...
		Если ПризнакНалогоплательщика = "2"
			ИЛИ (ПризнакНалогоплательщика = "1" И Форма.СтруктураРеквизитовФормы.ИП) Тогда
...
Процедура РазделыОтчетаПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1"
		ИЛИ Элемент.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел4" Тогда
...
Процедура ВывестиМакетСтраницыВТабличныйДокументНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	ИзменитьРамкиОбластейТабличногоДокумента(ЭтаФорма, ИмяТекРаздела);
...
Процедура ВставитьДополнительнуюСтраницуНаСервере(ИмяТекущейСтраницыПанели = Неопределено)
	...
	ИзменитьРамкиОбластейТабличногоДокумента(ЭтаФорма, ИмяТекущейСтраницыПанели);
...
Процедура ОчиститьТабличноеПоле(Форма, ВыбТабличноеПолеИмя)
	...
	ИначеЕсли ВыбТабличноеПолеИмя = "Раздел1" Тогда
	...
	ИначеЕсли ВыбТабличноеПолеИмя = "Раздел4" Тогда
		
		МассивМногострочныхБлоков.Добавить(Форма.СтруктураРеквизитовФормы.ИдМСЧ_Раздел4);
		...
		Если ТекущееКоличествоСтрок > ИсходноеКоличествоСтрокГруппы Тогда
			...
			Пока НСтр > ИсходноеКоличествоСтрокГруппы Цикл
				...
				ИначеЕсли ВыбТабличноеПолеИмя = "Раздел4" Тогда
					...
					НомерВерхЯчейкиОбласти = ОбластьЯчейки.Верх;
					НомерНижнЯчейкиОбласти = ОбластьЯчейки.Низ + 9;
				...
				ТабличныйДокумент = ВыбТабличноеПоле;
...
Процедура СформироватьРаздел1НаСервере()
	...
	Если СтруктураДанныхРаздел3.Количество() > 0 Тогда
		...
		Раздел3Строка050 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, СтруктураДанныхРаздел3.П000030005003);
	Иначе
		Раздел3Строка010 = 0;
		Раздел3Строка050 = 0;
	КонецЕсли;
	...
	Если ИтоговаяТаблица.Количество() > 0 Тогда
		...
		Для Инд = 0 По ИтоговаяТаблица.Количество() - 1 Цикл
			...
			НоваяСтрокаМСЧ.П000010002003 =
				РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, СуммаНалога);
...
Процедура ЗаполнитьАвтоЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ СтруктураРеквизитовФормы.ИП Тогда
		Если НЕ ЗначениеЗаполнено(КПП) Тогда
			...
			Сообщение.Текст = НСтр(
				...
				|en = 'Для заполнения отчета необходимо значение КПП налогоплательщика. Внесите исправление в справочник организаций и обновите сведения в декларации (меню ""Еще"" - ""Обновить"").'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(КПП) <> 9 Тогда
			...
			Сообщение.Текст = НСтр(
				...
				|en = 'Задан КПП налогоплательщика, не состоящий из 9 символов. Внесите исправление в справочник организаций и обновите сведения в декларации (меню ""Еще"" - ""Обновить"").'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(КодНО) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не задан код налогового органа, в который представляется декларация.';
								|en = 'Не задан код налогового органа, в который представляется декларация.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(КодНО) <> 4 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодНО) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Задан код налогового органа, не состоящий из 4 цифр.';
								|en = 'Задан код налогового органа, не состоящий из 4 цифр.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Иначе
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			...
			|en = 'Для налогоплательщика по %1 не найдена соответствующая запись в справочнике ""Регистрации в налоговом органе"". Заполнение декларации прервано.'"),
...
Процедура ЗаполнитьАвтоНаСервере(ОтчетЗаполненУспешно)
	...
	Если РегистрацииВНО.Количество() > 0 Тогда
		...
		Контейнер.Вставить("Раздел4_ДанныеДопСтрок", ДанныеФормыВЗначение(П00004000, Тип("ТаблицаЗначений")));
		
		РегламентированнаяОтчетностьПереопределяемый.ЗаполнитьОтчет(
			ИмяОтчета(ИмяФормы),
			ИмяФормыОтчета(ИмяФормы), ПараметрыОтчета, Контейнер);
		...
		ЗначениеВДанныеФормы(Контейнер.Раздел4_ДанныеДопСтрок, П00004000);
		
		Если П00004000.Количество() = 0 Тогда
			П00004000.Добавить();
		КонецЕсли;
...
Процедура ЗаполнитьСтруктурыДанныхРазделовЗначениямиИзДопСтрок()
	...
	СтруктураДанныхРаздел4, П00004000);
...
Процедура ОчиститьТекущуюСтраницу(Команда)
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел3" Тогда
		СтруктураРеквизитовФормы.Вставить("Раздел3Строка040ДоОчистки", ТабличныйДокумент.Области.П000030004003.Значение);
	КонецЕсли;
...
Процедура ОчиститьТекущуюСтраницуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		РегламентированнаяОтчетностьКлиентСервер.ОтрисоватьЗначкиУдаленияСтрок(ЭтаФорма);
		Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел3"
			И СтруктураРеквизитовФормы.Свойство("Раздел3Строка040ДоОчистки") Тогда
			ТабличныйДокумент.Области.П000030004003.Значение = СтруктураРеквизитовФормы["Раздел3Строка040ДоОчистки"];
		КонецЕсли;
	КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ДанныеРазделов.Вставить(СтруктураРеквизитовФормы.ИдМСЧ_Раздел1,
		ДанныеФормыВЗначение(ЭтаФорма[СтруктураРеквизитовФормы.ИдМСЧ_Раздел1], Тип("ТаблицаЗначений")));
	ДанныеРазделов.Вставить(СтруктураРеквизитовФормы.ИдМСЧ_Раздел2_1,
		ДанныеФормыВЗначение(ЭтаФорма[СтруктураРеквизитовФормы.ИдМСЧ_Раздел2_1], Тип("ТаблицаЗначений")));
	ДанныеРазделов.Вставить(СтруктураРеквизитовФормы.ИдМСЧ_Раздел2_2,
		ДанныеФормыВЗначение(ЭтаФорма[СтруктураРеквизитовФормы.ИдМСЧ_Раздел2_2], Тип("ТаблицаЗначений")));
	ДанныеРазделов.Вставить(СтруктураРеквизитовФормы.ИдМСЧ_Раздел4,
		ДанныеФормыВЗначение(ЭтаФорма[СтруктураРеквизитовФормы.ИдМСЧ_Раздел4], Тип("ТаблицаЗначений")));	
...
Функция УниверсальнаяВыгрузка(ТекстВыгрузки = Неопределено, ПроверятьБлокировки = Истина) Экспорт
	...
	Если СтруктураРеквизитовФормы.мПроверятьСоотношенияПриПечатиИВыгрузки = Истина Тогда
		РегламентированнаяОтчетностьКлиент.ПроверитьКонтрольныеСоотношения(ЭтаФорма, ОбработкаКС, Истина);
	КонецЕсли;
	Если НЕ УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки, ПроверятьБлокировки) Тогда
		Возврат Ложь;
	КонецЕсли;
...
&НаСервере
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки, ПроверятьБлокировки)
	...
	ПараметрыРеглОтчета.Вставить("ПроверятьБлокировки", ПроверятьБлокировки);
	...
	Если ТипЗнч(РезультатВыгрузки) = Тип("Массив") Тогда
		...
		РеглОтчетВыгружен = РезультатВыгрузки[0].РеглОтчетВыгружен;
		
	Иначе
		
		РеглОтчетВыгружен = РезультатВыгрузки.РеглОтчетВыгружен;
		
		Если РеглОтчетВыгружен Тогда
...
Функция СформироватьСтруктуруПоказателей()
	...
	ОбъектОтчета.СформироватьСтруктуруПоказателей(СтруктураРеквизитовФормы.мВыбраннаяФорма, СтруктураРеквизитовФормы);
...
Процедура ОчисткаРегламентированногоОтчетаНаСервере(ИмяСтраницы, ЗапускатьОтдельнуюОбработкуПослеОчисткиЛиста, РежимОчистки, Многостраничность, КолВоСтраницВРазделе) Экспорт
	...
	Если СтрНайти(РежимОчистки, "ОчиститьТекущийРаздел") > 0 Тогда
		...
		Если (Многостраничность) И (КолВоСтраницВРазделе > 1) Тогда
			...
			ИзменитьРамкиОбластейТабличногоДокумента(ЭтаФорма, ИмяСтраницы);
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если СтрНачинаетсяС(Область.Имя, "П000040004003") Тогда
		Если ЗначениеЗаполнено(Область.Значение)
			И (Область.Значение > СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета
			ИЛИ Область.Значение < '19000101') Тогда
			Область.Очистить();
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Дата регистрации контрольно-кассовой техники в налоговом органе не должна выходить за пределы периода с 01.01.1900 по "
			+ Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ=дд.ММ.гггг") + ". Значение очищено.'");
			Сообщение.Сообщить();
		КонецЕсли;
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П000040005003") Тогда
		Если РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Область.Значение) > 18000 Тогда
			Область.Очистить();
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Сумма расходов по приобретению одного экземпляра контрольно-кассовой техники не может превышать 18000 рублей. Значение очищено.';
									|en = 'Сумма расходов по приобретению одного экземпляра контрольно-кассовой техники не может превышать 18000 рублей. Значение очищено.'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
Процедура ТабличныйДокументПриАктивизации(Элемент)
	...
	Если ЗначениеЗаполнено(ИмяГруппы) Тогда
		...
		Если ИмяГруппы <> "П0000200062" И ИмяГруппы <> "П0000200065" Тогда
			...
			ТестовоеИмя = ИмяГруппы + "1003_" + Формат(мСтруктураИсхКолвоСтрокРазделов[ИмяГруппы] + 1, "ЧГ=");
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	ПроверитьКС_1_11а_доп(СтруктураРезультата);
	ПроверитьКС_1_11б_доп(СтруктураРезультата);
	ПроверитьКС_1_12_доп(СтруктураРезультата);
...
Процедура ПроверитьКС_1_1(СтруктураРезультата)
	...
	ОписаниеКС = НСтр("ru = 'Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 020 Раздел1 по ОКТМО = %1)
							|должна быть равна произведению суммы налога, подлежащей уплате в бюджет (строка 050 Раздела 3) и результата деления суммы строк 110
							|всех Разделов 2 (по ОКТМО = %1) и суммы налога, исчисленного по всем ОКТМО (строка 010 Раздела 3)';
							|en = 'Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 020 Раздел1 по ОКТМО = %1)
							|должна быть равна произведению суммы налога, подлежащей уплате в бюджет (строка 050 Раздела 3) и результата деления суммы строк 110
							|всех Разделов 2 (по ОКТМО = %1) и суммы налога, исчисленного по всем ОКТМО (строка 010 Раздела 3)'");
	...
	Для Индекс = 1 По КоличествоБлоков Цикл
		ИмяПоказателяОКТМО = "П000010002003_" + Формат(Индекс, "ЧГ=");
	...
	Для Индекс = 1 По КоличествоБлоков Цикл
		...
		Р3Стр050 = ПоказательПоСтроке("Р3Стр050");
		ВставитьЗначениеПоказателя(СтруктураРаздела3, Р3Стр050);
		ВнестиПоказательВРасшифровку(Р3Стр050, "Раздел3");
		СуммаНалогаДолжноБытьВсего = Р3Стр050.Значение;
		//СуммаНалогаПоВсемОКТМО = 0;
		...
		Иначе
			...
			ПраваяЧасть = ?(Р3Стр010.Значение = 0, 0, РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма,
				Р3Стр050.Значение * СуммаР2Стр110 / Р3Стр010.Значение));
			...
			Иначе
				ПраваяЧастьРасшифровка = Р3Стр050.ЗначениеСтр + " * " + СуммаР2Стр110Стр + " / " + Р3Стр010.ЗначениеСтр;
...
Процедура ПроверитьКС_1_2(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка раздела 2 (КС 1.2)';
						|en = 'Проверка раздела 2 (КС 1.2)'");
	ОписаниеКС = НСтр("ru = 'Код вида предпринимательской деятельности (строка 010 Раздела 2) должен соответствовать значению из справочника ""Коды видов предпринимательской деятельности и значения базовой доходности на единицу физического показателя""';
						|en = 'Код вида предпринимательской деятельности (строка 010 Раздела 2) должен соответствовать значению из справочника ""Коды видов предпринимательской деятельности и значения базовой доходности на единицу физического показателя""'");
	
	Нарушение      = НСтр("ru = 'Статья 346.27 НК РФ';
							|en = 'Статья 346.27 НК РФ'");
	ОписаниеОшибки = НСтр("ru = 'Неправомерное применение системы налогообложения в виде ЕНВД';
							|en = 'Неправомерное применение системы налогообложения в виде ЕНВД'");
	
	ИмяРаздела = "Раздел2";
	НазваниеСписка = "СпискиВыбора2018Кв4";
	НазваниеОбласти = "КодыВидовДеятельности";
	ТекстВЗаголовкеКРасшифровке = "Соответствие кода вида предпринимательской деятельности справочному";
	...
	Для Каждого Страница Из СтруктураРеквизитовФормы.мДанныеОтчета[ИмяРаздела] Цикл
		...
		Если НЕ НайденнаяСтр = Неопределено Тогда
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
			ЭтоОшибка = Ложь;
		...
		Расшифровка = "Код """ + Р2Стр010.Значение + """" + ?(НайденнаяСтр = Неопределено, " не", "") + " соответствует справочному";
...
Процедура ПроверитьКС_1_3(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка раздела 2 (КС 1.3)';
						|en = 'Проверка раздела 2 (КС 1.3)'");
	ОписаниеКС   = НСтр("ru = 'Налоговая база в 1-ом месяце отчетного периода (строка 070 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 070 графы 2, строки 070 графы 3 деленое на количество календарных дней в 1-ом месяце отчетного периода';
						|en = 'Налоговая база в 1-ом месяце отчетного периода (строка 070 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 070 графы 2, строки 070 графы 3 деленое на количество календарных дней в 1-ом месяце отчетного периода'");
	
	Нарушение      = НСтр("ru = 'Пункт 10 статьи 346.29 НК РФ';
							|en = 'Пункт 10 статьи 346.29 НК РФ'");
	ОписаниеОшибки = НСтр("ru = 'Неправильное исчисление налоговой базы за данный месяц';
							|en = 'Неправильное исчисление налоговой базы за данный месяц'");
	
	Р2Стр040     = ПоказательПоСтроке("Р2Стр040");
	Р2Стр050     = ПоказательПоСтроке("Р2Стр050");
	Р2Стр060     = ПоказательПоСтроке("Р2Стр060");
	Р2Стр070     = ПоказательПоСтроке("Р2Стр070");
	Р2Стр070Гр02 = ПоказательПоСтроке("Р2Стр070Гр02");
	Р2Стр070Гр03 = ПоказательПоСтроке("Р2Стр070Гр03");
	
	НомерСтраницы = 1;
	
	КалендарныхДнейВПервомОтчетномМесяце = День(КонецМесяца(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета));
	...
	Для каждого Страница Из СтруктураРеквизитовФормы.мДанныеОтчета.Раздел2 Цикл
		...
		ВставитьЗначениеПоказателя(Раздел2, Р2Стр050);
		ВставитьЗначениеПоказателя(Раздел2, Р2Стр060);
		ВставитьЗначениеПоказателя(Раздел2, Р2Стр070);
		ВставитьЗначениеПоказателя(Раздел2, Р2Стр070Гр02);
		ВставитьЗначениеПоказателя(Раздел2, Р2Стр070Гр03);
		
		СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
		ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
		
		ВнестиПоказательВРасшифровку(Р2Стр040,     "Раздел2", НомерСтраницы);
		ВнестиПоказательВРасшифровку(Р2Стр050,     "Раздел2", НомерСтраницы);
		ВнестиПоказательВРасшифровку(Р2Стр060,     "Раздел2", НомерСтраницы);
		ВнестиПоказательВРасшифровку(Р2Стр070,     "Раздел2", НомерСтраницы);
		ВнестиПоказательВРасшифровку(Р2Стр070Гр02, "Раздел2", НомерСтраницы);
		ВнестиПоказательВРасшифровку(Р2Стр070Гр03, "Раздел2", НомерСтраницы);
		
		КоличествоДней = ?(Р2Стр070Гр03.Значение > 0, Р2Стр070Гр03.Значение, КалендарныхДнейВПервомОтчетномМесяце);
		
		ПраваяЧасть = Р2Стр040.Значение * Р2Стр050.Значение * Р2Стр060.Значение * Р2Стр070Гр02.Значение * КоличествоДней / КалендарныхДнейВПервомОтчетномМесяце;
		ПраваяЧастьРасшифровка = "" + Р2Стр040.Значение + " * " + Р2Стр050.Значение + " * " + Р2Стр060.Значение +" * " + Р2Стр070Гр02.Значение + " * "
		+ КоличествоДней + " / " + КалендарныхДнейВПервомОтчетномМесяце;
		
		КС = ПоказателиКС(Р2Стр070.Значение, ПраваяЧасть);
...
Процедура ПроверитьКС_1_4(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка раздела 2 (КС 1.4)';
						|en = 'Проверка раздела 2 (КС 1.4)'");
	ОписаниеКС   = НСтр("ru = 'Налоговая база во 2-ом месяце отчетного периода (строка 080 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 080 графы 2, строки 080 графы 3 деленое на количество календарных дней во 2-ом месяце отчетного периода';
						|en = 'Налоговая база во 2-ом месяце отчетного периода (строка 080 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 080 графы 2, строки 080 графы 3 деленое на количество календарных дней во 2-ом месяце отчетного периода'");
	...
	Р2Стр080Гр02 = ПоказательПоСтроке("Р2Стр080Гр02");
	Р2Стр080Гр03 = ПоказательПоСтроке("Р2Стр080Гр03");
	
	НомерСтраницы = 1;
	
	КалендарныхДнейВоВторомОтчетномМесяце = День(КонецМесяца(КонецМесяца(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) + 1));
	...
	Для каждого Страница Из СтруктураРеквизитовФормы.мДанныеОтчета.Раздел2 Цикл
		...
		ВставитьЗначениеПоказателя(Раздел2, Р2Стр080Гр02);
		ВставитьЗначениеПоказателя(Раздел2, Р2Стр080Гр03);
		...
		ВнестиПоказательВРасшифровку(Р2Стр080Гр02, "Раздел2", НомерСтраницы);
		ВнестиПоказательВРасшифровку(Р2Стр080Гр03, "Раздел2", НомерСтраницы);
		
		КоличествоДней = ?(Р2Стр080Гр03.Значение > 0, Р2Стр080Гр03.Значение, КалендарныхДнейВоВторомОтчетномМесяце);
		
		ПраваяЧасть = Р2Стр040.Значение * Р2Стр050.Значение * Р2Стр060.Значение * Р2Стр080Гр02.Значение * КоличествоДней / КалендарныхДнейВоВторомОтчетномМесяце;
		ПраваяЧастьРасшифровка = "" + Р2Стр040.Значение + " * " + Р2Стр050.Значение + " * " + Р2Стр060.Значение +" * " + Р2Стр080Гр02.Значение + " * "
		+ КоличествоДней + " / " + КалендарныхДнейВоВторомОтчетномМесяце;
		
		КС = ПоказателиКС(Р2Стр080.Значение, ПраваяЧасть);
...
Процедура ПроверитьКС_1_5(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка раздела 2 (КС 1.5)';
						|en = 'Проверка раздела 2 (КС 1.5)'");
	ОписаниеКС   = НСтр("ru = 'Налоговая база в 3-ем месяце отчетного периода (строка 090 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 090 графы 2, строки 090 графы 3 деленое на количество календарных дней в 3-ем месяце отчетного периода';
						|en = 'Налоговая база в 3-ем месяце отчетного периода (строка 090 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 090 графы 2, строки 090 графы 3 деленое на количество календарных дней в 3-ем месяце отчетного периода'");
	...
	Р2Стр090Гр02 = ПоказательПоСтроке("Р2Стр090Гр02");
	Р2Стр090Гр03 = ПоказательПоСтроке("Р2Стр090Гр03");
	
	НомерСтраницы = 1;
	
	КалендарныхДнейВТретьемОтчетномМесяце = День(КонецМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
	...
	Для каждого Страница Из СтруктураРеквизитовФормы.мДанныеОтчета.Раздел2 Цикл
		...
		ВставитьЗначениеПоказателя(Раздел2, Р2Стр090Гр02);
		ВставитьЗначениеПоказателя(Раздел2, Р2Стр090Гр03);
		...
		ВнестиПоказательВРасшифровку(Р2Стр090Гр02, "Раздел2", НомерСтраницы);
		ВнестиПоказательВРасшифровку(Р2Стр090Гр03, "Раздел2", НомерСтраницы);
		
		КоличествоДней = ?(Р2Стр090Гр03.Значение > 0, Р2Стр090Гр03.Значение, КалендарныхДнейВТретьемОтчетномМесяце);
		
		ПраваяЧасть = Р2Стр040.Значение * Р2Стр050.Значение * Р2Стр060.Значение * Р2Стр090Гр02.Значение * КоличествоДней / КалендарныхДнейВТретьемОтчетномМесяце;
		ПраваяЧастьРасшифровка = "" + Р2Стр040.Значение + " * " + Р2Стр050.Значение + " * " + Р2Стр060.Значение +" * " + Р2Стр090Гр02.Значение + " * "
		+ КоличествоДней + " / " + КалендарныхДнейВТретьемОтчетномМесяце;
		
		КС = ПоказателиКС(Р2Стр090.Значение, ПраваяЧасть);
...
Процедура ПроверитьКС_1_6(СтруктураРезультата)
	...
	ОписаниеКС   = НСтр("ru = 'Сумма налоговой базы по ЕНВД (строка 100 Раздела 2) должна быть равна сумме значений по строкам 070, 080, 090 графы 4 Раздела 2 по одноименному ОКТМО (строка 030)';
						|en = 'Сумма налоговой базы по ЕНВД (строка 100 Раздела 2) должна быть равна сумме значений по строкам 070, 080, 090 графы 4 Раздела 2 по одноименному ОКТМО (строка 030)'");
	
	Нарушение      = НСтр("ru = 'п.п. 2, 4 ст.346.29 НК РФ';
							|en = 'п.п. 2, 4 ст.346.29 НК РФ'");
	ОписаниеОшибки = НСтр("ru = 'Неправильно исчислена налоговая база за налоговый период.';
							|en = 'Неправильно исчислена налоговая база за налоговый период.'");
...
Процедура ПроверитьКС_1_7(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка раздела 2 (КС 1.7)';
						|en = 'Проверка раздела 2 (КС 1.7)'");
	ОписаниеКС   = НСтр("ru = 'Сумма исчисленного единого налога на вмененный доход (строка 110 Раздела 2) должна быть равна произведению налоговой базы (строка 100 Раздела 2) и налоговой ставки по одноименному ОКТМО (строка 105 Раздела 2) деленному на 100';
						|en = 'Сумма исчисленного единого налога на вмененный доход (строка 110 Раздела 2) должна быть равна произведению налоговой базы (строка 100 Раздела 2) и налоговой ставки по одноименному ОКТМО (строка 105 Раздела 2) деленному на 100'");
	
	Нарушение      = НСтр("ru = 'п.п. 2, 4 ст.346.29 и ст.246.31 НК РФ';
							|en = 'п.п. 2, 4 ст.346.29 и ст.246.31 НК РФ'");
	ОписаниеОшибки = НСтр("ru = 'Неправильное исчисление суммы ЕНВД за налоговый период';
							|en = 'Неправильное исчисление суммы ЕНВД за налоговый период'");
	
	Р2Стр100 = ПоказательПоСтроке("Р2Стр100");
	Р2Стр105 = ПоказательПоСтроке("Р2Стр105");
	Р2Стр110 = ПоказательПоСтроке("Р2Стр110");
	...
	Для каждого Страница Из СтруктураРеквизитовФормы.мДанныеОтчета.Раздел2 Цикл
		...
		ВставитьЗначениеПоказателя(Раздел2, Р2Стр100);
		ВставитьЗначениеПоказателя(Раздел2, Р2Стр105);
		
		СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
		ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
		
		ВнестиПоказательВРасшифровку(Р2Стр100, "Раздел2", НомерСтраницы);
		ВнестиПоказательВРасшифровку(Р2Стр105, "Раздел2", НомерСтраницы);
		ВнестиПоказательВРасшифровку(Р2Стр110, "Раздел2", НомерСтраницы);
		
		КС = ПоказателиКС(Р2Стр110.Значение, (Р2Стр100.Значение * Р2Стр105.Значение / 100));
		
		Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р2Стр100.ЗначениеСтр + " * " + Р2Стр105.ЗначениеСтр + "% Итог [" + КС.ПраваяЧастьСтр + "]";
...
Процедура ПроверитьКС_1_8(СтруктураРезультата)
	...
	ОписаниеКС   = НСтр("ru = 'Сумма исчисленного единого налога на вмененный доход (строка 010 Раздел 3) должна быть равна сумме строк 110 всех заполненных Разделов 2';
						|en = 'Сумма исчисленного единого налога на вмененный доход (строка 010 Раздел 3) должна быть равна сумме строк 110 всех заполненных Разделов 2'");
...
Процедура ПроверитьКС_1_9(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка раздела 3 (КС 1.9)';
						|en = 'Проверка раздела 3 (КС 1.9)'");
	ОписаниеКС   = НСтр("ru='Сумма единого налога на вмененный доход, подлежащая к уплате (строка 050 Раздела 3) " +
		"должна быть равна разнице значений строки 010 Раздела 3 и строк 020, 030 Раздела 3, если результат такого " +
		"вычитания больше или равен 50% строки 010 Раздела 3 и строка 005 Раздела 3 = ""1""" + ?(СтруктураРеквизитовФормы.ИП,
		". Для ИП строка 050 Раздела 3 может быть уменьшена на значение строки 040 Раздела 3 до 0'", "'"));
	
	ШаблонРасшифровки	= НСтр("ru = '%1, когда строка 005 Раздела 3 = ""1""';
								|en = '%1, когда строка 005 Раздела 3 = ""1""'");
	ШаблонРасшифровки1	= НСтр("ru = '%1 = %2 - %3 - %4, но не менее %2 х 0,5 Итог [%5]';
								|en = '%1 = %2 - %3 - %4, но не менее %2 х 0,5 Итог [%5]'");
	ШаблонРасшифровки2	= НСтр("ru = '%1 = (%2 - %3 - %4, но не менее %2 х 0,5 Итог [%5]) - %6, но не менее 0 Итог [%7]';
								|en = '%1 = (%2 - %3 - %4, но не менее %2 х 0,5 Итог [%5]) - %6, но не менее 0 Итог [%7]'");
	Нарушение      = НСтр("ru = 'Арифметическая ошибка';
							|en = 'Арифметическая ошибка'");
	ОписаниеОшибки = НСтр("ru=''");
	...
	ВставитьЗначениеПоказателя(Раздел3, Р3Стр050);
	ВставитьЗначениеПоказателя(Раздел3, Р3Стр010);
	...
	Если НЕ Р3Стр005.Значение = "1" Тогда
		Возврат;
	КонецЕсли;
	...
	КС1 = ПоказателиКС(Р3Стр010.Значение * 0.5, Р3Стр010.Значение - Р3Стр020.Значение - Р3Стр030.Значение);
	КС2 = ПоказателиКС(Р3Стр050.Значение, КС1.ПраваяЧасть - Р3Стр040.Значение);
	
	Если НЕ СтруктураРеквизитовФормы.ИП Тогда
		УсловиеВыполнено = КС2.ЛеваяЧасть = Макс(КС1.ЛеваяЧасть, КС1.ПраваяЧасть);
	Иначе
		УсловиеВыполнено = КС2.ЛеваяЧасть = Макс(0, Макс(КС1.ЛеваяЧасть, КС1.ПраваяЧасть) - Р3Стр040.Значение);
	КонецЕсли;
	...
	Расшифровка1	= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонРасшифровки1, КС2.ЛеваяЧасть, 
		Р3Стр010.Значение, Р3Стр020.Значение, Р3Стр030.Значение, Макс(КС1.ЛеваяЧасть, КС1.ПраваяЧасть));
	Расшифровка2	= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонРасшифровки2, КС2.ЛеваяЧасть, 
		Р3Стр010.Значение, Р3Стр020.Значение, Р3Стр030.Значение, Макс(КС1.ЛеваяЧасть, КС1.ПраваяЧасть), Р3Стр040.Значение, 
		Макс(0, Макс(КС1.ЛеваяЧасть, КС1.ПраваяЧасть) - Р3Стр040.Значение));
	Расшифровка		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонРасшифровки, 
		?(СтруктураРеквизитовФормы.ИП, Расшифровка2, Расшифровка1));
...
Процедура ПроверитьКС_1_10(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка раздела 3 (КС 1.10)';
						|en = 'Проверка раздела 3 (КС 1.10)'");
	ОписаниеКС   = НСтр("ru='Сумма единого налога на вмененный доход, подлежащая к уплате (строка 050 Раздела 3) " +
		"должна быть равна разнице значений по строкам 010 Раздела 3 и по строкам 030, 040 Раздела 3, если результат такого " +
		"вычитания больше или равен нулю и строка 005 Раздела 3 = ""2""'");
	
	ШаблонРасшифровки = НСтр("ru = '%1 = %2 - %3 - %4, но не менее 0 Итог [%5], когда строка 005 Раздела 3 = ""2""';
							|en = '%1 = %2 - %3 - %4, но не менее 0 Итог [%5], когда строка 005 Раздела 3 = ""2""'");
	Нарушение      = НСтр("ru = 'Арифметическая ошибка';
							|en = 'Арифметическая ошибка'");
	ОписаниеОшибки = НСтр("ru=''");
	...
	Если НЕ Р3Стр005.Значение = "2" Тогда
		Возврат;
	КонецЕсли;
	...
	КС = ПоказателиКС(Р3Стр050.Значение, Р3Стр010.Значение - Р3Стр030.Значение - Р3Стр040.Значение);
	УсловиеВыполнено = КС.ЛеваяЧасть = Макс(0, КС.ПраваяЧасть);
	...
	Расшифровка	= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонРасшифровки, КС.ЛеваяЧасть, 
		Р3Стр010.Значение, Р3Стр030.Значение, Р3Стр040.Значение, Макс(0, КС.ПраваяЧасть));
...
&НаСервере
Процедура ПроверитьКС_1_11а_доп(СтруктураРезультата)
	
	//Отключено как неуниверсальное
	Возврат;
	...
	ТекстФормулы = НСтр("ru = 'Проверка раздела 3 (КС 1.11а.доп)';
						|en = 'Проверка раздела 3 (КС 1.11а.доп)'");
	ОписаниеКС   = НСтр("ru='Сумма расходов ИП на приобретение ККТ, уменьшающая общую сумму ЕНВД (строка 040 Раздела 3) " +
		"должна быть равна разнице значений строки 010 Раздела 3 и строк 020, 030 Раздела 3 (но не менее 50% от суммы " +
		"строки 10), и строки 050 Раздела 3, когда строка 005 Раздела 3 = ""1""'");
	
	ШаблонРасшифровки	= НСтр("ru = '%1 = (%2 - %3 - %4, но не менее %2 х 0,5 Итог [%5]) - %6 Итог [%7]';
								|en = '%1 = (%2 - %3 - %4, но не менее %2 х 0,5 Итог [%5]) - %6 Итог [%7]'");
	Нарушение      = НСтр("ru = 'Арифметическая ошибка';
							|en = 'Арифметическая ошибка'");
	ОписаниеОшибки = НСтр("ru=''");
	...
	Если НЕ (Р3Стр005.Значение = "1" И СтруктураРеквизитовФормы.ИП) Тогда
		Возврат;
	КонецЕсли;
	...
	Расчеты = ПоказателиКС(Р3Стр010.Значение * 0.5, Р3Стр010.Значение - Р3Стр020.Значение - Р3Стр030.Значение);
	КС = ПоказателиКС(Р3Стр040.Значение, Макс(Расчеты.ЛеваяЧасть, Расчеты.ПраваяЧасть) - Р3Стр050.Значение);
	
	УсловиеВыполнено = КС.ЛеваяЧасть = КС.ПраваяЧасть;
	...
	Расшифровка	= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонРасшифровки, КС.ЛеваяЧасть, 
		Р3Стр010.Значение, Р3Стр020.Значение, Р3Стр030.Значение, Макс(Расчеты.ЛеваяЧасть, Расчеты.ПраваяЧасть), 
		Р3Стр050.Значение, КС.ПраваяЧасть);
...
&НаСервере
Процедура ПроверитьКС_1_11б_доп(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru = 'Проверка раздела 3 (КС 1.11б.доп)';
						|en = 'Проверка раздела 3 (КС 1.11б.доп)'");
	ОписаниеКС   = НСтр("ru='Сумма расходов ИП на приобретение ККТ, уменьшающая общую сумму ЕНВД (строка 040 Раздела 3) " +
		"должна быть равна разнице значений строки 010 Раздела 3 и строк 030, 050 Раздела 3, когда строка 005 Раздела 3 = ""2""'");
	
	ШаблонРасшифровки	= НСтр("ru = '%1 = %2 - %3 - %4 Итог [%5]';
								|en = '%1 = %2 - %3 - %4 Итог [%5]'");
	Нарушение      = НСтр("ru = 'Арифметическая ошибка';
							|en = 'Арифметическая ошибка'");
	ОписаниеОшибки = НСтр("ru=''");
	...
	Если НЕ (Р3Стр005.Значение = "2" И СтруктураРеквизитовФормы.ИП) Тогда
		Возврат;
	КонецЕсли;
	...
	КС = ПоказателиКС(Р3Стр040.Значение, Р3Стр010.Значение - Р3Стр030.Значение - Р3Стр050.Значение);
	...
	Расшифровка	= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонРасшифровки, КС.ЛеваяЧасть, 
		Р3Стр010.Значение, Р3Стр030.Значение, Р3Стр050.Значение, КС.ПраваяЧасть);
...
&НаСервере
Процедура ПроверитьКС_1_12_доп(СтруктураРезультата)
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка Раздела 3 (КС 1.12а.доп)';
													|en = 'Проверка Раздела 3 (КС 1.12а.доп)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Статья 346.32 НК РФ';
													|en = 'Статья 346.32 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма расходов по приобретению ККТ (строка 040 Раздела 3) = сумме строк 050 Раздела 4 для налогоплател';
			|en = 'Сумма расходов по приобретению ККТ (строка 040 Раздела 3) = сумме строк 050 Раздела 4 для налогоплател'") +
		НСтр("ru = 'ьщиков - индивидуальных предпринимателей';
			|en = 'ьщиков - индивидуальных предпринимателей'"));
	Заголовки.Вставить("ОписаниеОшибки"	, НСтр("ru = 'Неверно рассчитана сумма расходов по приобретению ККТ';
													|en = 'Неверно рассчитана сумма расходов по приобретению ККТ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 Итог [%3]';
													|en = '%1 = %2 Итог [%3]'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("НомерКС"			, 121);
	
	ПроверитьКС_1_12_код(Настройки);
	СтруктураРезультата = ОбработатьРезультат(Настройки, СтруктураРезультата);
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка Раздела 3 (КС 1.12б.доп)';
													|en = 'Проверка Раздела 3 (КС 1.12б.доп)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Статья 346.32 НК РФ';
													|en = 'Статья 346.32 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма расходов по приобретению ККТ (строка 040 Раздела 3) = 0 для налогоплательщиков - юридических лиц';
			|en = 'Сумма расходов по приобретению ККТ (строка 040 Раздела 3) = 0 для налогоплательщиков - юридических лиц'"));
	Заголовки.Вставить("ОписаниеОшибки"	, НСтр("ru = 'Неправомерно отражена сумма расходов по приобретению ККТ';
													|en = 'Неправомерно отражена сумма расходов по приобретению ККТ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = 0';
													|en = '%1 = 0'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("НомерКС"			, 122);
...
&НаСервере
Процедура ПроверитьКС_1_12_код(Настройки)
	
	Если Настройки.НомерКС = 121 И НЕ СтруктураРеквизитовФормы.ИП 
		ИЛИ Настройки.НомерКС = 122 И СтруктураРеквизитовФормы.ИП Тогда
		Возврат;
	КонецЕсли;
	...
	ПоказателиР3 = ЗаполнитьПоказатели("Р3Стр040", Раздел3);
	РасшифроватьПоказатели(ПоказателиР3, Настройки, "Раздел3", "1");
	
	СуммаР4 = 0;
	СуммаР4Стр = "";
	Если Настройки.НомерКС = 121 Тогда
		КоличествоСтрок = КоличествоСтрок(Раздел4, ПоказательПоСтроке("Р4Стр050").Имя);
		Для Счетчик = 1 По КоличествоСтрок Цикл
			ПоказателиР4 = ЗаполнитьПоказатели("Р4Стр050", Раздел4, Счетчик);
			РасшифроватьПоказатели(ПоказателиР4, Настройки, "Раздел4", "1");
			СуммаР4 = СуммаР4 + ПоказателиР4.Р4Стр050.Значение;
			СуммаР4Стр = СуммаР4Стр + ПоказателиР4.Р4Стр050.ЗначениеСтр + " + ";
		КонецЦикла;
	КонецЕсли;
	
	Настройки.ЭтоОшибка		= НЕ ПоказателиР3.Р3Стр040.Значение = ?(Настройки.НомерКС = 121, СуммаР4, 0);
	Настройки.Расшифровка	= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
		ПоказателиР3.Р3Стр040.ЗначениеСтр, РасшифровкаВСтр(СуммаР4Стр), СуммаР4);
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	Если УсловныйКодПоказателя = "КодОКТМО" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Код по ОКТМО (строка 010 раздела 1)';
												|en = 'Код по ОКТМО (строка 010 раздела 1)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1Стр020" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 020 раздела 1)';
												|en = 'Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 020 раздела 1)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр010" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Код вида предпринимательской деятельности';
												|en = 'Код вида предпринимательской деятельности'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр040" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Базовая доходность на единицу физического показателя в месяц';
												|en = 'Базовая доходность на единицу физического показателя в месяц'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр050" Тогда
		Результат.Вставить("Имя", "П000020005003");
		Результат.Вставить("Представление", НСтр("ru = 'Корректирующий коэффициент К1';
												|en = 'Корректирующий коэффициент К1'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр060" Тогда
		Результат.Вставить("Имя", "П000020006003");
		Результат.Вставить("Представление", НСтр("ru = 'Корректирующий коэффициент К2';
												|en = 'Корректирующий коэффициент К2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070Гр02" Тогда
		Результат.Вставить("Имя", "П000020007002");
		Результат.Вставить("Представление", НСтр("ru = 'Величина физического показателя';
												|en = 'Величина физического показателя'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070Гр03" Тогда
		Результат.Вставить("Имя", "П000020007003");
		Результат.Вставить("Представление", НСтр("ru = 'Количество календарных дней';
												|en = 'Количество календарных дней'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Налоговая база в 1 месяце квартала';
												|en = 'Налоговая база в 1 месяце квартала'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр080Гр02" Тогда
		Результат.Вставить("Имя", "П000020008002");
		Результат.Вставить("Представление", НСтр("ru = 'Величина физического показателя';
												|en = 'Величина физического показателя'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр080Гр03" Тогда
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр080" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Налоговая база во 2 месяце квартала';
												|en = 'Налоговая база во 2 месяце квартала'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр090Гр02" Тогда
		Результат.Вставить("Имя", "П000020009002");
		Результат.Вставить("Представление", НСтр("ru = 'Величина физического показателя';
												|en = 'Величина физического показателя'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр090Гр03" Тогда
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр090" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Налоговая база в 3 месяце квартала';
												|en = 'Налоговая база в 3 месяце квартала'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр100" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Налоговая база всего';
												|en = 'Налоговая база всего'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр105" Тогда
		Результат.Вставить("Имя", "П000020010503");
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр110" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма исчисленного единого налога на вмененный доход за налоговый период';
												|en = 'Сумма исчисленного единого налога на вмененный доход за налоговый период'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр010" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма исчисленного за налоговый период единого налога на вмененный доход';
												|en = 'Сумма исчисленного за налоговый период единого налога на вмененный доход'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр020" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма страховых взносов, на которые может быть уменьшена сумма исчисленного за налоговый период налога';
												|en = 'Сумма страховых взносов, на которые может быть уменьшена сумма исчисленного за налоговый период налога'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр030" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма стаховых взносов, уплаченных индивидуальным предпринимателем в Пенсионный фонд Российской Федерации и в Федеральный фонд обязательного медицинского страхования в фиксированном размере';
												|en = 'Сумма стаховых взносов, уплаченных индивидуальным предпринимателем в Пенсионный фонд Российской Федерации и в Федеральный фонд обязательного медицинского страхования в фиксированном размере'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр040" Тогда
		Результат.Вставить("Имя", "П000030004003");
		Результат.Вставить("Представление", НСтр("ru = 'Общая сумма расходов по приобретению контрольно-кассовой техники, уменьшающая общую сумму единого налога на вмененный доход, подлежащую уплате в бюджет за налоговый период';
												|en = 'Общая сумма расходов по приобретению контрольно-кассовой техники, уменьшающая общую сумму единого налога на вмененный доход, подлежащую уплате в бюджет за налоговый период'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр050" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Общая сумма единого налога на вмененный доход, подлежащая уплате в бюджет за налоговый период';
												|en = 'Общая сумма единого налога на вмененный доход, подлежащая уплате в бюджет за налоговый период'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	
	ИначеЕсли УсловныйКодПоказателя = "Р4Стр050" Тогда
		Результат.Вставить("Имя", "П000040005003_%1");
		Результат.Вставить("Представление", НСтр("ru = 'Сумма расходов по приобретению экземпляра контрольно-кассовой техники';
												|en = 'Сумма расходов по приобретению экземпляра контрольно-кассовой техники'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
...
Процедура РасшифроватьПоказатели(Знач СтруктураПоказателей, 
	...
	Для каждого ЭлементКоллекции Из СтруктураПоказателей Цикл
		...
		ВнестиПоказательВРасшифровку(ЭлементКоллекции.Значение, НазваниеСтраницы, РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомерСтраницы));
...
Процедура ВставитьЗначениеМногострочногоПоказателя(ИсточникДанных, Показатель, НомерСтроки)
	...
	НомерСтрокиСтр = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомерСтроки);
...
Процедура ДобавитьЭлементВСтруктуруОшибок(СтруктураОшибок, ТекстФормулы, ОписаниеКС, ОбщееЧислоОшибок, ОбщееЧислоБезОшибок, Расшифровка, ЭтоОшибка, Описание = Неопределено, Нарушение = Неопределено)
	...
	СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																																								|en = '%1'"), Нарушение));
	СтруктураОшибок.Вставить("Расшифровка" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
																																									|en = '%1'"), Расшифровка));
	СтруктураОшибок.Вставить("ЭтоОшибка" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), ЭтоОшибка);
	СтруктураОшибок.Вставить("ТекстовоеПредставлениеФормулы" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1';
...
Функция ПредставлениеПоказателя(Показатель, ПолноеПредставление = Истина)
	...
	Если ЗначениеЗаполнено(Показатель.Имя) Тогда
		Если Сред(Показатель.Имя, 6, 1) = "2" И Сред(Показатель.Имя, 9, 2) >= "07" И Сред(Показатель.Имя, 9, 2) <= "09" Тогда
		...
		МассивПредставлений.Добавить("Раздел " + Сред(Показатель.Имя, 6, 1));
	КонецЕсли;
...
Функция РасшифровкаВСтр(СтрокаДляРасшифровки)
	...
	ИначеЕсли ТипЗнч(СтрокаДляРасшифровки) = Тип("Число") Тогда
		Возврат РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, СтрокаДляРасшифровки));
	Иначе
		Возврат СтрокаДляРасшифровки;
	КонецЕсли;
КонецФункции
...
Функция ПолучитьВозможныеЗначения(НазваниеСписка, НазваниеОбласти)
	...
	Макет = ОбъектОтчета(ИмяФормы).ПолучитьМакет(НазваниеСписка);
