...
Функция РазмерОграниченияУменьшенияСуммыСтраховогоВзноса(Форма, ПредварительнаяСуммаЕНВД)
	...
	Если Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2014-01-01' И Форма.СтруктураРеквизитовФормы.ОпределятьТрудНаемныхРабочих И НЕ Форма.СтруктураРеквизитовФормы.ИПИспользуетТрудНаемныхРаботников Тогда
		Результат = ПредварительнаяСуммаЕНВД;
	ИначеЕсли Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < '2014-01-01' И Форма.СтруктураРеквизитовФормы.ОпределятьТрудНаемныхРабочих И НЕ Форма.СтруктураРеквизитовФормы.ИПИспользуетТрудНаемныхРаботников Тогда
		Результат = 35664.66;
	Иначе
...
Процедура ВвестиКодУслуги(Элемент, Область, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	ПараметрыДляЗаполненияТаблицыЗначений.Вставить("ИмяМакета", "СпискиВыбора2013Кв4");
	...
	ПараметрыДляЗаполненияТаблицыЗначений.Вставить("ИмяОтчета", ИмяОтчета(ЭтаФорма.ИмяФормы));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	ОКТМОРаздела1.Колонки.Добавить("ОКТМО");
	ОКТМОРаздела1.Колонки.Добавить("НомерЛиста");
	ОКТМОРаздела1.Колонки.Добавить("НомерОКТМО");
	
	ОКТМОРаздела2 = Новый ТаблицаЗначений;
	ОКТМОРаздела2.Колонки.Добавить("ОКТМО");
	ОКТМОРаздела2.Колонки.Добавить("НомерЛиста");
	...
	Для Каждого Лист Из Раздел1 Цикл
		...
		КонецЕсли;
		Для Инд = 1 По 8 Цикл
			ИмяПоляОКТМО = "П000010002003_" + Инд;
			ТекОКТМО = СокрЛП(ДанныеЛиста[0].Значение[ИмяПоляОКТМО]);
			ТекСумма = ДанныеЛиста[0].Значение["П000010003003_" + Инд];
			ДлинаОКТМО = СтрДлина(ТекОКТМО);
			Если (ДлинаОКТМО <> 0 И ДлинаОКТМО <> 11 И ДлинаОКТМО <> 8) ИЛИ (ДлинаОКТМО = 0 И ЗначениеСуммыЗаполнено(ТекСумма)) Тогда	
			...
			Если ЗначениеЗаполнено(ТекОКТМО) Тогда
				...
				Если ОКТМОРаздела1.НайтиСтроки(Новый Структура("ОКТМО", ТекОКТМО)).Количество() <> 0 Тогда	
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 1 на странице " + ИндексЛиста + " задан неуникальный код по ОКТМО!", "Раздел1", ИмяПоляОКТМО, ИндексЛиста);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				НовСтр = ОКТМОРаздела1.Добавить();
				НовСтр.ОКТМО = ТекОКТМО;
				НовСтр.НомерЛиста = Раздел1.Индекс(Лист) + 1;
				НовСтр.НомерОКТМО = Инд;
			КонецЕсли;
	...
	Если ОКТМОРаздела1.Количество() = 0 Тогда	
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не задано ни одно из значений кода по ОКТМО в разделе 1!", "Раздел1", "П000010002003_1", 1);
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Для Каждого Лист2 Из Раздел2 Цикл
		...
		Если	ЗначениеЗаполнено(ДанныеЛиста2[0].Значение.П000020001000)			ИЛИ ЗначениеЗаполнено(ДанныеЛиста2[0].Значение.П000020002001)
			...
			ОКТМО = СокрЛП(ДанныеЛиста2[0].Значение.П000020003000);
	...
	Если ОКТМОРаздела2.Количество() = 0 Тогда	
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не задано ни одно из значений кода по ОКТМО в разделе 2!", "Раздел2", "П000020003000", 1);
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// проверка соответствия экземпляров раздела 1 экземплярам раздела 2
	Для Каждого ЕдОКТМОРаздела1 Из ОКТМОРаздела1 Цикл	
		Если ОКТМОРаздела2.НайтиСтроки(Новый Структура("ОКТМО", ЕдОКТМОРаздела1.ОКТМО)).Количество() = 0 Тогда	
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Для значения кода по ОКТМО """ + ЕдОКТМОРаздела1.ОКТМО + """ на странице " + ЕдОКТМОРаздела1.НомерЛиста + " раздела 1 отсутствуют соответствующие экземпляры раздела 2!", "Раздел1", "П000010002003_" + ЕдОКТМОРаздела1.НомерОКТМО, ЕдОКТМОРаздела1.НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого ЕдОКТМОРаздела2 Из ОКТМОРаздела2 Цикл	
		Если ОКТМОРаздела1.НайтиСтроки(Новый Структура("ОКТМО", ЕдОКТМОРаздела2.ОКТМО)).Количество() = 0 Тогда	
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Для значения кода по ОКТМО """ + ЕдОКТМОРаздела2.ОКТМО + """ на странице " + ЕдОКТМОРаздела2.НомерЛиста + " раздела 2 отсутствуют соответствующие экземпляры раздела 1!", "Раздел2", "П000020003000", ЕдОКТМОРаздела2.НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	Для Каждого ЛистРаздела1 Из Раздел1 Цикл
		...
		Для БлокРаздела1 = 1 По 8 Цикл
			...
			ЗадатьЗначениеЭлемента(УзелРаздел1, "ОКТМО", ОКАТО);
...
Процедура ПроверкаКС_1_1(СтруктураРезультата)
	...
	Для каждого Страница Из СтруктураРеквизитовФормы.мДанныеОтчета.Раздел1 Цикл
		...
		Для Индекс = 1 По 8 Цикл
			...
			ОписаниеКС = "Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 030 Раздел 1) по данному ОКТМО должна быть равна произведению суммы налога, подлежащей уплате (строка 060 Раздела 3) и результата деления суммы строк 100 всех Разделов 2 по данному ОКТМО и налоговой базы, исчисленной по всем ОКТМО (строка 010 Раздела 3)";
...
Процедура ПроверкаКС_1_2(СтруктураРезультата)
	...
	ИмяРаздела = "Раздел2";
	НазваниеСписка = "СпискиВыбора2013Кв4";
		Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2014-01-01' Тогда
		НазваниеОбласти = "ВидыУслуг2014";
	ИначеЕсли СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2013-01-01' Тогда
		НазваниеОбласти = "ВидыУслуг2013";
	Иначе
...
Процедура ПроверкаКС_1_4(СтруктураРезультата)
	...
	Для каждого Страница Из СтруктураРеквизитовФормы.мДанныеОтчета.Раздел2 Цикл
		...
		ОписаниеКС = "Сумма налоговой базы по ЕНВД (строка 100 Раздела 2) должна быть равна произведению  базовой доходности с учетом корректирующих коэффициентов и суммы величин физических показателей за отчетный период (строка 040 * строка 080 * строка 090 * (строка 050 + строка 060 + строка 070)) по одноименному ОКТМО (строка 030)";
...
Процедура ПроверкаКС_1_5(СтруктураРезультата)
	...
	Для каждого Страница Из СтруктураРеквизитовФормы.мДанныеОтчета.Раздел2 Цикл
		...
		ОписаниеКС = "Сумма исчисленного единого налога на вмененный доход (строка 110 Раздела 2) должна быть равна произведению налоговой базы (строка 100 Раздела 2) и налоговой ставки 15% по одноименному ОКТМО.";
		СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
		РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, 
			, СчетчикФормулДляРасшифровок);
...
Процедура ПроверкаКС_1_6(СтруктураРезультата)
	...
	ОписаниеКС = "Налоговая база, исчисленная по всем ОКТМО (строка 010 Раздел 3) должна быть равна сумме строк 100 всех заполненных Разделов 2.";
	СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
	РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, 
		, СчетчикФормулДляРасшифровок);
