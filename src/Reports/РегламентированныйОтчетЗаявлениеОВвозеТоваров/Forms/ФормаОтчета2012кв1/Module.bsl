...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = "Сведения о зачетах";
	СтрокаУровня1.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = 1;
	СтрокаУровня1.ВыгрузитьСтраницу        = 2;
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Раздел1",    "ФормаОтчета2012кв1_Раздел1");
	СоответствиеРазделовМакетам.Вставить("Раздел2",    "ФормаОтчета2012кв1_Раздел2");
	СоответствиеРазделовМакетам.Вставить("Раздел3",    "ФормаОтчета2012кв1_Раздел3");
	СоответствиеРазделовМакетам.Вставить("Приложение", "ФормаОтчета2012кв1_Приложение");
	СоответствиеРазделовМакетам.Вставить("Сведения",   "ФормаОтчета2012кв1_Сведения");
	СоответствиеРазделовМакетам.Вставить("Зачет",      "ФормаОтчета2012кв1_Зачет");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы",             "ON_ZVLRPOK_2_430_14_05_04_01.xsd");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(Раздел1.ПродавецМестоНахожд) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 1 не задано местонахождение (место жительства) продавца!", "Раздел1", "АдресПрод1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Пока Раздел1.Свойство("П000010000101_" + Формат(НомСтроки, "ЧГ=")) Цикл
		...
		ЗначениеЕдИзмТовНБАкц = ЭтаФорма.П0000100001[СтрНомСтроки - 1].П000010000124;
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1" Тогда
		...
		ИначеЕсли СтрНайти(Область.Имя, "П000010000107") > 0 Тогда
			...
			ДополнительныеПараметры = Новый Структура("Область, СтандартнаяОбработка, Элемент, ПолеТабличногоДокументаРаздел1", Область, СтандартнаяОбработка, Элемент, ПолеТабличногоДокументаРаздел1);
