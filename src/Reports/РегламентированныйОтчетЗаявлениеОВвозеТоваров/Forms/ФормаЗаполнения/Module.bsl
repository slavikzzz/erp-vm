...
Процедура ОК(Команда)
	...
	Для Каждого Строка Из Документы Цикл
		...
		Если НЕ ЗначениеЗаполнено(Строка.Дата) ИЛИ НЕ ЗначениеЗаполнено(Строка.Номер + Строка.Серия) Тогда
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Требуется дозаполнить строку %1.';
																							|en = 'Требуется дозаполнить строку %1.'"), НомерСтроки);
			Сообщение.Сообщить();
