...
Процедура ПроверитьВозможностьВыгрузки_ПараметрыВыгрузки(СтатусПроверки)
	...
	ИначеЕсли СтрДлина(СокрЛП(КодНО))<> 4 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах неверно заполнен код налогового органа.';
																				|en = 'В сведениях об отправителе и подписантах неверно заполнен код налогового органа.'"), "Раздел1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ОтправительЯвляетсяЮЛ Тогда
		Если НЕ ЗначениеЗаполнено(НаимОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнено наименование организации отправителя.';
																					|en = 'В сведениях об отправителе и подписантах не заполнено наименование организации отправителя.'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ИННЮЛОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнен ИНН организации отправителя.';
																					|en = 'В сведениях об отправителе и подписантах не заполнен ИНН организации отправителя.'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(КППОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнен КПП организации отправителя.';
																					|en = 'В сведениях об отправителе и подписантах не заполнен КПП организации отправителя.'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
	Иначе
		Если НЕ ЗначениеЗаполнено(ФамилияФЛОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнена фамилия отправителя.';
																					|en = 'В сведениях об отправителе и подписантах не заполнена фамилия отправителя.'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ИмяФЛОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнено имя отправителя.';
																					|en = 'В сведениях об отправителе и подписантах не заполнено имя отправителя.'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ФамилияПодп) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнена фамилия подписанта.';
																				|en = 'В сведениях об отправителе и подписантах не заполнена фамилия подписанта.'"), "Раздел1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ИмяПодп) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнено имя подписанта.';
																				|en = 'В сведениях об отправителе и подписантах не заполнено имя подписанта.'"), "Раздел1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПрПодп = 3 Тогда
		Если НЕ ЗначениеЗаполнено(НаимДовПодп) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнено наименование документа, подтверждающего полномочия представителя.';
																					|en = 'В сведениях об отправителе и подписантах не заполнено наименование документа, подтверждающего полномочия представителя.'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(НомерДовПодп) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнен номер документа, подтверждающего полномочия представителя.';
																					|en = 'В сведениях об отправителе и подписантах не заполнен номер документа, подтверждающего полномочия представителя.'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ДатаДовПодп) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнена дата документа, подтверждающего полномочия представителя.';
																					|en = 'В сведениях об отправителе и подписантах не заполнена дата документа, подтверждающего полномочия представителя.'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
