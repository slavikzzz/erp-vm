...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "19/11/2012";
	...
	Если СписокСохранения.Свойство( "ДанныеМногострочныхРазделов", ДанныеМногострочныхРазделов) Тогда
		...
		Для Каждого ЭлементСтруктуры Из ДанныеМногострочныхРазделов Цикл
			...
			Если НЕ НайденноеЗначение = Неопределено Тогда
				...
				Если ЭлементСтруктуры.Ключ = СтруктураРеквизитовФормы.мИдГруппы1
				   И ЭлементСтруктуры.Значение.Колонки.Найти("П000010000125") = Неопределено Тогда
					
					ЭлементСтруктуры.Значение.Колонки.Добавить("П000010000125", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(5, 0, ДопустимыйЗнак.Неотрицательный)));
...
Процедура УдалитьСтраницуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Для Каждого КЗ Из мСтруктураМногострочныхРазделов Цикл 
		...
		Если Обл <> Неопределено Тогда 
			...
			Пока Истина Цикл 
				Инд = Инд + 1;
				Обл = ТабличныйДокумент.Области.Найти("УдалитьСтроку" + КЗ.Ключ + "01_" + Инд);
				Если Обл = Неопределено Тогда 
					Прервать;
				КонецЕсли;
				Обл.Гиперссылка = Истина;
...
Процедура Обновить(Команда)
	...
	ОбновитьНаСервере();
	КтоПредставляетПриИзменении("");
	КтоОтправляетПриИзменении("");
	Подписант1ПриИзменении("");
	Модифицированность = Истина;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого СтрокаУровня1 Из мДеревоВыбранныхСтраниц.ПолучитьЭлементы() Цикл
		...
		Если СтрокаУровня1.ВыводНаПечать = 0 Или "Отправители" = СтрокаУровня1.ИмяСтраницы Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Раздел1",    "ФормаОтчета2012кв4_Раздел1");
	СоответствиеРазделовМакетам.Вставить("Раздел2",    "ФормаОтчета2012кв4_Раздел2");
	СоответствиеРазделовМакетам.Вставить("Раздел3",    "ФормаОтчета2012кв4_Раздел3");
	СоответствиеРазделовМакетам.Вставить("Приложение", "ФормаОтчета2012кв4_Приложение");
	СоответствиеРазделовМакетам.Вставить("Сведения",   "ФормаОтчета2012кв4_Сведения");
	СоответствиеРазделовМакетам.Вставить("Зачет",      "ФормаОтчета2012кв4_Зачет");
...
Процедура ВывестиНаПечатьЛистРаздел1(СтруктураПараметров)
	...
	МакетПечатиРаздела = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2012Кв3_Раздел1");
	
	МакетОбразцаПечатиРаздела = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2012Кв3_Раздел1");
...
Функция СформироватьСтруктуруПараметров()
	...
	Если мСтруктураРеквизитовВыгрузки.ПрОтпр = 1 Тогда
		...
		Если НЕ ПустаяСтрока(мСтруктураРеквизитовВыгрузки.ФамилияПодп2 + мСтруктураРеквизитовВыгрузки.ИмяПодп2 + мСтруктураРеквизитовВыгрузки.ОтчествоПодп2) Тогда
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СтруктураXML506");
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	АдресПродавцаРаздел1 = ДополнительныеДанныеПоАдресу(ДополнительныеДанныеОдностраничныхРазделов, "АдресПродавцаРаздел1");
	КодКЛАДРПродР1 = КодКЛАДРПоАдресу(АдресПродавцаРаздел1);
	...
	АдресПокупателяРаздел1 = ДополнительныеДанныеПоАдресу(ДополнительныеДанныеОдностраничныхРазделов, "АдресПокупателяРаздел1");
	КодКЛАДРПокР1 = КодКЛАДРПоАдресу(АдресПокупателяРаздел1);
	...
	АдресКомиссионераРаздел1 = ДополнительныеДанныеПоАдресу(ДополнительныеДанныеОдностраничныхРазделов, "АдресКомиссионераРаздел1");
	КодКЛАДРОрг = КодКЛАДРПоАдресу(АдресКомиссионераРаздел1);
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел1, "П000010000101_" + Формат(НомерСтроки, "ЧГ=")) Цикл
		...
		Если ЗначениеЗаполнено(Раздел1["П000010000102_" + Формат(НомерСтроки, "ЧГ=")]) Тогда
			...
			УстановитьЗначениеЭлемента(УзелСвТовар, "ЕдИзмТов", СокрЛП(Раздел1[ИмяПоляЕдИзм]));
			
			КодВалюты = СокрЛП(Раздел1["П000010000107_" + Формат(НомерСтроки, "ЧГ=")]);
			БазаВалюты = 0;
			Если Раздел1.Свойство("П000010000125_" + Формат(НомерСтроки, "ЧГ=")) Тогда
				БазаВалюты = Раздел1["П000010000125_" + Формат(НомерСтроки, "ЧГ=")];
			КонецЕсли;
			...
			Если ЗаполнятьЕдИзмТовНБАкц Тогда
				ИмяСлужебногоПоля = "П000010000124_" + Формат(НомерСтроки, "ЧГ=");
				ИмяПоляПредставления = "П000010000123_" + Формат(НомерСтроки, "ЧГ=");
				...
				Иначе
					...
					ЗначениеЕдИзмТовНБАкц = Раздел1[ИмяСлужебногоПоля];
				КонецЕсли;
				УстановитьЗначениеЭлемента(УзелСвТовар, "ЕдИзмТовНБАкц", ЗначениеЕдИзмТовНБАкц);
			КонецЕсли;
			...
			Для Каждого СтрТТДокумент Из ТТДокументы Цикл
				...
				УстановитьЗначениеЭлемента(УзелСвТСД, "СерНомТСД", ТТДСерия + ?(ПустаяСтрока(ТТДСерия) ИЛИ ПустаяСтрока(ТТДНомер), "", ", ") + ТТДНомер);
	...
	Иначе
		ТипПрод_3 = "";
	КонецЕсли;
	УстановитьЗначениеЭлемента(УзелСвКонтракт3, "ТипПродР3", ТипПрод_3);
	...
	Иначе
		ТипПок_3 = "";
	КонецЕсли;
	УстановитьЗначениеЭлемента(УзелСвКонтракт3, "ТипПокР3", ТипПок_3);
	...
	АдресПродавцаРаздел3 = ДополнительныеДанныеПоАдресу(ДополнительныеДанныеОдностраничныхРазделов, "АдресПродавцаРаздел3");
	КодКЛАДРПродР3 = КодКЛАДРПоАдресу(АдресПродавцаРаздел3);
	...
	АдресПокупателяРаздел3 = ДополнительныеДанныеПоАдресу(ДополнительныеДанныеОдностраничныхРазделов, "АдресПокупателяРаздел3");
	КодКЛАДРПокР3 = КодКЛАДРПоАдресу(АдресПокупателяРаздел3);
	...
	Для Каждого ЛистСведений Из Сведения Цикл
		ДанныеЛиста = ЛистСведений.Данные[0].Значение;
	...
	Для Каждого ЛистЗачет Из Зачет Цикл
		...
		УзелСвПлДок = СкопироватьУзел(УзелРодитель, ШаблонУзелСвПлДок);
		ЗаполнитьДаннымиУзел(ДанныеЛиста,УзелСвПлДок);
		УстановитьЗначениеЭлемента(УзелСвПлДок, "ТипПлат", ДанныеЛиста.ТипПлатЗачет);
	...
	Для Каждого ЛистПриложения Из Приложение Цикл
		...
		УстановитьЗначениеЭлемента(УзелСвКонтрПр, "ТипПродП", ТипПрод_Пр);
		...
		Иначе
			ТипПок_Пр = "";
		КонецЕсли;
		УстановитьЗначениеЭлемента(УзелСвКонтрПр, "ТипПокП", ТипПок_Пр);
		...
		АдресПродавца = ДополнительныеДанныеПоАдресу(ДополнительныеДанныеЛиста, "АдресПродавца");
		КодКЛАДРПродП = КодКЛАДРПоАдресу(АдресПродавца);
		УстановитьЗначениеЭлемента(УзелСвКонтрПр, "КодКЛАДРПродП", КодКЛАДРПродП);
		
		АдресПокупателя = ДополнительныеДанныеПоАдресу(ДополнительныеДанныеЛиста, "АдресПокупателя");
		КодКЛАДРПокП = КодКЛАДРПоАдресу(АдресПокупателя);
		УстановитьЗначениеЭлемента(УзелСвКонтрПр, "КодКЛАДРПокП", КодКЛАДРПокП);
		
		УстановитьЗначениеЭлемента(УзелСвКонтрПр, "ИдНомПродП", РазделитьИННКПП(ДанныеЛиста.ПродавецИНН_КПП).ИНН);
		УстановитьЗначениеЭлемента(УзелСвКонтрПр, "КПППродП",   РазделитьИННКПП(ДанныеЛиста.ПродавецИНН_КПП).КПП);
		
		УстановитьЗначениеЭлемента(УзелСвКонтрПр, "ИдНомПокП",  РазделитьИННКПП(ДанныеЛиста.ПокупательИНН_КПП).ИНН);
		УстановитьЗначениеЭлемента(УзелСвКонтрПр, "КПППокП",    РазделитьИННКПП(ДанныеЛиста.ПокупательИНН_КПП).КПП);
...
Функция КодКЛАДРПоАдресу(Адрес)
	...
	Если АдресЗаполненПоКладр(Адрес) Тогда
		...
		Если Адрес.Страна = "643" Тогда
			...
			АдресВXML = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(Адрес, , СтруктураРеквизитовФормы.СправочникиВидыКонтактнойИнформацииАдресОрганизации);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(Раздел1.КодСтраныПродавец) Тогда
		...
		СтатусПроверки = Ложь;
	ИначеЕсли НЕ ЗначениеСодержитсяВСписке(Раздел1.КодСтраныПродавец, "112,398,643") Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 1 код страны организации-продавца должен быть 112, 398 или 643!", "Раздел1", "КодСтраныПродавец");
		СтатусПроверки = Ложь;
	ИначеЕсли ПустаяСтрока(Раздел3.КодСтраныПродавец) Тогда
		Если Раздел1.КодСтраныПокупатель = "643" Тогда
			Если НЕ ЗначениеСодержитсяВСписке(Раздел1.КодСтраныПродавец, "112,398") Тогда
				...
				СтатусПроверки = Ложь;
			КонецЕсли;
		ИначеЕсли Раздел1.КодСтраныПокупатель = "398" Тогда
			Если НЕ ЗначениеСодержитсяВСписке(Раздел1.КодСтраныПродавец, "112,643") Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 1 код страны организации-продавца должен быть 112 или 643!", "Раздел1", "КодСтраныПродавец");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;	
	 ...
	 ИЛИ ЗначениеЗаполнено(Раздел3.ДатаСпецификации) Тогда
		...
		Иначе
			Если НЕ ЗначениеСодержитсяВСписке(Раздел3.КодСтраныПродавец, "112,398,643") Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 3 код страны организации-продавца должен быть 398, 112 или 643!", "Раздел3", "КодСтраныПродавец");
			Иначе
				Если (Раздел1.КодСтраныПокупатель = "643") И НЕ ЗначениеСодержитсяВСписке(Раздел3.КодСтраныПродавец, "112,398") Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 3 код страны организации-продавца должен быть 112 или 398!", "Раздел3", "КодСтраныПродавец");
				ИначеЕсли (Раздел1.КодСтраныПокупатель = "398") И НЕ ЗначениеСодержитсяВСписке(Раздел3.КодСтраныПродавец, "112,643") Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 3 код страны организации-продавца должен быть 112 или 643!", "Раздел3", "КодСтраныПродавец");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(КодИФНС) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнен код налогового органа!';
																				|en = 'В сведениях об отправителе и подписантах не заполнен код налогового органа!'"), "Раздел1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если (ОтправительЯвляетсяЮЛ И НЕ ЗначениеЗаполнено(ИННЮЛОтпр)) ИЛИ (НЕ ОтправительЯвляетсяЮЛ И НЕ ЗначениеЗаполнено(ИННФЛОтпр)) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнен ИНН отправителя!';
																				|en = 'В сведениях об отправителе и подписантах не заполнен ИНН отправителя!'"), "Раздел1");
		СтатусПроверки = Ложь;
	КонецЕсли; 
	...
	Если ПрПодп1 = 3 Тогда
		Если НЕ ЗначениеЗаполнено(НаимДовПодп1) ИЛИ НЕ ЗначениеЗаполнено(НомерДовПодп1) ИЛИ НЕ ЗначениеЗаполнено(ДатаДовПодп1) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнены сведения о доверенности подписанта 1!';
																					|en = 'В сведениях об отправителе и подписантах не заполнены сведения о доверенности подписанта 1!'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ОтправленПочтой И ПрКурьер = 3 Тогда
		Если НЕ ЗначениеЗаполнено(НаимДовКурьер) ИЛИ НЕ ЗначениеЗаполнено(НомерДовКурьер) ИЛИ НЕ ЗначениеЗаполнено(ДатаДовКурьер) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru = 'В сведениях об отправителе и подписантах не заполнены сведения о доверенности курьера!';
																					|en = 'В сведениях об отправителе и подписантах не заполнены сведения о доверенности курьера!'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура СохранитьДанныеОбласти()
	...
	Если КтоПредставляет = 1 Тогда
		мСтруктураРеквизитовВыгрузки.ОтправленПочтой = Истина;
	ИначеЕсли КтоПредставляет = 2 Тогда
		мСтруктураРеквизитовВыгрузки.ОтправленПочтой = Ложь;
		Если СтруктураРеквизитовФормы.ЭтоЮрЛицо = Истина Тогда 
			мСтруктураРеквизитовВыгрузки.ПрКурьер = 2;
		Иначе
			мСтруктураРеквизитовВыгрузки.ПрКурьер = 1;
		КонецЕсли;
	ИначеЕсли КтоПредставляет = 3 Тогда
		мСтруктураРеквизитовВыгрузки.ОтправленПочтой = Ложь;
		мСтруктураРеквизитовВыгрузки.ПрКурьер = 3;
	КонецЕсли;
	мСтруктураРеквизитовВыгрузки.ФамилияКурьер = ФамилияПредставление;
	мСтруктураРеквизитовВыгрузки.ИмяКурьер = ИмяПредставление;
	мСтруктураРеквизитовВыгрузки.ОтчествоКурьер = ОтчествоПредставление;
	мСтруктураРеквизитовВыгрузки.ИННФЛКурьер = ИННПредставляет;
	мСтруктураРеквизитовВыгрузки.НаимДовКурьер = ДоверенностьПредставляет;
	мСтруктураРеквизитовВыгрузки.НомерДовКурьер = НомерДоверенностьПредставляет;
	мСтруктураРеквизитовВыгрузки.ДатаДовКурьер = ДатаДоверенностьПредставляет;
	...
	//Подписант1
	мСтруктураРеквизитовВыгрузки.ПрПодп1 = Подписант1;
	мСтруктураРеквизитовВыгрузки.ФамилияПодп1 = ФамилияПодписант1;
	мСтруктураРеквизитовВыгрузки.ИмяПодп1 = ИмяПодписант1;
	мСтруктураРеквизитовВыгрузки.ОтчествоПодп1 = ОтчествоПодписант1;
	мСтруктураРеквизитовВыгрузки.ИННФЛПодп1 = ИННПодписант1;
	мСтруктураРеквизитовВыгрузки.ДолжностьПодп1 = ДолжностьПодписант1;
	мСтруктураРеквизитовВыгрузки.НаимДовПодп1 = ДоверенностьПодписант1;
	мСтруктураРеквизитовВыгрузки.НомерДовПодп1 = НомерДоверенностиПодписант1;
	мСтруктураРеквизитовВыгрузки.ДатаДовПодп1 = ДатаДоверенностиПодписант1;
	
	//Подписант2
	мСтруктураРеквизитовВыгрузки.ФамилияПодп2 = ФамилияПодписант2;
	мСтруктураРеквизитовВыгрузки.ИмяПодп2 = ИмяПодписант2;
	мСтруктураРеквизитовВыгрузки.ОтчествоПодп2 = ОтчествоПодписант2;
	мСтруктураРеквизитовВыгрузки.ИННФЛПодп2 = ИННПодписант2;
	мСтруктураРеквизитовВыгрузки.ДолжностьПодп2 = ДолжностьПодписант2;
	мСтруктураРеквизитовВыгрузки.НаимДовПодп2 = ДоверенностьПодписант2;
	мСтруктураРеквизитовВыгрузки.НомерДовПодп2 = НомерДоверенностиПодписант2;
	мСтруктураРеквизитовВыгрузки.ДатаДовПодп2 = ДатаДоверенностиПодписант2;
КонецПроцедуры
...
Процедура ВосстановитьСохраненныеДанныеОбласти()
	...
	КодИФНС = мСтруктураРеквизитовВыгрузки.КодИФНС;
	Если мСтруктураРеквизитовВыгрузки.ОтправленПочтой Тогда
		КтоПредставляет = 1;
	Иначе
		Если мСтруктураРеквизитовВыгрузки.ПрКурьер = 3 Тогда 
			КтоПредставляет = 3;
		...
		КонецЕсли;
	КонецЕсли;
	ФамилияПредставление = мСтруктураРеквизитовВыгрузки.ФамилияКурьер;
	ИмяПредставление = мСтруктураРеквизитовВыгрузки.ИмяКурьер;
	ОтчествоПредставление = мСтруктураРеквизитовВыгрузки.ОтчествоКурьер;
	ИННПредставляет = мСтруктураРеквизитовВыгрузки.ИННФЛКурьер;
	ДоверенностьПредставляет = мСтруктураРеквизитовВыгрузки.НаимДовКурьер;
	НомерДоверенностьПредставляет = мСтруктураРеквизитовВыгрузки.НомерДовКурьер;
	ДатаДоверенностьПредставляет = мСтруктураРеквизитовВыгрузки.ДатаДовКурьер;
	...
	//Подписант1
	Подписант1 = мСтруктураРеквизитовВыгрузки.ПрПодп1;
	ФамилияПодписант1 = мСтруктураРеквизитовВыгрузки.ФамилияПодп1;
	ИмяПодписант1 = мСтруктураРеквизитовВыгрузки.ИмяПодп1;
	ОтчествоПодписант1 = мСтруктураРеквизитовВыгрузки.ОтчествоПодп1;
	ИННПодписант1 = мСтруктураРеквизитовВыгрузки.ИННФЛПодп1;
	ДолжностьПодписант1 = мСтруктураРеквизитовВыгрузки.ДолжностьПодп1;
	ДоверенностьПодписант1 = мСтруктураРеквизитовВыгрузки.НаимДовПодп1;
	НомерДоверенностиПодписант1 = мСтруктураРеквизитовВыгрузки.НомерДовПодп1;
	ДатаДоверенностиПодписант1 = мСтруктураРеквизитовВыгрузки.ДатаДовПодп1;
	
	//Подписант2
	ФамилияПодписант2 = мСтруктураРеквизитовВыгрузки.ФамилияПодп2;
	ИмяПодписант2 = мСтруктураРеквизитовВыгрузки.ИмяПодп2;
	ОтчествоПодписант2 = мСтруктураРеквизитовВыгрузки.ОтчествоПодп2;
	ИННПодписант2 = мСтруктураРеквизитовВыгрузки.ИННФЛПодп2;
	ДолжностьПодписант2 = мСтруктураРеквизитовВыгрузки.ДолжностьПодп2;
	ДоверенностьПодписант2 = мСтруктураРеквизитовВыгрузки.НаимДовПодп2;
	НомерДоверенностиПодписант2 = мСтруктураРеквизитовВыгрузки.НомерДовПодп2;
	ДатаДоверенностиПодписант2 = мСтруктураРеквизитовВыгрузки.ДатаДовПодп2;
КонецПроцедуры
...
Процедура ИнициализацияОбласти()
	...
	Если СтруктураРеквизитовФормы.ЭтоЮрЛицо = Истина Тогда 
		СВ = Элементы.Подписант1.СписокВыбора;
		СВ.Удалить(0);
		Если (Подписант1 = 0) Тогда 
			Подписант1 = 2;
		КонецЕсли;
		Элементы.КтоПредставляет.СписокВыбора[1].Представление = "Законный представитель организации";
	Иначе
		СВ = Элементы.Подписант1.СписокВыбора;
		СВ.Удалить(1);
		Если (Подписант1 = 0) Тогда 
	...
	Если (КтоПредставляет = 0) Тогда 
		КтоПредставляет = 1;
	КонецЕсли;
	
	Элементы.ГруппаДоверенностьПодписант1.Доступность = (Подписант1 = 3);
	
	Если КтоПредставляет = 1 Тогда 
		Элементы.ГруппаФИОПредставляет.Доступность = Ложь;
		Элементы.ИННПредставляет.Доступность = Ложь;
		Элементы.Декорация3.Доступность = Ложь;
		Элементы.ГруппаДоверенностьПредставляет.Доступность = Ложь;
	Иначе
		Элементы.ГруппаФИОПредставляет.Доступность = Истина;
		Элементы.ИННПредставляет.Доступность = Истина;
		Элементы.Декорация3.Доступность = Истина;
		Элементы.ГруппаДоверенностьПредставляет.Доступность = (КтоПредставляет = 3);
	КонецЕсли;
...
&НаКлиенте
Процедура Подписант1ПриИзменении(Элемент)
	Модифицированность = Истина;
	Элементы.ГруппаДоверенностьПодписант1.Доступность = (Подписант1 = 3);
КонецПроцедуры
...
&НаКлиенте
Процедура КтоПредставляетПриИзменении(Элемент)
	Модифицированность = Истина;
	Если КтоПредставляет = 1 Тогда 
		Элементы.ГруппаФИОПредставляет.Доступность = Ложь;
		Элементы.ИННПредставляет.Доступность = Ложь;
...
&НаКлиенте
Процедура ФамилияПредставлениеПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры
...
&НаКлиенте
Процедура ОтчествоПредставлениеПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ИННПредставляетПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ДоверенностьПредставляетПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура НомерДоверенностьПредставляетПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ДатаДоверенностьПредставляетПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры
...
&НаКлиенте
Процедура ФамилияПодписант1ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ИмяПодписант1ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ОтчествоПодписант1ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ИННПодписант1ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ДолжностьПодписант1ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ДоверенностьПодписант1ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура НомерДоверенностиПодписант1ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ДатаДоверенностиПодписант1ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ФамилияПодписант2ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ИмяПодписант2ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ОтчествоПодписант2ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ИННПодписант2ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ДолжностьПодписант2ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ДоверенностьПодписант2ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура НомерДоверенностиПодписант2ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ДатаДоверенностиПодписант2ПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры
#КонецОбласти
