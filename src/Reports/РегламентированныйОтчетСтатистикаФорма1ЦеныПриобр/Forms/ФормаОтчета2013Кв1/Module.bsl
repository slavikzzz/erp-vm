...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "18/12/2013";
	...
	// Многострочные части Раздела 1.
	Для НомГруппы = 1 По 175 Цикл
		
		Если НомГруппы >= 30 И НомГруппы <= 32 Тогда
			Продолжить;
		КонецЕсли;
		
		мСтруктураМногострочныхРазделов.Вставить(
			"П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ="), "П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ="));
		
		Если НомГруппы >= 41 И НомГруппы <= 50 Тогда
			КолИсхСтрокИдГруппы = 4;
		ИначеЕсли НомГруппы = 115 Тогда
			КолИсхСтрокИдГруппы = 3;
		Иначе
			КолИсхСтрокИдГруппы = 2;
		КонецЕсли;
		
		мСтруктураИсхКолвоСтрокРазделов.Вставить("П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ="), КолИсхСтрокИдГруппы);
		
		мСтруктураКолвоКолонокРазделов.Вставить("П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ="), 6);
		
		СтруктураРеквизитовФормы.мСоответствияМнгЧ.Вставить(
			"П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ="), "МногострочнаяЧасть1_" + Формат(НомГруппы, "ЧГ="));
		
	КонецЦикла;
	
	// Многострочные части Раздела 3.
	Для НомГруппы = 1 По 27 Цикл
		
		мСтруктураМногострочныхРазделов.Вставить(
			"П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ="), "П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ="));
		
		Если НомГруппы = 14 Или НомГруппы = 17 Или НомГруппы = 18
			Или (НомГруппы >= 24 И НомГруппы <= 27) Тогда
			КолИсхСтрокИдГруппы = 4;
		Иначе
			КолИсхСтрокИдГруппы = 2;
		КонецЕсли;
		
		мСтруктураИсхКолвоСтрокРазделов.Вставить("П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ="), КолИсхСтрокИдГруппы);
		
		мСтруктураКолвоКолонокРазделов.Вставить("П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ="), 6);
		
		СтруктураРеквизитовФормы.мСоответствияМнгЧ.Вставить(
			"П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ="), "МногострочнаяЧасть3_" + Формат(НомГруппы, "ЧГ="));
	...
	Если СписокСохранения.Свойство("ДанныеМногострочныхРазделов", ДанныеМногострочныхРазделов)
		...
		Для Каждого ЭлементСтруктуры Из ДанныеМногострочныхРазделов Цикл
			...
			Если НЕ НайденноеЗначение = Неопределено Тогда
				...
				Если ЭлементСтруктуры.Значение.Колонки.Найти("П000001009673") = Неопределено Тогда 
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			...
			Если ((Лев(Область.Имя, 7) = "П000001" Или Лев(Область.Имя, 7) = "П000003")
				И Сред(Область.Имя, 12, 2) = "06")
				Или (Лев(Область.Имя, 7) = "П000002" И Сред(Область.Имя, 12, 2) = "05") Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей
		= ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("Списки2013Кв1");
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "Причина", 10, , "Выбор причины изменения цены",
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Область.Значение = ?(ЗначениеЗаполнено(ВыборКод), Число(СокрЛП(ВыборКод)), 0);
...
Функция УниверсальнаяВыгрузкаXMLПроверкаДанныхНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	Если ПараметрыВыгрузки = Неопределено Тогда 
		ПараметрыВыгрузки = СформироватьСтруктуруПараметров();
	КонецЕсли;	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2013Кв1");
	ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки);
	Если НЕ РегламентированнаяОтчетность.ПроверитьДанные(ЭтаФорма, ДеревоВыгрузки) Тогда
		КоллекцияСписковВыбора = "";
		Возврат Ложь;
...
Функция СформироватьСтруктуруПоказателей()
	...
	ДополнительныеПоказатели1ЦеныПриобр(мДанныеОтчета.ПолеТабличногоДокументаФормаОтчета);
...
&НаСервере
Процедура ДополнительныеПоказатели1ЦеныПриобр(ТекСтруктура)
	...
	Пока ТекСтруктура.Свойство("П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04", ТекЗначение) Цикл
		
		Если Не ЗначениеЗаполнено(ТекЗначение) Тогда
			НомГруппы = НомГруппы + 1;
			Продолжить;
		КонецЕсли;
		
		// Данные строки "Уголь" не выгружаем
		// (код ОКПД "10.10.10" - отсутствует в шаблоне выгрузки Росстата).
		Если НомГруппы = 2 Тогда
			НомГруппы = НомГруппы + 1;
			Продолжить;
		КонецЕсли;
		...
		ТекСтруктура.Вставить(
			"ПД00001000001_" + Формат(НомСтр, "ЧГ="), КодЕдиницыИзмерения1ЦеныПриобр(1, НомГруппы));
		
		ТекСтруктура.Вставить(
			"ПД00001000002_" + Формат(НомСтр, "ЧГ="), КодНаОсновеОКПД1ЦеныПриобр(1, НомГруппы));
		
		ТекСтруктура.Вставить(
			"ПД00001000003_" + Формат(НомСтр, "ЧГ="), ТекСтруктура["П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04"]);
		
		ТекСтруктура.Вставить(
			"ПД00001000004_" + Формат(НомСтр, "ЧГ="), ТекСтруктура["П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "05"]);
		
		ТекСтруктура.Вставить(
			"ПД00001000005_" + Формат(НомСтр, "ЧГ="), ТекСтруктура["П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "06"]);
	...
	Пока ТекСтруктура.Свойство("П000002" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "01", ТекЗначение) Цикл
		...
		ТекСтруктураКодов = КодНаОсновеОКПД1ЦеныПриобр(2, НомГруппы);
		
		ТекСтруктура.Вставить(
			"ПД00002000001_" + Формат(НомСтр, "ЧГ="), ТекСтруктураКодов["Код2"]);
		
		ТекСтруктура.Вставить(
			"ПД00002000002_" + Формат(НомСтр, "ЧГ="), ТекСтруктураКодов["Код1"]);
		
		ТекСтруктура.Вставить(
			"ПД00002000003_" + Формат(НомСтр, "ЧГ="), ТекСтруктура["П000002" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "01"]);
		
		ТекСтруктура.Вставить(
			"ПД00002000004_" + Формат(НомСтр, "ЧГ="), ТекСтруктура["П000002" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "02"]);
		
		ТекСтруктура.Вставить(
			"ПД00002000005_" + Формат(НомСтр, "ЧГ="), ТекСтруктура["П000002" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]);
		
		ТекСтруктура.Вставить(
			"ПД00002000006_" + Формат(НомСтр, "ЧГ="), ТекСтруктура["П000002" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04"]);
		
		ТекСтруктура.Вставить(
			"ПД00002000007_" + Формат(НомСтр, "ЧГ="), ТекСтруктура["П000002" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "05"]);
	...
	Пока ТекСтруктура.Свойство("П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04", ТекЗначение) Цикл
		...
		ТекСтруктура.Вставить(
			"ПД00003000001_" + Формат(НомСтр, "ЧГ="), КодЕдиницыИзмерения1ЦеныПриобр(3, НомГруппы));
		
		ТекСтруктура.Вставить(
			"ПД00003000002_" + Формат(НомСтр, "ЧГ="), КодНаОсновеОКПД1ЦеныПриобр(3, НомГруппы));
		
		ТекСтруктура.Вставить(
			"ПД00003000003_" + Формат(НомСтр, "ЧГ="), ТекСтруктура["П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04"]);
		
		ТекСтруктура.Вставить(
			"ПД00003000004_" + Формат(НомСтр, "ЧГ="), ТекСтруктура["П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "05"]);
		
		ТекСтруктура.Вставить(
			"ПД00003000005_" + Формат(НомСтр, "ЧГ="), ТекСтруктура["П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "06"]);
...
&НаСервере
Функция КодЕдиницыИзмерения1ЦеныПриобр(НомерРаздела, НомерГруппы)
	
	Если НомерРаздела = 1 Тогда
		
		Если НомерГруппы = 1 Тогда                              // м3
			Возврат 113;
		ИначеЕсли НомерГруппы >= 2 И НомерГруппы <= 4 Тогда     // т
			Возврат 168;
		ИначеЕсли НомерГруппы = 5 Тогда                         // тыс м3
			Возврат 114;
		ИначеЕсли НомерГруппы >= 6 И НомерГруппы <= 16 Тогда    // т
			Возврат 168;
		ИначеЕсли НомерГруппы >= 17 И НомерГруппы <= 20 Тогда   // м3
			Возврат 113;
		ИначеЕсли НомерГруппы = 21 Тогда                        // м2
			Возврат 55;
		ИначеЕсли НомерГруппы >= 22 И НомерГруппы <= 24 Тогда   // м3
			Возврат 113;
		ИначеЕсли НомерГруппы = 25 Тогда                        // м2
			Возврат 55;
		ИначеЕсли НомерГруппы = 26 Тогда                        // т
			Возврат 168;
		ИначеЕсли НомерГруппы = 27 Тогда                        // м2
			Возврат 55;
		ИначеЕсли НомерГруппы = 28 Тогда                        // тыс усл кирп
			Возврат 894;
		ИначеЕсли НомерГруппы >= 29 И НомерГруппы <= 40 Тогда   // т
			Возврат 168;
		ИначеЕсли НомерГруппы = 41 Тогда                        // кг
			Возврат 166;
		ИначеЕсли НомерГруппы = 42 Тогда                        // л
			Возврат 112;
		ИначеЕсли НомерГруппы = 43 Тогда                        // кг
			Возврат 166;
		ИначеЕсли НомерГруппы = 44 Тогда                        // л
			Возврат 112;
		ИначеЕсли НомерГруппы = 45 Тогда                        // кг
			Возврат 166;
		ИначеЕсли НомерГруппы = 46 Тогда                        // л
			Возврат 112;
		ИначеЕсли НомерГруппы = 47 Тогда                        // кг
			Возврат 166;
		ИначеЕсли НомерГруппы = 48 Тогда                        // л
			Возврат 112;
		ИначеЕсли НомерГруппы = 49 Тогда                        // кг
			Возврат 166;
		ИначеЕсли НомерГруппы = 50 Тогда                        // л
			Возврат 112;
		ИначеЕсли НомерГруппы >= 51 И НомерГруппы <= 64 Тогда   // тыс см3 (т.е. литр)
			Возврат 112;
		ИначеЕсли НомерГруппы >= 65 И НомерГруппы <= 175 Тогда  // шт
			Возврат 796;
		ИначеЕсли НомерГруппы >= 176 И НомерГруппы <= 179 Тогда // МВт.ч
			Возврат 246;
		ИначеЕсли НомерГруппы = 180 Тогда                       // ГКал
			Возврат 233;
		ИначеЕсли НомерГруппы = 181 Тогда                       // тыс м3
			Возврат 114;
		КонецЕсли;
		
	ИначеЕсли НомерРаздела = 3 Тогда
		
		Если НомерГруппы >= 1 И НомерГруппы <= 5 Тогда        // га
			Возврат 59;
		ИначеЕсли НомерГруппы = 6 Тогда                       // т
			Возврат 168;
		ИначеЕсли НомерГруппы = 7 Тогда                       // га
			Возврат 59;
		ИначеЕсли НомерГруппы >= 8 И НомерГруппы <= 24 Тогда  // гол
			Возврат 836;
		ИначеЕсли НомерГруппы = 25 Тогда                      // шт
			Возврат 796;
		ИначеЕсли НомерГруппы >= 26 И НомерГруппы <= 27 Тогда // гол
			Возврат 836;
		КонецЕсли;
...
&НаСервере
Функция КодНаОсновеОКПД1ЦеныПриобр(НомерРаздела, НомерГруппы)
	
	Если НомерРаздела = 1 Или НомерРаздела = 3 Тогда
		
		ОбластьЦена = ПолеТабличногоДокументаФормаОтчета.Области["П"
			+ Формат(НомерРаздела, "ЧЦ=6; ЧВН=; ЧГ=") + Формат(НомерГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04"];
		ОбластьКодТекст = ПолеТабличногоДокументаФормаОтчета.Область(
			ОбластьЦена.Верх, ОбластьЦена.Лево - 2, ОбластьЦена.Низ, ОбластьЦена.Право - 2).Текст;
			
		ОбластьКодТекст = СтрЗаменить(ОбластьКодТекст, "*", "");
		Пока СтрНайти(ОбластьКодТекст, " ") <> 0 Цикл
			ОбластьКодТекст = СтрЗаменить(ОбластьКодТекст, " ", "");
		КонецЦикла;
		
		Возврат ОбластьКодТекст;// возвращаем строку
	...
	ИначеЕсли НомерРаздела = 2 Тогда
		...
		ОбластьЦена = ПолеТабличногоДокументаФормаОтчета.Области["П"
			+ Формат(НомерРаздела, "ЧЦ=6; ЧВН=; ЧГ=") + Формат(НомерГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "01"];
		ОбластьКодТекст = ПолеТабличногоДокументаФормаОтчета.Область(
			ОбластьЦена.Верх, ОбластьЦена.Лево - 1, ОбластьЦена.Низ, ОбластьЦена.Право - 1).Текст;
		...
		СтруктураКодов.Вставить("Код1", ОбластьКодТекст);
		
		ОбластьЦена = ПолеТабличногоДокументаФормаОтчета.Области["П"
			+ Формат(НомерРаздела, "ЧЦ=6; ЧВН=; ЧГ=") + Формат(НомерГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "03"];
		ОбластьКодТекст = ПолеТабличногоДокументаФормаОтчета.Область(
			ОбластьЦена.Верх, ОбластьЦена.Лево - 1, ОбластьЦена.Низ, ОбластьЦена.Право - 1).Текст;
		...
		СтруктураКодов.Вставить("Код2", ОбластьКодТекст);
		
		Возврат СтруктураКодов;// возвращаем структуру
...
Функция РазделТекущейОбласти(Форма, ТекущаяОбласть)
	...
	ИначеЕсли СтрНайти(ИмяТекущейОбласти, "ДобавитьСтроку") > 0 Тогда
		ИмяТекущейОбласти = Форма.ПолеТабличногоДокументаФормаОтчета.Область(ТекущаяОбласть.Верх - 1, 2, ТекущаяОбласть.Верх - 1, 2).Имя;
	КонецЕсли;
