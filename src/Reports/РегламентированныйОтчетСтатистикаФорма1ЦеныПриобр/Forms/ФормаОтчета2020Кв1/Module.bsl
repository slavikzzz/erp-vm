...
Процедура ДополнитьРеквизитыФормы()
	...
	Для Раздел = 1 По 3 Цикл
		...
		Для ИндексМнгЧасти = 1 По КолМнгЧастей Цикл
			
			ДобавляемыеРеквизиты.Очистить();
			ИмяДобавляемогоРеквизита = "П0000" + Раздел + "010" + Формат(ИндексМнгЧасти, "ЧЦ=2; ЧВН=");
			...
			Для Гр = 1 По ГрКон Цикл
				Если Раздел = 1 Тогда
					Если Гр = 7 Или Гр = 10 Тогда
						ТипКолонки = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(17, 2));
					ИначеЕсли Гр = 8 Или Гр = 11 Тогда
						ТипКолонки = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 1));
					Иначе
						ТипКолонки = Новый ОписаниеТипов("Строка");
					КонецЕсли;
				ИначеЕсли Раздел = 2 Тогда
					Если Гр = 5 Или Гр = 8 Тогда
						ТипКолонки = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(17, 2));
					ИначеЕсли Гр = 6 Или Гр = 9 Тогда
						ТипКолонки = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 1));
					Иначе
						ТипКолонки = Новый ОписаниеТипов("Строка");
					КонецЕсли;
				ИначеЕсли Раздел = 3 Тогда
					Если Гр = 6 Или Гр = 9 Тогда
						ТипКолонки = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(17, 2));
					ИначеЕсли Гр = 7 Или Гр = 10 Тогда
						ТипКолонки = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 1));
					Иначе
						ТипКолонки = Новый ОписаниеТипов("Строка");
					КонецЕсли;
			...
			КонецЦикла;
			ИзменитьРеквизиты(ДобавляемыеРеквизиты);
			СтруктураРеквизитовФормы.Вставить("мИдГруппы" + ОбщИндексМнгЧасти, ИмяДобавляемогоРеквизита);
			...
			СтруктураРеквизитовФормы.мСоответствияМнгЧ.Вставить(ИмяДобавляемогоРеквизита, "МногострочнаяЧасть" + ОбщИндексМнгЧасти);
			ОбщИндексМнгЧасти = ОбщИндексМнгЧасти + 1;
		КонецЦикла;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если СтрНайти(Область.Имя, ":") > 0 Тогда
	...
	Иначе
		...
		Если СтруктураРеквизитовФормы.мАвтоВыборКодов Или Область.Защита = Истина Тогда
			Возврат;
		КонецЕсли;
		
		ТабличныйДокумент = ЭтотОбъект["ПолеТабличногоДокументаФормаОтчета"];
		
		Если Лев(Область.Имя, 1) = "П" Тогда
			Раздел = Число(Сред(Область.Имя, 6, 1));
			Графа  = Число(Сред(Область.Имя, 12, 2));
			НомСтр = Сред(Область.Имя, 10, 2);
		Иначе
			Возврат;
		КонецЕсли;
		
		Если Графа = 1 Или Графа = 3 Тогда
			ПодстрокаПоиска = "П0000" + Раздел + "010" + НомСтр + "01_";
			ПодстрокаЗамены = "П0000" + Раздел + "010" + НомСтр + "03_";
			КодПоказателя = ТабличныйДокумент.Области[СтрЗаменить(Область.Имя, ПодстрокаПоиска, ПодстрокаЗамены)].Значение;
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "pril_vid_okpd_" + Раздел + НомСтр);
		ИначеЕсли (Раздел = 1 И Графа = 6) Или (Раздел = 3 И Графа = 5) Тогда
			СтандартнаяОбработка = Ложь;
			...
			МассивВыбора = ПараметрВводаПоля.ТаблицаЗначений.НайтиСтроки(Новый Структура("Ключ",
					ТабличныйДокумент.Области[Лев(Область.Имя, 12) + "3_" + Сред(Область.Имя, 15)].Значение));
			...
			Возврат;
		ИначеЕсли (Раздел = 1 Или Раздел = 2) И Графа = 4 Тогда
			КодПоказателя = Область.Значение;
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникТипТовара");
		ИначеЕсли (Раздел = 1 И Графа = 9)
			  Или (Раздел = 2 И Графа = 7)
			  Или (Раздел = 3 И Графа = 8) Тогда
			  
			КодПоказателя = Область.Значение;
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникПричиныИзмененияЦены");
		КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Раздел = 1 По 3 Цикл
		...
		Если Раздел = 1 Тогда
			НомерОсновнойЧастиКон = 42;
		ИначеЕсли Раздел = 2 Тогда
			НомерОсновнойЧастиКон = 26;
		ИначеЕсли Раздел = 3 Тогда
			НомерОсновнойЧастиКон = 19;
		КонецЕсли;
		
		Для НомерОсновнойЧасти = 4 По НомерОсновнойЧастиКон Цикл
			
			НаимТекущаяОсновнаяЧасть = "ОсновнаяЧасть" + Раздел + "_" + НомерОсновнойЧасти;
			ТекущаяОсновнаяЧасть     = ТабличныйДокумент.ПолучитьОбласть(НаимТекущаяОсновнаяЧасть);
			НомерДобавлениеСтроки = НомерДобавлениеСтроки + 1;
			
			Если Не ПечатнаяФорма.ПроверитьВывод(ТекущаяОсновнаяЧасть) Тогда
				СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-цены приобретения");
				ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			...
			ПечатнаяФорма.Вывести(ТекущаяОсновнаяЧасть);
			
			ВерхМногострочнойЧасти = ТабличныйДокумент.Области[НаимТекущаяОсновнаяЧасть].Низ + 1;
			НизМногострочнойЧасти = ТабличныйДокумент.Области["ДобавлениеСтроки" + НомерДобавлениеСтроки].Верх - 1;
		...
		Если Раздел = 1 Тогда
			НаимОбластьПодвалРаздела = "ОсновнаяЧасть1_43";
		ИначеЕсли Раздел = 2 Тогда
			НаимОбластьПодвалРаздела = "ОсновнаяЧасть2_27";
		ИначеЕсли Раздел = 3 Тогда
			НаимОбластьПодвалРаздела = "ОсновнаяЧасть3_20";
		КонецЕсли;
		
		Если ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть(НаимОбластьПодвалРаздела)) Тогда
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть(НаимОбластьПодвалРаздела));
		Иначе
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-цены приобретения");
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть(НаимОбластьПодвалРаздела));
		...
		Если Раздел = 3 Тогда
			ОбластьПодвал = ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть4");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникТипТовара",
		512, , "Выбор тип товара", КоллекцияСписковВыбора["ТипыТоваров"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникПричиныИзмененияЦены",
		512, , "Выбор причины изменения цены", КоллекцияСписковВыбора["ПричиныИзмененияЦены"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникЕИ",
		512, , "Выбор единицы измерения", КоллекцияСписковВыбора["s_eiokdp"]);
	...
	Для Раздел = 1 По 3 Цикл
		...
		ИначеЕсли Раздел = 3 Тогда
			СтрокаМнгЧКон = 16;
		КонецЕсли;
		Для СтрокаМнгЧ = 1 По СтрокаМнгЧКон Цикл
			ОКПД2МнгЧ = КоллекцияСписковВыбора["s_vid_okpd"].СкопироватьКолонки();
			НовСтр = ОКПД2МнгЧ.Добавить();
			НовСтр.Название = "Пустое значение";
			СправочникСтроки = "pril_vid_okpd_" + Раздел + Формат(СтрокаМнгЧ, "ЧЦ=2; ЧВН=");
			Для Каждого Стр Из КоллекцияСписковВыбора[СправочникСтроки] Цикл
				НС = КоллекцияСписковВыбора["s_vid_okpd"].НайтиСтроки(Новый Структура("Код", Стр.Код));
				Если НС.Количество() > 0 Тогда 
					НовСтр = ОКПД2МнгЧ.Добавить();
					ЗаполнитьЗначенияСвойств(НовСтр, НС[0]);
				КонецЕсли;
			КонецЦикла;
			РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, СправочникСтроки,
				512, , "Выбор ОКПД2", ОКПД2МнгЧ);
		КонецЦикла;
	КонецЦикла;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	ИсходноеЗначение = КодПоказателя;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	РезультатНазвание = ?(ВыборНазвание = "Пустое значение", "", ВыборНазвание);
	
	ОбластьИмя = ДополнительныеПараметры.Область.Имя;
	ЛевОбластьИмя = Лев(ДополнительныеПараметры.Область.Имя, 6);
	
	Раздел = Число(Сред(ОбластьИмя, 6, 1));
	Графа = Число(Сред(ОбластьИмя, 13, 1));
	НомСтр = Сред(ОбластьИмя, 10, 2);
	
	Если Графа = 1 Или Графа = 3 Тогда
		КолонкаНазвание = "П0000" + Раздел + "010" + НомСтр + "01_" + Сред(ОбластьИмя, 15);
		КолонкаКод		= "П0000" + Раздел + "010" + НомСтр + "03_" + Сред(ОбластьИмя, 15);
		Если Графа = 1 Тогда
			ПолеТабличногоДокументаФормаОтчета.Области[ОбластьИмя].Значение = РезультатНазвание;
			ПолеТабличногоДокументаФормаОтчета.Области[КолонкаКод].Значение = ВыборКод;
		Иначе
			ПолеТабличногоДокументаФормаОтчета.Области[ОбластьИмя].Значение = ВыборКод;
			ПолеТабличногоДокументаФормаОтчета.Области[КолонкаНазвание].Значение = РезультатНазвание;
		КонецЕсли;
	ИначеЕсли (Раздел = 1 И Графа = 6) Или (Раздел = 3 И Графа = 5) Тогда
		Если Раздел = 1 Тогда
			ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(ОбластьИмя, "П00001010" + НомСтр + "06_", "П00001010" + НомСтр + "05_")].Значение = ВыборКод;
			ПолеТабличногоДокументаФормаОтчета.Области[ОбластьИмя].Значение = РезультатНазвание;
		Иначе
			ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(ОбластьИмя, "П00003010" + НомСтр + "05_", "П00003010" + НомСтр + "04_")].Значение = ВыборКод;
			ПолеТабличногоДокументаФормаОтчета.Области[ОбластьИмя].Значение = РезультатНазвание;
		КонецЕсли;
	Иначе
		ПолеТабличногоДокументаФормаОтчета.Области[ОбластьИмя].Значение = ВыборКод;
