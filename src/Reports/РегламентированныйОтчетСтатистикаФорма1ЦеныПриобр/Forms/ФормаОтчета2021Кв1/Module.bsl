...
Процедура ДополнитьРеквизитыФормы()
	...
	Для Раздел = 1 По 3 Цикл
		...
		ИмяДобавляемогоРеквизита = "П00" + Раздел + "0010001";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если СтрНайти(Область.Имя, "П001001000101_") = 1 Или СтрНайти(Область.Имя, "П001001000102_") = 1
		 Или СтрНайти(Область.Имя, "П002001000101_") = 1 Или СтрНайти(Область.Имя, "П002001000102_") = 1
		 Или СтрНайти(Область.Имя, "П003001000101_") = 1 Или СтрНайти(Область.Имя, "П003001000102_") = 1 Тогда
			...
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникРаздел" + Сред(Область.Имя, 4,1) + "_ОКПД2");
			КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		ИначеЕсли СтрНайти(Область.Имя, "П001001000104_") = 1 Или СтрНайти(Область.Имя, "П002001000104_") = 1 Тогда
			ОбластьДляВвода = Область;
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникТипыТоваров");
			КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		ИначеЕсли СтрНайти(Область.Имя, "П001001000108_") = 1 Или СтрНайти(Область.Имя, "П002001000107_") = 1
			  Или СтрНайти(Область.Имя, "П003001000107_") = 1 Тогда
			ОбластьДляВвода = Область;
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникПричиныИзмененияЦены");
			КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		ИначеЕсли Сред(Область.Имя, 13, 1) = "3"
			  Или (Сред(Область.Имя, 13, 1) = "5" И Сред(Область.Имя, 4, 1) = "1")
			  Или (Сред(Область.Имя, 13, 1) = "4" И Сред(Область.Имя, 4, 1) = "3") Тогда
			
			Если Сред(Область.Имя, 13, 1) = "3" Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
					ЭтотОбъект, "СправочникКодТовараПоОКПД2");
				МассивВыбора = ПараметрВводаПоля.ТаблицаЗначений.НайтиСтроки(Новый Структура("Ключ",
					ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 12) + "2" + Сред(Область.Имя, 14)].Значение));
			Иначе
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
					ЭтотОбъект, "СправочникЕдиницаИзмеренияПоОКЕИ");
				МассивВыбора = ПараметрВводаПоля.ТаблицаЗначений.НайтиСтроки(Новый Структура("Ключ",
					ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 12) + "3" + Сред(Область.Имя, 14)].Значение));
			КонецЕсли;
	...
	Если ТЗВыбора.Количество() > 0 Тогда
		ВвестиПоказатель(Элемент, ОбластьДляВвода, СмежнаяОбласть, Ложь,
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ТекущаяОбласть = ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть4");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникТипыТоваров",
		512, , "Выбор тип товара", КоллекцияСписковВыбора["ТипыТоваров"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникРаздел1_ОКПД2",
		512, , "Выбор кода ОКПД2", КоллекцияСписковВыбора["Раздел1_ОКПД2"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникРаздел2_ОКПД2",
		512, , "Выбор кода ОКПД2", КоллекцияСписковВыбора["Раздел2_ОКПД2"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникРаздел3_ОКПД2",
		512, , "Выбор кода ОКПД2", КоллекцияСписковВыбора["Раздел3_ОКПД2"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникКодТовараПоОКПД2",
		512, , "Выбор кода вида товара", КоллекцияСписковВыбора["КодТовараПоОКПД2"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникЕдиницаИзмеренияПоОКЕИ",
		512, , "Выбор единицы измерения", КоллекцияСписковВыбора["ЕдиницаИзмеренияПоОКЕИ"]);
