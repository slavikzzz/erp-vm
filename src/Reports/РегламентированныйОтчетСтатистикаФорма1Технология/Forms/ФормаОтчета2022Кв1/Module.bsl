...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы2 = "П0000300284";
	СтруктураРеквизитовФормы.мИдГруппы3 = "П0000601602";
	...
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы1, 6);
	...
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы3, 2);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если СтрНайти(Область.Имя, "П000010100102_") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ГруппаПередовыхТехнологий");
		ИначеЕсли СтрНайти(Область.Имя, "П0000300284") = 1 Тогда
			Область           = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "01_", Лев(Область.Имя, 11) + "02_"));
			СмежнаяОбласть    = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "02_", Лев(Область.Имя, 11) + "01_"));
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ГруппаПередовыхТехнологий");
		ИначеЕсли СтрНайти(Область.Имя, "П000010100103_") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ОКВЭД");
		ИначеЕсли СтрНайти(Область.Имя, "П000010100104_") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ПризнакНовизны");
		ИначеЕсли СтрНайти(Область.Имя, "П000010100105_") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ПризнакТехнологии");
		ИначеЕсли Лев(Область.Имя, 8) = "П0000401" Тогда
		...
		ИначеЕсли Лев(Область.Имя, 8) = "П0000301" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникЭффектыВнедрения");
		ИначеЕсли Лев(Область.Имя, 8) = "П0000501" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникФакторыПрепятствующиеВнедрению");
		КонецЕсли;
		
	КонецЕсли;
	
	Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
		ВвестиПоказатель(Элемент, Область, СмежнаяОбласть,
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	// Определяем текущую область таблицы
	Если ИмяГруппы = "П0000101001" Тогда
...
&НаКлиенте
Процедура ВвестиПоказатель(Элемент, Область, СмежнаяОбласть = Неопределено, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Области["П000010100101_1"].Значение = "101";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 5 По 13 Цикл
		...
		СуммаПоСтр = 0;
		Для Стр = 270 По 283 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000201269" + Графа].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 262 По 268 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000201261" + Графа].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 253 По 260 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000201252" + Графа].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 242 По 251 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000201241" + Графа].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 233 По 240 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000201232" + Графа].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 227 По 231 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000201226" + Графа].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 206 По 225 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000201205" + Графа].Значение = СуммаПоСтр;
		
		Области["П0000201202" + Графа].Значение
			= Области["П0000201203" + Графа].Значение
			+ Области["П0000201204" + Графа].Значение;
			
		Области["П0000201201" + Графа].Значение
			= Области["П0000201202" + Графа].Значение
			+ Области["П0000201205" + Графа].Значение
			+ Области["П0000201226" + Графа].Значение
			+ Области["П0000201232" + Графа].Значение
			+ Области["П0000201241" + Графа].Значение
			+ Области["П0000201252" + Графа].Значение
			+ Области["П0000201261" + Графа].Значение
			+ Области["П0000201269" + Графа].Значение;
		
	КонецЦикла;
	
	Для Стр = 201 По 283 Цикл
		Области["П0000201" + Стр + "04"].Значение = Области["П0000201" + Стр + "05"].Значение
			+ Области["П0000201" + Стр + "06"].Значение
			+ Области["П0000201" + Стр + "07"].Значение
			+ Области["П0000201" + Стр + "08"].Значение;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	МассивСтрокАвторасчета =
		СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("201;202;205;226;232;241;252;261;269", ";");
	
	Для Стр = 201 По 283 Цикл
		Если МассивСтрокАвторасчета.Найти(Строка(Стр)) <> Неопределено Тогда
			Для Гр = 5 По 13 Цикл
				мФормаОтчета.Области["П0000201" + Стр + Формат(Гр,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
				мФормаОтчета.Области["П0000201" + Стр + Формат(Гр,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
			КонецЦикла;
		КонецЕсли;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	П0000300284.Очистить();
	П0000601602.Очистить();
	РегламентированнаяОтчетность.НачальныеОперацииСМногострочнымиБлоками(ЭтотОбъект);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	МассивПроверки.Добавить(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_1"));
	МассивПроверки.Добавить(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_2"));
	...
	ПечатнаяФорма.Вывести(ПолеТабличногоДокументаФормаОтчета.ПолучитьОбласть("ПустаяСтрока"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_2"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_5"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-технология");
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ПолеТабличногоДокументаФормаОтчета.ПолучитьОбласть("ПустаяСтрока"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_2"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_6"));
	
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_7"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_8"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_9"));
	Область3_10 = ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_10");
	
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть3_9.Низ + 1;
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверки.Добавить(Область3_10);
		Если ПечатнаяФорма.ПроверитьВывод(МассивПроверки) Тогда
			ПечатнаяФорма.Вывести(ТекущаяОбластьМногострочнойЧасти);
		Иначе
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-технология");
	...
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_10"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-технология");
	...
	Если ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть6_4")) Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ОКВЭД",
		512, , "Выбор кода ОКВЭД", ОКВЭД);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ГруппаПередовыхТехнологий",
		512, , "Выбор кода группы технологий", ГруппаПередовыхТехнологий);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ПризнакТехнологии",
		512, , "Выбор признака", ПризнакТехнологии);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ПризнакНовизны",
		512, , "Выбор признака", ПризнакНовизны);
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникЭффектыВнедрения",
		512, , "Выбор эффекта", КоллекцияСписковВыбора["ЭффектыВнедрения"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникФакторыПрепятствующиеВнедрению",
		512, , "Выбор фактора", КоллекцияСписковВыбора["ФакторыПрепятствующиеВнедрению"]);
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Пока Истина Цикл
		...
		Если Не ПараметрыВыгрузки.Свойство("П000010100101_" + ИндСтр) Тогда
			Прервать;
		КонецЕсли;
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000010100102_" + ИндСтр]) Или ЗначениеЗаполнено(ПараметрыВыгрузки["П000010100103_" + ИндСтр]) Тогда
			ПараметрыВыгрузки.Вставить("П000010100101_" + ИндСтр, ИндСтр);
			ПараметрыВыгрузки.Вставить("ПД00010100107_" + ИндСтр, "1");
		КонецЕсли;
	КонецЦикла;
	...
	Пока Истина Цикл
		Инд = Инд + 1;
		ИндСтр = Формат(Инд, "ЧГ=");
		Если Не ПараметрыВыгрузки.Свойство("П000030028402_" + ИндСтр) Тогда
			Прервать;
		КонецЕсли;
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000030028402_" + ИндСтр]) Тогда
			ПараметрыВыгрузки.Вставить("ПД00030028404_" + ИндСтр, "1");
		КонецЕсли;
	КонецЦикла;
	...
	Пока Истина Цикл
		Инд = Инд + 1;
		ИндСтр = Формат(Инд, "ЧГ=");
		Если Не ПараметрыВыгрузки.Свойство("П000060160202_" + ИндСтр) Тогда
			Прервать;
		КонецЕсли;
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000060160202_" + ИндСтр]) Тогда
			ПараметрыВыгрузки.Вставить("ПД00060160203_" + ИндСтр, "1");
		КонецЕсли;
	КонецЦикла;
