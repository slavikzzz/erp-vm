...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы2 = "П0000601602";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		ИначеЕсли (Лев(Область.Имя, 8) = "П0000201" И Прав(Область.Имя, 2) = "14") Или Лев(Область.Имя, 8) = "П0000401" Тогда
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Графа = 5 По 13 Цикл
		Гр = Формат(Графа, "ЧЦ=2; ЧВН=");
		
		СуммаПоСтр = 0;
		Для Стр = 272 По 285 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Гр].Значение;
		КонецЦикла;
		Области["П0000201271" + Гр].Значение = СуммаПоСтр;
		
		Области["П0000201267" + Гр].Значение = Области["П0000201268" + Гр].Значение
			+ Области["П0000201269" + Гр].Значение
			+ Области["П0000201270" + Гр].Значение;
			
		Области["П0000201261" + Гр].Значение = Области["П0000201262" + Гр].Значение
			+ Области["П0000201263" + Гр].Значение
			+ Области["П0000201264" + Гр].Значение
			+ Области["П0000201265" + Гр].Значение
			+ Области["П0000201266" + Гр].Значение;
			
		СуммаПоСтр = 0;
		Для Стр = 253 По 260 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Гр].Значение;
		КонецЦикла;
		Области["П0000201252" + Гр].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 243 По 251 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Гр].Значение;
		КонецЦикла;
		Области["П0000201242" + Гр].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 233 По 241 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Гр].Значение;
		КонецЦикла;
		Области["П0000201232" + Гр].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 227 По 231 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Гр].Значение;
		КонецЦикла;
		Области["П0000201226" + Гр].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 206 По 225 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Гр].Значение;
		КонецЦикла;
		Области["П0000201205" + Гр].Значение = СуммаПоСтр;
		
		Области["П0000201202" + Гр].Значение = Области["П0000201203" + Гр].Значение
			+ Области["П0000201204" + Гр].Значение;
			
		Области["П0000201201" + Гр].Значение = Области["П0000201202" + Гр].Значение
			+ Области["П0000201205" + Гр].Значение
			+ Области["П0000201226" + Гр].Значение
			+ Области["П0000201232" + Гр].Значение
			+ Области["П0000201242" + Гр].Значение
			+ Области["П0000201252" + Гр].Значение
			+ Области["П0000201261" + Гр].Значение
			+ Области["П0000201267" + Гр].Значение
			+ Области["П0000201271" + Гр].Значение;
		
	КонецЦикла;
	
	Для Стр = 201 По 285 Цикл
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	МассивСтрокАвторасчета =
		СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("201;202;205;226;232;242;252;261;267;271", ";");
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Пока Истина Цикл 
		...
		Если Не ПараметрыВыгрузки.Свойство("П000010100101_" + ИндСтр) Тогда
			Прервать;
		КонецЕсли;
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000010100103_" + ИндСтр]) Или ЗначениеЗаполнено(ПараметрыВыгрузки["П000010100102_" + ИндСтр]) Тогда
	...
	Пока Истина Цикл 
		...
		Если Не ПараметрыВыгрузки.Свойство("П000060160202_" + ИндСтр) Тогда
			Прервать;
		КонецЕсли;
		ОКПО = ПолеТабличногоДокументаФормаОтчета.Области["П000060160202_"+ ИндСтр].Значение;
		ПараметрыВыгрузки.Вставить("ПП000060160202_"+ ИндСтр, ?(ЗначениеЗаполнено(ОКПО), ОКПО, ""));
		ПараметрыВыгрузки.Вставить("ПП000060160203_"+ ИндСтр, ?(ЗначениеЗаполнено(ОКПО), 1, ""));
	КонецЦикла;
