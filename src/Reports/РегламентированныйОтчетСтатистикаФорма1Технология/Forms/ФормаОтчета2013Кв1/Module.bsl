...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 5 По 12 Цикл 
		...
		мФормаОтчета.Области["П02"+ГрСтр+"_233"].Значение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, мФормаОтчета.Области["П02"+ГрСтр+"_234"].Значение)
														+ РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, мФормаОтчета.Области["П02"+ГрСтр+"_235"].Значение)
														+ РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, мФормаОтчета.Области["П02"+ГрСтр+"_236"].Значение);
		мФормаОтчета.Области["П02"+ГрСтр+"_230"].Значение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, мФормаОтчета.Области["П02"+ГрСтр+"_231"].Значение)
		...
		мФормаОтчета.Области["П02"+ГрСтр+"_201"].Значение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, мФормаОтчета.Области["П02"+ГрСтр+"_202"].Значение)
														...
														+ РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, мФормаОтчета.Области["П02"+ГрСтр+"_233"].Значение);
													КонецЦикла;
	
	Для Стр = 201 По 236 Цикл 
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Область.Значение) = Тип("Число") Тогда 
		Попытка
			Область.Значение = ?(РезультатВыбора["Код"]="",0,Число(РезультатВыбора["Код"]));
		Исключение
			Область.Значение = 0;
		КонецПопытки;
	Иначе
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Для Стр = 201 По 236 Цикл 
		ТекстФормулы = "по строке " + Стр + " гр.4 = гр.9 + гр.10 + гр.11";
		Описание     = "по строке " + Стр + " гр.4 <> гр.9 + гр.10 + гр.11";
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П0204_" + Стр]);
		ПраваяЧасть1 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П0209_" + Стр]);
		ПраваяЧасть2 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П0210_" + Стр]);
		ПраваяЧасть3 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П0211_" + Стр]);
		...
		Зн12 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П0212_" + Стр]);
		Зн04 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П0204_" + Стр]);
		ТекстФормулы = "по строке " + Стр + " если гр.12>0, то гр.4>0";
		Описание = "по строке " + Стр + " гр.12>0 и гр.4=0";
		Расшифровка = "гр.12=" + Строка(Зн12) + ", гр.4=" + Строка(Зн04);
		
		Если Зн12 = 0 Или Зн04 > 0 Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
			ЭтоОшибка = Истина;
