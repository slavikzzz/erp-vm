...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "11/09/2018";
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Пока Истина Цикл 
		...
		Если Не ПараметрыВыгрузки.Свойство("П000001000101_" + ИндСтр) Тогда 
			Прервать;
		КонецЕсли;
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000103_" + ИндСтр]) Или ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000102_" + ИндСтр]) Тогда 
			ПараметрыВыгрузки.Вставить("П000001000100_" + ИндСтр, ИндСтр);
		КонецЕсли;
	КонецЦикла;
