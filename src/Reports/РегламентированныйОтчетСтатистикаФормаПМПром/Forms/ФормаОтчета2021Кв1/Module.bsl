...
Процедура ФормированиеТаблицыВыбораПродукции()
	...
	МассивОтбора =
		СвойстваПоказателей.НайтиСтроки(Новый Структура("Показатель", "П002000100100"))[0].ТаблицаЗначений.Выгрузить(
...
Процедура СформироватьСпискиВыбора()
	...
	512, ,                          , КоллекцияСписковВыбора["СправочникПриложениеДляВводаОКПД"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П002000100101",
	512, , "Выбор продукции"        , КоллекцияСписковВыбора["СправочникОКПД"]);
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "04" Тогда
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(
			КолонкаПоиска, ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "04_" + НомерСтроки].Значение));
	ИначеЕсли Сред(Область.Имя, 12, 2) = "02" Или Сред(Область.Имя, 12, 2) = "03" Тогда
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(
			КолонкаПоиска, Формат(ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "03_" + НомерСтроки].Значение, "ЧГ=")));
	Иначе
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если СтрНайти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы1) > 0 Тогда
		...
		Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "04" Тогда
			...
			ПредыдущееЗначениеВКолонке01 = ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "01_" + НомерСтроки].Значение;
			РезультатНазвание = ?(ВыборНазвание = "Пустое значение", "", ВыборНазвание);
			ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "01_" + НомерСтроки].Значение =
				?(ЗначениеЗаполнено(ВыборКод), РезультатНазвание, "");
			ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "04_" + НомерСтроки].Значение =
				?(ВыборКод = "0", "", ВыборКод);
			
			// Приводим в соответствие зависимые ячейки - наим. единицы измерения и ее код.
			Если ЗначениеЗаполнено(ВыборКод) Тогда
				Если ПредыдущееЗначениеВКолонке01 <> ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "01_" + НомерСтроки].Значение Тогда
					ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "02_" + НомерСтроки].Значение = "";
					ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "03_" + НомерСтроки].Значение = 0;
				КонецЕсли;
			Иначе
				ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "02_" + НомерСтроки].Значение = "";
				ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "03_" + НомерСтроки].Значение = 0;
			...
			Если ВыборКод <> "0" Тогда
				ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "05_" + НомерСтроки].Формат = "";
				ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "06_" + НомерСтроки].Формат = "";
			КонецЕсли;
		ИначеЕсли Сред(Область.Имя, 12, 2) = "02" Или Сред(Область.Имя, 12, 2) = "03" Тогда
			
			ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "02_" + НомерСтроки].Значение =
				?(ЗначениеЗаполнено(РезультатВыбора["Код"]), РезультатВыбора["Название"], "");
			ПолеТабличногоДокументаФормаОтчета.Области[ИмяГруппы + "03_" + НомерСтроки].Значение = РезультатВыбора["Код"];
		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			...
			Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "04" Тогда
				ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе,
					ТЗВыбораПродукции, СтандартнаяОбработка, КолонкаПоиска);
			Иначе
				МассивВыбора = ПараметрВводаПоля.ТаблицаЗначений.НайтиСтроки(Новый Структура("Ключ",
					ПолеТабличногоДокументаФормаОтчета.Области["П002000100104_" + Сред(Область.Имя, 15)].Значение));
				ТЗВыбора.Очистить();
