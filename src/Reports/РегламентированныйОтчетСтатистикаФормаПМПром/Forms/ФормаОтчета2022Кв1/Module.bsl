...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.ОтображатьКнопкуЗаполнить Тогда
		...
		Если НЕ РегламентированнаяОтчетностьПереопределяемый.РеализованаНастройкаАвтозаполняемыхПоказателейРеглОтчета(ИДОтчета, ИДРедакцииОтчета) Тогда
			...
			ГруппаКнопок3 = ЭтаФорма.КоманднаяПанель.ПодчиненныеЭлементы.Найти("ГруппаКнопок3");
...
Процедура ФормироватьСтруктуруСтраницОтчета() Экспорт
	...
	СтрокаУровня1.Представление            = "Статистика: Форма ПМ-пром";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Сред(Область.Имя, 12, 2) = "04" Тогда
		...
		ОбластьКод = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, Сред(Область.Имя, 1, 11) + "04_", Сред(Область.Имя, 1, 11) + "05_"));
		ОбластьНаим  = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, Сред(Область.Имя, 1, 11) + "05_", Сред(Область.Имя, 1, 11) + "04_"));
		
		НомСтр = Сред(Область.Имя, СтрНайти(Область.Имя, "_") + 1);
		
		КодОКПД2 = СокрЛП(Строка(ПолеТабличногоДокументаФормаОтчета.Области[Сред(Область.Имя, 1, 11) + "03_" + НомСтр].Значение));
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, ИмяСправочника);
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателей.Добавить(ВыбТабличноеПоле.Области["Код1"].Значение,                      "Код1");
	СписокПоказателей.Добавить(ВыбТабличноеПоле.Области["Код2"].Значение,                      "Код2");
	СписокПоказателей.Добавить(ВыбТабличноеПоле.Области["Код3"].Значение,                      "Код3");
	СписокПоказателей.Добавить(ВыбТабличноеПоле.Области["Код4"].Значение,                      "Код4");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма ПМ-пром");
	...
	Если ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3")) Тогда
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок1",
		РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтотОбъект, П0020001001, Ложь));
