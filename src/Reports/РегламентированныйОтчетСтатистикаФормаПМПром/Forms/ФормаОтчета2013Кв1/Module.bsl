...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "28/07/2009";
...
Процедура ДоступностьИЦветЯчеекХарактеристикСоответственноМатериаламПМПром(ПарамФорма)
	
	Попытка
		Для НомСтр = 1 По ПарамФорма.П0020001001.Количество() Цикл
			Если ЗначениеЗаполнено(ПарамФорма.ТабличныйДокумент.Области["П002000100101_" + Формат(НомСтр, "ЧГ=")].Значение) Тогда
				ПарамФорма.ТабличныйДокумент.Области["П002000100102_" + Формат(НомСтр, "ЧГ=")].ЦветФона = ПарамФорма.СтруктураРеквизитовФормы.ЦветЖелтый;
				ПарамФорма.ТабличныйДокумент.Области["П002000100102_" + Формат(НомСтр, "ЧГ=")].Защита = Ложь;
				ПарамФорма.ТабличныйДокумент.Области["П002000100103_" + Формат(НомСтр, "ЧГ=")].ЦветФона = ПарамФорма.СтруктураРеквизитовФормы.ЦветЖелтый;
				ПарамФорма.ТабличныйДокумент.Области["П002000100103_" + Формат(НомСтр, "ЧГ=")].Защита = Ложь;
			Иначе
				ПарамФорма.ТабличныйДокумент.Области["П002000100102_" + Формат(НомСтр, "ЧГ=")].ЦветФона = ПарамФорма.СтруктураРеквизитовФормы.ЦветБелый;
				ПарамФорма.ТабличныйДокумент.Области["П002000100102_" + Формат(НомСтр, "ЧГ=")].Защита = Истина;
				ПарамФорма.ТабличныйДокумент.Области["П002000100103_" + Формат(НомСтр, "ЧГ=")].ЦветФона = ПарамФорма.СтруктураРеквизитовФормы.ЦветБелый;
				ПарамФорма.ТабличныйДокумент.Области["П002000100103_" + Формат(НомСтр, "ЧГ=")].Защита = Истина;
			КонецЕсли;
		КонецЦикла;
	Исключение
	Конецпопытки;
...
Процедура ОчиститьТабличноеПоле(Форма, ВыбТабличноеПолеИмя) Экспорт
	...
	ДоступностьИЦветЯчеекХарактеристикСоответственноМатериаламПМПром(Форма);
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если СтрНайти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы1) > 0 Тогда
		...
		Если Сред(Область.Имя, 12, 2) = "01" Тогда
			...
			ТабличныйДокумент.Области[Область.Имя].Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборНазвание, "");
			...
			Если ЗначениеЗаполнено(ВыборКод) Тогда
				...
				КонецЕсли;
				ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "02_" + НомерСтроки].ЦветФона = Новый Цвет(255, 255, 192);
				ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "02_" + НомерСтроки].Защита = Ложь;
				ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "03_" + НомерСтроки].ЦветФона = Новый Цвет(255, 255, 192);
				ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "03_" + НомерСтроки].Защита = Ложь;
			Иначе
				ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "02_" + НомерСтроки].Значение = "";
				ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "02_" + НомерСтроки].ЦветФона = Новый Цвет();
				ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "02_" + НомерСтроки].Защита = Истина;
				ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "03_" + НомерСтроки].Значение = 0;
				ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "03_" + НомерСтроки].ЦветФона = Новый Цвет();
				ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "03_" + НомерСтроки].Защита = Истина;
			КонецЕсли;
...
Процедура РазделыОтчетаПриАктивизацииСтроки(Элемент)
	...
	ИначеЕсли Элемент.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1" Тогда
		...
		ДоступностьИЦветЯчеекХарактеристикСоответственноМатериаламПМПром(ЭтаФорма);
	КонецЕсли;
