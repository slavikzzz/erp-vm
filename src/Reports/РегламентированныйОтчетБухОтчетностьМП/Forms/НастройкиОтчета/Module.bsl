...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДоступныеЕдиницыИзмерения = Неопределено
	 ИЛИ Параметры.ДоступныеЕдиницыИзмерения.Найти(Перечисления.ПорядкиОкругленияОтчетности.Окр1) <> Неопределено Тогда
		Элементы.ПредставлениеЕдиницыИзмерения.СписокВыбора.Добавить("в рублях");
	КонецЕсли;
	
	Если Параметры.ДоступныеЕдиницыИзмерения = Неопределено
	 ИЛИ Параметры.ДоступныеЕдиницыИзмерения.Найти(Перечисления.ПорядкиОкругленияОтчетности.Окр1000) <> Неопределено Тогда
		Элементы.ПредставлениеЕдиницыИзмерения.СписокВыбора.Добавить("в тысячах рублей");
	КонецЕсли;
	
	Если Параметры.ДоступныеЕдиницыИзмерения = Неопределено
	 ИЛИ Параметры.ДоступныеЕдиницыИзмерения.Найти(Перечисления.ПорядкиОкругленияОтчетности.Окр1000000) <> Неопределено Тогда
	...
	Если мПараметры.ПараметрыОтображенияСумм И НЕ Параметры.ЕдиницаИзмерения = Неопределено Тогда
		...
		ПроверитьТочность(ЕдиницаИзмерения, ТочностьЕдиницыИзмерения,
			ПеречислениеПорядкиОкругленияОтчетностиОкр1,
			ПеречислениеПорядкиОкругленияОтчетностиОкр1000,
			ПеречислениеПорядкиОкругленияОтчетностиОкр1000000);
	...
	Иначе
		Элементы.РазрешитьВстроеннуюПечать.Видимость = Ложь;
	КонецЕсли;
	...
	Иначе
		Элементы.ОтключитьСчетчикСтраниц.Видимость = Ложь;
	КонецЕсли;
	...
	Иначе
		Элементы.ОтклВыборЗначений.Видимость = Ложь;
	КонецЕсли;
	...
	Иначе
		Элементы.Автосохранение.Видимость = Ложь;
	...
	Иначе
		Элементы.ПроверятьСоотношенияПриПечатиИВыгрузки.Видимость = Ложь;
	КонецЕсли;
	...
	Иначе
		Элементы.ОтключитьАвтоматическийРасчет.Видимость = Ложь;
	КонецЕсли;
	...
	Иначе
		Элементы.СохранятьРасшифровкуАвтозаполнения.Видимость = Ложь;
	КонецЕсли;
	...
	Иначе
		Элементы.ОтключитьАвтоматическоеУправлениеВариантами.Видимость = Ложь;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если мПараметры.ИмеетсяРеквизитДеревоВыбранныхСтраниц Тогда
		...
		// Многострочные названия разделов обрезаются, требуется приведение к одной строке.
		Для Каждого СтрокаСпискаОтчетов Из Дерево.ПолучитьЭлементы() Цикл
			СтрокаСпискаОтчетов.Представление = СтрЗаменить(СтрокаСпискаОтчетов.Представление, Символы.ПС, " ");
		КонецЦикла;
