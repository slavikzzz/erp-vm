...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.ИмяСохраняемогоПараметра = "РегламентированныйОтчетБухОтчетностьМП_ФормаОтчета2012Кв4_2_Изменения";
	СтруктураРеквизитовФормы.СохраненныйВыборПользователя = Загрузить(СтруктураРеквизитовФормы.ИмяСохраняемогоПараметра);
	СтруктураРеквизитовФормы.КоличествоСтрокИнформационногоМакета = СтрЧислоСтрок(ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаОтчета2012Кв4_2_Изменения").ПолучитьТекст());
...
#Область ПанельОтправкаБухОтчетностиВРосстат
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если Область.Имя = "ДатаПодписиОтчета" Тогда
		...
		Если СтруктураДанныхБаланс.Свойство("ДатаПодписиОтчета") Тогда
		...
		Если СтруктураДанныхОЦИПС.Свойство("ДатаПодписиОтчета") Тогда
			СтруктураДанныхОЦИПС.ДатаПодписиОтчета = ДатаПодписи;
		КонецЕсли;
...
Процедура ОткрытьСоставПоказателей(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ДеревоНастроек", ЭтотОбъект[НастройкиСоставаПоказателейОтчета[СтруктураРеквизитовФормы.НаимТекущегоРаздела]]);
...
Функция РазделыОтчетаСДанными()
	...
	Возврат СтрРазделить("Баланс,ОДДС,ОИК,ОПУ,ОЦИПС", ",", Ложь);
	...
	Для Графа = 4 По 5 Цикл
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
		 ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета["П00010062000" + Графа], ОбластьИмя) > 0 Тогда
			РасчетСтроки6200 = ПоказательОтчета(Форма, РазмещениеДанных, "П00010062100" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010062150" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010062200" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010062300" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010062400" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010062500" + Графа);
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
		 ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета["П00010063100" + Графа], ОбластьИмя) > 0 Тогда
			РасчетСтроки6310 = ПоказательОтчета(Форма, РазмещениеДанных, "П00010063110" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010063120" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010063130" + Графа);
			
			УстановитьЗначениеПоказателя(РазмещениеДанных, "П00010063100" + Графа, РасчетСтроки6310);
			
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
		 ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета["П00010063200" + Графа], ОбластьИмя) > 0 Тогда
			РасчетСтроки6320 = ПоказательОтчета(Форма, РазмещениеДанных, "П00010063210" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010063220" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010063230" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010063240" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010063250" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010063260" + Графа);
			
			УстановитьЗначениеПоказателя(РазмещениеДанных, "П00010063200" + Графа, РасчетСтроки6320);
			
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
		 ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета["П00010063000" + Графа], ОбластьИмя) > 0 Тогда
			РасчетСтроки6300 = ПоказательОтчета(Форма, РазмещениеДанных, "П00010063100" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010063200" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010063300" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010063500" + Графа);
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
		 ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета["П00010064000" + Графа], ОбластьИмя) > 0 Тогда
			РасчетСтроки6400 = ПоказательОтчета(Форма, РазмещениеДанных, "П00010061000" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010062000" + Графа)
				+ ПоказательОтчета(Форма, РазмещениеДанных, "П00010063000" + Графа);
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт
	...
	Для Каждого Настройки Из НастройкиСоставаПоказателейОтчета Цикл
		...
		КонвертироватьДанныеФормыДеревоВДеревоЗначений(НовыйЭлементСтруктуры, ЭтотОбъект[Настройки.Значение]);
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если СохраненныеДанные.Свойство("НастройкиСоставаПоказателейОтчета", СохраненныеНастройкиСоставаПоказателейОтчета) Тогда
...
Процедура ВывестиНаПечатьЛист(ПараметрыПечати)
	...
	ИначеЕсли ПараметрыПечати.ИмяСтраницы = "ОЦИПС" Тогда
		ВывестиНаПечатьОЦИПС(ПараметрыПечати);
...
&НаСервере
Процедура ВывестиНаПечатьОЦИПС(ПараметрыПечати)
	
	СписокПечатныхФорм = Новый СписокЗначений;
	
	СоставОтчета = ЭтотОбъект[НастройкиСоставаПоказателейОтчета.ОЦИПС];
	ИсходныйТабличныйДокумент = ТабличныйДокумент;
	...
	Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОЦИПСРаздел62_Тело") И ПоказательВключенВОтчет(СоставОтчета, "ОЦИПСРаздел62") Тогда
		ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть62", ВертикальныеОбласти);
		Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6210") И ПоказательВключенВОтчет(СоставОтчета, "Строка6210") Тогда
			ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6210", ВертикальныеОбласти);
		КонецЕсли;
		Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6215") И ПоказательВключенВОтчет(СоставОтчета, "Строка6215") Тогда
			ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6215", ВертикальныеОбласти);
		КонецЕсли;
		Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6220") И ПоказательВключенВОтчет(СоставОтчета, "Строка6220") Тогда
			ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6220", ВертикальныеОбласти);
		КонецЕсли;
		Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6230") И ПоказательВключенВОтчет(СоставОтчета, "Строка6230") Тогда
			ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6230", ВертикальныеОбласти);
		КонецЕсли;
		Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6240") И ПоказательВключенВОтчет(СоставОтчета, "Строка6240") Тогда
			ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6240", ВертикальныеОбласти);
		КонецЕсли;
		Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6250") И ПоказательВключенВОтчет(СоставОтчета, "Строка6250") Тогда
			ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6250", ВертикальныеОбласти);
		КонецЕсли;
		Если ПоказательВключенВОтчет(СоставОтчета, "Строка6200") Тогда
			ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6200", ВертикальныеОбласти);
		КонецЕсли;
	КонецЕсли;
	
	Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОЦИПСРаздел63_Тело") И ПоказательВключенВОтчет(СоставОтчета, "ОЦИПСРаздел63") Тогда
		ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть63", ВертикальныеОбласти);
		
		ЗаполненыеПодчиненныеСтроки6310 = ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6311")
		                              ИЛИ ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6312")
		                              ИЛИ ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6313");
		Если ЗаполненыеПодчиненныеСтроки6310 И ПоказательВключенВОтчет(СоставОтчета, "Строка6310") Тогда
			ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6310", ВертикальныеОбласти);
			Если (ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6311") И ПоказательВключенВОтчет(СоставОтчета, "Строка6311"))
			 ИЛИ (ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6311") И ПоказательВключенВОтчет(СоставОтчета, "Строка6312"))
			 ИЛИ (ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6311") И ПоказательВключенВОтчет(СоставОтчета, "Строка6313")) Тогда
				ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ВТомЧисле6310", ВертикальныеОбласти);
			КонецЕсли;
			Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6311") И ПоказательВключенВОтчет(СоставОтчета, "Строка6311") Тогда
				ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6311", ВертикальныеОбласти);
			КонецЕсли;
			Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6312") И ПоказательВключенВОтчет(СоставОтчета, "Строка6312") Тогда
				ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6312", ВертикальныеОбласти);
			КонецЕсли;
			Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6313") И ПоказательВключенВОтчет(СоставОтчета, "Строка6313") Тогда
				ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6313", ВертикальныеОбласти);
			КонецЕсли;
		КонецЕсли;
		
		ЗаполненыеПодчиненныеСтроки6320 = ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6321")
		                              ИЛИ ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6322")
		                              ИЛИ ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6323")
		                              ИЛИ ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6324")
		                              ИЛИ ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6325")
		                              ИЛИ ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6326");
		Если ЗаполненыеПодчиненныеСтроки6320 И ПоказательВключенВОтчет(СоставОтчета, "Строка6320") Тогда
			ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6320", ВертикальныеОбласти);
			Если (ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6321") И ПоказательВключенВОтчет(СоставОтчета, "Строка6321"))
			 ИЛИ (ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6322") И ПоказательВключенВОтчет(СоставОтчета, "Строка6322"))
			 ИЛИ (ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6323") И ПоказательВключенВОтчет(СоставОтчета, "Строка6323"))
			 ИЛИ (ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6324") И ПоказательВключенВОтчет(СоставОтчета, "Строка6324"))
			 ИЛИ (ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6325") И ПоказательВключенВОтчет(СоставОтчета, "Строка6325"))
			 ИЛИ (ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6326") И ПоказательВключенВОтчет(СоставОтчета, "Строка6326")) Тогда
				ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ВТомЧисле6320", ВертикальныеОбласти);
			КонецЕсли;
			Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6321") И ПоказательВключенВОтчет(СоставОтчета, "Строка6321") Тогда
				ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6321", ВертикальныеОбласти);
			КонецЕсли;
			Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6322") И ПоказательВключенВОтчет(СоставОтчета, "Строка6322") Тогда
				ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6322", ВертикальныеОбласти);
			КонецЕсли;
			Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6323") И ПоказательВключенВОтчет(СоставОтчета, "Строка6323") Тогда
				ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6323", ВертикальныеОбласти);
			КонецЕсли;
			Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6324") И ПоказательВключенВОтчет(СоставОтчета, "Строка6324") Тогда
				ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6324", ВертикальныеОбласти);
			КонецЕсли;
			Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6325") И ПоказательВключенВОтчет(СоставОтчета, "Строка6325") Тогда
				ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6325", ВертикальныеОбласти);
			КонецЕсли;
			Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6326") И ПоказательВключенВОтчет(СоставОтчета, "Строка6326") Тогда
				ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6326", ВертикальныеОбласти);
			КонецЕсли;
		КонецЕсли;
		
		Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6330") И ПоказательВключенВОтчет(СоставОтчета, "Строка6330") Тогда
			ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6330", ВертикальныеОбласти);
		КонецЕсли;
		Если ОбластьОтчетаЗаполнена(ИсходныйТабличныйДокумент, "ОсновнаяЧасть6350") И ПоказательВключенВОтчет(СоставОтчета, "Строка6350") Тогда
			ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6350", ВертикальныеОбласти);
		КонецЕсли;
		Если ПоказательВключенВОтчет(СоставОтчета, "Строка6300") Тогда
			ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6300", ВертикальныеОбласти);
		КонецЕсли;
	КонецЕсли;
	
	ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ОсновнаяЧасть6400", ВертикальныеОбласти);
	ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "ПустаяСтрока");
	ВывестиОбластьМакета(ПечатнаяФорма, ИсходныйТабличныйДокумент, "Ограничители");
	УдалитьНенужноеИзПечатнойФормы(ПечатнаяФорма);
	СписокПечатныхФорм.Добавить(ПечатнаяФорма);
...
Функция Загрузить(ИмяСохраняемогоПараметра)
	...
	Возврат ХранилищеНастроекДанныхФорм.Загрузить("Отчет.РегламентированныйОтчетБухОтчетностьМП.Форма.УведомлениеОбИзменениях", ИмяСохраняемогоПараметра);
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСписковВыбора = ОбъектОтчета(ИмяФормы).ПолучитьМакет("СпискиВыбора2012Кв4_2");
...
Процедура СформироватьСоставПоказателей()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ИмяФормы).ПолучитьМакет(ИмяМакетаСоставаПоказателей());
	...
	Для Каждого Отчет Из МакетСоставаПоказателей.Области Цикл
		...
		Для Ном = Отчет.Верх По Отчет.Низ Цикл
			...
			Если ИмяСтраницы = "ОЦИПС" И чВариантЗаполнения = 3 Тогда
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма, ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если ВПрограммеИзмененаДатаПодписи Тогда
		...
		КонецЕсли;
		Если Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < '2012-12-01' И НЕ Форма.ТабличныйДокумент.Области.Найти("ОргБухгалтер") = Неопределено Тогда
	...
	Если Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < '2012-12-01' И НЕ Форма.ТабличныйДокумент.Области.Найти("ОргБухгалтер") = Неопределено И ((Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) ИЛИ (ПустаяСтрока(Форма.ТабличныйДокумент.Области.ОргБухгалтер.Значение))) Тогда
		Форма.ТабличныйДокумент.Области.ОргБухгалтер.Значение = ФИОБух;
	КонецЕсли;
...
Функция ИмяМакетаСоставаПоказателей()
	...
	Возврат "СоставПоказателей2012Кв4_2";
...
Процедура ВосстановитьСохраненныеНастройкиСоставаПоказателей(ИмяРаздела, НастройкиРаздела, СписокНастроек)
	...
	Для Каждого СтрокаНастройки Из НастройкиРаздела.Строки Цикл
		...
		СтрокаНастроекОтчета = РегламентированнаяОтчетностьКлиентСервер.НайтиЭлементВДанныхФормыДерево(ЭтотОбъект[НастройкиСоставаПоказателейОтчета[ИмяРаздела]].ПолучитьЭлементы(), "Идентификатор", Идентификатор);
...
Функция НазваниеОЦИПС()
	...
	Иначе
		Возврат "Отчет о целевом"+Символы.ПС+"использовании полученных"+Символы.ПС+"средств";
	КонецЕсли;
...
Процедура ОчиститьНаСервере(НаимТекущегоРаздела = "", ОчищатьНомерКорректировки = Истина)
	...
	Если НЕ ЗначениеЗаполнено(НаимТекущегоРаздела)
	 ИЛИ НаимТекущегоРаздела = "ОЦИПС" Тогда
		...
		СброситьНастройкиСоставаПоказателей("ОЦИПС");
	КонецЕсли;
...
Функция СформироватьСтруктуруПоказателей()
	...
	Если ПустаяСтрока(ПоказателиОтчета.ПолеТабличногоДокументаБаланс.ОргПочтАдрес) Тогда
		СтруктураРеквизитовФормы.мАдресДляВыгрузки = "";
	ИначеЕсли НЕ СохраненныеДанные.Свойство("Адрес9Запятых", АдресДляВыгрузки) ИЛИ НЕ ЗначениеЗаполнено(АдресДляВыгрузки) Тогда
		Адреса = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(СтруктураРеквизитовФормы.Организация, ДатаПодписи, "АдрФакт, АдрЮР, АдрПочт");
		Если ПустаяСтрока(СтрЗаменить(Адреса.АдрФакт, ",", "")) Тогда
			АдрФакт = "";
		Иначе
	...
	мДанныеОтчета.Вставить("СтруктураМногострочныхЧастей", СохраненныеДанные.СтруктураМногострочныхЧастей);
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ПрефиксИмениФайлаВыгрузки", "NO_BUHMP");
	...
	СписокОКОПФ = "85,20101,20102,20103,20104,20105,20106,20107,20109,20110,20111,20112,20113,20114,20115,21002";
	...
	ПараметрыВыгрузки.Вставить("ВключитьОЦИПС", РазделВключенВОтчет("ОЦИПС", ДеревоНастройкиСтраниц));
	...
	Для Каждого РазделОтчета Из НастройкиСоставаПоказателейОтчета Цикл
		НастройкиРаздела = ЭтотОбъект[РазделОтчета.Значение];
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.Период) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Выгрузка за указанный период не предусмотрена по формату представления в электронной форме!", "Баланс", "ДатаБаланса");
		СтатусПроверки = Ложь;
		Возврат СтатусПроверки;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.КодИФНС) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "В справочнике ""Организации"" для текущей организации не задан код ИФНС, необходимый для формирования имени файла выгрузки!", "Баланс");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.КППОрг) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "В справочнике ""Организации"" для текущей организации не задан КПП, необходимый для формирования корректного файла выгрузки!", "Баланс");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.НаимОрг) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На указано полное наименование организации!", "Баланс", "ОргНазв");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ИННОрг) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан ИНН организации!", "Баланс", "ОргИНН");
		СтатусПроверки = Ложь;
	ИначеЕсли НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ПараметрыВыгрузки.ИННОрг, Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Задан некорректный ИНН юридического лица!", "Баланс", "ОргИНН");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПараметрыВыгрузки.Свойство("ПрПодп") Тогда
		// Формат выгрузки 5.03.
		Если ПараметрыВыгрузки.ПрПодп = "1" Тогда
			// Руководитель организации.
			Если ПустаяСтрока(ПараметрыВыгрузки.ПодпФамилия) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не удалось определить фамилию руководителя!", "Баланс", "ОргДиректор");
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если ПустаяСтрока(ПараметрыВыгрузки.ПодпИмя) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не удалось определить имя руководителя!", "Баланс", "ОргДиректор");
				СтатусПроверки = Ложь;
			КонецЕсли;
		...
		Иначе
			...
			Если ПустаяСтрока(ПараметрыВыгрузки.ПодпФамилия) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не удалось определить фамилию уполномоченного представителя!", "Баланс", "ОргДиректор");
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если ПустаяСтрока(ПараметрыВыгрузки.ПодпИмя) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не удалось определить имя уполномоченного представителя!", "Баланс", "ОргДиректор");
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если ПустаяСтрока(ПараметрыВыгрузки.ПодпНаимДок) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задано наименование документа, удостоверяющего полномочия представителя!", "Баланс", "");
				СтатусПроверки = Ложь;
			КонецЕсли;
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ПараметрыВыгрузки.КодПодписантаБух) Тогда
			Если ПустаяСтрока(ПараметрыВыгрузки.ФамилияБух) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не удалось определить фамилию главного бухгалтера!", "Баланс", "ОргБухгалтер");
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если ПустаяСтрока(ПараметрыВыгрузки.ИмяБух) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не удалось определить имя главного бухгалтера!", "Баланс", "ОргБухгалтер");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ПараметрыВыгрузки.ОКПО) И (СтрДлина(ПараметрыВыгрузки.ОКПО) < 8 ИЛИ СтрДлина(ПараметрыВыгрузки.ОКПО) > 10 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПараметрыВыгрузки.ОКПО)) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На титульном листе задан некорректный код по ОКПО!", "Баланс", "ОргКодОКПО");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКВЭД) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указано значение кода вида деятельности по ОКВЭД!", "Баланс", "ОргКодОКВЭД");
		СтатусПроверки = Ложь;
	ИначеЕсли НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(ПараметрыВыгрузки.ОКВЭД) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Указано некорректное значение кода вида деятельности по ОКВЭД! Допустимы значения, соответствующие следующим шаблонам: ""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).!", "Баланс", "ОргКодОКВЭД");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКОПФ) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан код по ОКОПФ!", "Баланс", "ОргКодОКОПФ");
		СтатусПроверки = Ложь;
	ИначеЕсли (СтрДлина(ПараметрыВыгрузки.ОКОПФ) <> 2 И СтрДлина(ПараметрыВыгрузки.ОКОПФ) <> 5) ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПараметрыВыгрузки.ОКОПФ) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Задан некорректный код по ОКОПФ!", "Баланс", "ОргКодОКОПФ");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКФС) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан код по ОКФС!", "Баланс", "ОргКодОКФС");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.ОКФС) <> 2 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПараметрыВыгрузки.ОКФС) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Задан некорректный код по ОКФС!", "Баланс", "ОргКодОКФС");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКЕИ) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан код по ОКЕИ!", "Баланс", "КодЕИ");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.ОКЕИ) <> 3 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПараметрыВыгрузки.ОКЕИ) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Задан некорректный код по ОКЕИ!", "Баланс", "КодЕИ");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.АдрКодРегион) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Требуется указание как минимум кода региона в адресе!", "Баланс", "ОргПочтАдрес");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если СтрДлина(СокрЛП(мДанныеОтчета.ПолеТабличногоДокументаБаланс.П000010000702)) <> 4 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указан код строки ""Капитал и резервы"" бухгалтерского баланса", "Баланс", "П000010000702");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	// Коды строк в отчете о прибылях и убытках.
...
Процедура ПроверитьКодСтрокиБаланс(СтатусПроверки, ИмяПоказателя, НаименованиеСтроки)
	...
	Если ЗначениеЗаполнено(КодСтроки) ИЛИ ЗначениеЗаполнено(Графа03) ИЛИ ЗначениеЗаполнено(Графа04) ИЛИ ЗначениеЗаполнено(Графа05) Тогда
		Если ПустаяСтрока(КодСтроки) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				"Не указан код строки """ + НаименованиеСтроки + """ бухгалтерского баланса",
				"Баланс", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(КодСтроки) <> 4 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				"Неверно указан код строки """ + НаименованиеСтроки + """ бухгалтерского баланса",
				"Баланс", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьКодСтрокиОПУ(СтатусПроверки, ИмяПоказателя, НаименованиеСтроки)
	...
	Если ЗначениеЗаполнено(КодСтроки) ИЛИ ЗначениеЗаполнено(Графа03) ИЛИ ЗначениеЗаполнено(Графа04) Тогда
		Если ПустаяСтрока(КодСтроки) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				"Не указан код строки """ + НаименованиеСтроки + """ отчета о прибылях и убытках",
				"ОПУ", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(КодСтроки) <> 4 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				"Неверно указан код строки """ + НаименованиеСтроки + """ отчета о прибылях и убытках",
				"ОПУ", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьУсловныеЭлементы(Знач ПараметрыВыгрузки, Узел)
	...
	Для Инд = 1 По КоличествоСтрок Цикл
		...
		Если НЕ ПустаяСтрока(ТекСтр.Условие) Тогда
			Если НЕ РегламентированнаяОтчетность.УсловиеВыполнено(ЭтотОбъект, ПараметрыВыгрузки, ТекСтр.Условие, "Баланс") Тогда
...
Процедура ИнициализироватьНастройкиСоставаПоказателейОтчетов()
	...
	ТекстXML = ОбъектОтчета(ИмяФормы).ПолучитьМакет(ИмяМакетаНастройкиСоставаОтчетов()).ПолучитьТекст();
...
Функция ИмяМакетаНастройкиСоставаОтчетов()
	...
	Возврат "НастройкиСоставаОтчетов2012Кв4_2";
...
Процедура НастройкаСоставаОтчета(УзелРаздела)
	...
	Для Каждого Узел Из УзелРаздела.ДочерниеУзлы Цикл
		...
		Если ИмяУзла = "Показатель" Тогда
			РазобратьНастройкуПоказателя(Узел, ЭтотОбъект["Настройки" + НаименованиеРаздела]);
		КонецЕсли;
	КонецЦикла;
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Для Каждого Элемент Из мДеревоВыбранныхСтраниц.ПолучитьЭлементы() Цикл
		...
		Если Элемент.ПоказатьСтраницу Тогда
			...
			Если Элемент.ИмяСтраницы = "Баланс" Тогда
	...
	Контейнер.Вставить("ОЦИПС",  СтруктураДанныхОЦИПС);
