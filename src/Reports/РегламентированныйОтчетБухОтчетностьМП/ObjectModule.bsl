...
Функция СтруктураСтраницОтчета(ИмяФормыОтчета, СтруктураРеквизитовФормы, ДатаПодписи) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Недопустимое имя формы при вызове процедуры СтруктураСтраницОтчета()';
								|en = 'Недопустимое имя формы при вызове процедуры СтруктураСтраницОтчета()'");
	КонецЕсли;
...
Функция ПолучитьСписокВычитаемыхПоказателей(ИмяФормы) Экспорт
	...
	МассивВычитаемыхДопСтрок = СтрРазделить(СписокВычитаемыхСтрок, ",", Ложь);
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.КППОрг) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
			НСтр("ru = 'В справочнике ""Организации"" для текущей организации не задан КПП, необходимый для формирования корректного файла выгрузки';
				|en = 'В справочнике ""Организации"" для текущей организации не задан КПП, необходимый для формирования корректного файла выгрузки'"),
			"Баланс");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	ИначеЕсли НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
			НСтр("ru = 'Задан некорректный ИНН юридического лица';
				|en = 'Задан некорректный ИНН юридического лица'"),
			"Баланс", "ОргИНН");
		СтатусПроверки = Ложь;
	КонецЕсли;
