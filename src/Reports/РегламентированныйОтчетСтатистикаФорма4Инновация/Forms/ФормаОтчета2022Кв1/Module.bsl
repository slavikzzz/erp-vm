...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0001301235";
...
Процедура ДоступностьРазделов()
	...
	Если ЧетныйГод(Прав(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета),1)) Тогда
		
		Для НомСтр = 1101 По 1117 Цикл
			ПолеТабличногоДокументаФормаОтчета.Области["П000110" + Формат(НомСтр, "ЧГ=") + "03"].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области["П000110" + Формат(НомСтр, "ЧГ=") + "03"].Защита = Истина;
			ПолеТабличногоДокументаФормаОтчета.Области["П000110" + Формат(НомСтр, "ЧГ=") + "03"].Значение = "";
		КонецЦикла;
		
		ПолеТабличногоДокументаФормаОтчета.Области["П000120121103"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П000120121103"].Защита = Истина;
		ПолеТабличногоДокументаФормаОтчета.Области["П000120121103"].Значение = "";
		
		ПолеТабличногоДокументаФормаОтчета.Области["П000120122203"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П000120122203"].Защита = Истина;
		ПолеТабличногоДокументаФормаОтчета.Области["П000120122203"].Значение = "";
		
		Для Н = 1 По 2 Цикл
			НомСтр = ?(Н = 1, 216, 232);
			ПолеТабличногоДокументаФормаОтчета.Области["П0001201" + НомСтр + "03"].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области["П0001201" + НомСтр + "03"].Защита = Истина;
			ПолеТабличногоДокументаФормаОтчета.Области["П0001201" + НомСтр + "03"].Значение = "X";
		КонецЦикла;
	Иначе
		Для НомСтр = 801 По 822 Цикл
			ПолеТабличногоДокументаФормаОтчета.Области["П0000800" + НомСтр + "03"].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области["П0000800" + НомСтр + "03"].Защита = Истина;
			ПолеТабличногоДокументаФормаОтчета.Области["П0000800" + НомСтр + "03"].Значение = "";
		КонецЦикла;
		
		Для Гр = 3 По 8 Цикл
			Для НомСтр = 1001 По 1007 Цикл
				ИмяЯчейки = "П000100" + Формат(НомСтр, "ЧГ=") + Формат(Гр, "ЧЦ=2; ЧВН=");
		...
		Если ПолеТабличногоДокументаФормаОтчета.Области["П000020021003"].Значение = 1 Тогда
			ЦветРаздела16             = Новый Цвет(255, 255, 192);
			ЗащитаРаздела16           = Ложь;
			ОчиститьЗначениеРаздела16 = Ложь;
		Иначе
			ЦветРаздела16             = Новый Цвет();
			ЗащитаРаздела16           = Истина;
			ОчиститьЗначениеРаздела16 = Истина;
		КонецЕсли;
		Для НомСтр = 1101 По 1117 Цикл
			ПолеТабличногоДокументаФормаОтчета.Области["П000110" + Формат(НомСтр, "ЧГ=") + "03"].ЦветФона = ЦветРаздела16;
			ПолеТабличногоДокументаФормаОтчета.Области["П000110" + Формат(НомСтр, "ЧГ=") + "03"].Защита = ЗащитаРаздела16;
			Если ОчиститьЗначениеРаздела16 Тогда
				ПолеТабличногоДокументаФормаОтчета.Области["П000110" + Формат(НомСтр, "ЧГ=") + "03"].Значение = "";
			КонецЕсли;
		КонецЦикла;
		
		МассивСтрок =
			СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("208;210;219;221", ";");
		Для Каждого ЭлМасс Из МассивСтрок Цикл
			ИмяЯчейки = "П0001201" + ЭлМасс + "03";
			ПолеТабличногоДокументаФормаОтчета.Области[ИмяЯчейки].СодержитЗначение = Ложь;
			ПолеТабличногоДокументаФормаОтчета.Области[ИмяЯчейки].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области[ИмяЯчейки].Защита = Истина;
			ПолеТабличногоДокументаФормаОтчета.Области[ИмяЯчейки].Текст = "Х";
		КонецЦикла;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		ИмяПарам = Область.Имя;
		
		Если ОбластьДляСправочникаДаНет(ИмяПарам) Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникДаНет");
		ИначеЕсли Область.Имя = "П000010010201" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникГруппаСтранГоловнойОрганизации");
		ИначеЕсли ОбластьДляСправочникаВзаимодействие(ИмяПарам) Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникВзаимодействие");
		ИначеЕсли Область.Имя = "П000010011201" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникНаиболееВажныйРынокСбыта");
		ИначеЕсли Область.Имя = "П000010011801" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникЧастотаРабот");
		ИначеЕсли СтрНайти(Область.Имя, "П0000200211") = 1 Или СтрНайти(Область.Имя, "П0000200212") = 1 Тогда
			СтандартнаяОбработка = Ложь;
			ЗначСтарое = Область.Значение;
			ИмяОбласти = Лев(Область.Имя, 11);
			
			Для Гр = 3 По 6 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области[ИмяОбласти + Формат(Гр,"ЧЦ=2; ЧВН=")].Очистить();
			КонецЦикла;
			
			Если Не ЗначениеЗаполнено(ЗначСтарое) Тогда
				Если Сред(Область.Имя, 13) = "3" Тогда
					Область.Значение = 110;
				ИначеЕсли Сред(Область.Имя, 13) = "4" Тогда
					Область.Значение = 111;
				ИначеЕсли Сред(Область.Имя, 13) = "5" Тогда
					Область.Значение = 135;
				ИначеЕсли Сред(Область.Имя, 13) = "6" Тогда
					Область.Значение = 112;
				КонецЕсли;
			КонецЕсли;
			Модифицированность = Истина;
			Возврат;
		ИначеЕсли СтрНайти(Область.Имя, "П00004004") = 1 И Число(Сред(Область.Имя, 10, 2)) <= 20 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтаФорма, "СправочникФакторыПрепятствующие");
		ИначеЕсли СтрНайти(Область.Имя, "П00006006") = 1 И Число(Сред(Область.Имя, 10, 2)) <= 24 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтаФорма, "СправочникВоздействиеИнновации");
		ИначеЕсли СтрНайти(Область.Имя, "П00008008") = 1 Или СтрНайти(Область.Имя, "П00009009") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтаФорма, "СправочникЗначимость");
		ИначеЕсли СтрНайти(Область.Имя, "П00012012") = 1 Тогда
			Если Число(Сред(Область.Имя, 10, 2)) <= 16 Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
					ЭтаФорма, "СправочникЯсностьВопросов");
			ИначеЕсли Число(Сред(Область.Имя, 10, 2)) >= 17 И Число(Сред(Область.Имя, 10, 2)) <= 32 Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтаФорма, "СправочникТочностьДанных");
			КонецЕсли;
		КонецЕсли;
...
&НаСервере
Функция ОбластьДляСправочникаДаНет(ИмяПарам)
	
	Возврат ИмяПарам = "П000010010101"
		Или ИмяПарам = "П000020020103"
		Или ИмяПарам = "П000020020203"
		Или ИмяПарам = "П000020020303"
		Или ИмяПарам = "П000020020403"
		Или ИмяПарам = "П000020020503"
		Или ИмяПарам = "П000020020603"
		Или ИмяПарам = "П000020020703"
		Или ИмяПарам = "П000020020803"
		Или ИмяПарам = "П000020020903"
		Или ИмяПарам = "П000020021003"
		Или	ИмяПарам = "П000030031003"
		Или ИмяПарам = "П000030031103"
		Или ИмяПарам = "П000030031203"
		Или ИмяПарам = "П000060062501"
		Или (СтрНайти(ИмяПарам, "П00011011") = 1 И Число(Сред(ИмяПарам, 10, 2)) <= 17)
		Или (СтрНайти(ИмяПарам, "П00012012") = 1 И Число(Сред(ИмяПарам, 10, 2)) >= 33 И Число(Сред(ИмяПарам, 10, 2)) <= 41);
	
КонецФункции

&НаСервере
Функция ОбластьДляСправочникаВзаимодействие(ИмяПарам)
	
	Возврат ИмяПарам = "П000010010301"
		Или ИмяПарам = "П000010010401"
		Или ИмяПарам = "П000010010501"
		Или ИмяПарам = "П000010010601"
		Или ИмяПарам = "П000010010701"
		Или ИмяПарам = "П000010010801"
		Или ИмяПарам = "П000010010901"
		Или ИмяПарам = "П000010011001";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Области["П000030030203"].Значение = Области["П000030030303"].Значение + Области["П000030030403"].Значение;
	Области["П000030030204"].Значение = Области["П000030030304"].Значение + Области["П000030030404"].Значение;
	Области["П000030030205"].Значение = Области["П000030030305"].Значение + Области["П000030030405"].Значение;
	Области["П000030030206"].Значение = Области["П000030030306"].Значение;
	Области["П000030030207"].Значение = Области["П000030030307"].Значение;
	
	Для НомКол = 3 По 4 Цикл
		СуммаПоКол = 0;
		Для НомСтр = 2 По 11 Цикл
			СуммаПоКол = СуммаПоКол
				+ Области["П00005005" + Формат(НомСтр, "ЧЦ=2; ЧВН=") + Формат(НомКол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00005005010" + НомКол].Значение = СуммаПоКол;
	КонецЦикла;
	
	Для НомКол = 3 По 8 Цикл
		СуммаПоКол = 0;
		Для НомСтр = 1002 По 1007 Цикл
			СуммаПоКол = СуммаПоКол
				+ Области["П000100" + Формат(НомСтр, "ЧЦ=4; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00010010010" + НомКол].Значение = СуммаПоКол;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 3 По 7 Цикл
		мФормаОтчета.Области["П00003003020" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00003003020" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	мФормаОтчета.Области["П000050050103"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000050050103"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000050050104"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000050050104"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	Для Гр = 3 По 8 Цикл
		мФормаОтчета.Области["П00010010010" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00010010010" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	П0000500526.Очистить();
	П0001301235.Очистить();
	РегламентированнаяОтчетность.НачальныеОперацииСМногострочнымиБлоками(ЭтотОбъект);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для НомРаздел = 1 По 11 Цикл
		
		Если НомРаздел = 5 Тогда
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел5_1"));
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел5_2"));
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел5_3"));
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма № 4-инновация");
			
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел5_2"));
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел5_4"));
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма № 4-инновация");
			
			Продолжить;
		ИначеЕсли НомРаздел = 11 Тогда
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел11_1"));
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел11_2"));
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел11_3"));
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма № 4-инновация");
			
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ПустаяСтрока"));
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел11_2"));
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел11_4"));
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма № 4-инновация");
			Продолжить;
		КонецЕсли;
		
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел" + НомРаздел));
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма № 4-инновация");
	КонецЦикла;
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел12_1"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел12_2"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел12_3"));
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.Раздел12_3.Низ + 1;
	...
	Если ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("Раздел12_4")) Тогда
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел12_4"));
	Иначе
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма № 4-инновация");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел12_4"));
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей =
		ОбъектОтчета(ИмяФормы).ПолучитьМакет(СтруктураРеквизитовФормы["ИмяМакетаСписков"]);
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникГруппаСтранГоловнойОрганизации",
	512, , "Выбор кода", КоллекцияСписковВыбора["ГруппаСтранГоловнойОрганизации"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникВзаимодействие",
	512, , "Выбор кода", КоллекцияСписковВыбора["Взаимодействие"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникНаиболееВажныйРынокСбыта",
	512, , "Выбор кода", КоллекцияСписковВыбора["НаиболееВажныйРынокСбыта"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникЧастотаРабот",
	512, , "Выбор кода", КоллекцияСписковВыбора["ЧастотаРабот"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникФакторыПрепятствующие",
	512, , "Выбор оценочного кода", КоллекцияСписковВыбора["ФакторыПрепятствующие"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникВоздействиеИнновации",
	512, , "Выбор оценочного кода", КоллекцияСписковВыбора["ВоздействиеИнновации"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникЗначимость",
	512, , "Выбор оценочного кода", КоллекцияСписковВыбора["Значимость"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникЯсностьВопросов",
	512, , "Выбор оценочного кода", КоллекцияСписковВыбора["ЯсностьВопросов"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникТочностьДанных",
	512, , "Выбор оценочного кода", КоллекцияСписковВыбора["ТочностьДанных"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникРазработчикИнноваций",
	512, , "Выбор", КоллекцияСписковВыбора["РазработчикИнноваций"]);
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П000010010201"].Значение) Тогда
		СтруктураПоиска = Новый Структура("Код", Строка(ПолеТабличногоДокументаФормаОтчета.Области["П000010010201"].Значение));
		МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
			"СправочникГруппаСтранГоловнойОрганизации").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
		Если МассивВыбора.Количество() = 1 Тогда
			ПараметрыВыгрузки.Вставить("П000010010201", МассивВыбора[0].Ключ);
		КонецЕсли;
	КонецЕсли;
	
	Для Каждого Параметр Из ПараметрыВыгрузки Цикл
		Если ОбластьДляСправочникаВзаимодействие(Параметр.Ключ) И ЗначениеЗаполнено(Параметр.Значение) Тогда
			СтруктураПоиска = Новый Структура("Код", Строка(Параметр.Значение));
			МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
				"СправочникВзаимодействие").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
			Если МассивВыбора.Количество() = 1 Тогда
				ПараметрыВыгрузки.Вставить(Параметр.Ключ, МассивВыбора[0].Ключ);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П000010011201"].Значение) Тогда
		СтруктураПоиска = Новый Структура("Код", Строка(ПолеТабличногоДокументаФормаОтчета.Области["П000010011201"].Значение));
		МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
			"СправочникНаиболееВажныйРынокСбыта").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
		Если МассивВыбора.Количество() = 1 Тогда
			ПараметрыВыгрузки.Вставить("П000010011201", МассивВыбора[0].Ключ);
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П000010011801"].Значение) Тогда
		СтруктураПоиска = Новый Структура("Код", Строка(ПолеТабличногоДокументаФормаОтчета.Области["П000010011801"].Значение));
		МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
			"СправочникЧастотаРабот").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
		Если МассивВыбора.Количество() = 1 Тогда
			ПараметрыВыгрузки.Вставить("П000010011801", МассивВыбора[0].Ключ);
		КонецЕсли;
	КонецЕсли;
	
	Для Стр = 211 По 212 Цикл
		ИмяПоказателя = "";
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0000200" + Стр + "03"].Значение) Тогда
			ИмяПоказателя = "П0000200" + Стр + "03";
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0000200" + Стр + "04"].Значение) Тогда
			ИмяПоказателя = "П0000200" + Стр + "04";
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0000200" + Стр + "05"].Значение) Тогда
			ИмяПоказателя = "П0000200" + Стр + "05";
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0000200" + Стр + "06"].Значение) Тогда
			ИмяПоказателя = "П0000200" + Стр + "06";
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ИмяПоказателя) Тогда
			СтруктураПоиска = Новый Структура("Код", Строка(ПолеТабличногоДокументаФормаОтчета.Области[ИмяПоказателя].Значение));
			МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
				"СправочникРазработчикИнноваций").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
			Если МассивВыбора.Количество() = 1 Тогда
				ПараметрыВыгрузки.Вставить(ИмяПоказателя, МассивВыбора[0].Ключ);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Для Стр = 401 По 420 Цикл
		ИмяПоказателя = "П0000400" + Стр + "03";
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области[ИмяПоказателя].Значение) Тогда
			СтруктураПоиска = Новый Структура("Код", Строка(ПолеТабличногоДокументаФормаОтчета.Области[ИмяПоказателя].Значение));
			МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
				"СправочникФакторыПрепятствующие").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
			Если МассивВыбора.Количество() = 1 Тогда
				ПараметрыВыгрузки.Вставить(ИмяПоказателя, МассивВыбора[0].Ключ);
			КонецЕсли;
		КонецЕсли;
	...
	Для Стр = 601 По 624 Цикл
		ИмяПоказателя = "П0000600" + Стр + "03";
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области[ИмяПоказателя].Значение) Тогда
			СтруктураПоиска = Новый Структура("Код", Строка(ПолеТабличногоДокументаФормаОтчета.Области[ИмяПоказателя].Значение));
			МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
				"СправочникВоздействиеИнновации").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
			Если МассивВыбора.Количество() = 1 Тогда
				ПараметрыВыгрузки.Вставить(ИмяПоказателя, МассивВыбора[0].Ключ);
			КонецЕсли;
		КонецЕсли;
	...
	Для Н = 1 По 2 Цикл
		...
		Для Стр = НомСтрНач По НомСтрКон Цикл
			Если Н = 1 Тогда
				ИмяПоказателя = "П0000800" + Стр + "03";
			Иначе
				ИмяПоказателя = "П0000900" + Стр + "03";
			КонецЕсли;
			Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области[ИмяПоказателя].Значение) Тогда
				СтруктураПоиска = Новый Структура("Код", Строка(ПолеТабличногоДокументаФормаОтчета.Области[ИмяПоказателя].Значение));
				МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
					"СправочникЗначимость").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
				Если МассивВыбора.Количество() = 1 Тогда
					ПараметрыВыгрузки.Вставить(ИмяПоказателя, МассивВыбора[0].Ключ);
				КонецЕсли;
			КонецЕсли;
	...
	Для Стр = 1201 По 1211 Цикл
		ИмяПоказателя = "П000120" + Формат(Стр, "ЧГ=") + "03";
		Если ПолеТабличногоДокументаФормаОтчета.Области[ИмяПоказателя].СодержитЗначение
		   И ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области[ИмяПоказателя].Значение) Тогда
			СтруктураПоиска = Новый Структура("Код", Строка(ПолеТабличногоДокументаФормаОтчета.Области[ИмяПоказателя].Значение));
			МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
				"СправочникЯсностьВопросов").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
			Если МассивВыбора.Количество() = 1 Тогда
				ПараметрыВыгрузки.Вставить(ИмяПоказателя, МассивВыбора[0].Ключ);
			КонецЕсли;
		КонецЕсли;
	...
	Для Стр = 1212 По 1222 Цикл
		ИмяПоказателя = "П000120" + Формат(Стр, "ЧГ=") + "03";
		Если ПолеТабличногоДокументаФормаОтчета.Области[ИмяПоказателя].СодержитЗначение
		   И ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области[ИмяПоказателя].Значение) Тогда
			СтруктураПоиска = Новый Структура("Код", Строка(ПолеТабличногоДокументаФормаОтчета.Области[ИмяПоказателя].Значение));
			МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
				"СправочникТочностьДанных").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
			Если МассивВыбора.Количество() = 1 Тогда
				ПараметрыВыгрузки.Вставить(ИмяПоказателя, МассивВыбора[0].Ключ);
			КонецЕсли;
		КонецЕсли;
	...
	Пока Истина Цикл
		Инд = Инд + 1;
		Обл = ПолеТабличногоДокументаФормаОтчета.Области.Найти("П000130123502_" + Инд);
		Если Обл = Неопределено Тогда
			Прервать;
		КонецЕсли;
		Если ЗначениеЗаполнено(Обл.Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД00130123503_" + Инд, "1");
		КонецЕсли;
	КонецЦикла;
