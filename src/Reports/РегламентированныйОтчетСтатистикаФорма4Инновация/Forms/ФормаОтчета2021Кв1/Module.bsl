...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0000500526";
	СтруктураРеквизитовФормы.мИдГруппы2 = "П0001301235";
	...
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы1, 1);
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы2, 2);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		ИначеЕсли СтрНайти(Область.Имя, "П0000200211") = 1 Или СтрНайти(Область.Имя, "П0000200212") = 1 Тогда
			...
			Для Гр = 3 По 6 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области[ИмяОбласти + Формат(Гр,"ЧЦ=2; ЧВН=")].Значение = Неопределено;
			КонецЦикла;
		...
		ИначеЕсли СтрНайти(Область.Имя, "П00012012") = 1 Тогда
			Если Число(Сред(Область.Имя, 10, 2)) <= 11 Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
					ЭтаФорма, "СправочникЯсностьВопросов");
			ИначеЕсли Число(Сред(Область.Имя, 10, 2)) >= 12 И Число(Сред(Область.Имя, 10, 2)) <= 22 Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
...
Функция ОбластьДляСправочникаДаНет(ИмяПарам)
	...
	Возврат ИмяПарам = "П000010010101"
		...
		Или (СтрНайти(ИмяПарам, "П00012012") = 1 И Число(Сред(ИмяПарам, 10, 2)) >= 23 И Число(Сред(ИмяПарам, 10, 2)) <= 31);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для НомРаздел = 1 По 12 Цикл
		...
		Если НомРаздел = 5 Тогда
			...
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел5_5"));
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел5_6"));
			ВерхМногострочнойЧасти = ТабличныйДокумент.Области.Раздел5_6.Низ + 1;
	...
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел13_1"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел13_2"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел13_3"));
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.Раздел13_3.Низ + 1;
	...
	Если ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("Раздел13_4")) Тогда
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел13_4"));
	Иначе
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма № 4-инновация");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел13_4"));
