...
Процедура ДоступностьРазделов()
	...
	Иначе
		...
		ПолеТабличногоДокументаФормаОтчета.Области["П000120120803"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П000120120803"].Защита = Истина;
		
		ПолеТабличногоДокументаФормаОтчета.Области["П000120121903"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П000120121903"].Защита = Истина;
	КонецЕсли;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		ИначеЕсли СтрНайти(Область.Имя, "П0000200211") = 1 Или СтрНайти(Область.Имя, "П0000200212") = 1 Тогда
			...
			Модифицированность = Истина;
			Возврат;
		ИначеЕсли СтрНайти(Область.Имя, "П00004004") = 1 И Число(Сред(Область.Имя, 10, 2)) <= 19 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
...
Процедура СформироватьСпискиВыбора()
	...
	512, , "Выбор", КоллекцияСписковВыбора["ДаНет"]);
