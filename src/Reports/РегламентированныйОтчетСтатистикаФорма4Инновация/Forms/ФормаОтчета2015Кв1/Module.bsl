...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "06/10/2015";
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = "Разделы 4-7";
	СтрокаУровня1.ОриентацияСтраницы       = "Ландшафт";
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = 1;
	СтрокаУровня1.ВыгрузитьСтраницу        = 2;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2015Кв1_Титульный", "ОсновнаяЧасть", СтруктураДанныхТитульный);
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-инновация");
	
	Для Ном = 1 По 3 Цикл
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2015Кв1_Разделы1_3", "Раздел"+Ном, СтруктураДанныхРазделы1_3);
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-инновация");
	КонецЦикла;
	
	Для Ном = 0 По 2 Цикл
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2015Кв1_Разделы5_7", "Раздел5_"+Ном, СтруктураДанныхРазделы5_7);
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-инновация");
	КонецЦикла;
	
	Для Ном = 6 По 7 Цикл
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2015Кв1_Разделы5_7", "Раздел"+Ном, СтруктураДанныхРазделы5_7);
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-инновация");
	КонецЦикла;
	
	Для Ном = 9 По 10 Цикл
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2015Кв1_Разделы8_10", "Раздел"+Ном, СтруктураДанныхРазделы8_10);
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-инновация");
	КонецЦикла;
	
	Для Ном = 11 По 12 Цикл
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2015Кв1_Разделы11_12", "Раздел"+Ном, СтруктураДанныхРазделы11_12);
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-инновация");
	КонецЦикла;
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2015Кв1_Разделы11_12", "Раздел12_Доп", СтруктураДанныхРазделы11_12);
	ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2015Кв1_Подписи", "Подписи", СтруктураДанныхПодписи);
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-инновация");
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтруктураРеквизитовФормы.мАвтоВыборКодов = Ложь Тогда 
		...
		ИначеЕсли СтрНайти(Область.Имя, "П0808")= 1
			Или (СтрНайти(Область.Имя, "П0909")= 1 И СтрДлина(Область.Имя) = 7 И Область.Имя <> "П090913") Тогда
