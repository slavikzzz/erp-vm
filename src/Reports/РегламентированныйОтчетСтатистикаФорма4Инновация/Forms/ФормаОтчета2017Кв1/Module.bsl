...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураДанныхРазделы1_3 = Новый Структура;
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1 = мДеревоСтраницОтчета.ПолучитьЭлементы().Добавить();
	СтрокаУровня1.ИмяСтраницы              = "Разделы1_3";
	...
	СтрокаУровня1.Представление            = "Разделы 9-10";
	СтрокаУровня1.ОриентацияСтраницы       = "Ландшафт";
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = 1;
	СтрокаУровня1.ВыгрузитьСтраницу        = 2;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ЧастотаРабот", 512, , "Выбор кода", КоллекцияСписковВыбора["work_freq"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "Стр110", 512, , "Выбор кода", КоллекцияСписковВыбора["NACERev2"]);
...
Процедура СохранитьНаКлиентеСВопросом(Автосохранение, Вариант, ВыполняемоеОповещение, ОткрытьФормуВопросаПриИзмененииВидаДокументаНомераКорректировки, ПодобныйОтчетСуществует)
	...
	Если ПодобныйОтчетСуществует Тогда
		...
		ТекстВопроса = НСтр("ru = 'Внимание! Подобный отчет уже существует! Записать новый?';
							|en = 'Внимание! Подобный отчет уже существует! Записать новый?'");
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Если ИмяТекущейСтраницыПанели = "Разделы1_3" Тогда
		...
		ОбластиТабДок["П030309_6"].Значение = ОбластиТабДок["П030310_6"].Значение;
		ОбластиТабДок["П030309_7"].Значение = ОбластиТабДок["П030310_7"].Значение;
		ОбластиТабДок["П030309_3"].Значение = ОбластиТабДок["П030310_3"].Значение+ОбластиТабДок["П030311_3"].Значение;
		ОбластиТабДок["П030309_4"].Значение = ОбластиТабДок["П030310_4"].Значение+ОбластиТабДок["П030311_4"].Значение;
		ОбластиТабДок["П030309_5"].Значение = ОбластиТабДок["П030310_5"].Значение+ОбластиТабДок["П030311_5"].Значение;
	...
	ИначеЕсли ИмяТекущейСтраницыПанели = "Разделы11_12" Тогда
		ОбластиТабДок = Форма.ТабличныйДокумент.Области;
		Для Стр = 14 По 27 Цикл
			Если Стр > 16 И Стр < 25 Тогда 
				Продолжить;
			КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2017Кв1_Титульный", "ОсновнаяЧасть1_1", СтруктураДанныхТитульный);
	ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2017Кв1_Титульный", "ОсновнаяЧасть1_2", СтруктураДанныхТитульный);
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-инновация");
	
	Для Ном = 1 По 5 Цикл
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2017Кв1_Разделы1_3", "Раздел"+Ном, СтруктураДанныхРазделы1_3);
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-инновация");
	КонецЦикла;
	
	Для Ном = 1 По 4 Цикл
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2017Кв1_Разделы5_7", "Раздел"+Ном, СтруктураДанныхРазделы5_7);
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-инновация");
	КонецЦикла;
	
	Для Ном = 9 По 10 Цикл
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2017Кв1_Разделы8_10", "Раздел"+Ном, СтруктураДанныхРазделы8_10);
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-инновация");
	КонецЦикла;
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2017Кв1_Разделы11_12", "Раздел11", СтруктураДанныхРазделы11_12);
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-инновация");
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2017Кв1_Разделы11_12", "Раздел12", СтруктураДанныхРазделы11_12);
	ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2017Кв1_Разделы11_12", "Раздел12_Доп", СтруктураДанныхРазделы11_12);
	ЗаполнитьДаннымиТабличныйДокумент(ПечатнаяФорма, "ФормаОтчета2017Кв1_Подписи", "Подписи", СтруктураДанныхПодписи);
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 4-инновация");
...
Процедура ОчиститьНаСервере() Экспорт
	...
	СтруктураДанныхРазделы1_3.Очистить();
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтруктураРеквизитовФормы.мАвтоВыборКодов = Ложь Тогда 
		Если (СтрНайти(Область.Имя, "П1111")= 1 И СтрДлина(Область.Имя) = 7)
			...
			Или Область.Имя = "П01301217" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ДаНет");
		ИначеЕсли Область.Имя = "П010111" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ЧастотаРабот");
		...
		ИначеЕсли СтрНайти(Область.Имя, "П0404")= 1 И Число(Прав(Область.Имя, 2)) <= 16 Тогда
		...
		ИначеЕсли СтрНайти(Область.Имя, "П0808")= 1
			Или (СтрНайти(Область.Имя, "П0909")= 1 И СтрДлина(Область.Имя) = 7 И Область.Имя <> "П090914") Тогда
		...
		ИначеЕсли Область.Имя = "П010110" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "Стр110");
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ИмяТекущейСтраницыПанели = "Разделы1_3" 
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Разделы1_3 = мДанныеОтчета.ПолеТабличногоДокументаРазделы1_3;
	...
	ПараметрыВыгрузки.Вставить("ОргЭлектроннаяПочта", СокрЛП(Подписи.ОргАдресЭлектроннойПочты));
	...
	ПараметрыВыгрузки.Вставить("Стр533", ?(СтрДлина(СокрЛП(Разделы5_7.П05053301))=0, 0, 1));
	
	Для Стр = 213 По 217 Цикл 
		СтрЗн1 = СокрЛП(Разделы1_3["П020"+Стр+"_1"]);
		СтрЗн2 = СокрЛП(Разделы1_3["П020"+Стр+"_2"]);
		Если СтрЗн1 = "" Тогда 
			СтрЗн1 = СтрЗн2;
			СтрЗн2 = "";
		КонецЕсли;
