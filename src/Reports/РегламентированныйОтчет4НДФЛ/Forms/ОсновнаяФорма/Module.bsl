...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Элементы.Организация.СписокВыбора.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Декларацию представляют: %1';
															|en = 'Декларацию представляют: %1'"), КтоСдаетОтчет()));
	КонецЕсли;
...
Процедура ВыбратьФорму(Команда)
	...
	РегламентированнаяОтчетностьКлиент.ВыбратьФормуОтчетаИзДействующегоСписка(ЭтотОбъект,
		...
		СтрШаблон(НСтр("ru = 'Декларацию представляют: %1';
						|en = 'Декларацию представляют: %1'"), НадписьКтоСдаетОтчет));
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ОткрытьФормуОтчета.КнопкаПоУмолчанию = (КоличествоФорм > 0);
	Элементы.НадписьСрокПредставленияОтчета.Видимость = (КоличествоФорм > 0);
	Элементы.НадписьКтоСдаетОтчет.Видимость = (КоличествоФорм > 0);
	
	Если Год(Форма.мДатаКонцаПериодаОтчета) >= ГодОтменыДекларации() Тогда
		Элементы.ОписаниеНормативДок.Видимость = Истина;
		Форма.ОписаниеНормативДок = НСтр("ru = 'Утратила силу (Федеральный закон от 15.04.2019 N 63-ФЗ).';
										|en = 'Утратила силу (Федеральный закон от 15.04.2019 N 63-ФЗ).'");
	КонецЕсли;
...
Функция СрокПредставленияОтчетности(Форма)
	...
	Возврат НСтр("ru = 'В пятидневный срок по истечении месяца со дня появления дохода, а также в случае увеличения или уменьшения дохода более чем на 50%.';
				|en = 'В пятидневный срок по истечении месяца со дня появления дохода, а также в случае увеличения или уменьшения дохода более чем на 50%.'");
...
Функция КтоСдаетОтчет()
	...
	Возврат НСтр("ru = 'Индивидуальные предприниматели и лица, занимающиеся частной практикой (нотариусы, адвокаты, учредившие адвокатские кабинеты).';
				|en = 'Индивидуальные предприниматели и лица, занимающиеся частной практикой (нотариусы, адвокаты, учредившие адвокатские кабинеты).'");
	
КонецФункции

&НаСервереБезКонтекста
Функция ГодОтменыДекларации()
	
	Возврат Год(Отчеты.РегламентированныйОтчет4НДФЛ.ДатаОтменыДекларации());
