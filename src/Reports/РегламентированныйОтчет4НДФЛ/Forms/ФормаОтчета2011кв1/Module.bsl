...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "27/12/2010";
	
	// Код отчета, равен КНД
	СтруктураРеквизитовФормы.мКодОтчета = "1151021";
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ЭтаФорма[ТекущийЭлемент.Имя].Области[КодПоказателя].Значение = СтрокаРезультатВыбора;
...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		СтрокаСведений = "ИННФЛ, КодНО, ТипНП, ТелДом, ФИО,ОКАТО";
		...
		Если ДанныеФЛ.Количество() > 0 Тогда
			...
			ТабличныйДокумент.Области.Фамилия.Значение = СокрЛП(ДанныеФЛ.Фамилия);
			ТабличныйДокумент.Области.Имя.Значение = СокрЛП(ДанныеФЛ.Имя);
			ТабличныйДокумент.Области.Отчество.Значение = СокрЛП(ДанныеФЛ.Отчество);
...
Процедура ЗаполнитьСведенияОПодписантах() Экспорт
	...
	Иначе
		...
		ФИОПодписант = РегламентированнаяОтчетность.РазложитьФИО(Подписант);
		
	КонецЕсли;
	
	ТабличныйДокумент.Области.ПрПодп.Значение = ТипПодписанта;
	ТабличныйДокумент.Области.ОргПодписантФамилия.Значение = ФИОПодписант.Фамилия;
	ТабличныйДокумент.Области.ОргПодписантИмя.Значение = ФИОПодписант.Имя;
	ТабличныйДокумент.Области.ОргПодписантОтчество.Значение = ФИОПодписант.Отчество;
...
Процедура ЗаполнитьПустыеСтруктурыДанных()
	...
	Для Каждого Раздел Из мСвойстваРазделовДекларации.ПолучитьЭлементы() Цикл
		...
		Если ЭтаФорма["СтруктураДанных" + Раздел.ИмяСтраницы].Количество() = 0 Тогда
...
Процедура Очистить(ВыполняемоеОповещение = Неопределено) Экспорт
	...
	ТекстВопроса = "Внимание! Отчет будет очищен." + Символы.ПС
				 + "Продолжить?";
	
	РегламентированнаяОтчетностьКлиент.Очистить(ЭтаФорма, ВыполняемоеОповещение,,ТекстВопроса);
...
Процедура ЗаполнитьСведенияОПодписантахИКодПоОКАТО(Ссылка)
	...
	ДлинаЗначения = СтрДлина(СокрЛП(ОКАТО));
	Для Инд = ДлинаЗначения + 1 По 11 Цикл
		ОКАТО = СокрЛП(ОКАТО) + "0";
	КонецЦикла;
...
Процедура ПриОткрытии(Отказ)
	...
	Если СтрНайти(Заголовок, СтруктураРеквизитовФормы.СтрПериодОтчета) = 0 Тогда
		Заголовок = "Декларация 4-НДФЛ " + СтруктураРеквизитовФормы.СтрПериодОтчета;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	мПараметры.Вставить("КНД",					"1151021");
	...
	мПараметры.Вставить("ПредДоход",			Титульный.СуммаПредполагаемогоДохода);	
	...
	мПараметры.Вставить("ИмяФайлаСхемы",				"NO_NDFL4_1_034_00_05_01_01.xsd");
	мПараметры.Вставить("ПрефиксИмениФайлаВыгрузки",	"NO_NDFL4");
	
	мПараметры.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "НДФЛ4");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПараметрыВыгрузки.ТипПодписанта = "1" 
		И (ЗначениеЗаполнено(ПараметрыВыгрузки.ИмяПодписанта)  
		...
		ИЛИ ЗначениеЗаполнено(ПараметрыВыгрузки.ФамилияПодписанта))
	Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Информация о представителе налогоплательщика должна быть очищена", "Титульный", "ОргПодписантФамилия");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПараметрыВыгрузки.ТипПодписанта = "2" 
		И (НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ИмяПодписанта)  
		ИЛИ НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ФамилияПодписанта))
		И НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.НаимОргПредставителя)
	Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнена информация о педставителе налогоплательщика", "Титульный", "ОргПодписантФамилия");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	
	Если ПараметрыВыгрузки.ТипПодписанта = "2" 
		И НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.НаимДокПредставителя)  
	Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, """Наименование документа, подтверждающего полномочия представителя"" должно быть заполнено", "Титульный", "ДокУпПред");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	// заполнение 
	Если НЕ СтраницаПуста(мДанныеОтчета.Титульный, "Титульный", УзелРодитель) Тогда
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Титульный",	 		"МЧБ_2011");
