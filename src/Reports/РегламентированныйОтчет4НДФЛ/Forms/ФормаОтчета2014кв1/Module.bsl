...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТабличныйДокумент.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(ИмяМакетаТекущегоРаздела));
...
Процедура ОчиститьОтчет(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Внимание! Отчет будет очищен.
		|Продолжить?';
		|en = 'Внимание! Отчет будет очищен.
		|Продолжить?'");
...
Процедура СформироватьСпискиВыбора()
	...
	КоллекцияСписковВыбора = Отчеты.РегламентированныйОтчет4НДФЛ.СпискиВыбора(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("Списки2014кв1"));
		...
		Если ЗначениеЗаполнено(ПараметрыВыгрузки.ИмяПодписанта)
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				НСтр("ru = 'Информация о представителе налогоплательщика должна быть очищена';
					|en = 'Информация о представителе налогоплательщика должна быть очищена'"),
				"Титульный",
				"ОргПодписантФамилия");
		...
		Если Не ЗначениеЗаполнено(ПараметрыВыгрузки.ИмяПодписанта) И Не ЗначениеЗаполнено(ПараметрыВыгрузки.ФамилияПодписанта) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				НСтр("ru = 'Не заполнена информация о педставителе налогоплательщика';
					|en = 'Не заполнена информация о педставителе налогоплательщика'"),
				"Титульный",
				"ОргПодписантФамилия");
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ПараметрыВыгрузки.НаимДокПредставителя) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				НСтр("ru = 'Не заполнено наименование документа, подтверждающего полномочия представителя';
					|en = 'Не заполнено наименование документа, подтверждающего полномочия представителя'"),
				"Титульный",
				"ДокУпПред");
		КонецЕсли;
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Титульный",	 		"МЧБ_2014");
