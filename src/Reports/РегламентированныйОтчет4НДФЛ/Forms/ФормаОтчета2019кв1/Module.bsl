...
Процедура ПоказатьПериод()
	...
	ОбластьОтчетныйГод = ПолеТабличногоДокументаТитульный.Области.Найти("ОтчетГод");
	Если ОбластьОтчетныйГод <> Неопределено Тогда
		...
		ОбластьОтчетныйГод.Значение = ОтчетныйГод;
	КонецЕсли;
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ПараметрыОтчета = Отчеты.РегламентированныйОтчет4НДФЛ.ПараметрыВыгрузки(
