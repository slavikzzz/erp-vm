...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = "0610016";
	// Код отчета, равен коду по ОКУД
	СтруктураРеквизитовФормы.мПечатныеФормы = Новый СписокЗначений;
	
	ПолеТабличногоДокументаФормаОтчета.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2013Кв4_ФормаОтчета"));
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0000003001";
...
Процедура СформироватьСпискиВыбора()
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("Списки2013Кв4");
	...
	Для Каждого Область Из МакетСоставаПоказателей.Области Цикл
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
			...
			ТаблСписка.Колонки.Добавить("НазваниеДоп",Строка128,, МакетСоставаПоказателей.Область(ВерхОбласти, 2, ВерхОбласти, 2).ШиринаКолонки);
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
		НоваяСтрока = КоллекцияСписковВыбора["КодСтроки"].Вставить(КоллекцияСписковВыбора["s_okun"].Количество());
		НоваяСтрока.Код = "80";
		НоваяСтрока.Название = "Строка 80"
	КонецЕсли;
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "КодСтроки",   512, , "Выбор кода строки",   КоллекцияСписковВыбора["КодСтроки"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ОКУН",        512, , "Выбор кода по ОКУН",   КоллекцияСписковВыбора["s_okun"]);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("s_okp", КоллекцияСписковВыбора["s_okp"]);
	Запрос.УстановитьПараметр("okp_55", КоллекцияСписковВыбора["okp_55"]);
	Запрос.УстановитьПараметр("okp_6", КоллекцияСписковВыбора["okp_6"]);
	Запрос.УстановитьПараметр("okp_7", КоллекцияСписковВыбора["okp_7"]);
	Запрос.УстановитьПараметр("okp_ei_55", КоллекцияСписковВыбора["okp_ei_55"]);
	Запрос.УстановитьПараметр("okp_ei_70", КоллекцияСписковВыбора["okp_ei_70"]);
	Запрос.УстановитьПараметр("okp_ei_80", КоллекцияСписковВыбора["okp_ei_80"]);
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	okp_ei_55.Код КАК Код,
	|	okp_ei_55.Название КАК КодОКПД,
	|	okp_ei_55.НазваниеДоп КАК Название
	|ПОМЕСТИТЬ ВТ_okp_ei_55
	|ИЗ &okp_ei_55 КАК okp_ei_55
	|ИНДЕКСИРОВАТЬ ПО
	|	okp_ei_55.Название
	|;
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	okp_ei_70.Код КАК Код,
	|	okp_ei_70.Название КАК КодОКПД,
	|	okp_ei_70.НазваниеДоп КАК Название
	|ПОМЕСТИТЬ ВТ_okp_ei_70
	|ИЗ &okp_ei_70 КАК okp_ei_70
	|ИНДЕКСИРОВАТЬ ПО
	|	okp_ei_70.Название
	|;
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	okp_ei_80.Код КАК Код,
	|	okp_ei_80.Название КАК КодОКПД,
	|	okp_ei_80.НазваниеДоп КАК Название
	|ПОМЕСТИТЬ ВТ_okp_ei_80
	|ИЗ &okp_ei_80 КАК okp_ei_80
	|ИНДЕКСИРОВАТЬ ПО
	|	okp_ei_80.Название
	|;
	|
	...
	|ИЗ
	|	ВТ_okp_55 КАК okp_55
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|	ВТ_s_okp КАК ОКПД
	|	ПО okp_55.Код = ОКПД.Код
	|;
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ОКПД.Код КАК Код,
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|	ВТ_s_okp КАК ОКПД
	|	ПО okp_7.Код = ОКПД.Код
	|;
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	okp_ei_55.Код КАК Код,
	|	okp_ei_55.Название КАК Название,
	|	okp_ei_55.КодОКПД КАК ФильтрОКПД
	|ИЗ
	|	ВТ_okp_ei_55 КАК okp_ei_55
	|;
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	okp_ei_70.Код КАК Код,
	|	okp_ei_70.Название КАК Название,
	|	okp_ei_70.КодОКПД КАК ФильтрОКПД
	|ИЗ
	|	ВТ_okp_ei_70 КАК okp_ei_70
	|;
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	okp_ei_80.Код КАК Код,
	|	okp_ei_80.Название КАК Название,
	|	okp_ei_80.КодОКПД КАК ФильтрОКПД
	|ИЗ
	|	ВТ_okp_ei_80 КАК okp_ei_80
	|;
	|
	|ВЫБРАТЬ
	|	okp_ei_80.Код КАК Код,
	...
	ОКП55 = Результат[7].Выгрузить();
	ОКП6 = Результат[8].Выгрузить();
	ОКП7 = Результат[9].Выгрузить();
	ЕИОКП55 = Результат[10].Выгрузить();
	ЕИОКП70 = Результат[11].Выгрузить();
	ЕИОКП80 = Результат[12].Выгрузить();
	
	НоваяСтрокаТЗВыбора = ОКП55.Вставить(0);
	НоваяСтрокаТЗВыбора.Код = "";
	НоваяСтрокаТЗВыбора.Название = "Пустое значение";
	
	НоваяСтрокаТЗВыбора = ОКП6.Вставить(0);
	НоваяСтрокаТЗВыбора.Код = "";
	НоваяСтрокаТЗВыбора.Название = "Пустое значение";
	
	НоваяСтрокаТЗВыбора = ОКП7.Вставить(0);
	НоваяСтрокаТЗВыбора.Код = "";
	НоваяСтрокаТЗВыбора.Название = "Пустое значение";
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ОКП55", 512, , "Выбор кода по ОКП", ОКП55);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ОКП6", 512, , "Выбор кода по ОКП", ОКП6);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ОКП7", 512, , "Выбор кода по ОКП", ОКП7);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ЕИОКП55", 512, , "Выбор кода единицы измерения", ЕИОКП55);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ЕИОКП70", 512, , "Выбор кода единицы измерения", ЕИОКП70);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ЕИОКП80", 512, , "Выбор кода единицы измерения", ЕИОКП80);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ЕИОКПОтфильтрованный", 512, , "Выбор кода единицы измерения", Результат[13].Выгрузить());
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ЗаполнениеПервойСтрокиТЧ = Варианты.НайтиСтроки(Новый Структура("КодПоказателя", "П000000300101"));
	Если ЗначениеЗаполнено(ЗаполнениеПервойСтрокиТЧ) И ЗаполнениеПервойСтрокиТЧ.Количество() > 0 Тогда 
		ЕстьАвтозаполнениеТЧ = Истина;
	Иначе
		ЕстьАвтозаполнениеТЧ = Ложь;
	КонецЕсли;
	
	Если ЕстьАвтозаполнениеТЧ Тогда
		Попытка
			Строка = 0;
			Пока Истина Цикл 
				Строка = Строка + 1;
				Для Гр = 1 По 12 Цикл
					ИмяОбласти = "П0000003001" + Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=") + "_" + Строка;
					ПолеТабличногоДокументаФормаОтчета.Области[ИмяОбласти].Защита = Ложь;
					ПолеТабличногоДокументаФормаОтчета.Области[ИмяОбласти].ЦветФона = мСтруктураЦветовРаскраски._3;
				КонецЦикла;
			КонецЦикла;
		Исключение
		КонецПопытки;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	Область4_2 = ПолеТабличногоДокументаФормаОтчета.Области.ОсновнаяЧасть4_2;
	Область4_3 = ПолеТабличногоДокументаФормаОтчета.Области.ОсновнаяЧасть4_3;
	Если Область4_2.Низ + 14 < Область4_3.Верх Тогда 
		ОбластьДляУдаления = ПолеТабличногоДокументаФормаОтчета.Область(Область4_2.Низ + 14, ,Область4_3.Верх - 1, );
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 Тогда
		УдалитьСтроку("");
	ИначеЕсли СтрНайти(Область.Имя, "П000000300102_") = 1 Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "КодСтроки");
	...
	ИначеЕсли Не СтруктураРеквизитовФормы.мАвтоВыборКодов И Не Область.Защита И СтрНайти(Область.Имя, "П0000003001") = 1 Тогда 
		
		НомерСтроки = Сред(Область.Имя, 15);
		ЗначВСтолбцеБ = ПолеТабличногоДокументаФормаОтчета.Области["П000000300102_" + НомерСтроки].Значение;
		
		Если СтрНайти(Область.Имя, "П000000300105_") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ОКУН");
			КодПоказателя = ПараметрВводаПоля.Показатель;
			КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
			ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, СтрЗаменить(Область.Имя, "П000000300105_", "П000000300101_"));
		ИначеЕсли СтрНайти(Область.Имя, "П000000300104_") = 1 Тогда
			Если ЗначВСтолбцеБ = "50" Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ОКП55");
			ИначеЕсли ЗначВСтолбцеБ = "70" Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ОКП6");
			ИначеЕсли ЗначВСтолбцеБ = "80" Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ОКП7");
			КонецЕсли;
			КодПоказателя = ПараметрВводаПоля.Показатель;
			КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
			ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, СтрЗаменить(Область.Имя, "П000000300104_", "П000000300101_"));
		ИначеЕсли СтрНайти(Область.Имя, "П000000300101_") = 1 Тогда 
			Если ЗначВСтолбцеБ = "50" Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ОКП55");
				СмежнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области["П000000300104_" + НомерСтроки];
				КодПоказателя = ПараметрВводаПоля.Показатель;
				КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
				ВвестиПоказатель(Элемент, СмежнаяОбласть, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, Область.Имя);
			ИначеЕсли ЗначВСтолбцеБ = "60" Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ОКУН");
				СмежнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области["П000000300105_" + НомерСтроки];
				КодПоказателя = ПараметрВводаПоля.Показатель;
				КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
				ВвестиПоказатель(Элемент, СмежнаяОбласть, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, Область.Имя);
			ИначеЕсли ЗначВСтолбцеБ = "70" Тогда
			...
			ИначеЕсли ЗначВСтолбцеБ = "80" Тогда 
				...
				ВвестиПоказатель(Элемент, СмежнаяОбласть, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, Область.Имя);
			КонецЕсли;
		ИначеЕсли СтрНайти(Область.Имя, "П000000300103_") = 1 Или СтрНайти(Область.Имя, "П000000300106_") = 1 Тогда
			ОсновнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области["П000000300106_" + НомерСтроки];
			СмежнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области["П000000300103_" + НомерСтроки];
			
			Если ЗначВСтолбцеБ = "50" Тогда
				ОтборЗначение = ПолеТабличногоДокументаФормаОтчета.Области["П000000300104_" + НомерСтроки].Значение;
				ОтфильтроватьЕдиницыИзмеренияПоКодуОКПД(ОтборЗначение, "ЕИОКП55");
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ЕИОКПОтфильтрованный");
				КодПоказателя = ПараметрВводаПоля.Показатель;
				КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
				ВвестиПоказатель(Элемент, ОсновнаяОбласть, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, СмежнаяОбласть.Имя);
			ИначеЕсли ЗначВСтолбцеБ = "70" Тогда
				ОтборЗначение = ПолеТабличногоДокументаФормаОтчета.Области["П000000300104_" + НомерСтроки].Значение;
				ОтфильтроватьЕдиницыИзмеренияПоКодуОКПД(ОтборЗначение, "ЕИОКП70");
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ЕИОКПОтфильтрованный");
				КодПоказателя = ПараметрВводаПоля.Показатель;
				КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
				ВвестиПоказатель(Элемент, ОсновнаяОбласть, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, СмежнаяОбласть.Имя);
			ИначеЕсли ЗначВСтолбцеБ = "80" Тогда 
				ОтборЗначение = ПолеТабличногоДокументаФормаОтчета.Области["П000000300104_" + НомерСтроки].Значение;
				ОтфильтроватьЕдиницыИзмеренияПоКодуОКПД(ОтборЗначение, "ЕИОКП80");
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ЕИОКПОтфильтрованный");
				КодПоказателя = ПараметрВводаПоля.Показатель;
				КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
				ВвестиПоказатель(Элемент, ОсновнаяОбласть, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, СмежнаяОбласть.Имя);
...
&НаКлиенте
Функция РеализованоАвтозаполнение()
	Возврат ЕстьАвтозаполнениеТЧ;
КонецФункции

&НаКлиенте
Процедура ОбнулитьСтроку(НомерСтроки)
	Если РеализованоАвтозаполнение() Тогда 
		Возврат;
	КонецЕсли;
	
	Префикс = "П0000003001";
	ЗнКол2 = "";
	Для ДляКол = 1 По 12 Цикл
		Если ДляКол = 2 Тогда
			ЗнКол2 = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + Формат(ДляКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_" + НомерСтроки].Значение;
			Продолжить;
		КонецЕсли;
		
		Если ДляКол = 3 И ЗнКол2 = "60" Тогда
			ПолеТабличногоДокументаФормаОтчета.Области[Префикс + Формат(ДляКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_" + НомерСтроки].Значение = "тыс. рублей";
			Продолжить;
		КонецЕсли;
		
		Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + Формат(ДляКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_" + НомерСтроки];
		Ячейка.Значение = Неопределено;
	КонецЦикла;
КонецПроцедуры

&НаКлиенте
Процедура УстановитьДоступностьВСтроках()
	...
	Попытка
		Пока Истина Цикл 
			...
			Область = ПолеТабличногоДокументаФормаОтчета.Области["П000000300102_" + НомерСтроки];
			Показатель = Область.Значение;
			НомерСтрокиСтр = Сред(Область.Имя, 15);
			Если Показатель = "80" Тогда
				УстановитьДоступностьВСтроке(НомерСтрокиСтр,Истина,Истина,Истина,,Истина,,,Истина,,,Истина);
				УстановитьФорматВСтроке80(НомерСтрокиСтр);
			ИначеЕсли Показатель = "70" Тогда
				УстановитьДоступностьВСтроке(НомерСтрокиСтр,Истина,Истина,Истина,,Истина,,,Истина,,,Истина);
				УстановитьФорматВСтроке70(НомерСтрокиСтр);
			ИначеЕсли Показатель = "60" Тогда
				УстановитьДоступностьВСтроке(НомерСтрокиСтр,Истина,,,Истина,,Истина);
				ПолеТабличногоДокументаФормаОтчета.Области["П000000300103_" + НомерСтрокиСтр].Значение = "тыс. рублей";
				УстановитьФорматВСтроке60(НомерСтрокиСтр);
			ИначеЕсли Показатель = "50" Тогда
				УстановитьДоступностьВСтроке(НомерСтрокиСтр,Истина,Истина,Истина,,Истина,Истина,Истина,Истина,Истина,Истина,Истина);
				УстановитьФорматВСтроке50(НомерСтрокиСтр);
			Иначе
				УстановитьДоступностьВСтроке(НомерСтрокиСтр);
			КонецЕсли;
			
			ОбнулитьНедоступные(НомерСтрокиСтр);
		КонецЦикла;
	Исключение
	КонецПопытки;
...
&НаКлиенте
Процедура ОбнулитьНедоступные(НомерСтроки)
	Если РеализованоАвтозаполнение() Тогда 
		Возврат;
	КонецЕсли;
	Префикс = "П0000003001";
	
	ЗнКол2 = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + "02_" + НомерСтроки].Значение;
	Для ДляКол = 1 По 12 Цикл
		Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + Формат(ДляКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_" + НомерСтроки];
		Если Ячейка.Защита 
			И (ЗнКол2 <> "60" Или ДляКол <> 3)Тогда 
			Ячейка.Значение = Неопределено;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
&НаКлиенте
Процедура УстановитьФорматВСтроке50(НомерСтроки)
	...
	ОписаниеТиповЧисло15_3 = Новый ОписаниеТипов(Массив, Новый КвалификаторыЧисла(15,3,ДопустимыйЗнак.Неотрицательный));
	
	Для Инд = 7 по 12 Цикл 
		Ячейка = ПолеТабличногоДокументаФормаОтчета.Области["П0000003001" + Формат(Инд, "ЧЦ=2; ЧВН=; ЧГ=") + "_" + НомерСтроки];
		Ячейка.ТипЗначения = ОписаниеТиповЧисло15_3;
		Ячейка.Формат = "ЧЦ=15; ЧДЦ=3;";
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьФорматВСтроке60(НомерСтроки)
	Если РеализованоАвтозаполнение() Тогда 
		Возврат;
	КонецЕсли;
	Массив = Новый Массив;
	Массив.Добавить(Тип("Число"));
	ОписаниеТиповЧисло15_1 = Новый ОписаниеТипов(Массив, Новый КвалификаторыЧисла(15,1,ДопустимыйЗнак.Неотрицательный));
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области["П000000300107_" + НомерСтроки];
	Ячейка.ТипЗначения = ОписаниеТиповЧисло15_1;
	Ячейка.Формат = "ЧЦ=15; ЧДЦ=1;";
КонецПроцедуры

&НаКлиенте
Процедура УстановитьФорматВСтроке70(НомерСтроки)
	Если РеализованоАвтозаполнение() Тогда 
		Возврат;
	КонецЕсли;
	Массив = Новый Массив;
	Массив.Добавить(Тип("Число"));
	ОписаниеТиповЧисло15_2 = Новый ОписаниеТипов(Массив, Новый КвалификаторыЧисла(15,2,ДопустимыйЗнак.Неотрицательный));
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области["П000000300109_" + НомерСтроки];
	Ячейка.ТипЗначения = ОписаниеТиповЧисло15_2;
	Ячейка.Формат = "ЧЦ=15; ЧДЦ=2;";
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области["П000000300112_" + НомерСтроки];
	Ячейка.ТипЗначения = ОписаниеТиповЧисло15_2;
	Ячейка.Формат = "ЧЦ=15; ЧДЦ=2;";
КонецПроцедуры

&НаКлиенте
Процедура УстановитьФорматВСтроке80(НомерСтроки)
	Если РеализованоАвтозаполнение() Тогда 
		Возврат;
	КонецЕсли;
	Массив = Новый Массив;
	Массив.Добавить(Тип("Число"));
	ОписаниеТиповЧисло15_2 = Новый ОписаниеТипов(Массив, Новый КвалификаторыЧисла(15,2,ДопустимыйЗнак.Неотрицательный));
	ОписаниеТиповЧисло14_2 = Новый ОписаниеТипов(Массив, Новый КвалификаторыЧисла(14,2,ДопустимыйЗнак.Неотрицательный));
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области["П000000300109_" + НомерСтроки];
	Ячейка.ТипЗначения = ОписаниеТиповЧисло15_2;
	Ячейка.Формат = "ЧЦ=15; ЧДЦ=2;";
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области["П000000300112_" + НомерСтроки];
	Ячейка.ТипЗначения = ОписаниеТиповЧисло14_2;
	Ячейка.Формат = "ЧЦ=14; ЧДЦ=2;";
КонецПроцедуры

&НаКлиенте
Процедура УстановитьДоступностьВСтроке(НомерСтроки, ДостКол1 = Ложь, ДостКол3 = Ложь, ДостКол4 = Ложь, ДостКол5 = Ложь, ДостКол6 = Ложь,
	ДостКол7 = Ложь, ДостКол8 = Ложь, ДостКол9 = Ложь, ДостКол10 = Ложь, ДостКол11 = Ложь, ДостКол12 = Ложь)
	...
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + "01_" + НомерСтроки];
	Ячейка.Защита = Не ДостКол1;
	Ячейка.ЦветФона = ?(ДостКол1, мСтруктураЦветовРаскраски._0, мСтруктураЦветовРаскраски._5);
	
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + "03_" + НомерСтроки];
	Ячейка.Защита = Не ДостКол3;
	Ячейка.ЦветФона = ?(ДостКол3, мСтруктураЦветовРаскраски._0, мСтруктураЦветовРаскраски._5);
	
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + "04_" + НомерСтроки];
	Ячейка.Защита = Не ДостКол4;
	Ячейка.ЦветФона = ?(ДостКол4, мСтруктураЦветовРаскраски._0, мСтруктураЦветовРаскраски._5);
	
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + "05_" + НомерСтроки];
	Ячейка.Защита = Не ДостКол5;
	Ячейка.ЦветФона = ?(ДостКол5, мСтруктураЦветовРаскраски._0, мСтруктураЦветовРаскраски._5);
	
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + "06_" + НомерСтроки];
	Ячейка.Защита = Не ДостКол6;
	Ячейка.ЦветФона = ?(ДостКол6, мСтруктураЦветовРаскраски._0, мСтруктураЦветовРаскраски._5);
	
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + "07_" + НомерСтроки];
	Ячейка.Защита = Не ДостКол7;
	Ячейка.ЦветФона = ?(ДостКол7, мСтруктураЦветовРаскраски._0, мСтруктураЦветовРаскраски._5);
	
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + "08_" + НомерСтроки];
	Ячейка.Защита = Не ДостКол8;
	Ячейка.ЦветФона = ?(ДостКол8, мСтруктураЦветовРаскраски._0, мСтруктураЦветовРаскраски._5);
	
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + "09_" + НомерСтроки];
	Ячейка.Защита = Не ДостКол9;
	Ячейка.ЦветФона = ?(ДостКол9, мСтруктураЦветовРаскраски._0, мСтруктураЦветовРаскраски._5);
	
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + "10_" + НомерСтроки];
	Ячейка.Защита = Не ДостКол10;
	Ячейка.ЦветФона = ?(ДостКол10, мСтруктураЦветовРаскраски._0, мСтруктураЦветовРаскраски._5);
	
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + "11_" + НомерСтроки];
	Ячейка.Защита = Не ДостКол11;
	Ячейка.ЦветФона = ?(ДостКол11, мСтруктураЦветовРаскраски._0, мСтруктураЦветовРаскраски._5);
	
	Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + "12_" + НомерСтроки];
	Ячейка.Защита = Не ДостКол12;
	Ячейка.ЦветФона = ?(ДостКол12, мСтруктураЦветовРаскраски._0, мСтруктураЦветовРаскраски._5);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-5М");
	...
	// Многострочная часть.
	ВерхМногострочнойЧасти = Области.ОсновнаяЧасть4_2.Низ + 1;
	НизМногострочнойЧасти = Области.ДобавлениеСтроки1.Верх - 1;
	Область4_3 = ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_3");
	...
	Если ПечатнаяФорма.ПроверитьВывод(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_3")) Тогда
	...
	Если ПечатнаяФорма.ПроверитьВывод(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_4")) Тогда
		ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_4"));
	Иначе
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-5М");
		ПечатнаяФорма = Новый ТабличныйДокумент();
		ПечатнаяФорма.ОтображатьЗаголовки = Ложь;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если НЕ РегламентированнаяОтчетность.ПроверитьРазделыНаНаличиеИнформации(ЭтаФорма, ДеревоВыгрузки, ИмяПоляТаблДокумента) Тогда
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Страницы отчета не заполнены!", ИмяПоляТаблДокумента, "");
		СтатусПроверки = Ложь;
		...
		Если Стр.Тип = "С" ИЛИ Стр.Тип = "C" Тогда // учтем оба варианта: кириллицу и латиницу
			Если Стр.Многострочность И Стр.Код = Параметры.ИмяКлючевогоУзлаСодержательнойЧасти Тогда
				...
				Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Параметры, КлючПодчиненногоЭлемента + "_" + Формат(НомСтр, "ЧГ=")) Цикл
					Если Стр.Раздел = Параметры[КлючПодчиненногоЭлемента + "_" + Формат(НомСтр, "ЧГ=")] Тогда 
...
Функция ЗаполнитьАвтоНаСервере()
	...
	КонецЦикла;
	Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок1", РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтаФорма, П0000003001, Ложь));
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	//1. Строка 04 >= строки 05 по соответствующим графам.
	Знак = ">=";
	Для НомерКолонки = 1 По 2 Цикл
		...
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100040" + (2 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100050" + (2 + НомерКолонки)]);
	...
	//2. Строка 04 >= строки 06 по соответствующим графам.
	Знак = ">=";
	Для НомерКолонки = 1 По 2 Цикл
		ТекстФормулы = "стр. 04 гр. " + НомерКолонки + " >= стр. 06 гр. " + НомерКолонки;
		Описание     = "стр. 04 гр. " + НомерКолонки + " < стр. 06 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100040" + (2 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100060" + (2 + НомерКолонки)]);
		Расшифровка = Строка(ЛеваяЧасть) + " " + Знак + " " + Строка(ПраваяЧасть);
	...
	//3. Строка 05 >= строки 07 по соответствующим графам.
	Знак = ">=";
	Для НомерКолонки = 1 По 2 Цикл
		ТекстФормулы = "стр. 05 гр. " + НомерКолонки + " >= стр. 07 гр. " + НомерКолонки;
		Описание     = "стр. 05 гр. " + НомерКолонки + " < стр. 07 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100050" + (2 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100070" + (2 + НомерКолонки)]);
		Расшифровка = Строка(ЛеваяЧасть) + " " + Знак + " " + Строка(ПраваяЧасть);
	...
	//4. Строка 06 >= строки 07 по соответствующим графам.
	Знак = ">=";
	Для НомерКолонки = 1 По 2 Цикл
		ТекстФормулы = "стр. 06 гр. " + НомерКолонки + " >= стр. 07 гр. " + НомерКолонки;
		Описание     = "стр. 06 гр. " + НомерКолонки + " < стр. 07 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100060" + (2 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100070" + (2 + НомерКолонки)]);
		Расшифровка = Строка(ЛеваяЧасть) + " " + Знак + " " + Строка(ПраваяЧасть);
	...
	//5. Строка 08 >= строки 09 по соответствующим графам.
	Знак = ">=";
	Для НомерКолонки = 1 По 2 Цикл
		ТекстФормулы = "стр. 08 гр. " + НомерКолонки + " >= стр. 09 гр. " + НомерКолонки;
		Описание     = "стр. 08 гр. " + НомерКолонки + " < стр. 09 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100080" + (2 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100090" + (2 + НомерКолонки)]);
		Расшифровка = Строка(ЛеваяЧасть) + " " + Знак + " " + Строка(ПраваяЧасть);
	...
	//6. Строка 08 >= строка 10 + строка 12 по соответствующим графам.
	Знак = ">=";
	Для НомерКолонки = 1 По 2 Цикл
		ТекстФормулы = "стр. 08 гр. " + НомерКолонки + " >= стр. 10 + стр. 12 гр. " + НомерКолонки;
		Описание     = "стр. 08 гр. " + НомерКолонки + " < стр. 10 + стр. 12 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100080" + (2 + НомерКолонки)]);
		ПраваяЧасть_1 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100100" + (2 + НомерКолонки)]);
		ПраваяЧасть_2 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100120" + (2 + НомерКолонки)]);
	...
	//7. Строка 09 >= строка 11 + строка 13 по соответствующим графам.
	Знак = ">=";
	Для НомерКолонки = 1 По 2 Цикл
		ТекстФормулы = "стр. 09 гр. " + НомерКолонки + " >= стр. 11 + стр. 13 гр. " + НомерКолонки;
		Описание     = "стр. 09 гр. " + НомерКолонки + " < стр. 11 + стр. 13 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100090" + (2 + НомерКолонки)]);
		ПраваяЧасть_1 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100110" + (2 + НомерКолонки)]);
		ПраваяЧасть_2 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100130" + (2 + НомерКолонки)]);
		Расшифровка = Строка(ЛеваяЧасть) + " " + Знак + " " + Строка(ПраваяЧасть_1) + " + " + Строка(ПраваяЧасть_2);
	...
	//8. Строка 10 >= строки 11 по соответствующим графам.
	Знак = ">=";
	Для НомерКолонки = 1 По 2 Цикл
		ТекстФормулы = "стр. 10 гр. " + НомерКолонки + " >= стр. 11 гр. " + НомерКолонки;
		Описание     = "стр. 10 гр. " + НомерКолонки + " < стр. 11 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100100" + (2 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100110" + (2 + НомерКолонки)]);
		Расшифровка = Строка(ЛеваяЧасть) + " " + Знак + " " + Строка(ПраваяЧасть);
	...
	//9. Строка 12 >= строки 13 по соответствующим графам.
	Знак = ">=";
	Для НомерКолонки = 1 По 2 Цикл
		ТекстФормулы = "стр. 12 гр. " + НомерКолонки + " >= стр. 13 гр. " + НомерКолонки;
		Описание     = "стр. 12 гр. " + НомерКолонки + " < стр. 13 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100120" + (2 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00100130" + (2 + НомерКолонки)]);
		Расшифровка = Строка(ЛеваяЧасть) + " " + Знак + " " + Строка(ПраваяЧасть);
	...
	//14. Строка 28 >= строки 29 по соответствующим графам.
	Знак = ">=";
	Для НомерКолонки = 1 По 2 Цикл
		ТекстФормулы = "стр. 28 гр. " + НомерКолонки + " >= стр. 29 гр. " + НомерКолонки;
		Описание     = "стр. 28 гр. " + НомерКолонки + " < стр. 29 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00200280" + (3 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00200290" + (3 + НомерКолонки)]);
		Расшифровка = Строка(ЛеваяЧасть) + " " + Знак + " " + Строка(ПраваяЧасть);
	...
	//15. Строка 30 >= строки 31 по соответствующим графам.
	Знак = ">=";
	Для НомерКолонки = 1 По 2 Цикл
		ТекстФормулы = "стр. 30 гр. " + НомерКолонки + " >= стр. 31 гр. " + НомерКолонки;
		Описание     = "стр. 30 гр. " + НомерКолонки + " < стр. 31 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00200300" + (3 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00200310" + (3 + НомерКолонки)]);
		Расшифровка = Строка(ЛеваяЧасть) + " " + Знак + " " + Строка(ПраваяЧасть);
	...
	//16. Если строка 28 <> 0, то строка 30 <> 0 по соответствующим графам.
	Знак = "<>";
	Для НомерКолонки = 1 По 2 Цикл
		ТекстФормулы = "Если стр. 28 гр. " + НомерКолонки + " <> 0 , то стр. 30 гр. " + НомерКолонки + " <> 0";
		Описание     = "стр. 28 гр. " + НомерКолонки + " <> 0 и стр. 30 гр. " + НомерКолонки + " = 0";;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00200280" + (3 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00200300" + (3 + НомерКолонки)]);
	...
	//17. Если строка 29 <> 0, то строка 31 <> 0 по соответствующим графам.
	Знак = "<>";
	Для НомерКолонки = 1 По 2 Цикл
		ТекстФормулы = "Если стр. 29 гр. " + НомерКолонки + " <> 0 , то стр. 31 гр. " + НомерКолонки + " <> 0";
		Описание     = "стр. 29 гр. " + НомерКолонки + " <> 0 и стр. 31 гр. " + НомерКолонки + " = 0";;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00200290" + (3 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00200310" + (3 + НомерКолонки)]);
		Расшифровка = Строка(ЛеваяЧасть) + " , " + Строка(ПраваяЧасть);
	...
	//21. Строка с кодом 50 графа 3 >= графы 4.
	Знак = ">=";
	НомерСтроки = 0;
	ТекстФормулы = "Для стр. с кодом 50 гр. 3 >= гр.4";
	Описание     = "Для стр. с кодом 50 гр. 3 < гр.4";;
	Попытка
		Пока Истина Цикл
			НомерСтроки = НомерСтроки + 1;
			КодСтроки   = Показатели["П000000300102_" + НомерСтроки];
			Если КодСтроки <> "50" Тогда 
				Продолжить;
			КонецЕсли;
			
			ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П000000300109_" + НомерСтроки]);
			ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П000000300110_" + НомерСтроки]);
			Расшифровка = Строка(ЛеваяЧасть) + " " + Знак + " " + Строка(ПраваяЧасть);
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ОбластьНаименование) Тогда
		...
		Если СтрНайти(ОбластьНаименование, "П000000300101_") = 1 Тогда 
			ЗнКол2 = ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(ОбластьНаименование, "П000000300101_", "П000000300102_")].Значение;
			Если ЗнКол2 <> "60" Тогда 
				ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(ОбластьНаименование, "П000000300101_", "П000000300103_")].Значение = Неопределено;
				ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(ОбластьНаименование, "П000000300101_", "П000000300106_")].Значение = Неопределено;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если СтрНайти(Область.Имя, "П000000300102_") = 1 Тогда
		Показатель = Область.Значение;
		НомерСтроки = Сред(Область.Имя, 15);
		Если Показатель = "80" Тогда
			УстановитьДоступностьВСтроке(НомерСтроки,Истина,Истина,Истина,,Истина,,,Истина,,,Истина);
			УстановитьФорматВСтроке80(НомерСтроки);
		ИначеЕсли Показатель = "70" Тогда
			УстановитьДоступностьВСтроке(НомерСтроки,Истина,Истина,Истина,,Истина,,,Истина,,,Истина);
			УстановитьФорматВСтроке70(НомерСтроки);
		ИначеЕсли Показатель = "60" Тогда
			УстановитьДоступностьВСтроке(НомерСтроки,Истина,,,Истина,,Истина);
			ПолеТабличногоДокументаФормаОтчета.Области["П000000300103_" + НомерСтроки].Значение = "тыс. рублей";
			УстановитьФорматВСтроке60(НомерСтроки);
		ИначеЕсли Показатель = "50" Тогда
			УстановитьДоступностьВСтроке(НомерСтроки,Истина,Истина,Истина,,Истина,Истина,Истина,Истина,Истина,Истина,Истина);
			УстановитьФорматВСтроке50(НомерСтроки);
		Иначе
			УстановитьДоступностьВСтроке(НомерСтроки);
		КонецЕсли;
		
		Если ПредыдущийПоказатель <> Показатель Тогда
			ОбнулитьСтроку(НомерСтроки);
		КонецЕсли;
		ОбнулитьНедоступные(НомерСтроки);
	КонецЕсли;
