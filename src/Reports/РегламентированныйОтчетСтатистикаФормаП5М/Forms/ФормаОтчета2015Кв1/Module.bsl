...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/11/2014";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Не СтруктураРеквизитовФормы.мАвтоВыборКодов И Область.Защита = Ложь И СтрНайти(Область.Имя, "П0000003001") = 1 Тогда 
...
Процедура УстановитьДоступностьВСтроках()
	...
	Пока Истина Цикл 
		...
		Область = ПолеТабличногоДокументаФормаОтчета.Области.Найти("П000000300102_" + НомерСтроки);
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Пока Истина Цикл
		НомерСтроки = НомерСтроки + 1;
		Если Не Показатели.Свойство("П000000300102_" + НомерСтроки) Тогда 
			Прервать;
		КонецЕсли;
