...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолеТабличногоДокументаФормаОтчета.Вывести(ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаОтчета2021Кв1_ФормаОтчета"));
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Варианты = РегламентированнаяОтчетностьВызовСервера.ПолучитьТаблицуВариантовЗаполненияНаСервере(ЭтотОбъект, "ПолеТабличногоДокументаФормаОтчета");
	...
	ИначеЕсли Не СтруктураРеквизитовФормы.мАвтоВыборКодов И Область.Защита = Ложь И СтрНайти(Область.Имя, "П0000003001") = 1 Тогда 
		...
		ИначеЕсли СтрНайти(Область.Имя, "П000000300101_") = 1 Тогда 
			...
			ИначеЕсли ЗначВСтолбцеБ = "80" Тогда 
				...
				ВвестиПоказатель(Элемент, СмежнаяОбласть, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, Область.Имя);
			КонецЕсли;
		ИначеЕсли СтрНайти(Область.Имя, "П000000300103_") = 1 Или СтрНайти(Область.Имя, "П000000300105_") = 1 Тогда
...
Процедура ОтфильтроватьЕдиницыИзмеренияПоКодуОКПД(КодОКПД, ИмяСписка)
	...
	Если Таб1.Количество() > 0 Тогда
		ЗначениеВДанныеФормы(Таб1, РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ЕИОКПОтфильтрованный").ТаблицаЗначений);
	Иначе
		ЗначениеВДанныеФормы(Результат[2].Выгрузить(), РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ЕИОКПОтфильтрованный").ТаблицаЗначений);
	КонецЕсли;
	
	НоваяСтрока = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ЕИОКПОтфильтрованный").ТаблицаЗначений.Вставить(0);
	НоваяСтрока.Код = "";
...
Функция ЗаполнитьАвтоНаСервере()
	...
	КонецЦикла;
	Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок1", РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтотОбъект, П0000003001, Ложь));
