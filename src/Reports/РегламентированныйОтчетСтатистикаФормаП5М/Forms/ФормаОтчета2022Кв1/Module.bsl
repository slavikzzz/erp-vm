...
Процедура СформироватьСпискиВыбора()
	...
	Для Каждого Область Из МакетСоставаПоказателей.Области Цикл
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
			...
			ТаблСписка.Колонки.Добавить(
				"НазваниеДоп", Строка128, , МакетСоставаПоказателей.Область(ВерхОбласти, 2, ВерхОбласти, 2).ШиринаКолонки);
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(
		СвойстваПоказателей, "КодСтроки",   512, , "Выбор кода строки",   КоллекцияСписковВыбора["КодСтроки"]);
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если ПустаяСтрока(ПолеТабличногоДокументаФормаОтчета.Области.П001000703.Значение)
	   И ПустаяСтрока(ПолеТабличногоДокументаФормаОтчета.Области.П001000803.Значение) Тогда
		ПолеТабличногоДокументаФормаОтчета.Области.П001000703.Значение = "07";
		ПолеТабличногоДокументаФормаОтчета.Области.П001000803.Значение = "08";
	КонецЕсли;
	...
	Варианты = РегламентированнаяОтчетностьВызовСервера.ПолучитьТаблицуВариантовЗаполненияНаСервере(
	...
	Для Каждого ПоказателиСтраницы Из ПоказателиОтчета Цикл
		...
		Для Каждого Показатель Из ПоказателиТаблПоле Цикл
			...
			РегламентированнаяОтчетность.УстановитьЗначениеИФорматВПоле(
				ТекТабличноеПоле, ИмяПоказателя, ЗначениеПоказателя);
		КонецЦикла;
	КонецЦикла;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	РегламентированнаяОтчетность.УстановитьЦветФорматДоступностьЯчеек(ЭтотОбъект);
	РегламентированнаяОтчетность.ЗаполнитьДанныеВШапкеОбособленныхПодразделений(ЭтотОбъект); //нов
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 Тогда
		УдалитьСтроку("");
	ИначеЕсли Лев(Область.Имя, 8) = "П0010007" Или Лев(Область.Имя, 8) = "П0010008" Тогда
		СтандартнаяОбработка = Ложь;
		
		Если Лев(Область.Имя, 8) = "П0010007" Тогда
			СмежнаяОбластьИмя = "П001000803";
		Иначе
			СмежнаяОбластьИмя = "П001000703";
		КонецЕсли;
		СмежнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Область(СмежнаяОбластьИмя);
		
		Если СтрДлина(Область.Значение) = 4 Тогда
			Область.Значение = Сред(Область.Имя, 7, 2);
		Иначе
			Область.Значение = "(" + Сред(Область.Имя, 7, 2) + ")";
			СмежнаяОбласть.Значение = Сред(СмежнаяОбласть.Имя, 7, 2);
		КонецЕсли;
		
		Модифицированность = Истина;
	ИначеЕсли СтрНайти(Область.Имя, "П000000300102_") = 1 Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "КодСтроки");
		ПредыдущийПоказатель = Область.Значение;
		КодПоказателя = ПараметрВводаПоля.Показатель;
		КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь,
			ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка,, ПредыдущийПоказатель);
	ИначеЕсли Не СтруктураРеквизитовФормы.мАвтоВыборКодов И Область.Защита = Ложь И СтрНайти(Область.Имя, "П0000003001") = 1 Тогда 
		...
		Если СтрНайти(Область.Имя, "П000000300104_") = 1 Тогда
			Если ЗначВСтолбцеБ = "50" Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ОКП55");
			ИначеЕсли ЗначВСтолбцеБ = "70" Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ОКП6");
			ИначеЕсли ЗначВСтолбцеБ = "80" Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ОКП7");
			Иначе 
				Возврат;
			КонецЕсли;
			КодПоказателя = ПараметрВводаПоля.Показатель;
			КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
			ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь,
				ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, СтрЗаменить(Область.Имя, "П000000300104_", "П000000300101_"));
		ИначеЕсли СтрНайти(Область.Имя, "П000000300101_") = 1 Тогда 
			Если ЗначВСтолбцеБ = "50" Тогда
				...
				ВвестиПоказатель(Элемент, СмежнаяОбласть, КодПоказателя, Ложь,
					ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, Область.Имя);
			ИначеЕсли ЗначВСтолбцеБ = "70" Тогда
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ОКП6");
				СмежнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области["П000000300104_" + НомерСтроки];
				КодПоказателя = ПараметрВводаПоля.Показатель;
			...
			ИначеЕсли ЗначВСтолбцеБ = "80" Тогда 
				...
				ВвестиПоказатель(Элемент, СмежнаяОбласть, КодПоказателя, Ложь,
					ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, Область.Имя);
			КонецЕсли;
		ИначеЕсли СтрНайти(Область.Имя, "П000000300103_") = 1 Тогда
			ОсновнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области["П000000300105_" + НомерСтроки];
			...
			Если ЗначВСтолбцеБ = "50" Тогда
				...
				ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ЕИОКПОтфильтрованный");
				КодПоказателя = ПараметрВводаПоля.Показатель;
				КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
				ВвестиПоказатель(Элемент, ОсновнаяОбласть, КодПоказателя, Ложь,
					ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, СмежнаяОбласть.Имя);
			ИначеЕсли ЗначВСтолбцеБ = "70" Тогда
				ОтборЗначение = ПолеТабличногоДокументаФормаОтчета.Области["П000000300104_" + НомерСтроки].Значение;
...
Процедура ОтфильтроватьЕдиницыИзмеренияПоКодуОКПД(КодОКПД, ИмяСписка)
	...
	СписокЕдиницИзмерения = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, ИмяСписка).ТаблицаЗначений;
	...
	Если Таб1.Количество() > 0 Тогда
		ЗначениеВДанныеФормы(Таб1,
			РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "ЕИОКПОтфильтрованный").ТаблицаЗначений);
	Иначе
		ЗначениеВДанныеФормы(Результат[2].Выгрузить(), РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "ЕИОКПОтфильтрованный").ТаблицаЗначений);
	КонецЕсли;
	
	НоваяСтрока = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
		ЭтотОбъект, "ЕИОКПОтфильтрованный").ТаблицаЗначений.Вставить(0);
	НоваяСтрока.Код = "";
...
Процедура ОбнулитьНедоступные(НомерСтроки)
	...
	Для ДляКол = 1 По 11 Цикл
		...
		КонецЕсли;
		Ячейка = ПолеТабличногоДокументаФормаОтчета.Области[Префикс + Формат(ДляКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_" + НомерСтроки];
		Если Ячейка.Защита И (ДляКол <> 3)Тогда 
			Ячейка.Значение = Неопределено;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-5 (м)");
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ОтчПериод  = (Месяц(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) / 3);
	...
	ПараметрыВыгрузки.Вставить("Зн_П001000703", ?(ТабДокумент.П001000703 = "(07)", "07", "08"));
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, ОбластьНаименование = "", ПредыдущийПоказатель = Неопределено) Экспорт
	...
	ДополнительныеПараметры = Новый Структура("Область, ОбластьНаименование, ПредыдущийПоказатель",
		Область, ОбластьНаименование, ПредыдущийПоказатель);
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ОбластьНаименование) Тогда
		...
		Если СтрНайти(ОбластьНаименование, "П000000300101_") = 1 Тогда 
			...
			ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(ОбластьНаименование, "П000000300101_", "П000000300105_")].Значение = Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОткрытьФормуНастроек(Команда)
	...
	Если ФормаНастройкиОтчета.Открыта() Тогда
		...
		ДополнительныеПараметры =
			Новый Структура("ФормаНастройкиОтчета, ЗначениеАвторасчета", ФормаНастройкиОтчета, ЗначениеАвторасчета);
		ОписаниеОповещения =
			Новый ОписаниеОповещения("ВопросЗавершитьПредыдущуюОперациюЗавершение", ЭтотОбъект, ДополнительныеПараметры);
