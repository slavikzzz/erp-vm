...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если ЕстьАвтозаполнениеТЧ Тогда
		...
		Пока Истина Цикл 
			Строка = Строка + 1;
			Для Гр = 1 По 12 Цикл
				ИмяОбласти = "П0000003001" + Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=") + "_" + Формат(Строка, "ЧГ=");
				Область = ПолеТабличногоДокументаФормаОтчета.Области.Найти(ИмяОбласти);
				Если Область = Неопределено Тогда
					Прервать;
				КонецЕсли;
				Область.Защита = Ложь;
				Область.ЦветФона = мСтруктураЦветовРаскраски._3;
			КонецЦикла;
			
			Если ПолеТабличногоДокументаФормаОтчета.Области.Найти("П000000300101_" + Формат(Строка, "ЧГ=")) = Неопределено Тогда 
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
