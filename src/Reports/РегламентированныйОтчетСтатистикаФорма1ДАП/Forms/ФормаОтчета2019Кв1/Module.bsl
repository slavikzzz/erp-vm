...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = "0610019";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	Если Год(Параметры.мДатаНачалаПериодаОтчета) = 2019 Тогда
		ИмяМакетаАтр           = "АтрибВыгрузкиXML2019Кв1";
		ИмяМакетаСхемыВыгрузки = "СхемаВыгрузкиXML2019Кв1";
		ИмяМакетаШаблон        = "ШаблонРосстата2019Кв1";
	Иначе
		ИмяМакетаАтр           = "АтрибВыгрузкиXML2020Кв1";
		ИмяМакетаСхемыВыгрузки = "СхемаВыгрузкиXML2020Кв1";
		ИмяМакетаШаблон        = "ШаблонРосстата2020Кв1";
	КонецЕсли;
	...
	СтруктураРеквизитовФормы["ИмяМакетаШаблона"] = ИмяМакетаШаблон;
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если Не БезОткрытияФормы Тогда
		// В случае подачи отчета не за июнь и не за декабрь, установим защиту и цвет по умолчанию для раздела 11 Формы 1-ДАП.
		ДоступностьРазделов();
	КонецЕсли;
...
Процедура ДоступностьРазделов()
	...
	Если Не (Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6
		Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12) Тогда
		Для НомСтр = 1 По 5 Цикл
			ПолеТабличногоДокументаФормаОтчета.Области["П000011"
				+ Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области["П000011"
				+ Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"].Защита = Истина;
		КонецЦикла;
	КонецЕсли;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если СтруктураРеквизитовФормы.мСкопированаФорма <> Неопределено Тогда
		Если Не (Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6 
			Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12) Тогда
			Для НомСтр = 1 По 5 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П000011" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"].Значение = "";
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВосстановитьДанныеОтчетаСтарогоОбразца(ПоказателиОтчета)
	...
	Для Каждого Показатель Из ПоказателиТаблПоле Цикл
		...
		Если (ЧастьПоказателя1 = "П00020")
			И (ЧастьПоказателя2 >= "01" И ЧастьПоказателя2 <= "12")
			И (ЧастьПоказателя3 >= "01" И ЧастьПоказателя3 <= "06") Тогда
			ИмяНовогоПоказателя = ПрефиксИмениПоказателя
				+ "П0002021"
				+ ЧастьПоказателя3
		...
		ИначеЕсли (ЧастьПоказателя1 = "П00050") И (ЧастьПоказателя2 >= "01" И ЧастьПоказателя2 <= "11") Тогда
			...
			ИначеЕсли ЧастьПоказателя3 >= "06_1" И ЧастьПоказателя3 <= "06_3" Тогда
				ИмяНовогоПоказателя = ПрефиксИмениПоказателя
					+ "П0005001"
					+ Формат(Число(Прав(ЧастьПоказателя3, 1)) + 5, "ЧЦ=2; ЧВН=; ЧГ=")
					+ "_" + Формат(Число(ЧастьПоказателя2), "ЧГ=");
			ИначеЕсли ЧастьПоказателя3 >= "07" И ЧастьПоказателя3 <= "10" Тогда
				ИмяНовогоПоказателя = ПрефиксИмениПоказателя
				+ "П0005001"
				+ Формат(Число(ЧастьПоказателя3) + 2, "ЧЦ=2; ЧВН=; ЧГ=")
				+ "_" + Формат(Число(ЧастьПоказателя2), "ЧГ=");
			КонецЕсли
		КонецЕсли;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ТекГруппа = "П000002" Тогда
			СтандартнаяОбработка = Ложь;
			УстановитьЗначокВГруппеЯчеек(Область, 3, 3, 0);
			Модифицированность = Истина;
		ИначеЕсли ТекГруппа = "П000003" Тогда
			СтандартнаяОбработка = Ложь;
			УстановитьЗначокВГруппеЯчеек(Область, 3, 3, 1);
			Модифицированность = Истина;
		ИначеЕсли ТекГруппа = "П000004" Тогда
			СтандартнаяОбработка = Ложь;
			Для НомКол = 1 по 8 Цикл
				Если Область.Имя = "П0000040001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") Тогда
					мФормаОтчета.Области["П0000040001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
					= ?(мФормаОтчета.Области["П0000040001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X", "", "X");
				Иначе
					мФормаОтчета.Области["П0000040001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
				КонецЕсли;
			КонецЦикла;
		...
		ИначеЕсли ТекГруппа = "П000007" Тогда
			...
			Для НомКол = 2 по 5 Цикл
				Если Область.Имя = "П0000070001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") Тогда
					мФормаОтчета.Области["П0000070001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение 
					= ?(мФормаОтчета.Области["П0000070001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X", "", "X");
					Если мФормаОтчета.Области["П0000070001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X" Тогда
						мФормаОтчета.Области["П0000070002" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
						мФормаОтчета.Области["П0000070003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					КонецЕсли;
				КонецЕсли;
				Если Область.Имя = "П0000070002" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") Тогда
					мФормаОтчета.Области["П0000070002" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение 
					= ?(мФормаОтчета.Области["П0000070002" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X", "", "X");
					Если мФормаОтчета.Области["П0000070002" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X" Тогда
						мФормаОтчета.Области["П0000070001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
						мФормаОтчета.Области["П0000070003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					КонецЕсли;
				КонецЕсли;
				Если Область.Имя = "П0000070003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") Тогда
					мФормаОтчета.Области["П0000070003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение 
					= ?(мФормаОтчета.Области["П0000070003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X", "", "X");
					Если мФормаОтчета.Области["П0000070003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X" Тогда
						мФормаОтчета.Области["П0000070001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
						мФормаОтчета.Области["П0000070002" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					КонецЕсли;
				КонецЕсли;
		...
		ИначеЕсли ТекГруппа = "П000010" Тогда
			...
			Для НомСтр = 1 по 13 Цикл
				Если Область.Имя = "П000010" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02" Тогда
					мФормаОтчета.Области["П000010" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"].Значение
					= ?(мФормаОтчета.Области["П000010" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"].Значение = "X", "", "X");
				КонецЕсли;
			КонецЦикла;
			Модифицированность = Истина;
		ИначеЕсли ТекГруппа = "П000011" Тогда
			СтандартнаяОбработка = Ложь;
			Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6
				Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
				Для НомСтр = 1 по 5 Цикл
					Если Область.Имя = "П000011" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02" Тогда
						мФормаОтчета.Области["П000011" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"].Значение
						= ?(мФормаОтчета.Области["П000011" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"].Значение = "X", "", "X");
					КонецЕсли;
				КонецЦикла;
				Модифицированность = Истина;
			КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-ДАП");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1ДАП");
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	РегламентированнаяОтчетность.ОбработатьУсловныеЭлементы(
		ЭтотОбъект, ПараметрыВыгрузки, ДеревоВыгрузки); // формируем точную структуру XML-макета
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	// По разделу 4.
	НомКол = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П0000040001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")) Цикл
		ПараметрыВыгрузки.Вставить("П0000040001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч", 
		?(ТабДокумент["П0000040001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = "X", 1, 0));
		НомКол = НомКол + 1;
	КонецЦикла;
	// По разделу 5.
	НомКол = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П0000050001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")) Цикл
		ПараметрыВыгрузки.Вставить("П0000050001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч", 
		?(ТабДокумент["П0000050001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = "X", 1, 0));
		НомКол = НомКол + 1;
	КонецЦикла;
	// По разделу 6.
	ПараметрыВыгрузки.Вставить("П000006000101_Ч", ТабДокумент["П000006000101"]);	
	// По разделу 7.
	...
	// По разделу 9.
	НомКол = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П0000090001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")) Цикл
		ПараметрыВыгрузки.Вставить("П0000090001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч", 
		?(ТабДокумент["П0000090001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = "X", 1, 0));
		НомКол = НомКол + 1;
	КонецЦикла;
	// По разделу 10.
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000010" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02") Цикл
		ПараметрыВыгрузки.Вставить("П000010" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02" + "_Ч", 
		?(ТабДокумент["П000010" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"] = "X", 1, 0));
		НомСтр = НомСтр + 1;
	КонецЦикла;
	// По разделу 11.
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000011" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02") Цикл
		ПараметрыВыгрузки.Вставить("П000011" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02" + "_Ч", 
		?(ТабДокумент["П000011" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"] = "X", 1, 0));
		НомСтр = НомСтр + 1;
	КонецЦикла;
