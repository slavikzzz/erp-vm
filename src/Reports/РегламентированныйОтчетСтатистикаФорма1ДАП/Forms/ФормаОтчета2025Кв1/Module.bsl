...
Процедура ДоступностьРазделов()
	...
	Если Не (Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 3
		...
		Для Раздел = 6 По 9 Цикл
			РазделСтр = Формат(Раздел, "ЧЦ=3; ЧВН=");
			...
			Для Гр = 1 По ГрКон Цикл
				...
				Для НомСтр = 1 По СтрКон Цикл
					ИмяЯчейки = "П" + РазделСтр + "001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Графа;
	...
	Если Не (Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6
		...
		Для Раздел = 10 По 11 Цикл
			РазделСтр = Формат(Раздел, "ЧЦ=3; ЧВН=");
			
			Если Раздел = 10 Тогда
				ГрКон = 1;
				СтрКон = 6;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ТекГруппа = "П001001" Или  ТекГруппа = "П005001" Или ТекГруппа = "П007001" Тогда
			...
			КонецЕсли;
			Модифицированность = Истина;
		ИначеЕсли ТекГруппа = "П002001" Тогда
			СтандартнаяОбработка = Ложь;
			УстановитьЗначокВГруппеЯчеек(Область, 1, 1, 1, 8);
			Модифицированность = Истина;
		ИначеЕсли ТекГруппа = "П003001" Тогда
			СтандартнаяОбработка = Ложь;
			УстановитьЗначокВГруппеЯчеек(Область, 1, 1, 1, 3);
			Модифицированность = Истина;
		ИначеЕсли ТекГруппа = "П006001" Или ТекГруппа = "П008001" Тогда
			СтандартнаяОбработка = Ложь;
			НомерСтроки = Число(Сред(Область.Имя, 10, 2));
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 1, 3);
			Модифицированность = Истина;
		ИначеЕсли ТекГруппа = "П009001" Или ТекГруппа = "П010001" Или ТекГруппа = "П011001" Тогда
			СтандартнаяОбработка = Ложь;
			НомерСтроки = Число(Сред(Область.Имя, 10, 2));
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 1, 1);
			Модифицированность = Истина;
		КонецЕсли;
...
Процедура УстановитьЗначокВГруппеЯчеек(Область, НачальнаяСтрокаВГруппе, КонечнаяСтрокаВГруппе, НачальнаяКолонкаВГруппе, КонечнаяКолонкаВГруппе, МножественныйВыбор = Ложь)
	...
	Для Строка = НачальнаяСтрокаВГруппе По КонечнаяСтрокаВГруппе Цикл
		Для Колонка = НачальнаяКолонкаВГруппе По КонечнаяКолонкаВГруппе Цикл
			ТекЯчейка = Раздел + Прав("000" + Строка, 4) + Прав("0" + Колонка, 2);
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Для Гр = 1 По 9 Цикл
		...
		Для Стр = 1 По 7 Цикл
			Если (Стр = 4 И Гр >= 4)
			 Или (Стр >= 5 И Гр <= 3) Тогда
				Продолжить;
			КонецЕсли;
			
			НомСтр = Формат(Стр, "ЧЦ=4; ЧВН=; ЧГ=");
			НаборПоказателей = НаборПоказателей + "П001001" + НомСтр + Графа + ";";
		КонецЦикла;
	КонецЦикла;
	...
	Для Гр = 1 По 8 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		НаборПоказателей = НаборПоказателей + "П0020010001" + Графа + ";";
	КонецЦикла;
	
	// По разделу 3
	Для Гр = 1 По 3 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		НаборПоказателей = НаборПоказателей + "П0030010001" + Графа + ";";
	КонецЦикла;
	
	// По разделу 5
	Для Гр = 1 По 9 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		НаборПоказателей = НаборПоказателей + "П0050010001" + Графа + ";";
	КонецЦикла;
	...
	Для Гр = 1 По 3 Цикл
		...
		Для Стр = 1 По 4 Цикл
			НомСтр = Формат(Стр, "ЧЦ=4; ЧВН=; ЧГ=");
			НаборПоказателей = НаборПоказателей + "П006001" + НомСтр + Графа + ";";
		КонецЦикла;
	КонецЦикла;
	...
	Для Гр = 1 По 9 Цикл
		...
		Для Стр = 1 По 3 Цикл
			НомСтр = Формат(Стр, "ЧЦ=4; ЧВН=; ЧГ=");
			НаборПоказателей = НаборПоказателей + "П007001" + НомСтр + Графа + ";";
		КонецЦикла;
	КонецЦикла;
	...
	Для Гр = 1 По 3 Цикл
		...
		Для Стр = 1 По 2 Цикл
			НомСтр = Формат(Стр, "ЧЦ=4; ЧВН=; ЧГ=");
			НаборПоказателей = НаборПоказателей + "П008001" + НомСтр + Графа + ";";
		КонецЦикла;
	КонецЦикла;
	...
	Для Стр = 1 По 11 Цикл
		НомСтр = Формат(Стр, "ЧЦ=4; ЧВН=; ЧГ=");
		НаборПоказателей = НаборПоказателей + "П009001" + НомСтр + "01;";
	КонецЦикла;
	
	// По разделу 10
	Для Стр = 1 По 6 Цикл
		НомСтр = Формат(Стр, "ЧЦ=4; ЧВН=; ЧГ=");
		НаборПоказателей = НаборПоказателей + "П010001" + НомСтр + "01;";
	КонецЦикла;
	
	// По разделу 11
	Для Стр = 1 По 5 Цикл
		НомСтр = Формат(Стр, "ЧЦ=4; ЧВН=; ЧГ=");
		НаборПоказателей = НаборПоказателей + "П011001" + НомСтр + "01;";
	КонецЦикла;
	...
	Для Каждого ЭлМасс Из МассивПоказателей Цикл
		ПараметрыВыгрузки.Вставить(ЭлМасс, ?(ЗначениеЗаполнено(ТабДокумент[ЭлМасс]), "1", "0"));
	КонецЦикла;
