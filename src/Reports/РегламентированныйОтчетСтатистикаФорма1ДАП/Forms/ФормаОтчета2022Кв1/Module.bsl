...
Процедура ДоступностьРазделов()
	...
	Если Не (Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6
		...
		Для НомСтр = 1 По 6 Цикл
			ПолеТабличногоДокументаФормаОтчета.Области["П000001"
...
Процедура ПоказатьПериод()
	...
	Если Периодичность = Перечисления.Периодичность.Месяц Тогда
		...
		СтруктураРеквизитовФормы.СтрПериодОтчета =
			НРег(Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'ММММ гггг'")) + " года";
	Иначе
		СтрПериод = "" + Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) / 3 + " квартал "
			+ Формат(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ЧГ=") + " года";
		ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета.Значение = "за " + СтрПериод;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если СтруктураРеквизитовФормы.мСкопированаФорма <> Неопределено Тогда
		Если Не (Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6 
			Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12) Тогда
			Для НомСтр = 1 По 6 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"].Значение = "";
			КонецЦикла;
			Для НомСтр = 1 По 5 Цикл
	...
	Иначе
		...
		Если ТекГруппа = "П000001" Тогда
			...
			Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6
				Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
				Для НомСтр = 1 по 6 Цикл
					Если Область.Имя = "П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02" Тогда
						мФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"].Значение
						= ?(мФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"].Значение = "X", "", "X");
					КонецЕсли;
				КонецЦикла;
				Модифицированность = Истина;
			КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-ДАП");
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	// По разделу 1.
	Для Стр = 1 По 6 Цикл
		НаборПоказателей = НаборПоказателей + "П000001000" + Стр + "02;";
	КонецЦикла;
	
	// По разделу 2.
	Для Гр = 1 По 9 Цикл
		НаборПоказателей = НаборПоказателей + "П00000200010" + Гр + ";";
	КонецЦикла;
	...
	Для Гр = 2 По 10 Цикл
		...
		Для Стр = 1 По 8 Цикл
			...
			НаборПоказателей = НаборПоказателей + "П00000300" + НомСтр + Графа + ";";
		КонецЦикла;
	КонецЦикла;
	
	// По разделу 4.
	Для Гр = 1 По 8 Цикл
		НаборПоказателей = НаборПоказателей + "П00000400010" + Гр + ";";
	КонецЦикла;
	
	// По разделу 5.
	Для Гр = 1 По 3 Цикл
		НаборПоказателей = НаборПоказателей + "П00000500010" + Гр + ";";
	КонецЦикла;
	...
	Для Гр = 2 По 5 Цикл
		...
		Для Стр = 1 По 3 Цикл
			НомСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
			НаборПоказателей = НаборПоказателей + "П00000700" + НомСтр + Графа + ";";
		КонецЦикла;
	КонецЦикла;
	...
	Для Гр = 1 По 6 Цикл
		...
		Для Стр = 1 По 4 Цикл
			НомСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
			НаборПоказателей = НаборПоказателей + "П00000800" + НомСтр + Графа + ";";
		КонецЦикла;
	КонецЦикла;
	...
	Для Гр = 1 По 9 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		НаборПоказателей = НаборПоказателей + "П0000090001" + Графа + ";";
	КонецЦикла;
	
	// По разделу 10.
	Для Стр = 1 По 13 Цикл
		НомСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
		НаборПоказателей = НаборПоказателей + "П00001000" + НомСтр + "02;";
	КонецЦикла;
	
	Для Стр = 1 По 5 Цикл
		НомСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
		НаборПоказателей = НаборПоказателей + "П00001100" + НомСтр + "02;";
	КонецЦикла;
	
	МассивПоказателей = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(НаборПоказателей, ";", Истина);
	Для Каждого ЭлМасс Из МассивПоказателей Цикл
		ПараметрыВыгрузки.Вставить(ЭлМасс, ?(ТабДокумент[ЭлМасс] = "X", "1", "0"));
	КонецЦикла;
