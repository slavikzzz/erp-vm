...
Процедура ОбработкаПериодичностьОтчета(Форма)
	...
	Если Форма.мДатаКонцаПериодаОтчета < '20220101' Или Форма.мДатаКонцаПериодаОтчета >= '20230101' Тогда
	...
	Если Форма.ПолеВыбораПериодичность = Форма.ПеречислениеПериодичностьКвартал Тогда
		СтрПериодОтчета = ПредставлениеПериода(НачалоДня(Форма.мДатаНачалаПериодаОтчета), КонецКвартала(Форма.мДатаКонцаПериодаОтчета), "ФП = Истина" )
	Иначе
...
Процедура ПолеВыбораПериодичностьПриИзменении(Элемент)
	мПериодичность = ПолеВыбораПериодичность;
	Если ЭтотОбъект.ПолеВыбораПериодичность = ЭтотОбъект.ПеречислениеПериодичностьКвартал Тогда
		ЭтотОбъект.мДатаКонцаПериодаОтчета  = КонецКвартала(ЭтотОбъект.мДатаКонцаПериодаОтчета);
		ЭтотОбъект.мДатаНачалаПериодаОтчета = НачалоКвартала(ЭтотОбъект.мДатаКонцаПериодаОтчета);
	Иначе
		ЭтотОбъект.мДатаКонцаПериодаОтчета  = КонецМесяца(ЭтотОбъект.мДатаКонцаПериодаОтчета);
		ЭтотОбъект.мДатаНачалаПериодаОтчета = НачалоМесяца(ЭтотОбъект.мДатаКонцаПериодаОтчета);
	КонецЕсли;
	ОбработкаПериодичностьОтчета(ЭтотОбъект);
КонецПроцедуры
