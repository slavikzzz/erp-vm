...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы2 = "П0000401001";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("ИмуществоКазны");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ПолныйОКВЭД",
		512, , "Выбор кода ОКВЭД",   КоллекцияСписковВыбора["ПолныйОКВЭД"]);
КонецПроцедуры
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ВедетсяУчетИмуществаКазны = Истина;
	...
	ВедетсяУчетИмуществаКазны = Ложь;
	...
	Если Не ВедетсяУчетИмуществаКазны Тогда
		ПолеТабличногоДокументаФормаОтчета.УдалитьОбласть(
			ПолеТабличногоДокументаФормаОтчета.Область("НастройкиЗаполнения"), ТипСмещенияТабличногоДокумента.ПоВертикали);
	КонецЕсли;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	СписокСохранения.Свойство("ИмуществоКазны", СтруктураРеквизитовФормы.ИмуществоКазны);
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	Если Не ВедетсяУчетИмуществаКазны Тогда 
		ПолеТабличногоДокументаФормаОтчета.УдалитьОбласть(ПолеТабличногоДокументаФормаОтчета.Область("НастройкиЗаполнения"), ТипСмещенияТабличногоДокумента.ПоВертикали);
	КонецЕсли;
	
	П0000201001.Очистить();
	П0000401001.Очистить();
	РегламентированнаяОтчетность.НачальныеОперацииСМногострочнымиБлоками(ЭтотОбъект);
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 12 Цикл
		...
		Области["П0000101009" + ГрСтр].Значение = Области["П0000101010" + ГрСтр].Значение
			+ Области["П0000101012" + ГрСтр].Значение;
		
		Области["П0000101005" + ГрСтр].Значение = Области["П0000101006" + ГрСтр].Значение
			+ Области["П0000101007" + ГрСтр].Значение
			+ Области["П0000101008" + ГрСтр].Значение;
		
		Области["П0000101001" + ГрСтр].Значение = Области["П0000101002" + ГрСтр].Значение
			+ Области["П0000101004" + ГрСтр].Значение
			+ Области["П0000101005" + ГрСтр].Значение
			+ Области["П0000101009" + ГрСтр].Значение
			+ Области["П0000101013" + ГрСтр].Значение
			+ Области["П0000101019" + ГрСтр].Значение;
	КонецЦикла;
...
Процедура ПеренумероватьСтроки(Форма)
	Области = Форма.ПолеТабличногоДокументаФормаОтчета.Области;
	Области["П000020100102_1"].Значение = "15";
	Инд = 1;
	Пока Истина Цикл
		Инд = Инд + 1;
		...
		КонецЕсли;
		Если Инд > 4 Тогда
			Обл.Значение = "18." + Формат((Инд - 4), "ЧГ=");
		Иначе
			Обл.Значение = 15 + (Инд - 1);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 И Область.Гиперссылка Тогда
		УдалитьСтроку("");
	ИначеЕсли Область.Имя = "ИмуществоКазны" Тогда 
		СтандартнаяОбработка = Ложь;
		Область.Значение = Не Область.Значение;
		СтруктураРеквизитовФормы.ИмуществоКазны = Область.Значение;
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П0000101") И Прав(Область.Имя, 2) = "13" Тогда
...
Функция ПередСохранением(КодИФНС = "не применимо", ПодобныйОтчетСуществует="", Вариант="", ОткрытьФормуВопросаПриИзмененииВидаДокументаНомераКорректировки="", Автосохранение="", ПоказателиОтчета="")
	...
	Если РегламентированнаяОтчетность.БылиИзмененыКлючевыеРеквизитыОтчета(ЭтотОбъект, КодИФНС)
		...
		Если ВедетсяУчетИмуществаКазны Тогда
			Попытка
				ПодобныйОтчетСуществует = Ложь;
				Вариант = СтруктураРеквизитовФормы.ВидДокумента * НомерКорректировки;
				Запрос = Новый Запрос;
				Запрос.УстановитьПараметр("мСохраненныйДок", ?(СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено, Неопределено, СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка));
				Запрос.УстановитьПараметр("ИсточникОтчета", Метаданные.Отчеты[Сред(Лев(СтрЗаменить(ИмяФормы, "Внешний", ""), СтрНайти(СтрЗаменить(ИмяФормы, "Внешний", ""), ".Форма.") - 1), 7)].Имя);
				
				Запрос.УстановитьПараметр("Организация", СтруктураРеквизитовФормы.Организация);
				Запрос.УстановитьПараметр("ДатаНачалаНач", НачалоДня(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета));
				Запрос.УстановитьПараметр("ДатаНачалаКон", КонецДня(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета));
				Запрос.УстановитьПараметр("ДатаКонцаНач", НачалоДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
				Запрос.УстановитьПараметр("ДатаКонцаКон", КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
				...
				Пока Выборка.Следующий() Цикл 
					...
					Если Не ДанныеОтчета.Свойство("ИмуществоКазны") Тогда 
						Продолжить;
					ИначеЕсли ДанныеОтчета.ИмуществоКазны = СтруктураРеквизитовФормы.ИмуществоКазны Тогда 
						ПодобныйОтчетСуществует = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант = Неопределено, ВидДокументаНомерКорректировкиИзменен = Неопределено, ПоказателиОтчета = Неопределено) Экспорт
	...
	СписокСохранения.Вставить("ИмуществоКазны", СтруктураРеквизитовФормы.ИмуществоКазны);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 11 (краткая)");
	...
	Область5_4 = ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть5_4");
	
	Если П0000401001.Количество() > 1 Тогда
		...
		Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
			...
			МассивПроверки = Новый Массив;
			МассивПроверки.Добавить(ТекущаяОбластьМногострочнойЧасти);
			МассивПроверки.Добавить(Область5_4);
			Если ПечатнаяФорма.ПроверитьВывод(МассивПроверки) Тогда
				ПечатнаяФорма.Вывести(ТекущаяОбластьМногострочнойЧасти);
			Иначе
				СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 11 (краткая)");
	...
	Иначе
		...
		Для Гр = 1 По 3 Цикл
			Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П00004010010" + Гр + "_1"].Значение) Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ПечатнаяФорма.Вывести(Область5_4);
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 11 (краткая)");
...
Процедура ФормироватьСтруктуруСтраницОтчета() Экспорт
	...
	СтрокаУровня1.Представление            = "Статистика: Форма 11 (краткая)";
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Инд = 1;
	Пока ПараметрыВыгрузки.Свойство("П000040100102_" + Формат(Инд, "ЧГ=")) Цикл 
		Инд = Инд + 1;
	КонецЦикла;
	ПараметрыВыгрузки.Вставить("П000040100102_" + Формат(Инд, "ЧГ="), ПараметрыВыгрузки["П000040100002"]);
	ПараметрыВыгрузки.Вставить("П000040100103_" + Формат(Инд, "ЧГ="), ПараметрыВыгрузки["П000040100003"]);
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	Если Контейнер.Свойство("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок1", ТаблицаГруппы)
	...
	Если Контейнер.Свойство("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок2", ТаблицаГруппы)
	   И ТаблицаГруппы <> Неопределено Тогда
	...
	РегламентированнаяОтчетность.УстановитьЦветФорматДоступностьЯчеек(ЭтотОбъект);
	ОблИмуществоКазны = ПолеТабличногоДокументаФормаОтчета.Области.Найти("ИмуществоКазны");
	Если ОблИмуществоКазны <> Неопределено Тогда
		Расчет(ЭтотОбъект);
		ОблИмуществоКазны.Значение = СтруктураРеквизитовФормы.ИмуществоКазны;
	КонецЕсли;
	Модифицированность = Истина;
	Доступность = Истина;
КонецПроцедуры
...
Функция ЗаполнитьАвтоНаСервере()
	...
	ПараметрыОтчета.Вставить("ИмуществоКазны",                      СтруктураРеквизитовФормы.ИмуществоКазны);
...
Процедура ВывестиМакетВТаблДокНаСервере()
	...
	Если Не ВедетсяУчетИмуществаКазны Тогда 
		ПолеТабличногоДокументаФормаОтчета.УдалитьОбласть(
			ПолеТабличногоДокументаФормаОтчета.Область("НастройкиЗаполнения"),
