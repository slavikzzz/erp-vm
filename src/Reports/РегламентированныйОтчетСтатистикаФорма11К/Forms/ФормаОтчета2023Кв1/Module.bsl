...
Процедура УстановитьЗначенияГрафы(ЭтотОбъект)
	...
	Для Стр = 2 По 13 Цикл
		...
		Области["П00001010" + СтрНом + "13"].Значение = 4;
	КонецЦикла;
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	Пока ПараметрыВыгрузки.Свойство("П000040100102_" + Формат(Инд, "ЧГ=")) Цикл
		Инд = Инд + 1;
	КонецЦикла;
	Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000040102603"]) Тогда
		ПараметрыВыгрузки.Вставить("П000040100102_" + Формат(Инд, "ЧГ="), ПараметрыВыгрузки["П000040102602"]);
		ПараметрыВыгрузки.Вставить("П000040100103_" + Формат(Инд, "ЧГ="), ПараметрыВыгрузки["П000040102603"]);
		Если ПараметрыВыгрузки.Свойство("ЧНП000040102602") Тогда
			ПараметрыВыгрузки.Вставить("ЧНП000040100102_" + Формат(Инд, "ЧГ="), ПараметрыВыгрузки["ЧНП000040102602"]);
		КонецЕсли;
	КонецЕсли;
