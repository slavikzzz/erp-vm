...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если Не ВедетсяУчетИмуществаКазны Тогда 
		ТабличныйДокумент.УдалитьОбласть(ТабличныйДокумент.Область("НастройкиЗаполнения"), ТипСмещенияТабличногоДокумента.ПоВертикали);
	КонецЕсли;
...
Функция ПередСохранением(КодИФНС, ПодобныйОтчетСуществует, Вариант, ОткрытьФормуВопросаПриИзмененииВидаДокументаНомераКорректировки, Автосохранение, ПоказателиОтчета)
	...
	Если РегламентированнаяОтчетность.БылиИзмененыКлючевыеРеквизитыОтчета(ЭтаФорма, КодИФНС)
		И РегламентированнаяОтчетность.СуществуетДокументСАналогичнымиРеквизитами(ЭтаФорма, "не применимо") Тогда
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	Если ИмяТекущейСтраницыПанели = "Титульный" Тогда
		...
		Для НомКол = 3 По 11 Цикл
			...
			Области["П000500" + НомКолСтр].Значение = Области["П000600" + НомКолСтр].Значение + Области["П000700" + НомКолСтр].Значение + Области["П000800" + НомКолСтр].Значение;
			Области["П000900" + НомКолСтр].Значение = Области["П001000" + НомКолСтр].Значение + Области["П001200" + НомКолСтр].Значение;
			Области["П000100" + НомКолСтр].Значение = Области["П000200" + НомКолСтр].Значение + Области["П000400" + НомКолСтр].Значение + Области["П000500" + НомКолСтр].Значение
													+ Области["П000900" + НомКолСтр].Значение + Области["П001300" + НомКолСтр].Значение + Области["П001400" + НомКолСтр].Значение;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Функция ПечатьНаСервере(ВидПечати)
	...
	НК = СтруктураРеквизитовФормы.мПечатныеФормы[0].Значение.Области.Найти("НастройкиЗаполнения");
	Если НК <> Неопределено Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы[0].Значение.УдалитьОбласть(НК, ТипСмещенияТабличногоДокумента.ПоВертикали);
	КонецЕсли;
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	Модифицированность = Истина;
	ОблИмуществоКазны = ТабличныйДокумент.Области.Найти("ИмуществоКазны");
	Если ОблИмуществоКазны <> Неопределено Тогда
		Расчет(ЭтотОбъект, "Титульный");
