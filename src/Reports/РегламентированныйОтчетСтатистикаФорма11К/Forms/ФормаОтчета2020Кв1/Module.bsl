...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "10/07/2019";
...
Процедура ПеренумероватьСтроки(Форма)
	...
	Пока Истина Цикл 
		...
		Если Обл = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		Обл.Значение = "17." + Формат((Инд - 2), "ЧГ=");
	КонецЦикла;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П001") И Прав(Область.Имя, 2) = "13" Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	ОблОбособленноеПодразделение = ПолеТабличногоДокументаФормаОтчета.Области.ОбособленноеПодразделение;
	Если ИсключитьДанныеОбособленныхПодразделений Тогда 
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Пока Истина Цикл 
		...
		Если Не ПараметрыВыгрузки.Свойство("П000102000103_" + ИндСтр, ОКВЭД) Тогда 
			Прервать;
		КонецЕсли;
		Если ЗначениеЗаполнено(ОКВЭД) Тогда 
			Для Гр = 3 По 13 Цикл
				ГрСтр = Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=");
				ПараметрыВыгрузки.Вставить("П0001020001" + ГрСтр, ПараметрыВыгрузки["П0001020001" + ГрСтр + "_" + ИндСтр]);
				ПараметрыВыгрузки["П0001020001" + ГрСтр + "_" + ИндСтр] = Неопределено;
				Если ПараметрыВыгрузки.Свойство("ЧНП0001020001" + ГрСтр + "_" + ИндСтр) Тогда 
					ПараметрыВыгрузки.Вставить("ЧНП0001020001" + ГрСтр, ПараметрыВыгрузки["ЧНП0001020001" + ГрСтр + "_" + ИндСтр]);
					ПараметрыВыгрузки["ЧНП0001020001" + ГрСтр + "_" + ИндСтр] = Неопределено;
				КонецЕсли;
			КонецЦикла;
			Прервать;
		КонецЕсли;
