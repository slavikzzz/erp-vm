...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "30/08/2018";
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	КолонкиСИзмНижнГраницей.Добавить(10);
	...
	ИменаПодчиненныхГрупп = Новый Массив;
	КолонкиСИзмНижнГраницей = Новый Массив;
	КолонкиСИзмНижнГраницей.Добавить(5);
...
Процедура ИнициализироватьМногоуровневыеРазделы()
	...
	Для каждого Элем Из мСтруктураМногоуровневыхРазделов Цикл
		...
		ИтоговыеСуммыПоКодамОпераций.Добавить(Новый Структура("ИННОрг,НалБазаИтого", "", 0));
		...
		Если мСтруктураМногоуровневыхРазделов.Свойство(Раздел.ИмяСтраницы) Тогда
			...
			Если ДеревоДанныхРазделУровень0.Строки.Количество() = 1 Тогда
				Если ДеревоДанныхРазделУровень0.Строки[0].Данные.Количество() = 0 Тогда
					...
					ТабличныйДокумент.Область("КодОперации").Значение = "1011431";
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СпискиВыбора2018Кв4");
	...
	Для Каждого Область Из МакетСоставаПоказателей.Области Цикл
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
			...
			ТаблСписка.Колонки.Добавить("Кратко",,,
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1200014", , ,
		"Выбор кода единицы измерения по ОКЕИ", КоллекцияСписковВыбора["КодыПоОКЕИ"]);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 8) = "П1000020" Тогда
		...
		ДопПараметры = Новый Структура("ОблКод,ОблНаим", Область, ТабличныйДокумент.Область("П1000021" + Постфикс));
		ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьФормуВыбораСтраныЗавершение", ЭтотОбъект, ДопПараметры);
		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений.Количество() > 0 Тогда
			...
			Если ТребРезультат <> 0 Тогда
				...
				Для Сч = 1 По РазмерТЗ Цикл
					...
					Рез = ?(ЭлементТЗ.Кратко = "", 0, Число(ЭлементТЗ.Кратко));
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ИмяПоказателя = "ИННОрг" Тогда
		ИННОрг = СтрЗаменить(ЗначПоказателя, " ", "");
		ДлинаИННОрг = СтрДлина(ИННОрг);
		Если ДлинаИННОрг = 10 ИЛИ ДлинаИННОрг = 0 Тогда
			Область.Значение = ИННОрг;
		Иначе
			Область.Значение = "";
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр(
			"ru = 'ИНН организации розничной торговли должен состоять из 10 цифр. Значение очищено.';
			|en = 'ИНН организации розничной торговли должен состоять из 10 цифр. Значение очищено.'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
	
	Если СтрНачинаетсяС(ИмяПоказателя, "П1100008") Тогда
		Если ЗначПоказателя > 16777215 Тогда
			Область.Значение = 0;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр(
			"ru = 'Номер фискального документа не должен превышать значения 16777215. Значение очищено.';
			|en = 'Номер фискального документа не должен превышать значения 16777215. Значение очищено.'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
	
	Если СтрНачинаетсяС(ИмяПоказателя, "П1200014") Тогда
		КодПоОКЕИ = СтрЗаменить(ЗначПоказателя, " ", "");
		ДлинаКодПоОКЕИ = СтрДлина(КодПоОКЕИ);
		Если ДлинаКодПоОКЕИ = 3 ИЛИ ДлинаКодПоОКЕИ = 4 ИЛИ ДлинаКодПоОКЕИ = 0 Тогда
			Область.Значение = КодПоОКЕИ;
		Иначе
			Область.Значение = "";
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр(
			"ru = 'Код единицы измерения по ОКЕИ должен состоять из 3 или 4 цифр. Значение очищено.';
			|en = 'Код единицы измерения по ОКЕИ должен состоять из 3 или 4 цифр. Значение очищено.'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы110 Тогда
		Подстрока = "кассовый чек";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный кассовый чек?';
							|en = 'Удалить выбранный кассовый чек?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы120 Тогда
		Подстрока = "сведения о товаре";
		ТекстВопроса = НСтр("ru = 'Удалить выбранные сведения о товаре?';
							|en = 'Удалить выбранные сведения о товаре?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПериодИзДекларацииНДС()
	...
	Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.ДекларацияНДС) Тогда
		...
		КонецЕсли;
		ВозможныеКодыНологовыхПериодов = СтрРазделить("21|22|23|24|51|54|55|56", "|", Ложь);
		Если ВозможныеКодыНологовыхПериодов.Найти(ПериодДекларацииНДС) = Неопределено Тогда
			ПериодДекларацииНДС = "";
		КонецЕсли;
	КонецЕсли;
...
Процедура ОткрытьФормуВыбораСтраныЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ОблаКод  = ДополнительныеПараметры.ОблКод;
	ОблаНаим = ДополнительныеПараметры.ОблНаим;
	
	Если Результат <> Неопределено Тогда
		РеквизитыЭлемента = РеквизитыЭлементаСправочника(Результат);
		Если ОблаКод.Значение = РеквизитыЭлемента.ТрехзначныйЦифровойКод Тогда
			ОблаКод.Очистить();
			ОблаНаим.Очистить();
		Иначе
			ОблаКод.Значение = РеквизитыЭлемента.ТрехзначныйЦифровойКод;
			ОблаНаим.Значение = РеквизитыЭлемента.КраткоеНаименование;
		КонецЕсли;
		Модифицированность = Истина;
	КонецЕсли;
...
Функция РеквизитыЭлементаСправочника(Результат)
	...
	ТрехзначныйЦифровойКод = "";
	КраткоеНаименование = "";
	
	РезультатЭтоСправочникСсылка = Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(Результат));
	Если РезультатЭтоСправочникСсылка Тогда
		ТрехзначныйЦифровойКод = Результат.Код;
		КраткоеНаименование = Результат.Наименование;
	КонецЕсли;
	
	Возврат Новый Структура("ТрехзначныйЦифровойКод, КраткоеНаименование", ТрехзначныйЦифровойКод, КраткоеНаименование);
...
Функция ДанныеТабличногоДокумента(НаимРаздела)
	...
	Если мСтруктураМногоуровневыхРазделов.Свойство(НаимРаздела) Тогда
		...
		СтруктураДанных.Вставить("ИННОрг", ТабличныйДокумент.Область("ИННОрг").Значение);
		СтруктураДанных.Вставить("НаименованиеОрг", ТабличныйДокумент.Область("НаименованиеОрг").Значение);
	...
	Если мСтруктураМногоуровневыхРазделов.Свойство(ИмяТекРаздела) Тогда
		...
		Если НЕ СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
			...
			Для каждого ЭлМассива Из СтруктураРеквизитовФормы.ИтоговыеСуммыПоКодамОпераций Цикл
				Если ЭлМассива["ИННОрг"] = ИННОрг Тогда
...
Процедура ДобавитьСтраницуНаСервере(ИмяРаздела)
	...
	Если ИмяРаздела = "Реестр" Тогда
		СтруктураРеквизитовФормы.ИтоговыеСуммыПоКодамОпераций.Добавить(
			Новый Структура("ИННОрг,НалБазаИтого", "", 0));
	КонецЕсли;
...
Функция ПревышеноДопустимоеКоличествоСтрокТабличногоДокумента(ИмяРаздела, ИмяГруппы)
	...
	ДопустимоеКоличествоСтрок = 5000;
	...
	Если ИмяРаздела = "Реестр" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя)
				...
				ИННОрг = Форма.ТабличныйДокумент.Области["ИННОрг"].Значение;
				СуммаНалБаз = 0;
				Если ЗначениеЗаполнено(ИННОрг) Тогда
					Ном1 = 1;
					Пока Форма.ТабличныйДокумент.Области.Найти("П1200019_" + Формат(Ном1, "ЧГ=") + "_1") <> Неопределено Цикл
						Ном2 = 1;
						Пока Форма.ТабличныйДокумент.Области.Найти(
							"П1200019_" + Формат(Ном1, "ЧГ=") + "_" + Формат(Ном2, "ЧГ=")) <> Неопределено Цикл
							СуммаНалБаз = СуммаНалБаз + Форма.ТабличныйДокумент.Области[
							"П1200019_" + Формат(Ном1, "ЧГ=") + "_" + Формат(Ном2, "ЧГ=")].Значение;
							Ном2 = Ном2 + 1;
						КонецЦикла;
						Ном1 = Ном1 + 1;
					КонецЦикла;
					Форма.СтруктураРеквизитовФормы.ИтоговыеСуммыПоКодамОпераций[НомСтраницы - 1]["ИННОрг"] = ИННОрг;
					Форма.СтруктураРеквизитовФормы.ИтоговыеСуммыПоКодамОпераций[НомСтраницы - 1]["НалБазаИтого"] = Окр(СуммаНалБаз);
					СуммаНалБаз = 0;
					...
					КонецЦикла;
				Иначе
					Форма.СтруктураРеквизитовФормы.ИтоговыеСуммыПоКодамОпераций[НомСтраницы - 1]["ИННОрг"] = "";
					Форма.СтруктураРеквизитовФормы.ИтоговыеСуммыПоКодамОпераций[НомСтраницы - 1]["НалБазаИтого"] = 0;
				КонецЕсли;
				Форма.ТабличныйДокумент.Области["НалБазаИтого"].Значение = Окр(СуммаНалБаз);
	...
	Если ИмяРаздела = "Реестр" Тогда
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если Ветка = АктивнаяВетка Тогда
				Ветка.Данные["ИННОрг"] = Форма.ТабличныйДокумент.Области["ИННОрг"].Значение;
				...
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					...
					Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
						Ветка_П12000.Данные.Вставить("П1200019", Форма.ТабличныйДокумент.Области["П1200019_"
			...
			Если ЗначениеЗаполнено(ИННОрг) Тогда
				...
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					ДанныеМнЧ_П12000 = Ветка_П10000.ДанныеМногострочныхЧастей.П12000.Строки;
					Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
						СуммаНалБаз = СуммаНалБаз + Ветка_П12000.Данные["П1200019"];
					КонецЦикла;
				КонецЦикла;
				Форма.СтруктураРеквизитовФормы.ИтоговыеСуммыПоКодамОпераций[НомСтраницы]["ИННОрг"] = ИННОрг;
				Форма.СтруктураРеквизитовФормы.ИтоговыеСуммыПоКодамОпераций[НомСтраницы]["НалБазаИтого"] = Окр(СуммаНалБаз);
				СуммаНалБаз = 0;
				Для каждого ЭлМассива Из Форма.СтруктураРеквизитовФормы.ИтоговыеСуммыПоКодамОпераций Цикл
					Если ЭлМассива["ИННОрг"] = ИННОрг Тогда
						СуммаНалБаз = СуммаНалБаз + ЭлМассива["НалБазаИтого"];
				...
				КонецЦикла;
			Иначе
				Форма.СтруктураРеквизитовФормы.ИтоговыеСуммыПоКодамОпераций[НомСтраницы]["ИННОрг"] = "";
				Форма.СтруктураРеквизитовФормы.ИтоговыеСуммыПоКодамОпераций[НомСтраницы]["НалБазаИтого"] = 0;
			КонецЕсли;
			Ветка.Данные["НалБазаИтого"] = Окр(СуммаНалБаз);
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("НалБазаИтого", "ИННОрг;П1200019");
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревьяДляСравнения.Вставить("ДеревоСравненияРеестра",  ИзвлечьСтруктуруXML("СхемаВыгрузки501"));
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревьяДляЗагрузки.Вставить("ДеревоЗагрузкиРеестра",  ИзвлечьСтруктуруXML("СхемаВыгрузки501"));
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	ДеревоВыгрузкиРеестра = ИзвлечьСтруктуруXML("СхемаВыгрузки501");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ДолжОтв", СокрЛП(Титульный.Должность));
	
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрЧек");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "ON_RRCHEKTF"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ПериодОтч) Тогда
		...
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.ПериодОтч) < 2
		ИЛИ СтрНайти(ВозможныеКодыПериода, ПараметрыВыгрузки.ПериодОтч) = 0 Тогда
	...
	// Особая проверка на одинаковые наименования организаций при одних и тех же ИНН.
	ТЗ = Новый ТаблицаЗначений;
	ТЗ.Колонки.Добавить("НомерСтраницы");
	ТЗ.Колонки.Добавить("ИННОрг");
	ТЗ.Колонки.Добавить("НаименованиеОрг");
	
	Для Инд = 0 По СтруктураРеквизитовФормы.ИтоговыеСуммыПоКодамОпераций.Количество() - 1 Цикл
		ИННОрг = СокрЛП(СтруктураРеквизитовФормы.ИтоговыеСуммыПоКодамОпераций[Инд].ИННОрг);
		НаименованиеОрг = СокрЛП(ДанныеРеестра.Строки[Инд].Данные.НаименованиеОрг);
		Если ЗначениеЗаполнено(ИННОрг) И ЗначениеЗаполнено(НаименованиеОрг) Тогда
			НовСтр = ТЗ.Добавить();
			НовСтр.НомерСтраницы = Формат(Инд + 1, "ЧГ=");
			НовСтр.ИННОрг = ИННОрг;
			НовСтр.НаименованиеОрг = НаименованиеОрг;
		КонецЕсли;
	КонецЦикла;
	
	ТЗ_ИННОрг = ТЗ.Скопировать();
	ТЗ_ИННОрг.Свернуть("ИННОрг");
	
	Если ТЗ.Количество() <> ТЗ_ИННОрг.Количество() Тогда
		
		Для каждого Стр_ИННОрг Из ТЗ_ИННОрг Цикл
			
			Мас_Стр_ТЗ = ТЗ.НайтиСтроки(Новый Структура("ИННОрг", Стр_ИННОрг.ИННОрг));
			
			Если Мас_Стр_ТЗ.Количество() > 1 Тогда
				
				ПредНаименованиеОрг = Неопределено;
				СледНаименованиеОрг = Неопределено;
				ЕстьРазныеНаименованияОрг = Ложь;
				ПредставленияСтраниц = "";
				Для Инд_Мас_Стр_ТЗ = 0 По Мас_Стр_ТЗ.Количество() - 1 Цикл
					ТекНаименованиеОрг = Мас_Стр_ТЗ[Инд_Мас_Стр_ТЗ]["НаименованиеОрг"];
					Если Инд_Мас_Стр_ТЗ > 0 Тогда
						ПредНаименованиеОрг = Мас_Стр_ТЗ[Инд_Мас_Стр_ТЗ - 1]["НаименованиеОрг"];
					Иначе
						ПредНаименованиеОрг = Неопределено;
					КонецЕсли;
					Если Инд_Мас_Стр_ТЗ < Мас_Стр_ТЗ.Количество() - 1 Тогда
						СледНаименованиеОрг = Мас_Стр_ТЗ[Инд_Мас_Стр_ТЗ + 1]["НаименованиеОрг"];
					Иначе
						СледНаименованиеОрг = Неопределено;
					КонецЕсли;
					ЕстьРазныеНаименованияОрг = ЕстьРазныеНаименованияОрг
					ИЛИ (ПредНаименованиеОрг <> Неопределено И ПредНаименованиеОрг <> ТекНаименованиеОрг)
					ИЛИ (СледНаименованиеОрг <> Неопределено И СледНаименованиеОрг <> ТекНаименованиеОрг);
					ПредставленияСтраниц = ПредставленияСтраниц + ", " + Мас_Стр_ТЗ[Инд_Мас_Стр_ТЗ].НомерСтраницы;
				КонецЦикла;
				ПредставленияСтраниц = Сред(ПредставленияСтраниц, 3);
				
				Если ЕстьРазныеНаименованияОрг Тогда
					СообщитьВТС("На страницах " + ПредставленияСтраниц
					+ " заданы различные наименования организации розничной торговли, выдавшей документ (чек) для компенсации суммы налога, при одинаковых значениях ИНН.",
					"Реестр", "НаименованиеОрг", Мас_Стр_ТЗ[0].НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Если ДлинаКодОперации <> 7 Тогда
			СообщитьВТС("Страница " + НомерСтраницы_Строка + ": не"
			+ ?(ДлинаКодОперации = 0, "", "верно") + " задан код операции.",
			"Реестр", "КодОперации", НомерСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		ДлинаИННОрг = СтрДлина(СокрЛП(Ветка.Данные["ИННОрг"]));
		Если ДлинаИННОрг <> 10 Тогда
			СообщитьВТС("Страница " + НомерСтраницы_Строка + ": не" + ?(ДлинаИННОрг = 0, "", "верно")
			+ " задан ИНН организации розничной торговли, выдавшей документ (чек) для компенсации суммы налога.",
			"Реестр", "ИННОрг", НомерСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(Ветка.Данные["НаименованиеОрг"]) Тогда
			СообщитьВТС("Страница " + НомерСтраницы_Строка
			+ ": не задано наименование организации розничной торговли, выдавшей документ (чек) для компенсации суммы налога.",
			"Реестр", "НаименованиеОрг", НомерСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000004"]) Тогда
				...
				+ " не задан порядковый номер документа (чека) для компенсации суммы налога.",
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000005"]) Тогда
				...
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " не задана дата документа (чека) для компенсации суммы налога.",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П10000.Данные["П1000005"]) Тогда
				ИмяПок = "П1000005" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " задана дата документа (чека) для компенсации суммы налога, выходящая за пределы периода с 01.01.1900 по 31.12.2099.",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ДлинаКодИнГос = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000020"]));
			Если ДлинаКодИнГос <> 3 Тогда
				ИмяПок = "П1000020" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не" + ?(ДлинаКодИнГос = 0, "", "верно")
				+ " задан цифровой код иностранного государства, уполномоченным органом которого выдан паспорт физическому лицу - гражданину иностранного государства.",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000021"]) Тогда
				ИмяПок = "П1000021" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " не задано краткое наименование иностранного государства, уполномоченным органом которого выдан паспорт физическому лицу - гражданину иностранного государства.",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000022"]) Тогда
				ИмяПок = "П1000022" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " не задана дата выплаты (перечисления) компенсации суммы налога иностранному физическому лицу.",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П10000.Данные["П1000022"]) Тогда
				ИмяПок = "П1000022" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " задана дата выплаты (перечисления) компенсации суммы налога иностранному физическому лицу, выходящая за пределы периода с 01.01.1900 по 31.12.2099.",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100006"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан порядковый номер кассового чека.",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				Если Ветка_П11000.Данные["П1100008"] > 16777215 Тогда
					ИмяПок = "П1100008" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " задан номер фискального документа, указанного в кассовом чеке, превышающий число 16777215.",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100010"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана дата кассового чека.",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П11000.Данные["П1100010"]) Тогда
					ИмяПок = "П1100010" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " задана дата кассового чека, выходящая за пределы периода с 01.01.1900 по 31.12.2099.",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200011"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан номер (артикул) товара, указанного в документе (чеке) для компенсации суммы налога.",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200012"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование реализованных товаров, указанных в документе (чеке) для компенсации суммы налога.",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ДлинаКодЕдИзм = СтрДлина(СокрЛП(Ветка_П12000.Данные["П1200014"]));
				Если НЕ (ДлинаКодЕдИзм = 3 ИЛИ ДлинаКодЕдИзм = 4) Тогда
					ИмяПок = "П1200014" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не" + ?(ДлинаКодЕдИзм = 0, "", "верно")
					+ " задана единица измерения, указанная в документе (чеке) для компенсации суммы налога.",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	Для каждого Ветка Из ДеревоДанных.Строки Цикл
		ИННОрг = СокрЛП(Ветка.Данные["ИННОрг"]);
		Если ЗначениеЗаполнено(ИННОрг) Тогда
			_ИННОрг = "_" + ИННОрг;
			Если НЕ УпорядоченныеДанныеДерева.Свойство(_ИННОрг) Тогда
				УпорядоченныеДанныеДерева.Вставить(_ИННОрг, Новый Массив);
			КонецЕсли;
			УпорядоченныеДанныеДерева[_ИННОрг].Добавить(ДеревоДанных.Строки.Индекс(Ветка));
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрЧек = СкопироватьУзел(УзелРодитель.Родитель, УзелРодитель);
		
		Для каждого Эл Из ЭкзРеестрЧек.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		СвОргТорг = ПолучитьПодчиненныйЭлемент(ЭкзРеестрЧек, "СвОргТорг");
		Для каждого Эл Из СвОргТорг.Строки Цикл
			ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
		КонецЦикла;
		
		СвЧек = ПолучитьПодчиненныйЭлемент(ЭкзРеестрЧек, "СвЧек");
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСвЧек = СкопироватьУзел(ЭкзРеестрЧек, СвЧек, ЭкзРеестрЧек.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСвЧек.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
						ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
					КонецЕсли;
				КонецЦикла;
				СвВыдЧек = ПолучитьПодчиненныйЭлемент(ЭкзСвЧек, "СвВыдЧек");
				Для каждого Эл Из СвВыдЧек.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
						ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
					КонецЕсли;
				КонецЦикла;
				...
				// 11000.
				СвКасЧек = ПолучитьПодчиненныйЭлемент(СвВыдЧек, "СвКасЧек");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					ЭкзСвКасЧек = СкопироватьУзел(СвВыдЧек, СвКасЧек, СвВыдЧек.Строки.Количество() - 1);
					Для каждого Эл Из ЭкзСвКасЧек.Строки Цикл
						Если (Эл.Код = "НомФН" ИЛИ Эл.Код = "ФПД") И НЕ ЗначениеЗаполнено(Ветка_П11000.Данные[Эл.Ключ]) Тогда
							ВывестиПоказательВXML(Эл, "1");
						ИначеЕсли Эл.Код = "НомФД" И НЕ ЗначениеЗаполнено(Ветка_П11000.Данные[Эл.Ключ]) Тогда
							ВывестиПоказательВXML(Эл, 1);
						Иначе
				...
				// 12000.
				СвТовПозЧек = ПолучитьПодчиненныйЭлемент(ЭкзСвЧек, "СвТовПозЧек");
				ДанныеМнЧ_П12000 = Ветка_П10000.ДанныеМногострочныхЧастей.П12000.Строки;
				Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
					ЭкзСвТовПозЧек = СкопироватьУзел(ЭкзСвЧек, СвТовПозЧек, ЭкзСвЧек.Строки.Количество() - 1);
					Для каждого Эл Из ЭкзСвТовПозЧек.Строки Цикл
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_ДокументыДляКомпенсацииНалога");
...
Процедура ВывестиНаПечатьОтчет(СтруктураПараметров, ВыбранныеСтраницы, СтрПоля = Неопределено)
	...
	Если ВыбранныеСтраницы.Найти("Титульный") <> Неопределено Тогда
		...
		ИмяМакета = "ПечатныйБланк2018Кв4_Титульный";
	...
	Если ВыбранныеСтраницы.Найти("Реестр") <> Неопределено Тогда
		...
		ИмяМакета = "ПечатныйБланк2018Кв4_Реестр";
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			СтрокаДокумент.Параметры.НомерПП = Формат(ДеревоДанныхРаздела.Строки.Индекс(Страница) + 1, "ЧГ=");
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				КолСтрок12 = Строки12.Количество();
				
				МаксКолСтрок11_12 = Макс(КолСтрок11, КолСтрок12);
				
				Для Инд = 1 По МаксКолСтрок11_12 Цикл
					...
					Если Инд <= КолСтрок11 Тогда
						СтрокаДокумент.Параметры.Заполнить(Строки11[Инд - 1].Данные);
					КонецЕсли;
					Если Инд <= КолСтрок12 Тогда
						СтрокаДокумент.Параметры.Заполнить(Строки12[Инд - 1].Данные);
					КонецЕсли;
				...
				Если НЕ ПечФорма.ПроверитьВывод(МассивПроверки) Тогда
					...
					ПерваяСтрокаДокументВерх = 0;
					
					Пока ПечФорма.Области.Количество() > 0 Цикл
						Если ПечФорма.Области[0].Имя = "ПодШапка" Тогда
							ПерваяСтрокаДокументВерх = ПечФорма.Области[0].Низ + 1;
						КонецЕсли;
						ПечФорма.Области[0].Имя = "";
					КонецЦикла;
					
					Если ПерваяСтрокаДокументВерх > 0 Тогда
						
						НомерКолонки = 2;
						ОблОбъед = ПечФорма.Область(ПерваяСтрокаДокументВерх, НомерКолонки, ПечФорма.ВысотаТаблицы, НомерКолонки);
						ОблОбъед.Объединить();
						...
						Обл = ПечФорма.Область(1, , ПечФорма.ВысотаТаблицы, );
						Обл.Имя = "_";
...
Процедура ПронумероватьСтрокиТабличнойЧасти(ИмяРаздела)
	...
	Если ИмяРаздела = "Реестр" Тогда
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если Ветка = АктивнаяВетка Тогда
				Ветка.Данные["ИННОрг"] = ТабличныйДокумент.Области["ИННОрг"].Значение;
			КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(ИННОрг) Тогда
				НомСтроки = 0;
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				КонецЦикла;
			Иначе
				ЭлемСписка = СписокМаксНомеровСтрокПоКодамОпераций.НайтиПоЗначению(ИННОрг);
				Если ЭлемСписка = Неопределено Тогда
					...
					Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
						...
						КонецЕсли;
					КонецЦикла;
					СписокМаксНомеровСтрокПоКодамОпераций.Добавить(ИННОрг, Формат(НомСтроки, "ЧГ="));
				Иначе
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если КодПоказателя = "П1200014" Тогда
		ПараметрыФормы.Вставить("ВключитьВидимостьКолонкиКратко", Истина);
		ПараметрыФормы.Вставить("НаимКолонкиНазвание", "Наименование единицы измерения");
		ПараметрыФормы.Вставить("НаимКолонкиКратко",   "Условное обозначение");
	КонецЕсли;
