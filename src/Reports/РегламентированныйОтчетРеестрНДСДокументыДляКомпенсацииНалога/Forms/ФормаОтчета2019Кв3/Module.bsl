...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "06/08/2019";
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	КолонкиСИзмНижнГраницей.Добавить(7);
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СпискиВыбора2019Кв3");
	...
	//Реестр.
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1200011", , ,
		"Выбор кода единицы измерения по ОКЕИ", КоллекцияСписковВыбора["КодыПоОКЕИ"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1000020", 1, ,
		"Выбор признака выплаты", КоллекцияСписковВыбора["ПризнакиВыплаты"]);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П1200013") И НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		
		СтандартнаяОбработка = Ложь;
		
		СписокСтавок = Новый СписокЗначений;
		СписокСтавок.Добавить("", "Пустое значение");
		СписокСтавок.Добавить("10");
		СписокСтавок.Добавить("10/110");
		СписокСтавок.Добавить("18");
		СписокСтавок.Добавить("18/118");
		СписокСтавок.Добавить("20");
		СписокСтавок.Добавить("20/120");
		...
		СписокСтавок.НайтиПоЗначению(СокрЛП(Область.Значение)));
		
	ИначеЕсли Лев(Область.Имя, 8) = "П1000017" Тогда
		...
		ДопПараметры = Новый Структура("ОблКод,ОблНаим", Область, ТабличныйДокумент.Область("П1000018" + Постфикс));
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если СтрНачинаетсяС(ИмяПоказателя, "П1200011") Тогда
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяТекРаздела)
	...
	ИначеЕсли ИмяТекРаздела = "Реестр" Тогда
		...
		ОбрабОбласть1 = Форма.ТабличныйДокумент.Области["ИННОрг"];
		ОбрабОбласть2 = Форма.ТабличныйДокумент.Области["НаименованиеОрг"];
		Если Форма.СтруктураРеквизитовФормы.НомераСтрокМногоуровнегоРаздела.Количество() > 0
			И Форма.СтруктураРеквизитовФормы.НомераСтрокМногоуровнегоРаздела[0] > 1 Тогда
			ЗапретитьВводДанныхВОбласть(Форма.мСтруктураЦветовРаскраски, ОбрабОбласть1, Ложь);
			ЗапретитьВводДанныхВОбласть(Форма.мСтруктураЦветовРаскраски, ОбрабОбласть2, Ложь);
		Иначе
			РазрешитьВводДанныхВОбласть(Форма.мСтруктураЦветовРаскраски, ОбрабОбласть1);
			РазрешитьВводДанныхВОбласть(Форма.мСтруктураЦветовРаскраски, ОбрабОбласть2);
		КонецЕсли;
...
Процедура ВывестиДанныеВТабличныйДокумент(ИмяТекРаздела, НовыеНомераСтрокМногоуровнегоРаздела = Неопределено)
	...
	Если мСтруктураМногоуровневыхРазделов.Свойство(ИмяТекРаздела) Тогда
		...
		ДанныеПервойВеткиРеестра = ДеревоДанныхРаздела.Строки[0].Данные;
		ТабличныйДокумент.Область("ИННОрг").Значение = СокрЛП(ДанныеПервойВеткиРеестра["ИННОрг"]);
		ТабличныйДокумент.Область("НаименованиеОрг").Значение = СокрЛП(ДанныеПервойВеткиРеестра["НаименованиеОрг"]);
...
Процедура ДобавитьСтраницуНаСервере(ИмяРаздела)
	...
	ДанныеПервойВеткиРеестра = ДеревоДанных.Строки[0].Данные;
	ТабличныйДокумент.Область("ИННОрг").Значение = СокрЛП(ДанныеПервойВеткиРеестра["ИННОрг"]);
	ТабличныйДокумент.Область("НаименованиеОрг").Значение = СокрЛП(ДанныеПервойВеткиРеестра["НаименованиеОрг"]);
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя)
				...
				Если ЗначениеЗаполнено(КодОперации) Тогда
					Ном1 = 1;
					Пока Форма.ТабличныйДокумент.Области.Найти("П1200016_" + Формат(Ном1, "ЧГ=") + "_1") <> Неопределено Цикл
						Ном2 = 1;
						Пока Форма.ТабличныйДокумент.Области.Найти(
							"П1200016_" + Формат(Ном1, "ЧГ=") + "_" + Формат(Ном2, "ЧГ=")) <> Неопределено Цикл
							СуммаНалБаз = СуммаНалБаз + Форма.ТабличныйДокумент.Области[
							"П1200016_" + Формат(Ном1, "ЧГ=") + "_" + Формат(Ном2, "ЧГ=")].Значение;
							Ном2 = Ном2 + 1;
						КонецЦикла;
						Ном1 = Ном1 + 1;
					КонецЦикла;
	...
	Если ИмяРаздела = "Реестр" Тогда
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если Ветка = АктивнаяВетка Тогда
				Ветка.Данные["КодОперации"] = Форма.ТабличныйДокумент.Области["КодОперации"].Значение;
				...
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					...
					Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
						Ветка_П12000.Данные.Вставить("П1200016", Форма.ТабличныйДокумент.Области["П1200016_"
			...
			Если ЗначениеЗаполнено(КодОперации) Тогда
				...
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					ДанныеМнЧ_П12000 = Ветка_П10000.ДанныеМногострочныхЧастей.П12000.Строки;
					Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
						СуммаНалБаз = СуммаНалБаз + Ветка_П12000.Данные["П1200016"];
					КонецЦикла;
				КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("НалБазаИтого", "КодОперации;П1200016");
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревьяДляСравнения.Вставить("ДеревоСравненияРеестра",  ИзвлечьСтруктуруXML("СхемаВыгрузки501_2"));
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревьяДляЗагрузки.Вставить("ДеревоЗагрузкиРеестра",  ИзвлечьСтруктуруXML("СхемаВыгрузки501_2"));
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	ДеревоВыгрузкиРеестра = ИзвлечьСтруктуруXML("СхемаВыгрузки501_2");
...
Функция СформироватьСтруктуруПараметров()
	...
	СохрСтруктура = СохраненныеДанныеОтчета();
	ДанныеПервойВеткиРеестра = СохрСтруктура.ДанныеМногоуровневыхРазделов["Реестр"].Строки[0].Данные;
	ПараметрыВыгрузки.Вставить("КодОперации", СокрЛП(ДанныеПервойВеткиРеестра["КодОперации"]));
	ПараметрыВыгрузки.Вставить("НалБазаИтого", ДанныеПервойВеткиРеестра["НалБазаИтого"]);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		ЭтоПерваяВеткаРеестра = НомерСтраницы = 1;
		
		Если ЭтоПерваяВеткаРеестра Тогда
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			ДлинаКодИнГос = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000017"]));
			Если ДлинаКодИнГос <> 3 Тогда
				ИмяПок = "П1000017" + "_" + НомерСтрокиП10000_Строка;
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000018"]) Тогда
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000019"]) Тогда
				...
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П10000.Данные["П1000019"]) Тогда
				ИмяПок = "П1000019" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000020"]) Тогда
				ИмяПок = "П1000020" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " не задан признак выплаты (перечисления) компенсации суммы налога иностранному физическому лицу",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			Иначе
				ВозможныеЗначения_Стр = "1, 2, 3, 4";
				ВозможныеЗначения = СтрРазделить(ВозможныеЗначения_Стр, ", ", Ложь);
				Если ВозможныеЗначения.Найти(СокрЛП(Ветка_П10000.Данные["П1000020"])) = Неопределено Тогда
					ИмяПок = "П1000020" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " некорректно задан признак выплаты (перечисления) компенсации суммы налога иностранному физическому лицу. Возможны значения: "
					+ ВозможныеЗначения_Стр, "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100007"]) Тогда
					ИмяПок = "П1100007" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана дата кассового чека",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П11000.Данные["П1100007"]) Тогда
					ИмяПок = "П1100007" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
			...
			Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200008"]) Тогда
					ИмяПок = "П1200008" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200009"]) Тогда
				...
				ДлинаКодЕдИзм = СтрДлина(СокрЛП(Ветка_П12000.Данные["П1200011"]));
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200013"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана ставка НДС",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				Иначе
					ВозможныеЗначения_Стр = "10, 10/110, 18, 18/118, 20, 20/120";
					ВозможныеЗначения = СтрРазделить(ВозможныеЗначения_Стр, ", ", Ложь);
					Если ВозможныеЗначения.Найти(СокрЛП(Ветка_П12000.Данные["П1200013"])) = Неопределено Тогда
						ИмяПок = "П1200013" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " некорректно задана ставка НДС. Возможны значения: " + ВозможныеЗначения_Стр,
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		СвОргТорг = ПолучитьПодчиненныйЭлемент(УзелРодитель, "СвОргТорг");
		Для каждого Эл Из СвОргТорг.Строки Цикл
			ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
		КонецЦикла;
		
		СвЧек = ПолучитьПодчиненныйЭлемент(УзелРодитель, "СвЧек");
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСвЧек = СкопироватьУзел(УзелРодитель, СвЧек, УзелРодитель.Строки.Количество() - 1);
...
Процедура ВывестиНаПечатьОтчет(СтруктураПараметров, ВыбранныеСтраницы, СтрПоля = Неопределено)
	...
	Если ВыбранныеСтраницы.Найти("Титульный") <> Неопределено Тогда
		...
		ИмяМакета = "ПечатныйБланк2019Кв3_Титульный";
		...
		ДанныеПервойВеткиРеестра = ПолучитьИзВременногоХранилища(СтруктураРеквизитовФормы[
		"АдресВоВрХранилищеДеревоДанныхРеестр"]).Строки[0].Данные;
		
		ПечФорма.Параметры.Заполнить(ДанныеПервойВеткиРеестра);
	...
	Если ВыбранныеСтраницы.Найти("Реестр") <> Неопределено Тогда
		...
		ИмяМакета = "ПечатныйБланк2019Кв3_Реестр";
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			ЭтоПерваяВеткаРеестра = ДеревоДанныхРаздела.Строки.Индекс(Страница) = 0;
			...
			Если ЭтоПерваяВеткаРеестра Тогда
				СтрокаДокумент.Параметры.НомерПП = "1";
			Иначе
				СтрокаДокумент.Параметры.НомерПП = "";
				СтрокаДокумент.Параметры.ИННОрг = "";
				СтрокаДокумент.Параметры.НаименованиеОрг = "";
			КонецЕсли;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если КодПоказателя = "П1200011" Тогда
