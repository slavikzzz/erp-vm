...
Процедура СформироватьСпискиВыбора()
	...
	//Реестр.
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1100011", , ,
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П1100013") И НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если СтрНачинаетсяС(ИмяПоказателя, "П1100011") Тогда
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы110 Тогда
		Подстрока = "сведения по товарным позициям";
		ТекстВопроса = НСтр("ru = 'Удалить выбранные сведения по товарным позициям?';
							|en = 'Удалить выбранные сведения по товарным позициям?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя)
				...
				Если ЗначениеЗаполнено(КодОперации) Тогда
					Ном1 = 1;
					Пока Форма.ТабличныйДокумент.Области.Найти("П1100016_" + Формат(Ном1, "ЧГ=") + "_1") <> Неопределено Цикл
						Ном2 = 1;
						Пока Форма.ТабличныйДокумент.Области.Найти(
							"П1100016_" + Формат(Ном1, "ЧГ=") + "_" + Формат(Ном2, "ЧГ=")) <> Неопределено Цикл
							СуммаНалБаз = СуммаНалБаз + Форма.ТабличныйДокумент.Области[
							"П1100016_" + Формат(Ном1, "ЧГ=") + "_" + Формат(Ном2, "ЧГ=")].Значение;
							Ном2 = Ном2 + 1;
						КонецЦикла;
						Ном1 = Ном1 + 1;
					КонецЦикла;
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если Ветка = АктивнаяВетка Тогда
				...
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					...
					Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
						Ветка_П11000.Данные.Вставить("П1100016", Форма.ТабличныйДокумент.Области["П1100016_"
			...
			Если ЗначениеЗаполнено(КодОперации) Тогда
				...
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
					Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
						СуммаНалБаз = СуммаНалБаз + Ветка_П11000.Данные["П1100016"];
					КонецЦикла;
				КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("НалБазаИтого", "КодОперации;П1100016");
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100095"]) Тогда
					ИмяПок = "П1100095" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан номер позиции товара в документе (чеке) для компенсации суммы налога",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				ДлинаКодЕдИзм = СтрДлина(СокрЛП(Ветка_П11000.Данные["П1100011"]));
				...
				Иначе
					...
					ВозможныеЗначения = СтрРазделить(ВозможныеЗначения_Стр, ", ", Ложь);
					Если ВозможныеЗначения.Найти(СокрЛП(Ветка_П11000.Данные["П1100013"])) = Неопределено Тогда
						ИмяПок = "П1100013" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	УпорядоченныеДанныеДерева = Новый ТаблицаЗначений;
	УпорядоченныеДанныеДерева.Колонки.Добавить("КлючГруппыВетокРеестра");
	УпорядоченныеДанныеДерева.Колонки.Добавить("ГруппаВетокРеестра");
	
	Для каждого Ветка Из ДеревоДанных.Строки Цикл
		
		ТекКлючГруппыВетокРеестра = СокрЛП(Ветка.Данные["ИННОрг"]) + СокрЛП(Ветка.Данные["НаименованиеОрг"]);
		
		Если ЗначениеЗаполнено(ТекКлючГруппыВетокРеестра) Тогда
			
			СтрокиУДД = УпорядоченныеДанныеДерева.НайтиСтроки(
			Новый Структура("КлючГруппыВетокРеестра", ТекКлючГруппыВетокРеестра));
			
			Если СтрокиУДД.Количество() = 0 Тогда
				СтрокаУДД = УпорядоченныеДанныеДерева.Добавить();
				СтрокаУДД.КлючГруппыВетокРеестра = ТекКлючГруппыВетокРеестра;
				СтрокаУДД.ГруппаВетокРеестра     = Новый Массив;
			Иначе
				СтрокаУДД = СтрокиУДД[0];
			КонецЕсли;
			
			СтрокаУДД.ГруппаВетокРеестра.Добавить(ДеревоДанных.Строки.Индекс(Ветка));
			
		КонецЕсли;
		
	КонецЦикла;
	
	СвРеестрЧек = ПолучитьПодчиненныйЭлемент(УзелРодитель, "СвРеестрЧек");
	
	Для каждого СтрокаУДД Из УпорядоченныеДанныеДерева Цикл
		
		ЭкзСвРеестрЧек = СкопироватьУзел(УзелРодитель, СвРеестрЧек, УзелРодитель.Строки.Количество() - 1);
		
		ГруппаВетокРеестра = СтрокаУДД.ГруппаВетокРеестра;
		
		ПерваяВетка = ДеревоДанных.Строки[ГруппаВетокРеестра[0]];
		
		СвОргТорг = ПолучитьПодчиненныйЭлемент(ЭкзСвРеестрЧек, "СвОргТорг");
		Для каждого Эл Из СвОргТорг.Строки Цикл
			ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
		КонецЦикла;
		
		СвЧек = ПолучитьПодчиненныйЭлемент(ЭкзСвРеестрЧек, "СвЧек");
		
		Для каждого ИндексВетки Из ГруппаВетокРеестра Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСвЧек = СкопироватьУзел(ЭкзСвРеестрЧек, СвЧек, ЭкзСвРеестрЧек.Строки.Количество() - 1);
	...
	РегламентированнаяОтчетность.УдалитьУзел(СвРеестрЧек);
...
Процедура ВывестиНаПечатьОтчет(СтруктураПараметров, ВыбранныеСтраницы, СтрПоля = Неопределено)
	...
	Если ВыбранныеСтраницы.Найти("Титульный") <> Неопределено Тогда
		...
		ИмяМакета = "ПечатныйБланк2021Кв1_Титульный";
	...
	Если ВыбранныеСтраницы.Найти("Реестр") <> Неопределено Тогда
		...
		ИмяМакета = "ПечатныйБланк2021Кв1_Реестр";
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				МаксКолСтрок11 = Макс(КолСтрок11, 1);
				
				Для Инд = 1 По МаксКолСтрок11 Цикл
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если КодПоказателя = "П1100011" Тогда
