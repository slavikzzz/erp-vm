...
Функция ПолучитьТаблицуФорм() Экспорт 
	...
	Стр = Результат.Добавить();
	Стр.ИмяФормы = "Форма2018_1";
	Стр.ОписаниеФормы = "В редакции от 18.03.2019 письмо ФНС России N ММВ-7-3/138@, КНД 1112020 в формате 5.02";
	Стр.ДатаНачала = Дата("20180101");
	Стр.ДатаКонца = Дата("20500101");
	
	Стр = Результат.Добавить();
	Стр.ИмяФормы = "Форма2024_1";
	Стр.ОписаниеФормы = "В редакции от 16.10.2024 письмо ФНС России N ЕД-7-3/859@, КНД 1154003 в формате 5.01";
...
Функция СформироватьВыгрузкуИПолучитьДанные(ДокОбъект) Экспорт 
	...
	Если ДокОбъект.ИмяФормы = "Форма2018_1" Тогда 
		СтруктураВыгрузки.Вставить("Данные", УведомлениеОСпецрежимахНалогообложения.ПолучитьМакетДвоичныхДанных(ДокОбъект.ИмяОтчета, "TIFF_2018_1"));
		СтруктураВыгрузки.Вставить("ИмяФайла", "1112020_5.02000_02.tif");
		Возврат СтруктураВыгрузки;
	ИначеЕсли ДокОбъект.ИмяФормы = "Форма2024_1" Тогда 
		...
		СтруктураВыгрузки.Вставить("ИмяФайла", "1154003_5.01000_01.tif");
		Возврат СтруктураВыгрузки;
	КонецЕсли;
КонецФункции
...
Функция СформироватьСписокЛистовФорма2024_1(Объект) Экспорт
	...
	ИННКПП.Вставить("ИНН", СтруктураПараметров.ДанныеУведомления.Титульная.ИННШапка);
	
	НомСтр = 0;
	УведомлениеОСпецрежимахНалогообложения.НапечататьСтруктуру(Объект.ИмяОтчета,
		СтруктураПараметров.ДанныеУведомления["Титульная"], НомСтр, "Печать_Форма2024_1_Титульная", ПечатнаяФорма, ИННКПП);
	УведомлениеОСпецрежимахНалогообложения.ПоложитьПФВСписокЛистов(Объект, Листы, ПечатнаяФорма, НомСтр);
	
	Для Каждого Стр Из СтруктураПараметров.ДанныеМногостраничныхРазделов.СвПрККТ Цикл 
		УведомлениеОСпецрежимахНалогообложения.НапечататьСтруктуру(Объект.ИмяОтчета,
			Стр.Значение, НомСтр, "Печать_Форма2024_1_ЛистА", ПечатнаяФорма, ИННКПП);
		УведомлениеОСпецрежимахНалогообложения.ПоложитьПФВСписокЛистов(Объект, Листы, ПечатнаяФорма, НомСтр);
	КонецЦикла;
	
	Для Каждого Стр Из СтруктураПараметров.ДанныеМногостраничныхРазделов.СвПатСумНалУм Цикл 
		СвПатСумНалУм = Стр.Значение;
		СвПатСумНалУм.Вставить("СумРасхККТ", СтруктураПараметров.ДанныеУведомления.УмСумНалПрККТ.СумРасхККТ);
		СвПатСумНалУм.Вставить("СумРасхККТПревНал", СтруктураПараметров.ДанныеУведомления.УмСумНалПрККТ.СумРасхККТПревНал);
		УведомлениеОСпецрежимахНалогообложения.НапечататьСтруктуру(Объект.ИмяОтчета,
			СвПатСумНалУм, НомСтр, "Печать_Форма2024_1_ЛистБ", ПечатнаяФорма, ИННКПП);
		УведомлениеОСпецрежимахНалогообложения.ПоложитьПФВСписокЛистов(Объект, Листы, ПечатнаяФорма, НомСтр);
	КонецЦикла;
...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2018_1(Данные, УникальныйИдентификатор)
	...
	Если Титульная.ПрПодп = "2" Тогда 
		УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаПодписантаНалоговойОтчетности(Данные, ТаблицаОшибок, "Форма2018_1_Титульная", Истина);
	Иначе
		УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаПодписантаНалоговойОтчетности(Данные, ТаблицаОшибок, "Форма2018_1_Титульная", Ложь);
	КонецЕсли;
	УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаКодаНО(Титульная.КодНО, ТаблицаОшибок, "Форма2018_1_Титульная");
	УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаИНН(Титульная, ТаблицаОшибок, "Форма2018_1_Титульная", "ИННШапка", Истина, Ложь);
...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2024_1(Данные, УникальныйИдентификатор)
	...
	КонецЕсли;
	УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаПодписантаНалоговойОтчетности(Данные, ТаблицаОшибок, "Титульная", Титульная.ПРИЗНАК_НП_ПОДВАЛ = "2");
	УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаКодаНО(Титульная.КодНО, ТаблицаОшибок, "Титульная");
	УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаИНН(Титульная, ТаблицаОшибок, "Титульная", "ИННШапка", Истина, Ложь);
	
	Для Каждого Стр Из Данные.ДанныеМногостраничныхРазделов.СвПрККТ Цикл 
		СтрЗначение = Стр.Значение;
		
		Если Не ЗначениеЗаполнено(СтрЗначение.МоделККТ) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указано наименование модели контрольно-кассовой техники", "СвПрККТ", "МоделККТ", СтрЗначение.УИД));
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрЗначение.НомерККТ) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указан заводской номер экземпляра модели контрольно-кассовой техники", "СвПрККТ", "НомерККТ", СтрЗначение.УИД));
		КонецЕсли;
		Если СтрДлина(СокрЛП(СтрЗначение.РегНомерККТ)) <> 16 Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указан / неправильно указан регистрационный номер контрольно-кассовой техники", "СвПрККТ", "РегНомерККТ", СтрЗначение.УИД));
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрЗначение.ДатаРегККТ) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указана дата регистрации контрольно-кассовой техники в налоговом органе", "СвПрККТ", "ДатаРегККТ", СтрЗначение.УИД));
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрЗначение.СумРасхККТ) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указана сумма расходов", "СвПрККТ", "СумРасхККТ", СтрЗначение.УИД));
		ИначеЕсли СтрЗначение.СумРасхККТ > 28000 Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Сумма расходов не должна превышать 28000 рублей", "СвПрККТ", "СумРасхККТ", СтрЗначение.УИД));
		КонецЕсли;
	КонецЦикла;
	
	УмСумНалПрККТ = Данные.ДанныеУведомления.УмСумНалПрККТ;
	Если Не ЗначениеЗаполнено(УмСумНалПрККТ.СумРасхККТ) Тогда 
		ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
			"Не указана общая сумма расходов", "УмСумНалПрККТ", "ОбщСумРасхККТ"));
	КонецЕсли;
	
	Для Каждого Стр Из Данные.ДанныеМногостраничныхРазделов.СвПатСумНалУм Цикл 
		СтрЗначение = Стр.Значение;
		
		Если Не ЗначениеЗаполнено(СтрЗначение.НомерПат) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указан номер патента", "СвПатСумНалУм", "НомерПат", СтрЗначение.УИД));
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрЗначение.ДатаВыдПат) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указана дата выдачи патента", "СвПатСумНалУм", "ДатаВыдПат", СтрЗначение.УИД));
		КонецЕсли;
	КонецЦикла;
...
Функция ДанныеУведомления(Ссылка)
	ДанныеИБ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "ДанныеУведомления,Организация,РегистрацияВИФНС,Дата,ИмяФормы");
	Если ДанныеИБ.ИмяФормы = "Форма2018_1" Тогда 
		Возврат ДанныеУведомления_Форма2018_1(Ссылка, ДанныеИБ);
	ИначеЕсли ДанныеИБ.ИмяФормы = "Форма2024_1" Тогда 
		Возврат ДанныеУведомления_Форма2024_1(Ссылка, ДанныеИБ);
	КонецЕсли;
КонецФункции

Функция ДанныеУведомления_Форма2024_1(Ссылка, ДанныеИБ)
	...
	Для Каждого ЛистБ Из СтруктураПараметров.ДанныеМногостраничныхРазделов.СвПатСумНалУм Цикл 
		Если УведомлениеОСпецрежимахНалогообложения.СтраницаЗаполнена(ЛистБ.Значение) Тогда 
			НовСтр = ТаблицаБ.Добавить();
			НовСтр.Стр120 = ЛистБ.Значение.НомерПат;
			НовСтр.Стр130 = ЛистБ.Значение.ДатаВыдПат;
			НовСтр.Стр140 = ЛистБ.Значение.СумПерСрНалУм;
			НовСтр.Стр150 = ЛистБ.Значение.ПерСрУплНал;
			НовСтр.Стр160 = ЛистБ.Значение.СумРасхККТПерУмНал;
			НовСтр.Стр170 = ЛистБ.Значение.ВтСрУплНал;
			НовСтр.Стр180 = ЛистБ.Значение.СумВтСрНалУм;
			НовСтр.Стр190 = ЛистБ.Значение.ТрСрУплНал;
			НовСтр.Стр200 = ЛистБ.Значение.СумРасхККТВтУмНал;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого ЛистА Из СтруктураПараметров.ДанныеМногостраничныхРазделов.СвПрККТ Цикл 
		Если УведомлениеОСпецрежимахНалогообложения.СтраницаЗаполнена(ЛистА.Значение) Тогда 
			ЗаполнитьЗначенияСвойств(ТаблицаА.Добавить(), ЛистА.Значение);
		КонецЕсли;
	КонецЦикла;
	...
	Результат.Вставить("Стр110", СтруктураПараметров.ДанныеУведомления.УмСумНалПрККТ.СумРасхККТ);
	Результат.Вставить("Стр210", СтруктураПараметров.ДанныеУведомления.УмСумНалПрККТ.СумРасхККТПревНал);
	Возврат Результат;
КонецФункции

Функция ДанныеУведомления_Форма2018_1(Ссылка, ДанныеИБ)
	СтруктураПараметров = ДанныеИБ.ДанныеУведомления.Получить();
