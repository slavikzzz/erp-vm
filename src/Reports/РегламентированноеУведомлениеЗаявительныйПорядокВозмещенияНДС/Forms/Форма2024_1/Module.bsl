...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УведомлениеОСпецрежимахНалогообложения.СформироватьСпискиВыбора(ЭтотОбъект, "СпискиВыбора2024");
...
Процедура ПредставлениеУведомленияВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ЭтотОбъект["Подключена1СОтчетность"] И Область.Имя = "ИмяФайлДекл" Тогда 
		СтандартнаяОбработка = Ложь;
		...
		ОткрытьФорму("ОбщаяФорма.РегламентированнаяОтчетность", ДополнительныеПараметры, , , , , 
			Новый ОписаниеОповещения("ВыборСданногоОтчетаЗавершение", ЭтотОбъект, Область), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ВыборСданногоОтчетаЗавершение(РезультатВыбора, Область) Экспорт
	Если РезультатВыбора <> Неопределено Тогда
		ИмяФайла = РегламентированнаяОтчетностьВызовСервера.ИмяФайлаСданногоОбъекта1СОтчетности(РезультатВыбора);
...
Процедура ЗаполнитьНачальныеДанные() Экспорт
	...
	Иначе 
		СтрокаСведений = "ИННФЛ,КППЮЛ,ТелДом,ФамилияИП,ИмяИП,ОтчествоИП";
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(Объект.Организация, Объект.ДатаПодписи, СтрокаСведений);
...
Процедура СформироватьДеревоСтраниц() Экспорт
	...
	Стр001.ИмяМакета = "Титульная_2024_1";
	...
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "ЗаявВозм_2024_1";
	Стр001.Многостраничность = Ложь;
	Стр001.Многострочность = Истина;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "ЗаявВозм";
	Стр001.МногострочныеЧасти.Добавить("МнгСтр");
	
	Стр001 = КорневойУровень.Добавить();
	Стр001.Наименование = "Сведения о возврате";
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "СведВозм_2024_1";
	Стр001.Многостраничность = Ложь;
	Стр001.Многострочность = Ложь;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "СведВозм";
...
Процедура ДеревоСтраницПриАктивизацииСтрокиЗавершение() Экспорт 
	...
	КонецЕсли;
	ЭтотОбъект["ТекущиеМногострочныеЧасти"] = ОбщегоНазначенияКлиентСервер.СкопироватьСписокЗначений(Элемент.ТекущиеДанные.МногострочныеЧасти);
КонецПроцедуры
