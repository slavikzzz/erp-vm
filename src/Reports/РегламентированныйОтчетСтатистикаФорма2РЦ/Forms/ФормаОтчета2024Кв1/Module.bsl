...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы1, 28);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		Если Сред(Область.Имя, 12, 2) = "02" Или Сред(Область.Имя, 12, 2) = "03" Тогда
			...
			КолонкаПоиска = ?(Прав(Область.Имя, 2) = "02", "Название", "Код");
		ИначеЕсли Сред(Область.Имя, 12, 2) = "13" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникКодыИсточникаПриобретенияТовара");
			КолонкаПоиска = "Код";
		КонецЕсли;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	СмежнаяОбласть = СтрЗаменить(Область.Имя, "П000010100102_", "П000010100103_");
	ИсходноеЗначение = Формат(ПолеТабличногоДокументаФормаОтчета.Области[СмежнаяОбласть].Значение, "ЧГ=");
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Сред(Область.Имя, 12, 2) = "13" Тогда
		Область.Значение = РезультатКод;
	Иначе
		Область.Значение = ?(Сред(Область.Имя, 12, 2) = "02", РезультатНазвание, РезультатКод);
		КолонкаЗамены = ?(Сред(Область.Имя, 12, 2) = "02", "03", "02");
		ОбластьЗамены = Лев(Область.Имя, 11) + КолонкаЗамены + Сред(Область.Имя, 14);
		ПолеТабличногоДокументаФормаОтчета.Области[ОбластьЗамены].Значение
			= ?(Сред(Область.Имя, 12, 2) = "02", РезультатКод, РезультатНазвание);
	КонецЕсли;
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Пока Истина Цикл
		Инд = Инд + 1;
		Обл = Области.Найти("П000010100101_" + Формат(Инд, "ЧГ="));
		Если Обл = Неопределено Тогда
			Прервать;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Области["П000010100102_" + Формат(Инд, "ЧГ=")].Значение) Тогда
			НПП = НПП + 1;
			Области["П000010100101_" + Формат(Инд, "ЧГ=")].Значение = НПП;
		Иначе
			Области["П000010100101_" + Формат(Инд, "ЧГ=")].Значение = "";
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Пока Истина Цикл
		...
		Инд = Инд + 1;
		ОблИмя = "П000010100107_" + Формат(Инд, "ЧГ=");
		
		Обл = Области.Найти(ОблИмя);
		Если Обл = Неопределено Тогда
			Прервать;
		КонецЕсли;
		
		Области["П000010100107_" + Формат(Инд, "ЧГ=")].Значение
			= Области["П000010100108_" + Формат(Инд, "ЧГ=")].Значение
			+ Области["П000010100109_" + Формат(Инд, "ЧГ=")].Значение
			+ Области["П000010100110_" + Формат(Инд, "ЧГ=")].Значение;
	...
	Пока Истина Цикл
		...
		Инд = Инд + 1;
		ОблИмя = "П000010100104_" + Формат(Инд, "ЧГ=");
		...
		Области["П000010100104_" + Формат(Инд, "ЧГ=")].Значение
			= Области["П000010100105_" + Формат(Инд, "ЧГ=")].Значение
			+ Области["П000010100107_" + Формат(Инд, "ЧГ=")].Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	ОтклАвтоРасчетЗначение = СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет;
	...
	Пока Истина Цикл
		...
		Обл = мФормаОтчета.Области.Найти(ОблИмя);
		Если Обл = Неопределено Тогда
			Прервать;
		КонецЕсли;
		
		мФормаОтчета.Области[ОблИмя].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области[ОблИмя].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 2-РЦ");
	...
	ТекущаяОбласть = ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть1_3");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникКодыИсточникаПриобретенияТовара",
		512, , "Выбор кода источника приобретения товара", КоллекцияСписковВыбора["КодыИсточникаПриобретенияТовара"]);
