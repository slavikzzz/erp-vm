...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Инд = 1 По 3 Цикл 
		мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы["мИдГруппы" + Инд], 5);
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Строка1_3 = ПечатнаяФормаМакет.ПолучитьОбласть("Строка13");
	Страница2_Верх = ПечатнаяФормаМакет.ПолучитьОбласть("Страница2_Верх");
	ОсновнаяЧасть2 = ПечатнаяФормаМакет.ПолучитьОбласть("Страница02_Подвал");
	...
	МассивОбластей.Добавить(Строка1_3);
	МассивОбластей.Добавить(Страница2_Верх);
	МассивОбластей.Добавить(ОсновнаяЧасть2);
	...
	Для Инд = 1 По 2 Цикл
		Если Инд = 2 Тогда
			Если Не нСтр.ПроверитьВывод(Строка1_3) Тогда 
		...
		Пока Истина Цикл 
			...
			МнгСтрШаблон.Области.П100.Значение = "1." + (Инд+1) + "." + ИтерСтр + ". " + ПолеТабличногоДокументаФормаОтчета.Области.Найти("П010" + Инд + "00000001_" + ИтерСтр).Значение;
			МнгСтрШаблон.Области.П101.Значение = ПолеТабличногоДокументаФормаОтчета.Области.Найти("П010" + Инд + "00000002_" + ИтерСтр).Значение;
			МнгСтрШаблон.Области.П102.Значение = ПолеТабличногоДокументаФормаОтчета.Области.Найти("П010" + Инд + "00000003_" + ИтерСтр).Значение;
	...
	Если Не нСтр.ПроверитьВывод(ПечатнаяФормаМакет.ПолучитьОбласть("Строка2")) Тогда 
		нСтр.Вывести(ГраницаСверху);
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(нСтр, "ОН00003");
		нСтр = РегламентированнаяОтчетность.НоваяСтраница();
		нСтр.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
	...
	Для Инд = 3 По 4 Цикл
		Стр2х = ПечатнаяФормаМакет.ПолучитьОбласть("Строка2" + (Инд-2));
		Если Не нСтр.ПроверитьВывод(Стр2х) Тогда 
			нСтр.Вывести(ГраницаСверху);
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(нСтр, "ОН00003");
			нСтр = РегламентированнаяОтчетность.НоваяСтраница();
			нСтр.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
		...
		Пока Истина Цикл 
			...
			МнгСтрШаблон.Области.П100.Значение = "2." + (Инд-2) + "." + ИтерСтр + ". " + ПолеТабличногоДокументаФормаОтчета.Области.Найти("П010" + Инд + "00000001_" + ИтерСтр).Значение;
			МнгСтрШаблон.Области.П101.Значение = ПолеТабличногоДокументаФормаОтчета.Области.Найти("П010" + Инд + "00000002_" + ИтерСтр).Значение;
			МнгСтрШаблон.Области.П102.Значение = ПолеТабличногоДокументаФормаОтчета.Области.Найти("П010" + Инд + "00000003_" + ИтерСтр).Значение;
			Если Не нСтр.ПроверитьВывод(МнгСтрШаблон) Тогда 
				нСтр.Вывести(ГраницаСверху);
	...
	Пока Истина Цикл 
		...
		МнгСтрШаблон.Области.П200.Значение = "4.1." + ИтерСтр + ". " + ПолеТабличногоДокументаФормаОтчета.Области.Найти("П020100000001_" + ИтерСтр).Значение;
	...
	Если Не нСтр.ПроверитьВывод(ОсновнаяЧасть2) Тогда 
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(нСтр, "ОН00003");
		нСтр = РегламентированнаяОтчетность.НоваяСтраница();
		нСтр.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
		нСтр.Вывести(ПечатнаяФормаМакет.ПолучитьОбласть("Заголовок_Стр2"));
	КонецЕсли;
	нСтр.Вывести(ОсновнаяЧасть2);
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(нСтр, "ОН00003");
