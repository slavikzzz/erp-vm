...
Процедура ДобавитьСтрокуНаСервере(ТекущаяОбласть)
	ВставитьДополнительнуюСтроку(ЭтаФорма, ТекущаяОбласть);
КонецПроцедуры
...
Процедура УдалитьСтроку(Команда)
	...
	ИмяГруппы      = РазделТекущейОбласти(ЭтаФорма, ТекТабличноеПоле.ТекущаяОбласть.Имя);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	нСтр.Вывести(ПечатнаяФормаМакет.ПолучитьОбласть("Стр13"));
	Итер = 0;
	Пока Истина Цикл 
		Итер = Итер + 1;
		ИтерСтр = Формат(Итер, "ЧГ=");
	...
	Пока Истина Цикл 
		...
		МнгСтрШаблон.Области.П299.Значение = "2.2." + ИтерСтр;
		МнгСтрШаблон.Области.П200.Значение = ПолеТабличногоДокументаФормаОтчета.Области.Найти("П010400000001_" + ИтерСтр).Значение;
		МнгСтрШаблон.Области.П201.Значение = ПолеТабличногоДокументаФормаОтчета.Области.Найти("П010400000002_" + ИтерСтр).Значение;
		нСтр.Вывести(МнгСтрШаблон);
	КонецЦикла;
