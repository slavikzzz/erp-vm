...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0601033";
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0000201016";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СредняяЧисленность.Количество() = 0 Тогда
		...
		СредняяЧисленность.Добавить(СписокМесяцев, НСтр("ru = 'Численность работников';
														|en = 'Численность работников'"));
		СписокМесяцев = Отчеты.РегламентированныйОтчетСтатистикаФормаМПСП.СписокМесяцев();
		СредняяЧисленность.Добавить(СписокМесяцев, НСтр("ru = 'Численность работников списочного состава (без внешних совместителей)';
														|en = 'Численность работников списочного состава (без внешних совместителей)'"));
		СписокМесяцев = Отчеты.РегламентированныйОтчетСтатистикаФормаМПСП.СписокМесяцев();
		СредняяЧисленность.Добавить(СписокМесяцев, НСтр("ru = 'Численность работников за 2019 год';
														|en = 'Численность работников за 2019 год'"));
...
Процедура ЗаполнитьСведенияОбОрганизацииНаСервере(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Области["ИННЮЛ2"].Значение = Области["ИННЮЛ"].Значение;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		ИначеЕсли Лев(Область.Имя, 3) = "Нет" Тогда
			...
			Модифицированность = Истина;
			Возврат;
		ИначеЕсли Лев(Область.Имя, 8) = "П0000101" И (Прав(Область.Имя, 1) = "2" Или Прав(Область.Имя, 1) = "4") Тогда
			Если ЗначениеЗаполнено(Область.Значение) Тогда
				Область.Значение = "";
			Иначе
				Область.Значение = "X";
		...
		Если Не СтруктураРеквизитовФормы.мАвтоВыборКодов
		   И (СтрНачинаетсяС(Область.Имя, "П000020101601") Или СтрНачинаетсяС(Область.Имя, "П000020101603")) Тогда
			...
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				ОблКод = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "П000020101601", "П000020101603"));
...
Процедура ОткрытьФормуДетализацииЧисленность(Элемент, ДетализируемаяЧисленность)
	...
	ОткрытьФорму("Отчет.РегламентированныйОтчетСтатистикаФормаМПСП.Форма.СреднесписочнаяЧисленность",
...
Процедура ЗавершениеРедактированияЧисленности(РезультатЗакрытия, ДетализируемаяЧисленность) Экспорт
	...
	Если СредняяЧисленность.Индекс(ДетализируемаяЧисленность) = 0 Тогда
		ЭтотОбъект.ПолеТабличногоДокументаФормаОтчета.Области["П000020100903"].Значение = СреднееЗаГод;
	ИначеЕсли СредняяЧисленность.Индекс(ДетализируемаяЧисленность) = 1 Тогда
		ЭтотОбъект.ПолеТабличногоДокументаФормаОтчета.Области["П000020101003"].Значение = СреднееЗаГод;
	ИначеЕсли СредняяЧисленность.Индекс(ДетализируемаяЧисленность) = 2 Тогда
		ЭтотОбъект.ПолеТабличногоДокументаФормаОтчета.Области["П000020101301"].Значение = СреднееЗаГод;
	КонецЕсли;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "П000020101603", "П000020101601")).Значение
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Области["П000020101602_1"].Значение = "16";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Пока Истина Цикл
		Инд = Инд + 1;
		Обл = Области.Найти("П000020101604_" + Формат(Инд, "ЧГ="));
		Если Обл = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		Сумм = Сумм + Обл.Значение;
	КонецЦикла;
	Области["П000020101504"].Значение = Сумм;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателей.Добавить(ВыбТабличноеПоле.Области["ИННЮЛ"].Значение,                     "ИННЮЛ");
	СписокПоказателей.Добавить(ВыбТабличноеПоле.Области["ИННЮЛ2"].Значение,                    "ИННЮЛ2");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Н = 5 По 9 Цикл
		Если ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть2_" + Н)) Тогда
			ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть2_" + Н));
		Иначе
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма МП-сп");
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			ПечатнаяФорма.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
...
Процедура ФормироватьСтруктуруСтраницОтчета() Экспорт
	...
	СтрокаУровня1.Представление            = "Статистика: Форма МП-сп";
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("ИНН", СокрЛП(ТабДокумент.ИННЮЛ));
	...
	ПараметрыВыгрузки.Вставить("ПП5", ?(ЗначениеЗаполнено(ТабДокумент.П000010100502), 70, 85));
	ПараметрыВыгрузки.Вставить("ПП6", ?(ЗначениеЗаполнено(ТабДокумент.П000010100602), 70, 85));
	ПараметрыВыгрузки.Вставить("ПП7", ?(ЗначениеЗаполнено(ТабДокумент.П000010100704), 70, 85));
	ПараметрыВыгрузки.Вставить("ПП8", ?(ЗначениеЗаполнено(ТабДокумент.П000010100804), 70, 85));
	
	ПараметрыВыгрузки.Вставить("ПП20", ?(ЗначениеЗаполнено(ТабДокумент.Да2), 70, 85));
	ПараметрыВыгрузки.Вставить("ПП22", ?(ЗначениеЗаполнено(ТабДокумент.Да3), 70, 85));
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок1",
		РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтотОбъект, П0000201016, Ложь));
