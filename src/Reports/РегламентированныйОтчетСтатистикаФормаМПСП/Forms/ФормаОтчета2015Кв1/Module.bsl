...
Процедура УстановитьДоступностьДаНет(ТабДок, СтруктураРеквизитовФормы)
	Да41 = ЗначениеЗаполнено(ТабДок.Области.Да41.Значение);
	Нет41 = ЗначениеЗаполнено(ТабДок.Области.Нет41.Значение);
	П42 = ТабДок.Области.П42;
	П43 = ТабДок.Области.П43;
	П44 = ТабДок.Области.П44;
	П45 = ТабДок.Области.П45;
	П46 = ТабДок.Области.П46;
	Да48 = ТабДок.Области.Да48;
	Нет48 = ТабДок.Области.Нет48;
	
	Если Да41 Тогда
		УстановитьДоступностьОбласти(Истина, П42, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Истина, П43, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Истина, П44, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Истина, П45, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Истина, П46, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Ложь, Да48, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Ложь, Нет48, СтруктураРеквизитовФормы);
	ИначеЕсли Нет41 Тогда
		УстановитьДоступностьОбласти(Ложь, П42, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Ложь, П43, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Ложь, П44, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Ложь, П45, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Ложь, П46, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Истина, Да48, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Истина, Нет48, СтруктураРеквизитовФормы);
	Иначе
		УстановитьДоступностьОбласти(Ложь, П42, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Ложь, П43, СтруктураРеквизитовФормы);
		УстановитьДоступностьОбласти(Ложь, П44, СтруктураРеквизитовФормы);
	...
	Да12 = ЗначениеЗаполнено(ТабДок.Области.Да12.Значение);
	Если Да12 Тогда
		УстановитьДоступностьОбласти(Истина, ТабДок.Области.П13, СтруктураРеквизитовФормы);
	Иначе
		УстановитьДоступностьОбласти(Ложь, ТабДок.Области.П13, СтруктураРеквизитовФормы);
	КонецЕсли;
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма, ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если (НЕ ЗначениеЗаполнено(Форма.ПолеТабличногоДокументаФормаОтчета.Области.ИНН.Значение)) ИЛИ (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) Тогда
		Форма.ПолеТабличногоДокументаФормаОтчета.Области.ИНН.Значение = ИННЮЛ;
	КонецЕсли;
	...
	Если (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) Или (ПустаяСтрока(Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргАдрес.Значение)) Тогда
		Если (ЗначениеЗаполнено(СтрЗаменить(АдрПочт, ",", ""))) Тогда
			...
			Форма.СтруктураРеквизитовФормы.мАдресвФорматеДляВыгрузки1 = АдрПочт;
		Иначе
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Лев(Область.Имя, 3) = "П16" Или Лев(Область.Имя, 3) = "П24"
			Или Область.Имя = "П42" Или Область.Имя = "П43" Или Область.Имя = "П44"
			Или Область.Имя = "П45" Или Область.Имя = "П46" Тогда
			Если ЗначениеЗаполнено(Область.Значение) Тогда 
				Область.Значение = "";
			Иначе
				Область.Значение = "X";
...
&НаСервере
Процедура ВывестиЧисло(ПФ, Число, ПрефиксОбласти, ДлинаОбласти, ЗнаковПослеЗапятой, ВыводитьНоль = Истина, ВыводитьЛидирующиеНули = ложь)
	...
	КонецЕсли;
	Если ВыводитьЛидирующиеНули Тогда 
		ЧислоСтр = Прав("000000000000000000000000" + ЧислоСтр, ДлинаОбласти);
	Иначе 
...
&НаСервере
Процедура ЗаполнитьПустыми(ПФ, Дл, ПрефиксОбласти, Символ = "")
	Для Инд = 1 По Дл Цикл 
		ПФ.Области[ПрефиксОбласти+Инд].Текст = Символ;
	КонецЦикла;
КонецПроцедуры
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПечатнаяФорма.Области.НаимОрг.Текст = ПолеТабличногоДокументаФормаОтчета.Области.ОргНазв.Значение;
	...
	ВывестиЧисло(ПечатнаяФорма, ПолеТабличногоДокументаФормаОтчета.Области.П111.Значение, "П11м_", 2, 0, Ложь, Истина);
	ВывестиЧисло(ПечатнаяФорма, ПолеТабличногоДокументаФормаОтчета.Области.П112.Значение, "П11г_", 4, 0, Ложь);
	ВывестиЧисло(ПечатнаяФорма, ПолеТабличногоДокументаФормаОтчета.Области.П13.Значение, "П13_", 2, 0, Ложь, Истина);
	...
	Для Инд = 1 По 13 Цикл
		ПечатнаяФорма.Области["ОГРН_"+Инд].Текст = Сред(ПолеТабличногоДокументаФормаОтчета.Области.ОГРН.Значение, Инд, 1);
	КонецЦикла;
	Для Инд = 1 По 8 Цикл
		ПечатнаяФорма.Области["ОКПО_"+Инд].Текст = Сред(ПолеТабличногоДокументаФормаОтчета.Области.ОргКодОКПО.Значение, Инд, 1);
	КонецЦикла;
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма МП-сп");
	
	Стр1 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатныйБланк_2015Кв1_Лист2");
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.ОриентацияСтраницы  = ОриентацияСтраницы.Портрет;
	ПечатнаяФорма.Вывести(Стр1);
	ПечатнаяФорма.Области.Да15.Текст = ПолеТабличногоДокументаФормаОтчета.Области.Да15.Значение;
	ПечатнаяФорма.Области.Нет15.Текст = ПолеТабличногоДокументаФормаОтчета.Области.Нет15.Значение;
	ПечатнаяФорма.Области.Да25.Текст = ПолеТабличногоДокументаФормаОтчета.Области.Да25.Значение;
	ПечатнаяФорма.Области.Нет25.Текст = ПолеТабличногоДокументаФормаОтчета.Области.Нет25.Значение;
	ПечатнаяФорма.Области.П161.Текст = ПолеТабличногоДокументаФормаОтчета.Области.П161.Значение;
	ПечатнаяФорма.Области.П162.Текст = ПолеТабличногоДокументаФормаОтчета.Области.П162.Значение;
	ПечатнаяФорма.Области.П163.Текст = ПолеТабличногоДокументаФормаОтчета.Области.П163.Значение;
	ПечатнаяФорма.Области.П164.Текст = ПолеТабличногоДокументаФормаОтчета.Области.П164.Значение;
	ПечатнаяФорма.Области.П241.Текст = ПолеТабличногоДокументаФормаОтчета.Области.П241.Значение;
	ПечатнаяФорма.Области.П242.Текст = ПолеТабличногоДокументаФормаОтчета.Области.П242.Значение;
	ПечатнаяФорма.Области.П243.Текст = ПолеТабличногоДокументаФормаОтчета.Области.П243.Значение;
	
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П211.Значение, "П211_", 4, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П212.Значение, "П212_", 4, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П213.Значение, "П213_", 7, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П214.Значение, "П214_", 7, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П215.Значение, "П215_", 4, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П221.Значение, "П221_", 7, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П222.Значение, "П222_", 7, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П223.Значение, "П223_", 7, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П224.Значение, "П224_", 7, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П225.Значение, "П225_", 7, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П23.Значение, "П23_", 7, 1, Ложь);
	
	Если (ПолеТабличногоДокументаФормаОтчета.Области.П221.Значение >= 1000000)
		Или (ПолеТабличногоДокументаФормаОтчета.Области.П222.Значение >= 1000000)
		Или (ПолеТабличногоДокументаФормаОтчета.Области.П223.Значение >= 1000000)
		Или (ПолеТабличногоДокументаФормаОтчета.Области.П224.Значение >= 1000000)
		Или (ПолеТабличногоДокументаФормаОтчета.Области.П225.Значение >= 1000000)
		Или (ПолеТабличногоДокументаФормаОтчета.Области.П23.Значение >= 1000000) Тогда
		
		ОбщегоНазначения.СообщитьПользователю("Сумма по строкам 17-22 превышает допустимое количество знаков на печатной форме. Представьте форму в электронном виде");
	КонецЕсли;
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма МП-сп");
	
	Стр1 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатныйБланк_2015Кв1_Лист3");
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.ОриентацияСтраницы  = ОриентацияСтраницы.Портрет;
	ПечатнаяФорма.Вывести(Стр1);
	Для Инд = 1 По 10 Цикл
	...
	ПечатнаяФорма.Области.Да29.Текст = ПолеТабличногоДокументаФормаОтчета.Области.Да29.Значение;
	ПечатнаяФорма.Области.Нет29.Текст = ПолеТабличногоДокументаФормаОтчета.Области.Нет29.Значение;
	
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П264.Значение, "П264_", 8, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П265.Значение, "П265_", 8, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П27.Значение, "П27_", 8, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П281.Значение, "П281_", 8, 1, Ложь);
	ВывестиЧисло(ПечатнаяФорма, 10*ПолеТабличногоДокументаФормаОтчета.Области.П282.Значение, "П282_", 8, 1, Ложь);
	Для Инд = 1 По 6 Цикл 
		ПечатнаяФорма.Области["П26_Наим_"+Инд].Текст = ПолеТабличногоДокументаФормаОтчета.Области["П000006000101_"+Инд].Значение;
		ВывестиЧисло(ПечатнаяФорма, ПолеТабличногоДокументаФормаОтчета.Области["П000006000104_"+Инд].Значение*10, "П26_"+Инд+"_", 8, 1, Ложь);
		ВывестиОКВЭД(ПечатнаяФорма, ПолеТабличногоДокументаФормаОтчета.Области["П000006000103_"+Инд].Значение, "П26_оквэд_"+Инд+"_");
	КонецЦикла;
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма МП-сп");
	
	Стр1 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатныйБланк_2015Кв1_Лист4");
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.ОриентацияСтраницы  = ОриентацияСтраницы.Портрет;
	ПечатнаяФорма.Вывести(Стр1);
	
	ПечатнаяФорма.Области.Да32.Текст = ПолеТабличногоДокументаФормаОтчета.Области.Да32.Значение;
	ПечатнаяФорма.Области.Нет32.Текст = ПолеТабличногоДокументаФормаОтчета.Области.Нет32.Значение;
	ПечатнаяФорма.Области.Да41.Текст = ПолеТабличногоДокументаФормаОтчета.Области.Да41.Значение;
	ПечатнаяФорма.Области.Нет41.Текст = ПолеТабличногоДокументаФормаОтчета.Области.Нет41.Значение;
	ПечатнаяФорма.Области.Да48.Текст = ПолеТабличногоДокументаФормаОтчета.Области.Да48.Значение;
	ПечатнаяФорма.Области.Нет48.Текст = ПолеТабличногоДокументаФормаОтчета.Области.Нет48.Значение;
	ПечатнаяФорма.Области.П42.Текст = ПолеТабличногоДокументаФормаОтчета.Области.П42.Значение;
	ПечатнаяФорма.Области.П43.Текст = ПолеТабличногоДокументаФормаОтчета.Области.П43.Значение;
	ПечатнаяФорма.Области.П44.Текст = ПолеТабличногоДокументаФормаОтчета.Области.П44.Значение;
	ПечатнаяФорма.Области.П45.Текст = ПолеТабличногоДокументаФормаОтчета.Области.П45.Значение;
	ПечатнаяФорма.Области.П46.Текст = ПолеТабличногоДокументаФормаОтчета.Области.П46.Значение;
	
	Для Инд = 1 По 4 Цикл 
		ВывестиЧисло(ПечатнаяФорма, ПолеТабличногоДокументаФормаОтчета.Области["П313"+Инд].Значение, "П313_"+Инд+"_", 6, 0, Ложь);
		ВывестиЧисло(ПечатнаяФорма, ПолеТабличногоДокументаФормаОтчета.Области["П314"+Инд].Значение, "П314_"+Инд+"_", 6, 0, Ложь);
		ВывестиЧисло(ПечатнаяФорма, ПолеТабличногоДокументаФормаОтчета.Области["П315"+Инд].Значение, "П315_"+Инд+"_", 6, 0, Ложь);
		ВывестиЧисло(ПечатнаяФорма, ПолеТабличногоДокументаФормаОтчета.Области["П316"+Инд].Значение, "П316_"+Инд+"_", 6, 0, Ложь);
	КонецЦикла;
	...
	Если ПолеТабличногоДокументаФормаОтчета.Области.Найти("П000006000101_7") <> Неопределено Тогда 
		Инд = 6;
		Стр4 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатныйБланк_2015Кв1_Лист3");
		...
		Пока Истина Цикл 
			...
			ПечатнаяФорма.Области.П26_Наим_6.Текст = ПолеТабличногоДокументаФормаОтчета.Области["П000006000101_"+ИндСтр].Значение;
			ЗаполнитьПустыми(ПечатнаяФорма, 8, "П26_6_");
			ВывестиЧисло(ПечатнаяФорма, ПолеТабличногоДокументаФормаОтчета.Области["П000006000104_"+ИндСтр].Значение*10, "П26_6_", 8, 1, Ложь);
			ВывестиОКВЭД(ПечатнаяФорма, ПолеТабличногоДокументаФормаОтчета.Области["П000006000103_"+ИндСтр].Значение, "П26_оквэд_6_");
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2015Кв1");
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("ОГРН", СокрЛП(ТабДокумент.ОГРН));
	...
	Если ЗначениеЗаполнено(ТабДокумент.Да15) Тогда 
		ПараметрыВыгрузки.Вставить("ПП15", 1);
	ИначеЕсли ЗначениеЗаполнено(ТабДокумент.Нет15) Тогда 
		ПараметрыВыгрузки.Вставить("ПП15", 2);
	Иначе
		ПараметрыВыгрузки.Вставить("ПП15", 0);
	КонецЕсли;
	
	ПараметрыВыгрузки.Вставить("ПП161", ?(ЗначениеЗаполнено(ТабДокумент.П161), 1, 0));
	ПараметрыВыгрузки.Вставить("ПП162", ?(ЗначениеЗаполнено(ТабДокумент.П162), 1, 0));
	ПараметрыВыгрузки.Вставить("ПП163", ?(ЗначениеЗаполнено(ТабДокумент.П163), 1, 0));
	ПараметрыВыгрузки.Вставить("ПП164", ?(ЗначениеЗаполнено(ТабДокумент.П164), 1, 0));
	
	ПараметрыВыгрузки.Вставить("ПП241", ?(ЗначениеЗаполнено(ТабДокумент.П241), 1, 0));
	ПараметрыВыгрузки.Вставить("ПП242", ?(ЗначениеЗаполнено(ТабДокумент.П242), 1, 0));
	ПараметрыВыгрузки.Вставить("ПП243", ?(ЗначениеЗаполнено(ТабДокумент.П243), 1, 0));
	
	Если ЗначениеЗаполнено(ТабДокумент.Да25) Тогда 
		ПараметрыВыгрузки.Вставить("ПП25", 1);
	ИначеЕсли ЗначениеЗаполнено(ТабДокумент.Нет25) Тогда 
		ПараметрыВыгрузки.Вставить("ПП25", 2);
	Иначе
		ПараметрыВыгрузки.Вставить("ПП25", 0);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТабДокумент.Да29) Тогда 
		ПараметрыВыгрузки.Вставить("ПП29", 1);
	ИначеЕсли ЗначениеЗаполнено(ТабДокумент.Нет29) Тогда 
		ПараметрыВыгрузки.Вставить("ПП29", 2);
	Иначе
		ПараметрыВыгрузки.Вставить("ПП29", 0);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТабДокумент.Да32) Тогда 
		ПараметрыВыгрузки.Вставить("ПП32", 1);
	ИначеЕсли ЗначениеЗаполнено(ТабДокумент.Нет32) Тогда 
		ПараметрыВыгрузки.Вставить("ПП32", 2);
	Иначе
		ПараметрыВыгрузки.Вставить("ПП32", 0);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТабДокумент.Да41) Тогда 
		ПараметрыВыгрузки.Вставить("ПП41", 1);
	ИначеЕсли ЗначениеЗаполнено(ТабДокумент.Нет41) Тогда 
		ПараметрыВыгрузки.Вставить("ПП41", 2);
	Иначе
		ПараметрыВыгрузки.Вставить("ПП41", 0);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТабДокумент.Да48) Тогда 
		ПараметрыВыгрузки.Вставить("ПП48", 1);
	ИначеЕсли ЗначениеЗаполнено(ТабДокумент.Нет48) Тогда 
		ПараметрыВыгрузки.Вставить("ПП48", 2);
	Иначе
		ПараметрыВыгрузки.Вставить("ПП48", 0);
	КонецЕсли;
	
	ПараметрыВыгрузки.Вставить("ПП42", ?(ЗначениеЗаполнено(ТабДокумент.П42), 1, 0));
	ПараметрыВыгрузки.Вставить("ПП43", ?(ЗначениеЗаполнено(ТабДокумент.П43), 1, 0));
	ПараметрыВыгрузки.Вставить("ПП44", ?(ЗначениеЗаполнено(ТабДокумент.П44), 1, 0));
	ПараметрыВыгрузки.Вставить("ПП45", ?(ЗначениеЗаполнено(ТабДокумент.П45), 1, 0));
	ПараметрыВыгрузки.Вставить("ПП46", ?(ЗначениеЗаполнено(ТабДокумент.П46), 1, 0));
