...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0617002";
...
Процедура ДоступностьРазделов()
	...
	ИмяПоказателя = "П000010100104";
	Если мФормаОтчета.Области[ИмяПоказателя].Значение = "1" Тогда
		ПунктыЦветФона = Новый Цвет();
		ПунктыЗащита = Истина;
	Иначе
		ПунктыЦветФона = Новый Цвет(255, 255, 192);
		ПунктыЗащита = Ложь;
	КонецЕсли;
	...
	Для Гр = 3 По 7 Цикл
		...
		Для Стр = 1 По 6 Цикл
			СоставЯчеек = СоставЯчеек + "П000010100" + Стр + Графа + ";";
		КонецЦикла;
	КонецЦикла;
	
	Для Стр = 1 По 10 Цикл
		СтрНомер = Формат(Стр, "ЧЦ=2; ЧВН=");
		СоставЯчеек = СоставЯчеек + "П00001020" + СтрНомер + "03" + ";";
	КонецЦикла;
	
	Для Стр = 1 По 5 Цикл
		СтрНомер = Формат(Стр, "ЧЦ=2; ЧВН=");
		СоставЯчеек = СоставЯчеек + "П00001030" + СтрНомер + "03" + ";";
	КонецЦикла;
	
	МассивЯчеек = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СоставЯчеек, ";", Истина);
	Для Каждого ЭлМасс Из МассивЯчеек Цикл
		мФормаОтчета.Области[ЭлМасс].ЦветФона = ПунктыЦветФона;
		мФормаОтчета.Области[ЭлМасс].Защита   = ПунктыЗащита;
		Если мФормаОтчета.Области[ИмяПоказателя].Значение = "1" Тогда
			мФормаОтчета.Области[ЭлМасс].Значение = "";
		КонецЕсли;
	КонецЦикла;
...
Процедура ПоказатьПериод()
	...
	ПериодОтчета = " на 10 октября " + СтруктураРеквизитовФормы.СтрПериодОтчета;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 6) = "П00001" Или Лев(Область.Имя, 6) = "П00002" Тогда
		
		Если Область.Защита Тогда
			Возврат;
		КонецЕсли;
		
		ТекГруппа = Лев(Область.Имя, 8);
		
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		
		НомерСтроки = Число(Сред(Область.Имя, 10, 2));
		НомерКолонки = Число(Прав(Область.Имя, 2));
		
		Если ТекГруппа = "П0000101" Тогда
			Если НомерКолонки = 3 Или НомерКолонки = 4 Тогда
				УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 3, 4);
			ИначеЕсли НомерКолонки >= 5 И НомерКолонки <= 7 Тогда
				УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 5, 7);
			ИначеЕсли НомерКолонки >= 8 И НомерКолонки <= 10 Тогда
				УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 8, 10);
			КонецЕсли;
		ИначеЕсли ТекГруппа = "П0000102" Тогда
			Если НомерСтроки = 1 Или НомерСтроки = 2 Тогда
				УстановитьЗначокВГруппеЯчеек(Область, 1, 2, НомерКолонки, НомерКолонки);
			Иначе
				УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 3, 4, Истина);
			КонецЕсли;
		ИначеЕсли ТекГруппа = "П0000103" Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 3, 4, Истина);
		ИначеЕсли ТекГруппа = "П0000104" Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 3, 5);
		ИначеЕсли ТекГруппа = "П0000105" Тогда
			...
			КонецЕсли;
		ИначеЕсли ТекГруппа = "П0000106" Тогда
			...
			КонецЕсли;
		ИначеЕсли ТекГруппа = "П0000201" Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 3, 11);
		ИначеЕсли ТекГруппа = "П0000202" Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 3, 8);
		ИначеЕсли ТекГруппа = "П0000203" Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 3, 4, Истина);
		ИначеЕсли ТекГруппа = "П0000204" Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 3, 6, Истина);
		ИначеЕсли ТекГруппа = "П0000205" Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 3, 4, Истина);
		ИначеЕсли ТекГруппа = "П0000206" Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 3, 9);
		КонецЕсли;
...
Процедура УстановитьЗначокВГруппеЯчеек(Область, НачальнаяСтрокаВГруппе, КонечнаяСтрокаВГруппе, НачальнаяКолонкаВГруппе, КонечнаяКолонкаВГруппе, МножественныйВыбор = Ложь)
	...
	РазделПодраздел = Лев(Область.Имя, 8);
	
	ТекЗначениеОбласти = Область.Значение;
	НовЗначениеОбласти = ?(Область.Значение = "", "1", "");
	
	Для НомерСтроки = НачальнаяСтрокаВГруппе По КонечнаяСтрокаВГруппе Цикл
		Для Колонка = НачальнаяКолонкаВГруппе По КонечнаяКолонкаВГруппе Цикл
			ТекЯчейка = РазделПодраздел + Прав("00" + НомерСтроки, 3) + Прав("00" + Колонка, 2);
			Если мФормаОтчета.Области.Найти(ТекЯчейка) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма ИАП");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "ИАП");
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма ИАП");
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Для Каждого Эл Из ТабДокумент Цикл
		...
		Если ИмяПарам = "П00001" Или ИмяПарам = "П00002" Тогда
