...
Процедура Расчет(Форма) 
	...
	Для Гр = 4 По 9 Цикл 
		...
		Области["П0000010008" + ГрСтр].Значение = Области["П0000010009" + ГрСтр].Значение + Области["П0000010010" + ГрСтр].Значение
												 + Области["П0000010011" + ГрСтр].Значение + Области["П0000010012" + ГрСтр].Значение
												  + Области["П0000010014" + ГрСтр].Значение;
	КонецЦикла;
...
Функция ПолучитьПредидущийПериод() Экспорт 
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <> 1 Тогда 
		Результат.мДатаНачалаПериодаОтчета = НачалоГода(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
