...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "16/05/2013";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	// Данные по строке 1 графы 8 должны быть равны сумме данных строк 2, 3 и 10 по графе 8.
	Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000001000108"].Значение = Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000001000208"].Значение
	+ Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000001000308"].Значение + Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000001001008"].Значение;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// Данные по строке 3 должны быть больше либо равны сумме данных строк 4, 5, 6, 7, 8 и 9 по соответствующим графам.
	Для НомКол = 4 По 9 Цикл
		ТекСумма = 0;
		Для НомСтр = 4 По 9 Цикл
		...
		Если ТекСумма > ПараметрыВыгрузки["П0000010003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: данные по строке 3 должны быть больше либо равны сумме данных строк 4, 5, 6, 7, 8 и 9 по соответствующим графам!",
	...
	// Данные по строке 10 должны быть больше или равны сумме данных строк 11, 12, 13, 14 и 15 по соответствующим графам.
	Для НомКол = 4 По 9 Цикл
		ТекСумма = 0;
		Для НомСтр = 11 По 15 Цикл
			ТекСумма = ТекСумма + ПараметрыВыгрузки["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"];
		КонецЦикла;
		Если ТекСумма > ПараметрыВыгрузки["П0000010010" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: данные по строке 10 должны быть больше или равны сумме данных строк 11, 12, 13, 14 и 15 по соответствующим графам!",
...
Функция СформироватьСтруктуруПараметров()
	...
	Для НомСтр = 1 По 17 Цикл
		Для НомКол = 4 По 9 Цикл
			Если НомСтр = 1 И НомКол <> 8 Тогда
				Продолжить;
			КонецЕсли;
