...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0607009";
...
Процедура ПолеТабличногоДокументаФормаОтчетаПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если Область.СодержитЗначение И Область.ТипЗначения.СодержитТип(Тип("Число")) Тогда
		Если Область.Значение = 0 И (Область.ТипЗначения.КвалификаторыЧисла.РазрядностьДробнойЧасти > 0 И Область.ТипЗначения.КвалификаторыЧисла.РазрядностьДробнойЧасти <= 3) Тогда
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-алкоголь (опт)");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1АлкогольОпт");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// КОНТРОЛЬ показателей формы (Указания ФСН по заполнению Формы 1-алкоголь (опт)).
	// Данные графы 4 должны быть больше или равны данным графы 5.
	Для НомСтр = 1 По 16 Цикл
		Если ПараметрыВыгрузки["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04_Ч"]
			< ПараметрыВыгрузки["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "05_Ч"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в строке " + Формат(НомСтр, "ЧГ=")
			+ " значение по графе 4 должно быть больше или равно значению по графе 5!",
			ИмяПоляТаблДокумента, "П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Данные графы 6 должны быть больше или равны данным графы 7.
	Для НомСтр = 1 По 16 Цикл
		Если ПараметрыВыгрузки["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "06_Ч"]
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: в строке " + Формат(НомСтр, "ЧГ=")
			+ " значение по графе 6 должно быть больше или равно значению по графе 7!",
			ИмяПоляТаблДокумента, "П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "06");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Данные по строке 1 должны быть больше или равны сумме данных строк 2, 3, 4, 5, 6 и 7 по соответствующим графам.
	Для НомКол = 4 По 9 Цикл
		ТекСумма = 0;
		Для НомСтр = 2 По 7 Цикл
			ТекСумма = ТекСумма + ПараметрыВыгрузки["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=")
			+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"];
		КонецЦикла;
		Если ТекСумма > ПараметрыВыгрузки["П0000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по графе " + Формат(НомКол, "ЧГ=")
			+ " значение в строке 1 должно быть больше или равно сумме значений в строках 2, 3, 4, 5, 6 и 7!",
			ИмяПоляТаблДокумента, "П0000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Данные по строке 8 должны быть больше или равны сумме данных строк 9, 10, 11, 12 и 13 по соответствующим графам.
	Для НомКол = 4 По 9 Цикл
		ТекСумма = 0;
		Для НомСтр = 9 По 13 Цикл
			...
			+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"];
		КонецЦикла;
		Если ТекСумма > ПараметрыВыгрузки["П0000010008" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по графе " + Формат(НомКол, "ЧГ=")
			+ " значение в строке 8 должно быть больше или равно сумме значений в строках 9, 10, 11, 12 и 13!",
...
Функция СформироватьСтруктуруПараметров()
	...
	// Для ПОСЛЕДУЮЩЕГО (ПЕРЕД ВЫГРУЗКОЙ) контроля показателей формы (Указания ФСН по заполнению Формы 1-алкоголь (опт)).
	Для НомСтр = 1 По 16 Цикл
		Для НомКол = 4 По 9 Цикл
			ПараметрыВыгрузки.Вставить("П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч", ТабДокумент["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]);
		КонецЦикла;
	КонецЦикла;
