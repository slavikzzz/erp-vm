...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр р.НДС прилож.09 21к2") Тогда
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	ИменаГраф.Добавить("П1100024");
	ИменаГраф.Добавить("П1100015");
	ИменаГраф.Добавить("П1100025");
	ИменаГраф.Добавить("П1100016");
	ИменаГраф.Добавить("П1100017");
	ИменаПодчиненныхГрупп = Новый Массив;
	СтруктураМногострочныхЧастей.Вставить(
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 8) = "П1000012"
		ИЛИ Лев(Область.Имя, 8) = "П1000013" Тогда
	...
	ИначеЕсли Лев(Область.Имя, 8) = "П1000005"
		ИЛИ Лев(Область.Имя, 8) = "П1000008"
		ИЛИ Лев(Область.Имя, 8) = "П1000011" Тогда
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы110 Тогда
		Подстрока = "рейс";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный рейс?';
							|en = 'Удалить выбранный рейс?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрАВН_9");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRAVNNDS"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRAVNNDS.2"));
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000002"]) Тогда
				...
				+ " не задан номер транспортного, товаросопроводительного и (или) иного документа",
			...
			Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000091"]) Тогда // СведФЛ
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000003"]) Тогда
					ДлинаИННФЛ = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000003"]));
					...
					КонецЕсли;
				КонецЕсли;
				ФИО = РегламентированнаяОтчетность.РазложитьФИО(Ветка_П10000.Данные["П1000004"]);
			...
			Иначе // СведОрг
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000003"]) Тогда
					ДлинаИНН = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000003"]));
			...
			Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000092"]) Тогда // СведФЛ
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000006"]) Тогда
					ДлинаИННФЛ = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000006"]));
					Если ДлинаИННФЛ <> 12 Тогда
					...
					КонецЕсли;
				КонецЕсли;
				ФИО = РегламентированнаяОтчетность.РазложитьФИО(Ветка_П10000.Данные["П1000007"]);
				Если НЕ ЗначениеЗаполнено(ФИО.Фамилия) Тогда
			...
			Иначе // СведОрг
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000006"]) Тогда
					ДлинаИНН = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000006"]));
					Если ДлинаИНН <> 10 Тогда
						ИмяПок = "П1000006" + "_" + НомерСтрокиП10000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " неверно задан ИНН получателя", "Реестр", ИмяПок, НомерСтраницы);
			...
			Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000093"]) Тогда // СведФЛ
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000009"]) Тогда
					ДлинаИННФЛ = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000009"]));
					Если ДлинаИННФЛ <> 12 Тогда
						ИмяПок = "П1000009" + "_" + НомерСтрокиП10000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " неверно задан ИНН агента авиакомпании", "Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
				ФИО = РегламентированнаяОтчетность.РазложитьФИО(Ветка_П10000.Данные["П1000010"]);
				Если НЕ ЗначениеЗаполнено(ФИО.Фамилия) Тогда
					ИмяПок = "П1000010" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана фамилия агента авиакомпании", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(ФИО.Имя) Тогда
					ИмяПок = "П1000010" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано имя агента авиакомпании", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			Иначе // СведОрг
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000009"]) Тогда
					ДлинаИНН = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000009"]));
					Если ДлинаИНН <> 10 Тогда
						ИмяПок = "П1000009" + "_" + НомерСтрокиП10000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " неверно задан ИНН агента авиакомпании", "Реестр", ИмяПок, НомерСтраницы);
					...
					КонецЕсли;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000010"]) Тогда
					ИмяПок = "П1000010" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование агента авиакомпании", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000011"]) Тогда
				...
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " не задан адрес агента авиакомпании", "Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ДлинаСтранАПОтпр = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000012"]));
			Если ДлинаСтранАПОтпр <> 2 Тогда
				ИмяПок = "П1000012" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не" + ?(ДлинаСтранАПОтпр = 0, "", "верно")
				+ " задан код страны аэропорта отправления в маршруте", "Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			ДлинаСтранАПНазн = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000013"]));
			Если ДлинаСтранАПНазн <> 2 Тогда
				ИмяПок = "П1000013" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не" + ?(ДлинаСтранАПНазн = 0, "", "верно")
				+ " задан код страны аэропорта назначения в маршруте", "Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100014"]) Тогда
					ИмяПок = "П1100014" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование аэропорта отправления в рейсе",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100024"]) Тогда
					ИмяПок = "П1100024" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан код аэропорта отправления в рейсе",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100015"]) Тогда
					ИмяПок = "П1100015" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование аэропорта назначения в рейсе",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100025"]) Тогда
					ИмяПок = "П1100025" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан код аэропорта назначения в рейсе",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100016"]) Тогда
					ИмяПок = "П1100016" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан номер рейса",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100017"]) Тогда
					ИмяПок = "П1100017" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана дата рейса",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрАВН_9 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрАВН_9");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрАВН_9 = СкопироватьУзел(УзелРодитель.Родитель, РеестрАВН_9);
		Для каждого Эл Из ЭкзРеестрАВН_9.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		// Узел "СведАВН" имеется только в схеме выгрузки "Сведений".
		// Если он не найден, значит, сейчас формируются данные "Реестра"
		// (для выгрузки первого из 2-х файлов).
		СведАВН = ПолучитьПодчиненныйЭлемент(ЭкзРеестрАВН_9, "СведАВН");
		Если СведАВН = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведАВН = СкопироватьУзел(ЭкзРеестрАВН_9, СведАВН, ЭкзРеестрАВН_9.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСведАВН.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
				...
				// Отправитель.
				Отпр = ПолучитьПодчиненныйЭлемент(ЭкзСведАВН, "Отпр");
				Для каждого Эл Из Отпр.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
						ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
					КонецЕсли;
				КонецЦикла;
				Парам = Новый Структура("Данные,ИмяОблПризнакИП,ИмяОблНаим",
					Ветка_П10000.Данные, "П1000091", "П1000004");
				ДобавитьУзелСведенийОбОрганизацииИлиИП(Отпр, Парам);
				
				// Агент авиакомпании.
				АгентАв = ПолучитьПодчиненныйЭлемент(ЭкзСведАВН, "АгентАв");
				Для каждого Эл Из АгентАв.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
						ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
					КонецЕсли;
				КонецЦикла;
				Парам = Новый Структура("Данные,ИмяОблПризнакИП,ИмяОблНаим",
					Ветка_П10000.Данные, "П1000093", "П1000010");
				ДобавитьУзелСведенийОбОрганизацииИлиИП(АгентАв, Парам);
				
				// Получатель.
				Получ = ПолучитьПодчиненныйЭлемент(ЭкзСведАВН, "Получ");
				Для каждого Эл Из Получ.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
						ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
					КонецЕсли;
				КонецЦикла;
				Парам = Новый Структура("Данные,ИмяОблПризнакИП,ИмяОблНаим",
					Ветка_П10000.Данные, "П1000092", "П1000007");
				ДобавитьУзелСведенийОбОрганизацииИлиИП(Получ, Парам);
				
				// Маршрут.
				Маршрут = ПолучитьПодчиненныйЭлемент(ЭкзСведАВН, "Маршрут");
				Для каждого Эл Из Маршрут.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
						ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
					КонецЕсли;
				КонецЦикла;
				// 11000 (рейсы).
				Рейс = ПолучитьПодчиненныйЭлемент(Маршрут, "Рейс");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					
					ЭкзРейс = СкопироватьУзел(Маршрут, Рейс, Маршрут.Строки.Количество() - 1);
					Для каждого Эл Из ЭкзРейс.Строки Цикл
						Если Эл.Строки.Количество() = 0 Тогда
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_9");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_9_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Для каждого Строка11 Из Строки11 Цикл
					...
					Если ЗначениеЗаполнено(Строка11.Данные.П1100014) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100014));
					КонецЕсли;
					Если ЗначениеЗаполнено(Строка11.Данные.П1100024) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100024));
					КонецЕсли;
					СтрокаДокумент.Параметры.П1100014 = СтрСоединить(МасТекЗнач, "; ");
					
					// Особая обработка для графы 15.
					МасТекЗнач = Новый Массив;
					Если ЗначениеЗаполнено(Строка11.Данные.П1100015) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100015));
					КонецЕсли;
					Если ЗначениеЗаполнено(Строка11.Данные.П1100025) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100025));
					КонецЕсли;
					СтрокаДокумент.Параметры.П1100015 = СтрСоединить(МасТекЗнач, "; ");
				...
				Если ЗначениеЗаполнено(ОблОбъедСтрока1_Верх) И ЗначениеЗаполнено(ОблОбъедСтрока1_Низ)
					...
					Для НомерКолонки = 2 По 20 Цикл
						Если НомерКолонки >= 15 И НомерКолонки <= 18 Тогда
							Продолжить;
						КонецЕсли;
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Реестр НДС Приложение 9");
...
&НаСервере
Процедура ПроверкаКС_9_1_1()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '9.1.1';
													|en = '9.1.1'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 9';
														|en = 'Реестр 9'"));
...
&НаСервере
Процедура ПроверкаКС_9_1_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '9.1.2';
													|en = '9.1.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 9';
														|en = 'Реестр 9'"));
...
&НаСервере
Процедура ПроверкаКС_9_1_3а()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '9.1.3.а';
													|en = '9.1.3.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 9';
														|en = 'Реестр 9'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны аэпорта отправления в гр.12 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны аэпорта отправления в гр.12 Реестра должен соответствовать справочнику ОКСМ'"));
	...
	Настройки.Вставить("ИмяПоказателя", "П1000012_");
	ПроверкаКС_9_1_3_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_9_1_3б()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '9.1.3.б';
													|en = '9.1.3.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 9';
														|en = 'Реестр 9'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны аэропорта назначения в гр.13 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны аэропорта назначения в гр.13 Реестра должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Код страны %1 соответствует справочнику ОКСМ';
													|en = 'Код страны %1 соответствует справочнику ОКСМ'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный код страны';
														|en = 'Ошибочный код страны'"));
	...
	Настройки.Вставить("ИмяПоказателя", "П1000013_");
	ПроверкаКС_9_1_3_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_9_1_3_код(Настройки)
...
&НаСервере
Процедура ПроверкаКС_9_1_4()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '9.1.4';
													|en = '9.1.4'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 9';
														|en = 'Реестр 9'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'п.3.9 ст.165 НК РФ';
													|en = 'п.3.9 ст.165 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Номер авианакладной в гр.2 Реестра должен состоять из 11 цифр';
			|en = 'Номер авианакладной в гр.2 Реестра должен состоять из 11 цифр'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Номер %1 соответствует формату';
													|en = 'Номер %1 соответствует формату'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный номер авианакладной';
														|en = 'Ошибочный номер авианакладной'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1, Строка %2';
													|en = 'Страница %1, Строка %2'"));
	Настройки = ОбработатьНастройки(Заголовки);
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			...
			Настройки.ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р01Г001.Значение) 
				И НЕ (СтрДлина(Показатели.Р01Г001.Значение) = 11
				И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Показатели.Р01Г001.Значение));
...
&НаСервере
Процедура ПроверкаКС_9_2_1а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '9.2.1.а';
													|en = '9.2.1.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 9';
														|en = 'Реестр 9'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_9_2_1б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '9.2.1.б';
													|en = '9.2.1.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 9';
														|en = 'Реестр 9'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
