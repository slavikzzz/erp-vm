...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Если РазделЗаполнен(ДанныеРеестра.Строки, Ложь) Тогда
		...
		ПроверитьКС_9_1_1(СтруктураРезультата);
		ПроверитьКС_9_1_2(СтруктураРезультата);
		ПроверитьКС_9_1_3(СтруктураРезультата);
		ПроверитьКС_9_1_4(СтруктураРезультата);
		ПроверитьКС_9_2_1(СтруктураРезультата);
		ПроверитьКС_9_2_2(СтруктураРезультата);
		ПроверитьКС_9_2_3(СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_9_1_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 9.1.1';
												|en = 'Проверка КС 9.1.1'"));
...
&НаСервере
Процедура ПроверитьКС_9_1_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 9.1.2';
												|en = 'Проверка КС 9.1.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_9_1_3(СтруктураРезультата)
	
	Заголовки		= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		,НСтр("ru = 'Проверка КС 9.1.3';
													|en = 'Проверка КС 9.1.3'"));
	Заголовки.Вставить("ОписаниеКС"			,
		НСтр("ru = 'Указанный код страны аэропорта отправления (графа 12 Реестра) должен соответствовать справочнику ОКСМ';
			|en = 'Указанный код страны аэропорта отправления (графа 12 Реестра) должен соответствовать справочнику ОКСМ'"));
	...
	Заголовки.Вставить("ШаблонРасшифровки"	,НСтр("ru = 'Указанный код: (""%1"")';
													|en = 'Указанный код: (""%1"")'"));
	Заголовки.Вставить("Показатель"			,НСтр("ru = 'АэропортОтправления';
													|en = 'АэропортОтправления'"));
	...
	Заголовки.Вставить("ТекстФормулы"		,НСтр("ru = 'Проверка КС 9.1.3';
													|en = 'Проверка КС 9.1.3'"));
	Заголовки.Вставить("ОписаниеКС"			,
		НСтр("ru = 'Указанный код страны аэропорта назначения (графа 13 Реестра) должен соответствовать справочнику ОКСМ';
			|en = 'Указанный код страны аэропорта назначения (графа 13 Реестра) должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонОшибки"		,
	...
	Заголовки.Вставить("ШаблонРасшифровки"	,НСтр("ru = 'Указанный код: (""%1"")';
													|en = 'Указанный код: (""%1"")'"));
	Заголовки.Вставить("Показатель"			,НСтр("ru = 'АэропортНазначения';
													|en = 'АэропортНазначения'"));
	Заголовки.Вставить("ПроверятьПустые"	,Ложь);
	Настройки	= ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_9_1_4(СтруктураРезультата)
	
	Заголовки		= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		,НСтр("ru = 'Проверка КС 9.1.4';
													|en = 'Проверка КС 9.1.4'"));
	Заголовки.Вставить("Нарушение"			,НСтр("ru = 'п. 3.1 ст. 165 НК РФ';
													|en = 'п. 3.1 ст. 165 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			,
		НСтр("ru = 'Проверка формата номера авианакладной. С учетом нормализации номер должен быть 11 цифр (графа 2 Реестра)';
			|en = 'Проверка формата номера авианакладной. С учетом нормализации номер должен быть 11 цифр (графа 2 Реестра)'"));
	Заголовки.Вставить("ШаблонОшибки"		,	
	...
	ПроверитьКС_СоответствиеНомераАвианакладнойФормату(СтруктураРезультата, Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверитьКС_9_2_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 9.2.1';
												|en = 'Проверка КС 9.2.1'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_9_2_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 9.2.2';
												|en = 'Проверка КС 9.2.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_9_2_3(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 9.2.3';
												|en = 'Проверка КС 9.2.3'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_СоответствиеНомераАвианакладнойФормату(СтруктураРезультата, Настройки);
	...
	Для каждого СтраницаРаздела Из ДанныеРеестра.Строки Цикл
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл
			...
			Настройки.Вставить("ДопСведенияПараметры", Новый Структура("НомерСтраницы, НомерСтроки", НомерСтраницы, 
				НомерРасчетный));
			ЗарегистрироватьФормулу(Настройки);
			Показатели		= ЗаполнитьПоказатели("НомерДокумента", ДанныеСтраницы[НомерСтроки - 1].Данные, НомерСтроки);
			РасшифроватьПоказатели(Показатели, Настройки, "Реестр", НомерСтраницы);
			Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	ИначеЕсли УсловныйКодПоказателя = "АэропортОтправления" Тогда
	...
	ИначеЕсли УсловныйКодПоказателя = "АэропортНазначения" Тогда
