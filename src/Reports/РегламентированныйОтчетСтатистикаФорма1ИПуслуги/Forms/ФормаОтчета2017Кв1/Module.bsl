...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "08/11/2017";
	// Код отчета, равен КНД.
	СтруктураРеквизитовФормы.мКодОтчета   = "0609709";
...
Процедура ПоказатьПериод()
	...
	ПериодОтчета = " за " + Сред(СтрПериод, 1, СтрДлина(СтрПериод)-2) + "год";
...
Процедура СформироватьСпискиВыбора()
	...
	КодыОКУН = КоллекцияСписковВыбора["СправочникОКУН"].СкопироватьКолонки();
	НовСтр = КодыОКУН.Добавить();
	ЗаполнитьЗначенияСвойств(НовСтр, КоллекцияСписковВыбора["СправочникОКУН"][0]);
	Для Каждого Стр Из КоллекцияСписковВыбора["СправочникОКУН"] Цикл 
		Если КоллекцияСписковВыбора["ФильтрОКУН"].НайтиСтроки(Новый Структура("Код", Стр.Код)).Количество() > 0 Тогда 
			НовСтр = КодыОКУН.Добавить();
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ПОЛЕОКУН", 250, , "Выбор из списка ОКУН", КодыОКУН);
КонецПроцедуры
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если СтрНайти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы1) > 0 Тогда
		Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "03" Тогда
			...
			ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "03_"+НомерСтроки].Значение = ВыборКод;
		КонецЕсли;
	КонецЕсли;
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Титульный" Тогда
		...
		КонецЕсли;
	Иначе
		Если СтрНачинаетсяС(Область.Имя, "П002000100101_") Или СтрНачинаетсяС(Область.Имя, "П002000100103_") Тогда 
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ПОЛЕОКУН");
...
&НаКлиентеНаСервереБезКонтекста
Процедура ПеренумероватьСтроки(ТД)
	Инд = 1;
	Пока Истина Цикл
		Обл = ТД.Области.Найти("П002000100102_" + Формат(Инд, "ЧГ="));
		Если Обл = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		Обл.Значение = ?(Инд < 100, Формат(Инд, "ЧЦ=2; ЧВН="), Формат(Инд, "ЧГ="));
		Инд = Инд + 1;
	КонецЦикла;
КонецПроцедуры
