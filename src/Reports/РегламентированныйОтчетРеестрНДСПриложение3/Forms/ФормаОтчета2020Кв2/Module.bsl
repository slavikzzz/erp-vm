	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000003"]) Тогда
				...
				+ НомПП + " не задана дата документа подтверждающего оказание услуг по организации транспортировки газа",
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Если РазделЗаполнен(ДанныеРеестра.Строки, Ложь) Тогда
		...
		ПроверитьКС_3_1_1(СтруктураРезультата);
		ПроверитьКС_3_1_2(СтруктураРезультата);
		ПроверитьКС_3_1_3(СтруктураРезультата);
		ПроверитьКС_3_2_1(СтруктураРезультата);
		ПроверитьКС_3_2_2(СтруктураРезультата);
		ПроверитьКС_3_2_3(СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_3_1_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 3.1.1';
												|en = 'Проверка КС 3.1.1'"));
...
&НаСервере
Процедура ПроверитьКС_3_1_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 3.1.2';
												|en = 'Проверка КС 3.1.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_3_1_3(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		,НСтр("ru = 'Проверка КС 3.1.3';
													|en = 'Проверка КС 3.1.3'"));
	Заголовки.Вставить("ОписаниеКС"			,
		НСтр("ru = 'Контроль соблюдения положений п. 9 ст. 165 Кодекса (учет 180 дней). С даты документа, ';
			|en = 'Контроль соблюдения положений п. 9 ст. 165 Кодекса (учет 180 дней). С даты документа, '") +
		НСтр("ru = 'подтверждающего оказание услуг, в графе 3, не должно пройти больше 180 дней';
			|en = 'подтверждающего оказание услуг, в графе 3, не должно пройти больше 180 дней'"));
	Настройки	= ОбработатьНастройки(		,Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_3_2_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 3.2.1';
												|en = 'Проверка КС 3.2.1'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_3_2_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 3.2.2';
												|en = 'Проверка КС 3.2.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_3_2_3(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 3.2.3';
												|en = 'Проверка КС 3.2.3'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
Процедура ПроверитьКС_Контроль180Дней(СтруктураРезультата, Настройки)
	...
	Настройки.Вставить("ШаблонОшибки"		
		,НСтр("ru = 'С даты документа, подтверждающего оказание услуг, прошло более 180 дней (просрочка: %1 дн.)';
				|en = 'С даты документа, подтверждающего оказание услуг, прошло более 180 дней (просрочка: %1 дн.)'"));
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	ИначеЕсли УсловныйКодПоказателя = "ДатаДокумента" Тогда
		...
		Результат.Вставить("Представление", Нстр("ru = 'Дата документа, подтверждающего оказание услуг';
												|en = 'Дата документа, подтверждающего оказание услуг'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
