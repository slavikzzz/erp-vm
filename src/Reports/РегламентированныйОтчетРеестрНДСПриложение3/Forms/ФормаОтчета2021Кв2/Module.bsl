...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр р.НДС прилож.03 21к2") Тогда
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 8) = "П1000009" Тогда
...
Функция РеквизитыЭлементаСправочника(Результат)
	...
	Возврат Новый Структура("Код", Результат.Код);
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя)
				...
				Если ЗначениеЗаполнено(КодОперации) Тогда
					Ном = 1;
					Пока Форма.ТабличныйДокумент.Области.Найти("П1000011_" + Формат(Ном, "ЧГ=")) <> Неопределено Цикл
						СуммаНалБаз = СуммаНалБаз
							+ Форма.ТабличныйДокумент.Области["П1000011_" + Формат(Ном, "ЧГ=")].Значение;
						Ном = Ном + 1;
					КонецЦикла;
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если Ветка = АктивнаяВетка Тогда
				...
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					Ветка_П10000.Данные.Вставить("П1000011", Форма.ТабличныйДокумент.Области["П1000011_"
			...
			Если ЗначениеЗаполнено(КодОперации) Тогда
				ДанныеМнЧ_П10000 = Ветка.ДанныеМногострочныхЧастей.П10000.Строки;
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					СуммаНалБаз = СуммаНалБаз + Ветка_П10000.Данные["П1000011"];
				КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("НалБазаИтого", "КодОперации;П1000011");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрТрГаз_3");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRGAZNDS"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRGAZNDS.2"));
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000004"]) Тогда
				...
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не задана дата оказания услуги",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000006"]) Тогда
				...
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не задана дата договора на оказание услуг",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			ДлинаКодВалютДог = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000009"]));
			Если ДлинаКодВалютДог <> 3 Тогда
				ИмяПок = "П1000009" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не" + ?(ДлинаКодВалютДог = 0, "", "верно")
				+ " задан код валюты договора",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрТрГаз_3 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрТрГаз_3");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрТрГаз_3 = СкопироватьУзел(УзелРодитель.Родитель, РеестрТрГаз_3);
		Для каждого Эл Из ЭкзРеестрТрГаз_3.Строки Цикл
		...
		СведДкПдтвУслуг = ПолучитьПодчиненныйЭлемент(ЭкзРеестрТрГаз_3, "СведДкПдтвУслуг");
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведДкПдтвУслуг = СкопироватьУзел(ЭкзРеестрТрГаз_3, СведДкПдтвУслуг, ЭкзРеестрТрГаз_3.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСведДкПдтвУслуг.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
						Если Эл.Код = "НомерДк" ИЛИ Эл.Код = "НомерДог" Тогда
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_3");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_3_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Реестр НДС Приложение 3");
...
#Область ОбщиеДанные
	...
	ДанныеКС.Вставить("ГрафаНалоговойБазы"		, "11");
...
&НаСервере
Процедура ПроверкаКС_3_1_1()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '3.1.1';
													|en = '3.1.1'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 3';
														|en = 'Реестр 3'"));
...
&НаСервере
Процедура ПроверкаКС_3_1_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '3.1.2';
													|en = '3.1.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 3';
														|en = 'Реестр 3'"));
...
&НаСервере
Процедура ПроверкаКС_3_1_3()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '3.1.3';
													|en = '3.1.3'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 3';
														|en = 'Реестр 3'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'п.9 ст.165 НК РФ';
													|en = 'п.9 ст.165 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'С даты документа в графе 3 Реестра, подтверждающего оказание услуг, должно пройти не ';
			|en = 'С даты документа в графе 3 Реестра, подтверждающего оказание услуг, должно пройти не '") +
		НСтр("ru = 'более 180 дней';
			|en = 'более 180 дней'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 + 180 дн. Итог [%2] >= %3';
													|en = '%1 + 180 дн. Итог [%2] >= %3'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочная дата документа';
														|en = 'Ошибочная дата документа'"));
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			...
			РасчетнаяДата = Показатели.Р01Г001.Значение + (180 * 86400);
			Настройки.ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р01Г001.Значение)
				И РасчетнаяДата < ДанныеКС.Отчет.ДатаНачала;
			Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р01Г001.ЗначениеСтр,
				Формат(?(ЗначениеЗаполнено(Показатели.Р01Г001.Значение), РасчетнаяДата, 
				Показатели.Р01Г001.Значение), "ДФ=dd.MM.yy; ДП=(...)"), 
				Формат(ДанныеКС.Отчет.ДатаНачала, "ДФ=dd.MM.yy; ДП=(...)"));
...
&НаСервере
Процедура ПроверкаКС_3_2_1а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '3.2.1.а';
													|en = '3.2.1.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 3';
														|en = 'Реестр 3'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_3_2_1б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '3.2.1.б';
													|en = '3.2.1.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 3';
														|en = 'Реестр 3'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
