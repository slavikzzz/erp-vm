...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	СтрСум = "1001101010201030104010501060107110721073108010901100111011201150"
			+ "16201630163216401660169017001720173017701780";
...
Процедура ДополнитьВыгрузкуПараметрамиПараметрами(ТабДокумент)
	...
	Пока ТабДокумент.Свойство("П003000100001_" + Формат(Инд, "ЧГ=0")) Цикл 
		...
		ТабДокумент.Вставить("СТР0202" + Суффикс, ТабДокумент["П003000100002" + Суффикс]);
		КТот = 2;
		Для К = 5 По 8 Цикл
			КТот = КТот + 1;
	...
	Суффикс = "_" + Формат(Инд, "ЧГ=0");
	ТабДокумент.Вставить("СТР0202" + Суффикс, "9500");
	ТабДокумент.Вставить("СТР0203" + Суффикс, "");
	КТот = 3;
	Для К = 6 По 8 Цикл
		КТот = КТот + 1;
		КТотСтр = Формат(КТот, "ЧЦ=2; ЧВН=; ЧГ=0");
	...
	Для Каждого КЗ Из СтрокиРаздела1 Цикл
		...
		Если ЕстьНенулевыеПоСтроке Тогда 
			ТабДокумент.Вставить("СТР0100" + Суффикс, КЗ.Ключ);
		Иначе
			ТабДокумент.Вставить("СТР0100" + Суффикс);
		КонецЕсли;
		Инд = Инд + 1;
	КонецЦикла;
	...
	Для СтрУголь = 1 По 5 Цикл
		...
		Если ЕстьНенулевыеПоСтроке Тогда 
			ТабДокумент.Вставить("СТР0100" + Суффикс, ТабДокумент["П000001116012" + СуффиксУголь]);
			Инд = Инд + 1;
		Иначе
			ТабДокумент.Вставить("СТР0100" + Суффикс);
		КонецЕсли;
