...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ЛевОбластьИмя = "П003000100001_" Или ЛевОбластьИмя = "П003000100009_" 
			...
			Или ЛевОбластьИмя = "П003000400001_" Или ЛевОбластьИмя = "П003000400011_" Тогда
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	//строка 33 графа 3 = сумме строк 34, 35, 36 по графе 3.
	Области.П001001003303.Значение
		= Области.П001001003403.Значение
		+ Области.П001001003503.Значение
		+ Области.П001001003603.Значение;
	
	//строка 17 по графам 3, 4 = сумме строк  19, 24 по графам 3, 4;
	Области.П001001001703.Значение = Области.П001001001903.Значение + Области.П001001002403.Значение;
	Области.П001001001704.Значение = Области.П001001001904.Значение + Области.П001001002404.Значение;
	
	//строка 16 по графам 1÷4, 7÷11 = сумме строк 18, 23 по графам 1÷4, 7÷11;
	Для Гр = 1 По 11 Цикл
		...
		Области["П0010010016" + Графа].Значение = Области["П0010010018" + Графа].Значение
			+ Области["П0010010023" + Графа].Значение;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 1 По 11 Цикл
		...
		мФормаОтчета.Области["П0010010016" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0010010016" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Гр = 3 По 4 Цикл
		мФормаОтчета.Области["П00100100170" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00100100170" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	мФормаОтчета.Области["П001001003303"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001003303"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Для Стр = 1 По 4 Цикл
		Области["П003000" + Стр + "00002_1"].Значение = "39";
	КонецЦикла;
...
Процедура ЗаполнитьКлючиМногострочнойЧасти(Форма, ТекОбласть, ПослеВыбораИзСписка = Ложь)
	...
	ИначеЕсли СтрНачинаетсяС(ТекОбластьИмя, "П003000100009_")
		...
		Или СтрНачинаетсяС(ТекОбластьИмя, "П003000400011_") Тогда
