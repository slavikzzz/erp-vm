...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// т.к. они уже учтены в отборе основных слагаемых/вычитаемых.
	...
	Для Каждого ПоказательРасшифровки Из ПоказателиРасшифровки Цикл
		...
		Иначе
			ТекстСообщения = НСтр("ru = 'В показателе ""%1"" использован неподдерживаемый дополнительный алгоритм ""%2""';
									|en = 'Unsupported additional algorithm ""%2"" is used in the ""%1"" indicator'");
	...
	// Возможно, что из-за дополнительных алгоритмов удалены все показатели для конкретного поля.
	// Например, если не настроен дополнительный отбор, то алгоритм ДобавитьОтбор отключает
	// все составляющие расчета для данной ячейки
	Если Показатели.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = Форма.ПредставлениеОтчета
		...
		+ " " + БухгалтерскиеОтчетыВызовСервера.ПолучитьТекстОрганизация(Отчет.Организация, Ложь, Отчет.КонецПериода);
...
Процедура СформироватьОтчетПоСоставномуПоказателю(Показатели, ПоказателиОтбора, ЗаголовокФормыОтчета)
	...
	Область.Параметры.Организация 	= Отчет.Организация;
	...
	Если ДеревоНастроек.Количество() > 0 Тогда
		...
		Для Каждого СтрокаДерева Из ДеревоНастроек Цикл
			...
			Если Настройка <> Неопределено И Настройка.Элементы.Количество() > 0 Тогда
				...
				// Так как в компоновщике инициализированы настройки, которыми оперирует данный отбор,
...
Процедура ВывестиЗаголовокОтчета(ПараметрыОтчета, Результат) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.Организация) Тогда
		ТекстОрганизация = БухгалтерскиеОтчетыВызовСервера.ПолучитьТекстОрганизация(ПараметрыОтчета.Организация, Ложь, Отчет.КонецПериода);
