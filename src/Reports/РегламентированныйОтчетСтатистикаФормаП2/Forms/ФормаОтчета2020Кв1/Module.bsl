...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "12/08/2019";
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	КонецЦикла;
	ЭтотОбъект.ПолеТабличногоДокументаФормаОтчета.Области["ВыбратьОтчетПрошлогоПериода"].Значение = "Выбор отчета предыдущего периода";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Области.П000100106.Значение = Области.П000100206.Значение
								...
								+Области.П000101406.Значение;
	
	Области.П000102003.Значение = Области.П000102103.Значение
								+Области.П000102203.Значение;
	
	Области.П000102004.Значение = Области.П000102104.Значение
								+Области.П000102204.Значение;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверки.Добавить(ТабДок.ПолучитьОбласть("ОсновнаяЧасть3_3"));
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок2", РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтаФорма, П0000003046, Ложь));
...
Процедура УдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, ТекущаяСтраница = Неопределено) Экспорт
	...
	Если СтруктураГруппы.Количество() > ИсходноеКоличествоСтрокГруппы Тогда
		...
		ВысотаСтр = ?(ИмяГруппы = Форма.СтруктураРеквизитовФормы.мИдГруппы1, 1, 0);
		
		// номера строк удаляемой области
		НомерВерхЯчейкиОбласти = ОбластьЯчейки.Верх - ВысотаСтр;
		НомерНижнЯчейкиОбласти = ОбластьЯчейки.Низ + ВысотаСтр;
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	ВысотаСтр = ?(ИмяГруппы = Форма.СтруктураРеквизитовФормы.мИдГруппы1, 2, 0);
...
Процедура ДобавитьНеобходимоеКоличествоДопСтрокНаСервере(ТекТабличноеПоле, ТекТабличноеПолеИмя, ИсходноеКоличествоСтрокГруппы, ТекущееКоличествоСтрокГруппы, ИмяГруппы, СтруктураГруппы) Экспорт
	МногострочнаяЧастьОбл = ?(ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы1, "МногострочнаяЧасть", "МногострочнаяЧасть2");
	Дельта1 = ?(ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы1, 2, 0);
	
	МногострочнаяЧастьВерх = ТекТабличноеПоле.Области[МногострочнаяЧастьОбл].Верх;
	МногострочнаяЧастьНиз = ТекТабличноеПоле.Области[МногострочнаяЧастьОбл].Низ;
	
	ОбластьДопСтроки = ТекТабличноеПоле.ПолучитьОбласть(МногострочнаяЧастьНиз - Дельта1, , МногострочнаяЧастьНиз);
		
	ТекТабличноеПоле.ВставитьОбласть(ОбластьДопСтроки.Область(), ТекТабличноеПоле.Область(МногострочнаяЧастьНиз + 1, , МногострочнаяЧастьНиз + (1+Дельта1)*(ТекущееКоличествоСтрокГруппы - ИсходноеКоличествоСтрокГруппы)), ТипСмещенияТабличногоДокумента.ПоВертикали);
	...
	Для НомНовойСтроки = ИсходноеКоличествоСтрокГруппы По ТекущееКоличествоСтрокГруппы + 1 Цикл
		...
		Для Каждого Область Из ОбластьДопСтроки.Области Цикл
			...
			ОбластьТаблДок = ТекТабличноеПоле.Область(МногострочнаяЧастьНиз + (1+Дельта1)*(ТекущееКоличествоСтрокГруппы - НомНовойСтроки - 1) + Область.Верх,
														Область.Лево,
														МногострочнаяЧастьНиз + (1+Дельта1)*(ТекущееКоличествоСтрокГруппы - НомНовойСтроки - 1) + Область.Верх,
														Область.Право);
	...
	ТекТабличноеПоле.Область(ТекТабличноеПоле.Области[МногострочнаяЧастьОбл].Верх, , ТекТабличноеПоле.Области[МногострочнаяЧастьОбл].Низ).Имя = "";
	
	ТекТабличноеПоле.Область(МногострочнаяЧастьВерх, , МногострочнаяЧастьНиз).Имя = МногострочнаяЧастьОбл;
...
Процедура ПронумероватьМногострочныеСтроки(ПолеТабличногоДокументаФормаОтчета)
	ПолеТабличногоДокументаФормаОтчета.Области["П000000101502_1"].Значение = "17";
	ПолеТабличногоДокументаФормаОтчета.Области["П000000101502_2"].Значение = "18";
	ПолеТабличногоДокументаФормаОтчета.Области["П000000101502_3"].Значение = "19";
