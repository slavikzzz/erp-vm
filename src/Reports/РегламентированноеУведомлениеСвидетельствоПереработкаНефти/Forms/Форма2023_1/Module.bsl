...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.СвидетельствоПереработкаНефти;
...
Процедура ЗаполнитьНачальныеДанные() Экспорт
	...
	СтрокаСведений = "ИННЮЛ,НаимЮЛПол,КППЮЛ,ТелОрганизации,НаимЮЛСокр,АдрПочт,АдрФакт";
	...
	ДанныеУведомленияТитульный.Вставить("МестоНахож", СведенияОбОрганизации.АдрПочт);
	ДанныеУведомленияТитульный.Вставить("АдрОсущ", СведенияОбОрганизации.АдрФакт);
...
Процедура СформироватьДеревоСтраниц() Экспорт
	...
	Стр001 = КорневойУровень.Добавить();
	Стр001.Наименование = "Перечень" + Символы.ПС + "производственных" + Символы.ПС + "мощностей";
	Стр001.ИндексКартинки = 1;
	...
	Стр0001.ИмяМакета = "ПерПроизвДата_2023";
	Стр0001.Многостраничность = Истина;
	Стр0001.Многострочность = Ложь;
	Стр0001.УИД = Новый УникальныйИдентификатор;
	Стр0001.ИДНаименования = "ПерПроизвДата";
	
	Стр001 = Стр0001.ПолучитьЭлементы().Добавить();
	Стр001.Наименование = "Сведения о" + Символы.ПС + "технологическом" + Символы.ПС + "процессе";
	Стр001.ИндексКартинки = 1;
	Стр001.Многостраничность = Истина;
	Стр001.Многострочность = Истина;
	...
	Стр0001.Наименование = "Стр. 1";
	Стр0001.ИндексКартинки = 1;
	Стр0001.ИмяМакета = "ТехнПроц_2023";
	Стр0001.Многостраничность = Истина;
	Стр0001.Многострочность = Истина;
	Стр0001.УИД = Новый УникальныйИдентификатор;
	Стр0001.ИДНаименования = "ТехнПроц";
...
Процедура ЗагрузитьИзXMLНаСервере(ПараметрыЗагрузкиXML)
	...
	Узлы = УведомлениеОСпецрежимахНалогообложения.НайтиУзелыВДеревеПоПути(ДеревоЗагрузки, "Файл/Документ/ЗаявНефт/ВидДеят", "ИмяЭлемента");
	Для Каждого Узел Из Узлы Цикл
		Если ЭтотОбъект["ДанныеУведомления"].Титульная.Свойство("Флаг" + Узел.ЗначениеЭлемента) Тогда
			ЭтотОбъект["ДанныеУведомления"].Титульная["Флаг" + Узел.ЗначениеЭлемента] = Истина;
		КонецЕсли;
	КонецЦикла;
	Модифицированность = Истина;
КонецПроцедуры
