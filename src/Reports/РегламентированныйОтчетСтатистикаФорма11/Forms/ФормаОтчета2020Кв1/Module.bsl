...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Пока Истина Цикл 
		...
		Если ЗначениеЗаполнено(ОКВЭД) Тогда 
			Если Стр16Заполнена Тогда
				Префикс = "П2001020001";
				Стр17Заполнена = Истина;
			Иначе
				Префикс = "П1001020001";
				Стр16Заполнена = Истина;
			КонецЕсли;
			Для Гр = 3 По 14 Цикл
				ГрСтр = Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=");
				ПараметрыВыгрузки.Вставить(Префикс + ГрСтр, ПараметрыВыгрузки["П0001020001" + ГрСтр + "_" + ИндСтр]);
				...
				Если ПараметрыВыгрузки.Свойство("ЧНП0001020001" + ГрСтр + "_" + ИндСтр) Тогда 
					ПараметрыВыгрузки.Вставить("ЧН" + Префикс + ГрСтр, ПараметрыВыгрузки["ЧНП0001020001" + ГрСтр + "_" + ИндСтр]);
					ПараметрыВыгрузки["ЧНП0001020001" + ГрСтр + "_" + ИндСтр] = Неопределено;
				КонецЕсли;
			КонецЦикла;
			Если Стр17Заполнена Тогда 
				Прервать;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
