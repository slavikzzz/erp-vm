...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 4 Цикл
		Области["П00004010380" + Гр].Значение = Области["П00004010390" + Гр].Значение
			+ Области["П00004010410" + Гр].Значение
			+ Области["П00004010420" + Гр].Значение
			+ Области["П00004010430" + Гр].Значение
			+ Области["П00004010440" + Гр].Значение;
	КонецЦикла;
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Пока ПараметрыВыгрузки.Свойство("П000050100102_" + Формат(Инд, "ЧГ=")) Цикл 
		Инд = Инд + 1;
	КонецЦикла;
	ПараметрыВыгрузки.Вставить("П000050100102_" + Формат(Инд, "ЧГ="), ПараметрыВыгрузки["П000050100002"]);
	ПараметрыВыгрузки.Вставить("П000050100103_" + Формат(Инд, "ЧГ="), ПараметрыВыгрузки["П000050100003"]);
