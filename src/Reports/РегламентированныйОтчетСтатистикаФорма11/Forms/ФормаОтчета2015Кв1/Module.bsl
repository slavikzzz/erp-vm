...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	СтруктураДанных.Свойство("Титульный", Титульный);
	СтруктураДанных.Вставить("Подразделения", Подразделения);
	СтруктураДанных.Свойство("ТаблицаРасшифровки", ТаблицаРасшифровки);
	Если ТипЗнч(Подразделения) <> Тип("ТаблицаЗначений") Тогда 
		Возврат;
	КонецЕсли;
	Модифицированность = Истина;
	Доступность = Истина;
...
Функция ЗаполнитьАвтоНаСервере()
	...
	КТ = Контейнер.Вставить("Титульный");
	Для каждого ЭлементСтруктуры Из СтруктураДанныхТитульный Цикл
		КТ.Вставить(ЭлементСтруктуры.Ключ, РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтаФорма, ЭлементСтруктуры.Значение, Ложь));
	КонецЦикла;
	Контейнер.Вставить("Подразделения", ТаблицаСтраницыПодразделения);
