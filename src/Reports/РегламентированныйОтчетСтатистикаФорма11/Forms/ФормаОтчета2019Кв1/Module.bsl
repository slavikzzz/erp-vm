...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "23/07/2018";
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ПолныйОКВЭД", 512, , "Выбор кода", КоллекцияСписковВыбора["ПолныйОКВЭД"]);
КонецПроцедуры
...
Функция ОпределитьИдМногострочногоРазделаПоТабличномуПолю(ТекТабличноеПоле) Экспорт
	Если "Титульный" = ТекТабличноеПоле Тогда
		Возврат СтруктураРеквизитовФормы.мИдГруппы1;
	КонецЕсли;
КонецФункции
	...
	Если (Неопределено <> Форма.ТабличныйДокумент.Области.Найти("ОргКодОКПО"))
		...
		Если (НЕ ЗначениеЗаполнено(Форма.ТабличныйДокумент.Области.ОргНазв.Значение)) ИЛИ (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) Тогда
			Если ЗначениеЗаполнено(Форма.ОбособленноеПодразделение) Тогда 
				Форма["ТабличныйДокумент"].Области.ОргНазв.Значение = Строка(Форма.ОбособленноеПодразделение) + ?(ПустаяСтрока(НаимЮЛПол), "", " (" + НаимЮЛПол + ")");
			Иначе
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	Если ИмяТекущейСтраницыПанели = "Титульный" Тогда
		...
		Для НомКол = 3 По 13 Цикл
			...
			Области["П0010500" + НомКолСтр].Значение = Области["П0010600" + НомКолСтр].Значение + Области["П0010700" + НомКолСтр].Значение + Области["П0010800" + НомКолСтр].Значение;
			Области["П0010900" + НомКолСтр].Значение = Области["П0011000" + НомКолСтр].Значение + Области["П0011200" + НомКолСтр].Значение;
			Области["П0010100" + НомКолСтр].Значение = Области["П0010200" + НомКолСтр].Значение + Области["П0010400" + НомКолСтр].Значение + Области["П0010500" + НомКолСтр].Значение
											+ Области["П0010900" + НомКолСтр].Значение + Области["П0011300" + НомКолСтр].Значение + Области["П0011400" + НомКолСтр].Значение;
		КонецЦикла;

		Для НомКол = 3 По 4 Цикл
			НомКолСтр = Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=");
			Области["П0100046" + НомКолСтр].Значение =
			Области["П0100047" + НомКолСтр].Значение + Области["П0100049" + НомКолСтр].Значение
			+ Области["П0100050" + НомКолСтр].Значение + Области["П0100051" + НомКолСтр].Значение;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	ИначеЕсли ИмяТекРаздела = "Титульный" Тогда
		...
		ТабличныйДокумент.Области.ИсключитьДанныеОбособленныхПодразделений.Значение = ИсключитьДанныеОбособленныхПодразделений;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Инд = 0;
	Для Каждого Лист Из СтруктураРеквизитовФормы.мПечатныеФормы Цикл 
		Обл = Лист.Значение.Области.Найти("ОргАдресЭлектроннойПочты");
		Если Обл <> Неопределено Тогда 
			ПослСтр = СтруктураРеквизитовФормы.мПечатныеФормы[СтруктураРеквизитовФормы.мПечатныеФормы.Количество() - 1].Значение;
			ПослСтр.Вывести(Лист.Значение);
			СтруктураРеквизитовФормы.мПечатныеФормы.Удалить(Инд);
			Прервать;
		КонецЕсли;
		Инд = Инд + 1;
	КонецЦикла;
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Подразделения",   "ФормаОтчета2019Кв1_Подразделения");
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 Тогда
		УдалитьСтроку("");
	ИначеЕсли Лев(Область.Имя, СтрДлина("П000102000101_")) = "П000102000101_" Тогда
	...
	ИначеЕсли Область.Имя = "КодОКВЭД" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ПолныйОКВЭД");
		КодПоказателя = ПараметрВводаПоля.Показатель;
		КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка);
...
Процедура ЗаполнитьДаннымиПодразделенияДопСтраница(Подразделение)
	Попытка
		...
		ТабличныйДокумент.Области["КодОКПО"].Значение = ОКПО;
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	Для Каждого ПоказателиСтраницы Из Контейнер Цикл
		...
		Для Каждого Пок Из СтруктураДанныхТитульный Цикл 
			ДанныеИзКонтейнера = Неопределено;
			Если ПоказателиТаблПоле.Свойство(Пок.Ключ, ДанныеИзКонтейнера) 
				И ТипЗнч(ДанныеИзКонтейнера) = ТипЗнч(Пок.Значение) Тогда 

				СтруктураДанныхТитульный[Пок.Ключ] = ДанныеИзКонтейнера;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	НадоФормироватьДерево = Ложь;
	Если Контейнер.Свойство("ТаблицаСтраницыПодразделения", ТаблицаГруппы) И ТаблицаГруппы <> Неопределено Тогда
		Если ТаблицаСтраницыПодразделения.Количество() > ТаблицаГруппы.Количество() Тогда 
			Для Инд = ТаблицаГруппы.Количество() По ТаблицаСтраницыПодразделения.Количество() - 1 Цикл 
				ТаблицаСтраницыПодразделения.Удалить(Инд);
				НадоФормироватьДерево = Истина;
			КонецЦикла;
		КонецЕсли;
		Для Инд = 0 По ТаблицаГруппы.Количество() - 1 Цикл 
			Стр = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(ТаблицаГруппы[Инд]);
			НовыеДанные = РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтаФорма, Стр);
			ТаблицаСтраницыПодразделения[Инд].Данные[0].Значение = НовыеДанные;
		КонецЦикла;
	КонецЕсли;
	
	Если НадоФормироватьДерево Тогда 
		СформироватьДеревоРазделовОтчета(ЭтаФорма);
	КонецЕсли;
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Контейнер.Вставить("ПолеТабличногоДокументаТитульный_ДанныеДопСтрок", РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтаФорма, П0001020001, Ложь));
	ТаблицаСтраницыПодразделенияКонтейнер = РеквизитФормыВЗначение("ТаблицаСтраницыПодразделения");
	
	ДопСтраницы = Новый ТаблицаЗначений;
	Для Каждого КЗ Из ТаблицаСтраницыПодразделенияКонтейнер[0].Данные[0].Значение Цикл
		ДопСтраницы.Колонки.Добавить(КЗ.Ключ, Новый ОписаниеТипов(Строка(ТипЗнч(КЗ.Значение))));
	КонецЦикла;
	
	Для Каждого Стр Из ТаблицаСтраницыПодразделенияКонтейнер Цикл
		НовыеДанные = РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтаФорма, Стр.Данные[0].Значение, Ложь);
		ЗаполнитьЗначенияСвойств(ДопСтраницы.Добавить(), НовыеДанные);
	КонецЦикла;
	Контейнер.Вставить("ТаблицаСтраницыПодразделения", ДопСтраницы);
...
Процедура ПеренумероватьСтроки(ТабДок)
	Для Инд = 1 По 4 Цикл
		ТабДок.Области["П000102000114_" + Инд].Значение = "" + (14+Инд);
	КонецЦикла;
	
	Инд = 5;
	Пока Истина Цикл 
		Обл = ТабДок.Области.Найти("П000102000114_" + Инд);
		Если Обл = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		Обл.Значение = "18-"+(Инд-4);
		Инд = Инд + 1;
	КонецЦикла;
КонецПроцедуры
