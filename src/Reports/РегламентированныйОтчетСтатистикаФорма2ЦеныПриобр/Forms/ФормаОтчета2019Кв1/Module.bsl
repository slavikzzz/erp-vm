...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		КолонкаПоиска = "Код";
		Если Лев(Область.Имя, 13) = "П000001000103" Тогда 
			КодОКПД = ПолеТабличногоДокументаФормаОтчета.Область(СтрЗаменить(Область.Имя, "П000001000103", "П000001000102")).Значение;
		...
		ИначеЕсли Лев(Область.Имя, 13) = "П000001000104" Тогда 
			КодОКПД = ПолеТабличногоДокументаФормаОтчета.Область(СтрЗаменить(Область.Имя, "П000001000104", "П000001000102")).Значение;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000102",  15, , "Выбор товара", КоллекцияСписковВыбора["СправочникТоваров"]);
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если СтрНайти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы1) > 0 Тогда
		...
		//	ВыборКод = Строка(РезультатВыбора["Код"]);
		//	ВыборНазвание = Строка(РезультатВыбора["Название"]);
		//	ПолеТабличногоДокументаФормаОтчета.Области[Область.Имя].Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборНазвание, "");
		Иначе
			ВыборКод = Строка(РезультатВыбора["Код"]);
