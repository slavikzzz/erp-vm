...
Процедура СохранитьНаКлиенте(Автосохранение = Ложь, ВыполняемоеОповещение = Неопределено) Экспорт
	...
	#КонецЕсли
	ПоказателиОтчета.ПолеТабличногоДокументаФормаОтчета.ОргАдрес = ПолеТабличногоДокументаФормаОтчета.Области.ОргАдрес.Значение;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.Имя = "ОргАдрес" Тогда
		РегламентированнаяОтчетностьКлиент.ОбработкаАдресаВСтатистическомОтчетеJSON(ЭтаФорма, Область, СтандартнаяОбработка);
	...
	Иначе
		...
		КолонкаПоиска = "Код";
		Если Лев(Область.Имя, 13) = "П000001000102" Тогда 
			КодОКПД = ПолеТабличногоДокументаФормаОтчета.Область(СтрЗаменить(Область.Имя, "П000001000102", "П000001000103")).Значение;
			Для Каждого Стр Из РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "s_vidtov").ТаблицаЗначений Цикл 
				Если Не ЗначениеЗаполнено(КодОКПД) Или КодОКПД = Стр.Название Тогда 
		...
		ИначеЕсли Лев(Область.Имя, 13) = "П000001000104" Тогда 
			КодОКПД = ПолеТабличногоДокументаФормаОтчета.Область(СтрЗаменить(Область.Имя, "П000001000104", "П000001000103")).Значение;
			Для Каждого Стр Из РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "s_okei").ТаблицаЗначений Цикл 
				Если Не ЗначениеЗаполнено(КодОКПД) Или КодОКПД = Стр.Название Тогда 
					НСтр = ТЗВыбора.Добавить();
					НСтр.Код = Стр.Код;
					НСтр.Название = Стр.РезультатПроверки;
			...
			КонецЦикла;
			КодПоказателя = Область.Значение;
		ИначеЕсли ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			КодПоказателя = ПараметрВводаПоля.Показатель;
			КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		КонецЕсли;
		
		Если ТЗВыбора.Количество() > 0 Тогда 
			ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ?(ПараметрВводаПоля <> Неопределено, ПараметрВводаПоля.ТекстПриВыборе, "Выбор кода"), ТЗВыбора, СтандартнаяОбработка, КолонкаПоиска);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 2-цены приобретения");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "2ЦеныПриобретения");
	...
	Для Каждого МПФ Из СтруктураРеквизитовФормы.мПечатныеФормы Цикл 
		Для Каждого Обл Из МПФ.Значение.Области Цикл 
			Если СтрНачинаетсяС(Обл.Имя, "П000001000103_") Тогда 
...
Процедура СформироватьСпискиВыбора()
	...
	Для Каждого Область Из МакетСоставаПоказателей.Области Цикл
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
			...
			ТаблСписка.Колонки.Добавить("РезультатПроверки",,, МакетСоставаПоказателей.Область(ВерхОбласти, 3, ВерхОбласти, 3).ШиринаКолонки);
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000101", 250, , "Выбор товара", КоллекцияСписковВыбора["СправочникТоваров"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000103",  15, , "Выбор товара", КоллекцияСписковВыбора["СправочникТоваров"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000104",   5, , "Выбор единицы измерения", КоллекцияСписковВыбора["СправочникЕдиницИзмерения"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000109",  10, , "Выбор причины изменения цены", КоллекцияСписковВыбора["СправочникПричинИзмененияЦены"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_vidtov",  10, , "Выбор типа товара", КоллекцияСписковВыбора["s_vidtov"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_okei",  10, , "Выбор типа товара", КоллекцияСписковВыбора["s_okei"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000106",  10, , "Выбор причины изменения цены", КоллекцияСписковВыбора["s_tiptov"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000109",  10, , "Выбор причины изменения цены", КоллекцияСписковВыбора["СправочникПричинИзмененияЦены"]);
КонецПроцедуры
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если СтрНайти(Область.Имя, "П000001000101") > 0 Тогда
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(КолонкаПоиска, ПолеТабличногоДокументаФормаОтчета.Области["П000001000102" + Сред(Область.Имя, 14)].Значение));	
	ИначеЕсли СтрНайти(Область.Имя, "П000001000102") > 0 Или СтрНайти(Область.Имя, "П000001000106") > 0 Тогда
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(КолонкаПоиска, ИсходноеЗначение));	
	Иначе
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура("Название", ИсходноеЗначение));	
	КонецЕсли;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если СтрНайти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы1) > 0 Тогда
		Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "03" Тогда
			...
			ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "03" + Суффикс].Значение = ВыборКод;
			ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "02" + Суффикс].Значение = "";
			ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "01" + Суффикс].Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборНазвание, "");
		ИначеЕсли Сред(Область.Имя, 12, 2) = "02" Тогда
			КодПоказателя = СтруктураРеквизитовФормы.мИдГруппы1 + "02";
			ВыборКод = Строка(РезультатВыбора["Код"]);
			ВыборНазвание = Строка(РезультатВыбора["Название"]);
			Суффикс = СтрЗаменить(Область.Имя, КодПоказателя, "");
			ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "02" + Суффикс].Значение = ВыборКод;
			ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "01" + Суффикс].Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборНазвание, "");
		ИначеЕсли Сред(Область.Имя, 12, 2) = "04" Тогда
			ВыборКод = Строка(РезультатВыбора["Код"]);
			КодПоказателя = СтруктураРеквизитовФормы.мИдГруппы1 + "04";
			Суффикс = СтрЗаменить(Область.Имя, КодПоказателя, "");
			ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "04" + Суффикс].Значение = ВыборКод;
			ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "05" + Суффикс].Значение = ?(ЗначениеЗаполнено(ВыборКод), Строка(РезультатВыбора["Название"]), "");
		Иначе
			ВыборКод = Строка(РезультатВыбора["Код"]);
...
Функция СформироватьСтруктуруПоказателей()
	...
	ДополнительныеПоказатели2ЦеныПриобретения(мДанныеОтчета.ПолеТабличногоДокументаФормаОтчета, "П000001000103", "Название");
	ДополнительныеПоказатели2ЦеныПриобретения(мДанныеОтчета.ПолеТабличногоДокументаФормаОтчета, "П000001000106", "Код");
	
КонецФункции

&НаСервере
Процедура ДополнительныеПоказатели2ЦеныПриобретения(ТекСтруктура, ТекПоказатель, ТекКолонкаПоиска)
	...
	Пока ТекСтруктура.Свойство(ТекПоказатель + "_" + Формат(НомСуффикса, "ЧГ="), ТекЗначение) Цикл
		Попытка
			ТекСтрокаТЗ = СвойстваПоказателей.НайтиСтроки(Новый Структура("Показатель", ТекПоказатель))[0].ТаблицаЗначений.НайтиСтроки(Новый Структура(ТекКолонкаПоиска, ТекЗначение))[0];
		Исключение
		...
		Если ТекСтрокаТЗ <> Неопределено Тогда
			...
			Иначе
				Если ЗначениеЗаполнено(ТекСтруктура["П000001000102_" +  Формат(НомСуффикса, "ЧГ=")])
				 Или ЗначениеЗаполнено(ТекСтруктура["П000001000104_" +  Формат(НомСуффикса, "ЧГ=")])
				 Или ЗначениеЗаполнено(ТекСтруктура["П000001000105_" +  Формат(НомСуффикса, "ЧГ=")]) Тогда
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Пока Истина Цикл 
		...
		Если Не РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000001000104_" + НомСтр) Тогда 
			Прервать;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТабДокумент["П000001000105_" + НомСтр])
			Или ЗначениеЗаполнено(ТабДокумент["П000001000107_" + НомСтр]) Тогда 
			
			ПараметрыВыгрузки.Вставить("ПД00001000105_" + НомСтр, ТабДокумент["П000001000105_" + НомСтр]);
			ПараметрыВыгрузки.Вставить("ПД00001000199_" + НомСтр, 7);
			ПараметрыВыгрузки.Вставить("ПД00001000107_" + НомСтр, ТабДокумент["П000001000107_" + НомСтр]);
		ИначеЕсли ЗначениеЗаполнено(ТабДокумент["П000001000106_" + НомСтр])
			...
			ПараметрыВыгрузки.Вставить("ПД00001000105_" + НомСтр, ТабДокумент["П000001000106_" + НомСтр]);
			ПараметрыВыгрузки.Вставить("ПД00001000199_" + НомСтр, 8);
			ПараметрыВыгрузки.Вставить("ПД00001000107_" + НомСтр, ТабДокумент["П000001000108_" + НомСтр]);
		Иначе
			ПараметрыВыгрузки.Вставить("ПД00001000105_" + НомСтр, 0);
			ПараметрыВыгрузки.Вставить("ПД00001000199_" + НомСтр, 0);
			ПараметрыВыгрузки.Вставить("ПД00001000107_" + НомСтр, 0);
		КонецЕсли;
