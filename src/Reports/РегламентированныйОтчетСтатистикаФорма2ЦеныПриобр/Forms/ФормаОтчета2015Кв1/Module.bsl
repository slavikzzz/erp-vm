...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "19/12/2013";
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000101", 250, , "Выбор товара",
		КоллекцияСписковВыбора["СправочникТоваров"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000102",  15, , "Выбор товара",
		КоллекцияСписковВыбора["СправочникТоваров"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000103",   5, , "Выбор единицы измерения",
		КоллекцияСписковВыбора["СправочникЕдиницИзмерения"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000106",  10, , "Выбор причины изменения цены",
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// КОНТРОЛЬ показателей формы (Указания ФСН по заполнению Формы 2-цены приобретения).
	// Если значение в графе 1 <> 0, то значение в графе 2 <> 0, и наоборот.
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, "П000001000104_" + Формат(НомСтр, "ЧГ=")) Цикл
		Если (ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000104_" + Формат(НомСтр, "ЧГ=")]) И Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000105_" + Формат(НомСтр, "ЧГ=")]))
		 Или (ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000105_" + Формат(НомСтр, "ЧГ=")]) И Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000104_" + Формат(НомСтр, "ЧГ=")])) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если значение в графе 1 не равно 0, то значение в графе 2 не равно 0, и наоборот!",
				ИмяПоляТаблДокумента, ?(ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000104_" + Формат(НомСтр, "ЧГ=")]), "П000001000105_", "П000001000104_") + Формат(НомСтр, "ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
		НомСтр = НомСтр + 1;
	КонецЦикла;
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки)
	...
	Для Каждого Стр Из Узел_section.Строки Цикл 
		...
		Для Каждого Строка Из Стр.Строки Цикл 
			Если Строка.Ключ = "ПД00001000106" Тогда 
				ЕстьПричина = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Функция СформироватьСтруктуруПараметров()
	...
	// Для ПОСЛЕДУЮЩЕГО (ПЕРЕД ВЫГРУЗКОЙ) контроля показателей формы (Указания ФСН по заполнению Формы 2-цены приобретения).
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000001000104_" + Формат(НомСтр, "ЧГ=")) Цикл
