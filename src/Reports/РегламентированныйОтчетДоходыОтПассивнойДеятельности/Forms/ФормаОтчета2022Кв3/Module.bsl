...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.дох.от пассивн.деят.",
		'20221015') Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	//Раздел 1.
	//
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010040003", 1, ,
	"Выбор признака", КоллекцияСписковВыбора["ПризнакиДаНет"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010050003", 1, ,
	"Выбор признака", КоллекцияСписковВыбора["ПризнакиДаНет"]);
	...
	Если ИмяРаздела = "Раздел1" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				ТДО["П000010030003"].Значение = ?(ТДО["П000010010003"].Значение = 0,
				0, Окр(ТДО["П000010020003"].Значение / ТДО["П000010010003"].Значение * 100));
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел1" Тогда
		...
		Ветка.Данные.Вставить("П000010030003", ?(Ветка.Данные["П000010010003"] = 0,
		0, Окр(Ветка.Данные["П000010020003"] / Ветка.Данные["П000010010003"] * 100)));
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РасчДохПассивКурил");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "UT_DOHPASKURIL"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ПоМесту) Тогда
		СообщитьВТС("Не задан код места нахождения (учета) расчета", "Титульный", "ПоМесту");
		СтатусПроверки = Ложь;
	ИначеЕсли ВозможныеКодыПоМесту.МассивЗначений.Найти(ПараметрыВыгрузки.ПоМесту) = Неопределено Тогда
		СообщитьВТС("Код места нахождения (учета) расчета должен принимать одно из значений: "
	...
	ВозможныеПризнакиДаНет = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000010040003");
	ЗначПок = СокрЛП(Данные["П000010040003"]);
	Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
		СообщитьВТС("Не заполнена строка 400 расчета", "Раздел1", "П000010040003", НомСтраницы);
		СтатусПроверки = Ложь;
	ИначеЕсли ВозможныеПризнакиДаНет.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
		СообщитьВТС("Строка 400 расчета должна принимать одно из значений: "
		+ ВозможныеПризнакиДаНет.СтрокаЗначений, "Раздел1", "П000010040003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	ВозможныеПризнакиДаНет = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000010050003");
	ЗначПок = СокрЛП(Данные["П000010050003"]);
	Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
		СообщитьВТС("Не заполнена строка 500 расчета", "Раздел1", "П000010050003", НомСтраницы);
		СтатусПроверки = Ложь;
	ИначеЕсли ВозможныеПризнакиДаНет.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
		СообщитьВТС("Строка 500 расчета должна принимать одно из значений: "
		+ ВозможныеПризнакиДаНет.СтрокаЗначений, "Раздел1", "П000010050003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция ВыгрузкаОтчета(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "ДоходыОтПассивнойДеятельности");
