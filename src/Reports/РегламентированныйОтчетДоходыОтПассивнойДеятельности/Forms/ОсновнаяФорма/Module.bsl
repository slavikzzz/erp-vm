...
Функция ПолучитьСрокПредставленияОтчетности(Форма)
	...
	"ДФ=""дд ММММ гггг 'года'""") + " (п.5 ст.246.30 НК РФ).'");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(мДатаНачалаПериодаОтчета) И НЕ ЗначениеЗаполнено(мДатаКонцаПериодаОтчета) Тогда
		...
		мДатаКонцаПериодаОтчета  = ?(Год(мДатаКонцаПериодаОтчета) < 2022, КонецГода('20221231'), мДатаКонцаПериодаОтчета);
...
Процедура ВыбратьФорму(Команда)
	...
	"ru='Расчет доли доходов от пассивной деятельности в сумме всех доходов, учитываемых при определении организацией, "
	+ "зарегистрированной на территории Южно-Курильского, Курильского или Северо-Курильского городского округа, "
	+ "налоговой базы по налогу на прибыль организаций, представляют организации.'");
...
Функция ДоступныеОрганизацииОтсутствуютТекст()
	...
	"ru='Расчет доли доходов от пассивной деятельности в сумме всех доходов, учитываемых при определении организацией, "
	+ "зарегистрированной на территории Южно-Курильского, Курильского или Северо-Курильского городского округа, "
	+ "налоговой базы по налогу на прибыль организаций, представляют только организации.
...
Процедура ОтобразитьПоясненияКПериодуОтчета(Форма)
	...
	Если Форма.ГраницыПервогоНалоговогоПериода <> Неопределено Тогда
		...
		Если КонецГода(Форма.мДатаКонцаПериодаОтчета) < Форма.ГраницыПервогоНалоговогоПериода.Конец
			...
			"ru = 'Расчет доли доходов от пассивной деятельности за %1 сдавать не нужно. Период с даты регистрации %2 по %3 включается в расчет за налоговый период %4.';
			|en = 'Расчет доли доходов от пассивной деятельности за %1 сдавать не нужно. Период с даты регистрации %2 по %3 включается в расчет за налоговый период %4.'");
		...
		ИначеЕсли КонецГода(Форма.мДатаКонцаПериодаОтчета) = Форма.ГраницыПервогоНалоговогоПериода.Конец Тогда
			...
			ШаблонСообщения = НСтр("ru = 'Период с даты регистрации %1 по %2 включается в расчет доли доходов от пассивной деятельности за %3.';
									|en = 'Период с даты регистрации %1 по %2 включается в расчет доли доходов от пассивной деятельности за %3.'");
