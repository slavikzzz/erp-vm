...
Функция ОтборыИНастройкиЗаполненыНеправильно()
	...
	Если (ПериодФормированияОтчета.ДатаНачала = Дата(1,1,1) Или  ПериодФормированияОтчета.ДатаОкончания = Дата(1,1,1))
		...
		ТекстСообщения = НСтр("ru = 'В случае, когда используется дополнительный период для анализа, у основного периода должны быть указаны дата начала и дата конца.';
								|en = 'When an extra period is used for analysis, specify end date and start date of the basic period.'");
	...
	Если НЕ ВыводитьДиаграммы И НЕ ВыводитьТаблицы Тогда
		...
		ТекстСообщения = НСтр("ru = 'В настройках формирования отчета, на закладке ""Прочее"", необходимо выбрать хотя бы один из следующих режимов вывода - таблицы или диаграммы.';
								|en = 'Select at least one of the following output modes - tables or charts - in the report generation settings on the ""Other"" tab.'");
	...
	Если ПроцентнаяСтавка = 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'В настройках формирования отчета, на закладке ""Прочее"", необходимо указать процентную ставку для расчета финансовых потерь.';
								|en = 'Select an interest rate to calculate financial losses in the report generation settings on tab ""Other"".'");
...
Процедура ЗаполнитьДеревоНаборовДанных()
	...
	ДобавленнаяСтрока = НовыйЭлементДерева(ДеревоНастроек,"ПродажиПрибыльВыручка",НСтр("ru = 'Продажи, прибыль, выручка';
																						|en = 'Sales, profit, revenue'"),Истина);
	НовыйЭлементДерева(ДобавленнаяСтрока,"Продажи",НСтр("ru = 'Продажи';
														|en = 'Sales'"),Истина);
	НовыйЭлементДерева(ДобавленнаяСтрока,"Прибыль",НСтр("ru = 'Прибыль';
														|en = 'Profit'"),Истина);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ПоступлениеДС",НСтр("ru = 'Поступление денежных средств';
																|en = 'Incoming payment'"),Истина);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ПоступлениеДС_МинусСебестоимость",НСтр("ru = 'Выручка (по оплате) минус себестоимость (по отгрузке)';
																				|en = 'Revenue (by payment) minus cost (by shipment)'"),Истина);
	
	ДобавленнаяСтрока = НовыйЭлементДерева(ДеревоНастроек,"СоблюдениеУсловийПродаж",НСтр("ru = 'Соблюдение условий продаж';
																						|en = 'Compliance with sales terms and conditions'"),Истина);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ПотериПросроченнаяДебиторка",НСтр("ru = 'Потери в результате превышения сроков погашения дебиторской задолженности';
																			|en = 'Losses due to late receivables repayment'"),Истина);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ПредоставленныеРучныеСкидки",НСтр("ru = 'Предоставленные ручные скидки';
																			|en = 'Discount adjustments provided'"),ИспользоватьРучныеСкидкиВПродажах);
	
	ДобавленнаяСтрока = НовыйЭлементДерева(ДеревоНастроек,"ЭффективностьРаботы",НСтр("ru = 'Эффективность работы';
																					|en = 'Work efficiency'"),ИспользоватьСделкиСКлиентами ИЛИ ИспользоватьВзаимодействия);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ЭффективностьВеденияСделок",НСтр("ru = 'Эффективность ведения сделок';
																			|en = 'Efficiency of sales opportunity management'"),ИспользоватьСделкиСКлиентами);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ЭффективностьВзаимодействий",НСтр("ru = 'Эффективность взаимодействий';
																			|en = 'Interaction efficiency'"),ИспользоватьВзаимодействия И ИспользоватьСделкиСКлиентами);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ПродажиНаОднуСделку",НСтр("ru = 'Продажи на одну сделку';
																	|en = 'Sales per sales opportunity'"),ИспользоватьСделкиСКлиентами);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ПродажиНаОдноВзаимодействие",НСтр("ru = 'Продажи на одно взаимодействие';
																			|en = 'Sales per one interaction'"),ИспользоватьВзаимодействия);
	
	ДобавленнаяСтрока = НовыйЭлементДерева(ДеревоНастроек,"ПричиныПотерь",НСтр("ru = 'Причины потерь';
																				|en = 'Reasons for losses'"),ИспользоватьСделкиСКлиентами ИЛИ ФиксироватьПервичныйИнтерес);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ОсновныеЭтапыПотерь",НСтр("ru = 'Основные этапы потерь';
																	|en = 'Main stages of losses'"),ИспользоватьСделкиСКлиентами);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ПричиныПроигрышаСделок",НСтр("ru = 'Причины проигрыша сделок';
																		|en = 'Reasons for losing sales opportunities'"),ИспользоватьСделкиСКлиентами);
	НовыйЭлементДерева(ДобавленнаяСтрока,"НеудовлетворениеПервичногоСпроса",НСтр("ru = 'Причины неудовлетворения первичного спроса';
																				|en = 'Reasons for primary demand non-fulfillment'"),ФиксироватьПервичныйИнтерес);
	
	ДобавленнаяСтрока = НовыйЭлементДерева(ДеревоНастроек,"ПривлечениеУдержаниеКлиентов",НСтр("ru = 'Привлечение и удержание клиентов';
																								|en = 'Attracting and retaining customers'"),ИспользуетсяКлассификацияПартнеров);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ДинамикаКлиентскойБазы",НСтр("ru = 'Динамика клиентской базы';
																		|en = 'Customer base trends'"),ИспользуетсяКлассификацияПартнеров);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ПродажиНовымКлиентам",НСтр("ru = 'Продажи новым клиентам';
																	|en = 'Sales to new customers'"),ИспользуетсяКлассификацияПартнеров);
	
	ДобавленнаяСтрока = НовыйЭлементДерева(ДеревоНастроек,"ПоддержкаЛояльностиКлиентов",НСтр("ru = 'Поддержка лояльности клиентов';
																							|en = 'Customer loyalty support'"),ФиксироватьПретензииКлиентов);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ВозникшиеПретензииКлиентов",НСтр("ru = 'Возникшие претензии клиентов';
																			|en = 'New customer complaints'"),ФиксироватьПретензииКлиентов);
	НовыйЭлементДерева(ДобавленнаяСтрока,"ПричиныВозникновенияПретензий",НСтр("ru = 'Причины возникновения претензий клиентов';
																				|en = 'Reasons for customer complaints'"),ФиксироватьПретензииКлиентов);
