...
Функция ПолучитьСрокПредставленияОтчетности(Форма)
	...
	= СтрСрокПредставления + Формат(ДатаСрокаПредставления, "ДФ=""дд ММММ гггг 'года'""") + " (п.2 ст.386 НК РФ)";
...
Процедура ПоказатьПериод(Форма)
	...
	Иначе
		...
		НСтр("ru = 'Утратила силу (Федеральный закон от 15.04.2019 N 63-ФЗ).';
			|en = 'Утратила силу (Федеральный закон от 15.04.2019 N 63-ФЗ).'"), НСтр("ru = 'Отсутствует в программе.';
	...
	Форма.Элементы.ГруппаКтоСдаетОтчет.Видимость = ?(ФормаУтратилаСилу, Ложь, Истина);
	
	РегламентированнаяОтчетностьКлиентСервер.ВыборФормыРегламентированногоОтчетаПоУмолчанию(Форма);
	
	ИндексБС_4_11_5197 = Неопределено;
	Для каждого ЭлФорма Из Форма.РезультирующаяТаблица Цикл
		Если ЭлФорма.ФормаОтчета = "ФормаОтчета2013Кв4" И СтрЗаканчиваетсяНа(ЭлФорма.РедакцияФормы, "БС-4-11/5197@.") Тогда
			ИндексБС_4_11_5197 = Форма.РезультирующаяТаблица.Индекс(ЭлФорма);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Если ИндексБС_4_11_5197 <> Неопределено Тогда
		Для каждого ЭлФорма Из Форма.РезультирующаяТаблица Цикл
			Если ЭлФорма.ФормаОтчета = "ФормаОтчета2013Кв4" И СтрЗаканчиваетсяНа(ЭлФорма.РедакцияФормы, "ММВ-7-11/478@.") Тогда
				Форма.РезультирующаяТаблица.Сдвинуть(ИндексБС_4_11_5197, - 1);
				Если ИндексБС_4_11_5197 = 1 Тогда
...
Процедура ИзменитьПериод(Форма, Шаг)
	...
	Если Число(формат(Форма.мДатаКонцаПериодаОтчета, "ДФ=к"))
		- Число(формат(Форма.мДатаначалаПериодаОтчета, "ДФ=к")) = 3 Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
		...
		МодульОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
			ЭтаФорма,
			"БП.Отчет.РегламентированныйОтчетАвансыИмущество",
			"ОсновнаяФорма",
			,
			НСтр("ru = 'Новости: Расчет по авансовому платежу по налогу на имущество организаций';
				|en = 'Новости: Расчет по авансовому платежу по налогу на имущество организаций'"),
			Ложь,
...
Процедура ОткрытьФормуОтчета(Команда)
	...
	ПараметрыФормы.Вставить("ФормаВРедакцииПисьмаБС_4_11_5197",
	?(СтрНайти(ПолеРедакцияФормы, "БС-4-11/5197@") > 0, Истина, Ложь));
...
Процедура ВыбратьФорму(Команда)
	...
	"ru = 'Налоговый расчет по авансовым платежам по налогу на имущество представляют организации (п.1 ст.373 НК РФ).';
	|en = 'Налоговый расчет по авансовым платежам по налогу на имущество представляют организации (п.1 ст.373 НК РФ).'");
	
	Оп = Новый ОписаниеОповещения("ВыбратьФормуЗавершение", ЭтотОбъект);
	РегламентированнаяОтчетностьКлиент.ВыбратьФормуОтчетаИзДействующегоСписка(ЭтаФорма, Оп, ДопТекстОписания);
...
Функция ДоступныеОрганизацииОтсутствуютТекст()
	...
	|en = 'Налоговый расчет по авансовым платежам по налогу на имущество представляют только организации (п.1 ст.373 НК РФ).
	|В справочнике ""Организации"" сведения об организациях отсутствуют.'");
