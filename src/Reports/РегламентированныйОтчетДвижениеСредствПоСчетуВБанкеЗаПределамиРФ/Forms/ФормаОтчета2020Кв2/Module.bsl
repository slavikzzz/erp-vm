...
Процедура ИнициализироватьМногоуровневыеРазделы()
	...
	мСтруктураМногоуровневыхРазделов.Вставить("Лист3", СтруктураРаздела);
...
Процедура ФормироватьСтруктуруСтраницОтчета() Экспорт
	...
	СтрокаУровня1 = мДеревоСтраницОтчета.ПолучитьЭлементы().Добавить();
	СтрокаУровня1.ИмяСтраницы        = "Лист3";
...
Процедура ЗаполнитьПериодОтчета()
	...
	ДеревоРаздела.Строки[0].Данные.Вставить("П000Л20000001", НачалоДня(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета));
	ДеревоРаздела.Строки[0].Данные.Вставить("П000Л20000002", НачалоДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "П000Л30000001" Тогда
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Титульный" Тогда
		...
		Секция_ИдФайлКорр = Форма.ТабличныйДокумент.Области.Секция_ИдФайлКорр;
		ОблИдФайлКорр = Форма.ТабличныйДокумент.Области.ИдФайлКорр;
		Если Форма.ТабличныйДокумент.Области.НомерКорректировки.Значение > 0 Тогда
		...
		ОблНаимФил = Форма.ТабличныйДокумент.Области.НаимФил;
		ОблАдрФил = Форма.ТабличныйДокумент.Области.АдрФил;
		ОблКодСтраны = Форма.ТабличныйДокумент.Области.КодСтраны;
		...
		ОблПодписантИНН = Форма.ТабличныйДокумент.Области.ПодписантИНН;
		Если СокрЛП(Форма.ТабличныйДокумент.Области.ПрПодп.Значение) = "3" Тогда
			ЗапретитьВводДанныхВОбласть(Форма, ОблПодписантИНН);
		Иначе
			РазрешитьВводДанныхВОбласть(Форма, ОблПодписантИНН);
		КонецЕсли;
		
	ИначеЕсли ИмяРаздела = "Лист3" Тогда
...
Процедура УдалитьДопСтрокуНаСервере()
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Лист3" Тогда
		
		Расчет(ЭтаФорма, СтруктураРеквизитовФормы.НаимТекущегоРаздела, "П000Л30000003");
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Пока Обл <> Неопределено Цикл
					...
					Обл = ТДО.Найти("П000Л5М100002_" + ИндексСтр);
				КонецЦикла;
				ТДО["П000Л30000005"].Значение = Сумма;
				
				ТДО["П000Л30000003"].Значение
				= ТДО["П000Л30000002"].Значение + ТДО["П000Л30000004"].Значение - ТДО["П000Л30000005"].Значение;
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Лист3" Тогда
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			КонецЦикла;
			Ветка.Данные.Вставить("П000Л30000005", Сумма);
			
			Ветка.Данные.Вставить("П000Л30000003",
			Ветка.Данные["П000Л30000002"] + Ветка.Данные["П000Л30000004"] - Ветка.Данные["П000Л30000005"]);
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Лист3", Структ);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок.Вид > 0 И НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ИдФайлКорр) Тогда
		СообщитьВТС("Не задан идентификатор корректируемого файла (имя корректируемого (отменяемого) первичного файла (без расширения))",
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ПараметрыВыгрузки.НаимФил)
			...
			Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.НаимФил) Тогда
				СообщитьВТС("Не задано наименование филиала, представительства или иного подразделения юридического лица - резидента, расположенного за пределами территории Российской Федерации",
				"Титульный", "НаимФил");
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.АдрФил) Тогда
				СообщитьВТС("Не задано место нахождения филиала, представительства или иного подразделения юридического лица - резидента, расположенного за пределами территории Российской Федерации",
				"Титульный", "АдрФил");
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.КодСтраны) Тогда
				СообщитьВТС("Не задан код страны филиала, представительства или иного подразделения юридического лица - резидента, расположенного за пределами территории Российской Федерации",
				"Титульный", "КодСтраны");
				СтатусПроверки = Ложь;
			ИначеЕсли СтрДлина(ПараметрыВыгрузки.КодСтраны) <> 3 Тогда
				СообщитьВТС("Задан код страны филиала, представительства или иного подразделения юридического лица - резидента, расположенного за пределами территории Российской Федерации, не состоящий из 3 цифр",
				"Титульный", "КодСтраны");
				СтатусПроверки = Ложь;
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрыВыгрузки.ПодписантИНН) Тогда
		Если ПараметрыВыгрузки.ПрПодп = "3" Тогда
			СообщитьВТС("Не должен быть заполнен ИНН подписанта, если признак подписанта равен ""3""", "Титульный", "ПодписантИНН");
			СтатусПроверки = Ложь;
	...
	Если ЗаполненРаздел(ДеревоРаздела, "Лист2") Тогда
		...
		ИначеЕсли ВозмКоды.МассивЗначений.Найти(ПрОрг) = Неопределено Тогда
			СообщитьВТС("Сведения о счете: признак организации должен принимать одно из значений: " + ВозмКоды.СтрокаЗначений,
			"Лист2", "П000Л20000003", НомСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		ИначеЕсли НЕ СтроковыеФункцииКлиентСервер.ТолькоЛатиницаВСтроке(
			...
			СообщитьВТС("Сведения о счете: наименование должно быть заполнено с использованием букв латинского алфавита, пробелов и символов 0123456789!""`@#№$%&'()[]<>{}*+~=_-.,:;/|\^?",
			"Лист2", "П000Л20000004", НомСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ЗначПок) ИЛИ ПрОрг = "1" Тогда
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("Сведения о счете: SWIFT код или БИК должен быть задан, если признак организации равен 1", "Лист2", "П000Л20000005", НомСтраницы);
				СтатусПроверки = Ложь;
			Иначе
				...
				Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЛатиницаВСтроке(ЗначПок, Истина, "0123456789") Тогда
					СообщитьВТС("Сведения о счете: SWIFT код или БИК должен быть заполнен только цифрами и буквами латинского алфавита",
					"Лист2", "П000Л20000005", НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
		...
		Иначе
			Пока ЗначПок <> "" Цикл
				Если Лев(ЗначПок, 1) <> " " И СтроковыеФункцииКлиентСерверРФ.ТолькоКириллицаВСтроке(Лев(ЗначПок, 1), Истина) Тогда
					СообщитьВТС("Сведения о счете: полный адрес должен быть заполнен без использования букв русского алфавита",
					"Лист2", "П000Л20000007", НомСтраницы);
					СтатусПроверки = Ложь;
					Прервать;
				Иначе
					ЗначПок = ?(СтрДлина(ЗначПок) > 1, Сред(ЗначПок, 2), "");
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ЗначПок) И НЕ ДатаВходитВРазрешенныйПериод(ЗначПок) Тогда
			СообщитьВТС("Сведения о счете: задана дата выдачи разрешения, выходящая за пределы периода с 01.01.1900 по 31.12.2099",
	...
	// Лист 3.
	//
	ДеревоРаздела = СохрСтруктура.ДанныеМногоуровневыхРазделов["Лист3"];
	
	Если ЗаполненРаздел(ДеревоРаздела, "Лист3") Тогда
		...
		Для каждого Ветка Из ДеревоРаздела.Строки Цикл
			...
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				...
				"Лист3", "П000Л30000001", НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли СтрДлина(ЗначПок) <> 3 Тогда
			...
			Если Ветка.Данные["П000Л30000004"] > 0 И НЕ ЗаполненаМногострочнаяЧасть(ДанныеМнЧ) Тогда
				...
				"Лист3", "П000Л4М100001_1", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
				Если ЗаполненаСтрокаМногострочнойЧасти(СтрокаМнЧ) Тогда
					...
					Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
						...
						+ " Листа 4 не задан код вида операции", "Лист3", "П000Л4М100001_" + НомСтроки, НомСтраницы);
						СтатусПроверки = Ложь;
					ИначеЕсли СтрДлина(ЗначПок) <> 5 Тогда
						СообщитьВТС("Движение средств -> Страница " + НомСтраницы + ": в строке " + НомСтроки
						+ " Листа 4 задан код вида операции, не состоящий из 5 символов", "Лист3", "П000Л4М100001_" + НомСтроки, НомСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					...
					Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
						СообщитьВТС("Движение средств -> Страница " + НомСтраницы + ": в строке " + НомСтроки
						+ " Листа 4 не задана сумма", "Лист3", "П000Л4М100002_" + НомСтроки, НомСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
			...
			Если Ветка.Данные["П000Л30000005"] > 0 И НЕ ЗаполненаМногострочнаяЧасть(ДанныеМнЧ) Тогда
				СообщитьВТС("Движение средств -> Страница " + НомСтраницы
				+ ": должен быть заполнен Лист 5 при заполненной строке ""Списано денежных средств за отчетный период - всего""",
				"Лист3", "П000Л5М100001_1", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
				Если ЗаполненаСтрокаМногострочнойЧасти(СтрокаМнЧ) Тогда
					...
					Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
						СообщитьВТС("Движение средств -> Страница " + НомСтраницы + ": в строке " + НомСтроки
						+ " Листа 5 не задан код вида операции", "Лист3", "П000Л5М100001_" + НомСтроки, НомСтраницы);
						СтатусПроверки = Ложь;
					ИначеЕсли СтрДлина(ЗначПок) <> 5 Тогда
						СообщитьВТС("Движение средств -> Страница " + НомСтраницы + ": в строке " + НомСтроки
						+ " Листа 5 задан код вида операции, не состоящий из 5 символов", "Лист3", "П000Л5М100001_" + НомСтроки, НомСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					...
					Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
						СообщитьВТС("Движение средств -> Страница " + НомСтраницы + ": в строке " + НомСтроки
						+ " Листа 5 не задана сумма", "Лист3", "П000Л5М100002_" + НомСтроки, НомСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
	...
	Иначе
		...
		СообщитьВТС("Не заполнены сведения о движении средств", "Лист3", "П000Л30000001", "1");
		СтатусПроверки = Ложь;
...
Процедура ВывестиНаПечатьДанныеМногоуровневогоРаздела(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	ИначеЕсли ИмяТекРаздела = "Лист3" Тогда
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	ИначеЕсли Ячейка.Раздел = "Лист3" Тогда
