...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0020010001";
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "06",
		512, , "Выбор признака образования", КоллекцияСписковВыбора["Образование"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "13",
		512, , "Выбор признака категори персонала", КоллекцияСписковВыбора["КатегорииПерсонала"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "01",
		512, , "Выбор профессии", КоллекцияСписковВыбора["Профессии"]);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли РегламентированнаяОтчетностьКлиент.ВыбраннаяОбластьРегламентированногоОтчетаНеВерна(ЭтаФорма, Область, СтандартнаяОбработка) Тогда
		Возврат;
	ИначеЕсли Лев(Область.Имя, 4) = "П002" И Сред(Область.Имя, 12, 2) = "04" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "04");
	ИначеЕсли Лев(Область.Имя, 4) = "П002" И Сред(Область.Имя, 12, 2) = "06" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "06");
	ИначеЕсли Лев(Область.Имя, 4) = "П002" И Сред(Область.Имя, 12, 2) = "13" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "13");
	ИначеЕсли Лев(Область.Имя, 4) = "П002" И (Сред(Область.Имя, 12, 2) = "02" Или Сред(Область.Имя, 12, 2) = "03") Тогда
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	СтандартнаяОбработка = Ложь;
	СмежнаяОбл = СтрЗаменить(Область.Имя, "П002" + Сред(Область.Имя, 5, 7) + "02", "П002" + Сред(Область.Имя, 5, 7) + "03");
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Лев(Область.Имя, 4) = "П002" И Сред(Область.Имя, 12, 2) = "03" Тогда
		ВыборНазв = ?(ЗначениеЗаполнено(ВыборКод),СокрЛП(РезультатВыбора["Название"]),"");
		СмежнаяОбл = СтрЗаменить(Область.Имя, "П002" + Сред(Область.Имя, 5, 7) + "03", "П002" + Сред(Область.Имя, 5, 7) + "02");
		ИсхОбласть = ПолеТабличногоДокументаФормаОтчета.Области[СмежнаяОбл];
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Пока Истина Цикл
		Стр = Стр + 1;
		СтрФормат = Формат(Стр, "ЧГ=");
		ОблГрА = Области.Найти("П002001000101_" + СтрФормат);
		Если ОблГрА = Неопределено Тогда
			Прервать;
		Иначе
			Если Стр < 10 Тогда
				ОблГрА.Значение = "0" + СтрФормат;
			Иначе
				ОблГрА.Значение = СтрФормат;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "") Экспорт
	...
	Пока Истина Цикл
		Стр = Стр + 1;
		СтрФормат = Формат(Стр, "ЧГ=");
		ОблГр8 = Области.Найти("П002001000108_" + СтрФормат);
		Если ОблГр8 = Неопределено Тогда
			Прервать;
		Иначе
			ОблГр8.Значение = Области.Найти("П002001000109_" + СтрФормат).Значение
							+ Области.Найти("П002001000110_" + СтрФормат).Значение
							+ Области.Найти("П002001000111_" + СтрФормат).Значение;
		КонецЕсли;
	КонецЦикла;
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	Если КолСтрок = 66 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Условия заполнения не позволяют добавить больше 66 строк таблицы.';
								|en = 'Условия заполнения не позволяют добавить больше 66 строк таблицы.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть2_4")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 57-Т");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если СтрКоллекцииПоказателей.Количество() = 1 Тогда
		...
		Для Каждого ЭлТабДока Из мДанныеОтчета.ПолеТабличногоДокументаФормаОтчета Цикл
			Если Лев(ЭлТабДока.Ключ, 4) = "П002" И Сред(ЭлТабДока.Ключ, 12, 2) = "03" Тогда
