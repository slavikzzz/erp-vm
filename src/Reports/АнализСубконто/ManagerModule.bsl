...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета) Экспорт 
	...
	Возврат СтрШаблон(
		НСтр("ru = 'Анализ субконто %1%2';
			|en = '%1%2 extra dimension analysis'"), 
		ТекстСубконто, 
...
Процедура ЗаполнитьПравилаРасшифровки(Правила) Экспорт
	...
	Правило.ШаблонПредставления = НСтр("ru = 'Карточка субконто [Субконто]';
										|en = '[Субконто] extra dimension card'");
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	МакетПодвалаОтчета = БухгалтерскиеОтчеты.ПолучитьМакетПодвала(МакетКомпоновки);
	
	Если ПараметрыОтчета.ПоказательВалютнаяСумма Или ПараметрыОтчета.ПоказательКоличество Тогда
		...
		МакетГруппировкиВалюта = БухгалтерскиеОтчеты.ПодобратьЭлементыИзТелаМакета(
									МакетКомпоновки, "Валюта", ПараметрыПоиска);
	...
	Для Каждого Макет Из МакетКомпоновки.Макеты Цикл 
		...
		Если ПараметрыОтчета.ПоказательВалютнаяСумма И КоличествоПоказателей <> 1 Тогда
			...
			Индекс = Индекс + 1;
			Если МакетГруппировкиВалюта.Найти(Макет) = Неопределено
			   И Макет <> МакетПодвалаОтчета
			   И Макет.Макет.Количество() > Индекс Тогда
