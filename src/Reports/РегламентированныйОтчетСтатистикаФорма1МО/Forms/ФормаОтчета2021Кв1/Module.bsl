...
Процедура ЗаполнитьСведенияОбОрганизацииДляОтчетаСтатистики(Форма, ВПрограммеИзмененаДатаПодписи = Ложь, ИмяПоля = "ПолеТабличногоДокументаФормаОтчета", СхемаНаимПолноеКраткое = Ложь) Экспорт
	...
	Попытка
		Если (НЕ ЗначениеЗаполнено(Форма[ИмяПоля].Области.ОргКодОКТМО.Значение))
			ИЛИ (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) Тогда
			Форма[ИмяПоля].Области.ОргКодОКТМО.Значение = ОКТМО;
		КонецЕсли;
	Исключение
	КонецПопытки;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "КодТипаОбразования" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "СправочникКодТипаМуниципальногоОбразования");
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 4 По 5 Цикл
		...
		ТекСумма = 0;
		Для Стр = 3 По 14 Цикл
			НомСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
			ТекСумма = ТекСумма + Области["П00001010" + НомСтр + Графа].Значение;
		КонецЦикла;
		Области["П0000101002" + Графа].Значение = ТекСумма;
	КонецЦикла;
	
	Для Гр = 4 По 5 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ТекСумма = 0;
		Для Стр = 16 По 24 Цикл
			ТекСумма = ТекСумма + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101015" + Графа].Значение = ТекСумма;
	КонецЦикла;
	
	Для Гр = 4 По 5 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		Области["П0000101025" + Графа].Значение
			= Области["П0000101027" + Графа].Значение
			+ Области["П0000101029" + Графа].Значение
			+ Области["П0000101031" + Графа].Значение
			+ Области["П0000101033" + Графа].Значение
			+ Области["П0000101035" + Графа].Значение
			+ Области["П0000101037" + Графа].Значение
			+ Области["П0000101039" + Графа].Значение;
	КонецЦикла;
		
	Для Гр = 4 По 5 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		Области["П0000101026" + Графа].Значение
			= Области["П0000101028" + Графа].Значение
			+ Области["П0000101030" + Графа].Значение
			+ Области["П0000101032" + Графа].Значение
			+ Области["П0000101034" + Графа].Значение
			+ Области["П0000101036" + Графа].Значение
			+ Области["П0000101038" + Графа].Значение
			+ Области["П0000101040" + Графа].Значение;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 4 по 5 Цикл
		...
		мФормаОтчета.Области["П00001010020" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00001010020" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
		...
		мФормаОтчета.Области["П00001010250" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00001010250" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П00001010260" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00001010260" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателей.Добавить(ВыбТабПоле.Области["ОргКодОКТМО"].Значение,               "ОргКодОКТМО");
	СписокПоказателей.Добавить(ВыбТабПоле.Области["КодТипаОбразования"].Значение,        "КодТипаОбразования");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникКодТипаМуниципальногоОбразования",
	512, , "Выбор кода типа муниципального образования", КоллекцияСписковВыбора["КодТипаМуниципальногоОбразования"]);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКТМО) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			"Не заполнено ОКТМО организации!", ИмяПоляТаблДокумента, "ОргКодОКТМО");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.КодПоселения) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			"Не указан статус муниципального образования!", ИмяПоляТаблДокумента, "КодТипаОбразования");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("ОКТМО", СокрЛП(ТабДокумент.ОргКодОКТМО));
	...
	ПараметрыВыгрузки.Вставить("КодПоселения", СокрЛП(ТабДокумент.КодТипаОбразования));
