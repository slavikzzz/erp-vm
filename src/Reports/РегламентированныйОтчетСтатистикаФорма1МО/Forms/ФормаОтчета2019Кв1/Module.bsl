...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "05/07/2019";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 8) = "СтатусМО" Тогда
		...
		Если Область.Значение Тогда
			...
			мФормаОтчета.Области["СтатусМО14"].Значение = ?(мФормаОтчета.Области["СтатусМО14"].Имя = Область.Имя, Истина, Ложь);
			мФормаОтчета.Области["СтатусМО15"].Значение = ?(мФормаОтчета.Области["СтатусМО15"].Имя = Область.Имя, Истина, Ложь);
			мФормаОтчета.Области["СтатусМО16"].Значение = ?(мФормаОтчета.Области["СтатусМО16"].Имя = Область.Имя, Истина, Ложь);
			мФормаОтчета.Области["СтатусМО17"].Значение = ?(мФормаОтчета.Области["СтатусМО17"].Имя = Область.Имя, Истина, Ложь);
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 4 По 5 Цикл 
		...
		Для НомСтр = 1 По 12 Цикл
			ТекСумма = ТекСумма + Области["П02" + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + "000" + Гр].Значение;
		КонецЦикла;
		Области["П0200000" + Гр].Значение = ТекСумма;
	КонецЦикла;
	
	Для Гр = 4 По 5 Цикл 
		ТекСумма = 0;
		Для НомСтр = 1 По 9 Цикл
			ТекСумма = ТекСумма + Области["П03" + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + "000" + Гр].Значение;
		КонецЦикла;
		Области["П0300000" + Гр].Значение = ТекСумма;
	КонецЦикла;
	
	Для Гр = 4 По 5 Цикл 
		ТекСумма = 0;
		Для НомСтр = 2 По 8 Цикл
			ТекСумма = ТекСумма + Области["П04" + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + "000" + Гр].Значение;
		КонецЦикла;
		Области["П0401000" + Гр].Значение = ТекСумма;
	КонецЦикла;
	
	Для Гр = 4 По 5 Цикл 
		ТекСумма = 0;
		Для НомСтр = 2 По 8 Цикл
			ТекСумма = ТекСумма + Области["П04" + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + "010" + Гр].Значение;
		КонецЦикла;
		Области["П0401010" + Гр].Значение = ТекСумма;
	КонецЦикла;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКТМО) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не заполнено ОКТМО организации!", ИмяПоляТаблДокумента, "");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("ОКТМО", СокрЛП(СведенияОбОрганизации.ОКТМО));
	...
	Если ТабДокумент.СтатусМО13 = Истина Тогда 
		ПараметрыВыгрузки.Вставить("КодПоселения", "13");
	ИначеЕсли ТабДокумент.СтатусМО15 = Истина Тогда
		ПараметрыВыгрузки.Вставить("КодПоселения", "15");
	ИначеЕсли ТабДокумент.СтатусМО12 = Истина Тогда
		ПараметрыВыгрузки.Вставить("КодПоселения", "16");
	ИначеЕсли ТабДокумент.СтатусМО16 = Истина Тогда
		ПараметрыВыгрузки.Вставить("КодПоселения", "17");
	ИначеЕсли ТабДокумент.СтатусМО10 = Истина Тогда
		ПараметрыВыгрузки.Вставить("КодПоселения", "12");
	ИначеЕсли ТабДокумент.СтатусМО14 = Истина Тогда
		ПараметрыВыгрузки.Вставить("КодПоселения", "10");
	ИначеЕсли ТабДокумент.СтатусМО11 = Истина Тогда
		ПараметрыВыгрузки.Вставить("КодПоселения", "11");
	ИначеЕсли ТабДокумент.СтатусМО17 = Истина Тогда
		ПараметрыВыгрузки.Вставить("КодПоселения", "17");
	Иначе 
		ПараметрыВыгрузки.Вставить("КодПоселения", "");
	КонецЕсли;
