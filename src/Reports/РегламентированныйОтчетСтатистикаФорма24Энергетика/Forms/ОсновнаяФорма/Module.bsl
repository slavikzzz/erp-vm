...
Процедура ПоказатьПериод(Форма)
	...
	Иначе
		...
		Если НачалоДня(Форма.мДатаНачалаПериодаОтчета) >= '20140101' Тогда
			Форма.ОписаниеНормативДок = "Форма утратила силу в соответствии с приказом Росстата от 04.09.2014 № 547.";
		Иначе
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(мДатаНачалаПериодаОтчета) И НЕ ЗначениеЗаполнено(мДатаКонцаПериодаОтчета) Тогда
		...
		Если мДатаНачалаПериодаОтчета < Дата("20130101") Тогда 
			мДатаНачалаПериодаОтчета = Дата("20130101");
