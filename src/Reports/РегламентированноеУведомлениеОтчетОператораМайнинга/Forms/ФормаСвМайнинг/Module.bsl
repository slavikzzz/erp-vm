...
Процедура Проверить(Команда)
	...
	Если Не ЗначениеЗаполнено(Тариф) Тогда 
		ОбщегоНазначенияКлиент.СообщитьПользователю("Не указан тариф", , "Тариф");
		Возврат;
	КонецЕсли;
...
Функция ЗаполненоКорректно()
	Если (Тариф = 0) Или (СвОборудСправоч.Количество() + СвОборудПроизв.Количество() = 0)
		Или (Не ПрОтсВотчер И СвВотчер.Количество() = 0) Или (СвДобытВалют.Количество() = 0) Тогда 
		Возврат Ложь;
	КонецЕсли;
...
Функция СвМайнингУведомлениеМайнераВJSON()
	...
	ОтчетностьМайнеров.ЗаписатьПакетПоказателей(Новый Структура("ПрОтсВотчер, Тариф", ПрОтсВотчер, Тариф), ЗаписьJSON);
