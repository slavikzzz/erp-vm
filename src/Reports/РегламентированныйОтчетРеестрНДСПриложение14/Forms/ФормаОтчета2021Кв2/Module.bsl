...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр р.НДС прилож.14 21к2") Тогда
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	// Группа П12000.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П1200005");
	ИменаГраф.Добавить("П1200006");
	ИменаПодчиненныхГрупп = Новый Массив;
	СтруктураМногострочныхЧастей.Вставить(
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы110 Тогда
		Подстрока = "перевозочный документ";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный перевозочный документ?';
							|en = 'Удалить выбранный перевозочный документ?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы120 Тогда
		Подстрока = "маршрут";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный маршрут?';
							|en = 'Удалить выбранный маршрут?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревьяДляСравнения.Вставить("ДеревоСравненияРеестра",  ИзвлечьСтруктуруXML("СхемаВыгрузки506_1"));
	ДеревьяДляСравнения.Вставить("ДеревоСравненияСведений", ИзвлечьСтруктуруXML("СхемаВыгрузки506_2"));
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревьяДляЗагрузки.Вставить("ДеревоЗагрузкиРеестра",  ИзвлечьСтруктуруXML("СхемаВыгрузки506_1"));
	ДеревьяДляЗагрузки.Вставить("ДеревоЗагрузкиСведений", ИзвлечьСтруктуруXML("СхемаВыгрузки506_2"));
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки, ПроверятьВозможностьВыгрузки)
	...
	ДеревоВыгрузкиРеестра  = ИзвлечьСтруктуруXML("СхемаВыгрузки506_1");
	ДеревоВыгрузкиСведений = ИзвлечьСтруктуруXML("СхемаВыгрузки506_2");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрПрвзД_14");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRPRVZNDS"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRPRVZNDS.2"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузкиРеестра, ДеревоВыгрузкиСведений)
	...
	ВозмКодыВидаТС_Стр = "10, 20, 30, 40";
	ВозмКодыВидаТС = СтрРазделить(ВозмКодыВидаТС_Стр, ", ", Ложь);
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			КодВидТС = СокрЛП(Ветка_П10000.Данные["П1000002"]);
			
			Если НЕ ЗначениеЗаполнено(КодВидТС) Тогда
				ИмяПок = "П1000002" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " не задан код вида транспорта", "Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ВозмКодыВидаТС.Найти(КодВидТС) = Неопределено Тогда
				ИмяПок = "П1000002" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " задан некорректный код вида транспорта. Возможны значения: " + ВозмКодыВидаТС_Стр,
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100003"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан номер перевозочного документа", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000004"]) Тогда
				...
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не задана дата оказания услуги",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П10000.Данные["П1000004"]) Тогда
				ИмяПок = "П1000004" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " задана дата оказания услуги, выходящая за пределы периода с 01.01.1900 по 31.12.2099",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200005"]) Тогда
					ИмяПок = "П1200005" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан пункт отправления в маршруте", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200006"]) Тогда
					ИмяПок = "П1200006" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан пункт назначения в маршруте", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрПрвзД_14 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрПрвзД_14");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрПрвзД_14 = СкопироватьУзел(УзелРодитель.Родитель, РеестрПрвзД_14);
		Для каждого Эл Из ЭкзРеестрПрвзД_14.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		// Узел "СведПрвзД" имеется только в схеме выгрузки "Сведений".
		// Если он не найден, значит, сейчас формируются данные "Реестра"
		// (для выгрузки первого из 2-х файлов).
		СведПрвзД = ПолучитьПодчиненныйЭлемент(ЭкзРеестрПрвзД_14, "СведПрвзД");
		Если СведПрвзД = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведПрвзД = СкопироватьУзел(ЭкзРеестрПрвзД_14, СведПрвзД, ЭкзРеестрПрвзД_14.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСведПрвзД.Строки Цикл
					Если Эл.Строки.Количество() = 0
						И Эл.Код <> "НомПрвзДок" Тогда
						ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
					КонецЕсли;
				КонецЦикла;
				
				// 11000.
				НомПрвзДок = ПолучитьПодчиненныйЭлемент(ЭкзСведПрвзД, "НомПрвзДок");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					ЭкзНомПрвзДок = СкопироватьУзел(ЭкзСведПрвзД, НомПрвзДок, ЭкзСведПрвзД.Строки.Количество() - 1);
					ВывестиПоказательВXML(ЭкзНомПрвзДок, Ветка_П11000.Данные[ЭкзНомПрвзДок.Ключ]);
				КонецЦикла;
				
				// 12000.
				Маршрут = ПолучитьПодчиненныйЭлемент(ЭкзСведПрвзД, "Маршрут");
				ДанныеМнЧ_П12000 = Ветка_П10000.ДанныеМногострочныхЧастей.П12000.Строки;
				Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
					
					ЭкзМаршрут = СкопироватьУзел(ЭкзСведПрвзД, Маршрут, ЭкзСведПрвзД.Строки.Количество() - 1);
					Для каждого Эл Из ЭкзМаршрут.Строки Цикл
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_14");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_14_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
...
Процедура ВывестиНаПечатьОтчет(СтруктураПараметров, ВыбранныеСтраницы, СтрПоля = Неопределено)
	...
	Если ВыбранныеСтраницы.Найти("Реестр") <> Неопределено Тогда
		...
		ПечФорма1.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Если ЗначениеЗаполнено(ОблОбъедСтрока1_Верх) И ЗначениеЗаполнено(ОблОбъедСтрока1_Низ)
					...
					Для НомерКолонки = 2 По 9 Цикл
						Если НомерКолонки = 6 ИЛИ НомерКолонки = 7 Тогда
							Продолжить;
						КонецЕсли;
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Реестр НДС Приложение 14");
...
&НаСервере
Процедура ПроверкаКС_14_1_1()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '14.1.1';
													|en = '14.1.1'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 14';
														|en = 'Реестр 14'"));
...
&НаСервере
Процедура ПроверкаКС_14_1_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '14.1.2';
													|en = '14.1.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 14';
														|en = 'Реестр 14'"));
...
&НаСервере
Процедура ПроверкаКС_14_2_1а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '14.2.1.а';
													|en = '14.2.1.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 14';
														|en = 'Реестр 14'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_14_2_1б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '14.2.1.б';
													|en = '14.2.1.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 14';
														|en = 'Реестр 14'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_14_2_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '14.2.2';
													|en = '14.2.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 14';
														|en = 'Реестр 14'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
