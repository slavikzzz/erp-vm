...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "22/03/2012";
...
Процедура УстановитьФорматВывода(Форма) Экспорт
	...
	Форма.ПолеТабличногоДокументаФормаОтчета.Области.П000010018004.Формат = Форма.СтруктураРеквизитовФормы.мСтрокаФормата;
	...
	Если мФормаОтчета.Области.П000010001003.Значение > 100 Тогда
		Если мФормаОтчета.Области.П000010004005.Значение > 0 Тогда
			...
			Если мФормаОтчета.Области.П000010004005.Значение > КолвоМолодежи4 Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Форма 1-квотирование: в строке 04 можно указать не более %1 рабочих мест для молодежи';
																								|en = 'Форма 1-квотирование: в строке 04 можно указать не более %1 рабочих мест для молодежи'"), КолвоМолодежи4);
				Сообщение.Сообщить();
	...
	Если мФормаОтчета.Области.П000010002003.Значение > 100 Тогда
		Если мФормаОтчета.Области.П000010005005.Значение > 0 Тогда
			...
			Если мФормаОтчета.Области.П000010005005.Значение > КолвоМолодежи5 Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Форма 1-квотирование: в строке 05 можно указать не более %1 рабочих мест для молодежи';
																								|en = 'Форма 1-квотирование: в строке 05 можно указать не более %1 рабочих мест для молодежи'"), КолвоМолодежи5);
				Сообщение.Сообщить();
	...
	Если мФормаОтчета.Области.П000010003003.Значение > 100 Тогда
		Если мФормаОтчета.Области.П000010006005.Значение > 0 Тогда
			...
			Если мФормаОтчета.Области.П000010006005.Значение > КолвоМолодежи6 Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Форма 1-квотирование: в строке 06 можно указать не более %1 рабочих мест для молодежи';
																								|en = 'Форма 1-квотирование: в строке 06 можно указать не более %1 рабочих мест для молодежи'"), КолвоМолодежи6);
				Сообщение.Сообщить();
