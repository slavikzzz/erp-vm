...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр р.НДС прилож.05 21к2") Тогда
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	// Группа П11100.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П1110005");
	ИменаГраф.Добавить("П1110006");
	ИменаГраф.Добавить("П1110007");
	ИменаПодчиненныхГрупп = Новый Массив;
	СтруктураМногострочныхЧастей.Вставить(
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1100004", 2, ,
		"Выбор кода вида транспортного средства", КоллекцияСписковВыбора["КодыВидаТС"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1110005", 2, ,
...
Процедура СохранитьНаКлиенте(Автосохранение = Ложь, ВыполняемоеОповещение = Неопределено) Экспорт
	...
	ОписаниеОповещенияПослеВопросаРЭЦ = Новый ОписаниеОповещения("ВопросИзменитьОтправленныйВРЭЦ", ЭтотОбъект, ДополнительныеПараметры);
	Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСохраненныйДок) И Модифицированность = Истина Тогда
		ВозможностьРедактированияОтчетаРЭЦ = ДокументооборотСРЭЦВызовСервера.ПроверитьВозможностьРедактированияОтчета(СтруктураРеквизитовФормы.мСохраненныйДок);
		Если ВозможностьРедактированияОтчетаРЭЦ.ВозможностьРедактировать = Ложь И Автосохранение = Ложь Тогда   
				...
				Если ЗначениеЗаполнено(КодОперации) Тогда
					...
					Пока Форма.ТабличныйДокумент.Области.Найти("П1000003_" + Формат(Ном, "ЧГ=")) <> Неопределено Цикл
						СуммаНалБаз = СуммаНалБаз
							+ Форма.ТабличныйДокумент.Области["П1000003_" + Формат(Ном, "ЧГ=")].Значение;
						Ном = Ном + 1;
					КонецЦикла;
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если Ветка = АктивнаяВетка Тогда
				...
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					Ветка_П10000.Данные.Вставить("П1000003", Форма.ТабличныйДокумент.Области["П1000003_"
			...
			Если ЗначениеЗаполнено(КодОперации) Тогда
				ДанныеМнЧ_П10000 = Ветка.ДанныеМногострочныхЧастей.П10000.Строки;
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					СуммаНалБаз = СуммаНалБаз + Ветка_П10000.Данные["П1000003"];
				КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("НалБазаИтого", "КодОперации;П1000003");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрТРД_5");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRTRDNDS"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRTRDNDS.2"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузкиРеестра, ДеревоВыгрузкиСведений)
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			ДлинаРегНомТД = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000002"]));
			Если НЕ (ДлинаРегНомТД = 23 ИЛИ (ДлинаРегНомТД >= 25 И ДлинаРегНомТД <= 29)) Тогда
				...
				+ " задан регистрационный номер таможенной декларации (полной таможенной декларации). Возможное количество символов в номере: 23 или от 25 до 29",
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				КодВидТС = СокрЛП(Ветка_П11000.Данные["П1100004"]);
				...
				Если ДлинаКодВидТС <> 2 Тогда
					...
					+ " задан код вида транспортного средства, которым товары ввозились на территорию Российской Федерации или вывозились с территории Российской Федерации",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Для каждого Ветка_П11100 Из ДанныеМнЧ_П11100 Цикл
					...
					Если НЕ (ЗначениеЗаполнено(Ветка_П11100.Данные["П1110005"])
						ИЛИ ЗначениеЗаполнено(Ветка_П11100.Данные["П1110006"])
						ИЛИ ЗначениеЗаполнено(Ветка_П11100.Данные["П1110007"])) Тогда
						Продолжить;
					КонецЕсли;
					
					ЕстьХотяБыОднаЗапись_П11100 = Истина;
					
					ДлинаВидТрДок = СтрДлина(СокрЛП(Ветка_П11100.Данные["П1110005"]));
					Если ДлинаВидТрДок <> 2 Тогда
						ИмяПок = "П1110005" + "_" + НомерСтрокиП10000_Строка
						+ "_" + НомерСтрокиП11000_Строка + "_" + НомерСтрокиП11100_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомПП + " не" + ?(ДлинаВидТрДок = 0, "", "верно")
						+ " задан вид транспортного, товаросопроводительного и (или) иного документа, подтверждающего вывоз товаров за пределы Российской Федерации или ввоз товаров на территорию Российской Федерации",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					Если НЕ ЗначениеЗаполнено(Ветка_П11100.Данные["П1110007"]) Тогда
						ИмяПок = "П1110007" + "_" + НомерСтрокиП10000_Строка
						+ "_" + НомерСтрокиП11000_Строка + "_" + НомерСтрокиП11100_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомПП + " не задана дата транспортного, товаросопроводительного и (или) иного документа, подтверждающего вывоз товаров за пределы Российской Федерации или ввоз товаров на территорию Российской Федерации",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				...
				Если ЗначениеЗаполнено(КодВидТС) И КодВидТС <> "71" И КодВидТС <> "72" И НЕ ЕстьХотяБыОднаЗапись_П11100 Тогда
					ИмяПок = "П1110005" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка + "_1";
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " должны быть заданы сведения о транспортном, товаросопроводительном и (или) ином документе, подтверждающем вывоз товаров за пределы Российской Федерации или ввоз товаров на территорию Российской Федерации, если значение кода вида транспортного средства, которым товары ввозились на территорию Российской Федерации или вывозились с территории Российской Федерации, неравно 71, 72",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрТРД_5 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрТРД_5");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрТРД_5 = СкопироватьУзел(УзелРодитель.Родитель, РеестрТРД_5);
		Для каждого Эл Из ЭкзРеестрТРД_5.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		// Узел "СведТД" имеется только в схеме выгрузки "Сведений".
		// Если он не найден, значит, сейчас формируются данные "Реестра"
		// (для выгрузки первого из 2-х файлов).
		СведТД = ПолучитьПодчиненныйЭлемент(ЭкзРеестрТРД_5, "СведТД");
		Если СведТД = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведТД = СкопироватьУзел(ЭкзРеестрТРД_5, СведТД, ЭкзРеестрТРД_5.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСведТД.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
				...
				// 11000.
				СвВидТрДок = ПолучитьПодчиненныйЭлемент(ЭкзСведТД, "СвВидТрДок");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					
					ЭкзСвВидТрДок = СкопироватьУзел(ЭкзСведТД, СвВидТрДок, ЭкзСведТД.Строки.Количество() - 1);
					...
					Для каждого Ветка_П11100 Из ДанныеМнЧ_П11100 Цикл
						...
						Для каждого Эл Из ЭкзТрДок.Строки Цикл
							Если Эл.Код = "НомТрДок" Тогда
								...
								Заполн_ВидТрДок = ЗначениеЗаполнено(Ветка_П11100.Данные["П1110005"]);
								Заполн_ДатаТрДок = ЗначениеЗаполнено(Ветка_П11100.Данные["П1110007"]);
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_5");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_5_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Если ОблОбъедСтрока1_Верх <> ОблОбъедСтрока1_Низ Тогда
					...
					Для НомерКолонки = 2 По 9 Цикл
						Если НомерКолонки >= 5 И НомерКолонки <= 8 Тогда
							Продолжить;
						КонецЕсли;
...
Функция ЗаполнитьАвтоНаСервере()
	...
	РезультатЗапуска = ДлительныеОперации.ВыполнитьВФоне("РегламентированнаяОтчетность.Автозаполнение_РеестрПоНДС_Приложение5",
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Реестр НДС Приложение 5");
...
#Область ОбщиеДанные
	...
	ДанныеКС.Вставить("ГрафаНалоговойБазы"		, "03");
...
&НаСервере
Процедура ПроверкаКС_5_1_1()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '5.1.1';
													|en = '5.1.1'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 5';
														|en = 'Реестр 5'"));
...
&НаСервере
Процедура ПроверкаКС_5_1_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '5.1.2';
													|en = '5.1.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 5';
														|en = 'Реестр 5'"));
...
&НаСервере
Процедура ПроверкаКС_5_1_3а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '5.1.3.а';
													|en = '5.1.3.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 5';
														|en = 'Реестр 5'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001"		, "П1000002_");
	СформироватьПоказатели(Показатели, ИменаПоказателей);
...
&НаСервере
Процедура ПроверкаКС_5_1_3б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '5.1.3.б';
													|en = '5.1.3.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 5';
														|en = 'Реестр 5'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_5_1_3в()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '5.1.3.в';
													|en = '5.1.3.в'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 5';
														|en = 'Реестр 5'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_5_1_4()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '5.1.4';
													|en = '5.1.4'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 5';
														|en = 'Реестр 5'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001"		, "П1100004_");
	ИменаПоказателей.Вставить("Р01Г002"		, "П1110005_");
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			Для Счетчик2 = 1 По СтраницаРеестра["КолвоСтрокП11000_" + XMLСтрока(Счетчик)]  Цикл
				ЗаполнитьПоказатель(Показатели.Р01Г001, СтраницаРеестра, Счетчик, Счетчик2);
					Для Счетчик3 = 1 По СтраницаРеестра["КолвоСтрокП11100_" + XMLСтрока(Счетчик) + "_" + XMLСтрока(Счетчик2)]  Цикл
...
&НаСервере
Процедура ПроверкаКС_5_2_1а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '5.2.1.а';
													|en = '5.2.1.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 5';
														|en = 'Реестр 5'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_5_2_1б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '5.2.1.б';
													|en = '5.2.1.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 5';
														|en = 'Реестр 5'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_5_2_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '5.2.2';
													|en = '5.2.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 5';
														|en = 'Реестр 5'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
#Область ПроверкаВРЭЦ

&НаКлиенте
Процедура ПроверитьВРЭЦ(Команда)
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("Форма", ЭтаФорма);
	ОписаниеОповещения = Новый ОписаниеОповещения("ПроверитьВРЭЦПродолжение", ЭтотОбъект, ДополнительныеПараметры);
	
	Если Не ЗначениеЗаполнено (СтруктураРеквизитовФормы["ДекларацияНДС"]) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Для проверки в сервисе ""Мой экспорт"" необходимо заполнить декларацию НДС.';
								|en = 'Для проверки в сервисе ""Мой экспорт"" необходимо заполнить декларацию НДС.'");
		Сообщение.Сообщить();
	Иначе
		Если Модифицированность Тогда
			СохранитьНаКлиенте(, ОписаниеОповещения)
...
&НаКлиенте
Процедура ПроверитьВРЭЦПродолжение(Результат, ДополнительныеПараметры) Экспорт
	
	ПараметрыФормы = Новый Структура("Организация, ДекларацияНДС, РеестрНДС",
	СтруктураРеквизитовФормы.Организация, СтруктураРеквизитовФормы.ДекларацияНДС, СтруктураРеквизитовФормы.мСохраненныйДок);
	ФормаДляПроверкиДанныхВРЭЦ = ПолучитьФорму("ОбщаяФорма.ПодготовкаДанныхДляПроверкиВРЭЦ", ПараметрыФормы, ЭтаФорма, УникальныйИдентификатор);
	ФормаДляПроверкиДанныхВРЭЦ.Заголовок = НСтр("ru = 'Подготовка и отправка данных в сервис ""Мой экспорт""';
												|en = 'Подготовка и отправка данных в сервис ""Мой экспорт""'");
	ФормаДляПроверкиДанныхВРЭЦ.Открыть();

КонецПроцедуры

&НаКлиенте
Процедура ЭтапыОтправкиРЭЦНажатие(Элемент)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОткрытьСостояниеОтправкиИзПанелиОтправки(ЭтаФорма, "РЭЦ");
КонецПроцедуры

&НаКлиенте
Процедура ГиперссылкаПротоколРЭЦНажатие(Элемент)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОткрытьПротоколИзПанелиОтправки(ЭтаФорма, "РЭЦ");
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьОтправкуРЭЦ(Команда)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОбновитьОтправкуИзПанелиОтправки(ЭтаФорма, "РЭЦ");
КонецПроцедуры

&НаКлиенте
Процедура ЗакрытьПанельИнфоМойЭкспортНажатие(Элемент)
	
	БольшеНеПоказыватьИнформационнуюПанельРЭЦ();
	
КонецПроцедуры

&НаСервере
Процедура БольшеНеПоказыватьИнформационнуюПанельРЭЦ()
	
	ХранилищеОбщихНастроек.Сохранить(
		"ДокументооборотСКонтролирующимиОрганами_РегламентированныйОтчетРеестрНДСПриложение1_УбратьИнформационнуюПанельРЭЦ",
		,
		Истина);
	
	УстановитьВидимостьИнформационнойПанелиРЭЦ();
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьИнформационнойПанелиРЭЦ()

	ПанельУбранаПользователем = ХранилищеОбщихНастроек.Загрузить("ДокументооборотСКонтролирующимиОрганами_РегламентированныйОтчетРеестрНДСПриложение1_УбратьИнформационнуюПанельРЭЦ") = Истина;
	
	Если ПанельУбранаПользователем Тогда
		Элементы.ИнформационнаяПанельРЭЦ.Видимость = Ложь;
	Иначе
		Элементы.ИнформационнаяПанельРЭЦ.Видимость = Истина;
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ДекорацияИнфоМойЭкспортОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ПерейтиПоНавигационнойСсылке(ДокументооборотСРЭЦКлиентСервер.АдресСтатьиОСервисеРЭЦ());
