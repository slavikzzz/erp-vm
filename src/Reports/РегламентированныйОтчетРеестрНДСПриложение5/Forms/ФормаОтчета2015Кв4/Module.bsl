...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Если РазделЗаполнен(ДанныеРеестра.Строки, Ложь) Тогда
		...
		ПроверитьКС_5_1_1(СтруктураРезультата);
		ПроверитьКС_5_1_2(СтруктураРезультата);
		ПроверитьКС_5_1_3(СтруктураРезультата);
		ПроверитьКС_5_1_4(СтруктураРезультата);
		ПроверитьКС_5_2_1(СтруктураРезультата);
		ПроверитьКС_5_2_2(СтруктураРезультата);
		ПроверитьКС_5_2_3(СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_5_1_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 5.1.1';
												|en = 'Проверка КС 5.1.1'"));
...
&НаСервере
Процедура ПроверитьКС_5_1_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 5.1.2';
												|en = 'Проверка КС 5.1.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_5_1_3(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		,НСтр("ru = 'Проверка КС 5.1.3';
													|en = 'Проверка КС 5.1.3'"));
	Заголовки.Вставить("Нарушение"			,НСтр("ru = 'п. 1, п. 3.2, п. 3.3, п. 3.6, п. 4 ст. 165 НК РФ';
													|en = 'п. 1, п. 3.2, п. 3.3, п. 3.6, п. 4 ст. 165 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			,НСтр("ru = 'Регистрационный номер ДТ должен отвечать следующим требованиям:
		...
		|en = 'Регистрационный номер ДТ должен отвечать следующим требованиям:
		|-должен состоять из 23 символов и иметь вид ""TTTTTTTT/DDMMGG/NNNNNNN"", / - косая черта вправо, разделитель показателей;
		|-DDMMGG – дата декларации на товары (день, месяц, год последние 2 цифры);
		|-GG - значение года оформления ДТ, должно быть не меньше значения текущего года уменьшенного на 4 и не больше текущего;
		|-NNNNNNN – порядковый номер декларации на товары для данного таможенного органа, должен быть больше 0'"));
	Заголовки.Вставить("ПроверятьПустые"	,Истина);
	Заголовки.Вставить("Декларирование"		,Ложь);
	Настройки	= ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_5_1_4(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 5.1.4';
												|en = 'Проверка КС 5.1.4'"));
...
&НаСервере
Процедура ПроверитьКС_5_2_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 5.2.1';
												|en = 'Проверка КС 5.2.1'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_5_2_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 5.2.2';
												|en = 'Проверка КС 5.2.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_5_2_3(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 5.2.3';
												|en = 'Проверка КС 5.2.3'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
Процедура ПроверитьКС_СоответствиеВидаДокументаВидуТранспорта_3Уровня(СтруктураРезультата, Настройки)
	...
	Для каждого СтраницаРаздела Из ДанныеРеестра.Строки Цикл
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл
			...
			НомерРасчетный	= НомерРасчетный + 1;
			СписокСтрок2Уровень	= ДанныеСтраницы[НомерСтроки - 1].ДанныеМногострочныхЧастей.П11000.Строки;
				...
				Иначе
					Для каждого СтрокаПравил Из Настройки.ТаблицаПравил Цикл
						Если СтрНайти(СтрокаПравил.ВидТранспорта, ПоказательВТ.ВидТранспорта.Значение) = 0 Тогда
							Продолжить;
						Иначе
							СписокСтрок3Уровень = Строка2Уровень.ДанныеМногострочныхЧастей.П11100.Строки;
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	ИначеЕсли УсловныйКодПоказателя = "ВидТранспорта" Тогда
		Результат.Вставить("Имя", "П1100004%1%2%3%4");
	...
	ИначеЕсли УсловныйКодПоказателя = "ВидДокумента" Тогда
		Результат.Вставить("Имя", "П1110005%1%2%3%4%5%6");
