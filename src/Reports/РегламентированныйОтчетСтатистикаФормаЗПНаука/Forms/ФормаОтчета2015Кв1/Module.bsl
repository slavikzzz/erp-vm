...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если СтрЧислоВхождений(РезультатВыбора["Код"], ".") < 1 Тогда
        ...
        НСтр("ru = 'Выбранный код (%1) принадлежит группе. Выбрать можно только конечный код.';
			|en = 'Выбранный код (%1) принадлежит группе. Выбрать можно только конечный код.'"), СокрЛП(РезультатВыбора["Код"])));
    КонецЕсли;
...
Процедура ПроверитьВыгрузкуОтчета()
	...
	Если КонецМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <> КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда 
		...
		Сообщение.Текст = НСтр("ru = 'За данный период электронное представление не предусмотрено.';
								|en = 'За данный период электронное представление не предусмотрено.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
