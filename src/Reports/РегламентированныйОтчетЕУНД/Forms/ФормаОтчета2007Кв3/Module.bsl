	...
	ИначеЕсли (СтрНайти(ОбластьИмя, "НомерКвартала") > 0) И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
		...
		Если НЕ СтрокаНДС Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Графа 4 заполняется только для налога на добавленную стоимость.';
									|en = 'Графа 4 заполняется только для налога на добавленную стоимость.'");
			Сообщение.Сообщить();			
			Возврат;
		КонецЕсли;
...
Процедура ОтметитьЭлементыСписокНалоговЗавершение(Список, ДополнительныеПараметры) Экспорт
	...
	Если Список <> Неопределено Тогда
		...
		Если СписокНалогов.НайтиПоЗначению("25").Пометка И СписокНалогов.НайтиПоЗначению("26.2").Пометка Тогда
			...
			ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Одновременный выбор налогов ""%1"" и ""%2"" не допускается!';
																								|en = 'Одновременный выбор налогов ""%1"" и ""%2"" не допускается!'"), СписокНалогов.НайтиПоЗначению("25").Представление, СписокНалогов.НайтиПоЗначению("26.2").Представление));
...
Функция ИзвлечьСтруктуруXML()
	...
	Если ПрименяетсяФормат402(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда
		Макет = ОбъектОтчета(ИмяФормы).ПолучитьМакет("СтруктураXML402");
	Иначе
		Макет = ОбъектОтчета(ИмяФормы).ПолучитьМакет("СтруктураXML2007");
	КонецЕсли;
