...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "03/09/2013";
	...
	Иначе
		...
		Иначе
			...
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				...
				Если Лев(Область.Имя, 13) = "П000005000102" Или Лев(Область.Имя, 13) = "П000005000105" Тогда
					Попытка
					...
					Исключение
						Если Не СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
							Суффикс = Сред(Область.Имя, 15);
							Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П000005000101_" + Суффикс].Значение) Тогда
								Сообщение = Новый СообщениеПользователю;
								Сообщение.Текст = НСтр("ru = 'Не найдена единица измерения для наименования продукции, указанной в графе 1 раздела 5!';
														|en = 'Не найдена единица измерения для наименования продукции, указанной в графе 1 раздела 5!'");
								Сообщение.Сообщить();
							Иначе
								Сообщение = Новый СообщениеПользователю;
								Сообщение.Текст = НСтр("ru = 'Не указано наименование продукции в графе 1 раздела 5 для выбора единицы измерения!';
														|en = 'Не указано наименование продукции в графе 1 раздела 5 для выбора единицы измерения!'");
								Сообщение.Сообщить();
							КонецЕсли;
							СтандартнаяОбработка = Ложь;
						КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000005000104", 250, , "Выбор продукции",
		КоллекцияСписковВыбора["СправочникПродукции"]);
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000005000105", 250, , "Выбор единицы измерения",
		КоллекцияСписковВыбора["СправочникЕдиницИзмерения"]);
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	ИначеЕсли Лев(Область.Имя, 13) = "П000005000101" Или Лев(Область.Имя, 13) = "П000005000104" Тогда
		Суффикс = СтрЗаменить(Область.Имя, КодПоказателя, "");
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура("Код", ЭтаФорма[ТекущийЭлемент.Имя].Области["П000005000104" + Суффикс].Значение));
	ИначеЕсли Лев(Область.Имя, 13) = "П000005000102" Или Лев(Область.Имя, 13) = "П000005000105" Тогда
		Суффикс = СтрЗаменить(Область.Имя, КодПоказателя, "");
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура("Код", ЭтаФорма[ТекущийЭлемент.Имя].Области["П000005000105" + Суффикс].Значение));
	Иначе
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(КолонкаПоиска, ИсходноеЗначение));
	КонецЕсли;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
    ...
    ИначеЕсли Лев(Область.Имя, 13) = "П000005000101" Или Лев(Область.Имя, 13) = "П000005000104" Тогда
        ...
        Суффикс = СтрЗаменить(Область.Имя, КодПоказателя, "");
        ПредыдущееЗначениеВКолонке01 = ЭтаФорма[ТекущийЭлемент.Имя].Области["П000005000101" + Суффикс].Значение;
        ВыборКод = Строка(РезультатВыбора["Код"]);
        ВыборНазвание = Строка(РезультатВыбора["Название"]);
        
        ЭтаФорма[ТекущийЭлемент.Имя].Области["П000005000101" + Суффикс].Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборНазвание, "");
        ЭтаФорма[ТекущийЭлемент.Имя].Области["П000005000104" + Суффикс].Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборКод,      "");
        
        // Изменилась продукция - очищаем значения в остальных графах по текущей строке.
        Если ПредыдущееЗначениеВКолонке01 <> ЭтаФорма[ТекущийЭлемент.Имя].Области["П000005000101" + Суффикс].Значение Тогда
            ЭтаФорма[ТекущийЭлемент.Имя].Области["П000005000102" + Суффикс].Значение = "";
            ЭтаФорма[ТекущийЭлемент.Имя].Области["П000005000103" + Суффикс].Значение = 0;
            ЭтаФорма[ТекущийЭлемент.Имя].Области["П000005000105" + Суффикс].Значение = 0;
        КонецЕсли;
    ...
    ИначеЕсли Лев(Область.Имя, 13) = "П000005000102" Или Лев(Область.Имя, 13) = "П000005000105" Тогда
        ...
        ЭтаФорма[ТекущийЭлемент.Имя].Области["П000005000102" + Суффикс].Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборНазвание, "");
        ЭтаФорма[ТекущийЭлемент.Имя].Области["П000005000105" + Суффикс].Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборКод,      "");
