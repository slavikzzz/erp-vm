...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "10/06/2013";
	...
	СтруктураРеквизитовФормы.мИдГруппы2 = "П0000050001";
...
Процедура ДоступностьИЦветЯчеекЕдиницИзмеренияСоответственноПродукции1ИПсвед(ПарамФорма)
	
	Для НомСтр = 1 По ПарамФорма.П0000050001.Количество() Цикл
		Если ЗначениеЗаполнено(ПарамФорма.ПолеТабличногоДокументаФормаОтчета.Области["П000005000101_" + Формат(НомСтр, "ЧГ=")].Значение) Тогда
			ПарамФорма.ПолеТабличногоДокументаФормаОтчета.Области["П000005000102_" + Формат(НомСтр, "ЧГ=")].ЦветФона = ПарамФорма.СтруктураРеквизитовФормы.ЦветЖелтый;
			ПарамФорма.ПолеТабличногоДокументаФормаОтчета.Области["П000005000102_" + Формат(НомСтр, "ЧГ=")].Защита = Ложь;
		Иначе
			ПарамФорма.ПолеТабличногоДокументаФормаОтчета.Области["П000005000102_" + Формат(НомСтр, "ЧГ=")].ЦветФона = ПарамФорма.СтруктураРеквизитовФормы.ЦветБелый;
			ПарамФорма.ПолеТабличногоДокументаФормаОтчета.Области["П000005000102_" + Формат(НомСтр, "ЧГ=")].Защита = Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	ДоступностьИЦветЯчеекЕдиницИзмеренияСоответственноПродукции1ИПсвед(ЭтаФорма);
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	Если ИмяГруппы = "П0000050001" Тогда
	...
	Иначе
		...
		Иначе
			...
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				...
				Если Лев(Область.Имя, 13) = "П000005000102" Тогда
					// Фильтр единиц измерения по продукции, указанной в Колонке 01 Раздела 5.
					КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
					ОтборПоКодуПродукции = Новый Структура;
					ОтборПоКодуПродукции.Вставить("КодПродукции", СвойстваПоказателей.НайтиСтроки(Новый Структура("Показатель", "П000005000101"))[0].ТаблицаЗначений.НайтиСтроки(Новый Структура("Название", ПолеТабличногоДокументаФормаОтчета.Области["П000005000101_" + Сред(Область.Имя, 15)].Значение))[0].Код);
					ФильтрМассив = ПараметрВводаПоля.ТаблицаЗначений.НайтиСтроки(ОтборПоКодуПродукции);
					ТЗВыбораЕИ.Очистить();
					Для Каждого СтрФильтрМассива Из ФильтрМассив Цикл
						НовСтрТЗ = ТЗВыбораЕИ.Добавить();
						НовСтрТЗ.Код = СтрФильтрМассива.Код;
						НовСтрТЗ.Название = СтрФильтрМассива.Название;
					КонецЦикла;
					ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе,                        ТЗВыбораЕИ, СтандартнаяОбработка, КолонкаПоиска);
				Иначе
...
&НаКлиенте
Процедура ПолеТабличногоДокументаФормаОтчетаВыборВведитеВыручкуТипЧисла30Завершение(Значение, ДополнительныеПараметры) Экспорт
	
	ПервоначальноеЗначение = ?(Значение = Неопределено, ДополнительныеПараметры.ПервоначальноеЗначение, Значение);
...
&НаКлиенте
Процедура ПолеТабличногоДокументаФормаОтчетаВыборВведитеВыручкуТипЧисла20Завершение(Значение, ДополнительныеПараметры) Экспорт
...
Функция ПечатьНаСервере(ВидПечати)
	...
	// Объединение ячеек с наименованием вида деятельности
	// (Раздел 4, для корректного отображения и печати).
	Линия = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
	Для Каждого ПечФорма Из СтруктураРеквизитовФормы.мПечатныеФормы Цикл
		ТекПустаяСтрока = ПечФорма.Значение.Области.Найти("ПустаяСтрока1");
		Если ТекПустаяСтрока <> Неопределено Тогда
			СтруктураРеквизитовФормы.мПечатныеФормы[СтруктураРеквизитовФормы.мПечатныеФормы.Индекс(ПечФорма)].Значение.Область("ПустаяСтрока1").Имя = "";
		КонецЕсли;
		Для Каждого ТекОбласть Из ПечФорма.Значение.Области Цикл
			Если СтрНайти(ТекОбласть.Имя, "П000004000103") > 0 Тогда
				ТекОбласть.АвтовысотаСтроки = Ложь;
				ТекОбласть.ВысотаСтроки = 15;
				ПечФорма.Значение.Область(ТекОбласть.Верх, 2, ТекОбласть.Низ + 1, 2).Объединить();
				ПечФорма.Значение.Область(ТекОбласть.Верх, 2, ТекОбласть.Низ + 1, 2).ГраницаСправа = Линия;
				ПечФорма.Значение.Область(ТекОбласть.Верх, 2, ТекОбласть.Низ + 1, 2).ГраницаСнизу = Линия;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	// Удаление повторяющихся заголовков Разделов 4 и 4.1.
	...
	Пока ТекИндекс2 > 0 Цикл
		...
		Для Каждого ТекОбласть2 Из СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.Области Цикл
			Если СтрНайти(ТекОбласть2.Имя, "П000004000103") > 0 Тогда
				Для Каждого ТекОбласть1 Из СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс1].Значение.Области Цикл
					Если СтрНайти(ТекОбласть1.Имя, "П000004000103") > 0 Тогда
						// Удаление.
						СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.УдалитьОбласть(СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.Область("ОсновнаяЧасть3"), ТипСмещенияТабличногоДокумента.ПоВертикали);
						ЦиклДляВнутреннийПрерван = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
				// Дополнительно - удаление лишних (переносящихся с предыдущей страницы) пустых строк вверху.
				Пока СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.Область(2, 3, 2, 3).Защита = Истина
					И СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.Область(2, 3, 2, 3).Текст = "" Цикл
					СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.УдалитьОбласть(СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.Область(2, , 2, ), ТипСмещенияТабличногоДокумента.ПоВертикали);
				КонецЦикла;
				// Дополнительно - удаление лишних пустых строк внизу.
				ТекВысотаТаблицы = СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.ВысотаТаблицы;
				Пока СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.Область(ТекВысотаТаблицы - 1, 3, ТекВысотаТаблицы - 1, 3).Защита = Истина
					И СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.Область(ТекВысотаТаблицы - 2, 3, ТекВысотаТаблицы - 2, 3).Защита = Истина Цикл
					СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.УдалитьОбласть(СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.Область(ТекВысотаТаблицы - 1, , ТекВысотаТаблицы - 1, ), ТипСмещенияТабличногоДокумента.ПоВертикали);
					ТекВысотаТаблицы = ТекВысотаТаблицы - 1;
				КонецЦикла;
	...
	// Удаление повторяющихся заголовков Раздела 5 и текста над ним.
	ТекИндекс2 = СтруктураРеквизитовФормы.мПечатныеФормы.Количество() - 1;
	ЦиклДляВнутреннийПрерван = Ложь;
	Пока ТекИндекс2 > 0 Цикл
		ТекИндекс1 = ТекИндекс2 - 1;
		Для Каждого ТекОбласть2 Из СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.Области Цикл
			Если СтрНайти(ТекОбласть2.Имя, "П000005000101") > 0 Тогда
				Для Каждого ТекОбласть1 Из СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс1].Значение.Области Цикл
					Если СтрНайти(ТекОбласть1.Имя, "П000005000101") > 0 Тогда
						СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.УдалитьОбласть(СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.Область("ОсновнаяЧасть5"), ТипСмещенияТабличногоДокумента.ПоВертикали);
						ЦиклДляВнутреннийПрерван = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000005000101", 250, , "Выбор продукции",
		КоллекцияСписковВыбора["СправочникПродукции"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000005000102", 250, , "Выбор единицы измерения",
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Область.Имя = КодПоказателя ИЛИ СтрНайти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы1) > 0 ИЛИ СтрНайти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы2) > 0 Тогда
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Лев(Область.Имя, 13) = "П000005000101" Тогда
		
		Суффикс = СтрЗаменить(Область.Имя, КодПоказателя, "");
		ПредыдущееЗначениеВКолонке01 = Область.Значение;
		ВыборКод = Строка(РезультатВыбора["Код"]);
		ВыборНазвание = Строка(РезультатВыбора["Название"]);
		Область.Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборНазвание, "");
		
		// Приводим в соответствие зависимую ячейку - единицу измерения продукции.
		Если ЗначениеЗаполнено(ВыборКод) Тогда
			Если ПредыдущееЗначениеВКолонке01 <> ЭтаФорма[ТекущийЭлемент.Имя].Области[СтруктураРеквизитовФормы.мИдГруппы2 + "01" + Суффикс].Значение Тогда
				ЭтаФорма[ТекущийЭлемент.Имя].Области[СтруктураРеквизитовФормы.мИдГруппы2 + "02" + Суффикс].Значение = "";
			КонецЕсли;
			ЭтаФорма[ТекущийЭлемент.Имя].Области[СтруктураРеквизитовФормы.мИдГруппы2 + "02" + Суффикс].ЦветФона = Новый Цвет(255, 255, 192);
			ЭтаФорма[ТекущийЭлемент.Имя].Области[СтруктураРеквизитовФормы.мИдГруппы2 + "02" + Суффикс].Защита = Ложь;
		Иначе
			ЭтаФорма[ТекущийЭлемент.Имя].Области[СтруктураРеквизитовФормы.мИдГруппы2 + "02" + Суффикс].Значение = "";
			ЭтаФорма[ТекущийЭлемент.Имя].Области[СтруктураРеквизитовФормы.мИдГруппы2 + "02" + Суффикс].ЦветФона = Новый Цвет();
			ЭтаФорма[ТекущийЭлемент.Имя].Области[СтруктураРеквизитовФормы.мИдГруппы2 + "02" + Суффикс].Защита = Истина;
		КонецЕсли;
		
	ИначеЕсли Лев(Область.Имя, 13) = "П000005000102" Тогда
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если Не РегламентированнаяОтчетность.ПроверитьРазделыНаНаличиеИнформации(ЭтаФорма, ДеревоВыгрузки, "ПолеТабличногоДокументаФормаОтчета", Неопределено, Истина)
		И ПараметрыВыгрузки["П000001000101"] = 2 И ПараметрыВыгрузки["П000001000201"] = 2
		И ПараметрыВыгрузки["П000002000101"] = 2 И ПараметрыВыгрузки["П000002000201"] = 2 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
	...
	// КОНТРОЛЬ показателей формы (Указания ФСН по заполнению Формы 1-ИП).
	// Если на вопрос 1 заполнен ответ «нет», то должен быть заполнен ответ на вопрос 1.1.
	Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000201"])
		И Не (ЗначениеЗаполнено(ПараметрыВыгрузки["П000101000101"]) Или ЗначениеЗаполнено(ПараметрыВыгрузки["П000101000201"])) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: если на вопрос 1 заполнен ответ «нет», то должен быть заполнен ответ на вопрос 1.1!",
			ИмяПоляТаблДокумента, "П000101000101");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если заполнен ответ на вопрос 3, то должен быть ответ «да» на вопрос 1.
	Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000003000101"])
		И Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000101"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: если заполнен ответ на вопрос 3, то должен быть ответ «да» на вопрос 1!",
			ИмяПоляТаблДокумента, "П000001000101");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Сумма данных по заполненным строкам графы 4.1 = 100.
	НомСтр = 1;
	ТекСумма = 0;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, "ПЧ00104000101_" + Формат(НомСтр, "ЧГ=")) Цикл
		ТекСумма = ТекСумма + ПараметрыВыгрузки["ПЧ00104000101_" + Формат(НомСтр, "ЧГ=")];
		НомСтр = НомСтр + 1;
	КонецЦикла;
	Если ТекСумма <> 100 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: Сумма данных по заполненным строкам графы 4.1 = 100!",
			ИмяПоляТаблДокумента, "П000004000103_1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если заполнена графа 4.1, то должен быть заполнен ответ на вопрос 3 и наоборот.
	Если (ТекСумма > 0 И Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000003000101"]))
		Или (ЗначениеЗаполнено(ПараметрыВыгрузки["П000003000101"]) И ТекСумма = 0) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: если заполнена графа 4.1, то должен быть заполнен ответ на вопрос 3 и наоборот!",
			ИмяПоляТаблДокумента, ?(ТекСумма > 0 И Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000003000101"]), "П000003000101", "П000004000103_1"));
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура ДополнительныеПоказатели1ИП(ТекСтруктура)
	...
	// Раздел 5.
	НомСтр = 1;
	Пока ТекСтруктура.Свойство("П000005000101_" + Формат(НомСтр, "ЧГ="), ТекЗначение) Цикл
		ТекСтрокаТЗ = СвойстваПоказателей.НайтиСтроки(Новый Структура("Показатель", "П000005000101"))[0].ТаблицаЗначений.НайтиСтроки(Новый Структура("Название", ТекЗначение));
		Если ТекСтрокаТЗ <> Неопределено Тогда
			Если ТекСтрокаТЗ.Количество() > 0 Тогда
				ТекСтруктура.Вставить("ПД00005000101_" + Формат(НомСтр, "ЧГ="), СокрЛП(ТекСтрокаТЗ[0].Код));
			Иначе
				ТекСтруктура.Вставить("ПД00005000101_" + Формат(НомСтр, "ЧГ="), "");
			КонецЕсли;
		Иначе
			ТекСтруктура.Вставить("ПД00005000101_" + Формат(НомСтр, "ЧГ="), "");
		КонецЕсли;
	...
	КонецЦикла;
	НомСтр = 1;
	Пока ТекСтруктура.Свойство("П000005000102_" + Формат(НомСтр, "ЧГ="), ТекЗначение) Цикл
		ТекСтрокаТЗ = СвойстваПоказателей.НайтиСтроки(Новый Структура("Показатель", "П000005000102"))[0].ТаблицаЗначений.НайтиСтроки(Новый Структура("Название", ТекЗначение));
		Если ТекСтрокаТЗ <> Неопределено Тогда
			Если ТекСтрокаТЗ.Количество() > 0 Тогда
				ТекСтруктура.Вставить("ПД00005000102_" + Формат(НомСтр, "ЧГ="), Число(ТекСтрокаТЗ[0].Код));
			Иначе
				ТекСтруктура.Вставить("ПД00005000102_" + Формат(НомСтр, "ЧГ="), 0);
			КонецЕсли;
		Иначе
			ТекСтруктура.Вставить("ПД00005000102_" + Формат(НомСтр, "ЧГ="), 0);
		КонецЕсли;
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок2", РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтаФорма, П0000050001, Ложь));
