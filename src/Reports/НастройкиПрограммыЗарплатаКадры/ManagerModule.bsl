...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	НастройкиВарианта.Описание =
		НСтр("ru = 'Сводный отчет по настройкам расчета зарплаты и кадрового учета.';
			|en = 'Summary report on settings of payroll and HR recordkeeping.'");
	...
	НастройкиВарианта.Описание =
		НСтр("ru = 'Отчет по отдельным разделам настроек расчета зарплаты и кадрового учета.';
			|en = 'Report on separate sections of payroll and HR recordkeeping settings.'");
...
Процедура ПодготовитьДанныеПоРазделуОрганизация(ТаблицаПоказателей)
	...
	ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСистемыНалогообложения.СрезПоследних()", "ПрименяетсяЕНВД", 
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
		...
		МодульОхранаТруда.ДобавитьПоказателиОхраныТруда(ТаблицаПоказателей);
	КонецЕсли;
...
Процедура ПодготовитьДанныеПоРазделуРасчетЗарплаты(ТаблицаПоказателей)
	...
	Если Не БазоваяВерсияКонфигурации Тогда
		...
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСтатистикиПерсонала", "ФормаЗПЗдрав", 
			"ЗП-Здрав (Сфера здравоохранения)", "Расчет зарплаты", "", "Ложь", 4, 1, 4);
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСтатистикиПерсонала", "ФормаЗПКультура", 
			"ЗП-Культура (Сфера культуры)", "Расчет зарплаты", "", "Ложь", 4, 1, 4);
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСтатистикиПерсонала", "ФормаЗПНаука", 
			"ЗП-Наука (Научные исследования и разработки)", "Расчет зарплаты", "", "Ложь", 4, 1, 4);
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСтатистикиПерсонала", "ФормаЗПОбразование", 
			"ЗП-Образование (Сфера образования)", "Расчет зарплаты", "", "Ложь", 4, 1, 4);
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСтатистикиПерсонала", "ФормаЗПСоц", 
			"ЗП-Соц (Сфера социального обслуживания)", "Расчет зарплаты", "", "Ложь", 4, 1, 4);
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСтатистикиПерсонала", "ФормаЗПФизическаяКультураИСпорт", 
			"ЗП-физическая культура и спорт (Область физической культуры и спорта)", "Расчет зарплаты", "", "Ложь", 4, 1, 4);
	Иначе
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСтатистикиПерсонала", "ИспользоватьОтчетностьМониторингаРаботниковСоциальнойСферы", 
			"Предприятие социальной сферы", "Расчет зарплаты", "Прочие настройки расчета зарплаты", "Ложь", 4, 7, 3);
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСтатистикиПерсонала", "ФормаЗПЗдрав", 
			"ЗП-Здрав (Сфера здравоохранения)", "Расчет зарплаты", "Прочие настройки расчета зарплаты", "Ложь", 4, 7, 4);
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСтатистикиПерсонала", "ФормаЗПКультура", 
			"ЗП-Культура (Сфера культуры)", "Расчет зарплаты", "Прочие настройки расчета зарплаты", "Ложь", 4, 7, 4);
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСтатистикиПерсонала", "ФормаЗПНаука", 
			"ЗП-Наука (Научные исследования и разработки)", "Расчет зарплаты", "Прочие настройки расчета зарплаты", "Ложь", 4, 7, 4);
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСтатистикиПерсонала", "ФормаЗПОбразование", 
			"ЗП-Образование (Сфера образования)", "Расчет зарплаты", "Прочие настройки расчета зарплаты", "Ложь", 4, 7, 4);
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСтатистикиПерсонала", "ФормаЗПСоц", 
			"ЗП-Соц (Сфера социального обслуживания)", "Расчет зарплаты", "Прочие настройки расчета зарплаты", "Ложь", 4, 7, 4);
		ДобавитьПоказатель(ТаблицаПоказателей, "РегистрСведений.НастройкиСтатистикиПерсонала", "ФормаЗПФизическаяКультураИСпорт", 
			"ЗП-физическая культура и спорт (Область физической культуры и спорта)", "Расчет зарплаты", "Прочие настройки расчета зарплаты", "Ложь", 4, 7, 4);
	КонецЕсли;
