...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1000002", 2, ,
		"Выбор кода вида транспорта", КоллекцияСписковВыбора["КодыВидаТС"]);
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы120 Тогда
		Подстрока = "контракт";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный контракт?';
							|en = 'Удалить выбранный контракт?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если ДлинаКодВидТС <> 2 Тогда
				...
				+ " задан код вида транспорта море/авиа", "Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			ИначеЕсли ПризнДекл = "1" ИЛИ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000004"]) Тогда
				...
				Если НЕ (ДлинаРегНомТД = 23 ИЛИ (ДлинаРегНомТД >= 25 И ДлинаРегНомТД <= 29)) Тогда
					...
					+ " задан регистрационный номер декларации на товары. Возможное количество символов в номере: 23 или от 25 до 29",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000005"]) Тогда
				...
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не задана дата вывоза припасов",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П10000.Данные["П1000005"]) Тогда
				...
				+ " задана дата вывоза припасов, выходящая за пределы периода с 01.01.1900 по 31.12.2099",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100008"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не задана дата транспортного,"
					+ " товаросопроводительного и (или) иного документа, подтверждающего вывоз припасов",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П11000.Данные["П1100008"]) Тогда
					ИмяПок = "П1100008" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " задана дата транспортного, товаросопроводительного и (или) иного документа, подтверждающего вывоз припасов,"
					+ " выходящая за пределы периода с 01.01.1900 по 31.12.2099", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Для каждого Ветка_П11100 Из ДанныеМнЧ_П11100 Цикл
					...
					Если НЕ ЗначениеЗаполнено(Ветка_П11100.Данные["П1110010"]) Тогда
						...
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " не задан код товара по товарной номенклатуре внешнеэкономической деятельности (ТН ВЭД) (по припасам)",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
			...
			Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200014"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана дата контракта/договора", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П12000.Данные["П1200014"]) Тогда
					ИмяПок = "П1200014" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " задана дата контракта/договора,"
					+ " выходящая за пределы периода с 01.01.1900 по 31.12.2099", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				// 12000.
				СведКонтрДог = ПолучитьПодчиненныйЭлемент(ЭкзСведОперРеал, "СведКонтрДог");
				ДанныеМнЧ_П12000 = Ветка_П10000.ДанныеМногострочныхЧастей.П12000.Строки;
				Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
					
					ЭкзСведКонтрДог = СкопироватьУзел(ЭкзСведОперРеал, СведКонтрДог, ЭкзСведОперРеал.Строки.Количество() - 1);
					Для каждого Эл Из ЭкзСведКонтрДог.Строки Цикл
						
						Если Эл.Код = "НомКонтрДог" Тогда
	...
	Если ВыбранныеСтраницы.Найти("Реестр") <> Неопределено Тогда
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				// Особая обработка данных П11000, П11100, П12000.
				//
				КолСтрок11_111 = 0;
				Строки11 = Строка1.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Строка11 Из Строки11 Цикл
					КолСтрок11_111 = КолСтрок11_111 + Строка11.ДанныеМногострочныхЧастей.П11100.Строки.Количество();
				КонецЦикла;
				
				Строки12 = Строка1.ДанныеМногострочныхЧастей.П12000.Строки;
				КолСтрок12 = Строки12.Количество();
				
				МаксКолСтрок11_111_12 = Макс(КолСтрок11_111, КолСтрок12);
				
				ИменаГраф11
				= СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы110]["ИменаГраф"];
				ИменаГраф111
				= СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы111]["ИменаГраф"];
				ИменаГраф12
				...
				// Подготовка данных П11000, П11100, П12000 перед выводом в печатную форму.
				Для Инд = 1 По МаксКолСтрок11_111_12 Цикл
					ДанныеДляВыводаСтрокиВПечФорму = Новый Структура;
					Если Инд <= КолСтрок12 Тогда
						Для каждого ИмяГрафы Из ИменаГраф12 Цикл
							ДанныеДляВыводаСтрокиВПечФорму.Вставить(ИмяГрафы, Строки12[Инд - 1].Данные[ИмяГрафы]);
						КонецЦикла;
					КонецЕсли;
					...
					Для каждого Строка111 Из Строки111 Цикл
						...
						Если Строки111.Индекс(Строка111) = 0 Тогда
							Для каждого ИмяГрафы Из ИменаГраф11 Цикл
								ДанныеДляВыводаСтрокиВПечФорму.Вставить(ИмяГрафы, Строка11.Данные[ИмяГрафы]);
							КонецЦикла;
						КонецЕсли;
						Для каждого ИмяГрафы Из ИменаГраф111 Цикл
							ДанныеДляВыводаСтрокиВПечФорму.Вставить(ИмяГрафы, Строка111.Данные[ИмяГрафы]);
						КонецЦикла;
						Если КолСтрок111 > 1 Тогда
							Если Строки111.Индекс(Строка111) = 0 Тогда
								ДанныеДляВыводаСтрокиВПечФорму.Вставить("ОблОбъедСтрока11_Верх");
							ИначеЕсли Строки111.Индекс(Строка111) = КолСтрок111 - 1 Тогда
								ДанныеДляВыводаСтрокиВПечФорму.Вставить("ОблОбъедСтрока11_Низ");
							КонецЕсли;
						КонецЕсли;
						Инд = Инд + 1;
					КонецЦикла;
				...
				// Вывод предварительно подготовленных данных П11000, П11100, П12000, в печатную форму.
				ОблОбъедСтрока11_Верх = Неопределено;
				ОблОбъедСтрока11_Низ  = Неопределено;
				Для Инд = 0 По МасДанныхДляВыводаСтрокВПечФорму.Количество() - 1 Цикл
					...
					Если ДанныеДляВыводаСтрокиВПечФорму.Свойство("ОблОбъедСтрока11_Верх") Тогда
						ОблОбъедСтрока11_Верх = ПечФорма1СтрокаДокумент.Верх;
					ИначеЕсли ДанныеДляВыводаСтрокиВПечФорму.Свойство("ОблОбъедСтрока11_Низ") Тогда
						ОблОбъедСтрока11_Низ = ПечФорма1СтрокаДокумент.Низ;
					КонецЕсли;
					
					Если ЗначениеЗаполнено(ОблОбъедСтрока11_Верх) И ЗначениеЗаполнено(ОблОбъедСтрока11_Низ) Тогда
...
Процедура Проверить(Команда)
	...
	|от 26.12.2023 № ЕД-7-15/1003@, будет
