...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Если РазделЗаполнен(ДанныеРеестра.Строки, Ложь) Тогда
		...
		ПроверитьКС_6_1_1(СтруктураРезультата);
		ПроверитьКС_6_1_2(СтруктураРезультата);
		ПроверитьКС_6_1_3(СтруктураРезультата);
		ПроверитьКС_6_1_4(СтруктураРезультата);
		ПроверитьКС_6_2_1(СтруктураРезультата);
		ПроверитьКС_6_2_2(СтруктураРезультата);
		ПроверитьКС_6_2_3(СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_6_1_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 6.1.1';
												|en = 'Проверка КС 6.1.1'"));
...
&НаСервере
Процедура ПроверитьКС_6_1_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 6.1.2';
												|en = 'Проверка КС 6.1.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_6_1_3(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		,НСтр("ru = 'Проверка КС 6.1.3';
													|en = 'Проверка КС 6.1.3'"));
	Заголовки.Вставить("Нарушение"			,НСтр("ru = 'Ошибка заполнения';
													|en = 'Ошибка заполнения'"));
	Заголовки.Вставить("ОписаниеКС"			,
		НСтр("ru = 'Если признак таможенного декларирования (графа 3 Реестра) = 1, то регистрационный ';
			|en = 'Если признак таможенного декларирования (графа 3 Реестра) = 1, то регистрационный '") +
		НСтр("ru = 'номер ДТ (графа 4 Реестра) должен быть указан';
			|en = 'номер ДТ (графа 4 Реестра) должен быть указан'"));
	Заголовки.Вставить("ШаблонОшибки"		,
		НСтр("ru = 'Не указан номер таможенной декларации при указанном признаке таможенного декларирования';
			|en = 'Не указан номер таможенной декларации при указанном признаке таможенного декларирования'"));
	...
	Для каждого СтраницаРаздела Из ДанныеРеестра.Строки Цикл
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл
			...
			Настройки.Вставить("ДопСведенияПараметры", Новый Структура("НомерСтраницы, НомерСтроки", НомерСтраницы, 
				НомерРасчетный));
			Показатели = ЗаполнитьПоказатели("НомерГТД, ПризнакДекларирования", ДанныеСтраницы[НомерСтроки - 1].Данные, 
				НомерСтроки);
				
			Если Не Показатели.ПризнакДекларирования.ЗначениеСтр = "1" Тогда
				Продолжить;
			КонецЕсли;
			...
			Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
				Показатели.НомерГТД.Значение);
			Настройки.ЭтоОшибка	= Не ЗначениеЗаполнено(Показатели.НомерГТД.Значение);
...
&НаСервере
Процедура ПроверитьКС_6_1_4(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		,НСтр("ru = 'Проверка КС 6.1.4';
													|en = 'Проверка КС 6.1.4'"));
	Заголовки.Вставить("Нарушение"			,НСтр("ru = 'п. 1 ст. 165 НК РФ';
													|en = 'п. 1 ст. 165 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			,
		НСтр("ru = 'Если регистрационный номер ДТ (графа 4 Реестра) указан, то он должен отвечать следующим требованиям:
		...
		|en = 'Если регистрационный номер ДТ (графа 4 Реестра) указан, то он должен отвечать следующим требованиям:
		|-должен состоять из 23 символов и иметь вид ""TTTTTTTT/DDMMGG/NNNNNNN"", / - косая черта вправо, разделитель показателей;
		|-DDMMGG – дата декларации на товары (день, месяц, год последние 2 цифры);
		|-GG - значение года оформления ДТ, должно быть не меньше значения текущего года уменьшенного на 4 и не больше текущего;
		|-NNNNNNN – порядковый номер декларации на товары для данного таможенного органа, должен быть больше 0'"));
...
&НаСервере
Процедура ПроверитьКС_6_2_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 6.2.1';
												|en = 'Проверка КС 6.2.1'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_6_2_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 6.2.2';
												|en = 'Проверка КС 6.2.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_6_2_3(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 6.2.3';
												|en = 'Проверка КС 6.2.3'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
