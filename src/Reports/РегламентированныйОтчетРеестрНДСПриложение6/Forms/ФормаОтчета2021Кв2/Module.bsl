...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр р.НДС прилож.06 21к2") Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1000003", 1, ,
		"Выбор признака таможенного декларирования", КоллекцияСписковВыбора["ПризнакиТамДекл"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1100006", 2, ,
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрТСДПр_6");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRDPRNDS"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRDPRNDS.2"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузкиРеестра, ДеревоВыгрузкиСведений)
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			ДлинаКодВидТС = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000002"]));
			Если ДлинаКодВидТС <> 2 Тогда
				...
				+ " задан цифровой код вида транспорта море/авиа",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ПризнДекл = СокрЛП(Ветка_П10000.Данные["П1000003"]);
			
			Если НЕ ЗначениеЗаполнено(ПризнДекл) Тогда
			...
			ИначеЕсли ПризнДекл = "1" ИЛИ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000004"]) Тогда
				
				ДлинаРегНомТД = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000004"]));
				Если НЕ (ДлинаРегНомТД = 23 ИЛИ (ДлинаРегНомТД >= 25 И ДлинаРегНомТД <= 29)) Тогда
					...
					+ " задан регистрационный номер таможенной декларации. Возможное количество символов в номере: 23 или от 25 до 29",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000005"]) Тогда
				...
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не задана дата отметки о вывозе припасов",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				ДлинаВидТрДок = СтрДлина(СокрЛП(Ветка_П11000.Данные["П1100006"]));
				Если ДлинаВидТрДок <> 2 Тогда
					...
					+ " задан вид транспортного, товаросопроводительного и (или) иного документа, подтверждающего вывоз припасов",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100008"]) Тогда
					...
					+ " не задана дата транспортного, товаросопроводительного и (или) иного документа, подтверждающего вывоз припасов",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Для каждого Ветка_П11100 Из ДанныеМнЧ_П11100 Цикл
					...
					Если НЕ ЗначениеЗаполнено(Ветка_П11100.Данные["П1110010"]) Тогда
						...
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомПП + " не задан код товара по товарной номенклатуре внешнеэкономической деятельности (ТН ВЭД) (по припасам)",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрТСДПр_6 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрТСДПр_6");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрТСДПр_6 = СкопироватьУзел(УзелРодитель.Родитель, РеестрТСДПр_6);
		Для каждого Эл Из ЭкзРеестрТСДПр_6.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		// Узел "СведОперРеал" имеется только в схеме выгрузки "Сведений".
		// Если он не найден, значит, сейчас формируются данные "Реестра"
		// (для выгрузки первого из 2-х файлов).
		СведОперРеал = ПолучитьПодчиненныйЭлемент(ЭкзРеестрТСДПр_6, "СведОперРеал");
		Если СведОперРеал = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведОперРеал = СкопироватьУзел(ЭкзРеестрТСДПр_6, СведОперРеал, ЭкзРеестрТСДПр_6.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСведОперРеал.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
				...
				// 11000.
				СвТрДок = ПолучитьПодчиненныйЭлемент(ЭкзСведОперРеал, "СвТрДок");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					
					ЭкзСвТрДок = СкопироватьУзел(ЭкзСведОперРеал, СвТрДок, ЭкзСведОперРеал.Строки.Количество() - 1);
					...
					// 11100.
					СведТовар = ПолучитьПодчиненныйЭлемент(ЭкзСвТрДок, "СведТовар");
					ДанныеМнЧ_П11100 = Ветка_П11000.ДанныеМногострочныхЧастей.П11100.Строки;
					Для каждого Ветка_П11100 Из ДанныеМнЧ_П11100 Цикл
						
						ЭкзСведТовар = СкопироватьУзел(ЭкзСвТрДок, СведТовар, ЭкзСвТрДок.Строки.Количество() - 1);
						Для каждого Эл Из ЭкзСведТовар.Строки Цикл
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_6");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_6_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Если ОблОбъедСтрока1_Верх <> ОблОбъедСтрока1_Низ Тогда
					...
					Для НомерКолонки = 2 По 14 Цикл
						Если НомерКолонки >= 7 И НомерКолонки <= 12 Тогда
							Продолжить;
						КонецЕсли;
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Реестр НДС Приложение 6");
...
Процедура ПроверкаКС_6_1_1()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '6.1.1';
													|en = '6.1.1'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 6';
														|en = 'Реестр 6'"));
...
&НаСервере
Процедура ПроверкаКС_6_1_3()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '6.1.3';
													|en = '6.1.3'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 6';
														|en = 'Реестр 6'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Если признак таможенного декларирования в гр.3 Реестра = 1, то регистрационный номер ГТД ';
			|en = 'Если признак таможенного декларирования в гр.3 Реестра = 1, то регистрационный номер ГТД '") +
		НСтр("ru = 'в гр.4 Реестра должен быть заполнен';
			|en = 'в гр.4 Реестра должен быть заполнен'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Признак декларирования %1, регистрационный номер ГТД %2';
													|en = 'Признак декларирования %1, регистрационный номер ГТД %2'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный номер ГТД';
														|en = 'Ошибочный номер ГТД'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001"		, "П1000003_");
	ИменаПоказателей.Вставить("Р01Г002"		, "П1000004_");
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			...
			Настройки.ЭтоОшибка = Показатели.Р01Г001.Значение = "1" И НЕ ЗначениеЗаполнено(Показатели.Р01Г002.Значение);
...
&НаСервере
Процедура ПроверкаКС_6_1_4а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '6.1.4.а';
													|en = '6.1.4.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 6';
														|en = 'Реестр 6'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_6_1_4б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '6.1.4.б';
													|en = '6.1.4.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 6';
														|en = 'Реестр 6'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_6_1_4в()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '6.1.4.в';
													|en = '6.1.4.в'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 6';
														|en = 'Реестр 6'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_6_2_1а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '6.2.1.а';
													|en = '6.2.1.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 6';
														|en = 'Реестр 6'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_6_2_1б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '6.2.1.б';
													|en = '6.2.1.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 6';
														|en = 'Реестр 6'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_6_2_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '6.2.2';
													|en = '6.2.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 6';
														|en = 'Реестр 6'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
