...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "12/10/2022";
...
Процедура СформироватьСпискиВыбора()
		...
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
			...
			Для НомСтр = ВерхОбласти По НизОбласти Цикл
				...
				Если КодПоказателя <> "###" Тогда
					...
					Если (Область.Имя = "КБК_Р1" ИЛИ Область.Имя = "КБК_Р2") И КодПоказателя = "18210302490011000110"
						И СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < '20250101' Тогда
						
						Продолжить;
						
					ИначеЕсли Область.Имя = "КодыВидаПТ_Р2_П2" И КодПоказателя = "737"
						И СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < '20250101' Тогда
						
						Продолжить;
						
					ИначеЕсли Область.Имя = "КодыПоказателя_Р21" И КодПоказателя = "10060"
						И СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < '20250101' Тогда
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2025 Тогда
		ИмяСекции_КодыПоказателя_Р23 = "КодыПоказателя_Р23_20250101";
	Иначе
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "П001050006003" И НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		...
		Если Год(ДатаВторогоПредшествующегоМесяца) <= 2026 Тогда
			
			СтандартнаяОбработка = Ложь;
			
			СписокКодов = Новый СписокЗначений;
			СписокКодов.Добавить(0, "Пустое значение                 ");
			Если Год(ДатаВторогоПредшествующегоМесяца) = 2023 Тогда
	...
	Если ИмяРаздела = "Раздел2" Тогда
		...
		Иначе
			...
			ИначеЕсли ГодДействия = 2024 Тогда
				...
				Ставка = ?(КодВидаПодакцизногоТовара = "510",     0, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "520",    58, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "531",   557, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "532",   557, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "533",   912, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "534",  1555, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "535",  1609, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "536",  1662, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "630", 10425, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "640",  6628, Ставка);
				
				Ставка = ?(КодВидаПодакцизногоТовара = "650", Окр(13100 + 4865 * 1), Ставка);
				
				Ставка = ?(КодВидаПодакцизногоТовара = "665", 15048, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "671",  2800, Ставка);
				...
				Ставка = ?(КодВидаПодакцизногоТовара = "676", 15458, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "680",  9000, Ставка);
			...
			ИначеЕсли ГодДействия = 2027 Тогда
				...
				Ставка = ?(КодВидаПодакцизногоТовара = "536",  1822, Ставка);
		...
		Для каждого Приложение Из СтруктураИсточник.Строки Цикл
			...
			Иначе
				...
				Для каждого СтраницаПриложения Из СтруктураИсточник.Строки Цикл
					...
					ДанныеСтраницы = Новый Структура;
					ДанныеСтраницы.Вставить("НомерСтраницы", СтруктураПриемник.Страница + "," +
...
Процедура СформироватьТаблицыОтчета()
	...
	ТаблицаР01.Колонки.Добавить("Р01С020"		, ТипСтрока);
	...
	ТаблицаР24.Колонки.Добавить("Р02С020"		, ТипСтрока);
	ТаблицаР24.Колонки.Добавить("Р24Г001"		, ТипСтрока5);
	ТаблицаР24.Колонки.Добавить("Р24Г012"		, ТипЧисло);
	ТаблицаР24.Колонки.Добавить("Р24Г022"		, ТипЧисло);
	...
	ТаблицаР03.Колонки.Добавить("Р03С030"		, ТипСтрока);
...
#Область Раздел_2
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел2 Цикл
		...
		Для Счетчик = 1 По СтраницаРаздела.КолвоСтрокП00024М1 Цикл
			...
			Если Показатели.Р24Г001.Значение = "40001" Тогда
				НоваяСтрока.Р24Г012			= Показатели.Р24Г002.Значение;
			Иначе
				НоваяСтрока.Р24Г022			= Показатели.Р24Г002.Значение;
			КонецЕсли;
			НоваяСтрока.НомерСтраницы	= СтраницаРаздела.НомерСтраницы;
			НоваяСтрока.НомерСтроки		= Счетчик;
...
#Область Раздел_3
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		...
		НоваяСтрока.Р03С030			= Показатели.Р03С030.Значение;
...
&НаСервере
Процедура ПроверкаКС_10_1_к()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 1';
														|en = 'Раздел 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '10.1.к';
													|en = '10.1.к'"));
	...
	Для каждого КБК_Р1 Из ДанныеКС.КБК_Р1 Цикл
		...
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
			Показатели.Р01С030.Итог, ЗначСтр(КБК_Р1));
...
&НаСервере
Процедура ПроверкаКС_10_2_к()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 1';
														|en = 'Раздел 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '10.2.к';
													|en = '10.2.к'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Сумма акциза в стр.030 Раздела 1 = (гр.2 подр. ';
			|en = 'Сумма акциза в стр.030 Раздела 1 = (гр.2 подр. '") + 
		НСтр("ru = '2.4 Раздела 2 по коду 40001 в гр.1 подр.2.4 Раздела 2 - гр.2 подр. 2.4 Раздела 2 ';
			|en = '2.4 Раздела 2 по коду 40001 в гр.1 подр.2.4 Раздела 2 - гр.2 подр. 2.4 Раздела 2 '") +
		НСтр("ru = 'по кодам 40002, 40004 в гр.1 подр.2.4 Раздела 2 + стр.110 Раздела 3) в разрезе КБК';
			|en = 'по кодам 40002, 40004 в гр.1 подр.2.4 Раздела 2 + стр.110 Раздела 3) в разрезе КБК'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = %2 (%3 - %4 + %5), КБК %6';
													|en = '%1 = %2 (%3 - %4 + %5), КБК %6'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная сумма акциза';
													|en = 'Возможно ошибочная сумма акциза'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru = 'Уточните сумму акциза в стр.030 Раздела 1';
													|en = 'Уточните сумму акциза в стр.030 Раздела 1'"));
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@';
													|en = 'Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@'")); 
	Заголовки.Вставить("Комментарий"		, НСтр("ru = 'Критическая проверка';
													|en = 'Критическая проверка'"));
	...
	НастройкиКС.Вставить("МассивКодов"	, СтрРазделить("40001,40002,40004", ","));

	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01С030"	, "П00001М103000_");
	ИменаПоказателей.Вставить("Р24Г001"	, "П00024М100001_");
	ИменаПоказателей.Вставить("Р24Г012"	, "П00024М100002_");
	ИменаПоказателей.Вставить("Р24Г022"	, "П00024М100002_");
	ИменаПоказателей.Вставить("Р03С110"	, "П000030011003");
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	...
	Для каждого КБК_Р1 Из ДанныеКС.КБК_Р1 Цикл
		...
		ЗарегистрироватьПроверку();
		Показатели.Р01С030.Итог = 0;
		Показатели.Р24Г012.Итог = 0;
		Показатели.Р24Г022.Итог = 0;
		Показатели.Р03С110.Итог = 0;
		
		ПараметрыОтбораР1 = Новый Структура;
		ПараметрыОтбораР1.Вставить("Р01С020", КБК_Р1);
		МассивСтрокР1 = ДанныеКС.ТаблицаР01.НайтиСтроки(ПараметрыОтбораР1);
		
		Если НЕ МассивСтрокР1.Количество() = 0 Тогда
			Для каждого СтрокаР1 Из МассивСтрокР1 Цикл
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01С030, СтрокаР1, СтрокаР1.НомерСтроки);
			КонецЦикла;
		КонецЕсли;
		
		ПараметрыОтбораР2 = Новый Структура;
		ПараметрыОтбораР2.Вставить("Р02С020", КБК_Р1);
		МассивСтрокР2 = ДанныеКС.ТаблицаР24.НайтиСтроки(ПараметрыОтбораР2);
		
		Для каждого СтрокаР2 Из МассивСтрокР2 Цикл 
			ЗаполнитьПоказатель(Показатели.Р24Г001, СтрокаР2, СтрокаР2.НомерСтроки);
			Если НастройкиКС.МассивКодов.Найти(Показатели.Р24Г001.Значение) = Неопределено Тогда 
				Продолжить;
			КонецЕсли;
			Если Показатели.Р24Г001.Значение = "40001" Тогда
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р24Г012, СтрокаР2, СтрокаР2.НомерСтроки);
			Иначе	
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р24Г022, СтрокаР2, СтрокаР2.НомерСтроки);
			КонецЕсли;
		КонецЦикла;
		
		ПараметрыОтбораР3 = Новый Структура;
		ПараметрыОтбораР3.Вставить("Р03С030", КБК_Р1);
		МассивСтрокР3 = ДанныеКС.ТаблицаР03.НайтиСтроки(ПараметрыОтбораР3);
		
		Для каждого СтрокаР3 Из МассивСтрокР3 Цикл 
			ЗаполнитьИРасшифроватьПоказатель(Показатели.Р03С110, СтрокаР3);
		КонецЦикла;
		
		РасчетнаяСумма = Макс(0, Показатели.Р24Г012.Итог + Показатели.Р24Г022.Итог + Показатели.Р03С110.Итог);
		НастройкиКС.ЭтоОшибка = НЕ Показатели.Р01С030.Итог = РасчетнаяСумма;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
			Показатели.Р01С030.Итог, РасчетнаяСумма, Показатели.Р24Г012.Итог, -Показатели.Р24Г022.Итог, 
			Показатели.Р03С110.Итог, ЗначСтр(КБК_Р1));
		ДополнитьТаблицуРезультата();
