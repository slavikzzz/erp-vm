...
Процедура СформироватьСпискиВыбора()
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2022 Тогда
		...
		ИмяСекции_КБК_Р1 = "КБК_Р1_20220101";
		ИмяСекции_КБК_Р2 = "КБК_Р2_20220101";
		
		ИмяСекции_КодыВидаПТ_Р2 = "КодыВидаПТ_Р2_20220101";
		
		ИмяСекции_КодыПоказателя_Р21 = "КодыПоказателя_Р21_20220101";
		ИмяСекции_КодыПоказателя_Р23 = "КодыПоказателя_Р23_20220101";
		
	Иначе
		
		ИмяСекции_КБК_Р1 = "КБК_Р1";
		ИмяСекции_КБК_Р2 = "КБК_Р2";
		
		ИмяСекции_КодыВидаПТ_Р2 = "КодыВидаПТ_Р2";
		
		ИмяСекции_КодыПоказателя_Р21 = "КодыПоказателя_Р21";
		ИмяСекции_КодыПоказателя_Р23 = "КодыПоказателя_Р23";
	...
	"Выбор кода бюджетной классификации", КоллекцияСписковВыбора[ИмяСекции_КБК_Р1]);
	
	//Раздел 2.
	//
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020001003", 3, ,
	"Выбор кода вида подакцизного товара", КоллекцияСписковВыбора[ИмяСекции_КодыВидаПТ_Р2]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020002003", 0, ,
	"Выбор кода бюджетной классификации", КоллекцияСписковВыбора[ИмяСекции_КБК_Р2]);
	//Подраздел 2.1.
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П00021М100001", 5, ,
	"Выбор кода показателя", КоллекцияСписковВыбора[ИмяСекции_КодыПоказателя_Р21]);
	//Подраздел 2.2.
	...
	"Выбор кода показателя", КоллекцияСписковВыбора[ИмяСекции_КодыПоказателя_Р23]);
	//Подраздел 2.4.
	...
	"Выбор кода вида подакцизного товара", КоллекцияСписковВыбора["КодыВидаПТ_П2"]);
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П001030002503", 1, ,
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00104М114003") И НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		...
		Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <= 2024 Тогда
			...
			Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 2021 Тогда
				СписокКодов.Добавить(53250, "53 250 рублей за 1 тонну");
	...
	ИначеЕсли Область.Имя = "П001050006003" И НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		...
		Если Год(ДатаВторогоПредшествующегоМесяца) <= 2024 Тогда
			
			СтандартнаяОбработка = Ложь;
			
			СписокКодов = Новый СписокЗначений;
			СписокКодов.Добавить(0, "Пустое значение                 ");
			Если Год(ДатаВторогоПредшествующегоМесяца) = 2020 Тогда
				СписокКодов.Добавить(48300, "48 300 рублей за 1 тонну");
			ИначеЕсли Год(ДатаВторогоПредшествующегоМесяца) = 2021 Тогда
				СписокКодов.Добавить(50700, "50 700 рублей за 1 тонну");
			ИначеЕсли Год(ДатаВторогоПредшествующегоМесяца) = 2022 Тогда
	...
	Если СтрНачинаетсяС(ИмяПоказателя, "П00001М103000")
		...
		Если ЗначениеЗаполнено(ЗначПоказателя) Тогда
			...
			Если МассивКодовСтрок.Количество() > 0 Тогда
				...
				"ru = 'Не допускается заполнение более одной из строк 030, 040, 050. %1';
				|en = 'Не допускается заполнение более одной из строк 030, 040, 050. %1'"), ПредстСтрок);
				Сообщение.Сообщить();
			КонецЕсли;
		КонецЕсли;
...
Функция ЗначенияОпределенныеПоВидуПодакцизногоТовара(КодВидаПодакцизногоТовара, ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел2" Тогда
		...
		ИначеЕсли КодВидаПодакцизногоТовара = "680" И ГодДействия >= 2022 Тогда
			КБК = "18210302420011000110";
		ИначеЕсли КодВидаПодакцизногоТовара = "681" И ГодДействия >= 2022 Тогда
			КБК = "18210302430011000110";
		ИначеЕсли КодВидаПодакцизногоТовара = "740" И ГодДействия >= 2022 Тогда
			КБК = "18210302440011000110";
		ИначеЕсли КодВидаПодакцизногоТовара = "741" И ГодДействия >= 2022 Тогда
		...
		Иначе
			...
			Если ГодДействия = 2021 Тогда
				
				Ставка = ?(КодВидаПодакцизногоТовара = "510",    0, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "520",   51, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "531",  491, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "532",  491, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "533",  804, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "534", 1370, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "535", 1418, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "536", 1464, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "630", 9188, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "640", 5841, Ставка);
				
				Ставка = ?(КодВидаПодакцизногоТовара = "650", Окр(13100 + 4865 * 0.500), Ставка);
				
				Ставка = ?(КодВидаПодакцизногоТовара = "665", 13262, Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "671",  2800, Ставка);
				
				Ставка = ?(КодВидаПодакцизногоТовара = "672", Окр(2800 + 774 * 0.500), Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "673", Окр(2800 + 774 * 0.500), Ставка);
				Ставка = ?(КодВидаПодакцизногоТовара = "674", Окр(2800 + 774 * 0.500), Ставка);
				
				Ставка = ?(КодВидаПодакцизногоТовара = "676", 13624, Ставка);
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Пока Обл <> Неопределено Цикл
					...
					Если КодР24 = "40001" ИЛИ КодР24 = "40002" ИЛИ КодР24 = "40004" Тогда
						...
						КодыР22ДляСуммирования.Добавить("20016");
	...
	ИначеЕсли ИмяРаздела = "Приложение5" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				ТДО["П001050003003"].Значение = ТДО["П001050005003"].Значение - ТДО["П001050006003"].Значение;
				
				Если ТДО["П001050003003"].Значение > 0 Тогда
					ТДО["П001050002003"].Значение = "1";
				ИначеЕсли ТДО["П001050003003"].Значение < 0 Тогда
				...
				Пока ОблСтр150 <> Неопределено Цикл
					...
					СтавкаУмножНаОбъем = ТДО["П001050007003"].Значение * ТДО["П00105М114003_" + ИндексСтр].Значение;
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Приложение5" Тогда
		...
		Ветка.Данные.Вставить("П001050003003", Ветка.Данные["П001050005003"] - Ветка.Данные["П001050006003"]);
		
		Если Ветка.Данные["П001050003003"] > 0 Тогда
			Ветка.Данные.Вставить("П001050002003", "1");
		ИначеЕсли Ветка.Данные["П001050003003"] < 0 Тогда
		...
		Для каждого СтрокаМнЧ Из ДеревоМнЧП5М1.Строки Цикл
			...
			СтавкаУмножНаОбъем = Ветка.Данные["П001050007003"] * СтрокаМнЧ.Данные["П00105М114003"];
...
Процедура СформироватьРаздел1(ТекстСообщения = "")
	...
	Если ТЗ.Количество() > 0 Тогда
		...
		Для Инд = 0 По ТЗ.Количество() - 1 Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТЗ.Стр040) И ЗначениеЗаполнено(СтрокаТЗ.Стр050) Тогда
				
				// Отчет заполнен некорректно.
				ТекстСообщения = НСтр(
				"ru='Раздел 1 заполнен некорректно (не допускается заполнение более одной из строк 030, 040, 050)."
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Для каждого Ветка Из ДеревоРаздела.Строки Цикл
		...
		Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
			...
			ИначеЕсли ЗначениеЗаполнено(СтрокаМнЧ.Данные["П00001М104000"])
				И ЗначениеЗаполнено(СтрокаМнЧ.Данные["П00001М105000"]) Тогда
				ИмяЗаполненногоПоказателя = "П00001М105000";
			КонецЕсли;
			Если ЗначениеЗаполнено(ИмяЗаполненногоПоказателя) Тогда
				СообщитьВТС("Раздел 1 -> Страница " + НомСтраницы + ": в строке " + НомСтроки
				+ " не допускается заполнение более одной из строк 030, 040, 050",
	...
	ВозмКодыСтр025 = ВозможныеЗначенияИзСпискаВыбораПоказателя("П001030002503");
	...
	Для каждого Ветка Из ДеревоРаздела.Строки Цикл
		...
		Если ЗаполненаСтраницаРаздела(Ветка, "Приложение3") Тогда
			...
			ЗначПок = СокрЛП(Ветка.Данные["П001030002503"]);
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("Приложение 3 -> Страница " + НомСтраницы
				+ ": не задан признак свидетельства", "Приложение3", "П001030002503", НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ВозмКодыСтр025.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
				СообщитьВТС("Приложение 3 -> Страница " + НомСтраницы
				+ ": признак свидетельства должен принимать одно из значений: "
				+ ВозмКодыСтр025.СтрокаЗначений, "Приложение3", "П001030002503", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(СокрЛП(Ветка.Данные["П001030003003"])) Тогда
				СообщитьВТС("Приложение 3 -> Страница " + НомСтраницы + ": не задана серия свидетельства",
				"Приложение3", "П001030003003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(СокрЛП(Ветка.Данные["П001030003004"])) Тогда
				СообщитьВТС("Приложение 3 -> Страница " + НомСтраницы + ": не задан номер свидетельства",
				"Приложение3", "П001030003004", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
		...
		Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
			...
			Для каждого Эл Из ЭкзСумНалПУКод.Строки Цикл
				Если Эл.Код = "АкцизПУ"
					...
					И СтрокаМнЧ.Данные["П00001М105000"] = 0 Тогда
					Эл.Обязательность = "О";
				КонецЕсли;
...
Процедура СформироватьСлужебныеДанные()
	...
	МассивПолейСписочныхЗначений.Добавить("П001030002503");
...
Процедура ПроверкаКС_7_04()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.80 НК РФ';
													|en = 'ст.80 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Значение кода признака свидетельства в стр.025 Приложения 3 должно соответствовать списку ';
			|en = 'Значение кода признака свидетельства в стр.025 Приложения 3 должно соответствовать списку '") +
		НСтр("ru = 'допустимых значений';
			|en = 'допустимых значений'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 в списке допустимых значений';
													|en = '%1 в списке допустимых значений'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("П03С025"		, "П001030002503");
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	Список	= ДанныеКС.СоответствиеСписков.Получить(Показатели.П03С025.ИмяПоказателя);
	...
	Если НЕ Список = Неопределено Тогда
		Для каждого СтраницаРаздела Из ДанныеКС.Приложение3 Цикл
			...
			Настройки.ЭтоОшибка = Список.Получить(Показатели.П03С025.Значение) = Неопределено;
			Настройки.Расшифровка = СтрЗаменить(Настройки.ШаблонРасшифровки, "%1", Показатели.П03С025.ЗначениеСтр);
			ДополнитьСтруктуруОшибок(Настройки);
		КонецЦикла;
	КонецЕсли;
...
Процедура ПроверкаКС_7_05()
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("П03С031"		, "П001030003003");
	ИменаПоказателей.Вставить("П03С032"		, "П001030003004");
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Приложение3 Цикл
		...
		Настройки.ЭтоОшибка = СтраницаРаздела.РазделЗаполнен И НЕ ЗначениеЗаполнено(Показатели.П03С031.Значение)
			И НЕ ЗначениеЗаполнено(Показатели.П03С032.Значение);
		Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			Показатели.П03С031.ЗначениеСтр, Показатели.П03С032.ЗначениеСтр);
		ДополнитьСтруктуруОшибок(Настройки);
	КонецЦикла;
...
Процедура ПроверкаКС_8_11()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.80 НК РФ';
													|en = 'ст.80 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма акциза в стр.090 Приложения 4 = стр.080 Приложения 4 х К';
			|en = 'Сумма акциза в стр.090 Приложения 4 = стр.080 Приложения 4 х К'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 х %3 Итог [%4]';
													|en = '%1 = %2 х %3 Итог [%4]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочная сумма акциза';
														|en = 'Ошибочная сумма акциза'"));
...
Процедура ПроверкаКС_9_16()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.80 НК РФ';
													|en = 'ст.80 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма вычета в стр.150 Приложения 5 = (стр.070 х стр.140 Приложения 5 х 2) + стр.100 Приложения 5, ';
			|en = 'Сумма вычета в стр.150 Приложения 5 = (стр.070 х стр.140 Приложения 5 х 2) + стр.100 Приложения 5, '") +
...
Процедура ПроверкаКС_9_17()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.80 НК РФ';
													|en = 'ст.80 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма вычета в стр.150 Приложения 5 = (стр.070 х стр.140 Приложения 5) + стр.100 Приложения 5, ';
			|en = 'Сумма вычета в стр.150 Приложения 5 = (стр.070 х стр.140 Приложения 5) + стр.100 Приложения 5, '") +
...
Процедура ПроверкаКС_9_17_код(Настройки)
	...
	Для Счетчик = 1 По ДанныеКС.Приложение5.КолвоСтрокП00105М1 Цикл
		ЗарегистрироватьФормулу(Настройки);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.П05С080, ДанныеКС.Приложение5, Счетчик);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.П05С150, ДанныеКС.Приложение5, Счетчик);
		РасшифроватьПоказатель(Показатели.П05С070);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.П05С140, ДанныеКС.Приложение5, Счетчик);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.П05С100, ДанныеКС.Приложение5, Счетчик);
		
		РасчетнаяСумма = Окр(Показатели.П05С070.Значение * Показатели.П05С140.Значение * Настройки.Кф + 
		...
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			Показатели.П05С150.Значение, Показатели.П05С070.Значение, Показатели.П05С140.Значение, Показатели.П05С100.Значение,
...
Процедура ПроверкаКС_9_18()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.80 НК РФ';
													|en = 'ст.80 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма вычета в стр.150 Приложения 5 = (стр.070 х стр.140 Приложения 5 х 2) + стр.110 + стр.120 ';
			|en = 'Сумма вычета в стр.150 Приложения 5 = (стр.070 х стр.140 Приложения 5 х 2) + стр.110 + стр.120 '") +
...
Процедура ПроверкаКС_9_19()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.80 НК РФ';
													|en = 'ст.80 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма вычета в стр.150 Приложения 5 = (стр.070 х стр.140 Приложения 5) + стр.110 + стр.120 ';
			|en = 'Сумма вычета в стр.150 Приложения 5 = (стр.070 х стр.140 Приложения 5) + стр.110 + стр.120 '") +
...
Процедура ПроверкаКС_9_19_код(Настройки)
	...
	Для Счетчик = 1 По ДанныеКС.Приложение5.КолвоСтрокП00105М1 Цикл
		...
		РасшифроватьПоказатель(Показатели.П05С070);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.П05С140, ДанныеКС.Приложение5, Счетчик);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.П05С110, ДанныеКС.Приложение5, Счетчик);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.П05С120, ДанныеКС.Приложение5, Счетчик);
		...
		Настройки.ЭтоОшибка = Показатели.П05С080.Значение = Настройки.КодПоказателя 
			И НЕ Показатели.П05С150.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			Показатели.П05С150.Значение, Показатели.П05С070.Значение, Показатели.П05С140.Значение, Показатели.П05С110.Значение,
...
Процедура ПроверкаКС_9_20()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.80 НК РФ';
													|en = 'ст.80 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма вычета в стр.150 Приложения 5 = (стр.070 х стр.140 Приложения 5 х 2) + стр.130 Приложения 5, ';
			|en = 'Сумма вычета в стр.150 Приложения 5 = (стр.070 х стр.140 Приложения 5 х 2) + стр.130 Приложения 5, '") +
...
Процедура ПроверкаКС_9_21()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.80 НК РФ';
													|en = 'ст.80 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма вычета в стр.150 Приложения 5 = (стр.070 х стр.140 Приложения 5) + стр.130 Приложения 5, ';
			|en = 'Сумма вычета в стр.150 Приложения 5 = (стр.070 х стр.140 Приложения 5) + стр.130 Приложения 5, '") +
...
Процедура ПроверкаКС_9_21_код(Настройки)
	...
	Для Счетчик = 1 По ДанныеКС.Приложение5.КолвоСтрокП00105М1 Цикл
		...
		РасшифроватьПоказатель(Показатели.П05С070);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.П05С140, ДанныеКС.Приложение5, Счетчик);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.П05С130, ДанныеКС.Приложение5, Счетчик);
		...
		Настройки.ЭтоОшибка = Показатели.П05С080.Значение = Настройки.КодПоказателя 
			И НЕ Показатели.П05С150.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			Показатели.П05С150.Значение, Показатели.П05С070.Значение, Показатели.П05С140.Значение, Показатели.П05С130.Значение,
			РасчетнаяСумма, Показатели.П05С080.ЗначениеСтр);
		ДополнитьСтруктуруОшибок(Настройки);
	КонецЦикла; 
...
Процедура ПроверкаКС_9_22()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.80 НК РФ';
													|en = 'ст.80 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма вычета в стр.150 Приложения 5 = стр.070 х стр.140 Приложения 5 х 2, ';
			|en = 'Сумма вычета в стр.150 Приложения 5 = стр.070 х стр.140 Приложения 5 х 2, '") +
...
Процедура ПроверкаКС_9_23()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.80 НК РФ';
													|en = 'ст.80 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма вычета в стр.150 Приложения 5 = стр.070 х стр.140 Приложения 5, ';
			|en = 'Сумма вычета в стр.150 Приложения 5 = стр.070 х стр.140 Приложения 5, '") +
...
Процедура ПроверкаКС_9_23_код(Настройки)
	...
	Для Счетчик = 1 По ДанныеКС.Приложение5.КолвоСтрокП00105М1 Цикл
		...
		Настройки.ЭтоОшибка = Показатели.П05С080.Значение = Настройки.КодПоказателя 
			И НЕ Показатели.П05С150.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			Показатели.П05С150.Значение, Показатели.П05С070.Значение, Показатели.П05С140.Значение,
			РасчетнаяСумма, Показатели.П05С080.ЗначениеСтр);
		ДополнитьСтруктуруОшибок(Настройки);
	КонецЦикла; 
