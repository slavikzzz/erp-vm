...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "06/12/2013";
	// Код отчета, равен коду по ОКУД.
	СтруктураРеквизитовФормы.мКодОтчета   = "0601010";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	// Гр. 7 = сумме гр. (8, 9) по строкам 1-11.
	Для НомСтр = 1 По 11 Цикл
		ТекСумма = 0;
		Для НомКол = 8 По 9 Цикл
			ТекСумма = ТекСумма + мФормаОтчета.Области["П000001"
			+ Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		мФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "07"].Значение = ТекСумма;
	КонецЦикла;
	
	// Гр. 3 = сумме гр. (4, 5, 6, 7) по строкам 1-11.
	Для НомСтр = 1 По 11 Цикл
		ТекСумма = 0;
		Для НомКол = 4 По 7 Цикл
			...
			+ Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		мФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Значение = ТекСумма;
	КонецЦикла;
	
	// Стр. 14 = стр. 12 - стр. 13 по всем графам.
	Для НомКол = 3 По 9 Цикл
		мФормаОтчета.Области["П0000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
		= мФормаОтчета.Области["П0000010012" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
		- мФормаОтчета.Области["П0000010013" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого Обл Из ОбластиСтроки Цикл
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-администрация");
	КонецЦикла;
