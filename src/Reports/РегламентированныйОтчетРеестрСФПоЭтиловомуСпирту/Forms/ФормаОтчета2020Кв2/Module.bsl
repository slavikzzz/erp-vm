...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "14/06/2020";
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	// Группа П12100.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П1210011");
	ИменаГраф.Добавить("П1210012");
	ИменаГраф.Добавить("П1210013");
	ИменаГраф.Добавить("П1210014");
	ИменаПодчиненныхГрупп = Новый Массив;
	СтруктураМногострочныхЧастей.Вставить(
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П1000001") Тогда
		
		СтандартнаяОбработка = Ложь;
		
		Сп = Новый СписокЗначений;
		Сп.Добавить("",    "Пустое значение");
		Сп.Добавить("111", "111 - этиловый спирт из пищевого сырья денатурированный");
		Сп.Добавить("121", "121 - этиловый спирт из непищевого сырья денатурированный");
		Сп.Добавить("160", "160 - этиловый спирт (за исключением этилового спирта-сырца, а также спирта денатурированного) из пищевого сырья");
		Сп.Добавить("170", "170 - этиловый спирт (за исключением этилового спирта-сырца, а также спирта денатурированного) из всех видов сырья, за исключением пищевого");
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	ИначеЕсли ИмяПоказателя = "ИНН1" Тогда
		...
		Иначе
			...
			|en = 'ИНН должен состоять из 10 или 12 цифр. Значение очищено.'");
			Сообщение.Сообщить();
		КонецЕсли;
	ИначеЕсли ИмяПоказателя = "КПП1" Тогда
		...
		Иначе
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр(
			"ru = 'КПП должен состоять из 9 символов. Значение очищено.';
...
Функция ДополнительныйТекстПриДобавленииУдаленииОтключенииСтрок(Знач ИмяГруппы)
	...
	Если ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы100 Тогда
		...
		ТекстВопросаПередУдалениемСтроки = НСтр("ru = 'Удалить выбранный счет-фактуру?';
												|en = 'Удалить выбранный счет-фактуру?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы110 Тогда
		...
		ТекстВопросаПередУдалениемСтроки = НСтр("ru = 'Удалить выбранное исправление в счете-фактуре?';
												|en = 'Удалить выбранное исправление в счете-фактуре?'");
		ТекстВопросаПередОтключениемСтроки = НСтр("ru = 'Будут удалены исправления в счете-фактуре.
		|
		|Продолжить?';
		|en = 'Будут удалены исправления в счете-фактуре.
		|
		|Продолжить?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы120 Тогда
		...
		ТекстВопросаПередУдалениемСтроки = НСтр("ru = 'Удалить выбранный корректировочный счет-фактуру?';
												|en = 'Удалить выбранный корректировочный счет-фактуру?'");
		ТекстВопросаПередОтключениемСтроки = НСтр("ru = 'Будут удалены корректировочные счета-фактуры.
		|
		|Продолжить?';
		|en = 'Будут удалены корректировочные счета-фактуры.
		|
		|Продолжить?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы121 Тогда
		...
		ТекстВопросаПередУдалениемСтроки = НСтр("ru = 'Удалить выбранное исправление в корректировочном счете-фактуре?';
												|en = 'Удалить выбранное исправление в корректировочном счете-фактуре?'");
		ТекстВопросаПередОтключениемСтроки = НСтр("ru = 'Будут удалены исправления в корректировочном счете-фактуре.
		|
		|Продолжить?';
		|en = 'Будут удалены исправления в корректировочном счете-фактуре.
		|
		|Продолжить?'");
	КонецЕсли;
	...
	Если ТабличныйДокумент.Области.Найти(ИмяПервойГрафы + Координаты + "_1") = Неопределено Тогда
		...
		Для каждого ИмяГрафы Из МассивИменГраф Цикл
			Если ИмяГрафы = "П1000001"
				...
				ИЛИ ИмяГрафы = "П1200008"
				ИЛИ ИмяГрафы = "П1210012" Тогда
				Данные.Вставить(ИмяГрафы, "");
			ИначеЕсли ИмяГрафы = "П1000003"
				ИЛИ ИмяГрафы = "П1100006"
				ИЛИ ИмяГрафы = "П1200009"
				ИЛИ ИмяГрафы = "П1210013" Тогда
...
Процедура ВывестиНаПечатьОтчет(СтруктураПараметров, ВыбранныеСтраницы, СтрПоля = Неопределено)
	...
	ИмяМакета = "ПечатныйБланк2020Кв2";
	...
	СпиртИзПищевогоСырьяДенатур   = ПечФорма.Получитьобласть("СпиртИзПищевогоСырьяДенатур");
	СпиртИзНепищевогоСырьяДенатур = ПечФорма.Получитьобласть("СпиртИзНепищевогоСырьяДенатур");
	СпиртИзПищевогоСырья          = ПечФорма.Получитьобласть("СпиртИзПищевогоСырья");
	СпиртИзНепищевогоСырья        = ПечФорма.Получитьобласть("СпиртИзНепищевогоСырья");
	СпиртПусто                    = ПечФорма.Получитьобласть("СпиртПусто");
	
	ШапкаПусто = ПечФорма.Получитьобласть("ШапкаПусто");
	...
	Для каждого СтраницаРеестра Из ДеревоДанныхРаздела.Строки Цикл
		...
		Если СтраницаРеестраИндекс = 0 Тогда
			...
			ЕстьЗаполненныйОбъемСпирта = Ложь;
			
			Если ЗначениеЗаполнено(СтруктураДанныхТитульный.СпиртИзПищевогоСырьяДенатур) Тогда
				СпиртИзПищевогоСырьяДенатур.Параметры.Заполнить(СтруктураДанныхТитульный);
				ПечФорма.Вывести(СпиртИзПищевогоСырьяДенатур);
				ЕстьЗаполненныйОбъемСпирта = Истина;
			КонецЕсли;
			Если ЗначениеЗаполнено(СтруктураДанныхТитульный.СпиртИзНепищевогоСырьяДенатур) Тогда
				СпиртИзНепищевогоСырьяДенатур.Параметры.Заполнить(СтруктураДанныхТитульный);
				ПечФорма.Вывести(СпиртИзНепищевогоСырьяДенатур);
				ЕстьЗаполненныйОбъемСпирта = Истина;
			КонецЕсли;
			Если ЗначениеЗаполнено(СтруктураДанныхТитульный.СпиртИзПищевогоСырья) Тогда
				СпиртИзПищевогоСырья.Параметры.Заполнить(СтруктураДанныхТитульный);
				ПечФорма.Вывести(СпиртИзПищевогоСырья);
				ЕстьЗаполненныйОбъемСпирта = Истина;
			КонецЕсли;
			Если ЗначениеЗаполнено(СтруктураДанныхТитульный.СпиртИзНепищевогоСырья) Тогда
				СпиртИзНепищевогоСырья.Параметры.Заполнить(СтруктураДанныхТитульный);
				ПечФорма.Вывести(СпиртИзНепищевогоСырья);
				ЕстьЗаполненныйОбъемСпирта = Истина;
			КонецЕсли;
			
			Если НЕ ЕстьЗаполненныйОбъемСпирта Тогда
				ПечФорма.Вывести(СпиртПусто);
			КонецЕсли;
			
			ПечФорма.Вывести(ШапкаПусто);
		...
		Для каждого СтрокаУр0 Из СтраницаРеестра.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы100].Строки Цикл
			...
			//
			ШапкаДокумент.Параметры.П1000001 = СтрокаУр0.Данные["П1000001"];
			//
			...
			Для каждого ПодчСтрокаВторойМнЧУр1 Из СтрокаУр0.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы120].Строки Цикл
				...
				Для каждого ПодчСтрокаМнЧУр2 Из ПодчСтрокаВторойМнЧУр1.ДанныеМногострочныхЧастей[
					...
					Если СтруктураЗаполнена(ПодчСтрокаМнЧУр2.Данные) Тогда
						...
						ОбщегоНазначения.СкопироватьРекурсивно(ПодчСтрокаМнЧУр2.Данные));
					КонецЕсли;
				КонецЦикла;
			...
			Если МассивДанныхИсправленийКоррСФ.Количество() = 0 Тогда
				МассивДанныхИсправленийКоррСФ.Добавить(ОбщегоНазначения.СкопироватьРекурсивно(
			...
			Для каждого ПодчСтрокаПервойМнЧУр1 Из СтрокаУр0.ДанныеМногострочныхЧастей[
				...
				ОбщегоНазначения.СкопироватьРекурсивно(ПодчСтрокаПервойМнЧУр1.Данные));
			КонецЦикла;
