...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КолКолонокИдГруппыТовары     = 13;
	КолКолонокИдГруппыУпаковка   = 14;
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли (Не СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда 
		...
		ИначеЕсли (Область.Имя = "П100" Или  Область.Имя = "П200") Тогда
			СтандартнаяОбработка = Ложь;
...
Функция УниверсальнаяВыгрузкаXMLПроверкаДанныхНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	Если ЗначениеЗаполнено(Доверенность) Тогда
		...
		Приложение.ТипФайлаВыгрузки = "РАСЧЕТЭКОСБОР.ПРИЛОЖЕНИЕ";
...
Процедура ЗаполнитьТовары(Узел)
	...
	Для Каждого Стр Из ТаблицаСтраницыТовары Цикл 
		...
		П100 = СтрЗаменить(СтрЗаменить(Данные.П100, "Группа N", ""), " ", "");
		Если Не ЗначениеЗаполнено(ОТЧ.ПривестиЗначение(П100)) Тогда 
			Продолжить;
		КонецЕсли;
		...
		Для Каждого Стр2 Из ДанныеДопСтрок Цикл 
			...
			utilization_product = НовУзел_product;
			product_group = РегламентированнаяОтчетность.ПолучитьПодчиненныйЭлемент(utilization_product, "product_group");
			...
			okpd2 = РегламентированнаяОтчетность.ПолучитьПодчиненныйЭлемент(utilization_product, "okpd2");
			...
			tnved = РегламентированнаяОтчетность.ПолучитьПодчиненныйЭлемент(utilization_product, "tnved");
			...
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(utilization_product, "qty_all_product", Стр2.П1100006);
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(utilization_product, "qty_total", Стр2.П1100008);
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(utilization_product, "qty_waste", Стр2.П1100009);
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(utilization_product, "qty_for_payment", Стр2.П1100010);
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(НовУзел_product, "sum", Стр2.П1100012);
		КонецЦикла;
	КонецЦикла;
...
Процедура ЗаполнитьУпаковка(Узел)
	...
	Для Каждого Стр Из ТаблицаСтраницыУпаковка Цикл 
		...
		П200 = СтрЗаменить(СтрЗаменить(Данные.П200, "Группа N", ""), " ", "");
		Если Не ЗначениеЗаполнено(ОТЧ.ПривестиЗначение(П200)) Тогда 
			Продолжить;
		КонецЕсли;
		...
		Для Каждого Стр2 Из ДанныеДопСтрок Цикл 
			...
			utilization_package = НовУзел_package;
			product_group = РегламентированнаяОтчетность.ПолучитьПодчиненныйЭлемент(utilization_package, "product_group");
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(product_group, "number", П200);
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(product_group, "group_name", ИмяГруппы);
			
			okpd2 = РегламентированнаяОтчетность.ПолучитьПодчиненныйЭлемент(utilization_package, "okpd2");
			Если ЗначениеЗаполнено(Стр2.П2100003) Тогда 
				РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(okpd2, "type", "object");
				РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(okpd2, "ref", "Rpn.Services.Nsi.Models.Okpd2");
				РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(okpd2, "readonly", "true");
				РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(okpd2, "code", Стр2.П2100003);
			Иначе
				РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(okpd2, "type", "null");
				РегламентированнаяОтчетность.УдалитьУзел(РегламентированнаяОтчетность.ПолучитьПодчиненныйЭлемент(okpd2, "ref"));
				РегламентированнаяОтчетность.УдалитьУзел(РегламентированнаяОтчетность.ПолучитьПодчиненныйЭлемент(okpd2, "readonly"));
			...
			tnved = РегламентированнаяОтчетность.ПолучитьПодчиненныйЭлемент(utilization_package, "tnved");
			Если ЗначениеЗаполнено(Стр2.П2100005) Тогда 
				РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(tnved, "type", "object");
				РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(tnved, "ref", "Rpn.Services.Nsi.Models.Tnved");
				РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(tnved, "readonly", "true");
				РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(tnved, "code", Стр2.П2100005);
			Иначе
				РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(tnved, "type", "null");
				РегламентированнаяОтчетность.УдалитьУзел(РегламентированнаяОтчетность.ПолучитьПодчиненныйЭлемент(tnved, "ref"));
				РегламентированнаяОтчетность.УдалитьУзел(РегламентированнаяОтчетность.ПолучитьПодчиненныйЭлемент(tnved, "readonly"));
			...
			material = РегламентированнаяОтчетность.ПолучитьПодчиненныйЭлемент(utilization_package, "material");
			Если ЗначениеЗаполнено(Стр2.П2100013) Тогда 
				...
				РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(material, "code", Стр2.П2100013);
			Иначе
			...
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(utilization_package, "qty_all_product", Стр2.П2100006);
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(utilization_package, "qty_total", Стр2.П2100008);
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(utilization_package, "qty_waste", Стр2.П2100009);
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(utilization_package, "qty_for_payment", Стр2.П2100010);
			РегламентированнаяОтчетность.УстановитьЗначениеЭлемента(НовУзел_package, "sum", Стр2.П2100012);
		КонецЦикла;
	КонецЦикла;
...
Функция СформироватьСтруктуруПараметров()
	...
	Возврат ПараметрыВыгрузки;КонецФункции
