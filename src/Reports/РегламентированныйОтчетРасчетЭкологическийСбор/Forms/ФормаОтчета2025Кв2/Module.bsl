...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2025Кв2";
	...
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппыТовУпакНеИсп, 12);
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппыТовУпакИсп, 14);
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "22/01/2025";
	СтруктураРеквизитовФормы.мКодОтчета = "РПН_30.12.2024_1990_Прил2";
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ТабДок = ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаОтчета2025Кв2_ПФ");
	...
	Для Каждого Стр Из ТаблицаСтраницыТовУпакИсп Цикл
		...
		Для Каждого ДДС Из Стр.ДанныеДопСтрок Цикл 
			Для Каждого СтрМнг Из ДанныеФормыВЗначение(ДДС.Значение.П21000, Тип("ТаблицаЗначений")) Цикл
				Инд = Инд + 1;
				Обл1 = ТабДок.ПолучитьОбласть("ТовУпакИсп_Строка");
				СтруктураВобластиМакета(Обл1.Области, ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтрМнг));
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено
		...
		Комментарий = "Приложение № 2. Расчет суммы экологического сбора по товарам, упаковке,"
			+ " ввезенным из государств, не являющихся членами Евразийского экономического союза";
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницы = "", ОбластьИмя = "") Экспорт
	...
	Если СтрНачинаетсяС(ОбластьИмя, "П1100009_") Или СтрНачинаетсяС(ОбластьИмя, "П1100007_")  Или СтрНачинаетсяС(ОбластьИмя, "П1100008_") Тогда 
		...
		О11 = Области.Найти("П1100011_" + СредОбластьИмя);
		
		О10.Значение = О8.Значение * О9.Значение;
		О11.Значение = (О10.Значение * О7.Значение) / 1000.0;
	...
	ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П2100007_") Или СтрНачинаетсяС(ОбластьИмя, "П2100008_")
		Или СтрНачинаетсяС(ОбластьИмя, "П2100010_") Или СтрНачинаетсяС(ОбластьИмя, "П2100011_") Тогда 
		О1 = Области.Найти("П2100007_" + СредОбластьИмя);
		О2 = Области.Найти("П2100008_" + СредОбластьИмя);
		О3 = Области.Найти("П2100009_" + СредОбластьИмя);
		О4 = Области.Найти("П2100010_" + СредОбластьИмя);
		О5 = Области.Найти("П2100011_" + СредОбластьИмя);
		О6 = Области.Найти("П2100012_" + СредОбластьИмя);
		О7 = Области.Найти("П2100013_" + СредОбластьИмя);
		
		О3.Значение = О1.Значение * О2.Значение;
		О6.Значение = О4.Значение * О5.Значение;
		О7.Значение = ((О3.Значение * О6.Значение) / 1000.0);
		СохранитьЗначениеПоказателяВСтруктуре(Форма, "ТовУпакИсп", О3.Имя, О3.Значение);
		СохранитьЗначениеПоказателяВСтруктуре(Форма, "ТовУпакИсп", О6.Имя, О6.Значение);
		СохранитьЗначениеПоказателяВСтруктуре(Форма, "ТовУпакИсп", О7.Имя, О7.Значение);
	ИначеЕсли Не ЗначениеЗаполнено(ОбластьИмя) Тогда 
		Для Каждого СтрОбласть Из СтрРазделить("П1100007_1,П2100009_1", ",") Цикл 
...
Процедура РасчетИтогов(Форма)
	...
	Если Области.Найти("П040100") <> Неопределено Тогда 
		Для Каждого Стр Из СтрРазделить("П040100,П040200", ",") Цикл 
	...
	Для Каждого Стр Из Форма.ТаблицаСтраницыТовУпакНеИсп Цикл
		Для Каждого ДДС Из Стр.ДанныеДопСтрок Цикл 
			Для Каждого СтрМнг Из ДДС.Значение.П11000 Цикл
				СтрМнг.П1100010 = СтрМнг.П1100009 * СтрМнг.П1100008;
				СтрМнг.П1100011 = СтрМнг.П1100007 * СтрМнг.П1100010 / 1000;
				П01 = П01 + СтрМнг.П1100011;
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
	...
	Для Каждого Стр Из Форма.ТаблицаСтраницыТовУпакИсп Цикл
		Для Каждого ДДС Из Стр.ДанныеДопСтрок Цикл 
			Для Каждого СтрМнг Из ДДС.Значение.П21000 Цикл
				...
				СтрМнг.П2100013 = (СтрМнг.П2100009 * СтрМнг.П2100012) / 1000;
				П02 = П02 + СтрМнг.П2100013;
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
