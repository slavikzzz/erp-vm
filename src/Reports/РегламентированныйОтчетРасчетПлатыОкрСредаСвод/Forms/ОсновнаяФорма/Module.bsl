...
Процедура ПоказатьПериод(Форма)
	...
	Иначе
		...
		Если Форма.мДатаКонцаПериодаОтчета >= '20160801' Тогда
			Форма.ОписаниеНормативДок = "Утратила силу с 21.08.2016 в соответствии с Приказом Ростехнадзора от 15.07.2016 N 301";
		Иначе
...
Процедура ОткрытьФормуОтчета(Команда)
	...
	Если Год(мДатаКонцаПериодаОтчета) >= 2016 Тогда 
		ОткрытьФорму("Отчет.РегламентированныйОтчетРасчетПлатыОкрСредаСвод.Форма.ФормаПредупреждение2016",,ЭтаФорма,,,,ОО,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Возврат;
	КонецЕсли;
