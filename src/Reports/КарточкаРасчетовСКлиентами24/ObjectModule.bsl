...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(НастройкиОтчета, "ТекстНачальныйОстаток", НСтр("ru = 'Начальный остаток';
...
Функция ТекстЗапроса()
	...
	|		ТОГДА ВЫБОР КОГДА СУММА(Расчеты.ПлановаяОплата) > 0 ИЛИ СУММА(Расчеты.ПлановаяОтгрузка) > 0
	|				// Для заказов или реализаций в пути в первую очередь проверяем плановую оплату
	|				ТОГДА ВЫБОР КОГДА СУММА(Расчеты.ПлановаяОплата) = 0 
	|					ТОГДА ВЫБОР КОГДА СУММА(Расчеты.ПлановаяОтгрузка) > СУММА(Расчеты.Зачтено) И СУММА(Расчеты.ПлановаяОтгрузка) > 0
	...
	|						КОНЕЦ
	|					ИНАЧЕ ВЫБОР КОГДА СУММА(Расчеты.Зачтено) > 0
	|							// Реализации в пути уменьшают плановую оплату на сумму зачета
	|							ТОГДА СУММА(Расчеты.ПлановаяОплата) + СУММА(Расчеты.Зачтено)
	|						ИНАЧЕ СУММА(Расчеты.ПлановаяОплата)
	|					КОНЕЦ
	|				КОНЕЦ
	|			ИНАЧЕ ВЫБОР КОГДА СУММА(Расчеты.Зачтено)+СУММА(Расчеты.ПлановаяОтгрузка) = 0 
