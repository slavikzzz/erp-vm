...
Процедура ПоказатьПериод()
	...
	СтруктураРеквизитовФормы.СтрПериодОтчета = " за "
		+ НРег(Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг'")) + " г.";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 Тогда
		УдалитьСтроку("");
		СтандартнаяОбработка = Ложь;
	ИначеЕсли Лев(Область.Имя, 14) ="П000102000101_" Тогда
		
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "КодыРаздел2");
		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			СмежнаяОбласть =
				ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "П000102000101_", "П000102000107_"));
			ВвестиПоказатель(Элемент, СмежнаяОбласть, ПараметрВводаПоля.Показатель, Ложь,
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 10 Цикл
		...
		Если Области.Найти("П0108" + ГрСтр) <> Неопределено Тогда 
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	//
	//Область4_2 = ПолеТабличногоДокументаФормаОтчета.Области.ОсновнаяЧасть3;
	...
	//КонецЕсли;
	//
	//ЭтотОбъект[СтруктураРеквизитовФормы.мИдГруппы1].Очистить();
	//Для Инд = 1 По мСтруктураИсхКолвоСтрокРазделов[СтруктураРеквизитовФормы.мИдГруппы1] Цикл
	//	ЭтотОбъект[СтруктураРеквизитовФормы.мИдГруппы1].Добавить();
	//КонецЦикла;
	//
	...
	//ВыбТабличноеПоле.Вывести(ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаОтчета2017Кв1_ФормаОтчета"));
	//
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3_5")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 11-НА");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "Коды",
		512, , "Коды", КоллекцияСписковВыбора["Коды"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "КодыРаздел2",
		512, , "Выбор кода", КоллекцияСписковВыбора["КодыРаздел2"]);
КонецПроцедуры
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ПолеТабличногоДокументаФормаОтчета.Области.Найти(
		СтрЗаменить(Область.Имя, "П000102000107_", "П000102000101_")).Значение = РезультатНазвание;
...
Процедура УдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, ТекущаяСтраница = Неопределено) Экспорт
	...
	Иначе
		...
		Для Каждого СтрокаТаблицы Из СтруктураГруппы Цикл
			...
			НомерСтрокиВФормеТаблицыСлед = Формат(НомерСтрокиВТаблице + 1 + 1, "ЧГ=0");
			...
			Для НомерГрафы = 0 По Форма.мСтруктураКолвоКолонокРазделов[ИмяГруппы] Цикл
				...
				Если СтруктураГруппы[0].Свойство(КолонкаТаблицыИмя) Тогда
					...
					Если РегламентированнаяОтчетностьКлиентСервер.ЯвляетсяЧислом(ОбластьЯчейкиТаблицы.Значение) Тогда
						Если ОбластьЯчейкиТаблицыСлед = Неопределено Тогда
							ОбластьЯчейкиТаблицы.Формат = "";
						Иначе
							ОбластьЯчейкиТаблицы.Формат = ОбластьЯчейкиТаблицыСлед.Формат;
						КонецЕсли;
					КонецЕсли;
...
Процедура ПеренумероватьСтроки(ТабДок)
	...
	Для Инд = 1 По 7 Цикл
		ТабДок.Области["П000102000102_" + Инд].Значение = "" + (8+Инд);
	КонецЦикла;
	
	Инд = 1;
	Пока Истина Цикл
		Обл = ТабДок.Области.Найти("П000102000102_" + Инд);
