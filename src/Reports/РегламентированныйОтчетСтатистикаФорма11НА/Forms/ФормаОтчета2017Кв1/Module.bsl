...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "28/06/2017";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 14) ="П000102000101_" Тогда
		...
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "КодыРаздел2");
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 10 Цикл
		...
		Области["П0101"+ГрСтр].Значение = Области["П0102"+ГрСтр].Значение+Области["П0107"+ГрСтр].Значение;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ЭтаФорма[СтруктураРеквизитовФормы.мИдГруппы1].Очистить();
	Для Инд = 1 По мСтруктураИсхКолвоСтрокРазделов[СтруктураРеквизитовФормы.мИдГруппы1] Цикл 
		ЭтаФорма[СтруктураРеквизитовФормы.мИдГруппы1].Добавить();
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПечатнаяФорма.Автомасштаб         = Истина;
	ПечатнаяФорма.ОриентацияСтраницы  = ОриентацияСтраницы.Ландшафт;
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть3_0"));
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть3"));
	
	ВерхМногострочнойЧасти = Области.ОсновнаяЧасть3.Низ + 1;
	НизМногострочнойЧасти = Области.ДобавлениеСтроки1.Верх - 1;
	Область4 = ТабДок.ПолучитьОбласть("ОсновнаяЧасть4");
	...
	Область4_1 = ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_1");
	Если ПечатнаяФорма.ПроверитьВывод(Область4_1) Тогда
		ПечатнаяФорма.Вывести(Область4_1);
	Иначе
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 11-НА");
		ПечатнаяФорма = Новый ТабличныйДокумент();
		ПечатнаяФорма.ОтображатьЗаголовки = Ложь;
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Пока Истина Цикл
		...
		Если Не ПараметрыВыгрузки.Свойство("П000102000101_" + ИндСтр) Тогда
			Прервать;
		КонецЕсли;
		
		ТабДокОбласть = ПолеТабличногоДокументаФормаОтчета.Области;
		СтрокаЗаполнена = Ложь;
		Для Гр = 3 По 6 Цикл
			ПоказательИмя = "П00010200010" + Гр + "_" + ИндСтр;
			Если ЗначениеЗаполнено(ТабДокОбласть[ПоказательИмя].Значение) Или ТабДокОбласть[ПоказательИмя].Формат = "ЧН=0" Тогда
		...
		Если СтрокаЗаполнена Тогда
			НомСтр = НомСтр + 1;
			ПараметрыВыгрузки.Вставить("ПД00102000102_" + ИндСтр, Формат(НомСтр, "ЧГ="));
		КонецЕсли;
	КонецЦикла;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "КодыРаздел2", 512, , "КодыРаздел2", КоллекцияСписковВыбора["КодыРаздел2"]);
КонецПроцедуры
