...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы110 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Удалить выбранную партию товара?';
							|en = 'Удалить выбранную партию товара?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ЗаполнитьАвтоЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗапуска.Статус = "Выполнено" Тогда
		ВывестиПредупреждениеПослеАвтозаполнения();
...
#Область ОтображениеБаннераСИнформациейОЗаполненииОтчета

&НаСервере
Процедура НастройкаВидимостиБаннера()
	
	ПараметрыОтчета = Новый Структура();
	ПараметрыОтчета.Вставить("Организация",              СтруктураРеквизитовФормы.Организация);
	ПараметрыОтчета.Вставить("мДатаНачалаПериодаОтчета", СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
	ПараметрыОтчета.Вставить("мДатаКонцаПериодаОтчета",  СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	ПараметрыОтчета.Вставить("ПараметрыБаннера",         Истина);
	
	Показатели = РегламентированнаяОтчетностьПереопределяемый.ПолучитьСведенияОПоказателяхОтчета(
		ИмяОтчета(ЭтотОбъект.ИмяФормы),
		ИмяФормыОтчета(ЭтотОбъект.ИмяФормы),
		ПараметрыОтчета);
	
	Если Показатели.Свойство("ОтображатьБаннер") И Показатели.ОтображатьБаннер = Истина Тогда
		
		Элементы.ГруппаИнформацияОНеобходимостиСдачиОтчета.Видимость = Показатели.ОтображатьБаннер;
		
		Элементы.БаннерВключениеФункциональностиПоПрослеживаемости.Заголовок
			= Показатели.ЗаголовокБаннерВключениеФункциональностиПоПрослеживаемости;
		Элементы.БаннерСсылкаНаДопИнформацию.Заголовок              = Показатели.ЗаголовокБаннерСсылкаНаДопИнформацию;
		Элементы.ГруппаИнформацияОНеобходимостиСдачиОтчета.ЦветФона = Показатели.ЦветФонаБаннера;
		Элементы.КотикБаннер.Картинка                               = Показатели.КотикБаннер;
		Элементы.ЗакрытьБаннер.Картинка                             = Показатели.ЗакрытьБаннер;
		ВедетсяУчетПрослеживаемыхТоваров                            = Показатели.ВедетсяУчетПрослеживаемыхТоваров;
		НазваниеФормыФункциональностиПопрослеживаемымТоварам        = Показатели.ФормаФункциональности;
		ПараметрыОткрытияФормыФункциональности                      = Показатели.ПараметрыОткрытияФормыФункциональности;
		
	Иначе
		
		Элементы.ГруппаИнформацияОНеобходимостиСдачиОтчета.Видимость = Ложь;
...
&НаКлиенте
Процедура БаннерВключениеФункциональностиПоПрослеживаемостиОбработкаНавигационнойСсылки(Элемент,
	НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьФормуВключенияФункциональности" Тогда
		
		СтандартнаяОбработка = Ложь;
		ОписаниеОповещения = Новый ОписаниеОповещения("ИзмененаФункциональностьПоУчетуПрослеживаемыхТоваров", ЭтотОбъект);
		
		ОткрытьФорму(НазваниеФормыФункциональностиПопрослеживаемымТоварам,
		ПараметрыОткрытияФормыФункциональности, ЭтаФорма ,,,, ОписаниеОповещения);
...
&НаКлиенте
Процедура ИзмененаФункциональностьПоУчетуПрослеживаемыхТоваров(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ВывестиПредупреждениеПослеАвтозаполнения()

	Если (ТипЗнч(СтруктураРеквизитовФормы.ПредупреждениеПослеАвтозаполнения) = Тип("Строка") ИЛИ
		ТипЗнч(СтруктураРеквизитовФормы.ПредупреждениеПослеАвтозаполнения) = Тип("ФорматированнаяСтрока"))
		И СтруктураРеквизитовФормы.ПредупреждениеПослеАвтозаполнения <> "" Тогда
		ПоказатьПредупреждение(, СтруктураРеквизитовФормы.ПредупреждениеПослеАвтозаполнения);
	КонецЕсли;
	
	СтруктураРеквизитовФормы.Вставить("ПредупреждениеПослеАвтозаполнения", Неопределено);
