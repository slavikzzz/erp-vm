...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.НаименованиеОтчета = ИмяОтчета(ИмяФормы);
	СтруктураРеквизитовФормы.НаименованиеФормыОтчета = Сред(ИмяФормы, СтрНайти(ИмяФормы, "ФормаОтчета"));
...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Результат = РегламентированнаяОтчетностьВызовСервера.ЗаполнитьСведенияОбОрганизацииНаСервере(ЭтотОбъект, Сведения);
...
Процедура ОбновитьПользовательскиеНастройки()
	...
	"ru = 'Изменилась версия формы отчета.%1Выполняется обновление пользовательских настроек.';
	...
	Если ЕстьИзмененияВариантыЗаполнения ИЛИ ЕстьИзмененияНастройкаСтраниц Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Обновление пользовательских настроек успешно завершено!';
													|en = 'Обновление пользовательских настроек успешно завершено!'"));
	Иначе
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Изменения в структуре пользовательских настроек не обнаружены!';
...
Процедура ОткрытьФормуВыбораСтраниц(ВариантВыбора)
	...
	ФормаНастройкиОтчета = РегламентированнаяОтчетностьКлиент.ПолучитьОбщуюФормуПоИмени("НастройкиОтчета", ПараметрыФормы, ЭтотОбъект, "дляВыбораСтраницОтчета");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Приложение3" Тогда
		СохрТЗМногострочнойГруппы = ЭтотОбъект[СтруктураРеквизитовФормы.мИдГруппы2].Выгрузить();
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Приложение5" Тогда
		СохрТЗМногострочнойГруппы = ЭтотОбъект[СтруктураРеквизитовФормы.мИдГруппы3].Выгрузить();
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Приложение2" Тогда
		ЭтотОбъект[СтруктураРеквизитовФормы.мИдГруппы1].Загрузить(СохрТЗМногострочнойГруппы);
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Приложение3" Тогда
		ЭтотОбъект[СтруктураРеквизитовФормы.мИдГруппы2].Загрузить(СохрТЗМногострочнойГруппы);
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Приложение5" Тогда
		ЭтотОбъект[СтруктураРеквизитовФормы.мИдГруппы3].Загрузить(СохрТЗМногострочнойГруппы);
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Приложение6" Тогда
...
Функция ПроверитьТитульныйЛистИСведенияОФизЛицах(ВыводитьСообщения = Истина, ЧтоПроверять = "12", ПечатаемыеЛисты = Неопределено)
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Источник данных для формирования отчета не определен!';
													|en = 'Источник данных для формирования отчета не определен!'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция ПроверитьПредписанныеПоказатели(БратьИзФормы = Истина, ВыводитьСообщения = Истина, ЧтоПроверять = "12", ПечатаемыеЛисты = Неопределено)
	...
	ФормаВыводаСообщений = ЭтотОбъект;
	ИмяРазделаПред = "";
	СтатусПроверки = 0;
	ПредставлениеОтчета = "";
	...
	Иначе
		...
		Если ДанныеОтчета = Неопределено Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр(
			"ru = 'Данные для формирования отчета не заданы. Не удалось проверить правильность заполнения!';
			|en = 'Данные для формирования отчета не заданы. Не удалось проверить правильность заполнения!'"));
			Возврат Ложь;
		КонецЕсли;
		ПоказателиОтчета = ДанныеОтчета.ПоказателиОтчета;
		...
		Для Каждого ЭлементСтруктуры Из ДанныеОтчета.ДанныеВариантовАвтоЗаполнения Цикл
			...
			МассивТаблЗнач = ЭтотОбъект["ТаблицаВариантыЗаполнения" + ЭлементСтруктуры.Ключ];
...
&НаСервере
Функция УниверсальнаяВыгрузка(Текст = Неопределено, ПроверятьБлокировки = Истина, ПроверятьВозможностьВыгрузки = Истина) Экспорт
...
Процедура УстановитьФорматВыводаНаСервере(НаимРаздела)
	...
	РегламентированнаяОтчетностьКлиентСервер.УстановитьФорматВывода(ЭтотОбъект, НаимРаздела);
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если Область.Имя = "КодОКАТО1" И ЗначениеЗаполнено(Область.Значение) Тогда
		...
		ИначеЕсли НЕ ДлинаЗначения = 8 И НЕ ДлинаЗначения = 11 Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Код ОКТМО(ОКАТО) должен состоять из 8 или 11 цифр.';
															|en = 'Код ОКТМО(ОКАТО) должен состоять из 8 или 11 цифр.'"));
		КонецЕсли;
	...
	Если Область.Имя = "ОКВЭДРаздел1" Тогда
		...
		Если НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(Область.Значение) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'В разделе 1 задано некорректное значение кода вида деятельности по ОКВЭД!%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).';
				|en = 'В разделе 1 задано некорректное значение кода вида деятельности по ОКВЭД!%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).'"),
...
Процедура ПроставитьСведенияОПредставителеОрганизацииНаКлиенте() Экспорт
	...
	ПроставитьСведенияОПредставителеОрганизации(ЭтотОбъект);
...
Процедура РасчетРаздела3(ВызываетсяИзЗаполнитьАвто)
	...
	ТблПриложение5 = ЭтотОбъект[мСтруктураМногостраничныхРазделов.Приложение5];
	ТблПриложение6 = ЭтотОбъект[мСтруктураМногостраничныхРазделов.Приложение6];
...
Процедура РасчетРаздела2(ВызываетсяИзЗаполнитьАвто)
	...
	ТблПриложение1 = ЭтотОбъект[мСтруктураМногостраничныхРазделов.Приложение1];
	ТблПриложение2 = ЭтотОбъект[мСтруктураМногостраничныхРазделов.Приложение2];
	...
	ТблПриложение3 = ЭтотОбъект[мСтруктураМногостраничныхРазделов.Приложение3];
...
Процедура ПроверкаРаздел1 (СтруктураРезультата)
	...
	Если Число(Титульный.ОтчетГод) >= 2010 Тогда
		...
		ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел1.СуммаНалога25Число1) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230022008));
		
		Расшифровка = "" + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел1.СуммаНалога25Число1) + " = " + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230022008);
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел1.СуммаНалогаКУменьшению1) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230023008));
	
	Расшифровка = "" + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел1.СуммаНалогаКУменьшению1) + " = " + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230023008);
...
Процедура ПроверкаРаздел2 (СтруктураРезультата)
	...
	Если КолСтраниц1 > 0 Или Раздел2.П000210002009 <> 0 Тогда
		...
		ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210002009) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, П2Ст040Гр13));
	...
	Если КолСтраниц1 > 0 Или Раздел2.П000210007009 <> 0 Тогда
		...
		ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210007009) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, П2Ст040Гр13));
	...
	Если КолСтраниц1 > 0 Или Раздел2.П000210003009 <> 0 Тогда
		...
		ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210003009) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, П2Ст040Гр14));
	...
	Если КолСтраниц1 > 0 Или Раздел2.П000210008009 <> 0 Тогда
		...
		ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210008009) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, П2Ст040Гр14));
	...
	Если КолСтраниц1 > 0 Или Раздел2.П000210004009 <> 0 Тогда
		...
		ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210004009) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, П3Ст040Гр13));
	...
	Если КолСтраниц1 > 0 Или Раздел2.П000210009009 <> 0 Тогда
		...
		ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210009009) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, П3Ст040Гр13));
	...
	Если КолСтраниц1 > 0 Или Раздел2.П000210005009 <> 0 Тогда
		...
		ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210005009) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, П3Ст040Гр14));
	...
	Если КолСтраниц1 > 0 Или Раздел2.П000210010009 <> 0 Тогда
		...
		ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210010009) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, П3Ст040Гр14));
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210001009) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210002009) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210003009) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210004009) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210005009));
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210006009) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210007009) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210008009) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210009009) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210010009));
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220011008) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220011006 * Раздел2.П000220011007));
	
	Расшифровка = "" + Раздел2.П000220011008 + " = " + Раздел2.П000220011006 + " х " + Раздел2.П000220011007 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220011006 * Раздел2.П000220011007)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220012008) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220012006 * Раздел2.П000220012007));
	
	Расшифровка = "" + Раздел2.П000220012008 + " = " + Раздел2.П000220012006 + " х " + Раздел2.П000220012007 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220012006 * Раздел2.П000220012007)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220013008) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220013006 * Раздел2.П000220013007));
	
	Расшифровка = "" + Раздел2.П000220013008 + " = " + Раздел2.П000220013006 + " х " + Раздел2.П000220013007 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220013006 * Раздел2.П000220013007)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220014008) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220014006 * Раздел2.П000220014007));
	
	Расшифровка = "" + Раздел2.П000220014008 + " = " + Раздел2.П000220014006 + " х " + Раздел2.П000220014007 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220014006 * Раздел2.П000220014007)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220015008) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220015006 * Раздел2.П000220015007));
	
	Расшифровка = "" + Раздел2.П000220015008 + " = " + Раздел2.П000220015006 + " х " + Раздел2.П000220015007 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220015006 * Раздел2.П000220015007)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220016008) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220016006 * Раздел2.П000220016007));
	
	Расшифровка = "" + Раздел2.П000220016008 + " = " + Раздел2.П000220016006 + " х " + Раздел2.П000220016007 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220016006 * Раздел2.П000220016007)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220017008) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220017006 * Раздел2.П000220017007));
	
	Расшифровка = "" + Раздел2.П000220017008 + " = " + Раздел2.П000220017006 + " х " + Раздел2.П000220017007 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220017006 * Раздел2.П000220017007)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220018008) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220018006 * Раздел2.П000220018007));
	
	Расшифровка = "" + Раздел2.П000220018008 + " = " + Раздел2.П000220018006 + " х " + Раздел2.П000220018007 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220018006 * Раздел2.П000220018007)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220019008) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220019006 * Раздел2.П000220019007));
	
	Расшифровка = "" + Раздел2.П000220019008 + " = " + Раздел2.П000220019006 + " х " + Раздел2.П000220019007 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220019006 * Раздел2.П000220019007)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230020008) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210001009) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000210006009) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220011008) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220014008) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000220017008) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030013007));
	...
	Если Раздел2.П000230020008 >= Раздел2.П000230021008 Тогда
		...
		ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230022008) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230020008) - РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230021008));
		
		Расшифровка = "" + Раздел2.П000230022008 + " = " + Раздел2.П000230020008 + " - " + Раздел2.П000230021008 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230020008) - РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230021008)) + "]";
	...
	Если Раздел2.П000230020008 <= Раздел2.П000230021008 Тогда
		...
		ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230023008) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230021008) - РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230020008));
		
		Расшифровка = "" + Раздел2.П000230023008 + " = " + Раздел2.П000230021008 + " - " + Раздел2.П000230020008 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230021008) - РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел2.П000230020008)) + "]";
...
Процедура ПроверкаРаздел3 (СтруктураРезультата)
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030002007) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030002005 * Раздел3.П000030002006));
	
	Расшифровка = "" + Раздел3.П000030002007 + " = " + Раздел3.П000030002005 + " х " + Раздел3.П000030002006 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030002005 * Раздел3.П000030002006)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030003007) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030003005 * Раздел3.П000030003006));
	
	Расшифровка = "" + Раздел3.П000030003007 + " = " + Раздел3.П000030003005 + " х " + Раздел3.П000030003006 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030003005 * Раздел3.П000030003006)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030004007) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030004005 * Раздел3.П000030004006));
	
	Расшифровка = "" + Раздел3.П000030004007 + " = " + Раздел3.П000030004005 + " х " + Раздел3.П000030004006 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030004005 * Раздел3.П000030004006)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005107) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005105 * Раздел3.П000030005106));
	
	Расшифровка = "" + Раздел3.П000030005107 + " = " + Раздел3.П000030005105 + " х " + Раздел3.П000030005106 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005105 * Раздел3.П000030005106)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005207) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005205 * Раздел3.П000030005206));
	
	Расшифровка = "" + Раздел3.П000030005207 + " = " + Раздел3.П000030005205 + " х " + Раздел3.П000030005206 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005205 * Раздел3.П000030005206)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005307) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005305 * Раздел3.П000030005306));
	
	Расшифровка = "" + Раздел3.П000030005307 + " = " + Раздел3.П000030005305 + " х " + Раздел3.П000030005306 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005305 * Раздел3.П000030005306)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006107) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006105 * Раздел3.П000030006106));
	
	Расшифровка = "" + Раздел3.П000030006107 + " = " + Раздел3.П000030006105 + " х " + Раздел3.П000030006106 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006105 * Раздел3.П000030006106)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006207) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006205 * Раздел3.П000030006206));
	
	Расшифровка = "" + Раздел3.П000030006207 + " = " + Раздел3.П000030006205 + " х " + Раздел3.П000030006206 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006205 * Раздел3.П000030006206)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006307) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006305 * Раздел3.П000030006306));
	
	Расшифровка = "" + Раздел3.П000030006307 + " = " + Раздел3.П000030006305 + " х " + Раздел3.П000030006306 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006305 * Раздел3.П000030006306)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030008007) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030008005 * Раздел3.П000030008006));
	
	Расшифровка = "" + Раздел3.П000030008007 + " = " + Раздел3.П000030008005 + " х " + Раздел3.П000030008006 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030008005 * Раздел3.П000030008006)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030009007) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030009005 * Раздел3.П000030009006));
	
	Расшифровка = "" + Раздел3.П000030009007 + " = " + Раздел3.П000030009005 + " х " + Раздел3.П000030009006 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030009005 * Раздел3.П000030009006)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030010007) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030010005 * Раздел3.П000030010006));
	
	Расшифровка = "" + Раздел3.П000030010007 + " = " + Раздел3.П000030010005 + " х " + Раздел3.П000030010006 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030010005 * Раздел3.П000030010006)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011107) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011105 * Раздел3.П000030011106));
	
	Расшифровка = "" + Раздел3.П000030011107 + " = " + Раздел3.П000030011105 + " х " + Раздел3.П000030011106 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011105 * Раздел3.П000030011106)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011207) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011205 * Раздел3.П000030011206));
	
	Расшифровка = "" + Раздел3.П000030011207 + " = " + Раздел3.П000030011205 + " х " + Раздел3.П000030011206 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011205 * Раздел3.П000030011206)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011307) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011305 * Раздел3.П000030011306));
	
	Расшифровка = "" + Раздел3.П000030011307 + " = " + Раздел3.П000030011305 + " х " + Раздел3.П000030011306 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011305 * Раздел3.П000030011306)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012107) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012105 * Раздел3.П000030012106));
	
	Расшифровка = "" + Раздел3.П000030012107 + " = " + Раздел3.П000030012105 + " х " + Раздел3.П000030012106 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012105 * Раздел3.П000030012106)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012207) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012205 * Раздел3.П000030012206));
	
	Расшифровка = "" + Раздел3.П000030012207 + " = " + Раздел3.П000030012205 + " х " + Раздел3.П000030012206 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012205 * Раздел3.П000030012206)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012307) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012305 * Раздел3.П000030012306));
	
	Расшифровка = "" + Раздел3.П000030012307 + " = " + Раздел3.П000030012305 + " х " + Раздел3.П000030012306 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012305 * Раздел3.П000030012306)) + "]";
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030001007) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030002007) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030003007) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030004007) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005007) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006007));
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005007) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005107) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005207) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030005307));
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006007) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006107) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006207) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030006307));
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030007007) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030008007) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030009007) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030010007) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011007) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012007));
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011007) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011107) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011207) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030011307));
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012007) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012107) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012207) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030012307));
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030013007) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030001007) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030007007));
	
	Расшифровка = "" + Раздел3.П000030013007 + " = " + Раздел3.П000030001007 + " + " + Раздел3.П000030007007 + " Итог [" + (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030001007) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел3.П000030007007)) + "]";
...
Процедура ПроверкаРаздел4 (СтруктураРезультата)
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040011005) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040001005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040002005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040003005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040004005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040005005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040006005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040007005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040008005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040009005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040010005));
	...
	ЭтоОшибка = Не (РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040022005) = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040012005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040013005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040014005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040015005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040016005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040017005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040018005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040019005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040020005) + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтотОбъект, Раздел4.П000040021005));
