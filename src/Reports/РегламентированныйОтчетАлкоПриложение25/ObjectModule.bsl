...
Перем СписокКодовДеятельности, СписокКодовПродукции, СписокКодовВидовРозничнойПродажи;
...
Процедура ОбработкаЗаписи(ИмяРегистра, Запись,
	...
	Если ИмяРегистра = "СведенияРеглОтчетАлкоПрил25Раздел1Обороты" Тогда
	...
	Если ИмяРегистра = "СведенияРеглОтчетАлкоПрил25Раздел2Поступления" 
		ИЛИ ИмяРегистра = "СведенияРеглОтчетАлкоПрил25Раздел3Возвраты" Тогда

		ИдГруппы = ?(ИмяРегистра = "СведенияРеглОтчетАлкоПрил25Раздел2Поступления", "П0000200003", "П0000300003");
		...
		// Лицензия
		Запись[ИдГруппы + "09"] = СокрЛП(Запись[ИдГруппы + "09"]);
		ВидДеятельности = Запись[ИдГруппы + "09"];
...
Процедура РасчетРаздел1(Запись)
	...
	Запись.П000010000309 = Запись.П000010000307 + Запись.П000010000308;
		
	Запись.П000010000313 = Запись.П000010000310 + Запись.П000010000311
		+ Запись.П000010000312 + Запись.П000010000309;

	Запись.П000010000318 = Запись.П000010000314 + Запись.П000010000315
		+ Запись.П000010000316 + Запись.П000010000317;

	Запись.П000010000319 = Запись.П000010000306 + Запись.П000010000313
		- Запись.П000010000318;
...
Процедура ПроверитьЗаполнениеДекларации(СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		РегламентированнаяОтчетностьАЛКО.ВерноеЗаполнениеОП(
					...
					СписокКодовВидовРозничнойПродажи);
		...
		Для Каждого ЭлементОП Из ДанныеОП["Декларация"] Цикл
			...
			Для каждого ЭлементДвиженияПоОП Из ДанныеДвиженийПоОП Цикл
				...
				Для Каждого Элемент_ДанныеДвижений_ПоОП_ПоКоду Из ДанныеДвижений_ПоОП_ПоКоду Цикл
					...
					ПоступлениеПоРазделу1 = СтрокаСведенийОПроизводителеИмпортере["П100000000009"];
					...
					Если ПоступлениеПоРазделу1 <> ПоступлениеПоРазделу2 Тогда
						...
						Показатель = СтруктураРеквизитовФормы.мИдГруппы12 + "09_"
						...
						ТекстОписанияОшибки = СтрокаГде
							+ " не совпадает значение по графе 09 ""Поступление - закупки - итого"" Раздела 1 и сумма объемов по накладным из Раздела 2 (Поступления)!
							|Подробно:
							|ОП: " + СтрОбособленноеПодразделение + "							
							|Код продукции: " + КодВидаПродукции + "
							|Производитель/импортер: " + СтрПроизводитель + "
							|сумма по графе 09 Раздела 1 """
					...
					ВозвратПоРазделу1 = СтрокаСведенийОПроизводителеИмпортере["П100000000016"];
					...
					Если ВозвратПоРазделу1 <> ВозвратПоРазделу3 Тогда
						...
						// Указывать будем на поле итога в Разделе 1.
						Показатель = СтруктураРеквизитовФормы.мИдГруппы12 + "16_"
							+ НомерСтрокиРаздел1Строковый;
						...
						ТекстОписанияОшибки = СтрокаГде
							+ " не совпадает значение по графе 16 ""Возврат поставщику"" Раздела 1 и сумма объемов по накладным из Раздела 3 (Возвраты)!
							|Подробно:
							|ОП: " + СтрОбособленноеПодразделение + "							
							|Код продукции: " + КодВидаПродукции + "
							|Производитель/импортер: " + СтрПроизводитель + "
							|сумма по графе 16 Раздела 1 """
...
Процедура ПроверитьЗаполнениеТаблицыРаздел2(СтатусПроверки, ИдГруппы,
	...
	ИмяВидаДеятельностиЛицензии = ИдГруппы + "09";
	
	ИмяДатыТТН = ИдГруппы + "10";
	ИмяНомерТТН = ИдГруппы + "11";
	ИмяНомерГТД = ИдГруппы + "12";
	ИмяОбъемПоТТН = ИдГруппы + "13";

	ИмяРаздела = "Декларация";
	СтрВКакомРазделе = "В разделе Декларация";
	КоличествоКолонокВРазделе = 13;
	...
	Пока Истина Цикл
		...
		Для каждого СтрокаДекларации Из РезЗапросаДанных Цикл
			...
			// Контролировать ничего не нужно - заполнение вида деятельности необязательно,
			// при этом нет уже серии-номера и т.д. как в 11 декларации.
			ЗначениеГрафыВидДеятельностиЛицензии = СокрЛП(СтрокаДекларации[ИмяВидаДеятельностиЛицензии]);
				
			ЛицензияЗаполнена = ЗначениеЗаполнено(ЗначениеГрафыВидДеятельностиЛицензии);
			//
			////////////////////////////////////////////////////////////////////////////////
			...
			Если НужноПерекрестноПроверятьПроизводителей
				...
				Если СправочникиОтчета["Поставщики"].Получить(ИдПоставщикаЛиц) = Неопределено Тогда
					...
					Если ЛицензияЗаполнена Тогда					
						СправочникиОтчета["Поставщики"][ИдПоставщикаЛиц].Вставить("П000000000011", ЗначениеГрафыВидДеятельностиЛицензии);					
					КонецЕсли; 
...
Процедура ЗаполнитьСправочникиДекларации(УзелСправочники)
	...
	Для Каждого ЭлементСправочникиПоставщики Из СправочникиОтчета["Поставщики"] Цикл
		...
		Если НЕ ПустаяСтрока(СтрокаПоставщика["П000000000011"]) Тогда
			УстановитьЗначениеЭлемента(Узел_Поставщики, "П000000000011", СтрокаПоставщика["П000000000011"]);		
		КонецЕсли; 
...
Процедура ЗаполнитьСведенияДекларации(НовыйУзелОбъемОборота, ИдОП,
	...
	ТекстЗапросаРаздела = "ВЫБРАТЬ
		|	РегистрСведенийАлко.*
		|ИЗ
		|	РегистрСведений.СведенияРеглОтчетАлкоПрил25Раздел2Поступления КАК РегистрСведенийАлко
		|ГДЕ
	...
	ТекстЗапросаРаздела_КодПрод_Произв_ИдЛицензии = "ВЫБРАТЬ
		|	РегистрСведенийАлко.*
		|ИЗ
		|	РегистрСведений.СведенияРеглОтчетАлкоПрил25Раздел2Поступления КАК РегистрСведенийАлко
		|ГДЕ
		|	РегистрСведенийАлко.ИДДокИндСтраницы = &ИДДокИндСтраницы
		|	И РегистрСведенийАлко.ХешИдКодПродПроизв = &ХешИдКодПродПроизв
		|	И РегистрСведенийАлко.ХешИдЛицензии = &ХешИдЛицензии
		|	И РегистрСведенийАлко.Активно = ИСТИНА";
	...
	Если (ВсегоСтрокПоРазделу1 + КолСтрокПоРазделу2 <= Лимит)
		...
		Иначе
			...
			ИмяРегистра = "СведенияРеглОтчетАлкоПрил25Раздел2Поступления";
		...
		ТаблицаЗапросаРаздел2.Индексы.Добавить("ХешИдКодПродПроизв, ХешИдЛицензии"); 
	...
	Если (ВсегоСтрокПоРазделу1 + КолСтрокПоРазделу3 <= Лимит)
		...
		Иначе
			// Формируем пустую таблицу с нужными колонками.
			ИмяРегистра = "СведенияРеглОтчетАлкоПрил25Раздел3Возвраты";
		...
		ТаблицаЗапросаРаздел3.Индексы.Добавить("ХешИдКодПродПроизв, ХешИдЛицензии");
		...
		Для Каждого Элемент_ДанныеДвижений_ПоОП_ПоКоду Из ДанныеДвижений_ПоОП_ПоКоду Цикл
			...
			Для Каждого ЭлементПоставщика Из Поставщики Цикл
				...
				Если ЗапросПоРазделу2НеВыполнен И ЗапросПоКодуРаздел2НеВыполнен
					...
					ЗапросСтрокРаздел2.Текст = ТекстЗапросаРаздела_КодПрод_Произв_ИдЛицензии;
					ЗапросСтрокРаздел2.Параметры.Вставить("ИДДокИндСтраницы", ИДДокИндСтраницыРаздел2);
					ЗапросСтрокРаздел2.Параметры.Вставить("ХешИдКодПродПроизв", ХешИдКодПродПроизв);
					ЗапросСтрокРаздел2.Параметры.Вставить("ХешИдЛицензии", ХешИдЛицензии);
					ТаблицаЗапросаРаздел2 = ЗапросСтрокРаздел2.Выполнить().Выгрузить();
				...
				Если ЗапросПоРазделу3НеВыполнен И ЗапросПоКодуРаздел3НеВыполнен
					...
					// Выполняем запрос	по поставщику - лицензии.										
					ТекстЗапросаРаздела3 = СтрЗаменить(ТекстЗапросаРаздела_КодПрод_Произв_ИдЛицензии, 
							"СведенияРеглОтчетАлкоПрил25Раздел2Поступления",
							"СведенияРеглОтчетАлкоПрил25Раздел3Возвраты");
					ТекстЗапросаРаздела3 = СтрЗаменить(ТекстЗапросаРаздела3, "П00002", "П00003");
					...
					ЗапросСтрокРаздел3.Параметры.Вставить("ИДДокИндСтраницы", ИДДокИндСтраницыРаздел3);
					ЗапросСтрокРаздел3.Параметры.Вставить("ХешИдКодПродПроизв", ХешИдКодПродПроизв);
					ЗапросСтрокРаздел3.Параметры.Вставить("ХешИдЛицензии", ХешИдЛицензии);
					ТаблицаЗапросаРаздел3 = ЗапросСтрокРаздел3.Выполнить().Выгрузить();
				...
				Для Каждого СтрокаПродукции Из НайденныеСтрокиРаздел2 Цикл
					...
					Для Счетчик = 10 По 13 Цикл
	
						КолонкаСтрокаПродукции = Формат(Счетчик, "ЧЦ=2; ЧН=; ЧГ=0; ЧВН="); // 10, ..13.
						КолонкаВыгрузки = Формат(Счетчик + 3, "ЧЦ=2; ЧН=; ЧГ=0; ЧВН="); // 13, ..16.
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, КорневойУзел,
	...
	Для Каждого ЭлементОП Из ДанныеОП["Декларация"] Цикл
		...
		Если СведенияОбОП.Свойство("ВидРозничнойПродажи") 
			И ТипЗнч(СведенияОбОП.ВидРозничнойПродажи) = Тип("Число")
			И СведенияОбОП.ВидРозничнойПродажи > 0 Тогда
		
			ВидРозничнойПродажи = СведенияОбОП.ВидРозничнойПродажи;
			ОтсутствуетИнтернет = СведенияОбОП.ОтсутствуетИнтернет;
			
			УстановитьЗначениеЭлемента(Узел_ОбъемОборота, "ОтсутИнет", ?(ОтсутствуетИнтернет, "V", ""));
			УстановитьЗначениеЭлемента(Узел_ОбъемОборота, "ВидПрод", ВидРозничнойПродажи);
...
Функция ПроверитьВыгрузку(ПараметрыДокумента, ЭтоВыгрузка = Ложь) Экспорт
	...
	СписокКодовПродукции = РегламентированнаяОтчетностьАЛКО.МассивКодовИзМакетаСписки(ИмяОбластиКодовМакета, 
			ДатаКонцаОтчета, ИмяФормыОбъекта, ОбъектЭтогоОтчета, , КоллекцияСписковВыбора);
	ИмяОбластиКодовМакета = "ВидыРозничнойПродажи";
	СписокКодовВидовРозничнойПродажи = РегламентированнаяОтчетностьАЛКО.МассивКодовИзМакетаСписки(ИмяОбластиКодовМакета, 
			ДатаКонцаОтчета, ИмяФормыОбъекта, ОбъектЭтогоОтчета, , КоллекцияСписковВыбора);
...
// В структуре СтруктураПараметровСтраницы уже установлены свойства 
	...
	НастройкиАлгоритмаПечати.Вставить("КолонкиСуммированияОрг", "06,07,08,09,10,11,13,14,15,16,18,19,20");
	НастройкиАлгоритмаПечати.Вставить("КолонкиНеВыводитьОрг", "12,17");
	...
	СтруктураИнформацииИтога.Вставить("ТекстПояснения", "Итого по коду %#02#%");
	СтруктураИнформацииИтога.Вставить("КолонкиГруппировок", "02");
	СтруктураИнформацииИтога.Вставить("КолонкиСуммирования", "06,07,08,09,10,11,13,14,15,16,18,19,20");
	...
	СтруктураИнформацииИтога2.Вставить("ТекстПояснения", "ИТОГО");
	СтруктураИнформацииИтога2.Вставить("КолонкиГруппировок", "");
	СтруктураИнформацииИтога2.Вставить("КолонкиСуммирования", "06,07,08,09,10,11,13,14,15,16,18,19,20");
...
// В структуре СтруктураПараметровСтраницы уже установлены свойства 
	...
	СписокПереносимыхПолей.Добавить(26, "09");
	...
	СтруктураИнформацииИтога.Вставить("ТекстПояснения", "Итого по коду %#02#%");
	СтруктураИнформацииИтога.Вставить("КолонкиГруппировок", "02");
	СтруктураИнформацииИтога.Вставить("КолонкиСуммирования", "13");
	...
	СтруктураИнформацииИтога2.Вставить("ТекстПояснения", "ИТОГО");
	СтруктураИнформацииИтога2.Вставить("КолонкиГруппировок", "");
	СтруктураИнформацииИтога2.Вставить("КолонкиСуммирования", "13");
...
Функция ВерсияФорматаВыгрузки(Знач НаДату = Неопределено, ВыбраннаяФорма = Неопределено) Экспорт
	...
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение25.ВерсияФорматаВыгрузки(НаДату, ВыбраннаяФорма);
		
КонецФункции

Функция ТаблицаФормОтчета() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение25.ТаблицаФормОтчета();
	
КонецФункции

Функция ДанныеРеглОтчета(ЭкземплярРеглОтчета) Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение25.ДанныеРеглОтчета(ЭкземплярРеглОтчета);
	
КонецФункции

Функция ДеревоФормИФорматов() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение25.ДеревоФормИФорматов();
