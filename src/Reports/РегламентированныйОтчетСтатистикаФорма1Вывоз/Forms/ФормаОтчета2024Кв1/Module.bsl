...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого СтрокаУровня1 Из мДеревоВыбранныхСтраниц.ПолучитьЭлементы() Цикл
		...
		Если НЕ мСтруктураМногостраничныхРазделов.Свойство(СтрокаУровня1.ИмяСтраницы) Тогда
			...
			НепечатаемаяОбласть2 = ТабличныйДокумент.Области.Найти("НепечатаемаяОбласть2");
			Если НепечатаемаяОбласть2 <> Неопределено Тогда
				ПечатнаяФорма.УдалитьОбласть(ПечатнаяФорма.Области.НепечатаемаяОбласть2, ТипСмещенияТабличногоДокумента.ПоВертикали);
			КонецЕсли;
