...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0000102003";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы["ИмяМакетаАтрибутовВыгрузки"] = "АтрибВыгрузкиXML2017Кв1_" + СтрПериод;
	СтруктураРеквизитовФормы["ИмяМакетаСхемыВыгрузки"] = "СхемаВыгрузкиXML2017Кв1_" + СтрПериод;
	СтруктураРеквизитовФормы["ИмяМакетаШаблона"] = "ШаблонРосстата2017Кв1_" + СтрПериод;
	СтруктураРеквизитовФормы["ИмяМакетаСписков"] = "Списки2017Кв1_" + СтрПериод;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Область.Имя = "НаимТовар" Тогда
		ИсходноеЗначение = ТабличныйДокумент.Области["П000010100101"].Значение;
	ИначеЕсли Область.Имя = "П000010100102" Тогда
		ИсходноеЗначение = ТабличныйДокумент.Области["П000010100107"].Значение;
	ИначеЕсли СтрНайти(Область.Имя, "_") > 0 Или Лев(Область.Имя, 8) = "П0000102" Тогда
		...
		Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "02" Тогда
			...
			ПодстрокаЗамены = Лев(Область.Имя, 11) + "02" + МнгСтр;
		Иначе
			ПодстрокаПоиска = Лев(Область.Имя, 11) + "04" + МнгСтр;
			ПодстрокаЗамены = Лев(Область.Имя, 11) + "05" + МнгСтр;
		КонецЕсли;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Область.Имя = "НаимТовар" Или Область.Имя = "П000010100101" Тогда
		Если Область.Имя = "НаимТовар" Тогда
			Область.Значение = РезультатНазвание;
			ТабличныйДокумент.Области["П000010100101"].Значение = РезультатКод;
		Иначе
			Область.Значение = РезультатКод;
			ТабличныйДокумент.Области["НаимТовар"].Значение = РезультатНазвание;
		КонецЕсли;
		ТабличныйДокумент.Области["П000010100102"].Значение = "";
		ТабличныйДокумент.Области["П000010100107"].Значение = "";
	ИначеЕсли Область.Имя = "П000010100102" Тогда
		Область.Значение = РезультатНазвание;
		ТабличныйДокумент.Области["П000010100107"].Значение = РезультатКод;
	ИначеЕсли СтрНайти(Область.Имя, "_") > 0 Или Лев(Область.Имя, 8) = "П0000102" Тогда
		
		Графа = Сред(Область.Имя, 13, 1);
		МнгСтр = ?(СтрНайти(Область.Имя, "_") > 0, "_", "");
		
		Если Графа = "1" Или Графа = "4" Тогда
			ГрафаПоиска = ?(Графа = "1", "01", "04");
			ГрафаЗамены = ?(Графа = "1", "02", "05");
			ПодстрокаПоиска = Лев(Область.Имя, 11) + ГрафаПоиска + МнгСтр;
			ПодстрокаЗамены = Лев(Область.Имя, 11) + ГрафаЗамены + МнгСтр;
			Область.Значение = РезультатНазвание;
		...
		ИначеЕсли Графа = "2" Или Графа = "5" Тогда
			ГрафаПоиска = ?(Графа = "2", "02", "05");
			ГрафаЗамены = ?(Графа = "2", "01", "04");
			ПодстрокаПоиска = Лев(Область.Имя, 11) + ГрафаПоиска + МнгСтр;
			ПодстрокаЗамены = Лев(Область.Имя, 11) + ГрафаЗамены + МнгСтр;
			Область.Значение = РезультатКод;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "", Перерасчет = Ложь)
	...
	Если Не Перерасчет Тогда
		Если ИмяТекущейСтраницыПанели = "Раздел1" Тогда
			...
			Пока Истина Цикл
				Инд  = Инд + 1;
				Обл  = Форма.ТабличныйДокумент.Области.Найти("П000010200303" + "_" + Строка(Инд));
				Обл2 = Форма.ТабличныйДокумент.Области.Найти("П000010200306" + "_" + Строка(Инд));
				Если Обл = Неопределено Тогда
					Прервать;
				Иначе
					ОбщЗначение = ОбщЗначение + Обл.Значение + Обл2.Значение;
				КонецЕсли;
			КонецЦикла;
			Для ДопСтрока = 4 По 9 Цикл
				ОбщЗначение = ОбщЗначение + Форма.ТабличныйДокумент.Области["П000010200" + ДопСтрока + "03"].Значение;
			КонецЦикла;
			Области["П000010200103"].Значение = ОбщЗначение;
		КонецЕсли;
	КонецЕсли;
	...
	Если Перерасчет Тогда
		Для Каждого Лист Из Форма.ТаблицаСтраницыРаздела1 Цикл
			...
			Пока Истина Цикл
				...
				ЛистПоказатели.Свойство("П000010200303" + "_" + Строка(Инд), ПоказательЗначение);
				ЛистПоказатели.Свойство("П000010200306" + "_" + Строка(Инд), ПоказательЗначение2);
				Если ПоказательЗначение = Неопределено Тогда
					Прервать;
				Иначе
					ОбщЗначениеКолонка = ОбщЗначениеКолонка + ПоказательЗначение + ПоказательЗначение2;
				КонецЕсли;
			КонецЦикла;
			
			Для ДопСтрока = 4 По 9 Цикл
				ОбщЗначениеКолонка = ОбщЗначениеКолонка + ЛистПоказатели["П000010200" + ДопСтрока + "03"];
			КонецЦикла;
			
			ЛистПоказатели["П000010200103"] = ОбщЗначениеКолонка;
		КонецЦикла;
	КонецЕсли;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	ЯчейкаАвторасчета = мФормаОтчета.Области.Найти("П000010200103");
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	Для НомерГрафы = 1 По Форма.мСтруктураКолвоКолонокРазделов[ИмяГруппы] Цикл
		...
		Если СтруктураГруппы[0].Свойство(ИмяКолонки) Тогда
			...
			// Назначаем имена областей для удаления строк
			Если НомерГрафы = 4 Тогда
				НовОбластьЯчейкиУдалитьСтроку = ТекТабличноеПоле.Область(
					ОбластьЯчейки.Верх, 7, ОбластьЯчейки.Верх, 7);
				НовОбластьЯчейкиУдалитьСтроку.Имя = "УдалитьСтроку" + ИмяНовОбластиЯчейки;
				ИсхОбластьЯчейкиУдалитьСтроку = ТекТабличноеПоле.Область(
					ОбластьЯчейкиИсходнойОбласти.Верх, 7, ОбластьЯчейкиИсходнойОбласти.Верх, 7);
				ИсхОбластьЯчейкиУдалитьСтроку.Имя = "УдалитьСтроку" + ИмяОбластиЯчейки;
			КонецЕсли;
...
Процедура УдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, ТекущаяСтраница = Неопределено, ИмяОбласти = Неопределено) Экспорт
	...
	Стр = "";
	Для ДопСтрока = 4 По 9 Цикл
		Стр = Стр + "П000010200" + ДопСтрока + "01" + ";";
	КонецЦикла;
	МассивДопСтрок = РегламентированнаяОтчетностьКлиентСервер.РазобратьСтрокуВМассивПоРазделителю(Стр, ";");
	
	Если МассивДопСтрок.Найти(СтрЗаменить(ИмяОбласти, "УдалитьСтроку", "")) <> Неопределено Тогда
		Для Гр = 1 По 3 Цикл
			ИмяПоказателя = Лев(СтрЗаменить(ИмяОбласти, "УдалитьСтроку", ""), 11) + Формат(Гр, "ЧЦ=2; ЧВН=");
			Форма.ТабличныйДокумент.Области[ИмяПоказателя].Значение = "";
		КонецЦикла;
		Форма.Модифицированность = Истина;
		Возврат;
	КонецЕсли;
...
Процедура УдалитьСтроку(ОбластьИмя)
	...
	Если ИмяГруппы = Неопределено И МассивДопСтрок.Найти(СтрЗаменить(ОбластьИмя, "УдалитьСтроку", "")) = Неопределено Тогда
	...
	Для Каждого СтрокаУровня1 Из мДеревоВыбранныхСтраниц.ПолучитьЭлементы() Цикл
		...
		Иначе
			...
			Для Каждого Страница Из ТаблицаСтраницРаздела Цикл
				...
				Если Не ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть1_4")) Тогда
					СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-вывоз");
					ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
				КонецЕсли;
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1" Тогда
		...
		ТЗВыбора.Очистить();
		ИмяПоказателя = Лев(Область.Имя, 11);
		
		Если Область.Имя = "НаимТовар" Или ИмяПоказателя = "П0000101001" Тогда
			
			Графа = Прав(Область.Имя, 1);
			
			Если Область.Имя = "НаимТовар" Или Графа = "1" Тогда
			...
			ИначеЕсли Графа = "2" Тогда
				...
				КодЕИ = ТабличныйДокумент.Области["П000010100107"].Значение;
				КодОКПД2 = ТабличныйДокумент.Области["П000010100101"].Значение;
			...
			ИначеЕсли Графа = "4" Или Графа = "5" Или Графа = "6" Тогда
				УстановитьЗначокВГруппеЯчеек(Область, 1, 1, 4, 6);
				Модифицированность = Истина;
			КонецЕсли;
			
		ИначеЕсли Лев(Область.Имя, 8) = "П0000102"
			    И Не (Сред(Область.Имя, 13, 1) = "3" Или Сред(Область.Имя, 13, 1) = "6") Тогда
...
Процедура УстановитьЗначокВГруппеЯчеек(Область, НачальнаяСтрокаВГруппе, КонечнаяСтрокаВГруппе, НачальнаяКолонкаВГруппе, КонечнаяКолонкаВГруппе, МножественныйВыбор = Ложь)
	...
	мФормаОтчета = ТабличныйДокумент;
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ИмяТекущейСтраницыПанели = "Раздел1" И Лев(ОбластьИмя, 8) = "П0000102"
	   И (Сред(ОбластьИмя, 13, 1) = "3" Или Сред(ОбластьИмя, 13, 1) = "6") Тогда
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Для Каждого Лист Из ЭтотОбъект.ТаблицаСтраницыРаздела1 Цикл
		...
		Для Графа = 4 По 6 Цикл
			Если ЛистПоказатели["П00001010010" + Графа] = "Х" Тогда
				ИсточникПрод = Строка(Графа - 3);
				Прервать;
			КонецЕсли;
		КонецЦикла;
		...
		Для Группа = 1 По 2 Цикл
			...
			Пока Истина Цикл
				Инд = Инд + 1;
				ПоказательЗначение = Неопределено;
				ЛистПоказатели.Свойство("П0000102003" + ?(Группа = 1, "03", "06") + "_" + Строка(Инд), ПоказательЗначение);
				Если ПоказательЗначение = Неопределено Тогда
					Прервать;
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(ПоказательЗначение) Тогда
					...
					ПДИнд = ПДИнд + 1;
					ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0100101" + "_" + Строка(ПДИнд), ЛистПоказатели["П000010100101"]);
					ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0100107" + "_" + Строка(ПДИнд), ЛистПоказатели["П000010100107"]);
					
					ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0100104" + "_" + Строка(ПДИнд), ИсточникПрод);
					
					ПоказательКодТерритории = ЛистПоказатели["П0000102003" + ?(Группа = 1, "02", "05") + "_" + Строка(Инд)];
					ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0200302" + "_" + Строка(ПДИнд), ПоказательКодТерритории);
					
					ПоказательПродано = ЛистПоказатели["П0000102003" + ?(Группа = 1, "03", "06") + "_" + Строка(Инд)];
					ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0200203" + "_" + Строка(ПДИнд), ПоказательПродано);
		...
		Для ДопСтрока = 4 По 9 Цикл
			ДопСтрокаПоказательПродано = ЛистПоказатели["П000010200" + ДопСтрока + "03"];
			Если ЗначениеЗаполнено(ДопСтрокаПоказательПродано) Тогда
				ПДИнд = ПДИнд + 1;
				ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0100101" + "_" + Строка(ПДИнд), ЛистПоказатели["П000010100101"]);
				ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0100107" + "_" + Строка(ПДИнд), ЛистПоказатели["П000010100107"]);
				ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0100104" + "_" + Строка(ПДИнд), ИсточникПрод);
				ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0200302" + "_" + Строка(ПДИнд), ЛистПоказатели["П000010200" + ДопСтрока + "02"]);
				ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0200203" + "_" + Строка(ПДИнд), ДопСтрокаПоказательПродано);
			КонецЕсли;
		КонецЦикла;
		...
		ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0100107" + "_" + Строка(ПДИнд), ЛистПоказатели["П000010100107"]);
		ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0100104" + "_" + Строка(ПДИнд), ИсточникПрод);
		ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0200302" + "_" + Строка(ПДИнд), "643");
		ПараметрыВыгрузки.Вставить("ПД000" + РазделНомер + "0200203" + "_" + Строка(ПДИнд), ЛистПоказатели["П000010200103"]);
