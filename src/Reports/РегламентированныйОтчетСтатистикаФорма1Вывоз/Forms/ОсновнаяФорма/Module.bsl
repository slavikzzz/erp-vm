...
Процедура ИзменитьПериод(Форма, Шаг)
	...
	Если Месяц(Форма.мДатаКонцаПериодаОтчета) = 12 Тогда
		Форма.мПериодичность = ПредопределенноеЗначение("Перечисление.Периодичность.Год");
	Иначе
		Форма.мПериодичность = ПредопределенноеЗначение("Перечисление.Периодичность.Квартал");
	КонецЕсли;
