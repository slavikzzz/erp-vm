...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		ИначеЕсли Область.Имя = "ОргКодЗнач1" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ОргКодЗнач1_" + Лев(СокрЛП(ПолеТабличногоДокументаФормаОтчета.Области["ОргКодЗнач"].Значение), 1));
...
Процедура СформироватьСпискиВыбора()
	...
	КонецЦикла;
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ОргКодЗнач1_6", 254, , "Выбор кода типа организации по ЛКСД", КоллекцияСписковВыбора["СправочникТиповОрганизацииВСекторе6"]);
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Область.Имя = "ОргКодЗнач1" И Не ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["ОргКодЗнач"].Значение) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю("Код сектора деятельности не выбран: значение в графе 3 кодовой части формы должно равняться 100, 200, 300, 400 или 600!");
		Возврат;
	ИначеЕсли Область.Имя = "ОргКодЗнач1" И ПолеТабличногоДокументаФормаОтчета.Области["ОргКодЗнач"].Значение <> "100"
										  И ПолеТабличногоДокументаФормаОтчета.Области["ОргКодЗнач"].Значение <> "200"
										  И ПолеТабличногоДокументаФормаОтчета.Области["ОргКодЗнач"].Значение <> "300"
										  И ПолеТабличногоДокументаФормаОтчета.Области["ОргКодЗнач"].Значение <> "600"
										  И ПолеТабличногоДокументаФормаОтчета.Области["ОргКодЗнач"].Значение <> "400" Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю("Код сектора деятельности указан неверно: значение в графе 3 кодовой части формы должно равняться 100, 200, 300, 400 или 600!");
		Возврат;
	КонецЕсли;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.СекторДеят) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			...
			ИмяПоляТаблДокумента, "ОргКодЗнач1");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("СекторДеят", СокрЛП(ТабДокумент.ОргКодЗнач1));
