...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Область.Имя = "ОргКодЗнач1" И Не ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["ОргКодЗнач"].Значение) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю("Код сектора деятельности не выбран: значение в графе 3 кодовой части формы должно равняться 1, 2, 3 или 4!", СтатусСообщения.Важное);
		Возврат;
	ИначеЕсли Область.Имя = "ОргКодЗнач1" И ПолеТабличногоДокументаФормаОтчета.Области["ОргКодЗнач"].Значение <> "1"
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю("Код сектора деятельности указан неверно: значение в графе 3 кодовой части формы должно равняться 1, 2, 3 или 4!", СтатусСообщения.Важное);
		Возврат;
	КонецЕсли;
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 15.10.2014 № 612";
	
	ТекстФормулы = "стр.101 >= стр.102";
	Описание     = "стр.101 < стр.102";
	ЛеваяЧасть  = Показатели["П000001010103"];
	ПраваяЧасть = Показатели["П000001010203"];
	...
	ТекстФормулы = "стр.101 >= стр.107";
	Описание     = "стр.101 < стр.107";
	ЛеваяЧасть  = Показатели["П000001010103"];
	ПраваяЧасть = Показатели["П000001010703"];
	Расшифровка = Строка(ЛеваяЧасть) + " >= " + Строка(ПраваяЧасть);
	Если ЛеваяЧасть >= ПраваяЧасть Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	...
	ЛеваяЧасть  = Показатели["П000001010303"];
	ПраваяЧасть = Показатели["П000001010403"];
	Расшифровка = Строка(ЛеваяЧасть) + " >= " + Строка(ПраваяЧасть);
	Если ЛеваяЧасть >= ПраваяЧасть Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	...
	ТекстФормулы = "стр.104 >= стр.105";
	Описание     = "стр.104 < стр.105";
	ЛеваяЧасть  = Показатели["П000001010403"];
	ПраваяЧасть = Показатели["П000001010503"];
	Расшифровка = Строка(ЛеваяЧасть) + " >= " + Строка(ПраваяЧасть);
	Если ЛеваяЧасть >= ПраваяЧасть Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
