		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			...
			ИначеЕсли Сред(Область.Имя, 12, 2) = "04" Тогда
				МассивВыбора = ПараметрВводаПоля.ТаблицаЗначений.НайтиСтроки(Новый Структура("Ключ",
					Строка(ПолеТабличногоДокументаФормаОтчета.Области["П002001020103"].Значение)));
				ТЗВыбора.Очистить();
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Форма.ПолеТабличногоДокументаФормаОтчета.Области["П001001010203"].Значение =
		  Форма.ПолеТабличногоДокументаФормаОтчета.Области["П001001010303"].Значение
		+ Форма.ПолеТабличногоДокументаФормаОтчета.Области["П001001010603"].Значение;
	
	// Строка 101 = Строка 102 + Строка 107.
	Форма.ПолеТабличногоДокументаФормаОтчета.Области["П001001010103"].Значение =
		  Форма.ПолеТабличногоДокументаФормаОтчета.Области["П001001010203"].Значение
		+ Форма.ПолеТабличногоДокументаФормаОтчета.Области["П001001010703"].Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П001001010103"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001010103"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П001001010203"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001010203"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П002001020103",
	512, , "Выбор кода сектора организации по ЛКСД", КоллекцияСписковВыбора["СправочникСекторОрганизации"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П002001020104",
	512, , "Выбор кода типа организации по ЛКСД", КоллекцияСписковВыбора["СправочникТипыОрганизации"]);
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ОбластьСектор = ПолеТабличногоДокументаФормаОтчета.Области["П002001020103"];
	ОбластьТип    = ПолеТабличногоДокументаФормаОтчета.Области["П002001020104"];
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	СтрокаСведений = "ФИОРук, ОКАТО, ОКВЭД2, ОКОПФ";
	...
	ПараметрыВыгрузки.Вставить("СекторДеят", СокрЛП(ТабДокумент.ОргКодЗнач));
