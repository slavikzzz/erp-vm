...
Процедура СформироватьСпискиВыбора()
	...
	НовСтрока = ТаблСписка.Добавить();
	НовСтрока.Код = "3";
	НовСтрока.Название = "Спортивные школы (СШ)";
	
	НовСтрока = ТаблСписка.Добавить();
	НовСтрока.Код = "3.1";
	НовСтрока.Название = "Спортивные школы олимпийского резерва (СШОР)";
	
	НовСтрока = ТаблСписка.Добавить();
	НовСтрока.Код = "3.1.1";
	НовСтрока.Название = "Спортивно-адаптивные школы (САШ)";
	
	НовСтрока = ТаблСписка.Добавить();
	НовСтрока.Код = "3.2";
	НовСтрока.Название = "Центры спортивной подготовки (ЦСП)";
	
	НовСтрока = ТаблСписка.Добавить();
	НовСтрока.Код = "3.3";
	НовСтрока.Название = "Училища олимпийского резерва (УОР)";
	
	КоллекцияСписковВыбора.Вставить("Типы", ТаблСписка);
	
	СвойстваПоказателей.Очистить();
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникТипы",
	512, , "Выбор типа отчитывающейся организации", КоллекцияСписковВыбора["Типы"]);
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 1 По 5 Цикл
		...
		мФормаОтчета.Области["П0000100008" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000100008" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 1 По 5 Цикл
		...
		Области["П0000100008" + Графа].Значение
			= Области["П0000100009" + Графа].Значение
			+ Области["П0000100012" + Графа].Значение
			+ Области["П0000100013" + Графа].Значение
			+ Области["П0000100014" + Графа].Значение;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	СписокСохранения.Свойство("РегистрацияРеспондента", РегистрацияРеспондента);
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант = Неопределено, ВидДокументаНомерКорректировкиИзменен = Неопределено, ПоказателиОтчета = Неопределено) Экспорт
	...
	СписокСохранения.Вставить("РегистрацияРеспондента", РегистрацияРеспондента);
...
Функция ЗаполнитьАвтоНаСервере()
	...
	ПараметрыОтчета.Вставить("ОбособленноеПодразделение",                ОбособленноеПодразделение);
	ПараметрыОтчета.Вставить("ИсключитьДанныеОбособленныхПодразделений", ИсключитьДанныеОбособленныхПодразделений);
	ПараметрыОтчета.Вставить("РегистрацияРеспондента",                   РегистрацияРеспондента);
