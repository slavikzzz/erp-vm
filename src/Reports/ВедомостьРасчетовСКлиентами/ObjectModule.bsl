...
Функция ТекстЗапросаРасчетыПоСрокам(МассивВыбранныхПолей, НастройкиОтчета, ВычисляемыеПоля)
	...
	Для Каждого ИмяПоля Из МассивПолей Цикл
		Если МассивВыбранныхПолей.Найти(ИмяПоля) <> Неопределено Тогда
			Если ИмяПоля = "ДолгКлиентаНачальныйОстаток" Тогда
			...
			ИначеЕсли ИмяПоля = "НашДолгНачальныйОстаток" Тогда
				...
				ЕстьРесурсЭтогоЗапроса = Истина;
			ИначеЕсли ИмяПоля = "ДолгКлиентаКонечныйОстаток" Тогда
				ПоляРесурсовОстаткиОбороты.Добавить("
					|ВЫБОР &ДанныеОтчета
					|	КОГДА 4
	...
	|	ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)                     КАК КорКлиент,
	...
	|	Расчеты.КорКлиент                               КАК КорКлиент,
...
Функция ТекстЗапросаПланОплат(МассивВыбранныхПолей, НастройкиОтчета, ВычисляемыеПоля)
	...
	|	
	|ИЗ
	|	РегистрНакопления.РасчетыСКлиентамиПланОплат.ОстаткиИОбороты({(&НачалоПериода)}, {(&КонецПериода)}, Авто, ,
	|		АналитикаУчетаПоПартнерам В
	|				(ВЫБРАТЬ
	|					ВТ_АналитикаУчетаПоПартнерам.КлючАналитики
	|				ИЗ
...
Функция ТекстЗапросаПланОтгрузок(МассивВыбранныхПолей, НастройкиОтчета, ВычисляемыеПоля)
	...
	Для Каждого ИмяПоля Из МассивПолей Цикл
		Если МассивВыбранныхПолей.Найти(ИмяПоля) <> Неопределено Тогда
			Если ИмяПоля = "ПлановаяОтгрузкаНачальныйОстаток" Тогда
				ПоляРесурсовОстаткиОбороты.Добавить("
					|РасчетыПланОтгрузок.СуммаНачальныйОстаток *
					|		КурсВалюты.Коэффициент КАК ПлановаяОтгрузкаНачальныйОстаток");
				ЕстьРесурсЭтогоЗапроса = Истина;
			ИначеЕсли ИмяПоля = "ПлановаяОтгрузкаКонечныйОстаток" Тогда
				ПоляРесурсовОстаткиОбороты.Добавить("
	...
	|	
	|ИЗ
	|	РегистрНакопления.РасчетыСКлиентамиПланОтгрузок.ОстаткиИОбороты({&НачалоПериода}, {&КонецПериода}, Авто, ,
	|		АналитикаУчетаПоПартнерам В
	|				(ВЫБРАТЬ
	|					ВТ_АналитикаУчетаПоПартнерам.КлючАналитики
	|				ИЗ
...
Функция ТекстЗапросаРасчеты(МассивВыбранныхПолей)
	...
	Для Каждого ИмяПоля Из МассивПолей Цикл
		Если МассивВыбранныхПолей.Найти(ИмяПоля) <> Неопределено Тогда
			Если ИмяПоля = "Оплачивается" Тогда
				ПоляРесурсовОстаткиОбороты.Добавить("
					|РасчетыСКлиентами.ОплачиваетсяОборот * КурсВалюты.Коэффициент КАК Оплачивается");
				ЕстьРесурсЭтогоЗапроса = Истина;
			ИначеЕсли ИмяПоля = "ОплачиваетсяНачальныйОстаток" Тогда
				ПоляРесурсовОстаткиОбороты.Добавить("
					|РасчетыСКлиентами.ОплачиваетсяНачальныйОстаток * КурсВалюты.Коэффициент КАК ОплачиваетсяНачальныйОстаток");
				ЕстьРесурсЭтогоЗапроса = Истина;
			ИначеЕсли ИмяПоля = "ОплачиваетсяКонечныйОстаток" Тогда
				ПоляРесурсовОстаткиОбороты.Добавить("
					|РасчетыСКлиентами.ОплачиваетсяКонечныйОстаток * КурсВалюты.Коэффициент КАК ОплачиваетсяКонечныйОстаток");
				ЕстьРесурсЭтогоЗапроса = Истина;
			ИначеЕсли ИмяПоля = "Отгружается" Тогда
				ПоляРесурсовОстаткиОбороты.Добавить("
					|РасчетыСКлиентами.ОтгружаетсяОборот * КурсВалюты.Коэффициент КАК Отгружается"); 
				ЕстьРесурсЭтогоЗапроса = Истина;
			ИначеЕсли ИмяПоля = "ОтгружаетсяНачальныйОстаток" Тогда
				ПоляРесурсовОстаткиОбороты.Добавить("
					|РасчетыСКлиентами.ОтгружаетсяНачальныйОстаток * КурсВалюты.Коэффициент КАК ОтгружаетсяНачальныйОстаток"); 
				ЕстьРесурсЭтогоЗапроса = Истина;
			ИначеЕсли ИмяПоля = "ОтгружаетсяКонечныйОстаток" Тогда
				ПоляРесурсовОстаткиОбороты.Добавить("
					|РасчетыСКлиентами.ОтгружаетсяКонечныйОстаток * КурсВалюты.Коэффициент КАК ОтгружаетсяКонечныйОстаток"); 
				ЕстьРесурсЭтогоЗапроса = Истина;
			Иначе
				ПоляРесурсовОстаткиОбороты.Добавить(СтрШаблон(ШаблонПоля, "0", ИмяПоля));
			КонецЕсли;
...
Функция ТекстЗапросаВтКурсыВалют()
	...
	|				{(&КонецПериода)},
	|				Валюта = &ВалютаУправленческогоУчета) КАК КурсВалютыУпр
...
Функция ПолучитьТипыДокументовПлана()
	...
	ТекстТиповДокументов = ТекстТиповДокументов + "ТИП(Документ.ЗаказДавальца), ";
	//-- Устарело_Переработка24
	ТекстТиповДокументов = ТекстТиповДокументов + "ТИП(Документ.ЗаказДавальца2_5), ";
	//-- НЕ УТКА
...
Функция ПоляРесурсов()
	...
	МассивПолей.Добавить("Отгружается");
	
	МассивПолей.Добавить("ДолгКлиентаКонечныйОстаток");
	МассивПолей.Добавить("НашДолгКонечныйОстаток");
	МассивПолей.Добавить("ПлановаяОплатаКонечныйОстаток");
	МассивПолей.Добавить("ПлановаяОтгрузкаКонечныйОстаток");
	МассивПолей.Добавить("ОплачиваетсяКонечныйОстаток");
	МассивПолей.Добавить("ОтгружаетсяКонечныйОстаток");
