		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если ВыпУсловиеПоКодуОперации Тогда
				Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000011"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана дата проставления календарного штемпеля пограничной железнодорожной станции или станции назначения, либо станции отправления.",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Если РазделЗаполнен(ДанныеРеестра.Строки, Ложь) Тогда
		...
		ПроверитьКС_13_1_1(СтруктураРезультата);
		ПроверитьКС_13_1_2(СтруктураРезультата);
		ПроверитьКС_13_1_3(СтруктураРезультата);
		ПроверитьКС_13_1_4(СтруктураРезультата);
		ПроверитьКС_13_1_5(СтруктураРезультата);
		ПроверитьКС_13_1_6(СтруктураРезультата);
		ПроверитьКС_13_1_7(СтруктураРезультата);
		ПроверитьКС_13_1_8(СтруктураРезультата);
		ПроверитьКС_13_2_1(СтруктураРезультата);
		ПроверитьКС_13_2_2(СтруктураРезультата);
		ПроверитьКС_13_2_3(СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_13_1_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 13.1.1';
												|en = 'Проверка КС 13.1.1'"));
...
&НаСервере
Процедура ПроверитьКС_13_1_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 13.1.2';
												|en = 'Проверка КС 13.1.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_13_1_3(СтруктураРезультата)
	
	Заголовки		= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		,НСтр("ru = 'Проверка КС 13.1.3';
													|en = 'Проверка КС 13.1.3'"));
	Заголовки.Вставить("ОписаниеКС"			,
		НСтр("ru = 'Если указан код государства отправления (графа 4 Реестра), то он должен соответствовать справочнику ОКСМ';
			|en = 'Если указан код государства отправления (графа 4 Реестра), то он должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонОшибки"		,
		НСтр("ru = 'Указанный код государства отправления товара отсутствует в справочнике ОКСМ';
			|en = 'Указанный код государства отправления товара отсутствует в справочнике ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	,НСтр("ru = 'Указанный код: (""%1"")';
													|en = 'Указанный код: (""%1"")'"));
	Заголовки.Вставить("Показатель"			,НСтр("ru = 'СтранаОтпрКод';
													|en = 'СтранаОтпрКод'"));
	Заголовки.Вставить("ПроверятьПустые"	,Ложь);
	Настройки	= ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	
	ПроверитьКС_СоответствиеСправочникуОКСМ(СтруктураРезультата, Настройки);
	
	Заголовки		= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		,НСтр("ru = 'Проверка КС 13.1.3';
													|en = 'Проверка КС 13.1.3'"));
	Заголовки.Вставить("ОписаниеКС"			,
		НСтр("ru = 'Если указан код государства назначения товара (графа 5 Реестра), то он должен соответствовать справочнику ОКСМ';
			|en = 'Если указан код государства назначения товара (графа 5 Реестра), то он должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонОшибки"		,
		НСтр("ru = 'Указанный код государства назначения товара отсутствует в справочнике ОКСМ';
			|en = 'Указанный код государства назначения товара отсутствует в справочнике ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	,НСтр("ru = 'Указанный код: (""%1"")';
													|en = 'Указанный код: (""%1"")'"));
	Заголовки.Вставить("Показатель"			,НСтр("ru = 'СтранаНазнКод';
													|en = 'СтранаНазнКод'"));
	Заголовки.Вставить("ПроверятьПустые"	,Ложь);
	Настройки	= ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_13_1_4(СтруктураРезультата)
	
	Настройки	= ОбработатьНастройки(,, СтруктураРезультата);
	Настройки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 13.1.4';
												|en = 'Проверка КС 13.1.4'"));
	Настройки.Вставить("ОписаниеКС"		,
		НСтр("ru = 'Проверка заполнения сведений по государству отправления товара (графа 4 Реестра)';
			|en = 'Проверка заполнения сведений по государству отправления товара (графа 4 Реестра)'"));
	Настройки.Вставить("ШаблонОшибки"	,
		НСтр("ru = 'Одновременно отсутствуют сведения о коде и наименовании государства отправления товара';
			|en = 'Одновременно отсутствуют сведения о коде и наименовании государства отправления товара'"));
	Настройки.Вставить("ВидыОпераций"	,Неопределено);
	Настройки.Показатели.Добавить("СтранаОтпрКод");
	Настройки.Показатели.Добавить("СтранаОтпрНаим");
	
	ПроверитьКС_КодИНаименованиеЗаполнены(СтруктураРезультата, Настройки);
	
	Настройки	= ОбработатьНастройки(,, СтруктураРезультата);
	Настройки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 13.1.4';
												|en = 'Проверка КС 13.1.4'"));
	Настройки.Вставить("ОписаниеКС"		,
		НСтр("ru = 'Проверка заполнения сведений по государству назначения товара (графа 5 Реестра)';
			|en = 'Проверка заполнения сведений по государству назначения товара (графа 5 Реестра)'"));
	Настройки.Вставить("ШаблонОшибки"	,
		НСтр("ru = 'Одновременно отсутствуют сведения о коде и наименовании государства назначения товара';
			|en = 'Одновременно отсутствуют сведения о коде и наименовании государства назначения товара'"));
	Настройки.Вставить("ВидыОпераций"	,Неопределено);
	Настройки.Показатели.Добавить("СтранаНазнКод");
	Настройки.Показатели.Добавить("СтранаНазнНаим");
...
&НаСервере
Процедура ПроверитьКС_13_1_5(СтруктураРезультата)
	
	Настройки	= ОбработатьНастройки(,, СтруктураРезультата);
	Настройки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 13.1.5';
												|en = 'Проверка КС 13.1.5'"));
	Настройки.Вставить("ОписаниеКС"		,
		НСтр("ru = 'Проверка заполнения сведений по входной железнодорожной станции (пограничной и (или) припортовой) ';
			|en = 'Проверка заполнения сведений по входной железнодорожной станции (пограничной и (или) припортовой) '") +
		НСтр("ru = '(графы 6 и 7 Реестра), если Код операции = 1010439 или 1010445';
			|en = '(графы 6 и 7 Реестра), если Код операции = 1010439 или 1010445'"));
	Настройки.Вставить("ШаблонОшибки"	,
		НСтр("ru = 'Одновременно отсутствуют сведения о коде входной железнодорожной станции ';
			|en = 'Одновременно отсутствуют сведения о коде входной железнодорожной станции '") +
		НСтр("ru = '(пограничной и (или) припортовой) и о её наименовании';
			|en = '(пограничной и (или) припортовой) и о её наименовании'"));
	ВидыОпераций = Новый Соответствие;
	ВидыОпераций.Вставить("1010439", "1010439");
	ВидыОпераций.Вставить("1010445", "1010445");
	Настройки.Вставить("ВидыОпераций"	,ВидыОпераций);
	Настройки.Показатели.Добавить("СтанцияВхКод");
	Настройки.Показатели.Добавить("СтанцияВхНаим");
...
&НаСервере
Процедура ПроверитьКС_13_1_6(СтруктураРезультата)
	
	Настройки	= ОбработатьНастройки(,, СтруктураРезультата);
	Настройки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 13.1.6';
												|en = 'Проверка КС 13.1.6'"));
	Настройки.Вставить("ОписаниеКС"		,
		НСтр("ru = 'Проверка заполнения сведений по выходной железнодорожной станции (пограничной и ';
			|en = 'Проверка заполнения сведений по выходной железнодорожной станции (пограничной и '") +
		НСтр("ru = '(или) припортовой) (графы 8 и 9 Реестра)';
			|en = '(или) припортовой) (графы 8 и 9 Реестра)'"));
	Настройки.Вставить("ШаблонОшибки"	,
		НСтр("ru = 'Одновременно отсутствуют сведения о коде выходной железнодорожной станции ';
			|en = 'Одновременно отсутствуют сведения о коде выходной железнодорожной станции '") +
		НСтр("ru = '(пограничной и (или) припортовой) и о её наименовании';
			|en = '(пограничной и (или) припортовой) и о её наименовании'"));
	Настройки.Вставить("ВидыОпераций"	,Неопределено);
	Настройки.Показатели.Добавить("СтанцияВыхКод");
	Настройки.Показатели.Добавить("СтанцияВыхНаим");
...
&НаСервере
Процедура ПроверитьКС_13_1_7(СтруктураРезультата)
	
	Настройки	= ОбработатьНастройки(,, СтруктураРезультата);
	Настройки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 13.1.7';
												|en = 'Проверка КС 13.1.7'"));
	Настройки.Вставить("ОписаниеКС"		,
		НСтр("ru = 'Проверка даты помещения товара под таможенную процедуру экспорта, таможенного транзита, реэкспорта ';
			|en = 'Проверка даты помещения товара под таможенную процедуру экспорта, таможенного транзита, реэкспорта '") +
		НСтр("ru = '(графа 10 Реестра), если Код операции = 1010439 или 1010443';
			|en = '(графа 10 Реестра), если Код операции = 1010439 или 1010443'"));
	Настройки.Вставить("ШаблонОшибки"	,
		НСтр("ru = 'Не указана дата помещения товара под таможенную процедуру экспорта, таможенного транзита, ';
			|en = 'Не указана дата помещения товара под таможенную процедуру экспорта, таможенного транзита, '") +
		НСтр("ru = 'реэкспорта при указанном коде операции';
			|en = 'реэкспорта при указанном коде операции'"));
	ВидыОпераций = Новый Соответствие;
	ВидыОпераций.Вставить("1010439", "1010439");
	ВидыОпераций.Вставить("1010443", "1010443");
	Настройки.Вставить("ВидыОпераций"	,ВидыОпераций);
	Настройки.Вставить("Показатель"		,НСтр("ru = 'ДатаЭкспорта';
												|en = 'ДатаЭкспорта'"));
	
	ПроверитьКС_ДатаЗаполнена(СтруктураРезультата, Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверитьКС_13_1_8(СтруктураРезультата)
	
	Настройки	= ОбработатьНастройки(,, СтруктураРезультата);
	Настройки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 13.1.8';
												|en = 'Проверка КС 13.1.8'"));
	Настройки.Вставить("ОписаниеКС"		,
		НСтр("ru = 'Проверка даты проставления календарного штемпеля пограничной железнодорожной станции или станции ';
			|en = 'Проверка даты проставления календарного штемпеля пограничной железнодорожной станции или станции '") +
		НСтр("ru = 'назначения, либо станции отправления (графа 11 Реестра), если Код Операции = 1010444 или 1010445';
			|en = 'назначения, либо станции отправления (графа 11 Реестра), если Код Операции = 1010444 или 1010445'"));
	Настройки.Вставить("ШаблонОшибки"	,
		НСтр("ru = 'Не указана дата проставления календарного штемпеля пограничной железнодорожной станции или станции ';
			|en = 'Не указана дата проставления календарного штемпеля пограничной железнодорожной станции или станции '") +
		НСтр("ru = 'назначения, либо станции отправления при указанном коде операции';
			|en = 'назначения, либо станции отправления при указанном коде операции'"));
	ВидыОпераций = Новый Соответствие;
	ВидыОпераций.Вставить("1010444", "1010444");
	ВидыОпераций.Вставить("1010445", "1010445");
	Настройки.Вставить("ВидыОпераций"	,ВидыОпераций);
	Настройки.Вставить("Показатель"		,НСтр("ru = 'ДатаШтемпеля';
												|en = 'ДатаШтемпеля'"));
...
&НаСервере
Процедура ПроверитьКС_13_2_1(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 13.2.1';
												|en = 'Проверка КС 13.2.1'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_13_2_2(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 13.2.2';
												|en = 'Проверка КС 13.2.2'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_13_2_3(СтруктураРезультата)
	
	Заголовки	= Новый Структура;
	Заголовки.Вставить("ТекстФормулы"	,НСтр("ru = 'Проверка КС 13.2.3';
												|en = 'Проверка КС 13.2.3'"));
	Настройки	= ОбработатьНастройки(	, Заголовки, СтруктураРезультата);
...
&НаСервере
Процедура ПроверитьКС_СоответствиеСправочникуОКСМ(СтруктураРезультата, Настройки)
	
	Настройки.Вставить("Нарушение"			,НСтр("ru = 'Ошибка заполнения';
													|en = 'Ошибка заполнения'"));
	Настройки.Вставить("ШаблонДопСведений"	,НСтр("ru = 'Страница %1, Строка %2';
													|en = 'Страница %1, Строка %2'"));
	Если Не ЗначениеЗаполнено(Настройки.ШаблонОшибки) Тогда
		Настройки.Вставить("ШаблонОшибки"		,НСтр("ru = 'Значение отсутствует в справочнике ОКСМ';
														|en = 'Значение отсутствует в справочнике ОКСМ'"));
	КонецЕсли;
	Настройки.Вставить("ШаблонОшибки2"		,НСтр("ru = 'Значение не заполнено';
	...
	Для каждого СтраницаРаздела Из ДанныеРеестра.Строки Цикл
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл
			...
			Настройки.Вставить("ДопСведенияПараметры", Новый Структура("НомерСтраницы, НомерСтроки", НомерСтраницы, 
				НомерРасчетный));
			Показатели		= ЗаполнитьПоказатели(Настройки.Показатель, ДанныеСтраницы[НомерСтроки - 1].Данные, НомерСтроки);
			ПоказательКода	= Показатели[Настройки.Показатель];
			
			Если Не Настройки.ПроверятьПустые Тогда
				Если Не ЗначениеЗаполнено(ПоказательКода.Значение) Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			...
			РасшифроватьПоказатели(Показатели, Настройки, "Реестр", НомерСтраницы);
			Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
				ПоказательКода.Значение);
			Настройки.ОписаниеОшибки = ?(ЗначениеЗаполнено(ПоказательКода.Значение), Настройки.ШаблонОшибки, 
				Настройки.ШаблонОшибки2);
			Настройки.ЭтоОшибка		
				= УправлениеКонтактнойИнформацией.ДанныеКлассификатораСтранМираПоКоду(ПоказательКода.ЗначениеСтр, "Альфа2") 
				= Неопределено;
			ДобавитьЭлементВСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверитьКС_КодИНаименованиеЗаполнены(СтруктураРезультата, Настройки);
	
	Настройки.Вставить("Нарушение"			,НСтр("ru = 'Ошибка заполнения';
													|en = 'Ошибка заполнения'"));
	Настройки.Вставить("ШаблонРасшифровки"	,НСтр("ru = 'Указанные сведения: (""%1"")';
													|en = 'Указанные сведения: (""%1"")'"));
	Настройки.Вставить("ШаблонДопСведений"	,НСтр("ru = 'Страница %1%2%3';
													|en = 'Страница %1%2%3'"));
	КолвоСтрокРасч			= Новый Соответствие;
	НомерСтраницы			= 0;
	...
	Для каждого СтраницаРаздела Из ДанныеРеестра.Строки Цикл
		...
		Если Не Настройки.ВидыОпераций = Неопределено Тогда
			Настройки.Вставить("ДопСведенияПараметры", Новый Структура("НомерСтраницы", НомерСтраницы));
			Показатели1		= ЗаполнитьПоказатели("КодОперации", СтраницаРаздела.Данные);
			Если Настройки.ВидыОпераций.Получить(Показатели1.КодОперации.Значение) = Неопределено Тогда
				Продолжить;
			КонецЕсли; 
		КонецЕсли;
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл
			...
			НомерРасчетный	= НомерРасчетный + 1;
			Настройки.Вставить("ДопСведенияПараметры", Новый Структура("НомерСтраницы, Ключ, НомерСтроки", НомерСтраницы, 
				", Строка ", НомерРасчетный));
			Показатели2		= ЗаполнитьПоказатели(СтрСоединить(Настройки.Показатели, ", "), 
				ДанныеСтраницы[НомерСтроки - 1].Данные, НомерСтроки);
			ЗарегистрироватьФормулу(Настройки);
			Если Не Настройки.ВидыОпераций = Неопределено Тогда
				РасшифроватьПоказатели(Показатели1, Настройки, "Реестр", НомерСтраницы);
			КонецЕсли;
			РасшифроватьПоказатели(Показатели2, Настройки, "Реестр", НомерСтраницы);
			КодОбъекта		= Показатели2[Настройки.Показатели[0]];
			НаименованиеОбъекта = Показатели2[Настройки.Показатели[1]];
			Настройки.ОписаниеОшибки = Настройки.ШаблонОшибки;
			ПодстрокаРасшифровки = "%1%2%3";
			ПодстрокаРасшифровки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПодстрокаРасшифровки, 
				КодОбъекта.Значение, ",", НаименованиеОбъекта.Значение);
			МассивПодстрок	= СтрРазделить(ПодстрокаРасшифровки, ",", Ложь);
			Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
				СтрСоединить(МассивПодстрок, ", "));
			Настройки.ЭтоОшибка	= Не ЗначениеЗаполнено(КодОбъекта.Значение) И Не 
				ЗначениеЗаполнено(НаименованиеОбъекта.Значение);
			ДобавитьЭлементВСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверитьКС_ДатаЗаполнена(СтруктураРезультата, Настройки);
	...
	Для каждого СтраницаРаздела Из ДанныеРеестра.Строки Цикл
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл
			...
			Настройки.Вставить("ДопСведенияПараметры", Новый Структура("НомерСтраницы, Ключ, НомерСтроки", НомерСтраницы, 
				", Строка ", НомерРасчетный));
			Показатели2		= ЗаполнитьПоказатели(Настройки.Показатель, ДанныеСтраницы[НомерСтроки - 1].Данные, НомерСтроки);
			ЗарегистрироватьФормулу(Настройки);
			Если Не Настройки.ВидыОпераций = Неопределено Тогда
				РасшифроватьПоказатели(Показатели1, Настройки, "Реестр", НомерСтраницы);
			КонецЕсли;
			РасшифроватьПоказатели(Показатели2, Настройки, "Реестр", НомерСтраницы);
			ДатаДокумента	= Показатели2[Настройки.Показатель];
			Настройки.ОписаниеОшибки = Настройки.ШаблонОшибки;
			Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
				ДатаДокумента.ЗначениеСтр);
			Настройки.ЭтоОшибка	= Не ЗначениеЗаполнено(ДатаДокумента.Значение);
			ДобавитьЭлементВСтруктуруОшибок(Настройки);
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	ИначеЕсли УсловныйКодПоказателя = "СтранаОтпрКод" Тогда
		Результат.Вставить("Имя", "П1000041%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Код государства отправления товара';
												|en = 'Код государства отправления товара'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "СтранаОтпрНаим" Тогда
		Результат.Вставить("Имя", "П1000042%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Наименование государства отправления товара';
												|en = 'Наименование государства отправления товара'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "СтранаНазнКод" Тогда
		Результат.Вставить("Имя", "П1000051%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Код государства назначения товара';
												|en = 'Код государства назначения товара'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "СтранаНазнНаим" Тогда
		Результат.Вставить("Имя", "П1000052%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Наименование государства назначения товара';
												|en = 'Наименование государства назначения товара'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "СтанцияВхНаим" Тогда
		Результат.Вставить("Имя", "П1000006%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Наименование входной ж/д станции';
												|en = 'Наименование входной ж/д станции'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "СтанцияВхКод" Тогда
		Результат.Вставить("Имя", "П1000007%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Код входной ж/д станции';
												|en = 'Код входной ж/д станции'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "СтанцияВыхНаим" Тогда
		Результат.Вставить("Имя", "П1000008%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Наименование выходной ж/д станции';
												|en = 'Наименование выходной ж/д станции'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "СтанцияВыхКод" Тогда
		Результат.Вставить("Имя", "П1000009%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Код выходной ж/д станции';
												|en = 'Код выходной ж/д станции'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "ДатаЭкспорта" Тогда
		Результат.Вставить("Имя", "П1000010%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Дата помещения товара под таможенную процедуру экспорта';
												|en = 'Дата помещения товара под таможенную процедуру экспорта'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "ДатаШтемпеля" Тогда
		Результат.Вставить("Имя", "П1000011%1%2");
		Результат.Вставить("Представление", Нстр("ru = 'Дата проставления календарного штемпеля';
												|en = 'Дата проставления календарного штемпеля'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
