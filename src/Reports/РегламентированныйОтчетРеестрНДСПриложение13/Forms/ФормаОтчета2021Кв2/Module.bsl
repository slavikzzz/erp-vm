...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр р.НДС прилож.13 21к2") Тогда
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 8) = "П1000041"
		ИЛИ Лев(Область.Имя, 8) = "П1000042"
		ИЛИ Лев(Область.Имя, 8) = "П1000051"
		ИЛИ Лев(Область.Имя, 8) = "П1000052" Тогда
		...
		Если Лев(Область.Имя, 7) = "П100004" Тогда
			ОбластьКод  = ТабличныйДокумент.Области["П1000041" + Постфикс];
			ОбластьНаим = ТабличныйДокумент.Области["П1000042" + Постфикс];
		Иначе
			ОбластьКод  = ТабличныйДокумент.Области["П1000051" + Постфикс];
...
Процедура ОткрытьФормуВыбораСтраныЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если ОбластьКод.Значение = РеквизитыЭлемента.ДвузначныйБуквенныйКод Тогда
			ОбластьКод.Очистить();
			ОбластьНаим.Очистить();
		Иначе
			ОбластьКод.Значение = РеквизитыЭлемента.ДвузначныйБуквенныйКод;
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрЖДТр_13");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRGDTRNDS"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRGDTRNDS.2"));
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000002"]) Тогда
				...
				+ НомПП + " не задана дата реализации работы (услуги)",
			...
			Если НЕ (ВыпУсловие1 ИЛИ ВыпУсловие2) Тогда
				...
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не задано государство отправления товара",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000041"]) Тогда
				Если СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000041"])) <> 2 Тогда
					ИмяПок = "П1000041" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " неверно задан код государства отправления товара",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
			...
			Если НЕ (ВыпУсловие1 ИЛИ ВыпУсловие2) Тогда
				...
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не задано государство назначения товара",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000051"]) Тогда
				Если СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000051"])) <> 2 Тогда
					ИмяПок = "П1000051" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " неверно задан код государства назначения товара",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
			ВыпУсловиеПоКодуОперации
				= (Ветка.Данные["КодОперации"] = "1010439"
				ИЛИ Ветка.Данные["КодОперации"] = "1010445");
			Если ВыпУсловиеПоКодуОперации Тогда
				ВыпУсловие1 = ЗначениеЗаполнено(Ветка_П10000.Данные["П1000006"]);
				ВыпУсловие2 = ЗначениеЗаполнено(Ветка_П10000.Данные["П1000007"]);
				Если НЕ (ВыпУсловие1 ИЛИ ВыпУсловие2) Тогда
					ИмяПок = "П1000006" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " не задана информация о входной пограничной и (или) припортовой ж/д станции",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
			Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000007"]) Тогда
				Если СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000007"])) <> 6 Тогда
					ИмяПок = "П1000007" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " неверно задан код входной пограничной и (или) припортовой ж/д станции",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
			ВыпУсловие1 = ЗначениеЗаполнено(Ветка_П10000.Данные["П1000008"]);
			ВыпУсловие2 = ЗначениеЗаполнено(Ветка_П10000.Данные["П1000009"]);
			Если НЕ (ВыпУсловие1 ИЛИ ВыпУсловие2) Тогда
				ИмяПок = "П1000008" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не задана информация о выходной пограничной и (или) припортовой ж/д станции",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000009"]) Тогда
				Если СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000009"])) <> 6 Тогда
					ИмяПок = "П1000009" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " неверно задан код выходной пограничной и (или) припортовой ж/д станции",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
			ВыпУсловиеПоКодуОперации
				= (Ветка.Данные["КодОперации"] = "1010439"
				ИЛИ Ветка.Данные["КодОперации"] = "1010443");
			Если ВыпУсловиеПоКодуОперации Тогда
				Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000010"]) Тогда
					ИмяПок = "П1000010" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана дата помещения товара под таможенную процедуру экспорта, таможенного транзита, реэкспорта",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
			ВыпУсловиеПоКодуОперации
				= (Ветка.Данные["КодОперации"] = "1010444"
				ИЛИ Ветка.Данные["КодОперации"] = "1010445");
			Если ВыпУсловиеПоКодуОперации Тогда
				Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000011"]) Тогда
					ИмяПок = "П1000011" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана дата календарного штемпеля пограничной железнодорожной станции или станции назначения либо станции отправления",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрЖДТр_13 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрЖДТр_13");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрЖДТр_13 = СкопироватьУзел(УзелРодитель.Родитель, РеестрЖДТр_13);
		Для каждого Эл Из ЭкзРеестрЖДТр_13.Строки Цикл
		...
		СведЖДУслуг = ПолучитьПодчиненныйЭлемент(ЭкзРеестрЖДТр_13, "СведЖДУслуг");
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведЖДУслуг = СкопироватьУзел(ЭкзРеестрЖДТр_13, СведЖДУслуг, ЭкзРеестрЖДТр_13.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСведЖДУслуг.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
						Если Эл.Код = "НомПрвзДок" Тогда
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_13");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_13_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Реестр НДС Приложение 13");
...
#Область ОбщиеДанные
	...
	ДанныеКС.Вставить("ГрафаНалоговойБазы"		, "12");
...
&НаСервере
Процедура ПроверкаКС_13_1_1()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13.1.1';
													|en = '13.1.1'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 13';
														|en = 'Реестр 13'"));
...
&НаСервере
Процедура ПроверкаКС_13_1_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13.1.2';
													|en = '13.1.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 13';
														|en = 'Реестр 13'"));
...
&НаСервере
Процедура ПроверкаКС_13_1_3а()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13.1.3.а';
													|en = '13.1.3.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 13';
														|en = 'Реестр 13'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
	...
	Настройки.Вставить("ИмяПоказателя", "П1000041_");
	ПроверкаКС_13_1_3_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_13_1_3б()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13.1.3.б';
													|en = '13.1.3.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 13';
														|en = 'Реестр 13'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны назначения в гр.5 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны назначения в гр.5 Реестра должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Код страны %1 соответствует справочнику ОКСМ';
													|en = 'Код страны %1 соответствует справочнику ОКСМ'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный код страны';
														|en = 'Ошибочный код страны'"));
	...
	Настройки.Вставить("ИмяПоказателя", "П1000051_");
	ПроверкаКС_13_1_3_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_13_1_3_код(Настройки)
...
&НаСервере
Процедура ПроверкаКС_13_1_4а()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13.1.4.а';
													|en = '13.1.4.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 13';
														|en = 'Реестр 13'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Из сведений о стране отправления в гр.4 Реестра должны быть указаны код страны отправления ';
			|en = 'Из сведений о стране отправления в гр.4 Реестра должны быть указаны код страны отправления '") +
		НСтр("ru = 'и (или) наименование страны отправления';
			|en = 'и (или) наименование страны отправления'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Сведения о стране отправления %1, %2 заполнены';
													|en = 'Сведения о стране отправления %1, %2 заполнены'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не заполнены сведения о стране отправления';
														|en = 'Не заполнены сведения о стране отправления'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1, Строка %2';
													|en = 'Страница %1, Строка %2'"));
	Настройки = ОбработатьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001", "П1000042_");
	ИменаПоказателей.Вставить("Р01Г002", "П1000041_");
	
	Настройки.Вставить("ИменаПоказателей", ИменаПоказателей);
	ПроверкаКС_13_1_4_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_13_1_4б()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13.1.4.б';
													|en = '13.1.4.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 13';
														|en = 'Реестр 13'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Из сведений о стране назначения в гр.5 Реестра должны быть указаны код страны назначения ';
			|en = 'Из сведений о стране назначения в гр.5 Реестра должны быть указаны код страны назначения '") +
		НСтр("ru = 'и (или) наименование страны назначения';
			|en = 'и (или) наименование страны назначения'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Сведения о стране назначения %1, %2 заполнены';
													|en = 'Сведения о стране назначения %1, %2 заполнены'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не заполнены сведения о стране назначения';
														|en = 'Не заполнены сведения о стране назначения'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1, Строка %2';
													|en = 'Страница %1, Строка %2'"));
	Настройки = ОбработатьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001", "П1000052_");
	ИменаПоказателей.Вставить("Р01Г002", "П1000051_");
...
&НаСервере
Процедура ПроверкаКС_13_1_4_код(Настройки)
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			...
			Настройки.ЭтоОшибка = СтраницаРеестра.РазделЗаполнен И НЕ (ЗначениеЗаполнено(Показатели.Р01Г001.Значение)
				ИЛИ ЗначениеЗаполнено(Показатели.Р01Г002.Значение));
			Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, Показатели.Р01Г001.ЗначениеСтр,
...
&НаСервере
Процедура ПроверкаКС_13_1_5()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13.1.5';
													|en = '13.1.5'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 13';
														|en = 'Реестр 13'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Из сведений о входной ж/д станции в гр.6, 7 Реестра должны быть указаны код входной ж/д станции ';
			|en = 'Из сведений о входной ж/д станции в гр.6, 7 Реестра должны быть указаны код входной ж/д станции '") +
		НСтр("ru = 'и (или) наименование входной ж/д станции, если код операции = 1010439, 1010445';
			|en = 'и (или) наименование входной ж/д станции, если код операции = 1010439, 1010445'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Сведения о ж/д станции %1, %2 заполнены';
													|en = 'Сведения о ж/д станции %1, %2 заполнены'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не заполнены сведения о ж/д станции';
														|en = 'Не заполнены сведения о ж/д станции'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1, Строка %2';
													|en = 'Страница %1, Строка %2'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001", "П1000007_");
	ИменаПоказателей.Вставить("Р01Г002", "П1000006_");
	ИменаПоказателей.Вставить("Р01С001", "КодОперации");
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		...
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			...
			ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01Г001, СтраницаРеестра, Счетчик);
			ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01Г002, СтраницаРеестра, Счетчик);
			РасшифроватьПоказатель(Показатели.Р01С001);
	
			Настройки.ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р01С001.Значение) 
				И НЕ СтрНайти("1010439,1010445", Показатели.Р01С001.Значение) = 0 
				И НЕ (ЗначениеЗаполнено(Показатели.Р01Г001.Значение) ИЛИ ЗначениеЗаполнено(Показатели.Р01Г002.Значение));
			Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, Показатели.Р01Г001.ЗначениеСтр,
				Показатели.Р01Г002.ЗначениеСтр);
			Настройки.ДопСведения = Дополнение;
			ДополнитьСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверкаКС_13_1_6()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13.1.6';
													|en = '13.1.6'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 13';
														|en = 'Реестр 13'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Из сведений о выходной ж/д станции в гр.8, 9 Реестра должны быть указаны код выходной ж/д станции ';
			|en = 'Из сведений о выходной ж/д станции в гр.8, 9 Реестра должны быть указаны код выходной ж/д станции '") +
		НСтр("ru = 'и (или) наименование выходной ж/д станции';
			|en = 'и (или) наименование выходной ж/д станции'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Сведения о ж/д станции %1, %2 заполнены';
													|en = 'Сведения о ж/д станции %1, %2 заполнены'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не заполнены сведения о ж/д станции';
														|en = 'Не заполнены сведения о ж/д станции'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001", "П1000009_");
	ИменаПоказателей.Вставить("Р01Г002", "П1000008_");
...
&НаСервере
Процедура ПроверкаКС_13_1_7()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13.1.7';
													|en = '13.1.7'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 13';
														|en = 'Реестр 13'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Дата отметки таможенного органа в гр.10 Реестра должна быть заполнена, если код операции ';
			|en = 'Дата отметки таможенного органа в гр.10 Реестра должна быть заполнена, если код операции '") +
		НСтр("ru = '= 1010439, 1010443';
			|en = '= 1010439, 1010443'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Дата отметки таможенного органа %1 заполнена';
													|en = 'Дата отметки таможенного органа %1 заполнена'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не заполнена дата отметки таможенного органа';
														|en = 'Не заполнена дата отметки таможенного органа'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1, Строка %2';
													|en = 'Страница %1, Строка %2'"));
	Заголовки.Вставить("ШаблонДополнения"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001", "П1000010_");
	ИменаПоказателей.Вставить("Р01С001", "КодОперации");
	
	Настройки.Вставить("ИменаПоказателей", ИменаПоказателей);
	Настройки.Вставить("КодыОпераций", "1010439,1010443");
	ПроверкаКС_13_1_8_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_13_1_8()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13.1.8';
													|en = '13.1.8'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 13';
														|en = 'Реестр 13'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Дата отметки таможенного органа в гр.11 Реестра должна быть заполнена, если код операции ';
			|en = 'Дата отметки таможенного органа в гр.11 Реестра должна быть заполнена, если код операции '") +
		НСтр("ru = '= 1010444, 1010445';
			|en = '= 1010444, 1010445'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Дата отметки таможенного органа %1 заполнена';
													|en = 'Дата отметки таможенного органа %1 заполнена'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не заполнена дата отметки таможенного органа';
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001", "П1000011_");
	ИменаПоказателей.Вставить("Р01С001", "КодОперации");
	
	Настройки.Вставить("ИменаПоказателей", ИменаПоказателей);
	Настройки.Вставить("КодыОпераций", "1010444,1010445");
	ПроверкаКС_13_1_8_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_13_1_8_код(Настройки)
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		...
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			...
			Настройки.ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р01С001.Значение) 
				И НЕ СтрНайти(Настройки.КодыОпераций, Показатели.Р01С001.Значение) = 0 
				И НЕ ЗначениеЗаполнено(Показатели.Р01Г001.Значение);
			Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, Показатели.Р01Г001.ЗначениеСтр);
			Настройки.ДопСведения = Дополнение;
			ДополнитьСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверкаКС_13_2_1а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13.2.1.а';
													|en = '13.2.1.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 13';
														|en = 'Реестр 13'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_13_2_1б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13.2.1.б';
													|en = '13.2.1.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 13';
														|en = 'Реестр 13'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_13_2_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13.2.2';
													|en = '13.2.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 13';
														|en = 'Реестр 13'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
