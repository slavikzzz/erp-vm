...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы110 Тогда
		Подстрока = "номер декларации";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный номер декларации?';
							|en = 'Удалить выбранный номер декларации?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				ДлинаРегНомТД = СтрДлина(СокрЛП(Ветка_П11000.Данные["П1100010"]));
				ИмяПок = "П1100010" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
				Если ВыпУсловиеПоКодуОперации И ДлинаРегНомТД = 0 Тогда
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан номер декларации на товары, по которой товары помещались под таможенную процедуру экспорта,"
					+ " реэкспорта или таможенного транзита", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли ДлинаРегНомТД > 0 И НЕ (ДлинаРегНомТД = 23 ИЛИ (ДлинаРегНомТД >= 25 И ДлинаРегНомТД <= 29)) Тогда
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " неверно задан номер декларации на товары, по которой товары помещались под таможенную процедуру экспорта,"
					+ " реэкспорта или таможенного транзита", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				// 11000.
				РегНомТД = ПолучитьПодчиненныйЭлемент(ЭкзСведЖДУслуг, "РегНомТД");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					ЭкзРегНомТД = СкопироватьУзел(ЭкзСведЖДУслуг, РегНомТД, ЭкзСведЖДУслуг.Строки.Количество() - 1);
	...
	Если ВыбранныеСтраницы.Найти("Реестр") <> Неопределено Тогда
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				// Особая обработка для графы 10.
				МасТекЗнач = Новый Массив;
				Для каждого Строка11 Из Строки11 Цикл
					Если ЗначениеЗаполнено(Строка11.Данные.П1100010) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100010));
					КонецЕсли;
				КонецЦикла;
				СтрокаДокумент.Параметры.П1100010 = СтрСоединить(МасТекЗнач, "; ");
