...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = "0609255";
...
Процедура ЗаполнитьСведенияОбОрганизацииНаСервере(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	ЗаполнитьСведенияОбОрганизацииДляОтчетаСтатистики(ЭтотОбъект, ВПрограммеИзмененаДатаПодписи, , Истина);
...
Процедура ЗаполнитьСведенияОбОрганизацииДляОтчетаСтатистики(Форма, ВПрограммеИзмененаДатаПодписи = Ложь, ИмяПоля = "ПолеТабличногоДокументаФормаОтчета", СхемаНаимПолноеКраткое = Ложь) Экспорт
	...
	Если (НЕ ЗначениеЗаполнено(Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргКодОКТМО.Значение))
		ИЛИ (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) Тогда
		Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргКодОКТМО.Значение = ОКТМО;
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Стр = 1 По 4 Цикл
		...
		Для Гр = 6 По 8 Цикл
			...
			ТекСумма = ТекСумма + мФормаОтчета.Области["П00100100" + НомСтр + Графа].Значение;
		КонецЦикла;
		мФормаОтчета.Области["П00100100" + НомСтр + "05"].Значение = ТекСумма;
	КонецЦикла;
	...
	Для Гр = 3 По 8 Цикл
		...
		Для Стр = 2 По 4 Цикл
			...
			ТекСумма = ТекСумма + мФормаОтчета.Области["П00100100" + НомСтр + Графа].Значение;
		КонецЦикла;
		мФормаОтчета.Области["П0010010001" + Графа].Значение = ТекСумма;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 3 По 8 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		мФормаОтчета.Области["П0010010001" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0010010001" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
		Если Гр = 5 Тогда
			Для Стр = 2 По 4 Цикл
				мФормаОтчета.Области["П001001000" + Стр + Графа].ЦветФона = ЦветФонаЗначение;
				мФормаОтчета.Области["П001001000" + Стр + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого Обл Из ОбластиСтроки Цикл
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 26-ЖКХ");
	КонецЦикла;
