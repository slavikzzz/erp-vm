...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "03/08/2015";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для НомКол = 3 По 5 Цикл
		...
		ТекСумма = 0;
		Для НомСтр = 26 По 29 Цикл
			Если Области.Найти("П00000100" + НомСтр + НомКолСтр) <> Неопределено Тогда 
				ТекСумма = ТекСумма + Области["П00000100" + НомСтр + НомКолСтр].Значение;
			КонецЕсли;
		КонецЦикла;
		Области["П0000010025" + НомКолСтр].Значение = ТекСумма;
		
		ТекСумма = 0;
		Для НомСтр = 18 По 23 Цикл
			ТекСумма = ТекСумма + Области["П00000100" + НомСтр + НомКолСтр].Значение;
		КонецЦикла;
		Области["П0000010013" + НомКолСтр].Значение = ТекСумма;
		
		ТекСумма = 0;
		Для НомСтр = 4 По 12 Цикл
			ТекСумма = ТекСумма + Области["П00000100" + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + НомКолСтр].Значение;
		КонецЦикла;
		Области["П0000010003" + НомКолСтр].Значение = ТекСумма;
		
		Области["П0000010001" + НомКолСтр].Значение = Области["П0000010002" + НомКолСтр].Значение + Области["П0000010003" + НомКолСтр].Значение
													+ Области["П0000010013" + НомКолСтр].Значение + Области["П0000010024" + НомКолСтр].Значение;
	КонецЦикла;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если Метаданные.Отчеты.Найти("РегламентированныйОтчетСтатистикаПрочиеФормы") <> Неопределено Тогда 
	...
	Для НомКол = 3 По 5 Цикл
		// Стр.01 >= стр.25.
		Если Не (ПараметрыВыгрузки["П0000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			>= ПараметрыВыгрузки["П0000010025" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			...
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Стр.13 > = сумме стр. 14 -:- 17.
		Если Не (ПараметрыВыгрузки["П0000010013" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			>= ПараметрыВыгрузки["П0000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			 + ПараметрыВыгрузки["П0000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			 + ПараметрыВыгрузки["П0000010016" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			 + ПараметрыВыгрузки["П0000010017" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				ЭтаФорма, "Не выполнено условие контроля: по гр. " + Формат(НомКол, "ЧГ=") + " значение в стр. 13 должно быть больше или равно сумме значений в стр. с 14 по 17!",
	...
	Если ЗначениеЗаполнено(ПараметрыВыгрузки.ОКАТО) Тогда
		...
		Если СтрНайти("86, 87, 98, 93, 30, 11, , 44, 47, 64, 77", Лев(СокрЛП(ПараметрыВыгрузки.ОКАТО), 2)) > 0
			...
			Для НомКол = 3 По 5 Цикл
				Если Не (ПараметрыВыгрузки["П0000010030" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
					= ПараметрыВыгрузки["П0000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
						ЭтаФорма, "Не выполнено условие контроля: если код территории: 86, 87, 98, 93, 30, 11, 11100, 44, 47, 64, 71100, 71140, 77, то значение в стр. 30 должно быть равно значению в стр. 01 по гр. " + Формат(НомКол, "ЧГ=") + "!",
		...
		Если СтрНайти("84, 81, 76, 04, 57, 05, 08, 10, 25, 69, 71", Лев(СокрЛП(ПараметрыВыгрузки.ОКАТО), 2)) > 0 Тогда
			Для НомКол = 3 По 5 Цикл
				Если Не (0 < ПараметрыВыгрузки["П0000010030" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
					И ПараметрыВыгрузки["П0000010030" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] < ПараметрыВыгрузки["П0000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
						ЭтаФорма, "Не выполнено условие контроля: если код территории: 84, 81, 76, 04, 57, 05, 08, 10, 25, 69, 71, то 0 < стр. 30 < стр. 01 по гр. " + Формат(НомКол, "ЧГ=") + "!",
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Для Каждого ЭлемСтруктуры Из ТабДокумент Цикл
		ПараметрыВыгрузки.Вставить(ЭлемСтруктуры.Ключ, ЭлемСтруктуры.Значение);
	КонецЦикла;
