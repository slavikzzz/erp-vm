...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "14/01/2014";
	// Код отчета, равен коду по ОКУД.
	СтруктураРеквизитовФормы.мКодОтчета = "0608037";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для НомКол = 3 По 5 Цикл
		// Стр.03 = сумме стр. 04, 05, 06, 07, 08, 09, 10, 11.
		ТекСумма = 0;
		...
		КонецЦикла;
		мФормаОтчета.Области["П0000010003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = ТекСумма;
		// Стр.12 = сумме стр. 17 -:- 22.
		ТекСумма = 0;
		Для НомСтр = 17 По 22 Цикл
			ТекСумма = ТекСумма + мФормаОтчета.Области["П000001"
				+ Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=")
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		мФормаОтчета.Области["П0000010012" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = ТекСумма;
		// Стр.25 = сумме стр. 26 -:- 31.
		ТекСумма = 0;
		Для НомСтр = 26 По 31 Цикл
			// Учитываем защищенные ячейки с установленным символом "X".
			Если НомКол = 3 И НомСтр >= 27 И НомСтр <= 29 Тогда
				Продолжить;
			КонецЕсли;
			ТекСумма = ТекСумма + мФормаОтчета.Области["П000001"
				+ Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=")
				+ Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		мФормаОтчета.Области["П0000010025" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = ТекСумма;
		// Стр.01 = сумме стр.02, 03, 12, 23, 24.
		мФормаОтчета.Области["П0000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			= мФормаОтчета.Области["П0000010002" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			+ мФормаОтчета.Области["П0000010003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			+ мФормаОтчета.Области["П0000010012" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			+ мФормаОтчета.Области["П0000010023" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
			+ мФормаОтчета.Области["П0000010024" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 3-приватизация");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "3Приватизация");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКАТО) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Не задан код ОКАТО организации!",
	...
	// КОНТРОЛЬ показателей формы (Указания по заполнению Формы 3-приватизация).
	Для НомКол = 3 По 5 Цикл
		// Стр.01 >= стр.25.
		Если Не (ПараметрыВыгрузки["ПР000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				ЭтаФорма, "Не выполнено условие контроля: по гр. " + Формат(НомКол, "ЧГ=") + " значение в стр. 01 должно быть больше или равно значению в стр. 25!",
				ИмяПоляТаблДокумента, "П0000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Стр.12 > = сумме стр. 13 -:- 16.
		Если Не (ПараметрыВыгрузки["ПР000010012" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			>= ПараметрыВыгрузки["ПР000010013" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			 + ПараметрыВыгрузки["ПР000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			 + ПараметрыВыгрузки["ПР000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			 + ПараметрыВыгрузки["ПР000010016" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				ЭтаФорма, "Не выполнено условие контроля: по гр. " + Формат(НомКол, "ЧГ=") + " значение в стр. 12 должно быть больше или равно сумме значений в стр. с 13 по 16!",
				ИмяПоляТаблДокумента, "П0000010012" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	Если ЗначениеЗаполнено(ПараметрыВыгрузки.ОКАТО) Тогда
		// Если код территории: 86, 87, 98, 93, 30, 11, 11100, 44, 47, 64, 71100, 71140, 77, то: стр.32 = стр.01.
		// Вся территория указанных субъектов Российской Федерации отнесена к районам Крайнего Севера или местностям, приравненным к ним.
		Если СтрНайти("86, 87, 98, 93, 30, 11, 44, 47, 64, 77", Лев(СокрЛП(ПараметрыВыгрузки.ОКАТО), 2)) > 0
			Или СтрНайти("11100, 71100, 71140", Лев(СокрЛП(ПараметрыВыгрузки.ОКАТО), 5)) > 0 Тогда
			Для НомКол = 3 По 5 Цикл
				Если Не (ПараметрыВыгрузки["ПР000010032" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
					...
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
						ЭтаФорма, "Не выполнено условие контроля: если код территории: 86, 87, 98, 93, 30, 11, 11100, 44, 47, 64, 71100, 71140, 77, то значение в стр. 32 должно быть равно значению в стр. 01 по гр. " + Формат(НомКол, "ЧГ=") + "!",
						ИмяПоляТаблДокумента, "П0000010032" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		// Если код территории: 84, 81, 76, 04, 57, 05, 08, 10, 25, 69, 71,  то: 0 < стр.32 < стр.01.
		// Территория указанных субъектов Российской Федерации частично отнесена к районам Крайнего Севера или местностям, приравненным к ним.
		Если СтрНайти("84, 81, 76, 04, 57, 05, 08, 10, 25, 69, 71", Лев(СокрЛП(ПараметрыВыгрузки.ОКАТО), 2)) > 0 Тогда
			Для НомКол = 3 По 5 Цикл
				Если Не (0 < ПараметрыВыгрузки["ПР000010032" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
					И ПараметрыВыгрузки["ПР000010032" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] < ПараметрыВыгрузки["ПР000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
						ЭтаФорма, "Не выполнено условие контроля: если код территории: 84, 81, 76, 04, 57, 05, 08, 10, 25, 69, 71, то 0 < стр. 32 < стр. 01 по гр. " + Формат(НомКол, "ЧГ=") + "!",
						ИмяПоляТаблДокумента, "П0000010032" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЦикла;
