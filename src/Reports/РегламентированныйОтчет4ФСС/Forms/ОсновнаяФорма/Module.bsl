...
Процедура ПоказатьПериод(Форма)
	...
	Иначе
		...
		НСтр("ru = 'С 01.01.2023 Постановлением Правления ПФР от 31.10.2022 N 245п введена единая форма ЕФС-1';
			|en = 'С 01.01.2023 Постановлением Правления ПФР от 31.10.2022 N 245п введена единая форма ЕФС-1'"),
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
		...
		МодульОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
			...
			НСтр("ru = 'Новости: Расчетная ведомость по средствам ФСС (форма 4-ФСС)';
				|en = 'Новости: Расчетная ведомость по средствам ФСС (форма 4-ФСС)'"),
			Ложь,
