...
Функция ВыгрузкаВозможна(ИмяФормы,
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.НомерСтрахователя) Тогда
		...
		НСтр("ru = 'Не задан номер страхователя';
			|en = 'Не задан номер страхователя'"), "Титульный", "ОргФССКод");
		СтатусПроверки = Ложь;
	...
	ИначеЕсли НЕ СтрДлина(СокрЛП(ПараметрыВыгрузки.НомерСтрахователя)) = 10 Тогда
		...
		НСтр("ru = 'Длина регистрационного номера страхователя должна быть равна 10-ти символам';
			|en = 'Длина регистрационного номера страхователя должна быть равна 10-ти символам'"), "Титульный", "ОргФССКод");
		СтатусПроверки = Ложь;
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.КодПодчиненности) Тогда
		...
		НСтр("ru = 'Не задан код подчиненности';
			|en = 'Не задан код подчиненности'"), "Титульный", "КодПодчиненности");
		СтатусПроверки = Ложь;
	...
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.КодПодчиненности) <> 5 Тогда
		...
		НСтр("ru = 'Задан код подчиненности, не состоящий из 5 цифр';
			|en = 'Задан код подчиненности, не состоящий из 5 цифр'"), "Титульный", "КодПодчиненности");
		СтатусПроверки = Ложь;
	...
	Иначе
		...
		Если ПоследнийСимвол <> "1" И ПоследнийСимвол <> "2" И ПоследнийСимвол <> "3" И ПоследнийСимвол <> "4" Тогда
			...
			НСтр("ru = 'Код подчиненности должен заканчиваться на цифру 1, 2, 3 или 4';
				|en = 'Код подчиненности должен заканчиваться на цифру 1, 2, 3 или 4'"),
	...
	Если НЕ ПустаяСтрока(СокрЛП(СтрЗаменить(ПараметрыВыгрузки.ОКВЭД, ".", "")))
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
		НСтр("ru = 'На титульном листе указано некорректное значение кода вида деятельности по ОКВЭД';
			|en = 'На титульном листе указано некорректное значение кода вида деятельности по ОКВЭД'"), "Титульный", "ОКВЭД");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2022Кв2") Тогда
		...
		Если ЗаполненаТаблица1_1 Тогда
			...
			Если НЕ ЗаполненаХотяБыОднаСКЕ Тогда
				...
				|en = 'Таблица 1.1 должна содержать сведения по выделенным самостоятельным классификационным единицам'"),
				"Таблица1_1", ИмяПок);
				СтатусПроверки = Ложь;
		...
		Если ДанныеТаблица1["П000010000503"] = 0 Тогда
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				РезультатВыгрузки, НСтр("ru = 'В таблице 1, в строке 5 не указан размер страхового тарифа в соответствии с классом профессионального риска.';
										|en = 'В таблице 1, в строке 5 не указан размер страхового тарифа в соответствии с классом профессионального риска.'"),
