...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0615069";
	...
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы1, 9);
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено 
		...
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
			СтруктураРеквизитовФормы.Организация, , "ФИО");
		ПолеТабличногоДокументаФормаОтчета.Области.ОргНазв.Значение = СокрЛП(СведенияОбОрганизации.ФИО);		
	КонецЕсли;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 Тогда
		УдалитьСтроку("");
	ИначеЕсли Область.Имя = "П000000000101" Или Область.Имя = "П000000000201" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникДат");
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Область.Имя = "П000000000101" 
		Или Область.Имя = "П000000000201" Тогда
...
&НаКлиентеНаСервереБезКонтекста
Процедура ПеренумерацияСтрокТаблицы(Форма, Область)
	
	Области = Форма.ПолеТабличногоДокументаФормаОтчета.Области;
	
	ОбластьИмя = Лев(Область.Имя, 13);
	
	Если НеобходимаяОбластьДляПеренумерации(ОбластьИмя) Тогда
		 
		НПП = 0;		
		Для Стр = 1 По Форма["П0000200001"].Количество() Цикл
			
			Если НаличиеЗначенияВСтрокеТаблицы(Области, Стр) Тогда
				НПП = НПП + 1;
				Области["П000020000101_" + Стр].Значение = НПП;
			Иначе
				Области["П000020000101_" + Стр].Значение = 0;
			КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция НеобходимаяОбластьДляПеренумерации(ОбластьИмя)
	
	Возврат ОбластьИмя = "П000020000102"
		Или ОбластьИмя = "П000020000103"
		Или ОбластьИмя = "П000020000104"
		Или ОбластьИмя = "П000020000105"
		Или ОбластьИмя = "УдалитьСтроку";

КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция НаличиеЗначенияВСтрокеТаблицы(Области, Стр)

	Возврат ЗначениеЗаполнено(Области["П000020000102_" + Стр].Значение)
		Или ЗначениеЗаполнено(Области["П000020000103_" + Стр].Значение)
		Или ЗначениеЗаполнено(Области["П000020000104_" + Стр].Значение)
		Или ЗначениеЗаполнено(Области["П000020000105_" + Стр].Значение);
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Области["П000010001003"].Значение = Области["П000010001004"].Значение
		+ Области["П000010001005"].Значение + Области["П000010001006"].Значение
		+ Области["П000010001007"].Значение + Области["П000010001008"].Значение
		+ Области["П000010001009"].Значение;
...
Процедура ОткрытьФормуВыбораСтраницЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеАвторасчета <> СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
		...
		Если СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет = Ложь Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения(
				"ОткрытьФормуВыбораСтраницЗавершениеВопросЗавершение", ЭтотОбъект);
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П000010001003"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000010001003"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-ИП (автогруз)");
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверки.Добавить(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3"));
		МассивПроверки.Добавить(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть4"));
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникДат",
	512, , "Выбор даты", КоллекцияСписковВыбора["Даты"]);
