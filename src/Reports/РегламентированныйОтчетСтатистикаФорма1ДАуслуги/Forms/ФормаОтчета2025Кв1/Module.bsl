...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли РегламентированнаяОтчетностьКлиент.ВыбраннаяОбластьРегламентированногоОтчетаНеВерна(ЭтаФорма, Область, СтандартнаяОбработка) Тогда
		Возврат;
	ИначеЕсли ТекГруппа = "П00100" Тогда
		...
		КонецЕсли;
	ИначеЕсли ТекГруппа = "П00200" Тогда
		...
		КонецЕсли;
	ИначеЕсли ТекГруппа = "П00300" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		УстановитьЗначокВГруппеЯчеек(Область, 8, 18, 1, 1, Истина);
	ИначеЕсли ТекГруппа = "П00400" Тогда
		
		Если Область.Защита Тогда
			Возврат;
		КонецЕсли;
		
		Если Лев(Область.Имя, 8) = "П0040010" Тогда
			СтандартнаяОбработка = Ложь;
			Модифицированность = Истина;
			УстановитьЗначокВГруппеЯчеек(Область, 19, 21, 1, 1);
		ИначеЕсли Лев(Область.Имя, 8) = "П0040020" Тогда
			Если Прав(Область.Имя, 1) = "3" Тогда
				СтандартнаяОбработка = Ложь;
				Модифицированность = Истина;
				УстановитьЗначокВГруппеЯчеек(Область, 22, 24, 3, 3, Истина);
			Иначе
				СтандартнаяОбработка = Ложь;
				Модифицированность = Истина;
				НомерСтроки = Число(Сред(Область.Имя, 10, 2));
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П001001000101", 250, ,
...
Процедура ДополнитьПараметрыВыгрузки(ПараметрыВыгрузки)
	...
	Для Стр = 2 По 7 Цикл
		...
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П00200100" + СтрСтр + "03"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД3010" + СтрСтр + "01", "138");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00200100" + СтрСтр + "04"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД3010" + СтрСтр + "01", "139");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00200100" + СтрСтр + "05"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД3010" + СтрСтр + "01", "140");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П00200100" + СтрСтр + "06"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД3010" + СтрСтр + "02", "28");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00200100" + СтрСтр + "07"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД3010" + СтрСтр + "02", "2");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00200100" + СтрСтр + "08"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД3010" + СтрСтр + "02", "29");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П00200100" + СтрСтр + "09"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД3010" + СтрСтр + "03", "28");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00200100" + СтрСтр + "10"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД3010" + СтрСтр + "03", "2");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00200100" + СтрСтр + "11"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД3010" + СтрСтр + "03", "29");
		КонецЕсли;
	КонецЦикла;
	
	Для Стр = 22 По 24 Цикл
		СтрСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
		
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П00400200" + СтрСтр + "04"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД4010" + СтрСтр + "01", "138");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00400200" + СтрСтр + "05"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД4010" + СтрСтр + "01", "139");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00400200" + СтрСтр + "06"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД4010" + СтрСтр + "01", "140");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П00400200" + СтрСтр + "07"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД4010" + СтрСтр + "02", "28");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00400200" + СтрСтр + "08"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД4010" + СтрСтр + "02", "2");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00400200" + СтрСтр + "09"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД4010" + СтрСтр + "02", "29");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П00400200" + СтрСтр + "10"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД4010" + СтрСтр + "03", "28");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00400200" + СтрСтр + "11"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД4010" + СтрСтр + "03", "2");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00400200" + СтрСтр + "12"]) Тогда
			ПараметрыВыгрузки.Вставить("ПД4010" + СтрСтр + "03", "29");
		КонецЕсли;
	КонецЦикла;
