...
Процедура ДоступностьРазделов()
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <> 12 Тогда
		Для НомСтр = 11 По 15 Цикл
			ПолеТабличногоДокументаФормаОтчета.Области["П00004010" + НомСтр + "03"].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области["П00004010" + НомСтр + "03"].Защита = Истина;
			Если СтруктураРеквизитовФормы.мСкопированаФорма <> Неопределено Тогда
				ПолеТабличногоДокументаФормаОтчета.Области["П00004010" + НомСтр + "03"].Значение = "";
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Если СтрНачинаетсяС(ПолеТабличногоДокументаФормаОтчета.Области["П000020100101"].Значение, "79") Тогда
		ЦветРаздела6             = Новый Цвет(255, 255, 192);
		ЗащитаРаздела6           = Ложь;
		ОчиститьЗначениеРаздела6 = Ложь;
	Иначе
		ЦветРаздела6             = Новый Цвет();
		ЗащитаРаздела6           = Истина;
		ОчиститьЗначениеРаздела6 = Истина;
	КонецЕсли;
	
	Для НомСтр = 26 По 28 Цикл
		ПолеТабличногоДокументаФормаОтчета.Области["П00006010" + НомСтр + "03"].ЦветФона = ЦветРаздела6;
		ПолеТабличногоДокументаФормаОтчета.Области["П00006010" + НомСтр + "03"].Защита   = ЗащитаРаздела6;
		Если ОчиститьЗначениеРаздела6 Тогда
			ПолеТабличногоДокументаФормаОтчета.Области["П00006010" + НомСтр + "03"].Значение = "";
		КонецЕсли;
	КонецЦикла;
	Для НомСтр = 29 По 31 Цикл
		Для Гр = 3 По 12 Цикл
			Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
			ПолеТабличногоДокументаФормаОтчета.Области["П00006020" + НомСтр + Графа].ЦветФона = ЦветРаздела6;
			ПолеТабличногоДокументаФормаОтчета.Области["П00006020" + НомСтр + Графа].Защита   = ЗащитаРаздела6;
			Если ОчиститьЗначениеРаздела6 Тогда
				ПолеТабличногоДокументаФормаОтчета.Области["П00006020" + НомСтр + Графа].Значение = "";
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	Если СтрНачинаетсяС(ПолеТабличногоДокументаФормаОтчета.Области["П000020100101"].Значение, "68.31.") Тогда
		ЦветРаздела7             = Новый Цвет(255, 255, 192);
		ЗащитаРаздела7           = Ложь;
		ОчиститьЗначениеРаздела7 = Ложь;
	Иначе
		ЦветРаздела7             = Новый Цвет();
		ЗащитаРаздела7           = Истина;
		ОчиститьЗначениеРаздела7 = Истина;
	КонецЕсли;
	
	Для НомСтр = 32 По 33 Цикл
		ПолеТабличногоДокументаФормаОтчета.Области["П00007010" + НомСтр + "03"].ЦветФона = ЦветРаздела7;
		ПолеТабличногоДокументаФормаОтчета.Области["П00007010" + НомСтр + "03"].Защита   = ЗащитаРаздела7;
		Если ОчиститьЗначениеРаздела7 Тогда
			ПолеТабличногоДокументаФормаОтчета.Области["П00007010" + НомСтр + "03"].Значение = "";
		КонецЕсли;
	КонецЦикла;
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант = Неопределено, ВидДокументаНомерКорректировкиИзменен = Неопределено, ПоказателиОтчета = Неопределено) Экспорт
	...
	Если НЕ РегламентированнаяОтчетность.ПриЗаписиРегламентированногоОтчетаНаСервере(ЭтотОбъект, КодИФНС, Автосохранение,
...
Функция ОпределитьИдМногострочногоРазделаПоАктивнойСтранице(Форма, ИмяЛиста = Неопределено, ТекущаяСтраница = Неопределено, ИдГрупп = Неопределено)
	...
	Если ИмяТекущейСтраницыПанели = "ФормаОтчета" Тогда
		...
		ИмяГруппы = РегламентированнаяОтчетностьКлиентСервер.РазделТекущейОбласти(Форма,
			Форма.Элементы.ПолеТабличногоДокументаФормаОтчета.ТекущаяОбласть);
...
Процедура ВывестиРазделВТабличныйДокумент(Форма, ИмяГруппы, СтруктураГруппы)
	...
	Если ТекущееКоличествоСтрокГруппы > ИсходноеКоличествоСтрокГруппы Тогда
		...
		РегламентированнаяОтчетностьКлиентСервер.ДобавитьНеобходимоеКоличествоДопСтрок(Форма, ТекТабличноеПоле,
			ТекТабличноеПолеИмя, ИсходноеКоличествоСтрокГруппы, ТекущееКоличествоСтрокГруппы,
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.Имя = "ОргАдрес" Тогда
		РегламентированнаяОтчетностьКлиент.ОбработкаАдресаВСтатистическомОтчетеJSON(
			ЭтотОбъект, Область, СтандартнаяОбработка);
	ИначеЕсли ТекГруппа = "П00001" Тогда
		СтандартнаяОбработка = Ложь;
		УстановитьЗначокВГруппеЯчеек(Область, 1, 1, 3, 6);
		Модифицированность = Истина;
	ИначеЕсли ТекГруппа = "П00002" Тогда
		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			ВвестиПоказатель(Элемент, Область, ПараметрВводаПоля.Показатель, Ложь, ПараметрВводаПоля.ТекстПриВыборе,
				ПараметрВводаПоля.ТаблицаЗначений, СтандартнаяОбработка, "Код");
		КонецЕсли;
	ИначеЕсли ТекГруппа = "П00003" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		НомерСтроки = Число(Сред(Область.Имя, 10, 2));
		Если СтрНайти("345", Прав(Область.Имя, 1)) > 0 Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 3, 5);
		ИначеЕсли СтрНайти("678", Прав(Область.Имя, 1)) > 0 Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 6, 8);
		ИначеЕсли СтрНайти("9", Прав(Область.Имя, 1)) > 0
			  Или СтрНайти("10", Прав(Область.Имя, 2)) > 0
			  Или СтрНайти("11", Прав(Область.Имя, 2)) > 0 Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 9, 11);
		КонецЕсли;
	ИначеЕсли ТекГруппа = "П00004" Тогда
		...
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		УстановитьЗначокВГруппеЯчеек(Область, 11, 15, 3, 3, Истина);
	ИначеЕсли ТекГруппа = "П00005" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		УстановитьЗначокВГруппеЯчеек(Область, 16, 25, 3, 3, Истина);
	ИначеЕсли ТекГруппа = "П00006" Тогда
		Если Область.Защита Тогда
			Возврат;
		КонецЕсли;
		Если Лев(Область.Имя, 8) = "П0000601" Тогда
			СтандартнаяОбработка = Ложь;
			Модифицированность = Истина;
			УстановитьЗначокВГруппеЯчеек(Область, 26, 28, 3, 3);
		ИначеЕсли Лев(Область.Имя, 8) = "П0000602" Тогда
			Если Прав(Область.Имя, 1) = "3" Тогда
				СтандартнаяОбработка = Ложь;
				Модифицированность = Истина;
				УстановитьЗначокВГруппеЯчеек(Область, 29, 31, 3, 3, Истина);
			Иначе
				СтандартнаяОбработка = Ложь;
				Модифицированность = Истина;
				НомерСтроки = Число(Сред(Область.Имя, 10, 2));
				Если СтрНайти("456", Прав(Область.Имя, 1)) > 0 Тогда
					УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 4, 6);
				ИначеЕсли СтрНайти("789", Прав(Область.Имя, 1)) > 0 Тогда
					УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 7, 9);
				ИначеЕсли СтрНайти("10", Прав(Область.Имя, 1)) > 0
					  Или СтрНайти("11", Прав(Область.Имя, 2)) > 0
					  Или СтрНайти("12", Прав(Область.Имя, 2)) > 0 Тогда
					УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 10, 12);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли Область.Имя = "П00006" Тогда
		Если Область.Защита Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьДоступностьЭУВыбораОбособленныхПодразделений(Форма)
	Форма.ОбособленноеПодразделение = ?(НЕ Форма.ИсключитьДанныеОбособленныхПодразделений,
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-ДА (услуги)");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020100101", 250, ,
		"Выбор кода ОКВЭД", КоллекцияСписковВыбора["СправочникОКВЭД"]);
КонецПроцедуры
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Область.Значение = Строка(РезультатВыбора["Код"]);
...
Процедура ДополнитьПараметрыВыгрузки(ПараметрыВыгрузки)
	Если ЗначениеЗаполнено(ПараметрыВыгрузки.П000010100103) Тогда 
		ПараметрыВыгрузки.Вставить("П0000101001", "28");
	ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки.П000010100104) Тогда 
		ПараметрыВыгрузки.Вставить("П0000101001", "88");
	ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки.П000010100105) Тогда 
		ПараметрыВыгрузки.Вставить("П0000101001", "79");
	ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки.П000010100106) Тогда 
		ПараметрыВыгрузки.Вставить("П0000101001", "62");
	КонецЕсли;
	
	Для Стр = 3 По 10 Цикл 
		СтрСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
		
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П00003010" + СтрСтр + "03"]) Тогда 
			ПараметрыВыгрузки.Вставить("П03010" + СтрСтр + "01", "138");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00003010" + СтрСтр + "04"]) Тогда 
			ПараметрыВыгрузки.Вставить("П03010" + СтрСтр + "01", "139");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00003010" + СтрСтр + "05"]) Тогда 
			ПараметрыВыгрузки.Вставить("П03010" + СтрСтр + "01", "140");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П00003010" + СтрСтр + "06"]) Тогда 
			ПараметрыВыгрузки.Вставить("П03010" + СтрСтр + "02", "28");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00003010" + СтрСтр + "07"]) Тогда 
			ПараметрыВыгрузки.Вставить("П03010" + СтрСтр + "02", "2");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00003010" + СтрСтр + "08"]) Тогда 
			ПараметрыВыгрузки.Вставить("П03010" + СтрСтр + "02", "29");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П00003010" + СтрСтр + "09"]) Тогда 
			ПараметрыВыгрузки.Вставить("П03010" + СтрСтр + "03", "28");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00003010" + СтрСтр + "10"]) Тогда 
			ПараметрыВыгрузки.Вставить("П03010" + СтрСтр + "03", "2");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00003010" + СтрСтр + "11"]) Тогда 
			ПараметрыВыгрузки.Вставить("П03010" + СтрСтр + "03", "29");
		КонецЕсли;
	КонецЦикла;
	
	Для Стр = 29 По 31 Цикл 
		СтрСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
		
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П00006020" + СтрСтр + "04"]) Тогда 
			ПараметрыВыгрузки.Вставить("П06010" + СтрСтр + "01", "138");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00006020" + СтрСтр + "05"]) Тогда 
			ПараметрыВыгрузки.Вставить("П06010" + СтрСтр + "01", "139");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00006020" + СтрСтр + "06"]) Тогда 
			ПараметрыВыгрузки.Вставить("П06010" + СтрСтр + "01", "140");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П00006020" + СтрСтр + "07"]) Тогда 
			ПараметрыВыгрузки.Вставить("П06010" + СтрСтр + "02", "28");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00006020" + СтрСтр + "08"]) Тогда 
			ПараметрыВыгрузки.Вставить("П06010" + СтрСтр + "02", "2");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00006020" + СтрСтр + "09"]) Тогда 
			ПараметрыВыгрузки.Вставить("П06010" + СтрСтр + "02", "29");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П00006020" + СтрСтр + "10"]) Тогда 
			ПараметрыВыгрузки.Вставить("П06010" + СтрСтр + "03", "28");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00006020" + СтрСтр + "11"]) Тогда 
			ПараметрыВыгрузки.Вставить("П06010" + СтрСтр + "03", "2");
		ИначеЕсли ЗначениеЗаполнено(ПараметрыВыгрузки["П00006020" + СтрСтр + "12"]) Тогда 
			ПараметрыВыгрузки.Вставить("П06010" + СтрСтр + "03", "29");
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
