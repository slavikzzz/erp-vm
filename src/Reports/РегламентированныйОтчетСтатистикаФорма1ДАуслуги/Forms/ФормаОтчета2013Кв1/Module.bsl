...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "28/06/2013";
	// Код отчета, равен коду по ОКУД
	СтруктураРеквизитовФормы.мКодОтчета   = "0609708";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если Не БезОткрытияФормы Тогда
		// В случае подачи отчета за II или IV квартал, установим защиту и цвет по умолчанию для раздела 4 Формы 1-ДА (услуги).
		ДоступностьРазделов();
	КонецЕсли;
...
Процедура ДоступностьРазделов()
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6 Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
		Для НомСтр = 29 По 33 Цикл
			Для НомКол = 3 По 5 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П000004" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].ЦветФона = Новый Цвет();
				ПолеТабличногоДокументаФормаОтчета.Области["П000004" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Защита = Истина;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
&НаСервере
Процедура ФормированиеТаблицыВыбораВидаДеятельности(НомСтрВидаДеятельности)
	
	ТЗВыб = СвойстваПоказателей.НайтиСтроки(Новый Структура("Показатель","П000001"))[0].ТаблицаЗначений.Выгрузить();
	ТЗВыб.ЗаполнитьЗначения(Ложь, "ПризнакОтбора");
	
	МассивОтбора = СвойстваПоказателей.НайтиСтроки(Новый Структура("Показатель","П000000"))[0].ТаблицаЗначений.Выгрузить(Новый Структура("Название", НомСтрВидаДеятельности)).Скопировать(Новый Структура("ПризнакОтбора", Ложь), "Код");
	...
	ЗначениеВДанныеФормы(ТЗВыб.Скопировать(Новый Структура("ПризнакОтбора", Истина), "Код,Название"), ТЗВыбораВидаДеятельности);
...
Процедура ВосстановитьСохраненныеДанные()
	...
	// Восстановим выделенный вид деятельности (показатель Раздела 1 Формы 1-ДА (услуги)).
	СписокСохранения.Свойство( "ВидДеятельности", ВидДеятельности);
	Если ВидДеятельности <> Неопределено Тогда
		ПолеТабличногоДокументаФормаОтчета.Области[ВидДеятельности].Текст = "(" + ПолеТабличногоДокументаФормаОтчета.Области[ВидДеятельности].Текст + ")";
	КонецЕсли;
	...
	// Дополнительно, для Формы 1-ДА (услуги).
	Если СтруктураРеквизитовФормы.мСкопированаФорма <> Неопределено Тогда
		Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6 Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
			Для НомСтр = 29 По 33 Цикл
				Для НомКол = 3 По 5 Цикл
					ПолеТабличногоДокументаФормаОтчета.Области["П000004" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
				КонецЦикла;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант = Неопределено, ВидДокументаНомерКорректировкиИзменен = Неопределено, ПоказателиОтчета = Неопределено) Экспорт
	...
	// Дополнительно, для Формы 1-ДА (услуги):
	// вставляем выделенный вид деятельности (показатель Раздела 1).
	Для НомСтр = 2 По 16 Цикл
		Если СтрДлина(ПолеТабличногоДокументаФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Текст) = 4 Тогда
			СписокСохранения.Вставить("ВидДеятельности", ПолеТабличногоДокументаФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Имя);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.Имя = "T" Или СтрНайти(Область.Имя, "R") = 1 Тогда 
	...
	ИначеЕсли ТекГруппа = "П000001" И Сред(Область.Имя, 12, 2) = "03" Тогда
		СтандартнаяОбработка = Ложь;
		Если СтрДлина(Область.Текст) = 2 Тогда
			// Ввод кода ОКВЭД (для выгрузки).
			ПредыдущийКодОКВЭД = мФормаОтчета.Области["КодОКВЭД"].Значение;
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, ТекГруппа);
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				ФормированиеТаблицыВыбораВидаДеятельности(Сред(Область.Имя, 10, 2));
				ДополнительныеПоказатели = Новый Структура("ТекГруппа, мФормаОтчета, ПредыдущийКодОКВЭД", ТекГруппа, мФормаОтчета, ПредыдущийКодОКВЭД);
				ВвестиПоказатель(Элемент, Область, ПараметрВводаПоля.Показатель, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбораВидаДеятельности, СтандартнаяОбработка, "Код", ДополнительныеПоказатели);
				Возврат;	
			КонецЕсли;
			Если ПредыдущийКодОКВЭД <> мФормаОтчета.Области["КодОКВЭД"].Значение Тогда
				Область.Текст = "(" + Область.Текст + ")";
				НомИсключение = Число(Сред(Область.Имя, 10, 2));
				// Убираем скобки из текста предыдущего выделенного показателя.
				Для НомСтр = 2 по 16 Цикл
					Если НомСтр = НомИсключение Тогда
						Продолжить;
					КонецЕсли;
					Если СтрДлина(мФормаОтчета.Области[ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Текст) = 4 Тогда
						мФормаОтчета.Области[ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Текст = Сред(мФормаОтчета.Области[ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Текст, 2, 2);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		Иначе
			Область.Текст = Сред(Область.Текст, 2, 2);
			мФормаОтчета.Области["КодОКВЭД"].Значение = "";
		КонецЕсли;
		Модифицированность = Истина;
	ИначеЕсли ТекГруппа = "П000002" Тогда
		СтандартнаяОбработка = Ложь;
		УстановитьЗначокВГруппеЯчеек(Область, 3, 3, 2);
		Модифицированность = Истина;
	ИначеЕсли ТекГруппа = "П000003" Тогда
		...
		Для НомКол = 3 по 6 Цикл
			Если НомКол = 4 Тогда
				продолжить;
			КонецЕсли;
			Для НомСтр = 26 По 28 Цикл
				Если Область.Имя = ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") Тогда
					мФормаОтчета.Области[ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение 
					= ?(мФормаОтчета.Области[ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X", "", "X");
					Если мФормаОтчета.Области[ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X" Тогда
						Для ТекНомСтр = 26 По 28 Цикл
							Если ТекНомСтр = НомСтр Тогда
								Продолжить;
							КонецЕсли;
							мФормаОтчета.Области[ТекГруппа + Формат(ТекНомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
						КонецЦикла;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
	...
	ИначеЕсли ТекГруппа = "П000004" Тогда
		Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 3 Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 9 Тогда
			СтандартнаяОбработка = Ложь;
			УстановитьЗначокВГруппеЯчеек(Область, 1, 3, 2);
			Модифицированность = Истина;
		КонецЕсли;
	ИначеЕсли ТекГруппа = "П000005" Тогда
		СтандартнаяОбработка = Ложь;
		Для НомСтр = 34 по 45 Цикл
			Если Область.Имя = ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03" Тогда
				мФормаОтчета.Области[ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Значение
				= ?(мФормаОтчета.Области[ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Значение = "X", "", "X");
			КонецЕсли;
		КонецЦикла;
		Модифицированность = Истина;
	ИначеЕсли ТекГруппа = "П000106" Тогда
		СтандартнаяОбработка = Ложь;
		Для НомСтр = 46 по 48 Цикл
			Если Область.Имя = ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03" Тогда
				мФормаОтчета.Области[ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Значение
				= ?(мФормаОтчета.Области[ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Значение = "X", "", "X");
			Иначе
		...
		КонецЦикла;
		Модифицированность = Истина;
	ИначеЕсли ТекГруппа = "П000206" Тогда
		СтандартнаяОбработка = Ложь;
		Если Сред(Область.Имя, 12, 2) = "03" Тогда
			...
			КонецЦикла;
		Иначе
			УстановитьЗначокВГруппеЯчеек(Область, 3, 3, 3);
		КонецЕсли;
		Модифицированность = Истина;
	ИначеЕсли ТекГруппа = "П000107" Тогда
		СтандартнаяОбработка = Ложь;
		Если Сред(Область.Имя, 12, 2) = "03" Тогда
			Для НомСтр = 52 по 63 Цикл
				Если Область.Имя = ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03" Тогда
					мФормаОтчета.Области[ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Значение
					= ?(мФормаОтчета.Области[ТекГруппа + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Значение = "X", "", "X");
				КонецЕсли;
		...
		КонецЕсли;
		Модифицированность = Истина;
	ИначеЕсли ТекГруппа = "П000207" Тогда
		СтандартнаяОбработка = Ложь;
		СтрокаРаздела = Лев(Область.Имя, 11);
		Смещение = 2;
		Для НомИтерации = 1 По 2 Цикл
			Если Область.Имя = СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 4, "ЧЦ=2; ЧВН=; ЧГ=") Тогда
				мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 4, "ЧЦ=2; ЧВН=; ЧГ=")].Значение 
				= ?(мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 4, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X", "", "X");
				Если мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 4, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X" Тогда
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 3, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 2, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 1, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение,     "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
				КонецЕсли;
			КонецЕсли;
			Если Область.Имя = СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 3, "ЧЦ=2; ЧВН=; ЧГ=") Тогда
				мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 3, "ЧЦ=2; ЧВН=; ЧГ=")].Значение 
				= ?(мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 3, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X", "", "X");
				Если мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 3, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X" Тогда
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 4, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 2, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 1, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение,     "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
				КонецЕсли;
			КонецЕсли;
			Если Область.Имя = СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 2, "ЧЦ=2; ЧВН=; ЧГ=") Тогда
				мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 2, "ЧЦ=2; ЧВН=; ЧГ=")].Значение 
				= ?(мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 2, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X", "", "X");
				Если мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 2, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X" Тогда
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 4, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 3, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 1, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение,     "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
				КонецЕсли;
			КонецЕсли;
			Если Область.Имя = СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 1, "ЧЦ=2; ЧВН=; ЧГ=") Тогда
				мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 1, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
				= ?(мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 1, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X", "", "X");
				Если мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение - 1, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X" Тогда
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 4, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 3, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 2, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение,     "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
				КонецЕсли;
			КонецЕсли;
			Если Область.Имя = СтрокаРаздела + Формат(5 * НомИтерации + Смещение, "ЧЦ=2; ЧВН=; ЧГ=") Тогда
				мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
				= ?(мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X", "", "X");
				Если мФормаОтчета.Области[СтрокаРаздела + Формат(5 * НомИтерации + Смещение,     "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "X" Тогда
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 4, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 3, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 2, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
					мФормаОтчета.Области[СтрокаРаздела  + Формат(5 * НомИтерации + Смещение - 1, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = "";
...
Процедура Очистить() Экспорт
	...
	Для НомСтр = 2 по 16 Цикл
		Если СтрДлина(ПолеТабличногоДокументаФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Текст) = 4 Тогда
			ПолеТабличногоДокументаФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Текст = Сред(ПолеТабличногоДокументаФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Текст, 2, 2);
			ПолеТабличногоДокументаФормаОтчета.Области["КодОКВЭД"].Значение = "";
		КонецЕсли;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-ДА (услуги)");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1ДАуслуги");
	...
	// Дополнительно, для Формы 1-ДА (услуги):
	// удаление строки с Кодом ОКВЭД (Раздел 1), предназначенным для выгрузки (не для печати).
	СтруктураРеквизитовФормы.мПечатныеФормы[1].Значение.УдалитьОбласть(СтруктураРеквизитовФормы.мПечатныеФормы[1].Значение.Области["НеПечатаемаяЧасть"], ТипСмещенияТабличногоДокумента.ПоВертикали);
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000000", 250, ,                                               , КоллекцияСписковВыбора["СправочникПриложениеДляВводаОКВЭД"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001", 250, , "Выбор кода ОКВЭД (предназначен для выгрузки)", КоллекцияСписковВыбора["СправочникОКВЭД"]);
	
КонецПроцедуры

&НаКлиенте
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код", ДополнительныеПоказатели = Неопределено) Экспорт
	
	ТекГруппа = ДополнительныеПоказатели.ТекГруппа;
	мФормаОтчета = ДополнительныеПоказатели.мФормаОтчета;
	ПредыдущийКодОКВЭД = ДополнительныеПоказатели.ПредыдущийКодОКВЭД;
	...
	ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура("Код", ПолеТабличногоДокументаФормаОтчета.Области["КодОКВЭД"].Значение));
	
	ДополнительныеПараметры = Новый Структура("мФормаОтчета, Область, ПредыдущийКодОКВЭД, ТекГруппа", мФормаОтчета, Область, ПредыдущийКодОКВЭД, ТекГруппа);
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	мФормаОтчета = ДополнительныеПараметры.мФормаОтчета;
	Область = ДополнительныеПараметры.Область;
	ПредыдущийКодОКВЭД = ДополнительныеПараметры.ПредыдущийКодОКВЭД;
	ТекГруппа = ДополнительныеПараметры.ТекГруппа;
	
	Если РезультатВыбора = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПолеТабличногоДокументаФормаОтчета.Области["КодОКВЭД"].Значение = Строка(РезультатВыбора["Код"]);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.КодОКВЭД) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не указан основной вид деятельности организации!",
		ИмяПоляТаблДокумента, "П000001000203");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	// КОНТРОЛЬ показателей формы 1-ДА (услуги).
	// По разделу 2.
	Для НомСтр = 17 По 25 Цикл
		НомКолНач = 3;
		НомКолКон = 5;
		Пока НомКолКон <= 11 Цикл
			ОтмеченоВариантовОтвета = 0;
			Для НомКол = НомКолНач По НомКолКон Цикл
				Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"]) Тогда
					ОтмеченоВариантовОтвета = ОтмеченоВариантовОтвета + 1;
				КонецЕсли;
			КонецЦикла;
			Если ОтмеченоВариантовОтвета <> 1 Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: раздел 2 - в графах " + НомКолНач + "-" + НомКолКон + " по строке " + НомСтр + " должен быть один вариант ответа!",
				ИмяПоляТаблДокумента, "П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКолНач, "ЧЦ=2; ЧВН=; ЧГ="));
				СтатусПроверки = Ложь;
			КонецЕсли;
			НомКолНач = НомКолНач + 3;
			НомКолКон = НомКолКон + 3;
		КонецЦикла;
	КонецЦикла;
	// По разделу 3.
	Для НомКол = 3 По 6 Цикл
		...
		Если ОтмеченоВариантовОтвета <> 1 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: раздел 3 - в графе " + НомКол + " должен быть один вариант ответа!",
			ИмяПоляТаблДокумента, "П0000030026" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// По разделу 4.
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 3 Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 9 Тогда
		Для НомСтр = 29 По 33 Цикл
			ОтмеченоВариантовОтвета = 0;
			Для НомКол = 3 По 5 Цикл
				Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000004" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"]) Тогда
					ОтмеченоВариантовОтвета = ОтмеченоВариантовОтвета + 1;
				КонецЕсли;
			КонецЦикла;
			Если ОтмеченоВариантовОтвета <> 1 Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: раздел 4 - в графах 3-5 по строке " + НомСтр + " должен быть один вариант ответа!",
				ИмяПоляТаблДокумента, "П000004" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	ОтмеченоВариантовОтвета = 0;
	Для НомСтр = 34 По 45 Цикл
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000005" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
			ОтмеченоВариантовОтвета = ОтмеченоВариантовОтвета + 1;
		КонецЕсли;
	КонецЦикла;
	Если ОтмеченоВариантовОтвета = 0 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: раздел 5 - необходимо указать один или несколько вариантов ответа!",
		ИмяПоляТаблДокумента, "П000005003403");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// По разделу 6.
	// Туристская деятельность (за исключением экскурсионной деятельности).
	Если Лев(СокрЛП(ПараметрыВыгрузки.КодОКВЭД), 4) = "63.3" И СокрЛП(ПараметрыВыгрузки.КодОКВЭД) <> "63.30.4" Тогда
		// По разделу 6.1.
		ОтмеченоВариантовОтвета = 0;
		Для НомСтр = 46 По 48 Цикл
			Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000106" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
				ОтмеченоВариантовОтвета = ОтмеченоВариантовОтвета + 1;
			КонецЕсли;
		КонецЦикла;
		Если ОтмеченоВариантовОтвета = 0 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: раздел 6.1 - необходимо указать один вариант ответа!",
			ИмяПоляТаблДокумента, "П000106004603");
			СтатусПроверки = Ложь;
		КонецЕсли;
		// По разделу 6.2.
		Для НомСтр = 49 По 51 Цикл
			Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000206" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
				Продолжить;
			КонецЕсли;
			НомКолНач = 4;
			НомКолКон = 6;
			Пока НомКолКон <= 12 Цикл
				ОтмеченоВариантовОтвета = 0;
				Для НомКол = НомКолНач По НомКолКон Цикл
					Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000206" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"]) Тогда
						ОтмеченоВариантовОтвета = ОтмеченоВариантовОтвета + 1;
					КонецЕсли;
				КонецЦикла;
				Если ОтмеченоВариантовОтвета <> 1 Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: раздел 6.2 - в графах " + НомКолНач + "-" + НомКолКон + " по строке " + НомСтр + " один вариант ответа!",
					ИмяПоляТаблДокумента, "П000206" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКолНач, "ЧЦ=2; ЧВН=; ЧГ="));
					СтатусПроверки = Ложь;
				КонецЕсли;
				НомКолНач = НомКолНач + 3;
				НомКолКон = НомКолКон + 3;
	...
	Если Лев(СокрЛП(ПараметрыВыгрузки.КодОКВЭД), 7) = "70.31.1" Тогда
		// По разделу 7.1.
		ЗаполненаОднаИзСтрок52_57 = Ложь;
		ЗаполненаОднаИзСтрок52_63 = Ложь;
		ПроверитьСтроку64 = Ложь;
		ПроверитьСтроку65 = Ложь;
		ПроверитьСтроку66 = Ложь;
		ПроверитьСтроку67 = Ложь;
		ПроверитьСтроку68 = Ложь;
		ПроверитьСтроку69 = Ложь;
		Для НомСтр = 52 По 63 Цикл
			Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000107" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч"]) Тогда
				Продолжить;
			Иначе
				ЗаполненаОднаИзСтрок52_63 = Истина;
				// Условия последующего контроля раздела 7.2.
				Если НомСтр >=52 И НомСтр <= 57 Тогда
					ЗаполненаОднаИзСтрок52_57 = Истина;
				КонецЕсли;
				Если НомСтр = 52 Тогда
					ПроверитьСтроку64 = Истина;
				ИначеЕсли НомСтр = 53 Тогда
					ПроверитьСтроку65 = Истина;
				ИначеЕсли НомСтр = 54 Тогда
					ПроверитьСтроку66 = Истина;
				ИначеЕсли НомСтр = 55 Тогда
					ПроверитьСтроку67 = Истина;
				ИначеЕсли НомСтр = 56 Тогда
					ПроверитьСтроку68 = Истина;
				ИначеЕсли НомСтр = 57 Тогда
					ПроверитьСтроку69 = Истина;
				КонецЕсли;
			КонецЕсли;
			НомКолНач = 4;
			НомКолКон = 6;
			Пока НомКолКон <= 12 Цикл
				ОтмеченоВариантовОтвета = 0;
				Для НомКол = НомКолНач По НомКолКон Цикл
					Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000107" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"]) Тогда
						ОтмеченоВариантовОтвета = ОтмеченоВариантовОтвета + 1;
					КонецЕсли;
				КонецЦикла;
				Если ОтмеченоВариантовОтвета <> 1 Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: раздел 7.1 - в графах " + НомКолНач + "-" + НомКолКон + " по строке " + НомСтр + " один вариант ответа!",
					ИмяПоляТаблДокумента, "П000107" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКолНач, "ЧЦ=2; ЧВН=; ЧГ="));
					СтатусПроверки = Ложь;
				КонецЕсли;
				НомКолНач = НомКолНач + 3;
				НомКолКон = НомКолКон + 3;
			КонецЦикла;
		КонецЦикла;
		Если Не ЗаполненаОднаИзСтрок52_63 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: если заполнена строка 16 раздела 1, то заполняется раздел 7.1!",
			ИмяПоляТаблДокумента, "П000107005203");
			СтатусПроверки = Ложь;
		КонецЕсли;
		Если ЗаполненаОднаИзСтрок52_57 Тогда
			// По разделу 7.2.
			Для НомСтр = 64 По 69 Цикл
				Если Не ((НомСтр = 64 И ПроверитьСтроку64) Или (НомСтр = 65 И ПроверитьСтроку65) Или (НомСтр = 66 И ПроверитьСтроку66)
					Или (НомСтр = 67 И ПроверитьСтроку67) Или (НомСтр = 68 И ПроверитьСтроку68) Или (НомСтр = 69 И ПроверитьСтроку69)) Тогда
					Продолжить;
				КонецЕсли;
				НомКолНач = 3;
				НомКолКон = 7;
				Пока НомКолКон <= 12 Цикл
					ОтмеченоВариантовОтвета = 0;
					Для НомКол = НомКолНач По НомКолКон Цикл
						Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000207" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"]) Тогда
							ОтмеченоВариантовОтвета = ОтмеченоВариантовОтвета + 1;
						КонецЕсли;
					КонецЦикла;
					Если ОтмеченоВариантовОтвета <> 1 Тогда
						РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
						"Не выполнено условие контроля: раздел 7.2 - в графах " + НомКолНач + "-" + НомКолКон + " по строке " + НомСтр + " один вариант ответа!",
						ИмяПоляТаблДокумента, "П000207" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКолНач, "ЧЦ=2; ЧВН=; ЧГ="));
						СтатусПроверки = Ложь;
					КонецЕсли;
					НомКолНач = НомКолНач + 5;
					НомКолКон = НомКолКон + 5;
				КонецЦикла;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	// Дополнительно, для Формы 1-ДА (услуги).
	ПараметрыВыгрузки.Вставить("КодОКВЭД", СокрЛП(ТабДокумент.КодОКВЭД));
	...
	Для НомСтр = 17 По 25 Цикл
		...
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")) Цикл
			...
			?(ТабДокумент["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = "X", 1, ""));
			НомКол = НомКол + 1;
		КонецЦикла;
	КонецЦикла;
	// По разделу 3.
	Для НомСтр = 26 По 28 Цикл
		...
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")) Цикл
			...
			?(ТабДокумент["П000003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = "X", 1, ""));
			НомКол = НомКол + 1;
			НомКол = ?(НомКол = 4, 5, НомКол);
		КонецЦикла;
	КонецЦикла;
	// По разделу 4.
	Для НомСтр = 29 По 33 Цикл
		...
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000004" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")) Цикл
			...
			?(ТабДокумент["П000004" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = "X", 1, ""));
			НомКол = НомКол + 1;
		КонецЦикла;
	КонецЦикла;
	// По разделу 5.
	Для НомСтр = 34 По 45 Цикл
		Если РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000005" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03") Тогда
			ПараметрыВыгрузки.Вставить("П000005" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч",
			?(ТабДокумент["П000005" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"] = "X", 1, ""));
		Иначе
			Продолжить;
		КонецЕсли;
	КонецЦикла;
	// По разделу 6.1.
	Для НомСтр = 46 По 48 Цикл
		Если РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000106" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03") Тогда
			ПараметрыВыгрузки.Вставить("П000106" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03_Ч",
			?(ТабДокумент["П000106" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"] = "X", 1, ""));
		Иначе
			Продолжить;
		КонецЕсли;
	КонецЦикла;
	...
	Для НомСтр = 49 По 51 Цикл
		НомКол = 3;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000206" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")) Цикл
			ПараметрыВыгрузки.Вставить("П000206" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч",
			?(ТабДокумент["П000206" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = "X", 1, ""));
			НомКол = НомКол + 1;
		КонецЦикла;
	КонецЦикла;
	// По разделу 7.1.
	Для НомСтр = 52 По 63 Цикл
		НомКол = 3;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000107" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")) Цикл
			ПараметрыВыгрузки.Вставить("П000107" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч",
			?(ТабДокумент["П000107" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = "X", 1, ""));
			НомКол = НомКол + 1;
		КонецЦикла;
	КонецЦикла;
	// По разделу 7.2.
	Для НомСтр = 64 По 69 Цикл
		НомКол = 3;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000207" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")) Цикл
			ПараметрыВыгрузки.Вставить("П000207" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч",
			?(ТабДокумент["П000207" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = "X", 1, ""));
			НомКол = НомКол + 1;
		КонецЦикла;
	КонецЦикла;
