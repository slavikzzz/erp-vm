...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "12/10/2017";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "КодОКВЭД" Тогда
		ПредыдущийКодОКВЭД = мФормаОтчета.Области["КодОКВЭД"].Значение;
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "П000001");
	...
	ИначеЕсли ТекГруппа = "П000001" И Сред(Область.Имя, 12, 2) = "03" Тогда
		...
		Если СтрДлина(Область.Текст) = 2 Тогда
			НовТекст = "(" + Область.Текст + ")";
		Иначе
			НовТекст = Сред(Область.Текст, 2, 2);
		КонецЕсли;
		Для НомСтр = 2 по 16 Цикл
