...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	// Группа П20000.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П2000019");
	ИменаГраф.Добавить("П2000020");
	ИменаГраф.Добавить("П2000021");
	ИменаГраф.Добавить("П2000022");
	ИменаГраф.Добавить("П2000023");
...
Процедура ИнициализироватьМногоуровневыеРазделы()
	...
	Для каждого Элем Из мСтруктураМногоуровневыхРазделов Цикл
		...
		ИначеЕсли ИмяРаздела = "Таблица2" Тогда
			...
			СтруктураСуммПоКодуОперации.Вставить("ИтогоСуммаГрафа23", 0);
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяТекРаздела)
	...
	Иначе
		...
		ИначеЕсли ИмяТекРаздела = "Таблица2" Тогда
			...
			ОбрабОбласти.Добавить(ТДО["ИтогоСуммаГрафа23"]);
	...
	Если мСтруктураМногоуровневыхРазделов.Свойство(НаимРаздела) Тогда
		...
		Если НЕ СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
			...
			ИначеЕсли НаимРаздела = "Таблица2" Тогда
				...
				ИтогоСуммаГрафа23 = ТабличныйДокумент.Области["ИтогоСуммаГрафа23"].Значение;
				...
				Для каждого Ветка Из ДеревоДанныхРаздела.Строки Цикл
					Если Ветка.Данные["КодОперации"] = КодОперации Тогда
						...
						Ветка.Данные["ИтогоСуммаГрафа23"] = ИтогоСуммаГрафа23;
...
Функция ДанныеТабличногоДокумента(НаимРаздела)
	...
	Если мСтруктураМногоуровневыхРазделов.Свойство(НаимРаздела) Тогда
		...
		ИначеЕсли НаимРаздела = "Таблица2" Тогда
			...
			СтруктураДанных.Вставить("ИтогоСуммаГрафа23", ТабличныйДокумент.Область("ИтогоСуммаГрафа23").Значение);
			...
			ИначеЕсли ИмяТекРаздела = "Таблица2" Тогда
				...
				Для каждого ЭлМассива Из СтруктураРеквизитовФормы["ИтоговыеСуммыПоКодамОпераций_" + ИмяТекРаздела] Цикл
					Если ЭлМассива["КодОперации"] = КодОперации Тогда
						...
						СуммаГрафа23 = СуммаГрафа23 + ЭлМассива["ИтогоСуммаГрафа23"];
				...
				ТабличныйДокумент.Области["ИтогоСуммаГрафа23"].Значение = СуммаГрафа23;
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя)
				...
				Пока Форма.ТабличныйДокумент.Области.Найти("П1000007_" + Формат(Ном, "ЧГ=")) <> Неопределено Цикл
					СуммаГрафа7 = СуммаГрафа7 + Форма.ТабличныйДокумент.Области["П1000007_" + Формат(Ном, "ЧГ=")].Значение;
					Ном = Ном + 1;
				КонецЦикла;
	...
	ИначеЕсли ИмяРаздела = "Таблица2" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя)
				...
				СуммаГрафа22 = 0;
				Ном = 1;
				Пока Форма.ТабличныйДокумент.Области.Найти("П2000022_" + Формат(Ном, "ЧГ=")) <> Неопределено Цикл
					СуммаГрафа22 = СуммаГрафа22 + Форма.ТабличныйДокумент.Области["П2000022_" + Формат(Ном, "ЧГ=")].Значение;
					Ном = Ном + 1;
				КонецЦикла;
				Форма.СтруктураРеквизитовФормы["ИтоговыеСуммыПоКодамОпераций_" + ИмяРаздела][НомСтраницы - 1]["КодОперации"]
					= КодОперации;
				...
				Для каждого ЭлМассива Из Форма.СтруктураРеквизитовФормы["ИтоговыеСуммыПоКодамОпераций_" + ИмяРаздела] Цикл
					...
					КонецЕсли;
				КонецЦикла;
				Форма.ТабличныйДокумент.Области["ИтогоСуммаГрафа22"].Значение = СуммаГрафа22;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ОбластьИмя)
				И ((СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.ИтогоСуммаГрафа23, ОбластьИмя) > 0)
				ИЛИ (СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.ИтогоСуммаГрафа23,
				Лев(ОбластьИмя, Форма.СтруктураРеквизитовФормы.ДлинаИмениГрафы)) > 0)) Тогда
				...
				СуммаГрафа23 = 0;
				Ном = 1;
				Пока Форма.ТабличныйДокумент.Области.Найти("П2000023_" + Формат(Ном, "ЧГ=")) <> Неопределено Цикл
					СуммаГрафа23 = СуммаГрафа23 + Форма.ТабличныйДокумент.Области["П2000023_" + Формат(Ном, "ЧГ=")].Значение;
					Ном = Ном + 1;
				КонецЦикла;
				Форма.СтруктураРеквизитовФормы["ИтоговыеСуммыПоКодамОпераций_" + ИмяРаздела][НомСтраницы - 1]["КодОперации"]
					= КодОперации;
				Форма.СтруктураРеквизитовФормы["ИтоговыеСуммыПоКодамОпераций_" + ИмяРаздела][НомСтраницы - 1]["ИтогоСуммаГрафа23"]
					= СуммаГрафа23;
				СуммаГрафа23 = 0;
				Для каждого ЭлМассива Из Форма.СтруктураРеквизитовФормы["ИтоговыеСуммыПоКодамОпераций_" + ИмяРаздела] Цикл
					Если ЭлМассива["КодОперации"] = КодОперации Тогда
					...
					КонецЕсли;
				КонецЦикла;
				Форма.ТабличныйДокумент.Области["ИтогоСуммаГрафа23"].Значение = СуммаГрафа23;
			КонецЕсли;
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если Ветка = АктивнаяВетка Тогда
				...
				Для каждого Ветка_П20000 Из ДанныеМнЧ_П20000 Цикл
					Ветка_П20000.Данные.Вставить("П2000022", Форма.ТабличныйДокумент.Области["П2000022_"
					+ Формат(ДанныеМнЧ_П20000.Индекс(Ветка_П20000) + 1, "ЧГ=")].Значение);
					Ветка_П20000.Данные.Вставить("П2000023", Форма.ТабличныйДокумент.Области["П2000023_"
					+ Формат(ДанныеМнЧ_П20000.Индекс(Ветка_П20000) + 1, "ЧГ=")].Значение);
			...
			ДанныеМнЧ_П20000 = Ветка.ДанныеМногострочныхЧастей.П20000.Строки;
			Для каждого Ветка_П20000 Из ДанныеМнЧ_П20000 Цикл
				СуммаГрафа22 = СуммаГрафа22 + Ветка_П20000.Данные["П2000022"];
				СуммаГрафа23 = СуммаГрафа23 + Ветка_П20000.Данные["П2000023"];
			...
			Форма.СтруктураРеквизитовФормы["ИтоговыеСуммыПоКодамОпераций_" + ИмяРаздела][НомСтраницы]["ИтогоСуммаГрафа22"]
				= СуммаГрафа22;
			Форма.СтруктураРеквизитовФормы["ИтоговыеСуммыПоКодамОпераций_" + ИмяРаздела][НомСтраницы]["ИтогоСуммаГрафа23"]
				= СуммаГрафа23;
			...
			Ветка.Данные["ИтогоСуммаГрафа23"] = СуммаГрафа23;
			...
			Если Ветка = АктивнаяВетка Тогда
				...
				Форма.ТабличныйДокумент.Области["ИтогоСуммаГрафа23"].Значение = Ветка.Данные["ИтогоСуммаГрафа23"];
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("ИтогоСуммаГрафа22", "П2000022");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("ИтогоСуммаГрафа23", "П2000023");
		...
		Для каждого Ветка Из ДанныеРеестра.Строки Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				Если ЗаполненаСтрокаМногострочнойЧасти(Ветка_П10000,
					СтрРазделить("П1000004,П1000005,П1000006,П1000007,П1000008", ",", Ложь)) Тогда
					...
					Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные[ИмяПок]) Тогда
						...
						+ НомерСтрокиП10000_Строка + " по графе 4 не задано наименование документа",
					...
					Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные[ИмяПок]) Тогда
						СообщитьВТС("Реестры по продавцам -> Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомерСтрокиП10000_Строка + " по графе 5 не задан номер документа или ""-"" при отсутствии номера",
						"Таблица1", ИмяПок + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					...
					Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные[ИмяПок]) Тогда
						СообщитьВТС("Реестры по продавцам -> Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомерСтрокиП10000_Строка + " по графе 6 не задана дата документа",
						"Таблица1", ИмяПок + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
						СтатусПроверки = Ложь;
					ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П10000.Данные[ИмяПок]) Тогда
						...
						+ " по графе 6 задана дата документа, выходящая за пределы периода с 01.01.1900 по 31.12.2099",
						"Таблица1", ИмяПок + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				...
				Если ЗаполненаСтрокаМногострочнойЧасти(Ветка_П10000,
					СтрРазделить("П1000009,П1000010,П1000011,П1000012,П1000013", ",", Ложь)) Тогда
					
					ИмяПок = "П1000009";
					Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные[ИмяПок]) Тогда
						СообщитьВТС("Реестры по продавцам -> Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомерСтрокиП10000_Строка + " по графе 9 не задано наименование документа",
						"Таблица1", ИмяПок + "_" + НомерСтрокиП10000_Строка, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
		...
		Для каждого Ветка Из ДанныеРеестра.Строки Цикл
			...
			Для каждого Ветка_П20000 Из ДанныеМнЧ_П20000 Цикл
				...
				Если ЗаполненаСтрокаМногострочнойЧасти(Ветка_П20000,
					СтрРазделить("П2000019,П2000020,П2000021,П2000022,П2000023", ",", Ложь)) Тогда
					...
					Если НЕ ЗначениеЗаполнено(Ветка_П20000.Данные[ИмяПок]) Тогда
						...
						+ НомерСтрокиП20000_Строка + " по графе 19 не задано наименование документа",
					...
					Если НЕ ЗначениеЗаполнено(Ветка_П20000.Данные[ИмяПок]) Тогда
						СообщитьВТС("Реестры по покупателям -> Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомерСтрокиП20000_Строка + " по графе 20 не задан номер документа или ""-"" при отсутствии номера",
						"Таблица2", ИмяПок + "_" + НомерСтрокиП20000_Строка, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					...
					Если НЕ ЗначениеЗаполнено(Ветка_П20000.Данные[ИмяПок]) Тогда
						СообщитьВТС("Реестры по покупателям -> Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомерСтрокиП20000_Строка + " по графе 21 не задана дата документа",
						"Таблица2", ИмяПок + "_" + НомерСтрокиП20000_Строка, НомерСтраницы);
						СтатусПроверки = Ложь;
					ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П20000.Данные[ИмяПок]) Тогда
						...
						+ " по графе 21 задана дата документа, выходящая за пределы периода с 01.01.1900 по 31.12.2099",
						"Таблица2", ИмяПок + "_" + НомерСтрокиП20000_Строка, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	//
	ЗаполнитьДаннымиУзел(ДеревоДанных1.Строки[0].Данные, РрАк201_29_45);
	ЗаполнитьДаннымиУзел(ДеревоДанных2.Строки[0].Данные, РрАк201_29_67);
	...
	//
	РрАк201_29_45Пр = ПолучитьПодчиненныйЭлемент(РрАк201_29_45, "РрАк201.29.45Пр");
	Для каждого Ветка Из ДеревоДанных1.Строки Цикл
		
		ЭкзРрАк201_29_45Пр = СкопироватьУзел(РрАк201_29_45, РрАк201_29_45Пр, РрАк201_29_45.Строки.Количество() - 1);
		
		ЗаполнитьДаннымиУзел(Ветка.Данные, ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_29_45Пр, "КонрагПр"));
		ЗаполнитьДаннымиУзел(Ветка.Данные, ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_29_45Пр, "КодПТ"));
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			Если ЗаполненаСтрокаМногострочнойЧасти(Ветка_П10000,
				СтрРазделить("П1000004,П1000005,П1000006,П1000007,П1000008", ",", Ложь)) Тогда
				ЭкзДокПдтвВыч4 = СкопироватьУзел(ЭкзРрАк201_29_45Пр, ДокПдтвВыч4, ЭкзРрАк201_29_45Пр.Строки.Количество() - 1);
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			Если ЗаполненаСтрокаМногострочнойЧасти(Ветка_П10000,
				СтрРазделить("П1000009,П1000010,П1000011,П1000012,П1000013", ",", Ложь)) Тогда
				ЭкзДокПдтвВыч5 = СкопироватьУзел(ЭкзРрАк201_29_45Пр, ДокПдтвВыч5, ЭкзРрАк201_29_45Пр.Строки.Количество() - 1);
	...
	РрАк201_29_67Пок = ПолучитьПодчиненныйЭлемент(РрАк201_29_67, "РрАк201.29.67Пок");
	Для каждого Ветка Из ДеревоДанных2.Строки Цикл
		
		ЭкзРрАк201_29_67Пок = СкопироватьУзел(РрАк201_29_67, РрАк201_29_67Пок, РрАк201_29_67.Строки.Количество() - 1);
		
		ЗаполнитьДаннымиУзел(Ветка.Данные, ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_29_67Пок, "КонрагПок"));
		ЗаполнитьДаннымиУзел(Ветка.Данные, ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_29_67Пок, "КодПТ"));
		ЗаполнитьДаннымиУзел(Ветка.Данные, ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_29_67Пок, "КодПок"));
		
		ДанныеМнЧ_П20000 = Ветка.ДанныеМногострочныхЧастей.П20000.Строки;
		
		ДокПдтвВыч6 = ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_29_67Пок, "ДокПдтвВыч6");
		Для каждого Ветка_П20000 Из ДанныеМнЧ_П20000 Цикл
			Если ЗаполненаСтрокаМногострочнойЧасти(Ветка_П20000,
				СтрРазделить("П2000019,П2000020,П2000021,П2000022,П2000023", ",", Ложь)) Тогда
				ЭкзДокПдтвВыч6 = СкопироватьУзел(ЭкзРрАк201_29_67Пок, ДокПдтвВыч6, ЭкзРрАк201_29_67Пок.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзДокПдтвВыч6.Строки Цикл
			...
			КонецЕсли;
		КонецЦикла;
		РегламентированнаяОтчетность.УдалитьУзел(ДокПдтвВыч6);
		
		ДокПдтвВыч7 = ПолучитьПодчиненныйЭлемент(ЭкзРрАк201_29_67Пок, "ДокПдтвВыч7");
		Для каждого Ветка_П20000 Из ДанныеМнЧ_П20000 Цикл
			Если ЗаполненаСтрокаМногострочнойЧасти(Ветка_П20000,
				СтрРазделить("П2000024,П2000025,П2000026,П2000027,П2000028", ",", Ложь)) Тогда
				ЭкзДокПдтвВыч7 = СкопироватьУзел(ЭкзРрАк201_29_67Пок, ДокПдтвВыч7, ЭкзРрАк201_29_67Пок.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзДокПдтвВыч7.Строки Цикл
					Если Эл.Код = "НомерДок" И НЕ ЗначениеЗаполнено(Ветка_П20000.Данные[Эл.Ключ]) Тогда
						ВывестиПоказательВXML(Эл, "-");
					Иначе
						ВывестиПоказательВXML(Эл, Ветка_П20000.Данные[Эл.Ключ]);
			...
			КонецЕсли;
		КонецЦикла;
		РегламентированнаяОтчетность.УдалитьУзел(ДокПдтвВыч7);
		
	КонецЦикла;
	РегламентированнаяОтчетность.УдалитьУзел(РрАк201_29_67Пок);
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Если НЕ ПечФорма.ПроверитьВывод(МассивПроверки) Тогда
					...
					Обл = ПечФорма.Область(ПечФорма.ВысотаТаблицы, 2, ПечФорма.ВысотаТаблицы, 4);
					Если Обл <> Неопределено Тогда
						Обл.ГраницаСнизу = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная);
					КонецЕсли;
