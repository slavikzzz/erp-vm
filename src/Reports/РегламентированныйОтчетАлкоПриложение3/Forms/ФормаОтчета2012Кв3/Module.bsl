...
Процедура ПроверитьЗаполнениеТаблицыДекларации(СтатусПроверки, ИдГруппы, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста)
	...
	Для Каждого СтрокаДекларации Из ДанныеДопСтрок Цикл
		...
		Если НЕ ЗначениеЗаполнено(ЗначениеГрафы) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации не указано содержание этилового спирта!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателя, НомерЛиста);
		КонецЕсли;
	...
	Для Каждого Дубликат Из ТаблицаДубликатов Цикл
		...
		Если НЕ ПустаяСтрока(АдресДубликата) Тогда
			ТекстОписанияОшибки = "В таблице "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации повторяются сочетания кода вида продукции и содержания этилового спирта в строках " + ПереченьСтрок + "!";
