...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = 1152021;
	...
	СтруктураРеквизитовФормы.КНД = "1152021";
	СтруктураРеквизитовФормы.НаименованиеОтчетаДляВыгрузки = "ДЕКЛАРАЦИЯ ОБ ОБЪЕМАХ ПРОИЗВОДСТВА И ОБОРОТА АЛКОГОЛЬНОЙ И СПИРТОСОДЕРЖАЩЕЙ ПРОДУКЦИИ";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Иначе // При открытии или при копировании сохраненного отчета восстанавливаем сохраненные данные.
		ВосстановитьСохраненныеДанные();
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", Знач ОбластьИмя = "") Экспорт
	...
	Если ИмяТекущейСтраницыПанели = "Декларация" Тогда
		...
		Для Инд = 1 По СтруктураГруппы.Количество() Цикл
			...
			Колонка10 = Колонка10 + Форма.ТабличныйДокумент.Области["П000010000010_" + ИндСтр].Значение;
			Колонка11 = Колонка11 + Форма.ТабличныйДокумент.Области["П000010000011_" + ИндСтр].Значение;
			Колонка12 = Колонка12 + Форма.ТабличныйДокумент.Области["П000010000012_" + ИндСтр].Значение;
		КонецЦикла;
		
		Форма.ТабличныйДокумент.Области.П000000000010.Значение = Колонка10;
		Форма.ТабличныйДокумент.Области.П000000000011.Значение = Колонка11;
		Форма.ТабличныйДокумент.Области.П000000000012.Значение = Колонка12;
		
		Форма.ТабличныйДокумент.Области.П000000000008.Значение = Колонка10
		+ Колонка11
		+ Колонка12
		+ Форма.ТабличныйДокумент.Области.П000000000013.Значение;
		
		Форма.ТабличныйДокумент.Области.П000000000016.Значение = Форма.ТабличныйДокумент.Области.П000000000005.Значение
		+ Форма.ТабличныйДокумент.Области.П000000000006.Значение
		+ Форма.ТабличныйДокумент.Области.П000000000007.Значение
		- Форма.ТабличныйДокумент.Области.П000000000008.Значение
...
&НаКлиенте
Процедура ОткрытьФормуВыбораЗначенияДекларацияЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОткрытьФормуВыбораЗначенияВидДеятельностиЗавершение(Результат, ДополнительныеПараметры1) Экспорт
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы",				"ALK_1152021_4_01.xsd");
	ПараметрыВыгрузки.Вставить("ПрефиксИмениФайлаВыгрузки",	"A3");
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	//ОбъемПрОб
	УзелОбъемПрОбШаблон = ПолучитьПодчиненныйЭлемент(УзелРодитель, "ОбъемПрОб");
	
	НомерСтраницы = 1;
	Для каждого ЭкземплярРазделаДекларация Из РазделДекларация  Цикл
		ДанныеЭкз = ЭкземплярРазделаДекларация.Данные[0].Значение;
		УзелОбъемПрОб = СкопироватьУзел(УзелРодитель, УзелОбъемПрОбШаблон);
		ЗаполнитьДаннымиУзел(ДанныеЭкз, УзелОбъемПрОб);	
		ЗадатьЗначениеЭлемента(УзелОбъемПрОб, "П000000000001", Формат(НомерСтраницы, "ЧГ="));
		
		УзелОбъемПрОбРФШаблон = ПолучитьПодчиненныйЭлемент(УзелОбъемПрОб, "ОбъемПрОбРФ");
		...
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеЭкз, "П000010000001_" + Формат(НомерСтроки, "ЧГ=")) Цикл
			...
			Если ЗначениеЗаполнено(ДанныеЭкз["П000010000009_" + Формат(НомерСтроки, "ЧГ=")]) Тогда	
				ЧислоВыгруженныхСтрок = ЧислоВыгруженныхСтрок + 1;					
				УзелОбъемПрОбРФ = СкопироватьУзел(УзелОбъемПрОб, УзелОбъемПрОбРФШаблон);
				ВыгрузитьСтрокуМногострочнойЧасти(ДанныеЭкз, УзелОбъемПрОбРФ, НомерСтроки);
				ЗадатьЗначениеЭлемента(УзелОбъемПрОбРФ, "П000010000001", Формат(ЧислоВыгруженныхСтрок, "ЧГ="));
				ЗадатьЗначениеЭлемента(УзелОбъемПрОбРФ, "П000010000004", ДанныеЭкз["П000000000004"]);
			КонецЕсли;
