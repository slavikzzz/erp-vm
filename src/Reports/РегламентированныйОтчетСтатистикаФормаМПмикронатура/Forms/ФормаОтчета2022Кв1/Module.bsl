...
Процедура ФормированиеТаблицыВыбораПродукции()
	...
	МассивОтбора = СвойстваПоказателей.НайтиСтроки(
		Новый Структура("Показатель", "П002000100100"))[0].ТаблицаЗначений.Выгрузить(
		Новый Структура("ПризнакОтбора", Ложь), "Код");
	...
	НоваяСтрока = ТЗВыбораПродукции.Вставить(0);
	НоваяСтрока.Код = "";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(Область.Имя, ":") > 0 Тогда
		ФВозврат = Истина;
	ИначеЕсли Лев(Область.Имя, 1) = "П" Тогда
		Графа = Сред(Область.Имя, 12, 2);
		ИмяЯчейкиОКВЭД = Лев(Область.Имя, 12) + "2_" + Сред(Область.Имя, 15);
		ЗначениеЯчейкиОКВЭД = ПолеТабличногоДокументаФормаОтчета.Области[ИмяЯчейкиОКВЭД].Значение;
		Если Графа = "04" Или (Графа = "03" И Не ЗначениеЗаполнено(ЗначениеЯчейкиОКВЭД)) Тогда
			ФВозврат = Истина;
		КонецЕсли;
	КонецЕсли;
	Если ФВозврат Тогда
	...
	Иначе
		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			...
			Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "02" Тогда
				Если Сред(Область.Имя, 11, 1) = "1" Тогда
				...
				КонецЕсли;
			Иначе
				МассивВыбора = ПараметрВводаПоля.ТаблицаЗначений.НайтиСтроки(
					Новый Структура("Ключ", ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 12) + "2_" + Сред(Область.Имя, 15)].Значение));
				ТЗВыбора.Очистить();
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Прав(КодПоказателя, 1) = "3" Тогда
		СтрЕдИзм = СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "03_", Лев(Область.Имя, 11) + "04_");
		ИсходноеЗначение = ПолеТабличногодокументаформаотчета.Области[СтрЕдИзм].Значение;
	Иначе
		СтрОКПД = СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "01_", Лев(Область.Имя, 11) + "02_");
		ИсходноеЗначение = ПолеТабличногодокументаформаотчета.Области[СтрОКПД].Значение;
	КонецЕсли;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если СтрНайти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы1) > 0
		...
		Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "02" Тогда
			...
			Иначе
				ПредыдущееЗначениеВКолонке01 = ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы2 + "01_" + НомерСтроки].Значение;
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(ВыборКод) Тогда
				Если (СтрНайти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы1) > 0
				   И ПредыдущееЗначениеВКолонке01 <> ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "01_" + НомерСтроки].Значение)
				 Или (СтрНайти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы2) > 0
				   И ПредыдущееЗначениеВКолонке01 <> ТабличныйДокумент.Области[СтруктураРеквизитовФормы.мИдГруппы2 + "01_" + НомерСтроки].Значение) Тогда
					О3.Значение = "";
					О4.Значение = "";
				КонецЕсли;
			Иначе
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	П0020001001.Очистить();
	П0020001002.Очистить();
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверки.Добавить(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть1_2"));
	...
	Иначе
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма МП(микро)-натура");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ПолеТабличногоДокументаФормаОтчета.ПолучитьОбласть("Подписи"));
	КонецЕсли;
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма МП(микро)-натура");
...
Процедура СформироватьСпискиВыбора()
	...
	Для Каждого Область Из МакетСоставаПоказателей.Области Цикл
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
			...
			ТаблСписка.Колонки.Добавить("Ключ"    , , ,
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П002000100101",
		250, , "Выбор продукции", ТЗР1);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П002000100102",
		250, , "Выбор продукции", ТЗР1);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П002000100204",
		250, , "Выбор единицы измерения", КоллекцияСписковВыбора["СправочникЕдиницИзмерения"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П002000100203",
		250, , "Выбор единицы измерения", КоллекцияСписковВыбора["СправочникЕдиницИзмерения"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П002000100200",
		250, , , КоллекцияСписковВыбора["ПриложениеРаздел2"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П002000100201",
		250, , "Выбор продукции", ТЗР2);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П002000100202",
		250, , "Выбор продукции", ТЗР2);
