	...
	Если ИнформацияОСвязях.Статус.Код <> Неопределено Тогда
		Если ИнформацияОСвязях.Статус.Код = "001" Тогда
			ОбластиМакета.Наименование.Параметры.ДействующаяОрганизация = НСтр("ru = 'Действующая организация (по данным ЕГРЮЛ)';
																				|en = 'Valid company (by USRLE data)'");
	...
	Если КоличествоОрганизаций > 0 Тогда
		ОрганизацииПоЭтомуАдресу = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			НСтр("ru = '%1 организаций;%1 организация;%1 организации;%1 организации;%1 организаций;%1 организаций';
				|en = '%1 companies;%1 company;%1 companies;%1 companies;%1 companies;%1 companies'"),
	...
	Иначе
		...
		Для Итератор = 0 По ВГраница Цикл
			...
			ЯвляетсяРуководителем =
				ТекущийРуководитель.ОрганизацииГдеРуководитель.Найти(ИнформацияОСвязях.ИНН, "ИНН") <> Неопределено;
			Если ЯвляетсяРуководителем Тогда
				КоличествоОрганизацийГдеРуководитель = КоличествоОрганизацийГдеРуководитель - 1;
				ШаблонСтроки = НСтр("ru = 'Руководитель еще в %1';
									|en = 'Manager also in %1'");
			КонецЕсли;
			...
			ЯвляетсяУчредителем =
				ТекущийРуководитель.ОрганизацииГдеУчредитель.Найти(ИнформацияОСвязях.ИНН, "ИНН") <> Неопределено;
				...
				ИнформацияОРуководителе = ИнформацияОРуководителе
					+ ?(ТекущийРуководитель.ОрганизацииГдеРуководительПоФИО ИЛИ ТекущийРуководитель.ОрганизацииГдеУчредительПоФИО,
						" (" + НСтр("ru = 'поиск проведен по ФИО и может быть неточным';
									|en = 'search was performed by full name and may be inaccurate'") + ")",
						"");
			КонецЕсли;
	...
	Если ГодыЕстьОтчетность.Количество() = 3 Тогда
		НаличиеБухгалтерскойОтчетности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Есть данные за %1, %2 и %3 годы';
				|en = 'There are data for %1, %2, and %3'"),
	...
	ИначеЕсли ГодыЕстьОтчетность.Количество() = 2 Тогда
		НаличиеБухгалтерскойОтчетности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Есть данные за %1 и %2 годы, отсутствует за %3 год';
				|en = 'There are data for %1 and %2, but missing for %3'"),
			ГодыЕстьОтчетность[0],
			ГодыЕстьОтчетность[1],
			ГодыНетОтчетности[0]);
	ИначеЕсли ГодыЕстьОтчетность.Количество() = 1 Тогда
		НаличиеБухгалтерскойОтчетности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Есть данные за %1 год, отсутствует за %2 и %3 годы';
				|en = 'There are data for %1, but missing for %2 and %3'"),
			ГодыЕстьОтчетность[0],
			ГодыНетОтчетности[0],
			ГодыНетОтчетности[1]);
	Иначе
		НаличиеБухгалтерскойОтчетности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отсутствуют данные за %1, %2 и %3 годы';
				|en = 'Data for %1, %2, and %3 are missing'"),
			ГодыНетОтчетности[0],
			ГодыНетОтчетности[1],
			ГодыНетОтчетности[2]);
	КонецЕсли;
	...
	Иначе
		...
		Если ПроверокСНарушениями = 0 Тогда
			...
			ШаблонТекста = ШаблонТекста + НСтр("ru = '%1 проверок, нарушений не выявлено;';
												|en = '%1 checks, no violations detected;'");
		...
		Иначе
			...
			КонецЕсли;
			РезультатыПроверок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Известны результаты %1, %2';
					|en = 'Known results %1, %2'"), ТекстПроверки, ТекстНарушения);
		КонецЕсли;
			...
			Если ЭтоЮридическоеЛицо Тогда
				ОбластиМакета.ДействующаяОрганизация.Параметры.ДействующаяОрганизация =
					НСтр("ru = 'Организация есть в базе ЕГРН';
						|en = 'Company already exists in the USRT base'");
			Иначе
				ОбластиМакета.ДействующаяОрганизация.Параметры.ДействующаяОрганизация =
					НСтр("ru = 'ИП есть в базе ЕГРН';
						|en = 'This IE is already in the USRT base'");
			КонецЕсли;
			ТабличныйДокумент.Вывести(ОбластиМакета.ДействующаяОрганизация);
		Иначе
			Если НЕ ЭтоЮридическоеЛицо
				...
				ОбластиМакета.НедействующаяОрганизация.Параметры.НедействующаяОрганизация =
					НСтр("ru = 'Не действует (по данным ЕГРН)';
						|en = 'Not valid (according to USRT)'");
			Иначе
				ОбластиМакета.НедействующаяОрганизация.Параметры.НедействующаяОрганизация =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = '%1 (по данным ЕГРН)';
							|en = '%1 (according to USRT data)'"),
		...
		СколькоЕщеОрганизаций = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
					НСтр("ru = '%1 организациях;%1 организация;%1 организации;%1 организации;%1 организаций;%1 организаций';
						|en = '%1 companies;%1 company;%1 companies;%1 company;%1 companies;%1 companies'"),
					КоличествоОрганизаций));
		...
		Если КоличествоОрганизаций > 0 Тогда
			...
			Если ВыводитьЕще Тогда
				...
				ШаблонСтроки = НСтр("ru = 'Является руководителем еще в %1%2:';
									|en = 'This is a manager also in %1%2:'");
			Иначе
				ШаблонСтроки = НСтр("ru = 'Является руководителем в %1%2:';
									|en = 'This is a manager in %1%2:'");
			КонецЕсли;
		...
		Если КоличествоОрганизаций > 0 Тогда
			...
			Если ВыводитьЕще Тогда
				...
				ШаблонСтроки = НСтр("ru = 'Является учредителем еще в %1%2:';
									|en = 'This is a founder also in %1%2:'");
			Иначе
				ШаблонСтроки = НСтр("ru = 'Является учредителем в %1%2:';
									|en = 'This is a founder in %1%2:'");
			КонецЕсли;
			...
			СколькоЕщеОрганизаций = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
					НСтр("ru = '%1 организаций;%1 организации;%1 организациях;%1 организациях;%1 организациях;%1 организациях';
						|en = '%1 companies;%1 companies;%1 companies;%1 companies;%1 companies;%1 companies'"),
					КоличествоОрганизаций),
	...
	ДанныеДосье.Вставить("ИмяФайлаБухгалтерскаяОтчетность", 
		СтроковыеФункции.СтрокаЛатиницей(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Досье %1 - бухгалтерская отчетность';
																		|en = 'Profile %1 - accounting reporting'"),
				ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
					ИнформацияОСвязях.НаименованиеСокращенное, ""))));
	...
	ДанныеДосье.Вставить("ИмяФайлаАнализОтчетности", 
		СтроковыеФункции.СтрокаЛатиницей(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Досье %1 - анализ отчетности';
																		|en = 'Profile %1 - reporting analysis'"),
				ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
					ИнформацияОСвязях.НаименованиеСокращенное, ""))));
	...
	ДанныеДосье.Вставить("ИмяФайлаФинансовыйАнализ", 
		СтроковыеФункции.СтрокаЛатиницей(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Досье %1 - финансовый анализ';
																		|en = 'Profile %1 - financial analysis'"),
				ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
					ИнформацияОСвязях.НаименованиеСокращенное, ""))));
...
Процедура ВывестиРазделКредитоспособность(ТабличныйДокумент, СтрокаДанных, ПараметрыВывода)
	...
	ТекстТочкиДиаграммы    = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Оценка кредитоспособности S = %1';
			|en = 'Creditworthiness assessment S = %1'"),
		Кредитоспособность);
	Диаграмма.УстановитьЗначение(0, 0, ЗначениеТочкиДиаграммы, , ТекстТочкиДиаграммы);
	// Комментарии к диаграмме
		...
		Если Год > "2016" Тогда
			...
			Иначе
				ТекстОшибки = НСтр(
					...
					|en = 'The ""Data of tax burden and profitability"" subsystem is not implemented'",
					ОбщегоНазначения.КодОсновногоЯзыка());
				РаботаСКонтрагентами.ЗаписатьОшибкуВЖурналРегистрации(
					ТекстОшибки,
					РаботаСКонтрагентами.ИдентификаторСервисаКонтрагент(),
					НСтр("ru = 'Получение значений показателей';
						|en = 'Receive indicator values'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Для каждого СтрокаПериода Из ТаблицаПериоды Цикл
			...
			Для каждого СтрокаПроверки Из ПроверкиПериода Цикл
				...
				Проверка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1, %2",
					...
					?(СтрокаПроверки.ПлановаяПроверка, НСтр("ru = 'плановая проверка';
															|en = 'scheduled inspection'"), НСтр("ru = 'внеплановая проверка';
																							|en = 'unannounced inspection'")));
