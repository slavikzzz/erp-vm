...
Процедура ЗаполнитьЖурналСправокДанными(УведомлениеОбъект, ДанныеУведомления, ДанныеСправок) Экспорт 
	...
	ХранимаяСтруктураДанных.Вставить("ДанныеМногостраничныхРазделов", Новый Структура("СведОплОбрУсл", Новый СписокЗначений));
	...
	Для Каждого Справка Из ДанныеСправок Цикл 
		...
		ХранимаяСтруктураДанных.ДанныеМногостраничныхРазделов.СведОплОбрУсл.Добавить(СтруктураСправки);
...
Функция ПолучитьТаблицуФорм() Экспорт 
	...
	Стр = Результат.Добавить();
	Стр.ИмяФормы = "Форма2023_1";
	Стр.ОписаниеФормы = "В соответствии с приказом ФНС России от 18.10.2023 № ЕД-7-11/755@";
...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2023_1(Данные, УникальныйИдентификатор)
	...
	Для Каждого Стр Из Данные.ДанныеМногостраничныхРазделов.СведОплОбрУсл Цикл 
		...
		УведомлениеОСпецрежимахНалогообложения.ПроверкаДанныхПолучателяСправки(СведОплОбрУсл, "СведОплОбрУсл", ТаблицаОшибок, СведОплОбрУсл.ПрОбуч);
		Если Не ЗначениеЗаполнено(СведОплОбрУсл.НомерСвед) Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СведОплОбрУсл.НомерСвед) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указан / неправильно указан номер", "СведОплОбрУсл", "НомерСвед", СведОплОбрУсл.УИД));
		КонецЕсли;
		...
		КонецЕсли;
		Если СведОплОбрУсл.ПрФормОбуч = "0" И СведОплОбрУсл.ПрОбуч = "0" Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"При заочной форме обучения налогоплательщик и обучающийся должны совпадать", "СведОплОбрУсл", "ПрОбуч", СведОплОбрУсл.УИД));
		КонецЕсли;
	КонецЦикла;
...
Функция СформироватьСписокЛистовФорма2023_1(Объект) Экспорт
	Возврат УведомлениеОСпецрежимахНалогообложения.СформироватьЖурналСправок(Объект, "СведОплОбрУсл");
КонецФункции

Функция СформироватьСправку(Объект, УИДТекущаяСтраница) Экспорт
	Возврат УведомлениеОСпецрежимахНалогообложения.СформироватьСправку(Объект, УИДТекущаяСтраница, "СведОплОбрУсл");
КонецФункции
...
Функция ДанныеСправки2023_1(Объект, ТекстВыгрузки)
	Попытка
		...
		Если КНД <> "1151158" Или ВерсФорм <> "5.01" Тогда 
			...
			Если КНД = "1184045" Тогда 
				ОбщегоНазначения.СообщитьПользователю("Выбранный файл является журналом справок (КНД 1184045), но не справкой");
			КонецЕсли;
			Возврат Неопределено;
		КонецЕсли;
