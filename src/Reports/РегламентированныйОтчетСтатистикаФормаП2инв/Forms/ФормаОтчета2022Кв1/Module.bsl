...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ТЗКодыОКВЭД_51",
	512, , "Выбор кода вида деятельности по ОКВЭД",   ТЗКодыОКВЭД_51);
КонецПроцедуры
...
Процедура ДоступностьРазделов()
	...
	ИмяГруппы = "П0060000001";
	Если ЗначениеЗаполнено(ОбособленноеПодразделение) Тогда
		
		Для Гр = 1 По 6 Цикл
			ИмяЯчейки = "П00500350" + Гр;
			мФормаОтчета.Области[ИмяЯчейки].ЦветФона = Новый Цвет();
			мФормаОтчета.Области[ИмяЯчейки].Защита = Истина;
			мФормаОтчета.Области[ИмяЯчейки].Значение = "";
		КонецЦикла;
		
		Для Стр = 1 По ЭтотОбъект[ИмяГруппы].Количество() Цикл
			
			мФормаОтчета.Области["УдалитьСтроку" + ИмяГруппы + "01_" + Стр].Гиперссылка = Ложь;
			мФормаОтчета.Области["УдалитьСтроку" + ИмяГруппы + "01_" + Стр].Текст = "";
			
			Для Гр = 1 По ЭтотОбъект.мСтруктураКолвоКолонокРазделов[ИмяГруппы] Цикл
				Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
				мФормаОтчета.Области[ИмяГруппы + Графа + "_" + Стр].ЦветФона = Новый Цвет();
				мФормаОтчета.Области[ИмяГруппы + Графа + "_" + Стр].Защита = Истина;
				мФормаОтчета.Области[ИмяГруппы + Графа + "_" + Стр].Значение = "";
			КонецЦикла;
			
		КонецЦикла;
		
		Если мФормаОтчета.Области.Найти("ДобавлениеСтроки2") <> Неопределено Тогда
			мФормаОтчета.УдалитьОбласть(мФормаОтчета.Области["ДобавлениеСтроки2"], ТипСмещенияТабличногоДокумента.ПоВертикали);
		КонецЕсли;
	...
	Иначе
		...
		Если мФормаОтчета.Области.Найти("ДобавлениеСтроки2") = Неопределено Тогда
			...
			ОбластьДобавлениеСтроки2 = ОбъектОтчета(ИмяФормы).ПолучитьМакет(
				СтруктураРеквизитовФормы["ИмяМакетаТабличногоДокумента"]).ПолучитьОбласть("ДобавлениеСтроки2");
			
			ОсновнаяЧасть4Верх = мФормаОтчета.Области["ОсновнаяЧасть4"].Верх;
			
			мФормаОтчета.ВставитьОбласть(
				ОбластьДобавлениеСтроки2.Область(),
				мФормаОтчета.Область(ОсновнаяЧасть4Верх, , ОсновнаяЧасть4Верх),
		...
		Для Гр = 1 По 6 Цикл
			...
			мФормаОтчета.Области[ИмяЯчейки].ЦветФона = Новый Цвет(255, 255, 192);
			мФормаОтчета.Области[ИмяЯчейки].Защита = Ложь;
		КонецЦикла;
		
		Для Стр = 1 По ЭтотОбъект[ИмяГруппы].Количество() Цикл
			мФормаОтчета.Области["УдалитьСтроку" + ИмяГруппы + "01_" + Стр].Гиперссылка = Истина;
			мФормаОтчета.Области["УдалитьСтроку" + ИмяГруппы + "01_" + Стр].Текст = "х";
			Для Гр = 1 По ЭтотОбъект.мСтруктураКолвоКолонокРазделов[ИмяГруппы] Цикл
				Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
				Если (Гр = 1 Или Гр = 5) И Стр < 5 Тогда
					мФормаОтчета.Области[ИмяГруппы + Графа + "_" + Стр].ЦветФона = Новый Цвет();
					мФормаОтчета.Области[ИмяГруппы + Графа + "_" + Стр].Защита = Истина;
				Иначе
					мФормаОтчета.Области[ИмяГруппы + Графа + "_" + Стр].ЦветФона = Новый Цвет(255, 255, 192);
					мФормаОтчета.Области[ИмяГруппы + Графа + "_" + Стр].Защита = Ложь;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	П0040000032.Очистить();
	П0060000001.Очистить();
	РегламентированнаяОтчетность.НачальныеОперацииСМногострочнымиБлоками(ЭтотОбъект);
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Стр = 2 По 20 Цикл
		Если Стр = 5 Или Стр = 19 Тогда
			Продолжить;
		КонецЕсли;
		
		СтрСтр = "П0101" + Формат(Стр, "ЧЦ=2; ЧВН=");
		ит = 0;
		Для Гр = 4 По 6 Цикл
			Обл = Области.Найти(СтрСтр + "0" + Гр);
			Если Обл <> Неопределено Тогда 
				ит = ит + Обл.Значение;
			КонецЕсли;
		КонецЦикла;
		Области.Найти(СтрСтр + "03").Значение = Ит;
	КонецЦикла;
	
	МС = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("02;03;04;06;07;09;11;12;18", ";");
	Для Гр = 3 По 9 Цикл
		ит = 0;
		Для Каждого Стр Из МС Цикл 
			Обл = Области.Найти("П0101" + Стр + "0" + Гр);
			Если Обл <> Неопределено Тогда 
				ит = ит + Обл.Значение;
			КонецЕсли;
		КонецЦикла;
		Области.Найти("П0101010" + Гр).Значение = Ит;
	КонецЦикла;
	
	Области.П01033003.Значение = Области.П01033103.Значение + Области.П01033203.Значение + Области.П01033303.Значение;
	...
	Области["П005003501"].Значение = Области["П005003502"].Значение + Области["П005003503"].Значение;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНайти(Область.Имя, "П004000003203_") = 1 И Не СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ТЗКодыОКВЭД_51");
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Для Стр = 1 По 4 Цикл
		Области["П006000000101_" + Стр].Значение = Стр;
		Если Стр = 1 Тогда
			Области["П006000000105_" + Стр].Значение = "36";
		КонецЕсли;
	КонецЦикла;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ПолеТабличногоДокументаФормаОтчета.Области.Найти(
		СтрЗаменить(Область.Имя, "П004000003203_", "П004000003201_")).Значение = РезультатНазвание;
