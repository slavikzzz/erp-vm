...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "08/09/2017";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Стр = 2 По 19 Цикл 
		Если Стр = 5 Или Стр = 18 Или (Стр >= 12 И Стр <= 16) Тогда 
			Продолжить;
		КонецЕсли;
		
		СтрСтр = "П00100" + Формат(Стр, "ЧЦ=2; ЧВН=");
		ит = 0;
		Для Гр = 4 По 6 Цикл 
	...
	Для Гр = 3 По 9 Цикл
		...
		Для Каждого Стр Из МС Цикл 
			Обл = Области.Найти("П00100" + Стр + "0" + Гр);
			Если Обл <> Неопределено Тогда 
				ит = ит + Обл.Значение;
			КонецЕсли;
		КонецЦикла;
		Области.Найти("П00100010"+Гр).Значение = Ит;
	КонецЦикла;
	
	Для Гр = 3 По 6 Цикл 
		ит = 0;
		Для Стр = 22 По 44 Цикл 
			Обл = Области.Найти("П00200" + Стр + "0" + Гр);
			Если Обл <> Неопределено Тогда 
				ит = ит + Обл.Значение;
			КонецЕсли;
		КонецЦикла;
		Области.Найти("П00200210" + Гр).Значение = Ит;
	КонецЦикла;
	
	Области.П003004603.Значение = Области.П003004703.Значение
								+Области.П003004803.Значение
								+Области.П003004903.Значение;
