...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы2 = "П0060000001";
	СтруктураРеквизитовФормы.мИдГруппы3 = "П0010000008";
	СтруктураРеквизитовФормы.мИдГруппы4 = "П0010000010";
	СтруктураРеквизитовФормы.мИдГруппы5 = "П0010000012";
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок <> Неопределено
	   И Не СтруктураРеквизитовФормы.мСохраненныйДок.ДанныеОтчета.Получить().ДанныеМногострочныхРазделов.Свойство("П0010000008") Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru = 'Создайте новый отчет. Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.';
								|en = 'Создайте новый отчет. Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.'");
		Сообщение.Сообщить();
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "dic_1000017221",
		512, , "Выбор кода", КоллекцияСписковВыбора["dic_1000017221"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "dic_1000017217",
		512, , "Выбор кода", КоллекцияСписковВыбора["dic_1000017217"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "dic_1000017219",
		512, , "Выбор кода", КоллекцияСписковВыбора["dic_1000017219"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "dic_1000016687",
		512, , "Выбор кода ОКВЭД2", КоллекцияСписковВыбора["dic_1000016687"]);
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ОбластьДляУдаления = ПолеТабличногоДокументаФормаОтчета.Области.Найти("НеПечатаемаяЧасть");
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ОбластьДляУдаления = ВыбТабПоле.Области.Найти("НеПечатаемаяЧасть");
	Если ОбластьДляУдаления <> Неопределено Тогда
	...
	ОбластьДляУдаления = ВыбТабПоле.Области.Найти("НеПечатаемаяЧасть");
	Если ОбластьДляУдаления <> Неопределено Тогда
		ВыбТабПоле.УдалитьОбласть(ОбластьДляУдаления, ТипСмещенияТабличногоДокумента.ПоВертикали);
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 1 По 5 Цикл
		...
		Пока Истина Цикл
			СтрНом = СтрНом + 1;
			Если Области.Найти("П0010000008" + Графа + "_" + СтрНом) = Неопределено Тогда
				Прервать;
			КонецЕсли;
			
			Знч = Знч + Области["П0010000008" + Графа + "_" + СтрНом].Значение;
		КонецЦикла;
		Области["П0010010007" + Графа].Значение = Знч;
		
	КонецЦикла;
	
	//стр. 09 = Σ стр. 10 по гр. 1−5;
	Для Гр = 1 По 5 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		...
		Пока Истина Цикл
			СтрНом = СтрНом + 1;
			Если Области.Найти("П0010000010" + Графа + "_" + СтрНом) = Неопределено Тогда
				Прервать;
			КонецЕсли;
			
			Знч = Знч + Области["П0010000010" + Графа + "_" + СтрНом].Значение;
		КонецЦикла;
		Области["П0010010009" + Графа].Значение = Знч;
		
	КонецЦикла;
	
	//стр. 11 = Σ стр. 12 по гр. 1–7;
	Для Гр = 1 По 7 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		...
		Пока Истина Цикл
			СтрНом = СтрНом + 1;
			Если Области.Найти("П0010000012" + Графа + "_" + СтрНом) = Неопределено Тогда
				Прервать;
			КонецЕсли;
			
			Знч = Знч + Области["П0010000012" + Графа + "_" + СтрНом].Значение;
		КонецЦикла;
		Области["П0010010011" + Графа].Значение = Знч;
		
	КонецЦикла;
	
	//гр.1 по строкам 02-04, 06, 07, 09, 11, 13, 20 = (гр.2 + гр.3 + гр.4) по строкам 02-04, 06, 07, 09, 11, 13, 20;
	Для Стр = 2 По 20 Цикл
		//13 строка исключена из расчёта. Коллизия правил в указе.
		Если Стр = 5 Или Стр = 8 Или Стр = 10 Или Стр = 12 Или (Стр >= 13 И Стр < 20) Тогда
			Продолжить;
		КонецЕсли;
		НомСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
		Области["П00100100" + НомСтр + "01"].Значение
			= Области["П00100100" + НомСтр + "02"].Значение
			+ Области["П00100100" + НомСтр + "03"].Значение
			+ Области["П00100100" + НомСтр + "04"].Значение;
	КонецЦикла;
	
	//гр. 1 по стр. 13–19 = (гр. 2 + гр. 4) по стр. 13–19;
	Для Стр = 13 По 19 Цикл
		Области["П00100100" + Стр + "01"].Значение
			= Области["П00100100" + Стр + "02"].Значение
			+ Области["П00100100" + Стр + "04"].Значение;
	КонецЦикла;
	
	//гр. 1 по стр. 22 = (гр. 2 + гр. 3) по стр. 22;
	Области.П001001002201.Значение = Области.П001001002202.Значение + Области.П001001002203.Значение;
	
	//стр. 01 = стр. (02+03+04+06+07+09+11+13+20) по гр. 2,4;
	Для Гр = 2 По 4 Цикл
		Если Гр = 3 Тогда
			Продолжить;
		КонецЕсли;
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		Области["П0010010001" + Графа].Значение
			= Области["П0010010002" + Графа].Значение
			+ Области["П0010010003" + Графа].Значение
			+ Области["П0010010004" + Графа].Значение
			+ Области["П0010010006" + Графа].Значение
			+ Области["П0010010007" + Графа].Значение
			+ Области["П0010010009" + Графа].Значение
			+ Области["П0010010011" + Графа].Значение
			+ Области["П0010010013" + Графа].Значение
			+ Области["П0010010020" + Графа].Значение;
	КонецЦикла;
	
	//стр. 01 = (стр. 02 + стр. 03 + стр. 04 + стр. 06 + стр. 07 + стр. 09 + стр. 11 + стр. 20) по гр. 3;
	Области["П001001000103"].Значение
		= Области["П001001000203"].Значение
		+ Области["П001001000303"].Значение
		+ Области["П001001000403"].Значение
		+ Области["П001001000603"].Значение
		+ Области["П001001000703"].Значение
		+ Области["П001001000903"].Значение
		+ Области["П001001001103"].Значение
		+ Области["П001001002003"].Значение;
	
	//стр. 01 = (стр. 07 + стр. 09 + стр. 11 + стр. 13 + стр. 20) по гр. 5;
	Области["П001001000105"].Значение
		= Области["П001001000705"].Значение
		+ Области["П001001000905"].Значение
		+ Области["П001001001105"].Значение
		+ Области["П001001001305"].Значение
		+ Области["П001001002005"].Значение;
	
	//стр. 01 = (стр. 02 + стр. 03 + стр. 04 + стр. 07 + стр. 09 + стр. 11 + стр. 13 + стр. 20) по гр. 6, 7;
	Для Гр = 6 По 7 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		Области["П0010010001" + Графа].Значение
			= Области["П0010010002" + Графа].Значение
	...
	//Раздел 2
	//стр. 23 гр. 1 = (стр. 24 + стр. 25 + стр. 26 + стр. 27 + стр. 28 + стр. 29 + стр. 30 + стр.31 + стр. 32 + стр. 33 + стр. 34 + стр. 35 + стр. 36 +
	//+ стр. 37 + стр. 38 + стр. 39 + стр. 40 + стр. 42 + стр. 44 + стр. 45 + стр. 47 + стр. 49 + стр. 50) по гр. 1;
	СуммаПоСтр23 = 0;
	Для НомСтр = 24 По 50 Цикл
		Если НомСтр = 41 Или НомСтр = 43 Или НомСтр = 46 Или НомСтр = 48 Тогда
			Продолжить;
		КонецЕсли;
		СуммаПоСтр23 = СуммаПоСтр23 + Области["П00200100" + НомСтр + "01"].Значение;
	КонецЦикла;
	Области["П002001002301"].Значение = СуммаПоСтр23;
	
	//Раздел 3
	//стр. 51 = (стр. 52 + стр. 53 + стр. 54) по гр. 1.
	Области.П01033103.Значение = Области.П01033203.Значение + Области.П01033303.Значение + Области.П01033403.Значение;
	...
	Пока Истина Цикл
		...
		//гр. 7 = (гр. 8 + гр. 9 + гр. 10) по всем строкам.
		...
		//гр. 2 = (гр. 3 + гр. 5 + гр. 6 + гр. 7 + гр. 11 + гр. 12 ) по всем строкам;
	...
	//Раздел 5.
	//гр. 1 = гр. 2 + гр. 3;
	Области["П005003601"].Значение = Области["П005003602"].Значение + Области["П005003603"].Значение;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если СтрНайти(Область.Имя, "П001000000807_") = 1 Тогда
			
			ОбластьЗаменаИмя        = СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "07_", Лев(Область.Имя, 11) + "06_");
			СмежнаяОбластьЗаменаИмя = СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "06_", Лев(Область.Имя, 11) + "07_");
			...
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "dic_1000017221");
			
		ИначеЕсли СтрНайти(Область.Имя, "П001000001007_") = 1 Тогда
			...
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "dic_1000017219");
			
		ИначеЕсли СтрНайти(Область.Имя, "П001000001209_") = 1 Тогда
			
			ОбластьЗаменаИмя        = СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "09_", Лев(Область.Имя, 11) + "08_");
			СмежнаяОбластьЗаменаИмя = СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "08_", Лев(Область.Имя, 11) + "09_");
			
			Область          = ПолеТабличногоДокументаФормаОтчета.Области.Найти(ОбластьЗаменаИмя);
			СмежнаяОбласть   = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СмежнаяОбластьЗаменаИмя);
			
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "dic_1000017217");
			
		ИначеЕсли СтрНайти(Область.Имя, "П004000003201_") = 1 Или СтрНайти(Область.Имя, "П004000003203_") = 1 Тогда
			...
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "dic_1000016687");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_2_1"));
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_2_2"));
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_2_3"));
	
	ВерхМногострочнойЧасти = ТабДок.Области.ОсновнаяЧасть1_2_3.Низ + 1;
	...
	ОсновнаяЧасть1_3_2 = ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_3_2");
	Если ПечатнаяФорма.ПроверитьВывод(ОсновнаяЧасть1_3_2) Тогда
		ПечатнаяФорма.Вывести(ОсновнаяЧасть1_3_2);
	Иначе
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-2(инвест)");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ПустаяСтрока"));
	...
	ВерхМногострочнойЧасти = ТабДок.Области.ОсновнаяЧасть1_3_2.Низ + 1;
	...
	ОсновнаяЧасть1_3_3 = ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_3_3");
	Если ПечатнаяФорма.ПроверитьВывод(ОсновнаяЧасть1_3_3) Тогда
		ПечатнаяФорма.Вывести(ОсновнаяЧасть1_3_3);
	Иначе
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-2(инвест)");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ПустаяСтрока"));
	...
	ВерхМногострочнойЧасти = ТабДок.Области.ОсновнаяЧасть1_3_3.Низ + 1;
	...
	ОсновнаяЧасть1_3_4 = ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_3_4");
	Если ПечатнаяФорма.ПроверитьВывод(ОсновнаяЧасть1_3_4) Тогда
		ПечатнаяФорма.Вывести(ОсновнаяЧасть1_3_4);
	Иначе
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-2(инвест)");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ПустаяСтрока"));
	...
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_7"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-2(инвест)");
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_6"));
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_8"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-2(инвест)");
	...
	ОсновнаяЧасть2_5 = ТабДок.ПолучитьОбласть("ОсновнаяЧасть2_5");
	Если ПечатнаяФорма.ПроверитьВывод(ОсновнаяЧасть2_5) Тогда
		ПечатнаяФорма.Вывести(ОсновнаяЧасть2_5);
	Иначе
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-2(инвест)");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ОсновнаяЧасть2_5);
	...
	ВерхМногострочнойЧасти = ТабДок.Области.ОсновнаяЧасть3_3.Низ + 1;
	НизМногострочнойЧасти = ТабДок.Области.МногострочнаяЧасть2.Низ;
	ТекущийВерх = ВерхМногострочнойЧасти;
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		ТекущаяОбластьМногострочнойЧасти = ТабДок.ПолучитьОбласть(ТекущийВерх, , ТекущийВерх);
		Если ПечатнаяФорма.ПроверитьВывод(ТекущаяОбластьМногострочнойЧасти) Тогда
	...
	ОсновнаяЧасть4 = ТабДок.ПолучитьОбласть("ОсновнаяЧасть4");
	Если ПечатнаяФорма.ПроверитьВывод(ОсновнаяЧасть4) Тогда
		ПечатнаяФорма.Вывести(ОсновнаяЧасть4);
	Иначе
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П-2(инвест)");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ОсновнаяЧасть4);
...
Процедура ФормироватьСтруктуруСтраницОтчета() Экспорт
	...
	СтрокаУровня1.Представление            = "Статистика: Форма П-2 (инвест)";
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	Перем ТаблицаГруппы;
	Перем ТаблицаГруппы2;
	Перем ТаблицаГруппы3;
	Перем ТаблицаГруппы4;
	Перем ТаблицаГруппы5;
	...
	Если Контейнер.Свойство("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок2", ТаблицаГруппы2) И ТаблицаГруппы2 <> Неопределено Тогда
		КонвертированнаяТаблицаГруппы2 =
			РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтотОбъект, ТаблицаГруппы2);
		ЗначениеВДанныеФормы(КонвертированнаяТаблицаГруппы2, ЭтотОбъект[ИмяГруппы2]);
		ВывестиРазделВТабличныйДокументНаСервере(ИмяГруппы2, ЭтотОбъект[ИмяГруппы2]);
	КонецЕсли;
	
	ИмяГруппы3 = СтруктураРеквизитовФормы.мИдГруппы3;
	Если Контейнер.Свойство("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок3", ТаблицаГруппы3) И ТаблицаГруппы3 <> Неопределено Тогда
		КонвертированнаяТаблицаГруппы3 =
			РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтотОбъект, ТаблицаГруппы3);
		ЗначениеВДанныеФормы(КонвертированнаяТаблицаГруппы3, ЭтотОбъект[ИмяГруппы3]);
		ВывестиРазделВТабличныйДокументНаСервере(ИмяГруппы3, ЭтотОбъект[ИмяГруппы3]);
	КонецЕсли;
	
	ИмяГруппы4 = СтруктураРеквизитовФормы.мИдГруппы4;
	Если Контейнер.Свойство("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок4", ТаблицаГруппы4) И ТаблицаГруппы4 <> Неопределено Тогда
		КонвертированнаяТаблицаГруппы4 =
			РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтотОбъект, ТаблицаГруппы4);
		ЗначениеВДанныеФормы(КонвертированнаяТаблицаГруппы4, ЭтотОбъект[ИмяГруппы4]);
		ВывестиРазделВТабличныйДокументНаСервере(ИмяГруппы4, ЭтотОбъект[ИмяГруппы4]);
	КонецЕсли;
	
	ИмяГруппы5 = СтруктураРеквизитовФормы.мИдГруппы5;
	Если Контейнер.Свойство("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок5", ТаблицаГруппы5) И ТаблицаГруппы5 <> Неопределено Тогда
		КонвертированнаяТаблицаГруппы5 =
			РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтотОбъект, ТаблицаГруппы5);
		ЗначениеВДанныеФормы(КонвертированнаяТаблицаГруппы5, ЭтотОбъект[ИмяГруппы5]);
		ВывестиРазделВТабличныйДокументНаСервере(ИмяГруппы5, ЭтотОбъект[ИмяГруппы5]);
	КонецЕсли;
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок1",
		РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтотОбъект, П0040000032, Ложь));
	
	Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок2",
		РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтотОбъект, П0060000001, Ложь));
...
Процедура УдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, ТекущаяСтраница = Неопределено) Экспорт
	...
	Если ИмяГруппы = "П0010000008" Тогда
		ИмяТекущейОбласти = ИмяГруппы + "07_" + НомТекущейСтроки;
	ИначеЕсли ИмяГруппы = "П0010000010" Тогда
		ИмяТекущейОбласти = ИмяГруппы + "07_" + НомТекущейСтроки;
	ИначеЕсли ИмяГруппы = "П0010000012" Тогда
		ИмяТекущейОбласти = ИмяГруппы + "09_" + НомТекущейСтроки;
	Иначе
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	// Определяем текущую область таблицы
	Если ИмяГруппы = "П0010000008" Тогда
		ИмяТекущейОбласти = ИмяГруппы + "07_" + КолСтрокСтр1;
	ИначеЕсли ИмяГруппы = "П0010000010" Тогда
		ИмяТекущейОбласти = ИмяГруппы + "07_" + КолСтрокСтр1;
	ИначеЕсли ИмяГруппы = "П0010000012" Тогда
		ИмяТекущейОбласти = ИмяГруппы + "09_" + КолСтрокСтр1;
	Иначе
