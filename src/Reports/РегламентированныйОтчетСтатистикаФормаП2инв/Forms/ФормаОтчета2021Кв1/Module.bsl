...
Процедура ДоступностьРазделов()
	...
	Если ЗначениеЗаполнено(ОбособленноеПодразделение) Тогда
		...
		Для Стр = 1 По ЭтотОбъект.П0060000001.Количество() Цикл
			
			мФормаОтчета.Области["УдалитьСтрокуП006000000101" + "_" + Стр].Гиперссылка = Ложь;
			мФормаОтчета.Области["УдалитьСтрокуП006000000101" + "_" + Стр].Текст = "";
			
			Для Гр = 1 По 4 Цикл
				мФормаОтчета.Области["П00600000010" + Гр + "_" + Стр].ЦветФона = Новый Цвет();
				мФормаОтчета.Области["П00600000010" + Гр + "_" + Стр].Защита = Истина;
				мФормаОтчета.Области["П00600000010" + Гр + "_" + Стр].Значение = "";
			КонецЦикла;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Стр = 2 По 19 Цикл
		Если Стр = 5 Или Стр = 18 Или Стр = 20 Тогда 
			Продолжить;
		КонецЕсли;
	...
	МС = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("02;03;04;06;07;09;11;12;17", ";");
	Для Гр = 3 По 9 Цикл
		ит = 0;
	...
	Области.П01032903.Значение = Области.П01033003.Значение + Области.П01033103.Значение + Области.П01033203.Значение;
	...
	Области["П005003401"].Значение = Области["П005003402"].Значение + Области["П005003403"].Значение;
