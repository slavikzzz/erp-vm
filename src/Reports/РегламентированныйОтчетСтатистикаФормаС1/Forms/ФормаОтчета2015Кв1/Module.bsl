...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "20/10/2014";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если СтрНайти(Область.Имя, "П00000200") > 0 Тогда
			Если Прав(Область.Имя, 2) = "05" Тогда 
				Возврат;
			КонецЕсли;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабличноеПоле.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(
		"ФормаОтчета2015Кв1_ФормаОтчета"));
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревоДляЗагрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, ПолучитьИмяМакетаДереваВыгрузки());
...
Функция ПроверитьПоказатели() Экспорт
	...
	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, ПолучитьИмяМакетаДереваВыгрузки());
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ИмяМакета = ?(Месяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "АтрибВыгрузкиXML2015Кв1_месяц", "АтрибВыгрузкиXML2015Кв1_год");
...
&НаСервере
Функция ПолучитьИмяМакетаДереваВыгрузки()
	Если Месяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда 
		Возврат "СхемаВыгрузкиXML2015Кв1_месяц";
	Иначе 
		Возврат "СхемаВыгрузкиXML2015Кв1_год";
	КонецЕсли;
КонецФункции
...
Функция ПолучитьПараметрыИнициализацииУниверсальногоОтчета()
	Если Месяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда 
		ИмяСхемы = "СхемаВыгрузкиXML2015Кв1_месяц";
		ИмяМакета = "АтрибВыгрузкиXML2015Кв1_месяц";
		ИмяШаблона = "ШаблонРосстата2015Кв1_месяц";
	Иначе 
		ИмяСхемы = "СхемаВыгрузкиXML2015Кв1_год";
		ИмяМакета = "АтрибВыгрузкиXML2015Кв1_год";
		ИмяШаблона = "ШаблонРосстата2015Кв1_год";
	КонецЕсли;
