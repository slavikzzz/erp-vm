...
Процедура ИзменитьПериод(Форма, Шаг)
	...
	Иначе
		Форма.мДатаКонцаПериодаОтчета  = КонецКвартала(ДобавитьМесяц(Форма.мДатаКонцаПериодаОтчета, Шаг * 5));
		Форма.мДатаНачалаПериодаОтчета = НачалоГода(Форма.мДатаКонцаПериодаОтчета);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(мДатаНачалаПериодаОтчета) И НЕ ЗначениеЗаполнено(мДатаКонцаПериодаОтчета) Тогда
		НомМес = Месяц(ТекущаяДатаСеанса());
		
		Если (НомМес >= 1) И (НомМес <= 6) Тогда
			НомМес1 = "06";
			КолД    = "30";
		Иначе
			НомМес1 = 12;
		...
		КонечнаяДата = КонецДня(Дата(Формат(ТекущаяДатаСеанса(), "ДФ=гггг") + НомМес1 + КолД));
		
		мДатаКонцаПериодаОтчета  = КонечнаяДата;
