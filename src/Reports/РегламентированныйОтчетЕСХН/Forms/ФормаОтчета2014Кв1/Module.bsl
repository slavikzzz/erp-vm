...
Процедура СортироватьСведенияРаздела21()
	...
	Если ТребуетсяСортировка Тогда
		...
		Для Инд = 2 По 11 Цикл
			...
			Если ГодУбытка <> 0 Тогда
				СписокСортировки.Добавить(СуммаУбытка, РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(ГодУбытка));
...
Процедура ВставитьДанныеБлокаУбытков(НомерПервойСтрокиБлока)
	...
	Для Инд = 1 По 10 Цикл
		...
		Если ЗначениеЗаполнено(Графа1) ИЛИ ЗначениеЗаполнено(Графа3) Тогда
			...
			Раздел21.Вставить("П10021" + Формат(НомерПервойСтрокиБлока, "ЧЦ=5; ЧВН=; ЧГ=0") + "01_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(ДобавленоСтрок), Графа1);
			Раздел21.Вставить("П10021" + Формат(НомерПервойСтрокиБлока, "ЧЦ=5; ЧВН=; ЧГ=0") + "03_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(ДобавленоСтрок), Графа3);
		КонецЕсли;
...
Процедура ПроверитьЗаполнение_Раздел3(СтатусПроверки)
	...
	Инд = 1;
	Пока ДанныеРаздела.Свойство("П000030000101_" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Инд)) Цикл
...
Процедура ПроверитьКС_1_9а(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Сумма ЕСХН, исчисленного за налоговый период (строка 050 Раздела 2) должна быть равна разнице строк 030 и 040 Раздела 2, умноженной на 6%, если значение строки 030 Раздела 2 больше значения строки 040 Раздела 2';
							|en = 'Сумма ЕСХН, исчисленного за налоговый период (строка 050 Раздела 2) должна быть равна разнице строк 030 и 040 Раздела 2, умноженной на 6%, если значение строки 030 Раздела 2 больше значения строки 040 Раздела 2'");
...
Процедура ПроверитьКС_1_9б(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Сумма ЕСХН, исчисленного за налоговый период (строка 050 Раздела 2) должна быть равна разнице строк 030 и 040 Раздела 2, умноженной на 0.5%, если значение строки 030 Раздела 2 больше значения строки 040 Раздела 2';
							|en = 'Сумма ЕСХН, исчисленного за налоговый период (строка 050 Раздела 2) должна быть равна разнице строк 030 и 040 Раздела 2, умноженной на 0.5%, если значение строки 030 Раздела 2 больше значения строки 040 Раздела 2'");
	ОписаниеОшибки = НСтр("ru = 'Неправильное исчисление суммы единого сельскохозяйственного налога за налоговый период';
							|en = 'Неправильное исчисление суммы единого сельскохозяйственного налога за налоговый период'");
	Нарушение      = НСтр("ru = 'ст.346.8, п.1 ст. 346.9 НК РФ';
							|en = 'ст.346.8, п.1 ст. 346.9 НК РФ'");
...
Процедура ПроверитьКС_1_14(СтруктураРезультата)
	...
	Если (СуммаКорректировки - Р2Стр040.Значение) > 0 Тогда
		Коммент = "Сумма убытка " + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(ОтчетныйГод - 10) + " года ("
