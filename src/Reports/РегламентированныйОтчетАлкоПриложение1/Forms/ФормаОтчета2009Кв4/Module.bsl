...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы1  = "П0000100000"; // Декларация
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/01/2006"; // Первый вариант
	...
	СтруктураРеквизитовФормы.КНД = "1152014";
	СтруктураРеквизитовФормы.НаименованиеОтчетаДляВыгрузки = "ДЕКЛАРАЦИЯ ОБ ОБЪЕМАХ ПРОИЗВОДСТВА И ОБОРОТА ЭТИЛОВОГО СПИРТА";
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000000000007", "П000010000008;П000000000010");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000000000008", "П000010000008");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000000000011", "П000000000012;П000000000013;П000000000014;П000000000015");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000000000017", "П000000000004;П000000000005;П000000000006;П000000000007;П000000000011;П000000000016;П000010000008;П000000000010;П000000000012;П000000000013;П000000000014;П000000000015");
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета, ИзменитьПризнакАктивностиСтраницы = Истина, РазделОтчета = Ложь)
	...
	ИначеЕсли ИмяТекРаздела = "Декларация" Тогда
		...
		НаимТаблицыСтраницы = "ТаблицаСтраницыДекларация";
		СтруктураДанных = Новый Структура;
		СписокДанных = ЭтаФорма[НаимТаблицыСтраницы][НомерСтраницыРазделаОтчета - 1].Данные;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", Знач ОбластьИмя = "") Экспорт
	...
	Если ИмяТекущейСтраницыПанели = "Декларация" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000000000007, ОбластьИмя) > 0
			ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000000000008, ОбластьИмя) > 0 Тогда   
			...
			Для Инд = 1 По СтруктураГруппы.Количество() Цикл
				...
				Колонка8 = Колонка8 + Форма.ТабличныйДокумент.Области["П000010000008_" + ИндСтр].Значение;
			КонецЦикла;
			
			Форма.ТабличныйДокумент.Области.П000000000007.Значение = Колонка8 +	Форма.ТабличныйДокумент.Области.П000000000010.Значение;
			Форма.ТабличныйДокумент.Области.П000000000008.Значение = Колонка8;
			
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000000000011, ОбластьИмя) > 0 Тогда   
			
			Форма.ТабличныйДокумент.Области.П000000000011.Значение = Форма.ТабличныйДокумент.Области.П000000000012.Значение
			+ Форма.ТабличныйДокумент.Области.П000000000013.Значение
			+ Форма.ТабличныйДокумент.Области.П000000000014.Значение
			+ Форма.ТабличныйДокумент.Области.П000000000015.Значение;
			
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000000000017, ОбластьИмя) > 0 Тогда   
			
			Форма.ТабличныйДокумент.Области.П000000000017.Значение = Форма.ТабличныйДокумент.Области.П000000000004.Значение
			+ Форма.ТабличныйДокумент.Области.П000000000005.Значение
			+ Форма.ТабличныйДокумент.Области.П000000000006.Значение
			- Форма.ТабличныйДокумент.Области.П000000000007.Значение
			- Форма.ТабличныйДокумент.Области.П000000000011.Значение
			- Форма.ТабличныйДокумент.Области.П000000000016.Значение;
...
Процедура УдалитьСтраницу(Команда)
	...
	Если ЭтаФорма[мСтруктураМногостраничныхРазделов.Титульный][0].АктивнаяСтраница Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Нельзя удалять первую страницу титульного листа данной декларации.';
								|en = 'Нельзя удалять первую страницу титульного листа данной декларации.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ВставитьДополнительнуюСтраницуНаСервере(ИмяТекущейСтраницыПанели = Неопределено)
	...
	ИначеЕсли ИмяТекущейСтраницыПанели = "Декларация" Тогда
		...
		РегламентированнаяОтчетность.ВставитьДополнительнуюСтраницуНаСервере(ЭтаФорма, ИмяТекущейСтраницыПанели);
		ТабличныйДокумент.Области.НомерПП.Значение = ЭтаФорма[мСтруктураМногостраничныхРазделов.Декларация].Количество();
...
Процедура ОчиститьНаСервере() Экспорт
	...
	ТаблицаСтраницыДекларация.Очистить();
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Титульный" Тогда
		...
		ИначеЕсли Область.Имя = "ВидДеятельности" Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьФормуВыбораЗначенияВидДеятельностиЗавершение", ЭтотОбъект);
		...
		Если ((СтрНайти(Область.Имя, "П000000000003") > 0)
		 ИЛИ (СтрНайти(Область.Имя, "П000000000002") > 0))
		   И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
			...
			СтандартнаяОбработка = Ложь;
			ТекКод = ТабличныйДокумент.Области.П000000000003.Значение;
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьФормуВыбораЗначенияДекларацияЗавершение", ЭтотОбъект);
			ОткрытьФорму("Обработка.ОбщиеОбъектыРеглОтчетности.Форма.ФормаВыбораЗначенияИзТаблицы", ПараметрыФормы, ЭтаФорма,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаКлиенте
Процедура ОткрытьФормуВыбораЗначенияВидДеятельностиЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОткрытьФормуВыбораЗначенияДекларацияЗавершение(Результат, ДополнительныеПараметры1) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если НЕ ЗначениеЗаполнено(Результат["Код"]) Тогда
			...
			ТабличныйДокумент.Области.П000000000002.Значение = "";
			ТабличныйДокумент.Области.П000000000003.Значение = "";
			
		Иначе
			
			ТабличныйДокумент.Области.П000000000002.Значение = Результат["Название"];
			ТабличныйДокумент.Области.П000000000003.Значение = Результат["Код"];
...
Функция ПроверитьТитульныйЛистИСведенияОФизЛицах(БратьИзФормы = Истина, ВыводитьСообщения = Истина, ЧтоПроверять = "12", ПечатаемыеЛисты = Неопределено)
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		Если ВыводитьСообщения Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1Источник данных для формирования отчета не определен!';
																							|en = '%1Источник данных для формирования отчета не определен!'"), ПредставлениеОтчета);
			Сообщение.Сообщить();
		КонецЕсли;
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(ИННДиректор) Тогда
		...
		СерияИНомерРук = "";
		ВыдУдЛичнРук = "";
		ДатаУдЛичнРук = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа(Тип("Дата"));
	...
	Если ПустаяСтрока(ИННБухгалтер) Тогда
		...
		СерияИНомерБух = "";
		ВыдУдЛичнБух = "";
		ДатаУдЛичнБух = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа(Тип("Дата"));
	...
	Если НЕ ПустаяСтрока(ОргУП) Тогда
		...
		Если ПустаяСтрока(ИННУП) Тогда
			...
			СерияИНомерУП = "";
			ВыдУдЛичнУП = "";
			ДатаУдЛичнУП = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа(Тип("Дата"));
...
Процедура ВыгрузитьТитульныйЛистИСведенияОФизЛицах(Текст)
	...
	Если ПустаяСтрока(ИННУП) И НЕ ПустаяСтрока(ОргУП) Тогда
		...
		СерияИНомерУпПред = "";
		ВыдУдЛичнУпПред = "";
		ДатаУдЛичнУпПред = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа(Тип("Дата"));
...
Процедура ОбработатьРезультирующийТекстВыгрузки(Текст)
	...
	Пока Инд <= Текст.КоличествоСтрок() Цикл
		...
		ИначеЕсли ПустаяСтрока(ПослП000000000008) И (Тек13Симв = "П000010000001" ИЛИ Тек13Симв = "П000010000002" ИЛИ Тек13Симв = "П000010000003" ИЛИ Тек13Симв = "П000010000008" ИЛИ Тек13Симв = "П000010000009") Тогда
	...
	ПокРаздела1.Колонки.Добавить("П000010000008");
	...
	Для Инд = 1 По КолСтрок Цикл
		...
		Тек13Симв = Лев(ТекСтрока, 13);
		Если Тек13Симв = "П000010000009" Тогда
			НовСтр = ПокРаздела1.Вставить(0);
			НовСтр.П000010000001 = Сред(Текст.ПолучитьСтроку(ТекИнд - 4), 15);
			НовСтр.П000010000002 = Сред(Текст.ПолучитьСтроку(ТекИнд - 3), 15);
			НовСтр.П000010000003 = Сред(Текст.ПолучитьСтроку(ТекИнд - 2), 15);
			НовСтр.П000010000008 = Сред(Текст.ПолучитьСтроку(ТекИнд - 1), 15);
			НовСтр.П000010000009 = Сред(ТекСтрока, 15);
			Текст.УдалитьСтроку(ТекИнд - 4);
			Текст.УдалитьСтроку(ТекИнд - 4);
			Текст.УдалитьСтроку(ТекИнд - 4);
			Текст.УдалитьСтроку(ТекИнд - 4);
			Текст.УдалитьСтроку(ТекИнд - 4);
	...
	Для Каждого Стр Из ПокРаздела1 Цикл
		...
		Текст.ДобавитьСтроку("П000010000008:" + Стр.П000010000008);
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы",             "ALK_1152014_4_01.xsd");
	ПараметрыВыгрузки.Вставить("ПрефиксИмениФайлаВыгрузки", "A1");
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РазделДекларация = мДанныеОтчета.Декларация;
	...
	//ОбъемПроизводства
	УзелОбъемПроизводстваШаблон = ПолучитьПодчиненныйЭлемент(УзелРодитель, "ОбъемПроизводства");
	
	НомерСтраницы = 1;
	Для каждого ЭкземплярРазделаДекларация Из РазделДекларация  Цикл
		ДанныеЭкз = ЭкземплярРазделаДекларация.Данные[0].Значение;
		УзелОбъемПроизводства = СкопироватьУзел(УзелРодитель, УзелОбъемПроизводстваШаблон);
		ЗаполнитьДаннымиУзел(ДанныеЭкз, УзелОбъемПроизводства);
		ЗадатьЗначениеЭлемента(УзелОбъемПроизводства, "П000000000001", Формат(НомерСтраницы, "ЧГ="));
		
		УзелОбъемОтгрузкиШаблон = ПолучитьПодчиненныйЭлемент(УзелОбъемПроизводства, "ОбъемОтгрузки");
		ЧислоВыгруженныхСтрок = 0;
		НомерСтроки = 1;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеЭкз, "П000010000001_" + Формат(НомерСтроки, "ЧГ=")) Цикл
			Если ЗначениеЗаполнено(ДанныеЭкз["П000010000008_" + Формат(НомерСтроки, "ЧГ=")]) Тогда
				ЧислоВыгруженныхСтрок = ЧислоВыгруженныхСтрок + 1;
				УзелОбъемОтгрузки = СкопироватьУзел(УзелОбъемПроизводства, УзелОбъемОтгрузкиШаблон);
				ВыгрузитьСтрокуМногострочнойЧасти(ДанныеЭкз, УзелОбъемОтгрузки, НомерСтроки);
				ЗадатьЗначениеЭлемента(УзелОбъемОтгрузки, "П000010000001", Формат(ЧислоВыгруженныхСтрок, "ЧГ="));
			КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого СтрокаУровня1 Из мДеревоВыбранныхСтраниц.ПолучитьЭлементы() Цикл
		...
		Если СтрокаУровня1.ИмяСтраницы = "Декларация" Тогда
...
&НаСервере
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено, ИндексСтраницы)
...
Процедура СформироватьПечатныеФормыРазделаДекларация(СтруктураПараметровСтраницы, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	Для Каждого Страница Из ТаблицаСтраницРаздела Цикл
		...
		ВывестиМакетВТаблДокНаСервере("Декларация", ТекущийИндекс + 1, Ложь);
		...
		Если ТаблицаСтраницРаздела.Индекс(Страница) = 0 Тогда // первая страница
			...
			ПолеТабличногоДокументаВремТаблДок.Вывести(ТабличныйДокумент.ПолучитьОбласть());
		Иначе
			НомерСтрокиПриемника = ПолеТабличногоДокументаВремТаблДок.Области.ПустаяСтрока.Верх;
			ТаблДокДо = ПолеТабличногоДокументаВремТаблДок.ПолучитьОбласть(1, , НомерСтрокиПриемника - 1, );
			ТаблДокПосле = ПолеТабличногоДокументаВремТаблДок.ПолучитьОбласть(НомерСтрокиПриемника, , ПолеТабличногоДокументаВремТаблДок.ВысотаТаблицы, );
			ТаблДокМежду = ТабличныйДокумент.ПолучитьОбласть(ТабличныйДокумент.Области.ИтоговаяЧастьРаздела.Верх, , ТабличныйДокумент.Области.ПустаяСтрока.Верх - 1, );
			ПолеТабличногоДокументаВремТаблДок.Очистить();
			ПолеТабличногоДокументаВремТаблДок.Вывести(ТаблДокДо);
			ПолеТабличногоДокументаВремТаблДок.Вывести(ТаблДокМежду);
			ПолеТабличногоДокументаВремТаблДок.Вывести(ТаблДокпосле);
		КонецЕсли;
	КонецЦикла;

	НачалоМнЧ = ПолеТабличногоДокументаВремТаблДок.Области.ОсновнаяЧасть.Низ + 1;
	КонецМнЧ  = ПолеТабличногоДокументаВремТаблДок.Области.ПустаяСтрока.Верх - 1;
	
	УдаляемыеОбласти = Новый Массив;
	Для Каждого Обл Из ПолеТабличногоДокументаВремТаблДок.Области Цикл
	...
	ПолеТабличногоДокументаВремТаблДок.Область(НачалоМнЧ, 0, КонецМнЧ, 0).Имя = "МногострочнаяЧасть";
...
Процедура ТабличныйДокументПриАктивизации(Элемент)
	...
	Если СтрЧислоВхождений(Элемент.ТекущаяОбласть.Имя, "П0000100000") > 0 Тогда
