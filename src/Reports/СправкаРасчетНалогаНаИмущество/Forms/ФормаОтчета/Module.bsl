...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокПредставление = Новый Массив;
	ЗаголовокПредставление.Добавить(НСтр("ru = 'Справка-расчет налога на имущество';
										|en = 'Detailed calculation of property tax'"));
	ЗаголовокПредставление.Добавить(БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(
										Отчет.НачалоПериода, КонецДня(Отчет.КонецПериода)));
	ЗаголовокОтчета = СтрСоединить(ЗаголовокПредставление, " ");
...
Процедура ЗаполнитьНастройкамиПоУмолчанию(ЗаполняемыеНастройки) Экспорт
	...
	Если ЗаполняемыеНастройки.Свойство("Группировка")
		...
		Строка.Поле             = "ИФНС";
		Строка.Представление    = НСтр("ru = 'ИФНС';
										|en = 'IFTS'");
		...
		Строка.Представление    = НСтр("ru = 'Код по ОКТМО';
										|en = 'RNCMT code'");
		Строка.ТипГруппировки   = 0;
		Строка.Использование    = Ложь;
		...
		Строка.Представление    = НСтр("ru = 'Код вида имущества';
										|en = 'Property kind code'");
		Строка.ТипГруппировки   = 0;
		Строка.Использование    = Ложь;
		
		Строка = Отчет.Группировка.Добавить();
		Строка.Поле             = "НалоговаяЛьгота";
		Строка.Представление    = НСтр("ru = 'Код налоговой льготы';
										|en = 'Tax relief code'");
		Строка.ТипГруппировки   = 0;
		Строка.Использование    = Ложь;
		...
		Строка.Представление    = НСтр("ru = 'Основное средство';
										|en = 'Fixed asset'");
		Строка.ТипГруппировки   = 0;
		...
		Строка.Представление    = НСтр("ru = 'Детальные записи';
										|en = 'Detailed records'");
		Строка.ТипГруппировки   = 0;
		Строка.Использование    = Истина;
		Строка.Предопределенная = Истина;
...
Процедура НадписьИспользуетсяПрежнийРасчетНалогаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "Подробнее" Тогда
		ПерейтиПоНавигационнойСсылке("https://its.1c.ru/bmk/imu2022");
	КонецЕсли;
