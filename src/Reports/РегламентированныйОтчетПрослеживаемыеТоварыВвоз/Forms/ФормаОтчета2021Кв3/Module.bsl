...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
		...
		МодульОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
			ЭтаФорма,
			"БП.Отчет.РО_ПрослеживаемыеТоварыВвоз",
			ИмяФормы(ИмяФормы),
			,
			НСтр("ru = 'Новости: Уведомление о ввозе товаров, подлежащих прослеживаемости';
				|en = 'News: Notification of import of goods to be traced'"),
			Ложь,
...
Процедура УстановитьЧисловойФорматОбласти(ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел1" Тогда
		ТабличныйДокумент.Области["П000010001400"].ТипЗначения = СтруктураРеквизитовФормы.ТипКоличествоПрослеживаемогоТовара;
	КонецЕсли;
