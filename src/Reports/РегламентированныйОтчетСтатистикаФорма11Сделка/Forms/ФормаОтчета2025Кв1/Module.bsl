...
Функция ПолучитьМассивКатегорий()
	Возврат СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("210;220;310;320;330", ";", Истина);
КонецФункции
	...
	ИначеЕсли Не СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		...
		Если СтрНачинаетсяС(Область.Имя, "П00001") И СтрНайти(Область.Имя, "09_") > 0 Тогда
			...
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				...
				ВвестиПоказатель(Элемент, Область, , ПараметрВводаПоля.ТекстПриВыборе,
		...
		ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00002") И СтрНайти(Область.Имя, "04_") > 0 Тогда
			...
			КонецЕсли;
			Возврат;
		ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00001") И (Найти(Область.Имя, "01_") > 0 Или Найти(Область.Имя, "03_") > 0) Тогда
			ОбластьДляВвода = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "01_", Лев(Область.Имя, 11) + "03_"));
			СмежнаяОбласть  = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "03_", Лев(Область.Имя, 11) + "01_"));
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, Лев(ОбластьДляВвода.Имя, 13));
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				СтандартнаяОбработка = Ложь;
				КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
				ВвестиПоказатель(Элемент, ОбластьДляВвода, СмежнаяОбласть, ПараметрВводаПоля.ТекстПриВыборе,
					ТЗВыбора, СтандартнаяОбработка);
			КонецЕсли;
			Возврат;
		ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00002") И (Найти(Область.Имя, "01_") > 0 Или Найти(Область.Имя, "03_") > 0) Тогда
			ОбластьДляВвода = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "01_", Лев(Область.Имя, 11) + "03_"));
			СмежнаяОбласть  = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "03_", Лев(Область.Имя, 11) + "01_"));
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, Лев(ОбластьДляВвода.Имя, 13));
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
...
Процедура ПеренумерацияСтрокТаблицы(ПолеТабличногоДокументаФормаОтчета)
	...
	Для Раздел = 1 По 2 Цикл
		Для Инд = 1 По 3 Цикл
			Области["П0000" + Раздел + "0121002_" + Инд].Значение = Инд;
			
			Области["П0000" + Раздел + "0122002_" + Инд].Значение = Инд;
			
			Области["П0000" + Раздел + "0131002_" + Инд].Значение = Инд;
			
			Области["П0000" + Раздел + "0132002_" + Инд].Значение = Инд;
			
			Области["П0000" + Раздел + "0133002_" + Инд].Значение = Инд;
		КонецЦикла;
	КонецЦикла;
