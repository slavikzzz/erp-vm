...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "15/07/2019";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Не СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		...
		Если СтрНачинаетсяС(Область.Имя, "П01") И СтрНайти(Область.Имя, "000010_") > 0 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "КодыР1Гр10");
			...
			КонецЕсли;
			Возврат;
		ИначеЕсли СтрНачинаетсяС(Область.Имя, "П02") И СтрНайти(Область.Имя, "00000004") > 0 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "КодыР2Гр10");
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				СтандартнаяОбработка = Ложь;
				КолонкаПоиска = "Код";
				КодПоказателя = ПараметрВводаПоля.Показатель;
			...
			КонецЕсли;
			Возврат;
		ИначеЕсли СтрНачинаетсяС(Область.Имя, "П01") И СтрНайти(Область.Имя, "000003_") > 0 Тогда
			...
			КонецЕсли;
			Возврат;
		ИначеЕсли СтрНачинаетсяС(Область.Имя, "П02") И СтрНайти(Область.Имя, "000003") > 0 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ВсеОКОФ");
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				СтандартнаяОбработка = Ложь;
				КолонкаПоиска = "Код";
				КодПоказателя = ПараметрВводаПоля.Показатель;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого Категория Из ПолучитьМассивКатегорий() Цикл 
		Секция = ПолеТабличногоДокументаФормаОтчета.ПолучитьОбласть("МнгСтр" + Категория);
		...
		ВерхМногострочнойЧасти = ПолеТабличногоДокументаФормаОтчета.Области["МнгСтр" + Категория].Верх + 1;
	...
	ПечатнаяФорма.Вывести(ПолеТабличногоДокументаФормаОтчета.ПолучитьОбласть("Подвал"));
	Подписи = ПолеТабличногоДокументаФормаОтчета.ПолучитьОбласть("ОсновнаяЧасть2");
...
Процедура СформироватьСпискиВыбора()
	...
	ВсеОКОФ = Новый ТаблицаЗначений;
	ВсеОКОФ.Колонки.Добавить("Код",Строка32);
	ВсеОКОФ.Колонки.Добавить("Название",Строка512);
	...
	Для Каждого КЗ Из СпискиВыбораПоКатегориям Цикл 
		РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П01" + КЗ.Ключ + "0000003", , , , СпискиВыбораПоКатегориям[КЗ.Ключ]);
	КонецЦикла;
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ВсеОКОФ", , , , ВсеОКОФ);
	...
	Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
		...
		КонецЦикла;
		Для Каждого КЗ Из СпискиВыбораПоКатегориям Цикл 
			РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "КодыР1Гр10", , , , ТаблСписка);
		КонецЦикла;
	КонецЕсли;
	...
	Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
		...
		КонецЦикла;
		Для Каждого КЗ Из СпискиВыбораПоКатегориям Цикл 
			РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "КодыР2Гр10", , , , ТаблСписка);
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если СтрНачинаетсяС(Область.Имя, "П01") Тогда 
		ОСм = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "0003_", "0001_"));
		Если ОСм.Имя <> Область.Имя Тогда 
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П02") Тогда 
		ОСм = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "000003", "000001"));
		Если ОСм.Имя <> Область.Имя Тогда 
			Если ЗначениеЗаполнено(Область.Значение) Тогда 
				ОСм.Значение = РезультатВыбора["Название"];
			Иначе
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Загрузка из xml будет реализована в следующих версиях';
													|en = 'Загрузка из xml будет реализована в следующих версиях'"));
	Возврат;
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(СтруктураРеквизитовФормы["ИмяМакетаТабличногоДокумента"]));
	...
	Для Каждого Стр Из ПолучитьМассивКатегорий() Цикл 
		...
		Если Контейнер.Свойство("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок" + Стр, ТаблицаГруппы) И ТаблицаГруппы <> Неопределено Тогда
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Для Каждого Стр Из ПолучитьМассивКатегорий() Цикл 
		Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок" + Стр, РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтаФорма, ЭтаФорма[СтруктураРеквизитовФормы["мИдГруппы" + Стр]], Ложь));
	КонецЦикла;
