...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0606009";
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	ИмяГруппыСинхронизируемой = ?(ИмяГруппы = "П0010010001", "П0020010001", "П0010010001");
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 Тогда
		УдалитьСтроку("");
	ИначеЕсли Лев(Область.Имя, 13) = "П001001000102" Или Лев(Область.Имя, 13) = "П001001000104" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "СправочникДни");
	ИначеЕсли Лев(Область.Имя, 13) = "П001001000103" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "СправочникМесяцНачала");
	ИначеЕсли Лев(Область.Имя, 13) = "П001001000105" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "СправочникМесяцОкончания");
	ИначеЕсли Лев(Область.Имя, 13) = "П002001000108" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "СправочникПричиныЗабастовки");
	КонецЕсли;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Колонка = Сред(Область.Имя, 12, 2);
	ОтключитьВидимостьКолонкиКод = ?(Колонка = "02" Или Колонка = "04", Истина, Ложь);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Заголовок",                    ТекстВыбора);
	ПараметрыФормы.Вставить("ТаблицаЗначений",              ТаблицаВыбора);
	ПараметрыФормы.Вставить("СтруктураДляПоиска",           Новый Структура(КолонкаПоиска, ИсходноеЗначение));
	ПараметрыФормы.Вставить("ОтключитьВидимостьКолонкиКод", ОтключитьВидимостьКолонкиКод);
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Результат = ?(РезультатВыбора["Название"] = "Пустое значение" , "", РезультатКод);
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект, Область)
	...
	НПП = 0;
	Пока Истина Цикл
		ИмяПоказателя1 = "П001001000102_" + Инд;
		Обл = Области.Найти(ИмяПоказателя1);
		Если Обл = Неопределено Тогда
			Прервать;
		КонецЕсли;
		ИмяПоказателя2 = "П001001000103_" + Инд;
		ИмяПоказателя3 = "П001001000104_" + Инд;
		ИмяПоказателя4 = "П001001000105_" + Инд;
		Если ЗначениеЗаполнено(Области[ИмяПоказателя1].Значение)
		 Или ЗначениеЗаполнено(Области[ИмяПоказателя2].Значение)
		 Или ЗначениеЗаполнено(Области[ИмяПоказателя3].Значение)
		 Или ЗначениеЗаполнено(Области[ИмяПоказателя4].Значение) Тогда
			НПП = НПП + 1;
			Области["П001001000101_" + Инд].Значение = НПП;
		Иначе
			Области["П001001000101_" + Инд].Значение = "";
		КонецЕсли;
		Инд = Инд + 1;
	КонецЦикла;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	П0010010001.Очистить();
	П0020010001.Очистить();
	РегламентированнаяОтчетность.НачальныеОперацииСМногострочнымиБлоками(ЭтотОбъект);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-ПР");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникДни",
	512, , "Выбор дня", КоллекцияСписковВыбора["Дни"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникМесяцНачала",
	512, , "Выбор месяца", КоллекцияСписковВыбора["МесяцНачала"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникМесяцОкончания",
	512, , "Выбор месяца", КоллекцияСписковВыбора["МесяцОкончания"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникПричиныЗабастовки",
	512, , "Выбор причины забастовки", КоллекцияСписковВыбора["ПричиныЗабастовки"]);
