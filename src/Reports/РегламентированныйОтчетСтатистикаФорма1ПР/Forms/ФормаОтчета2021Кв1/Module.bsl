...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.Имя = "ОргАдрес" Тогда
		РегламентированнаяОтчетностьКлиент.ОбработкаАдресаВСтатистическомОтчетеJSON(
			ЭтотОбъект, Область, СтандартнаяОбработка);
	ИначеЕсли Область.Имя = "П000010000101_1" Или Область.Имя = "П000010000103_1" Тогда
		КолонкаПоиска = "Код";
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "СправочникДни");
	ИначеЕсли Область.Имя = "П000010000102_1" Или Область.Имя = "П000010000104_1" Тогда
		КолонкаПоиска = "Код";
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "СправочникМесяцы");
	ИначеЕсли Лев(Область.Имя, 13) = "П000010000116" Или Лев(Область.Имя, 13) = "П000010000117" Тогда
		КолонкаПоиска = ?(Лев(Область.Имя, 13) = "П000010000116", "Код", "Название");
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "СправочникПричиныЗабостовки");
	КонецЕсли;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	ОтключитьВидимостьКолонкиКод = ?(Колонка = "01" Или Колонка = "03", Истина, Ложь);
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Результат = ?(РезультатВыбора["Название"] = "Пустое значение" , "",
		?(Лев(Область.Имя, 13) = "П000010000117", РезультатНазвание, РезультатКод));
	
	Область.Значение = Результат;
	
	Если Лев(Область.Имя, 13) = "П000010000117" Тогда
		ЯчейкаЗамены = СтрЗаменить(Область.Имя, "П000010000117_", "П000010000116_");
		ПолеТабличногоДокументаФормаОтчета.Области[ЯчейкаЗамены].Значение = РезультатКод;
	ИначеЕсли Лев(Область.Имя, 13) = "П000010000116" Тогда
		ЯчейкаЗамены = СтрЗаменить(Область.Имя, "П000010000116_", "П000010000117_");
		ПолеТабличногоДокументаФормаОтчета.Области[ЯчейкаЗамены].Значение = ?(РезультатНазвание = "Пустое значение", "",
			РезультатНазвание);
	КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-ПР");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1-ПР");
	...
	СтруктураРеквизитовФормы.мПечатныеФормы[2].Значение.УдалитьОбласть(
		СтруктураРеквизитовФормы.мПечатныеФормы[2].Значение.Области["НеПечатаемаяЧасть"],
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникПричиныЗабостовки",
	512, , "Выбор причины забостовки", КоллекцияСписковВыбора["ПричиныЗабостовки"]);
