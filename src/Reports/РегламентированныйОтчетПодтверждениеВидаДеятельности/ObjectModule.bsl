...
Функция СформироватьАвтоматическиРеглОтчет(ПараметрыАвтоформирования, КорректирующийОтчет) Экспорт
	...
	ПараметрыЗаписи.Вставить("СтруктураХраненияФайлаПояснительнойЗаписки", Неопределено);
...
Процедура СформироватьСтруктуруРеквизитовФормы(ИмяФормы, СтруктураРеквизитовФормы) Экспорт
	...
	СтруктураРеквизитовФормы.Вставить("АдресВоВрХранилищеФайлПояснительнойЗаписки");
...
Процедура ЗаполнитьСведенияОбОрганизации(ИмяФормы,
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2024
			И ОргСведения.Свойство("РегНомерСФР")
			И ТипЗнч(ОргСведения.РегНомерСФР) = Тип("Строка")
			И ЗначениеЗаполнено(ОргСведения.РегНомерСФР) Тогда
			
			РегНом = Лев(СокрЛП(ОргСведения.РегНомерСФР), 10);
			
		Иначе
			
			Если НЕ ЗначениеЗаполнено(РегНомПФР) Тогда
				ОргСведения.Свойство("РегНомПФР", РегНомПФР);
				РегНом = СокрЛП(СтрЗаменить(РегНомПФР, "-", ""));
			КонецЕсли;
	...
	Если ТипЗнч(СтруктураДанныхПриложение) = Тип("ТабличныйДокумент") Тогда
		...
		Если НЕ СтруктураДанныхПриложение.Области.Найти("РегистрационныйНомерФСС") = Неопределено Тогда 
			СтруктураДанныхПриложение.Области.РегистрационныйНомерФСС.Значение = РегНом;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(СтруктураДанныхПриложение) = Тип("Структура") Тогда
		...
		Если СтруктураДанныхПриложение.Свойство("РегистрационныйНомерФСС") Тогда
			СтруктураДанныхПриложение.РегистрационныйНомерФСС = РегНом;
		КонецЕсли;
...
Функция ЗаписатьДокумент(ИмяФормы, ПараметрыЗаписи) Экспорт
	...
	СтруктураХранения = ПараметрыЗаписи.СтруктураХраненияФайлаПояснительнойЗаписки;
	...
	Если СтруктураХранения <> Неопределено Тогда
		...
		ЗаписьРегистраСведений.РегламентированныйОтчет = ДокументРегламентированныйОтчет.Ссылка;
		ЗаписьРегистраСведений.ВидДополнительногоФайла = "ФайлПояснительнойЗаписки";
...
Функция ВывестиПоказательВXML(ИмяФормы, Узел, ЗначениеПоказателя)
	...
	Если Узел.Формат = "T" ИЛИ Узел.Формат = "Т" Тогда // Учтем оба варианта: кириллицу и латиницу.
		...
		КонецЕсли;
		Если Узел.Код = "content" Тогда
			Узел.Значение = ЗначениеПоказателяСтр;
		Иначе
...
Функция ВыгрузкаВозможна(ИмяФормы,
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.РегНомер) Тогда
		...
		"ru = 'Не задан регистрационный номер страхователя';
		|en = 'Не задан регистрационный номер страхователя'"), "Приложение1", "РегистрационныйНомерФСС");
		СтатусПроверки = Ложь;
	ИначеЕсли (СтрДлина(ПараметрыВыгрузки.РегНомер) <> 12 И СтрДлина(ПараметрыВыгрузки.РегНомер) <> 10)
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru = 'Задан регистрационный номер страхователя, не состоящий из 12 или 10 цифр';
		|en = 'Задан регистрационный номер страхователя, не состоящий из 12 или 10 цифр'"),
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.КодТеррОргана) Тогда
		...
		|en = 'Не задан код территориального органа Фонда пенсионного и социального страхования Российской Федерации'"),
	...
	Если СтрДлина(ПараметрыВыгрузки.ТекстПояснЗап) > 4000 Тогда
		...
		|en = 'Задан текст пояснительной записки, состоящий из более чем 4000 символов'"),
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ИНН) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru = 'Не задан ИНН организации';
		|en = 'Не задан ИНН организации'"), "Приложение2", "ИННОрганизации");
		СтатусПроверки = Ложь;
	Иначе
		ИНН = СокрЛП(ПараметрыВыгрузки.ИНН);
		Если СтрДлина(ИНН) <> 10 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ИНН) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
			"ru = 'Задан ИНН организации, не состоящий из 10 цифр';
			|en = 'Задан ИНН организации, не состоящий из 10 цифр'"), "Приложение2", "ИННОрганизации");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.КПП) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru = 'Не задан КПП организации';
		|en = 'Не задан КПП организации'"), "Приложение2", "КППОрганизации");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(СокрЛП(ПараметрыВыгрузки.КПП)) <> 9 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru = 'Задан КПП организации, не состоящий из 9 символов';
		|en = 'Задан КПП организации, не состоящий из 9 символов'"), "Приложение2", "КППОрганизации");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрыВыгрузки.ОГРН) Тогда
		...
		Если СтрДлина(ОГРН) <> 13 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ОГРН) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
			"ru = 'Задан ОГРН, не состоящий из 13 цифр';
			|en = 'Задан ОГРН, не состоящий из 13 цифр'"), "Приложение2", "ОГРН");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ДатаРег) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru = 'Не задана дата регистрации организации';
		|en = 'Не задана дата регистрации организации'"), "Приложение2", "ДатаРегистрации");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.МестоРег) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru = 'Не задано место регистрации организации';
		|en = 'Не задано место регистрации организации'"), "Приложение2", "МестоРегистрации");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ЮрАдрес) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru = 'Не задан юридический адрес организации';
		|en = 'Не задан юридический адрес организации'"), "Приложение2", "ЮрАдресОрганизации");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПараметрыВыгрузки.ДатаНачХозДеят = "" Тогда
		...
		|en = 'Не задана дата начала хозяйственной деятельности'"),
	...
	ИначеЕсли ПараметрыВыгрузки.ДатаНачХозДеят = Неопределено Тогда
		...
		|en = 'Задана дата начала хозяйственной деятельности, не соответствующая шаблону ""ДД.ММ.ГГГГ""'"),
		"Приложение2", "ДатаНачалаХозДеятельности");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ФИОРук) Тогда
		...
		|en = 'Не заданы фамилия, имя, отчество руководителя организации'"),
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ФИОГлавБух) Тогда
		...
		|en = 'Не заданы фамилия, имя, отчество главного бухгалтера организации'"),
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПараметрыВыгрузки.СреднЧислПредГод, , Истина) Тогда
		...
		|en = 'Задана среднесписочная численность работающих за предыдущий год, не состоящая только из цифр'"),
	...
	ИначеЕсли СтрДлина(СокрЛП(ПараметрыВыгрузки.СреднЧислПредГод)) > 6 Тогда
		...
		|en = 'Задана среднесписочная численность работающих за предыдущий год, превышающая число 999999'"),
		"Приложение2", "СреднесписочнаяЧисленность");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ ЗаполненаХотяБыОднастрока Тогда
		...
		|en = 'В распределении доходов и поступлений за предыдущий финансовый год не заполнена ни одна строка'"),
...
Функция СформироватьСтруктуруПараметров(ИмяФормы,
	...
	ПараметрыВыгрузки.Вставить("СодержимоеПояснЗап", "");
	ПараметрыВыгрузки.Вставить("РасширениеПояснЗап", "");
	ПараметрыВыгрузки.Вставить("РазмерПояснЗап",     0);
	...
	Если ЗаписьРегистраСведений.Выбран() Тогда
		...
		ДвДанные = ЗаписьРегистраСведений.СодержимоеФайла.Получить();
		B64 = Base64Строка(ДвДанные);
		B64 = СтрЗаменить(B64, Символы.ПС, "");
		B64 = СтрЗаменить(B64, Символы.ВК, "");
		
		МасПодстрок = СтрРазделить(ЗаписьРегистраСведений.ИмяФайла, ".");
		РасширениеПояснЗап = МасПодстрок[МасПодстрок.Количество() - 1];
		
		ПараметрыВыгрузки.Вставить("СодержимоеПояснЗап", B64);
		ПараметрыВыгрузки.Вставить("РасширениеПояснЗап", РасширениеПояснЗап);
		ПараметрыВыгрузки.Вставить("РазмерПояснЗап",     ЗаписьРегистраСведений.Размер);
...
Функция ПечатныйБланк(ПараметрыПечати) Экспорт
	...
	Если НЕ ПараметрыПечати.Свойство("СтруктураРеквизитовФормы", СтруктураРеквизитовФормы) Тогда
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСохраненныйДок) Тогда
			...
			СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета = СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания;
		КонецЕсли;	
	КонецЕсли;
