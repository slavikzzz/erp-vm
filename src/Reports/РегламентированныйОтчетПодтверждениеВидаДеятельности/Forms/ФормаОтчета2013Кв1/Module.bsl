...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОчиститьСтруктуруХраненияФайлаПояснительнойЗаписки();
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	Если ИмяТекРаздела = "Приложение3"
	   И СтруктураРеквизитовФормы.Редакция75н Тогда
		...
		ТабличныйДокумент.Вывести(ОбъектОтчета(ИмяФормы).ПолучитьМакет(Сред(ИмяФормы, СтрНайти(ИмяФормы, "ФормаОтчета")) + "_" + ИмяТекРаздела + "_ВРедакцииПриказа75н"));
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 И Область.Гиперссылка Тогда
		УдалитьСтроку("");
	ИначеЕсли Область.Имя = "ФайлПояснительнойЗаписки" Тогда
		СтандартнаяОбработка = Ложь;
		Если Область.Значение = "Добавить" Тогда
			ДобавитьФайл();
		Иначе
			СохранитьНаДискФайл();
		КонецЕсли;
		Возврат;
	ИначеЕсли Область.Имя = "УдалитьФайлПояснительнойЗаписки" Тогда
		СтандартнаяОбработка = Ложь;
		УдалитьФайл();
		Возврат;
	...
	Если (СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Приложение1" И ИмяПоказателя = "ОКВЭД")
	 ...
	 ИЛИ (СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Приложение3" И СтрНайти(ИмяПоказателя, "П000030000102") = 1) Тогда
		...
		Если ЗначениеЗаполнено(ОКВЭД) И (ДлинаОКВЭД < 4
			...
			|en = 'Задано некорректное значение кода вида деятельности по ОКВЭД!%1Допустимы значения, соответствующие следующим шаблонам:%2""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).'"),
...
Процедура ЗаполнитьРегистрирующийОрганНаСервере(ТекстСообщения = "")
	...
	Если МестоРегистрации = "" Тогда
		ТекстСообщения = НСтр("ru = 'В реквизитах организации не найдены сведения о регистрирующем органе';
								|en = 'В реквизитах организации не найдены сведения о регистрирующем органе'");
	КонецЕсли;
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант,
	...
	ПараметрыЗаписи.Вставить("СтруктураХраненияФайлаПояснительнойЗаписки", ПолучитьИзВременногоХранилища(
		СтруктураРеквизитовФормы["АдресВоВрХранилищеФайлПояснительнойЗаписки"]));
...
Процедура ВосстановитьСохраненныеДанные()
	...
	ВосстановитьДанныеДополнительныхФайлов();
...
&НаСервере
Процедура ВосстановитьДанныеДополнительныхФайлов()
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок <> Неопределено Тогда
		...
		Если ЗаписьРегистраСведений.Выбран() Тогда
			...
			ПоместитьВоВременноеХранилище(СтруктураХранения, УникальныйИдентификатор));
...
&НаСервере
Процедура ОчиститьСтруктуруХраненияФайлаПояснительнойЗаписки()
...
#Область ДействияСФайломПояснительнойЗаписки
...
Процедура ДобавитьФайлЗавершение(Результат, АдресФайла, ВыбИмяФайла, Парам) Экспорт
	...
	ВозможныеРасширения.Добавить(".TXT");
	...
	ВозможныеРасширения.Добавить(".ZIP");
	ВозможныеРасширения.Добавить(".RAR");
	...
	Если ВозможныеРасширения.Найти(РасшДл4) = Неопределено
		...
		ШаблонТекста = НСтр(
			...
			|en = 'Файл должен иметь одно из допустимых расширений:%1'");
...
Процедура СохранитьНаДискФайл()
	...
	СтруктураХранения = ПолучитьИзВременногоХранилища(
		СтруктураРеквизитовФормы["АдресВоВрХранилищеФайлПояснительнойЗаписки"]);
...
Процедура ПереопределитьЗначенияСвойствЯчеекТабличногоДокумента(ИмяРаздела)
	...
	Если ИмяРаздела = "Приложение1" Тогда
		...
		Если СтруктураХранения = Неопределено Тогда
			Обл = ТабличныйДокумент.Области.Найти("ФайлПояснительнойЗаписки");
			Если Обл <> Неопределено Тогда
				Обл.Значение = "Добавить";
			КонецЕсли;
			Обл = ТабличныйДокумент.Области.Найти("УдалитьФайлПояснительнойЗаписки");
			Если Обл <> Неопределено Тогда
		...
		Иначе
			Обл = ТабличныйДокумент.Области.Найти("ФайлПояснительнойЗаписки");
			Если Обл <> Неопределено Тогда
				Обл.Значение = СтруктураХранения["Имя"];
			КонецЕсли;
			Обл = ТабличныйДокумент.Области.Найти("УдалитьФайлПояснительнойЗаписки");
			Если Обл <> Неопределено Тогда
