...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/12/2011";
...
Процедура ПоказатьПериод()
	...
	ТабличныйДокумент.Области.ОтчетныйГод.Значение = ОтчетныйГод;
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма, ИзмененаДатаПодписи = Ложь)
	...
	Если НЕ ИзмененаДатаПодписи Тогда
		...
		Форма.ТабличныйДокумент.Области.ТелефонОрганизации.Значение = СокрЛП(СведенияОбОрганизации.ТелОрганизации);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 7) = "КодИМНС" Тогда
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010002005", "П000010002003;П000010002004");	
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010002105", "П000010002103;П000010002104");	
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010002205", "П000010002203;П000010002204");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010002305", "П000010002303;П000010002304");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010002405", "П000010002403;П000010002404");	
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010002505", "П000010002503;П000010002504");	
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010002605", "П000010002603;П000010002604");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010002705", "П000010002703;П000010002704");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010002805", "П000010002803;П000010002804");	
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010002905", "П000010002903;П000010002904");	
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010003005", "П000010003003;П000010003004");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010003105", "П000010003103;П000010003104");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010003205", "П000010003203;П000010003204");
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "") Экспорт
	...
	Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Расчет" Тогда
		
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(
			Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000010001303, ОбластьИмя) > 0 Тогда
			Строка010 = Форма.ТабличныйДокумент.Области.П000010001003.Значение;
			Строка011 = Форма.ТабличныйДокумент.Области.П000010001103.Значение;
			Строка012 = Форма.ТабличныйДокумент.Области.П000010001203.Значение;
			Если Строка010 <> 0 Тогда
				ЗначениеСтроки013 = (Строка011 + Строка012) / Строка010 * 100;
			Иначе
				ЗначениеСтроки013 = 0;
			КонецЕсли;
			Форма.ТабличныйДокумент.Области.П000010001303.Значение = ЗначениеСтроки013;
		КонецЕсли;
		
		Для НомерСтроки = 20 По 32 Цикл
			ИмяСтроки = "П00001000" + НомерСтроки;
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(
				Форма.СтруктураЗависимыхЯчеекДляАвторасчета[ИмяСтроки + "05"], ОбластьИмя) > 0 Тогда
				Графа3 = Форма.ТабличныйДокумент.Области[ИмяСтроки + "03"].Значение;
				Графа4 = Форма.ТабличныйДокумент.Области[ИмяСтроки + "04"].Значение;
				Если Графа3 <> 0 Тогда
					ЗначениеГрафы5 = Графа4 / Графа3 * 100;
				Иначе
					ЗначениеГрафы5 = 0;
				КонецЕсли;
				Форма.ТабличныйДокумент.Области[ИмяСтроки + "05"].Значение = ЗначениеГрафы5;
			КонецЕсли;
		КонецЦикла;
		
	Иначе
		
		Если Форма.СтруктураДанныхРасчет.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(
			Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000010001303, ОбластьИмя) > 0 Тогда
			Строка010 = Форма.СтруктураДанныхРасчет.П000010001003;
			Строка011 = Форма.СтруктураДанныхРасчет.П000010001103;
			Строка012 = Форма.СтруктураДанныхРасчет.П000010001203;
			Если Строка010 <> 0 Тогда
				ЗначениеСтроки013 = (Строка011 + Строка012) / Строка010 * 100;
			Иначе
				ЗначениеСтроки013 = 0;
			КонецЕсли;
			Форма.СтруктураДанныхРасчет.Вставить("П000010001303", ЗначениеСтроки013);
		КонецЕсли;
		...
		Для НомерСтроки = 20 По 32 Цикл
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(
				Форма.СтруктураЗависимыхЯчеекДляАвторасчета[ИмяСтроки + "05"], ОбластьИмя) > 0 Тогда
				Графа3 = Форма.СтруктураДанныхРасчет[ИмяСтроки + "03"];
				Графа4 = Форма.СтруктураДанныхРасчет[ИмяСтроки + "04"];
				Если Графа3 <> 0 Тогда
					ЗначениеГрафы5 = Графа4 / Графа3 * 100;
				Иначе
					ЗначениеГрафы5 = 0;
				КонецЕсли;
				Форма.СтруктураДанныхРасчет.Вставить(ИмяСтроки + "05", ЗначениеГрафы5);
			КонецЕсли;
		КонецЦикла;
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Титульный", "МЧБ_2011Кв4_1");
	СоответствиеРазделовМакетам.Вставить("Расчет",	  "МЧБ_2011Кв4_2");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы",             "NO_DOHOBMED_1_123_00_05_01_01.xsd");
	ПараметрыВыгрузки.Вставить("ПрефиксИмениФайлаВыгрузки", "NO_DOHOBMED");
	...
	ПараметрыВыгрузки.Вставить("ПрПодп",             СокрЛП(Титульный.ТипПодписанта));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	Если ПараметрыВыгрузки.ПрПодп = "2" И ПустаяСтрока(ПараметрыВыгрузки.НаимДокПредставителя) Тогда
	...
	// Проверка соотношения показателей в разделе "Расчет".
	ДанныеРазделаРасчет = мДанныеОтчета.ПолеТабличногоДокументаРасчет;
	
	ЗначениеСтроки010 = ДанныеРазделаРасчет.П000010001003;
	ЗначениеСтроки011 = ДанныеРазделаРасчет.П000010001103;
	ЗначениеСтроки012 = ДанныеРазделаРасчет.П000010001203;
	
	СоотношениеНарушено = (ЗначениеСтроки010 < (ЗначениеСтроки011 + ЗначениеСтроки012));
	Если СоотношениеНарушено Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Сумма значения строк 011 и 012 не может превышать значение строки 010!", "Расчет", "П000010001103");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Для Инд = 20 По 32 Цикл
		НомерСтроки = Формат(Инд, "ЧЦ=3; ЧВН=; ЧГ=0");
		ИмяСтроки = "П0000100" + НомерСтроки;
		
		ЗначениеГрафы3 = ДанныеРазделаРасчет[ИмяСтроки + "03"];
		ЗначениеГрафы4 = ДанныеРазделаРасчет[ИмяСтроки + "04"];
		
		СоотношениеНарушено = (ЗначениеГрафы3 < ЗначениеГрафы4);
		Если СоотношениеНарушено Тогда
			ТекстСообщения = "Значение в графе 4 строки " + НомерСтроки + " не может превышать значение в графе 3!";
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, ТекстСообщения, "Расчет", ИмяСтроки + "04");
			СтатусПроверки = Ложь;
		КонецЕсли;
...
Функция СформироватьИдФайл(ПараметрыВыгрузки)
	...
	Возврат ПараметрыВыгрузки.ПрефиксИмениФайлаВыгрузки
			...
			+ "_" + ?(ПараметрыВыгрузки.Свойство("ЭтоПБОЮЛ") И ПараметрыВыгрузки.ЭтоПБОЮЛ, СокрЛП(ПараметрыВыгрузки.ИННЮЛОрг), СокрЛП(ПараметрыВыгрузки.ИННЮЛОрг) + СокрЛП(ПараметрыВыгрузки.КППОрг))
