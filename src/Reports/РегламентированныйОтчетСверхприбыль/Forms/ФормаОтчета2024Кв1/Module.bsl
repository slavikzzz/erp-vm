...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("ЗаполняетсяРаздел21", Ложь);
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если ЗаполнитьЗначенияПоУмолчанию Тогда
		...
		ДеревоР1_2 = ПолучитьИзВременногоХранилища(СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхРазделы1_2"]);
		
		Если ДеревоР1_2.Строки.Количество() = 1 Тогда
			...
			ДеревоР1_2.Строки[0].Данные.Вставить("П000010000103", ОКТМО);
			
			СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхРазделы1_2"]
			= ПоместитьВоВременноеХранилище(ДеревоР1_2, УникальныйИдентификатор);
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено
		...
		Если ЗаполняетсяРаздел21() Тогда
			
			ИнициализироватьМногоуровневыйРаздел("Раздел21");
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	ИменаГраф.Добавить("П00002М113501");
	ИменаГраф.Добавить("П00002М113502");
	ИменаГраф.Добавить("П00002М113503");
...
Процедура ИнициализироватьМногоуровневыеРазделы()
	...
	мСтруктураМногоуровневыхРазделов.Вставить("Разделы1_2", СтруктураРаздела);
...
Процедура ФормироватьСтруктуруСтраницОтчета() Экспорт
	...
	СтрокаУровня1 = мДеревоСтраницОтчета.ПолучитьЭлементы().Добавить();
	СтрокаУровня1.ИмяСтраницы        = "Разделы1_2";
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СпискиВыбора2024Кв1");
	...
	//Раздел 2.
	//
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020007003", 1, ,
	"Выбор признака", КоллекцияСписковВыбора["ПризнакиДаНет"]);
	
	//Раздел 2.1.
	//
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000210007003", 1, ,
	"Выбор признака", КоллекцияСписковВыбора["ПризнакиДаНет"]);
	
	//Раздел 3.
	//
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000030022003", 1, ,
	"Выбор кода реорганизации", КоллекцияСписковВыбора["КодыРеорганизацииР3"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000030028003", 1, ,
	"Выбор признака", КоллекцияСписковВыбора["ПризнакиДаНет"]);
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ИмяРаздела = "Титульный" Тогда
		ЗаполнитьПериодОтчета(ЭтаФорма);
	ИначеЕсли ИмяРаздела = "Разделы1_2" И СокрЛП(ТабличныйДокумент.Области["П000020007003"].Значение) <> "1" Тогда
		ИнициализироватьМногоуровневыйРаздел("Раздел21");
	КонецЕсли;
...
Процедура ОбработкаВыбораИФНСЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено Тогда
		...
		УстановитьКодПоОКТМОПриАктивномТитульномЛисте(РезультатВыбора.Ссылка);
...
&НаСервере
Процедура УстановитьКодПоОКТМОПриАктивномТитульномЛисте(СсылкаНаРегистрациюВНО = Неопределено)
	...
	Если ДеревоР1_2.Строки.Количество() = 1 Тогда
		...
		ДеревоР1_2.Строки[0].Данные.Вставить("П000010000103", СокрЛП(ОКТМО));
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Разделы1_2" Тогда
	...
	ИначеЕсли ИмяРаздела = "Раздел21" Тогда
		...
		ТДО["СекцияПояснение_Раздел21"].Видимость = НЕ Форма.СтруктураРеквизитовФормы.ЗаполняетсяРаздел21;
		
		Если Форма.СтруктураРеквизитовФормы.ЗаполняетсяРаздел21 Тогда
		...
		Иначе
			...
			Для каждого Обл Из ТДО Цикл
				Если СтрНачинаетсяС(Обл.Имя, "П0") И Обл.СодержитЗначение = Истина Тогда
					...
					Обл.Формат = ?(ТипЗнч(Обл.Значение) = Тип("Число"), "", Обл.Формат);
				КонецЕсли;
			КонецЦикла;
	...
	ИначеЕсли ИмяРаздела = "Раздел3" Тогда
		...
		ЗаполняетсяДоляНпл = СокрЛП(ТДО["П000030022003"].Значение) = "3";
		
		ТДО["СекцияПояснение_Раздел3"].Видимость = НЕ ЗаполняетсяДоляНпл;
		
		Обл = ТДО["П000030029003"];
		Если ЗаполняетсяДоляНпл Тогда
			РазрешитьВводДанныхВОбласть(Форма, Обл);
			Обл.Формат = СтрЗаменить(Обл.Формат, "ЧН=-", "ЧН=0,00000000");
		Иначе
			ЗапретитьВводДанныхВОбласть(Форма, Обл);
			Обл.Формат = СтрЗаменить(Обл.Формат, "ЧН=0,00000000", "ЧН=-");
		КонецЕсли;
...
Процедура ВывестиДанныеВТабличныйДокумент(ИмяТекРаздела, НовыеНомераСтрокМногоуровнегоРаздела = Неопределено)
	...
	Если мСтруктураМногоуровневыхРазделов.Свойство(ИмяТекРаздела) Тогда
		...
		// Особая обработка.
		СтруктураРеквизитовФормы.ЗаполняетсяРаздел21 = ЗаполняетсяРаздел21();
...
&НаСервере
Функция ЗаполняетсяРаздел21()
	
	ДеревоР1_2 = ПолучитьИзВременногоХранилища(СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхРазделы1_2"]);
	
	Возврат СокрЛП(ДеревоР1_2.Строки[0].Данные["П000020007003"]) = "1";
...
Процедура Расчет(Форма, ИмяРаздела = "", ОбластьИмя = "") Экспорт
	...
	Если ИмяРаздела = "Разделы1_2" Тогда
		
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела И ЗначениеЗаполнено(ОбластьИмя) Тогда
			
			// Раздел 2.
			//
			ТДО["П000020005003"].Значение = Макс(0, Окр((ТДО["П000020001003"].Значение + ТДО["П000020002003"].Значение) / 2));
			
			ТДО["П000020006003"].Значение = Макс(0, Окр((ТДО["П000020003003"].Значение + ТДО["П000020004003"].Значение) / 2));
			
			Если СокрЛП(ТДО["П000020007003"].Значение) = "0" Тогда
				ТДО["П000020010003"].Значение = Макс(0, ТДО["П000020006003"].Значение - ТДО["П000020005003"].Значение);
			ИначеЕсли СокрЛП(ТДО["П000020007003"].Значение) = "1" Тогда
				ТДО["П000020010003"].Значение = Макс(0, Окр(ТДО["П000020004003"].Значение / 2));
			Иначе
				ТДО["П000020010003"].Значение = 0;
			КонецЕсли;
			
			ТДО["П000020012003"].Значение = Макс(0, Окр(ТДО["П000020010003"].Значение * 10 / 100));
			
			ИтогоСтр135 = 0;
			РегламентированнаяОтчетностьКлиентСервер.УстановитьИндекс(Индекс, ИндексСтр);
			Пока ТДО.Найти("П00002М113503_" + ИндексСтр) <> Неопределено Цикл
				ИтогоСтр135 = ИтогоСтр135 + ТДО["П00002М113503_" + ИндексСтр].Значение;
				РегламентированнаяОтчетностьКлиентСервер.УвеличитьИндекс(Индекс, ИндексСтр);
			КонецЦикла;
			ТДО["П000020013003"].Значение = Макс(0, ИтогоСтр135);
			
			ТДО["П000020012503"].Значение = Макс(0, Мин(Окр(ТДО["П000020013003"].Значение),
			Окр(ТДО["П000020012003"].Значение / 2, 0, РежимОкругления.Окр15как10)));
			
			// Раздел 1.
			//
			ТДО["П000010001003"].Значение = Макс(0, ТДО["П000020012003"].Значение - ТДО["П000020012503"].Значение);
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Разделы1_2" Тогда
		...
		ДерМнЧ = Ветка.ДанныеМногострочныхЧастей[Форма.СтруктураРеквизитовФормы.мИдГруппыР2М1];
		
		// Раздел 2.
		//
		Дан.Вставить("П000020005003", Макс(0, Окр((Дан["П000020001003"] + Дан["П000020002003"]) / 2)));
		
		Дан.Вставить("П000020006003", Макс(0, Окр((Дан["П000020003003"] + Дан["П000020004003"]) / 2)));
		
		Если СокрЛП(Дан["П000020007003"]) = "0" Тогда
			Дан.Вставить("П000020010003", Макс(0, Дан["П000020006003"] - Дан["П000020005003"]));
		ИначеЕсли СокрЛП(Дан["П000020007003"]) = "1" Тогда
			Дан.Вставить("П000020010003", Макс(0, Окр(Дан["П000020004003"] / 2)));
		Иначе
			Дан.Вставить("П000020010003", 0);
		КонецЕсли;
		
		Дан.Вставить("П000020012003", Макс(0, Окр(Дан["П000020010003"] * 10 / 100)));
		
		ИтогоСтр135 = 0;
		Для каждого СтрМнЧ Из ДерМнЧ.Строки Цикл
			ИтогоСтр135 = ИтогоСтр135 + СтрМнЧ.Данные["П00002М113503"];
		КонецЦикла;
		Дан.Вставить("П000020013003", Макс(0, ИтогоСтр135));
		
		Дан.Вставить("П000020012503", Макс(0, Мин(Окр(Дан["П000020013003"]),
		Окр(Дан["П000020012003"] / 2, 0, РежимОкругления.Окр15как10))));
		
		// Раздел 1.
		//
		Дан.Вставить("П000010001003", Макс(0, Дан["П000020012003"] - Дан["П000020012503"]));
	...
	ИначеЕсли ИмяРаздела = "Раздел21" Тогда
		...
		ДеревоДанныхР1_2 = ПолучитьИзВременногоХранилища(
		Форма.СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхРазделы1_2"]);
		ВеткаР1_2 = ДеревоДанныхР1_2.Строки[0];
		ДанР1_2 = ВеткаР1_2.Данные;
		
		Если СокрЛП(ДанР1_2["П000020007003"]) = "1" Тогда
			
			Дан.Вставить("П000210005003", Макс(0, Окр((Дан["П000210001003"] + Дан["П000210002003"]) / 2, 2)));
			
			Дан.Вставить("П000210006003", Макс(0, Окр((Дан["П000210003003"] + Дан["П000210004003"]) / 2, 2)));
			
			Дан.Вставить("П000210008003", Макс(0,
			?(ДанР1_2["П000020005003"] = 0, 0, Окр(ДанР1_2["П000020006003"] / ДанР1_2["П000020005003"], 6))));
			
			Дан.Вставить("П000210009003", Макс(0,
			?(Дан["П000210005003"] = 0, 0, Окр(Дан["П000210006003"] / Дан["П000210005003"], 6))));
			
		Иначе
			
			Дан.Вставить("П000210005003", 0);
			Дан.Вставить("П000210006003", 0);
			Дан.Вставить("П000210008003", 0);
			Дан.Вставить("П000210009003", 0);
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ.Вставить("П000020012503");
	Структ.Вставить("П000020013003");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Разделы1_2", Структ);
	
	Структ = Новый Структура;
	Структ.Вставить("П000210005003");
	Структ.Вставить("П000210006003");
	Структ.Вставить("П000210008003");
	Структ.Вставить("П000210009003");
...
Функция ЗаголовокРегламентированногоОтчета(ДатаНач, ДатаКон, Периодичность = Неопределено) Экспорт
	...
	Если СтрНайти(ЭтаФорма.Заголовок, СтрПериодОтчета) = 0 Тогда
		Возврат ЭтаФорма.Заголовок + " в " + СтрПериодОтчета;
	Иначе
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "НалСверхПриб");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "NO_SVERHPRIB"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.НаимОрг) Тогда
		СообщитьВТС("Не задано полное наименование организации", "Титульный", "НаимОрг");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	// Разделы 1, 2.
	//
	ДерРазд = СохрСтруктура.ДанныеМногоуровневыхРазделов["Разделы1_2"];
	НомСтраницы = "1";
	Дан = ДерРазд.Строки[0].Данные;
	ДерМнЧ = ДерРазд.Строки[0].ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР2М1];
	
	// Раздел 1.
	//
	ДлЗнПок = СтрДлина(СокрЛП(Дан["П000010000103"]));
	Если ДлЗнПок <> 8 И ДлЗнПок <> 11 Тогда
		СообщитьВТС("Раздел 1: " + ?(ДлЗнПок = 0, "не ", "") + "задан код по ОКТМО" + ?(ДлЗнПок = 0,
		"", ", не состоящий из 8 или 11 цифр"), "Разделы1_2", "П000010000103", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если Дан["П000010001003"] < 0 Тогда
		СообщитьВТС("Раздел 1: в строке 010 значение не должно быть отрицательным",
		"Разделы1_2", "П000010001003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	// Раздел 2.
	//
	Если Дан["П000020001003"] < 0 Тогда
		СообщитьВТС("Раздел 2: в строке 010 значение не должно быть отрицательным",
		"Разделы1_2", "П000020001003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Дан["П000020002003"] < 0 Тогда
		СообщитьВТС("Раздел 2: в строке 020 значение не должно быть отрицательным",
		"Разделы1_2", "П000020002003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Дан["П000020003003"] < 0 Тогда
		СообщитьВТС("Раздел 2: в строке 030 значение не должно быть отрицательным",
		"Разделы1_2", "П000020003003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Дан["П000020004003"] < 0 Тогда
		СообщитьВТС("Раздел 2: в строке 040 значение не должно быть отрицательным",
		"Разделы1_2", "П000020004003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Дан["П000020005003"] < 0 Тогда
		СообщитьВТС("Раздел 2: в строке 050 значение не должно быть отрицательным",
		"Разделы1_2", "П000020005003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Дан["П000020006003"] < 0 Тогда
		СообщитьВТС("Раздел 2: в строке 060 значение не должно быть отрицательным",
		"Разделы1_2", "П000020006003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	ВозмПрДаНет = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000020007003");
	ЗнПок = СокрЛП(Дан["П000020007003"]);
	Если НЕ ЗначениеЗаполнено(ЗнПок) Тогда
		СообщитьВТС("Раздел 2: не задан признак заполнения Раздела 2.1 Декларации",
		"Разделы1_2", "П000020007003", НомСтраницы);
		СтатусПроверки = Ложь;
	ИначеЕсли ВозмПрДаНет.МассивЗначений.Найти(ЗнПок) = Неопределено Тогда
		СообщитьВТС("Раздел 2: признак заполнения Раздела 2.1 Декларации должен принимать одно из значений: "
		+ ВозмПрДаНет.СтрокаЗначений, "Разделы1_2", "П000020007003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	ЗаполняетсяРаздел21 = ЗнПок = "1";
	
	Если Дан["П000020010003"] < 0 Тогда
		СообщитьВТС("Раздел 2: в строке 100 значение не должно быть отрицательным",
		"Разделы1_2", "П000020010003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Дан["П000020012003"] < 0 Тогда
		СообщитьВТС("Раздел 2: в строке 120 значение не должно быть отрицательным",
		"Разделы1_2", "П000020012003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Дан["П000020012503"] < 0 Тогда
		СообщитьВТС("Раздел 2: в строке 125 значение не должно быть отрицательным",
		"Разделы1_2", "П000020012503", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Дан["П000020013003"] < 0 Тогда
		СообщитьВТС("Раздел 2: в строке 130 значение не должно быть отрицательным",
		"Разделы1_2", "П000020013003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ЗаполненаМногострочнаяЧасть(ДерМнЧ) Тогда
		Для каждого СтрМнЧ Из ДерМнЧ.Строки Цикл
			ИндСтрМнЧ = ДерМнЧ.Строки.Индекс(СтрМнЧ);
			НомСтрМнЧ = Формат(ИндСтрМнЧ + 1, "ЧГ=");
			
			ЗнПок = СтрМнЧ.Данные["П00002М113501"];
			Если НЕ ДатаВходитВРазрешенныйПериод(ЗнПок) Тогда
				СообщитьВТС("Раздел 2: в строке " + НомСтрМнЧ
				+ " группы строк 135 " + ?(ЗначениеЗаполнено(ЗнПок), "", "не ") + "задана дата документа"
				+ ?(ЗначениеЗаполнено(ЗнПок), ", выходящая за пределы периода с 01.01.1900 по 31.12.2099", ""),
				"Разделы1_2", "П00002М113501_" + НомСтрМнЧ, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(СокрЛП(СтрМнЧ.Данные["П00002М113502"])) Тогда
				СообщитьВТС("Раздел 2: в строке " + НомСтрМнЧ + " группы строк 135 не задан номер документа",
				"Разделы1_2", "П00002М113502_" + НомСтрМнЧ, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если СтрМнЧ.Данные["П00002М113503"] < 0 Тогда
				СообщитьВТС("Раздел 2: в строке " + НомСтрМнЧ
				+ " группы строк 135 значение суммы не должно быть отрицательным",
				"Разделы1_2", "П00002М113503_" + НомСтрМнЧ, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
	...
	// Раздел 2.1.
	//
	ДерРазд = СохрСтруктура.ДанныеМногоуровневыхРазделов["Раздел21"];
	НомСтраницы = "1";
	
	Если ЗаполняетсяРаздел21 Тогда
		Если ЗаполненРаздел(ДерРазд, "Раздел21") Тогда
			
			Дан = ДерРазд.Строки[0].Данные;
			
			Если Дан["П000210001003"] < 0 Тогда
				СообщитьВТС("Раздел 2.1: в строке 010 значение не должно быть отрицательным",
				"Раздел21", "П000210001003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если Дан["П000210002003"] < 0 Тогда
				СообщитьВТС("Раздел 2.1: в строке 020 значение не должно быть отрицательным",
				"Раздел21", "П000210002003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если Дан["П000210003003"] < 0 Тогда
				СообщитьВТС("Раздел 2.1: в строке 030 значение не должно быть отрицательным",
				"Раздел21", "П000210003003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если Дан["П000210004003"] < 0 Тогда
				СообщитьВТС("Раздел 2.1: в строке 040 значение не должно быть отрицательным",
				"Раздел21", "П000210004003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если Дан["П000210005003"] < 0 Тогда
				СообщитьВТС("Раздел 2.1: в строке 050 значение не должно быть отрицательным",
				"Раздел21", "П000210005003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если Дан["П000210006003"] < 0 Тогда
				СообщитьВТС("Раздел 2.1: в строке 060 значение не должно быть отрицательным",
				"Раздел21", "П000210006003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ВозмПрДаНет = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000210007003");
			ЗнПок = СокрЛП(Дан["П000210007003"]);
			Если НЕ ЗначениеЗаполнено(ЗнПок) Тогда
				СообщитьВТС("Раздел 2.1: не задан Признак распределения прибыли за 2021 и (или) 2022 год",
				"Раздел21", "П000210007003", НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ВозмПрДаНет.МассивЗначений.Найти(ЗнПок) = Неопределено Тогда
				СообщитьВТС("Раздел 2.1: Признак распределения прибыли за 2021 и (или) 2022 год должен принимать одно из значений: "
				+ ВозмПрДаНет.СтрокаЗначений, "Раздел21", "П000210007003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если Дан["П000210008003"] < 0 Тогда
				СообщитьВТС("Раздел 2.1: в строке 080 значение не должно быть отрицательным",
				"Раздел21", "П000210008003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если Дан["П000210009003"] < 0 Тогда
				СообщитьВТС("Раздел 2.1: в строке 090 значение не должно быть отрицательным",
				"Раздел21", "П000210009003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
		Иначе
			
			СообщитьВТС("Раздел 2.1 должен быть заполнен, если в строке с кодом 070 Раздела 2 задано значение ""1""",
			"Раздел21", "П000210001003", НомСтраницы);
			СтатусПроверки = Ложь;
			
		Конецесли;
	ИначеЕсли ЗаполненРаздел(ДерРазд, "Раздел21") Тогда
		
		СообщитьВТС("Раздел 2.1 не должен быть заполнен, если в строке с кодом 070 Раздела 2 не задано значение ""1""",
		"Раздел21", "П000210001003", НомСтраницы);
		СтатусПроверки = Ложь;
	...
	// Раздел 3.
	//
	ДерРазд = СохрСтруктура.ДанныеМногоуровневыхРазделов["Раздел3"];
	
	Если ЗаполненРаздел(ДерРазд, "Раздел3") Тогда
		
		ВозмКодыРеорг = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000030022003");
		ВозмПрДаНет   = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000030028003");
		
		Для каждого Ветка Из ДерРазд.Строки Цикл
			
			НомСтраницы = Формат(ДерРазд.Строки.Индекс(Ветка) + 1, "ЧГ=");
			Дан = Ветка.Данные;
			
			ЗнПок = СокрЛП(Дан["П000030022003"]);
			Если НЕ ЗначениеЗаполнено(ЗнПок) Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": не задан код формы реорганизации",
				"Раздел3", "П000030022003", НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ВозмКодыРеорг.МассивЗначений.Найти(ЗнПок) = Неопределено Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": код формы реорганизации должен принимать одно из значений: "
				+ ВозмКодыРеорг.СтрокаЗначений, "Раздел3", "П000030022003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ЗнПок = СокрЛП(Дан["П000030025003"]);
			Если НЕ ЗначениеЗаполнено(ЗнПок) Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": не задан ИНН реорганизованной организациии",
				"Раздел3", "П000030025003", НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли СтрДлина(ЗнПок) <> 10 Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы
				+ ": задан ИНН реорганизованной организациии, не состоящий из 10 цифр", "Раздел3", "П000030025003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ЗнПок = СокрЛП(Дан["П000030026003"]);
			Если НЕ ЗначениеЗаполнено(ЗнПок) Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы
				+ ": не задано полное наименование реорганизованной организациии", "Раздел3", "П000030026003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ЗнПок = Дан["П000030027003"];
			Если НЕ ДатаВходитВРазрешенныйПериод(ЗнПок) Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": " + ?(ЗначениеЗаполнено(ЗнПок), "", "не ")
				+ "задана дата завершения реорганизации" + ?(ЗначениеЗаполнено(ЗнПок),
				", выходящая за пределы периода с 01.01.1900 по 31.12.2099", ""), "Раздел3", "П000030027003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ЗнПок = СокрЛП(Дан["П000030028003"]);
			Если НЕ ЗначениеЗаполнено(ЗнПок) Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": не задан признак корректировки налоговой базы",
				"Раздел3", "П000030028003", НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ВозмПрДаНет.МассивЗначений.Найти(ЗнПок) = Неопределено Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы
				+ ": признак корректировки налоговой базы должен принимать одно из значений: "
				+ ВозмПрДаНет.СтрокаЗначений, "Раздел3", "П000030028003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если СокрЛП(Дан["П000030022003"]) <> "3" И ЗначениеЗаполнено(Дан["П000030029003"]) Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы
				+ ": строка 290 не должна быть заполнена, если значение в строке 220 не равно ""3""",
				"Раздел3", "П000030029003", НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли Дан["П000030029003"] < 0 Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": в строке 290 значение не должно быть отрицательным",
				"Раздел3", "П000030029003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если Дан["П000030030003"] < 0 Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": в строке 300 значение не должно быть отрицательным",
				"Раздел3", "П000030030003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если Дан["П000030031003"] < 0 Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": в строке 310 значение не должно быть отрицательным",
				"Раздел3", "П000030031003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если Дан["П000030032003"] < 0 Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": в строке 320 значение не должно быть отрицательным",
				"Раздел3", "П000030032003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если Дан["П000030033003"] < 0 Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": в строке 330 значение не должно быть отрицательным",
				"Раздел3", "П000030033003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, ПараметрыВыгрузки)
	...
	ЗаполнитьДаннымиУзел(ДерРазд.Строки[0].Данные, УзелРодитель);
	// Многострочные данные.
	ДерМнЧ = ДерРазд.Строки[0].ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР2М1];
	РасчСумНал  = ПолучитьПодчиненныйЭлемент(УзелРодитель, "РасчСумНал");
	РеквСумПлат = ПолучитьПодчиненныйЭлемент(РасчСумНал, "РеквСумПлат");
	Если ЗаполненаМногострочнаяЧасть(ДерМнЧ) Тогда
		Для каждого СтрМнЧ Из ДерМнЧ.Строки Цикл
			ЗаполнитьДаннымиУзел(СтрМнЧ.Данные,
			СкопироватьУзел(РасчСумНал, РеквСумПлат, РасчСумНал.Строки.Количество() - 1));
		КонецЦикла;
	КонецЕсли;
	РегламентированнаяОтчетность.УдалитьУзел(РеквСумПлат);
	
	// Раздел 2.1.
	//
	ДерРазд = СохрСтруктура.ДанныеМногоуровневыхРазделов["Раздел21"];
	ПокСтоимАкт = ПолучитьПодчиненныйЭлемент(РасчСумНал, "ПокСтоимАкт");
	Если ЗаполненРаздел(ДерРазд, "Раздел21") Тогда
		ЗаполнитьДаннымиУзел(ДерРазд.Строки[0].Данные,
		СкопироватьУзел(РасчСумНал, ПокСтоимАкт, РасчСумНал.Строки.Количество() - 1));
	КонецЕсли;
	РегламентированнаяОтчетность.УдалитьУзел(ПокСтоимАкт);
	
	// Раздел 3.
	//
	ДерРазд = СохрСтруктура.ДанныеМногоуровневыхРазделов["Раздел3"];
	СведРеорг = ПолучитьПодчиненныйЭлемент(УзелРодитель, "СведРеорг");
	Если ЗаполненРаздел(ДерРазд, "Раздел3") Тогда
		Для каждого Ветка Из ДерРазд.Строки Цикл
			ЭкзСведРеорг = СкопироватьУзел(УзелРодитель, СведРеорг, УзелРодитель.Строки.Количество() - 1);
			
			ДоляНП = ПолучитьПодчиненныйЭлемент(ЭкзСведРеорг, "ДоляНП");
			Если Ветка.Данные["П000030022003"] = "3" Тогда
				ДоляНП.Обязательность = "0";
			Иначе
				РегламентированнаяОтчетность.УдалитьУзел(ДоляНП);
			Конецесли;
			
			ЗаполнитьДаннымиУзел(Ветка.Данные, ЭкзСведРеорг);
		КонецЦикла;
	КонецЕсли;
	РегламентированнаяОтчетность.УдалитьУзел(СведРеорг);
...
Функция ВыгрузкаОтчета(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "Сверхприбыль");
...
Процедура ВывестиНаПечатьДанныеМногоуровневогоРаздела(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	Если ИмяТекРаздела = "Разделы1_2" Тогда
		...
		ИмяП00002М113501 = "П00002М113501";
		ИмяП00002М113502 = "П00002М113502";
		ИмяП00002М113503 = "П00002М113503";
		Для НомЛиста = 1 По КолЛистовРаздела Цикл
			...
			Для ИндСтрокиМнЧ = 1 По СтрокМнЧНаЛисте Цикл
				...
				НовИмяП00002М113501 = ИмяП00002М113501 + _ИндСтрокиМнЧ;
				НовИмяП00002М113502 = ИмяП00002М113502 + _ИндСтрокиМнЧ;
				НовИмяП00002М113503 = ИмяП00002М113503 + _ИндСтрокиМнЧ;
				ДанныеМнЧДляПечати.Вставить(НовИмяП00002М113501, '00010101');
				ДанныеМнЧДляПечати.Вставить(НовИмяП00002М113502, "");
				ДанныеМнЧДляПечати.Вставить(НовИмяП00002М113503, 0);
				Если КолСтрокМнЧ >= ИндСПриращением Тогда
					ДанныеМнЧДляПечати.Вставить(НовИмяП00002М113501, ДанныеМнЧ.Строки[ИндСПриращением - 1].Данные[ИмяП00002М113501]);
					ДанныеМнЧДляПечати.Вставить(НовИмяП00002М113502, ДанныеМнЧ.Строки[ИндСПриращением - 1].Данные[ИмяП00002М113502]);
					ДанныеМнЧДляПечати.Вставить(НовИмяП00002М113503, ДанныеМнЧ.Строки[ИндСПриращением - 1].Данные[ИмяП00002М113503]);
				КонецЕсли;
			КонецЦикла;
	...
	ИначеЕсли ИмяТекРаздела = "Раздел21" ИЛИ ИмяТекРаздела = "Раздел3" Тогда
...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	Для Каждого Область Из ИсходныйМакет.Области Цикл
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник
			...
			Если ((ПараметрыПечати.ИмяСтраницы = "Разделы1_2"
				И (Область.Имя = "П000020013003" ИЛИ СтрНачинаетсяС(Область.Имя, "П00002М113503")))
				ИЛИ (ПараметрыПечати.ИмяСтраницы = "Раздел21"
				И (Область.Имя = "П000210001003" ИЛИ Область.Имя = "П000210002003" ИЛИ Область.Имя = "П000210003003"
				ИЛИ Область.Имя = "П000210004003" ИЛИ Область.Имя = "П000210005003" ИЛИ Область.Имя = "П000210006003")))
				И Область.Значение = 0 Тогда
				
				НовСтр.Значение = "0";
				НовСтр.ЗначениеМЧБ = "0-----------.00";
				
			ИначеЕсли ПараметрыПечати.ИмяСтраницы = "Раздел21"
				И (Область.Имя = "П000210008003" ИЛИ Область.Имя = "П000210009003") И Область.Значение = 0 Тогда
				
				НовСтр.Значение = "0";
				НовСтр.ЗначениеМЧБ = "0-----------.000000";
				
			ИначеЕсли ПараметрыПечати.ИмяСтраницы = "Раздел3" И Область.Имя = "П000030029003" И Область.Значение = 0
				И СокрЛП(ТабличныйДокумент.Области.Найти("П000030022003").Значение) = "3" Тогда
	...
	Для Каждого Показатель Из Показатели Цикл
		Если Показатель.Имя = "П000010000103" Тогда
						...
						Если ТипЗнч(РезультатПоиска.Значение)=Тип("Число") Тогда
							...
							Иначе
								...
								ИначеЕсли ТекАдресЗначенияОбласти.ИндексЗнакоместа > ИндексТочкиЧислаНаМЧБ Тогда
									...
									ПрисвоитьЗначениеОбластиДробнойЧастиПриРазделенииНаМЧБНаЦелуюИДробнуюЧасть(Область,
...
&НаСервере
Процедура ПрисвоитьЗначениеОбластиДробнойЧастиПриРазделенииНаМЧБНаЦелуюИДробнуюЧасть(
	Область, Знач ДробнаяЧасть, ИндексЗнакоместа = 0)
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	ДанныеКС.Вставить("Документирование"		, Ложь);
	ДанныеКС.Вставить("ТаблицаРезультата"		, Отчеты.ПроверкаКонтрольныхСоотношений.СформироватьТаблицуРезультата());
	ДанныеКС.Вставить("ТаблицаРасшифровки"		, Отчеты.ПроверкаКонтрольныхСоотношений.СформироватьТаблицуРасшифровки());
...
#КонецОбласти
	...
	РегламентированнаяОтчетность.ПоместитьВКэш(ДанныеКС.ТаблицаРезультата, УникальныйИдентификатор,
		ДанныеКС.АдресРезультата);
	РегламентированнаяОтчетность.ПоместитьВКэш(ДанныеКС.ТаблицаРасшифровки, УникальныйИдентификатор,
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Декларация по налогу на сверхприбыль");
	...
	ДанныеКС 			= Новый Структура;
	ДанныеКС.Вставить("Документирование", Истина);
	ДанныеКС.Вставить("Счетчик"			, 0);
	ДанныеКС.Вставить("ТаблицаОписаний"	, Отчеты.ПроверкаКонтрольныхСоотношений.СформироватьТаблицуОписаний());
...
#КонецОбласти
	...
	АдресТаблицыОписаний = ПоместитьВоВременноеХранилище(ДанныеКС.ТаблицаОписаний, Новый УникальныйИдентификатор);
...
Процедура СформироватьСлужебныеДанные()
	...
	ИменаРазделов.Вставить("Р01", "Разделы1_2");
	ИменаРазделов.Вставить("Р02", "Разделы1_2");
	...
	ПоляТитульногоЛиста.Вставить("ДатаПодписи"			, "Дата подписи");
	ПоляТитульногоЛиста.Вставить("НомерКорректировки"	, "Номер корректировки");
	...
	МассивПолейСписочныхЗначений.Добавить("П000030022003");
	...
	Для каждого СтрокаТЗ Из ТаблицаСписков Цикл
		Если НЕ МассивПолейСписочныхЗначений.Найти(СтрокаТЗ.Показатель) = Неопределено Тогда
			...
			Для каждого СтрокаЗначения Из СтрокаТЗ.ТаблицаЗначений Цикл
				Если ЗначениеЗаполнено(СтрокаЗначения.Код) Тогда
					МассивЗначений.Добавить(СтрокаЗначения.Код);
				КонецЕсли;
			КонецЦикла;
			СоответствиеСписков.Вставить(СтрокаТЗ.Показатель, МассивЗначений);
		КонецЕсли;
	КонецЦикла;
...
Процедура СформироватьДанныеОтчета()
	...
	ДанныеКС.Вставить("Разделы1_2", ДанныеКС.Разделы1_2[0]);
	ДанныеКС.Вставить("Раздел21", ДанныеКС.Раздел21[0]);
...
Процедура РасшифроватьПоказатель(Показатель)
	...
	НоваяСтрока							= ДанныеКС.ТаблицаРасшифровки.Добавить();
...
Процедура ДополнитьТаблицуРезультата()
	...
	НоваяСтрока = ДанныеКС.ТаблицаРезультата.Добавить();
...
Процедура УдалитьСтрокиТаблицыРасшифровки(НомерЭлемента)
	...
	УдаляемыеСтроки = ДанныеКС.ТаблицаРасшифровки.НайтиСтроки(ПараметрыОтбора);
	
	Для каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
		ДанныеКС.ТаблицаРасшифровки.Удалить(УдаляемаяСтрока);
	КонецЦикла; 
...
Процедура ПодготовитьНастройки(Заголовки)
	...
	Если ДанныеКС.Документирование Тогда
		ДанныеКС.Счетчик = ДанныеКС.Счетчик + 1;
		СтрокаОписания = ДанныеКС.ТаблицаОписаний.Добавить();
...
#Область Проверки_Раздела_0

&НаСервере
Процедура ПроверкаКС_01()
	...
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Дата представления отчета <= 25.01.24, если номер корректировки = 0';
			|en = 'Дата представления отчета <= 25.01.24, если номер корректировки = 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 <= 25.01.24, если %2 = 0';
													|en = '%1 <= 25.01.24, если %2 = 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно несвоевременное представление отчета';
													|en = 'Возможно несвоевременное представление отчета'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните дату подписи отчета на Титульном листе");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	...
	ИменаПоказателей.Вставить("Р00С002"		, "НомерКорректировки");
	...
	УсловиеВыполнено = ДанныеКС.ГодОтчета = 2024 И ДанныеКС.Раздел0.НомерКорректировки = 0;
	
	НастройкиКС.ЭтоОшибка = УсловиеВыполнено И Показатели.Р00С001.Значение >= '20240126';
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р00С001.ЗначениеСтр, Показатели.Р00С002.Значение);
...
Процедура ПроверкаКС_02()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '2';
													|en = '2'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма налога в стр.010 Раздела 1 = стр.(120 - 125) Раздела 2';
													|en = 'Сумма налога в стр.010 Раздела 1 = стр.(120 - 125) Раздела 2'"));
	...
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налога в стр.010 Раздела 1");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01С001"		, "П000010001003");
	ИменаПоказателей.Вставить("Р02С002"		, "П000020012003");
	ИменаПоказателей.Вставить("Р02С003"		, "П000020012503");
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	ЗарегистрироватьПроверку();
		
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Разделы1_2);
	РасчетнаяСумма = Макс(0, Показатели.Р02С002.Значение - Показатели.Р02С003.Значение);
	
	НастройкиКС.ЭтоОшибка = НЕ Показатели.Р01С001.Значение = РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р01С001.Значение, РасчетнаяСумма, Показатели.Р02С002.Значение, Показатели.Р02С003.Значение);
	ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_03_1()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '3.1';
													|en = '3.1'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.010 Раздела 2 >= ∑ стр.300 Раздела 3';
													|en = 'Сумма прибыли в стр.010 Раздела 2 >= ∑ стр.300 Раздела 3'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= %2';
													|en = '%1 >= %2'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму прибыли в стр.010 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("ПоказательР2"		, "П000020001003");
	НастройкиКС.Вставить("ПоказательР3"		, "П000030030003");
...
&НаСервере
Процедура ПроверкаКС_03_код()
	
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р02С001"		, НастройкиКС.ПоказательР2);
	ИменаПоказателей.Вставить("Р03С002"		, НастройкиКС.ПоказательР3);
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	ЗарегистрироватьПроверку(); 
	Раздел3Заполнен = Ложь;
		
	ЗаполнитьИРасшифроватьПоказатель(Показатели.Р02С001, ДанныеКС.Разделы1_2);
	
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
	
		ЗаполнитьИРасшифроватьПоказатель(Показатели.Р03С002, СтраницаРаздела);
		Раздел3Заполнен = Раздел3Заполнен ИЛИ СтраницаРаздела.РазделЗаполнен;
	
	КонецЦикла;
	
	НастройкиКС.ЭтоОшибка = Раздел3Заполнен И Показатели.Р02С001.Значение < Показатели.Р03С002.Итог;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р02С001.Значение, Показатели.Р03С002.Итог);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_03_2()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '3.2';
													|en = '3.2'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.020 Раздела 2 >= ∑ стр.310 Раздела 3';
													|en = 'Сумма прибыли в стр.020 Раздела 2 >= ∑ стр.310 Раздела 3'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = %2';
													|en = '%1 = %2'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму прибыли в стр.020 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("ПоказательР2"		, "П000020002003");
	НастройкиКС.Вставить("ПоказательР3"		, "П000030031003");
...
Процедура ПроверкаКС_03_3()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '3.3';
													|en = '3.3'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.030 Раздела 2 >= ∑ стр.320 Раздела 3';
													|en = 'Сумма прибыли в стр.030 Раздела 2 >= ∑ стр.320 Раздела 3'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = %2';
													|en = '%1 = %2'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму прибыли в стр.030 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("ПоказательР2"		, "П000020003003");
	НастройкиКС.Вставить("ПоказательР3"		, "П000030032003");
...
Процедура ПроверкаКС_03_4()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '3.4';
													|en = '3.4'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.040 Раздела 2 >= ∑ стр.330 Раздела 3';
													|en = 'Сумма прибыли в стр.040 Раздела 2 >= ∑ стр.330 Раздела 3'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = %2';
													|en = '%1 = %2'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму прибыли в стр.040 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("ПоказательР2"		, "П000020004003");
	НастройкиКС.Вставить("ПоказательР3"		, "П000030033003");
...
Процедура ПроверкаКС_04()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '4';
													|en = '4'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Средняя сумма прибыли в стр.050 Раздела 2 = стр.(010 + 020)/2 Раздела 2';
			|en = 'Средняя сумма прибыли в стр.050 Раздела 2 = стр.(010 + 020)/2 Раздела 2'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = %2 ((%3 + %4)/2)';
													|en = '%1 = %2 ((%3 + %4)/2)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните среднюю сумму прибыли в стр.050 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "П000020005003");
	НастройкиКС.Вставить("Показатель2"		, "П000020001003");
	НастройкиКС.Вставить("Показатель3"		, "П000020002003");
...
&НаСервере
Процедура ПроверкаКС_04_код()
	
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р02С001"		, НастройкиКС.Показатель1);
	ИменаПоказателей.Вставить("Р02С002"		, НастройкиКС.Показатель2);
	ИменаПоказателей.Вставить("Р02С003"		, НастройкиКС.Показатель3);
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	ЗарегистрироватьПроверку(); 
		
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Разделы1_2);
	РасчетнаяСумма = Макс(0, Окр((Показатели.Р02С002.Значение + Показатели.Р02С003.Значение) / 2)); 
...
Процедура ПроверкаКС_05()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '5';
													|en = '5'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Средняя сумма прибыли в стр.060 Раздела 2 = стр.(030 + 040)/2 Раздела 2';
			|en = 'Средняя сумма прибыли в стр.060 Раздела 2 = стр.(030 + 040)/2 Раздела 2'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = %2 ((%3 + %4)/2)';
													|en = '%1 = %2 ((%3 + %4)/2)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните среднюю сумму прибыли в стр.060 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "П000020006003");
	НастройкиКС.Вставить("Показатель2"		, "П000020003003");
	НастройкиКС.Вставить("Показатель3"		, "П000020004003");
...
Процедура ПроверкаКС_06_1()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '6.1';
													|en = '6.1'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Налоговая база в стр.100 Раздела 2 = стр.(060 - 050) Раздела 2, если стр.070 Раздела 2 = 0';
			|en = 'Налоговая база в стр.100 Раздела 2 = стр.(060 - 050) Раздела 2, если стр.070 Раздела 2 = 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = %2 (%3 - %4), если %5 = 0';
													|en = '%1 = %2 (%3 - %4), если %5 = 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина налоговой базы';
													|en = 'Возможно ошибочная величина налоговой базы'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните налоговую базу в стр.100 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р02С001"		, "П000020010003");
	ИменаПоказателей.Вставить("Р02С002"		, "П000020006003");
	ИменаПоказателей.Вставить("Р02С003"		, "П000020005003");
	ИменаПоказателей.Вставить("Р02С004"		, "П000020007003");
	...
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Разделы1_2);
	РасчетнаяСумма = Макс(0, Показатели.Р02С002.Значение - Показатели.Р02С003.Значение);
	УсловиеВыполнено = Показатели.Р02С004.Значение = "0";
	
	НастройкиКС.ЭтоОшибка = УсловиеВыполнено И НЕ Показатели.Р02С001.Значение = РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р02С001.Значение, РасчетнаяСумма, Показатели.Р02С002.Значение, 
		Показатели.Р02С003.Значение, Показатели.Р02С004.ЗначениеСтр);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_06_2()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '6.2';
													|en = '6.2'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Налоговая база в стр.100 Раздела 2 = (стр.040 Раздела 2)/2, если стр.070 Раздела 2 = 1';
			|en = 'Налоговая база в стр.100 Раздела 2 = (стр.040 Раздела 2)/2, если стр.070 Раздела 2 = 1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = %2 (%3/2), если %4 = 1';
													|en = '%1 = %2 (%3/2), если %4 = 1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина налоговой базы';
													|en = 'Возможно ошибочная величина налоговой базы'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните налоговую базу в стр.100 Раздела 2");
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р02С001"		, "П000020010003");
	ИменаПоказателей.Вставить("Р02С002"		, "П000020004003");
	ИменаПоказателей.Вставить("Р02С004"		, "П000020007003");
	...
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Разделы1_2);
	РасчетнаяСумма = Макс(0, Окр(Показатели.Р02С002.Значение / 2));
	УсловиеВыполнено = Показатели.Р02С004.Значение = "1";
	
	НастройкиКС.ЭтоОшибка = УсловиеВыполнено И НЕ Показатели.Р02С001.Значение = РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р02С001.Значение, РасчетнаяСумма, Показатели.Р02С002.Значение, Показатели.Р02С004.ЗначениеСтр);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_07()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '7';
													|en = '7'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Стр.(060 - 050) Раздела 2 > (стр.040 Раздела 2)/2, стр.080 Раздела 2.1 < стр.090 ';
			|en = 'Стр.(060 - 050) Раздела 2 > (стр.040 Раздела 2)/2, стр.080 Раздела 2.1 < стр.090 '") + 
		НСтр("ru = 'Раздела 2.1, стр.070 Раздела 2.1 = 0, если стр.070 Раздела 2 = 1';
			|en = 'Раздела 2.1, стр.070 Раздела 2.1 = 0, если стр.070 Раздела 2 = 1'")); 
	Заголовки.Вставить("ШаблонДетализации"	, 
		НСтр("ru = '%1 (%2 - %3) > %4 (%5/2), %6 < %7, %8 = 0, если %9 = 1';
			|en = '%1 (%2 - %3) > %4 (%5/2), %6 < %7, %8 = 0, если %9 = 1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочные величины показателей';
													|en = 'Возможно ошибочные величины показателей'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните показатели в Разделах 2, 2.1");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателейР2.Вставить("Р02С001"		, "П000020006003");
	ИменаПоказателейР2.Вставить("Р02С002"		, "П000020005003");
	ИменаПоказателейР2.Вставить("Р02С003"		, "П000020004003");
	ИменаПоказателейР2.Вставить("Р02С004"		, "П000020007003");
	
	ИменаПоказателей21 = Новый Структура;
	ИменаПоказателей21.Вставить("Р21С005"		, "П000210008003");
	ИменаПоказателей21.Вставить("Р21С006"		, "П000210009003");
	ИменаПоказателей21.Вставить("Р21С007"		, "П000210007003");
	
	ПоказателиР2 = СформироватьПоказатели(ИменаПоказателейР2);
	Показатели21 = СформироватьПоказатели(ИменаПоказателей21);
	ЗарегистрироватьПроверку(); 
		
	ЗаполнитьИРасшифроватьПоказатели(ПоказателиР2, ДанныеКС.Разделы1_2);
	ЗаполнитьИРасшифроватьПоказатели(Показатели21, ДанныеКС.Раздел21);
	
	РасчетнаяСумма1 = Макс(0, ПоказателиР2.Р02С001.Значение - ПоказателиР2.Р02С002.Значение);
	РасчетнаяСумма2 = Макс(0, Окр(ПоказателиР2.Р02С003.Значение / 2));
	
	Условие1Выполнено = ПоказателиР2.Р02С004.Значение = "1";
	Условие2Выполнено = РасчетнаяСумма1 > РасчетнаяСумма2;
	Условие3Выполнено = Показатели21.Р21С005.Значение < Показатели21.Р21С006.Значение;
	Условие4Выполнено = Показатели21.Р21С007.Значение = "0";
	
	НастройкиКС.ЭтоОшибка = Условие1Выполнено И НЕ (Условие2Выполнено И Условие3Выполнено И Условие4Выполнено);
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		РасчетнаяСумма1, ПоказателиР2.Р02С001.Значение, ПоказателиР2.Р02С002.Значение, РасчетнаяСумма2, 
		ПоказателиР2.Р02С003.Значение, Показатели21.Р21С005.Значение, Показатели21.Р21С006.Значение, 
		Показатели21.Р21С007.ЗначениеСтр, ПоказателиР2.Р02С004.ЗначениеСтр);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_08()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '8';
													|en = '8'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма налога в стр.120 Раздела 2 = стр.100 Раздела 2 х 10%';
													|en = 'Сумма налога в стр.120 Раздела 2 = стр.100 Раздела 2 х 10%'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = %2 (%3 х 10 / 100)';
													|en = '%1 = %2 (%3 х 10 / 100)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина суммы налога';
													|en = 'Возможно ошибочная величина суммы налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налога в стр.120 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р02С001"		, "П000020012003");
	ИменаПоказателей.Вставить("Р02С002"		, "П000020010003");
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	ЗарегистрироватьПроверку(); 
		
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Разделы1_2);
	РасчетнаяСумма = Макс(0, Окр(Показатели.Р02С002.Значение * 10 / 100));
...
Процедура ПроверкаКС_09()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '9';
													|en = '9'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Сумма налогового вычета в стр.125 Раздела 2 <= стр.130 Раздела 2, но не более (стр.120 Раздела 2)/2';
			|en = 'Сумма налогового вычета в стр.125 Раздела 2 <= стр.130 Раздела 2, но не более (стр.120 Раздела 2)/2'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 <= %2, %1 <= %3 (%4/2)';
													|en = '%1 <= %2, %1 <= %3 (%4/2)'"));
	...
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налогового вычета в стр.125 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р02С001"		, "П000020012503");
	ИменаПоказателей.Вставить("Р02С002"		, "П000020013003");
	ИменаПоказателей.Вставить("Р02С003"		, "П000020012003");
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	ЗарегистрироватьПроверку(); 
		
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Разделы1_2);
	РасчетнаяСумма1 = Макс(0, Окр(Показатели.Р02С002.Значение));
	РасчетнаяСумма2 = Макс(0, Окр(Показатели.Р02С003.Значение / 2));
	
	НастройкиКС.ЭтоОшибка = Показатели.Р02С001.Значение > РасчетнаяСумма1 
		ИЛИ Показатели.Р02С001.Значение > РасчетнаяСумма2;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р02С001.Значение, РасчетнаяСумма1, РасчетнаяСумма2, Показатели.Р02С003.Значение);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_10()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '10';
													|en = '10'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Сумма обеспечительного платежа в стр.130 Раздела 2 <= ∑ стр.135 Раздела 2';
			|en = 'Сумма обеспечительного платежа в стр.130 Раздела 2 <= ∑ стр.135 Раздела 2'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 <= %2';
													|en = '%1 <= %2'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная сумма обеспечительного платежа';
													|en = 'Возможно ошибочная сумма обеспечительного платежа'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму обеспечительного платежа в стр.130 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р02С001"		, "П000020013003");
	ИменаПоказателей.Вставить("Р02С002"		, "П00002М113503_");
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	ЗарегистрироватьПроверку(); 
	
	Показатели.Р02С001.Округление = Ложь;
	Показатели.Р02С002.Округление = Ложь;
		
	ЗаполнитьИРасшифроватьПоказатель(Показатели.Р02С001, ДанныеКС.Разделы1_2);
	
	Для Счетчик = 1 По ДанныеКС.Разделы1_2.КолвоСтрокП00002М1 Цикл
		ЗаполнитьИРасшифроватьПоказатель(Показатели.Р02С002, ДанныеКС.Разделы1_2, Счетчик);
	КонецЦикла;
	
	РасчетнаяСумма = Макс(0, Показатели.Р02С002.Итог);
	
	НастройкиКС.ЭтоОшибка = Показатели.Р02С001.Значение > РасчетнаяСумма; 
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р02С001.Значение, РасчетнаяСумма);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_11_1()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '11.1';
													|en = '11.1'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.010 Раздела 2 >= 0';
													|en = 'Сумма прибыли в стр.010 Раздела 2 >= 0'"));
	...
	НастройкиКС.Вставить("Показатель1"		, "П000020001003");
...
Процедура ПроверкаКС_11_код()
	...
	НастройкиКС.ЭтоОшибка = Показатели.Р02С001.Значение < 0;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р02С001.Значение);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_11_2()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '11.2';
													|en = '11.2'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.020 Раздела 2 >= 0';
													|en = 'Сумма прибыли в стр.020 Раздела 2 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	...
	НастройкиКС.Вставить("Показатель1"		, "П000020002003");
...
&НаСервере
Процедура ПроверкаКС_11_3()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2';
														|en = 'Раздел 2'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '11.3';
													|en = '11.3'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.030 Раздела 2 >= 0';
													|en = 'Сумма прибыли в стр.030 Раздела 2 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	...
	НастройкиКС.Вставить("Показатель1"		, "П000020003003");
...
&НаСервере
Процедура ПроверкаКС_11_4()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2';
														|en = 'Раздел 2'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '11.4';
													|en = '11.4'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.040 Раздела 2 >= 0';
													|en = 'Сумма прибыли в стр.040 Раздела 2 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	...
	НастройкиКС.Вставить("Показатель1"		, "П000020004003");
...
&НаСервере
Процедура ПроверкаКС_11_5()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2';
														|en = 'Раздел 2'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '11.5';
													|en = '11.5'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.050 Раздела 2 >= 0';
													|en = 'Сумма прибыли в стр.050 Раздела 2 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму прибыли в стр.050 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
...
&НаСервере
Процедура ПроверкаКС_11_6()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2';
														|en = 'Раздел 2'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '11.6';
													|en = '11.6'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.060 Раздела 2 >= 0';
													|en = 'Сумма прибыли в стр.060 Раздела 2 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму прибыли в стр.060 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
...
&НаСервере
Процедура ПроверкаКС_11_7()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2';
														|en = 'Раздел 2'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '11.7';
													|en = '11.7'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма налоговой базы в стр.100 Раздела 2 >= 0';
													|en = 'Сумма налоговой базы в стр.100 Раздела 2 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налоговой базы в стр.100 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "П000020010003");
...
&НаСервере
Процедура ПроверкаКС_11_8()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2';
														|en = 'Раздел 2'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '11.8';
													|en = '11.8'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма налога в стр.120 Раздела 2 >= 0';
													|en = 'Сумма налога в стр.120 Раздела 2 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
...
&НаСервере
Процедура ПроверкаКС_11_9()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2';
														|en = 'Раздел 2'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '11.9';
													|en = '11.9'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма налогового вычета в стр.125 Раздела 2 >= 0';
													|en = 'Сумма налогового вычета в стр.125 Раздела 2 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	...
	НастройкиКС.Вставить("Показатель1"		, "П000020012503");
...
&НаСервере
Процедура ПроверкаКС_11_10()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2';
														|en = 'Раздел 2'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '11.10';
													|en = '11.10'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма обеспечительного платежа в стр.130 Раздела 2 >= 0';
													|en = 'Сумма обеспечительного платежа в стр.130 Раздела 2 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
...
&НаСервере
Процедура ПроверкаКС_11_11()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2';
														|en = 'Раздел 2'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '11.11';
													|en = '11.11'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма обеспечительного платежа в стр.135 Раздела 2 >= 0';
													|en = 'Сумма обеспечительного платежа в стр.135 Раздела 2 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму обеспечительного платежа в стр.135 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Строка %1';
													|en = 'Строка %1'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура; 
	ИменаПоказателей.Вставить("Р02С001"		, "П00002М113503_");
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	Показатели.Р02С001.Округление = Ложь;
	
	Для Счетчик = 1 По ДанныеКС.Разделы1_2.КолвоСтрокП00002М1 Цикл
		ЗарегистрироватьПроверку(); 
		ЗаполнитьИРасшифроватьПоказатель(Показатели.Р02С001, ДанныеКС.Разделы1_2, Счетчик);
...
Процедура ПроверкаКС_12()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '12';
													|en = '12'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Дата документа в стр.135 Раздела 2 в интервале с 01.10.23 по 30.11.23';
			|en = 'Дата документа в стр.135 Раздела 2 в интервале с 01.10.23 по 30.11.23'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '01.10.23 <= %1 <= 30.11.23';
													|en = '01.10.23 <= %1 <= 30.11.23'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная дата документа';
													|en = 'Возможно ошибочная дата документа'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните дату документа в стр.135 Раздела 2");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Строка %1';
													|en = 'Строка %1'"));
	...
	ИменаПоказателей = Новый Структура; 
	ИменаПоказателей.Вставить("Р02С001"		, "П00002М113501_");
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	...
	Для Счетчик = 1 По ДанныеКС.Разделы1_2.КолвоСтрокП00002М1 Цикл
		...
		НастройкиКС.ЭтоОшибка = ДанныеКС.ГодОтчета = 2024 И ЗначениеЗаполнено(Показатели.Р02С001.Значение) 
			И (Показатели.Р02С001.Значение < '20231001' ИЛИ Показатели.Р02С001.Значение >= '20231201'); 
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
...
#Область Проверки_Раздела_21

&НаСервере
Процедура ПроверкаКС_13()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '13';
													|en = '13'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Средняя стоимость активов в стр.050 Раздела 2.1 = стр.(010 + 020)/2 Раздела 2.1';
			|en = 'Средняя стоимость активов в стр.050 Раздела 2.1 = стр.(010 + 020)/2 Раздела 2.1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = %2 ((%3 + %4)/2)';
													|en = '%1 = %2 ((%3 + %4)/2)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните среднюю стоимость активов в стр.050 Раздела 2.1");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "П000210005003");
	НастройкиКС.Вставить("Показатель2"		, "П000210001003");
	НастройкиКС.Вставить("Показатель3"		, "П000210002003");
...
&НаСервере
Процедура ПроверкаКС_13_код()
	
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р21С001"		, НастройкиКС.Показатель1);
	ИменаПоказателей.Вставить("Р21С002"		, НастройкиКС.Показатель2);
	ИменаПоказателей.Вставить("Р21С003"		, НастройкиКС.Показатель3);
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	ЗарегистрироватьПроверку(); 
		
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Раздел21);
	РасчетнаяСумма = Макс(0, Окр((Показатели.Р21С002.Значение + Показатели.Р21С003.Значение) / 2, 2)); 
	
	НастройкиКС.ЭтоОшибка = НЕ Показатели.Р21С001.Значение = РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р21С001.Значение, РасчетнаяСумма, Показатели.Р21С002.Значение, Показатели.Р21С003.Значение);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_14()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '14';
													|en = '14'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Средняя стоимость активов в стр.060 Раздела 2.1 = стр.(030 + 040)/2 Раздела 2.1';
			|en = 'Средняя стоимость активов в стр.060 Раздела 2.1 = стр.(030 + 040)/2 Раздела 2.1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = %2 ((%3 + %4)/2)';
													|en = '%1 = %2 ((%3 + %4)/2)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните среднюю стоимость активов в стр.060 Раздела 2.1");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "П000210006003");
	НастройкиКС.Вставить("Показатель2"		, "П000210003003");
	НастройкиКС.Вставить("Показатель3"		, "П000210004003");
...
Процедура ПроверкаКС_15()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '15';
													|en = '15'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Признак распределения прибыли в стр.070 Раздела 2.1 = 0';
			|en = 'Признак распределения прибыли в стр.070 Раздела 2.1 = 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = 0';
													|en = '%1 = 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Укажите 0 в стр.070 Раздела 2.1");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р21С001"		, "П000210007003");
	...
	НастройкиКС.ЭтоОшибка = ДанныеКС.Раздел21.РазделЗаполнен И НЕ Показатели.Р21С001.Значение = "0";
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р21С001.ЗначениеСтр);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_16_1()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '16.1';
													|en = '16.1'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Отношение средних величин в стр.080 Раздела 2.1 = стр.(060/050) Раздела 2, если стр.050 Раздела 2 ≠ 0';
			|en = 'Отношение средних величин в стр.080 Раздела 2.1 = стр.(060/050) Раздела 2, если стр.050 Раздела 2 ≠ 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = %2 (%3/%4), если %4 ≠ 0';
													|en = '%1 = %2 (%3/%4), если %4 ≠ 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните отношение средних величин в стр.080 Раздела 2.1");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р21С001"		, "П000210008003");
	ИменаПоказателей.Вставить("Р02С002"		, "П000020006003");
	ИменаПоказателей.Вставить("Р02С003"		, "П000020005003");
	...
	ЗаполнитьИРасшифроватьПоказатель(Показатели.Р21С001, ДанныеКС.Раздел21);
	ЗаполнитьИРасшифроватьПоказатель(Показатели.Р02С002, ДанныеКС.Разделы1_2);
	ЗаполнитьИРасшифроватьПоказатель(Показатели.Р02С003, ДанныеКС.Разделы1_2);
	
	УсловиеВыполнено = НЕ Показатели.Р02С003.Значение = 0;
	Знаменатель = ?(УсловиеВыполнено, Показатели.Р02С003.Значение, 1); 
	РасчетнаяСумма = Окр(Показатели.Р02С002.Значение / Знаменатель, 6);
	
	НастройкиКС.ЭтоОшибка = УсловиеВыполнено И НЕ Показатели.Р21С001.Значение = РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р21С001.Значение, РасчетнаяСумма, Показатели.Р02С002.Значение, Показатели.Р02С003.Значение);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_16_2()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '16.2';
													|en = '16.2'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Отношение средних величин в стр.080 Раздела 2.1 = стр.060 Раздела 2, если стр.050 Раздела 2 = 0';
			|en = 'Отношение средних величин в стр.080 Раздела 2.1 = стр.060 Раздела 2, если стр.050 Раздела 2 = 0'"));
	...
	УсловиеВыполнено = Показатели.Р02С003.Значение = 0;
	
	НастройкиКС.ЭтоОшибка = УсловиеВыполнено И НЕ Показатели.Р21С001.Значение = Показатели.Р02С002.Значение;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р21С001.Значение, Показатели.Р02С002.Значение, Показатели.Р02С003.Значение);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_17()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '17';
													|en = '17'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Отношение средних величин в стр.090 Раздела 2.1 = стр.(060/050) Раздела 2.1';
			|en = 'Отношение средних величин в стр.090 Раздела 2.1 = стр.(060/050) Раздела 2.1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = %2 (%3/%4)';
													|en = '%1 = %2 (%3/%4)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните отношение средних величин в стр.090 Раздела 2.1");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р21С001"		, "П000210009003");
	ИменаПоказателей.Вставить("Р21С002"		, "П000210006003");
	ИменаПоказателей.Вставить("Р21С003"		, "П000210005003");
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	ЗарегистрироватьПроверку(); 
		
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Раздел21);
	РасчетнаяСумма =?(Показатели.Р21С003.Значение = 0, 0, Окр(Показатели.Р21С002.Значение / Показатели.Р21С003.Значение, 6));
...
Процедура ПроверкаКС_18()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '18';
													|en = '18'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Отношение средних величин в стр.090 Раздела 2.1 > стр.080 Раздела 2.1';
			|en = 'Отношение средних величин в стр.090 Раздела 2.1 > стр.080 Раздела 2.1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 > %2';
													|en = '%1 > %2'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р21С001"		, "П000210009003");
	ИменаПоказателей.Вставить("Р21С002"		, "П000210008003");
	...
	НастройкиКС.ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р21С001.Значение) 
		И НЕ Показатели.Р21С001.Значение > Показатели.Р21С002.Значение;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р21С001.Значение, Показатели.Р21С002.Значение);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_19_1()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '19.1';
													|en = '19.1'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Стоимость активов в стр.010 Раздела 2.1 >= 0';
													|en = 'Стоимость активов в стр.010 Раздела 2.1 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните стоимость активов в стр.010 Раздела 2.1");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "П000210001003");
...
Процедура ПроверкаКС_19_код()
	...
	НастройкиКС.ЭтоОшибка = Показатели.Р21С001.Значение < 0;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
...
Процедура ПроверкаКС_19_2()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '19.2';
													|en = '19.2'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Стоимость активов в стр.020 Раздела 2.1 >= 0';
													|en = 'Стоимость активов в стр.020 Раздела 2.1 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните стоимость активов в стр.020 Раздела 2.1");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "П000210002003");
...
&НаСервере
Процедура ПроверкаКС_19_3()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2.1';
														|en = 'Раздел 2.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '19.3';
													|en = '19.3'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Стоимость активов в стр.030 Раздела 2.1 >= 0';
													|en = 'Стоимость активов в стр.030 Раздела 2.1 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните стоимость активов в стр.030 Раздела 2.1");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "П000210003003");
...
&НаСервере
Процедура ПроверкаКС_19_4()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2.1';
														|en = 'Раздел 2.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '19.4';
													|en = '19.4'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Стоимость активов в стр.040 Раздела 2.1 >= 0';
													|en = 'Стоимость активов в стр.040 Раздела 2.1 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните стоимость активов в стр.040 Раздела 2.1");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "П000210004003");
...
&НаСервере
Процедура ПроверкаКС_19_5()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2.1';
														|en = 'Раздел 2.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '19.5';
													|en = '19.5'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Стоимость активов в стр.050 Раздела 2.1 >= 0';
													|en = 'Стоимость активов в стр.050 Раздела 2.1 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните стоимость активов в стр.050 Раздела 2.1");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
...
&НаСервере
Процедура ПроверкаКС_19_6()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2.1';
														|en = 'Раздел 2.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '19.6';
													|en = '19.6'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Стоимость активов в стр.060 Раздела 2.1 >= 0';
													|en = 'Стоимость активов в стр.060 Раздела 2.1 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните стоимость активов в стр.060 Раздела 2.1");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	ПодготовитьНастройки(Заголовки);
...
&НаСервере
Процедура ПроверкаКС_19_7()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2.1';
														|en = 'Раздел 2.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '19.7';
													|en = '19.7'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Отношение средних величин в стр.080 Раздела 2.1 >= 0';
													|en = 'Отношение средних величин в стр.080 Раздела 2.1 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	...
	НастройкиКС.Вставить("Показатель1"		, "П000210008003");
...
&НаСервере
Процедура ПроверкаКС_19_8()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 2.1';
														|en = 'Раздел 2.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '19.8';
													|en = '19.8'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Отношение средних величин в стр.090 Раздела 2.1 >= 0';
													|en = 'Отношение средних величин в стр.090 Раздела 2.1 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	...
	НастройкиКС.Вставить("Показатель1"		, "П000210009003");
...
&НаСервере
Процедура ПроверкаКС_20()
	
	Если ДанныеКС.Документирование Тогда
		СтрокаЗначений = "1, 2, 3, 5";
	Иначе
		МассивЗначений = ДанныеКС.СоответствиеСписков.Получить("П000030022003");
		СтрокаЗначений = СтрСоединить(МассивЗначений, ", ");
	КонецЕсли;
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '20';
													|en = '20'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Код формы реорганизации в стр.220 Раздела 3 = ';
													|en = 'Код формы реорганизации в стр.220 Раздела 3 = '") + СтрокаЗначений);
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 = ';
													|en = '%1 = '") + СтрокаЗначений);
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните код формы реорганизации в стр.220 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р03С001"		, "П000030022003");
	Показатели = СформироватьПоказатели(ИменаПоказателей);
...
Процедура ПроверкаКС_21()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '21';
													|en = '21'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Дата реорганизации в стр.270 Раздела 3 в интервале с 01.01.18 по 31.12.23';
			|en = 'Дата реорганизации в стр.270 Раздела 3 в интервале с 01.01.18 по 31.12.23'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '01.01.18 <= %1 <= 31.12.23';
													|en = '01.01.18 <= %1 <= 31.12.23'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная дата реорганизации';
													|en = 'Возможно ошибочная дата реорганизации'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните дату реорганизации в стр.270 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р03С001"		, "П000030027003");
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		...
		НастройкиКС.ЭтоОшибка = ДанныеКС.ГодОтчета = 2024 И ЗначениеЗаполнено(Показатели.Р03С001.Значение) 
			И (Показатели.Р03С001.Значение < '20180101' ИЛИ Показатели.Р03С001.Значение >= '20240101'); 
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Р03С001.ЗначениеСтр);
		НастройкиКС.ДопСведения = СтрШаблон(НастройкиКС.ШаблонДопСведений,
			СтраницаРаздела.НомерСтраницы);
...
Процедура ПроверкаКС_22_1()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '22.1';
													|en = '22.1'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.300 Раздела 3 >= 0';
													|en = 'Сумма прибыли в стр.300 Раздела 3 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму прибыли в стр.300 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	...
	НастройкиКС.Вставить("Показатель1"		, "П000030030003");
...
&НаСервере
Процедура ПроверкаКС_22_код()
	
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р03С001"		, НастройкиКС.Показатель1);
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		...
		НастройкиКС.ЭтоОшибка = Показатели.Р03С001.Значение < 0;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Р03С001.Значение);
		НастройкиКС.ДопСведения = СтрШаблон(НастройкиКС.ШаблонДопСведений,
			СтраницаРаздела.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_22_2()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '22.2';
													|en = '22.2'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.310 Раздела 3 >= 0';
													|en = 'Сумма прибыли в стр.310 Раздела 3 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму прибыли в стр.310 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	...
	НастройкиКС.Вставить("Показатель1"		, "П000030031003");
...
&НаСервере
Процедура ПроверкаКС_22_3()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 3';
														|en = 'Раздел 3'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '22.3';
													|en = '22.3'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.320 Раздела 3 >= 0';
													|en = 'Сумма прибыли в стр.320 Раздела 3 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму прибыли в стр.320 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	...
	НастройкиКС.Вставить("Показатель1"		, "П000030032003");
...
&НаСервере
Процедура ПроверкаКС_22_4()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Раздел 3';
														|en = 'Раздел 3'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '22.4';
													|en = '22.4'"));
	Заголовки.Вставить("Описание"			, НСтр("ru = 'Сумма прибыли в стр.330 Раздела 3 >= 0';
													|en = 'Сумма прибыли в стр.330 Раздела 3 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 >= 0';
													|en = '%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму прибыли в стр.330 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	...
	НастройкиКС.Вставить("Показатель1"		, "П000030033003");
...
Процедура ПроверкаКС_23()
	...
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '23';
													|en = '23'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru = 'Доля при разделении в стр.290 Раздела 3 ≠ 0, если стр.220 Раздела 3 = 3';
			|en = 'Доля при разделении в стр.290 Раздела 3 ≠ 0, если стр.220 Раздела 3 = 3'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru = '%1 ≠ 0, если %2 = 3';
													|en = '%1 ≠ 0, если %2 = 3'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Возможно ошибочная величина показателя';
													|en = 'Возможно ошибочная величина показателя'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните значение доли при разделении в стр.290 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@';
													|en = 'Письмо ФНС России от 04.12.2023 № СД-4-3/15200@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р03С001"		, "П000030029003");
	ИменаПоказателей.Вставить("Р03С002"		, "П000030022003");
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		...
		НастройкиКС.ЭтоОшибка = Показатели.Р03С002.Значение = "3"
			И Показатели.Р03С001.Значение = 0;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Р03С001.Значение, Показатели.Р03С002.ЗначениеСтр);
		НастройкиКС.ДопСведения = СтрШаблон(НастройкиКС.ШаблонДопСведений,
			СтраницаРаздела.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	Если Ячейка.Раздел = "Разделы1_2" ИЛИ Ячейка.Раздел = "Раздел21" Тогда
...
Процедура ОбновитьСведенияОбОрганизации()
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Титульный" Тогда
		...
		УстановитьКодПоОКТМОПриАктивномТитульномЛисте();
...
Процедура СформироватьЗаголовокОтчета()
	...
	Заголовок = "Декларация по налогу на сверхприбыль в "
	+ СтруктураРеквизитовФормы.СтрПериодОтчета + ПредстНомКорр + " (" + СтруктураРеквизитовФормы.НаимОрганизации + ")"
	+ ?(СтрНайти(ЭтаФорма.ИмяФормы, "Внешний") > 0, " - внешний отчет", "");
