...
Процедура ПрочитатьПараметры()
	...
	ПараметрНачало = Неопределено;
	Если Параметры.Свойство("Начало", ПараметрНачало) Тогда
		Начало = ПараметрНачало;
	КонецЕсли;
	...
	ИначеЕсли НачалоНедели(ПараметрНачало) = НачалоНедели(Параметры.Окончание) Тогда
...
Процедура РассчитатьПериодПоСмежнымОперациям(ПериодВыборкиНачало, ПериодВыборкиОкончание)
	...
	Если ЗначениеЗаполнено(НайденныеСтроки) Тогда
		...
		Пока ТекущийИндекс > 0 Цикл
			...
			ТекущаяСтрока = Периоды[ТекущийИндекс];//СтрокаТаблицыЗначений из см. СмежныеОперации -
...
// Параметры:
// 	ПериодВыборкиНачало - Дата - 
// 	ПериодВыборкиОкончание - Дата - 
// Возвращаемое значение:
// 	ТаблицаЗначений - Описание - где:
// * Начало - Дата - 
// * Окончание - Дата - 
&НаСервере
