...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 4 По 5 Цикл
		...
		Для Стр = 53 По 60 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101052" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 64 По 67 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101063" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 69 По 72 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101068" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 74 По 77 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101073" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 79 По 82 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101078" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 84 По 87 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101083" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 89 По 96 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00002010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000201088" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 100 По 103 Цикл
			СуммПоСтр = СуммПоСтр + Области["П0000201" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000201099" + Графа].Значение = СуммПоСтр;
		
		Области["П0000201098" + Графа].Значение = Области["П0000201099" + Графа].Значение
			+ Области["П0000201104" + Графа].Значение;
		
		СуммПоСтр = 0;
		Для Стр = 124 По 127 Цикл
			СуммПоСтр = СуммПоСтр + Области["П0000501" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000501123" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 129 По 136 Цикл
			СуммПоСтр = СуммПоСтр + Области["П0000501" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000501128" + Графа].Значение = СуммПоСтр;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 4 По 5 Цикл
		...
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101052" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101063" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101068" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101073" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101078" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101083" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000201088" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000201098" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000201099" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000501123" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000501128" + Графа + ";";
	КонецЦикла;
