...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 4 По 5 Цикл
		...
		//строка 01 = ∑стр. 02 – 09 по всем графам
		СуммПоСтр = 0;
		Для Стр = 2 По 9 Цикл
			СуммПоСтр = СуммПоСтр + Области["П001001000" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0010010001" + Графа].Значение = СуммПоСтр;
		
		//строка 10 = ∑стр. 11 – 18 по всем графам
		СуммПоСтр = 0;
		Для Стр = 11 По 18 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00100100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0010010010" + Графа].Значение = СуммПоСтр;
		
		//строка 19 = ∑стр. 20 – 27 по всем графам
		СуммПоСтр = 0;
		Для Стр = 20 По 27 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00100100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0010010019" + Графа].Значение = СуммПоСтр;
		
		//строка 36 = ∑стр. 37 – 44 по всем графам
		СуммПоСтр = 0;
		Для Стр = 37 По 44 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00100100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0010010036" + Графа].Значение = СуммПоСтр;
		
		//строка 47 = ∑стр. 48 – 51 по всем графам
		СуммПоСтр = 0;
		Для Стр = 48 По 51 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00100100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0010010047" + Графа].Значение = СуммПоСтр;
		
		//строка 52 =∑стр. 53 – 56 по всем графам
		СуммПоСтр = 0;
		Для Стр = 53 По 56 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00100100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0010010052" + Графа].Значение = СуммПоСтр;
		
		//строка 57 = ∑стр. 58 – 61 по всем графам
		СуммПоСтр = 0;
		Для Стр = 58 По 61 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00100100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0010010057" + Графа].Значение = СуммПоСтр;
		
		//строка 62 = ∑стр. 63 – 66 по всем графам
		СуммПоСтр = 0;
		Для Стр = 63 По 66 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00100100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0010010062" + Графа].Значение = СуммПоСтр;
		
		//строка 67 = ∑стр. 68 – 71 по всем графам
		СуммПоСтр = 0;
		Для Стр = 68 По 71 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00100100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0010010067" + Графа].Значение = СуммПоСтр;
	...
	Для Гр = 3 По 4 Цикл
		...
		//Раздел 2
		//строка 72 = ∑стр. 73 – 80 по всем графам
		СуммПоСтр = 0;
		Для Стр = 73 По 80 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00200100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0020010072" + Графа].Значение = СуммПоСтр;
		
		//строка 83 = ∑стр. 84 – 87 по всем графам
		СуммПоСтр = 0;
		Для Стр = 84 По 87 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00200100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0020010083" + Графа].Значение = СуммПоСтр;
		
		//строка 82 = строка 83 + строка 88 по всем графам
		Области["П0020010082" + Графа].Значение
			= Области["П0020010083" + Графа].Значение
			+ Области["П0020010088" + Графа].Значение;
		
		//Раздел 5
		//строка 107 = ∑стр. 108 – 111 по всем графам
		СуммПоСтр = 0;
		Для Стр = 108 По 111 Цикл
			СуммПоСтр = СуммПоСтр + Области["П0050010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0050010107" + Графа].Значение = СуммПоСтр;
		
		//строка 112 = ∑стр. 113 – 120 по всем графам
		СуммПоСтр = 0;
		Для Стр = 113 По 120 Цикл
			СуммПоСтр = СуммПоСтр + Области["П0050010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0050010112" + Графа].Значение = СуммПоСтр;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 4 По 5 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0010010001" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0010010010" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0010010019" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0010010036" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0010010047" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0010010052" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0010010057" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0010010062" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0010010067" + Графа + ";";
	КонецЦикла;
	
	Для Гр = 3 По 4 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0020010072" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0020010082" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0020010083" + Графа + ";";
		
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0050010107" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0050010112" + Графа + ";";
	КонецЦикла;
