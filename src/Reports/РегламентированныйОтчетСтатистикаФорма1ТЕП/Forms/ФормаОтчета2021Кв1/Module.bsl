...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 4 По 5 Цикл
		...
		СуммПоСтр = 0;
		Для Стр = 2 По 9 Цикл
			СуммПоСтр = СуммПоСтр + Области["П000010100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101001" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 11 По 18 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101010" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 20 По 27 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101019" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 29 По 36 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101028" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 38 По 45 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101037" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 54 По 61 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101053" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 65 По 68 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101064" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 70 По 73 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101069" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 75 По 78 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101074" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 80 По 83 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101079" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 85 По 88 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00001010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000101084" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 90 По 97 Цикл
			СуммПоСтр = СуммПоСтр + Области["П00002010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000201089" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 101 По 104 Цикл
			СуммПоСтр = СуммПоСтр + Области["П0000201" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000201100" + Графа].Значение = СуммПоСтр;
		
		Области["П0000201099" + Графа].Значение = Области["П0000201100" + Графа].Значение
			+ Области["П0000201105" + Графа].Значение;
		
		СуммПоСтр = 0;
		Для Стр = 125 По 128 Цикл
			СуммПоСтр = СуммПоСтр + Области["П0000501" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000501124" + Графа].Значение = СуммПоСтр;
		
		СуммПоСтр = 0;
		Для Стр = 130 По 137 Цикл
			СуммПоСтр = СуммПоСтр + Области["П0000501" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0000501129" + Графа].Значение = СуммПоСтр;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 4 По 5 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101001" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101010" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101019" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101028" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101037" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101053" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101064" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101069" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101074" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101079" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000101084" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000201089" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000201099" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000201100" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000501124" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0000501129" + Графа + ";";
	КонецЦикла;
