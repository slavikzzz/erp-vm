...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 4 Цикл
		...
		//Раздел 5
		//строка 109 = ∑стр. 110 – 113 по всем графам
		СуммПоСтр = 0;
		Для Стр = 110 По 113 Цикл
			СуммПоСтр = СуммПоСтр + Области["П0050010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0050010109" + Графа].Значение = СуммПоСтр;
		
		//строка 114 = ∑стр. 115 – 122 по всем графам
		СуммПоСтр = 0;
		Для Стр = 115 По 122 Цикл
			СуммПоСтр = СуммПоСтр + Области["П0050010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0050010114" + Графа].Значение = СуммПоСтр;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 3 По 4 Цикл
		...
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0050010109" + Графа + ";";
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0050010114" + Графа + ";";
	КонецЦикла;
