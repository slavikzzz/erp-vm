...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки, ЕстьПредупреждения = Ложь)
	...
	Если НЕ (ПараметрыВыгрузки.ОтчетГод >= 2020 И ПараметрыВыгрузки.ОтчетГод <= 2023) Тогда
		СообщитьВТС("ОШИБКА: Задан год представления отчета, не входящий в интервал с 2020 г. по 2023 г.", "Титульный", "NamePeriod");
		СтатусПроверки = Ложь;
	КонецЕсли;
