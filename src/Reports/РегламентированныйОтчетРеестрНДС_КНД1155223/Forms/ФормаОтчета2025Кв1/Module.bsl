...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	// Группа П13000.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П1300008");
	ИменаГраф.Добавить("П1300009");
	ИменаПодчиненныхГрупп = Новый Массив;
	СтруктураМногострочныхЧастей.Вставить(
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П1200006") Тогда
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы110 Тогда
		Подстрока = "сведения из счета";
		ТекстВопроса = НСтр("ru = 'Удалить выбранные сведения из счета?';
							|en = 'Удалить выбранные сведения из счета?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы120 Тогда
		Подстрока = "адрес доставки";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный адрес доставки?';
							|en = 'Удалить выбранный адрес доставки?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы130 Тогда
		Подстрока = "договор аренды";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный договор аренды?';
							|en = 'Удалить выбранный договор аренды?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция ВысотаМногострочнойЧасти(ИмяГруппы, ТаблДок = Неопределено)
	...
	Если ТаблДок = Неопределено Тогда
		ТаблДок = ОбъектОтчета(ИмяФормы).ПолучитьМакет(Сред(ИмяФормы, СтрНайти(ИмяФормы, "ФормаОтчета"))
			+ "_" + СтруктураРеквизитовФормы.НаимТекущегоРаздела).ПолучитьОбласть();
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрДТСК");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRDT165.7.1"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRDT165.7.1-2"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузкиРеестра, ДеревоВыгрузкиСведений)
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			ДлинаРегНомДТ = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000002"]));
			Если НЕ (ДлинаРегНомДТ = 23 ИЛИ (ДлинаРегНомДТ >= 25 И ДлинаРегНомДТ <= 29)) Тогда
				ИмяПок = "П1000002" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не" + ?(ДлинаРегНомДТ = 0,
				"", "верно") + " задан регистрационный номер декларации на товары (экспорт). "
				+ "Возможное количество символов в номере: 23 или от 25 до 29", "Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100005"]) Тогда
					...
					+ " не задана дата платежа", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П11000.Данные["П1100005"]) Тогда
					ИмяПок = "П1100005" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " задана дата платежа,"
			...
			Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200006"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан адрес доставки товаров покупателю - физическому лицу", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			Для каждого Ветка_П13000 Из ДанныеМнЧ_П13000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П13000.Данные["П1300009"]) Тогда
					ИмяПок = "П1300009" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП13000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана дата договора", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П13000.Данные["П1300009"]) Тогда
					ИмяПок = "П1300009" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП13000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " задана дата договора,"
					+ " выходящая за пределы периода с 01.01.1900 по 31.12.2099", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрДТСК = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрДТСК");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрДТСК = СкопироватьУзел(УзелРодитель.Родитель, РеестрДТСК);
		Для каждого Эл Из ЭкзРеестрДТСК.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		// Узел "СведДТ" имеется только в схеме выгрузки "Сведений".
		// Если он не найден, значит, сейчас формируются данные "Реестра"
		// (для выгрузки первого из 2-х файлов).
		СведДТ = ПолучитьПодчиненныйЭлемент(ЭкзРеестрДТСК, "СведДТ");
		Если СведДТ = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведДТ = СкопироватьУзел(ЭкзРеестрДТСК, СведДТ, ЭкзРеестрДТСК.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСведДТ.Строки Цикл
					Если Эл.Строки.Количество() = 0 И Эл.Код <> "АдрДостТов" Тогда
						ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
					КонецЕсли;
				КонецЦикла;
				
				// 11000.
				СведСчОплТов = ПолучитьПодчиненныйЭлемент(ЭкзСведДТ, "СведСчОплТов");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					ЭкзСведСчОплТов = СкопироватьУзел(ЭкзСведДТ, СведСчОплТов, ЭкзСведДТ.Строки.Количество() - 1);
					Для каждого Эл Из ЭкзСведСчОплТов.Строки Цикл
						ВывестиПоказательВXML(Эл, Ветка_П11000.Данные[Эл.Ключ]);
					КонецЦикла;
				КонецЦикла;
				
				// 12000.
				АдрДостТов = ПолучитьПодчиненныйЭлемент(ЭкзСведДТ, "АдрДостТов");
				ДанныеМнЧ_П12000 = Ветка_П10000.ДанныеМногострочныхЧастей.П12000.Строки;
				Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
					ЭкзАдрДостТов = СкопироватьУзел(ЭкзСведДТ, АдрДостТов, ЭкзСведДТ.Строки.Количество() - 1);
					ВывестиПоказательВXML(ЭкзАдрДостТов, Ветка_П12000.Данные[ЭкзАдрДостТов.Ключ]);
				КонецЦикла;
				
				// 13000.
				СведДогАрен = ПолучитьПодчиненныйЭлемент(ЭкзСведДТ, "СведДогАрен");
				ДанныеМнЧ_П13000 = Ветка_П10000.ДанныеМногострочныхЧастей.П13000.Строки;
				Для каждого Ветка_П13000 Из ДанныеМнЧ_П13000 Цикл
					ЭкзСведДогАрен = СкопироватьУзел(ЭкзСведДТ, СведДогАрен, ЭкзСведДТ.Строки.Количество() - 1);
					Для каждого Эл Из ЭкзСведДогАрен.Строки Цикл
						Если Эл.Код = "НомДогАрен" Тогда
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_КНД1155223");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_КНД1155223_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
...
Процедура ВывестиНаПечатьОтчет(СтруктураПараметров, ВыбранныеСтраницы, СтрПоля = Неопределено)
	...
	Если ВыбранныеСтраницы.Найти("Титульный") <> Неопределено Тогда
		...
		ИмяМакета = "ПечатныйБланк2025Кв1_Титульный";
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Для каждого Строка12 Из Строки12 Цикл
					Если ЗначениеЗаполнено(Строка12.Данные.П1200006) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка12.Данные.П1200006));
					КонецЕсли;
				КонецЦикла;
				СтрокаДокумент.Параметры.П1200006 = СтрСоединить(МасТекЗнач, "; ");
				...
				КолСтрок13 = Строки13.Количество();
				
				МаксКолСтрок11_13 = Макс(КолСтрок11, КолСтрок13);
				
				Для Инд = 1 По МаксКолСтрок11_13 Цикл
					...
					КонецЕсли;
					Если Инд <= КолСтрок13 Тогда
						СтрокаДокумент.Параметры.Заполнить(Строки13[Инд - 1].Данные);
					КонецЕсли;
