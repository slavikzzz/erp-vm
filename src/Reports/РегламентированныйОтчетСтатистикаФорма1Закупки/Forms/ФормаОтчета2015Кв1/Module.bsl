...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "15/04/2015";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 13 Цикл 
		...
		Области["П01106" + ГрСтр].Значение = Области["П01107" + ГрСтр].Значение + Области["П01108" + ГрСтр].Значение + Области["П01109" + ГрСтр].Значение;
	КонецЦикла;
	...
	Для Гр = 3 По 13 Цикл 
		...
		КонецЕсли;
		ГрСтр =  Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=");
		Области["П01206" + ГрСтр].Значение = Области["П01207" + ГрСтр].Значение + Области["П01208" + ГрСтр].Значение
		 	+ Области["П01209" + ГрСтр].Значение + Области["П01210" + ГрСтр].Значение;
	КонецЦикла;
	...
	Для Гр = 4 По 12 Цикл
		ГрСтр =  Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=");
		ЗнП0210103 = ЗнП0210103 + Области["П02101" + ГрСтр].Значение;
		ЗнП0310103 = ЗнП0310103 + Области["П03101" + ГрСтр].Значение;
	КонецЦикла;
	Области.П0210103.Значение = ЗнП0210103;
	Области.П0310103.Значение = ЗнП0310103;
	
	ЗнП0410103 = 0;
	Для Гр = 4 По 16 Цикл
		ГрСтр =  Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=");
		ЗнП0410103 = ЗнП0410103 + Области["П04101" + ГрСтр].Значение;
	КонецЦикла;
	Области.П0410103.Значение = ЗнП0410103;
КонецПроцедуры
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть1_1а"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть1_1б"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-закупки");
	
	Для Инд = 1 По 4 Цикл 
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ЗаголовокРаздел"+Инд));
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("Раздел"+Инд+"Шапка"));
		Верх = ТабличныйДокумент.Область("Раздел"+Инд).Верх;
		Низ = ТабличныйДокумент.Область("Раздел"+Инд).Низ;
		Стр = Верх;
		Пока Стр <= Низ Цикл
			Обл = ТабличныйДокумент.ПолучитьОбласть(Стр,,Стр);
			Если ПечатнаяФорма.ПроверитьВывод(Обл) Тогда
	...
	Обл = ТабличныйДокумент.ПолучитьОбласть("Подвал");
	Если ПечатнаяФорма.ПроверитьВывод(Обл) Тогда
		ПечатнаяФорма.Вывести(Обл);
	Иначе
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-закупки");
