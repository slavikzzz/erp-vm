...
Процедура ДоступностьРазделов()
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
		...
		ОчищатьЗначение       = Ложь;
	Иначе
		ЦветРазделСправочно   = Новый Цвет();
		ЗащитаРазделСправочно = Истина;
		ОчищатьЗначение       = Истина;
	КонецЕсли;
	
	Для Стр = 67 По 72 Цикл
		мФормаОтчета.Области["П00300100" + Стр + "03"].ЦветФона = ЦветРазделСправочно;
		мФормаОтчета.Области["П00300100" + Стр + "03"].Защита = ЗащитаРазделСправочно;
		Если ОчищатьЗначение Тогда
			мФормаОтчета.Области["П00300100" + Стр + "03"].Значение = "";
		КонецЕсли;
		Если Стр = 67 Тогда
			Для Гр = 4 По 7 Цикл
				Если Гр = 5 Тогда
				...
				мФормаОтчета.Области["П00300100" + Стр + Графа].ЦветФона = ЦветРазделСправочно;
				мФормаОтчета.Области["П00300100" + Стр + Графа].Защита = ЗащитаРазделСправочно;
				Если ОчищатьЗначение Тогда
					мФормаОтчета.Области["П00300100" + Стр + Графа].Значение = "";
				КонецЕсли;
			КонецЦикла;
		ИначеЕсли Стр = 68 Тогда
			мФормаОтчета.Области["П00300100" + Стр + "05"].ЦветФона = ЦветРазделСправочно;
			мФормаОтчета.Области["П00300100" + Стр + "05"].Защита = ЗащитаРазделСправочно;
			Если ОчищатьЗначение Тогда
				мФормаОтчета.Области["П00300100" + Стр + "05"].Значение = "";
			КонецЕсли;
			
			мФормаОтчета.Области["П00300100" + Стр + "07"].ЦветФона = ЦветРазделСправочно;
			мФормаОтчета.Области["П00300100" + Стр + "07"].Защита = ЗащитаРазделСправочно;
			Если ОчищатьЗначение Тогда
				мФормаОтчета.Области["П00300100" + Стр + "07"].Значение = "";
			КонецЕсли;
		ИначеЕсли Стр = 69 Или Стр = 70 Тогда
			мФормаОтчета.Области["П00300100" + Стр + "04"].ЦветФона = ЦветРазделСправочно;
			мФормаОтчета.Области["П00300100" + Стр + "04"].Защита = ЗащитаРазделСправочно;
			Если ОчищатьЗначение Тогда
				мФормаОтчета.Области["П00300100" + Стр + "04"].Значение = "";
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 по 9 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		Области["П0020010008" + Графа].Значение = Области["П0020010009" + Графа].Значение + Области["П0020010014" + Графа].Значение;
	КонецЦикла;
	
	Для Гр = 3 по 7 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаПоСтр = 0;
		Для Стр = 40 по 52 Цикл 
			Если Стр = 45 Или Стр = 46 Или Стр = 49 Тогда 
				Продолжить;
			КонецЕсли;
			СуммаПоСтр = СуммаПоСтр + Области["П00300100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0030010039" + Графа].Значение = СуммаПоСтр;
	КонецЦикла;
	
	Для Гр = 3 по 7 Цикл
		Если Гр = 4 Или Гр = 5 Тогда
			Продолжить;
		КонецЕсли;
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаПоСтр = 0;
		Для Стр = 54 по 66 Цикл 
			Если Стр = 59 Или Стр = 60 Или Стр = 63 Тогда
				Продолжить;
			КонецЕсли;
			СуммаПоСтр = СуммаПоСтр + Области["П00300100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0030010053" + Графа].Значение = СуммаПоСтр;
	КонецЦикла;
	
	Для Гр = 4 по 9 Цикл
		Если Гр = 5 Или Гр = 6 Или Гр = 7 Тогда
			Продолжить;
		КонецЕсли;
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаПоСтр = 0;
		Для Стр = 54 по 65 Цикл
			Если Стр = 59 Или Стр = 60 Или Стр = 63 Тогда
				Продолжить;
			КонецЕсли;
			СуммаПоСтр = СуммаПоСтр + Области["П00300100" + Стр + Графа].Значение;
	...
	Для Гр = 5 по 11 Цикл
		Если Гр = 6 Или Гр = 7 Или Гр = 8 Или Гр = 9 Тогда
			Продолжить;
		КонецЕсли;
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаПоСтр = 0;
		Для Стр = 54 по 62 Цикл
			Если Стр = 59 Или Стр = 60 Тогда
				Продолжить;
			КонецЕсли;
			СуммаПоСтр = СуммаПоСтр + Области["П00300100" + Стр + Графа].Значение;
		КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 3 По 9 Цикл
		мФормаОтчета.Области["П00200100080" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00200100080" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Гр = 3 По 7 Цикл
		мФормаОтчета.Области["П00300100390" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00300100390" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Гр = 3 По 11 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		мФормаОтчета.Области["П0030010053" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0030010053" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
