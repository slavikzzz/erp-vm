...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "09/04/2021";
	СтруктураРеквизитовФормы.мКодОтчета   = "0609227";
...
Процедура ДоступностьРазделов()
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
		ЦветРазделСправочно   = Новый Цвет(255, 255, 192);
		ЗащитаРазделСправочно = Ложь;
	Иначе
		ЦветРазделСправочно   = Новый Цвет();
		ЗащитаРазделСправочно = Истина;
	КонецЕсли;
	
	Для Стр = 64 По 69 Цикл
		мФормаОтчета.Области["П0400" + Стр + "03"].ЦветФона = ЦветРазделСправочно;
		мФормаОтчета.Области["П0400" + Стр + "03"].Защита = ЗащитаРазделСправочно;
		Если Стр = 64 Тогда
			Для Гр = 4 По 7 Цикл
				Если Гр = 5 Тогда
					Продолжить;
				КонецЕсли;
				мФормаОтчета.Области["П0400640" + Гр].ЦветФона = ЦветРазделСправочно;
				мФормаОтчета.Области["П0400640" + Гр].Защита = ЗащитаРазделСправочно;
			КонецЦикла;
		ИначеЕсли Стр = 65 Тогда
			мФормаОтчета.Области["П04006505"].ЦветФона = ЦветРазделСправочно;
			мФормаОтчета.Области["П04006505"].Защита = ЗащитаРазделСправочно;
			
			мФормаОтчета.Области["П04006507"].ЦветФона = ЦветРазделСправочно;
			мФормаОтчета.Области["П04006507"].Защита = ЗащитаРазделСправочно;
		ИначеЕсли Стр = 66 Или Стр = 67 Тогда
			мФормаОтчета.Области["П0400" + Стр + "04"].ЦветФона = ЦветРазделСправочно;
			мФормаОтчета.Области["П0400" + Стр + "04"].Защита = ЗащитаРазделСправочно;
		КонецЕсли;
	КонецЦикла;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если СтруктураРеквизитовФормы.мСкопированаФорма <> Неопределено Тогда
		Если Не (НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета
			...
			мФормаОтчета = ЭтотОбъект.ПолеТабличногоДокументаФормаОтчета;
			Для Стр = 64 По 69 Цикл
				мФормаОтчета.Области["П0400" + Стр + "03"].Значение = 0;
				Если Стр = 64 Тогда
					Для Гр = 4 По 7 Цикл
						Если Гр = 5 Тогда
							Продолжить;
						КонецЕсли;
						мФормаОтчета.Области["П0400640" + Гр].Значение = 0;
					КонецЦикла;
				ИначеЕсли Стр = 65 Тогда
					мФормаОтчета.Области["П04006505"].Значение = 0;
					мФормаОтчета.Области["П04006507"].Значение = 0;
				ИначеЕсли Стр = 66 Или Стр = 67 Тогда
					мФормаОтчета.Области["П0400" + Стр + "04"].Значение = 0;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 по 11 Цикл
		...
		Тот = 0;
		Для Стр = 51 по 63 Цикл 
			Если Стр = 56 Или Стр = 57 Или Стр = 60 Или Области.Найти("П0301" + Стр + ГрСтр) = Неопределено Тогда 
				Продолжить;
			КонецЕсли;
			Тот = Тот + Области["П0301" + Стр + ГрСтр].Значение;
		КонецЦикла;
		Области["П030150" + ГрСтр].Значение = Тот;
	КонецЦикла;
	Для Гр = 3 по 7 Цикл
		ГрСтр = Формат(Гр, "ЧЦ=2; ЧВН=");
		Тот = 0;
		Для Стр = 37 по 49 Цикл 
			Если Стр = 42 Или Стр = 43 Или Стр = 46 Тогда 
				Продолжить;
			КонецЕсли;
			Тот = Тот + Области["П0300" + Стр + ГрСтр].Значение;
		КонецЦикла;
		Области["П030036" + ГрСтр].Значение = Тот;
	КонецЦикла;
	Для Гр = 3 по 9 Цикл
		ГрСтр = Формат(Гр, "ЧЦ=2; ЧВН=");
		Области["П020008" + ГрСтр].Значение = Области["П020009" + ГрСтр].Значение + Области["П020013" + ГрСтр].Значение;
	КонецЦикла;
КонецПроцедуры
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 3 По 9 Цикл
		мФормаОтчета.Области["П0200080" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0200080" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Гр = 3 По 7 Цикл
		мФормаОтчета.Области["П0300360" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0300360" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Гр = 3 По 11 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		мФормаОтчета.Области["П030150" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П030150" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 22-ЖКХ (ресурсы)");
