...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "StrCode",
		512, , "Выбор кода строки",   КоллекцияСписковВыбора["StrCode"]);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.Имя = "ОргАдрес" Тогда
		РегламентированнаяОтчетностьКлиент.ОбработкаАдресаВСтатистическомОтчетеJSON(
			ЭтотОбъект, Область, СтандартнаяОбработка);
	ИначеЕсли СтрНайти("П000101503,П000101603", Область.Имя) > 0 Тогда
		
		СтандартнаяОбработка = Ложь;
		
		НовЗначение = ?(Область.Значение = "V", "", "V");
		Область.Значение = НовЗначение;
		
		Если НовЗначение = "V" Тогда
