...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы1, 12);
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы2, 6);
	
	мСтруктураКолвоКолонокРазделов = Новый Структура;
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы1, 4);
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы2, 10);
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "1000013443", 512, , "Выбор кода");
...
Процедура ФормироватьСтруктуруСтраницОтчета() Экспорт
	...
	СтрокаУровня1.Представление            = "Статистика: Форма П-1";
...
Процедура ДоступностьРазделов()
	...
	Если НЕ (Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 3
	    Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6
	    Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)  = 9
	    Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12) Тогда
		Для Стр = 7 По 14 Цикл
			СтрНом    = Формат(Стр, "ЧЦ=2; ЧВН=");
			ЯчейкаИмя = "П00010" + СтрНом + "03";
			ПолеТабличногоДокументаФормаОтчета.Области[ЯчейкаИмя].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области[ЯчейкаИмя].Защита = Истина;
		КонецЦикла;
	КонецЕсли;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Не СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		...
		Если СтрНайти("П000101403", Область.Имя) > 0 Тогда
			...
			КонецЕсли;
		ИначеЕсли СтрНайти(Область.Имя, "П000500103_") = 1 Тогда
			...
			Если ПараметрВводаПоля <> Неопределено Тогда
				ОбластьКод  = ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(Область.Имя, "П000500103_", "П000500105_")];
				ОбластьНаим = ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(Область.Имя, "П000500105_", "П000500103_")];
		...
		ИначеЕсли СтрНайти(Область.Имя, "П000500101_") = 1 Или СтрНайти(Область.Имя, "П000500104_") = 1 Тогда
			...
			Если ПараметрВводаПоля <> Неопределено Тогда
				ОбластьКод = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "П000500101_", "П000500104_"));
				ОбластьНаим  = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "П000500104_", "П000500101_"));
				
				МассивОчистки = Новый Массив;
				МассивОчистки.Добавить(ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(ОбластьКод.Имя, "П000500104_", "П000500103_")]);
				МассивОчистки.Добавить(ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(ОбластьКод.Имя, "П000500104_", "П000500105_")]);
		...
		ИначеЕсли СтрНайти(Область.Имя, "П000500102_") = 1 Тогда
			...
			МассивОчистки = Новый Массив;
			МассивОчистки.Добавить(ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(Область.Имя, "П000500102_", "П000500101_")]);
			МассивОчистки.Добавить(ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(Область.Имя, "П000500102_", "П000500103_")]);
			МассивОчистки.Добавить(ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(Область.Имя, "П000500102_", "П000500104_")]);
			МассивОчистки.Добавить(ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(Область.Имя, "П000500102_", "П000500105_")]);
		...
		ИначеЕсли СтрНайти(Область.Имя, "П000202101_") = 1 Или СтрНайти(Область.Имя, "П000202103_") = 1 Тогда
			ИмяСправочника = "1000013443";
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, ИмяСправочника);
			
			ОбластьКод = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "П000202101_", "П000202103_"));
			ОбластьНаим  = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "П000202103_", "П000202101_"));
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	ТаблицаВариантовЗаполнения =
		Форма.ТаблицаВариантыЗаполненияПолеТабличногоДокументаФормаОтчета[0].ТаблицаВариантовЗаполнения;
	...
	Для НомСтр = 1 По Форма.П0002021.Количество() Цикл
		Сумма1 = Сумма1
			+ РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Области["П000202104_" + НомСтр].Значение);
	КонецЦикла;
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	РегламентированнаяОтчетность.ОбработатьУсловныеЭлементы(ЭтотОбъект, ПараметрыВыгрузки, ДеревоВыгрузки); // исключить строки из многострочной части
...
Функция ЗаполнитьАвтоНаСервере()
	...
	КонвертированныеЗначения1 =
		РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтотОбъект, П0002021, Ложь);
	
	КонвертированныеЗначения2 =
		РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтотОбъект, П0005001, Ложь);
	
	Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок1", КонвертированныеЗначения1);
	Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок2", КонвертированныеЗначения2);
...
Процедура ЗаполнитьДаннымиОтчетовНаСервере(ТекстОписанияПроблемы)
	...
	ЗаполнениеСтатистическойОтчетности.ЗаполнитьДаннымиПредидущегоПериода(
		ЭтотОбъект, "МакетПереносаДанных2019Кв1", ТекстОписанияПроблемы);
КонецПроцедуры
