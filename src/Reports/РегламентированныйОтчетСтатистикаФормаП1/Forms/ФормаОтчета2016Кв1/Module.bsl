...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНайти("П000101503,П000101603,П000101703,П000101803,П000101903,П000102003", Область.Имя) > 0 Тогда
...
Процедура ВыгрузитьОтчет(ВыгружатьСОшибками = Ложь) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ВыгрузитьОтчетПродолжение", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ВыгрузитьОтчетЗавершение(ТекФормаП1Пр3, ДополнительныеПараметры) Экспорт
	...
	ФормаВыгрузкиРеглОтчета.СформироватьИЗаписать(СпДокОсн,,,,,,,,,ДополнительныеПараметры.ВыгружатьСОшибками,,,, СтруктураДополнительныхПараметров);
...
&НаСервере
Функция ОбъединитьТекстыXML_П1(ТекстXMLОсн, ТекстXMLДоп, ПараметрыИнициализации = Неопределено) Экспорт
	...
	ИтоговыйРезультат = РегламентированнаяОтчетность.ВыгрузитьДеревоВXML(ДеревоСтруктуры, Парам);
	Если ЗначениеЗаполнено(ПараметрыИнициализации) Тогда 
		Текст = Новый ТекстовыйДокумент;
		Текст.УстановитьТекст(ИтоговыйРезультат);
		ИмяКонечногоФайла = ПолучитьИмяВременногоФайла();
		Текст.Записать(ИмяКонечногоФайла, "utf-8");
		ВХ = Новый ДвоичныеДанные(ИмяКонечногоФайла);
		ПараметрыИнициализации.Вставить("ВХ", Новый ХранилищеЗначения(ВХ));
		УдалитьФайлы(ИмяКонечногоФайла);
	КонецЕсли;
...
&НаСервере
Процедура ПоместитьТекстВыгрузкиВПараметры(ПараметрыИнициализации)
	Текст = Новый ТекстовыйДокумент;
	Текст.УстановитьТекст(ПараметрыИнициализации.ТекстВыгрузки);
	ИмяКонечногоФайла = ПолучитьИмяВременногоФайла();
	Текст.Записать(ИмяКонечногоФайла, "utf-8");
	ВХ = Новый ДвоичныеДанные(ИмяКонечногоФайла);
	ПараметрыИнициализации.Вставить("ВХ", Новый ХранилищеЗначения(ВХ));
...
&НаСервере
Функция ПолучитьПараметрыИнициализацииУниверсальногоОтчета(НеобходимоЗагрузитьШаблон = Истина)
	Попытка
		...
		СведШаблон = УниверсальныйОтчетСтатистики.ПолучитьСведенияОШаблоне(ЭтаФорма, "АтрибВыгрузкиXML2016Кв1");
		Для Каждого КЗ Из СведШаблон Цикл 
			Результат.Вставить(КЗ.Ключ, КЗ.Значение);
		КонецЦикла;
		
		Если НеобходимоЗагрузитьШаблон Тогда 
			ДанныеШаблона = УниверсальныйОтчетСтатистики.ПолучитьШаблон(ИмяФормы, "ШаблонРосстата2016Кв1");
		...
		Результат.Вставить("ИмяМакетаАтрибутов", "АтрибВыгрузкиXML2016Кв1");
		Результат.Вставить("ИмяМакетаШаблона", "ШаблонРосстата2016Кв1");
		Результат.Вставить("ИмяСхемы", "СхемаВыгрузкиXML2016Кв1");
...
Процедура ПроверкаКСШаблоном(Команда)
	ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Проверка контрольных соотношений в неактуальной версии формы не поддерживается';
													|en = 'Проверка контрольных соотношений в неактуальной версии формы не поддерживается'"));
	Возврат;
...
Процедура ПроверкаКСШаблономПослеСохраненияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ДокументОтчета = ПолучитьДокументИлиПараметрыОтбораП1Пр3(ПараметрыОтбораОтчета);
	Если ЗначениеЗаполнено(ДокументОтчета) Тогда 
		ИмяТекФормы = ПолучитьФормуДляПериода(ПараметрыОтбораОтчета.ИсточникОтчета, ПараметрыОтбораОтчета.ДатаКонцаПериодаОтчета);
	...
	КонецЕсли;
	ПараметрыИнициализации = ПолучитьПараметрыИнициализацииУниверсальногоОтчета(Ложь);
	
	Если ТипЗнч(ФормаОтчетаП1Пр3) = Тип("ФормаКлиентскогоПриложения") Тогда
		ТекстВыгрузкиП1Пр3 = ФормаОтчетаП1Пр3.ПолучитьТекстВыгрузки(ПараметрыИнициализации);
		ТекстВыгрузки = ОбъединитьТекстыXML_П1(ПараметрыИнициализации.ТекстВыгрузки, ТекстВыгрузкиП1Пр3, ПараметрыИнициализации);
	Иначе
		ПоместитьТекстВыгрузкиВПараметры(ПараметрыИнициализации);
	КонецЕсли;
