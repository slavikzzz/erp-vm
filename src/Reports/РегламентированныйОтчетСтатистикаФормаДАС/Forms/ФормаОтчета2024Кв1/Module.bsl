...
Процедура ДоступностьРазделов()
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 3
		...
		Для НомСтр = 1 По 5 Цикл
			ИмяЯчейки = "П000120100" + НомСтр + "01";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Область.Защита 
		 ...
		 Или Лев(Область.Имя, 3) = "Орг" 
		 Или Область.Имя = "ДатаСоставленияОтчета" Тогда
			Возврат;
		КонецЕсли;
		...
		Если ТекГруппа = "П00001" Тогда
			УстановитьЗначокВГруппеЯчеек(Область, 1, 6, 1, 1);
		ИначеЕсли ТекГруппа = "П00002" Тогда
			УстановитьЗначокВГруппеЯчеек(Область, 1, 1, 1, 8);
		ИначеЕсли ТекГруппа = "П00003" Тогда
			НомерСтроки = Число(Сред(Область.Имя, 11, 1));
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 1, 7);
		ИначеЕсли ТекГруппа = "П00004" Тогда
			УстановитьЗначокВГруппеЯчеек(Область, 1, 1, 1, 3);
		...
		ИначеЕсли ТекГруппа = "П00008" Тогда
			НомерСтроки = Число(Сред(Область.Имя, 11, 1));
			Если СтрНайти("1234", Прав(Область.Имя, 1)) > 0 Тогда
		...
		ИначеЕсли ТекГруппа = "П00009" Тогда
			УстановитьЗначокВГруппеЯчеек(Область, 1, 13, 1, 1, Истина);
		ИначеЕсли ТекГруппа = "П00010" Тогда
			НомерКолонки = Число(Прав(Область.Имя, 1));
			Если НомерКолонки = 1 Тогда
				УстановитьЗначокВГруппеЯчеек(Область, 1, 3, НомерКолонки, НомерКолонки);
			Иначе
				УстановитьЗначокВГруппеЯчеек(Область, 4, 6, НомерКолонки, НомерКолонки);
			КонецЕсли;
		ИначеЕсли ТекГруппа = "П00011" Тогда
			НомерСтроки = Число(Сред(Область.Имя, 10, 2));
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 1, 6);
		ИначеЕсли ТекГруппа = "П00012" Тогда
			Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6
			 Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
				УстановитьЗначокВГруппеЯчеек(Область, 1, 5, 1, 1);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьЗначокВГруппеЯчеек(Область, НачальнаяСтрокаВГруппе, КонечнаяСтрокаВГруппе, НачальнаяКолонкаВГруппе, КонечнаяКолонкаВГруппе, МножественныйВыбор = Ложь)
	...
	Если Лев(Область.Имя, 6) = "П00009" И НовЗначениеОбласти <> "" Тогда
		...
		Для Н = 1 По 13 Цикл
			ОтмеченнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области["П00009010" + Формат(Н, "ЧЦ=2; ЧВН=") + "01"];
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	// По разделу 1.
	ДополнитьПараметрыВыгрузки(ТабДокумент, ПараметрыВыгрузки, "П0000101", 1, 1, 1, 6);
	
	// По разделу 2.
	ДополнитьПараметрыВыгрузки(ТабДокумент, ПараметрыВыгрузки, "П0000201", 1, 8, 1, 1);
	
	// По разделу 3.
	ДополнитьПараметрыВыгрузки(ТабДокумент, ПараметрыВыгрузки, "П0000301", 1, 7, 1, 2);
	
	// По разделу 4.
	ДополнитьПараметрыВыгрузки(ТабДокумент, ПараметрыВыгрузки, "П0000401", 1, 3, 1, 1);
	
	// По разделу 5.
	ДополнитьПараметрыВыгрузки(ТабДокумент, ПараметрыВыгрузки, "П0000501", 1, 3, 1, 1);
	
	// По разделу 6.
	ДополнитьПараметрыВыгрузки(ТабДокумент, ПараметрыВыгрузки, "П0000601", 1, 6, 1, 4);
	
	// По разделу 7.
	ДополнитьПараметрыВыгрузки(ТабДокумент, ПараметрыВыгрузки, "П0000701", 1, 4, 1, 5);
	
	// По разделу 8.
	ДополнитьПараметрыВыгрузки(ТабДокумент, ПараметрыВыгрузки, "П0000801", 1, 7, 1, 7);
	
	// По разделу 9.
	ДополнитьПараметрыВыгрузки(ТабДокумент, ПараметрыВыгрузки, "П0000901", 1, 1, 1, 13);
	
	// По разделу 10.
	ДополнитьПараметрыВыгрузки(ТабДокумент, ПараметрыВыгрузки, "П0001001", 1, 2, 1, 6);
	
	// По разделу 11.
	ДополнитьПараметрыВыгрузки(ТабДокумент, ПараметрыВыгрузки, "П0001101", 1, 6, 1, 2);
	
	// По разделу 12.
	ДополнитьПараметрыВыгрузки(ТабДокумент, ПараметрыВыгрузки, "П0001201", 1, 1, 1, 5);
...
Процедура ДополнитьПараметрыВыгрузки(ТДок, ПараметрыВыгрузки, ОснЧастьИмени,
									 НачИндСтроки, КонИндСтроки, НачИндСтолбца, КонИндСтолбца)
	
	Для ИндСтроки = НачИндСтроки По КонИндСтроки Цикл
		СтрИндСтроки = Формат(ИндСтроки, "ЧЦ=2; ЧВН=; ЧГ=");
		Для ИндСтолбца = НачИндСтолбца По КонИндСтолбца Цикл
			СтрИндСтолбца = Формат(ИндСтолбца, "ЧЦ=3; ЧВН=; ЧГ=");
			
			ИмяКлюча = ОснЧастьИмени + СтрИндСтолбца + СтрИндСтроки;
			
			Если РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТДок, ИмяКлюча) Тогда
				ПараметрыВыгрузки.Вставить(ИмяКлюча + "_Ч", ?(ЗначениеЗаполнено(ТДок[ИмяКлюча]), 1, 0));
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
