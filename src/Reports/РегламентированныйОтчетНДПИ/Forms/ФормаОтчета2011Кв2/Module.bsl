...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "28/04/2011";
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Титульный",   "МЧБ_2011кв2_01_Титул");
	СоответствиеРазделовМакетам.Вставить("Раздел1",     "МЧБ_2011кв2_02_Раздел1");
	СоответствиеРазделовМакетам.Вставить("Раздел2",     "МЧБ_2011кв2_03_Раздел2");
	СоответствиеРазделовМакетам.Вставить("Раздел3",     "МЧБ_2011кв2_04_Раздел3");
	СоответствиеРазделовМакетам.Вставить("Раздел41",    "МЧБ_2011кв2_05_Раздел41");
	СоответствиеРазделовМакетам.Вставить("Раздел42",    "МЧБ_2011кв2_06_Раздел42");
	СоответствиеРазделовМакетам.Вставить("Раздел43",    "МЧБ_2011кв2_07_Раздел43");
...
Процедура СформироватьСпискиВыбора()
	...
	//Раздел 1
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000100001001", 20, , "Выбор КБК", КоллекцияСписковВыбора[ИмяСекцииКБК]);
	//Раздел 2
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000200002003", 20, , "Выбор КБК", КоллекцияСписковВыбора[ИмяСекцииКБК]);
	...
	//Раздел 4
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000400001003", 20, , "Выбор КБК", КоллекцияСписковВыбора[ИмяСекцииКБК]);
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы",				"NO_NDPI_1_010_00_05_01_01.xsd");
	...
	Для Каждого Стр Из Раздел2 Цикл
		...
		Пока ДанныеСтраницы.Свойство("П000200010010_" + Формат(НомерСтроки, "ЧГ=")) Цикл
			...
			Если СерияРаздел2МнчДлина <> 3 Тогда
				СообщитьВТС("В разделе 2 на странице " + НомерСтраницы + " в строке " + НомерСтроки + " не" + ?(СерияРаздел2МнчДлина = 0, "", "верно") + " задано значение серии лицензии на пользование недрами!", "Раздел2", СерияРаздел2МнчКод, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			НомерРаздел2МнчДлина = СтрДлина(НомерРаздел2Мнч);
			Если НомерРаздел2МнчДлина <> 5 Тогда
				СообщитьВТС("В разделе 2 на странице " + НомерСтраницы + " в строке " + НомерСтроки + " не" + ?(НомерРаздел2МнчДлина = 0, "", "верно") + " задано значение номера лицензии на пользование недрами!", "Раздел2", НомерРаздел2МнчКод, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			КодРаздел2МнчДлина = СтрДлина(КодРаздел2Мнч);
			Если КодРаздел2МнчДлина <> 2 Тогда
				СообщитьВТС("В разделе 2 на странице " + НомерСтраницы + " в строке " + НомерСтроки + " не" + ?(КодРаздел2МнчДлина = 0, "", "верно") + " задано значение вида лицензии на пользование недрами!", "Раздел2", КодРаздел2МнчКод, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если КодДПИРаздел2 = "03100" И НЕ ЗначениеЗаполнено(КвРаздел2Мнч) Тогда
				СообщитьВТС("В разделе 2 на странице " + НомерСтраницы + " в строке " + НомерСтроки + " не задано значение коэффициента Кв (обязательно при коде добытого полезного ископаемого 03100)!", "Раздел2", КвРаздел2МнчКод, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницы = "", ОбластьИмя = "") Экспорт
	...
	ИначеЕсли ИмяТекущейСтраницы = "Раздел3" Тогда
		...
		//Сбор данных  
		П000310001003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Данные.П000310001003);
		П000310002003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Данные.П000310002003);
		П000310003003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Данные.П000310003003);
		П000310005003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Данные.П000310005003);
		П000310006003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Данные.П000310006003);
		П000310007003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Данные.П000310007003);
		П000310008003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, Данные.П000310008003);
		...
		П000310010003 = П000310004003 + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(Форма, П000310009003);
