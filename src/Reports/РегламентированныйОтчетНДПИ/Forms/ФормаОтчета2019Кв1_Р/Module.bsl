...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2019Кв1_Р";
	...
	СтруктураРеквизитовФормы.ВариантыФорматовВыгрузки.Добавить(СтруктураРеквизитовФормы.мСоставПоказателей, "Формат 5.05", Истина);
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	ПрефиксИменМакетов = "ПечатныйБланк2019Кв1_Р_";
...
Процедура ВывестиНаПечатьДанныеМногоуровневогоРаздела(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	Если ИмяТекРаздела = "Раздел2" Тогда
		...
		Для НомСтрокиВДереве = 1 По ДеревоДанныхРаздела.Строки.Количество() Цикл
			...
			Для НомСтрокиВВетке2 = 1 По Ветка2.Строки.Количество() Цикл
				...
				// Вывод листа с кодом "01009033".
				ВывестиНаПечатьЛист(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля);
				
				// Определение количества листов с кодом "01009040",
				// относящихся к выведенному листу с кодом "01009033".
				//
				НомерСтроки211 = 0;
				...
				КолЛистов01009040ПоКолСтрок211 = ?(ДробКол = ЦелКол, ЦелКол, ЦелКол + 1);
				КолЛистов01009040ПоКолСтрок212 = 0;
				Для НомСтрокиВВетке21 = 1 По Ветка21.Строки.Количество() Цикл
					...
					КолЛистов01009040ПоКолСтрок212 = КолЛистов01009040ПоКолСтрок212 + ?(ДробКол = ЦелКол, ЦелКол, ЦелКол + 1);
				КонецЦикла;
				// Необходимое количество листов с кодом "01009040".
				КолЛистов01009040 = Макс(КолЛистов01009040ПоКолСтрок211, КолЛистов01009040ПоКолСтрок212);
				
				// Формирование данных для заполнения листов с кодом "01009040".
				//
				МассивДанных211 = Новый Массив;
				Для НомЛиста = 1 По КолЛистов01009040 Цикл
				...
				НомСтрокиВВетке21 = 1;
				НомЛиста = 1;
				Пока НомЛиста <= КолЛистов01009040 Цикл
		...
		Для НомСтрокиВДереве = 1 По ДеревоДанныхРаздела.Строки.Количество() Цикл
			...
			Для НомСтрокиВВетке3 = 1 По Ветка3.Строки.Количество() Цикл
				...
				Для НомСтрокиВВетке31 = 1 По Ветка31.Строки.Количество() Цикл
					...
					// Определение текущего количества листов с кодом "01009057".
					//
					НомерСтроки311 = 0;
					...
					КолТекЛистов01009057ПоКолСтрок311 = ?(ДробКол = ЦелКол, ЦелКол, ЦелКол + 1);
					
					// Формирование данных многосторочной части 3.1.1 для заполнения листов с кодом "01009057".
					//
					МассивДанных311 = Новый Массив;
					Для НомЛиста = 1 По КолТекЛистов01009057ПоКолСтрок311 Цикл
						ПриращениеКНомеру = (НомЛиста - 1) * 3;
	...
	ИначеЕсли ИмяТекРаздела = "Раздел5" Тогда
		...
		// Вывод листов с кодом "01009071".
		//
		Для НомСтрокиВДереве = 1 По ДеревоДанныхРаздела.Строки.Количество() Цикл
			
			Ветка5 = ДеревоДанныхРаздела.Строки[НомСтрокиВДереве - 1];
			
			// Формирование данных многосторочной части 5.1 для заполнения листов с кодом "01009071".
			//
	...
	ИначеЕсли ИмяТекРаздела = "Раздел8" Тогда
		...
		Для НомСтрокиВДереве = 1 По ДеревоДанныхРаздела.Строки.Количество() Цикл
			...
			Для НомСтрокиВВетке8 = 1 По Ветка8.Строки.Количество() Цикл
				...
				// Определение текущего количества листов с кодом "01009125".
				//
				НомерСтрокиМнЧ = 0;
				...
				КолТекЛистов01009125ПоКолСтрокМнЧ = ?(ДробКол = ЦелКол, ЦелКол, ЦелКол + 1);
				
				// Формирование данных многосторочной части 8.1 для заполнения листов с кодом "01009125".
				//
				МассивДанныхМнЧ = Новый Массив;
				Для НомЛиста = 1 По КолТекЛистов01009125ПоКолСтрокМнЧ Цикл
					ПриращениеКНомеру = (НомЛиста - 1) * 3;
				...
				Для НомЛиста = 1 По КолТекЛистов01009125ПоКолСтрокМнЧ Цикл
					...
					ТабличныйДокумент.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел81"));
					...
					Если НомСтрокиВВетке8 = 1 И НомЛиста = 1 Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СпискиВыбора2019Кв1_Р");
	...
	"Выбор кода основания налогообложения", КоллекцияСписковВыбора["ОснованияНалогообложения"]);
	...
	"Выбор кода вида полезного ископаемого", КоллекцияСписковВыбора["ВидыПИ"]);
	...
	"Выбор кода бюджетной классификации", КоллекцияСписковВыбора["КБК"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000800011003",  1, ,
	...
	Если ПроверятьРаздел5 Тогда
		...
		Для каждого СтраницаРаздела5 Из ДанныеРаздела5.Строки Цикл
			...
			Для каждого СтраницаРаздела51 Из СтраницаРаздела5.Строки Цикл
				...
				Если СерЛицРаздел51Длина <> 3 И СерЛицРаздел51Длина <> 4 Тогда
					СообщитьВТС("Раздел 5 -> Уплата налога. Страница " + НомерСтраницы5
					+ " -> Раздел 5.1. Страница " + НомерСтраницы51
	...
	Если ПроверятьРаздел8 Тогда
		...
		Для каждого СтраницаРаздела8 Из ДанныеРаздела8.Строки Цикл
			...
			Для каждого СтраницаРаздела81 Из СтраницаРаздела8.Строки Цикл
				...
				ГодНачРаздел81 = СокрЛП(СтраницаРаздела81.Данные["П000800007003"]);
				ПрКгРаздел81   = СокрЛП(СтраницаРаздела81.Данные["П000800011003"]);
				...
				КонецЕсли;
				// Серия, номер и вид лицензии на пользование недрами.
				СерЛицРаздел81Длина = СтрДлина(СерЛицРаздел81);
				Если СерЛицРаздел81Длина <> 3 Тогда
					СообщитьВТС("Раздел 8 -> Уплата налога. Страница " + НомерСтраницы
					+ " -> Расчет налога. Страница " + НомерСтраницы81
					+ ": не" + ?(СерЛицРаздел81Длина = 0, "", "верно")
					+ " задана серия лицензии на пользование недрами",
					"Раздел81", "П000800005003", КоординатыСтраницы81);
					СтатусПроверки = Ложь;
				КонецЕсли;
				НомЛицРаздел81Длина = СтрДлина(НомЛицРаздел81);
				Если НомЛицРаздел81Длина <> 5 Тогда
					СообщитьВТС("Раздел 8 -> Уплата налога. Страница " + НомерСтраницы
					+ " -> Расчет налога. Страница " + НомерСтраницы81
					+ ": не" + ?(НомЛицРаздел81Длина = 0, "", "верно")
					+ " задан номер лицензии на пользование недрами",
					"Раздел81", "П000800005004", КоординатыСтраницы81);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ВидЛицРаздел81Длина = СтрДлина(ВидЛицРаздел81);
				Если ВидЛицРаздел81Длина <> 2 Тогда
					СообщитьВТС("Раздел 8 -> Уплата налога. Страница " + НомерСтраницы
					+ " -> Расчет налога. Страница " + НомерСтраницы81
					+ ": не" + ?(ВидЛицРаздел81Длина = 0, "", "верно")
					+ " задан вид лицензии на пользование недрами",
					"Раздел81", "П000800005005", КоординатыСтраницы81);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Если ГодНачРаздел81Длина <> 4 Тогда
					...
					"Раздел81", "П000800007003", КоординатыСтраницы81);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Если НЕ ЗначениеЗаполнено(ПрКгРаздел81) Тогда
					...
					"Раздел81", "П000800011003", КоординатыСтраницы81);
					СтатусПроверки = Ложь;
...
Процедура ДобавитьСтраницуВРазделе8НаСервере()
	...
	ДеревоДанныхРаздел8 = ПолучитьИзВременногоХранилища(СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхРаздел8"]);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "П000800007003" Тогда
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	ПроверитьКС_8_1(СтруктураРезультата);
	ПроверитьКС_8_2(СтруктураРезультата);
	ПроверитьКС_8_3(СтруктураРезультата);
	ПроверитьКС_8_4(СтруктураРезультата);
...
Процедура ПроверитьКС_1_1(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Раздел 1 (КС 1.1)';
							|en = 'Раздел 1 (КС 1.1)'");
	ОписаниеКС      = НСтр("ru = 'КБК по строке 010 Раздела 1 должен соответствовать КБК, указанному в строке 020 Раздела';
							|en = 'КБК по строке 010 Раздела 1 должен соответствовать КБК, указанному в строке 020 Раздела'")
		+ НСтр("ru = ' 2 или в строке 020 Раздела 3 или в строке 020 раздела 4 или в строке 020 Раздела 5 или в строке 010 Р';
				|en = ' 2 или в строке 020 Раздела 3 или в строке 020 раздела 4 или в строке 020 Раздела 5 или в строке 010 Р'")
		+ НСтр("ru = 'аздела 7 или в строке 020 Раздела 8';
				|en = 'аздела 7 или в строке 020 Раздела 8'");
	ОписаниеОшибки  = НСтр("ru = 'Необходимо убедиться в правильности заполнения КБК в Разделе 2, 3, 4, 5, 7 и 8';
							|en = 'Необходимо убедиться в правильности заполнения КБК в Разделе 2, 3, 4, 5, 7 и 8'");
	...
	Р8Ст20Гр03 = ПоказательПоСтроке("Р8Ст20Гр03");
	...
	Раздел7 = мДанныеОтчета.Раздел7;
	Раздел8 = ДанныеИзДереваРаздела("Раздел8");
	...
	Для Инд = 1 По КолвоСтрокВМногострочнойЧасти Цикл
		...
		Для Каждого СтраницаРаздела Из Раздел2 Цикл
			...
			Иначе
				НомерСтраницыСтр = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Раздел2.Индекс(СтраницаРаздела) + 1);
			КонецЕсли;
			...
			Если Р1Ст1Гр01.Значение = Р2Ст20Гр03.Значение Тогда
				...
				МассивРасшифровки.Добавить(Р1Ст1Гр01.ЗначениеСтр + " = " + Р2Ст20Гр03.ЗначениеСтр);
				Прервать;
			КонецЕсли;
		...
		Для Каждого СтраницаРаздела Из Раздел3 Цикл
			...
			Если ДанныеСтраницы.Свойство("НомерСтраницы") Тогда
				НомерСтраницыСтр = ДанныеСтраницы.НомерСтраницы;
			Иначе
				НомерСтраницыСтр = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Раздел3.Индекс(СтраницаРаздела) + 1);
			КонецЕсли;
			...
			Если Р1Ст1Гр01.Значение = Р3Ст20Гр03.Значение Тогда
				...
				МассивРасшифровки.Добавить(Р1Ст1Гр01.ЗначениеСтр + " = " + Р3Ст20Гр03.ЗначениеСтр);
				Прервать;
			КонецЕсли;
		...
		Для Каждого СтраницаР4 Из Раздел4 Цикл
			...
			НомерСтраницыР4Стр = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Раздел4.Индекс(СтраницаР4) + 1);
			...
			Если Р1Ст1Гр01.Значение = Р4Ст20Гр03.Значение Тогда
				...
				МассивРасшифровки.Добавить(Р1Ст1Гр01.ЗначениеСтр + " = " + Р4Ст20Гр03.ЗначениеСтр);
				Прервать;
			КонецЕсли;
		...
		Для Каждого СтраницаРаздела Из Раздел5 Цикл
			...
			Если Р1Ст1Гр01.Значение = Р5Ст20Гр03.Значение Тогда
				...
				МассивРасшифровки.Добавить(Р1Ст1Гр01.ЗначениеСтр + " = " + Р5Ст20Гр03.ЗначениеСтр);
				Прервать;
			КонецЕсли;
		...
		Для Каждого СтраницаР7 Из Раздел7 Цикл
			...
			НомерСтраницыР7Стр = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Раздел7.Индекс(СтраницаР7) + 1);
			...
			Если Р1Ст1Гр01.Значение = Р7Ст10Гр03.Значение Тогда
				...
				МассивРасшифровки.Добавить(Р1Ст1Гр01.ЗначениеСтр + " = " + Р7Ст10Гр03.ЗначениеСтр);
				Прервать;
			КонецЕсли;
			
		КонецЦикла;
		
		Для Каждого СтраницаРаздела Из Раздел8 Цикл
			ДанныеСтраницы = СтраницаРаздела.Данные;
			НомерСтраницыСтр = "";
			Если ДанныеСтраницы.Свойство("НомерСтраницы") Тогда
				НомерСтраницыСтр = ДанныеСтраницы.НомерСтраницы;
			Иначе
				НомерСтраницыСтр = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Раздел8.Индекс(СтраницаРаздела) + 1);
			КонецЕсли;
			
			ВставитьЗначениеПоказателя(ДанныеСтраницы, Р8Ст20Гр03);
			
			Если Р1Ст1Гр01.Значение = Р8Ст20Гр03.Значение Тогда
				ВнестиПоказательВРасшифровку(Р8Ст20Гр03, Р8Ст20Гр03.Раздел, НомерСтраницыМнУ(СтраницаРаздела.Представление, Р8Ст20Гр03.ПорядокСтраницы));
				МассивРасшифровки.Добавить(Р1Ст1Гр01.ЗначениеСтр + " = " + Р8Ст20Гр03.ЗначениеСтр);
				Прервать;
			КонецЕсли;
		...
		Если Р1Ст1Гр01.Значение = Р2Ст20Гр03.Значение
			...
			ИЛИ Р1Ст1Гр01.Значение = Р7Ст10Гр03.Значение
			ИЛИ Р1Ст1Гр01.Значение = Р8Ст20Гр03.Значение Тогда
			ЭтоОшибка = Ложь;
		...
		Если МассивРасшифровки.Количество() = 0 Тогда
			МассивРасшифровки.Добавить(Р1Ст1Гр01.ЗначениеСтр + " = ("""")");
		КонецЕсли;
		Расшифровка = СтрСоединить(МассивРасшифровки, "; ");
...
Процедура ПроверитьКС_1_2(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru = 'Раздел 1 (КС 1.2)';
							|en = 'Раздел 1 (КС 1.2)'");
	ОписаниеКС      = НСтр("ru = 'Код по ОКТМО в строке 020 Раздела 1 должен соответствовать коду по ОКТМО, указанному в ';
							|en = 'Код по ОКТМО в строке 020 Раздела 1 должен соответствовать коду по ОКТМО, указанному в '")
		+ НСтр("ru = 'строке 040 Раздела 2.1 или в строке 080 Раздела 3.1 или в строке 040 Раздела 4 или в строке 050 раздел';
				|en = 'строке 040 Раздела 2.1 или в строке 080 Раздела 3.1 или в строке 040 Раздела 4 или в строке 050 раздел'")
		+ НСтр("ru = 'а 5.1 или в строке 040 Раздела 7 или в строке 040 Раздела 8';
				|en = 'а 5.1 или в строке 040 Раздела 7 или в строке 040 Раздела 8'");
	ОписаниеОшибки  = НСтр("ru = 'Необходимо убедиться в правильности заполнения ОКТМО в Разделе 2, 3, 4, 5, 7 и 8';
							|en = 'Необходимо убедиться в правильности заполнения ОКТМО в Разделе 2, 3, 4, 5, 7 и 8'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
	...
	Р8Ст40Гр03 = ПоказательПоСтроке("Р8Ст40Гр03");
	...
	Для Инд = 1 По КолвоСтрокВМногострочнойЧасти Цикл
		...
		Для Каждого Страница Из Раздел2 Цикл
			...
			Если ДанныеСтраницы.Свойство("НомерСтраницы") Тогда
				НомерСтраницыСтр = ДанныеСтраницы.НомерСтраницы;
			Иначе
				НомерСтраницыСтр = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Раздел2.Индекс(Страница) + 1);
			КонецЕсли;
			...
			Если Р1Ст1Гр02.Значение = Р2Ст40Гр03.Значение Тогда
				...
				МассивРасшифровки.Добавить(Р1Ст1Гр02.ЗначениеСтр + " = " + Р2Ст40Гр03.ЗначениеСтр);
				Прервать;
			КонецЕсли;
		...
		Для Каждого Страница Из Раздел3 Цикл
			...
			Если Р1Ст1Гр02.Значение = Р3Ст80Гр03.Значение Тогда
				...
				МассивРасшифровки.Добавить(Р1Ст1Гр02.ЗначениеСтр + " = " + Р3Ст80Гр03.ЗначениеСтр);
				Прервать;
			КонецЕсли;
		...
		Для Каждого СтраницаР4 Из Раздел4 Цикл
			...
			Если Р1Ст1Гр02.Значение = Р4Ст40Гр03.Значение Тогда
				...
				МассивРасшифровки.Добавить(Р1Ст1Гр02.ЗначениеСтр + " = " + Р4Ст40Гр03.ЗначениеСтр);
				Прервать;
			КонецЕсли;
		...
		Для Каждого Страница Из Раздел5 Цикл
			...
			Если Р1Ст1Гр02.Значение = Р5Ст50Гр03.Значение Тогда
				...
				МассивРасшифровки.Добавить(Р1Ст1Гр02.ЗначениеСтр + " = " + Р5Ст50Гр03.ЗначениеСтр);
				Прервать;
			КонецЕсли;
		...
		Для Каждого СтраницаР7 Из Раздел7 Цикл
			...
			Если Р1Ст1Гр02.Значение = Р7Ст40Гр03.Значение Тогда
				...
				МассивРасшифровки.Добавить(Р1Ст1Гр02.ЗначениеСтр + " = " + Р7Ст40Гр03.ЗначениеСтр);
				Прервать;
			КонецЕсли;
			
		КонецЦикла;
		
		Для Каждого Страница Из Раздел8 Цикл
			ДанныеСтраницы = Страница.Данные;
			НомерСтраницыСтр = "";
			Если ДанныеСтраницы.Свойство("НомерСтраницы") Тогда
				НомерСтраницыСтр = ДанныеСтраницы.НомерСтраницы;
			Иначе
				НомерСтраницыСтр = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Раздел8.Индекс(Страница) + 1);
			КонецЕсли;
			
			ВставитьЗначениеПоказателя(ДанныеСтраницы, Р8Ст40Гр03);
			
			Если Р1Ст1Гр02.Значение = Р8Ст40Гр03.Значение Тогда
				ВнестиПоказательВРасшифровку(Р8Ст40Гр03, Р8Ст40Гр03.Раздел, НомерСтраницыМнУ(Страница.Представление, Р8Ст40Гр03.ПорядокСтраницы));
				МассивРасшифровки.Добавить(Р1Ст1Гр02.ЗначениеСтр + " = " + Р8Ст40Гр03.ЗначениеСтр);
				Прервать;
			КонецЕсли;
		...
		Если Р1Ст1Гр02.Значение = Р2Ст40Гр03.Значение
			...
			ИЛИ Р1Ст1Гр02.Значение = Р7Ст40Гр03.Значение
			ИЛИ Р1Ст1Гр02.Значение = Р8Ст40Гр03.Значение Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		...
		Если МассивРасшифровки.Количество() = 0 Тогда
			МассивРасшифровки.Добавить(Р1Ст1Гр02.ЗначениеСтр + " = ("""")");
		КонецЕсли;
		Расшифровка = СтрСоединить(МассивРасшифровки, "; ");
...
Процедура ПроверитьКС_2_1(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код вида добытого полезного ископаемого в строке 010 Раздела 2 должен быть равен ""03100""';
							|en = 'Код вида добытого полезного ископаемого в строке 010 Раздела 2 должен быть равен ""03100""'");
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан код вида полезных ископаемых в Разделе 2';
							|en = 'Неверно указан код вида полезных ископаемых в Разделе 2'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_2_2(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'КБК по строке 020 Раздела 2 должен быть равен КБК, указанному в строке 010 Раздела 1';
							|en = 'КБК по строке 020 Раздела 2 должен быть равен КБК, указанному в строке 010 Раздела 1'");
	
	ОписаниеОшибки  = НСтр("ru = 'Необходимо проверить правильность указания КБК в Разделе 2';
							|en = 'Необходимо проверить правильность указания КБК в Разделе 2'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_2_3(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'КБК по строке 020 Раздела 2 должен быть равен ""18210701011011000110"" или ""18210701040011000110""';
							|en = 'КБК по строке 020 Раздела 2 должен быть равен ""18210701011011000110"" или ""18210701040011000110""'");
	ОписаниеОшибки  = НСтр("ru = 'Необходимо проверить правильность указания КБК в Разделе 2';
							|en = 'Необходимо проверить правильность указания КБК в Разделе 2'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_2_4(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код единицы измерения в строке 030 Раздела 2 должен быть равен ""168""';
							|en = 'Код единицы измерения в строке 030 Раздела 2 должен быть равен ""168""'");
	ОписаниеОшибки  = НСтр("ru = 'Необходимо проверить правильность указания кода по ОКЕИ в Разделе 2';
							|en = 'Необходимо проверить правильность указания кода по ОКЕИ в Разделе 2'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_2_5(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код по ОКТМО в строке 040 Раздела 2.1 должен быть равен коду по ОКТМО в строке 020 Раздела 1';
							|en = 'Код по ОКТМО в строке 040 Раздела 2.1 должен быть равен коду по ОКТМО в строке 020 Раздела 1'");
	ОписаниеОшибки  = НСтр("ru = 'Необходимо проверить правильность указания ОКТМО в Разделе 2.1';
							|en = 'Необходимо проверить правильность указания ОКТМО в Разделе 2.1'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_2_6(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Коэффициент, характеризующий степень выработанности запасов участка недр Кв в строке 090 Раздела 2.1 должен быть равен 0.3, если Степень выработанности запасов нефти участка недр (значение строки 100 Раздела 2.1) > 1';
							|en = 'Коэффициент, характеризующий степень выработанности запасов участка недр Кв в строке 090 Раздела 2.1 должен быть равен 0.3, если Степень выработанности запасов нефти участка недр (значение строки 100 Раздела 2.1) > 1'");
	Нарушение       = НСтр("ru = 'ст.342.5 НК РФ';
							|en = 'ст.342.5 НК РФ'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Кв в Разделе 2.1';
							|en = 'Допущена ошибка при определении коэффициента Кв в Разделе 2.1'");
...
Процедура ПроверитьКС_2_7(СтруктураРезультата)
	...
	ОписаниеОшибки		= НСтр("ru = 'Допущена ошибка при определении показателя Дм в Разделе 2.1';
								|en = 'Допущена ошибка при определении показателя Дм в Разделе 2.1'");
	ШаблонРасшифровки	= НСтр("ru = '%1 = %2 х %3 х (1 - %4 х %5 х 1 х %6 х %7) - %8 Итог [%9]';
								|en = '%1 = %2 х %3 х (1 - %4 х %5 х 1 х %6 х %7) - %8 Итог [%9]'");
	Нарушение			= НСтр("ru = 'ст.342.5 НК РФ';
								|en = 'ст.342.5 НК РФ'");
...
Процедура ПроверитьКС_2_8(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Коэффициент, характеризующий степень выработанности запасов участка недр Кв в строке 090 Раздела 2.1 должен быть равен 1, если Степень выработанности запасов нефти участка недр (значение строки 100 Раздела 2.1) < 0,8';
							|en = 'Коэффициент, характеризующий степень выработанности запасов участка недр Кв в строке 090 Раздела 2.1 должен быть равен 1, если Степень выработанности запасов нефти участка недр (значение строки 100 Раздела 2.1) < 0,8'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Кв в Разделе 2.1';
							|en = 'Допущена ошибка при определении коэффициента Кв в Разделе 2.1'");
	Нарушение       = НСтр("ru = 'ст.342.5 НК РФ';
							|en = 'ст.342.5 НК РФ'");
...
Процедура ПроверитьКС_2_9(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Коэффициент, характеризующий регион добычи и свойства нефти (Ккан) в строке 120 Раздела 2.1 должен быть равен 0 или 1';
							|en = 'Коэффициент, характеризующий регион добычи и свойства нефти (Ккан) в строке 120 Раздела 2.1 должен быть равен 0 или 1'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Ккан в Разделе 2.1';
							|en = 'Допущена ошибка при определении коэффициента Ккан в Разделе 2.1'");
	Нарушение       = НСтр("ru = 'ст.342.5 НК РФ';
							|en = 'ст.342.5 НК РФ'");
...
Процедура ПроверитьКС_2_10(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Коэффициент, характеризующий регион добычи и свойства нефти (Ккан) в строке 120 Раздела 2.1 должен быть равен ""0"", если заполнена графа 2 Раздела 2.1.1, для которой значение графы 1 Раздела 2.1.1 = ""2001"" - ""2008"" или ""4001"" - ""4008""';
							|en = 'Коэффициент, характеризующий регион добычи и свойства нефти (Ккан) в строке 120 Раздела 2.1 должен быть равен ""0"", если заполнена графа 2 Раздела 2.1.1, для которой значение графы 1 Раздела 2.1.1 = ""2001"" - ""2008"" или ""4001"" - ""4008""'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Ккан в Разделе 2.1';
							|en = 'Допущена ошибка при определении коэффициента Ккан в Разделе 2.1'");
	Нарушение       = НСтр("ru = 'ст.342.5 НК РФ';
							|en = 'ст.342.5 НК РФ'");
...
Процедура ПроверитьКС_2_12(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Сумма налога к уплате в строке 150 Раздела 2.1 должна быть равна разнице показателей строк 130 и 140 Раздела 2.1';
							|en = 'Сумма налога к уплате в строке 150 Раздела 2.1 должна быть равна разнице показателей строк 130 и 140 Раздела 2.1'");
	Нарушение       = НСтр("ru = 'ст.342 НК РФ и ст.343.2 НК РФ';
							|en = 'ст.342 НК РФ и ст.343.2 НК РФ'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении суммы налога к уплате с учетом налогового вычета (возможна арифметическая ошибка)';
							|en = 'Допущена ошибка при определении суммы налога к уплате с учетом налогового вычета (возможна арифметическая ошибка)'");
...
Процедура ПроверитьКС_2_13(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Показатель, характеризующий особенность добычи нефти Дм по строке 210 Раздела 2.1.2 должен быть равен произведению строк 070 и 080 Раздела 2.1, если Количество нефти в графе 2 Раздела 2.1.1 имеет код основания (графа 1 Раздела 2.1.1 = ""2001"" и ""4001""';
							|en = 'Показатель, характеризующий особенность добычи нефти Дм по строке 210 Раздела 2.1.2 должен быть равен произведению строк 070 и 080 Раздела 2.1, если Количество нефти в графе 2 Раздела 2.1.1 имеет код основания (графа 1 Раздела 2.1.1 = ""2001"" и ""4001""'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении показателя Дм в Разделе 2.1.2';
							|en = 'Допущена ошибка при определении показателя Дм в Разделе 2.1.2'");
	Нарушение       = НСтр("ru = 'ст.342.5 НК РФ';
							|en = 'ст.342.5 НК РФ'");
...
Процедура ПроверитьКС_2_14(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Коэффициент, характеризующий степень сложности добычи нефти Кд по строке 230 Раздела 2.1.2 должен быть равен ""0,2"" или ""0,4"" или ""0,8"" или ""1""';
							|en = 'Коэффициент, характеризующий степень сложности добычи нефти Кд по строке 230 Раздела 2.1.2 должен быть равен ""0,2"" или ""0,4"" или ""0,8"" или ""1""'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении показателя Кд в Разделе 2.1.2';
							|en = 'Допущена ошибка при определении показателя Кд в Разделе 2.1.2'");
	Нарушение       = НСтр("ru = 'ст.342.5 НК РФ';
							|en = 'ст.342.5 НК РФ'");
...
Процедура ПроверитьКС_2_15(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Коэффициент, характеризующий степень выработанности запасов участка недр Кв по строке 090 Раздела 2.1 должен быть равен ""1"", если значение строки 230 Раздела 2.1.2 < 1';
							|en = 'Коэффициент, характеризующий степень выработанности запасов участка недр Кв по строке 090 Раздела 2.1 должен быть равен ""1"", если значение строки 230 Раздела 2.1.2 < 1'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении показателя Кв в Разделе 2.1.2';
							|en = 'Допущена ошибка при определении показателя Кв в Разделе 2.1.2'");
	Нарушение       = НСтр("ru = 'ст.342.5 НК РФ';
							|en = 'ст.342.5 НК РФ'");
...
Процедура ПроверитьКС_2_16(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Количество налоговых периодов применения коэффициента Кд<1 по строке 240 раздела 2.1.2 должно быть меньше или равен ""180"", если значение строки 230 Раздела 2.1.2 < 1';
							|en = 'Количество налоговых периодов применения коэффициента Кд<1 по строке 240 раздела 2.1.2 должно быть меньше или равен ""180"", если значение строки 230 Раздела 2.1.2 < 1'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении срока применения коэффициента Кд<1 в Разделе 2.1.2';
							|en = 'Допущена ошибка при определении срока применения коэффициента Кд<1 в Разделе 2.1.2'");
...
Процедура ПроверитьКС_2_17(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Коэффициент, характеризующий степень выработанности конкрентной залежи углеводородного сырья Кдв по строке 250 Раздела 2.1.2 должен быть равен ""1"", если значение строки 230 Раздела 2.1.2 < 1 и значение строки 220 Раздела 2.1.2 < 0,8';
							|en = 'Коэффициент, характеризующий степень выработанности конкрентной залежи углеводородного сырья Кдв по строке 250 Раздела 2.1.2 должен быть равен ""1"", если значение строки 230 Раздела 2.1.2 < 1 и значение строки 220 Раздела 2.1.2 < 0,8'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Кдв в Разделе 2.1.2';
							|en = 'Допущена ошибка при определении коэффициента Кдв в Разделе 2.1.2'");
	Нарушение       = НСтр("ru = 'ст.342.2 НК РФ';
							|en = 'ст.342.2 НК РФ'");
...
Процедура ПроверитьКС_2_18(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Коэффициент, характеризующий степень выработанности конкрентной залежи углеводородного сырья Кдв по строке 250 Раздела 2.1.2 должен быть равен ""0,3"", если значение строки 230 Раздела 2.1.2 < 1 и значение строки 220 Раздела 2.1.2 > 1';
							|en = 'Коэффициент, характеризующий степень выработанности конкрентной залежи углеводородного сырья Кдв по строке 250 Раздела 2.1.2 должен быть равен ""0,3"", если значение строки 230 Раздела 2.1.2 < 1 и значение строки 220 Раздела 2.1.2 > 1'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Кдв в Разделе 2.1.2';
							|en = 'Допущена ошибка при определении коэффициента Кдв в Разделе 2.1.2'");
	Нарушение       = НСтр("ru = 'ст.342.2 НК РФ';
							|en = 'ст.342.2 НК РФ'");
...
Процедура ПроверитьКС_2_19(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Количество налоговых периодов применения ставки налога 0 рублей в строке 260 Раздела 2.1.2 должно быть <=""180"", если Количество нефти в графе 2 Раздела 2.1.1 имеет код основания (графа 1 Раздела 2.1.1) = ""1084""';
							|en = 'Количество налоговых периодов применения ставки налога 0 рублей в строке 260 Раздела 2.1.2 должно быть <=""180"", если Количество нефти в графе 2 Раздела 2.1.1 имеет код основания (графа 1 Раздела 2.1.1) = ""1084""'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении количества налоговых периодов в Разделе 2.1.2';
							|en = 'Допущена ошибка при определении количества налоговых периодов в Разделе 2.1.2'");
...
Процедура ПроверитьКС_3_1(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код вида добытого полезного ископаемого по строке 010 Раздела 3 должен быть равен ""03200"" или ""03300""';
							|en = 'Код вида добытого полезного ископаемого по строке 010 Раздела 3 должен быть равен ""03200"" или ""03300""'");
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан код вида ПИ в Разделе 3';
							|en = 'Неверно указан код вида ПИ в Разделе 3'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_3_2(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'КБК по строке 020 Раздела 3 должен быть равен КБК в строке 010 Раздела 1';
							|en = 'КБК по строке 020 Раздела 3 должен быть равен КБК в строке 010 Раздела 1'");
	ОписаниеОшибки  = НСтр("ru = 'КБК, указанный в Разделе 3, отсутствует в Разделе 1. Либо неверно указан КБК в Разделе 3';
							|en = 'КБК, указанный в Разделе 3, отсутствует в Разделе 1. Либо неверно указан КБК в Разделе 3'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_3_3(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'КБК по строке 020 Раздела 3 должен быть равен ""18210701012011000110"" или ""18210701013011000110"" или ""18210701040011000110""';
							|en = 'КБК по строке 020 Раздела 3 должен быть равен ""18210701012011000110"" или ""18210701013011000110"" или ""18210701040011000110""'");
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан КБК в Разделе 3';
							|en = 'Неверно указан КБК в Разделе 3'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_3_4(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код единицы измерения в строке 030 Раздела 3 должен быть равен ""168"" или ""114""';
							|en = 'Код единицы измерения в строке 030 Раздела 3 должен быть равен ""168"" или ""114""'");
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан код по ОКЕИ в Разделе 3';
							|en = 'Неверно указан код по ОКЕИ в Разделе 3'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_3_5(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код по ОКТМО по строке 080 Раздела 3.1 должен быть равен ОКТМО в строке 020 Раздела 1';
							|en = 'Код по ОКТМО по строке 080 Раздела 3.1 должен быть равен ОКТМО в строке 020 Раздела 1'");
	ОписаниеОшибки  = НСтр("ru = 'ОКТМО, указанный в Разделе 3, отсутствует в Разделе 1. Либо неверно указан ОКТМО в Разделе 3';
							|en = 'ОКТМО, указанный в Разделе 3, отсутствует в Разделе 1. Либо неверно указан ОКТМО в Разделе 3'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_3_6(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Сумма исчисленного налога по строке 040 Раздела 3 должна быть равна сумме всех значений строки 190 Раздела 3.1.1';
							|en = 'Сумма исчисленного налога по строке 040 Раздела 3 должна быть равна сумме всех значений строки 190 Раздела 3.1.1'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении суммы исчисленного налога по участку недр (возможна арифметическая ошибка)';
							|en = 'Допущена ошибка при определении суммы исчисленного налога по участку недр (возможна арифметическая ошибка)'");
	Нарушение       = НСтр("ru = 'ст.342 НК РФ и ст.342.4 НК РФ';
							|en = 'ст.342 НК РФ и ст.342.4 НК РФ'");
...
Процедура ПроверитьКС_3_7(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Коэффициент, характеризующий долю реализации газа Ов по строке 060 Раздела 3 должен быть равен ""0,64"" или ""1""';
							|en = 'Коэффициент, характеризующий долю реализации газа Ов по строке 060 Раздела 3 должен быть равен ""0,64"" или ""1""'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Ов';
							|en = 'Допущена ошибка при определении коэффициента Ов'");
...
Процедура ПроверитьКС_3_8(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Коэффициент, характеризующий долю добытого газа по строке 070 Раздела 3 должен быть равен (35 х сумма всех значений графы 2 Раздела 3.1.1 для Кода вида добытого полезного ископаемого = ""03300"" (строка 010 Раздела 3) и Код единицы измерения в строке 030 Раздела 3 = ""114"") / (35 х (сумма всех значений графы 2 Раздела 3.1.1 для Кода вида добытого полезного ископаемого = ""03300"" (строка 010 Раздела 3) и Код единицы измерения в строке 030 Раздела 3 = ""114"" + Сумма всех значений графы 2 Раздела 5.1 для Кода вида добытого полезного ископаемого = ""03400"" (строка 010 Раздела 5) и Код единицы измерения в строке 030 Раздела 5 = ""114"") + (42 х (сумма всех значений графы 2 Раздела 2.1.1 и графы 2 Раздела 2.1.2 для Кода вида добытого полезного ископаемого = ""03100"" (строка 010 Раздела 2) и Код единицы измерения в строке 030 Раздела 2 = ""168"" + сумма всех значений графы 2 Раздела 3.1.1 для Кода вида добытого полезного ископаемого = ""03200"" (строка 010 Раздела 3) и Код единицы измерения в строке 030 Раздела 3 = ""168""';
							|en = 'Коэффициент, характеризующий долю добытого газа по строке 070 Раздела 3 должен быть равен (35 х сумма всех значений графы 2 Раздела 3.1.1 для Кода вида добытого полезного ископаемого = ""03300"" (строка 010 Раздела 3) и Код единицы измерения в строке 030 Раздела 3 = ""114"") / (35 х (сумма всех значений графы 2 Раздела 3.1.1 для Кода вида добытого полезного ископаемого = ""03300"" (строка 010 Раздела 3) и Код единицы измерения в строке 030 Раздела 3 = ""114"" + Сумма всех значений графы 2 Раздела 5.1 для Кода вида добытого полезного ископаемого = ""03400"" (строка 010 Раздела 5) и Код единицы измерения в строке 030 Раздела 5 = ""114"") + (42 х (сумма всех значений графы 2 Раздела 2.1.1 и графы 2 Раздела 2.1.2 для Кода вида добытого полезного ископаемого = ""03100"" (строка 010 Раздела 2) и Код единицы измерения в строке 030 Раздела 2 = ""168"" + сумма всех значений графы 2 Раздела 3.1.1 для Кода вида добытого полезного ископаемого = ""03200"" (строка 010 Раздела 3) и Код единицы измерения в строке 030 Раздела 3 = ""168""'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Кгпн (возможна арифметическая ошибка)';
							|en = 'Допущена ошибка при определении коэффициента Кгпн (возможна арифметическая ошибка)'");
	Нарушение       = НСтр("ru = 'ст.342.4 НК РФ';
							|en = 'ст.342.4 НК РФ'");
...
Процедура ПроверитьКС_3_9(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Коэффициент Дг по строке 130 Раздела 3.1 должен быть равен (35 х сумма всех значений графы 2 Раздела 3.1.1 для кода вида добытого полезного ископаемого = ""03300"" (строка 010 Раздела 3) и код единицы измерения в строке 030 Раздела 3 = ""114"") / (35 х сумма всех значений графы 2 Раздела 3.1.1 для кода вида добытого полезного ископаемого = ""03300"" (строка 010 Раздела 3) и код единицы измерения в строке 030 Раздела 3 = ""114"") + (42 х сумма всех значений графы 2 Раздела 3.1.1 для кода вида добытого полезного ископаемого = ""03200"" (строка 010 Раздела 3) и код единицы измерения в строке 030 Раздела 3 = ""168"")';
							|en = 'Коэффициент Дг по строке 130 Раздела 3.1 должен быть равен (35 х сумма всех значений графы 2 Раздела 3.1.1 для кода вида добытого полезного ископаемого = ""03300"" (строка 010 Раздела 3) и код единицы измерения в строке 030 Раздела 3 = ""114"") / (35 х сумма всех значений графы 2 Раздела 3.1.1 для кода вида добытого полезного ископаемого = ""03300"" (строка 010 Раздела 3) и код единицы измерения в строке 030 Раздела 3 = ""114"") + (42 х сумма всех значений графы 2 Раздела 3.1.1 для кода вида добытого полезного ископаемого = ""03200"" (строка 010 Раздела 3) и код единицы измерения в строке 030 Раздела 3 = ""168"")'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Дг (возможна арифметическая ошибка)';
							|en = 'Допущена ошибка при определении коэффициента Дг (возможна арифметическая ошибка)'");
	Нарушение       = НСтр("ru = 'ст.342.4 НК РФ';
							|en = 'ст.342.4 НК РФ'");
...
Процедура ПроверитьКС_3_10(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Значение коэффициента Квг (строка 170 Раздела 3.1.1) должно быть равно 2,75 - 2,5 х строка 100 Раздела 3.1, если значение строки 100 Раздела 3.1 > 0,7 и <= 0,9';
							|en = 'Значение коэффициента Квг (строка 170 Раздела 3.1.1) должно быть равно 2,75 - 2,5 х строка 100 Раздела 3.1, если значение строки 100 Раздела 3.1 > 0,7 и <= 0,9'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Квг (возможна арифметическая ошибка)';
							|en = 'Допущена ошибка при определении коэффициента Квг (возможна арифметическая ошибка)'");
	Нарушение       = НСтр("ru = 'ст.342.4 НК РФ';
							|en = 'ст.342.4 НК РФ'");
...
Процедура ПроверитьКС_3_11(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Значение коэффициента Квг (строка 170 Раздела 3.1.1) должно быть равно ""0,5"", если значение строки 100 Раздела 3.1 > 0,9';
							|en = 'Значение коэффициента Квг (строка 170 Раздела 3.1.1) должно быть равно ""0,5"", если значение строки 100 Раздела 3.1 > 0,9'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Квг (возможна арифметическая ошибка)';
							|en = 'Допущена ошибка при определении коэффициента Квг (возможна арифметическая ошибка)'");
	Нарушение       = НСтр("ru = 'ст.342.4 НК РФ';
							|en = 'ст.342.4 НК РФ'");
...
Процедура ПроверитьКС_3_12(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Значение коэффициента Квг (строка 170 Раздела 3.1.1) должно быть равно ""1"", если значение строки 100 Раздела 3.1 <= 0,7';
							|en = 'Значение коэффициента Квг (строка 170 Раздела 3.1.1) должно быть равно ""1"", если значение строки 100 Раздела 3.1 <= 0,7'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Квг (возможна арифметическая ошибка)';
							|en = 'Допущена ошибка при определении коэффициента Квг (возможна арифметическая ошибка)'");
	Нарушение       = НСтр("ru = 'ст.342.4 НК РФ';
							|en = 'ст.342.4 НК РФ'");
...
Процедура ПроверитьКС_3_13(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Значение коэффициента Кгз (строка 170 Раздела 3.1.1) должно быть равно ""1"", если значение строки 150 Раздела 3.1.1 <= 1700';
							|en = 'Значение коэффициента Кгз (строка 170 Раздела 3.1.1) должно быть равно ""1"", если значение строки 150 Раздела 3.1.1 <= 1700'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Кгз (возможна арифметическая ошибка)';
							|en = 'Допущена ошибка при определении коэффициента Кгз (возможна арифметическая ошибка)'");
	Нарушение       = НСтр("ru = 'ст.342.4 НК РФ';
							|en = 'ст.342.4 НК РФ'");
...
Процедура ПроверитьКС_3_14(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Значение коэффициента Кгз (строка 170 Раздела 3.1.1) должно быть равно ""0,64"", если значение строки 150 Раздела 3.1.1 > 1700 и <= 3300';
							|en = 'Значение коэффициента Кгз (строка 170 Раздела 3.1.1) должно быть равно ""0,64"", если значение строки 150 Раздела 3.1.1 > 1700 и <= 3300'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Кгз (возможна арифметическая ошибка)';
							|en = 'Допущена ошибка при определении коэффициента Кгз (возможна арифметическая ошибка)'");
	Нарушение       = НСтр("ru = 'ст.342.4 НК РФ';
							|en = 'ст.342.4 НК РФ'");
...
Процедура ПроверитьКС_3_15(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Значение коэффициента Кгз (строка 170 Раздела 3.1.1) должно быть равно ""0,5"", если значение строки 150 Раздела 3.1.1 > 3300';
							|en = 'Значение коэффициента Кгз (строка 170 Раздела 3.1.1) должно быть равно ""0,5"", если значение строки 150 Раздела 3.1.1 > 3300'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Кгз (возможна арифметическая ошибка)';
							|en = 'Допущена ошибка при определении коэффициента Кгз (возможна арифметическая ошибка)'");
	Нарушение       = НСтр("ru = 'ст.342.4 НК РФ';
							|en = 'ст.342.4 НК РФ'");
...
Процедура ПроверитьКС_3_16(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Коэффициент, характеризующий сложность добычи газа горючего природного и(или) газового конденсата Кс по строке 180 Раздела 3.1.1 должен быть равен минимальному значению строки 170 Раздела 3.1.1 (min из Квг, Кр, Кгз, Кас, Корз)';
							|en = 'Коэффициент, характеризующий сложность добычи газа горючего природного и(или) газового конденсата Кс по строке 180 Раздела 3.1.1 должен быть равен минимальному значению строки 170 Раздела 3.1.1 (min из Квг, Кр, Кгз, Кас, Корз)'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Кс (возможна арифметическая ошибка)';
							|en = 'Допущена ошибка при определении коэффициента Кс (возможна арифметическая ошибка)'");
	Нарушение       = НСтр("ru = 'ст.342.4 НК РФ';
							|en = 'ст.342.4 НК РФ'");
...
Процедура ПроверитьКС_4_1(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код вида полезного ископаемого по строке 010 Раздела 4 должен быть равен ""03100"" или ""03200"" или ""03300"" или ""03401""';
							|en = 'Код вида полезного ископаемого по строке 010 Раздела 4 должен быть равен ""03100"" или ""03200"" или ""03300"" или ""03401""'");
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан код вида ПИ в Разделе 4';
							|en = 'Неверно указан код вида ПИ в Разделе 4'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
	...
	Для Каждого СтраницаРаздела Из Раздел4 Цикл
		ДанныеСтраницы = СтраницаРаздела.Данные[0].Значение;
		НомерСтраницыСтр = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Раздел4.Индекс(СтраницаРаздела) + 1);
...
Процедура ПроверитьКС_4_2(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'КБК по строке 020 Раздела 4 должен быть равен КБК в строке 010 Раздела 1';
							|en = 'КБК по строке 020 Раздела 4 должен быть равен КБК в строке 010 Раздела 1'");
	ОписаниеОшибки  = НСтр("ru = 'КБК, указанный в Разделе 4, отсутствует в Разделе 1. Либо неверно указан КБК в Разделе 4';
							|en = 'КБК, указанный в Разделе 4, отсутствует в Разделе 1. Либо неверно указан КБК в Разделе 4'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");	
...
Процедура ПроверитьКС_4_3(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'КБК по строке 020 Раздела 4 должен быть равен ""18210701011011000110"" или ""18210701012011000110"" или ""18210701013011000110"" или ""00000000000000000000"" (если строка 010 Раздела 4 = ""03401"")';
							|en = 'КБК по строке 020 Раздела 4 должен быть равен ""18210701011011000110"" или ""18210701012011000110"" или ""18210701013011000110"" или ""00000000000000000000"" (если строка 010 Раздела 4 = ""03401"")'");
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан КБК в Разделе 4';
							|en = 'Неверно указан КБК в Разделе 4'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");	
...
Процедура ПроверитьКС_4_4(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код единицы измерения в строке 030 Раздела 4 должен быть равен ""168"" или ""114"" или ""113""';
							|en = 'Код единицы измерения в строке 030 Раздела 4 должен быть равен ""168"" или ""114"" или ""113""'");
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан код по ОКЕИ в Разделе 4';
							|en = 'Неверно указан код по ОКЕИ в Разделе 4'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");	
...
Процедура ПроверитьКС_4_5(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код по ОКТМО по строке 040 Раздела 4 должен быть равен ОКТМО в строке 020 Раздела 1';
							|en = 'Код по ОКТМО по строке 040 Раздела 4 должен быть равен ОКТМО в строке 020 Раздела 1'");
	ОписаниеОшибки  = НСтр("ru = 'ОКТМО, указанный в Разделе 4, отсутствует в Разделе 1. Либо неверно указан ОКТМО в Разделе 4';
							|en = 'ОКТМО, указанный в Разделе 4, отсутствует в Разделе 1. Либо неверно указан ОКТМО в Разделе 4'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");	
...
Процедура ПроверитьКС_4_6(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Налоговая база по строке 130 Раздела 4 должна быть равна сумме значений по графам 2 раздела 4 с кодом основания по графе 1 Раздела 4 = ""7000""-""7006"" х Минимальную стоимость единицы полезного ископаемого по строке 090 раздела 4, если Стоимость единицы полезного ископаемого по строке 080 Раздела 4 < Минимальной стоимости единицы полезного ископаемого по строке 090 раздела 4 или значение Стоимости единицы полезного ископаемого по строке 080 Раздела 4 не заполнено';
							|en = 'Налоговая база по строке 130 Раздела 4 должна быть равна сумме значений по графам 2 раздела 4 с кодом основания по графе 1 Раздела 4 = ""7000""-""7006"" х Минимальную стоимость единицы полезного ископаемого по строке 090 раздела 4, если Стоимость единицы полезного ископаемого по строке 080 Раздела 4 < Минимальной стоимости единицы полезного ископаемого по строке 090 раздела 4 или значение Стоимости единицы полезного ископаемого по строке 080 Раздела 4 не заполнено'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении налоговой базы (возможна арифметическая ошибка) в Разделе 4';
							|en = 'Допущена ошибка при определении налоговой базы (возможна арифметическая ошибка) в Разделе 4'");
	Нарушение       = НСтр("ru = 'ст.340 и 341.1 НК РФ';
							|en = 'ст.340 и 341.1 НК РФ'");
...
Процедура ПроверитьКС_4_7(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Налоговая база по строке 130 Раздела 4 должна быть равна нулю, если Код вида полезного ископаемого в строке 010 Раздела 4 = ""03401""';
							|en = 'Налоговая база по строке 130 Раздела 4 должна быть равна нулю, если Код вида полезного ископаемого в строке 010 Раздела 4 = ""03401""'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении налоговой базы (возможна арифметическая ошибка) в Разделе 4';
							|en = 'Допущена ошибка при определении налоговой базы (возможна арифметическая ошибка) в Разделе 4'");
	Нарушение       = НСтр("ru = 'ст.340 и 341.1 НК РФ';
							|en = 'ст.340 и 341.1 НК РФ'");
...
Процедура ПроверитьКС_4_8(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Сумма исчисленного налога по строке 140 Раздела 4 должна быть равна нулю, если Код вида полезного ископаемого в строке 010 Раздела 4 = ""03401""';
							|en = 'Сумма исчисленного налога по строке 140 Раздела 4 должна быть равна нулю, если Код вида полезного ископаемого в строке 010 Раздела 4 = ""03401""'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении налоговой базы (возможна арифметическая ошибка) в Разделе 4';
							|en = 'Допущена ошибка при определении налоговой базы (возможна арифметическая ошибка) в Разделе 4'");
	Нарушение       = НСтр("ru = 'ст.340 и 341.1 НК РФ';
							|en = 'ст.340 и 341.1 НК РФ'");
...
Процедура ПроверитьКС_5_1(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код вида добытого полезного ископаемого по строке 010 Раздела 5 должен быть отличен от ""03100"" или ""03200"" или ""03401"" или ""01100"" или ""01150"" или ""01300"" или ""01350""';
							|en = 'Код вида добытого полезного ископаемого по строке 010 Раздела 5 должен быть отличен от ""03100"" или ""03200"" или ""03401"" или ""01100"" или ""01150"" или ""01300"" или ""01350""'");
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан код вида ПИ в Разделе 5';
							|en = 'Неверно указан код вида ПИ в Разделе 5'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_5_2(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'КБК по строке 020 Раздела 5 должен быть равен КБК в строке 010 Раздела 1';
							|en = 'КБК по строке 020 Раздела 5 должен быть равен КБК в строке 010 Раздела 1'");
	ОписаниеОшибки  = НСтр("ru = 'КБК, указанный в Разделе 5, отсутствует в Разделе 1. Либо неверно указан КБК в Разделе 5';
							|en = 'КБК, указанный в Разделе 5, отсутствует в Разделе 1. Либо неверно указан КБК в Разделе 5'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_5_3(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'КБК по строке 020 Раздела 5 должен быть равен ""00000000000000000000"" если строка 010 Раздела 5 = ""03400""';
							|en = 'КБК по строке 020 Раздела 5 должен быть равен ""00000000000000000000"" если строка 010 Раздела 5 = ""03400""'");
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан КБК в Разделе 5';
							|en = 'Неверно указан КБК в Разделе 5'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_5_4(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'КБК по строке 020 Раздела 5 должен быть отличен от ""18210701011011000110"" или ""18210701012011000110"" или ""18210701013011000110"" или ""18210701060011000110""';
							|en = 'КБК по строке 020 Раздела 5 должен быть отличен от ""18210701011011000110"" или ""18210701012011000110"" или ""18210701013011000110"" или ""18210701060011000110""'");
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан КБК в Разделе 5';
							|en = 'Неверно указан КБК в Разделе 5'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_5_5(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Налоговая ставка в строке 040 Раздела 5 должна быть равна 0 или 3,8 или 4,0 или 4,8 или 5,5 или 6,0 или 6,5 или 7,5 или 8,0';
							|en = 'Налоговая ставка в строке 040 Раздела 5 должна быть равна 0 или 3,8 или 4,0 или 4,8 или 5,5 или 6,0 или 6,5 или 7,5 или 8,0'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении налоговой ставки в Разделе 5';
							|en = 'Допущена ошибка при определении налоговой ставки в Разделе 5'");
	Нарушение       = НСтр("ru = 'ст.342 НК РФ';
							|en = 'ст.342 НК РФ'");
...
Процедура ПроверитьКС_5_6(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код по ОКТМО по строке 050 Раздела 5 должен быть равен ОКТМО в строке 020 Раздела 1';
							|en = 'Код по ОКТМО по строке 050 Раздела 5 должен быть равен ОКТМО в строке 020 Раздела 1'");
	ОписаниеОшибки  = НСтр("ru = 'ОКТМО, указанный в Разделе 5, отсутствует в Разделе 1. Либо неверно указан ОКТМО в Разделе 5';
							|en = 'ОКТМО, указанный в Разделе 5, отсутствует в Разделе 1. Либо неверно указан ОКТМО в Разделе 5'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
...
Процедура ПроверитьКС_5_7(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Стоимость единицы добытого полезного ископаемого по строке 070 Раздела 5.2 должна быть равна (Выручка по строке 100 раздела 5.2 - Расходы по строке 120 Раздела 5.2) / Количество реализованного добытого полезного ископаемого по строке 090 Раздела 5.2, если Код вида добытого полезного ископаемого по строке 010 раздела 5.2 не равно ""13001"" - ""13008"" или ""11301"" - ""11310"" или ""12001"" - ""12007"" и строка 110 раздела 5.2 не заполнена';
							|en = 'Стоимость единицы добытого полезного ископаемого по строке 070 Раздела 5.2 должна быть равна (Выручка по строке 100 раздела 5.2 - Расходы по строке 120 Раздела 5.2) / Количество реализованного добытого полезного ископаемого по строке 090 Раздела 5.2, если Код вида добытого полезного ископаемого по строке 010 раздела 5.2 не равно ""13001"" - ""13008"" или ""11301"" - ""11310"" или ""12001"" - ""12007"" и строка 110 раздела 5.2 не заполнена'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении стоимости единицы полезного ископаемого (возможна арифметическая ошибка) в Разделе 5.2';
							|en = 'Допущена ошибка при определении стоимости единицы полезного ископаемого (возможна арифметическая ошибка) в Разделе 5.2'");
...
Процедура ПроверитьКС_5_8(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Стоимость единицы добытого полезного ископаемого по строке 070 Раздела 5.2 должна быть равна Доля содержания по строке 080 раздела 5.2 х (Выручка по строке 100 раздела 5.2 - Расходы по строке 120 Раздела 5.2 - расходы по аффинажу по строке 130 раздела 5.2) / Количество реализованного добытого полезного ископаемого по строке 090 Раздела 5.2, если Код вида добытого полезного ископаемого по строке 010 раздела 5.2 равен ""13001"" - ""13008"" и строка 110 раздела 5.2 не заполнена';
							|en = 'Стоимость единицы добытого полезного ископаемого по строке 070 Раздела 5.2 должна быть равна Доля содержания по строке 080 раздела 5.2 х (Выручка по строке 100 раздела 5.2 - Расходы по строке 120 Раздела 5.2 - расходы по аффинажу по строке 130 раздела 5.2) / Количество реализованного добытого полезного ископаемого по строке 090 Раздела 5.2, если Код вида добытого полезного ископаемого по строке 010 раздела 5.2 равен ""13001"" - ""13008"" и строка 110 раздела 5.2 не заполнена'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении стоимости единицы полезного ископаемого (возможна арифметическая ошибка) в Разделе 5.2';
							|en = 'Допущена ошибка при определении стоимости единицы полезного ископаемого (возможна арифметическая ошибка) в Разделе 5.2'");
	Нарушение       = НСтр("ru = 'ст.340 НК РФ';
							|en = 'ст.340 НК РФ'");
...
Процедура ПроверитьКС_5_9(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Налоговая база по строке 140 Раздела 5.2 должна быть равна сумме значений графы 2 раздела 5.1 с кодом основания по графе 1 Раздела 5.1 = ""2000"" или ""4000"" х Стоимость единицы добытого полезного ископаемого по строке 070 раздела 5.2';
							|en = 'Налоговая база по строке 140 Раздела 5.2 должна быть равна сумме значений графы 2 раздела 5.1 с кодом основания по графе 1 Раздела 5.1 = ""2000"" или ""4000"" х Стоимость единицы добытого полезного ископаемого по строке 070 раздела 5.2'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении налоговой базы (возможна арифметическая ошибка) в Разделе 5.2';
							|en = 'Допущена ошибка при определении налоговой базы (возможна арифметическая ошибка) в Разделе 5.2'");
	Нарушение       = НСтр("ru = 'ст.340 НК РФ';
							|en = 'ст.340 НК РФ'");
...
Процедура ПроверитьКС_6_1(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Сумма прямых расходов по строке 040 Раздела 6.1 должна быть равна сумме Прямых расходов по строке 010 Раздела 6.1 и Остатка незавершенного производства на начало периода по строке 020 Раздела 6.1 за минусом Остатка незавершенного производства на конец периода по строке 030 Раздела 6.1';
							|en = 'Сумма прямых расходов по строке 040 Раздела 6.1 должна быть равна сумме Прямых расходов по строке 010 Раздела 6.1 и Остатка незавершенного производства на начало периода по строке 020 Раздела 6.1 за минусом Остатка незавершенного производства на конец периода по строке 030 Раздела 6.1'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении суммы прямых расходов, относящихся к добытым полезным ископаемым';
							|en = 'Допущена ошибка при определении суммы прямых расходов, относящихся к добытым полезным ископаемым'");
...
Процедура ПроверитьКС_6_2(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Сумма косвенных и иных расходов по строке 090 раздела 6.1 должна быть равна сумме Внереализационных расходов по строке 050 Раздела 6.1, Косвенных расходов по строке 060 Раздела 6.1, Косвенных расходов по строке 070 раздела 6.1 * Общую сумму прямых расходов по строке 080 Раздела 6.1';
							|en = 'Сумма косвенных и иных расходов по строке 090 раздела 6.1 должна быть равна сумме Внереализационных расходов по строке 050 Раздела 6.1, Косвенных расходов по строке 060 Раздела 6.1, Косвенных расходов по строке 070 раздела 6.1 * Общую сумму прямых расходов по строке 080 Раздела 6.1'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении суммы косвенных и иных расходов, относящихся к добытым полезным ископаемым';
							|en = 'Допущена ошибка при определении суммы косвенных и иных расходов, относящихся к добытым полезным ископаемым'");
	Нарушение       = НСтр("ru = 'п.4 ст.340 НК РФ';
							|en = 'п.4 ст.340 НК РФ'");
...
Процедура ПроверитьКС_6_3(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Общая сумма расходов по строке 100 Раздела 6.1 должна быть равна сумме прямых расходов по строке 040 Раздела 6.1 и сумме косвенных и иных расходов по строке 090 Раздела 6.1';
							|en = 'Общая сумма расходов по строке 100 Раздела 6.1 должна быть равна сумме прямых расходов по строке 040 Раздела 6.1 и сумме косвенных и иных расходов по строке 090 Раздела 6.1'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении общей суммы расходов по добыче полезных ископаемых';
							|en = 'Допущена ошибка при определении общей суммы расходов по добыче полезных ископаемых'");
	Нарушение       = НСтр("ru = 'п.4 ст.340 НК РФ';
							|en = 'п.4 ст.340 НК РФ'");
...
Процедура ПроверитьКС_6_4(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Общая сумма по графе 3 Раздела 6.2 должна быть равна 1';
							|en = 'Общая сумма по графе 3 Раздела 6.2 должна быть равна 1'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении доли добытого полезного ископаемого в общем количестве добытых полезных ископаемых';
							|en = 'Допущена ошибка при определении доли добытого полезного ископаемого в общем количестве добытых полезных ископаемых'");
	Нарушение       = НСтр("ru = 'ст.340 НК РФ';
							|en = 'ст.340 НК РФ'");
...
Процедура ПроверитьКС_6_5(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Сумма расходов по добыче полезного ископаемого в графе 4 Раздела 6.2 должна быть равна произведению Общей суммы расходов по строке 100 Раздела 6.1 и соответствующей строки графы 3 Раздела 6.2, если значение рассчитываемой строки графы 2 Раздела 6.2 заполнено и отлично от нуля';
							|en = 'Сумма расходов по добыче полезного ископаемого в графе 4 Раздела 6.2 должна быть равна произведению Общей суммы расходов по строке 100 Раздела 6.1 и соответствующей строки графы 3 Раздела 6.2, если значение рассчитываемой строки графы 2 Раздела 6.2 заполнено и отлично от нуля'");
	ОписаниеОшибки  = НСтр("ru = 'Ошибка при определении расходов по добыче полезного ископаемого';
							|en = 'Ошибка при определении расходов по добыче полезного ископаемого'");
	Нарушение       = НСтр("ru = 'ст.340 НК РФ';
							|en = 'ст.340 НК РФ'");
...
Процедура ПроверитьКС_6_6(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Сумма расходов по графе 2 Раздела 6.2 должна быть равна Количеству добытого полезного ископаемого в графе 2 Раздела 5.1, если значение в графе 2 Раздела 6.2 заполнено и не равно нулю и Код добытого полезного ископаемого в соответствующей графе 1 Раздела 6.2 должен быть равен Коду вида в строке 010 Раздела 5';
							|en = 'Сумма расходов по графе 2 Раздела 6.2 должна быть равна Количеству добытого полезного ископаемого в графе 2 Раздела 5.1, если значение в графе 2 Раздела 6.2 заполнено и не равно нулю и Код добытого полезного ископаемого в соответствующей графе 1 Раздела 6.2 должен быть равен Коду вида в строке 010 Раздела 5'");
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при опредлении количества добытого полезного ископаемого';
							|en = 'Допущена ошибка при опредлении количества добытого полезного ископаемого'");
...
Процедура ПроверитьКС_7_1(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'КБК по строке 010 Раздела 7 должен быть равен КБК, указанному в строке 010 Раздела 1';
							|en = 'КБК по строке 010 Раздела 7 должен быть равен КБК, указанному в строке 010 Раздела 1'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");	
	ОписаниеОшибки  = НСтр("ru = 'КБК, указанный в Разделе 7, отсутствует в Разделе 1. Либо неверно указан КБК в Разделе 7';
							|en = 'КБК, указанный в Разделе 7, отсутствует в Разделе 1. Либо неверно указан КБК в Разделе 7'");
	...
	Для Каждого СтраницаРаздела Из Раздел7 Цикл
		ДанныеСтраницы = СтраницаРаздела.Данные[0].Значение;
		НомерСтраницыСтр = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Раздел7.Индекс(СтраницаРаздела) + 1);
...
Процедура ПроверитьКС_7_10(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Остаток неучтенных при определении налогового вычета расходов по строке 160 Раздела 7.2 должен быть равен Остаток неучтенных при определении налогового вычета расходов на начало налогового периода по строке 060 Раздела 7.2 + Сумма расходов по строке 070 Раздела 7.2 - Сумма расходов по строке 110 Раздела 7.2';
							|en = 'Остаток неучтенных при определении налогового вычета расходов по строке 160 Раздела 7.2 должен быть равен Остаток неучтенных при определении налогового вычета расходов на начало налогового периода по строке 060 Раздела 7.2 + Сумма расходов по строке 070 Раздела 7.2 - Сумма расходов по строке 110 Раздела 7.2'");
	...
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении остатка неучтенных при определении налогового вычета расходов';
							|en = 'Допущена ошибка при определении остатка неучтенных при определении налогового вычета расходов'");
...
Процедура ПроверитьКС_7_11(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Раздел 7 (КС 7.11). Остаток суммы налогового вычета на конец налогового периода по строке 210 должен быть равен Сумме налогового вычета по строке 180 Раздела 7.3 за вычетом Суммы налогового вычета по строкам 190 и 200 Раздела 7.3';
							|en = 'Раздел 7 (КС 7.11). Остаток суммы налогового вычета на конец налогового периода по строке 210 должен быть равен Сумме налогового вычета по строке 180 Раздела 7.3 за вычетом Суммы налогового вычета по строкам 190 и 200 Раздела 7.3'");
	Нарушение       = НСтр("ru = 'п.4 ст.343.1 НК РФ';
							|en = 'п.4 ст.343.1 НК РФ'");	
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении суммы расходов, включаемых в налоговый вычет';
							|en = 'Допущена ошибка при определении суммы расходов, включаемых в налоговый вычет'");
...
Процедура ПроверитьКС_7_2(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код единицы измерения в строке 030 Раздела 7 должен быть равен ""168""';
							|en = 'Код единицы измерения в строке 030 Раздела 7 должен быть равен ""168""'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");	
	ОписаниеОшибки  = НСтр("ru = 'Необходимо проверить правильность указания кода по ОКЕИ в Разделе 7';
							|en = 'Необходимо проверить правильность указания кода по ОКЕИ в Разделе 7'");
...
Процедура ПроверитьКС_7_3(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код по ОКТМО в строке 040 Раздела 7 должен быть равен коду по ОКТМО в строке 020 Раздела 1';
							|en = 'Код по ОКТМО в строке 040 Раздела 7 должен быть равен коду по ОКТМО в строке 020 Раздела 1'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");	
	ОписаниеОшибки  = НСтр("ru = 'ОКТМО, указанный в Разделе 7, отсутствует в Разделе 1. Либо неверно указан ОКТМО в Разделе 7';
							|en = 'ОКТМО, указанный в Разделе 7, отсутствует в Разделе 1. Либо неверно указан ОКТМО в Разделе 7'");
...
Процедура ПроверитьКС_7_4(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Код вида добытого полезного ископаемого в графе 1 Раздела 7.1 должен быть равен ""01100"" или ""01150"" или ""01300"" или ""01350""';
							|en = 'Код вида добытого полезного ископаемого в графе 1 Раздела 7.1 должен быть равен ""01100"" или ""01150"" или ""01300"" или ""01350""'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");	
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан код вида полезных ископаемых в Разделе 7.1';
							|en = 'Неверно указан код вида полезных ископаемых в Разделе 7.1'");
...
Процедура ПроверитьКС_7_5(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Сумма расходов в строке 070 Раздела 7.2 должна быть равна сумме расходов по строкам 080, 090 и 100 Раздела 7.2';
							|en = 'Сумма расходов в строке 070 Раздела 7.2 должна быть равна сумме расходов по строкам 080, 090 и 100 Раздела 7.2'");
	Нарушение       = НСтр("ru = 'ст.343.1 НК РФ';
							|en = 'ст.343.1 НК РФ'");	
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении суммы расходов, связанных с обеспечением безопасных условий и охраны труда';
							|en = 'Допущена ошибка при определении суммы расходов, связанных с обеспечением безопасных условий и охраны труда'");
...
Процедура ПроверитьКС_7_6(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Сумма расходов в строке 110 Раздела 7.2 должна быть равна Остаток неучтенных при определении налогового вычета расходов на начало налогового периода по строке 060 Раздела 7.2 + Сумма расходов по строке 070 Раздела 7.2, если результат суммирования <= Предельной величине налогового вычета по строке 140 Раздела 7.2';
							|en = 'Сумма расходов в строке 110 Раздела 7.2 должна быть равна Остаток неучтенных при определении налогового вычета расходов на начало налогового периода по строке 060 Раздела 7.2 + Сумма расходов по строке 070 Раздела 7.2, если результат суммирования <= Предельной величине налогового вычета по строке 140 Раздела 7.2'");
	Нарушение       = НСтр("ru = 'ст.343.1 НК РФ';
							|en = 'ст.343.1 НК РФ'");	
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении налогового вычета';
							|en = 'Допущена ошибка при определении налогового вычета'");
...
Процедура ПроверитьКС_7_7(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Значение коэффициента Кт в строке 120 Раздела 7.2 должно быть <= 0,3';
							|en = 'Значение коэффициента Кт в строке 120 Раздела 7.2 должно быть <= 0,3'");
	Нарушение       = НСтр("ru = 'п.3 ст.343.1 НК РФ';
							|en = 'п.3 ст.343.1 НК РФ'");	
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении коэффициента Кт';
							|en = 'Допущена ошибка при определении коэффициента Кт'");
...
Процедура ПроверитьКС_7_8(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Предельная величина налогового вычета по строке 140 Раздела 7.2 должна быть равна произведению Коэффициента Кт по строке 120 Раздела 7.2 и Суммы исчисленного налога по строке 130 Раздела 7.2';
							|en = 'Предельная величина налогового вычета по строке 140 Раздела 7.2 должна быть равна произведению Коэффициента Кт по строке 120 Раздела 7.2 и Суммы исчисленного налога по строке 130 Раздела 7.2'");
	Нарушение       = НСтр("ru = 'ст.343.1 НК РФ';
							|en = 'ст.343.1 НК РФ'");	
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении предельной величины налогового вычета';
							|en = 'Допущена ошибка при определении предельной величины налогового вычета'");
...
Процедура ПроверитьКС_7_9(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru = 'Сумма налога исчисленного по строке 150 Раздела 7.2 должна быть равна Сумма исчисленного налога по строке 130 Раздела 7.2 - Сумма расходов по строке 110 Раздела 7.2';
							|en = 'Сумма налога исчисленного по строке 150 Раздела 7.2 должна быть равна Сумма исчисленного налога по строке 130 Раздела 7.2 - Сумма расходов по строке 110 Раздела 7.2'");
	Нарушение       = НСтр("ru = 'п.3 ст.343.1 НК РФ';
							|en = 'п.3 ст.343.1 НК РФ'");	
	ОписаниеОшибки  = НСтр("ru = 'Допущена ошибка при определении суммы налога с учетом налогового вычета';
							|en = 'Допущена ошибка при определении суммы налога с учетом налогового вычета'");
...
&НаСервере
Процедура ПроверитьКС_8_1(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы    = НСтр("ru = 'Раздел 8 (КС 8.1)';
							|en = 'Раздел 8 (КС 8.1)'");
	ОписаниеКС      = НСтр("ru = 'Код вида добытого полезного ископаемого в строке 010 Раздела 8 должен быть равен ""03100""';
							|en = 'Код вида добытого полезного ископаемого в строке 010 Раздела 8 должен быть равен ""03100""'");
	ОписаниеОшибки  = НСтр("ru = 'Неверно указан код вида полезных ископаемых в Разделе 8';
							|en = 'Неверно указан код вида полезных ископаемых в Разделе 8'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
	
	Р8Ст10Гр03 = ПоказательПоСтроке("Р8Ст10Гр03");
	Раздел8 = ДанныеИзДереваРаздела("Раздел8");
	
	Для Каждого СтраницаРаздела Из Раздел8 Цикл
		ДанныеСтраницыР8 = СтраницаРаздела.Данные;
		НомерСтраницыСтр = "";
		Если ДанныеСтраницыР8.Свойство("НомерСтраницы") Тогда
			НомерСтраницыСтр = ДанныеСтраницыР8.НомерСтраницы;
		Иначе
			НомерСтраницыСтр = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Раздел8.Индекс(СтраницаРаздела) + 1);
		КонецЕсли;
		
		ВставитьЗначениеПоказателя(ДанныеСтраницыР8, Р8Ст10Гр03);
		Если НЕ ЗначениеЗаполнено(Р8Ст10Гр03.Значение) Тогда
			Продолжить;
		КонецЕсли;
		СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
		ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
		
		ВнестиПоказательВРасшифровку(Р8Ст10Гр03, Р8Ст10Гр03.Раздел, НомерСтраницыМнУ(СтраницаРаздела.Представление, Р8Ст10Гр03.ПорядокСтраницы));
		
		КС = ПоказателиКС(Р8Ст10Гр03.Значение, "03100");
		Расшифровка = КС.ЛеваяЧастьСтр + " = " + КС.ПраваяЧастьСтр;
...
Процедура ПроверитьКС_8_2(СтруктураРезультата)
	...
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	ТекстФормулы    = НСтр("ru = 'Раздел 8 (КС 8.2)';
							|en = 'Раздел 8 (КС 8.2)'");
	ОписаниеКС      = НСтр("ru = 'КБК по строке 020 Раздела 8 должен быть равен КБК, указанному в строке 010 Раздела 1';
							|en = 'КБК по строке 020 Раздела 8 должен быть равен КБК, указанному в строке 010 Раздела 1'");
	
	ОписаниеОшибки  = НСтр("ru = 'Необходимо проверить правильность указания КБК в Разделе 8';
							|en = 'Необходимо проверить правильность указания КБК в Разделе 8'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
	...
	Для Каждого СтраницаРаздела Из Раздел8 Цикл
		...
		ВставитьЗначениеПоказателя(ДанныеСтраницыР8, Р8Ст20Гр03);
		Если НЕ ЗначениеЗаполнено(Р8Ст20Гр03.Значение) Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Инд = 1 По КолвоСтрокВМногострочнойЧасти Цикл
			...
			Если Р8Ст20Гр03.Значение = Р1Ст1Гр01.Значение Тогда
				ВнестиПоказательВРасшифровку(Р1Ст1Гр01, Р1Ст1Гр01.Раздел, );
				СоотношениеВыполнено = Истина;
				Прервать;
			КонецЕсли;
		...
		Если СоотношениеВыполнено Тогда
			Расшифровка = Р8Ст20Гр03.ЗначениеСтр + " = " + Р1Ст1Гр01.ЗначениеСтр;
			
		Иначе
			Расшифровка = Р8Ст20Гр03.ЗначениеСтр + " = [нет соответствующего КБК]";
...
&НаСервере
Процедура ПроверитьКС_8_3(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы    = НСтр("ru = 'Раздел 8 (КС 8.3)';
							|en = 'Раздел 8 (КС 8.3)'");
	ОписаниеКС      = НСтр("ru = 'Код единицы измерения в строке 030 Раздела 8 должен быть равен ""168""';
							|en = 'Код единицы измерения в строке 030 Раздела 8 должен быть равен ""168""'");
	ОписаниеОшибки  = НСтр("ru = 'Необходимо проверить правильность указания кода по ОКЕИ в Разделе 8';
							|en = 'Необходимо проверить правильность указания кода по ОКЕИ в Разделе 8'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
	
	Р8Ст30Гр03 = ПоказательПоСтроке("Р8Ст30Гр03");
	Раздел8 = ДанныеИзДереваРаздела("Раздел8");
	...
	Для Каждого СтраницаРаздела Из Раздел8 Цикл
		...
		ВставитьЗначениеПоказателя(ДанныеСтраницы, Р8Ст30Гр03);
		ВнестиПоказательВРасшифровку(Р8Ст30Гр03, Р8Ст30Гр03.Раздел, НомерСтраницыМнУ(СтраницаРаздела.Представление, Р8Ст30Гр03.ПорядокСтраницы));
		
		КС = ПоказателиКС(Р8Ст30Гр03.Значение, "168");
		Расшифровка = ПредставлениеДляРасшифровки(КС.ЛеваяЧастьСтр) + " = " + КС.ПраваяЧастьСтр;
...
&НаСервере
Процедура ПроверитьКС_8_4(СтруктураРезультата)
	
	ОбщееЧислоОшибок    = СтруктураРезультата.КоличествоНайденныхОшибок;
	ОбщееЧислоБезОшибок = СтруктураРезультата.КоличествоНайденныхБезОшибок;
	СтруктураОшибок     = СтруктураРезультата.СтруктураОшибок;
	
	ТекстФормулы    = НСтр("ru = 'Раздел 8 (КС 8.4)';
							|en = 'Раздел 8 (КС 8.4)'");
	ОписаниеКС      = НСтр("ru = 'Код по ОКТМО по строке 040 Раздела 8 должен быть равен ОКТМО в строке 020 Раздела 1';
							|en = 'Код по ОКТМО по строке 040 Раздела 8 должен быть равен ОКТМО в строке 020 Раздела 1'");
	ОписаниеОшибки  = НСтр("ru = 'ОКТМО, указанный в Разделе 8, отсутствует в Разделе 1. Либо неверно указан ОКТМО в Разделе 8';
							|en = 'ОКТМО, указанный в Разделе 8, отсутствует в Разделе 1. Либо неверно указан ОКТМО в Разделе 8'");
	Нарушение       = НСтр("ru = 'ст.80 НК РФ';
							|en = 'ст.80 НК РФ'");
	...
	Р1Ст2Гр01 = ПоказательПоСтроке("Р1Ст2Гр01");
	Раздел1 = мДанныеОтчета.Раздел1;
	Раздел8 = ДанныеМногоуровнегоРаздела("Раздел8");
	
	КолвоСтрокВМногострочнойЧасти = КоличествоСтрок(Раздел1, "П000100000102_");
	
	//Определяем уровень обхода
	УровеньОбхода = ?(Р8Ст40Гр03.Свойство("ПорядокСтраницы"), Р8Ст40Гр03.ПорядокСтраницы, 0);
	ДанныеРаздела = Раздел8.Скопировать();
	ДанныеРаздела.Свернуть(КолонкиДляСворачивания(УровеньОбхода) + Р8Ст40Гр03.Имя);
	
	Для Каждого СтрокаДанных Из ДанныеРаздела Цикл
		ВставитьЗначениеПоказателя(СтрокаДанных, Р8Ст40Гр03);		
		Если НЕ ЗначениеЗаполнено(Р8Ст40Гр03.Значение) Тогда
			Продолжить;
		КонецЕсли;
		...
		ВнестиПоказательВРасшифровку(Р8Ст40Гр03, Р8Ст40Гр03.Раздел, НомерСтраницыПоКолонкам(СтрокаДанных));
		
		КСВыполнено = Ложь;
		Для Инд = 1 По КолвоСтрокВМногострочнойЧасти Цикл
			ВставитьЗначениеМногострочногоПоказателя(Раздел1, Р1Ст2Гр01, Инд);
			
			Если Р8Ст40Гр03.Значение = Р1Ст2Гр01.Значение Тогда
				КСВыполнено = Истина;
				Прервать;
		...
		Расшифровка = Р8Ст40Гр03.ЗначениеСтр + " = " + ?(КСВыполнено, Р1Ст2Гр01.ЗначениеСтр, "[...]") ;
...
Функция СуммаПодУровня(УровеньСтраницы, Раздел, Параметр, Результат = Неопределено)
	...
	Для Каждого СтраницаРаздела Из Раздел Цикл
		...
		Если ДанныеСтраницы.Свойство("НомерСтраницы") Тогда
			НомерСтраницыСтр = ДанныеСтраницы.НомерСтраницы;
		Иначе
			НомерСтраницыСтр = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Раздел.Индекс(СтраницаРаздела) + 1);
		КонецЕсли;
...
Функция ДанныеНумерации(НомСтрокиВДереве, НомСтрокиВВетке2, НомСтрокиВВетке21 = "")
	...
	Разделитель = ",";
	НомерСтраницы = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомСтрокиВДереве);
	Если НомСтрокиВВетке2  <> "" Тогда
		НомерСтраницы = НомерСтраницы + Разделитель + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомСтрокиВВетке2);
	КонецЕсли;
	Если НомСтрокиВВетке21  <> "" Тогда
		НомерСтраницы = НомерСтраницы + Разделитель + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомСтрокиВВетке21);
	КонецЕсли;
...
Функция НомерСтраницыПоКолонкам(СтрокаДанных, ПорядокСтраницы = Неопределено)
	...
	Для Каждого Колонка Из КолонкиСтроки Цикл
		...
		Если Лев(ИмяКолонки, 7) = "НомМнУ1" Тогда
			НомМнУ1 = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(СтрокаДанных[ИмяКолонки]);
		ИначеЕсли Лев(ИмяКолонки, 7) = "НомМнУ2" Тогда
			НомМнУ2 = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(СтрокаДанных[ИмяКолонки]);
		ИначеЕсли Лев(ИмяКолонки, 7) = "НомМнУ3" Тогда
			НомМнУ3 = РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(СтрокаДанных[ИмяКолонки]);
		КонецЕсли;
...
Функция НомерСтраницы(НомСтрокиВДереве, НомСтрокиВВетке2, НомСтрокиВВетке21 = "")
	...
	Разделитель = ",";
	Возврат РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомСтрокиВДереве) + Разделитель + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомСтрокиВВетке2) + Разделитель + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(НомСтрокиВВетке21);
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Ст30Гр03" Тогда
		...
		Результат.Вставить("Представление", Нстр("ru = 'Код единицы измерения количества добытого полезного ископаемого по ОКЕИ';
												|en = 'Код единицы измерения количества добытого полезного ископаемого по ОКЕИ'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р73Ст1Гр05" Тогда
		...
		Результат.Вставить("Представление", Нстр("ru = 'Остаток суммы налогового вычета на конец налогового периода (стр.180 - стр.190 - стр.200)';
												|en = 'Остаток суммы налогового вычета на конец налогового периода (стр.180 - стр.190 - стр.200)'"));
		//Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р73Ст1Гр02" Тогда
		...
		Результат.Вставить("Представление", Нстр("ru = 'Сумма налогового вычета, образовавшаяся на месяц и год, указанные в строке 170';
												|en = 'Сумма налогового вычета, образовавшаяся на месяц и год, указанные в строке 170'"));
		//Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р73Ст1Гр03" Тогда
		...
		Результат.Вставить("Представление", Нстр("ru = 'Сумма налогового вычета, использованная в предыдущие налоговые периоды';
												|en = 'Сумма налогового вычета, использованная в предыдущие налоговые периоды'"));
		//Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р73Ст1Гр04" Тогда
		...
		Результат.Вставить("Представление", Нстр("ru = 'Сумма налогового вычета, использованная в налоговом периоде';
												|en = 'Сумма налогового вычета, использованная в налоговом периоде'"));
		//Результат.Вставить("ИзменяемаяРазмерность", Истина);
	
	ИначеЕсли УсловныйКодПоказателя = "Р8Ст10Гр03" Тогда
		Результат.Вставить("Имя", "П000800001003");
		Результат.Вставить("Представление", Нстр("ru = 'Код вида добытого полезного ископаемого';
												|en = 'Код вида добытого полезного ископаемого'"));
	
	ИначеЕсли УсловныйКодПоказателя = "Р8Ст20Гр03" Тогда
		Результат.Вставить("Имя", "П000800002003");
		Результат.Вставить("Представление", Нстр("ru = 'Код бюджетной классификации';
												|en = 'Код бюджетной классификации'"));
	
	ИначеЕсли УсловныйКодПоказателя = "Р8Ст30Гр03" Тогда
		Результат.Вставить("Имя", "П000800003003");
		Результат.Вставить("Представление", Нстр("ru = 'Код единицы измерения количества добытого полезного ископаемого';
												|en = 'Код единицы измерения количества добытого полезного ископаемого'"));
	
	ИначеЕсли УсловныйКодПоказателя = "Р8Ст40Гр03" Тогда
		Результат.Вставить("Имя", "П000800004003");
		Результат.Вставить("Представление", Нстр("ru = 'Код по ОКТМО';
												|en = 'Код по ОКТМО'"));
	
	КонецЕсли;
	
	Результат.Представление = ПредставлениеПоказателя(Результат.Имя) + ?(ЗначениеЗаполнено(Результат.Представление), 
...
Функция ПредставлениеПоказателя(ИмяПоказателя)
	...
	Раздел.Вставить("П0008", "Раздела 8");
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Графа %1 строка %2 %3';
																		|en = 'Графа %1 строка %2 %3'"), ПодстрокаГрафы,
...
Процедура ДобавитьКординаты(Результат)
	...
	ИначеЕсли СтрНайти("П000800001003|П000800002003|П000800003003", ИмяПоказателя) <> 0 Тогда
		Результат.Вставить("Раздел", "Раздел8");
		Результат.Вставить("ПорядокСтраницы", 1);
	
	ИначеЕсли СтрНайти("П000800004003", ИмяПоказателя) <> 0 Тогда
		Результат.Вставить("Раздел", "Раздел81");
		Результат.Вставить("ПорядокСтраницы", 2);
...
Функция КоличествоСтрок(Данные, Показатель)
	Инд = 1; Количество = 0;
	Пока Данные.Свойство(Показатель + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Инд)) Цикл
...
Процедура СобратьМассивМногострочныхПоказателей(МассивПоказателей, Раздел, Показатель);
	Инд = 1;
	Пока Раздел.Свойство(Показатель + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Инд)) Цикл
		ЗначениеПоказателя = Раздел[Показатель + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Инд)];
...
Функция КолонкиДляСворачивания(УровеньОбхода)
	...
	Для Сч = 1 По УровеньОбхода Цикл
		Результат = Результат + "НомМнУ" + РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(Сч) + ","; 
	КонецЦикла;
	Возврат Результат;	
...
Процедура СуммироватьМнЧ(Раздел, Параметр, УсловияСуммирования)
	...
	Параметр.Вставить("ЗначениеСтр", РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(СуммаМнЧ));
...
Процедура СуммироватьМнС(Раздел, Параметр, УсловияСуммирования = Неопределено)
	...
	Параметр.Вставить("ЗначениеСтр", РегламентированнаяОтчетностьКлиентСервер.СтрЧГ0(СуммаМнC));
