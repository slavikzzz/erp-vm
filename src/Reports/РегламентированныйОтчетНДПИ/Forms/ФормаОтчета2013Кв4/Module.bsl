...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Раздел41",  ПрефиксИменМакетов + "Раздел41");
	СоответствиеРазделовМакетам.Вставить("Раздел42",  ПрефиксИменМакетов + "Раздел42");
	СоответствиеРазделовМакетам.Вставить("Раздел43",  ПрефиксИменМакетов + "Раздел43");
...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	Для Каждого Показатель Из Показатели Цикл
		Если СтрНайти(Показатель.Имя, "П000100001002") = 1
		 ИЛИ СтрНайти(Показатель.Имя, "П000200010010") = 1
		 ИЛИ СтрНайти(Показатель.Имя, "П000410004001") = 1 Тогда
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Пока Раздел1.Свойство("П000100001001_" + Формат(НомерСтроки, "ЧГ=")) Цикл
		...
		ИмяЯчейкиОКТМОРаздел1 = "П000100001002_" + Формат(НомерСтроки, "ЧГ=");
		...
		Если НЕ ЗначениеЗаполнено(КБКРаздел1) И НЕ ЗначениеЗаполнено(ОКТМОРаздел1) И НЕ ЗначениеЗаполнено(СуммаРаздел1) Тогда
		...
		Если ДлинаОКТМОРаздел1 <> 11 И ДлинаОКТМОРаздел1 <> 8 Тогда	
			СообщитьВТС("В разделе 1 в строке " + НомерСтроки + " не" + ?(ДлинаОКТМОРаздел1 = 0, "", "верно") + " задан код по ОКТМО!", "Раздел1", ИмяЯчейкиОКТМОРаздел1);
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		ЗаполненХотяБыОдинИзБлоковРаздела1 = ЗаполненХотяБыОдинИзБлоковРаздела1 ИЛИ ЗначениеЗаполнено(КБКРаздел1) ИЛИ ЗначениеЗаполнено(ОКТМОРаздел1);
	...
	Для Каждого Стр Из Раздел2 Цикл
		...
		Пока ДанныеСтраницы.Свойство("П000200010010_" + Формат(НомерСтроки, "ЧГ=")) Цикл
			...
			ОКТМОРаздел2МнчКод = "П000200010010_" + Формат(НомерСтроки, "ЧГ=");
			...
			ОКТМОРаздел2Мнч = СокрЛП(ДанныеСтраницы[ОКТМОРаздел2МнчКод]);
			...
			Если НЕ ЗначениеЗаполнено(ОКТМОРаздел2Мнч)
			...
			ОКТМОРаздел2МнчДлина = СтрДлина(ОКТМОРаздел2Мнч);
			Если ОКТМОРаздел2МнчДлина <> 11 И ОКТМОРаздел2МнчДлина <> 8 Тогда
				СообщитьВТС("В разделе 2 на странице " + НомерСтраницы + " в строке " + НомерСтроки + " не" + ?(ОКТМОРаздел2МнчДлина = 0, "", "верно") + " задан код по ОКТМО!", "Раздел2", ОКТМОРаздел2МнчКод, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
	...
	Для Каждого СтрРаздел41 Из Раздел41 Цикл
		...
		Пока ДанныеСтраницыРаздел41.Свойство("П000410004001_" + Формат(НомерСтроки, "ЧГ=")) Цикл
			...
			ОКТМОКод = "П000410004001_" + НомерСтрокиСтр;
			...
			ОКТМО = СокрЛП(ДанныеСтраницыРаздел41[ОКТМОКод]);
			...
			Если ОКТМОДлина <> 11 И ОКТМОДлина <> 8 Тогда
				СообщитьВТС("В разделе 4.1 на странице " + НомерСтраницы + " в строке " + НомерСтроки + " не" + ?(ОКТМОДлина = 0, "", "верно") + " задан код по ОКТМО!", "Раздел41", ОКТМОКод, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	ИначеЕсли ИмяТекРаздела = "Раздел2"
		...
		Если ТабличныйДокумент.Области.Найти("УдалитьСтроку" + ИмяГруппыДопСтрок + "01_2") = Неопределено Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Если ТабличныйДокумент.Области.Найти("УдалитьСтроку" + ИмяГруппыДопСтрок + "10_2") = Неопределено Тогда
			Обл = ТабличныйДокумент.Области.Найти("УдалитьСтроку" + ИмяГруппыДопСтрок + "10_1");
			Если Обл <> Неопределено Тогда 
				Обл.Гиперссылка = Ложь;
				Обл.Текст = "";
			КонецЕсли;
...
Процедура ДобавитьСтраницу(Команда)
	...
	Для Каждого КЗ Из мСтруктураМногострочныхРазделов Цикл 
		...
		Если Обл <> Неопределено Тогда 
			...
			Обл.Текст = "";
		КонецЕсли;
		Обл = ТабличныйДокумент.Области.Найти("УдалитьСтроку" + КЗ.Ключ + "10_1");
		Если Обл <> Неопределено Тогда 
			Обл.Гиперссылка = Ложь;
			Обл.Текст = "";
		КонецЕсли;
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	Пока Истина Цикл 
		Инд = Инд + 1;
		Обл = ТекТабличноеПоле.Области.Найти("УдалитьСтроку" + ИмяГруппы + "10_" + Инд);
		Если Обл = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		Обл.Гиперссылка = Истина;
...
Процедура ПроверкаКС_1_2(СтруктураРезультата)
	...
	Пока мДанныеОтчета.Раздел1.Свойство("П000100001002_" + НомерСтроки, ЗначениеСтроки) Цикл
		...
		ОписаниеКС = "Код по ОКТМО в строке 020 Раздела 1 должен соответствовать коду по ОКТМО, указанному в графе 1 Раздела 2 или в графе 1 Раздела 4";
		СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
		...
		РегламентированнаяОтчетность.ЗарегистрироватьПоказательДляРасшифровки(СписокРасшифровкиПоказателей, ИмяПоказателяМнЧ,
			"Код по ОКТМО Раздела 1 по доп. строке " + НомерСтроки, ЛеваяЧасть, "", СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка, "Раздел1",
		...
		Расшифровка = """" + ЛеваяЧасть + """" + " = " + ?(ПустаяСтрока(ПраваяЧасть), """" """", """" + ПраваяЧасть + """");
		Описание = "Необходимо убедиться в правильности заполнения кода по ОКТМО в Разделе 2 и Разделе 4";
		Нарушение = "ст. 80 НК";
	...
	Для каждого СтраницаР2 Из мДанныеОтчета.Раздел2 Цикл
		...
		Пока СтраницаР2.Данные[0].Значение.Свойство(ИмяПоказателяР2 + НомерСтрокиР2, ЗначениеПоказателяР2) Цикл
			...
			Если ПустаяСтрока(ЗначениеПоказателяР2) Тогда 
				...
				Продолжить;
			КонецЕсли;
			ЛеваяЧасть = СокрЛП(ЗначениеПоказателяР2);
			
			ОписаниеКС = "ОКТМО, указанный в разделе 2, должен присутствовать в разделе 1.";
			СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
			РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, , 
				СчетчикФормулДляРасшифровок);
				
			РегламентированнаяОтчетность.ЗарегистрироватьПоказательДляРасшифровки(СписокРасшифровкиПоказателей,
				ИмяПоказателяР2 + НомерСтрокиР2, "Код по ОКТМО. Раздел 2, Строка 020, Страница " + НомерСтраницы, ЛеваяЧасть, "",
			...
			Расшифровка = """" + ЛеваяЧасть + """" + " = " + ?(ПустаяСтрока(ПраваяЧасть), """" """", """" + ПраваяЧасть + """");
			Описание = "ОКТМО, указанный в разделе 2, отсутствует в разделе 1. Либо неверно указан ОКТМО в разделе 2.";
			Нарушение = "ст. 80 НК";
...
Процедура ПроверкаКС_4_2(СтруктураРезультата)
	...
	Для каждого СтраницаР41 Из мДанныеОтчета.Раздел41 Цикл
		...
		НаименованиеПоказателяР41 = "Код по ОКТМО. Раздел 4, Строка № ";
		Пока СтраницаР41.Данные[0].Значение.Свойство(ИмяПоказателяР41 + НомерСтрокиР41, ЗначениеПоказателяР41) Цикл
			...
			ОписаниеКС = "Код по ОКТМО в графе 1 Раздела 4 должен соответствовать коду по ОКТМО, указанному в строке 020 Раздела 1";
			СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
			РегламентированнаяОтчетность.ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ОписаниеКС, , 
				СчетчикФормулДляРасшифровок);
			...
			РегламентированнаяОтчетность.ЗарегистрироватьПоказательДляРасшифровки(СписокРасшифровкиПоказателей,
				...
				"Код по ОКТМО. Раздел 4.1, Строка № " + НомерСтрокиР41 + ", Страница " + НомерСтраницы, ЛеваяЧасть, "",
				СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка, "Раздел41", Неопределено, НомерСтраницы,
				СчетчикФормулДляРасшифровок);
			...
			ЗначениеПоказателяР1 = Неопределено;
			ИмяПоказателяР1 = "П000100001002_";
			НаименованиеПоказателяР1 = "Код по ОКТМО. Раздел 1, Строка № ";
			Пока мДанныеОтчета.Раздел1.Свойство(ИмяПоказателяР1 + НомерСтрокиР1, ЗначениеПоказателяР1) Цикл
			...
			Расшифровка = """" + ЛеваяЧасть + """" + " = " + ?(ПустаяСтрока(ПраваяЧасть), """" """", """" + ПраваяЧасть + """");
			Описание = "Необходимо убедиться в правильности заполнения кода по ОКТМО в Разделе 1 и Разделе 4";
			Нарушение = "ст. 80 НК";
...
Процедура ДобавитьЭлементВСтруктуруОшибок(СтруктураОшибок, ТекстФормулы, ОписаниеКС, ОбщееЧислоОшибок, ОбщееЧислоБезОшибок, Расшифровка, ЭтоОшибка, Описание = Неопределено, Нарушение = Неопределено)
	...
	СтруктураОшибок.Вставить("ТекстовоеПредставлениеФормулы" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1. %2';
																																													|en = '%1. %2'"), ТекстФормулы, ОписаниеКС));
