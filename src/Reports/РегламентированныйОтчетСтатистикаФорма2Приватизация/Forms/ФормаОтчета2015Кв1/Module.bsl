...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если Не ЗначениеЗаполнено(ПараметрыВыгрузки.П000001000102) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
	...
	Если Не ЗначениеЗаполнено(ПараметрыВыгрузки.П000001000402) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
	...
	Если Не ЗначениеЗаполнено(ПараметрыВыгрузки.П000001000502) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
	...
	Если Не ЗначениеЗаполнено(ПараметрыВыгрузки.П000001000602) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
	...
	Если (ПараметрыВыгрузки.П000001000402 <> "11" И ПараметрыВыгрузки.П000001000402 <> "12"
			И ПараметрыВыгрузки.П000001000402 <> "13" И ПараметрыВыгрузки.П000001000402 <> "14") Тогда 
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "В разделе I гр.2 стр. 4 = кодам по ОКФС 11, 12, 13, 14",
			ИмяПоляТаблДокумента, "П000001000402");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если (ПараметрыВыгрузки.П000001000602 <> "10" И ПараметрыВыгрузки.П000001000602 <> "11") Тогда 
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "В разделе I гр.2 стр. 6 = 10 или 11",
			ИмяПоляТаблДокумента, "П000001000602");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ((ПараметрыВыгрузки.П000002000304 <= 0 И  ПараметрыВыгрузки.П000002000404 > 0)
		Или (ПараметрыВыгрузки.П000002000304 > 0 И  ПараметрыВыгрузки.П000002000404 <= 0)) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Если в разделе II стр.03 > 0, то стр.04 > 0 и наоборот",
			ИмяПоляТаблДокумента, "П000002000304");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПараметрыВыгрузки.П000001000602 = "10" И
		(ПараметрыВыгрузки.П000002000604 <= 0 Или ПараметрыВыгрузки.П000002000704 <= 0
		Или ПараметрыВыгрузки.П000002000804 <> 0 Или ПараметрыВыгрузки.П000002000904 <> 0
		Или (ПараметрыВыгрузки.П000002001004 <> 0 И ПараметрыВыгрузки.П000002001004 <> 1)) Тогда 
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Если в разделе I гр.2 стр. 6 = 10, то в разделе II стр. 06, 07 > 0 и стр.10 = 0  либо =1, при этом стр. 08, 09 - не заполняются",
			ИмяПоляТаблДокумента, "П000002000604");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПараметрыВыгрузки.П000001000602 = "10" И
		(ПараметрыВыгрузки.П000002000104 <> ПараметрыВыгрузки.П000002000604 * ПараметрыВыгрузки.П000002000704) Тогда 
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Если в разделе I гр.2 стр. 6 = 10, то в разделе II стр.01 = стр. 06 х стр.07",
			ИмяПоляТаблДокумента, "П000002000604");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПараметрыВыгрузки.П000001000602 = "11" И
		(ПараметрыВыгрузки.П000002000604 <> 0 Или ПараметрыВыгрузки.П000002000704 <> 0
		Или ПараметрыВыгрузки.П000002000804 <= 0 Или ПараметрыВыгрузки.П000002000904 <= 0) Тогда 
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Если в разделе I гр.2 стр. 6 = 11, то в разделе II стр.08, 09 > 0, при этом стр. 06, 07 - не заполняются",
			ИмяПоляТаблДокумента, "П000002000604");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПараметрыВыгрузки.П000001000602 = "11" И
		(ПараметрыВыгрузки.П000002000104 * ПараметрыВыгрузки.П000002000804 <> ПараметрыВыгрузки.П000002000904 * 100) Тогда 
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, "Если в разделе I гр.2 стр. 6 = 11, то в разделе II стр.01 = 100*(стр.09/стр.08)",
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("ПривНомВРеестре", ПараметрыВыгрузки["П000001000102"]);
	ПараметрыВыгрузки.Вставить("ПривОКВЭД",       ПараметрыВыгрузки["П000001000502"]);
	ПараметрыВыгрузки.Вставить("ПривОКФС",        ПараметрыВыгрузки["П000001000402"]);
	ПараметрыВыгрузки.Вставить("ПривКодСпособа",  ПараметрыВыгрузки["П000001000602"]);
