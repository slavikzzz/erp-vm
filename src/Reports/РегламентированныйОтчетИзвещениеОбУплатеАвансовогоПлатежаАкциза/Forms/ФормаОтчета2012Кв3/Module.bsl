...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТабличныйДокумент.Вывести(ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаОтчета2012Кв3_Извещение"));
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.изв.упл.аванс.алк.") Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ИмяФормы).ПолучитьМакет("Списки2012Кв3");
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2022 Тогда
		ИмяСекцииКодаВидаСпирта    = "КодВидаСпирта_20201201";
		ИмяСекцииКодаВидаПродукции = "КодВидаПродукции_20220101";
	ИначеЕсли СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '20201201' Тогда
		ИмяСекцииКодаВидаСпирта    = "КодВидаСпирта_20201201";
		ИмяСекцииКодаВидаПродукции = "КодВидаПродукции_20201201";
	ИначеЕсли Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2020 Тогда
		ИмяСекцииКодаВидаСпирта    = "КодВидаСпирта_20200101";
		ИмяСекцииКодаВидаПродукции = "КодВидаПродукции_20200101";
	Иначе
		ИмяСекцииКодаВидаСпирта    = "КодВидаСпирта";
		ИмяСекцииКодаВидаПродукции = "КодВидаПродукции";
	КонецЕсли;
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000100000102", 3, ,
	"Выбор кода вида спирта", КоллекцияСписковВыбора[ИмяСекцииКодаВидаСпирта]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000100000104", 3, ,
	"Выбор кода вида алкогольной/спиртосодержащей продукции", КоллекцияСписковВыбора[ИмяСекцииКодаВидаПродукции]);
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы",             "NO_IZUPLAKAL_1_121_00_05_02_01.xsd"); // Приведено справочно
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = ОбъектОтчета(ИмяФормы).ПолучитьМакет("СтруктураXML502");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Пока ПТД_Извещение.Свойство("П000100000101_" + НомерСтрокиСтр) Цикл
		...
		ИмяЯчейкиКодВидаСпирта = "П000100000102_" + НомерСтрокиСтр;
		ИмяЯчейкиОбъемСпирта = "П000100000103_" + НомерСтрокиСтр;
		ИмяЯчейкиКодВидаПродукции = "П000100000104_" + НомерСтрокиСтр;
		ИмяЯчейкиСуммаАвПлатежа = "П000100000105_" + НомерСтрокиСтр;
		ИмяЯчейкиНомерПлатежногоПоручения = "П000100000106_" + НомерСтрокиСтр;
		ИмяЯчейкиДатаплатежногоПоручениеия = "П000100000107_" + НомерСтрокиСтр;
		
		КодВидаСпирта = СокрЛП(ПТД_Извещение[ИмяЯчейкиКодВидаСпирта]);
		ОбъемСпирта = ПТД_Извещение[ИмяЯчейкиОбъемСпирта];
		КодВидаПродукции = СокрЛП(ПТД_Извещение[ИмяЯчейкиКодВидаПродукции]);
		СуммаАвПлатежа = ПТД_Извещение[ИмяЯчейкиСуммаАвПлатежа];
		НомерПП = СокрЛП(ПТД_Извещение[ИмяЯчейкиНомерПлатежногоПоручения]);
		ДатаПП = ПТД_Извещение[ИмяЯчейкиДатаплатежногоПоручениеия];
		
		СтрокаЗаполнена = ЗначениеЗаполнено(КодВидаСпирта) 
		              ИЛИ ОбъемСпирта <> 0 
		              ИЛИ ЗначениеЗаполнено(КодВидаПродукции)
		              ИЛИ СуммаАвПлатежа <> 0 
		              ИЛИ ЗначениеЗаполнено(НомерПП)
		              ИЛИ ЗначениеЗаполнено(ДатаПП);
		
		Если СтрокаЗаполнена Тогда
			ВсеПоказателиЗаполнены = ЗначениеЗаполнено(КодВидаСпирта) 
			                       И ОбъемСпирта <> 0 
			                       И ЗначениеЗаполнено(КодВидаПродукции)
			                       И СуммаАвПлатежа <> 0 
			                       И ЗначениеЗаполнено(НомерПП)
			                       И ЗначениеЗаполнено(ДатаПП);
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	НомерСтроки = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Извещение, "П000100000101_" + Формат(НомерСтроки, "ЧГ=")) Цикл
		
		ЧислоВыгруженныхСтрок = ЧислоВыгруженныхСтрок + 1;
		УзелСвАвансы = СкопироватьУзел(УзелРодитель, ШаблонУзелСвУплАванс);
		ВыгрузитьСтрокуМногострочнойЧасти(Извещение, УзелСвАвансы, НомерСтроки);
