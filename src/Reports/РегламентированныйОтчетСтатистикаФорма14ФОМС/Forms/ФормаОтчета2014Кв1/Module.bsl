...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0608042";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы["ИмяМакетаТабличногоДокумента"] = "ФормаОтчета2014Кв1_ФормаОтчета";
	СтруктураРеквизитовФормы["ИмяМакетаАтрибутовВыгрузки"] = "АтрибВыгрузкиXML2014Кв1";
	СтруктураРеквизитовФормы["ИмяМакетаСхемыВыгрузки"] = "СхемаВыгрузкиXML2014Кв1";
	СтруктураРеквизитовФормы["ИмяМакетаШаблона"] = "ШаблонРосстата2014Кв1";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Кол = 3 По 5 Цикл
		...
		СуммаПоКол = 0;
		Для Стр = 3 По 5 Цикл
			СуммаПоКол = СуммаПоКол + Области["П000010000" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00001000020" + Кол].Значение = СуммаПоКол;
		
		Области["П00001000010" + Кол].Значение = Области["П00001000020" + Кол].Значение
			+ Области["П00001000060" + Кол].Значение
			+ Области["П00001000070" + Кол].Значение;
		
	КонецЦикла;
	
	Области["П000020000803"].Значение = Области["П000020000903"].Значение
		+ Области["П000020001003"].Значение
		+ Области["П000020001103"].Значение;
		
	Области["П000020000303"].Значение = Области["П000020000403"].Значение
		+ Области["П000020000603"].Значение
		+ Области["П000020000703"].Значение;
	
	Области["П000020000203"].Значение = Области["П000020000303"].Значение
		+ Области["П000020000803"].Значение
		+ Области["П000020001203"].Значение;
		
	Для Раздел = 3 По 4 Цикл
		...
		Пока Стр >= 1 Цикл
			...
			Для Кол = 4 По КолонкаКон Цикл
				
				Если Стр = 1 Тогда
					
					Области["П0000" + Раздел + "000010" + Кол].Значение = Области["П0000" + Раздел + "000020" + Кол].Значение
						+ Области["П0000" + Раздел + "000060" + Кол].Значение
						+ Области["П0000" + Раздел + "000160" + Кол].Значение
						+ Области["П0000" + Раздел + "000170" + Кол].Значение
						+ Области["П0000" + Раздел + "000180" + Кол].Значение;
						
				ИначеЕсли Стр = 2 Тогда
						
					СуммаПоКол = 0;
					Для СтрТаб = 3 По 5 Цикл
						СуммаПоКол = СуммаПоКол
							+ Области["П0000" + Раздел + "000" + Формат(СтрТаб, "ЧЦ=2; ЧВН=") + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
					КонецЦикла;
					Области["П0000" + Раздел + "000020" + Кол].Значение = СуммаПоКол;
					
				ИначеЕсли Стр = 6 Тогда
					
					СуммаПоКол = 0;
					Для СтрТаб = 7 По 12 Цикл
						СуммаПоКол = СуммаПоКол
							+ Области["П0000" + Раздел + "000" + Формат(СтрТаб, "ЧЦ=2; ЧВН=") + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
					КонецЦикла;
					Области["П0000" + Раздел + "000060" + Кол].Значение = СуммаПоКол;
					
				ИначеЕсли Стр = 18 Тогда
					
					Области["П0000" + Раздел + "000180" + Кол].Значение = Области["П0000" + Раздел + "000190" + Кол].Значение
						+ Области["П0000" + Раздел + "000230" + Кол].Значение
						+ Области["П0000" + Раздел + "000240" + Кол].Значение;
						
				ИначеЕсли Стр = 19 Тогда
					
					СуммаПоКол = 0;
					Для СтрТаб = 20 По 22 Цикл
						СуммаПоКол = СуммаПоКол
							+ Области["П0000" + Раздел + "000" + Формат(СтрТаб, "ЧЦ=2; ЧВН=") + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
					КонецЦикла;
					Области["П0000" + Раздел + "000190" + Кол].Значение = СуммаПоКол;
					
				ИначеЕсли Стр = 24 Тогда
					
					СуммаПоКол = 0;
					Для СтрТаб = 25 По 31 Цикл
						СуммаПоКол = СуммаПоКол
							+ Области["П0000" + Раздел + "000" + Формат(СтрТаб, "ЧЦ=2; ЧВН=") + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
					КонецЦикла;
					Области["П0000" + Раздел + "000240" + Кол].Значение = СуммаПоКол;
					
				КонецЕсли;
				
				СуммаПоСтр = СуммаПоСтр
					+ Области["П0000" + Раздел + "000" + Формат(Стр, "ЧЦ=2; ЧВН=") + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
				
			КонецЦикла;
			
			Области["П0000" + Раздел + "000" + Формат(Стр, "ЧЦ=2; ЧВН=") + "03"].Значение = СуммаПоСтр;
		...
		Если Раздел = 3 Тогда
			Области["П000030003203"].Значение = Области["П000020000103"].Значение
				+ Области["П000020000203"].Значение
				- Области["П000030000103"].Значение;
		КонецЕсли;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Кол = 3 По 5 Цикл
		...
		Для Стр = 1 По 2 Цикл
			...
			мФормаОтчета.Области["П000010000" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
			мФормаОтчета.Области["П000010000" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
			
		КонецЦикла;
		
	КонецЦикла;
	
	мФормаОтчета.Области["П000020000203"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000020000203"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000020000303"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000020000303"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000020000803"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000020000803"].Защита   = Не ОтклАвтоРасчетЗначение;
	...
	Для Раздел = 3 По 4 Цикл
		...
		Для Стр = 1 По 31 Цикл
			...
			ИмяЯчейки = "П0000" + Раздел + "000" + Формат(Стр, "ЧЦ=2; ЧВН=") + "03";
			
			мФормаОтчета.Области[ИмяЯчейки].ЦветФона = ЦветФонаЗначение;
			мФормаОтчета.Области[ИмяЯчейки].Защита   = Не ОтклАвтоРасчетЗначение;
			
			Если СтрокаСуммы Тогда
				
				Для Кол = 3 По КолонкаКон Цикл
					
					мФормаОтчета.Области["П0000" + Раздел + "000010" + Кол].ЦветФона = ЦветФонаЗначение;
					мФормаОтчета.Области["П0000" + Раздел + "000010" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
					
					мФормаОтчета.Области["П0000" + Раздел + "000020" + Кол].ЦветФона = ЦветФонаЗначение;
					мФормаОтчета.Области["П0000" + Раздел + "000020" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
					
					мФормаОтчета.Области["П0000" + Раздел + "000060" + Кол].ЦветФона = ЦветФонаЗначение;
					мФормаОтчета.Области["П0000" + Раздел + "000060" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
					
					мФормаОтчета.Области["П0000" + Раздел + "000180" + Кол].ЦветФона = ЦветФонаЗначение;
					мФормаОтчета.Области["П0000" + Раздел + "000180" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
					
					мФормаОтчета.Области["П0000" + Раздел + "000190" + Кол].ЦветФона = ЦветФонаЗначение;
					мФормаОтчета.Области["П0000" + Раздел + "000190" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
					
					мФормаОтчета.Области["П0000" + Раздел + "000240" + Кол].ЦветФона = ЦветФонаЗначение;
					мФормаОтчета.Области["П0000" + Раздел + "000240" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
	...
	мФормаОтчета.Области["П000030003203"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000030003203"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 14-Ф (ОМС)");
