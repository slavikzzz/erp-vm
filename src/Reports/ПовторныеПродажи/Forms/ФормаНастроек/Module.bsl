...
Процедура АктивныеДоПриИзменении(Элемент)
	...
	Элементы.ДекорацияЗасыпающиеОт.Заголовок = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = 'от [ЗасыпающиеОт] до';
																											|en = 'from [ЗасыпающиеОт] to'"), Значения);
	...
	Если РасширенныйРежим = 0 Тогда
		...
		Элементы.ДекорацияЗасыпающиеПерспективныеОт.Заголовок = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = 'от [ЗасыпающиеПерспективныеОт] до';
																															|en = 'from [ЗасыпающиеПерспективныеОт] to'"), Значения);
		Элементы.ДекорацияЗасыпающиеПерспективныеОт.Заголовок = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = 'от [ЗасыпающиеВерныеОт] до';
																															|en = 'from [ЗасыпающиеВерныеОт] to'"), Значения);
	КонецЕсли;
...
Процедура АктивныеВерныеДоПриИзменении(Элемент)
	...
	Элементы.ДекорацияЗасыпающиеВерныеОт.Заголовок = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = 'от [ЗасыпающиеВерныеОт] до';
																												|en = 'from [ЗасыпающиеВерныеОт] to'"), Значения);
...
Функция ИнтервалыКорректны()
	...
	Если АктивныеДо = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Некорректно задан интервал дней.';
														|en = 'Incorrect interval of days.'"),,"АктивныеДо");
		ИнтервалыКорректны = Ложь;
	КонецЕсли;
	
	Если АктивныеПерспективныеДо = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Некорректно задан интервал дней.';
														|en = 'Incorrect interval of days.'"),,"АктивныеПерспективныеДо");
		ИнтервалыКорректны = Ложь;
	КонецЕсли;
	
	Если АктивныеВерныеДо = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Некорректно задан интервал дней.';
														|en = 'Incorrect interval of days.'"),,"АктивныеВерныеДо");
		ИнтервалыКорректны = Ложь;
	КонецЕсли;
	
	Если ЗасыпающиеДо < ЗасыпающиеОт Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Некорректно задан интервал дней.';
														|en = 'Incorrect interval of days.'"),,"ЗасыпающиеДо");
		ИнтервалыКорректны = Ложь;
	КонецЕсли;

	Если ЗасыпающиеПерспективныеДо < ЗасыпающиеПерспективныеОт Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Некорректно задан интервал дней.';
														|en = 'Incorrect interval of days.'"),,"ЗасыпающиеПерспективныеДо");
		ИнтервалыКорректны = Ложь;
	КонецЕсли;

	Если ЗасыпающиеВерныеДо < ЗасыпающиеВерныеОт Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Некорректно задан интервал дней.';
														|en = 'Incorrect interval of days.'"),,"ЗасыпающиеВерныеДо");
		ИнтервалыКорректны = Ложь;
	КонецЕсли;
