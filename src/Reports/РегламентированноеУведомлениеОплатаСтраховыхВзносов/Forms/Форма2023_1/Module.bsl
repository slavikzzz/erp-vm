...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ОплатаСтраховыхВзносов;
...
Процедура ПредставлениеУведомленияПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	ИначеЕсли Область.Имя = "Фамилия" Или Область.Имя = "Имя" Или Область.Имя = "Отчество" Тогда
		УведомлениеОСпецрежимахНалогообложенияКлиентСервер.ИмяСтраницыВДеревеЖурналаСправок(
			ДеревоСтраниц, ЭтотОбъект["УИДТекущаяСтраница"], ЭтотОбъект["ДанныеМногостраничныхРазделов"]["СведОплСтрВзн"]);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗагрузитьСправкуИзXML(ДополнительныеПараметры)
	УведомлениеОСпецрежимахНалогообложения.ЗагрузитьСправкуИзXMLЗаполнениеДанных(ЭтотОбъект, ДополнительныеПараметры, "СведОплСтрВзн");
	УведомлениеОСпецрежимахНалогообложенияКлиентСервер.ИмяСтраницыВДеревеЖурналаСправок(
		ДеревоСтраниц, ЭтотОбъект["УИДТекущаяСтраница"], ЭтотОбъект["ДанныеМногостраничныхРазделов"]["СведОплСтрВзн"])
КонецПроцедуры
...
Процедура СформироватьДеревоСтраниц() Экспорт
	...
	СтрРег.ИндексКартинки = 1;
	СтрРег.ИмяМакета = "СведОплСтрВзн_2023";
	СтрРег.Многостраничность = Истина;
	СтрРег.Многострочность = Ложь;
	СтрРег.УИД = Новый УникальныйИдентификатор;
	СтрРег.ИДНаименования = "СведОплСтрВзн";
КонецПроцедуры
...
Функция СформироватьВыгрузкуИПолучитьДанные() Экспорт 
	...
	СтруктураВыгрузки.Вставить("ИмяФайла", "1151159_5.01000_04.tif");
