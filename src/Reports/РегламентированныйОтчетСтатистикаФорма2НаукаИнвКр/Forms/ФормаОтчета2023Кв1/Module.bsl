...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0604009";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	//стр. 307 = сумме стр. 308-311, 313, 314;
	Области["П003001030703"].Значение
		= Области["П003001030803"].Значение
		+ Области["П003001030903"].Значение
		+ Области["П003001031003"].Значение
		+ Области["П003001031103"].Значение
		+ Области["П003001031303"].Значение
		+ Области["П003001031403"].Значение;
	
	Для Гр = 4 По 14 Цикл
		
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		
		//стр. 246 = сумме стр. 247–260;
		СумаПоСтр = 0;
		Для Стр = 247 По 260 Цикл
			СумаПоСтр = СумаПоСтр + Области["П0020010" + Стр + Графа].Значение
		КонецЦикла;
		Области["П0020010246" + Графа].Значение = СумаПоСтр;
		
		
		//стр. 232 = сумме стр. 233–241;
		СумаПоСтр = 0;
		Для Стр = 233 По 241 Цикл
			СумаПоСтр = СумаПоСтр + Области["П0020010" + Стр + Графа].Значение
		КонецЦикла;
		Области["П0020010232" + Графа].Значение = СумаПоСтр;
		
		
		//стр. 219 = сумме стр. 220–231;
		СумаПоСтр = 0;
		Для Стр = 220 По 231 Цикл
			СумаПоСтр = СумаПоСтр + Области["П0020010" + Стр + Графа].Значение
		КонецЦикла;
		Области["П0020010219" + Графа].Значение = СумаПоСтр;
		
		
		//стр. 210 = сумме стр. 211–218;
		СумаПоСтр = 0;
		Для Стр = 211 По 218 Цикл
			СумаПоСтр = СумаПоСтр + Области["П0020010" + Стр + Графа].Значение
		КонецЦикла;
		Области["П0020010210" + Графа].Значение = СумаПоСтр;
		
		
		//стр. 203 = сумме стр. 204–209;
		СумаПоСтр = 0;
		Для Стр = 204 По 209 Цикл
			СумаПоСтр = СумаПоСтр + Области["П0020010" + Стр + Графа].Значение
		КонецЦикла;
		Области["П0020010203" + Графа].Значение = СумаПоСтр;
		
		
		//стр. 202 = сумме стр. 203, 210, 219, 232;
		Области["П0020010202" + Графа].Значение
			= Области["П0020010203" + Графа].Значение
			+ Области["П0020010210" + Графа].Значение
			+ Области["П0020010219" + Графа].Значение
			+ Области["П0020010232" + Графа].Значение;
	...
	//по строкам 201–263:
	//гр. 3 = сумме гр. 4–9;
	Для Стр = 201 По 263 Цикл
		СумаПоГр = 0;
		Для Гр = 4 По 9 Цикл
			Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
			СумаПоГр = СумаПоГр + Области["П0020010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0020010" + Стр + "03"].Значение = СумаПоГр;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Стр = 201 По 263 Цикл
		...
		Если Стр = 202 Или Стр = 203 Или Стр = 210
		 Или Стр = 219 Или Стр = 232 Или Стр = 246 Тогда
			ГрКон = 14;
		Иначе
			ГрКон = 3;
		КонецЕсли;
		...
		Для Гр = 3 По ГрКон Цикл
			Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
			мФормаОтчета.Области["П0020010" + Стр + Графа].ЦветФона = ЦветФонаЗначение;
			мФормаОтчета.Области["П0020010" + Стр + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
		КонецЦикла;
		
	КонецЦикла;
	
	мФормаОтчета.Области["П003001030703"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П003001030703"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 2-наука (ИНВ) (краткая)");
