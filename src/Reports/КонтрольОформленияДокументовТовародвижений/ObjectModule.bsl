...
Функция ТекстЗапросаПередачаМатериаловВПроизводство()
	...
	// Наборы данных - Поля - Доступные значения.
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	&СтрокаВидОперацииПередачаМатериаловВПроизводство КАК ВидОперации,
	|	&СтрокаТипОтклоненияОформлениеСкладскихДокументов КАК ТипОтклонения,
	|	ВЫБОР
	|		КОГДА ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
	|			ТОГДА &СтрокаРекомендацияЗавершитеОформлениеРасходныхОрдеровНаТовары
...
//-- НЕ УТКА
	...
	|ГДЕ
	|	ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
	|	И (ТоварыКОтгрузке.ДокументОтгрузки ССЫЛКА Документ.ДвижениеПродукцииИМатериалов
	|	И ЕСТЬNULL(РеквизитыРаспоряжения.ХозяйственнаяОперация, 
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаВПроизводство)) В (
	...
	|	ИЛИ НЕ ТоварыКОтгрузке.ДокументОтгрузки ССЫЛКА Документ.ДвижениеПродукцииИМатериалов)
	|{ГДЕ
	|	(&СтрокаВидОперацииПередачаМатериаловВПроизводство) КАК ВидОперации,
	|	(&СтрокаТипОтклоненияОформлениеСкладскихДокументов) КАК ТипОтклонения,
	|	(ВЫБОР
	...
	|
	|ВЫБРАТЬ
	|	&СтрокаВидОперацииПередачаМатериаловВПроизводство,
	|	&СтрокаТипОтклоненияОформлениеФинансовыхДокументов,
	|	&СтрокаРекомендацияОформитеПередачуМатериаловВПроизводствоВСтатусеПринято,
	...
	|	И ПередачаМатериаловВПроизводство.Ссылка.Дата <= &ДатаКонцаДня
	|	И &ОтборТоваровПоПередачеМатериаловВПроизводство
	|{ГДЕ
	|	(&СтрокаВидОперацииПередачаМатериаловВПроизводство) КАК ВидОперации,
	|	(&СтрокаТипОтклоненияОформлениеФинансовыхДокументов) КАК ТипОтклонения,
	|	(&СтрокаРекомендацияОформитеПередачуМатериаловВПроизводствоВСтатусеПринято) КАК Рекомендация,
...
Функция ТекстЗапросаВозвратыМатериаловИзПроизводства()
	...
	// Наборы данных - Поля - Доступные значения.
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	&СтрокаВидОперацииВозвратыМатериаловИзПроизводства КАК ВидОперации,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
	|			ТОГДА &СтрокаТипОтклоненияОформлениеФинансовыхДокументов
	|		ИНАЧЕ &СтрокаТипОтклоненияОформлениеСкладскихДокументов
	...
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
	|			ТОГДА &СтрокаРекомендацияОформитеВозвратМатериаловИзПроизводства
	|		ИНАЧЕ &СтрокаРекомендацияОформитеПриходныеОрдераНаТовары
	|	КОНЕЦ КАК Рекомендация,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
	...
	|				ТИП(Документ.ДвижениеПродукцииИМатериалов)
	//++ Локализация
	|				,ТИП(Документ.ВозвратМатериаловИзПроизводства)
	//-- Локализация
...
//-- НЕ УТКА
	...
	|			И НЕ РеквизитыРаспоряжения.ДополнительнаяЗапись
	|ГДЕ
	|	(ТоварыКОформлениюПоступления.ДокументПоступления ССЫЛКА Документ.ДвижениеПродукцииИМатериалов
	|	И ЕСТЬNULL(РеквизитыРаспоряжения.ХозяйственнаяОперация, 
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратИзПроизводства)) В (
	...
	|	ИЛИ НЕ ТоварыКОформлениюПоступления.ДокументПоступления ССЫЛКА Документ.ДвижениеПродукцииИМатериалов)
	|{ГДЕ
	|	(&СтрокаВидОперацииВозвратыМатериаловИзПроизводства) КАК ВидОперации,
	|	(ВЫБОР
	|			КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
...
Функция ТекстЗапросаПриемкаГотовойПродукции()
	...
	// Наборы данных - Поля - Доступные значения.
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	&СтрокаВидОперацииПриемкаГотовойПродукции КАК ВидОперации,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
	|			ТОГДА &СтрокаТипОтклоненияОформлениеФинансовыхДокументов
	|		ИНАЧЕ &СтрокаТипОтклоненияОформлениеСкладскихДокументов
	...
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
	|			ТОГДА &СтрокаРекомендацияОформитеВыпускПродукции
	|		ИНАЧЕ &СтрокаРекомендацияОформитеПриходныеОрдераНаТовары
	|	КОНЕЦ КАК Рекомендация,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
...
//-- НЕ УТКА
	...
	|	(ТоварыКОформлениюПоступления.ДокументПоступления ССЫЛКА Документ.ДвижениеПродукцииИМатериалов
	|	И ЕСТЬNULL(РеквизитыРаспоряжения.ХозяйственнаяОперация, 
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукцииНаСклад)) В (
...
Функция ТекстЗапросаПереработкаСырьяУПереработчика()
	...
	// Наборы данных - Поля - Доступные значения.
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	&СтрокаВидОперацииПереработкаСырьяУПереработчика КАК ВидОперации,
	|	&СтрокаТипОтклоненияОформлениеСкладскихДокументов КАК ТипОтклонения,
	|	ВЫБОР
	|		КОГДА ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
	|			ТОГДА &СтрокаРекомендацияЗавершитеОформлениеРасходныхОрдеровНаТовары
	...
	|			ТИП(Документ.ПередачаТоваровХранителю))
	|		И &ОтборТоваровПоРегистру
	...
	|ГДЕ
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОтгрузке.ДокументОтгрузки) = ТИП(Документ.ПередачаТоваровХранителю)
	...
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ
	|	И ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
	|{ГДЕ
	|	(&СтрокаВидОперацииПереработкаСырьяУПереработчика) КАК ВидОперации,
	|	(&СтрокаТипОтклоненияОформлениеСкладскихДокументов) КАК ТипОтклонения,
	|	(ВЫБОР
...
Функция ТекстЗапросаВозвратыСырьяОтПереработчиков()
	...
	// Наборы данных - Поля - Доступные значения.
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	&СтрокаВидОперацииВозвратыСырьяОтПереработчиков КАК ВидОперации,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
	|			ТОГДА &СтрокаТипОтклоненияОформлениеФинансовыхДокументов
	|		ИНАЧЕ &СтрокаТипОтклоненияОформлениеСкладскихДокументов
	...
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
	|			ТОГДА &СтрокаРекомендацияОформитеВозвратСырьяОтПереработчика
	|		ИНАЧЕ &СтрокаРекомендацияОформитеПриходныеОрдераНаТовары
	|	КОНЕЦ КАК Рекомендация,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
	...
	|ГДЕ
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТоварыКОформлениюПоступления.ДокументПоступления) = ТИП(Документ.ПоступлениеТоваровОтХранителя)
	|			ТОГДА ВЫРАЗИТЬ(ТоварыКОформлениюПоступления.ДокументПоступления КАК Документ.ПоступлениеТоваровОтХранителя).ХозяйственнаяОперация = 
...
Функция ТекстЗапросаПоступленияПродукцииОтПереработчиков()
	...
	// Наборы данных - Поля - Доступные значения.
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	&СтрокаВидОперацииПоступленияПродукцииОтПереработчиков КАК ВидОперации,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
	|			ТОГДА &СтрокаТипОтклоненияОформлениеФинансовыхДокументов
	|		ИНАЧЕ &СтрокаТипОтклоненияОформлениеСкладскихДокументов
	...
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
	|			ТОГДА &СтрокаРекомендацияОформитеПоступлениеОтПереработчика
	|		ИНАЧЕ &СтрокаРекомендацияОформитеПриходныеОрдераНаТовары
	|	КОНЕЦ КАК Рекомендация,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
...
Функция ТекстЗапросаПереработкаСырьяДавальца()
	...
	// Наборы данных - Поля - Доступные значения.
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	&СтрокаВидОперацииПереработкаСырьяДавальца КАК ВидОперации,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
	|			ТОГДА &СтрокаТипОтклоненияОформлениеФинансовыхДокументов
	|		ИНАЧЕ &СтрокаТипОтклоненияОформлениеСкладскихДокументов
	...
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
	|			ТОГДА &СтрокаРекомендацияОформитеПоступлениеСырьяОтДавальца
	|		ИНАЧЕ &СтрокаРекомендацияОформитеПриходныеОрдераНаТовары
	|	КОНЕЦ КАК Рекомендация,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
...
Функция ТекстЗапросаВозвратыСырьяДавальцам()
	...
	// Наборы данных - Поля - Доступные значения.
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	&СтрокаВидОперацииВозвратыСырьяДавальцам КАК ВидОперации,
	|	&СтрокаТипОтклоненияОформлениеСкладскихДокументов КАК ТипОтклонения,
	|	ВЫБОР
	|		КОГДА ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
	|			ТОГДА &СтрокаРекомендацияЗавершитеОформлениеРасходныхОрдеровНаТовары
...
Функция ТекстЗапросаПередачиПродукцииДавальцам()
	...
	// Наборы данных - Поля - Доступные значения.
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	&СтрокаВидОперацииПередачиПродукцииДавальцам КАК ВидОперации,
	|	&СтрокаТипОтклоненияОформлениеСкладскихДокументов КАК ТипОтклонения,
	|	ВЫБОР
	|		КОГДА ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
	|			ТОГДА &СтрокаРекомендацияЗавершитеОформлениеРасходныхОрдеровНаТовары
...
Функция ТекстЗапросаКонтрольОформленияДокументовТовародвижений(ИспользуетсяОтборТоваров)
	...
	//++ НЕ УТКА
	ПравоЧтениеДвижениеПродукцииИМатериалов		= ПравоДоступа("Чтение", Метаданные.Документы.ДвижениеПродукцииИМатериалов);
	//-- НЕ УТКА
	...
	ПравоЧтениеПередачаСырьяПереработчику		= ПравоДоступа("Чтение", Метаданные.Документы.ПередачаСырьяПереработчику);
	ПравоЧтениеВозвратСырьяОтПереработчика		= ПравоДоступа("Чтение", Метаданные.Документы.ВозвратСырьяОтПереработчика);
	//-- Устарело_Переработка24
	
	ПравоЧтениеЗаказПереработчику2_5			= ПравоДоступа("Чтение", Метаданные.Документы.ЗаказПереработчику2_5);
	...
	//-- Устарело_Переработка24
	ПравоЧтениеЗаказДавальца2_5					= ПравоДоступа("Чтение", Метаданные.Документы.ЗаказДавальца2_5);
	...
	Если ПравоЧтениеТоварыКОтгрузке
		...
		//-- Локализация
		И ПравоЧтениеДвижениеПродукцииИМатериалов
		//-- НЕ УТКА
	...
	Если ПравоЧтениеТоварыКОтгрузке
		И (ПравоЧтениеЗаказПереработчику2_5
				И ПравоЧтениеПередачаТоваровХранителю
			//++ Устарело_Переработка24
			Или ПравоЧтениеЗаказПереработчику
				И ПравоЧтениеПередачаСырьяПереработчику
			//-- Устарело_Переработка24
			Или Ложь)
	...
	КонецЕсли;
	Если ПравоЧтениеТоварыКОформлениюПоступления
		И (ПравоЧтениеПоступлениеТоваровОтХранителя
			//++ Устарело_Переработка24
			Или ПравоЧтениеВозвратСырьяОтПереработчика
			//-- Устарело_Переработка24
			Или Ложь)
	...
	Если ПравоЧтениеТоварыКОформлениюПоступления
		И (ПравоЧтениеЗаказДавальца2_5
			//++ Устарело_Переработка24
			Или ПравоЧтениеЗаказДавальца
			//-- Устарело_Переработка24
			Или Ложь)
...
Процедура ЗаменитьПараметрыОтбораТоваров(ТекстЗапроса, ИспользуетсяОтборТоваров)
	...
	//++ Локализация
	ТекстЗапросаОтборТоваровПоПередачеМатериаловВПроизводство = "ИСТИНА";
	//-- Локализация
	...
	Если ИспользуетсяОтборТоваров Тогда
		...
		//++ Локализация
		ТекстЗапросаОтборТоваровПоПередачеМатериаловВПроизводство =
		"(ПередачаМатериаловВПроизводство.Ссылка.Подразделение, ПередачаМатериаловВПроизводство.Номенклатура, ПередачаМатериаловВПроизводство.Характеристика, ПередачаМатериаловВПроизводство.Назначение, ПередачаМатериаловВПроизводство.Серия) В
		|					(ВЫБРАТЬ
		|						ТаблицаОтборов.Склад			КАК Склад,
		|						ТаблицаОтборов.Номенклатура		КАК Номенклатура,
		|						ТаблицаОтборов.Характеристика	КАК Характеристика,
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
								"&ОтборТоваровПоПередачеМатериаловВПроизводство",
								ТекстЗапросаОтборТоваровПоПередачеМатериаловВПроизводство);
	//-- Локализация
...
Функция ДоступныеВидыОперацийИЗапросы()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство") Тогда
		ДобавитьВидОперацииИЗапрос(Доступные, ВидыОперацийИСинонимы, "ПередачаМатериаловВПроизводство", "ТекстЗапросаПередачаМатериаловВПроизводство");
		ДобавитьВидОперацииИЗапрос(Доступные, ВидыОперацийИСинонимы, "ВозвратыМатериаловИзПроизводства", "ТекстЗапросаВозвратыМатериаловИзПроизводства");
		ДобавитьВидОперацииИЗапрос(Доступные, ВидыОперацийИСинонимы, "ПриемкаГотовойПродукции", "ТекстЗапросаПриемкаГотовойПродукции");
	КонецЕсли;
	//++ Устарело_Переработка24
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне") Тогда
		ДобавитьВидОперацииИЗапрос(Доступные, ВидыОперацийИСинонимы, "ПереработкаСырьяУПереработчика", "ТекстЗапросаПереработкаСырьяУПереработчика");
		ДобавитьВидОперацииИЗапрос(Доступные, ВидыОперацийИСинонимы, "ВозвратыСырьяОтПереработчиков", "ТекстЗапросаВозвратыСырьяОтПереработчиков");
		ДобавитьВидОперацииИЗапрос(Доступные, ВидыОперацийИСинонимы, "ПоступленияПродукцииОтПереработчиков", "ТекстЗапросаПоступленияПродукцииОтПереработчиков");
	КонецЕсли;
	//-- Устарело_Переработка24
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья") Тогда
		ДобавитьВидОперацииИЗапрос(Доступные, ВидыОперацийИСинонимы, "ПереработкаСырьяДавальца", "ТекстЗапросаПереработкаСырьяДавальца");
		ДобавитьВидОперацииИЗапрос(Доступные, ВидыОперацийИСинонимы, "ВозвратыСырьяДавальцам", "ТекстЗапросаВозвратыСырьяДавальцам");
		ДобавитьВидОперацииИЗапрос(Доступные, ВидыОперацийИСинонимы, "ПередачиПродукцииДавальцам", "ТекстЗапросаПередачиПродукцииДавальцам");
	КонецЕсли;
	//-- Устарело_Переработка24
