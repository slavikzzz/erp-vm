...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2022Кв3";
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СоставПоказателей2022Кв3");
...
Процедура ЗаполнитьСведенияОбОрганизацииНаСервере()
	...
	СтрокаСведений = "НаимЮЛПол, ФИО, РегистрационныйНомерФСС, НомерСтрахователяФСС, АдрЮР, АдрФакт, АдрПрописки";
	...
	НаименованиеСРеквизитами = НаименованиеОрганизации + Символы.ПС
	                         ...
	                         + "; Регистрационный номер: " + СокрЛП(
							 ?(ЗначениеЗаполнено(СведенияОбОрганизации.НомерСтрахователяФСС),
							   СведенияОбОрганизации.НомерСтрахователяФСС, СведенияОбОрганизации.РегистрационныйНомерФСС))
...
Процедура ЗаполнитьДатуПодписи(Форма)
	...
	Форма.СтруктураДанныхТаблица1.Вставить("ДатаПодписиДень",  ДеньСтрокой);
	Форма.СтруктураДанныхТаблица1.Вставить("ДатаПодписиМесяц", МесяцСтрокой);
	Форма.СтруктураДанныхТаблица1.Вставить("ДатаПодписиГод",   ГодСтрокой);
	...
	Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Таблица1" Тогда
		...
		Форма.ТабличныйДокумент.Области.ДатаПодписиДень.Значение  = ДеньСтрокой;
		Форма.ТабличныйДокумент.Области.ДатаПодписиМесяц.Значение = МесяцСтрокой;
		Форма.ТабличныйДокумент.Области.ДатаПодписиГод.Значение   = ГодСтрокой;
...
Процедура ЗаполнитьСведенияОПодписантах()
	...
	СтруктураДанныхТаблица1.Вставить("РуководительФИО", Руководитель);
	СтруктураДанныхТаблица1.Вставить("ГлавбухФИО", Главбух);
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Таблица1" Тогда
		...
		ТабличныйДокумент.Области.РуководительФИО.Значение = Руководитель;
		ТабличныйДокумент.Области.ГлавбухФИО.Значение      = Главбух;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого СтрокаУровня1 Из мДеревоВыбранныхСтраниц.ПолучитьЭлементы() Цикл
		...
		Если СтрокаУровня1.ИмяСтраницы = "Таблица1" Тогда
			...
			ТабличныйДокумент.Область("ПолеНомерКорректировки"), ТипСмещенияТабличногоДокумента.ПоВертикали);
			ТабличныйДокумент.УдалитьОбласть(
			ТабличныйДокумент.Область("СекцияДатаПодписи"), ТипСмещенияТабличногоДокумента.ПоВертикали);
			...
			Для Каждого Область Из ТабличныйДокумент.Области Цикл
				Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник
					...
					Область.ЦветФона = мСтруктураЦветовРаскраски["_5"];
				КонецЕсли;
			КонецЦикла;
		...
		Если СтрокаУровня1.ИмяСтраницы = "Таблица2" Тогда
			...
			Для НомПечФормы = 1 По СтруктураРеквизитовФормы.мПечатныеФормы.Количество() - 1 Цикл
				...
				ТабличныйДокумент.УдалитьОбласть(ТабличныйДокумент.Область("КнопкаДобавленияСтроки"),
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Таблица1" Тогда
		...
		Форма.ТабличныйДокумент.Области.П000010000143.Значение
		= Форма.ТабличныйДокумент.Области.П000010000101.Значение
		...
		+ Форма.ТабличныйДокумент.Области.П000010000121.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000123.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000125.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000127.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000129.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000131.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000133.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000135.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000137.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000139.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000141.Значение;
		
		Форма.ТабличныйДокумент.Области.П000010000144.Значение
		= Форма.ТабличныйДокумент.Области.П000010000102.Значение
		...
		+ Форма.ТабличныйДокумент.Области.П000010000122.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000124.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000126.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000128.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000130.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000132.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000134.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000136.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000138.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000140.Значение
		+ Форма.ТабличныйДокумент.Области.П000010000142.Значение;
		
	Иначе
		
		Если Форма.СтруктураДанныхТаблица1.Количество() > 0 Тогда
			
			Форма.СтруктураДанныхТаблица1.П000010000143
			= Форма.СтруктураДанныхТаблица1.П000010000101
			...
			+ Форма.СтруктураДанныхТаблица1.П000010000121
			+ Форма.СтруктураДанныхТаблица1.П000010000123
			+ Форма.СтруктураДанныхТаблица1.П000010000125
			+ Форма.СтруктураДанныхТаблица1.П000010000127
			+ Форма.СтруктураДанныхТаблица1.П000010000129
			+ Форма.СтруктураДанныхТаблица1.П000010000131
			+ Форма.СтруктураДанныхТаблица1.П000010000133
			+ Форма.СтруктураДанныхТаблица1.П000010000135
			+ Форма.СтруктураДанныхТаблица1.П000010000137
			+ Форма.СтруктураДанныхТаблица1.П000010000139
			+ Форма.СтруктураДанныхТаблица1.П000010000141;
			
			Форма.СтруктураДанныхТаблица1.П000010000144
			= Форма.СтруктураДанныхТаблица1.П000010000102
			...
			+ Форма.СтруктураДанныхТаблица1.П000010000122
			+ Форма.СтруктураДанныхТаблица1.П000010000124
			+ Форма.СтруктураДанныхТаблица1.П000010000126
			+ Форма.СтруктураДанныхТаблица1.П000010000128
			+ Форма.СтруктураДанныхТаблица1.П000010000130
			+ Форма.СтруктураДанныхТаблица1.П000010000132
			+ Форма.СтруктураДанныхТаблица1.П000010000134
			+ Форма.СтруктураДанныхТаблица1.П000010000136
			+ Форма.СтруктураДанныхТаблица1.П000010000138
			+ Форма.СтруктураДанныхТаблица1.П000010000140
			+ Форма.СтруктураДанныхТаблица1.П000010000142;
	...
	Для Инд = 0 По (МассивМногострочныхБлоков.Количество() - 1) Цикл
		...
		Если ТекущееКоличествоСтрок > ИсходноеКоличествоСтрокГруппы Тогда
			...
			Пока НСтр > ИсходноеКоличествоСтрокГруппы Цикл
				...
				ВыбТабличноеПоле, НомерВерхЯчейкиОбласти, НомерНижнЯчейкиОбласти);
...
Процедура ОткрытьФормуВыбораСтраниц(ВариантВыбора)
	...
	"НастройкиОтчета", ПараметрыФормы, ЭтаФорма, "дляВыбораСтраницОтчета");
