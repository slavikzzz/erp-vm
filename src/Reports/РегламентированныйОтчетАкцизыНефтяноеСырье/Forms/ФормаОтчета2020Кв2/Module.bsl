...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СпискиВыбора2020Кв2");
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "П000210014003" И НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		...
		ИначеЕсли Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2020 Тогда
			СписокКодов.Добавить(0.333, "0,333");
		Иначе
			СписокКодов.Добавить(0.167, "0,167");
		КонецЕсли;
	...
	ИначеЕсли Область.Имя = "П000210015003" И НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
	...
	ИначеЕсли Область.Имя = "П000310003003" ИЛИ Область.Имя = "П000310003004" Тогда
		...
		МесяцИГод_Дата = СтроковыеФункцииКлиентСервер.СтрокаВДату("01."
		+ СокрЛП(ТабличныйДокумент.Область("П000310003003").Значение)
		+ СокрЛП(ТабличныйДокумент.Область("П000310003004").Значение));
		Если ЗначениеЗаполнено(МесяцИГод_Дата) Тогда
			ПараметрыВыбора.Вставить("НачалоПериода", МесяцИГод_Дата);
			ПараметрыВыбора.Вставить("КонецПериода",  КонецМесяца(МесяцИГод_Дата));
		Иначе
		...
		СтруктПарам = Новый Структура;
		СтруктПарам.Вставить("ОблКодПериода", ТабличныйДокумент.Область("П000310003003"));
		СтруктПарам.Вставить("ОблГодПериода", ТабличныйДокумент.Область("П000310003004"));
		СтруктПарам.Вставить("Область", Область);
		СтруктПарам.Вставить("Элемент", Элемент);
	...
	ИначеЕсли Область.Имя = "П000310006503" ИЛИ Область.Имя = "П000310014503" ИЛИ Область.Имя = "П000310030503" Тогда
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	ИначеЕсли ИмяПоказателя = "П000310003003" ИЛИ ИмяПоказателя = "П000310003004" Тогда
		
		СтруктПарам = Новый Структура;
		СтруктПарам.Вставить("КодПериода", ТабличныйДокумент.Область("П000310003003").Значение);
		СтруктПарам.Вставить("ГодПериода", ТабличныйДокумент.Область("П000310003004").Значение);
		СтруктПарам.Вставить("НомерСтраницы", Элементы.РазделыОтчета.ТекущиеДанные.КолонкаНомерСтраницыРазделаОтчета);
		
		Если Элементы.РазделыОтчета.ТекущиеДанные.ПолучитьРодителя() = Неопределено Тогда
			Элементы.РазделыОтчета.ТекущиеДанные.ПолучитьЭлементы()[0].КолонкаРазделыОтчета
			= ПредставлениеСтраницы_Раздел31(СтруктПарам);
		Иначе
			Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчета = ПредставлениеСтраницы_Раздел31(СтруктПарам);
		КонецЕсли;
	...
	ИначеЕсли ИмяПоказателя = "П000030002003" ИЛИ ИмяПоказателя = "П000030002503" Тогда
		...
		ОблПарногоПоказателя = ТабличныйДокумент.Области.Найти(ИмяПарногоПоказателя);
	ИначеЕсли ИмяПоказателя = "П000310008003" ИЛИ ИмяПоказателя = "П000310009003" Тогда
		ИмяПарногоПоказателя = ?(ИмяПоказателя = "П000310008003", "П000310009003", "П000310008003");
		ОблПарногоПоказателя = ТабличныйДокумент.Области.Найти(ИмяПарногоПоказателя);
	ИначеЕсли ИмяПоказателя = "П000310010003" ИЛИ ИмяПоказателя = "П000310011003" Тогда
		ИмяПарногоПоказателя = ?(ИмяПоказателя = "П000310010003", "П000310011003", "П000310010003");
		ОблПарногоПоказателя = ТабличныйДокумент.Области.Найти(ИмяПарногоПоказателя);
	КонецЕсли;
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел2" ИЛИ ИмяРаздела = "Раздел21" ИЛИ ИмяРаздела = "Раздел31" Тогда
		...
		Если ИмяРаздела = "Раздел21" Тогда
			...
			ЕстьДоговор = Форма.ТабличныйДокумент.Области.Найти("П000210002503").Значение = 1;
			
			Если ЕстьДоговор Тогда
				Форма.ТабличныйДокумент.Область("РеквизитыДоговора").Видимость = Истина;
				Форма.ТабличныйДокумент.Область("ПсевдоРеквизитыДоговора").Видимость = Ложь;
			Иначе
				Форма.ТабличныйДокумент.Область("П000210003003").Очистить();
				Форма.ТабличныйДокумент.Область("П000210004003").Очистить();
				Форма.ТабличныйДокумент.Область("РеквизитыДоговора").Видимость = Ложь;
				Форма.ТабличныйДокумент.Область("ПсевдоРеквизитыДоговора").Видимость = Истина;
			КонецЕсли;
			...
			Если НЕ (РезультатПроверки.ВыполненоУсловие1
				...
				РазрешитьВводДанныхВОбласть(Форма, Форма.ТабличныйДокумент.Область("П000210005003"));
...
Процедура ДобавитьСтраницу(Команда)
	...
	ИначеЕсли ИмяРаздела = "Раздел21" ИЛИ ИмяРаздела = "Раздел31" Тогда
	...
	ИначеЕсли ИмяРаздела = "Раздел31" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				ТДО["П000310032003"].Значение = Окр(
				((ТДО["П000310033003"].Значение - ТДО["П000310034003"].Значение - ТДО["П000310035003"].Значение)
				* ТДО["П000310036003"].Значение + ТДО["П000310037003"].Значение) * (1 + ТДО["П000310038003"].Значение / 100));
				
				ТДО["П000310031003"].Значение = ТДО["П000310032003"].Значение - ТДО["П000310039003"].Значение;
				
				ТДО["П000310030503"].Значение = ?(ТДО["П000310031003"].Значение > 0, 1, 0);
				
				ТДО["П000310016003"].Значение = Окр(
				((ТДО["П000310017003"].Значение - ТДО["П000310018003"].Значение - ТДО["П000310019003"].Значение)
				* ТДО["П000310020003"].Значение + ТДО["П000310021003"].Значение) * (1 + ТДО["П000310022003"].Значение / 100));
				
				ТДО["П000310015003"].Значение = ТДО["П000310016003"].Значение - ТДО["П000310023003"].Значение;
				
				ТДО["П000310014503"].Значение = ?(ТДО["П000310015003"].Значение > 0, 1, 0);
				
				ТДО["П000310007003"].Значение = Окр(
				  ТДО["П000310014503"].Значение * ТДО["П000310013003"].Значение * ТДО["П000310024003"].Значение
				+ ТДО["П000310030503"].Значение * ТДО["П000310029003"].Значение * ТДО["П000310040003"].Значение
				+ ТДО["П000310025003"].Значение * ТДО["П000310027003"].Значение
				+ ТДО["П000310041003"].Значение * ТДО["П000310043003"].Значение);
				
				ТДО["П000310006503"].Значение = ?(ТДО["П000310007003"].Значение > 0, 1, 0);
				
				ТДО["П000310006003"].Значение = ТДО["П000310005003"].Значение * 2;
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел31" Тогда
		...
		Для каждого Ветка_3 Из ДеревоДанных.Строки Цикл
			...
			Для каждого Ветка Из Ветка_3.Строки[0].Строки Цикл
				
				Ветка.Данные.Вставить("П000310032003", Окр(
				((Ветка.Данные["П000310033003"] - Ветка.Данные["П000310034003"] - Ветка.Данные["П000310035003"])
				* Ветка.Данные["П000310036003"] + Ветка.Данные["П000310037003"]) * (1 + Ветка.Данные["П000310038003"] / 100)));
				
				Ветка.Данные.Вставить("П000310031003", Ветка.Данные["П000310032003"] - Ветка.Данные["П000310039003"]);
				
				Ветка.Данные.Вставить("П000310030503", ?(Ветка.Данные["П000310031003"] > 0, 1, 0));
				
				Ветка.Данные.Вставить("П000310016003", Окр(
				((Ветка.Данные["П000310017003"] - Ветка.Данные["П000310018003"] - Ветка.Данные["П000310019003"])
				* Ветка.Данные["П000310020003"] + Ветка.Данные["П000310021003"]) * (1 + Ветка.Данные["П000310022003"] / 100)));
				
				Ветка.Данные.Вставить("П000310015003", Ветка.Данные["П000310016003"] - Ветка.Данные["П000310023003"]);
				
				Ветка.Данные.Вставить("П000310014503", ?(Ветка.Данные["П000310015003"] > 0, 1, 0));
				
				Ветка.Данные.Вставить("П000310007003", Окр(
				  Ветка.Данные["П000310014503"] * Ветка.Данные["П000310013003"] * Ветка.Данные["П000310024003"]
				+ Ветка.Данные["П000310030503"] * Ветка.Данные["П000310029003"] * Ветка.Данные["П000310040003"]
				+ Ветка.Данные["П000310025003"] * Ветка.Данные["П000310027003"]
				+ Ветка.Данные["П000310041003"] * Ветка.Данные["П000310043003"]));
				
				Ветка.Данные.Вставить("П000310006503", ?(Ветка.Данные["П000310007003"] > 0, 1, 0));
				
				Ветка.Данные.Вставить("П000310006003", Ветка.Данные["П000310005003"] * 2);
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ.Вставить("П000310032003");
	Структ.Вставить("П000310031003");
	Структ.Вставить("П000310030503");
	Структ.Вставить("П000310016003");
	Структ.Вставить("П000310015003");
	Структ.Вставить("П000310014503");
	Структ.Вставить("П000310007003");
	Структ.Вставить("П000310006503");
	...
	Если ЗаполненРаздел3 Тогда
		...
		Для каждого Ветка Из ДеревоРаздела.Строки Цикл
			...
			Для каждого ПодчВетка Из Ветка.Строки[0].Строки Цикл
				...
				ЗначПок = СокрЛП(ПодчВетка.Данные["П000310003003"]);
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + " -> Раздел 3.1 -> Страница " + НомПодчСтраницы
					+ ": не задан код налогового периода", "Раздел31", "П000310003003", КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ЗначПок = СокрЛП(ПодчВетка.Данные["П000310003004"]);
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + " -> Раздел 3.1 -> Страница " + НомПодчСтраницы
					+ ": не задан год налогового периода", "Раздел31", "П000310003004", КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ЗначПок = ПодчВетка.Данные["П000310006503"];
				Если ЗначПок <> 0 И ЗначПок <> 1 Тогда
					СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + " -> Раздел 3.1 -> Страница "
					+ НомПодчСтраницы + ": задан некорректный признак величины Кдемп. Возможны значения: 0, 1",
					"Раздел31", "П000310006503", КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ЗаполненыСведения_АБ = ЗначениеЗаполнено(ПодчВетка.Данные["П000310013003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310014003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310014503"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310015003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310016003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310017003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310018003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310019003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310020003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310021003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310022003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310023003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310024003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310025003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310026003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310027003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310028003"]);
				
				ЗаполненыСведения_ДТ = ЗначениеЗаполнено(ПодчВетка.Данные["П000310029003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310030003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310030503"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310031003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310032003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310033003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310034003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310035003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310036003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310037003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310038003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310039003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310040003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310041003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310042003"])
				ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310043003"]) ИЛИ ЗначениеЗаполнено(ПодчВетка.Данные["П000310044003"]);
				
				Если НЕ ЗаполненыСведения_АБ И НЕ ЗаполненыСведения_ДТ Тогда
					
					СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + " -> Раздел 3.1 -> Страница "
					+ НомПодчСтраницы + ": не заполнен подраздел 3.1.1 и(или) подраздел 3.1.2",
					"Раздел31", "П000310013003", КоординатыПодчВетки);
					СтатусПроверки = Ложь;
					
				Иначе
					
					ЗначПок = ПодчВетка.Данные["П000310014503"];
					Если ЗначПок <> 0 И ЗначПок <> 1 Тогда
						СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + " -> Раздел 3.1 -> Страница "
						+ НомПодчСтраницы + ": задан некорректный признак показателя (Даб). Возможны значения: 0, 1",
						"Раздел31", "П000310014503", КоординатыПодчВетки);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					ЗначПок = ПодчВетка.Данные["П000310030503"];
					Если ЗначПок <> 0 И ЗначПок <> 1 Тогда
						СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + " -> Раздел 3.1 -> Страница "
						+ НомПодчСтраницы + ": задан некорректный признак показателя (Ддт). Возможны значения: 0, 1",
						"Раздел31", "П000310030503", КоординатыПодчВетки);
						СтатусПроверки = Ложь;
					КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, ПараметрыВыгрузки)
	...
	Для каждого Ветка Из ДеревоРаздела.Строки Цикл
		...
		Для каждого Эл Из ЭкзРасчСумАкцВыч.Строки Цикл
			...
			Если Эл.Код = "ОбщСумУпл"
				И Ветка.Данные["П000030002003"] = 0 И Ветка.Данные["П000030002503"] = 0 Тогда
				Эл.Обязательность = "О";
			КонецЕсли;
			
		КонецЦикла;
		
		СведНач = ПолучитьПодчиненныйЭлемент(ЭкзРасчСумАкцВыч, "СведНач");
		
		Для каждого ПодчВетка Из Ветка.Строки[0].Строки Цикл
			
			ЭкзСведНач = СкопироватьУзел(ЭкзРасчСумАкцВыч, СведНач, ЭкзРасчСумАкцВыч.Строки.Количество() - 1);
			
			Для каждого Эл Из ЭкзСведНач.Строки Цикл
				
				Если Эл.Строки.Количество() = 0 Тогда
					ВывестиПоказательВXML(Эл, ПодчВетка.Данные[Эл.Ключ]);
				КонецЕсли;
				
				Если Эл.Код = "СумВычКдемп"
					И ПодчВетка.Данные["П000310008003"] = 0 И ПодчВетка.Данные["П000310009003"] = 0 Тогда
					Эл.Обязательность = "О";
				ИначеЕсли Эл.Код = "СумАкцУпл"
					И ПодчВетка.Данные["П000310010003"] = 0 И ПодчВетка.Данные["П000310011003"] = 0 Тогда
					Эл.Обязательность = "О";
				КонецЕсли;
				
			КонецЦикла;
			
			ЗаполнитьДаннымиУзел(ПодчВетка.Данные, ПолучитьПодчиненныйЭлемент(ЭкзСведНач, "СведАБ"));
			ЗаполнитьДаннымиУзел(ПодчВетка.Данные, ПолучитьПодчиненныйЭлемент(ЭкзСведНач, "СведДТ"));
			
		КонецЦикла;
		
		РегламентированнаяОтчетность.УдалитьУзел(СведНач);
...
Процедура ОчиститьТекущуюСтраницуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел21" Тогда
		...
		СтруктПарам.Вставить("НомерСтраницы", Элементы.РазделыОтчета.ТекущиеДанные.КолонкаНомерСтраницыРазделаОтчета);
		Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчета = ПредставлениеСтраницы_Раздел21(СтруктПарам);
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел31" Тогда
		СтруктПарам = Новый Структура;
		СтруктПарам.Вставить("КодПериода", ТабличныйДокумент.Область("П000310003003").Значение);
		СтруктПарам.Вставить("ГодПериода", ТабличныйДокумент.Область("П000310003004").Значение);
		СтруктПарам.Вставить("НомерСтраницы", Элементы.РазделыОтчета.ТекущиеДанные.КолонкаНомерСтраницыРазделаОтчета);
...
Процедура ВывестиНаПечатьДанныеМногоуровневогоРаздела(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	ИначеЕсли ИмяТекРаздела = "Раздел3" Тогда
		...
		МакетР31 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(Сред(
		ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_Раздел31");;
		
		Для каждого ВеткаР3 Из ДеревоДанныхРаздела.Строки Цикл
			
			Для каждого ВеткаР31 Из ВеткаР3.Строки[0].Строки Цикл
				
				ТабличныйДокумент.Очистить();
				ТабличныйДокумент.Вывести(МакетР3);
				ТабличныйДокумент.Вывести(МакетР31);
				
				Если ВеткаР3.Строки[0].Строки.Индекс(ВеткаР31) = 0 Тогда
				...
				НемногострочныеДанные = ВеткаР31.Данные;
				Для каждого Пок Из НемногострочныеДанные Цикл
					Обл = ТабличныйДокумент.Области.Найти(Пок.Ключ);
					Если Обл <> Неопределено Тогда
						Обл.Значение = Пок.Значение;
				...
				СтруктураПараметровРаздела = ОбщегоНазначения.СкопироватьРекурсивно(СтруктураПараметров);
				СтруктураПараметровРаздела.Вставить("ИмяСтраницы", "Раздел3_АБ");
				ВывестиНаПечатьЛист(СтруктураПараметровРаздела, ОбщиеРеквизитыРазделов, СтрПоля);
				...
				СтруктураПараметровРаздела = ОбщегоНазначения.СкопироватьРекурсивно(СтруктураПараметров);
				СтруктураПараметровРаздела.Вставить("ИмяСтраницы", "Раздел3_ДТ");
				ВывестиНаПечатьЛист(СтруктураПараметровРаздела, ОбщиеРеквизитыРазделов, СтрПоля);
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	ПрефиксИменМакетов = "ПечатныйБланк2020Кв2_";
...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	Если СтрНачинаетсяС(ПараметрыПечати.ИмяСтраницы, "Раздел2") Тогда
		...
		// Особая обработка: случай превышения формата Строка(11).
		//
		МаксимальнаяДлинаНомераДоговора = 11;
		НомерДоговора = СокрЛП(ТабличныйДокумент.Область("П000210003003").Значение);
		Если СтрДлина(НомерДоговора) > МаксимальнаяДлинаНомераДоговора Тогда
			Обл = МакетМЧБ.Области.Найти("НомерДоговора11");
			Если Обл <> Неопределено Тогда
				МакетМЧБ.УдалитьОбласть(Обл, ТипСмещенияТабличногоДокумента.ПоВертикали);
				НовСтр = Показатели.Добавить();
				НовСтр.Имя = "П100210003003";
				НовСтр.Значение = НомерДоговора;
				НовСтр.ЗначениеМЧБ = НомерДоговора;
			КонецЕсли;
		Иначе
			Обл = МакетМЧБ.Области.Найти("НомерДоговора20");
			Если Обл <> Неопределено Тогда
				МакетМЧБ.УдалитьОбласть(Обл, ТипСмещенияТабличногоДокумента.ПоВертикали);
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли СтрНачинаетсяС(ПараметрыПечати.ИмяСтраницы, "Раздел3") Тогда
		...
		// Особая обработка: случай превышения формата Число(8,4).
		//
		СредняяЦена = ТабличныйДокумент.Область("П000310026003").Значение;
		МинимальноеЗначениеПревышенияМаксимальногоВозможногоЗначенияСреднейЦеныАБ = 10000;
		Если СредняяЦена >= МинимальноеЗначениеПревышенияМаксимальногоВозможногоЗначенияСреднейЦеныАБ Тогда
			Обл = МакетМЧБ.Области.Найти("СредняяЦенаАБ8_4");
			Если Обл <> Неопределено Тогда
				МакетМЧБ.УдалитьОбласть(Обл, ТипСмещенияТабличногоДокумента.ПоВертикали);
				НовСтр = Показатели.Добавить();
				НовСтр.Имя = "П100310026003";
				НовСтр.Значение = СредняяЦена;
				НовСтр.ЗначениеМЧБ = СредняяЦена;
			КонецЕсли;
		Иначе
			Обл = МакетМЧБ.Области.Найти("СредняяЦенаАБ10_4");
			Если Обл <> Неопределено Тогда
				МакетМЧБ.УдалитьОбласть(Обл, ТипСмещенияТабличногоДокумента.ПоВертикали);
			КонецЕсли;
		КонецЕсли;
		...
		// Особая обработка: случай превышения формата Число(8,4).
		//
		СредняяЦена = ТабличныйДокумент.Область("П000310042003").Значение;
		МинимальноеЗначениеПревышенияМаксимальногоВозможногоЗначенияСреднейЦеныДТ = 10000;
		Если СредняяЦена >= МинимальноеЗначениеПревышенияМаксимальногоВозможногоЗначенияСреднейЦеныДТ Тогда
			Обл = МакетМЧБ.Области.Найти("СредняяЦенаДТ8_4");
			Если Обл <> Неопределено Тогда
				МакетМЧБ.УдалитьОбласть(Обл, ТипСмещенияТабличногоДокумента.ПоВертикали);
				НовСтр = Показатели.Добавить();
				НовСтр.Имя = "П100310042003";
				НовСтр.Значение = СредняяЦена;
				НовСтр.ЗначениеМЧБ = СредняяЦена;
			КонецЕсли;
		Иначе
			Обл = МакетМЧБ.Области.Найти("СредняяЦенаДТ10_4");
			Если Обл <> Неопределено Тогда
				МакетМЧБ.УдалитьОбласть(Обл, ТипСмещенияТабличногоДокумента.ПоВертикали);
			КонецЕсли;
		КонецЕсли;
		
		МасИменПок4_4 = Новый Массив;
		МасИменПок4_4.Добавить("П000310026003");
		МасИменПок4_4.Добавить("П000310042003");
		Для каждого ИмяПок Из МасИменПок4_4 Цикл
			Пок = Показатели.Найти(ИмяПок, "Имя");
			Если Пок <> Неопределено И Пок.Значение = 0 Тогда
				Пок.Значение = "0---.0---";
				Пок.ЗначениеМЧБ = Пок.Значение;
		...
		МасИменПок10_3.Добавить("П000310027003");
		МасИменПок10_3.Добавить("П000310028003");
		МасИменПок10_3.Добавить("П000310030003");
		МасИменПок10_3.Добавить("П000310043003");
		МасИменПок10_3.Добавить("П000310044003");
		Для каждого ИмяПок Из МасИменПок10_3 Цикл
			Пок = Показатели.Найти(ИмяПок, "Имя");
			Если Пок <> Неопределено И Пок.Значение = 0 Тогда
				Пок.Значение = "0---------.0--";
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	ДанныеКС.Вставить("Раздел3_ТНП"	, Раздел3ЕстьТНП());
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеФормы"	, "2020Кв2");
...
Процедура СформироватьСлужебныеДанные()
	...
	РазделыПоказателей.Вставить("Р32", "Раздел31");
	РазделыПоказателей.Вставить("Р33", "Раздел31");
...
&НаСервере
Функция Раздел3ЕстьТНП()
	
	Перем Показатели;
	
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р31С033", "П000310003003");
	ИменаПоказателей.Вставить("Р31С034", "П000310003004");
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			...
			ЗаполнитьПоказатель(Показатели.Р31С033, СтраницаПодраздела);	
			ЗаполнитьПоказатель(Показатели.Р31С034, СтраницаПодраздела);
			
			ЕстьТНП = ЕстьТНП ИЛИ (Показатели.Р31С033.Значение = ДанныеКС.МесОтчетаСтр
				И Показатели.Р31С034.Значение = ДанныеКС.ГодОтчетаСтр);
...
Процедура ПроверкаКС_2_05()
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма акциза к уплате в стр.030 Раздела 1 = стр.015 ';
		...
		НСтр("ru = 'Раздела 2, если Раздел 3 не заполнен';
			|en = 'Раздела 2, если Раздел 3 не заполнен'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2';
													|en = '%1 = %2'"));
	...
	Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р01С030.Итог, 
		Показатели.Р02С015.Итог);
	ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_2_06()
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма акциза к уменьшению в стр.040 Раздела 1 = стр.020 ';
		...
		НСтр("ru = 'Раздела 3 - стр.015 Раздела 2, если в стр.030 подр.3.1 Раздела 3 отсутствует текущий налоговый ';
			|en = 'Раздела 3 - стр.015 Раздела 2, если в стр.030 подр.3.1 Раздела 3 отсутствует текущий налоговый '") +
		НСтр("ru = 'период, стр.025 Раздела 3 = 0 и стр.020 Раздела 3 > стр.015 Раздела 2';
			|en = 'период, стр.025 Раздела 3 = 0 и стр.020 Раздела 3 > стр.015 Раздела 2'"));
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		...
		ЗаполнитьПоказатель(Показатели.Р03С025, СтраницаРаздела);
	КонецЦикла;
	
	РасчетнаяСумма = Показатели.Р03С020.Итог - Показатели.Р02С015.Итог;
	Настройки.ЭтоОшибка = ДанныеКС.РазделЗаполнен_Раздел3 И НЕ ДанныеКС.Раздел3_ТНП И Показатели.Р03С025.Итог = 0 
		И Показатели.Р03С020.Итог > Показатели.Р02С015.Итог И НЕ Показатели.Р01С040.Итог = РасчетнаяСумма;
	Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р01С040.Итог, 
		Показатели.Р03С020.Итог, Показатели.Р02С015.Итог, РасчетнаяСумма);
	ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_2_07()
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма акциза к уплате в стр.030 Раздела 1 = стр.015 ';
														|en = 'Сумма акциза к уплате в стр.030 Раздела 1 = стр.015 '") +
		НСтр("ru = 'Раздела 2 - стр.020 Раздела 3, если в стр.030 подр.3.1 Раздела 3 отсутствует текущий налоговый ';
			|en = 'Раздела 2 - стр.020 Раздела 3, если в стр.030 подр.3.1 Раздела 3 отсутствует текущий налоговый '") +
		НСтр("ru = 'период, стр.025 Раздела 3 = 0 и стр.020 Раздела 3 < стр.015 Раздела 2';
			|en = 'период, стр.025 Раздела 3 = 0 и стр.020 Раздела 3 < стр.015 Раздела 2'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 - %3 Итог [%4]';
													|en = '%1 = %2 - %3 Итог [%4]'"));
	...
	РасчетнаяСумма = Показатели.Р02С015.Итог - Показатели.Р03С020.Итог;
	Настройки.ЭтоОшибка = ДанныеКС.РазделЗаполнен_Раздел3 И НЕ ДанныеКС.Раздел3_ТНП И Показатели.Р03С025.Итог = 0 
		И Показатели.Р03С020.Итог < Показатели.Р02С015.Итог И НЕ Показатели.Р01С030.Итог = РасчетнаяСумма;
	Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р01С030.Итог, 
		Показатели.Р02С015.Итог, Показатели.Р03С020.Итог, РасчетнаяСумма);
	ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_2_08()
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма акциза к уплате в стр.030 Раздела 1 = стр.025 ';
		...
		НСтр("ru = 'Раздела 3, если в стр.030 подр.3.1 Раздела 3 присутствует текущий налоговый период и стр.020 ';
			|en = 'Раздела 3, если в стр.030 подр.3.1 Раздела 3 присутствует текущий налоговый период и стр.020 '") +
		НСтр("ru = 'Раздела 3 = 0';
			|en = 'Раздела 3 = 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2';
													|en = '%1 = %2'"));
	...
	Настройки.ЭтоОшибка = ДанныеКС.РазделЗаполнен_Раздел3 И ДанныеКС.Раздел3_ТНП И Показатели.Р03С020.Итог = 0 
		И НЕ Показатели.Р01С030.Итог = Показатели.Р03С025.Итог;
	Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р01С030.Итог, 
		Показатели.Р03С025.Итог);
	ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_2_09()
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма акциза к уменьшению в стр.040 Раздела 1 = стр.020 ';
														|en = 'Сумма акциза к уменьшению в стр.040 Раздела 1 = стр.020 '") +
		НСтр("ru = 'Раздела 3, если в стр.030 подр.3.1 Раздела 3 присутствует текущий налоговый период и стр.025 ';
			|en = 'Раздела 3, если в стр.030 подр.3.1 Раздела 3 присутствует текущий налоговый период и стр.025 '") +
		НСтр("ru = 'Раздела 3 = 0';
			|en = 'Раздела 3 = 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2';
													|en = '%1 = %2'"));
	...
	Настройки.ЭтоОшибка = ДанныеКС.РазделЗаполнен_Раздел3 И ДанныеКС.Раздел3_ТНП И Показатели.Р03С025.Итог = 0 
		И НЕ Показатели.Р01С040.Итог = Показатели.Р03С020.Итог;
	Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р01С040.Итог, 
		Показатели.Р03С020.Итог);
	ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_4_07()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.200 НК РФ';
													|en = 'ст.200 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Признак величины Кдемп в стр.065 подр.3.1 Раздела 3 = 0, ';
														|en = 'Признак величины Кдемп в стр.065 подр.3.1 Раздела 3 = 0, '") +
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р31С065", "П000310006503");
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			...
			Настройки.ЭтоОшибка = Показатели.Р31С070.Значение = 0 И НЕ Показатели.Р31С065.Значение = 0;
			Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
				Показатели.Р31С065.Значение, Показатели.Р31С070.Значение);
			ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_4_08()
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			...
			Настройки.ЭтоОшибка = НЕ Показатели.Р31С070.Значение = 0 И Показатели.Р31С060.Значение = 0;
			Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
				Показатели.Р31С060.Значение, Показатели.Р31С070.Значение);
			ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_4_09()
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма акциза в стр.080 подр.3.1 Раздела 3 = стр.060 ';
		...
		НСтр("ru = '+ стр.070 подр.3.1 Раздела 3, если стр.065 подр.3.1 Раздела 3 = 1';
			|en = '+ стр.070 подр.3.1 Раздела 3, если стр.065 подр.3.1 Раздела 3 = 1'"));
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			...
			РасчетнаяСумма = Показатели.Р31С060.Значение + Показатели.Р31С070.Значение;
			Настройки.ЭтоОшибка = Показатели.Р31С065.Значение = 1 И НЕ Показатели.Р31С080.Значение = РасчетнаяСумма;
			Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
				Показатели.Р31С080.Значение, Показатели.Р31С060.Значение, Показатели.Р31С070.Значение, 
				РасчетнаяСумма, Показатели.Р31С065.Значение);
			ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_4_11()
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Объем бензина (дизтоплива) в стр.130 подр.3.1.1 Раздела 3 или ';
		...
		НСтр("ru = 'стр.290 подр.3.1.2 Раздела 3 <> 0, если стр.070 подр.3.1 Раздела 3 <> 0';
			|en = 'стр.290 подр.3.1.2 Раздела 3 <> 0, если стр.070 подр.3.1 Раздела 3 <> 0'"));
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			...
			Настройки.ЭтоОшибка = НЕ Показатели.Р31С070.Значение = 0 
				И Показатели.Р32С130.Значение = 0 И Показатели.Р33С290.Значение = 0;
			Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
				Показатели.Р32С130.Значение, Показатели.Р33С290.Значение, Показатели.Р31С070.Значение);
			ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_4_12()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.200 НК РФ';
													|en = 'ст.200 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма акциза в стр.090 подр.3.1 Раздела 3 = 0, ';
														|en = 'Сумма акциза в стр.090 подр.3.1 Раздела 3 = 0, '") +
		НСтр("ru = 'если стр.080 подр.3.1 Раздела 3 > 0';
			|en = 'если стр.080 подр.3.1 Раздела 3 > 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = 0, когда %2 > 0';
													|en = '%1 = 0, когда %2 > 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочная сумма акциза';
														|en = 'Ошибочная сумма акциза'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р31С001", "П000310009003");
...
Процедура ПроверкаКС_4_13()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.200 НК РФ';
													|en = 'ст.200 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма акциза в стр.080 подр.3.1 Раздела 3 = 0, ';
														|en = 'Сумма акциза в стр.080 подр.3.1 Раздела 3 = 0, '") +
		НСтр("ru = 'если стр.090 подр.3.1 Раздела 3 > 0';
			|en = 'если стр.090 подр.3.1 Раздела 3 > 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = 0, когда %2 > 0';
													|en = '%1 = 0, когда %2 > 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочная сумма акциза';
														|en = 'Ошибочная сумма акциза'"));
	...
	ИменаПоказателей.Вставить("Р31С002", "П000310009003");
	Настройки.Вставить("ИменаПоказателей", ИменаПоказателей);
...
Процедура ПроверкаКС_4_14()
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма акциза в стр.080 подр.3.1 Раздела 3 = стр.060 ';
														|en = 'Сумма акциза в стр.080 подр.3.1 Раздела 3 = стр.060 '") +
		НСтр("ru = '- стр.070 подр.3.1 Раздела 3, если стр.065 подр.3.1 Раздела 3 = 0 и 0 < стр.070 <= стр.060';
			|en = '- стр.070 подр.3.1 Раздела 3, если стр.065 подр.3.1 Раздела 3 = 0 и 0 < стр.070 <= стр.060'") +
		НСтр("ru = ' подр.3.1 Раздела 3';
			|en = ' подр.3.1 Раздела 3'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 - %3 Итог [%4], когда 0 < %3 <= %2 и %5 = 0';
													|en = '%1 = %2 - %3 Итог [%4], когда 0 < %3 <= %2 и %5 = 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочная сумма акциза';
														|en = 'Ошибочная сумма акциза'"));
	Настройки = ОбработатьНастройки(Заголовки);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			...
			РасчетнаяСумма = Показатели.Р31С060.Значение - Показатели.Р31С070.Значение;
			Настройки.ЭтоОшибка = Показатели.Р31С065.Значение = 0 И Показатели.Р31С070.Значение > 0 
				И Показатели.Р31С070.Значение <= Показатели.Р31С060.Значение И НЕ Показатели.Р31С080.Значение = РасчетнаяСумма;
			Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
				Показатели.Р31С080.Значение, Показатели.Р31С060.Значение, Показатели.Р31С070.Значение, РасчетнаяСумма, 
				Показатели.Р31С065.Значение);
			ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_4_15()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.200 НК РФ';
													|en = 'ст.200 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Сумма акциза в стр.090 подр.3.1 Раздела 3 = стр.070 ';
														|en = 'Сумма акциза в стр.090 подр.3.1 Раздела 3 = стр.070 '") +
		НСтр("ru = '- стр.060 подр.3.1 Раздела 3, если стр.065 подр.3.1 Раздела 3 = 0 и стр.070 > стр.060 > 0 ';
			|en = '- стр.060 подр.3.1 Раздела 3, если стр.065 подр.3.1 Раздела 3 = 0 и стр.070 > стр.060 > 0 '") +
		НСтр("ru = 'подр.3.1 Раздела 3';
			|en = 'подр.3.1 Раздела 3'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 - %3 Итог [%4], когда %2 > %3 > 0 и %5 = 0';
													|en = '%1 = %2 - %3 Итог [%4], когда %2 > %3 > 0 и %5 = 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочная сумма акциза';
														|en = 'Ошибочная сумма акциза'"));
	Настройки = ОбработатьНастройки(Заголовки);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			...
			РасчетнаяСумма = Показатели.Р31С070.Значение - Показатели.Р31С060.Значение;
			Настройки.ЭтоОшибка = Показатели.Р31С065.Значение = 0 И Показатели.Р31С060.Значение > 0 
				И Показатели.Р31С070.Значение > Показатели.Р31С060.Значение И НЕ Показатели.Р31С090.Значение = РасчетнаяСумма;
			Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
				Показатели.Р31С090.Значение, Показатели.Р31С070.Значение, Показатели.Р31С060.Значение, РасчетнаяСумма, 
				Показатели.Р31С065.Значение);
			ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_4_26()
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		...
		ЗарегистрироватьФормулу(Настройки);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.Р03С020, СтраницаРаздела);
		Показатели.Р31С060.Итог = 0;
		
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			ЗаполнитьИРасшифроватьПоказатель(Показатели.Р31С060, СтраницаПодраздела);
		КонецЦикла;
			
		Настройки.ЭтоОшибка = Показатели.Р03С020.Значение > 0 И НЕ Показатели.Р31С060.Итог > 0;
		Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
			Показатели.Р31С060.Итог, Показатели.Р03С020.Значение);
		ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_5_01()
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Требуется выполнение соотношения: стр.150 подр.3.1.1 ';
		...
		НСтр("ru = 'Раздела 3 = стр.160 - стр.230 подр.3.1.1 Раздела 3, если стр.130 подр.3.1.1 Раздела 3 > 0';
			|en = 'Раздела 3 = стр.160 - стр.230 подр.3.1.1 Раздела 3, если стр.130 подр.3.1.1 Раздела 3 > 0'"));
	...
	ИменаПоказателей.Вставить("Р32С230", "П000310023003");
	ИменаПоказателей.Вставить("Р32С130", "П000310013003");
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			...
			РасчетнаяСумма = Показатели.Р32С160.Значение - Показатели.Р32С230.Значение;
			Настройки.ЭтоОшибка = Показатели.Р32С130.Значение > 0 И НЕ Показатели.Р32С150.Значение = РасчетнаяСумма;
			Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
				Показатели.Р32С150.Значение, Показатели.Р32С160.Значение, Показатели.Р32С230.Значение, РасчетнаяСумма,
...
Процедура ПроверкаКС_5_02()
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Признак показателя Даб в стр.145 подр.3.1.1 Раздела 3 = 1, ';
		...
		НСтр("ru = 'если стр.160 - стр.230 подр.3.1.1 Раздела 3 > 0';
			|en = 'если стр.160 - стр.230 подр.3.1.1 Раздела 3 > 0'"));
...
Процедура ПроверкаКС_5_03_код(Настройки)
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			...
			Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
				Показатели.Р32С145.Значение, Показатели.Р32С160.Значение, Показатели.Р32С230.Значение, РасчетнаяСумма);
			ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_5_05()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.200 НК РФ';
													|en = 'ст.200 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Коэффициент Каб_комп в стр.240 подр.3.1.1 Раздела 3 = 0,75 ';
														|en = 'Коэффициент Каб_комп в стр.240 подр.3.1.1 Раздела 3 = 0,75 '") +
		НСтр("ru = 'если год в стр.030 подр.3.1 Раздела 3 = 2019 и стр.130 подр.3.1.1 Раздела 3 > 0';
			|en = 'если год в стр.030 подр.3.1 Раздела 3 = 2019 и стр.130 подр.3.1.1 Раздела 3 > 0'"));
...
Процедура ПроверкаКС_5_06()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.200 НК РФ';
													|en = 'ст.200 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Коэффициент Каб_комп в стр.240 подр.3.1.1 Раздела 3 = 0,68 ';
														|en = 'Коэффициент Каб_комп в стр.240 подр.3.1.1 Раздела 3 = 0,68 '") +
		НСтр("ru = 'если год в стр.030 подр.3.1 Раздела 3 >= 2020 и стр.130 подр.3.1.1 Раздела 3 > 0';
			|en = 'если год в стр.030 подр.3.1 Раздела 3 >= 2020 и стр.130 подр.3.1.1 Раздела 3 > 0'"));
...
Процедура ПроверкаКС_5_06_код(Настройки)
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р32С240", "П000310024003");
	ИменаПоказателей.Вставить("Р31С032", "П000310003004");
	ИменаПоказателей.Вставить("Р32С130", "П000310013003");
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			...
			Если Настройки.НомерКС = "5.5" Тогда
				Настройки.ЭтоОшибка = Показатели.Р31С032.Значение = "2019" И Показатели.Р32С130.Значение > 0 
					И НЕ Показатели.Р32С240.Значение = 0.75;
			Иначе
				Настройки.ЭтоОшибка = Показатели.Р31С032.Значение >= "2020" И Показатели.Р32С130.Значение > 0 
					И НЕ Показатели.Р32С240.Значение = 0.68;
			КонецЕсли;
			
			Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
				Показатели.Р32С240.Значение, Показатели.Р31С032.ЗначениеСтр, Показатели.Р32С130.Значение);
			ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_5_07()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.200 НК РФ';
													|en = 'ст.200 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Объем сырья в стр.120 подр.3.1 Раздела 3 > 0, ';
														|en = 'Объем сырья в стр.120 подр.3.1 Раздела 3 > 0, '") +
		НСтр("ru = 'если стр.280 подр.3.1.1 Раздела 3 > 0';
			|en = 'если стр.280 подр.3.1.1 Раздела 3 > 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 > 0 когда %2 > 0';
													|en = '%1 > 0 когда %2 > 0'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р31С001", "П000310012003");
	ИменаПоказателей.Вставить("Р32С001", "П000310028003");
	Настройки.Вставить("ИменаПоказателей", ИменаПоказателей);
	
	ПроверкаКС_5_07_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_5_07_код(Настройки)
...
Процедура ПроверкаКС_6_01()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.200 НК РФ';
													|en = 'ст.200 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Требуется выполнение соотношения: стр.310 подр.3.1.2 ';
														|en = 'Требуется выполнение соотношения: стр.310 подр.3.1.2 '") +
		НСтр("ru = 'Раздела 3 = стр.320 - стр.390 подр.3.1.2 Раздела 3, если стр.290 подр.3.1.2 Раздела 3 > 0';
			|en = 'Раздела 3 = стр.320 - стр.390 подр.3.1.2 Раздела 3, если стр.290 подр.3.1.2 Раздела 3 > 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 - %3 Итог [%4], когда %5 > 0';
													|en = '%1 = %2 - %3 Итог [%4], когда %5 > 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не выполнено соотношение';
														|en = 'Не выполнено соотношение'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р33С310", "П000310031003");
	ИменаПоказателей.Вставить("Р33С320", "П000310032003");
	ИменаПоказателей.Вставить("Р33С390", "П000310039003");
	ИменаПоказателей.Вставить("Р33С290", "П000310029003");
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			...
			РасчетнаяСумма = Показатели.Р33С320.Значение - Показатели.Р33С390.Значение;
			Настройки.ЭтоОшибка = Показатели.Р33С290.Значение > 0 И НЕ Показатели.Р33С310.Значение = РасчетнаяСумма;
			Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
				Показатели.Р33С310.Значение, Показатели.Р33С320.Значение, Показатели.Р33С390.Значение, РасчетнаяСумма,
				Показатели.Р33С290.Значение);
			ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_6_02()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.200 НК РФ';
													|en = 'ст.200 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Признак показателя Ддт в стр.305 подр.3.1.2 Раздела 3 = 1, ';
														|en = 'Признак показателя Ддт в стр.305 подр.3.1.2 Раздела 3 = 1, '") +
		НСтр("ru = 'если стр.320 - стр.390 подр.3.1.2 Раздела 3 > 0';
			|en = 'если стр.320 - стр.390 подр.3.1.2 Раздела 3 > 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = 1, когда %2 - %3 Итог [%4] > 0';
													|en = '%1 = 1, когда %2 - %3 Итог [%4] > 0'"));
...
Процедура ПроверкаКС_6_03()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.200 НК РФ';
													|en = 'ст.200 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Признак показателя Ддт в стр.305 подр.3.1.2 Раздела 3 = 0, ';
														|en = 'Признак показателя Ддт в стр.305 подр.3.1.2 Раздела 3 = 0, '") +
		НСтр("ru = 'если стр.320 - стр.390 подр.3.1.2 Раздела 3 <= 0';
			|en = 'если стр.320 - стр.390 подр.3.1.2 Раздела 3 <= 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = 0, когда %2 - %3 Итог [%4] <= 0';
													|en = '%1 = 0, когда %2 - %3 Итог [%4] <= 0'"));
...
Процедура ПроверкаКС_6_03_код(Настройки)
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р33С305", "П000310030503");
	ИменаПоказателей.Вставить("Р33С320", "П000310032003");
	ИменаПоказателей.Вставить("Р33С390", "П000310039003");
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			...
			Если Настройки.НомерКС = "6.2" Тогда
				Настройки.ЭтоОшибка = РасчетнаяСумма > 0 И НЕ Показатели.Р33С305.Значение = 1;;
			Иначе
				Настройки.ЭтоОшибка = НЕ РасчетнаяСумма > 0 И НЕ Показатели.Р33С305.Значение = 0;;
			КонецЕсли;
			
			Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
				Показатели.Р33С305.Значение, Показатели.Р33С320.Значение, Показатели.Р33С390.Значение, РасчетнаяСумма);
			ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_6_05()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.200 НК РФ';
													|en = 'ст.200 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Коэффициент Кдт_комп в стр.400 подр.3.1.2 Раздела 3 = 0,7 ';
														|en = 'Коэффициент Кдт_комп в стр.400 подр.3.1.2 Раздела 3 = 0,7 '") +
		НСтр("ru = 'если год в стр.030 подр.3.1 Раздела 3 = 2019 и стр.290 подр.3.1.2 Раздела 3 > 0';
			|en = 'если год в стр.030 подр.3.1 Раздела 3 = 2019 и стр.290 подр.3.1.2 Раздела 3 > 0'"));
...
Процедура ПроверкаКС_6_06()
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'ст.200 НК РФ';
													|en = 'ст.200 НК РФ'"));
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Коэффициент Кдт_комп в стр.400 подр.3.1.2 Раздела 3 = 0,65 ';
														|en = 'Коэффициент Кдт_комп в стр.400 подр.3.1.2 Раздела 3 = 0,65 '") +
		НСтр("ru = 'если год в стр.030 подр.3.1 Раздела 3 >= 2020 и стр.290 подр.3.1.2 Раздела 3 > 0';
			|en = 'если год в стр.030 подр.3.1 Раздела 3 >= 2020 и стр.290 подр.3.1.2 Раздела 3 > 0'"));
...
Процедура ПроверкаКС_6_06_код(Настройки)
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р33С400", "П000310040003");
	ИменаПоказателей.Вставить("Р31С032", "П000310003004");
	ИменаПоказателей.Вставить("Р33С290", "П000310029003");
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел3 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел31 Цикл
			...
			Если Настройки.НомерКС = "6.5" Тогда
				Настройки.ЭтоОшибка = Показатели.Р31С032.Значение = "2019" И Показатели.Р33С290.Значение > 0 
					И НЕ Показатели.Р33С400.Значение = 0.7;
			Иначе
				Настройки.ЭтоОшибка = Показатели.Р31С032.Значение >= "2020" И Показатели.Р33С290.Значение > 0 
					И НЕ Показатели.Р33С400.Значение = 0.65;
			КонецЕсли;
			
			Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
				Показатели.Р33С400.Значение, Показатели.Р31С032.ЗначениеСтр, Показатели.Р33С290.Значение);
			ДополнитьСтруктуруОшибок(Настройки);
...
Процедура ПроверкаКС_6_07()
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Объем сырья в стр.120 подр.3.1 Раздела 3 > 0, ';
														|en = 'Объем сырья в стр.120 подр.3.1 Раздела 3 > 0, '") +
		НСтр("ru = 'если стр.440 подр.3.1.2 Раздела 3 > 0';
			|en = 'если стр.440 подр.3.1.2 Раздела 3 > 0'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 > 0 когда %2 > 0';
													|en = '%1 > 0 когда %2 > 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочное значение объема сырья';
														|en = 'Ошибочное значение объема сырья'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р31С001", "П000310012003");
	ИменаПоказателей.Вставить("Р32С001", "П000310044003");
	Настройки.Вставить("ИменаПоказателей", ИменаПоказателей);
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	ИначеЕсли Ячейка.Раздел = "Раздел21" ИЛИ Ячейка.Раздел = "Раздел31" Тогда
							...
							Для ИндПодчРаздела = 0 По КолПодчРазделов - 1 Цикл
								...
								Если СтруктураПодчРаздела.Свойство("Многостраничность") Тогда
									...
									Для НомерСтраницыПодчРаздела = 1 По КолСтрокВДеревеДанныхПодчРаздела Цикл
										...
										ИначеЕсли ИмяПодчРаздела = "Раздел31" Тогда
											
											ДанныеСтраницы = ДеревоДанныхРаздела.Строки[НомерСтраницыРаздела - 1].Строки[
											ИндПодчРаздела].Строки[НомерСтраницыПодчРаздела - 1].Данные;
											
											СтруктПарам = Новый Структура;
											СтруктПарам.Вставить("КодПериода", ДанныеСтраницы["П000310003003"]);
											СтруктПарам.Вставить("ГодПериода", ДанныеСтраницы["П000310003004"]);
											СтруктПарам.Вставить("НомерСтраницы", НомерСтраницыПодчРаздела);
											
											ЭлементПодчРаздел.КолонкаРазделыОтчета = ПредставлениеСтраницы_Раздел31(СтруктПарам);
...
&НаКлиентеНаСервереБезКонтекста
Функция ПредставлениеСтраницы_Раздел31(СтруктураПараметров)
	
	НомерСтраницы = СтруктураПараметров.НомерСтраницы;
	КодПериода = СокрЛП(СтруктураПараметров.КодПериода);
	ГодПериода = СокрЛП(СтруктураПараметров.ГодПериода);
	
	Результат = "";
	Если ЗначениеЗаполнено(КодПериода) И ЗначениеЗаполнено(ГодПериода) Тогда
		НачДата = НачалоМесяца(Дата(ГодПериода + КодПериода + "01"));
		Результат = ПредставлениеПериода(НачДата, КонецМесяца(НачДата), "ФП = Истина");
	КонецЕсли;
	
	Возврат "Раздел 3.1. " + ?(ЗначениеЗаполнено(Результат), "Сведения за " + Результат, "Стр. " + Формат(НомерСтраницы, "ЧГ="));
