...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0609040";
...
Процедура СформироватьСпискиВыбора()
	...
	ТаблСписка.Колонки.Добавить("Название", , ,115);
	...
	НовСтрока = ТаблСписка.Добавить();
	НовСтрока.Код = "104";
	НовСтрока.Название = "Мероприятия по воспроизводству лесов и лесоразведению, проведенные в границах Байкальской природной территории";
	
	НовСтрока = ТаблСписка.Добавить();
	НовСтрока.Код = "105";
	НовСтрока.Название = "Мероприятия по воспроизводству лесов и лесоразведению, проведенные за пределами Байкальской природной территории";
	
	КоллекцияСписковВыбора.Вставить("Место", ТаблСписка);
	
	СвойстваПоказателей.Очистить();
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникМесто",
	512, , "Выбор кода места мероприятия", КоллекцияСписковВыбора["Место"]);
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для НомКол = 3 По 4 Цикл
		Области["П00001000020" + НомКол].Значение = Области["П00001000030" + НомКол].Значение
			+ Области["П00001000040" + НомКол].Значение;
		Области["П00001000010" + НомКол].Значение = Области["П00001000020" + НомКол].Значение
			+ Области["П00001000080" + НомКол].Значение + Области["П00001000110" + НомКол].Значение;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П000010000203"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000010000203"].Защита   = Не ОтклАвтоРасчетЗначение;
	...
	мФормаОтчета.Области["П000010000204"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000010000204"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста", "Форма 1-ЛХ");
	СтруктураПараметров.Вставить("ОриентацияЛиста", "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы", "1-ЛХ");
	...
	НК = СтруктураРеквизитовФормы.мПечатныеФормы[0].Значение.Области.Найти("НеПечатаемаяЧасть");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.Место) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			"Не указано место проведения мероприятий!", ИмяПоляТаблДокумента, "ОргКодМесто");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("Место", СокрЛП(ТабДокумент.ОргКодМесто));
