...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "13/03/2025";
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РасчСРВыр");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "NO_RASSVIR"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	ЗаполненСРВПоПредыдущимГодам = Ложь;
	Для НомСтроки = 11 По 19 Цикл
		...
		Если ЗначениеЗаполнено(Данные[ИмяПок_Сум]) ИЛИ ЗначениеЗаполнено(ТекГод) Тогда
			ЗаполненСРВПоПредыдущимГодам = Истина;
		...
		КонецЕсли;
	КонецЦикла;
	Если НЕ ЗаполненСРВПоПредыдущимГодам Тогда
		СообщитьВТС(
		"Не задан совокупный размер выручки по предыдущим годам (должна быть заполнена хотя бы одна из строк 011-019)",
	...
	ТекГод = СокрЛП(Данные[ИмяПок_Год]);
	Если НЕ ЗначениеЗаполнено(ТекГод) Тогда
		СообщитьВТС("В строке 030 не задан год, с которого начинается расчет совокупного размера выручки",
		"Раздел1", ИмяПок_Год, "1");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ТекГод) < 4 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ТекГод) Тогда
		СообщитьВТС("В строке 030 задано значение года, с которого начинается расчет совокупного размера выручки,"
		+ " не состоящее из 4 цифр", "Раздел1", ИмяПок_Год, "1");
		СтатусПроверки = Ложь;
	Иначе
		ТекГод_Число = Число(ТекГод);
		Если ТекГод_Число > ПараметрыВыгрузки.ОтчетГод_Число Тогда
			СообщитьВТС("В строке 030 значение года, с которого начинается расчет совокупного размера выручки,"
			+ " не должно превышать значение отчетного года", "Раздел1", ИмяПок_Год, "1");
			СтатусПроверки = Ложь;
		Иначе
	...
	ТекГод = СокрЛП(Данные[ИмяПок_Год]);
	Если НЕ ЗначениеЗаполнено(ТекГод) Тогда
		СообщитьВТС("В строке 030 не задан год, которым заканчивается расчет совокупного размера выручки",
		"Раздел1", ИмяПок_Год, "1");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ТекГод) < 4 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ТекГод) Тогда
		СообщитьВТС("В строке 030 задано значение года, которым заканчивается расчет совокупного размера выручки,"
		+ " не состоящее из 4 цифр", "Раздел1", ИмяПок_Год, "1");
		СтатусПроверки = Ложь;
	Иначе
		ТекГод_Число = Число(ТекГод);
		Если ТекГод_Число <> ПараметрыВыгрузки.ОтчетГод_Число Тогда
			СообщитьВТС("В строке 030 значение года, которым заканчивается расчет совокупного размера выручки,"
			+ " должно быть равно значению отчетного года", "Раздел1", ИмяПок_Год, "1");
			СтатусПроверки = Ложь;
		Иначе
	...
	Если КорректноеЗначениеГодаНачалаРасчета > 0 И КорректноеЗначениеГодаОкончанияРасчета > 0
		...
		СообщитьВТС("В строке 030 значение года, с которого начинается расчет совокупного размера выручки,"
		+ " не должно превышать значение года, которым заканчивается расчет совокупного размера выручки",
...
Функция ВыгрузкаОтчета(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РасчетСовокупногоРазмераВыручки");
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	ПрефиксИменМакетов = "ПечатныйБланк2018Кв4_";
