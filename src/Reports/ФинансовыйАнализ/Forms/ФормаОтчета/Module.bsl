...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	Форма.Заголовок = СтрШаблон(НСтр("ru = 'Финансовый анализ %1 за %2 год';
									|en = 'Financial analysis %1 for %2'"),
		БухгалтерскиеОтчетыВызовСервера.ПолучитьТекстОрганизация(Отчет.Организация, Ложь, Отчет.Период),
...
Процедура УстановитьВыбранныйПериод()
	...
	КонецПрошлогоМесяца = НачалоМесяца(ОбщегоНазначенияКлиент.ДатаСеанса())-1;
	...
	Если Отчет.Период > КонецПрошлогоМесяца Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			...
			|Отчет можно сформировать только по %2.';
			|en = 'Date was changed to %1.  
			|Report can be generated by %2 only.'"), Формат(КонецПрошлогоМесяца, "ДЛФ=D"), НРег(Формат(КонецПрошлогоМесяца, "ДФ=MMMM"))));
	КонецЕсли;
	...
	РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Формирование отчета: Финансовый анализ: %1';
				|en = 'Generate report: Financial analysis: %1'"),
			Отчет.Организация));
