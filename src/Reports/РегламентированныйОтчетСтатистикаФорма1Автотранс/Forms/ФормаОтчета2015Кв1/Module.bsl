...
Процедура ПоказатьПериод()
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета2.Значение = "за январь - "
	+ НРег(Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'ММММ гггг'")) + " г.";
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если (ПараметрыВыгрузки["П000001000105_Ч"] <> 0 Или ПараметрыВыгрузки["П000001000106_Ч"] <> 0)
	...
	Для НомКол = 3 По 6 Цикл
		...
		КонецЕсли;
	КонецЦикла;
	// По всем графам: стр. 21 ≥ стр. 23.
	Для НомКол = 3 По 6 Цикл
		Если ПараметрыВыгрузки["П0000020021" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] < ПараметрыВыгрузки["П0000020023" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по всем графам раздела 2, значение в строке 21 должно быть больше или равняться значению в строке 23!",
