...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "06/05/2013";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ВидОрг1.Значение = 1;
	ПолеТабличногоДокументаФормаОтчета.Области.ВидОрг2.Значение = 2;
	ПолеТабличногоДокументаФормаОтчета.Области.ВидОрг3.Значение = 3;
	...
	Если Не БезОткрытияФормы Тогда
		// В случае подачи отчета не за год, установим защиту и цвет по умолчанию в Форме 1-автотранс для граф 6, 7 раздела 2.
		ДоступностьРазделов();
	КонецЕсли;
...
Процедура ДоступностьРазделов()
	...
	Если Не КонецГода(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета Тогда
		Для НомСтр = 21 По 23 Цикл
			ПолеТабличногоДокументаФормаОтчета.Области["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "06"].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "06"].Защита = Истина;
			ПолеТабличногоДокументаФормаОтчета.Области["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "07"].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "07"].Защита = Истина;
		КонецЦикла;
	КонецЕсли;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если СтруктураРеквизитовФормы.мСкопированаФорма <> Неопределено Тогда
		Если Не КонецГода(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета Тогда
			Для НомСтр = 21 По 23 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "06"].Значение = 0;
				ПолеТабличногоДокументаФормаОтчета.Области["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "07"].Значение = 0;
			КонецЦикла;
			РасчетНаСервере("", "");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 6) = "ВидОрг" Тогда
		...
		Суффикс = Прав(Область.Имя, 1);
		ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["ВидОрг" + Суффикс].Значение =
		?(ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["ВидОрг" + Суффикс].Значение = "X", Суффикс, "X");
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	// Раздел 2.
	// По всем строкам: гр. 3 равна сумме граф с 4 по 7.
	Для НомСтр = 21 По 23 Цикл
		ТекСумма = 0;
		Для НомКол = 4 По 7 Цикл
			ТекСумма = ТекСумма
			+ Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Значение = ТекСумма;
	КонецЦикла;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабличноеПоле.Области.ВидОрг1.Значение = 1;
	ВыбТабличноеПоле.Области.ВидОрг2.Значение = 2;
	ВыбТабличноеПоле.Области.ВидОрг3.Значение = 3;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// Раздел 1.
	// По всем графам:
	// если стр. 11 ≠ 0, то стр. 14 ≠ 0;
	// если стр. 12 ≠ 0, то стр. 15 ≠ 0;
	// если стр. 13 ≠ 0, то стр. 16 ≠ 0;
	Для НомКол = 5 По 8 Цикл
		Если ПараметрыВыгрузки["П0000010011" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] <> 0
			И ПараметрыВыгрузки["П0000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] = 0 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по всем графам раздела 1, если строка 11 не равна 0, то строка 14 не равна 0!",
	...
	КонецЦикла;
	Для НомКол = 5 По 8 Цикл
		Если ПараметрыВыгрузки["П0000010012" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] <> 0
			И ПараметрыВыгрузки["П0000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] = 0 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по всем графам раздела 1, если строка 12 не равна 0, то строка 15 не равна 0!",
	...
	КонецЦикла;
	Для НомКол = 5 По 8 Цикл
		Если ПараметрыВыгрузки["П0000010013" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] <> 0
			И ПараметрыВыгрузки["П0000010016" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] = 0 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по всем графам раздела 1, если строка 13 не равна 0, то строка 16 не равна 0!",
	...
	// Гр. 5 ≥ Гр. 7;
	// Гр. 6 ≥ Гр. 8.
	Для НомСтр = 11 По 16 Цикл
		Если ПараметрыВыгрузки["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "05_Ч"]
			< ПараметрыВыгрузки["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "07_Ч"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по всем строкам раздела 1 графа 5 больше или равна графе 7!",
	...
	КонецЦикла;
	Для НомСтр = 11 По 16 Цикл
		Если ПараметрыВыгрузки["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "06_Ч"]
			< ПараметрыВыгрузки["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "08_Ч"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по всем строкам раздела 1 графа 6 больше или равна графе 8!",
	...
	// Справочно.
	// Если стр. 11 гр. 7 ≠ 0, то стр. 19 ≠ 0.
	Если ПараметрыВыгрузки["П000001001107_Ч"] <> 0 И ПараметрыВыгрузки["П000001001907_Ч"] = 0 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если строка 11 графы 7 раздела 1 не равна 0, то строка 19 не равна 0!",
		ИмяПоляТаблДокумента, "П000001001907");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если стр. 12 гр. 7 ≠ 0, то стр. 18 ≠ 0.
	Если ПараметрыВыгрузки["П000001001207_Ч"] <> 0 И ПараметрыВыгрузки["П000001001807_Ч"] = 0 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если строка 12 графы 7 раздела 1 не равна 0, то строка 18 не равна 0!",
		ИмяПоляТаблДокумента, "П000001001807");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Если стр. 13 гр. 7 ≠ 0, то стр. 17 ≠ 0.
	Если ПараметрыВыгрузки["П000001001307_Ч"] <> 0 И ПараметрыВыгрузки["П000001001707_Ч"] = 0 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
		"Не выполнено условие контроля: если строка 13 графы 7 раздела 1 не равна 0, то строка 17 не равна 0!",
		ИмяПоляТаблДокумента, "П000001001707");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Для НомКол = 3 По 7 Цикл
		Если ПараметрыВыгрузки["П0000020022" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"]
			< ПараметрыВыгрузки["П0000020023" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по всем графам раздела 2 строка 22 больше или равна строке 23!",
...
Функция СформироватьСтруктуруПараметров()
	...
	// По титульному листу, дополнительно.
	ПараметрыВыгрузки.Вставить("ВидОрг1_Ч", ?(ТабДокумент.ВидОрг1 = "X", 0, 1));
	ПараметрыВыгрузки.Вставить("ВидОрг2_Ч", ?(ТабДокумент.ВидОрг2 = "X", 0, 1));
	ПараметрыВыгрузки.Вставить("ВидОрг3_Ч", ?(ТабДокумент.ВидОрг3 = "X", 0, 1));
	// По разделу 1 и Справочно.
	Для НомСтр = 11 По 19 Цикл
		Для НомКол = 5 По 8 Цикл
			Если НомСтр > 16 И НомКол <> 7 Тогда
				Продолжить;
			КонецЕсли;
