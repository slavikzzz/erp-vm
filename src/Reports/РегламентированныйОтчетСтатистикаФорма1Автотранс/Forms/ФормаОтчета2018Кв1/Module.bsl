	...
	ИначеЕсли Область.Имя = "ТипРеспондента" Тогда
		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			...
			ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь,
				ПараметрВводаПоля.ТекстПриВыборе, ПараметрВводаПоля.ТаблицаЗначений, СтандартнаяОбработка, "Название");
		КонецЕсли;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыборЗавершение(ВыбЭлемент, ДополнительныеПараметры1) Экспорт
	...
	Если ВыбЭлемент = Неопределено Тогда
		Если Не ЗначениеЗаполнено(Область.Значение) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю("Не выбран год.");
		КонецЕсли;
	Иначе
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Область.Имя = "ТипРеспондента" Тогда
		
		РезультатКод = РезультатВыбора["Код"];
		РезультатНазвание = РезультатВыбора["Название"];
		Результат = ?(РезультатВыбора["Название"] = "Пустое значение" , "", РезультатНазвание);
		
		Область.Значение = Результат;
		ПолеТабличногоДокументаФормаОтчета.Области["ТипРеспондентаКод"].Значение = РезультатКод;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателейНеПодлежащихОчистке.Добавить(ВыбТабличноеПоле.Области["ТипРеспондента"].Значение,           "ТипРеспондента");
	СписокПоказателейНеПодлежащихОчистке.Добавить(ВыбТабличноеПоле.Области["ТипРеспондентаКод"].Значение,        "ТипРеспондентаКод");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	НО = СтруктураРеквизитовФормы.мПечатныеФормы[0].Значение.Области.Найти("НеПечатаемаяЧасть");
	Если НО <> Неопределено Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы[0].Значение.УдалитьОбласть(НО, ТипСмещенияТабличногоДокумента.ПоВертикали);
	КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникТипРеспондента",
	512, , "Выбор типа респондента", КоллекцияСписковВыбора["ТипРеспондента"]);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ТипРеспондентаКод) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			"Не указан тип респондента!", ИмяПоляТаблДокумента, "ВидОрг1");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров() Экспорт
	...
	Если ТабДокумент.ВидОрг1 = Истина Или ЗначениеЗаполнено(ТабДокумент.ВидОрг1) Тогда 
		ПараметрыВыгрузки.Вставить("ТипРеспондентаКод", "1");
	ИначеЕсли ТабДокумент.ВидОрг2 = Истина Или ЗначениеЗаполнено(ТабДокумент.ВидОрг2) Тогда 
		ПараметрыВыгрузки.Вставить("ТипРеспондентаКод", "2");
	Иначе
		ПараметрыВыгрузки.Вставить("ТипРеспондентаКод", "");
	КонецЕсли;
