...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = "0612005";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		ТекТабличноеПоле.Области["ГородскаяМестность"].Значение = Истина;
		ТекТабличноеПоле.Области["СельскаяМестность"].Значение = Истина;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	// Дополнительно, для стат. отчета 1-ИЖС.
	ПолеТабличногоДокументаФормаОтчета.Области["ГородскаяМестность"].Шрифт = Новый Шрифт(
		"Arial", 8, , , , Не ПолеТабличногоДокументаФормаОтчета.Области["ГородскаяМестность"].Значение);
	ПолеТабличногоДокументаФормаОтчета.Области["СельскаяМестность"].Шрифт  = Новый Шрифт(
		"Arial", 8, , , , Не ПолеТабличногоДокументаФормаОтчета.Области["СельскаяМестность"].Значение);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "ГородскаяМестность" Или Область.Имя = "СельскаяМестность" Тогда
		
		СтандартнаяОбработка = Ложь;
		
		Область.Значение = Не Область.Значение;
		
		Если Не ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["ГородскаяМестность"].Значение
			И Не ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["СельскаяМестность"].Значение Тогда
			Если Область.Имя = "ГородскаяМестность" Тогда
				ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["СельскаяМестность"].Значение = Истина;
			ИначеЕсли Область.Имя = "СельскаяМестность" Тогда
				ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["ГородскаяМестность"].Значение = Истина;
			КонецЕсли;
		КонецЕсли;
		
		// Если значение равно "Ложь", устанавливаем зачеркнутый шрифт.
		ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["ГородскаяМестность"].Шрифт = Новый Шрифт(
			"Arial", 8, , , , Не ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["ГородскаяМестность"].Значение);
		ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["СельскаяМестность"].Шрифт  = Новый Шрифт(
			"Arial", 8, , , , Не ЭтаФорма.ПолеТабличногоДокументаФормаОтчета.Области["СельскаяМестность"].Значение);
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	// Строка 01 = сумме строк (02 + 03 + 04 + 05 + 06 + 07 + 08) по графам 3, 4.
	...
	// Строка 20 = сумме строк (21 + 22 + 23 + 24) по графе 3.
	ТекСумма = 0;
	Для НомСтр = 21 По 24 Цикл
		ТекСумма = ТекСумма + мФормаОтчета.Области["П000002"
		+ Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Значение;
	КонецЦикла;
	мФормаОтчета.Области["П000002002003"].Значение = ТекСумма;
...
Процедура ОчиститьТабличноеПоле(Форма)
	...
	// Дополнительно, для стат. отчета 1-ИЖС.
	Форма.ПолеТабличногоДокументаФормаОтчета.Области["ГородскаяМестность"].Значение = Истина;
	Форма.ПолеТабличногоДокументаФормаОтчета.Области["ГородскаяМестность"].Шрифт    = Новый Шрифт("Arial", 8, , , , Ложь);
	Форма.ПолеТабличногоДокументаФормаОтчета.Области["СельскаяМестность"].Значение  = Истина;
	Форма.ПолеТабличногоДокументаФормаОтчета.Области["СельскаяМестность"].Шрифт     = Новый Шрифт("Arial", 8, , , , Ложь);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого Обл Из ОбластиСтроки Цикл
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-ИЖС");
	КонецЦикла;
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2014Кв1");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПараметрыВыгрузки.ТипПоселения = "0" Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не указан тип местности (""городская местность"" или ""сельская местность"")!", ИмяПоляТаблДокумента, "ГородскаяМестность");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	// КОНТРОЛЬ показателей формы (Указания по заполнению Формы 1-ИЖС).
	
	// Строка 01 = сумме строк (09 + 10 + 11 + 12 + 13) по графе 3.
	Если Не (ПараметрыВыгрузки["ПР00001000103"]
		= ПараметрыВыгрузки["ПР00001000903"]
		+ ПараметрыВыгрузки["ПР00001001003"]
		+ ПараметрыВыгрузки["ПР00001001103"]
		+ ПараметрыВыгрузки["ПР00001001203"]
		+ ПараметрыВыгрузки["ПР00001001303"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по гр. 3 значение в стр. 01 должно быть равно сумме значений в строках 09, 10, 11, 12, 13!",
			ИмяПоляТаблДокумента, "П000001000103");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Строка 01 >= сумме строк (09 + 10 + 11 + 12 + 13) по графе 4.
	Если Не (ПараметрыВыгрузки["ПР00001000104"]
		>= ПараметрыВыгрузки["ПР00001000904"]
		 + ПараметрыВыгрузки["ПР00001001004"]
		 + ПараметрыВыгрузки["ПР00001001104"]
		 + ПараметрыВыгрузки["ПР00001001204"]
		 + ПараметрыВыгрузки["ПР00001001304"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по гр. 4 значение в стр. 01 должно быть больше или равно сумме значений в строках 09, 10, 11, 12, 13!",
			ИмяПоляТаблДокумента, "П000001000104");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Строка 01 >= строки 14 по графам 3, 4.
	Для НомКол = 3 По 4 Цикл
		Если Не (ПараметрыВыгрузки["ПР000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			>= ПараметрыВыгрузки["ПР000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по гр. " + Формат(НомКол, "ЧГ=")
					+ " значение в стр. 01 должно быть больше или равно значению в строке 14!",
				ИмяПоляТаблДокумента, "П0000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Строка 01 >= строки 15 по графам 3, 4.
	Для НомКол = 3 По 4 Цикл
		Если Не (ПараметрыВыгрузки["ПР000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			>= ПараметрыВыгрузки["ПР000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по гр. " + Формат(НомКол, "ЧГ=")
					+ " значение в стр. 01 должно быть больше или равно значению в строке 15!",
				ИмяПоляТаблДокумента, "П0000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Строка 01 >= строки 16 по графам 3, 4.
	Для НомКол = 3 По 4 Цикл
		Если Не (ПараметрыВыгрузки["ПР000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			>= ПараметрыВыгрузки["ПР000010016" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по гр. " + Формат(НомКол, "ЧГ=")
					+ " значение в стр. 01 должно быть больше или равно значению в строке 16!",
				ИмяПоляТаблДокумента, "П0000010016" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Строка 01 >= строки 17 по графам 3, 4.
	Для НомКол = 3 По 4 Цикл
		Если Не (ПараметрыВыгрузки["ПР000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			>= ПараметрыВыгрузки["ПР000010017" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по гр. " + Формат(НомКол, "ЧГ=")
					+ " значение в стр. 01 должно быть больше или равно значению в строке 17!",
				ИмяПоляТаблДокумента, "П0000010017" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Строка 01 >= строки 18 по графам 3, 4.
	Для НомКол = 3 По 4 Цикл
		Если Не (ПараметрыВыгрузки["ПР000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			>= ПараметрыВыгрузки["ПР000010018" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по гр. " + Формат(НомКол, "ЧГ=")
					+ " значение в стр. 01 должно быть больше или равно значению в строке 18!",
				ИмяПоляТаблДокумента, "П0000010018" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Строка 01 >= строки 19 по графам 3, 4.
	Для НомКол = 3 По 4 Цикл
		Если Не (ПараметрыВыгрузки["ПР000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			>= ПараметрыВыгрузки["ПР000010019" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по гр. " + Формат(НомКол, "ЧГ=")
					+ " значение в стр. 01 должно быть больше или равно значению в строке 19!",
				ИмяПоляТаблДокумента, "П0000010019" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Строка 15 >= строки 14 по графам 3, 4.
	Для НомКол = 3 По 4 Цикл
		Если Не (ПараметрыВыгрузки["ПР000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			>= ПараметрыВыгрузки["ПР000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по гр. " + Формат(НомКол, "ЧГ=")
					+ " значение в стр. 15 должно быть больше или равно значению в строке 14!",
				ИмяПоляТаблДокумента, "П0000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Строка 16 >= строки 14 по графам 3, 4.
	Для НомКол = 3 По 4 Цикл
		Если Не (ПараметрыВыгрузки["ПР000010016" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			>= ПараметрыВыгрузки["ПР000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по гр. " + Формат(НомКол, "ЧГ=")
					+ " значение в стр. 16 должно быть больше или равно значению в строке 14!",
				ИмяПоляТаблДокумента, "П0000010016" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Строка 17 >= строки 14 по графам 3, 4.
	Для НомКол = 3 По 4 Цикл
		Если Не (ПараметрыВыгрузки["ПР000010017" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			>= ПараметрыВыгрузки["ПР000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по гр. " + Формат(НомКол, "ЧГ=")
					+ " значение в стр. 17 должно быть больше или равно значению в строке 14!",
				ИмяПоляТаблДокумента, "П0000010017" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Строка 18 >= строки 14 по графам 3, 4.
	Для НомКол = 3 По 4 Цикл
		Если Не (ПараметрыВыгрузки["ПР000010018" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			>= ПараметрыВыгрузки["ПР000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по гр. " + Формат(НомКол, "ЧГ=")
					+ " значение в стр. 18 должно быть больше или равно значению в строке 14!",
				ИмяПоляТаблДокумента, "П0000010018" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Если Строка 14 = строке 01 по графам 3, 4,то:
	//  - строки 15, 16, 17, 18 = строке 14 по графам 3, 4;
	//  - строка 19 по графам 3, 4 = 0.
	Для НомКол = 3 По 4 Цикл
		Если ПараметрыВыгрузки["ПР000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			= ПараметрыВыгрузки["ПР000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] Тогда
			Если Не (ПараметрыВыгрузки["ПР000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
				= ПараметрыВыгрузки["ПР000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: если по гр. " + Формат(НомКол, "ЧГ=")
						+ " значение в стр. 14 равно значению в стр. 01, то значение в стр. 15 должно быть равно значению в строке 14!",
					ИмяПоляТаблДокумента, "П0000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
				СтатусПроверки = Ложь;
			ИначеЕсли Не (ПараметрыВыгрузки["ПР000010016" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
				= ПараметрыВыгрузки["ПР000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: если по гр. " + Формат(НомКол, "ЧГ=")
						+ " значение в стр. 14 равно значению в стр. 01, то значение в стр. 16 должно быть равно значению в строке 14!",
					ИмяПоляТаблДокумента, "П0000010016" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
				СтатусПроверки = Ложь;
			ИначеЕсли Не (ПараметрыВыгрузки["ПР000010017" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
				= ПараметрыВыгрузки["ПР000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: если по гр. " + Формат(НомКол, "ЧГ=")
						+ " значение в стр. 14 равно значению в стр. 01, то значение в стр. 17 должно быть равно значению в строке 14!",
					ИмяПоляТаблДокумента, "П0000010017" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
				СтатусПроверки = Ложь;
			ИначеЕсли Не (ПараметрыВыгрузки["ПР000010018" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
				= ПараметрыВыгрузки["ПР000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: если по гр. " + Формат(НомКол, "ЧГ=")
						+ " значение в стр. 14 равно значению в стр. 01, то значение в стр. 18 должно быть равно значению в строке 14!",
					ИмяПоляТаблДокумента, "П0000010018" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
				СтатусПроверки = Ложь;
			ИначеЕсли Не (ПараметрыВыгрузки["ПР000010019" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = 0) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: если по гр. " + Формат(НомКол, "ЧГ=")
						+ " значение в стр. 14 равно значению в стр. 01, то значение в стр. 19 должно быть равно нулю!",
					ИмяПоляТаблДокумента, "П0000010019" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	// Если Строка 19 = строке 01, то строки 14, 15, 16, 17, 18 = 0 по графам 3, 4.
	Для НомКол = 3 По 4 Цикл
		Если ПараметрыВыгрузки["ПР000010019" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")]
			= ПараметрыВыгрузки["ПР000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] Тогда
			Если Не (ПараметрыВыгрузки["ПР000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = 0) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: если по гр. " + Формат(НомКол, "ЧГ=")
						+ " значение в стр. 19 равно значению в стр. 01, то значение в стр. 14 должно быть равно нулю!",
					ИмяПоляТаблДокумента, "П0000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
				СтатусПроверки = Ложь;
			ИначеЕсли Не (ПараметрыВыгрузки["ПР000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = 0) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: если по гр. " + Формат(НомКол, "ЧГ=")
						+ " значение в стр. 19 равно значению в стр. 01, то значение в стр. 15 должно быть равно нулю!",
					ИмяПоляТаблДокумента, "П0000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
				СтатусПроверки = Ложь;
			ИначеЕсли Не (ПараметрыВыгрузки["ПР000010016" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = 0) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: если по гр. " + Формат(НомКол, "ЧГ=")
						+ " значение в стр. 19 равно значению в стр. 01, то значение в стр. 16 должно быть равно нулю!",
					ИмяПоляТаблДокумента, "П0000010016" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
				СтатусПроверки = Ложь;
			ИначеЕсли Не (ПараметрыВыгрузки["ПР000010017" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = 0) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: если по гр. " + Формат(НомКол, "ЧГ=")
						+ " значение в стр. 19 равно значению в стр. 01, то значение в стр. 17 должно быть равно нулю!",
					ИмяПоляТаблДокумента, "П0000010017" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
				СтатусПроверки = Ложь;
			ИначеЕсли Не (ПараметрыВыгрузки["ПР000010018" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")] = 0) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: если по гр. " + Формат(НомКол, "ЧГ=")
						+ " значение в стр. 19 равно значению в стр. 01, то значение в стр. 18 должно быть равно нулю!",
					ИмяПоляТаблДокумента, "П0000010018" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	// Строка 01 по графе 4 > строки 20 по графе 3.
	Если ПараметрыВыгрузки["ПР00002002003"] <> 0 Тогда
		Если Не (ПараметрыВыгрузки["ПР00001000104"] > ПараметрыВыгрузки["ПР00002002003"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: значение в стр. 01 по гр. 04 должно быть больше значения в стр. 20 по гр. 03!",
				ИмяПоляТаблДокумента, "П000001000104");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	// Если Строка 01 по графе 3 ≠ 0, то строка 01 по графе 4 > строки 01 по графе 3.
	Если ПараметрыВыгрузки["ПР00001000103"] <> 0 Тогда
		Если Не (ПараметрыВыгрузки["ПР00001000104"] > ПараметрыВыгрузки["ПР00001000103"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в стр. 01 значение по гр. 04 должно быть больше значения по гр. 03!",
				ИмяПоляТаблДокумента, "П000001000104");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	// Строка 20 >= строки 01 по графе 3.
	Если Не (ПараметрыВыгрузки["ПР00002002003"] >= ПараметрыВыгрузки["ПР00001000103"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: по гр. 3 значение в стр. 20 должно быть больше или равно значению в строке 01!",
			ИмяПоляТаблДокумента, "П000002002003");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Дополнительный контроль (из шаблона для выгрузки) по строкам 09 - 19:
	// если гр. 3 > 0, то гр. 4 > 0.
	Для НомСтр = 9 По 19 Цикл
		Если ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"] > 0 Тогда
			Если ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"] = 0 Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: в стр. " + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=")
						+ ", если значение по гр. 3 больше нуля, то значение в гр. 4 должно быть больше нуля!",
...
Функция СформироватьСтруктуруПараметров()
	...
	// Для ПОСЛЕДУЮЩЕГО (ПЕРЕД ВЫГРУЗКОЙ) контроля показателей формы (Указания ФСН по заполнению Формы 1-ИЖС).
	// Тип поселения (тит. лист).
	ТипПоселения = "0";
	Если ТабДокумент["ГородскаяМестность"] = Ложь Тогда
		ТипПоселения = "сельское";
	ИначеЕсли ТабДокумент["СельскаяМестность"] = Ложь Тогда
		ТипПоселения = "городское";
	КонецЕсли;
	ПараметрыВыгрузки.Вставить("ТипПоселения", ТипПоселения);
	// Раздел 1.
	Для НомКол = 3 По 4 Цикл
	...
	КонецЦикла;
	// Раздел 2.
	Для НомСтр = 20 По 21 Цикл
		ПараметрыВыгрузки.Вставить("ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03",
