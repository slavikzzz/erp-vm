...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли СтрНайти(Область.Имя, "П000090190201") = 1 Тогда
		Область.Значение = РезультатНазвание;
		ПолеТабличногоДокументаФормаОтчета.Области["П000090190203_" + Прав(Область.Имя, 1)].Значение = РезультатКод;
	ИначеЕсли СтрНайти(Область.Имя, "П000090190203") = 1 Тогда
		Область.Значение = РезультатКод;
		ПолеТабличногоДокументаФормаОтчета.Области["П000090190201_" + Прав(Область.Имя, 1)].Значение = РезультатНазвание;
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Кол = 3 По 8 Цикл
		СуммаПоКол = 0;
		Для Стр = 320 По 325 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000301" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00003013190" + Кол].Значение = СуммаПоКол;
		
		СуммаПоКол = 0;
		Для Стр = 312 По 318 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000301" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00003013110" + Кол].Значение = СуммаПоКол;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Кол = 3 По 8 Цикл
		...
		мФормаОтчета.Области["П00003013110" + Кол].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00003013110" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П00003013190" + Кол].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00003013190" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
