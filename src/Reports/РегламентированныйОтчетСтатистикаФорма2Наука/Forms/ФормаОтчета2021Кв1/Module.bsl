...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0604011";
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0000901902";
...
Процедура ДоступностьРазделов()
	...
	Если ЧетныйГод(Прав(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета),1)) Тогда
		...
		Для Стр = 1 По ЭтотОбъект.П0000901902.Количество() Цикл
			
			мФормаОтчета.Области["УдалитьСтрокуП000090190201" + "_" + Стр].Гиперссылка = Ложь;
			мФормаОтчета.Области["УдалитьСтрокуП000090190201" + "_" + Стр].Текст = "";
			
			Для Кол = 1 По 4 Цикл
				мФормаОтчета.Области["П0000901902" + Формат(Кол, "ЧЦ=2; ЧВН=") + "_" + Стр].ЦветФона = Новый Цвет();
				мФормаОтчета.Области["П0000901902" + Формат(Кол, "ЧЦ=2; ЧВН=") + "_" + Стр].Защита = Истина;
				мФормаОтчета.Области["П0000901902" + Формат(Кол, "ЧЦ=2; ЧВН=") + "_" + Стр].Значение = "";
			КонецЦикла;
			
		КонецЦикла;
		
		мФормаОтчета.Области["П000090190104_1"].ЦветФона = Новый Цвет();
		мФормаОтчета.Области["П000090190104_1"].Защита = Истина;
		мФормаОтчета.Области["П000090190104_1"].Значение = "";
		
		Если мФормаОтчета.Области.Найти("ДобавлениеСтроки1") <> Неопределено Тогда
			мФормаОтчета.УдалитьОбласть(мФормаОтчета.Области["ДобавлениеСтроки1"], ТипСмещенияТабличногоДокумента.ПоВертикали);
		КонецЕсли;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 Тогда
		УдалитьСтроку("");
	ИначеЕсли Область.Имя = "ТипОрганизации" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "СправочникТипыОрганизаций");
		КолонкаПоиска = "Название";
	ИначеЕсли СтрНайти(Область.Имя, "П000090190201") = 1 Или СтрНайти(Область.Имя, "П000090190203") = 1 Тогда
		Если Область.Защита Тогда
			Возврат;
		КонецЕсли;
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
			ЭтотОбъект, "СправочникОКВЭД2");
		Если СтрНайти(Область.Имя, "П000090190201") = 1 Тогда
			КолонкаПоиска = "Название";
		Иначе
			КолонкаПоиска = "Код";
		КонецЕсли;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Область.Имя = "ТипОрганизации" Тогда
		Область.Значение = РезультатНазвание;
		ПолеТабличногоДокументаФормаОтчета.Области["ПТипОрганизации"].Значение = РезультатКод;
	ИначеЕсли СтрНайти(Область.Имя, "П000090190201") = 1 Тогда
		Область.Значение = РезультатНазвание;
		ПолеТабличногоДокументаФормаОтчета.Области["П000090190203_" + Сред(Область.Имя, 15)].Значение = РезультатКод;
	ИначеЕсли СтрНайти(Область.Имя, "П000090190203") = 1 Тогда
		Область.Значение = РезультатКод;
		ПолеТабличногоДокументаФормаОтчета.Области["П000090190201_" + Сред(Область.Имя, 15)].Значение = РезультатНазвание;
	КонецЕсли;
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект, Область)
	...
	Стр = 1;
	Пока Истина Цикл
		Обл = Области.Найти("П000090190202_" + Стр);
		Если Обл = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Области["П000090190201_" + Стр].Значение)
			Или ЗначениеЗаполнено(Области["П000090190203_" + Стр].Значение)
			Или ЗначениеЗаполнено(Области["П000090190204_" + Стр].Значение) Тогда
			НПП = НПП + 1;
			Области["П000090190202_" + Стр].Значение = НПП;
		Иначе
			Области["П000090190202_" + Стр].Значение = "";
		КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	СтрНач = 102;
	Для Кол = 4 По 8 Цикл
		Если Кол = 7 Или Кол = 8 Тогда
			СтрНач = 103;
		КонецЕсли;
		СуммаПоКол = 0;
		Для Стр = СтрНач По 105 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000101" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00001011010" + Кол].Значение = СуммаПоКол;
	КонецЦикла;
	
	Для Стр = 101 По 105 Цикл
		Если Стр = 102 Тогда
			Области["П0000101" + Стр + "03"].Значение = Области["П0000101" + Стр + "04"].Значение;
			Продолжить;
		КонецЕсли;
		Области["П0000101" + Стр + "03"].Значение =  Области["П0000101" + Стр + "04"].Значение
			+ Области["П0000101" + Стр + "07"].Значение
			+ Области["П0000101" + Стр + "08"].Значение;
	КонецЦикла;
	
	Для Кол = 3 По 4 Цикл
		СуммаПоКол = 0;
		Для Стр = 202 По 205 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000201" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00002012010" + Кол].Значение = СуммаПоКол;
	КонецЦикла;
	
	Для Кол = 3 По 8 Цикл
		СуммаПоКол = 0;
		Для Стр = 321 По 325 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000301" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00003013200" + Кол].Значение = СуммаПоКол;
		
		СуммаПоКол = 0;
		Для Стр = 313 По 319 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000301" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00003013120" + Кол].Значение = СуммаПоКол;
		
		СуммаПоКол = 0;
		Для Стр = 303 По 308 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000301" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00003013020" + Кол].Значение = СуммаПоКол;
		
		Области["П00003013010" + Кол].Значение = Области["П00003013020" + Кол].Значение
			+ Области["П00003013090" + Кол].Значение
			+ Области["П00003013100" + Кол].Значение
			+ Области["П00003013110" + Кол].Значение
			+ Области["П00003013120" + Кол].Значение
			+ Области["П00003013200" + Кол].Значение;
	КонецЦикла;
	
	Области["П000040140203"].Значение = Области["П000040142213"].Значение + Области["П000040142413"].Значение;
	
	Для Кол = 3 По 8 Цикл
		СуммаПоКол = 0;
		Для Стр = 402 По 411 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000401" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00004014010" + Кол].Значение = СуммаПоКол;
	КонецЦикла;
	
	Для Гр = 3 По 13 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		
		Области["П0000401414" + Графа].Значение = Области["П0000401415" + Графа].Значение
			+ Области["П0000401418" + Графа].Значение
			+ Области["П0000401419" + Графа].Значение
			+ Области["П0000401420" + Графа].Значение;
		
	КонецЦикла;
	
	Области["П000040141413"].Значение = 
		(Области["П000040141403"].Значение + Области["П000040141404"].Значение) - Области["П000040141411"].Значение;
		
	Области["П000040142613"].Значение = Области["П000040140303"].Значение + Области["П000040140403"].Значение;
	
	Для Кол = 4 По 9 Цикл
		СуммаПоКол = 0;
		Для Стр = 511 По 517 Цикл
			Если Стр = 514 Или Стр = 516 Тогда
				Продолжить;
			КонецЕсли;
			СуммаПоКол = СуммаПоКол + Области["П0000501" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000501510" + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение = СуммаПоКол;
		
		СуммаПоКол = 0;
		Для Стр = 504 По 509 Цикл
			Если Стр = 505 Тогда
				Продолжить;
			КонецЕсли;
			СуммаПоКол = СуммаПоКол + Области["П0000501" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000501503" + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение = СуммаПоКол;
		
		Стр503 = Области["П00005015030" + Кол].Значение;
		Стр510 = Области["П00005015100" + Кол].Значение;
		Области["П00005015020" + Кол].Значение = Стр503 + Стр510;
		
		Стр502 = Области["П00005015020" + Кол].Значение;
		Стр518 = Области["П00005015180" + Кол].Значение;
		Области["П00005015010" + Кол].Значение = Стр502 + Стр518;
	КонецЦикла;
	...
	Пока Стр >= 501 Цикл
		СуммаПоСтр = 0;
		Для Кол = 4 По 9 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000501" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000501" + Стр + "03"].Значение = СуммаПоСтр;
		Стр = Стр -1;
	КонецЦикла;
	
	Для Кол = 3 По 4 Цикл
		СуммаПоКол = 0;
		Для Стр = 532 По 534 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000501" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00005015310" + Кол].Значение = СуммаПоКол;
		
		Области["П00005015240" + Кол].Значение = Области["П00005015250" + Кол].Значение
			+ Области["П00005015260" + Кол].Значение;
		
		Области["П00005015230" + Кол].Значение = Области["П00005015240" + Кол].Значение
			+ Области["П00005015280" + Кол].Значение
			+ Области["П00005015300" + Кол].Значение
			+ Области["П00005015310" + Кол].Значение
			+ Области["П00005015350" + Кол].Значение;
	КонецЦикла;
	
	Для Кол = 4 По 9 Цикл
		СуммаПоКол = 0;
		Для Стр = 613 По 616 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000601" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00006016120" + Кол].Значение = СуммаПоКол;
		
		Области["П00006016030" + Кол].Значение = Области["П00006016040" + Кол].Значение
			+ Области["П00006016050" + Кол].Значение;
		
		СуммаПоКол = Области["П00006016020" + Кол].Значение + Области["П00006016030" + Кол].Значение;
		Для Стр = 606 По 612 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000601" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00006016010" + Кол].Значение = СуммаПоКол;
	КонецЦикла;
	...
	Пока Стр >= 601 Цикл
		СуммаПоСтр = 0;
		Для Кол = 4 По 9 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000601" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000601" + Стр + "03"].Значение = СуммаПоСтр;
		Стр = Стр -1;
	КонецЦикла;
	
	Для Стр = 617 По 618 Цикл
		Области["П0000601" + Стр + "03"].Значение = Области["П0000601" + Стр + "04"].Значение;
	КонецЦикла;
	
	Для Стр = 619 По 620 Цикл
		СуммаПоСтр = Области["П0000601" + Стр + "04"].Значение + Области["П0000601" + Стр + "06"].Значение;
		Области["П0000601" + Стр + "03"].Значение = СуммаПоСтр;
	КонецЦикла;
	
	Для Стр = 701 По 707 Цикл
		СуммаПоСтр = Области["П0000701" + Стр + "04"].Значение;
		Для Кол = 6 По 9 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000701" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000701" + Стр + "03"].Значение = СуммаПоСтр;
	КонецЦикла;
	
	СуммаПоКол = 0;
	Для Стр = 824 По 826 Цикл
		СуммаПоКол = СуммаПоКол + Области["П0000801" + Стр + "04"].Значение;
	КонецЦикла;
	Области["П000080182304"].Значение = СуммаПоКол;
	
	СуммаПоКол = 0;
	Для Стр = 806 По 817 Цикл
		СуммаПоКол = СуммаПоКол + Области["П0000801" + Стр + "04"].Значение;
	КонецЦикла;
	Области["П000080180504"].Значение = СуммаПоКол;
	
	СуммаПоКол = 0;
	Для Стр = 818 По 822 Цикл
		СуммаПоКол = СуммаПоКол + Области["П0000801" + Стр + "04"].Значение;
	КонецЦикла;
	Области["П000080180204"].Значение = СуммаПоКол
		+ Области["П000080180304"].Значение
		+ Области["П000080180404"].Значение
		+ Области["П000080180504"].Значение;
	
	Области["П000080180104"].Значение = Области["П000080180204"].Значение
		+ Области["П000080182304"].Значение
		+ Области["П000080182904"].Значение
		+ Области["П000080183004"].Значение
		+ Области["П000080183104"].Значение
		+ Области["П000080183204"].Значение;
		
	Стр = 1;
	СуммаПоКол = 0;
	Пока Истина Цикл
		Обл = Области.Найти("П000090190204_" + Стр);
		Если Обл = Неопределено Тогда
			Прервать;
		КонецЕсли;
		...
		Стр = Стр + 1;
	КонецЦикла;
	Области["П000090190104_1"].Значение = СуммаПоКол;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Кол = 3 По 8 Цикл
		Если Кол = 3 Тогда
			Для Стр = 2 По 5 Цикл
				мФормаОтчета.Области["П000010110" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
				мФормаОтчета.Области["П000010110" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
			КонецЦикла;
		КонецЕсли;
		мФормаОтчета.Области["П00001011010" + Кол].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00001011010" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Кол = 3 По 4 Цикл
		мФормаОтчета.Области["П00002012010" + Кол].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00002012010" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Кол = 3 По 8 Цикл
		
		Для Стр = 301 По 302 Цикл
			мФормаОтчета.Области["П0000301" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
			мФормаОтчета.Области["П0000301" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
		КонецЦикла;
		
		мФормаОтчета.Области["П00003013120" + Кол].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00003013120" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П00003013200" + Кол].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00003013200" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Кол = 3 По 8 Цикл
		мФормаОтчета.Области["П00004014010" + Кол].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00004014010" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	мФормаОтчета.Области["П000040140203"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000040140203"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	Для Кол = 3 По 13 Цикл
		мФормаОтчета.Области["П0000401414" + Формат(Кол,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000401414" + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	мФормаОтчета.Области["П000040142613"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000040142613"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	Для Стр = 501 По 521 Цикл
		мФормаОтчета.Области["П0000501" + Стр + "03"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000501" + Стр + "03"].Защита   = Не ОтклАвтоРасчетЗначение;
		Если Стр <= 503 Или Стр = 510 Тогда
			Для Кол = 4 По 9 Цикл
				мФормаОтчета.Области["П0000501" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
				мФормаОтчета.Области["П0000501" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	
	Для Кол = 3 По 4 Цикл
		Для Стр = 523 По 524 Цикл
			мФормаОтчета.Области["П0000501" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
			мФормаОтчета.Области["П0000501" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
		КонецЦикла;
		мФормаОтчета.Области["П00005015310" + Кол].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00005015310" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Стр = 601 По 616 Цикл
		мФормаОтчета.Области["П0000601" + Стр + "03"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000601" + Стр + "03"].Защита   = Не ОтклАвтоРасчетЗначение;
		Если Стр = 601 Или Стр = 603 Или Стр = 612 Тогда
			Для Кол = 4 По 9 Цикл
				мФормаОтчета.Области["П0000601" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
				мФормаОтчета.Области["П0000601" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	
	Для Стр = 617 По 620 Цикл
		мФормаОтчета.Области["П0000601" + Стр + "03"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000601" + Стр + "03"].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Стр = 701 По 707 Цикл
		мФормаОтчета.Области["П0000701" + Стр + "03"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000701" + Стр + "03"].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	мФормаОтчета.Области["П000080180104"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000080180104"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000080180204"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000080180204"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000080180504"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000080180504"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000080182304"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000080182304"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	Если Не ЧетныйГод(Прав(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета),1)) Тогда
		мФормаОтчета.Области["П000090190104_1"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П000090190104_1"].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЕсли;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателей.Добавить(ВыбТабПоле.Области["ПТипОрганизации"].Значение,           "ПТипОрганизации");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 2-Наука");
	...
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть5_5"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 2-Наука");
	...
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть8_1"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть8_2"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть8_3"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 2-Наука");
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ПустаяСтрока"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть8_2"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть8_4"));
	...
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть9_3"));
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть9_3.Низ + 1;
	Если ЧетныйГод(Прав(Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета),1)) Тогда
		НизМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть9_4.Верх - 1;
	Иначе
	...
	Если ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть9_4")) Тогда
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть9_4"));
	Иначе
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 2-Наука");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть9_4"));
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникТипыОрганизаций",
	512, , "Выбор типа организации", КоллекцияСписковВыбора["ТипыОрганизаций"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникОКВЭД2",
	512, , "Выбор кода ОКВЭД2", КоллекцияСписковВыбора["ОКВЭД2"]);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(мДанныеОтчета.ПолеТабличногоДокументаФормаОтчета.ТипОрганизации) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			"Не указан тип организации!", ИмяПоляТаблДокумента, "ТипОрганизации");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("ТипОрганизации", ТабДокумент.ПТипОрганизации);
