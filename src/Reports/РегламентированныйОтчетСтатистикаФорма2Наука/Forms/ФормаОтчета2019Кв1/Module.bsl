...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0000209902";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНайти(Область.Имя, "П000020990201") = 1 Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтаФорма, "СправочникОКВЭД2");
		КолонкаПоиска = "Название";
	ИначеЕсли СтрНайти(Область.Имя, "П000020990203") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтаФорма, "СправочникОКВЭД2");
		КолонкаПоиска = "Код";
	КонецЕсли;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли СтрНайти(Область.Имя, "П000020990201") = 1 Тогда
		Область.Значение = РезультатНазвание;
		ПолеТабличногоДокументаФормаОтчета.Области["П000020990203_" + Прав(Область.Имя, 1)].Значение = РезультатКод;
	ИначеЕсли СтрНайти(Область.Имя, "П000020990203") = 1 Тогда
		Область.Значение = РезультатКод;
		ПолеТабличногоДокументаФормаОтчета.Области["П000020990201_" + Прав(Область.Имя, 1)].Значение = РезультатНазвание;
	КонецЕсли;
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект, Область)
	...
	Стр = 1;
	Пока Истина Цикл
		Обл = Области.Найти("П000020990202_" + Стр);
		Если Обл = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Области["П000020990201_" + Стр].Значение)
			Или ЗначениеЗаполнено(Области["П000020990203_" + Стр].Значение)
			Или ЗначениеЗаполнено(Области["П000020990204_" + Стр].Значение) Тогда
			НПП = НПП + 1;
			Области["П000020990202_" + Стр].Значение = НПП;
		Иначе
			Области["П000020990202_" + Стр].Значение = "";
		КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Кол = 3 По 4 Цикл
		...
		Для Стр = 202 По 205 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000102" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00001022010" + Кол].Значение = СуммаПоКол;
	КонецЦикла;
	
	Для Кол = 3 По 8 Цикл
		СуммаПоКол = 0;
		Для Стр = 320 По 325 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000103" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00001033190" + Кол].Значение = СуммаПоКол;
		
		СуммаПоКол = 0;
		Для Стр = 312 По 318 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000103" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00001033110" + Кол].Значение = СуммаПоКол;
		
		СуммаПоКол = 0;
		Для Стр = 303 По 307 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000103" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00001033020" + Кол].Значение = СуммаПоКол;
		
		Области["П00001033010" + Кол].Значение = Области["П00001033020" + Кол].Значение
			+ Области["П00001033080" + Кол].Значение
			+ Области["П00001033090" + Кол].Значение
			+ Области["П00001033100" + Кол].Значение
			+ Области["П00001033110" + Кол].Значение
			+ Области["П00001033190" + Кол].Значение;
	КонецЦикла;
		
	Для Кол = 3 По 8 Цикл
		СуммаПоКол = 0;
		Для Стр = 402 По 411 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000104" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00001044010" + Кол].Значение = СуммаПоКол;
	КонецЦикла;
	
	Для Стр = 415 По 421 Цикл
		Если Стр = 416 Или Стр = 417 Тогда
			КолНач = 6;
		Иначе
			КолНач = 5;
		КонецЕсли;
		
		СуммаПоСтрокеДляКол4 = 0;
		Для Кол = КолНач По 7 Цикл
			СуммаПоСтрокеДляКол4 = СуммаПоСтрокеДляКол4 + Области["П0000104" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000104" + Стр + "04"].Значение = СуммаПоСтрокеДляКол4;
		
		Области["П0000104" + Стр + "08"].Значение = Области["П0000104" + Стр + "09"].Значение
			+ Области["П0000104" + Стр + "10"].Значение
			+ Области["П0000104" + Стр + "11"].Значение;
	КонецЦикла;
	
	Для Кол = 3 По 12 Цикл
		СуммаПоКол = Области["П0000104415" + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение
			+ Области["П0000104418" + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение
			+ Области["П0000104419" + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение
			+ Области["П0000104420" + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		Области["П0000104414" + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение = СуммаПоКол;
	КонецЦикла;
	...
	Для Кол = 4 По 9 Цикл
		...
		Для Стр = 511 По 517 Цикл
			Если Стр = 514 Или Стр = 516 Тогда
				Продолжить;
			КонецЕсли;
			СуммаПоКол = СуммаПоКол + Области["П0000205" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000205510" + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение = СуммаПоКол;
		...
		Для Стр = 504 По 509 Цикл
			...
			СуммаПоКол = СуммаПоКол + Области["П0000205" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000205503" + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение = СуммаПоКол;
		
		Стр503 = Области["П00002055030" + Кол].Значение;
		Стр510 = Области["П00002055100" + Кол].Значение;
		Области["П00002055020" + Кол].Значение = Стр503 + Стр510;
		
		Стр502 = Области["П00002055020" + Кол].Значение;
		Стр518 = Области["П00002055180" + Кол].Значение;
		Области["П00002055010" + Кол].Значение = Стр502 + Стр518;
	КонецЦикла;
	...
	Пока Стр >= 501 Цикл
		СуммаПоСтр = 0;
		Для Кол = 4 По 9 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000205" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000205" + Стр + "03"].Значение = СуммаПоСтр;
		Стр = Стр -1;
	КонецЦикла;
	...
	Для Кол = 3 По 4 Цикл
		...
		Для Стр = 532 По 534 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000205" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00002055310" + Кол].Значение = СуммаПоКол;
		
		Области["П00002055240" + Кол].Значение = Области["П00002055250" + Кол].Значение
			+ Области["П00002055260" + Кол].Значение;
		
		Области["П00002055230" + Кол].Значение = Области["П00002055240" + Кол].Значение
			+ Области["П00002055280" + Кол].Значение
			+ Области["П00002055300" + Кол].Значение
			+ Области["П00002055310" + Кол].Значение
			+ Области["П00002055350" + Кол].Значение;
	КонецЦикла;
	
	Для Кол = 4 По 9 Цикл
		СуммаПоКол = 0;
		Для Стр = 613 По 616 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000206" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00002066120" + Кол].Значение = СуммаПоКол;
		
		Области["П00002066030" + Кол].Значение = Области["П00002066040" + Кол].Значение
			+ Области["П00002066050" + Кол].Значение;
		
		СуммаПоКол = Области["П00002066020" + Кол].Значение + Области["П00002066030" + Кол].Значение;
		Для Стр = 606 По 612 Цикл
			СуммаПоКол = СуммаПоКол + Области["П0000206" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П00002066010" + Кол].Значение = СуммаПоКол;
	КонецЦикла;
	...
	Пока Стр >= 601 Цикл
		СуммаПоСтр = 0;
		Для Кол = 4 По 9 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000206" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000206" + Стр + "03"].Значение = СуммаПоСтр;
		Стр = Стр -1;
	КонецЦикла;
	
	Для Стр = 617 По 618 Цикл
		Области["П0000206" + Стр + "03"].Значение = Области["П0000206" + Стр + "04"].Значение;
	КонецЦикла;
	
	Для Стр = 619 По 620 Цикл
		СуммаПоСтр = Области["П0000206" + Стр + "04"].Значение + Области["П0000206" + Стр + "06"].Значение;
		Области["П0000206" + Стр + "03"].Значение = СуммаПоСтр;
	КонецЦикла;
	
	Для Стр = 701 По 707 Цикл
		СуммаПоСтр = Области["П0000207" + Стр + "04"].Значение;
		Для Кол = 6 По 9 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000207" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000207" + Стр + "03"].Значение = СуммаПоСтр;
	КонецЦикла;
	
	СуммаПоКол = 0;
	Для Стр = 824 По 826 Цикл
		СуммаПоКол = СуммаПоКол + Области["П0000208" + Стр + "04"].Значение;
	КонецЦикла;
	Области["П000020882304"].Значение = СуммаПоКол;
	
	СуммаПоКол = 0;
	Для Стр = 806 По 817 Цикл
		СуммаПоКол = СуммаПоКол + Области["П0000208" + Стр + "04"].Значение;
	КонецЦикла;
	Области["П000020880504"].Значение = СуммаПоКол;
	...
	Для Стр = 818 По 822 Цикл
		СуммаПоКол = СуммаПоКол + Области["П0000208" + Стр + "04"].Значение;
	КонецЦикла;
	Области["П000020880204"].Значение = СуммаПоКол
		+ Области["П000020880304"].Значение
		+ Области["П000020880404"].Значение
		+ Области["П000020880504"].Значение;
	
	Области["П000020880104"].Значение = Области["П000020880204"].Значение
		+ Области["П000020882304"].Значение
		+ Области["П000020882904"].Значение
		+ Области["П000020883004"].Значение
		+ Области["П000020883104"].Значение
		+ Области["П000020883204"].Значение;
	
	Стр = 1;
	СуммаПоКол = 0;
	Пока Истина Цикл
		Обл = Области.Найти("П000020990204_" + Стр);
		Если Обл = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		...
		Стр = Стр + 1;
	КонецЦикла;
	Области["П000020990104_1"].Значение = СуммаПоКол;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Кол = 3 По 4 Цикл
		мФормаОтчета.Области["П00001022010" + Кол].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00001022010" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Кол = 3 По 8 Цикл
		
		Для Стр = 301 По 302 Цикл
			мФормаОтчета.Области["П0000103" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
			мФормаОтчета.Области["П0000103" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
		КонецЦикла;
		
		мФормаОтчета.Области["П00001033110" + Кол].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00001033110" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П00001033190" + Кол].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00001033190" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	
	Для Кол = 3 По 8 Цикл
		мФормаОтчета.Области["П00001044010" + Кол].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00001044010" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	
	Для Кол = 3 По 12 Цикл
		мФормаОтчета.Области["П0000104414" + Формат(Кол,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000104414" + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
		Если Кол = 4 Или Кол = 8 Тогда
			Для Стр = 415 По 421 Цикл
				мФормаОтчета.Области["П0000104" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
				мФормаОтчета.Области["П0000104" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	
	Для Стр = 501 По 521 Цикл
		мФормаОтчета.Области["П0000205" + Стр + "03"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000205" + Стр + "03"].Защита   = Не ОтклАвтоРасчетЗначение;
		Если Стр <= 503 Или Стр = 510 Тогда
			Для Кол = 4 По 9 Цикл
				мФормаОтчета.Области["П0000205" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
				мФормаОтчета.Области["П0000205" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	...
	Для Кол = 3 По 4 Цикл
		Для Стр = 523 По 524 Цикл
			...
			мФормаОтчета.Области["П0000205" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
		КонецЦикла;
		мФормаОтчета.Области["П00002055310" + Кол].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00002055310" + Кол].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Стр = 601 По 616 Цикл
		мФормаОтчета.Области["П0000206" + Стр + "03"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000206" + Стр + "03"].Защита   = Не ОтклАвтоРасчетЗначение;
		Если Стр = 601 Или Стр = 603 Или Стр = 612 Тогда
			Для Кол = 4 По 9 Цикл
				мФормаОтчета.Области["П0000206" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
				мФормаОтчета.Области["П0000206" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	...
	Для Стр = 701 По 707 Цикл
		мФормаОтчета.Области["П0000207" + Стр + "03"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000207" + Стр + "03"].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	
	мФормаОтчета.Области["П000020880104"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000020880104"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000020880204"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000020880204"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000020880504"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000020880504"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000020882304"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000020882304"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П000020990104_1"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000020990104_1"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть8"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 2-Наука");
	
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть9"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 2-Наука");
	...
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть10_2"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть10_4"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть10_5"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть10_6"));
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть10_6.Низ + 1;
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверки.Добавить(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть10_7"));
	...
	КонецЦикла;
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть10_7"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 2-Наука");
