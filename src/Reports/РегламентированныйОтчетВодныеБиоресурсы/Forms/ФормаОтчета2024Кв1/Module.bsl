...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если СтрНачинаетсяС(ИмяПоказателя, "П00002М107001") Тогда
		...
		Иначе
			...
			|en = 'Код объекта водных биологических ресурсов должен состоять из 4 цифр. Значение очищено.'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
Функция КоэффициентДефлятор(Форма, КодОВБР)
	...
	Если ЗначениеЗаполнено(КодОВБР) И Год(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2025 И СтрРазделить(
