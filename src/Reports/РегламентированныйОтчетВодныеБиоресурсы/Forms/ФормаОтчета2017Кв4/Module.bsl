...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.водн.биорес.5.03") Тогда
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ИмяПоказателя = "П000020007003" Тогда
		...
		Иначе
			...
			|en = 'Код ставки сбора должен состоять из 2 цифр. Значение очищено.'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
	...
	Если СтрНачинаетсяС(ИмяПоказателя, "П00002М108001") Тогда
		...
		Иначе
			...
			|en = 'Код наименования объекта водных биологических ресурсов должен состоять из 4 цифр. Значение очищено.'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПараметрыВыгрузки.Свойство("НомКорр") И ПараметрыВыгрузки["НомКорр"] = "0" Тогда
		СообщитьВТС("По форме, утвержденной приказом ФНС России от 14.06.2017 № ММВ-7-3/505@, с 26.02.2024 по 31.12.2024"
		+ " возможно представление только корректирующих Сведений, в случае, если первичные Сведения были представлены"
		+ " по этой форме (на основании письма ФНС России от 15.02.2024 № СД-4-3/1665@). При необходимости представления"
		+ " первичных Сведений, создайте отчет по форме, утвержденной приказом ФНС России от 17.11.2023 № ЕД-7-3/853@",
	...
	Если ЗаполненРаздел(ДеревоРаздела, "Раздел1") Тогда
		...
		Для каждого СтраницаРаздела Из ДеревоРаздела.Строки Цикл
			...
			Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
				...
				Если ЗначениеЗаполнено(СтрокаМнЧ.Данные["П00001М105001"])
					ИЛИ ЗначениеЗаполнено(СтрокаМнЧ.Данные["П00001М106002"]) Тогда
