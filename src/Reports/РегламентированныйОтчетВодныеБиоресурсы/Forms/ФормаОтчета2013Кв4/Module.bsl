...
Процедура ДобавитьСтраницу(Команда)
	...
	Обл.Гиперссылка = Истина;
	Обл.Текст = "Удалить страницу";
	ИмяОбласти = ?(Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1", СтруктураРеквизитовФормы.мИдГруппы1, СтруктураРеквизитовФормы.мИдГруппы2);
	Обл = ТабличныйДокумент.Области["УдалитьСтроку" + ИмяОбласти + "01_1"];
	Обл.Гиперссылка = Ложь;
	Обл.Текст = "";
КонецПроцедуры
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Для Каждого ЛистРаздела1 Из Раздел1 Цикл
		...
		Если ДлинаОКТМОРаздел1 <> 11 И ДлинаОКТМОРаздел1 <> 8 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 1 на странице " + НомерЛиста + " не " + ?(ДлинаОКТМОРаздел1 = 0, "", "верно") + " задан код по ОКТМО!", "Раздел1", "П000010002003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
