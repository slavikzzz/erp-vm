...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "14/02/2014";
	// Код отчета, равен коду по ОКУД.
	СтруктураРеквизитовФормы.мКодОтчета   = "1606305";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Области.П0103.Значение = Области.П0104.Значение + Области.П0105.Значение + Области.П0106.Значение;
	Области.П0107.Значение = Области.П0102.Значение + Области.П0103.Значение;
	
	Области.П0208.Значение = Области.П0209.Значение + Области.П0210.Значение + Области.П0211.Значение
							+ Области.П0212.Значение + Области.П0213.Значение;
	Области.П0222.Значение = Области.П0223.Значение + Области.П0224.Значение + Области.П0225.Значение;
	
	Сумм = 0;
	Для Стр = 27 По 34 Цикл
		Сумм = Сумм + Области["П02"+Стр].Значение;
	КонецЦикла;
	Области.П0226.Значение = Сумм;
	Области.П0238.Значение = Области.П0239.Значение + Области.П0241.Значение + Области.П0242.Значение;
	Области.П0244.Значение = Области.П0208.Значение + Области.П0215.Значение + Области.П0218.Значение
							+ Области.П0220.Значение + Области.П0222.Значение + Области.П0226.Значение
							 + Области.П0235.Значение + Области.П0237.Значение + Области.П0238.Значение + Области.П0243.Значение;
КонецПроцедуры
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Ном = 1 По 4 Цикл
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма №1 (рабочая сила)");
	КонецЦикла;
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 29.01.2014 № 55";
	
	СоответствиеБР = Новый Соответствие;
	СоответствиеБР[8] = 14;
	СоответствиеБР[18] = 19;
	СоответствиеБР[20] = 21;
	СоответствиеБР[35] = 36;
	СоответствиеБР[39] = 40;
	
	Для Каждого КЗ Из СоответствиеБР Цикл 
		СтрБР = Формат(КЗ.Ключ, "ЧЦ=2; ЧВН=; ЧГ=;");
		СтрМР = Формат(КЗ.Значение, "ЧЦ=2; ЧВН=; ЧГ=;");
		ЗнСтрБР = Показатели["П02" + СтрБР];
		ЗнСтрМР = Показатели["П02" + СтрМР];
		
		ТекстФормулы = "стр." + СтрБР+" >= стр." + СтрМР;
		Описание     = "стр." + СтрБР+" < стр." + СтрМР;
		Расшифровка = Строка(ЗнСтрБР) + " >= " + Строка(ЗнСтрМР);
		Если ЗнСтрБР >= ЗнСтрМР Тогда
			ЭтоОшибка = Ложь;
	...
	ЗнСтрБР = Показатели["П0215"];
	ЗнСтрМР1 = Показатели["П0216"];
	ЗнСтрМР2 = Показатели["П0217"];
	ТекстФормулы = "стр.15 >= стр.16 + стр.17";
	Описание     = "стр.15 < стр.16 + стр.17";
	Расшифровка = Строка(ЗнСтрБР) + " >= " + Строка(ЗнСтрМР1) + " + " + Строка(ЗнСтрМР2);
	Если ЗнСтрБР >= ЗнСтрМР1 + ЗнСтрМР2 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
