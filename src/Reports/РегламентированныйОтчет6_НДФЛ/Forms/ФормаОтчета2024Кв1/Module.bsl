...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр 6-НДФЛ 5.04") Тогда
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ = Новый Структура;
	Если РасчетЗаПервыйКвартал Тогда
		Структ.Вставить("П000020016003", "П000020016103;П000020016203;П000020016303;П000020016403");
		Структ.Вставить("П000020019003", "П000020019103;П000020019203;П000020019303;П000020019403");
	КонецЕсли;
	...
	Если РасчетЗаПервыйКвартал Тогда
		...
		Структ.Вставить("П000010003001", "П000010003101;П000010003201;П000010003301;П000010003401");
	КонецЕсли;
...
Функция ПечатьРеестраНаСервере(ВидПечати, ЗаголовокПечатнойФормы)
	...
	Макет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатныйБланк2024Кв1_Реестр");
	...
	СтрКолонкиСуммирования = "ДохОбщ,Выч,НалИсч,ФиксАв,НалПрЗач,НалУд,НалНеУд,НалИзлУд,НалВозвр";
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Для ИндСтрТЗ = 1 По ТЗ.Количество() - 1 Цикл
				...
				Если ИндСтрТЗ = ТЗ.Количество() - 1 Тогда
					...
					Обл = ПФ.Область(ОблВерх, 5, ОблВерх, 15);
...
Процедура Проверить(Команда)
	...
	|от 19.09.2023 № ЕД-7-11/649@, будет
	...
	Если НЕ РасчетСоставленЗаГод Тогда
		...
		Если Область.Имя = "КодФормыРеорганизации" Тогда
			...
			Если ЗначениеЗаполнено(ПрежнийКодФормыРеорганизации) И НЕ ЗначениеЗаполнено(НовыйКодФормыРеорганизации) Тогда
				...
				|en = 'При изменении кода формы реорганизации с ""%1"" на ""%2""
				|будет установлен код отчетного периода ""%3"". В этом случае
				|Приложение 1 не представляется, оно будет очищено и исключено
