...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Иначе
		ВосстановитьСохраненныеДанные();
		Если СтруктураРеквизитовФормы.мСкопированаФорма <> Неопределено Тогда
			ИсходныйОтчетЗаПервыйКвартал = Месяц(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) = 3;
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСкопированаФорма) Тогда
			НовыйОтчетЗаПервыйКвартал = Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 3;
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.6-НДФЛ 24к2",
		'20240101') Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	Если СтруктураРеквизитовФормы.ИП Тогда
		ИмяСекции_КодыПериода = "КодыПериода_ПоМесту120_124_320_"
			+ Формат(Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) / 3, "ЧГ=");
	Иначе
		ИмяСекции_КодыПериода = "КодыПериода_ПоМесту220_"
			+ Формат(Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) / 3, "ЧГ=");
	КонецЕсли;
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "Период", 2, ,
	"Выбор отчетного периода", КоллекцияСписковВыбора[ИмяСекции_КодыПериода]);
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2025 Тогда
		ИмяСекции_КБК = "КБК_20250101";
		ИмяСекции_КодыВычета_Приложение1_Раздел3 = "КодыВычета_Приложение1_Раздел3_20250101";
		ИмяСекции_КодыДохода = "КодыДохода_20250101";
		ИмяСекции_КодыВычета_Приложение1_Приложение = "КодыВычета_Приложение1_Приложение_20250101";
	Иначе
		ИмяСекции_КБК = "КБК";
		ИмяСекции_КодыВычета_Приложение1_Раздел3 = "КодыВычета_Приложение1_Раздел3";
		ИмяСекции_КодыДохода = "КодыДохода";
		ИмяСекции_КодыВычета_Приложение1_Приложение = "КодыВычета_Приложение1_Приложение";
	КонецЕсли;
	...
	"Выбор кода вычета", КоллекцияСписковВыбора[ИмяСекции_КодыВычета_Приложение1_Раздел3]);
	...
	"Выбор кода дохода", КоллекцияСписковВыбора[ИмяСекции_КодыДохода]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000П1М600004", 3, ,
	"Выбор кода вычета", КоллекцияСписковВыбора[ИмяСекции_КодыВычета_Приложение1_Приложение]);
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ТипЗнч(ЗначПоказателя) = Тип("Число") И ЗначПоказателя < 0 Тогда
		...
		|en = 'Не допускается заполнение расчета отрицательными числовыми значениями. Значение очищено'");
		Сообщение.Сообщить();
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Титульный" Тогда
		...
		Если (Форма.СтруктураРеквизитовФормы.ИП И (ЗначПоМесту = "120" ИЛИ ЗначПоМесту = "124" ИЛИ ЗначПоМесту = "320"))
			ИЛИ (НЕ Форма.СтруктураРеквизитовФормы.ИП И ЗначПоМесту = "220" И НЕ ЗначениеЗаполнено(ЗначКодФормыРеорг)) Тогда
	...
	ИначеЕсли ИмяРаздела = "Раздел1" ИЛИ ИмяРаздела = "Раздел2" Тогда
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Приложение1" Тогда
		...
		Если СтрНачинаетсяС(ИмяОбласти, СтруктураРеквизитовФормы.мИдГруппыП1М4) Тогда
			ТекстПредупреждения = НСтр("ru = 'Не предусмотрено добавление строк в разделе 4 приложения 1 к Расчету';
										|en = 'Не предусмотрено добавление строк в разделе 4 приложения 1 к Расчету'");
		КонецЕсли;
...
Процедура ВставитьСтроку(Команда)
	...
	Если Тип(Элементы.ТабличныйДокумент.ТекущаяОбласть) = Тип("ОбластьЯчеекТабличногоДокумента") Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Не предусмотрена вставка для текущей строки';
									|en = 'Не предусмотрена вставка для текущей строки'");
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Если РасчетЗаПервыйКвартал Тогда
					...
					Если СтрЧислоВхождений(СтруктЗависимыхЯчеек["П000010002001"], ОбластьИмя) > 0 Тогда
						...
						+ ТДО["П000010002501"].Значение + ТДО["П000010002601"].Значение;
					КонецЕсли;
					
					Если СтрЧислоВхождений(СтруктЗависимыхЯчеек["П000010003001"], ОбластьИмя) > 0 Тогда
						ТДО["П000010003001"].Значение = ТДО["П000010003101"].Значение + ТДО["П000010003201"].Значение
						+ ТДО["П000010003301"].Значение + ТДО["П000010003401"].Значение
						+ ТДО["П000010003501"].Значение + ТДО["П000010003601"].Значение;
					КонецЕсли;
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Если РасчетЗаПервыйКвартал Тогда
					...
					Если СтрЧислоВхождений(СтруктЗависимыхЯчеек["П000020016003"], ОбластьИмя) > 0 Тогда
						ТДО["П000020016003"].Значение = ТДО["П000020016103"].Значение + ТДО["П000020016203"].Значение
						+ ТДО["П000020016303"].Значение + ТДО["П000020016403"].Значение
						+ ТДО["П000020016503"].Значение + ТДО["П000020016603"].Значение;
					КонецЕсли;
					
					Если СтрЧислоВхождений(СтруктЗависимыхЯчеек["П000020019003"], ОбластьИмя) > 0 Тогда
						ТДО["П000020019003"].Значение = ТДО["П000020019103"].Значение + ТДО["П000020019203"].Значение
						+ ТДО["П000020019303"].Значение + ТДО["П000020019403"].Значение
						+ ТДО["П000020019503"].Значение + ТДО["П000020019603"].Значение;
					КонецЕсли;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ = Новый Структура;
	Если РасчетЗаПервыйКвартал Тогда
		Структ.Вставить("П000020016003", "П000020016103;П000020016203;П000020016303;П000020016403;П000020016503;П000020016603");
		Структ.Вставить("П000020019003", "П000020019103;П000020019203;П000020019303;П000020019403;П000020019503;П000020019603");
	КонецЕсли;
	...
	Структ = Новый Структура;
	Если РасчетЗаПервыйКвартал Тогда
		Структ.Вставить("П000010002001", "П000010002101;П000010002201;П000010002301;П000010002401;П000010002501;П000010002601");
		Структ.Вставить("П000010003001", "П000010003101;П000010003201;П000010003301;П000010003401;П000010003501;П000010003601");
	КонецЕсли;
...
Функция ПечатьРеестраНаСервере(ВидПечати, ЗаголовокПечатнойФормы)
	...
	Макет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатныйБланк2024Кв2_Реестр");
	...
	ТЗ.Колонки.Добавить("НалУпВИн", ОписаниеТиповЧ_15_0);
	...
	СтрКолонкиСуммирования = "ДохОбщ,Выч,НалИсч,ФиксАв,НалПрЗач,НалУпВИн,НалУд,НалНеУд,НалИзлУд,НалВозвр";
	...
	Если ОбъектОтчета.ЗаполненРаздел(СтруктураРеквизитовФормы.мВыбраннаяФорма, ДеревоДанных,
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
				...
				НовСтрТЗ.НалУпВИн = ДанныеСтрокиМнЧ["П000П1М100011"];
	...
	Если ОбъектОтчета.ЗаполненРаздел(СтруктураРеквизитовФормы.мВыбраннаяФорма, ДеревоДанных,
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			НовСтрТЗ.ДохОбщ   = Ветка.Данные["П000020012003"];
			...
			НовСтрТЗ.НалУпВИн = Ветка.Данные["П000020015603"];
...
Процедура Расшифровать(Команда)
	...
	// При расшифровке строк разделов 1 и 2 дополнительно передаются:
...
Процедура Проверить(Команда)
	...
	ПараметрыПоискаКопий.Вставить("КПП"					, КодыСТитульногоЛиста().КПП);
	ПараметрыПоискаКопий.Вставить("ОКТМО"				, КодыСТитульногоЛиста().ОКТМО);
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	ПараметрыВыполнения.Вставить("мСохраненныйДок"						, СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка);
	...
	ПараметрыЗапуска.НаименованиеФоновогоЗадания = 
		НСтр("ru = 'Проверка контрольных соотношений 6-НДФЛ';
			|en = 'Проверка контрольных соотношений 6-НДФЛ'");
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	Возврат ОбъектОтчета.ВыполнитьДокументированиеОбъект_2024Кв2();
...
Функция ВыполнитьПоискКопийРеглОтчетовНаСервере(ДополнительныеПараметры)
	...
	|	И Документ.КПП = &КПП
	|	И Документ.ИсточникОтчета = ""РегламентированныйОтчет6_НДФЛ""
	|	И (Документ.ВыбраннаяФорма = ""ФормаОтчета2024Кв1""
	|			ИЛИ Документ.ВыбраннаяФорма = ""ФормаОтчета2024Кв2"")
	|	И ГОД(Документ.ДатаОкончания) = ГОД(&ДатаОтчета)
	|	И ВЫБОР
	|			КОГДА МЕСЯЦ(&ДатаОтчета) = 12
	|				ТОГДА МЕСЯЦ(Документ.ДатаОкончания) <= МЕСЯЦ(&ДатаОтчета)
	|			ИНАЧЕ МЕСЯЦ(Документ.ДатаОкончания) < МЕСЯЦ(&ДатаОтчета)
	|		КОНЕЦ
	|	И ВЫБОР
	...
	Для ОбратныйИндекс = 1 По КолвоСтрок Цикл
		...
		Если НЕ ОКТМО = ДополнительныеПараметры.ОКТМО Тогда
			ТаблицаДанных.Удалить(СтрокаТаблицы);
			Продолжить;
		КонецЕсли;
		
		СтрокаТаблицы.ОКТМО = ДополнительныеПараметры.ОКТМО;
		СтрокаТаблицы.Дополнительно = "КПП " + ДополнительныеПараметры.КПП + ", ОКТМО " + ДополнительныеПараметры.ОКТМО;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РасчетСоставленЗаГод Тогда
		...
		Если Область.Имя = "ПоМесту" Тогда
			...
			ТекКодФормыРеорганизации = СокрЛП(ТабличныйДокумент.Области.Найти("КодФормыРеорганизации").Значение);
			ТекПериод = СокрЛП(ТабличныйДокумент.Области.Найти("Период").Значение);
			
			ДополнительныеПараметры = Новый Структура("Область, НовыйКодПоМесту", Область, НовыйКодПоМесту);
			
			Если (ПрежнийКодПоМесту = "220" И НовыйКодПоМесту <> "220" И НЕ ЗначениеЗаполнено(ТекКодФормыРеорганизации)
				И СтрРазделить("51,52,53", ",", Ложь).Найти(ТекПериод) <> Неопределено)
				ИЛИ ((ПрежнийКодПоМесту = "120" ИЛИ ПрежнийКодПоМесту = "124" ИЛИ ПрежнийКодПоМесту = "320")
				И НЕ (НовыйКодПоМесту = "120" ИЛИ НовыйКодПоМесту = "124" ИЛИ НовыйКодПоМесту = "320")
				И СтрРазделить("83,84,85", ",", Ложь).Найти(ТекПериод) <> Неопределено) Тогда
				...
				НСтр("ru = 'При изменении кода места представления с ""%1"" на ""%2""
				|будет установлен код отчетного периода ""%3"". В этом
				|случае Приложение 1 не представляется, оно будет очищено
				|и исключено из состава дерева разделов. Продолжить?';
				|en = 'При изменении кода места представления с ""%1"" на ""%2""
				|будет установлен код отчетного периода ""%3"". В этом
				|случае Приложение 1 не представляется, оно будет очищено
				|и исключено из состава дерева разделов. Продолжить?'"), ПрежнийКодПоМесту,
				?(НовыйКодПоМесту = "", "Пустое значение", НовыйКодПоМесту), НовыйКодПериода);
				ПоказатьВопрос(Оп, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 60, КодВозвратаДиалога.Нет);
				Возврат;
			ИначеЕсли НЕ ЗначениеЗаполнено(ПрежнийКодПоМесту) И ЗначениеЗаполнено(НовыйКодПоМесту) Тогда
		...
		ИначеЕсли Область.Имя = "КодФормыРеорганизации" Тогда
			...
			ТекКодПоМесту = СокрЛП(ТабличныйДокумент.Области.Найти("ПоМесту").Значение);
			
			ДополнительныеПараметры
				= Новый Структура("Область, НовыйКодФормыРеорганизации", Область, НовыйКодФормыРеорганизации);
			
			Если ЗначениеЗаполнено(ПрежнийКодФормыРеорганизации) И НЕ ЗначениеЗаполнено(НовыйКодФормыРеорганизации)
				И ТекКодПоМесту <> "220" Тогда
				НовыйКодПериода = СтруктураРеквизитовФормы.КодОтчетногоПериода[
				"НеЗаполненКодФормыРеорганизации_" + Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)];
				Оп = Новый ОписаниеОповещения("ВопросВвестиКодФормыРеорганизацииЗавершение", ЭтотОбъект, ДополнительныеПараметры);
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При изменении кода формы реорганизации с ""%1""
				|на ""%2"" будет установлен код отчетного
				|периода ""%3"". В этом случае Приложение 1 не
				|представляется, оно будет очищено и исключено
				|из состава дерева разделов. Продолжить?';
				|en = 'При изменении кода формы реорганизации с ""%1""
				|на ""%2"" будет установлен код отчетного
				|периода ""%3"". В этом случае Приложение 1 не
				|представляется, оно будет очищено и исключено
		...
		ИначеЕсли Область.Имя = "Период" Тогда
			...
			ДополнительныеПараметры = Новый Структура("Область, НовыйПериод", Область, НовыйПериод);
			...
			Если ПрежнийПериод_МожноВключатьПриложение1 И НЕ НовыйПериод_МожноВключатьПриложение1 Тогда
				...
				|из состава дерева разделов. Продолжить?';
				|en = 'При коде отчетного периода ""%1"" Приложение 1
				|не представляется, оно будет очищено и исключено
