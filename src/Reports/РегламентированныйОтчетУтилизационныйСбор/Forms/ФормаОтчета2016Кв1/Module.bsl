...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.ВариантыФорматовВыгрузки.Добавить(
		СтруктураРеквизитовФормы.мСоставПоказателей, "Формат 5.03", Истина);
...
Процедура ПроверитьЗаполнение_Раздел1(Параметры, СтатусПроверки)
	...
	Для Каждого Лист Из ДанныеРаздела Цикл
		...
		ИначеЕсли НЕ ЗначениеСодержитсяВСписке(ПризнакТС, ДопустимыеЗначения) Тогда
			...
			+ " раздела 1 признак транспортного средства должен принимать одно из значений: 10, 11, 12, 20, 25.";
		...
		ИначеЕсли НЕ ЗначениеСодержитсяВСписке(КатегорияТС, ДопустимыеЗначения) Тогда
			ТекстОписанияОшибки = "На странице " + НомерЛиста
			+ " раздела 1 категория (вид) транспортного средства должна принимать одно из значений: M1, M2, M3, M1G, M2G, M3G, N1, N2, N3, N1G, N2G, N3G, O1, O2, O3, O4.";
		...
		Иначе
			...
			Если РазрешенныеТипыДвигателя.Найти(ТипДвигателя) = Неопределено Тогда
				СообщитьВТС("На странице " + НомерЛиста + " раздела 1 тип двигателя должен принимать одно из значений: 0, 3.",
		...
		СерияНомерПаспорта = Серия1 + Серия2 + Номер;
		Если НЕ ЗначениеЗаполнено(СерияНомерПаспорта) Тогда
			СообщитьВТС("На странице " + НомерЛиста + " раздела 1 не указаны серия и номер паспорта.", "Раздел1", "П000010018001", НомерЛиста);
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(СерияНомерПаспорта) < 10 Тогда
			
			ДлинаСерия1 = СтрДлина(Серия1);
			ДлинаСерия2 = СтрДлина(Серия2);
			ДлинаНомер  = СтрДлина(Номер);
			
			Если ДлинаСерия1 < 2 Тогда
				СообщениеОбОшибке = "На странице " + НомерЛиста + " раздела 1 не" + ?(ДлинаСерия1 = 0,
				"", "верно") + " указаны 1-й и 2-й символы в серии паспорта.";
				СообщитьВТС(СообщениеОбОшибке, "Раздел1", "П000010018001", НомерЛиста);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если ДлинаСерия2 < 2 Тогда
				СообщениеОбОшибке = "На странице " + НомерЛиста + " раздела 1 не" + ?(ДлинаСерия2 = 0,
				"", "верно") + " указаны 3-й и 4-й символы в серии паспорта.";
				СообщитьВТС(СообщениеОбОшибке, "Раздел1", "П000010018002", НомерЛиста);
				СтатусПроверки = Ложь;
			КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ДатаВыдачиПаспорта) Тогда
			СообщитьВТС("На странице " + НомерЛиста + " раздела 1 не указана дата выдачи паспорта.",
