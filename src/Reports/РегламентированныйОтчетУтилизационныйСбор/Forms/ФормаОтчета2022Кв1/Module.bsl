...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если ЗаполнитьЗначенияПоУмолчанию ИЛИ ОтчетСкопированОрганизацияИзменилась Тогда
		...
		ДанныеР1.Вставить("П000010001003", "18211208000012000120");
...
Процедура СформироватьСпискиВыбора()
	...
	//Раздел 3.
	//
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000030004003", 2, ,
	"Выбор признака транспортного средства", КоллекцияСписковВыбора["ПризнакиТС"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000030009003", 0, ,
	"Выбор кода категории транспортного средства", КоллекцияСписковВыбора["КодыКатегорийТС"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000030011003", 1, ,
	"Выбор типа двигателя", КоллекцияСписковВыбора["ТипыДвигателя"]);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "П000030014003" И НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		
		СтандартнаяОбработка = Ложь;
		
		Ставки = Новый СписокЗначений;
		Ставки.Добавить(0);
		КатегорияТС = СокрЛП(ТДО["П000030009003"].Значение);
		Если СтрРазделить("M1,M1G", ",", Ложь).Найти(КатегорияТС) <> Неопределено Тогда
			Ставки.Добавить(20000,   "20 000");
		ИначеЕсли ЗначениеЗаполнено(КатегорияТС) Тогда
			Ставки.Добавить(150000, "150 000");
		Иначе
			Ставки.Добавить(20000,   "20 000");
			Ставки.Добавить(150000, "150 000");
		КонецЕсли;
	...
	Если ИмяПоказателя = "НалоговыйОрган" Тогда
		...
		Иначе
			...
			Сообщение.Текст = НСтр("ru = 'Код налогового органа должен состоять из 4 цифр. Значение очищено';
									|en = 'Код налогового органа должен состоять из 4 цифр. Значение очищено'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел3" Тогда
		...
		БазоваяСтавка = ТДО.Найти("П000030014003").Значение;
		
		Если БазоваяСтавка = 0 Тогда
			ЗапретитьВводДанныхВОбласть(Форма, ТДО["П000030015003"]);
			ЗапретитьВводДанныхВОбласть(Форма, ТДО["П000030016003"]);
			Инд = АвтоРасчПоказатели.Найти("П000030016003");
			Если Инд <> Неопределено Тогда
				АвтоРасчПоказатели.Удалить(Инд);
			КонецЕсли;
		Иначе
			РазрешитьВводДанныхВОбласть(Форма, ТДО["П000030015003"]);
			РазрешитьВводДанныхВОбласть(Форма, ТДО["П000030016003"]);
		КонецЕсли;
	...
	Если ИмяРаздела = "Раздел3" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				// Стр.160.
				ТДО["П000030016003"].Значение = Окр(ТДО["П000030014003"].Значение * ТДО["П000030015003"].Значение);
				
				// Стр.170.
				ТДО["П000030017003"].Значение = ТДО["П000030016003"].Значение - ТДО["П000030013003"].Значение;
	...
	Если ИмяРаздела = "Раздел3" Тогда
		...
		Для каждого Ветка_2 Из ДеревоДанных.Строки Цикл
			...
			Для каждого Ветка Из Ветка_2.Строки[0].Строки Цикл
				...
				// Стр.160.
				Данные.Вставить("П000030016003", Окр(Данные["П000030014003"] * Данные["П000030015003"]));
				
				// Стр.170.
				Данные.Вставить("П000030017003", Данные["П000030016003"] - Данные["П000030013003"]);
				
				СуммаСтр170 = СуммаСтр170 + Макс(0, Данные["П000030017003"]);
		...
		ДанныеР1.Вставить("П000010003003", СуммаСтр170);
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ.Вставить("П000030016003");
	Структ.Вставить("П000030017003");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "УтилСбор");
	...
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "NO_UTILSB"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ЗаполненРаздел(ДеревоРаздела, "Раздел2") Тогда
		...
		ВозможныеПризнакиТС    = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000030004003");
		ВозможныеКатегорииТС   = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000030009003");
		ВозможныеТипыДвигателя = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000030011003");
		...
		Для каждого Ветка Из ДеревоРаздела.Строки Цикл
			...
			Для каждого ПодчВетка Из Ветка.Строки[0].Строки Цикл
				...
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					...
					+ ": не задан признак транспортного средства", "Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				ИначеЕсли ВозможныеПризнакиТС.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": некорректно задан признак транспортного средства. Возможны значения: "
					+ ВозможныеПризнакиТС.СтрокаЗначений, "Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": не задан идентификационный номер транспортного средства (VIN)", "Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				ИначеЕсли НЕ ИдНомТССоответствуетТребованиям(ЗначПок, ТекстСообщения) Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": неверно задан идентификационный номер транспортного средства (VIN)"
					+ ?(ЗначениеЗаполнено(ТекстСообщения), ". " + ТекстСообщения, ""), "Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ИмяПок = "П000030008003";
				ЗначПок = ПодчВетка.Данные[ИмяПок];
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": не задана дата выпуска транспортного средства", "Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(ЗначПок) Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": задана дата выпуска транспортного средства, выходящая за пределы периода с 01.01.1900 по 31.12.2099",
					"Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ИмяПок = "П000030009003";
				КатегорияТС = СокрЛП(ПодчВетка.Данные[ИмяПок]);
				Если НЕ ЗначениеЗаполнено(КатегорияТС) Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": не задана категория (вид) транспортного средства", "Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				ИначеЕсли ВозможныеКатегорииТС.МассивЗначений.Найти(КатегорияТС) = Неопределено Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": некорректно задана категория (вид) транспортного средства. Возможны значения: "
					+ ВозможныеКатегорииТС.СтрокаЗначений, "Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				Иначе
					ИмяПок = "П000030014003";
					ЗначПок = ПодчВетка.Данные[ИмяПок];
					СтрокаПроверки = "M1, M1G";
					Если СтрРазделить(СтрокаПроверки, ", ", Ложь).Найти(КатегорияТС) <> Неопределено
						И НЕ (ЗначПок = 20000 ИЛИ ЗначПок = 0) Тогда
						СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
						+ ": базовая ставка может быть равна 20 000 рублей или 0 рублей, если категория (вида) транспортного средства "
						+ "равна M1 или M1G", "Раздел3", ИмяПок, КоординатыПодчВетки);
						СтатусПроверки = Ложь;
					ИначеЕсли СтрРазделить(СтрокаПроверки, ", ", Ложь).Найти(КатегорияТС) = Неопределено
						И НЕ (ЗначПок = 150000 ИЛИ ЗначПок = 0) Тогда
						СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
						+ ": базовая ставка может быть равна 150 000 рублей или 0 рублей, если категория (вида) транспортного средства "
						+ "не равна M1 или M1G", "Раздел3", ИмяПок, КоординатыПодчВетки);
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
				
				ИмяПок = "П000030011003";
				ЗначПок = СокрЛП(ПодчВетка.Данные[ИмяПок]);
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": не задан тип двигателя", "Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				ИначеЕсли ВозможныеТипыДвигателя.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": некорректно задан тип двигателя. Возможны значения: " + ВозможныеТипыДвигателя.СтрокаЗначений,
					"Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				ЗначПок = ПодчВетка.Данные[ИмяПок];
				Если ЗначПок <> Окр(ПодчВетка.Данные["П000030014003"] * ПодчВетка.Данные["П000030015003"]) Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": стр.160 должна быть равна произведению стр.140 и стр.150", "Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ИмяПок = "П000030017003";
				ЗначПок = ПодчВетка.Данные[ИмяПок];
				Если ЗначПок <> ПодчВетка.Данные["П000030016003"] - ПодчВетка.Данные["П000030013003"] Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": стр.170 должна быть равна разности стр.160 и стр.130", "Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ИмяПок = "П000030018003";
				ЗначПок = СокрЛП(ПодчВетка.Данные[ИмяПок]);
				...
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					...
					СтатусПроверки = Ложь;
				ИначеЕсли НЕ (ДлинаЗначПок = 15 И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ЗначПок, , Истина)) Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": серия, номер паспорта должны быть представлены в виде 15 цифр", "Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				ИначеЕсли ЗначПок = "000000000000000" Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
					+ ": для серии, номера паспорта не допускается значение 000000000000000", "Раздел3", ИмяПок, КоординатыПодчВетки);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ИмяПок = "П000030019003";
				ЗначПок = ПодчВетка.Данные[ИмяПок];
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + " -> Раздел 3 -> Страница " + НомПодчСтраницы
...
&НаСервере
Функция ИдНомТССоответствуетТребованиям(Знач ИдНомТС, ТекстСообщения)
	
	ИдНомТС = СокрЛП(ИдНомТС);
	
	Если СтрДлина(ИдНомТС) <> 17 Тогда
		
		ТекстСообщения = НСтр("ru = 'Значение должно состоять из 17 символов';
								|en = 'Значение должно состоять из 17 символов'");
		
		Возврат Ложь;
		
	Иначе
		
		ЕстьХотяБы1Цифра = Ложь;
		ЕстьХотяБы1БукваЛатиницы = Ложь;
		
		Для Ном = 1 По 17 Цикл
			
			ТекСимвол = Сред(ИдНомТС, Ном, 1);
			
			ЕстьХотяБы1Цифра = ЕстьХотяБы1Цифра ИЛИ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ТекСимвол);
			
			ЕстьХотяБы1БукваЛатиницы = ЕстьХотяБы1БукваЛатиницы
			ИЛИ (ЗначениеЗаполнено(ТекСимвол) И СтроковыеФункцииКлиентСервер.ТолькоЛатиницаВСтроке(ТекСимвол));
			
			Если ЕстьХотяБы1Цифра И ЕстьХотяБы1БукваЛатиницы Тогда
		...
		Если НЕ (ЕстьХотяБы1Цифра И ЕстьХотяБы1БукваЛатиницы) Тогда
			
			ТекстСообщения = НСтр("ru = 'Допускаются цифры и буквы латинского алфавита';
									|en = 'Допускаются цифры и буквы латинского алфавита'");
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, ПараметрыВыгрузки)
	...
	Для каждого Ветка Из ДеревоРаздела.Строки Цикл
		...
		Для каждого ПодчВетка Из Ветка.Строки[0].Строки Цикл
			...
			КатегорияТС = СокрЛП(ПодчВетка.Данные["П000030009003"]);
			Для каждого Эл Из ЭкзРасчСбор.Строки Цикл
				Если (Эл.Ключ = "П000030010003" И СтрРазделить(
					"M1,M2,M3,M1G,M2G,M3G", ",", Ложь).Найти(КатегорияТС) <> Неопределено)
					ИЛИ (Эл.Ключ = "П000030012003" И СтрРазделить(
					"N1,N2,N3,N1G,N2G,N3G,O4", ",", Ложь).Найти(КатегорияТС) <> Неопределено) Тогда
					Эл.Обязательность = "О";
				КонецЕсли;
			КонецЦикла;
...
Функция ВыгрузкаОтчета(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "УтилизационныйСбор");
...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	Для Каждого Область Из ИсходныйМакет.Области Цикл
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник
			...
			Если Область.Имя = "П000030015003" И Область.Значение = 0 Тогда
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Расчет утил сбора колесные ТС");
...
Процедура СформироватьДанныеОтчета()
	...
	ТаблицаР03.Колонки.Добавить("Р03С050"		, ТипСтрока);
	ТаблицаР03.Колонки.Добавить("Р03С180"		, ТипСтрока);
	...
	ДанныеКС.Вставить("Турборежим", КолвоСтраницРаздела3 > 300);
	...
	Если НЕ СтруктураИсточник.Строки.Количество() = 0 Тогда
		Для каждого Приложение Из СтруктураИсточник.Строки Цикл
				...
				Для каждого СтраницаПриложения Из Приложение.Строки Цикл
					...
					НоваяСтрока.НомерСтраницы = ДанныеСтраницы.НомерСтраницы;
					НоваяСтрока.Р03С050 = СокрЛП(ДанныеСтраницы.П000030005003);
					НоваяСтрока.Р03С180 = СокрЛП(ДанныеСтраницы.П000030018003);
...
Процедура СформироватьСлужебныеДанные()
	...
	ИменаРазделов.Вставить("Р04", "Сведения");
	...
	ПредставленияРазделов.Вставить("Р04", "Сведений о физическом лице");
	...
	// Раздел 3
	МассивПолейСписочныхЗначений.Добавить("П000030004003");
	МассивПолейСписочныхЗначений.Добавить("П000030009003");
...
Процедура СформироватьПриоритетыРазделов()
	...
	МассивКатегорий1 = Новый Массив;
	МассивКатегорий1.Добавить("M1");
	МассивКатегорий1.Добавить("M1G");
	
	ДанныеКС.Вставить("МассивКатегорий1", МассивКатегорий1);
...
Процедура ПроверкаКС_1_01()
	...
	Заголовки.Вставить("Раздел"				, "Раздел 1");
	Заголовки.Вставить("НомерКС"			, "1.01");
	Заголовки.Вставить("Описание"			, "Значение КБК в стр.010 Раздела 1 должно соответствовать списку " +
		"допустимых значений");
	...
	Заголовки.Вставить("Рекомендации"		, НСтр("ru = 'Выберите КБК в стр.010 Раздела 1 из списка значений';
													|en = 'Выберите КБК в стр.010 Раздела 1 из списка значений'"));
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	...
	Если НЕ Список = Неопределено Тогда
		...
		НастройкиКС.ЭтоОшибка = Список.Получить(Показатели.Р01С010.Значение) = Неопределено;
...
Процедура ПроверкаКС_1_02()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 1");
	Заголовки.Вставить("НомерКС"			, "1.02");
	Заголовки.Вставить("Описание"			, "Сумма утилизационного сбора в стр.030 Раздела 1 = сумме всех " +
		"стр.170 Раздела 3");
	Заголовки.Вставить("ШаблонДетализации"	, "%1 = %2");
	Заголовки.Вставить("Нарушение"			, "Возможно ошибочная сумма утилизационного сбора");
	Заголовки.Вставить("Рекомендации"		, "Укажите в стр.030 Раздела 1 сумму утилизационного сбора из стр.170 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru=''"));
	ПодготовитьНастройки(Заголовки);
	...
	Для каждого СтраницаРаздела2 Из ДанныеКС.Раздел2 Цикл
		Для каждого СтраницаРаздела3 Из СтраницаРаздела2.Раздел21 Цикл
			...
			Если ДанныеКС.Турборежим Тогда
				ЗаполнитьПоказатель(Показатели.Р03С170, СтраницаРаздела3);
			Иначе
				Показатели.Р03С170.Дополнение = "Страница " + СтраницаРаздела3.НомерСтраницы;
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р03С170, СтраницаРаздела3);
			КонецЕсли;
			
			Стр170Итог = Стр170Итог + Макс(0, Показатели.Р03С170.Значение); 
		
		КонецЦикла; 
	КонецЦикла; 
	
	НастройкиКС.ЭтоОшибка = НЕ Показатели.Р01С030.Значение = Стр170Итог;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р01С030.Значение, Стр170Итог);
	ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_01()
	...
	Заголовки.Вставить("НомерКС"			, "2.01");
	Заголовки.Вставить("Описание"			, "Дата составления расчета на Титульном листе >= всех стр.190 Раздела 3");
	Заголовки.Вставить("ШаблонДетализации"	, "%1 >= %2");
	Заголовки.Вставить("Нарушение"			, "Возможно ошибочная дата выдачи паспорта ТС");
	Заголовки.Вставить("Рекомендации"		, "Уточните дату выдачи паспорта ТС в стр.190 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru=''"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей.Вставить("Р03С190"		, "П000030019003");
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	
	ЗарегистрироватьПроверку();
	ЗаполнитьИРасшифроватьПоказатель(Показатели.Р00С001, ДанныеКС.Раздел0);
	МаксДатаВыдачи = '00010101';
	
	Для каждого СтраницаРаздела2 Из ДанныеКС.Раздел2 Цикл
		Для каждого СтраницаРаздела3 Из СтраницаРаздела2.Раздел21 Цикл
			
			Если ДанныеКС.Турборежим Тогда
				ЗаполнитьПоказатель(Показатели.Р03С190, СтраницаРаздела3);
			Иначе
				Показатели.Р03С190.Дополнение = "Страница " + СтраницаРаздела3.НомерСтраницы;
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р03С190, СтраницаРаздела3);
			КонецЕсли;
			
			МаксДатаВыдачи = Макс(МаксДатаВыдачи, Показатели.Р03С190.Значение);
		
		КонецЦикла; 
	КонецЦикла; 
	
	НастройкиКС.ЭтоОшибка = Показатели.Р00С001.Значение < МаксДатаВыдачи;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р00С001.ЗначениеСтр, Формат(МаксДатаВыдачи, "ДФ=dd.MM.yy; ДП=(...)"));
	ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_02()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 3");
	Заголовки.Вставить("НомерКС"			, "2.02");
	Заголовки.Вставить("Описание"			, "Значение признака транспортного средства в стр.040 Раздела 3 должно " +
		"соответствовать списку допустимых значений");
	Заголовки.Вставить("ШаблонДетализации"	, "Недопустимое значение %1");
	Заголовки.Вставить("Нарушение"			, "Возможно ошибочное значение признака транспортного средства");
	Заголовки.Вставить("Рекомендации"		, 
		"Выберите признак транспортного средства в стр.040 Раздела 3 из списка значений");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, "Страница %1 Раздела 3");
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("ИмяПоказателя", "П000030004003");
	ПроверкаКС_2_02_код();
	...
	Если НЕ Список = Неопределено Тогда
		...
		Для каждого СтраницаРаздела2 Из ДанныеКС.Раздел2 Цикл
			Для каждого СтраницаРаздела3 Из СтраницаРаздела2.Раздел21 Цикл
				...
				НастройкиКС.ЭтоОшибка = Список.Получить(Показатели.Р03С001.Значение) = Неопределено;
				
				Если ДанныеКС.Турборежим И НЕ НастройкиКС.ЭтоОшибка Тогда
					Продолжить;
				КонецЕсли; 
				...
				НастройкиКС.ДопСведения = СтрШаблон(НастройкиКС.ШаблонДопСведений, 
					СтраницаРаздела3.НомерСтраницы);
				ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_03()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 3");
	Заголовки.Вставить("НомерКС"			, "2.03");
	Заголовки.Вставить("Описание"			, "Сумма базовой ставки в стр.140 Раздела 3 = 20.000, 0, если категория " +
		"ТС в стр.090 Раздела 3 = " + СтрСоединить(ДанныеКС.МассивКатегорий1, ", "));
	Заголовки.Вставить("ШаблонДетализации"	, 
		"%1 = 20.000, 0, если %2 = " + СтрСоединить(ДанныеКС.МассивКатегорий1, ", "));
	Заголовки.Вставить("Нарушение"			, "Возможно ошибочная сумма базовой ставки ТС");
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму базовой ставки ТС в стр.140 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, "Страница %1 Раздела 3");
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р03С001"		, "П000030014003");
	ИменаПоказателей.Вставить("Р03С002"		, "П000030009003");
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	
	МассивСумм = Новый Массив;
	МассивСумм.Добавить(20000);
	...
	Для каждого СтраницаРаздела2 Из ДанныеКС.Раздел2 Цикл
		Для каждого СтраницаРаздела3 Из СтраницаРаздела2.Раздел21 Цикл
				...
				НастройкиКС.ЭтоОшибка = НЕ ДанныеКС.МассивКатегорий1.Найти(Показатели.Р03С002.Значение) = Неопределено
					И МассивСумм.Найти(Показатели.Р03С001.Значение) = Неопределено;
...
Процедура ПроверкаКС_2_04()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 3");
	Заголовки.Вставить("НомерКС"			, "2.04");
	Заголовки.Вставить("Описание"			, "Сумма базовой ставки в стр.140 Раздела 3 = 150.000, 0, если категория " +
		"ТС в стр.090 Раздела 3 <> " + СтрСоединить(ДанныеКС.МассивКатегорий1, ", "));
	Заголовки.Вставить("ШаблонДетализации"	, 
		"%1 = 150.000, 0, если %2 <> " + СтрСоединить(ДанныеКС.МассивКатегорий1, ", "));
	Заголовки.Вставить("Нарушение"			, "Возможно ошибочная сумма базовой ставки ТС");
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму базовой ставки ТС в стр.140 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	...
	Для каждого СтраницаРаздела2 Из ДанныеКС.Раздел2 Цикл
		Для каждого СтраницаРаздела3 Из СтраницаРаздела2.Раздел21 Цикл
				...
				ЗаполнитьПоказатели(Показатели, СтраницаРаздела3);
				НастройкиКС.ЭтоОшибка = ДанныеКС.МассивКатегорий1.Найти(Показатели.Р03С002.Значение) = Неопределено
					И МассивСумм.Найти(Показатели.Р03С001.Значение) = Неопределено;
...
Процедура ПроверкаКС_2_05()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 3");
	Заголовки.Вставить("НомерКС"			, "2.05");
	Заголовки.Вставить("Описание"			, "Значение категории транспортного средства в стр.090 Раздела 3 должно " +
		"соответствовать списку допустимых значений");
	Заголовки.Вставить("ШаблонДетализации"	, "Недопустимое значение %1");
	Заголовки.Вставить("Нарушение"			, "Возможно ошибочное значение категории транспортного средства");
	Заголовки.Вставить("Рекомендации"		, 
		"Выберите категорию транспортного средства в стр.090 Раздела 3 из списка значений");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, "Страница %1 Раздела 3");
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("ИмяПоказателя", "П000030009003");
	ПроверкаКС_2_02_код();
...
Процедура ПроверкаКС_2_06()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 3");
	Заголовки.Вставить("НомерКС"			, "2.06");
	Заголовки.Вставить("Описание"			, "Категория транспортного средства в стр.090 Раздела 3 = " +
		"O4, если значение признака в стр.040 Раздела 3 = 25, 26");
	Заголовки.Вставить("ШаблонДетализации"	, "%1 = О4, если %2 = 25, 26");
	Заголовки.Вставить("Нарушение"			, "Возможно ошибочное значение категории транспортного средства");
	Заголовки.Вставить("Рекомендации"		, "Укажите категорию транспортного средства О4 в стр.090 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, "Страница %1 Раздела 3");
	ПодготовитьНастройки(Заголовки);
	...
	МассивЗначений.Добавить("25");
	МассивЗначений.Добавить("26");
	...
	Для каждого СтраницаРаздела2 Из ДанныеКС.Раздел2 Цикл
		Для каждого СтраницаРаздела3 Из СтраницаРаздела2.Раздел21 Цикл
			...
			НастройкиКС.ЭтоОшибка = НЕ МассивЗначений.Найти(Показатели.Р03С040.Значение) = Неопределено
				И НЕ Показатели.Р03С090.Значение = "O4";
			...
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
				Показатели.Р03С090.ЗначениеСтр, Показатели.Р03С040.ЗначениеСтр);
			НастройкиКС.ДопСведения = СтрШаблон(НастройкиКС.ШаблонДопСведений, 
				СтраницаРаздела3.НомерСтраницы);
			ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_07()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 3");
	Заголовки.Вставить("НомерКС"			, "2.07");
	Заголовки.Вставить("Описание"			, "Значение типа двигателя транспортного средства в стр.110 Раздела 3 должно " +
		"соответствовать списку допустимых значений");
	Заголовки.Вставить("ШаблонДетализации"	, "Недопустимое значение %1");
	Заголовки.Вставить("Нарушение"			, "Возможно ошибочное значение типа двигателя транспортного средства");
	Заголовки.Вставить("Рекомендации"		, 
		"Выберите тип двигателя транспортного средства в стр.110 Раздела 3 из списка значений");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, "Страница %1 Раздела 3");
	ПодготовитьНастройки(Заголовки);
...
Процедура ПроверкаКС_2_08()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 3");
	Заголовки.Вставить("НомерКС"			, "2.08");
	Заголовки.Вставить("Описание"			, "Сумма базовой ставки в стр.140 Раздела 3 = 0, 20.000, 150.000");
	Заголовки.Вставить("ШаблонДетализации"	, "Недопустимое значение %1");
	Заголовки.Вставить("Нарушение"			, "Возможно ошибочная сумма базовой ставки");
	Заголовки.Вставить("Рекомендации"		, "Уточните значение базовой ставки в стр.140 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, "Страница %1 Раздела 3");
	ПодготовитьНастройки(Заголовки);
	...
	МассивЗначений.Добавить(20000);
	МассивЗначений.Добавить(150000);
	...
	Для каждого СтраницаРаздела2 Из ДанныеКС.Раздел2 Цикл
		Для каждого СтраницаРаздела3 Из СтраницаРаздела2.Раздел21 Цикл
			...
			НастройкиКС.ЭтоОшибка = СтраницаРаздела3.РазделЗаполнен
				И МассивЗначений.Найти(Показатели.Р03С140.Значение) = Неопределено;
			...
			ЗарегистрироватьПроверку();
			РасшифроватьПоказатель(Показатели.Р03С140);
			
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
				Показатели.Р03С140.Значение);
			НастройкиКС.ДопСведения = СтрШаблон(НастройкиКС.ШаблонДопСведений, 
				СтраницаРаздела3.НомерСтраницы);
			ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_10()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 3");
	Заголовки.Вставить("НомерКС"			, "2.10");
	Заголовки.Вставить("Описание"			, "Сумма утилизационного сбора в стр.160 Раздела 3 = стр.(140 х 150) Раздела 3");
	Заголовки.Вставить("ШаблонДетализации"	, "%1 = %2 (%3 х %4)");
	Заголовки.Вставить("Нарушение"			, "Возможно ошибочная сумма утилизационного сбора");
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму утилизационного сбора в стр.160 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, "Страница %1 Раздела 3");
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р03С160", "П000030016003");
	ИменаПоказателей.Вставить("Р03С140", "П000030014003");
	...
	Для каждого СтраницаРаздела2 Из ДанныеКС.Раздел2 Цикл
		Для каждого СтраницаРаздела3 Из СтраницаРаздела2.Раздел21 Цикл
			...
			ЗаполнитьПоказатели(Показатели, СтраницаРаздела3);
			РасчетнаяСумма = Окр(Показатели.Р03С140.Значение * Показатели.Р03С150.Значение);
			
			НастройкиКС.ЭтоОшибка = НЕ Показатели.Р03С160.Значение = РасчетнаяСумма;
			...
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
				Показатели.Р03С160.Значение, РасчетнаяСумма, Показатели.Р03С140.Значение, Показатели.Р03С150.Значение);
			НастройкиКС.ДопСведения = СтрШаблон(НастройкиКС.ШаблонДопСведений, 
				СтраницаРаздела3.НомерСтраницы);
			ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_11()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 3");
	Заголовки.Вставить("НомерКС"			, "2.11");
	Заголовки.Вставить("Описание"			, "Сумма утилизационного сбора в стр.170 Раздела 3 = стр.(160 - 130) Раздела 3");
	Заголовки.Вставить("ШаблонДетализации"	, "%1 = %2 (%3 - %4)");
	Заголовки.Вставить("Нарушение"			, "Возможно ошибочная сумма утилизационного сбора");
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму утилизационного сбора в стр.170 Раздела 3");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, "Страница %1 Раздела 3");
	ПодготовитьНастройки(Заголовки);
	...
	Для каждого СтраницаРаздела2 Из ДанныеКС.Раздел2 Цикл
		Для каждого СтраницаРаздела3 Из СтраницаРаздела2.Раздел21 Цикл
			...
			ЗаполнитьПоказатели(Показатели, СтраницаРаздела3);
			РасчетнаяСумма = Макс(0, Показатели.Р03С160.Значение - Показатели.Р03С130.Значение);
			
			НастройкиКС.ЭтоОшибка = НЕ Показатели.Р03С170.Значение = РасчетнаяСумма;
			...
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
				Показатели.Р03С170.Значение, РасчетнаяСумма, Показатели.Р03С160.Значение, Показатели.Р03С130.Значение);
			НастройкиКС.ДопСведения = СтрШаблон(НастройкиКС.ШаблонДопСведений, 
				СтраницаРаздела3.НомерСтраницы);
			ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_12()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 3");
	Заголовки.Вставить("НомерКС"			, "2.12");
	Заголовки.Вставить("Описание"			, "Идентификационный номер транспортного средства (VIN) в стр.050 Раздела 3 " +
		"должен быть уникальным для всех страниц Раздела 3");
	Заголовки.Вставить("ШаблонДетализации"	, "Неуникальное значение %1");
	Заголовки.Вставить("Нарушение"			, "Повторяющиеся значения идентификационного номера ТС (VIN)");
	Заголовки.Вставить("Рекомендации"		, "Удалите повторяющиеся идентификационные номера");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, "Страницы %1 Раздела 3");
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "Р03С050");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000030005003");
...
Процедура ПроверкаКС_2_12_код()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_1.Р03С050 КАК Р03С050,
	|	СУММА(ВТ_1.Колво) КАК Колво
	|ПОМЕСТИТЬ ВТ_2
	|ИЗ
	|	ВТ_1 КАК ВТ_1
	|
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "Р03С050", НастройкиКС.Показатель1);
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока ВыборкаГр.Следующий() Цикл
			Если НЕ ЗначениеЗаполнено(ВыборкаГр.Р03С001) Тогда
				Продолжить;
			КонецЕсли;
			...
			Пока Счетчик < 300 И Выборка.Следующий() Цикл
				ЗаполнитьПоказатель(Показатели.Р03С001, Выборка);
				Показатели.Р03С001.Дополнение = НСтр("ru = 'Страница ';
													|en = 'Страница '") + Выборка.НомерСтраницы;
				РасшифроватьПоказатель(Показатели.Р03С001);
				Счетчик = Счетчик + 1;
				Если МассивСтраниц.Количество() < 10 Тогда
					МассивСтраниц.Добавить(Выборка.НомерСтраницы);
				КонецЕсли;
			КонецЦикла; 
...
Процедура ПроверкаКС_2_15()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 3");
	Заголовки.Вставить("НомерКС"			, "2.15");
	Заголовки.Вставить("Описание"			, "Серия и номер паспорта транспортного средства в стр.180 Раздела 3 " +
		"должны быть уникальными для всех страниц Раздела 3");
	Заголовки.Вставить("ШаблонДетализации"	, "Неуникальное значение %1");
	Заголовки.Вставить("Нарушение"			, "Повторяющиеся значения серии и номера паспорта ТС");
	Заголовки.Вставить("Рекомендации"		, "Удалите повторяющиеся серии и номера паспорта ТС");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, "Страницы %1 Раздела 3");
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "Р03С180");
	НастройкиКС.Вставить("ИмяПоказателя"	, "П000030018003");
...
Процедура ПроверкаКС_2_18()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 3");
	Заголовки.Вставить("НомерКС"			, "2.18");
	Заголовки.Вставить("Описание"			, "Серия и номер паспорта транспортного средства в стр.180 Раздела 3 " +
		"должны быть заполнены");
	Заголовки.Вставить("ШаблонДетализации"	, "Серия и номер паспорта ТС: %1");
	Заголовки.Вставить("Нарушение"			, "Не заполнена серия и номер паспорта ТС");
	Заголовки.Вставить("Рекомендации"		, "Заполните серию и номер паспорта ТС");
	Заголовки.Вставить("Основание"			, НСтр("ru = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@';
													|en = 'Письмо ФНС России от 04.03.2024 № СД-4-3/2448@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, "Страница %1 Раздела 3");
	ПодготовитьНастройки(Заголовки);
	...
	Для каждого СтраницаРаздела2 Из ДанныеКС.Раздел2 Цикл
		Для каждого СтраницаРаздела3 Из СтраницаРаздела2.Раздел21 Цикл
			...
			ЗаполнитьПоказатель(Показатели.Р03С180, СтраницаРаздела3);
			НастройкиКС.ЭтоОшибка = СтраницаРаздела3.РазделЗаполнен И НЕ ЗначениеЗаполнено(Показатели.Р03С180.Значение);
			...
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
				Показатели.Р03С180.ЗначениеСтр);
			НастройкиКС.ДопСведения = СтрШаблон(НастройкиКС.ШаблонДопСведений, 
				СтраницаРаздела3.НомерСтраницы);
			ДополнитьТаблицуРезультата();
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Область.Имя = "ПоМесту" Тогда
		...
		Если НовоеЗначение = "112" И ПрежнееЗначение <> "112" Тогда
			...
			|en = 'При входе в режим заполнения для физического лица,
			|не являющегося индивидуальным предпринимателем, будут
			|очищены сведения Титульного листа. Продолжить?'");
		...
		ИначеЕсли НовоеЗначение <> "112" И ПрежнееЗначение = "112" Тогда
			...
			|en = 'При выходе из режима заполнения для физического лица,
			|не являющегося индивидуальным предпринимателем, будут
			|перезаполнены сведения Титульного листа. Продолжить?'");
			ПоказатьВопрос(Оп, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 60, КодВозвратаДиалога.Нет);
			Возврат;
