...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Попытка
		СтруктураРеквизитовФормы.НаимОрганизацииСокр =
			СокрЛП(СтруктураРеквизитовФормы.Организация.НаименованиеСокращенное);
	Исключение
		СтруктураРеквизитовФормы.НаимОрганизацииСокр = "";
	КонецПопытки;
	...
	Элементы.ФормаЗаполнитьДаннымиОтчетов.Видимость =
		(Перечисления.Периодичность.Месяц = СтруктураРеквизитовФормы.мПериодичность);
	СтруктураРеквизитовФормы.Вставить("РеализованоАвтозаполнениеДаннымиПредидущегоПериода",
		(Перечисления.Периодичность.Месяц = СтруктураРеквизитовФормы.мПериодичность));
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "") Экспорт
	...
	Для Гр = 4 По 5 Цикл
		Тот = 0;
		Для Стр = 2 По 34 Цикл
			Если Стр = 13 Или Стр = 18 Или Стр = 26 Или Стр = 29 Тогда
				Продолжить;
			КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Страница = 1 По 2 Цикл
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_" + Страница));
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма П (услуги)");
	КонецЦикла;
...
Процедура ФормироватьСтруктуруСтраницОтчета() Экспорт
	...
	СтрокаУровня1.Представление            = "Статистика: Форма П (услуги)";
...
Функция СформироватьПараметрыВыбораОтчетаПредыдущегоПериода() Экспорт 
	...
	Парам.Вставить("ИсточникОтчета", "РегламентированныйОтчетСтатистикаФормаПУслуги");
	Парам.Вставить("ВидОтправляемогоДокумента",
				   ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.СтатистикаФормаПУслуги"));
...
Процедура ЗаполнитьДаннымиОтчетовНаСервере(ТекстОписанияПроблемы)
	ЗаполнениеСтатистическойОтчетности.ЗаполнитьДаннымиПредидущегоПериода(
		ЭтотОбъект, "МакетПереносаДанных2022Кв1", ТекстОписанияПроблемы);
КонецПроцедуры
