...
Процедура ИзменитьПериод(Форма, Шаг)
	...
	Форма.Элементы.НадписьВыборПериода.Видимость = (Год(Форма.мДатаКонцаПериодаОтчета) <= 2019);
	Форма.Элементы.ПолеВыбораПериодичность.СписокВыбора.Очистить();
	Если (Год(Форма.мДатаКонцаПериодаОтчета) <= 2019) Тогда 
		Форма.Элементы.ПолеВыбораПериодичность.СписокВыбора.Добавить(Форма.ПеречислениеПериодичностьМесяц);
		Форма.Элементы.ПолеВыбораПериодичность.СписокВыбора.Добавить(Форма.ПеречислениеПериодичностьКвартал);
	Иначе
		Форма.Элементы.ПолеВыбораПериодичность.СписокВыбора.Добавить(Форма.ПеречислениеПериодичностьМесяц);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(мДатаНачалаПериодаОтчета) И НЕ ЗначениеЗаполнено(мДатаКонцаПериодаОтчета) Тогда
		...
		Иначе 
			...
			мДатаНачалаПериодаОтчета = НачалоМесяца(мДатаКонцаПериодаОтчета);
		КонецЕсли;
	ИначеЕсли Год(мДатаНачалаПериодаОтчета) >= 2020 Тогда
		мПериодичность = ПеречислениеПериодичностьМесяц;
		мДатаНачалаПериодаОтчета = НачалоМесяца(мДатаКонцаПериодаОтчета);
	КонецЕсли;
