...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(НастройкаПериодОтчетаЗначение) 
		...
		Если НЕ НастройкаПериодаЗаданаКорректно(НастройкаПериодОтчетаЗначение.ДатаНачала,
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Длительность шага периодов сравнения не должна быть меньше периода отчета.';
																	|en = 'Length of the comparison period step should not be less than the report period.'"), , , , Отказ);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
