...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.Имя = "ОргАдрес" Тогда
		РегламентированнаяОтчетностьКлиент.ОбработкаАдресаВСтатистическомОтчетеJSON(
			ЭтотОбъект, Область, СтандартнаяОбработка);
	ИначеЕсли Область.Имя = "КодОбразования" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "КодОбразования");
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для НомКол = 2 По 3 Цикл
		Области["П0000010027" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
		= Области["П0000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
		- Области["П0000010023" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		
		Области["П0000010046" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
		= Области["П0000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
		- Области["П0000010028" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-МБ");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если Не ЗначениеЗаполнено(ПараметрыВыгрузки.КодОбразования) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			"Не указан код муниципально образования!", ИмяПоляТаблДокумента, "КодОбразования");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
		Если Не ЗначениеЗаполнено(ПараметрыВыгрузки.КодОКТМО) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			"Не указан код ОКТМО!", ИмяПоляТаблДокумента, "ОКТМО");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("КодОбразования", СокрЛП(ТабДокумент.КодОбразования));
	
	КодОКТМО = СокрЛП(ТабДокумент.ОКТМО);
	Если ЗначениеЗаполнено(КодОКТМО) Тогда
		Пока СтрДлина(КодОКТМО) < 11 Цикл
			КодОКТМО = КодОКТМО + "0";
		КонецЦикла;
	КонецЕсли;
	ПараметрыВыгрузки.Вставить("КодОКТМО", КодОКТМО);
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "КодОбразования",
		512, , "Выбор кода", КоллекцияСписковВыбора["КодОбразования"]);
