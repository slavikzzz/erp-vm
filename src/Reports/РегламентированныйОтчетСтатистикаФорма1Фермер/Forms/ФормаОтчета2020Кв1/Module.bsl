...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
    ...
    Если Лев(Область.Имя, 13) = "П000004000102" Тогда
		...
		ОбластьПарам = ПолеТабличногоДокументаФормаОтчета.Область("П000004000101_" + Прав(Область.Имя, 1));
		ОбластьПарам.Значение = ВыборКод;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	// Строка 1 = сумме стр. 6, с 8 по 13, 37; 38
	СуммаГр1 = 0;
	СуммаГр1 = Области["П01000601"].Значение;
	Для Гр = 8 По 13 Цикл
		СуммаГр1 = СуммаГр1 + Области["П01" + Формат(Гр, "ЧЦ=4; ЧВН=; ЧГ=0") + "01"].Значение;
	КонецЦикла;
	СуммаГр1 = СуммаГр1 + Области["П01003701"].Значение + Области["П01003801"].Значение;
	Области["П01000101"].Значение = СуммаГр1;
	
	// Строка 28 = сумме стр. с 14 по 24
	СуммаГр2 = 0;
	Для Гр = 14 По 24 Цикл
		СуммаГр2 = СуммаГр2 + Области["П01" + Формат(Гр, "ЧЦ=4; ЧВН=; ЧГ=0") + "01"].Значение;
	КонецЦикла;
	Области["П01002801"].Значение = СуммаГр2;
		
	// Строка 43 = сумме стр. с 29 по 42
	СуммаГр3 = 0;
	Для Гр = 29 По 42 Цикл
		СуммаГр3 = СуммаГр3 + Области["П01" + Формат(Гр, "ЧЦ=4; ЧВН=; ЧГ=0") + "01"].Значение;
	КонецЦикла;
	Области["П01004301"].Значение = СуммаГр3;
		
	// Строка 52 = сумме стр. с 45 по 51
	СуммаГр4 = 0;
	Для Гр = 45 По 51 Цикл		
		Если Гр= 49 Тогда
			СуммаГр4 = СуммаГр4 + Области["П000004000103_1"].Значение;
		ИначеЕсли Гр = 50 Тогда
			СуммаГр4 = СуммаГр4 + Области["П000004000103_2"].Значение;
		ИначеЕсли Гр = 51 Тогда	
			СуммаГр4 = СуммаГр4 + Области["П000004000103_3"].Значение;
		Иначе
			СуммаГр4 = СуммаГр4 + Области["П01" + Формат(Гр, "ЧЦ=4; ЧВН=; ЧГ=0") + "01"].Значение;
		КонецЕсли;	
	КонецЦикла;
	Области["П01005201"].Значение = СуммаГр4;
	
	// Строка 62 = сумме стр. с 55 по 61
	СуммаГр5 = 0;
	Для Гр = 55 По 61 Цикл
		СуммаГр5 = СуммаГр5 + Области["П01" + Формат(Гр, "ЧЦ=4; ЧВН=; ЧГ=0") + "01"].Значение;
	КонецЦикла;
	Области["П01006201"].Значение = СуммаГр5;
	
	// Строка 63 = сумме стр. с 9 по 13, 28, 43, 44, с 52 по 54, 62	
	СуммаГр6 = 0;	
	Для Гр = 9 По 13 Цикл
		СуммаГр6 = СуммаГр6 + Области["П01" + Формат(Гр, "ЧЦ=4; ЧВН=; ЧГ=0") + "01"].Значение;
	КонецЦикла;
	СуммаГр6 = СуммаГр6 + Области["П01002801"].Значение + Области["П01004301"].Значение + Области["П01004401"].Значение;
	Для Гр = 52 По 54 Цикл
		СуммаГр6 = СуммаГр6 + Области["П01" + Формат(Гр, "ЧЦ=4; ЧВН=; ЧГ=0") + "01"].Значение;
	КонецЦикла;
	СуммаГр6 = СуммаГр6 + Области["П01006201"].Значение;
	Области["П01006301"].Значение = СуммаГр6;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П01000101"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П01000101"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П01002801"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П01002801"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П01004301"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П01004301"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П01005201"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П01005201"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П01006201"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П01006201"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П01006301"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П01006301"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-Фермер");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1-Фермер");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000004000102", 250, , "Выбор",
		КоллекцияСписковВыбора["ОвощиОткрытогоГрунта"]);
