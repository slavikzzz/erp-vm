...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	СмежнаяОбласть.Значение = ВыборКод;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Пока Истина Цикл
		...
		Если Области.Найти("П001001000903" + "_" + СтрокаИнд) = Неопределено Тогда
			Прервать;
		КонецЕсли;
		СуммаПоСтроке9 = СуммаПоСтроке9 + Области["П001001000903" + "_" + СтрокаИнд].Значение;
	КонецЦикла;
	Области["П001001001003"].Значение = Области["П001001000803"].Значение + СуммаПоСтроке9;
	...
	Пока Истина Цикл
		...
		Если Области.Найти("П001001001403" + "_" + СтрокаИнд) = Неопределено Тогда
			Прервать;
		КонецЕсли;
		СуммаПоСтроке14 = СуммаПоСтроке14 + Области["П001001001403" + "_" + СтрокаИнд].Значение;
	КонецЦикла;
	Области["П001001001503"].Значение
		= Области["П001001001103"].Значение + Области["П001001001203"].Значение
		+ Области["П001001001303"].Значение + СуммаПоСтроке14;
	...
	Пока Истина Цикл
		...
		Если Области.Найти("П001001001703" + "_" + СтрокаИнд) = Неопределено Тогда
			Прервать;
		КонецЕсли;
		СуммаПоСтроке17 = СуммаПоСтроке17 + Области["П001001001703" + "_" + СтрокаИнд].Значение;
	КонецЦикла;
	Области["П001001001803"].Значение = СуммаПоСтроке17;
	...
	Пока Истина Цикл
		...
		Если Области.Найти("П001001002103" + "_" + СтрокаИнд) = Неопределено Тогда
			Прервать;
		КонецЕсли;
		СуммаПоСтроке21 = СуммаПоСтроке21 + Области["П001001002103" + "_" + СтрокаИнд].Значение;
	КонецЦикла;
	Области["П001001002303"].Значение = СуммаПоСтроке21 + Области["П001001002203"].Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П001001001503"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001001503"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П001001001803"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001001803"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П001001002303"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001002303"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П001001000301", 250, ,
	"Выбор кода культура", КоллекцияСписковВыбора["ОвощиСтрока3"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П001001000701", 250, ,
	"Выбор кода культура", КоллекцияСписковВыбора["ОвощиСтрока7"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П001001000901", 250, ,
	"Выбор кода культура", КоллекцияСписковВыбора["ОвощиСтрока9"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П001001001401", 250, ,
	"Выбор кода культура", КоллекцияСписковВыбора["ОвощиСтрока14"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П001001001701", 250, ,
	"Выбор кода культура", КоллекцияСписковВыбора["ОвощиСтрока17"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П001001002101", 250, ,
