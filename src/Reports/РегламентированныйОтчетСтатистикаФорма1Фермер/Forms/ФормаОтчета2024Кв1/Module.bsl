...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0611001";
...
Функция ПолучитьМассивКатегорий()
	Возврат СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
		"П0010010003;П0010010007;П0010010009;П0010010014;П0010010017;П0010010021", ";", Истина);
КонецФункции
...
Процедура ДополнитьРеквизитыФормы()
	...
	Для Каждого Стр Из КатегорииКлассификатора Цикл
		...
		ИзменитьРеквизиты(ДобавляемыеРеквизиты);
		СтруктураРеквизитовФормы.Вставить("мИдГруппы" + ИндексГруппы, ИмяДобавляемогоРеквизита);
		...
		мСтруктураКолвоКолонокРазделов.Вставить(ИмяДобавляемогоРеквизита, 3);
		СтруктураРеквизитовФормы.мСоответствияМнгЧ.Вставить(ИмяДобавляемогоРеквизита, "МногострочнаяЧасть" + ИндексГруппы);
...
Процедура ПоказатьПериод()
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчетаТабЧасть.Значение = "Всего посеяно озимых и яровых под урожай "
		+ Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг'") + " года";
					...
					Если СтруктураГруппы[0].Свойство(КолонкаИмя) Тогда
						...
						Если НомерГрафы = КодКолонкиПервойЯчейкиСтроки Тогда
							ОбластьЯчейкиУдалитьСтроку = ТекТабличноеПоле.Область(
								ОбластьСдвинутойЯчейки.Верх, 3, ОбластьСдвинутойЯчейки.Верх, 3);
		...
		Если СтруктураГруппы[0].Свойство(ИмяКолонки) Тогда
			...
			Если НомерГрафы = КодКолонкиПервойЯчейкиСтроки Тогда
				...
				ИсхОбластьЯчейкиУдалитьСтроку = ТекТабличноеПоле.Область(
					ОбластьЯчейкиИсходнойОбласти.Верх, 3, ОбластьЯчейкиИсходнойОбласти.Верх, 3);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 И Область.Гиперссылка Тогда
		УдалитьСтроку("");
	ИначеЕсли СтрНайти(Область.Имя, "_") > 1 И (Сред(Область.Имя, 13, 1) = "1" Или Сред(Область.Имя, 13, 1) = "2") Тогда
		...
		Если Сред(Область.Имя, 10, 2) = "03" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникОвощиСтрока3");
		ИначеЕсли Сред(Область.Имя, 10, 2) = "07" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникОвощиСтрока7");
		ИначеЕсли Сред(Область.Имя, 10, 2) = "09" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникОвощиСтрока9");
		ИначеЕсли Сред(Область.Имя, 10, 2) = "14" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникОвощиСтрока14");
		ИначеЕсли Сред(Область.Имя, 10, 2) = "17" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникОвощиСтрока17");
		ИначеЕсли Сред(Область.Имя, 10, 2) = "21" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникОвощиСтрока21");
		КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	СуммаПоСтроке10 = 0;
	Пока Истина Цикл
		СтрокаИнд = СтрокаИнд + 1;
		Если Области.Найти("П001001000903" + "_" + СтрокаИнд) = Неопределено Тогда
			Прервать;
		КонецЕсли;
		СуммаПоСтроке10 = СуммаПоСтроке10 + Области["П001001000903" + "_" + СтрокаИнд].Значение;
	КонецЦикла;
	Области["П001001001003"].Значение = Области["П001001000803"].Значение + СуммаПоСтроке10;
	...
	Пока Истина Цикл
		СтрокаИнд = СтрокаИнд + 1;
		Если Области.Найти("П001001001403" + "_" + СтрокаИнд) = Неопределено Тогда
			Прервать;
		КонецЕсли;
		СуммаПоСтроке17 = СуммаПоСтроке17 + Области["П001001001403" + "_" + СтрокаИнд].Значение;
	КонецЦикла;
	Области["П001001001703"].Значение
		= Области["П001001001103"].Значение
		+ Области["П001001001203"].Значение
		+ Области["П001001001303"].Значение
		+ Области["П001001001403"].Значение
		+ Области["П001001001503"].Значение
		+ СуммаПоСтроке17;
	...
	Пока Истина Цикл
		СтрокаИнд = СтрокаИнд + 1;
		Если Области.Найти("П001001001703" + "_" + СтрокаИнд) = Неопределено Тогда
			Прервать;
		КонецЕсли;
		СуммаПоСтроке20 = СуммаПоСтроке20 + Области["П001001001703" + "_" + СтрокаИнд].Значение;
	КонецЦикла;
	Области["П001001002003"].Значение = СуммаПоСтроке20;
	...
	Пока Истина Цикл
		СтрокаИнд = СтрокаИнд + 1;
		Если Области.Найти("П001001002103" + "_" + СтрокаИнд) = Неопределено Тогда
			Прервать;
		КонецЕсли;
		СуммаПоСтроке25 = СуммаПоСтроке25 + Области["П001001002103" + "_" + СтрокаИнд].Значение;
	КонецЦикла;
	Области["П001001002503"].Значение = Области["П001001002403"].Значение + СуммаПоСтроке25;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П001001001003"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001001003"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П001001001703"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001001703"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П001001002003"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001002003"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П001001002503"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001002503"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-Фермер");
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		Иначе
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма Форма 1-Фермер");
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть2_1"));
			ПечатнаяФорма.Вывести(ТекущаяОбластьМногострочнойЧасти);
		КонецЕсли;
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабДок.ПолучитьОбласть("ОсновнаяЧасть2_3")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-Фермер");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть2_1"));
	КонецЕсли;
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабДок.ПолучитьОбласть("ОсновнаяЧасть3_3")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-Фермер");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть3_1"));
	КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникОвощиСтрока3", 250, ,
	"Выбор кода культура", КоллекцияСписковВыбора["ОвощиСтрока3"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникОвощиСтрока7", 250, ,
	"Выбор кода культура", КоллекцияСписковВыбора["ОвощиСтрока7"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникОвощиСтрока9", 250, ,
	"Выбор кода культура", КоллекцияСписковВыбора["ОвощиСтрока9"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникОвощиСтрока14", 250, ,
	"Выбор кода культура", КоллекцияСписковВыбора["ОвощиСтрока14"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникОвощиСтрока17", 250, ,
	"Выбор кода культура", КоллекцияСписковВыбора["ОвощиСтрока17"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникОвощиСтрока21", 250, ,
	"Выбор кода культура", КоллекцияСписковВыбора["ОвощиСтрока21"]);
