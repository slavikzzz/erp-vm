...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(ДатаТекущегоСреза) Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнена дата текущего среза. Данное значение заполняется автоматически при выборе даты анализа, ближайшими значением периода расчета ABC и XYZ классификации.';
	...
	Если Не ЗначениеЗаполнено(ДатаПредыдущегоСреза) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнена дата предыдущего среза.';
																|en = 'The previous slice date is required.'"),,"ДатаПредыдущегоСреза");
	КонецЕсли;
