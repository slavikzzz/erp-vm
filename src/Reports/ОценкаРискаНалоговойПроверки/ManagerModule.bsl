...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	РезультатПредставлениеПроверки = НоваяСтруктураПредставленияПроверки();
	РезультатПроверкиКритериев              = РезультатПредставлениеПроверки.РезультатПроверкиКритериев;
	РезультатПроверкиКритериевПечатнаяФорма = РезультатПредставлениеПроверки.РезультатПроверкиКритериевПечатнаяФорма;
	...
	// Отчет может формироваться только по рискам, связанным со значительными вычетами по НДС.
	Если ПараметрыОтчета.ТолькоНДС Тогда
		ЗначительныеВычетыПоНДС(ПараметрыОтчета, РезультатыПроверки, РезультатПредставлениеПроверки, ДанныеУчетнойПолитики);
	Иначе
...
// Возвращает долю вычетов от начислений НДС за необходимое количество кварталов, включая переданный
//
// Параметры:
//  Организация - СправочникСсылка.Организации - организация, по которой необходимо получить долю вычета.
//  КонецПериода - Дата - конец периода, на который необходимо получить долю вычета.
//  КоличествоКварталов - количество кварталов за которые нужно собрать данные по КонецПериода, максимально 4
//
// Возвращаемые параметры:
//  РезультатПроверки - Структура с ключами:
//    * ДоляВычетовНДС - доля вычета от начислений НДС
//    * ВсегоВычетов - общая сумма вычетов
//    * ВсегоНачислений - общая сумма начислений
//
Функция ДоляВычетовОтНачисленийНДС(Организация, КонецПериода, КоличествоКварталов = 4) Экспорт
	
	КоличествоКварталовВГоду = 4;
	КоличествоКварталов = Мин(КоличествоКварталов, КоличествоКварталовВГоду);
	
	ПараметрыПроверки = Новый Структура("Организация, КонецПериода, КоличествоКварталов", 
		Организация, КонецКвартала(КонецПериода), КоличествоКварталов);
	РезультатПроверки = Новый Структура("ДоляВычетовНДС, ВсегоВычетов, ВсегоНачислений", 0,0,0);
	РезультатПредставлениеПроверки = НоваяСтруктураПредставленияПроверки();
	ДанныеУчетнойПолитики = НовыйДанныеУчетнойПолитики();
	ПодготовитьДанныеУчетнойПолитики(ДанныеУчетнойПолитики, ПараметрыПроверки);

	// Обход возможных "битых" деклараций по НДС.
	Попытка 
		ЗначительныеВычетыПоНДС(
			ПараметрыПроверки, РезультатПроверки, РезультатПредставлениеПроверки, ДанныеУчетнойПолитики, Истина);
	Исключение
		ТекстОшибки = НСтр("ru = 'Не удалось получить сведения о вычетах НДС.
			|%1';
			|en = 'Cannot receive records on VAT deductions.
			|%1'");
		ОписаниеОшибки = СтрШаблон(ТекстОшибки, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		
		ЗаписьЖурналаРегистрации(
			НСТр("ru = 'Ошибка получения сведений о вычетах НДС';
				|en = 'An error occurred when receiving records on VAT deductions'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Функция ПодготовитьСведенияПредельныхЗначений(ПараметрыОтчета)
	...
	Если ИспользоватьОКВЭД2 Тогда
		...
		// Получаем данные из загруженного классификатора
		Показатели = ДанныеНагрузкиИРентабельности.ЗначениеПоказателейПоКоду(КодОКВЭД, ПараметрыОтчета.НачалоПериода);
		Если Показатели = Неопределено
			Или Показатели.РентабельностьПродаж = Неопределено Тогда
			Возврат СведенияПредельныхЗначений;
		КонецЕсли;
		
		ПредельноеЗначениеНалоговойНагрузки = Показатели.НалоговаяНагрузка;
		ПредельноеЗначениеРентабельностиПродаж = Показатели.РентабельностьПродаж;
		ПредельноеЗначениеРентабельностиАктивов = Показатели.РентабельностьАктивов;
		
	Иначе
		
		// Получаем данные из общего макета
...
Функция ПолучитьДанныеРегистраУчетнаяПолитика(Организация, КонецПериода)
	...
	Запрос.Текст = УчетнаяПолитикаПереопределяемый.ТекстЗапросаУчетнойПолитикиПоПериодам();
...
Процедура ЗаполнитьДанныеУчетнойПолитики(ДанныеУчетнойПолитики, ТекущаяЗапись, КоличествоЛетДействияУчетнойПолитики)
	...
	Для НомерГода = 0 По КоличествоЛетДействияУчетнойПолитики Цикл
		...
		НоваяЗапись.СистемаНалогообложения 					= УчетнаяПолитика.СистемаНалогообложения(ТекущаяЗапись.Организация, НоваяЗапись.Период);
		НоваяЗапись.ПрименяетсяОсобыйПорядокНалогообложения = Ложь;
		НоваяЗапись.ПлательщикНалогаНаПрибыль 				= УчетнаяПолитика.ПлательщикНалогаНаПрибыль(ТекущаяЗапись.Организация, НоваяЗапись.Период);
		НоваяЗапись.ПлательщикНДС			 				= УчетнаяПолитика.ПлательщикНДС(ТекущаяЗапись.Организация, НоваяЗапись.Период);
		НоваяЗапись.ПлательщикЕНВД			 				= УчетнаяПолитика.ПлательщикЕНВД(ТекущаяЗапись.Организация, НоваяЗапись.Период);
		НоваяЗапись.ПрименяетсяУСН			 				= УчетнаяПолитика.ПрименяетсяУСН(ТекущаяЗапись.Организация, НоваяЗапись.Период);
		НоваяЗапись.ПрименяетсяУСНДоходы 					= УчетнаяПолитика.ПрименяетсяУСНДоходы(ТекущаяЗапись.Организация, НоваяЗапись.Период);
		НоваяЗапись.ПрименяетсяУСНДоходыМинусРасходы		= УчетнаяПолитика.ПрименяетсяУСНДоходыМинусРасходы(ТекущаяЗапись.Организация, НоваяЗапись.Период);
		НоваяЗапись.ПлательщикНДФЛ		 					= Ложь;
		НоваяЗапись.ПоддержкаПБУ18		 					= УчетнаяПолитика.ПоддержкаПБУ18(ТекущаяЗапись.Организация, НоваяЗапись.Период);
	КонецЦикла;
...
Процедура НалоговаяНагрузка(ПараметрыОтчета, ДанныеУчетнойПолитики, СведенияПредельныхЗначений, 
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, Месяц, Счет В (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДФЛ), ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДФЛ_ДоходыСвышеПредельнойВеличины)), , Организация = &Организация, , ) КАК ХозрасчетныйОбороты";
	...
	ИначеЕсли Год(КонецПериода) = 2017 Тогда
	...
	ИначеЕсли Год(КонецПериода) = 2019 Тогда
		ПриложениеПриказа.Вставить("ПриложениеПриказа", "https://its.1c.ru/db/garant#content:74130133:hdoc:3000");
	Иначе // 2020
		ПриложениеПриказа.Вставить("ПриложениеПриказа", "https://its.1c.ru/db/garant#content:400630205:hdoc:100");
	КонецЕсли;
...
Функция НовыйОтчетыНетДанных()
	...
	ОтчетыНетДанных.Колонки.Добавить("НомерКвартала", Новый ОписаниеТипов("Число"));
...
Процедура ПолучитьРегламентированныеОтчетыОбычнымПериодом(ПериодыОтчета, ИсточникОтчета, ПоказателиОтчетов, 
		НаименованиеПоказателя, ДанныеРегламентированногоОтчета, ОтчетыНетДанных, ЭтоПроверкаВнеОтчета = Ложь)
	...
	Если Не ЭтоПроверкаВнеОтчета Тогда
		ЗаполнитьОтчетыНетДанных(ОтчетыНетДанных, РезультатЗапроса, ПериодыОтчета, ИсточникОтчета);
	КонецЕсли;
...
Процедура ЗаполнитьОтчетыНетДанных(ОтчетыНетДанных, РезультатЗапроса, ПериодыОтчета, ИсточникОтчета)
	...
	ГодовойОтчет.Добавить("РегламентированныйОтчетЕФС1ОСС");
	...
	ТекущийГод = Формат(Год(ДатаОкончания), "ЧГ=");
	ПредыдущийГод = Формат(Год(ДатаОкончания) - 1, "ЧГ=");
	ФорматСтроки = "ФП=Ложь";
	
	// Отчет формируется за один или несколько кварталов с начала года.
	// Для вычетов НДС всегда проверяется 4 последних квартала, соответственно
	// для этого случая возможны кварталы из разных годов.
	// Получим количество кварталов и для каждого вычислим порядковый номер.
	КоличествоМесяцевВКвартале = 3;
	ПервыйКвартал = Месяц(КонецКвартала(ДатаНачала))/КоличествоМесяцевВКвартале;
	ПоследнийКвартал = Месяц(КонецКвартала(ДатаОкончания))/КоличествоМесяцевВКвартале;
	
	КоличествоКварталов = 1;
	Если ПервыйКвартал > ПоследнийКвартал Тогда // НДС за 4 последних квартала
		КоличествоКварталов = 4;
	Иначе // несколько кварталов одного года
		КоличествоКварталов = ПоследнийКвартал - ПервыйКвартал + 1;
	КонецЕсли;
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		Для НомерКвартала = 1 По КоличествоКварталов Цикл
			
			// Вычислим номера кварталов для каждого года
			ПорядковыйНомерКвартала = ПорядковыйНомерКварталаВГоду(ПоследнийКвартал, НомерКвартала, КоличествоКварталов);
			Год = ?(ПорядковыйНомерКвартала > ПоследнийКвартал, ПредыдущийГод, ТекущийГод);
			
			ДатаОкончания = КонецКвартала(Дата(Год, ПорядковыйНомерКвартала* КоличествоМесяцевВКвартале, 1));
			...
			НоваяСтрокаНетДанных.НетОтчета = Новый Структура("ИсточникОтчета, НачалоПериода, КонецПериода", 
				ИсточникОтчета, ДатаНачала, ДатаОкончания);
			НоваяСтрокаНетДанных.НомерКвартала = ПорядковыйНомерКвартала;
	...
	Иначе
		...
		Если ДатыОтчетов.Количество() <> МассивКварталов.Количество() Тогда
			Пока Выборка.Следующий() Цикл
				НомерКвартала = Месяц(КонецКвартала(Выборка.ДатаОтчета))/КоличествоМесяцевВКвартале;
				Если МассивКварталов.Найти(НомерКвартала) <> Неопределено Тогда
					Индекс = МассивКварталов.Найти(НомерКвартала);
			...
			Для каждого ОтсутствующийКвартал Из МассивКварталов Цикл
				...
				Год = ?(ОтсутствующийКвартал > ПоследнийКвартал, ПредыдущийГод, ТекущийГод);
				
				ДатаОкончания = КонецКвартала(Дата(Год, ОтсутствующийКвартал*КоличествоМесяцевВКвартале, 1));
				ДатаНачала = НачалоКвартала(ДатаОкончания);
				...
				НоваяСтрокаНетДанных.НетОтчета = Новый Структура("ИсточникОтчета, НачалоПериода, КонецПериода", 
					ИсточникОтчета, ДатаНачала, ДатаОкончания);
				НоваяСтрокаНетДанных.НомерКвартала = ОтсутствующийКвартал;
			КонецЦикла;
...
Функция ПолучитьНаименованиеОтчета(ИсточникОтчета)
	...
	ИначеЕсли ИсточникОтчета = "РегламентированныйОтчетЕдиныйНалогНаВмененныйДоход" Тогда
		НаименованиеОтчета = НСтр("ru = 'декларацию по ЕНВД за';
									|en = 'UTII declaration for'") + " ";
	ИначеЕсли ИсточникОтчета = "РегламентированныйОтчет3НДФЛ" Тогда
		НаименованиеОтчета = НСтр("ru = 'декларацию 3-НДФЛ за';
									|en = '3-NDFL declaration for'") + " ";
	ИначеЕсли ИсточникОтчета = "РегламентированныйОтчет4ФСС" Тогда
		НаименованиеОтчета = НСтр("ru = 'расчет 4-ФСС за';
									|en = 'calculation of 4-SSF for'") + " ";
	ИначеЕсли ИсточникОтчета = "РегламентированныйОтчетЕФС1ОСС" Тогда
		НаименованиеОтчета = НСтр("ru = 'расчет ЕФС-1 за';
									|en = 'calculation of Unified Information Form-1 for'") + " ";
	КонецЕсли;
...
Функция ПолучитьМассивКварталов(ПериодыОтчета)
	...
	// Получим количество кварталов и для каждого вычислим порядковый номер.
	КоличествоМесяцевВКвартале = 3;
	ПервыйКвартал = Месяц(КонецКвартала(ПериодыОтчета.НачалоПериода))/КоличествоМесяцевВКвартале;
	ПоследнийКвартал = Месяц(КонецКвартала(ПериодыОтчета.КонецПериода))/КоличествоМесяцевВКвартале;
	...
	Для НомерКвартала = 1 По КоличествоКварталов Цикл
		ПорядковыйНомерКвартала = ПорядковыйНомерКварталаВГоду(ПоследнийКвартал, НомерКвартала, КоличествоКварталов);
		МассивКварталов.Добавить(ПорядковыйНомерКвартала);
	КонецЦикла;
...
Функция ПорядковыйНомерКварталаВГоду(ПоследнийКвартал, НомерКвартала, КоличествоКварталов = 4)
	
	КоличествоКварталовВГоду = 4;
	Смещение = КоличествоКварталовВГоду - КоличествоКварталов;
	
	ПорядковыйНомерКвартала = (ПоследнийКвартал + НомерКвартала + Смещение) % КоличествоКварталовВГоду;
	
	Возврат ?(ПорядковыйНомерКвартала > 0, ПорядковыйНомерКвартала, КоличествоКварталовВГоду) ;
...
Функция НовыйИсточникиДополнительныхОтчетов(ЭтоОСН, ЭтоЕНВД, ПлательщикНДС)
	...
	Если ЭтоОСН Тогда
		...
		ИсточникиОтчетов.Добавить("РегламентированныйОтчетАкцизыНефтяноеСырье");
...
Функция ПолучитьНаименованияПоказателейНалоговаяНагрузка(ИсточникиОтчетов, КонецПериода)
	...
	Для каждого Источник Из ИсточникиОтчетов Цикл
		...
		ИначеЕсли Источник = "РегламентированныйОтчетАкцизыНефтяноеСырье" Тогда
			НаименованияПоказателей.Добавить("СуммаАкцизовНаНефтяноеСырье");
...
Функция ПолучитьНомерПериода(ИсточникОтчета)
	...
	ИначеЕсли ИсточникОтчета = "РегламентированныйОтчетАкцизыГСМ" Тогда
		НомерПериода = 12;
	ИначеЕсли ИсточникОтчета = "РегламентированныйОтчетАкцизыНефтяноеСырье" Тогда
		НомерПериода = 12;
...
Процедура ЗаполнитьРасшифровкуНалоговаяНагрузкаПоПериодуКварталМесяц(СтрокиИсточника, НаименованиеПериода, 
		НаименованиеРасшифровки, НоваяСтрокаРасшифровки, ПараметрыРасшифровки, ЭтоКвартал = Ложь, ПериодыВРазнахГодах = Ложь)
	...
	Для НомерЭлемента = 0 По КоличествоЭлементов Цикл
		...
		Если НомерЭлемента = КоличествоЭлементов Тогда
			...
			Иначе
				...
				ПараметрыРасшифровки.Вставить(НаименованиеРасшифровки + ТекущийПериод, Неопределено);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
		...
		Если ТекущаяСтрокаУчетнойПолитики.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Упрощенная Тогда
			...
			Иначе
				...
				Если РасчетНалогаУСН <> Неопределено Тогда
					...
					ИсточникДанных.ЗначениеПоказателя     = ?(НалоговаяБаза = 0, НСтр("ru = 'Нет данных';
																						|en = 'No data'"), НалоговаяБаза);
...
Процедура СоздатьВТПрочихДоходовИРасходов(МенеджерВременныхТаблиц)
	...
	Если Метаданные.Справочники.Найти("ПрочиеДоходыИРасходы") <> Неопределено Тогда
		// БП, БРУ.
		ИмяИсточникаПрочиеДоходы 	= "Справочник.ПрочиеДоходыИРасходы";
		ИмяИсточникаПрочиеРасходы 	= "Справочник.ПрочиеДоходыИРасходы";
		ИмяРеквизитаВидПрочихДоходов= "ПрочиеДоходыИРасходы.ВидПрочихДоходовИРасходов";
		ИмяРеквизитаВидПрочихРасходов= "ПрочиеДоходыИРасходы.ВидПрочихДоходовИРасходов";
		
	ИначеЕсли Метаданные.ПланыВидовХарактеристик.Найти("СтатьиДоходов") <> Неопределено Тогда
		// УП.
		ИмяИсточникаПрочиеДоходы 	= "ПланВидовХарактеристик.СтатьиДоходов";
		ИмяИсточникаПрочиеРасходы 	= "ПланВидовХарактеристик.СтатьиРасходов";
		ИмяРеквизитаВидПрочихДоходов= "ПрочиеДоходыИРасходы.ВидПрочихДоходовИРасходов";
		ИмяРеквизитаВидПрочихРасходов= "ПрочиеДоходыИРасходы.ВидПрочихДоходовИРасходов";
	КонецЕсли;
	...
	|ИЗ
	|	&ИмяИсточникаПрочиеДоходы КАК ПрочиеДоходыИРасходы
	|ГДЕ
	|	&ИмяРеквизитаВидПрочихДоходов В (&ЗначениеРеквизита_2310_2320_2340)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|ИЗ
	|	&ИмяИсточникаПрочиеРасходы КАК ПрочиеДоходыИРасходы
	|ГДЕ
	|	&ИмяРеквизитаВидПрочихРасходов В (&ЗначениеРеквизитаКВычету_2310_2320_2340)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|	&ИмяИсточникаПрочиеДоходы КАК ПрочиеДоходыИРасходы
	|ГДЕ
	|	&ИмяРеквизитаВидПрочихДоходов В (&ЗначениеРеквизита_2330_2350)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|	&ИмяИсточникаПрочиеРасходы КАК ПрочиеДоходыИРасходы
	|ГДЕ
	|	&ИмяРеквизитаВидПрочихРасходов В (&ЗначениеРеквизитаКВычету_2330_2350)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяИсточникаПрочиеДоходы", ИмяИсточникаПрочиеДоходы);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяИсточникаПрочиеРасходы", ИмяИсточникаПрочиеРасходы);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяРеквизитаВидПрочихДоходов", ИмяРеквизитаВидПрочихДоходов);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяРеквизитаВидПрочихРасходов", ИмяРеквизитаВидПрочихРасходов);
...
Процедура ЗначительныеВычетыПоНДС(ПараметрыОтчета, РезультатыПроверки, РезультатПредставлениеПроверки,
	ДанныеУчетнойПолитики = Неопределено, ЭтоПроверкаВнеОтчета = Ложь)
		
	Если ДанныеУчетнойПолитики = Неопределено Тогда 
		ДанныеУчетнойПолитики = НовыйДанныеУчетнойПолитики();
		ПодготовитьДанныеУчетнойПолитики(ДанныеУчетнойПолитики, ПараметрыОтчета);
	КонецЕсли;
	
	РезультатПроверкиКритериев   = РезультатПредставлениеПроверки.РезультатПроверкиКритериев;
	РезультатПроверкиКритериевПФ = РезультатПредставлениеПроверки.РезультатПроверкиКритериевПечатнаяФорма;
	
	Организация = ПараметрыОтчета.Организация;
	// Показатели по НДС всегда смотрим за 12 месяцев, если не передали другое значение.
	КоличествоКварталов = 4;
	Если ПараметрыОтчета.Свойство("КоличествоКварталов") Тогда 
		КоличествоКварталов  = ПараметрыОтчета.КоличествоКварталов;
	КонецЕсли;
	
	КоличествоМесяцевВКвартале = 3;
	НачалоПериода = НачалоКвартала(ДобавитьМесяц(
		ПараметрыОтчета.КонецПериода, -КоличествоМесяцевВКвартале*(КоличествоКварталов-1)));
	КонецПериода = КонецМесяца(ПараметрыОтчета.КонецПериода);
	ДатаРегистрации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "ДатаРегистрации");
	
	// В расшифровку "Подробнее" запишем ссылку на пункт приказа о безопасной доле вычета.
	Подробнее = Новый Структура("СсылкаНаКритерий", "http://its.1c.ru/db/garant#content:12053820:1:2003");
	
	// В расшифровку доли вычета запишем либо ссылку на пункт параметров учета,
	// либо ссылку на пункт приказа о безопасной доле вычета.
	МожноКонтролироватьДолюВычетаНДС =  Ложь;
	
	Если МожноКонтролироватьДолюВычетаНДС Тогда
		РасшифровкаДоли = Новый Структура("ИзменитьБезопаснуюДолюВычета", Истина);
	Иначе
		РасшифровкаДоли = Подробнее;
	КонецЕсли;
	
	НаименованиеВременнойТаблицы = "";
	
	МинимальноеЗначениеДолиВычета = 0;
	...
	Если ДатаРегистрации > КонецПериода Тогда
		Если НЕ ЭтоПроверкаВнеОтчета Тогда 
			...
			ДанныеЗаполненияМакета);
		КонецЕсли;
		// Считаем, что доля вычетов 0%
		РезультатыПроверки.Вставить("ДоляВычетовНДС", МинимальноеЗначениеДолиВычета);
		Возврат;
	КонецЕсли;
	...
	НачалоПериода = БлижайшийНалоговыйПериод.Период;
	...
	ТолькоДекларацияПоНДС = Ложь;
	
	ПараметрыОтчета.Свойство("ТолькоНДС", ТолькоДекларацияПоНДС);
	
	ТолькоДекларацияПоНДС = ?(ТолькоДекларацияПоНДС = Неопределено, Ложь, ТолькоДекларацияПоНДС);
	
	Если ТолькоДекларацияПоНДС ИЛИ ЭтоПроверкаВнеОтчета Тогда
		ПоказателиОтчетов = РегламентированнаяОтчетность.ПоказателиОценкиРискаВыезднойНалоговойПроверки(
			Организация, НачалоПериода, КонецПериода, ИсточникОтчета);
	Иначе
		ПоказателиОтчетов = РегламентированнаяОтчетность.ПоказателиОценкиРискаВыезднойНалоговойПроверки(
	...
	ПолучитьРегламентированныеОтчетыОбычнымПериодом(ПериодыОтчета, ИсточникОтчета, ПоказателиОтчетов, 
		НаименованиеПоказателя, ДанныеРегламентированногоОтчетаНДС, ОтчетыНетДанных, ЭтоПроверкаВнеОтчета);
	...
	МинимальноеЗначениеДолиВычета = 0;
	ДоляВычетовБезНачислений = 100;
	Если НЕ НДСНачислен Тогда
		Если СуммаВычетовНДС = 0 Тогда
			// Значит за период отсутствуют вычеты и начисления, считаем такую ситуацию нормальной.
			ДоляВычетовНДС = МинимальноеЗначениеДолиВычета;
		Иначе
			// Такой случай скорее исключение, считаем такую ситуацию превышением, обозначим долю вычета как 100%.
			ДоляВычетовНДС = ДоляВычетовБезНачислений;
		КонецЕсли;
	Иначе
		ДоляВычетовНДС = Окр(100*(СуммаВычетовНДС/СуммаНачисленногоНДС), 2, 1);
	КонецЕсли;
	
	РезультатыПроверки.Вставить("ДоляВычетовНДС",  ДоляВычетовНДС);
	РезультатыПроверки.Вставить("ВсегоВычетов",    СуммаВычетовНДС);
	РезультатыПроверки.Вставить("ВсегоНачислений", СуммаНачисленногоНДС);
	...
	Если Не ЭтоПроверкаВнеОтчета Тогда
		...
		ДанныеЗаполненияМакетаРасчетПоказателей.Вставить("Подробнее_ДоляВычета", РасшифровкаДоли);
		
		ЗаполнитьМакетОтчетаЕстьРезультат(РезультатПроверкиКритериев, РезультатПроверкиКритериевПФ, ДанныеЗаполненияМакета, 
		ДанныеЗаполненияМакетаРасчетПоказателей);
		ЗаполнитьМакетОтчетаРасшифровкойЗначительныеВычетыПоНДС(РезультатПроверкиКритериев, РезультатПроверкиКритериевПФ, 
		ДанныеРасшифровкиКритерия, КонецПериода, ОтчетыНетДанных, ПараметрыОтчета.ТолькоНДС);
	КонецЕсли;
...
Процедура ЗаполнитьМакетОтчетаРасшифровкойЗначительныеВычетыПоНДС(РезультатПроверкиКритериев, РезультатПроверкиКритериевПФ, 
		ДанныеРасшифровкиКритерия, КонецПериода, ОтчетыНетДанных, ТолькоНДС = Ложь)
	...
	ОбластьОтступ_3 = МакетОтчета.ПолучитьОбласть("Отступ_3");
	ЦветТекстаГиперссылки = ЦветаСтиля.ОценкаРискаНалоговойПроверкиТекстГиперссылкиЦвет;
	ПоследнийКвартал = Месяц(КонецПериода)/КоличествоМесяцевВКвартале;
	ТекущийГод = Формат(Год(КонецПериода), "ЧГ=");
	ПредыдущийГод = Формат(Год(КонецПериода) - 1, "ЧГ=");
	
	СтрокаВычеты     = ДанныеРасшифровкиКритерия.Найти("СуммаВычетовПоНДС", "Показатель");
	СтрокаНачисления = ДанныеРасшифровкиКритерия.Найти("СуммаНачисленногоНДС", "Показатель");
	
	ОбластьРасшифровкиНалогаВычеты = МакетОтчета.ПолучитьОбласть("ПериодКвартал_3");
	ОбластьРасшифровкиНалогаНачисления = МакетОтчета.ПолучитьОбласть("ПериодКвартал_3");
	ОбластьРасшифровкиНалогаПФВычеты = МакетОтчетаПФ.ПолучитьОбласть("ПериодКвартал_3");
	ОбластьРасшифровкиНалогаПФНачисления = МакетОтчетаПФ.ПолучитьОбласть("ПериодКвартал_3");
	
	ПустоеЗначениеВычеты = Новый Структура;
	ПустоеЗначениеНачисления = Новый Структура;
	
	// Для проверки значительных вычетов по НДС всегда берутся 4 квартала, включая последний.
	// Выводим по порядку от раннего к позднему.
	Для НомерКвартала = 1 По КоличествоКварталовВГоду Цикл
		
		ПорядковыйНомерКвартала = ПорядковыйНомерКварталаВГоду(ПоследнийКвартал, НомерКвартала);
		Год = ?(ПорядковыйНомерКвартала > ПоследнийКвартал, ПредыдущийГод, ТекущийГод);
		
		// Макет описания показателей
		ПараметрКвартал = Строка(НомерКвартала) + "_Квартал";
		ПараметрГод = Строка(НомерКвартала) + "_Год";
		
		// Макет значения показателей
		ПараметрМакета = "Квартал" + НомерКвартала;
		ПараметрМакетаРасшифровка = "РасшифровкаКвартал" + НомерКвартала;
		
		// Параметры значения показателей
		ПоказательКвартал = "Квартал_" + ПорядковыйНомерКвартала;
		РасшифровкаКвартал = "РасшифровкаКвартал_" + ПорядковыйНомерКвартала;
		
		// Описание показателей
		ОбластьРасшифровкиНалогаВычеты.Параметры[ПараметрКвартал] = ПорядковыйНомерКвартала;
		ОбластьРасшифровкиНалогаВычеты.Параметры[ПараметрГод]     = Год;
		ОбластьРасшифровкиНалогаНачисления.Параметры[ПараметрКвартал] = ПорядковыйНомерКвартала;
		ОбластьРасшифровкиНалогаНачисления.Параметры[ПараметрГод]     = Год;
		//Печатная форма
		ОбластьРасшифровкиНалогаПФВычеты.Параметры[ПараметрКвартал] = ПорядковыйНомерКвартала;
		ОбластьРасшифровкиНалогаПФВычеты.Параметры[ПараметрГод]     = Год;
		ОбластьРасшифровкиНалогаПФНачисления.Параметры[ПараметрКвартал] = ПорядковыйНомерКвартала;
		ОбластьРасшифровкиНалогаПФНачисления.Параметры[ПараметрГод]     = Год;
		
		СтрокаНетДанныхЗаКвартал = ОтчетыНетДанных.Найти(ПорядковыйНомерКвартала, "НомерКвартала");
		Если СтрокаНетДанныхЗаКвартал <> Неопределено Тогда
			
			// Вычеты
			ОбластьРасшифровкиНалогаВычеты.Параметры[ПараметрМакета] =  НСтр("ru = 'Нет данных';
																			|en = 'No data'");
			ОбластьРасшифровкиНалогаВычеты.Параметры[ПараметрМакетаРасшифровка] = СтрокаНетДанныхЗаКвартал.НетОтчета;
			ОбластьРасшифровкиЯчейки = ОбластьРасшифровкиНалогаВычеты.Область(ПараметрМакета);
			ОбластьРасшифровкиЯчейки.Гиперссылка = Истина;
			ОбластьРасшифровкиЯчейки.ЦветТекста = ЦветТекстаГиперссылки;
			
			// Начисления
			ОбластьРасшифровкиНалогаНачисления.Параметры[ПараметрМакета] =  НСтр("ru = 'Нет данных';
																				|en = 'No data'");
			ОбластьРасшифровкиНалогаНачисления.Параметры[ПараметрМакетаРасшифровка] = СтрокаНетДанныхЗаКвартал.НетОтчета;
			ОбластьРасшифровкиЯчейки = ОбластьРасшифровкиНалогаНачисления.Область(ПараметрМакета);
			ОбластьРасшифровкиЯчейки.Гиперссылка = Истина;
			ОбластьРасшифровкиЯчейки.ЦветТекста = ЦветТекстаГиперссылки;
			
		Иначе
			// Вычеты
			Если СтрокаВычеты <> Неопределено // деятельность за квартал могла не вестись
				И СтрокаВычеты.ПараметрыРасшифровки.Свойство(ПоказательКвартал) Тогда
				// Значение показателей
				ОбластьРасшифровкиНалогаВычеты.Параметры[ПараметрМакета] = СтрокаВычеты.ПараметрыРасшифровки[ПоказательКвартал];
				ОбластьРасшифровкиНалогаВычеты.Параметры[ПараметрМакетаРасшифровка] = СтрокаВычеты.ПараметрыРасшифровки[РасшифровкаКвартал];
				ОбластьРасшифровкиЯчейки = ОбластьРасшифровкиНалогаВычеты.Область(ПараметрМакета);
				ОбластьРасшифровкиЯчейки.Гиперссылка = Истина;
				ОбластьРасшифровкиЯчейки.ЦветТекста = ЦветТекстаГиперссылки;
				//Печатная форма
				ОбластьРасшифровкиНалогаПФВычеты.Параметры[ПараметрМакета] = СтрокаВычеты.ПараметрыРасшифровки[ПоказательКвартал];
			Иначе
				ПустоеЗначениеВычеты.Вставить(ПараметрМакета, "-");
			КонецЕсли;
			
			// Начисления
			Если СтрокаНачисления <> Неопределено // деятельность за квартал могла не вестись
				И СтрокаНачисления.ПараметрыРасшифровки.Свойство(ПоказательКвартал) Тогда
				// Значение показателей
				ОбластьРасшифровкиНалогаНачисления.Параметры[ПараметрМакета] = СтрокаНачисления.ПараметрыРасшифровки[ПоказательКвартал];
				ОбластьРасшифровкиНалогаНачисления.Параметры[ПараметрМакетаРасшифровка] = СтрокаНачисления.ПараметрыРасшифровки[РасшифровкаКвартал];
				ОбластьРасшифровкиЯчейки = ОбластьРасшифровкиНалогаНачисления.Область(ПараметрМакета);
				ОбластьРасшифровкиЯчейки.Гиперссылка = Истина;
				ОбластьРасшифровкиЯчейки.ЦветТекста = ЦветТекстаГиперссылки;
				//Печатная форма
				ОбластьРасшифровкиНалогаПФНачисления.Параметры[ПараметрМакета] = СтрокаНачисления.ПараметрыРасшифровки[ПоказательКвартал];
			Иначе
				ПустоеЗначениеНачисления.Вставить(ПараметрМакета, "-");
			КонецЕсли;
		КонецЕсли;
	...
	// Вычеты
	ОбластьРасшифровкиНалогаВычеты.Параметры.Заполнить(ПустоеЗначениеВычеты);
	РезультатПроверкиКритериев.Вывести(ОбластьРасшифровкиНалогаВычеты, 2);
	РезультатПроверкиКритериев.Вывести(ОбластьОтступ_3, 2);
	
	ОбластьРасшифровкиНалогаПФВычеты.Параметры.Заполнить(ПустоеЗначениеВычеты);
	РезультатПроверкиКритериевПФ.Вывести(ОбластьРасшифровкиНалогаПФВычеты, 2);
	РезультатПроверкиКритериевПФ.Вывести(ОбластьОтступ_3, 2);
	...
	Если СтрокаНачисления <> Неопределено Тогда 
		ОбластьНачисления.Параметры.СуммаНачисленногоНДС = СтрокаНачисления.ЗначениеПоказателя;
		ОбластьНачисленияПФ.Параметры.СуммаНачисленногоНДС = СтрокаНачисления.ЗначениеПоказателя;
	Иначе
		ОбластьНачисления.Параметры.СуммаНачисленногоНДС   = 0;
		ОбластьНачисленияПФ.Параметры.СуммаНачисленногоНДС = 0;
	КонецЕсли;
	РезультатПроверкиКритериев.Вывести(ОбластьНачисления, 2);
	РезультатПроверкиКритериевПФ.Вывести(ОбластьНачисленияПФ, 2);
	
	ОбластьРасшифровкиНалогаНачисления.Параметры.Заполнить(ПустоеЗначениеНачисления);
	РезультатПроверкиКритериев.Вывести(ОбластьРасшифровкиНалогаНачисления, 2);
	РезультатПроверкиКритериев.Вывести(ОбластьОтступ_3, 2);
	
	ОбластьРасшифровкиНалогаПФНачисления.Параметры.Заполнить(ПустоеЗначениеНачисления);
	РезультатПроверкиКритериевПФ.Вывести(ОбластьРасшифровкиНалогаПФНачисления, 2);
	РезультатПроверкиКритериевПФ.Вывести(ОбластьОтступ_3, 2);
	
	РезультатПроверкиКритериев.ЗакончитьГруппуСтрок();
	
	// В режиме "ТолькоНДС" отчет формируется с развернутой группой.
	Если ТолькоНДС Тогда 
		РезультатПроверкиКритериев.ПоказатьУровеньГруппировокСтрок(1);
	КонецЕсли;
...
Процедура СреднемесячныйУровеньЗарплаты(ПараметрыОтчета, ДанныеУчетнойПолитики, СведенияПредельныхЗначений, 
	...
	ПредельноеЗначениеСреднейЗарплаты = РегистрыСведений.СредняяЗарплатаПоРегионамИОКВЭД.ВеличинаЗарплаты(
		Адрес["КодРегиона"],
		СведенияПредельныхЗначений.КодОКВЭД,
		НачалоПериода);
	...
	ПредельноеЗначениеСреднейЗарплаты = Окр(ПредельноеЗначениеСреднейЗарплаты);
	...
	Если Год(НачалоПериода) <= 2022 Тогда
		ИсточникОтчета = "РегламентированныйОтчет4ФСС";
		ИмяОбластиМакета = "Калькулятор_СреднемесячныйУровеньЗарплаты2022";
		ИмяОбластиСравнениеПоказателей = "РезультатСравненияСреднемесячныйУровеньЗарплаты2022";
	Иначе
		ИсточникОтчета = "РегламентированныйОтчетЕФС1ОСС";
		ИмяОбластиМакета = "Калькулятор_СреднемесячныйУровеньЗарплаты2023";
		ИмяОбластиСравнениеПоказателей = "РезультатСравненияСреднемесячныйУровеньЗарплаты2023";
	КонецЕсли;
	...
	СредняяЗарплата = Окр(СуммаНачисленныхВыплат/КоличествоМесяцев/СреднесписочнаяЧисленность);
	...
	ДанныеЗаполненияМакета.Вставить("РасчетПоказателей", ИмяОбластиМакета);
	...
	ДанныеЗаполненияМакетаРасчетПоказателей.Вставить("СравнениеПоказателей", ИмяОбластиСравнениеПоказателей);
...
Функция ПолучитьАдресВВидеСоответствия(Организация)
	...
	КонтактнаяИнформация = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(МассивОбъектов, , ВидыКИ, ТекущаяДатаСеанса());
...
Процедура ПриближениеКПредельнымЗначениямСпецРежима(ПараметрыОтчета, ДанныеУчетнойПолитики, РезультатПроверкиКритериев,
	...
	Если ГодРегистрации = Год(КонецПериода)
		...
		И УчетУСНПереопределяемый.НалоговыйПериодПропущен(Организация, КонецПериода, ДатаРегистрации) Тогда
	...
	Для НомерСтрокиУчетнойПолитики = 0 По КоличествоЗаписейУчетнойПолитики - 1 Цикл
		...
		Если ТекущаяСтрокаУчетнойПолитики.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Упрощенная Тогда
			...
			Если Год(ТекущаяСтрокаУчетнойПолитики.Период) = ГодРегистрации
				И УчетУСНПереопределяемый.НалоговыйПериодПропущен(Организация, ТекущаяСтрокаУчетнойПолитики.Период, ДатаРегистрации) Тогда
...
Процедура ПолучитьДанныеСреднесписочнойЧисленности(ПериодыОтчета, ИсточникОтчета, ПоказателиОтчетов,
	...
	ГраницаСреднейЧисленностиРаботников = 
		КонтрольПраваПримененияСпецрежимаКлиентСервер.ГраницаЧисленностиРаботниковДляПримененияПовышеннойСтавкиУСН(ПериодыОтчета.КонецПериода);
	
	ПредельныйРазмерЧисленностиРаботников = Окр(0.95 * ГраницаСреднейЧисленностиРаботников, 0);
	Пока ВыборкаРезультата.Следующий() Цикл
		...
		Если ВыборкаРезультата.ЗначениеПоказателя > ПредельныйРазмерЧисленностиРаботников Тогда
			КоличествоПредельныхЗначенийПоСтроке = КоличествоПредельныхЗначенийПоСтроке + 1;
		...
		НоваяСтрокаДанныхЧисленности.ПредельныйРазмер = ПредельныйРазмерЧисленностиРаботников;
		НоваяСтрокаДанныхЧисленности.ГраницаПоказателя = ГраницаСреднейЧисленностиРаботников;
	КонецЦикла;
...
Процедура ПолучитьДанныеДоходыУСН(ПериодыОтчета, Организация, ДанныеПолученныхДоходов, 
	...
	БазовыйЛимитДоходов = КонтрольПраваПримененияСпецрежимаКлиентСервер.БазовыйЛимитДоходовДляПримененияПовышеннойСтавкиУСН(
		ПериодыОтчета.КонецПериода);
	ГраницаПримененияУСНПоДоходам =
		КонтрольПраваПримененияСпецрежимаКлиентСервер.ГраницаДоходовДляПримененияПовышеннойСтавкиУСН(ПериодыОтчета.КонецПериода);
	
	// В 2021 году коэффициент-дефлятор для верхней границы применения УСН не применяется, применяется только для нижней.
	// В отчете за 2021 год не нужно базовый лимит доходов умножать на коэффициент-дефлятор.
	ИспользуетсяДефлятор = ГраницаПримененияУСНПоДоходам <> БазовыйЛимитДоходов;
	...
	Если ИспользуетсяДефлятор Тогда
		РасшифровкаЗначения.Вставить("ЛимитДоходовПредставление",
			СтрШаблон(НСтр("ru = '%1 млн. руб.';
							|en = '%1 million rub.'"), Формат(БазовыйЛимитДоходов, "ЧДЦ=; ЧС=6")));
	Иначе
		РасшифровкаЗначения.Вставить("ЛимитДоходовПредставление",
			СтрШаблон(НСтр("ru = '%1 млн. руб.';
							|en = '%1 million rub.'"), Формат(ГраницаПримененияУСНПоДоходам, "ЧДЦ=; ЧС=6")));
	КонецЕсли;
...
Процедура ПолучитьДанныеОстаточнойСтоимостиОС(Организация, ДанныеОстаточнойСтоимостиОС, ПериодыОтчета,
	...
	Запрос.Текст = УчетОСПереопределяемый.ТекстЗапросаДанныеСтоимостиОС();
	...
	РасшифровкаЗначения.Вставить("ЛимитСтоимостиОСПредставление",
		СтрШаблон(НСтр("ru = '%1 млн. руб.';
						|en = '%1 million rub.'"), Формат(НормативныйЛимитОстаточнойСтоимости, "ЧДЦ=; ЧС=6")));
...
Функция НовыйДанныеСреднеСписочнойЧисленности()
	...
	ДанныеСреднесписочнойЧисленности.Колонки.Добавить("ПредельныйРазмер", ТипЧисло);
	ДанныеСреднесписочнойЧисленности.Колонки.Добавить("ГраницаПоказателя", ТипЧисло);
...
Процедура ЗаполнитьРасшифровкуСреднесписочнойЧисленности(ДанныеСреднесписочнойЧисленности, 
	...
	Для ИндексСтроки = 0 По МаксимальныйИндекс Цикл
		...
		КонецЕсли;
		ПараметрыРасшифровки.Вставить("ПредельныйРазмер", ТекущаяСтрока.ПредельныйРазмер);
		ПараметрыРасшифровки.Вставить("ГраницаПоказателя", ТекущаяСтрока.ГраницаПоказателя);
		ПараметрыРасшифровки.Вставить("ПредельныйРазмерЧисленностиПредставление",
			СтрШаблон(НСтр("ru = '%1 человек';
							|en = '%1 persons'"), ТекущаяСтрока.ПредельныйРазмер));
		ПараметрыРасшифровки.Вставить("ГраницаЧисленностиРаботников", ТекущаяСтрока.ГраницаПоказателя);
	...
	Для Каждого СтрокаРасшифровки Из ДанныеРасшифровкиКритерия Цикл
		...
		ЯчейкаЗаголовокПредельнаяОстаточнаяСтоимостьОС.Примечание.Текст =
			СтрШаблон(НСтр("ru = '95 %1 от максимального значения: %2';
							|en = '95 %1 of the maximum value: %2'"),
			"%",
...
Процедура ЗаполнитьМакетОтчетаРасшифровкамиПриближенийПоСреднесписочнойЧисленности(РезультатПроверкиКритериев, 
	...
	ТекущийПредельныйРазмер = Неопределено;
	Для Каждого СтрокаРасшифровки Из ДанныеРасшифровкиКритерия Цикл
		
		Если ТекущийПредельныйРазмер <> СтрокаРасшифровки.ПараметрыРасшифровки.ПредельныйРазмер Тогда
			ТекущийПредельныйРазмер = СтрокаРасшифровки.ПараметрыРасшифровки.ПредельныйРазмер;
			ОбластьРасшифровкаЯчейки = ОбластьРасчетПоказателей.Область("СредняяЧисленностьРаботников");
			ОбластьРасшифровкаЯчейки.Примечание.Текст = СтрШаблон(
					НСтр("ru = '95 %% от предельного значения - %1 чел.';
						|en = '95 %% of the limit value - %1 persons'"),
					СтрокаРасшифровки.ПараметрыРасшифровки.ГраницаПоказателя);
			ОбластьРасчетПоказателей.Параметры.Заполнить(СтрокаРасшифровки.ПараметрыРасшифровки);
			ОбластьРасчетПоказателейПФ.Параметры.Заполнить(СтрокаРасшифровки.ПараметрыРасшифровки);
			РезультатПроверкиКритериев.Вывести(ОбластьРасчетПоказателей, 2);
			РезультатПроверкиКритериевПФ.Вывести(ОбластьРасчетПоказателейПФ, 2);
		КонецЕсли;
	...
	Для Каждого СтрокаРасшифровки Из ДанныеРасшифровкиКритерия Цикл
		...
		ТекстПримечания =
			НСтр("ru = '95 %1 от максимального значения:
						|%2%3';
						|en = '95 %1 of the maximum value:
						|%2%3'");
		ЯчейкаЗаголовокПредельныйРазмерДоходов.Примечание.Текст = СтрШаблон(ТекстПримечания,
			"%",
			СтрокаРасшифровки.РасшифровкаЗначения.ЛимитДоходовПредставление,
...
Процедура УровеньРентабельности(ПараметрыОтчета, ДанныеУчетнойПолитики, СведенияПредельныхЗначений, 
	...
	ИначеЕсли Год(КонецПериода) = 2019 Тогда
		ПриложениеПриказа.Вставить("ПриложениеПриказа", "https://its.1c.ru/db/garant#content:74130133:hdoc:4000");
	Иначе // 2020
		ПриложениеПриказа.Вставить("ПриложениеПриказа", "https://its.1c.ru/db/garant#content:400630205:hdoc:200");
	КонецЕсли;
...
Процедура ПолучитьДанныеОтчетаОФинансовыхРезультатах(Организация, ПериодыОтчета, ЧистаяПрибыль, ПрибыльПродаж,
	...
	// Счет 99 "Прибыли и убытки" предназначен для обобщения информации о формировании конечного финансового
	...
	// По окончании отчетного года при составлении годовой бухгалтерской отчетности счет 99 "Прибыли и убытки"
	// закрывается. При этом заключительной записью декабря сумма чистой прибыли (убытка) отчетного года
	// списывается со счета 99 "Прибыли и убытки" в кредит (дебет) счета 84 "Нераспределенная прибыль (непокрытый убыток)".
	Запрос.УстановитьПараметр(
		"СчетНераспределеннаяПрибыль",
		БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.НераспределеннаяПрибыль));
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(СУММА(ИтогиГода.СуммаОборот), 0) КАК Сумма
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНачалаПериода, &ДатаКонецПериода, , Счет В (&СчетПрибылиУбытки), , Организация = &Организация, КорСчет В (&СчетНераспределеннаяПрибыль), ) КАК ИтогиГода
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	-ЕСТЬNULL(СУММА(ВнутриГода.СуммаОборот), 0)
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНачалаПериода, &ДатаКонецПериода, , Счет В (&СчетПрибылиУбытки), , Организация = &Организация, , ) КАК ВнутриГода";
	...
	СебестоимостьПроданныхТоваров = Результат[2].Выгрузить().Итог("Сумма");
	
	ЧистаяПрибыль = Результат[3].Выгрузить().Итог("Сумма");
...
Функция ПолучитьСтоимостьАктивов(Организация, ПериодыОтчета)
	...
	СКД1_1.Добавить(ПланыСчетов.Хозрасчетный.ОсновноеПроизводство);
		...
		Если ДанныеЗаполненияМакета.Свойство("Калькулятор_ТемпыРасходовДополнительная") Тогда
			НаименованиеКритерия = 
				...
				+ Символы.ПС + НСтр("ru = 'есть несоответствие темпов роста расходов над доходами в налоговой';
									|en = 'there is a mismatch of growth rate of expenses over income in tax'") + " "
				+ Символы.ПС + НСтр("ru = 'и бухгалтерской отчетности';
									|en = 'and accounting reporting'");
		Иначе
			НаименованиеКритерия = 
				МакетОписаниеРезультатПроверки.Область(ОбластьТекстНаименованиеКритерия + "|ЕстьОснования").Текст;
		КонецЕсли;
...
Процедура ЗаполнитьМакетОтчетаЗаголовокОтчета(ПараметрыОтчета, РезультатВыполнения, РезультатПроверкиКритериев, 
	...
	ШаблонНетОснованийДляПроверки  = НСтр("ru = 'Нет оснований для налоговой проверки';
											|en = 'No reasons for tax audit'");
	ШаблонЕстьОснованияДляПроверки = НСтр("ru = 'Есть основания для налоговой проверки';
											|en = 'There are reasons for tax audit'");
	ШаблонКоличествоКритериев      = НСтр("ru = ' %1 из 12 критериев';
											|en = ' %1 of 12 criteria'");
	
	Если КоличествоВыполненныхКритериев = 0 Тогда
		Если КоличествоНеВыполненныхКритериев > 0 Тогда
			СообщениеКоличествоКритериев = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонКоличествоКритериев, КоличествоНевыполненныхКритериев);
			СообщениеНетОснованийДляПроверки = ШаблонНетОснованийДляПроверки + 
				?(ПараметрыОтчета.ТолькоНДС, "", СообщениеКоличествоКритериев);
			ДанныеЗаполненияМакета.Вставить("СообщениеНетОснованийДляПроверки", СообщениеНетОснованийДляПроверки);
	...
	Иначе
		Результат.Вывести(ОбластьОтступа_3);
		СообщениеКоличествоКритериев = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонКоличествоКритериев, КоличествоВыполненныхКритериев);
		СообщениеЕстьОснованияДляПроверки = ШаблонЕстьОснованияДляПроверки + 
			?(ПараметрыОтчета.ТолькоНДС, "", СообщениеКоличествоКритериев);
		ДанныеЗаполненияМакета.Вставить("СообщениеЕстьОснованияДляПроверки", СообщениеЕстьОснованияДляПроверки);
...
Функция ДанныеРасчетаУСН(Параметры)
	...
	КонтекстРасчета = УчетУснПереопределяемый.НовыйКонтекстРасчетаНалога(Параметры.Организация, Параметры.КонецПериода);
	...
	ДанныеРасчета = УчетУснПереопределяемый.ПодготовитьРасчетНалога(КонтекстРасчета);
...
Функция ОписаниеПричиныОтчетностьНеСдается(ПредставлениеНалога, Организация, КонецПериода, НалоговыйПериод)
	...
	Если НалоговыйПериод = Перечисления.Периодичность.Год Тогда
		ШаблонСообщения = НСтр("ru = 'Декларация по %5 за %1 год не сдается.
			...
			|en = 'Declaration on %5 for %1 is not submitted. 
			|Period from registration date %2 to %3
			| is included in declarations for %4. 
			|Criterion will be calculated next year.'");
	...
	ИначеЕсли НалоговыйПериод = Перечисления.Периодичность.Квартал Тогда
		ШаблонСообщения = НСтр("ru = 'Декларация по %7 за %1 квартал %2 года не сдается.
			...
			|en = 'Declaration on %7 for %1 quarter of %2 is not submitted. 
			|Period from registration date %3 to %4
			| is included in declarations for %5 quarter of %6. 
			| Criterion will be calculated in the next quarter.'");
...
Функция НоваяСтруктураПредставленияПроверки()
	
	СтруктураПредставленияПроверки = Новый Структура;
	СтруктураПредставленияПроверки.Вставить("РезультатПроверкиКритериев", Новый ТабличныйДокумент);
	СтруктураПредставленияПроверки.Вставить("РезультатПроверкиКритериевПечатнаяФорма", Новый ТабличныйДокумент);
	
	Возврат СтруктураПредставленияПроверки;
