...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2021_1(Данные, УникальныйИдентификатор)
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(Титульная.ФИО_РУКОВОДИТЕЛЯ_ПРЕДСТАВИТЕЛЯ) И Титульная.ПрПодп = "2" Тогда 
			УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаПодписантаНалоговойОтчетности(Данные, ТаблицаОшибок, "Форма2021_1_Титульная", Истина);
		КонецЕсли;
		УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаКодаНО(Титульная.КодНО, ТаблицаОшибок, "Форма2021_1_Титульная");
		УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаИНН(Титульная, ТаблицаОшибок, "Форма2021_1_Титульная", "ИННШапка", Ложь, Ложь);
		...
		Для Каждого Стр Из Данные.ДанныеМногостраничныхРазделов.Форма2021_1_ЛистА Цикл 
			...
			Если УведомлениеОСпецрежимахНалогообложения.АдресНаМногостраничнойСтраницеЗаполнен(СтрЗначение) Тогда 
				...
				КонецЕсли;
			Иначе
				Если Не СтрПодобнаПоРегулярномуВыражению(СтрЗначение.КодНОУчет, "[0-9]{4}") Тогда
					ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
						"не указан / неправильно указан код налогового органа", "Форма2021_1_ЛистА", "КодНОУчет", СтрЗначение.УИД));
				КонецЕсли;
			КонецЕсли;
			
			Если Не СтрПодобнаПоРегулярномуВыражению(СтрЗначение.КодРегион, "[0-9]{2}") Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
		...
		Для Каждого Стр Из Данные.ДанныеМногостраничныхРазделов.Форма2021_1_ЛистБ Цикл 
			...
			КонецЕсли;
			Если ЗначениеЗаполнено(СтрЗначение.КодНОУчет) И Не СтрПодобнаПоРегулярномуВыражению(СтрЗначение.КодНОУчет, "[0-9]{4}") Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Неправильно указан налоговый орган", "Форма2021_1_ЛистБ", "КодНОУчет", СтрЗначение.УИД));
			КонецЕсли;
