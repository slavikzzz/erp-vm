...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	УдалитьЦеновыеПоказателиИзВыбранныхПолейРасшифровки(КомпоновщикНастроек, НастройкиОтчета);
...
Процедура ВключитьОтображениеГруппыПолейРесурсовДопРасходов(КоллекцияЭлементовСтруктурыНастроекКД)
	...
	Для Каждого ЭлементСтруктурыНастроекКД Из КоллекцияЭлементовСтруктурыНастроекКД Цикл
		Если ЭлементСтруктурыНастроекКД.ПредставлениеПользовательскойНастройки = НСтр("ru = 'Детализация до исходных затрат';
																						|en = 'Itemize up to initial costs'") Тогда
	...
	Для Каждого Группировка Из Группировки Цикл
		...
		Для Каждого ПолеГруппировки Из ПоляГруппировки Цикл 
			...
			Если КоличествоПолейАналитикиОборотов > 1 Тогда
				ВызватьИсключение НСтр("ru = 'Множественное использование полей аналитики оборотов в группировках не поддерживается.';
										|en = 'Multiple usage of turnover dimension fields is not supported in groupings.'") ;
			КонецЕсли;
		КонецЦикла;
		Если КоличествоПолейАналитикиОборотов >= 1 И ПоляГруппировки.Количество() > 1 Тогда
			ВызватьИсключение НСтр("ru = 'Совместное использование в группировках полей аналитики оборотов и других полей не поддерживается.';
									|en = 'You cannot use both turnover dimension field and other fields together in groupings.'") ;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ВывестиПредупреждениеОбОтбореПоРесурсам(ДокументРезультат, СписокПолей)
	...
	ТекстПредупреждения = НСтр("ru = 'В отчете ""Себестоимость товаров"" не поддерживаются общие отборы по количественным и стоимостным показателям для всего отчета (настройки отчета - закладка ""Фильтры"").
								...
								|en = 'The Inventory cost report does not support common filters by quantity and cost for the entire report (the Filters tab in the report settings).
								|Move such filters to grouping filters. Go to the report settings, open the Structure tab, and click Advanced at the bottom of the settings window. Open a grouping and set a filter on the Filters tab.
								|Add the filters in all the groupings marked for display in the report. These groupings are selected on the Structure tab.
								|The Itemize up to initial costs grouping does not support filters by quantity.
								|
								|Disable filters by the following fields or move them to groupings:'");
...
Процедура ВывестиПредупреждениеОбИспользованииДетализацииИНеполногоПериода(ДокументРезультат)
	...
	ТекстПредупреждения = НСтр("ru = 'В отчете ""Себестоимость товаров"" не поддерживается группировка по детализации до исходных затрат (поля из группы ""Аналитика доп. расходов""), 
								...
								|en = 'The ""Inventory cost"" report does not support grouping by itemization up to initial costs (fields from the ""Additional expenses dimension"" group) 
								|when the report is generated for a part of the month(s). 
								|To display grouping by itemization up to initial costs, select a period that includes the whole month(s).'");
...
Процедура УдалитьЦеновыеПоказателиИзВыбранныхПолейРасшифровки(КомпоновщикНастроек, НастройкиОтчета)
	Если КомпоновщикНастроек.ФиксированныеНастройки.ДополнительныеСвойства.Свойство("РежимРасшифровки")
		И КомпоновщикНастроек.ФиксированныеНастройки.ДополнительныеСвойства.РежимРасшифровки Тогда
		Показатели = Новый Массив;
		...
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(НастройкиОтчета, Показатели);
	КонецЕсли;
КонецПроцедуры
