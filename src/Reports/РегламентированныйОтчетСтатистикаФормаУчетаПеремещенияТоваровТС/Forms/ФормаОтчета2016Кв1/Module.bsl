...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "20/01/2016";
	// Код отчета, равен коду по ОКУД
	СтруктураРеквизитовФормы.мКодОтчета   = "1000000";
		
	ИмяРаздела = "ФормаОтчета";
	
    ТабличныйДокумент.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2016Кв1_ФормаОтчета"));
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНайти(Область.Имя, "Декларация_") > 0 Тогда
		...
		ВводРеквизитов = ПолучитьФорму("Отчет.РегламентированныйОтчетСтатистикаФормаУчетаПеремещенияТоваровТС.Форма.ФормаВводаГрафа20", ПараметрыФормы, ЭтаФорма);
	...
	ИначеЕсли Область.Имя = "ПолнСокрНаим" Или Область.Имя = "ИннКппОгрн" Тогда
		...
		ВводРеквизитов = ПолучитьФорму("Отчет.РегламентированныйОтчетСтатистикаФормаУчетаПеремещенияТоваровТС.Форма.ФормаВводаГрафаНазвРекв", ПараметрыФормы, ЭтаФорма);
...
Процедура ЗаполнитьПоказателиИзРеквизитовФормы(Форма)
	...
	ОтвЛицоФинУрКоды = ОтвЛицоФинУрКоды + ?(ПустаяСтрока(Форма.СтруктураРеквизитовФормы.ОтвЛицоФинУрРФ_ОГРН), "", " / " + Форма.СтруктураРеквизитовФормы.ОтвЛицоФинУрРФ_ОГРН);
...
Функция ПолучитьСведенияОКонтрагенте(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	Попытка
		...
		Сведения.Вставить("АдрЮР", УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(Организация, Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента));
	Исключение
	КонецПопытки;
	Возврат Сведения;
КонецФункции
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СхемаВыгрузкиXML2016Кв1");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Пока Истина Цикл 
		...
		Если Не ЗначениеЗаполнено(ТабличныйДокумент.Области["ВесТовара"+Суффикс].Значение) Тогда 
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не указана вес товара!", ИмяПоляТаблДокумента, "ВесТовара"+Суффикс);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТабличныйДокумент.Области["ЦенаТовараВалРубль"+Суффикс].Значение)
			...
			Если Курс < 50 Или Курс > 120 Тогда 
		...
		Если (ЗначениеЗаполнено(ТабличныйДокумент.Области["ДатаРегистрацииСтр"+Суффикс].Значение) 
			...
			Или Не ЗначениеЗаполнено(ТабличныйДокумент.Области["НомерДок"+Суффикс].Значение)) Тогда 
