...
Процедура ЗаполнитьСписокВыбораЗаявленийОВвозеТоваров()
	...
	Если ИмяДокументаЗаявлениеОВвозеТоваров = Неопределено Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Заявление о ввозе товаров не поддерживается в этой конфигурации, выгрузка и отправка статистической формы учета перемещения товаров по заявлению невозможны';
													|en = 'Заявление о ввозе товаров не поддерживается в этой конфигурации, выгрузка и отправка статистической формы учета перемещения товаров по заявлению невозможны'"));
		Возврат;
	КонецЕсли;
	...
	Элементы.ЗаявлениеОВвозеТоваров.СписокВыбора.Вставить(0, Документы[ИмяДокументаЗаявлениеОВвозеТоваров].ПустаяСсылка(), НСтр("ru = '<< Заявление о ввозе товаров не выбрано >>';
																																|en = '<< Заявление о ввозе товаров не выбрано >>'"));
...
Процедура СохранитьНаКлиентеСВопросом(Автосохранение, Вариант, ВыполняемоеОповещение, ОткрытьФормуВопросаПриИзмененииВидаДокументаНомераКорректировки, ПодобныйОтчетСуществует, ПоказателиОтчета)
	...
	Если ПодобныйОтчетСуществует Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отчет с видом %1 уже существует.
			...
			|Сохранить отчет с таким же видом?'"), ?(Вариант = 0, НСтр("ru = '""Первичный""';
																		|en = '""Первичный""'"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '""Корректирующий/%1""';
																																							|en = '""Корректирующий/%1""'"), Вариант)));
...
Процедура ПроверитьВыгрузкуОтчета()
	...
	Если НЕ ЗначениеЗаполнено(СтруктураРеквизитовФормы.Организация) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю("Организация не выбрана! Выгрузка невозможна.");
		Возврат;
	КонецЕсли;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ЗаявлениеОВвозеТоваров) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не выбран документ в поле ""Заявление о ввозе товаров""! Выгрузка невозможна.';
													|en = 'Не выбран документ в поле ""Заявление о ввозе товаров""! Выгрузка невозможна.'"));
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура ПечатьПослеСохранения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ИмяДокументаЗаявлениеОВвозеТоваров = Неопределено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Заявление о ввозе товаров не поддерживается в этой конфигурации, выгрузка и отправка статистической формы учета перемещения товаров по заявлению невозможны!';
														|en = 'Заявление о ввозе товаров не поддерживается в этой конфигурации, выгрузка и отправка статистической формы учета перемещения товаров по заявлению невозможны!'"));
		Возврат;
	КонецЕсли;
