...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("Получатель_ОПИндекс", Получатель_ОПИндекс);
	Параметры.Свойство("Получатель_ОПНазвание", Получатель_ОПНазвание);
	Параметры.Свойство("Получатель_ОПНаселенныйПункт", Получатель_ОПНаселенныйПункт);
	Параметры.Свойство("Получатель_ОПНСА", Получатель_ОПНСА);
	Параметры.Свойство("Получатель_ОПОбласть", Получатель_ОПОбласть);
	Параметры.Свойство("Получатель_ОПСтранаКод", Получатель_ОПСтранаКод);
	Параметры.Свойство("Получатель_ОПСтранаНаименование", Получатель_ОПСтранаНаименование);
	Параметры.Свойство("Получатель_ОПУлицаНомерДома", Получатель_ОПУлицаНомерДома);
	...
	Иначе
		...
		Элементы.СведенияОДокументе.Видимость = Ложь;
	КонецЕсли;
	...
	Если Параметры.ПолучательСтранаКод <> "RU" Тогда 
		Элементы.Получатель_Форма.АвтоОтметкаНезаполненного = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура Получатель_ОПСтранаНаименованиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	...
	ПараметрыФормы.Вставить("НаимКолонкиНазвание",            "Название");
	ПараметрыФормы.Вставить("СтруктураДляПоиска",             Новый Структура("Кратко", Получатель_ОПСтранаНаименование));
	ПараметрыФормы.Вставить("ВключитьВидимостьКолонкиКратко", Ложь);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("Получатель_ОПСтранаНаименованиеНачалоВыбораЗавершение", ЭтотОбъект);

	ОткрытьФорму("Обработка.ОбщиеОбъектыРеглОтчетности.Форма.ФормаВыбораЗначенияИзТаблицы", ПараметрыФормы, ЭтаФорма,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

&НаКлиенте
Процедура ПолучательСтранаНаименованиеПриИзменении(Элемент)
	Если Не ЗначениеЗаполнено(ПолучательСтранаНаименование) Тогда 
		ПолучательСтранаКод = "";
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Получатель_ОПСтранаНаименованиеПриИзменении(Элемент)
	Если Не ЗначениеЗаполнено(Получатель_ОПСтранаНаименование) Тогда 
		Получатель_ОПСтранаКод = "";
	КонецЕсли;
КонецПроцедуры
...
Процедура ВыборДокументаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат <> Неопределено Тогда
		Получатель_ВидДокКод = Результат["Код"];
		Получатель_ВидДокНаим = ВРег(Результат["Кратко"]);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Получатель_ОПСтранаНаименованиеНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат <> Неопределено Тогда
		Получатель_ОПСтранаКод = Результат["Код"];
		Получатель_ОПСтранаНаименование = Результат["Кратко"];
	КонецЕсли;
КонецПроцедуры
...
Процедура ПолучательНаименованиеНачалоВыбораЗавершение(СтруктураВозврата, ДополнительныеПараметры) Экспорт 
	...
	Если ЗначениеЗаполнено(ПолучательОбласть) И Не ЗначениеЗаполнено(ПолучательНаселенныйПункт) Тогда 
		ПолучательНаселенныйПункт = "-";
	КонецЕсли;
