...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(мДатаНачалаПериодаОтчета) И НЕ ЗначениеЗаполнено(мДатаКонцаПериодаОтчета) Тогда
		мДатаКонцаПериодаОтчета  = КонецМесяца(ДобавитьМесяц(КонецМесяца(ТекущаяДатаСеанса()), ?(День(ТекущаяДатаСеанса()) < 10, -1 , 0)));  // не позднее 10 числа месяца, следующего за отчетным месяцем
...
Процедура ОткрытьФормуОтчета(Команда)
	...
	Если мСкопированаФорма <> Неопределено Тогда
		...
		Если мВыбраннаяФорма <> мСкопированаФорма Тогда
			...
			Если мСкопированаФорма = "ФормаОтчетаПоЗаявлениюОВвозеТоваров" Тогда
				ПоказатьПредупреждение(, НСтр("ru = 'Копирование данных из статистической формы учета перемещения товаров, созданной по заявлению о ввозе товаров, не поддерживается!';
												|en = 'Копирование данных из статистической формы учета перемещения товаров, созданной по заявлению о ввозе товаров, не поддерживается!'"));
			Иначе
