...
//++ НЕ УТКА
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ПодстановкаМаксимумГрафикПроизводстваДатаОтгрузкиДатаПоступления",
		"ВЫБОР
		|	КОГДА &ПодстановкаДатаПоступленияПоГрафикуПроизводства < Потребности.ЖелаемаяДатаОтгрузки ТОГДА
		|		Потребности.ЖелаемаяДатаОтгрузки
		|	ИНАЧЕ
		|		&ПодстановкаДатаПоступленияПоГрафикуПроизводства
		|	КОНЕЦ");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ПодстановкаДатаПоступленияПоГрафикуПроизводства",
		"ВЫБОР
		|	КОГДА ИнформацияОДоступности.ДатаПоступления = ДАТАВРЕМЯ(1, 1, 1)
		|		ТОГДА НАЧАЛОПЕРИОДА(ДатыГрафика.Окончание, ДЕНЬ)
		|	ИНАЧЕ ИнформацияОДоступности.ДатаПоступления
		|КОНЕЦ");
//-- НЕ УТКА
