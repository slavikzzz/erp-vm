...
Процедура УстановитьЗначенияПараметровЗапроса(Запрос,СтруктураФормированияОтчета)
	...
	Запрос.УстановитьПараметр("ОжиданиеОплатыДоОбеспечения",НСтр("ru = 'Получить оплату до обеспечения';
																|en = 'Receive payment before supply'"));
	...
	Запрос.УстановитьПараметр("ОжиданиеПредоплатыДоОтгрузки",НСтр("ru = 'Получить предоплату до отгрузки';
																	|en = 'Receive prepayment before shipment'"));
	...
	Запрос.УстановитьПараметр("ОжиданиеОплатыПослеОтгрузки",НСтр("ru = 'Получить оплату после отгрузки';
																|en = 'Receive payment after shipment'"));
	...
	Запрос.УстановитьПараметр("ОжиданиеОплатыПослеОтгрузкиЗаявкаНаВозврат",НСтр("ru = 'Получить оплату после отгрузки по замене';
																				|en = 'Receive payment after shipment for replacement'"));
...
Функция ТекстУсловияПоОбъектамИсключаемымИзТекстаЗапросаПрочиеЗадачиИсполнителя()
	...
	|		И НЕ ЗадачаИсполнителя.Предмет ССЫЛКА Документ.ДвижениеПродукцииИМатериалов
	|";
...
Функция ТекстЗапросаЗаказМатериаловВПроизводство(Запрос,СтруктураФормированияОтчета)
	...
	|	МИНИМУМ(КОНЕЦПЕРИОДА(ЗаказМатериаловВПроизводствоТовары.НачалоОтгрузки, ДЕНЬ)) КАК ДатаСобытия,
	|	ДокументЗаказМатериаловВПроизводство.Представление КАК Описание,
	|	NULL КАК ДопРеквизитОписание1,
	|	NULL КАК ДопРеквизитОписание2,
	|	ВЫБОР
	|		КОГДА НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ) < ЗаказМатериаловВПроизводствоТовары.НачалоОтгрузки
	|			ТОГДА &Предстоящее
	|		КОГДА НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ) = ЗаказМатериаловВПроизводствоТовары.НачалоОтгрузки
	|			ТОГДА &Текущее
	|		ИНАЧЕ &Просроченное
	...
	|	И ЗаказМатериаловВПроизводствоТовары.НачалоОтгрузки < &ДатаОкончания  И &ТекстУсловияОтборДокумент
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ДокументЗаказМатериаловВПроизводство.Дата КАК ДатаОбъектаСобытия,
	|	КОНЕЦПЕРИОДА(ЗаказМатериаловВПроизводствоТовары.НачалоОтгрузки, ДЕНЬ) КАК ДатаСобытия,
	...
	|				И ВЫРАЗИТЬ(Распоряжение КАК Документ.ЗаказМатериаловВПроизводство).Товары.НачалоОтгрузки < &ДатаОкончания
	|				И ВЫРАЗИТЬ(Распоряжение КАК Документ.ЗаказМатериаловВПроизводство).Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовМатериаловВПроизводство.КВыполнению) И &ТекстУсловияОтборРаспоряжение) КАК ЗаказыМатериаловВПроизводствоОстатки
	...
	Запрос.УстановитьПараметр("ПодготовитьКОтгрузке",НСтр("ru = 'Подготовить к отгрузке';
															|en = 'Prepare to ship'"));
	Запрос.УстановитьПараметр("ВыполнитьЗаказМатериаловВПроизводство",НСтр("ru = 'Выполнить заказ';
																			|en = 'Fulfill order'"));
	Запрос.УстановитьПараметр("ЗакрытьЗаказМатериаловВПроизводство",НСтр("ru = 'Закрыть заказ';
																		|en = 'Close order'"));
	...
	Если НЕ ПустаяСтрока(СтруктураФормированияОтчета.ТекстУсловияПоПользователям) Тогда
		...
		ТекстУсловияОтбор ="
		|И ЗаказМатериаловВПроизводство.Ответственный" + СтруктураФормированияОтчета.ТекстУсловияПоПользователям;
		ТекстУсловияОтборРаспоряжение ="
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"И &ТекстУсловияОтборРаспоряжение",ТекстУсловияОтборРаспоряжение);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"И &ТекстУсловияОтбор",ТекстУсловияОтбор);
	ОбработатьТекстПодзапросаПоВидуСобытия(ТекстЗапроса,СтруктураФормированияОтчета.ТекущийНомерПодзапросаПоВидамСобытий);
...
Функция ТекстЗапросаПередачаМатериаловВПроизводство(Запрос,СтруктураФормированияОтчета)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством2_2") Тогда
		...
		ИмяОбъектаМетаданных = "Документ.ДвижениеПродукцииИМатериалов";
		ТекстЗапроса = "
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|   ,&ТекстПоместить
		|ИЗ
		|	Документ.ДвижениеПродукцииИМатериалов КАК ПередачаМатериаловВПроизводство
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Склады КАК Склады
		|		ПО ПередачаМатериаловВПроизводство.Отправитель = Склады.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыКОтгрузке.Остатки(, ДокументОтгрузки ССЫЛКА Документ.ДвижениеПродукцииИМатериалов И &ТекстУсловияОтборДокументОтгрузки) КАК ТоварыКОтгрузкеОстатки
		...
		|	И ПередачаМатериаловВПроизводство.ХозяйственнаяОперация В 
	...
	Иначе
		...
		ИмяОбъектаМетаданных = "Документ.ПередачаМатериаловВПроизводство";
		ТекстЗапроса = "
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПередачаМатериаловВПроизводство.Ссылка КАК СсылкаНаОбъектСобытия,
		|	ПередачаМатериаловВПроизводство.Ссылка КАК Предмет,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыКОтгрузке.Остатки(, ДокументОтгрузки ССЫЛКА Документ.ПередачаМатериаловВПроизводство И &ТекстУсловияОтборДокументОтгрузки) КАК ТоварыКОтгрузкеОстатки
		|		ПО ТоварыКОтгрузкеОстатки.ДокументОтгрузки = ПередачаМатериаловВПроизводство.Ссылка
		|ГДЕ
		|	ПередачаМатериаловВПроизводство.Проведен
		|	И ВЫБОР
	...
	Запрос.УстановитьПараметр("ПроконтролироватьОтгрузкуПередачаМатериалов",НСтр("ru = 'Проконтролировать отгрузку';
																				|en = 'Monitor shipment'"));
	
	ДобавитьЗапросПоЗадачам(ТекстЗапроса,ИмяОбъектаМетаданных,СтруктураФормированияОтчета);
	...
	ДобавитьВМассивПроверяемыхНаПредметДоступаТипов(СтруктураФормированияОтчета,ИмяОбъектаМетаданных);
...
Функция ТекстЗапросаДоверенностьПолучениеТоваров(Запрос,СтруктураФормированияОтчета)
	...
	Запрос.УстановитьПараметр("ПроконтролироватьСдачуДоверенности",НСтр("ru = 'Проконтролировать сдачу доверенности';
																		|en = 'Monitor authorization letter delivery'"));
...
Функция ТекстЗапросаЗаявкиНаВозврат(Запрос,СтруктураФормированияОтчета)
	...
	Если СтруктураФормированияОтчета.ИспользуетсяНоваяАрхитектураРасчетов Тогда
		...
		|					ИЛИ ЗаказКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОбеспечению)) И &ТекстУсловияЗаказ) КАК ЗаказыКлиентовОстатки
		|	ПО
		...
		|ГДЕ
		|	ЕСТЬNULL(ЗаказыКлиентовОстатки.КОформлениюОстаток,0) <> ЕСТЬNULL(ТоварыКОтгрузкеОстатки.КОтгрузкеОстаток, 0) И &ТекстУсловияОтборДокумент
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ
...
Функция ТекстЗапросаЗаказыКлиентов(Запрос,СтруктураФормированияОтчета)
	...
	Если СтруктураФормированияОтчета.ИспользуетсяНоваяАрхитектураРасчетов Тогда
		...
		|			ИЛИ ЗаказКлиента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.КОбеспечению)) И &ТекстУсловияОтборЗаказКлиента) КАК ЗаказыКлиентовОстатки
		|	ПО
		...
		|	И ЕСТЬNULL(ТоварыКОтгрузкеОстатки.КОтгрузкеОстаток, 0) > 0 
		|	И ЕСТЬNULL(ЗаказыКлиентовОстатки.КОформлениюОстаток,0) <> ЕСТЬNULL(ТоварыКОтгрузкеОстатки.КОтгрузкеОстаток, 0) И &ТекстУсловияОтборДокумент
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
...
Функция ТекстЗапросаДоговораКонтрагентов(Запрос,СтруктураФормированияОтчета)
	...
	Иначе
		...
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаОстатковКлиентовКОплате КАК ТаблицаОстатковКлиентов
		|		ПО ЭтапыГрафикаИсполненияДоговора.Ссылка.Договор = ТаблицаОстатковКлиентов.Объект
	...
	Запрос.УстановитьПараметр("ЗаканчиваетсяСрокДействияДоговора",НСтр("ru = 'Отработать окончание срока действия договора с контрагентом';
																		|en = 'Process counterparty contract expiration'"));
...
Функция ТекстЗапросаЗапланированныеВзаимодействия(Запрос, СтруктураФормированияОтчета)
	...
	Запрос.УстановитьПараметр("РассмотретьЗапланированноеВзаимодействие",НСтр("ru = 'Отработать запланированное взаимодействие';
																				|en = 'Process planned interaction'"));
...
Функция ТекстЗапросаДниРожденияКонтактныхЛиц(Запрос,СтруктураФормированияОтчета)
	...
	Запрос.УстановитьПараметр("ДеньРожденияКонтактногоЛица",НСтр("ru = 'Поздравить с днем рождения контактное лицо партнера';
																|en = 'Wish happy birthday to the partner''s contact person'"));
	Запрос.УстановитьПараметр("ДеньРожденияПартнера",НСтр("ru = 'Поздравить с днем рождения партнера';
															|en = 'Wish happy birthday to the partner'"));
	...
	Пока ВыборкаСтатус.Следующий() Цикл
		...
		Иначе
			Если ВыборкаСтатус.Статус = "Текущее" Тогда
				...
				ОбластьГруппировка.Параметры.Значение = НСтр("ru = 'Сегодня';
															|en = 'Today'") + " ( " + Формат(ТекущаяДатаСеанса(),"ДЛФ=DD") + " ) - " + ВыборкаСтатус.ДатаОбъектаСобытия;
			...
			ИначеЕсли ВыборкаСтатус.Статус = "Просроченное" Тогда
				...
				ОбластьГруппировка.Параметры.Значение = НСтр("ru = 'Просроченные';
															|en = 'Overdue'") + " - " + ВыборкаСтатус.ДатаОбъектаСобытия;
		...
		Пока ВыборкаДата.Следующий() Цикл
			...
			Если (ВыборкаСтатус.Статус = "Предстоящее"
				...
				ИначеЕсли  НачалоДня(ВыборкаДата.ДатаСобытия) - НачалоДня(ТекущаяДатаСеанса()) = 1 Тогда
					ТекстГруппировкиПредстоящее = НСтр("ru = 'Завтра';
														|en = 'Tomorrow'") + " ( " + Формат(ВыборкаДата.ДатаСобытия,"ДЛФ=DD") + " ) - " + ВыборкаДата.ДатаОбъектаСобытия;
				Иначе
...
Процедура ОтработатьАктуальностьРасчетов(ОбластьСтрока,Выборка)
	...
	Если Выборка.РасчетыНеактуальныСКлиентом ИЛИ Выборка.РасчетыНеактуальныСПоставщиком Тогда
		...
		ОбластьСобытие.ЦветТекста = ЦветаСтиля.ЦветТекстаОтмененнойСтрокиДокумента;
		ОбластьСтрока.Параметры.ТипСобытия = НСтр("ru = '[Расчет не актуален]';
													|en = '[Calculation is not relevant]'") + Символы.ПС + ОбластьСтрока.Параметры.ТипСобытия;
...
Процедура ПреобразоватьПредставлениеПредмета(Описание,Выборка)
	...
	Если ТипЗнч(Выборка.Предмет) = Тип("СправочникСсылка.СоглашенияСПоставщиками") ИЛИ
		...
		Описание = НСтр("ru = 'Соглашение с поставщиком';
						|en = 'Terms of purchase'") + " '" + Описание + "'";
		...
		Если Дата(Выборка.ДатаОбъектаСобытия) <> Дата(1,1,1) Тогда
			Описание = Описание + " " + НСтр("ru = 'от';
											|en = 'dated'") +" " + Формат(Выборка.ДатаОбъектаСобытия,"ДЛФ=D");
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("СправочникСсылка.СделкиСКлиентами") Тогда
		
		Описание = НСтр("ru = 'Сделка';
						|en = 'Sales opportunity'") + " '" + Описание + "' " + НСтр("ru = 'от';
																		|en = 'dated'") + " " + Формат(Выборка.ДатаОбъектаСобытия,"ДЛФ=D");
		
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("СправочникСсылка.Проекты") Тогда
		
		Описание = НСтр("ru = 'Проект';
						|en = 'Project'") + " '" + Описание + "'";
		
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("СправочникСсылка.МаркетинговыеМероприятия") Тогда
		
		Описание = НСтр("ru = 'Маркетинговое мероприятие';
						|en = 'Marketing activity'") + " '" + Описание + "'";
		
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		
		Описание = НСтр("ru = 'Договор';
						|en = 'Contract'") + " " + ПредставлениеТипаДоговораПоОперации(Выборка.ДопРеквизитОписание1) + " '" + Описание + "'";
	...
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
		...
		Если ТипЗнч(Выборка.СсылкаНаОбъектСобытия) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
			Описание = НСтр("ru = 'Входящее письмо';
							|en = 'Incoming email'") + " '" + ?(ПустаяСтрока(Выборка.Описание),НСтр("ru = 'Тема не указана';
																									|en = 'Subject is not specified'") + " ",Выборка.Описание) + "' "
			           + НСтр("ru = 'от';
								|en = 'dated'") + " " + Выборка.ДатаОбъектаСобытия;
		Иначе
			Описание = НСтр("ru = 'Входящее письмо';
							|en = 'Incoming email'") + " '" + ?(ПустаяСтрока(Выборка.ДопРеквизитОписание1),НСтр("ru = 'Тема не указана';
																												|en = 'Subject is not specified'") + " ",Выборка.ДопРеквизитОписание1)
			           + "' " + НСтр("ru = 'от';
									|en = 'dated'") + " " + Выборка.ДопРеквизитОписание2;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") Тогда
		...
		Если ТипЗнч(Выборка.СсылкаНаОбъектСобытия) = Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") Тогда
			Описание = НСтр("ru = 'Исходящее письмо';
							|en = 'Outgoing email'") + " '" 
			           + ?(ПустаяСтрока(Выборка.Описание),НСтр("ru = 'Тема не указана';
																|en = 'Subject is not specified'") + " ",Выборка.Описание) 
			           + "' " + НСтр("ru = 'от';
									|en = 'dated'") + " " + Выборка.ДатаОбъектаСобытия;
		Иначе
	...
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("ДокументСсылка.Встреча") Тогда
		...
		Если ТипЗнч(Выборка.СсылкаНаОбъектСобытия) = Тип("ДокументСсылка.Встреча") Тогда
			Описание = НСтр("ru = 'Встреча';
							|en = 'Meeting'") + " '" + ?(ПустаяСтрока(Выборка.Описание),НСтр("ru = 'Тема не указана';
																							|en = 'Subject is not specified'") + " ",Выборка.Описание) + "' "
			           + НСтр("ru = 'от';
								|en = 'dated'") + " " + Выборка.ДатаОбъектаСобытия;
		Иначе
			Описание = НСтр("ru = 'Встреча';
							|en = 'Meeting'") + " '" + ?(ПустаяСтрока(Выборка.ДопРеквизитОписание1),НСтр("ru = 'Тема не указана';
																										|en = 'Subject is not specified'") + " ",Выборка.ДопРеквизитОписание1)
			           + "' " + НСтр("ru = 'от';
									|en = 'dated'") + " " + Выборка.ДопРеквизитОписание2;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("ДокументСсылка.ЗапланированноеВзаимодействие") Тогда
		...
		Если ТипЗнч(Выборка.СсылкаНаОбъектСобытия) = Тип("ДокументСсылка.ЗапланированноеВзаимодействие") Тогда
			Описание = НСтр("ru = 'Запланированное взаимодействие';
			           ...
			           ?(ПустаяСтрока(Выборка.Описание),НСтр("ru = 'Тема не указана';
															|en = 'Subject is not specified'") + " ",Выборка.Описание) + "' " + НСтр("ru = 'от';
																															|en = 'dated'") + " " + Выборка.ДатаОбъектаСобытия;
		Иначе
			Описание = НСтр("ru = 'Запланированное взаимодействие';
							|en = 'Scheduled interaction'") + " '"
			           + ?(ПустаяСтрока(Выборка.ДопРеквизитОписание1),НСтр("ru = 'Тема не указана';
																			|en = 'Subject is not specified'") 
			           + " ",Выборка.ДопРеквизитОписание1) + "' " + НСтр("ru = 'от';
																		|en = 'dated'") + " " + Выборка.ДопРеквизитОписание2;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("ДокументСсылка.ТелефонныйЗвонок") Тогда
		...
		Иначе
			...
			Описание = НСтр("ru = 'Телефонный звонок';
							|en = 'Phone call'") + " '" + ?(ПустаяСтрока(Выборка.ДопРеквизитОписание1),НСтр("ru = 'Тема не указана';
																													|en = 'Subject is not specified'") + " ",Выборка.ДопРеквизитОписание1) +
			           "' " + НСтр("ru = 'от';
									|en = 'dated'") + " " + Выборка.ДопРеквизитОписание2;
	...
	ИначеЕсли ТипЗнч(Выборка.Предмет) = Тип("СправочникСсылка.ЗадачиПроектов") Тогда
		...
		Если ТипЗнч(Выборка.ДопРеквизитОписание1) = Тип("СправочникСсылка.Претензии") Тогда
			Описание = НСтр("ru = 'Задача проекта по претензии клиента';
							|en = 'Project task on the customer complaint'") + " '" + Описание + "'";
		ИначеЕсли ТипЗнч(Выборка.ДопРеквизитОписание1) = Тип("СправочникСсылка.СделкиСКлиентами") Тогда
			Описание = НСтр("ru = 'Задача проекта по сделке';
							|en = 'Project task under sales opportunity'")  + " '" + Описание + "'";
		ИначеЕсли ТипЗнч(Выборка.ДопРеквизитОписание1) = Тип("СправочникСсылка.МаркетинговыеМероприятия") Тогда
			Описание = НСтр("ru = 'Задача проекта по маркетинговому мероприятию';
							|en = 'Project task for marketing activity'")  + " '" + Описание + "'";
		Иначе
			Описание = НСтр("ru = 'Задача проекта';
							|en = 'Project task'") + " '" + Описание + "'";
		КонецЕсли;
