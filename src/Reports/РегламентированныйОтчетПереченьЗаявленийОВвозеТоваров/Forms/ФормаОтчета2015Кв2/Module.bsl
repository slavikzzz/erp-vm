...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("КвартальнаяФорма");
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма, ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		Если ЗначениеЗаполнено(Форма.СтруктураРеквизитовФормы.РегистрацияВНалОргане) Тогда
			...
			Форма.ТабличныйДокумент.Области["НалоговыйОрган"].Значение
			= СокрЛП(Форма.СтруктураРеквизитовФормы.РегистрацияВНалОргане.Код);
...
Процедура ВосстановитьЗначенияМногострочнойЧастиРаздела1СтарогоОбразца()
	...
	ЭтаФорма.П0000100010.Очистить();
	ЭтаФорма.П0000100010.Добавить();
	
	Пока ДанныеРаздела1.Свойство("П000010001001_" + Формат(НомСтрМНЧ, "ЧГ="), ТекЗначение) Цикл
		
		ЭлемКоллекции = ?(ЭтаФорма.П0000100010.Количество() >= НомСтрМНЧ,
			ЭтаФорма.П0000100010[НомСтрМНЧ - 1],
			ЭтаФорма.П0000100010.Добавить());
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	СтруктураРеквизитовФормы.КвартальнаяФорма
...
Процедура СформироватьСпискиВыбора()
		...
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
			...
			Для НомСтр = ВерхОбласти По НизОбласти Цикл
				...
				Если КодПоказателя <> "###" Тогда
					...
					Если Область.Имя = "ПериодыМесяц" Тогда
	...
	Если СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц Тогда
		...
		"Выбор кода налогового периода", КоллекцияСписковВыбора["ПериодыМесяц"]);
	КонецЕсли;
...
Процедура ПроверитьЗаполнение_ТитульныйЛист(Параметры, СтатусПроверки)
	...
	Если НЕ ЗначениеЗаполнено(Параметры.Период) Тогда
		...
		"На титульном листе не задан код налогового периода", "Титульный", "Период");
		СтатусПроверки = Ложь;
	ИначеЕсли СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц Тогда
		...
		Если ВозмКодыПериода.МассивЗначений.Найти(Параметры.Период) = Неопределено Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"На титульном листе задано некорректное значение кода налогового периода. Возможны значения: "
			+ ВозмКодыПериода.СтрокаЗначений, "Титульный", "Период");
			СтатусПроверки = Ложь;
		ИначеЕсли НЕ ЭтоПБОЮЛ И ЗначениеЗаполнено(Параметры.ФормРеорг) И НЕ (Лев(Параметры.Период, 1) = "7"
			ИЛИ Лев(Параметры.Период, 1) = "8" ИЛИ Лев(Параметры.Период, 1) = "B" ИЛИ Лев(Параметры.Период, 1) = "D") Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"На титульном листе задано некорректное значение кода налогового периода при заполненном коде формы реорганизации (ликвидации)",
			"Титульный", "Период");
			СтатусПроверки = Ложь;
		ИначеЕсли НЕ ЭтоПБОЮЛ И НЕ ЗначениеЗаполнено(Параметры.ФормРеорг) И НЕ (Лев(Параметры.Период, 1) = "0"
			ИЛИ Лев(Параметры.Период, 1) = "1" ИЛИ Лев(Параметры.Период, 1) = "A" ИЛИ Лев(Параметры.Период, 1) = "C") Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"На титульном листе задано некорректное значение кода налогового периода при незаполненном коде формы реорганизации (ликвидации)",
			"Титульный", "Период");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяСтраницыПанели = "")
	...
	Если ИмяСтраницыПанели = "Титульный" Тогда
		...
		ОбрабатываемаяОбласть = Форма.ТабличныйДокумент.Области.Период;
		Если Форма.СтруктураРеквизитовФормы.КвартальнаяФорма Тогда
			ЗапретитьВводДанныхВОбласть(Форма, ОбрабатываемаяОбласть, Ложь);
		КонецЕсли;
		
	КонецЕсли;
	
	ПереопределитьЗначенияСвойствЯчеекТабличногоДокумента(Форма, ИмяСтраницыПанели);
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Контейнер.Вставить("Раздел1", ДанныеФормыВЗначение(П0000100010, Тип("ТаблицаЗначений")));
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	ЗначениеВДанныеФормы(Контейнер["Раздел1"], П0000100010);
	
	Если П0000100010.Количество() = 0 Тогда
		П0000100010.Добавить();
	КонецЕсли;
	
	СтруктураДанныхРаздел1.Очистить();
	
	РегламентированнаяОтчетность.ЗаполнитьСтруктуруДанныхРазделаЗначениямиИзДопСтрок(
	СтруктураДанныхРаздел1, П0000100010);
...
Процедура Расшифровать(Команда)
	...
	Графа4 = Неопределено;
	Графа5 = Неопределено;
	НомерТекущейСтроки = Неопределено;
	
	Если СтрНачинаетсяС(ЯчейкаРасшифровки.Имя, "П0") И Сред(ЯчейкаРасшифровки.Имя, 14, 1) = "_"
		И ЯчейкаРасшифровки.СодержитЗначение = Истина Тогда
		
		Постфикс = Сред(ЯчейкаРасшифровки.Имя, 14);
		
		Графа1 = ?(ТДО.Найти("П000010001001" + Постфикс) = Неопределено,
		Графа1, СокрЛП(ТДО["П000010001001" + Постфикс].Значение));
		
		Графа2 = ?(ТДО.Найти("П000010001002" + Постфикс) = Неопределено,
		Графа2, ТДО["П000010001002" + Постфикс].Значение);
		
		Графа3 = ?(ТДО.Найти("П000010001003" + Постфикс) = Неопределено,
		Графа3, СокрЛП(ТДО["П000010001003" + Постфикс].Значение));
		
		Графа4 = ?(ТДО.Найти("П000010001004" + Постфикс) = Неопределено,
		Графа4, СокрЛП(ТДО["П000010001004" + Постфикс].Значение));
		
		Графа5 = ?(ТДО.Найти("П000010001005" + Постфикс) = Неопределено,
		Графа5, СокрЛП(ТДО["П000010001005" + Постфикс].Значение));
		
		НомерТекущейСтроки = Число(Сред(Постфикс, 2));
		НомерТекущейСтроки = ?(ТипЗнч(НомерТекущейСтроки) = Тип("Число"), НомерТекущейСтроки, Неопределено);
	...
	ПараметрыОтчета.Вставить("Графа1", Графа1);
	ПараметрыОтчета.Вставить("Графа2", Графа2);
	ПараметрыОтчета.Вставить("Графа3", Графа3);
	ПараметрыОтчета.Вставить("Графа4", Графа4);
	ПараметрыОтчета.Вставить("Графа5", Графа5);
	ПараметрыОтчета.Вставить("НомерТекущейСтроки", НомерТекущейСтроки);
...
Процедура ПереопределитьЗначенияСвойствЯчеекТабличногоДокумента(Форма, ИмяРаздела)
	...
	Для каждого Обл Из Форма.ТабличныйДокумент.Области Цикл
		...
		Если СтрНачинаетсяС(Обл.Имя, "П0000100010") Тогда
