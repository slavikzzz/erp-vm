...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбъектОтчет=ОбъектОтчета(ИмяФормы);
...
Процедура ИнициализацияМногостраничныхРазделов()
	...
	СтрПерваяСтраница = ТаблицаСтраницыРазделаРаздел2.Добавить();
	СтрПерваяСтраница.Представление               = "Новая страница";
	СтрПерваяСтраница.АктивнаяСтраница            = Истина;
	СтрПерваяСтраница.АвтоматическоеПредставление = Истина;
	
	СтрПерваяСтраница = ТаблицаСтраницыРазделаРаздел3.Добавить();
	СтрПерваяСтраница.Представление               = "Новая страница";
	СтрПерваяСтраница.АктивнаяСтраница            = Истина;
	СтрПерваяСтраница.АвтоматическоеПредставление = Истина;
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСкопированаФорма) Тогда
			...
			Если НЕ (Год(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания)
				= Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)
				...
				Для каждого СтраницаРаздел3 Из ТаблицаСтраницыРазделаРаздел3 Цикл
					Если ЗначениеЗаполнено(СтраницаРаздел3.Данные[0].Значение.П000030009003_Числ)
						ИЛИ ЗначениеЗаполнено(СтраницаРаздел3.Данные[0].Значение.П000030009003_Знам) Тогда
						СтраницаРаздел3.Данные[0].Значение.Вставить("П000030009003_Числ", "");
						СтраницаРаздел3.Данные[0].Значение.Вставить("П000030009003_Знам", "");
						СообщениеОбИзменениях
						= НСтр("ru = 'В разделе 3 очищено значение показателя ""Коэффициент К"". Произведен перерасчет всех рассчитываемых показателей.';
								|en = 'В разделе 3 очищено значение показателя ""Коэффициент К"". Произведен перерасчет всех рассчитываемых показателей.'")
					КонецЕсли;
				КонецЦикла;
	...
	Если ЗначениеЗаполнено(СообщениеОбИзменениях) Тогда
		...
		СформироватьРаздел1();
		РасчетНаСервере();
		ОбщегоНазначения.СообщитьПользователю(СообщениеОбИзменениях);
	КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСписковВыбора = ОбъектОтчета(ИмяФормы).ПолучитьМакет("СпискиВыбора2013Кв4");
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Для каждого СтраницаРаздел3 Из ТаблицаСтраницыРазделаРаздел3 Цикл
		// Конвертация данных: изменением типа значения в строке 090 раздела 3.
		Если ТипЗнч(СтраницаРаздел3.Данные[0].Значение.П000030009003_Числ) = Тип("Число") Тогда
			СтраницаРаздел3.Данные[0].Значение.Вставить("П000030009003_Числ",
			Формат(СтраницаРаздел3.Данные[0].Значение.П000030009003_Числ));
		КонецЕсли;
		Если ТипЗнч(СтраницаРаздел3.Данные[0].Значение.П000030009003_Знам) = Тип("Число") Тогда
			СтраницаРаздел3.Данные[0].Значение.Вставить("П000030009003_Знам",
			Формат(СтраницаРаздел3.Данные[0].Значение.П000030009003_Знам));
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьЦветФонаЯчейкиНаКлиенте(ИмяЯчейки = Неопределено, ВариантЗаполнения = Неопределено, ИмяТекущейСтраницыПанели = "") Экспорт
	...
	РегламентированнаяОтчетностьКлиентСервер.УстановитьЦветФонаЯчейки(ЭтотОбъект, ИмяЯчейки, ВариантЗаполнения, ИмяТекущейСтраницыПанели);
	...
	ИначеЕсли ИмяСтраницыПанели = "Раздел3" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел3" Тогда
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000030010003, ОбластьИмя) > 0 Тогда			
				...
				Если ЗначениеЗаполнено(П000030009003_Числ) И ЗначениеЗаполнено(П000030009003_Знам) Тогда
					Если Число(П000030009003_Знам) > 0 Тогда
						Строка090_Числ = Число(П000030009003_Числ);
						Строка090_Знам = Число(П000030009003_Знам);
					КонецЕсли;
				КонецЕсли;
				
				П000030008003 = ПолеТабличногоДокументаРаздел3.П000030008003.Значение;
				П000030010003 = Макс(РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, ПолеТабличногоДокументаРаздел3.П000030006003.Значение * П000030008003 * Строка090_Числ / Строка090_Знам / 100), 0);
...
Процедура ПолныйРасчетРаздела3(Форма, ОбластьИмя = "")
	...
	Для Каждого СтраницаРаздела3 Из Форма.ТаблицаСтраницыРазделаРаздел3 Цикл
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000030010003, ОбластьИмя) > 0 Тогда			
			...
			П000030010003 = Макс(РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, ПолеТабличногоДокументаРаздел3.П000030006003 * П000030008003 * Строка090_Числ / Строка090_Знам / 100), 0);
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Область)
	...
	Если ИмяПоказателя = "ОКВЭД" И НЕ ПустаяСтрока(Область.Значение) И НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(Область.Значение) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			...
			|en = 'На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД!%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).'"),
			Символы.ПС, Символы.ПС));
	КонецЕсли;
	
	Если ИмяПоказателя = "П000030009003_Числ" Тогда
		П000030009003_Числ = СокрЛП(Область.Значение);
		Область.Значение = ?(СтрДлина(П000030009003_Числ) = 2 И Лев(П000030009003_Числ, 1) = "0",
		Прав(П000030009003_Числ, 1), Область.Значение);
		Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) < 2016 Тогда
			П000030009003_Числ = СокрЛП(Область.Значение);
			Область.Значение = ?(П000030009003_Числ = "0", "", Область.Значение);
		КонецЕсли;
		Если ЗначениеЗаполнено(Область.Значение) Тогда
			Если Число(Область.Значение) > 12 Тогда
				Область.Значение = "";
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр(
					"ru = 'Значение в числителе коэффициента К не должно превышать 12. Значение показателя очищено.';
					|en = 'Значение в числителе коэффициента К не должно превышать 12. Значение показателя очищено.'"));
			КонецЕсли;
		КонецЕсли;
		Если ЗначениеЗаполнено(Область.Значение) Тогда
			ТабличныйДокумент.Области.Найти("П000030009003_Знам").Значение = "12";
		Иначе
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяСтраницыПанели = "")
	...
	ИначеЕсли ИмяСтраницыПанели = "Раздел3" Тогда
		...
		ОбрабатываемаяОбласть = Форма.ТабличныйДокумент.Области.П000030009003_Знам;
...
Процедура СохранитьДанныеРаздела(НаимРаздела, ОчищатьРазделЕслиЕстьМногострочность, ИзменятьАктивнуюСтраницу = Истина)	
	...
	ИначеЕсли НаимРаздела = "Раздел2"		
		...
		АктивнаяСтраница = ЭтотОбъект[НаимТаблицыСтраницы].НайтиСтроки(Новый Структура("АктивнаяСтраница", Истина));
...
Процедура ВставитьДополнительнуюСтраницуНаСервере(ИмяТекущейСтраницыПанели = Неопределено)
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел3" Тогда
		ТабличныйДокумент.Области.П000030009003_Знам.Значение = "";
	КонецЕсли;
	УстановитьДоступностьОбластей(ЭтотОбъект, ИмяТекущейСтраницыПанели);
	СтруктураРеквизитовФормы.ВыводитьСтраницуВТаблДокНаСервере = Ложь;
...
Процедура ВставитьДополнительнуюСтроку(ТекущаяСтраница)
	...
	Если ИмяГруппы = Неопределено Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр(
			"ru = 'Не выбран многострочный раздел, в который необходимо добавить строку.';
...
Процедура СформироватьРаздел1()
	...
	ТаблДанных = ЭтотОбъект[мСтруктураМногостраничныхРазделов.Раздел2];
	Для Каждого Стр из ТаблДанных Цикл
		...
		Если НЕ ПустаяСтрока(ОКТМО) И НЕ ПустаяСтрока(ВидИмущества) И СтрНайти(ПодСтрПоиска, ВидИмущества) <> 0 Тогда
			...
			Если ВидИмущества = "4" Тогда
				МинЗнач = Мин(РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, Стр.Данные[0].Значение.П000020022003) - РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, Стр.Данные[0].Значение.П000020025003),
				              РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект,Стр.Данные[0].Значение.П000020026003));
				
				НовСтр.Сумма = -МинЗнач;
			Иначе
				НовСтр.Сумма = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, Стр.Данные[0].Значение.П000020022003)
				             - РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, Стр.Данные[0].Значение.П000020023003)
				             - РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, Стр.Данные[0].Значение.П000020025003);
	...
	ТаблДанных = ЭтотОбъект[мСтруктураМногостраничныхРазделов.Раздел3];
	Для Каждого Стр из ТаблДанных Цикл
		...
		Если НЕ ПустаяСтрока(ОКТМО) Тогда
			...
			НовСтр.Сумма = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, Стр.Данные[0].Значение.П000030010003)
			             - РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, Стр.Данные[0].Значение.П000030011003)
			             - РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, Стр.Данные[0].Значение.П000030013003);
	...
	ТаблицаМСЧРаздела1 = ЭтотОбъект[мСтруктураМногострочныхРазделов[СтруктураРеквизитовФормы.ИдМСЧ_Раздел1]];
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	МакетМЧБ = ОбъектОтчета(ИмяФормы).ПолучитьМакет(МЧБРаздела);
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = ОбъектОтчета(ИмяФормы).ПолучитьМакет(ИмяМакетаСтруктурыВыгрузки());
	...
	Пока ДанныеРаздела.Свойство("П000010001003" + Постфикс) Цикл
		...
		Если БлокЗаполнен Тогда
			...
			Если ДлинаОКТМО <> 11 И ДлинаОКТМО <> 8 Тогда
				...
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, СообщениеОбОшибке, "Раздел1", ИмяОбластиОКТМО);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Если ДлинаКБК <> 20 Тогда
				...
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, СообщениеОбОшибке, "Раздел1", ИмяОбластиКБК);
				СтатусПроверки = Ложь;
			КонецЕсли;
...
Процедура ПроверитьЗаполнение_Раздел2(СочетаниеРазделов, СтатусПроверки)
	...
	ПодСтрПоискаКодовВидаИмущества = "1234567";
	...
	Для Каждого Лист Из ДанныеРаздела Цикл
		...
		Если НЕ ЗначениеЗаполнено(КодВидаИмущества) ИЛИ СтрНайти(ПодСтрПоискаКодовВидаИмущества, КодВидаИмущества) = 0 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 2 код вида имущества должен принимать одно из значений: ""1"", ""2"", ""3"", ""4"", ""5"", ""6"", ""7""" + ДопКодВидаИмуществаВСообщении + ".", "Раздел2", "П000020000101", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		ДлинаОКТМО = СтрДлина(ОКТМО);
		Если ДлинаОКТМО <> 11 И ДлинаОКТМО <> 8 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 2 не" + ?(ДлинаОКТМО = 0, "", "верно") + " задан код по ОКТМО", "Раздел2", "П000020001001", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ДлинаКодаЛьготы <> 0 И ДлинаКодаЛьготы <> 7 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 2 неверно задан код налоговой льготы", "Раздел2", "П000020016003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ДанныеЛиста.П000020018003_Числ) ИЛИ ЗначениеЗаполнено(ДанныеЛиста.П000020018003_Знам) Тогда
			Если НЕ ЗначениеЗаполнено(ДанныеЛиста.П000020018003_Числ) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 2 не задан числитель в доле балансовой стоимости объекта недвижимого имущества на территории субъекта РФ", "Раздел2", "П000020018003_Числ", НомерЛиста);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ ЗначениеЗаполнено(ДанныеЛиста.П000020018003_Знам) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 2 не задан знаменатель в доле балансовой стоимости объекта недвижимого имущества на территории субъекта РФ", "Раздел2", "П000020018003_Знам", НомерЛиста);
				СтатусПроверки = Ложь;
			ИначеЕсли ДанныеЛиста.П000020018003_Знам < ДанныеЛиста.П000020018003_Числ Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 2 числитель больше знаменателя в доле балансовой стоимости объекта недвижимого имущества на территории субъекта РФ", "Раздел2", "П000020018003_Числ", НомерЛиста);
				СтатусПроверки = Ложь;
			КонецЕсли;
		ИначеЕсли КодВидаИмущества = "2" Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 2 не задана доля балансовой стоимости объекта недвижимого имущества на территории субъекта РФ", "Раздел2", "П000020018003_Числ", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ДлинаКодаЛьготы <> 0 И ДлинаКодаЛьготы <> 7 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 2 неверно задан код налоговой льготы (установленной в виде понижения налоговой ставки)", "Раздел2", "П000020020003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ДлинаКодаЛьготы <> 0 И ДлинаКодаЛьготы <> 7 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 2 неверно задан код налоговой льготы (в виде уменьшения суммы налога, подлежащей уплате в бюджет)", "Раздел2", "П000020024003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
...
Процедура ПроверитьЗаполнение_Раздел3(СочетаниеРазделов, СтатусПроверки)
	...
	Для Каждого Лист Из ДанныеРаздела Цикл
		...
		ДлинаОКТМО = СтрДлина(ОКТМО);
		Если ДлинаОКТМО <> 11 И ДлинаОКТМО <> 8 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 не" + ?(ДлинаОКТМО = 0, "", "верно") + " задан код по ОКТМО", "Раздел3", "П000030001001", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(КадастровыйНомерЗдания) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 не задан кадастровый номер здания (строения, сооружения)", "Раздел3", "П000030001401", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ЗаполненаКадастроваяСтоимость И ЗаполненаИнвентаризационнаяСтоимость Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 одновременно заполнены сведения о кадастровой и инвентаризационной стоимости", "Раздел3", "П000030002003", НомерЛиста);
			СтатусПроверки = Ложь;
		Иначе
			Если ДанныеЛиста.П000030002003 < ДанныеЛиста.П000030002503 Тогда;
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 значение в строке 025 превышает значение в строке 020", "Раздел3", "П000030002503", НомерЛиста);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если ДанныеЛиста.П000030003003 < ДанныеЛиста.П000030003503 Тогда;
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 значение в строке 035 превышает значение в строке 030", "Раздел3", "П000030003503", НомерЛиста);
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если ДлинаКодаЛьготы <> 0 И ДлинаКодаЛьготы <> 7 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 неверно задан код налоговой льготы", "Раздел3", "П000030004003_КодЛьготы", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ДанныеЛиста.П000030005003_Числ) ИЛИ ЗначениеЗаполнено(ДанныеЛиста.П000030005003_Знам) Тогда
			Если НЕ ЗначениеЗаполнено(ДанныеЛиста.П000030005003_Числ) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 не задан числитель в доле инвентаризационной стоимости объекта недвижимого имущества на территории субъекта РФ", "Раздел3", "П000030005003_Числ", НомерЛиста);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ ЗначениеЗаполнено(ДанныеЛиста.П000030005003_Знам) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 не задан знаменатель в доле инвентаризационной стоимости объекта недвижимого имущества на территории субъекта РФ", "Раздел3", "П000030005003_Знам", НомерЛиста);
				СтатусПроверки = Ложь;
			ИначеЕсли ДанныеЛиста.П000030005003_Знам < ДанныеЛиста.П000030005003_Числ Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 числитель больше знаменателя в доле инвентаризационной стоимости объекта недвижимого имущества на территории субъекта РФ", "Раздел3", "П000030005003_Числ", НомерЛиста);
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если ДлинаКодаЛьготы <> 0 И ДлинаКодаЛьготы <> 7 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 неверно задан код налоговой льготы (установленной в виде понижения налоговой ставки)", "Раздел3", "П000030007003_КодЛьготы", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ДанныеЛиста.П000030009003_Числ) ИЛИ ЗначениеЗаполнено(ДанныеЛиста.П000030009003_Знам) Тогда
			Если НЕ ЗначениеЗаполнено(ДанныеЛиста.П000030009003_Числ) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 не задан числитель коэффициента К", "Раздел3", "П000030009003_Числ", НомерЛиста);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ ЗначениеЗаполнено(ДанныеЛиста.П000030009003_Знам) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 не задан знаменатель коэффициента К", "Раздел3", "П000030009003_Знам", НомерЛиста);
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если ДлинаКодаЛьготы <> 0 И ДлинаКодаЛьготы <> 7 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 неверно задан код налоговой льготы (в виде уменьшения суммы налога, подлежащей уплате в бюджет)", "Раздел3", "П000030012003_КодЛьготы", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
...
Процедура ПроверитьЗаполнение_СочетаниеРазделов(СочетаниеРазделов, СтатусПроверки)
	...
	Если СочетанияРаздела1.Количество() = 0 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "В разделе 1 не заполнен ни один блок с информацией о суммах налога", "Раздел1", "П000010001003_1");
		СтатусПроверки = Ложь;
	...
	Если СочетанияРаздела2.Количество() = 0 И СочетанияРаздела3.Количество() = 0 Тогда
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, СообщениеОбОшибке, "Раздел2", "П000020001001", 1);
		СтатусПроверки = Ложь;
	...
	Для Каждого СтрокаПроверки Из ПовторенияРаздела1 Цикл
		Если СтрокаПроверки.Счетчик1 > 1 И ЗначениеЗаполнено(СтрокаПроверки.ОКТМО) И ЗначениеЗаполнено(СтрокаПроверки.КБК) Тогда
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, СообщениеОбОшибке, "Раздел1", ПозицияПерехода, СтраницаПерехода);
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого Сочетание Из СочетаниеРазделовСвернутое Цикл
		...
		Если Сочетание.Счетчик1 <> 0 И Сочетание.Счетчик2 = 0 И Сочетание.Счетчик3 = 0
		   И ЗначениеЗаполнено(Сочетание.ОКТМО) И ЗначениеЗаполнено(Сочетание.КБК) Тогда
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, СообщениеОбОшибке, "Раздел1", ПозицияПерехода);
		...
		Если Сочетание.Счетчик1 = 0 И Сочетание.Счетчик2 <> 0 И Сочетание.Счетчик3 = 0
		   И ЗначениеЗаполнено(Сочетание.ОКТМО) И ЗначениеЗаполнено(Сочетание.КБК) Тогда
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, СообщениеОбОшибке, "Раздел2", ПозицияПерехода, СтраницаПерехода);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если Сочетание.Счетчик1 = 0 И Сочетание.Счетчик2 = 0 И Сочетание.Счетчик3 <> 0
		   И ЗначениеЗаполнено(Сочетание.ОКТМО) И ЗначениеЗаполнено(Сочетание.КБК) Тогда
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, СообщениеОбОшибке, "Раздел3", ПозицияПерехода, СтраницаПерехода);
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьЗаполнение_ТитульныйЛист(ПараметрыВыгрузки, СтатусПроверки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ИННОрг) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан ИНН организации", "Титульный", "ИНН1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.КППОрг) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан КПП организации", "Титульный", "КПП1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПараметрыВыгрузки.Период <> "34" И ПараметрыВыгрузки.Период <> "50" Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Значение кода налогового периода должно быть равно ""34"" или ""50""", "Титульный", "Период");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.НаимОрг) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указано полное наименование организации", "Титульный", "НаимОрг");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.КодНО) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан код налогового органа", "Титульный", "НалоговыйОрган");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКВЭД) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На титульном листе не задано значение кода вида деятельности по ОКВЭД", "Титульный", "ОКВЭД");
		СтатусПроверки = Ложь;
	ИначеЕсли НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(ПараметрыВыгрузки.ОКВЭД) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД! Допустимы значения, соответствующие следующим шаблонам: ""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).", "Титульный", "ОКВЭД");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ПоМесту) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На титульном листе не задан код места, по которому представляется документ", "Титульный", "ПоМесту");
		СтатусПроверки = Ложь;
	ИначеЕсли ВозможныеКодыПоМесту.Найти(ПараметрыВыгрузки.ПоМесту) = Неопределено Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На титульном листе задан некорректный код места, по которому представляется документ", "Титульный", "ПоМесту");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ПодпФамилия) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задана фамилия подписанта", "Титульный", "ОргПодписант");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ПодпИмя) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задано имя подписанта", "Титульный", "ОргПодписант");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПараметрыВыгрузки.ПрПодп = "2" И ПустаяСтрока(ПараметрыВыгрузки.ПодпНаимДок) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задано наименование документа, удостоверяющего полномочия представителя", "Титульный", "ДокУпПред");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрыВыгрузки.ИННРеорг) ИЛИ ЗначениеЗаполнено(ПараметрыВыгрузки.КППРеорг) ИЛИ ЗначениеЗаполнено(ПараметрыВыгрузки.ФормРеорг) Тогда
		...
		Если ПустаяСтрока(ПараметрыВыгрузки.ФормРеорг) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан код формы реорганизации", "Титульный", "КодФормыРеорганизации");
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если НЕ ПустаяСтрока(ПараметрыВыгрузки.ФормРеорг) И ПараметрыВыгрузки.ФормРеорг <> "0" Тогда
			...
			Если ДлинаИННРеорг <> 10 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПараметрыВыгрузки.ИННРеорг) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не" + ?(ДлинаИННРеорг = 0, "", "верно") + " задан ИНН реорганизованной организации", "Титульный", "ИННРеорганизованнойОрганизации");
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ДлинаКППРеорг = СтрДлина(ПараметрыВыгрузки.КППРеорг);
			Если ДлинаКППРеорг <> 9 Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не" + ?(ДлинаКППРеорг = 0, "", "верно") + " задан КПП реорганизованной организации", "Титульный", "КППРеорганизованнойОрганизации");
				СтатусПроверки = Ложь;
			КонецЕсли;
...
Функция ЗначениеСоответствуетФормату(Стр)
	...
	Если Стр.Формат = "Т" ИЛИ Стр.Формат = "T" Тогда // учтем оба варианта: кириллицу и латиницу
		...
		Если ДлинаЗначения < Стр.МинРазмерность Тогда
			СтатусПроверки = Ложь;
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Формирование XML: Длина значения показателя """ + РегламентированнаяОтчетность.ПредставлениеПоказателя(Стр) + """ (" + АдресПоказателя(Стр)+ ") меньше минимально возможной (" + ДлинаЗначения + " < " + Стр.МинРазмерность + ")", "Титульный", "");
		КонецЕсли;
		Если ДлинаЗначения > Стр.МаксРазмерность Тогда
			СтатусПроверки = Ложь;
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Формирование XML: Длина значения показателя """ + РегламентированнаяОтчетность.ПредставлениеПоказателя(Стр) + """  (" + АдресПоказателя(Стр)+ ") больше максимально возможной (" + ДлинаЗначения + " > " + Стр.МаксРазмерность + ")", "Титульный", "");
		КонецЕсли;
	КонецЕсли;
	Возврат СтатусПроверки;
...
Процедура ИзвлечьСохраненныеПоказателиОтчета()
	...
	Если мСтруктураМногостраничныхРазделов.Свойство(СтруктураРеквизитовФормы.НаимТекущегоРаздела, ТекущийРаздел) Тогда
		АктивнаяСтраница = ЭтотОбъект[ТекущийРаздел].НайтиСтроки(Новый Структура("АктивнаяСтраница", Истина));
		Если АктивнаяСтраница.Количество() > 0 Тогда
			АктивнаяСтраница = АктивнаяСтраница[0];
			ИндексАктивнойСтраницы = ЭтотОбъект[ТекущийРаздел].Индекс(АктивнаяСтраница);
		КонецЕсли;
	КонецЕсли;
	...
	Если мСтруктураМногостраничныхРазделов.Свойство(СтруктураРеквизитовФормы.НаимТекущегоРаздела, ТекущийРаздел) Тогда
		Если ИндексАктивнойСтраницы <> Неопределено Тогда
			ЭтотОбъект[ТекущийРаздел][ИндексАктивнойСтраницы].АктивнаяСтраница = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если ТаблицаЗначенийСДанными <> Неопределено Тогда
		...
		Если ТаблицаЗначенийСДанными.Колонки.Найти("Данные") <> Неопределено Тогда
			...
			Для Каждого СтрокаТаблицыЗначений Из ТаблицаЗначенийСДанными Цикл
				СтрокаТаблицыЗначений.Данные = ЭтотОбъект[ИмяСтруктурыДанных][Ном].Данные[0].Значение;
				Ном = Ном + 1;
			КонецЦикла;
		КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из ТаблицаСДанными Цикл
			Если ТипЗнч(СтрокаТаблицы.ДанныеДопСтрок) = Тип("Структура") Тогда
				Для Каждого ЭлСтруктуры Из СтрокаТаблицы.ДанныеДопСтрок Цикл
					...
					СтрокаТаблЗначДанныеДопСтрок = ЭтотОбъект["Хранилище" + ОпределитьИдМногострочногоРазделаПоАктивнойСтраницеНаСервере(ИмяРаздела)].Добавить().Таблица;
...
Процедура ПроверитьКС_1_1(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Сумма налога. подлежащая уплате в бюджет (строка 030 Раздела 1) должна быть равна строка 220 раздела 2 - (строка 230 Раздела 2 + строка 250 раздела 2) + строка 100 Раздела 3 - (строка 110 Раздела 3 + строка 130 Раздела 3) по соответствующим кодам ОКТМО и КБК. а строка 040 раздела 1 должна быть не заполнена, если
	                          ...
	                          |en = 'Сумма налога. подлежащая уплате в бюджет (строка 030 Раздела 1) должна быть равна строка 220 раздела 2 - (строка 230 Раздела 2 + строка 250 раздела 2) + строка 100 Раздела 3 - (строка 110 Раздела 3 + строка 130 Раздела 3) по соответствующим кодам ОКТМО и КБК. а строка 040 раздела 1 должна быть не заполнена, если
	                          |- отсутствует Раздел 2 с кодом имущества = ""4"",
	                          |- строка 220 Раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2) + строка 100 Раздела 3 - (строка 110 Раздела 3 + строка 130 Раздела 3) > 0'");
	
	Нарушение       = НСтр("ru = 'НК РФ ст. 45, НК РФ ст.382 п.2';
							|en = 'НК РФ ст. 45, НК РФ ст.382 п.2'");
	ОписаниеОшибки1 = НСтр("ru = 'Завышена сумма налога, подлежащая уплате в бюджет';
							|en = 'Завышена сумма налога, подлежащая уплате в бюджет'");
	ОписаниеОшибки2 = НСтр("ru = 'Занижена сумма налога, подлежащая уплате в бюджет';
							|en = 'Занижена сумма налога, подлежащая уплате в бюджет'");
...
Процедура ПроверитьКС_1_2(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Сумма налога, подлежащая уплате в бюджет (строка 030 Раздела 1) должна быть равна [строка 220 Раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)] всех разделов 2 с кодами имущества = ""1"", ""2"", ""3"", ""5"", ""6"", ""8"" - MIN [{строка 220 раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)} или строка 260 Раздела 2] Раздела 2 с кодом вида имущества = ""4"", а строка 040 Раздела 1 должна быть не заполнена, если
								| 1) присутствует Раздел 2 с кодом имущества = ""4"";
								| 2) [строка 220 Раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)] всех разделов 2 с кодами имущества = ""1"", ""2"", ""3"", ""5"", ""6"", ""8"" с соответствующими кодами ОКТМО и КБК - MIN [{строка 220 раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)} или строка 260 Раздела 2] Раздела 2 с кодом вида имущества = ""4"" > 0';
								|en = 'Сумма налога, подлежащая уплате в бюджет (строка 030 Раздела 1) должна быть равна [строка 220 Раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)] всех разделов 2 с кодами имущества = ""1"", ""2"", ""3"", ""5"", ""6"", ""8"" - MIN [{строка 220 раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)} или строка 260 Раздела 2] Раздела 2 с кодом вида имущества = ""4"", а строка 040 Раздела 1 должна быть не заполнена, если
								| 1) присутствует Раздел 2 с кодом имущества = ""4"";
								| 2) [строка 220 Раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)] всех разделов 2 с кодами имущества = ""1"", ""2"", ""3"", ""5"", ""6"", ""8"" с соответствующими кодами ОКТМО и КБК - MIN [{строка 220 раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)} или строка 260 Раздела 2] Раздела 2 с кодом вида имущества = ""4"" > 0'");
	...
	Пока Раздел1.Свойство("П000010001003" + Постфикс) Цикл
		...
		КонтрольноеЗначениеУм = 0;
		КонтрольноеЗначениеВч = 0;
		ЕстьРаздел2СКодом4 = Ложь;
		ЕстьСведенияИсчислений = Ложь;
		...
		Для Каждого СтраницаР2 Из Раздел2 Цикл
			...
			Если Р2Стр001.Значение = "4" Тогда
				ЕстьРаздел2СКодом4 = Истина;
				КонтрольноеЗначениеВч = КонтрольноеЗначениеВч
				                    + Мин(Р2Стр220.Значение - (Р2Стр230.Значение + Р2Стр250.Значение), Р2Стр260.Значение);
			ИначеЕсли ЗначениеСодержитсяВСписке(Р2Стр001.Значение, "1, 2, 3, 5, 6, 8") Тогда
				КонтрольноеЗначениеУм = КонтрольноеЗначениеУм + Р2Стр220.Значение - (Р2Стр230.Значение + Р2Стр250.Значение);
			КонецЕсли;
		КонецЦикла;
		
		ПроверятьСоотношение = ЕстьРаздел2СКодом4 И ((КонтрольноеЗначениеУм - КонтрольноеЗначениеВч) > 0);
		Если НЕ ПроверятьСоотношение Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого СтраницаР2 Из Раздел2 Цикл
			...
			Если Р2Стр001.Значение = "4" Тогда
				...
				ТекстРасшифровки = "MIN((" + Р2Стр220.ЗначениеСтр + " - (" + Р2Стр230.ЗначениеСтр + " + " + Р2Стр250.ЗначениеСтр + "), " + Р2Стр260.ЗначениеСтр + ") + ";
		...
		КонтрольноеЗначение = КонтрольноеЗначениеУм - КонтрольноеЗначениеВч;
		Если ЕстьСведенияИсчислений Тогда
			Расшифровка = Лев(Расшифровка, СтрДлина(Расшифровка) - 3);
			РасшифровкаУсловие = Лев(РасшифровкаУсловие, СтрДлина(РасшифровкаУсловие) - 3);
...
Процедура ПроверитьКС_1_3(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Сумма налога, исчисленная к уменьшению (строка 040 Раздел 1) по соответствующим кодам ОКТМО и КБК должна быть равна - [строка 220 Раздел 2 - (строка 230 Раздел 2 + строка 250 Раздел 2) + строка 100 Раздел 3 - (строка 110 Раздел 3 + строка 130 Раздел 3)] и строка 030 Раздела 1 не заполнена, если:
	                          ...
	                          |en = 'Сумма налога, исчисленная к уменьшению (строка 040 Раздел 1) по соответствующим кодам ОКТМО и КБК должна быть равна - [строка 220 Раздел 2 - (строка 230 Раздел 2 + строка 250 Раздел 2) + строка 100 Раздел 3 - (строка 110 Раздел 3 + строка 130 Раздел 3)] и строка 030 Раздела 1 не заполнена, если:
	                          |- отсутствует раздел 2 с кодом вида имущества = ""4"",
	                          |- строка 220 Раздел 2 - (строка 230 Раздел 2 + строка 250 Раздел 2) + строка 100 Раздел 3 - (строка 110 Раздел 3 + строка 130 Раздел 3) < 0'");
	...
	ОписаниеОшибки1 = НСтр("ru = 'Завышена сумма налога, исчисленная к уменьшению';
							|en = 'Завышена сумма налога, исчисленная к уменьшению'");
	ОписаниеОшибки2 = НСтр("ru = 'Занижена сумма налога, исчисленная к уменьшению';
							|en = 'Занижена сумма налога, исчисленная к уменьшению'");
...
Процедура ПроверитьКС_1_4(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Сумма налога, исчисленная к уменьшению (строка 040 Раздела 1) должна быть равна - (строка 220 Раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)) всех разделов 2 с кодами имущества = ""1"", ""2"", ""3"", ""5"", ""6"", ""8"" - MIN ((строка 220 раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)) или строка 260 Раздела 2) Раздела 2 с кодом вида имущества = ""4""; строка 030 Раздела 1 должна быть не заполнена, если
	                          |- присутствует Раздел 2 с кодом имущества = ""4"";
	                          |- (строка 220 Раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)) всех разделов 2 с кодами имущества = ""1"", ""2"", ""3"", ""5"", ""6"", ""8"" с соответствующими кодами ОКТМО и КБК - MIN ((строка 220 раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)) или строка 260 Раздела 2) Раздела 2 с кодом вида имущества = ""4"" < 0';
	                          |en = 'Сумма налога, исчисленная к уменьшению (строка 040 Раздела 1) должна быть равна - (строка 220 Раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)) всех разделов 2 с кодами имущества = ""1"", ""2"", ""3"", ""5"", ""6"", ""8"" - MIN ((строка 220 раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)) или строка 260 Раздела 2) Раздела 2 с кодом вида имущества = ""4""; строка 030 Раздела 1 должна быть не заполнена, если
	                          |- присутствует Раздел 2 с кодом имущества = ""4"";
	                          |- (строка 220 Раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)) всех разделов 2 с кодами имущества = ""1"", ""2"", ""3"", ""5"", ""6"", ""8"" с соответствующими кодами ОКТМО и КБК - MIN ((строка 220 раздела 2 - (строка 230 Раздела 2 + строка 250 Раздела 2)) или строка 260 Раздела 2) Раздела 2 с кодом вида имущества = ""4"" < 0'");
		...
		Для Каждого СтраницаР2 Из Раздел2 Цикл
			...
			Если Р2Стр001.Значение = "4" Тогда
				...
				КонтрольноеЗначениеВч = КонтрольноеЗначениеВч
				                    + Мин(Р2Стр220.Значение - (Р2Стр230.Значение + Р2Стр250.Значение), Р2Стр260.Значение)
			ИначеЕсли ЗначениеСодержитсяВСписке(Р2Стр001.Значение, "1, 2, 3, 5, 6, 8") Тогда
				КонтрольноеЗначениеУм = КонтрольноеЗначениеУм + Р2Стр220.Значение
				                    - (Р2Стр230.Значение + Р2Стр250.Значение);
			КонецЕсли;
		КонецЦикла;
		КонтрольноеЗначение = КонтрольноеЗначениеУм - КонтрольноеЗначениеВч;
...
Процедура ПроверитьКС_1_5(СтруктураРезультата)
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		КонтрольноеЗначение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, Окр(КонтрольноеЗначение, 2));
...
Процедура ПроверитьКС_1_6(СтруктураРезультата)
	...
	ОписаниеОшибки1 = НСтр("ru = 'Завышена среднегодовая стоимость необлагаемого налогом имущества за налоговый период';
							|en = 'Завышена среднегодовая стоимость необлагаемого налогом имущества за налоговый период'");
	ОписаниеОшибки2 = НСтр("ru = 'Занижена среднегодовая стоимость необлагаемого налогом имущества за налоговый период';
							|en = 'Занижена среднегодовая стоимость необлагаемого налогом имущества за налоговый период'");
...
Процедура ПроверитьКС_1_7(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Средняя стоимость необлагаемого налогом имущества за отчетный период (строка 170 Раздела 2) должна быть равна нулю, если значение строки 160 Раздела 2 равно нулю или средняя стоимость необлагаемого налогом имущества за отчетный период (строка 170 Раздела 2) должна быть отличной от нуля, если значение строки 160 Раздела 2 отлично от нуля';
							|en = 'Средняя стоимость необлагаемого налогом имущества за отчетный период (строка 170 Раздела 2) должна быть равна нулю, если значение строки 160 Раздела 2 равно нулю или средняя стоимость необлагаемого налогом имущества за отчетный период (строка 170 Раздела 2) должна быть отличной от нуля, если значение строки 160 Раздела 2 отлично от нуля'");
	
	Нарушение      = НСтр("ru = 'НК РФ ст.376 п.4';
							|en = 'НК РФ ст.376 п.4'");
	ОписаниеОшибки = НСтр("ru = 'Возможно неправомерное применение льготы по налогу';
							|en = 'Возможно неправомерное применение льготы по налогу'");
...
Процедура ПроверитьКС_1_8(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Налоговая база (строка 190 Раздела 2) должна быть равна произведению разницы строк 150 и 170 Раздела 2 и строки 180 Раздела 2, если код вида имущества (строка 001 Раздела 2) = ""2""';
							|en = 'Налоговая база (строка 190 Раздела 2) должна быть равна произведению разницы строк 150 и 170 Раздела 2 и строки 180 Раздела 2, если код вида имущества (строка 001 Раздела 2) = ""2""'");
	
	Нарушение      = НСтр("ru = 'НК РФ ст.375 п.1, НК РФ ст.376 п.2';
							|en = 'НК РФ ст.375 п.1, НК РФ ст.376 п.2'");
	ОписаниеОшибки1 = НСтр("ru = 'Завышена сумма налога';
							|en = 'Завышена сумма налога'");
	ОписаниеОшибки2 = НСтр("ru = 'Занижена сумма налога';
							|en = 'Занижена сумма налога'");
...
Процедура ПроверитьКС_1_9(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ru = 'Проверка раздела 2 (КС 1.9)';
							|en = 'Проверка раздела 2 (КС 1.9)'");
	ОписаниеКС     = НСтр("ru = 'Налоговая база (строка 190 Раздела 2) должна быть равна разнице строк 150 и 170 Раздела 2, если код вида имущества (строка 001 Раздела 2) = ""1"" или = ""3"" или = ""4"" или = ""5"" или = ""6"" или = ""8""';
							|en = 'Налоговая база (строка 190 Раздела 2) должна быть равна разнице строк 150 и 170 Раздела 2, если код вида имущества (строка 001 Раздела 2) = ""1"" или = ""3"" или = ""4"" или = ""5"" или = ""6"" или = ""8""'");
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		ПроверятьСоотношение = ЗначениеСодержитсяВСписке(Р2Стр001.Значение, "1, 3, 4, 5, 6, 8");
		Если НЕ ПроверятьСоотношение Тогда
			Продолжить;
		КонецЕсли;
		...
		Расшифровка = Р2Стр190.ЗначениеСтр + " = " + Р2Стр150.ЗначениеСтр
		            ...
		            + " Итог [" + КонтрольноеЗначение + "]"
		            + " при условии, что """ + Р2Стр001.ЗначениеСтр + """ = [""1"", ""3"", ""4"", ""5"", ""6"", ""8""]";
...
Процедура ПроверитьКС_1_10(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Сумма налога за налоговый период (строка 220 Раздела 2) должна быть равна произведению строк 190 и 210 Раздела 2 деленое на 100';
							|en = 'Сумма налога за налоговый период (строка 220 Раздела 2) должна быть равна произведению строк 190 и 210 Раздела 2 деленое на 100'");
	
	Нарушение       = НСтр("ru = 'НК РФ ст.382 п.1, НК РФ ст.383 п.4';
							|en = 'НК РФ ст.382 п.1, НК РФ ст.383 п.4'");
	ОписаниеОшибки1 = НСтр("ru = 'Завышена сумма налога';
							|en = 'Завышена сумма налога'");
	ОписаниеОшибки2 = НСтр("ru = 'Занижена сумма налога';
							|en = 'Занижена сумма налога'");
...
Процедура ПроверитьКС_1_11(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Сумма авансовых платежей, исчисленная за отчетные периоды (строка 230 Раздела 2) должна быть равна сумме авансовых платежей по строкам 180 Раздела 2 налогового расчета по авансовому платежу за 1 квартал, полугодие, 9 месяцев теккущего налогового периода';
							|en = 'Сумма авансовых платежей, исчисленная за отчетные периоды (строка 230 Раздела 2) должна быть равна сумме авансовых платежей по строкам 180 Раздела 2 налогового расчета по авансовому платежу за 1 квартал, полугодие, 9 месяцев теккущего налогового периода'");
	
	Нарушение      = НСтр("ru = 'НК РФ ст.382 п.2 и 4';
							|en = 'НК РФ ст.382 п.2 и 4'");
	ОписаниеОшибки1 = НСтр("ru = 'Завышена сумма налога';
							|en = 'Завышена сумма налога'");
	ОписаниеОшибки2 = НСтр("ru = 'Занижена сумма налога';
							|en = 'Занижена сумма налога'");
...
Процедура ПроверитьКС_1_12(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ru = 'Проверка раздела 2 (КС 1.12)';
							|en = 'Проверка раздела 2 (КС 1.12)'");
	ОписаниеКС     = НСтр("ru = 'Сумма налоговой льготы, уменьшающей сумму налога, подлежащую уплате в бюджет (строка 250 Раздела 2) должна быть не равна нулю, если значение строки 240 Раздела 2 равно ""';
							|en = 'Сумма налоговой льготы, уменьшающей сумму налога, подлежащую уплате в бюджет (строка 250 Раздела 2) должна быть не равна нулю, если значение строки 240 Раздела 2 равно ""'") + РазрешенныйКодЛьготы + """";
...
Процедура ПроверитьКС_1_15(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ru = 'Проверка раздела 2 (КС 1.15)';
							|en = 'Проверка раздела 2 (КС 1.15)'");
	ОписаниеКС     = НСтр("ru = 'Код налоговой льготы, установленной в виде понижения налоговой ставки (строка 200 Раздела 2) должен быть равен ';
							|en = 'Код налоговой льготы, установленной в виде понижения налоговой ставки (строка 200 Раздела 2) должен быть равен '") + РазрешенныйКодЛьготы + ", если такая льгота указана";
	
	Нарушение      = НСтр("ru = 'НК РФ ст. 372, п. 2';
							|en = 'НК РФ ст. 372, п. 2'");
	ОписаниеОшибки = НСтр("ru = 'Возможно неправомерное применение льгот';
							|en = 'Возможно неправомерное применение льгот'");
...
Процедура ПроверитьКС_1_16(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ru = 'Проверка раздела 2 (КС 1.16)';
							|en = 'Проверка раздела 2 (КС 1.16)'");
	ОписаниеКС     = НСтр("ru = 'Код налоговой льготы в виде уменьшения суммы налога, подлежащей уплате в бюджет (строка 240 Раздела 2) должен быть равен ';
							|en = 'Код налоговой льготы в виде уменьшения суммы налога, подлежащей уплате в бюджет (строка 240 Раздела 2) должен быть равен '") + РазрешенныйКодЛьготы + ", если такая льгота указана";
...
Процедура ПроверитьКС_1_17(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ru = 'Проверка раздела 2 (КС 1.17)';
							|en = 'Проверка раздела 2 (КС 1.17)'");
	ОписаниеКС     = НСтр("ru = 'Сумма налоговой льготы, уменьшающей сумму налога, подлежащую уплате в бюджет (строка 250 Раздела 2) должна быть равна нулю, если строка 240 раздела не равна ""';
							|en = 'Сумма налоговой льготы, уменьшающей сумму налога, подлежащую уплате в бюджет (строка 250 Раздела 2) должна быть равна нулю, если строка 240 раздела не равна ""'") + РазрешенныйКодЛьготы + """";
...
Процедура ПроверитьКС_1_18(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Налоговая база (строка 060 раздела 3) должна быть равна разнице значений строк 030 и 035 Раздела 3, если значение строки 050 Раздела 3 = 0';
							|en = 'Налоговая база (строка 060 раздела 3) должна быть равна разнице значений строк 030 и 035 Раздела 3, если значение строки 050 Раздела 3 = 0'");
	
	Нарушение       = НСтр("ru = 'НК РФ ст.376 п.2, НК РФ ст.375 п.2';
							|en = 'НК РФ ст.376 п.2, НК РФ ст.375 п.2'");
	ОписаниеОшибки1 = НСтр("ru = 'Возможно завышение суммы налога';
							|en = 'Возможно завышение суммы налога'");
	ОписаниеОшибки2 = НСтр("ru = 'Возможно занижение суммы налога';
							|en = 'Возможно занижение суммы налога'");
...
Процедура ПроверитьКС_1_19(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Налоговая база (строка 060 раздела 3) должна быть равна произведению значения строки 050 Раздела и разницы значений строк 030 и 035 Раздела 3, если значение строки 050 Раздела 3 не равно 0';
							|en = 'Налоговая база (строка 060 раздела 3) должна быть равна произведению значения строки 050 Раздела и разницы значений строк 030 и 035 Раздела 3, если значение строки 050 Раздела 3 не равно 0'");
...
Процедура ПроверитьКС_1_20(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Налоговая база (строка 060 Раздела 3) должна быть равна (строка 020 Раздела 3 - строка 025 Раздела 3) * строка 050 Раздела 3';
							|en = 'Налоговая база (строка 060 Раздела 3) должна быть равна (строка 020 Раздела 3 - строка 025 Раздела 3) * строка 050 Раздела 3'");
	
	Нарушение       = НСтр("ru = 'НК РФ ст.376 п.2, НК РФ ст.375 п.2,3';
							|en = 'НК РФ ст.376 п.2, НК РФ ст.375 п.2,3'");
	ОписаниеОшибки1 = НСтр("ru = 'Завышена сумма налога';
							|en = 'Завышена сумма налога'");
	ОписаниеОшибки2 = НСтр("ru = 'Занижена сумма налога';
							|en = 'Занижена сумма налога'");
...
Процедура ПроверитьКС_1_21(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Сумма налога за налоговый период (строка 100 Раздела 3) должна быть равна строка 060 Раздела 3 * строка 080 Раздела 3 * строка 090 Раздела 3 / 100, при условии, что строка 090 Раздела 3 заполнена';
							|en = 'Сумма налога за налоговый период (строка 100 Раздела 3) должна быть равна строка 060 Раздела 3 * строка 080 Раздела 3 * строка 090 Раздела 3 / 100, при условии, что строка 090 Раздела 3 заполнена'");
	
	Нарушение       = НСтр("ru = 'НК РФ ст.375 п.2, НК РФ ст.382 п.5';
							|en = 'НК РФ ст.375 п.2, НК РФ ст.382 п.5'");
	ОписаниеОшибки1 = НСтр("ru = 'Завышена сумма налога';
							|en = 'Завышена сумма налога'");
	ОписаниеОшибки2 = НСтр("ru = 'Занижена сумма налога';
							|en = 'Занижена сумма налога'");
...
Процедура ПроверитьКС_1_22(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru = 'Сумма налога за налоговый период (строка 100 Раздела 3) должна быть равна строка 060 Раздела 3 х строка 080 Раздела 3 / 100, при условии, что строка 090 Раздела 3 не заполнена';
							|en = 'Сумма налога за налоговый период (строка 100 Раздела 3) должна быть равна строка 060 Раздела 3 х строка 080 Раздела 3 / 100, при условии, что строка 090 Раздела 3 не заполнена'");
...
Процедура ПроверитьКС_1_23(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ru = 'Проверка раздела 3 (КС 1.23)';
							|en = 'Проверка раздела 3 (КС 1.23)'");
	ОписаниеКС     = НСтр("ru = 'Сумма налоговой льготы, уменьшающей сумму налога, подлежащую уплате в бюджет (строка 130 Раздела 3) должна быть заполнена, если значение строки 120 Раздела 3 = ""';
							|en = 'Сумма налоговой льготы, уменьшающей сумму налога, подлежащую уплате в бюджет (строка 130 Раздела 3) должна быть заполнена, если значение строки 120 Раздела 3 = ""'") + РазрешенныйКодЛьготы + """";
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1Стр020" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Код бюджетной классификации (строка 020 раздела 1)';
												|en = 'Код бюджетной классификации (строка 020 раздела 1)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1Стр030" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма налога, подлежащая уплате в бюджет (руб.) (строка 030 раздела 1)';
												|en = 'Сумма налога, подлежащая уплате в бюджет (руб.) (строка 030 раздела 1)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1Стр040" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма налога, исчисленная к уменьшению (руб.) (строка 040 раздела 1)';
												|en = 'Сумма налога, исчисленная к уменьшению (руб.) (строка 040 раздела 1)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр001" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Код вида имущества (строка 001%1 раздела 2)';
												|en = 'Код вида имущества (строка 001%1 раздела 2)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр010" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Код по ОКТМО (строка 010%1 раздела 2)';
												|en = 'Код по ОКТМО (строка 010%1 раздела 2)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр020Гр3" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.01 (строка 020 графа 3%1 раздела 2)';
												|en = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.01 (строка 020 графа 3%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр030Гр3" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.02 (строка 030 графа 3%1 раздела 2)';
												|en = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.02 (строка 030 графа 3%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр040Гр3" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.03 (строка 040 графа 3%1 раздела 2)';
												|en = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.03 (строка 040 графа 3%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр050Гр3" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.04 (строка 050 графа 3%1 раздела 2)';
												|en = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.04 (строка 050 графа 3%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр060Гр3" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.05 (строка 060 графа 3%1 раздела 2)';
												|en = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.05 (строка 060 графа 3%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070Гр3" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.06 (строка 070 графа 3%1 раздела 2)';
												|en = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.06 (строка 070 графа 3%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр080Гр3" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.07 (строка 080 графа 3%1 раздела 2)';
												|en = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.07 (строка 080 графа 3%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр090Гр3" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.08 (строка 090 графа 3%1 раздела 2)';
												|en = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.08 (строка 090 графа 3%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр100Гр3" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.09 (строка 100 графа 3%1 раздела 2)';
												|en = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.09 (строка 100 графа 3%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр110Гр3" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.10 (строка 110 графа 3%1 раздела 2)';
												|en = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.10 (строка 110 графа 3%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр120Гр3" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.11 (строка 120 графа 3%1 раздела 2)';
												|en = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.11 (строка 120 графа 3%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр130Гр3" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.12 (строка 130 графа 3%1 раздела 2)';
												|en = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 01.12 (строка 130 графа 3%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр140Гр3" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 31.12 (строка 140 графа 3%1 раздела 2)';
												|en = 'Остаточная стоимость основных средств, признаваемых объектом налогообложения по состоянию на 31.12 (строка 140 графа 3%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр020Гр4" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость льготируемого имущества по состоянию на 01.01 (строка 020 графа 4%1 раздела 2)';
												|en = 'Остаточная стоимость льготируемого имущества по состоянию на 01.01 (строка 020 графа 4%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр030Гр4" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость льготируемого имущества по состоянию на 01.02 (строка 030 графа 4%1 раздела 2)';
												|en = 'Остаточная стоимость льготируемого имущества по состоянию на 01.02 (строка 030 графа 4%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр040Гр4" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость льготируемого имущества по состоянию на 01.03 (строка 040 графа 4%1 раздела 2)';
												|en = 'Остаточная стоимость льготируемого имущества по состоянию на 01.03 (строка 040 графа 4%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр050Гр4" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость льготируемого имущества по состоянию на 01.04 (строка 050 графа 4%1 раздела 2)';
												|en = 'Остаточная стоимость льготируемого имущества по состоянию на 01.04 (строка 050 графа 4%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр060Гр4" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость льготируемого имущества по состоянию на 01.05 (строка 060 графа 4%1 раздела 2)';
												|en = 'Остаточная стоимость льготируемого имущества по состоянию на 01.05 (строка 060 графа 4%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070Гр4" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость льготируемого имущества по состоянию на 01.06 (строка 070 графа 4%1 раздела 2)';
												|en = 'Остаточная стоимость льготируемого имущества по состоянию на 01.06 (строка 070 графа 4%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр080Гр4" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость льготируемого имущества по состоянию на 01.07 (строка 080 графа 4%1 раздела 2)';
												|en = 'Остаточная стоимость льготируемого имущества по состоянию на 01.07 (строка 080 графа 4%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр090Гр4" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость льготируемого имущества по состоянию на 01.08 (строка 090 графа 4%1 раздела 2)';
												|en = 'Остаточная стоимость льготируемого имущества по состоянию на 01.08 (строка 090 графа 4%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр100Гр4" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость льготируемого имущества по состоянию на 01.09 (строка 100 графа 4%1 раздела 2)';
												|en = 'Остаточная стоимость льготируемого имущества по состоянию на 01.09 (строка 100 графа 4%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр110Гр4" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость льготируемого имущества по состоянию на 01.10 (строка 110 графа 4%1 раздела 2)';
												|en = 'Остаточная стоимость льготируемого имущества по состоянию на 01.10 (строка 110 графа 4%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр120Гр4" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость льготируемого имущества по состоянию на 01.11 (строка 120 графа 4%1 раздела 2)';
												|en = 'Остаточная стоимость льготируемого имущества по состоянию на 01.11 (строка 120 графа 4%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр130Гр4" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость льготируемого имущества по состоянию на 01.12 (строка 130 графа 4%1 раздела 2)';
												|en = 'Остаточная стоимость льготируемого имущества по состоянию на 01.12 (строка 130 графа 4%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр140Гр4" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Остаточная стоимость льготируемого имущества по состоянию на 31.12 (строка 140 графа 4%1 раздела 2)';
												|en = 'Остаточная стоимость льготируемого имущества по состоянию на 31.12 (строка 140 графа 4%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр150" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Среднегодовая стоимость имущества за налоговый период (строка 150%1 раздела 2)';
												|en = 'Среднегодовая стоимость имущества за налоговый период (строка 150%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр160" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Код налоговой льготы (строка 160%1 раздела 2)';
												|en = 'Код налоговой льготы (строка 160%1 раздела 2)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр170" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Среднегодовая стоимость необлагаемого налогом имущества за налоговый период (строка 170%1 раздела 2)';
												|en = 'Среднегодовая стоимость необлагаемого налогом имущества за налоговый период (строка 170%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр180ч" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Доля балансовой стоимости объекта недвижимого имущества на территории соответствующего субъекта Российской Федерации (числитель) (строка 180%1 раздела 2)';
												|en = 'Доля балансовой стоимости объекта недвижимого имущества на территории соответствующего субъекта Российской Федерации (числитель) (строка 180%1 раздела 2)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр180з" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Доля балансовой стоимости объекта недвижимого имущества на территории соответствующего субъекта Российской Федерации (знаменатель) (строка 180%1 раздела 2)';
												|en = 'Доля балансовой стоимости объекта недвижимого имущества на территории соответствующего субъекта Российской Федерации (знаменатель) (строка 180%1 раздела 2)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр190" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Налоговая база (строка 190%1 раздела 2)';
												|en = 'Налоговая база (строка 190%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр200" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Код налоговой льготы (установленной в виде понижения налоговой ставки) (строка 200%1 раздела 2)';
												|en = 'Код налоговой льготы (установленной в виде понижения налоговой ставки) (строка 200%1 раздела 2)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр210" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Налоговая ставка (%) (строка 210%1 раздела 2)';
												|en = 'Налоговая ставка (%) (строка 210%1 раздела 2)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр220" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма налога за налоговый период (строка 220%1 раздела 2)';
												|en = 'Сумма налога за налоговый период (строка 220%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр230" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма авансовых платежей, исчисленная за отчетные периоды (строка 230%1 раздела 2)';
												|en = 'Сумма авансовых платежей, исчисленная за отчетные периоды (строка 230%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр240" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Код налоговой льготы (в виде уменьшения суммы налога, подлежащей уплате в бюджет) (строка 240%1 раздела 2)';
												|en = 'Код налоговой льготы (в виде уменьшения суммы налога, подлежащей уплате в бюджет) (строка 240%1 раздела 2)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр250" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма налоговой льготы, уменьшающей сумму налога, подлежащую уплате в бюджет (строка 250%1 раздела 2)';
												|en = 'Сумма налоговой льготы, уменьшающей сумму налога, подлежащую уплате в бюджет (строка 250%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр260" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма налога, уплаченная за пределами Российской Федерации (строка 260%1 раздела 2)';
												|en = 'Сумма налога, уплаченная за пределами Российской Федерации (строка 260%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр010" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Код по ОКТМО (строка 010%1 раздела 3)';
												|en = 'Код по ОКТМО (строка 010%1 раздела 3)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр020" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Кадастровая стоимость на 1 января налогового периода (строка 020%1 раздела 3)';
												|en = 'Кадастровая стоимость на 1 января налогового периода (строка 020%1 раздела 3)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр025" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'в том числе необлагаемая налогом кадастровая стоимость (строка 025%1 раздела 3)';
												|en = 'в том числе необлагаемая налогом кадастровая стоимость (строка 025%1 раздела 3)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр030" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Инвентаризационная стоимость на 1 января налогового периода (строка 030%1 раздела 3)';
												|en = 'Инвентаризационная стоимость на 1 января налогового периода (строка 030%1 раздела 3)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр035" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'в том числе необлагаемая налогом инвентаризационная стоимость (строка 035%1 раздела 3)';
												|en = 'в том числе необлагаемая налогом инвентаризационная стоимость (строка 035%1 раздела 3)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр050ч" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Доля стоимости объекта недвижимого имущества на территории субъекта Российской Федерации (числитель) (строка 050%1 раздела 3)';
												|en = 'Доля стоимости объекта недвижимого имущества на территории субъекта Российской Федерации (числитель) (строка 050%1 раздела 3)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр050з" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Доля стоимости объекта недвижимого имущества на территории субъекта Российской Федерации (знаменатель) (строка 050%1 раздела 3)';
												|en = 'Доля стоимости объекта недвижимого имущества на территории субъекта Российской Федерации (знаменатель) (строка 050%1 раздела 3)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр060" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Налоговая база (строка 060%1 раздела 3)';
												|en = 'Налоговая база (строка 060%1 раздела 3)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр080" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Налоговая ставка (%) (строка 080%1 раздела 3)';
												|en = 'Налоговая ставка (%) (строка 080%1 раздела 3)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр090ч" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Коэффициент К (числитель) (строка 090%1 раздела 3)';
												|en = 'Коэффициент К (числитель) (строка 090%1 раздела 3)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр090з" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Коэффициент К (знаменатель) (строка 090%1 раздела 3)';
												|en = 'Коэффициент К (знаменатель) (строка 090%1 раздела 3)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр100" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма налога за налоговый период (строка 100%1 раздела 3)';
												|en = 'Сумма налога за налоговый период (строка 100%1 раздела 3)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр110" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма авансовых платежей, исчисленная за отчетные периоды (строка 110%1 раздела 3)';
												|en = 'Сумма авансовых платежей, исчисленная за отчетные периоды (строка 110%1 раздела 3)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр120" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Код налоговой льготы (в виде уменьшения суммы налога, подлежащей уплате в бюджет) (строка 120%1 раздела 2)';
												|en = 'Код налоговой льготы (в виде уменьшения суммы налога, подлежащей уплате в бюджет) (строка 120%1 раздела 2)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр130" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru = 'Сумма налоговой льготы, уменьшающей сумму налога, подлежащую уплате в бюджет (строка 130%1 раздела 3)';
												|en = 'Сумма налоговой льготы, уменьшающей сумму налога, подлежащую уплате в бюджет (строка 130%1 раздела 3)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
