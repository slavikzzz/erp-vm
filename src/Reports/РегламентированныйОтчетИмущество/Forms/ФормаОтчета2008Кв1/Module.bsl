...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ИмяФормы).ПолучитьМакет("Списки2008Кв1");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	Если ПараметрыВыгрузки.ПериодОтч <> "34" Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Значение кода налогового периода должно быть равно ""34""!", "Титульный", "НаимОрг");
		СтатусПроверки = Ложь;
	КонецЕсли;
		...
		Для ИндексБлока = 1 По 7 Цикл
			...
			Если ЗначениеЗаполнено(ДанныеЛиста["П000010001000_" + ИндексБлока])
				...
				Если ДлинаОКАТО <> 11 Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "В разделе 1 на странице " + ИндексЛиста + " не" + ?(ДлинаОКАТО = 0, "", " верно") + " задан код по ОКАТО!", "Раздел1", ИмяОбластиОКАТО, ИндексЛиста);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ДлинаКБК = СтрДлина(КБК);
				Если ДлинаКБК <> 20 Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "В разделе 1 на странице " + ИндексЛиста + " не" + ?(ДлинаКБК = 0, "", " верно") + " задан КБК!", "Раздел1", ИмяОбластиКБК, ИндексЛиста);
					СтатусПроверки = Ложь;
				КонецЕсли;
	...
	Для Каждого Лист2 Из Раздел2 Цикл
		...
		Если КодВидаИмущества <> "1" И КодВидаИмущества <> "2" И КодВидаИмущества <> "3" И КодВидаИмущества <> "4" Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 2 код вида имущества должен быть установлен в значение ""1"", ""2"", ""3"" или ""4""!", "Раздел2", "П000020000100", НомерЛиста);
		...
		Если НЕ ОшибкаКодаВидаИмущества И НЕ ОшибкаОКАТО
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Для значений кода вида имущества и кода по ОКАТО на странице " + НомерЛиста + " раздела 2 не найдены соответствующие значения кода по ОКАТО и КБК в разделе 1!", "Раздел2", "П000020000100", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
	...
	Для Каждого Лист3 Из Раздел3 Цикл
		...
		ИначеЕсли ОКАТОКБКРаздела1.НайтиСтроки(Новый Структура("ОКАТО, КБК", ОКАТО, СоответствиеКодаВидаИмуществаКБК["2"])).Количество() = 0 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Для значения кода по ОКАТО на странице " + НомерЛиста + " раздела 3 не найдены соответствующие значения кода по ОКАТО и КБК = """ + СоответствиеКодаВидаИмуществаКБК["2"] + """ в разделе 1!", "Раздел3", "П000030001000", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ДлинаКодаЛьготы <> 0 И ДлинаКодаЛьготы <> 7 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 неверно задан код налоговой льготы!", "Раздел3", "П000030004003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ДлинаКодаЛьготы <> 0 И ДлинаКодаЛьготы <> 7 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 неверно задан код налоговой льготы (установленной в виде понижения налоговой ставки)!", "Раздел3", "П000030007003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ДлинаКодаЛьготы <> 0 И ДлинаКодаЛьготы <> 7 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "На странице " + НомерЛиста + " раздела 3 неверно задан код налоговой льготы (в виде уменьшения суммы налога, подлежащей уплате в бюджет)!", "Раздел3", "П000030012003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
