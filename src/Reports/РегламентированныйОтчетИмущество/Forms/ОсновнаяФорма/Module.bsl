...
Функция ПолучитьСрокПредставленияОтчетности(Форма)
	...
	Если ГодПериода >= 2023 Тогда
		ДатаСрокаПредставления = Дата(ГодПериода + 1, 2, 25);
	Иначе
		ДатаСрокаПредставления = Дата(ГодПериода + 1, 3, ?(ГодПериода = 2022, 25, 30));
	КонецЕсли;
...
Процедура ВыбратьФорму(Команда)
	...
	ЭтотОбъект, ОписаниеОповещения, ДопТекстОписания);
