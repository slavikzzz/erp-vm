	...
	Если ИмяПоказателя = "П000030009003" Тогда
		Если ЗначПоказателя > 1 Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Коэффициент Кв не должен превышать 1. Значение очищено';
															|en = 'Коэффициент Кв не должен превышать 1. Значение очищено'"));
		КонецЕсли;
	КонецЕсли;
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = ИзвлечьСтруктуруXML("СхемаВыгрузки507");
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревоДляЗагрузки = ИзвлечьСтруктуруXML("СхемаВыгрузки507");
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	ДеревоВыгрузки = ИзвлечьСтруктуруXML("СхемаВыгрузки507");
...
Процедура СформироватьСтруктуруДанныхКС()
	...
	Т21.Колонки.Добавить("СЗПК"		, Новый ОписаниеТипов("Строка",,К01));
	...
	Для каждого Элемент Из Раздел21 Цикл
		...
		СтраницаР21 = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(Элемент.Данные);
		СтраницаР21.Вставить("Страница", НомерСтраницы);
		ДанныеКС.Р21.Добавить(СтраницаР21);
			
		НоваяСтрока = ДанныеКС.Т21.Добавить();
		НоваяСтрока.СЗПК		= СокрЛП(Элемент.Данные.П000210000200);
...
Процедура ПроверкаКС_1_13()
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = ' 2012500, 2010501, 2010502, 2010503, 2010504, 2010505, 2010506, 2010507, 2010508';
			|en = ' 2012500, 2010501, 2010502, 2010503, 2010504, 2010505, 2010506, 2010507, 2010508'"));
...
Процедура ПроверкаКС_1_35()
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = '2010501, 2010502, 2010503, 2010504, 2010505, 2010506, 2010507, 2010508';
			|en = '2010501, 2010502, 2010503, 2010504, 2010505, 2010506, 2010507, 2010508'"));
	Заголовки.Вставить("ОписаниеОшибки"	, НСтр("ru = 'Ошибочный код налоговой льготы';
													|en = 'Ошибочный код налоговой льготы'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1 Раздела 3';
													|en = 'Страница %1 Раздела 3'"));
...
Процедура ПроверкаКС_2_01_доп()
	...
	Заголовки.Вставить("ОписаниеКС"			, НСтр("ru = 'Значение признака налогоплательщика в стр.005 Раздела 1 ';
														|en = 'Значение признака налогоплательщика в стр.005 Раздела 1 '") 
		+ НСтр("ru = 'должно соответствовать списку допустимых значений';
				|en = 'должно соответствовать списку допустимых значений'"));
...
Процедура ОтправитьВКонтролирующийОрган(Команда)
	...
	Если НомерКорректировки = 0
		...
		ПоказатьПредупреждение(, НСтр("ru = 'С 01.01.2021 не представляется первичная декларация по форме, утвержденной приказом ФНС России от 28.07.2020 № ЕД-7-21/475@, за расчетный период 2020 год';
										|en = 'С 01.01.2021 не представляется первичная декларация по форме, утвержденной приказом ФНС России от 28.07.2020 № ЕД-7-21/475@, за расчетный период 2020 год'"));
		Возврат;
	КонецЕсли;
