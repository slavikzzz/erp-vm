...
Процедура ПечатьНаСервере_Расчет(ДП)
	...
	Для Каждого Стр Из ТаблицаСтраницыРасчет Цикл
		...
		Если Данные.Свойство("КодКБКОтходы") И Данные.КодКБКОтходы = "04811201042016000120" Тогда 
			КБКСбрКод = "2";
		Иначе
			КБКСбрКод = "1";
		КонецЕсли;
		...
		Для Инд = 1 По КолСтраницВсего Цикл
			...
			Если нСтр.Области.Найти("П024_93") <> Неопределено Тогда 
				нСтр.Области.Найти("П024_93").Текст = КБКСбрКод;
			КонецЕсли;
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(нСтр, "Декларация");
		КонецЦикла;
...
Процедура ЗаполнитьПоОКТМО(ОКТМО)
	...
	ТабличныйДокумент.Области.П124.Значение = Данные.Разделы3.П000030002025;
...
Функция СобратьДанныеПоОКТМО(ОКТМО)
	...
	СводныеДанные.Вставить("Разделы3", Новый Структура("П000030002023, П000030002024, П000050002025, П000030002025"));
	Для Каждого Стр Из ТаблицаСтраницыРаздела3 Цикл
