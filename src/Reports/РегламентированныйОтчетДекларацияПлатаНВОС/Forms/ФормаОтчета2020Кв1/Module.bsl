...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КолКолонокИдГруппы3     = 26;
	КолКолонокИдГруппы4     = 21;
	КолКолонокИдГруппы5     = 26;
	...
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы11,   КолИсхСтрокИдГруппы4);
	...
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы11,    КолКолонокИдГруппы4);
...
Процедура ПечатьНаСервере_Раздел3(МассивДляПроверки, ДП)
	Лист1 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2020Кв1_Раздел3ч1");
	Лист2 = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2020Кв1_Раздел3ч2");
...
Процедура ПечатьНаСервере_Раздел3_1(МассивДляПроверки, ДП)
	Лист = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2020Кв1_Раздел3_1");
...
Процедура ПечатьНаСервере_Раздел1_12(МассивДляПроверки, ДП, ИмяРаздела)
	Макет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2020Кв1_" + ИмяРаздела);
...
Процедура ПечатьНаСервере_Расчет(ДП)
	Лист = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2020Кв1_Расчет");
	...
	Для Каждого Стр Из ТаблицаСтраницыРасчет Цикл
		...
		Если Данные.Свойство("КодКБКОтходы") И Данные.КодКБКОтходы = "04811201042020000120" Тогда 
			КБКСбрКод = "2";
		Иначе
			КБКСбрКод = "1";
		КонецЕсли;
...
Процедура ПечатьНаСервере_Титульный(ДП)
	Лист = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2020Кв1_Титульный");
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ БезОткрытияФормы Тогда
		...
		ЗагрузитьПоказателиИзМакета("Гр14_Разд11_10");
		ЗагрузитьПоказателиИзМакета("Гр14_Разд11");
		...
		ЗагрузитьПоказателиИзМакета("ВВРаздел2");
		ЗагрузитьПоказателиИзМакета("КодыКБК");
		ЗагрузитьПоказателиИзМакета("Коэфф_Кнмм");
		ЗагрузитьПоказателиИзМакета("Коэфф_Кот");
		ЗагрузитьПоказателиИзМакета("Коэфф_Кво");
		ЗагрузитьПоказателиИзМакета("Коэфф_Квр");
		ЗагрузитьПоказателиИзМакета("Коэфф_Ксл");
		ЗагрузитьПоказателиИзМакета("Коэфф_Кл");
		ЗагрузитьПоказателиИзМакета("Коэфф_Кисп");
		ЗагрузитьПоказателиИзМакета("Коэфф_Код");
	КонецЕсли;
	...
	Если ТекОбласть <> Неопределено Тогда	
		...
		Для Ном = ТекОбласть.Верх По ТекОбласть.Низ Цикл
			...
			Иначе
				...
				ИначеЕсли РазделВМакете = "Гр14_Разд11_10" Тогда
					НовСтрока = мГр14_Разд11_10.Добавить();
				ИначеЕсли РазделВМакете = "Гр14_Разд11" Тогда
					НовСтрока = мГр14_Разд11.Добавить();
				...
				ИначеЕсли РазделВМакете = "ВВРаздел2" Тогда
					НовСтрока = мВВРаздел2.Добавить();
				ИначеЕсли РазделВМакете = "КодыКБК" Тогда
					НовСтрока = КодыКБК.Добавить();
				ИначеЕсли РазделВМакете = "Коэфф_Кнмм" Тогда
					НовСтрока = мКоэфф_Кнмм.Добавить();
				ИначеЕсли РазделВМакете = "Коэфф_Кот" Тогда
					НовСтрока = мКоэфф_Кот.Добавить();
				ИначеЕсли РазделВМакете = "Коэфф_Кво" Тогда
					НовСтрока = мКоэфф_Кво.Добавить();
				ИначеЕсли РазделВМакете = "Коэфф_Квр" Тогда
					НовСтрока = мКоэфф_Квр.Добавить();
				ИначеЕсли РазделВМакете = "Коэфф_Ксл" Тогда
					НовСтрока = мКоэфф_Ксл.Добавить();
				ИначеЕсли РазделВМакете = "Коэфф_Кл" Тогда
					НовСтрока = мКоэфф_Кл.Добавить();
				ИначеЕсли РазделВМакете = "Коэфф_Кисп" Тогда
					НовСтрока = мКоэфф_Кисп.Добавить();
				ИначеЕсли РазделВМакете = "Коэфф_Код" Тогда
					НовСтрока = мКоэфф_Код.Добавить();
				КонецЕсли;
...
Процедура ДобавитьФайлЗавершение(Результат, АдресФайла, ВыбИмяФайла, Парам) Экспорт
	...
	Если НЕ (ВРег(Прав(ВыбИмяФайла, 4)) = ".JPEG"
		...
		ТекстПредупреждения = НСтр(
			...
			|en = 'Файл приложения должен иметь одно из допустимых расширений: JPEG, PDF, JPG!'");
	КонецЕсли;
...
Процедура СохранитьНаДискФайл(ИмяОбласти)
	...
	Попытка
		Если Не ЗначениеЗаполнено(СтруктураХранения.ИмяФайла) Тогда 
			Возврат;
		КонецЕсли;
		НачатьПолучениеФайлаССервера(СтруктураХранения.ХранилищеФайла, 
			СтруктураХранения.ИмяФайла, Новый ПараметрыДиалогаПолученияФайлов("Выберете папку для сохранения", Истина));
	Исключение
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли (СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1_1") И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
		...
		Если СтрНачинаетсяС(ОбластьИмя, "П1100010_") Или СтрНачинаетсяС(ОбластьИмя, "П2000010_") Тогда 
			Таб = мГр10_Разд11_10;
		ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П1100011_") Тогда 
			Таб = мКоэфф_Квр;
		ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П1100012_") Тогда 
			Таб = мГр12_Разд11_10;
		ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П1100013_") Тогда 
			Таб = мГр13_Разд11_10;
		ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П1100014_") Тогда 
			Таб = мГр14_Разд11;
		ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П1100015_") Тогда 
			Таб = мГр14_Разд11_10;
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1_2" И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
		...
		ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П1100007_") Тогда 
			Таб = мКоэфф_Кот;
		ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П1100008_") Тогда 
			Таб = мГр14_Разд11_10;
		ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П1100002_") Тогда
			ИмяПроцедуры = "ВводНазванияЗавершение";
			Таб = мВВРаздел1;
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел2" И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
		...
		ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П1100010_") Или СтрНачинаетсяС(ОбластьИмя, "П2000010_") Тогда 
			Таб = мГр10_Разд11_10;
		ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П1100016_") Или СтрНачинаетсяС(ОбластьИмя, "П2000016_") Тогда 
			Таб = мГр14_Разд11_10;
		ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П1100015_") Или СтрНачинаетсяС(ОбластьИмя, "П2000015_") Тогда 
			Таб = мКоэфф_Кво;
		ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П1100002_") Или СтрНачинаетсяС(ОбластьИмя, "П2000002_") Тогда 
			Таб = мВВРаздел2;
			ТолькоНазвание = Истина;
		Иначе
			Возврат;
		КонецЕсли;
		...
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура("Название", Неопределено));
		ДополнительныеПараметры = Новый Структура("Элемент, Область", Элемент, Область);
		Если ТолькоНазвание Тогда 
			ДополнительныеПараметры.Вставить("ТолькоНазвание");
		КонецЕсли;
	...
	ИначеЕсли (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) И СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел3_1" Тогда 
		Если (СтрНачинаетсяС(Область.Имя, "П000040001002_") Или СтрНачинаетсяС(Область.Имя, "П000040001003_"))
			Или (СтрНачинаетсяС(Область.Имя, "П000060001002_") Или СтрНачинаетсяС(Область.Имя, "П000060001003_")) Тогда 
		...
		Иначе
			ОбластьИмя = Область.Имя;
			Если СтрНачинаетсяС(ОбластьИмя, "П000040001017_") Или СтрНачинаетсяС(ОбластьИмя, "П000060001017_") Тогда 
				Таб = мГр14_Разд11_10;
			ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П000040001004_") Или СтрНачинаетсяС(ОбластьИмя, "П000060001004_") Тогда 
				Таб = мГр4_Разд3;
			ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П000040001016_") Или СтрНачинаетсяС(ОбластьИмя, "П000060001016_") Тогда 
				Таб = мГр13_Разд11_10;
			ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П000040001015_") Или СтрНачинаетсяС(ОбластьИмя, "П000060001015_") Тогда 
				Таб = мГр21_Разд3;
			ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П000040001014_") Или СтрНачинаетсяС(ОбластьИмя, "П000060001014_") Тогда 
				Таб = мГр17_Разд3;
			ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П000040001013_") Или СтрНачинаетсяС(ОбластьИмя, "П000060001013_") Тогда 
				Таб = мКоэфф_Ксл;
			ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П000040001012_") Или СтрНачинаетсяС(ОбластьИмя, "П000060001012_") Тогда 
				Таб = мКоэфф_Кл;
			Иначе
				Возврат;
			КонецЕсли;
	...
	ИначеЕсли (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) И СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел3" Тогда 
		...
		Иначе
			...
			ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П000030001022_") Или СтрНачинаетсяС(ОбластьИмя, "П000050001022_") Тогда
				Таб = мГр13_Разд11_10;
			ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П000030001023_") Или СтрНачинаетсяС(ОбластьИмя, "П000050001023_") Тогда
				Таб = мГр14_Разд11_10;
			ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П000030001018_") Или СтрНачинаетсяС(ОбластьИмя, "П000050001018_") Тогда
				Таб = мКоэфф_Ксл;
			ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П000030001016_") Или СтрНачинаетсяС(ОбластьИмя, "П000050001016_") Тогда
				Таб = мКоэфф_Кисп;
			ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П000030001019_") Или СтрНачинаетсяС(ОбластьИмя, "П000050001019_") Тогда
				Таб = мКоэфф_Код;
			ИначеЕсли СтрНачинаетсяС(ОбластьИмя, "П000030001017_") Или СтрНачинаетсяС(ОбластьИмя, "П000050001017_") Тогда
				Таб = мКоэфф_Кл;
			Иначе
				Возврат;
			КонецЕсли;
			...
			ОткрытьФорму("Обработка.ОбщиеОбъектыРеглОтчетности.Форма.ФормаВыбораЗначенияИзТаблицы", ПараметрыФормы, ЭтаФорма,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	ИначеЕсли (СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Расчет")
		И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
		
		ИмяПроцедуры = "ВводКоэффициентаЗавершение";
		ОбластьИмя = Область.Имя;
		Если ОбластьИмя = "КодКБКОтходы" Тогда 
			Таб = КодыКБК;
		Иначе
			Возврат;
		КонецЕсли;
		...
		ПервоначальноеЗначение = Область.Значение;
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Заголовок",          "Выбор кода КБК");
		ПараметрыФормы.Вставить("ТаблицаЗначений",    Таб);
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура("Название", Неопределено));
		ДополнительныеПараметры = Новый Структура("Элемент, Область", Элемент, Область);
		ОписаниеОповещения = Новый ОписаниеОповещения(ИмяПроцедуры, ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ВводИзФККОЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00006") Тогда
		ТабличныйДокумент.Области[СтрЗаменить(Область.Имя, "П000060001002_", "П000060001003_")].Значение = Результат.Код;
		ТабличныйДокумент.Области[СтрЗаменить(Область.Имя, "П000060001003_", "П000060001002_")].Значение = Результат.Название;
	Иначе 
		Возврат;
	КонецЕсли;
...
Процедура ВводКоэффициентаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Область.Значение = Результат.Код;
	Если ДополнительныеПараметры.Свойство("ТолькоНазвание") И ЗначениеЗаполнено(Результат.Код) Тогда
...
Функция ПроверитьПоказатели() Экспорт
	...
	ДеревоВыгрузки = РегламентированнаяОтчетность.ИзвлечьСтруктуруXML(ЭтаФорма.ИмяФормы, "СхемаВыгрузкиXML2020Кв1");
...
Функция УниверсальнаяВыгрузка(ТекстВыгрузки = Неопределено, ПроверятьБлокировки = Истина, ИмяФайлаВыгрузки = "", ВыгружатьСОшибками = Ложь) Экспорт
	...
	ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Выгрузка отчета не предусмотрена';
													|en = 'Выгрузка отчета не предусмотрена'"));
	Возврат Ложь;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ПредставляетсяВ) Или ПустаяСтрока(ПараметрыВыгрузки.КодПредставляетсяВ) Тогда
...
Процедура ЗаполнитьДополнительнымиДаннымиИтогиПоОКАТО(УзелРасчет, ДанныеСтраницы)
	...
	Если ДанныеСтраницы.Свойство("КодКБКОтходы") И ЗначениеЗаполнено(ДанныеСтраницы["КодКБКОтходы"]) Тогда 
		СКБК[ДанныеСтраницы["КодКБКОтходы"]] = "П124,П134,П154,П164";
	Иначе
		СКБК["04811201041016000120"] = "П124,П134,П154,П164";
	КонецЕсли;
	...
	УзелОКАТО = ПолучитьПодчиненныйЭлемент(УзелРасчет, "CP_ADV_PAYMENTS");
	СКБК.Очистить();
	СКБК["04811201010016000120"] = "П1412,П1413,П1414";
	СКБК["04811201070016000120"] = "П1422,П1423,П1424";
	СКБК["04811201030016000120"] = "П1432,П1433,П1434";
	Если ДанныеСтраницы.Свойство("КодКБКОтходы") И ЗначениеЗаполнено(ДанныеСтраницы["КодКБКОтходы"]) Тогда 
		СКБК[ДанныеСтраницы["КодКБКОтходы"]] = "П1442,П1443,П1444";
	Иначе
		СКБК["04811201041016000120"] = "П1442,П1443,П1444";
	КонецЕсли;
	
	Для Каждого КЗ Из СКБК Цикл 
		Поля = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КЗ.Значение, ",");
		Если ЗначениеЗаполнено(ДанныеСтраницы[Поля[0]]) Или ЗначениеЗаполнено(ДанныеСтраницы[Поля[1]]) Или ЗначениеЗаполнено(ДанныеСтраницы[Поля[2]]) Тогда 
			УзелОКАТОТек = НовыйУзелИзПрототипа(УзелОКАТО);
			УстановитьЗначениеЭлемента(УзелОКАТОТек, "KBK", КЗ.Ключ);
			УстановитьЗначениеЭлемента(УзелОКАТОТек, "AP_Q1", ДанныеСтраницы[Поля[0]]);
			УстановитьЗначениеЭлемента(УзелОКАТОТек, "AP_Q2", ДанныеСтраницы[Поля[1]]);
			УстановитьЗначениеЭлемента(УзелОКАТОТек, "AP_Q3", ДанныеСтраницы[Поля[2]]);
		КонецЕсли;
	КонецЦикла;
	РегламентированнаяОтчетность.УдалитьУзел(УзелОКАТО);
КонецПроцедуры
...
Процедура ЗаполнитьДополнительнымиДанными_Раздел3(Узел3, ОКАТОФильтр)
	Для Каждого Стр Из ТаблицаСтраницыРаздела3 Цикл 
		...
		Если ОКАТОФильтр = Данные.ОКТМОГруппировочный Тогда 
...
Процедура ЗаполнитьДополнительнымиДанными_Раздел3_1(Узел3_1, ОКАТОФильтр)
	Для Каждого Стр Из ТаблицаСтраницыРаздела3_1 Цикл 
		...
		Если ОКАТОФильтр = Данные.ОКТМОГруппировочный Тогда 
			...
			УстановитьЗначениеЭлемента(НовУзел, "WSP_TKO", "1");
...
Функция СформироватьСтруктуруПараметров()
	...
	СтрокаСведений = "ОКВЭД2, ОКАТО, ОКТМО, ОКПО, ОГРН, ТипНП, ДатаРегистрации, ИННЮЛ, КППЮЛ, НаимЮЛПол, АдрЮР, АдрФакт, АдрПочт, ";
	...
	ПараметрыВыгрузки.Вставить("КодПредставляетсяВ", ТабДокументТитульный.ПредставляетсяВКод);
...
Функция СформироватьСтруктуруДанныхДопСтрокСтраницы(ИмяСтраницы) Экспорт
	...
	ИначеЕсли ИмяСтраницы = "Раздел3_1" Тогда
		...
		ОбновитьСтруктуруМногострочногоРаздела(ЭтаФорма, СтруктураРеквизитовФормы.мИдГруппы11);
		...
		ПромежуточнаяТаблица = ЭтаФорма[ПромежуточнаяТаблица];
		ТаблицаДопСтрокРаздел4 = ЭтаФорма["Хранилище" + СтруктураРеквизитовФормы.мИдГруппы4].Добавить().Таблица;
		...
		ПромежуточнаяТаблица = ЭтаФорма[ПромежуточнаяТаблица];
		ТаблицаДопСтрокРаздел11 = ЭтаФорма["Хранилище" + СтруктураРеквизитовФормы.мИдГруппы11].Добавить().Таблица;
