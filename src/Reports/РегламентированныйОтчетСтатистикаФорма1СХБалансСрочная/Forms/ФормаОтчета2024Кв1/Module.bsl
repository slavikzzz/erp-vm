...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	Если Периодичность = Перечисления.Периодичность.Квартал Тогда
		ИмяМакетаАтр = "АтрибВыгрузкиXML2024Кв1_квартал";
		ИмяШаблона   = "ШаблонРосстата2024Кв1_квартал";
	Иначе
		ИмяМакетаАтр = "АтрибВыгрузкиXML2024Кв1_год";
		ИмяШаблона   = "ШаблонРосстата2024Кв1_год";
	КонецЕсли;
...
Процедура ДополнитьРеквизитыФормы()
	...
	Для Инд = 1 По 3 Цикл
		...
		ИмяДобавляемогоРеквизита = "П002001000" + Инд;
		...
		Для Гр = 1 По 4 Цикл
			...
			ИначеЕсли Гр = 2 Тогда
				ТипКолонки = Новый ОписаниеТипов("Строка");
			ИначеЕсли Гр = 3 Тогда
				ТипКолонки = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(18, 0));
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Лев(Область.Имя, 4) = "П002" И (Сред(Область.Имя, 13, 1) = "1" Или Сред(Область.Имя, 13, 1) = "4") Тогда
			ИмяСправочника = "СправочникOKATO";
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Области["П002001000102_1"].Значение = "100";
	
	Области["П002001000202_1"].Значение = "200";
	
	Области["П002001000302_1"].Значение = "300";
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-СХ (баланс)-срочная");
	...
	ВерхМногострочнойЧасти = ТабДок.Области.ОсновнаяЧасть4_3.Низ + 1;
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_4")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-СХ (баланс)-срочная");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ПустаяСтрока"));
	КонецЕсли;
	...
	ВерхМногострочнойЧасти = ТабДок.Области.ОсновнаяЧасть4_4.Низ + 1;
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_5")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-СХ (баланс)-срочная");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ПустаяСтрока"));
	КонецЕсли;
	...
	ВерхМногострочнойЧасти = ТабДок.Области.ОсновнаяЧасть4_5.Низ + 1;
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверки.Добавить(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_6"));
		Если ПечатнаяФорма.ПроверитьВывод(ТекущаяОбластьМногострочнойЧасти) Тогда
			ПечатнаяФорма.Вывести(ТекущаяОбластьМногострочнойЧасти);
		Иначе
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-СХ (баланс)-срочная");
	...
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_6"));
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_7")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-СХ (баланс)-срочная");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_7"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-СХ (баланс)-срочная");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникOKATO",
		512, , "Выбор кода ОКАТО", КоллекцияСписковВыбора["OKATO"]);
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	ОчиститьФорматыЯчеекПередВыгрузкой1СХБалансСрочная(ПараметрыВыгрузки);
...
&НаСервере
Процедура ОчиститьФорматыЯчеекПередВыгрузкой1СХБалансСрочная(ПараметрыВыгрузки)
	
	Для Каждого ЭлСтрук Из ПараметрыВыгрузки Цикл
		Если ЭлСтрук.Значение = "ЧН='³'" Тогда
			ПараметрыВыгрузки.Удалить(ЭлСтрук.Ключ);
		КонецЕсли;
	КонецЦикла;
