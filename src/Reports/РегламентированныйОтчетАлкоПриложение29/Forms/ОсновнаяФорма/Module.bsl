...
Функция ИмяОбъектаФормы()
	...
	ИмяОбъектаФормы = Лев(ФормаИмяФормы, ПозКонцаИмениОтчета - 1);
	
	ПозНачалаИмениОтчета = СтрНайти(ИмяОбъектаФормы, ".");
	
	ИмяОбъектаФормы = Сред(ИмяОбъектаФормы, ПозНачалаИмениОтчета + 1);
	
	Возврат ИмяОбъектаФормы;
...
Процедура ПоказатьПериод(Форма)
	...
	// Для периодов ранее 2015 года ссылку Изменения законадательства скрываем.
	ГодПериода = Год(Форма.мДатаКонцаПериодаОтчета);
	Форма.Элементы.ПолеСсылкаИзмененияЗаконодательства.Видимость = ГодПериода > 2014;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяОбъектаФормы = ИмяОбъектаФормы(); 
 	ЗначениеВДанныеФормы(ОбъектОтчета().ТаблицаФормОтчета(), мТаблицаФормОтчета);
	...
	ИсточникОтчета = РегламентированнаяОтчетностьВызовСервера.ИсточникОтчета(Строка(ЭтаФорма.ИмяФормы));
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
		...
		МодульОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
			ЭтаФорма,
			"БП.Отчет.РО_АлкоПриложение29.ОсновнаяФорма",
			"Отчет.РегламентированныйОтчетАлкоПриложение29",
			,
			НСтр("ru = 'Новости: Декларация об объеме винограда, использованного для производства винодельческой продукции с защищенным географическим указанием, с защищенным наименованием места происхождения и полного цикла производства дистиллятов';
				|en = 'Новости: Декларация об объеме винограда, использованного для производства винодельческой продукции с защищенным географическим указанием, с защищенным наименованием места происхождения и полного цикла производства дистиллятов'"),
			Ложь,
...
Функция ДоступныеОрганизацииОтсутствуютТекст()
	...
	Возврат НСтр(
	"ru = 'Декларацию об объеме винограда, использованного для производства винодельческой продукции с защищенным географическим указанием, 
	|с защищенным наименованием места происхождения и полного цикла производства дистиллятов
	|представляют только организации (юридические лица)(ПП РФ от 13.05.2016 г. № 411).
	|В справочнике ""Организации"" сведения об организациях отсутствуют.';
	|en = 'Декларацию об объеме винограда, использованного для производства винодельческой продукции с защищенным географическим указанием, 
	|с защищенным наименованием места происхождения и полного цикла производства дистиллятов
	|представляют только организации (юридические лица)(ПП РФ от 13.05.2016 г. № 411).
	|В справочнике ""Организации"" сведения об организациях отсутствуют.'");
