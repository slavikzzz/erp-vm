...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр р.НДС прилож.12 21к2") Тогда
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	// Группа П11000.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П1100002");
	ИменаГраф.Добавить("П1100003");
	ИменаГраф.Добавить("П1100004");
	ИменаГраф.Добавить("П1100051");
	ИменаГраф.Добавить("П1100052");
	ИменаГраф.Добавить("П1100006");
	ИменаГраф.Добавить("П1100007");
	...
	ИменаПодчиненныхГрупп.Добавить(СтруктураРеквизитовФормы.мИдГруппы112);
	СтруктураМногострочныхЧастей.Вставить(
	...
	СтруктураМногострочныхЧастей.Вставить(
		...
		ИменаГраф, 1, 2, ИменаПодчиненныхГрупп, КолонкиСИзмНижнГраницей));
	
	// Группа П11200.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П1120011");
	ИменаГраф.Добавить("П1120012");
	ИменаПодчиненныхГрупп = Новый Массив;
	КолонкиСИзмНижнГраницей = Новый Массив;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1100002", 2, ,
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 8) = "П1100006"
		ИЛИ Лев(Область.Имя, 8) = "П1100007"
		ИЛИ Лев(Область.Имя, 8) = "П1100008" Тогда
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы111 Тогда
		Подстрока = "рейс прибытия";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный рейс прибытия?';
							|en = 'Удалить выбранный рейс прибытия?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы112 Тогда
		Подстрока = "рейс убытия";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный рейс убытия?';
							|en = 'Удалить выбранный рейс убытия?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя)
				...
				Если ЗначениеЗаполнено(КодОперации) Тогда
					Ном = 1;
					Пока Форма.ТабличныйДокумент.Области.Найти("П1000014_" + Формат(Ном, "ЧГ=")) <> Неопределено Цикл
						СуммаНалБаз = СуммаНалБаз
							+ Форма.ТабличныйДокумент.Области["П1000014_" + Формат(Ном, "ЧГ=")].Значение;
						Ном = Ном + 1;
					КонецЦикла;
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если Ветка = АктивнаяВетка Тогда
				...
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					Ветка_П10000.Данные.Вставить("П1000014", Форма.ТабличныйДокумент.Области["П1000014_"
			...
			Если ЗначениеЗаполнено(КодОперации) Тогда
				ДанныеМнЧ_П10000 = Ветка.ДанныеМногострочныхЧастей.П10000.Строки;
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					СуммаНалБаз = СуммаНалБаз + Ветка_П10000.Данные["П1000014"];
				КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("НалБазаИтого", "КодОперации;П1000014");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрАвПер_12");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRAVPRVZNDS"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRAVPRVZNDS.2"));
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				ДлинаВидПрвзДок = СтрДлина(СокрЛП(Ветка_П11000.Данные["П1100002"]));
				Если ДлинаВидПрвзДок <> 2 Тогда
					ИмяПок = "П1100002" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " не" + ?(ДлинаВидПрвзДок = 0, "", "верно")
					+ " задан вид перевозочного, товаросопроводительного или иного документа",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100003"]) Тогда
					...
					+ " не задан номер перевозочного, товаросопроводительного или иного документа",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100004"]) Тогда
					...
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана дата перевозочного, товаросопроводительного или иного документа",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100051"]) Тогда
					ИмяПок = "П1100051" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование аэропорта места прибытия (убытия) воздушного судна с товаром на территорию Российской Федерации (с территории Российской Федерации)",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100052"]) Тогда
					ИмяПок = "П1100052" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задан код аэропорта места прибытия (убытия) воздушного судна с товаром на территорию Российской Федерации (с территории Российской Федерации)",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П11000.Данные["П1100013"]) Тогда
					ИмяПок = "П1100013" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана дата отметки российского таможенного органа",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ДлинаСтранАПОтпр = СтрДлина(СокрЛП(Ветка_П11000.Данные["П1100006"]));
				Если ДлинаСтранАПОтпр <> 2 Тогда
					...
					+ " задан код страны аэропорта отправления",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ДлинаСтранАППриб = СтрДлина(СокрЛП(Ветка_П11000.Данные["П1100007"]));
				Если ДлинаСтранАППриб <> 2 Тогда
					ИмяПок = "П1100007" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
					+ НомПП + " не" + ?(ДлинаСтранАППриб = 0, "", "верно")
					+ " задан код страны аэропорта прибытия/убытия",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ДлинаСтранАПНазн = СтрДлина(СокрЛП(Ветка_П11000.Данные["П1100008"]));
				Если ДлинаСтранАПНазн <> 2 Тогда
					...
					+ " задан код страны аэропорта назначения",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Для каждого Ветка_П11100 Из ДанныеМнЧ_П11100 Цикл
					...
					Если НЕ ЗначениеЗаполнено(Ветка_П11100.Данные["П1110009"]) Тогда
						ИмяПок = "П1110009" + "_" + НомерСтрокиП10000_Строка
						+ "_" + НомерСтрокиП11000_Строка + "_" + НомерСтрокиП11100_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомПП + " не задан номер рейса прибытия",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					Если НЕ ЗначениеЗаполнено(Ветка_П11100.Данные["П1110010"]) Тогда
						ИмяПок = "П1110010" + "_" + НомерСтрокиП10000_Строка
						+ "_" + НомерСтрокиП11000_Строка + "_" + НомерСтрокиП11100_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомПП + " не задана дата рейса прибытия",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				...
				// 11200.
				ДанныеМнЧ_П11200 = Ветка_П11000.ДанныеМногострочныхЧастей.П11200.Строки;
				Для каждого Ветка_П11200 Из ДанныеМнЧ_П11200 Цикл
					
					НомерСтрокиП11200 = ДанныеМнЧ_П11200.Индекс(Ветка_П11200) + 1;
					НомерСтрокиП11200_Строка = Формат(НомерСтрокиП11200, "ЧГ=");
					
					Если НЕ ЗначениеЗаполнено(Ветка_П11200.Данные["П1120011"]) Тогда
						ИмяПок = "П1120011" + "_" + НомерСтрокиП10000_Строка
						+ "_" + НомерСтрокиП11000_Строка + "_" + НомерСтрокиП11200_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомПП + " не задан номер рейса убытия",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					Если НЕ ЗначениеЗаполнено(Ветка_П11200.Данные["П1120012"]) Тогда
						ИмяПок = "П1120012" + "_" + НомерСтрокиП10000_Строка
						+ "_" + НомерСтрокиП11000_Строка + "_" + НомерСтрокиП11200_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
						+ НомПП + " не задана дата рейса убытия",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрАвПер_12 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрАвПер_12");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрАвПер_12 = СкопироватьУзел(УзелРодитель.Родитель, РеестрАвПер_12);
		Для каждого Эл Из ЭкзРеестрАвПер_12.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		// Узел "СвОбОпер" имеется только в схеме выгрузки "Сведений".
		// Если он не найден, значит, сейчас формируются данные "Реестра"
		// (для выгрузки первого из 2-х файлов).
		СвОбОпер = ПолучитьПодчиненныйЭлемент(ЭкзРеестрАвПер_12, "СвОбОпер");
		Если СвОбОпер = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСвОбОпер = СкопироватьУзел(ЭкзРеестрАвПер_12, СвОбОпер, ЭкзРеестрАвПер_12.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСвОбОпер.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
				...
				// 11000.
				СвТрДок = ПолучитьПодчиненныйЭлемент(ЭкзСвОбОпер, "СвТрДок");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					
					ЭкзСвТрДок = СкопироватьУзел(ЭкзСвОбОпер, СвТрДок, ЭкзСвОбОпер.Строки.Количество() - 1);
					...
					Маршрут = ПолучитьПодчиненныйЭлемент(ЭкзСвТрДок, "Маршрут");
					...
					// 11100.
					РейсПр = ПолучитьПодчиненныйЭлемент(Маршрут, "РейсПр");
					ДанныеМнЧ_П11100 = Ветка_П11000.ДанныеМногострочныхЧастей.П11100.Строки;
					Для каждого Ветка_П11100 Из ДанныеМнЧ_П11100 Цикл
						
						ЭкзРейсПр = СкопироватьУзел(Маршрут, РейсПр, Маршрут.Строки.Количество() - 1);
						Для каждого Эл Из ЭкзРейсПр.Строки Цикл
							Если Эл.Строки.Количество() = 0 Тогда
					...
					// 11200.
					РейсУб = ПолучитьПодчиненныйЭлемент(Маршрут, "РейсУб");
					ДанныеМнЧ_П11200 = Ветка_П11000.ДанныеМногострочныхЧастей.П11200.Строки;
					Для каждого Ветка_П11200 Из ДанныеМнЧ_П11200 Цикл
						
						ЭкзРейсУб = СкопироватьУзел(Маршрут, РейсУб, Маршрут.Строки.Количество() - 1);
						Для каждого Эл Из ЭкзРейсУб.Строки Цикл
							Если Эл.Строки.Количество() = 0 Тогда
								ВывестиПоказательВXML(Эл, Ветка_П11200.Данные[Эл.Ключ]);
							КонецЕсли;
						КонецЦикла;
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_12");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_12_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Для каждого Строка11 Из Строки11 Цикл
					...
					Если ЗначениеЗаполнено(Строка11.Данные.П1100051) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100051));
					КонецЕсли;
					Если ЗначениеЗаполнено(Строка11.Данные.П1100052) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100052));
					КонецЕсли;
					СтрокаДокумент.Параметры.П1100005 = СтрСоединить(МасТекЗнач, "; ");
					
					// П11100, П11200.
					//
					Строки111 = Строка11.ДанныеМногострочныхЧастей.П11100.Строки;
					Строки112 = Строка11.ДанныеМногострочныхЧастей.П11200.Строки;
					
					КолСтрок111 = Строки111.Количество();
					КолСтрок112 = Строки112.Количество();
					
					МаксКолСтрок111_112 = Макс(КолСтрок111, КолСтрок112);
					
					Для Инд = 1 По МаксКолСтрок111_112 Цикл
						...
						Если Инд <= КолСтрок111 Тогда
							СтрокаДокумент.Параметры.Заполнить(Строки111[Инд - 1].Данные);
						КонецЕсли;
						Если Инд <= КолСтрок112 Тогда
							СтрокаДокумент.Параметры.Заполнить(Строки112[Инд - 1].Данные);
						КонецЕсли;
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Реестр НДС Приложение 12");
...
#Область ОбщиеДанные
	...
	ДанныеКС.Вставить("ГрафаНалоговойБазы"		, "14");
...
&НаСервере
Процедура ПроверкаКС_12_1_1()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '12.1.1';
													|en = '12.1.1'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 12';
														|en = 'Реестр 12'"));
...
&НаСервере
Процедура ПроверкаКС_12_1_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '12.1.2';
													|en = '12.1.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 12';
														|en = 'Реестр 12'"));
...
&НаСервере
Процедура ПроверкаКС_12_1_3а()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '12.1.3.а';
													|en = '12.1.3.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 12';
														|en = 'Реестр 12'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны аэропорта отправления в гр.6 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны аэропорта отправления в гр.6 Реестра должен соответствовать справочнику ОКСМ'"));
	...
	Настройки.Вставить("ИмяПоказателя", "П1100006_");
	ПроверкаКС_12_1_3_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_12_1_3б()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '12.1.3.б';
													|en = '12.1.3.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 12';
														|en = 'Реестр 12'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны аэропорта прибытия в гр.7 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны аэропорта прибытия в гр.7 Реестра должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Код страны %1 соответствует справочнику ОКСМ';
													|en = 'Код страны %1 соответствует справочнику ОКСМ'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный код страны';
														|en = 'Ошибочный код страны'"));
	...
	Настройки.Вставить("ИмяПоказателя", "П1100007_");
	ПроверкаКС_12_1_3_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_12_1_3в()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '12.1.3.в';
													|en = '12.1.3.в'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 12';
														|en = 'Реестр 12'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны аэропорта назначения в гр.8 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны аэропорта назначения в гр.8 Реестра должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Код страны %1 соответствует справочнику ОКСМ';
													|en = 'Код страны %1 соответствует справочнику ОКСМ'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный код страны';
														|en = 'Ошибочный код страны'"));
	...
	Настройки.Вставить("ИмяПоказателя", "П1100008_");
	ПроверкаКС_12_1_3_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_12_1_3_код(Настройки)
...
&НаСервере
Процедура ПроверкаКС_12_1_4()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '12.1.4';
													|en = '12.1.4'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 12';
														|en = 'Реестр 12'"));
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Номер авианакладной в гр.3 Реестра должен состоять из 11 цифр, если в качестве вида перевозочного ';
			|en = 'Номер авианакладной в гр.3 Реестра должен состоять из 11 цифр, если в качестве вида перевозочного '") +
		НСтр("ru = 'документа в гр.2 Реестра указана авианакладная (гр.2 Реестра = 04)';
			|en = 'документа в гр.2 Реестра указана авианакладная (гр.2 Реестра = 04)'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Номер %1 соответствует формату, когда %2 = 04';
													|en = 'Номер %1 соответствует формату, когда %2 = 04'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01Г001", "П1100003_");
	ИменаПоказателей.Вставить("Р01Г002", "П1100002_");
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			Для Счетчик2 = 1 По СтраницаРеестра["КолвоСтрокП11000_" + XMLСтрока(Счетчик)]  Цикл
				...
				Настройки.ЭтоОшибка = Показатели.Р01Г002.Значение = "04" И НЕ (СтрДлина(Показатели.Р01Г001.Значение) = 11
				...
				Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р01Г001.ЗначениеСтр,
					Показатели.Р01Г002.ЗначениеСтр);
				Настройки.ДопСведения = Дополнение;
				ДополнитьСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверкаКС_12_2_1а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '12.2.1.а';
													|en = '12.2.1.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 12';
														|en = 'Реестр 12'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_12_2_1б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '12.2.1.б';
													|en = '12.2.1.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 12';
														|en = 'Реестр 12'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_12_2_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '12.2.2';
													|en = '12.2.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 12';
														|en = 'Реестр 12'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
	...
	Для каждого СтрокаДерева Из ДанныеКС.ДеревоРеестра.Строки Цикл
		...
		Настройки.Расшифровка = СтрШаблон(Настройки.ШаблонРасшифровки, Показатели.Р01С002.Значение, 
			РасчетнаяСумма, Показатели.Р01С001.ЗначениеСтр);
		Настройки.ДопСведения = ПодставитьПараметрыВСтроку(Настройки.ШаблонДопСведений, Показатели.Р01С001.ЗначениеСтр);
		ДополнитьСтруктуруОшибок(Настройки);
