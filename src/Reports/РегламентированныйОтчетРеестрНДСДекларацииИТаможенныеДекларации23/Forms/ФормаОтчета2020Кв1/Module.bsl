...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрТДCN23");
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если НЕ (ДлинаРегНомТД >= 23 И ДлинаРегНомТД <= 29) Тогда
				...
				"", "верно") + " задан регистрационный номер таможенной декларации. Возможное количество символов: от 23 до 29",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Если КодВидДок = "2" ИЛИ ЗначениеЗаполнено(СокрЛП(Ветка_П10000.Данные["П1000004"])) Тогда
				...
				Если ДлинаИдНом <> 13 Тогда
					...
					+ " задан идентификационный номер отправления CN23" + ?(ДлинаИдНом = 0, "", ", не состоящий из 13 символов"),
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрТД23 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрТДCN23");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрТД23 = СкопироватьУзел(УзелРодитель.Родитель, РеестрТД23);
		Для каждого Эл Из ЭкзРеестрТД23.Строки Цикл
		...
		СведОперМПО = ПолучитьПодчиненныйЭлемент(ЭкзРеестрТД23, "СведОперМПО");
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведОперМПО = СкопироватьУзел(ЭкзРеестрТД23, СведОперМПО, ЭкзРеестрТД23.Строки.Количество() - 1);
