...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П1000091") Тогда
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрДТCN23");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRTDCN23"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRTDCN23.2"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузкиРеестра, ДеревоВыгрузкиСведений)
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			КодВидДок = СокрЛП(Ветка_П10000.Данные["П1000002"]);
			Если КодВидДок <> "1" И КодВидДок <> "2" Тогда
				ИмяПок = "П1000002" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не" + ?(ЗначениеЗаполнено(КодВидДок),
				"верно", "") + " задан код вида документа" + ?(ЗначениеЗаполнено(КодВидДок), ". Возможны значения: 1, 2", ""),
			...
			Если НЕ (ДлинаРегНомТД >= 23 И ДлинаРегНомТД <= 29) Тогда
				...
				"", "верно") + " задан регистрационный номер декларации на товары. Возможное количество символов: от 23 до 29",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если КодВидДок = "2" ИЛИ ЗначениеЗаполнено(СокрЛП(Ветка_П10000.Данные["П1000004"])) Тогда
				ДлинаИдНом = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000004"]));
				Если ДлинаИдНом <> 13 Тогда
					ИмяПок = "П1000004" + "_" + НомерСтрокиП10000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + ?(ДлинаИдНом = 0, " не", "")
					+ " задан номер отправления из таможенной декларации CN 23" + ?(ДлинаИдНом = 0,
					"", ", не состоящий из 13 символов"), "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000091"]) Тогда // СведФЛ
				ФИО = РегламентированнаяОтчетность.РазложитьФИО(Ветка_П10000.Данные["П1000005"]);
				...
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000006"]) Тогда
					...
					Если ДлинаИННФЛ <> 12 Тогда
						...
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " задан ИНН отправителя, не состоящий из 12 цифр", "Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
			Иначе // СведОрг
				...
				Если ЗначениеЗаполнено(Ветка_П10000.Данные["П1000006"]) Тогда
					...
					Если ДлинаИНН <> 10 Тогда
						ИмяПок = "П1000006" + "_" + НомерСтрокиП10000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " задан ИНН отправителя, не состоящий из 10 цифр", "Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрДТ23 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрДТCN23");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрДТ23 = СкопироватьУзел(УзелРодитель.Родитель, РеестрДТ23);
		Для каждого Эл Из ЭкзРеестрДТ23.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		// Узел "СведОперМПО" имеется только в схеме выгрузки "Сведений".
		// Если он не найден, значит, сейчас формируются данные "Реестра" (для выгрузки первого из 2-х файлов).
		СведОперМПО = ПолучитьПодчиненныйЭлемент(ЭкзРеестрДТ23, "СведОперМПО");
		Если СведОперМПО = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведОперМПО = СкопироватьУзел(ЭкзРеестрДТ23, СведОперМПО, ЭкзРеестрДТ23.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСведОперМПО.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
				...
				// Отправитель.
				СвОтпрМПО = ПолучитьПодчиненныйЭлемент(ЭкзСведОперМПО, "СвОтпрМПО");
				Парам = Новый Структура("Данные,ИмяОблПризнакИП,ИмяОблНаим", Ветка_П10000.Данные, "П1000091", "П1000005");
				ДобавитьУзелСведенийОбОрганизацииИлиИП(СвОтпрМПО, Парам);
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_ДекларацииИТаможенныеДекларации23");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_ДекларацииИТаможенныеДекларации23_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
