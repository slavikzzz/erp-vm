...
&НаКлиенте
Перем ПараметрыОбработчикаОжидания; // См. ДлительныеОперацииКлиент.ИнициализироватьПараметрыОбработчикаОжидания
...
Процедура ВосстановитьЗначения(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Изменения в настройках будут утеряны, продолжить?';
						|en = 'Changes in the settings will be lost. Continue?'");
...
Процедура ПередЗагрузкойВариантаНаСервере(Настройки)
	...
	Если ЭтоПредопределенныйВариант Тогда
		...
		Отчет.Регистр = КлючТекущегоВарианта;
...
#Область ОбработчикиСобытийЭлементовГруппыГруппировка
...
Процедура ГруппировкаПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элемент.ТекущиеДанные.Предопределенная И Не Элемент.ТекущийЭлемент.Имя = "ГруппировкаИспользование" И ЭтоПредопределенныйВариант Тогда
		Отказ = Истина;
	КонецЕсли;
...
Процедура ГруппировкаПередУдалением(Элемент, Отказ)
	...
	Если Элемент.ТекущиеДанные.Предопределенная И ЭтоПредопределенныйВариант Тогда
		Отказ = Истина;
	КонецЕсли;
...
Функция ПолучитьНазваниеРегистра(Вариант)
	...
	Возврат ?(ПолноеНаименование = Неопределено,Вариант.Представление,ПолноеНаименование.Наименование); 
...
Функция СформироватьОтчетНаСервере() Экспорт
	...
	БухгалтерскиеОтчеты.УстановитьТекущуюСхемуКомпоновкиДанныхОтчета(ЭтотОбъект);
...
Функция ИнициализироватьПредопределенныйВариант(Вариант = Неопределено)
	...
	Отчет.ТекущаяСхемаКомпоновкиДанных = "СхемаКомпоновкиДанных";
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	Если СокрЛП(Отчет.НазваниеРегистра) = "" Тогда
		
		Отчет.НазваниеРегистра  = НСтр("ru = 'Регистр налогового учета';
										|en = 'Tax ledger'");
		
	КонецЕсли;
	
	ЗаголовокОтчета = Отчет.НазваниеРегистра + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(Отчет.НачалоПериода, Отчет.КонецПериода);
