...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы",				"NO_IZOSVAKAL_1_121_00_05_01_01.xsd");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Пока мРаздел1.Свойство("П000100000101_" + Формат(НомерСтроки, "ЧГ=")) Цикл
		...
		ИмяЯчейкиНаимБанка = "П000100000106_" + Формат(НомерСтроки, "ЧГ=");
		ИмяЯчейкиИННБанка = "П000100000107_" + Формат(НомерСтроки, "ЧГ=");
		ИмяЯчейкиКППБанка = "П000100000108_" + Формат(НомерСтроки, "ЧГ=");
		ИмяЯчейкиБИКБанка = "П000100000109_" + Формат(НомерСтроки, "ЧГ=");
		
		ИмяЯчейкиСуммаГарант = "П000100000110_" + Формат(НомерСтроки, "ЧГ=");
		ИмяЯчейкиНомерГарантии = "П000100000111_" + Формат(НомерСтроки, "ЧГ=");
		ИмяЯчейкиДатаНачГарантии = "П000100000112_" + Формат(НомерСтроки, "ЧГ=");
		ИмяЯчейкиДатаКонГарантии = "П000100000113_" + Формат(НомерСтроки, "ЧГ=");
		...
		НамБанка = СокрЛП(мРаздел1[ИмяЯчейкиНаимБанка]);
		ИННБанка = СокрЛП(мРаздел1[ИмяЯчейкиИННБанка]);
		КППБанка = СокрЛП(мРаздел1[ИмяЯчейкиКППБанка]);
		БИКБанка = СокрЛП(мРаздел1[ИмяЯчейкиБИКБанка]);
 		
		СуммаГарант = мРаздел1[ИмяЯчейкиСуммаГарант];
		НомерГарантии = СокрЛП(мРаздел1[ИмяЯчейкиНомерГарантии]);
		ДатаНачГарантии = мРаздел1[ИмяЯчейкиДатаНачГарантии];
		ДатаКонГарантии = мРаздел1[ИмяЯчейкиДатаКонГарантии];
		
		
		НомерСтроки = НомерСтроки + 1;
		
	//	 проверяем только заполненные блоки
		Если НЕ ЗначениеЗаполнено(КодВидаСпирта) И ОбъемСпирта = 0 И НЕ ЗначениеЗаполнено(КодВидаПродукции) И СуммаАвПлатежа = 0 И НЕ ЗначениеЗаполнено(НамБанка) И НЕ  ЗначениеЗаполнено(ИННБанка) 
			И НЕ ЗначениеЗаполнено(КППБанка) И НЕ ЗначениеЗаполнено(БИКБанка) И СуммаГарант = 0 И НЕ ЗначениеЗаполнено(НомерГарантии) И НЕ ЗначениеЗаполнено(ДатаНачГарантии) И НЕ ЗначениеЗаполнено(ДатаКонГарантии) тогда 
			Продолжить;
		КонецЕсли;
		
		
		Если  НЕ ЗначениеЗаполнено(КодВидаСпирта) ИЛИ    ОбъемСпирта = 0 ИЛИ НЕ ЗначениеЗаполнено(КодВидаПродукции) ИЛИ  СуммаАвПлатежа = 0 ИЛИ НЕ ЗначениеЗаполнено(НамБанка) ИЛИ НЕ  ЗначениеЗаполнено(ИННБанка) 
			ИЛИ НЕ ЗначениеЗаполнено(КППБанка) ИЛИ НЕ ЗначениеЗаполнено(БИКБанка) ИЛИ СуммаГарант = 0 ИЛИ НЕ ЗначениеЗаполнено(НомерГарантии) ИЛИ НЕ ЗначениеЗаполнено(ДатаНачГарантии) ИЛИ НЕ ЗначениеЗаполнено(ДатаКонГарантии) тогда 
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В строке номер "+(НомерСтроки - 1)+" заполнены не все показатели!", "Раздел1", "П000100000101_" + (НомерСтроки-1), 0);
			СтатусПроверки = Ложь;
		КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	ШаблонУзелСвУплАванс = ПолучитьПодчиненныйЭлемент(УзелРодитель, "СвОсвАванс");
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(мРаздел1, "П000100000101_" + Формат(НомерСтроки, "ЧГ=")) Цикл
			...
			УзелСвОБанке = ПолучитьПодчиненныйЭлемент(УзелСвАвансы, "СведБанк");
	
		    ЗадатьЗначениеЭлемента(УзелСвОБанке, "НаимБанк", мРаздел1["П000100000106_"+Формат(НомерСтроки, "ЧГ=")]);
			ЗадатьЗначениеЭлемента(УзелСвОБанке, "ИННЮЛ_Банк", мРаздел1["П000100000107_"+Формат(НомерСтроки, "ЧГ=")]);
			ЗадатьЗначениеЭлемента(УзелСвОБанке, "КПП_Банк", мРаздел1["П000100000108_"+Формат(НомерСтроки, "ЧГ=")]);
			ЗадатьЗначениеЭлемента(УзелСвОБанке, "БИК", мРаздел1["П000100000109_"+Формат(НомерСтроки, "ЧГ=")]);
