...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ИспользоватьПартнеровКакКонтрагентов Тогда
		...
		Заголовок = НСтр("ru = 'Контактная информация по контрагентам';
						|en = 'Contact details by counterparties'");
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	Если Настройки.Получить("ВидимостьОтбора") <> Неопределено Тогда
		...
		Элементы.ПоказатьСкрытьОтбор.Заголовок = ?(ВидимостьОтбора, НСтр("ru = 'Скрыть настройки';
																		|en = 'Hide settings'"), НСтр("ru = 'Показать настройки';
																											|en = 'Show settings'"));
	КонецЕсли;
...
Функция ОтборыЗаполненыКорректно(Форма)
	...
	СообщениеВыбранныеКИКЛ   = НСтр("ru = 'Необходимо выбрать хотя бы один вид контактной информации контактного лица';
									|en = 'Select at least one contact details kind of contact person'");
	СообщениеВыбранныеРоли   = НСтр("ru = 'Необходимо выбрать хотя бы одну роль контактного лица';
									|en = 'Select at least one role of a contact person'");
...
Процедура ЗаполнитьТаблицыВыбранныхПолей()
	...
	НоваяСтрокаТаблицы(ВыбранныеПоляПартнера, НСтр("ru = 'Публичное наименование';
													|en = 'Public name'"), "НаименованиеПолное");
	Если ПолучитьФункциональнуюОпцию("ИспользоватьБизнесРегионы") Тогда
		 НоваяСтрокаТаблицы(ВыбранныеПоляПартнера, НСтр("ru = 'Бизнес-регион';
														|en = 'Business region'"), "БизнесРегион");
	КонецЕсли;
	НоваяСтрокаТаблицы(ВыбранныеПоляПартнера, НСтр("ru = 'Основной менеджер';
													|en = 'Employee in charge'"), "ОсновнойМенеджер");
	НоваяСтрокаТаблицы(ВыбранныеПоляПартнера, НСтр("ru = 'Дата регистрации';
													|en = 'Registration date'"), "ДатаРегистрации");
	Если ПравоДоступа("Просмотр", Метаданные.РегистрыСведений.ABCXYZКлассификацияКлиентов) Тогда
		НоваяСтрокаТаблицы(ВыбранныеПоляПартнера, НСтр("ru = 'ABC\XYZ классификация';
														|en = 'ABC\XYZ classification'"), "Классификация");
	КонецЕсли;
	...
	НоваяСтрокаТаблицы(РолиКЛ, НСтр("ru = '<Роль не указана>';
									|en = '<Role is not specified>'"), Справочники.РолиКонтактныхЛицПартнеров.ПустаяСсылка());
