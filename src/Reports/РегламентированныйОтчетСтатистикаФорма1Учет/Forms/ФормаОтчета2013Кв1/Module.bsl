...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	// Раздел 2.
	// Данные по строке 09 по графам 6, 7 = сумме данных по строкам 10 и 21  по соответствующим графам.
	Для НомКол = 6 По 7 Цикл
		ТекСумма = Форма.ПолеТабличногоДокументаФормаОтчета.Области["П00000200100" + Формат(НомКол, "ЧЦ=1; ЧВН=; ЧГ=")].Значение +
					Форма.ПолеТабличногоДокументаФормаОтчета.Области["П00000200210" + Формат(НомКол, "ЧЦ=1; ЧВН=; ЧГ=")].Значение;
		Форма.ПолеТабличногоДокументаФормаОтчета.Области["П00000200090" + Формат(НомКол, "ЧЦ=1; ЧВН=; ЧГ=")].Значение = ТекСумма;
	КонецЦикла;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// Данные по строке 09 по графам 6, 7 <= суммы данных по строкам (06+08) и  строкам (07+08).
	Стр06 = ПараметрыВыгрузки["П000001000605_Ч"];
	Стр07 = ПараметрыВыгрузки["П000001000705_Ч"];
	Стр08 = ПараметрыВыгрузки["П000001000805_Ч"];
	Для НомКол = 6 По 7 Цикл
		Значение09 = ПараметрыВыгрузки["П0000020009" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"];
		Если Значение09 > Стр06 + Стр08 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + НомКол + " значение в строке 09 должно быть меньше либо равно суммы значений в строках 06 и 08!",
				ИмяПоляТаблДокумента, "П0000020009" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
		Если Значение09 > Стр07 + Стр08 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + НомКол + " значение в строке 09 должно быть меньше либо равно суммы значений в строках 07 и 08!",
				ИмяПоляТаблДокумента, "П0000020009" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	
	// Данные по строке 10 должны быть больше или равны сумме данных по строкам 11-20.
	Для НомКол = 4 По 8 Цикл
		ТекЗнач = 0;
		ФормНомКол = Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч";
		Для НомСтр = 11 По 20 Цикл 
			ТекЗнач = ТекЗнач + ПараметрыВыгрузки["П00000200" + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + ФормНомКол];
		КонецЦикла;
		Значение10 = ПараметрыВыгрузки["П0000020010" + ФормНомКол];
		Если Значение10 < ТекЗнач Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + НомКол + " значение в строке 10 должно быть больше либо равно суммы значений в строках 11-20!",
				ИмяПоляТаблДокумента, "П0000020011" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	
	// Данные по строке 21 должны быть больше или равна сумме данных по строкам 22, 24-28, 30-32.
	Для НомКол = 4 По 8 Цикл
		ФормНомКол = Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч";
		ТекЗнач = ПараметрыВыгрузки["П0000020022" + ФормНомКол];
		Для НомСтр = 24 По 28 Цикл 
			ТекЗнач = ТекЗнач + ПараметрыВыгрузки["П00000200" + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + ФормНомКол];
		КонецЦикла;
		Для НомСтр = 30 По 32 Цикл 
			ТекЗнач = ТекЗнач + ПараметрыВыгрузки["П00000200" + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=") + ФормНомКол];
		КонецЦикла;

		Значение21 = ПараметрыВыгрузки["П0000020021" + ФормНомКол];
		Если Значение21 < ТекЗнач Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + НомКол + " значение в строке 21 должно быть больше либо равно суммы значений по строкам 22, 24-28, 30-32!",
				ИмяПоляТаблДокумента, "П0000020022" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	
	// Данные по строке 23 по графам 04-08 <= данным по строке 22 по соответствующим графам.
	Для НомКол = 4 По 8 Цикл
		Значение22 = ПараметрыВыгрузки["П0000020022" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"];
		Значение23 = ПараметрыВыгрузки["П0000020023" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"];
		Если Значение23 > Значение22 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + НомКол + " значение в строке 23 должно быть меньше либо равно значеня в строке 22!",
				ИмяПоляТаблДокумента, "П0000020023" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	
	// Данные по строке 29 по графам 04-08 <= данным по строке 28 по соответствующим графам.
	Для НомКол = 4 По 8 Цикл
		Значение28 = ПараметрыВыгрузки["П0000020028" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"];
		Значение29 = ПараметрыВыгрузки["П0000020029" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"];
		Если Значение29 > Значение28 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: по графе " + НомКол + " значение в строке 29 должно быть меньше либо равно значеня в строке 28!",
				ИмяПоляТаблДокумента, "П0000020029" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
