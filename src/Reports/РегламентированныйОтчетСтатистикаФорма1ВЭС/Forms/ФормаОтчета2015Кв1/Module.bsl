...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = "0613005";
	
	// ОПИСАНИЕ ПАРАМЕТРОВ МНОГОСТРОЧНЫХ ГРУПП
	// Идентификаторы многострочных групп
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0000020014";
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма, ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		ОргСведения.Свойство("ДатаРегистрации", ДатаРегистрации);
	...
	Если (НЕ ЗначениеЗаполнено(Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргДатаРегГод.Значение))
		ИЛИ (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) Тогда
		стрУказаннаяДата = Формат(ДатаРегистрации, "ДФ=""ддММгггг""");
			Если ЗначениеЗаполнено(стрУказаннаяДата) Тогда
				Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргДатаРегДень.Значение  = Число(Сред(стрУказаннаяДата, 1, 2));
				Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргДатаРегМесяц.Значение = Число(Сред(стрУказаннаяДата, 3, 2));
				Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргДатаРегГод.Значение   = Число(Сред(стрУказаннаяДата, 5));
			КонецЕсли;
	КонецЕсли;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 Тогда
		УдалитьСтроку("");
	ИначеЕсли СтрНайти(Область.Имя, "ОргДатаРег") > 0 Тогда
		
		СтандартнаяОбработка = Ложь;

		стрУказаннаяДата = Формат(ПолеТабличногоДокументаФормаОтчета.Области["ОргДатаРегГод"].Значение,   "ЧЦ=4; ЧВН=; ЧГ=")
						 + Формат(ПолеТабличногоДокументаФормаОтчета.Области["ОргДатаРегМесяц"].Значение, "ЧЦ=2; ЧВН=; ЧГ=")
						 + Формат(ПолеТабличногоДокументаФормаОтчета.Области["ОргДатаРегДень"].Значение,  "ЧЦ=2; ЧВН=; ЧГ=");
		...
		ДополнительныеПараметры = Новый Структура("УказанннаяДата", УказанннаяДата);
...
&НаКлиенте
Процедура ПолеТабличногоДокументаФормаОтчетаВыборЗавершение(Дата, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(УказанннаяДата) Тогда
		стрУказаннаяДата = Формат(УказанннаяДата, "ДФ=""ддММгггг""");
		ПолеТабличногоДокументаФормаОтчета.Области["ОргДатаРегДень"].Значение  = Число(Сред(стрУказаннаяДата, 1, 2));
		ПолеТабличногоДокументаФормаОтчета.Области["ОргДатаРегМесяц"].Значение = Число(Сред(стрУказаннаяДата, 3, 2));
		ПолеТабличногоДокументаФормаОтчета.Области["ОргДатаРегГод"].Значение   = Число(Сред(стрУказаннаяДата, 5));
	Иначе
		ПолеТабличногоДокументаФормаОтчета.Области["ОргДатаРегДень"].Значение  = 0;
		ПолеТабличногоДокументаФормаОтчета.Области["ОргДатаРегМесяц"].Значение = 0;
		ПолеТабличногоДокументаФормаОтчета.Области["ОргДатаРегГод"].Значение   = 0;
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	// Стр. 02 = стр. 03 + стр. 04 + стр. 05 + стр. 06 + стр. 08 + стр. 09 + стр. 10 + стр. 12.
	ТекСумма = 0;
	Для НомСтр = 3 По 12 Цикл
		Если НомСтр = 7 Или НомСтр = 11 Тогда
			Продолжить;
		КонецЕсли;
		ТекСумма = ТекСумма + мФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"].Значение;
	КонецЦикла;
	мФормаОтчета.Области["П000001000203"].Значение = ТекСумма;
	
	// Стр. 13 = сумме строк с кодом строки 14.
	ТекСумма = 0;
	Для НомСтр = 1 По Форма.П0000020014.Количество() Цикл
		ТекСумма = ТекСумма + мФормаОтчета.Области["П000002001404_" + Формат(НомСтр, "ЧГ=")].Значение;
	КонецЦикла;
	мФормаОтчета.Области["П000002001304"].Значение = ТекСумма;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателейНеПодлежащихОчистке.Добавить(ВыбТабличноеПоле.Области["ОргДатаРегДень"].Значение,          "ОргДатаРегДень");
	СписокПоказателейНеПодлежащихОчистке.Добавить(ВыбТабличноеПоле.Области["ОргДатаРегМесяц"].Значение,         "ОргДатаРегМесяц");
	СписокПоказателейНеПодлежащихОчистке.Добавить(ВыбТабличноеПоле.Области["ОргДатаРегГод"].Значение,           "ОргДатаРегГод");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-ВЭС");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1ВЭС");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000002001401", 250, , "Выбор страны",
		КоллекцияСписковВыбора["СправочникСтран"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000002001403", 250, , "Выбор страны",
		КоллекцияСписковВыбора["СправочникСтран"]);
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Сред(Область.Имя, 12, 2) = "01" Тогда
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(КолонкаПоиска, Формат(ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 11) + "03" + Сред(Область.Имя, 14)].Значение, "ЧГ=")));	
	ИначеЕсли Сред(Область.Имя, 12, 2) = "03" Тогда
		ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура(КолонкаПоиска, Формат(Область.Значение, "ЧГ=")));	
	КонецЕсли;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "03" Тогда
		ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 11) + "01" + Сред(Область.Имя, 14)].Значение = ?(ВыборКод <> "", ВыборНазв, "");
		ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 11) + "03" + Сред(Область.Имя, 14)].Значение = ?(ВыборКод <> "", Число(ВыборКод), 0);
	КонецЕсли;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// КОНТРОЛЬ показателей формы (Указания ФСН по заполнению Формы 1-ВЭС).
	// Стр. 06 ≥ стр. 07.
	Если Не (ПараметрыВыгрузки["ПР00001000603"] >= ПараметрыВыгрузки["ПР00001000703"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: значение в стр. 06 по гр. 3 должно быть больше или равно значению в стр. 07 по гр. 3!",
			ИмяПоляТаблДокумента, "П000001000603");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Стр. 10 ≥ стр. 11.
	Если Не (ПараметрыВыгрузки["ПР00001001003"] >= ПараметрыВыгрузки["ПР00001001103"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: значение в стр. 10 по гр. 3 должно быть больше или равно значению в стр. 11 по гр. 3!",
			ИмяПоляТаблДокумента, "П000001001003");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Стр. 13 ≤ стр. 02 (гр. 3) раздела I.
	Если Не (ПараметрыВыгрузки["ПР00002001304"] <= ПараметрыВыгрузки["ПР00001000203"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: значение в стр. 13 по гр. 4 должно быть меньше или равно значению в стр. 02 по гр. 3!",
			ИмяПоляТаблДокумента, "П000002001304");
		СтатусПроверки = Ложь;
	КонецЕсли;
	// Стр. 25 ≥ стр. 26.
	Если Не (ПараметрыВыгрузки["ПР00003002505"] >= ПараметрыВыгрузки["ПР00003002605"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: значение в стр. 25 по гр. 5 должно быть больше или равно значению в стр. 26 по гр. 5!",
			ИмяПоляТаблДокумента, "П000003002505");
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Не (ПараметрыВыгрузки["ПР00003002506"] >= ПараметрыВыгрузки["ПР00003002606"]) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: значение в стр. 25 по гр. 6 должно быть больше или равно значению в стр. 26 по гр. 6!",
			ИмяПоляТаблДокумента, "П000003002506");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	// Для ПОСЛЕДУЮЩЕГО (ПЕРЕД ВЫГРУЗКОЙ) контроля показателей формы (Указания ФСН по заполнению Формы 1-ВЭС).
	ПараметрыВыгрузки.Вставить("ПР00001000203", ТабДокумент["П000001000203"]);
	ПараметрыВыгрузки.Вставить("ПР00001000603", ТабДокумент["П000001000603"]);
	ПараметрыВыгрузки.Вставить("ПР00001000703", ТабДокумент["П000001000703"]);
	ПараметрыВыгрузки.Вставить("ПР00001001003", ТабДокумент["П000001001003"]);
	ПараметрыВыгрузки.Вставить("ПР00001001103", ТабДокумент["П000001001103"]);
	ПараметрыВыгрузки.Вставить("ПР00002001304", ТабДокумент["П000002001304"]);
	ПараметрыВыгрузки.Вставить("ПР00003002505", ТабДокумент["П000003002505"]);
	ПараметрыВыгрузки.Вставить("ПР00003002506", ТабДокумент["П000003002506"]);
	ПараметрыВыгрузки.Вставить("ПР00003002605", ТабДокумент["П000003002605"]);
	ПараметрыВыгрузки.Вставить("ПР00003002606", ТабДокумент["П000003002606"]);
