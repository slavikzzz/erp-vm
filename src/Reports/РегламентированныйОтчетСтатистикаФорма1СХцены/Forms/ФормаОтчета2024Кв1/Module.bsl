	...
	Для Раздел = 1 По 2 Цикл
		...
		Для Гр = 1 По ГрКон Цикл
			...
			Иначе
				Если Раздел = 1 Тогда
					Если Гр = 7 Тогда
						ТипКолонки = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(24, 2));
					ИначеЕсли Гр = 8 Тогда
						ТипКолонки = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(24, 4));
					ИначеЕсли Гр = 9 Или Гр = 10 Тогда
						ТипКолонки = Новый ОписаниеТипов("Строка");
					КонецЕсли;
				ИначеЕсли Раздел = 2 Тогда
					Если Гр = 8 Тогда
						ТипКолонки = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(16, 4));
					Иначе
						ТипКолонки = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(14, 2));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			
			ДобавляемыйРеквизит = Новый РеквизитФормы(ИмяДобавляемогоРеквизита + Формат(Гр, "ЧЦ=2; ЧВН="), ТипКолонки, ИмяДобавляемогоРеквизита);
			ДобавляемыеРеквизиты.Добавить(ДобавляемыйРеквизит);
			
		КонецЦикла;
		
		ИзменитьРеквизиты(ДобавляемыеРеквизиты);
		СтруктураРеквизитовФормы.Вставить("мИдГруппы" + Раздел, ИмяДобавляемогоРеквизита);
		мСтруктураМногострочныхРазделов.Вставить(ИмяДобавляемогоРеквизита, ИмяДобавляемогоРеквизита);
		мСтруктураИсхКолвоСтрокРазделов.Вставить(ИмяДобавляемогоРеквизита, ИсхКолвоСтрок);
		мСтруктураКолвоКолонокРазделов.Вставить(ИмяДобавляемогоРеквизита, ГрКон);
		СтруктураРеквизитовФормы.мСоответствияМнгЧ.Вставить(ИмяДобавляемогоРеквизита, "МногострочнаяЧасть" + Раздел);
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант = Неопределено, ВидДокументаНомерКорректировкиИзменен = Неопределено, ПоказателиОтчета = Неопределено) Экспорт
	...
	Если НЕ РегламентированнаяОтчетность.ПриЗаписиРегламентированногоОтчетаНаСервере(ЭтаФорма, КодИФНС,
...
Процедура ПеренумерацияСтрокТаблицы(ПолеТабличногоДокументаФормаОтчета)
	...
	ПолеТабличногоДокументаФормаОтчета.Области["П000001000102_1"].Значение = "1";
	ПолеТабличногоДокументаФормаОтчета.Области["П000002000102_1"].Значение = "2";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли (Не (Область.Защита = Истина)) И СтрНайти(Область.Имя, "_") = 14 Тогда 
		...
		Если СтрНайти(Область.Имя, "П00000" + Раздел + "000101_") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ОКПД");
			ОбластьДляВвода = ПолеТабличногоДокументаФормаОтчета.Области.Найти(
				СтрЗаменить(Область.Имя,"П00000" + Раздел + "000101_", "П00000" + Раздел + "000103_"));
			СмежнаяОбластьИмя = Область.Имя;
		ИначеЕсли СтрНайти(Область.Имя, "П00000" + Раздел + "000103_") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ОКПД");
			ОбластьДляВвода = Область;
			СмежнаяОбластьИмя = СтрЗаменить(Область.Имя, "П00000" + Раздел + "000103_", "П00000" + Раздел + "000101_");
		ИначеЕсли СтрНайти(Область.Имя, "П00000" + Раздел + "000104_") = 1 Тогда
			ОбластьОКПД2 = ПолеТабличногоДокументаФормаОтчета.Области["П00000" + Раздел + "000103_" + Сред(Область.Имя, 15)].Значение;
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "s_ceni_reg_sx");
			МассивВыбора = ПараметрВводаПоля.ТаблицаЗначений.НайтиСтроки(Новый Структура("РезультатПроверки", ОбластьОКПД2));
			...
			ОбластьДляВвода = Область;
			СмежнаяОбластьИмя = "";
		ИначеЕсли СтрНайти(Область.Имя, "П000001000109_") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "Причина");
			ОбластьДляВвода = Область;
			СмежнаяОбластьИмя = "";
		ИначеЕсли СтрНайти(Область.Имя, "П00000" + Раздел + "000105_") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "Реализация");
			ОбластьДляВвода = Область;
			СмежнаяОбластьИмя = "";
		ИначеЕсли СтрНайти(Область.Имя, "П00000" + Раздел + "000106_") = 1 Тогда
			ОбластьВидТовара = ПолеТабличногоДокументаФормаОтчета.Области["П00000" + Раздел + "000104_" + Сред(Область.Имя, 15)].Значение;
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "s_okpd_ei");
			МассивВыбора = ПараметрВводаПоля.ТаблицаЗначений.НайтиСтроки(Новый Структура("РезультатПроверки", ОбластьВидТовара));
			Для Каждого ЭлемМассива Из МассивВыбора Цикл
				НовСтрТЗВыбора = ТЗВыбора.Добавить();
				НовСтрТЗВыбора.Код = ЭлемМассива.Код;
				НовСтрТЗВыбора.Название = ЭлемМассива.Название;
			...
			ОбластьДляВвода = Область;
			СмежнаяОбластьИмя = "";
		ИначеЕсли СтрНайти(Область.Имя, "П000001000110_") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ПричинаИзмененияКоличества");
			ОбластьДляВвода = Область;
			СмежнаяОбластьИмя = "";
		Иначе
			Возврат;
		...
		ВвестиПоказатель(Элемент, ОбластьДляВвода, КодПоказателя, Ложь,
			ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, СмежнаяОбластьИмя);
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ПеренумерацияСтрокТаблицы(ВыбТабличноеПоле);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Область1_2 = ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть1_2");
	...
	Область2_2 = ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть2_2");
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверки = Новый Массив;
		МассивПроверки.Добавить(ТекущаяОбластьМногострочнойЧасти);
		МассивПроверки.Добавить(Область2_3);
		Если ПечатнаяФорма.ПроверитьВывод(МассивПроверки) Тогда
			ПечатнаяФорма.Вывести(ТекущаяОбластьМногострочнойЧасти);
		Иначе
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-СХ-цены");
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			ПечатнаяФорма.Вывести(Область2_2);
			ПечатнаяФорма.Вывести(ТекущаяОбластьМногострочнойЧасти);
		КонецЕсли;
		ТекущийВерх = ТекущийВерх + 1;
	КонецЦикла;
	...
	ТекущаяОбласть = ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть3");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "Причина",
		250, , "Выбор код причины", КоллекцияСписковВыбора["s_prich"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ОКПД",
		250, , "Выбор кода ОКПД2", КоллекцияСписковВыбора["s_okpd"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "Реализация",
		250, , "Выбор кода канала реализации", КоллекцияСписковВыбора["s_sell_code"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ЕдиницыИзмерения",
		250, , "Выбор единицы измерения", КоллекцияСписковВыбора["s_measure_units"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_okpd_ei",
		250, , "Выбор единицы измерения", КоллекцияСписковВыбора["s_okpd_ei"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_ceni_reg_sx",
		250, , "Выбор кода вида товара", КоллекцияСписковВыбора["s_ceni_reg_sx"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ПричинаИзмененияКоличества",
		250, , "Выбор кода причины", КоллекцияСписковВыбора["s_prichizmkolich"]);
КонецПроцедуры
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "03" Тогда
		Если Область.Значение <> РезультатВыбора["Код"] Тогда
			ОблКодВидаТовара = ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 12) + "4" + Сред(Область.Имя, 14)];
			ОблЕИ = ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 12) + "6" + Сред(Область.Имя, 14)];
			
			ОблКодВидаТовара.Значение = "";
			ОблЕИ.Значение = "";
		КонецЕсли;
	КонецЕсли;
	...
	Если Сред(Область.Имя, 12, 2) = "01" Или Сред(Область.Имя, 12, 2) = "03" Или Сред(Область.Имя, 12, 2) = "04" Или Сред(Область.Имя, 12, 2) = "05" Тогда
			Наименование1 = "";
			Наименование2 = "";
			Наименование3 = "";
			Для Каждого Элемент Из СвойстваПоказателей Цикл
				Если Элемент.Показатель = "ОКПД" Тогда
					ОбластьКол3 = ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 12) + "3" + Сред(Область.Имя, 14)];
					Если ЗначениеЗаполнено(ОбластьКол3.Значение) Тогда
						...
						КонецЕсли;
					КонецЕсли;
				ИначеЕсли Элемент.Показатель = "s_ceni_reg_sx" Тогда
					ОбластьКол3 = ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 12) + "3" + Сред(Область.Имя, 14)];
					Если ЗначениеЗаполнено(ОбластьКол3.Значение) Тогда
						ОбластьКол4 = ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 12) + "4" + Сред(Область.Имя, 14)];
						МассивВыбора = Элемент.ТаблицаЗначений.НайтиСтроки(Новый Структура("Код", ОбластьКол4.Значение));
						Если МассивВыбора.Количество() = 1 Тогда
							Наименование2 = МассивВыбора[0].Название;
						КонецЕсли;
					КонецЕсли;
				ИначеЕсли Элемент.Показатель = "Реализация" Тогда
