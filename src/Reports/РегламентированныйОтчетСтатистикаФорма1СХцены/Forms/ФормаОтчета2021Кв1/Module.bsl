...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли (Не (Область.Защита = Истина)) И СтрНайти(Область.Имя, "_") = 14 Тогда 
		...
		ВвестиПоказатель(Элемент, ОбластьДляВвода, КодПоказателя, Ложь,
			ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, СмежнаяОбластьИмя);
	КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого МПФ Из СтруктураРеквизитовФормы.мПечатныеФормы Цикл 
		Для Каждого Обл Из МПФ.Значение.Области Цикл 
			Если СтрНачинаетсяС(Обл.Имя, "П000001000104_") Тогда 
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ОКПД",
		250, , "Выбор кода ОКПД", КоллекцияСписковВыбора["s_okpd"]);
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_ceni_reg_sx",
		250, , "Выбор единицы измерения", КоллекцияСписковВыбора["s_ceni_reg_sx"]);
КонецПроцедуры
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если СтрНайти(Область.Имя, "П000001000103_") = 1 Тогда
		O3 = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "П000001000103_", "П000001000104_"));
		O4 = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "П000001000103_", "П000001000106_"));
		
		Если ЗначениеЗаполнено(РезультатВыбора["Код"]) Тогда 
			ОС = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "s_ceni_reg_sx").ТаблицаЗначений.НайтиСтроки(Новый Структура("Название", РезультатВыбора["Код"]));
			Если ОС.Количество() > 0 Тогда 
				O3.Значение = ОС[0].Код;
			Иначе
				O3.Значение = "";
			КонецЕсли;
			
			ОС = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "s_okpd_ei").ТаблицаЗначений.НайтиСтроки(Новый Структура("Название", РезультатВыбора["Код"]));
			Если ОС.Количество() > 0 Тогда 
				O4.Значение = ОС[0].Код;
			Иначе
				O4.Значение = "";
			КонецЕсли;
		Иначе 
