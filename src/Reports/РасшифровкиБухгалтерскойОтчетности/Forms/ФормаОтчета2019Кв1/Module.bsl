...
Процедура ПодготовитьДляОтправкиВБанк(Команда)
	...
	ШаблонТекстаСостояния = НСтр("ru = 'Подготовка отчета %1 для отправки ...';
								|en = 'Подготовка отчета %1 для отправки ...'");
	...
	Если НЕ ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСохраненныйДок) Тогда
		// У несохраненного отчета печатные формы будут сформированы при записи.
...
Процедура ИнициализироватьДеревоРазделовОтчета()
	...
	СтрокаУровня1.Представление            = "Карточка счета";
	СтрокаУровня1.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = НеПоказывать;
	СтрокаУровня1.ВыгрузитьСтраницу        = 2;
...
Процедура СформироватьОборотноСальдовуюВедомость(МакетРазделаСПараметрами, Шаблон, ДанныеРаздела)
	...
	Область_Шапка.Параметры.Период = СтрШаблон(
		НСтр("ru = 'за %1';
			|en = 'за %1'"), ПредставлениеПериода(ДатаНачалаПериода, ДатаКонцаПериода));
...
Функция ЕстьБланкОтчета(ДополнительныеДанные)
	
	Результат = ДополнительныеДанные <> Неопределено И ДополнительныеДанные.Свойство("БланкОтчета");
...
Процедура СформироватьСводныйОтчетПоБанковскимСчетам(МакетРазделаСПараметрами, Шаблон, ДанныеРаздела)
	...
	ЭтоКарточкаСчета = ЕстьБланкОтчета(ДанныеРаздела.ДополнительныеДанные);
	...
	Если ЭтоОтчетЗаКонкретныйПериод Тогда
		Область_Шапка.Параметры.Период = СтрШаблон(
			НСтр("ru = 'за %1';
				|en = 'за %1'"), ПредставлениеПериода(ДатаНачалаОтчета, ДатаКонцаОтчета));
	КонецЕсли;
	
	Если Не ЭтоКарточкаСчета Тогда
		МакетРаздела.Вывести(Область_Шапка);
	КонецЕсли;
	...
	Иначе
		Если ЭтоКарточкаСчета Тогда
			ВывестиКарточкаСчета(ДанныеРаздела, МакетРаздела);
			
			// Если есть Карточка счета, то выводить отчет по банковским счетам не требуется.
			Возврат;
		КонецЕсли;
...
&НаСервере
Процедура ВывестиКарточкаСчета(ДанныеРаздела, МакетРаздела)
	
	ВысотаКарточкиСчета = ДанныеРаздела.ДополнительныеДанные.БланкОтчета.ВысотаТаблицы;
	
	Если ВысотаКарточкиСчета = 0 Тогда
		Возврат;
	КонецЕсли;
	
	// Чтобы сохранить формат строк Карточки счета, необходимо оформить их
	// в отдельную область, для этого найдем их расположение между областям
	// Заголовка и Подписи.
	
	НизОбластьЗаголовка = 1;
	НизСтрокиКарточкаСчета = ВысотаКарточкиСчета;
	Для Каждого Область Из ДанныеРаздела.ДополнительныеДанные.БланкОтчета.Области Цикл
		Если Область.Низ = ВысотаКарточкиСчета Тогда
			// Нашли область Подписи, значит область строк Карточки счета закачивает на 1 выше.
			НизСтрокиКарточкаСчета = Область.Верх - 1;
		КонецЕсли;
		
		Если Область.Верх = 1 Тогда
			// Область Заголовка начинается в начале таблицы,
			// но ее нижняя граница находится ниже любой из ее вложенных областей.
			НизОбластьЗаголовка = Макс(НизОбластьЗаголовка, Область.Низ);
		КонецЕсли;
	КонецЦикла;
	
	// Область строк Карточки счета начинается на 1 ниже области Заголовка.
	ВерхСтрокиКарточкаСчета = НизОбластьЗаголовка + 1;
	
	КарточкаСчета = Новый ТабличныйДокумент;
	КарточкаСчета.Вывести(ДанныеРаздела.ДополнительныеДанные.БланкОтчета);
	АдресОбласти = "R" + Формат(ВерхСтрокиКарточкаСчета, "ЧГ=")
		+?(НизСтрокиКарточкаСчета = 0, "", ":R" + Формат(НизСтрокиКарточкаСчета, "ЧГ="));
	ОбластьКарточкаСчета = КарточкаСчета.Область(АдресОбласти);
	ОбластьКарточкаСчета.Имя = "КарточкаСчета";
	ОбластьКарточкаСчета.СоздатьФорматСтрок();
	
	// Создадим разделитель сверху отчета, чтобы отделить его от области Получатели.
	ПустаяСтрока = КарточкаСчета.Область("R" + Формат(КарточкаСчета.ВысотаТаблицы + 1, "ЧГ="));
	КарточкаСчета.ВставитьОбласть(ПустаяСтрока, КарточкаСчета.Область("R1"), ТипСмещенияТабличногоДокумента.ПоВертикали);
	
	// Выведем итоговый результат в рабочий макет.
	МакетРаздела.Вывести(КарточкаСчета);
...
Процедура ВывестиСведенияПоСубсчетамИБанковскимСчетам(ПараметрыВывода)
	...
	Для Каждого СтрокаСчета Из СтрокиДерева Цикл
		Если ОписаниеТипаБанковскиеСчета.СодержитТип(ТипЗнч(СтрокаСчета.Счет)) Тогда
			...
			ЗаголовокОтчета = "Карточка счета " + СчетВерхнегоУровня.Код
			                + " за " + ПредставлениеПериода(ДатаНачалаОтчета, ДатаКонцаОтчета)
			...
			ПараметрыДетальногоПеречня.Вставить("БИКБанка", СтрокаСчета.БИКБанка);
			ПараметрыДетальногоПеречня.Вставить("НомерСчета", СтрокаСчета.НомерСчета);
...
Функция СсылкаНаОтчет()
	...
	ИначеЕсли ТипЗнч(СтруктураРеквизитовФормы.мСохраненныйДок) = Тип("ДокументОбъект.РегламентированныйОтчет")
	        И ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка) Тогда
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт
	...
	УдалосьСоздатьОбъект = РегламентированнаяОтчетность.ПриЗаписиРегламентированногоОтчетаНаСервере(
		ЭтаФорма, КодИФНС, Автосохранение, Вариант,
		ВидДокументаНомерКорректировкиИзменен, СтруктураРеквизитовФормы.мСохраненныйДок);
	
	Если НЕ УдалосьСоздатьОбъект Тогда
	...
	СтруктураРеквизитовФормы.мСохраненныйДок.НаименованиеОтчета         = Нстр("ru = 'Расшифровки бухгалтерской отчетности для заявки на кредит';
																				|en = 'Расшифровки бухгалтерской отчетности для заявки на кредит'");
	...
	ПодготовленныеФайлы = ФайлыПредставленийОтчетовДляБанка();
	...
	Попытка
		...
		СохранитьПечатныеМакетыОтчетов(СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка, ПодготовленныеФайлы);
...
Процедура ВывестиОбластьПолучатели(Шаблон, МакетРаздела, Получатели)
	...
	ОбластьПолучателиОтчета = МакетРаздела.Область(1,, МакетРаздела.ВысотаТаблицы,);
	ОбластьПолучателиОтчета.Имя = "ПолучателиОтчета";
	ОбластьПолучателиОтчета.СоздатьФорматСтрок();
...
Функция ПредставлениеПолучателей(НаименованияПолучателей)
	...
	Иначе
		...
		ТекстПолучатели = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
			НСтр("ru = ';%1 получателю;;%1 получателям;%1 получателям;%1 получателям';
				|en = ';%1 получателю;;%1 получателям;%1 получателям;%1 получателям'"), КоличествоПолучателей);
...
Функция ОбновитьСписокСтраницРаздела(ИмяРаздела, СписокОтчетов)
	...
	Раздел = Отчеты.РасшифровкиБухгалтерскойОтчетности.ТекущееИмяРаздела(ИмяРаздела);
	
	// Ищем разделы из списка переданных отчетов в списке ранее заполненных.
	Для каждого ОписаниеОтчета Из СписокОтчетов Цикл
		Если ОписаниеОтчета.ВидОтчета <> Раздел Тогда
			Продолжить;
		КонецЕсли;
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			СтрокаСпискаСтраниц.Наименование = СтрШаблон(НСтр("ru = '%1 %2';
																|en = '%1 %2'"), СвойстваСчета.Код, СвойстваСчета.Наименование);
...
#Область ПредставлениеОтчетовВФорматеExcel
...
Процедура ПодготовитьФайлыДляБанка()
	...
	НачатьТранзакцию();
	СохранитьПечатныеМакетыОтчетов(СтруктураРеквизитовФормы.мСохраненныйДок, ПодготовленныеФайлы);
	ЗафиксироватьТранзакцию();
	
КонецПроцедуры

&НаСервере
Функция ФайлыПредставленийОтчетовДляБанка()
	
	ИсходныйРаздел = СтруктураРеквизитовФормы.НаимТекущегоРаздела;
	ИсходныйНомерСтраницы = СтруктураРеквизитовФормы.НомераСтрокМногоуровнегоРаздела;
	
	СохранитьДанныеРаздела(ИсходныйРаздел);
	
	ПодготовленныеФайлы = Новый ТаблицаЗначений;
	ПодготовленныеФайлы.Колонки.Добавить("ВидДополнительногоФайла");
	ПодготовленныеФайлы.Колонки.Добавить("СодержимоеФайла");
	ПодготовленныеФайлы.Колонки.Добавить("ИмяФайла");
	
	Для Каждого СтрокаУровня1 Из мДеревоВыбранныхСтраниц.ПолучитьЭлементы() Цикл
		Если СтрокаУровня1.ПоказатьСтраницу = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		ИмяРаздела = СтрокаУровня1.ИмяСтраницы;
		
		Если ИмяРаздела = "ОборотноСальдоваяВедомость" Тогда
			ПодготовитьФайлыДляБанка_ОСВ(ИмяРаздела, ПодготовленныеФайлы)
		ИначеЕсли ИмяРаздела = "АнализСчета" Тогда
			ПодготовитьФайлыДляБанка_АнализСчета(ИмяРаздела, ПодготовленныеФайлы)
		ИначеЕсли ИмяРаздела = "ОперацииПоБанковскимСчетам" Тогда
			ПодготовитьФайлыДляБанка_ОперацииПоБанковскимСчетам(ИмяРаздела, ПодготовленныеФайлы);
		КонецЕсли;
		
	КонецЦикла;
	
	ВывестиДанныеВТабличныйДокумент(ИсходныйРаздел, ИсходныйНомерСтраницы);
	
	Возврат ПодготовленныеФайлы;
	
КонецФункции

&НаСервере
Процедура ПодготовитьФайлыДляБанка_ОСВ(ИмяРаздела, ПодготовленныеФайлы)
	
	ДеревоДанныхРаздела = ПолучитьИзВременногоХранилища(
		СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанных" + ИмяРаздела]);
	
	Для Каждого СтрокаОтчета Из ДеревоДанныхРаздела.Строки Цикл
		ИндексныйАдрес = ИндексныйАдресВДереве(СтрокаОтчета);
		ВывестиДанныеВТабличныйДокумент(ИмяРаздела, ИндексныйАдрес);
		
		УдалитьПереченьБанковИзОтчета(ТабличныйДокумент);
		
		ОтчетДляЗаписи = ПодготовленныеФайлы.Добавить();
		ОтчетДляЗаписи.ВидДополнительногоФайла = "БланкОтчета." + "OSV."
			+ Формат(СтрокаОтчета.Параметры.ДатаНачалаПериода, "ДФ=yyyyMMdd") + "_"
			+ Формат(СтрокаОтчета.Параметры.ДатаКонцаПериода, "ДФ=yyyyMMdd");
		ОтчетДляЗаписи.СодержимоеФайла = ТабличныйДокумент.ПолучитьОбласть();
		ОтчетДляЗаписи.ИмяФайла = ОтчетДляЗаписи.ВидДополнительногоФайла + ".mxl";
...
&НаСервере
Процедура ПодготовитьФайлыДляБанка_АнализСчета(ИмяРаздела, ПодготовленныеФайлы)
	...
	Для НомерСтраницыРаздела = 1 По КолСтрокВДеревеДанныхРаздела Цикл
		ОтчетПоСчету = ДеревоДанныхРаздела.Строки[НомерСтраницыРаздела - 1];
		СчетОтчета = ОтчетПоСчету.Параметры.Счет;
		СтрокаСтраницы = СписокСтраниц.Найти(СчетОтчета, "");
		
		СтраницаВключенаВОтчет = (СтрокаСтраницы <> Неопределено И СтрокаСтраницы.Включен = Истина);
		
		Если НЕ СтраницаВключенаВОтчет Тогда
			Продолжить;
		КонецЕсли;
		
		Для Каждого СтрокаОтчета Из ОтчетПоСчету.Строки Цикл
			ИндексныйАдрес = ИндексныйАдресВДереве(СтрокаОтчета);
			ВывестиДанныеВТабличныйДокумент(ИмяРаздела, ИндексныйАдрес);
			
			ВыборкаСегментов = ОбъектОтчета().ВыборкаСегментовОтчетаПоСчетуПериоду(
				ИмяРаздела, СтрокаОтчета.Параметры.Счет,
				СтрокаОтчета.Параметры.ДатаНачалаПериода, СтрокаОтчета.Параметры.ДатаКонцаПериода,
				СсылкаНаОтчет());
			ВсегоСегментов = ВыборкаСегментов.Количество();
			
			Для НомерСегмента = 1 По ВсегоСегментов Цикл
				Если НомерСегмента > 1 Тогда
					СформироватьВывестиСекциюОтчета(ИмяРаздела, НомерСегмента);
				КонецЕсли;
				
				УдалитьПереченьБанковИзОтчета(ТабличныйДокумент);
				
				Если ВсегоСегментов > 1 Тогда
					// Очистка областей навигации.
					ОчиститьОбластьОтчета(ТабличныйДокумент, "НавигацияПервый");
					ОчиститьОбластьОтчета(ТабличныйДокумент, "НавигацияПредыдущий");
					ОчиститьОбластьОтчета(ТабличныйДокумент, "НавигацияСледующий");
					ОчиститьОбластьОтчета(ТабличныйДокумент, "НавигацияПоследний");
				КонецЕсли;
				
				ВидДополнительногоФайла = "БланкОтчета." + "ANS_" + СчетОтчета.Код + "."
					+ Формат(СтрокаОтчета.Параметры.ДатаНачалаПериода, "ДФ=yyyyMMdd") + "_"
					+ Формат(СтрокаОтчета.Параметры.ДатаКонцаПериода, "ДФ=yyyyMMdd");
				Если ВсегоСегментов > 1 Тогда
					ВидДополнительногоФайла = ВидДополнительногоФайла + "_" + СтрокаЧГ0(НомерСегмента);
				КонецЕсли;
				
				ОтчетДляЗаписи = ПодготовленныеФайлы.Добавить();
				ОтчетДляЗаписи.ВидДополнительногоФайла = ВидДополнительногоФайла;
				ОтчетДляЗаписи.СодержимоеФайла = ТабличныйДокумент.ПолучитьОбласть();
				ОтчетДляЗаписи.ИмяФайла = ОтчетДляЗаписи.ВидДополнительногоФайла + ".mxl";
...
&НаСервере
Процедура УдалитьПереченьБанковИзОтчета(ТабличныйДокументОтчета)
	
	УдаляемаяОбласть = ТабличныйДокументОтчета.Область(1, , 2, );
	ТабличныйДокументОтчета.УдалитьОбласть(УдаляемаяОбласть, ТипСмещенияТабличногоДокумента.ПоВертикали);
	
КонецПроцедуры

&НаСервере
Процедура ОчиститьОбластьОтчета(ТабличныйДокументОтчета, ИмяОбласти)
	
	ОчищамаяОбласть = ТабличныйДокументОтчета.Области.Найти(ИмяОбласти);
	ОчищамаяОбласть.Очистить(Истина, Истина, Истина);
	
КонецПроцедуры

&НаСервере
Процедура ПодготовитьФайлыДляБанка_ОперацииПоБанковскимСчетам(ИмяРаздела, ПодготовленныеФайлы)
	...
	Для НомерСтраницыРаздела = 1 По КолСтрокВДеревеДанныхРаздела Цикл
		...
		Для Каждого СтрокаОтчета Из ОтчетПоСчету.Строки Цикл
			Если ЕстьБланкОтчета(СтрокаОтчета.ДополнительныеДанные) Тогда
				// В банк будет выгружена сохраненная Карточка счета.
				БланкОтчета = СтрокаОтчета.ДополнительныеДанные.БланкОтчета;
				ОтчетДляЗаписи = ПодготовленныеФайлы.Добавить();
				
				ВидДополнительногоФайла = ОтчетностьВБанкиСлужебный.ПрефиксКарточкиСчета(
					СчетОтчета, СтрокаОтчета.Параметры.ДатаНачалаПериода, СтрокаОтчета.Параметры.ДатаКонцаПериода);
				ОтчетДляЗаписи.ВидДополнительногоФайла = СтрЗаменить(ВидДополнительногоФайла, "StTRN", "TRN");
				ОтчетДляЗаписи.СодержимоеФайла = БланкОтчета.ПолучитьОбласть();
				ОтчетДляЗаписи.ИмяФайла = ОтчетДляЗаписи.ВидДополнительногоФайла + ".mxl";
			...
			БанковскиеСчетаОтчета = Новый ТаблицаЗначений;
			БанковскиеСчетаОтчета.Колонки.Добавить("БИКБанка");
			БанковскиеСчетаОтчета.Колонки.Добавить("НомерСчета");
			БанковскиеСчетаОтчета.Колонки.Добавить("ПрефиксИдентификатораДанных");
			БанковскиеСчетаОтчета.Колонки.Добавить("ЗаголовокОтчета");
			
			ВысотаТаблицы = ТабличныйДокумент.ВысотаТаблицы;
			
			// Удаление гиперссылки перехода, выделение доступных детальных отчетов по банковским счетам.
			Для Инд = 1 По ВысотаТаблицы Цикл;
				ГоризонтальнаяОбласть = ТабличныйДокумент.ПолучитьОбласть(Инд, , Инд, );
				ШиринаСтроки = ГоризонтальнаяОбласть.ШиринаТаблицы;
				
				Для ИндПоСтолбцу = 1 По ШиринаСтроки Цикл
					Область = ТабличныйДокумент.Область(Инд, ИндПоСтолбцу);
					Расшифровка = Область.Расшифровка;
					
					Если ЗначениеЗаполнено(Расшифровка) И ТипЗнч(Расшифровка) = Тип("Структура")
					   И Расшифровка.Свойство("ВидПерехода") И Расшифровка.ВидПерехода = "ДетальныйПеречень" Тогда
						СтрокаСчета = БанковскиеСчетаОтчета.Добавить();
						СтрокаСчета.БИКБанка = Расшифровка.БИКБанка;
						СтрокаСчета.НомерСчета = Расшифровка.НомерСчета;
						СтрокаСчета.ПрефиксИдентификатораДанных = Расшифровка.ПрефиксИдентификатораДанных;
						СтрокаСчета.ЗаголовокОтчета = Расшифровка.ЗаголовокОтчета;
						
						Область.Очистить(Истина, Истина, Ложь);
			...
			ОтчетДляЗаписи = ПодготовленныеФайлы.Добавить();
			ОтчетДляЗаписи.ВидДополнительногоФайла = "БланкОтчета." + "TRN_" + СчетОтчета.Код + "."
				+ Формат(СтрокаОтчета.Параметры.ДатаНачалаПериода, "ДФ=yyyyMMdd") + "_"
				+ Формат(СтрокаОтчета.Параметры.ДатаКонцаПериода, "ДФ=yyyyMMdd");
			ОтчетДляЗаписи.СодержимоеФайла = ТабличныйДокумент.ПолучитьОбласть();
			ОтчетДляЗаписи.ИмяФайла = ОтчетДляЗаписи.ВидДополнительногоФайла + ".mxl";
			...
			Для Каждого БанковскийСчетОтчета Из БанковскиеСчетаОтчета Цикл
				КонтекстФормирования = Отчеты.РасшифровкиБухгалтерскойОтчетности.НовыйКонтекстФормирования();
				КонтекстФормирования.РегламентированныйОтчет = СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка;
				КонтекстФормирования.ПрефиксИдентификатораДанных = БанковскийСчетОтчета.ПрефиксИдентификатораДанных;
				КонтекстФормирования.ИмяМакетаОтчета = "ФормаОтчета2019Кв1_ПереченьОпераций";
				КонтекстФормирования.ПолеДокументаОтчета = ТабличныйДокумент;
				КонтекстФормирования.Вариант = "ДляПечати";
				КонтекстФормирования.Заголовок = БанковскийСчетОтчета.ЗаголовокОтчета;
				
				ВыборкаСегментов = Отчеты.РасшифровкиБухгалтерскойОтчетности.ВыборкаСегментовОтчета(
					КонтекстФормирования.РегламентированныйОтчет, КонтекстФормирования.ПрефиксИдентификатораДанных);
				ВсегоСегментов = ВыборкаСегментов.Количество();
				
				Для НомерСегмента = 1 По ВсегоСегментов Цикл
					КонтекстФормирования.Вставить("НомерСекции", НомерСегмента);
					
					Отчеты.РасшифровкиБухгалтерскойОтчетности.СформироватьСтраницуПеречняБанковскихОпераций(
						КонтекстФормирования);
					
					ВидДополнительногоФайла = "БланкОтчета." + "TRN_" + СчетОтчета.Код + "_"
						+ БанковскийСчетОтчета.БИКБанка + "_" + БанковскийСчетОтчета.НомерСчета + "."
						+ Формат(СтрокаОтчета.Параметры.ДатаНачалаПериода, "ДФ=yyyyMMdd") + "_"
						+ Формат(СтрокаОтчета.Параметры.ДатаКонцаПериода, "ДФ=yyyyMMdd");
					Если ВсегоСегментов > 1 Тогда
						ВидДополнительногоФайла = ВидДополнительногоФайла + "_" + СтрокаЧГ0(НомерСегмента);
...
&НаСервере
Функция ИндексныйАдресВДереве(СтрокаДереваЗначений)
	
	ИндексныйАдрес = Новый Массив;
	
	Курсор = СтрокаДереваЗначений;
	
	Пока Истина Цикл
		Родитель = Курсор.Родитель;
		
		Если Родитель = Неопределено Тогда
			Родитель = СтрокаДереваЗначений.Владелец();
			ИндексныйАдрес.Вставить(0, Родитель.Строки.Индекс(Курсор) + 1);
			Прервать;
	...
	Возврат ИндексныйАдрес;
	
КонецФункции

&НаСервере
Процедура СохранитьПечатныеМакетыОтчетов(ДокументСДанными, ПодготовленныеФайлы)
	...
	Для Каждого ЗаписьРегистраСведений Из ЗаписиРегистраСведений Цикл
		Если СтрНайти(ЗаписьРегистраСведений.ВидДополнительногоФайла, "БланкОтчета.") = 1
			// Исходные бланки отчета Карточка счета не удаляем.
			И СтрНайти(ЗаписьРегистраСведений.ВидДополнительногоФайла, "StTRN") = 0 Тогда
	...
	Для Каждого ПодготовленныйФайл Из ПодготовленныеФайлы Цикл
		ЗаписьРегистраСведений = ЗаписиРегистраСведений.Добавить();
		ЗаписьРегистраСведений.РегламентированныйОтчет = ДокументСДанными;
		ЗаписьРегистраСведений.ВидДополнительногоФайла = ПодготовленныйФайл.ВидДополнительногоФайла;
		
		ЗаписьРегистраСведений.СодержимоеФайла = Новый ХранилищеЗначения(
			ПодготовленныйФайл.СодержимоеФайла);
		ЗаписьРегистраСведений.ИмяФайла = ПодготовленныйФайл.ИмяФайла;
...
Процедура СообщитьОЗавершенииПодготовкиФайловДляБанка()
	...
	ШаблонТекстаПредупреждения = НСтр("ru = 'В информационной базе сохранены печатные формы отчета %1 для передачи в составе заявки';
										|en = 'В информационной базе сохранены печатные формы отчета %1 для передачи в составе заявки'");
