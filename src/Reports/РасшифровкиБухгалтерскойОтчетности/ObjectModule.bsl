...
Функция ПроверитьВозможностьВыгрузки(КонтекстФормирования, ПараметрыВыгрузки, РезультатВыгрузки = Неопределено) Экспорт
	...
	ПроверитьЗаполнение_ОперацииИКарточкаСчета(СтатусПроверки, ПараметрыВыгрузки, СтруктураРеквизитовФормы, РезультатВыгрузки);
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(КонтекстФормирования, "ТаблицаСообщений") Тогда
		Если ТипЗнч(КонтекстФормирования) = Тип("ФормаКлиентскогоПриложения") Тогда
...
Процедура ВосстановитьСохраненныеДанныеБезКонтекста(СтруктураРеквизитовФормы, СтруктураОтчета, СохраненныйОтчет, КонтекстВосстановления) Экспорт
	...
	ВызовИзФормы = (ТипЗнч(КонтекстВосстановления) = Тип("ФормаКлиентскогоПриложения"));
...
Функция УниверсальнаяВыгрузка(Параметры) Экспорт
	...
	Выгрузить_ОперацииИКарточкаСчета(КонтекстФормирования, ПараметрыВыгрузки, РезультатыВыгрузки);
	
	УстановитьПривилегированныйРежим(Ложь);

	// Найдем и очистим хранилища значений, содержащие данные отчетов во внутреннем представлении, использованные для выгрузки.
	СтруктураРеквизитовФормы = КонтекстФормирования.СтруктураРеквизитовФормы;
	Для Каждого Раздел Из СтруктураРеквизитовФормы Цикл
		Если СтрНачинаетсяС(Раздел.Ключ, "АдресВоВрХранилищеДеревоДанных")
		   И ЭтоАдресВременногоХранилища(Раздел.Значение) Тогда
			УдалитьИзВременногоХранилища(Раздел.Значение);
			СтруктураРеквизитовФормы[Раздел.Ключ] = "";
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого ПериодОтчета Из ДанныеРаздела.Строки Цикл
		...
		Если ПериодОтчета.ДополнительныеДанные = Неопределено Тогда
			ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Оборотно-сальдовая ведомость за %1 не заполнена';
											|en = 'Оборотно-сальдовая ведомость за %1 не заполнена'"), 
		...
		Для каждого ПериодОтчета Из СтраницаРаздела.Строки Цикл
			...
			Если СегментыСтраницы.Количество() = 0 Тогда
				ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Анализ счета %1 за %2 не заполнен';
												|en = 'Анализ счета %1 за %2 не заполнен'"),
					ПараметрыСтраницы.Счет,
...
Процедура ПроверитьЗаполнение_ОперацииИКарточкаСчета(СтатусПроверки, ПараметрыВыгрузки, СтруктураРеквизитовФормы, РезультатВыгрузки)
	...
	Для каждого СтраницаРаздела Из ДанныеРаздела.Строки Цикл
		...
		Для каждого ПериодОтчета Из СтраницаРаздела.Строки Цикл
			...
			Если НЕ РазделСоответствуетОтбору(ПараметрыВыгрузки, ИмяРаздела, ПериодОтчета.Параметры, Ложь) Тогда
				Продолжить;
			КонецЕсли;
			...
			Если ПериодОтчета.ДополнительныеДанные = Неопределено Тогда
				ОписаниеОшибки = СтрШаблон(НСтр("ru = 'Операции на счете %1 за %2 не заполнены';
												|en = 'Операции на счете %1 за %2 не заполнены'"),
...
Процедура Выгрузить_ОперацииИКарточкаСчета(КонтекстФормирования, ПараметрыВыгрузки, РезультатыВыгрузки)
	...
	СчетаДляКоторыхПредусмотренФорматВыгрузки = ОтчетностьВБанкиСлужебный.ПолучитьСчетаССубконтоБанковскиеСчета();
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела.Строки Цикл
		...
		Для каждого ПериодОтчета Из СтраницаРаздела.Строки Цикл
			...
			// Данные для выгрузки в xml валидны только для счетов учета банка (где есть субконто БанковскиеСчета).
			// Но для механизма выгрузки файла Карточки счета требуется инициализация результата выгрузки и заданное имя файла.
			ДвоичныеДанныеФайла = Неопределено;
			Если СчетаДляКоторыхПредусмотренФорматВыгрузки.Найти(ПараметрыСтраницы.Счет) <> Неопределено Тогда
				ДвоичныеДанныеФайла = ДвоичноеЭлектронноеПредставлениеОтчета(
					ИмяРаздела, ПараметрыВыгрузки, КонтекстФормирования, ПериодОтчета);
			КонецЕсли;
...
Функция РазделСоответствуетОтбору(ПараметрыВыгрузки,
								  ИмяРаздела,
								  ПараметрыСтраницыРаздела = Неопределено,
								  ПроверятьДерево = Истина)
	
	Если ПроверятьДерево И НЕ РазделВключенВОтчет(ИмяРаздела, ПараметрыВыгрузки.ДеревоНастройкиСтраниц) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если Отбор.ВидОтчета <> Раздел Тогда
