...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = "0612007";
...
Процедура ПоказатьПериод()
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета.Значение = "на 1 января " + Формат(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета ,1), "ДФ=""гггг 'года'""");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 12-строительство");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "12Строительство");
	...
	СтруктураПараметров.Вставить("ВидПечати",          ВидПечати);
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 12-строительство");
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата 04.09.2014 №548";
	
	Для Стр = 1 По 43 Цикл
		СтрСтр = "П010" + Формат(Стр, "ЧЦ=2; ЧВН=; ЧГ=;");
		СтрСтр2 = Формат(Стр, "ЧЦ=2; ЧВН=; ЧГ=;");
		ЗнГр3 = 0;
		ЗнГр4 = 0;
		ЗнГр6 = 0;
		ЗнГр7 = 0;
		
		Показатели.Свойство(СтрСтр + "03", ЗнГр3);
		Показатели.Свойство(СтрСтр + "04", ЗнГр4);
		Показатели.Свойство(СтрСтр + "06", ЗнГр6);
		Показатели.Свойство(СтрСтр + "07", ЗнГр7);
		
		Если ЗнГр4 <> Неопределено Тогда 
			ТекстФормулы = "По строке " + СтрСтр2 + " гр.3 >= гр.4";
			Описание     = "По строке " + СтрСтр2 + " гр.3 < гр.4";
		...
		ТекстФормулы = "По строке " + СтрСтр2 + " гр.3 >= гр.6+гр.7";
		Описание     = "По строке " + СтрСтр2 + " гр.3 < гр.6+гр.7";
		Расшифровка = Строка(ЗнГр3) + " >= " + Строка(ЗнГр6) + " + " + Строка(ЗнГр7);
		
		Если ЗнГр3 >= ЗнГр6+ЗнГр7 Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
			ЭтоОшибка = Истина;
	...
	Для Стр = 44 По 45 Цикл
		...
		Показатели.Свойство(СтрСтр + "03", ЗнГр3);
		Показатели.Свойство(СтрСтр + "04", ЗнГр4);
		Показатели.Свойство(СтрСтр + "05", ЗнГр5);
		
		ТекстФормулы = "По строке " + СтрСтр2 + " гр.3 >= гр.4+гр.5";
		Описание     = "По строке " + СтрСтр2 + " гр.3 < гр.4+гр.5";
		Расшифровка = Строка(ЗнГр3) + " >= " + Строка(ЗнГр4) + " + " + Строка(ЗнГр5);
		
		Если ЗнГр3 >= ЗнГр4+ЗнГр5 Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
			ЭтоОшибка = Истина;
	...
	СоответствиеСтрок[1] = Новый Массив;
	СоответствиеСтрок[1].Добавить(2);
	СоответствиеСтрок[1].Добавить(3);
	СоответствиеСтрок[1].Добавить(4);
	СоответствиеСтрок[1].Добавить(5);
	
	СоответствиеСтрок[6] = Новый Массив;
	СоответствиеСтрок[6].Добавить(7);
	СоответствиеСтрок[6].Добавить(8);
	
	СоответствиеСтрок[9] = Новый Массив;
	СоответствиеСтрок[9].Добавить(10);
	
	СоответствиеСтрок[12] = Новый Массив;
	СоответствиеСтрок[12].Добавить(13);
	СоответствиеСтрок[12].Добавить(14);
	СоответствиеСтрок[12].Добавить(15);
	
	СоответствиеСтрок[17] = Новый Массив;
	СоответствиеСтрок[17].Добавить(18);
	СоответствиеСтрок[17].Добавить(19);
	
	СоответствиеСтрок[20] = Новый Массив;
	СоответствиеСтрок[20].Добавить(21);
	СоответствиеСтрок[20].Добавить(22);
	
	СоответствиеСтрок[23] = Новый Массив;
	СоответствиеСтрок[23].Добавить(24);
	
	СоответствиеСтрок[25] = Новый Массив;
	СоответствиеСтрок[25].Добавить(26);
	
	СоответствиеСтрок[37] = Новый Массив;
	СоответствиеСтрок[37].Добавить(38);
	
	Для Каждого КЗ Из СоответствиеСтрок Цикл
		Для Гр = 3 По 7 Цикл 
			Если Не Показатели.Свойство("П010"+Формат(КЗ.Ключ, "ЧЦ=2; ЧВН=; ЧГ=;")+"0"+Гр) Тогда 
				Продолжить;
			КонецЕсли;
			
			ЗнЛев = Показатели["П010"+Формат(КЗ.Ключ, "ЧЦ=2; ЧВН=; ЧГ=;")+"0"+Гр];
			ЗнПрав = Показатели["П010"+Формат(КЗ.Значение[0], "ЧЦ=2; ЧВН=; ЧГ=;")+"0"+Гр];
			ТекстФормулы = "По гр."+Гр+" стр." + Формат(КЗ.Ключ, "ЧЦ=2; ЧВН=; ЧГ=;") + " >= стр." + Формат(КЗ.Значение[0], "ЧЦ=2; ЧВН=; ЧГ=;");
			Описание     = "По гр."+Гр+" стр." + Формат(КЗ.Ключ, "ЧЦ=2; ЧВН=; ЧГ=;") + " >= стр." + Формат(КЗ.Значение[0], "ЧЦ=2; ЧВН=; ЧГ=;");
			Расшифровка  = Строка(ЗнЛев) + " >= " + Строка(ЗнПрав);
			
			Для Инд = 1 По КЗ.Значение.Количество()-1 Цикл
				ЗнПравЛок    = Показатели["П010"+Формат(КЗ.Значение[Инд], "ЧЦ=2; ЧВН=; ЧГ=;")+"0"+Гр];
				ЗнПрав       = ЗнПрав + ЗнПравЛок;
				ТекстФормулы = ТекстФормулы + " + стр." + Формат(КЗ.Значение[Инд], "ЧЦ=2; ЧВН=; ЧГ=;");
				Описание     = Описание + " + стр." + Формат(КЗ.Значение[Инд], "ЧЦ=2; ЧВН=; ЧГ=;");
				Расшифровка  = Расшифровка + " + " + Строка(ЗнПравЛок);
			КонецЦикла;
