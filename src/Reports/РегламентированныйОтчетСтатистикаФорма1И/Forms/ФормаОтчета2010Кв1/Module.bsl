...
Функция СформироватьИмяФайла(ПараметрыВыгрузки, ДопАтрибуты)
	...
	ПараметрыВыгрузки.Свойство("КС", ПолученноеЗначение);
	КодКС   = СокрЛП(ПолученноеЗначение);
	
	ИмяФайлаXML = КодОКУД + "_" + КодФормы + "_" + Периодичность + "_" + КодОКПО + "_" + РасчетныйГод + "_" + ОтчетныйПериод;
	ИмяФайлаXML = ИмяФайлаXML + ?(ПустаяСтрока(КодВИ), "", "_" + КодВИ) + ?(ПустаяСтрока(КодКС), "", "_" + КодКС) + ?(ПустаяСтрока(КодОКСМ), "", "_" + КодОКСМ);
...
Функция СформироватьСтруктуруПараметров()
	...
	Период    = 3; //количество месяцев в периоде
	...
	ПараметрыВыгрузки.Вставить("КС", "0"); //принадлежность к Крайнему Северу
