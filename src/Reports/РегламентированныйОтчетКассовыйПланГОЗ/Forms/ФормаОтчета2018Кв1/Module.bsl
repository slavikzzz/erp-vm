...
Процедура ЗаполнитьАвто(Команда)
	...
	Если ПолучитьИзВременногоХранилища(СтруктураРеквизитовФормы["АдресВоВременномХранилищеКонтракты"]) = Неопределено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не выбран контракт. Заполнение отчета прервано.';
														|en = 'Не выбран контракт. Заполнение отчета прервано.'"));
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВыбранныйКонтракт) Тогда 
		...
		Если ТипЗнч(Контракты) = Тип("Массив")
			...
			Если ДанныеКонтракта <> Неопределено Тогда
				...
				ТабличныйДокумент.Области["ContractDate"].Значение = ДанныеКонтракта.ДатаЗаключения;
				Модифицированность = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = ИзвлечьСтруктуруXML("СхемаВыгрузкиКв1");
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревоДляЗагрузки = ИзвлечьСтруктуруXML("СхемаВыгрузкиКв1");
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	ДеревоВыгрузки = ИзвлечьСтруктуруXML("СхемаВыгрузкиКв1");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("CreateDateParam", XMLСтрока(ДатаФормирования) + Постфикс);
	...
	ПараметрыВыгрузки.Вставить("CreateDateParamContainer", ДатаФормирования);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	МакетПФ = ОбъектОтчета(ИмяФормы).ПолучитьМакет("ПечатныйБланк2018Кв1");
	...
	ПечатнаяФорма = НовыйТабличныйДокументДляПечати(ОриентацияСтраницы.Ландшафт);
	ПечатнаяФорма.Вывести(МакетПФ.ПолучитьОбласть("Таблица3"));
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	Если Ячейка.Раздел = "Титульный" Или Ячейка.Раздел = "КассовыйПлан" Тогда
		РегламентированнаяОтчетностьКлиент.АктивизироватьЯчейку(ЭтотОбъект, Ячейка);
	ИначеЕсли Ячейка.Раздел = "ПричиныОтклонения" Тогда
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Контейнер.Вставить("КассовыйПлан", СтруктураДанныхКассовыйПлан);
...
Процедура ЗагрузитьПодготовленныеДанные() Экспорт
	...
	ЗаполнитьЗначенияСвойств(СтруктураДанныхКассовыйПлан, Контейнер["КассовыйПлан"]);
