	...
	Если ИмяПоказателя = "INN" Тогда
		...
		Иначе
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(НСтр("ru = 'ИНН %1 должен состоять из %2 цифр. Значение очищено.';
																		|en = 'ИНН %1 должен состоять из %2 цифр. Значение очищено.'"),
...
Асинх Процедура ОчиститьТекущуюСтраницу(Команда)
	Если КодВозвратаДиалога.Да = Ждать ВопросАсинх(НСтр("ru = 'Внимание! Будет очищена текущая страница. Продолжить?';
														|en = 'Внимание! Будет очищена текущая страница. Продолжить?'"), РежимДиалогаВопрос.ДаНет) Тогда 
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Для Каждого Стр Из СохрСтруктура.ДанныеМногоуровневыхРазделов.ПоКонтрактам.Строки Цикл 
		...
		Если Не УведомлениеОСпецрежимахНалогообложения.СтраницаЗаполнена(ПоКонтракту) Тогда 
			Продолжить;
		КонецЕсли;
		НомСтраницы = НомСтраницы + 1;
