	...
	Если НЕ ((Область.Имя = "ОргНазв") Или (Область.Имя = "ОргКодОКПО") Или (Область.Имя = "ОргОтрасль")
		...
		Если Лев(Область.Имя, 12) = "П00010056650"
			...
			Для Каждого Сообщ Из МассивСообщений Цикл
				ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщ);
			КонецЦикла;
		КонецЕсли;
...
Процедура ОтправитьВБанк(Команда)
	...
	Если СтруктураРеквизитовФормы.ИП Тогда
		ПоказатьПредупреждение( , НСтр("ru = 'Для индивидуальных предпринимателей отправка не предусмотрена форматом.';
										|en = 'Для индивидуальных предпринимателей отправка не предусмотрена форматом.'"));
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ВыполнитьНастройкуСтрок()
	...
	ФормаНастроек = ПолучитьФорму("Отчет.РегламентированныйОтчетБухОтчетность.Форма.НастройкаРасшифровкиОтдельныхПоказателей",
		ПараметрыФормы);
...
Процедура ИнициализироватьОписанияМногострочныхЧастей()
	...
	ТекстОписанияСтруктуры = Отчеты[Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(ИмяМакетаСтруктурыМногострочныхЧастей).ПолучитьТекст();
...
Процедура СформироватьСоставПоказателей()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(НастройкиФормы.МакетСоставаПоказателей);
...
Процедура СформироватьНастройкиСоставаДополнительныхСтрок(ПеременнаяНастроекСостава)
	...
	МакетНастроекПоУмолчанию = Отчеты[Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(НастройкиФормы.МакетНастройкиСоставаПоказателей);
...
Процедура СохранитьДанныеРаздела(НаимРаздела, ОчищатьРазделЕслиЕстьМногострочность)
	...
	ЭтоРазделБезВводаДанных = ТипЗнч(ЭтотОбъект["СтруктураДанных" + НаимРаздела]) = Тип("Структура")
	                        И ЭтотОбъект["СтруктураДанных" + НаимРаздела].Свойство("АдресПредставления");
...
Процедура ИзвлечьИсходнуюСтруктуруНастроек(Знач МакетНастроек, ИмяСекции, ДеревоНастроек)
	...
	Если ТипЗнч(МакетНастроек) = Тип("Строка") Тогда
		МакетНастроек = Отчеты[Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(МакетНастроек);
	КонецЕсли;
	...
	Для Каждого Раздел Из мСвойстваРазделовДекларации.ПолучитьЭлементы() Цикл
		...
		Если НЕ Раздел.МногостраничностьВРазделе
		  ...
		  И НЕ Раздел.ИмяСтраницы = "ДополнительныеФайлы" Тогда
			...
			ТабличныйДокумент.Очистить();
			ТабличныйДокумент.Вывести(Отчеты[Сред(Лев(ИмяФормы,
				СтрНайти(ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(Сред(ИмяФормы,
				СтрНайти(ИмяФормы, "ФормаОтчета")) + "_" + ?(Раздел.ИмяСтраницы = "ФормаОтчета",
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если ТипЗнч(РазделыБезВводаСведений) = Тип("Структура") Тогда
		Для Каждого РазделБезВводаСведений Из РазделыБезВводаСведений Цикл
			...
			Попытка
				ПоместитьВоВременноеХранилище(ДанныеРаздела.Представление, ЭтотОбъект["СтруктураДанных" + ИмяРаздела].АдресПредставления);
				ПоместитьВоВременноеХранилище(ДанныеРаздела.Сведения,      ЭтотОбъект["СтруктураДанных" + ИмяРаздела].АдресСведений);
			Исключение
				ПоместитьВоВременноеХранилище(Новый ТабличныйДокумент, ЭтотОбъект["СтруктураДанных" + ИмяРаздела].АдресПредставления);
				ПоместитьВоВременноеХранилище(Новый ТаблицаЗначений,   ЭтотОбъект["СтруктураДанных" + ИмяРаздела].АдресСведений);
			КонецПопытки;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого Раздел Из мСвойстваРазделовДекларации.ПолучитьЭлементы() Цикл
		...
		Если ПоказателиОтчета.Свойство(Префикс + ИмяСтраницы)
		   И НЕ ПоказателиОтчета[Префикс + ИмяСтраницы].Свойство("АдресСведений") Тогда
			ЭтотОбъект["СтруктураДанных" + ИмяСтраницы] = ПоказателиОтчета[Префикс + ИмяСтраницы];
		КонецЕсли;
...
Процедура КонвертироватьТаблицуВариантовЗаполнения()
	...
	Если ЭтотОбъект["ТаблицаВариантыЗаполненияОИК"][0].ТаблицаВариантовЗаполнения.НайтиСтроки(
		Новый Структура("КодПоказателя", "П100100321008")).Количество() = 0 Тогда
		
		НовыйЭлемент = ЭтотОбъект["ТаблицаВариантыЗаполненияОИК"][0].ТаблицаВариантовЗаполнения.Добавить();
	...
	Если СохраненныйСоставДополнительныхСтрок.СтруктураСоставов.Свойство("Баланс", СохраненныйСоставДопСтрокБаланса) Тогда
		...
		Для Каждого СтрокаУровня1 Из СохраненныйСоставДопстрокБаланса.Строки Цикл
			Для Каждого СтрокаУровня2 Из СтрокаУровня1.Строки Цикл
				...
				НайденнаяСтрока = РегламентированнаяОтчетностьКлиентСервер.НайтиЭлементВДанныхФормыДерево(ЭтотОбъект[СоставДополнительныхСтрок.СтруктураСоставов.Баланс].ПолучитьЭлементы(), "ИмяОбластиДопСтроки", ИмяОбластиДопСтроки);
...
Процедура ОткрытьФормуВыбораСтраниц(ВариантВыбора)
	...
	ФормаНастройкиОтчета = ПолучитьФорму("Отчет.БухгалтерскаяОтчетностьВБанк.Форма.НастройкиОтчета", ПараметрыФормы, ЭтотОбъект);
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = Отчеты[Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СхемаВыгрузки506");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.Период) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Выгрузка за указанный период не предусмотрена по формату представления в электронной форме!", "Баланс", "ДатаБаланса");
		СтатусПроверки = Ложь;
		Возврат СтатусПроверки;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.КодИФНС) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "В справочнике ""Организации"" для текущей организации не задан код ИФНС, необходимый для формирования имени файла выгрузки!", "Баланс");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.КППОрг) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "В справочнике ""Организации"" для текущей организации не задан КПП, необходимый для формирования корректного файла выгрузки!", "Баланс");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.НаимОрг) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "На указано полное наименование организации!", "Баланс", "ОргНазв");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ИННОрг) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан ИНН организации!", "Баланс", "ОргИНН");
		СтатусПроверки = Ложь;
	ИначеЕсли НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ПараметрыВыгрузки.ИННОрг, Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Задан некорректный ИНН юридического лица!", "Баланс", "ОргИНН");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПараметрыВыгрузки.ПрПодп = "1" Тогда
		...
		Если ПустаяСтрока(ПараметрыВыгрузки.ПодпФамилия) Тогда
			СообщитьВТаблицуСообщений(ЭтотОбъект, "Не удалось определить фамилию руководителя!", "Баланс", "ОргДиректор");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если ПустаяСтрока(ПараметрыВыгрузки.ПодпИмя) Тогда
			СообщитьВТаблицуСообщений(ЭтотОбъект, "Не удалось определить имя руководителя!", "Баланс", "ОргДиректор");
			СтатусПроверки = Ложь;
		КонецЕсли;
	...
	Иначе
		...
		Если ПустаяСтрока(ПараметрыВыгрузки.ПодпФамилия) Тогда
			СообщитьВТаблицуСообщений(ЭтотОбъект, "Не удалось определить фамилию уполномоченного представителя!", "Баланс", "ОргДиректор");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если ПустаяСтрока(ПараметрыВыгрузки.ПодпИмя) Тогда
			СообщитьВТаблицуСообщений(ЭтотОбъект, "Не удалось определить имя уполномоченного представителя!", "Баланс", "ОргДиректор");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если ПустаяСтрока(ПараметрыВыгрузки.ПодпНаимДок) Тогда
			СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задано наименование документа, удостоверяющего полномочия представителя!", "Баланс", "");
			СтатусПроверки = Ложь;
		КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ПараметрыВыгрузки.ОКПО) И (СтрДлина(ПараметрыВыгрузки.ОКПО) < 8 ИЛИ СтрДлина(ПараметрыВыгрузки.ОКПО) > 10 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПараметрыВыгрузки.ОКПО)) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "На титульном листе задан некорректный код по ОКПО!", "Баланс", "ОргКодОКПО");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКВЭД) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указано значение кода вида деятельности по ОКВЭД!", "Баланс", "ОргКодОКВЭД");
		СтатусПроверки = Ложь;
	ИначеЕсли НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(ПараметрыВыгрузки.ОКВЭД) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Указано некорректное значение кода вида деятельности по ОКВЭД! Допустимы значения, соответствующие следующим шаблонам: ""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).!", "Баланс", "ОргКодОКВЭД");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКОПФ) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан код по ОКОПФ!", "Баланс", "ОргКодОКОПФ");
		СтатусПроверки = Ложь;
	ИначеЕсли (СтрДлина(ПараметрыВыгрузки.ОКОПФ) <> 2 И СтрДлина(ПараметрыВыгрузки.ОКОПФ) <> 5) ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПараметрыВыгрузки.ОКОПФ) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Задан некорректный код по ОКОПФ!", "Баланс", "ОргКодОКОПФ");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКФС) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан код по ОКФС!", "Баланс", "ОргКодОКФС");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.ОКФС) <> 2 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПараметрыВыгрузки.ОКФС) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Задан некорректный код по ОКФС!", "Баланс", "ОргКодОКФС");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ОКЕИ) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан код по ОКЕИ!", "Баланс", "КодЕИ");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.ОКЕИ) <> 3 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПараметрыВыгрузки.ОКЕИ) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Задан некорректный код по ОКЕИ!", "Баланс", "КодЕИ");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.АдрКодРегион) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Требуется указание как минимум кода региона в адресе!", "Баланс", "ОргПочтАдрес");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Для Каждого ЭлементСтруктуры Из мДанныеОтчета.СтруктураМногострочныхЧастей Цикл
		...
		Если МногострочнаяЧасть.ИмяПоляТабличногоДокумента = "ПолеТабличногоДокумента" + "Баланс" Тогда
			СообщениеОбОшибке = НСтр("ru = 'В бухгалтерском балансе не задано наименование строки расшифровки!';
									|en = 'В бухгалтерском балансе не задано наименование строки расшифровки!'");
		...
		ИначеЕсли МногострочнаяЧасть.ИмяПоляТабличногоДокумента = "ПолеТабличногоДокумента" + "ФормаОтчета" Тогда
			СообщениеОбОшибке = НСтр("ru = 'В отчете о прибылях и убытках не задано наименование строки расшифровки!';
									|en = 'В отчете о прибылях и убытках не задано наименование строки расшифровки!'");
		...
		ИначеЕсли МногострочнаяЧасть.ИмяПоляТабличногоДокумента = "ПолеТабличногоДокумента" + "ОДДС" Тогда
			СообщениеОбОшибке = НСтр("ru = 'В отчете о движении денежных средств не задано наименование строки расшифровки!';
									|en = 'В отчете о движении денежных средств не задано наименование строки расшифровки!'");
...
Функция ПолучитьСписокВычитаемыхПоказателей()
	...
	МассивВычитаемыхДопСтрок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СписокВычитаемыхДопСтрок, ",");
	...
	Пока ДанныеОтчета.Свойство(ИмяПоказателяНаименования + "_" + СтрокаЧГ0(НомерСтроки), НаименованиеСтроки) Цикл
		Если ПустаяСтрока(НаименованиеСтроки) Тогда
			...
			Если ЗаполненыЧисловыеПоказатели Тогда
				...
				СообщитьВТаблицуСообщений(ЭтотОбъект, СообщениеОбОшибке, ИмяОтчета, ПолноеИмяПоказателяНаименования);
...
Функция ДвоичныеДанныеФайла(Текст)
	...
	ТекстовыйДокумент.Записать(ИмяВременногоФайла, "windows-1251");
...
Функция МакетРаздела(ИмяРаздела)
	...
	ИмяМакетаРаздела = Сред(ИмяФормы, СтрНайти(ИмяФормы, "ФормаОтчета"))
	                 + "_" + ИмяРаздела;
	Отчет = Отчеты[Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7)];
...
Процедура ДополнитьДаннымиРазделаБезВводаСведений(РазделыБезВводаСведений, ИмяРаздела)
	...
	ДанныеРаздела.Вставить("Представление", ПолучитьИзВременногоХранилища(ЭтотОбъект["СтруктураДанных" + ИмяРаздела].АдресПредставления));
	ДанныеРаздела.Вставить("Сведения", ПолучитьИзВременногоХранилища(ЭтотОбъект["СтруктураДанных" + ИмяРаздела].АдресСведений));
...
Функция МакетРаздела15(ИмяРаздела)
	...
	Если ЗначениеЗаполнено(ИмяМакетаРаздела) Тогда
		ИмяМакета = Сред(ИмяФормы, СтрНайти(ИмяФормы, "ФормаОтчета")) + "_" + ИмяМакетаРаздела;
		МакетРаздела = Отчеты[Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(ИмяМакета);
	Иначе
		АдресМакета = ЭтотОбъект["СтруктураДанных" + ИмяРаздела].АдресПредставления;
