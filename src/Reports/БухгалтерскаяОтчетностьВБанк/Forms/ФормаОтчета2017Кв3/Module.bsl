...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КоллекцияЦветовРаскраски = РегламентированнаяОтчетность.КоллекцияЦветовРаскраски();
	
	мСтруктураЦветовРаскраски = Новый Структура;
	мСтруктураЦветовРаскраски.Вставить("_0", КоллекцияЦветовРаскраски.ЦветЗаполняетсяОднозначноВручную);
	мСтруктураЦветовРаскраски.Вставить("_1", КоллекцияЦветовРаскраски.ЦветЗаполняетсяТолькоВручную);
	мСтруктураЦветовРаскраски.Вставить("_2", КоллекцияЦветовРаскраски.ЦветЗаполняетсяАвтоКорректир);
	мСтруктураЦветовРаскраски.Вставить("_3", КоллекцияЦветовРаскраски.ЦветЗаполняетсяАвтоБезКорректир);
	мСтруктураЦветовРаскраски.Вставить("_4", КоллекцияЦветовРаскраски.ЦветЗаполняетсяАвтоАрифметика);
	мСтруктураЦветовРаскраски.Вставить("_5", КоллекцияЦветовРаскраски.ЦветЗаполняетсяАвтоматически);
	...
	ТекстДекорации = НСтр("ru = 'Выберите бухгалтерскую отчетность за %1 для отправки в Сбербанк:';
							|en = 'Выберите бухгалтерскую отчетность за %1 для отправки в Сбербанк:'");
	ПреставлениеПериодаБухотчетности = ПреставлениеПериодаБухОтчетности(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	Элементы.ПояснениеКВыборуБухОтчетности.Заголовок = СтрШаблон(ТекстДекорации, ПреставлениеПериодаБухотчетности);
	...
	Если Модифицированность Тогда
		...
		Если ЗавершениеРаботы Тогда
			ТекстПредупреждения = НСтр("ru = 'Данные отчета были изменены.
			                               ...
			                               |en = 'Данные отчета были изменены.
			                               |Перед завершением работы рекомендуется записать отчет,
			                               |иначе измененные данные будут утеряны.'");
			Возврат;
		КонецЕсли;
		...
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Данные были изменены. Сохранить изменения?';
										|en = 'Данные были изменены. Сохранить изменения?'"), РежимДиалогаВопрос.ДаНетОтмена, , КодВозвратаДиалога.Да);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Расшифровка) И ТипЗнч(Расшифровка) = Тип("Структура")
	   И Расшифровка.Свойство("ВидПерехода") И Расшифровка.ВидПерехода = "ДетальныйПеречень" Тогда
		...
		Если ДействуетСВерсииФормата("5.11.01") Тогда
			ПараметрыФормы.Вставить("ИмяМакетаОтчета", "ФормаОтчета2017Кв3_ПереченьОпераций5_11_01");
		Иначе
		...
		ОткрытьФорму("Отчет.БухгалтерскаяОтчетностьВБанк.Форма.ДетальныйПереченьОпераций", ПараметрыФормы, ЭтотОбъект, ПрефиксИдентификатораДанных);
	...
	Если СтрНайти(ПоляВводаВалюты, Лев(ИмяОбласти, 13)) > 0 Тогда
		...
		ОткрытьФорму("Справочник.Валюты.ФормаВыбора", ПараметрыФормы, ЭтотОбъект, , , ,
	...
	Если СтрНайти(ИмяОбласти, "П00011М100101") = 1 Тогда
		...
		Если ИндексОписания <= СтруктураРеквизитовФормы.ДополнительныеФайлы.ВГраница() Тогда
			...
			Попытка
				ПараметрыДиалога = Новый ПараметрыДиалогаПолученияФайлов(НСтр("ru = 'Выберите путь для сохранения файла';
																				|en = 'Выберите путь для сохранения файла'"));
				НачатьПолучениеФайлаССервера(ОписаниеФайла.АдресХранения, ОписаниеФайла.ИмяФайла, ПараметрыДиалога);
...
Процедура ТабличныйДокументПриАктивизации(Элемент)
	...
	КонтекстноеМенюУдалитьСтроку = Элементы.ТабличныйДокументКонтекстноеМенюУдалитьСтроку;
	КонтекстноеМенюДобавитьСтроку = Элементы.ТабличныйДокументКонтекстноеМенюДобавитьСтроку;
	
	УстановитьДоступностьДопстрокВКонтекстномМеню(Элемент,
		КонтекстноеМенюУдалитьСтроку, КонтекстноеМенюДобавитьСтроку);
	
КонецПроцедуры

&НаКлиенте
Процедура ТабличныйДокументСПризнакомВеденияУчетаПриАктивизации(Элемент)
	
	КонтекстноеМенюУдалитьСтроку = Элементы.ТабличныйДокументКонтекстноеМенюУдалитьСтроку1;
	КонтекстноеМенюДобавитьСтроку = Элементы.ТабличныйДокументКонтекстноеМенюДобавитьСтроку1;
...
&НаКлиенте
Процедура ПредставлениеБухОтчетностиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ИмяФормыВыбора = "Отчет.БухгалтерскаяОтчетностьВБанк.Форма.ВыборРегламентированногоОтчета";
	
	ПреставлениеПериодаБухотчетности = ПреставлениеПериодаБухОтчетности(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	ТекстЗаголовка = СтрШаблон(НСтр("ru = 'Выбор бухгалтерской отчетности за %1';
									|en = 'Выбор бухгалтерской отчетности за %1'"), ПреставлениеПериодаБухотчетности);
	
	ПараметрыФормыВыбора = Новый Структура;
	ПараметрыФормыВыбора.Вставить("ИсточникОтчета", "РегламентированныйОтчетБухОтчетность");
	ПараметрыФормыВыбора.Вставить("Организация", СтруктураРеквизитовФормы.Организация);
	ПараметрыФормыВыбора.Вставить("Заголовок", ТекстЗаголовка);
	ПараметрыФормыВыбора.Вставить("ДатаНачала", СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
	ПараметрыФормыВыбора.Вставить("ДатаОкончания", СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	
	ОповещениеОбработчикаВыбора = Новый ОписаниеОповещения(
		"ПредставлениеБухОтчетностиОбработкаВыбора", ЭтотОбъект);
...
&НаКлиенте
Процедура ПредставлениеБухОтчетностиОчистка(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	СтруктураРеквизитовФормы.Отчеты_Бухотчетность = Неопределено;
	СтруктураРеквизитовФормы.Отчеты_БухотчетностьПредставление = "";
	
	ПредставлениеБухОтчетности = СтруктураРеквизитовФормы.Отчеты_БухотчетностьПредставление;
...
&НаКлиенте
Процедура ПредставлениеБухОтчетностиОткрытие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.Отчеты_Бухотчетность) Тогда
		ПоказатьЗначение( , СтруктураРеквизитовФормы.Отчеты_Бухотчетность);
	КонецЕсли;
...
Процедура Расшифровать(Команда)
	...
	Если ТипЗнч(НеотображаемыеПоказатели) = Тип("Структура")
	   ...
	   И ЗначениеЗаполнено(НеотображаемыеПоказатели.Расшифровка) Тогда
		...
		ИДОтчета = Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7);
...
Процедура ОтправитьВБанк(Команда)
	...
	Если НЕ КорректнаяЕдиницаИзмеренияТочность Тогда
		ТекстПредупреждения = НСтр("ru = 'Для отправки установите единицу измерения ""в тысячах рублей"" или ""в миллионах рублей"" с точностью ""0""';
									|en = 'Для отправки установите единицу измерения ""в тысячах рублей"" или ""в миллионах рублей"" с точностью ""0""'");
	...
	КонечныйОбработчикСледа = Новый ОписаниеОповещения("ОтправитьВБанкЗавершение", ЭтотОбъект);
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьЦифровойСлед", ЭтотОбъект, КонечныйОбработчикСледа);
...
Процедура ОчиститьТекущийРаздел(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Внимание! Будет очищен текущий раздел.
		|Продолжить?';
		|en = 'Внимание! Будет очищен текущий раздел.
		|Продолжить?'");
...
Процедура ОчиститьТекущуюСтраницу(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Внимание! Будет очищена текущая страница.
		|Продолжить?';
		|en = 'Внимание! Будет очищена текущая страница.
		|Продолжить?'");
...
Процедура Выгрузить(Команда)
	...
	Если НЕ КорректнаяЕдиницаИзмеренияТочность Тогда
		ТекстПредупреждения = НСтр("ru = 'Для выгрузки пакета отчетности на диск установите единицу измерения ""в тысячах рублей"" или ""в миллионах рублей"" с точностью ""0""';
									|en = 'Для выгрузки пакета отчетности на диск установите единицу измерения ""в тысячах рублей"" или ""в миллионах рублей"" с точностью ""0""'");
		ПоказатьПредупреждение( , ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	
	КонечныйОбработчикСледа = Новый ОписаниеОповещения("ВыгрузитьЗавершение", ЭтотОбъект);
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьЦифровойСлед", ЭтотОбъект, КонечныйОбработчикСледа);
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("Отчеты_Бухотчетность"); // ссылка на документ регламентированного отчета
	СтруктураРеквизитовФормы.Вставить("Отчеты_БухотчетностьПредставление", "");
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		СтруктураРеквизитовФормы.ВерсияФормата = "5.12.11";
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	Для Инд = 1 По 17 Цикл
		ИменаГраф.Добавить("П00006М1001" + СтрЧГ0(Инд, 2));
	КонецЦикла;
	
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппы6М1, ОписаниеСтруктурыМСЧ(ИменаГраф, 0));
		
	СтруктураМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы6М1].ИменаПодчиненныхГрупп.Добавить(
		СтруктураРеквизитовФормы.мИдГруппы6М2);
	
	// Группа П00006М2.
	ИменаГраф = Новый Массив;
	Для Инд = 1 По 5 Цикл
		ИменаГраф.Добавить("П00006М2001" + СтрЧГ0(Инд, 2));
	КонецЦикла;
	
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппы6М2, ОписаниеСтруктурыМСЧ(ИменаГраф, 1));
	
	// Группа П00007М1.
	ИменаГраф = Новый Массив;
	Для Инд = 1 По 32 Цикл
		ИменаГраф.Добавить("П00007М1000" + СтрЧГ0(Инд, 2));
	КонецЦикла;
	
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппы7М1, ОписаниеСтруктурыМСЧ(ИменаГраф, 0));
	
	СтруктураМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы7М1].ИменаПодчиненныхГрупп.Добавить(
		СтруктураРеквизитовФормы.мИдГруппы7М2);
	
	// Группа П00007М2.
	ИменаГраф = Новый Массив;
	Для Инд = 1 По 5 Цикл
		ИменаГраф.Добавить("П00007М2000" + СтрЧГ0(Инд, 2));
	КонецЦикла;
	
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппы7М2, ОписаниеСтруктурыМСЧ(ИменаГраф, 1));
	
	// Группа П00008М1.
	ИменаГраф = Новый Массив;
	Для Инд = 1 По 32 Цикл
		ИменаГраф.Добавить("П00008М1000" + СтрЧГ0(Инд, 2));
	КонецЦикла;
	
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппы8М1, ОписаниеСтруктурыМСЧ(ИменаГраф, 0));
	
	СтруктураМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппы8М1].ИменаПодчиненныхГрупп.Добавить(
		СтруктураРеквизитовФормы.мИдГруппы8М2);
	
	// Группа П00008М2.
	ИменаГраф = Новый Массив;
	Для Инд = 1 По 5 Цикл
		ИменаГраф.Добавить("П00008М2000" + СтрЧГ0(Инд, 2));
	КонецЦикла;
	
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппы8М2, ОписаниеСтруктурыМСЧ(ИменаГраф, 1));
	...
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппы10М1, ОписаниеСтруктурыМСЧ(ИменаГраф, 0));
	...
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппы11М1, ОписаниеСтруктурыМСЧ(ИменаГраф, 0));
	
	// Группа П00012М1.
	ИменаГраф = Новый Массив;
	Для Инд = 1 По 12 Цикл
		ИменаГраф.Добавить("П00012М1001" + СтрЧГ0(Инд, 2));
	КонецЦикла;
	
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппы12М1, ОписаниеСтруктурыМСЧ(ИменаГраф, 0));
	
	// Группа П00013М1.
	ИменаГраф = Новый Массив;
	Для Инд = 1 По 12 Цикл
		ИменаГраф.Добавить("П00013М1001" + СтрЧГ0(Инд, 2));
	КонецЦикла;
	
	СтруктураМногострочныхЧастей.Вставить(СтруктураРеквизитовФормы.мИдГруппы13М1, ОписаниеСтруктурыМСЧ(ИменаГраф, 0));
	
	СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей = СтруктураМногострочныхЧастей;
	
КонецПроцедуры

// Описание структуры многострочной части.
// 
// Параметры:
//   ИменаГраф - Массив из Строка
//   УровеньПодчинения - Число
//   ИменаПодчиненныхГрупп - Массив из Строка
// 
// Возвращаемое значение:
//   Структура:
//     * ИменаГраф - Массив из Строка
//     * ИсхКолСтрок - Число - исходное количество строк МСЧ - 1
//     * УровеньПодчинения - Число
//     * ИменаПодчиненныхГрупп - Массив из Строка - пустой массив
// 
&НаСервере
Функция ОписаниеСтруктурыМСЧ(ИменаГраф, УровеньПодчинения)
	
	СтруктураМСЧ = Новый Структура;
	
	СтруктураМСЧ.Вставить("ИменаГраф", ИменаГраф);
	СтруктураМСЧ.Вставить("ИсхКолСтрок", 1);
	СтруктураМСЧ.Вставить("УровеньПодчинения", УровеньПодчинения);
	СтруктураМСЧ.Вставить("ИменаПодчиненныхГрупп", Новый Массив);
	
	Возврат СтруктураМСЧ;
...
Процедура ИнициализироватьДеревоРазделовОтчета()
	...
	СтрокаУровня1 = мДеревоСтраницОтчета.ПолучитьЭлементы().Добавить();
	СтрокаУровня1.ИмяСтраницы              = "БухОтчетность";
	СтрокаУровня1.Представление            = "Бухгалтерская отчетность";
	СтрокаУровня1.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = ПоказыватьЗаКварталНеИП;
	СтрокаУровня1.ВыгрузитьСтраницу        = 2;
	СтрокаУровня1.АвторасчетПоказателей    = Истина;
	СтрокаУровня1.СтраницаВывода           = "БухОтчетность";
	...
	СтрокаУровня2.Представление            = "Карточка счета 51 «Расчетные счета»";
	СтрокаУровня2.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня2.ВыводНаПечать            = 1;
	...
	СтрокаУровня2.Представление            = "Карточка счета 52 «Валютные счета»";
	СтрокаУровня2.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня2.ВыводНаПечать            = 1;
	СтрокаУровня2.ПоказатьСтраницу         = НеПоказывать;
	СтрокаУровня2.ВыгрузитьСтраницу        = 2;
	...
	СтрокаУровня2.Представление            = "Карточка счета 55 «Специальные счета в банках»";
	СтрокаУровня2.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня2.ВыводНаПечать            = 1;
	СтрокаУровня2.ПоказатьСтраницу         = НеПоказывать;
	СтрокаУровня2.ВыгрузитьСтраницу        = 2;
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт
	...
	СтруктураРеквизитовФормы.мСохраненныйДок.НаименованиеОтчета         = Нстр("ru = 'Бухгалтерская отчетность для Сбербанка';
																				|en = 'Бухгалтерская отчетность для Сбербанка'");
	...
	// Отчеты для банка, подготовленные в 1С-Отчетности.
	Отчеты1СОтчетности = Новый ТаблицаЗначений;
	Отчеты1СОтчетности.Колонки.Добавить("ВидОтчета"); // вид регламентированного отчета по имени метаданных
	Отчеты1СОтчетности.Колонки.Добавить("РегламентированныйОтчет"); // ссылка на документ регламентированного отчета
	Отчеты1СОтчетности.Колонки.Добавить("Представление"); // пользовательское представление регламентированного отчета
	
	Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.Отчеты_Бухотчетность) Тогда
		Отчет1СОтчетности = Отчеты1СОтчетности.Добавить();
		Отчет1СОтчетности.ВидОтчета = "РегламентированныйОтчетБухОтчетность";
		Отчет1СОтчетности.РегламентированныйОтчет = СтруктураРеквизитовФормы.Отчеты_Бухотчетность;
		Отчет1СОтчетности.Представление = СтруктураРеквизитовФормы.Отчеты_БухотчетностьПредставление;
	КонецЕсли;
	
	СписокСохранения.Вставить("Отчеты1СОтчетности", Отчеты1СОтчетности);
	...
	Попытка
		...
		Для Каждого СтруктураДопФайла Из СтруктураРеквизитовФормы.ДополнительныеФайлы Цикл
			...
			ЗаписьРегистраСведений.ВидДополнительногоФайла = ПрефиксВидаФайла + СтрЧГ0(НомерФайла);
...
Процедура ВосстановитьСохраненныеДанные()
	...
	// Отчеты для банка, подготовленные в 1С-Отчетности.
	Отчеты1СОтчетности = Неопределено;
	
	Если СписокСохранения.Свойство("Отчеты1СОтчетности", Отчеты1СОтчетности) Тогда
		Для Каждого Отчет1СОтчетности Из Отчеты1СОтчетности Цикл
			Если Отчет1СОтчетности.ВидОтчета = "РегламентированныйОтчетБухОтчетность" Тогда
				СтруктураРеквизитовФормы.Отчеты_Бухотчетность = Отчет1СОтчетности.РегламентированныйОтчет;
				СтруктураРеквизитовФормы.Отчеты_БухотчетностьПредставление = Отчет1СОтчетности.Представление;
	...
	Если СписокСохранения.Свойство("ДополнительныеФайлы", ДополнительныеФайлы) Тогда
		НомерФайла = 0;
		Для НомерФайла = 1 По ДополнительныеФайлы.Количество() Цикл
			ЗаписьРегистраСведений = РегистрыСведений.ДополнительныеФайлыРегламентированныхОтчетов.СоздатьМенеджерЗаписи();
			ЗаписьРегистраСведений.РегламентированныйОтчет = СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка;
			ЗаписьРегистраСведений.ВидДополнительногоФайла = "ДополнительныйФайл." + СтрЧГ0(НомерФайла);
			
			ЗаписьРегистраСведений.Прочитать();
			
			Если ЗаписьРегистраСведений.Выбран() Тогда
				АдресХранения = ПоместитьВоВременноеХранилище(ЗаписьРегистраСведений.СодержимоеФайла.Получить(),
	...
	Для Каждого ИмяСтраницы Из УпрощенныеРазделы Цикл
		Если ПоказателиОтчета.Свойство(Префикс + ИмяСтраницы) Тогда
			ЗаполнитьЗначенияСвойств(ЭтотОбъект["СтруктураДанных" + ИмяСтраницы], ПоказателиОтчета[Префикс + ИмяСтраницы]);
		КонецЕсли;
	КонецЦикла;
...
#Область ЦифровойСлед

&НаКлиенте
Процедура ЗаполнитьЦифровойСлед(Результат, КонечныйОбработчикСледа) Экспорт
	
	#Если НЕ ВебКлиент Тогда
	
	ОписаниеОповещенияИзвлеченияАдресов = Новый ОписаниеОповещения(
		"ЗаполнитьЦифровойСледЗавершение", ЭтотОбъект, КонечныйОбработчикСледа);
	НачатьПолучениеИнформацииОСетевыхАдаптерах(ОписаниеОповещенияИзвлеченияАдресов);
	Возврат;
	
	#КонецЕсли
	
	ПараметрыФормированияXML = НовыеПараметрыФормированияXML();
	ВыполнитьОбработкуОповещения(КонечныйОбработчикСледа, ПараметрыФормированияXML);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьЦифровойСледЗавершение(ВсеАдаптеры, КонечныйОбработчикСледа) Экспорт
	...
	Если ВсеАдаптеры.Количество() > 0 Тогда
		...
		ПараметрыФормированияXML.IPАдрес = ТекущийАдаптер.IPАдрес;
		ПараметрыФормированияXML.MACАдрес = ТекущийАдаптер.MACАдрес;
...
Функция НовоеОписаниеТипизированныхФайлов()
	...
	ОписаниеФайлов[ТипФайла].ЗаголовокВыбора =
		НСтр("ru = 'Выбрать письмо-заверение об отсутствии изменений в учредительных документах';
			|en = 'Выбрать письмо-заверение об отсутствии изменений в учредительных документах'");
	...
	ОписаниеФайлов[ТипФайла].ЗаголовокВыбора = НСтр("ru = 'Выбрать расшифровку производных финансовых инструментов';
													|en = 'Выбрать расшифровку производных финансовых инструментов'");
	...
	ОписаниеФайлов[ТипФайла].ЗаголовокВыбора = НСтр("ru = 'Выбрать отчет о ходе реализации проекта';
													|en = 'Выбрать отчет о ходе реализации проекта'");
	ОписаниеФайлов[ТипФайла].ОграничениеПоРасширению = "*.xls, *.xlsx";
	ОписаниеФайлов[ТипФайла].ОграничениеПоРазмеру = 2 * Мебибайт();
	...
	ОписаниеФайлов[ТипФайла].ЗаголовокВыбора = НСтр("ru = 'Выбрать отчет о реализации квартир';
													|en = 'Выбрать отчет о реализации квартир'");
	ОписаниеФайлов[ТипФайла].ОграничениеПоРасширению = "*.xls, *.xlsx";
	ОписаниеФайлов[ТипФайла].ОграничениеПоРазмеру = 2 * Мебибайт();
	...
	ОписаниеФайлов[ТипФайла].ЗаголовокВыбора = НСтр("ru = 'Выбрать производственную программу';
													|en = 'Выбрать производственную программу'");
	ОписаниеФайлов[ТипФайла].ОграничениеПоРасширению = "*.xls, *.xlsx";
	ОписаниеФайлов[ТипФайла].ОграничениеПоРазмеру = 2 * Мебибайт();
	...
	ОписаниеФайлов[ТипФайла].ЗаголовокВыбора = НСтр("ru = 'Выбрать справку об отсутствии лизинговых обязательств';
													|en = 'Выбрать справку об отсутствии лизинговых обязательств'");
	ОписаниеФайлов[ТипФайла].ОграничениеПоРасширению = "*.doc, *.docx, *.pdf";
	ОписаниеФайлов[ТипФайла].ОграничениеПоРазмеру = 5 * Мебибайт();
	...
	ОписаниеФайлов[ТипФайла].ЗаголовокВыбора = НСтр("ru = 'Выбрать справку об отсутствии дебиторской задолженности';
													|en = 'Выбрать справку об отсутствии дебиторской задолженности'");
	ОписаниеФайлов[ТипФайла].ОграничениеПоРасширению = "*.doc, *.docx, *.pdf";
	ОписаниеФайлов[ТипФайла].ОграничениеПоРазмеру = 5 * Мебибайт();
	...
	ОписаниеФайлов[ТипФайла].ЗаголовокВыбора = НСтр("ru = 'Выбрать справку об отсутствии кредиторской задолженности';
													|en = 'Выбрать справку об отсутствии кредиторской задолженности'");
	ОписаниеФайлов[ТипФайла].ОграничениеПоРасширению = "*.doc, *.docx, *.pdf";
	ОписаниеФайлов[ТипФайла].ОграничениеПоРазмеру = 5 * Мебибайт();
	...
	ОписаниеФайлов[ТипФайла].ЗаголовокВыбора = НСтр("ru = 'Выбрать справку об отсутствии финансовых вложений';
													|en = 'Выбрать справку об отсутствии финансовых вложений'");
	ОписаниеФайлов[ТипФайла].ОграничениеПоРасширению = "*.doc, *.docx, *.pdf";
	ОписаниеФайлов[ТипФайла].ОграничениеПоРазмеру = 5 * Мебибайт();
...
Процедура УстановитьПризнакиЗаполненияРазделовПоСведениямИБ()
	...
	ИмяФормыРедакцииОтчета = Сред(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") + 7);
...
Процедура ОбновитьОтображениеСостоянияОтчета()
	...
	Если СтатусОтчета = НСтр("ru = 'Отправлено';
							|en = 'Отправлено'") Тогда
		ЦветФона = ЦветаСтиля.ЦветФонаТекущейОтправки;
	ИначеЕсли СтатусОтчета = НСтр("ru = 'Отклонено';
									|en = 'Отклонено'") Тогда
		ЦветФона = ЦветаСтиля.ЦветФонаОшибкиОтправки;
	ИначеЕсли СтатусОтчета = НСтр("ru = 'Доставлено';
									|en = 'Доставлено'") Тогда
...
Функция МакетРаздела(ИмяРаздела)
	...
	МакетРаздела = Отчеты[Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(ИмяМакета);
...
Функция СборныйМакетРаздела(ИмяРаздела, ДанныеРаздела)
	...
	ИначеЕсли ИмяРаздела = "ОСВПоСчету" ИЛИ ИмяРаздела = "АнализСчета" Тогда
		...
		Иначе
			Если ИмяРаздела = "ОСВПоСчету" Тогда
				Если ДействуетСВерсииФормата("5.11.01") Тогда
					Шаблон = МакетРаздела(ИмяРаздела + "5_11_01");
					СформироватьСекциюОСВПоСчету_5_11_01(МакетРаздела, Шаблон, ДанныеРаздела);
				Иначе
			...
			ИначеЕсли ИмяРаздела = "АнализСчета" Тогда
				Если ДействуетСВерсииФормата("5.11.01") Тогда
					Шаблон = МакетРаздела(ИмяРаздела + "5_11_01");
					СформироватьСекциюАнализаСчета_5_11_01(МакетРаздела, Шаблон, ДанныеРаздела);
				Иначе
	...
	ИначеЕсли ИмяРаздела = "ПрочиеПоказатели" ИЛИ ИмяРаздела = "СтруктураВыручки" Тогда
		Шаблон.Параметры.НомерКвартала = СтрЧГ0(НомерКвартала);
		Шаблон.Параметры.ГодОтчета = СтрЧГ0(ГодаОтчета);
...
Процедура СформироватьСекциюОСВПоСчету(МакетРаздела, Шаблон, ДанныеРаздела, НомерСекции = 1)
	...
	Иначе
		Если ВыборкаСегментов.Количество() > 1 Тогда
			...
			НомерСтраницы = "Стр. " + СтрЧГ0(НомерСекции) + " из " + СтрЧГ0(ВыборкаСегментов.Количество());
		...
		ВидДополнительногоФайла = ИмяРаздела + "." + СчетУчета.УникальныйИдентификатор() + "." + СтрЧГ0(НомерСекции);
...
&НаСервере
Процедура СформироватьСекциюОСВПоСчету_5_11_01(МакетРаздела, Шаблон, ДанныеРаздела, НомерСекции = 1)
	
	Если ДействуетСВерсииФормата("5.12.10") Тогда
		ПрефиксСегментов = "ОСВПоСчетуКв";
		ДатаНачалаОтчета = НачалоКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		ДатаКонцаОтчета  = КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		
		ПрефиксСегментовНГ = "ОСВПоСчетуНГ";
		ДатаНачалаОтчетаНГ = НачалоГода(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		ДатаКонцаОтчетаНГ  = КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		
	Иначе
		ПрефиксСегментов   = "ОСВПоСчету";
		ПрефиксСегментовНГ = "НеЗаполняется";
		ДатаНачалаОтчета = НачалоМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		ДатаКонцаОтчета  = КонецМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		
	КонецЕсли;
	
	ОтражатьКоличество = ОтражатьКоличествоВОСВПоСчету(ДанныеРаздела.Параметры.Счет);
	
	СтруктураРеквизитовФормы.АктивныйСегмент = НомерСекции;
	
	СчетУчета = ДанныеРаздела.Параметры.Счет;
	ВыборкаСегментов = ВыборкаСегментовОтчетаПоСчету(ПрефиксСегментов, СчетУчета);
	ВыборкаСегментовНГ = ВыборкаСегментовОтчетаПоСчету(ПрефиксСегментовНГ, СчетУчета);
	
	ВсегоСегментовОтчетный   = ВыборкаСегментов.Количество();
	ВсегоСегментовСНачалаГода = ВыборкаСегментовНГ.Количество();
	
	ВсегоСегментовВОтчете = ВсегоСегментовОтчетный + ВсегоСегментовСНачалаГода;
	
	Область_Шапка = Шаблон.ПолучитьОбласть("Шапка");
	Область_Шапка.Параметры.ОсновнойСчет = ДанныеРаздела.Параметры.Счет.Код;
	
	Если ВсегоСегментовВОтчете = 0 Тогда
		Область_Шапка.Параметры.ПредставлениеПериода =
			НРег(ПредставлениеПериода(ДатаНачалаОтчета, ДатаКонцаОтчета, "ФП=Истина"));
		МакетРаздела.Вывести(Область_Шапка);
		МакетРаздела.Вывести(Шаблон.ПолучитьОбласть("ЗаполнениеОтчета"));
		
	Иначе
		Если НомерСекции > ВсегоСегментовОтчетный Тогда
			Область_Шапка.Параметры.ПредставлениеПериода =
				НРег(ПредставлениеПериода(ДатаНачалаОтчетаНГ, ДатаКонцаОтчетаНГ, "ФП=Истина"));
		Иначе
			Область_Шапка.Параметры.ПредставлениеПериода =
				НРег(ПредставлениеПериода(ДатаНачалаОтчета, ДатаКонцаОтчета, "ФП=Истина"));
		КонецЕсли;
		...
		Если ВсегоСегментовВОтчете > 1 Тогда
			Область_Навигация = Шаблон.ПолучитьОбласть("Навигация");
			НомерСтраницы = "Стр. " + СтрЧГ0(НомерСекции) + " из " + СтрЧГ0(ВсегоСегментовВОтчете);
			Область_Навигация.Параметры.НомерСтраницы = НомерСтраницы;
			МакетРаздела.Вывести(Область_Навигация);
		КонецЕсли;
		
		Область_ШапкаТаблицы = Шаблон.ПолучитьОбласть("ШапкаТаблицы" + ?(ОтражатьКоличество, "Кол", ""));
		
		Если НомерСекции > ВсегоСегментовОтчетный Тогда
			ПрефиксИтогов = ПрефиксСегментовНГ;
			ВидДополнительногоФайла = ПрефиксСегментовНГ + "." + СчетУчета.УникальныйИдентификатор() + "."
				+ СтрЧГ0(НомерСекции - ВсегоСегментовОтчетный);
			Область_ШапкаТаблицы.Параметры.НачалоПериода = Формат(ДатаНачалаОтчетаНГ, "ДФ=dd.MM.yyyy");
			Область_ШапкаТаблицы.Параметры.КонецПериода  = Формат(ДатаКонцаОтчетаНГ, "ДФ=dd.MM.yyyy");
		Иначе
			ПрефиксИтогов = ПрефиксСегментов;
			ВидДополнительногоФайла = ПрефиксСегментов + "." + СчетУчета.УникальныйИдентификатор() + "."
				+ СтрЧГ0(НомерСекции);
			Область_ШапкаТаблицы.Параметры.НачалоПериода = Формат(ДатаНачалаОтчета, "ДФ=dd.MM.yyyy");
			Область_ШапкаТаблицы.Параметры.КонецПериода  = Формат(ДатаКонцаОтчета, "ДФ=dd.MM.yyyy");
		КонецЕсли;
		
		ДанныеСекции = ДанныеРегистраДополнительныхФайлов(ВидДополнительногоФайла);
		ВыводитСекциюИтоги = (НомерСекции = 1) ИЛИ (НомерСекции = ВсегоСегментовОтчетный + 1);
		
		МакетРаздела.Вывести(Область_ШапкаТаблицы);
		
		Если ВыводитСекциюИтоги Тогда
			ВыборкаСегментовИтогов = ВыборкаСегментовОтчетаПоСчету(ПрефиксИтогов + "Итоги", СчетУчета);
			
			Если ВыборкаСегментовИтогов.Следующий() Тогда
				ИтогиОтчета = ДанныеРегистраДополнительныхФайлов(ВыборкаСегментовИтогов.ВидДополнительногоФайла);
				
				Область_ИтогТаблицы = Шаблон.ПолучитьОбласть("ИтогТаблицы" + ?(ОтражатьКоличество, "Кол", ""));
				Область_ИтогТаблицы.Параметры.Заполнить(ИтогиОтчета);
				МакетРаздела.Вывести(Область_ИтогТаблицы);
		...
		Для Каждого СтрокаПоСчету Из ДанныеСекции.Строки Цикл
			Область_СтрокаТаблицы = Шаблон.ПолучитьОбласть("СтрокаТаблицы" + ?(ОтражатьКоличество, "Кол", ""));
			Область_СтрокаТаблицы.Параметры.Заполнить(СтрокаПоСчету);
			Область_СтрокаТаблицы.Параметры.ПредставлениеСчетаСубконто = СтрокаПоСчету.КодСчета;
			МакетРаздела.Вывести(Область_СтрокаТаблицы, 1);
			
			Для Каждого Субконто1 Из СтрокаПоСчету.Строки Цикл
				Область_СтрокаТаблицы = Шаблон.ПолучитьОбласть("СтрокаТаблицы" + ?(ОтражатьКоличество, "Кол", ""));
				Область_СтрокаТаблицы.Параметры.Заполнить(Субконто1);
				
				Если НЕ ЗначениеЗаполнено(Субконто1.ПредставлениеСубконто1) Тогда
					Область_СтрокаТаблицы.Параметры.ПредставлениеСчетаСубконто = "<...>";
				Иначе
					Область_СтрокаТаблицы.Параметры.ПредставлениеСчетаСубконто = Субконто1.ПредставлениеСубконто1;
				КонецЕсли;
				
				МакетРаздела.Вывести(Область_СтрокаТаблицы, 2);
				
				Для Каждого Субконто2 Из Субконто1.Строки Цикл
					Область_СтрокаТаблицы = Шаблон.ПолучитьОбласть("СтрокаТаблицы" + ?(ОтражатьКоличество, "Кол", ""));
					Область_СтрокаТаблицы.Параметры.Заполнить(Субконто2);
					
					Если НЕ ЗначениеЗаполнено(Субконто2.ПредставлениеСубконто2) Тогда
						Область_СтрокаТаблицы.Параметры.ПредставлениеСчетаСубконто = "<...>";
					Иначе
						Область_СтрокаТаблицы.Параметры.ПредставлениеСчетаСубконто = Субконто2.ПредставлениеСубконто2;
					КонецЕсли;
					
					МакетРаздела.Вывести(Область_СтрокаТаблицы, 3);
...
&НаСервере
Процедура СформироватьСекциюАнализаСчета_5_11_01(МакетРаздела, Шаблон, ДанныеРаздела, НомерСекции = 1)
	...
	Иначе
		...
		Для Каждого СтрокаПоСчету Из ДанныеСекции.Строки Цикл
			...
			Для Каждого Субконто1 Из СтрокаПоСчету.Строки Цикл
				Если ЗначениеЗаполнено(Субконто1.КорСчет) Тогда
					Область_СтрокаТаблицыКорСчет = Шаблон.ПолучитьОбласть("СтрокаТаблицыКорСчет" + ?(ОтражатьКоличество, "Кол", ""));
					Область_СтрокаТаблицыКорСчет.Параметры.Заполнить(Субконто1);
					
					МакетРаздела.Вывести(Область_СтрокаТаблицыКорСчет, 2);
				...
				Иначе
					...
					Для Каждого Субконто2 Из Субконто1.Строки Цикл
						Если ЗначениеЗаполнено(Субконто2.КорСчет) Тогда
							Область_СтрокаТаблицыКорСчет = Шаблон.ПолучитьОбласть("СтрокаТаблицыКорСчет" + ?(ОтражатьКоличество, "Кол", ""));
							Область_СтрокаТаблицыКорСчет.Параметры.Заполнить(Субконто2);
							
							МакетРаздела.Вывести(Область_СтрокаТаблицыКорСчет, 3);
						...
						Иначе
							...
							Для Каждого Корреспонденция Из Субконто2.Строки Цикл
								Область_СтрокаТаблицыКорСчет = Шаблон.ПолучитьОбласть("СтрокаТаблицыКорСчет" + ?(ОтражатьКоличество, "Кол", ""));
								Область_СтрокаТаблицыКорСчет.Параметры.Заполнить(Корреспонденция);
								
								МакетРаздела.Вывести(Область_СтрокаТаблицыКорСчет, 4);
...
Процедура СформироватьПредставлениеОбеспечений(МакетРаздела, Шаблон, ДанныеРаздела)
	...
	Для Каждого СтрокаТаблицы Из ДанныеОтчета Цикл
		...
		ИмяОбластиСтроки = "Строка_" + СтрЧГ0(НомерСтроки);
...
Процедура ИнициализироватьТекстыГиперссылокДополнительныхФайлов()
	...
	Для Каждого ОписаниеФайла Из СтруктураРеквизитовФормы.ДополнительныеТипизированныеФайлы Цикл
		...
		ТекстГиперссылки = СтрШаблон(ТекстГиперссылки, ХранилищеОписания.ОграничениеПоРасширению, ПредельныйРазмерМб);
...
Процедура ВывестиЗадолженностьПоОбъектам(ЗадолженностьПоОбъектам, МакетРаздела, Шаблон, Интервалы)
	...
	Для Каждого ЗадолженностьПоОбъекту Из ЗадолженностьПоОбъектам.Строки Цикл
		...
		ПостфиксИмени = "_" + СтрЧГ0(ИндексСтрокиПоСрочности)
		              + "_" + СтрЧГ0(ИндексСтрокиПоВидуЗадолженности)
		              + "_" + СтрЧГ0(ИндексСтрокиПоОбъекту);
...
Процедура ПоказатьРаздел(ИмяРаздела, НовыеНомераСтрокМногоуровнегоРаздела = Неопределено)
	...
	Если ИмяРаздела <> "ОтчетыФНС" И ИмяРаздела <> "БухОтчетность" Тогда
...
Процедура ВывестиДанныеВТабличныйДокумент(ИмяТекРаздела, НовыеНомераСтрокМногоуровнегоРаздела = Неопределено)
	...
	ИначеЕсли ИмяТекРаздела = "ОтчетыФНС" ИЛИ ИмяТекРаздела = "БухОтчетность" Тогда
...
&НаКлиенте
Процедура ВыполнитьДобавлениеСтраницы()
...
&НаКлиенте
Процедура ВыполнитьУдалениеСтраницы()
...
&НаКлиенте
Процедура ВыполнитьДобавлениеСтроки()
...
&НаКлиенте
Процедура ВыполнитьУдалениеСтроки()
...
Асинх Процедура ДобавитьФайл()
	...
	ПараметрыДиалога.Фильтр = ОписаниеФильтра + " (" + ОграничениеПоРасширению + ")|" + МаскаВыбора;
	
	ПомещенныеФайлы = Ждать ПоместитьФайлыНаСерверАсинх( , , ПараметрыДиалога, УникальныйИдентификатор);
	
	Если ПомещенныеФайлы <> Неопределено Тогда
		ИмяФайла = ПомещенныеФайлы[0].СсылкаНаФайл.Имя;
		АдресХранения = ПомещенныеФайлы[0].Адрес;
		Если НЕ ДобавитьФайлНаСервере(ИмяФайла, АдресХранения) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Размер файла превышает 5 Мб. Приложите файл меньшего размера';
					|en = 'Размер файла превышает 5 Мб. Приложите файл меньшего размера'"));
		Иначе
			РасчитатьПоказатели_ДополнительныеФайлы();
			Модифицированность = Истина;
		КонецЕсли;
...
&НаСервере
Функция ДобавитьФайлНаСервере(ИмяФайла, АдресХранения)
	...
	ПолноеОписаниеФайла = ПолноеОписаниеФайла(ИмяФайла, АдресХранения, РазмерФайла);
...
&НаКлиенте
Асинх Процедура ВыбратьОткрытьТипизированныйФайл(ИмяОбласти)
	...
	Если ЗначениеЗаполнено(ТипФайла) Тогда
		...
		Если ЗначениеЗаполнено(ХранилищеОписания.Адрес) Тогда
			Попытка
				ПараметрыДиалога = Новый ПараметрыДиалогаПолученияФайлов(НСтр("ru = 'Выберите путь для сохранения файла';
																				|en = 'Выберите путь для сохранения файла'"));
				Ждать ПолучитьФайлССервераАсинх(ХранилищеОписания.Адрес, ХранилищеОписания.Имя, ПараметрыДиалога);
		...
		Иначе
			...
			Если ПомещенныеФайлы <> Неопределено Тогда
				...
				ФайлДобавлен = ДобавитьТипизированныйФайлНаСервере(
					ИмяФайла, АдресХранения, ТипФайла, ОграничениеПоРазмеру);
				...
				Иначе
					ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(
						НСтр("ru = 'Размер файла превышает %1 Мб. Приложите файл меньшего размера';
							|en = 'Размер файла превышает %1 Мб. Приложите файл меньшего размера'"),
...
&НаСервере
Функция ДобавитьТипизированныйФайлНаСервере(ИмяФайла, АдресХранения, ТипФайла, ОграничениеПоРазмеру)
	...
	ХранилищеОписания.Имя    = ИмяФайла;
...
Процедура СохранитьДанныеРаздела(НаимРаздела, ВеткаРаздела = Неопределено)
	...
	ИначеЕсли НаимРаздела = "ОтчетыФНС" ИЛИ НаимРаздела = "БухОтчетность" Тогда
...
Процедура СохранитьНаКлиенте(Автосохранение = Ложь, ВыполняемоеОповещение = Неопределено) Экспорт
	...
	ПараметрыВызоваПередСохранением = ПараметрыВызоваПередСохранением();
	ПараметрыВызоваПередСохранением.ПодобныйОтчетСуществует = ПодобныйОтчетСуществует;
	ПараметрыВызоваПередСохранением.Вариант = Вариант;
	ПараметрыВызоваПередСохранением.ОткрытьФормуВопросаПриИзмененииВидаДокументаНомераКорректировки =
		ОткрытьФормуВопросаПриИзмененииВидаДокументаНомераКорректировки;
	ПараметрыВызоваПередСохранением.Автосохранение = Автосохранение;
	ПараметрыВызоваПередСохранением.НаимТекРаздела = НаименованиеТекущегоРаздела;
	
	РезультатСохранения = ПередСохранением(ПараметрыВызоваПередСохранением);
...
// Конструктор параметров функции ПередСохранением
//
&НаСервере
Функция ПараметрыВызоваПередСохранением()
	
	ИменованныеПараметры = Новый Структура;
	ИменованныеПараметры.Вставить("КодИФНС", "не применимо");
	ИменованныеПараметры.Вставить("ПодобныйОтчетСуществует");
	ИменованныеПараметры.Вставить("Вариант");
	ИменованныеПараметры.Вставить("ОткрытьФормуВопросаПриИзмененииВидаДокументаНомераКорректировки");
	ИменованныеПараметры.Вставить("Автосохранение");
	ИменованныеПараметры.Вставить("НаимТекРаздела");
	
	Возврат ИменованныеПараметры;
	
КонецФункции

&НаСервере
Функция ПередСохранением(ИменованныеПараметры)
	
	КодИФНС = ИменованныеПараметры.КодИФНС;
	ПодобныйОтчетСуществует = ИменованныеПараметры.ПодобныйОтчетСуществует;
	Вариант = ИменованныеПараметры.Вариант;
	ОткрытьФормуВопросаПриИзмененииВидаДокументаНомераКорректировки =
		ИменованныеПараметры.ОткрытьФормуВопросаПриИзмененииВидаДокументаНомераКорректировки;
	Автосохранение = ИменованныеПараметры.Автосохранение;
	НаимТекРаздела = ИменованныеПараметры.НаимТекРаздела;
...
&НаСервере
Процедура СохранитьДеревоСтраницОтчета(Знач СохраняемыйОтчет)
	
	Если СохраняемыйОтчет <> Неопределено Тогда
		Если ТипЗнч(СохраняемыйОтчет) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			СохраняемыйОтчет = СохраняемыйОтчет.ПолучитьОбъект();
		КонецЕсли;
		
		Если НЕ СохраняемыйОтчет.ЭтоНовый() Тогда
			ХранилищеДанных = Новый ХранилищеЗначения(РеквизитФормыВЗначение("мДеревоСтраницОтчета"));
			СохраняемыйОтчет.ДеревоНастройкиСтраниц = ХранилищеДанных;
			Попытка
				СохраняемыйОтчет.Записать();
			Исключение
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Регламентированная отчетность. Сохранение списка разделов отчета';
												|en = 'Регламентированная отчетность. Сохранение списка разделов отчета'"),
					УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.РегламентированныйОтчет,
					СохраняемыйОтчет.Ссылка, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
		КонецЕсли;
...
Процедура ВосстановитьСохраненныеНастройкиДереваРазделов()
	...
	Если ТипЗнч(ДеревоНастройкиСтраниц) = Тип("ДеревоЗначений") Тогда
		...
		Если НЕ ДействуетСВерсииФормата("5.12.05") Тогда
			РазделБухОтчетность = ИсходноеДеревоСтраниц.Строки.Найти("БухОтчетность", "ИмяСтраницы", Истина);
			Если РазделБухОтчетность <> Неопределено Тогда
				РазделБухОтчетность.ПоказатьСтраницу = -1;
			КонецЕсли;
		КонецЕсли;
...
Функция СтрЧГ0(ФорматируемоеЧисло, ДлинаСтроки = 0)
	...
	Если ДлинаСтроки <> 0 Тогда
		Возврат Формат(ФорматируемоеЧисло, СтрШаблон("ЧЦ=%1; ЧН=; ЧВН=; ЧГ=0;", ДлинаСтроки));
	Иначе
...
Функция ЗаполнитьАвтоНаСервере(ЗаполнятьВсеРазделы)
	...
	ПараметрыПроцедуры.Вставить("ИДОтчета", Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7));
	ПараметрыПроцедуры.Вставить("ИДРедакцииОтчета", Сред(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") + 7));
...
Функция ПараметрыАвтозаполнения(ЗаполнятьВсеРазделы)
	...
	ПараметрыАвтозаполнения.Вставить("УникальныйИдентификаторФормы", УникальныйИдентификатор);
	...
	ПараметрыАвтозаполнения.Вставить("ВерсияФормата", СтруктураРеквизитовФормы.ВерсияФормата);
...
Процедура ДополнитьПараметрыАвтозаполнения_ЛизингОбязательства(ПараметрыАвтозаполнения, УсловияВключенияРазделов)
	...
	Если ДействуетСВерсииФормата("5.10.01") Тогда
		ПараметрыОтчета = ДанныеРаздела.Строки[0].Параметры;
		ПараметрыОтчета.СтатусЗаполнения = "заполнен";
		ПоместитьВоВременноеХранилище(ДанныеРаздела, АдресДанныхРаздела);
	КонецЕсли;
...
Процедура ДополнитьПараметрыАвтозаполнения_ОСВПоСчету(ПараметрыАвтозаполнения, УсловияВключенияРазделов)
	...
	ПериодыЗаполнения = Новый Массив;
	
	Если ДействуетСВерсииФормата("5.12.10") Тогда
		// Отчет за квартал.
		ПериодЗаполнения = ДобавитьПериодЗаполненияОСВПоСчету(ПериодыЗаполнения);
		ПериодЗаполнения.КонецПериода = КонецКвартала(ПараметрыАвтозаполнения.мДатаКонцаПериодаОтчета);
		ПериодЗаполнения.НачалоПериода = НачалоКвартала(ПериодЗаполнения.КонецПериода);
		ПериодЗаполнения.ПрефиксСегментов = "ОСВПоСчетуКв";
		
		Если Месяц(ПараметрыАвтозаполнения.мДатаКонцаПериодаОтчета) > 3 Тогда
			// Отчет с начала года по конец квартала.
			ПериодЗаполнения = ДобавитьПериодЗаполненияОСВПоСчету(ПериодыЗаполнения);
			ПериодЗаполнения.КонецПериода = КонецКвартала(ПараметрыАвтозаполнения.мДатаКонцаПериодаОтчета);
			ПериодЗаполнения.НачалоПериода = НачалоГода(ПериодЗаполнения.КонецПериода);
			ПериодЗаполнения.ПрефиксСегментов = "ОСВПоСчетуНГ";
		КонецЕсли;
		
	Иначе
		// Отчет за месяц.
		ПериодЗаполнения = ДобавитьПериодЗаполненияОСВПоСчету(ПериодыЗаполнения);
		ПериодЗаполнения.КонецПериода = КонецМесяца(ПараметрыАвтозаполнения.мДатаКонцаПериодаОтчета);
		ПериодЗаполнения.НачалоПериода = НачалоМесяца(ПериодЗаполнения.КонецПериода);
		ПериодЗаполнения.ПрефиксСегментов = "ОСВПоСчету";
		
	КонецЕсли;
	
	ПараметрыАвтозаполнения.Вставить(ИмяРаздела + "_ПериодыЗаполнения", ПериодыЗаполнения);
	
КонецПроцедуры

// Добавляет и возвращает новый период в массив периодов за которые формируется ОСВ по счету.
//
// Параметры:
//   ПериодыЗаполнения - Массив из Структура - хранилище периодов ОСВ по счету.
//
// Возвращаемое значение:
//   Структура:
//     * НачалоПериода - Дата
//     * КонецПериода - Дата
//     * ПрефиксСегментов - Строка - префикс имен сегментов использумый для сохранения данных в регистрах.
//
&НаСервере
Функция ДобавитьПериодЗаполненияОСВПоСчету(ПериодыЗаполнения)
	
	ПериодЗаполнения = Новый Структура;
	
	ПериодЗаполнения.Вставить("НачалоПериода", '0001-01-01');
	ПериодЗаполнения.Вставить("КонецПериода",  '0001-01-01');
	ПериодЗаполнения.Вставить("ПрефиксСегментов", "");
	
	ПериодыЗаполнения.Добавить(ПериодЗаполнения);
...
&НаКлиенте
Процедура ОтправитьВБанкЗавершение(ПараметрыФормированияXML, ДополнительныеПараметры) Экспорт
	
	ДвоичныеДанныеФайла = Неопределено;
	ИмяПакетаДляБанка = ПодготовитьОкружениеДляОтправкиИВыгрузки(ДвоичныеДанныеФайла, ПараметрыФормированияXML);
...
&НаКлиенте
Функция УниверсальнаяВыгрузкаXML(ДвоичныеДанныеФайла, ПараметрыФормированияXML) Экспорт
	...
	Если НЕ УниверсальнаяВыгрузкаXMLНаСервере(ДвоичныеДанныеФайла, ПараметрыФормированияXML) Тогда
		Возврат Ложь;
	КонецЕсли;
...
&НаСервере
Функция УниверсальнаяВыгрузкаXMLНаСервере(ДвоичныеДанныеФайла, ПараметрыФормированияXML)
	
	НомерВыгрузки = НомерВыгрузкиВБанк(ПараметрыФормированияXML.ДатаВыгрузки, СтруктураРеквизитовФормы.Организация);
	ПараметрыФормированияXML.НомерВыгрузки = НомерВыгрузки;
	
	ИмяФайлаПредставления = ПараметрыФормированияXML.ПрефиксИмениФайлаПредставления + "_" + НомерВыгрузки;
	ПостфиксПредставлений = ПараметрыФормированияXML.ПостфиксПредставлений;
	
	ПодготовитьСведенияСохраненногоОтчета();
	ПараметрыВыгрузки = ПараметрыЭлектронногоПредставления(ПараметрыФормированияXML);
	...
	ДеревоРазделов = ПараметрыВыгрузки.ДеревоНастройкиСтраниц;
	ДеревоРазделов.Колонки.Добавить("ПреобразоватьВМассив", Новый ОписаниеТипов("Булево"));
	ОтборВсехСтрок = Новый Структура("ПреобразоватьВМассив", Ложь);
	СтрокиДереваРазделов = ДеревоРазделов.Строки.НайтиСтроки(ОтборВсехСтрок, Истина);
	ДеревоРазделов.Колонки.Удалить("ПреобразоватьВМассив");
	
	ЕстьОсновныеОтчеты = Ложь;
	ЕстьБанковскиеОперации = Ложь;
	Для Каждого СтрокаДерева Из СтрокиДереваРазделов Цикл
		Если СтрокаДерева.ИмяСтраницы = "Операции51" ИЛИ СтрокаДерева.ИмяСтраницы = "Операции52"
		 ИЛИ СтрокаДерева.ИмяСтраницы = "Операции51" Тогда
			ЕстьБанковскиеОперации = ЕстьБанковскиеОперации ИЛИ РазделВключенВОтчет(СтрокаДерева.ИмяСтраницы, ДеревоРазделов);
		ИначеЕсли СтрокаДерева.ИмяСтраницы <> "Титульный" И СтрокаДерева.Строки.Количество() = 0 Тогда
			ЕстьОсновныеОтчеты = ЕстьОсновныеОтчеты ИЛИ РазделВключенВОтчет(СтрокаДерева.ИмяСтраницы, ДеревоРазделов);
		КонецЕсли;
	КонецЦикла;
	
	ВременныйКаталогПакета = ФайловаяСистема.СоздатьВременныйКаталог();
	
	СформированныеФайлы = СформироватьФайлыПредставленийРазделов(ВременныйКаталогПакета, ПостфиксПредставлений + "_" + НомерВыгрузки);
	
	ПараметрыВыгрузки.Вставить("СформированныеФайлы", СформированныеФайлы);
	
	ПолноеИмяФайлаПредставления = "";
	ПолноеИмяФайлаПредставленияБанковскихОпераций = "";
	
	Если ЕстьОсновныеОтчеты Тогда
		ПолноеИмяФайлаПредставления = ВременныйКаталогПакета + ИмяФайлаПредставления + ".xml";
		СформироватьФайлЭлектронногоПредставленияОсновныеОтчеты(ПараметрыВыгрузки, ПолноеИмяФайлаПредставления);
	КонецЕсли;
	
	Если ЕстьБанковскиеОперации Тогда
		НомерВыгрузкиБанковскихОпераций = НомерВыгрузки;
		Если ЕстьОсновныеОтчеты Тогда
			НомерВыгрузкиБанковскихОпераций = НомерВыгрузкиВБанк(ПараметрыФормированияXML.ДатаВыгрузки, СтруктураРеквизитовФормы.Организация);
			ИнициализироватьИдентификаторыФайлаВыгрузки(ПараметрыВыгрузки);
		КонецЕсли;
		
		ПолноеИмяФайлаПредставленияБанковскихОпераций = ВременныйКаталогПакета
			+ ПараметрыФормированияXML.ПрефиксИмениФайлаПредставления + "_" + НомерВыгрузкиБанковскихОпераций + ".xml";
		СформироватьФайлЭлектронногоПредставленияБанковскиеОперации(ПараметрыВыгрузки, ПолноеИмяФайлаПредставленияБанковскихОпераций);
		
	КонецЕсли;
	
	Если РазделВключенВОтчет("БухОтчетность", ПараметрыВыгрузки.ДеревоНастройкиСтраниц) Тогда
		ПолноеИмяФайлаБухотчетности = ВременныйКаталогПакета + ИмяФайлаПредставления + "_BUHOTCH" + ".xml";
		...
		РезультатВыгрузки = ИнтерфейсыВзаимодействияБРО.ВыгрузитьРегламентированныйОтчет(
			СтруктураРеквизитовФормы.Отчеты_Бухотчетность, ДополнительныеПараметрыВыгрузки);
		...
		Если РезультатВыгрузки.РеглОтчетВыгружен Тогда
			...
			ДвоичныеДанныеФайла.Записать(ПолноеИмяФайлаБухотчетности);
			
			СформированныйФайл = СформированныеФайлы.Добавить();
			СформированныйФайл.ИмяРаздела = "БухОтчетность";
			СформированныйФайл.ПолноеИмяФайла = ПолноеИмяФайлаБухотчетности;
			
		КонецЕсли;
		
		РезультатПолученияФайлов = ИнтерфейсыВзаимодействияБРО.ФайлыРегламентированногоОтчета(
			СтруктураРеквизитовФормы.Отчеты_Бухотчетность);
		
		ОбщиеПараметры = Новый Структура;
		ОбщиеПараметры.Вставить("РезультатПолученияФайлов", РезультатПолученияФайлов);
		ОбщиеПараметры.Вставить("ВременныйКаталогПакета", ВременныйКаталогПакета);
		ОбщиеПараметры.Вставить("ПостфиксПредставлений", ПостфиксПредставлений);
		ОбщиеПараметры.Вставить("СформированныеФайлы", СформированныеФайлы);
		
		ПараметрыРаздела = Новый Структура;
		ПараметрыРаздела.Вставить("ПрефиксФайлаОтчета");
		ПараметрыРаздела.Вставить("ПрефиксФайлаСохранения");
		ПараметрыРаздела.Вставить("ИмяРаздела");
		
		ПараметрыРаздела.ПрефиксФайлаОтчета = "БланкОтчета.Бухгалтерский баланс";
		ПараметрыРаздела.ПрефиксФайлаСохранения = "BALANS_";
		ПараметрыРаздела.ИмяРаздела = "Баланс";
		СохранитьФайлПредставленияБухОтчетности(ОбщиеПараметры, ПараметрыРаздела);
		
		ПараметрыРаздела.ПрефиксФайлаОтчета = "БланкОтчета.Отчет о финансовых результатах";
		ПараметрыРаздела.ПрефиксФайлаСохранения = "OFR_";
		ПараметрыРаздела.ИмяРаздела = "ОФР";
		СохранитьФайлПредставленияБухОтчетности(ОбщиеПараметры, ПараметрыРаздела);
		
		ПараметрыРаздела.ПрефиксФайлаОтчета = "БланкОтчета.Отчет об изменениях капитала";
		ПараметрыРаздела.ПрефиксФайлаСохранения = "OIK_";
		ПараметрыРаздела.ИмяРаздела = "ОИК";
		СохранитьФайлПредставленияБухОтчетности(ОбщиеПараметры, ПараметрыРаздела);
		
		ПараметрыРаздела.ПрефиксФайлаОтчета = "БланкОтчета.Отчет о движении денежных средств";
		ПараметрыРаздела.ПрефиксФайлаСохранения = "ODDS_";
		ПараметрыРаздела.ИмяРаздела = "ОДДС";
		СохранитьФайлПредставленияБухОтчетности(ОбщиеПараметры, ПараметрыРаздела);
		
		ПараметрыРаздела.ПрефиксФайлаОтчета = "БланкОтчета.Отчет о целевом использовании средств";
		ПараметрыРаздела.ПрефиксФайлаСохранения = "OCIS_";
		ПараметрыРаздела.ИмяРаздела = "ОЦИС";
		СохранитьФайлПредставленияБухОтчетности(ОбщиеПараметры, ПараметрыРаздела);
	...
	Для Каждого СформированныйФайл Из СформированныеФайлы Цикл
		ИмяФайлаРаздела = СформированныйФайл.ПолноеИмяФайла;
	...
	Если ЕстьБанковскиеОперации Тогда
		ПакетОтправки.Добавить(ПолноеИмяФайлаПредставленияБанковскихОпераций);
	КонецЕсли;
...
&НаСервере
Процедура ПодготовитьСведенияСохраненногоОтчета()
...
&НаСервере
Функция ПараметрыЭлектронногоПредставления(ДополнительныеПараметры = Неопределено)
	
	IPАдрес = "";
	MACАдрес = "";
	
	Если ДополнительныеПараметры <> Неопределено Тогда
		IPАдрес = ДополнительныеПараметры.IPАдрес;
		MACАдрес = ДополнительныеПараметры.MACАдрес;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(IPАдрес) И НЕ ЗначениеЗаполнено(MACАдрес) Тогда
		ЦифровойСлед = ОтчетностьВБанкиСлужебный.ЦифровойСлед();
		IPАдрес = ЦифровойСлед.IPАдрес;
		MACАдрес = ЦифровойСлед.MACАдрес;
	КонецЕсли;
	...
	ПараметрыВыгрузки.Вставить("ИдентификаторФайлаВыгрузки", "");
	...
	ПараметрыВыгрузки.Вставить("IP", IPАдрес);
	ПараметрыВыгрузки.Вставить("MAC", MACАдрес);
...
&НаСервере
Процедура ИнициализироватьИдентификаторыФайлаВыгрузки(ПараметрыВыгрузки)
	
	ПараметрыВыгрузки.ИдентификаторФайлаВыгрузки = Строка(Новый УникальныйИдентификатор);
	
	ПараметрыВыгрузки.ИдФайл = СформироватьИдФайл(ПараметрыВыгрузки);
...
&НаСервере
Процедура СохранитьФайлПредставленияБухОтчетности(ОбщиеПараметры, ПараметрыРаздела)
	
	РезультатПолученияФайлов = ОбщиеПараметры.РезультатПолученияФайлов;
	ВременныйКаталогПакета = ОбщиеПараметры.ВременныйКаталогПакета;
	ПостфиксПредставлений = ОбщиеПараметры.ПостфиксПредставлений;
	СформированныеФайлы = ОбщиеПараметры.СформированныеФайлы;
	
	ПрефиксФайлаОтчета = ПараметрыРаздела.ПрефиксФайлаОтчета;
	ПрефиксФайлаСохранения = ПараметрыРаздела.ПрефиксФайлаСохранения;
	ИмяРаздела = ПараметрыРаздела.ИмяРаздела;
	...
	Для Каждого ФайлПредставления Из РезультатПолученияФайлов.ФайлыОтчета Цикл
	...
	Если ПечатныеМакетыОтчета.Количество() <> 0 Тогда
		...
		ИмяФайлаРаздела = ПрефиксФайлаСохранения
			+ ПостфиксПредставлений + РасширениеФайла;
		ПолноеИмяФайлаРаздела = ВременныйКаталогПакета
			+ ИмяФайлаРаздела;
		
		ДополнитьПредставлениеИменемФайла(СохраняемыйОтчет, ИмяФайлаРаздела);
		
		СохраняемыйОтчет.Записать(ПолноеИмяФайлаРаздела, ФорматСохранения);
		
		ХешСумма = ХешСуммаФайла(ПолноеИмяФайлаРаздела);
		
		СформированныйФайл = СформированныеФайлы.Добавить();
		СформированныйФайл.ИмяРаздела = ИмяРаздела;
		СформированныйФайл.ИмяФайла = ИмяФайлаРаздела;
		СформированныйФайл.ПолноеИмяФайла = ПолноеИмяФайлаРаздела;
		СформированныйФайл.ХешСумма = ХешСумма;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	ПроверитьЗаполнение_БухОтчетность(СтатусПроверки, ПараметрыВыгрузки);
...
Процедура ПроверитьЗаполнение_Титульный(СтатусПроверки, ПараметрыВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.НаимОрг) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "На указано полное наименование организации", ИмяРаздела, "НаименованиеОрганизации");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.НаимОргКраткое) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "На указано полное наименование организации", ИмяРаздела, "НаименованиеОрганизацииКраткое");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если СтруктураРеквизитовФормы.ИП Тогда
		Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ИННФЛ) Тогда
			СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указан ИНН индивидуального предпринимателя", ИмяРаздела, "ИНН");
			СтатусПроверки = Ложь;
		ИначеЕсли НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ПараметрыВыгрузки.ИННФЛ, Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо) Тогда
			СообщитьВТаблицуСообщений(ЭтотОбъект, "Указан некорректный ИНН индивидуального предпринимателя", ИмяРаздела, "ИНН");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
	Иначе
		Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ИННОрг) Тогда
			СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указан ИНН организации", ИмяРаздела, "ИНН");
			СтатусПроверки = Ложь;
		ИначеЕсли НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ПараметрыВыгрузки.ИННОрг, Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо) Тогда
			СообщитьВТаблицуСообщений(ЭтотОбъект, "Указан некорректный ИНН юридического лица", ИмяРаздела, "ИНН");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если ПустаяСтрока(ПараметрыВыгрузки.КППОрг) Тогда
			СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указан КПП юридического лица", ИмяРаздела, "КПП");
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(ПараметрыВыгрузки.КППОрг) <> 9 Тогда
			СообщитьВТаблицуСообщений(ЭтотОбъект, "Указан некорректный КПП юридического лица", ИмяРаздела, "КПП");
			СтатусПроверки = Ложь;
		КонецЕсли;
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ПодпФамилия) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указана фамилия лица, утвердившего отчетность", ИмяРаздела, "Подписант");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ПодпИмя) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект, "Не указано имя лица, утвердившего отчетность", ИмяРаздела, "Подписант");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура ПроверитьЗаполнение_РасшифровкиОбеспеченийПолученные(СтатусПроверки, ПараметрыВыгрузки)
	...
	Для Каждого СтрокаОтчета Из СтрокиОтчета Цикл
		НомерСтроки = СтрЧГ0(СтрокиОтчета.Индекс(СтрокаОтчета) + 1);
...
Процедура ПроверитьЗаполнение_ЗадолженностьПоКредитамЗаймам(СтатусПроверки, ПараметрыВыгрузки, ПараметрыПроверки)
	...
	ОписаниеОшибкиЗаполнения = СтрШаблон(ОписаниеОшибкиЗаполнения, НазваниеРаздела);
	...
	Для Каждого СтрокаМСЧ1 Из МСЧ1.Строки Цикл
		...
		Постфикс1 = "_" + СтрЧГ0(НомерСтроки);
		...
		Если СтрокаЗаполнена Тогда
			...
			Если НЕ ЗначениеЗаполнено(КодСчета) Тогда
				ОписаниеОшибки = "В строке " + СтрЧГ0(НомерСтроки) + ВидВРасшифровке
				               + " не указан код счета учета";
				ЗафиксироватьОшибкуЗаполнения(СтатусПроверки, ОписаниеОшибки, ИмяРаздела, ИмяПоказателяМакета);
				
			Иначе
				СчетБухУчета =  ОтчетностьВБанкиСлужебный.СчетБухУчета(КодСчета);
				Если НЕ ЗначениеЗаполнено(СчетБухУчета) Тогда
					ОписаниеОшибки = "В строке " + СтрЧГ0(НомерСтроки) + ВидВРасшифровке
					               + " указан код счета учета, отсутствующий в плане счетов";
					ЗафиксироватьОшибкуЗаполнения(СтатусПроверки, ОписаниеОшибки, ИмяРаздела, ИмяПоказателяМакета);
				КонецЕсли;
			...
			Если УказанДоговор Тогда
				...
				Для Каждого СтрокаМСЧ2 Из МСЧ2.Строки Цикл
					НомерСтрокиГрафикаПлатежей = СтрЧГ0(МСЧ2.Строки.Индекс(СтрокаМСЧ2) + 1);
					...
					Если ЗаполненГрафикПлатежей И НЕ ЗначениеЗаполнено(ДатаПлатежа) Тогда
						...
						ОписаниеОшибки = "В строке " + НомерСтрокиГрафикаПлатежей + " графика платежей"
						               + " строки " + СтрЧГ0(НомерСтроки) + ВидВРасшифровке
		...
		Для Каждого СтрокаМСЧ Из МногострочнаяЧасть.Строки Цикл
			...
			Если СтрокаЗаполнена Тогда
				...
				Если НЕ ЗначениеЗаполнено(КомпанияНаименование) Тогда
					ОписаниеОшибки = "В строке " + СтрЧГ0(НомерСтроки) + " страницы " + СтрЧГ0(НомерСтраницы)
					               + " расходов по финансовому лизингу"
					               + " не указано наименование компании";
					ЗафиксироватьОшибкуЗаполнения(СтатусПроверки, ОписаниеОшибки, ИмяРаздела, ИмяПоказателя + Постфикс, СтрЧГ0(НомерСтраницы));
				КонецЕсли;
				...
				Для Каждого СтрокаМСЧ2 Из МСЧ2.Строки Цикл
					...
					Если ЗаполненГрафикПлатежей И НЕ ЗначениеЗаполнено(ДатаПлатежа) Тогда
						...
						ОписаниеОшибки = "В строке " + НомерСтрокиГрафикаПлатежей + " графика платежей"
						               + " строки " + СтрЧГ0(НомерСтроки) + " расходов по финансовому лизингу"
						               + " не указана дата платежа";
						ЗафиксироватьОшибкуЗаполнения(СтатусПроверки, ОписаниеОшибки, ИмяРаздела, ИмяПоказателяМакета);
...
Процедура ПроверитьЗаполнение_ОСВПоСчету(СтатусПроверки, ПараметрыВыгрузки)
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела.Строки Цикл
		...
		Если ДействуетСВерсииФормата("5.12.10") Тогда
			ВыборкаСегментов   = ВыборкаСегментовОтчетаПоСчету("ОСВПоСчетуКв", ПараметрыСтраницы.Счет);
			ВыборкаСегментовНГ = ВыборкаСегментовОтчетаПоСчету("ОСВПоСчетуНГ", ПараметрыСтраницы.Счет);
			ВсегоСегментов = ВыборкаСегментов.Количество() + ВыборкаСегментовНГ.Количество();
...
Процедура ПроверитьЗаполнение_СтруктураВыручки(СтатусПроверки, ПараметрыВыгрузки)
	...
	Для Каждого СтрокаМСЧ Из МногострочнаяЧасть.Строки Цикл
		...
		Если СтрокаЗаполнена Тогда
			...
			Если НЕ ЗначениеЗаполнено(КомпанияНаименование) Тогда
				ОписаниеОшибки = "В строке " + СтрЧГ0(НомерСтроки)
			...
			Если НЕ ЗначениеЗаполнено(ЗначениеПоказателя) Тогда
				ОписаниеОшибки = "В строке " + СтрЧГ0(НомерСтроки) + " многострочной части"
				               + " справки о структуре выручки"
...
Процедура ПроверитьЗаполнение_ПрочиеПоказатели(СтатусПроверки, ПараметрыВыгрузки)
	...
	Если ПустаяСтрока(ДанныеСтраницы[ИмяПоказателя]) Тогда
		СообщитьВТаблицуСообщений(ЭтотОбъект,
...
&НаСервере
Процедура ПроверитьЗаполнение_БухОтчетность(СтатусПроверки, ПараметрыВыгрузки)
	
	ИмяРаздела = "БухОтчетность";
	
	Если НЕ РазделВключенВОтчет(ИмяРаздела, ПараметрыВыгрузки.ДеревоНастройкиСтраниц) Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(СтруктураРеквизитовФормы.Отчеты_Бухотчетность) Тогда
		ОписаниеОшибки = "Не выбрана бухгалтерская отчетность для отправки";
		ЗафиксироватьОшибкуЗаполнения(СтатусПроверки, ОписаниеОшибки, ИмяРаздела);
		Возврат;
	КонецЕсли;
	
	ДанныеОтчета = СтруктураРеквизитовФормы.Отчеты_Бухотчетность.ДанныеОтчета.Получить();
	
	// В Сбербанк передается только обычная отчетность.
	ЭтоОбычнаяОтчетность = (ДанныеОтчета.ВидСдачиОтчетности = 1);
	
	Если НЕ ЭтоОбычнаяОтчетность Тогда
		СтатусПроверки = Ложь;
		
		НовоеСообщениеОбОшибкеВОтчете = ТаблицаСообщений.Добавить();
		НовоеСообщениеОбОшибкеВОтчете.Раздел = "Баланс";
		НовоеСообщениеОбОшибкеВОтчете.ИмяЯчейки = "ВидОтчетности";
		НовоеСообщениеОбОшибкеВОтчете.Описание = 
			НСтр("ru = 'Получение ликвидационной бухгалтерской отчетности не поддерживается Сбербанком. Измените тип на обычный или выберите другой экземпляр бухгалтерской отчетности.';
				|en = 'Получение ликвидационной бухгалтерской отчетности не поддерживается Сбербанком. Измените тип на обычный или выберите другой экземпляр бухгалтерской отчетности.'");
		НовоеСообщениеОбОшибкеВОтчете.ОтчетДок = СтруктураРеквизитовФормы.Отчеты_Бухотчетность;
		НовоеСообщениеОбОшибкеВОтчете.Отчет = РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(
			СтруктураРеквизитовФормы.Отчеты_Бухотчетность);
	...
	Если НЕ РезультатВыгрузки.РеглОтчетВыгружен Тогда
		...
		Для Каждого СообщениеОбОшибкеВОтчете Из РезультатВыгрузки.ТаблицаСообщений Цикл
			НовоеСообщениеОбОшибкеВОтчете = ТаблицаСообщений.Добавить();
			ЗаполнитьЗначенияСвойств(НовоеСообщениеОбОшибкеВОтчете, СообщениеОбОшибкеВОтчете);
	...
	Если ЗначениеЗаполнено(РезультатПолученияФайлов.ОписаниеОшибки) Тогда
		СтатусПроверки = Ложь;
		
		НовоеСообщениеОбОшибкеВОтчете = ТаблицаСообщений.Добавить();
		НовоеСообщениеОбОшибкеВОтчете.Раздел = "Баланс";
		НовоеСообщениеОбОшибкеВОтчете.Описание = РезультатПолученияФайлов.ОписаниеОшибки;
		НовоеСообщениеОбОшибкеВОтчете.ОтчетДок = СтруктураРеквизитовФормы.Отчеты_Бухотчетность;
		НовоеСообщениеОбОшибкеВОтчете.Отчет = РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(
			СтруктураРеквизитовФормы.Отчеты_Бухотчетность);
	...
	// Проверка схождения актива и пассива.
...
Процедура ЗафиксироватьОшибкуЗаполнения(СтатусПроверки, Описание, Раздел, ИмяЯчейки = "", ФИО = "")
	...
	СообщитьВТаблицуСообщений(ЭтотОбъект, Описание, Раздел, ИмяЯчейки, ФИО);
	СтатусПроверки = Ложь;
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	ИначеЕсли ИмяРаздела = "ОтчетыФНС" ИЛИ ИмяРаздела = "БухОтчетность" Тогда
...
Процедура АктивизироватьЯчейкуВТаблице(Ячейка)
	...
	Если НЕ ТекущаяСтраницаРаздела = Неопределено Тогда
		...
		Элементы.РазделыОтчета.ТекущаяСтрока = ТекущаяСтраницаРаздела.ПолучитьИдентификатор();
	КонецЕсли;
	...
	Иначе
		...
		Иначе
			...
			Если НЕ НайденнаяОбласть = Неопределено Тогда
				ЭлементОтображения.ТекущаяОбласть = НайденнаяОбласть;
			Иначе
				ЭлементОтображения.ТекущаяОбласть = ТабличныйДокумент.Область(Ячейка.ИмяЯчейки);
			КонецЕсли;
...
#Область ФормированиеЭлектронногоПредставления
...
Функция ИзвлечьСтруктуруXML()
	...
	Если ДействуетСВерсииФормата("5.11.01") Тогда
		ИмяСхемыВыгрузки = "СхемаВыгрузки5_11_01";
	Иначе
		ИмяСхемыВыгрузки = "СхемаВыгрузки506_29";
	КонецЕсли;
	
	ИмяОтчета = Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7);
	Макет = Отчеты[ИмяОтчета].ПолучитьМакет(ИмяСхемыВыгрузки);
...
Процедура ВывестиУзел_Баланс(Компоновщик, ПараметрыВыгрузки, СтруктураВыгрузки)
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела.Строки Цикл
		ДанныеСтраницы = ОбщегоНазначения.СкопироватьРекурсивно(СтраницаРаздела.Данные);
		...
		ДанныеСтраницы.Вставить("ХешСумма", ХешСуммаФайлаРаздела(ИмяРаздела, ПараметрыВыгрузки));
...
Процедура ВывестиУзел_РасшифровкиОбеспечений(Компоновщик, ПараметрыВыгрузки, СтруктураВыгрузки)
	...
	Если ДействуетСВерсииФормата("5.10.01") И СтраницаРаздела.Параметры.СтатусЗаполнения = "пустой" Тогда
		...
		ВывестиХешСумму(Компоновщик, ИмяРазделаПолученные, ПараметрыВыгрузки);
	...
	ИначеЕсли РазделВключенВОтчет(ИмяРазделаПолученные, ПараметрыВыгрузки.ДеревоНастройкиСтраниц)
	   И СтраницаРаздела.ДополнительныеДанные <> Неопределено Тогда
		ВывестиСтраницуВУзел_РасшифровкиОбеспеченийПолученные(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки);
	...
	Если ДействуетСВерсииФормата("5.10.01") И СтраницаРаздела.Параметры.СтатусЗаполнения = "пустой" Тогда
		...
		ВывестиАтрибут(Компоновщик, "ОтчетПустой", "true");
		ВывестиХешСумму(Компоновщик, ИмяРазделаВыданные, ПараметрыВыгрузки);
	...
	ИначеЕсли РазделВключенВОтчет(ИмяРазделаВыданные, ПараметрыВыгрузки.ДеревоНастройкиСтраниц)
	   И СтраницаРаздела.ДополнительныеДанные <> Неопределено Тогда
		ВывестиСтраницуВУзел_РасшифровкиОбеспеченийВыданные(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки);
...
&НаСервере
Процедура ВывестиСтраницуВУзел_РасшифровкиОбеспеченийПолученные(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки)
	...
	ВывестиХешСумму(Компоновщик, "РасшифровкиОбеспеченийПолученные", ПараметрыВыгрузки);
...
&НаСервере
Процедура ВывестиСтраницуВУзел_РасшифровкиОбеспеченийВыданные(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки)
	...
	ВывестиХешСумму(Компоновщик, "РасшифровкиОбеспеченийВыданные", ПараметрыВыгрузки);
...
Процедура ВывестиУзел_ЗадолженностьПоКредитамЗаймам(Компоновщик, ПараметрыВыгрузки, СтруктураВыгрузки)
	...
	Если ДействуетСВерсииФормата("5.10.01") И СтраницаРаздела.Параметры.СтатусЗаполнения = "пустой" Тогда
		...
		ВывестиАтрибут(Компоновщик, "ОтчетПустой", "true");
		ВывестиХешСумму(Компоновщик, ИмяРазделаКраткосрочная, ПараметрыВыгрузки);
	...
	ИначеЕсли РазделВключенВОтчет(ИмяРазделаКраткосрочная, ПараметрыВыгрузки.ДеревоНастройкиСтраниц) Тогда
		...
		ПараметрыУзла.Вставить("СтраницаРаздела",   СтраницаРаздела);
		ПараметрыУзла.Вставить("ПрефиксРаздела",    "П00007");
		ПараметрыУзла.Вставить("ИмяУзлаСтраницы",   "ОСВ_66");
		ПараметрыУзла.Вставить("ИмяРаздела",        ИмяРазделаКраткосрочная);
		ПараметрыУзла.Вставить("ПараметрыВыгрузки", ПараметрыВыгрузки);
		
		ВывестиСтраницуВУзел_ЗадолженностьПоКредитамЗаймам(Компоновщик, ПараметрыУзла);
	...
	Если ДействуетСВерсииФормата("5.10.01") И СтраницаРаздела.Параметры.СтатусЗаполнения = "пустой" Тогда
		...
		ВывестиАтрибут(Компоновщик, "ОтчетПустой", "true");
		ВывестиХешСумму(Компоновщик, ИмяРазделаДолгосрочная, ПараметрыВыгрузки);
	...
	ИначеЕсли РазделВключенВОтчет(ИмяРазделаДолгосрочная, ПараметрыВыгрузки.ДеревоНастройкиСтраниц) Тогда
		ПараметрыУзла = Новый Структура;
		ПараметрыУзла.Вставить("СтраницаРаздела",   СтраницаРаздела);
		ПараметрыУзла.Вставить("ПрефиксРаздела",    "П00008");
		ПараметрыУзла.Вставить("ИмяУзлаСтраницы",   "ОСВ_67");
		ПараметрыУзла.Вставить("ИмяРаздела",        ИмяРазделаДолгосрочная);
		ПараметрыУзла.Вставить("ПараметрыВыгрузки", ПараметрыВыгрузки);
...
&НаСервере
Процедура ВывестиСтраницуВУзел_ЗадолженностьПоКредитамЗаймам(Компоновщик, ПараметрыУзла)
	
	СтраницаРаздела   = ПараметрыУзла.СтраницаРаздела;
	ПрефиксРаздела    = ПараметрыУзла.ПрефиксРаздела;
	ИмяУзлаСтраницы   = ПараметрыУзла.ИмяУзлаСтраницы;
	ИмяРаздела        = ПараметрыУзла.ИмяРаздела;
	ПараметрыВыгрузки = ПараметрыУзла.ПараметрыВыгрузки;
	
	ФорматнаяСтрока = "ЧДЦ=3; ЧС=3; ЧН=; ЧГ=0";
	
	Компоновщик.ЗаписатьНачалоЭлемента(ИмяУзлаСтраницы);
	
	ВывестиХешСумму(Компоновщик, ИмяРаздела, ПараметрыВыгрузки);
...
Процедура ВывестиУзел_ЛизингОбязательства(Компоновщик, ПараметрыВыгрузки, СтруктураВыгрузки)
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела.Строки Цикл
		...
		Если ДействуетСВерсииФормата("5.10.01") И ПараметрыСтраницы.СтатусЗаполнения = "пустой" Тогда
			ВывестиСтраницуВУзел_ЛизингОбязательства_ОтчетПустой(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки);
		Иначе
			ВывестиСтраницуВУзел_ЛизингОбязательства(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки);
		КонецЕсли;
...
&НаСервере
Процедура ВывестиСтраницуВУзел_ЛизингОбязательства(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки)
	...
	ВывестиХешСумму(Компоновщик, "ЛизингОбязательства", ПараметрыВыгрузки);
...
&НаСервере
Процедура ВывестиСтраницуВУзел_ЛизингОбязательства_ОтчетПустой(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки)
...
Процедура ВывестиУзел_ОСВ(Компоновщик, ПараметрыВыгрузки, СтруктураВыгрузки)
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела.Строки Цикл
		...
		Если СтраницаВыгружается Тогда
			ВывестиСтраницуВУзел_ОСВ(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки);
		КонецЕсли;
...
&НаСервере
Процедура ВывестиСтраницуВУзел_ОСВ(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки)
	...
	ПараметрыУзла = ОтчетностьВБанкиСлужебный.НовыеПараметрыУзла_ОСВ();
	ПараметрыУзла.Период = КодМесяца;
	ПараметрыУзла.ОтчетГод = ОтчетГод;
	ПараметрыУзла.ХешСумма = ХешСуммаФайлаРаздела("ОСВ", ПараметрыВыгрузки);
	ПараметрыУзла.ВерсияОтчета = СтруктураРеквизитовФормы.ВерсияФормата;
	
	ОтчетностьВБанкиСлужебный.ВывестиСтраницуВУзел_ОСВ(Компоновщик, СтрокиОСВ, ПараметрыУзла);
...
Процедура ВывестиУзел_ОСВПоСчетам(Компоновщик, ПараметрыВыгрузки, СтруктураВыгрузки)
	...
	Если ВыгружаемыеСчета.Количество() > 0 Тогда
		Если ДействуетСВерсииФормата("5.12.10") Тогда
			// Выгрузка за квартал.
			КодКвартала = КодКварталаОСВ(НомерКвартала);
			...
			Для Каждого ВыгружаемыйСчет Из ВыгружаемыеСчета Цикл
				ВывестиСтраницуВУзел_ОСВПоСчетам_ОСВПоСчету_5_11_01(Компоновщик, ВыгружаемыйСчет, ПараметрыВыгрузки, "ОСВПоСчетуКв");
			КонецЦикла;
			
			Компоновщик.ЗаписатьКонецЭлемента(); // ОСВПоСчетам
			
			// Выгрузка с начала года по конец квартала.
			Если НомерКвартала <> 1 Тогда
				КодСНачалаГода = КодСНачалаГода(НомерКвартала);
				ОтчетГод  = ПредставлениеГода(ГодОтчета);
				ОКЕИ      = "383";
				
				Компоновщик.ЗаписатьНачалоЭлемента("ОСВПоСчетам");
				
				ВывестиАтрибут(Компоновщик, "Период",   КодСНачалаГода);
				ВывестиАтрибут(Компоновщик, "ОтчетГод", ОтчетГод);
				ВывестиАтрибут(Компоновщик, "ОКЕИ", ОКЕИ);
				
				Для Каждого ВыгружаемыйСчет Из ВыгружаемыеСчета Цикл
					ВывестиСтраницуВУзел_ОСВПоСчетам_ОСВПоСчету_5_11_01(Компоновщик, ВыгружаемыйСчет, ПараметрыВыгрузки, "ОСВПоСчетуНГ");
				КонецЦикла;
		...
		Иначе
			...
			Для Каждого ВыгружаемыйСчет Из ВыгружаемыеСчета Цикл
				Если ДействуетСВерсииФормата("5.11.01") Тогда
					ВывестиСтраницуВУзел_ОСВПоСчетам_ОСВПоСчету_5_11_01(Компоновщик, ВыгружаемыйСчет, ПараметрыВыгрузки, "ОСВПоСчету");
				Иначе
					ВывестиСтраницуВУзел_ОСВПоСчетам_ОСВПоСчету(Компоновщик, ВыгружаемыйСчет, ПараметрыВыгрузки);
				КонецЕсли;
			КонецЦикла;
...
&НаСервере
Процедура ВывестиСтраницуВУзел_ОСВПоСчетам_ОСВПоСчету(Компоновщик, ВыгружаемыйСчет, ПараметрыВыгрузки)
	...
	ВывестиХешСуммуПоСчету(Компоновщик, ИмяРаздела, ВыгружаемыйСчет.Код, ПараметрыВыгрузки);
	
	СегментыСтраницы = ВыборкаСегментовОтчетаПоСчету(ИмяРаздела, ВыгружаемыйСчет);
	ВсегоСегментов = СегментыСтраницы.Количество();
	
	Для Инд = 1 По ВсегоСегментов Цикл
		ВидДополнительногоФайла = ИмяРаздела + "." + ВыгружаемыйСчет.УникальныйИдентификатор() + "." + СтрЧГ0(Инд);
...
&НаСервере
Процедура ВывестиСтраницуВУзел_ОСВПоСчетам_ОСВПоСчету_5_11_01(Компоновщик, ВыгружаемыйСчет, ПараметрыВыгрузки, ПрефиксСегментов)
	...
	ВывестиХешСуммуПоСчету(Компоновщик, "ОСВПоСчету", ВыгружаемыйСчет.Код, ПараметрыВыгрузки);
	
	СегментыСтраницы = ВыборкаСегментовОтчетаПоСчету(ПрефиксСегментов, ВыгружаемыйСчет);
	ВсегоСегментов = СегментыСтраницы.Количество();
	
	ВыводимыйСубсчет = Неопределено;
	ЗакрыватьУзел_Счет = Ложь;
	
	ОтражатьКоличество = ОтражатьКоличествоВОСВПоСчету(ВыгружаемыйСчет);
	
	Для Инд = 1 По ВсегоСегментов Цикл
		ВидДополнительногоФайла = ПрефиксСегментов + "." + ВыгружаемыйСчет.УникальныйИдентификатор() + "." + СтрЧГ0(Инд);
		ДанныеСегмента = ДанныеРегистраДополнительныхФайлов(ВидДополнительногоФайла);
		
		Для Каждого СтрокаПоСчету Из ДанныеСегмента.Строки Цикл
			Если ВыводимыйСубсчет <> СтрокаПоСчету.Счет Тогда
				Если ВыводимыйСубсчет <> Неопределено Тогда
					Компоновщик.ЗаписатьКонецЭлемента(); // Счет
					ЗакрыватьУзел_Счет = Ложь;
				КонецЕсли;
				
				ВыводимыйСубсчет = СтрокаПоСчету.Счет;
				
				Компоновщик.ЗаписатьНачалоЭлемента("Счет");
				ЗакрыватьУзел_Счет = Истина;
				
				ВывестиАтрибут(Компоновщик, "КодСчета", СтрокаПоСчету.КодСчета, "Т,8");
				
				ВывестиАтрибут(Компоновщик, "СНД", СтрокаПоСчету.СНД, "Ч,15,2");
				ВывестиАтрибут(Компоновщик, "СНК", СтрокаПоСчету.СНК, "Ч,15,2");
				ВывестиАтрибут(Компоновщик, "ДО",  СтрокаПоСчету.ДО,  "Ч,15,2");
				ВывестиАтрибут(Компоновщик, "КО",  СтрокаПоСчету.КО,  "Ч,15,2");
				ВывестиАтрибут(Компоновщик, "СКД", СтрокаПоСчету.СКД, "Ч,15,2");
				ВывестиАтрибут(Компоновщик, "СКК", СтрокаПоСчету.СКК, "Ч,15,2");
				
				Если ОтражатьКоличество
				   И РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(СтрокаПоСчету, "КНОД") Тогда
					ВывестиАтрибут(Компоновщик, "КНОД", СтрокаПоСчету.КНОД, "Ч,15,3");
					ВывестиАтрибут(Компоновщик, "КНОК", СтрокаПоСчету.КНОК, "Ч,15,3");
					ВывестиАтрибут(Компоновщик, "КДО",  СтрокаПоСчету.КДО,  "Ч,15,3");
					ВывестиАтрибут(Компоновщик, "ККО",  СтрокаПоСчету.ККО,  "Ч,15,3");
					ВывестиАтрибут(Компоновщик, "ККОД", СтрокаПоСчету.ККОД, "Ч,15,3");
					ВывестиАтрибут(Компоновщик, "ККОК", СтрокаПоСчету.ККОК, "Ч,15,3");
				КонецЕсли;
			...
			Для Каждого Субконто1 Из СтрокаПоСчету.Строки Цикл
				...
				ПредставлениеСубконто = ПредставлениеСубконтоИННКПП(Субконто1, 1);
				
				ВывестиАтрибут(Компоновщик, "Представление", ПредставлениеСубконто, "Т,1000");
				ВывестиАтрибут(Компоновщик, "ТипСубконто", Строка(ТипЗнч(Субконто1.Субконто1)), "Т,100");
				ВывестиНеобязательныйАтрибут(Компоновщик, "ИНН", ИННКонтрагентаПоСубконто(Субконто1, 1), "Т,12");
				
				ВывестиАтрибут(Компоновщик, "СНД", Субконто1.СНД, "Ч,15,2");
				ВывестиАтрибут(Компоновщик, "СНК", Субконто1.СНК, "Ч,15,2");
				ВывестиАтрибут(Компоновщик, "ДО",  Субконто1.ДО,  "Ч,15,2");
				ВывестиАтрибут(Компоновщик, "КО",  Субконто1.КО,  "Ч,15,2");
				ВывестиАтрибут(Компоновщик, "СКД", Субконто1.СКД, "Ч,15,2");
				ВывестиАтрибут(Компоновщик, "СКК", Субконто1.СКК, "Ч,15,2");
			
				Если ОтражатьКоличество
				   И РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Субконто1, "КНОД") Тогда
					ВывестиАтрибут(Компоновщик, "КНОД", Субконто1.КНОД, "Ч,15,3");
					ВывестиАтрибут(Компоновщик, "КНОК", Субконто1.КНОК, "Ч,15,3");
					ВывестиАтрибут(Компоновщик, "КДО",  Субконто1.КДО,  "Ч,15,3");
					ВывестиАтрибут(Компоновщик, "ККО",  Субконто1.ККО,  "Ч,15,3");
					ВывестиАтрибут(Компоновщик, "ККОД", Субконто1.ККОД, "Ч,15,3");
					ВывестиАтрибут(Компоновщик, "ККОК", Субконто1.ККОК, "Ч,15,3");
				КонецЕсли;
				
				Для Каждого Субконто2 Из Субконто1.Строки Цикл
					Компоновщик.ЗаписатьНачалоЭлемента("Субконто2");
					
					ПредставлениеСубконто = ПредставлениеСубконтоИННКПП(Субконто2, 2);
					
					ВывестиАтрибут(Компоновщик, "Представление", ПредставлениеСубконто, "Т,1000");
					ВывестиАтрибут(Компоновщик, "ТипСубконто", Строка(ТипЗнч(Субконто2.Субконто2)), "Т,100");
					ВывестиНеобязательныйАтрибут(Компоновщик, "ИНН", ИННКонтрагентаПоСубконто(Субконто2, 2), "Т,12");
					
					ВывестиАтрибут(Компоновщик, "СНД", Субконто2.СНД, "Ч,15,2");
					ВывестиАтрибут(Компоновщик, "СНК", Субконто2.СНК, "Ч,15,2");
					ВывестиАтрибут(Компоновщик, "ДО",  Субконто2.ДО,  "Ч,15,2");
					ВывестиАтрибут(Компоновщик, "КО",  Субконто2.КО,  "Ч,15,2");
					ВывестиАтрибут(Компоновщик, "СКД", Субконто2.СКД, "Ч,15,2");
					ВывестиАтрибут(Компоновщик, "СКК", Субконто2.СКК, "Ч,15,2");
					
					Если ОтражатьКоличество
					   И РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Субконто2, "КНОД") Тогда
						ВывестиАтрибут(Компоновщик, "КНОД", Субконто2.КНОД, "Ч,15,3");
						ВывестиАтрибут(Компоновщик, "КНОК", Субконто2.КНОК, "Ч,15,3");
						ВывестиАтрибут(Компоновщик, "КДО",  Субконто2.КДО,  "Ч,15,3");
						ВывестиАтрибут(Компоновщик, "ККО",  Субконто2.ККО,  "Ч,15,3");
						ВывестиАтрибут(Компоновщик, "ККОД", Субконто2.ККОД, "Ч,15,3");
						ВывестиАтрибут(Компоновщик, "ККОК", Субконто2.ККОК, "Ч,15,3");
					КонецЕсли;
...
Процедура ВывестиУзел_АнализСчетов(Компоновщик, ПараметрыВыгрузки, СтруктураВыгрузки)
	...
	Если ВыгружаемыеСчета.Количество() > 0 Тогда
		...
		Для Каждого ВыгружаемыйСчет Из ВыгружаемыеСчета Цикл
			Если ДействуетСВерсииФормата("5.11.01") Тогда
				ВывестиСтраницуВУзел_АнализСчетов_АнализСчета_5_11_01(Компоновщик, ВыгружаемыйСчет, ПараметрыВыгрузки);
			Иначе
				ВывестиСтраницуВУзел_АнализСчетов_АнализСчета(Компоновщик, ВыгружаемыйСчет, ПараметрыВыгрузки);
			КонецЕсли;
		КонецЦикла;
...
&НаСервере
Процедура ВывестиСтраницуВУзел_АнализСчетов_АнализСчета(Компоновщик, ВыгружаемыйСчет, ПараметрыВыгрузки)
...
&НаСервере
Процедура ВывестиСтраницуВУзел_АнализСчетов_АнализСчета_5_11_01(Компоновщик, ВыгружаемыйСчет, ПараметрыВыгрузки)
	...
	ОтражатьКоличество = ОтражатьКоличествоВАнализеСчета(ВыгружаемыйСчет);
	...
	Для Инд = 1 По ВсегоСегментов Цикл
		...
		Для Каждого СтрокаПоСчету Из ДанныеСегмента.Строки Цикл
			...
			Для Каждого Субконто1 Из СтрокаПоСчету.Строки Цикл
				Если ЗначениеЗаполнено(Субконто1.КорСчет) Тогда
					Корреспонденция = Субконто1;
					ВывестиУниверсальныйУзел_Корреспонденция(Компоновщик, Корреспонденция);
...
Процедура ВывестиУзел_ОперБанкСч(Компоновщик, ПараметрыВыгрузки)
	...
	Если НЕ ДействуетСВерсииФормата("5.11.01") Тогда
		ПараметрыВыгрузкиУзла = Новый Структура;
		ПараметрыВыгрузкиУзла.Вставить("ВыгружатьАтрибутЗабалансовый", Ложь);
		ОтчетностьВБанкиСлужебный.ВывестиУзел_БухгалтерскиеСчета(Компоновщик, ПараметрыВыгрузкиУзла);
	КонецЕсли;
...
Процедура ВывестиУзел_ПрочДохРасх(Компоновщик, ПараметрыВыгрузки)
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела.Строки Цикл
		...
		Если СтраницаВыгружается Тогда
			ВывестиСтраницуВУзел_ПрочДохРасх(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки);
		КонецЕсли;
...
&НаСервере
Процедура ВывестиСтраницуВУзел_ПрочДохРасх(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки)
	...
	ВывестиХешСумму(Компоновщик, "ДоходыРасходы", ПараметрыВыгрузки);
...
Процедура ВывестиУзел_РасшФинВлож(Компоновщик, ПараметрыВыгрузки, СтруктураВыгрузки)
	...
	Если РазделВключенВОтчет(ИмяРазделаКраткосрочная, ПараметрыВыгрузки.ДеревоНастройкиСтраниц) Тогда
		...
		ПараметрыУзла = Новый Структура;
		ПараметрыУзла.Вставить("СтраницаРаздела",   СтраницаРаздела);
		ПараметрыУзла.Вставить("ПрефиксРаздела",    "П00012");
		ПараметрыУзла.Вставить("ИмяУзлаСтраницы",   "ФинВложКср");
		ПараметрыУзла.Вставить("ИмяРаздела",        ИмяРазделаКраткосрочная);
		ПараметрыУзла.Вставить("ПараметрыВыгрузки", ПараметрыВыгрузки);
		
		Если ДействуетСВерсииФормата("5.10.01") И СтраницаРаздела.Параметры.СтатусЗаполнения = "пустой" Тогда
			ВывестиСтраницуВУзел_РасшФинВлож_ОтчетПустой(Компоновщик, ПараметрыУзла);
		Иначе
			ВывестиСтраницуВУзел_РасшФинВлож(Компоновщик, ПараметрыУзла);
		КонецЕсли;
	...
	Если РазделВключенВОтчет(ИмяРазделаДолгосрочная, ПараметрыВыгрузки.ДеревоНастройкиСтраниц) Тогда
		...
		ПараметрыУзла = Новый Структура;
		ПараметрыУзла.Вставить("СтраницаРаздела",   СтраницаРаздела);
		ПараметрыУзла.Вставить("ПрефиксРаздела",    "П00013");
		ПараметрыУзла.Вставить("ИмяУзлаСтраницы",   "ФинВложДср");
		ПараметрыУзла.Вставить("ИмяРаздела",        ИмяРазделаДолгосрочная);
		ПараметрыУзла.Вставить("ПараметрыВыгрузки", ПараметрыВыгрузки);
...
&НаСервере
Процедура ВывестиСтраницуВУзел_РасшФинВлож(Компоновщик, ПараметрыУзла)
...
&НаСервере
Процедура ВывестиСтраницуВУзел_РасшФинВлож_ОтчетПустой(Компоновщик, ПараметрыУзла)
...
Процедура ВывестиУзел_СтруктВыр(Компоновщик, ПараметрыВыгрузки)
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела.Строки Цикл
		ВывестиСтраницуВУзел_СтруктВыр(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки);
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Процедура ВывестиСтраницуВУзел_СтруктВыр(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки)
	...
	ВывестиХешСумму(Компоновщик, "СтруктураВыручки", ПараметрыВыгрузки);
...
Процедура ВывестиУзел_ПрочиеПоказатели(Компоновщик, ПараметрыВыгрузки)
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела.Строки Цикл
		ВывестиСтраницуВУзел_ПрочиеПоказатели(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки);
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Процедура ВывестиСтраницуВУзел_ПрочиеПоказатели(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки)
	...
	ВывестиХешСумму(Компоновщик, "ПрочиеПоказатели", ПараметрыВыгрузки);
...
Процедура ВывестиУзел_ОперБанкСч_СчетУчета_БанкСч(Компоновщик, СтрокиСчетов, Знач ПрефиксИдентификатораДанных)
	...
	Для Каждого СтрокаСчета Из СтрокиСчетов Цикл
		...
		ЭтоВалютныйСчет = (ТребуетсяРазыменование И СтрокаСчета.Счет.Валютный)
		              ИЛИ (НЕ ТребуетсяРазыменование И СтрокаСчета.Валютный);
		...
		Если ОписаниеТипаБанковскиеСчета.СодержитТип(ТипЗнч(СтрокаСчета.Счет)) Тогда
			...
			Если НЕ ТребуетсяРазыменование Тогда
				...
				Если ДействуетСВерсииФормата("5.12.11")
				   И РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(СтрокаСчета, "СВИФТБИК") Тогда
			...
			Если ВыборкаСегментов.Количество() > 0 Тогда
				Для НомерСекции = 1 По ВыборкаСегментов.Количество() Цикл
					ВидДополнительногоФайла = ПрефиксИДПоБС + "." + СтрЧГ0(НомерСекции);
					СтрокиСекции = ДанныеРегистраДополнительныхФайлов(ВидДополнительногоФайла);
					...
					Если СтрокиСекции <> Неопределено Тогда
						Для Каждого СтрокаСекции Из СтрокиСекции Цикл
							...
							Если ДействуетСВерсииФормата("5.12.05") Тогда
							...
							Если ДействуетСВерсииФормата("5.12.01") Тогда
							...
							Если ЗначениеЗаполнено(СтрокаСекции.Контрагент) Тогда
								...
								КонецЕсли;
								Если ДействуетСВерсииФормата("5.12.11")
								   И РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(СтрокаСекции, "СчетКонтрагентаСВИФТБИК") Тогда
							...
							Если ДействуетСВерсииФормата("5.12.07") И ЗначениеЗаполнено(СтрокаСекции.ПП_КБК_104) Тогда
...
Процедура ВывестиУзел_ДебКредЗадолж(Компоновщик, ПараметрыВыгрузки)
	...
	Если Интервалы = Неопределено
	   И РазделВключенВОтчет("Кредиторская", ПараметрыВыгрузки.ДеревоНастройкиСтраниц) Тогда
		ДанныеРаздела = ДанныеМетаРаздела.Строки[0].Строки[1].Строки[0];
		Если НЕ (ДействуетСВерсииФормата("5.10.01") И ДанныеРаздела.Параметры.СтатусЗаполнения = "пустой") Тогда
			Интервалы = ДанныеРаздела.ДополнительныеДанные.Интервалы;
		КонецЕсли;
	...
	Если РазделВключенВОтчет("Дебиторская", ПараметрыВыгрузки.ДеревоНастройкиСтраниц) Тогда
		...
		Если ДействуетСВерсииФормата("5.10.01") Тогда
			...
			ВывестиХешСумму(Компоновщик, "Дебиторская", ПараметрыВыгрузки);
		КонецЕсли;
	...
	Если РазделВключенВОтчет("Кредиторская", ПараметрыВыгрузки.ДеревоНастройкиСтраниц) Тогда
		...
		Если ДействуетСВерсииФормата("5.10.01") Тогда
			...
			ВывестиХешСумму(Компоновщик, "Кредиторская", ПараметрыВыгрузки);
		КонецЕсли;
	...
	Для Каждого ЗадолженностьПоПредметуУчета Из ЗадолженностьПоВиду.Строки Цикл
		...
		Если ЗначениеЗаполнено(ЗадолженностьПоПредметуУчета.Контрагент) Тогда
			...
			Для Каждого Интервал Из Интервалы Цикл
				...
				ИмяКолонки = "ПросроченнаяПоСроку" + СтрЧГ0(Интервал.Код);
...
Процедура ВывестиУзел_ДопФайлы(Компоновщик, ПараметрыВыгрузки)
	...
	Для НомерФайла = 1 По СтруктураРеквизитовФормы.ДополнительныеФайлы.Количество() Цикл
			ЗаписьРегистраСведений = РегистрыСведений.ДополнительныеФайлыРегламентированныхОтчетов.СоздатьМенеджерЗаписи();
			ЗаписьРегистраСведений.РегламентированныйОтчет = СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка;
			ЗаписьРегистраСведений.ВидДополнительногоФайла = "ДополнительныйФайл." + СтрЧГ0(НомерФайла);
...
&НаСервере
Функция ХешСуммаФайлаРаздела(ИмяРаздела, ПараметрыВыгрузки)
	
	СформированныеФайлы = ПараметрыВыгрузки.СформированныеФайлы;
	
	ХешСумма = "";
	
	СтрокаФайла = СформированныеФайлы.Найти(ИмяРаздела, "ИмяРаздела");
	
	Если СтрокаФайла <> Неопределено Тогда
		ХешСумма = СтрокаФайла.ХешСумма;
	КонецЕсли;
...
&НаСервере
Процедура ВывестиХешСумму(Компоновщик, ИмяРаздела, ПараметрыВыгрузки)
	...
	Если ДействуетСВерсииФормата("5.11.01") Тогда
		...
		Если СтрокаФайла <> Неопределено Тогда
			ВывестиАтрибут(Компоновщик, "ХешСумма", СтрокаФайла.ХешСумма);
		КонецЕсли;
...
&НаСервере
Процедура ВывестиХешСуммуПоСчету(Компоновщик, ИмяРаздела, КодСчета, ПараметрыВыгрузки);
	
	Если ДействуетСВерсииФормата("5.11.01") Тогда
		СформированныеФайлы = ПараметрыВыгрузки.СформированныеФайлы;
		
		УсловияПоиска = Новый Структура("ИмяРаздела, КодСчета", ИмяРаздела, КодСчета);
		СтрокаФайла = СформированныеФайлы.НайтиСтроки(УсловияПоиска);
		
		Если СтрокаФайла.Количество() > 0 Тогда
			ВывестиАтрибут(Компоновщик, "ХешСумма", СтрокаФайла[0].ХешСумма);
		КонецЕсли;
		 ...
		 ИЛИ ЗаполненаСтрокаМСЧ(ПараметрыВыгрузки, Сведения, ШаблонВыгрузки, НомерСтроки) Тогда
			...
			Для Каждого СтрокаШаблона Из ШаблонВыгрузки.Строки Цикл
				Если СтрокаШаблона.Тип = "А" Тогда
					ЗначениеАтрибута = Неопределено;
					Если ЗначениеЗаполнено(СтрокаШаблона.Ключ) Тогда
						ИмяПоказателя = СтрокаШаблона.Ключ + "_" + СтрЧГ0(НомерСтроки);
...
Функция ФорматАтрибута(СтрокаШаблона)
	...
	Если СтрокаШаблона.Формат = "T" Тогда
		...
		Если ЗначениеЗаполнено(СтрокаШаблона.МаксРазмерность) Тогда
			ФорматАтрибута = ФорматАтрибута + "," + СтрЧГ0(СтрокаШаблона.МаксРазмерность);
		КонецЕсли;
	...
	ИначеЕсли СтрокаШаблона.Формат = "N" Тогда
		...
		Если ЗначениеЗаполнено(СтрокаШаблона.МаксРазмерность) И СтрокаШаблона.МаксРазмерность <> 99999 Тогда
			...
			Если ЗначениеЗаполнено(СтрокаШаблона.МинРазмерность) И СтрокаШаблона.МинРазмерность <> 99999 Тогда
				ФорматАтрибута = ФорматАтрибута + "," + СтрЧГ0(СтрокаШаблона.МинРазмерность);
			КонецЕсли;
		КонецЕсли;
...
&НаСервере
Функция КодКварталаОСВ(НомерКвартала)
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Функция КодКварталаОСВ():
			|непредусмотренный номер квартала';
			|en = 'Функция КодКварталаОСВ():
			|непредусмотренный номер квартала'");
...
&НаСервере
Функция КодСНачалаГода(НомерКвартала)
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Функция КодСНачалаГода():
			|непредусмотренный номер квартала';
			|en = 'Функция КодСНачалаГода():
			|непредусмотренный номер квартала'");
		
	КонецЕсли;
	
	Возврат КодСНачалаГода;
...
Функция РазделВключенВОтчет(ИмяРаздела, Знач СписокРазделов = Неопределено)
	...
	Если СписокРазделов = Неопределено Тогда
		СписокРазделов = РеквизитФормыВЗначение("мДеревоВыбранныхСтраниц");
	КонецЕсли;
...
&НаСервере
Процедура ОчиститьРазделОСВПоСчету()
	...
	ШаблонОтбора = ИмяРаздела + "%";
	ВыборкаСегментов = ВыборкаСегментовОтчетаПоШаблону(ШаблонОтбора);
...
Процедура ОчиститьТекущийРазделЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ИмяРаздела = "Титульный" И НЕ ИмяРаздела = "ОтчетыФНС" И НЕ ИмяРаздела = "БухОтчетность" Тогда
		СтруктураРаздела = мСтруктураМногоуровневыхРазделов[ИмяРаздела];
...
Процедура ВопросОчиститьТекущуюСтраницуЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		Если ИмяРаздела = "ОСВПоСчету" Тогда
			ОчиститьСтраницуРазделСоСпискомСтраниц("ОСВПоСчету");
			ОчиститьСтраницуРазделСоСпискомСтраниц("ОСВПоСчетуКв");
			ОчиститьСтраницуРазделСоСпискомСтраниц("ОСВПоСчетуНГ");
			ПоказатьРаздел(ИмяРаздела, СтруктураРеквизитовФормы.НомераСтрокМногоуровнегоРаздела);
			
		ИначеЕсли ИмяРаздела = "АнализСчета" Тогда
			ОчиститьСтраницуРазделСоСпискомСтраниц("АнализСчета");
			ПоказатьРаздел(ИмяРаздела, СтруктураРеквизитовФормы.НомераСтрокМногоуровнегоРаздела);
...
Процедура УстановитьДоступностьОбластей_ЗадолженностьПоКредитамЗаймам(Форма, Префикс)
	...
	Пока Истина Цикл
		...
		Если НЕ ЗначениеЗаполнено(ОбластьОпределитель.Значение)
		 ИЛИ ОбластьОпределитель.Значение = "RUB" Тогда
			...
			Для Инд = 20 По 25 Цикл
				ИмяОбласти = Префикс + "М1000" + СтрЧГ0(Инд) + Постфикс;
		...
		Пока Истина Цикл
			...
			Постфикс2 = Постфикс + "_" + СтрЧГ0(НомерСтроки2);
...
Процедура ОткрытьФормуВыбораСтраниц(ВариантВыбора)
	...
	ПараметрыОтображения.Вставить("ПроверкаСоотношений", Ложь);
	ПараметрыОтображения.Вставить("Автосохранение", Ложь);
	ПараметрыОтображения.Вставить("СчетчикСтраниц", Ложь);
	ПараметрыОтображения.Вставить("ЗначенияИзСписка", Ложь);
	ПараметрыОтображения.Вставить("ИмеетсяРеквизитДеревоВыбранныхСтраниц", Истина);
	ПараметрыОтображения.Вставить("СохранятьРасшифровкуАвтозаполнения", Ложь);
	ПараметрыОтображения.Вставить("РазрешитьВстроеннуюПечать", Ложь);
	ПараметрыОтображения.Вставить("ОтключитьАвтоРасчет", Ложь);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЕдиницаИзмерения", СтруктураРеквизитовФормы.ЕдиницаИзмерения);
	ПараметрыФормы.Вставить("ТочностьЕдиницыИзмерения", СтруктураРеквизитовФормы.ТочностьЕдиницыИзмерения);
	ПараметрыФормы.Вставить("ФлажокОтклАвтоРасчет", СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет);
	ПараметрыФормы.Вставить("мПараметры", ПараметрыОтображения);
	
	ФормаНастройкиОтчета = ПолучитьФорму("Отчет.БухгалтерскаяОтчетностьВБанк.Форма.НастройкиОтчета",
...
Процедура ОткрытьФормуВыбораСтраницЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ПроверитьИдентичностьДеревьевРазделов(ЗначенияИдентичны, мДеревоСтраницОтчета, мДеревоВыбранныхСтраниц);
	...
	Если ИсходноеЗначениеАвторасчета <> СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
		...
		Если СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет = Ложь Тогда
			ДополнительныеПараметры = Новый Структура(
				...
				НомерСтраницыРазделаОтчета, ТекущийРазделОтчетаСокрНаим, ЗначенияИдентичны);
			ОписаниеОповещения = Новый ОписаниеОповещения(
...
Процедура ОбнулитьВалютныеСуммы_ЛизингОбязательства(Область)
	...
	Если СтрНайти("П00006М100109", ОсновнаяЧастИмени) <> 0 Тогда
		Если НЕ ЗначениеЗаполнено(КодВалютыВОбласти) ИЛИ КодВалютыВОбласти = "RUB" Тогда
			...
			Пока Истина Цикл
				НомерСтроки = НомерСтроки + 1;
				Постфикс2 = Постфикс + "_" + СтрЧГ0(НомерСтроки);
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСписковВыбора = Отчеты[Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СпискиВыбора2017Кв3");
...
Процедура НастроитьОтчетПоСчетам(ИмяРаздела)
	...
	ФормаНастройкиСпискаСчетов = ПолучитьФорму("Отчет.БухгалтерскаяОтчетностьВБанк.Форма.НастройкаСпискаСчетов",
		ПараметрыФормы, ЭтотОбъект);
	
	КопироватьДанныеФормы(ЭтотОбъект["СписокСтраниц" + ИмяРаздела], ФормаНастройкиСпискаСчетов.СоставОтчетаПоСчетам);
...
Процедура НастроитьОтчетПоСчетамЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Для Инд = 0 По РезультатВыбора.ВГраница() Цикл
		СтрокаПоСчету = ЭтотОбъект["СписокСтраниц" + ИмяРаздела][Инд];
...
Процедура ПерейтиНаСегментОтчета(ИмяРаздела, ИмяКомандыПерехода)
	...
	Если ИмяРаздела = "ОСВПоСчету" Тогда
		...
		СчетУчета = НеотображаемыеПоказатели.Счет;
		ВыборкаСегментов   = ВыборкаСегментовОтчетаПоСчету(ПрефиксСегментов, СчетУчета);
		ВыборкаСегментовНГ = ВыборкаСегментовОтчетаПоСчету(ПрефиксСегментовНГ, СчетУчета);
		ВсегоСегментов = ВыборкаСегментов.Количество() + ВыборкаСегментовНГ.Количество();
...
Процедура СформироватьВывестиСекциюОтчета(ИмяРаздела, НомерСекции)
	...
	Если ИмяРаздела = "ОСВПоСчету" И ДействуетСВерсииФормата("5.11.01") Тогда
		Шаблон = МакетРаздела(ИмяРаздела + "5_11_01");
	Иначе
		Шаблон = МакетРаздела(ИмяРаздела);
	КонецЕсли;
	...
	Если ИмяРаздела = "ОСВПоСчету" Тогда
		Если ДействуетСВерсииФормата("5.11.01") Тогда
			СформироватьСекциюОСВПоСчету_5_11_01(МакетРаздела, Шаблон, ТекВетка, НомерСекции);
		Иначе
	...
	ИначеЕсли ИмяРаздела = "АнализСчета" Тогда
		Если ДействуетСВерсииФормата("5.11.01") Тогда
			СформироватьСекциюАнализаСчета_5_11_01(МакетРаздела, Шаблон, ТекВетка, НомерСекции);
		Иначе
...
&НаКлиенте
Процедура ПредставлениеБухОтчетностиОбработкаВыбора(РезультатВыбора, ДополнительныеПараметры) Экспорт
	
	Если РезультатВыбора <> Неопределено Тогда
		СтруктураРеквизитовФормы.Отчеты_Бухотчетность = РезультатВыбора.РегламентированныйОтчет;
		СтруктураРеквизитовФормы.Отчеты_БухотчетностьПредставление = РезультатВыбора.СостояниеОтчета;
		
		ОтметитьРазделКакЗаполненный("БухОтчетность");
...
&НаКлиенте
Процедура ВыгрузитьЗавершение(ПараметрыФормированияXML, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяПакетаДляБанка) Тогда
		...
		ФайловаяСистемаКлиент.СохранитьФайл(ОбработчикЗавершения, АдресДанныхДляСохранения,
			ИмяПакетаДляБанка, ПараметрыСохранения);
...
&НаКлиенте
Функция НовыеПараметрыФормированияXML()
	
	ПараметрыФормированияXML = Новый Структура();
	ПараметрыФормированияXML.Вставить("IPАдрес", "");
	ПараметрыФормированияXML.Вставить("MACАдрес", "");
	ПараметрыФормированияXML.Вставить("НомерВыгрузки", "");
	ПараметрыФормированияXML.Вставить("ПрефиксИмениФайлаПредставления", "");
	ПараметрыФормированияXML.Вставить("ПостфиксПредставлений", "");
	ПараметрыФормированияXML.Вставить("ДатаВыгрузки", '0001-01-01');
	
	Возврат ПараметрыФормированияXML;
	
КонецФункции

&НаКлиенте
Функция ПодготовитьОкружениеДляОтправкиИВыгрузки(ДвоичныеДанныеФайла, ПараметрыФормированияXML)
	
	ТаблицаСообщений.Очистить();
	
	ПараметрыФормированияXML.ДатаВыгрузки = ТекущаяДата();
	...
	МоментВыгрузки = Формат(ПараметрыФормированияXML.ДатаВыгрузки, "ДФ=yyyyMMddHHmmss");
	
	ПараметрыФормированияXML.ПрефиксИмениФайлаПредставления = ИНН
		+ "_" + ?(ЗначениеЗаполнено(КПП), КПП, "000000000")
		+ "_" + ГодОтчета + "_" + КварталОтчета
		+ "_" + МоментВыгрузки;
	
	ПараметрыФормированияXML.ПостфиксПредставлений = ИНН
		+ "_" + ?(ЗначениеЗаполнено(КПП), КПП, "000000000")
		+ "_" + ГодОтчета + "_" + МесяцОтчета
		+ "_1C_" + МоментВыгрузки;
	
	СформированТекстЭлектронногоПредставления = УниверсальнаяВыгрузкаXML(ДвоичныеДанныеФайла, ПараметрыФормированияXML);
	...
	ИмяПакетаДляБанка = ПараметрыФормированияXML.ПрефиксИмениФайлаПредставления
		+ "_" + ПараметрыФормированияXML.НомерВыгрузки + ".zip";
...
&НаСервере
Процедура СформироватьФайлЭлектронногоПредставленияОсновныеОтчеты(ПараметрыВыгрузки, ИмяВременногоФайла)
	...
	ОтчетностьВБанкиСлужебный.ВывестиГоловныеУзлыПакета(Компоновщик, ПараметрыВыгрузки);
	...
	ОтчетностьВБанкиСлужебный.ВывестиЗавершающиеУзлыПакета(Компоновщик);
...
&НаСервере
Процедура СформироватьФайлЭлектронногоПредставленияБанковскиеОперации(ПараметрыВыгрузки, ИмяВременногоФайла)
...
Функция СформироватьФайлыПредставленийРазделов(ВременныйКаталогПакета, ПостфиксПредставлений)
	...
	СформированныеФайлы = Новый ТаблицаЗначений;
	СформированныеФайлы.Колонки.Добавить("ИмяРаздела");
	СформированныеФайлы.Колонки.Добавить("КодСчета");
	СформированныеФайлы.Колонки.Добавить("ИмяФайла");
	СформированныеФайлы.Колонки.Добавить("ПолноеИмяФайла");
	СформированныеФайлы.Колонки.Добавить("ХешСумма");
	...
	СписокРазделов = РеквизитФормыВЗначение("мДеревоВыбранныхСтраниц");
	ОбойтиРазделыОтчетности(СписокРазделов, ПараметрыСохраненияПредставлений, СформированныеФайлы);
	
	ПоказатьРаздел(ИсходныйРаздел, ИсходныеНомераСтрокМногоуровнегоРаздела);
	
	Возврат СформированныеФайлы;
...
&НаСервере
Процедура ОбойтиРазделыОтчетности(СписокРазделов, ПараметрыСохраненияПредставлений, СформированныеФайлы)
	
	Для Каждого СтрокаРаздела Из СписокРазделов.Строки Цикл
		Если СтрокаРаздела.Строки.Количество() <> 0 Тогда
			ОбойтиРазделыОтчетности(СтрокаРаздела, ПараметрыСохраненияПредставлений, СформированныеФайлы);
			
		Иначе
			СохранитьПредставлениеОтчетаВXLSX(СтрокаРаздела, ПараметрыСохраненияПредставлений, СформированныеФайлы);
...
&НаСервере
Процедура СохранитьПредставлениеОтчетаВXLSX(СтрокаРаздела, ПараметрыСохраненияПредставлений, СформированныеФайлы)
	...
	Если ИмяРаздела = "ОСВПоСчету" ИЛИ ИмяРаздела = "АнализСчета" Тогда
		...
		Для Каждого СтраницаРаздела Из СписокСтраниц Цикл
			...
			ПолныйПрефикс = СтрШаблон(ПолныйПрефикс, СтраницаРаздела.Счет.Код);
			
			ИмяФайлаРаздела = ПолныйПрефикс + ПараметрыСохраненияПредставлений.ПостфиксПредставлений + РасширениеФайла;
			ПолноеИмяФайлаРаздела = ПараметрыСохраненияПредставлений.ВременныйКаталогПакета + ИмяФайлаРаздела;
			
			ДополнитьПредставлениеИменемФайла(ТабличныйДокумент, ИмяФайлаРаздела);
			
			ТабличныйДокумент.Записать(ПолноеИмяФайлаРаздела, ФорматСохранения);
			
			ХешСумма = ХешСуммаФайла(ПолноеИмяФайлаРаздела);
			
			СформированныйФайл = СформированныеФайлы.Добавить();
			СформированныйФайл.ИмяРаздела     = ИмяРаздела;
			СформированныйФайл.КодСчета       = СтраницаРаздела.Счет.Код;
			СформированныйФайл.ИмяФайла       = ИмяФайлаРаздела;
			СформированныйФайл.ПолноеИмяФайла = ПолноеИмяФайлаРаздела;
			СформированныйФайл.ХешСумма       = ХешСумма;
	...
	ИначеЕсли ПараметрыСохраненияПредставлений.Свойство(ИмяРаздела) Тогда
		...
		ИмяФайлаРаздела = ПараметрыСохраненияПредставлений[ИмяРаздела].Префикс
			+ ПараметрыСохраненияПредставлений.ПостфиксПредставлений + РасширениеФайла;
		
		ПолноеИмяФайлаРаздела = ПараметрыСохраненияПредставлений.ВременныйКаталогПакета
			+ ИмяФайлаРаздела;
		...
		СформированныйФайл = СформированныеФайлы.Добавить();
		СформированныйФайл.ИмяРаздела     = ИмяРаздела;
		СформированныйФайл.КодСчета       = "";
		СформированныйФайл.ИмяФайла       = ИмяФайлаРаздела;
		СформированныйФайл.ПолноеИмяФайла = ПолноеИмяФайлаРаздела;
		СформированныйФайл.ХешСумма       = ХешСумма;
		
	ИначеЕсли ИмяРаздела = "ФНС" ИЛИ ИмяРаздела = "ДополнительныеФайлы" ИЛИ ИмяРаздела = "Титульный"
	      ИЛИ ИмяРаздела = "Операции51" ИЛИ ИмяРаздела = "Операции52" ИЛИ ИмяРаздела = "Операции55"
	      ИЛИ ИмяРаздела = "ОтчетыФНС" ИЛИ ИмяРаздела = "БухОтчетность" Тогда
...
&НаСервере
Процедура ДополнитьПредставлениеИменемФайла(ПредставлениеОтчета, ИмяФайлаРаздела)
	
	СекцияИмениФайла = Новый ТабличныйДокумент;
	ОбластьИмениФайла = СекцияИмениФайла.Область(2, 1);
	ОбластьИмениФайла.Текст = "#ИмяФайла " + ИмяФайлаРаздела;
	
	ПредставлениеОтчета.Вывести(СекцияИмениФайла);
	
КонецПроцедуры

&НаСервере
Функция ХешСуммаФайла(ПолноеИмяФайла)
	
	ХешированиеДанных = Новый ХешированиеДанных(ХешФункция.MD5);
	ХешированиеДанных.ДобавитьФайл(ПолноеИмяФайла);
...
&НаСервере
Функция ДействуетСВерсииФормата(НомерВерсии)
	
	ВерсияОтчета = СтруктураРеквизитовФормы.ВерсияФормата;
	
	Возврат ОтчетностьВБанкиСлужебный.ДействуетСВерсииФормата(НомерВерсии, ВерсияОтчета);
...
&НаКлиенте
Процедура СформироватьМенюОчистки(Элемент)
	...
	ВиденПунктОчисткиРаздела = Истина;
	Если ИмяРаздела = "РасшифровкиОбеспеченийПолученные" ИЛИ ИмяРаздела = "РасшифровкиОбеспеченийВыданные"
	 ИЛИ ИмяРаздела = "ЗадолженностьПоКредитамЗаймамКраткосрочная"
	 ИЛИ ИмяРаздела = "ЗадолженностьПоКредитамЗаймамДолгосрочная"
	 ИЛИ ИмяРаздела = "Дебиторская" ИЛИ ИмяРаздела = "Кредиторская"
	 ИЛИ ИмяРаздела = "Операции51" ИЛИ ИмяРаздела = "Операции52" ИЛИ ИмяРаздела = "Операции55"
	 ИЛИ ИмяРаздела = "ФинансовыеВложенияКраткосрочные" ИЛИ ИмяРаздела = "ФинансовыеВложенияДолгосрочные" Тогда
		ВиденПунктОчисткиРаздела = Ложь;
	КонецЕсли;
	
	Элементы.ОчиститьТекущийРаздел.Видимость = ВиденПунктОчисткиРаздела;
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьДоступностьДопстрокВКонтекстномМеню(Элемент,
	КонтекстноеМенюУдалитьСтроку, КонтекстноеМенюДобавитьСтроку)
	
	КонтекстноеМенюУдалитьСтроку.Доступность = Ложь;
	КонтекстноеМенюДобавитьСтроку.Доступность = Ложь;
	
	ИмяГруппы = МногострочнаяЧастьТекущейОбласти(ЭтотОбъект, Элемент.ТекущаяОбласть.Имя);
	
	Если ЗначениеЗаполнено(ИмяГруппы) Тогда
		
		КонтекстноеМенюДобавитьСтроку.Доступность = Истина;
		...
		Если СтрЧислоВхождений(Элемент.ТекущаяОбласть.Имя, "ДобавитьСтроку") = 0
			...
			КонтекстноеМенюУдалитьСтроку.Доступность = Истина;
		КонецЕсли;
	...
	ДопТекст = ДополнительныйТекстПриДобавленииУдаленииСтрок(ИмяГруппы);
	Если ДопТекст <> Неопределено Тогда
		КонтекстноеМенюДобавитьСтроку.Заголовок =
			ДопТекст.ЗаголовокКонтекстногоМенюДобавитьСтроку;
		КонтекстноеМенюУдалитьСтроку.Заголовок =
...
&НаСервере
Функция ОтражатьКоличествоВОСВПоСчету(Счет)
	
	ПараметрыОтправкиСчета = ОтчетностьВБанкиСлужебный.ПараметрыОтправкиСчета(Счет);
	
	Возврат ДействуетСВерсииФормата("5.12.09")
		И ПараметрыОтправкиСчета.Найти(Истина, "Количественный") <> Неопределено;
	
КонецФункции

&НаСервере
Функция ОтражатьКоличествоВАнализеСчета(Счет)
...
Процедура ОбработатьРазделыОтчетаПриАктивизацииСтроки()
	...
	СформироватьМенюОчистки(Элемент);
...
&НаКлиентеНаСервереБезКонтекста
Функция ПреставлениеПериодаБухОтчетности(ДатаКонцаПериодаОтчета)
	
	НомерКвартала = Месяц(КонецКвартала(ДатаКонцаПериодаОтчета)) / 3;
	ГодОтчета = Год(ДатаКонцаПериодаОтчета);
	Если НомерКвартала = 4 Тогда
		ПреставлениеПериода = СтрЧГ0(ГодОтчета) + НСтр("ru = ' г.';
														|en = ' г.'");
	Иначе
		ПреставлениеПериода = СтрЧГ0(НомерКвартала) + НСтр("ru = ' квартал ';
															|en = ' квартал '")
		                    + СтрЧГ0(ГодОтчета) + НСтр("ru = ' г.';
														|en = ' г.'");
	КонецЕсли;
	
	Возврат ПреставлениеПериода;
