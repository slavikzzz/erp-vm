...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	СохрЗначВКвадратиках(ЭтотОбъект);
...
Процедура ВосстановитьСведенияОФизическихЛицахИзЛистаФизЛицаСтарогоОбразца()
	...
	Для Каждого ЭлементТабл Из ЭтотОбъект[мСтруктураМногостраничныхРазделов.ФизЛица] Цикл
	...
	Если Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация
	 ИЛИ ПустаяСтрока(Форма.СтруктураРеквизитовФормы.мОКАТОРаздел1) Тогда
		Если (НЕ Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация)
			...
			КодТерриториальногоОбразования
				= РегламентированнаяОтчетностьКлиентСервер.ДополнитьСтроку(КодТерриториальногоОбразования, 11, " ");
			Форма.СтруктураРеквизитовФормы.мОКАТОРаздел1 = КодТерриториальногоОбразования;
			РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(
				Форма, "Раздел1", "ОКАТО", 11, КодТерриториальногоОбразования);
		КонецЕсли;
	КонецЕсли;
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета, ЗаголовокРаздела = Ложь)
	...
	Если ИмяТекРаздела = "ФизЛица" Тогда
		...
		Для Каждого ТаблицаСтраницы Из ЭтотОбъект[НаимТаблицыСтраницы] Цикл
			...
			Если ЭтотОбъект[НаимТаблицыСтраницы].Количество() = 1 ИЛИ ЗаголовокРаздела Тогда
	...
	ИначеЕсли ИмяТекРаздела = "Раздел1"
		...
		Для Каждого ОбластьТД Из ТабличныйДокумент.Области Цикл
			Если СтрЧислоВхождений(ОбластьТД.Имя, "1_2") > 0 Тогда
...
Процедура РасчетНаКлиенте(ИмяТекущейСтраницыПанели = "", НужнаяСтрокаИлиКолонка = Неопределено) Экспорт
	...
	Расчет(ЭтотОбъект, ИмяТекущейСтраницыПанели, НужнаяСтрокаИлиКолонка);
		...
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник И Область.СодержитЗначение Тогда
			...
			ИначеЕсли ПараметрыПечати.ИмяСтраницы = "Раздел1" Тогда
				...
				Если СтрЧислоВхождений(Область.Имя, "КБК") > 0
					...
					ЗначОбласти = СтрЗаменить(Строка(Область.Значение)," ","");
					Если СтрЧислоВхождений(Область.Имя, "ОКАТО") > 0 И СтрДлина(ЗначОбласти) > 0 Тогда
						Для Ном = (СтрДлина(ЗначОбласти) + 1) По 11 Цикл
...
Функция ПроверитьТитульныйЛистИСведенияОФизЛицах(БратьИзФормы = Истина, ВыводитьСообщения = Истина, ЧтоПроверять = "12", ПечатаемыеЛисты = Неопределено)
	...
	Иначе
		Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = '%1Источник данных для формирования отчета не определен!';
	...
	Если НЕ ЭтоПБОЮЛ Тогда
		Если ПустаяСтрока(ОргУП) ИЛИ НЕ ПустаяСтрока(ОргБухгалтер) Тогда
...
Функция ПроверитьКорректностьЗаполненияОКТМО()
	...
	Разделы = СтруктураРеквизитовФормы.мСохраненныйДок.ДанныеОтчета.Получить().ДанныеМногостраничныхРазделов;
	
	ТаблЗначРаздел1 = Разделы.Раздел1;
	ТаблЗначРаздел2 = Разделы.Раздел2;
	
	Для ТаблИндекс = 0 По ТаблЗначРаздел1.Количество() - 1 Цикл
		
		ОКАТОРаздел1 = СокрЛП(ТаблЗначРаздел1[ТаблИндекс].Данные.ОКАТО);
		ДлинаОКАТО = СтрДлина(ОКАТОРаздел1);
		...
		Если НЕ (ДлинаОКАТО = 11 ИЛИ ДлинаОКАТО = 8 ИЛИ ДлинаОКАТО = 0) Тогда
			...
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'На странице %1 раздела 1 неверно задан код по ОКТМО!';
				|en = 'На странице %1 раздела 1 неверно задан код по ОКТМО!'"), Формат(ТаблИндекс + 1, "ЧГ="));
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, ТекстОписанияОшибки, "Раздел1", "ОКАТО", ТаблИндекс + 1);
	...
	Для ТаблИндекс = 0 По ТаблЗначРаздел2.Количество() - 1 Цикл
		
		ОКАТОРаздел2 = СокрЛП(ТаблЗначРаздел2[ТаблИндекс].Данные.ОКАТО);
		ДлинаОКАТО = СтрДлина(ОКАТОРаздел2);
		
		Если НЕ (ДлинаОКАТО = 11 ИЛИ ДлинаОКАТО = 8 ИЛИ ДлинаОКАТО = 0) Тогда
			
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'На странице %1 раздела 2 неверно задан код по ОКТМО!';
				|en = 'На странице %1 раздела 2 неверно задан код по ОКТМО!'"), Формат(ТаблИндекс + 1, "ЧГ="));
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, ТекстОписанияОшибки, "Раздел2", "ОКАТО", ТаблИндекс + 1);
...
Функция ПолучитьДанныеФизЛиц(СтруктураМногостраничныхРазделов = Неопределено)
	...
	Для Каждого СтрМногоСтраничныхРазделов Из ЭтотОбъект[мСтруктураМногостраничныхРазд.ФизЛица] Цикл
...
Процедура ВыводДанныхОФизическомЛице(Знач СвязьУстановлена, Знач Данные, ТипДанных, ОграниченияВыводаУчесть = Ложь, ТаблицаСтраницРазделаМ, ИндексВТабл);
	...
	Если НЕ СвязьУстановлена Тогда
		...
		Если СтрТаблицаСтраницы.Количество() = 0 Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Ошибка установки активной страницы раздела ФЛ';
...
Процедура УдалениеДополнительныхСтрокПриОчисткеТабличногоПоля(ПромТаблицаДопСтрокРаздела,ВыбТабличноеПоле,ВыбТабличноеПолеИмя,ИмяГруппыДопСтрок,ИсходноеКоличествоСтрокГруппы) Экспорт
	...
	НСтр = ЭтотОбъект[ПромТаблицаДопСтрокРаздела].Количество();
	Пока НСтр > ИсходноеКоличествоСтрокГруппы Цикл
		
		ТекСтрокаТаблицы = ЭтотОбъект[ПромТаблицаДопСтрокРаздела][НСтр - 1];
		НомерГрафы = 1;
		...
		ЭтотОбъект[ВыбТабличноеПолеИмя] = ВыбТабличноеПоле;
		
		// удаляем из структуры многострочного раздела соответствующую строку
		ЭтотОбъект[ПромТаблицаДопСтрокРаздела].Удалить(ТекСтрокаТаблицы);
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ОбластьИмя = "ОКАТО" И ЗначениеЗаполнено(Область.Значение) Тогда
		
		Область.Значение = СтрЗаменить(Область.Значение, " ", "");
		ДлинаОКТМО = СтрДлина(СокрЛП(Область.Значение));
		Если НЕ (ДлинаОКТМО = 0 ИЛИ ДлинаОКТМО = 8 ИЛИ ДлинаОКТМО = 11) Тогда
			
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Код по ОКТМО должен состоять из 8 или 11 цифр!';
															|en = 'Код по ОКТМО должен состоять из 8 или 11 цифр!'"));
			
		КонецЕсли;
		
	ИначеЕсли ИмяТекущейСтраницыПанели = "Титульный"
		    И ОбластьИмя = "ДатаПодписи" Тогда
	...
	ИначеЕсли ИмяТекущейСтраницыПанели = "Титульный"
	        И ОбластьИмя = "НомКоррект" Тогда	
			
		НомерКорректировки = ТабличныйДокумент.Области.НомКоррект.Значение;
	...
	Если (ИмяТекущейСтраницыПанели = "Раздел2") Или (ИмяТекущейСтраницыПанели = "Раздел1") Тогда
		...
		Группа = РегламентированнаяОтчетностьКлиент.РазделТекущейОбласти(ЭтотОбъект, Элемент.ТекущаяОбласть);
		Если Группа <> Неопределено Тогда
			НомерСтрокиМнгЧ = Сред(ОбластьИмя, 15);
			РегламентированнаяОтчетностьКлиентСервер.СохранитьЗначениеВМногостраничномБлоке(
				ЭтотОбъект, Группа, НомерСтрокиМнгЧ, Сред(ОбластьИмя, 1, 13), Элемент.ТекущаяОбласть.Значение);
		КонецЕсли;
...
Функция ПроверкаРаздел1(СтруктураРезультата)
	...
	Для Каждого Страница Из Раздел1 Цикл
		...
		Пока Данные.Свойство("П000010006001_" + Инд) Цикл
			...
			ЛеваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, Число(Данные[ИмяПоказателяЛевойЧасти]));
			
			ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтотОбъект, Число(Данные["П000010006004_" + Инд]) * Число(Данные["П000010006008_" + Инд]) *
...
&НаСервере
Функция ОбъектОтчета(ЭтотОбъектИмя)
