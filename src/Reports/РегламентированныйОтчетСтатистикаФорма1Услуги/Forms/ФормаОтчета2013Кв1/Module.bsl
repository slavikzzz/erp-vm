...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	ТекСумма = 0;
	Для НомСтр = 3 По 16 Цикл
		ТекСумма = ТекСумма + Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"].Значение;
	КонецЦикла;
	Области["П000001000204"].Значение = ТекСумма;
	
	// Переменная "ТекСумма" намеренно не обнуляется.
	Для НомСтр = 17 По 31 Цикл
		ТекСумма = ТекСумма + Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"].Значение;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-услуги");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1Услуги");
...
Функция ПолучитьПараметрыИнициализацииУниверсальногоОтчета()
	Возврат УниверсальныйОтчетСтатистики.ПолучитьПараметрыИнициализацииУниверсальногоОтчета(ЭтаФорма, "АтрибВыгрузкиXML2013Кв1", "ШаблонРосстата2013Кв1", "СхемаВыгрузкиXML2013Кв1", Истина, Ложь);
КонецФункции
