...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_oktmo_frm",
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 4 По 5 Цикл
		...
		ТекСумма = 0;
		Для НомСтр = 2 По 36 Цикл
			ТекСумма = ТекСумма + Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Графа].Значение;
		КонецЦикла;
		Области["П0000010001" + Графа].Значение = ТекСумма;
	КонецЦикла;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли РегламентированнаяОтчетностьКлиент.ВыбраннаяОбластьРегламентированногоОтчетаНеВерна(ЭтаФорма, Область, СтандартнаяОбработка) Тогда
		Возврат;
	Иначе
		Если Лев(Область.Имя, 13) = "П002000000101" Или Лев(Область.Имя, 13) = "П002000000102" Тогда
			ИмяСправочника = "s_oktmo_frm";
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, ИмяСправочника);
			
			ОбластьКод = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "01_", Лев(Область.Имя, 11) + "02_"));
			ОбластьНаим  = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, Лев(Область.Имя, 11) + "02_", Лев(Область.Имя, 11) + "01_"));
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Область2_3 = ТабДок.ПолучитьОбласть("ОсновнаяЧасть2_3");
