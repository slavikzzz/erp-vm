...
Процедура ПеренумерацияСтрокТаблицы(ПолеТабличногоДокументаФормаОтчета)
	ПолеТабличногоДокументаФормаОтчета.Области["П002000000102_1"].Значение = "37";
КонецПроцедуры
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли РегламентированнаяОтчетностьКлиент.ВыбраннаяОбластьРегламентированногоОтчетаНеВерна(ЭтаФорма, Область, СтандартнаяОбработка) Тогда
		Возврат;
	Иначе
		НомерГрафы = Сред(Область.Имя, 13, 1);
		Если НомерГрафы = "5" Или НомерГрафы = "6" Или НомерГрафы = "7" Тогда
			СтандартнаяОбработка = Ложь;
			//Область.Значение = ?(Область.Значение = "", "1", "");
			Область.Значение = ?(Область.Значение = "", "1", "");
		ИначеЕсли Лев(Область.Имя, 13) = "П002000000101" Или Лев(Область.Имя, 13) = "П002000000103" Тогда
