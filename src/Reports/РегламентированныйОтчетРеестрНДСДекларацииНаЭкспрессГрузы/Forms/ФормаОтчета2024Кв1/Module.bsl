...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы110 Тогда
		Подстрока = "номер накладной";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный номер накладной?';
							|en = 'Удалить выбранный номер накладной?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ИмяРаздела = "Реестр" Тогда
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если ЗначениеЗаполнено(КодОперации) Тогда
				ДанныеМнЧ_П10000 = Ветка.ДанныеМногострочныхЧастей.П10000.Строки;
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					СуммаНалБаз = СуммаНалБаз + Ветка_П10000.Данные["П1000006"];
				КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("НалБазаИтого", "КодОперации;П1000006");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрДТЭГ");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRDTEG"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRDTEG.2"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузкиРеестра, ДеревоВыгрузкиСведений)
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если НЕ (ДлинаРегНомДТ >= 23 И ДлинаРегНомДТ <= 29) Тогда
				ИмяПок = "П1000002" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП + " не" + ?(ДлинаРегНомДТ = 0,
				"", "верно") + " задан регистрационный номер декларации. Возможное количество символов: от 23 до 29",
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если НЕ ЗначениеЗаполнено(СокрЛП(Ветка_П11000.Данные["П1100003"])) Тогда
					...
					+ " не задан номер индивидуальной накладной", "Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000005"]) Тогда
				...
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " не задана дата документа, содержащего сведения о стоимости реализованного товара",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли НЕ ДатаВходитВРазрешенныйПериод(Ветка_П10000.Данные["П1000005"]) Тогда
				ИмяПок = "П1000005" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				+ " задана дата документа, содержащего сведения о стоимости реализованного товара,"
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрДТЭГ = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрДТЭГ");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрДТЭГ = СкопироватьУзел(УзелРодитель.Родитель, РеестрДТЭГ);
		Для каждого Эл Из ЭкзРеестрДТЭГ.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		// Узел "СведДТЭГ" имеется только в схеме выгрузки "Сведений".
		// Если он не найден, значит, сейчас формируются данные "Реестра" (для выгрузки первого из 2-х файлов).
		СведДТЭГ = ПолучитьПодчиненныйЭлемент(ЭкзРеестрДТЭГ, "СведДТЭГ");
		Если СведДТЭГ = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведДТЭГ = СкопироватьУзел(ЭкзРеестрДТЭГ, СведДТЭГ, ЭкзРеестрДТЭГ.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСведДТЭГ.Строки Цикл
					Если Эл.Строки.Количество() = 0 И Эл.Код <> "НомИндНаклад" Тогда
						Если Эл.Код = "НомДокСтоим" Тогда
				...
				// 11000.
				НомИндНаклад = ПолучитьПодчиненныйЭлемент(ЭкзСведДТЭГ, "НомИндНаклад");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					ЭкзНомИндНаклад = СкопироватьУзел(ЭкзСведДТЭГ, НомИндНаклад);
					ВывестиПоказательВXML(ЭкзНомИндНаклад, Ветка_П11000.Данные[ЭкзНомИндНаклад.Ключ]);
				КонецЦикла;
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_ДекларацииНаЭкспрессГрузы");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_ДекларацииНаЭкспрессГрузы_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
