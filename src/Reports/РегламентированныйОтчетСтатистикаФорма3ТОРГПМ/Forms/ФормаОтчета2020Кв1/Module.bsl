...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолеТабличногоДокументаФормаОтчета.Вывести(ОбъектОтчета(ЭтотОбъект.ИмяФормы).ПолучитьМакет("ФормаОтчета2020Кв1_ФормаОтчета"));
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтотОбъект.ИмяФормы).ПолучитьМакет("Список2020кв1");
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "КодПоНомеруСтроки", 10,, "Выбор типа отчитывающейся организации", КоллекцияСписковВыбора["КодПоНомеруСтроки"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "Территории", 10,, "Выбор территории", КоллекцияСписковВыбора["Территории"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "Образования", 10,, "Выбор муниципального образования", КоллекцияСписковВыбора["Образования"]);
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.ОтображатьКнопкуЗаполнить Тогда
		...
		ИДРедакцииОтчета = Сред(ЭтотОбъект.ИмяФормы, СтрНайти(ЭтотОбъект.ИмяФормы, ".Форма.") + 7);
		...
		Если НЕ РегламентированнаяОтчетностьПереопределяемый.РеализованаНастройкаАвтозаполняемыхПоказателейРеглОтчета(ИДОтчета, ИДРедакцииОтчета) Тогда
			...
			КнопкаФормыЗаполнить = Элементы.Вставить("КнопкаЗаполнить", Тип("КнопкаФормы"), ЭтотОбъект.КоманднаяПанель.ПодчиненныеЭлементы.ГруппаКнопок3, Элементы.Расшифровать);
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант = Неопределено, ВидДокументаНомерКорректировкиИзменен = Неопределено, ПоказателиОтчета = Неопределено) Экспорт
	...
	СтруктураРеквизитовФормы.мСохраненныйДок.ИсточникОтчета = Метаданные.Отчеты[Сред(Лев(СтрЗаменить(ЭтотОбъект.ИмяФормы, "Внешний", ""), СтрНайти(СтрЗаменить(ЭтотОбъект.ИмяФормы, "Внешний", ""), ".Форма.") - 1), 7)].Имя;
	СтруктураРеквизитовФормы.мСохраненныйДок.НаименованиеОтчета = Метаданные.Отчеты[Сред(Лев(СтрЗаменить(ЭтотОбъект.ИмяФормы, "Внешний", ""), СтрНайти(СтрЗаменить(ЭтотОбъект.ИмяФормы, "Внешний", ""), ".Форма.") - 1), 7)].ОсновнаяФорма.Синоним;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 Тогда
		УдалитьСтроку("");
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П000002000002") Или СтрНачинаетсяС(Область.Имя, "П000002000001") Тогда 
		ОбластьДляВвода = ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(Область.Имя, "П000002000001", "П000002000002")];
		ИмяСмежнойОбласти = СтрЗаменить(Область.Имя, "П000002000002", "П000002000001");
		...
		ВвестиПоказатель(ОбластьДляВвода, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе,
			ТЗВыбора, СтандартнаяОбработка, ИмяСмежнойОбласти);
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П000002000004") Или СтрНачинаетсяС(Область.Имя, "П000002000003") Тогда 
		ОбластьДляВвода = ПолеТабличногоДокументаФормаОтчета.Области[СтрЗаменить(Область.Имя, "П000002000003", "П000002000004")];
		ИмяСмежнойОбласти = СтрЗаменить(Область.Имя, "П000002000004", "П000002000003");
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабличноеПоле.Вывести(ОбъектОтчета(ЭтотОбъект.ИмяФормы).ПолучитьМакет("ФормаОтчета2020Кв1_ФормаОтчета"));
	РегламентированнаяОтчетность.УстановитьЦветФорматДоступностьЯчеек(ЭтотОбъект);
	...
	КонецЦикла;
	ПолеТабличногоДокументаФормаОтчета.Области["ВыбратьОтчетПрошлогоПериода"].Значение = "Выбор отчета предыдущего периода";
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть1а1"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть1а2"));
	...
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ШапкаМногострочнойЧасти"));
	
	ОбластьШапкаМногострочнойЧасти = ТабличныйДокумент.ПолучитьОбласть("ШапкаМногострочнойЧасти");
	ОбластьДобавлениеСтроки1 = ТабличныйДокумент.ПолучитьОбласть("ДобавлениеСтроки1");
	ОбластьОсновнаяЧасть6_1 = ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть6_1");
	ВерхМногострочнойЧасти = ТабличныйДокумент.Области.ШапкаМногострочнойЧасти.Низ + 1;
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверки.Добавить(ОбластьОсновнаяЧасть6_1);
		Если ПечатнаяФорма.ПроверитьВывод(МассивПроверки) Тогда
			ПечатнаяФорма.Вывести(ТекущаяОбластьМногострочнойЧасти);
		Иначе
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 3-ТОРГ(ПМ)");
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			ПечатнаяФорма.Вывести(ОбластьШапкаМногострочнойЧасти);
			ПечатнаяФорма.Вывести(ТекущаяОбластьМногострочнойЧасти);
		КонецЕсли;
		ТекущийВерх = ТекущийВерх + 1;
	КонецЦикла;
	ПечатнаяФорма.Вывести(ОбластьОсновнаяЧасть6_1);
	
	ОбластьОсновнаяЧасть7 = ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть7");
	МассивПроверки.Очистить();
	МассивПроверки.Добавить(ОбластьОсновнаяЧасть7);
	Если Не ПечатнаяФорма.ПроверитьВывод(МассивПроверки) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 3-ТОРГ(ПМ)");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
	
	ПечатнаяФорма.Вывести(ОбластьОсновнаяЧасть7);
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 3-ТОРГ(ПМ)");
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ДопАтрибуты = РегламентированнаяОтчетность.СформироватьСтруктуруДопАтрибутов(ЭтотОбъект.ИмяФормы, "АтрибВыгрузкиXML2020Кв1");
...
Процедура Расшифровать(Команда)
	...
	ЭтаФормаИмя = ЭтотОбъект.ИмяФормы;
	Если СтрЧислоВхождений(ЭтаФормаИмя, "ВнешнийОтчет.") > 0 Тогда
		ЭтаФормаИмя = СтрЗаменить(ЭтаФормаИмя, "ВнешнийОтчет.", "");
	ИначеЕсли СтрЧислоВхождений(ЭтаФормаИмя, "Отчет.") > 0 Тогда
		ЭтаФормаИмя = СтрЗаменить(ЭтаФормаИмя, "Отчет.", "");
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	ТекТабличноеПоле.Вывести(
		ОбъектОтчета(ЭтотОбъект.ИмяФормы).ПолучитьМакет("ФормаОтчета2020Кв1_ФормаОтчета"));
...
Функция СформироватьПараметрыВыбораОтчетаПредыдущегоПериода() Экспорт 
	...
	Парам.Вставить("ВидОтправляемогоДокумента", ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.СтатистикаФорма3ТОРГПМ"));
