...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "01/08/2013";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	// Раздел 1.
	// Данные по строке 38 должны быть равны сумме строк 39-45, 48, 52-71, 73-78, 81.
	Для НомКол = 4 По 7 Цикл
		...
		ТекСумма = ТекСумма + Форма.ПолеТабличногоДокументаФормаОтчета.Области["П0000010081" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		...
		КонецЦикла;
		Для НомСтр = 52 По 71 Цикл
			ТекСумма = ТекСумма + Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
		Для НомСтр = 73 По 78 Цикл
			ТекСумма = ТекСумма + Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// По разделу 1.
	// Данные по всем графам строки 03 должны быть больше или равны суммы строк 04-07.
	Для НомКол = 4 По 7 Цикл
		ТекСумма = 0;
		...
		Если ПараметрыВыгрузки["П0000010003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] < ТекСумма Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 1 по графе " + НомКол + " значение в строке 03 должно быть больше или равно сумме значений в строках с 04 по 07!",
				ИмяПоляТаблДокумента, "П0000010003" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Данные по всем графам строки 08 должны быть больше или равны строки 09.
	Для НомКол = 4 По 7 Цикл
		Если ПараметрыВыгрузки["П0000010008" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] < ПараметрыВыгрузки["П0000010009" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 1 по графе " + НомКол + " значение в строке 08 должно быть больше или равно значению в строке 09!",
				ИмяПоляТаблДокумента, "П0000010008" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Данные по всем графам строки 10 должны быть больше или равны суммы строк 11-13.
	Для НомКол = 4 По 7 Цикл
		ТекСумма = 0;
		...
		Если ПараметрыВыгрузки["П0000010010" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] < ТекСумма Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 1 по графе " + НомКол + " значение в строке 10 должно быть больше или равно сумме значений в строках с 11 по 13!",
				ИмяПоляТаблДокумента, "П0000010010" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Данные по всем графам строки 14 должны быть больше или равны суммы строк 15, 18, 19.
	Для НомКол = 4 По 7 Цикл
		Если ПараметрыВыгрузки["П0000010014" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] < (ПараметрыВыгрузки["П0000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"]
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 1 по графе " + НомКол + " значение в строке 14 должно быть больше или равно сумме значений в строках 15, 18, 19!",
	...
	Для НомКол = 4 По 7 Цикл
		Если ПараметрыВыгрузки["П0000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] < (ПараметрыВыгрузки["П0000010016" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"]
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 1 по графе " + НомКол + " значение в строке 15 должно быть больше или равно сумме значений в строках 16 и 17!",
				ИмяПоляТаблДокумента, "П0000010015" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Данные по всем графам строки 24 должны быть больше или равны строки 25.
	Для НомКол = 4 По 7 Цикл
		Если ПараметрыВыгрузки["П0000010024" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] < ПараметрыВыгрузки["П0000010025" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 1 по графе " + НомКол + " значение в строке 24 должно быть больше или равно значению в строке 25!",
				ИмяПоляТаблДокумента, "П0000010024" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Данные по всем графам строки 45 должны быть больше или равны суммы строк 46 и 47.
	Для НомКол = 4 По 7 Цикл
		Если ПараметрыВыгрузки["П0000010045" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] < (ПараметрыВыгрузки["П0000010046" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"]
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 1 по графе " + НомКол + " значение в строке 45 должно быть больше или равно сумме значений в строках 46 и 47!",
				ИмяПоляТаблДокумента, "П0000010045" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Данные по всем графам строки 48 должны быть больше или равны суммы строк 49-51.
	Для НомКол = 4 По 7 Цикл
		ТекСумма = 0;
		...
		Если ПараметрыВыгрузки["П0000010048" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] < ТекСумма Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 1 по графе " + НомКол + " значение в строке 48 должно быть больше или равно сумме значений в строках с 49 по 51!",
				ИмяПоляТаблДокумента, "П0000010048" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Данные по всем графам строки 78 должны быть больше или равны суммы строк 79 и 80.
	Для НомКол = 4 По 7 Цикл
		Если ПараметрыВыгрузки["П0000010078" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] < (ПараметрыВыгрузки["П0000010079" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"]
			+ ПараметрыВыгрузки["П0000010080" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 1 по графе " + НомКол + " значение в строке 78 должно быть больше или равно сумме значений в строках 79 и 80!",
				ИмяПоляТаблДокумента, "П0000010078" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// По разделу 2.
	// Данные по всем графам строки 82 должны быть больше или равны сумме строк 83-88.
	Для НомКол = 4 По 7 Цикл
		ТекСумма = 0;
		Для НомСтр = 83 По 88 Цикл
			ТекСумма = ТекСумма + ПараметрыВыгрузки["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"];
		КонецЦикла;
		Если ПараметрыВыгрузки["П0000020082" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] < ТекСумма Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 2 по графе " + НомКол + " значение в строке 82 должно быть больше или равно сумме значений в строках с 83 по 88!",
				ИмяПоляТаблДокумента, "П0000020082" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	// Данные по всем графам строки 89 должны быть больше или равны сумме строк 90-94.
	Для НомКол = 4 По 7 Цикл
		ТекСумма = 0;
		Для НомСтр = 90 По 94 Цикл
			ТекСумма = ТекСумма + ПараметрыВыгрузки["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"];
		КонецЦикла;
		Если ПараметрыВыгрузки["П0000020089" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_Ч"] < ТекСумма Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 2 по графе " + НомКол + " значение в строке 89 должно быть больше или равно сумме значений в строках с 90 по 94!",
				ИмяПоляТаблДокумента, "П0000020089" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ="));
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
