...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	ИдГрупп = Новый Структура;
	ИдГрупп.Вставить("мИдГруппы1", ЭтаФорма.СтруктураРеквизитовФормы.мИдГруппы1);
	...
	ИначеЕсли ИмяРаздела = "Раздел1" Тогда
		...
		Если ИмяОбласти = "П000010002003" И ЗначениеЗаполнено(Область.Значение) Тогда
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Для Каждого ЛистРаздела1 Из Раздел1 Цикл
		...
		Если ДлинаОКАТОРаздел1 <> 11 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе на странице " + НомерЛиста + " не " + ?(ДлинаОКАТОРаздел1 = 0, "", "верно") + " задан код по ОКАТО!", "Раздел1", "П000010002003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;		
