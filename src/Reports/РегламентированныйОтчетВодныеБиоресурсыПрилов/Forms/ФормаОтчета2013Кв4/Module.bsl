...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы1  = "П0000100080";
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "23/09/2010";
	
	// Код отчета, равен КНД
	СтруктураРеквизитовФормы.мКодОтчета = "1110022";
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010003003", "П000010008003");		
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено ИЛИ ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСкопированаФорма) Тогда		
		...
		Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
			...
			Если ТаблицаСтраницыРазделаРаздел1.Количество() = 1
				И ТипЗнч(ТаблицаСтраницыРазделаРаздел1[0].Данные) = Тип("СписокЗначений")
				И ТаблицаСтраницыРазделаРаздел1[0].Данные.Количество() = 1 Тогда
				
				ДанныеСтр1Разд1 = ТаблицаСтраницыРазделаРаздел1[0].Данные[0].Значение;
				
				Если ТипЗнч(ДанныеСтр1Разд1) = Тип("Структура") Тогда
					...
					Если СведенияОбОрганизации.Свойство("ОКТМО") Тогда
						...
						ДанныеСтр1Разд1.Вставить("П000010002003", СокрЛП(СведенияОбОрганизации.ОКТМО));
...
КонецФункции // ПолучитьНазванияОрганизацийСтрокой()
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ИмяФормы).ПолучитьМакет("СпискиВыбора2013Кв4");
	...
	Для Инд=1 По 16 Цикл
		РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010008001_", 4, , "Выбор кода наименования", КоллекцияСписковВыбора["ОбъектыВодныхБиологическихРесурсов"]);
	КонецЦикла;	
...
Процедура ФормироватьСтруктуруСтраницОтчета()
	...
	СтрокаУровня1.Представление            = "Раздел";
	СтрокаУровня1.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = 1;
	СтрокаУровня1.ВыгрузитьСтраницу        = 2;
...
Процедура УдалитьНеобходимоеКоличествоДопСтрокНаСервере(ВыбТабличноеПоле, ВыбТабличноеПолеИмя, ИмяГруппыДопСтрок) Экспорт
	...
	Если ТекущееКоличествоСтрок > ИсходноеКоличествоСтрокГруппы Тогда
		...
		Пока НСтр > ИсходноеКоличествоСтрокГруппы Цикл
			...
			ИмяПервойЯчейкиУдаляемойСтроки  = ИмяПервойКолонки + НСтрСтр;
	...
	Иначе
		...
		Для Каждого Строка Из ТаблицаВариантыЗаполнения Цикл
			...
			Для Инд = 1 по КолвоСтрокВМногострочнойЧасти Цикл
				...
				Если Число (ВариантЗаполнения) = 4 Тогда
...
Процедура РазделыОтчетаПриАктивизацииСтроки(Элемент)
	...
	Элементы.ТабличныйДокументКонтекстноеМенюГруппаСтроки.Видимость = Элемент.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1";
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	Если ИмяТекРаздела = "Раздел1" Тогда
		...
		Если ТабличныйДокумент.Области.Найти("УдалитьСтроку" + СтруктураРеквизитовФормы.мИдГруппы1 + "01_2") = Неопределено Тогда
			Обл = ТабличныйДокумент.Области["УдалитьСтроку" + СтруктураРеквизитовФормы.мИдГруппы1 + "01_1"];
	...
	Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000010003003, ОбластьИмя) > 0 Тогда
			...
			Для Каждого Зап Из ТаблицаБлока Цикл
				...
				П000010008003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Зап.П000010008003);
				
				//аккумулирование общих итогов
				ОбщаяСумма = ОбщаяСумма + П000010008003;
		
			КонецЦикла;
			
			Форма.ТабличныйДокумент.Области.П000010003003.Значение = ОбщаяСумма;
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	ИмяГруппы = ОпределитьИдМногострочногоРазделаПоАктивнойСтранице(ИмяТекущейСтраницыПанели,СтруктураРеквизитовФормы.НаимТекущегоРаздела, ИдГрупп);
	...
	Если ИмяРаздела = "Титульный" Тогда
		...
		Если СтрНайти(ИмяОбласти, "ОКВЭД") > 0 Тогда
	...
	ИначеЕсли ИмяРаздела = "Раздел1" Тогда
		...
		Если ПозицияПодстроки > 0 Тогда
			...
			Если СтрНайти(ИмяОбласти, "П000010008003") > 0 Тогда
...
Процедура УдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, ТекущаяСтраница) Экспорт
	...
	#Если НаКлиенте Тогда
		Форма.РасчетНаКлиенте(Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела,"П000010008003");
	#Иначе
		Форма.РасчетНаСервере(Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела,"П000010008003");
	#КонецЕсли
...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	// ОКТМО.
	ПоказательОКТМО = Показатели.Найти("П000010002003", "Имя");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("КНД",			"1110022");
	...
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы",				"NO_PRVBR_1_104_00_05_01_01.xsd");
	ПараметрыВыгрузки.Вставить("ПрефиксИмениФайлаВыгрузки",	"NO_PRVBR");
	
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "ВБРПрил");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Для Каждого ЛистРаздела1 Из Раздел1 Цикл
		...
		Если ДлинаКБКРаздел1 <> 20 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе на странице " + НомерЛиста + " не " + ?(ДлинаКБКРаздел1 = 0, "", "верно") + " задан КБК!", "Раздел1", "П000010001003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// проверяем ОКТМО
		ОКТМОРаздел1 = СокрЛП(ДанныеЛистаРаздела1.П000010002003);
		ДлинаОКТМОРаздел1 = СтрДлина(ОКТМОРаздел1);
		Если ДлинаОКТМОРаздел1 <> 11 И ДлинаОКТМОРаздел1 <> 8 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе на странице " + НомерЛиста + " не " + ?(ДлинаОКТМОРаздел1 = 0, "", "верно") + " задан код по ОКТМО!", "Раздел1", "П000010002003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
				
		//проверяем общую сумму единовременного взноса, подлежащую уплате в бюджет
		Если НЕ ЗначениеЗаполнено(ДанныеЛистаРаздела1.П000010003003) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе на странице " + НомерЛиста + " не задана общая сумма единовременного взноса!", "Раздел1", "П000010003003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
				
		//проверяем дату уплаты единовременного взноса
		Если НЕ ЗначениеЗаполнено(ДанныеЛистаРаздела1.П000010004003) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе на странице " + НомерЛиста + " не задана дата уплаты единовременного взноса!", "Раздел1", "П000010004003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;

		//проверяем серия разрешения
		Если НЕ ЗначениеЗаполнено(СокрЛП(ДанныеЛистаРаздела1.П000010005003)) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе на странице " + НомерЛиста + " не задана серия разрешения!", "Раздел1", "П000010005003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		//проверяем номер разрешения
		Если НЕ ЗначениеЗаполнено(СокрЛП(ДанныеЛистаРаздела1.П000010006003)) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе на странице " + НомерЛиста + " не задан номер разрешения!", "Раздел1", "П000010006003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		//проверяем дату получения разрешения
		Если НЕ ЗначениеЗаполнено(ДанныеЛистаРаздела1.П000010007003) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе на странице " + НомерЛиста + " не задана дата получения разрешения!", "Раздел1", "П000010007003", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// должна быть заполнена хотя бы одна строка табличной части
		Если МногострочнаяЧастьПуста(ДанныеЛистаРаздела1, "П000010008001, П000010008002, П000010008003") Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе на странице " + НомерЛиста + " не задана ни одна строка табличной части!", "Раздел1", "П000010008001_1", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// перебираем все строки табличной части
		НомерСтроки = 1;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеЛистаРаздела1, "П000010008001_" + Формат(НомерСтроки, "ЧГ=")) Цикл
			
			П000010008001 = СокрЛП(ДанныеЛистаРаздела1["П000010008001_" + Формат(НомерСтроки, "ЧГ=")]);
			П000010008002 = ДанныеЛистаРаздела1["П000010008002_" + Формат(НомерСтроки, "ЧГ=")];
			П000010008003 = ДанныеЛистаРаздела1["П000010008003_" + Формат(НомерСтроки, "ЧГ=")];
			
			Если ЗначениеЗаполнено(П000010008001) ИЛИ ЗначениеЗаполнено(П000010008002) ИЛИ ЗначениеЗаполнено(П000010008003) Тогда
				// проверяем код вида
				ДлинаКода = СтрДлина(П000010008001);
				Если ДлинаКода <> 4 Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе на странице " + НомерЛиста + " в строке " + НомерСтроки + " не " + ?(ДлинаКода = 0, "", "верно") + " задан код наименования объектов водных биологических ресурсов!", "Раздел1", "П000010008001_" + Формат(НомерСтроки, "ЧГ="), НомерЛиста);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				// проверяем разрешенный объем прилова
				Если НЕ ЗначениеЗаполнено(П000010008002) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе на странице " + НомерЛиста + " в строке " + НомерСтроки + " не задан разрешенный объем прилова!", "Раздел1", "П000010008002_" + Формат(НомерСтроки, "ЧГ="), НомерЛиста);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				// проверяем сумму единовременного взноса
				Если НЕ ЗначениеЗаполнено(П000010008003) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе на странице " + НомерЛиста + " в строке " + НомерСтроки + " не задана сумма единовременного взноса!", "Раздел1", "П000010008003_" + Формат(НомерСтроки, "ЧГ="), НомерЛиста);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	Если НомерЛиста = 0 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнен раздел!", "Раздел1", "П000010001003", 1);
		СтатусПроверки = Ложь;
	КонецЕсли;	
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	УзелКолВБРСумПУШаблон = ПолучитьПодчиненныйЭлемент(УзелРодитель, "КолВБРСумПУ");
	
	Для Каждого ЛистРаздела1 Из Раздел1 Цикл
				
		ДанныеЛистРаздела1 = ЛистРаздела1.Данные[0].Значение;
		
		УзелКолВБРСумПУ = СкопироватьУзел(УзелРодитель, УзелКолВБРСумПУШаблон);
		ЗаполнитьДаннымиУзел(ДанныеЛистРаздела1, УзелКолВБРСумПУ);
				
		УзелСумОбВБРШаблон = ПолучитьПодчиненныйЭлемент(УзелКолВБРСумПУ, "СумОбВБР");
		Инд = 1;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеЛистРаздела1, "П000010008001_" + Формат(Инд, "ЧГ=")) Цикл
			УзелСумОбВБР = СкопироватьУзел(УзелКолВБРСумПУ, УзелСумОбВБРШаблон);
			ВыгрузитьСтрокуМногострочнойЧасти(ДанныеЛистРаздела1, УзелСумОбВБР, Инд);
			Инд = Инд + 1;
		КонецЦикла;		
...
&НаКлиенте
Процедура СпециальныеОперацииПослеОчисткиЛистаНаКлиенте(ИмяСтраницы, УдалениДопЭкз) Экспорт
...
&НаСервере
Процедура СпециальныеОперацииПослеОчисткиЛистаНаСервере(ИмяСтраницы, УдалениДопЭкз) Экспорт
