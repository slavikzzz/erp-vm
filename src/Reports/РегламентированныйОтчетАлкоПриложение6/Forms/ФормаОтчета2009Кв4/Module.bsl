...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = 1152024;
	...
	СтруктураРеквизитовФормы.КНД = "1152024";
	СтруктураРеквизитовФормы.НаименованиеОтчетаДляВыгрузки = "ДЕКЛАРАЦИЯ ОБ ОБЪЕМАХ ПОСТАВКИ ЭТИЛОВОГО СПИРТА, АЛКОГОЛЬНОЙ И СПИРТОСОДЕРЖАЩЕЙ ПРОДУКЦИИ";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("мПризнакРознТорг");
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ БезОткрытияФормы Тогда
		...
		СтруктураРеквизитовФормы.мПризнакРознТорг = "В РОЗНИЧНУЮ ТОРГОВЛЮ";
	...
	Для Каждого СтрокаТаблицы Из СтруктураГруппы Цикл
		...
		Для НомерГрафы = 1 По Форма.мСтруктураКолвоКолонокРазделов[ИмяГруппы] Цикл
			...
			Для Ном = 1 По ?(НомерГрафы = 9, 4, ?(НомерГрафы = 11, 2, 1)) Цикл
				...
				Если НомерГрафы = 9 ИЛИ НомерГрафы = 11 Тогда
				...
				Если СтруктураГруппы[0].Свойство(КолонкаТаблицыИмя) Тогда
					...
					Если (КолонкаТаблицыИмя = "П000000000004") Или (КолонкаТаблицыИмя = "П000010003004") Тогда
						ЗначениеЯчейки           = СтрокаТаблицы[КолонкаТаблицыИмя];
						Если ЗначениеЯчейки = Форма.СтруктураРеквизитовФормы.мПризнакРознТорг Тогда
							ОперацияНадСтрокойМногострочногоБлока(Форма, ИмяГруппы, Истина, Формат(НомерСтрокиВФормеТаблицы, "ЧГ=0"))
						КонецЕсли;
					КонецЕсли;
...
Процедура УдалитьДополнительнуюСтроку()
	...
	Иначе // Строка не удаляется в форме табличного документа, а должна осуществляться сдвиг строк.
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Титульный" Тогда
		...
		ИначеЕсли Область.Имя = "ВидДеятельности" Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьФормуВыбораЗначениеВидДеятельностиЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура ОткрытьФормуВыбораЗначениеВидДеятельностиЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОткрытьФормуВыбораСтраницЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ОбработатьРезультирующийТекстВыгрузки(Текст)
	...
	ЧтоМенять = "П000000000004:В РОЗНИЧНУЮ ТОРГОВЛЮ
	|П000000000005:
	|П000000000006:
	|П000000000007:
	|П000000000008:643,0,-,,,,,,,
	...
	НаЧтоМенять = "П000000000004:В РОЗНИЧНУЮ ТОРГОВЛЮ
	|П000000000005:
	|П000000000006:
	|П000000000007:
	|П000000000008:
	...
	Текст.УстановитьТекст(СтрЗаменить(Текст.ПолучитьТекст(), ЧтоМенять, НаЧтоМенять));
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы",				"ALK_1152024_4_01.xsd");
	ПараметрыВыгрузки.Вставить("ПрефиксИмениФайлаВыгрузки",	"A6");
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	//ОбъемПоставки
	УзелОбъемПоставкиШаблон = ПолучитьПодчиненныйЭлемент(УзелРодитель, "ОбъемПоставки");
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(РазделДекларация, "П000000000003_" + стрНомСтроки) Цикл
		Если ЗначениеЗаполнено(РазделДекларация["П000000000003_" + стрНомСтроки]) Тогда
			...
			УзелОбъемПоставки = СкопироватьУзел(УзелРодитель, УзелОбъемПоставкиШаблон);
			ВыгрузитьСтрокуМногострочнойЧасти(РазделДекларация, УзелОбъемПоставки, НомерСтроки);
			ЗадатьЗначениеЭлемента(УзелОбъемПоставки, "П000000000001", Формат(ЧислоВыгруженныхСтрок, "ЧГ="));
			...
			Если СтруктураАдреса.Страна = "643" Тогда
				//это резидент
				РегламентированнаяОтчетность.УдалитьУзел(ПолучитьПодчиненныйЭлемент(УзелОбъемПоставки, "Иностр"));
				УзелРезидент = ПолучитьПодчиненныйЭлемент(УзелОбъемПоставки, "Резидент");	
			...
			Иначе
				//это иностранец
				РегламентированнаяОтчетность.УдалитьУзел(ПолучитьПодчиненныйЭлемент(УзелОбъемПоставки, "Резидент"));
				УзелИностр = ПолучитьПодчиненныйЭлемент(УзелОбъемПоставки, "Иностр");
...
Процедура ЗаполнитьНаОснованииИсточника(Знач ИдГруппаИсточника)
	...
	Для Каждого Стр Из ТаблицаРаздела Цикл
		...
		// Только для приложения 6, в случае, если в первой таблице введено РОЗНИЧНАЯ ТОРГОВЛЯ, то в доп. таблице блокируем ненужные ячейки.
		Если Данные.Наименование = СтруктураРеквизитовФормы.мПризнакРознТорг Тогда
			ОперацияНадСтрокойМногострочногоБлока(ЭтаФорма, ПаралГруппа, Истина, ИндС);
		КонецЕсли;
...
Процедура СформироватьРазделДляПечати(ИдГруппа, СтруктураПараметровСтраницы, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	Если КолвоПустых = ТаблицаСвер.Количество() Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Нет сведений для печати листа %1 в особом режиме.';
																						|en = 'Нет сведений для печати листа %1 в особом режиме.'"), ТекстВывод);
		Сообщение.Сообщить();
