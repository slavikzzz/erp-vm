...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета, РазделОтчета = Ложь)
	...
	ИначеЕсли ИмяТекРаздела = "Декларация"
		...
		ТабличныйДокумент.ТекущаяОбласть = ТабличныйДокумент.Область("R1");
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница, ИмяОбласти = Неопределено)
	...
	Если ТекущаяСтраница = "Возвраты" И ИмяГруппы = "П0000200001" Тогда
		СтраницыРаздела = Неопределено;
		Форма.мСтруктураМногостраничныхРазделов.Свойство("Возвраты", СтраницыРаздела);
		СтраницыРаздела = Форма[СтраницыРаздела];
		АктивнаяСтраница = СтраницыРаздела.НайтиСтроки(Новый Структура("АктивнаяСтраница", Истина));
...
Процедура УдалитьСтраницу(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Удалить текущую страницу?';
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ИмяОбласти = "ЗаявлениеОКорректировкеПредставление" Тогда
		...
		Иначе
			...
			ПараметрыФормы.Вставить("НомерПриложения",          "6");
...
Функция СформироватьСтруктуруПоказателейИСтруктуруПараметров()
	...
	// тут же формируем структуру параметров
...
Процедура ЗаполнитьСведенияПоПодразделению(УзелВыгрузки, ДанныеВыгрузки)
	...
	Для Каждого СтрокаВыгрузки Из ТаблицаВыгрузкиДекларация Цикл
		...
		СтрокаВыгрузки.ИдПолучателя = ПолучитьИдентификаторПолучателя(ДанныеВыгрузки, СтрокаВыгрузки, СтруктураРеквизитовФормы.мИдГруппы12);
	...
	Для Каждого СтрокаВыгрузки Из ТаблицаВыгрузкиВозвраты Цикл
		...
		СтрокаВыгрузки.ИдПолучателя = ПолучитьИдентификаторПолучателя(ДанныеВыгрузки, СтрокаВыгрузки, СтруктураРеквизитовФормы.мИдГруппы22);
	...
	Для Каждого КодВидаПродукции Из КодыВидовПродукции Цикл
		...
		Для Каждого ПроизводительИмпортер Из ПроизводителиИмпортеры Цикл
			...
			Для Каждого Получатель Из Получатели Цикл
				...
				КонецЦикла;
				РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_Поставка);
			...
			КонецЦикла;
			РегламентированнаяОтчетность.УдалитьУзел(ПрототипУзла_Получатель);
