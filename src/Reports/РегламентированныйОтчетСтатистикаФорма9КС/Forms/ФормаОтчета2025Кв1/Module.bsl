...
Процедура ДоступностьРазделов()
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <> 1 Тогда
		...
		Для Инд = 1 По ЭтотОбъект["П0020010001"].Количество() Цикл
			Для Гр = 1 По ЭтотОбъект.мСтруктураКолвоКолонокРазделов["П0020010001"] Цикл
				Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
				мФормаОтчета.Области["П0020010001" + Графа + "_" + Инд].ЦветФона = Новый Цвет();
				мФормаОтчета.Области["П0020010001" + Графа + "_" + Инд].Защита = Истина;
				мФормаОтчета.Области["П0020010001" + Графа + "_" + Инд].Значение = "";
			КонецЦикла;
			мФормаОтчета.Области["УдалитьСтрокуП002001000101" + "_" + Инд].Гиперссылка = Ложь;
			мФормаОтчета.Области["УдалитьСтрокуП002001000101" + "_" + Инд].Текст = "";
		КонецЦикла;
	...
	Если СписокСохранения.Свойство("ДанныеМногострочныхРазделов", ДанныеМногострочныхРазделов) 
		...
		Для Каждого ЭлементСтруктуры Из ДанныеМногострочныхРазделов Цикл
			...
			Если НЕ НайденноеЗначение = Неопределено Тогда
				...
				Если ЭлементСтруктуры.Ключ = "П0020010001"
				   И Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <> 1 Тогда
					
					СтруктураГруппы = ЭтотОбъект[НайденноеЗначение];
					СтруктураГруппы.Очистить();
					мСтруктураИсхКолвоСтрокРазделов.Свойство(ЭлементСтруктуры.Ключ, ИсходноеКоличествоСтрокГруппы);
...
Процедура ПеренумерацияСтрокТаблицы(ПолеТабличногоДокументаФормаОтчета)
	...
	ПолеТабличногоДокументаФормаОтчета.Области["П001001000109_1"].Значение = "1";
	ПолеТабличногоДокументаФормаОтчета.Области["П002001000119_1"].Значение = "2";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		ТЗВыбора.Очистить();
		Если СтрНайти(Область.Имя, "П001001000101_") = 1 Или СтрНайти(Область.Имя, "П001001000102_") = 1
		 Или СтрНайти(Область.Имя, "П002001000101_") = 1 Или СтрНайти(Область.Имя, "П002001000102_") = 1 Тогда
			
			СмежнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "02_", "01_"));
			ОбластьДляВвода = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "01_", "02_"));
			
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникМатериалы");
		...
		ИначеЕсли Сред(Область.Имя, 12, 2) = "03" Тогда
			...
			ТекстПриВыборе = "Выбор марки материала";
			КодОКПД =
				ПолеТабличногоДокументаФормаОтчета.Область(Лев(Область.Имя, 11) + "02" + Сред(Область.Имя, 14)).Значение;
			Для Каждого Стр Из РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникМаркиМатериалов").ТаблицаЗначений Цикл
				Если Не ЗначениеЗаполнено(КодОКПД) Или КодОКПД = Стр.Ключ Тогда
					НСтр = ТЗВыбора.Добавить();
					НСтр.Код = Стр.Код;
					НСтр.Название = Стр.Название;
				КонецЕсли;
			КонецЦикла;
			НовСтрТЗВыбора = ТЗВыбора.Вставить(0);
			НовСтрТЗВыбора.Код = "";
		...
		ИначеЕсли Сред(Область.Имя, 12, 2) = "04" Тогда
			...
			КодОКПД =
				ПолеТабличногоДокументаФормаОтчета.Область(Лев(Область.Имя, 11) + "02" + Сред(Область.Имя, 14)).Значение;
			Для Каждого Стр Из РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникЕдиницыИзмерения").ТаблицаЗначений Цикл
				Если Не ЗначениеЗаполнено(КодОКПД) Или КодОКПД = Стр.Ключ Тогда
					НСтр = ТЗВыбора.Добавить();
					НСтр.Код = Стр.Код;
					НСтр.Название = Стр.Название;
			...
			НовСтрТЗВыбора.Код = "";
			НовСтрТЗВыбора.Название = "Пустое значение";
		ИначеЕсли СтрНайти(Область.Имя, "П001001000107_") = 1 Тогда
			ОбластьДляВвода = Область;
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникПричиныИзмененияЦены");
			ТекстПриВыборе = ПараметрВводаПоля.ТекстПриВыборе;
			КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		ИначеЕсли СтрНайти(Область.Имя, "П001001000108_") = 1 Тогда
			ОбластьДляВвода = Область;
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
				ЭтотОбъект, "СправочникПричиныИзмененияКоличества");
			ТекстПриВыборе = ПараметрВводаПоля.ТекстПриВыборе;
			КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		КонецЕсли;
	...
	Если ТЗВыбора.Количество() > 0 Тогда
		ВвестиПоказатель(Элемент, ОбластьДляВвода, СмежнаяОбласть, Ложь, ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВвестиПоказатель(Элемент, Область, СмежнаяОбласть = Неопределено, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код", ПоляДляОчистки = Неопределено) Экспорт
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 9-КС");
	...
	Иначе
		НизМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть2_3.Верх - 1;
	КонецЕсли;
	ТекущийВерх = ВерхМногострочнойЧасти;
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		ТекущаяОбластьМногострочнойЧасти = ТабличныйДокумент.ПолучитьОбласть(ТекущийВерх, , ТекущийВерх);
	...
	ТекущаяОбласть = ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть2_4");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникМатериалы",
		250, , "Выбор материала", КоллекцияСписковВыбора["Материалы"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникЕдиницыИзмерения",
		250, , "Выбор единицы измерения", КоллекцияСписковВыбора["ЕдиницыИзмерения"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникМаркиМатериалов",
		250, , "Выбор марки материала", КоллекцияСписковВыбора["МаркиМатериалов"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникПричиныИзмененияЦены",
		10, , "Выбор причины изменения цены", КоллекцияСписковВыбора["ПричиныИзмененияЦены"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникПричиныИзмененияКоличества",
		10, , "Выбор причины изменения количества", КоллекцияСписковВыбора["ПричиныИзмененияКоличества"]);
...
Процедура ПолеТабличногоДокументаФормаОтчетаПриАктивизации(Элемент)
	...
	Если ИмяГруппы = "П0020010001" И Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <> 1 Тогда
		Возврат;
	КонецЕсли;
