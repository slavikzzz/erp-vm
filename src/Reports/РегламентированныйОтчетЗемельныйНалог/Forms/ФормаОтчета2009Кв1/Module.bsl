...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/01/2009";
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000020018003", "П000020021003;П000020017003");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000020019003", "П000020021003;П000020017003;П000020016003;П000020014003;П000020012003_Числ;П000020012003_Знам;П000020010003;П000020008003;П000020006003_Числ;П000020006003_Знам;П000020005003;П000020018003;П000020021003;П000020017003");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000020022003", "П000020021003");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000020023003", "П000020020003;П000020021003");
    СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000020028003", "П000020027003;П000020025003");
...
Процедура СформироватьСпискиВыбора()
	...
	Для Инд=1 По 5 Цикл
		РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010001003_" + Инд, 20, , "Выбор КБК", КоллекцияСписковВыбора["КодыКБК"]);
		РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010002003_" + Инд, 11, "ОКАТО");
	КонецЦикла;
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020001003", 20, , "Выбор КБК", КоллекцияСписковВыбора["КодыКБК"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020002003", 11, "ОКАТО");
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020003003", 12, , "Выбор категории земли", КоллекцияСписковВыбора["КатегорииЗемли"]);
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020007003", 7, , "Выбор льготы", КоллекцияСписковВыбора["КодыЛьгот1"]);
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020011003", 7, , "Выбор льготы", КоллекцияСписковВыбора["КодыЛьгот3"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020013003", 7, , "Выбор льготы", КоллекцияСписковВыбора["КодыЛьгот4"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020015003", 7, , "Выбор льготы", КоллекцияСписковВыбора["КодыЛьгот5"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020024003", 7, , "Выбор льготы", КоллекцияСписковВыбора["КодыЛьгот6"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020026003", 7, , "Выбор льготы", КоллекцияСписковВыбора["КодыЛьгот7"]);
...
КонецПроцедуры // ОбработкаВыбораИФНС()
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Если ИмяТекущейСтраницыПанели = "Раздел2" Тогда
		...
		Если ОбластьИмя = "П000020004003" Тогда
			
			Расчет(Форма, "Раздел2", "П000020019003");
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000020019003, ОбластьИмя) > 0 Тогда
			...
			Если Форма.ТабличныйДокумент.Области.П000020017003.Значение = 0 Тогда
				П000020018003 = 1;
			Иначе			
				П000020018003 = Макс((Форма.ТабличныйДокумент.Области.П000020021003.Значение - Форма.ТабличныйДокумент.Области.П000020017003.Значение) / ЧислоМесяцев, 0);
			КонецЕсли;
			
			Форма.ТабличныйДокумент.Области.П000020018003.Значение = П000020018003;
			
			// Получим коэф. без округления.
			П000020018003 = Форма.ТабличныйДокумент.Области.П000020018003.Значение;
			
			// Очистим значение базы.
			Форма.ТабличныйДокумент.Области.П000020019003.Значение = 0;
			
			П000020006003 = Форма.ТабличныйДокумент.Области.П000020006003_Числ.Значение / ?(Форма.ТабличныйДокумент.Области.П000020006003_Знам.Значение = 0, 1, Форма.ТабличныйДокумент.Области.П000020006003_Знам.Значение);
			Если П000020006003 > 1 Тогда
				Если Форма.ТабличныйДокумент.Области.П000020006003_Знам.Значение = 0 Тогда
			...
			Если П000020006003 = 0 Тогда
				Значение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ТабличныйДокумент.Области.П000020005003.Значение);
			Иначе
				Значение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ТабличныйДокумент.Области.П000020005003.Значение) * П000020006003;
			КонецЕсли;
			
			П000020008003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ТабличныйДокумент.Области.П000020008003.Значение);
			...
			П000020016003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ТабличныйДокумент.Области.П000020016003.Значение);
			Сумма140и160 = П000020014003 + П000020016003;
			
			// Доля
			П000020012003 = Форма.ТабличныйДокумент.Области.П000020012003_Числ.Значение / ?(Форма.ТабличныйДокумент.Области.П000020012003_Знам.Значение = 0, 1, Форма.ТабличныйДокумент.Области.П000020012003_Знам.Значение);
			Если П000020012003 > 1 Тогда
				Если Форма.ТабличныйДокумент.Области.П000020012003_Знам.Значение = 0 Тогда
			...
			Если Сумма080и100 > 0 Тогда
				Форма.ТабличныйДокумент.Области.П000020019003.Значение = Макс(Значение - Сумма080и100, 0);
			ИначеЕсли Сумма140и160 > 0 Тогда
				Если П000020014003 > 0 Тогда
					Форма.ТабличныйДокумент.Области.П000020019003.Значение = Макс(Значение - П000020014003, 0);
				ИначеЕсли П000020016003 > 0 Тогда
					Форма.ТабличныйДокумент.Области.П000020019003.Значение = Макс(Значение - П000020016003, 0);
				КонецЕсли;
			ИначеЕсли П000020012003 > 0 Тогда
				Форма.ТабличныйДокумент.Области.П000020019003.Значение = Макс((Значение - (Значение * П000020012003 * (1 -  П000020018003))), 0);
			Иначе
				// Льгот по данному объекту нет.
				Форма.ТабличныйДокумент.Области.П000020019003.Значение = Макс(Значение - Сумма080и100, 0);
			КонецЕсли;
			
			Расчет(Форма, "Раздел2", "П000020020003");
			
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000020023003, ОбластьИмя) > 0 Тогда
			...
			Форма.ТабличныйДокумент.Области.П000020022003.Значение = Форма.ТабличныйДокумент.Области.П000020021003.Значение / ЧислоМесяцев;
			
			Если НЕ ЗначениеЗаполнено(Форма.ТабличныйДокумент.Области.П000020004003.Значение) Тогда
				...
				Форма.ТабличныйДокумент.Области.П000020023003.Значение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ТабличныйДокумент.Области.П000020019003.Значение)
																			* Форма.ТабличныйДокумент.Области.П000020020003.Значение 
																			* Форма.ТабличныйДокумент.Области.П000020022003.Значение / 100;
				
			ИначеЕсли Форма.ТабличныйДокумент.Области.П000020004003.Значение = "1" Тогда
				...
				Форма.ТабличныйДокумент.Области.П000020023003.Значение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ТабличныйДокумент.Области.П000020019003.Значение) 
																			* Форма.ТабличныйДокумент.Области.П000020020003.Значение 
																			* Форма.ТабличныйДокумент.Области.П000020022003.Значение / 100 * 2;
				
			ИначеЕсли Форма.ТабличныйДокумент.Области.П000020004003.Значение = "2" Тогда
				...
				Форма.ТабличныйДокумент.Области.П000020023003.Значение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ТабличныйДокумент.Области.П000020019003.Значение)
																			* Форма.ТабличныйДокумент.Области.П000020020003.Значение 
																			* Форма.ТабличныйДокумент.Области.П000020022003.Значение / 100 * 4;
				
			КонецЕсли;
			
			Форма.ТабличныйДокумент.Области.П000020023003.Значение = Макс(Форма.ТабличныйДокумент.Области.П000020023003.Значение, 0);
			
			Расчет(Форма, "Раздел2", "П000020027003");

		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000020028003, ОбластьИмя) > 0 Тогда
			
			Форма.ТабличныйДокумент.Области.П000020028003.Значение = Макс(РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ТабличныйДокумент.Области.П000020023003.Значение)
			- РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ТабличныйДокумент.Области.П000020025003.Значение) 
			- РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ТабличныйДокумент.Области.П000020027003.Значение), 0);
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	Иначе
		...
		Если ИмяТекРаздела = "Раздел2" Тогда
			...
			// По умолчанию коэф. К1 устанавливаем в 1.
			Если ТабличныйДокумент.Области.П000020017003.Значение = 0 Тогда
				ТабличныйДокумент.Области.П000020018003.Значение = 1
			КонецЕсли;
...
Процедура ОчиститьТабличноеПоле(Форма, ВыбТабличноеПолеИмя)
	...
	Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Титульный" Тогда
		...
		Форма.ТабличныйДокумент.Области.НомерКорректировки.Значение = Строка(НомерКорректировки);
	КонецЕсли;
...
Процедура СформироватьРаздел1НаСервере()
	...
	Для Каждого Стр из ТаблДанных Цикл
		...
		ОкрП000020029003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Стр.Данные[0].Значение.П000020029003);	
		...
		Если НЕ ПустаяСтрока(КБК) Тогда
			...
			Если НЕ ПустаяСтрока(ОКАТО) Тогда
				...
				НовСтр.КБК   = КБК;
				НовСтр.ОКАТО = ОКАТО;
				НовСтр.Налог = ОкрП000020028003 - ОкрП000020029003;
			Иначе
				Сообщение = Новый СообщениеПользователю;
				
				Сообщение.Текст = НСтр("ru = 'Не заполнен код ОКАТО на странице Раздела 2!';
										|en = 'Не заполнен код ОКАТО на странице Раздела 2!'");
	...
	Для Инд = 0 По ДанныеДляЗаполнения.Количество() - 1 Цикл
		...
		Если ТекСтр.Налог > 0 Тогда
			ТабличныйДокумент.Области["П000010003003_" + ОкончаниеИмениОбласти].Значение = ТекСтр.Налог;
		Иначе
			ТабличныйДокумент.Области["П000010004003_" + ОкончаниеИмениОбласти].Значение = -ТекСтр.Налог;
		КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы",				"NO_ZEMND_1_075_00_05_01_01.xsd");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если КБКОКАТОРаздела1.Количество() = 0 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не задано ни одно из значений кодов по ОКАТО и КБК в разделе 1!", "Раздел1", "П000010001003_1", 1);
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Для Каждого Лист2 Из Раздел2 Цикл
		...
		Если 	ЗначениеЗаполнено(ДанныеЛиста2[0].Значение.П000020000100) ИЛИ ЗначениеЗаполнено(ДанныеЛиста2[0].Значение.П000020003003)
			...
			ИЛИ ЗначениеЗаполнено(ДанныеЛиста2[0].Значение.П000020020003)
			ИЛИ	ЗначениеЗаполнено(ДанныеЛиста2[0].Значение.П000020017003)
			ИЛИ	ЗначениеСуммыЗаполнено(ДанныеЛиста2[0].Значение.П000020019003) ИЛИ ЗначениеЗаполнено(ДанныеЛиста2[0].Значение.П000020007003)
			...
			ИЛИ	ЗначениеСуммыЗаполнено(ДанныеЛиста2[0].Значение.П000020010003) ИЛИ ЗначениеЗаполнено(ДанныеЛиста2[0].Значение.П000020011003)
			...
			ИЛИ	ЗначениеСуммыЗаполнено(ДанныеЛиста2[0].Значение.П000020028003) ИЛИ ЗначениеСуммыЗаполнено(ДанныеЛиста2[0].Значение.П000020029003)
			ИЛИ	ЗначениеЗаполнено(ДанныеЛиста2[0].Значение.П000020024003) ИЛИ ЗначениеСуммыЗаполнено(ДанныеЛиста2[0].Значение.П000020025003)
			ИЛИ	ЗначениеЗаполнено(ДанныеЛиста2[0].Значение.П000020026003) ИЛИ ЗначениеСуммыЗаполнено(ДанныеЛиста2[0].Значение.П000020027003) Тогда
			
			НовСтр = КБКОКАТОРаздела2.Добавить();
			НовСтр.ОКАТО = СокрЛП(ДанныеЛиста2[0].Значение.П000020002003);
			...
			Если ДлинаП000020002003 <> 11 Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На странице " + НомерЛиста + " раздела 2 не" + ?(ДлинаП000020002003 = 0, "", "верно") + " указано значение кода по ОКАТО!", "Раздел2", "П000020002003", НомерЛиста);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			ДлинаП000020007003 = СтрДлина(СокрЛП(ДанныеЛиста2[0].Значение.П000020007003));
			...
			ДлинаП000020011003 = СтрДлина(СокрЛП(ДанныеЛиста2[0].Значение.П000020011003));
			...
			ДлинаП000020013003 = СтрДлина(СокрЛП(ДанныеЛиста2[0].Значение.П000020013003));
			Если ДлинаП000020013003 <> 0 И ДлинаП000020013003 <> 7 Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На странице " + НомерЛиста + " раздела 2 неверно задан код налоговой льготы в строке 130!", "Раздел2", "П000020013003", НомерЛиста);
				СтатусПроверки = Ложь;
			ИначеЕсли ДлинаП000020013003 = 0 И ЗначениеСуммыЗаполнено(ДанныеЛиста2[0].Значение.П000020014003) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На странице " + НомерЛиста + " раздела 2 не задан код налоговой льготы в строке 130!", "Раздел2", "П000020013003", НомерЛиста);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ДлинаП000020015003 = СтрДлина(СокрЛП(ДанныеЛиста2[0].Значение.П000020015003));
			Если ДлинаП000020015003 <> 0 И ДлинаП000020015003 <> 7 Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На странице " + НомерЛиста + " раздела 2 неверно задан код налоговой льготы в строке 150!", "Раздел2", "П000020015003", НомерЛиста);
				СтатусПроверки = Ложь;
			ИначеЕсли ДлинаП000020015003 = 0 И ЗначениеСуммыЗаполнено(ДанныеЛиста2[0].Значение.П000020016003) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На странице " + НомерЛиста + " раздела 2 не задан код налоговой льготы в строке 150!", "Раздел2", "П000020015003", НомерЛиста);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ДлинаП000020024003 = СтрДлина(СокрЛП(ДанныеЛиста2[0].Значение.П000020024003));
			...
			ДлинаП000020026003 = СтрДлина(СокрЛП(ДанныеЛиста2[0].Значение.П000020026003));
	...
	Если КБКОКАТОРаздела2.Количество() = 0 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не задано ни одно из значений кодов по ОКАТО и КБК  в разделе 2!", "Раздел2", "П000020001003", 1);
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Для Каждого ЕдКБКОКАТОРаздела2 Из КБКОКАТОРаздела2 Цикл
		Если КБКОКАТОРаздела1.НайтиСтроки(Новый Структура("КБК, ОКАТО", ЕдКБКОКАТОРаздела2.КБК, ЕдКБКОКАТОРаздела2.ОКАТО)).Количество() = 0 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Для значений кода по ОКАТО """ + ЕдКБКОКАТОРаздела2.ОКАТО + """ и КБК """ + ЕдКБКОКАТОРаздела2.КБК + """ на странице " + ЕдКБКОКАТОРаздела2.НомерЛиста + " раздела 2 отсутствуют соответствующие значения в разделе 1!", "Раздел2", "П000020001003", ЕдКБКОКАТОРаздела2.НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	Для Каждого ЛистРаздела1 Из Раздел1 Цикл
		...
		Если НЕ ПустаяСтрока(ДанныеЛистаРаздела1[0].Значение.П000010000100) Тогда
			ЗадатьЗначениеЭлемента(УзелСумНалПУ, "НаимСРП", СокрЛП(ДанныеЛистаРаздела1[0].Значение.П000010000100));
		КонецЕсли;
		
		Для БлокРаздела1 = 1 По 5 Цикл
			
			КБК = СокрЛП(ДанныеЛистаРаздела1[0].Значение["П000010001003_" + БлокРаздела1]);
			ОКАТО = СокрЛП(ДанныеЛистаРаздела1[0].Значение["П000010002003_" + БлокРаздела1]);
			СумНал = ДанныеЛистаРаздела1[0].Значение["П000010003003_" + БлокРаздела1];
			СумНалУменьш = ДанныеЛистаРаздела1[0].Значение["П000010004003_" + БлокРаздела1];
			...
			Для Каждого ЛистРаздела2 Из Раздел2 Цикл
				...
				Если КБК <> СокрЛП(ДанныеЛистаРаздела2[0].Значение.П000020001003) ИЛИ ОКАТО <> СокрЛП(ДанныеЛистаРаздела2[0].Значение.П000020002003) Тогда
					Продолжить;
				КонецЕсли;
				
				УзелРаздел2 = СкопироватьУзел(УзелРаздел1, УзелРасчПлатЗН);
				ЗаполнитьДаннымиУзел(ДанныеЛистаРаздела2[0].Значение, УзелРаздел2);
				
				// выгружаем значения долей
				Если ЗначениеЗаполнено(ДанныеЛистаРаздела2[0].Значение.П000020006003_Числ) ИЛИ ЗначениеЗаполнено(ДанныеЛистаРаздела2[0].Значение.П000020006003_Знам) Тогда
					ЗадатьЗначениеЭлемента(УзелРаздел2, "ДоляЗУ", Формат(ДанныеЛистаРаздела2[0].Значение.П000020006003_Числ, "ЧН=0; ЧГ=") + "/" + Формат(ДанныеЛистаРаздела2[0].Значение.П000020006003_Знам, "ЧН=0; ЧГ="));
				КонецЕсли;
				
				Если ЗначениеЗаполнено(ДанныеЛистаРаздела2[0].Значение.П000020012003_Числ) ИЛИ ЗначениеЗаполнено(ДанныеЛистаРаздела2[0].Значение.П000020012003_Знам) Тогда
					УзелЛьгот387_2Пл = ПолучитьПодчиненныйЭлемент(ПолучитьПодчиненныйЭлемент(УзелРаздел2, "ОпрНалБаза"), "Льгот387_2Пл");
					ЗадатьЗначениеЭлемента(УзелЛьгот387_2Пл, "ДоляПлЗУ", Формат(ДанныеЛистаРаздела2[0].Значение.П000020012003_Числ, "ЧН=0; ЧГ=") + "/" + Формат(ДанныеЛистаРаздела2[0].Значение.П000020012003_Знам, "ЧН=0; ЧГ="));
				КонецЕсли;
