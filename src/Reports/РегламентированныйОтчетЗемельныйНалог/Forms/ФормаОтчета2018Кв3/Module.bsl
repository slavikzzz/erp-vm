...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если ЗаполнитьЗначенияПоУмолчанию Тогда
		...
		= ?(РегламентированнаяОтчетность.ОрганизацияЯвляетсяКрупнейшимНалогоплательщиком(СтруктураПараметров), "213", "270");
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ИмяФормы).ПолучитьМакет("СпискиВыбора2018Кв3");
		...
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
			...
			Для НомСтр = ВерхОбласти По НизОбласти Цикл
				...
				Если КодПоказателя <> "###" Тогда
					...
					// Особая обработка в декларации за 2020 год: учтены указания в письмах
					// ФНС России от 31.10.2019 № БС-4-21/22311@ и от 10.06.2020 № БС-4-21/9576@.
					//
					Если КодПоказателя = "0" И Область.Имя = "ФормыРеорганизации"
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '20190101' Тогда
		ИмяСекции_КодыНалоговыхЛьгот_Стр200 = "КодыНалоговыхЛьгот_Стр200_20190101";
	Иначе
		ИмяСекции_КодыНалоговыхЛьгот_Стр200 = "КодыНалоговыхЛьгот_Стр200";
	КонецЕсли;
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020020003", 7, ,
	"Выбор кода налоговой льготы", КоллекцияСписковВыбора[ИмяСекции_КодыНалоговыхЛьгот_Стр200]);
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если СтрНачинаетсяС(ИмяПоказателя, "П000020006003") ИЛИ СтрНачинаетсяС(ИмяПоказателя, "П000020010003") Тогда
		...
		Если ОблЧисл.Значение > 0 И ОблЗнам.Значение > 0 И ОблЧисл.Значение > ОблЗнам.Значение Тогда
			...
			"ru = 'В строке %1 числитель не должен быть больше знаменателя. Значение очищено.';
			|en = 'В строке %1 числитель не должен быть больше знаменателя. Значение очищено.'"), Сред(ИмяПоказателя, 9, 3));
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
	...
	Если ИмяПоказателя = "П000020013003" ИЛИ ИмяПоказателя = "П000020016003" Тогда
		Если ЗначПоказателя > 12 Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтрШаблон(НСтр(
			"ru = 'Значение в строке %1 не должно быть больше 12. Значение очищено.';
			|en = 'Значение в строке %1 не должно быть больше 12. Значение очищено.'"), Сред(ИмяПоказателя, 9, 3));
			Сообщение.Сообщить();
		Иначе
			...
			Если ОблКолМесСобств.Значение > 0 И ОблКолМесЛьготы.Значение > ОблКолМесСобств.Значение Тогда
				...
				Сообщение.Текст = НСтр("ru = 'Значение в строке 160 не должно быть больше значения в строке 130. Значение очищено.';
										|en = 'Значение в строке 160 не должно быть больше значения в строке 130. Значение очищено.'");
				Сообщение.Сообщить();
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ИмяПоказателя = "П000020007003_КодЛьготы" ИЛИ ИмяПоказателя = "П000020009003_КодЛьготы" Тогда
		...
		Если ЗначениеЗаполнено(ЗначПоказателя) Тогда
			...
			Если МассивОчищаемыхСтрок.Количество() > 0 Тогда
				...
				|en = 'Не допускается одновременное заполнение групп строк: 070, 080; 090, 100. %1 %2 %3.'"),
	...
	Если ИмяПоказателя = "П000020018003_КодЛьготы"
	 ИЛИ ИмяПоказателя = "П000020020003"
	 ИЛИ ИмяПоказателя = "П000020022003_КодЛьготы"
	 ИЛИ ИмяПоказателя = "П000020024003_КодЛьготы" Тогда
		...
		Если ЗначениеЗаполнено(ЗначПоказателя) Тогда
			...
			ОблСтр240_КодЛьготы = ТабличныйДокумент.Области.Найти("П000020024003_КодЛьготы");
			...
			Стр240_КодЛьготы = СокрЛП(ОблСтр240_КодЛьготы.Значение);
			...
			Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 2020 Тогда
				...
				// ФНС России от 31.10.2019 № БС-4-21/22311@ и от 10.06.2020 № БС-4-21/9576@.
				//
				Если ИмяПоказателя = "П000020020003"
				 ИЛИ ИмяПоказателя = "П000020022003_КодЛьготы"
				 ИЛИ ИмяПоказателя = "П000020024003_КодЛьготы" Тогда
				...
				КонецЕсли;
				Если ИмяПоказателя = "П000020018003_КодЛьготы"
				 ИЛИ ИмяПоказателя = "П000020020003" Тогда
					Если ЗначениеЗаполнено(Стр220_КодЛьготы) Тогда
						...
						КонецЕсли;
					КонецЕсли;
					Если ЗначениеЗаполнено(Стр240_КодЛьготы) Тогда
						ОблСтр240_КодЛьготы.Очистить();
						МассивОчищаемыхСтрок.Добавить("240");
						Если ТабличныйДокумент.Области.Найти("П000020024003_ПониженнаяСтавка").Значение > 0 Тогда
							МассивОчищаемыхСтрок.Добавить("240а");
						КонецЕсли;
						Если ТабличныйДокумент.Области.Найти("П000020024503").Значение > 0 Тогда
							МассивОчищаемыхСтрок.Добавить("245");
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				...
				Если МассивОчищаемыхСтрок.Количество() > 0 Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтрШаблон(НСтр(
					"ru = 'Не допускается одновременное заполнение групп строк: 180, 190; 200, 210; 220, 220а, 230, 240, 240а, 245. %1 %2 %3';
					|en = 'Не допускается одновременное заполнение групп строк: 180, 190; 200, 210; 220, 220а, 230, 240, 240а, 245. %1 %2 %3'"),
					ТекстНачало, ТекстСередина, ТекстОкончание);
					Сообщение.Сообщить();
				КонецЕсли;
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел2" Тогда
		...
		Если ЗначениеЗаполнено(КодЛьготы) Тогда
			Если КодЛьготы = "3021501" ИЛИ КодЛьготы = "3021502" ИЛИ КодЛьготы = "3021503" ИЛИ КодЛьготы = "3021504" Тогда
		...
		ОблСтр240_ПониженнаяСтавка = Форма.ТабличныйДокумент.Области["П000020024003_ПониженнаяСтавка"];
		ОблСтр245 = Форма.ТабличныйДокумент.Области["П000020024503"];
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(КодЛьготы) Тогда
			РазрешитьВводДанныхВОбласть(Форма, ОблСтр240_ПониженнаяСтавка);
			РазрешитьВводДанныхВОбласть(Форма, ОблСтр245);
			Если ОблСтр240_ПониженнаяСтавка.Значение = 0 Тогда
				ИндексАвтоРасчПоказателя = АвтоРасчПоказатели.Найти("П000020024503");
				Если ИндексАвтоРасчПоказателя <> Неопределено Тогда
					АвтоРасчПоказатели.Удалить(ИндексАвтоРасчПоказателя);
				КонецЕсли;
			КонецЕсли;
		Иначе
			ЗапретитьВводДанныхВОбласть(Форма, ОблСтр240_ПониженнаяСтавка);
	...
	ИначеЕсли ИмяРаздела = "Раздел2" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				// Стр. 245.
				//
				Если СтрЧислоВхождений(СтруктЗависимыхЯчеек.П000020024503, ОбластьИмя) > 0 Тогда
					
					КодЛьготы = СокрЛП(Форма.ТабличныйДокумент.Области["П000020024003_КодЛьготы"].Значение);
					Стр240_ПониженнаяСтавка = Форма.ТабличныйДокумент.Области["П000020024003_ПониженнаяСтавка"].Значение;
					...
					Если ЗначениеЗаполнено(КодЛьготы) Тогда
						...
						Если Стр240_ПониженнаяСтавка > 0 Тогда
							...
							Форма.ТабличныйДокумент.Области["П000020024503"].Значение
							= РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Стр245);
					...
					Иначе
						...
						Форма.ТабличныйДокумент.Области["П000020024503"].Значение = 0;
				...
				Если СтрЧислоВхождений(СтруктЗависимыхЯчеек.П000020025003, ОбластьИмя) > 0 Тогда
					...
					Стр245 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
					Форма, Форма.ТабличныйДокумент.Области["П000020024503"].Значение);
					...
					ИначеЕсли Стр230 > 0 И Стр245 > 0 Тогда
						
						// Особая обработка в декларации за 2020 год: учтены указания в письмах
						// ФНС России от 31.10.2019 № БС-4-21/22311@ и от 10.06.2020 № БС-4-21/9576@.
						//
						Стр250 = Стр150 - Стр230 - Стр245;
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел2" Тогда
		...
		ПараметрыРасчета.Вставить("ИмяОтчета",   РегламентированнаяОтчетностьКлиентСервер.ИмяОтчета(Форма.ИмяФормы));
		ПараметрыРасчета.Вставить("ИмяФормы",    РегламентированнаяОтчетностьКлиентСервер.ИмяФормыОтчета(Форма.ИмяФормы));
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ.Вставить("П000020024503", "П000020004003;П000020005003;П000020006003_Числ;П000020006003_Знам;П000020007003_КодЛьготы;П000020008003;П000020009003_КодЛьготы;П000020010003_Числ;П000020010003_Знам;П000020011003;П000020012003;П000020013003;П000020014003;П000020014503;П000020016003;П000020017003;П000020024003_КодЛьготы;П000020024003_ПониженнаяСтавка");
	Структ.Вставить("П000020025003", "П000020004003;П000020005003;П000020006003_Числ;П000020006003_Знам;П000020007003_КодЛьготы;П000020008003;П000020009003_КодЛьготы;П000020010003_Числ;П000020010003_Знам;П000020011003;П000020012003;П000020013003;П000020014003;П000020014503;П000020015003;П000020016003;П000020017003;П000020018003_КодЛьготы;П000020019003;П000020020003;П000020021003;П000020022003_КодЛьготы;П000020022003_Процент;П000020023003;П000020024003_КодЛьготы;П000020024003_ПониженнаяСтавка;П000020024503");
...
&НаКлиенте
Функция УниверсальнаяВыгрузка(ТекстВыгрузки = Неопределено, ПроверятьБлокировки = Истина, ДляОтправкиВФНС = Ложь) Экспорт
	...
	Если НЕ УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки, ДляОтправкиВФНС) Тогда
...
&НаСервере
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки, ДляОтправкиВФНС)
	
	СформироватьСтруктуруПоказателей();
	ПараметрыВыгрузки = СформироватьСтруктуруПараметров();
	
	ПараметрыВыгрузки.Вставить("ДляОтправкиВФНС", ДляОтправкиВФНС);
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ДляОтправкиВФНС", Ложь);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПараметрыВыгрузки.ДляОтправкиВФНС Тогда
		Если Год(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) < 2020 Тогда
	...
	Если ПараметрыВыгрузки.ДляОтправкиВФНС Тогда
		...
		Если Год(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) > 2020 Тогда
			...
			СообщитьВТС("Начиная с отчетности за 2020 год декларация утратила силу (приказ ФНС России от 04.09.2019 № ММВ-7-21/440@).",
			"Титульный", "ОтчетГод");
			СтатусПроверки = Ложь;
			
		ИначеЕсли Год(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) = 2020 Тогда
			
			Если ПараметрыВыгрузки.НомКорр = "0" Тогда
				
				СообщитьВТС("Начиная с отчетности за 2020 год декларация утратила силу (приказ ФНС России от 04.09.2019 № ММВ-7-21/440@)."
				+ " С 01.01.2021 за 2020 год можно представить только корректировочную декларацию, если первоначально декларация была"
				+ " представлена в течение 2020 года в случае реорганизации организации (письмо ФНС России от 31.10.2019 № БС-4-21/22311@).",
			...
			Иначе
				...
				Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ФормРеорг) Тогда
					СообщитьВТС("В декларации за 2020 год должен быть заполнен код формы реорганизации"
					+ " (письмо ФНС России от 31.10.2019 № БС-4-21/22311@).", "Титульный", "КодФормыРеорганизации");
					СтатусПроверки = Ложь;
				Иначе
					Если ВозможныеКодыФормыРеорганизции.Найти(ПараметрыВыгрузки.ФормРеорг) = Неопределено Тогда
						СообщитьВТС("В декларации за 2020 год код формы реорганизации должен быть равен одному из следующих значений: "
						+ СтрСоединить(ВозможныеКодыФормыРеорганизции, ", ") + " (письмо ФНС России от 31.10.2019 № БС-4-21/22311@).",
	...
	Для каждого СтраницаРаздела Из ДеревоРаздела.Строки Цикл
		...
		Если ЗаполненаСтраницаРаздела Тогда
			...
			Если ЗначениеЗаполнено(КодЛьготы) Тогда
				Если СтрДлина(КодЛьготы) <> 7 Тогда
					СообщитьВТС("На странице " + НомСтраницы
					+ " раздела 2 код налоговой льготы в виде освобождения от налогообложения (строка 200), если заполнен, должен состоять из 7 символов.",
			...
			Если ЗначениеЗаполнено(КодЛьготы) Тогда
				ВозможныеЗначения = ВозможныеЗначенияПоДаннымСпискаВыбора("П000020022003_КодЛьготы");
				Если ВозможныеЗначения.Количество() > 0 И ВозможныеЗначения.Найти(КодЛьготы) = Неопределено Тогда
					СообщитьВТС("На странице " + НомСтраницы
					+ " раздела 2 код налоговой льготы в виде уменьшения суммы налога, если заполнен, должен принимать одно из значений: """
					+ СтрСоединить(ВозможныеЗначения, """, """) + """.", "Раздел2", "П000020022003_КодЛьготы", НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
...
Процедура УстановитьФорматВыводаНаСервере(ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел2" Тогда
		...
		ИменаНемногострочныхПоказателей.Добавить("П000020024503");
			...
			Если Раздел.Ключ = "Раздел2" Тогда
				...
				Для Каждого СтраницаРаздела2 Из ДеревоРаздела2.Строки Цикл
					...
					Если СокрЛП(ДанныеСтраницыРаздела2["П000020018003_КодЛьготы"]) = "3022400" Тогда
						...
						КонецЕсли;
					КонецЕсли;
					Если ЗначениеЗаполнено(ДанныеСтраницыРаздела2["П000020022003_КодЛьготы"]) Тогда
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Раздел1Заполнен = РазделЗаполнен(мДанныеОтчета.Раздел1, "Раздел1");
	Раздел2Заполнен = РазделЗаполнен(мДанныеОтчета.Раздел2, "Раздел2");
	...
	Если Раздел2Заполнен Тогда
		...
		ПроверитьКС_1_38_доп(СтруктураРезультата);
		ПроверитьКС_1_39_доп(СтруктураРезультата);
		ПроверитьКС_1_40_доп(СтруктураРезультата);
		ПроверитьКС_1_41_доп(СтруктураРезультата);
		ПроверитьКС_1_42_доп(СтруктураРезультата);
		ПроверитьКС_1_43_доп(СтруктураРезультата);
		ПроверитьКС_1_44_доп(СтруктураРезультата);
		ПроверитьКС_1_45_доп(СтруктураРезультата);
		ПроверитьКС_1_46_доп(СтруктураРезультата);
	КонецЕсли;
...
Процедура ПроверитьКС_1_01(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 021 Раздела 1) ';
			|en = 'Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 021 Раздела 1) '") +
		НСтр("ru = 'должна быть равна сумме всех значений строк 250 Раздела 2 по соответствующим кодам ОКТМО и КБК';
			|en = 'должна быть равна сумме всех значений строк 250 Раздела 2 по соответствующим кодам ОКТМО и КБК'"));
	...
	Настройки.ОписаниеОшибок.Добавить(
		НСтр("ru = 'Завышена исчисленная сумма земельного налога, подлежащая уплате в бюджет за налоговый период';
			|en = 'Завышена исчисленная сумма земельного налога, подлежащая уплате в бюджет за налоговый период'"));
	Настройки.ОписаниеОшибок.Добавить(
		НСтр("ru = 'Занижена исчисленная сумма земельного налога, подлежащая уплате в бюджет за налоговый период';
			|en = 'Занижена исчисленная сумма земельного налога, подлежащая уплате в бюджет за налоговый период'"));
...
Процедура ПроверитьКС_1_02(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма налога, подлежащая уплате в бюджет (строка 030 Раздела 1) должна быть равна разнице значения ';
			|en = 'Сумма налога, подлежащая уплате в бюджет (строка 030 Раздела 1) должна быть равна разнице значения '") +
		НСтр("ru = 'строки 021 Раздела 1 и суммы строк 023, 025, 027 Раздела 1 по соответствующим кодам ОКТМО и ';
			|en = 'строки 021 Раздела 1 и суммы строк 023, 025, 027 Раздела 1 по соответствующим кодам ОКТМО и '") +
		НСтр("ru = 'КБК, если такая разница больше или равна нулю';
			|en = 'КБК, если такая разница больше или равна нулю'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'КБК %1, ОКТМО %2';
													|en = 'КБК %1, ОКТМО %2'"));
	...
	Настройки.ОписаниеОшибок.Добавить(НСтр("ru = 'Завышена сумма земельного налога, подлежащая уплате в бюджет';
											|en = 'Завышена сумма земельного налога, подлежащая уплате в бюджет'"));
	Настройки.ОписаниеОшибок.Добавить(НСтр("ru = 'Занижена сумма земельного налога, подлежащая уплате в бюджет';
											|en = 'Занижена сумма земельного налога, подлежащая уплате в бюджет'"));
	ТаблицаСоответствий = ТаблицаСоответствий(мДанныеОтчета.Раздел1);
...
Процедура ПроверитьКС_1_03(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма налога, исчисленная к уменьшению (строка 040 Раздела 1) должна быть равна разнице суммы строк ';
			|en = 'Сумма налога, исчисленная к уменьшению (строка 040 Раздела 1) должна быть равна разнице суммы строк '") +
		НСтр("ru = '023, 025, 027 Раздела 1 и значения строки 021 Раздела 1 по соответствующим кодам ОКТМО и КБК, ';
			|en = '023, 025, 027 Раздела 1 и значения строки 021 Раздела 1 по соответствующим кодам ОКТМО и КБК, '") +
		НСтр("ru = 'если такая разница больше нуля';
			|en = 'если такая разница больше нуля'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'КБК %1, ОКТМО %2';
													|en = 'КБК %1, ОКТМО %2'"));
...
Процедура ПроверитьКС_1_05(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Значение строки 080 Раздела 2 должно быть заполнено, если заполнена строка 070 Раздела 2';
			|en = 'Значение строки 080 Раздела 2 должно быть заполнено, если заполнена строка 070 Раздела 2'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
		НСтр("ru = 'Значение строки 080 (""%1"") > 0 при условии что значение строки 070 (""%2"") заполнено';
			|en = 'Значение строки 080 (""%1"") > 0 при условии что значение строки 070 (""%2"") заполнено'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не исчислена сумма налоговой льготы';
														|en = 'Не исчислена сумма налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_06(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код налоговой льготы (строка 090 Раздела 2) должен быть заполнен, если значение строки 100 больше нуля';
			|en = 'Код налоговой льготы (строка 090 Раздела 2) должен быть заполнен, если значение строки 100 больше нуля'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
		НСтр("ru = 'Значение строки 090 заполнено (""%1"") при условии что значение строки 100 (""%2/%3"") > 0';
			|en = 'Значение строки 090 заполнено (""%1"") при условии что значение строки 100 (""%2/%3"") > 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не указан код налоговой льготы';
														|en = 'Не указан код налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_07(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Значение строки 100 Раздела 2 (Доля необлагаемой площади земельного участка) должно быть заполнено, ';
			|en = 'Значение строки 100 Раздела 2 (Доля необлагаемой площади земельного участка) должно быть заполнено, '") +
		НСтр("ru = 'если заполнена строка 090 Раздела 2';
			|en = 'если заполнена строка 090 Раздела 2'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
		НСтр("ru = 'Значение строки 100 (""%1/%2"") > 0 при условии, что значение строки 090 (""%3"") заполнено';
			|en = 'Значение строки 100 (""%1/%2"") > 0 при условии, что значение строки 090 (""%3"") заполнено'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не указана доля необлагаемой площади земельного участка';
														|en = 'Не указана доля необлагаемой площади земельного участка'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_08(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код налоговой льготы (строка 180 Раздела 2) должен быть заполнен, если значение строки 190 больше нуля';
			|en = 'Код налоговой льготы (строка 180 Раздела 2) должен быть заполнен, если значение строки 190 больше нуля'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
		НСтр("ru = 'Значение строки 180 заполнено (""%1"") при условии что значение строки 190 (""%2"") > 0';
			|en = 'Значение строки 180 заполнено (""%1"") при условии что значение строки 190 (""%2"") > 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не указан код налоговой льготы';
														|en = 'Не указан код налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_09(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Значение строки 190 Раздела 2 (Сумма налоговой льготы) должно быть заполнено, если заполнена ';
			|en = 'Значение строки 190 Раздела 2 (Сумма налоговой льготы) должно быть заполнено, если заполнена '") +
		НСтр("ru = 'строка 180 Раздела 2строка 180 Раздела 2';
			|en = 'строка 180 Раздела 2строка 180 Раздела 2'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
		НСтр("ru = 'Значение строки 190 (""%1"") > 0 при условии что значение строки 180 (""%2"") заполнено';
			|en = 'Значение строки 190 (""%1"") > 0 при условии что значение строки 180 (""%2"") заполнено'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не исчислена сумма налоговой льготы';
														|en = 'Не исчислена сумма налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_10(СтруктураРезультата)
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ ст.395, ст. 7';
													|en = 'НК РФ ст.395, ст. 7'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код налоговой льготы (строка 200 Раздела 2) должен быть заполнен, если значение строки 210 больше нуля';
			|en = 'Код налоговой льготы (строка 200 Раздела 2) должен быть заполнен, если значение строки 210 больше нуля'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
		НСтр("ru = 'Значение строки 200 заполнено (""%1"") при условии что значение строки 210 (""%2"") > 0';
			|en = 'Значение строки 200 заполнено (""%1"") при условии что значение строки 210 (""%2"") > 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не указан код налоговой льготы';
														|en = 'Не указан код налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_11(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Значение строки 210 Раздела 2 (Сумма налоговой льготы) должно быть заполнено, если заполнена ';
			|en = 'Значение строки 210 Раздела 2 (Сумма налоговой льготы) должно быть заполнено, если заполнена '") +
		НСтр("ru = 'строка 200 Раздела 2';
			|en = 'строка 200 Раздела 2'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
		НСтр("ru = 'Значение строки 210 (""%1"") > 0 при условии что значение строки 200 (""%2"") заполнено';
			|en = 'Значение строки 210 (""%1"") > 0 при условии что значение строки 200 (""%2"") заполнено'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не исчислена сумма налоговой льготы';
														|en = 'Не исчислена сумма налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_12(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код налоговой льготы (строка 220 Раздела 2) должен быть заполнен, если значение строки 230 больше нуля';
			|en = 'Код налоговой льготы (строка 220 Раздела 2) должен быть заполнен, если значение строки 230 больше нуля'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
		НСтр("ru = 'Значение строки 220 заполнено (""%1"") при условии что значение строки 230 (""%2"") > 0';
			|en = 'Значение строки 220 заполнено (""%1"") при условии что значение строки 230 (""%2"") > 0'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не указан код налоговой льготы';
														|en = 'Не указан код налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_13(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Значение строки 230 Раздела 2 (Сумма налоговой льготы) должно быть заполнено, если заполнена ';
			|en = 'Значение строки 230 Раздела 2 (Сумма налоговой льготы) должно быть заполнено, если заполнена '") +
		НСтр("ru = 'строка 220 Раздела 2';
			|en = 'строка 220 Раздела 2'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
		НСтр("ru = 'Значение строки 230 (""%1"") > 0 при условии что значение строки 220 (""%2"") заполнено';
			|en = 'Значение строки 230 (""%1"") > 0 при условии что значение строки 220 (""%2"") заполнено'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не исчислена сумма налоговой льготы';
														|en = 'Не исчислена сумма налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_14(СтруктураРезультата)
	...
	// Формула КС: Р2С241 > 0 если Р2С240 заполнена
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка КС 1.14';
													|en = 'Проверка КС 1.14'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ п.2 ст.387';
													|en = 'НК РФ п.2 ст.387'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Значение строки 240а Раздела 2 должно быть заполнено, если заполнена строка 240 Раздела 2';
			|en = 'Значение строки 240а Раздела 2 должно быть заполнено, если заполнена строка 240 Раздела 2'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
		НСтр("ru = 'Значение строки 240а (""%1"") > 0 при условии что значение строки 240 (""%2"") заполнено';
			|en = 'Значение строки 240а (""%1"") > 0 при условии что значение строки 240 (""%2"") заполнено'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Возможно неправомерное применение льготы';
														|en = 'Возможно неправомерное применение льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С240_Код, Р2С241", Раздел2);
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р2С240_Код.Значение) И Не Показатели.Р2С241.Значение > 0;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			Показатели.Р2С241.Значение, Показатели.Р2С240_Код.Значение);
		Настройки.ЭтоОшибка = ЭтоОшибка;
		ДобавитьЭлементВСтруктуруОшибок(Настройки);
...
Процедура ПроверитьКС_1_15(СтруктураРезультата)
	...
	// Формула КС: Р2С160 > 0 если Р2С170, Р2С180 и Р2С190 заполнены или Р2С170, Р2С200 и Р2С210 заполнены
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ п.2 ст. 387, ст.395, ст.7';
													|en = 'НК РФ п.2 ст. 387, ст.395, ст.7'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Количество полных месяцев использования льготы (строка 160 Раздела 2) должно ';
			|en = 'Количество полных месяцев использования льготы (строка 160 Раздела 2) должно '") +
		НСтр("ru = 'быть больше нуля, если заполнены строки 170, 180 и 190 Раздела 2 или строки 170, 200 и 210 Раздела 2';
			|en = 'быть больше нуля, если заполнены строки 170, 180 и 190 Раздела 2 или строки 170, 200 и 210 Раздела 2'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Значение строки 160: (""%1"")';
													|en = 'Значение строки 160: (""%1"")'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Возможно неправомерное применение льготы';
														|en = 'Возможно неправомерное применение льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С160, Р2С170, Р2С180_Код, Р2С190, Р2С200, Р2С210", Раздел2);
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = (ЗначениеЗаполнено(Показатели.Р2С180_Код.Значение) И ЗначениеЗаполнено(Показатели.Р2С190.Значение) Или
			...
			И ЗначениеЗаполнено(Показатели.Р2С170.Значение) И Не Показатели.Р2С160.Значение > 0;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_16(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма налоговой льготы (строка 190 Раздела 2) должна быть равна значению строки 150 ';
			|en = 'Сумма налоговой льготы (строка 190 Раздела 2) должна быть равна значению строки 150 '") +
		НСтр("ru = 'Раздела 2 х (1 - строка 170 Раздела 2)';
			|en = 'Раздела 2 х (1 - строка 170 Раздела 2)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	...
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Неправильно определен коэффициент Кл';
														|en = 'Неправильно определен коэффициент Кл'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_17(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма налоговой льготы (строка 210 Раздела 2) должна быть равна значению строки 150 ';
			|en = 'Сумма налоговой льготы (строка 210 Раздела 2) должна быть равна значению строки 150 '") +
		НСтр("ru = 'Раздела 2 х (1 - строка 170 Раздела 2)';
			|en = 'Раздела 2 х (1 - строка 170 Раздела 2)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
...
Процедура ПроверитьКС_1_18а(СтруктураРезультата)
	...
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка КС 1.18а';
													|en = 'Проверка КС 1.18а'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ п.2 ст. 387';
													|en = 'НК РФ п.2 ст. 387'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Налоговая база (строка 110 Раздела 2) должна быть равна разнице значений строк ';
			|en = 'Налоговая база (строка 110 Раздела 2) должна быть равна разнице значений строк '") +
		НСтр("ru = '050 и 080, если такая разница больше нуля и строка 060 не заполнена';
			|en = '050 и 080, если такая разница больше нуля и строка 060 не заполнена'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	...
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru = 'Неправильно определена не облагаемая налогом сумма, уменьшающая величину налоговой базы';
			|en = 'Неправильно определена не облагаемая налогом сумма, уменьшающая величину налоговой базы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_18б(СтруктураРезультата)
	...
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка КС 1.18б';
													|en = 'Проверка КС 1.18б'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ п.2 ст. 387';
													|en = 'НК РФ п.2 ст. 387'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Налоговая база (строка 110 Раздела 2) должна быть равна нулю, если разница ';
			|en = 'Налоговая база (строка 110 Раздела 2) должна быть равна нулю, если разница '") +
		НСтр("ru = 'значений строк 050 и 080 меньше нуля и строка 060 Раздела 2 не заполнена';
			|en = 'значений строк 050 и 080 меньше нуля и строка 060 Раздела 2 не заполнена'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
...
Процедура ПроверитьКС_1_19(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Налоговая база (строка 110 Раздела 2) должна быть равна строке 050 - (строка 050 х строка 100 ';
			|en = 'Налоговая база (строка 110 Раздела 2) должна быть равна строке 050 - (строка 050 х строка 100 '") +
		НСтр("ru = 'Раздела 2 х (1 - строка 170 Раздела 2)), если строка 100 Раздела 2 отлична от нуля';
			|en = 'Раздела 2 х (1 - строка 170 Раздела 2)), если строка 100 Раздела 2 отлична от нуля'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 - (%2 х %3/%4 х (1 - %5)) Итог [%6]';
													|en = '%1 = %2 - (%2 х %3/%4 х (1 - %5)) Итог [%6]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Неправильно определена налоговая база';
														|en = 'Неправильно определена налоговая база'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_20(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Налоговая база (строка 110 Раздела 2) должна быть равна (строка 050 ';
			|en = 'Налоговая база (строка 110 Раздела 2) должна быть равна (строка 050 '") +
		НСтр("ru = 'Раздела 2 х строка 060 Раздела 2) - строка 080 Раздела 2';
			|en = 'Раздела 2 х строка 060 Раздела 2) - строка 080 Раздела 2'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
...
Процедура ПроверитьКС_1_21(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Налоговая база (строка 110 Раздела 2) должна быть равна (строка 050 Раздела 2 х строка 060 ';
			|en = 'Налоговая база (строка 110 Раздела 2) должна быть равна (строка 050 Раздела 2 х строка 060 '") +
		НСтр("ru = 'Раздела 2), если такое произведение больше нуля и строки 080, 100 Раздела 2 не заполнены. ';
			|en = 'Раздела 2), если такое произведение больше нуля и строки 080, 100 Раздела 2 не заполнены. '") +
		НСтр("ru = 'Иначе, строка 110 раздела 2 = 0';
			|en = 'Иначе, строка 110 раздела 2 = 0'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
...
Процедура ПроверитьКС_1_22(СтруктураРезультата)
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ п.2 ст.387, п.5 ст.391';
													|en = 'НК РФ п.2 ст.387, п.5 ст.391'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Налоговая база (строка 110 Раздела 2) должна быть равна (строка 050 Раздела 2 х строка 060 Раздела ';
			|en = 'Налоговая база (строка 110 Раздела 2) должна быть равна (строка 050 Раздела 2 х строка 060 Раздела '") +
		НСтр("ru = '2) - строка 080 Раздела 2, если такая разница больше нуля. Иначе, строка 110 раздела 2 = 0';
			|en = '2) - строка 080 Раздела 2, если такая разница больше нуля. Иначе, строка 110 раздела 2 = 0'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = (%2 х %3/%4) - %5 Итог [%6]';
													|en = '%1 = (%2 х %3/%4) - %5 Итог [%6]'"));
...
Процедура ПроверитьКС_1_23а(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = 'Раздела 2) - (строка 050 раздела 2 х строка 060 Раздела 2) х (строка 100 раздела 2 х (1 - ';
			|en = 'Раздела 2) - (строка 050 раздела 2 х строка 060 Раздела 2) х (строка 100 раздела 2 х (1 - '") +
		НСтр("ru = 'строка 170 Раздела 2), если такая разница больше нуля и строки 060, 100 Раздела 2 заполнены';
			|en = 'строка 170 Раздела 2), если такая разница больше нуля и строки 060, 100 Раздела 2 заполнены'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
		НСтр("ru = '%1 = (%2 х %3/%4) - (%2 х %3/%4) х (%5/%6 х (1 - %7)) Итог [%8]';
			|en = '%1 = (%2 х %3/%4) - (%2 х %3/%4) х (%5/%6 х (1 - %7)) Итог [%8]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Неправильно определена налоговая база';
														|en = 'Неправильно определена налоговая база'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_23б(СтруктураРезультата)
	...
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка КС 1.23б';
													|en = 'Проверка КС 1.23б'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ п.2 ст. 387';
													|en = 'НК РФ п.2 ст. 387'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Налоговая база (строка 110 Раздела 2) должна быть равна нулю, если (строка 050 Раздела 2 х строка ';
			|en = 'Налоговая база (строка 110 Раздела 2) должна быть равна нулю, если (строка 050 Раздела 2 х строка '") +
		НСтр("ru = '060 Раздела 2) - (строка 050 раздела 2 х строка 060 Раздела 2) х (строка 100 раздела 2 х (1 - ';
			|en = '060 Раздела 2) - (строка 050 раздела 2 х строка 060 Раздела 2) х (строка 100 раздела 2 х (1 - '") +
		НСтр("ru = 'строка 170 Раздела 2) меньше или равна нулю и строки 060, 100 Раздела 2 заполнены';
			|en = 'строка 170 Раздела 2) меньше или равна нулю и строки 060, 100 Раздела 2 заполнены'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = 0;
...
Процедура ПроверитьКС_1_24(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Коэффициент Кв (строка 140 раздела 2) должен быть равен ""1"", если значение строки 130 = ""12"" ';
			|en = 'Коэффициент Кв (строка 140 раздела 2) должен быть равен ""1"", если значение строки 130 = ""12"" '") +
		НСтр("ru = '(земельный участок использовался в течение всего налогового периода)';
			|en = '(земельный участок использовался в течение всего налогового периода)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Значение строки 140: (""%1"")';
													|en = 'Значение строки 140: (""%1"")'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Неправильно указан коэффициент Кв';
														|en = 'Неправильно указан коэффициент Кв'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_25(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма исчисленного налога (строка 150 Раздела 2) должна быть равна (строка 110 Раздела 2 ';
			|en = 'Сумма исчисленного налога (строка 150 Раздела 2) должна быть равна (строка 110 Раздела 2 '") +
		НСтр("ru = 'х строка 120 Раздела 2 х строка 140 Раздела 2) / 100 (при отсутствии коэффициента Ки)';
			|en = 'х строка 120 Раздела 2 х строка 140 Раздела 2) / 100 (при отсутствии коэффициента Ки)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1, Кадастровый номер %2';
													|en = 'Раздел 2, Страница %1, Кадастровый номер %2'"));
	...
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Неправильно указана сумма исчисленного налога';
														|en = 'Неправильно указана сумма исчисленного налога'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_26(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма исчисленного налога (строка 150 Раздела 2) должна быть равна (строка 110 Раздела 2 х строка 120';
			|en = 'Сумма исчисленного налога (строка 150 Раздела 2) должна быть равна (строка 110 Раздела 2 х строка 120'") +
		НСтр("ru = ' Раздела 2 х строка 140 Раздела 2 х строка 145 Раздела 2) / 100, (при наличии коэффициента Ки)';
			|en = ' Раздела 2 х строка 140 Раздела 2 х строка 145 Раздела 2) / 100, (при наличии коэффициента Ки)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1, Кадастровый номер %2';
													|en = 'Раздел 2, Страница %1, Кадастровый номер %2'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = (%2 х %3 х %4 х %5) / 100 Итог [%6]';
													|en = '%1 = (%2 х %3 х %4 х %5) / 100 Итог [%6]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Неправильно указана сумма исчисленного налога';
														|en = 'Неправильно указана сумма исчисленного налога'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_27(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Для налогоплательщиков в отношении земельных участков, приобретенных (предоставленных) в ';
			|en = 'Для налогоплательщиков в отношении земельных участков, приобретенных (предоставленных) в '") +
		НСтр("ru = 'собственность на условиях осуществления на них жилищного строительства, в том случае, если ';
			|en = 'собственность на условиях осуществления на них жилищного строительства, в том случае, если '") +
		НСтр("ru = 'в течение налогового периода изменение размера коэффициента 2 на коэффициент 4 не происходит ';
			|en = 'в течение налогового периода изменение размера коэффициента 2 на коэффициент 4 не происходит '") +
		НСтр("ru = 'и отсутствует коэффициент Ки и если по строке 040 Раздела 2 указано значение ""1"", то строка ';
			|en = 'и отсутствует коэффициент Ки и если по строке 040 Раздела 2 указано значение ""1"", то строка '") +
		НСтр("ru = '150 Раздела 2 = ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2) / 100) х 2';
			|en = '150 Раздела 2 = ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2) / 100) х 2'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1, Кадастровый номер %2';
													|en = 'Раздел 2, Страница %1, Кадастровый номер %2'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = ((%2 х %3 х %4) / 100) х 2 Итог [%5]';
													|en = '%1 = ((%2 х %3 х %4) / 100) х 2 Итог [%5]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru = 'Неправильно указана сумма исчисленного налога с учетом коэффициента 2';
			|en = 'Неправильно указана сумма исчисленного налога с учетом коэффициента 2'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
...
Процедура ПроверитьКС_1_28(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = 'и при наличии коэффициента Ки и если по строке 040 Раздела 2 указано значение ""1"", то строка ';
			|en = 'и при наличии коэффициента Ки и если по строке 040 Раздела 2 указано значение ""1"", то строка '") +
		НСтр("ru = '150 Раздела 2 = ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2 х строка ';
			|en = '150 Раздела 2 = ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2 х строка '") +
		НСтр("ru = '145 Раздела 2) / 100) х 2';
			|en = '145 Раздела 2) / 100) х 2'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1, Кадастровый номер %2';
													|en = 'Раздел 2, Страница %1, Кадастровый номер %2'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = ((%2 х %3 х %4 х %6) / 100) х 2 Итог [%5]';
													|en = '%1 = ((%2 х %3 х %4 х %6) / 100) х 2 Итог [%5]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru = 'Неправильно указана сумма исчисленного налога с учетом коэффициента 2';
			|en = 'Неправильно указана сумма исчисленного налога с учетом коэффициента 2'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
...
Процедура ПроверитьКС_1_29(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = 'и отсутствует коэффициент Ки и если по строке 040 Раздела 2 указано значение ""2"", то строка ';
			|en = 'и отсутствует коэффициент Ки и если по строке 040 Раздела 2 указано значение ""2"", то строка '") +
		НСтр("ru = '150 Раздела 2 = ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2) / 100) х 4';
			|en = '150 Раздела 2 = ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2) / 100) х 4'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1, Кадастровый номер %2';
													|en = 'Раздел 2, Страница %1, Кадастровый номер %2'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = ((%2 х %3 х %4) / 100) х 4 Итог [%5]';
													|en = '%1 = ((%2 х %3 х %4) / 100) х 4 Итог [%5]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru = 'Неправильно указана сумма исчисленного налога с учетом коэффициента 4';
			|en = 'Неправильно указана сумма исчисленного налога с учетом коэффициента 4'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
...
Процедура ПроверитьКС_1_30(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = 'и при наличии коэффициента Ки и если по строке 040 Раздела 2 указано значение ""2"", то строка ';
			|en = 'и при наличии коэффициента Ки и если по строке 040 Раздела 2 указано значение ""2"", то строка '") +
		НСтр("ru = '150 Раздела 2 = ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2 х строка ';
			|en = '150 Раздела 2 = ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2 х строка '") +
		НСтр("ru = '145 Раздела 2) / 100) х 4';
			|en = '145 Раздела 2) / 100) х 4'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1, Кадастровый номер %2';
													|en = 'Раздел 2, Страница %1, Кадастровый номер %2'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = ((%2 х %3 х %4 х %6) / 100) х 4 Итог [%5]';
													|en = '%1 = ((%2 х %3 х %4 х %6) / 100) х 4 Итог [%5]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru = 'Неправильно указана сумма исчисленного налога с учетом коэффициента 4';
			|en = 'Неправильно указана сумма исчисленного налога с учетом коэффициента 4'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
...
Процедура ПроверитьКС_1_31(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = 'собственность на условиях осуществления на них жилищного строительства, в случае изменения в ';
			|en = 'собственность на условиях осуществления на них жилищного строительства, в случае изменения в '") +
		НСтр("ru = 'течение налогового периода размера коэффициента 2 на коэффициент 4, если по одному листу в строке ';
			|en = 'течение налогового периода размера коэффициента 2 на коэффициент 4, если по одному листу в строке '") +
		НСтр("ru = '040 Раздела 2 указано значение ""1"", а по второму листу значение ""2"", то по одному листу строка ';
			|en = '040 Раздела 2 указано значение ""1"", а по второму листу значение ""2"", то по одному листу строка '") +
		НСтр("ru = '150 Раздела 2 = ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2) / 100) х 2 ';
			|en = '150 Раздела 2 = ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2) / 100) х 2 '") +
		НСтр("ru = '(при отсутствии коэффициента Ки), а по второму листу строка 150 Раздела 2 = ((строка 110 Раздела ';
			|en = '(при отсутствии коэффициента Ки), а по второму листу строка 150 Раздела 2 = ((строка 110 Раздела '") +
		НСтр("ru = '2 х строка 120 Раздела 2 х строка 140 Раздела 2) / 100) х 4 (при отсутствии коэффициента Ки)';
			|en = '2 х строка 120 Раздела 2 х строка 140 Раздела 2) / 100) х 4 (при отсутствии коэффициента Ки)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1, Кадастровый номер %2';
													|en = 'Раздел 2, Страница %1, Кадастровый номер %2'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = ((%2 х %3 х %4) / 100) х %5 Итог [%6]';
													|en = '%1 = ((%2 х %3 х %4) / 100) х %5 Итог [%6]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru = 'Неправильно указана сумма исчисленного налога с учетом коэффициентов 2 и 4';
			|en = 'Неправильно указана сумма исчисленного налога с учетом коэффициентов 2 и 4'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("КиРавноНулю", Истина);
...
Процедура ПроверитьКС_1_32(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = '150 Раздела 2 = ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2 х строка 145 ';
			|en = '150 Раздела 2 = ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2 х строка 145 '") +
		НСтр("ru = 'Раздела 2) / 100) х 2 (при наличии коэффициента Ки), а по второму листу строка 150 Раздела 2 ';
			|en = 'Раздела 2) / 100) х 2 (при наличии коэффициента Ки), а по второму листу строка 150 Раздела 2 '") +
		НСтр("ru = '= ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2 х строка 145 Раздела 2) / ';
			|en = '= ((строка 110 Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2 х строка 145 Раздела 2) / '") +
		НСтр("ru = '100) х 4 (при наличии коэффициента Ки)';
			|en = '100) х 4 (при наличии коэффициента Ки)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1, Кадастровый номер %2';
													|en = 'Раздел 2, Страница %1, Кадастровый номер %2'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = ((%2 х %3 х %4 х %7) / 100) х %5 Итог [%6]';
													|en = '%1 = ((%2 х %3 х %4 х %7) / 100) х %5 Итог [%6]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru = 'Неправильно указана сумма исчисленного налога с учетом коэффициента 2 и 4';
			|en = 'Неправильно указана сумма исчисленного налога с учетом коэффициента 2 и 4'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("КиРавноНулю", Ложь);
...
Процедура ПроверитьКС_1_33(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма налоговой льготы (строка 230 Раздела 2) должна быть равна произведению значения строки 150 ';
			|en = 'Сумма налоговой льготы (строка 230 Раздела 2) должна быть равна произведению значения строки 150 '") +
		НСтр("ru = 'Раздела 2 х процент, на который уменьшается исчисленная сумма налога (строка 220а) / 100';
			|en = 'Раздела 2 х процент, на который уменьшается исчисленная сумма налога (строка 220а) / 100'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	...
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Неправильно указана сумма налоговой льготы';
														|en = 'Неправильно указана сумма налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
...
Процедура ПроверитьКС_1_34(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 250 Раздела 2) ';
			|en = 'Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 250 Раздела 2) '") +
		НСтр("ru = 'должна быть равна разнице значение строк 150 и 230 Раздела 2, если строка 230 Раздела 2 заполнена';
			|en = 'должна быть равна разнице значение строк 150 и 230 Раздела 2, если строка 230 Раздела 2 заполнена'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 - %3 Итог [%4]';
													|en = '%1 = %2 - %3 Итог [%4]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru = 'Неправильно определена исчисленная сумма земельного налога, подлежащая уплате в бюджет ';
			|en = 'Неправильно определена исчисленная сумма земельного налога, подлежащая уплате в бюджет '") +
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С150, Р2С230, Р2С245, Р2С250", Раздел2);
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы,, "Р2С245");
		
		РасчетнаяСумма = Показатели.Р2С150.Значение - Показатели.Р2С230.Значение;
		Настройки.ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р2С230.Значение) 
			И НЕ ЗначениеЗаполнено(Показатели.Р2С245.Значение) И Не Показатели.Р2С250.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_35(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = 'должна быть равна разнице значение строк 150 и 190 Раздела 2, если строка 190 Раздела 2 заполнена';
			|en = 'должна быть равна разнице значение строк 150 и 190 Раздела 2, если строка 190 Раздела 2 заполнена'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 - %3 Итог [%4]';
													|en = '%1 = %2 - %3 Итог [%4]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru = 'Неправильно определена исчисленная сумма земельного налога, подлежащая уплате в ';
			|en = 'Неправильно определена исчисленная сумма земельного налога, подлежащая уплате в '") +
		НСтр("ru = 'бюджет за налоговый период';
			|en = 'бюджет за налоговый период'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С150, Р2С190, Р2С250", Раздел2);
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		
		РасчетнаяСумма = Показатели.Р2С150.Значение - Показатели.Р2С190.Значение;
		Настройки.ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р2С190.Значение) И Не Показатели.Р2С250.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_36(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = 'должна быть равна разнице значений строк 150 и 210 Раздела 2, если строка 210 Раздела 2 заполнена';
			|en = 'должна быть равна разнице значений строк 150 и 210 Раздела 2, если строка 210 Раздела 2 заполнена'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 - %3 Итог [%4]';
													|en = '%1 = %2 - %3 Итог [%4]'"));
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С150, Р2С210, Р2С250", Раздел2);
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		
		РасчетнаяСумма = Показатели.Р2С150.Значение - Показатели.Р2С210.Значение;
		Настройки.ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р2С210.Значение) И НЕ Показатели.Р2С250.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_37(СтруктураРезультата)
	...
	// Формула КС: Р2С250 = Р2С150 - Р2С245 если Р2С245 заполнена	
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 250 Раздела 2) ';
			|en = 'Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 250 Раздела 2) '") +
		НСтр("ru = 'должна быть равна разнице значений строк 150 и 245 Раздела 2, если строка 245 Раздела 2 заполнена';
			|en = 'должна быть равна разнице значений строк 150 и 245 Раздела 2, если строка 245 Раздела 2 заполнена'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 - %3 Итог [%4]';
													|en = '%1 = %2 - %3 Итог [%4]'"));
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы,, "Р2С230");
		
		РасчетнаяСумма = Показатели.Р2С150.Значение - Показатели.Р2С245.Значение;
		Настройки.ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р2С245.Значение) 
			И НЕ ЗначениеЗаполнено(Показатели.Р2С230.Значение) И НЕ Показатели.Р2С250.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			Показатели.Р2С250.Значение, Показатели.Р2С150.Значение, Показатели.Р2С245.Значение, РасчетнаяСумма);
		ДобавитьЭлементВСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверитьКС_1_38_доп(СтруктураРезультата)
	
	// КС отключено, т.к. стр. 245 может быть не заполнена, если стр. 120 = стр. 240а Раздела 2
	Возврат;
	
	// Формула КС: Р2С245 > 0 если Р2С240 заполнена
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка КС 1.38.доп';
													|en = 'Проверка КС 1.38.доп'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ п.2 ст.387';
													|en = 'НК РФ п.2 ст.387'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Значение строки 245 Раздела 2 должно быть заполнено, если заполнена строка 240 Раздела 2';
			|en = 'Значение строки 245 Раздела 2 должно быть заполнено, если заполнена строка 240 Раздела 2'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
		НСтр("ru = 'Значение строки 245 (""%1"") > 0 при условии что значение строки 240 (""%2"") заполнено';
			|en = 'Значение строки 245 (""%1"") > 0 при условии что значение строки 240 (""%2"") заполнено'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не исчислена сумма налоговой льготы';
														|en = 'Не исчислена сумма налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С240_Код, Р2С245", Раздел2);
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р2С240_Код.Значение) И Не Показатели.Р2С245.Значение > 0;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			Показатели.Р2С245.Значение, Показатели.Р2С240_Код.Значение);
		Настройки.ЭтоОшибка = ЭтоОшибка;
		ДобавитьЭлементВСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверитьКС_1_39_доп(СтруктураРезультата)
	
	// Формула КС: Р2С245 = Р2С110 * (Р2С120 - Р2С241) * Р2С140 / 100 если Р2С240 заполнена, Р2С145 = 0, Р2С040 = 0
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка КС 1.39.доп';
													|en = 'Проверка КС 1.39.доп'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ ст.396';
													|en = 'НК РФ ст.396'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'При наличии налоговой льготы в виде снижения налоговой ставки (строка 240 Раздела 2), строка 245 Разде';
			|en = 'При наличии налоговой льготы в виде снижения налоговой ставки (строка 240 Раздела 2), строка 245 Разде'") +
		НСтр("ru = 'ла 2 = строка 110 Раздела 2 х (строка 120 Раздела 2 - строка 240а Раздела 2) х строка 140 Раздела 2 / ';
			|en = 'ла 2 = строка 110 Раздела 2 х (строка 120 Раздела 2 - строка 240а Раздела 2) х строка 140 Раздела 2 / '") +
		НСтр("ru = '100 (при отсутствии коэффициента Ки)';
			|en = '100 (при отсутствии коэффициента Ки)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 х (%3 - %4) х %5 / 100 Итог [%8]';
													|en = '%1 = %2 х (%3 - %4) х %5 / 100 Итог [%8]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru = 'Неправильно исчислена сумма налоговой льготы';
			|en = 'Неправильно исчислена сумма налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("НомерКС", 39);
	
	ПроверитьКС_1_44_код(Настройки);
	СтруктураРезультата = ОбработатьРезультат(Настройки, СтруктураРезультата);
	
КонецПроцедуры

&НаСервере
Процедура ПроверитьКС_1_40_доп(СтруктураРезультата)
	
	// Формула КС: Р2С245 = Р2С110 * (Р2С120 - Р2С241) * Р2С140 * Р2С145 / 100 если Р2С240 заполнена, Р2С145 > 0, 
	// Р2С040 = 0
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка КС 1.40.доп';
													|en = 'Проверка КС 1.40.доп'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ ст.396';
													|en = 'НК РФ ст.396'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'При наличии налоговой льготы в виде снижения налоговой ставки (строка 240 Раздела 2), строка 245 Разде';
			|en = 'При наличии налоговой льготы в виде снижения налоговой ставки (строка 240 Раздела 2), строка 245 Разде'") +
		НСтр("ru = 'ла 2 = строка 110 Раздела 2 х (строка 120 Раздела 2 - строка 240а Раздела 2) х строка 140 Раздела 2 х ';
			|en = 'ла 2 = строка 110 Раздела 2 х (строка 120 Раздела 2 - строка 240а Раздела 2) х строка 140 Раздела 2 х '") +
		НСтр("ru = 'строка 145 Раздела 2 / 100 (при наличии коэффициента Ки)';
			|en = 'строка 145 Раздела 2 / 100 (при наличии коэффициента Ки)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 х (%3 - %4) х %5 х %6 / 100 Итог [%8]';
													|en = '%1 = %2 х (%3 - %4) х %5 х %6 / 100 Итог [%8]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru = 'Неправильно исчислена сумма налоговой льготы';
			|en = 'Неправильно исчислена сумма налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("НомерКС", 40);
...
&НаСервере
Процедура ПроверитьКС_1_41_доп(СтруктураРезультата)
	
	// Формула КС: Р2С245 = Р2С110 * (Р2С120 - Р2С241) * Р2С140 * 2 / 100 если Р2С240 заполнена, Р2С145 = 0 и Р2С040 = 1
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка КС 1.41.доп';
													|en = 'Проверка КС 1.41.доп'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ ст.396';
													|en = 'НК РФ ст.396'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'При наличии налоговой льготы в виде снижения налоговой ставки (строка 240 Раздела 2), строка 245 Разде';
		...
		НСтр("ru = 'ла 2 = строка 110 Раздела 2 х (строка 120 Раздела 2 - строка 240а Раздела 2) х строка 140 Раздела 2 х ';
			|en = 'ла 2 = строка 110 Раздела 2 х (строка 120 Раздела 2 - строка 240а Раздела 2) х строка 140 Раздела 2 х '") +
		НСтр("ru = '2 / 100 (при отсутствии коэффициента Ки и осуществлении жилищного строительства (строка 040 Раздела 2 ';
			|en = '2 / 100 (при отсутствии коэффициента Ки и осуществлении жилищного строительства (строка 040 Раздела 2 '") +
		НСтр("ru = '= 1))';
			|en = '= 1))'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 х (%3 - %4) х %5 х %7 / 100 Итог [%8]';
													|en = '%1 = %2 х (%3 - %4) х %5 х %7 / 100 Итог [%8]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru = 'Неправильно исчислена сумма налоговой льготы';
			|en = 'Неправильно исчислена сумма налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("НомерКС", 41);
...
&НаСервере
Процедура ПроверитьКС_1_42_доп(СтруктураРезультата)
	
	// Формула КС: Р2С245 = Р2С110 * (Р2С120 - Р2С241) * Р2С140 * Р2С145 * 2 / 100 если Р2С240 заполнена, Р2С145 > 0 
	// и Р2С040 = 1
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка КС 1.42.доп';
													|en = 'Проверка КС 1.42.доп'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ ст.396';
													|en = 'НК РФ ст.396'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'При наличии налоговой льготы в виде снижения налоговой ставки (строка 240 Раздела 2), строка 245 Разде';
		...
		НСтр("ru = 'ла 2 = строка 110 Раздела 2 х (строка 120 Раздела 2 - строка 240а Раздела 2) х строка 140 Раздела 2 х ';
			|en = 'ла 2 = строка 110 Раздела 2 х (строка 120 Раздела 2 - строка 240а Раздела 2) х строка 140 Раздела 2 х '") +
		НСтр("ru = 'строка 145 Раздела 2 х 2 / 100 (при наличии коэффициента Ки и осуществлении жилищного строительства (с';
			|en = 'строка 145 Раздела 2 х 2 / 100 (при наличии коэффициента Ки и осуществлении жилищного строительства (с'") +
		НСтр("ru = 'трока 040 Раздела 2 = 1))';
			|en = 'трока 040 Раздела 2 = 1))'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 х (%3 - %4) х %5 х %6 х %7 / 100 Итог [%8]';
													|en = '%1 = %2 х (%3 - %4) х %5 х %6 х %7 / 100 Итог [%8]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru = 'Неправильно исчислена сумма налоговой льготы';
			|en = 'Неправильно исчислена сумма налоговой льготы'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("НомерКС", 42);
...
&НаСервере
Процедура ПроверитьКС_1_43_доп(СтруктураРезультата)
	
	// Формула КС: Р2С245 = Р2С110 * (Р2С120 - Р2С241) * Р2С140 * 4 / 100 если Р2С240 заполнена, Р2С145 = 0 и Р2С040 = 2
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка КС 1.43.доп';
													|en = 'Проверка КС 1.43.доп'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ ст.396';
													|en = 'НК РФ ст.396'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'При наличии налоговой льготы в виде снижения налоговой ставки (строка 240 Раздела 2), строка 245 Разде';
		...
		НСтр("ru = 'ла 2 = строка 110 Раздела 2 х (строка 120 Раздела 2 - строка 240а Раздела 2) х строка 140 Раздела 2 х ';
			|en = 'ла 2 = строка 110 Раздела 2 х (строка 120 Раздела 2 - строка 240а Раздела 2) х строка 140 Раздела 2 х '") +
		НСтр("ru = '4 / 100 (при отсутствии коэффициента Ки и осуществлении жилищного строительства (строка 040 Раздела 2 ';
			|en = '4 / 100 (при отсутствии коэффициента Ки и осуществлении жилищного строительства (строка 040 Раздела 2 '") +
		НСтр("ru = '= 2))';
			|en = '= 2))'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
...
&НаСервере
Процедура ПроверитьКС_1_44_доп(СтруктураРезультата)
	
	// Формула КС: Р2С245 = Р2С110 * (Р2С120 - Р2С241) * Р2С140 * Р2С145 * 2 / 100 если Р2С240 заполнена, Р2С145 > 0 
	// и Р2С040 = 2
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка КС 1.44.доп';
													|en = 'Проверка КС 1.44.доп'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ ст.396';
													|en = 'НК РФ ст.396'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'При наличии налоговой льготы в виде снижения налоговой ставки (строка 240 Раздела 2), строка 245 Разде';
		...
		НСтр("ru = 'ла 2 = строка 110 Раздела 2 х (строка 120 Раздела 2 - строка 240а Раздела 2) х строка 140 Раздела 2 х ';
			|en = 'ла 2 = строка 110 Раздела 2 х (строка 120 Раздела 2 - строка 240а Раздела 2) х строка 140 Раздела 2 х '") +
		НСтр("ru = 'строка 145 Раздела 2 х 4 / 100 (при наличии коэффициента Ки и осуществлении жилищного строительства (с';
			|en = 'строка 145 Раздела 2 х 4 / 100 (при наличии коэффициента Ки и осуществлении жилищного строительства (с'") +
		НСтр("ru = 'трока 040 Раздела 2 = 2))';
			|en = 'трока 040 Раздела 2 = 2))'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2 х (%3 - %4) х %5 х %6 х %7 / 100 Итог [%8]';
													|en = '%1 = %2 х (%3 - %4) х %5 х %6 х %7 / 100 Итог [%8]'"));
...
&НаСервере
Процедура ПроверитьКС_1_44_код(Настройки)
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С240_Код, Р2С245, Р2С110, Р2С120, Р2С241, Р2С140, Р2С145, Р2С040", Раздел2);
		
		Если Настройки.НомерКС = 39 Тогда
			Исключения = "Р2С145, Р2С040";
		ИначеЕсли Настройки.НомерКС = 40 Тогда
			Исключения = "Р2С040";
		ИначеЕсли Настройки.НомерКС = 41 ИЛИ Настройки.НомерКС = 43 Тогда
			Исключения = "Р2С145";
		Иначе
			Исключения = "";
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(Показатели.Р2С240_Код.Значение) ИЛИ НЕ ЗначениеЗаполнено(Показатели.Р2С241.Значение) Тогда
			Продолжить;
		КонецЕсли;
		
		Если Настройки.НомерКС = 39 И (ЗначениеЗаполнено(Показатели.Р2С145.Значение) 
			ИЛИ ЗначениеЗаполнено(Показатели.Р2С040.Значение)) Тогда
			Продолжить;
		КонецЕсли; 
		
		Если Настройки.НомерКС = 40 И (НЕ ЗначениеЗаполнено(Показатели.Р2С145.Значение) 
			ИЛИ ЗначениеЗаполнено(Показатели.Р2С040.Значение)) Тогда
			Продолжить;
		КонецЕсли; 
		
		Если Настройки.НомерКС = 41 И (ЗначениеЗаполнено(Показатели.Р2С145.Значение) 
			ИЛИ НЕ Показатели.Р2С040.Значение = "1") Тогда
			Продолжить;
		КонецЕсли; 
		
		Если Настройки.НомерКС = 42 И (НЕ ЗначениеЗаполнено(Показатели.Р2С145.Значение) 
			ИЛИ НЕ Показатели.Р2С040.Значение = "1") Тогда
			Продолжить;
		КонецЕсли; 
		
		Если Настройки.НомерКС = 43 И (ЗначениеЗаполнено(Показатели.Р2С145.Значение) 
			ИЛИ НЕ Показатели.Р2С040.Значение = "2") Тогда
			Продолжить;
		КонецЕсли; 
		
		Если Настройки.НомерКС = 44 И (НЕ ЗначениеЗаполнено(Показатели.Р2С145.Значение) 
			ИЛИ НЕ Показатели.Р2С040.Значение = "2") Тогда
			Продолжить;
		КонецЕсли; 
		...
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы,, Исключения);
		Показатели.Р2С145.Значение = ?(Показатели.Р2С145.Значение > 0, Показатели.Р2С145.Значение, 1);
		Коэффициент = ?(ЗначениеЗаполнено(Показатели.Р2С040.Значение) 
			И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Показатели.Р2С040.Значение), 
			Число(Показатели.Р2С040.Значение) * 2, 1);
		Разность = Макс(Показатели.Р2С120.Значение - Показатели.Р2С241.Значение, 0);
		РасчетнаяСумма = Округлить(Показатели.Р2С110.Значение * Разность * Показатели.Р2С140.Значение 
			* Показатели.Р2С145.Значение * Коэффициент / 100);
		Настройки.ЭтоОшибка = НЕ Показатели.Р2С245.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			Показатели.Р2С245.Значение, Показатели.Р2С110.Значение, Показатели.Р2С120.Значение, Показатели.Р2С241.Значение, 
			Показатели.Р2С140.Значение, Показатели.Р2С145.Значение, Коэффициент, РасчетнаяСумма);
		ДобавитьЭлементВСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверитьКС_1_45_доп(СтруктураРезультата)
	
	// Формула КС: Р2С250 = Р2С150 если Р2С190, Р2С210, Р2С230, Р2С245 = 0
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка КС 1.45.доп';
													|en = 'Проверка КС 1.45.доп'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ ст.396';
													|en = 'НК РФ ст.396'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 250 Раздела 2) ';
			|en = 'Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 250 Раздела 2) '") +
		НСтр("ru = 'должна быть равна строке 150 Раздела 2, если строки 190, 210, 230, 245 Раздела 2 не заполнены';
			|en = 'должна быть равна строке 150 Раздела 2, если строки 190, 210, 230, 245 Раздела 2 не заполнены'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = %2';
													|en = '%1 = %2'"));
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С150, Р2С190, Р2С210, Р2С230, Р2С245, Р2С250", Раздел2);
		Если ЗначениеЗаполнено(Показатели.Р2С190.Значение) ИЛИ ЗначениеЗаполнено(Показатели.Р2С210.Значение)
			ИЛИ ЗначениеЗаполнено(Показатели.Р2С230.Значение) ИЛИ ЗначениеЗаполнено(Показатели.Р2С245.Значение) Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = Не Показатели.Р2С250.Значение = Показатели.Р2С150.Значение;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			Показатели.Р2С150.Значение, Показатели.Р2С250.Значение);
		Настройки.ЭтоОшибка = ЭтоОшибка;
		ДобавитьЭлементВСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверитьКС_1_46_доп(СтруктураРезультата)
	
	// Формула КС: Р2С250 = Р2С150 - Р2С230 - Р2С245 если Р2С230, Р2С245 заполнены
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка КС 1.46.доп';
													|en = 'Проверка КС 1.46.доп'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ ст.396';
													|en = 'НК РФ ст.396'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 250 Раздела 2) должна';
			|en = 'Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 250 Раздела 2) должна'") +
		НСтр("ru = ' быть равна разнице значение строк 150 и 230, 245 Раздела 2, если строки 230, 245 Раздела 2 заполнены';
			|en = ' быть равна разнице значение строк 150 и 230, 245 Раздела 2, если строки 230, 245 Раздела 2 заполнены'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2, Страница %1';
													|en = 'Раздел 2, Страница %1'"));
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		РасчетнаяСумма = Показатели.Р2С150.Значение - Показатели.Р2С230.Значение - Показатели.Р2С245.Значение;
		Настройки.ЭтоОшибка = ЗначениеЗаполнено(Показатели.Р2С230.Значение) 
			И ЗначениеЗаполнено(Показатели.Р2С245.Значение) И Не Показатели.Р2С250.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			Показатели.Р2С250.Значение, Показатели.Р2С150.Значение, Показатели.Р2С230.Значение, Показатели.Р2С245.Значение, 
			РасчетнаяСумма);
		ДобавитьЭлементВСтруктуруОшибок(Настройки);
...
Процедура ПроверитьКС_2_01_доп(СтруктураРезультата)
	...
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка Раздела 1 (КС 2.1.доп)';
													|en = 'Проверка Раздела 1 (КС 2.1.доп)'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("РасшифровкаСписка"	, "");
	Настройки.Вставить("ПредставлениеПоказателя", НСтр("ru = 'Значение КБК (стр.010 Раздела 1)';
														|en = 'Значение КБК (стр.010 Раздела 1)'"));
	ТаблицаСоответствий = ТаблицаСоответствий(мДанныеОтчета.Раздел1);
	
	Для каждого СтрокаСоответствия Из ТаблицаСоответствий Цикл
		
		ПоказателиР1 = ЗаполнитьПоказатели("Р1С010", 
			мДанныеОтчета.Раздел1[СтрокаСоответствия.НомерСтраницы - 1].Данные, СтрокаСоответствия.НомерСтроки);
			
		Если НЕ ЗначениеЗаполнено(ПоказателиР1.Р1С010.Значение) Тогда
			Продолжить;
		КонецЕсли;
			
		Настройки.ЭтоОшибка	= ПроверитьСписочноеЗначение(Настройки, ПоказателиР1.Р1С010);
...
Процедура ПроверитьКС_2_02_доп(СтруктураРезультата)
	...
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка Раздела 2 (КС 2.2.доп)';
													|en = 'Проверка Раздела 2 (КС 2.2.доп)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("Показатель"			, НСтр("ru = 'Р2С010';
														|en = 'Р2С010'"));
	Настройки.Вставить("ПредставлениеПоказателя"
											, НСтр("ru = 'Значение КБК (стр.010 Раздела 2)';
													|en = 'Значение КБК (стр.010 Раздела 2)'"));
	Настройки.Вставить("РасшифровкаСписка"	, "");
	
	ПроверитьКС_2_10_код(Настройки);
	СтруктураРезультата = ОбработатьРезультат(Настройки, СтруктураРезультата);
...
Процедура ПроверитьКС_2_03_доп(СтруктураРезультата)
	...
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка Раздела 2 (КС 2.3.доп)';
													|en = 'Проверка Раздела 2 (КС 2.3.доп)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("Показатель"			, НСтр("ru = 'Р2С030';
														|en = 'Р2С030'"));
	Настройки.Вставить("ПредставлениеПоказателя"
											, НСтр("ru = 'Значение категории земель (стр.030 Раздела 2)';
													|en = 'Значение категории земель (стр.030 Раздела 2)'"));
	Настройки.Вставить("РасшифровкаСписка"	, "");
...
Процедура ПроверитьКС_2_04_доп(СтруктураРезультата)
	...
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка Раздела 2 (КС 2.4.доп)';
													|en = 'Проверка Раздела 2 (КС 2.4.доп)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("Показатель"			, НСтр("ru = 'Р2С040';
														|en = 'Р2С040'"));
	Настройки.Вставить("ПредставлениеПоказателя"
											, НСтр("ru = 'Значение периода строительства (стр.040 Раздела 2)';
													|en = 'Значение периода строительства (стр.040 Раздела 2)'"));
	Настройки.Вставить("РасшифровкаСписка"	, "");
...
Процедура ПроверитьКС_2_05_доп(СтруктураРезультата)
	...
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка Раздела 2 (КС 2.5.доп)';
													|en = 'Проверка Раздела 2 (КС 2.5.доп)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("Показатель"			, НСтр("ru = 'Р2С070_Код';
														|en = 'Р2С070_Код'"));
	Настройки.Вставить("ПредставлениеПоказателя"
		, НСтр("ru = 'Значение кода налоговой льготы в виде не облагаемой налогом суммы (стр.070 Раздела 2)';
				|en = 'Значение кода налоговой льготы в виде не облагаемой налогом суммы (стр.070 Раздела 2)'"));
	Настройки.Вставить("РасшифровкаСписка"	, "");
...
Процедура ПроверитьКС_2_06_доп(СтруктураРезультата)
	...
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка Раздела 2 (КС 2.6.доп)';
													|en = 'Проверка Раздела 2 (КС 2.6.доп)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("Показатель"			, НСтр("ru = 'Р2С090_Код';
														|en = 'Р2С090_Код'"));
	Настройки.Вставить("ПредставлениеПоказателя",
		НСтр("ru = 'Значение кода налоговой льготы в виде доли необлагаемой площади земельного участка (стр.090 Раздела 2)';
			|en = 'Значение кода налоговой льготы в виде доли необлагаемой площади земельного участка (стр.090 Раздела 2)'"));
	Настройки.Вставить("РасшифровкаСписка"	, "");
...
Процедура ПроверитьКС_2_07_доп(СтруктураРезультата)
	...
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка Раздела 2 (КС 2.7.доп)';
													|en = 'Проверка Раздела 2 (КС 2.7.доп)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("Показатель"			, НСтр("ru = 'Р2С180_Код';
														|en = 'Р2С180_Код'"));
	Настройки.Вставить("ПредставлениеПоказателя"
		, НСтр("ru = 'Значение кода налоговой льготы в виде освобождения от налогообложения (стр.180 Раздела 2)';
				|en = 'Значение кода налоговой льготы в виде освобождения от налогообложения (стр.180 Раздела 2)'"));
	Настройки.Вставить("РасшифровкаСписка"	, "");
...
Процедура ПроверитьКС_2_08_доп(СтруктураРезультата)
	...
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка Раздела 2 (КС 2.8.доп)';
													|en = 'Проверка Раздела 2 (КС 2.8.доп)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("Показатель"			, НСтр("ru = 'Р2С200';
														|en = 'Р2С200'"));
	Настройки.Вставить("ПредставлениеПоказателя"
		, НСтр("ru = 'Значение кода налоговой льготы в виде освобождения от налогообложения (стр.200 Раздела 2)';
				|en = 'Значение кода налоговой льготы в виде освобождения от налогообложения (стр.200 Раздела 2)'"));
	Настройки.Вставить("РасшифровкаСписка"	, "");
...
Процедура ПроверитьКС_2_09_доп(СтруктураРезультата)
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка Раздела 2 (КС 2.9.доп)';
													|en = 'Проверка Раздела 2 (КС 2.9.доп)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("Показатель"			, НСтр("ru = 'Р2С220_Код';
														|en = 'Р2С220_Код'"));
	Настройки.Вставить("ПредставлениеПоказателя"
		, НСтр("ru = 'Значение кода налоговой льготы в виде уменьшения суммы налога (стр.220 Раздела 2)';
				|en = 'Значение кода налоговой льготы в виде уменьшения суммы налога (стр.220 Раздела 2)'"));
	Настройки.Вставить("РасшифровкаСписка"	, "");
...
Процедура ПроверитьКС_2_10_доп(СтруктураРезультата)
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru = 'Проверка Раздела 2 (КС 2.10.доп)';
													|en = 'Проверка Раздела 2 (КС 2.10.доп)'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1';
													|en = 'Страница %1'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	Настройки.Вставить("Показатель"			, НСтр("ru = 'Р2С240_Код';
														|en = 'Р2С240_Код'"));
	Настройки.Вставить("ПредставлениеПоказателя"
		, НСтр("ru = 'Значение кода налоговой льготы в виде снижения налоговой ставки (стр.240 Раздела 2)';
				|en = 'Значение кода налоговой льготы в виде снижения налоговой ставки (стр.240 Раздела 2)'"));
	Настройки.Вставить("РасшифровкаСписка"	, "");
...
&НаСервере
Процедура ПроверитьКС_2_10_код(Настройки)
	
	КоличествоСтраниц = мДанныеОтчета.Раздел2.Количество();
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели(Настройки.Показатель, Раздел2);
...
Функция ПроверитьСписочноеЗначение(Настройки, Показатель, КоличествоЗначений = 20, Направление = "")
	...
	ИмяПоказателя	= ?(СтрНайти(Показатель.Имя, "П00001") = 0, Показатель.Имя, СтрЗаменить(Показатель.Имя, "_%1", ""));
		...
		Если НЕ ЗначениеЗаполнено(Настройки.ОписаниеОшибки) Тогда
			Настройки.Вставить("ОписаниеОшибки"	, Настройки.ПредставлениеПоказателя 
				+ НСтр("ru = ' должно соответствовать одному из следующих значений: ';
						|en = ' должно соответствовать одному из следующих значений: '") + Настройки.РасшифровкаСписка);
		КонецЕсли;
	...
	Если УсловныйКодПоказателя = "Р1С010" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Код бюджетной классификации';
				|en = 'Код бюджетной классификации'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1С021" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (в рублях)';
				|en = 'Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (в рублях)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1С023" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Сумма авансовых платежей, подлежащая уплате в бюджет за первый квартал (в рублях)';
				|en = 'Сумма авансовых платежей, подлежащая уплате в бюджет за первый квартал (в рублях)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1С025" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Сумма авансовых платежей, подлежащая уплате в бюджет за второй квартал (в рублях)';
				|en = 'Сумма авансовых платежей, подлежащая уплате в бюджет за второй квартал (в рублях)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1С027" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Сумма авансовых платежей, подлежащая уплате в бюджет за третий квартал (в рублях)';
				|en = 'Сумма авансовых платежей, подлежащая уплате в бюджет за третий квартал (в рублях)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1С030" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Сумма налога, подлежащая уплате в бюджет (в рублях)';
				|en = 'Сумма налога, подлежащая уплате в бюджет (в рублях)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1С040" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Сумма налога, исчисленная к уменьшению (в рублях)';
				|en = 'Сумма налога, исчисленная к уменьшению (в рублях)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2КНЗУ" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Кадастровый номер земельного участка';
				|en = 'Кадастровый номер земельного участка'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С050" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Кадастровая стоимость (доля кадастровой стоимости) земельного участка / ';
				|en = 'Кадастровая стоимость (доля кадастровой стоимости) земельного участка / '") +
			НСтр("ru = 'Нормативная цена земли (в рублях)';
				|en = 'Нормативная цена земли (в рублях)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С060_Числитель" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Доля налогоплательщика в праве на земельный участок (числитель)';
				|en = 'Доля налогоплательщика в праве на земельный участок (числитель)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С060_Знаменатель" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Доля налогоплательщика в праве на земельный участок (знаменатель)';
				|en = 'Доля налогоплательщика в праве на земельный участок (знаменатель)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С070_Код" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Код налоговой льготы в виде не облагаемой налогом суммы';
				|en = 'Код налоговой льготы в виде не облагаемой налогом суммы'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С070_Основание" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Основание налоговой льготы в виде не облагаемой налогом суммы';
				|en = 'Основание налоговой льготы в виде не облагаемой налогом суммы'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С080" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Не облагаемая налогом сумма (в рублях) (п. 2 ст. 387 НК РФ)';
				|en = 'Не облагаемая налогом сумма (в рублях) (п. 2 ст. 387 НК РФ)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С090_Код" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Код налоговой льготы в виде доли необлагаемой площади земельного участка';
				|en = 'Код налоговой льготы в виде доли необлагаемой площади земельного участка'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С090_Основание" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Основание налоговой льготы в виде доли необлагаемой площади земельного участка';
				|en = 'Основание налоговой льготы в виде доли необлагаемой площади земельного участка'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С100_Числитель" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Доля необлагаемой площади земельного участка (п. 2 ст. 387 НК РФ) (числитель)';
				|en = 'Доля необлагаемой площади земельного участка (п. 2 ст. 387 НК РФ) (числитель)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С100_Знаменатель" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Доля необлагаемой площади земельного участка (п. 2 ст. 387 НК РФ) (знаменатель)';
				|en = 'Доля необлагаемой площади земельного участка (п. 2 ст. 387 НК РФ) (знаменатель)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С130" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Количество полных месяцев владения земельным участком в течение налогового периода';
				|en = 'Количество полных месяцев владения земельным участком в течение налогового периода'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С150" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Сумма исчисленного налога (в рублях)';
				|en = 'Сумма исчисленного налога (в рублях)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С160" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Количество полных месяцев использования льготы';
				|en = 'Количество полных месяцев использования льготы'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С180_Код" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Код налоговой льготы в виде освобождения от налогообложения';
				|en = 'Код налоговой льготы в виде освобождения от налогообложения'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С180_Основание" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Основание налоговой льготы в виде освобождения от налогообложения';
				|en = 'Основание налоговой льготы в виде освобождения от налогообложения'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С190" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Сумма налоговой льготы (в рублях) (п. 2 ст. 387 НК РФ)';
				|en = 'Сумма налоговой льготы (в рублях) (п. 2 ст. 387 НК РФ)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С210" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Сумма налоговой льготы (в рублях) (ст. 395, ст. 7 НК РФ)';
				|en = 'Сумма налоговой льготы (в рублях) (ст. 395, ст. 7 НК РФ)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С220_Код" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Код налоговой льготы в виде уменьшения суммы налога';
				|en = 'Код налоговой льготы в виде уменьшения суммы налога'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С220_Основание" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Основание налоговой льготы в виде уменьшения суммы налога';
				|en = 'Основание налоговой льготы в виде уменьшения суммы налога'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С220" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Процент, уменьшающий исчисленную сумму налога';
				|en = 'Процент, уменьшающий исчисленную сумму налога'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С240_Код" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Код налоговой льготы в виде снижения налоговой ставки';
				|en = 'Код налоговой льготы в виде снижения налоговой ставки'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С240_Основание" Тогда
		...
		Результат.Вставить("Представление", 
			НСтр("ru = 'Основание налоговой льготы в виде снижения налоговой ставки';
				|en = 'Основание налоговой льготы в виде снижения налоговой ставки'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2С241" Тогда
		Результат.Вставить("Имя", "П000020024003_ПониженнаяСтавка");
		Результат.Вставить("Представление", 
			НСтр("ru = 'Пониженная налоговая ставка (%)';
				|en = 'Пониженная налоговая ставка (%)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2С245" Тогда
		Результат.Вставить("Имя", "П000020024503");
		Результат.Вставить("Представление", 
			НСтр("ru = 'Сумма налоговой льготы (в рублях) (п. 2 ст. 387 НК РФ)';
				|en = 'Сумма налоговой льготы (в рублях) (п. 2 ст. 387 НК РФ)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
...
Процедура ПолучитьСписочныеЗначения()
	...
	МассивПроверяемых.Добавить("П000010001003"			); // кбк
	МассивПроверяемых.Добавить("П000020001003"			); // кбк
	МассивПроверяемых.Добавить("П000020003003"			); // категория земель
	МассивПроверяемых.Добавить("П000020004003"			); // период строительства
	МассивПроверяемых.Добавить("П000020007003_КодЛьготы"); // код льготы
	МассивПроверяемых.Добавить("П000020009003_КодЛьготы"); // код льготы
	МассивПроверяемых.Добавить("П000020018003_КодЛьготы"); // код льготы
	МассивПроверяемых.Добавить("П000020020003"			); // код льготы
	МассивПроверяемых.Добавить("П000020022003_КодЛьготы"); // код льготы
...
Функция ПредставлениеПоказателя(Показатель, ПолноеПредставление, НомерСтраницы = Неопределено)
	...
	Если Показатель.Имя = "П000020024003_ПониженнаяСтавка" Тогда
		ПодстрокаСтроки = Нстр("ru = '240а';
								|en = '240а'");
	Иначе
		ПодстрокаСтроки = Сред(Показатель.Имя, 9, 3);
	КонецЕсли;
	ПодстрокаСтраницы = ?(ПодстрокаРаздела = "Раздел 1", "", Нстр("ru = ' Страница ';
																	|en = ' Страница '") + НомерСтраницы);
