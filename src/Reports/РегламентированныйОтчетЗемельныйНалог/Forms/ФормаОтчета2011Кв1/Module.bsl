...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("СпискиВыбора2011Кв1");
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Титульный",	"МЧБ_2011Кв1_Титульный");
	СоответствиеРазделовМакетам.Вставить("Раздел1",		"МЧБ_2011Кв1_Раздел1");
	СоответствиеРазделовМакетам.Вставить("Раздел2",		"МЧБ_2011Кв1_Раздел2");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Для Каждого Лист Из Раздел1 Цикл
		...
		Для Инд = 1 По 2 Цикл
			...
			БлокЗаполнен = НЕ ПустаяСтрока(ТекКБК) ИЛИ НЕ ПустаяСтрока(ТекОКАТО)
	...
	ПовторыКБКОКАТО = КБКОКАТОРаздела1.Скопировать();
	ПовторыКБКОКАТО.Колонки.Добавить("Количество");
	ПовторыКБКОКАТО.ЗаполнитьЗначения(1, "Количество");
	ПовторыКБКОКАТО.Свернуть("КБК, ОКАТО", "Количество");
	
	Для Каждого КБКОКАТО Из ПовторыКБКОКАТО Цикл
		Если КБКОКАТО.Количество > 1 Тогда
			Если ЗначениеЗаполнено(КБКОКАТО.КБК) И ЗначениеЗаполнено(КБКОКАТО.ОКАТО) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 1 обнаружено повторяющееся сочетание кода КБК " + КБКОКАТО.КБК + " и кода ОКАТО " + КБКОКАТО.ОКАТО + "!", "Раздел1", , 1);
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
