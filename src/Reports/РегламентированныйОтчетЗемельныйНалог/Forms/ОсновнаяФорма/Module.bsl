...
Процедура ПоказатьПериод(Форма)
	...
	Форма.НадписьКтоСдаетОтчет = ?(ГодПериода > 2014, НСтр("ru = 'Только организации (п.1 ст. 398 НК РФ).';
															|en = 'Только организации (п.1 ст. 398 НК РФ).'"), "");
	...
	Иначе
		...
		НСтр("ru = 'Утратила силу с 2021 года. Основание: приказ ФНС России от 04.09.2019 N ММВ-7-21/440@';
			|en = 'Утратила силу с 2021 года. Основание: приказ ФНС России от 04.09.2019 N ММВ-7-21/440@'"),
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтчетОригиналЗа2020Год = (ВРег(СокрЛП(мСкопированаФорма)) = ВРег("ФормаОтчета2018Кв3"))
	И мСохраненныйДок <> Неопределено И Год(мСохраненныйДок.ДатаОкончания) = 2020;
...
Процедура ОткрытьФормуОтчета(Команда)
	...
	Если мСкопированаФорма <> Неопределено Тогда
		...
		ИначеЕсли Год(мДатаКонцаПериодаОтчета) < 2020 И ОтчетОригиналЗа2020Год Тогда
			
			ПоказатьПредупреждение(,НСтр("ru = 'Копирование формы отчета за 2020 год на более ранние периоды не предусмотрено!';
										|en = 'Копирование формы отчета за 2020 год на более ранние периоды не предусмотрено!'"));
			Возврат;
...
Процедура ВыбратьФорму(Команда)
	...
	ДопТекстОписания = НСтр("ru = 'Декларацию по земельному налогу представляют организации (п.1 ст. 398 НК РФ).';
							|en = 'Декларацию по земельному налогу представляют организации (п.1 ст. 398 НК РФ).'");
...
Функция ДоступныеОрганизацииОтсутствуютТекст()
	...
	Возврат НСтр(
		...
		|en = 'Декларацию по земельному налогу представляют только организации (п.1 ст. 398 НК РФ).
		|В справочнике ""Организации"" сведения об организациях отсутствуют.'");
