...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "01/06/2017";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	РегламентированнаяОтчетностьКлиентСервер.ИмяФормыОтчета(ЭтаФорма.ИмяФормы) + "_" + СтруктураРеквизитовФормы.НаимТекущегоРаздела));
...
Процедура ИнициализироватьМногоуровневыеРазделы()
	...
	Для каждого Элем Из мСтруктураМногоуровневыхРазделов Цикл
		...
		ТабличныйДокумент.Очистить();
		ТабличныйДокумент.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(
		РегламентированнаяОтчетностьКлиентСервер.ИмяФормыОтчета(ЭтаФорма.ИмяФормы) + "_" + Элем.Ключ));
...
Процедура ИнициализироватьПодчиненныеРазделы(Знач ИмяРаздела, Ветка)
	...
	Если мСтруктураМногоуровневыхРазделов[ИмяРаздела].Свойство("ИменаПодчиненныхРазделов") Тогда
		...
		Для каждого ИмяПодчРаздела Из ИменаПодчРазделов Цикл
			...
			ТабличныйДокумент.Очистить();
			ТабличныйДокумент.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(
			РегламентированнаяОтчетностьКлиентСервер.ИмяФормыОтчета(ЭтаФорма.ИмяФормы) + "_" + ИмяПодчРаздела));
...
Процедура ИнициализироватьОбычныеРазделы(МассивИменРазделов = Неопределено)
	...
	Для каждого ИмяРаздела Из МассивИменРазделов Цикл
		...
		ТабличныйДокумент.Очистить();
		ТабличныйДокумент.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(
		РегламентированнаяОтчетностьКлиентСервер.ИмяФормыОтчета(ЭтаФорма.ИмяФормы) + "_" + ИмяРаздела));
...
Процедура ПолучитьСведенияОПоказателяхОтчета()
	...
	РегламентированнаяОтчетностьКлиентСервер.ИмяОтчета(ИмяФормы), РегламентированнаяОтчетностьКлиентСервер.ИмяФормыОтчета(ЭтаФорма.ИмяФормы), ПараметрыОтчета);
...
Процедура СформироватьСпискиВыбора()
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '20180101' Тогда
		ИмяСекцииКодыКатегорийЗемель = "КодыКатегорийЗемель20180101";
	Иначе
		ИмяСекцииКодыКатегорийЗемель = "КодыКатегорийЗемель";
	КонецЕсли;
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020003003", 12, ,
	"Выбор кода категории земель", КоллекцияСписковВыбора[ИмяСекцииКодыКатегорийЗемель]);
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС, КПП, Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт
	...
	СтруктураРеквизитовФормы.мСохраненныйДок.ИсточникОтчета           = Метаданные.Отчеты[РегламентированнаяОтчетностьКлиентСервер.ИмяОтчета(ЭтаФорма.ИмяФормы)].Имя;
	СтруктураРеквизитовФормы.мСохраненныйДок.НаименованиеОтчета       = Метаданные.Отчеты[РегламентированнаяОтчетностьКлиентСервер.ИмяОтчета(ЭтаФорма.ИмяФормы)].ОсновнаяФорма.Синоним;
	...
	Если ИмяПоказателя = "П000020018003_КодЛьготы" ИЛИ ИмяПоказателя = "П000020020003"
		...
		Если ЗначениеЗаполнено(ЗначПоказателя) Тогда
			...
			Если МассивОчищаемыхСтрок.Количество() > 0 Тогда
				...
				|en = 'Не допускается одновременное заполнение групп строк: 180, 190; 200, 210; 220, 220а, 230. %1 %2 %3.'"),
				ТекстНачало, ТекстСередина, ТекстОкончание);
				Сообщение.Сообщить();
			КонецЕсли;
...
Процедура ВывестиДанныеВТабличныйДокумент(ИмяТекРаздела, НовыеНомераСтрокМногоуровнегоРаздела = Неопределено)
	...
	ТабличныйДокумент.Очистить();
	ТабличныйДокумент.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(
	РегламентированнаяОтчетностьКлиентСервер.ИмяФормыОтчета(ЭтаФорма.ИмяФормы) + "_" + ИмяТекРаздела));
	...
	ИначеЕсли ИмяРаздела = "Раздел2" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Если Стр160 > 0 Тогда
					Стр170 = Окр(Макс(Форма.ТабличныйДокумент.Области["П000020013003"].Значение - Стр160, 0) / 12, 4);
				КонецЕсли;
				...
				Если СтрЧислоВхождений(СтруктЗависимыхЯчеек.П000020014003, ОбластьИмя) > 0 Тогда
					...
					= Окр(Форма.ТабличныйДокумент.Области["П000020013003"].Значение / 12, 4);
				...
				Если СтрЧислоВхождений(СтруктЗависимыхЯчеек.П000020015003, ОбластьИмя) > 0 Тогда
					...
					ОсноваДляРасчетаСтр150 = Стр110 * Стр120 * Стр140 / 100;
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел2" Тогда
		...
		Для каждого ВеткаР2 Из ДеревоДанных.Строки Цикл
			...
			Стр170 = 0;
			Для каждого Пок Из ВеткаР2.Данные Цикл
				Если СтруктЗависимыхЯчеек.Свойство(Пок.Ключ) Тогда
					Продолжить;
				КонецЕсли;
			...
			Стр160 = ВеткаР2.Данные["П000020016003"];
			Если Стр160 > 0 Тогда
				Стр170 = Окр(Макс(ВеткаР2.Данные["П000020013003"] - Стр160, 0) / 12, 4);
			КонецЕсли;
			
			ВеткаР2.Данные.Вставить("П000020017003", Стр170);
			...
			Строка060_Числ = 1;
			Строка060_Знам = 1;
			Если ЗначениеЗаполнено(ВеткаР2.Данные["П000020006003_Числ"])
				И ЗначениеЗаполнено(ВеткаР2.Данные["П000020006003_Знам"]) Тогда
				Строка060_Числ = ВеткаР2.Данные["П000020006003_Числ"];
				Строка060_Знам = ВеткаР2.Данные["П000020006003_Знам"];
			КонецЕсли;
			...
			Строка100_Числ = 1;
			Строка100_Знам = 1;
			Если ЗначениеЗаполнено(ВеткаР2.Данные["П000020010003_Числ"])
				И ЗначениеЗаполнено(ВеткаР2.Данные["П000020010003_Знам"]) Тогда
				Строка100_Числ = ВеткаР2.Данные["П000020010003_Числ"];
				Строка100_Знам = ВеткаР2.Данные["П000020010003_Знам"];
				ЗаполненаСтр100 = Истина;
			КонецЕсли;
			
			Стр170 = ВеткаР2.Данные["П000020017003"];
			...
			ВеткаР2.Данные.Вставить("П000020011003",
			РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Стр110));
			
			// Стр. 140.
			//
			ВеткаР2.Данные.Вставить("П000020014003", Окр(ВеткаР2.Данные["П000020013003"] / 12, 4));
			
			// Стр. 150.
			//
			Стр040 = СокрЛП(ВеткаР2.Данные["П000020004003"]);
			
			Стр110 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, ВеткаР2.Данные["П000020011003"]);
			
			Стр120 = ВеткаР2.Данные["П000020012003"];
			
			Стр140 = ВеткаР2.Данные["П000020014003"];
			...
			//
			Стр190 = 0;
			КодЛьготы = СокрЛП(ВеткаР2.Данные["П000020018003_КодЛьготы"]);
			...
			//
			Стр210 = 0;
			КодЛьготы = СокрЛП(ВеткаР2.Данные["П000020020003"]);
			...
			ВеткаР2.Данные.Вставить("П000020021003",
			РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Стр210));
			
			// Стр. 230.
			//
			КодЛьготы = СокрЛП(ВеткаР2.Данные["П000020022003_КодЛьготы"]);
			Стр220_Процент = ВеткаР2.Данные["П000020022003_Процент"];
			
			Если ЗначениеЗаполнено(КодЛьготы) Тогда
				
				Если Стр220_Процент > 0 Тогда
					
					Стр150 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, ВеткаР2.Данные["П000020015003"]);
					
					Стр230 = Стр150 * Стр220_Процент / 100;
					
					ВеткаР2.Данные.Вставить("П000020023003",
					РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Стр230));
			...
			Иначе
				...
				ВеткаР2.Данные.Вставить("П000020023003", 0);
			...
			Стр190 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
			Форма, ВеткаР2.Данные["П000020019003"]);
			
			Стр210 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
			Форма, ВеткаР2.Данные["П000020021003"]);
			
			Стр230 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
			Форма, ВеткаР2.Данные["П000020023003"]);
			...
			ВеткаР2.Данные.Вставить("П000020025003", Макс(Стр250, 0));
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ.Вставить("П000020015003", "П000020004003;П000020005003;П000020006003_Числ;П000020006003_Знам;П000020007003_КодЛьготы;П000020008003;П000020009003_КодЛьготы;П000020010003_Числ;П000020010003_Знам;П000020011003;П000020012003;П000020013003;П000020014003;П000020016003;П000020017003");
	Структ.Вставить("П000020017003");
	Структ.Вставить("П000020019003", "П000020004003;П000020005003;П000020006003_Числ;П000020006003_Знам;П000020007003_КодЛьготы;П000020008003;П000020009003_КодЛьготы;П000020010003_Числ;П000020010003_Знам;П000020011003;П000020012003;П000020013003;П000020014003;П000020015003;П000020016003;П000020017003;П000020018003_КодЛьготы");
	Структ.Вставить("П000020021003", "П000020004003;П000020005003;П000020006003_Числ;П000020006003_Знам;П000020007003_КодЛьготы;П000020008003;П000020009003_КодЛьготы;П000020010003_Числ;П000020010003_Знам;П000020011003;П000020012003;П000020013003;П000020014003;П000020015003;П000020016003;П000020017003;П000020020003");
	Структ.Вставить("П000020023003", "П000020004003;П000020005003;П000020006003_Числ;П000020006003_Знам;П000020007003_КодЛьготы;П000020008003;П000020009003_КодЛьготы;П000020010003_Числ;П000020010003_Знам;П000020011003;П000020012003;П000020013003;П000020014003;П000020015003;П000020016003;П000020017003;П000020022003_КодЛьготы;П000020022003_Процент");
	Структ.Вставить("П000020025003", "П000020004003;П000020005003;П000020006003_Числ;П000020006003_Знам;П000020007003_КодЛьготы;П000020008003;П000020009003_КодЛьготы;П000020010003_Числ;П000020010003_Знам;П000020011003;П000020012003;П000020013003;П000020014003;П000020015003;П000020016003;П000020017003;П000020018003_КодЛьготы;П000020019003;П000020020003;П000020021003;П000020022003_КодЛьготы;П000020022003_Процент;П000020023003");
...
Процедура ОткрытьФормуВыбораСтраниц(ВариантВыбора)
	...
	мПараметры.Вставить("ИмяОтчета", РегламентированнаяОтчетностьКлиентСервер.ИмяОтчета(ЭтаФорма.ИмяФормы));
...
Процедура ОчисткаРегламентированногоОтчетаНаСервере(ИмяСтраницы) Экспорт
	...
	Иначе
		...
		ТабличныйДокумент.Очистить();
		ТабличныйДокумент.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(
		РегламентированнаяОтчетностьКлиентСервер.ИмяФормыОтчета(ЭтаФорма.ИмяФормы) + "_" + ИмяСтраницы));
...
Функция ЗаполнитьАвтоНаСервере()
	...
	ПараметрыПроцедуры.Вставить("ИДОтчета", РегламентированнаяОтчетностьКлиентСервер.ИмяОтчета(ЭтаФорма.ИмяФормы));
	ПараметрыПроцедуры.Вставить("ИДРедакцииОтчета", РегламентированнаяОтчетностьКлиентСервер.ИмяФормыОтчета(ЭтаФорма.ИмяФормы));
...
Процедура Расшифровать(Команда)
	...
	РегламентированнаяОтчетностьКлиент.ОткрытьРасшифровкуОтчета(РегламентированнаяОтчетностьКлиентСервер.ИмяОтчета(ЭтаФорма.ИмяФормы),
	РегламентированнаяОтчетностьКлиентСервер.ИмяФормыОтчета(ЭтаФорма.ИмяФормы), ИДИменПоказателей, ПараметрыОтчета)
...
Процедура ПроверитьКС_1_04(СтруктураРезультата)
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Если Не Показатели.Р2С080.Значение > 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		ЭтоОшибка = Не ЗначениеЗаполнено(Показатели.Р2С070_Код.Значение);
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_05(СтруктураРезультата)
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С070_Код, Р2С080", Раздел2);
		Если Не ЗначениеЗаполнено(Показатели.Р2С070_Код.Значение) Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = Не Показатели.Р2С080.Значение > 0;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_06(СтруктураРезультата)
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Если Не (ЗначениеЗаполнено(Показатели.Р2С100_Числитель.Значение) И 
		...
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = Не ЗначениеЗаполнено(Показатели.Р2С090_Код.Значение);
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_07(СтруктураРезультата)
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С090_Код, Р2С100_Числитель, Р2С100_Знаменатель", Раздел2);
		Если Не ЗначениеЗаполнено(Показатели.Р2С090_Код.Значение) Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = Не (ЗначениеЗаполнено(Показатели.Р2С100_Числитель.Значение) И 
			ЗначениеЗаполнено(Показатели.Р2С100_Знаменатель.Значение));
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_08(СтруктураРезультата)
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Если Не Показатели.Р2С190.Значение > 0 Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = Не ЗначениеЗаполнено(Показатели.Р2С180_Код.Значение);
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_09(СтруктураРезультата)
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С180_Код, Р2С190", Раздел2);
		Если Не ЗначениеЗаполнено(Показатели.Р2С180_Код.Значение) Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = Не Показатели.Р2С190.Значение > 0;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_10(СтруктураРезультата)
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Если Не Показатели.Р2С210.Значение > 0 Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = Не ЗначениеЗаполнено(Показатели.Р2С200.Значение);
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_11(СтруктураРезультата)
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С200, Р2С210", Раздел2);
		Если Не ЗначениеЗаполнено(Показатели.Р2С200.Значение) Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = Не Показатели.Р2С210.Значение > 0;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_12(СтруктураРезультата)
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Если Не Показатели.Р2С230.Значение > 0 Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = Не ЗначениеЗаполнено(Показатели.Р2С220_Код.Значение);
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_13(СтруктураРезультата)
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С220_Код, Р2С230", Раздел2);
		Если Не ЗначениеЗаполнено(Показатели.Р2С220_Код.Значение) Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = Не Показатели.Р2С230.Значение > 0;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_15(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = 'быть больше нуля, если заполнены строки 180 и 190 Раздела 2 или строки 200 и 210 Раздела 2';
			|en = 'быть больше нуля, если заполнены строки 180 и 190 Раздела 2 или строки 200 и 210 Раздела 2'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2 Страница %1';
													|en = 'Раздел 2 Страница %1'"));
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Если Не (ЗначениеЗаполнено(Показатели.Р2С180_Код.Значение) И ЗначениеЗаполнено(Показатели.Р2С190.Значение) Или
			ЗначениеЗаполнено(Показатели.Р2С200.Значение) И ЗначениеЗаполнено(Показатели.Р2С210.Значение)) Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		ЭтоОшибка = Не Показатели.Р2С160.Значение > 0;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
...
Процедура ПроверитьКС_1_25(СтруктураРезультата)
	...
	// Формула КС: Р2С150 = Р2С110 * Р2С120 * Р2С140 / 100 если Р2С040 не заполнена
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ ст.396';
													|en = 'НК РФ ст.396'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма исчисленного налога (строка 150 Раздела 2) должна быть равна (строка 110 ';
			|en = 'Сумма исчисленного налога (строка 150 Раздела 2) должна быть равна (строка 110 '") +
		НСтр("ru = 'Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2) / 100';
			|en = 'Раздела 2 х строка 120 Раздела 2 х строка 140 Раздела 2) / 100'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2 Страница %1';
													|en = 'Раздел 2 Страница %1'"));
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С040, Р2С110, Р2С120, Р2С140, Р2С150", Раздел2);
		Если ЗначениеЗаполнено(Показатели.Р2С040.Значение) Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
...
Процедура ПроверитьКС_1_26(СтруктураРезультата)
	...
	// Формула КС: Р2С150 = Р2С110 * Р2С120 * Р2С140 * 2 / 100 если Р2С040 = "1"
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'НК РФ ст.396';
													|en = 'НК РФ ст.396'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма исчисленного налога (строка 150 Раздела 2) должна быть равна (строка 110 Раздела 2 х ';
			|en = 'Сумма исчисленного налога (строка 150 Раздела 2) должна быть равна (строка 110 Раздела 2 х '") +
		НСтр("ru = 'строка 120 Раздела 2 х строка 140 Раздела 2 х 2) / 100, если значение строки 040 раздела 2 = ""1""';
			|en = 'строка 120 Раздела 2 х строка 140 Раздела 2 х 2) / 100, если значение строки 040 раздела 2 = ""1""'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2 Страница %1';
													|en = 'Раздел 2 Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = (%2 х %3 х %4 х 2) / 100 Итог [%5]';
													|en = '%1 = (%2 х %3 х %4 х 2) / 100 Итог [%5]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С040, Р2С110, Р2С120, Р2С140, Р2С150", Раздел2);
		Если Не Показатели.Р2С040.Значение = "1" Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		РасчетнаяСумма = Округлить(Показатели.Р2С110.Значение * Показатели.Р2С120.Значение * 
			Показатели.Р2С140.Значение * 2 / 100);
		ЭтоОшибка = Не Показатели.Р2С150.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			Показатели.Р2С150.Значение, Показатели.Р2С110.Значение, Показатели.Р2С120.Значение, Показатели.Р2С140.Значение, 
			РасчетнаяСумма);
...
Процедура ПроверитьКС_1_27(СтруктураРезультата)
	...
	// Формула КС: Р2С150 = Р2С110 * Р2С120 * Р2С140 * 4 / 100 если Р2С040 = "2"
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Сумма исчисленного налога (строка 150 Раздела 2) должна быть равна (строка 110 Раздела 2 х ';
			|en = 'Сумма исчисленного налога (строка 150 Раздела 2) должна быть равна (строка 110 Раздела 2 х '") +
		НСтр("ru = 'строка 120 Раздела 2 х строка 140 Раздела 2 х 4) / 100, если значение строки 040 раздела 2 = ""2""';
			|en = 'строка 120 Раздела 2 х строка 140 Раздела 2 х 4) / 100, если значение строки 040 раздела 2 = ""2""'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Раздел 2 Страница %1';
													|en = 'Раздел 2 Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = '%1 = (%2 х %3 х %4 х 4) / 100 Итог [%5]';
													|en = '%1 = (%2 х %3 х %4 х 4) / 100 Итог [%5]'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С040, Р2С110, Р2С120, Р2С140, Р2С150", Раздел2);
		Если Не Показатели.Р2С040.Значение = "2" Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		РасчетнаяСумма = Округлить(Показатели.Р2С110.Значение * Показатели.Р2С120.Значение * 
			Показатели.Р2С140.Значение * 4 / 100);
		ЭтоОшибка = Не Показатели.Р2С150.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
			Показатели.Р2С150.Значение, Показатели.Р2С110.Значение, Показатели.Р2С120.Значение, Показатели.Р2С140.Значение, 
			РасчетнаяСумма);
