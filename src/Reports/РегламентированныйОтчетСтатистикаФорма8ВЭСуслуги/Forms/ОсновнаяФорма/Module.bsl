...
Процедура ИзменитьПериод(Форма, Шаг)
	...
	Если Форма.мДатаКонцаПериодаОтчета > Дата('20131231235959') Тогда
		// Отчетность за год.
		Форма.мДатаКонцаПериодаОтчета = КонецГода(ДобавитьМесяц(Форма.мДатаКонцаПериодаОтчета, Шаг * 12));
	ИначеЕсли Форма.мДатаКонцаПериодаОтчета = Дата('20131231235959') Тогда
		Если Шаг > 0 Тогда
			// Отчетность за год.
			Форма.мДатаКонцаПериодаОтчета = КонецГода(ДобавитьМесяц(Форма.мДатаКонцаПериодаОтчета, Шаг * 12));
		ИначеЕсли Шаг < 0 Тогда
			// Отчетность за квартал, нарастающим итогом с начала года.
