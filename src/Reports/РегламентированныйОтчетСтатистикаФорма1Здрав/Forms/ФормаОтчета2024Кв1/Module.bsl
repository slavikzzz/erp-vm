...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0609302";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 4 Цикл
		...
		Для Стр = 7 По 19 Цикл
			НомСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
			СуммаПоКол = СуммаПоКол + Области["П00400100" + НомСтр + Графа].Значение;
		КонецЦикла;
		
		Области["П0040010001" + Графа].Значение
			= Области["П0040010002" + Графа].Значение
			+ Области["П0040010004" + Графа].Значение
			+ СуммаПоКол;
	...
	Для Гр = 3 По 4 Цикл
		...
		СуммаПоКол1 = 0;
		Для Стр = 2 По 43 Цикл
			НомСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
			Если (Стр > 2 И Стр < 13)
			 Или (Стр > 16 И Стр < 22)
			 Или (Стр > 24 И Стр < 26)
			 Или (Стр > 29 И Стр < 31)
			 Или (Стр > 32 И Стр < 34) Тогда
				Продолжить;
			КонецЕсли;
			
			СуммаПоКол1 = СуммаПоКол1 + Области["П00500100" + НомСтр + Графа].Значение;
		КонецЦикла;
		Области["П0050010001" + Графа].Значение = СуммаПоКол1;
		
		СуммаПоКол2 = 0;
		Для Стр = 46 По 53 Цикл
			НомСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
			СуммаПоКол2 = СуммаПоКол2 + Области["П00500100" + НомСтр + Графа].Значение;
		КонецЦикла;
		Области["П0050010045" + Графа].Значение = СуммаПоКол2;
	...
	Для Гр = 5 По 11 Цикл
		...
		Области["П0070020005" + Графа].Значение
			= Области["П0070020006" + Графа].Значение
			+ Области["П0070020008" + Графа].Значение;
		
		СуммаПоКол3 = 0;
		Для Стр = 16 По 22 Цикл
			Если Стр = 19 Или Стр = 20 Тогда
				Продолжить;
			КонецЕсли;
			НомСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
			СуммаПоКол3 = СуммаПоКол3 + Области["П00700200" + НомСтр + Графа].Значение;
		КонецЦикла;
		Области["П0070020015" + Графа].Значение = СуммаПоКол3;
		
		Области["П0070020002" + Графа].Значение
			= Области["П0070020003" + Графа].Значение
			+ Области["П0070020004" + Графа].Значение
			+ Области["П0070020005" + Графа].Значение
			+ Области["П0070020010" + Графа].Значение
			+ Области["П0070020011" + Графа].Значение;
			
		Области["П0070020001" + Графа].Значение
			= Области["П0070020002" + Графа].Значение
			+ Области["П0070020015" + Графа].Значение;
	КонецЦикла;
	
	Для Стр = 1 По 25 Цикл
		...
		Для Гр = 5 По 11 Цикл
			Графа     = Формат(Гр, "ЧЦ=2; ЧВН=");
			СуммаПоГр = СуммаПоГр + Области["П00700200" + СтрНом + Графа].Значение;
		КонецЦикла;
		
		Области["П00700200" + СтрНом + "04"].Значение = СуммаПоГр;
	...
	Для Гр = 4 По 6 Цикл
		Графа     = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаПоГр = 0;
		Для Стр = 2 По 21 Цикл
			Если Стр = 16 И Гр = 6 Тогда
				Продолжить;
			КонецЕсли;
			СтрНом    = Формат(Стр, "ЧЦ=2; ЧВН=");
			СуммаПоГр = СуммаПоГр + Области["П00800100" + СтрНом + Графа].Значение;
		КонецЦикла;
		Области["П0080010001" + Графа].Значение = СуммаПоГр;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 3 По 4 Цикл
		...
		мФормаОтчета.Области["П0040010001" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0040010001" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П0050010001" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0050010001" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П0050010045" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0050010045" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
	...
	Для Стр = 1 По 25 Цикл
		...
		СтрНом    = Формат(Стр, "ЧЦ=2; ЧВН=");
		Если Стр = 1 Или Стр = 2 Или Стр = 5 Или Стр = 15 Тогда
			Для Гр = 5 По 11 Цикл
				Графа     = Формат(Гр, "ЧЦ=2; ЧВН=");
				мФормаОтчета.Области["П00700200" + СтрНом + Графа].ЦветФона = ЦветФонаЗначение;
				мФормаОтчета.Области["П00700200" + СтрНом + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
			КонецЦикла;
		КонецЕсли;
		
		мФормаОтчета.Области["П00700200" + СтрНом + "04"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00700200" + СтрНом + "04"].Защита   = Не ОтклАвтоРасчетЗначение;
		
	КонецЦикла;
	
	Для Гр = 4 По 6 Цикл
		Графа     = Формат(Гр, "ЧЦ=2; ЧВН=");
		мФормаОтчета.Области["П0080010001" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0080010001" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-здрав");
	
	Для Раздел = 2 По 11 Цикл
		
		Если Раздел = 6 Или Раздел = 7 Или Раздел = 8 Тогда
			
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть" + Раздел + "_1"));
			ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть" + Раздел + "_2"));
			ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть" + Раздел + "_3"));
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-здрав");
			
			ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
			ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ПустаяСтрока"));
			ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть" + Раздел + "_2"));
			ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть" + Раздел + "_4"));
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-здрав");
...
Функция УниверсальнаяВыгрузкаXMLПроверкаВозможностиВыгрузкиНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтотОбъект,
		СтруктураРеквизитовФормы["ИмяМакетаСхемыВыгрузки"]);
	Возврат ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки);
КонецФункции
