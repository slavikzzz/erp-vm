...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П0000101") Тогда
		...
		Если Прч >= 101 И Прч <= 115 Тогда
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 101 по 115 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П0000101" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		ИначеЕсли Прч >= 116 И Прч <= 119 Тогда 
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 116 по 119 Цикл 
				ПолеТабличногоДокументаФормаОтчета.Области["П0000101" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		ИначеЕсли Прч >= 120 И Прч <= 121 Тогда 
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 120 по 121 Цикл 
				ПолеТабличногоДокументаФормаОтчета.Области["П0000101" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		ИначеЕсли Прч >= 122 И Прч <= 127 Тогда 
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 122 по 127 Цикл 
				ПолеТабличногоДокументаФормаОтчета.Области["П0000101" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Области.П000030130201.Значение = Области.П000030130301.Значение + Области.П000030130401.Значение;
	Области.П000030130202.Значение = Области.П000030130302.Значение + Области.П000030130402.Значение;
	Области.П000030130203.Значение = Области.П000030130303.Значение + Области.П000030130403.Значение;
	
	Области.П000030130501.Значение = Области.П000030130601.Значение + Области.П000030130701.Значение;
	Области.П000030130502.Значение = Области.П000030130602.Значение + Области.П000030130702.Значение;
	Области.П000030130503.Значение = Области.П000030130603.Значение + Области.П000030130703.Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 1 По 3 Цикл
		мФормаОтчета.Области["П00003013020" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00003013020" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П00003013050" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00003013050" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
