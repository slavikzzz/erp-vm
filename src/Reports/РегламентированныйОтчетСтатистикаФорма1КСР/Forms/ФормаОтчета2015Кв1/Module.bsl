...
Процедура ПолеТабличногоДокументаФормаОтчетаПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если Область.Имя = "ПОТКРЫТИЕ" Тогда
		ПолеТабличногоДокументаФормаОтчета.Области.П0136.Значение = ЗначениеЗаполнено(Область.Значение);
	КонецЕсли;
	Если Область.Имя = "ПЗАКРЫТИЕ" Тогда
		ПолеТабличногоДокументаФормаОтчета.Области.П0137.Значение = ЗначениеЗаполнено(Область.Значение);
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Области.П0345.Значение = Области.П0346.Значение + Области.П0348.Значение + Области.П0349.Значение;
	Области.П0772.Значение = Области.П0773.Значение + Области.П0775.Значение + Области.П0776.Значение;
	Области.П0787.Значение = Области.П0788.Значение + Области.П0789.Значение + Области.П0790.Значение + Области.П0791.Значение;
КонецПроцедуры
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ПараметрыВыгрузки.Вставить("Зн36", "");
	Если Не ЗначениеЗаполнено(Откр) Или Не ЗначениеЗаполнено(Закр) Тогда
		Если ТабДокумент["П0134"] = Ложь Тогда 
			ПараметрыВыгрузки.Вставить("Зн36", 4);
		Иначе
			ПараметрыВыгрузки.Вставить("Зн36", "");
		КонецЕсли;
	Иначе
		Если День(Откр) = 1 И Месяц(Откр) = 4 И (День(Закр) = 31 Или День(Закр) = 30) И Месяц(Закр) = 10 Тогда 
			ПараметрыВыгрузки.Вставить("Зн36", 1);
		ИначеЕсли День(Откр) = 1 И Месяц(Откр) = 5 И (День(Закр) = 31 Или День(Закр) = 30) И Месяц(Закр) = 10 Тогда 
			ПараметрыВыгрузки.Вставить("Зн36", 2);
		ИначеЕсли День(Откр) = 1 И Месяц(Откр) = 6 И День(Закр) = 31 И Месяц(Закр) = 8 Тогда 
			ПараметрыВыгрузки.Вставить("Зн36", 3);
		ИначеЕсли ТабДокумент["П0134"] = Ложь Тогда
			ПараметрыВыгрузки.Вставить("Зн36", 4);
		Иначе
			ПараметрыВыгрузки.Вставить("Зн36", "");
		КонецЕсли;
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 02.07.2015 № 295";
	
	Зн38 = Показатели["П0238"];
	Зн39 = Показатели["П0239"];
	ТекстФормулы = "стр.38 >= стр.39";
	Описание = "стр.38 < стр.39";
	Расшифровка = Строка(Зн38)+" >= "+Строка(Зн39);
	Если Зн38 >= Зн39 Тогда
		ЭтоОшибка = Ложь;
	...
	Зн40 = Показатели["П0240"];
	ТекстФормулы = "стр.38 >= стр.40";
	Описание = "стр.38 < стр.40";
	Расшифровка = Строка(Зн38)+" >= "+Строка(Зн40);
	Если Зн38 >= Зн40 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	Иначе
		ЭтоОшибка = Истина;
	...
	Зн38 = Показатели["П0346"];
	Зн39 = Показатели["П0347"];
	ТекстФормулы = "стр.46 >= стр.47";
	Описание = "стр.46 < стр.47";
	Расшифровка = Строка(Зн38)+" >= "+Строка(Зн39);
	Если Зн38 >= Зн39 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	...
	Зн38 = Показатели["П0661"];
	Зн39 = Показатели["П0668"];
	ТекстФормулы = "стр.61 >= стр.68";
	Описание = "стр.61 < стр.68";
	Расшифровка = Строка(Зн38)+" >= "+Строка(Зн39);
	Если Зн38 >= Зн39 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	...
	Зн38 = Показатели["П0773"];
	Зн39 = Показатели["П0774"];
	ТекстФормулы = "стр.73 >= стр.74";
	Описание = "стр.73 < стр.74";
	Расшифровка = Строка(Зн38)+" >= "+Строка(Зн39);
	Если Зн38 >= Зн39 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	...
	Зн46_49 = Новый Массив;
	Зн46_49.Добавить(Показатели["П0346"]);
	Зн46_49.Добавить(Показатели["П0347"]);
	Зн46_49.Добавить(Показатели["П0348"]);
	Зн46_49.Добавить(Показатели["П0349"]);
	
	Сумм53_56 = Новый Массив;
	Для Инд = 0 По 3 Цикл 
		Сумм53_56.Добавить(0);
		Для Гр = 3 По 8 Цикл 
			Сумм53_56[Инд] = Сумм53_56[Инд] + Показатели["П04" + (53+Инд) + "0"+Гр];
		КонецЦикла;
		
		ТекстФормулы = "стр."+(46+Инд)+" = сумма граф 3-8 стр."+(53+Инд);
		Описание = "стр."+(46+Инд)+" <> сумма граф 3-8 стр."+(53+Инд);
		Расшифровка = Строка(Зн46_49[Инд])+" = "+Строка(Сумм53_56[Инд]);
		
		Если Зн46_49[Инд] = Сумм53_56[Инд] Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
			ЭтоОшибка = Истина;
	...
	Сумм57_60 = Новый Массив;
	Для Инд = 0 По 3 Цикл 
		Сумм57_60.Добавить(0);
		Для Гр = 3 По 9 Цикл 
			Сумм57_60[Инд] = Сумм57_60[Инд] + Показатели["П05" + (57+Инд) + "0"+Гр];
		КонецЦикла;
		
		ТекстФормулы = "стр."+(46+Инд)+" => сумма граф 3-8 стр."+(57+Инд);
		Описание = "стр."+(46+Инд)+" < сумма граф 3-8 стр."+(57+Инд);
		Расшифровка = Строка(Зн46_49[Инд])+" >= "+Строка(Сумм57_60[Инд]);
		
		Если Зн46_49[Инд] >= Сумм57_60[Инд] Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
			ЭтоОшибка = Истина;
	...
	Зн38 = Показатели["П0661"];
	Зн39 = Показатели["П0662"]+Показатели["П0663"]+Показатели["П0664"]+Показатели["П0665"]+Показатели["П0666"]+Показатели["П0667"];
	ТекстФормулы = "стр.61 >= сумма стр.62-67";
	Описание = "стр.61 < сумма стр.62-67";
	Расшифровка = Строка(Зн38)+" >= "+Строка(Зн39);
	Если Зн38 >= Зн39 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
	...
	Зн38 = Показатели["П0776"];
	Зн39 = Показатели["П0777"]+Показатели["П0778"]+Показатели["П0779"]+Показатели["П0780"]+Показатели["П0781"];
	ТекстФормулы = "стр.76 >= сумма стр.77-81";
	Описание = "стр.76 < сумма стр.77-81";
	Расшифровка = Строка(Зн38)+" >= "+Строка(Зн39);
	Если Зн38 >= Зн39 Тогда
		ЭтоОшибка = Ложь;
		ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
