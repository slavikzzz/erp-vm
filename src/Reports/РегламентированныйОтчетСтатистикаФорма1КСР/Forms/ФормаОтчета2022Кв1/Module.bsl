...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0000301001";
	...
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы1, 10);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 Тогда
		УдалитьСтроку("");
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П0000101") Тогда
		Прч = Число(Сред(Область.Имя, 9, 3));
		Если Прч >= 101 И Прч <= 114 Тогда
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 101 по 114 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П0000101" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		ИначеЕсли Прч >= 115 И Прч <= 118 Тогда
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 115 по 118 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П0000101" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		ИначеЕсли Прч >= 119 И Прч <= 120 Тогда
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 119 по 120 Цикл 
				ПолеТабличногоДокументаФормаОтчета.Области["П0000101" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		ИначеЕсли Прч >= 121 И Прч <= 126 Тогда
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 121 по 126 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П0000101" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		КонецЕсли;
		...
		Иначе
			...
			ЛевОблИмя = СтрЗаменить(ЛевОблИмя, "П000030100101", "П000030100102");
			ЛевОблИмя = СтрЗаменить(ЛевОблИмя, "П000030100104", "П000030100105");
			Суффикс = Сред(Область.Имя, 14);
			
			Если ЛевОблИмя = "П000030100102" Или ЛевОблИмя = "П000030100105" Тогда
				...
				Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
					Если ЛевОблИмя = "П000030100102" Тогда
						КодПоказателя = ПолеТабличногоДокументаФормаОтчета.Области["П000030100102" + Суффикс].Значение;
					Иначе
						КодПоказателя = ПолеТабличногоДокументаФормаОтчета.Области["П000030100105" + Суффикс].Значение;
					КонецЕсли;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ЛевОбластьИмя = Лев(Область.Имя, 13);
	Суффикс = Сред(Область.Имя, 14);
	
	РезультатКод = РезультатВыбора["Код"];
	РезультатНазвание = ?(РезультатВыбора["Название"] = "Пустое значение" , "", РезультатВыбора["Название"]);
	
	Если ЛевОбластьИмя = "П000030100101" Или ЛевОбластьИмя = "П000030100102" Тогда
		ПолеТабличногоДокументаФормаОтчета.Области["П000030100102" + Суффикс].Значение = РезультатКод;
		ПолеТабличногоДокументаФормаОтчета.Области["П000030100101" + Суффикс].Значение = РезультатНазвание;
	ИначеЕсли ЛевОбластьИмя = "П000030100104" Или ЛевОбластьИмя = "П000030100105" Тогда
		ПолеТабличногоДокументаФормаОтчета.Области["П000030100105" + Суффикс].Значение = РезультатКод;
		ПолеТабличногоДокументаФормаОтчета.Области["П000030100104" + Суффикс].Значение = РезультатНазвание;
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 2 По 4 Цикл
		...
		Области["П0000301301" + Графа].Значение
			= Области["П0000301302" + Графа].Значение
			+ Области["П0000301303" + Графа].Значение;
			
		Области["П0000301304" + Графа].Значение
			= Области["П0000301305" + Графа].Значение
			+ Области["П0000301306" + Графа].Значение;
			
		Области["П0000301307" + Графа].Значение
			= Области["П0000301308" + Графа].Значение
			+ Области["П0000301309" + Графа].Значение;
	КонецЦикла;
		
	Для Стр = 301 По 310 Цикл
		Области["П0000301" + Стр + "01"].Значение
			= Области["П0000301" + Стр + "02"].Значение
			+ Области["П0000301" + Стр + "03"].Значение
			+ Области["П0000301" + Стр + "04"].Значение;
	КонецЦикла;
		
	Области["П000030131101"].Значение
		= Области["П000030131201"].Значение
		+ Области["П000030131301"].Значение
		+ Области["П000030131401"].Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 2 По 4 Цикл
		мФормаОтчета.Области["П00003013010" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00003013010" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П00003013040" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00003013040" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П00003013070" + Гр].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П00003013070" + Гр].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Стр = 301 По 310 Цикл
		мФормаОтчета.Области["П0000301" + Стр + "01"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000301" + Стр + "01"].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
		
	мФормаОтчета.Области["П000030131101"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000030131101"].Защита   = Не ОтклАвтоРасчетЗначение;
КонецПроцедуры
