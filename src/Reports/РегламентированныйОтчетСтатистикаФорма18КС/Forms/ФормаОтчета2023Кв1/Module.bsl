...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0617008";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	//строка 01 = сумме строк 03+ 19 + 28 + 46 + 57 + 60 + 67 по графе 2
	Области["П001001000102"].Значение
		= Области["П001001000302"].Значение
		+ Области["П001001001902"].Значение
		+ Области["П001001002802"].Значение
		+ Области["П001001004602"].Значение
		+ Области["П001001005702"].Значение
		+ Области["П001001006002"].Значение
		+ Области["П001001006702"].Значение;
	
	//строка 04 = сумме строк 05 + 06 + 07 по графам 1, 2
	Для Гр = 1 По 2 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		Области["П0010010004" + Графа].Значение
			= Области["П0010010005" + Графа].Значение
			+ Области["П0010010006" + Графа].Значение
			+ Области["П0010010007" + Графа].Значение;
	КонецЦикла;
	
	//строка 67 = сумме строк 68 + 69 по графе 2
	Области["П001001006702"].Значение
		= Области["П001001006802"].Значение
		+ Области["П001001006902"].Значение;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П001001000102"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001000102"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П001001000401"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001000401"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П001001000402"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001000402"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П001001006702"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001006702"].Защита   = Не ОтклАвтоРасчетЗначение;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 18-КС");
