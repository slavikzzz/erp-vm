	...
	Если СтруктураПараметров.ДополнительныеЛистыЗаТекущийПериод Или Не СтруктураПараметров.ФормироватьДополнительныеЛисты Тогда
		Если СтруктураПараметров.КорректируемыйПериод Тогда
			СписокСообщений.Добавить(НСтр("ru = 'В указанном периоде отчета были внесены изменения в предшествующие налоговые периоды. 
				...
				|en = 'Previous fiscal periods were changed in the specified report period.
				|To generate additional sheets for the adjusted fiscal periods in the current report,
				|select check box ""Generate additional sheets"" and then click ""for the adjusted period""'"));
		КонецЕсли;
		Если СтруктураПараметров.ФормироватьДополнительныеЛисты И Не СтруктураПараметров.ТекущийПериод Тогда
			СписокСообщений.Добавить(НСтр("ru = 'В указанном периоде отчета не вносились изменения в последующих налоговых периодах. 
				...
				|en = 'There are no changes in the specified report period which were made in the subsequent fiscal periods.
				|Generating additional sheets for the current fiscal period is not required.'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ СтруктураПараметров.ДополнительныеЛистыЗаТекущийПериод Или Не СтруктураПараметров.ФормироватьДополнительныеЛисты Тогда
		Если СтруктураПараметров.ТекущийПериод Тогда
			СписокСообщений.Добавить(НСтр("ru = 'В указанном периоде отчета есть изменения, внесенные в последующих налоговых периодах. 
				...
				|en = 'There are changes in the specified report period which were made in the subsequent fiscal periods.
				|To generate additional sheets for the current fiscal period in the current report, 
				|select check box ""Generate additional sheets"" and select value ""for the current period"".'"));
		КонецЕсли;
		Если СтруктураПараметров.ФормироватьДополнительныеЛисты И Не СтруктураПараметров.КорректируемыйПериод Тогда
			СписокСообщений.Добавить(НСтр("ru = 'В указанном периоде отчета не вносились изменения в предыдущие налоговые периоды. 
				...
				|en = 'There are no changes in the specified report period which were made in the previous fiscal periods.
				|Generating additional sheets for the current fiscal period is not required.'"));
		КонецЕсли;
	КонецЕсли;
...
Функция ПроверитьНаличиеДопЛистовКнигиПродаж(СтруктураПараметров) Экспорт
	...
	СтруктураПараметров.Вставить("ПлательщикНДСПрименяющийУСН",
		УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(СтруктураПараметров.Организация, ДатаПроверкиПараметровУчета));
...
Функция ПолучитьИтогиЗаПериодКнигаПродаж(СтруктураПараметров) Экспорт 
	...
	ЭтоПечатьВыписок = Ложь;
	Если СтруктураПараметров.Свойство("ЭтоВыписка") Тогда
		Если СтруктураПараметров.ЭтоВыписка Тогда
			ЭтоПечатьВыписок = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЗаписиКнигиПродаж.МесяцДляВыписок КАК Период,
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		&МесяцДляВыписок КАК МесяцДляВыписок,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗаписиКнигиПродаж.МесяцДляВыписок";
	...
	Если ЭтоПечатьВыписок Тогда
		Запрос.Текст = СтрЗаменить(
		Запрос.Текст, 
		"&МесяцДляВыписок", "НДСЗаписиКнигиПродажОбороты.Период");
	Иначе
		Запрос.Текст = СтрЗаменить(
		Запрос.Текст,
		"СГРУППИРОВАТЬ ПО
		...
		Запрос.Текст = СтрЗаменить(
		Запрос.Текст, 
		"&МесяцДляВыписок", "Истина");
		Запрос.Текст = СтрЗаменить(
		Запрос.Текст, 
		"Месяц", "Период");
		Запрос.Текст = СтрЗаменить(
		Запрос.Текст, 
		"ЗаписиКнигиПродаж.ПериодДляВыписок", "Истина");
	КонецЕсли;
	...
	Если НЕ ИтогЗаПериод.Пустой() Тогда
		...
		Если ЭтоПечатьВыписок Тогда
			Возврат ИтогЗаПериод.Выгрузить();
		Иначе
	...
	Иначе
		...
		ИтогЗаПериод.Вставить("ВсегоПродаж",       0);
		ИтогЗаПериод.Вставить("СуммаБезНДС5",      0);
		ИтогЗаПериод.Вставить("НДС5",              0);
		ИтогЗаПериод.Вставить("СуммаБезНДС7",      0);
		ИтогЗаПериод.Вставить("НДС7",              0);
		ИтогЗаПериод.Вставить("СуммаБезНДС10",     0);
		ИтогЗаПериод.Вставить("НДС10",             0);
		ИтогЗаПериод.Вставить("СуммаБезНДС18",     0);
		ИтогЗаПериод.Вставить("НДС18",             0);
		ИтогЗаПериод.Вставить("СуммаБезНДС20",     0);
		ИтогЗаПериод.Вставить("НДС20",             0);
		ИтогЗаПериод.Вставить("НДС0",              0);
		ИтогЗаПериод.Вставить("СуммаСовсемБезНДС", 0);
		Если ЭтоПечатьВыписок Тогда
			ИтогЗаПериод.Вставить("Период", '00010101');
		КонецЕсли;
...
Процедура ОпределитьПараметрыФормированияОтчета(СтруктураПараметров)
	...
	ПлательщикНДСПрименяющийУСН = УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(СтруктураПараметров.Организация, ДатаПроверкиПараметровУчета);
	...
	СтруктураПараметров.Вставить("ПлательщикНДСПрименяющийУСН", ПлательщикНДСПрименяющийУСН);
	СтруктураПараметров.Вставить("ПрименяетсяОсвобождениеОтУплатыНДС", 
		УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС(СтруктураПараметров.Организация, ДатаПроверкиПараметровУчета));
...
Процедура СформироватьОсновнойРаздел(СтруктураПараметров, Результат);
	...
	ПрименяетсяПостановление1448 = УчетНДСПереопределяемый.ПрименяетсяПостановление1448(СтруктураПараметров.КонецПериода);
	
	Если ПрименяетсяПостановление1448 Тогда 
		Макет = ПолучитьОбщийМакет("КнигаПродаж1448"); // Общий макет для книги и доп. листов
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		ИтогиРаздел9 = Новый Структура(
			"СтПродБезНДС20,СтПродБезНДС18,СтПродБезНДС10,СтПродБезНДС0, СтПродБезНДС7, СтПродБезНДС5,
			|СумНДСВсКПр20,СумНДСВсКПр18,СумНДСВсКПр10,СтПродОсвВсКПр, СумНДСВсКПр7, СумНДСВсКПр5");
		...
		ИтогиРаздел9.СтПродБезНДС7 = СтрокаИтогПоОрганизации.СуммаБезНДС7;
		ИтогиРаздел9.СтПродБезНДС5 = СтрокаИтогПоОрганизации.СуммаБезНДС5;
		ИтогиРаздел9.СумНДСВсКПр7 = СтрокаИтогПоОрганизации.НДС7;
		ИтогиРаздел9.СумНДСВсКПр5 = СтрокаИтогПоОрганизации.НДС5;
	...
	ИначеЕсли СтруктураПараметров.ЗаполнениеДокумента
		...
		Если ПрименяетсяПостановление1448 Тогда
			ОбщиеСведения.Вставить("СтТовРубКоп7",  СтрокаИтогПоОрганизации.СуммаБезНДС7);
			ОбщиеСведения.Вставить("СтТовРубКоп5",  СтрокаИтогПоОрганизации.СуммаБезНДС5);
			ОбщиеСведения.Вставить("СумНДСРубКоп7", СтрокаИтогПоОрганизации.НДС7);
			ОбщиеСведения.Вставить("СумНДСРубКоп5", СтрокаИтогПоОрганизации.НДС5);
		КонецЕсли;
...
Процедура СформироватьДополнительныеЛисты(СтруктураПараметров, Результат)
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		ИтогиРаздел91 = Новый Структура(
			"ИтСтПродКПр20,ИтСтПродКПр18,ИтСтПродКПр10,ИтСтПродКПр7,ИтСтПродКПр5,ИтСтПродКПр0,
			|СумНДСИтКПр20,СумНДСИтКПр18,СумНДСИтКПр10,СумНДСИтКПр7,СумНДСИтКПр5,ИтСтПродОсвКПр,
			|СтПродВсП1Р9_20,СтПродВсП1Р9_18,СтПродВсП1Р9_10,СтПродВсП1Р9_7,СтПродВсП1Р9_5,СтПродВсП1Р9_0,
			|СумНДСВсП1Р9_20,СумНДСВсП1Р9_18,СумНДСВсП1Р9_10,СумНДСВсП1Р9_7,СумНДСВсП1Р9_5,СтПродОсвП1Р9Вс");
		...
		ИтогиРаздел91.ИтСтПродКПр7   = ИтогПоКнигеПродаж.СуммаБезНДС7;
		ИтогиРаздел91.ИтСтПродКПр5   = ИтогПоКнигеПродаж.СуммаБезНДС5;
		...
		ИтогиРаздел91.СумНДСИтКПр7   = ИтогПоКнигеПродаж.НДС7;
		ИтогиРаздел91.СумНДСИтКПр5   = ИтогПоКнигеПродаж.НДС5;
	...
	Пока ИтогПоПериодам.Следующий() Цикл
		...
		Пока ИтогПоПериодамКорректировки.Следующий() Цикл;
			...
			ПрименяетсяПостановление1448 = 
				УчетНДСПереопределяемый.ПрименяетсяПостановление1448(ИтогПоПериодамКорректировки.КонецНалоговогоПериода);
			Если ПрименяетсяПостановление1448 Тогда
				Макет = ПолучитьОбщийМакет("КнигаПродаж1448"); // Общий макет для книги и доп. листов
				СтруктураСекций.Вставить("СекцияСтрокаРНПТ", Макет.ПолучитьОбласть("СтрокаРНПТ"));
			ИначеЕсли СтруктураПараметров.ВерсияПостановленияНДС1137 >= 5 Тогда
			...
			Если СтруктураПараметров.ЗаполнениеДокумента
				...
				Если ПрименяетсяПостановление1448 Тогда
					Итого.Вставить("СтТовРубКоп7",  ИтогЗаПериод.СуммаБезНДС7);
					Итого.Вставить("СтТовРубКоп5",  ИтогЗаПериод.СуммаБезНДС5);
					Итого.Вставить("СумНДСРубКоп7", ИтогЗаПериод.НДС7);
					Итого.Вставить("СумНДСРубКоп5", ИтогЗаПериод.НДС5);
				КонецЕсли;
			...
			Если СтруктураПараметров.ЗаполнениеДокумента
				...
				Если ПрименяетсяПостановление1448 Тогда
					Всего.Вставить("СтТовРубКоп7",  ИтогЗаПериод.СуммаБезНДС7);
					Всего.Вставить("СтТовРубКоп5",  ИтогЗаПериод.СуммаБезНДС5);
					Всего.Вставить("СумНДСРубКоп7", ИтогЗаПериод.НДС7);
					Всего.Вставить("СумНДСРубКоп5", ИтогЗаПериод.НДС5);
				КонецЕсли;
		...
		НазваниеРаздела = СтрШаблон(НСтр("ru = 'Доп.листы за %1';
										|en = 'Additional sheets for %1'"), НалоговыйПериод);
	...
	Если СтруктураПараметров.ЗаполнениеДекларации Тогда
		...
		Если СтруктураПараметров.СчетчикСтрокТекущегоСегмента <> 0 Тогда
			...
			Если ПерваяСтрокаСегмента = 1 Тогда 
				...
				Если СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
					ТабличныйДокумент.Область("СуммаБезНДС7Приложение").Значение     = ИтогЗаПериод.СуммаБезНДС7;
					ТабличныйДокумент.Область("НДС7Приложение").Значение             = ИтогЗаПериод.НДС7;
					ТабличныйДокумент.Область("СуммаБезНДС5Приложение").Значение     = ИтогЗаПериод.СуммаБезНДС5;
					ТабличныйДокумент.Область("НДС5Приложение").Значение             = ИтогЗаПериод.НДС5;
				КонецЕсли;
		...
		ИтогиРаздел91.СтПродВсП1Р9_7  = ИтогЗаПериод.СуммаБезНДС7;
		ИтогиРаздел91.СтПродВсП1Р9_5  = ИтогЗаПериод.СуммаБезНДС5;
		...
		ИтогиРаздел91.СумНДСВсП1Р9_7  = ИтогЗаПериод.НДС7;
		ИтогиРаздел91.СумНДСВсП1Р9_5  = ИтогЗаПериод.НДС5;
...
Процедура ВывестиПодвал(СтруктураПараметров, ТабличныйДокумент, Макет)
	...
	Иначе
		...
		СведенияОЮрФизЛице = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(
			СтруктураПараметров.Организация, СтруктураПараметров.КонецПериода);
		Свидетельство = БухгалтерскийУчетПереопределяемый.СвидетельствоОРегистрации(СведенияОЮрФизЛице);
	КонецЕсли;
...
Процедура ИнициализироватьТаблицыДанныхНДС(СтруктураПараметров)
	...
	Если СтруктураПараметров.ЗаполнениеДокумента
		...
		Записи.Колонки.Добавить("СтТовРубКоп7",       ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
		Записи.Колонки.Добавить("СтТовРубКоп5",       ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
		...
		Записи.Колонки.Добавить("СумНДСРубКоп7",      ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
		Записи.Колонки.Добавить("СумНДСРубКоп5",      ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
		...
		Иначе
			ОбщиеСведения = Новый Структура(
				"СтТовРубКоп20,СтТовРубКоп18,СтТовРубКоп10,СтТовРубКоп0,СумНДСРубКоп20,СумНДСРубКоп18,СумНДСРубКоп10,СтПродОсвНДСРубКоп");
			Если ПрименяетсяПостановление1448 Тогда
				ОбщиеСведения.Вставить("СтТовРубКоп7");
				ОбщиеСведения.Вставить("СтТовРубКоп5");
				ОбщиеСведения.Вставить("СумНДСРубКоп7");
				ОбщиеСведения.Вставить("СумНДСРубКоп5");
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		...
		Если СтруктураПараметров.ЗаполнениеДекларации
		   И СтруктураПараметров.ПравилаЗаполнения = "2025Кв1" Тогда
			ИтогиРаздел9.Вставить("СтПродБезНДС7");
			ИтогиРаздел9.Вставить("СтПродБезНДС5");
			ИтогиРаздел9.Вставить("СумНДСВсКПр7");
			ИтогиРаздел9.Вставить("СумНДСВсКПр5");
			
			ИтогиРаздел91.Вставить("ИтСтПродКПр7");
			ИтогиРаздел91.Вставить("ИтСтПродКПр5");
			ИтогиРаздел91.Вставить("СумНДСИтКПр7");
			ИтогиРаздел91.Вставить("СумНДСИтКПр5");
			ИтогиРаздел91.Вставить("СтПродВсП1Р9_7");
			ИтогиРаздел91.Вставить("СтПродВсП1Р9_5");
			ИтогиРаздел91.Вставить("СумНДСВсП1Р9_7");
			ИтогиРаздел91.Вставить("СумНДСВсП1Р9_5");
		КонецЕсли;
...
Процедура ПреобразоватьЗаписиКнигиПродаж(
	...
	Иначе
		...
		ТекстРозничнаяПродажа = НСтр("ru = 'Розничная продажа';
									|en = 'Retail'", ОбщегоНазначения.КодОсновногоЯзыка());
				...
				Пока ЗаписьКниги.Следующий() Цикл
					...
					Если ЗаписьКниги.НалоговыйАгентПокупательТоваров Тогда
						...
						Пока ДетальнаяЗапись.Следующий() Цикл
							...
							ИтогЗаПериод.СуммаБезНДС7      = ИтогЗаПериод.СуммаБезНДС7 + ДетальнаяЗапись.СуммаБезНДС7;
							ИтогЗаПериод.НДС7              = ИтогЗаПериод.НДС7 + ДетальнаяЗапись.НДС7;
							ИтогЗаПериод.СуммаБезНДС5      = ИтогЗаПериод.СуммаБезНДС5 + ДетальнаяЗапись.СуммаБезНДС5;
							ИтогЗаПериод.НДС5              = ИтогЗаПериод.НДС5 + ДетальнаяЗапись.НДС5;
					...
					Иначе
						...
						Если СтруктураПараметров.НалоговыйПериод >= '20250101' Тогда 
						...
						ИначеЕсли СтруктураПараметров.НалоговыйПериод >= '20141001' Тогда
...
Функция ДополнительноеУсловиеЗапроса(СтруктураПараметров, ЗаписьДополнительногоЛиста = Ложь, ИмяТаблицы = "")
	...
	ИначеЕсли (НЕ СтруктураПараметров.ПлательщикНДС И НЕ СтруктураПараметров.ПлательщикНДСПрименяющийУСН) Тогда
...
Функция ТекстЗапросаПодготовленныеЗаписиКнигиПродаж(СтруктураПараметров)
	...
	|	ВЫБОР
	|		КОГДА ЗаписиКнигиПродаж.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.НалогИсчисляетПокупатель)
	|			ТОГДА ЗаписиКнигиПродаж.СуммаБезНДС7
	|		КОГДА ЗаписиКнигиПродаж.НаАванс
	|				И НЕ ЗаписиКнигиПродаж.НалоговыйАгентПокупательТоваров
	|			ТОГДА 0
	...
	|	ВЫБОР
	|		КОГДА ЗаписиКнигиПродаж.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.НалогИсчисляетПокупатель)
	|			ТОГДА ЗаписиКнигиПродаж.СуммаБезНДС5
	|		КОГДА ЗаписиКнигиПродаж.НаАванс
	|				И НЕ ЗаписиКнигиПродаж.НалоговыйАгентПокупательТоваров
	|			ТОГДА 0
...
Функция ТекстЗапросаКнигаПродаж(СтруктураПараметров)
	...
	|		ИНАЧЕ ПодготовленныеЗаписиКнигиПродаж.Контрагент
	|	КОНЕЦ КАК Контрагент,
	|	ВЫБОР
	|		КОГДА ПодготовленныеЗаписиКнигиПродаж.РозничнаяПродажа
	|			ТОГДА """"
	|		ИНАЧЕ ЕСТЬNULL(СведенияОПокупателях.ИНН, """")
	...
	|	ПодготовленныеЗаписиКнигиПродаж.СуммаБезНДС7 КАК СуммаБезНДС7,
	|	ПодготовленныеЗаписиКнигиПродаж.НДС7 КАК НДС7,
	|	ПодготовленныеЗаписиКнигиПродаж.СуммаБезНДС5 КАК СуммаБезНДС5,
	|	ПодготовленныеЗаписиКнигиПродаж.НДС5 КАК НДС5,
...
Функция НоваяСтрокаИтоговКнигиПродаж()
	...
	Результат.Вставить("СуммаБезНДС7",      0);
	Результат.Вставить("СуммаБезНДС5",      0);
	Результат.Вставить("НДС20",             0);
	Результат.Вставить("НДС18",             0);
	Результат.Вставить("НДС7",              0);
	Результат.Вставить("НДС5",              0);
