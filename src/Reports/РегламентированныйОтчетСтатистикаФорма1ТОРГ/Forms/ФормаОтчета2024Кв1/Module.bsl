...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	//Данные по строке 01 «Оптовая торговля отдельными видами товаров» должны равняться сумме строк, указанных в разделе 1, кроме строк 65, 66, 67, 19, 20, 21
	ТекСумма = 0;
	Для Инд = 2 По 123 Цикл
		Если Инд = 65 Или Инд = 66 Или Инд = 67 Или Инд = 19 Или Инд = 20 Или Инд = 21
	...
	//Раздел 2
	//стр. 83 = сумме строк 84, 87-89, 98-102, 104-105, 109, 111, 113, 115, 117, 119, 121-122, 128-129, 131, 133-138, 145, 147-158, 162-163, 165, 167, 169-172, 176
	ТекСумма = 0;
	СтрокиНабор = "84, 87-89, 98-102, 104-105, 109, 111, 113, 115, 117, 119, 121-122, 128-129, 131, 133-138, 145, 147-158, 162-163, 165, 167, 169-172, 176";
	Для Каждого Элт Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрЗаменить(СтрокиНабор, " ", ""), ",", Истина) Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Области["П0208303"].Значение = ТекСумма;
	...
	//стр.02 = сумме строк 03, 08, 24, 32-37, 40–41, 51, 60, 64, 71–73, 77–78, 81
	ТекСумма = 0;
	СтрокиНабор = "03, 08, 24, 32-37, 40-41, 51, 60, 64, 71-73, 77-78, 81";
	Для Каждого Элт Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрЗаменить(СтрокиНабор, " ", ""), ",", Истина) Цикл
		Если СтрНайти(Элт, "-") > 0 Тогда 
			ВерхНиз = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Элт, "-");
			Для Инд1 = ОТЧ.ПривестиЗначение(ВерхНиз[0]) По ОТЧ.ПривестиЗначение(ВерхНиз[1]) Цикл
	...
	//стр.01 = сумме строк 02, 83
	Области["П0200103"].Значение = Области["П0200203"].Значение + Области["П0208303"].Значение;
