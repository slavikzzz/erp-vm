...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "17/08/2018";
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Мкт = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2018Кв1_ФормаОтчета");
	Для Стр = 37 По 40 Цикл
		ПФ = "_" + Формат((Стр-36), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000101000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 5, Стр, 5).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00101000102"+ПФ, Мкт.Область(Стр, 4, Стр, 4).Текст);
	КонецЦикла;
	Для Стр = 42 По 50 Цикл
		ПФ = "_" + Формат((Стр-41), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000201000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 5, Стр, 5).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00201000102"+ПФ, Мкт.Область(Стр, 4, Стр, 4).Текст);
	КонецЦикла;
	Для Стр = 52 По 74 Цикл
		ПФ = "_" + Формат((Стр-51), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000301000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 5, Стр, 5).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00301000102"+ПФ, Мкт.Область(Стр, 4, Стр, 4).Текст);
	КонецЦикла;
	Для Стр = 80 По 108 Цикл
		ПФ = "_" + Формат((Стр-79), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000401000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 5, Стр, 5).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00401000102"+ПФ, Мкт.Область(Стр, 4, Стр, 4).Текст);
	КонецЦикла;
	Для Стр = 110 По 114 Цикл
		ПФ = "_" + Формат((Стр-109), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000501000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 5, Стр, 5).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00501000102"+ПФ, Мкт.Область(Стр, 4, Стр, 4).Текст);
	КонецЦикла;
	Для Стр = 120 По 134 Цикл
		ПФ = "_" + Формат((Стр-119), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000601000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 5, Стр, 5).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00601000102"+ПФ, Мкт.Область(Стр, 4, Стр, 4).Текст);
	КонецЦикла;
	Для Стр = 136 По 161 Цикл
		ПФ = "_" + Формат((Стр-135), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000701000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 5, Стр, 5).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00701000102"+ПФ, Мкт.Область(Стр, 4, Стр, 4).Текст);
	КонецЦикла;
	
	Инд = 0;
	Для Стр = 185 По 291 Цикл
		Если Мкт.Область(Стр, 5, Стр, 5).СодержитЗначение = Истина Тогда 
			Инд = Инд + 1;
			ПФ = "_" + Формат(Инд, "ЧГ=");
			ПараметрыВыгрузки.Вставить("П000102000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 5, Стр, 5).Имя]);
			ПараметрыВыгрузки.Вставить("ПД00102000102"+ПФ, Мкт.Область(Стр, 4, Стр, 4).Текст);
		КонецЕсли;
	КонецЦикла;
	Инд = 0;
	Для Стр = 293 По 413 Цикл
		Если Мкт.Область(Стр, 5, Стр, 5).СодержитЗначение = Истина Тогда 
			Инд = Инд + 1;
			ПФ = "_" + Формат(Инд, "ЧГ=");
			ПараметрыВыгрузки.Вставить("П000202000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 5, Стр, 5).Имя]);
			ПараметрыВыгрузки.Вставить("ПД00202000102"+ПФ, Мкт.Область(Стр, 4, Стр, 4).Текст);
		КонецЕсли;
	КонецЦикла;
...
Функция ЗагрузитьСпискиВыбораНаСервере()
	...
	ПараметрыЗаполнения.Вставить("Макет", "Списки2018Кв1");
