...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 8) = "П0000031" Тогда
		...
		ВвестиПоказатель(Элемент, , Область, ИмяСправочника, , ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, "Название", СтандартнаяОбработка);
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	//Раздел 1
	// стр. 62 = сумме строк 63-65
	Области["П0106203"].Значение = Области["П0106303"].Значение + Области["П0106403"].Значение + Области["П0106503"].Значение;
	
	//Данные по строке 01 «Оптовая торговля отдельными видами товаров» должны равняться сумме  строк, указанных в разделе 1,
	//кроме строк 63, 64, 65
	ТекСумма = 0;
	...
	//Раздел 2
	//стр. 113 = сумме строк 114-116
	Области["П0211303"].Значение = Области["П0211403"].Значение + Области["П0211503"].Значение + Области["П0211603"].Значение;
	
	//стр. 88 = сумме строк 89, 92-94, 103-107, 112-113, 117, 119, 121, 123, 125, 127, 129-130, 136-137, 139, 141-142, 146-149, 156, 160-172, 176-177, 179, 181, 183-184, 188
	ТекСумма = 0;
	Для Каждого Элт Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрЗаменить("89, 92-94, 103-107, 112-113, 117, 119, 121, 123, 125, 127, 129-130, 136-137, 139, 141-142, 146-149, 156, 160-172, 176-177, 179, 181, 183-184, 188", " ", ""), ",", Истина) Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Области["П0208803"].Значение = ТекСумма;
	...
	//стр.02 = сумме строк 03, 08, 24, 32-37, 40–41, 53, 66, 71, 78–79, 83–84, 87
	ТекСумма = 0;
	Для Каждого Элт Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрЗаменить("03, 08, 24, 32-37, 40-41, 53, 66, 71, 78-79, 83-84, 87", " ", ""), ",", Истина) Цикл
		Если СтрНайти(Элт, "-") > 0 Тогда
			ВерхНиз = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Элт, "-");
			Для Инд1 = ОТЧ.ПривестиЗначение(ВерхНиз[0]) По ОТЧ.ПривестиЗначение(ВерхНиз[1]) Цикл
				ТекСумма = ТекСумма + Области["П02" + Формат(Инд1, "ЧЦ=3; ЧВН=") + "03"].Значение;
	...
	//стр.01 = сумме строк 02, 88
	Области["П0200103"].Значение = Области["П0200203"].Значение + Области["П0208803"].Значение;
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Области["П000004000101_1"].Значение = "01";
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Если ПолеТабличногоДокументаФормаОтчета.Области["П000003100101"].Значение = "Да" Тогда
		ПараметрыВыгрузки.Вставить("ПД00003100101", "70");
	ИначеЕсли ПолеТабличногоДокументаФормаОтчета.Области["П000003100101"].Значение = "Нет" Тогда
		ПараметрыВыгрузки.Вставить("ПД00003100101", "85");
	КонецЕсли;
