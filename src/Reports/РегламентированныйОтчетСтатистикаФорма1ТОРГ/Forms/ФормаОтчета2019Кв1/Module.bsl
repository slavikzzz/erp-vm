...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "16/08/2019";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для НомСтр = 1 По СтруктураГруппы.Количество() Цикл
		ТекСумма = ТекСумма + Области["П000701000103_" + Формат(НомСтр, "ЧГ=")].Значение;
	КонецЦикла;
	Области["П000701000003"].Значение = ТекСумма;
	
	Области["П0106203"].Значение = Области["П0106303"].Значение + Области["П0106403"].Значение + Области["П0106503"].Значение;
	
	ТекСумма = 0;
	Для Инд = 2 По 119 Цикл 
		Если Инд = 63 Или Инд = 64 Или Инд = 65 Или Области.Найти("П01" + Формат(Инд, "ЧЦ=3; ЧВН=") + "03") = Неопределено Тогда 
			Продолжить;
		КонецЕсли;
		ТекСумма = ТекСумма + Области["П01" + Формат(Инд, "ЧЦ=3; ЧВН=") + "03"].Значение;
	КонецЦикла;
	Области["П0100103"].Значение = ТекСумма + Области["П000701000003"].Значение;
	
	ТекСумма = 0;
	Для Инд = 8 по 12 Цикл 
		ТекСумма = ТекСумма + Области["П020" + Формат(Инд, "ЧЦ=2; ЧВН=") + "03"].Значение;
	КонецЦикла;
	Области["П0200703"].Значение = ТекСумма;
	Области["П0210903"].Значение = Области["П0211003"].Значение + Области["П0211103"].Значение + Области["П0211203"].Значение;
	Области["П0212703"].Значение = Области["П0212803"].Значение + Области["П0212903"].Значение + Области["П0213003"].Значение;
	
	ТекСумма = 0;
	Для Каждого Элт Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрЗаменить("85, 88-90, 99-103, 108-109, 113, 115, 117, 119, 121, 123, 125-126, 132-133, 135, 137-138, 142-145, 152, 157-169, 173-174, 176, 178, 180-181, 185", " ", ""), ",", Истина) Цикл 
		...
		КонецЕсли;
	КонецЦикла;
	Области["П0208403"].Значение = ТекСумма;
	
	ТекСумма = 0;
	Для Каждого Элт Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрЗаменить("03, 06, 22, 30-35, 38-39, 50, 63, 68, 75-76, 79-80, 83", " ", ""), ",", Истина) Цикл 
		Если СтрНайти(Элт, "-") > 0 Тогда 
			ВерхНиз = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Элт, "-");
			Для Инд1 = ОТЧ.ПривестиЗначение(ВерхНиз[0]) По ОТЧ.ПривестиЗначение(ВерхНиз[1]) Цикл 
				ТекСумма = ТекСумма + Области["П02" + Формат(Инд1, "ЧЦ=3; ЧВН=") + "03"].Значение;
	...
	Области["П0200103"].Значение = Области["П0200203"].Значение + Области["П0208403"].Значение;
КонецПроцедуры
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Попытка
		АдресВХ = СтруктураРеквизитовФормы.СписокПечатаемыхЛистов[СтруктураРеквизитовФормы.СписокПечатаемыхЛистов.Количество()-1].Значение[0];
		ТД = ПолучитьИзВременногоХранилища(АдресВХ);
		ТД.УдалитьОбласть(ТД.Область(1, , ТД.Области["ОсновнаяЧасть13_2"].Низ,), ТипСмещенияТабличногоДокумента.ПоВертикали);
		Для Каждого Обл Из ТД.Области Цикл 
			Если СтрНачинаетсяС(Обл.Имя, "П000004000105_") Тогда
				ТД.УдалитьОбласть(ТД.Область(1, , Обл.Низ,), ТипСмещенияТабличногоДокумента.ПоВертикали);
				Прервать;
			КонецЕсли;
		КонецЦикла;
		ПоместитьВоВременноеХранилище(ТД, АдресВХ);
	Исключение
	КонецПопытки;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	МакетСоотв = ОбъектОтчета(ИмяФормы).ПолучитьМакет("Списки2019Кв1");
	Мкт = ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаОтчета2019Кв1_ФормаОтчета");
	...
	Для Стр = 37 По 40 Цикл
		ПФ = "_" + Формат((Стр-36), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000101000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 6, Стр, 6).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00101000102"+ПФ, Мкт.Область(Стр, 5, Стр, 5).Текст);
		ПараметрыВыгрузки.Вставить("ПД00101000103"+ПФ, ТЗ[Мкт.Область(Стр, 5, Стр, 5).Текст]);
	КонецЦикла;
	Для Стр = 42 По 50 Цикл
		ПФ = "_" + Формат((Стр-41), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000201000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 6, Стр, 6).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00201000102"+ПФ, Мкт.Область(Стр, 5, Стр, 5).Текст);
		ПараметрыВыгрузки.Вставить("ПД00201000103"+ПФ, ТЗ[Мкт.Область(Стр, 5, Стр, 5).Текст]);
	КонецЦикла;
	Для Стр = 52 По 74 Цикл
		ПФ = "_" + Формат((Стр-51), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000301000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 6, Стр, 6).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00301000102"+ПФ, Мкт.Область(Стр, 5, Стр, 5).Текст);
		ПараметрыВыгрузки.Вставить("ПД00301000103"+ПФ, ТЗ[Мкт.Область(Стр, 5, Стр, 5).Текст]);
	КонецЦикла;
	Для Стр = 80 По 108 Цикл
		ПФ = "_" + Формат((Стр-79), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000401000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 6, Стр, 6).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00401000102"+ПФ, Мкт.Область(Стр, 5, Стр, 5).Текст);
		ПараметрыВыгрузки.Вставить("ПД00401000103"+ПФ, ТЗ[Мкт.Область(Стр, 5, Стр, 5).Текст]);
	КонецЦикла;
	Для Стр = 110 По 114 Цикл
		ПФ = "_" + Формат((Стр-109), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000501000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 6, Стр, 6).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00501000102"+ПФ, Мкт.Область(Стр, 5, Стр, 5).Текст);
		ПараметрыВыгрузки.Вставить("ПД00501000103"+ПФ, ТЗ[Мкт.Область(Стр, 5, Стр, 5).Текст]);
	КонецЦикла;
	Для Стр = 120 По 134 Цикл
		ПФ = "_" + Формат((Стр-119), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000601000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 6, Стр, 6).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00601000102"+ПФ, Мкт.Область(Стр, 5, Стр, 5).Текст);
		ПараметрыВыгрузки.Вставить("ПД00601000103"+ПФ, ТЗ[Мкт.Область(Стр, 5, Стр, 5).Текст]);
	КонецЦикла;
	Для Стр = 136 По 161 Цикл
		ПФ = "_" + Формат((Стр-135), "ЧГ=");
		ПараметрыВыгрузки.Вставить("ПМ00701000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 6, Стр, 6).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00701000102"+ПФ, Мкт.Область(Стр, 5, Стр, 5).Текст);
		ПараметрыВыгрузки.Вставить("ПД00701000103"+ПФ, ТЗ[Мкт.Область(Стр, 5, Стр, 5).Текст]);
	КонецЦикла;
	
	Инд = 0;
	Для Стр = 184 По 291 Цикл
		Если Мкт.Область(Стр, 6, Стр, 6).СодержитЗначение = Истина Тогда 
			Инд = Инд + 1;
			ПФ = "_" + Формат(Инд, "ЧГ=");
			ПараметрыВыгрузки.Вставить("П000102000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 6, Стр, 6).Имя]);
			ПараметрыВыгрузки.Вставить("ПД00102000102"+ПФ, Мкт.Область(Стр, 5, Стр, 5).Текст);
			ПараметрыВыгрузки.Вставить("ПД00102000103"+ПФ, ТЗ[Мкт.Область(Стр, 5, Стр, 5).Текст]);
		КонецЕсли;
	КонецЦикла;
	Инд = 0;
	Для Стр = 292 По 417 Цикл
		Если Мкт.Область(Стр, 6, Стр, 6).СодержитЗначение = Истина Тогда 
			Инд = Инд + 1;
			ПФ = "_" + Формат(Инд, "ЧГ=");
			ПараметрыВыгрузки.Вставить("П000202000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 6, Стр, 6).Имя]);
			ПараметрыВыгрузки.Вставить("ПД00202000102"+ПФ, Мкт.Область(Стр, 5, Стр, 5).Текст);
			ПараметрыВыгрузки.Вставить("ПД00202000103"+ПФ, ТЗ[Мкт.Область(Стр, 5, Стр, 5).Текст]);
		КонецЕсли;
	КонецЦикла;
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок2", РегламентированнаяОтчетность.КонвертированныеЗначенияПоказателейСтатистическойОтчетности(ЭтаФорма, П0007010001, Ложь));
