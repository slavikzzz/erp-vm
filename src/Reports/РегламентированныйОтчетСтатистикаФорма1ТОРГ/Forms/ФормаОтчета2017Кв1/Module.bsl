...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	Мкт = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2017Кв1_ФормаОтчета");
	...
	КонецЦикла;
	Для Стр = 111 По 115 Цикл
		ПФ = "_" + Формат((Стр-110), "ЧГ=");
	...
	КонецЦикла;
	Для Стр = 120 По 138 Цикл
		ПФ = "_" + Формат((Стр-114), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000501000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 5, Стр, 5).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00501000102"+ПФ, Мкт.Область(Стр, 4, Стр, 4).Текст);
	КонецЦикла;
	Для Стр = 140 По 157 Цикл
		ПФ = "_" + Формат((Стр-139), "ЧГ=");
...
Функция ЗагрузитьСпискиВыбораНаСервере()
	...
	ПараметрыЗаполнения.Вставить("Макет", "Списки2017Кв1");
