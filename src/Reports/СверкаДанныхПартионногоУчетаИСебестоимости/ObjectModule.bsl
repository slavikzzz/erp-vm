...
Функция СформироватьТаблицуСверкиРегистров(КомпоновщикНастроек)
	...
	ТекстЗапроса = ""
		...
		+ ТекстЗапросаПартииПроизводственныхЗатрат()
			+ ОбъединениеТекстовЗапроса()
		//++ НЕ УТ
		+ ТекстЗапросаПартииНезавершенногоПроизводства()
			+ ОбъединениеТекстовЗапроса()
		//-- НЕ УТ
...
Функция ТекстЗапросаПартииНезавершенногоПроизводства()
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	""%1"" 										КАК ИсточникДанных,
	|	""ПартииНезавершенногоПроизводства""		КАК РазделИсточникаДанных,
	|	1											КАК Знак,
	|
	|	Партии.Организация 							КАК Организация,
	|	Аналитика.Склад 							КАК Склад,
	...
	|	Партии.Регистратор 							КАК Регистратор,
	|	НАЧАЛОПЕРИОДА(Партии.Период, МЕСЯЦ) 		КАК Период
	|//ПОМЕСТИТЬ ПартииНезавершенногоПроизводства
	|ИЗ
	|	РегистрНакопления.ПартииНезавершенногоПроизводства.ОстаткиИОбороты(
	|			&НачалоПериода,
	|			&КонецПериода,
	|			Регистратор,
	|			,
...
Процедура ПолучитьДоступныеОрганизации(Организация)
	...
	|		Т.Организация
	|	ИЗ
	|		РегистрНакопления.ПартииНезавершенногоПроизводства.Обороты(&НачалоПериода, &КонецПериода, , ) КАК Т
	|	
	//-- НЕ УТ
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
...
Процедура ДобавитьПредупреждениеОбОсобенностяхФормированияОтчета(ДокументРезультат)
		...
		Если НЕ ЗначениеЗаполнено(ДатаПереходаНаПартионныйУчетВерсии22) Тогда
			ТекстПредупреждения = 
				НСтр("ru = 'Отчет не используется при включенном партионном учете версии 2.2.';
					|en = 'The report is not used with enabled batch accounting of version 2.2.'");
		Иначе
			ТекстПредупреждения = 
				...
				|en = 'You can generate the report only by periods where batch accounting of version 2.2 was not used.
				|Date of switch over to batch accounting of version 2.2: %1'");
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(МассивОрганизаций) Тогда
		...
		Если ЗначениеЗаполнено(МассивОрганизацийСФИФОСкользящая) Тогда
			ТекстПредупреждения = 
				...
				|	%2';
				|en = 'For the specified period, there are no turnovers by companies that used the ""FIFO (rolling valuation)"" cost evaluation method on %1:
				|	%2'") + Символы.ПС;
		Иначе
			ТекстПредупреждения = 
				НСтр("ru = 'Нет организаций, у которых на %1 использовался метод оценки стоимости ""ФИФО (скользящая оценка)"":';
					|en = 'No companies which used the ""FIFO (rolling valuation)"" cost estimation method as of %1:'") + Символы.ПС;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(МассивИсключенныхОрганизаций) Тогда
			ТекстПредупреждения = ТекстПредупреждения
				+ НСтр("ru = 'В отчет не включены организации, имеющие метод оценки стоимости, отличный от ""ФИФО (скользящая оценка)"":
					|	%3';
					|en = 'Companies which have a cost evaluation method different from ""FIFO (rolling valuation)"" are not included in the report:  
					|	%3'");
		КонецЕсли;
	...
	ИначеЕсли ЗначениеЗаполнено(МассивИсключенныхОрганизаций) Тогда
		...
		ТекстПредупреждения = 
			...
			|	%3';
			|en = 'Report is generated for turnovers only of those companies who used the ""FIFO (rolling valuation)"" cost evaluation method on %1:
			|	%2
			|Companies with cost evaluation method other than ""FIFO (rolling valuation)"" are not included in the report:
			|	%3'");
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстПредупреждения,
			Формат(НачалоПериода, "ДЛФ=D"),
