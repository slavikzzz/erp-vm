...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "07/02/2014";

	// Код отчета, равен коду по ОКУД
	СтруктураРеквизитовФормы.мКодОтчета   = "0607068";
...
Процедура Расчет(Форма) 
	...
	Для Стр = 2 По 15 Цикл
		СтрСтр = Формат(Стр, "ЧЦ=2; ЧВН=; ЧГ=;");
		Области["П01" + СтрСтр + "08"].Значение = Области["П01" + СтрСтр + "04"].Значение + Области["П01" + СтрСтр + "05"].Значение
												- Области["П01" + СтрСтр + "06"].Значение - Области["П01" + СтрСтр + "07"].Значение;
	КонецЦикла;
	
	Для Гр = 4 По 9 Цикл
		ГрСтр = Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=;");
		Сумм = Области["П0102" + ГрСтр].Значение;
		Для Стр = 3 По 15 Цикл
			СтрСтр = Формат(Стр, "ЧЦ=2; ЧВН=; ЧГ=;");
			Сумм = Сумм + Области["П01" + СтрСтр + ГрСтр].Значение;
		КонецЦикла;
		Области["П0101" + ГрСтр].Значение = Сумм;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого Обл Из ОбластиСтроки Цикл
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-ДМ");
	КонецЦикла;
...
Процедура СообщитьОНевозможностиВыгрузки()
	РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Выгрузка в электронном виде для формы 1-ДМ не предусмотрена.", "", "");
КонецПроцедуры
