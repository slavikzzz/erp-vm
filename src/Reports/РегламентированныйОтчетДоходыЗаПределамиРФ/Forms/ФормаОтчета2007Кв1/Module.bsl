		...
		Если ЭлементДереваСтраницОтчета.ПоказатьСтраницу = 1 Тогда
			...
			Если ЭлементРазделовОтчета.КолонкаРазделыОтчетаСокрНаим = "Раздел1" Тогда
				...
				Для НомерСтраницы = 1 По ДанныеОбИсточникеКоличество Цикл
					...
					ИсточникДохода.КолонкаРазделыОтчета              = "Источник дохода " + НомерСтраницы;
					...
					Для НомерСтраницыСведенияОДоходах = 1 По СведенияОДоходахКоличество Цикл
						...
						СведенияОДходах.КолонкаРазделыОтчета              = "Сведения о доходах " + НомерСтраницыСведенияОДоходах;
	...
	ИначеЕсли ИмяТекРаздела = "Раздел1" Тогда
		...
		Если СтруктураДанныхСледующаяСтраница <> Неопределено Тогда
			Для Каждого Элемент Из СтруктураДанныхСледующаяСтраница Цикл
				...
				Обл = ТабличныйДокумент.Области.Найти(Элемент.Ключ);
				Если Обл <> Неопределено И Обл.СодержитЗначение Тогда 
					Обл.Значение = Элемент.Значение;
				КонецЕсли;
...
Процедура ВывестиРазделВТабДокНаСервере(ТекТабличноеПоле, ИмяТекТабличногоПоля, НовДанныеДопСтрокРаздела) Экспорт
	...
	Для Каждого Обл Из ЭтаФорма[ИмяТекТабличногоПоля].Области Цикл
	КонецЦикла;
	...
	Если ИмяТекущейСтраницыПанели = "Раздел2" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000020011004, ОбластьИмя) > 0 Тогда
			...
			Для Ном = 1 По СтруктураГруппы2.Количество() Цикл
				...
				П000020011004 = П000020011004 + Форма.ТабличныйДокумент.Области["П000020010004_" + Ном].Значение;
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000020011008, ОбластьИмя) > 0 Тогда
			...
			Для Ном = 1 По СтруктураГруппы2.Количество() Цикл
				...
				П000020011008 = П000020011008 + Форма.ТабличныйДокумент.Области["П000020010008_" + Ном].Значение;
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000020011009, ОбластьИмя) > 0 Тогда
			...
			Для Ном = 1 По СтруктураГруппы2.Количество() Цикл
				...
				П000020011009 = П000020011009 + Форма.ТабличныйДокумент.Области["П000020010009_" + Ном].Значение;
	...
	Если ИмяТекущейСтраницыПанели = "Раздел3" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000030006005, ОбластьИмя) > 0 Тогда
			...
			Для Ном = 1 По СтруктураГруппы3_2_1.Количество() Цикл
				...
				П000030006005 = П000030006005 + Форма.ТабличныйДокумент.Области["П000030007005_" + Ном].Значение;
				
			КонецЦикла;
			
			Для Ном = 1 По СтруктураГруппы3_2_2.Количество() Цикл
				
				П000030006005 = П000030006005 + Форма.ТабличныйДокумент.Области["П000030008005_" + Ном].Значение;
				
			КонецЦикла;
			
			Для Ном = 1 По СтруктураГруппы3_2_3.Количество() Цикл
				
				П000030006005 = П000030006005 + Форма.ТабличныйДокумент.Области["П000030009005_" + Ном].Значение;
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000030006007, ОбластьИмя) > 0 Тогда
			...
			Для Ном = 1 По СтруктураГруппы3_2_1.Количество() Цикл
				...
				П000030006007 = П000030006007 + Форма.ТабличныйДокумент.Области["П000030007007_" + Ном].Значение;
				
			КонецЦикла;
			
			Для Ном = 1 По СтруктураГруппы3_2_2.Количество() Цикл
				
				П000030006007 = П000030006007 + Форма.ТабличныйДокумент.Области["П000030008007_" + Ном].Значение;
				
			КонецЦикла;
			
			Для Ном = 1 По СтруктураГруппы3_2_3.Количество() Цикл
				
				П000030006007 = П000030006007 + Форма.ТабличныйДокумент.Области["П000030009007_" + Ном].Значение;
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000030006008, ОбластьИмя) > 0 Тогда
			...
			Для Ном = 1 По СтруктураГруппы3_2_1.Количество() Цикл
				...
				П000030006008 = П000030006008 + Форма.ТабличныйДокумент.Области["П000030007008_" + Ном].Значение;
				
			КонецЦикла;
			
			Для Ном = 1 По СтруктураГруппы3_2_2.Количество() Цикл
				
				П000030006008 = П000030006008 + Форма.ТабличныйДокумент.Области["П000030008008_" + Ном].Значение;
				
			КонецЦикла;
			
			Для Ном = 1 По СтруктураГруппы3_2_3.Количество() Цикл
				
				П000030006008 = П000030006008 + Форма.ТабличныйДокумент.Области["П000030009008_" + Ном].Значение;
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000030010005, ОбластьИмя) > 0 Тогда
			...
			Для Ном = 1 По СтруктураГруппы3_3_1.Количество() Цикл
				...
				П000030010005 = П000030010005 + Форма.ТабличныйДокумент.Области["П000030011005_" + Ном].Значение;
				
			КонецЦикла;
			
			Для Ном = 1 По СтруктураГруппы3_3_2.Количество() Цикл
				
				П000030010005 = П000030010005 + Форма.ТабличныйДокумент.Области["П000030012005_" + Ном].Значение;
				
			КонецЦикла;
			
			Для Ном = 1 По СтруктураГруппы3_3_3.Количество() Цикл
				
				П000030010005 = П000030010005 + Форма.ТабличныйДокумент.Области["П000030013005_" + Ном].Значение;
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000030010007, ОбластьИмя) > 0 Тогда
			...
			Для Ном = 1 По СтруктураГруппы3_3_1.Количество() Цикл
				...
				П000030010007 = П000030010007 + Форма.ТабличныйДокумент.Области["П000030011007_" + Ном].Значение;
				
			КонецЦикла;
			
			Для Ном = 1 По СтруктураГруппы3_3_2.Количество() Цикл
				
				П000030010007 = П000030010007 + Форма.ТабличныйДокумент.Области["П000030012007_" + Ном].Значение;
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000030010008, ОбластьИмя) > 0 Тогда
			...
			Для Ном = 1 По СтруктураГруппы3_3_1.Количество() Цикл
				...
				П000030010008 = П000030010008 + Форма.ТабличныйДокумент.Области["П000030011008_" + Ном].Значение;
				
			КонецЦикла;
			
			Для Ном = 1 По СтруктураГруппы3_3_2.Количество() Цикл
				
				П000030010008 = П000030010008 + Форма.ТабличныйДокумент.Области["П000030012008_" + Ном].Значение;
				
			КонецЦикла;
			
			Для Ном = 1 По СтруктураГруппы3_3_3.Количество() Цикл
				
				П000030010008 = П000030010008 + Форма.ТабличныйДокумент.Области["П000030013008_" + Ном].Значение;
...
Процедура СохранитьНаКлиенте(Автосохранение = Ложь, ВыполняемоеОповещение = Неопределено) Экспорт
	...
	Если СтруктураДанныхТитульный.Количество() = 0 Тогда
...
Процедура УдалитьСтраницуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Для Каждого СтраницаРаздела Из ТекущиеДанные.ПолучитьЭлементы() Цикл
		...
		Если КопияТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1" Тогда
			СтраницаРаздела.КолонкаРазделыОтчета = "Источник дохода " + НомерСтраницы;
		ИначеЕсли КопияТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1_2" Тогда
			СтраницаРаздела.КолонкаРазделыОтчета = "Сведения о доходах " + НомерСтраницы;
		Иначе
...
Процедура УдалитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	Если СтруктураГруппы.Количество() = ИсходноеКоличествоСтрокГруппы Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Нельзя удалять не добавленные строки.';
								|en = 'Нельзя удалять не добавленные строки.'");
...
Процедура ПослеВставкиДополнительнойСтраницыРаздел1_2(Форма, ИмяТекущейСтраницыПанели, ПерейтиНаВставленныйЛист)
	...
	Если ИмяТекущейСтраницыПанели = "Раздел1" Тогда
		...
		ДобавляемаяСтраница.КолонкаРазделыОтчета              = "Источник дохода " + НомерДобавляемойСтраницы;
	...
	ДобавляемаяСтраница.КолонкаРазделыОтчета              = "Сведения о доходах " + НомерДобавляемойСтраницы;
...
Процедура Очистить() Экспорт
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Внимание! Будут очищены все разделы (листы) отчета.%1Продолжить операцию?';
																				|en = 'Внимание! Будут очищены все разделы (листы) отчета.%1Продолжить операцию?'"), Символы.ПС);
	...
	Если НЕ РегламентированнаяОтчетность.ИННСоответствуетТребованиямБезТипаНаСервере(ОргИНН) Тогда
		Если ВыводитьСообщения Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Неверно задан ИНН организации!';
									|en = 'Неверно задан ИНН организации!'");
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1" Тогда
		...
		Если Лев(Ячейка, 9) = "КодСтраны" Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьФормуВыбораЗначенияВыборСтраныРаздел1Завершение", ЭтотОбъект, ДополнительныеПараметры);
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1_2" Тогда
		...
		Если (Ячейка = "П000010010003") Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ВвестиДатуНачальнаяЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		...
		ИначеЕсли (Ячейка = "П000010007003") Или (Ячейка = "П000010008003") Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьФормуВыбораЗначенияВыборКодаРаздел1Завершение", ЭтотОбъект, ДополнительныеПараметры);
			ОткрытьФорму("Обработка.ОбщиеОбъектыРеглОтчетности.Форма.ФормаВыбораЗначенияИзТаблицы", ПараметрыФормы, ЭтаФорма,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел2" Тогда
		...
		Если СтрНайти(Ячейка,"КодВЭД") > 0 Тогда
			...
			ПараметрыФормы.Вставить("СтруктураДляПоиска", Новый Структура("Код", СтруктураРеквизитовФормы.мКодВЭДРаздел2));
...
&НаКлиенте
Процедура ВвестиДатуНачальнаяЗавершение(Дата, ДополнительныеПараметры) Экспорт
	...
	Если (Дата <> Неопределено) Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ВвестиДатуКонечнаяЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ВвестиДатуКонечнаяЗавершение(Дата, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОткрытьФормуВыбораЗначенияВыборСтраныРаздел1Завершение(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОткрытьФормуВыбораЗначенияВыборКодаРаздел1Завершение(Результат, ДополнительныеПараметры) Экспорт
