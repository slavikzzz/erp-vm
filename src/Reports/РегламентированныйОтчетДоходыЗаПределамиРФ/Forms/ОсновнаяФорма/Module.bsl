...
КонецФункции // ПолучитьСрокПредставленияОтчетности()
...
Процедура ПоказатьПериод(Форма)
	...
	Форма.НадписьКтоСдаетОтчет = НСтр("ru = 'Только организации (абз.2 п.1 ст.246 НК РФ).';
										|en = 'Только организации (абз.2 п.1 ст.246 НК РФ).'");
...
Процедура ПолеВыбораПериодичностиПоказаПериодаПриИзменении(Элемент)
	...
	Если (СтрНайти(ВРег(СтрВыбораПериодичностиПоказаПериода),"КВАРТАЛ") > 1) или 
		 (СтрНайти(ВРег(СтрВыбораПериодичностиПоказаПериода),"ГОД") > 1) или 
		 (СтрНайти(ВРег(СтрВыбораПериодичностиПоказаПериода),"МЕСЯЦЕВ") > 1) или 
		 (СтрВыбораПериодичностиПоказаПериода = СтрПериодОтчетаГод) Тогда
...
Процедура ВыбратьФорму(Команда)
	...
	ДопТекстОписания = НСтр("ru = 'Декларацию о доходах, полученных российской организацией за пределами РФ представляют организации (абз.2 п.1 ст.246 НК РФ).';
							|en = 'Декларацию о доходах, полученных российской организацией за пределами РФ представляют организации (абз.2 п.1 ст.246 НК РФ).'");
...
Функция ДоступныеОрганизацииОтсутствуютТекст()
	...
	Возврат НСтр(
	"ru = 'Декларацию о доходах, полученных российской организацией за пределами РФ, представляют только организации (абз.2 п.1 ст.246 НК РФ).
	|В справочнике ""Организации"" сведения об организациях отсутствуют.';
	|en = 'Декларацию о доходах, полученных российской организацией за пределами РФ, представляют только организации (абз.2 п.1 ст.246 НК РФ).
	|В справочнике ""Организации"" сведения об организациях отсутствуют.'");
