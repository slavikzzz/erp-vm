...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "02/11/2016";
...
Процедура ПоказатьПериод()
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета.Значение = "январь - " + НРег(Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'ММММ гггг'")) + " года";
				...
				Для НомерГрафы = 1 По Форма.мСтруктураКолвоКолонокРазделов[ИмяГруппыСинхронизируемой] Цикл
					...
					Если СтруктураГруппыСинхронизируемой[0].Свойство(КолонкаИмяСинхронизируемое) Тогда
						...
						Если ОбластьСдвинутойЯчейкиСинхронизируемой.СодержитЗначение <> Истина Тогда
							Если НЕ ПустаяСтрока(ТекСтрока[КолонкаИмяСинхронизируемое]) И ОбластьСдвинутойЯчейкиСинхронизируемой.Имя <> "П000202102_1" Тогда
								// обновляем порядковый номер строки
			...
			Для НомерГрафы = 1 По Форма.мСтруктураКолвоКолонокРазделов[ИмяГруппыСинхронизируемой] Цикл
				...
				Если СтруктураГруппыСинхронизируемой[0].Свойство(КолонкаТаблицыИмяСинхронизируемое) Тогда
					...
					Если ОбластьЯчейкиТаблицыСинхронизируемой.СодержитЗначение <> Истина Тогда
						...
						// обновляем порядковый номер строки
						НовыйПорядковыйНомерСинхронизируемый = Строка(ПервыйНомерВГруппеСинхронизируемой + НомерСтрокиВТаблицеСинхронизируемой);
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Если ЗначениеЗаполнено(НомСтр) Тогда
		...
		ИначеЕсли СтрНайти("01.02", Сред(ОбластьИмя, 12, 2)) > 0 Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000104",   5, , "Выбор мощности", КоллекцияСписковВыбора["СправочникМощностей"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000105", 254, , "Выбор мощности", КоллекцияСписковВыбора["СправочникМощностей"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000002000104",   5, , "Выбор мощности", КоллекцияСписковВыбора["СправочникМощностей"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000002000105", 254, , "Выбор мощности", КоллекцияСписковВыбора["СправочникМощностей"]);
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если СтрНайти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы1) > 0 Или СтрНайти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы2) > 0 Тогда
		...
		ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы1 + "05" + Суффикс].Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборНазвание, "");
		ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы2 + "04" + Суффикс].Значение = ВыборКод;
		ПолеТабличногоДокументаФормаОтчета.Области[СтруктураРеквизитовФормы.мИдГруппы2 + "05" + Суффикс].Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборНазвание, "");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПараметрыВыгрузки.КодСтройки = "0" Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не задан код стройки организации!",
		...
		Если Стр.Тип = "С" ИЛИ Стр.Тип = "C" Тогда // учтем оба варианта: кириллицу и латиницу
			Если Стр.Многострочность Тогда
				...
				Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, КлючПодчиненногоЭлемента + "_" + Формат(НомСтр, "ЧГ=")) Цикл
					Если ЭтоСтройка(ПараметрыВыгрузки, НомСтр) Тогда
						Прервать;
					КонецЕсли;
...
Функция ЭтоСтройка(ТекПараметры, ТекНомСтр)
	...
	Если ЗначениеЗаполнено(ЗначениеПоказателя1) Или ЗначениеЗаполнено(ЗначениеПоказателя2) Или ЗначениеЗаполнено(ЗначениеПоказателя3)
		Или ЗначениеЗаполнено(ЗначениеПоказателя4) Или ЗначениеЗаполнено(ЗначениеПоказателя5) Тогда
		Возврат Ложь;
	Иначе 
		Возврат Истина;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ИмяАтрибутов = ?(СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц, "АтрибВыгрузкиXML2017Кв1_месяц", "АтрибВыгрузкиXML2017Кв1_год");
