...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолеТабличногоДокументаФормаОтчета.Вывести(ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаОтчета2015Кв1_ФормаОтчета"));
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "П0000500001";
...
Процедура ДоступностьРазделов()
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 3
		...
		Для НомСтр = 1 По 5 Цикл
			ПолеТабличногоДокументаФормаОтчета.Области["П000200000" + НомСтр + "02"].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области["П000200000" + НомСтр + "02"].Защита = Истина;
		КонецЦикла;	
	КонецЕсли;	
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 13) = "П000050000101" Тогда		
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникОКВЭД");
	...
	ИначеЕсли ТекГруппа = "П00004" Тогда		
		УстановитьЗначокВГруппеЯчеек(Область, 1, 2, 1, 1);
		Модифицированность = Истина;
	...
	ИначеЕсли ТекГруппа = "П00007" Тогда		
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		НомерКолонки = Число(Прав(Область.Имя, 1));
		УстановитьЗначокВГруппеЯчеек(Область, 1, 3, НомерКолонки, НомерКолонки);
	...
	ИначеЕсли ТекГруппа = "П00012" Тогда		
		...
		Модифицированность = Истина;
		НомерКолонки = Число(Прав(Область.Имя, 1));
		УстановитьЗначокВГруппеЯчеек(Область, 1, 5, НомерКолонки, НомерКолонки);
	ИначеЕсли ТекГруппа = "П00014" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		УстановитьЗначокВГруппеЯчеек(Область, 1, 14, 1, 1, Истина);
	...
	ИначеЕсли ТекГруппа = "П00016" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		УстановитьЗначокВГруппеЯчеек(Область, 1, 1, 1, 5);
	ИначеЕсли ТекГруппа = "П00017" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		УстановитьЗначокВГруппеЯчеек(Область, 1, 1, 1, 7);
	ИначеЕсли ТекГруппа = "П00018" Или ТекГруппа = "П00019" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		УстановитьЗначокВГруппеЯчеек(Область, 1, 1, 1, 8);
	ИначеЕсли ТекГруппа = "П00020" Тогда
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Результат = ?(РезультатВыбора["Название"] = "Пустое значение", "", РезультатВыбора["Код"]);

	Если Область.Имя = "П000000000101" ИЛИ Область.Имя = "П000000000201" Тогда
...
Процедура УстановитьЗначокВГруппеЯчеек(Область, НачальнаяСтрокаВГруппе, КонечнаяСтрокаВГруппе, НачальнаяКолонкаВГруппе, КонечнаяКолонкаВГруппе, МножественныйВыбор = Ложь)
	...
	Если Лев(Область.Имя, 6) = "П00014" И НовЗначениеОбласти <> "" Тогда
		КолВТаб = 0;
		МассОбластей = Новый Массив;
		Для Н=1 По 14 Цикл
			ОтмеченнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области["П00014000" + Формат(Н, "ЧЦ=2; ЧВН=") + "01"];
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабПоле.Очистить();
	ВыбТабПоле.Вывести(ОбъектОтчета(ИмяФормы).ПолучитьМакет("ФормаОтчета2015Кв1_ФормаОтчета"));
	
	ПолеТабличногоДокументаФормаОтчета = ВыбТабПоле;
	
	П0000500001.Очистить();
	РегламентированнаяОтчетность.НачальныеОперацииСМногострочнымиБлоками(ЭтотОбъект);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-Конъюнктура (опт)");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1-Конъюнктура (опт)");
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ИмяФормы).ПолучитьМакет("Списки2015кв1");
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(
		СвойстваПоказателей, "СправочникОКВЭД", 512, , "Выбор кода ОКВЭД", КоллекцияСписковВыбора["ОКВЭД"]);
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтотОбъект, "СхемаВыгрузкиXML2015Кв1");
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревоДляЗагрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтотОбъект, "СхемаВыгрузкиXML2015Кв1");
...
Функция ПроверитьПоказатели() Экспорт
	...
	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтотОбъект, "СхемаВыгрузкиXML2015Кв1");
...
Функция УниверсальнаяВыгрузкаXMLПроверкаДанныхНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	РегламентированнаяОтчетность.ИзменитьВерсиюФорматаСПроверкойСовместимостиШаблонов(
		ЭтотОбъект, "АтрибВыгрузкиXML2015Кв1", "ШаблонРосстата2015Кв1", ПараметрыВыгрузки);
	ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки);
	Если НЕ РегламентированнаяОтчетность.ПроверитьДанные(ЭтотОбъект, ДеревоВыгрузки) Тогда
		КоллекцияСписковВыбора = "";
		Возврат Ложь;
...
&НаСервере
Процедура ПреобразоватьЗначенияПараметровВыгрузки(ТабДокумент) Экспорт 
	
	Для Каждого Эл Из ТабДокумент Цикл
		
		ИмяПарам = Лев(Эл.Ключ, 6);
		
		Если НеобходимыйЭлементДляЗамены(ИмяПарам) Тогда
...
&НаСервере
Функция НеобходимыйЭлементДляЗамены(ИмяПарам)
	
	Возврат ИмяПарам = "П00004"
		Или ИмяПарам = "П00006"
		Или ИмяПарам = "П00007"
		Или ИмяПарам = "П00010"
		Или ИмяПарам = "П00012"
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ДопАтрибуты = РегламентированнаяОтчетность.СформироватьСтруктуруДопАтрибутов(ИмяФормы, "АтрибВыгрузкиXML2015Кв1");
	...
	ПреобразоватьЗначенияПараметровВыгрузки(ТабДокумент);
...
Функция ПроверитьКонтрольныеСоотношенияИзШаблонаВФоне()
	...
	ПараметрыПроверкиКонтрольныхСоотношений.Вставить("ШаблонРосстата", "ШаблонРосстата2015Кв1");
	ПараметрыПроверкиКонтрольныхСоотношений.Вставить("Атрибуты", "АтрибВыгрузкиXML2015Кв1");
	ПараметрыПроверкиКонтрольныхСоотношений.Вставить("СхемаВыгрузки", "СхемаВыгрузкиXML2015Кв1");
