...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0607002";
...
Процедура ДоступностьРазделов()
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 3
		...
		Для НомСтр = 1 По 5 Цикл
			ПолеТабличногоДокументаФормаОтчета.Области["П015001000102_" + НомСтр].ЦветФона = Новый Цвет();
			ПолеТабличногоДокументаФормаОтчета.Области["П015001000102_" + НомСтр].Значение = "";
			ПолеТабличногоДокументаФормаОтчета.Области["П015001000102_" + НомСтр].Защита = Истина;
		КонецЦикла;
	КонецЕсли;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли РегламентированнаяОтчетностьКлиент.ВыбраннаяОбластьРегламентированногоОтчетаНеВерна(ЭтаФорма, Область, СтандартнаяОбработка) Тогда
		Возврат;
	ИначеЕсли Лев(Область.Имя, 13) = "П001001000302" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ВремяРаботыОрганизации");
	ИначеЕсли Лев(Область.Имя, 13) = "П002001000101" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ОКВЭД2");
	ИначеЕсли ТекГруппа = "П003" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		НомерСтроки = Число(Сред(Область.Имя, 10, 2));
		Если СтрНайти("234", Прав(Область.Имя, 1)) > 0 Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 2, 4);
		ИначеЕсли СтрНайти("567", Прав(Область.Имя, 1)) > 0 Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 5, 7);
		КонецЕсли;
	ИначеЕсли ТекГруппа = "П004" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		НомерСтроки = Число(Сред(Область.Имя, 10, 2));
		Если СтрНайти("234", Прав(Область.Имя, 1)) > 0 Тогда
		...
		ИначеЕсли СтрНайти("567", Прав(Область.Имя, 1)) > 0 Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 5, 7);
		ИначеЕсли СтрНайти("89", Прав(Область.Имя, 1)) > 0
			  Или СтрНайти("10", Прав(Область.Имя, 2)) > 0 Тогда
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 8, 10);
		КонецЕсли;
	ИначеЕсли ТекГруппа = "П005" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		НомерСтроки = Число(Сред(Область.Имя, 10, 2));
		Если СтрНайти("234", Прав(Область.Имя, 1)) > 0 Тогда
		...
		ИначеЕсли СтрНайти("89", Прав(Область.Имя, 1)) > 0
			...
			УстановитьЗначокВГруппеЯчеек(Область, НомерСтроки, НомерСтроки, 8, 10);
		КонецЕсли;
	ИначеЕсли ТекГруппа = "П010" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		УстановитьЗначокВГруппеЯчеек(Область, 1, 14, 2, 2, Истина);
	ИначеЕсли ТекГруппа = "П011" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		УстановитьЗначокВГруппеЯчеек(Область, 1, 5, 2, 2);
	ИначеЕсли ТекГруппа = "П012" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		УстановитьЗначокВГруппеЯчеек(Область, 1, 7, 2, 2);
	ИначеЕсли ТекГруппа = "П013" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		УстановитьЗначокВГруппеЯчеек(Область, 1, 8, 2, 2);
	ИначеЕсли ТекГруппа = "П014" Тогда
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
		УстановитьЗначокВГруппеЯчеек(Область, 1, 8, 2, 2);
	ИначеЕсли ТекГруппа = "П015" Тогда
		Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6 Или
			 Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
			СтандартнаяОбработка = Ложь;
			Модифицированность = Истина;
			УстановитьЗначокВГруппеЯчеек(Область, 1, 5, 2, 2, Истина);
		КонецЕсли;
	КонецЕсли;
	
	Если (ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено)
	   И НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		ВвестиПоказатель(Элемент, Область, Область.Значение, Ложь,
...
&НаКлиенте
Процедура УстановитьЗначокВГруппеЯчеек(Область, НачальнаяСтрокаВГруппе, КонечнаяСтрокаВГруппе, НачальнаяКолонкаВГруппе, КонечнаяКолонкаВГруппе, МножественныйВыбор = Ложь)
	
	мФормаОтчета = ПолеТабличногоДокументаФормаОтчета;
	Раздел = Лев(Область.Имя, 8);
	Многострочка = ?(СтрНайти(Область.Имя, "_") > 0, Истина, Ложь);
	
	ТекЗначениеОбласти = Область.Значение;
	НовЗначениеОбласти = ?(Область.Значение = "", "1", "");
	...
	Для Строка = НачальнаяСтрокаВГруппе По КонечнаяСтрокаВГруппе Цикл
		Для Колонка = НачальнаяКолонкаВГруппе По КонечнаяКолонкаВГруппе Цикл
			...
			Если Многострочка Тогда
				ТекЯчейка = Лев(Область.Имя, 14) + Строка;
			Иначе
				ТекЯчейка = Раздел + Прав("00" + Строка, 3) + Прав("00" + Колонка, 2);
			КонецЕсли;
			
			Если Область.Имя = мФормаОтчета.Области[ТекЯчейка].Имя Тогда 
				Область.Значение = НовЗначениеОбласти;
			ИначеЕсли Не МножественныйВыбор Тогда
				мФормаОтчета.Области[ТекЯчейка].Значение = ?(НовЗначениеОбласти = "", "", ТекЗначениеОбласти);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-Конъюнктура (опт)");
	...
	Для Н = 2 По 5 Цикл
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть" + Н));
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-Конъюнктура (опт)");
	КонецЦикла;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ОКВЭД2",
		512, , "Выбор кода ОКВЭД", КоллекцияСписковВыбора["ОКВЭД2"]);
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ВремяРаботыОрганизации",
		512, , "Выбор кода", КоллекцияСписковВыбора["ВремяРаботыОрганизации"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ФакторыОграничивающиеДеятельность",
		512, , "Выбор фактора", КоллекцияСписковВыбора["ФакторыОграничивающиеДеятельность"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ЧислоПерепродаж",
		512, , "Выбор числа перепродаж", КоллекцияСписковВыбора["ЧислоПерепродаж"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ДоляЭлектроннойПродажи",
		512, , "Выбор доли электронной продажи", КоллекцияСписковВыбора["ДоляЭлектроннойПродажи"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СреднийУровеньТорговойОценки",
		512, , "Выбор уровня торговой оценки", КоллекцияСписковВыбора["СреднийУровеньТорговойОценки"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СостояниеКонкурентнойСреды",
		512, , "Выбор уровня торговой оценки", КоллекцияСписковВыбора["СостояниеКонкурентнойСреды"]);
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Для Стр = 1 По 10 Цикл
		НомСтр = Формат(Стр, "ЧЦ=3; ЧВН=");
		
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0030010" + НомСтр + "02"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД030010" + НомСтр + "02", "1");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0030010" + НомСтр + "03"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД030010" + НомСтр + "02", "2");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0030010" + НомСтр + "04"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД030010" + НомСтр + "02", "3");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0030010" + НомСтр + "05"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД030010" + НомСтр + "04", "1");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0030010" + НомСтр + "06"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД030010" + НомСтр + "04", "2");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0030010" + НомСтр + "07"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД030010" + НомСтр + "04", "3");
		КонецЕсли;
	КонецЦикла;
	
	НомСтр = "001";
	Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0040010" + НомСтр + "02"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД040010" + НомСтр + "02", "16");
	ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0040010" + НомСтр + "03"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД040010" + НомСтр + "02", "17");
	ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0040010" + НомСтр + "04"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД040010" + НомСтр + "02", "18");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0040010" + НомСтр + "05"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД040010" + НомСтр + "05", "19");
	ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0040010" + НомСтр + "06"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД040010" + НомСтр + "05", "2");
	ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0040010" + НомСтр + "07"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД040010" + НомСтр + "05", "20");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0040010" + НомСтр + "08"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД040010" + НомСтр + "08", "19");
	ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0040010" + НомСтр + "09"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД040010" + НомСтр + "08", "2");
	ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0040010" + НомСтр + "10"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД040010" + НомСтр + "08", "20");
	КонецЕсли;
	...
	Для Стр = 1 По 2 Цикл
		...
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0050010" + НомСтр + "02"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД050010" + НомСтр + "02", "7");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0050010" + НомСтр + "03"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД050010" + НомСтр + "02", "8");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0050010" + НомСтр + "04"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД050010" + НомСтр + "02", "9");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0050010" + НомСтр + "05"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД050010" + НомСтр + "05", "1");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0050010" + НомСтр + "06"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД050010" + НомСтр + "05", "2");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0050010" + НомСтр + "07"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД050010" + НомСтр + "05", "3");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0050010" + НомСтр + "08"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД050010" + НомСтр + "08", "1");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0050010" + НомСтр + "09"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД050010" + НомСтр + "08", "2");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0050010" + НомСтр + "10"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД050010" + НомСтр + "08", "3");
		КонецЕсли;
		
	КонецЦикла;
	
	Для Стр = 1 По 14 Цикл
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П010001000102_" + Стр].Значение) Тогда
			СтруктураПоиска = Новый Структура("Ключ", Строка(Стр));
			МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
				"ФакторыОграничивающиеДеятельность").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
			Если МассивВыбора.Количество() = 1 Тогда
				ПараметрыВыгрузки.Вставить("П010001000102_" + Стр, МассивВыбора[0].Код);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Для Стр = 1 По 5 Цикл
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П011001000" + Стр + "02"].Значение) Тогда
			СтруктураПоиска = Новый Структура("Ключ", Строка(Стр));
			МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
				"ЧислоПерепродаж").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
			Если МассивВыбора.Количество() = 1 Тогда
				ПараметрыВыгрузки.Вставить("ПД11001000102", МассивВыбора[0].Код);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Для Стр = 1 По 7 Цикл
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П012001000" + Стр + "02"].Значение) Тогда
			СтруктураПоиска = Новый Структура("Ключ", Строка(Стр));
			МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
				"ДоляЭлектроннойПродажи").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
			Если МассивВыбора.Количество() = 1 Тогда
				ПараметрыВыгрузки.Вставить("ПД12001000102", МассивВыбора[0].Код);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Для Стр = 1 По 8 Цикл
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П013001000" + Стр + "02"].Значение) Тогда
			СтруктураПоиска = Новый Структура("Ключ", Строка(Стр));
			МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
				"СреднийУровеньТорговойОценки").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
			Если МассивВыбора.Количество() = 1 Тогда
				ПараметрыВыгрузки.Вставить("ПД13001000102", МассивВыбора[0].Код);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Для Стр = 1 По 8 Цикл
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П014001000" + Стр + "02"].Значение) Тогда
			СтруктураПоиска = Новый Структура("Ключ", Строка(Стр));
			МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
				"СреднийУровеньТорговойОценки").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
			Если МассивВыбора.Количество() = 1 Тогда
				ПараметрыВыгрузки.Вставить("ПД14001000102", МассивВыбора[0].Код);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Для Стр = 1 По 5 Цикл
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П015001000102_" + Стр].Значение) Тогда
			СтруктураПоиска = Новый Структура("Ключ", Строка(Стр));
			МассивВыбора = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект,
				"СостояниеКонкурентнойСреды").ТаблицаЗначений.НайтиСтроки(СтруктураПоиска);
			Если МассивВыбора.Количество() = 1 Тогда
				ПараметрыВыгрузки.Вставить("П015001000102_" + Стр, МассивВыбора[0].Код);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
