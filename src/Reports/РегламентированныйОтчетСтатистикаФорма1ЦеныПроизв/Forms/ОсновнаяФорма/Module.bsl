...
Процедура ПоказатьПериод(Форма)
	...
	Если КоличествоФорм >= 1 Тогда
		...
		Если (Форма.мПериодичность = ПредопределенноеЗначение("Перечисление.Периодичность.Год"))
			Или (Форма.мПериодичность = ПредопределенноеЗначение("Перечисление.Периодичность.Месяц") И Месяц(Форма.мДатаКонцаПериодаОтчета) = 12) Тогда 
	...
	Если (Форма.мПериодичность = ПредопределенноеЗначение("Перечисление.Периодичность.Год"))
		...
		Если Форма.мПериодичность = ПредопределенноеЗначение("Перечисление.Периодичность.Год") Тогда 
			Для Каждого Стр Из Форма.РезультирующаяТаблица Цикл 
				Если КонецДня(Стр.ДатаКонецДействия) = КонецДня(Форма.мДатаКонцаПериодаОтчета) Тогда 
					Форма.РезультирующаяТаблица.Удалить(Стр);
					Прервать;
				КонецЕсли;
			КонецЦикла;
		Иначе
...
Процедура ИзменитьПериод(Форма, Шаг)
	...
	Если Шаг <> 0 Тогда
		Если Форма.мДатаНачалаПериодаОтчета >= Дата('20201201') Тогда
		...
		Иначе
			Если Форма.мДатаКонцаПериодаОтчета = КонецГода(Форма.мДатаКонцаПериодаОтчета) И Форма.мДатаНачалаПериодаОтчета = НачалоМесяца(Форма.мДатаКонцаПериодаОтчета) И Шаг = 1 Тогда
				Форма.мДатаНачалаПериодаОтчета = НачалоГода(Форма.мДатаКонцаПериодаОтчета);
			ИначеЕсли Форма.мДатаКонцаПериодаОтчета = КонецМесяца(Форма.мДатаНачалаПериодаОтчета) И Форма.мДатаНачалаПериодаОтчета = НачалоГода(Форма.мДатаНачалаПериодаОтчета) И Шаг = -1 Тогда
				Форма.мДатаКонцаПериодаОтчета  = КонецМесяца(ДобавитьМесяц(Форма.мДатаКонцаПериодаОтчета, Шаг));
				Форма.мДатаНачалаПериодаОтчета = НачалоГода(Форма.мДатаКонцаПериодаОтчета);
			ИначеЕсли Форма.мДатаКонцаПериодаОтчета = КонецГода(Форма.мДатаКонцаПериодаОтчета) И Форма.мДатаНачалаПериодаОтчета = НачалоГода(Форма.мДатаНачалаПериодаОтчета) И Шаг = -1 Тогда
				Форма.мДатаНачалаПериодаОтчета = НачалоМесяца(Форма.мДатаКонцаПериодаОтчета);
			Иначе
				Форма.мДатаКонцаПериодаОтчета  = КонецМесяца(ДобавитьМесяц(Форма.мДатаКонцаПериодаОтчета, Шаг));
				Форма.мДатаНачалаПериодаОтчета = НачалоМесяца(Форма.мДатаКонцаПериодаОтчета);
	...
	Форма.мПериодичность = ПериодичностьФормы(Форма);
...
&НаКлиентеНаСервереБезКонтекста
Функция ПериодичностьФормы(Форма)
	
	Если КонецГода(Форма.мДатаНачалаПериодаОтчета) = Форма.мДатаКонцаПериодаОтчета
	   И НачалоГода(Форма.мДатаКонцаПериодаОтчета) = Форма.мДатаНачалаПериодаОтчета Тогда
		Возврат Форма.ПеречислениеПериодичностьГод;
	Иначе
		Возврат Форма.ПеречислениеПериодичностьМесяц;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мПериодичность = ПериодичностьФормы(ЭтаФорма);
