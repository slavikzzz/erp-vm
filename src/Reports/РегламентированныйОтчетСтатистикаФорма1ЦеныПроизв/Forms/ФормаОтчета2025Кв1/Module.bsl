...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы1, 29);
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы2, 11);
	
	мСтруктураКолвоКолонокРазделов = Новый Структура;
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы1, 10);
	мСтруктураКолвоКолонокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы2, 20);
...
Процедура ПеренумерацияСтрокТаблицы(ПолеТабличногоДокументаФормаОтчета)
	...
	ПолеТабличногоДокументаФормаОтчета.Области["П001001000110_1"].Значение = "1";
	ПолеТабличногоДокументаФормаОтчета.Области["П002001000120_1"].Значение = "2";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") > 0 Тогда
		УдалитьСтроку("");
	ИначеЕсли
		Сред(Область.Имя, 12, 2) = "01" Или  Сред(Область.Имя, 12, 2) = "02" Тогда
		ИмяСправочника = "1000002818";
		...
		МассивОчистки.Добавить(ПолеТабличногоДокументаФормаОтчета.Области[Сред(Область.Имя, 1, 11) + "05_" + НомСтр]);
	...
	ИначеЕсли Сред(Область.Имя, 12, 2) = "03" Тогда
		ИмяСправочника = "s_okpd2_ceni_reg_prom_2025";
		
		НомСтр = Сред(Область.Имя, СтрНайти(Область.Имя, "_") + 1);
		
		КодТовараОКПД2 = СокрЛП(Строка(ПолеТабличногоДокументаФормаОтчета.Области[Сред(Область.Имя, 1, 11) + "02_" + НомСтр].Значение));
		...
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, ИмяСправочника);
		ВвестиПоказатель(Элемент, Область, , ИмяСправочника, КодТовараОКПД2, ПараметрВводаПоля.ТекстПриВыборе, , ,СтандартнаяОбработка, МассивОчистки);
	ИначеЕсли Сред(Область.Имя, 12, 2) = "04" Тогда
		ИмяСправочника = "s_ceni_reg_prom_okei_2025";
		НомСтр = Сред(Область.Имя, СтрНайти(Область.Имя, "_") + 1);
		КодТовараОКПД2 = СокрЛП(Строка(ПолеТабличногоДокументаФормаОтчета.Области[Сред(Область.Имя, 1, 11) + "03_" + НомСтр].Значение));
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, ИмяСправочника);
		ВвестиПоказатель(Элемент, Область, , ИмяСправочника, КодТовараОКПД2, ПараметрВводаПоля.ТекстПриВыборе, , ,СтандартнаяОбработка);
	ИначеЕсли Сред(Область.Имя, 12, 2) = "05" Тогда
		ИмяСправочника = "1000015830";
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, ИмяСправочника);
		ВвестиПоказатель(Элемент, Область, , ИмяСправочника, , ПараметрВводаПоля.ТекстПриВыборе, , ,СтандартнаяОбработка);
	ИначеЕсли СтрНайти(Область.Имя, "П001001000108_") = 1 Тогда
		ИмяСправочника = "1000013910";
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, ИмяСправочника);
		ВвестиПоказатель(Элемент, Область, , ИмяСправочника, , ПараметрВводаПоля.ТекстПриВыборе, , ,СтандартнаяОбработка);
	ИначеЕсли СтрНайти(Область.Имя, "П001001000109_") = 1 Тогда
		ИмяСправочника = "1000013909";
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, ИмяСправочника);
		ВвестиПоказатель(Элемент, Область, , ИмяСправочника, , ПараметрВводаПоля.ТекстПриВыборе, , ,СтандартнаяОбработка);
	КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	Для Каждого СтрОбл Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("ОсновнаяЧасть3_1,ОсновнаяЧасть3_2", ",") Цикл 
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "1000013910",
		10, , "Выбор причины изменения цены");
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "1000015830",
		250, , "Выбор канала реализации");
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "1000013909",
		250, , "Выбор причины изменения объема реализации");
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_okpd2_ceni_reg_prom_2025",
		512, , "Выбор локального кода вида товара(услуги)");
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "1000002818",
		512, , "Выбор локального кода товара по ОКПД2");
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "s_ceni_reg_prom_okei_2025",
		512, , "Выбор единицы измерения");
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Сред(ОбластьКод, 12, 2) = "05"
	 Или (Сред(ОбластьКод, 4, 1) = "1" И (Сред(ОбластьКод, 12, 2) = "08" Или Сред(ОбластьКод, 12, 2) = "09")) Тогда
		ВыборКод = Строка(РезультатВыбора["Код"]);
		Если ВыборКод = "0" Тогда
			ОбластьКод.Формат = "ЧН=0";
		Иначе
			ОбластьКод.Формат = "";
		КонецЕсли;
		ОбластьКод.Значение = ВыборКод;
	КонецЕсли;
	
	НаименованиеОбщ = СформироватьТекстПоГрафеА(ОбластьКод.Имя);
	ПоказательИмяГрафаА = Лев(ОбластьКод.Имя, 12) + "1" + Сред(ОбластьКод.Имя, 14);
	ПолеТабличногоДокументаФормаОтчета.Области[ПоказательИмяГрафаА].Значение = НаименованиеОбщ;
...
&НаСервере
Функция СформироватьТекстПоГрафеА(ОбластьКодИмя)
	
	//Определим коды граф
	Графа2 = ПолеТабличногоДокументаФормаОтчета.Области[Лев(ОбластьКодИмя, 12) + "2" + Сред(ОбластьКодИмя, 14)].Значение;
	Графа3 = ПолеТабличногоДокументаФормаОтчета.Области[Лев(ОбластьКодИмя, 12) + "3" + Сред(ОбластьКодИмя, 14)].Значение;
	Графа5 = ПолеТабличногоДокументаФормаОтчета.Области[Лев(ОбластьКодИмя, 12) + "5" + Сред(ОбластьКодИмя, 14)].Значение;
	
	НаборСправочников = РегистрыСведений[ЭтотОбъект.СтруктураИнформацияСправочников.РегистрИмя].СоздатьНаборЗаписей();
	НаборСправочников.Отбор.ИсточникОтчета.Установить(ЭтотОбъект.СтруктураИнформацияСправочников.ИсточникОтчета);
	
	Наименование = "";
	
	Если ЗначениеЗаполнено(Графа2) Тогда
		НаборСправочников.Отбор.Справочник.Установить("1000002818");
		НаборСправочников.Отбор.Код.Установить(Графа2);
		НаборСправочников.Отбор.Фильтр.Установить("");
		НаборСправочников.Прочитать();
		
		Если НаборСправочников.Количество() = 1 Тогда
			НайденноеЗначение = НаборСправочников[0].Наименование;
			Наименование = Наименование + ?(НайденноеЗначение = "Пустое значение", "", НайденноеЗначение);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Графа3) Тогда
			НаборСправочников.Отбор.Справочник.Установить("s_okpd2_ceni_reg_prom_2025");
			НаборСправочников.Отбор.Код.Установить(Графа3);
			НаборСправочников.Отбор.Фильтр.Установить(Графа2);
			НаборСправочников.Прочитать();
		...
		Если ЗначениеЗаполнено(Графа5) Тогда
			НаборСправочников.Отбор.Справочник.Установить("1000015830");
			НаборСправочников.Отбор.Код.Установить(Графа5);
			НаборСправочников.Отбор.Фильтр.Установить("");
			НаборСправочников.Прочитать();
