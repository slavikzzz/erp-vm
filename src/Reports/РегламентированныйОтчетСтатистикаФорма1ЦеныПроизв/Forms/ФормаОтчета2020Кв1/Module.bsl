...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ИмяАтрибутов = ?(СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц, "АтрибВыгрузкиXML2020Кв1_месяц", "АтрибВыгрузкиXML2020Кв1_год");
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			...
			ТЗВыбора.Очистить();
			Для каждого СтрокаТаблицы Из ПараметрВводаПоля.ТаблицаЗначений Цикл
				НоваяСтрока = ТЗВыбора.Добавить();
				НоваяСтрока.Код = СтрокаТаблицы.Код;
				НоваяСтрока.Название = СтрокаТаблицы.Название;
				НоваяСтрока.Кратко = СтрокаТаблицы.УслОбозначение;
			КонецЦикла;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000106", 250, , "Выбор единицы измерения",
		КоллекцияСписковВыбора["СправочникЕдиницИзмерения"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000105", 250, , "Выбор канала реализации",
		КоллекцияСписковВыбора["СправочникКаналовРеализации"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000110", 250, , "Выбор причины изменения объема реализации",
		КоллекцияСписковВыбора["СправочникПричинОбъемОтгрузки"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000002000106", 250, , "Выбор единицы измерения",
		КоллекцияСписковВыбора["СправочникЕдиницИзмерения"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000002000105", 250, , "Выбор канала реализации",
		КоллекцияСписковВыбора["СправочникКаналовРеализации"]);
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Сред(Область.Имя, 12, 2) = "09" Или Сред(Область.Имя, 12, 2) = "10" Или Сред(Область.Имя, 12, 2) = "05" Тогда
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Сред(Область.Имя, 12, 2) = "06" Тогда
		...
		ВыборУслОбозначение = Строка(РезультатВыбора["Кратко"]);
		Область.Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборУслОбозначение, "");
...
Функция УниверсальнаяВыгрузкаXMLПроверкаДанныхНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	КонецЕсли;
	ИмяШаблона = ?(СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц, "ШаблонРосстата2020Кв1_месяц", "ШаблонРосстата2020Кв1_год");
	ИмяАтрибутов = ?(СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц, "АтрибВыгрузкиXML2020Кв1_месяц", "АтрибВыгрузкиXML2020Кв1_год");
...
Функция СформироватьСтруктуруПоказателей()
	...
	ДополнительныеПоказатели1ЦеныПроизв(мДанныеОтчета.ПолеТабличногоДокументаФормаОтчета);
	ДополнительныеПоказатели2ЦеныПроизв(мДанныеОтчета.ПолеТабличногоДокументаФормаОтчета);
	
КонецФункции

&НаСервере
Процедура ДополнительныеПоказатели1ЦеныПроизв(ТекСтруктура)
...
&НаСервере
Процедура ДополнительныеПоказатели2ЦеныПроизв(ТекСтруктура)
	
	Перем ТекЗначение, ТекЗначение3, ТекЗначение4;
	
	НомСуффикса = 1;
	Пока ТекСтруктура.Свойство("П000002000106_" + Формат(НомСуффикса, "ЧГ="), ТекЗначение) Цикл
		Попытка
			ТекСтрокаТЗ = СвойстваПоказателей.НайтиСтроки(Новый Структура("Показатель", "П000002000106"))[0].ТаблицаЗначений.НайтиСтроки(Новый Структура("УслОбозначение", ТекЗначение))[0];
		Исключение
			ТекСтрокаТЗ = Неопределено;
		КонецПопытки;
		Если ТекСтрокаТЗ <> Неопределено Тогда
			Если ЗначениеЗаполнено(ТекСтрокаТЗ.Код) Тогда
				ТекСтруктура.Вставить("ПД00002000106_" + Формат(НомСуффикса, "ЧГ="), Число(ТекСтрокаТЗ.Код));
			Иначе
				ТекСтруктура.Вставить("ПД00002000106_" + Формат(НомСуффикса, "ЧГ="), "");
			КонецЕсли;
		Иначе
			ТекСтруктура.Вставить("ПД00002000106_" + Формат(НомСуффикса, "ЧГ="), "");
		КонецЕсли;
	...
	НомСуффикса = 1;
	Пока ТекСтруктура.Свойство("П000002000103_" + Формат(НомСуффикса, "ЧГ="), ТекЗначение3)
		И ТекСтруктура.Свойство("П000002000104_" + Формат(НомСуффикса, "ЧГ="), ТекЗначение4) Цикл
		Если ЗначениеЗаполнено(ТекЗначение3) И ЗначениеЗаполнено(ТекЗначение4) Тогда
			ТекСтруктура.Вставить("ПД00002000134_" + Формат(НомСуффикса, "ЧГ="), СокрЛП(ТекЗначение4) + "/ " + СокрЛП(ТекЗначение3));
		ИначеЕсли ЗначениеЗаполнено(ТекЗначение3) Тогда
			ТекСтруктура.Вставить("ПД00002000134_" + Формат(НомСуффикса, "ЧГ="), СокрЛП(ТекЗначение3));
		ИначеЕсли ЗначениеЗаполнено(ТекЗначение4) Тогда
			ТекСтруктура.Вставить("ПД00002000134_" + Формат(НомСуффикса, "ЧГ="), СокрЛП(ТекЗначение4));
		Иначе
			ТекСтруктура.Вставить("ПД00002000134_" + Формат(НомСуффикса, "ЧГ="), "");
		КонецЕсли;
		НомСуффикса = НомСуффикса + 1;
	КонецЦикла;
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ИмяАтрибутов = ?(НачалоМесяца(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = НачалоМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "АтрибВыгрузкиXML2020Кв1_месяц", "АтрибВыгрузкиXML2020Кв1_год");
