...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	мСтруктураИсхКолвоСтрокРазделов.Вставить(СтруктураРеквизитовФормы.мИдГруппы2, 10);
	...
	ПолеТабличногоДокументаФормаОтчета.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2021Кв1_ФормаОтчета"));
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ИмяАтрибутов = ?(СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц, "АтрибВыгрузкиXML2021Кв1_месяц", "АтрибВыгрузкиXML2021Кв1_год");
...
Процедура ЗаполнитьСведенияОбОрганизацииНаСервере(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	РегламентированнаяОтчетностьКлиентСервер.ЗаполнитьСведенияОбОрганизацииДляОтчетаСтатистики(
		ЭтаФорма, ВПрограммеИзмененаДатаПодписи, , Истина);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			...
			Если Сред(Область.Имя, 12, 2) = "03" Или Сред(Область.Имя, 12, 2) = "05"
			 Или Сред(Область.Имя, 12, 2) = "09" Или Сред(Область.Имя, 12, 2) = "10" Тогда
			...
			Иначе
				МассивВыбора = ПараметрВводаПоля.ТаблицаЗначений.НайтиСтроки(Новый Структура("Ключ",
					ПолеТабличногоДокументаФормаОтчета.Области["П00000" + Сред(Область.Имя, 7, 1) + "000103_" + Сред(Область.Имя, 15)].Значение));
				ТЗВыбора.Очистить();
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабличноеПоле.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2021Кв1_ФормаОтчета"));
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("Списки2021Кв1");
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000109",
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000110",
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000103",
		512, , "Выбор локального кода вида товара(услуги)", КоллекцияСписковВыбора["СправочникЛокальныйКодВидаТовара"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000002000103",
		512, , "Выбор локального кода вида товара(услуги)", КоллекцияСписковВыбора["СправочникЛокальныйКодВидаТовара"]);
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000001000106",
		512, , "Выбор единицы измерения", КоллекцияСписковВыбора["СправочникЕдиницаИзмеренияПоОКЕИ"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000002000106",
		512, , "Выбор единицы измерения", КоллекцияСписковВыбора["СправочникЕдиницаИзмеренияПоОКЕИ"]);
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Сред(Область.Имя, 12, 2) = "03" Или Сред(Область.Имя, 12, 2) = "04" Тогда
		...
		Если Сред(Область.Имя, 12, 2) = "03" Тогда
			СмежнаяОбласть = СтрЗаменить(Область.Имя, Лев(Область.Имя, 13), Лев(Область.Имя, 11) + "04");
			ПолеТабличногоДокументаФормаОтчета.Области[СмежнаяОбласть].Значение = "";
		КонецЕсли;
	ИначеЕсли Сред(Область.Имя, 12, 2) = "06" Тогда
		ВыборКод = Строка(РезультатВыбора["Код"]);
		Область.Значение = ?(ЗначениеЗаполнено(ВыборКод), ВыборКод, "");
	ИначеЕсли  Сред(Область.Имя, 12, 2) = "05" Или Сред(Область.Имя, 12, 2) = "09" Или Сред(Область.Имя, 12, 2) = "10" Тогда
		ВыборКод = Строка(РезультатВыбора["Код"]);
	...
	Если Сред(Область.Имя, 12, 2) = "03" Или Сред(Область.Имя, 12, 2) = "04" Или Сред(Область.Имя, 12, 2) = "05" Тогда
			Наименование1 = "";
			Наименование2 = "";
			Наименование3 = "";
			Для Каждого Элемент Из СвойстваПоказателей Цикл
				Если Элемент.Показатель = "П000001000103" Тогда
					...
					Если ЗначениеЗаполнено(ОбластьКол3.Значение) Тогда
						...
						КонецЕсли;
					КонецЕсли;
				ИначеЕсли Элемент.Показатель = "П000001000104" Тогда
					ОбластьКол4 = ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 12) + "4" + Сред(Область.Имя, 14)];
					Если ЗначениеЗаполнено(ОбластьКол4.Значение) Тогда
						ОбластьКол3 = ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 12) + "3" + Сред(Область.Имя, 14)];
						МассивВыбора = Элемент.ТаблицаЗначений.НайтиСтроки(Новый Структура("Ключ", ОбластьКол3.Значение));
						Если МассивВыбора.Количество() = 1 Тогда
							Наименование2 = МассивВыбора[0].Название;
						КонецЕсли;
					КонецЕсли;
				ИначеЕсли Элемент.Показатель = "П000001000105" Тогда
			...
			Если НаименованиеОбщ <> "" Тогда
				ПолеТабличногоДокументаФормаОтчета.Области[Лев(Область.Имя, 12) + "2" + Сред(Область.Имя, 14)].Значение = НаименованиеОбщ;
			КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура НумерацияСтрок1ЦеныПриобр()
	НомСтр = П0000010001.Количество();
	ВнутреннийЦиклПрерван = Ложь;
	Пока НомСтр > 0 Цикл
		Для НомКол = 2 По 10 Цикл
			Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0000010001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_" + Формат(НомСтр, "ЧГ=")].Значение) Тогда
	...
	Для НомСуффикса = 1 По НомСтр Цикл
		ПолеТабличногоДокументаФормаОтчета.Области["П000001000101_" + Формат(НомСуффикса, "ЧГ=")].Значение = НомСуффикса;
	КонецЦикла;
	Для НомСуффикса = НомСтр + 1 По П0000010001.Количество() Цикл
		ПолеТабличногоДокументаФормаОтчета.Области["П000001000101_" + Формат(НомСуффикса, "ЧГ=")].Значение = 0;
		ПолеТабличногоДокументаФормаОтчета.Области["П000001000101_" + Формат(НомСуффикса, "ЧГ=")].Формат = "ЧН=' '";
	КонецЦикла;
	
	НомСтр = П0000020001.Количество();
	ВнутреннийЦиклПрерван = Ложь;
	Пока НомСтр > 0 Цикл
		Для НомКол = 2 По 20 Цикл
			Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П0000020001" + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=") + "_" + Формат(НомСтр, "ЧГ=")].Значение) Тогда
				ВнутреннийЦиклПрерван = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	...
	КонецЦикла;
	Для НомСуффикса = 1 По НомСтр Цикл
		ПолеТабличногоДокументаФормаОтчета.Области["П000002000101_" + Формат(НомСуффикса, "ЧГ=")].Значение = НомСуффикса;
	КонецЦикла;
	Для НомСуффикса = НомСтр + 1 По П0000020001.Количество() Цикл
		ПолеТабличногоДокументаФормаОтчета.Области["П000002000101_" + Формат(НомСуффикса, "ЧГ=")].Значение = 0;
		ПолеТабличногоДокументаФормаОтчета.Области["П000002000101_" + Формат(НомСуффикса, "ЧГ=")].Формат = "ЧН=' '";
	КонецЦикла;
КонецПроцедуры
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2021Кв1");
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревоДляЗагрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2021Кв1");
...
Функция ПроверитьПоказатели() Экспорт
	...
	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2021Кв1");
...
Функция УниверсальнаяВыгрузкаXMLПроверкаДанныхНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	Если СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Год Тогда
		СтрокаДляОчистки = ДеревоВыгрузки.Строки.Найти("П000001000110", "Ключ", Истина);
		СтрокаДляОчистки.Ключ = "";
	КонецЕсли;
	ИмяШаблона = ?(СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц, "ШаблонРосстата2021Кв1_месяц", "ШаблонРосстата2021Кв1_год");
	ИмяАтрибутов = ?(СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц, "АтрибВыгрузкиXML2021Кв1_месяц", "АтрибВыгрузкиXML2021Кв1_год");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ОргЭлектроннаяПочта) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не указан адрес электронной почты!",
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ИмяАтрибутов = ?(НачалоМесяца(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = НачалоМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "АтрибВыгрузкиXML2021Кв1_месяц", "АтрибВыгрузкиXML2021Кв1_год");
