...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "30/08/2013";
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-цены производителей");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1ЦеныПроизводителей");
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Сред(Область.Имя, 12, 2) = "09" Или Сред(Область.Имя, 12, 2) = "05" Тогда
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли  Сред(Область.Имя, 12, 2) = "05" Или Сред(Область.Имя, 12, 2) = "09" Тогда
		ВыборКод = Строка(РезультатВыбора["Код"]);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// КОНТРОЛЬ показателей формы (Указания ФСН по заполнению Формы 1-цены производителей).
	
	// Если значение в графе 3 «Причина изменения цены» не равно нулю, то значение в графе 1 «Цена…» тоже не должно быть равным нулю.
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, "П000001000109_" + Формат(НомСтр, "ЧГ=")) Цикл
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000109_" + Формат(НомСтр, "ЧГ=")]) И Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000107_" + Формат(НомСтр, "ЧГ=")]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если значение в графе 3 «Причина изменения цены» не равно нулю, то значение в графе 1 «Цена…» тоже не должно быть равным нулю!",
	...
	// Если в графе 1 «Цена…» по какой-либо позиции значение не равно нулю, то значение
	// в графе 2 «Отгружено товара…» тоже не должно быть равным нулю, и наоборот (для годовой отчетности).
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета = НачалоДня(КонецГода(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета))
		И СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета = НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) Тогда
		НомСтр = 1;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, "П000001000107_" + Формат(НомСтр, "ЧГ=")) Цикл
			Если (ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000107_" + Формат(НомСтр, "ЧГ=")]) И Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000108_" + Формат(НомСтр, "ЧГ=")]))
				Или (ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000108_" + Формат(НомСтр, "ЧГ=")]) И Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000107_" + Формат(НомСтр, "ЧГ=")])) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: если в графе 1 «Цена…» по какой-либо позиции значение не равно нулю, то значение в графе 2 «Отгружено товара…» тоже не должно быть равным нулю, и наоборот!",
					ИмяПоляТаблДокумента, ?(ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000107_" + Формат(НомСтр, "ЧГ=")]), "П000001000108_" + Формат(НомСтр, "ЧГ="), "П000001000107_" + Формат(НомСтр, "ЧГ=")));
				СтатусПроверки = Ложь;
			КонецЕсли;
			НомСтр = НомСтр + 1;
		КонецЦикла;
...
Функция СформироватьСтруктуруПараметров()
	...
	// Для ПОСЛЕДУЮЩЕГО (ПЕРЕД ВЫГРУЗКОЙ) контроля показателей формы (Указания ФСН по заполнению Формы 1-цены производителей).
	
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000001000107_" + Формат(НомСтр, "ЧГ=")) Цикл
...
Функция СформироватьСтруктуруДопАтрибутов()
	...
	ИмяОбласти = ?(СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц, "АтрПериодМесяц", "АтрПериодГод");
