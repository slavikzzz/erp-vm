...
Процедура ДоступностьРазделов()
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <> 12 Тогда
		...
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008703"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008703"].Защита = Истина;
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008703"].Значение = "";
		
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008604"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008604"].Защита = Истина;
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008604"].Значение = "";
		
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008705"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008705"].Защита = Истина;
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008705"].Значение = "";
		
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008606"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008606"].Защита = Истина;
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008606"].Значение = "";
		
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008607"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008607"].Защита = Истина;
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008607"].Значение = "";
		
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008707"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008707"].Защита = Истина;
		ПолеТабличногоДокументаФормаОтчета.Области["П003001008707"].Значение = "";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	СуммПоСтр = 0;
	Для Стр = 4 По 10 Цикл
		СтрНомер = Формат(Стр, "ЧЦ=2; ЧВН=");
		СуммПоСтр = СуммПоСтр + Области["П00100100" + СтрНомер + "03"].Значение;
	КонецЦикла;
	Области["П001001000303"].Значение = СуммПоСтр;
	...
	СуммПоСтр = 0;
	Для Стр = 19 По 25 Цикл
		СуммПоСтр = СуммПоСтр + Области["П00100100" + Стр + "03"].Значение;
	КонецЦикла;
	Области["П001001001803"].Значение = СуммПоСтр;
	
	Области["П001001002603"].Значение
		= Области["П001001002703"].Значение
		+ Области["П001001002803"].Значение
		+ Области["П001001002903"].Значение;
	
	СуммПоСтр = 0;
	Для Стр = 34 По 40 Цикл
		СуммПоСтр = СуммПоСтр + Области["П00100100" + Стр + "03"].Значение;
	КонецЦикла;
	Области["П001001003303"].Значение = СуммПоСтр;
	
	Области["П001001004103"].Значение
		= Области["П001001004203"].Значение
		+ Области["П001001004303"].Значение
		+ Области["П001001004403"].Значение;
		
	СуммПоСтр = 0;
	Для Стр = 50 По 57 Цикл
		СуммПоСтр = СуммПоСтр + Области["П00100100" + Стр + "03"].Значение;
	КонецЦикла;
	Области["П001001004903"].Значение = СуммПоСтр;
	
	Для Гр = 3 по 7 Цикл 
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		
		Области["П0030010064" + Графа].Значение
			= Области["П0030010065" + Графа].Значение
			+ Области["П0030010066" + Графа].Значение;
		
		СуммПоСтр = 0;
		Для Стр = 72 По 84 Цикл 
			Если Стр = 77 Или Стр = 78 Или Стр = 81 Тогда
				Продолжить;
			КонецЕсли;
			СуммПоСтр = СуммПоСтр + Области["П00300100" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0030010071" + Графа].Значение = СуммПоСтр;
		
		Области["П0030010085" + Графа].Значение
			= Области["П0030010064" + Графа].Значение
			+ Области["П0030010070" + Графа].Значение
			+ Области["П0030010071" + Графа].Значение;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	ЯчейкиАвторасчета =
		"П001001000103;П001001000303;П001001001103;П001001001803;П001001002603;П001001003303;П001001004103;П001001004903;";
	...
	Для Н = 1 По 3 Цикл
		Для Гр = 3 По 7 Цикл
			...
			ИначеЕсли Н = 3 Тогда
				Стр = "85";
			КонецЕсли;
			ЯчейкаИмя = "П00300100" + Стр + Графа +";";
			ЯчейкиАвторасчета = ЯчейкиАвторасчета + ЯчейкаИмя;
		КонецЦикла;
	КонецЦикла;
