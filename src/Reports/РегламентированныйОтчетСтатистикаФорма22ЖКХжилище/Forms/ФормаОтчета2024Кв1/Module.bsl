...
Процедура ДоступностьРазделов()
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <> 12 Тогда
		Для Гр = 3 По 7 Цикл
			Для Стр = 56 По 61 Цикл
				Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
				ПоказательИмя = "П00400100" + Стр + Графа;
				
				Обл = ПолеТабличногоДокументаФормаОтчета.Области.Найти(ПоказательИмя);
				Если Обл = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				ПолеТабличногоДокументаФормаОтчета.Области[ПоказательИмя].ЦветФона = Новый Цвет();
				ПолеТабличногоДокументаФормаОтчета.Области[ПоказательИмя].Защита = Истина;
				ПолеТабличногоДокументаФормаОтчета.Области[ПоказательИмя].Значение = "";
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	//Раздел 1
	//стр. 01 по графе 3 = стр. 02 + стр. 08 по графе 3
	Области["П001001000103"].Значение
		= Области["П001001000203"].Значение
		+ Области["П001001000803"].Значение;
	
	//стр. 04 по графе 3 = стр. 05 + стр. 06 + стр. 07 по графе 3
	Области["П001001000403"].Значение
		= Области["П001001000503"].Значение
		+ Области["П001001000603"].Значение
		+ Области["П001001000703"].Значение;
	
	//стр. 12 по графе 3 = стр. 13 + стр. 14 + стр. 15 по графе 3
	Области["П001001001203"].Значение
		= Области["П001001001303"].Значение
		+ Области["П001001001403"].Значение
		+ Области["П001001001503"].Значение;
	
	//стр. 20 по графе 3 = стр. 21 + стр. 22 + стр. 23 по графе 3
	Области["П001001002003"].Значение
		= Области["П001001002103"].Значение
		+ Области["П001001002203"].Значение
		+ Области["П001001002303"].Значение;
	
	//Раздел 3
	//стр.36по графам 3 – 7 = стр. 37 + стр. 38 по соответствующим графам
	Для Гр = 3 по 7 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		
		Области["П0030010036" + Графа].Значение
			= Области["П0030010037" + Графа].Значение
			+ Области["П0030010038" + Графа].Значение;
	КонецЦикла;
	
	//стр. 41 по графам 3 – 7 =  стр. 42–46, 49, 50, 52–54 по соответствующим графам
	Для Гр = 3 по 7 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		
		Области["П0030010041" + Графа].Значение
			= Области["П0030010042" + Графа].Значение
			+ Области["П0030010043" + Графа].Значение
			+ Области["П0030010044" + Графа].Значение
			+ Области["П0030010045" + Графа].Значение
			+ Области["П0030010046" + Графа].Значение
			+ Области["П0030010049" + Графа].Значение
			+ Области["П0030010050" + Графа].Значение
			+ Области["П0030010052" + Графа].Значение
			+ Области["П0030010053" + Графа].Значение
			+ Области["П0030010054" + Графа].Значение;
	КонецЦикла;
	
	//стр. 55 по графам 3 – 7 = стр. 36+стр. 40 + стр. 41
	Для Гр = 3 по 7 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		
		Области["П0030010055" + Графа].Значение
			= Области["П0030010036" + Графа].Значение
			+ Области["П0030010040" + Графа].Значение
			+ Области["П0030010041" + Графа].Значение;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П001001000103"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001000103"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П001001000403"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001000403"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П001001001203"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П001001001203"].Защита   = Не ОтклАвтоРасчетЗначение;
	...
	Для Гр = 3 По 7 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		мФормаОтчета.Области["П0030010036" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0030010036" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П0030010041" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0030010041" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
		
		мФормаОтчета.Области["П0030010055" + Графа].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0030010055" + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
