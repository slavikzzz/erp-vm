...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "14/08/2019";
	...
	Если СтруктураРеквизитовФормы.мСкопированаФорма <> Неопределено Тогда
		Если Не (НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета
			...
			Для НомСтр = 37 По 38 Цикл
				Для НомКол = 3 По 8 Цикл 
					Если НомСтр = 38 И НомКол = 3 Тогда
						Продолжить;
					КонецЕсли;
					ОбластьИмя1 = "П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=");
					Обл = ПолеТабличногоДокументаФормаОтчета.Области.Найти(ОбластьИмя1);
			...
			Для НомСтр = 61 По 66 Цикл
				Для НомКол = 3 По 8 Цикл
					ОбластьИмя2 = "П000004" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(НомКол, "ЧЦ=2; ЧВН=; ЧГ=");
					Обл = ПолеТабличногоДокументаФормаОтчета.Области.Найти(ОбластьИмя2);
					Если Обл <> Неопределено Тогда 
						Обл.Значение = 0;
					КонецЕсли;
				КонецЦикла;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Области["П024403"].Значение = Области["П024503"].Значение + Области["П024603"].Значение + Области["П024703"].Значение;
	Области["П022903"].Значение = Области["П023003"].Значение + Области["П023103"].Значение + Области["П023203"].Значение;
	Области["П021403"].Значение = Области["П021503"].Значение + Области["П021603"].Значение + Области["П021703"].Значение;
	Области["П020403"].Значение = Области["П020503"].Значение + Области["П021803"].Значение;
