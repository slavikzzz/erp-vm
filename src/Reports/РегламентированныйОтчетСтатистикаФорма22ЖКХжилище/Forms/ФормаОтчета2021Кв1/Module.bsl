...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = "0609226";
...
Процедура ДоступностьРазделов()
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <> 12 Тогда
		...
		ПолеТабличногоДокументаФормаОтчета.Области["П049003"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П049003"].Защита = Истина;
		
		ПолеТабличногоДокументаФормаОтчета.Области["П048904"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П048904"].Защита = Истина;
		
		ПолеТабличногоДокументаФормаОтчета.Области["П048905"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П048905"].Защита = Истина;
		
		ПолеТабличногоДокументаФормаОтчета.Области["П049005"].ЦветФона = Новый Цвет();
		ПолеТабличногоДокументаФормаОтчета.Области["П049005"].Защита = Истина;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет = ?(Авторасчет = Неопределено, Истина, Авторасчет);
	...
	Если СтруктураРеквизитовФормы.мСкопированаФорма <> Неопределено Тогда
		Если Не (НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) = СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета
			...
			Для НомСтр = 89 По 90 Цикл
				Для НомКол = 3 По 5 Цикл
					
					Если (НомСтр = 89 И НомКол = 3)
					 Или (НомСтр = 90 И НомКол = 4) Тогда
						Продолжить;
					КонецЕсли;
					
					Обл = ПолеТабличногоДокументаФормаОтчета.Области.Найти("П04" + НомСтр + Формат(НомКол, "ЧЦ=2; ЧВН="));
	...
	Для Каждого Показатель Из ПоказателиТаблПоле Цикл
		...
		Если (ЧастьПоказателя1 = "П00020")
			...
			ИмяНовогоПоказателя = ПрефиксИмениПоказателя
				+ "П0002021" + ЧастьПоказателя3	+ "_" + Формат(Число(ЧастьПоказателя2), "ЧГ=");
		ИначеЕсли (ЧастьПоказателя1 = "П00050") И (ЧастьПоказателя2 >= "01" И ЧастьПоказателя2 <= "11") Тогда
			Если ЧастьПоказателя3 >= "01" И ЧастьПоказателя3 <= "05" Тогда
				ИмяНовогоПоказателя = ПрефиксИмениПоказателя
					+ "П0005001" + ЧастьПоказателя3 + "_" + Формат(Число(ЧастьПоказателя2), "ЧГ=");
			ИначеЕсли ЧастьПоказателя3 >= "06_1" И ЧастьПоказателя3 <= "06_3" Тогда
				ИмяНовогоПоказателя = ПрефиксИмениПоказателя
					+ "П0005001" + Формат(Число(Прав(ЧастьПоказателя3, 1)) + 5, "ЧЦ=2; ЧВН=; ЧГ=")
					+ "_" + Формат(Число(ЧастьПоказателя2), "ЧГ=");
			ИначеЕсли ЧастьПоказателя3 >= "07" И ЧастьПоказателя3 <= "10" Тогда
				ИмяНовогоПоказателя = ПрефиксИмениПоказателя
					+ "П0005001" + Формат(Число(ЧастьПоказателя3) + 2, "ЧЦ=2; ЧВН=; ЧГ=")
					+ "_" + Формат(Число(ЧастьПоказателя2), "ЧГ=");
			КонецЕсли
		КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	СуммПоСтр = 0;
	Для Стр = 53 По 60 Цикл
		СуммПоСтр = СуммПоСтр + Области["П02" + Стр + "03"].Значение;
	КонецЦикла;
	Области["П025203"].Значение = СуммПоСтр;
	
	Области["П024403"].Значение = Области["П024503"].Значение
		+ Области["П024603"].Значение + Области["П024703"].Значение;
	
	СуммПоСтр = 0;
	Для Стр = 37 По 43 Цикл
		СуммПоСтр = СуммПоСтр + Области["П02" + Стр + "03"].Значение;
	КонецЦикла;
	Области["П023603"].Значение = СуммПоСтр;
	
	Области["П022903"].Значение = Области["П023003"].Значение
		+ Области["П023103"].Значение + Области["П023203"].Значение;
	
	СуммПоСтр = 0;
	Для Стр = 22 По 28 Цикл
		СуммПоСтр = СуммПоСтр + Области["П02" + Стр + "03"].Значение;
	КонецЦикла;
	Области["П022103"].Значение = СуммПоСтр;
	
	Области["П021403"].Значение = Области["П021503"].Значение
		+ Области["П021603"].Значение + Области["П021703"].Значение;
	
	СуммПоСтр = 0;
	Для Стр = 7 По 13 Цикл
		СуммПоСтр = СуммПоСтр + Области["П02" + Формат(Стр, "ЧЦ=2; ЧВН=") + "03"].Значение;
	КонецЦикла;
	Области["П020603"].Значение = СуммПоСтр;
	
	Области["П020403"].Значение = Области["П020503"].Значение
		+ Области["П021803"].Значение;
	
	Для Гр = 3 по 7 Цикл 
		Области["П04670" + Гр].Значение = Области["П04680" + Гр].Значение + Области["П04690" + Гр].Значение;
		Тот = 0;
		Для Инд = 75 по 87 Цикл 
			Если Инд = 80 Или Инд = 81 Или Инд = 84 Тогда 
				Продолжить;
			КонецЕсли;
			Тот = Тот + Области["П04" + Инд + "0" + Гр].Значение;
		КонецЦикла;
		Области["П04740" + Гр].Значение = Тот;
		Области["П04880" + Гр].Значение = Области["П04670" + Гр].Значение
			+ Области["П04730" + Гр].Значение
			+ Области["П04740" + Гр].Значение;
		КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П020403"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П020403"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П020603"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П020603"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П021403"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П021403"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П022103"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П022103"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П022903"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П022903"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П023603"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П023603"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П024403"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П024403"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П025203"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П025203"].Защита   = Не ОтклАвтоРасчетЗначение;
	...
	Для Н = 1 По 3 Цикл
		...
		Для Кол = 3 По 7 Цикл
			мФормаОтчета.Области["П04" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
			мФормаОтчета.Области["П04" + Стр + Формат(Кол, "ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
		КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 22-ЖКХ (жилище)");
	...
	ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_5"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 22-ЖКХ (жилище)");
