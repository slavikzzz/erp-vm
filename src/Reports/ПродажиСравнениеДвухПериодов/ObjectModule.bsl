...
Процедура УстановитьРасширенноеОформлениеСерии(Серия) Экспорт
	...
	Если СтрНайти(Серия.Текст, НСтр("ru = 'прошлого';
									|en = 'previous'")) Тогда
		Серия.Цвет = WebЦвета.ГолубойСоСтальнымОттенком;	
	ИначеЕсли СтрНайти(Серия.Текст, НСтр("ru = 'прошлый';
										|en = 'previous'")) Тогда
		Серия.Цвет = WebЦвета.СветлоСерый;
	ИначеЕсли СтрНайти(Серия.Текст, НСтр("ru = 'Выручка';
										|en = 'Revenue'"))
		ИЛИ СтрНайти(Серия.Текст, НСтр("ru = 'Количество';
										|en = 'Quantity'")) Тогда
		Серия.Цвет = WebЦвета.СинийСоСтальнымОттенком;
	ИначеЕсли СтрНайти(Серия.Текст, НСтр("ru = 'Спад';
										|en = 'Slippage'")) > 0 Тогда
		Серия.Цвет = WebЦвета.ЛососьСветлый;
	ИначеЕсли СтрНайти(Серия.Текст, НСтр("ru = 'Прирост ниже среднего';
										|en = 'Growth is below-average'")) > 0 Тогда
		Серия.Цвет = WebЦвета.СветлоЗолотистый;
	ИначеЕсли СтрНайти(Серия.Текст, НСтр("ru = 'Прирост';
										|en = 'Growth'")) > 0 Тогда
