...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы.
	СтруктураРеквизитовФормы.мВерсияФормы = "28/02/2014";
	// Код отчета, равен коду по ОКУД.
	СтруктураРеквизитовФормы.мКодОтчета = "0608007";
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли (Не (Область.Защита = Истина)) И СтрНайти(Область.Имя, "_") = 14 Тогда 
		...
		Если СтрНайти(Область.Имя, "П000001000101_") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ОКАТО");
			ОбластьДляВвода = Область;
			СмежнаяОбласть = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя,"П000001000101_","П000001000102_"));
			ИгнорироватьФлагВыборИзСписка = Истина;
		ИначеЕсли СтрНайти(Область.Имя, "П000001000102_") = 1 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ОКАТО");
			ОбластьДляВвода = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя,"П000001000102_","П000001000101_"));
		...
		ВвестиПоказатель(Элемент, ОбластьДляВвода, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, СмежнаяОбласть.Имя, ИгнорироватьФлагВыборИзСписка);
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	КонецЕсли;
	мФормаОтчета = Форма.ПолеТабличногоДокументаФормаОтчета.Области;
	
	Для Инд = 0 По 3 Цикл 
		Для Гр = 3 По 10 Цикл 
			мФормаОтчета["П0100" + (Инд+1) + Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = мФормаОтчета["П0102" + Инд + Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
																					+ мФормаОтчета["П0103" + Инд + Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
																					+ мФормаОтчета["П0104" + Инд + Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
	КонецЦикла;
	
	МассивСтр = Новый Массив;
	МассивСтр.Добавить(4);
	МассивСтр.Добавить(23);
	МассивСтр.Добавить(33);
	МассивСтр.Добавить(43);
	
	Для Каждого Стр Из МассивСтр Цикл
		Для Гр = 5 По 10 Цикл 
			мФормаОтчета["П010" + Формат(Стр, "ЧЦ=2; ЧВН=; ЧГ=") + Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=")].Значение = 
				мФормаОтчета["П010" + Формат(Стр+1, "ЧЦ=2; ЧВН=; ЧГ=") + Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
				+ мФормаОтчета["П010" + Формат(Стр+2, "ЧЦ=2; ЧВН=; ЧГ=") + Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=")].Значение
				+ мФормаОтчета["П010" + Формат(Стр+3, "ЧЦ=2; ЧВН=; ЧГ=") + Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=")].Значение;
		КонецЦикла;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-ФП");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1ФП");
	...
	Для Инд = 1 По 3 Цикл 
		...
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-ФП");
	КонецЦикла;
	...
	Если ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть4_3")) Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("s_okato", КоллекцияСписковВыбора["s_okato"]);
	Запрос.УстановитьПараметр("pril_okato_1", КоллекцияСписковВыбора["pril_okato_1"]);
	
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	s_okato.Код КАК Код,
	|	s_okato.Название КАК Название
	|ПОМЕСТИТЬ ВТ_s_okato
	|ИЗ
	|	&s_okato КАК s_okato
	|ИНДЕКСИРОВАТЬ ПО
	|	s_okato.Код
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	pril_okato_1.Код КАК Код
	|ПОМЕСТИТЬ ВТ_pril_okato_1
	|ИЗ &pril_okato_1 КАК pril_okato_1
	|ИНДЕКСИРОВАТЬ ПО
	|	pril_okato_1.Код
	|;
	|
	...
	|	s_okato.Название КАК Название
	|ИЗ
	|	ВТ_s_okato КАК s_okato
	|ГДЕ
	|	s_okato.Код В (ВЫБРАТЬ pril_okato_1.Код ИЗ ВТ_pril_okato_1 КАК pril_okato_1)";
	
	Результат = Запрос.ВыполнитьПакет();
	ОКАТО = Результат[2].Выгрузить();
	НоваяСтрокаТЗВыбора = ОКАТО.Вставить(0);
	НоваяСтрокаТЗВыбора.Код = "";
	НоваяСтрокаТЗВыбора.Название = "Пустое значение";
	
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ОКАТО", 512, , "Выбор кода ОКАТО", ОКАТО);
...
Функция СформироватьСтруктуруПараметров()
	...
	ОтчПериод  = Строка(12 + (Месяц(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания)/3));
	...
	Если ЗначениеЗаполнено(ТабДокумент["П000001000102"]) И Не ЗначениеЗаполнено(ТабДокумент["П000001000202"]) Тогда
		ПараметрыВыгрузки.Вставить("КодФЦП", ТабДокумент["П000001000102"]);
	ИначеЕсли Не ЗначениеЗаполнено(ТабДокумент["П000001000102"]) И ЗначениеЗаполнено(ТабДокумент["П000001000202"]) Тогда
		ПараметрыВыгрузки.Вставить("КодФЦП", ТабДокумент["П000001000202"]);
	ИначеЕсли ЗначениеЗаполнено(ТабДокумент["П000001000102"]) И ЗначениеЗаполнено(ТабДокумент["П000001000202"]) Тогда
		ПараметрыВыгрузки.Вставить("КодФЦП", ТабДокумент["П000001000202"]);
	Иначе
		ПараметрыВыгрузки.Вставить("КодФЦП", "");
	КонецЕсли;
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 27.04.2012 № 420";
	
	Стр = "001002003004005006007020021022023024025026030031032033034035036040041042043044045046";
	МассивСтрок = Новый Массив;
	
	Пока СтрДлина(Стр) > 0 Цикл 
		МассивСтрок.Добавить(Лев(Стр, 3));
		Стр = Сред(Стр, 4);
	КонецЦикла;
	
	МассивЗначений = Новый Массив(6);
	
	Для Каждого Стр Из МассивСтрок Цикл 
		Для Гр = 5 По 10 Цикл 
			МассивЗначений[Гр - 5] = Показатели["П01" + Стр + Формат(Гр, "ЧЦ=2; ЧВН=; ЧГ=")];
		КонецЦикла;
		
		Для Инд = 1 По 3 Цикл 
			ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, МассивЗначений[2*(Инд-1)]);
			ПраваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, МассивЗначений[2*(Инд-1) + 1]);
			ТекстФормулы = "по строке " + Стр + " гр." + (2*Инд + 3) + " >= гр." + (2*Инд + 4);
			Описание = "по строке " + Стр + " гр." + (2*Инд + 3) + " < гр." + (2*Инд + 4);
	...
	Попытка
		...
		Пока Истина Цикл
			Для Гр = 3 По 9 Цикл
				МассивЗначений[Гр-3] = МассивЗначений[Гр-3] + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00000100010"+Гр+"_"+Стр]);
			КонецЦикла;
			Стр = Стр + 1;
		КонецЦикла;
	Исключение
	...
	Для Гр = 3 По 9 Цикл 
		СоотвГр = ?(Гр=3, Гр, Гр+1);
		ЛеваяЧасть  = МассивЗначений[Гр-3];
		ПраваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П01003" + Формат(СоотвГр, "ЧЦ=2; ЧВН=; ЧГ=")]);
		ТекстФормулы = "сумма данных строк раздела II по гр. " + гр + "  = стр.003 гр." + СоотвГр;
		Описание = "сумма данных строк раздела II по гр. " + гр + "  <> стр.003 гр." + СоотвГр;
		Расшифровка = Строка(ЛеваяЧасть) + " = " + Строка(ПраваяЧасть);
