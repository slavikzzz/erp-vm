...
Процедура ПредставлениеУведомленияВыбор(Элемент, Область, СтандартнаяОбработка)
	Если Область.Имя = "ПересчитатьИтоговыеДанныеЦепочек" Тогда 
	...
	Если ЭтоОбластьНомер(Область.Имя) Тогда 
		...
		ОткрытьФорму("Отчет." + СтрРазделить(ИмяФормы, ".")[1] + ".Форма.ФормаВыборНомерУчастника502", ПараметрыВыборНомерУчастника,
...
Процедура ПересчитатьИтоговыеДанныеЦепочек()
	...
	Для Каждого Стр Из ЭтотОбъект["ДанныеМногостраничныхРазделов"].РаскрКонтрЛицМККосв Цикл 
		Если ЭтотОбъект["УИДТекущаяСтраница"] = Стр.Значение.УИД Тогда 
			Стр.Значение.ДоляУчКосвИт = ИтоговоеЗначение;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	ЭтотОбъект["ДанныеДопСтрок"].МнгСтр = ПоместитьВоВременноеХранилище(ДопСтроки, ЭтотОбъект["ДанныеДопСтрок"].МнгСтр);
...
Процедура ОсобаяОбработкаЭлементов(Путь, СтрЗагружаемоеЗначение, Страница) Экспорт 
	Если Путь = "Файл/Документ/СвКонтрЛицМК/СвКонтрЛиц/СвИОКонтрЛиц/НомерКонтрЛиц"
		Или Путь = "Файл/Документ/СвКонтрЛицМК/СвКонтрЛиц/СвРосОКонтрЛиц/НомерКонтрЛиц"
		Или Путь = "Файл/Документ/СвКонтрЛицМК/СвКонтрЛиц/СвФЛКонтрЛиц/НомерКонтрЛиц" Тогда 
		Страница.НомерКонтрЛиц = ЭтотОбъект["ОТЧ"].ПривестиЗначение(Прав(СтрЗагружаемоеЗначение.ЗначениеЭлемента, 5));
	ИначеЕсли Путь = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/СвУчПослКосв/СвИО/НомерУчПосл"
		Или Путь = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/СвУчПослКосв/СвРосОрг/НомерУчПосл"
		Или Путь = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/СвУчПослКосв/СведИСБезЮЛ/НомерУчПосл" Тогда 
		Страница.НомерУчПосл = ЭтотОбъект["ОТЧ"].ПривестиЗначение(Прав(СтрЗагружаемоеЗначение.ЗначениеЭлемента, 5));
	ИначеЕсли Путь = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/НомерПосл"
		Или Путь = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/СвУчПослКосв/СвИО/НомерПосл"
		Или Путь = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/СвУчПослКосв/СвРосОрг/НомерПосл"
		Или Путь = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/СвУчПослКосв/СведИСБезЮЛ/НомерПосл" Тогда 
...
Функция ПолучитьТаблицуОсобыхПолей()
	...
	НовСтр.ПутьXML = "Файл/Документ/СвКонтрЛицМК/СвКонтрЛиц/СвИОКонтрЛиц/НомерКонтрЛиц";
	
	НовСтр = ТаблицаОсобыхПолейВВыгрузке.Добавить();
	НовСтр.ПутьXML = "Файл/Документ/СвКонтрЛицМК/СвКонтрЛиц/СвРосОКонтрЛиц/НомерКонтрЛиц";
	
	НовСтр = ТаблицаОсобыхПолейВВыгрузке.Добавить();
	НовСтр.ПутьXML = "Файл/Документ/СвКонтрЛицМК/СвКонтрЛиц/СвФЛКонтрЛиц/НомерКонтрЛиц";
	
	НовСтр = ТаблицаОсобыхПолейВВыгрузке.Добавить();
	НовСтр.ПутьXML = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/СвУчПослКосв/СвИО/НомерУчПосл";
	
	НовСтр = ТаблицаОсобыхПолейВВыгрузке.Добавить();
	НовСтр.ПутьXML = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/СвУчПослКосв/СвИО/НомерПосл";
	
	НовСтр = ТаблицаОсобыхПолейВВыгрузке.Добавить();
	НовСтр.ПутьXML = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/НомерПосл";
	
	НовСтр = ТаблицаОсобыхПолейВВыгрузке.Добавить();
	НовСтр.ПутьXML = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/СвУчПослКосв/СвРосОрг/НомерУчПосл";
	
	НовСтр = ТаблицаОсобыхПолейВВыгрузке.Добавить();
	НовСтр.ПутьXML = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/СвУчПослКосв/СвРосОрг/НомерПосл";
	
	НовСтр = ТаблицаОсобыхПолейВВыгрузке.Добавить();
	НовСтр.ПутьXML = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/СвУчПослКосв/СведИСБезЮЛ/НомерУчПосл";
	
	НовСтр = ТаблицаОсобыхПолейВВыгрузке.Добавить();
	НовСтр.ПутьXML = "Файл/Документ/СвКонтрЛицМК/РаскрКонтрЛицМККосв/РаскрУчМК/СвУчПослКосв/СведИСБезЮЛ/НомерПосл";
