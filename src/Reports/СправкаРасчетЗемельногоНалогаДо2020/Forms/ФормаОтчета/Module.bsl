...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЭтотОбъект.ГруппировкиПроинициализированы Тогда 
...
Процедура НадписьПереходОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ОткрытьФорму("Отчет.СправкаРасчетЗемельногоНалога.Форма", ПараметрыФормы);
...
Процедура ТекстПриНеобходимостиАктуализацииОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	ПараметрыАктуализации = НовыйПараметрыАктуализацииОтчета();
	...
	Если СтрНайти(НавигационнаяСсылка, "ЗакрытиеМесяца") <> 0 Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ЗакрытиеМесяцаЗавершение", ЭтотОбъект);
		ОткрытьФорму("Обработка.ОперацииЗакрытияМесяца.Форма.ЗакрытиеМесяца", ПараметрыАктуализации, , , , , Оповещение);
...
&НаСервере
Функция НовыйПараметрыАктуализацииОтчета()
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = СтрШаблон(НСтр("ru = '(Устарела) Справка-расчет земельного налога%1';
									|en = '(Outdated) Detailed calculation of land tax value%1'"), ПредставлениеПериодаРасчета);
