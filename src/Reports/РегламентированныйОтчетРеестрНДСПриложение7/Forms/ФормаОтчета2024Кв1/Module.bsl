...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.р.НДС прилож.07 24к1",
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 8) = "П1000004"
		...
		ИЛИ Лев(Область.Имя, 8) = "П1200023" Тогда
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузкиРеестра, ДеревоВыгрузкиСведений)
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		УсловиеОбязательностиВыполнено
		= СтрРазделить("1010424,1010426,1010427,1010435,1010436", ",", Ложь).Найти(
		СокрЛП(Ветка.Данные["КодОперации"])) <> Неопределено;
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если (УсловиеОбязательностиВыполнено ИЛИ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000004"]))
			...
			Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
				...
				Если НЕ (ЗначениеЗаполнено(Ветка_П12000.Данные["П1200009"])
					...
					ИЛИ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200023"])) Тогда
					Продолжить;
				КонецЕсли;
				...
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200013"]) Тогда
					...
					СтатусПроверки = Ложь;
				КонецЕсли;
				ДлинаКодПортРазгр = СтрДлина(СокрЛП(Ветка_П12000.Данные["П1200023"]));
				Если ДлинаКодПортРазгр <> 2 Тогда
					ИмяПок = "П1200023" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
				...
				Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
					...
					Для каждого Эл Из ЭкзСведТрДокМ.Строки Цикл
						Если Эл.Код = "НомТрДокМ" Тогда
							...
							Заполн_НаимПортРазгр = ЗначениеЗаполнено(Ветка_П12000.Данные["П1200013"]);
							Заполн_КодПортРазгр  = ЗначениеЗаполнено(Ветка_П12000.Данные["П1200023"]);
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Для каждого Строка12 Из Строки12 Цикл
					...
					МасТекЗнач = Новый Массив;
					Если ЗначениеЗаполнено(Строка12.Данные.П1200013) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка12.Данные.П1200013));
					КонецЕсли;
					Если ЗначениеЗаполнено(Строка12.Данные.П1200023) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка12.Данные.П1200023));
					КонецЕсли;
					СтрокаДокумент.Параметры.П1200013 = СтрСоединить(МасТекЗнач, "; ");
				...
				Если ЗначениеЗаполнено(ОблОбъедСтрока1_Верх) И ЗначениеЗаполнено(ОблОбъедСтрока1_Низ)
					...
					Для НомерКолонки = 2 По 9 Цикл
						...
						ОблОбъед.Объединить();
					КонецЦикла;
					ОблОбъед = ПечФорма1.Область(ОблОбъедСтрока1_Верх, 15, ОблОбъедСтрока1_Низ, 15);
					ОблОбъед.Объединить();
