...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.р.НДС прилож.07 25к2",
		'20250401') Тогда
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	ИменаПодчиненныхГрупп.Добавить(СтруктураРеквизитовФормы.мИдГруппы130);
	...
	// Группа П13000.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П1300010");
	ИменаГраф.Добавить("П1300011");
	ИменаГраф.Добавить("П1300012");
	ИменаГраф.Добавить("П1300013");
	ИменаГраф.Добавить("П1300014");
	ИменаГраф.Добавить("П1300024");
	ИменаПодчиненныхГрупп = Новый Массив;
	СтруктураМногострочныхЧастей.Вставить(
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1300010", 2, ,
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 8) = "П1000005"
		ИЛИ Лев(Область.Имя, 8) = "П1000007"
		ИЛИ Лев(Область.Имя, 8) = "П1300024" Тогда
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы110
		ИЛИ ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы120 Тогда
		Подстрока = "номер";
		ТекстВопроса = НСтр("ru = 'Удалить выбранный номер?';
							|en = 'Удалить выбранный номер?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы130 Тогда
		Подстрока = "сведения об убытии товара";
		ТекстВопроса = НСтр("ru = 'Удалить выбранные сведения об убытии товара?';
							|en = 'Удалить выбранные сведения об убытии товара?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
				...
				Если ЗначениеЗаполнено(КодОперации) Тогда
					...
					Пока Форма.ТабличныйДокумент.Области.Найти("П1000009_" + Формат(Ном, "ЧГ=")) <> Неопределено Цикл
						СуммаНалБаз = СуммаНалБаз
							+ Форма.ТабличныйДокумент.Области["П1000009_" + Формат(Ном, "ЧГ=")].Значение;
						Ном = Ном + 1;
					КонецЦикла;
	...
	Если ИмяРаздела = "Реестр" Тогда
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если ЗначениеЗаполнено(КодОперации) Тогда
				ДанныеМнЧ_П10000 = Ветка.ДанныеМногострочныхЧастей.П10000.Строки;
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					СуммаНалБаз = СуммаНалБаз + Ветка_П10000.Данные["П1000009"];
				КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("НалБазаИтого", "КодОперации;П1000009");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрЖДН_1");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузкиРеестра, ДеревоВыгрузкиСведений)
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		УсловиеОбязательностиВыполнено = СтрРазделить("1010426,1010427,1010435,1010436",
			",", Ложь).Найти(СокрЛП(Ветка.Данные["КодОперации"])) <> Неопределено;
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000002"]) Тогда
				...
				+ НомПП + " не задан номер перевозочного документа", "Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ДлинаСтранОтпр = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000005"]));
			Если (УсловиеОбязательностиВыполнено ИЛИ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000005"]))
				И ДлинаСтранОтпр <> 2 Тогда
			...
			ДлинаСтанцОтпр = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000006"]));
			Если (УсловиеОбязательностиВыполнено ИЛИ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000006"]))
				И ДлинаСтанцОтпр <> 6 Тогда
				ИмяПок = "П1000006" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не" + ?(ДлинаСтанцОтпр = 0, "", "верно")
				+ " задан код станции отправления в маршруте",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ДлинаСтранНазн = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000007"]));
			Если (УсловиеОбязательностиВыполнено ИЛИ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000007"]))
				И ДлинаСтранНазн <> 2 Тогда
			...
			ДлинаСтанцНазн = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000008"]));
			Если (УсловиеОбязательностиВыполнено ИЛИ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000008"]))
				И ДлинаСтанцНазн <> 6 Тогда
				ИмяПок = "П1000008" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не" + ?(ДлинаСтанцНазн = 0, "", "верно")
				+ " задан код станции назначения в маршруте",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				Если ДлинаРегНомТД > 0
					...
					+ " неверно задан регистрационный номер декларации на товары (при вывозе)",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
				...
				ДлинаРегНомТД = СтрДлина(СокрЛП(Ветка_П12000.Данные["П1200004"]));
				Если ДлинаРегНомТД > 0
					И НЕ (ДлинаРегНомТД = 23 ИЛИ (ДлинаРегНомТД >= 25 И ДлинаРегНомТД <= 29)) Тогда
					ИмяПок = "П1200004" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " неверно задан регистрационный номер транзитной декларации или декларации на товары (при ввозе)",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			// 13000.
			ДанныеМнЧ_П13000 = Ветка_П10000.ДанныеМногострочныхЧастей.П13000.Строки;
			Для каждого Ветка_П13000 Из ДанныеМнЧ_П13000 Цикл
				
				НомерСтрокиП13000 = ДанныеМнЧ_П13000.Индекс(Ветка_П13000) + 1;
				НомерСтрокиП13000_Строка = Формат(НомерСтрокиП13000, "ЧГ=");
				
				Если НЕ (ЗначениеЗаполнено(Ветка_П13000.Данные["П1300010"])
					ИЛИ ЗначениеЗаполнено(Ветка_П13000.Данные["П1300011"])
					ИЛИ ЗначениеЗаполнено(Ветка_П13000.Данные["П1300012"])
					ИЛИ ЗначениеЗаполнено(Ветка_П13000.Данные["П1300013"])
					ИЛИ ЗначениеЗаполнено(Ветка_П13000.Данные["П1300014"])
					ИЛИ ЗначениеЗаполнено(Ветка_П13000.Данные["П1300024"])) Тогда
					Продолжить;
				КонецЕсли;
				
				ВидТрДокМ = СокрЛП(Ветка_П13000.Данные["П1300010"]);
				
				ДлинаВидТрДокМ = СтрДлина(ВидТрДокМ);
				Если ДлинаВидТрДокМ <> 2 Тогда
					ИмяПок = "П1300010" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП13000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					...
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П13000.Данные["П1300012"]) Тогда
					ИмяПок = "П1300012" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП13000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					...
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П13000.Данные["П1300013"]) Тогда
					ИмяПок = "П1300013" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП13000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование судна",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П13000.Данные["П1300014"]) Тогда
					ИмяПок = "П1300014" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП13000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование порта разгрузки судна",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ДлинаКодПортРазгр = СтрДлина(СокрЛП(Ветка_П13000.Данные["П1300024"]));
				Если ДлинаКодПортРазгр <> 2 Тогда
					ИмяПок = "П1300024" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП13000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не" + ?(ДлинаКодПортРазгр = 0, "", "верно")
					+ " задан код страны порта разгрузки судна",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрЖДН_1 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрЖДН_1");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрЖДН_1 = СкопироватьУзел(УзелРодитель.Родитель, РеестрЖДН_1);
		Для каждого Эл Из ЭкзРеестрЖДН_1.Строки Цикл
		...
		СведТрДок = ПолучитьПодчиненныйЭлемент(ЭкзРеестрЖДН_1, "СведТрДок");
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведТрДок = СкопироватьУзел(ЭкзРеестрЖДН_1, СведТрДок, ЭкзРеестрЖДН_1.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСведТрДок.Строки Цикл
					Если Эл.Строки.Количество() = 0
						И Эл.Код <> "РегНомДТ" И Эл.Код <> "РегНомТД_ДТ" Тогда
						ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
					КонецЕсли;
				КонецЦикла;
				
				// 11000.
				РегНомДТ = ПолучитьПодчиненныйЭлемент(ЭкзСведТрДок, "РегНомДТ");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					ЭкзРегНомДТ = СкопироватьУзел(ЭкзСведТрДок, РегНомДТ, ЭкзСведТрДок.Строки.Количество() - 1);
					ВывестиПоказательВXML(ЭкзРегНомДТ, Ветка_П11000.Данные[ЭкзРегНомДТ.Ключ]);
				КонецЦикла;
				
				// 12000.
				РегНомТД_ДТ = ПолучитьПодчиненныйЭлемент(ЭкзСведТрДок, "РегНомТД_ДТ");
				ДанныеМнЧ_П12000 = Ветка_П10000.ДанныеМногострочныхЧастей.П12000.Строки;
				Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
					ЭкзРегНомТД_ДТ = СкопироватьУзел(ЭкзСведТрДок, РегНомТД_ДТ, ЭкзСведТрДок.Строки.Количество() - 1);
					ВывестиПоказательВXML(ЭкзРегНомТД_ДТ, Ветка_П12000.Данные[ЭкзРегНомТД_ДТ.Ключ]);
				КонецЦикла;
				...
				Для каждого Ветка_П13000 Из ДанныеМнЧ_П13000 Цикл
					...
					Для каждого Эл Из ЭкзСведТрДокМ.Строки Цикл
						Если Эл.Код = "НомТрДокМ" Тогда
							ТекЗнач = СокрЛП(Ветка_П13000.Данные[Эл.Ключ]);
							Заполн_ВидТрДокМ     = ЗначениеЗаполнено(Ветка_П13000.Данные["П1300010"]);
							Заполн_ДатаТрДокМ    = ЗначениеЗаполнено(Ветка_П13000.Данные["П1300012"]);
							Заполн_НаимСудн      = ЗначениеЗаполнено(Ветка_П13000.Данные["П1300013"]);
							Заполн_НаимПортРазгр = ЗначениеЗаполнено(Ветка_П13000.Данные["П1300014"]);
							Заполн_КодПортРазгр  = ЗначениеЗаполнено(Ветка_П13000.Данные["П1300024"]);
						...
						Иначе
							ВывестиПоказательВXML(Эл, Ветка_П13000.Данные[Эл.Ключ]);
						КонецЕсли;
					КонецЦикла;
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Для каждого Строка12 Из Строки12 Цикл
					Если ЗначениеЗаполнено(Строка12.Данные.П1200004) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка12.Данные.П1200004));
					КонецЕсли;
				КонецЦикла;
				СтрокаДокумент.Параметры.П1200004 = СтрСоединить(МасТекЗнач, "; ");
				
				// П13000.
				//
				Строки13 = Строка1.ДанныеМногострочныхЧастей.П13000.Строки;
				
				Для каждого Строка13 Из Строки13 Цикл
					
					Если Строки13.Индекс(Строка13) > 0 Тогда
						Для ИндПарам = 0 По СтрокаДокумент.Параметры.Количество() - 1 Цикл
							СтрокаДокумент.Параметры[ИндПарам] = Неопределено;
						КонецЦикла;
					КонецЕсли;
					
					СтрокаДокумент.Параметры.Заполнить(Строка13.Данные);
					
					// Особая обработка для графы 14.
					МасТекЗнач = Новый Массив;
					Если ЗначениеЗаполнено(Строка13.Данные.П1300014) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка13.Данные.П1300014));
					КонецЕсли;
					Если ЗначениеЗаполнено(Строка13.Данные.П1300024) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка13.Данные.П1300024));
					КонецЕсли;
					СтрокаДокумент.Параметры.П1300014 = СтрСоединить(МасТекЗнач, "; ");
...
Процедура Проверить(Команда)
	...
	|от 15.11.2024 № ЕД-7-15/1036@, будет реализована
