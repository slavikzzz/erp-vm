...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр р.НДС прилож.07 21к2") Тогда
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	// Группа П12000.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П1200009");
	ИменаГраф.Добавить("П1200010");
	ИменаГраф.Добавить("П1200011");
	ИменаГраф.Добавить("П1200012");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П1200009", 2, ,
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Лев(Область.Имя, 8) = "П1000004"
		ИЛИ Лев(Область.Имя, 8) = "П1000006"
		ИЛИ Лев(Область.Имя, 8) = "П1200024" Тогда
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы120 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Удалить выбранные сведения об убытии товара морским транспортом?';
							|en = 'Удалить выбранные сведения об убытии товара морским транспортом?'");
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ИмяРаздела = "Реестр" Тогда
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если ЗначениеЗаполнено(КодОперации) Тогда
				ДанныеМнЧ_П10000 = Ветка.ДанныеМногострочныхЧастей.П10000.Строки;
				Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
					СуммаНалБаз = СуммаНалБаз + Ветка_П10000.Данные["П1000008"];
				КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("НалБазаИтого", "КодОперации;П1000008");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "РеестрЖДН_7");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRZDNNDS"));
	
	ПараметрыВыгрузки.Вставить("ИдСведФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "KO_RRZDNNDS.2"));
	...
	Для каждого Ветка Из ДанныеРеестра.Строки Цикл
		...
		Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
			...
			Если НЕ ЗначениеЗаполнено(Ветка_П10000.Данные["П1000002"]) Тогда
				...
				+ НомПП + " не задан номер транспортного, товаросопроводительного и (или) иного документа",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			ДлинаСтранОтпр = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000004"]));
			Если ДлинаСтранОтпр <> 2 Тогда
				ИмяПок = "П1000004" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не" + ?(ДлинаСтранОтпр = 0, "", "верно")
				+ " задана страна отправления в маршруте",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			ДлинаСтанцОтпр = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000005"]));
			Если ДлинаСтанцОтпр <> 6 Тогда
				ИмяПок = "П1000005" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не" + ?(ДлинаСтанцОтпр = 0, "", "верно")
				+ " задана станция отправления в маршруте",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			ДлинаСтранНазн = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000006"]));
			Если ДлинаСтранНазн <> 2 Тогда
				ИмяПок = "П1000006" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не" + ?(ДлинаСтранНазн = 0, "", "верно")
				+ " задана страна назначения в маршруте",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			ДлинаСтанцНазн = СтрДлина(СокрЛП(Ветка_П10000.Данные["П1000007"]));
			Если ДлинаСтанцНазн <> 6 Тогда
				ИмяПок = "П1000007" + "_" + НомерСтрокиП10000_Строка;
				СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке "
				+ НомПП + " не" + ?(ДлинаСтанцНазн = 0, "", "верно")
				+ " задана станция назначения в маршруте",
				"Реестр", ИмяПок, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
				...
				ДлинаРегНомТД = СтрДлина(СокрЛП(Ветка_П11000.Данные["П1100003"]));
				Если ДлинаРегНомТД > 0
					И НЕ (ДлинаРегНомТД = 23 ИЛИ (ДлинаРегНомТД >= 25 И ДлинаРегНомТД <= 29)) Тогда
					ИмяПок = "П1100003" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП11000_Строка;
			...
			ЕстьХотяБыОднаЗапись_П12000 = Ложь;
			Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
				...
				Если НЕ (ЗначениеЗаполнено(Ветка_П12000.Данные["П1200009"])
					ИЛИ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200010"])
					ИЛИ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200011"])
					ИЛИ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200012"])
					ИЛИ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200013"])
					ИЛИ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200014"])
					ИЛИ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200024"])) Тогда
					Продолжить;
				КонецЕсли;
				
				ЕстьХотяБыОднаЗапись_П12000 = Истина;
				
				ВидТрДокМ = СокрЛП(Ветка_П12000.Данные["П1200009"]);
				
				ДлинаВидТрДокМ = СтрДлина(ВидТрДокМ);
				Если ДлинаВидТрДокМ <> 2 Тогда
					ИмяПок = "П1200009" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не" + ?(ДлинаВидТрДокМ = 0, "", "верно")
					+ " задан вид транспортного, товаросопроводительного и (или) иного документа, указанного в соответствии с абзацем вторым подпункта 3 пункта 3.7 статьи 165 Налогового кодекса Российской Федерации",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200011"]) Тогда
					ИмяПок = "П1200011" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задана дата транспортного, товаросопроводительного и (или) иного документа, указанного в соответствии с абзацем вторым подпункта 3 пункта 3.7 статьи 165 Налогового кодекса Российской Федерации",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200012"]) Тогда
					ИмяПок = "П1200012" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование судна",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200014"]) Тогда
					ИмяПок = "П1200014" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не задано наименование порта разгрузки",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				ДлинаКодПортРазгр = СтрДлина(СокрЛП(Ветка_П12000.Данные["П1200024"]));
				Если ДлинаКодПортРазгр <> 2 Тогда
					ИмяПок = "П1200024" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " не" + ?(ДлинаКодПортРазгр = 0, "", "верно")
					+ " задан код страны порта разгрузки",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				Если ВидТрДокМ = "06" Тогда
					Если НЕ ЗначениеЗаполнено(Ветка_П12000.Данные["П1200013"]) Тогда
						ИмяПок = "П1200013" + "_" + НомерСтрокиП10000_Строка + "_" + НомерСтрокиП12000_Строка;
						СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
						+ " не задана дата отметки российского таможенного органа ""Погрузка разрешена"" (при вывозе)",
						"Реестр", ИмяПок, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
				КонецЕсли;
			...
			КонецЦикла;
			Если Ветка.Данные["КодОперации"] = "1010435" ИЛИ Ветка.Данные["КодОперации"] = "1010436" Тогда
				Если НЕ ЕстьХотяБыОднаЗапись_П12000 Тогда
					ИмяПок = "П1200009" + "_" + НомерСтрокиП10000_Строка + "_1";
					СообщитьВТС("Страница " + НомерСтраницы_Строка + ": в строке " + НомПП
					+ " должны быть заданы сведения о транспортном, товаросопроводительном, и (или) ином документе, подтверждающем факт убытия товара морским (речным) транспортом, если код операции принимает значения 1010435, 1010436",
					"Реестр", ИмяПок, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	РеестрЖДН_7 = ПолучитьПодчиненныйЭлемент(УзелРодитель.Родитель, "РеестрЖДН_7");
	...
	Для каждого ЭлемСтруктуры Из УпорядоченныеДанныеДерева Цикл
		...
		ЭкзРеестрЖДН_7 = СкопироватьУзел(УзелРодитель.Родитель, РеестрЖДН_7);
		Для каждого Эл Из ЭкзРеестрЖДН_7.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				ВывестиПоказательВXML(Эл, ПерваяВетка.Данные[Эл.Ключ]);
			КонецЕсли;
		КонецЦикла;
		
		// Узел "СведТрДок" имеется только в схеме выгрузки "Сведений".
		// Если он не найден, значит, сейчас формируются данные "Реестра"
		// (для выгрузки первого из 2-х файлов).
		СведТрДок = ПолучитьПодчиненныйЭлемент(ЭкзРеестрЖДН_7, "СведТрДок");
		Если СведТрДок = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Для каждого ИндексВетки Из МассивУпорядоченныхДанных Цикл
			...
			Для каждого Ветка_П10000 Из ДанныеМнЧ_П10000 Цикл
				...
				ЭкзСведТрДок = СкопироватьУзел(ЭкзРеестрЖДН_7, СведТрДок, ЭкзРеестрЖДН_7.Строки.Количество() - 1);
				Для каждого Эл Из ЭкзСведТрДок.Строки Цикл
					Если Эл.Строки.Количество() = 0
						И Эл.Код <> "РегНомТД" Тогда
						ВывестиПоказательВXML(Эл, Ветка_П10000.Данные[Эл.Ключ]);
					КонецЕсли;
				КонецЦикла;
				
				// 11000.
				РегНомТД = ПолучитьПодчиненныйЭлемент(ЭкзСведТрДок, "РегНомТД");
				ДанныеМнЧ_П11000 = Ветка_П10000.ДанныеМногострочныхЧастей.П11000.Строки;
				Для каждого Ветка_П11000 Из ДанныеМнЧ_П11000 Цикл
					ЭкзРегНомТД = СкопироватьУзел(ЭкзСведТрДок, РегНомТД, ЭкзСведТрДок.Строки.Количество() - 1);
					ВывестиПоказательВXML(ЭкзРегНомТД, Ветка_П11000.Данные[ЭкзРегНомТД.Ключ]);
				КонецЦикла;
				
				// 12000.
				СведТрДокМ = ПолучитьПодчиненныйЭлемент(ЭкзСведТрДок, "СведТрДокМ");
				ДанныеМнЧ_П12000 = Ветка_П10000.ДанныеМногострочныхЧастей.П12000.Строки;
				Для каждого Ветка_П12000 Из ДанныеМнЧ_П12000 Цикл
					
					ЭкзСведТрДокМ = СкопироватьУзел(ЭкзСведТрДок, СведТрДокМ, ЭкзСведТрДок.Строки.Количество() - 1);
					Для каждого Эл Из ЭкзСведТрДокМ.Строки Цикл
						Если Эл.Код = "НомТрДокМ" Тогда
							ТекЗнач = СокрЛП(Ветка_П12000.Данные[Эл.Ключ]);
							Заполн_ВидТрДокМ     = ЗначениеЗаполнено(Ветка_П12000.Данные["П1200009"]);
							Заполн_ДатаТрДокМ    = ЗначениеЗаполнено(Ветка_П12000.Данные["П1200011"]);
							Заполн_НаимСудн      = ЗначениеЗаполнено(Ветка_П12000.Данные["П1200012"]);
							Заполн_НаимПортРазгр = ЗначениеЗаполнено(Ветка_П12000.Данные["П1200014"]);
							Заполн_КодПортРазгр  = ЗначениеЗаполнено(Ветка_П12000.Данные["П1200024"]);
							ВывестиПоказательВXML(Эл, ?(НЕ ЗначениеЗаполнено(ТекЗнач) И (Заполн_ВидТрДокМ
								ИЛИ Заполн_ДатаТрДокМ ИЛИ Заполн_НаимСудн ИЛИ Заполн_НаимПортРазгр
								ИЛИ Заполн_КодПортРазгр), "б/н", ТекЗнач));
						Иначе
...
Функция ВыгрузкаРеестра(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_7");
...
Функция ВыгрузкаСведений(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("АдресФайлаВыгрузки", АдресВоВремХранилище);
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "РеестрНДС_7_Сведения");
	РезультатВыгрузки.Вставить("КодировкаФайлаВыгрузки", КодировкаФайлаВыгрузки);
		...
		Для каждого Страница Из ДеревоДанныхРаздела.Строки Цикл
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Для каждого Строка11 Из Строки11 Цикл
					Если ЗначениеЗаполнено(Строка11.Данные.П1100003) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка11.Данные.П1100003));
					КонецЕсли;
				КонецЦикла;
				СтрокаДокумент.Параметры.П1100003 = СтрСоединить(МасТекЗнач, "; ");
				...
				Для каждого Строка12 Из Строки12 Цикл
					...
					// Особая обработка для графы 14.
					МасТекЗнач = Новый Массив;
					Если ЗначениеЗаполнено(Строка12.Данные.П1200014) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка12.Данные.П1200014));
					КонецЕсли;
					Если ЗначениеЗаполнено(Строка12.Данные.П1200024) Тогда
						МасТекЗнач.Добавить(СокрЛП(Строка12.Данные.П1200024));
					КонецЕсли;
					СтрокаДокумент.Параметры.П1200014 = СтрСоединить(МасТекЗнач, "; ");
				...
				Если ЗначениеЗаполнено(ОблОбъедСтрока1_Верх) И ЗначениеЗаполнено(ОблОбъедСтрока1_Низ)
					...
					Для НомерКолонки = 2 По 9 Цикл
						...
						ОблОбъед.Объединить();
					КонецЦикла;
					ОблОбъед = ПечФорма1.Область(ОблОбъедСтрока1_Верх, 16, ОблОбъедСтрока1_Низ, 16);
					ОблОбъед.Объединить();
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	СтруктураРезультата.Вставить("НаименованиеОтчета"	, "Реестр НДС Приложение 7");
...
#Область ОбщиеДанные
	...
	ДанныеКС.Вставить("ГрафаНалоговойБазы"		, "08");
...
&НаСервере
Процедура ПроверкаКС_7_1_1()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '7.1.1';
													|en = '7.1.1'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 7';
														|en = 'Реестр 7'"));
...
&НаСервере
Процедура ПроверкаКС_7_1_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '7.1.2';
													|en = '7.1.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 7';
														|en = 'Реестр 7'"));
...
&НаСервере
Процедура ПроверкаКС_7_1_3()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '7.1.3';
													|en = '7.1.3'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 7';
														|en = 'Реестр 7'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны отправления в гр.4 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны отправления в гр.4 Реестра должен соответствовать справочнику ОКСМ'"));
	...
	Настройки.Вставить("ИмяПоказателя", "П1000004_");
	ПроверкаКС_7_1_4_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_7_1_4()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '7.1.4';
													|en = '7.1.4'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 7';
														|en = 'Реестр 7'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны назначения в гр.6 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны назначения в гр.6 Реестра должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Код страны %1 соответствует справочнику ОКСМ';
													|en = 'Код страны %1 соответствует справочнику ОКСМ'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный код страны';
														|en = 'Ошибочный код страны'"));
	...
	Настройки.Вставить("ИмяПоказателя", "П1000006_");
	ПроверкаКС_7_1_4_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_7_1_4_код(Настройки)
...
&НаСервере
Процедура ПроверкаКС_7_1_5()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '7.1.5';
													|en = '7.1.5'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 7';
														|en = 'Реестр 7'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Код страны порта разгрузки в гр.14 Реестра должен соответствовать справочнику ОКСМ';
			|en = 'Код страны порта разгрузки в гр.14 Реестра должен соответствовать справочнику ОКСМ'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru = 'Код страны %1 соответствует справочнику ОКСМ';
													|en = 'Код страны %1 соответствует справочнику ОКСМ'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Ошибочный код страны';
														|en = 'Ошибочный код страны'"));
	...
	Настройки.Вставить("ИмяПоказателя", "П1200024_");
	ПроверкаКС_7_1_5_код(Настройки);
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_7_1_5_код(Настройки)
	...
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			Для Счетчик2 = 1 По СтраницаРеестра["КолвоСтрокП12000_" + XMLСтрока(Счетчик)]  Цикл
				...
				Показатели.Р01Г001.Дополнение = Дополнение;
				ЗаполнитьИРасшифроватьПоказатели(Показатели, СтраницаРеестра, Счетчик, Счетчик2);
...
&НаСервере
Процедура ПроверкаКС_7_1_6()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '7.1.6';
													|en = '7.1.6'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 7';
														|en = 'Реестр 7'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
													|en = 'Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru = 'Если код операции равен 1010435, 1010436, то должны быть указаны вид, номер и дата ';
			|en = 'Если код операции равен 1010435, 1010436, то должны быть указаны вид, номер и дата '") +
		НСтр("ru = 'товаросопроводительного документа, подтверждающего факт убытия товара морским, речным транспортом';
			|en = 'товаросопроводительного документа, подтверждающего факт убытия товара морским, речным транспортом'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
		НСтр("ru = 'Вид %1, номер %2, дата %3 документа указаны, когда %4 = 1010435, 1010436';
			|en = 'Вид %1, номер %2, дата %3 документа указаны, когда %4 = 1010435, 1010436'"));
	Заголовки.Вставить("ОписаниеОшибки"		, НСтр("ru = 'Не указаны реквизиты товаросопроводительного документа';
														|en = 'Не указаны реквизиты товаросопроводительного документа'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru = 'Страница %1, Строка %2';
													|en = 'Страница %1, Строка %2'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01С001", "КодОперации");
	ИменаПоказателей.Вставить("Р01Г001", "П1200009_");
	ИменаПоказателей.Вставить("Р01Г002", "П1200010_");
	ИменаПоказателей.Вставить("Р01Г003", "П1200011_");
	
	СформироватьПоказатели(Показатели, ИменаПоказателей);
	
	Для каждого СтраницаРеестра Из ДанныеКС.Реестр Цикл
		ЗаполнитьПоказатель(Показатели.Р01С001, СтраницаРеестра);
		КодСовпадает = ЗначениеЗаполнено(Показатели.Р01С001.Значение) 
			И НЕ СтрНайти("1010435,1010436", Показатели.Р01С001.Значение) = 0;
		...
		Для Счетчик = 1 По СтраницаРеестра.КолвоСтрокП10000 Цикл
			Для Счетчик2 = 1 По СтраницаРеестра["КолвоСтрокП12000_" + XMLСтрока(Счетчик)]  Цикл
				...
				Дополнение = ПодставитьПараметрыВСтроку(Настройки.ШаблонДопСведений, СтраницаРеестра.Страница,
					XMLСтрока(Счетчик) + "-" + XMLСтрока(Счетчик2));
				Дополнение2 = ПодставитьПараметрыВСтроку(Настройки.ШаблонДополнения, СтраницаРеестра.Страница);
				Показатели.Р01С001.Дополнение = Дополнение2;
				Показатели.Р01Г001.Дополнение = Дополнение;
				...
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01Г002, СтраницаРеестра, Счетчик, Счетчик2);
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01Г003, СтраницаРеестра, Счетчик, Счетчик2);
		
				Настройки.ЭтоОшибка = КодСовпадает И НЕ (ЗначениеЗаполнено(Показатели.Р01Г001.Значение) И 
					ЗначениеЗаполнено(Показатели.Р01Г002.Значение) И ЗначениеЗаполнено(Показатели.Р01Г003.Значение));
				Настройки.Расшифровка = ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, Показатели.Р01Г001.ЗначениеСтр,
					Показатели.Р01Г002.ЗначениеСтр, Показатели.Р01Г003.ЗначениеСтр, Показатели.Р01С001.ЗначениеСтр);
				Настройки.ДопСведения = Дополнение;
				ДополнитьСтруктуруОшибок(Настройки);
...
&НаСервере
Процедура ПроверкаКС_7_2_1а()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '7.2.1.а';
													|en = '7.2.1.а'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 7';
														|en = 'Реестр 7'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_7_2_1б()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '7.2.1.б';
													|en = '7.2.1.б'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 7';
														|en = 'Реестр 7'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
...
&НаСервере
Процедура ПроверкаКС_7_2_2()
	
	Перем Показатели;
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("НомерКС"			, НСтр("ru = '7.2.2';
													|en = '7.2.2'"));
	Заголовки.Вставить("Раздел"				, НСтр("ru = 'Реестр 7';
														|en = 'Реестр 7'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru = 'Ошибка ввода';
