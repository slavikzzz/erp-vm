	...
	Если Ответ = Неопределено Тогда
		...
		Если ПотокЗаписи = Неопределено Тогда	
			...
			Ответ.УстановитьТелоИзСтроки(СтрШаблон(НСтр("ru = 'Не удалось открыть поток записи файла. Идентификатор запроса %1';
														|en = 'Cannot open a write stream. Query ID: %1'"), 
				ПараметрыЗапроса.ID));
			Возврат Ответ;
		...
		Если ЗначениеЗаполнено(ПолученныйДиапазон) Тогда
			Если ПотокЗаписи.Размер() < ПолученныйДиапазон.Начало Тогда
				...
				Ответ.УстановитьТелоИзСтроки(СтрШаблон(НСтр("ru = 'Размер файла %1 меньше ожидаемого %2. Идентификатор запроса %3';
															|en = 'The %1 file size is less than the expected size %2. Query ID is %3'"),
					ПотокЗаписи.Размер(),
		...
		Иначе
			...
			Если ОписаниеДанных.УдалитьФайлДанных Тогда
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(НСтр("ru = 'ПередачаДанных';
													|en = 'DataTransfer'", Метаданные.ОсновнойЯзык.КодЯзыка), УровеньЖурналаРегистрации.Ошибка,,, ПередачаДанныхКлиентСервер.ПодробныйТекстОшибки(ИнформацияОбОшибке()));
...
// 	ИдентификаторХранилища - Строка - имя объектного хранилища.
// 	ИдентификаторДанных - Строка - идентификатор объекта.
...
//
Функция ОписаниеДанныхЛогическогоХранилища(МенеджерХранилища, Знач ИдентификаторХранилища, Знач ИдентификаторДанных, Ответ)
	...
	Попытка
		...
		Описание = МенеджерХранилища.Описание(ИдентификаторХранилища, ИдентификаторДанных);
